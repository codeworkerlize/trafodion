...
if dist_type == 'Ubuntu':
  additional_javaoptions=" -Djdk.net.URLClassPath.disableClassPathURLCheck=true "
else:
  additional_javaoptions=" "
...
def shell_call(cmd):
    ...
    hpipe = subprocess.Popen(cmd, stdin=subprocess.PIPE,
                             ...
                             bufsize=1, close_fds=ON_POSIX, shell=True, executable='/bin/bash')
...
def get_hadoop_component_ver(distro, component):
    ...
    if 'HBASE_CNF_DIR' in os.environ and "local_hadoop" in os.environ['HBASE_CNF_DIR']:
        ...
        rpm_ver = shell_call(gvars.my_ROOT + '/hadoop_ver.sh ' + component)
        #if os.path.exists("/opt/cloudera/parcels/CDH"):
        #  rpm_ver = shell_call(gvars.my_ROOT + '/hadoop_ver.sh ' + component)
        #else:
        #  rpm_ver = subprocess.Popen(["rpm", "-q", "--qf", '%{VERSION}', component], stdout=subprocess.PIPE).communicate()[0]
...
if not ArgList._no_maven:
    ...
    output = shell_call(gvars.my_EXPORT_CMD + '; free -m; mvn test -Dmaven.repo.local=' +
                        ArgList._maven_local_repo +  additional_javaoptions + ' -Dtest=' + ArgList._tests)
    ...
    gvars.my_RUN_CMD = (os.path.join(ArgList._javahome, 'bin/java') + ' -cp ' + myclasspath +
                        ' ' + myoptions + additional_javaoptions + ' -Duser.timezone=GMT -Dtrafjdbc.properties=' +
