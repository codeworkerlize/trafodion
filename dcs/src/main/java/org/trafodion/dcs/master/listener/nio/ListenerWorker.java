...
import org.trafodion.dcs.master.listener.RequestKeepAliveCheck;
import org.trafodion.dcs.master.listener.RequestUnknown;
import org.trafodion.dcs.util.GetJavaProperty;

public class ListenerWorker extends Thread {
    private static  final Logger LOG = LoggerFactory.getLogger(ListenerWorker.class);
    private List<DataEvent> queue = new LinkedList<DataEvent>();
    private RequestKeepAliveCheck requestKeepAliveCheck = null;
    ...
    static
    {
        try {
            ...
            LOG.error("T2 Driver Class not found in CLASSPATH : <{}>.", e.getMessage(), e);
            System.exit(-1);
        }
    }
    ListenerWorker(ConfigReader configReader) {
        requestUnknown = new RequestUnknown(configReader);
        requestKeepAliveCheck = new RequestKeepAliveCheck(configReader);
    ...
    public void run() {
        ...
        while(true) {
            try {
                ...
                if (clientData.getHeader().getOperationId()
                        == ListenerConstants.DCS_MASTER_KEEPALIVECHECK) {
                    requestKeepAliveCheck.processRequest(clientData);
                } else {
                    requestUnknown.processRequest(clientData);
                }
                // Return to sender
