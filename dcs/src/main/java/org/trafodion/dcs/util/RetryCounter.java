...
public class RetryCounter {
    ...
    public void sleepUntilNextRetry() throws InterruptedException {
        ...
        if (LOG.isInfoEnabled())
            LOG.info("Sleeping <{}> ms before retry #<{}>...", sleepTime, attempts);
    ...
    public boolean shouldRetryInnerMinutes() {
        LOG.debug("retryInterval = <{}>. queue size = <{}>. max retries = <{}>. ", retryInterval, queue.size(), getMaxRetries());
        if (retryInterval == 0) {
            return true;
        }
        if (queue.size() < getMaxRetries()) {
            ...
            if (delta < 0) {
                LOG.error("Reject!!! Attempt to restart mxosrvr in <{}> minutes...it can't restart mxosrvr larger than <{}> times in <{}> minutes",
                        TimeUnit.MILLISECONDS.toMinutes(currentTime - firstRetryTime), getMaxRetries(), this.retryInterval);
                return false;
            }
            queue.poll();
    ...
    private long calcDelta(long realMillis) {
        LOG.debug("realMillis = <{}>. retryInterval = <{}>.", realMillis,
