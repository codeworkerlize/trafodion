...
public final class ServerManager implements Callable {
    ...
    public ServerManager(Configuration conf,ZkClient zkc,DcsNetworkConfiguration netConf,
        ...
        this.parentDcsZnode = this.conf.get(Constants.ZOOKEEPER_ZNODE_PARENT,Constants.DEFAULT_ZOOKEEPER_ZNODE_DCS_PARENT);           
    ...
    private void featureCheck() throws IOException{
        String value = GetJavaProperty.getEnv("TRAF_HOME");
    ...
    private void getCertificate() throws IOException {
        String path;
        CLUSTERNAME = GetJavaProperty.getEnv("CLUSTERNAME");
        if(CLUSTERNAME == null || CLUSTERNAME.length() == 0 ) {
            path = TRAF_HOME + "/sqcert/server.crt";
        }
        else {
            HOME = GetJavaProperty.getEnv("HOME");
    ...
    private void getMaster(){
        ...
        while(! found){
            try {
                Stat stat = zkc.exists(parentDcsZnode + Constants.DEFAULT_ZOOKEEPER_ZNODE_MASTER,false);
                if(stat != null) {
                    List<String> nodes = zkc.getChildren(parentDcsZnode + Constants.DEFAULT_ZOOKEEPER_ZNODE_MASTER,null);
                    if( ! nodes.isEmpty()) {
    ...
    private void registerInRunning(int instance) {
        String znode = parentDcsZnode + Constants.DEFAULT_ZOOKEEPER_ZNODE_SERVERS_RUNNING + "/" + hostName + ":" + instance + ":" + infoPort + ":" + System.currentTimeMillis(); 
        try {
    ...
    private String getPortMap() {
        ...
        String nodeRegisteredPath = parentDcsZnode + Constants.DEFAULT_ZOOKEEPER_ZNODE_SERVERS_REGISTERED;
        try {
