...
//  webAppContext.setWar(appDir + "/" + name);
    ...
    if(isSecurity) {
      if (LOG.isInfoEnabled()) {
        ...
        sh.setUserRealm(new HashUserRealm("Trafodion WebUI", GetJavaProperty.getProperty(Constants.DCS_CONF_DIR) + "/realm.properties"));
  ...
  protected void addDefaultApps(ContextHandlerCollection parent,
    ...
    String logDir = GetJavaProperty.getProperty(Constants.DCS_LOG_DIR);
    if (logDir != null) {
    ...
    logDir = GetJavaProperty.getProperty(Constants.DCS_TRAFODION_LOG);
    if (logDir != null) {
...
//     addServlet("conf", "/conf", ConfServlet.class);
  ...
  public void addJerseyResourcePackage(final String packageName,
    ...
    if (LOG.isInfoEnabled()) {
      LOG.info("addJerseyResourcePackage: packageName <{}>, pathSpec <{}>.", packageName, pathSpec);
    }
  ...
  public void addFilter(String name, String classname,
    ...
    if (LOG.isInfoEnabled())
        LOG.info("Added filter <{}> (class <{}>) to context <{}>.", name, classname, webAppContext.getDisplayName());
    ...
    for (Map.Entry<Context, Boolean> e : defaultContexts.entrySet()) {
      if (e.getValue()) {
        ...
        if (LOG.isInfoEnabled())
          LOG.info("Added filter <{}> (class <{}>) to context <{}>.", name, classname, ctx.getDisplayName());
      }
    }
        ...
        while (true) {
          try {
            ...
            LOG.debug("Port returned by webServer.getConnectors()[0]." +
                    "getLocalPort() before open() is <{}>. Opening the listener on <{}>.", port, oriPort);
            listener.open();
            port = listener.getLocalPort();
            LOG.debug("Port returned by listener.getLocalPort() after open() is <{}>. Port returned by webServer.getConnectors()[0].getLocalPort() after open() is <{}> ",
                listener.getLocalPort(), webServer.getConnectors()[0].getLocalPort());
