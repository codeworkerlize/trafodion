...
function check_node {
       for myinterface in `edb_pdsh -w $1 /sbin/ip link show|cut -d: -f2- | cut -c2- | awk -F': ' '/^[0-9]+:.*/ {print $2;}'`; do
		  ip_output=$(edb_pdsh -w $1 /sbin/ip addr show $myinterface | cut -d: -f2-)
		  if [ $gv_externalip_set -eq 1 -a $external_only -eq 1 ]; then
				...
				if [ $status -eq 0 ]; then
					 ...
					 if [ $? -eq 1 -o "$1" != "$gv_myhostname" ]; then
						  ...
						  dcsEcho "    Virtual ip $gv_float_external_ip is in use on node $1 bound to interface $myinterface($unbindlb), unbinding..."
						  dcsEcho "    edb_pdsh -w $1 sudo /sbin/ip addr del $unbindip dev $myinterface"
						  edb_pdsh -w $1 sudo /sbin/ip addr del $unbindip dev $myinterface

						  status=$?
						  if [ $status -ne 0 ]; then
								dcsEcho "    Failed to unbind status is: $status"
								exit $gv_error
						  fi
					 else
						  dcsEcho "    Virtual ip $gv_float_external_ip is already bound to $myinterface on node $1 - skip unbind"
...
function Check_VirtualIP_InUse_Unbind {
        dcsEcho "Check all configured  master nodes to see if virtual ip address is in use and if necessary, unbind "
        mynode=""
        configuredMasterNodes=`cat $TRAF_CONF/dcs/masters|/usr/bin/paste -sd ' ' -`
        masterNodes=$(echo $configuredMasterNodes |sed -e 's/ / -w /g')

        #Validate if external ip is in use
        dcsEcho "    Configured master nodes are: $configuredMasterNodes"
        externalNodes=`edb_pdsh -w $masterNodes /sbin/ip addr show | grep -w $gv_float_external_ip | awk -F' ' '/^.+:[[:space:]]+.*/ {print $1;}' | cut -d':' -f1 | sed '/^$/d'`
        if [ ! -z "$externalNodes" ]; then
                  dcsEcho "    Found node: `echo $externalNodes`"
                  external_only=1
                  internal_only=0
...
if [ $gv_externalip_set -eq 1 ]; then
   ...
   bcast=`/sbin/ip addr show $gv_float_external_interface | grep "inet $gv_local_ip.*$gv_float_external_interface\$" | awk '{print $4}'`
   mask=`/sbin/ip addr show $gv_float_external_interface | grep "inet $gv_local_ip.*$gv_float_external_interface\$" | awk '{print $2}' | cut -d'/' -f2`
