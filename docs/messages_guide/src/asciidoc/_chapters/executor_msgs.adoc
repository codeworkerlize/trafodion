...
```
<auth-name> could not be verified as a valid database authorization ID. There was
a problem reading metadata. An internal query returned SQLCODE <error-no>.
```

Where <auth-name> is a user, role, user group, or tenant.

Where <error-no> is a SQL error describing the error.

*Cause:* The metadata table "\_MD_".AUTHS contains authorization names known to
{project-name}.  While trying to ascertain if <auth-name> is defined, an unexpected
error <error-no> is returned.  

*Effect:* This operation fails.

*Recovery:* See if the error returned in <error-no> can be fixed and retry the request.
...
```
<auth-name> is not a registered database user or role.
```

Where <auth-name> is a user or role.

*Cause:* While connecting or performing DDL operations, a user or role name can
be specified.  {project-name} verifies that the specified name exists in the metadata.
This error is returned if the <auth-name> does not exist in the metadata.

*Effect:* The operation fails.

*Recovery:* Specify the correct user or role name and retry the request.

[[SQL-8747]]
== SQL 8747

```
<tenant-name> is not a registered tenant.
```

Where <tenant-name> is the specified tenant.

*Cause:* A request to connect to {project-name} failed because the specified
tenant name <tenant-name> does not exist.

*Effect:* The operation fails.

*Recovery:* Specify the correct tenant name and retry.
...
```
Unable to assign process <process-name> to cgroup <cgroup-name>, reason: <details>.
```

Where <process-name> is the process reporting the error.

Where <cgroup-name> is the cgroup assigned to the current tenant.

Where <details> contains additional message text describing why the operation failed.

*Cause:* A user connects and specifies a tenant.  When the user executes a query, 
a set of processes are allocated, and each allocated process is assigned to the 
tenant's cgroup. This error is returned because the process allocated to the query 
(<process-name>) could not be assigned to tenant's cgroup.
...
```
Unable to create cgroup <cgroup-name> for process <process-name>, reason: <details>.
```
Where <process-name> is the process reporting the error.
...
*Cause:* A user connects and specifies a tenant.  When the user executes a query, 
a set of processes are allocated, and each allocated process is assigned to the 
tenant's cgroup. Generally, cgroups for each tenant have been created before users 
connect to run queries.  But depending on how {project-name} is managed, creation 
of cgroups could happen during query execution.  Therefore, if any processes 
allocated to the query notice that the cgroup does not exist, they create the 
cgroup before continuing. This error is returned because process (<process-name>) 
was unable to create the cgroup.
