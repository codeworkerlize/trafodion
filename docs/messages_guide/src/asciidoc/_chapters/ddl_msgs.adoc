...
```
The directory server groups associated with the user must be also one of the 
directory server groups for the requested tenant.  There is no match so 
connection is refused.
```

*Cause:* Your system administrator has enabled the control query default 
TRAF_TENANT_GROUP_CHECK.  When enabled, the list of directory server groups 
assigned to the user is checked against the list of directory server groups 
assigned to the tenant.  The same group must exist in both lists. This error
is returned if there is not a match.  

*Effect:* The operation fails.

*Recovery:* Try connecting using a different tenant.  If this does not work, 
then ask your security administrator to assign you one of the directory server 
groups associated with the tenant.  If the control query default TRAF_TENANT_GROUP_CHECK 
is turned off, then this check is not performed and connections are allowed.
...
```
Nodes <node-list> are not part of the cluster.
```

Where <node-list> is a list of nodes specified during a create or alter resource group request.

*Cause:* The nodes specified in <node-list> are not valid nodes for the cluster.

*Effect:* The operation fails.

*Recovery:* Please enter valid node names and retry the request.
...
Where <auth-name> is the name of a user, role, user group, or tenant.

Where <auth-type is one of the following: user, role, user group, or tenant.

*Cause:* A request was made to read metadata for an operation associated with an 
authorization name.  An authorization name can be a user, role, user group, or tenant.
The authorization name was found but it was not the type expected. For example,
you enter the following statement: "alter tenant DB\__ROOT tenant size 4".  
The authorization name DB__ROOT exists in the metadata but is it an user,  not a tenant. 
In this case, the message: "DB__ROOT is not a tenant" is returned.  
...
```
User group <user-group> is already associated with tenant <tenant-name>.
```

Where <user-group> is the name of a directory server group.

Where <tenant-name> is the name of the tenant being altered.

*Cause:* Trying to alter a tenant to add a user group.  However, the tenant is
already assigned the user group.

*Effect:* The tenant is not altered.
...
```
Resource group <resource-group> does not exist.
```

Where <resource-group> is the resource group specified in the command.

*Cause:* Attempting to drop resource group <resource-group> from a tenant 
and the resource group specified does not exist.

*Effect:* The operation fails.

*Recovery:* Please specify a valid resource group name.  You can execute the 
command "GET RESOURCE GROUPS" to return the list of available resource groups.
...
```
Schema <schema-name> cannot be specified as the default schema for tenant 
<tenant-name> because admin role <role-name> has not been granted any 
privileges.  In order to set a default schema for a tenant, the tenant's 
admin role must be able to access objects in the schema.
```

Where <schema-name> is the name of the requested default schema.

Where <tenant-name> is the tenant being created or altered.

*Cause:* In order to assign a default schema to a tenant, the tenant owner must 
have been granted at least one schema level privilege.  This error is returned
if the tenant owner <role-name> has not been granted any schema level
privileges for <schema-name>.

*Effect:* The operation fails.

*Recovery:* Specify a different default schema or ask the system administator to
grant schema level privileges on <schema-name> to the tenant administrator <role-name>.
...
```
With the new tenant definition for tenant <tenant-name>, at least 
<number-nodes> of the <max-nodes> nodes of this cluster or resource 
group are oversubscribed. The cluster or resource group(s) has or have a 
capacity of <max-units> units, of which <allocated-units> are allocated to 
tenants. For example: Node <node> has a capacity of <node-units> units and has 
<current-units> units allocated.
```

Where <tenant-name> is the name of the tenant being created or altered.

Where <number-nodes> is the number of oversubscribed nodes.

Where <max-nodes> is the number of nodes available to the tenant.

Where <max-units> is the number of compute units available for the tenant.

Where <allocated-units> are the number of units allocated.

Where <node> is one of the nodes in the cluster.

Where <node-units> is the number of units available for the <node>.

Where <current-units> is the number of units that have been assigned to <node> 
upon completion of the request.

*Cause:* Tenants are allocated one or more compute units.  A single  compute
unit consists of 4 cores and 32 GB memory.  A cluster must have, at a minimum
of 4 compute units available in order to create a tenant.  To determine the 
number of available units, {project-name} obtains the list of nodes in the cluster, 
calculates the number of units available based on the node type, and distributes 
them on a first come first serve basis.  This warning is returned when you have 
oversubscribed, that is there is not enough compute units left to assign to 
the tenant.

*Effect:* The operation completes with oversubscription. However, resources
may not be available to run required work loads for the tenant. Once all the 
resources are used, queries will not run until existing resources are freed up.    

*Recovery:* This is a warning.  If you don't want to oversubscribe or want to
adjust the amount of oversubscription, update control query defaults
TENANT_OVERSUBSCRIBE_WARN_NODE_RATIO and TENANT_OVERSUBSCRIBE_ERR_NODE_RATIO.
...
With the new tenant definition for tenant <tenant-name>, at least 
<number-nodes> of the <max-nodes> nodes of this cluster or resource 
group would be oversubscribed. The cluster or resource group(s) has or have a 
capacity of <max-units>  units, of which <allocated-units> would be allocated 
to tenants. For example: Node <node> has a capacity of <node-units> units and 
would have <oversubscribed-units> units allocated.
```
...
Where <oversubscribed-units> is the number of units that would be assigned 
...
number of available units, {project-name} obtains the list of nodes in the cluster, 
calculates the number of units available based on the node type, and distributes 
them on a first come first serve basis.  This error is returned when there are 
not enough compute units left to assign to the tenant.

*Effect:* The operation fails.

*Recovery:* You have run out of compute units on your cluster.  You need to 
decrease the number of units to assign to the tenant, adjust the compute units 
available to the tenant, or allow oversubscription.  To allow oversubscription, 
update control query defaults
TENANT_OVERSUBSCRIBE_WARN_NODE_RATIO and TENANT_OVERSUBSCRIBE_ERR_NODE_RATIO.
...
```
Error registering, altering, or unregistering tenant <tenant-name>. Reason: <details>.
```
...
*Cause:* As part of managing a tenant, unit allocations are adjusted, tenant 
details are stored in Zookeeper, and Linux cgroups are updated.  An unexpected 
...
```
Trying to drop the last resource group assigned to tenant <tenant-name>.  
The tenant must be assigned at least one resource group.
```

Where <tenant-name> is the tenant being altered.

*Cause:* A tenant is associated with one or more resource groups.  Cannot drop 
the last resource group assigned to the tenant.
...
```
Resource group <resource-group> is not assigned to tenant <tenant-name>.
```
...
*Cause:* The tenant <tenant-name> is being altered to drop one or more resource
groups.  The resource group <resource-group> specified in the command is not
assigned to the tenant.

*Effect:* The operation fails.

*Recovery:* Please enter a valid resource group name and retry. A SHOWDDL TENANT
<tenant-name> displays the resource groups assigned to the tenant.
...
```
Tenant <tenant-name> is defined using <tenant-type1> but the alter tenant request assumes <tenant-type2>.
```

Where <tenant-name> is the tenant being altered.

Where <tenant-type1> is either adaptive segmentation or resource groups.
 
Where <tenant-type2> is either adaptive segmentation or resource groups.
...
trying to switch between adaptive segmentation and resource groups, or between 
resource groups and adaptive segmentation.
...
```
Resource group <resource-group> cannot be specified with other resource groups. 
Unable to create or alter tenant <tenant-name>.
```
...
*Cause:* The default resource group (DB__RGROUP_DEFAULT) is a special resource 
group that means all nodes in the cluster.  This error is returned when 
DB_RGROUP_DEFAULT is specified with other resource groups, or when other resource
groups are specified with DB_RGROUP_DEFAULT.  
...
```
Additional nodes have been added to resource group <resource-group>.  
Please perform "ALTER TENANT ... BALANCE" for tenants <list-of-tenants> to get 
optimum node placement.
```

Where <resource-group> is the resource group being altered.

Where <list-of-tenants> contains tenant names that need to be balanced.

*Cause:* When nodes are added to resource forks, tenants can use the new node's
resources for better query distribution and to avoid oversubscription.  
In order to take advantage of the additional nodes, the tenant must be re-balanced.  
This is performed by altering each tenant in <list-of-tenants> using the BALANCE option.

*Effect:* The operation completes but additional operations are required to 
utilize the new node resource(s).
...
== SQL 1104
```
Trying to drop the last node assigned to resource group <resource-group>. 
The resource group must be assigned at least one node.
```

Where <resource-group> is the resource group being altered.

*Cause:* A resource group defines defines a pool of one of more nodes.  
You are attempting to remove the last node in the pool.
...
== SQL 1105
```
Resource group <resource-group> cannot be owned by tenant admin role <admin-role>. 
Please specify a different user or role in the AUTHORIZATION clause.
```

Where <resource-group> is the resource group being created.

Where <admin-role> is the role that handles administrator tasks for a tenant.

*Cause:* When a resource group is created, you can assign ownership through the
AUTHORIZATION clause.  However, you cannot assigned ownership to a role that acts
as an administrator for a tenant.  

*Effect:* The operation fails.

*Recovery:* Specify a different user or role in the AUTHORIZATION clause. When
choosing a resource group owner, please choose a user or role that will exist 
for the life of the resource group and any tenants assigned the resource group.
...
```
Cannot drop role <role-name>.  Role owns one or more resources including (<list-of-resources>).
```

Where <role-name> is the role being dropped.

Where <list-of-resources> is the list of resource groups that are owned by <role-name>.

*Cause:*  Unable to drop role <role-name> because it is owner of one or more resources.

*Effect:* The operation fails.

*Recovery:* Drop the resource groups defined in <list-of-resources> and retry
the DROP ROLE command.  Dropping the resource groups also require tenants assigned
to the resource group to be dropped.
...
```
ALTER RESOURCE GROUP <resource-group> failed during a drop node operation.  With 
fewer nodes, there are not enough compute units available to satisfy tenant <tenant-name>.
```

Where <resource-group> is the resource group being altered.

Where <tenant-name> is the name of the tenant that could not be balanced.

*Cause:* Resource groups define a pool of nodes which are assigned to one or 
more tenants.  Tenants are assigned one or more resource groups.  When executing 
queries, processing is limited to combined pool of nodes associated with the 
tenant.  When a resource group is altered to reduce the number of available 
nodes, {project-name} balances affected tenants based on the reduced set of nodes. 
This error is returned if the balance operation is unable to assign the required 
compute units to <tenant-name>. 

*Effect:* The operation fails.

*Recovery:* You have run out of compute units for <tenant-name>.  You need to 
decrease the number of units (tenant size) for <tenant-name>, assign 
<tenant-name> other resource groups, or allow oversubscription.  To allow 
oversubscription, update control query defaults
TENANT_OVERSUBSCRIBE_WARN_NODE_RATIO and TENANT_OVERSUBSCRIBE_ERR_NODE_RATIO.
...
```
Schema <schema-name> exists but is not assigned to tenant <tenant-name>.  
```

Where <schema-name> is the name of an {project-name} database schema.

Where <tenant-name> is the tenant being created or altered.

*Cause:* {project-name} supports different kinds of schemas including tenant 
schemas and global schemas.  Tenant schemas are created and managed by the 
tenant owner whereas global schemas are managed by the database administrator.  
When creating a tenant, the list of schemas specified in the SCHEMAS clause are
treated as tenant schemas.  When the tenant is dropped, the schemas are also
dropped.  When altering a tenant, the list of schemas has to be new schemas or 
schemas that are currently assigned to the tenant. 

*Effect:* The operation fails.

*Recovery:* Please specify schemas that are assigned to the tenant or do not exist.
...
```
No admin role has been specified for tenant <tenant-size>.
```

Where <tenant-name> is the tenant being created.

*Cause:* When creating a tenant, a tenant administrator role is required.

*Effect:* The operation fails.

*Recovery:* Please specify the "ADMIN ROLE <role-name>" and resubmit.
...
```
The specified role <admin-role> cannot be assigned to tenant <tenant-name>.
``` 

Where <admin-role> is the role to handle administrator tasks for a tenant.

Where <tenant-name> is the name of the tenant to be administered.

*Cause:* The specified <admin-role> cannot be used for the tenant.  This can happen if
<admin-role> is not a role (for example a user) or if <admin-role> is already used by
another tenant.  An admin role can only administer a single tenant.

*Recovery:* Specify a different <admin-role> and resubmit.
...
```
This operation is not support on LDAP authentication.
```

*Cause:* The statement which like ALTER USER abc IDENTIFIED BY xxx is only used by local authentication.
...
```
Setup user with password in LDAP mode is not supported.
```

*Cause:* The statement which like CREATE USER abc IDENTIFIED BY xxx is only used by local authentication.

*Effect:* The operation fails.Warning only, ignore the password
...
```
The suffix 'CONFIG' is only use for the LDAP authentication and will be ignored.
```

*Cause:* While executing a CREATE/REGISTER GROUP with suffix of CONFIG or CREATE/REGISTER USER with suffix of CONFIG command to configure the name of section
in the LDAP config file for this entry,the current authentication is not type of LDAP.

*Effect:* Just a warning message,will be used the default section name instead of the given value.
...
```
User <user-name> is not member of group <group-name>.
```

Where <user-name> is a user name that you specified.

Where <group-name> is a group name that you specified.

*Cause:* While executing a ALTER GROUP with suffix of REMOVE MEMBERS or ALTER USER with suffix of REMOVE GROUPS command to
let the given user quits the given group,the given user is not a member of given group.


*Effect:* The operation fails.

*Recovery:* Please check user name of the given user is correct.

[[SQL-1365]]
== SQL 1365

```
Unable to drop admin role <admin-role> for tenant <tenant-name>.  
To drop the role, please drop the tenant.
```

Where <admin-role> is the role that handles administrator tasks for a tenant.

Where <tenant-name> is the name of the tenant that is administered by the <admin-role>.

*Cause:* Each tenant is optionally assigned an admin role.  This role is 
assigned to the tenant until the tenant is dropped.  You attempted to drop
the role but the tenant still exists.

*Effect:* The operation fails.

*Recovery:* To drop the <admin-role> you must drop tenant <tenant-name>.

[[SQL-1415]]
== SQL 1415

```
Tenant <tenant-name> is reserved.  Please specify a valid tenant name.
```

Where <tenant-name> is the name of a {project-name} database tenant.

*Cause:* {project-name} reserves certain tenant names for its own use.  An attempt
was made to alter this tenant. 
...
```
<auth-name> is not a group.
```

Where <auth-name> is the authorization name of the object you specified.

*Cause:* Recheck the name of object which is specified for you is a vaild authorization name for an exists group.
...
```
The authorization password "<authorization-password>" contains one or more invalid characters. A valid password can only contain these characters: [a-zA-Z_0-9-@./] And using statement 'GET PWDPOLICY' to view the complexity rule for password.
```

Where <authorization-password> is the user password you specified.

*Cause:* You attempted to execute a CREATE USER or ALTER USER command, but the user password you specified contains characters that
are not permitted.

*Effect:* The operation fails.

*Recovery:* Correct the user password and resubmit.

[[SQL-1372]]
== SQL 1372

```
Only allowed to change self password.
```

*Cause:* Execute ALTER USER statement to change one's password is only allowed for self password,except DB__ROOT is ok.
...
```
Invalid syntax for password check policy "<string>" on "<where the string is defined>".
```
Where <string> is the password check policy syntax that you defined.
Like '2117FFEE0012'-- consists of 12 bytes chars.

Where <where the string is defined> is a place where <string> is deined in.

*Cause:* The program will parse in the order of 'environment variables' -> 'initial values on defaults table' -> 'default rules' to
get the syntax of the password check policy. so when <where the string is defined> is 'environment variable' or 'inner default value', 
it will be ignored,and the next vaild value will be used. When users see this log, please check if the syntax located on <where the string is defined> 
is correct.

*Effect:* Just a warning.
...
*Cause:* execute a CREATE USER or ALTER USER command with empty but no null string for password.
...
```
The authorization password "<authorization-password>" exceeds max password length.
```

Where <authorization-password> is the user password you specified.

*Cause:* You attempted to execute a CREATE USER or ALTER USER command, but the user password you specified exceeds max password length.
...
```
CACHE value must be greater than 1 and less than or equal to (maxValue-startValue)/incrementValue+1 for <purpose>.
```
...
```
User group <user-group> is not a valid user group for tenant <tenant-name>.
```
...
*Cause:* You are altering a tenant to remove <user-group>, however <user-group>
is not assigned to tenant <tenant-name>.

*Effect:* The operation fails.

*Recovery:* Please specify a valid user group  for the tenant.  A SHOWDDL TENANT
<tenant-name> displays valid user group names.
