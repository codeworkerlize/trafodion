...
class HadoopDiscover(object):
    ...
    def stop_cluster(self):
        cmd = self.hg.post('%s/%s/commands/stop' % (self.v1_url, self.cluster_name.replace(' ', '%20')))
        if cmd['active']:
            self.cmd_id = cmd['id']
        else:
            self.cmd_id = -1
    ...
    def restart_cluster(self):
        cmd = self.hg.post('%s/%s/commands/restart' % (self.v1_url, self.cluster_name.replace(' ', '%20')))
        if cmd['active']:
            self.cmd_id = cmd['id']
        else:
            self.cmd_id = -1
    ...
    def deploy_cfg(self):
        cmd = self.hg.post('%s/commands/deployClientConfig' % self.v6_cluster_url)
        if cmd['active']:
            self.cmd_id = cmd['id']
        else:
            self.cmd_id = -1
    ...
    def check_cluster_status(self):
        try:
            if self.cmd_id != -1:
                cmd = self.hg.get('%s/%s' % (self.v1_check_url, self.cmd_id))
                return cmd['success']
            elif self.cmd_id == -1:
                return True
        except:
            err_m('Failed to get %s status info from CDH' % self.cluster_name)
