...
def run(dbcfgs, options, mode='install', param='', user='', pwd='', log_file='', stat_file=''):
    ...
    """
    if not stat_file:
        stat_file = '%s/add_%s.status' % (INSTALLER_LOC, mode)
    if not log_file:
        log_file = '%s/add_%s_%s.log' % (DEF_LOG_DIR, mode, time.strftime('%Y%m%d_%H%M%S'))
    ...
    cdhnodes = dbcfgs['cdhnodes'].split(',')
    ...
    try:
        if mode == 'install' or mode == 'uninstall':
            # 'install' mode needs to be run on all cdhnodes
            remote_cdh_instances = [RemoteRun(host, user=user, pwd=pwd, verbose=verbose) for host in cdhnodes]
            # trafnodes should be part of cdhnodes
            remote_traf_instances = [instance for instance in remote_cdh_instances if instance.host in trafnodes]
            # do init on all cdh_nodes
            threads = [Thread(target=r.initialize) for r in remote_cdh_instances]
        else:
        ...
        for cfg in script_cfgs:
            ...
            elif node == 'cdhnodes' or node == 'trafnodes':
                if node == 'cdhnodes':
                    instances = remote_cdh_instances
                elif node == 'trafnodes':
