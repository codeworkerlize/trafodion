...
def log_err(errtext):
    ...
    tp = ParseInI(ADDON_CFG_TMP_FILE, 'dbconfigs')
    tp.save(cfgs)
    err_m(errtext)
...
def user_input(options, prompt_mode=True, pwd=''):
    ...
    if os.path.exists(ADDON_CFG_TMP_FILE) and prompt_mode == True:
        tp = ParseInI(ADDON_CFG_TMP_FILE, 'dbconfigs')
        cfgs = tp.load()
        if not cfgs:
    ...
    cluster_nodes = hadoop_discover.get_cluster_nodes()
    cfgs['cdhnodes'] = ','.join([node.lower() for node in cluster_nodes])
    ...
    if os.path.exists(TRAF_CFG_FILE):
        ...
        cfgs['node_list'] = ','.join(run_cmd_as_user(cfgs['traf_user'], 'trafconf -name', logger_output=False).split())
    else:
        g('traf_user')
    ...
    cfgs['parcels_dir'] = hadoop_discover.get_parcel_dir()
    cfgs['hdfs_ver'] = hadoop_discover.get_hdfs_ver()
    cfgs['hbase_ver'] = hadoop_discover.get_hbase_ver()

    u.notify_user(ADDON_CFG_FILE, UNINSTALL_STATUS_FILE)


def get_options():
    usage = 'usage: %prog [options]\n'
    usage += '  EsgynDB Add-On installation tool.'
...
def main():
    """ Add-on_install main loop """
    global cfgs

    # handle parser option
    options = get_options()

    format_output('Add-On UnInstallation ToolKit')

    config_file = ADDON_CFG_FILE
    ...
    format_output('UnInstallation Start')
    ...
    if options.logfile and options.statfile:
        wrapper.run(cfgs, options, mode='uninstall', pwd=pwd, log_file=options.logfile, stat_file=options.statfile)
    elif options.logfile:
        wrapper.run(cfgs, options, mode='uninstall', pwd=pwd, log_file=options.logfile)
    elif options.statfile:
        wrapper.run(cfgs, options, mode='uninstall', pwd=pwd, stat_file=options.statfile)
    else:
        wrapper.run(cfgs, options, mode='uninstall', pwd=pwd)

    format_output('UnInstallation Complete')
    ...
    try:
        ...
        if config_file == ADDON_CFG_FILE and os.path.exists(config_file):
            os.rename(config_file, config_file + '.bak' + ts)
    except OSError:
        log_err('Cannot rename config file')

    # remove temp config file
    if os.path.exists(ADDON_CFG_TMP_FILE): os.remove(ADDON_CFG_TMP_FILE)

if __name__ == "__main__":
    try:
        main()
    except (KeyboardInterrupt, EOFError):
        tp = ParseInI(ADDON_CFG_TMP_FILE, 'configs')
        tp.save(cfgs)
        print '\nAborted...'
