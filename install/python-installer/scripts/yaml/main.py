...
import scripts.yaml
from scripts.yaml.error import UnsafeLoaderWarning, YAMLError  # NOQA

from scripts.yaml.tokens import *  # NOQA
from scripts.yaml.events import *  # NOQA
from scripts.yaml.nodes import *  # NOQA

from scripts.yaml.loader import BaseLoader, SafeLoader, Loader, RoundTripLoader  # NOQA
from scripts.yaml.dumper import BaseDumper, SafeDumper, Dumper, RoundTripDumper  # NOQA
from scripts.yaml.compat import StringIO, BytesIO, with_metaclass, PY3, nprint
from scripts.yaml.resolver import VersionedResolver, Resolver  # NOQA
from scripts.yaml.representer import (
    BaseRepresenter,
    SafeRepresenter,
    Representer,
    RoundTripRepresenter,
)
from scripts.yaml.constructor import (
    BaseConstructor,
    SafeConstructor,
    Constructor,
    RoundTripConstructor,
)
from scripts.yaml.loader import Loader as UnsafeLoader

if False:  # MYPY
    from typing import List, Set, Dict, Union, Any, Callable, Optional, Text  # NOQA
    from scripts.yaml.compat import StreamType, StreamTextType, VersionType  # NOQA
...
try:
    from _scripts.yaml import CParser, CEmitter  # type: ignore
except:  # NOQA
...
class YAML(object):
    def __init__(
        ...
        self.Resolver = scripts.yaml.resolver.VersionedResolver  # type: Any
        ...
        if 'rt' in self.typ:
            ...
            self.Emitter = (
                scripts.yaml.emitter.Emitter if pure or CEmitter is None else CEmitter
            )
            self.Representer = scripts.yaml.representer.SafeRepresenter
            self.Parser = scripts.yaml.parser.Parser if pure or CParser is None else CParser
            self.Composer = scripts.yaml.composer.Composer
            self.Constructor = scripts.yaml.constructor.SafeConstructor
        elif 'base' in self.typ:
            self.Emitter = scripts.yaml.emitter.Emitter
            self.Representer = scripts.yaml.representer.BaseRepresenter
            self.Parser = scripts.yaml.parser.Parser if pure or CParser is None else CParser
            self.Composer = scripts.yaml.composer.Composer
            self.Constructor = scripts.yaml.constructor.BaseConstructor
        elif 'unsafe' in self.typ:
            self.Emitter = (
                scripts.yaml.emitter.Emitter if pure or CEmitter is None else CEmitter
            )
            self.Representer = scripts.yaml.representer.Representer
            self.Parser = scripts.yaml.parser.Parser if pure or CParser is None else CParser
            self.Composer = scripts.yaml.composer.Composer
            self.Constructor = scripts.yaml.constructor.Constructor
        else:
            setup_rt = True
            typ_found = 0
        if setup_rt:
            ...
            self.Emitter = scripts.yaml.emitter.Emitter  # type: Any
            self.Serializer = scripts.yaml.serializer.Serializer
            self.Representer = scripts.yaml.representer.RoundTripRepresenter  # type: Any
            self.Scanner = scripts.yaml.scanner.RoundTripScanner
            # no optimized rt-parser yet
            self.Parser = scripts.yaml.parser.RoundTripParser  # type: Any
            self.Composer = scripts.yaml.composer.Composer  # type: Any
            self.Constructor = scripts.yaml.constructor.RoundTripConstructor  # type: Any
        del setup_rt
        self.stream = None
        self.canonical = None
    ...
    def get_constructor_parser(self, stream):
        ...
        if self.Parser is not CParser:
            if self.Reader is None:
                self.Reader = scripts.yaml.reader.Reader
            if self.Scanner is None:
                self.Scanner = scripts.yaml.scanner.Scanner
            self.reader.stream = stream
        else:
            if self.Reader is not None:
                if self.Scanner is None:
                    self.Scanner = scripts.yaml.scanner.Scanner
                self.Parser = scripts.yaml.parser.Parser
                self.reader.stream = stream
                ...
                # is actually used
                rslvr = self.Resolver
                # if rslvr is scripts.yaml.resolver.VersionedResolver:
                #     rslvr = scripts.yaml.resolver.Resolver
    ...
    def get_serializer_representer_emitter(self, stream, tlca):
        ...
        rslvr = (
            scripts.yaml.resolver.BaseResolver
            if 'base' in self.typ
            else scripts.yaml.resolver.Resolver
        )
    ...
    def map(self, **kw):
        # type: (Any) -> Any
        if 'rt' in self.typ:
            from scripts.yaml.comments import CommentedMap
    ...
    def seq(self, *args):
        # type: (Any) -> Any
        if 'rt' in self.typ:
            from scripts.yaml.comments import CommentedSeq
...
def add_implicit_resolver(
    ...
    if Loader:
        if hasattr(Loader, 'add_implicit_resolver'):
            ...
            Loader, (BaseLoader, SafeLoader, scripts.yaml.loader.Loader, RoundTripLoader)
        ):
            Resolver.add_implicit_resolver(tag, regexp, first)
        else:
            raise NotImplementedError
    if Dumper:
        if hasattr(Dumper, 'add_implicit_resolver'):
            ...
            Dumper, (BaseDumper, SafeDumper, scripts.yaml.dumper.Dumper, RoundTripDumper)
        ):
            Resolver.add_implicit_resolver(tag, regexp, first)
        else:
            raise NotImplementedError
...
def add_multi_constructor(tag_prefix, multi_constructor, Loader=None, constructor=Constructor):
    ...
    if Loader is None:
        ...
        elif issubclass(Loader, scripts.yaml.loader.Loader):
            Constructor.add_multi_constructor(tag_prefix, multi_constructor)
        elif issubclass(Loader, RoundTripLoader):
