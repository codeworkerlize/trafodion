...
class RemoteRun(Remote):
    ...
    def __init__(self, host, user='', pwd='', port='', verbose=False, quiet=False):
    ...
    def initialize(self):
        super(RemoteRun, self).__init__(self.host, self.user, self.pwd, self.port)
...
def run(dbcfgs, options, mode='install', param='', user='', pwd='', log_file='', stat_file=''):
    ...
    if mode in ['install', 'preinstall', 'allinstall']:
    ...
    no_deploy_ldap = True if hasattr(
        options, 'local') and options.local else False
    ...
    if enhance_ha or dbcfgs['distro'] == 'CDH_APACHE':
    ...
    if install_qian:
        skipped_scripts += ['deploy_hadoop']

    if reinstall:
        skipped_scripts += ['hadoop_ops', 'hadoop_mods', 'apache_mods', 'traf_dep', 'traf_kerberos']

    if ldap_only:
        skipped_scripts += ['gen_repo', 'services_set']
    else:
        deploy_ldap = dbcfgs['deploy_ldap'].upper()
        if deploy_ldap == 'N':
            skipped_scripts.append('deploy_ldap')
    ...
    # run sub scripts
    try:
        if mode == 'install' or mode == 'allinstall':
            # 'install' mode needs to be run on all rsnodes
            remote_rs_instances = [RemoteRun(host, user=user, pwd=pwd, port=ssh_port, verbose=verbose) for host in rsnodes]
        ...
        elif mode == 'alldiscover':
            # 'install' mode needs to be run on all rsnodes
            remote_rs_instances = [RemoteRun(host, user=user, pwd=pwd, port=ssh_port, verbose=verbose, quiet=True) for host in rsnodes]
            # trafnodes should be part of rsnodes
            remote_traf_instances = [instance for instance in remote_rs_instances if instance.host in trafnodes]
            # do init on all rsnodes
            threads = [Thread(target=r.initialize) for r in remote_rs_instances]
            ...
            status.set_status()
            remote_traf_instances = [RemoteRun(host, user=user, pwd=pwd, port=ssh_port, verbose=verbose) for host in trafnodes]
            ...
            remote_traf_instances = [RemoteRun(host, user=user, pwd=pwd, port=ssh_port, verbose=verbose, quiet=True) for host in trafnodes]
            # do init in threads to improve performance
            threads = [Thread(target=r.initialize) for r in remote_traf_instances]
        ...
        for cfg in script_cfgs:
            ...
            elif node == 'rsnodes' or node == 'trafnodes' or node == 'cdhnodes':
                if node == 'rsnodes' or node == 'cdhnodes':
