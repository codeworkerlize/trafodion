...
void Queue::cleanup() {
  ...
  // when the Queue is non-empty, tail points to a dummy entry, so get
  // rid of that now
  if (tail) {
    // REVISIT
    if (heap_)
  ...
  // Coding oddity: if we try using NULL here we get in trouble (I tried it!)
  // because one of the Q_Entry template possibilities is integer. Someday
  // we should replace Queue with an stl object perhaps.
  curr = 0;
  head = 0;
  tail = 0;
}
...
void *Queue::getCurrEntryPtr() { return curr; }
...
NABoolean Queue::remove(void *entry) {
  ...
  */
  ...
  if (!temp->entry) return FALSE;  // Soln 10-040521-6302
...
// delete from entry all the way to prev of curr
NABoolean Queue::removeStartingFrom(void *entry) {
  Q_Entry *temp = NULL;
...
int Queue::unpack(void *base, void *reallocator) {
...
void HashQueue::insert(const char *data, int dataLength, void *entry) {
  ...
  if (!sequentialAdd()) {
    ...
    while (current_ && current_->next_) current_ = current_->next_;

    HashQueueEntry *hqe = new (heap_) HashQueueEntry(entry, current_, NULL, hashValue_);
...
void HashQueue::remove(void *entry) {
  ...
  if (lastReturned_ == NULL || lastReturned_->entry_ != entry) {
    ...
    for (currentChain_ = 0, current_ = NULL; (currentChain_ < hashTableSize_); currentChain_++) {
      current_ = hashTable_[currentChain_];
      while (current_ && (current_->entry_ != entry)) current_ = current_->next_;
      if (current_ && current_->entry_ == entry) break;
    };
  } else
...
void HashQueue::remove(const char *data, int dataLength, void *entry) {
  ...
  if (lastReturned_) removeLastReturned();
  currentChain_ = hashTableSize_;
  current_ = NULL;
  hashValue_ = 0;
}
...
void HashQueue::removeLastReturned() {
  ...
  // now take care of the prev_ pointer of the next entry
  if (lastReturned_->next_) lastReturned_->next_->prev_ = lastReturned_->prev_;
