...
ExplainDesc::ExplainDesc(int numCols, int recLength, Space *space)
  ...
  explainCols_ = (ExplainTupleColDesc *)new (space) char[sizeof(ExplainTupleColDesc) * numCols_];
...
ExplainTuple::ExplainTuple(ExplainTuple *leftChild, ExplainTuple *rightChild, ExplainDesc *explainDesc)
...
ExplainTuple::~ExplainTuple() {
  if (explainTupleStr_) delete explainTupleStr_;
}
...
short ExplainTuple::genExplainTupleData(Space *space) {
  if (explainTupleStr_) {
    ...
    if (descrColSize >= descCursor_)
      ...
      usedRecLength = getRecLength();  // truncate explainTupleStr_
    setUsedRecLength(usedRecLength);
    explainTupleData_ = space->allocateAndCopyToAlignedSpace(explainTupleStr_, usedRecLength);
...
void ExplainTuple::setCol(int col, void const *value,
  ...
  if (e) {
    ...
    UInt32 vcIndicatorLen = 0;
    if (col == ExplainTupleCols::EX_COL_DESCRIPT && DFS2REC::isAnyCharacter(dataType))
      vcIndicatorLen = CharType::determineVCHdrSize(len * CharInfo::minBytesPerChar(CharInfo::DefaultCharSet));
    if (vcIndicatorLen == 0) vcIndicatorLen = ExpTupleDesc::VC_ACTUAL_LENGTH;
    UInt32 nullOffset = 0;
    UInt32 vcOffset = 0;
    NABoolean isNullable = (getColNullFlag(col) != 0);
    UInt32 colOffset =
        ExpTupleDesc::sqlarkExplodedOffsets(offset, len, dataType, isNullable, &nullOffset, &vcOffset, vcIndicatorLen);
...
void ExplainTuple::setModuleName(const char *modName) { setCol(EX_COL_MODNAME, modName, 0, 0); }
...
int ExplainTuple::getSeqNum() {
  ...
  if (e) {
    ...
    UInt32 nullOffset = 0;
    UInt32 colOffset = ExpTupleDesc::sqlarkExplodedOffsets(offset, len, dataType, isNullable, &nullOffset);

    if ((isNullable) && (*((Int16 *)((char *)e + nullOffset)) != 0)) {
...
void ExplainTupleMaster::setTableName(const char *tabName) { setCol(EX_COL_TABLENAME, tabName, 0, 0); }

void ExplainTupleMaster::setCardinality(double card) {
  float fcard = (float)card;

  setCol(EX_COL_CARD, &fcard, 0, 0);
}

void ExplainTupleMaster::setOperatorCost(double opCost) {
  float fopCost = (float)opCost;

  setCol(EX_COL_OPCOST, &fopCost, 0, 0);
}

void ExplainTupleMaster::setTotalCost(double totCost) {
  float ftotCost = (float)totCost;

  setCol(EX_COL_TOTCOST, &ftotCost, 0, 0);
}

void ExplainTupleMaster::setDetailCost(char *detail) {
  if (detail)
...
void ExplainTupleMaster::setDescription(const char *desc) {
