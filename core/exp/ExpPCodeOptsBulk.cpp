...
PCodeInst *PCodeCfg::findHashInst(PCodeOperand *def, PCodeInst *start) {
  ...
  FOREACH_INST_IN_BLOCK_BACKWARDS_AT(block, inst, start) {
    for (CollIndex i = 0; i < inst->getWOps().entries(); i++) {
      if (defIndex == inst->getWOps()[i]->getBvIndex()) {
        if (inst->isHash() || inst->getOpcode() == PCIT::HASHCOMB_BULK_MBIN32U) return inst;
  ...
  if (block->reachingDefsTab_->find(defIndex, TRUE /* In */))
    if (block->getPreds().entries() > 0) return findHashInst(def, block->getPreds()[0]->getLastInst());
...
void PCodeCfg::bulkHashComb() {
  ...
  FOREACH_BLOCK(block, firstInst, lastInst, index){FOREACH_INST_IN_BLOCK(
      block, inst){if (inst->getOpcode() == PCIT::HASHCOMB_MBIN32U_MBIN32U_MBIN32U){PCodeInst * newInst;
  ...
  if (!HASH_ALL && (!src1 || !src2)) continue;
...
if (!REORDER_HASH) return;

FOREACH_BLOCK(block, firstInst, lastInst,
              index){FOREACH_INST_IN_BLOCK(block, inst){if (inst->getOpcode() == PCIT::HASHCOMB_BULK_MBIN32U){
...
void PCodeCfg::codeMotion() {
  FOREACH_BLOCK(block, firstInst, lastInst, index) { localCodeMotion(block); }
  ENDFE_BLOCK
}
...
void PCodeCfg::localCodeMotion(PCodeBlock *block){
    FOREACH_INST_IN_BLOCK(block, inst){// Group certain instructions as close together as possible
                                       if ((inst->getOpcode() == PCIT::RANGE_MFLT64) || (inst->isNullBitmap()) ||
                                           (inst->isCopyMove() || inst->isConstMove())){PCodeInst *insertInst = NULL;
...
FOREACH_INST_IN_BLOCK_BACKWARDS_AT(block, prev, inst->prev) {
  ...
  if (inst->isCopyMove() && prev->isCopyMove()) {
    ...
    if ((inst->getWOps()[0]->getStackIndex() == prev->getWOps()[0]->getStackIndex()) &&
        (inst->getWOps()[0]->getOffset() >= prev->getWOps()[0]->getOffset())) {
      // Next group by reads
      if ((inst->getROps()[0]->getStackIndex() == prev->getROps()[0]->getStackIndex()) &&
          (inst->getROps()[0]->getOffset() >= prev->getROps()[0]->getOffset())) {
        ...
        if ((prev->getWOps()[0]->getStackIndex() != prev->getROps()[0]->getStackIndex()) &&
          ...
          if (opc == PCIT::MOVE_MBIN8_MBIN8_IBIN32S) nInst->code[5] = len;
    ...
    // Check moves first
    if (inst->isConstMove() && (prev->isConstMove() || prev->isCopyMove()) &&
        (inst->getWOps()[0]->getStackIndex() == prev->getWOps()[0]->getStackIndex()))
      break;
  ...
  for (i = 0; !found && (i < prev->getWOps().entries()); i++)
    for (j = 0; !found && (j < inst->getWOps().entries()); j++)
      if (inst->getWOps()[j]->canOverlap(prev->getWOps()[i])) found = TRUE;
  ...
  for (i = 0; !found && (i < prev->getWOps().entries()); i++)
    for (j = 0; !found && (j < inst->getROps().entries()); j++)
      if (inst->getROps()[j]->canOverlap(prev->getWOps()[i])) found = TRUE;
  ...
  for (i = 0; !found && (i < prev->getROps().entries()); i++)
    for (j = 0; !found && (j < inst->getWOps().entries()); j++)
      if (inst->getWOps()[j]->canOverlap(prev->getROps()[i])) found = TRUE;
...
if (insertInst != NULL) {
  ...
  // copy move
  if ((insertInst->prev == NULL) && (!inst->isCopyMove())) continue;
...
NABoolean PCodeCfg::removeBulkNullBranch() {
  ...
  FOREACH_BLOCK_REV_DFO_AT(start, block, firstInst, lastInst, index) {
    if (lastInst && lastInst->isNullBranch()) {
      ...
      if (lastInst->isVarcharNullBranch() && lastInst->containsIndirectVarcharOperand()) continue;
...
NABoolean PCodeCfg::bulkNullBranch() {
  ...
  FOREACH_BLOCK_REV_DFO(block, firstInst, lastInst, index) {
    ...
    FOREACH_INST_IN_BLOCK(block, inst) {
      for (i = 0; i < inst->getWOps().entries(); i++) defs += inst->getWOps()[i]->getBvIndex();
    }
    ENDFE_INST_IN_BLOCK
      ...
      for (i = 0; i < lastInst->getROps().entries(); i++) {
...
#endif
        {
          ...
          for (j = 0; j < list.entries(); j++)
            if (rd == list[j]->getBvIndex()) break;

          // If operand does not already exist in the list, add it.
          if (j == list.entries()) list.insert(read);
        }
      }

      if (checked) numOfProcessedNB++;

      // If there are too many null branches, return.
      if (numOfNB > 1000) return FALSE;
    }
  }
  ENDFE_BLOCK_REV_DFO
  ...
  if ((numOfProcessedNB < 2) || (list.entries() > 50)) return FALSE;

  // Mark all blocks as unvisited - used in copyCfg()
  clearVisitedFlags();

  // Create a map table used to map copy blocks to original blocks.  Make a copy  // of the cfg starting from
  // startBlock.
  ...
  for (i = 0; i < list.entries(); i++) {
    ...
    bitmapMaxNullBitIndex = ((nullBitIndex > bitmapMaxNullBitIndex) ? nullBitIndex : bitmapMaxNullBitIndex);
    ...
    if (bitmapIdx != list[i]->getStackIndex()) differentRowsFound = TRUE;
  }
  ...
  if (alignedFormat && !packedFormat) {
    ...
    if (real < limit) differentRowsFound = TRUE;
  }
  ...
  if (startBlock == entryBlock_) entryBlock_ = newBlock;
      ...
      FOREACH_INST_IN_BLOCK_AT(block, inst, start) {
        ...
        if (inst->isCopyMove() || inst->isConstMove()) {
          if (list.isEmpty()) {
            ...
            list.insert(inst);
            if (inst->getROps().entries() > 0) srcStackIndex = inst->getROps()[0]->getStackIndex();

            if (inst->getOpcode() == PCIT::MOVE_MBIN8_MBIN8_IBIN32S) length++;
          } else if (inst->getWOps()[0]->getStackIndex() == list[0]->getWOps()[0]->getStackIndex()) {
            ...
            for (i = list.entries() - 1; i >= 0; i--) {
              ...
              if ((i == 0) || (inst->getROps().entries() && (inst->getROps()[0]->getBvIndex() == prevWIdx)) ||
                  (list[i]->getROps().entries() && (instWIdx == list[i]->getROps()[0]->getBvIndex())) ||
                  (instWIdx == prevWIdx)) {
