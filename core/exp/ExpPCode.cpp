...
unsigned char PCodeSegment::getClassVersionID() { return 1; }
...
void PCodeSegment::replaceAttributesPtr(ex_clause *clauses, Space *space) {
  ...
  while (pcode[0] != PCIT::END) {
    ...
    if ((addrs[0] != -1) && (pcode[0] != PCIT::CLAUSE_BRANCH) && (pcode[0] != PCIT::CLAUSE_EVAL)) {
...
void PCodeSegment::replaceClauseEvalPtr(ex_clause *oldClause, ex_clause *newClause) {
  PCodeBinary *pcode = getPCodeBinary();
  if (!pcode) return;
...
int PCode::IsTemporaryAccess(PCI *pci) {
  ...
  for (int i = 0, op = 0; i < pci->getNumberAddressingModes();
    ...
    if ((pci->getOperand(op) == 0) && (pci->getOperand(op + 1) == 1)) return 1;
  }
...
int PCI::temporaryStoreLoadMatch(PCI *store, PCI *load) {
  ...
  if ((store->getOperand(6) == load->getOperand(6)) && (store->getOperand(2) == load->getOperand(5))) return 1;
...
int PCI::temporaryStoreLoadOverlap(PCI *store, PCI *load) {
  ...
  for (int i = 0, op = 0; i < load->getNumberAddressingModes();
    ...
    if (!PCIT::isMemoryAddressingMode(am)) continue;
    if (!((load->getOperand(op) == 0) && (load->getOperand(op + 1) == 1))) continue;
    loadStart = load->getOperand(op + 2);
    loadLength = load->getMemoryOperandLength(am, op);
    loadEnd = loadStart + loadLength;
    if (((loadStart >= storeStart) && (loadStart < storeEnd)) || ((loadEnd > storeStart) && (loadEnd <= storeEnd)) ||
  ...
  for (int i = 0, op = 0; i < use->getNumberAddressingModes(); i++) {
    ...
    if (PCIT::isMemoryAddressingMode(am)) {
      ...
      if (storTgtAtp == useAtp && storTgtAtpIndex == useAtpIndex) {
        ...
        if (storTgtStart >= useEnd || storTgtEnd <= useStart) {
          // No overlap, do not replace but continue to check
          // next operand of the use PCI

        } else if (storTgtStart <= useStart && storTgtEnd >= useEnd &&
                   (i != 0 || use->getOperation() == PCIT::Op_RANGE_LOW ||
...
PCode::PCode(CollHeap *heap, Space *space)
    : pciList_(heap), heap_(heap), space_(space), profileCounts_(0), profileTimes_(0){};
...
#define AM_LENGTH(a)                                                                                                \
  ((a == PCIT::MATTR6)                                                                                              \
       ? 6                                                                                                          \
       : (a == PCIT::MUNIV || a == PCIT::MATTR5)                                                                    \
             ? 5                                                                                                    \
             : (a == PCIT::MATTR4 || a == PCIT::IATTR4)                                                             \
                   ? 4                                                                                              \
                   : (a == PCIT::IATTR3 || a == PCIT::MATTR3)                                                       \
                         ? 3                                                                                        \
                         : (a == PCIT::IATTR2 || a == PCIT::MBIN8 || a == PCIT::MBIN8S || a == PCIT::MBIN8U ||      \
                            a == PCIT::MBIN16U || a == PCIT::MBIN16S || a == PCIT::MBIN32U || a == PCIT::MBIN32S || \
                            a == PCIT::MBIN64U || a == PCIT::MBIN64S || a == PCIT::MPTR32 || a == PCIT::MASCII ||   \
                            a == PCIT::MUNI || a == PCIT::MBIGS || a == PCIT::MBIGU || a == PCIT::MDECS ||          \
                            a == PCIT::MDECU || a == PCIT::IBIN64S || a == PCIT::MFLT32 || a == PCIT::MFLT64)       \
                               ? 2                                                                                  \
                               : (a == PCIT::IBIN8U || a == PCIT::IBIN16U || a == PCIT::IBIN32S)                    \
                                     ? 1                                                                            \
                                     : (a == PCIT::IPTR) ? PCODEBINARIES_PER_PTR : 0)
...
static const PCIMap opcodeMap[] = {
    ...
    I4(Op_NOT_NULL_BRANCH, MBIN16S, MBIN16S, MBIN16S, IBIN32S,
       NOT_NULL_BRANCH_MBIN16S_MBIN16S_MBIN16S_IBIN32S),                      // Instruction 20
    ...
    IopCodeNotInUse(23), IopCodeNotInUse(24), IopCodeNotInUse(25), IopCodeNotInUse(26), IopCodeNotInUse(27),
    ...
    IopCodeNotInUse(153), IopCodeNotInUse(154), IopCodeNotInUse(155),
    ...
    IopCodeNotInUse(232), IopCodeNotInUse(233), IopCodeNotInUse(234), IopCodeNotInUse(235), IopCodeNotInUse(236),
    ...
    IopCodeNotInUse(252), IopCodeNotInUse(253), IopCodeNotInUse(254), IopCodeNotInUse(255), IopCodeNotInUse(256),
    IopCodeNotInUse(257), IopCodeNotInUse(258), IopCodeNotInUse(259), IopCodeNotInUse(260), IopCodeNotInUse(261),
    ...
    IopCodeNotInUse(277), IopCodeNotInUse(278), IopCodeNotInUse(279),
...
int PCode::getInstructionLength(PCodeBinary *pcode) {
  ...
  if ((opcode == PCIT::HASHCOMB_BULK_MBIN32U) || (opcode == PCIT::NOT_NULL_BRANCH_BULK) ||
  ...
  for (i = 0; i < PCIT::LAST_PCODE_INSTR; i++) {
    if (opcodeMap[i].instruction == code) {
      ...
      if (getenv("SQLMX_DEBUG_PCIT")) {
        cerr << " Instruction " << (PCIT::Instruction)opcodeMap[i].opcode << endl;
      }
#endif
...
#ifdef _DEBUG
  fprintf(stderr, "ERROR(getInstruction) -- Unknown instruction, opcode = %d\n", pci->getOperation());
#endif  // _DEBUG
...
void PCode::displayContents(PCIList code, Space *space) {
  ...
  while (pci != NULL) {
    ...
    if (pci->getNumberAddressingModes() > 0) {
      str_sprintf(tbuf, "%s", PCIT::addressingModeString(pci->getAddressingMode(0)));
...
Int16 PCIT::getDataTypeForMemoryAddressingMode(PCIT::AddressingMode am) {
  switch (am) {
    ...
    case PCIT::MBIN64U:
      ...
      break;
    case PCIT::MFLT32:
      return REC_IEEE_FLOAT32;
      break;
    case PCIT::MFLT64:
      return REC_IEEE_FLOAT64;
      break;
    case PCIT::MDECS:
...
const char *PCIT::operationString(PCIT::Operation op) {
  switch (op) {
    ...
    case PCIT::Op_NULL:
      return ("NULL");
    case PCIT::Op_NOT_NULL_BRANCH:
      return ("NOT_NULL_BRANCH");
    case PCIT::Op_NOT_NULL_BRANCH_COMP:
      return ("NOT_NULL_BRANCH_COMP");
    case PCIT::Op_NOT_NULL_BITMAP_BRANCH:
      return ("NOT_NULL_BITMAP_BRANCH");
    case PCIT::Op_NULL_BITMAP:
      return ("NULL_BITMAP");
    case PCIT::Op_NULL_BYTES:
      return ("NULL_BYTES");
    case PCIT::Op_NULL_VIOLATION:
      return ("NULL_VIOLATION");

    case PCIT::Op_UPDATE_ROWLEN:
      return ("UPDATE_ROWLEN");
    case PCIT::Op_UPDATE_ROWLEN2:
      return ("UPDATE_ROWLEN2");
    case PCIT::Op_UPDATE_ROWLEN3:
      return ("UPDATE_ROWLEN3");
    ...
    case PCIT::Op_GENFUNC:
      return ("GENFUNC");
    case PCIT::Op_HASH:
      return ("HASH");
    case PCIT::Op_FILL_MEM_BYTES:
    ...
    case PCIT::Op_TARGET:
      return ("TARGET");
    case PCIT::Op_RANGE_LOW:
      return ("RANGE_LOW");
    case PCIT::Op_RANGE_HIGH:
      return ("RANGE_HIGH");
    case PCIT::Op_REPLACE_NULL:
      return ("REPLACE_NULL");

    case PCIT::Op_CLAUSE:
      return ("CLAUSE");
    case PCIT::Op_CLAUSE_BRANCH:
      return ("CLAUSE_BRANCH");
    case PCIT::Op_CLAUSE_EVAL:
      return ("CLAUSE_EVAL");

    case PCIT::Op_PROFILE:
      return ("PROFILE");

    case PCIT::Op_NOP:
      return ("NOP");

    case PCIT::Op_HASH2_DISTRIB:
    ...
    case PCIT::Op_COPYVARROW:
      return ("COPYVARROW");

    case PCIT::Op_CONV_VC_PTR:
      return ("CONV_VC_PTR");
...
int PCI::getMemoryOperandLength(PCIT::AddressingMode am, int op) {
  switch (PCode::getInstruction(this)) {
    ...
    case PCIT::HASH_MBIN32U_MPTR32_IBIN32S_IBIN32S:
      if (am == PCIT::MBIN32U) return 4;
      if (am == PCIT::MPTR32) return getOperand(7);
      return 0;
    ...
    default:
      if (am == PCIT::MBIN8) {
        ...
        if ((getOperation() == PCIT::Op_GENFUNC) && (getOperand(0) == ITM_CONCAT) && (op == 4)) {
...
PCIList PCode::loadOpDataVCAddress(Attributes *attr, int loc, CollHeap *heap) {
  if (attr->isIndirectVC()) {
    ...
    OL ol(attr->getAtp(), attr->getAtpIndex(), attr->getOffset(), attr->getVoaOffset(), attr->getLength(), comboLen,
          ex_clause::MAX_OPERANDS + loc);
    PCI pci(PCIT::Op_OPDATA, aml, ol);
    pciList.append(pci);
    return pciList;
  } else {
    return loadOpDataAddress(attr, attr->getVCLenIndOffset(), ex_clause::MAX_OPERANDS + loc, heap);
  }
}
...
PCIList PCode::storeShortValue(UInt32 value, Attributes *attr, UInt32 offset, CollHeap *heap) {
  PCIList pciList(heap);
...
PCIList PCode::storeShortVoa(Attributes *attr, CollHeap *heap) {
  ...
  if ((attr->isSQLMXDiskFormat() && (attr->getVoaOffset() != ExpOffsetMax) && (attr->getOffset() != ExpOffsetMax))) {
    pciList.append(storeShortValue(attr->getOffset() - attr->getVCIndicatorLength(), attr, attr->getVoaOffset(), heap));
  }
  return pciList;
}
...
PCIList PCode::storeVoa(Attributes *attr, CollHeap *heap) {
  ...
  if ((attr->getVoaOffset() > 0) && (attr->getVoaOffset() != ExpOffsetMax) && (attr->getOffset() != ExpOffsetMax) &&
      attr->isSQLMXDiskFormat()) {
    if (attr->isSQLMXAlignedFormat())
      return (storeShortVoa(attr, heap));
    else
      return storeVoaValue(attr, attr->getVoaOffset(),
                           (attr->getOffset() - attr->getVCIndicatorLength() - attr->getNullIndicatorLength()), heap);
  }
...
PCIList PCode::storeVoaValue(Attributes *attr, UInt32 offset, UInt32 value, CollHeap *heap, short varOnly) {
  PCIList pciList(heap);
...
PCIList PCode::updateRowLen(Attributes *attr, CollHeap *heap, UInt32 flags) {
  ...
  if (attr->getVCIndicatorLength() > 0 && attr->isSQLMXDiskFormat()) {
    UInt32 comboLen1 = 0;
    char *comboPtr1 = (char *)&comboLen1;
    comboPtr1[0] = (char)attr->getNullIndicatorLength(), comboPtr1[1] = (char)attr->getVCIndicatorLength();

    UInt32 alignment =
        (attr->isSQLMXAlignedFormat() && (attr->getNextFieldIndex() == ExpOffsetMax)) ? ExpAlignedFormat::ALIGNMENT : 0;
...
PCIList PCode::copyVarRow(Attributes *dst, Attributes *src, UInt32 lastVOAoffset, Int16 lastVcIndicatorLength,
...
PCIList PCode::moveVarcharValue(Attributes *dst, Attributes *src, CollHeap *heap) {
  PCIList code(heap);
...
// source or destination can be "indirect"
// For disk format, it handles setting the VOA in the target.
PCIList PCode::moveVarcharFixedValue(Attributes *dst, Attributes *src, CollHeap *heap) {
  PCIList code(heap);
...
// source or destination can be "indirect"
// For disk format, it handles setting the VOA in the target.
PCIList PCode::moveFixedVarcharValue(Attributes *dst, Attributes *src, CollHeap *heap) {
  PCIList code(heap);
...
PCIList PCode::convertVarcharPtrToTarget(Attributes *dst, Attributes *src, CollHeap *heap) {
  PCIList code(heap);
  ...
  if (dst->getDatatype() == REC_FLOAT32) {
    ...
    AML aml(PCIT::MATTR5, PCIT::MATTR5);
    OL ol(dst->getAtp(), dst->getAtpIndex(), dst->getOffset(), dst->getVoaOffset(), dst->getLength(), comboLen2,
          src->getAtp(), src->getAtpIndex(), src->getOffset(), src->getVoaOffset(), src->getLength(), comboLen1);
    PCI pci(PCIT::Op_CONV_VC_PTR, aml, ol);
    code.append(pci);
  }
...
PCIID PCode::zeroFillNullValue(Attributes *dst, PCIList &code, PCIID notNullBranch, int genUncondJump) {
  ...
  if (dst->isVariableLength()) {
    ...
    if (dst->isSQLMXDiskFormat() || dst->isTrafodionDiskFormat())
      len = 0;
    else
      len = dst->getLength();
...
PCIList PCode::isNull(Attributes *attrDst, Attributes *attrSrc, CollHeap *heap) {
  PCIList pciList(heap);
...
PCIList PCode::isNotNull(Attributes *attrDst, Attributes *attrSrc, CollHeap *heap) {
  PCIList pciList(heap);
...
PCIID PCode::nullBranchHelper(AttributesPtr *attrs, Attributes *attrA, Attributes *attrB, PCIList &code) {
  PCIID nulBranch = 0;
  ...
  if (attrB) {
    ...
    if (tgt->getNullFlag()) {
      ...
      OL ol(tgt->getAtp(), tgt->getAtpIndex(), tgtNullIndOff, attrA->getAtp(), attrA->getAtpIndex(), attrANullIndOff,
            attrB->getAtp(), attrB->getAtpIndex(), attrBNullIndOff,
            EXPAND_PCODEATTRNULL3(*(UInt32 *)&tpf, tgt, attrA, attrB), 0);
  ...
  //
    ...
    if (tgt->getNullFlag()) {
      ...
      OL ol(tgt->getAtp(), tgt->getAtpIndex(), tgtNullIndOff, attrA->getAtp(), attrA->getAtpIndex(), attrANullIndOff,
            EXPAND_PCODEATTRNULL2(*(UInt32 *)&tpf, tgt, attrA), 0);
...
PCIID PCode::nullBranchHelperForComp(AttributesPtr *attrs, Attributes *attrA, Attributes *attrB,
  ...
  if (isSpecialNulls) {
    ...
    if (!attrs[1]->getNullFlag() || !attrs[2]->getNullFlag()) {
      ...
      // If the input arg is null, then output is null.
      AML aml(PCIT::MBIN32S, PCIT::MBIN32S, PCIT::IATTR3, PCIT::IBIN32S);
      OL ol(tgt->getAtp(), tgt->getAtpIndex(), tgt->getOffset(), src->getAtp(), src->getAtpIndex(), srcNullIndOff,
            EXPAND_PCODEATTRNULL2(*(UInt32 *)&tpf, tgt, src), 0);
  ...
  //
    ...
    // If either of the input args are null, then output is null.
    AML aml(PCIT::MBIN32S, PCIT::MBIN32S, PCIT::MBIN32S, PCIT::IATTR4, PCIT::IBIN32S);
    OL ol(tgt->getAtp(), tgt->getAtpIndex(), tgt->getOffset(), attrA->getAtp(), attrA->getAtpIndex(), attrANullIndOff,
          attrB->getAtp(), attrB->getAtpIndex(), attrBNullIndOff,
          EXPAND_PCODEATTRNULL3(*(UInt32 *)&tpf, tgt, attrA, attrB), 0);
...
PCIID PCode::nullBranchHelperForHash(AttributesPtr *attrs, Attributes *attrA, Attributes *attrB, PCIList &code) {
  PCIID nulBranch = 0;
  ...
  OL ol(tgt->getAtp(), tgt->getAtpIndex(), tgt->getOffset(), attrA->getAtp(), attrA->getAtpIndex(),
        attrA->getNullIndOffset(), attrA->getNullBitIndex(), ex_clause::NULL_HASH, 0);
...
PCIID PCode::nullBranch(ex_clause *clause, PCIList &code, AttributesPtr *attrs) {
  ...
  //
  if (!clause->isAnyOperandNullable()) return 0;
  ...
  if (attrA) {
    if ((clause->getType() == ex_clause::COMP_TYPE) || (clause->getType() == ex_clause::LIKE_TYPE))
      nulBranch = nullBranchHelperForComp(attrs, attrA, attrB, (clause->isSpecialNulls() != 0), code);
  ...
  if (!attrA && clause->isAnyOutputNullable()) {
    ...
    if (tgt->isSQLMXAlignedFormat()) {
      ...
      OL ol(tgt->getAtp(), tgt->getAtpIndex(), tgt->getNullIndOffset(), PCIT::NULL_BITMAP_SET,
            (Int16)tgt->getNullBitIndex(), 0);
