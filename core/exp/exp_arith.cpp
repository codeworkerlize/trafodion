  ...
  *ov = 0;
  ...
  if (quotient) {
    ...
    rc = convDoIt(temp_data[1], BigNum::BIGNUM_TEMP_LEN, REC_NUM_BIG_SIGNED, BigNum::BIGNUM_TEMP_PRECISION, 0,
                  temp_data[0], 8, REC_BIN64_SIGNED, 0, 0, NULL, 0, NULL, NULL);
    if (rc) {
      *ov = 1;
      return -1;
    }
...
static void CopyAttributes(char *dst, char *dstNull, char *dstVC, Attributes *dstAttr, char *src, char *srcNotNull,
                           char *srcVC, Attributes *srcAttr) {
  if (dstAttr->getNullFlag() && !srcNotNull)
    ExpTupleDesc::setNullValue(dstNull, dstAttr->getNullBitIndex(), dstAttr->getTupleFormat());
  else if (dstAttr->getNullFlag())
    ExpTupleDesc::clearNullValue(dstNull, dstAttr->getNullBitIndex(), dstAttr->getTupleFormat());

  if (srcNotNull) str_cpy_all(dst, src, dstAttr->getLength());
}
...
//
ex_expr::exp_return_type ex_arith_sum_clause::processNulls(char *null_data[], CollHeap *heap,
  ...
  //
  if (!null_data[1]) {
    CopyAttributes(null_data[2 * MAX_OPERANDS + 0], null_data[0], null_data[MAX_OPERANDS + 0], getOperand(0),
                   null_data[2 * MAX_OPERANDS + 2], null_data[2], null_data[MAX_OPERANDS + 2], getOperand(2));
    return ex_expr::EXPR_NULL;
  }
  ...
  if (!null_data[2]) {
    CopyAttributes(null_data[2 * MAX_OPERANDS + 0], null_data[0], null_data[MAX_OPERANDS + 0], getOperand(0),
                   null_data[2 * MAX_OPERANDS + 1], null_data[1], null_data[MAX_OPERANDS + 1], getOperand(1));
    return ex_expr::EXPR_NULL;
  }
...
//  N  N  do nothing (A=N)
//
ex_expr::exp_return_type ex_arith_count_clause::processNulls(char *null_data[], CollHeap *heap,
                                                             ComDiagsArea **diagsArea) {
...
ex_expr::exp_return_type ex_arith_clause::eval(char *op_data[], CollHeap *heap, ComDiagsArea **diagsArea) {
  switch (getInstruction()) {
    ...
    case ADD_FLOAT64_FLOAT64_FLOAT64: {
      short ov;
      *(double *)op_data[0] = MathReal64Add(*(double *)op_data[1], *(double *)op_data[2], &ov);
      if (ov) {
        ExRaiseSqlError(heap, diagsArea, EXE_NUMERIC_OVERFLOW);
        return ex_expr::EXPR_ERROR;
      }
    ...
    case ADD_DATETIME_INTERVAL_DATETIME:
      if (((ExpDatetime *)getOperand(0))
              ->arithDatetimeInterval(ExpDatetime::DATETIME_ADD, (ExpDatetime *)getOperand(1), getOperand(2),
                                      op_data[1], op_data[2], op_data[0], heap, diagsArea) != 0)
        return ex_expr::EXPR_ERROR;
      break;
    case ADD_INTERVAL_DATETIME_DATETIME:
      if (((ExpDatetime *)getOperand(0))
              ->arithDatetimeInterval(ExpDatetime::DATETIME_ADD, (ExpDatetime *)getOperand(2), getOperand(1),
                                      op_data[2], op_data[1], op_data[0], heap, diagsArea) != 0)
        return ex_expr::EXPR_ERROR;
      break;
    ...
    case SUB_FLOAT64_FLOAT64_FLOAT64: {
      short ov;
      *(double *)op_data[0] = MathReal64Sub(*(double *)op_data[1], *(double *)op_data[2], &ov);
      if (ov) {
        ExRaiseSqlError(heap, diagsArea, EXE_NUMERIC_OVERFLOW);
        return ex_expr::EXPR_ERROR;
      }
    ...
    case SUB_DATETIME_INTERVAL_DATETIME:
      if (((ExpDatetime *)getOperand(0))
              ->arithDatetimeInterval(ExpDatetime::DATETIME_SUB, (ExpDatetime *)getOperand(1), getOperand(2),
                                      op_data[1], op_data[2], op_data[0], heap, diagsArea) != 0)
        return ex_expr::EXPR_ERROR;
      break;
    case SUB_DATETIME_DATETIME_INTERVAL:
      if (((ExpDatetime *)getOperand(1))
              ->subDatetimeDatetime(getOperand(1), getOperand(0), op_data[1], op_data[2], op_data[0], heap,
                                    diagsArea) != 0)
        return ex_expr::EXPR_ERROR;
      break;
    ...
    case MUL_FLOAT64_FLOAT64_FLOAT64: {
      short ov;
      *(double *)op_data[0] = MathReal64Mul(*(double *)op_data[1], *(double *)op_data[2], &ov);
      if (ov) {
        ExRaiseSqlError(heap, diagsArea, EXE_NUMERIC_OVERFLOW);
        return ex_expr::EXPR_ERROR;
      }
    ...
    case DIV_FLOAT64_FLOAT64_FLOAT64: {
      ...
      short ov;
      *(double *)op_data[0] = MathReal64Div(*(double *)op_data[1], *(double *)op_data[2], &ov);
      if (ov) {
        ExRaiseSqlError(heap, diagsArea, EXE_NUMERIC_OVERFLOW);
        return ex_expr::EXPR_ERROR;
      }
    ...
    case DIV_COMPLEX:
      if (((ComplexType *)getOperand(0))->div(getOperand(1), getOperand(2), op_data, heap, diagsArea))
        return ex_expr::EXPR_ERROR;
      break;
...
ex_expr::exp_return_type ex_arith_clause::evalUnsupportedOperations(char *op_data[], CollHeap *heap,
                                                                    ComDiagsArea **diagsArea) {
