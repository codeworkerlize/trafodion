...
ex_expr::exp_return_type ex_expr::pCodeGenerate(Space *space, CollHeap *heap, UInt32 f) {
#ifdef _DEBUG
  if (getenv("NO_PCODE")) return ex_expr::EXPR_OK;
#endif
  ...
  if (!space) return ex_expr::EXPR_OK;
  ...
  if (pCodeMode_ & PCODE_EVAL) {
    while (clause) {
      ...
      for (short i = 0; i < clause->getNumOperands(); i++) {
        if (((clause->getOperand(i)->getOffset() == UINT_MAX) && !handleIndirectVC()) ||
  ...
  //
    ...
    while (clause) {
      ...
      // CQD says otherwise or if this operation is for insert/update.
      for (short i = 0; i < clause->getNumOperands(); i++) {
        if (!clause->getOperand(i)) continue;
      ...
      if (clause->getNumOperands() > 0) {
        if ((clause->getOperand()[0])->getVCIndicatorLength() > 0 && (clause->getOperand()[0])->isSQLMXDiskFormat() &&
...
#ifdef _DEBUG
  if (dumpPci == 1) PCode::dumpContents(code, buf, bufLen);
#endif
  ...
  for (pci = iter.first(); pci; pci = iter.next()) {
    if ((NOT containsClauseEval) && (PCode::IsClauseEvalInstruction(pci))) containsClauseEval = TRUE;
    ...
    if (PCode::IsBranchInstruction(pci)) {
      ...
      for (PCI *tpci = iter2.first(); tpci; tpci = iter2.next()) {
        ...
        //
        if (PCode::IsTargetInstruction(tpci) && (pci->getLongOperand(0) == tpci->getLongOperand(0))) {
      ...
      for (int i = 0, op = 0; i < pci->getNumberAddressingModes();
        ...
        if (pci->getOperand(op + 1) == 0)
          ...
          if (pci->getOperand(op) == 0)
            ...
            pci->setLongOperand(op + 2, pci->getLongOperand(op + 2) - persistentArea);
        }
      }
    }
  ...
  if (pCodeMode_ & PCODE_OPTIMIZE) {
    ...
    if ((iter.first()) && (iter.last() == iter.first()) &&
        (PCode::getInstruction(iter.first()) == PCIT::MOVE_MBIN8_MBIN8_IBIN32S))
    ...
    for (storePci = iter.first(); storePci; storePci = iter.next()) {
      ...
      if (PCode::getInstruction(storePci) == PCIT::MOVE_MBIN8_MBIN8_IBIN32S) {
        ...
        for (PCI *nextPci = iterNext.next(); nextPci && !(pCodeMode_ & PCODE_LLO); nextPci = iterNext.next()) {
          ...
          if ((nextPci->getOperand(0) == storePci->getOperand(0)) &&
              ...
              (nextPci->getOperand(2) == (storePci->getOperand(2) + storePci->getOperand(6))) &&
              (nextPci->getOperand(5) == (storePci->getOperand(5) + storePci->getOperand(6)))) {
            // The length of the combined MV is the sum
            // of the two MV's.
            //
            storePci->setOperand(6, storePci->getOperand(6) + nextPci->getOperand(6));
...
#endif
        {
          if (NOT doPCodeMoveFastpathOpt) {
            ...
            if (length == 1 || length == 2 || length == 4 || length == 8) {
              ...
              // length operand.
              //
              OL ol(storePci->getOperand(0), storePci->getOperand(1), storePci->getOperand(2), storePci->getOperand(3),
                    storePci->getOperand(4), storePci->getOperand(5));
...
#ifdef _DEBUG
  if (dumpPci == 1) PCode::dumpContents(pCode_.getPointer()->getPCodeBinary(), buf, bufLen);
#endif

  // get rid of the temporary object
  if (!ex_expr_base::forShowplan(f)) setPCodeObject(0);
...
#ifdef _DEBUG
  if (getenv("NO_PCODE_MOVE_FASTPATH")) return ex_expr::EXPR_OK;
#endif
  if (pc) {
    if ((pc[0] == 2) && (pc[5] == PCIT::MOVE_MBIN8_MBIN8_IBIN32S) &&
