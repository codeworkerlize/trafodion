...
#define EXP_SHIFT_XOR(op1, op2) ((((op1) << 3) | ((op1) >> 29)) ^ (op2))

// Clear the sign bit and toggle one other bit accordingly.
// result is UInt32
#define EXP_CLEAR_SIGNBIT(result) (((result)&0x80000000) ? ((result) ^ 0x80000002) : (result))
...
#define SCRAMBLE_3  0x2ec3
  ...
  result = 0;
  while (length >= 4) {  // longer than 4 bytes and not 8 bytes
    z = (*source++) * SCRAMBLE_0;
    result = EXP_SHIFT_XOR(result, z);
    length -= 4;
    if (length < 4) break;
  ...
  if (length == 1)
    ...
    z = *((UInt16 *)source);
    if (length == 3) z = (z << 8) + ((unsigned char *)source)[2];
  }
  z *= MULT_FACTOR;
  result = EXP_SHIFT_XOR(result, z);
  return EXP_CLEAR_SIGNBIT(result);
...
ex_expr::exp_return_type getDefaultValueForAddedColumn(UInt32 field_num,  // field number whose default value is to be
                                                       ...
                                                       Attributes *op, char **opdata, char **nulldata,
                                                       char **varlendata, CollHeap *heap, ComDiagsArea **diagsArea) {
  char *defaultValue = td->getAttr((Int16)field_num)->getDefaultValue();
  Attributes::DefaultClass dc = td->getAttr((Int16)field_num)->getDefaultClass();

  //
  // Default values are stored in the packed SQLMX_FORMAT.
  // This may be a different format than the attribute tuple data format.

  if ((dc == Attributes::NO_DEFAULT) || ((dc == Attributes::DEFAULT_NULL) && (op->getNullFlag() == 0))) {
    ...
    *nulldata = defaultValue;
    ...
    if (dc == Attributes::DEFAULT_NULL)
      *nulldata = NULL;
      ...
      if (ExpTupleDesc::isNullValue(defaultValue, op->getNullBitIndex(), ExpTupleDesc::SQLMX_FORMAT))
        *nulldata = NULL;
      else
  ...
  if (op->getVCIndicatorLength() > 0) *varlendata = defaultValue;
...
void computeDataPtr(char *start_data_ptr,  // start of data row
  ...
  if (((*op)->getNullFlag()) && (nulldata)) *nulldata = data_ptr;

  if (((*op)->getVCIndicatorLength() > 0) && (varlendata)) *varlendata = data_ptr + (*op)->getNullIndicatorLength();

  if (opdata) *opdata = data_ptr + (*op)->getNullIndicatorLength() + (*op)->getVCIndicatorLength();
}

void displayData(Int16 dataType, char *dataPtr) {
  switch (dataType) {
...
ex_expr::exp_return_type ex_expr::evalClauses(ex_clause *clause, atp_struct *atp1, atp_struct *atp2, atp_struct *atp3,
  ...
  UInt32 varOffset = ExpOffsetMax, voaOffset = 0, offset = 0, ff;
  Int16 varIndLen;
  Int16 nullIndLen;
  ...
  while (clause) {
...
#ifdef TRACE_EXPR_EVAL
    // break inside the block to have a chance to trace how the operands
    // are setup.
    ...
    for (Int16 i = 0; i < numOperands; i++, op++, opdata++, nulldata++, vardata++) {
      // -------------------------------------------------------------
      ...
      if ((atpix < 2) && (NOT atps[0]->getTupp(atpix).isAllocated())) {
        if (!getFixupConstsAndTemps()) {
          char *base;
          if (atpix == 0)
            base = getConstantsArea();
          else if (atpnum == 0)
            base = getTempsArea();
          else
            base = getPersistentArea();
        ...
        register char *dataPtr = (atp->getTupp(atpix)).getDataPointer();
        ...
        switch ((*op)->getTupleFormat()) {
          ...
          case ExpTupleDesc::PACKED_FORMAT: {
            ...
            if (((*op)->isAddedCol()) &&
                (((*op)->getOffset() == ExpOffsetMax) ||
                 ((datalen > 0) &&
                  ((((*op)->getVCIndicatorLength() > 0) && ((*op)->getOffset() > (UInt32)datalen)) ||
                   (((*op)->getVCIndicatorLength() <= 0) && ((*op)->getOffset() >= (UInt32)datalen)))))) {
              ...
              if ((*op)->getOffset() == ExpOffsetMax) {
                ...
                // default needed
                if ((datalen > 0) &&
                    ((*opdata > (dataPtr + datalen)) || ((*opdata == (dataPtr + datalen)) && (*vardata == NULL))))
                  defValNeeded = TRUE;
              } else
                defValNeeded = TRUE;
              ...
              if ((defValNeeded) && (mode == exp_READ)) {
                ...
                retcode = getDefaultValueForAddedColumn((*op)->getDefaultFieldNum(), tuppDesc, *op, &(*opdata),
                                                        &(*nulldata), &(*vardata), getHeap(), &diagsArea);
              ...
              //----------------------------------------------
              if ((lastFldIndex != NULL) && (!setLastFldIndex) &&  // last index not set before
            ...
            {
              if ((mode == exp_WRITE) && (rowLen) && (*op)->isSQLPackedFormat()) {
              ...
              if (dataPtr) {
                *opdata = dataPtr + (*op)->getOffset();
                ...
                if ((*op)->getVCIndicatorLength() > 0) *vardata = dataPtr + (*op)->getVCLenIndOffset();
              } else {
          ...
          case ExpTupleDesc::SQLMX_ALIGNED_FORMAT: {
            ...
            if (dataPtr) {
              UInt32 nullOff = 0;
              NABoolean isAlignedFormat = (*op)->isSQLMXAlignedFormat();
              offset = (*op)->getOffset();
              ...
              if (mode == exp_WRITE) {
                if (offset == ExpOffsetMax) {
                  ...
                  offset = varOffset;
                  ExpTupleDesc::setVoaValue(dataPtr, voaOffset, offset, tdf);
                  if (nullable) {
                    nullOff = (isAlignedFormat ? (*op)->getNullIndOffset() : offset);
                ...
                } else  // offsets computed at compile time
                {
                  if (varIndLen > 0) {
                    *vardata = dataPtr + (*op)->getVCLenIndOffset();
                    ...
                    currAttr = *op;
                    varOpFound = TRUE;
                  } else if ((!varOpFound) && rowLen && isAlignedFormat &&
                             ((offset >= varOffset) || (varOffset == ExpOffsetMax))) {
                  ...
                  // of this fields data
                  ExpTupleDesc::setVoaValue(dataPtr, voaOffset, offset - varIndLen - nullIndLen, tdf);

                  if (nullable) *nulldata = dataPtr + (*op)->getNullIndOffset();
                }
              ...
              } else  // read mode
              {
                ...
                *nulldata = dataPtr + (*op)->getNullIndOffset();
                ...
                // a tuple descriptor
                if (tuppDesc != NULL && tuppDesc->addedFieldPresent()) {
                  ff = ExpTupleDesc::getFirstFixedOffset(dataPtr, tdf);

                  if (Attributes::isDefaultValueNeeded((*op), tuppDesc, ff, varIndLen, voaOffset, dataPtr, datalen)) {
                    ComDiagsArea *diagsArea = atp1->getDiagsArea();
                    ...
                    break;
                  } else {  // not an added column needing special attn.
                    // but there may be other added columns present

                    // if variable field, then read the offset from
                    // the data record
                    if (varIndLen > 0) {
                      offset = ExpTupleDesc::getVoaOffset(dataPtr, voaOffset, tdf);
                    ...
                    // indicator bytes.
                    if (nullable) {
                      *nulldata = dataPtr + (isAlignedFormat ? ExpAlignedFormat::getBitmapOffset(dataPtr) : offset);
                      offset += nullIndLen;
                    }
                ...
                else if (varIndLen > 0 && ((*op)->isForceFixed()) && offset != ExpOffsetMax) {
                  if (nullable) *nulldata = (dataPtr + (isAlignedFormat ? (*op)->getNullIndOffset() : offset));
                } else if (varIndLen > 0) {
                  offset = ExpTupleDesc::getVoaOffset(dataPtr, voaOffset, tdf);
                  if (nullable) {
                    // null indicator does not change for aligned format
                    // but follows data for MX (packed) format
                    *nulldata = (dataPtr + (isAlignedFormat ? (*op)->getNullIndOffset() : offset));
                    offset += nullIndLen;
                  }
                }
                ...
                // length bytes.
                if (varIndLen > 0 && (*op)->isForceFixed() && offset != ExpOffsetMax) {
                  *vardata = dataPtr + offset - varIndLen;
                } else if (varIndLen > 0) {
                  *vardata = dataPtr + offset;
                  offset += varIndLen;
                }
      ...
      if (((*op)->getNullFlag()) &&  // nullable and
          ...
          (i > 0))                   // not result
      {
        if (ExpTupleDesc::isNullValue(*nulldata, (*op)->getNullBitIndex(), (*op)->getTupleFormat())) {
          (*nulldata) = 0;
        }
      }
    ...
    // -----------------------------------------------------------------
    ComDiagsArea *diagsArea = atp1->getDiagsArea();
    retcode = clause->useProcessNulls() ? clause->processNulls(op_data, getHeap(), &diagsArea) : ex_expr::EXPR_OK;
    ...
    if (retcode == ex_expr::EXPR_OK)  // do the work
    {
      ...
      if (clauseToDisplay == ct) {
        ...
        displayData(clause->getOperand(operandToDisplay)->getDatatype(), dataPtr);
      }
      ct++;
#endif

      if (clause->evalNeedAtps())
        retcode = clause->eval(&op_data[2 * ex_clause::MAX_OPERANDS], atp1, atp2, atp3, getHeap(), &diagsArea);
      else
        retcode = clause->eval(&op_data[2 * ex_clause::MAX_OPERANDS], getHeap(), &diagsArea);
    }

    if (retcode == ex_expr::EXPR_OK && clause->getOperType() == ITM_CONVERT &&
        ((ex_conv_clause *)clause)->getLastVOAoffset() > 0 && ((ex_conv_clause *)clause)->getComputedLength() > 0) {
      ...
      currAttr = *(clause->getOperand());
        ...
        if ((currAttr && retcode == ex_expr::EXPR_OK) ||
            (currAttr && (retcode == ex_expr::EXPR_NULL) &&
      ...
      // for aligned format.
      if (currAttr->isSQLMXAlignedFormat()) prevAttr = currAttr;
    }

    if (diagsArea && (diagsArea != atp1->getDiagsArea())) atp1->setDiagsArea(diagsArea);

    if (retcode == ex_expr::EXPR_ERROR) return retcode;
  ...
  if (rowLen && (varOffset != ExpOffsetMax)) {
    ...
    if (currAttr)
      ex_clause::evalSetRowLength(currAttr, atps[currAttr->getAtp()], rowLen, varOffset);
    else
      *rowLen = varOffset;
  }
  return retcode;
...
static ex_expr::exp_return_type alignAndEval(ex_clause *clause, char **op_data, atp_struct *atp1, atp_struct *atp2,
                                             atp_struct *atp3, CollHeap *heap, ComDiagsArea **diagsArea) {
  ex_expr::exp_return_type retCode = ex_expr::EXPR_OK;
  ...
  if (clause->evalNeedAtps())
    retCode = clause->eval(op_data, atp1, atp2, atp3, heap, diagsArea);
  else
...
#define PTR_TO_PCODE(the_type, name, pcode_offset) the_type *name = (the_type *)&pCode[pcode_offset];
...
#define BASE_PTR_DEF_ASSIGN(the_type, name, pcode_offset) the_type *name = (the_type *)stack[pCode[pcode_offset]];

// Define a new variable name of the type, and assign it a value from pCode
#define DEF_ASSIGN(the_type, name, pcode_offset) the_type name = (the_type)pCode[pcode_offset];

// Same as DEF_ASSIGN but handle pointer in pcode binary
#define DEF_ASSIGN_PTR(the_type, name, pcode_offset) the_type name = *(the_type *)&(pCode[pcode_offset]);

#define MOVE_INSTR(target_type, src_type)                                                       \
  *((target_type *)(stack[pCode[0]] + pCode[1])) = *((src_type *)(stack[pCode[2]] + pCode[3])); \
  pCode += 4;                                                                                   \
  break;

#define MOVE_CAST_INSTR(target_type, cast_type, src_type)                                                    \
  *((target_type *)(stack[pCode[0]] + pCode[1])) = (cast_type)(*((src_type *)(stack[pCode[2]] + pCode[3]))); \
  pCode += 4;                                                                                                \
  break;
...
int bignumSub(UInt16 *tgt, UInt16 *src1, UInt16 *src2, int length) {
  ...
  if (TRUE) {
    ...
    if (src1Sign != src2Sign) performAdd = FALSE;
  }
  ...
  if (origSrc1Sign) BIGN_SET_SIGN(origSrc1, length);

  if (origSrc2Sign) BIGN_SET_SIGN(origSrc2, length);
...
#ifdef _aarch64_
      const char *Start = static_cast<const char *>(pageStart);
      const char *End = Start + lenToAllow;
...
ex_expr::exp_return_type ex_expr_base::evalFast4(PCodeBinary *p, atp_struct *atp1, atp_struct *atp2, ex_expr *e) {
...
ex_expr::exp_return_type ex_expr_base::evalFast202(PCodeBinary *p, atp_struct *atp1, atp_struct *atp2, ex_expr *e) {
  SETUP_EVAL_STK(p, atp1, atp2, e);
#pragma refaligned 8 pCode
...
ex_expr::exp_return_type ex_expr_base::evalFast203(PCodeBinary *p, atp_struct *atp1, atp_struct *atp2, ex_expr *e) {
  SETUP_EVAL_STK(p, atp1, atp2, e);
#pragma refaligned 8 pCode
...
ex_expr::exp_return_type ex_expr_base::evalFast94(PCodeBinary *p, atp_struct *atp1, atp_struct *atp2, ex_expr *e) {
  SETUP_EVAL_STK(p, atp1, atp2, e);
#pragma refaligned 8 pCode
...
ex_expr::exp_return_type ex_expr_base::evalFast170(PCodeBinary *p, atp_struct *atp1, atp_struct *atp2, ex_expr *e) {
  SETUP_EVAL_STK(p, atp1, atp2, e);
#pragma refaligned 8 pCode
...
ex_expr::exp_return_type ex_expr_base::evalFast171(PCodeBinary *p, atp_struct *atp1, atp_struct *atp2, ex_expr *e) {
  SETUP_EVAL_STK(p, atp1, atp2, e);
#pragma refaligned 8 pCode
...
ex_expr::exp_return_type ex_expr_base::evalFast173(PCodeBinary *p, atp_struct *atp1, atp_struct *atp2, ex_expr *e) {
  SETUP_EVAL_STK(p, atp1, atp2, e);
#pragma refaligned 8 pCode
...
ex_expr::exp_return_type ex_expr_base::evalFast223(PCodeBinary *p, atp_struct *atp1, atp_struct *atp2, ex_expr *e) {
  SETUP_EVAL_STK(p, atp1, atp2, e);
#pragma refaligned 8 pCode
...
ex_expr::exp_return_type ex_expr_base::evalFast33(PCodeBinary *p, atp_struct *atp1, atp_struct *atp2, ex_expr *e) {
  SETUP_EVAL_STK(p, atp1, atp2, e);
#pragma refaligned 8 pCode
...
ex_expr::exp_return_type ex_expr_base::evalFast36(PCodeBinary *p, atp_struct *atp1, atp_struct *atp2, ex_expr *e) {
  SETUP_EVAL_STK(p, atp1, atp2, e);
#pragma refaligned 8 pCode
...
ex_expr::exp_return_type ex_expr_base::evalFast37(PCodeBinary *p, atp_struct *atp1, atp_struct *atp2, ex_expr *e) {
  SETUP_EVAL_STK(p, atp1, atp2, e);
#pragma refaligned 8 pCode
...
ex_expr::exp_return_type ex_expr_base::evalFast40(PCodeBinary *p, atp_struct *atp1, atp_struct *atp2, ex_expr *e) {
  SETUP_EVAL_STK(p, atp1, atp2, e);
#pragma refaligned 8 pCode
...
ex_expr::exp_return_type ex_expr_base::evalFast117(PCodeBinary *p, atp_struct *atp1, atp_struct *atp2, ex_expr *e) {
  SETUP_EVAL_STK(p, atp1, atp2, e);
#pragma refaligned 8 pCode
  ...
  if (x[0] != y[0]) return ex_expr::EXPR_FALSE;

  return (memcmp(x, y, pCode[6]) == 0) ? ex_expr::EXPR_TRUE : ex_expr::EXPR_FALSE;
}

ex_expr::exp_return_type ex_expr_base::evalFast130(PCodeBinary *p, atp_struct *atp1, atp_struct *atp2, ex_expr *e) {
  SETUP_EVAL_STK(p, atp1, atp2, e);
#pragma refaligned 8 pCode
...
ex_expr::exp_return_type ex_expr_base::evalFast131(PCodeBinary *p, atp_struct *atp1, atp_struct *atp2, ex_expr *e) {
  SETUP_EVAL_STK(p, atp1, atp2, e);
#pragma refaligned 8 pCode
...
ex_expr::exp_return_type ex_expr_base::evalFast132(PCodeBinary *p, atp_struct *atp1, atp_struct *atp2, ex_expr *e) {
  SETUP_EVAL_STK(p, atp1, atp2, e);
#pragma refaligned 8 pCode
...
ex_expr::exp_return_type ex_expr_base::evalFast133(PCodeBinary *p, atp_struct *atp1, atp_struct *atp2, ex_expr *e) {
  SETUP_EVAL_STK(p, atp1, atp2, e);
#pragma refaligned 8 pCode
...
ex_expr::exp_return_type ex_expr_base::evalFast162(PCodeBinary *p, atp_struct *atp1, atp_struct *atp2, ex_expr *e) {
  SETUP_EVAL_STK(p, atp1, atp2, e);
#pragma refaligned 8 pCode
...
ex_expr::exp_return_type ex_expr_base::evalFast262(PCodeBinary *p, atp_struct *atp1, atp_struct *atp2, ex_expr *e) {
  SETUP_EVAL_STK(p, atp1, atp2, e);
#pragma refaligned 8 pCode
  ...
  NABoolean srcNull = ExpTupleDesc::isNullValue((char *)(stack[pCode[2]] + pCode3), (Int16)pCode[6],
                                                (ExpTupleDesc::TupleDataFormat)pCode[5]);

  if (srcNull && isNullCheck) return ex_expr::EXPR_TRUE;
...
ex_expr::exp_return_type ex_expr_base::evalFast275(PCodeBinary *p, atp_struct *atp1, atp_struct *atp2, ex_expr *e) {
  SETUP_EVAL_STK(p, atp1, atp2, e);
#pragma refaligned 8 pCode
...
ex_expr::exp_return_type ex_expr::evalPCodeAligned(PCodeBinary *pCode32, atp_struct *atp1, atp_struct *atp2,
                                                   int *rowLen) {
  return ex_expr::EXPR_ERROR;
}
    ...
    switch (pCodeOpc) {
      ...
      case PCIT::OPDATA_MATTR5_IBIN32S: {
        ...
        do {
          ...
          if (pCodeOpc == PCIT::OPDATA_MPTR32_IBIN32S_IBIN32S) {
            ...
            NABoolean valueNull = ExpAlignedFormat::isNullValue((char *)src, (Int16)pCode[2]);
            ...
            if (vcLoc == ex_clause::MAX_OPERANDS) {
              ...
              dataPtr += ExpTupleDesc::getVarOffset(dataPtr, offset, voaOffset,
                                                    vcIndLen,   // vcIndLen
            ...
            if (pCodeOpc == PCIT::OPDATA_MBIN16U_IBIN32S) src = (char *)(*((UInt16 *)src) == 0);
      ...
      case PCIT::CLAUSE_EVAL:
        ...
        if (!(pCode[PCODEBINARIES_PER_PTR] && clause->processNulls(opData)))
          retCode = alignAndEval(clause, opDataData, atp1, atp2, atp3, getHeap(), &diagsArea);
      ...
      case PCIT::HASHCOMB_BULK_MBIN32U: {
        ...
        do {
          ...
          UInt32 h2 = (len2 == 0) ? *((UInt32 *)src2) : (UInt32)(result & 0xFFFFFFFF);
        ...
        combRes = hashVals[0];
        for (i = 1; i < numOfOps; i++) combRes = ((combRes << 1) | (combRes >> 31)) ^ hashVals[i];
      ...
      case PCIT::MOVE_MATTR5_MATTR5: {
        ...
        src += ExpTupleDesc::getVarOffset(src, srcOffset, srcVoaOffset,
                                          srcVCIndLen,   // vcIndLen
                                          srcNullIndLen  // nullIndLen
        );
        ...
        tgt += ExpTupleDesc::getTgtVarOffset(tgt,
                                             ...
                                             tgtVCIndLen, tgtNullIndLen, varOffset, copyLen);
      ...
      case PCIT::MOVE_MATTR5_MASCII_IBIN32S: {
        ...
        tgt +=
            ExpTupleDesc::getTgtVarOffset(tgt, tgtOffset, tgtVoaOffset, tgtVCIndLen, tgtNullIndLen, varOffset, copyLen);
      ...
      case PCIT::CONVVCPTR_MBIN64S_MATTR5_IBIN32S: {
        ...
        if (srcNumericVal == -1) {
          ...
          if (diagsArea != atp1->getDiagsArea()) atp1->setDiagsArea(diagsArea);
          if (er == ex_expr::EXPR_ERROR) return ex_expr::EXPR_ERROR;
        }
      ...
      case PCIT::CONVVCPTR_MFLT32_MATTR5_IBIN32S: {
        ...
        if (srcNumericVal == -1) {
          diagsArea = atp1->getDiagsArea();
          ex_expr::exp_return_type er =
              convDoIt(ptrSrc, srcLen, REC_BYTE_F_ASCII, 0, 0, (char *)&srcNumericVal, sizeof(double), REC_FLOAT32, 0,
                       0, NULL, 0, heap_, &diagsArea, CONV_ASCII_FLOAT64, NULL, 0);
          if (diagsArea != atp1->getDiagsArea()) atp1->setDiagsArea(diagsArea);
          if (er == ex_expr::EXPR_ERROR) return ex_expr::EXPR_ERROR;
        }
      ...
      case PCIT::CONVVCPTR_MATTR5_MATTR5: {
        ...
        if (copyLen < srcLen) {
          diagsArea = atp1->getDiagsArea();
          ex_expr::exp_return_type er =
              convDoIt(ptrSrc, srcLen, REC_BYTE_F_ASCII, 0, 0, (tgt + tgtVCIndLen), maxTgtLen, REC_BYTE_V_ASCII, 0, 0,
                       tgt, tgtVCIndLen, heap_, &diagsArea, CONV_ASCII_F_V, NULL, 0);
          if (diagsArea != atp1->getDiagsArea()) atp1->setDiagsArea(diagsArea);
          if (er == ex_expr::EXPR_ERROR) return ex_expr::EXPR_ERROR;
        } else {
          ExpTupleDesc::setVarLength(tgt, copyLen, tgtVCIndLen);  // set variable length value
      ...
      case PCIT::STRLEN_MBIN32U_MUNIV: {
        ...
        *tgt = (pCodeOpc == PCIT::STRLEN_MBIN32U_MATTR5) ? srcLen : (srcLen >> 1);
      ...
      case PCIT::LIKE_MBIN32S_MATTR5_MATTR5_IBIN32S_IBIN32S: {
        ...
        if (srcVCIndLen > 0) {
          srcStr += ExpTupleDesc::getVarOffset(srcStr, srcOffset, srcVoaOffset, srcVCIndLen, srcNullIndLen);
        ...
        if (patVCIndLen > 0) {
          patStr += ExpTupleDesc::getVarOffset(patStr, patOffset, patVoaOffset, patVCIndLen, patNullIndLen);
        ...
        for (int i = 0; i < numOfPatterns; i++) {
          ...
          if ((i == 0) && (flags & ex_like_clause_base::LIKE_HEAD)) fastCheck = TRUE;
          ...
          // Perform search
          pos = ex_function_position::findPosition(pat, patLen, tempSrc, tempSrcLen, fastCheck);
      ...
      case PCIT::POS_MBIN32S_MATTR5_MATTR5: {
        ...
        if (patLen == 0)
          ...
          pos = ex_function_position::findPosition(patStr, patLen, srcStr, srcLen, FALSE);
      ...
      case PCIT::CONCAT_MATTR5_MATTR5_MATTR5: {
        ...
        if (src1VCIndLen > 0) {
          src1 += ExpTupleDesc::getVarOffset(src1, src1Offset, src1VoaOffset, src1VCIndLen, src1NullIndLen);
        ...
        if (src2VCIndLen > 0) {
          src2 += ExpTupleDesc::getVarOffset(src2, src2Offset, src2VoaOffset, src2VCIndLen, src2NullIndLen);
        ...
        if (tgtVCIndLen > 0) {
          ...
          tgt += ExpTupleDesc::getTgtVarOffset(tgt, tgtOffset, tgtVoaOffset, tgtVCIndLen, tgtNullIndLen, varOffset,
                                               tgtLen);
      ...
      case PCIT::SUBSTR_MATTR5_MATTR5_MBIN32S_MBIN32S: {
        ...
        {
          src += ExpTupleDesc::getVarOffset(src, srcOffset, srcVoaOffset, srcVCIndLen, srcNullIndLen);
        ...
        tgt += ExpTupleDesc::getTgtVarOffset(tgt, tgtOffset, tgtVoaOffset, tgtVCIndLen, tgtNullIndLen, varOffset, len0);
        ...
        if (len0) str_cpy_all(tgt, src, len0);
      ...
      case PCIT::NULL_BYTES: {
        ...
        if (nullIndOffset == ExpOffsetMax) nullIndOffset = varOffset;
      ...
      case PCIT::NOT_NULL_BRANCH_MBIN32S_MBIN32S_MBIN32S_IATTR4_IBIN32S:
        ...
        // If neither source operands are null, then clear target null indicator.
        {
          ...
          {
            ExpTupleDesc::setNullValue((char *)(stack[pCode[0]] + op1NullIndOff), (Int16)attrs->op1NullBitIndex_,
      ...
      case PCIT::NULL_VIOLATION_MPTR32_MPTR32_IPTR_IPTR:
        ...
        // where c is a nullable column this gets pushed to the EID.
        {
          ...
          NABoolean arg1Null = ExpTupleDesc::isNullValue(ptr_0, arg1->getNullBitIndex(), arg1->getTupleFormat());
          NABoolean arg2Null =
              (arg2 == 0 ? FALSE : ExpTupleDesc::isNullValue(ptr_2, arg2->getNullBitIndex(), arg2->getTupleFormat()));
      ...
      case PCIT::UPDATE_ROWLEN3_MATTR5_IBIN32S: {
        {
          ...
          offset += ExpTupleDesc::getVarLength(data + offset, attrVCIndLen);
          ...
          if (rowLen != NULL) {
            ...
            // in aligned format
            if (newLen > 0) {
              offset = ExpAlignedFormat::adjustDataLength(data, offset, newLen);
            }
      ...
      case PCIT::COPYVARROW_MBIN8_MBIN8_IBIN32S_IBIN32S_IBIN32S_IBIN32S: {
        {
          ...
          {                           // case of null instantiated row
            ...
            copyLength = ExpTupleDesc::getVarOffset(srcData,
                                                    ...
                                                    attrVCIndLen, attrNullIndLen);

            copyLength += ExpTupleDesc::getVarLength(srcData + copyLength, attrVCIndLen);
      ...
      case PCIT::DIV_MBIN64S_MBIN64S_MBIN64S_ROUND: {
        ...
        if (divToDownscale) {
          ...
          temp = EXP_FIXED_OV_MUL(*xptr, 10, &ov);
          if (ov) {
          ...
          temp = EXP_FIXED_OV_DIV(temp, *yptr, &ov);
          if (ov) {
            // division by zero
            goto Error1_;
          }
                ...
                if (!divToDownscale) {
                  ...
                  if (biggerPrecision) {
                    ...
                    while (!vGT5) {
                      ...
                      rc = EXP_FIXED_BIGN_OV_MUL(&opST, &opST, op_data);
                      if (rc) {
                        // end of digits.
                        break;
                      }
                      ...
                      rc = EXP_FIXED_BIGN_OV_DIV(&opBN, &opST, op_data);
                      if (rc) {
                        // Something went wrong, lets consider
                        // it as end of digits.
                        break;
                      ...
                      op_data[0] = result2;
                      op_data[1] = (char *)&ten;
                      result3 = EXP_FIXED_BIGN_OV_MOD(&opBN, &opST, op_data, &ov);
                      if (ov) {
                        // end of digits.
                        break;
                      }
...
#else
        unsigned int fpsr;
        // Read the status bit from the fpsr register and put it into fpsr,
        // and then write to fpsr after clearing 0
        __asm__("mrs %[fpsr], FPSR" : [ fpsr ] "=r"(fpsr));
        fpsr &= ~(FE_OVERFLOW | FE_INVALID | FE_DIVBYZERO | FE_UNDERFLOW);
        __asm__("msr FPSR, %[src]" : : [ src ] "r"(fpsr));

        flt64_0 = flt64_1 + flt64_2;

        unsigned int cs;
        // Read out the results of relevant state bits in fpsr to memory
        __asm__("mrs %[cs], FPSR" : [ cs ] "=r"(cs));
#endif
      ...
      case PCIT::HASH_MBIN32U_MATTR5: {
        ...
        if (attrVCIndLen > 0)  // this check may not be needed
        {
          ...
          len = ExpTupleDesc::getVarLength(data + offset, attrVCIndLen);
          offset += attrVCIndLen;
        }
        ...
        if (pCodeOpc == PCIT::HASH_MBIN32U_MUNIV) {
          ...
          while ((len > 0) && (wstr[len - 1] == unicode_char_set::space_char())) len--;
      ...
      case PCIT::FILL_MEM_BYTES_VARIABLE: {
        ...
        if (copyLen > 0) str_pad(tgt, copyLen, padchar);
      ...
      case PCIT::COMP_MBIN32S_MATTR5_MATTR5_IBIN32S: {
        ...
        if (pCodeOpc == PCIT::COMP_MBIN32S_MUNIV_MUNIV_IBIN32S) {
          compCode = wcharStringCompareWithPad((NAWchar *)(src1Data), len1 >> 1, (NAWchar *)(src2Data), len2 >> 1,
      ...
      case PCIT::GENFUNC_PCODE_1: {
        switch (pCode[0]) {
          ...
          case ITM_RANDOMNUM: {
            ...
            if (seed < 1) {
              if (seed == 0) {
                // compiled value was -1, that means
                // initialize it with the thread id
                ...
                // numeric overflow, go back to 1
                seed = 1;

              QRLogger::log(CAT_SQL_EX_SPLIT_BOTTOM, LL_DEBUG, "ex_expr::evalPCode(): updated seed=%d\n", seed);
            }
            *resultPtr = seed;
        ...
        switch (subOpc) {
          ...
          case ITM_TRIM: {
            ...
            tgt += ExpTupleDesc::getTgtVarOffset(tgt,
                                                 tgtOffset,     // tgt offset
                                                 tgtVoaOffset,  // tgt voa offset
                                                 tgtVCIndLen, tgtNullIndLen, varOffset, len0);
            if (tgtVCIndLen > 0) {
              ExpTupleDesc::setVarLength(tgt, len0, tgtVCIndLen);
              tgt += tgtVCIndLen;
            }

            if (len0 > 0) str_cpy_all(tgt, &src[start], len0);
          ...
          case ITM_REPEAT: {
            ...
            if ((repeatCnt < 0) || (repeatCnt * srcLen > maxTargetLen)) goto Error1_;
      ...
      case PCIT::COMP_MBIN32S_MBIGS_MBIGS_IBIN32S_IBIN32S: {
        ...
        if (src1Sign != src2Sign) {
          ...
          for (int i = len16 - 1; i >= 0; i--) {
            ...
            compCode = (src1[i] > src2[i]) ? ((src1Sign) ? 0 : 2) : ((src1Sign) ? 2 : 0);
            break;
          }
        }

        // Reset sign bits
        if (src1Sign) BIGN_SET_SIGN(src1, len);

        if (src2Sign) BIGN_SET_SIGN(src2, len);
      ...
      case PCIT::MOVE_MBIGS_MBIGS_IBIN32S_IBIN32S: {
        ...
        for (; srcLength16 > tgtLength16; srcLength16--) {
          if (((Int16 *)src)[srcLength16 - 1] != 0) {
            ...
            if (srcSign) BIGN_SET_SIGN(src, srcLength);
            goto Error1_;
          }
        }
        ...
        if (tgtLength > srcLength)
          for (i = 0; i < tgtLength16; i++) ((Int16 *)tgt)[i] = 0;

        // Copy over shorts from bignum src
        for (i = 0; i < srcLength16; i++) ((UInt16 *)tgt)[i] = ((UInt16 *)src)[i];
      ...
      case PCIT::MOVE_MBIGS_MBIN64S_IBIN32S: {
        ...
        for (i = 0; i < tgtLength16; i++) tgt[i] = 0;
        ...
        *tgt64 = srcVal;
        ...
        if (isNeg) BIGN_SET_SIGN(tgt, tgtLength);
      ...
      case PCIT::MOVE_MBIN64S_MBIGS_IBIN32S: {
        ...
        *tgt = *((long *)src);
        ...
        if ((srcSign) && (*tgt != LLONG_MIN)) *tgt = -(*tgt);
      ...
      case PCIT::MUL_MBIGS_MBIGS_MBIGS_IBIN32S: {
        ...
        while ((leftLength16 > 0) && (src1[leftLength16 - 1] == 0)) leftLength16--;

        while ((rightLength16 > 0) && (src2[rightLength16 - 1] == 0)) rightLength16--;
        ...
        if ((leftLength16 != 0) && (rightLength16 != 0)) {
          ...
          for (j = 0; j < rightLength16; j++) {
            ...
            if (carry) tgt[pos] = carry;
          }
        }

        if (src1Sign) BIGN_SET_SIGN(src1, leftLength);

        if (src2Sign) BIGN_SET_SIGN(src2, rightLength);

        if (src1Sign != src2Sign) BIGN_SET_SIGN(tgt, tgtLength);
      ...
      case PCIT::SUB_MBIGS_MBIGS_MBIGS_IBIN32S: {
        ...
        if (pCodeOpc == PCIT::SUM_MATTR3_MATTR3_IBIN32S_MBIGS_MBIGS_IBIN32S) {
          ...
          if (isNullable) {
            ...
            isNull = indxSrc == -1 ? *srcNull != 0 : ExpAlignedFormat::isNullValue((char *)srcNull, indxSrc);
            ...
            isNull = indxTgt == -1 ? *tgtNull != 0 : ExpAlignedFormat::isNullValue((char *)tgtNull, indxTgt);
      ...
      case PCIT::SWITCH_MBIN32S_MBIN64S_MPTR32_IBIN32S_IBIN32S: {
        ...
        for (i = 0; (i < 4) && (vals[index] != PCodeCfg::INVALID_INT64); i++) {
          ...
          if (++index >= (UInt32)size) break;
        }

        // Get jump table location, using default case if item not found
        if (!isInList) compCode = ((compCode == 0) ? pcOffs[size] : pcOffs[index]);
      ...
      case PCIT::SWITCH_MBIN32S_MATTR5_MPTR32_IBIN32S_IBIN32S: {
        ...
        if (srcVCIndLen > 0) {
          src1 += ExpTupleDesc::getVarOffset(src1, offset, voaOffset,
                                             srcVCIndLen,   // vcIndLen
                                             srcNullIndLen  // nullIndLen
          );
...
**/
ex_expr::exp_return_type ex_expr_base::reportOverflowError(atp_struct *atp1, PCodeBinary *pCodeOpcPtr,
                                                           PCodeBinary *pCode, Long *stack) {
  ComDiagsArea *diagsArea = atp1->getDiagsArea();
  ...
  switch (instr) {
    ...
    case PCIT::MOVE_MBIN64S_MBIGS_IBIN32S:
      op2 = ((char *)(stack[pCode[2]] + pCode[3]));
      ExRaiseDetailSqlError(getHeap(), &diagsArea, EXE_NUMERIC_OVERFLOW, op2, pCode[4], REC_NUM_BIG_SIGNED, 0,
                            REC_BIN64_SIGNED, 0);
      if (diagsArea != atp1->getDiagsArea()) atp1->setDiagsArea(diagsArea);
      return ex_expr::EXPR_ERROR;
      break;
    ...
    case PCIT::MOVE_MBIGS_MBIGS_IBIN32S_IBIN32S:
      op2 = ((char *)(stack[pCode[2]] + pCode[3]));
      ExRaiseDetailSqlError(getHeap(), &diagsArea, EXE_NUMERIC_OVERFLOW, op2, pCode[5], REC_NUM_BIG_SIGNED, 0,
  ...
  ExRaiseDetailSqlError(getHeap(), &diagsArea, ovfl, (PCIT::Instruction)instr, op1, op2);

  if (diagsArea != atp1->getDiagsArea()) atp1->setDiagsArea(diagsArea);
  return ex_expr::EXPR_ERROR;
}

ex_expr::exp_return_type ex_clause::eval(char * /*op_data*/[], CollHeap *, ComDiagsArea **) {
  // default eval method does nothing and returns OK
  return ex_expr::EXPR_OK;
};

ex_expr::exp_return_type ex_clause::eval(char * /*op_data*/[], atp_struct *, atp_struct *, atp_struct *, CollHeap *,
                                         ComDiagsArea **) {
  // default eval method does nothing and returns OK
  return ex_expr::EXPR_OK;
};
