...
#define MathCot(op, err)         tan(M_PI_2 - op)
...
ex_expr::exp_return_type ex_function_abs::eval(char *op_data[], CollHeap *heap, ComDiagsArea **diagsArea) {
  ...
  switch (getOperand(1)->getDatatype()) {
    case REC_BIN8_SIGNED:
      *(Int8 *)op_data[0] = (*(Int8 *)op_data[1] < 0 ? -*(Int8 *)op_data[1] : *(Int8 *)op_data[1]);
      break;

    case REC_BIN16_SIGNED:
      *(short *)op_data[0] = (*(short *)op_data[1] < 0 ? -*(short *)op_data[1] : *(short *)op_data[1]);
      break;
    ...
    case REC_BIN64_SIGNED:
      if (*(long *)op_data[1] == LLONG_MIN) {
        /*
          ...
          in decimal precision. So only largeint type can occur for the case.
          Ref: Abs::synthesizeType.
        */
        char tmpbuf[80] = {0};
        sprintf(tmpbuf, "The error occurred in function Abs(%lld).", LLONG_MIN);
    ...
    case REC_NUM_BIG_SIGNED:
      str_cpy_all((char *)op_data[0], (char *)op_data[1], getOperand(1)->getLength());
      BIGN_CLR_SIGN(op_data[0], getOperand(1)->getLength());
      break;
    case REC_FLOAT64:
...
ex_expr::exp_return_type ExFunctionMath::evalUnsupportedOperations(char *op_data[], CollHeap *heap,
                                                                   ComDiagsArea **diagsArea) {
  ExRaiseSqlError(heap, diagsArea, EXE_INTERNAL_ERROR);
  return ex_expr::EXPR_ERROR;
}
ex_expr::exp_return_type ExFunctionMath::eval(char *op_data[], CollHeap *heap, ComDiagsArea **diagsArea) {
  ex_expr::exp_return_type retcode = ex_expr::EXPR_OK;
  ...
  if (getOperand()) {
    switch (getOperType()) {
      ...
      case ITM_ROUND:
        if ((getOperand(0)->getDatatype() != REC_FLOAT64) && (getOperand(0)->getDatatype() != REC_NUM_BIG_SIGNED) &&
            (getOperand(0)->getDatatype() != REC_NUM_BIG_UNSIGNED))
          return evalUnsupportedOperations(op_data, heap, diagsArea);
        break;

      default:
        if (getOperand(0)->getDatatype() != REC_FLOAT64) return evalUnsupportedOperations(op_data, heap, diagsArea);
        break;
    }
  }
  ...
  switch (getOperType()) {
    ...
    case ITM_ROUND: {
      if ((getOperand(1)->getDatatype() == REC_NUM_BIG_SIGNED) ||
        ...
        // Is digit at rounding position greater than 5?
          ...
          if (roundComptiableMode_ == 1) {
            // cqd mode_comptiable_1 is on
            res = (floor4 + 1.0) * floor5;
          } else {
    ...
    case ITM_ACOS:
      if ((*(double *)op_data[1] < -1) || (*(double *)op_data[1] > 1)) {
        ExRaiseSqlError(heap, diagsArea, EXE_BAD_ARG_TO_MATH_FUNC);
    ...
    case ITM_ATAN2:
      ...
      *(double *)op_data[0] = MathAtan2(*(double *)op_data[1], *(double *)op_data[2], err);
      break;
    ...
    case ITM_CEIL:
      ...
      switch (getOperand(0)->getDatatype()) {
        ...
        case REC_NUM_BIG_UNSIGNED: {
          ...
          if (convDoubleToBigNum(op_data[0], getOperand(0)->getLength(), getOperand(0)->getDatatype(),
                                 getOperand(0)->getPrecision(), getOperand(0)->getScale(), temp, heap,
    ...
    case ITM_COT:
      ...
      *(double *)op_data[0] = MathCot(*(double *)op_data[1], err);
      if ((*(double *)op_data[0] > COT_ERR_BOUND) || (*(double *)op_data[0] < -COT_ERR_BOUND)) {
        ExRaiseSqlError(heap, diagsArea, EXE_BAD_ARG_TO_MATH_FUNC);
        **diagsArea << DgString0("COT");
        return ex_expr::EXPR_ERROR;
      }
      break;
    ...
    case ITM_EXPONENT:
    case ITM_POWER:
      if (((*(double *)op_data[1] == 0) && (*(double *)op_data[2] <= 0)) ||
          ((*(double *)op_data[1] < 0) && (MathFloor(*(double *)op_data[2], err) != *(double *)op_data[2]))) {
        ExRaiseSqlError(heap, diagsArea, EXE_BAD_ARG_TO_MATH_FUNC);
        **diagsArea << DgString0("POWER");
        return ex_expr::EXPR_ERROR;
      }

      *(double *)op_data[0] = MathPow(*(double *)op_data[1], *(double *)op_data[2], err);
      if (errno == ERANGE) {
        // Overflow
...
ex_expr::exp_return_type ExFunctionBitOper::eval(char *op_data[], CollHeap *heap, ComDiagsArea **diagsArea) {
  ex_expr::exp_return_type retcode = ex_expr::EXPR_OK;
