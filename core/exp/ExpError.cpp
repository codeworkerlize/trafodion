...
ComDiagsArea *ExAddCondition(CollHeap *heap, ComDiagsArea **diagsArea, int err, ComCondition **newCond, int *intParam1,
  ...
  if (gEnableRowLevelLock &&
      ((intParam1 != NULL) &&
       (*intParam1 == -HBASE_LOCK_TIME_OUT_ERROR || *intParam1 == -HBASE_LOCK_ROLLBACK_ERROR ||
        *intParam1 == -HBASE_DEAD_LOCK_ERROR || *intParam1 == -HBASE_RPC_TIME_OUT_ERROR ||
        *intParam1 == -HBASE_CANCEL_OPERATION || *intParam1 == -HBASE_LOCK_REGION_MOVE_ERROR ||
        *intParam1 == -HBASE_LOCK_REGION_SPLIT_ERROR || *intParam1 == -HBASE_LOCK_NOT_ENOUGH_RESOURCE))) {
    return *diagsArea;
  }
  ...
  if (intParam2) cond->setOptionalInteger(1, *intParam2);
  if (intParam3) cond->setOptionalInteger(2, *intParam3);
  if (stringParam1) cond->setOptionalString(0, stringParam1);
  if (stringParam2) cond->setOptionalString(1, stringParam2);
  if (stringParam3) cond->setOptionalString(2, stringParam3);

  da->acceptNewCondition();

  if (newCond) *newCond = cond;
...
ComDiagsArea *ExRaiseFunctionSqlError(CollHeap *heap, ComDiagsArea **diagsArea, ExeErrorCode err,
                                      NABoolean derivedFunction, OperatorTypeEnum origOperType, ComCondition **cond) {
...
void ExConvertErrorToString(CollHeap *heap, ComDiagsArea **diagsArea, char *src, int srcLength, Int16 srcType,
  ...
  retCode = convDoIt(src,
                     ...
                     heap, diagsArea, CONV_UNKNOWN, 0, CONV_CONTROL_LOOPING | CONV_ALLOW_INVALID_CODE_VALUE);
...
ComDiagsArea *ExRaiseDetailSqlError(CollHeap *heap, ComDiagsArea **diagsArea, ExeErrorCode err, int pciInst, char *op1,
  ...
  // for details.
  char *buf = new (heap) char[(numAModes * (VALUE_TYPE_LEN + OPVALUE_LEN)) + INSTR_LEN + FORMATTING];
  ...
  switch (numAModes) {
    case 1:
      ExConvertErrorToString(heap, diagsArea, op1, PCIT::getOperandLengthForAddressingMode(am[0]),
                             PCIT::getDataTypeForMemoryAddressingMode(am[0]), 0, (char *)&opStrings[0], OPVALUE_LEN);

      // construct a formated string and assign to diags area
      **diagsArea << DgSqlCode(-err);

      if (rangeInst) {
        str_sprintf(buf, " Source Value:%s not within limits of Target Type:%s(%s).", &opStrings[0],
      ...
      str_sprintf(buf1, " Instruction:%s Operation:%s.", PCIT::instructionString((PCIT::Instruction)pciInst),
    ...
    case 2:
      ...
      ExConvertErrorToString(heap, diagsArea, op2, PCIT::getOperandLengthForAddressingMode(am[1]),
                             PCIT::getDataTypeForMemoryAddressingMode(am[1]), 0, (char *)&opStrings[OPVALUE_LEN],
                             OPVALUE_LEN);

      // construct a formated string and assign to diags area
      **diagsArea << DgSqlCode(-err);
      str_sprintf(buf, " %s Type:%s(%s) %s Value:%s", rangeInst ? "Source" : "Operand1",
                  getDatatypeAsString(PCIT::getDataTypeForMemoryAddressingMode(am[0]), true),
                  PCIT::addressingModeString(am[0]), rangeInst ? "Source" : "Operand1", &opStrings[0]);
      str_sprintf(buf1, " %s Type:%s(%s)%s%s Value:%s.", rangeInst ? "Target" : "Operand2",
...
ComDiagsArea *ExRaiseDetailSqlError(CollHeap *heap, ComDiagsArea **diagsArea, ExeErrorCode err, ex_clause *clause,
  ...
  // formatting. See defines at beginning of this header file
  // for details.
  char *buf = new (heap) char[(numOperands * (VALUE_TYPE_LEN + OPVALUE_LEN)) + INSTR_LEN + FORMATTING];
  ...
  for (i = 1; i < numOperands; i++) {
    ...
    ExConvertErrorToString(heap, diagsArea, (char *)op_data[i], op->getLength(), op->getDatatype(), op->getScale(),
                           (char *)&opStrings[(i - 1) * OPVALUE_LEN], OPVALUE_LEN);
  }
  ...
  for (i = 1; i < numOperands; i++) {
    op = clause->getOperand(i);
    str_sprintf(buf1, " Operand%d Type:%s(%s) Operand%d Value:%s", i, getDatatypeAsString(op->getDatatype(), true),
                getDatatypeAsString(op->getDatatype(), false), i, &opStrings[(i - 1) * OPVALUE_LEN]);
    str_cat(buf, buf1, buf);
  }
  if (numOperands) {
    str_sprintf(buf1, ".");
  ...
  str_sprintf(buf1, " Clause Type:%d Clause number:%d Operation:%s.", clause->getType(), clause->clauseNum(),
...
ComDiagsArea *ExRaiseDetailSqlError(CollHeap *heap, ComDiagsArea **diagsArea, ExeErrorCode err, char *src,
  ...
  ExConvertErrorToString(heap, diagsArea, src, srcLength, srcType, srcScale, opString, OPVALUE_LEN);
  ...
  if (srcPrecision != -1) {
    ...
    convertTypeToText_basic(srcTypeAsText, srcType, srcLength, srcPrecision, srcScale, startField, endField, 0, 0, 0, 0,
                            (CharInfo::CharSet)srcScale, NULL, NULL, 0, FALSE);
    strcpy(srcDatatypeDetail, getDatatypeAsString(srcType, false));
  } else if ((DFS2REC::isCharacterString(srcType)) && (srcLength >= 0) && (srcScale > 0) && (srcPrecision == -1))
    str_sprintf(srcDatatypeDetail, "%s,%d BYTES,%s", getDatatypeAsString(srcType, false), srcLength,
  ...
  if (tgtLength != -1) {
    ...
    convertTypeToText_basic(tgtTypeAsText, tgtType, tgtLength, tgtPrecision, tgtScale, startField, endField, 0, 0, 0, 0,
                            (CharInfo::CharSet)tgtScale, NULL, NULL, 0, FALSE);
    strcpy(tgtDatatypeDetail, getDatatypeAsString(tgtType, false));
  } else if ((DFS2REC::isCharacterString(tgtType)) && (tgtLength >= 0) && (tgtScale > 0))
    str_sprintf(tgtDatatypeDetail, "%s,%d %s,%s", getDatatypeAsString(tgtType, false),
                tgtPrecision ? tgtPrecision : tgtLength, tgtPrecision ? "CHARS" : "BYTES",
                (tgtScale == CharInfo::ISO88591 ? "ISO88591" : "UTF8"));
  else
    strcpy(tgtDatatypeDetail, getDatatypeAsString(tgtType, false));

  str_sprintf(buf, " %s of Source Type:%s(%s) Source Value:%s to Target Type:%s(%s).",
              intermediate ? "Intermediate conversion" : "Conversion",
              (strlen(srcTypeAsText) == 0 ? getDatatypeAsString(srcType, true) : srcTypeAsText), srcDatatypeDetail,
              sourceValue ? sourceValue : opString,
              (strlen(tgtTypeAsText) == 0 ? getDatatypeAsString(tgtType, true) : tgtTypeAsText), tgtDatatypeDetail);
