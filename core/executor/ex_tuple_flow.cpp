...
ex_tcb *ExTupleFlowTdb::build(ex_globals *glob) {
  ...
  ExTupleFlowTcb *tuple_flow_tcb = new (glob->getSpace()) ExTupleFlowTcb(*this, *src_tcb, *tgt_tcb, glob);
...
ExTupleFlowTcb::ExTupleFlowTcb(const ExTupleFlowTdb &tuple_flow_tdb,
  ...
  if (tuple_flow_tdb.tgtExpr_)
    pool_ = new (glob->getSpace())
        sql_buffer_pool(tuple_flow_tdb.numBuffers_, tuple_flow_tdb.bufferSize_, glob->getSpace());
  ...
  qParent_.down = new (space) ex_queue(ex_queue::DOWN_QUEUE, tuple_flow_tdb.queueSizeDown_, from_parent_cri, space);
  ...
  ExTupleFlowPrivateState *p = new (space) ExTupleFlowPrivateState(this);
  qParent_.down->allocatePstate(p, this);
  delete p;

  qParent_.up = new (space) ex_queue(ex_queue::UP_QUEUE, tuple_flow_tdb.queueSizeUp_, to_parent_cri, space);

  // fixup expressions
  if (tflowTdb().tgtExpr_)
    (void)tflowTdb().tgtExpr_->fixup(0, getExpressionMode(), this, glob->getSpace(), glob->getDefaultHeap(), FALSE,
                                     glob);
}
...
short ExTupleFlowTcb::work() {
  ...
  ExTupleFlowPrivateState &pstate = *((ExTupleFlowPrivateState *)pentry_down->pstate);

  if ((tflowTdb().userSidetreeInsert()) && (pentry_down->downState.request == ex_queue::GET_EOD) &&
      (NOT pstate.parentEOD_)) {
    pstate.step_ = MOVE_EOD_TO_TGT_;
  } else if ((pstate.step_ != DONE_) && (pstate.step_ != CANCELLED_) &&
  ...
  while (1) {
    switch (pstate.step_) {
      case EMPTY_: {
        if (qSrc_.down->isFull()) return WORK_OK;

        ex_queue_entry *src_entry = qSrc_.down->getTailEntry();

        src_entry->downState.request = pentry_down->downState.request;
        src_entry->downState.requestValue = pentry_down->downState.requestValue;

        if ((tflowTdb().firstNRows() >= 0) && (pentry_down->downState.request != ex_queue::GET_N)) {
          src_entry->downState.request = ex_queue::GET_N;
          src_entry->downState.requestValue = tflowTdb().firstNRows();
        }

        src_entry->downState.parentIndex = qParent_.down->getHeadIndex();
      ...
      case MOVE_SRC_TO_TGT_: {
        ...
        while ((!qSrc_.up->isEmpty()) && (!qTgt_.down->isFull()) && (pstate.step_ != HANDLE_ERROR_)) {
          ...
          switch (src_entry->upState.status) {
            ...
            case ex_queue::Q_NO_DATA: {
              if ((tflowTdb().vsbbInsertOn()) && (pstate.tgtRowsSent_ == TRUE)) {
                if (tflowTdb().userSidetreeInsert()) {
                  tgt_entry->downState.request = ex_queue::GET_EOD_NO_ST_COMMIT;
                } else {
              ...
              if ((pstate.tgtRowsSent_ == FALSE) && (src_entry->getDiagsArea())) {
                ...
                if (src_entry->getDiagsArea()) src_entry->getDiagsArea()->incrRefCount();
              ...
              if (tflowTdb().sendEODtoTgt()) pstate.step_ = MOVE_EOD_TO_TGT_;
            } break;
            ...
            case ex_queue::Q_SQLERROR: {
              ...
              if (accumulatedDiagsArea) {
                ...
                if (!(accumulatedDiagsArea->canAcceptMoreErrors()) && tflowTdb().isNonFatalErrorTolerated()) {
                  pstate.nonFatalErrorSeen_ = FALSE;
                  pstate.step_ = HANDLE_ERROR_;
                }
              } else {
              ...
              if (pstate.nonFatalErrorSeen_ == TRUE) qSrc_.up->removeHead();
            } break;

            case ex_queue::Q_REC_SKIPPED: {
              pstate.srcRequestCount_++;
              ComDiagsArea *da = src_entry->getDiagsArea();
              if (da) pstate.nonFatalErrorSeen_ = TRUE;
              qSrc_.up->removeHead();
            } break;
        ...
        if ((pstate.step_ != HANDLE_ERROR_) && (pstate.step_ != MOVE_EOD_TO_TGT_)) pstate.step_ = PROCESS_TGT_;
      }  // MOVE_SRC_TO_TGT
      break;
      ...
      case MOVE_EOD_TO_TGT_: {
        ...
        if (qTgt_.down->isFull()) return WORK_OK;

        ex_queue_entry *tgt_entry = qTgt_.down->getTailEntry();
        tgt_entry->downState.request = ex_queue::GET_EOD;

        tgt_entry->downState.requestValue = pentry_down->downState.requestValue;
        tgt_entry->downState.parentIndex = qParent_.down->getHeadIndex();
      ...
      case PROCESS_TGT_: {
        ...
        if (pstate.step_ == HANDLE_ERROR_) break;
        ...
        if (((pstate.srcEOD_ == TRUE) || (pstate.parentEOD_ == TRUE)) && (qTgt_.down->isEmpty()))
          pstate.step_ = DONE_;
        else {
          if (NOT pstate.parentEOD_) pstate.step_ = MOVE_SRC_TO_TGT_;
      ...
      case HANDLE_ERROR_: {
        ...
        ComDiagsArea *da = pentry->getDiagsArea();
        if (tflowTdb().isNonFatalErrorTolerated() && !(da->canAcceptMoreErrors())) {
          ComDiagsArea *cliDiagsArea = pentry_down->getDiagsArea();
          da->removeLastErrorCondition();
          *da << DgSqlCode(-EXE_NONATOMIC_FAILURE_LIMIT_EXCEEDED) << DgInt0(cliDiagsArea->getLengthLimit());
        }
...
ExTupleFlowPrivateState::ExTupleFlowPrivateState(const ExTupleFlowTcb * /*tcb*/) { init(); }
