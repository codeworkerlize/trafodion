...
TupleSpace::TupleSpace(ByteCount tupleLen, BufferCount nBuffers, ByteCount bufferSize, UInt32 memoryQuotaMB,
                       ex_expr *copyExpr, ex_cri_desc *criDesc, NAMemory *heap, bool enableOverflow,
                       ExExeStmtGlobals *stmtGlobals, UInt16 scratchThresholdPct, ScratchOverflowMode ovMode,
                       bool yieldQuota)
    : currentTuple_(0),
      nTuples_(0),
      atp_(NULL),
      copyExpr_(copyExpr),
      memory_(bufferSize, memoryQuotaMB, nBuffers, enableOverflow ? NUM_RESERVE_BUFFERS : 0, heap, stmtGlobals,
              yieldQuota),
      overflowEnabled_(enableOverflow),
      read_(bufferSize),
      readCache_(heap),
      start_(bufferSize),
...
IoStatus TupleSpace::advance(void) {
  IoStatus status = OK;
  ...
  if ((status == OK) && !newReadBuffer) {
    ...
    if ((status == OK) && read_.isValid()) {
      ++currentTuple_;
      ex_assert((canReadTuples()), "advance() failed to fetch next read buffer");
    }
  }
...
IoStatus TupleSpace::current(atp_struct *&currentAtp) {
...
Int16 TupleSpace::getLastError(void) { return swap_.getLastError(); }

Int16 TupleSpace::getLastSqlCode(void) { return swap_.getLastSqlCode(); }

ByteCount64 TupleSpace::getMaxMemory(void) const { return memory_.getMaxAllocation(); }

ByteCount64 TupleSpace::getMemory(void) const { return memory_.size(); }

UInt32 TupleSpace::getQuotaMB(void) const { return memory_.getQuotaMB(); }

IoStatus TupleSpace::insert(atp_struct *row, atp_struct **rtAtp) {
...
void TupleSpace::reacquireResources(void) { memory_.reinitialize(); }

void TupleSpace::releaseResources(void) { reset(true); }

IoStatus TupleSpace::rewind(void) {
  IoStatus status = OK;
...
bool TupleSpace::cacheWriteBuffer(void) {
#if defined(_DEBUG)
...
IoStatus TupleSpace::completeFetch(bool *newReadBuffer) {
  ...
  if ((status == OK) && readCompleted) {
    ...
    if (!read_.isValid()) {
      ...
      ++currentTuple_;
      ex_assert((canReadTuples()), "completeFetch() failed to fetch next read buffer");
    }
...
atp_struct *TupleSpace::getAtp(const BufferReference &br) {
  atp_struct *atp = NULL;
...
void TupleSpace::getInitialBuffer(void) { setInitialBuffer(memory_.getBuffer(true)); }

TupleSpace::ReadBufferType TupleSpace::getReadBufferType(void) {
...
void TupleSpace::setInitialBuffer(char *buffer) {
  start_.set(buffer);
  write_.set(buffer);
  read_.set(buffer);
}

IoStatus TupleSpace::makeRoom(void) {
...
IoStatus TupleSpace::nextReadBuffer(void) {
...
void TupleSpace::promoteSwapBuffer(void) {
...
void TupleSpace::reset(bool allDone) {
  if (nTuples_ > 0) {
    resetAtp();
    reuseCacheBuffers();
    nTuples_ = 0;
...
void TupleSpace::resetAtp(void) { atp_->getTupp(SAVED_DUP_ATP_INDEX).setDataPointer(NULL); }

void TupleSpace::reuseCacheBuffers(void) {
