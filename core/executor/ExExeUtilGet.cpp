...
ex_tcb *ExExeUtilGetMetadataInfoTdb::build(ex_globals *glob) {
  ...
  if ((groupBy() || orderBy()) || (queryType() == ComTdbExeUtilGetMetadataInfo::OBJECTS_ON_TABLE_) ||
      (queryType() == ComTdbExeUtilGetMetadataInfo::OBJECTS_IN_SCHEMA_))
    exe_util_tcb = new (glob->getSpace()) ExExeUtilGetMetadataInfoComplexTcb(*this, glob);
  // else if (getVersion())
  //   exe_util_tcb =
  //     new(glob->getSpace()) ExExeUtilGetMetadataInfoVersionTcb(*this, glob);
    ...
    exe_util_tcb = new (glob->getSpace()) ExExeUtilGetHbaseObjectsTcb(*this, glob);
    ...
    exe_util_tcb = new (glob->getSpace()) ExExeUtilGetNamespaceObjectsTcb(*this, glob);
  else
    exe_util_tcb = new (glob->getSpace()) ExExeUtilGetMetadataInfoTcb(*this, glob);
...
///////////////////////////////////////////////////////////////
ExExeUtilGetMetadataInfoTcb::ExExeUtilGetMetadataInfoTcb(const ComTdbExeUtilGetMetadataInfo &exe_util_tdb,
                                                         ex_globals *glob)
...
// ----------------------------------------------------------------------------
static void getBitExtractsForObjectType(const NAString objectLit, NAString &bitExtracts) {
  ...
  if (objectLit == COM_BASE_TABLE_OBJECT_LIT) {
    ...
    bitExtracts += " case when bitextract(privileges_bitmap,58,1) = 1 then 'R' else '-' end || ";
  ...
  // Views are not granted delete, usage, execute, or create.
    ...
    bitExtracts += "'-' || ";
    // bit for update (60)
    bitExtracts += " case when bitextract(privileges_bitmap,60,1) = 1 then 'U' else '-' end || ";
    // no bit for usage (59)
    bitExtracts += "'-' || ";
    // bit for references (58)
    bitExtracts += " case when bitextract(privileges_bitmap,58,1) = 1 then 'R' else '-' end || ";
  ...
  // Sequence generators are granted usage, alter, and drop
    ...
    bitExtracts = "'----' || ";
    // bit for usage (59)
    bitExtracts += " case when bitextract(privileges_bitmap,59,1) = 1 then 'G' else '-' end || ";
  ...
  else /*COM_USER_DEFINED_ROUTINE_OBJECT_LIT*/
  {
    ...
    bitExtracts = "'------' || ";
    // bit for execute(57)
    bitExtracts += " case when bitextract(privileges_bitmap,57,1) = 1 then 'E' else '-' end || ";
...
static const QueryString getUsersForRoleQuery[] = {{" select translate(rtrim(RU.grantee_name) using ucs2toutf8) "},
                                                   {"   from %s.\"%s\".%s RU "},
...
static const QueryString getRolesForUserQuery[] = {{" select translate(rtrim(RU.role_name) using ucs2toutf8) "},
                                                   {"   from %s.\"%s\".%s RU "},
                                                   {" where (RU.grantor_ID != -2) and "},
                                                   {"       (RU.grantee_name='%s' %s) "},
...
static const QueryString getComponents[] = {{" select distinct translate(rtrim(component_name) using ucs2toutf8)  "},
...
static const QueryString getCatalogsQuery[] = {
    {" select * from (values ('TRAFODION'), ('HIVE')) "}, {" order by 1 desc "}, {" ; "}};

static const QueryString getTrafTablesInSchemaQuery[] = {{" select %sobject_name%s from "},
                                                         {"   %s.\"%s\".%s O %s"},
...
static const QueryString getTrafIndexesInSchemaQuery[] = {{" select distinct %sO.object_name%s from "},
                                                          {"  %s.\"%s\".%s I, "},
                                                          {"   %s.\"%s\".%s O %s "},
                                                          {"  where O.catalog_name = '%s' and "},
                                                          {"        O.schema_name = '%s'  and "},
...
static const QueryString getTrafPackagesInSchemaQuery[] = {{" select distinct object_name  from "},
                                                           {"   %s.\"%s\".%s T %s "},
...
static const QueryString getTrafProceduresInSchemaQuery[] = {{" select distinct T.object_name  from "},
                                                             {"   %s.\"%s\".%s T, %s.\"%s\".%s R %s"},
                                                             {"  where T.catalog_name = '%s' and "},
                                                             {"        T.schema_name = '%s'  and "},
...
static const QueryString getTrafTriggersInSchemaQuery[] = {{" select object_name  from "},
                                                           {"   %s.\"%s\".%s T, %s.\"%s\".%s R "},
                                                           {"  where T.catalog_name = '%s' and "},
                                                           {"        T.schema_name = '%s'  and "},
...
static const QueryString getTrafLibrariesInSchemaQuery[] = {{" select distinct T.object_name  from "},
                                                            {" %s.\"%s\".%s T "},
                                                            {"  where T.catalog_name = '%s' "},
                                                            {"    and T.schema_name = '%s' "},
...
static const QueryString getTrafRoutinesForAuthQuery[] = {{" select distinct trim(T.catalog_name) || '.\"' || "},
                                                          {"     trim(T.schema_name) || '\".' || trim(T.object_name) "},
...
static const QueryString getTrafFunctionsInSchemaQuery[] = {{" select distinct T.object_name  from "},
                                                            {"   %s.\"%s\".%s T, %s.\"%s\".%s R %s "},
                                                            {"  where T.catalog_name = '%s' and "},
                                                            {"        T.schema_name = '%s'  and "},
                                                            {"        T.object_type = 'UR'  and "},
...
static const QueryString getTrafTableFunctionsInSchemaQuery[] = {{" select distinct T.object_name  from "},
                                                                 {"   %s.\"%s\".%s T, %s.\"%s\".%s R %s "},
                                                                 {"  where T.catalog_name = '%s' and "},
                                                                 {"        T.schema_name = '%s'  and "},
                                                                 {"        T.object_type = 'UR'  and "},
...
static const QueryString getTrafSequencesInSchemaQuery[] = {{" select distinct T.object_name  from "},
                                                            {"   %s.\"%s\".%s T %s "},
                                                            {"  where T.catalog_name = '%s' and "},
                                                            {"        T.schema_name = '%s'  and "},
                                                            {"        object_type = 'SG' %s "},
...
static const QueryString getTrafViewsInCatalogQuery[] = {{" select distinct T.schema_name || '.' || "},
                                                         {" T.object_name from "},
                                                         {"   %s.\"%s\".%s T %s"},
...
static const QueryString getTrafViewsInSchemaQuery[] = {{" select distinct T.object_name from "},
                                                        {"   %s.\"%s\".%s T %s"},
                                                        {"  where T.catalog_name = '%s' and "},
                                                        {"        T.schema_name = '%s' and "},
                                                        {"        T.object_type = 'VI' %s "},
...
static const QueryString getTrafSchemasInCatalogQuery[] = {{" select distinct schema_name "},
                                                           {"   from %s.\"%s\".%s T %s "},
...
static const QueryString getTrafUsers[] = {{" select distinct auth_db_name,auth_ext_name "},
                                           {"   from %s.\"%s\".%s "},
                                           {"  where auth_type = 'U' %s "},
                                           {" order by 1 "},
                                           {"  ; "}};

static const QueryString getTrafGroups[] = {{" select distinct auth_db_name "},
                                            {"   from %s.\"%s\".%s "},
                                            {"  where auth_type = 'G' %s "},
                                            {" order by 1 "},
                                            {"  ; "}};
...
static const QueryString getTrafRoles[] = {{" select distinct auth_db_name "},
                                           {"   from %s.\"%s\".%s "},
                                           {"  where auth_type = 'R' %s "},
                                           {" union select * from (values ('PUBLIC')) "},
                                           {" order by 1 "},
...
static const QueryString getTrafResourceUsages1[] = {{" select distinct resource_name "},
                                                     {"   from %s.\"%s\".%s "},
                                                     {" where usage_type = 'T' and usage_name = '%s' %s "},
                                                     {" order by 1 "},
                                                     {"  ; "}};

static const QueryString getTrafResourceUsages2[] = {{" select distinct usage_name "},
                                                     {"   from %s.\"%s\".%s "},
                                                     {" where usage_type = 'N' and resource_name = '%s' %s "},
                                                     {" order by 1 "},
                                                     {"  ; "}};

static const QueryString getTrafResources[] = {{" select distinct resource_name "},
                                               {"   from %s.\"%s\".%s "},
...
static const QueryString getTrafTenants[] = {{" select distinct auth_db_name "},
                                             {"   from %s.\"%s\".%s "},
                                             {"  where auth_type = 'T' %s "},
                                             {" order by 1 "},
                                             {"  ; "}};
  ...
  while (1) {
    switch (vStep_) {
      ...
      case VIEWS_FETCH_PROLOGUE_: {
        ...
        char query[2000];
        str_sprintf(query, "get all views on view %s.\"%s\".\"%s\", no header;", qualTableName.getCatalogName().data(),
                    qualTableName.getSchemaName().data(), qualTableName.getObjectName().data());
  ...
  while (1) {
    switch (vStep_) {
      ...
      case VIEWS_FETCH_PROLOGUE_: {
        ...
        str_sprintf(query, "get all %s in %s %s.%s.%s, no header", objectStr, inStr,
                    qualObjName.getCatalogName().data(), qualObjName.getUnqualifiedSchemaNameAsAnsiString().data(),
...
short ExExeUtilGetMetadataInfoTcb::displayHeading() {
  ...
  if ((qparent_.up->getSize() - qparent_.up->getLength()) < 7) return 1;  // come back later

  switch (getMItdb().queryType_) {
    case ComTdbExeUtilGetMetadataInfo::USERS_FOR_MEMBERS_: {
      str_sprintf(headingBuf_, "Members of User Group %s", getMItdb().getParam1());
    } break;
    ...
    case ComTdbExeUtilGetMetadataInfo::SCHEMAS_IN_CATALOG_: {
      str_sprintf(headingBuf_, "Schemas in Catalog %s", getMItdb().getCat());
    } break;

    case ComTdbExeUtilGetMetadataInfo::INVALID_VIEWS_IN_CATALOG_: {
      str_sprintf(headingBuf_, "Invalid Views in Catalog %s", getMItdb().getCat());
    } break;

    case ComTdbExeUtilGetMetadataInfo::TABLES_IN_CATALOG_: {
      str_sprintf(headingBuf_, "Tables in Catalog %s", getMItdb().getCat());
    } break;

    case ComTdbExeUtilGetMetadataInfo::VIEWS_IN_CATALOG_: {
      str_sprintf(headingBuf_, "Views in Catalog %s", getMItdb().getCat());
    } break;
    ...
    case ComTdbExeUtilGetMetadataInfo::HBASE_REG_TABLES_IN_CATALOG_: {
      str_sprintf(headingBuf_, "HBase Registered Tables in Catalog %s", getMItdb().getCat());
    } break;
    ...
    case ComTdbExeUtilGetMetadataInfo::HBASE_MAP_TABLES_IN_CATALOG_: {
      str_sprintf(headingBuf_, "HBase Mapped Tables in Catalog %s", getMItdb().getCat());
    } break;

    case ComTdbExeUtilGetMetadataInfo::TABLES_IN_SCHEMA_: {
      str_sprintf(headingBuf_, "Tables in Schema %s.%s", getMItdb().getCat(), getMItdb().getSch());
    } break;

    case ComTdbExeUtilGetMetadataInfo::INDEXES_IN_SCHEMA_: {
      str_sprintf(headingBuf_, "Indexes in Schema %s.%s", getMItdb().getCat(), getMItdb().getSch());
    } break;

    case ComTdbExeUtilGetMetadataInfo::VIEWS_IN_SCHEMA_: {
      str_sprintf(headingBuf_, "Views in Schema %s.%s", getMItdb().getCat(), getMItdb().getSch());
    } break;

    case ComTdbExeUtilGetMetadataInfo::OBJECTS_IN_SCHEMA_:
    case ComTdbExeUtilGetMetadataInfo::OBJECTNAMES_IN_SCHEMA_: {
      str_sprintf(headingBuf_, "Objects in Schema %s.%s", getMItdb().getCat(), getMItdb().getSch());
    } break;

    case ComTdbExeUtilGetMetadataInfo::INVALID_VIEWS_IN_SCHEMA_: {
      str_sprintf(headingBuf_, "Invalid Views in Schema %s.%s", getMItdb().getCat(), getMItdb().getSch());
    } break;

    case ComTdbExeUtilGetMetadataInfo::LIBRARIES_IN_SCHEMA_: {
      str_sprintf(headingBuf_, "Libraries in Schema %s.%s", getMItdb().getCat(), getMItdb().getSch());
    } break;

    case ComTdbExeUtilGetMetadataInfo::PACKAGES_IN_SCHEMA_: {
      str_sprintf(headingBuf_, "Packages in Schema %s.%s", getMItdb().getCat(), getMItdb().getSch());
    } break;

    case ComTdbExeUtilGetMetadataInfo::PROCEDURES_IN_SCHEMA_: {
      str_sprintf(headingBuf_, "Procedures in Schema %s.%s", getMItdb().getCat(), getMItdb().getSch());
    } break;

    case ComTdbExeUtilGetMetadataInfo::FUNCTIONS_IN_SCHEMA_: {
      str_sprintf(headingBuf_, "Functions in Schema %s.%s", getMItdb().getCat(), getMItdb().getSch());
    } break;

    case ComTdbExeUtilGetMetadataInfo::TABLE_FUNCTIONS_IN_SCHEMA_: {
      str_sprintf(headingBuf_, "Table_mapping functions in Schema %s.%s", getMItdb().getCat(), getMItdb().getSch());
    } break;

    case ComTdbExeUtilGetMetadataInfo::INDEXES_ON_TABLE_: {
      str_sprintf(headingBuf_, "Indexes on Table %s.%s", getMItdb().getSch(), getMItdb().getObj());
    } break;

    case ComTdbExeUtilGetMetadataInfo::PRIVILEGES_ON_SCHEMA_: {
      str_sprintf(headingBuf_, "Privileges on Schema %s.%s", getMItdb().getCat(), getMItdb().getSch());
    } break;

    case ComTdbExeUtilGetMetadataInfo::PRIVILEGES_ON_TABLE_: {
      str_sprintf(headingBuf_, "Privileges on Table %s.%s", getMItdb().getSch(), getMItdb().getObj());
    } break;

    case ComTdbExeUtilGetMetadataInfo::PRIVILEGES_ON_VIEW_: {
      str_sprintf(headingBuf_, "Privileges on View %s.%s", getMItdb().getSch(), getMItdb().getObj());
    } break;

    case ComTdbExeUtilGetMetadataInfo::PRIVILEGES_ON_SEQUENCE_: {
      str_sprintf(headingBuf_, "Privileges on Sequence %s.%s", getMItdb().getSch(), getMItdb().getObj());
    } break;

    case ComTdbExeUtilGetMetadataInfo::PRIVILEGES_ON_LIBRARY_: {
      str_sprintf(headingBuf_, "Privileges on Library %s.%s", getMItdb().getSch(), getMItdb().getObj());
    } break;

    case ComTdbExeUtilGetMetadataInfo::PRIVILEGES_ON_ROUTINE_: {
      str_sprintf(headingBuf_, "Privileges on Routine %s.%s", getMItdb().getSch(), getMItdb().getObj());
    } break;

    case ComTdbExeUtilGetMetadataInfo::VIEWS_ON_TABLE_:
    case ComTdbExeUtilGetMetadataInfo::VIEWS_ON_VIEW_: {
      str_sprintf(headingBuf_,
                  (getMItdb().queryType_ == ComTdbExeUtilGetMetadataInfo::VIEWS_ON_TABLE_ ? "Views on Table %s.%s"
                                                                                          : "Views ON View %s.%s"),
                  getMItdb().getSch(), getMItdb().getObj());
    } break;

    case ComTdbExeUtilGetMetadataInfo::PARTITIONS_FOR_TABLE_: {
      str_sprintf(headingBuf_, "Partitions for Table %s.%s", getMItdb().getSch(), getMItdb().getObj());
    } break;

    case ComTdbExeUtilGetMetadataInfo::PARTITIONS_FOR_INDEX_: {
      str_sprintf(headingBuf_, "Partitions for Index %s.%s", getMItdb().getSch(), getMItdb().getObj());
    } break;

    case ComTdbExeUtilGetMetadataInfo::OBJECTS_ON_TABLE_: {
      str_sprintf(headingBuf_, "Objects on Table %s.%s", getMItdb().getSch(), getMItdb().getObj());
    } break;

    case ComTdbExeUtilGetMetadataInfo::SEQUENCES_IN_SCHEMA_: {
      str_sprintf(headingBuf_, "Sequences in schema %s.%s", getMItdb().getCat(), getMItdb().getSch());
    } break;

    case ComTdbExeUtilGetMetadataInfo::SEQUENCES_IN_CATALOG_: {
      str_sprintf(headingBuf_, "Sequences in catalog %s", getMItdb().getCat());
    } break;
    ...
    case ComTdbExeUtilGetMetadataInfo::OBJECTS_IN_VIEW_: {
      str_sprintf(
          ...
          (getMItdb().queryType_ == ComTdbExeUtilGetMetadataInfo::TABLES_IN_VIEW_
               ? "Tables in View %s.%s"
               : (getMItdb().queryType_ == ComTdbExeUtilGetMetadataInfo::VIEWS_IN_VIEW_ ? "Views in View %s.%s"
                                                                                        : "Objects in View %s.%s")),
          getMItdb().getSch(), getMItdb().getObj());
    } break;
    ...
    case ComTdbExeUtilGetMetadataInfo::USERS_: {
      str_sprintf(headingBuf_,
                  (getMItdb().queryType_ == ComTdbExeUtilGetMetadataInfo::USERS_ ? "Users" : "Current User"));
    } break;
    ...
    case ComTdbExeUtilGetMetadataInfo::COMPONENT_PRIVILEGES_: {
      if (getMItdb().getParam1())
        str_sprintf(headingBuf_, "Privilege information on Component %s for %s", getMItdb().getObj(),
                    getMItdb().getParam1());

      else
        str_sprintf(headingBuf_, "Operation information on Component %s", getMItdb().getObj());
      break;
    }

    case ComTdbExeUtilGetMetadataInfo::TRIGGERS_IN_SCHEMA_: {
      str_sprintf(headingBuf_, "Triggers in Schema %s.%s", getMItdb().getCat(), getMItdb().getSch());
    } break;
...
//
// ----------------------------------------------------------------------------
static NAString addFromClause(const NAString &authList, const bool &schemaNamePred) {
  char buf[authList.length() * 3 + MAX_SQL_IDENTIFIER_NAME_LEN * 9 + 200];
  NAString cmd;

  if (schemaNamePred)
    snprintf(buf, sizeof(buf),
             ", (select ob.object_uid "
             ...
             TRAFODION_SYSTEM_CATALOG, SEABASE_PRIVMGR_SCHEMA, PRIVMGR_SCHEMA_PRIVILEGES, authList.data(),
             TRAFODION_SYSTEM_CATALOG, SEABASE_MD_SCHEMA, SEABASE_OBJECTS, TRAFODION_SYSTEM_CATALOG,
             SEABASE_PRIVMGR_SCHEMA, PRIVMGR_OBJECT_PRIVILEGES, authList.data(), TRAFODION_SYSTEM_CATALOG,
             SEABASE_PRIVMGR_SCHEMA, PRIVMGR_COLUMN_PRIVILEGES, authList.data());
    ...
    snprintf(buf, sizeof(buf),
             ...
             " ))) as p(object_uid) ",
             TRAFODION_SYSTEM_CATALOG, SEABASE_PRIVMGR_SCHEMA, PRIVMGR_SCHEMA_PRIVILEGES, authList.data(),
             TRAFODION_SYSTEM_CATALOG, SEABASE_PRIVMGR_SCHEMA, PRIVMGR_OBJECT_PRIVILEGES, authList.data(),
             TRAFODION_SYSTEM_CATALOG, SEABASE_PRIVMGR_SCHEMA, PRIVMGR_COLUMN_PRIVILEGES, authList.data());
...
//
// ----------------------------------------------------------------------------
static NAString addWhereClause(const bool &schemaNamePred, const char *uidColumn) {
  NAString cmd;
  if (schemaNamePred)
    cmd += "and p.p_uid = ";
  else
...
// ----------------------------------------------------------------------------
NAString ExExeUtilGetMetadataInfoTcb::getGrantedPrivCmd(const NAString &authList, const char *cat,
                                                        const bool &includeSchemaObjs = false,
                                                        const NAString &qualifier, const NAString &uidColumn,
  ...
  if (includeSchemaObjs) {
    ...
    snprintf(buf, sizeof(buf),
             ...
             "   and grantee_id in %s ))",
             qualifier.data(), uidColumn.data(), cat, SEABASE_PRIVMGR_SCHEMA, PRIVMGR_OBJECT_PRIVILEGES,
             authList.data(), cat, SEABASE_PRIVMGR_SCHEMA, PRIVMGR_COLUMN_PRIVILEGES, authList.data(), cat,
             SEABASE_PRIVMGR_SCHEMA, PRIVMGR_SCHEMA_PRIVILEGES, (qualifier2) ? qualifier2 : qualifier.data(),
    ...
    snprintf(buf, sizeof(buf),
             ...
             qualifier.data(), uidColumn.data(), cat, SEABASE_PRIVMGR_SCHEMA, PRIVMGR_OBJECT_PRIVILEGES,
             authList.data(), cat, SEABASE_PRIVMGR_SCHEMA, PRIVMGR_COLUMN_PRIVILEGES, authList.data(), cat,
             SEABASE_PRIVMGR_SCHEMA, PRIVMGR_SCHEMA_PRIVILEGES, authList.data());
    cmd = buf;
  }
  return cmd;
}
...
char *ExExeUtilGetMetadataInfoTcb::getRoleList(bool &containsRootRole, const int userID, const char *catName,
  ...
  if (CmpSeabaseDDLauth::isUserID(userID) && enabled)
    sprintf(queryBuf_,
            "select distinct role_id from %s.\"%s\".%s ru left join "
    ...
    sprintf(queryBuf_, "select role_id from %s.\"%s\".%s where grantee_id = %d ", catName, schName, objName, userID);
...
long ExExeUtilGetMetadataInfoTcb::getObjectUID(const char *catName, const char *schName, const char *objName,
  ...
  sprintf(queryBuf_,
          ...
          catName, schName, objName, getMItdb().getCat(), getMItdb().getSch(), targetName, type);
...
// ----------------------------------------------------------------------------
NABoolean ExExeUtilGetMetadataInfoTcb::checkUserPrivs(ContextCli *currContext,
                                                      const ComTdbExeUtilGetMetadataInfo::QueryType queryType) {
  // if no authorization, everyone sees everything
  if (!CmpCommon::context()->isAuthorizationEnabled()) return FALSE;
  ...
  switch (queryType) {
    ...
    case ComTdbExeUtilGetMetadataInfo::ROLES_FOR_USER_:
    case ComTdbExeUtilGetMetadataInfo::ROLES_FOR_GROUP_: {
      if (componentPrivileges.hasSQLPriv(ComUser::getCurrentUser(), SQLOperation::MANAGE_ROLES, true)) return FALSE;
      break;
    }
    ...
    case ComTdbExeUtilGetMetadataInfo::USERS_:
    case ComTdbExeUtilGetMetadataInfo::USERS_FOR_ROLE_: {
      if (componentPrivileges.hasSQLPriv(ComUser::getCurrentUser(), SQLOperation::MANAGE_USERS, true)) return FALSE;
      break;
    }
    ...
    case ComTdbExeUtilGetMetadataInfo::USERS_FOR_MEMBERS_:
    case ComTdbExeUtilGetMetadataInfo::GROUPS_FOR_USER_: {
      if (componentPrivileges.hasSQLPriv(ComUser::getCurrentUser(), SQLOperation::MANAGE_GROUPS, true)) return FALSE;
      break;
    }
    ...
    case ComTdbExeUtilGetMetadataInfo::FUNCTIONS_FOR_LIBRARY_:
    case ComTdbExeUtilGetMetadataInfo::TABLE_FUNCTIONS_FOR_LIBRARY_: {
      if (componentPrivileges.hasSQLPriv(ComUser::getCurrentUser(), SQLOperation::MANAGE_LIBRARY, true)) return FALSE;
      break;
    }
    ...
    case ComTdbExeUtilGetMetadataInfo::NODES_IN_RGROUP_:
    case ComTdbExeUtilGetMetadataInfo::TENANTS_ON_RGROUP_: {
      if (componentPrivileges.hasSQLPriv(ComUser::getCurrentUser(), SQLOperation::MANAGE_RESOURCE_GROUPS, true) ||
          componentPrivileges.hasSQLPriv(ComUser::getCurrentUser(), SQLOperation::MANAGE_TENANTS, true))
        return FALSE;
      break;
    }
    ...
    case ComTdbExeUtilGetMetadataInfo::TENANTS_:
    case ComTdbExeUtilGetMetadataInfo::GROUPS_FOR_TENANT_: {
      if (componentPrivileges.hasSQLPriv(ComUser::getCurrentUser(), SQLOperation::MANAGE_TENANTS, true)) return FALSE;
      break;
    }
...
int ExExeUtilGetMetadataInfoTcb::colPrivsFrag(const char *authName, const char *cat, const NAString &privWhereClause,
  ...
  sprintf(queryBuf_,
          ...
          "from %s.\"%s\".%s",
          likeClause.data(), privWhereClause.data(), privWhereClause.data(), cat, SEABASE_PRIVMGR_SCHEMA,
  ...
  snprintf(msg, len,
           ...
           authName, privWhereClause.data(), hasGrants, (hasHive && hasGrants));
  ...
  if (hasGrants) {
    ...
    snprintf(buf, sizeof(buf), gluedQuery, cat, SEABASE_PRIVMGR_SCHEMA, PRIVMGR_COLUMN_PRIVILEGES, cat,
             SEABASE_MD_SCHEMA, SEABASE_COLUMNS, privWhereClause.data());
    colPrivsStmt = buf;
    ...
    if (hasHive) {
      ...
      snprintf(buf, sizeof(buf), gluedQuery, cat, SEABASE_PRIVMGR_SCHEMA, PRIVMGR_COLUMN_PRIVILEGES, cat,
               SEABASE_MD_SCHEMA, SEABASE_OBJECTS, privWhereClause.data());
      colPrivsStmt += buf;
      NADELETEBASIC(gluedQuery, getMyHeap());
    }
  }
    ...
    switch (step_) {
      ...
      case SETUP_HBASE_QUERY_: {
        ...
        if (getMItdb().returnFullyQualNames()) {
          str_sprintf(catSchValue, "'\"%s\".\"%s\".\"' || ", getMItdb().getCat(), getMItdb().getSch());
        ...
        if (getMItdb().queryType_ == ComTdbExeUtilGetMetadataInfo::TENANTS_ ||
          ...
          if (!enabled) {
            ExRaiseSqlError(getHeap(), &diagsArea_, -4222, NULL, NULL, NULL, "multi-tenant");
            step_ = HANDLE_ERROR_;
            break;
          }
        }
        ...
        if (getMItdb().queryType_ == ComTdbExeUtilGetMetadataInfo::GROUPS_ ||
          ...
          if (!enabled) {
            ExRaiseSqlError(getHeap(), &diagsArea_, -4222, NULL, NULL, NULL, "advanced");
            step_ = HANDLE_ERROR_;
            break;
          }
        }
        ...
        switch (getMItdb().queryType_) {
          case ComTdbExeUtilGetMetadataInfo::AUTHZ_STATUS_: {
            ...
            moveRowToUpQueue(outputBuf_);
            moveRowToUpQueue("");
            sprintf(outputBuf_, "Authorization %s.",
                    CmpCommon::context()->isAuthorizationEnabled() ? "enabled" : "disabled");
            moveRowToUpQueue(outputBuf_);
            moveRowToUpQueue(" ");
          ...
          case ComTdbExeUtilGetMetadataInfo::USERS_FOR_MEMBERS_: {
            ...
            if (-1 == CmpSeabaseDDLauth::getUserGroupMembers(getMItdb().getParam1(), memberList)) {
              ExRaiseSqlError(getHeap(), &diagsArea_, -CAT_IS_NOT_A_GROUP, NULL, NULL, NULL, getMItdb().getParam1());
              step_ = HANDLE_ERROR_;
            } else {
              if (memberList.empty()) {
                step_ = ENABLE_CQS_;
                ...
                for (std::vector<int32_t>::iterator itor = memberList.begin(); itor != memberList.end(); itor++) {
                  // for gcc 4.4.x
          ...
          case ComTdbExeUtilGetMetadataInfo::PASSWORD_POLICY_: {
            ...
            if (COM_LDAP_AUTH == CmpCommon::getAuthenticationType()) {
              ...
              ExRaiseSqlError(getHeap(), &diagsArea_, -CAT_NOT_ALLOWED_OPERATION_ON_LDAP_AUTH, NULL, NULL, NULL);
              step_ = HANDLE_ERROR_;
              continue;
            }
            /*
          ...
          */
            ...
            str_sprintf(sb, "Length for password %d - %d chars", syntax.min_password_length,
                        syntax.max_password_length);
            moveRowToUpQueue(sb);
            if (syntax.digital_chars_number == 0xFF && syntax.lower_chars_number == 0xFF &&
              ...
              moveRowToUpQueue(
                  "* Contains at least three or more of them: upper character, lower character, digit and special "
                  "symbols.");
            } else {
              if (syntax.digital_chars_number < 0xee || syntax.lower_chars_number < 0xee ||
                ...
                if (syntax.digital_chars_number < 0xee) moveRowToUpQueue("* Digital");
                if (syntax.symbol_chars_number < 0xee) moveRowToUpQueue("* Symbol Char");
                if (syntax.upper_chars_number < 0xee) moveRowToUpQueue("* Upper Char");
                if (syntax.lower_chars_number < 0xee) moveRowToUpQueue("* Lower Char");
              }
              if (syntax.digital_chars_number == 0xee || syntax.lower_chars_number == 0xee ||
                  syntax.symbol_chars_number == 0xee || syntax.upper_chars_number == 0xee) {
          ...
          case ComTdbExeUtilGetMetadataInfo::HBASE_REG_TABLES_IN_CATALOG_: {
            ...
            if (doPrivCheck) privWhereClause = getGrantedPrivCmd(authList, cat, false);
          ...
          case ComTdbExeUtilGetMetadataInfo::SCHEMAS_FOR_ROLE_: {
            ...
            if (!CmpSeabaseDDLauth::isRoleID(authID) && !ComUser::isPublicUserID(authID)) {
              ExRaiseSqlError(getHeap(), &diagsArea_, -CAT_IS_NOT_A_ROLE, NULL, NULL, NULL, getMItdb().getParam1());
              step_ = HANDLE_ERROR_;
              break;
            }
          ...
          case ComTdbExeUtilGetMetadataInfo::TRIGGERS_IN_SCHEMA_: {
            ...
            if (doPrivCheck) privWhereClause = getGrantedPrivCmd(authList, cat, true, NAString("T."));
          ...
          case ComTdbExeUtilGetMetadataInfo::LIBRARIES_IN_SCHEMA_: {
            ...
            if (doPrivCheck) {
              ...
              snprintf(buf, stmtSize,
                       ...
                       "          replace (p.schema_name, '\"', ''))"
                       "        and grantee_id in %s )))) ",
                       cat, pmsch, objPrivs, authList.data(), cat, pmsch, schPrivs, authList.data(), cat, sch,
                       library_usage, cat, pmsch, objPrivs, authList.data(), cat, pmsch, schPrivs, authList.data());
              privWhereClause = buf;
            }
          ...
          case ComTdbExeUtilGetMetadataInfo::NODES_FOR_TENANT_: {
            ...
            if (doPrivCheck) {
              ...
              if (strcmp(getMItdb().getParam1(), currContext->getTenantName()) != 0) {
                ...
                msg += ").";
                ExRaiseSqlError(getHeap(), &diagsArea_, -4218, NULL, NULL, NULL, "NODES FOR TENANT", msg.data());
                step_ = HANDLE_ERROR_;
                break;
              }
            }
          ...
          case ComTdbExeUtilGetMetadataInfo::NODES_IN_RGROUP_: {
            ...
            if (doPrivCheck) {
              ...
              str_sprintf(buf,
                          " and (resource_uid in (select resource_uid "
                          " from %s.\"%s\".%s where resource_creator = %d) or "
                          " (resource_uid in (select usage_uid from %s.\"%s\".%s "
                          " where tenant_id = %d))) ",
                          cat, tsch, resources, ComUser::getCurrentUser(), cat, tsch, tenant_usage,
          ...
          case ComTdbExeUtilGetMetadataInfo::RGROUPS_: {
            ...
            if (doPrivCheck) {
              char buf[COL_MAX_TABLE_LEN * 3 + 200];
              str_sprintf(buf,
                          " and (resource_creator = %d or "
                          "(resource_uid in (select usage_uid "
                          "from %s.\"%s\".%s "
                          "where tenant_id = %d))) ",
                          ComUser::getCurrentUser(), cat, tsch, tenant_usage, ComTenant::getCurrentTenantID());
              privWhereClause = buf;
            }
          ...
          case ComTdbExeUtilGetMetadataInfo::NODES_: {
            ...
            if (doPrivCheck) {
              char buf[COL_MAX_TABLE_LEN * 3 + 200];
              str_sprintf(buf,
                          " and resource_uid in (select resource_uid from %s.\"%s\".%s "
                          "where usage_uid = %d)",
                          cat, tsch, resource_usage, ComTenant::getCurrentTenantID());
              privWhereClause = buf;
            }
          ...
          case ComTdbExeUtilGetMetadataInfo::TENANTS_: {
            ...
            if (doPrivCheck) {
              char buf[1000];
              str_sprintf(buf,
                          " and auth_id in (select tenant_id from %s.\"%s\".%s "
          ...
          case ComTdbExeUtilGetMetadataInfo::TENANTS_ON_RGROUP_: {
            ...
            if (doPrivCheck) {
              ...
              str_sprintf(buf,
                          ...
                          ComTenant::getCurrentTenantID(), ComUser::getCurrentUser(), cat, tsch, resources,
                          getMItdb().getParam1());
              privWhereClause += buf;
            }
            param_[0] = cat;
          ...
          case ComTdbExeUtilGetMetadataInfo::GROUPS_FOR_TENANT_: {
            ...
            if (doPrivCheck) {
              char buf[authList.length() + 100];
              str_sprintf(buf, " and auth_id  = %d ", ComTenant::getCurrentTenantID());
              privWhereClause += buf;
            }
          ...
          case ComTdbExeUtilGetMetadataInfo::ROLES_FOR_GROUP_: {
            ...
            if (CmpSeabaseDDLauth::isUserID(authID)) {
              if (getMItdb().queryType_ == ComTdbExeUtilGetMetadataInfo::ROLES_FOR_GROUP_) {
                ExRaiseSqlError(getHeap(), &diagsArea_, -CAT_IS_NOT_CORRECT_AUTHID, NULL, NULL, NULL,
            ...
            if (CmpCommon::getDefault(ALLOW_ROLE_GRANTS_TO_GROUPS) == DF_ON ||
                (CmpCommon::getDefault(ALLOW_ROLE_GRANTS_TO_GROUPS) == DF_SYSTEM && !getenv("SQLMX_REGRESS")))
              getGroupList((CmpSeabaseDDLauth::isUserID(authID)) ? getMItdb().getParam1() : currUser, groupList);
            ...
            if (!groupList.isNull()) groupList.prepend(" or grantee_id in ");
          ...
          case ComTdbExeUtilGetMetadataInfo::VIEWS_FOR_USER_: {
            ...
            if ((CmpCommon::getDefault(TRAF_GET_HIVE_OBJECTS) == DF_ON) &&
              ...
              if (getMItdb().queryType_ == ComTdbExeUtilGetMetadataInfo::TABLES_FOR_USER_)
                privWhereClause +=
                    "union select 'HIVE',  upper(trim(schema_name)), "
          ...
          case ComTdbExeUtilGetMetadataInfo::LIBRARIES_FOR_USER_: {
            ...
            if (!ComUser::isRootUserID(authID) && !hasRootRole) {
              ...
              snprintf(buf, sizeof(buf),
                       ...
                       "          replace (p.schema_name, '\"', ''))"
                       "        and grantee_id in %s )))) ",
                       cat, pmsch, objPrivs, userRoleList, cat, pmsch, schPrivs, userRoleList, cat, sch, library_usage,
                       cat, pmsch, objPrivs, userRoleList, cat, pmsch, schPrivs, userRoleList);
              privWhereClause = buf;
            }

            if (authID != ComUser::getCurrentUser()) NADELETEBASIC(userRoleList, getHeap());
          ...
          case ComTdbExeUtilGetMetadataInfo::LIBRARIES_FOR_ROLE_: {
            ...
            if (authID != ROOT_ROLE_ID) {
              ...
              snprintf(buf, sizeof(buf),
                       ...
                       "          replace (p.schema_name, '\"', ''))"
                       "        and grantee_id = %d)))) ",
                       cat, pmsch, objPrivs, authID, cat, pmsch, schPrivs, authID, cat, sch, library_usage, cat, pmsch,
                       objPrivs, authID, cat, pmsch, schPrivs, authID);
              privWhereClause = buf;
            }
          ...
          default: {
            ExRaiseSqlError(getHeap(), &diagsArea_, -4218, NULL, NULL, NULL, "GET");
            step_ = HANDLE_ERROR_;
          } break;
        }

        if (step_ == HANDLE_ERROR_) break;
        ...
        str_sprintf(queryBuf_, gluedQuery, param_[0], param_[1], param_[2], param_[3], param_[4], param_[5], param_[6],
                    param_[7], param_[8], param_[9], param_[10], param_[11], param_[12], param_[13], param_[14],
                    param_[15], param_[16], param_[17], param_[18], param_[19], param_[20], param_[21]);
        NADELETEBASIC(gluedQuery, getMyHeap());
      ...
      case FETCH_ALL_ROWS_: {
        ...
        if (fetchAllRows(infoList_, queryBuf_, numOutputEntries_, FALSE, retcode) < 0) {
          cliInterface()->allocAndRetrieveSQLDiagnostics(diagsArea_);
          step_ = HANDLE_ERROR_;
          break;
        }
      ...
      case RETURN_ROW_: {
        ...
        if (getMItdb().queryType_ == ComTdbExeUtilGetMetadataInfo::ACTIVE_ROLES_) {
          ...
          while (outputStr.length() < 40) outputStr.append(' ');
...
// Not supported at this time
        ...
        if (exprRetCode == ex_expr::EXPR_TRUE) exprRetCode = evalScanExpr(ptr, len, TRUE);
        ...
        if ((getMItdb().queryType_ == ComTdbExeUtilGetMetadataInfo::TABLES_IN_VIEW_) ||
            ...
            (getMItdb().queryType_ == ComTdbExeUtilGetMetadataInfo::VIEWS_IN_VIEW_)) {
          QualifiedName qn(vi->get(2), vi->get(1), vi->get(0));
          objectName = qn.getCatalogName() + "." + qn.getUnqualifiedSchemaNameAsAnsiString() + "." +
                       qn.getUnqualifiedObjectNameAsAnsiString();

          if ((strcmp(vi->get(3), "BT") == 0) && (ComIsTrafodionExternalSchemaName(qn.getSchemaName()))) {
            objectName = ComConvertTrafNameToNativeName(qn.getCatalogName(), qn.getUnqualifiedSchemaNameAsAnsiString(),
                                                        qn.getUnqualifiedObjectNameAsAnsiString());
          }
...
///////////////////////////////////////////////////////////////
ExExeUtilGetMetadataInfoComplexTcb::ExExeUtilGetMetadataInfoComplexTcb(const ComTdbExeUtilGetMetadataInfo &exe_util_tdb,
                                                                       ex_globals *glob)
  ...
  while (1) {
    switch (step_) {
      ...
      case SETUP_QUERY_: {
        patternStr_[0] = '\0';
        if (getMItdb().getPattern()) {
          str_sprintf(patternStr_, ", match '%s' ", getMItdb().getPattern());
        }
        ...
        switch (getMItdb().queryType_) {
          case ComTdbExeUtilGetMetadataInfo::VIEWS_ON_TABLE_: {
            str_sprintf(queryBuf_,
                        "select * from (get all views on table \"%s\".\"%s\".\"%s\", no header %s %s) x(a) group by a "
                        "order by 1",
                        getMItdb().getCat(), getMItdb().getSch(), getMItdb().getObj(), rfn, patternStr_);
          } break;

          case ComTdbExeUtilGetMetadataInfo::VIEWS_ON_VIEW_: {
            str_sprintf(queryBuf_,
                        "select * from (get all views on view \"%s\".\"%s\".\"%s\", no header %s %s) xxx(aaa) group by "
                        "aaa order by 1",
                        getMItdb().getCat(), getMItdb().getSch(), getMItdb().getObj(), rfn, patternStr_);
          } break;
          ...
          case ComTdbExeUtilGetMetadataInfo::TABLES_IN_VIEW_: {
            str_sprintf(queryBuf_,
                        "select * from (get all tables in view \"%s\".\"%s\".\"%s\", no header %s) xxx(aaa) group by "
                        "aaa order by 1",
                        getMItdb().getCat(), getMItdb().getSch(), getMItdb().getObj(), patternStr_);
          } break;

          case ComTdbExeUtilGetMetadataInfo::VIEWS_IN_VIEW_: {
            str_sprintf(queryBuf_,
                        "select * from (get all views in view \"%s\".\"%s\".\"%s\", no header %s) xxx(aaa) group by "
                        "aaa order by 1",
                        getMItdb().getCat(), getMItdb().getSch(), getMItdb().getObj(), patternStr_);
          } break;
          ...
          case ComTdbExeUtilGetMetadataInfo::OBJECTS_IN_VIEW_: {
            str_sprintf(queryBuf_,
                        "select * from (get all objects in view \"%s\".\"%s\".\"%s\", no header %s) xxx(aaa) group by "
                        "aaa order by 1",
                        getMItdb().getCat(), getMItdb().getSch(), getMItdb().getObj(), patternStr_);
          } break;
...
// not supported at this time
...
#endif
          default: {
            ExRaiseSqlError(getHeap(), &diagsArea_, -4298, NULL, NULL, NULL, "GET");
            step_ = HANDLE_ERROR_;
          } break;
      ...
      case FETCH_ALL_ROWS_FOR_OBJECTS_: {
        ...
        str_sprintf(queryBuf_, "get indexes on table \"%s\".\"%s\".\"%s\" %s", getMItdb().getCat(), getMItdb().getSch(),
                    getMItdb().getObj(), patternStr_);
        ...
        // Get views on table
        str_sprintf(queryBuf_, "get %s views on table \"%s\".\"%s\".\"%s\" %s", ausStr, getMItdb().getCat(),
                    getMItdb().getSch(), getMItdb().getObj(), patternStr_);
        ...
        // Get mvs on table
        str_sprintf(queryBuf_, "get %s mvs on table \"%s\".\"%s\".\"%s\" %s", ausStr, getMItdb().getCat(),
                    getMItdb().getSch(), getMItdb().getObj(), patternStr_);
        ...
        str_sprintf(queryBuf_, "get synonyms on table \"%s\".\"%s\".\"%s\" %s", getMItdb().getCat(),
                    getMItdb().getSch(), getMItdb().getObj(), patternStr_);
        ...
        if (rowsFound) infoList_->removeTail();
      ...
      case FETCH_ALL_ROWS_IN_SCHEMA_: {
        ...
        const char *objectTypes[numTypes] = {"tables",    "views",      "indexes",   "sequences",
                                             "libraries", "procedures", "functions", "table_mapping functions",
                                             "packages"
        ...
        for (int ot = 0; ot < numTypes; ot++) {
          ...
          str_sprintf(queryBuf_, "get %s in schema \"%s\".\"%s\" %s", objectType.data(), getMItdb().getCat(),
                      getMItdb().getSch(), patternStr_);
...
///////////////////////////////////////////////////////////////
ExExeUtilGetHbaseObjectsTcb::ExExeUtilGetHbaseObjectsTcb(const ComTdbExeUtilGetMetadataInfo &exe_util_tdb,
                                                         ex_globals *glob)
    : ExExeUtilGetMetadataInfoTcb(exe_util_tdb, glob) {
  ...
  ehi_ = ExpHbaseInterface::newInstance(glob->getDefaultHeap(), (char *)exe_util_tdb.server(),
                                        (char *)exe_util_tdb.zkPort(), storageType,
                                        FALSE);  // replSync

  hbaseName_ = NULL;
  hbaseNameBuf_ = new (getGlobals()->getDefaultHeap()) char[ComMAX_3_PART_EXTERNAL_UTF8_NAME_LEN_IN_BYTES + 6 + 1];
  outBuf_ = new (getGlobals()->getDefaultHeap()) char[ComMAX_3_PART_EXTERNAL_UTF8_NAME_LEN_IN_BYTES + 6 + 1];
...
ExExeUtilGetHbaseObjectsTcb::~ExExeUtilGetHbaseObjectsTcb() {
  ...
  if (hbaseNameBuf_) NADELETEBASIC(hbaseNameBuf_, getGlobals()->getDefaultHeap());

  if (outBuf_) NADELETEBASIC(outBuf_, getGlobals()->getDefaultHeap());
}
  ...
  while (1) {
    switch (step_) {
      ...
      case SETUP_HBASE_QUERY_: {
        ...
        if (((currContext->getSqlParserFlags() & 0x20000) == 0) && !ComUser::isRootUserID() &&
            !ComUser::currentUserHasRole(ROOT_ROLE_ID) && !ComUser::currentUserHasRole(HBASE_ROLE_ID)) {
          step_ = DONE_;
          break;
        }
      ...
      case PROCESS_NEXT_ROW_: {
        ...
        if (numParts >= 2) schemaNamePart = parts[1];
        NAString objectNamePart;
        if (numParts >= 3) objectNamePart = parts[2];

        if (ComIsTrafodionReservedSchema("", catalogNamePart, schemaNamePart)) {
          sysObj = TRUE;
        }

        if ((getMItdb().externalObjs()) && (externalObj)) {
          step_ = EVAL_EXPR_;
          break;
        } else if ((getMItdb().systemObjs()) && (sysObj)) {
          step_ = EVAL_EXPR_;
          break;
        } else if ((getMItdb().userObjs()) && ((NOT sysObj) && (NOT externalObj))) {
          step_ = EVAL_EXPR_;
          break;
        }
...
///////////////////////////////////////////////////////////////
ExExeUtilGetNamespaceObjectsTcb::ExExeUtilGetNamespaceObjectsTcb(const ComTdbExeUtilGetMetadataInfo &exe_util_tdb,
                                                                 ex_globals *glob)
    : ExExeUtilGetMetadataInfoTcb(exe_util_tdb, glob) {
  int jniDebugPort = 0;
    ...
    switch (step_) {
      case INITIAL_: {
        ...
        if (!ComUser::currentUserHasElevatedPrivs(NULL)) {
          NAString privMDLoc =
              NAString(TRAFODION_SYSTEM_CATALOG) + NAString(".\"") + NAString(SEABASE_PRIVMGR_SCHEMA) + NAString("\"");
      ...
      case CHECK_NAMESPACE_EXISTS_: {
        retcode = ehi_->namespaceOperation(COM_CHECK_NAMESPACE_EXISTS, getMItdb().getObj(), 0, NULL, NULL, NULL);

        if (retcode == -HBC_ERROR_NAMESPACE_NOT_EXIST) {
          ExRaiseSqlError(getHeap(), &diagsArea_, -1067, NULL, NULL, NULL, getMItdb().getObj());
          step_ = HANDLE_ERROR_;
          break;
        }
      ...
      case GET_NAMESPACES_: {
        retcode = ehi_->namespaceOperation(COM_GET_NAMESPACES, NULL, 0, NULL, NULL, &namespaceObjects_);

        if (ExHbaseAccessTcb::setupError((NAHeap *)getMyHeap(), qparent_, retcode,
      ...
      case GET_OBJECTS_IN_NAMESPACE_: {
        retcode =
            ehi_->namespaceOperation(COM_GET_NAMESPACE_OBJECTS, getMItdb().getObj(), 0, NULL, NULL, &namespaceObjects_);
      ...
      case GET_NAMESPACE_CONFIG_: {
        retcode =
            ehi_->namespaceOperation(COM_GET_NAMESPACE_CONFIG, getMItdb().getObj(), 0, NULL, NULL, &namespaceObjects_);
      ...
      case PROCESS_NEXT_NAMESPACE_: {
        ...
        if (((getMItdb().queryType() == ComTdbExeUtilGetMetadataInfo::TRAFODION_NAMESPACES_) && (NOT isTraf)) ||
            ((getMItdb().queryType() == ComTdbExeUtilGetMetadataInfo::EXTERNAL_NAMESPACES_) && (NOT isExt)) ||
            ((getMItdb().queryType() == ComTdbExeUtilGetMetadataInfo::SYSTEM_NAMESPACES_) && (NOT isSys))) {
          step_ = PROCESS_NEXT_ROW_;
          break;
        }
...
ExExeUtilShowSetTcb::ExExeUtilShowSetTcb(const ComTdbExeUtilShowSet &exe_util_tdb, ex_globals *glob)
  ...
  while (1) {
    switch (step_) {
      ...
      case RETURN_HEADER_: {
        ...
        // Come back later.
        if ((qparent_.up->getSize() - qparent_.up->getLength()) < 3) return -1;
      ...
      case RETURNING_DEFAULT_: {
        ...
        while ((NOT eof) && (NOT isExternalized)) {
          ...
          if (ssTdb().getType() == ComTdbExeUtilShowSet::ALL_) isExternalized = TRUE;
        }
        ...
        strcpy(formattedStr, "  ");
        byte_str_cpy(&formattedStr[2], 28, attributeString, strlen(attributeString), ' ');
        formattedStr[2 + 28] = 0;

        if (attributeValue) strcat(formattedStr, attributeValue);
...
///////////////////////////////////////////////////////////////
ExExeUtilGetUIDTcb::ExExeUtilGetUIDTcb(const ComTdbExeUtilGetUID &exe_util_tdb, ex_globals *glob)
    : ExExeUtilTcb(exe_util_tdb, NULL, glob) {
  // Allocate the private state in each entry of the down queue
  qparent_.down->allocatePstate(this);
  ...
  while (1) {
    switch (step_) {
      ...
      case RETURN_UID_: {
        ...
        moveRowToUpQueue((const char *)&(getUIDTdb().uid_), sizeof(getUIDTdb().uid_), NULL, FALSE);
...
///////////////////////////////////////////////////////////////
ExExeUtilGetQIDTcb::ExExeUtilGetQIDTcb(const ComTdbExeUtilGetQID &exe_util_tdb, ex_globals *glob)
    : ExExeUtilTcb(exe_util_tdb, NULL, glob) {
  // Allocate the private state in each entry of the down queue
  qparent_.down->allocatePstate(this);
  ...
  while (1) {
    switch (step_) {
      ...
      case RETURN_QID_: {
        ...
        init_SQLSTMT_ID(&stmtId, SQLCLI_CURRENT_VERSION, stmt_name, &module, getQIDTdb().getStmtName(), NULL, NULL,
...
ex_tcb *ExExeUtilGetErrorInfoTdb::build(ex_globals *glob) {
  ...
  exe_util_tcb = new (glob->getSpace()) ExExeUtilGetErrorInfoTcb(*this, glob);
...
///////////////////////////////////////////////////////////////
ExExeUtilGetErrorInfoTcb::ExExeUtilGetErrorInfoTcb(const ComTdbExeUtilGetErrorInfo &exe_util_tdb, ex_globals *glob)
    : ExExeUtilTcb(exe_util_tdb, NULL, glob) {
  // Allocate the private state in each entry of the down queue
  qparent_.down->allocatePstate(this);
    ...
    switch (step_) {
      ...
      case RETURN_TEXT_: {
        ...
        if ((!msgNotFound) || (errNum == 0)) {
          UnicodeStringToLocale(CharInfo::ISO88591, errorMsg, NAWstrlen(errorMsg), isoErrorMsg, bufSize);

          if (errNum != 0) {
            GetErrorMessage(errType, errNum, errorMsg, CAUSE_TEXT);
            strcat(isoErrorMsg, "\n");
            int len = strlen(isoErrorMsg);
            UnicodeStringToLocale(CharInfo::ISO88591, errorMsg, NAWstrlen(errorMsg), isoErrorMsg + len, bufSize - len);
          ...
          if (errType == MAIN_ERROR) {
            str_sprintf(outputBuf_, "*** SQLSTATE (Err): %s SQLSTATE (Warn): %s", sqlstateErr, sqlstateWarn);
            moveRowToUpQueue(outputBuf_);
          }
          ...
          str_sprintf(outputBuf_, "*** WARNING[%d]", warnNum);
          moveRowToUpQueue(outputBuf_);

          str_sprintf(outputBuf_, "*** ERROR[16001] The error number %d is not used in SQL.", warnNum);
          moveRowToUpQueue(outputBuf_);
        }
...
///////////////////////////////////////////////////////////////
ExExeUtilRegionStatsTcb::ExExeUtilRegionStatsTcb(const ComTdbExeUtilRegionStats &exe_util_tdb, ex_globals *glob)
    : ExExeUtilTcb(exe_util_tdb, NULL, glob) {
...
short ExExeUtilRegionStatsTcb::collectStats(char *inTableName, char *INtableNameForUID, NABoolean replaceUID) {
  ...
  if (getDLStdb().getCatName() && (NAString(getDLStdb().getCatName()) == HBASE_SYSTEM_CATALOG)) isHbase = TRUE;
  ...
  if (extractParts(tableName, &catName_, &schName_, &objName_, TRUE)) {
    return -1;
  }
  ...
  if (replaceUID) {
    ...
    if (NAString(catName_) == HBASE_SYSTEM_CATALOG)
      ...
      extNameForHbase_ = NAString(catName_) + "." + NAString(schName_) + "." + NAString(objName_);
    else
      extNameForHbase_ = nameSpace + ":" + NAString(catName_) + "." + NAString(schName_) + "." + NAString(objName_);
  }
...
short ExExeUtilRegionStatsTcb::populateStats(int currIndex) {
  ...
  if (len >= sizeof(regionInfoBuf)) len = sizeof(regionInfoBuf) - 1;
  ...
  while (1) {
    switch (step_) {
      case INITIAL_: {
        ...
        if (getDLStdb().replaceNameByUID()) strcpy(tableNameForUID_, getDLStdb().getDataUIDName());
      ...
      case EVAL_INPUT_: {
        workAtp_->getTupp(getDLStdb().workAtpIndex()).setDataPointer(inputNameBuf_);

        ex_expr::exp_return_type exprRetCode = getDLStdb().inputExpr()->eval(pentry_down->getAtp(), workAtp_);
      ...
      case COLLECT_STATS_: {
        if (collectStats(tableName_, tableNameForUID_, getDLStdb().replaceNameByUID()) < 0) {
          ExRaiseSqlError(getHeap(), &diagsArea_, -8451, NULL, NULL, NULL, getSqlJniErrorStr());
          step_ = HANDLE_ERROR_;
          break;
        }
      ...
      case RETURN_STATS_BUF_: {
        ...
        short rc = 0;
        if (moveRowToUpQueue((char *)stats_, statsBufLen_, &rc, FALSE)) return rc;
...
///////////////////////////////////////////////////////////////
ExExeUtilRegionStatsFormatTcb::ExExeUtilRegionStatsFormatTcb(const ComTdbExeUtilRegionStats &exe_util_tdb,
                                                             ex_globals *glob)
  ...
  while (1) {
    switch (step_) {
      ...
      case RETURN_SUMMARY_: {
        ...
        int neededSize = SqlBufferNeededSize(14, 250);
        ...
        str_sprintf(buf, " ");
        if (moveRowToUpQueue(buf, strlen(buf), &rc)) return rc;
        ...
        str_sprintf(buf, "  RegionsLocation:         %s/data/default", hbaseRootdir_);
        if (moveRowToUpQueue(buf, strlen(buf), &rc)) return rc;
      ...
      case RETURN_DETAILS_: {
        if ((getDLStdb().summaryOnly()) || (regionInfoList_->entries() == 0)) {
          step_ = DONE_;
          break;
        }
        ...
        int neededSize = SqlBufferNeededSize(4, 250);
        if (!pool_->get_free_buffer(neededSize)) {
          return WORK_CALL_AGAIN;
        }
      ...
      case RETURN_REGION_INFO_: {
        ...
        int neededSize = SqlBufferNeededSize(4, 100);
        if (!pool_->get_free_buffer(neededSize)) {
          return WORK_CALL_AGAIN;
        }
...
///////////////////////////////////////////////////////////////
ExExeUtilClusterStatsTcb::ExExeUtilClusterStatsTcb(const ComTdbExeUtilRegionStats &exe_util_tdb, ex_globals *glob)
    : ExExeUtilRegionStatsTcb(exe_util_tdb, glob) {
  ...
  ehi_ = ExpHbaseInterface::newInstance(glob->getDefaultHeap(), (char *)"", (char *)"", COM_STORAGE_HBASE, FALSE);
  regionInfoList_ = NULL;
...
short ExExeUtilClusterStatsTcb::populateStats(int currIndex, NABoolean nullTerminate) {
  ...
  if (sep1) {
    ...
    if (numParts == 3) {
      ...
      if (nullTerminate) stats_->catalogName[strlen(parts[0])] = 0;

      str_cpy_all(stats_->schemaName, parts[1], strlen(parts[1]));
      if (nullTerminate) stats_->schemaName[strlen(parts[1])] = 0;

      str_cpy_all(stats_->objectName, parts[2], strlen(parts[2]));
      if (nullTerminate) stats_->objectName[strlen(parts[2])] = 0;
    }
