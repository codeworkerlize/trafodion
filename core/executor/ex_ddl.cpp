...
///////////////////////////////////////////////////////////////
ExDDLTcb::ExDDLTcb(const ComTdbDDL &ddl_tdb, ex_globals *glob)
    : ex_tcb(ddl_tdb, 1, glob), compilerVersion_((COM_VERSION)currContext()->getVersionOfCompiler()), workAtp_(NULL) {
  Space *space = (glob ? glob->getSpace() : 0);
  CollHeap *heap = (glob ? glob->getDefaultHeap() : 0);

  // Allocate the buffer pool
  pool_ = new (space) sql_buffer_pool(ddl_tdb.numBuffers_, ddl_tdb.bufferSize_, space);

  // Allocate the queue to communicate with parent
  qparent_.down = new (space) ex_queue(ex_queue::DOWN_QUEUE, ddl_tdb.queueSizeDown_, ddl_tdb.criDescDown_, space);
  ...
  qparent_.up = new (space) ex_queue(ex_queue::UP_QUEUE, ddl_tdb.queueSizeUp_, ddl_tdb.criDescUp_, space);

  if (ddl_tdb.workCriDesc_) workAtp_ = allocateAtp(ddl_tdb.workCriDesc_, glob->getSpace());

  tcbFlags_ = 0;

  if (ddl_tdb.inputExpr_) (void)ddl_tdb.inputExpr_->fixup(0, getExpressionMode(), this, space, heap, FALSE, glob);
  if (ddl_tdb.outputExpr_) (void)ddl_tdb.outputExpr_->fixup(0, getExpressionMode(), this, space, heap, FALSE, glob);
};
...
char *ExDDLTcb::getQueryIdInfo(int &len) {
  ...
  if (masterStmtGlobals) {
    // master executor case
    len = masterStmtGlobals->getStatement()->getUniqueStmtIdLen();
    return masterStmtGlobals->getStatement()->getUniqueStmtId();
  }

  // esp case
  ExEspStmtGlobals *espStmtGlobals = stmtGlobals->castToExEspStmtGlobals();

  if (espStmtGlobals) {
    len = espStmtGlobals->getQueryIdLen();
    return espStmtGlobals->getQueryId();
  }
...
ExExeStmtGlobals *ExDDLTcb::getExeStmtGlobals() {
  ExExeStmtGlobals *stmtGlobals = getGlobals()->castToExExeStmtGlobals();
  ...
  if (masterStmtGlobals) return masterStmtGlobals;

  ExEspStmtGlobals *espStmtGlobals = stmtGlobals->castToExEspStmtGlobals();

  return espStmtGlobals;
}
...
//////////////////////////////////////////////////////
short ExDDLTcb::work() {
  // Get the cli context and init the embedded compiler
  ContextCli *currContextCli = currContext();
  currContextCli->initEmbeddedArkcmp(ddlTdb().isExecuteInESP());

  ExExeStmtGlobals *globals = getExeStmtGlobals();
  NABoolean isEsp = !!(globals->castToExEspStmtGlobals());

  ExTransaction *ta = currContextCli->getTransaction();
  ...
  while (1) {
    ...
    if (ddlTdb().hbaseDDL()) {
      if (ddlTdb().hbaseDDLNoUserXn()) {
        ...
        // is off.
        if ((!ta->autoCommit()) || (ta->xnInProgress() && (NOT ta->implicitXn()))) {
          if (cpDiagsArea == NULL) cpDiagsArea = ComDiagsArea::allocate(getHeap());

          if (ta->xnInProgress())
            *cpDiagsArea << DgSqlCode(-20123) << DgString0("This DDL operation");
          else
            *cpDiagsArea << DgSqlCode(-20124) << DgString0("This DDL");
    ...
    if (ddlTdb().inputExpr_) {
      pool_->get_free_tuple(workAtp_->getTupp(ddlTdb().workAtpIndex_), ddlTdb().inputRowlen_);
    ...
    // but not both
    if (currContextCli->isEmbeddedArkcmpInitialized() && currContextCli->getSessionDefaults()->callEmbeddedArkcmp() &&
        ddlTdb().hbaseDDL() && CmpCommon::context() && (CmpCommon::context()->getRecursionLevel() == 0)) {
      ...
      if (dummyReply != NULL) currContextCli->exHeap()->deallocateMemory((void *)dummyReply);

      if (cpStatus == ExSqlComp::SUCCESS) {
        ExMasterStmtGlobals *masterStmtGlobals = globals->castToExMasterStmtGlobals();
        if (!masterStmtGlobals) {
          LOGINFO(CAT_SQL_CI, "ERROR!!, can not get syncQueryForBinglog status");
          goto endOfData;
        }
        if (masterStmtGlobals->getStatement()->syncQueryForBinglog() &&
          ...
          CmpContext *cmpContext = masterStmtGlobals->getStatement()->getContext()->getEmbeddedArkcmpContext();
          if (cmpContext) {
          ...
          ret = ehi->putData(eventID, masterStmtGlobals->getStatement()->getSrcStr(), 999, objschema.data(), NULL, 0);
          if (ret < 0) {
            // log this error
            NAString massge;
            massge.format("put sql->:%s to hbase, objschema = %s, eventID = %ld",
                          masterStmtGlobals->getStatement()->getSrcStr(), objschema.data(), eventID);
        ...
        if (cpStatus == ExSqlComp::ERROR) goto endOfData;
      }
  ...
  endOfData:
    ...
    if (cmp) {
      cmpStatus = cmp->changePriority(currContextCli->getSessionDefaults()->getMxcmpPriorityDelta(), TRUE);
      if (cmpStatus != ExSqlComp::SUCCESS) {
        // Add a warning that change priority failed.
        // Continue processing with the original default.
        if (cpDiagsArea == NULL) cpDiagsArea = ComDiagsArea::allocate(getHeap());
    ...
    if (ddlTdb().workCriDesc_) workAtp_->release();

    currContextCli->ddlStmtsExecuted() = TRUE;
    currContextCli->ddlStmtsInSPExecuted() = TRUE;

  }  // while
  return WORK_OK;
}

ExDDLwithStatusTcb::ExDDLwithStatusTcb(const ComTdbDDL &ddl_tdb, ex_globals *glob)
...
short ExDDLwithStatusTcb::work() {
  ...
  // For parallel plan, we need to decide whether this particular ESP
  // should perform the work. For some DDLs such as loading shared cache,
  // we only need to load for a subset of the nodes.
  NABoolean needToDoWork = TRUE;

  if (isEsp) {
    ExEspStmtGlobals *espGlobals = globals->castToExEspStmtGlobals();

    // Find out the fragment Id
    const ExFragId fragId = espGlobals->getMyFragId();

    // Find out the fragment key
    const ExFragKey fragKey = espGlobals->getFragmentKey(fragId);

    const ExEspFragInstanceDir *espFragDir = espGlobals->getEspFragInstanceDir();
    ...
    // should do the work.
    needToDoWork = msgFragment->needToWork(myInstNum);
  }
    ...
    switch (step_) {
      ...
      case SETUP_INITIAL_REQ_: {
        ...
        if (ddlTdb().getMDVersion())
          ...
          if (ddlTdb().getCheckOnly()) mdi_->setCheckOnly(TRUE);
          if (ddlTdb().getReturnDetails()) mdi_->setReturnDetails(TRUE);
        ...
        // Call either the embedded arkcmp, if exists, or external arkcmp
        // but not both
        if (currContextCli->isEmbeddedArkcmpInitialized() &&
            currContextCli->getSessionDefaults()->callEmbeddedArkcmp() && ddlTdb().hbaseDDL() && CmpCommon::context() &&
      ...
      case CALL_EMBEDDED_CMP_: {
        ...
        cmpStatus = CmpCommon::context()->compileDirect(data_, dataLen_, currContextCli->exHeap(),
                                                        ddlTdb().queryCharSet_, EXSQLCOMP::DDL_WITH_STATUS, replyBuf_,
                                                        replyBufLen_, currContextCli->getSqlParserFlags(), parentQid,
                                                        str_len(parentQid), cpDiagsArea, needToDoWork);

        if (currContextCli->getDiagsArea()) currContextCli->getDiagsArea()->clear();

        if ((cpDiagsArea) &&
            ((cpDiagsArea->getNumber(DgSqlCode::WARNING_) > 0) || (cpDiagsArea->getNumber(DgSqlCode::ERROR_) > 0))) {
          getDiagsArea()->mergeAfter(*cpDiagsArea);

          if (cpDiagsArea->getNumber(DgSqlCode::ERROR_) > 0) {
            step_ = HANDLE_ERROR_;
            break;
          }
        }
      ...
      case SEND_REQ_TO_CMP_: {
        ...
        if ((cmp_->getDiags()) && ((cmp_->getDiags()->getNumber(DgSqlCode::WARNING_) > 0) ||
      ...
      case PROCESS_REPLY_: {
        if (replyBuf_) {
          ...
          if (cmp_)
            ...
            currContextCli->exHeap()->deallocateMemory((void *)replyBuf_);
          replyBuf_ = NULL;
          replyBufLen_ = 0;
      ...
      case RETURN_STATUS_: {
        ...
        char buf[1000];
        if ((replyDWS_->msgLen() > 0) && (strlen(replyDWS_->msg()) > 0)) {
          if (replyDWS_->returnET()) {
            if (replyDWS_->done()) startTime_ = queryStartTime_;

            char timeBuf[100];
            ExExeUtilTcb::getTimeAsString((endTime_ - startTime_), timeBuf, TRUE);
            str_sprintf(buf, "%s {ET: %s}", replyDWS_->msg(), timeBuf);
          } else
            str_sprintf(buf, "%s", replyDWS_->msg());
          if (moveRowToUpQueue(&qparent_, ddlTdb().tuppIndex(), buf, 0, &rc)) return rc;
        }

        if (replyDWS_->endStep()) {
          if (moveRowToUpQueue(&qparent_, ddlTdb().tuppIndex(), " ", 0, &rc)) return rc;
        }
      ...
      case RETURN_STATUS_END_STEP_: {
        if (replyDWS_->done()) {
          if ((getDiagsArea()) && (getDiagsArea()->getNumber(DgSqlCode::ERROR_) > 0))
            step_ = HANDLE_ERROR_;
          else
            step_ = DONE_;
          break;
      ...
      case HANDLE_ERROR_: {
        if (handleError(&qparent_, getDiagsArea()) == 1) return WORK_OK;

        step_ = DONE_;
      } break;

      case DONE_: {
        if (handleDone(&qparent_, getDiagsArea()) == 1) return WORK_OK;
...
ExSqlComp *ExDDLTcb::getArkcmp(void) {
  ...
  if (!currContext()->getNumArkcmps()) return NULL;  // no regular compiler exists
...
ExDDLPrivateState::ExDDLPrivateState(const ExDDLTcb * /*tcb*/) { init(); }
...
ex_tcb *ExDescribeTdb::build(ex_globals *glob) {
  ...
  if ((ComTdbDescribe::DescribeType)type_ == ComTdbDescribe::ENVVARS_)
...
ExDescribeTcb::ExDescribeTcb(const ExDescribeTdb &describe_tdb, ex_globals *glob) : ExDDLTcb(describe_tdb, glob) {}
...
short ExDescribeTcb::work() {
  ...
  ComDiagsArea &diagsArea0 = currContext()->diags();
    ...
    switch (pstate.step_) {
      case EMPTY_: {
        ...
        if (describeTdb().inputExpr_) {
          pool_->get_free_tuple(workAtp_->getTupp(describeTdb().workAtpIndex_), describeTdb().inputRowlen_);
          if (describeTdb().inputExpr_->eval(pentry_down->getAtp(), workAtp_) == ex_expr::EXPR_ERROR) {
        ...
        if (currContext() && currContext()->isEmbeddedArkcmpInitialized() &&
          ...
          compStatus = CmpCommon::context()->compileDirect(
              describeTdb().query_, describeTdb().queryLen_, arkcmpHeap, describeTdb().queryCharSet_,
              EXSQLCOMP::DESCRIBE, pstate.dataPtr_, pstate.dataLen_, currContext()->getSqlParserFlags(), parentQid,
              str_len(parentQid), da);
          if (compStatus == ExSqlComp::SUCCESS) {
            ...
            if (!currContext()->diags().getNumber(DgSqlCode::ERROR_)) currContext()->diags().clear();
            pstate.currLen_ = 0;
            if (pstate.dataLen_ == pstate.currLen_)
              pstate.step_ = DONE_;
            else
              pstate.step_ = RETURNING_DATA_;  // success
            if ((ComTdbDescribe::DescribeType)describeTdb().type_ == ComTdbDescribe::LEAKS_) {
              short error;
              if (returnLeaks(error)) return error;
            }
          } else {
            pstate.step_ = HANDLE_ERROR_;
          }
          // ComDiagsDa is allocated in compileDirect, needs to be deallocated
          if (da != NULL) deleteTmpDa = TRUE;
        } else if (getArkcmp())  // regular arkcmp exists
        {
          ...
          ExSqlComp::ReturnStatus sendStatus =
              cmp->sendRequest(EXSQLCOMP::DESCRIBE, describeTdb().query_, describeTdb().queryLen_, TRUE, NULL,
          ...
          if (sendStatus != ExSqlComp::ERROR) pstate.step_ = REQUEST_SENT_;
        }
      } break;
      ...
      case REQUEST_SENT_: {
        ...
        // in private state.
        ExSqlComp::ReturnStatus replyStatus = cmp->getReply(pstate.dataPtr_, pstate.dataLen_);
      ...
      case RETURNING_DATA_: {
        ...
        if (copyLen > describeTdb().outputRowlen_) copyLen = (unsigned short)describeTdb().outputRowlen_;

        ex_queue_entry *up_entry = qparent_.up->getTailEntry();

        // allocate space
        if (pool_->get_free_tuple(up_entry->getTupp(describeTdb().tuppIndex_), copyLen)) {
        ...
        if (copyLen != actualLen) *(short *)dataPtr = copyLen - sizeof(short);
        ...
        if (da != NULL) {
          ...
          if (deleteTmpDa) da->decrRefCount();
          da = NULL;
        }
        ...
        if (pstate.currLen_ >= pstate.dataLen_) pstate.step_ = DONE_;
      } break;
      ...
      case DONE_: {
        if ((pstate.dataPtr_ != NULL) && (pstate.dataLen_ != 0)) {  // Delete the reply buffer.
        ...
        if (describeTdb().workCriDesc_) workAtp_->release();
...
int ExDescribeTcb::returnLeaks(short &error) {
  ...
  // allocate space
  if (pool_->get_free_tuple(up_entry->getTupp(describeTdb().tuppIndex_), describeTdb().outputRowlen_))
  ...
  char *entry = up_entry->getTupp(describeTdb().tuppIndex_).getDataPointer();

  // For ARKCMP and/or SQLCI
  if (HeapLogRoot::fetchLine(entry, describeTdb().flags_, pstate.dataPtr_,
                             pstate.dataLen_) == 1) {  // eof
...
///////////////////////////////////////////////////////////////
ExProcessVolatileTableTcb::ExProcessVolatileTableTcb(const ComTdbProcessVolatileTable &exe_util_tdb, ex_globals *glob)
    : ExDDLTcb(exe_util_tdb, glob), step_(INITIAL_) {
  // Allocate the private state in each entry of the down queue
...
short ExProcessVolatileTableTcb::work() {
  ...
  ExeCliInterface cliInterface(getHeap(), 0, currContext, masterGlob->getStatement()->getUniqueStmtId());

  while (1) {
    switch (step_) {
      case INITIAL_: {
        if ((pvtTdb().isCreate()) && (pvtTdb().isSchema()) &&
        ...
        if ((pvtTdb().isCreate()) && ((pvtTdb().isTable()) || (pvtTdb().isIndex())))
      ...
      case CREATE_VOLATILE_SCHEMA_: {
        if (NOT masterGlob->getStatement()->getContext()->volatileSchemaCreated()) {
          char *createVS = new (getHeap()) char[strlen("CREATE VOLATILE SCHEMA;") + 1];
      ...
      case SET_VOLATILE_SCHEMA_USAGE_CQD_: {
        char *sendCQD = new (getHeap()) char[strlen("CONTROL QUERY DEFAULT VOLATILE_SCHEMA_IN_USE 'TRUE';") + 1];
      ...
      case SEND_DDL_EXPR_: {
        ...
        if (currContext->isEmbeddedArkcmpInitialized() && currContext->getSessionDefaults()->callEmbeddedArkcmp() &&
          ...
          embCmpStatus = CmpCommon::context()->compileDirect(
              data, dataLen, currContext->exHeap(), SQLCHARSETCODE_UTF8, EXSQLCOMP::PROCESSDDL, dummyReply, dummyLength,
              currContext->getSqlParserFlags(), parentQid, str_len(parentQid), embCmpDiagsArea);
          getHeap()->deallocateMemory(data);
          if (dummyReply != NULL) currContext->exHeap()->deallocateMemory((void *)dummyReply);
          if (embCmpStatus == ExSqlComp::SUCCESS) {
            ...
            if (embCmpDiagsArea && embCmpDiagsArea->mainSQLCODE() == -CAT_SCHEMA_DOES_NOT_EXIST_ERROR) {
              // volatile schema does not exist. Create it and retry
              // the ddl statement.
              masterGlob->getStatement()->getContext()->setVolatileSchemaCreated(FALSE);
              currContext->diags().clear();
              embCmpDiagsArea = NULL;
              step_ = CREATE_VOLATILE_SCHEMA_;
              break;
            }
        ...
        {
          cmp = getGlobals()->castToExExeStmtGlobals()->castToExMasterStmtGlobals()->getCliGlobals()->getArkcmp();
          ...
          // will cause mxcmp to use the default charset as iso88591.
          // So we send the charset of this input string.
          cmpStatus = cmp->sendRequest(
              EXSQLCOMP::PROCESSDDL, data, dataLen, TRUE, NULL, SQLCHARSETCODE_UTF8, TRUE, /*resend, if needed*/
              masterGlob->getStatement()->getUniqueStmtId(), masterGlob->getStatement()->getUniqueStmtIdLen());
        ...
        if ((pvtTdb().isCreate()) && (pvtTdb().isSchema())) {
          masterGlob->getStatement()->getContext()->setVolatileSchemaCreated(TRUE);
        }

        if ((NOT pvtTdb().isCreate()) && (pvtTdb().isSchema())) {
          masterGlob->getStatement()->getContext()->setVolatileSchemaCreated(FALSE);
        }

        if ((pvtTdb().isCreate()) && (pvtTdb().isTable()))
          step_ = ADD_TO_VOL_TAB_LIST_;
        else if ((NOT pvtTdb().isCreate()) && (pvtTdb().isTable()))
      ...
      case ADD_TO_VOL_TAB_LIST_: {
        ...
        if (!volTabList) currContext->initVolTabList();

        volTabList = currContext->getVolTabList();

        volTabList->insert(pvtTdb().volTabName_, pvtTdb().volTabNameLen_, (void *)pvtTdb().volTabName_);
      ...
      case REMOVE_FROM_VOL_TAB_LIST_: {
        ...
        volTabList->position(pvtTdb().volTabName_, pvtTdb().volTabNameLen_);
      ...
      case RESET_VOLATILE_SCHEMA_USAGE_CQD_: {
        char *sendCQD = new (getHeap()) char[strlen("CONTROL QUERY DEFAULT VOLATILE_SCHEMA_IN_USE 'FALSE';") + 1];
      ...
      case DONE_: {
        ...
        if (embCmpDiagsArea) {
          ...
          diagsArea->mergeAfter(*embCmpDiagsArea);
          up_entry->setDiagsArea(diagsArea);
        }
...
///////////////////////////////////////////////////////////////
ExProcessInMemoryTableTcb::ExProcessInMemoryTableTcb(const ComTdbProcessInMemoryTable &exe_util_tdb, ex_globals *glob)
    : ExDDLTcb(exe_util_tdb, glob), step_(INITIAL_) {
  // Allocate the private state in each entry of the down queue
  qparent_.down->allocatePstate(this);
}
  ...
  while (1) {
    switch (step_) {
      ...
      case TURN_QUERY_CACHE_OFF_: {
        ...
        // turn it off.
        char *sendCQD = new (getHeap()) char[strlen("CONTROL QUERY DEFAULT QUERY_CACHE '0';") + 1];
      ...
      case SEND_DDL_EXPR_: {
        ...
        cmp = getGlobals()->castToExExeStmtGlobals()->castToExMasterStmtGlobals()->getCliGlobals()->getArkcmp();
        cmpStatus = cmp->sendRequest(EXSQLCOMP::PROCESSDDL, data, dataLen, TRUE, NULL, SQLCHARSETCODE_UTF8,
                                     TRUE /*resend, if needed*/);
      ...
      case DROP_: {
        ...
        if ((pimtTdb().isCreate()) && (cmp->getDiags()->mainSQLCODE() != -1055)) {
          char *dtQuery = new (getHeap()) char[strlen("DROP VOLATILE TABLE ; ") + strlen(pimtTdb().objName_) + 100];
      ...
      case ERROR_: {
        if (volSchCreatedHere_) {
          char *dtQuery = new (getHeap()) char[strlen("DROP IMPLICIT VOLATILE SCHEMA CASCADE; ") + 100];
