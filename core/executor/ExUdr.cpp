...
static const char *GetWorkRetcodeString(ExWorkProcRetcode r) {
  switch (r) {
    case WORK_OK:
      return "WORK_OK";
    case WORK_CALL_AGAIN:
      return "WORK_CALL_AGAIN";
    case WORK_POOL_BLOCKED:
      return "WORK_POOL_BLOCKED";
    case WORK_RESCHEDULE_AND_RETURN:
...
//----------------------------------------------------------------------
ExOperStats *ExUdrTcb::doAllocateStatsEntry(CollHeap *heap, ComTdb *tdb) {
  ...
  if (statsType == ComTdb::OPERATOR_STATS) {
    stat = new (heap) ExUDRBaseStats(heap, this, tdb);
  } else {
    stat = new (heap) ExUDRStats(heap, udrTdb->getRequestSqlBufferSize(), udrTdb->getReplySqlBufferSize(), tdb, this);
...
// ExUdrTcb methods
//----------------------------------------------------------------------
ExUdrTcb::ExUdrTcb(const ExUdrTdb &udrTdb, const ex_tcb **childTcbs, ex_globals *glob)
...
#endif
{
  ...
  if (isResultSet) {
    ExMasterStmtGlobals *masterStmtGlobals = stmtGlobals->castToExMasterStmtGlobals();
    ex_assert(masterStmtGlobals, "Must be ExMasterStmtGlobals to support UDR result set");

    masterStmtGlobals->acquireRSInfoFromParent(rsIndex_, udrHandle_, udrServer_, serverProcessId_, rsInfo_);

    InitializeUdrHandle(rsHandle_);
    ex_assert(RSHandleIsValid(rsHandle_), "Invalid RS handle generated by ExUdrTcb constructor");

#ifdef UDR_DEBUG
    char buf[300];
    serverProcessId_.toAscii(buf, 300);
    UdrDebug0("  *** This is a result set proxy");
    UdrDebug2("  *** RS index %u, RS handle " INT64_PRINTF_SPEC, rsIndex_, rsHandle_);
    UdrDebug2("  *** ExUdrServer %p, process ID %s", udrServer_, buf);
#endif
  } else {
    InitializeUdrHandle(udrHandle_);
    ex_assert(UdrHandleIsValid(udrHandle_), "Invalid UDR handle generated by ExUdrTcb constructor");
  }

  UdrDebug1("  UDR handle: " INT64_PRINTF_SPEC, udrHandle_);
  UdrDebug5("  %u params, %u input value%s, %u output value%s", udrTdb.getNumParams(), udrTdb.getNumInputValues(),
            PLURAL_SUFFIX(udrTdb.getNumInputValues()), udrTdb.getNumOutputValues(),
            PLURAL_SUFFIX(udrTdb.getNumOutputValues()));
  UdrDebug3("  Row length: %u request, %u reply, %u output", udrTdb.getRequestRowLen(), udrTdb.getReplyRowLen(),
            udrTdb.getOutputRowLen());
  UdrDebug2("  SqlBuffer size: %u request, %u reply", udrTdb.getRequestSqlBufferSize(), udrTdb.getReplySqlBufferSize());
  UdrDebug1("  Statement globals %p", getGlobals());
  UdrDebug1("  Transaction required: %s", udrTdb.getTransactionAttrs() == COM_TRANSACTION_REQUIRED ? "YES" : "NO");
  ...
  if (!isResultSet) {
    ...
    switch (language) {
      case COM_LANGUAGE_JAVA:
        ...
        UdrDebug1("  Runtime options '%s'", udrTdb.getRuntimeOptions() ? udrTdb.getRuntimeOptions() : "");
        UdrDebug1("  Option delimiters '%s'",
                  udrTdb.getRuntimeOptionDelimiters() ? udrTdb.getRuntimeOptionDelimiters() : "");
        break;
      case COM_LANGUAGE_C:
  ...
  if (udrTdb.getWorkCriDesc()) workAtp_ = allocateAtp(udrTdb.getWorkCriDesc(), globSpace);

  // Fixup expressions
  if (udrTdb.getInputExpression())
    udrTdb.getInputExpression()->fixup(0, getExpressionMode(), this, globSpace, globHeap, FALSE, glob);
  ...
  if (udrTdb.getPredicate())
    udrTdb.getPredicate()->fixup(0, getExpressionMode(), this, globSpace, globHeap, FALSE, glob);
  ...
  if (!isResultSet) {
    ...
    if (options == NULL || strlen(options) == 0) options = "OFF";
    if (delims == NULL || strlen(delims) == 0) delims = " ";
    NABoolean dedicated = FALSE;
    if (myTdb().isTmudf()) dedicated = TRUE;
  ...
  UdrDebug2("  This TCB will use ExUdrServer %p, ref count %d", udrServer_, udrServer_->getRefCount());
...
IpcEnvironment *ExUdrTcb::myIpcEnv() const { return myExeStmtGlobals()->getIpcEnvironment(); }

CollHeap *ExUdrTcb::getIpcHeap() const { return myIpcEnv()->getHeap(); }

ComDiagsArea *ExUdrTcb::getStatementDiags() const { return myExeStmtGlobals()->getDiagsArea(); }

void ExUdrTcb::setStatementDiags(ComDiagsArea *d) const { myExeStmtGlobals()->setGlobDiagsArea(d); }
...
int ExUdrTcb::fixup() {
  ...
  for (UInt32 i = 0; i <= numRetries && result != FIXUP_SUCCESS; i++) {
...
#ifdef UDR_DEBUG
    if (doTrace_ && traceFile_) udrServer_->setTraceFile(traceFile_);
#endif
    ...
    if (!isResultSet) status = udrServer_->start(&da, diagsHeap, transIdToSend, serverProcessId_, usesTransactions);
    ...
    if (status == ExUdrServer::EX_UDR_SUCCESS) {
      ...
      if (!isResultSet) {
        if (myTdb().getMaxResultSets() > 0) {
          ...
          UdrDebug2("    TCB %p will use IPC Connection %p", this, conn);
        } else {
          UdrDebug2("    TCB %p will use IPC Connection %p", this, udrServer_->getUdrControlConnection());
        }
      }
      ...
      if ((myTdb().getLibraryRedefTime() != -1) && (myTdb().getLibraryRedefTime() != 0)) {
        ...
        if (err = ComGenerateUdrCachedLibName(libOrJarName.data(), myTdb().getLibraryRedefTime(),
          ...
          if (err != EEXIST) {
            ...
            *getOrCreateStmtDiags() << DgSqlCode(-4316) << DgString0((char *)errNAString.data());
            return FIXUP_ERROR;
          }
        }
        ...
        if (stat(cachedFullName, &statbuf) != 0) {
          ...
          if (ExExeUtilLobExtractLibrary(&cliInterface, (char *)myTdb().getLibraryBlobHandle(),
                                         (char *)cachedFullName.data(), returnedDiags)) {
            *returnedDiags << DgSqlCode(-4316) << DgString0((char *)cachedFullName.data());
      ...
      if (sendControlMessage(isResultSet ? UDR_MSG_RS_LOAD : UDR_MSG_LOAD, TRUE,
  ...
  UdrDebug3("[END TCB FIXUP] %p. Return value is %d (%s)\n", this, result,
            (result == FIXUP_SUCCESS ? "SUCCESS" : "ERROR"));
  return result;
...
ExWorkProcRetcode ExUdrTcb::work() {
  ...
  result = checkReceive();
  UdrDebug1("  [WORK] checkReceive() returned %s", GetWorkRetcodeString(result));

  if (result == WORK_OK) {
    result = checkSend();
    UdrDebug1("  [WORK] checkSend() returned %s", GetWorkRetcodeString(result));
  }

  if (result == WORK_OK) {
    result = continueRequest();
    UdrDebug1("  [WORK] continueRequest() returned %s", GetWorkRetcodeString(result));
  }
...
ExWorkProcRetcode ExUdrTcb::checkReceive() {
  ...
  while (!done && result == WORK_OK && anyOutstandingQueueRequests() && !qParent_.up->isFull()) {
    if (getIpcBroken()) {
      UdrDebug0("  [WORK] IPC is broken");
      result = WORK_BAD_ERROR;
    } else {
      ex_queue_entry *downEntry = qParent_.down->getHeadEntry();
      ExUdrPrivateState &down_pstate = *((ExUdrPrivateState *)downEntry->pstate);
      ...
      switch (down_pstate.step_) {
        ...
        case CANCEL_AFTER_SEND: {
          if (getReplyBuffer() != NULL) {
            ...
            if (getStatementDiags() && getStatementDiags()->mainSQLCODE() < 0) {
...
//
NABoolean ExUdrTcb::insertUpQueueEntry(ex_queue::up_status status, ComDiagsArea *diags) {
  if (qParent_.up->isFull()) return FALSE;

  ex_queue_entry *upEntry = qParent_.up->getTailEntry();
  ex_queue_entry *downEntry = qParent_.down->getHeadEntry();
  ExUdrPrivateState &privateState = *((ExUdrPrivateState *)downEntry->pstate);
  ...
  if (status == ex_queue::Q_SQLERROR && myTdb().isTmudf()) {
...
ExWorkProcRetcode ExUdrTcb::buildAndSendRequestBuffer() {
  ...
  while (result == WORK_OK && state_ == WORK && qParent_.down->entryExists(nextToSend_)) {
      ...
      switch (request) {
        ...
        case ex_queue::GET_ALL: {
          ...
          if (sqlBuf->moveInSendOrReplyData(TRUE,                        // [IN] sending? (vs. replying)
            ...
            ex_assert(numRequestsAdded > 0, "Request buffer not large enough to hold a single row");
            ...
            if (myTdb().getInputExpression()) {
              ...
              if (expStatus != ex_expr::EXPR_OK)
                UdrDebug1("  [WORK] inputExpr.eval() returned %s", GetExpStatusString(expStatus));
#endif
              // FIXME only support aligned format for SPJ now
              if (myTdb().getRequestTuple()->isSQLMXAlignedTable()) {
                UInt32 diff = myTdb().getRequestRowLen() - rowLen;
                // FIXME add some threshhold for the shrinking?
                if (diff > 0) {
                  sqlBuf->resize_tupp_desc(rowLen, dataDesc->getTupleAddress());
                }
              }
    ...
    UdrDebug2("  [WORK] %u request%s added to the request buffer", numRequestsAdded, PLURAL_SUFFIX(numRequestsAdded));
    UdrDebug2("  [WORK] %u error%s encountered", numErrors, PLURAL_SUFFIX(numErrors));
    UdrDebug2("  [WORK] %u %s cancelled before being sent", numCancels, (numCancels == 1 ? "entry" : "entries"));

    if (sqlBuf->getTotalTuppDescs() > 0) {
      // If we enter this block then it's time to send a data request

      // If we are using ALIGNED format (and also use DENSE sql
      // buffer), shrink the send message by truncate the free space.
      if (sqlBuf->bufType() == SqlBufferHeader::DENSE_) {
        ex_assert(sqlBuf->getFreeSpace() >= 0, "Negative sql buffer free space size");
        dataStream_->shrinkSendMsg(sqlBuf->getFreeSpace());
      }

#ifdef UDR_DEBUG
      // Print a message if tracing is enabled or the
      // UDR_CONTINUE_DEBUG environment variable is set
      if (doTrace_ || doIpcTrace_ || (getenv("UDR_CONTINUE_DEBUG") != NULL)) {
        char stmtTx[256];
        char msgTx[256];
      ...
      if (udrStats_) {
        udrStats_->bufferStats()->sentBuffers().addEntry(sizeof(*requestBuffer_) + sqlBuf->get_used_size());
      ...
      if (rsInfo_) rsInfo_->enterUdrTx(*myExeStmtGlobals());
  ...
  UdrDebug1("  [WORK] END ExUdrTcb::buildAndSendRequestBuffer() %p", this);
  return result;
...
void ExUdrTcb::reportLoadReply(NABoolean loadWasSuccessful) {
  UdrDebug2("  TCB %p received a LOAD reply. Handle " INT64_PRINTF_SPEC, this, udrHandle_);
  UdrDebug1("    The load %s successful", loadWasSuccessful ? "was" : "was not");
...
// this TCB.
//
void ExUdrTcb::reportIpcError(IpcMessageStreamBase *stream, IpcConnection *conn) {
  UdrDebug1("  [BEGIN ExUdrTcb::reportIpcError(IpcConnection %p)]", conn);
  ...
  if (!getIpcBroken()) {
    ...
    if (diags->contains(-EXE_UDR_REPLY_ERROR)) udrErrorAlreadyInDiags = TRUE;
    ...
    if (conn) {
      ...
      // for this TCB
      if (!udrErrorAlreadyInDiags) conn->populateDiagsArea(diags, diagsHeap);
    ...
    if (!udrErrorAlreadyInDiags) *diags << DgSqlCode(-EXE_UDR_REPLY_ERROR);
    ...
    // an error.
    if (udrServer_->getServerProcessId() == serverProcessId_) udrServer_->setState(ExUdrServer::EX_UDR_BROKEN);
...
void ExUdrTcb::deallocateMessage(UdrMessageObj *m) {
  UdrDebug2("  ExUdrTcb %p deallocating message object %p", this, m);
  if (m) {
    //
...
void ExUdrTcb::releaseConnectionToServer() {
  ...
  if (conn && (myTdb().getMaxResultSets() > 0)) udrServer_->releaseConnection(conn);
}
...
void ExUdrTcb::allocateDataStream() {
  ...
  if (!dataStream_ && conn) {
    ...
    maxSqlBufSize = MAXOF(myTdb().getRequestSqlBufferSize(), myTdb().getReplySqlBufferSize());
    ...
    dataStream_ =
        new (getIpcHeap()) UdrClientDataStream(myIpcEnv(),
                                               1,                 // sendBufferLimit
                                               2,                 // inUseBufferLimit
                                               maxBufSize + pad,  // default buffer size
                                               this, myExeStmtGlobals(), isTransactional, stmtGlobals->getThreadInfo());

#ifdef UDR_DEBUG
    if (doIpcTrace_ && traceFile_) dataStream_->setTraceFile(traceFile_);
#endif
...
void ExUdrTcb::releaseServerResources() {
  if (state_ == SENDING_LOAD || state_ == LOAD_FAILED || serverResourcesAreLoaded()) {
...
//   cancelled.
NABoolean ExUdrTcb::sendControlMessage(UdrIpcObjectType t, NABoolean callbackRequired, char *cachedLibName,
                                       char *cachedLibPath) {
  ...
  ex_assert(outstandingControlStream_ == NULL, "Sending a UDR control message while one is already outstanding");
  ...
  switch (t) {
    case UDR_MSG_LOAD: {
      ...
      if (myExeStmtGlobals()->castToExMasterStmtGlobals())
        parentQid = myExeStmtGlobals()->castToExMasterStmtGlobals()->getStatement()->getUniqueStmtId();
      else if (myExeStmtGlobals()->castToExEspStmtGlobals())
        parentQid = myExeStmtGlobals()->castToExEspStmtGlobals()->getQueryId();
      else
        parentQid = "";
      ...
      if (GetCliGlobals()->currContext()->getClientInfo()) {
        clientInfo = GetCliGlobals()->currContext()->getClientInfo();
      }
      UdrLoadMsg *loadMsg = new (h) UdrLoadMsg(
          h, udrTdb.getSqlName(), udrTdb.getRoutineName(), udrTdb.getSignature(), containerName, pathName,
          udrTdb.getLibrarySqlName(), udrTdb.getTransactionAttrs(), udrTdb.getSqlAccessMode(), udrTdb.getLanguage(),
          udrTdb.getParamStyle(), udrTdb.getExternalSecurity(), udrTdb.getMaxResultSets(), udrTdb.getNumParams(),
          udrTdb.getNumInputValues(), udrTdb.getNumOutputValues(), udrTdb.getRequestSqlBufferSize(),
          udrTdb.getReplySqlBufferSize(), udrTdb.getRequestRowLen(), udrTdb.getReplyRowLen(), udrTdb.getUdrFlags(),
          udrTdb.getRoutineOwnerId(), parentQid, clientInfo, GetCliGlobals()->currContext()->getTenantName(),
          udrTdb.udrSerInvocationInfoLen_, udrTdb.udrSerInvocationInfo_, udrTdb.udrSerPlanInfoLen_,
          udrTdb.udrSerPlanInfo_, udrTdb.getJavaDebugPort(), udrTdb.getJavaDebugTimeout(), instanceNum, numInstances);
      ...
      for (i = 0; i < udrTdb.getNumParams(); i++) {
        ...
        if (formal.isIn()) {
          ...
          UdrParameterInfo info(i,                                // Formal: position
    ...
    case UDR_MSG_RS_LOAD: {
      ...
      UdrRSLoadMsg *rsLoadMsg = new (h) UdrRSLoadMsg(rsIndex_, udrTdb.getNumOutputValues(), udrTdb.getReplyRowLen(),
                                                     udrTdb.getReplySqlBufferSize(), udrTdb.getUdrFlags(), h);
      ...
      for (i = 0; i < udrTdb.getNumParams(); i++) {
        ...
        UdrParameterInfo info(i,                                // Formal: position
                              ...
                              actual.getVoaOffset()             //         varchar voa offset
        );
  ...
  if (msg != NULL && verifyUdrServerProcessId()) {
    ...
    s = new (getIpcHeap()) UdrClientControlStream(myIpcEnv(), (callbackRequired ? this : NULL), myExeStmtGlobals(),
                                                  FALSE, isTransactional, myExeStmtGlobals()->getThreadInfo());

#ifdef UDR_DEBUG
    if (doIpcTrace_ && traceFile_) s->setTraceFile(traceFile_);
    ...
    *s << *msg;
    if (callbackRequired) {
      UdrDebug2("    TCB %p storing pointer to control stream %p", this, s);
      outstandingControlStream_ = s;
    }
    UdrDebug2("    TCB %p sending data on control stream %p", this, s);

    if ((t == UDR_MSG_LOAD || t == UDR_MSG_RS_LOAD) && udrStats_) {
...
void ExUdrTcb::setTmUdfInfo(UdrLoadMsg *lm, const ExUdrTdb &tdb) {
  ...
  for (i = 0; i < numChildren(); i++) {
    ...
    UdrTableInputInfo udrTabInput(i, str_len(childtab->getCorrName()), childtab->getCorrName(),
                                  childtab->getNumColumns(), childtab->getRowLength());
    ...
    for (j = 0; j < childtab->getNumColumns(); j++) {
      ...
      UdrParameterInfo udrcolinfo(
          j, colinfo->getFlags(), fsType, ansiType, colNameLen, colName, colinfo->getPrecision(), colinfo->getScale(),
          colinfo->getEncodingCharSet(), colinfo->getCollation(), actual->getLength(), actual->getTupleFormat(),
          actual->getNullIndicatorLength(), actual->getNullIndOffset(), actual->getNullBitIndex(), actual->getOffset(),
          actual->getVCLenIndOffset(), actual->getVCIndicatorLength(), actual->getVoaOffset());
...
void ExUdrTcb::releaseRequestBuffer() {
  ...
  //
  if (requestBuffer_) {
    UdrDebug1("    Request buffer %p is being released", requestBuffer_);
    requestBuffer_ = NULL;
  }
...
void ExUdrTcb::releaseChildInputBuffer(int i) {
  ...
  if (childInputBuffers_[i]) {
    UdrDebug1("    Child Input buffer %p is being released", childInputBuffers_[i]);
...
UdrDataBuffer *ExUdrTcb::getRequestBuffer() {
  ...
  if (header == NULL) {
    ...
    result = new (*dataStream_, sqlBufferSize) UdrDataBuffer(sqlBufferSize, UdrDataBuffer::UDR_DATA_IN, NULL);
    ex_assert(result, "Could not allocate request. Stream buffer size may be low.");

    result->setHandle(udrHandle_);

    UdrDebug2("  [WORK] Created request buffer %p. SqlBuffer size %u", result, sqlBufferSize);
    UdrDebug1("  [WORK] Number of send buffers in the data stream is %d", dataStream_->numOfSendBuffers());
    ex_assert(dataStream_->numOfSendBuffers() == 1, "Multiple buffers for single request. Stream buf size too low.");
...
UdrDataBuffer *ExUdrTcb::getReplyBuffer() {
  ...
  if (replyBuffer_ == NULL) {
    ...
    if (dataStream_ && dataStream_->getNextReceiveMsg(msgType) && dataStream_->getNextObjType(msgType)) {
      switch (msgType) {
        case UDR_MSG_DATA_REPLY: {
          if (doIntegrityChecks) {
            IpcMessageObjSize objSize = dataStream_->getNextObjSize();
            replyBuffer_ = new (dataStream_->receiveMsgObj()) UdrDataBuffer(dataStream_, objSize, integrityCheckResult);
          } else {
            replyBuffer_ = new (dataStream_->receiveMsgObj()) UdrDataBuffer(dataStream_);
          }
          ...
          if (integrityCheckResult) {
            if (dataStream_->getNextObjType(msgType)) {
              ...
              if (msgType == UDR_MSG_RS_INFO) {
                ...
                integrityCheckResult = dataStream_->extractNextObj(*rsInfoMsg, doIntegrityChecks);

                if (integrityCheckResult) {
                  ex_assert(rsInfo_, "RS INFO msg received but ExRsInfo ptr is NULL");
            ...
            if (integrityCheckResult) {
              ...
              if (doTrace_ || doIpcTrace_) {
                ...
                UdrPrintf(f, "  [WORK] Received UdrDataBuffer %p, %d tupps", replyBuffer_,
        ...
        case UDR_MSG_ERROR_REPLY: {
          ...
          integrityCheckResult = dataStream_->extractNextObj(*reply, doIntegrityChecks);
          reply->decrRefCount();
          reply = NULL;
          if (!integrityCheckResult) {
...
ExWorkProcRetcode ExUdrTcb::returnSingleRow() {
  ...
  ControlInfo *ci = NULL;
  ex_queue_entry *upEntry = qParent_.up->getTailEntry();
  UdrDebug0("  [WORK] In  ::returnSingleRow");
  SqlBuffer *sqlBuf = replyBuffer_->getSqlBuffer();
  ...
  if (result == WORK_OK) {
    ...
    tupp reply_row;
    NABoolean endOfData = sqlBuf->moveOutSendOrReplyData(FALSE,                // [IN] sending? (vs. replying)
    ...
    UdrDebug2("  [WORK] Next reply row: eod %s, status %s", TF_STRING(endOfData), GetUpStatusString(status));
    ...
    if (!endOfData) {
      ...
      if (integrityCheckResult) {
        ...
        if (status == ex_queue::Q_OK_MMORE && returnedDiags && returnedDiags->getNumber(DgSqlCode::ERROR_) > 0)
          ...
          case ex_queue::Q_OK_MMORE: {
            if (down_pstate.step_ != CANCEL_AFTER_SEND) {
              ...
              if (validateDataRow(reply_row, validationDiags) == FALSE) {
                ex_assert(validationDiags, "validateDataRow() did not return diagnostics");
                ...
                if (numOutputValues > 0) {
                  upEntry->getTupp(myTdb().getNumOutputTupps() - 1) = (moveOutputValues ? output_row : reply_row);
                }
                ...
                if (moveOutputValues) {
                  ...
                  // in the up queue.
                  ex_expr::exp_return_type expStatus = myTdb().getOutputExpression()->eval(upEntry->getAtp(), workAtp_);
                  if (expStatus != ex_expr::EXPR_OK) {
                    outputExprError = TRUE;
                    UdrDebug1("  [WORK]   outputExpr.eval() returned %s", GetExpStatusString(expStatus));
                  }
                ...
                if (!outputExprError && predicate) {
                  ex_expr::exp_return_type expStatus = predicate->eval(upEntry->getAtp(), 0);

                  if (expStatus == ex_expr::EXPR_ERROR) {
                    UdrDebug1("  [WORK]   predicate.eval() returned %s", GetExpStatusString(expStatus));
                ...
                // statistics area
                if (udrBaseStats_ != NULL) udrBaseStats_->incActualRowsReturned();
...
NABoolean ExUdrTcb::verifyUdrServerProcessId() {
  ...
  if (!getIpcBroken() && udrServer_ && !ProcessIdIsNull(serverProcessId_)) {
    if (udrServer_->getState() != ExUdrServer::EX_UDR_BROKEN && udrServer_->getServerProcessId() == serverProcessId_) {
      result = TRUE;
    } else {
      setUdrTcbState(IPC_ERROR);
      ...
      *da << DgSqlCode(-EXE_UDR_SERVER_WENT_AWAY);
      ...
      if (udrServer_ && udrServer_->getUdrControlConnection()) {
        char buf2[300];
        const IpcProcessId &runningServer = udrServer_->getUdrControlConnection()->getOtherEnd();
...
void ExUdrTcb::attemptTransitionToWorkState() {
  ...
  //
  if (state_ == WORK_IO_ACTIVE && !anyOutstandingQueueRequests() && dataStream_->numOfResponsesPending() == 0)
    setUdrTcbState(WORK);
}
...
NABoolean ExUdrTcb::setUdrTcbState(UdrTcbState target) {
  ...
  if (doTrace_ || doStateTrace_) {
    FILE *f = traceFile_;
    UdrPrintf(f, "*** TCB %p state transition: %s -> %s", this, getUdrTcbStateString(source),
  ...
  switch (source) {
    ...
    case FIXUP:
      ...
      break;
    case SENDING_LOAD:
      if (target != WORK && target != LOAD_FAILED && target != SENDING_UNLOAD && target != IPC_ERROR) {
        result = FALSE;
      }
      break;
    case LOAD_FAILED:
      ...
      break;
    case WORK:
      if (target != WORK_IO_ACTIVE && target != SENDING_UNLOAD && target != IPC_ERROR &&
          target != SCALAR_INPUT_READY_TO_SEND) {
        result = FALSE;
      }
      break;
    case WORK_IO_ACTIVE:
      if (target != WORK && target != SENDING_UNLOAD && target != IPC_ERROR && target != READ_TABLE_INPUT_FROM_CHILD) {
        result = FALSE;
      }
      break;
    case SENDING_UNLOAD:
  ...
  if (!result) {
    char buf[256];
    str_sprintf(buf, "Invalid UDR state transition: %s -> %s", getUdrTcbStateString(source),
                getUdrTcbStateString(target));
    ex_assert(FALSE, buf);
  } else {
    state_ = target;
...
const char *ExUdrTcb::getUdrTcbStateString(UdrTcbState s) {
  switch (s) {
    ...
    case WORK:
      return "WORK";
    case WORK_IO_ACTIVE:
      return "WORK_IO_ACTIVE";
    case SENDING_UNLOAD:
      return "SENDING_UNLOAD";
    case IPC_ERROR:
      return "IPC_ERROR";
    case DONE:
      return "DONE";
    case SCALAR_INPUT_READY_TO_SEND:
      return "SCALAR_INPUT_READY_TO_SEND";
    case READ_TABLE_INPUT_FROM_CHILD:
      return "READ_TABLE_INPUT_FROM_CHILD";
    case RETURN_ROWS_FROM_CHILD:
      return "RETURN_ROWS_FROM_CHILD";
    case CHILD_INPUT_READY_TO_SEND:
    ...
    while (!done) {
      if (getIpcBroken()) {
        ...
        if (myTdb().isTmudf()) {
          ...
          printDataStreamState();
          if (dataStream_->numOfInputBuffers() > 0 || !replyBufferIsEmpty()) {
            UdrDebug0(
                "Not sending continue request since input buffers is >0 or reply buffer has data available to "
                "consume ");
            result = WORK_OK;
            done = TRUE;
            break;
        ...
        if (m == NULL) {
          ...
          if (doTrace_ || doIpcTrace_ || (getenv("UDR_CONTINUE_DEBUG") != NULL)) {
            ...
            TransIdToText(transIdToSend, msgTx, 256);
            if (isResultSet)
              UdrPrintf(f, "  [CONTINUE] Sending RS CONTINUE %d", (int)continueMsgsSent_ + 1);
            else
              UdrPrintf(f, "  [CONTINUE] Sending CONTINUE %d, msg tx %s", (int)continueMsgsSent_ + 1, msgTx);
          }
          ...
          if (state_ == WORK) setUdrTcbState(WORK_IO_ACTIVE);
...
ExWorkProcRetcode ExUdrTcb::tmudfWork() {
  ...
  UdrDebug1("  [TMUDF WORK] tmudfCheckReceive() returned %s", GetWorkRetcodeString(result));
  if (result == WORK_OK) {
    result = tmudfCheckSend();
    UdrDebug1("  [TMUDF WORK] tmudfCheckSend() returned %s", GetWorkRetcodeString(result));
  }
  if (result == WORK_OK) {
    result = continueRequest();
    UdrDebug1("  [TMUDF WORK] continueRequest() returned %s", GetWorkRetcodeString(result));
  }

  if (qParent_.down->isEmpty())  // Looking at the down queue entry alone may
                                 // not be correct. We may need to look at the state of the TCB too.
                                 // Perhaps have a new state - i.e WORK_EOD_SEEN ?
  {
...
ExWorkProcRetcode ExUdrTcb::tmudfCheckReceive() {
  ...
  while (!done && result == WORK_OK && anyOutstandingQueueRequests() && !qParent_.up->isFull() &&
         ((state_ != READ_TABLE_INPUT_FROM_CHILD) && (state_ != RETURN_ROWS_FROM_CHILD))) {
    if (getIpcBroken()) {
      ...
      switch (down_pstate.step_) {
        ...
        case CANCEL_AFTER_SEND: {
          ...
          if (repBuf && replyBufferIsEmpty() && !(down_pstate.numEodsFromChildTcbs_ == myTdb().numChildren())) {
            ...
            if (repBuf->sendMoreData()) {
              ...
              {
                UdrDebug0(" [TMUDF WORK checkreceive]!!!! ERROR EOD sent already 1 !!!!");
              }
              // see if udrserver has specified any table index
              if (repBuf->tableIndex() != -1) {
                down_pstate.currentChildTcbIndex_ = repBuf->tableIndex();
            ...
            result = returnSingleRow();
            if (replyBufferIsEmpty()) {
              UdrDebug0("[TMUDF WORK checkreceive]Reply Buffer Empty");
              // check if this is a reply to indicate that we can read
              // more from child tcbs.
              // if TRUE
              //  {
              ...
              //    done = TRUE;
              //  }
              if (repBuf->sendMoreData() && !(down_pstate.numEodsFromChildTcbs_ == myTdb().numChildren())) {
                // see if udrserver has specified any table index
                if (repBuf->tableIndex() != -1) {
                  down_pstate.currentChildTcbIndex_ = repBuf->tableIndex();
                  // change state_ from WORK_IO_ACTIVE to
                ...
                if (dataStream_->numOfInputBuffers() > 0) {
                  ...
                  UdrDebug0(
                      "[TMUDF WORK checkreceive] Received next reply buffer while processing one  stay in this method");
                }
                attemptTransitionToWorkState();
                break;
...
ExWorkProcRetcode ExUdrTcb::tmudfCheckSend() { return buildAndSendTmudfInput(); }

ExWorkProcRetcode ExUdrTcb::buildAndSendTmudfInput() {
  UdrDebug1("  [WORK] BEGIN ExUdrTcb::buildAndSendTmudfInput() 0x%08x", this);
  ...
  while (result == WORK_OK && !done && state_ != WORK_IO_ACTIVE &&
          ...
          case ex_queue::GET_ALL: {
            ...
            if (sqlBuf->moveInSendOrReplyData(TRUE,                        // [IN] sending? (vs. replying)
              ...
              if (myTdb().getInputExpression()) {
                ...
                expStatus = myTdb().getInputExpression()->eval(downEntry->getAtp(), workAtp_);
#ifdef UDR_DEBUG
                if (expStatus != ex_expr::EXPR_OK)
                  UdrDebug1("  [TMUDF WORK] scalar inputExpr.eval() returned %s", GetExpStatusString(expStatus));
#endif
                workAtp_->getTupp(myTdb().getRequestTuppIndex()).release();
      ...
      case SCALAR_INPUT_READY_TO_SEND: {
        ...
        if (sqlBuf->getTotalTuppDescs() > 0) {
          ...
          if (getStatsEntry() && getStatsEntry()->castToExUDRStats()) {
            getStatsEntry()->castToExUDRStats()->bufferStats()->sentBuffers().addEntry(sizeof(*requestBuffer_) +
                                                                                       sqlBuf->get_used_size());
            getStatsEntry()->castToExUDRStats()->bufferStats()->totalSentBytes() += sqlBuf->getSendBufferSize();
          }
      ...
      case RETURN_ROWS_FROM_CHILD: {
        ...
        if ((qChild_[currChild].up->isEmpty())) {
          ...
          if (childInputBuffers_[currChild]) {
            if (childInputBuffers_[currChild]->getSqlBuffer()->getTotalTuppDescs() > 0) {
        ...
        switch (child_status) {
          case ex_queue::Q_OK_MMORE: {
            ...
            if (childSqlBuf->moveInSendOrReplyData(
              ...
              // the message buffer.
              ex_assert(childSqlBuf->getTotalTuppDescs() > 0, "Request buffer not large enough to hold a single row");
              ...
              if (myTdb().getChildInputExpr(currChild)) {
                UInt32 childTuppIndex = myTdb().getTableDescInfo(currChild)->getOutputTuppIndex();

                workAtp_->getTupp(childTuppIndex) = dataDesc;

                // Evaluate the input expression.
                expStatus = myTdb().getChildInputExpr(currChild)->eval(centry->getAtp(), workAtp_);
#ifdef UDR_DEBUG
                if (expStatus != ex_expr::EXPR_OK)
                  UdrDebug1("  [TMUDF WORK] childInputExpr.eval() returned %s", GetExpStatusString(expStatus));
#endif
                // SS TBD change to getChildTuppIndex
                workAtp_->getTupp(childTuppIndex).release();

                if (expStatus == ex_expr::EXPR_ERROR) {
                  while (childSqlBuf->getTotalTuppDescs() > numTuppsBefore) {
                    childSqlBuf->remove_tuple_desc();
                  }
                  if (insertUpQueueEntry(ex_queue::Q_SQLERROR, centry->getDiagsArea())) {
                    pstate.step_ = ExUdrTcb::PRODUCE_EOD_AFTER_ERROR;
                    numErrors++;
                  } else {
                    processedUpEntry = FALSE;
              ...
              if (expStatus != ex_expr::EXPR_ERROR) pstate.step_ = ExUdrTcb::STARTED;
          ...
          case ex_queue::Q_NO_DATA: {
            ...
            UdrDebug1(" NUm Child EOD's so far = %u", pstate.numEodsFromChildTcbs_);
            qChild_[currChild].up->removeHead();
...
void ExUdrTcb::initializeDebugVariables() {
  ...
  if (doTrace_ || doStateTrace_ || doIpcTrace_) {
    ...
    const char *traceFileName = getenv(
        "UDR_"
        "DEBUG_FILE");
    ...
    if (myExeStmtGlobals()->castToExEspStmtGlobals()) {
      ...
      long numInstances = 0;
      long instanceNum = 0;
      instanceNum = myExeStmtGlobals()->getMyInstanceNumber();

      char esptraceFileName[100] = "";

      pid_t mypid = getpid();
      char *myRoot = getenv("TRAF_LOG");
      sprintf(esptraceFileName, "%s/espudrtrace.log%d_%d", myRoot, (int)instanceNum, (int)mypid);
...
void ExUdrTcb::addIntegrityCheckFailureToDiagsArea(ComDiagsArea *diags) const {
  ...
  if (!ProcessIdIsNull(serverProcessId_)) {
    *diags << DgSqlCode(-2037);
    myIpcEnv()->getMyOwnProcessId(IPC_DOM_GUA_PHANDLE).addProcIdToDiagsArea(*diags, 0);
...
void ExUdrTcb::tmudfCancelChildRequests(queue_index parentIndex) {
  for (int i = 0; i < numChildren(); i++) {
    if (tmudfStates_[i] == READING_FROM_CHILD) qChild_[i].down->cancelRequestWithParentIndex(parentIndex);
  }
}

NABoolean ExUdrTcb::validateDataRow(const tupp &replyTupp, ComDiagsArea *&diags) {
  const ExUdrTdb &tdb = myTdb();
  ComUInt32 numOutParams = tdb.getNumOutputValues();

  if (numOutParams == 0) return TRUE;
  ...
  NABoolean isProcedure = (tdb.getUdrType() == COM_PROCEDURE_TYPE ? TRUE : FALSE);

  ComUInt32 outPosition = 0;
  ComUInt32 startIndex = 0;
  if (!isProcedure) startIndex = numInParams;
...
NABoolean ExUdrTcb::serverResourcesAreLoaded() const {
  NABoolean result = FALSE;
  if (state_ == WORK || state_ == WORK_IO_ACTIVE || state_ == READ_TABLE_INPUT_FROM_CHILD ||
...
NABoolean ExUdrTcb::dataRequestsAreTransactional() const {
  ...
  //
  NABoolean result = FALSE;
  if (myTdb().getTransactionAttrs() == COM_TRANSACTION_REQUIRED && rsInfo_ == NULL) result = TRUE;
  return result;
}
...
//----------------------------------------------------------------------
ExUdrPrivateState::ExUdrPrivateState() { init(); }
