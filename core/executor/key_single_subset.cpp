...
keySingleSubsetEx::keySingleSubsetEx(const keyRangeGen &tdb_key, const short /* in_version */, sql_buffer_pool *pool,
                                     ex_globals *g, unsigned short mode, const ex_tcb *tcb)
    : keyRangeEx(tdb_key, g, 1, pool), keyReturned_(FALSE) {
  Space *space = g->getSpace();
  CollHeap *heap = g->getDefaultHeap();

  // mode = 0;  // 0 = PCODE_NONE
  if (bkPred()) (void)bkPred()->fixup(0, mode, tcb, space, heap, g->computeSpace(), g);
  if (ekPred()) (void)ekPred()->fixup(0, mode, tcb, space, heap, g->computeSpace(), g);
  if (bkExcludedExpr()) (void)bkExcludedExpr()->fixup(0, mode, tcb, space, heap, g->computeSpace(), g);
  if (ekExcludedExpr()) (void)ekExcludedExpr()->fixup(0, mode, tcb, space, heap, g->computeSpace(), g);
};
...
keyRangeEx::getNextKeyRangeReturnType keySingleSubsetEx::getNextKeyRange(atp_struct *atp0,
  ...
  if (!keyReturned_) {
    ...
    if (bkPred()) {
      ...
      workAtp_->getTupp(tdbKey_.getKeyValuesAtpIndex())
          .setDataPointer(
              ((tdbKey_.getKeytag() > 0) ? bkData_.getDataPointer() + sizeof(short) : bkData_.getDataPointer()));
      if (tdbKey_.getKeytag() > 0) {
        unsigned short keytag = tdbKey_.getKeytag();
        str_cpy_all(bkData_.getDataPointer(), (char *)&keytag, sizeof(short));
      }

      if ((rcbk = bkPred()->eval(atp0, workAtp_)) == ex_expr::EXPR_ERROR) return EXPRESSION_ERROR;

      bkConvErrorFlag = dataConvErrorFlag_;  // remember conv error flag

      if (bkExcludedExpr()) {
        if ((rcbkef = bkExcludedExpr()->eval(atp0, workAtp_)) == ex_expr::EXPR_ERROR) return EXPRESSION_ERROR;
    ...
    if (ekPred()) {
      ...
      workAtp_->getTupp(tdbKey_.getKeyValuesAtpIndex())
          .setDataPointer(
              ((tdbKey_.getKeytag() > 0) ? ekData_.getDataPointer() + sizeof(short) : ekData_.getDataPointer()));
      if (tdbKey_.getKeytag() > 0) {
        unsigned short keytag = tdbKey_.getKeytag();
        str_cpy_all(ekData_.getDataPointer(), (char *)&keytag, sizeof(short));
      }

      if ((rcek = ekPred()->eval(atp0, workAtp_)) == ex_expr::EXPR_ERROR) return EXPRESSION_ERROR;

      ekConvErrorFlag = dataConvErrorFlag_;  // remember conv error flag

      if (ekExcludedExpr()) {
        if ((rcekef = ekExcludedExpr()->eval(atp0, workAtp_)) == ex_expr::EXPR_ERROR) return EXPRESSION_ERROR;
