...
ex_tcb *ExProbeCacheTdb::build(ex_globals *glob) {
  ...
  ExProbeCacheTcb *pc_tcb = new (exe_glob->getSpace()) ExProbeCacheTcb(*this, *child_tcb, exe_glob);
...
ExProbeCacheTcb::ExProbeCacheTcb(const ExProbeCacheTdb &probeCacheTdb, const ex_tcb &child_tcb, ex_globals *glob)
  ...
  pool_ = new (space) ExSimpleSQLBuffer(probeCacheTdb.numInnerTuples_, probeCacheTdb.recLen_, space);
  ...
  probeHashTupp_.init(sizeof(probeHashVal_), NULL, (char *)(&probeHashVal_));
  workAtp_->getTupp(probeCacheTdb.hashValIdx_) = &probeHashTupp_;

  hashProbeExpr()->fixup(0, getExpressionMode(), this, space, heap, glob->computeSpace(), glob);

  probeEncodeTupp_.init(probeCacheTdb.probeLen_, NULL, (char *)(probeBytes_));
  workAtp_->getTupp(probeCacheTdb.encodedProbeDataIdx_) = &probeEncodeTupp_;

  encodeProbeExpr()->fixup(0, getExpressionMode(), this, space, heap, glob->computeSpace(), glob);

  if (moveInnerExpr()) moveInnerExpr()->fixup(0, getExpressionMode(), this, space, heap, glob->computeSpace(), glob);

  if (selectPred()) selectPred()->fixup(0, getExpressionMode(), this, space, heap, glob->computeSpace(), glob);

  pcm_ = new (space) ExPCMgr(space, probeCacheTdb.cacheSize_, probeCacheTdb.probeLen_, this);
}
...
ExOperStats *ExProbeCacheTcb::doAllocateStatsEntry(CollHeap *heap, ComTdb *tdb) {
  ...
  if (statsType == ComTdb::OPERATOR_STATS) {
    ...
    return new (heap) ExProbeCacheStats(heap, this, tdb, pcTdb->bufferSize_, pcTdb->cacheSize_);
  }
}
...
ExProbeCacheTcb::~ExProbeCacheTcb() { freeResources(); }
...
ExWorkProcRetcode ExProbeCacheTcb::workDown() {
  ...
  while (nextRequest_ != tlindex && !qchild_.down->isFull()) {
    ...
    ExProbeCachePrivateState &pstate = *((ExProbeCachePrivateState *)pentry->pstate);
    ...
    ex_assert(pstate.step_ == NOT_STARTED, "deja vu in ExProbeCacheTcb")
      ...
      case ex_queue::GET_ALL: {
        ...
        if ((hashProbeExpr()->eval(pentry->getAtp(), workAtp_)) != ex_expr::EXPR_OK) {
          ex_tcb::handleError(&qparent_, pentry->getDiagsArea());
          return -1;
        }
        // Evaluate the encode expression on the probe.
        if ((encodeProbeExpr()->eval(pentry->getAtp(), workAtp_)) != ex_expr::EXPR_OK)
        ...
        if (pcm_->addOrFindEntry(probeHashVal_, probeBytes_, nextRequest_, pstate.pcEntry_) == ExPCMgr::FOUND) {
          ...
          centry->downState.numGetNextsIssued = pentry->downState.numGetNextsIssued;
...
ExWorkProcRetcode ExProbeCacheTcb::workUp() {
  ...
  while ((qparent_.down->getHeadIndex() != nextRequest_) && !qparent_.up->isFull()) {
    ex_queue_entry *pentry_down = qparent_.down->getHeadEntry();
    ExProbeCachePrivateState &pstate = *((ExProbeCachePrivateState *)pentry_down->pstate);
    ...
    switch (pstate.step_) {
      case CACHE_MISS: {
        ...
        switch (reply->upState.status) {
          case ex_queue::Q_OK_MMORE: {
            MoveStatus moveRetCode = moveReplyToCache(*reply, *pstate.pcEntry_);
            ...
            if (moveRetCode == MOVE_BLOCKED) {
              ...
              makeReplyToParentUp(pentry_down, pstate, ex_queue::Q_OK_MMORE);
              ...
              qchild_.down->cancelRequestWithParentIndex(pstate.pcEntry_->probeQueueIndex_);

              break;
            } else {
              ex_assert(moveRetCode == MOVE_ERROR, "bad retcode from moveReplyToCache");
          ...
          case ex_queue::Q_SQLERROR: {
            ...
            makeReplyToParentUp(pentry_down, pstate, ex_queue::Q_SQLERROR);
      ...
      case CACHE_HIT: {
        switch (pstate.pcEntry_->upstateStatus_) {
          case ex_queue::Q_OK_MMORE: {
            ...
            makeReplyToParentUp(pentry_down, pstate, pstate.pcEntry_->upstateStatus_);
      ...
      case CANCELED_MISS: {
        ...
        if (pstate.pcEntry_->refCnt_ != 0) {
          ...
          switch (reply->upState.status) {
            case ex_queue::Q_OK_MMORE: {
              MoveStatus moveRetCode2 = moveReplyToCache(*reply, *pstate.pcEntry_);
              if (moveRetCode2 == MOVE_BLOCKED) {
                ...
                ex_assert(moveRetCode2 == MOVE_ERROR, "bad retcode from moveReplyToCache");
                // Don't break from this Q_OK_MMORE case, but
                // instead flow down as if Q_SQLERROR.  The
                // diagsArea should have been init'd in the
                // moveInnerExpr()->eval.
        ...
        if (stats) stats->incCanceledMiss();
        pstate.step_ = DONE_MISS;
        break;
      }
      case CANCELED_HIT: {
        if (stats) stats->incCanceledHit();
        pstate.step_ = DONE;
        break;
      }
      case CANCELED_NOT_STARTED: {
        if (stats) stats->incCanceledNotStarted();
        pstate.step_ = DONE;
        break;
      }
      case DONE_MISS: {
        ...
        while (!finishedDoneMiss) {
          ...
          if (reply2->upState.status == ex_queue::Q_NO_DATA) finishedDoneMiss = TRUE;
    ...
    if (pentry->downState.request == ex_queue::GET_NOMORE) {
      switch (pstate.step_) {
        ...
        case NOT_STARTED: {
          ...
          ex_assert(0, "canceling & parent down queue processing out of sync");
        }
        default: {
          ex_assert(0, "canceling found bad pstate.step_");
        }
      }
    }
    qindex++;
...
void ExProbeCacheTcb::makeReplyToParentUp(ex_queue_entry *pentry_down, ExProbeCachePrivateState &pstate,
  ...
  if ((reply_status == ex_queue::Q_OK_MMORE) && (pstate.pcEntry_->innerRowTupp_.isAllocated())) {
    ...
    // the pool.
    up_entry->getAtp()->getTupp(probeCacheTdb().tuppIndex_) = pstate.pcEntry_->innerRowTupp_;

    if (selectPred()) {
      ex_expr::exp_return_type evalRetCode = selectPred()->eval(up_entry->getAtp(), 0);

      if (evalRetCode == ex_expr::EXPR_FALSE)
        rowQualifies = 0;
      else if (evalRetCode != ex_expr::EXPR_TRUE) {
        ex_assert(evalRetCode == ex_expr::EXPR_ERROR, "invalid return code from expr eval");
  ...
  if (rowQualifies) {
    ...
    if (pstate.pcEntry_ && pstate.pcEntry_->diagsArea_) {
      ComDiagsArea *accumulatedDiagsArea = up_entry->getDiagsArea();
    ...
    // Insert the reply.
    qparent_.up->insert();
    if ((stats = getStatsEntry()) != NULL && reply_status == ex_queue::Q_OK_MMORE) stats->incActualRowsReturned();
  }
  return;
}
...
ExProbeCacheTcb::MoveStatus ExProbeCacheTcb::moveReplyToCache(ex_queue_entry &reply, ExPCE &pcEntry) {
  if (moveInnerExpr()) {
    ex_assert(!pcEntry.innerRowTupp_.isAllocated(), "reusing an allocated innerRowTupp");

    if (pool_->getFreeTuple(pcEntry.innerRowTupp_)) return MOVE_BLOCKED;

    workAtp_->getTupp(probeCacheTdb().innerRowDataIdx_) = pcEntry.innerRowTupp_;

    // Evaluate the move expression on the reply.
    ex_expr::exp_return_type innerMoveRtn = moveInnerExpr()->eval(reply.getAtp(), workAtp_);

    if (innerMoveRtn == ex_expr::EXPR_ERROR) return MOVE_ERROR;
  } else {
    ex_assert(pcEntry.innerRowTupp_.isAllocated() == FALSE, "Incorrectly initialized inneRowTupp_");
  }
...
void ExProbeCacheTcb::cancelInterest(ExPCE *pcEntry) {
  ...
  if ((pcEntry->refCnt_ == 0) && (pcEntry->upstateStatus_ == ex_queue::Q_INVALID)) {
    ...
    qchild_.down->cancelRequestWithParentIndex(pcEntry->probeQueueIndex_);
...
ExProbeCachePrivateState::ExProbeCachePrivateState() { init(); }
...
///////////////////////////////////////////////////////////////////
ExPCMgr::~ExPCMgr() {
  if (buckets_ != NULL) NADELETEBASIC(buckets_, space_);
  buckets_ = NULL;

  if (entries_ != NULL) NADELETEBASIC(entries_, space_);
  entries_ = NULL;
}
...
ExPCMgr::AddedOrFound ExPCMgr::addOrFindEntry(int probeHashVal, char *probeBytes, queue_index qIdxForCancel,
  ...
  while (pcEntry) {
    if ((pcEntry->probeHashVal_ == probeHashVal) && (memcmp((char *)pcEntry->probeData_, probeBytes, probeLen_) == 0)) {
...
ExPCE *ExPCMgr::addEntry(int buckNum, int probeHashVal, char *probeBytes, queue_index qIdxForCancel) {
    ...
    for (;;) {
      ...
      if (pcEntryToUnlink == pce) {
        if (prevPCE == NULL) {
          ...
          if ((stats) && (buckets_[bucketNum] == NULL)) stats->freeChain();
        } else {
  ...
  pce->refCnt_ = 1;
  if (stats) stats->updateUseCount(1);
