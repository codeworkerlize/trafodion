...
MdamIntervalList::MdamIntervalList() : firstIntervalPtr_(0), lastIntervalPtr_(0) {}
#endif /* NA_MDAM_EXECUTOR_DEBUG_ILTF */
...
// because access to the heap manager is needed.
  ...
  ex_assert(isEmpty(), "MdamIntervalList::~MdamIntervalList() called for a non-empty interval list.");
#endif /* NA_MDAM_EXECUTOR_DEBUG */
}
...
void MdamIntervalList::deleteAllIntervals(FixedSizeHeapManager &mdamIntervalHeap,
...
// Move entries from this into tempIntervalList.
  ...
  MdamIntervalListMerger getNextPoint(&tempIntervalList, &otherList, keyLen);
  ...
  while (endPoint1.exists()) {
    if (getNextPoint.getActiveIntervals() == 2) {
      ...
      MdamInterval *tempIntervalPtr = new (mdamIntervalHeap) MdamInterval(endPoint1, endPoint2);
  ...
  // can't.
  tempIntervalList.deleteAllIntervals(mdamIntervalHeap, mdamRefListEntryHeap);
...
// Move entries from this into tempIntervalList.
  ...
  while (getNextPoint(currentEndPoint)) {
    if (getNextPoint.getPreviousActiveIntervals() != 0 &&
      ...
      // the reference list.
      MdamInterval *tempIntervalPtr = new (mdamIntervalHeap) MdamInterval(
          previousEndPoint, currentEndPoint, intervalPtr0, intervalPtr1, disjunctNum, mdamRefListEntryHeap);
    ...
    if (currentEndPoint.end()) {
      previousEndPoint.reverseInclusion();
    };
  };  // end while
