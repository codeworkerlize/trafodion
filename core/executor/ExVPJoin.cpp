...
ex_tcb *ExVPJoinTdb::build(ex_globals *glob) {
  ...
  // then VPJoin TCB
  //
  ExVPJoinTcb *vpjTcb = new (glob->getSpace()) ExVPJoinTcb(*this, childTcbs, glob);
...
ExVPJoinTcb::ExVPJoinTcb(const ExVPJoinTdb &vpjTdb, const ex_tcb **childTcbs, ex_globals *glob)
    : ex_tcb(vpjTdb, 1, glob), childTcbs_(childTcbs) {
  ...
  //
  qParent_.down = new (space) ex_queue(ex_queue::DOWN_QUEUE, vpjTdb.fromParent_, vpjTdb.givenCriDesc_, space);
  ...
  //
  qParent_.up = new (space) ex_queue(ex_queue::UP_QUEUE, vpjTdb.toParent_, vpjTdb.returnedCriDesc_, space);

  if (filterPred()) (void)filterPred()->fixup(0, getExpressionMode(), this, glob->getSpace(), glob->getDefaultHeap());
...
ExWorkProcRetcode ExVPJoinTcb::workUp() {
  ...
  for (; (ix = qParent_.down->getHeadIndex()) != nextReqIx_; qParent_.down->removeHead()) {
    ...
    if (pstate.started_) {
      ...
      while (!eodReply) {
        ...
        //
        eodReply = qChild_[0].up->getHeadEntry()->upState.status == ex_queue::Q_NO_DATA;
        ...
        if (!requestCancelled && !eodReply) {
          ...
          if (!filterPred() || filterPred()->eval(resAtp, 0) == ex_expr::EXPR_TRUE) {
        ...
        for (i = 0; i < numChildren_; i++) qChild_[i].up->removeHead();
      }
    } else {
      // Request hasn't been started, which means that it is
      // cancelled and hasn't been sent to children.
      //
      ex_assert(request == ex_queue::GET_NOMORE, "Expected request to be cancelled");
...
void ExVPJoinTcb::cancelParentRequest(ex_queue_entry *pentry, queue_index pix) {
  ExVPJoinPrivateState &pstate = *((ExVPJoinPrivateState *)pentry->pstate);
...
ExWorkProcRetcode ExVPJoinTcb::workCancel() {
  ...
  for (queue_index ci = qParent_.down->getHeadIndex(); ci < nextReqIx_; ci++) {
    ex_queue_entry *pentry = qParent_.down->getQueueEntry(ci);

    if (pentry->downState.request == ex_queue::GET_NOMORE) cancelParentRequest(pentry, ci);
  }

  return WORK_OK;
}

short ExVPJoinTcb::sWorkDown(ex_tcb *tcb) { return ((ExVPJoinTcb *)tcb)->workDown(); }
short ExVPJoinTcb::sWorkUp(ex_tcb *tcb) { return ((ExVPJoinTcb *)tcb)->workUp(); }
short ExVPJoinTcb::sWorkCancel(ex_tcb *tcb) { return ((ExVPJoinTcb *)tcb)->workCancel(); }

ExVPJoinPrivateState::ExVPJoinPrivateState(const ExVPJoinTcb *tcb) { init(); }
