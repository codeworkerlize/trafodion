...
ExOperStats *ex_send_bottom_tcb::doAllocateStatsEntry(CollHeap *heap, ComTdb *tdb) {
  ...
  if (statsType == ComTdb::OPERATOR_STATS) {
    ...
    stat = new (heap) ExESPStats(heap, sendBottomTdb().getReplyBufferSize(), sendBottomTdb().getRequestBufferSize(),
                                 getParentInstanceNum(), this, tdb);
  }
  if (stat) {
...
ex_send_bottom_tcb::ex_send_bottom_tcb(const ex_send_bottom_tdb &sendBottomTdb, ExExeStmtGlobals *glob,
  ...
  // put into ATPs)
  qSplit_.down = new (space) ex_queue(ex_queue::DOWN_QUEUE, sendBottomTdb.queueSizeDown_, sendBottomTdb.criDescDown_,
                                      space, ex_queue::ALLOC_ATP);

  // there is no private state for send bottom nodes

  qSplit_.up = new (space) ex_queue(ex_queue::UP_QUEUE, sendBottomTdb.queueSizeUp_, sendBottomTdb.criDescUp_, space);
  ...
  // fixup expression
  if (moveOutputValues())
    (void)moveOutputValues()->fixup(0, getExpressionMode(), this, glob->getSpace(), glob->getDefaultHeap(), FALSE,
                                    glob);
  ...
  IpcMessageObjSize maxBufSize = requestBufferSize_ > replyBufferSize_ ? requestBufferSize_ : replyBufferSize_;
  ...
  if (numRequestBuffers < 2) numRequestBuffers = 2;

  ExEspInstanceThread *threadInfo = glob->getThreadInfo();

  // create a msg stream object to route msgs to my other two msg streams.
  routeMsgStream_ = new (glob->getSpace()) ExSendBottomRouteMessageStream(glob, this, threadInfo);

  workMsgStream_ = new (glob->getSpace())
      ExSendBottomWorkMessageStream(glob, numReplyBuffers, numRequestBuffers, maxBufSize, this, threadInfo);

  // create a cancel message stream object, just in case.
  cancelMsgStream_ = new (glob->getSpace()) ExSendBottomCancelMessageStream(glob, this, threadInfo);
  ...
  if (sendBottomTdb.getExchangeUsesSM()) {
    ...
    connection_ = new (glob->getSpace())
        SMConnection(glob->getIpcEnvironment(), smTarget_, numRequestBuffers + 1, maxBufSize, this, NULL, TRUE);
    ...
    EXSM_TRACE(EXSM_TRACE_INIT | EXSM_TRACE_TAG, "SNDB  frag %d inst %d", myFrag, (int)glob->getMyInstanceNumber());
    EXSM_TRACE(EXSM_TRACE_INIT | EXSM_TRACE_TAG, "SNDB  parent frag %d inst %d", parentFrag, parentInst);

    // regress/executor/TEST123 expects this line of output when
    // tracing is reduced to tags only
    EXSM_TRACE(EXSM_TRACE_INIT | EXSM_TRACE_TAG, "SNDB  tgt %d:%d:%" PRId64 ":%d", (int)smTarget_.node,
    ...
    EXSM_TRACE(EXSM_TRACE_INIT, "SNDB  request size %d", (int)requestBufferSize_);
    EXSM_TRACE(EXSM_TRACE_INIT, "SNDB  reply size %d", (int)replyBufferSize_);
    EXSM_TRACE(EXSM_TRACE_INIT, "SNDB  stream max buf %d", (int)workMsgStream_->getBufferSize());
    EXSM_TRACE(EXSM_TRACE_INIT, "SNDB  conn %p", connection_);
    EXSM_TRACE(EXSM_TRACE_INIT, "SNDB  req bufs %d reply bufs %d", (int)numRequestBuffers, (int)numReplyBuffers);
    EXSM_TRACE(EXSM_TRACE_INIT, "SNDB  down queue %d, up queue %d", downQueueLen, upQueueLen);
...
/////////////////////////////////////////////////////////////////////////////
// destructor
ex_send_bottom_tcb::~ex_send_bottom_tcb() { ex_send_bottom_tcb::freeResources(); }
...
void ex_send_bottom_tcb::freeResources() {
  delete qSplit_.up;
  delete qSplit_.down;
  if (workAtp_) deallocateAtp(workAtp_, getSpace());
...
void ex_send_bottom_tcb::registerSubtasks() {
  ...
  // top node.
  getGlobals()->getScheduler()->registerUnblockSubtask(sWork, this, qSplit_.down, "WK");
  getGlobals()->getScheduler()->registerInsertSubtask(sWork, this, qSplit_.up);

  // register a non-queue event for the IPC with the send top node
  ioSubtask_ = getGlobals()->getScheduler()->registerNonQueueSubtask(sWork, this);
  // need another non-queue event for the cancel message stream
  ioCancelSubtask_ = getGlobals()->getScheduler()->registerNonQueueSubtask(sCancel, this, "CN");
...
short ex_send_bottom_tcb::work() {
  ...
  EXSM_TRACE(EXSM_TRACE_WORK, "SNDB %p checkRequest rc %s", this, ExWorkProcRetcodeToString(result));

  if (result == WORK_OK) {
    result = checkReply();
    EXSM_TRACE(EXSM_TRACE_WORK, "SNDB %p checkReply rc %s", this, ExWorkProcRetcodeToString(result));
  }

  EXSM_TRACE(EXSM_TRACE_WORK, "SNDB %p END WORK %s", this, ExWorkProcRetcodeToString(result));
...
short ex_send_bottom_tcb::checkRequest() {
  ...
  while (NOT qSplit_.down->isFull()) {  // process data from send top
    ...
    if (currentRequestBuffer_ == NULL) {
      ...
      sb = currentRequestBuffer_->get_sql_buffer();
      sb->driveUnpack();
      sb->position();
    ...
    if (!isExtractConsumer_) getGlobals()->setStatsEnabled(sb->statsEnabled());
    ...
    while (NOT qSplit_.down->isFull()) {
      ...
      str_cpy_all((char *)&msgControlInfo, controlInfoAsTupp->getTupleAddress(), sizeof(ControlInfo));

      rowReceived = msgControlInfo.getIsDataRowPresent() ? sb->getNext() : NULL;
      ...
      if (msgControlInfo.getIsExtDiagsAreaPresent()) {
        ...
        ex_assert_both_sides(workMsgStream_->getNextObjType(msgType) AND msgType == IPC_SQL_DIAG_AREA,
      ...
      if (isExtractWorkDone_) {
        ...
        *diags << DgSqlCode(-EXE_PARALLEL_EXTRACT_OPEN_ERROR) << DgString0(phandle)
...
TupMsgBuffer *ex_send_bottom_tcb::getRequestBuffer() {
  ...
  while (workMsgStream_->getNextReceiveMsg(msgType)) {
    ex_assert_both_sides(msgType == IPC_MSG_SQLESP_DATA_REQUEST, "received message from unknown message stream");

    while (workMsgStream_->getNextObjType(msgType)) {
      if (sendBottomTdb().getExchangeUsesSM()) {
        ESPMessageObjTypeEnum t = (ESPMessageObjTypeEnum)msgType;
        EXSM_TRACE(EXSM_TRACE_BUFFER, "SNDB %p RECV %s", this, getESPMessageObjTypeString(t));
      }

      if (msgType == ESP_OPEN_HDR) {  // reply to open message, return int
        ExEspOpenReqHeader *reqHdr = new (workMsgStream_->receiveMsgObj()) ExEspOpenReqHeader(workMsgStream_);
        ...
        if (getStatsEntry() && getStatsEntry()->castToExESPStats())
          getStatsEntry()->castToExESPStats()->setSendTopStatID(reqHdr->statID_);

        // construct reply message and send back with
        // int
        ExEspReturnStatusReplyHeader *replyHdr = new (*workMsgStream_) ExEspReturnStatusReplyHeader((NAMemory *)NULL);
        replyHdr->key_ = reqHdr->key_;
        replyHdr->handle_ = getMyHandle();
        replyHdr->instanceState_ = ExEspReturnStatusReplyHeader::INSTANCE_ACTIVE;

        workMsgStream_->sendResponse();
      } else if (msgType == ESP_CONTINUE_HDR) {
        ExEspContinueReqHeader *reqHdr = new (workMsgStream_->receiveMsgObj()) ExEspContinueReqHeader(workMsgStream_);
        ...
        ExEspInputDataReqHeader *reqHdr = new (workMsgStream_->receiveMsgObj()) ExEspInputDataReqHeader(workMsgStream_);

        ex_assert_both_sides(workMsgStream_->getNextObjType(msgType) AND msgType == ESP_INPUT_SQL_BUFFER,
                             "input data header w/o input data buffer received");
        ex_assert_both_sides(!lateCancel_, "late cancel overlaps following request");

        getGlobals()->setInjectErrorAtQueue(reqHdr->injectErrorAtQueueFreq_);

        IpcMessageObj *msgHeap = workMsgStream_->receiveMsgObj();
        if (!msgHeap) return NULL;
...
short ex_send_bottom_tcb::checkReply() {
  ...
  while (NOT qSplit_.up->isEmpty() || currentReplyBuffer_) {
    if (currentReplyBuffer_ == NULL) {  // get sql buffer from message stream
      if (workMsgStream_->sendLimitReached()) {
        EXSM_TRACE(EXSM_TRACE_PROTOCOL, "SNDB %p send limit reached", this);
        return WORK_OK;
      }
      ...
      if (lateCancel_) {
        ...
        ex_assert_both_sides(lcentry->upState.status == ex_queue::Q_NO_DATA, "late cancel should not produce data");

        ExWorkProcRetcode result;
        // Reply the cancel message
        result = replyCancel();
        if (result != WORK_OK) return result;  // actually WORK_POOL_BLOCKED
      ...
      getReplyBuffer();
      if (currentReplyBuffer_ == NULL) {
        EXSM_TRACE(EXSM_TRACE_PROTOCOL, "SNDB %p checkReply buffer not available", this);
    ...
    // fill sql buffer with data from queue
    {
      ...
      NABoolean isRowToBeSent = (sentry->upState.status == ex_queue::Q_OK_MMORE);
      ...
      if (moveRetcode == SqlBuffer::BUFFER_FULL) {
        ...
        if (sentry->getDiagsArea()) {
          UInt32 diagsBytes = (UInt32)sentry->getDiagsArea()->packedLength();
        ...
        if (sentry->upState.status == ex_queue::Q_NO_DATA) {
          EXSM_TRACE(EXSM_TRACE_PROTOCOL, "SNDB %p Placed EOD in reply buf", this);
          isEOFEntry = TRUE;
          ...
          if (getGlobals()->getStatsArea() && !isExtractConsumer_) {
            ...
            isStatsToBeSent = getGlobals()->getStatsArea()->sendStats();
          }
        } else {
          isEOFEntry = FALSE;
    ...
    if (replySpaceAvailable AND NOT isEOFEntry) return WORK_OK;
    ...
    if (isStatsToBeSent) {
      ex_assert_both_sides(msgControlInfo, "Should have added a new ControlInfo for stats!");
      ...
      UInt32 statsBytes = (UInt32)getGlobals()->getStatsArea()->packedLength();
      ...
      EXSM_TRACE(EXSM_TRACE_PROTOCOL, "SNDB %p stats bytes %d final %d", this, (int)statsBytes, (int)finalReplyBytes);
    }

    // For SeaMonster, mark the next reply buffer as "last in batch" if
    // the send bottom queues are empty
    if (sendBottomTdb().getExchangeUsesSM() && qSplit_.up->isEmpty() && qSplit_.down->isEmpty())
      workMsgStream_->setSMLastInBatch();

    EXSM_TRACE(EXSM_TRACE_PROTOCOL, "SNDB %p SEND reply tupps %d", this, (int)sqlBuf->getTotalTuppDescs());
  ...
  if (qSplit_.up->isEmpty() AND qSplit_.down->isEmpty() AND(workMsgStream_->numOfInputBuffers() == 0)) {
    ...
    EXSM_TRACE(EXSM_TRACE_PROTOCOL, "SNDB %p calling releaseBuffers", this);
    workMsgStream_->releaseBuffers();  // do final garbage collection

    if (workMsgStream_->numOfOutputBuffers() == 0 AND workMsgStream_->numOfReplyTagBuffers() == 0) {
      finish();
    } else {
      EXSM_TRACE(EXSM_TRACE_PROTOCOL, "SNDB %p buffers stuck in data stream", this);
      EXSM_TRACE(EXSM_TRACE_PROTOCOL, "SNDB %p out %d reply tag %d", this, (int)workMsgStream_->numOfOutputBuffers(),
...
void ex_send_bottom_tcb::getReplyBuffer() {
  if (currentReplyBuffer_) return;

  // construct return data reply header in message
  ExEspReturnDataReplyHeader *hdr = new (*workMsgStream_) ExEspReturnDataReplyHeader((NAMemory *)NULL);
  if (hdr == NULL) return;

  // construct sql buffer(TupMsgBuffer) directly in message to avoid copy
  TupMsgBuffer *result =
      new (*workMsgStream_, replyBufferSize_) TupMsgBuffer(replyBufferSize_, TupMsgBuffer::MSG_OUT, workMsgStream_);
  if (result == NULL) ABORT("mismatched send bottom buffer size");
...
ExWorkProcRetcode ex_send_bottom_tcb::cancel() {
  ...
  EXSM_TRACE(EXSM_TRACE_CANCEL, "SNDB %p cancelReceiveBuffer %p late %d", this, cancelReceiveBuffer, (int)lateCancel_);

  if (cancelReceiveBuffer != NULL) {
    cancelReplyPending_ = TRUE;
    EXSM_TRACE(EXSM_TRACE_CANCEL, "SNDB %p cancelReplyPending_ TRUE", this);
    ...
    while ((controlInfoAsTupp = sb->getCurr())) {
      ...
      ex_assert_both_sides(msgControlInfo.getDownState().request == ex_queue::GET_NOMORE,
      ...
      if (lateCancel_) {
        ex_assert_both_sides(qSplit_.down->isEmpty(), "late cancel should see empty queue in send bottom");
        ex_queue_entry *sentry = qSplit_.down->getTailEntry();
        ...
        // cancelOnSight_ list.
        cancelOnSight_.insert(msgControlInfo.getDownState().parentIndex);
        ...
        // GET_NOMORE.
        if (currentRequestBuffer_->get_sql_buffer()->findAndCancel(msgControlInfo.getDownState().parentIndex, FALSE))
          cancelInDownQueue = FALSE;
        else
          cancelInDownQueue = TRUE;
      }

      if (cancelInDownQueue) qSplit_.down->cancelRequestWithParentIndex(msgControlInfo.getDownState().parentIndex);
    }
  ...
  if (!lateCancel_ && cancelReplyPending_) {
    ...
    if (result == WORK_OK) cancelReplyPending_ = FALSE;
  }

  EXSM_TRACE(EXSM_TRACE_CANCEL, "SNDB %p END CANCEL rc %s", this, ExWorkProcRetcodeToString(result));
  return result;
}
...
TupMsgBuffer *ex_send_bottom_tcb::getCancelRequestBuffer() {
  ...
  while (cancelMsgStream_->getNextReceiveMsg(msgType)) {
    ex_assert_both_sides(msgType == IPC_MSG_SQLESP_CANCEL_REQUEST, "received message from unknown message stream");
    ...
    while (cancelMsgStream_->getNextObjType(msgType)) {
      if (msgType == ESP_CANCEL_HDR) {
        ...
        ExEspCancelReqHeader *reqHdr = new (cancelMsgStream_->receiveMsgObj()) ExEspCancelReqHeader(cancelMsgStream_);
      } else if (msgType == ESP_LATE_CANCEL_HDR) {
        ExEspLateCancelReqHeader *reqHdr =
            new (cancelMsgStream_->receiveMsgObj()) ExEspLateCancelReqHeader(cancelMsgStream_);
      ...
      ex_assert_both_sides(cancelMsgStream_->getNextObjType(msgType) AND msgType == ESP_INPUT_SQL_BUFFER,
                           "cancel header w/o input data buffer received");

      TupMsgBuffer *buffer = new (cancelMsgStream_->receiveMsgObj()) TupMsgBuffer(cancelMsgStream_);
...
ExWorkProcRetcode ex_send_bottom_tcb::replyCancel() {
  ...
  // construct return data reply header in message
  ExEspCancelReplyHeader *hdr = new (*cancelMsgStream_) ExEspCancelReplyHeader((NAMemory *)NULL);

  if (hdr == NULL) {
    EXSM_TRACE(EXSM_TRACE_CANCEL, "SNDB %p replyCancel buffer not available", this);
  ...
  EXSM_TRACE(EXSM_TRACE_CANCEL, "SNDB %p END replyCancel rc %s", this, ExWorkProcRetcodeToString(result));
  return result;
}
...
void ex_send_bottom_tcb::start() {
  if (!isActive_) {
    ...
    isActive_ = TRUE;
    EXSM_TRACE(EXSM_TRACE_PROTOCOL, "SNDB %p started request, instDir reqs %d", this,
...
void ex_send_bottom_tcb::finish() {
  if (isActive_) {
    ...
    isActive_ = FALSE;
    EXSM_TRACE(EXSM_TRACE_PROTOCOL, "SNDB %p finished request, instDir reqs %d", this,
               (int)espInstanceDir_->getNumSendBottomRequests(myHandle_));
  }
}
...
void ex_send_bottom_tcb::startCancel() {
  ...
  EXSM_TRACE(EXSM_TRACE_CANCEL, "SNDB %p started cancel, instDir cancels %d", this,
...
void ex_send_bottom_tcb::finishCancel() {
  ...
  EXSM_TRACE(EXSM_TRACE_CANCEL, "SNDB %p finished cancel, instDir cancels %d", this,
             (int)espInstanceDir_->getNumSendBottomCancels(myHandle_));
}
...
void ex_send_bottom_tcb::setClient(IpcConnection *connection) {
  if (routeMsgStream_->getClient() && (routeMsgStream_->getClient() != connection)) {
    connection->dumpAndStopOtherEnd(true, false);
    ex_assert_both_sides(false, "Attempt to switch send bottom's client.");
  }
...
/////////////////////////////////////////////////////////////////////////////
// constructor
ExSendBottomRouteMessageStream::ExSendBottomRouteMessageStream(ExExeStmtGlobals *glob,
                                                               ex_send_bottom_tcb *sendBottomTcb,
                                                               ExEspInstanceThread *threadInfo)
    : IpcServerMsgStream(glob->getIpcEnvironment(), IPC_MSG_SQLESP_SERVER_ROUTING, CurrEspReplyMessageVersion,
                         0,  // sendBufferLimit
                         0,  // inUseBufferLimit
...
void ExSendBottomRouteMessageStream::actOnReceive(IpcConnection *connection) {
  if (sendBottomTcb_->sendBottomTdb().getExchangeUsesSM())
    EXSM_TRACE(EXSM_TRACE_PROTOCOL, "SNDB %p ROUTE s %p c %p", sendBottomTcb_, this, connection);
  ...
  IpcMessageObjType msgType;
  // TODO: replace this with some other call, this touches too many
  //       data members of the IpcBufferedMsgStream
  if (!getNextReceiveMsg(msgType)) {
    EXSM_TRACE(EXSM_TRACE_PROTOCOL, "SNDB %p ROUTE no data available", sendBottomTcb_);
  ...
  switch (msgType) {
    case ESP_OPEN_HDR:
      sendBottomTcb_->getEspFragInstanceDir()->openedSendBottom(sendBottomTcb_->getMyHandle());
...
void ExSendBottomWorkMessageStream::actOnSend(IpcConnection *connection) {
  if (sendBottomTcb_->sendBottomTdb().getExchangeUsesSM())
    EXSM_TRACE(EXSM_TRACE_PROTOCOL, "SNDB %p ACTS D %p c %p", sendBottomTcb_, this, connection);
...
void ExSendBottomWorkMessageStream::actOnReceive(IpcConnection *connection) {
  if (sendBottomTcb_->sendBottomTdb().getExchangeUsesSM())
    EXSM_TRACE(EXSM_TRACE_PROTOCOL, "SNDB %p ACTR D %p c %p", sendBottomTcb_, this, connection);
...
/////////////////////////////////////////////////////////////////////////////
// constructor
ExSendBottomCancelMessageStream::ExSendBottomCancelMessageStream(ExExeStmtGlobals *glob,
                                                                 ex_send_bottom_tcb *sendBottomTcb,
                                                                 ExEspInstanceThread *threadInfo)
    : IpcServerMsgStream(glob->getIpcEnvironment(), IPC_MSG_SQLESP_CANCEL_REPLY, CurrEspReplyMessageVersion,
                         0,                               // sendBufferLimit
                         0,                               // inUseBufferLimit
...
void ExSendBottomCancelMessageStream::actOnSend(IpcConnection *connection) {
  EXSM_TRACE(EXSM_TRACE_PROTOCOL, "SNDB %p ACTS C %p c %p", sendBottomTcb_, this, connection);

  if (connection) {
    if (getErrorInfo()) {
      ABORT("Error while replying to a cancel message");
    } else {
      EXSM_TRACE(EXSM_TRACE_PROTOCOL, "ExSendBottomCancelMessageStream::actSend(): incReplyMsg");
...
void ExSendBottomCancelMessageStream::actOnReceive(IpcConnection *connection) {
  EXSM_TRACE(EXSM_TRACE_CANCEL, "SNDB %p ACTR C %p c %p", sendBottomTcb_, this, connection);

  if (connection AND getErrorInfo()) {
    ABORT("Error while receiving data");
  }

  EXSM_TRACE(EXSM_TRACE_CANCEL, "ExSendBottomCancelMessageStream::actOnReceive()");
