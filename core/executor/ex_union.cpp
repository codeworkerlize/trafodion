...
ex_tcb *ex_union_tdb::build(ex_globals *glob) {
  ...
  ex_assert(!(isMergeUnion() && isOrderedUnion()), "MergeUnion, OrderedUnion can't be simultaneously true");

  //++ Triggers -
  ex_assert(!(isMergeUnion() && isBlockedUnion()), "MergeUnion, BlockedUnion can't be simultaneously true");

  ex_assert(numChildren() == 2, "ex_union_tcb::build - invalid number of children");

  left_tcb = tdbLeft_->build(glob);
  right_tcb = (tdbRight_ != (ComTdbPtr)NULL) ? tdbRight_->build(glob) : (ex_tcb *)NULL;

  if (isMergeUnion())
    union_tcb = new (glob->getSpace()) ex_m_union_tcb(*this, left_tcb, right_tcb, glob);
  else if (isOrderedUnion() || isBlockedUnion())  //++ Triggers - , add blocked union
    union_tcb =
        new (glob->getSpace()) ex_o_union_tcb(*this, left_tcb, right_tcb, glob, isBlockedUnion(), hasNoOutputs());
  else if (isConditionalUnion())
    union_tcb = new (glob->getSpace()) ex_c_union_tcb(*this, left_tcb, right_tcb, glob);
  else
    union_tcb = new (glob->getSpace()) ex_union_tcb(*this, left_tcb, right_tcb, glob);
...
ex_union_tcb::ex_union_tcb(const ex_union_tdb &union_tdb,  //
  ...
  pool_ = new (space) sql_buffer_pool(union_tdb.numBuffers_, union_tdb.bufferSize_, space);
  ...
  if (moveExpr(0))
    (void)moveExpr(0)->fixup(0, getExpressionMode(), this, glob->getSpace(), glob->getDefaultHeap(), FALSE, glob);

  // fixup right expression
  if (moveExpr(1))
    (void)moveExpr(1)->fixup(0, getExpressionMode(), this, glob->getSpace(), glob->getDefaultHeap(), FALSE, glob);
  ...
  if (condExpr())
    (void)condExpr()->fixup(0, getExpressionMode(), this, glob->getSpace(), glob->getDefaultHeap(), FALSE, glob);

  // fixup triggered action exception expression
  if (trigExceptExpr())
    (void)trigExceptExpr()->fixup(0, getExpressionMode(), this, glob->getSpace(), glob->getDefaultHeap(), FALSE, glob);
};
...
void ex_union_tcb::stop() {
  ...
  pentry->upState.setMatchNo(centry0->upState.getMatchNo() + centry1->upState.getMatchNo());
  ...
  if (c0da) da->mergeAfter(*c0da);
  if (c1da) da->mergeAfter(*c1da);
...
ExWorkProcRetcode ex_union_tcb::workDown() {
  ...
  while (qparent.down->entryExists(processedInputs_) && !childQueues_[0].down->isFull() &&
...
ExWorkProcRetcode ex_union_tcb::workUp() {
  ...
  while (qparent.down->getHeadIndex() != processedInputs_) {
    ...
    // parent
    while (!qparent.up->isFull() && (pstate.getErrorCode() || whichSide(pstate, side, endOfData))) {
      ...
      if ((pstate.childStates_[side] != CANCELLED_) && (centry->upState.status != ex_queue::Q_SQLERROR)) {
        // ++Triggers -
        ...
        if (hasNoOutputs() && centry->upState.status == ex_queue::Q_OK_MMORE &&
            !pentry_down->downState.request == ex_queue::GET_N) {
          // consume the child rows
          do {
            childQueues_[side].up->removeHead();
            if (!childQueues_[side].up->isEmpty()) centry = childQueues_[side].up->getHeadEntry();
          } while (!childQueues_[side].up->isEmpty() && centry->upState.status == ex_queue::Q_OK_MMORE);
        ...
        if (moveExpr(side)) {
          ...
          if (pool_->get_free_tuple(pentry->getAtp()->getTupp(union_tdb().tuppIndex_), union_tdb().unionReclen_)) {
          ...
          if (moveExpr(side)->eval(centry->getAtp(), pentry->getAtp()) == ex_expr::EXPR_ERROR) {
      ...
      else if ((pstate.childStates_[side] != CANCELLED_) && (centry->upState.status == ex_queue::Q_SQLERROR)) {
        if (trigExceptExpr())  // trigs
        {
          ...
          if (da->returnIndex(-ComDiags_TrigActionExceptionSQLCODE) == NULL_COLL_INDEX) {
      ...
      if (childQueues_[side].up->isEmpty() &&
          (numChildren() > 1)) {  // -- Triggers: should not get in for conditional, single child unions.
        // moving this row up made the queue empty, we can not
        // continue if there isn't a non EOD row in the other queue
        if (childQueues_[1 - side].up->isEmpty() || pstate.childStates_[1 - side] == DONE_) return WORK_OK;
      }
...
void ex_union_tcb::processError(ex_union_private_state &pstate, int &endOfData, atp_struct *atp) {
  ...
  if (pstate.childStates_[0] == DONE_ && pstate.childStates_[1] == DONE_) endOfData = 1;
}
...
ex_m_union_tcb::ex_m_union_tcb(const ex_union_tdb &union_tdb,  //
                               const ex_tcb *left_tcb,         // left queue pair
                               const ex_tcb *right_tcb,        // right queue pair
                               ex_globals *glob)
...
/////////////////////////////////////////////////////////////////
ex_o_union_tcb::ex_o_union_tcb(const ex_union_tdb &union_tdb,  //
                               const ex_tcb *left_tcb,         // left queue pair
                               const ex_tcb *right_tcb,        // right queue pair
                               ex_globals *glob,
...
int ex_o_union_tcb::whichSide(ex_union_private_state &pstate, int &side, int &endOfData) {
  ...
  if (childQueues_[pstate.whichSide_].up->getHeadEntry()->upState.status == ex_queue::Q_NO_DATA) {
    if (pstate.whichSide_ == 0) {
      ...
      ix = upQ->getHeadIndex();
      ix++;
      while (upQ->entryExists(ix) && upQ->getQueueEntry(ix)->upState.status == ex_queue::Q_NO_DATA) {
      ...
      if (blockedUnion_) phase3Event_->schedule();
      // -- Triggers -
      return 0;
    }
  }
...
ExWorkProcRetcode ex_o_union_tcb::workDownLeft() {
  ...
  while (qparent.down->entryExists(lprocessedInputs_) && !childQueues_[0].down->isFull()) startLeftchild();
...
ExWorkProcRetcode ex_o_union_tcb::workDownBlockedLeft() {
  ...
  if ((qparent.down->getHeadIndex() == lprocessedInputs_) && qparent.down->entryExists(lprocessedInputs_)) {
...
ExWorkProcRetcode ex_o_union_tcb::workDownRight() {
  ...
  while (qparent.down->entryExists(rprocessedInputs_) && rightRequestCnt_ > 0 && !childQueues_[1].down->isFull()) {
    startRightchild();
    rightRequestCnt_--;
  }
...
queue_index ex_o_union_tcb::whichSideParentIndex() {
  if ((lprocessedInputs_ - qparent.down->getHeadIndex()) > (rprocessedInputs_ - qparent.down->getHeadIndex()))
...
void ex_o_union_tcb::startRightchild() {
  ...
  ex_assert(pstate.whichSide_ == 0, "ex_o_union_tcb::startRightchild() right child started too early");
  ...
  if (union_tdb().inNotAtomicStmt()) rentry->setDiagsArea(0);
...
ex_c_union_tcb::ex_c_union_tcb(const ex_union_tdb &union_tdb, const ex_tcb *left_tcb, const ex_tcb *right_tcb,
                               ex_globals *glob)
    : ex_union_tcb(union_tdb, left_tcb, right_tcb, glob) {}
...
void ex_c_union_tcb::registerSubtasks() {
  ...
  for (i = 0; i < numChildren(); i++) sched->registerInsertSubtask(sWorkUp, this, childQueues_[i].up);
...
ExWorkProcRetcode ex_c_union_tcb::condWorkDown() {
  ...
  while (qparent.down->entryExists(processedInputs_)) {
    ...
    //
    ex_expr::exp_return_type retCode = condExpr()->eval(pentry_down->getAtp(), 0);
    ...
    if (retCode == ex_expr::EXPR_TRUE) {
      if (!(union_tdb().inNotAtomicStmt()))
        ...
        processedInputs_++;
        ComDiagsArea *da = ExRaiseSqlError(getGlobals()->getDefaultHeap(), pentry_down,
                                           (ExeErrorCode)-EXE_NOTATOMIC_ENABLED_AFTER_TRIGGER);
        pentry_down->setDiagsArea(da);
        workUpEvent_->schedule();
        continue;
      }
    } else if ((retCode == ex_expr::EXPR_FALSE) || (retCode == ex_expr::EXPR_NULL)) {
      pstate.whichChild_ = 1;
      ...
      if (trigExceptExpr())  // triggers
      {
        ...
        ex_assert((da != NULL) && (da->returnIndex(-ComDiags_TrigActionExceptionSQLCODE) == NULL_COLL_INDEX),
                  "Should not be here");
...
void ex_c_union_tcb::stop() {
  ...
  // Consume the child row.
  //
  if (pstate.validChild()) childQueues_[child].up->removeHead();
...
int ex_c_union_tcb::whichSide(ex_union_private_state &pstate, int &side, int &endOfData) {
  ...
  //
  if (childQueues_[child].up->isEmpty()) return 0;

  //
  // Check if the child's at the end of its data stream.
  //
  if (childQueues_[child].up->getHeadEntry()->upState.status == ex_queue::Q_NO_DATA) {
...
void ex_c_union_tcb::processError(ex_union_private_state &pstate, int &endOfData, atp_struct *atp) {
  ...
  if (!(childQueues_[child].up->isEmpty()) && pstate.validChild() && pstate.childStates_[child] != DONE_) {
...
void ex_c_union_tcb::processEODErrorOrWarning(NABoolean isWarning) {
  ...
  if (isWarning)
    da = ExRaiseSqlError(getGlobals()->getDefaultHeap(), puentry, (ExeErrorCode)-EXE_CS_EOD);
  else
    da = ExRaiseSqlError(getGlobals()->getDefaultHeap(), puentry, (ExeErrorCode)-EXE_CS_EOD_ROLLBACK_ERROR);
...
//
ex_union_private_state::ex_union_private_state() { init(); }
