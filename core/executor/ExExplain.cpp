...
ex_tcb *ExExplainTdb::build(ex_globals *glob) {
  ...
  explainTcb->initParamsTuple(getTupleLength(), criDescParams_, getLengthModName(), getLengthStmtPattern());
...
ExExplainTcb::ExExplainTcb(const ExExplainTdb &explainTdb, ex_globals *glob)
  ...
  pool_ = new (space) sql_buffer_pool(explainTdb.numBuffers_, explainTdb.bufferSize_, space);

  // Allocate the queues used to communicate with parent
  qParent_.down = new (space) ex_queue(ex_queue::DOWN_QUEUE, explainTdb.queueSizeDown_, explainTdb.criDescDown_, space);

  qParent_.up = new (space) ex_queue(ex_queue::UP_QUEUE, explainTdb.queueSizeUp_, explainTdb.criDescUp_, space);

  // fixup expressions

  if (getScanPred()) (void)getScanPred()->fixup(0, getExpressionMode(), this, space, heap, FALSE, glob);

  if (getParamsExpr()) (void)getParamsExpr()->fixup(0, getExpressionMode(), this, space, heap, FALSE, glob);
}
...
ex_queue_pair ExExplainTcb::getParentQueue() const { return (qParent_); };
...
int ExExplainTcb::loadModule() {
  ...
  if (!isNullModName()) {
    ...
    init_SQLMODULE_ID(&moduleId, SQLCLI_CURRENT_VERSION, modName_, 0, SQLCHARSETSTRING_ISO88591, strlen(modName_));
...
int ExExplainTcb::visitNode() {
  ...
  if (currentExplain_->getState() == ExplainTuple::SOME_EXPLAIN_INFO) {
    ...
    if (pool_->get_free_tuple(pEntryUp->getTupp(explainTdb().getTuppIndex()), currentExplain_->getUsedRecLength()))
      return 0;
    ...
    if ((qid_ == NULL) && (explainAddr_ == -1) && (explainStmt_ == NULL) && (explainPlan_ == NULL))
    ...
    str_cpy_all(pEntryUp->getTupp(explainTdb().getTuppIndex()).getDataPointer(), currentExplain_->getExplainTuple(),
    ...
    if (getScanPred()) predVal = getScanPred()->eval(pEntryUp->getAtp(), 0);
...
ExExplainTcb::traverseReturnCode ExExplainTcb::traverseTree() {
  ...
  while (currentExplain_) {
    ...
    // traversal
    if (qParent_.up->isFull()) return EXPL_TRAV_QUEUEFULL;
    ...
    if ((request == ex_queue::GET_NOMORE) ||
        ((request == ex_queue::GET_N) && ((UInt32)pEntryDown->downState.requestValue <= matchNo_)))
...
short ExExplainTcb::processExplainStmt() {
  ...
  if (!explainStmt_) return 0;

  if (explainFrag_) NADELETEBASIC(explainFrag_, getHeap());
  explainFrag_ = NULL;
  ...
  if (cliInterface.getExplainDataPtr()) {
    ...
    if (str_decode(explainFrag_, explainFragLen_, cliInterface.getExplainDataPtr(), cliInterface.getExplainDataLen()) <
        0) {
      diagsArea = pEntryDown->getAtp()->getDiagsArea();
      ExRaiseSqlError(getGlobals()->getDefaultHeap(), &diagsArea, EXE_NO_EXPLAIN_INFO);
      if (diagsArea != pEntryDown->getAtp()->getDiagsArea()) pEntryDown->getAtp()->setDiagsArea(diagsArea);
      goto label_error2;
    }

    // skip ExplainReposInfo and point explainAddr to actual explain structures.
    setExplainAddr((long)&explainFrag_[sizeof(ExplainReposInfo)]);
  } else {
    diagsArea = pEntryDown->getAtp()->getDiagsArea();
    ExRaiseSqlError(getGlobals()->getDefaultHeap(), &diagsArea, EXE_NO_EXPLAIN_INFO);
    if (diagsArea != pEntryDown->getAtp()->getDiagsArea()) pEntryDown->getAtp()->setDiagsArea(diagsArea);
...
short ExExplainTcb::processExplainPlan() {
  ...
  explainFrag_ = new (getHeap()) char[explainFragLen_];
  if (str_decode(explainFrag_, explainFragLen_, explainPlan_, explainPlanLen_) < 0) return 0;
  ...
  while (1) {
    switch (workState_) {
      ...
      case EXPL_GET_PARAMS:
        ...
        // paramsExpr.
        paramsAtp_->getTupp(explainTdb().criDescParams()->noTuples() - 1) = paramsTupp_;
        ...
        if ((modDir_) &&
            ((strlen(modDir_) >= 3) && (strncmp(modDir_, "/E/", 3) == 0) || (strncmp(modDir_, "/G/", 3) == 0))) {
      ...
      case EXPL_SEND_TO_SSMP: {
        ...
        ex_queue_entry *pEntryDown = qParent_.down->getHeadEntry();
        ComDiagsArea *diagsArea = pEntryDown->getAtp()->getDiagsArea();
        if (explainInfo == NULL) {
          // Pointer to request entry in parent down queue
          ExRaiseSqlError(getGlobals()->getDefaultHeap(), &diagsArea, EXE_NO_EXPLAIN_INFO);
          if (diagsArea != pEntryDown->getAtp()->getDiagsArea()) pEntryDown->getAtp()->setDiagsArea(diagsArea);
        ...
        if ((expDesc = (ExplainDesc *)expDesc->driveUnpack(explainFrag, &dummyExpDesc, cliGlobals->getIpcHeap())) ==
            NULL) {
...
ExplainDesc *ExExplainTcb::getNextExplainTree(long explainFragAddr) {
  if (NOT explainFromAddrProcessed_) {
    ...
    ComDiagsArea *diagsArea = pEntryDown->getAtp()->getDiagsArea();
    ExplainDesc dummyExpDesc;
    if ((expDesc = (ExplainDesc *)expDesc->driveUnpack(explainFrag, &dummyExpDesc, getSpace())) == NULL) {
      // ERROR during unpacking.
      // Most likely case is verison-unsupported.
      ExRaiseSqlError(getGlobals()->getDefaultHeap(), &diagsArea, EXE_NO_EXPLAIN_INFO);
      if (diagsArea != pEntryDown->getAtp()->getDiagsArea()) pEntryDown->getAtp()->setDiagsArea(diagsArea);
...
ExplainDesc *ExExplainTcb::getNextExplainTree() {
  if (explainAddr_ != -1) {
  ...
  if (!isNullStmtPattern()) {
    ...
    LikePatternString patternString(stmtPattern_, (stmtPattern_ ? str_len(stmtPattern_) : 0), CharInfo::ISO88591, "\\",
                                    1);
    ...
    if (pattern.error()) {
      ex_queue_entry *pEntryDown = qParent_.down->getHeadEntry();
      ComDiagsArea *diagsArea = pEntryDown->getAtp()->getDiagsArea();
      ExRaiseSqlError(getGlobals()->getDefaultHeap(), &diagsArea, EXE_INVALID_ESCAPE_SEQUENCE);
      if (diagsArea != pEntryDown->getAtp()->getDiagsArea()) pEntryDown->getAtp()->setDiagsArea(diagsArea);
    ...
    while (stmt = (Statement *)stmtList_->getNext()) {
      ...
      // Added for multi charset module names
      int moduleNameLen = getModNameLen(stmt->getModuleId());
      ...
      if (moduleName) length = (length > moduleNameLen) ? length : moduleNameLen;
      ...
      if (modNameMatches && (pattern.matches(ident, (ident ? str_len(ident) : 0), CharInfo::UTF8) == TRUE) &&
        ...
        ex_assert(stmt->getRootTdb()->getNodeType() == ComTdb::ex_ROOT, "Invalid TDB Tree");

        // Get the fragment directory of the statement.
        ExFragDir *fragDir = ((ex_root_tdb *)(stmt->getRootTdb()))->getFragDir();
        ...
        if (fragDir->getExplainFragDirEntry(fragOffset, fragLen, topNodeOffset) == 0) {
          ...
          //
          ExplainDesc dummyExpDesc;
          if ((expDesc = (ExplainDesc *)expDesc->driveUnpack(explainFrag, &dummyExpDesc, stmt->getUnpackSpace())) ==
              NULL) {
            // ERROR during unpacking.
...
void ExExplainTcb::initParamsTuple(int tupleLength, ex_cri_desc *criDescParams, int lengthModName,
                                   int lengthStmtPattern) {
...
void ExExplainTcb::copyParameters() {
  int i;
  ...
  if (!isNullModName()) {
    ...
    if (modName_[0] == '/') {
      ...
      while ((i > 0) && (NOT done)) {
        if (modName_[i] == '"') dQuoteSeen = NOT dQuoteSeen;
  ...
  if (!isNullStmtPattern()) {
    ...
    int paramsTupleLen = explainTdb().getLengthStmtPattern();
    if (explainTdb().getVCIndOffsetStmtPattern() != -1) {
      paramsTupleLen =
          explainTdb().getAttrStmtPattern()->getLength(&paramsTuple_[explainTdb().getVCIndOffsetStmtPattern()]);
    }
  ...
  if (modName_[0] == '\0') {
    int len = str_len(stmtPattern_);
    if (len > strlen("QID=") && str_ncmp(stmtPattern_, "QID=", 4) == 0)
      setQid(&stmtPattern_[4], len - 4);
    else if (len > strlen("EXPLAIN_QID=") && str_ncmp(stmtPattern_, "EXPLAIN_QID=", strlen("EXPLAIN_QID=")) == 0)
      setReposQid(&stmtPattern_[strlen("EXPLAIN_QID=")], len - strlen("EXPLAIN_QID="));
    else if (len > strlen("EXPLAIN_STMT=") && str_ncmp(stmtPattern_, "EXPLAIN_STMT=", strlen("EXPLAIN_STMT=")) == 0) {
      setExplainStmt(&stmtPattern_[strlen("EXPLAIN_STMT=")], len - strlen("EXPLAIN_STMT="));
    } else if (len > strlen("EXPLAIN_PLAN=") && str_ncmp(stmtPattern_, "EXPLAIN_PLAN=", strlen("EXPLAIN_PLAN=")) == 0) {
...
short ExExplainTcb::isNullModName() {
  ...
  if (explainTdb().getAttrModName()->getNullFlag()) {
    ...
    int nullOffset = explainTdb().getAttrModName()->getNullIndOffset();

    // We are assuming that the sizeof the Null Indicator is the
    // size of a short.
    ex_assert((explainTdb().getAttrModName()->getNullIndicatorLength() == sizeof(short)),
...
short ExExplainTcb::isNullStmtPattern() {
  // If the statement parameter attribute is Nullable
  if (explainTdb().getAttrStmtPattern()->getNullFlag()) {
    // Get the offset to the Null indicator in the paramsTuple_
    int nullOffset = explainTdb().getAttrStmtPattern()->getNullIndOffset();

    // We are assuming that the sizeof the Null Indicator is the
    // size of a short.
    ex_assert((explainTdb().getAttrStmtPattern()->getNullIndicatorLength() == sizeof(short)),
              "Null indicator not equal to sizeof(short)\n");
...
void ExExplainTcb::setQid(char *qid, int len) {
  if (qid_) {
    NADELETEBASIC(qid_, getHeap());
  }
...
void ExExplainTcb::setReposQid(char *reposQid, int len) {
  if (reposQid_) {
...
void ExExplainTcb::setExplainAddr(char *addr, int len) { explainAddr_ = str_atoi(addr, len); }

void ExExplainTcb::setExplainAddr(long addr) { explainAddr_ = addr; }

void ExExplainTcb::setExplainStmt(char *stmt, int len) { explainStmt_ = stmt; }

void ExExplainTcb::setExplainPlan(char *plan, int len) {
  explainPlan_ = plan;
  explainPlanLen_ = len;
}
...
RtsExplainFrag *ExExplainTcb::sendToSsmp() {
  ...
  if (getMasterCpu(qid_, (int)str_len(qid_), nodeName, MAX_SEGMENT_NAME_LEN + 1, cpu) == -1) {
    nodeName[0] = '\0';
    cpu = -1;
    IpcAllocateDiagsArea(diagsArea_, getHeap());
  ...
  if (sd)
    ...
    RtsTimeout = 0;
  explainReq = new (cliGlobals->getIpcHeap()) RtsExplainReq(rtsHandle, cliGlobals->getIpcHeap(), qid_, str_len(qid_));
  *ssmpMsgStream << *explainReq;
  ...
  if (!ssmpMsgStream->isReplyReceived()) {
    IpcAllocateDiagsArea(diagsArea_, getHeap());
    (*diagsArea_) << DgSqlCode(-EXE_RTS_TIMED_OUT) << DgString0(stmtPattern_) << DgInt0(RtsTimeout / 100);
    return NULL;
  }
  retryAttempts_ = 0;
...
short ExExplainTcb::getExplainData(ex_root_tdb *rootTdb, char *explain_ptr, int explain_buf_len,
  ...
  *ret_explain_len = 0;
  ...
  if (rootTdb->getFragDir()->getExplainFragDirEntry(fragOffset, fragLen, topNodeOffset) != 0) {
  ...
  *ret_explain_len = encodedFragLen;
  ...
  if ((!explain_ptr) || (explain_buf_len < (*ret_explain_len + 1 /*null terminator*/))) {
...
short ExExplainTcb::getExplainFromRepos(char *qid, int qidLen) {
  int cliRC = 0;
  ...
  str_sprintf(queryBuf,
              "select [first 1] explain_plan from %s.\"%s\".%s where query_id = '%s' and explain_plan is not null and "
              "char_length(explain_plan) > 0 order by exec_start_utc_ts ",
  ...
  if ((infoList->numEntries() == 0) || (infoList->numEntries() > 1)) {
    diagsArea = pEntryDown->getAtp()->getDiagsArea();
    ExRaiseSqlError(getGlobals()->getDefaultHeap(), &diagsArea, EXE_NO_QID_EXPLAIN_INFO);
    if (diagsArea != pEntryDown->getAtp()->getDiagsArea()) pEntryDown->getAtp()->setDiagsArea(diagsArea);
  ...
  if (vi->get(0, ptr, len)) goto label_error2;
...
short ExExplainTcb::storeExplainInRepos(CliGlobals *cliGlobals, long *execStartUtcTs, char *qid, int qidLen,
  ...
  if (!qid || (qidLen == 0) || (!execStartUtcTs)) return -1;
  ...
  str_sprintf(queryBuf,
              "update %s.\"%s\".%s set explain_plan = cast(? as varchar(%d) not null) where exec_start_utc_ts = "
              "CONVERTTIMESTAMP(%ld)  and query_id = '%s' ",
              TRAFODION_SYSCAT_LIT, SEABASE_REPOS_SCHEMA, REPOS_METRIC_QUERY_TABLE, REPOS_MAX_EXPLAIN_PLAN_LEN,
              *execStartUtcTs, qid);

  explainVarcharBuf = new (heap) char[sizeof(int) + explainDataLen];
  *(int *)explainVarcharBuf = explainDataLen;
  memcpy(&explainVarcharBuf[sizeof(int)], explainData, explainDataLen);
  cliRC = cliInterface.executeImmediateCEFC(queryBuf, explainVarcharBuf, sizeof(int) + explainDataLen, NULL, NULL,
                                            &rowsAffected);
  if (cliRC < 0) {
    goto label_error;
  }
  ...
  if (queryBuf) NADELETEBASIC(queryBuf, heap);

  if (explainVarcharBuf) NADELETEBASIC(explainVarcharBuf, heap);
