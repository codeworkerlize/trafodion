...
char *GetHistoryRowOLAP(void *data, int n, NABoolean leading, int winSize, int &retcode) {
  ...
  if (n < 0) {
    ...
    return NULL;
  } else if (n >= tcb->maxNumberHistoryRows_ && tcb->numberHistoryRows_ == tcb->maxNumberHistoryRows_) {
    retcode = -1;  // ERROR condition
    return NULL;
  } else if (n >= tcb->numberHistoryRows_) {
    if (leading || (n - tcb->numberHistoryRows_) >= winSize) retcode = 0;
    return NULL;
  }
...
// 3. Fixup expressions.
//
ExSequenceTcb::ExSequenceTcb(const ExSequenceTdb &myTdb, const ex_tcb &child_tcb, ex_globals *glob)
    : ex_tcb(myTdb, 1, glob),
      lastRow_(NULL),
      clusterDb_(NULL),
      cluster_(NULL),
  ...
  pool_ = new (space) sql_buffer_pool(myTdb.numBuffers_, myTdb.bufferSize_, space);
  ...
  ex_assert(maxNumberOfOLAPBuffers_ >= minNumberOfOLAPBuffers_, "maxNumberOfOLAPBuffers is too small");
  ...
  //
  if (sequenceExpr()) {
    ((ExpSequenceExpression *)sequenceExpr())->seqFixup((void *)this, GetHistoryRow, GetHistoryRowOLAP);
    sequenceExpr()->fixup(0, getExpressionMode(), this, space, heap_, FALSE, glob);
  }

  if (returnExpr()) {
    ((ExpSequenceExpression *)returnExpr())->seqFixup((void *)this, GetHistoryRow, GetHistoryRowFollowingOLAP);
    returnExpr()->fixup(0, getExpressionMode(), this, space, heap_, FALSE, glob);
  }

  if (postPred()) postPred()->fixup(0, getExpressionMode(), this, space, heap_, FALSE, glob);

  if (cancelExpr()) cancelExpr()->fixup(0, getExpressionMode(), this, space, heap_, FALSE, glob);

  if (checkPartitionChangeExpr()) {
    ((ExpSequenceExpression *)checkPartitionChangeExpr())->seqFixup((void *)this, GetHistoryRow, GetHistoryRowOLAP);
...
short ExSequenceTcb::work() {
  ...
  //
  for (queue_index tail = qparent_.down->getTailIndex(); (processedInputs_ != tail) && (!qchild_.down->isFull());
       processedInputs_++) {
  ...
  pstate = (ExSequencePrivateState *)pentry_down->pstate;
  request = pentry_down->downState.request;
  workAtp_->copyPartialAtp(pentry_down->getAtp(), 0, MINOF(myTdb().tuppIndex_, pentry_down->numTuples()) - 1);
  ...
  while (1) {
    ...
    switch (pstate->step_) {
      ...
      case ExSeq_CANCELLED: {
        ...
        //
        if (child_status == ex_queue::Q_NO_DATA) pstate->step_ = ExSeq_DONE;
      ...
      case ExSeq_WORKING_READ: {
        ...
        switch (centry->upState.status) {
          ...
          case ex_queue::Q_OK_MMORE: {
            if (checkPartitionChangeExpr() && currentHistRowPtr_) {
              workAtp_->getTupp(myTdb().tuppIndex_).setDataPointer(currentHistRowPtr_);
            ...
            if (isUnboundedFollowing()) {
              if (OLAPBuffersFlushed_) {
                ...
                NABoolean noMemory = advanceHistoryRow(TRUE /* checkMemoryPressure */);
                if (noMemory) {
            ...
            // workAtp_->display("readFunc() eval result", myTdb().getCriDescUp());
      ...
      case ExSeq_WORKING_RETURN: {
        ...
        if (!canReturnRows() && !getPartitionEnd() && !isUnboundedFollowing() &&
        ...
        //
        if (pool_->get_free_tuple(pentry_up->getTupp(myTdb().tuppIndex_), recLen())) return WORK_POOL_BLOCKED;

        char *tuppData = pentry_up->getTupp(myTdb().tuppIndex_).getDataPointer();

        advanceReturnHistoryRow();

        char *histData = currentRetHistRowPtr_;
        pentry_up->getTupp(myTdb().tuppIndex_).setDataPointer(histData);
        ...
        //
        pentry_up->getTupp(myTdb().tuppIndex_).setDataPointer(tuppData);
        ...
        if ((pstate->step_ != ExSeq_ERROR) && cancelExpr()) {
          ...
          retCode = cancelExpr()->eval(pentry_up->getAtp(), pentry_up->getAtp());
        ...
        if (isOverflowStarted()) {
          ...
          if (numberOfRowsReturnedBeforeReadOF_ == maxNumberOfRowsReturnedBeforeReadOF_) {
            ...
            for (int bufcount = numberOfOLAPBuffersFromOF_; bufcount; bufcount--) {
              ...
              if (bufcount > 1 && !afterLast) afterLast = firstOLAPBuffer_;  // cycle back
            }
      ...
      case ExSeq_END_OF_PARTITION: {
        ...
        if (lastRow_ && isUnboundedFollowing()) {
          ...
          // allocate a tuple header for workAtp
          if (pool_->get_free_tuple(workAtp_->getTupp(myTdb().tuppIndex_), 0)) return WORK_POOL_BLOCKED;
          // always points to last row when evaluating returnExpr_
          workAtp_->getTupp(myTdb().tuppIndex_).setDataPointer(lastRow_);
        }
      ...
      case ExSeq_OVERFLOW_READ: {
        assert(firstOLAPBufferFromOF_ && isUnboundedFollowing());
      ...
      case ExSeq_DONE: {
        ...
        pentry_up->upState.downIndex = qparent_.down->getHeadIndex();
        pentry_up->upState.setMatchNo(pstate->matchCount_);
        workAtp_->releasePartialAtp(0, MINOF(myTdb().tuppIndex_, pentry_down->numTuples()) - 1);
        ...
        //
        if (qparent_.down->getHeadIndex() == processedInputs_) return WORK_CALL_AGAIN;
...
NABoolean ExSequenceTcb::advanceHistoryRow(NABoolean checkMemoryPressure) {
  ...
  if (currentOLAPBuffer_ == NULL) {
    ...
    ex_assert(currentHistRowPtr_ == NULL,
              "ExSequenceTcb::advanceHistoryRow() - currentHistRowPtr_ is not a NULL pointer");
    currentHistRowPtr_ = currentOLAPBuffer_->getFirstRow();
    currentHistRowInOLAPBuffer_ = 0;
  } else {
    currentHistRowInOLAPBuffer_++;

    if (currentHistRowInOLAPBuffer_ == maxRowsInOLAPBuffer_) {  // reached end of this buffer
      currentHistRowInOLAPBuffer_ = 0;

      if (currentOLAPBuffer_->getNext() == NULL) {              // last buffer
        if (canAllocateOLAPBuffer() && !isOverflowStarted() &&  // once overflowing - no more new buffers
...
void ExSequenceTcb::advanceReturnHistoryRow() {
  if (currentRetOLAPBuffer_ == NULL)  // initially currentRetOLAPBuffer_ is null
  {
    ...
    ex_assert(currentRetHistRowPtr_ == NULL,
              "ExSequenceTcb::advanceReturnHistoryRow() - currentRetHistRowPtr_ is not a NULL pointer");
  ...
  if (currentRetHistRowInOLAPBuffer_ == maxRowsInOLAPBuffer_) {  // reached end of this buffer
    currentRetHistRowInOLAPBuffer_ = 0;

    if (currentRetOLAPBuffer_->getNext() == NULL) {  // last buffer
      currentRetOLAPBuffer_ = firstOLAPBuffer_;
      currentRetHistRowPtr_ = currentRetOLAPBuffer_->getFirstRow();
    } else {
...
NABoolean ExSequenceTcb::addNewOLAPBuffer(NABoolean checkMemoryPressure) {
  if (checkMemoryPressure && !clusterDb_->enoughMemory(olapBufferSize_))
  ...
  if (cluster_) {
    ...
    tmpBuf = new (heap_) HashBuffer(olapBufferSize_, allocRowLength_, false, heap_, clusterDb_, &rc_);
  }
  if (tmpBuf == NULL || tmpBuf->getDataPointer() == NULL) {
  ...
  maxNumberHistoryRows_ = numberOfOLAPBuffers_ * maxRowsInOLAPBuffer_;
  if (isUnboundedFollowing()) {
    maxNumberOfRowsReturnedBeforeReadOF_ = (numberOfOLAPBuffers_ - numberOfWinOLAPBuffers_) * maxRowsInOLAPBuffer_;
  }
...
void ExSequenceTcb::initializeHistory() {
  ...
  if (firstOLAPBuffer_ == NULL) {
    ...
    if (!shrinkOLAPBufferList()) ex_assert(0, "initializeHistory-- can not shrink buffer list");
  }
...
void ExSequenceTcb::createCluster() {
  ...
  if (myTdb().memoryQuotaMB() > 0 && myTdb().memoryQuotaMB() * ONE_MEG < availableMemory)
    availableMemory = myTdb().memoryQuotaMB() * ONE_MEG;

  int minMemQuotaMB = myTdb().isPossibleMultipleCalls() ? myTdb().memoryQuotaMB() : 0;

  // in case we recreate, delete the old objects (incl. old scratch file)
  if (cluster_) delete cluster_;
  if (clusterDb_) delete clusterDb_;

  int minB4Chk = myTdb().getBmoMinMemBeforePressureCheck() * ONE_MEG;

  clusterDb_ = new (heap_)
      ClusterDB(ClusterDB::SEQUENCE_OLAP, myTdb().OLAPBufferSize_,
                NULL,  // workAtp_,
                ...
                availableMemory, memMonitor, myTdb().pressureThreshold_, getGlobals()->castToExExeStmtGlobals(), &rc_,
                myTdb().isNoOverflow(), FALSE, /*isPartialGroupBy*/
                ...
                myTdb().memoryQuotaMB(), minMemQuotaMB,
  ...
  cluster_ = new (heap_) Cluster(Cluster::IN_MEMORY, clusterDb_,
...
ex_tcb_private_state *ExSequencePrivateState::allocate_new(const ex_tcb *tcb) {
  return new (((ex_tcb *)tcb)->getSpace()) ExSequencePrivateState();
};
