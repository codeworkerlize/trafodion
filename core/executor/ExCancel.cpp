...
ex_tcb *ExCancelTdb::build(ex_globals *glob) {
  ExMasterStmtGlobals *exe_glob = glob->castToExExeStmtGlobals()->castToExMasterStmtGlobals();

  ex_assert(exe_glob, "Cancel operator must be in master.");

  ExCancelTcb *cancel_tcb = new (exe_glob->getSpace()) ExCancelTcb(*this, exe_glob);
...
ExCancelTcb::~ExCancelTcb() { freeResources(); }
...
ExWorkProcRetcode ExCancelTcb::work() {
  ...
  while ((qparent_.down->isEmpty() == FALSE) && (qparent_.up->isFull() == FALSE)) {
    ...
    switch (step_) {
      case NOT_STARTED: {
        if (pentry_down->downState.request == ex_queue::GET_NOMORE)
          ...
          if (cancelTdb().getAction() == ComTdbCancel::CancelByPname) {
            int nid = -1;
            int rc = msg_mon_get_process_info(cancelTdb().getCancelPname(), &nid, &pid_);
            switch (rc) {
              case XZFIL_ERR_OK:
                cpu_ = (short)nid;
                break;
              ...
              default: {
                char buf[200];
                str_sprintf(buf,
                            "Unexpected error %d returned from "
                            "msg_mon_get_process_info",
                            rc);
                ex_assert(0, buf);
              }
            }
            if (step_ != NOT_STARTED) break;
            ...
            if (XZFIL_ERR_OK == rc)
              ...
              if ((XZFIL_ERR_NOTFOUND != rc) && (XZFIL_ERR_BADNAME != rc) && (XZFIL_ERR_NOSUCHDEV != rc)) {
            ...
            if (getMasterCpu(qid, qid_len, nodeName_, sizeof(nodeName_) - 1, cpu_) == -1) {
              ComDiagsArea *diagsArea = ComDiagsArea::allocate(getGlobals()->getDefaultHeap());
          ...
          cbServer_ =
              ssmpManager->getSsmpServer((NAHeap *)getGlobals()->getDefaultHeap(), nodeName_, cpu_, tempDiagsArea);
          if (cbServer_ == NULL) {
            reportError(tempDiagsArea, true, EXE_CANCEL_PROCESS_NOT_FOUND, nodeName_, cpu_);
          ...
          cancelStream_ =
              new (cliGlobals->getIpcHeap()) CancelMsgStream(cliGlobals->getEnvironment(), this, ssmpManager);
      ...
      case SEND_MESSAGE: {
        ...
        if (cancelTdb().actionIsCancel()) {
          ...
          NABoolean cancelEscalationSaveabend =
              cliGlobals->currContext()->getSessionDefaults()->getCancelEscalationSaveabend();

          bool cancelLogging = (TRUE == cliGlobals->currContext()->getSessionDefaults()->getCancelLogging());

          CancelQueryRequest *cancelMsg = new (cliGlobals->getIpcHeap()) CancelQueryRequest(
              rtsHandle, cliGlobals->getIpcHeap(), cancelStartTime, firstEscalationInterval, secondEscalationInterval,
              cancelEscalationSaveabend, cancelTdb().getCommentText(), str_len(cancelTdb().getCommentText()),
              cancelLogging, cancelTdb().action_ != ComTdbCancel::CancelByQid, pid_,
          ...
          *cancelStream_ << *cancelMsg;
          ...
          bool suspendLogging = (TRUE == cliGlobals->currContext()->getSessionDefaults()->getSuspendLogging());

          SuspendQueryRequest *suspendMsg = new (cliGlobals->getIpcHeap()) SuspendQueryRequest(
              rtsHandle, cliGlobals->getIpcHeap(), ComTdbCancel::Force == cancelTdb().forced_, suspendLogging);

          *cancelStream_ << *suspendMsg;

          suspendMsg->decrRefCount();
        } else {
          ex_assert(ComTdbCancel::Activate == cancelTdb().action_, "invalid action for ExCancelTcb");

          bool suspendLogging = (TRUE == cliGlobals->currContext()->getSessionDefaults()->getSuspendLogging());

          ActivateQueryRequest *activateMsg =
              new (cliGlobals->getIpcHeap()) ActivateQueryRequest(rtsHandle, cliGlobals->getIpcHeap(), suspendLogging);
      ...
      case GET_REPLY: {
        ...
        if ((cbServer_->getControlConnection()->getErrorInfo() != 0) || fakeError201) {
          ComDiagsArea *diagsArea = ComDiagsArea::allocate(getGlobals()->getDefaultHeap());

          cbServer_->getControlConnection()->populateDiagsArea(diagsArea, getGlobals()->getDefaultHeap());

          if (fakeError201) {
            *diagsArea << DgSqlCode(-2034) << DgInt0(201) << DgString0("I say") << DgString1("control broker");
          }
          ...
          if (diagsArea->contains(-8921)) {
            ...
            // debugged.
            cbServer_->getControlConnection()->dumpAndStopOtherEnd(true, false);
        ...
        if (!cancelStream_->moreObjects()) return WORK_OK;

        ControlQueryReply *reply =
            new (cliGlobals->getIpcHeap()) ControlQueryReply(INVALID_RTS_HANDLE, cliGlobals->getIpcHeap());
        ...
        *cancelStream_ >> *reply;
        ...
        if (reply->didAttemptControl()) {
          ...
          if (cancelStream_->moreObjects() && cancelStream_->getNextObjType() == IPC_SQL_DIAG_AREA) {
            ComDiagsArea *diagsArea = ComDiagsArea::allocate(getGlobals()->getDefaultHeap());

            *cancelStream_ >> *diagsArea;
            cancelStream_->clearAllObjects();

            if (retryQidNotActive_ && (diagsArea->mainSQLCODE() == -EXE_SUSPEND_QID_NOT_ACTIVE) &&
                (++retryCount_ <= 60)) {
              SQLMXLoggingArea::logExecRtInfo(__FILE__, __LINE__, "Retrying error 8672.", 0);
              DELAY(500);
...
void ExCancelTcb::reportError(ComDiagsArea *da, bool addCondition, int SQLCode, char *nodeName, short cpu) {
  ...
  if (prevDiagsArea) da->mergeAfter(*prevDiagsArea);
