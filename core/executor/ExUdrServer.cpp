...
#endif
{
  ex_assert(options && optionDelimiters, "No runtime options specified for UDR server startup");
...
CollHeap *ExUdrServer::myIpcHeap() const { return myIpcEnv()->getHeap(); }
...
ExUdrServer::ExUdrServerStatus ExUdrServer::start(ComDiagsArea **diags, CollHeap *diagsHeap, long transId,
  ...
  if (ready()) {
    ...
    // one ESP per node.
    IpcCpuNum collocatedCPU = myIpcEnv()->getMyOwnProcessId(IPC_DOM_GUA_PHANDLE).getCpuNum();

    ipcServer_ = udrServerClass_->allocateServerProcess(diags, diagsHeap, NULL, collocatedCPU, IPC_PRIORITY_DONT_CARE,
                                                        1,  // espLevel (not relevant for UDR servers)
                                                        usesTransactions, waitedCreation, nowaitDepth);
    ...
    if (diags && *diags) {
      ...
      if (sqlcode < 0) {
        ...
        //
        UdrDebug0("  ***");
        UdrDebug1("  *** WARNING: Errors occurred. Main SQLCODE is %d", sqlcode);
        UdrDebug0("  ***");
        ipcServer_ = NULL;
      }
    }
...
void ExUdrServer::sendStartupOptions(ComDiagsArea **diags, CollHeap *diagsHeap, long transId) {
  ...
  ex_assert(ipcServer_ && ipcServer_->getControlConnection(), "Do not call this method without a running server");
  ...
  if (userName_ == NULL && (str_cmp_ne(options_, "OFF") == 0 || str_cmp_ne(options_, "ANYTHING") == 0)) {
    return;
  }
  ...
  if (str_cmp_ne(options_, "OFF") != 0 && str_cmp_ne(options_, "ANYTHING") != 0) {
    if (userName_) {
      ...
      // Copy the user name
      str_cpy_all(optionsToSend + optionsLen + delimiterLen, userNameOption, userNameOptionLen);
    ...
    if (userName_) {
      if (userPassword_) {
        ...
        // Copy the user password
        str_cpy_all(optionsToSend + userNameOptionLen + delimiterLen, passwordOption, passwordOptionLen);
        optionsToSend[len] = '\0';
      } else {
  ...
  UdrClientControlStream *stream = new (ipcHeap) UdrClientControlStream(myIpcEnv(),
                                                                        NULL,  // tcb
                                                                        NULL,  // stmt globals
                                                                        TRUE,  // keep diags for caller
                                                                        isTransactional, threadInfo_);
  ...
  UdrSessionMsg *msg = new (ipcHeap) UdrSessionMsg(UdrSessionMsg::UDR_SESSION_TYPE_JAVA_OPTIONS, 0, ipcHeap);
...
IpcConnection *ExUdrServer::getUdrControlConnection() const {
  ...
  if (ready() && ipcServer_) {
    ...
    if (result && result->getState() == IpcConnection::ERROR_STATE) result = NULL;
  }
  return result;
}
...
IpcConnection *ExUdrServer::getAnIpcConnection() const {
  ...
  if (numFreeConns > 0) {
    ...
    conn = serverProcessId_.createConnectionToServer(myIpcEnv(), TRUE, nowaitDepth);
...
void ExUdrServer::releaseConnection(IpcConnection *conn) {
  ...
  if (conn == getUdrControlConnection()) return;
  ...
  if (!inUseConns_->remove(conn)) {
    ...
    if (getUdrControlConnection()) ex_assert(0, "A connection that is being released is not in use.");
  }
  ...
  if (conn->getState() == IpcConnection::ERROR_STATE || state_ == ExUdrServer::EX_UDR_BROKEN ||
...
NABoolean ExUdrServer::isIOPending(IpcConnection *conn) const {
  NABoolean result = FALSE;
  if (ipcServer_ && conn) {
    NABoolean ioPendingOnConnection = conn->sendIOPending() || conn->receiveIOPending();
    NABoolean anythingQueuedOnConnection =
        (conn->numQueuedSendMessages() > 0) || (conn->numQueuedReceiveMessages() > 0);
    if (ioPendingOnConnection || anythingQueuedOnConnection || conn->numReceiveCallbacksPending() > 0) {
      result = TRUE;
    }
  }
  return result;
...
void ExUdrServer::completeUdrRequests(IpcConnection *conn, NABoolean waitForAllIO) const {
#ifdef UDR_DEBUG
...
NABoolean ExUdrServer::match(const int &userId, const char *options, const char *optionDelimiters,
  ...
  if (threadInfo_ != threadInfo) {
    return FALSE;
  }

  if (str_cmp_ne(options, "ANYTHING") == 0 || str_cmp_ne(options_, "ANYTHING") == 0) {
    return TRUE;
  }

  if (str_cmp_ne(options, "OFF") == 0 && str_cmp_ne(options_, "OFF") == 0) {
    return TRUE;
  }

  if (str_cmp_ne(options_, options) == 0 && str_cmp_ne(optionDelimiters_, optionDelimiters) == 0) {
    return TRUE;
  }
...
// ExUdrServerManager
// -----------------------------------------------------------------------
ExUdrServerManager::ExUdrServerManager(IpcEnvironment *env, ComUInt32 maxServersPerGroup)
    : ipcEnvironment_(env),
...
#endif
{
  ...
  //
  udrServerClass_ = new (myIpcHeap()) IpcServerClass(ipcEnvironment_, IPC_SQLUDR_SERVER);
...
ExUdrServer *ExUdrServerManager::acquireUdrServer(const int &userId, const char *options, const char *optionDelimiters,
  ...
  for (CollIndex i = 0; i < entries; i++) {
    ...
    if (s->match(userId, options, optionDelimiters, threadInfo) && (!s->isDedicated())) {
      numServersMatched++;
  ...
  UdrDebug2("  Found %u matching server%s", numServersMatched, numServersMatched == 1 ? "" : "s");
  ...
  if (dedicated && reUseExistingServer && (udrServer->getRefCount() != 0)) {
  ...
  if (reUseExistingServer) {
    ...
    udrServer = new (myIpcHeap()) ExUdrServer(ipcEnvironment_, userId, newOptions, optionDelimiters, userName,
                                              userPassword, threadInfo, udrServerClass_);
#ifdef UDR_DEBUG
    if (traceFile_) {
  ...
  UdrDebug2("  Returning ExUdrServer %p, ref count %u", udrServer, udrServer->getRefCount());
...
void ExUdrServerManager::releaseUdrServer(ExUdrServer *udrServer) {
  ...
  for (CollIndex i = 0; !found && i < serverPool_.entries(); i++) {
    if (udrServer == serverPool_[i]) {
      ...
      UdrDebug2("  Found server %p. Ref count decremented to %u", udrServer, udrServer->getRefCount());
