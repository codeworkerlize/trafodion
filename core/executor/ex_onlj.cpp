...
#endif
{
  ...
  if ((isLeftJoin()) && (nljTdb.ljExpr_)) {
    pool_ = new (glob->getSpace()) sql_buffer_pool(nljTdb.numBuffers_, nljTdb.bufferSize_, glob->getSpace());

    // Allocate a NULL tuple for use in null instantiation.
    if (onljTdb().ljRecLen_ > 0) {
      int nullLength = onljTdb().ljRecLen_;
  ...
  // fixup expressions
  if (beforePred_)
    (void)beforePred_->fixup(0, getExpressionMode(), this, glob->getSpace(), glob->getDefaultHeap(), FALSE, glob);
  if (afterPred_)
    (void)afterPred_->fixup(0, getExpressionMode(), this, glob->getSpace(), glob->getDefaultHeap(), FALSE, glob);
  if (onljTdb().ljExpr_)
    (void)onljTdb().ljExpr_->fixup(0, getExpressionMode(), this, glob->getSpace(), glob->getDefaultHeap(), FALSE, glob);
}
...
void ExOnljTcb::cancelParentRequest(ex_queue_entry *pentry) {
  ...
  switch (pstate.leftStep_) {
    ...
    case NLJ_LEFT_DONE:
      ...
      for (; pstate.startRightIndex_ != pstate.endRightIndex_; pstate.startRightIndex_++) {
...
ExWorkProcRetcode ExOnljTcb::work_phase1() {
  ...
  QRLogger::log(CAT_SQL_EX_ONLJ, LL_DEBUG, "ExOnljTcb::work_phase1: TopToLeft(this ExOnljTcb=%p, timestamp=%s)", this,
                reportTimestamp());
  ...
  for (; phaseOne_ != tail; phaseOne_++) {
    ...
    // moved to phase one
    if (qleft_.down->isFull()) return WORK_OK;
    ...
    if (request == ex_queue::GET_NOMORE) {
      ...
      if (phaseOne_ == qparent_.down->getHeadIndex()) exceptionEvent3_->schedule();
    } else
        // BertBert VVV
    ...
    // BertBert ^^^
      ...
      if (request == ex_queue::GET_N && !(isLeftJoin() && afterPred_ == NULL))
  ...
  if (!qparent_.down->isEmpty()) {
    ...
    if ((pentry1->downState.numGetNextsIssued != pstate1.pushedDownGetNexts_) && !pstate1.qGetDoneFromLeft_) {
      ...
      if (pstate1.pushedDownGetNexts_ > pentry1->downState.numGetNextsIssued) {
        ...
        if (pentry1->downState.request == ex_queue::GET_NEXT_N ||
            pentry1->downState.request == ex_queue::GET_NEXT_N_SKIP) {
        ...
        } else if (pentry1->downState.request == ex_queue::GET_NEXT_N_MAYBE_WAIT ||
                   pentry1->downState.request == ex_queue::GET_NEXT_N_MAYBE_WAIT_SKIP) {
          if (pstate1.rightRecSkipped_ == ExConstants::EX_TRUE)
          ...
          pstate1.pushedDownGetNexts_++;
        } else if (pentry1->downState.request == ex_queue::GET_NEXT_0_MAYBE_WAIT) {
        ...
        if ((pentry1->downState.numGetNextsIssued - pstate1.pushedDownGetNexts_) > 1) {
          cout << "ExOnljTcb::work_phase1, too many requests."
               << " pentry1->downState.numGetNextsIssued = " << pentry1->downState.numGetNextsIssued
               << " pstate1.pushedDownGetNexts_ = " << pstate1.pushedDownGetNexts_ << endl;
        }
#endif
        ...
        if (pentry1->downState.request == ex_queue::GET_NEXT_N ||
          ...
          pstate1.pushedDownGetNexts_++;
                   ...
                   pentry1->downState.request == ex_queue::GET_NEXT_N_MAYBE_WAIT_SKIP) {
          if (pstate1.rightRecSkipped_ == ExConstants::EX_TRUE)
            qleft_.down->getNextNMaybeWaitSkipRequest(pentry1->downState.requestValue -
                                                      lentry1->downState.requestValue);
          else
...
ExWorkProcRetcode ExOnljTcb::work_phase2() {
  ...
  for (; phaseTwo_ != tail; phaseTwo_++) {
    ...
    ex_queue_entry *pentry = qparent_.down->getQueueEntry(lentry->upState.parentIndex);
    ExOnljPrivateState &pstate = *((ExOnljPrivateState *)pentry->pstate);
    ...
    if (request == ex_queue::GET_NOMORE &&
        (lentry->upState.status == ex_queue::Q_OK_MMORE || lentry->upState.status == ex_queue::Q_SQLERROR)) {
    ...
    switch (lentry->upState.status) {
      case ex_queue::Q_OK_MMORE:
        ...
        // moved to phase two
        if (qright_.down->isFull()) return WORK_OK;

        if (pstate.leftStep_ == NLJ_LEFT_STARTED) {
          QRLogger::log(CAT_SQL_EX_ONLJ, LL_DEBUG,
                        "ExOnljTcb::work_phase2: left_started(this ExOnljTcb=%p, timestamp=%s)", this,
                        reportTimestamp());
        ...
        if (beforePred_) retCode = beforePred_->eval(lentry->getAtp(), 0);
      ...
      case ex_queue::Q_SQLERROR:
        ...
        if (onljTdb().isRowsetIterator() && onljTdb().isNonFatalErrorTolerated() && da &&
            ((da->getNextRowNumber(ComCondition::NONFATAL_ERROR) == ComCondition::NONFATAL_ERROR))) {
          ...
          if ((da->getNonFatalErrorIndexToBeSet()) && (onljTdb().isSetNFErrorJoin())) {
            ...
            if (pstate.accumDiags_) {
              if (!(pstate.accumDiags_->canAcceptMoreErrors())) {
                ...
                if (!(da->canAcceptMoreErrors())) da->removeLastErrorCondition();

                *da << DgSqlCode(-EXE_NONATOMIC_FAILURE_LIMIT_EXCEEDED) << DgInt0(pstate.accumDiags_->getLengthLimit());
              } else
          ...
          // process this entry in phase 3
          {
            ...
            // wake up work_phase3() which may now be able to proceed
            exceptionEvent3_->schedule();
            if (!(onljTdb().isSetNFErrorJoin())) da->setNonFatalErrorIndexToBeSet(FALSE);
            break;
          }
        }
      ...
      case ex_queue::Q_NO_DATA:
        switch (pstate.leftStep_) {
          ...
          case NLJ_LEFT_NOT_EMPTY:
            ...
            if (!vsbbInsertOn() || pentry->downState.request == ex_queue::GET_NOMORE) {
              // the NO_DATA row from the left child needs to be taken out
              // of the queue in phase 3 after everything else was processed
              pstate.leftOnlyRows_++;
          ...
          case NLJ_SEND_EOD:
            ...
            if (qright_.down->isFull()) return WORK_CALL_AGAIN;
          ...
          case NLJ_LEFT_DONE:
          case NLJ_LEFT_CANCELLED:
            ex_assert(0, "ExOnljTcb::work_phase2() Invalid leftStep_ state for Q_NO_DATA");
            break;
        }
...
ExWorkProcRetcode ExOnljTcb::work_phase3() {
  ...
  QRLogger::log(CAT_SQL_EX_ONLJ, LL_DEBUG, "ExOnljTcb::work_phase3: (this ExOnljTcb=%p, timestamp=%s) called", this,
                reportTimestamp());

  for (; (phaseThree = qparent_.down->getHeadIndex()) != phaseOne_; qparent_.down->removeHead()) {
    ex_queue_entry *uentry = qparent_.up->getTailEntry();
    ...
    if (onljTdb().isRowsetIterator() && onljTdb().isNonFatalErrorTolerated()) {
      ...
      if (!processNonFatalErrorsInLeftUpQueue(projected, consumed)) return WORK_OK;
      if (projected)
        // restore a good value for uentry
        uentry = qparent_.up->getTailEntry();
    }

    if (pstate.leftStep_ == NLJ_LEFT_STARTED) return WORK_OK;  // no phase 3 work for this parent row yet
    ...
    for (; !qright_.up->isEmpty() && pstate.leftMatches_ > 0; qright_.up->removeHead()) {
      ...
      switch (right_status) {
        case ex_queue::Q_OK_MMORE: {
          ...
          if (rda && onljTdb().isRowsetIterator() && onljTdb().isNonFatalErrorTolerated() && rda->mainSQLCODE() < 0) {
          ...
          if (rda && onljTdb().isRowsetIterator() && onljTdb().isNonFatalErrorTolerated() &&
              (rda->mainSQLCODE() == 0)) {
            if (!qparent_.up->isFull()) {
          ...
          if (onljTdb().ljExpr_) {
            if (pool_->get_free_tuple(uentry->getAtp()->getTupp(onljTdb().criDescUp_->noTuples() - 1),
                                      onljTdb().ljRecLen_)) {
          ...
          if (!isAntiJoin()) {
            ...
            if (qparent_.up->isFull()) {
              ...
              phase3Timer_.stop();
              QRLogger::log(CAT_SQL_EX_ONLJ, LL_DEBUG,
                            "ExOnljTcb::work_phase3: upQ full(ExOnljTcb=%p, added rows=%ld, phase3s=%ld, timeSpent=%s)",
                            this, rows2upq, phase3s_, reportTimeDiff(phase3Timer_.elapsedTime()));
#endif
          ...
          break;
        }  // end of case ex_queue::Q_OK_MMORE:
...

        {
#ifdef NEED_INSTRUMENT_ONLJ
          QRLogger::log(CAT_SQL_EX_ONLJ, LL_DEBUG,
                        "ExOnljTcb::work_phase3: noMoreData(this ExOnljTcb=%p, phase3s=%ld, timestamp=%s)", this,
                        phase3s_, reportTimestamp());
#endif
          ...
          if ((isLeftJoin() || isAntiJoin()) && (pstate.outerMatched_ == ExConstants::EX_FALSE) &&
              request != ex_queue::GET_NOMORE && qleft_.up->getHeadEntry()->upState.status != ex_queue::Q_NO_DATA) {
            // check if we've got room in the up queue
            ...
            if (afterPred_) retCode = afterPred_->eval(uentry->getAtp(), 0);
          ...
          if (request != ex_queue::GET_NOMORE && rentry->getDiagsArea()) {
            ...
            if (onljTdb().isRowsetIterator() && onljTdb().isNonFatalErrorTolerated()) {
              ...
              if (pstate.accumDiags_) {
                if (!(pstate.accumDiags_->canAcceptMoreErrors())) {
                  ...
                  if (!rda->canAcceptMoreErrors()) rda->removeLastErrorCondition();
...

        {
          ...
          if (!onljTdb().isNonFatalErrorTolerated()) cancelParentRequest(pentry);

          break;

        }  // end of case ex_queue::Q_SQLERROR:

        default:
          ex_assert(0, "ExOnljTcb::work_phase3() Invalid state returned by right");
          break;
    ...
    QRLogger::log(CAT_SQL_EX_ONLJ, LL_DEBUG,
                  "ExOnljTcb::work_phase3: loop over rightQ over (this ExOnljTcb=%p, timest=%s, rows=%ld, phase3s=%d "
                  "times, rQempty=%d, pst.leftMatches=%d, timeSpent=%s)",
                  this, reportTimestamp(), rows2upq, phase3s_, qright_.up->isEmpty(), pstate.leftMatches_,
                  reportTimeDiff(phase3Timer_.elapsedTime()));
#endif
    ...
    if ((pentry->downState.request == ex_queue::GET_NEXT_N) ||
      ...
      if (!pstate.qGetDoneFromLeft_ && !pstate.qNoDataFromLeft_) return WORK_OK;
    ...
    while (pstate.leftOnlyRows_) {
      ...
      ex_assert(lentry->upState.parentIndex == phaseThree, "ex_queue::work3() left up queue out of sync");
      // if this was an error returned from the left child then
      // pass the error on to the parent (right request has already
      // been cancelled)
      if ((lentry->upState.status == ex_queue::Q_SQLERROR) || (lentry->upState.status == ex_queue::Q_REC_SKIPPED)) {
        if (onljTdb().isRowsetIterator() && onljTdb().isNonFatalErrorTolerated()) {
    ...
    if (pstate.leftStep_ == NLJ_LEFT_NOT_EMPTY) return WORK_OK;
...
ExWorkProcRetcode ExOnljTcb::workCancel() {
  ...
  for (queue_index ci = qparent_.down->getHeadIndex(); ci != phaseOne_; ci++) {
    ex_queue_entry *pentry = qparent_.down->getQueueEntry(ci);

    if (pentry->downState.request == ex_queue::GET_NOMORE) cancelParentRequest(pentry);
  }
...
NABoolean ExOnljTcb::processNonFatalErrorsInLeftUpQueue(NABoolean &project, NABoolean &consumed) {
...
//
ExOnljPrivateState::ExOnljPrivateState() { init(); }
