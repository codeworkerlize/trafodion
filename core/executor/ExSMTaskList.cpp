...
ExSMTaskList::ExSMTaskList()
    : buckets_(NULL), numBuckets_(EXSM_DEFAULT_NUM_BUCKETS), numTasks_(0), numInserts_(0), numCollisions_(0) {
  char *e = getenv("EXSM_NUM_BUCKETS");
  if (e && e[0]) {
    int temp = atoi(e);
    if (temp > 0) numBuckets_ = (uint32_t)temp;
  }

  buckets_ = new ExSMTask *[numBuckets_];
  memset(buckets_, 0, sizeof(ExSMTask *) * numBuckets_);
  EXSM_TRACE(EXSM_TRACE_INIT, "TASK LIST CTOR %p buckets %u", this, numBuckets_);
...
ExSMTaskList::~ExSMTaskList() {
  delete[] buckets_;
  EXSM_TRACE(EXSM_TRACE_INIT, "TASK LIST DTOR %p inserts %ld collisions %ld", this, numInserts_, numCollisions_);
...
ExSMTask *ExSMTaskList::findTask(const sm_target_t &target, bool doLock, bool doTrace) {
  if (doLock) lock();
  ...
  while (current) {
    ...
    if (doTrace)
      EXSM_TRACE(EXSM_TRACE_RDR_THR | EXSM_TRACE_WAIT,
                 ...
                 (char)ExSMTag_GetQualifierDisplay(target.tag), (int)receiver.node, (int)receiver.pid, receiver.id,
                 (int)ExSMTag_GetTagWithoutQualifier(receiver.tag), (char)ExSMTag_GetQualifierDisplay(receiver.tag));
