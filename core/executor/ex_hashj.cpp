...
const char *ex_hashj_tcb::HashJoinPhaseStr[] = {"PHASE_END", "PHASE_3", "PHASE_2", "PHASE_1"};
...
ex_tcb *ex_hashj_tdb::build(ex_globals *glob) {
  ...
  //
  if (isUniqueHashJoin()) {
    hashJoinTcb = new (glob->getSpace()) ExUniqueHashJoinTcb(*this, *leftChildTcb, *rightChildTcb, glob);
  } else {
    hashJoinTcb = new (glob->getSpace()) ex_hashj_tcb(*this, *leftChildTcb, *rightChildTcb, glob);
  }
...
ExOperStats *ex_hashj_tcb::doAllocateStatsEntry(CollHeap *heap, ComTdb *tdb) {
  ...
  if (statsType == ComTdb::OPERATOR_STATS)
    ...
    stat = (ExBMOStats *)new (heap) ExHashJoinStats(heap, this, tdb);
...
ex_hashj_tcb::ex_hashj_tcb(const ex_hashj_tdb &hashJoinTdb,
    ...
    : ex_tcb(hashJoinTdb, 1, glob),
      ...
      onlyReturnResultsWhenInnerMatched_(!hashJoinTdb.isLeftJoin() && !hashJoinTdb.isAntiSemiJoin()),
      isInnerEmpty_(TRUE),
      nullPool_(NULL),
      totalRowsEvaledByMinMax_(0),
#ifdef NEED_INSTRUMENT_HASHJ
      totalMinmaxEvalET_(0),
#endif
      sanityCheckCycle_(hashJoinTdb.rangeSpecSanityCheckCycle_) {
  bmoStats_ = NULL;
  hashJoinStats_ = NULL;
  ...
  rangeSpecPackingExpr_ = hashJoinTdb.rangeSpecPackingExpr_;
  ...
  if (isSemiJoin() && beforeJoinPred1_ == NULL && beforeJoinPred2_ == NULL && afterJoinPred1_ == NULL &&
      afterJoinPred2_ == NULL)
    doNotChainDup_ = TRUE;
  else if (isAntiSemiJoin() && beforeJoinPred1_ == NULL && beforeJoinPred2_ == NULL)
    doNotChainDup_ = TRUE;
  else
  ...
  // fixup expressions
  if (minMaxExpr_) (void)minMaxExpr_->fixup(0, getExpressionMode(), this, space_, heap_, FALSE, glob);
  if (rangeSpecPackingExpr_)
    (void)rangeSpecPackingExpr_->fixup(0, getExpressionMode(), this, space_, heap_, FALSE, glob);
  if (rightHashExpr_) (void)rightHashExpr_->fixup(0, getExpressionMode(), this, space_, heap_, FALSE, glob);
  if (rightMoveInExpr_) (void)rightMoveInExpr_->fixup(0, getExpressionMode(), this, space_, heap_, FALSE, glob);
  if (rightMoveOutExpr_) (void)rightMoveOutExpr_->fixup(0, getExpressionMode(), this, space_, heap_, FALSE, glob);
  if (rightSearchExpr_)
    (void)rightSearchExpr_->fixup(0, getExpressionMode(), this, space_, heap_, FALSE, glob);
  else {
  ...
  if (leftHashExpr_) (void)leftHashExpr_->fixup(0, getExpressionMode(), this, space_, heap_, FALSE, glob);
  if (leftMoveExpr_) (void)leftMoveExpr_->fixup(0, getExpressionMode(), this, space_, heap_, FALSE, glob);
  if (leftMoveInExpr_) (void)leftMoveInExpr_->fixup(0, getExpressionMode(), this, space_, heap_, FALSE, glob);
  if (leftMoveOutExpr_) (void)leftMoveOutExpr_->fixup(0, getExpressionMode(), this, space_, heap_, FALSE, glob);
  if (probeSearchExpr1_) (void)probeSearchExpr1_->fixup(0, getExpressionMode(), this, space_, heap_, FALSE, glob);
  if (probeSearchExpr2_) (void)probeSearchExpr2_->fixup(0, getExpressionMode(), this, space_, heap_, FALSE, glob);
  if (leftJoinExpr_) (void)leftJoinExpr_->fixup(0, getExpressionMode(), this, space_, heap_, FALSE, glob);
  if (nullInstForLeftJoinExpr_)
    (void)nullInstForLeftJoinExpr_->fixup(0, getExpressionMode(), this, space_, heap_, FALSE, glob);
  if (rightJoinExpr_) (void)rightJoinExpr_->fixup(0, getExpressionMode(), this, space_, heap_, FALSE, glob);
  if (nullInstForRightJoinExpr_)
    (void)nullInstForRightJoinExpr_->fixup(0, getExpressionMode(), this, space_, heap_, FALSE, glob);

  if (beforeJoinPred1_) (void)beforeJoinPred1_->fixup(0, getExpressionMode(), this, space_, heap_, FALSE, glob);
  if (beforeJoinPred2_) (void)beforeJoinPred2_->fixup(0, getExpressionMode(), this, space_, heap_, FALSE, glob);
  if (afterJoinPred1_) (void)afterJoinPred1_->fixup(0, getExpressionMode(), this, space_, heap_, FALSE, glob);
  if (afterJoinPred2_) (void)afterJoinPred2_->fixup(0, getExpressionMode(), this, space_, heap_, FALSE, glob);
  if (afterJoinPred3_) (void)afterJoinPred3_->fixup(0, getExpressionMode(), this, space_, heap_, FALSE, glob);
  if (afterJoinPred4_) (void)afterJoinPred4_->fixup(0, getExpressionMode(), this, space_, heap_, FALSE, glob);
  if (afterJoinPred5_) (void)afterJoinPred5_->fixup(0, getExpressionMode(), this, space_, heap_, FALSE, glob);

  if (isReuse()) {
    if (checkInputPred_) (void)checkInputPred_->fixup(0, getExpressionMode(), this, space_, heap_, FALSE, glob);
  ...
  if (checkInnerNullExpr_) (void)checkInnerNullExpr_->fixup(0, getExpressionMode(), this, space_, heap_, FALSE, glob);

  if (checkOuterNullExpr_) (void)checkOuterNullExpr_->fixup(0, getExpressionMode(), this, space_, heap_, FALSE, glob);
  ...
  workAtp_->getTupp(hashJoinTdb.extLeftRowAtpIndex_) = new (space_) tupp_descriptor;
  workAtp_->getTupp(hashJoinTdb.extRightRowAtpIndex1_) = new (space_) tupp_descriptor;
  workAtp_->getTupp(hashJoinTdb.extRightRowAtpIndex2_) = new (space_) tupp_descriptor;
  ...
  if (rightJoinExpr_ && nullInstForRightJoinExpr_ && (hashJoinTdb.instRowForRightJoinLength_ > 0)) {
...
ex_hashj_tcb::~ex_hashj_tcb() {
  ...
  if (workAtp_) deallocateAtp(workAtp_, space_);
...
void ex_hashj_tcb::releaseResultTupps() {
  if (hashJoinTdb().leftRowLength_) workAtp_->getTupp(hashJoinTdb().leftRowAtpIndex_).release();

  if (hashJoinTdb().rightRowLength_) workAtp_->getTupp(hashJoinTdb().rightRowAtpIndex_).release();

  if (isLeftJoin() && leftJoinExpr_) workAtp_->getTupp(hashJoinTdb().instRowForLeftJoinAtpIndex_).release();

  if (isRightJoin() && rightJoinExpr_) workAtp_->getTupp(hashJoinTdb().instRowForRightJoinAtpIndex_).release();
};
...
short ex_hashj_tcb::insertResult(HashRow *hashRow) {
  ...
  atp_struct *leftRowAtp = ((pstate.getPhase() == PHASE_2) ? leftQueue_.up->getHeadEntry()->getAtp() : workAtp_);
  ...
  if (hashJoinTdb().leftRowLength_) {
    if (pstate.getPhase() == PHASE_2) {
      SqlBufferHeader::moveStatus ms =
          resultPool_->moveIn(leftRowAtp, workAtp_, hashJoinTdb().leftRowAtpIndex_, hashJoinTdb().leftRowLength_,
                              leftMoveExpr_, true, hashJoinTdb().bufferSize_);
      ...
      char *rowPointer = leftRowAtp->getTupp(hashJoinTdb().extLeftRowAtpIndex_).getDataPointer();
      ...
      resultPool_->moveIn(workAtp_, hashJoinTdb().leftRowAtpIndex_, leftRowLength, rowPointer, true,
                          hashJoinTdb().bufferSize_);
    }
    // and assign the tupp to the parents queue entry
    atp->getTupp(hashJoinTdb().returnedLeftRowAtpIndex_) = workAtp_->getTupp(hashJoinTdb().leftRowAtpIndex_);
  ...
  if (isRightJoin() && rightJoinExpr_) {
    SqlBufferHeader::moveStatus ms =
        resultPool_->moveIn(downParentEntryAtp, workAtp_, hashJoinTdb().instRowForRightJoinAtpIndex_,
                            hashJoinTdb().instRowForRightJoinLength_, rightJoinExpr_, true, hashJoinTdb().bufferSize_);
  ...
  if (hashJoinTdb().rightRowLength_ && hashRow) {
    ...
    resultPool_->moveIn(workAtp_, hashJoinTdb().rightRowAtpIndex_, rightRowLength, hashRow->getData(), true,
                        hashJoinTdb().bufferSize_);

    atp->getTupp(hashJoinTdb().returnedRightRowAtpIndex_) = workAtp_->getTupp(hashJoinTdb().rightRowAtpIndex_);
  };
  ...
  if (isLeftJoin()) {
    if (hashRow) {
      ...
      if (leftJoinExpr_) {
        SqlBufferHeader::moveStatus ms = resultPool_->moveIn(
            downParentEntryAtp, workAtp_, hashJoinTdb().instRowForLeftJoinAtpIndex_,
            hashJoinTdb().instRowForLeftJoinLength_, leftJoinExpr_, true, hashJoinTdb().bufferSize_);
  ...
  if (pstate.accumDiags_) {
    ComDiagsArea *accumulatedDiagsArea = upParentEntry->getDiagsArea();
  ...
  if (bmoStats_) bmoStats_->setSpaceBufferCount(resultPool_->get_number_of_buffers());
  return 0;
};
...
short ex_hashj_tcb::processError(atp_struct *atp) {
  ex_assert(!parentQueue_.down->isEmpty(), "ex_hashj_tcb::processError() Unexpected empty parent down queue");

  ex_queue_entry *pEntryDown = parentQueue_.down->getHeadEntry();
  ex_queue_entry *upEntry = parentQueue_.up->getTailEntry();
  ex_hashj_private_state &pstate = *((ex_hashj_private_state *)pEntryDown->pstate);
  ...
  while (TRUE) {
    ...
    switch (pstate.getState()) {
      ...
      case HASHJ_CANCEL_AFTER_INNER: {
        ...
        if (rightQueue_.up->isEmpty() || parentQueue_.up->isFull()) {
          return WORK_OK;
        };

        if (workReadInner()) return WORK_OK;
      } break;
      case HASHJ_END_PHASE_1: {
        workEndPhase1();
        if (pstate.getState() == HASHJ_END_PHASE_1) return WORK_CALL_AGAIN;  // more work, but let others run too
      } break;
      case HASHJ_READ_OUTER: {
        ...
        if (numJoinedRowsRejected_ > xProductPreemptMax) return WORK_HASHJ_PREEMPT;

        if (workReadOuter()) return WORK_OK;
      } break;
      ...
      case HASHJ_RETURN_RIGHT_ROWS: {
        ...
        if (workReturnRightRows()) return WORK_OK;
      } break;
      ...
      case HASHJ_CANCELED: {
        ...
        NABoolean leftDone = pstate.delayedLeftRequest() ? TRUE : consumeForCancel(leftQueue_);

        // only check the right queue if this request built its own hash table
        // (not reusing)
        NABoolean rightDone = pstate.usingPreviousHT() ? TRUE : consumeForCancel(rightQueue_);
      ...
      case HASHJ_ERROR: {
        ...
        if (!da || !da->contains((int)-rc_)) {
          ...
          if (rc_ == EXE_SORT_ERROR) {
            ...
            str_sprintf(msg,
                        "Hash Join Scratch IO Error occurred. Scratch Error: %d, System Error: %d, System Error "
                        "Detail: %d, Details: %s",
...
ExWorkProcRetcode ex_hashj_tcb::workDown() {
  ...
  while (nextRequest_ != queue_tail) {
    ...
    ex_queue_entry *downParentEntry = parentQueue_.down->getQueueEntry(nextRequest_);
    ex_hashj_private_state &pstate = *((ex_hashj_private_state *)downParentEntry->pstate);
    ...
    switch (downParentEntry->downState.request) {
      ...
      case ex_queue::GET_ALL:
        ...
        if (isReuse()) {
          ...
          if (will_reuse_old_HT = isSameInputAgain(downParentEntry)) {
            ...
            if (moveInputExpr_) {
              ...
              if (moveInputExpr_->eval(downParentEntry->getAtp(), workAtp_) != ex_expr::EXPR_OK) {
        ...
        if (!will_reuse_old_HT) {
          ...
          rightEntry->downState.requestValue = downParentEntry->downState.requestValue;
          ...
          if (nextRequest_ == parentQueue_.down->getHeadIndex()) {
            if (hashJoinTdb().isUniqueHashJoin()) {
              ...
              if (uHashJoinTcb->allocateBufferHeap()) pstate.setState(HASHJ_ERROR);
            } else {
...
NABoolean ex_hashj_tcb::allocateClusters() {
  ...
  if (rightSearchExpr_ == NULL) {
    bucketsPerCluster = 1;
    noOfClusters = 1;
  } else if (hashJoinTdb().isUniqueHashJoin()) {
    ...
    if (totalInnerBuffers % totalBuffers) noOfClusters++;
  ...
  ClusterDB::HashOperator hashOperator =
      rightSearchExpr_ == NULL
          ? ClusterDB::CROSS_PRODUCT
          : hashJoinTdb().isUniqueHashJoin()
                ? ClusterDB::UNIQUE_HASH_JOIN
                : hashJoinTdb().isNoOverflow() ? ClusterDB::ORDERED_HASH_JOIN : ClusterDB::HYBRID_HASH_JOIN;

  clusterDb_ = new (heap_)
      ClusterDB(hashOperator, hashJoinTdb().hashBufferSize_, workAtp_, tdb.getExplainNodeId(),
                hashJoinTdb().extRightRowAtpIndex1_, hashJoinTdb().extRightRowAtpIndex2_, rightSearchExpr_, buckets_,
                bucketCount_, availableMemory, hashJoinTdb().pressureThreshold_, getGlobals()->castToExExeStmtGlobals(),
                &rc_, hashJoinTdb().isNoOverflow(), FALSE, /*isPartialGroupBy*/
                hashJoinTdb().minBuffersToFlush_, hashJoinTdb().numInBatch_,

                hashJoinTdb().forceOverflowEvery(), hashJoinTdb().forceHashLoopAfterNumBuffers(),
                ...
                hashJoinTdb().logDiagnostics(), hashJoinTdb().bufferedWrites(), hashJoinTdb().disableCmpHintsOverflow(),
                hashJoinTdb().memoryQuotaMB(), minMemQuotaMB,
                minB4Chk,  // BmoMinMemBeforePressureCheck
                hashJoinTdb().getBmoCitizenshipFactor(), hashJoinTdb().getMemoryContingencyMB(),
  ...
  if (!clusterDb_ || rc_ != EXE_OK) {
    if (!clusterDb_)
      rc_ = EXE_NO_MEM_TO_EXEC;  // allocate ClusterDB failed
    else
      delete clusterDb_;
    return (TRUE);
  };
  ...
  for (i = 0; i < noOfClusters; i++) {
    cluster = new (heap_) Cluster(Cluster::IN_MEMORY, clusterDb_, &buckets_[bucketIdx], bucketsPerCluster,
                                  hashJoinTdb().extRightRowLength_, hashJoinTdb().useVariableLength(),
                                  hashJoinTdb().considerBufferDefrag(), hashJoinTdb().extRightRowAtpIndex1_, TRUE,
                                  FALSE,  // for now only left joins
                                  cluster, &rc_);

    if (!cluster || rc_ != EXE_OK) {
      if (!cluster)
        rc_ = EXE_NO_MEM_TO_EXEC;  // could not allocate Cluster
      else
        delete cluster;
      return (TRUE);
    };
...
NABoolean ex_hashj_tcb::isFirst(queue_index ix, ex_hashj_private_state &pstate) {
  return ((pstate.reliesOnIdx == ix) && (parentQueue_.down->entryExists(ix + 1)) &&
          (((ex_hashj_private_state *)parentQueue_.down->getQueueEntry(ix + 1)->pstate)->reliesOnIdx == ix));
}
...
NABoolean ex_hashj_tcb::isLast(queue_index ix, ex_hashj_private_state &pstate) {
  return ((!parentQueue_.down->entryExists(ix + 1)) ||
          (((ex_hashj_private_state *)parentQueue_.down->getQueueEntry(ix + 1)->pstate)->reliesOnIdx !=
           pstate.reliesOnIdx));
}
...
//
//////////////////////////////////////////////////////////////
void ex_hashj_tcb::propagateCancel(queue_index ix, ex_hashj_private_state &pstate) {
...
ExWorkProcRetcode ex_hashj_tcb::workCancel() {
  ...
  while (ix != nextRequest_) {
    ...
    if (request == ex_queue::GET_NOMORE) {
      ...
      if (pstate.state_ != HASHJ_CANCELED && pstate.state_ != HASHJ_DONE) propagateCancel(ix, pstate);
    }
    ix++;
  }
  return WORK_OK;
...
short ex_hashj_tcb::workReadInner() {
  ...
  switch (rightEntry->upState.status) {
    case ex_queue::Q_OK_MMORE: {
      ...
      if (pstate.getOldState() != HASHJ_FLUSH_CLUSTER) {
        ...
        if (!rightHashExpr_)
          ...
          if (minMaxExpr() && minMaxExpr()->perrecExpr()) {
#ifdef NEED_INSTRUMENT_HASHJ
            workTimer_.start();
#endif
            ...
            totalRowsEvaledByMinMax_++;

            if (sanityCheckCycle_ > 0 && totalRowsEvaledByMinMax_ % sanityCheckCycle_ == 0) {
              ExFunctionRangeOfValues::conditionalSanityCheck(minMaxExpr()->perrecExpr()->getClauses(),
                                                              OperatorTypeEnum::ITM_RANGE_VALUES_INSERT);
            }
          ...
          // is not a NULL value
          if (checkInnerNullExpr_ && hashValue_ == ExHDPHash::nullHashValue /*666654765*/) {
      ...
      if (buckets_[bucketId].insert(rightEntry->getAtp(), rightMoveInExpr_, hashValue_, &rc_,
    ...
    case ex_queue::Q_INVALID: {
      ex_assert(0, "ex_hashj_tcb::workReadInner() Invalid state returned by right child");
    } break;
  };
  return 0;
};
...
void ex_hashj_tcb::workEndPhase1() {
  ...
  // If so, the left request was delayed until here.
    ...
    toLeftChildAtp_->dumpMinMaxTupp(buf, -1 /* show all tupps */);
#endif

    ScanFilterStatsList *filterStats = (bmoStats_) ? bmoStats_->getScanFilterStats()
                                                   : ((hashJoinStats_) ? hashJoinStats_->getScanFilterStats() : NULL);

    // Go over each range of values possibly constructed during the
    // eval of minMaxexpr_  and pack it into a byte string.
    // Note that in the above
    // code, only the min/max tupp is copied to the down
    // queue, the data that is pointered by the tupp
    // is still accessable from workAtp_ and manipulated
    ex_expr::exp_return_type retCode = ExFunctionRangeOfValues::conditionalPack(
        minMaxExpr()->perrecExpr()->getClauses(), workAtp_, OperatorTypeEnum::ITM_RANGE_VALUES_INSERT,
        "HASHJ: Pack RS with inserted values (B)", TRUE, filterStats, ScanFilterStats::COPY);

#ifdef NEED_INSTRUMENT_HASHJ
    QRLogger::log(CAT_SQL_EXE_MINMAX, LL_DEBUG,
                  "conditionalPack() done:  total minmax eval time: timestamp(int64)=%ld, workTime=%s, rows=%ld",
                  totalMinmaxEvalET_, reportTimeDiff(totalMinmaxEvalET_), totalRowsEvaledByMinMax_);
#endif
  ...
  while (currCluster_) {
    switch (currCluster_->getState()) {
      case Cluster::FLUSHED: {
        ...
        currCluster_->setUpOuter(hashJoinTdb().extLeftRowLength_, hashJoinTdb().extLeftRowAtpIndex_,
      ...
      case Cluster::IN_MEMORY: {
        if (currCluster_->getRowCount()) {  // chain the cluster only if has rows
          ...
          if (currCluster_->getRowCount() > 10000 && currCluster_->getNext())  // skip breaking for the last cluster
          {
  ...
  if (0 == totalRightRowsRead_ &&  // was the inner table empty ?
      onlyReturnResultsWhenInnerMatched_ && (!leftSideIUD())) {
  ...
  if (clusterDb_->sawPressure()) {  // only if an overflow happened
    ...
    for (numClusters = 0, numFlushed = 0, totalSize = 0, maxSize = 0, minSize = UINT_MAX,
        currCl = clusterDb_->getClusterList();
         currCl; currCl = currCl->getNext(), ++numClusters)
      if (currCl->isInner()) {
  ...
  if (hashJoinTdb().logDiagnostics()) {
    ...
    if (clusterDb_->memoryQuotaMB() > hashJoinTdb().memoryQuotaMB()) {
      char msg[512];
      str_sprintf(msg, "HJ End Phase 1 (%d). GRABBED additional quota %d MB", 0,
    ...
    if (clusterDb_->sawPressure()) {  // only if an overflow happened
      ...
      str_sprintf(msg,
                  "HJ End Phase 1 (%d). #inner rows: %ld, #buckets: %d, #clusters: %d, #flushed: %d, total size %d MB, "
                  "cluster size max- %d MB min- %d, variable size records: %s",
                  0, totalRightRowsRead_, bucketCount_, numClusters, numFlushed, totalSize, maxSize, minSize,
  ...
  if (hashJoinStats_) hashJoinStats_->incPhase();
...
short ex_hashj_tcb::workReadOuter() {
  ...
  switch (leftEntry->upState.status) {
    case ex_queue::Q_OK_MMORE: {
      if (pstate.getOldState() != HASHJ_FLUSH_CLUSTER) {
        ...
        if (!leftHashExpr_)
          ...
          if (!isInnerEmpty_ && checkOuterNullExpr_ && hashValue_ == ExHDPHash::nullHashValue /*666654765*/) {
      ...
      if (oCluster) {
        ...
        // phase 3 of the join
        if (oCluster->insert(leftEntry->getAtp(), leftMoveInExpr_, hashValue_, &rc_)) {
          leftQueue_.up->removeHead();
        } else {
          if (rc_) {
            // the insert caused an error
        ...
        if (buckets_[bucketId].getRowCount() || !onlyReturnResultsWhenInnerMatched_) {
          ...
          if (ht) {
            ...
            if (beforeMatch) {
              if (rightSearchExpr_ == NULL) {
                ...
                ht->position(&cursor_, leftEntry->getAtp(), workAtp_, hashJoinTdb().extRightRowAtpIndex1_,
                             probeSearchExpr1_, hashValue_, doNotChainDup_, hashJoinTdb().isReturnRightOrdered());
              }
            }
    ...
    case ex_queue::Q_INVALID: {
      ex_assert(0, "ex_hashj_tcb::workReadOuter() Invalid state returned by left child");
    } break;
  };
  return 0;
};
...
void ex_hashj_tcb::workEndPhase2() {
  ...
  if (isReuse()) {
    ...
    for (; !pstate.usingPreviousHT() && iCluster; iCluster = iCluster->getNext())
      if (hashJoinStats_) hashJoinStats_->addClusterStats(iCluster->getStats(hashJoinStats_->getHeap()));
  ...
  while (iCluster) {
    ...
    if (oCluster && (oCluster->getRowCount() || isRightJoin())) {
      ...
      if (oCluster->getFirstBuffer() && oCluster->getRowsInBuffer()) {
      ...
      if (hashJoinStats_) hashJoinStats_->addClusterStats(originalInnerCluster->getStats(hashJoinStats_->getHeap()));
    ...
    if (oCluster && !oCluster->getRowCount()) {
      // before we release the cluster, we collect some stats about it
      if (hashJoinStats_) hashJoinStats_->addClusterStats(oCluster->getStats(hashJoinStats_->getHeap()));
      delete oCluster;
  ...
  if (hashJoinTdb().logDiagnostics() /* && clusterDb_->sawPressure() */) {
    ...
    str_sprintf(
        ...
        0, totalSize, maxSize, minSize, hashJoinTdb().useVariableLength() ? "y" : "n");
    // log an EMS event and continue
    SQLMXLoggingArea::logExecRtInfo(NULL, 0, msg, tdb.getExplainNodeId());
  }
...
void ex_hashj_tcb::prepareForNextPairOfClusters(Cluster *iCluster) {
...
#else
    if (iCluster->numLoops()) {
      str_sprintf(msg, "HJ Finished cluster (%d) in Phase 3 (%d) with %d Hash-Loop iterations.", 0, 0,
                  iCluster->numLoops() + 1);  // add uncounted last iter
      // log an EMS event and continue
      SQLMXLoggingArea::logExecRtInfo(NULL, 0, msg, tdb.getExplainNodeId());
    }
  ...
  if (hashJoinStats_) {
    hashJoinStats_->addClusterStats(iCluster->getStats(hashJoinStats_->getHeap()));
    if (oCluster) hashJoinStats_->addClusterStats(oCluster->getStats(hashJoinStats_->getHeap()));
  };
  delete iCluster;
...
short ex_hashj_tcb::workProbe() {
  ...
  atp_struct *inputAndPhase2LeftRowAtp = phase2 ? leftQueue_.up->getHeadEntry()->getAtp() : downParentEntry->getAtp();
  ...
  for (HashRow *hashRow = hashTable ? hashTable->getNext(&cursor_) : NULL; hashRow;  // found a match ?
    ...
    workAtp_->getTupp(hashJoinTdb().extRightRowAtpIndex1_).setDataPointer(hashRow->getData());
    ...
    if (isRightJoin()) hashRow->setBit(TRUE);
  ...
  if (outerMatchedInner_ || onlyReturnResultsWhenInnerMatched_) return cleanupLeftRow();
  ...
  if (afterJoinPred) {
    ...
    switch (afterJoinPred->eval(inputAndPhase2LeftRowAtp, workAtp_)) {
      ...
      default:  // failed : on to the next left row
        if (leftJoinExpr_) workAtp_->getTupp(hashJoinTdb().instRowForLeftJoinAtpIndex_).release();
...
short ex_hashj_tcb::workReturnRightRows() {
  ...
  if (clusterDb_->getClusterReturnRightRows())  // either PHASE_3 or returning
    ...
    ex_assert(iCluster, "ex_hashj_tcb::workReturnRightRows No cluster to return rows");
  }
  ...
  while (iCluster) {
    if (iCluster->getRowCount() && iCluster->isInner() && (iCluster->getState() != Cluster::FLUSHED)) {
      ...
      ex_assert(hashTable, "ex_hashj_tcb::workReturnRightRows - hashtable must exist");
      ...
      for (HashRow *hashRow = hashTable->getNext(&cursor_); hashRow;  // all rows in the inner (right) table
           hashRow = hashTable->getNext(&cursor_))                    // get next HT match
      {
        ...
        if (buckets_[bucketId].getInnerCluster() == iCluster && !hashRow->bitSet())  // if row was unmatched
        {
          // first, we have to copy the input to this node
          upParentEntry->copyAtp(downParentEntryAtp);
          ...
          if (afterJoinPred5_) {
            ...
            ex_expr::exp_return_type retCode;
            retCode = afterJoinPred5_->eval(downParentEntryAtp, workAtp_);
            switch (retCode) {
              case ex_expr::EXPR_ERROR:
...
void ex_hashj_tcb::workReadOuterCluster() {
  ...
  while (row) {
    if (isSemiJoin() && oCluster->hasBitMap() && oCluster->testBitMap()) {
    ...
    // The expression does not expect the HashRow to be part of the row.
    // Adjust the datapointer in the work atp to point beyond the HashRow.
    workAtp_->getTupp(hashJoinTdb().extLeftRowAtpIndex_).setDataPointer(row->getData());
    beforeMatch = TRUE;
    ...
    if (beforeMatch) {
      ...
      if (rightSearchExpr_ == NULL) {
        ...
        ht->position(&cursor_, workAtp_, workAtp_, hashJoinTdb().extRightRowAtpIndex1_, probeSearchExpr2_,
                     row->hashValue(), doNotChainDup_);
      }
    }
...
void ex_hashj_tcb::workDone() {
  ...
  if (!leftQueue_.up->isEmpty() && leftQueue_.up->getHeadEntry()->upState.parentIndex == indexOfParent)
    leftQueue_.up->removeHead();

  if (!pstate.usingPreviousHT()) {  // so we may have rows in the right queue
    if (!rightQueue_.up->isEmpty() && rightQueue_.up->getHeadEntry()->upState.parentIndex == indexOfParent)
      rightQueue_.up->removeHead();
  }
  ...
  ex_assert(nextRequest_ != indexOfParent, "ex_hashj_tcb::workDone() expected nextRequest++ already");

  parentQueue_.down->removeHead();

  if (!hashJoinTdb().isUniqueHashJoin()) releaseResultTupps();

  if (hashJoinTdb().logDiagnostics() && !hashJoinTdb().isUniqueHashJoin()
...
ex_hashj_private_state::ex_hashj_private_state() { initState(); }
...
//
ExUniqueHashJoinTcb::ExUniqueHashJoinTcb(const ex_hashj_tdb &hashJoinTdb, const ex_tcb &leftChildTcb,
                                         const ex_tcb &rightChildTcb, ex_globals *glob)
...
ExUniqueHashJoinTcb::~ExUniqueHashJoinTcb() { freeResources(); }
  ...
  while (TRUE) {
    ...
    switch (pstate.getState()) {
      ...
      case HASHJ_CANCEL_AFTER_INNER: {
        ...
        //
        if (availRows_ == 0) {
          if (hashJoinTdb().considerBufferDefrag() && bufferPool_ && (defragLength = computeDefragLength()) > 0) {
            ...
            if (clusterDb_->enoughMemory(bufferSize_))  // got mem ?
              newBuffer = new (bufferHeap_, FALSE) HashBuffer(
                  bufferSize_, extRowSize_, hashJoinTdb().useVariableLength(), bufferHeap_, clusterDb_, &rc_);
            ...
            if (!newBuffer || rc_) {
              ...
              if (hashJoinStats_) {
                hashJoinStats_->updMemorySize(bufferHeap_->getAllocSize());
              }
            }
          }
        }
        ...
        //
        if (workReadInner(defragLength)) return WORK_OK;
      ...
      case HASHJ_END_PHASE_1: {
        ...
        if (minMaxExpr_ && pstate.delayedLeftRequest()) {
          ...
          ScanFilterStatsList *filterStats = (bmoStats_)
                                                 ? bmoStats_->getScanFilterStats()
                                                 : ((hashJoinStats_) ? hashJoinStats_->getScanFilterStats() : NULL);

          // Go over each range of values possibly constructed
          // during the eval of minMaxexpr_  and pack it into
          // a byte string in workAtp_. Note that in the above
          // code, only the min/max tupp is copied to the down
          // queue, the data that is pointered by the tupp
          // is still accessable from workAtp_ and manipulated
          // here.
          ex_expr::exp_return_type retCode = ExFunctionRangeOfValues::conditionalPack(
              minMaxExpr()->perrecExpr()->getClauses(), workAtp_, OperatorTypeEnum::ITM_RANGE_VALUES_INSERT,
              "Pack RS with inserted values (A)", TRUE, filterStats, ScanFilterStats::COPY);
...
short ExUniqueHashJoinTcb::workReadInner(UInt32 defragLength) {
  ...
  while (!upQueue->isEmpty() && (availRows > 0 || (defragLength > 0 && (defragLength = computeDefragLength()) > 0))) {
    ...
    //
    if (rightEntry->upState.status != ex_queue::Q_OK_MMORE) break;
    ...
    //
    retCode = rightHashExpr_->eval(rightEntry->getAtp(), workAtp_);
    if (retCode == ex_expr::EXPR_ERROR) break;
    ...
    if (rightMoveInExpr_) {
      if (defragLength == 0) {
        ...
        str_cpy_all(dataPointer->getData(), resultPool_->getDefragTd()->getTupleAddress(), defragLength);
...
#if (defined(_DEBUG))
        char txt[] = "hashjU";
        sql_buffer_pool::logDefragInfo(txt, bufferPool_->getMaxRowLength(), ROUND4(defragLength) + sizeof(HashRow),
                                       bufferPool_->getFreeSpace(), bufferPool_, bufferPool_->getRowCount());
#endif
      }
    }
...
NABoolean ExUniqueHashJoinTcb::workChainInner() {
  if (!hashTable_) {
    ...
    if (clusterDb_->enoughMemory(hashEntries * sizeof(HashTableHeader)))
      hashTable_ = new (bufferHeap_, FALSE) HashTable(hashEntries, FALSE, INT_MAX);
...
short ExUniqueHashJoinTcb::workReadOuter() {
  ...
  while (!leftQueue_.up->isEmpty() && !parentQueue_.up->isFull()) {
    ...
    // We will handle it outside the loop.
    //
    if (leftEntry->upState.status != ex_queue::Q_OK_MMORE) break;
    ...
    //
    HashRow *dataPointer;
    retCode = hashTable_->positionUniq(&dataPointer, leftRowAtp, workAtp_, atpIndex, probeSearchExpr1_, hashValue_);
    if (retCode == ex_expr::EXPR_TRUE) {
      ...
      if (rc != WORK_OK) return ((rc == WORK_CALL_AGAIN) ? WORK_OK : rc);
  ...
  if (!leftQueue_.up->isEmpty() && !parentQueue_.up->isFull()) {
    switch (leftEntry->upState.status) {
      ...
      default: {  // Do not expect a Q_INVALID or Q_OK_MMORE
        ex_assert(0,
...
///////////////////////////////////////////////////////////////////////////
short ExUniqueHashJoinTcb::workReturnRow(HashRow *dataPointer, atp_struct *leftRowAtp) {
  ex_queue_entry *upParentEntry = parentQueue_.up->getTailEntry();
  ex_queue_entry *downParentEntry = parentQueue_.down->getHeadEntry();
  ex_hashj_private_state &pstate = *((ex_hashj_private_state *)downParentEntry->pstate);
  ...
  if (isRightOutputNeeded_) {
    ...
    //
    tupp &rightTupp = upParentEntry->getAtp()->getTupp(returnedRightRowAtpIndex_);
    ...
    //
    str_cpy_all(rightTupp.getDataPointer(), dataPointer->getData(), size);
  }
...
UInt32 ExUniqueHashJoinTcb::computeDefragLength() {
  ...
  if (!hashJoinTdb().considerBufferDefrag() || rightQueue_.up->isEmpty()) {
    return 0;
  }
  ...
  UInt32 *rowLenPtr = &rowLen;
  if (rightMoveInExpr_) {
    ex_expr::exp_return_type retCode = rightMoveInExpr_->eval(rightEntry->getAtp(), workAtp_, 0, -1, rowLenPtr);
    if (retCode != ex_expr::EXPR_ERROR && rowLen != 0 &&
