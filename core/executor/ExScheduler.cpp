...
int ExScheduler::printALiner(int lineno, char *buf) {
  ...
  if (!st)  // not used entry
    ...
    rv =
        sprintf(buf, "%.4d  %.4s.%.2s(%8p) %6d  " PF64 "\n", lineno,
                (st->getTcb() ? st->getTcb()->getEyeCatcher() : "????"), (st->getTaskName() ? st->getTaskName() : "??"),
                st, subtaskTrace_[lineno].lastWorkRetcode_, subtaskTrace_[lineno].rmsTimeConsumed_);
  return rv;
}

void ExScheduler::scheduleAllTasks() {
...
ExWorkProcRetcode ExScheduler::work(long prevWaitTime) {
  ...
  if (costTh == -2) {
    char *costThreshold = getenv("RECORD_TIME_COST_EXE_SNIPPET");
  ...
  if (fragRootOperStats != NULL) {
    ...
    if (processStats != NULL) processStats->setRecentQid(fragRootOperStats->getQueryId());
  }
  ...
  while (subtask) {
    ...
    if (subtask->isScheduled()) {
      ...
      if (statsArea) doNotUpdateCounter = statsArea->donotUpdateCounters();
      if ((timer) && (NOT doNotUpdateCounter)) timer->start();
      ...
      if (costTh >= 0) {
        ...
        taskCost = taskCost / 1000;
        taskCost = (JULIANTIMESTAMP() / 1000) - taskCost;
        if (taskCost >= costTh) {
          QRWARN("ExScheduler subtask %d TN %s TC %d STC %d PID %d txID %ld TC %ld",
                 (subtcb ? subtcb->getNodeType() : -2), (subtcb ? typeid(*subtcb).name() : "null"), taskCount,
                 sTaskCount, pid, (GetCliGlobals() ? GetCliGlobals()->getTransactionId() : -1), taskCost);
        }
      }
      ...
      if (maxCpuTime_ != LLONG_MAX) {
        subtaskTrace_[lastCalledIdx_].dp2QidCummulative_ = subtaskCpuTime.getProcessBusyTime();
        subtaskTrace_[lastCalledIdx_].dp2TimeConsumed_ =
            subtaskTrace_[lastCalledIdx_].dp2QidCummulative_ - startQidCpuTime;
      }
#endif
      ...
      if (stats && (stats->getCollectStatsType() == ComTdb::ALL_STATS)) {
        ...
        if (stats->getDownQueueSize() || stats->getUpQueueSize()) {
          ex_queue_pair pq = subtask->tcb_->getParentQueue();
          if (pq.down != NULL) {
            stats->getDownQueueStats().addEntry((double)pq.down->getLength());
            stats->setDownQueueSize(pq.down->getSize());
          }
          if (pq.up != NULL) {
            stats->getUpQueueStats().addEntry((double)pq.up->getLength());
      ...
      if (rc != WORK_OK) {
        ...
        if (rc == WORK_POOL_BLOCKED) {
          ...
          if (rc != WORK_CALL_AGAIN) {
            ...
            // completion here.
            ExMasterStmtGlobals *masterGlob = glob_->castToExExeStmtGlobals()->castToExMasterStmtGlobals();

            if ((masterGlob) && (masterGlob->getRtFragTable()->getState() == ExRtFragTable::NO_ESPS_USED)) {
              glob_->castToExExeStmtGlobals()->getIpcEnvironment()->getAllConnections()->waitOnAll(IpcImmediately);
            }
            ...
            if (costTh >= 0) {
              ...
              if (timeCost >= costTh)
                QRWARN("ExScheduler total TC %d STC %d PID %d txID %ld TC %ld", taskCount, sTaskCount, pid,
                       (GetCliGlobals() ? GetCliGlobals()->getTransactionId() : -1), timeCost);
            }
    ...
    if (subtask == NULL && !suspended_) {
      if (listHadWork) {
        ...
        if (glob_->castToExExeStmtGlobals()->castToExEspStmtGlobals() != NULL) {
          if (!glob_->castToExExeStmtGlobals()->castToExEspStmtGlobals()->multiThreadedEsp())
            glob_->castToExExeStmtGlobals()->getIpcEnvironment()->getAllConnections()->waitOnAll(IpcImmediately);
        } else
          glob_->castToExExeStmtGlobals()->getIpcEnvironment()->getAllConnections()->waitOnAll(IpcImmediately);
      ...
      if (rootStats) {
        if ((subtaskLoopCnt_ == maxSubtaskLoops_) || (rc == WORK_HASHJ_PREEMPT)) {
...
void ExScheduler::registerInsertSubtask(ExWorkProcPtr subtask, ex_tcb *tcb, ex_queue *queue, const char *taskName) {
  ExSubtask *st = addOrFindSubtask(subtask, tcb, taskName);
  queue->setInsertSubtask(st);
}

void ExScheduler::registerUnblockSubtask(ExWorkProcPtr subtask, ex_tcb *tcb, ex_queue *queue, const char *taskName) {
  ExSubtask *st = addOrFindSubtask(subtask, tcb, taskName);
  queue->setUnblockSubtask(st);
}

void ExScheduler::registerCancelSubtask(ExWorkProcPtr subtask, ex_tcb *tcb, ex_queue *queue, const char *taskName) {
  ExSubtask *st = addOrFindSubtask(subtask, tcb, taskName);
  queue->setCancelSubtask(st);
}

// BertBert VV
void ExScheduler::registerNextSubtask(ExWorkProcPtr subtask, ex_tcb *tcb, ex_queue *queue, const char *taskName) {
  ExSubtask *st = addOrFindSubtask(subtask, tcb, taskName);
  queue->setNextSubtask(st);
}
// BertBert ^^

void ExScheduler::registerResizeSubtask(ExWorkProcPtr subtask, ex_tcb *tcb, ex_queue *queue, const char *taskName) {
  ExSubtask *st = addOrFindExceptionSubtask(subtask, tcb, taskName, glob_);
  queue->setResizeSubtask(st);
}

ExSubtask *ExScheduler::registerNonQueueSubtask(ExWorkProcPtr subtask, ex_tcb *tcb, const char *taskName) {
...
void ExScheduler::startGui() {
  ...
  if (!pExpFuncs_ && getenv("DISPLAY")) {
    ...
    if (dlptr != NULL) {
      ...
      if (GetExportedFunctions) pExpFuncs_ = GetExportedFunctions();
      if (pExpFuncs_ == NULL) dlclose(dlptr);
    } else  // dlopen() failed
    {
      char *msg = dlerror();
    }
...
void ExScheduler::stopGui() {
  msGui_ = FALSE;
  if (pExpFuncs_) pExpFuncs_->fpDisplayExecution(NULL, this);
}

void ExScheduler::getProcInfoForGui(int &frag, int &inst, int &numInst, int &nid, int &pid, char *procNameBuf,
                                    int procNameBufLen) {
...
ExSubtask *ExScheduler::addOrFindSubtask(ExWorkProcPtr workProc, ex_tcb *tcb, const char *taskName) {
  // Loop through existing subtasks to see whether this one
  // has already been added
  ExSubtask *result = subtasks_;

  while (result && (!(result->matches(workProc, tcb)))) result = result->getNext();
...
ExExceptionSubtask *ExScheduler::addOrFindExceptionSubtask(ExWorkProcPtr workProc, ex_tcb *tcb, const char *taskName,
                                                           ex_globals *glob) {
...
NABoolean ExScheduler::checkSuspendAndLimit() {
  ...
  if (rootStats_) {
    ...
    if ((measRootStats = rootStats_->castToExMeasStats()) != NULL) isFragSuspended = measRootStats->isFragSuspended();
    ...
    if (maxCpuTime_ == LLONG_MAX) {
      ...
      if (fragRootStats) localCpuTime = fragRootStats->getLocalCpuTime();
      if (measRootStats) localCpuTime = measRootStats->getLocalCpuTime();
...
ExOperStats *ExScheduler::getRootStats() { return (glob_->statsEnabled() ? rootStats_ : NULL); }

void ExScheduler::setRootTcb(ex_tcb *tcb) { localRootTcb_ = tcb; }
...
const char *ExWorkProcRetcodeToString(ExWorkProcRetcode r) {
  switch (r) {
    ...
    case WORK_NOBODY_WORKED:
      return "WORK_NOBODY_WORKED";
    case WORK_STATS_PREEMPT:
      return "WORK_STATS_PREEMPT";
...
void ExExceptionSubtask::addOrFindSubtaskEntry(ExWorkProcPtr workProc, ex_tcb *tcb, const char *taskName,
                                               ex_globals *glob) {
  ExExceptionSubtaskEntry *result = entryList_;

  while (result && (!(result->matches(workProc, tcb)))) {
    result = result->next_;
  }

  if (!result) {
    ExExceptionSubtaskEntry *e = new (glob->getSpace()) ExExceptionSubtaskEntry(workProc, tcb, taskName);
