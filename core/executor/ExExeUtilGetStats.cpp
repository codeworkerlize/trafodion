  ...
  switch (getStatsReqType()) {
    case SQLCLI_STATS_REQ_STMT:
      if (getStmtName() == NULL)
        ...
        if (compilerStats() || executorStats() || otherStats() || detailedStats() || oldFormat() || shortFormat() ||
            tokenizedFormat())
          exe_util_tcb = new (glob->getSpace()) ExExeUtilGetStatisticsTcb(*this, glob);
        else
...
///////////////////////////////////////////////////////////////
ExExeUtilGetStatisticsTcb::ExExeUtilGetStatisticsTcb(const ComTdbExeUtilGetStatistics &exe_util_tdb, ex_globals *glob)
                           ...
                           char *sstrbuf)      // OUT
{
  ...
  while (currPos < maxLen && str[currPos] != space) {
    ...
    if (str[currPos] == quote) {
      currPos++;
      // find end quote
      while (currPos < maxLen && str[currPos] != quote) currPos++;
      if (currPos < maxLen) currPos++;
    } else
      currPos++;
  }
...
//
void ExExeUtilGetStatisticsTcb::moveCompilationStatsToUpQueue(CompilationStatsData *cmpStats) {
  if (NULL != cmpStats) {
    char startTime[70], endTime[70];
    short timestamp[8];

    INTERPRETTIMESTAMP(cmpStats->compileStartTime(), timestamp);
    snprintf(startTime, sizeof(startTime), "%04d/%02d/%02d %02d:%02d:%02d.%03u%03u",
             timestamp[0],   // year
             timestamp[1],   // month
             timestamp[2],   // day
             timestamp[3],   // hour
    ...
    sprintf(statsBuf_, "Compile Start Time\t: %s", startTime);
    moveRowToUpQueue(statsBuf_);

    INTERPRETTIMESTAMP(cmpStats->compileEndTime(), timestamp);
    snprintf(endTime, sizeof(endTime), "%04d/%02d/%02d %02d:%02d:%02d.%03u%03u",
             timestamp[0],   // year
             timestamp[1],   // month
             timestamp[2],   // day
             timestamp[3],   // hour
    ...
    sprintf(statsBuf_, "Compile End Time\t: %s", endTime);
    moveRowToUpQueue(statsBuf_);

    sprintf(statsBuf_, "Compiler ID\t\t: %s", cmpStats->compilerId());
    moveRowToUpQueue(statsBuf_);

    sprintf(statsBuf_, "CPU Total\t\t: %d", cmpStats->cmpCpuTotal());
    moveRowToUpQueue(statsBuf_);

    sprintf(statsBuf_, "CPU Binder\t\t: %d", cmpStats->cmpCpuBinder());
    moveRowToUpQueue(statsBuf_);

    sprintf(statsBuf_, "CPU Normalizer\t\t: %d", cmpStats->cmpCpuNormalizer());
    moveRowToUpQueue(statsBuf_);

    sprintf(statsBuf_, "CPU Analyzer\t\t: %d", cmpStats->cmpCpuAnalyzer());
    moveRowToUpQueue(statsBuf_);

    sprintf(statsBuf_, "CPU Optimizer\t\t: %d", cmpStats->cmpCpuOptimizer());
    moveRowToUpQueue(statsBuf_);

    sprintf(statsBuf_, "CPU Generator\t\t: %d", cmpStats->cmpCpuGenerator());
    moveRowToUpQueue(statsBuf_);

    sprintf(statsBuf_, "Metadata Cache Hits\t: %d", cmpStats->metadataCacheHits());
    moveRowToUpQueue(statsBuf_);

    sprintf(statsBuf_, "Metadata Cache Lookups\t: %d", cmpStats->metadataCacheLookups());
    moveRowToUpQueue(statsBuf_);
    ...
    sprintf(statsBuf_, "Query Cache State\t: %s", queryCacheStateBuf);
    moveRowToUpQueue(statsBuf_);

    sprintf(statsBuf_, "Histogram Cache Hits\t: %d", cmpStats->histogramCacheHits());
    moveRowToUpQueue(statsBuf_);

    sprintf(statsBuf_, "Histogram Cache Lookups\t: %d", cmpStats->histogramCacheLookups());
    moveRowToUpQueue(statsBuf_);

    sprintf(statsBuf_, "Statement Heap Size\t: %d", cmpStats->stmtHeapSize());
    moveRowToUpQueue(statsBuf_);

    sprintf(statsBuf_, "Context Heap Size\t: %d", cmpStats->contextHeapSize());
    moveRowToUpQueue(statsBuf_);

    sprintf(statsBuf_, "Optimization Tasks\t: %d", cmpStats->optTasks());
    moveRowToUpQueue(statsBuf_);

    sprintf(statsBuf_, "Optimization Contexts\t: %d", cmpStats->optContexts());
    moveRowToUpQueue(statsBuf_);
    ...
    sprintf(statsBuf_, "Compile Info\t\t: %s", cmpStats->compileInfo());
    moveRowToUpQueue(statsBuf_);

    sprintf(statsBuf_, "Compile Info Length\t: %d", cmpStats->compileInfoLen());
    moveRowToUpQueue(statsBuf_);
  }
}
  ...
  while (1) {
    switch (step_) {
      case INITIAL_: {
        ...
        if (inputStmtName != NULL) {
          ...
          while ((NOT found) && (stmt = (Statement *)stmtList->getNext())) {
            ...
            if ((ident) && (str_len(inputStmtName) == str_len(ident)) &&
                (str_cmp(inputStmtName, ident, str_len(ident)) == 0))  // matches
            {
              stats_ = stmt->getStatsArea();
              if (stats_ == NULL) stats_ = stmt->getCompileStatsArea();
              found = TRUE;
            }
          }  // while
        } else {
      ...
      case RETURN_COMPILER_STATS_: {
        ...
        if ((qparent_.up->getSize() - qparent_.up->getLength()) < 20) return WORK_CALL_AGAIN;  // come back later
        ...
        FormatFloat(formattedFloatVal, intSize, valSize, qci.cpuTime(), FALSE, TRUE);
        strcat(statsBuf_, "CPU: ");
        strcat(statsBuf_, formattedFloatVal);

        FormatFloat(formattedFloatVal, intSize, valSize, qci.ioTime(), FALSE, TRUE);
        strcat(statsBuf_, "  IO: ");
        strcat(statsBuf_, formattedFloatVal);

        FormatFloat(formattedFloatVal, intSize, valSize, qci.msgTime(), FALSE, TRUE);
        strcat(statsBuf_, "  Msg: ");
        strcat(statsBuf_, formattedFloatVal);

        FormatFloat(formattedFloatVal, intSize, valSize, qci.idleTime(), FALSE, TRUE);
        strcat(statsBuf_, "  Idle: ");
        strcat(statsBuf_, formattedFloatVal);
        moveRowToUpQueue(statsBuf_);

        FormatFloat(formattedFloatVal, intSize, valSize, qci.numSeqIOs(), FALSE, TRUE);
        strcat(statsBuf_, "  SeqIOs: ");
        strcat(statsBuf_, formattedFloatVal);
        moveRowToUpQueue(statsBuf_);

        FormatFloat(formattedFloatVal, intSize, valSize, qci.numRandIOs(), FALSE, TRUE);
        strcat(statsBuf_, "  RandIOs: ");
        strcat(statsBuf_, formattedFloatVal);
        moveRowToUpQueue(statsBuf_);

        FormatFloat(formattedFloatVal, intSize, valSize, qci.totalTime(), FALSE, TRUE);
        strcpy(statsBuf_, "                       TotalTime: ");
        strcat(statsBuf_, formattedFloatVal);

        FormatFloat(formattedFloatVal, intSize, valSize, qci.totalMem(), FALSE, TRUE);
        ...
        FormatFloat(formattedFloatVal, intSize, valSize, qci.maxCpuUsage(), FALSE, TRUE);
        ...
        FormatFloat(formattedFloatVal, intSize, valSize, qci.cardinality(), FALSE, TRUE);
        strcpy(statsBuf_, "Rows                   ReturnedToUser: ");
        strcat(statsBuf_, formattedFloatVal);
        moveRowToUpQueue(statsBuf_);

        sprintf(statsBuf_, "Fragment Size(Kb)      Total: %-4d  Master: %-4d  ESP: %-4d  ", csi.totalFragmentSize(),
                csi.masterFragmentSize(), csi.espFragmentSize());
        moveRowToUpQueue(statsBuf_);

        sprintf(statsBuf_, "Operators              Total: %-3d", csi.totalOps());
        moveRowToUpQueue(statsBuf_);

        sprintf(statsBuf_, "  Joins                HJ: %-3d  MJ: %-3d  NJ: %-3d   Total: %-3d", csi.hj(), csi.mj(),
                csi.nj(), csi.totalJoins());
        moveRowToUpQueue(statsBuf_);
        sprintf(statsBuf_, "  Others               ESPExchange: %-3d  UDR: %-3d  BMO: %-3d", csi.exchangeOps(),
                csi.udr(), csi.bmo());

        moveRowToUpQueue(statsBuf_);

        if (masterStats->numOfRootEsps() > 0) {
          sprintf(statsBuf_, "Parallelism            Root(%d)  DOP(%d)", masterStats->numOfRootEsps(), csi.dop());
        } else {
          sprintf(statsBuf_, "Parallelism            Root(0)  DOP(%d)", csi.dop());
        }
        moveRowToUpQueue(statsBuf_);

        sprintf(statsBuf_, "Query Characteristics  MandatoryXP: %s  MissingStats: %s  FullScanOnTable: %s ",
                (csi.mandatoryCrossProduct() ? "Yes" : "No"), (csi.missingStats() ? "Yes" : "No"),
                (csi.fullScanOnTable() ? "Yes" : "No"));
        moveRowToUpQueue(statsBuf_);

        if (csi.fullScanOnTable()) {
          sprintf(statsBuf_, "                       RowsAccessedByFullScan %f", csi.dp2RowsAccessedForFullScan());
          moveRowToUpQueue(statsBuf_);
        }

        sprintf(statsBuf_, "Query Execution        DOP: %d  Affinity: %d  XnReqd: %s", csi.dop(), csi.affinityNumber(),
                (masterStats->xnReqd() ? "Yes" : "No"));
        moveRowToUpQueue(statsBuf_);
        if (csi.bmo() > 0) {
          sprintf(statsBuf_, "                       OverFlowMode: %s  OverFlowSize: %f",
                  ExBMOStats::getScratchOverflowMode(csi.ofMode()), csi.ofSize());
          moveRowToUpQueue(statsBuf_);
        }
      ...
      case RETURN_EXECUTOR_STATS_: {
        ...
        if (((!masterStats) || (masterStats->getElapsedStartTime() == -1) ||
        ...
        sprintf(statsBuf_, "Start Time             %04d/%02d/%02d %02d:%02d:%02d.%03u%03u", timestamp[0], timestamp[1],
                timestamp[2], timestamp[3], timestamp[4], timestamp[5], timestamp[6], timestamp[7]);
        moveRowToUpQueue(statsBuf_);

        // Display first row return time
        if ((masterStats->getFirstRowReturnTime() != -1) && (NOT getStatsTdb().oldFormat()) &&
            (NOT getStatsTdb().shortFormat())) {
          juliantimestamp = CONVERTTIMESTAMP(masterStats->getFirstRowReturnTime(), 0, -1, 0);

          INTERPRETTIMESTAMP(juliantimestamp, timestamp);
          sprintf(statsBuf_, "First Row Returned     %04d/%02d/%02d %02d:%02d:%02d.%03u%03u", timestamp[0],
                  timestamp[1], timestamp[2], timestamp[3], timestamp[4], timestamp[5], timestamp[6], timestamp[7]);
          moveRowToUpQueue(statsBuf_);
        }

        // Display End Time
        juliantimestamp = CONVERTTIMESTAMP(masterStats->getElapsedEndTime(), 0, -1, 0);

        INTERPRETTIMESTAMP(juliantimestamp, timestamp);
        sprintf(statsBuf_, "End Time               %04d/%02d/%02d %02d:%02d:%02d.%03u%03u", timestamp[0], timestamp[1],
                timestamp[2], timestamp[3], timestamp[4], timestamp[5], timestamp[6], timestamp[7]);
        moveRowToUpQueue(statsBuf_);
        ...
        hour = min / 60;
        min = min % 60;
        sprintf(statsBuf_, "Compile Time                      %02u:%02u:%02u.%06u", hour, min, sec, usec);
        moveRowToUpQueue(statsBuf_);
        ...
        if ((NOT getStatsTdb().oldFormat()) && (NOT getStatsTdb().shortFormat())) {
          ...
          hour = min / 60;
          min = min % 60;
          sprintf(statsBuf_, "Fixup Time                        %02u:%02u:%02u.%06u", hour, min, sec, usec);
          moveRowToUpQueue(statsBuf_);
          ...
          hour = min / 60;
          min = min % 60;
          sprintf(statsBuf_, "Freeup Time                       %02u:%02u:%02u.%06u", hour, min, sec, usec);
          moveRowToUpQueue(statsBuf_);
        }
        ...
        hour = min / 60;
        min = min % 60;
        sprintf(statsBuf_, "Execution Time                    %02u:%02u:%02u.%06u", hour, min, sec, usec);
        moveRowToUpQueue(statsBuf_);

        if ((NOT getStatsTdb().oldFormat()) && (NOT getStatsTdb().shortFormat())) {
          sprintf(statsBuf_, "Rows Affected          %ld", masterStats->getRowsAffected());
          moveRowToUpQueue(statsBuf_);
        }

#if defined(_DEBUG)
        if (getenv("DISPLAY_COMPILATION_STATS")) {
          ComTdbRoot *rootTdb = (ComTdbRoot *)masterGlob->getStatement()->getRootTdb();
      ...
      case RETURN_OTHER_STATS_: {
        ...
        // make sure there is enough space to move master stats
        if ((qparent_.up->getSize() - qparent_.up->getLength()) < 22) return WORK_CALL_AGAIN;  // come back later
      ...
      case SETUP_DETAILED_STATS_: {
        ...
        if ((statsMergeType_ == SQLCLIDEV_ACCUMULATED_STATS) || (statsMergeType_ == SQLCLIDEV_PERTABLE_STATS) ||
      ...
      case DISPLAY_HEADING_: {
        ...
        if ((qparent_.up->getSize() - qparent_.up->getLength()) < 10) return WORK_CALL_AGAIN;  // come back later
        ...
        if ((statsMergeType_ == SQLCLIDEV_ALL_STATS) || (statsMergeType_ == SQLCLIDEV_PERTABLE_STATS) ||
            (statsMergeType_ == SQLCLIDEV_OPERATOR_STATS) || (statsMergeType_ == SQLCLI_PROGRESS_STATS)) {
          // the fetch was successful
          if (statsMergeType_ == SQLCLIDEV_PERTABLE_STATS || statsMergeType_ == SQLCLI_PROGRESS_STATS) {
            if (hdfsAccess_) {
              sprintf(statsBuf_, "%-15s%15s%15s%10s%15s%15s", "Table Name", "Records", "Records", "Hdfs", "Hdfs I/O",
                      "Hdfs Access");
              moveRowToUpQueue(statsBuf_);

              sprintf(statsBuf_, "%15s%15s%15s%10s%15s%15s", " ", "Accessed", "Used", "I/Os", "Bytes", "Time(usec)");

            } else if (getStatsTdb().oldFormat()) {
              sprintf(statsBuf_, "%-15s%15s%12s%8s%10s%13s%6s", "Table Name", "Records", "Records", "Disk", "Message",
                      "Message", "Lock");
              moveRowToUpQueue(statsBuf_);

              sprintf(statsBuf_, "%15s%15s%12s%8s%10s%13s%6s", " ", "Accessed", "Used", "I/Os", "Count", "Bytes", "");
            } else {
              sprintf(statsBuf_, "%-15s%15s%15s%10s%10s%15s%6s%6s%15s", "Table Name", "Records", "Records", "Disk",
                      "Message", "Message", "Lock", "Lock", "Disk Process");
              moveRowToUpQueue(statsBuf_);

              sprintf(statsBuf_, "%15s%15s%15s%10s%10s%15s%6s%6s%15s", " ", "Accessed", "Used", "I/Os", "Count",
                      "Bytes", "Escl", "Wait", "Busy Time");
            }

            moveRowToUpQueue(statsBuf_);
          } else if ((statsMergeType_ == SQLCLIDEV_ALL_STATS) || (statsMergeType_ == SQLCLIDEV_OPERATOR_STATS)) {
            // Construct heading.
            sprintf(statsBuf_, "%4s%4s%4s%4s%19s%9s%9s%9s%20s%20s", "OP", "LC", "RC", "EX", "TDB_NAME", "EstRows",
                    "ActRows", "WC", "Oper Cpu Time", "Details");
            moveRowToUpQueue(statsBuf_);
      ...
      case RETURN_STATS_ROW_: {
        ...
        if ((statsMergeType_ == SQLCLIDEV_ALL_STATS) || (statsMergeType_ == SQLCLIDEV_OPERATOR_STATS))
      ...
      case FORMAT_AND_RETURN_PERTABLE_STATS_: {
        if ((qparent_.up->getSize() - qparent_.up->getLength()) < 4) return WORK_CALL_AGAIN;  // come back later
      ...
      case FORMAT_AND_RETURN_ACCUMULATED_STATS_: {
        // make sure there is enough space to move header
        if ((qparent_.up->getSize() - qparent_.up->getLength()) < 31) return WORK_CALL_AGAIN;  // come back later
...
ExExeUtilGetRTSStatisticsTcb::ExExeUtilGetRTSStatisticsTcb(const ComTdbExeUtilGetStatistics &exe_util_tdb,
                                                           ex_globals *glob)
    : ExExeUtilTcb(exe_util_tdb, NULL, glob),
      operatorStatsOption_(((ComTdbExeUtilGetStatistics &)exe_util_tdb).dataUsedStats() ? DATA_USED : ALL),
      filePath_(glob ? glob->getDefaultHeap() : NULL) {
  // Allocate the private state in each entry of the down queue
  qparent_.down->allocatePstate(this);
...
ExExeUtilGetRTSStatisticsTcb::~ExExeUtilGetRTSStatisticsTcb() {
  NADELETEBASIC(statsBuf_, getGlobals()->getDefaultHeap());
  if (sqlStatsDesc_ != NULL) NADELETEBASIC(sqlStatsDesc_, getGlobals()->getDefaultHeap());
...
void ExExeUtilGetRTSStatisticsTcb::formatOperStatItems(SQLSTATS_ITEM operStatsItems[]) {
  ...
  operStatsItems[6].str_value = new (getGlobals()->getDefaultHeap()) char[MAX_TDB_NAME_LEN + 1];
  ...
  operStatsItems[11].statsItem_id = SQLSTATS_ACT_DATA_USED;
  operStatsItems[12].statsItem_id = SQLSTATS_DETAIL;
  operStatsItems[12].str_value = new (getGlobals()->getDefaultHeap()) char[1001];
  operStatsItems[12].str_max_len = 1000;
  operStatsItems[13].statsItem_id = SQLSTATS_DOP;
}
...
// Est Rows Accessed, Est Rows Used, Act Rows Used, Act Data Used,
...
void ExExeUtilGetRTSStatisticsTcb::formatOperStats(SQLSTATS_ITEM *operStatsItems) {
  ...
  if (!isHeadingDisplayed_) {
    ...
    sprintf(statsBuf_, "%5s%5s%5s%5s%5s%5s %-25s%5s%13s%19s%19s%19s%19s %s", "LC", "RC", "Id", "PaId", "ExId", "Frag",
            "TDBName", "DOP", "Dispatches", "OperCPUTime", "EstRowsUsed", "ActRowsUsed", "ActDataUsed", "Details");
    moveRowToUpQueue(statsBuf_);
    moveRowToUpQueue("");
  }
  ...
  // dop
  formatInt64(operStatsItems[13], valString);
  sprintf(&statsBuf_[strlen(statsBuf_)], "%5s", valString);
  ...
  // Actual Data Used
  formatInt64(operStatsItems[11], valString);
  sprintf(&statsBuf_[strlen(statsBuf_)], "%19s", valString);

  // Detail
  if (operStatsItems[12].error_code)
    sprintf(&statsBuf_[strlen(statsBuf_)], "%s", "");
  else {
    operStatsItems[12].str_value[operStatsItems[12].str_ret_len] = '\0';
    sprintf(&statsBuf_[strlen(statsBuf_)], " %s", operStatsItems[12].str_value);
  }
}

// A method to format actual data used run-time stats. It also writes such
    ...
    switch (step_) {
      case INITIAL_: {
        ...
        cliRC = SQL_EXEC_GetStatistics2(getStatsTdb().statsReqType_, getStatsTdb().stmtName_,
                                        ...
                                        &statsCollectType_, sqlStatsDesc_, maxStatsDescEntries_, &retStatsDescEntries_);
        ...
        if (cliRC == -CLI_INSUFFICIENT_STATS_DESC) {
          ...
          if (statsMergeType == SQLCLI_SAME_STATS) statsMergeType = statsCollectType_;
      ...
      case GET_NEXT_STATS_DESC_ENTRY_: {
        ...
        if (currStatsDescEntry_ >= retStatsDescEntries_) {
          if (!filePath_.isNull()) {
            ...
            if (currContext) {
              CmpContext *c = currContext->getEmbeddedArkcmpContext();

              c->getQueryCache()->deCacheAll(queryHash_);
            }
          }
      ...
      case GET_MASTER_STATS_ENTRY_: {
        if (masterStatsItems_ == NULL) {
          maxMasterStatsItems_ = 39;
          masterStatsItems_ = new (getGlobals()->getDefaultHeap()) SQLSTATS_ITEM[maxMasterStatsItems_];
          ...
          masterStatsItems_[32].statsItem_id = SQLSTATS_QUERY_HASH;
          masterStatsItems_[33].statsItem_id = SQLSTATS_SLA_NAME;
          masterStatsItems_[34].statsItem_id = SQLSTATS_PROFILE_NAME;
          masterStatsItems_[35].statsItem_id = SQLSTATS_EXECUTE_COUNT;
          masterStatsItems_[36].statsItem_id = SQLSTATS_EXECUTE_TIME_MIN;
          masterStatsItems_[37].statsItem_id = SQLSTATS_EXECUTE_TIME_MAX;
          masterStatsItems_[38].statsItem_id = SQLSTATS_EXECUTE_TIME_AVG;
          // maxMasterStatsItems_ is set to 39
          masterStatsItems_[0].str_value = new (getGlobals()->getDefaultHeap()) char[ComSqlId::MAX_QUERY_ID_LEN + 1];
          masterStatsItems_[0].str_max_len = ComSqlId::MAX_QUERY_ID_LEN;
          // Parent Qid
          masterStatsItems_[15].str_value = new (getGlobals()->getDefaultHeap()) char[ComSqlId::MAX_QUERY_ID_LEN + 1];
          masterStatsItems_[15].str_max_len = ComSqlId::MAX_QUERY_ID_LEN;
          // Parent Qid System
          masterStatsItems_[16].str_value = new (getGlobals()->getDefaultHeap()) char[24];
          masterStatsItems_[16].str_max_len = 23;
          // Child Qid
          masterStatsItems_[17].str_value = new (getGlobals()->getDefaultHeap()) char[ComSqlId::MAX_QUERY_ID_LEN + 1];
          masterStatsItems_[17].str_max_len = ComSqlId::MAX_QUERY_ID_LEN;
          // Source Str
          masterStatsItems_[22].str_value = new (getGlobals()->getDefaultHeap()) char[RMS_STORE_SQL_SOURCE_LEN + 2];
          masterStatsItems_[22].str_max_len = RMS_STORE_SQL_SOURCE_LEN;
          masterStatsItems_[33].str_value = new (getGlobals()->getDefaultHeap()) char[MAX_SLA_NAME_LEN + 1];
          masterStatsItems_[33].str_max_len = MAX_SLA_NAME_LEN;
          masterStatsItems_[34].str_value = new (getGlobals()->getDefaultHeap()) char[MAX_PROFILE_NAME_LEN + 1];
          masterStatsItems_[34].str_max_len = MAX_PROFILE_NAME_LEN;
        } else
          initSqlStatsItems(masterStatsItems_, maxMasterStatsItems_, TRUE);
        cliRC = SQL_EXEC_GetStatisticsItems(getStatsTdb().statsReqType_, getStatsTdb().stmtName_,
                                            getStatsTdb().stmtName_ ? str_len(getStatsTdb().stmtName_) : 0,
                                            maxMasterStatsItems_, masterStatsItems_);
        if (cliRC < 0) {
          step_ = HANDLE_ERROR_;
        } else {
      ...
      case FORMAT_AND_RETURN_MASTER_STATS_: {
        for (; currStatsItemEntry_ < maxMasterStatsItems_; currStatsItemEntry_++) {
          ...
          // In data used mode, only get query hash field from the master stats.
          if (statsMergeType == SQLCLI_OPERATOR_STATS &&
              getOperatorStatsOption() == ExExeUtilGetRTSStatisticsTcb::DATA_USED &&
              masterStatsItems_[i].statsItem_id != SQLSTATS_QUERY_HASH)
            continue;
          ...
          switch (masterStatsItems_[i].statsItem_id) {
            ...
            case SQLSTATS_COMP_START_TIME:
              ...
              sprintf(statsBuf_, "%-25s%04d/%02d/%02d %02d:%02d:%02d.%03u%03u", "Compile Start Time", timestamp[0],
                      timestamp[1], timestamp[2], timestamp[3], timestamp[4], timestamp[5], timestamp[6], timestamp[7]);
              break;
            case SQLSTATS_COMP_END_TIME:
              if (masterStatsItems_[i].int64_value != -1) {
                jtime = CONVERTTIMESTAMP(masterStatsItems_[i].int64_value, 0, -1, NULL);
                INTERPRETTIMESTAMP(jtime, timestamp);
                sprintf(statsBuf_, "%-25s%04d/%02d/%02d %02d:%02d:%02d.%03u%03u", "Compile End Time", timestamp[0],
            ...
            case SQLSTATS_COMP_TIME:
              ...
              sprintf(statsBuf_, "%-34s%4u:%02u:%02u.%06u", "Compile Elapsed Time", hour, min, sec, usec);
              break;
            case SQLSTATS_EXECUTE_START_TIME:
              if (masterStatsItems_[i].int64_value != -1) {
                jtime = CONVERTTIMESTAMP(masterStatsItems_[i].int64_value, 0, -1, NULL);
                INTERPRETTIMESTAMP(jtime, timestamp);
                sprintf(statsBuf_, "%-25s%04d/%02d/%02d %02d:%02d:%02d.%03u%03u", "Execute Start Time", timestamp[0],
                        timestamp[1], timestamp[2], timestamp[3], timestamp[4], timestamp[5], timestamp[6],
                        timestamp[7]);
              } else
            ...
            case SQLSTATS_FIRST_ROW_RET_TIME:
              if (masterStatsItems_[i].int64_value != -1) {
                ...
                INTERPRETTIMESTAMP(jtime, timestamp);
                sprintf(statsBuf_, "%-25s%04d/%02d/%02d %02d:%02d:%02d.%03u%03u", "First Row Returned Time",
                        timestamp[0], timestamp[1], timestamp[2], timestamp[3], timestamp[4], timestamp[5],
                        timestamp[6], timestamp[7]);
              } else {
                sprintf(statsBuf_, "%-25s%s", "First Row Returned Time", "-1");
              }
              break;
            case SQLSTATS_CANCEL_TIME_ID:
              if (masterStatsItems_[i].int64_value != -1) {
                jtime = CONVERTTIMESTAMP(masterStatsItems_[i].int64_value, 0, -1, NULL);
                INTERPRETTIMESTAMP(jtime, timestamp);
                sprintf(statsBuf_, "%-25s%04d/%02d/%02d %02d:%02d:%02d.%03u%03u", "Cancel Time", timestamp[0],
                        timestamp[1], timestamp[2], timestamp[3], timestamp[4], timestamp[5], timestamp[6],
                        timestamp[7]);
              } else
            ...
            case SQLSTATS_EXECUTE_END_TIME:
              if (masterStatsItems_[i].int64_value != -1) {
                jtime = CONVERTTIMESTAMP(masterStatsItems_[i].int64_value, 0, -1, NULL);
                INTERPRETTIMESTAMP(jtime, timestamp);
                sprintf(statsBuf_, "%-25s%04d/%02d/%02d %02d:%02d:%02d.%03u%03u", "Execute End Time", timestamp[0],
                        timestamp[1], timestamp[2], timestamp[3], timestamp[4], timestamp[5], timestamp[6],
                        timestamp[7]);
              } else
            ...
            case SQLSTATS_FIXUP_TIME:
              ...
              hour = min / 60;
              min = min % 60;
              sprintf(statsBuf_, "%34s%4u:%02u:%02u.%06u", "Fixup Elapsed Time", hour, min, sec, usec);
              break;
            case SQLSTATS_STMT_STATE:
              stmtState = (short)masterStatsItems_[i].int64_value;
              if (stmtState >= SQLSTMT_STATE_UNKNOWN) stmtState = SQLSTMT_STATE_UNKNOWN;
              sprintf(statsBuf_, "%-25s%s", "State", Statement::stmtState((Statement::State)stmtState));
              break;
            ...
            case SQLSTATS_QUERY_TYPE:
              ...
              sprintf(statsBuf_, "%-25s%s", "Query Type", ComTdbRoot::getQueryTypeText(queryType));
              break;
            case SQLSTATS_SUBQUERY_TYPE:
              subqueryType = (short)masterStatsItems_[i].int64_value;
              sprintf(statsBuf_, "%-25s%s", "Sub Query Type", ComTdbRoot::getSubqueryTypeText(subqueryType));
              break;
            ...
            case SQLSTATS_TRANSID:
              sprintf(Int64Val, "%ld", masterStatsItems_[i].int64_value);
              intSize = str_len(Int64Val);
              sprintf(statsBuf_, "%-25s%s", "Transaction Id", Int64Val);
              break;
            case SQLSTATS_SUSPEND_TIME_ID:
              if (masterStatsItems_[i].int64_value != -1) {
                jtime = CONVERTTIMESTAMP(masterStatsItems_[i].int64_value, 0, -1, NULL);
                INTERPRETTIMESTAMP(jtime, timestamp);
                sprintf(statsBuf_, "%-25s%04d/%02d/%02d %02d:%02d:%02d.%03u%03u", "Last Suspend Time", timestamp[0],
                        timestamp[1], timestamp[2], timestamp[3], timestamp[4], timestamp[5], timestamp[6],
                        timestamp[7]);
              } else
            ...
            case SQLSTATS_QUERY_HASH:
              queryHash_ = masterStatsItems_[i].int64_value;
              sprintf(Int64Val, "%ld", queryHash_);
              intSize = str_len(Int64Val);
              sprintf(statsBuf_, "%-25s%s", "Query hash", Int64Val);
              break;
            case SQLSTATS_SLA_NAME:
              masterStatsItems_[i].str_value[masterStatsItems_[i].str_ret_len] = '\0';
              sprintf(statsBuf_, "%-25s%s", "SLA Name", masterStatsItems_[i].str_value);
              break;
            case SQLSTATS_PROFILE_NAME:
              masterStatsItems_[i].str_value[masterStatsItems_[i].str_ret_len] = '\0';
              sprintf(statsBuf_, "%-25s%s", "Profile Name", masterStatsItems_[i].str_value);
              break;
            default:
              statsBuf_[0] = '\0';
              break;
          ...
          if (strlen(statsBuf_) > 0)
            if (moveRowToUpQueue(statsBuf_, strlen(statsBuf_), &rc) == -1) return rc;
        }
        sprintf(statsBuf_, "%-25s%s", "Stats Collection Type", ExStatisticsArea::getStatsTypeText(statsCollectType_));
        if (moveRowToUpQueue(statsBuf_, strlen(statsBuf_), &rc) == -1) return rc;
        step_ = GET_NEXT_STATS_DESC_ENTRY_;
      } break;
      case GET_MEAS_STATS_ENTRY_: {
        if (measStatsItems_ == NULL) {
          ...
          measStatsItems_ = new (getGlobals()->getDefaultHeap()) SQLSTATS_ITEM[maxMeasStatsItems_];
        ...
        cliRC = SQL_EXEC_GetStatisticsItems(getStatsTdb().statsReqType_, getStatsTdb().stmtName_,
                                            getStatsTdb().stmtName_ ? str_len(getStatsTdb().stmtName_) : 0,
                                            maxMeasStatsItems_, measStatsItems_);
        if (cliRC < 0) {
          step_ = HANDLE_ERROR_;
        } else
      ...
      case GET_ROOTOPER_STATS_ENTRY_: {
        if (rootOperStatsItems_ == NULL) {
          ...
          rootOperStatsItems_ = new (getGlobals()->getDefaultHeap()) SQLSTATS_ITEM[maxRootOperStatsItems_];
        ...
        cliRC = SQL_EXEC_GetStatisticsItems(getStatsTdb().statsReqType_, getStatsTdb().stmtName_,
                                            getStatsTdb().stmtName_ ? str_len(getStatsTdb().stmtName_) : 0,
                                            maxRootOperStatsItems_, rootOperStatsItems_);
        if (cliRC < 0) {
          step_ = HANDLE_ERROR_;
        } else
      ...
      case FORMAT_AND_RETURN_ROOTOPER_STATS_: {
        for (; currStatsItemEntry_ < maxRootOperStatsItems_; currStatsItemEntry_++) {
          i = (short)currStatsItemEntry_;
          if (rootOperStatsItems_[i].error_code != 0) continue;
      ...
      case GET_HBASE_STATS_ENTRY_: {
        if (hbaseStatsItems_ == NULL) {
          maxHbaseStatsItems_ = 12;
          hbaseStatsItems_ = new (getGlobals()->getDefaultHeap()) SQLSTATS_ITEM[maxHbaseStatsItems_];
          ...
          hbaseStatsItems_[7].statsItem_id = SQLSTATS_ACT_ROWS_FILTERED;
          hbaseStatsItems_[8].statsItem_id = SQLSTATS_HBASE_IOS;
          hbaseStatsItems_[9].statsItem_id = SQLSTATS_HBASE_IO_BYTES;
          hbaseStatsItems_[10].statsItem_id = SQLSTATS_HBASE_IO_ELAPSED_TIME;
          hbaseStatsItems_[11].statsItem_id = SQLSTATS_HBASE_IO_MAX_TIME;
          // maxHbaseStatsItems_ is set to 11
          // SQLSTATS_TABLE_ANSI_NAM
          hbaseStatsItems_[2].str_value =
              new (getGlobals()->getDefaultHeap()) char[ComMAX_3_PART_EXTERNAL_UTF8_NAME_LEN_IN_BYTES + 1];
        ...
        cliRC = SQL_EXEC_GetStatisticsItems(getStatsTdb().statsReqType_, getStatsTdb().stmtName_,
                                            getStatsTdb().stmtName_ ? str_len(getStatsTdb().stmtName_) : 0,
                                            maxHbaseStatsItems_, hbaseStatsItems_);
        if (cliRC < 0) {
          step_ = HANDLE_ERROR_;
        } else {
          if (!isHeadingDisplayed_)
      ...
      case DISPLAY_HIVE_STATS_HEADING_: {
        if ((qparent_.up->getSize() - qparent_.up->getLength()) < 5) return WORK_CALL_AGAIN;
        moveRowToUpQueue(" ");
        if (singleLineFormat_) {
          sprintf(statsBuf_, "%5s%10s%15s%20s%15s%20s%20s%20s%20s%20s%20s%10s", "Id", "DOP", "EstRowsAccess",
                  "ActRowsAccess", "EstRowUsed", "ActRowsUsed", "ActRowsFiltered", "SE_IOs", "SE_IO_KBytes",
                  "SE_IO_SumTime", "SE_IO_MaxTime", "TableName");
          moveRowToUpQueue(statsBuf_);
        } else {
          sprintf(statsBuf_, "%5s%10s %-20s", "Id", "DOP", "Table Name");
          moveRowToUpQueue(statsBuf_);
          sprintf(statsBuf_, "%15s%20s%15s%20s%20s%20s%20s%20s%20s", "EstRowsAccess", "ActRowsAccess", "EstRowsUsed",
                  "ActRowsUsed", "ActRowsFiltered", "SE_IOs", "SE_IO_KBytes", "SE_IO_SumTime", "SE_IO_MaxTime");
          moveRowToUpQueue(statsBuf_);
        }
        isHeadingDisplayed_ = TRUE;
      ...
      case FORMAT_AND_RETURN_HIVE_STATS_: {
        ...
        for (; currStatsItemEntry_ < maxStatsItems; currStatsItemEntry_++) {
          i = (short)currStatsItemEntry_;
          if (statsItems[i].error_code != 0) continue;
        ...
        if (singleLineFormat_) sprintf(&statsBuf_[strlen(statsBuf_)], " %s", statsItems[2].str_value);
        if (moveRowToUpQueue(statsBuf_, strlen(statsBuf_), &rc) == -1) return rc;
        step_ = GET_NEXT_STATS_DESC_ENTRY_;
      } break;
      case GET_HIVE_STATS_ENTRY_: {
        if (hiveStatsItems_ == NULL) {
          ...
          hiveStatsItems_ = new (getGlobals()->getDefaultHeap()) SQLSTATS_ITEM[maxHiveStatsItems_];
        ...
        cliRC = SQL_EXEC_GetStatisticsItems(getStatsTdb().statsReqType_, getStatsTdb().stmtName_,
                                            getStatsTdb().stmtName_ ? str_len(getStatsTdb().stmtName_) : 0,
                                            maxHiveStatsItems_, hiveStatsItems_);
        if (cliRC < 0) {
          step_ = HANDLE_ERROR_;
        } else {
          if (!isHeadingDisplayed_)
      ...
      case GET_BMO_STATS_ENTRY_: {
        if (bmoStatsItems_ == NULL) {
          ...
          bmoStatsItems_ = new (getGlobals()->getDefaultHeap()) SQLSTATS_ITEM[maxBMOStatsItems_];
          ...
          // TDB_NAME
          bmoStatsItems_[1].str_value = new (getGlobals()->getDefaultHeap()) char[MAX_TDB_NAME_LEN + 1];
          bmoStatsItems_[1].str_max_len = MAX_TDB_NAME_LEN;
          // OVERFLOW_MODE
          bmoStatsItems_[2].str_value = new (getGlobals()->getDefaultHeap()) char[13];
          bmoStatsItems_[2].str_max_len = 12;
          // BMO_PHASE
          bmoStatsItems_[5].str_value = new (getGlobals()->getDefaultHeap()) char[12];
        ...
        cliRC = SQL_EXEC_GetStatisticsItems(getStatsTdb().statsReqType_, getStatsTdb().stmtName_,
                                            getStatsTdb().stmtName_ ? str_len(getStatsTdb().stmtName_) : 0,
                                            maxBMOStatsItems_, bmoStatsItems_);
        if (cliRC < 0) {
          step_ = HANDLE_ERROR_;
        } else {
      ...
      case DISPLAY_BMO_STATS_HEADING_: {
        ...
        moveRowToUpQueue(" ");
        if (singleLineFormat_) {
          sprintf(statsBuf_, "%5s%20s%5s%10s%10s%12s%20s%20s%20s%20s%20s%20s%20s%20s%10s%10s%20s%20s%20s%20s", "Id",
                  "TDBName", "Mode", "DOP", "TopN", "BMOPhase", "InterimRowCount", "CPUTime", "BMOHeapUsed",
                  "BMOHeapAllocated", "BMOHeapWM", "EstMemory", "BMOSpaceBufSz", "BMOSpaceBufCnt", "FileCnt",
                  "ScrIOSize", "ScrIORead", "ScrIOWritten", "ScrIOTime", "ScrIOMaxTime");
          moveRowToUpQueue(statsBuf_);
        } else {
          sprintf(statsBuf_, "%5s%20s%20s%10s%10s%20s%20s%20s", "Id", "TDBName", "Mode", "DOP", "TopN", "BMOPhase",
                  "InterimRowCount", "CPUTime");
          moveRowToUpQueue(statsBuf_);

          sprintf(statsBuf_, "%25s%20s%20s%20s%20s%20s", "BMOHeapUsed", "BMOHeapAllocated", "BMOHeapWM", "EstMemory",
                  "BMOSpaceBufSz", "BMOSpaceBufCnt");
          moveRowToUpQueue(statsBuf_);

          sprintf(statsBuf_, "%25s%20s%20s%20s%20s%20s", "ScrFileCnt", "ScrIOSize", "ScrIORead", "ScrIOWritten",
                  "ScrIOTime", "ScrIOMaxTime");
          moveRowToUpQueue(statsBuf_);
        }
        ...
        for (; currStatsItemEntry_ < maxBMOStatsItems_; currStatsItemEntry_++) {
          ...
          switch (bmoStatsItems_[i].statsItem_id) {
            ...
            case SQLSTATS_BMO_EST_MEMORY:
              ...
              sprintf(&statsBuf_[strlen(statsBuf_)], "%20s", formattedFloatVal);
              break;
            case SQLSTATS_BMO_SPACE_BUFFER_SIZE:
              sprintf(Int64Val, "%ld", bmoStatsItems_[i].int64_value);
              intSize = str_len(Int64Val);
      ...
      case GET_REPLICATE_STATS_ENTRY_: {
        if (replicateStatsItems_ == NULL) {
          ...
          replicateStatsItems_ = new (getGlobals()->getDefaultHeap()) SQLSTATS_ITEM[maxReplicateStatsItems_];
          ...
          // ANSI_NAME
          replicateStatsItems_[0].str_value =
              new (getGlobals()->getDefaultHeap()) char[ComMAX_3_PART_EXTERNAL_UTF8_NAME_LEN_IN_BYTES + 1];
          replicateStatsItems_[0].str_max_len = ComMAX_3_PART_EXTERNAL_UTF8_NAME_LEN_IN_BYTES;
          // SOURCE_SYSTEM_NAME
          replicateStatsItems_[1].str_value = new (getGlobals()->getDefaultHeap()) char[24];
          replicateStatsItems_[1].str_max_len = 23;
          // TARGET_SYSTEM_NAME
          replicateStatsItems_[2].str_value = new (getGlobals()->getDefaultHeap()) char[24];
          replicateStatsItems_[2].str_max_len = 23;
          // OBJECT_TYPE
          replicateStatsItems_[3].str_value = new (getGlobals()->getDefaultHeap()) char[51];
          replicateStatsItems_[3].str_max_len = 50;
          // REPL_TYPE
          replicateStatsItems_[4].str_value = new (getGlobals()->getDefaultHeap()) char[51];
          replicateStatsItems_[4].str_max_len = 50;
          // STATUS
          replicateStatsItems_[29].str_value = new (getGlobals()->getDefaultHeap()) char[51];
        ...
        cliRC = SQL_EXEC_GetStatisticsItems(getStatsTdb().statsReqType_, getStatsTdb().stmtName_,
                                            getStatsTdb().stmtName_ ? str_len(getStatsTdb().stmtName_) : 0,
                                            maxReplicateStatsItems_, replicateStatsItems_);
        if (cliRC < 0)
          step_ = HANDLE_ERROR_;
        else
      ...
      case FORMAT_AND_RETURN_REPLICATE_STATS_: {
        for (; currStatsItemEntry_ < maxReplicateStatsItems_; currStatsItemEntry_++) {
          i = (short)currStatsItemEntry_;
          if (replicateStatsItems_[i].error_code != 0) continue;
          switch (replicateStatsItems_[i].statsItem_id) {
            ...
            case SQLSTATS_REPL_PHASE0_START_TIME:
              if (replicateStatsItems_[i].int64_value == -1) continue;
      ...
      case GET_REPLICATOR_STATS_ENTRY_: {
        if (replicatorStatsItems_ == NULL) {
          ...
          replicatorStatsItems_ = new (getGlobals()->getDefaultHeap()) SQLSTATS_ITEM[maxReplicatorStatsItems_];
        ...
        cliRC = SQL_EXEC_GetStatisticsItems(getStatsTdb().statsReqType_, getStatsTdb().stmtName_,
                                            getStatsTdb().stmtName_ ? str_len(getStatsTdb().stmtName_) : 0,
                                            maxReplicatorStatsItems_, replicatorStatsItems_);
        if (cliRC < 0)
          step_ = HANDLE_ERROR_;
        else
      ...
      case FORMAT_AND_RETURN_REPLICATOR_STATS_: {
        for (; currStatsItemEntry_ < maxReplicatorStatsItems_; currStatsItemEntry_++) {
          i = (short)currStatsItemEntry_;
          if (replicatorStatsItems_[i].error_code != 0) continue;
      ...
      case GET_OPER_STATS_ENTRY_: {
        maxOperatorStatsItems_ = 14;
        if (operatorStatsItems_ == NULL) {
          operatorStatsItems_ = new (getGlobals()->getDefaultHeap()) SQLSTATS_ITEM[maxOperatorStatsItems_];
        ...
        cliRC = SQL_EXEC_GetStatisticsItems(getStatsTdb().statsReqType_, getStatsTdb().stmtName_,
                                            getStatsTdb().stmtName_ ? str_len(getStatsTdb().stmtName_) : 0,
                                            maxOperatorStatsItems_, operatorStatsItems_);
        if (cliRC < 0) {
          step_ = HANDLE_ERROR_;
        } else {
      ...
      case FORMAT_AND_RETURN_OPER_STATS_: {
        switch (getOperatorStatsOption()) {
          case ExExeUtilGetRTSStatisticsTcb::DATA_USED:
            formatOperStatsDataUsed(operatorStatsItems_);
            break;

          case ExExeUtilGetRTSStatisticsTcb::ALL:
          default:
      ...
      case GET_RMS_STATS_ENTRY_: {
        if (rmsStatsItems_ == NULL) {
          maxRMSStatsItems_ = 38;
          rmsStatsItems_ = new (getGlobals()->getDefaultHeap()) SQLSTATS_ITEM[maxRMSStatsItems_];
          ...
          rmsStatsItems_[34].statsItem_id = SQLSTATS_EPOCH_HEAP_ALLOC;
          rmsStatsItems_[35].statsItem_id = SQLSTATS_EPOCH_HEAP_USED;
          rmsStatsItems_[36].statsItem_id = SQLSTATS_EPOCH_HEAP_HIGH_WM;
          rmsStatsItems_[37].statsItem_id = SQLSTATS_EPOCH_ENTRIES;
          // maxRMSStatsItems_ is set to 38
          rmsStatsItems_[0].str_value = new (getGlobals()->getDefaultHeap()) char[MAX_SEGMENT_NAME_LEN + 1];
        ...
        cliRC = SQL_EXEC_GetStatisticsItems(getStatsTdb().statsReqType_, getStatsTdb().stmtName_,
                                            getStatsTdb().stmtName_ ? str_len(getStatsTdb().stmtName_) : 0,
                                            maxRMSStatsItems_, rmsStatsItems_);
        if (cliRC < 0) {
          step_ = HANDLE_ERROR_;
        } else
      ...
      case FORMAT_AND_RETURN_RMS_STATS_: {
        for (; currStatsItemEntry_ < maxRMSStatsItems_; currStatsItemEntry_++) {
          i = (short)currStatsItemEntry_;
          statsBuf_[0] = '\0';
          if (rmsStatsItems_[i].error_code != 0) continue;
          switch (rmsStatsItems_[i].statsItem_id) {
            ...
            case SQLSTATS_SSCP_TIMESTAMP:
              ...
              INTERPRETTIMESTAMP(jtime, timestamp);
              sprintf(statsBuf_, "%-30s%04d/%02d/%02d %02d:%02d:%02d.%03u%03u", "SSCP Creation Timestamp ",
            ...
            case SQLSTATS_SSMP_TIMESTAMP:
              jtime = CONVERTTIMESTAMP(rmsStatsItems_[i].int64_value, 0, -1, NULL);
              INTERPRETTIMESTAMP(jtime, timestamp);
              sprintf(statsBuf_, "%-30s%04d/%02d/%02d %02d:%02d:%02d.%03u%03u", "SSMP Creation Timestamp ",
                      timestamp[0], timestamp[1], timestamp[2], timestamp[3], timestamp[4], timestamp[5], timestamp[6],
                      timestamp[7]);
              break;
            ...
            case SQLSTATS_LAST_GC_TIME:
              jtime = CONVERTTIMESTAMP(rmsStatsItems_[i].int64_value, 0, -1, NULL);
              INTERPRETTIMESTAMP(jtime, timestamp);
              sprintf(statsBuf_, "%-30s%04d/%02d/%02d %02d:%02d:%02d.%03u%03u", "Last GC Time ", timestamp[0],
                      timestamp[1], timestamp[2], timestamp[3], timestamp[4], timestamp[5], timestamp[6], timestamp[7]);
              break;
            ...
            case SQLSTATS_RMS_STATS_RESET_TIMESTAMP:
              jtime = CONVERTTIMESTAMP(rmsStatsItems_[i].int64_value, 0, -1, NULL);
              INTERPRETTIMESTAMP(jtime, timestamp);
              sprintf(statsBuf_, "%-30s%04d/%02d/%02d %02d:%02d:%02d.%03u%03u", "RMS Stats Reset Timestamp ",
                      timestamp[0], timestamp[1], timestamp[2], timestamp[3], timestamp[4], timestamp[5], timestamp[6],
                      timestamp[7]);
              break;
            ...
            case SQLSTATS_EPOCH_HEAP_ALLOC:
              ...
              intSize = str_len(Int64Val);
              AddCommas(Int64Val, intSize);
              sprintf(statsBuf_, "%-30s%s", "Epoch Heap Allocated", Int64Val);
              break;
            case SQLSTATS_EPOCH_HEAP_USED:
              sprintf(Int64Val, "%ld", rmsStatsItems_[i].int64_value);
              intSize = str_len(Int64Val);
              AddCommas(Int64Val, intSize);
              sprintf(statsBuf_, "%-30s%s", "Epoch Heap Used", Int64Val);
              break;
            case SQLSTATS_EPOCH_HEAP_HIGH_WM:
              sprintf(Int64Val, "%ld", rmsStatsItems_[i].int64_value);
              intSize = str_len(Int64Val);
              AddCommas(Int64Val, intSize);
              sprintf(statsBuf_, "%-30s%s", "Epoch Heap High WM", Int64Val);
              break;
            case SQLSTATS_EPOCH_ENTRIES:
              sprintf(statsBuf_, "%-30s%-ld", "No.of Epoch Entries", rmsStatsItems_[i].int64_value);
              break;
          }
          if (strlen(statsBuf_) > 0)
            if (moveRowToUpQueue(statsBuf_, strlen(statsBuf_), &rc) == -1) return rc;
        ...
        if (moveRowToUpQueue(" ", strlen(" "), &rc) == -1) return rc;
        step_ = GET_NEXT_STATS_DESC_ENTRY_;
      } break;
      case GET_UDR_BASE_STATS_ENTRY_: {
        if (udrbaseStatsItems_ == NULL) {
          maxUDRBaseStatsItems_ = 8;
          udrbaseStatsItems_ = new (getGlobals()->getDefaultHeap()) SQLSTATS_ITEM[maxUDRBaseStatsItems_];
        ...
        cliRC = SQL_EXEC_GetStatisticsItems(getStatsTdb().statsReqType_, getStatsTdb().stmtName_,
                                            getStatsTdb().stmtName_ ? str_len(getStatsTdb().stmtName_) : 0,
                                            maxUDRBaseStatsItems_, udrbaseStatsItems_);
        if (cliRC < 0) {
          step_ = HANDLE_ERROR_;
        } else {
      ...
      case DISPLAY_UDR_BASE_STATS_HEADING_: {
        if ((qparent_.up->getSize() - qparent_.up->getLength()) < 3) return WORK_CALL_AGAIN;
        moveRowToUpQueue(" ");
        sprintf(statsBuf_, "%5s%-21s%-27s%-27s", "   Id", "UDR CPU Busy Time", "Recent Req.Timestamp",
                "Recent Reply Timestamp");
        moveRowToUpQueue(statsBuf_);
        sprintf(statsBuf_, "%-20s%-20s%-20s%-20s", "Req. Msg count", "Req. Msg Bytes", "Reply Msg Count",
                "Reply Msg Bytes");
        moveRowToUpQueue(statsBuf_);
      ...
      case FORMAT_AND_RETURN_UDR_BASE_STATS_: {
        for (; currStatsItemEntry_ < maxUDRBaseStatsItems_; currStatsItemEntry_++) {
          i = (short)currStatsItemEntry_;
          if (udrbaseStatsItems_[i].error_code != 0) continue;
          switch (udrbaseStatsItems_[i].statsItem_id) {
            ...
            case SQLSTATS_RECENT_REQ_TS:
              if (udrbaseStatsItems_[i].int64_value == -1) {
                ...
                INTERPRETTIMESTAMP(jtime, timestamp);
                sprintf(&statsBuf_[strlen(statsBuf_)], " %04d/%02d/%02d %02d:%02d:%02d.%03u%03u", timestamp[0],
                        timestamp[1], timestamp[2], timestamp[3], timestamp[4], timestamp[5], timestamp[6],
                        timestamp[7]);
              }
              break;
      ...
      case HANDLE_ERROR_: {
        ...
        ExHandleErrors(qparent_, pentry_down, 0, getGlobals(), (diagsArea->getNumber() > 0 ? diagsArea : NULL),
                       (ExeErrorCode)cliRC, NULL, NULL);
        if (diagsArea->getNumber() > 0) diagsArea->clear();
        step_ = DONE_;
      } break;
      case DONE_: {
        if (qparent_.up->isFull()) return WORK_OK;
...
char *ExExeUtilGetRTSStatisticsTcb::formatTimestamp(char *buf, long inTime) {
  ...
  if (inTime == -1)
    ...
    INTERPRETTIMESTAMP(jtime, timestamp);
    sprintf(buf, "%04d/%02d/%02d %02d:%02d:%02d.%03u%03u", timestamp[0], timestamp[1], timestamp[2], timestamp[3],
...
ExExeUtilGetProcessStatisticsTcb::ExExeUtilGetProcessStatisticsTcb(
    const ComTdbExeUtilGetProcessStatistics &exe_util_tdb, ex_globals *glob)
  ...
  while (1) {
    switch (step_) {
      ...
      case GET_PROCESS_STATS_AREA_: {
        cliRC = SQL_EXEC_GetStatisticsArea_Internal(getStatsTdb().statsReqType_, getStatsTdb().stmtName_,
                                                    getStatsTdb().stmtName_ ? str_len(getStatsTdb().stmtName_) : 0,
                                                    getStatsTdb().activeQueryNum_, getStatsTdb().statsMergeType_,
                                                    statsArea_);
      ...
      case GET_PROCESS_STATS_ENTRY_: {
        processStats_ =
            (ExProcessStats *)((ExStatisticsArea *)statsArea_)->get(ExOperStats::PROCESS_STATS, _UNINITIALIZED_TDB_ID);
      ...
      case FORMAT_AND_RETURN_PROCESS_STATS_: {
        if ((qparent_.up->getSize() - qparent_.up->getLength()) < 30) return -1;
        sprintf(outBuf, "Node Id:                      %d", processStats_->getNid());
        moveRowToUpQueue(outBuf);

        sprintf(outBuf, "PID                           %d", processStats_->getPid());
        moveRowToUpQueue(outBuf);

        if (processStats_->getStartTime() == -1)
          strcpy(outBuf, "Start Time                    -1");
        else {
          juliantimestamp = CONVERTTIMESTAMP(processStats_->getStartTime(), 0, -1, 0);

          INTERPRETTIMESTAMP(juliantimestamp, timestamp);
          sprintf(outBuf, "Start Time                    %04d/%02d/%02d %02d:%02d:%02d.%03u%03u", timestamp[0],
                  timestamp[1], timestamp[2], timestamp[3], timestamp[4], timestamp[5], timestamp[6], timestamp[7]);
        }
        moveRowToUpQueue(outBuf);

        sprintf(outBuf, "EXE Memory Allocated          %ld MB", (processStats_->getExeMemAlloc() >> 20));
        moveRowToUpQueue(outBuf);

        sprintf(outBuf, "EXE Memory Used High WM       %ld MB", (processStats_->getExeMemHighWM() >> 20));
        moveRowToUpQueue(outBuf);

        sprintf(outBuf, "EXE Memory Used               %ld MB", (processStats_->getExeMemUsed() >> 20));
        moveRowToUpQueue(outBuf);

        sprintf(outBuf, "IPC Memory Allocated          %ld MB", (processStats_->getIpcMemAlloc() >> 20));
        moveRowToUpQueue(outBuf);

        sprintf(outBuf, "IPC Memory Used High WM       %ld MB", (processStats_->getIpcMemHighWM() >> 20));
        moveRowToUpQueue(outBuf);

        sprintf(outBuf, "IPC Memory Used               %ld MB", (processStats_->getIpcMemUsed() >> 20));
        moveRowToUpQueue(outBuf);

        sprintf(outBuf, "Static Stmt Count             %d", processStats_->getStaticStmtCount());
        moveRowToUpQueue(outBuf);

        sprintf(outBuf, "Dynamic Stmt Count            %d", processStats_->getDynamicStmtCount());
        moveRowToUpQueue(outBuf);

        sprintf(outBuf, "Open Stmt Count               %d", processStats_->getOpenStmtCount());
        moveRowToUpQueue(outBuf);

        sprintf(outBuf, "Reclaimable Stmt Count        %d", processStats_->getCloseStmtCount());
        moveRowToUpQueue(outBuf);

        sprintf(outBuf, "Reclaimed Stmt Count          %d", processStats_->getReclaimStmtCount());
        moveRowToUpQueue(outBuf);

        sprintf(outBuf, "Total ESPs Started            %d", processStats_->getNumESPsStarted());
        moveRowToUpQueue(outBuf);

        sprintf(outBuf, "Total ESPs Startup Completed  %d", processStats_->getNumESPsStartupCompleted());
        moveRowToUpQueue(outBuf);

        sprintf(outBuf, "Total ESPs Error in Startup   %d", processStats_->getNumESPsBad());
        moveRowToUpQueue(outBuf);

        sprintf(outBuf, "Total ESPs Deleted            %d", processStats_->getNumESPsDeleted());
        moveRowToUpQueue(outBuf);

        sprintf(outBuf, "Num ESPs In Use               %d", processStats_->getNumESPsInUse());
        moveRowToUpQueue(outBuf);

        sprintf(outBuf, "Num ESPs Free                 %d", processStats_->getNumESPsFree());
        moveRowToUpQueue(outBuf);

        sprintf(outBuf, "Recent Qid                    %s",
                (processStats_->getRecentQid() ? processStats_->getRecentQid() : "NULL"));
        moveRowToUpQueue(outBuf);
...
ex_tcb *ExExeUtilGetObjectEpochStatsTdb::build(ex_globals *glob) {
  ex_tcb *tcb = new (glob->getSpace()) ExExeUtilGetObjectEpochStatsTcb(*this, glob);
...
////////////////////////////////////////////////////////////////
// Constructor for class ExExeUtilGetObjectEpochStatsTcb
///////////////////////////////////////////////////////////////
ExExeUtilGetObjectEpochStatsTcb::ExExeUtilGetObjectEpochStatsTcb(const ComTdbExeUtilGetObjectEpochStats &exe_util_tdb,
                                                                 ex_globals *glob)
    : ExExeUtilTcb(exe_util_tdb, NULL, glob) {
  // Allocate the private state in each entry of the down queue
  qparent_.down->allocatePstate(this);
...
short ExExeUtilGetObjectEpochStatsTcb::work() {
  ...
  // If no room in up queue, won't be able to return data/status. Come
  // back later.
  ...
  ExObjectEpochStats *epochStats = NULL;

  while (true) {
    switch (step_) {
      case INITIAL_: {
        cliRC = SQL_EXEC_GetObjectEpochStats_Internal(
            getObjectEpochStatsTdb().getObjectName(), getObjectEpochStatsTdb().getObjectNameLen(),
            getObjectEpochStatsTdb().getCpu(), getObjectEpochStatsTdb().getLocked(), stats_);
        if (cliRC < 0) {
          step_ = HANDLE_ERROR_;
          break;
        }

        // make sure there is enough space to move the header
        if ((qparent_.up->getSize() - qparent_.up->getLength()) < 10) return WORK_CALL_AGAIN;  // come back later

        moveRowToUpQueue(" ");
        moveRowToUpQueue("Object Epoch Cache");
        moveRowToUpQueue("==================");
        moveRowToUpQueue(" ");
        snprintf(outBuf, sizeof(outBuf), "Object Epoch Entries:         %d", stats_->numEntries());
        moveRowToUpQueue(outBuf);
        moveRowToUpQueue(" ");

        stats_->position();
        step_ = RETURN_RESULT_;
        break;
      }
      case RETURN_RESULT_: {
        operStats = stats_->getNext();
        if (operStats == NULL) {
          step_ = DONE_;
          break;
        }
        // make sure there is enough space to move the result
        if ((qparent_.up->getSize() - qparent_.up->getLength()) < 10) return WORK_CALL_AGAIN;  // come back later

        epochStats = operStats->castToExObjectEpochStats();
        ex_assert(epochStats != NULL, "Object other than ObjectEpochStats found");

        moveRowToUpQueue(" ");
        snprintf(outBuf, sizeof(outBuf), "Node Id:                      %d", epochStats->getCpu());
        moveRowToUpQueue(outBuf);
        snprintf(outBuf, sizeof(outBuf), "Object Name:                  %s", epochStats->getObjectName());
        moveRowToUpQueue(outBuf);
        snprintf(outBuf, sizeof(outBuf), "Object Type:                  %s",
                 comObjectTypeName(epochStats->getObjectType()));
        moveRowToUpQueue(outBuf);
        snprintf(outBuf, sizeof(outBuf), "Hash:                         %u", epochStats->getHash());
        moveRowToUpQueue(outBuf);
        snprintf(outBuf, sizeof(outBuf), "Redefine Time:                %ld", epochStats->getRedefTime());
        moveRowToUpQueue(outBuf);
        snprintf(outBuf, sizeof(outBuf), "Epoch:                        %d", epochStats->getEpoch());
        moveRowToUpQueue(outBuf);
        snprintf(outBuf, sizeof(outBuf), "Flags:                        %u %s", epochStats->getFlags(),
                 epochStats->getFlagsString());
        moveRowToUpQueue(outBuf);
        moveRowToUpQueue(" ");
        break;
      }
      ...
      case DONE_: {
        if (qparent_.up->isFull()) {
          // NOTE2ME: shouldn't we use WORK_CALL_AGAIN?
          //
      ...
      default: {
        ex_assert(0, "ExExeUtilGetObjectEpochStatsTcb step not yet supported");
        break;
      }
    }
  }
...
ex_tcb *ExExeUtilGetObjectLockStatsTdb::build(ex_globals *glob) {
  ex_tcb *tcb = new (glob->getSpace()) ExExeUtilGetObjectLockStatsTcb(*this, glob);
  tcb->registerSubtasks();
  return tcb;
}
...
////////////////////////////////////////////////////////////////
// Constructor for class ExExeUtilGetObjectLockStatsTcb
///////////////////////////////////////////////////////////////
ExExeUtilGetObjectLockStatsTcb::ExExeUtilGetObjectLockStatsTcb(const ComTdbExeUtilGetObjectLockStats &exe_util_tdb,
                                                               ex_globals *glob)
    : ExExeUtilTcb(exe_util_tdb, NULL, glob), distinctStats_(NULL) {
  // Allocate the private state in each entry of the down queue
  qparent_.down->allocatePstate(this);
  step_ = INITIAL_;
}

ExExeUtilGetObjectLockStatsTcb::~ExExeUtilGetObjectLockStatsTcb() {
  if (distinctStats_) {
    NADELETE(distinctStats_, ExStatisticsArea, distinctStats_->getHeap());
  }
}

ExStatisticsArea *ExExeUtilGetObjectLockStatsTcb::distinctLockStats() {
  NAMemory *heap = stats_->getHeap();
  ExStatisticsArea *statsArea =
      new (heap) ExStatisticsArea(heap, 0, ComTdb::OBJECT_LOCK_STATS, ComTdb::OBJECT_LOCK_STATS);
  ExOperStats *stat1 = NULL;
  ExOperStats *stat2 = NULL;
  ExObjectLockStats *lockStat1 = NULL;
  ...
  while (stat1 = stats_->getNext()) {
    ...
    while (stat2 = statsArea->getNext()) {
      ...
      ex_assert(lockStat1 != NULL && lockStat1 != NULL, "Object other than ObjectLockStats found");
      if (lockStat1->isSameLock(*lockStat2)) {
        found = true;
        break;
      }
    }
    if (!found) {
      lockStat1 = stat1->castToExObjectLockStats();
      ExOperStats *newStat = new (heap) ExObjectLockStats(
          heap, lockStat1->getCpu(), lockStat1->getObjectName(), lockStat1->getObjectType(), lockStat1->getPid(),
          lockStat1->getHolderNid(), lockStat1->getHolderPid(), lockStat1->getDMLLocked(), lockStat1->getDDLLocked());
      statsArea->insert(newStat);
    }
  }
  return statsArea;
}
...
short ExExeUtilGetObjectLockStatsTcb::work() {
  ...
  char outBuf[2000];
  ExOperStats *operStats = NULL;
  ExObjectLockStats *lockStats = NULL;
  ExStatisticsArea *stats = NULL;
  if (getObjectLockStatsTdb().isEntry()) {
    stats = stats_;
  } else {
    stats = distinctStats_;
  }

  while (true) {
    switch (step_) {
      case INITIAL_: {
        cliRC = SQL_EXEC_GetObjectLockStats_Internal(getObjectLockStatsTdb().getObjectName(),
                                                     getObjectLockStatsTdb().getObjectNameLen(),
                                                     getObjectLockStatsTdb().getCpu(), stats_);
        if (cliRC < 0) {
          step_ = HANDLE_ERROR_;
          break;
        }
        ...
        if (getObjectLockStatsTdb().isEntry()) {
          ...
          stats = distinctStats_ = distinctLockStats();
        }
        ...
        moveRowToUpQueue(" ");
        if (getObjectLockStatsTdb().isEntry()) {
          moveRowToUpQueue("Object Lock Entry Stats");
        } else {
          moveRowToUpQueue("Object Lock Stats");
        }
        moveRowToUpQueue("==================");
        moveRowToUpQueue(" ");
        if (getObjectLockStatsTdb().isEntry()) {
          snprintf(outBuf, sizeof(outBuf), "Object Lock Entries:          %d", stats->numEntries());
        } else {
          snprintf(outBuf, sizeof(outBuf), "Object Locks:                 %d", stats->numEntries());
        }
        moveRowToUpQueue(outBuf);
        moveRowToUpQueue(" ");
      ...
      case RETURN_RESULT_: {
        ...
        operStats = stats->getNext();
        if (operStats == NULL) {
          step_ = DONE_;
          break;
        }

        lockStats = operStats->castToExObjectLockStats();
        ex_assert(lockStats != NULL, "Object other than ObjectLockStats found");

        moveRowToUpQueue(" ");
        if (getObjectLockStatsTdb().isEntry()) {
          snprintf(outBuf, sizeof(outBuf), "Node Id:                      %d", lockStats->getCpu());
          moveRowToUpQueue(outBuf);
          snprintf(outBuf, sizeof(outBuf), "Process ID:                   %u", lockStats->getPid());
          moveRowToUpQueue(outBuf);
        }
        snprintf(outBuf, sizeof(outBuf), "Object Name:                  %s", lockStats->getObjectName());
        moveRowToUpQueue(outBuf);
        snprintf(outBuf, sizeof(outBuf), "Object Type:                  %s",
                 comObjectTypeName(lockStats->getObjectType()));
        moveRowToUpQueue(outBuf);
        snprintf(outBuf, sizeof(outBuf), "Lock Type:                    %s", lockStats->getLockString());
        moveRowToUpQueue(outBuf);
        snprintf(outBuf, sizeof(outBuf), "Locked By:                    %d %d", lockStats->getHolderNid(),
                 lockStats->getHolderPid());
        moveRowToUpQueue(outBuf);
        moveRowToUpQueue(" ");
        break;
      }
      ...
      default: {
        ex_assert(0, "ExExeUtilGetObjectLockStatsTcb step not yet supported");
        break;
      }
    }
  }
