...
HashTableCursor::HashTableCursor() { init(); }
HashTableCursor::~HashTableCursor() {}

HashTableHeader::HashTableHeader() { init(); }
...
HashTable::HashTable(int headerCount, NABoolean evenFactor, int primeFactor, NABoolean noHVDups,
  ...
  while (!header_) {
    ...
    if (evenFactor && 0 == headerCount_ % 2) headerCount_++;  // the count is even no more
    if (0 == headerCount_ % primeFactor) headerCount_ += 2;   // break the common factor, since primeFactor > 2

    header_ = (HashTableHeader *)collHeap()->allocateMemory((UInt32)(headerCount_ * sizeof(HashTableHeader)), FALSE);
  }

  if (doResize) resizeThreshold_ = (headerCount_ / 100) * HT_LOAD_FACTOR_PERCENT;
...
//////////////////////////////////////////////////////////////////////
void HashTable::insert(atp_struct *workAtp, HashRow *newRow, tupp &workAtpTupp1, tupp &workAtpTupp2,
                       ex_expr *searchExpr) {
  ex_assert(searchExpr, "This insert() should only be used for searchable entries. ");
  ...
  while (row != NULL) {
    if (newRowHV == row->hashValue()) {
      ...
      if (searchExpr->eval(workAtp, workAtp) == ex_expr::EXPR_TRUE)
        ...
        snprintf(msg, sizeof(msg), "Encountered two rows with different values but same hash values %d", newRowHV);
        // log an EMS event and continue
        SQLMXLoggingArea::logExecRtDebug(__FILE__, __LINE__, msg, 0);
        ...
        if ((chainIdx + 1) % headerCount_ == origChainIdx) {
          ...
          char msg[256];
          snprintf(msg, sizeof(msg), "Hash table of size %d is full, allow duplicates", headerCount_);
          // log an EMS event and continue
    ...
    if ((++numRowsScanned % logTrigger) == 0) {
      char msg[256];
      snprintf(msg, sizeof(msg), "Traversed %ld chains in hash table insert %d, hash value %d", numRowsScanned,
               origChainIdx, newRowHV);
      // log an EMS event and continue
      SQLMXLoggingArea::logExecRtDebug(__FILE__, __LINE__, msg, 0);
      // increase the distance between such messages exponentially
  ...
  if (cursor->currRow_ != cursor->endRow_) {
    ...
    if (cursor->currRow_ == NULL) {
      ...
      while ((cursor->currHeader_ < headerCount_) &&
        ...
        cursor->currHeader_++;
      if (cursor->currHeader_ < headerCount_) cursor->currRow_ = header_[cursor->currHeader_].row_;
    }
  } else
...
// calling getNext().
/////////////////////////////////////////////////////////////////////
void HashTable::position(HashTableCursor *cursor, atp_struct *rowAtp, atp_struct *workAtp, short hashTableRowAtpIndex,
                         ex_expr *searchExpr, SimpleHashValue hashValue, NABoolean noDupChaining,
                         NABoolean returnOrdered) {
  ex_assert(searchExpr, "This position() should only be used for searchable entries. ");
  ...
  long numRowsTraversed = 0;
  long logTrigger = 1000;
  ...
  // this loop.
  for (HashRow *row = header_[cursor->currHeader_].row_; row != NULL; row = row->next()) {
    numRowsTraversed++;

    // If the simple hash doesn't match, then skip to the next row.
    if (hashValue != row->hashValue()) continue;
    ...
    if (searchExpr->eval(rowAtp, workAtp) == ex_expr::EXPR_TRUE) {
      ...
      // 10-080320-1596.
      if (noDupChaining) return;

      // Start searching on the next row for the last matching row
      for (row = row->next(); row != NULL; row = row->next()) {
        if (row->hashValue() != hashValue) break;  // different HV, search no more (key-value must be different)
      ...
      if (numRowsTraversed >= logTrigger) {
        char msg[256];
        snprintf(msg, sizeof(msg), "Found %d matching rows after traversing %ld rows in chain %d for hash value %d",
                 matchCount, numRowsTraversed, cursor->currHeader_, hashValue);
        // log an EMS event and continue
        SQLMXLoggingArea::logExecRtDebug(__FILE__, __LINE__, msg, 0);
      }
  ...
  }  // search one or more hash chains for matching rows

  // At this point the cursor is pointing to the sub-chain of matched rows
  if (numRowsTraversed >= logTrigger) {
    char msg[256];
    snprintf(msg, sizeof(msg), "Did not find any matching rows after traversing %ld rows in chain %d for hash value %d",
             numRowsTraversed, cursor->currHeader_, hashValue);
    // log an EMS event and continue
    SQLMXLoggingArea::logExecRtDebug(__FILE__, __LINE__, msg, 0);
  }
...
//
ex_expr::exp_return_type HashTable::positionUniq(HashRow **currRow, atp_struct *leftRowAtp, atp_struct *workAtp,
                                                 short rightRowAtpIndex, ex_expr *searchExpr,
...
void HashTable::position(HashTableCursor *cursor) {
  ...
  if (i == headerCount_)
    ...
    while (!(header_[i].row_)) i--;

    cursor->endRow_ = header_[i].row_;
    while (cursor->endRow_->next_) cursor->endRow_ = cursor->endRow_->next_;
  }
