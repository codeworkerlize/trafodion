...
void *ExSM_ReaderFunction(void *arg) {
  ...
  EXSM_TRACE(EXSM_TRACE_RDR_THR | EXSM_TRACE_SM_CALLS, "%s BEGIN READER (%s %d)", readerPrefix,
  ...
  while (glob->getReaderThreadState() == ExSMGlobals::STARTED) {
    ...
    if ((tLevel != glob->getTraceLevel()) || (strcmp(tPrefix, glob->getTraceFilePrefix()))) {
    ...
    EXSM_TRACE(EXSM_TRACE_RDR_THR | EXSM_TRACE_WAIT, "%s LOOP %ld", readerPrefix, (long)loopCount);
    ...
    EXSM_TRACE(EXSM_TRACE_RDR_THR | EXSM_TRACE_WAIT | EXSM_TRACE_SM_CALLS, "%s  wait rc %d n %d h %p", readerPrefix,
    ...
    // assert.
    if (rc != SM_ERR_INCHUNK) exsm_assert_rc(rc, "SM_get");

    // Go back to the top of the main loop if no chunks were returned
    if (chunks == NULL || nchunks < 1) {
      retryCount++;
      EXSM_TRACE(EXSM_TRACE_RDR_THR | EXSM_TRACE_WAIT, "%s  wait will be retried, count %d", readerPrefix,
                 (int)retryCount);
      continue;
    }
    ...
    // Lock the task list
    smTaskList->lock();
    EXSM_TRACE(EXSM_TRACE_RDR_THR | EXSM_TRACE_WAIT, "%s   locked task list, tasks %d", readerPrefix,
    ...
    for (int32_t i = 0; i < nchunks; i++) {
      ...
      if ((chunk.flags & SM_FLAG_PREPOST) == 0) {
        ...
        if ((chunk.buff && chunk.size >= 4)) memcpy(&firstFourBytes, chunk.buff, 4);
        fourthArg = (int64_t)firstFourBytes;
      }

      ExSMEvent *event =
          ExSMEvent::add(ExSMEvent::Chunk, &chunk.tgt, chunk.errcode, chunk.size, chunk.flags, fourthArg);
      ...
      EXSM_TRACE(EXSM_TRACE_RDR_THR | EXSM_TRACE_WAIT | EXSM_TRACE_SM_CALLS,
                 "%s   %s %d src %d:%d:%" PRId64 ":%d:0x%c fl 0x%x", readerPrefix, (isRequest ? "REQUEST" : "REPLY"),
                 (int)count, (int)chunk.tgt.node, (int)chunk.tgt.pid, chunk.tgt.id,
                 (int)ExSMTag_GetTagWithoutQualifier(chunk.tgt.tag), (char)ExSMTag_GetQualifierDisplay(chunk.tgt.tag),
                 (int)chunk.flags);

      char buf[30];
      SM_strerror_r(chunk.errcode, buf, sizeof(buf));

      EXSM_TRACE(EXSM_TRACE_RDR_THR | EXSM_TRACE_WAIT | EXSM_TRACE_SM_CALLS, "%s   sz %d err %s h %p buf %p",
                 readerPrefix, (int)chunk.size, buf, chunk.handle, chunk.buff);

      // Add chunk size to the trace file if this is one fragment of a
      // SQL data buffer
      if (ExSMTag_GetSQLChunkFlag(chunk.tgt.tag))
        EXSM_TRACE(EXSM_TRACE_RDR_THR | EXSM_TRACE_BUFFER, "%s   RECV CHUNK %d", readerPrefix, (int)chunk.size);
      ...
      if (chunk.errcode != 0) {
        ...
        if (chunk.errcode == SM_ERR_LOCAL_TARGET) {
          ...
          if (!found) {
            // Case (a)
            ExSMEvent::add(ExSMEvent::IDNotActive, &chunk.tgt, chunk.errcode, chunk.flags);
            EXSM_TRACE(EXSM_TRACE_RDR_THR | EXSM_TRACE_SM_CALLS,
                       "ID is not active, LOCAL_TARGET error will be ignored");
            continue;
          }
        } else if (chunk.errcode == SM_ERR_NODE_DOWN || (chunk.errcode == SM_ERR_PREPOST) ||
      ...
      if (chunk.flags & SM_FLAG_SENDNOTIFICATION) {
        ...
        if (event) event->setOptional5((int64_t)chunk.handle);

        void *bufferAddr = ExSM_GetMessageBufferAddr(chunk.handle);
        ExSM_AddCompletedSendBuffer(bufferAddr);

        EXSM_TRACE(EXSM_TRACE_RDR_THR | EXSM_TRACE_SM_CALLS, "COMPLETED SEND data %p msg %p\n", chunk.buff,
                   chunk.handle);
      ...
      if ((chunk.flags & SM_FLAG_PREPOST) == 0) {
        ...
        if (EXSM_TRACE_ENABLED)
          shortMsg.writeToTrace(EXSM_TRACE_RDR_THR | EXSM_TRACE_WAIT | EXSM_TRACE_SM_CALLS, readerPrefix, "   ");
        ...
        if (shortMsgType == ExSMShortMessage::SHUTDOWN) {
          ...
          EXSM_TRACE(EXSM_TRACE_RDR_THR, "%s   Fixup reply count %d", readerPrefix, (int)glob->getFixupReplyCount());
          continue;
        }
      } else {
      ...
      bool doLock = false;
      bool doTrace = false;
      ExSMTask *task = smTaskList->findTask(targetForTaskLookup, doLock, doTrace);
      if (task == NULL) {
        EXSM_TRACE(EXSM_TRACE_RDR_THR | EXSM_TRACE_WAIT, "%s *** ERROR: SM TASK NOT FOUND", readerPrefix);

        // Perform the lookup again with tracing enabled. This lookup
        // is expected to fail just like the first lookup failed.
        doTrace = true;
        task = smTaskList->findTask(targetForTaskLookup, doLock, doTrace);

        exsm_assert(task == NULL, "Task lookup should not succeed after first lookup failed");
        ...
        if (!found) {
          ExSMEvent::add(ExSMEvent::IDNotActive, &chunk.tgt, chunk.errcode, chunk.flags);
          EXSM_TRACE(EXSM_TRACE_RDR_THR | EXSM_TRACE_SM_CALLS, "ID is not active, chunk will be ignored");
          continue;
        }
      ...
      if ((chunk.flags & SM_FLAG_PREPOST) != 0) {
        ...
        // trace file.
        EXSM_TRACE(EXSM_TRACE_RDR_THR | EXSM_TRACE_WAIT | EXSM_TRACE_SM_CALLS, "%s   msg type %s", readerPrefix,
                   ExSM_GetMessageBufferType(chunk.buff, chunk.size));
      }

      task->incrReceiveCount();

      EXSM_TRACE(EXSM_TRACE_RDR_THR | EXSM_TRACE_WAIT, "%s   tcb %p recv cnt %d", readerPrefix, task->getTCB(),
                 (int)task->getReceiveCount());
      ...
      if ((chunk.flags & SM_FLAG_PREPOST) == 0) {
        ...
        if (shortMsgType == ExSMShortMessage::SIZE) {
          ...
          void *largeBuffer = ExSM_AllocateMessageBuffer(msgSize, glob->getThreadSafeHeap());
          ...
          if (rc == SM_ERR_NOSERVICE || rc == SM_ERR_NOPEER || rc == SM_ERR_NODE_DOWN) {
            glob->handleReaderThreadError(rc, "ExSM_Post", dataHandle, smTaskList);
            return 0;
          }

          EXSM_TRACE(EXSM_TRACE_BUFFER | EXSM_TRACE_SM_CALLS, "PREPOST %d:%d:%" PRId64 ":%d:0x%c %p sz %d",
                     (int)target.node, (int)target.pid, target.id, (int)ExSMTag_GetTagWithoutQualifier(target.tag),
                     (char)ExSMTag_GetQualifierDisplay(target.tag), chunkAddr, (int)chunkSize);
          ...
          int32_t rc2 = m.send();
          if (rc2 == SM_ERR_NOSERVICE || rc2 == SM_ERR_NOPEER || rc2 == SM_ERR_NODE_DOWN) {
        ...
        // ACK
          ...
          EXSM_TRACE(EXSM_TRACE_RDR_THR | EXSM_TRACE_WAIT, "%s   ACK arrived, task %p", readerPrefix, task);
        }

        // UNKNOWN SHORT MESSAGE TYPE
        else {
          char buf[64];
          snprintf(buf, sizeof(buf), "Invalid short message type %d", (int)shortMsgType);
          exsm_assert(FALSE, buf);
        }
      ...
      if (receivedSQLBufferFragment) {
        ...
        exsm_assert(dataPointer, "Task queue entry contains a NULL buffer pointer");
      ...
      EXSM_TRACE(EXSM_TRACE_RDR_THR | EXSM_TRACE_WAIT, "%s   prepost %p, arrival %p", readerPrefix, expectedAddr,
                 actualAddr);
      exsm_assert(expectedAddr == actualAddr, "Expected address does not match actual");
      ...
      if (receivedSQLBufferFragment) {
        ...
        if (task->recvChunk_MoreExpected()) {
          ...
          EXSM_TRACE(EXSM_TRACE_BUFFER | EXSM_TRACE_SM_CALLS, "PREPOST %d:%d:%" PRId64 ":%d:0x%c %p", (int)target.node,
                     (int)target.pid, target.id, (int)ExSMTag_GetTagWithoutQualifier(target.tag),
                     (char)ExSMTag_GetQualifierDisplay(target.tag), chunkAddr, (int)chunkSize);
          ...
          exsm_assert(bufferForOutputQueue, "Task returned a NULL buffer pointer");
        ...
        exsm_assert(bufferForOutputQueue, "bufferForOutputQueue was not initialized");
      ...
      if (bufferForOutputQueue) {
        ...
        exsm_assert(scheduledAddr, "SM task scheduledAddr pointer is NULL");
        *scheduledAddr = 1;
        EXSM_TRACE(EXSM_TRACE_RDR_THR | EXSM_TRACE_WAIT, "%s   scheduled task %p", readerPrefix, task);
    ...
    smTaskList->unlock();
    EXSM_TRACE(EXSM_TRACE_RDR_THR | EXSM_TRACE_WAIT, "%s   unlocked task list", readerPrefix);
    ...
    EXSM_TRACE(EXSM_TRACE_RDR_THR | EXSM_TRACE_WAIT, "%s  SM_get_done rc %d", readerPrefix, (int)rc);
    ...
    if (glob->getReaderThreadState() != ExSMGlobals::PROCESSING_SHUTDOWN) {
      ...
      EXSM_TRACE(EXSM_TRACE_RDR_THR | EXSM_TRACE_WAIT, "%s  generated LRABBIT", readerPrefix);
    }
  ...
  EXSM_TRACE(EXSM_TRACE_RDR_THR, "%s Reader thread shutting down", readerPrefix);
