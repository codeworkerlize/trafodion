...
ESPTraceEntry::ESPTraceEntry(ex_globals *glob, char *t1) : globals_(glob) {
...
void ESPTraceEntry::createMessage(char *message) {
  ...
  sprintf(message, "%s PID: %d ESP#: %d %s ", timeBuf, pid_, espNum_, msgtext1_);
}

ESPTraceList::~ESPTraceList() { clearAndDestroy(); }
...
void ESPTraceList::insertNewTraceEntry(char *msg) {
  ...
  if (NOT espGlob->isAnESPAccess()) return;
...
ExExeStmtGlobals::ExExeStmtGlobals(short num_temps, CliGlobals *cliGlobals, short create_gui_sched, Space *space,
                                   CollHeap *heap)
  ...
  if (cliGlobals) setEventConsumed(cliGlobals->getEventConsumed());
  diagsArea_ = NULL;
...
void ExExeStmtGlobals::deleteMe(NABoolean fatalError) {
  while (!fatalError && anyCancelMsgesOut()) {
    ...
    if (getSMQueryID() > 0)
      EXSM_TRACE(EXSM_TRACE_CANCEL, "ExExeStmtGlobals::deleteMe: outstanding %d - waiting", (int)numCancelMsgesOut());
  ...
  if (diagsArea_) {
    ...
    if (diagsArea_->decrRefCount() == 0) diagsArea_ = NULL;
  }
...
ExExeStmtGlobals *ExExeStmtGlobals::castToExExeStmtGlobals() { return this; }

ExMasterStmtGlobals *ExExeStmtGlobals::castToExMasterStmtGlobals() { return NULL; }

ExEspStmtGlobals *ExExeStmtGlobals::castToExEspStmtGlobals() { return NULL; }

ComDiagsArea *ExExeStmtGlobals::getAllocatedDiagsArea() {
...
// instance.
ExUdrServer *ExExeStmtGlobals::acquireUdrServer(const char *runtimeOptions, const char *optionDelimiters,
                                                NABoolean dedicated) {
#ifdef UDR_DEBUG
  NABoolean doDebug = (getenv("UDR_SERVER_MGR_DEBUG") || getenv("UDR_DEBUG")) ? TRUE : FALSE;
  FILE *f = stdout;
  if (doDebug) {
  ...
  ex_assert(numUdrMsgsOut() == 0, "Cannot acquire a new UDR server while messages are outstanding");

  ContextCli *context = getContext();
  ExUdrServer *udrServ = context->acquireUdrServer(runtimeOptions, optionDelimiters, getThreadInfo(), dedicated);
...
void ExExeStmtGlobals::decrementSendTopMsgesOut() {
  ex_assert(numSendTopMsgesOut_ > 0, "Send top message counter should not drop below zero");
  numSendTopMsgesOut_--;
}

void ExExeStmtGlobals::decrementCancelMsgesOut() {
  ex_assert(numCancelMsgesOut_ > 0, "Cancel message counter should not drop below zero");
  numCancelMsgesOut_--;
}
...
void ExExeStmtGlobals::cancelOperation(long transId) {
  ...
  HBaseClient_JNI *hbaseClient = cliGlobals_->getHBaseClient();

  if (hbaseClient != NULL) {
    hbaseClient->cancelOperation(transId);
  }
}
...
ExMasterStmtGlobals::ExMasterStmtGlobals(short num_temps, CliGlobals *cliGlobals, Statement *statement,
                                         short create_gui_sched, Space *space, CollHeap *heap)
...
void ExMasterStmtGlobals::deleteMe(NABoolean fatalError) {
  ...
  if (extractInfo_) {
    ...
    if (extractInfo_->securityKey_) h->deallocateMemory(extractInfo_->securityKey_);
    ...
    if (extractInfo_->esps_ != NULL) {
      ...
      for (i = 0; i < lastEntry; i++) {
        if (espList->used(i)) {
          ...
          if (esp) {
            if (esp->phandleText_) h->deallocateMemory(esp->phandleText_);
...
ExMasterStmtGlobals *ExMasterStmtGlobals::castToExMasterStmtGlobals() { return this; }

char *ExMasterStmtGlobals::getFragmentPtr(ExFragId fragId) const {
  ex_assert(getFragDir() AND getStartAddr(), "Frag dir and starting address must be set first");
  return (char *)((char *)getStartAddr() + getFragDir()->getGlobalOffset(fragId));
}

IpcMessageObjSize ExMasterStmtGlobals::getFragmentLength(ExFragId fragId) const {
...
CancelState ExMasterStmtGlobals::setCancelState(CancelState newState) {
  ...
  if (old == CLI_CANCEL_DISABLE) return old;
  ...
  switch (newState) {
    ...
    case CLI_CANCEL_TCB_INVALID:
      ...
      // ---------------------------------------------------------
      if (old == CLI_CANCEL_TCB_INVALID) return old;
...
void ExMasterStmtGlobals::resetCancelState() {
  if ((cancelState_ == CLI_CANCEL_TCB_INVALID || cancelState_ == CLI_CANCEL_DISABLE)) return;

  getContext()->semaphoreLock();
  if (cancelState_ != CLI_CANCEL_REQUESTED) cancelState_ = CLI_CANCEL_TCB_INVALID;
  getContext()->semaphoreRelease();
}
...
void ExMasterStmtGlobals::setLocalTimeoutData(ComTdbRoot *rootTdb) {
  ...
  localSnapshotOfTimeoutChangeCounter_ = getContext()->getTimeoutChangeCounter();
...
NABoolean ExMasterStmtGlobals::timeoutSettingChanged() {
  ...
  if (localSnapshotOfTimeoutChangeCounter_ == getContext()->getTimeoutChangeCounter())
...
NABoolean ExMasterStmtGlobals::udrRuntimeOptionsChanged() const {
  ...
  if (udrServer) {
    ...
    if (newOptions) {
      ...
      // below is safe.
      if (str_cmp_ne(oldOptions, newOptions) != 0 || str_cmp_ne(oldDelims, newDelims) != 0) {
        changed = TRUE;
      }
...
ExRsInfo *ExMasterStmtGlobals::getResultSetInfo(NABoolean createIfNecessary) {
  if (createIfNecessary && resultSetInfo_ == NULL) resultSetInfo_ = new (getDefaultHeap()) ExRsInfo();
                                                  ...
                                                  ExRsInfo *&rsInfo)        // OUT
{
  ...
  ex_assert(otherGlobals->getUdrConnection(), "No connection to UDR server is available in parent");
...
void ExMasterStmtGlobals::insertExtractEsp(const IpcProcessId &pid) {
  ...
  if (extractInfo_ == NULL) {
    extractInfo_ = (ExExtractProducerInfo *)h->allocateMemory(sizeof(ExExtractProducerInfo));
    memset(extractInfo_, 0, sizeof(ExExtractProducerInfo));
  }

  if (extractInfo_->esps_ == NULL) extractInfo_->esps_ = new (h) ARRAY(ExExtractEspInfo *)(h);

  ExExtractEspInfo *esp = (ExExtractEspInfo *)h->allocateMemory(sizeof(ExExtractEspInfo));
...
const char *ExMasterStmtGlobals::getExtractEspPhandleText(int index) const {
  ...
  if (extractInfo_ && extractInfo_->esps_) {
    if (extractInfo_->esps_->used(index)) {
      ExExtractEspInfo *esp = extractInfo_->esps_->at(index);
      if (esp) result = esp->phandleText_;
    }
  }
  return result;
}
...
const char *ExMasterStmtGlobals::getExtractSecurityKey() const {
  const char *result = NULL;
  if (extractInfo_) result = extractInfo_->securityKey_;
  return result;
}
...
const char *ExMasterStmtGlobals::getSMTraceFilePrefix() const {
  const char *result = NULL;
  if (smQueryID_ > 0) result = getContext()->getSessionDefaults()->getExSMTraceFilePrefix();
  return result;
}
...
ExEspStmtGlobals::ExEspStmtGlobals(short num_temps, CliGlobals *cliGlobals, short create_gui_sched, Space *space,
  ...
  cachedMyInstanceNum_ = -1;
  cachedMyFragId_ = UINT_MAX;
}
...
void ExEspStmtGlobals::deleteMe(NABoolean fatalError) {
  ...
  if (statsGlobals != NULL) {
    int error = statsGlobals->getStatsSemaphore(espFragInstanceDir_->getSemId(), espFragInstanceDir_->getPid());
    if (stmtStats_ != NULL) statsGlobals->removeQuery(espFragInstanceDir_->getPid(), stmtStats_);
    statsGlobals->releaseStatsSemaphore(espFragInstanceDir_->getSemId(), espFragInstanceDir_->getPid());
    stmtStats_ = NULL;
  }
  ExExeStmtGlobals::deleteMe(fatalError);
}

ExEspStmtGlobals *ExEspStmtGlobals::castToExEspStmtGlobals() { return this; }

char *ExEspStmtGlobals::getFragmentPtr(ExFragId fragId) const {
  int handle = espFragInstanceDir_->findHandle(getFragmentKey(fragId));
...
ExFragId ExEspStmtGlobals::getMyFragId() const {
  if (cachedMyFragId_ == UINT_MAX) cachedMyFragId_ = espFragInstanceDir_->findKey(myHandle_).getFragId();
...
int ExEspStmtGlobals::getMyInstanceNumber() const {
  ...
  if (cachedMyInstanceNum_ >= 0) return cachedMyInstanceNum_;
  ...
  IpcProcessId myProcId(getIpcEnvironment()->getMyOwnProcessId(getInstanceProcessId(myFragId, 0).getDomain()));
  ...
  for (int i = 0;
    ...
    if (myProcId == getInstanceProcessId(myFragId, i))
      // myInstanceNum = i;
      cachedMyInstanceNum_ = i;
  }

  // make sure we found it
  ex_assert(
      // myInstanceNum != -1,
      cachedMyInstanceNum_ >= 0, "couldn't determine my own instance number");
...
void ExEspStmtGlobals::getMyNodeLocalInstanceNumber(int &myNodeLocalInstanceNumber, int &numOfLocalInstances) const {
  ...
  for (int i = 0; i < numInstances; i++) {
    ...
    else if (pid.getNodeName() == myNodeName AND pid.getCpuNum() == myCpuNum) {
      numOfLocalInstances++;
    }
  }
...
const char *ExEspStmtGlobals::getSMTraceFilePrefix() const {
  const char *result = NULL;
  if (smDownloadInfo_) result = smDownloadInfo_->getTraceFilePrefix();
  return result;
}

const ExScratchFileOptions *ExEspStmtGlobals::getScratchFileOptions() const {
  if (resourceInfo_) return resourceInfo_->getScratchFileOptions();
...
void ExEspStmtGlobals::setReplyTag(long transid, long savepointId, long pSavepointId, short replyTag) {
  ...
  if (getIpcEnvironment()->getControlConnection()->castToGuaReceiveControlConnection()->getBeginTransTag() == -1) {
    getTransid() = transid;
    getSavepointId() = savepointId;
    getPSavepointId() = pSavepointId;
  }
...
NABoolean ExEspStmtGlobals::restoreTransaction() {
  if (replyTag_ != GuaInvalidReplyTag) {
    ...
    getIpcEnvironment()->getControlConnection()->castToGuaReceiveControlConnection()->setUserTransReplyTag(replyTag_);

    getContext()->getTransaction()->setTransId(getTransid());
    if (getSavepointId() > 0) {
      // then restore implicit savepoint no matter
      // it's a user savepoint in mxosrvr or not
      getContext()->getTransaction()->setImplicitSavepointId(getSavepointId());
      getContext()->getTransaction()->setImplicitPSavepointId(getPSavepointId());
      getContext()->getTransaction()->setImplicitSavepointState(SP_GENERATED);
    }

    // if this fails we'll catch the error when we try to use the transaction
    return TRUE;
  } else {
    getContext()->getTransaction()->setTransId(-1);
    getContext()->getTransaction()->clearSavepoint();
    getContext()->getTransaction()->setImplicitSavepointId(NULL_SAVEPOINT_ID);
    getContext()->getTransaction()->setImplicitSavepointState(SP_SPNONE);

    // we don't have a transaction work request, this may be because
    // we don't need one or because we need to wait for one
    return NOT espFragInstanceDir_->getFragment(myHandle_)->getNeedsTransaction();
  }
}
...
void ExEspStmtGlobals::setSendTopTcbLateCancelling() { espFragInstanceDir_->startedLateCancelRequest(myHandle_); }

void ExEspStmtGlobals::resetSendTopTcbLateCancelling() { espFragInstanceDir_->finishedLateCancelRequest(myHandle_); }
...
ExEspInstanceThread *ExEspStmtGlobals::getThreadInfo() {
  if (multiThreadedEsp_) {
    ExEspInstanceThread *result = espFragInstanceDir_->getEspInstanceThread(myHandle_);

    ex_assert(result, "Missing thread info in multi-threaded ESP");
    return result;
  }
...
void ExExeStmtGlobals::decrementUdrTxMsgsOut() {
  numUdrTxMsgsOut_--;
  ex_assert(numUdrTxMsgsOut_ >= 0, "The UDR TX message counter has dropped below zero");
}
...
void ExExeStmtGlobals::decrementUdrNonTxMsgsOut() {
  numUdrNonTxMsgsOut_--;
  ex_assert(numUdrNonTxMsgsOut_ >= 0, "The UDR Non-TX message counter has dropped below zero");
}
...
void ExExeStmtGlobals::initSMGlobals() {
  ...
  diags = getDiagsArea();
  if (diags && diags->getNumber(DgSqlCode::ERROR_) > 0) return;
...
ExEspInstanceThread *ExExeStmtGlobals::getThreadInfo() {
  ExEspStmtGlobals *espGlob = castToExEspStmtGlobals();

  if (espGlob) return espGlob->getThreadInfo();
...
SequenceValueGenerator *ex_globals::seqGen() {
  if (!castToExExeStmtGlobals()) return NULL;

  return castToExExeStmtGlobals()->seqGen();
}

NABoolean ExEspStmtGlobals::isMyNodeName(const char *nm) {
  if (!nm) return FALSE;
