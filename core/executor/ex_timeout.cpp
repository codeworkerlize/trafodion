...
ex_tcb *ExTimeoutTdb::build(ex_globals *glob) {
  ExTimeoutTcb *timeout_tcb = new (glob->getSpace()) ExTimeoutTcb(*this, (ExMasterStmtGlobals *)glob);
...
///////////////////////////////////////////////////////////////
ExTimeoutTcb::ExTimeoutTcb(const ExTimeoutTdb &timeout_tdb, ExMasterStmtGlobals *glob) : ex_tcb(timeout_tdb, 1, glob) {
  Space *space = (glob ? glob->getSpace() : 0);
  CollHeap *heap = (glob ? glob->getDefaultHeap() : 0);

  // Allocate the buffer pool
  pool_ = new (space) sql_buffer_pool(timeout_tdb.numBuffers_, timeout_tdb.bufferSize_, space);

  // Allocate the queue to communicate with parent
  qparent_.down =
      new (space) ex_queue(ex_queue::DOWN_QUEUE, timeout_tdb.queueSizeDown_, timeout_tdb.criDescDown_, space);
  ...
  qparent_.up = new (space) ex_queue(ex_queue::UP_QUEUE, timeout_tdb.queueSizeUp_, timeout_tdb.criDescUp_, space);
    ...
    if (getenv("DEBUG_TIMEOUT")) {
      ...
      if (timeoutTdb().isStream()) {  // it was a SET STREAM TIMEOUT
        if (GlobalTimeouts->isStreamTimeoutSet()) {
          ...
          sprintf(errmsg, "Stream timeout was NOT SET ! \n");
        ...
        if (theTableName_[0] == '*') {  // For all tables
          ...
          if (!found)
            ...
            sprintf(errmsg, "Lock timeout for table %s is %d \n", theTableName_, timeoutValue);
        }
      }
      // emit message as an error ( msg 3066 has no text of its own )
      *diagsArea << DgSqlCode(-3066) << DgString0(errmsg);
      ExHandleArkcmpErrors(qparent_, pentry_down, 0, getGlobals(), diagsArea, (ExeErrorCode)-3066);
      ...
      if (timeoutTdb().isStream()) {  // it was a SET STREAM TIMEOUT
        ...
        if (theTableName_[0] == '*') {  // For all tables
          if (timeoutTdb().isReset())   // it was a RESET
            ...
            GlobalTimeouts->setAllLockTimeout(timeoutValue);
        } else {                       // per specific table
          if (timeoutTdb().isReset())  // it was a RESET
            GlobalTimeouts->resetTableLockTimeout(theTableName_);
          else
            GlobalTimeouts->setTableLockTimeout(theTableName_, timeoutValue);
        }
      }
      ...
      if (GlobalTimeouts->noLockTimeoutsSet() && !GlobalTimeouts->isStreamTimeoutSet()) currContext->clearTimeoutData();
...
void ExTimeoutTcb::handleErrors(ex_queue_entry *pentry_down, ComDiagsArea *da, ExeErrorCode err) {
...
ex_tcb_private_state *ExTimeoutPrivateState::allocate_new(const ex_tcb *tcb) {
  return new (((ex_tcb *)tcb)->getSpace()) ExTimeoutPrivateState((ExTimeoutTcb *)tcb);
};
