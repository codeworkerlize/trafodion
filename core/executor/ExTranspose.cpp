...
//
ex_tcb *ExTransposeTdb::build(ex_globals *glob) {
  ...
  //
  ExTransposeTcb *transTcb = new (glob->getSpace()) ExTransposeTcb(*this, *childTcb, glob);
...
//
ExTransposeTcb::ExTransposeTcb(const ExTransposeTdb &transTdbLocal, const ex_tcb &childTcb, ex_globals *glob)
  ...
  // space);
  //
  pool_ = new (space) ExSimpleSQLBuffer(transTdb().numBuffers_, transTdb().bufferSize_, transTdb().transRowLen_, space);
  ...
  //
  if (afterTransPred())
    afterTransPred()->fixup(0, getExpressionMode(), this, glob->getSpace(), glob->getDefaultHeap(), FALSE, glob);
}
...
void ExTransposeTcb::start() {
  ...
  ExTransposePrivateState *pState = ((ExTransposePrivateState *)pEntryDown->pstate);
...
ExWorkProcRetcode ExTransposeTcb::processCancel() {
...
ExWorkProcRetcode ExTransposeTcb::workUp() {
  ...
  while (qParent_.down->getHeadIndex() != processedInputs_) {
    ...
    while (!qParent_.up->isFull() && !childQueue_.up->isEmpty()) {
      ...
      if (cEntry->upState.status == ex_queue::Q_NO_DATA) {
        ...
        while (pState.transCount_ < transTdb().numTransExprs() && !qParent_.up->isFull()) {
          ...
          if (transTdb().criDescUp_->noTuples() == transTdb().criDescDown_->noTuples() + 1)
          ...
          //
          retCode = transColExpr(pState.transCount_)->eval(pEntry->getAtp(), cEntry->getAtp());
          ...
          if (afterTransPred()) retCode = afterTransPred()->eval(pEntry->getAtp(), cEntry->getAtp());
        ...
        //
        if (pState.transCount_ < transTdb().numTransExprs()) return WORK_OK;
      }
    ...
    // we must return.
    //
    if (qParent_.up->isFull() || childQueue_.up->isEmpty()) return WORK_OK;
...
void ExTransposeTcb::processError() {
  ex_queue_entry *pEntryDown = qParent_.down->getHeadEntry();
  ex_queue_entry *pEntry = qParent_.up->getTailEntry();
  ExTransposePrivateState &pState = *((ExTransposePrivateState *)pEntryDown->pstate);
...
//
ExTransposePrivateState::ExTransposePrivateState() { init(); }
...
//
void ExTransposePrivateState::init() {
  matchCount_ = 0;
  transCount_ = 0;
...
ex_tcb_private_state *ExTransposePrivateState::allocate_new(const ex_tcb *tcb) {
  return new (((ex_tcb *)tcb)->getSpace()) ExTransposePrivateState();
}
