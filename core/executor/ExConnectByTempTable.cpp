...
ExConnectByTempTableTcb::ExConnectByTempTableTcb(const ExConnectByTempTableTdb &connectbytemptable_tdb,
                                                 ...
                                                 ex_globals *glob)
    : ex_tcb(connectbytemptable_tdb, 1, glob), step_(INITIAL_) {
  childTcb_ = &child_tcb;
  ...
  pool_ = new (space) sql_buffer_pool(connectbytemptable_tdb.numBuffers_, connectbytemptable_tdb.bufferSize_, space);

  // get the queue that child use to communicate with me
  qchild_ = child_tcb.getParentQueue();

  // Allocate the queue to communicate with parent
  qparent_.down = new (space)
      ex_queue(ex_queue::DOWN_QUEUE, connectbytemptable_tdb.queueSizeDown_, connectbytemptable_tdb.criDescDown_, space);

  // Allocate the private state in each entry of the down queue
  ExConnectByTempTablePrivateState p(this);
  qparent_.down->allocatePstate(&p, this);

  qparent_.up = new (space)
      ex_queue(ex_queue::UP_QUEUE, connectbytemptable_tdb.queueSizeUp_, connectbytemptable_tdb.criDescUp_, space);
  ...
  probeEncodeTupp_.init(connectbytemptable_tdb.probeLen_, NULL, (char *)(probeBytes_));
  ...
  probeInputHashTupp_.init(sizeof(probeInputHashVal_), NULL, (char *)(&probeInputHashVal_));
  workAtp_->getTupp(connectbytemptable_tdb.hashInputValIdx_) = &probeInputHashTupp_;

  hashInputProbeExpr()->fixup(0, getExpressionMode(), this, space, heap, glob->computeSpace(), glob);

  probeInputEncodeTupp_.init(connectbytemptable_tdb.probeLen_, NULL, (char *)(probeInputBytes_));
  workAtp_->getTupp(connectbytemptable_tdb.encodeInputProbeDataIdx_) = &probeInputEncodeTupp_;
  encodeInputProbeExpr()->fixup(0, getExpressionMode(), this, space, heap, glob->computeSpace(), glob);

  if (moveInnerExpr()) moveInnerExpr()->fixup(0, getExpressionMode(), this, space, heap, glob->computeSpace(), glob);

  hashTable_ = new (space)
      ExConnectByHashTable(space, connectbytemptable_tdb.cacheSize_, connectbytemptable_tdb.probeLen_, this);

  dataPopulated_ = FALSE;
};

ExConnectByTempTableTcb::~ExConnectByTempTableTcb() { freeResources(); };
    ...
    switch (step_) {
      case INITIAL_: {
        ...
        if (dataPopulated_ == FALSE) {
          if (qparent_.down->isFull()) return WORK_OK;
      ...
      case POPULATE_DATA_: {
        ...
        switch (reply->upState.status) {
          case ex_queue::Q_OK_MMORE: {
            if ((hashProbeExpr()->eval(reply->getAtp(), workAtp_)) != ex_expr::EXPR_OK)
            ...
            MoveStatus moveRetCode = moveReplyToCache(*reply, pcEntry_);
      ...
      case PROCESS_REQUEST_: {
        ...
        switch (pentry_down->downState.request) {
          ...
          case ex_queue::GET_ALL: {
            if ((hashInputProbeExpr()->eval(pentry_down->getAtp(), workAtp_)) != ex_expr::EXPR_OK)
              ex_assert(0, "Unexpected result from hashInputExpr");
            if ((encodeInputProbeExpr()->eval(pentry_down->getAtp(), workAtp_)) != ex_expr::EXPR_OK)
              ex_assert(0, "Unexpected result from encodeInputExpr");

            if (hashTable_->findEntry(probeInputHashVal_, probeInputBytes_, nextRequest_, pstate.pcEntry_) ==
...
ExConnectByTempTableTcb::MoveStatus ExConnectByTempTableTcb::moveReplyToCache(ex_queue_entry &reply,
                                                                              ExConnectByHashEntry *pcEntry) {
  if (moveInnerExpr()) {
    if (pool_->get_free_tuple(pcEntry->innerRowTupp_, connectbytemptableTdb().recLen_)) return MOVE_BLOCKED;

    workAtp_->getTupp(connectbytemptableTdb().innerRowDataIdx_) = pcEntry->innerRowTupp_;
...
void ExConnectByTempTableTcb::makeReplyToParentUp(ex_queue_entry *pentry_down, ExConnectByTempTablePrivateState &pstate,
  ...
  if ((reply_status == ex_queue::Q_OK_MMORE) && (pstate.pcEntry_->innerRowTupp_.isAllocated())) {
    up_entry->getAtp()->getTupp(connectbytemptableTdb().tuppIndex_) = pstate.pcEntry_->innerRowTupp_;
    if (selectPred()) {
