...
#define GET_PARENT_QUEUE(i)                                \
  (((childTcbs_[i]->getNodeType() != ComTdb::ex_SEND_TOP)) \
       ? qParent_                                          \
...
ex_tcb *ex_split_top_tdb::build(ex_globals *glob) {
  ex_split_top_tcb *result = new (glob->getSpace()) ex_split_top_tcb(*this, glob->castToExExeStmtGlobals());
...
ex_split_top_tcb::ex_split_top_tcb(const ex_split_top_tdb &splitTopTdb, ExExeStmtGlobals *glob)
  ...
  workAtp_->getTupp(splitTopTdb.inputPartAtpIndex_).setDataPointer((char *)(&calculatedPartNum_));
  ...
  if (splitTopTdb.isSystemIdentity()) {
    ...
    int sidBufferSize = (splitTopTdb.bufferSize_ * splitTopTdb.bottomNumParts_ /* maxNumChildren_ for PAPA */);

    // allocate space to keep sidTuple
    pool_ = new (space) sql_buffer_pool(sidNumBuffers, sidBufferSize, space);

    tempChildAtp_ = allocateAtp(splitTopTdb.downCriDesc_, space);

  }  // if isSystemIdentity()

  // fixup expressions
  if (childInputPartFunction())
    (void)childInputPartFunction()->fixup(0, getExpressionMode(), this, glob->getSpace(), glob->getDefaultHeap(), FALSE,
                                          glob);

  if (mergeKeyExpr())
    (void)mergeKeyExpr()->fixup(0, getExpressionMode(), this, glob->getSpace(), glob->getDefaultHeap(), FALSE, glob);
  ...
  if (splitTopTdb.child_->getNodeType() == ComTdb::ex_SEND_TOP) {
    ...
    ex_send_top_tdb *sendTopTdb = (ex_send_top_tdb *)(splitTopTdb.child_.getPointer());
    ...
    if (sendTopTdb->getExtractConsumerFlag()) {
      ...
      maxNumChildren_ = numOfSourceESPs(glob->castToExMasterStmtGlobals() ? 1 : glob->getNumOfInstances(),
                                        glob->getNumOfInstances(childFragId), myInstanceNum);
      firstPartNum_ = myFirstSourceESP(glob->castToExMasterStmtGlobals() ? 1 : glob->getNumOfInstances(),
                                       glob->getNumOfInstances(childFragId), myInstanceNum);
    }
    ...
    for (int i = 0; i < maxNumChildren_; i++) {
      ...
      ((ex_send_top_tcb *)childTcbs_[i])->getParentQueueForSendTop().down->allocateAtps(glob->getSpace());

      childTcbsParentUpQ_.insertAt(i, ((ex_send_top_tcb *)childTcbs_[i])->getParentQueueForSendTop().up);

      childTcbsParentDownQ_.insertAt(i, ((ex_send_top_tcb *)childTcbs_[i])->getParentQueueForSendTop().down);
    }
  ...
  if (!isPapaNode() && maxNumChildren_ > 1)
...
ex_split_top_tcb::~ex_split_top_tcb() { freeResources(); }
...
void ex_split_top_tcb::addChild(int c, NABoolean atWorktime, ExOperStats *statsEntry) {
  ...
  if (splitTopTdb().getSetupSharedPool()) glob->setSharedPool(sharedPool_);
...
ExWorkProcRetcode ex_split_top_tcb::workDown() {
  ...
  while (qParent_.down->entryExists(processedInputs_) AND continueWithDownRequests AND tcbState_ == READY_TO_REQUEST) {
    ...
    ex_split_top_private_state *pstate = (ex_split_top_private_state *)pentry->pstate;
    ...
    ex_assert(request != ex_queue::GET_EMPTY, "Empty entry inserted in parent queue");
    ...
    switch (pstate->getState()) {
      case INITIAL:
        ...
        if (isPapaNode()) {
          ...
          if (request == ex_queue::GET_EOD) {
            ...
            if (splitTopTdb().isSystemIdentity()) {
              ...
              if (rc != WORK_OK) return rc;  // for now this can only
          ...
          if (splitTopTdb().childInputPartFunction_) {
            if (splitTopTdb().childInputPartFunction_->eval(pentry->getAtp(), workAtp_) == ex_expr::EXPR_ERROR) {
              // add error to pstate
              pstate->addDiagsArea(pentry->getAtp()->getDiagsArea());
              cancelChildRequests(processedInputs_);
              // for now
              ex_assert(FALSE, "error calculating input partition #");
              return WORK_BAD_ERROR;  // fix this later
            } else {
              ex_assert(FALSE, "input partitioning is not implemented yet");

              // is the calculated partition number valid?
              ex_assert(calculatedPartNum_ >= 0 AND(CollIndex) calculatedPartNum_ < numChildren,
            ...
            if (splitTopTdb().isMWayRepartition()) {
              ...
              pstate->addPartNumToDoRange(firstPartNum_, firstPartNum_ + maxNumChildren_ - 1);
            } else {
        ...
        if (request == ex_queue::GET_NEXT_N) {
          if (!isPapaNode()) ex_assert(FALSE, "GET_NEXT_N must be used in a Papa node");
        ...
        } else if ((request == ex_queue::GET_NEXT_N_MAYBE_WAIT) && (pstate->partNumsToDo_.entries() > 1)) {
          if (!isPapaNode()) ex_assert(FALSE, "GET_NEXT_N_MAYBE_WAIT must be used in a Papa node");
          pstate->setMaintainGetNextCounters();
...

      {
        ...
        for (CollIndex dp2PartNum = pstate->getFirstPartNumToDo(); dp2PartNum != NULL_COLL_INDEX;
          ...
          if (isPapaNode()) {
            ...
            if (childIndex == NULL_COLL_INDEX) {
              ...
              // right order
              ex_assert(mergeKeyExpr() == NULL, "Not enough PA nodes to merge sorted results");
              continue;
            } else {
              ...
              if (getGlobals()->getStatsArea()) {
                if (childTcbs_[childIndex]->getStatsEntry() == NULL) {
                  if (statsArray_[dp2PartNum] != NULL) {
                    ...
                    // statsArray.
                    childTcbs_[childIndex]->setStatsEntry(statsArray_[dp2PartNum]);
                  } else {
              ...
              if ((request != ex_queue::GET_EOD) && (request != ex_queue::GET_EOD_NO_ST_COMMIT))
                addPartNumReqSent(dp2PartNum);
            }
          } else if (splitTopTdb().isMWayRepartition()) {
            // for table M-Way Repar, find TCB associate to this partNum
            childIndex = getAssociatedChildIndex(dp2PartNum);
            ex_assert(childIndex != NULL_COLL_INDEX, "M-Way repartition is unable to find send top!");
          } else
          ...
          if (continueWithDownRequests AND NOT(pqDown->isFull())) {
            ...
            if (splitTopTdb().needToSendInputData()) {
              if (isPapaNode())
                centry->getTupp(splitTopTdb().partInputDataAtpIndex_) = inputDataTupps_->getTuppDescriptor(1);
              else
                centry->getTupp(splitTopTdb().partInputDataAtpIndex_) =
            ...
            // if needed
            if (isPapaNode()) {
              centry->getTupp(splitTopTdb().paPartNoAtpIndex_) = paPartNumTupps_->getTuppDescriptor(dp2PartNum + 1);
            ...
            childStates_[childIndex].highWaterMark_ = processedInputs_;
        ...
        if (pstate->partNumsToDoIsEmpty()) {
          ...
          if (serializeRequests_) tcbState_ = WAIT_FOR_ALL_REPLIES;
        } else {
      ...
      case PROCESS_GET_NEXT_N_MAYBE_WAIT: {
        ...
        // started.
        if (streamTimeout_ >= 0 && pstate->time_of_stream_get_next_usec_ == 0)  // Ext
        ...
        for (dp2PartNum = pstate->getFirstPartNumToDo(); dp2PartNum != NULL_COLL_INDEX;
             dp2PartNum = pstate->getNextPartNumToDo(dp2PartNum + 1)) {
          // Which TCB child does this request go to?
          CollIndex childIndex;
          childIndex = getAssociatedChildIndex(dp2PartNum);
          ...
          if (childIndex == NULL_COLL_INDEX) continueWithDownRequests = FALSE;
          ...
          if (continueWithDownRequests) {
                ...
                !pstate->rowsAvailable_.contains(dp2PartNum))  // Ext
            {
              ...
              if (NOT(pqDown->isEmpty())) {
...
#ifdef TRACE_PAPA_DEQUEUE
                cout << "Resending GET_NEXT_0 to " << dp2PartNum << childTcbs_[childIndex] << " "
                     << getTable(childTcbs_[childIndex]) << endl;
#endif
              } else {
                ...
                pstate->commandSent_ += dp2PartNum;  // Ext
#ifdef TRACE_PAPA_DEQUEUE
                cout << "Sending GET_NEXT_0 to " << dp2PartNum << childTcbs_[childIndex] << " "
                     << getTable(childTcbs_[childIndex]) << endl;
#endif
              }
            }                                                        // no command send, no rows available
            else if ((pstate->activePartNum_ == NULL_COLL_INDEX) &&  // Ext
                     ...
                     pstate->rowsAvailable_.contains(dp2PartNum))    // Ext
            {
              ...
              if (pentry->downState.numGetNextsIssued != pstate->satisfiedGetNexts_)  // Ext
              {
                ...
                if (centry->downState.requestValue > 100000)
                  pqDown->getNextNRequestInit(pentry->downState.requestValue - pstate->satisfiedRequestValue_);  // Ext
                else
                  pqDown->getNextNRequest(pentry->downState.requestValue - pstate->satisfiedRequestValue_);  // Ext
                // do GET_NEXT_N bookkeeping
                ...
                cout << "GET_NEXT_N to " << dp2PartNum << " childIndex " << childIndex
                     ...
                     << " requestValue = " << pentry->downState.requestValue << " satisfiedRequestValue_ "
                     << pstate->satisfiedRequestValue_  // Ext
                     << childTcbs_[childIndex] << " " << getTable(childTcbs_[childIndex]) << endl;
#endif
              }
            }
      ...
      case PROCESS_GET_NEXT_N: {
        ...
        if (!pstate->activePartNumCmdSent()) {
          ...
          if (NOT(pqDown->isEmpty())) {
            ...
            //  overflow.
            if (centry->downState.requestValue > 100000)
              pqDown->getNextNRequestInit(pentry->downState.requestValue - pstate->satisfiedRequestValue_);
            else
              pqDown->getNextNRequest(pentry->downState.requestValue - pstate->satisfiedRequestValue_);
            // do GET_NEXT_N bookkeeping
      ...
      case PROCESS_GET_NEXT_N_MAYBE_WAIT_ONE_PARTITION: {
        ...
        if (!pstate->activePartNumCmdSent()) {
          ...
          if (NOT(pqDown->isEmpty())) {
            ...
            //  overflow.
            if (centry->downState.requestValue > 100000)
              pqDown->getNextNMaybeWaitRequestInit(pentry->downState.requestValue - pstate->satisfiedRequestValue_);
            else
              pqDown->getNextNMaybeWaitRequest(pentry->downState.requestValue - pstate->satisfiedRequestValue_);
            // do GET_NEXT_N bookkeeping
            pstate->setActivePartNumCmdSent();
          } else {
            centry = pqDown->getTailEntry();
    ...
    // now check the entries inserted into child TCB's down queues
    // getFirstActiveChild, for debug.
...
ExWorkProcRetcode ex_split_top_tcb::workUp() {
  ...
  // ---------------------------------------------------------------------
  while (1) {
    if (qParentDown->isEmpty()) return WORK_OK;
    ...
    if (pstate->getState() == INITIAL) return WORK_OK;
    ...
    **/
    ...
    for (CollIndex i = pstate->getLastStaleChild(); (i = pstate->getPrevActiveChild(i)) != NULL_COLL_INDEX; i--) {
    ...
    while (parentQueueHasRoom) {
      ...
      if (mergeExpr) {
        ...
        iterLmt = (readyChildren_[currReady].entryCnt > 3) ? 3 : readyChildren_[currReady].entryCnt;
      }
        ...
        switch (centry->upState.status) {
          ...
          case ex_queue::Q_OK_MMORE: {
            ...
            if (request == ex_queue::GET_NEXT_N_MAYBE_WAIT)
              cout << "PAPA gets a row " << currChild << " numGetNextsIssued = " << pentry->downState.numGetNextsIssued
                   << " satisfiedGetNexts_ = " << pstateExt->satisfiedGetNexts_
                   << " requestValue = " << pentry->downState.requestValue << " satisfiedRequestValue_ "
                   << pstateExt->satisfiedRequestValue_ << " up queue head " << qParentUp->getHeadIndex()
                   << " up queue tail " << qParentUp->getTailIndex() << childTcbs_[currChild] << " "
                   << getTable(childTcbs_[currChild]) << endl;
#endif
            // ---------------------------------------------------
                ...
                if (pstate->activePartNum_ == NULL_COLL_INDEX && wait_timeout >= 0 &&
                  ...
                  if (pstate->satisfiedGetNexts_ < pentry->downState.numGetNextsIssued) {
                    ...
                    cout << "PAPA timeout currChild " << currChild
                         ...
                         << " requestValue = " << pentry->downState.requestValue << " satisfiedRequestValue_ "
                         << pstate->satisfiedRequestValue_ << " up queue head " << qParentUp->getHeadIndex()
                         << " up queue tail " << qParentUp->getTailIndex() << childTcbs_[currChild] << " "
                         << getTable(childTcbs_[currChild]) << endl;
#endif
                  } else {
#ifdef TRACE_PAPA_DEQUEUE
                    cout << "PAPA unreported timeout currChild " << currChild << endl;
#endif
                  }
                }
          ...
          case ex_queue::Q_NO_DATA:
            ...
            if (centry->getDiagsArea()) {
              ...
              ExMasterStmtGlobals *mGlob = getGlobals()->castToExExeStmtGlobals()->castToExMasterStmtGlobals();
              if (mGlob != NULL && mGlob->getStatement()->getQueryType() > ComTdbRoot::SQL_SELECT_NON_UNIQUE &&
                  mGlob->getStatement()->getQueryType() < ComTdbRoot::SQL_CONTROL) {
                ComMemLog::instance().memlog("%s, RowsAffected: %ld, %ld", mGlob->getStatement()->getUniqueStmtId(),
                                             pstate->getDiagsArea()->getRowCount(), da->getRowCount());
              }
              ...
              if (da->mainSQLCODE() < 0 && NOT splitTopTdb().isLRUOperation()) cancelChildRequests(pindex);
            }
            ...
            **/
            readyChildren_[readyChildren_[currReady].prev].next = readyChildren_[currReady].next;

            readyChildren_[readyChildren_[currReady].next].prev = readyChildren_[currReady].prev;
            ...
            if (pstateExt) pstateExt->activePartNum_ = pstate->getNextPartNumToDo(pstateExt->activePartNum_ + 1);
            pstate->clearActivePartNumCmdSent();
            // BertBert ^^^
            ...
            // associated with a partition
            {
              if (!splitTopTdb().bufferedInserts()) {
                ...
                if ((statsEntry != NULL) && isPapaNode()) {
                  statsArray_[childStates_[currChild].associatedPartNum_] = childTcbs_[currChild]->getStatsEntry();
                  childTcbs_[currChild]->setStatsEntry(NULL);
                }
                childStates_[currChild].associatedPartNum_ = SPLIT_TOP_UNASSOCIATED;
              }
    ...
    ex_assert(pstate->getState() != INITIAL, "Somehow we got here by a shortcut");

    if (pstate->getNumActiveChildren() == 0 AND pstate->partNumsToDoIsEmpty()) {
    ...
    if (pstate->getState() == END_OF_DATA) {
      if (qParentUp->isFull()) return WORK_OK;

      if (pstate->getDiagsArea() && pstate->getDiagsArea()->mainSQLCODE() < 0) {
      ...
      ex_assert(pstate->getDiagsArea() == NULL, "Left-over diags area in split top");

      pstate->init();
      if (pstateExt) pstateExt->init();
      ...
      // BertBert ^^
      if (!qParentDown->isEmpty()) workDownTask_->schedule();
    }
  }  // while not done
...
ExWorkProcRetcode ex_split_top_tcb::workCancel() {
  ...
  while (qParent_.down->entryExists(e)) {
    ...
    // got cancelled
    if (qParent_.down->getQueueEntry(e)->downState.request == ex_queue::GET_NOMORE) cancelChildRequests(e);
    e++;
  }
  return WORK_OK;
}
  ...
  if (qParent_.down->entryExists(parentIndex)) {
    ...
    switch (pstate->getState()) {
      ...
      default: {
        ...
        pstate->clearPartNumsToDo();
        for (CollIndex i = pstate->getLastActiveChild(); i != NULL_COLL_INDEX; i = pstate->getPrevActiveChild(i - 1))
          GET_PARENT_DOWN_QUEUE(i)->cancelRequestWithParentIndex(parentIndex);
...
CollIndex ex_split_top_tcb::getAssociatedChildIndex(int partNo) {
  ...
  if (splitTopTdb().isStaticPaAffinity()) {
    ...
    if (c == NULL_COLL_INDEX) {  // have not accessed this partition before
      ...
      if (numChildren_ == 0 && nc == 1) {  // the first PA already created for partition
      ...
      if (pn == partNo)
        ...
        ex_assert(childStates_[c].numActiveRequests_ == 0, "Working on two partitions at the same time!")
            childStates_[c]
                .associatedPartNum_ = partNo;
        return c;
      } else
...
CollIndex ex_split_top_tcb::findNextReadyChild(ex_split_top_private_state *pstate) {
  // ---------------------------------------------------------------------
  ...
  if (mergeKeyExpr() == NULL OR pstate->getState() == CANCELLING) {
    // -----------------------------------------------------------------
    ...
    CollIndex currChild = pstate->getPrevActiveChild(pstate->getCurrActiveChild() - 1);
    ...
    while (TRUE) {
      if (currChild == NULL_COLL_INDEX) {
        ...
        if (wrappedAround) return currChild;
    ...
    for (CollIndex ch = unmergedChildren_.getLastStaleBit(); (ch = unmergedChildren_.prevUsed(ch)) != NULL_COLL_INDEX;
         ch--) {
      ...
      if (NOT cqueue->isEmpty()) {
        ...
        switch (centry->upState.status) {
          case ex_queue::Q_OK_MMORE: {
            ...
            // for this purpose
            workAtp_->getTupp(mergeKeyAtpIndex()) = mergeKeyTupps_->getTuppDescriptor(ch + 1);

            if (mergeKeyExpr()->eval(centry->getAtp(), workAtp_) == ex_expr::EXPR_ERROR) {
            ...
            char *newKey = mergeKeyTupps_->getTuppDescriptor(ch + 1)->getTupleAddress();

            // do until we found the exact position
            while (highIndex > lowIndex) {
              CollIndex testIndex = (lowIndex + highIndex) / 2;

              ex_queue_entry *tentry = GET_PARENT_UP_QUEUE(mergeSequence_[testIndex])->getHeadEntry();
              // pointer to the key to test (note that it
              // must already have been encoded if it is
              // a member of the merge sequence)
              char *testKey = mergeKeyTupps_->getTuppDescriptor(mergeSequence_[testIndex] + 1)->getTupleAddress();
    ...
    if (mergeSequence_.entries() == pstate->getNumActiveChildren() AND mergeSequence_.entries() > 0) {
...
ex_tcb_private_state *ex_split_top_tcb::allocatePstates(int &numElems, int &pstateLength) {
  ...
  if (useExt) {
    ...
    result = (ex_split_top_private_state *)pexta.allocatePstates(this, numElems, pstateLength);
  } else {
    PstateAllocator<ex_split_top_private_state> pa;
    result = (ex_split_top_private_state *)pa.allocatePstates(this, numElems, pstateLength);
  }
...
ExOperStats *ex_split_top_tcb::doAllocateStatsEntry(CollHeap *heap, ComTdb *tdb) {
  ...
  if (statsType == ComTdb::OPERATOR_STATS) {
    ...
    stat = new (heap) ExSplitTopStats(heap, this, tdb);
  }
...
void ex_split_top_tcb::accumulatePartNumsReqSent() { accumPartNumsReqSent_ += partNumsReqSent_; }
...
ex_split_top_private_state::ex_split_top_private_state() : partNumsToDo_(NULL), activeChildren_(NULL) { init(); }
...
ex_split_top_private_state_ext::ex_split_top_private_state_ext()
    : ex_split_top_private_state(), commandSent_(NULL), rowsAvailable_(NULL) {
  init();
}
