...
ExStoredProcTcb::ExStoredProcTcb(const ExStoredProcTdb &sp_tdb, ex_globals *glob) : ex_tcb(sp_tdb, 1, glob) {
  Space *space = (glob ? glob->getSpace() : 0);

  // Allocate the buffer pool
  pool_ = new (space) sql_buffer_pool(sp_tdb.numBuffers_, sp_tdb.bufferSize_, space);

  // Allocate the queue to communicate with parent
  qparent_.down = new (space) ex_queue(ex_queue::DOWN_QUEUE, sp_tdb.queueSizeDown_, sp_tdb.criDescDown_, space);
  ...
  qparent_.up = new (space) ex_queue(ex_queue::UP_QUEUE, sp_tdb.queueSizeUp_, sp_tdb.criDescUp_, space);
  ...
  if (spTdb().getUseExistingArkcmp()) {
    ...
    arkcmp_ = new (glob->getDefaultHeap())
        ExSqlComp(0, glob->getDefaultHeap(), master_glob->getCliGlobals(), this, COM_VERS_COMPILER_VERSION, NULL,
                  master_glob->getStatement()->getContext()->getEnvironment());
  }
...
short ExStoredProcTcb::work() {
  ...
  if (!arkcmp_ && spTdb().getUseExistingArkcmp()) {
    ...
    ExStoredProcPrivateState &pstate = *((ExStoredProcPrivateState *)pentry_down->pstate);
  ...
  while (1) {
    switch (step_) {
      case BUILD_INPUT_BUFFER_: {
        ...
        while (NOT done) {
          if (pindex == qparent_.down->getTailIndex())
            ...
            if (inputBuffer_->moveInSendOrReplyData(TRUE,  // send(input) data
                                                    FALSE,
                                                    TRUE,  // move data
                                                    &(pentry_down->downState), sizeof(ControlInfo), 0,
                                                    spTdb().inputRowlen_, &dataDesc, 0,
                                                    NULL) == SqlBuffer::BUFFER_FULL) {
              ...
              // move input data
              if (spTdb().inputExpr_) {
                workAtp_->getTupp(spTdb().workAtpIndex_) = dataDesc;
      ...
      case PROCESS_REQUEST_: {
        ...
        CmpMessageISPRequest *ispRequest = new (getHeap()) CmpMessageISPRequest(
            (char *)spTdb().spName_, spTdb().extractInputExpr_, spTdb().extractInputExpr_->getLength(),
            spTdb().moveOutputExpr_, spTdb().moveOutputExpr_->getLength(), 0, 0, (void *)inputBuffer_, sz,
            spTdb().outputRowlen_, sz, getHeap(), master_glob->getStatement()->getUniqueStmtId(),
        ...
        cpStatus = CmpCommon::context()->compileDirect(
            (char *)ispRequest, dummyDatalen, master_glob->getStatement()->getContext()->exHeap(), dummyCharSet,
            CmpMessageObj::INTERNALSP_REQUEST, (char *&)returnedBuffer_, returnedBuflen,
      ...
      case PROCESS_GETNEXT_: {
        ...
        cpStatus = CmpCommon::context()->compileDirect(
            ...
            CmpMessageObj::INTERNALSP_GETNEXT, (char *&)returnedBuffer_, returnedBuflen,
            master_glob->getStatement()->getContext()->getSqlParserFlags(), NULL, 0, cpDiagsArea);
      ...
      case SEND_INPUT_BUFFER_: {
        ...
        if (arkcmp_->sendRequest(spTdb().spName_, spTdb().extractInputExpr_, spTdb().extractInputExpr_->getLength(),
                                 spTdb().moveOutputExpr_, spTdb().moveOutputExpr_->getLength(), 0, 0,
                                 (void *)inputBuffer_, sz, spTdb().outputRowlen_, sz, spTdb().getUseExistingArkcmp(),
                                 /* If true, then use a waited call.
                                   Use no-waited otherwise*/
                                 &requestId_, master_glob->getStatement()->getUniqueStmtId(),
                                 master_glob->getStatement()->getUniqueStmtIdLen()) != ExSqlComp::SUCCESS) {
          ...
          if (arkcmp_->badConnection()) {
            ...
            ex_assert(arkcmp_->getDiags(), "ExSqlComp::getReply returns error, but no diagnostics");
      ...
      case GET_REPLY_BUFFER_: {
        ...
        if (arkcmp_->badConnection()) {
          ...
          arkcmp_->getServer()->getControlConnection()->populateDiagsArea(da, getHeap());
      ...
      case RETURN_ROWS_: {
        ...
        if (returnedBuffer_->moveOutSendOrReplyData(FALSE,  // reply data
                                                    &(up_entry->upState), p, NULL,
                                                    &diags) == TRUE)  // no more rows in this buffer
        {
        ...
        switch (status) {
          case ex_queue::Q_OK_MMORE: {
            if (pstate.errorHappened_) {
              ...
              if (spTdb().predExpr_) {
                ex_expr::exp_return_type retCode = (spTdb().predExpr_->eval(up_entry->getAtp(), 0));
          ...
          case ex_queue::Q_NO_DATA: {
            if (diags) {
              ...
              da->unpackObj(diags->getType(), diags->getVersion(),
                            TRUE,  // sameEndianness,
                            0,     // dummy for now...
          ...
          case ex_queue::Q_INVALID:
            ex_assert(0, "ExStoredProcTcb::work() Invalid state returned by arkcmp");
            break;
        }  // switch status
      } break;
...
short ExSPInputOutput::inputValue(int fieldNum, char *inputRow, char *data, int datalen, NABoolean casting,
  ...
  if (casting == FALSE)
    convDoIt(&inputRow[attr->getOffset()], attr->getLength(&inputRow[attr->getVCLenIndOffset()]), attr->getDatatype(),
             ...
             &data[((attr->getVCIndicatorLength() <= 0) ? 0 : attr->getVCIndicatorLength())], attr->getLength(),
             attr->getDatatype(), attr->getPrecision(), attr->getScale(),
             (attr->getVCIndicatorLength() > 0 ? data : NULL), attr->getVCIndicatorLength(), NULL, /*CollHeap *heap,*/
             &diagsArea, CONV_UNKNOWN_LEFTPAD);
  else {
    //      TBD:
    ...
    // by data bytes.
    convDoIt(&inputRow[attr->getOffset()], attr->getLength(&inputRow[attr->getVCLenIndOffset()]), attr->getDatatype(),
             attr->getPrecision(), attr->getScale(), &data[varcharlen], datalen - varcharlen, REC_BYTE_V_ASCII, 0, 0,
             data, varcharlen, NULL, /*CollHeap *heap,*/
             &diagsArea, CONV_UNKNOWN);
  }
...
short ExSPInputOutput::outputValue(int fieldNum, char *outputRow, char *data, int datalen, NABoolean casting,
  ...
  if (data == NULL) {
    ...
    if (attr->getNullFlag()) {
      ...
      str_pad(&outputRow[nullIndOffset], nullIndicatorLen, '\377');
      return status;
    } else {
      // return error
  ...
  if (casting == FALSE)
    status = convDoIt(
        ...
        // if varchar, then data points to varchar length and
        // 'data + varcharIndLen' points to the real data.
        &data[((varcharIndLen <= 0) ? 0 : varcharIndLen)], attr->getLength(data), dataType, precision, scale,
        &outputRow[attr->getOffset()], attr->getLength(), dataType, precision, scale, &outputRow[varcharIndOffset],
        varcharIndLen, heap, &diagsArea, CONV_UNKNOWN_LEFTPAD);
  else
    status = convDoIt(&data[0], datalen, REC_BYTE_F_ASCII, precision, scale, &outputRow[attr->getOffset()],
                      attr->getLength(), dataType, precision, scale,
                      (varcharIndLen > 0 ? &outputRow[varcharIndOffset] : NULL), varcharIndLen, heap, &diagsArea,
                      (getCaseIndexArray() ? getCaseIndexArray()[fieldNum] : CONV_UNKNOWN_LEFTPAD));

  if ((status == ex_expr::EXPR_OK) && attr->getNullFlag()) {
    // make the result not null.
    str_pad(&outputRow[nullIndOffset], nullIndicatorLen, '\0');
  }
  return status;
}
