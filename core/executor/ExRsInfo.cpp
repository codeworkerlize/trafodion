...
#define exrsinfo_assert(p, msg)       \
  if (!(p)) {                         \
    NAAbort(__FILE__, __LINE__, msg); \
    NAExit(-1);                       \
  };
/*
 **********************************************************************
 ...
 */
RsInfo::RsInfo()
    : statement_(NULL), openAttempted_(FALSE), closeAttempted_(FALSE), prepared_(FALSE), proxySyntax_(NULL) {}
RsInfo::~RsInfo() {}
/*
 **********************************************************************
 * ExRsInfo functions
...
ExRsInfo::~ExRsInfo() {
  ...
  if (enterTxStream_) enterTxStream_->setExRsInfo(NULL);

  if (exitOrSuspendStream_) exitOrSuspendStream_->setExRsInfo(NULL);
...
void ExRsInfo::populate(int index, const char *proxySyntax) {
  ...
  if (rsInfoArray_.used(index)) {
    ...
    char *proxySyntax = rsInfo->getProxySyntax();
    if (proxySyntax) collHeap()->deallocateMemory(proxySyntax);
...
void ExRsInfo::bind(int index, Statement *statement) {
  ...
  if (rsInfoArray_.used(index)) {
    ...
    ExRsDebug2(" Entry %u exists and has stmt pointer %p", index, rsInfo->getStatement());
    rsInfo->setStatement(statement);
    rsInfo->setPrepared(FALSE);
  } else {
    ExRsDebug2(" About to add index=%u, statement=%p", index, statement);
...
void ExRsInfo::setCloseAttempted(int index) {
  ...
  if (rsInfoArray_.used(index)) {
    ...
    if (rsInfo) {
      ...
      if (rsInfo->getOpenAttempted() && !rsInfo->getCloseAttempted()) numClosedSinceLastCall_++;
...
NABoolean ExRsInfo::statementExists(int index) const {
  ...
  if (rsInfoArray_.used(index)) {
    RsInfo *rsInfo = rsInfoArray_[index];
    if (rsInfo && rsInfo->getStatement()) found = TRUE;
  }
...
NABoolean ExRsInfo::openAttempted(int index) const {
  ...
  ExRsDebug2("[END openAttempted] found=%s, openAttempted=%s", (found ? "TRUE" : "FALSE"),
             (openAttempted ? "TRUE" : "FALSE"));
...
NABoolean ExRsInfo::closeAttempted(int index) const {
  ...
  ExRsDebug2("[END closeAttempted] found=%s, closeAttempted=%s", (found ? "TRUE" : "FALSE"),
             (closeAttempted ? "TRUE" : "FALSE"));
...
NABoolean ExRsInfo::isPrepared(int index) const {
  ...
  ExRsDebug2("[END isPrepared] found=%s, prepared=%s", (found ? "TRUE" : "FALSE"), (prepared ? "TRUE" : "FALSE"));
...
NABoolean ExRsInfo::getRsInfo(int position,               // IN
  ...
  exrsinfo_assert(position > 0, "ExRsInfo::getRsInfo() - invalid position");
  NABoolean found = FALSE;
...
void ExRsInfo::setNumEntries(ComUInt32 maxEntries) {
  if (getNumEntries() < maxEntries) rsInfoArray_.resize(maxEntries + 1);
}
...
void ExRsInfo::enterUdrTx(ExExeStmtGlobals &stmtGlobals) {
  ...
  if (txDebug_) debug_ = TRUE;
  ExRsDebug3("[BEGIN enterUdrTx] %p, state %s, handle " INT64_PRINTF_SPEC, this, getTxStateString(txState_),
             udrHandle_);
#endif
...
#ifdef _DEBUG
  ExRsDebug2("[END enterUdrTx] %p, state %s", this, getTxStateString(txState_));
  debug_ = oldDebug;
#endif
}
...
void ExRsInfo::suspendUdrTx(ExExeStmtGlobals &stmtGlobals) {
  ...
  NABoolean oldDebug = debug_;
  if (txDebug_) debug_ = TRUE;
  ExRsDebug3("[BEGIN suspendUdrTx] %p, state %s, handle " INT64_PRINTF_SPEC, this, getTxStateString(txState_),
             udrHandle_);
#endif
...
#ifdef _DEBUG
  ExRsDebug2("[END suspendUdrTx] %p, state %s", this, getTxStateString(txState_));
  debug_ = oldDebug;
#endif
}
...
void ExRsInfo::exitUdrTx(ExExeStmtGlobals &stmtGlobals) {
  ...
  NABoolean oldDebug = debug_;
  if (txDebug_) debug_ = TRUE;
  ExRsDebug3("[BEGIN exitUdrTx] %p, state %s, handle " INT64_PRINTF_SPEC, this, getTxStateString(txState_), udrHandle_);
#endif
...
#ifdef _DEBUG
  ExRsDebug2("[END exitUdrTx] %p, state %s", this, getTxStateString(txState_));
  debug_ = oldDebug;
#endif
}
...
void ExRsInfo::sendTxMessage(ExExeStmtGlobals &stmtGlobals, TxMsgType t) {
  ...
  IpcConnection *conn = stmtGlobals.castToExMasterStmtGlobals()->getUdrConnection();
  ...
  if (!(ipcProcessId_ == otherEnd) || (udrServer_->getState() == ExUdrServer::EX_UDR_BROKEN)) {
#ifdef _DEBUG
    ...
    if (t == EXIT || t == SUSPEND) {
      exrsinfo_assert(enterTxStream_, "Cannot send EXIT or SUSPEND when ENTER stream is NULL");

      ExRsDebug1("  About to abandon I/O on ENTER TX stream %p", enterTxStream_);
  ...
  UdrClientControlStream *s = new (h) UdrClientControlStream(ipcEnv,
  ...
  if (t == ENTER) {
    exrsinfo_assert(enterTxStream_ == NULL, "Cannot send ENTER when stream is not NULL");
    ExRsDebug1("  Setting ENTER TX stream to %p", s);
    enterTxStream_ = s;
  } else if (t == EXIT || t == SUSPEND) {
    exrsinfo_assert(exitOrSuspendStream_ == NULL, "Cannot send EXIT or SUSPEND when stream is not NULL");
    ExRsDebug1("  Setting EXIT or SUSPEND stream to %p", s);
    exitOrSuspendStream_ = s;
  }

#ifdef _DEBUG
  if (debug_) s->setTraceFile(stdout);
#endif
...
void ExRsInfo::reportCompletion(UdrClientControlStream *s) {
  ExRsDebug2("[BEGIN reportCompletion] %p, state %s", this, getTxStateString(txState_));
  ExRsDebug1("  stream pointer %p", s);
  ExRsDebug2("  ENTER stream %p, EXIT/SUSPEND stream %p", enterTxStream_, exitOrSuspendStream_);
  ...
  ExRsDebug2("[END reportCompletion] %p, state %s", this, getTxStateString(txState_));
}
