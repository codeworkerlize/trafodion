...
void SwapSpace::discard(bool allDone) {
  if (numSwap_ > 0) {
    reset(allDone);
  }
}

IoStatus SwapSpace::fetchNextBuffer(void) {
...
Int16 SwapSpace::getLastError(void) {
  Int16 error = 0;
...
Int16 SwapSpace::getLastSqlCode(void) {
...
char *SwapSpace::promoteBuffer(void) {
  char *adoptee = NULL;
...
IoStatus SwapSpace::rewind(void) {
  IoStatus status = OK;
...
IoStatus SwapSpace::swapOut(char *buffer) {
  IoStatus status = OK;
  ...
  if (status == OK) {
    ...
    status = mapStatus(store_->writeThru(swapOutBuffer_, swapBufferSize_, ignoredBlockNum));
...
void SwapSpace::setIoEventHandler(ExSubtask *ioEventHandler) { ioEventHandler_ = ioEventHandler; }

// **********************************************************************
// * Private methods
// **********************************************************************

IoStatus SwapSpace::checkIOPending(bool *readCompleted) {
  IoStatus status = OK;

  ex_assert((ioPending_ != NONE), "checkIOPending not called via checkIO wrapper");
  ...
  if (status == OK) {
    ...
    if (ioPending_ == READ) {
      readingFirstBuffer_ = ((swapRead_ == swapStart_) && (swapRead_ != swapEnd_));
      if (readCompleted) {
        *readCompleted = true;
      }
    } else {
...
IoStatus SwapSpace::mapStatus(RESULT scratchStatus) const {
  IoStatus status = OK;
...
void SwapSpace::reset(bool allDone) {
  if (allDone) {
    store_->close();
  } else {
    store_->truncate();
