                                  ...
                                  int lockInInternal1PartNameOutBufLen)  // in bytes
{
  ...
  if (allowedLen < objInt1PartNameLen) {
    ...
    for (; *tempPtr != '\0';) hashValue += *tempPtr++;
  }
  str_cpy_all(lockInternal1PartNameOutBuf, objInternal1PartName, allowedLen);
...
ExExeUtilTcb::ExExeUtilTcb(const ComTdbExeUtil &exe_util_tdb,
  ...
  pool_ = new (space) sql_buffer_pool(exe_util_tdb.numBuffers_, exe_util_tdb.bufferSize_, space);
  ...
  qparent_.down =
      new (space) ex_queue(ex_queue::DOWN_QUEUE, exe_util_tdb.queueSizeDown_, exe_util_tdb.criDescDown_, space);

  qparent_.up = new (space) ex_queue(ex_queue::UP_QUEUE, exe_util_tdb.queueSizeUp_, exe_util_tdb.criDescUp_, space);
  ...
  if (exe_util_tdb.inputExpr_)
    (void)exe_util_tdb.inputExpr_->fixup(0, getExpressionMode(), this, space, heap, FALSE, glob);
  if (exe_util_tdb.outputExpr_)
    (void)exe_util_tdb.outputExpr_->fixup(0, getExpressionMode(), this, space, heap, FALSE, glob);

  if (exe_util_tdb.scanExpr_)
    (void)exe_util_tdb.scanExpr_->fixup(0, getExpressionMode(), this, space, heap, FALSE, glob);
  ...
  else if (glob->castToExExeStmtGlobals()->castToExEspStmtGlobals() &&
...
ExExeUtilTcb::~ExExeUtilTcb() {
  ...
  if (explQuery_) NADELETEBASIC(explQuery_, getHeap());
...
char *ExExeUtilTcb::getTimeAsString(long elapsedTime, char *timeBuf, NABoolean noUsec) {
  ...
  if (noUsec)
    str_sprintf(timeBuf, "%02u:%02u:%02u", hour, min, sec);
  else
    str_sprintf(timeBuf, "%02u:%02u:%02u.%03u", hour, min, sec, TO_FMT3u(usec));
...
char *ExExeUtilTcb::getStatusString(const char *operation, const char *status, const char *object, char *outBuf,
                                    NABoolean isET, char *timeBuf, char *queryBuf, char *errorBuf) {
  ...
  s[9] = 0;
  if (queryBuf) {
    str_sprintf(outBuf, "Task: %s  Status: %s  Command: %s", o, s, queryBuf);
  } else if (timeBuf && isET) {
    str_sprintf(outBuf, "Task: %s  Status: %s  Elapsed Time:    %s", o, s, timeBuf);
  } else if (errorBuf) {
    str_sprintf(outBuf, "Task: %s  Status: %s  Details: %s", o, s, errorBuf);
  } else if (timeBuf) {
    str_sprintf(outBuf, "Task: %s  Status: %s  Time: %s", o, s, timeBuf);
  } else {
    if (object)
      str_sprintf(outBuf, "Task: %s  Status: %s  Object: %s", o, s, object);
    else
      str_sprintf(outBuf, "Task: %s  Status: %s", o, s);
  }
    ...
    switch (pqStep_) {
      case PROLOGUE_: {
        ...
        if (displayStartTime) {
          ...
          if (moveRowToUpQueue(buf, 0, &rc)) return 1;
        }
      ...
      case FETCH_ROW_: {
        ...
        if ((retcode > 0) && (retcode != 100)) warning_ = retcode;

        if ((retcode != 100) && (cliInterface()->outputBuf()))
          pqStep_ = RETURN_ROW_;
        else
          pqStep_ = CLOSE_;
      } break;
      ...
      case ERROR_RETURN_: {
        ...
        if (moveErrorRow) {
          if (retcode == 0) {
            ...
            if (sqlcode < 0 && ec != NULL) *ec = sqlcode;

            if (cc->getOptionalStringCharSet(0) == CharInfo::ISO88591 ||
                cc->getOptionalStringCharSet(0) == CharInfo::UTF8)
          ...
          if (stringParam1) str_sprintf(&errorBuf[strlen(errorBuf)], ", %s", stringParam1);
          if (intParam1 != ComDiags_UnInitialized_Int) str_sprintf(&errorBuf[strlen(errorBuf)], ", %d", intParam1);

          char *outBuf = new (getHeap()) char[errorBufLen + 400];
          getStatusString(task, "Error", NULL, outBuf, FALSE, NULL, errorBuf);

          NADELETEBASIC(errorBuf, getHeap());

          if ((moveErrorRow) && (moveRowToUpQueue(outBuf, 0, &rc))) {
...
int ExExeUtilTcb::setCS(const char *csName, char *csValue, ExeCliInterface *cliInterface, ComDiagsArea *globalDiags) {
  ...
  cliRC = cliInterface->executeImmediate(buf, NULL, NULL, TRUE, NULL, FALSE, &globalDiags);
  if (cliRC < 0) {
    return cliRC;
  }
...
void ExExeUtilTcb::setMaintainControlTableTimeout(char *catalog) {
  ...
  cliRC = cliInterface()->executeImmediate(buf, timeoutHoldBuf, &timeoutHoldBufLen);
...
short ExExeUtilTcb::setSystemVersion() {
  ...
  if (sysVersionStrLen_ == 0) {
    ...
    cliRC = cliInterface()->executeImmediate(
        "select translate(substring(translate(a using utf8toucs2), 10, 4) using ucs2toiso88591) from (get version of "
        "system) x(a)",
...
static const QueryString getObjectUidQuery[] = {{" select cast(O.object_uid as char(24)) from "},
...
short ExExeUtilTcb::getObjectUid(char *catName, char *schName, char *objName, NABoolean isIndex, NABoolean isMv,
  ...
  str_sprintf(infoQuery, gluedQuery, catName, schName, objName, (isIndex ? "IX" : "TA"),
...
long ExExeUtilTcb::getObjectFlags(long objectUID) {
  ...
  exeQuery.format("select flags from %s.\"%s\".%s where object_uid = %ld", TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA,
                  SEABASE_OBJECTS, objectUID);

  cliRC = cliInterface()->fetchRowsPrologue(exeQuery.data(), TRUE /*no exec*/);
  if (cliRC < 0) {
    cliInterface()->allocAndRetrieveSQLDiagnostics(diagsArea_);
    return -1;
  }
...
short ExExeUtilTcb::alterObjectState(NABoolean online, char *tableName, char *failReason, NABoolean forPurgedata) {
  ...
  // make object online
  str_sprintf(buf, "ALTER TABLE %s %s %s", tableName, (online ? "ONLINE" : "OFFLINE"),
  ...
  if (cliRC < 0) {
    str_sprintf(failReason, "Could not alter the state of table %s to %s.", tableName, (online ? "online" : "offline"));
...
short ExExeUtilTcb::lockUnlockObject(char *tableName, NABoolean lock, NABoolean parallel, char *failReason) {
  ...
  if (parallel) {
    if (lock)
      str_sprintf(buf, "LOCK TABLE %s IN PROTECTED MODE NO INDEX LOCK PARALLEL EXECUTION ON", tableName);
    else
      str_sprintf(buf, "UNLOCK TABLE %s PARALLEL EXECUTION ON", tableName);
  } else {
    if (lock)
      str_sprintf(buf, "LOCK TABLE %s IN PROTECTED MODE", tableName);
    else
      str_sprintf(buf, "UNLOCK TABLE %s", tableName);
  }
  ...
  if (cliRC < 0) {
    if (lock)
      str_sprintf(failReason, "Could not lock table %s in protected mode using parallel execution.", tableName);
    else
      str_sprintf(failReason, "Could not unlock table %s using parallel execution.", tableName);
...
short ExExeUtilTcb::doubleQuoteStr(char *str, char *newStr, NABoolean singleQuote) {
...
short ExExeUtilTcb::alterDDLLock(NABoolean add, char *tableName, char *failReason, NABoolean isMV, int lockType,
  ...
  if (numParts == 3) {
    quotedParts0 = new (getGlobals()->getDefaultHeap()) char[strlen(parts[0]) * 2 + 1];
    quotedParts1 = new (getGlobals()->getDefaultHeap()) char[strlen(parts[1]) * 2 + 1];
  ...
  if (numParts == 3) {
    ...
    str_sprintf(buf, "CREATE TANDEM_CAT_REQUEST&1 %s %d <\"%s\".\"%s\".\"%s\"> <%s%s> <%s> <%d> %s %s",
                ...
                quotedParts0, quotedParts1, quotedLockName, tableName, "", (isMV ? "2" : "0"), lockType,
                (lockType == COM_UTIL_PURGEDATA ? "<9>" : "<0>"), skipDDLLockCheck ? sdlc : "");
  } else
    str_sprintf(buf, "CREATE TANDEM_CAT_REQUEST&1 %s %d <%s_DDL_LOCK%s> <%s%s> <%s> <%d> %s %s", (add ? "1" : "2"),
                //		(lockType == COM_UTIL_PURGEDATA ? 5 : 4),
                (skipDDLLockCheck ? 8 : 5),  //(lockType == COM_UTIL_PURGEDATA ? 5 : 4),
                tableName, (lockSuffix ? lockSuffix : ""), tableName, "", (isMV ? "2" : "0"), lockType,
                (lockType == COM_UTIL_PURGEDATA ? "<9>" : "<0>"), skipDDLLockCheck ? sdlc : "");
  ...
  if (cliRC < 0) {
    str_sprintf(failReason, "Could not %s ddl lock for object %s.", (add ? "add" : "drop"), tableName);
...
short ExExeUtilTcb::alterCorruptBit(short val, char *tableName, char *failReason, Queue *indexList) {
  ...
  str_sprintf(buf, "LABEL_ALTER TABLE %s PARALLEL EXECUTION ON OPCODE 9 '%s'", tableName, (val == 1 ? "1" : "0"));
  ...
  if (cliRC < 0) {
    str_sprintf(failReason, "Could not %s the corrupt bit on table %s.", (val == 1 ? "set" : "reset"), tableName);
    return -1;
  }
  ...
  if (indexList) {
    ...
    while (NOT indexList->atEnd()) {
      ...
      str_sprintf(buf, "LABEL_ALTER INDEX_TABLE %s PARALLEL EXECUTION ON OPCODE 9 '%s'", indexName,
                  (val == 1 ? "1" : "0"));
      ...
      if (cliRC < 0) {
        str_sprintf(failReason, "Could not %s the corrupt bit on index %s.", (val == 1 ? "set" : "reset"), indexName);
...
short ExExeUtilTcb::alterAuditFlag(NABoolean audited, char *tableName, NABoolean isIndex) {
  ...
  // change the corrupt bit in the label
  if (isIndex)
    str_sprintf(buf, "LABEL_ALTER INDEX_TABLE %s PARALLEL EXECUTION ON OPCODE %s ''", tableName,
                (audited ? "65" : "66"));
  else
    str_sprintf(buf, "LABEL_ALTER TABLE %s PARALLEL EXECUTION ON OPCODE %s ''", tableName, (audited ? "65" : "66"));
...
short ExExeUtilTcb::handleError() {
  ...
  if (diagsArea_ != NULL) diagsArea_->deAllocate();
  diagsArea_ = NULL;
  return rc;
}
...
short ExExeUtilTcb::createServer(char *serverName, const char *inPName, IpcServerTypeEnum serverType,
  ...
  IpcServerClass *sc = new (ipcHeap) IpcServerClass(env, serverType,
  ...
  if (!sc) {
    if (logError) {
      ...
      str_sprintf(emsText, "Failure creating IpcServerClass on \\%s cpu %d to %s %s for %s.", nodeName, cpu, operation,
  ...
  ipcServer = sc->allocateServerProcess(&diagsArea_, ipcHeap, nodeName, cpu, priority,
  ...
  if (!ipcServer) {
    if (logError && diagsArea_) {
      ...
      str_sprintf(emsText, "allocateServerProcess() failed with error %d on \\%s cpu %d to %s %s for %s.",
                  diagsArea_->mainSQLCODE(), nodeName, cpu, operation, partnName, (char *)exeUtilTdb().getTableName());
      SQLMXLoggingArea::logExecRtInfo(NULL, 0, emsText, 0);
    }
...
int ExExeUtilTcb::extractParts(char *objectName, char **paramParts0, char **paramParts1, char **paramParts2,
  ...
  if (extractedPartsObj_) delete extractedPartsObj_;

  extractedPartsObj_ = new (getHeap()) AnsiName(objectName);
  if ((rc = extractedPartsObj_->extractParts(numParts, parts, noValidate)) != 0 || (numParts != 3)) {
...
ex_expr::exp_return_type ExExeUtilTcb::evalScanExpr(char *ptr, int len, NABoolean copyToVCbuf) {
  ...
  if (exeUtilTdb().scanExpr_) {
    ...
    char *exprPtr = ptr;
    if (copyToVCbuf) {
      exprPtr = new (getGlobals()->getDefaultHeap()) char[SQL_VARCHAR_HDR_SIZE + len];
    ...
    workAtp_->getTupp(exeUtilTdb().workAtpIndex()).setDataPointer(exprPtr);

    exprRetCode = exeUtilTdb().scanExpr_->eval(pentry_down->getAtp(), workAtp_);

    if (exprPtr != ptr) NADELETEBASIC(exprPtr, getGlobals()->getDefaultHeap());
  }
