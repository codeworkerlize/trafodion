...

{
  ...
  while (exprList.getFirst(currExpr) == TRUE) {
    ...
    if (currExpr) {
      ...
      if ((this->getOperatorType() == REL_ROOT) && (unParsed == "input_variables") &&
...
ExplainTuple *RelExpr::addExplainInfo(ComTdb *tdb, ExplainTuple *leftChild, ExplainTuple *rightChild,
                                      Generator *generator) {
  ...
  ExplainDesc *explainDesc = (ExplainDesc *)generator->getFragmentDir()->getTopNode(fragIndex);
  ...
  if (blankIndex != NA_NPOS) operText.remove(blankIndex);  // cuts off chars including space

  // upshift
  operText.toUpper();

  NABoolean doExplainSpaceOpt = FALSE;
  if (CmpCommon::getDefault(EXPLAIN_SPACE_OPT) == DF_ON) doExplainSpaceOpt = TRUE;
  ...
  if (leftChild) explainTuple->setChildSeqNum(0, leftChild->getSeqNum());
  if (rightChild) explainTuple->setChildSeqNum(1, rightChild->getSeqNum());
  ...
  NAString costDetail, ocDetail = "OPERATOR_COST: Zero cost", ruDetail = "ROLLUP_COST: Zero cost";

  NABoolean explainForCalibration = (CmpCommon::getDefault(EXPLAIN_DETAIL_COST_FOR_CALIBRATION) == DF_ON);

  if (operatorCost) {
    explainTuple->setOperatorCost((MINOF(operatorCost->displayTotalCost(), 1e32)).getValue());
  ...
  if (rollUpCost) {
    explainTuple->setTotalCost((MINOF(rollUpCost->displayTotalCost(), 1e32)).getValue());
  ...
  if (CmpCommon::getDefault(COMPRESSED_INTERNAL_FORMAT_EXPLAIN) == DF_ON && criUp && criUp->noTuples() > 2) {
    ...
    if (hasTD) {
      ...
      for (UInt16 i = 2; i < criUp->noTuples(); i++) {
        ...
        snprintf(buf, sizeof(buf), "%d:%s ", i, tf);
        explainTuple->setDescription(buf);
      }
    }
  }
  ...
  if ((tdb->doOltQueryOptLean()) && (oltOptLean())) {
    char oltopt[100];
  ...
  if (generator->getEspLevel() == 0) memory_quota_str = "memory_quota: %d MB ";

  switch (tdb->getNodeType()) {
    case ComTdb::ex_ROOT: {
      if (quotaPerBMO > 0 || mlimit == 0) break;

      double BMOsMemory = generator->getTotalBMOsMemoryPerNode().value() / (1024 * 1024);
      double nBMOsTotalMemory = (generator->getTotalNBMOsMemoryPerNode()).value() / (1024 * 1024);
      snprintf(buf, 120, "est_memory_per_node: %.2f(Limit), %.2f(BMOs), %.2f(nBMOs) MB ", mlimit, BMOsMemory,
               nBMOsTotalMemory);
      fragdescr += buf;
    } break;

    case ComTdb::ex_HASHJ:
      sprintf(buf, memory_quota_str, ((ComTdbHashj *)tdb)->memoryQuotaMB());
      fragdescr += buf;
      break;
    ...
    case ComTdb::ex_SORT:
      sprintf(buf, memory_quota_str, (((ComTdbSort *)tdb)->getSortOptions())->memoryQuotaMB());
      fragdescr += buf;
      break;
    ...
    case ComTdb::ex_FIRST_N: {
      if (((FirstN *)this)->firstNPredUsed()) {
        sprintf(buf, "predicate_created : yes ");
        fragdescr += buf;
      }
    } break;
  ...
  sprintf(buf, "max_card_est: %g fragment_id: %d parent_frag: ", MINOF(getMaxCardEst(), 1e32).value(), cix);
  fragdescr += buf;
  ...
  if (mlimit > 0 && quotaPerBMO == 0) {
    ...
    if (reportMemoryEst == TRUE) {
      if (nodeType == ComTdb::ex_HASH_GRBY || nodeType == ComTdb::ex_HASHJ || nodeType == ComTdb::ex_SORT) {
  ...
  // Add total column length info
  NAString description("record_length: ");
  sprintf(buf, "%d ", tdb->getRecordLength());
  description += buf;
...
ExplainTuple *RelExpr::addSpecificExplainInfo(ExplainTupleMaster *explainTuple, ComTdb *tdb, Generator *generator) {
  return explainTuple;
}

ExplainTuple *FileScan::addSpecificExplainInfo(ExplainTupleMaster *explainTuple, ComTdb *tdb, Generator *generator) {
  ...
  switch (accessOptions().lockMode()) {
    ...
    case LOCK_MODE_NOT_SPECIFIED_:
      description += "not specified, defaulted to lock cursor ";
      break;
    default:
      description += "unknown ";
      break;
  ...
  switch (accessOptions().accessType()) {
    ...
    case TransMode::ACCESS_TYPE_NOT_SPECIFIED_:
      description += "not specified, defaulted to read committed ";
      break;
    default:
      description += "unknown ";
      break;
  ...
  if (natable->isPartitionNameSpecified()) {
    ...
    const NodeMapEntry *nmapentry =
        natable->getClusteringIndex()->getPartitioningFunction()->getNodeMap()->getNodeMapEntry(0);
  ...
  if (minMaxHJColumns().entries() > 0) {
    ExprNode *minMaxCols = minMaxHJColumns().rebuildExprTree(ITM_ITEM_LIST);
  ...
  if (((ComTdbHbaseAccess *)tdb)->sqHbaseTable()) {  // if trafodion table
    ...
    for (int j = 0; j < listOfColNames->numEntries(); j++) {
      ...
      if (colNameLen == sizeof(char))
        ...
        v = 0;
      if (j == 0)
        str_sprintf(buf, "%s%s%ld", colFam, (withAt ? "@" : ""), v);
      else
        str_sprintf(buf, ",%s%s%ld", colFam, (withAt ? "@" : ""), v);
...
static void appendPushedDownExpression(ComTdb *tdb, NAString &outNAString) {
  ...
  // AND or OR, the rest are operands. this function display the column, operator and replace any constant with ?. it
  // keeps reverse polish format this can be improved in the future for better readability.
  char buf[1000];
...
ExplainTuple *HbaseAccess::addSpecificExplainInfo(ExplainTupleMaster *explainTuple, ComTdb *tdb, Generator *generator) {
  NAString description("scan_type: " + getTypeText());
  ...
  if (((ComTdbHbaseAccess *)tdb)->rowsetOper()) {
    description += "rowset_vsbb_size: ";
    char vsbbSize[12];
    snprintf(vsbbSize, 12, "%u", ((ComTdbHbaseAccess *)tdb)->getHbaseRowsetVsbbSize());
    description += vsbbSize;
    description += " ";
  }
  ...
  if ((((ComTdbHbaseAccess *)tdb)->skipTransaction())) {
    description += "skip_transaction: ";
    if ((((ComTdbHbaseAccess *)tdb)->skipTransactionForced()))
      description += "FORCED ";
    else
      description += "ON ";
  }
  ...
  if (((ComTdbHbaseAccess *)tdb)->getHbaseSnapshotScanAttributes()->getUseSnapshotScan()) {
    ...
    snprintf(buf, TMP_BUFFER_SIZE, "%s ",
             ((ComTdbHbaseAccess *)tdb)->getHbaseSnapshotScanAttributes()->getSnapshotName());
    description += buf;

    description += "snapshot_temp_location: ";
    snprintf(buf, TMP_BUFFER_SIZE, "%s ",
             ((ComTdbHbaseAccess *)tdb)->getHbaseSnapshotScanAttributes()->getSnapScanTmpLocation());
    description += buf;
  }

  if (getTableDesc()->getNATable()->isSeabaseTable()) {
    if (((ComTdbHbaseAccess *)tdb)->useEncryption()) description += "encryption: used ";
  }
...
ExplainTuple *HbaseDelete::addSpecificExplainInfo(ExplainTupleMaster *explainTuple, ComTdb *tdb, Generator *generator) {
...
ExplainTuple *HbaseUpdate::addSpecificExplainInfo(ExplainTupleMaster *explainTuple, ComTdb *tdb, Generator *generator) {
  NAString description;
  ...
  } else if (listOfUpdSubsetRows_.entries() > 0) {
...
ExplainTuple *DDLExpr::addSpecificExplainInfo(ExplainTupleMaster *explainTuple, ComTdb *tdb, Generator *generator) {
  char buf[200];
  NAString buffer;

  ExprNode *ddlNode = getDDLNode();

  if (buffer.isNull()) buffer = "explain_information: not available.";
...
ExplainTuple *GroupByAgg::addSpecificExplainInfo(ExplainTupleMaster *explainTuple, ComTdb *tdb, Generator *generator) {
  NAString buffer;
...
ExplainTuple *Union::addSpecificExplainInfo(ExplainTupleMaster *explainTuple, ComTdb *tdb, Generator *generator) {
...
ExplainTuple *Join::addSpecificExplainInfo(ExplainTupleMaster *explainTuple, ComTdb *tdb, Generator *generator) {
  ...
  if (isNaturalJoin())
    buffer += "natural ";
  else if (isInnerJoin() OR isAntiSemiJoin() OR(type == REL_TSJ_FLOW) OR(type == REL_NESTED_JOIN_FLOW))
    buffer += "inner ";
  ...
  if (isHashJoin()) {
    ...
    if (getEquiJoinPredicates().isEmpty()) buffer += "(cross product) ";
  ...
  if (isHashJoin()) {
    if (((ComTdbHashj *)tdb)->useVariableLength()) buffer += "variable_length_tuples: yes ";
    ...
    if (hj->getMinMaxHostvars().entries()) {
      ExprNode *minMaxCols = hj->getMinMaxCols().rebuildExprTree(ITM_ITEM_LIST);
      ExprNode *minMaxHostvars = hj->getMinMaxHostvars().rebuildExprTree(ITM_ITEM_LIST);

      buffer += "min_max_cols: ";
      minMaxCols->unparse(buffer, OPTIMIZER_PHASE, EXPLAIN_FORMAT);
      buffer += " ";
      buffer += "min_max_hostvars: ";
      minMaxHostvars->unparse(buffer, OPTIMIZER_PHASE, EXPLAIN_FORMAT);
      buffer += " ";
    }

    if (((ComTdbHashj *)tdb)->beforePredOnOuterOnly()) buffer += "other_join_pred_first: yes ";
  }
...
static NABoolean isInternalCQD(DefaultConstants attr) {
  if ((attr == SESSION_ID) || (attr == SESSION_IN_USE) || (attr == VOLATILE_CATALOG) ||
...
NABoolean displayDuringRegressRun(DefaultConstants attr) {
  if ((attr == TRAF_READ_OBJECT_DESC) || (attr == TRAF_STORE_OBJECT_DESC) || (attr == ALLOW_INCOMPATIBLE_ASSIGNMENT) ||
      (attr == ALLOW_INCOMPATIBLE_COMPARISON) || (attr == ALLOW_INCOMPATIBLE_OPERATIONS) ||
      (attr == ALLOW_FIRSTN_IN_SUBQUERIES) || (attr == ALLOW_ORDER_BY_IN_SUBQUERIES) ||
      (attr == GROUP_OR_ORDER_BY_EXPR) || (attr == TRAF_USE_STORED_STATS) ||
      (attr == TRAF_USTAT_GENERATE_STORED_STATS) || (attr == TRAF_ENABLE_METADATA_LOAD_IN_CACHE) ||
      (attr == TRAF_LOB_VERSION2))
    return FALSE;
  else
    return TRUE;
}

ExplainTuple *RelRoot::addSpecificExplainInfo(ExplainTupleMaster *explainTuple, ComTdb *tdb, Generator *generator) {
  NAString statement;
  ...
  if ((child(0) && child(0)->castToRelExpr()->getOperatorType() == REL_DDL) && (sqlmxRegress)) {
  ...
  NABoolean explainStrategyParams = (CmpCommon::getDefault(EXPLAIN_STRATEGIZER_PARAMETERS) == DF_ON);
  NABoolean isTRoot = isTrueRoot();

  if (explainStrategyParams && (optimizationBudget > 0) && isTRoot) {
    char buffer[500];

    snprintf(buffer, 500,
             "InitialCPUCost: %.8e InitialScanCost: %.8e InitialCost: %.8e BudgetFactor: %e OrigOptBudget: %e "
             "OptBudget: %e Pass1Tasks: %f TaskFactor: %e nComplexity: %f 2nComplexity: %f n2Complexity: %f "
             "n3Complexity: %f fullComplexity: %f enumPotential: %d ",
             ((float)CURRSTMT_OPTDEFAULTS->getCpuCost()), ((float)CURRSTMT_OPTDEFAULTS->getScanCost()),
             ((float)CURRSTMT_OPTDEFAULTS->getTotalCost()), ((float)CURRSTMT_OPTDEFAULTS->getBudgetFactor()),
             ((float)CURRSTMT_OPTDEFAULTS->getOriginalOptimizationBudget()),
             ((float)CURRSTMT_OPTDEFAULTS->getOptimizationBudget()), ((float)CURRSTMT_OPTDEFAULTS->getPass1Tasks()),
             ((float)CURRSTMT_OPTDEFAULTS->getTaskFactor()), ((float)CURRSTMT_OPTDEFAULTS->getNComplexity()),
             ((float)CURRSTMT_OPTDEFAULTS->get2NComplexity()), ((float)CURRSTMT_OPTDEFAULTS->getN2Complexity()),
             ((float)CURRSTMT_OPTDEFAULTS->getN3Complexity()), ((float)CURRSTMT_OPTDEFAULTS->getExhaustiveComplexity()),
...
ExplainTuple *Exchange::addSpecificExplainInfo(ExplainTupleMaster *explainTuple, ComTdb *tdb, Generator *generator) {
  ...
  if (isEspExchange() && !isExtractConsumer_) {  // case when we have the nodes split_top, send_top, send_bottom,
    // and split_bottom
    ...
    /* Comment out. Record_length now is a common explain attribute
  ...
  if (CmpCommon::getDefault(COMPRESSED_INTERNAL_FORMAT_EXPLAIN) == DF_ON && isEspExchange()) {
    if (tdb->isCIFON()) {
      description += "CIF: ON ";
    } else {
      description += "CIF: OFF ";
  ...
  if (splitBottomTdb->getQueryUsesSM()) description += "seamonster_query: yes ";

  if (splitBottomTdb->getExchangeUsesSM()) description += "seamonster_exchange: yes ";

  if (getScanFilterExpressions().entries()) {
    ExprNode *minMaxExprs = getScanFilterExpressions().rebuildExprTree(ITM_ITEM_LIST);

    NAString buffer = "min_max_hostvars: ";
    minMaxExprs->unparse(buffer, OPTIMIZER_PHASE, EXPLAIN_FORMAT);
    buffer += " ";
...
ExplainTuple *Sort::addSpecificExplainInfo(ExplainTupleMaster *explainTuple, ComTdb *tdb, Generator *) {
  ...
  if (sortTdb->topNSortEnabled() && sortNRows()) description += "topn_enabled: yes ";
...
short GenericUpdate::addSpecificExplainInfo(ExplainTupleMaster *explainTuple, ComTdb *tdb, Generator *generator,
                                            NAString &description) {
  const NATable *natable = getIndexDesc()->getPrimaryTableDesc()->getNATable();
  ...
  NAString tableName;
  if ((indexIUD) || (getOptStoi() && getOptStoi()->getStoi()->isView()))
  ...
  */
  ...
  if (getUpdateCKorUniqueIndexKey()) description += " (implements update of clustering key or unique index key) ";
  if (natable->isPartitionNameSpecified()) {
    // this is an index iud
    description += "partition_name: ";
    const NodeMapEntry *nmapentry =
    ...
    if (nmapentry && nmapentry->getGivenName()) description += nmapentry->getGivenName();
  ...
  if (natable->isSeabaseTable() && (((ComTdbHbaseAccess *)tdb)->getTrafLoadFlushSize() > 0)) {
    char lbuf[20];
  ...
  if (natable->isSeabaseTable()) {
    ...
    if (natable->incrBackupEnabled()) description += "incremental_backup: enabled ";
  }

  return 0;
}

ExplainTuple *GenericUpdate::addSpecificExplainInfo(ExplainTupleMaster *explainTuple, ComTdb *tdb,
                                                    Generator *generator) {
  NAString description;
...
ExplainTuple *ProbeCache::addSpecificExplainInfo(ExplainTupleMaster *explainTuple, ComTdb *, Generator *generator) {
  char buf[256];
...
ExplainTuple *ExeUtilWnrInsert::addSpecificExplainInfo(ExplainTupleMaster *explainTuple, ComTdb *tdb,
                                                       Generator *generator) {
  char buf[1024];
  ComTdbExeUtilAqrWnrInsert *myTdb = (ComTdbExeUtilAqrWnrInsert *)tdb;

  sprintf(buf, "lock_target_table: %s ", myTdb->doLockTarget() ? "ON" : "OFF");
  explainTuple->setDescription(buf);
...
ExplainTuple *ExeUtilCreateTableAs::addSpecificExplainInfo(ExplainTupleMaster *explainTuple, ComTdb *tdb,
  ...
  char buf[maxBufLen];
  snprintf(buf, maxBufLen, "CreateQuery: %s ", (ctQuery_.length() > 0 ? ctQuery_.data() : "NULL"));
  explainTuple->setDescription(buf);

  snprintf(buf, maxBufLen, "InsertQuery: %s ", (viQuery_.length() > 0 ? viQuery_.data() : "NULL"));
  explainTuple->setDescription(buf);

  snprintf(buf, maxBufLen, "UpsertLoadQuery: %s ", (siQuery_.length() > 0 ? siQuery_.data() : "NULL"));
  explainTuple->setDescription(buf);

  snprintf(buf, maxBufLen, "UpdStatsQuery: %s ", (usQuery_.length() > 0 ? usQuery_.data() : "NULL"));
  explainTuple->setDescription(buf);
...
TrafDesc *ExplainFunc::createVirtualTableDesc() {
  ...
  // set ngram index flag
  indexDesc->indexesDesc()->setNgram(FALSE);
  indexDesc->indexesDesc()->setPartLocalBaseIndex(FALSE);
  indexDesc->indexesDesc()->setPartLocalIndex(FALSE);
  indexDesc->indexesDesc()->setPartGlobalIndex(FALSE);
...
ExplainTuple *ExeUtilLongRunning::addSpecificExplainInfo(ExplainTupleMaster *explainTuple, ComTdb *tdb,
                                                         Generator *generator) {
  char buf[256];
...
ExplainTuple *ControlRunningQuery::addSpecificExplainInfo(ExplainTupleMaster *explainTuple, ComTdb *tdb,
                                                          Generator *generator) {
  ...
  if (crqTdb->getAction() == ComTdbCancel::CancelByQid)
    ...
    char npBuf[80];
    sprintf(npBuf, "nid_pid: %d,%d ", crqTdb->getCancelNid(), crqTdb->getCancelPid());
    buffer += npBuf;
    addQidToExplain = false;
  } else if (crqTdb->getAction() == ComTdbCancel::Activate)
    buffer += "activate ";
  else {
    GenAssert(crqTdb->getAction() == ComTdbCancel::Suspend, "Missing explain logic for ControlRunningQuery action_.");
  ...
  if ((crqTdb->getAction() == ComTdbCancel::CancelByPname) || (crqTdb->getAction() == ComTdbCancel::CancelByNidPid)) {
    char numBuf[32];
...
ExplainTuple *ExeUtilCompositeUnnest::addSpecificExplainInfo(ExplainTupleMaster *explainTuple, ComTdb *tdb,
                                                             Generator *generator) {
  //  NAString unnestName = virtTableName_ + "." + colName_;
  NAString unnestName = virtTableName_ + "_" + colName_;
  explainTuple->setTableName(unnestName);
