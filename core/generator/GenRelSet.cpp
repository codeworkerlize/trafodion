...
short MergeUnion::codeGen(Generator *generator) {
  ...
  //
  GenAssert((child(0) AND child(1))OR(NOT child(0) AND NOT(child(1))), "MergeUnion -- missing one child");
  ...
  if (child(0) && child(1)) {
    ...
    if (child(0)->getGroupAttr()->getCharacteristicOutputs().isEmpty()) rowsFromLeft = FALSE;
    ...
    generator->setCriDesc(generator->getCriDesc(Generator::UP), Generator::DOWN);
    ...
    if (child(1)->getGroupAttr()->getCharacteristicOutputs().isEmpty()) rowsFromRight = FALSE;
  ...
  for (i = 0; i < colMapTable().entries(); i++) {
    ...
    if (vidu_node->getResult().getType().getTypeQualifier() != NA_ROWSET_TYPE) {
      ...
      SQLRowset *newRowset =
          new (generator->wHeap()) SQLRowset(generator->wHeap(), rowsetInfo->getElementType(),
                                             rowsetInfo->getMaxNumElements(), rowsetInfo->getNumElements());
      newRowset->useTotalSize() = TRUE;
      cnode = new (generator->wHeap()) Cast(((vidu_node->getLeftSource()).getValueDesc())->getItemExpr(), newRowset);
    }
    ...
    if (vidu_node->getResult().getType().getTypeQualifier() != NA_ROWSET_TYPE) {
      ...
      SQLRowset *newRowset =
          new (generator->wHeap()) SQLRowset(generator->wHeap(), rowsetInfo->getElementType(),
                                             rowsetInfo->getMaxNumElements(), rowsetInfo->getNumElements());
      newRowset->useTotalSize() = TRUE;
      cnode = new (generator->wHeap()) Cast(((vidu_node->getRightSource()).getValueDesc())->getItemExpr(), newRowset);
    }
  ...
  if (child(0) && child(1)) {
    exp_gen->generateContiguousMoveExpr(left_val_id_list,
                                        ...
                                        1, returned_desc->noTuples() - 1, ExpTupleDesc::SQLARK_EXPLODED_FORMAT,
                                        tuple_length, &left_expr, &tuple_desc, ExpTupleDesc::SHORT_FORMAT);

    exp_gen->generateContiguousMoveExpr(right_val_id_list,
                                        0,  // don't add convert nodes
                                        1, returned_desc->noTuples() - 1, ExpTupleDesc::SQLARK_EXPLODED_FORMAT,
                                        tuple_length, &right_expr);
  }
  ...
  for (i = 0; i < colMapTable().entries(); i++) {
    ...
    Attributes *attr = generator
                           ->addMapInfoToThis(my_map_table, vidu_node->getValueId(),
                                              generator->getMapInfo(left_val_id_list[i])->getAttr())
                           ->getAttr();
    attr->setAtp(0);
  }
  ...
  if (getMergeExpr()) {
    ...
    exp_gen->generateExpr(mergeExpr->getValueId(), ex_expr::exp_SCAN_PRED, &merge_expr);
  }
  ...
  if (NOT condExpr().isEmpty()) {
    ...
    exp_gen->generateExpr(condExp->getValueId(), ex_expr::exp_SCAN_PRED, &cond_expr);
  }
  ...
  if (NOT trigExceptExpr().isEmpty()) {
    ...
    exp_gen->generateExpr(trigExp->getValueId(), ex_expr::exp_SCAN_PRED, &trig_expr);
  }
  ...
  // row.
  UInt32 outputBuffSize = MAXOF(getDefault(GEN_UN_BUFFER_SIZE), tuple_length);
  outputBuffSize = SqlBufferNeededSize(1,  // # of tuples
                                       outputBuffSize, SqlBuffer::NORMAL_);

  ComTdbUnion *union_tdb = new (space) ComTdbUnion(
      left_child_tdb, right_child_tdb, left_expr, right_expr, merge_expr, cond_expr, trig_expr,
      tuple_length,                   // unioned rowlen
      returned_desc->noTuples() - 1,  // tupp index for
                                      // unioned buffer
      given_desc, returned_desc, (queue_index)getDefault(GEN_UN_SIZE_DOWN), (queue_index)getDefault(GEN_UN_SIZE_UP),
      (Cardinality)(getInputCardinality() * getEstRowsUsed()).getValue(), getDefault(GEN_UN_NUM_BUFFERS),
      outputBuffSize, getOrderedUnion(),
      getBlockedUnion(),  //++ Triggers -
      hasNoOutputs(),     //++ Triggers -
      rowsFromLeft, rowsFromRight, afterUpdate, getInNotAtomicStatement());

  union_tdb->setRecordLength(getGroupAttr()->getRecordLength());
  ...
  if (!generator->explainDisabled()) {
    generator->setExplainTuple(addExplainInfo(union_tdb, leftExplainTuple, rightExplainTuple, generator));
  }
...
short Union::codeGen(Generator *) { return -1; }
