...
//
//
RelExpr *PhysUnPackRows::preCodeGen(Generator *generator, const ValueIdSet &externalInputs,
  ...
  //
  unPackExpr().replaceVEGExpressions(availableValues, getGroupAttr()->getCharacteristicInputs());

  packingFactor().replaceVEGExpressions(availableValues, getGroupAttr()->getCharacteristicInputs());

  if (rowwiseRowset()) {
    if (rwrsInputSizeExpr())
      ((ItemExpr *)rwrsInputSizeExpr())
          ->replaceVEGExpressions(availableValues, getGroupAttr()->getCharacteristicInputs());
...
short PhysUnPackRows::codeGen(Generator *generator) {
  ...
  ex_cri_desc *returnedCriDesc = new (space) ex_cri_desc(givenCriDesc->noTuples() + 1, space);

  ex_cri_desc *workCriDesc = new (space) ex_cri_desc(givenCriDesc->noTuples() + 2, space);

  // unPackCols is the next to the last Tp in Atp 0, the work ATP.
  // and the last Tp in the returned ATP.
  //
  const int unPackColsAtpIndex = workCriDesc->noTuples() - 2;
  const int unPackColsAtp = 0;

  // The length of the new tuple which will contain the columns
  // generated by unPackRows
  //
  int unPackColsTupleLen;
  ...
  if (indexValue() != NULL_VALUE_ID) {
    ...
    expGen->processValIdList(indexValueList, ExpTupleDesc::SQLARK_EXPLODED_FORMAT, indexValueTupleLen, indexValueAtp,
                             indexValueAtpIndex, &indexValueTupleDesc, ExpTupleDesc::SHORT_FORMAT);

    GenAssert(indexValueTupleLen == sizeof(int), "UnPackRows::codeGen: Internal Error");
  }
  ...
  int packingFactorTupleLen;

  if (packingFactor().entries() > 0) {
    expGen->generateContiguousMoveExpr(packingFactor(), -1, unPackColsAtp, unPackColsAtpIndex,
                                       ExpTupleDesc::SQLARK_EXPLODED_FORMAT, packingFactorTupleLen, &packingFactorExpr);

    GenAssert(packingFactorTupleLen == sizeof(int), "UnPackRows::codeGen: Internal Error");
  }
  ...
  expGen->genGuardedContigMoveExpr(selectionPred(), getGroupAttr()->getCharacteristicOutputs(),
                                   0,  // No Convert Nodes added
                                   unPackColsAtp, unPackColsAtpIndex, ExpTupleDesc::SQLARK_EXPLODED_FORMAT,
                                   unPackColsTupleLen, &unPackColsExpr, &unPackColsTupleDesc,
                                   ExpTupleDesc::SHORT_FORMAT);

  workCriDesc->setTupleDescriptor(unPackColsAtpIndex, unPackColsTupleDesc);

  returnedCriDesc->setTupleDescriptor(unPackColsAtpIndex, unPackColsTupleDesc);
  ...
  int rwrsInputSizeExprLen = 0;
  int rwrsMaxInputRowlenExprLen = 0;
  int rwrsBufferAddrExprLen = 0;

  const int rwrsAtp = 1;
  const int rwrsAtpIndex = workCriDesc->noTuples() - 2;
  ExpTupleDesc *rwrsTupleDesc = 0;
  ValueIdList rwrsVidList;
  if (rowwiseRowset()) {
    rwrsVidList.insert(this->rwrsInputSizeExpr()->getValueId());
    expGen->generateContiguousMoveExpr(rwrsVidList, 0 /*don't add conv nodes*/, rwrsAtp, rwrsAtpIndex,
                                       ExpTupleDesc::SQLARK_EXPLODED_FORMAT, rwrsInputSizeExprLen, &rwrsInputSizeExpr,
                                       &rwrsTupleDesc, ExpTupleDesc::SHORT_FORMAT);

    rwrsVidList.clear();
    rwrsVidList.insert(this->rwrsMaxInputRowlenExpr()->getValueId());
    expGen->generateContiguousMoveExpr(rwrsVidList, 0 /*don't add conv nodes*/, rwrsAtp, rwrsAtpIndex,
                                       ExpTupleDesc::SQLARK_EXPLODED_FORMAT, rwrsMaxInputRowlenExprLen,
                                       &rwrsMaxInputRowlenExpr, &rwrsTupleDesc, ExpTupleDesc::SHORT_FORMAT);

    rwrsVidList.clear();
    rwrsVidList.insert(this->rwrsBufferAddrExpr()->getValueId());
    expGen->generateContiguousMoveExpr(rwrsVidList, 0 /*don't add conv nodes*/, rwrsAtp, rwrsAtpIndex,
                                       ExpTupleDesc::SQLARK_EXPLODED_FORMAT, rwrsBufferAddrExprLen, &rwrsBufferAddrExpr,
                                       &rwrsTupleDesc, ExpTupleDesc::SHORT_FORMAT);

    expGen->assignAtpAndAtpIndex(rwrsOutputVids(), unPackColsAtp, unPackColsAtpIndex);
  }
  ...
  for (ValueId outputValId = getGroupAttr()->getCharacteristicOutputs().init();
    ...
    generator->addMapInfoToThis(localMapTable, outputValId, generator->getMapInfo(outputValId)->getAttr());
  ...
  if (rowwiseRowset()) {
    unPackTdb = new (space) ComTdbUnPackRows(
        NULL,  // childTdb,
        rwrsInputSizeExpr, rwrsMaxInputRowlenExpr, rwrsBufferAddrExpr, rwrsAtpIndex, givenCriDesc, returnedCriDesc,
        workCriDesc, 16, 1024, (Cardinality)getGroupAttr()->getOutputLogPropList()[0]->getResultCardinality().value(),
        2, 20000);
  } else {
    ...
    double memoryLimitPerInstance =
        ...
        2;  // At runtime (ExUnpackRowsTcb::ExUnpackRowsTcb) we allocate twice the size of the queue. So ensure it fits
            // in to half the specified limit.
    ...
    unPackTdb = new (space)
        ComTdbUnPackRows(childTdb, packingFactorExpr, unPackColsExpr, unPackColsTupleLen, unPackColsAtpIndex,
                         indexValueAtpIndex, givenCriDesc, returnedCriDesc, workCriDesc, 16, upQueueSize,
  ...
  if (!generator->explainDisabled()) {
    generator->setExplainTuple(addExplainInfo(unPackTdb, childExplainTuple, 0, generator));
  }
