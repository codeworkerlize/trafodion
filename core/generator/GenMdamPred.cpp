...
short BiLogic::mdamPredGen(Generator *generator, MdamPred **head, MdamPred **tail, MdamCodeGenHelper &mdamHelper,
                           ItemExpr *parent) {
  short rc = 0;
  ...
  if (getOperatorType() == ITM_OR) {
    ...
    if (CmpCommon::getDefault(RANGESPEC_TRANSFORMATION) != DF_ON) {
      ...
      GenAssert(child0->getOperatorType() != ITM_AND, "mdamPredGen: unexpected AND under an OR.");
      GenAssert(child1->getOperatorType() != ITM_AND, "mdamPredGen: unexpected AND under an OR.");
    }
  } else  // getOperatorType() must be ITM_AND
  {
    ...
    GenAssert(child0->getOperatorType() != ITM_OR, "mdamPredGen: unexpected OR under an AND.");
    GenAssert(child1->getOperatorType() != ITM_OR, "mdamPredGen: unexpected OR under an AND.");
  }
...
short BiRelat::mdamPredGen(Generator *generator, MdamPred **head, MdamPred **tail, MdamCodeGenHelper &mdamHelper,
                           ItemExpr *parent) {
  short rc = 0;  // assume success

  enum MdamPred::MdamPredType predType = MdamPred::MDAM_EQ;  // just to initialize
  ...
  if (child1->getValueId() == keyColumn) {
    ...
    keyValue = child0;
    GenAssert(child0->getValueId() != keyColumn, "mdamPredGen:  unexpected form for key predicate.");
  ...
  NABoolean generateNarrow = keyValue->getValueId().getType().errorsCanOccur(*mdamHelper.getTargetType());

#ifdef _DEBUG
  if ((generateNarrow) && (getenv("NO_NARROWS")))  // for testing -- allows turning off Narrows
    generateNarrow = FALSE;
#endif

  NABoolean isVC2 = FALSE;
  if (keyColumn.getType().getTypeQualifier() == NA_CHARACTER_TYPE) {
    const CharType *char_type = (const CharType *)&keyColumn.getType();
  ...
  if (generateNarrow) {
    vnode = new (generator->wHeap()) Narrow(keyValue, mdamHelper.getDataConversionErrorFlag(),
                                            mdamHelper.getTargetType()->newCopy(generator->wHeap()));
    if (isVC2) ((Narrow *)vnode)->setPadUseZero(TRUE);
  } else {
    vnode = new (generator->wHeap()) Cast(keyValue, mdamHelper.getTargetType()->newCopy(generator->wHeap()));
    if (isVC2) ((Cast *)vnode)->setPadUseZero(TRUE);
  }
  ...
  rc = generator->getExpGenerator()->generateContiguousMoveExpr(vnodeList,
                                                                0,  // don't add convert nodes
                                                                mdamHelper.getAtp(), mdamHelper.getAtpIndex(),
                                                                mdamHelper.getTupleDataFormat(),
                                                                dummyLen,  // out
                                                                &vexpr);

  *head = *tail = new (generator->getSpace()) MdamPred(mdamHelper.getDisjunctNumber(), predType, vexpr);
...
void BiRelat::getMdamPredDetails(Generator *generator, MdamCodeGenHelper &mdamHelper, MdamPred::MdamPredType &predType,
                                 ex_expr **vexpr) {
  ...
  short rc = generator->getExpGenerator()->generateContiguousMoveExpr(vnodeList,
                                                                      0,  // don't add convert nodes
                                                                      mdamHelper.getAtp(), mdamHelper.getAtpIndex(),
                                                                      mdamHelper.getTupleDataFormat(),
                                                                      dummyLen,  // out
  ...
  GenAssert(rc == 0,
            "generateContiguousMoveExpr() returned error when called "
            "from BiRelat::getMdamPredDetails().");
}

// Generate MDAM_BETWEEN mdam predicate.
void BiLogic::mdamPredGenSubrange(Generator *generator, MdamPred **head, MdamPred **tail,
  ...
  if (predType1 == MdamPred::MDAM_GE)
    ...
    GenAssert(FALSE,
              "mdamPredGenSubrange: subrange does not conform to "
  ...
  *head = *tail =
      new (generator->getSpace()) MdamPred(mdamHelper.getDisjunctNumber(), MdamPred::MDAM_BETWEEN, vexpr1, vexpr2,
                                           val1Inclusive, val2Inclusive, (Int16)mdamHelper.isDescending());
}

short ItemExpr::mdamPredGen(Generator *, MdamPred **, MdamPred **, MdamCodeGenHelper &, ItemExpr *) {
  GenAssert(0, "Should never reach ItemExpr::mdamPredGen");
  return -1;
}

short TriRelational::mdamPredGen(Generator *generator, MdamPred **head, MdamPred **tail, MdamCodeGenHelper &mdamHelper,
                                 ItemExpr *parent) {
  ...
  if (generateNarrow) {
    vnode = new (generator->wHeap())
        Narrow(keyValue, mdamHelper.getDataConversionErrorFlag(), mdamHelper.getTargetType()->newCopy());
  } else {
    vnode = new (generator->wHeap()) Cast(keyValue, mdamHelper.getTargetType()->newCopy());
  }
  ...
  hnode = generator->getExpGenerator()->createExprTree("CASE WHEN @B1 THEN @A2 ELSE @A3 END", 0,
  ...
  generator->getExpGenerator()->processValIdList(vnodeList, mdamHelper.getTupleDataFormat(),
                                                 tupleLength,  // out
                                                 mdamHelper.getAtp(), mdamHelper.getAtpIndex());
  ...
  Attributes *dataCEFAttr = (generator->getMapInfo(dataCEFId))->getAttr();
  ...
  rc = generator->getExpGenerator()->generateListExpr(vnodeList, ex_expr::exp_ARITH_EXPR, &vexpr);
...
short UnLogic::mdamPredGen(Generator *generator, MdamPred **head, MdamPred **tail, MdamCodeGenHelper &mdamHelper,
                           ItemExpr *parent) {
  short rc = 0;  // assume success

  enum MdamPred::MdamPredType predType = MdamPred::MDAM_ISNULL;  // just to initialize
  ...
  *head = *tail = new (generator->getSpace())
      MdamPred(mdamHelper.getDisjunctNumber(), predType, 0 /* no expression for IS NULL and IS NOT NULL */);
