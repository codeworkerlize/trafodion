...
short ProbeCache::codeGen(Generator *generator) {
  ...
  if (!selectionPred().isEmpty()) {
    ...
    exp_gen->generateExpr(selPredTree->getValueId(), ex_expr::exp_SCAN_PRED, &selectPred);
  }
  ...
  if (innerRecExpr == NULL) {
    estimatedMemory = numCachedProbes_ * encodedInputLength;
  ...
  if (pcNumEntries < pdq2) pcNumEntries = pdq2;
  ...
  if (numInnerTuples_ > 0) {
    estimatedMemory = (numInnerTuples_ * innerRecLength) + (pcNumEntries * encodedInputLength);
    if (estimatedMemory > memoryLimitPerInstance) {
      numInnerTuples_ = memoryLimitPerInstance / (innerRecLength + encodedInputLength);
  ...
  ComTdbProbeCache *probeCacheTdb = new (space)
      ComTdbProbeCache(hvExpr, encodeInputExpr, innerRecExpr, selectPred, encodedInputLength, innerRecLength,
                       pcNumEntries, returned_desc->noTuples() - 1, hashValIdx, encodedProbeDataIdx, innerRowDataIdx,
                       child_tdb, given_desc, returned_desc, pDownSize, pUpSize,
                       (Cardinality)(getInputCardinality() * getEstRowsUsed()).getValue(), numInnerTuples_);

  probeCacheTdb->setRecordLength(getGroupAttr()->getRecordLength());
  generator->initTdbFields(probeCacheTdb);

  // Make sure that the ProbeCache queues were not set too large
  if ((probeCacheTdb->getInitialQueueSizeDown() > pDownSize) || (probeCacheTdb->getInitialQueueSizeUp() > pUpSize)) {
    probeCacheTdb->setQueueResizeParams(pDownSize, pUpSize, probeCacheTdb->getQueueResizeLimit(),
...
CostScalar ProbeCache::getEstimatedRunTimeMemoryUsage(Generator *generator, NABoolean perNode, int *numStreams) {
  ...
  if (phyProp) {
    ...
    double memoryLimitPerInstance =
        ActiveSchemaDB()->getDefaults().getAsLong(EXE_MEMORY_FOR_PROBE_CACHE_IN_MB) * 1024 * 1024;
    if (totalMemory > memoryLimitPerInstance) totalMemory = memoryLimitPerInstance;
    totalMemory *= numOfStreams;
  }
  if (numStreams != NULL) *numStreams = numOfStreams;
  if (perNode)
    totalMemory /= MINOF(MAXOF(CURRCONTEXT_CLUSTERINFO->getTotalNumberOfCPUs(), 1), numOfStreams);
  else
    totalMemory /= numOfStreams;
  return totalMemory;
}
