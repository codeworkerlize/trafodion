...
short MapValueIds::codeGen(Generator *generator) {
  ...
  for (CollIndex i = 0; i < topValues.entries(); i++) {
    ...
    if (mappedId != valId) {
      ...
      // target (upper) value.
      //
      ItemExpr *convValue = new (generator->wHeap()) Cast(mappedId.getItemExpr(), &(valId.getType()));
...
class NodeCountHelper {
 ...
 public:
  ...
  inline NABoolean operator==(const NodeCountHelper &other) const { return (nodeDiskCount_ == other.nodeDiskCount_); }
...
class NodeHashHelper {
 public:
  NodeHashHelper(const char *partn) {
    ...
    *dotPos = '\0';
    memmove(nodeDiskName_.nodeDiskName_, partn, sizeof nodeDiskName_.nodeDiskName_);
...
static void replaceBaseValue(ItemExpr *incomingExpr, ItemExpr *resultExpr, CollHeap *heap, NABoolean setFound) {
  ...
  if ((nc != 0) && (found == FALSE))  // have not found yet.
  {
    ...
    if (incomingExpr->getOperatorType() == ITM_ASSIGN && (child1 != NULL && child1->getOperatorType() == ITM_ASSIGN)) {
      newExpr = new (heap) BiArith(ITM_PLUS, resultExpr, child1);

      newExpr->synthTypeAndValueId(TRUE);
      // set type to original type after if has been changed to BigNum above
      newExpr->getValueId().changeType(new (heap) SQLLargeInt(heap, 1 /* signed */, 0 /* not null */));
...
//
/////////////////////////////////////////////////////////
const char *DDLExpr::getVirtualTableName() { return (producesOutput() ? "DDL_EXPR__" : NULL); }
  ...
  if (producesOutput()) {
    table_desc =
        Generator::createVirtualTableDesc(getVirtualTableName(),
                                          ...
                                          ComTdbDDL::getVirtTableNumCols(), ComTdbDDL::getVirtTableColumnInfo(),
                                          ComTdbDDL::getVirtTableNumKeys(), ComTdbDDL::getVirtTableKeyInfo());
  }
  return table_desc;
}
...
short DDLExpr::codeGen(Generator *generator) {
  ...
  if (producesOutput()) {
    ...
    short rc = processOutputRow(generator, work_atp, ddl_row_atp_index, returnedDesc);
    if (rc) {
      return -1;
    }
  }
  ...
  if (returnStatus_) {
    ComTdbDDLwithStatus *ddl_ws_tdb = new (space)
        ComTdbDDLwithStatus(ddlStmt, strlen(ddlStmt), (Int16)getDDLStmtTextCharSet(), catSchNameStr,
                            strlen(catSchNameStr), 0, 0,  // no input expr
                            0, 0,                         // no output expr
                            workCriDesc, (producesOutput() ? ddl_row_atp_index : 0), givenDesc, returnedDesc,
                            (queue_index)getDefault(GEN_DDL_SIZE_DOWN), (queue_index)getDefault(GEN_DDL_SIZE_UP),
                            getDefault(GEN_DDL_NUM_BUFFERS), getDefault(GEN_DDL_BUFFER_SIZE));
    ...
    if (isCleanup_) {
      ...
      StmtDDLCleanupObjects *co = getExprNode()->castToElemDDLNode()->castToStmtDDLCleanupObjects();

      if (co->checkOnly()) ddl_ws_tdb->setCheckOnly(TRUE);

      if (co->returnDetails()) ddl_ws_tdb->setReturnDetails(TRUE);
    ...
    ddl_tdb =
        new (space) ComTdbDDL(ddlStmt, strlen(ddlStmt), (Int16)getDDLStmtTextCharSet(), catSchNameStr,
                              strlen(catSchNameStr), 0, 0,  // no input expr
                              0, 0,                         // no output expr
                              workCriDesc, (producesOutput() ? ddl_row_atp_index : 0), givenDesc, returnedDesc,
                              (queue_index)getDefault(GEN_DDL_SIZE_DOWN), (queue_index)getDefault(GEN_DDL_SIZE_UP),
  ...
  if (isHbase_) {
    ...
    if (hbaseDDLNoUserXn_) ddl_tdb->setHbaseDDLNoUserXn(TRUE);
  }

  ddl_tdb->setExecuteInESP(isExecuteInESP());

  generator->initTdbFields(ddl_tdb);

  if (!generator->explainDisabled()) {
    generator->setExplainTuple(addExplainInfo(ddl_tdb, 0, 0, generator));
  }
  ...
  if (xnNeeded()) {
    ...
    else if (getExprNode() && getExprNode()->castToStmtDDLNode()->ddlXns() && (NOT hbaseDDLNoUserXn_)) {
...
short RelDumpLoad::codeGen(Generator *generator) { return DDLExpr::codeGen(generator); }

short RelGenLoadQueryCache::codeGen(Generator *generator) { return DDLExpr::codeGen(generator); }

short RelBackupRestore::codeGen(Generator *generator) { return DDLExpr::codeGen(generator); }
...
short ExeUtilMetadataUpgrade::codeGen(Generator *generator) {
  ...
  ComTdbDDLwithStatus *upgd_tdb = new (space) ComTdbDDLwithStatus(
      NULL, 0, 0, NULL, 0, 0, 0,  // no input expr
      0, 0,                       // no output expr
      NULL, 0, givenDesc, returnedDesc, (queue_index)getDefault(GEN_DDL_SIZE_DOWN),
      (queue_index)getDefault(GEN_DDL_SIZE_UP), getDefault(GEN_DDL_NUM_BUFFERS), getDefault(GEN_DDL_BUFFER_SIZE));
  ...
  if (!generator->explainDisabled()) {
    generator->setExplainTuple(addExplainInfo(upgd_tdb, 0, 0, generator));
  }
...
short FirstN::codeGen(Generator *generator) {
  ...
  // work 1 for REL_HBASE_ACCESS and NESTED_JOIN
  // todo for REL_EXCHANGE
  RelExpr *childExpr0 = child(0)->castToRelExpr();
  if (childExpr0 && (CmpCommon::getDefault(PUSHDOWN_LIMIT_OPT) == DF_ON)) {
    if (childExpr0->getOperatorType() == REL_HBASE_ACCESS) {
      HbaseAccess *ha0 = (HbaseAccess *)childExpr0;
      if (ha0->executorPred().isEmpty()) child(0)->setNeedPushDownLimit(TRUE);
    } else if (childExpr0->getOperatorType() == REL_NESTED_JOIN) {
      NestedJoin *nj0 = (NestedJoin *)childExpr0;
      if (nj0->isIndexJoin()) {
        child(0)->setNeedPushDownLimit(TRUE);
      }
    }
  }
  ...
  if (firstNRowsParam_) {
    ...
    NAType *newNAT = firstNRowsParam_->getValueId().getType().newCopy(generator->wHeap());
    ...
    expGen->generateContiguousMoveExpr(vidL,
                                       0,  // no convert nodes,
                                       work_atp, work_atp_index, ExpTupleDesc::SQLARK_EXPLODED_FORMAT, firstNValLen,
  ...
  if ((child(0)->getOperatorType() == REL_SORT) && firstNPredUsed()) {
    ((ComTdbSort *)child_tdb)->setRaiseFirstNError(TRUE);
  }

  ComTdbFirstN *firstN_tdb =
      new (space) ComTdbFirstN(child_tdb, firstNrows, firstNRowsExpr, work_cri_desc, given_desc, returned_desc,
                               child_tdb->getMaxQueueSizeDown(), child_tdb->getMaxQueueSizeUp(), 1, 4096);

  firstN_tdb->setRecordLength(getGroupAttr()->getRecordLength());

  generator->initTdbFields(firstN_tdb);

  if (!generator->explainDisabled()) {
    generator->setExplainTuple(addExplainInfo(firstN_tdb, childExplainTuple, 0, generator));
  }
...
TrafQuerySimilarityInfo *RelRoot::genSimilarityInfo(Generator *generator) {
  ...
  if (generator->getTrafSimTableInfoList().entries() > 0) qsi = new (space) TrafQuerySimilarityInfo(siList);

  CollIndex i = 0;

  for (CollIndex i = 0; i < generator->getTrafSimTableInfoList().entries(); i++) {
    TrafSimilarityTableInfo *genTsi = (TrafSimilarityTableInfo *)(generator->getTrafSimTableInfoList()[i]);

    char *genTablename = space->allocateAndCopyToAlignedSpace(genTsi->tableName(), str_len(genTsi->tableName()), 0);
    char *genRootDir = space->allocateAndCopyToAlignedSpace(genTsi->hdfsRootDir(), str_len(genTsi->hdfsRootDir()), 0);
    ...
    TrafSimilarityTableInfo *si =
        new (space) TrafSimilarityTableInfo(genTablename, genTsi->isHive(), genRootDir, genTsi->modTS(),
                                            genTsi->numPartnLevels(), NULL, genHdfsHostName, genTsi->hdfsPort());
...
short RelRoot::codeGen(Generator *generator) {
  // assume there is no min/max for type-1 through the query tree.
  generator->setStartMinMaxIndex(0);
  generator->setEndMinMaxIndex(0);
  ...
  if (isRootOfInternalRefresh()) generator->setInternalRefreshStatement();
  ...
  root_tdb->setRecordLength(getGroupAttr()->getRecordLength());
  ...
  if (accessOptions().accessType() != TransMode::ACCESS_TYPE_NOT_SPECIFIED_) {
    ...
    transMode->updateAccessModeFromIsolationLevel(TransMode::ATtoIL(accessOptions().accessType()));
  ...
  if (getHostArraysArea() && getHostArraysArea()->getRowwiseRowset()) {
    ...
    getHostArraysArea()->getBufferAttributes(packedFormat, compressed, dcompressInMaster, compressInMaster,
                                             partnNumInBuffer);
  ...
  for (i = 0; i < inputVars().entries(); i++) {
    ...
    if (item_expr->previousHostVar()) {
      ...
      if (i == j) {
        ...
        str_itoa(i, str2);
        str1 += str2;
        item_expr = new (generator->wHeap()) HostVar(str1, new (generator->wHeap()) SQLUnknown(generator->wHeap()));
    ...
    if ((op == ITM_HOSTVAR) || (op == ITM_DYN_PARAM)) {
      ...
      if (paramMode == COM_OUTPUT_COLUMN) continue;
    }
    ...
    {
      MapInfo *map = generator->addMapInfoToThis(generator->getLastMapTable(), val_id, NULL);
      ...
      if ((op == ITM_DYN_PARAM) && ((DynamicParam *)item_expr)->isRowInRowwiseRowset()) {
  ...
  Attributes **attrs = new (generator->wHeap()) Attributes *[newInputVars.entries()];
  ...
  for (i = 0; i < newInputVars.entries(); i++) {
    ...
    if (item_expr->getOperatorType() == ITM_UNIQUE_EXECUTE_ID) uniqueExecuteIdOffset = attrs[i]->getOffset();
    if (item_expr->getOperatorType() == ITM_GET_TRIGGERS_STATUS) {
      GenAssert(getTriggersList()->entries() > 0, "No triggers, yet TriggerStatusOffset != -1");
  ...
  if (updateCurrentOf()) {
    ...
    Attributes **attrs = new (generator->wHeap()) Attributes *[pkeyList().entries()];
    for (i = 0; i < pkeyList().entries(); i++) {
      attrs[i] = generator->addMapInfo(pkeyList()[i], NULL)->getAttr();
    }

    exp_gen->processAttributes(pkeyList().entries(), attrs, ExpTupleDesc::SQLARK_EXPLODED_FORMAT, pkey_len, 0 /*atp*/,
                               num_tupps /*atpIdx*/);
    num_tupps += 1;
  }
  ...
  if (cacheVars.entries() > 0) {
    cachedAttrs = new (generator->wHeap()) Attributes *[cacheVars.entries()];
    for (i = 0; i < cacheVars.entries(); i++) {
      cachedAttrs[i] = generator->addMapInfo(cacheVars[i], NULL)->getAttr();
    }

    exp_gen->processAttributes(cacheVars.entries(), cachedAttrs, ExpTupleDesc::SQLARK_EXPLODED_FORMAT, cacheVarsSize,
                               0 /*atp*/, num_tupps /*atpIdx*/);
    num_tupps += 1;
  }
  ...
  if (CmpCommon::getDefault(QUERY_CACHE_RUNTIME) == DF_ON) qCacheInfoIsClass = TRUE;

  char *parameterBuffer = (char *)new (space) char[cacheVarsSize];
  char *qCacheInfoBuf = NULL;
  if (qCacheInfoIsClass) {
    QCacheInfo *qCacheInfo = (QCacheInfo *)new (space) char[sizeof(QCacheInfo)];
  ...
  if (cseInfoList && CmpCommon::getDefault(CSE_CACHE_TEMP_QUERIES) == DF_OFF)
    for (CollIndex i = 0; i < cseInfoList->entries(); i++)
      if (cseInfoList->at(i)->usesATempTable()) generator->setNonCacheableCSEPlan(TRUE);
  ...
  if (rwrsVars.entries() > 0) {
    rwrsAttrs = new (generator->wHeap()) Attributes *[rwrsVars.entries()];
    for (i = 0; i < rwrsVars.entries(); i++) {
      rwrsAttrs[i] = generator->addMapInfo(rwrsVars[i], NULL)->getAttr();

      if (rwrsAttrs[i]->getCharSet() != CharInfo::ISO88591) useUnicodeDcompress = TRUE;
    }
  ...
  if (rwrsInfo) {
    rwrsInfo->setRwrsInfo(rwrsMaxSize, rwrsInputSizeIndex, rwrsMaxInputRowlenIndex, rwrsBufferAddrIndex,
                          rwrsPartnNumIndex, rwrsMaxInternalRowlen);

    if (rwrsInfo->rwrsIsCompressed()) rwrsInfo->setUseUnicodeDcompress(useUnicodeDcompress);
  }
  ...
  if ((newInputVars.entries() > 0) || (rwrsVars.entries() > 0)) {
    newInputVars.insert(rwrsVars);
    exp_gen->generateInputExpr(newInputVars, ex_expr::exp_INPUT_OUTPUT, &input_expr);
  }
  ...
  if (!generator->explainDisabled()) {
    ...
    if (generator->getExplainFragDirIndex() == NULL_COLL_INDEX) {
      ...
      generator->setExplainFragDirIndex(generator->getFragmentDir()->pushFragment(FragmentDir::EXPLAIN, 0));
    ...
    Space *explainSpace = generator->getFragmentDir()->getSpace(generator->getExplainFragDirIndex());
    ...
    compFragDir->setTopObj(generator->getExplainFragDirIndex(), (char *)explainDesc);
  }
  ...
  NABoolean isExtractConsumer = (childOperType() == REL_EXTRACT_SOURCE ? TRUE : FALSE);
  ...
  // If every scan is a parquet scan and every Parquet scan can
  // use Cpp reader, then set it at the generator level.
  NABoolean useCppReader = this->decideUseCppReaderForParquetScans();
  generator->setParquetUseCppReader(useCppReader);

  // If we can use cpp reader, or all scans are ORC, we can
  // use the native byte order bloom filters. This is a good
  // optimization since the keys of numeric types (such as integers)
  // will be in the native byte order during the hashing.
  generator->setBloomFilterUseNativeByteOrder(this->decideAllScansAreORCScans() || useCppReader);
  ...
  child(0)->codeGen(generator);
  ComTdb *child_tdb = (ComTdb *)(generator->getGenObj());
  if (child_tdb == (ComTdb *)NULL) childTdbIsNull = TRUE;

  // Remap the allocation of ESPs to Nodes/CPUs,
  // either by using a random approach or by using
  ...
  if (compExpr_.entries() > 0) {
    ...
    OperatorTypeEnum childType = child(0)->castToRelExpr()->getOperatorType();

    ComUInt32 numColumns = getRETDesc()->getDegree();

    if ((childType == REL_SP_PROXY || isExtractConsumer) && numColumns > 0) {
      ProxyFunc *proxy;
      ...
      if (childType == REL_SP_PROXY) {
        ...
        // between the two.
        GenAssert(childType == REL_EXCHANGE, "Child of root should be exchange for consumer query");
        GenAssert(child(0)->child(0), "Child of root should not be a leaf for consumer query");

        OperatorTypeEnum grandChildType = child(0)->child(0)->castToRelExpr()->getOperatorType();

        GenAssert(grandChildType == REL_EXTRACT_SOURCE, "Grandchild of root has unexpected type for consumer query");
      ...
      colNamesForExpr = new (generator->wHeap()) ConstNAStringPtr[numColumns];
      tblNamesForExpr = new (generator->wHeap()) ConstQualifiedNamePtr[numColumns];
    ...
    exp_gen->generateOutputExpr(compExpr_, ex_expr::exp_INPUT_OUTPUT, &output_expr, getRETDesc(), getSpOutParams(),
                                colNamesForExpr, tblNamesForExpr);
  }

  if (getPredExprTree()) {
    //      ItemExpr * newPredTree = executorPred().rebuildExprTree(ITM_AND,TRUE,TRUE);
    exp_gen->generateExpr(getPredExprTree()->getValueId(), ex_expr::exp_SCAN_PRED, &pred_expr);
  }
  ...
  if (updatableSelect() == TRUE) {
    ...
    exp_gen->generateContiguousMoveExpr(pkeyList(),
                                        1,  // add convert nodes,
                                        1, 2, ExpTupleDesc::SQLARK_EXPLODED_FORMAT, pkey_len, &pkey_expr, NULL,
  ...
  if (updateCurrentOf()) {
    if (currOfCursorName_->getOperatorType() == ITM_CONSTANT) {
      ...
      fetchedCursorName = space->allocateAndCopyToAlignedSpace((char *)(cv->getConstValue()), cv->getStorageSize(), 0);
    } else {
      ...
      for (i = 0; i < inputVars().entries(); i++) {
        ...
        if (cursorHvar->getOperatorType() == ITM_HOSTVAR) {
          ...
          if (hv->getName() == cursorHvar->getName()) fetchedCursorHvar = (short)i + 1;  // 1-based
        }
      }  // more input
  ...
  {
    GenAssert(updateCol().entries() == 0, "UPDATE CURRENT OF: updateCol non-zero");
  ...
  if (getTriggersList()) subjectTables = new (generator->wHeap()) LIST(NAString)(generator->wHeap());
  short j = 0;

  for (; j < noOfTables; j++) {
    stoiList[j] = new (space) SqlTableOpenInfo;
    SqlTableOpenInfo *genStoi = (SqlTableOpenInfo *)generator->getSqlTableOpenInfoList()[j];
    *(stoiList[j]) = *genStoi;
    ...
    if (genStoi->subjectTable() && subjectTables && !(subjectTables->contains(ansiName)) && !genStoi->isView()) {
    ...
    if (genStoi->getColumnListCount()) {
      stoiList[j]->setColumnList(new (space) short[genStoi->getColumnListCount()]);
  ...
  if (getTriggersList()) {
    GenAssert(subjectTables && (subjectTables->entries() > 0), "Mismatch: Triggers without Subject Tables");
    ...
    for (short k = 0; k < triggersCount; k++) triggersList[k] = getTriggersList()->at(k);
  }
  ...
  if (getViewStoiList().entries() > 0) {
    for (CollIndex i = 0; i < getViewStoiList().entries(); i++) {
      if (!viewStoiList) viewStoiList = new (space) Queue(space);

      SqlTableOpenInfo *stoi = new (space) SqlTableOpenInfo;
      *stoi = *getViewStoiList()[i]->getStoi();

      stoi->setFileName(new (space) char[strlen(getViewStoiList()[i]->getStoi()->fileName()) + 1]);
      strcpy(stoi->fileName(), getViewStoiList()[i]->getStoi()->fileName());

      stoi->setAnsiName(new (space) char[strlen(getViewStoiList()[i]->getStoi()->ansiName()) + 1]);
      strcpy(stoi->nonConstAnsiName(), getViewStoiList()[i]->getStoi()->ansiName());

      if (getViewStoiList()[i]->getStoi()->getColumnListCount()) {
        stoi->setColumnList(new (space) short[getViewStoiList()[i]->getStoi()->getColumnListCount()]);
  ...
  if (noOfUdrs) {
    ...
    for (short udrIdx = 0; udrIdx < noOfUdrs; udrIdx++) {
      ...
      SqlTableOpenInfo *genUdrStoi = (SqlTableOpenInfo *)bindWA->getUdrStoiList()[udrIdx]->getUdrStoi();

      *(udrStoiList[udrIdx]) = *genUdrStoi;

      udrStoiList[udrIdx]->setAnsiName(new (space) char[strlen(genUdrStoi->ansiName()) + 1]);
  ...
  if ((transMode->accessMode() == TransMode::READ_ONLY_) && (generator->needsReadWriteTransaction()))
    transMode->accessMode() = TransMode::READ_WRITE_;

  if (generator->withNoRollbackUsed() || (transMode->getRollbackMode() == TransMode::NO_ROLLBACK_)) {
    if (generator->withNoRollbackUsed()) transMode->rollbackMode() = TransMode::NO_ROLLBACK_IN_IUD_STATEMENT_;
    ...
    if (transMode->getAutoAbortIntervalInSeconds() == -1) transMode->autoAbortIntervalInSeconds() = -2;
  } else if ((NOT generator->needsReadWriteTransaction()) &&
             transMode->accessMode() != TransMode::READ_ONLY_SPECIFIED_BY_USER_ && containsLRU() == FALSE &&
  ...
  for (j = 0; j < numEntries; j++) lnil->setLateNameInfo(j, ((LateNameInfo *)(lnil + 1)) + j);
  ...
  if (generator->getLateNameInfoList().entries() > 0) {
    for (CollIndex i = 0; i < generator->getLateNameInfoList().entries(); i++) {
      ...
      if ((src->isVariable()) && (!src->isEnvVar())) {
        if (tgt->isCachedParam()) {
          ...
          if (!found) GenAssert(0, "Must find prototype hvar in input hvar");
        }
      ...
      if (tgt->compileTimeAnsiName()[0] == '\\') {
        if (NOT tgt->isVariable()) {
          ...
          if (tgt->isEnvVar())
            ...
            if (tgt->compileTimeAnsiName()[0] == '\\') tgt->setAnsiPhySame(TRUE);
          };
        }
      } else {
        if (tgt->isVariable()) {
          QualifiedName qn(tgt->compileTimeAnsiName(), 1, generator->wHeap(), generator->getBindWA());
          qn.applyDefaults(generator->currentCmpContext()->schemaDB_->getDefaultSchema());
          char *compileTimeAnsiName = space->AllocateAndCopyToAlignedSpace(qn.getQualifiedNameAsAnsiString(), 0);
          tgt->setCompileTimeName(compileTimeAnsiName, space);
        }
      }  // else

      if (tgt->isView()) viewPresent = TRUE;

      if (tgt->isVariable()) variablePresent = TRUE;
  ...
  ComASNodes *genAvailableNodes = NULL;
  const NAWNodeSet *availableNodes = CmpCommon::context()->getAvailableNodes();

  // generate multi-tenant information, but only if needed, i.e.
  // if this query is different from the default node info stored
  // for the connection and if this is a parallel query
  if (compFragDir->containsESPLayer() && availableNodes && !availableNodes->canUseAllNodes(CURRCONTEXT_CLUSTERINFO)) {
    NAText serializedNodes;
    char *serializedNodesBuf;

    availableNodes->serialize(serializedNodes);
    serializedNodesBuf = space->allocateAndCopyToAlignedSpace(serializedNodes.c_str(), serializedNodes.length() + 1, 0);

    genAvailableNodes = new (space) ComASNodes(serializedNodesBuf);
  }
  ...
  ExFragDir *exFragDir =
      new (space) ExFragDir(compFragDir->entries(), space, CmpCommon::getDefault(ESP_MULTI_FRAGMENTS) == DF_ON,
                            fragmentQuotas, (UInt16)CmpCommon::getDefaultLong(ESP_MULTI_FRAGMENT_QUOTA_VM),
                            (UInt8)CmpCommon::getDefaultLong(ESP_NUM_FRAGMENTS), genAvailableNodes);
#pragma warn(1506)  // warning elimination
  ...
  if (childOperType().match(REL_ANY_GEN_UPDATE)) updDelInsert = TRUE;
  ...
  if ((oltOptInfo().oltCliOpt()) && (viewStoiList == NULL) &&                      // no views
      (doTablenameOltOpt == TRUE) &&                                               // no late name info
      (fetchedCursorName == NULL) &&                                               // no upd/del curr of
      (fetchedCursorHvar < 0) && (delCurrOf == FALSE) && (getFirstNRows() == -1))  // no firstn specified
  {
    doOltQryOpt = TRUE;
  }
  ...
  if (updDelInsert && moreThanOneTable) retryableStmt = FALSE;

  if (childOperType() == REL_DDL) retryableStmt = FALSE;

  if (isExtractProducer || isExtractConsumer) retryableStmt = FALSE;
  ...
  if (noOfUdrs > 0) retryableStmt = FALSE;

  short maxResultSets = generator->getBindWA()->getMaxResultSets();

  char *queryCostInfoBuf = NULL;
  QueryCostInfo *queryCostInfo = (QueryCostInfo *)new (space) char[sizeof(QueryCostInfo)];
  ...
  if (getRollUpCost()) {
    ...
    totalMemPerCpu = generator->getTotalEstimatedMemory() /
                     ((generator->compilerStatsInfo().dop() > 0) ? generator->compilerStatsInfo().dop() : 1);
    totalMemPerCpuInKB = totalMemPerCpu / 1024;
    maxCpuUsage = generator->getMaxCpuUsage();
    queryCostInfo->setCostInfo(cpu, io, msg, idle, seqIOs, randIOs, total, cardinality, totalMemPerCpuInKB,
                               maxCpuUsage);
  ...
  CompilationStatsData *compilationStatsData = (CompilationStatsData *)new (space) CompilationStatsData(
      stats->compileStartTime(), stats->compileEndTime(), compilerId,
      stats->cmpPhaseLength(CompilationStats::CMP_PHASE_ALL), stats->cmpPhaseLength(CompilationStats::CMP_PHASE_BINDER),
      ...
      stats->cmpPhaseLength(CompilationStats::CMP_PHASE_GENERATOR), stats->metadataCacheHits(),
      stats->metadataCacheLookups(), stats->getQueryCacheState(), stats->histogramCacheHits(),
      stats->histogramCacheLookups(), stats->stmtHeapCurrentSize(), stats->cxtHeapCurrentSize(),
      stats->optimizationTasks(), stats->optimizationContexts(), stats->isRecompile(), compileInfo,
      stats->getCompileInfoLen());
  //
  // CompilerStats
  char *compilerStatsInfoBuf = NULL;
  CompilerStatsInfo *compilerStatsInfo = (CompilerStatsInfo *)new (space) char[sizeof(CompilerStatsInfo)];
  ...
  *compilerStatsInfo = generator->compilerStatsInfo();
  ...
  if (child_op_type == REL_EXE_UTIL || child_op_type == REL_DESCRIBE) {
    ...
    if (actual_op_type == REL_EXE_UTIL) {
      ...
      if (e->getExeUtilType() == ExeUtilExpr::DISPLAY_EXPLAIN_) input_expr = NULL;
    } else if (actual_op_type == REL_DESCRIBE) {
      Describe *d = (Describe *)lc;
      if (d->getFormat() == Describe::SHAPE_ || d->getFormat() == Describe::PLAN_) input_expr = NULL;
    }
  }

  // General notes on Apache Sentry security invalidation design:
  //
  // Trafodion GRANT/REVOKE has a wonderful event-driven security
  // invalidation scheme: When a REVOKE occurs, a security
  // invalidation key is generated for the effected objects and
  // propagated to all compiler instances in the cluster via RMS.
  //
  // Apache Hive tables might be secured via Apache Sentry.
  // Together they implement their own GRANT/REVOKE mechanism.
  // Unfortunately there is no notification service. The way a
  // client subsystem learns of a change is by polling Sentry.
  //
  // The scheme we use here is to put a timestamp into the root
  // TDB if our query references a Hive table secured by Sentry.
  // By default, that timestamp is 60 seconds into the future.
  // The Executor, when it enters "Execute" phase for a query,
  // checks for this timestamp, and if present, checks to see if
  // it is now in the past. If so, it generates an 8360 error,
  // which causes the query to be retried through the AQR
  // mechanism. The resulting query recompile bypasses the
  // query caches, but makes use of the NATableDB metadata cache.
  // When we access the corresponding Hive NATable object, it
  // too has a privilege check invalidation timestamp, and if
  // expired, the NATable code will poll Sentry. The bottom line
  // is, if a Hive/Sentry REVOKE had occurred, we'd detect it
  // then and error out the query. Otherwise we'd regenerate the
  // plan, coming through this code, and generating a new
  // privilege check timestamp in the process.
  //
  // In the future, we may add a metadata server for Hive
  // metadata. At that time, the polling responsibility could
  // be moved there, and that metadata server could generate
  // RMS events when it detects Hive/Sentry REVOKEs. At that
  // time, the present invalidation mechanism can be removed.
  ...
  root_tdb->init(
      ...
      (InputOutputExpr *)input_expr, (InputOutputExpr *)output_expr, input_vars_size, pkey_expr, pkey_len, pred_expr,
      work_cri_desc, exFragDir, transMode, fetchedCursorName, fetchedCursorHvar, delCurrOf, numUpdateCol, updateColList,
      (outputVarCntValid() && outputVarCnt()), noOfTables, getFirstNRows(), userInputVars,
      (getRollUpCost() ? getRollUpCost()->displayTotalCost().getValue() : 0), stoiList, lnil, viewStoiList, qsi, space,
      uniqueExecuteIdOffset,  //++ Triggers -
      triggersStatusOffset, triggersCount, triggersList, (short)generator->getTempTableId(),
      (short)baseTablenamePosition, updDelInsert, retryableStmt, getGroupAttr()->isStream(),
      // next flag is set for destructive stream access protocol.
      // Not needed for hbase/seabase access.
      (getGroupAttr()->isEmbeddedUpdateOrDelete() && (NOT hdfsAccess())), CmpCommon::getDefaultNumeric(STREAM_TIMEOUT),
      generator->getPlanId(), qCacheInfoBuf, cacheVarsSize, udrStoiList, noOfUdrs, maxResultSets, queryCostInfoBuf,
      newMvList, uninitializedMvCount, compilerStatsInfoBuf, rwrsInfoBuf, numObjectUIDs, objectUIDsPtr,
      compilationStatsData, sentryAuthExpirationTimeStamp, tmpLoc, listOfSnapshotscanTables, generator->getQueryHash());

  root_tdb->setTdbId(generator->getAndIncTdbId());

  if (childTdbIsNull) root_tdb->setChildTdbIsNull();
  if (generator->explainInRms()) root_tdb->setExplainInRms();

  OperatorTypeEnum childOper = childOperType();
  OperatorTypeEnum currChildOper = (child(0) ? child(0)->castToRelExpr()->getOperatorType() : childOper);

  if (qCacheInfoIsClass) root_tdb->setQCacheInfoIsClass(TRUE);

  if (getHostArraysArea() && getHostArraysArea()->getRowwiseRowset()) {
    root_tdb->setRowwiseRowsetInput(TRUE);
  } else {
    NABoolean singleRowInput = TRUE;
    if ((input_expr) && ((InputOutputExpr *)input_expr)->isCall()) singleRowInput = FALSE;

    if (CmpCommon::getDefault(COMP_BOOL_92) == DF_OFF) singleRowInput = FALSE;
  ...
  if (CmpCommon::getDefault(MAINTENANCE_WINDOW) == DF_ON) root_tdb->setMaintenanceWindow(TRUE);
  ...
  root_tdb->setAutoCollectRTStats(generator->getAutoCollectRTStats());
  ...
  if (checkNode->getOperatorType() == REL_EXCHANGE) checkNode = checkNode->child(0);

  if (checkNode->getOperatorType() == REL_PARTITION_ACCESS) checkNode = checkNode->child(0);

  if (checkNode->getOperatorType() == REL_COMPOUND_STMT ||
      (checkNode->getOperatorType() == REL_UNION || checkNode->getOperatorType() == REL_MERGE_UNION) &&
  ...
  if (checkNode->getOperatorType() == REL_DDL) {
    DDLExpr *ddlExp = (DDLExpr *)(checkNode);

    if (ddlExp->isTrafSharedCacheUpdateOp()) {
      root_tdb->setDlockForSharedCache(TRUE);
    }
  }
  ...
  // Disallow cancel.
  if (CmpCommon::getDefault(COMP_BOOL_20) == DF_ON) root_tdb->setMayNotCancel(TRUE);

  if (generator->mayNotCancel()) root_tdb->setMayNotCancel(TRUE);

  DDLExpr *ddlExpr = NULL;

  if (updDelInsert) {
    if ((childOper == REL_UNARY_INSERT) || (childOper == REL_LEAF_INSERT) || (childOper == REL_INSERT_CURSOR))
      root_tdb->setQueryType(ComTdbRoot::SQL_INSERT_NON_UNIQUE);
    else if ((childOper == REL_UNARY_UPDATE) || (childOper == REL_LEAF_UPDATE) || (childOper == REL_UPDATE_CURSOR))
      root_tdb->setQueryType(ComTdbRoot::SQL_UPDATE_NON_UNIQUE);
    else if ((childOper == REL_UNARY_DELETE) || (childOper == REL_LEAF_DELETE) || (childOper == REL_DELETE_CURSOR))
      root_tdb->setQueryType(ComTdbRoot::SQL_DELETE_NON_UNIQUE);
    root_tdb->setNumTrafReplicas(generator->getNumTrafReplicas());
  }

  if (output_expr) root_tdb->setQueryType(ComTdbRoot::SQL_SELECT_NON_UNIQUE);

  if ((updDelInsert) && (root_tdb->getQueryType() == ComTdbRoot::SQL_INSERT_NON_UNIQUE) && (rwrsInfo)) {
    root_tdb->setQueryType(ComTdbRoot::SQL_INSERT_RWRS);
  } else if ((updDelInsert) && child(0) && child(0)->castToRelExpr() &&
             (child(0)->castToRelExpr()->getOperatorType() == REL_NESTED_JOIN_FLOW)) {
    RelExpr *tuple_flow = child(0)->castToRelExpr();
    if (tuple_flow->child(1) && tuple_flow->child(1)->castToRelExpr() &&
        tuple_flow->child(1)->castToRelExpr()->getOperator().match(REL_ANY_HBASE_GEN_UPDATE) && (NOT output_expr) &&
        tuple_flow->child(0) && tuple_flow->child(0)->castToRelExpr() &&
        tuple_flow->child(0)->castToRelExpr()->getOperatorType() == REL_HBASE_ACCESS) {
      OperatorTypeEnum tempOper = tuple_flow->child(1)->castToRelExpr()->getOperatorType();
      GenericUpdate *gu = (GenericUpdate *)(tuple_flow->child(1)->castToRelExpr());
      HbaseAccess *scan = (HbaseAccess *)(tuple_flow->child(0)->castToRelExpr());
      if (gu->uniqueHbaseOper() && scan->uniqueHbaseOper()) {
        if (tempOper == REL_HBASE_UPDATE)
          root_tdb->setQueryType(ComTdbRoot::SQL_UPDATE_UNIQUE);
        else if (tempOper == REL_HBASE_DELETE)
          root_tdb->setQueryType(ComTdbRoot::SQL_DELETE_UNIQUE);
        else
          root_tdb->setQueryType(ComTdbRoot::SQL_INSERT_UNIQUE);
      }
    }
  } else if ((child(0)) && (child(0)->castToRelExpr()->getOperatorType() == REL_UTIL_INTERNALSP)) {
    root_tdb->setQueryType(ComTdbRoot::SQL_CAT_UTIL);
  } else if ((child(0)) && (child(0)->castToRelExpr()->getOperatorType() == REL_DESCRIBE)) {
    root_tdb->setSubqueryType(ComTdbRoot::SQL_DESCRIBE_QUERY);
  } else if ((child(0)) && (child(0)->castToRelExpr()->getOperatorType() == REL_EXE_UTIL)) {
    ...
    if (exeUtil->getExeUtilType() == ExeUtilExpr::CREATE_TABLE_AS_) {
      ...
      if (CmpCommon::getDefault(EXE_UTIL_RWRS) == DF_ON) root_tdb->setExeUtilRwrs(TRUE);
  ...
  } else if ((child(0)) && (ddlExpr = (DDLExpr *)(this->isDDLExpr()))) {
    if (ddlExpr->producesOutput())
      root_tdb->setQueryType(ComTdbRoot::SQL_DDL_WITH_STATUS);
    else if (foundUpdStat)
      root_tdb->setQueryType(ComTdbRoot::SQL_CAT_UTIL);
    else
      root_tdb->setQueryType(ComTdbRoot::SQL_DDL);

    root_tdb->setDDLSafeness(ddlExpr->getDDLSafeness());

    if (ddlExpr->isTrafSharedCacheOp()) {
      root_tdb->setSubqueryType(ComTdbRoot::SQL_DDL_SHARED_CACHE_OP);
    }
    ...
    OperatorTypeEnum currChildOper = child(0)->castToRelExpr()->getOperatorType();

    if (currChildOper == REL_CONTROL_QUERY_DEFAULT) {
      root_tdb->setMayNotCancel(TRUE);
      ControlQueryDefault *cqd = (ControlQueryDefault *)child(0)->castToRelExpr();
      if (cqd->dynamic()) {
        ...
        root_tdb->setQueryType(ComTdbRoot::SQL_CONTROL);
    } else if ((currChildOper == REL_CONTROL_QUERY_SHAPE) || (currChildOper == REL_CONTROL_TABLE)) {
      root_tdb->setMayNotCancel(TRUE);
      root_tdb->setQueryType(ComTdbRoot::SQL_CONTROL);
      ...
      root_tdb->setMayNotCancel(TRUE);
    else if (isTrueRoot() && REL_TUPLE == currChildOper &&
             REL_RENAME_TABLE == childOperType()) {  // for select 1 from dual
      ItemExpr *selectList = this->selectList();
      if (selectList)
        if (selectList->getOperatorType() == ITM_CONSTANT || selectList->getOperatorType() == ITM_CACHE_PARAM)
          root_tdb->setMayNotCancel(TRUE);
        else if (selectList->getOperatorType() == ITM_CAST) {  // trafci
          ItemExpr *child0 = selectList->child(0)->castToItemExpr();
          if (child0->getOperatorType() == ITM_CONSTANT || child0->getOperatorType() == ITM_CACHE_PARAM)
            root_tdb->setMayNotCancel(TRUE);
        }
    }
  }

  if (child(0) && child(0)->castToRelExpr() && child(0)->castToRelExpr()->getOperator().match(REL_ANY_HBASE)) {
    RelExpr *childExpr = child(0)->castToRelExpr();
    OperatorTypeEnum currChildOper = childExpr->getOperatorType();

    if ((childExpr->getOperator().match(REL_ANY_HBASE_GEN_UPDATE)) && (NOT output_expr)) {
  ...
  if (updDelInsert && child(0) && child(0)->castToRelExpr() && (NOT output_expr)) {
    if (root_tdb->getQueryType() == ComTdbRoot::SQL_INSERT_NON_UNIQUE) {
      if (child(0)->isUniqueOper()) root_tdb->setQueryType(ComTdbRoot::SQL_INSERT_UNIQUE);
    } else if (root_tdb->getQueryType() == ComTdbRoot::SQL_UPDATE_NON_UNIQUE) {
      if (child(0)->isUniqueOper()) root_tdb->setQueryType(ComTdbRoot::SQL_UPDATE_UNIQUE);
    } else if (root_tdb->getQueryType() == ComTdbRoot::SQL_DELETE_NON_UNIQUE) {
      if (child(0)->isUniqueOper()) root_tdb->setQueryType(ComTdbRoot::SQL_DELETE_UNIQUE);
    }
  }

  if (child(0) && child(0)->castToRelExpr() && root_tdb->getQueryType() == ComTdbRoot::SQL_SELECT_NON_UNIQUE) {
    if (child(0)->isUniqueOper()) root_tdb->setQueryType(ComTdbRoot::SQL_SELECT_UNIQUE);
  }

  // To help determine if it is safe to suspend.
  if (child(0) && child(0)->castToRelExpr()) {
    OperatorTypeEnum currChildOper = child(0)->castToRelExpr()->getOperatorType();
    if ((REL_DDL == currChildOper) || (REL_TRANSACTION == currChildOper) || (REL_EXE_UTIL == currChildOper))
      root_tdb->setMayAlterDb(TRUE);

    if (REL_LOCK == currChildOper) root_tdb->setSuspendMayHoldLock(TRUE);
  }

  if (generator->anySerialiableScan()) root_tdb->setSuspendMayHoldLock(TRUE);
  ...
  if (generator->aqrEnabled()) root_tdb->setAqrEnabled(TRUE);

  if (generator->cantReclaimQuery()) root_tdb->setCantReclaimQuery(TRUE);

  if (generator->useRegionXN()) root_tdb->setUseRegionXN(TRUE);
  ...
  if (generator->isTransactionNeeded()) {
    ...
    // add implicit savepoint support also for 'select for update'
    if (generator->foundAnUpdate() || generator->updatableSelect()) {
      if (generator->updAbortOnError() == TRUE) {
        ...
        if (generator->savepointEnabled() == TRUE) {
          root_tdb->setSavepointEnabled(TRUE);
        } else
          root_tdb->setUpdAbortOnError(-1);
      } else
        root_tdb->setUpdErrorOnError(-1);
  ...
  if ((oltOptLean()) && (doOltQryOpt)) {
    if ((NOT root_tdb->getUpdAbortOnError()) && (NOT root_tdb->savepointEnabled()) &&
        (NOT root_tdb->getUpdPartialOnError()) && (retryableStmt) && (NOT root_tdb->thereIsACompoundStatement())) {
  ...
  if (generator->inMemoryObjectDefn()) root_tdb->setInMemoryObjectDefn(TRUE);
  ...
  if (hdfsAccess()) root_tdb->setHdfsAccess(TRUE);

  if (generator->hiveWriteAccess()) root_tdb->setHiveWriteAccess(TRUE);
  ...
  if (!generator->explainDisabled()) {
    // finish up EXPLAIN
    ExplainTuple *rootExplainTuple = addExplainInfo(root_tdb, 0, 0, generator);
    ...
    if (childExplainTuple) {
      ...
      rootExplainTuple->setChildSeqNum(0, childExplainTuple->getSeqNum());
    }
  ...
  ExPartInputDataDesc **partInputDataDescs = new (generator->wHeap()) ExPartInputDataDesc *[compFragDir->entries()];
  ExEspNodeMap **nodeMap = new (generator->wHeap()) ExEspNodeMap *[compFragDir->entries()];
  for (i = 0; i < compFragDir->entries(); i++) {
    if (compFragDir->getPartitioningFunction(i) != NULL) {
      ...
      ((PartitioningFunction *)compFragDir->getPartitioningFunction(i))
          ->codeGen(generator, compFragDir->getPartInputDataLength(i));
      partInputDataDescs[i] = (ExPartInputDataDesc *)(generator->getGenObj());
      NodeMap::codeGen(compFragDir->getPartitioningFunction(i), compFragDir->getNumESPs(i), generator);
  ...
  for (i = 0; i < compFragDir->entries(); i++) {
    ...
    exFragDir->set(
        i, runTimeType, (ExFragId)compFragDir->getParentId(i), offset, currLength, -offsetOfTopNode,
        partInputDataDescs[i], nodeMap[i], compFragDir->getNumESPs(i), compFragDir->getEspLevel(i),
        compFragDir->getNeedsTransaction(i),
        (compressThreshold > 0 && runTimeType == ExFragDir::ESP && compressThreshold <= compFragDir->getNumESPs(i))
            ? TRUE
            : FALSE,  // DP2 fragment will be compressed later
                      // if parent ESP fragment is compressed
                      // see executor/ex_frag_rt.cpp
        compFragDir->getSoloFragment(i), BMOsMemoryUsage, compFragDir->getNumBMOs(i) > 0);
    offset += currLength;
  }  // for each fragment
  ...
  NABoolean recompWarn = (CmpCommon::getDefault(RECOMPILATION_WARNINGS) == DF_ON);
  ...
  if (!(childOperType_ == REL_CONTROL_SESSION) && (val) && (*val == "ON")) root_tdb->setFromShowplan();

  if (CmpCommon::getDefault(EXE_LOG_RETRY_IPC) == DF_ON) root_tdb->setLogRetriedIpcErrors(TRUE);

  if (anyEspFragments) {
    if (generator->getBindWA()->queryCanUseSeaMonster() && generator->getQueryUsesSM()) root_tdb->setQueryUsesSM();
  }
...
short Sort::generateTdb(Generator *generator, ComTdb *child_tdb, ex_expr *sortKeyExpr, ex_expr *sortRecExpr,
  ...
  UInt32 bufferSize_as_uint32 = (UInt32)(MINOF(CostScalar(UINT_MAX), bufferSize)).getValue();

  // allocate buffer to hold atleast ten rows, ten is just arbitrary number.
  // Stats will reflect actual buffer used.
  bufferSize_as_uint32 = MAXOF(bufferSize_as_uint32, sortRecLen * 10);
  ...
  if (CmpCommon::getDefault(EXE_BMO_SET_BUFFERED_WRITES) == DF_ON) sort_options->setBufferedWrites(TRUE);
  if (CmpCommon::getDefault(EXE_DIAGNOSTIC_EVENTS) == DF_ON) sort_options->setLogDiagnostics(TRUE);

  // Disable Compiler Hints checks if: CQD is ON or if SYSTEM - only for HDD
  if ((CmpCommon::getDefault(EXE_BMO_DISABLE_CMP_HINTS_OVERFLOW_SORT) == DF_ON) ||
      (((generator->getOverflowMode() == ComTdb::OFM_DISK) || (generator->getOverflowMode() == ComTdb::OFM_MMAP)) &&
       (CmpCommon::getDefault(EXE_BMO_DISABLE_CMP_HINTS_OVERFLOW_SORT) == DF_SYSTEM)))
    sort_options->setDisableCmpHintsOverflow(TRUE);

  if (CmpCommon::getDefault(EXE_BMO_DISABLE_OVERFLOW) == DF_ON) sort_options->dontOverflow() = TRUE;
  ...
  if (CmpCommon::getDefault(SORT_MEMORY_QUOTA_SYSTEM) != DF_OFF) {
    ...
    if (memoryQuotaMB > 0) {
      ...
      if (mlimitPerNode || numBMOsInFrag > 1 ||
        ...
        memoryQuotaMB = (short)computeMemoryQuota(
            generator->getEspLevel() == 0, mlimitPerNode, generator->getBMOsMemoryLimitPerNode().value(),
  ...
  if (memoryQuotaMB <= 0 && !sort_options->disableCmpHintsOverflow())  // compiler hints enabled
  {
  ...
  sort_options->pressureThreshold() = (short)getDefault(GEN_MEM_PRESSURE_THRESHOLD);

  short sortGrowthPercent = RelExpr::bmoGrowthPercent(getEstRowsUsed(), getMaxCardEst());

  sort_tdb = new (generator->getSpace()) ComTdbSort(
      sortKeyExpr, sortRecExpr, sortKeyLen, sortRecLen, sortPrefixKeyLen, returned_desc->noTuples() - 1, child_tdb,
      given_desc, returned_desc, work_cri_desc,

      // if sort input is from top, switch the UP and DOWN queue
      // sizes
      (sortFromTop() ? (queue_index)getDefault(GEN_SORT_SIZE_UP) : (queue_index)getDefault(GEN_SORT_SIZE_DOWN)),
      (queue_index)getDefault(GEN_SORT_SIZE_UP), (Cardinality)(getInputCardinality() * getEstRowsUsed()).getValue(),
      numBuffers, bufferSize_as_uint32, max_num_buffers, sort_options, sortGrowthPercent);

  sort_tdb->setRecordLength(getGroupAttr()->getRecordLength());
  ...
  if (generator->getUserSidetreeInsert()) sort_tdb->setUserSidetreeInsert(TRUE);

  if (getTolerateNonFatalError() == RelExpr::NOT_ATOMIC_) sort_tdb->setTolerateNonFatalError(TRUE);
  ...
  if (sortPrefixKeyLen > 0) ((ComTdbSort *)sort_tdb)->setPartialSort(TRUE);  // do partial sort
  ...
  if (sortNRows()) sort_tdb->setTopNThreshold(defs.getAsLong(GEN_SORT_TOPN_THRESHOLD));
  if (!generator->explainDisabled()) {
    generator->setExplainTuple(addExplainInfo(sort_tdb, childExplainTuple, 0, generator));
  }
...
short Sort::codeGen(Generator *generator) {
  ...
  for (sortKeyListIndex = 0; sortKeyListIndex < getSortKey().entries(); sortKeyListIndex++) {
    ItemExpr *skey_node = ((getSortKey()[sortKeyListIndex]).getValueDesc())->getItemExpr();
    ...
    if (skey_node->getValueId().getType().getVarLenHdrSize() > 0) {
      ...
      const CharType &char_type = (CharType &)(skey_node->getValueId().getType());
      ...
      if (!CollationInfo::isSystemCollation(char_type.getCollation())) {
        ...
        skey_node = new (generator->wHeap()) Cast(
            skey_node, (new (generator->wHeap()) SQLChar(
                           generator->wHeap(), CharLenInfo(char_type.getStrCharLimit(), char_type.getDataStorageSize()),
                           char_type.supportsSQLnull(), FALSE, FALSE, FALSE, char_type.getCharSet(),
                           char_type.getCollation(), char_type.getCoercibility())));

        if (char_type.isVarchar2() ||
            (isTrailingBlankSensitive && atoi(isTrailingBlankSensitive) == 1 && !isIgnoreTrailingBlank()))
          ((Cast *)skey_node)->setPadUseZero(TRUE);
      }
    }

    CompEncode *enode = new (generator->wHeap()) CompEncode(skey_node, desc_flag);
  ...
  */
  ...
  if (sortRecValIdList.entries() > 0) {
    if (!bmo_affinity && getCachedTupleFormat() != ExpTupleDesc::UNINITIALIZED_FORMAT &&
      ...
      tupleFormat = determineInternalFormat(sortRecValIdList, this, resizeCifRecord, generator, bmo_affinity,
  ...
  if (tupleFormat == ExpTupleDesc::SQLMX_ALIGNED_FORMAT) {
    exp_gen->generateContiguousMoveExpr(sortKeyValIdList,
                                        0,  // no conv nodes
                                        work_atp, work_atp_index, ExpTupleDesc::SQLMX_KEY_FORMAT, sortKeyLen,
                                        &sortKeyExpr,
                                        0,  // no tupp descr
                                        ExpTupleDesc::SHORT_FORMAT);
    ...
    exp_gen->generateContiguousMoveExpr(sortRecValIdList,
                                        0,  // no convert nodes
                                        work_atp, work_atp_index, tupleFormat, sortRecLen, &sortRecExpr, &tuple_desc,
                                        ExpTupleDesc::SHORT_FORMAT);
    ...
    exp_gen->generateContiguousMoveExpr(sortKeyValIdList,
                                        0,  // no conv nodes
                                        work_atp, work_atp_index, ExpTupleDesc::SQLARK_EXPLODED_FORMAT, sortRecLen,
                                        &sortRecExpr, &tuple_desc, ExpTupleDesc::SHORT_FORMAT);
  }
  ...
  returned_desc->setTupleDescriptor((UInt16)returned_desc->noTuples() - 1, tuple_desc);
  ...
  for (valId = getGroupAttr()->getCharacteristicOutputs().init();
    ...
    Attributes *attr = generator->getMapInfo(sortRecValIdList[sortRecListIndex++])->getAttr();

    // ...add it to the new map table as if it belonged to
    // the original value id...
    MapInfo *mi = generator->addMapInfoToThis(returnedMapTable, valId, attr);
  ...
  short rc = generateTdb(generator, child_tdb, sortKeyExpr, sortRecExpr, sortKeyLen, sortRecLen, sortPrefixKeyLen,
                         given_desc, returned_desc, work_cri_desc, saveNumEsps, childExplainTuple, resizeCifRecord,
                         considerBufferDefrag, (tupleFormat == ExpTupleDesc::SQLMX_ALIGNED_FORMAT));

  return rc;
}

ExpTupleDesc::TupleDataFormat Sort::determineInternalFormat(const ValueIdList &valIdList, RelExpr *relExpr,
  ...
  return generator->determineInternalFormat(valIdList, relExpr, resizeCifRecord, bmo_cif, bmo_affinity,
                                            considerBufferDefrag, sortKeyLength);
}
//////////////////////////////////////////////////////////////
//
// SortFromTop::codeGen()
...
short SortFromTop::codeGen(Generator *generator) {
  ...
  for (ii = 0; ii < getSortKey().entries(); ii++) {
    ItemExpr *skey_node = ((getSortKey()[ii]).getValueDesc())->getItemExpr();

    if (skey_node->getOperatorType() != ITM_INDEXCOLUMN) GenAssert(0, "Must be IndexColumn");

    IndexColumn *ic = (IndexColumn *)skey_node;
    NABoolean found = FALSE;
    CollIndex jj = 0;
    while ((NOT found) && (jj < getSortRecExpr().entries())) {
      const ItemExpr *assignExpr = getSortRecExpr()[jj].getItemExpr();

      ItemExpr *tgtCol = assignExpr->child(0)->castToItemExpr();
      if (tgtCol->getOperatorType() != ITM_BASECOLUMN) GenAssert(0, "Must be BaseColumn");
      ...
      if (ic->getNAColumn()->getColName() == ((BaseColumn *)tgtCol)->getNAColumn()->getColName()) {
        found = TRUE;
        ...
        if (skey_node->getValueId().getType().getVarLenHdrSize() > 0) {
          ...
          if (!CollationInfo::isSystemCollation(char_type.getCollation())) {
            skey_node = new (generator->wHeap())
                Cast(srcVal,
                     (new (generator->wHeap()) SQLChar(
                         generator->wHeap(), CharLenInfo(char_type.getStrCharLimit(), char_type.getDataStorageSize()),
                         char_type.supportsSQLnull(), FALSE, FALSE, FALSE, char_type.getCharSet(),
                         char_type.getCollation(), char_type.getCoercibility())));

            if (char_type.isVarchar2()) ((Cast *)skey_node)->setPadUseZero(TRUE);
          }
        } else {
          skey_node = new (generator->wHeap()) Cast(srcVal, &tgtValueId.getType());
        }
  ...
  if (sortKeyVIDlist.entries() > 0) {
    exp_gen->generateContiguousMoveExpr(sortKeyVIDlist,
                                        0,  // no conv nodes
                                        work_atp, work_atp_index, ExpTupleDesc::SQLMX_KEY_FORMAT, sort_key_len,
                                        &sort_key_expr,
                                        0,  // no tupp descr
                                        ExpTupleDesc::SHORT_FORMAT);
  }
  ...
  for (ii = 0; ii < getSortRecExpr().entries(); ii++) {
    const ItemExpr *assignExpr = getSortRecExpr()[ii].getItemExpr();

    ValueId tgtValueId = assignExpr->child(0)->castToItemExpr()->getValueId();
    ValueId srcValueId = assignExpr->child(1)->castToItemExpr()->getValueId();

    ItemExpr *ie = NULL;
    ie = new (generator->wHeap()) Cast(assignExpr->child(1), &tgtValueId.getType());
  ...
  if (sortRecVIDlist.entries() > 0) {
    tupleFormat =
        determineInternalFormat(sortRecVIDlist, this, resizeCifRecord, generator, FALSE, considerBufferDefrag);

    exp_gen->generateContiguousMoveExpr(sortRecVIDlist, 0 /*don't add conv nodes*/, work_atp, work_atp_index,
                                        tupleFormat, sort_rec_len, &sort_rec_expr, &tuple_desc,
                                        ExpTupleDesc::SHORT_FORMAT);
  }
  ...
  for (CollIndex i = 0; i < (CollIndex)sortRecVIDlist.entries(); i++) {
    ...
    Attributes *attrib = generator->getMapInfo(cn->getValueId())->getAttr();

    MapInfo *mi = generator->addMapInfoToThis(myMapTable, cn->child(0)->castToItemExpr()->getValueId(), attrib);
  ...
  short rc = generateTdb(generator, child_tdb, sort_key_expr, sort_rec_expr, sort_key_len, sort_rec_len,
                         0,  // sort_prefix_key_len,
                         given_desc, child_desc, work_cri_desc, saveNumEsps, childExplainTuple, resizeCifRecord,
                         considerBufferDefrag);
...
CostScalar Sort::getEstimatedRunTimeMemoryUsage(Generator *generator, NABoolean perNode, int *numStreams) {
  ...
  OperBMOQuota *operBMOQuota =
      new (generator->wHeap()) OperBMOQuota(getKey(), numOfStreams, estMemPerNode, estMemPerInst, rowsUsed, maxCard);
...
short Tuple::codeGen(Generator *generator) {
  ...
  ComTdbTupleLeaf *tuple_tdb = new (generator->getSpace())
      ComTdbTupleLeaf(qList,
                      ...
                      0,  // no tupp index
                      predExpr, generator->getCriDesc(Generator::DOWN), generator->getCriDesc(Generator::DOWN),
                      (queue_index)getDefault(GEN_TUPL_SIZE_DOWN), (queue_index)getDefault(GEN_TUPL_SIZE_UP),
                      (Cardinality)(getInputCardinality() * getEstRowsUsed()).getValue(),
                      getDefault(GEN_TUPL_NUM_BUFFERS), getDefault(GEN_TUPL_BUFFER_SIZE));

  tuple_tdb->setRecordLength(getGroupAttr()->getRecordLength());

  generator->initTdbFields(tuple_tdb);

  if (!generator->explainDisabled()) {
    generator->setExplainTuple(addExplainInfo(tuple_tdb, 0, 0, generator));
  }

  generator->setGenObj(this, tuple_tdb);
  generator->setCriDesc(generator->getCriDesc(Generator::DOWN), Generator::UP);
...
short TupleList::codeGen(Generator *generator) {
  ...
  for (CollIndex i = 0; i < nTupEntries; i++) {
    ...
    ItemExpr *tuple = ((ItemExpr *)tupleList[i])->child(0)->castToItemExpr();
    ...
    for (CollIndex j = 0; j < tupleExpr().entries(); j++) {
      ...
      if (castTo) {
        ...
        if ((hiveTextInsert()) && (DFS2REC::isBinaryString(tgtType.getFSDatatype())) &&
            (NOT DFS2REC::isAnyCharacter(srcType.getFSDatatype()))) {
          childNode = new (bindWA->wHeap()) ZZZBinderFunction(ITM_TO_CHAR, childNode);
        ...
        Assign *tmpAssign = new (bindWA->wHeap()) Assign(castToList()[j].getItemExpr(), childNode);
        ...
        if ((castNode->child(0)) && (castNode->child(0)->getOperatorType() == ITM_INSTANTIATE_NULL)) {
      ...
      if (mInfo) mInfo->resetCodeGenerated();
      castNode->unmarkAsPreCodeGenned();
      convVIDlist.insert(castNode->getValueId());
    }

    GenAssert(!bindWA->errStatus(), "bindWA");
    expGen->generateContiguousMoveExpr(convVIDlist, 0 /*don't add conv nodes*/, 0 /*atp*/, tuppIndex,
                                       generator->getInternalFormat(), tupleLen, &moveExpr, &tupleDesc,
                                       ExpTupleDesc::SHORT_FORMAT);
  ...
  ComTdbTupleLeaf *tupleTdb = new (generator->getSpace()) ComTdbTupleLeaf(
      qList, tupleLen, tuppIndex, predExpr, givenDesc, returnedDesc, (queue_index)getDefault(GEN_TUPL_SIZE_DOWN),
      (queue_index)upqueuelength, (Cardinality)(getInputCardinality() * getEstRowsUsed()).getValue(),
      getDefault(GEN_TUPL_NUM_BUFFERS), buffersize);

  tupleTdb->setRecordLength(getGroupAttr()->getRecordLength());

  generator->initTdbFields(tupleTdb);

  if (!generator->explainDisabled()) {
    generator->setExplainTuple(addExplainInfo(tupleTdb, 0, 0, generator));
  }
...
short ExplainFunc::codeGen(Generator *generator) {
  ...
  ex_cri_desc *paramsDesc = new (space) ex_cri_desc(givenDesc->noTuples() + 1, space);
  ...
  for (CollIndex i = 0; i < numColumns; i++) {
    ...
    attrs[i] = (generator->addMapInfo(col_node->getValueId(), 0))->getAttr();
  }
  ...
  if (!selectionPred().isEmpty()) {
    ...
    expGen->generateExpr(newPredTree->getValueId(), ex_expr::exp_SCAN_PRED, &explainExpr);
  }
  ...
  if (!generator->explainDisabled()) {
    generator->setExplainTuple(addExplainInfo(explainTdb, 0, 0, generator));
  }
  generator->setCriDesc(givenDesc, Generator::DOWN);
  generator->setCriDesc(returnedDesc, Generator::UP);
...
//
short PhysTranspose::codeGen(Generator *generator) {
  ...
  if (transOutputs.isEmpty()) {
    ...
    // along.
    //
    returnedCriDesc = new (space) ex_cri_desc(givenCriDesc->noTuples() + 1, space);
    ...
    //
    returnedCriDesc = new (space) ex_cri_desc(childCriDesc->noTuples() + 1, space);
  }
  ...
  for (unionListNum = 0; unionListNum < numUnionLists; unionListNum++) {
    ...
    if (transUnionVector()[unionListNum].entries() > 0)
      numExprs[unionListNum] =
          ((ValueIdUnion *)(transUnionVector()[unionListNum])[0].getValueDesc()->getItemExpr())->entries();
    else
  ...
  //
  GenAssert(totNumExprs == numKeyExprs * 2 || numKeyExprs == 0, "Transpose: Internal Error");
  ...
  for (exprNum = 0; exprNum < numMovExprs; exprNum++) {
    ...
    if (numKeyExprs != 0) {
      ...
      //
      ValueIdUnion *keyValIdUnion = (ValueIdUnion *)((transUnionVector()[0])[0].getValueDesc()->getItemExpr());

      // Extract one (exprNum) entry from the ValueIdUnion and add a
      // Cast to cast it to the proper type.
      //
      castExpr = new (generator->wHeap()) Cast(keyValIdUnion->getSource(exprNum).getValueDesc()->getItemExpr(),
      ...
      //
      GenAssert(!generator->getBindWA()->errStatus(), "Transpose: Internal Error");
    ...
    //
    for ((vectorEntryNum = 1, numCoveredExprs = numExprs[1], valExprNum = exprNum);
         ...
         (numCoveredExprs += numExprs[vectorEntryNum + 1], valExprNum -= numExprs[vectorEntryNum], vectorEntryNum++))
      ;
    ...
    GenAssert(vectorEntryNum > 0 && vectorEntryNum < numUnionLists, "Transpose: Internal Error");

    GenAssert(valExprNum < numExprs[vectorEntryNum], "Transpose: Internal Error");
    ...
    for (unionListNum = 1; unionListNum < numUnionLists; unionListNum++) {
      ...
      //
      for (CollIndex numValUnions = 0; numValUnions < transUnionVector()[unionListNum].entries(); numValUnions++) {
        ValueIdUnion *valValIdUnion =
            (ValueIdUnion *)((transUnionVector()[unionListNum])[numValUnions].getValueDesc()->getItemExpr());

        if (unionListNum == vectorEntryNum) {
          // Construct the value expression cast to the proper type.
          //
          castExpr = new (generator->wHeap()) Cast(valValIdUnion->getSource(valExprNum).getValueDesc()->getItemExpr(),
                                                   &(valValIdUnion->getResult().getType()));
        } else {
          // Construct NULL expression cast to the proper type.
          //
          castExpr = new (generator->wHeap()) Cast(new (space) ConstValue(), &(valValIdUnion->getResult().getType()));
        }
    ...
    //
    expGen->generateContiguousMoveExpr(ValueIds, 0, transColsAtp, transColsAtpIndex, generator->getInternalFormat(),
                                       transColsTupleLen, &transExprs[exprNum],
                                       (exprNum == 0 ? &transColsTupleDesc : 0), ExpTupleDesc::SHORT_FORMAT);
    // Set the tuple descriptor in the returned CRI descriptor.
    // This will be set only the first time through the loop.
    //
    if (exprNum == 0) {
      returnedCriDesc->setTupleDescriptor(transColsAtpIndex, transColsTupleDesc);
      ...
      for (unionListNum = 0; unionListNum < numUnionLists; unionListNum++) {
        ...
        for (CollIndex numValUnions = 0; numValUnions < transUnionVector()[unionListNum].entries(); numValUnions++) {
          ...
          //
          ValueId valId = (transUnionVector()[unionListNum])[numValUnions];
          ...
          //
          MapInfo *mapInfo =
              generator->addMapInfoToThis(localMapTable, valId, generator->getMapInfo(ValueIds[valIdNum])->getAttr());
  ...
  if (!selectionPred().isEmpty()) {
    ...
    expGen->generateExpr(newPredTree->getValueId(), ex_expr::exp_SCAN_PRED, &afterTransPred);
  }
  ...
  ComTdbTranspose *transTdb = new (space)
      ComTdbTranspose(childTdb, transExprs, numMovExprs, afterTransPred, transColsTupleLen, transColsAtpIndex,
                      givenCriDesc, returnedCriDesc, (queue_index)getDefault(GEN_TRSP_SIZE_DOWN),
                      //(queue_index)getDefault(GEN_TRSP_SIZE_UP),
                      upQ, (Cardinality)(getInputCardinality() * getEstRowsUsed()).getValue(),
                      getDefault(GEN_TRSP_NUM_BUFFERS), getDefault(GEN_TRSP_BUFFER_SIZE), space);

  transTdb->setRecordLength(getGroupAttr()->getRecordLength());
  ...
  //
  if (!generator->explainDisabled()) {
    generator->setExplainTuple(addExplainInfo(transTdb, childExplainTuple, 0, generator));
  }
...
short PhyPack::codeGen(Generator *generator) {
  ...
  ex_cri_desc *returnedCriDesc = new (space) ex_cri_desc(returnedNoOfTuples, space);
  ...
  Attributes **attrs = new (generator->wHeap()) Attributes *[noOfPackedCols];
  ...
  expGen->processAttributes(noOfPackedCols, attrs, ExpTupleDesc::SQLARK_EXPLODED_FORMAT, tupleLen, atp, atpIndex,
  ...
  ComTdbPackRows *packTdb = new (space) ComTdbPackRows(childTdb, packExpr, predExpr, returnedNoOfTuples - 1, tupleLen,
                                                       givenCriDesc, returnedCriDesc, (queue_index)8, (queue_index)8);
  ...
  if (NOT generator->explainDisabled()) {
    ...
    generator->setExplainTuple(addExplainInfo(packTdb, childExplainTuple, 0, generator));
  }
...
// -----------------------------------------------------------------------
const char *TableValuedFunction::getVirtualTableName() { return "??TableValuedFunction??"; }

TrafDesc *TableValuedFunction::createVirtualTableDesc() { return NULL; }
...
TrafDesc *StatisticsFunc::createVirtualTableDesc() {
  TrafDesc *table_desc =
      Generator::createVirtualTableDesc(getVirtualTableName(),
                                        NULL,  // let it decide what heap to use
                                        ComTdbStats::getVirtTableNumCols(), ComTdbStats::getVirtTableColumnInfo(),
                                        ComTdbStats::getVirtTableNumKeys(), ComTdbStats::getVirtTableKeyInfo());
  return table_desc;
}
...
short StatisticsFunc::codeGen(Generator *generator) {
  ...
  for (CollIndex i = 0; i < getTableDesc()->getColumnList().entries(); i++) {
    ItemExpr *col_node = (((getTableDesc()->getColumnList())[i]).getValueDesc())->getItemExpr();
  ...
  expGen->processAttributes(getTableDesc()->getColumnList().entries(), attrs, tupleFormat, tupleLength, work_atp,
                            stats_row_atp_index, &tupleDesc, ExpTupleDesc::LONG_FORMAT);
  ...
  if (!getProcInputParamsVids().isEmpty()) {
    expGen->generateContiguousMoveExpr(getProcInputParamsVids(), -1, work_atp, input_row_atp_index, tupleFormat,
                                       inputTupleLength, &inputExpr, &inputTupleDesc, ExpTupleDesc::LONG_FORMAT);
  ...
  ComTdbStats *statsTdb = new (space) ComTdbStats(tupleLength,       // Length of stats Tuple
                                                  ...
                                                  stats_row_atp_index, input_row_atp_index);
  ...
  // the parent of this node can get a handle on this explainTuple.
  if (!generator->explainDisabled()) {
    generator->setExplainTuple(addExplainInfo(statsTdb, 0, 0, generator));
  }
  generator->setCriDesc(givenDesc, Generator::DOWN);
  generator->setCriDesc(returnedDesc, Generator::UP);
...
short ControlRunningQuery::codeGen(Generator *generator) {
  ...
  GenAssert((child(0) == NULL) && (child(1) == NULL), "ControlRunningQuery does not expect any child.");

  char *qid = space->allocateAndCopyToAlignedSpace(queryId_, str_len(queryId_), 0);

  char *pname = space->allocateAndCopyToAlignedSpace(pname_, str_len(pname_), 0);

  char *comment = space->allocateAndCopyToAlignedSpace(comment_, str_len(comment_), 0);
  ...
  switch (action_) {
    case Cancel: {
      switch (qs_) {
        ...
        case ControlNidPid:
          break;
        default:
          GenAssert(0, "Invalid ControlRunningQuery::qs_");
          break;
      }
      break;
    }
    ...
    case Activate:
      break;
    default:
      GenAssert(0, "Invalid ControlRunningQuery::action_");
  }
  ...
  ComTdbCancel *exe_cancel_tdb =
      new (space) ComTdbCancel(qid, pname, nid_, pid_, getDefault(CANCEL_MINIMUM_BLOCKING_INTERVAL), a, forced_,
                               comment, generator->getCriDesc(Generator::DOWN), generator->getCriDesc(Generator::DOWN),
                               (queue_index)getDefault(GEN_DDL_SIZE_DOWN), (queue_index)getDefault(GEN_DDL_SIZE_UP));
  ...
  if (!generator->explainDisabled()) {
    generator->setExplainTuple(addExplainInfo(exe_cancel_tdb, 0, 0, generator));
  }
 ...
 *|------------------------------------|
 *| left child's data | pseudo columns |
 *|                   |                |
 *| ( D tupps )       | ( 1 tupp )     |
 *
 */
short ConnectBy::codeGen(Generator *generator) {
  ExpGenerator *expGen = generator->getExpGenerator();
  Space *space = generator->getSpace();
  ...
  short pathEvalAtpIndex = 5;

  short returnRowAtpIndex = -1;
  short returnedPseudoColRowAtpIndex = -1;
  short fixedPseudoColRowAtpIndex = -1;
  short pathPseudoColRowAtpIndex = -1;

  ex_expr *leftMoveExpr = 0;
  ex_expr *rightMoveExpr = 0;
  ex_expr *cond_expr = NULL;
  ex_expr *prior_cond_expr = NULL;
  ex_expr *left_path_expr = NULL;
  ex_expr *right_path_expr = NULL;

  ex_cri_desc *given_desc = generator->getCriDesc(Generator::DOWN);
  ex_cri_desc *workCriDesc = new (space) ex_cri_desc(6, space);
  ex_cri_desc *returned_desc = NULL;
  CmpCommon::context()->resetConnectByDual();
  if (pathItemList().entries() == 0) {
    returned_desc = new (space) ex_cri_desc(given_desc->noTuples() + 2, space);
    fixedPseudoColRowAtpIndex = returned_desc->noTuples() - 1;
    returnRowAtpIndex = returned_desc->noTuples() - 2;
  } else {
    returned_desc = new (space) ex_cri_desc(given_desc->noTuples() + 3, space);
    pathPseudoColRowAtpIndex = returned_desc->noTuples() - 1;
    fixedPseudoColRowAtpIndex = returned_desc->noTuples() - 2;
    returnRowAtpIndex = returned_desc->noTuples() - 3;
  }

  short rc = 0;
  generator->setCriDesc(given_desc, Generator::DOWN);
  rc = child(0)->codeGen(generator);
  ComTdb *s_child_tdb = (ComTdb *)(generator->getGenObj());
  ExplainTuple *childExplainTuple = generator->getExplainTuple();
  ex_cri_desc *left_child_desc = generator->getCriDesc(Generator::UP);

  ex_cri_desc *rightDownDesc = new (space) ex_cri_desc(given_desc->noTuples() + 1, space);
  for (int i = 0; i < given_desc->noTuples(); i++) {
    rightDownDesc->setTupleDescriptor(i, given_desc->getTupleDescriptor(i));
  }
  UInt32 priortupleLength = 0;
  MapTable *myMapTableMM = NULL;

  ExpTupleDesc *priorValueExprDesc = 0;
  short priorValsValsDownAtpIndex = rightDownDesc->noTuples() - 1;

  if (priorValues().entries() > 0) {
    MapTable *myMapTableMM1 = generator->appendAtEnd();
    myMapTableMM = generator->appendAtEnd();
    ExpTupleDesc *priorValTupleDesc = 0;
    expGen->processValIdList(priorValues(), ExpTupleDesc::SQLARK_EXPLODED_FORMAT, priortupleLength, 0,
                             priorValsValsDownAtpIndex, &priorValueExprDesc, ExpTupleDesc::LONG_FORMAT);
  ...
  generator->setCriDesc(rightDownDesc, Generator::DOWN);
  rc = child(1)->codeGen(generator);

  ComTdb *c_child_tdb = (ComTdb *)(generator->getGenObj());
  ExplainTuple *connExplainTuple = generator->getExplainTuple();
  ...
  if (pseudoColList().entries() > 0) {
    ...
    for (CollIndex i = 0; i < pseudoColList().entries(); i++) {
      ItemExpr *col_node = pseudoColList()[i].getItemExpr();
      attrs[i] = (generator->addMapInfo(col_node->getValueId(), 0))->getAttr();
    }
    ExpTupleDesc *pseudotupleDescReturn = 0;

    ExpTupleDesc::TupleDataFormat tupleFormat = ExpTupleDesc::SQLARK_EXPLODED_FORMAT;

    expGen->processAttributes(pseudoColList().entries(), attrs, tupleFormat, ptupleLength, 0, fixedPseudoColRowAtpIndex,
                              &pseudotupleDescReturn, ExpTupleDesc::LONG_FORMAT);
    returned_desc->setTupleDescriptor(fixedPseudoColRowAtpIndex, pseudotupleDescReturn);
    NADELETEBASIC(attrs, generator->wHeap());
  }
  if (pathItemList().entries() > 0) {
    Attributes **attrs = new (generator->wHeap()) Attributes *[1];
    ItemExpr *col_node = pathVid_.getItemExpr();
    attrs[0] = (generator->addMapInfo(col_node->getValueId(), 0))->getAttr();
    ExpTupleDesc *pathtupleDescReturn = 0;

    ExpTupleDesc::TupleDataFormat tupleFormat = ExpTupleDesc::SQLARK_EXPLODED_FORMAT;

    expGen->processAttributes(1, attrs, tupleFormat, pathTupleLen, 0, pathPseudoColRowAtpIndex, &pathtupleDescReturn,
                              ExpTupleDesc::LONG_FORMAT);
    returned_desc->setTupleDescriptor(pathPseudoColRowAtpIndex, pathtupleDescReturn);
    NADELETEBASIC(attrs, generator->wHeap());
  }
  ...
  CollIndex i;
  ExpTupleDesc *tuple_desc = 0;
  ex_expr *priorValMoveExpr2 = NULL;
  ex_expr *priorValMoveExpr1 = NULL;
  ExpTupleDesc *priorMove1TupeDesc = 0;

  ValueIdList priorLeftSrcList;
  ValueIdList priorRightSrcList;
  for (i = 0; i < colMapTable().entries(); i++) {
    ...
    cnode = new (generator->wHeap())
        Cast(((vidu_node->getLeftSource()).getValueDesc())->getItemExpr(), &(vidu_node->getResult().getType()));
    cnode->bindNode(generator->getBindWA());
    if (generator->getBindWA()->errStatus()) return 0;
    left_val_id_list.insert(cnode->getValueId());

    cnode = new (generator->wHeap())
        Cast(((vidu_node->getRightSource()).getValueDesc())->getItemExpr(), &(vidu_node->getResult().getType()));
    cnode->bindNode(generator->getBindWA());
    if (generator->getBindWA()->errStatus()) return 0;
    right_val_id_list.insert(cnode->getValueId());
  }
  // fill the priorRightSrcList and priorLeftSrcList
  for (int i = 0; i < childColList().entries(); i++) {
    Cast *newsnode;
    newsnode = new (generator->wHeap())
        Cast(((childColList()[i]).getValueDesc())->getItemExpr(), &parentColList()[i].getType());
    newsnode->bindNode(generator->getBindWA());
    if (generator->getBindWA()->errStatus()) return 0;
    priorLeftSrcList.insert(newsnode->getValueId());
    Cast *newcnode;
    newcnode = new (generator->wHeap())
        Cast(((rchildColList()[i]).getValueDesc())->getItemExpr(), &parentColList()[i].getType());
    newcnode->bindNode(generator->getBindWA());
    if (generator->getBindWA()->errStatus()) return 0;
  ...
  if (left_val_id_list.entries() > 0) {
    expGen->generateContiguousMoveExpr(left_val_id_list, 0,
                                       1,  // this is the connectby atp
                                       returnRowAtpIndex, tupleFormat,
                                       leftRowLength,  // actually this is the row length
                                       &leftMoveExpr, &tuple_desc, ExpTupleDesc::SHORT_FORMAT);
    expGen->generateContiguousMoveExpr(right_val_id_list,
                                       0,  // don't add convert nodes
                                       1, returnRowAtpIndex, tupleFormat, rightRowLength, &rightMoveExpr, &tuple_desc,
                                       ExpTupleDesc::SHORT_FORMAT);
  }
  returned_desc->setTupleDescriptor(returnRowAtpIndex, tuple_desc);
  ExpTupleDesc *path_tuple_desc = 0;
  UInt32 pathItemLength = 0;
  if (pathItemList().entries() > 0) {
    expGen->generateContiguousMoveExpr(leftPathItemList(), 1,
                                       1,  // this is the connectby atp
                                       pathEvalAtpIndex, ExpTupleDesc::SQLARK_EXPLODED_FORMAT,
                                       pathItemLength,  // actually this is the row length
                                       &left_path_expr, &path_tuple_desc, ExpTupleDesc::LONG_FORMAT);
    expGen->generateContiguousMoveExpr(rightPathItemList(), 1,
                                       1,  // this is the connectby atp
                                       pathEvalAtpIndex, ExpTupleDesc::SQLARK_EXPLODED_FORMAT,
                                       pathItemLength,  // actually this is the row length
                                       &right_path_expr);
    workCriDesc->setTupleDescriptor(pathEvalAtpIndex, path_tuple_desc);
  }

  ExpTupleDesc *priorPredExprDesc = 0;
  // generate predicate expr for connect by
  if (priorLeftSrcList.entries() > 0) {
    Attributes **attrs = new (generator->wHeap()) Attributes *[priorLeftSrcList.entries()];
    for (CollIndex i = 0; i < priorLeftSrcList.entries(); i++) {
      ItemExpr *col_node = priorLeftSrcList[i].getItemExpr();
      attrs[i] = (generator->addMapInfo(col_node->getValueId(), 0))->getAttr();
    }

    expGen->processAttributes(priorLeftSrcList.entries(), attrs, ExpTupleDesc::SQLARK_EXPLODED_FORMAT, priortupleLength,
                              0, returnRowAtpIndex, &priorPredExprDesc, ExpTupleDesc::SHORT_FORMAT);

    workCriDesc->setTupleDescriptor(priorPredAtpIndex, priorPredExprDesc);
    NADELETEBASIC(attrs, generator->wHeap());
  }

  ex_expr *priorPredExpr = NULL;
  expGen->generateContiguousMoveExpr(priorLeftSrcList,
                                     0,  // don't add convert nodes
                                     1, priorPredAtpIndex, ExpTupleDesc::SQLARK_EXPLODED_FORMAT, priortupleLength,
                                     &priorValMoveExpr1, &priorPredExprDesc, ExpTupleDesc::SHORT_FORMAT);

  expGen->generateContiguousMoveExpr(priorRightSrcList,
                                     0,  // don't add convert nodes
                                     1, priorPredAtpIndex, ExpTupleDesc::SQLARK_EXPLODED_FORMAT, priortupleLength,
                                     &priorValMoveExpr2);  //,priorMove1TupeDesc,ExpTupleDesc::SHORT_FORMAT);
  for (i = 0; i < colMapTable().entries(); i++) {
    ValueIdUnion *vidu_node = (ValueIdUnion *)(((colMapTable()[i]).getValueDesc())->getItemExpr());

    Attributes *attr =
        // generator->addMapInfoToThis(my_map_table, vidu_node->getValueId(),
        generator->addMapInfo(vidu_node->getValueId(), generator->getMapInfo(left_val_id_list[i])->getAttr())
            ->getAttr();
    attr->setAtp(0);
  }
  ...
  selectionPred() += condExpr();

  if (NOT selectionPred().isEmpty()) {
    ItemExpr *condExp = selectionPred().rebuildExprTree(ITM_AND, TRUE, TRUE);

    expGen->generateExpr(condExp->getValueId(), ex_expr::exp_SCAN_PRED, &cond_expr);
  }
  prior_cond_expr = 0;
  if (NOT priorCond().isEmpty()) {
    ItemExpr *condExp = priorCond().rebuildExprTree(ITM_AND, TRUE, TRUE);

    expGen->generateExpr(condExp->getValueId(), ex_expr::exp_SCAN_PRED, &prior_cond_expr);
  }

  ComTdbConnectBy *connectBy_tdb = new (space) ComTdbConnectBy(
      workCriDesc, given_desc, returned_desc, s_child_tdb->getMaxQueueSizeDown(), s_child_tdb->getMaxQueueSizeUp(), 20,
      4096 * 2000,  // the reason is to ensure one of the Agree query can have enough memory
      s_child_tdb, c_child_tdb, leftRowLength, ptupleLength, leftMoveExpr, rightMoveExpr, returnRowAtpIndex,
      fixedPseudoColRowAtpIndex,

      priorPredExpr, priorPredAtpIndex, priortupleLength, priorValMoveExpr1, priorValMoveExpr2,

      priorValsValsDownAtpIndex,

      rightDownDesc,

      cond_expr,

      pathEvalAtpIndex, left_path_expr, right_path_expr,

      pathPseudoColRowAtpIndex, pathTupleLen, pathItemLength,

      prior_cond_expr);

  connectBy_tdb->setRecordLength(getGroupAttr()->getRecordLength());
  connectBy_tdb->setDel(pathDel_);
  connectBy_tdb->setNoCycle(nocycle_);
  connectBy_tdb->setUseCache(useCache_);
  if (priorValMoveExpr1 == NULL)
    connectBy_tdb->setNoPrior(TRUE);
  else
    connectBy_tdb->setNoPrior(FALSE);

  generator->initTdbFields(connectBy_tdb);

  if (!generator->explainDisabled()) {
    generator->setExplainTuple(addExplainInfo(connectBy_tdb, childExplainTuple, connExplainTuple, generator));
  }

  generator->setCriDesc(given_desc, Generator::DOWN);
  generator->setCriDesc(returned_desc, Generator::UP);

  generator->setGenObj(this, connectBy_tdb);

  return 0;
}

short ConnectByTempTable::codeGen(Generator *generator) {
  ExpGenerator *expGen = generator->getExpGenerator();
  Space *space = generator->getSpace();
  ...
  unsigned short hashInputValIdx = 5;
  unsigned short encodeInputProbeDataIdx = 6;

  child(0)->codeGen(generator);
  ComTdb *child_tdb = (ComTdb *)(generator->getGenObj());
  ExplainTuple *childExplainTuple = generator->getExplainTuple();

  ex_cri_desc *work_cri_desc = new (space) ex_cri_desc(7, space);

  ValueIdList inputsToUse = probeValues();

  ValueIdList hvAsListInput;

  ItemExpr *probeHashAsIeInput = new (generator->wHeap()) HashDistPartHash(inputsToUse.rebuildExprTree(ITM_ITEM_LIST));

  probeHashAsIeInput->bindNode(generator->getBindWA());

  NumericType &nTypInput = (NumericType &)probeHashAsIeInput->getValueId().getType();
  GenAssert(nTypInput.isSigned() == FALSE, "Unexpected signed HashDistPartHash.");

  GenAssert(probeHashAsIeInput->getValueId().getType().supportsSQLnullLogical() == FALSE,
            "Unexpected nullable HashDistPartHash.");

  ItemExpr *hvAsIeInput = new (generator->wHeap())
      Cast(probeHashAsIeInput, new (generator->wHeap()) SQLInt(generator->wHeap(), FALSE,  // false == unsigned.
                                                               FALSE                       // false == not nullable.
                                                               ));

  hvAsIeInput->bindNode(generator->getBindWA());
  ...
  for (inputListIndex = 0; inputListIndex < inputsToUse.entries(); inputListIndex++) {
    ItemExpr *inputIeInput = (inputsToUse[inputListIndex].getValueDesc())->getItemExpr();

    if (inputIeInput->getValueId().getType().getVarLenHdrSize() > 0) {
      const CharType &char_type = (CharType &)(inputIeInput->getValueId().getType());

      if (!CollationInfo::isSystemCollation(char_type.getCollation())) {
        inputIeInput = new (generator->wHeap())
            Cast(inputIeInput,
                 (new (generator->wHeap()) SQLChar(
                     generator->wHeap(), CharLenInfo(char_type.getStrCharLimit(), char_type.getDataStorageSize()),
                     char_type.supportsSQLnull(), FALSE, FALSE, FALSE, char_type.getCharSet(), char_type.getCollation(),
                     char_type.getCoercibility())));
      }
    }

    CompEncode *enodeInput =
        new (generator->wHeap()) CompEncode(inputIeInput, FALSE /* ascend/descend doesn't matter*/);

    enodeInput->bindNode(generator->getBindWA());
    encodeInputAsListInput.insert(enodeInput->getValueId());
  }
  ...
  expGen->generateContiguousMoveExpr(encodeInputAsListInput,
                                     ...
                                     encodedInputLengthInput, &encodeInputHostVarExpr);
  ...
  expGen->generateContiguousMoveExpr(innerTableAsList,
                                     -1,  // do add conv nodes
                                     work_atp, innerRowDataIdx, tupleFormat, innerRecLength, &innerRecExpr,
                                     &innerRecTupleDesc, ExpTupleDesc::SHORT_FORMAT, &returnedMapTable);

  returned_desc->setTupleDescriptor(returned_desc->noTuples() - 1, innerRecTupleDesc);

  inputsToUse = keyColumns();
  ValueIdList hvAsList;

  ItemExpr *probeHashAsIe = new (generator->wHeap()) HashDistPartHash(inputsToUse.rebuildExprTree(ITM_ITEM_LIST));

  probeHashAsIe->bindNode(generator->getBindWA());

  NumericType &nTyp = (NumericType &)probeHashAsIe->getValueId().getType();
  GenAssert(nTyp.isSigned() == FALSE, "Unexpected signed HashDistPartHash.");

  GenAssert(probeHashAsIe->getValueId().getType().supportsSQLnullLogical() == FALSE,
            "Unexpected nullable HashDistPartHash.");

  ItemExpr *hvAsIe = new (generator->wHeap())
      Cast(probeHashAsIe, new (generator->wHeap()) SQLInt(generator->wHeap(), FALSE,  // false == unsigned.
                                                          FALSE                       // false == not nullable.
                                                          ));
  ...
  for (inputListIndex = 0; inputListIndex < inputsToUse.entries(); inputListIndex++) {
    ItemExpr *inputIe = (inputsToUse[inputListIndex].getValueDesc())->getItemExpr();

    if (inputIe->getValueId().getType().getVarLenHdrSize() > 0) {
      const CharType &char_type = (CharType &)(inputIe->getValueId().getType());

      if (!CollationInfo::isSystemCollation(char_type.getCollation())) {
        inputIe = new (generator->wHeap()) Cast(
            inputIe, (new (generator->wHeap()) SQLChar(
                         generator->wHeap(), CharLenInfo(char_type.getStrCharLimit(), char_type.getDataStorageSize()),
                         char_type.supportsSQLnull(), FALSE, FALSE, FALSE, char_type.getCharSet(),
                         char_type.getCollation(), char_type.getCoercibility())));
      }
    ...
    CompEncode *enode = new (generator->wHeap()) CompEncode(inputIe, FALSE /* ascend/descend doesn't matter*/);
  ...
  for (CollIndex i = 0; i < innerTableAsList.entries(); i++) {
    ...
    Attributes *attrib = generator->getMapInfo(innerValId)->getAttr();
  ...
  if (!selectionPred().isEmpty()) {
    ItemExpr *selPredTree = selectionPred().rebuildExprTree(ITM_AND, TRUE, TRUE);
    expGen->generateExpr(selPredTree->getValueId(), ex_expr::exp_SCAN_PRED, &selectPred);
  }

  ComTdbConnectByTempTable *connectByTempTable_tdb = new (space) ComTdbConnectByTempTable(
      child_tdb, hvExpr, encodeInputExpr, innerRecExpr, encodedInputLength, innerRecLength, pcNumEntries,
      returned_desc->noTuples() - 1, hashValIdx, encodedProbeDataIdx, innerRowDataIdx, work_cri_desc, given_desc,
      returned_desc, child_tdb->getMaxQueueSizeDown(), child_tdb->getMaxQueueSizeUp(), 1, 4096 * 10000,
      encodeInputHostVarExpr, hvInputExpr, hashInputValIdx, encodeInputProbeDataIdx, selectPred);

  connectByTempTable_tdb->setRecordLength(getGroupAttr()->getRecordLength());

  generator->initTdbFields(connectByTempTable_tdb);

  if (!generator->explainDisabled()) {
    generator->setExplainTuple(addExplainInfo(connectByTempTable_tdb, childExplainTuple, 0, generator));
  }

  generator->setGenObj(this, connectByTempTable_tdb);

  return 0;
}

// -----------------------------------------------------------------------
// QueryInvalidationFunc methods
// -----------------------------------------------------------------------

const char *QueryInvalidationFunc::getVirtualTableName() { return getVirtualTableNameStr(); }

TrafDesc *QueryInvalidationFunc::createVirtualTableDesc() {
  TrafDesc *table_desc = Generator::createVirtualTableDesc(
      getVirtualTableName(),
      NULL,  // let it decide what heap to use
      ComTdbQryInvalid::getVirtTableNumCols(), ComTdbQryInvalid::getVirtTableColumnInfo(),
      ComTdbQryInvalid::getVirtTableNumKeys(), ComTdbQryInvalid::getVirtTableKeyInfo());
  return table_desc;
}

short QueryInvalidationFunc::codeGen(Generator *generator) {
  ExpGenerator *expGen = generator->getExpGenerator();
  Space *space = generator->getSpace();
  ...
  // first two entries for consts and temps.
  // Entry 1(index #2) is
  // where the QI row will be moved to evaluate
  // the 'where' predicate.
  // Entry 2(index #3) is where the input row will be built.
  ...
  expGen->processAttributes(getTableDesc()->getColumnList().entries(), attrs, tupleFormat, tupleLength, work_atp,
                            qi_row_atp_index, &tupleDesc, ExpTupleDesc::LONG_FORMAT);

  NADELETEBASIC(attrs, generator->wHeap());

  workCriDesc->setTupleDescriptor(qi_row_atp_index, tupleDesc);
  ...
  ComTdbQryInvalid *qiTdb = new (space) ComTdbQryInvalid(tupleLength,       // Length of stats Tuple
                                                         ...
                                                         inputExpr, projExpr,
                                                         workCriDesc,  // Descriptor of work Atp
                                                         qi_row_atp_index, input_row_atp_index);

  generator->initTdbFields(qiTdb);

  // Add the explain Information for this node to the EXPLAIN
  // Fragment.  Set the explainTuple pointer in the generator so
  // the parent of this node can get a handle on this explainTuple.
  if (!generator->explainDisabled()) {
    generator->setExplainTuple(addExplainInfo(qiTdb, 0, 0, generator));
  }
  generator->setCriDesc(givenDesc, Generator::DOWN);
  generator->setCriDesc(returnedDesc, Generator::UP);
  generator->setGenObj(this, qiTdb);

  return 0;
}

// -----------------------------------------------------------------------
// QueryInvalidationFunc methods END
// -----------------------------------------------------------------------
