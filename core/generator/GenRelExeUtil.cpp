...
NAString GenBulkloadSampleTableName(const NATable *pTable) {
  NAString sName;
  if (!pTable) return sName;
  sName = pTable->getTableName().getCatalogName();
  sName.append(".")
      .append(pTable->getTableName().getSchemaName())
      .append(".")
...
short GenericUtilExpr::processOutputRow(Generator *generator, const int work_atp, const int output_row_atp_index,
  ...
  Attributes **attrs = new (generator->wHeap()) Attributes *[virtTableDesc->getColumnList().entries()];
  ...
  if (NOT noAtpOrIndexChange) {
    ...
    expGen->assignAtpAndAtpIndex(getVirtualTableDesc()->getColumnList(), 0, returnedDesc->noTuples() - 1);
  }
...
short ExeUtilProcessVolatileTable::codeGen(Generator *generator) {
  ...
  if (NOT isSchema_) {
    volTabName = space->AllocateAndCopyToAlignedSpace(GenGetQualifiedName(volTabName_), 0);
  }
  ...
  ComTdbProcessVolatileTable *pvt_tdb = new (space) ComTdbProcessVolatileTable(
      ddlStmt, strlen(ddlStmt), getDDLStmtTextCharSet(), volTabName, (volTabName ? strlen(volTabName) : 0), isCreate_,
      isTable_, isIndex_, isSchema_, catSchNameStr, strlen(catSchNameStr), 0, 0,  // no input expr
      0, 0,                                                                       // no output expr
  ...
  if (!generator->explainDisabled()) {
    generator->setExplainTuple(addExplainInfo(pvt_tdb, 0, 0, generator));
  }
...
//
/////////////////////////////////////////////////////////
const char *ExeUtilExpr::getVirtualTableName() { return ("EXE_UTIL_EXPR__"); }
...
TrafDesc *ExeUtilExpr::createVirtualTableDesc() {
  TrafDesc *table_desc =
      Generator::createVirtualTableDesc(getVirtualTableName(),
                                        ...
                                        ComTdbExeUtil::getVirtTableNumCols(), ComTdbExeUtil::getVirtTableColumnInfo(),
                                        ComTdbExeUtil::getVirtTableNumKeys(), ComTdbExeUtil::getVirtTableKeyInfo());
  return table_desc;
}
...
TrafDesc *ExeUtilDisplayExplain::createVirtualTableDesc() {
  ...
  TrafDesc *table_desc =
      Generator::createVirtualTableDesc(getVirtualTableName(),
                                        NULL,  // let it decide what heap to use
                                        ComTdbExeUtilDisplayExplain::getVirtTableNumCols(), vtci,
                                        ComTdbExeUtil::getVirtTableNumKeys(), ComTdbExeUtil::getVirtTableKeyInfo());
  return table_desc;
}
...
short ExeUtilDisplayExplain::codeGen(Generator *generator) {
  ...
  ComTdbExeUtilDisplayExplain *exe_util_tdb = new (space) ComTdbExeUtilDisplayExplain(
      ...
      0, 0,                                                                               // no output expr
      0, 0,                                                                               // no work cri desc
      colDescSize, outputRowSize, givenDesc, returnedDesc, (queue_index)8, (queue_index)1024,
      2,       // num buffers
      32000);  // bufferSIze
...
short ExeUtilDisplayExplainComplex::codeGen(Generator *generator) {
  ...
  objectName = space->AllocateAndCopyToAlignedSpace(objectName_, 0);
  ...
  if (getExprNode()->getOperatorType() == REL_EXE_UTIL) {
    if ((((ExeUtilExpr *)getExprNode()->castToRelExpr())->getExeUtilType()) == ExeUtilExpr::CREATE_TABLE_AS_) {
      ExeUtilCreateTableAs *ctas = (ExeUtilCreateTableAs *)getExprNode()->castToRelExpr();

      StmtDDLCreateTable *pNode = ctas->getExprNode()->castToStmtDDLNode()->castToStmtDDLCreateTable();
      if (pNode->loadIfExists()) exe_util_tdb->setLoadIfExists(TRUE);
    }
  }
...
short ExeUtilWnrInsert::codeGen(Generator *generator) {
  ...
  if (getUtilTableDesc() && getUtilTableDesc()->getNATable()->isVolatileTable()) {
    tablename = space->AllocateAndCopyToAlignedSpace(
        getTableName().getQualifiedNameObj().getQualifiedNameAsAnsiString(TRUE), 0);
  } else {
    tablename = space->AllocateAndCopyToAlignedSpace(GenGetQualifiedName(getTableName()), 0);
  }
  ...
  ComTdbExeUtilAqrWnrInsert *exe_util_tdb = new (space) ComTdbExeUtilAqrWnrInsert(
  ...
  if (CmpCommon::getDefault(AQR_WNR_LOCK_INSERT_TARGET) == DF_ON) exe_util_tdb->lockTarget(true);

  if (child_tdb) exe_util_tdb->setChildTdb(child_tdb);
  ...
  if (!generator->explainDisabled() && (CmpCommon::getDefault(AQR_WNR_EXPLAIN_INSERT) == DF_ON)) {
...
short ExeUtilCleanupVolatileTables::codeGen(Generator *generator) {
  ...
  ComTdbExeUtilCleanupVolatileTables *exe_util_tdb = new (space) ComTdbExeUtilCleanupVolatileTables(
      catName, (catName ? strlen(catName) : 0), 0, 0,  // no work cri desc
      (ex_cri_desc *)(generator->getCriDesc(Generator::DOWN)), (ex_cri_desc *)(generator->getCriDesc(Generator::DOWN)),
      (queue_index)getDefault(GEN_DDL_SIZE_DOWN), (queue_index)getDefault(GEN_DDL_SIZE_UP),
      getDefault(GEN_DDL_NUM_BUFFERS), getDefault(GEN_DDL_BUFFER_SIZE));
  ...
  if (type_ == ALL_TABLES_IN_ALL_CATS) exe_util_tdb->setCleanupAllTables(TRUE);

  if (CmpCommon::getDefault(CSE_CLEANUP_HIVE_TABLES) != DF_OFF) exe_util_tdb->setCleanupHiveCSETables(TRUE);
...
short ExeUtilGetVolatileInfo::codeGen(Generator *generator) {
  ...
  ComTdbExeUtilGetVolatileInfo *exe_util_tdb =
      new (space) ComTdbExeUtilGetVolatileInfo(param1, param2, 0, 0,  // no work cri desc
                                               givenDesc, returnedDesc, (queue_index)8, (queue_index)64,
  ...
  if (vtCatSpecified) exe_util_tdb->setVTCatSpecified(TRUE);
...
short ExeUtilGetErrorInfo::codeGen(Generator *generator) {
  ...
  ComTdbExeUtilGetErrorInfo *exe_util_tdb =
      new (space) ComTdbExeUtilGetErrorInfo(errType_, errNum_, 0, 0,  // no work cri desc
                                            givenDesc, returnedDesc, (queue_index)8, (queue_index)64,
                                            2,       // getDefault(GEN_DDL_NUM_BUFFERS),
                                            32000);  // getDefault(GEN_DDL_BUFFER_SIZE));
...
short ExeUtilCreateTableAs::codeGen(Generator *generator) {
  ...
  char *usQuery = space->allocateAlignedSpace(usQuery_.length() + 1);
...
short ExeUtilGetObjectEpochStats::codeGen(Generator *generator) {
  ...
  ComTdbExeUtilGetObjectEpochStats *exe_util_tdb = new (space) ComTdbExeUtilGetObjectEpochStats(
      tableName, tableNameLen, getCpu(), getLocked(), givenDesc, returnedDesc, (queue_index)8, (queue_index)512,
...
short ExeUtilGetObjectLockStats::codeGen(Generator *generator) {
  ...
  ComTdbExeUtilGetObjectLockStats *exe_util_tdb = new (space) ComTdbExeUtilGetObjectLockStats(
      tableName, tableNameLen, getCpu(), isEntry(), givenDesc, returnedDesc, (queue_index)8, (queue_index)512,
      2,       // getDeault(GEN_DDL_NUM_BUFFERS),
      32000);  // getDefault(GEN_DDL_BUFFER_SIZE));
...
short ExeUtilGetStatistics::codeGen(Generator *generator) {
  ...
  if (statsReqType_ == SQLCLI_STATS_REQ_PROCESS_INFO)
    exe_util_tdb = new (space) ComTdbExeUtilGetProcessStatistics(
        stmtName, statsReqType_, statsMergeType_, activeQueryNum_, 0, 0,  // no work cri desc
        givenDesc, returnedDesc, (queue_index)8, (queue_index)512,
        2,       // getDefault(GEN_DDL_NUM_BUFFERS),
        32000);  // getDefault(GEN_DDL_BUFFER_SIZE));
    ...
    exe_util_tdb = new (space)
        ComTdbExeUtilGetStatistics(stmtName, statsReqType_, statsMergeType_, activeQueryNum_, 0, 0,  // no work cri desc
                                   givenDesc, returnedDesc, (queue_index)8, (queue_index)512,
                                   2,      // getDefault(GEN_DDL_NUM_BUFFERS),
                                   32000,  // getDefault(GEN_DDL_BUFFER_SIZE));
                                   hostPtr, port, pathPtr, generator->getQueryHash());
  }
...
short ExeUtilGetProcessStatistics::codeGen(Generator *generator) { return ExeUtilGetStatistics::codeGen(generator); }
...
//
/////////////////////////////////////////////////////////
const char *ExeUtilGetUID::getVirtualTableName() { return "EXE_UTIL_GET_UID__"; }

TrafDesc *ExeUtilGetUID::createVirtualTableDesc() {
  TrafDesc *table_desc = Generator::createVirtualTableDesc(
      getVirtualTableName(),
      NULL,  // let it decide what heap to use
      ComTdbExeUtilGetUID::getVirtTableNumCols(), ComTdbExeUtilGetUID::getVirtTableColumnInfo(),
      ComTdbExeUtilGetUID::getVirtTableNumKeys(), ComTdbExeUtilGetUID::getVirtTableKeyInfo());
  return table_desc;
}
...
short ExeUtilGetUID::codeGen(Generator *generator) {
  ...
  ComTdbExeUtilGetUID *exe_util_tdb =
      new (space) ComTdbExeUtilGetUID(uid_, 0, 0,  // no work cri desc
                                      givenDesc, returnedDesc, (queue_index)8, (queue_index)512,
                                      2,       // getDefault(GEN_DDL_NUM_BUFFERS),
                                      32000);  // getDefault(GEN_DDL_BUFFER_SIZE));
...
//
/////////////////////////////////////////////////////////
const char *ExeUtilGetQID::getVirtualTableName() { return "EXE_UTIL_GET_QID__"; }

TrafDesc *ExeUtilGetQID::createVirtualTableDesc() {
  TrafDesc *table_desc = Generator::createVirtualTableDesc(
      getVirtualTableName(),
      NULL,  // let it decide what heap to use
      ComTdbExeUtilGetQID::getVirtTableNumCols(), ComTdbExeUtilGetQID::getVirtTableColumnInfo(),
      ComTdbExeUtilGetQID::getVirtTableNumKeys(), ComTdbExeUtilGetQID::getVirtTableKeyInfo());
  return table_desc;
}
...
short ExeUtilGetQID::codeGen(Generator *generator) {
  ...
  char *stmt_name = space->AllocateAndCopyToAlignedSpace(statement_, 0);

  ComTdbExeUtilGetQID *exe_util_tdb =
      new (space) ComTdbExeUtilGetQID(stmt_name, 0, 0,  // no work cri desc
                                      givenDesc, returnedDesc, (queue_index)8, (queue_index)512,
                                      2,       // getDefault(GEN_DDL_NUM_BUFFERS),
                                      32000);  // getDefault(GEN_DDL_BUFFER_SIZE));
...
short ExeUtilPopulateInMemStats::codeGen(Generator *generator) {
  ...
  inMemHistogramsTableName = space->AllocateAndCopyToAlignedSpace(inMemHistogramsStr, 0);
  inMemHistintsTableName = space->AllocateAndCopyToAlignedSpace(inMemHistintsStr, 0);

  sourceTableCatName = space->AllocateAndCopyToAlignedSpace(sourceTableName_.getQualifiedNameObj().getCatalogName(), 0);
  sourceTableSchName = space->AllocateAndCopyToAlignedSpace(sourceTableName_.getQualifiedNameObj().getSchemaName(), 0);
  sourceTableObjName = space->AllocateAndCopyToAlignedSpace(sourceTableName_.getQualifiedNameObj().getObjectName(), 0);
  sourceHistogramsTableName = space->AllocateAndCopyToAlignedSpace(sourceHistogramsStr, 0);
  sourceHistintsTableName = space->AllocateAndCopyToAlignedSpace(sourceHistintsStr, 0);

  ComTdbExeUtilPopulateInMemStats *exe_util_tdb = new (space) ComTdbExeUtilPopulateInMemStats(
      uid_, inMemHistogramsTableName, inMemHistintsTableName, sourceTableCatName, sourceTableSchName,
      sourceTableObjName, sourceHistogramsTableName, sourceHistintsTableName, 0, 0,  // no work cri desc
      givenDesc, returnedDesc, (queue_index)8, (queue_index)512,
      2,       // getDefault(GEN_DDL_NUM_BUFFERS),
      32000);  // getDefault(GEN_DDL_BUFFER_SIZE));
...
short ExeUtilGetMetadataInfo::codeGen(Generator *generator) {
  ...
  short rc = processOutputRow(generator, work_atp, exe_util_row_atp_index, returnedDesc, TRUE);
  if (rc) {
    return -1;
  }
  ...
  if (NOT pattern_.isNull()) {
    ItemExpr *colNode = (((getVirtualTableDesc()->getColumnList())[0]).getValueDesc())->getItemExpr();

    ItemExpr *patternNode = new (generator->wHeap()) ConstValue(pattern_.data());

    ItemExpr *escapeNode = new (generator->wHeap()) ConstValue("\\");

    ItemExpr *patternTree = new (generator->wHeap()) Like(colNode, patternNode, escapeNode);
  ...
  static const QueryInfoStruct qis[] = {
      ...
      //    {  "USER",   "MVS",       "",      "",         1,      0,        0,      0,
      //    ComTdbExeUtilGetMetadataInfo::MVS_IN_SCHEMA_ },
      //    {  "USER",   "MVGROUPS",  "",      "",         0,      0,        0,      0,
      //    ComTdbExeUtilGetMetadataInfo::MVGROUPS_IN_SCHEMA_ },
      //    {  "USER",   "SYNONYMS",  "",      "",         1,      0,        0,      0,
      //    ComTdbExeUtilGetMetadataInfo::SYNONYMS_IN_SCHEMA_ },
      ...
      //    {  "ALL",    "MVS",       "",      "",         1,      0,        0,      0,
      //    ComTdbExeUtilGetMetadataInfo::MVS_IN_SCHEMA_ },
      //    {  "ALL",    "MVGROUPS",  "",      "",         0,      0,        0,      0,
      //    ComTdbExeUtilGetMetadataInfo::MVGROUPS_IN_SCHEMA_ },
      //    {  "ALL",    "SYNONYMS",  "",      "",         1,      0,        0,      0,
      //    ComTdbExeUtilGetMetadataInfo::SYNONYMS_IN_SCHEMA_ },
      ...
      {"USER", "HBASE_REG_TABLES", "IN", "CATALOG", 1, 1, 0, 0,
       ComTdbExeUtilGetMetadataInfo::HBASE_REG_TABLES_IN_CATALOG_},
      //    {  "ALL",    "INVALID_VIEWS",   "IN",      "CATALOG",         1,      1,        0,      0,
      //    ComTdbExeUtilGetMetadataInfo::INVALID_VIEWS_IN_CATALOG_ },
      {"USER", "HBASE_MAP_TABLES", "IN", "CATALOG", 1, 1, 0, 0,
       ComTdbExeUtilGetMetadataInfo::HBASE_MAP_TABLES_IN_CATALOG_},
      ...
      //    {  "ALL",   "INVALID_VIEWS",   "IN",      "SCHEMA",         1,      2,        0,      0,
      //    ComTdbExeUtilGetMetadataInfo::INVALID_VIEWS_IN_SCHEMA_ },
      ...
      //    {  "USER",   "MVS",       "IN",    "SCHEMA",   1,      2,        0,      0,
      //    ComTdbExeUtilGetMetadataInfo::MVS_IN_SCHEMA_ },
      //    {  "USER",   "MVGROUPS",  "IN",    "SCHEMA",   0,      2,        0,      0,
      //    ComTdbExeUtilGetMetadataInfo::MVGROUPS_IN_SCHEMA_ },
      //    {  "USER",   "SYNONYMS",  "IN",    "SCHEMA",   1,      2,        0,      0,
      //    ComTdbExeUtilGetMetadataInfo::SYNONYMS_IN_SCHEMA_ },
      ...
      //    {  "ALL",    "MVS",       "IN",    "SCHEMA",   1,      2,        0,      0,
      //    ComTdbExeUtilGetMetadataInfo::MVS_IN_SCHEMA_ },
      //    {  "ALL",    "MVGROUPS",  "IN",    "SCHEMA",   0,      2,        0,      0,
      //    ComTdbExeUtilGetMetadataInfo::MVGROUPS_IN_SCHEMA_ },
      //    {  "ALL",    "SYNONYMS",  "IN",    "SCHEMA",   1,      2,        0,      0,
      //    ComTdbExeUtilGetMetadataInfo::SYNONYMS_IN_SCHEMA_ },
      //    {  "IUDLOG",    "TABLES",  "IN",    "SCHEMA",   1,      2,        0,      0,
      //    ComTdbExeUtilGetMetadataInfo::IUDLOG_TABLES_IN_SCHEMA_ },
      //    {  "RANGELOG",    "TABLES",  "IN",    "SCHEMA",   1,      2,        0,      0,
      //    ComTdbExeUtilGetMetadataInfo::RANGELOG_TABLES_IN_SCHEMA_ },
      //    {  "TRIGTEMP",    "TABLES",  "IN",    "SCHEMA",   1,      2,        0,      0,
      //    ComTdbExeUtilGetMetadataInfo::TRIGTEMP_TABLES_IN_SCHEMA_ },
      ...
      //    {  "USER",   "INDEXES",   "ON",    "MV",       0,      3,        0,      0,
      //    ComTdbExeUtilGetMetadataInfo::INDEXES_ON_MV_ },
      //    {  "USER",   "MVS",       "ON",    "TABLE",    0,      3,        0,      0,
      //    ComTdbExeUtilGetMetadataInfo::MVS_ON_TABLE_ },
      //    {  "USER",   "MVGROUPS",  "ON",    "TABLE",    0,      3,        0,      0,
      //    ComTdbExeUtilGetMetadataInfo::MVGROUPS_ON_TABLE_ },
      //    {  "USER",   "SYNONYMS",  "ON",    "TABLE",    0,      3,        0,      0,
      //    ComTdbExeUtilGetMetadataInfo::SYNONYMS_ON_TABLE_ },
      ...
      //    {  "ALL",    "INDEXES",   "ON",    "MV",       0,      3,        0,      0,
      //    ComTdbExeUtilGetMetadataInfo::INDEXES_ON_MV_ },
      //    {  "ALL",    "MVS",       "ON",    "TABLE",    0,      3,        1,      1,
      //    ComTdbExeUtilGetMetadataInfo::MVS_ON_TABLE_ },
      //    {  "ALL",    "MVGROUPS",  "ON",    "TABLE",    0,      3,        0,      0,
      //    ComTdbExeUtilGetMetadataInfo::MVGROUPS_ON_TABLE_ },
      //    {  "ALL",    "SYNONYMS",  "ON",    "TABLE",    0,      3,        0,      0,
      //    ComTdbExeUtilGetMetadataInfo::SYNONYMS_ON_TABLE_ },
      //
      //    {  "IUDLOG",    "TABLES",   "ON",  "TABLE",    0,      3,        0,      0,
      //    ComTdbExeUtilGetMetadataInfo::IUDLOG_TABLE_ON_TABLE_ },
      //    {  "RANGELOG",  "TABLES",   "ON",  "TABLE",    0,      3,        0,      0,
      //    ComTdbExeUtilGetMetadataInfo::RANGELOG_TABLE_ON_TABLE_ },
      //    {  "TRIGTEMP",  "TABLES",   "ON",  "TABLE",    0,      3,        0,      0,
      //    ComTdbExeUtilGetMetadataInfo::TRIGTEMP_TABLE_ON_TABLE_ },
      //    {  "IUDLOG",    "TABLES",   "ON",  "MV",       0,      3,        0,      0,
      //    ComTdbExeUtilGetMetadataInfo::IUDLOG_TABLE_ON_MV_ },
      ...
      //    {  "ALL",    "PRIVILEGES","ON",    "MV",       0,      3,        0,      0,
      //    ComTdbExeUtilGetMetadataInfo::PRIVILEGES_ON_MV_ },
      //    {  "USER",   "MVS",       "ON",    "MV",       0,      3,        0,      0,
      //    ComTdbExeUtilGetMetadataInfo::MVS_ON_MV_ },
      //    {  "ALL",    "MVS",       "ON",    "MV",       0,      3,        1,      1,
      //    ComTdbExeUtilGetMetadataInfo::MVS_ON_MV_ },

      //    {  "USER",   "PARTITIONS","FOR",   "TABLE",    0,      3,        0,      0,
      //    ComTdbExeUtilGetMetadataInfo::PARTITIONS_FOR_TABLE_ },
      //    {  "USER",   "PARTITIONS","ON",    "TABLE",    0,      3,        0,      0,
      //    ComTdbExeUtilGetMetadataInfo::PARTITIONS_FOR_TABLE_ },
      //    {  "USER",   "PARTITIONS","FOR",   "INDEX",    0,      3,        0,      0,
      //    ComTdbExeUtilGetMetadataInfo::PARTITIONS_FOR_INDEX_ },
      //    {  "USER",   "PARTITIONS","ON",    "INDEX",    0,      3,        0,      0,
      //    ComTdbExeUtilGetMetadataInfo::PARTITIONS_FOR_INDEX_ },
      ...
      //    {  "USER",   "TABLES",    "IN",    "MV",       0,      3,        0,      0,
      //    ComTdbExeUtilGetMetadataInfo::TABLES_IN_MV_ },
      //    {  "USER",   "MVS",       "IN",    "MV",       0,      3,        0,      0,
      //    ComTdbExeUtilGetMetadataInfo::MVS_IN_MV_ },
      //    {  "USER",   "OBJECTS",   "IN",    "MV",       0,      3,        0,      0,
      //    ComTdbExeUtilGetMetadataInfo::OBJECTS_IN_MV_ },

      //    {  "ALL",    "TABLES",    "IN",    "MV",       0,      3,        1,      1,
      //    ComTdbExeUtilGetMetadataInfo::TABLES_IN_MV_ },
      //    {  "ALL",    "MVS",       "IN",    "MV",       0,      3,        1,      1,
      //    ComTdbExeUtilGetMetadataInfo::MVS_IN_MV_ },
      //    {  "ALL",    "OBJECTS",   "IN",    "MV",       0,      3,        1,      1,
      //    ComTdbExeUtilGetMetadataInfo::OBJECTS_IN_MV_ },
      ...
      {"ALL", "TABLE_FUNCTIONS", "FOR", "LIBRARY", 0, 3, 0, 0,
       ComTdbExeUtilGetMetadataInfo::TABLE_FUNCTIONS_FOR_LIBRARY_},
      ...
      //    {  "ALL",    "MVS",       "FOR",   "USER",     0,      0,        0,      0,
      //    ComTdbExeUtilGetMetadataInfo::MVS_FOR_USER_ },
      //    {  "ALL",    "MVGROUPS",  "FOR",   "USER",     0,      0,        0,      0,
      //    ComTdbExeUtilGetMetadataInfo::MVGROUPS_FOR_USER_ },
      //    {  "ALL",    "SYNONYMS",  "FOR",   "USER",     0,      0,        0,      0,
      //    ComTdbExeUtilGetMetadataInfo::SYNONYMS_FOR_USER_ },
      //    {  "ALL",    "TRIGGERS",  "FOR",   "USER",     0,      0,        0,      0,
      //    ComTdbExeUtilGetMetadataInfo::TRIGGERS_FOR_USER_ },
      ...
      //  {  "SYSTEM", "MVS",       "FOR",   "USER",     0,      0,        0,      0,
      //  ComTdbExeUtilGetMetadataInfo::MVS_FOR_USER_ },
      //  {  "SYSTEM", "MVGROUPS",  "FOR",   "USER",     0,      0,        0,      0,
      //  ComTdbExeUtilGetMetadataInfo::MVGROUPS_FOR_USER_ },
      //  {  "SYSTEM", "SYNONYMS",  "FOR",   "USER",     0,      0,        0,      0,
      //  ComTdbExeUtilGetMetadataInfo::SYNONYMS_FOR_USER_ },
      //  {  "SYSTEM", "TRIGGERS",  "FOR",   "USER",     0,      0,        0,      0,
      //  ComTdbExeUtilGetMetadataInfo::TRIGGERS_FOR_USER_ },
      //  {  "SYSTEM", "VIEWS",     "FOR",   "USER",     0,      0,        0,      0,
      ...
      //    {  "USER",   "MVS",       "FOR",   "USER",     0,      0,        0,      0,
      //    ComTdbExeUtilGetMetadataInfo::MVS_FOR_USER_ },
      //    {  "USER",   "MVGROUPS",  "FOR",   "USER",     0,      0,        0,      0,
      //    ComTdbExeUtilGetMetadataInfo::MVGROUPS_FOR_USER_ },
      //    {  "USER",   "SYNONYMS",  "FOR",   "USER",     0,      0,        0,      0,
      //    ComTdbExeUtilGetMetadataInfo::SYNONYMS_FOR_USER_ },
      //    {  "USER",   "TRIGGERS",  "FOR",   "USER",     0,      0,        0,      0,
      //    ComTdbExeUtilGetMetadataInfo::TRIGGERS_FOR_USER_ },
  ...
  if ((queryType == ComTdbExeUtilGetMetadataInfo::NO_QUERY_) || (getVersion_ && (NOT getVersionSupported))) {
  ...
  if (objectType_ == "COMPONENT") objName = objectName_.getQualifiedNameObj().getObjectName();
  ...
  if ((catName.isNull()) &&
      (generator->currentCmpContext()->schemaDB_->getDefaultSchema().getCatalogName() == HIVE_SYSTEM_CATALOG) &&
    ...
    catName = generator->currentCmpContext()->schemaDB_->getDefaultSchema().getCatalogName();

    if (schName.isNull()) schName = generator->currentCmpContext()->schemaDB_->getDefaultSchema().getSchemaName();
  }

  if (((catName == hiveDefCatName) || (catName == HIVE_SYSTEM_CATALOG)) &&
  ...
  if (NOT hiveObjects()) {
    if (catName.isNull()) catName = generator->currentCmpContext()->schemaDB_->getDefaultSchema().getCatalogName();

    if (schName.isNull()) schName = generator->currentCmpContext()->schemaDB_->getDefaultSchema().getSchemaName();

    if (objName.isNull()) objName = "DUMMY__";
  }

  if (hiveObjects()) {
    if (CmpCommon::getDefault(MODE_SEAHIVE) == DF_OFF) {
      *CmpCommon::diags() << DgSqlCode(-4218) << DgString0("GET") << DgString1("Reason: CQD MODE_SEAHIVE is not set.");
  ...
  if (CmpSeabaseDDL::isSeabase(catName)) setHbaseObjects(TRUE);
  ...
  if (catName == HBASE_SYSTEM_CATALOG) {
    if (queryType == ComTdbExeUtilGetMetadataInfo::TABLES_IN_SCHEMA_) {
      *CmpCommon::diags() << DgSqlCode(-4218) << DgString0("GET")
                          << DgString1(
                                 " Reason: Cannot get native hbase tables using GET command. Use hbase shell interface "
  ...
  if ((infoType_ == "MVS") || (infoType_ == "MVGROUPS") || (infoType_ == "SYNONYMS") || (infoType_ == "PARTITIONS")) {
    NAString nas("GET ");
    nas += infoType_;
    *CmpCommon::diags() << DgSqlCode(-4222) << DgString0(nas);

    GenExit();
  }

  if ((maxParts > 0) && (NOT hiveObjects()) && (NOT hbaseObjects())) {
    CorrName cn(objName, generator->wHeap(), schName, catName);
    if (objectType_ == "INDEX") cn.setSpecialType(ExtendedQualName::INDEX_TABLE);
    if (objectType_ == "LIBRARY") cn.setSpecialType(ExtendedQualName::LIBRARY_TABLE);
    ...
    if ((!naTable) || (generator->getBindWA()->errStatus())) {
      ...
      if ((objectType_ == "CATALOG") &&
          ((CmpCommon::diags()->containsError(-1003)) || (CmpCommon::diags()->containsError(-4082)))) {
    ...
    if (((catName == hiveDefCatName) || (catName == HIVE_SYSTEM_CATALOG)) &&
        ((queryType == ComTdbExeUtilGetMetadataInfo::VIEWS_ON_TABLE_) &&
         (naTable && (NOT naTable->isRegistered()) && naTable->hasExternalTable()))) {
      // Convert the native name to its Trafodion form
      NAString tabName = ComConvertNativeNameToTrafName(catName, schName, objName);
    ...
    if (objectType_ == "TABLE") {
      if ((naTable->getViewFileName()) || (naTable->isAnMV())) {
        *CmpCommon::diags() << DgSqlCode(-4219);
        GenExit();
      }
  ...
  {
    gm_exe_util_tdb = new (space) ComTdbExeUtilGetMetadataInfo(
        queryType, cat, sch, obj, pattern, param1, scanExpr, workCriDesc, exe_util_row_atp_index,  // work cri desc
        givenDesc, returnedDesc, (queue_index)8, (queue_index)128, 2, 32000, server, zkPort);

    if (hbaseObjects()) {
      gm_exe_util_tdb->setIsHbase(TRUE);
    }

    if (withNamespace_) gm_exe_util_tdb->setWithNamespace(TRUE);
  ...
  if ((queryType == ComTdbExeUtilGetMetadataInfo::PARTITIONS_FOR_TABLE_) ||
    ...
    if (objectType_ == "INDEX") gm_exe_util_tdb->setIsIndex(TRUE);
  }

  if (!Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL)) {
    if (groupBy) gm_exe_util_tdb->setGroupBy(TRUE);

    if (orderBy) gm_exe_util_tdb->setOrderBy(TRUE);
  }
  generator->initTdbFields(exe_util_tdb);
...
static short addReferencingMVs(Generator *generator, char *mvName, Queue *refreshMvsList, Queue *reorgMvsList) {
  short retcode = 0;
  ...
  NATable *mvNATable = generator->getBindWA()->getNATable(cn);
  ...
  if (NOT mvNATable->getMvsUsingMe().isEmpty()) {
    ...
    for (CollIndex i = 0; i < mvMVList.entries(); i++) {
      char *mvMVname =
          space->AllocateAndCopyToAlignedSpace(generator->genGetNameAsAnsiNAString(mvMVList[i]->getMvName()), 0);

      if (refreshMvsList) refreshMvsList->insert(mvMVname);

      if (reorgMvsList) reorgMvsList->insert(mvMVname);

      // now add all mvs referencing mvMVname
      retcode = addReferencingMVs(generator, mvMVname, refreshMvsList, reorgMvsList);
  ...
  */
  ...
  if (getUtilTableDesc() && getUtilTableDesc()->getNATable()->isVolatileTable()) {
    ...
    */
    ...
    QualifiedName q("X", getTableName().getQualifiedNameObj().getSchemaName(),
  ...
  if (getUtilTableDesc() && getUtilTableDesc()->getNATable()->getParentTableName()) {
    parentTableName =
        new (generator->getSpace()) char[strlen(getUtilTableDesc()->getNATable()->getParentTableName()) + 1];
    strcpy(parentTableName, getUtilTableDesc()->getNATable()->getParentTableName());
  }

  if (!parentTableName && getParentTableNameLen() > 0) {
    parentTableName = new (generator->getSpace()) char[getParentTableNameLen() + 1];
  ...
  switch (type_) {
    case TABLE_:
      ot = ComTdbExeUtilMaintainObject::TABLE_;
      break;
    case INDEX_:
      ot = ComTdbExeUtilMaintainObject::INDEX_;
      break;
    case MV_:
      ot = ComTdbExeUtilMaintainObject::MV_;
      break;
    case MV_INDEX_:
      ot = ComTdbExeUtilMaintainObject::MV_INDEX_;
      break;
    case MV_LOG_:
      ot = ComTdbExeUtilMaintainObject::MV_LOG_;
      break;
    case CATALOG_:
      ot = ComTdbExeUtilMaintainObject::CATALOG_;
      break;
    case SCHEMA_:
      ot = ComTdbExeUtilMaintainObject::SCHEMA_;
      break;
    case CLEAN_MAINTAIN_:
      ot = ComTdbExeUtilMaintainObject::CLEAN_MAINTAIN_;
      break;
    default:
      ot = ComTdbExeUtilMaintainObject::NOOP_;
      break;
  }

  ComTdbExeUtilMaintainObject *exe_util_tdb = new (space)
      ComTdbExeUtilMaintainObject(tablename, strlen(tablename), schemaName, strlen(schemaName), ot, parentTableName,
                                  (parentTableName ? strlen(parentTableName) : 0), 0, 0,  // no input expr
                                  0, 0,                                                   // no output expr
                                  0, 0,                                                   // no work cri desc
                                  givenDesc, returnedDesc, (queue_index)8, (queue_index)64, 2, 10240);
  ...
  if (parentTableName && (getParentTableObjectUID() == 0)) {
    ...
    NATable *naParentTable = generator->getBindWA()->getNATable(cn);
  ...
  char *tempName = NULL;
  NAString maintainCatName = ActiveSchemaDB()->getDefaults().getValue(MAINTAIN_CATALOG);

  if (maintainCatName.isNull()) maintainCatName = "NEO";
  ...
  if (reorgIndex_ || getIndexLabelStats_) {
    ...
    char *indexName = NULL;
    if (type_ == INDEX_) {
      indexName = space->AllocateAndCopyToAlignedSpace(generator->genGetNameAsAnsiNAString(getTableName()), 0);
      ...
      for (CollIndex i = 0; i < indList.entries(); i++) {
        ...
        indexName = space->AllocateAndCopyToAlignedSpace(index->getExtIndexName(), 0);
  ...
  if ((type_ == TABLE_) && ((refreshMvs_) || (reorgMvs_) || (updStatsMvs_) || (reorgMvsIndex_))) {
    // create a list of all MVs referencing, directly or indirectly,
    // the given tablename.
    allUsingMvsList = new (generator->wHeap()) Queue(generator->wHeap());
    const UsingMvInfoList &usingMvList = getUtilTableDesc()->getNATable()->getMvsUsingMe();
    for (CollIndex i = 0; i < usingMvList.entries(); i++) {
      char *mvName =
          space->AllocateAndCopyToAlignedSpace(generator->genGetNameAsAnsiNAString(usingMvList[i]->getMvName()), 0);

      allUsingMvsList->insert(mvName);

      if (addReferencingMVs(generator, mvName, allUsingMvsList, NULL)) return -1;
    }
  }

  if ((refreshMvs_) || (reorgMvs_)) {
    if (refreshMvs_) refreshMvsList = new (space) Queue(space);

    if (reorgMvs_) reorgMvsList = new (space) Queue(space);

    char *mvName = NULL;
    if (type_ == MV_) {
      mvName = space->AllocateAndCopyToAlignedSpace(generator->genGetNameAsAnsiNAString(getTableName()), 0);

      if (refreshMvs_) refreshMvsList->insert(mvName);

      if (reorgMvs_) reorgMvsList->insert(mvName);
  ...
  if (refreshMvgroup_) {
    ...
    // the mvlist.
    char *mvName = NULL;
    if ((type_ == MVGROUP_) || ((type_ == TABLE_) && (refreshMvsList && (NOT refreshMvsList->isEmpty())))) {
  ...
  if (reorgMvgroup_) {
    ...
    // the mvlist.
    char *mvName = NULL;
    if ((type_ == MVGROUP_) || ((type_ == TABLE_) && (reorgMvsList && (NOT reorgMvsList->isEmpty())))) {
  ...
  if (updStatsMvs_) {
    if (updStatsMvs_) updStatsMvsList = new (space) Queue(space);

    char *mvName = NULL;
    if (type_ == MV_) {
      mvName = space->AllocateAndCopyToAlignedSpace(generator->genGetNameAsAnsiNAString(getTableName()), 0);

      if (updStatsMvs_) updStatsMvsList->insert(mvName);
  ...
  if (updStatsMvgroup_) {
    ...
    // the mvlist.
    char *mvName = NULL;
    if ((type_ == MVGROUP_) || ((type_ == TABLE_) && (updStatsMvsList && (NOT updStatsMvsList->isEmpty())))) {
  ...
  if (reorgMvsIndex_) {
    ...
    char *mvName = space->AllocateAndCopyToAlignedSpace(generator->genGetNameAsAnsiNAString(getTableName()), 0);
    if (type_ == MV_) {
      allUsingMvsList = new (generator->wHeap()) Queue(generator->wHeap());
      allUsingMvsList->insert(mvName);
    }
    ...
    if (type_ == MV_INDEX_) {
      if (reorgMvsIndex_) reorgMvsIndexList->insert(mvName);
    } else {
      ...
      while (allUsingMvsList->atEnd() == 0) {
        ...
        TableDesc *mvTableDesc = generator->getBindWA()->createTableDesc(mvNATable, cn);
        if (generator->getBindWA()->errStatus()) {
          GenExit();
          // GenAssert(! generator->getBindWA()->errStatus(),
        ...
        const LIST(IndexDesc *) indexList = mvTableDesc->getIndexes();
        for (CollIndex i = 0; i < indexList.entries(); i++) {
          ...
          char *indexName = space->AllocateAndCopyToAlignedSpace(index->getExtIndexName(), 0);
  ...
  if ((type_ == TABLE_) && (updStatsMvlog_) && (NULL != getMvLogTable())) {
    updStatsMvlogCols = " ON EVERY KEY, ";

    NABoolean firstColSeen = FALSE;
    const NAColumnArray &naColArr = getMvLogTable()->getNAColumnArray();
    for (CollIndex i = 0; i < naColArr.entries(); i++) {
      NAColumn *naCol = naColArr[i];
      if ((naCol->isSystemColumn()) || (naCol->isMvSystemAddedColumn())) continue;

      if (firstColSeen) updStatsMvlogCols += ", ";
  ...
  if (type_ == TABLES_) {
    for (CollIndex i = 0; i < multiTablesNames_.entries(); i++) {
      char *tableName =
          space->AllocateAndCopyToAlignedSpace(generator->genGetNameAsAnsiNAString(*(multiTablesNames_[i])), 0);
    ...
    for (CollIndex i = 0; i < skippedMultiTablesNames_.entries(); i++) {
      char *tableName =
          space->AllocateAndCopyToAlignedSpace(generator->genGetNameAsAnsiNAString(*(skippedMultiTablesNames_[i])), 0);
  ...
  if (NOT reorgTableOptions_.isNull()) {
    reorgTableOptions = space->allocateAlignedSpace(strlen(reorgTableOptions_) + 1);
    strcpy(reorgTableOptions, reorgTableOptions_);
  }
  if (NOT reorgIndexOptions_.isNull()) {
    reorgIndexOptions = space->allocateAlignedSpace(strlen(reorgIndexOptions_) + 1);
    strcpy(reorgIndexOptions, reorgIndexOptions_);
  }
  if (NOT updStatsTableOptions_.isNull()) {
    updStatsTableOptions = space->allocateAlignedSpace(strlen(updStatsTableOptions_) + 1);
    strcpy(updStatsTableOptions, updStatsTableOptions_);
  }
  if ((NOT updStatsMvlogOptions_.isNull()) || (NOT updStatsMvlogCols.isNull())) {
    if (NOT updStatsMvlogCols.isNull())
      updStatsMvlogOptions = space->allocateAlignedSpace(strlen(updStatsMvlogCols) + 1);
    else
      updStatsMvlogOptions = space->allocateAlignedSpace(strlen(updStatsMvlogOptions_) + 1);
    if (NOT updStatsMvlogCols.isNull())
  ...
  if (NOT updStatsMvsOptions_.isNull()) {
    updStatsMvsOptions = space->allocateAlignedSpace(strlen(updStatsMvsOptions_) + 1);
    strcpy(updStatsMvsOptions, updStatsMvsOptions_);
  }

  if (NOT updStatsMvgroupOptions_.isNull()) {
    updStatsMvgroupOptions = space->allocateAlignedSpace(strlen(updStatsMvgroupOptions_) + 1);
    strcpy(updStatsMvgroupOptions, updStatsMvgroupOptions_);
  }

  if (NOT refreshMvgroupOptions_.isNull()) {
    refreshMvgroupOptions = space->allocateAlignedSpace(strlen(refreshMvgroupOptions_) + 1);
    strcpy(refreshMvgroupOptions, refreshMvgroupOptions_);
  }
  if (NOT refreshMvsOptions_.isNull()) {
    refreshMvsOptions = space->allocateAlignedSpace(strlen(refreshMvsOptions_) + 1);
    strcpy(refreshMvsOptions, refreshMvsOptions_);
  }
  if (NOT reorgMvgroupOptions_.isNull()) {
    reorgMvgroupOptions = space->allocateAlignedSpace(strlen(reorgMvgroupOptions_) + 1);
    strcpy(reorgMvgroupOptions, reorgMvgroupOptions_);
  }
  if (NOT reorgMvsOptions_.isNull()) {
    reorgMvsOptions = space->allocateAlignedSpace(strlen(reorgMvsOptions_) + 1);
    strcpy(reorgMvsOptions, reorgMvsOptions_);
  }
  if (NOT reorgMvsIndexOptions_.isNull()) {
    reorgMvsIndexOptions = space->allocateAlignedSpace(strlen(reorgMvsIndexOptions_) + 1);
    strcpy(reorgMvsIndexOptions, reorgMvsIndexOptions_);
  }

  if (NOT cleanMaintainCITOptions_.isNull()) {
    cleanMaintainCITOptions = space->allocateAlignedSpace(strlen(cleanMaintainCITOptions_) + 1);
    strcpy(cleanMaintainCITOptions, cleanMaintainCITOptions_);
  }

  if (cleanMaintainCIT_) exe_util_tdb->setCleanMaintainCIT(TRUE);

  exe_util_tdb->setParams(
      reorgTable_, (reorgIndex_ && indexList && (NOT indexList->isEmpty())), updStatsTable_,
      (((type_ == TABLE_) && (updStatsMvlog_) && (NULL != getMvLogTable())) ? TRUE : FALSE),
      (updStatsMvsList && (NOT updStatsMvsList->isEmpty())),
      (updStatsMvgroupList && (NOT updStatsMvgroupList->isEmpty())),
      (refreshMvgroupList && (NOT refreshMvgroupList->isEmpty())), (refreshMvsList && (NOT refreshMvsList->isEmpty())),
      (reorgMvgroupList && (NOT reorgMvgroupList->isEmpty())), (reorgMvsList && (NOT reorgMvsList->isEmpty())),
      (reorgMvsIndexList && (NOT reorgMvsIndexList->isEmpty())), continueOnError_, cleanMaintainCIT_,
      getSchemaLabelStats_, getLabelStats_, getTableLabelStats_,
      getIndexLabelStats_ && indexList && (NOT indexList->isEmpty()), getLabelStatsIncIndexes_,
      getLabelStatsIncInternal_, getLabelStatsIncRelated_

  );

  if (display_) exe_util_tdb->setDisplay(TRUE);
  if (displayDetail_) exe_util_tdb->setDisplayDetail(TRUE);
  if (doTheSpecifiedTask_) exe_util_tdb->setDoSpecifiedTask(TRUE);
  if (getStatus_) exe_util_tdb->setGetStatus(TRUE);
  if (getDetails_) exe_util_tdb->setGetDetails(TRUE);
  if (noOutput_) exe_util_tdb->setNoOutput(TRUE);

  if (getUtilTableDesc() && getUtilTableDesc()->getNATable()->isVolatileTable())
    exe_util_tdb->setNoControlInfoUpdate(TRUE);

  if (CmpCommon::getDefault(USE_MAINTAIN_CONTROL_TABLE) == DF_OFF) exe_util_tdb->setNoControlInfoTable(TRUE);
  ...
  if (initialize_) exe_util_tdb->setInitializeMaintain(TRUE);
  if (reinitialize_) exe_util_tdb->setReInitializeMaintain(TRUE);
  if (drop_) exe_util_tdb->setDropMaintain(TRUE);
  if (dropView_) exe_util_tdb->setDropView(TRUE);
  if (createView_) exe_util_tdb->setCreateView(TRUE);

  if (all_) exe_util_tdb->setAllSpecified(TRUE);
  ...
  if (getTableLabelStats_) exe_util_tdb->setTableLabelStats(TRUE);
  if (getIndexLabelStats_) exe_util_tdb->setIndexLabelStats(TRUE);
  ...
  if (getSchemaLabelStats_) exe_util_tdb->setSchemaLabelStats(TRUE);

  if ((refreshMvgroup_) && (refreshMvs_) && (type_ == TABLE_)) {
    // If individual MV's are to be refresh, then skip the group refresh
    exe_util_tdb->setSkipRefreshMvs(TRUE);
  }

  if ((reorgMvgroup_) && (reorgMvs_) && (type_ == TABLE_)) {
  ...
  if ((updStatsMvgroup_) && (updStatsMvs_) && (type_ == TABLE_)) {
    // for QCD4, assume that mvgroup name is the same as tablename.
  ...
  exe_util_tdb->setLists(indexList, refreshMvgroupList, refreshMvsList, reorgMvgroupList, reorgMvsList,
                         reorgMvsIndexList, updStatsMvgroupList, updStatsMvsList, multiTablesNamesList,
                         skippedMultiTablesNamesList);
  exe_util_tdb->setMultiTablesCreateTimeList(multiTablesCreateTimeList);

  exe_util_tdb->setOptionsParams(reorgTableOptions, reorgIndexOptions, updStatsTableOptions, updStatsMvlogOptions,
                                 updStatsMvsOptions, updStatsMvgroupOptions, refreshMvgroupOptions, refreshMvsOptions,
                                 reorgMvgroupOptions, reorgMvsOptions, reorgMvsIndexOptions, cleanMaintainCITOptions);
...
TrafDesc *ExeUtilRegionStats::createVirtualTableDesc() {
  ...
  if (displayFormat_)
    ...
    table_desc = Generator::createVirtualTableDesc(getVirtualTableName(),
                                                   NULL,  // let it decide what heap to use
                                                   rs.getVirtTableNumCols(), rs.getVirtTableColumnInfo(),
                                                   rs.getVirtTableNumKeys(), rs.getVirtTableKeyInfo());
...
short ExeUtilRegionStats::codeGen(Generator *generator) {
  ...
  short rc =
      processOutputRow(generator, work_atp, exe_util_row_atp_index, returnedDesc, (NOT selectionPred().isEmpty()));
  if (rc) {
    return -1;
  }
  ...
  if (inputColList_) {
    ...
    ItemExpr *inputExpr = new (generator->wHeap())
        Cast(inputColList_, new (generator->wHeap())
    ...
    expGen->processValIdList(inputVIDList, ExpTupleDesc::SQLARK_EXPLODED_FORMAT, inputRowLen, work_atp,
                             exe_util_row_atp_index);

    expGen->generateContiguousMoveExpr(inputVIDList,
                                       0,  // don't add conv nodes
                                       work_atp, exe_util_row_atp_index, ExpTupleDesc::SQLARK_EXPLODED_FORMAT,
                                       inputRowLen, &input_expr);
  }
  ...
  char *tableName = NULL;
  const NATable *naTable = (getUtilTableDesc() ? getUtilTableDesc()->getNATable() : NULL);
  if (naTable && (naTable->isHbaseRowTable() || naTable->isHbaseCellTable())) {
    tableName = space->AllocateAndCopyToAlignedSpace(GenGetQualifiedName(getTableName(), FALSE, TRUE), 0);
  } else {
    if (FileScan::genTableName(generator, space, getTableName(),
                               (getUtilTableDesc() ? getUtilTableDesc()->getNATable() : NULL), NULL,
                               TRUE,  // quoted format
                               tableName)) {
  ...
  char *catName =
      space->allocateAndCopyToAlignedSpace(getTableName().getQualifiedNameObj().getCatalogName().data(),
                                           getTableName().getQualifiedNameObj().getCatalogName().length(), 0);
  char *schName =
      space->allocateAndCopyToAlignedSpace(getTableName().getQualifiedNameObj().getSchemaName().data(),
                                           getTableName().getQualifiedNameObj().getSchemaName().length(), 0);
  char *objName =
      space->allocateAndCopyToAlignedSpace(getTableName().getQualifiedNameObj().getObjectName().data(),
                                           getTableName().getQualifiedNameObj().getObjectName().length(), 0);

  ComTdbExeUtilRegionStats *exe_util_tdb = new (space) ComTdbExeUtilRegionStats(
      tableName, catName, schName, objName, input_expr, inputRowLen, scanExpr, workCriDesc, exe_util_row_atp_index,
      givenDesc, returnedDesc, (queue_index)64, (queue_index)64, 4, 64000);
  generator->initTdbFields(exe_util_tdb);

  if (USE_UUID_AS_HBASE_TABLENAME && ComIsUserTable(getTableName())) {
    char *tablenameForUID = NULL;
    if (FileScan::genTableName(generator, space, getTableName(),
                               (getUtilTableDesc() ? getUtilTableDesc()->getNATable() : NULL), NULL, FALSE,
                               tablenameForUID, TRUE)) {
      GenAssert(0, "genTableName failed");
    }
...
short ExeUtilLongRunning::codeGen(Generator *generator) {
  ...
  char *tablename = tablename = space->AllocateAndCopyToAlignedSpace(GenGetQualifiedName(getTableName()), 0);

  ex_cri_desc *givenDesc = generator->getCriDesc(Generator::DOWN);

  ex_cri_desc *returnedDesc = new (space) ex_cri_desc(givenDesc->noTuples(), space);

  ComTdbExeUtilLongRunning *exe_util_tdb = new (space) ComTdbExeUtilLongRunning(
      tablename, strlen(tablename),
  ...
  if (type_ == ExeUtilLongRunning::LR_DELETE) {
    ...
    // datamember lruStmt_
    NAString lruStmtString = constructLRUDeleteStatement(FALSE);  // withCK
    ...
    NAString lruStmtWithCKString = constructLRUDeleteStatement(TRUE);  // withCK

    lruStmtWithCK_ = space->allocateAlignedSpace(lruStmtWithCKString.length() + 1);
    ...
    char *pDefaultSch = space->allocateAlignedSpace(defaultSch.length() + 1);
    ...
    char *pDefaultCat = space->allocateAlignedSpace(defaultCat.length() + 1);
    ...
    if (getTableName().getSpecialType() == ExtendedQualName::IUD_LOG_TABLE) exe_util_tdb->setUseParserflags(TRUE);
  ...
  if (CmpCommon::getDefault(COMP_BOOL_190) == DF_ON) exe_util_tdb->setLongRunningQueryPlan(TRUE);
...
short ExeUtilShowSet::codeGen(Generator *generator) {
  ...
  ComTdbExeUtilShowSet *exe_util_tdb =
      new (space) ComTdbExeUtilShowSet(type, param1, param2, 0, 0,  // no work cri desc
                                       givenDesc, returnedDesc, (queue_index)8, (queue_index)64, 2,
                                       32000);  // getDefault(GEN_DDL_BUFFER_SIZE));
  generator->initTdbFields(exe_util_tdb);
...
short ExeUtilAQR::codeGen(Generator *generator) {
  ...
  short rc = processOutputRow(generator, 0, returnedDesc->noTuples() - 1, returnedDesc);
  if (rc) {
    return -1;
  }
...
short ExeUtilHbaseDDL::codeGen(Generator *generator) {
  ...
  ex_cri_desc *returnedDesc = generator->getCriDesc(Generator::DOWN);
  ...
  ComTdbHbaseAccess *ddl_tdb = new (space) ComTdbHbaseAccess(
      ...
      tablename, baseTableName, 0, colFamNameList, NULL, givenDesc, returnedDesc, downqueuelength, upqueuelength,
      0,  // Cardinality expectedRows,
      numBuffers, buffersize, server, zkPort);
...
TrafDesc *ExeUtilConnectby::createVirtualTableDesc() {
  if (isDual() == TRUE) {
    ...
    TrafDesc *column_desc = TrafMakeColumnDesc(table_desc->tableDesc()->tablename, "LEVEL",
                                               colnumber,  // INOUT
                                               REC_BIN32_UNSIGNED, 4, offset, FALSE, SQLCHARSETCODE_UNKNOWN, NULL);
    ...
    TrafDesc *col_desc = TrafMakeColumnDesc(table_desc->tableDesc()->tablename, "ROWNUM",
                                            colnumber,  // INOUT
                                            REC_BIN32_UNSIGNED, 4, tmpOffset, FALSE, SQLCHARSETCODE_UNKNOWN, NULL);
  ...
  int i = 0;
  int tmpOffset = 0;
  while (column_desc->next) {
    i++;
    column_desc = column_desc->next;
  }
  tmpOffset = column_desc->columnsDesc()->offset + 4;
  TrafDesc *col_desc = TrafMakeColumnDesc(getVirtualTableName(),
                                          "LEVEL",  // info->colName,
                                          i, REC_BIN32_UNSIGNED, 4, tmpOffset, FALSE, SQLCHARSETCODE_UNKNOWN, NULL);
  ...
  tmpOffset = column_desc->columnsDesc()->offset + 4;
  col_desc = TrafMakeColumnDesc(getVirtualTableName(),
  ...
  col_desc = TrafMakeColumnDesc(getVirtualTableName(),
                                ...
                                i, REC_BYTE_V_ASCII, 3000, tmpOffset, FALSE, SQLCHARSETCODE_UTF8, NULL);
  column_desc->next = col_desc;
  col_desc->columnsDesc()->colclass = 'S';
...
short ExeUtilConnectby::codeGen(Generator *generator) {
  ...
  Attributes **attrs = new (generator->wHeap()) Attributes *[getVirtualTableDesc()->getColumnList().entries()];
  ...
  expGen->processAttributes(userColList.entries(), attrs, tupleFormat, tupleLength, work_atp, theAtpIndex,
  ...
  if (hasDynParamInStart()) {
    ...
    for (ValueId dp = dynParmas_.init(); dynParmas_.next(dp); dynParmas_.advance(dp)) {
    ...
    expGen->generateContiguousMoveExpr(vidL,
                                       ...
                                       work_atp, theAtpIndexForDynParam, ExpTupleDesc::SQLARK_EXPLODED_FORMAT,
                                       dtupleLength, &input_expr);
    expGen->processValIdList(vidL,                                  // [IN] ValueIdList
  ...
  if (!mypredicates_.isEmpty()) {
    ItemExpr *selPredTree = mypredicates_.rebuildExprTree(ITM_AND, TRUE, TRUE);
  ...
  ComTdbExeUtilConnectby *exe_util_tdb = new (space) ComTdbExeUtilConnectby(
      stmtText, (stmtText ? strlen(stmtText) : 0), getStmtTextCharSet(), tblnm, tbl.length(), NULL, input_expr,
      dtupleLength, 0, 0, selectPred, workCriDesc, theAtpIndex, colDescSize, tupleLength, givenDesc, returnedDesc,
      (queue_index)2048, (queue_index)2048, 30, 320000, workCriDesc, NULL);

  exe_util_tdb->sourceDataTuppIndex_ = theAtpIndex;
  exe_util_tdb->dynParamTuppIndex = theAtpIndexForDynParam;
  exe_util_tdb->parentColName_ =
      parentColName_;                           // ((BiConnectBy*)getBiConnectBy())->getConnectBy()->getParentColName();
...
//
/////////////////////////////////////////////////////////
const char *ExeUtilCompositeUnnest::getVirtualTableName() { return virtTableName_.data(); }
...
short ExeUtilCompositeUnnest::codeGen(Generator *generator) {
  ...
  expGen->generateContiguousMoveExpr(extractColVIDList,
                                     1,  // add conv node
                                     work_atp, extractColAtpIndex, ExpTupleDesc::SQLARK_EXPLODED_FORMAT,
                                     extractColRowLen, &extractColExpr, &extractColTupleDesc,
  ...
  ItemExpr *extractColIE = new (generator->wHeap()) NATypeToItem((NAType *)&colNameExpr_->getValueId().getType());
  ...
  NAType *elemNumType = new (generator->wHeap()) SQLInt(generator->wHeap(), FALSE, FALSE);
  ...
  ItemExpr *compArrayElem = new (generator->wHeap()) CompositeExtract(extractColIE, elemNumIE);
  ...
  if (showPos_) {
    ...
    ItemExpr *cast = new (generator->wHeap()) Cast(ie, &elemNumIE->getValueId().getType());
  ...
  if (compArrayElem->getValueId().getType().isComposite()) {
    for (int elem = 1; elem <= ((CompositeType &)compArrayElem->getValueId().getType()).getNumElements(); elem++) {
      ...
      ItemExpr *cast = new (generator->wHeap()) Cast(elemIE, &elemIE->getValueId().getType());
  ...
  expGen->generateContiguousMoveExpr(unnestColsVIDList,
                                     ...
                                     work_atp, unnestColsAtpIndex, ExpTupleDesc::SQLARK_EXPLODED_FORMAT,
                                     unnestColsRowLen, &unnestColsExpr, &unnestColsTupleDesc,
                                     ExpTupleDesc::LONG_FORMAT);
  ...
  ComTdbExeUtilCompositeUnnest *exe_util_tdb = new (space) ComTdbExeUtilCompositeUnnest(
      scanExpr, extractColExpr, extractColAtpIndex, extractColRowLen, elemNumAtpIndex, elemNumLen, unnestColsExpr,
      unnestColsAtpIndex, unnestColsRowLen, workCriDesc, workAtpIndex, givenDesc, returnedDesc, (queue_index)8,
      (queue_index)1024, 10, 100000);
...
short ExeUtilHBaseBulkLoad::codeGen(Generator *generator) {
  ...
  if (inputParams().entries() > 0) {
    ...
    expGen->generateContiguousMoveExpr(inputParams(),
                                       1,  // add conv nodes
                                       work_atp, work_row_atp_index, ExpTupleDesc::SQLARK_EXPLODED_FORMAT, inputRowLen,
                                       &input_expr, &inputParamsTupleDesc, ExpTupleDesc::LONG_FORMAT);
  ...
  ComTdbExeUtilHBaseBulkLoad *exe_util_tdb = new (space) ComTdbExeUtilHBaseBulkLoad(
      tablename, strlen(tablename), ldQuery, input_expr, inputRowLen, workCriDesc, work_row_atp_index, givenDesc,
      returnedDesc, (queue_index)getDefault(GEN_DDL_SIZE_DOWN), (queue_index)getDefault(GEN_DDL_SIZE_UP),
      getDefault(GEN_DDL_NUM_BUFFERS),
      1024,  // getDefault(GEN_DDL_BUFFER_SIZE));
      errCountTab, logLocation);
...
short ExeUtilHBaseBulkLoadTask::codeGen(Generator *generator) {
  ...
  char *tablename = NULL;
  if (FileScan::genTableName(generator, space, getTableName(), getUtilTableDesc()->getNATable(), NULL, FALSE,
                             tablename)) {
    GenAssert(0, "genTableName failed");
  }

  char *baseTableName = NULL;
  if (FileScan::genTableName(generator, space, getUtilTableDesc()->getCorrNameObj(), getUtilTableDesc()->getNATable(),
                             NULL, FALSE, baseTableName)) {
    GenAssert(0, "genTableName failed");
  }
  ...
  char *indexNameForUID = NULL;
  {
    ...
    for (CollIndex i = 0; i < indList.entries(); i++) {
      IndexDesc *index = indList[i];
      if (FileScan::genTableName(generator, space, index->getIndexName(), getUtilTableDesc()->getNATable(), NULL, FALSE,
                                 indexName)) {
        GenAssert(0, "genTableName failed");
      }
      indexList->insert(indexName);
      ...
      if (USE_UUID_AS_HBASE_TABLENAME && ComIsUserTable(index->getIndexName())) {
        char *tablenameForUID = NULL;
        if (FileScan::genTableName(generator, space, index->getIndexName(), getUtilTableDesc()->getNATable(), NULL,
  ...
  ComTdbHbaseAccess *load_tdb = new (space) ComTdbHbaseAccess(
      ComTdbHbaseAccess::BULK_LOAD_TASK_, tablename, baseTableName, 0, 0, NULL, givenDesc, returnedDesc,
      downqueuelength, upqueuelength, expectedRows, numBuffers, buffersize, server, zkPort);
  ...
  if (!generator->explainDisabled()) {
    generator->setExplainTuple(addExplainInfo(load_tdb, 0, 0, generator));
  }
...
short ExeUtilSnapShotUpdataDelete::codeGen(Generator *generator) {
  ...
  ComTdbExeUtilUpdataDelete *exe_util_tdb = new (space) ComTdbExeUtilUpdataDelete(
      tablename, strlen(tablename), uldQuery, 0, 0,  // no work cri desc
      givenDesc, returnedDesc, (queue_index)getDefault(GEN_DDL_SIZE_DOWN), (queue_index)getDefault(GEN_DDL_SIZE_UP),
      getDefault(GEN_DDL_NUM_BUFFERS), getDefault(GEN_DDL_BUFFER_SIZE), storageType, server, zkPort);

  generator->initTdbFields(exe_util_tdb);

  if (incrBackupEnabled_) exe_util_tdb->setIncrBackupEnabled(TRUE);
