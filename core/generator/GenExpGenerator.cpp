...
ExpGenerator::ExpGenerator(Generator *generator_) {
  ...
  switch (pcodeOptLevel) {
    ...
    case DF_MINIMUM:
      ...
      pCodeMode_ = ex_expr::PCODE_ON | ex_expr::PCODE_EVAL;
      break;
  ...
  if (pcodeEnv == 0) pCodeMode_ = ex_expr::PCODE_NONE;
...
ExpGenerator::~ExpGenerator() { delete cache_; }
...
//////////////////////////////////////////////////////////
ItemExpr *ExpGenerator::getMinMaxValue(const NAType &type, short min_max /*0=min, -1=max, -2=max-without-null*/) {
  NABoolean includeNulls = (min_max > -2);
  NABoolean getMin = (min_max == 0);

  return new (CmpCommon::statementHeap()) ConstValue(&type, getMin, includeNulls);
}
...
// from an NAType.
/////////////////////////////////////////////////////////////////
Attributes *ExpGenerator::convertNATypeToAttributes(const NAType &naType_x, CollHeap *wHeap) {
  // ----------------------------------------------------------------
  ...
  if (naType->isComposite()) {
    const CompositeType *ct = (const CompositeType *)naType;
    CompositeAttributes *compAttr = new (wHeap) CompositeAttributes(ct->getNominalSize());
    compAttr->setDatatype(ct->getFSDatatype());
    compAttr->setVCIndicatorLength(ct->getVarLenHdrSize());
    compAttr->setNullFlag(ct->supportsSQLnull());
    compAttr->setNumElements(ct->getNumElements());
    compAttr->setCompFormat(ct->getCompFormat());

    result = compAttr;
  } else if (naType->isSimpleType()) {
    SimpleType *attr;
    ...
    if (naType->getTypeQualifier() == NA_CHARACTER_TYPE) {
      ...
      if (charType->isUpshifted()) attr->setUpshift(1);

      if (charType->isCaseinsensitive()) attr->setCaseinsensitive(1);

      if (charType->isSpaceSensitive()) attr->setSpaceSensitive();

      CharInfo::CharSet cs = charType->getCharSet();
      if (cs == CharInfo::UCS2 || cs == CharInfo::KANJI_MP || cs == CharInfo::KSC5601_MP) attr->setWidechar(1);
      ...
      attr->setCharSet(lobType->getCharSet());  // Handle charset
    }
    ...
    if (naType->getSimpleTypeName() == "BIG NUM") {
      ...
      BigNum *attr = new (wHeap) BigNum(numericType->getNominalSize(), numericType->getPrecision(),
                                        (short)numericType->getScale(), numericType->isUnsigned());
...
Attributes::DefaultClass ExpGenerator::getDefaultClass(const NAColumn *col) {
  ...
  switch (col->getDefaultClass()) {
    case COM_CURRENT_DEFAULT:
      dc = Attributes::DEFAULT_CURRENT;
      break;
    case COM_CURRENT_UT_DEFAULT:
      dc = Attributes::DEFAULT_CURRENT_UT;
      break;
    case COM_UUID_DEFAULT:
      dc = Attributes::DEFAULT_UUID;
      break;
    case COM_SYS_GUID_FUNCTION_DEFAULT:
      dc = Attributes::DEFAULT_SYS_GUID_FUNCTION;
      break;
    case COM_FUNCTION_DEFINED_DEFAULT:
      dc = Attributes::DEFAULT_FUNCTION;
      break;
    case COM_NO_DEFAULT:
    case COM_ALWAYS_COMPUTE_COMPUTED_COLUMN_DEFAULT:
    case COM_ALWAYS_DEFAULT_COMPUTED_COLUMN_DEFAULT:
      dc = Attributes::NO_DEFAULT;
      break;
    case COM_NULL_DEFAULT:
      dc = Attributes::DEFAULT_NULL;
      break;
    case COM_USER_DEFINED_DEFAULT:
      dc = Attributes::DEFAULT_USER;
      break;
    case COM_USER_FUNCTION_DEFAULT:
      dc = Attributes::DEFAULT_USER_FUNCTION;
      break;
    case COM_IDENTITY_GENERATED_BY_DEFAULT:
    case COM_IDENTITY_GENERATED_ALWAYS:
      dc = Attributes::DEFAULT_IDENTITY;
      break;
    case COM_CATALOG_FUNCTION_DEFAULT:
      dc = Attributes::DEFAULT_CATALOG_FUNCTION;
      break;
    case COM_SCHEMA_FUNCTION_DEFAULT:
      dc = Attributes::DEFAULT_SCHEMA_FUNCTION;
      break;
    default:
      dc = Attributes::INVALID_DEFAULT;
      break;
  }
...
short ExpGenerator::addDefaultValue(NAColumn *col, Attributes *attr, ComDiagsArea *diagsArea,
  ...
  if (dc == Attributes::INVALID_DEFAULT) {
    ...
    *da << DgSqlCode(-7001) << DgString0(col->getDefaultValue() ? col->getDefaultValue() : " ")
  ...
  const char *defVal = col->getInitDefaultValue();
  switch (dc) {
    case Attributes::DEFAULT_CURRENT:
      ...
      if (defVal != NULL && strlen(defVal) == TIMESTAMP_LEN) {
        castStr = new (wHeap()) NAString("CAST(TIMESTAMP '", wHeap());
        castStr->append(NAString(defVal, wHeap()));
    ...
    case Attributes::DEFAULT_NULL:
      if (attr->getNullFlag()) {
        // adjust default null length to null indicator len plus vc indicator len
        defaultValue =
            new (generator->getSpace()) char[ExpTupleDesc::NULL_INDICATOR_LENGTH + attr->getVCIndicatorLength()];
        // move null value
    ...
    case Attributes::DEFAULT_USER:
      if (col->getDefaultValue() != NULL) {
        if (col->getType()->getTypeQualifier() == NA_CHARACTER_TYPE) {
          ...
          if (colCharType->isUpshifted()) isUpshifted = TRUE;
        }
        ...
        da = (diagsArea ? diagsArea : CmpCommon::diags());
        *da << DgSqlCode(-7001) << DgString0("NULL POINTER") << DgString1(col->getFullColRefNameAsAnsiString());
        return -1;
      }
      break;
    ...
    case Attributes::DEFAULT_CATALOG_FUNCTION:
      if (defVal != NULL) {
        castStr = new (wHeap()) NAString("CAST( '", wHeap());
        castStr->append(NAString(defVal, wHeap()));
        castStr->append(NAString("' AS ", wHeap()));
    ...
    case Attributes::DEFAULT_SCHEMA_FUNCTION:
      if (defVal != NULL) {
        castStr = new (wHeap()) NAString("CAST( '", wHeap());
        castStr->append(NAString(defVal, wHeap()));
        castStr->append(NAString("' AS ", wHeap()));
  ...
  if (isUpshifted) castStr->append(NAString(" UPSHIFT ", wHeap()));
  if (!attr->getNullFlag())
  ...
  if (CmpSeabaseDDL::isEncodingNeededForSerialization(col)) {
    defaultValueExpr = new (wHeap()) CompEncode(defaultValueExpr, FALSE, -1, CollationInfo::Sort, TRUE);
  }
  ...
  if ((dc == Attributes::DEFAULT_USER) && (col->getType()->getSimpleTypeName() == "VARCHAR")) {
    defaultValueLen = str_len(col->getDefaultValue());
    defaultValue = new (generator->getSpace()) char[ExpTupleDesc::NULL_INDICATOR_LENGTH + attr->getVCIndicatorLength() +
                                                    defaultValueLen];
  } else
  ...
  ex_expr::exp_return_type evalReturnCode =
      defaultValueIdList.evalAtCompileTime(0, ExpTupleDesc::PACKED_FORMAT, defaultValue,
                                           attr->getDefaultValueStorageLength() + extraFltLen, &length, &offset, da);

  if ((evalReturnCode != ex_expr::EXPR_OK) || (da->mark() != daMark)) {
    ...
    *da << DgSqlCode(-7001) << DgString0(convertNAString(*cTemp, CmpCommon::statementHeap()))
        << DgString1(col->getFullColRefNameAsAnsiString());
    delete cTemp;
    return -1;
  }
  ...
  if (attr->getVCIndicatorLength() > 0) {
    ...
    // adjust default length to actual length
    if ((dc == Attributes::DEFAULT_USER) && (col->getType()->getSimpleTypeName() == "VARCHAR"))
      defaultValueAdj = new (generator->getSpace()) char[ExpTupleDesc::NULL_INDICATOR_LENGTH +
                                                         attr->getVCIndicatorLength() + defaultValueLen];
    else
      defaultValueAdj = new (generator->getSpace()) char[attr->getDefaultValueStorageLength()];
    short len;
    char *lenPtr;
...
void ExpGenerator::addDefaultValues(const ValueIdList &val_id_list, const NAColumnArray &naColArr,
                                    ExpTupleDesc *tupleDesc, NABoolean inputIsColumnList) {
  ...
  for (unsigned short i = 0; i < val_id_list.entries(); i++) {
    ...
    if (inputIsColumnList) {
      ...
      GenAssert(((ie->getOperatorType() == ITM_BASECOLUMN) || (ie->getOperatorType() == ITM_INDEXCOLUMN)),
    ...
    if (col->isAddedColumn()) {
      ...
      // the i-th attribute
      Attributes *ithAttr = generator->getMapInfo(val_id_list[i])->getAttr();
      ithAttr->setAddedCol();
  ...
  if ((setAddedColumn) && (tupleDesc)) tupleDesc->setAddedField();

  // Set EID Olt optimization off for tables with added columns.
  if (setAddedColumn && !(CmpCommon::getDefault(EXPAND_DP2_SHORT_ROWS) == DF_ON))
...
void ExpGenerator::copyDefaultValues(ExpTupleDesc *tgtTupleDesc, ExpTupleDesc *srcTupleDesc) {
  ...
  for (CollIndex i = 0; i < numAttrs; i++) {
    ...
    if (srcAttr->isAddedCol()) tgtAttr->setAddedCol();
    ...
    if (srcAttr->getDefaultValue()) {
      ...
      } else if ((srcAttr->getDefaultClass() == Attributes::DEFAULT_USER) && srcAttr->getVCIndicatorLength() > 0) {
        // is varchar
        tgtDefLen =
            ExpTupleDesc::NULL_INDICATOR_LENGTH + tgtAttr->getVCIndicatorLength() + str_len(srcAttr->getDefaultValue());
        srcDefLen =
            ExpTupleDesc::NULL_INDICATOR_LENGTH + srcAttr->getVCIndicatorLength() + str_len(srcAttr->getDefaultValue());
      } else {
        tgtDefLen = tgtAttr->getDefaultValueStorageLength();
        srcDefLen = srcAttr->getDefaultValueStorageLength();
      }
      ...
      if ((tgtDefLen == srcDefLen) && (tgtAttr->getNullFlag() == srcAttr->getNullFlag()) &&
        ...
        if (srcAttr->getNullFlag()) {
          str_cpy_all((char *)&nullVal, srcDefVal, ExpTupleDesc::NULL_INDICATOR_LENGTH);
          srcDefVal += ExpTupleDesc::NULL_INDICATOR_LENGTH;
        }

        if (tgtAttr->getNullFlag()) {
          str_cpy_all(tgtDefVal, (char *)&nullVal, ExpTupleDesc::NULL_INDICATOR_LENGTH);
...
short ExpGenerator::genColNameList(const NAColumnArray &naColArr, Queue *&colNameList) {
  colNameList = NULL;
  if (naColArr.entries() == 0) return 0;
  ...
  for (unsigned short i = 0; i < naColArr.entries(); i++) {
    ...
    char *colName = space->allocateAndCopyToAlignedSpace(col->getColName().data(), col->getColName().length());
...
short ExpGenerator::handleUnsupportedCast(Cast *castNode) {
  ...
  if (((DFS2REC::isTinyint(srcFsType)) && (NOT DFS2REC::isTinyint(tgtFsType))) ||
      ((NOT DFS2REC::isTinyint(srcFsType)) && (DFS2REC::isTinyint(tgtFsType))))
    tinyintCast = TRUE;

  if ((srcFsType == REC_BIN64_UNSIGNED) || (tgtFsType == REC_BIN64_UNSIGNED)) largeUnsignedCast = TRUE;

  if ((NOT tinyintCast) && (NOT largeUnsignedCast)) return 0;

  ex_conv_clause tempClause;
  if (tempClause.isConversionSupported(srcFsType, srcNAType.getNominalSize(), tgtFsType, tgtNAType.getNominalSize()))
    return 0;
  ...
  if (tinyintCast) {
    ...
    if (DFS2REC::isInterval(srcFsType))  // interval to tinyint
    {
      ...
      newType = new (generator->wHeap()) SQLNumeric(generator->wHeap(), sizeof(short), srcInt.getTotalPrecision(),
                                                    srcInt.getFractionPrecision(), TRUE, srcNAType.supportsSQLnull());
    } else if (DFS2REC::isInterval(tgtFsType))  // tinyint to interval
    {
      const NumericType &srcNum = (NumericType &)srcNAType;
      newType =
          new (generator->wHeap()) SQLNumeric(generator->wHeap(), sizeof(short), srcNum.getPrecision(),
                                              srcNum.getScale(), NOT srcNum.isUnsigned(), srcNAType.supportsSQLnull());
    } else if (DFS2REC::isTinyint(srcFsType))  // tinyint to non-tinyint
    {
      const NumericType &srcNum = (NumericType &)srcNAType;

      if ((srcNum.getScale() == 0) && (srcNum.binaryPrecision()))
        newType =
            new (generator->wHeap()) SQLSmall(generator->wHeap(), NOT srcNum.isUnsigned(), tgtNAType.supportsSQLnull());
      else
        newType = new (generator->wHeap())
            SQLNumeric(generator->wHeap(), sizeof(short), srcNum.getPrecision(), srcNum.getScale(),
                       NOT srcNum.isUnsigned(), tgtNAType.supportsSQLnull());
    } else if (DFS2REC::isTinyint(tgtFsType))  // non-tinyint to tinyint
    {
      const NumericType &srcNum = (NumericType &)srcNAType;
      const NumericType &tgtNum = (NumericType &)tgtNAType;

      if ((tgtNum.getScale() == 0) && (tgtNum.binaryPrecision()))
        newType =
            new (generator->wHeap()) SQLSmall(generator->wHeap(), NOT tgtNum.isUnsigned(), tgtNAType.supportsSQLnull());
      else
        newType = new (generator->wHeap())
            SQLNumeric(generator->wHeap(), sizeof(short), tgtNum.getPrecision(), tgtNum.getScale(),
                       NOT tgtNum.isUnsigned(), tgtNAType.supportsSQLnull());
    }

    ItemExpr *newChild = new (generator->wHeap()) Cast(castNode->child(0), newType);
  ...
  if ((srcFsType == REC_BIN64_UNSIGNED) || (tgtFsType == REC_BIN64_UNSIGNED)) {
    ...
    ItemExpr *newChild = new (generator->wHeap())
        Cast(castNode->child(0), new (generator->wHeap()) SQLLargeInt(generator->wHeap(), numSrc.getScale(), 1, TRUE,
...
ItemExpr *ExpGenerator::scaleBy10x(const ValueId &source, int exponent) {
  ...
  NABoolean ignoreSpecialRounding =
      (retTree->getOperatorType() != ITM_DIVIDE) ? FALSE : ((BiArith *)retTree)->ignoreSpecialRounding();
  //
  ...
  if ((retTree) && (downscale) && (retTree->getOperatorType() == ITM_DIVIDE)) {
    ((BiArith *)retTree)->setDivToDownscale(TRUE);

    if (ignoreSpecialRounding) ((BiArith *)retTree)->setIgnoreSpecialRounding();
  }
...
/////////////////////////////////////////////////////////////////
ItemExpr *ExpGenerator::matchScales(const ValueId &source, const NAType &targetType) {
  ...
  if (sourceTree != retTree) {
    ...
    if (handleUnsupportedCast((Cast *)retTree)) return NULL;
...
//////////////////////////////////////////////////////////////////
ItemExpr *ExpGenerator::matchScalesNoCast(const ValueId &source, const NAType &targetType) {
  ItemExpr *retTree = source.getItemExpr();
  //
  ...
  if (((sourceTypeQual != NA_NUMERIC_TYPE) && (sourceTypeQual != NA_INTERVAL_TYPE)) ||
      ((targetTypeQual != NA_NUMERIC_TYPE) && (targetTypeQual != NA_INTERVAL_TYPE)))
    return retTree;
  ...
  if (retTree->getOperatorType() == ITM_CONSTANT) {
    ...
    if (constant->isNull()) return retTree;
  }
  ...
  if ((sourceTypeQual == NA_INTERVAL_TYPE) && (targetTypeQual == NA_INTERVAL_TYPE)) {
    ...
    if (((NumericType &)source.getType()).isExact() && (NOT((NumericType &)targetType).isExact()) &&
...
ItemExpr *ExpGenerator::convertIntervalToNumeric(const ValueId &source) {
  ...
  //
  if (source.getType().getTypeQualifier() != NA_INTERVAL_TYPE) return retTree;
  ...
  if (sourceInterval.getStartField() != sourceInterval.getEndField())
    retTree = new (wHeap()) Cast(
        retTree, new (wHeap()) SQLInterval(wHeap(), sourceInterval.supportsSQLnull(), sourceInterval.getEndField(),
                                           sourceInterval.getTotalPrecision() - sourceInterval.getFractionPrecision(),
                                           sourceInterval.getEndField(), sourceInterval.getFractionPrecision()));
  //
...
/////////////////////////////////////////////////////////////////
ItemExpr *ExpGenerator::convertNumericToInterval(const ValueId &source, const NAType &targetType) {
  ItemExpr *retTree = source.getItemExpr();
  //
  // If the source is not numeric or the target type is not an interval,
  // return the source tree unchanged.
  //
  if ((source.getType().getTypeQualifier() != NA_NUMERIC_TYPE) || (targetType.getTypeQualifier() != NA_INTERVAL_TYPE))
    return retTree;
  ...
  SQLInterval *interval =
      new (wHeap()) SQLInterval(wHeap(), targetInterval.supportsSQLnull(), targetInterval.getEndField(),
                                targetInterval.getTotalPrecision() - targetInterval.getFractionPrecision(),
                                targetInterval.getEndField(), targetInterval.getFractionPrecision());

  retTree = ((UInt32)sourceNumeric.getScale() != targetInterval.getFractionPrecision())
...
/////////////////////////////////////////////////////////////////
ItemExpr *ExpGenerator::matchIntervalEndFields(const ValueId &source, const NAType &targetType) {
  ItemExpr *retTree = source.getItemExpr();
  //
  ...
  //
  if ((source.getType().getTypeQualifier() != NA_INTERVAL_TYPE) || (targetType.getTypeQualifier() != NA_INTERVAL_TYPE))
    return retTree;
  IntervalType &sourceInterval = (IntervalType &)source.getType();
  IntervalType &targetInterval = (IntervalType &)targetType;
  if (sourceInterval.getEndField() == targetInterval.getEndField()) return retTree;
...
short ExpGenerator::generateAggrExpr(const ValueIdSet &val_id_set, ex_expr::exp_node_type /* node_type */,
                                     ex_expr **expr, short gen_last_clause, NABoolean groupByOperation,
  ...
  for (val_id = val_id_set.init(); val_id_set.next(val_id); val_id_set.advance(val_id)) {
    ...
    if (item_expr->child(0)->castToItemExpr()->getValueId().getType().isComposite()) {
    ...
    switch (item_expr->getOperatorType()) {
      ...
      case ITM_SUM: {
        ...
        if (case_ == 1) {
          if (item_expr->getOperatorType() == ITM_MIN)
            initVal = getMinMaxValue(val_id.getType(), -2 /*max without null*/
            );
          else if (item_expr->getOperatorType() == ITM_MAX)
            initVal = getMinMaxValue(val_id.getType(), 0 /* min value */
            );
          else  // SUM
          {
            if (val_id.getType().getTypeQualifier() == NA_INTERVAL_TYPE) {
              ...
              initVal = new (wHeap()) ConstValue(type, (void *)&zero, type->getNominalSize(), nas);
            } else
              initVal = new (wHeap()) ConstValue(0);
          }
          newExpr = new (wHeap()) Cast(initVal, &(val_id.getType()));
        } else  // Case 2
        {
          initVal = generateNullConst(val_id.getType());
          NAType *newType = item_expr->getValueId().getType().newCopy(wHeap());
          newType->setSQLnullFlag();

          newExpr = new (wHeap()) Cast(initVal, newType);
        }
      ...
      case ITM_ORC_SUM_NV: {
        generator->getMapInfo(val_id)->codeGenerated();
        newExpr = new (wHeap()) Cast(new (wHeap()) ConstValue(0), &(val_id.getType()));
      } break;
      ...
      case ITM_ONE_ROW: {
        ...
        for (outvid = init_val_id_set.init(); init_val_id_set.next(outvid); init_val_id_set.advance(outvid)) {
      ...
      case ITM_PIVOT_GROUP: {
        generator->getMapInfo(val_id)->codeGenerated();
        newExpr = new (wHeap()) Cast(new (wHeap()) ConstValue(""), &(val_id.getType()));
      } break;
    ...
    if (newExpr != NULL) {
      ...
      map_info = generator->addMapInfo(newExpr->getValueId(), attr);
      (map_info->getAttr())->copyLocationAttrs(attr);
      if (attr->isForceFixed()) (map_info->getAttr())->setForceFixed();
    }
  }
  ...
  if (groupByOperation == FALSE) {
    ...
    for (val_id = val_id_set.init(); val_id_set.next(val_id); val_id_set.advance(val_id)) {
      ...
      if (!item_expr->isAnAggregate()) {
        ...
        switch (((Aggregate *)item_expr)->getEffectiveOperatorType()) {
          ...
          case ITM_PIVOT_GROUP: {
            generator->getMapInfo(val_id)->codeGenerated();
            ConstValue *const_value = generateNullConst(val_id.getType());

            NAType *newType = item_expr->getValueId().getType().newCopy(wHeap());
            newType->setSQLnullFlag();

            newExpr = new (wHeap()) Cast(const_value, newType);
    ...
    generateSetExpr(*newValIdSet, ex_expr::exp_ARITH_EXPR, &final_null_expr);
  ...
  // Collect attributes for the aggregate results
  UInt32 numAttrs = val_id_set.entries();
  ...
  for (val_id = val_id_set.init(); val_id_set.next(val_id); val_id_set.advance(val_id)) {
    ...
    switch (item_expr->getOperatorType()) {
      case ITM_MIN: {
        if (case_ == 1) {
          newExpr = createExprTree("@A2 < @A1", 0, 2, item_expr, item_expr->child(0));
          newExpr = new (wHeap()) AggrMinMax(item_expr->child(0), newExpr);
          if (item_expr->getValueId().getType().supportsSQLnull()) nullSwitcharoo = TRUE;
        }       // Case 1
        else {  // Case 2
          newExpr = createExprTree(
              "@A1 IS NULL OR"
      ...
      case ITM_SUM: {
        ...
        //
        if (case_ == 1) {
          NAType *sumType = item_expr->getValueId().getType().newCopy(wHeap());
          sumType->setNullable(FALSE);
          if (item_expr->child(0)->getValueId().getType().supportsSQLnull())
            newExpr = new (wHeap()) Cast(item_expr->child(0), sumType, ITM_CAST);
          else
            newExpr = item_expr->child(0);
          // newExpr = new(wHeap()) Cast(item_expr->child(0),
          //			    sumType,
          //			    ITM_CAST);
          newExpr = new (wHeap()) BiArithSum(ITM_PLUS, newExpr, item_expr);
        }
      ...
      case ITM_COUNT_NONULL: {
        ...
        //
          ...
          newExpr = new (wHeap()) ReplaceNull(item_expr->child(0), constOne, constZero);
          newExpr = new (wHeap()) Cast(newExpr, sumType, ITM_CAST);
          newExpr = new (wHeap()) BiArithSum(ITM_PLUS, newExpr, item_expr);
        }
      } break;
      ...
      case ITM_ANY_TRUE: {
        newExpr = createExprTree("CASE WHEN @B1 IS TRUE THEN @A2 WHEN @B1 IS UNKNOWN THEN @A3 ELSE @A4 END", 0, 4,
                                 item_expr->child(0), new (wHeap()) BoolVal(ITM_RETURN_TRUE),
                                 new (wHeap()) BoolVal(ITM_RETURN_NULL), 0);
      ...
      case ITM_ONE_ROW: {
        ...
        // returned so far.
        //
        newExpr = new (wHeap()) Aggregate(ITM_ONE_ROW, new (wHeap()) ConstValue(0), FALSE, ITM_ONE_ROW, ' ');
        newExpr->bindNode(generator->getBindWA());
      ...
      case ITM_PIVOT_GROUP: {
        ...
        newExpr = new (wHeap()) PivotGroup(ITM_PIVOT_GROUP, item_expr->child(0), item_expr->child(1), pg->orderBy(),
                                           pg->reqdOrder(), pg->maxLen(), pg->isDistinct());
      } break;
      ...
      case ITM_ONEROW: {
        ...
        newExpr = new (wHeap()) Cast(item_expr->child(0), &oneRowType, ITM_CAST);
        if (NOT raiseErrorGenerated) {
          ItemExpr *raiseError =
              new (wHeap()) Aggregate(ITM_ONE_ROW, new (wHeap()) ConstValue(0), FALSE, ITM_ONE_ROW, ' ');
    ...
    if (newExpr != NULL) {
      ...
      ExpTupleDesc::TupleDataFormat currentTupleFormat = attr->getTupleFormat();

      if (tdf == ExpTupleDesc::UNINITIALIZED_FORMAT)
        tdf = currentTupleFormat;
      else
        assert(tdf == currentTupleFormat);

      UInt32 endOffset = attr->getOffset() + attr->getLength();

      if (endOffset > tupleLength) tupleLength = endOffset;
  ...
  if (tupleDesc) *tupleDesc = new (getSpace()) ExpTupleDesc(numAttrs, attrs, tupleLength, tdf, tdescF, getSpace());
  ...
  for (val_id = val_id_set.init(); val_id_set.next(val_id); val_id_set.advance(val_id)) {
    ...
    if (item_expr->getOperatorType() == ITM_GROUPING) {
      ...
      ItemExpr *groupingExpr = new (wHeap()) AggrGrouping(ag->getRollupGroupIndex());
  ...
  if (NOT groupingValIdSet.isEmpty()) {
    ...
    generateSetExpr(groupingValIdSet, ex_expr::exp_ARITH_EXPR, &grouping_expr);
  ...
  *expr = new (getSpace()) AggrExpr(init_expr, perrec_expr,
                                    0,  // final_expr,
                                    final_null_expr, grouping_expr);
...
short ExpGenerator::generateArithExpr(const ValueId &val_id, ex_expr::exp_node_type node_type, ex_expr **expr)

{
  ...
  ItemExpr *temp = val_id.getValueDesc()->getItemExpr()->preCodeGen(generator);
  if (!temp) return -1;
...
short ExpGenerator::generateBulkMoveAligned(const ValueIdList &inValIdList, ValueIdList &outValIdList,
  ...
  MapInfo *srcMapInfo =
      generator->getMapInfoAsIs(inValIdList[0].getItemExpr()->getChild(0)->castToItemExpr()->getValueId());
  ...
  NABoolean bulkMoveNullVarchar = (CmpCommon::getDefault(BULK_MOVE_NULL_VARCHAR) == DF_ON);
  // Limit the amount of VARCHAR bytes in the bulk move. If we choose a bulk
  ...
  for (CollIndex i = 0; ((i < inValIdList.entries()) && (bulkMove)); i++) {
    ...
    srcMapInfo = generator->getMapInfoAsIs(inValIdList[i].getItemExpr()->getChild(0)->castToItemExpr()->getValueId());
    ...
    if (srcAttr && srcAttr->getVCIndicatorLength()) {
      varcharBytes += srcAttr->getLength();

      if (varcharBytes > maxVarcharBytes) return 0;

      if (srcAttr->getVoaOffset() > voaOffset) {
        ...
        alignment = (srcAttr->isSQLMXAlignedFormat() && (srcAttr->getNextFieldIndex() == ExpOffsetMax))
    ...
    if ((srcAttr && (NOT srcAttr->isSQLMXAlignedFormat())) || (tgtAttr && (NOT tgtAttr->isSQLMXAlignedFormat())))
      return 0;

    if ((!tgtAttr) || (!srcAttr) || (tgtAttr->getTupleFormat() != srcAttr->getTupleFormat()) ||
        (tgtAttr->getDatatype() != srcAttr->getDatatype()) ||
        ((NOT bulkMoveNullVarchar) && (tgtAttr->getNullFlag() || srcAttr->getNullFlag())) ||
        (tgtAttr->getNullFlag() != srcAttr->getNullFlag()) || (tgtAttr->getScale() != srcAttr->getScale()) ||
        (tgtAttr->getPrecision() != srcAttr->getPrecision()) || (tgtAttr->getLength() != srcAttr->getLength()) ||
        (srcAttr->getAtpIndex() == 0) || (srcAttr->getAtpIndex() == 1) || ((UInt32)tgtAttr->getAtp() != firstTgtAtp) ||
        ((UInt32)srcAttr->getAtp() != firstSrcAtp) || ((UInt32)tgtAttr->getAtpIndex() != firstTgtAtpIx) ||
        ((UInt32)srcAttr->getAtpIndex() != firstSrcAtpIx) || (tgtAttr->getOffset() != srcAttr->getOffset()) ||
        ...
        (tgtAttr->getNullIndOffset() != srcAttr->getNullIndOffset()) ||
        (tgtAttr->getVoaOffset() != srcAttr->getVoaOffset()) || (srcAttr->isAddedCol()) || (tgtAttr->isAddedCol()))
      bulkMove = FALSE;
  }
  ...
  if (bulkMove == TRUE) {
    ...
    ItemExpr *bulkMoveTgt =
        new (generator->wHeap()) Convert(bulkMoveSrc, voaOffset, vcIndicatorLength, nullIndicatorLength, alignment);
...
short ExpGenerator::generateBulkMove(const ValueIdList &inValIdList, ValueIdList &outValIdList, int tupleLength,
                                     int *bulkMoveSrcStartOffset)  // IN(O)
{
  if (bulkMoveSrcStartOffset)  // param passed in.
  {
  ...
  if (inValIdList.entries() == 0) return 0;
  ...
  if (!tgtMapInfo) return 0;  // must be in maptable to check for bulkmove attrs

  tgtAttr = tgtMapInfo->getAttr();

  srcMapInfo = generator->getMapInfoAsIs(inValIdList[0].getItemExpr()->getChild(0)->castToItemExpr()->getValueId());

  if (!srcMapInfo) return 0;  // must be in maptable to check for bulkmove attrs
  ...
  if ((NOT bulkMoveSrcStartOffset)  // parameter not passed in
      && (inValIdList.entries() == 0) && (NOT srcAttr->getNullFlag()))
    return 0;
  ...
  if (tgtAttr && tgtAttr->isSQLMXAlignedFormat()) {
    return generateBulkMoveAligned(inValIdList, outValIdList, tupleLength, bulkMoveSrcStartOffset);
  }
  ...
  for (CollIndex i = 0; ((i < inValIdList.entries()) && (bulkMove)); i++) {
    ...
    if (tgtAttr && tgtAttr->getNullFlag() && (firstTgtNullOffset == ExpOffsetMax))
      firstTgtNullOffset = tgtAttr->getNullIndOffset();
    if (srcAttr && srcAttr->getNullFlag() && (firstSrcNullOffset == ExpOffsetMax))
      firstSrcNullOffset = srcAttr->getNullIndOffset();
    if (tgtAttr && tgtAttr->isVariableLength() && (firstTgtVCLenIndOffset == ExpOffsetMax))
      firstTgtVCLenIndOffset = tgtAttr->getVCLenIndOffset();
    if (srcAttr && srcAttr->isVariableLength() && (firstSrcVCLenIndOffset == ExpOffsetMax))
      firstSrcVCLenIndOffset = srcAttr->getVCLenIndOffset();

    if ((!tgtAttr) || (!srcAttr) || ((NOT bulkMoveNullVarchar) && (tgtAttr->getNullFlag() || srcAttr->getNullFlag())) ||
        (tgtAttr->getNullFlag() != srcAttr->getNullFlag()) ||
        (tgtAttr->getTupleFormat() != srcAttr->getTupleFormat()) ||
        (tgtAttr->getDatatype() != srcAttr->getDatatype()) || (tgtAttr->getScale() != srcAttr->getScale()) ||
        (tgtAttr->getPrecision() != srcAttr->getPrecision()) || (tgtAttr->getLength() != srcAttr->getLength()) ||
        ((UInt32)tgtAttr->getAtp() != firstTgtAtp) || ((UInt32)srcAttr->getAtp() != firstSrcAtp) ||
        ((UInt32)tgtAttr->getAtpIndex() != firstTgtAtpIx) || ((UInt32)srcAttr->getAtpIndex() != firstSrcAtpIx) ||
        ((tgtAttr->getOffset() - firstTgtOffset) != (srcAttr->getOffset() - firstSrcOffset)) ||
        (tgtAttr->getNullFlag() && ((tgtAttr->getOffset() - tgtAttr->getNullIndOffset()) !=
                                    (srcAttr->getOffset() - srcAttr->getNullIndOffset()))) ||
        (tgtAttr->getNullFlag() &&
         ((tgtAttr->getNullIndOffset() - firstTgtNullOffset) != (srcAttr->getNullIndOffset() - firstSrcNullOffset))) ||
        ((NOT bulkMoveNullVarchar) &&
         ((tgtAttr->getVCIndicatorLength() > 0) || (srcAttr->getVCIndicatorLength() > 0))) ||
        (tgtAttr->isVariableLength() && ((tgtAttr->getVCLenIndOffset() - firstTgtVCLenIndOffset) !=
                                         (srcAttr->getVCLenIndOffset() - firstSrcVCLenIndOffset))) ||
        (srcAttr->getAtpIndex() == 0) || (srcAttr->getAtpIndex() == 1) || (srcAttr->isAddedCol()) ||
        (tgtAttr->isAddedCol()))
      bulkMove = FALSE;
  }
...
short ExpGenerator::generateContiguousMoveExpr(const ValueIdList &valIdList, short addConvNodes, int atp, int atpIndex,
  ...
  for (CollIndex i = 0; i < valIdList.entries(); i++) {
    ...
    if (alignedFormat && (colArray == NULL) && ((col = valIdList[i].getNAColumn(TRUE)) != NULL) && col->isAddedColumn())
      attrs[i]->setAddedCol();
  ...
  if ((moveExpr) && (bulkMoveSrcStartOffset) && (*bulkMoveSrcStartOffset == -2)) alwaysGenMoveExpr = TRUE;
  ...
  if (addConvNodes && doBulkMoves) {
    if (generateBulkMove(convValIdList, bulkMoveValueIdList, tupleLength, bulkMoveSrcStartOffset) == -1) return -1;
  ...
  if ((moveExpr) && ((!bulkMoveSrcStartOffset) || (*bulkMoveSrcStartOffset == -1) || (alwaysGenMoveExpr))) {
    generateListExpr(*newValIdList, ex_expr::exp_ARITH_EXPR, moveExpr, atp, atpIndex, hdrInfo);
  ...
  if (newMapTable) {
    ...
    *newMapTable = generator->unlinkLast();
    for (CollIndex i = 0; i < valIdList.entries(); i++) {
      ...
      // expression.
      if (convValIdList[i].getItemExpr()->getOperatorType() != ITM_CONSTANT) {
        // Get the attributes of the contiguous buffer...
        Attributes *contAttrib = generator->getMapInfo((convValIdList)[i])->getAttr();

        // ...add it to the new map table as if it belonged to
        // the original value id...
        MapInfo *mapInfo = generator->addMapInfoToThis((*newMapTable), valIdList[i], contAttrib);
  ...
  if (tgtValues != NULL) tgtValues->insert(convValIdList);
...
short ExpGenerator::genGuardedContigMoveExpr(const ValueIdSet guard, const ValueIdList &valIdList, short addConvNodes,
                                             ...
                                             ExpTupleDesc::TupleDescFormat tdescF, MapTable **newMapTable,
                                             ValueIdList *tgtValues, int start_offset) {
  // ---------------------------------------------------------------------
  // Generate an expression to take scattered values (given in valIdList)
  // and copy them to a contiguous buffer tuple (atp,atpIndex). The values
  // are allocated in the tuple in the sequence given in valIdList.
                                        ...
                                        int start_offset)                      // IN(O)
{
  ...
  for (CollIndex i = 0; i < val_id_list.entries(); i++) {
    ...
    if (ie->getValueId().getType().getVarLenHdrSize() > 0) {
      // Explode varchars.
      cast_node = new (generator->wHeap()) ExplodeVarchar(ie, &(ie->getValueId().getType()), FALSE);
    } else {
      cast_node = new (generator->wHeap()) Cast(ie, &(ie->getValueId().getType()));
    }
  ...
  return generateContiguousMoveExpr(explodeVidList, 0 /*don't add conv nodes*/, atp, atpIndex, tf, tupleLength,
                                    explodeExpr, tupleDesc, tdescF, newMapTable, tgtValues, start_offset);
}
...
short ExpGenerator::generateHeaderClause(int atp, int atpIndex, ExpHdrInfo *hdrInfo) {
  ...
  ExpTupleDesc::TupleDataFormat tdf =
      (hdrInfo->getBitmapEntryOffset() > 0 ? ExpTupleDesc::SQLMX_ALIGNED_FORMAT : ExpTupleDesc::SQLMX_FORMAT);

  // This is a showplan statement.
  if (getShowplan()) numOperands = (Int16)(numOperands * 2);

  attr = new (generator->wHeap()) Attributes *[numOperands];

  // Initialize an attribute for the full header.
  attr[0] = (Attributes *)new (generator->wHeap()) SimpleType(hdrInfo->getAdminSize(), 1, 0);
  ...
  if (getShowplan()) {
    ...
    attr[1] = new (generator->wHeap()) ShowplanAttributes(dummyVID, convertNAString(hdrStr, generator->wHeap()));
  }

  hdrClause =
      new (getSpace()) ExHeaderClause(attr, getSpace(), hdrInfo->getAdminSize(), hdrInfo->getBitmapEntryOffset(),
                                      hdrInfo->getBitmapOffset(), hdrInfo->getFirstFixedOffset());
  ...
  if (atpIndex == 1)  // temp data location
    addTempsLength(attr[0]->getLength());
...
// to this ValueId.
short ExpGenerator::generateExpr(const ValueId &val_id, ex_expr::exp_node_type node_type, ex_expr **expr) {
  initExprGen();
  startExprGen(expr, node_type);
...
/////////////////////////////////////////////////////////////////////////////
NABoolean ExpGenerator::processKeyEncodingOptimization(const NAColumnArray &allColumns,      /* IN */
                                                       const NAColumnArray &indexKeyColumns, /* IN */
                                                       const ValueIdList &indexKey,          /* IN */
                                                       const short keyTag,                   /* IN */
                                                       int &keyLen, int &firstKeyColumnOffset) {
  ...
  if (keyTag == 0)  // primary index
    prevColNumber = (CollIndex)indexKeyColumns[0]->getPosition();
  else {
    // index
  ...
  for (i = 0; i < indexKeyEntries; i++) {
    ...
    if ((valId.getType().isEncodingNeeded() == TRUE) ||
        ...
        (valId.getType().supportsSQLnull() == TRUE) || (indexKeyColumns.isAscending(i) == FALSE) ||
        ((naCol->getPosition() - prevColNumber) < 0) || ((naCol->getPosition() - prevColNumber) > 1) ||
        ((thisAttr->getOffset() - prevAttr->getOffset()) > prevAttr->getStorageLength()) || (!naCol->isStoredOnDisk()))
      return TRUE;
...
NABoolean ExpGenerator::isKeyEncodingNeeded(const IndexDesc *indexDesc, int &keyLen, int &firstKeyColumnOffset) {
  return processKeyEncodingOptimization(indexDesc->getNAFileSet()->getAllColumns(),
                                        indexDesc->getNAFileSet()->getIndexKeyColumns(), indexDesc->getIndexKey(),
                                        indexDesc->getNAFileSet()->getKeytag(), keyLen, firstKeyColumnOffset);
}
...
short ExpGenerator::generateKeyEncodeExpr(const IndexDesc *indexDesc, int atp, int atp_index,
                                          ExpTupleDesc::TupleDataFormat tf, int &keyLen, ex_expr **encode_expr,
                                          NABoolean optimizeKeyEncoding, int &firstKeyColumnOffset,
                                          const ValueIdList *inKeyList, NABoolean handleSerialization) {
  if ((optimizeKeyEncoding == TRUE) && (isKeyEncodingNeeded(indexDesc, keyLen, firstKeyColumnOffset) == FALSE))
    return 0;
  ...
  for (i = 0; i < numEntries; i++) {
    ...
    ItemExpr *col_node = (inKeyList ? (*inKeyList)[i].getItemExpr() : (indexDesc->getIndexKey())[i].getItemExpr());

    short desc_flag = TRUE;
    if ((indexDesc->getNAFileSet()->getIndexKeyColumns()).isAscending(i)) desc_flag = FALSE;

    if ((tf == ExpTupleDesc::SQLMX_KEY_FORMAT) && (col_node->getValueId().getType().getVarLenHdrSize() > 0) &&
      ...
      const CharType &char_t = (CharType &)(col_node->getValueId().getType());
      if (!CollationInfo::isSystemCollation(char_t.getCollation())) {
        if (DFS2REC::isCharacterString(col_node->getValueId().getType().getFSDatatype())) {
          col_node = new (wHeap())
              Cast(col_node,
                   (new (wHeap()) SQLChar(wHeap(), CharLenInfo(char_t.getStrCharLimit(), char_t.getDataStorageSize()),
                                          char_t.supportsSQLnull(), FALSE, FALSE, FALSE, char_t.getCharSet(),
                                          char_t.getCollation(), char_t.getCoercibility())));
          if (char_t.isVarchar2()) ((Cast *)col_node)->setPadUseZero(TRUE);
        } else {
          // binary. explode with zeroes
          col_node = new (wHeap()) Cast(
              col_node, (new (wHeap()) SQLBinaryString(wHeap(), char_t.getDataStorageSize(),
                                                       col_node->getValueId().getType().supportsSQLnull(), FALSE)));
        }
      }
    }
    ...
    if (handleSerialization) {
      ...
      if ((!isAlignedRowFormat) && CmpSeabaseDDL::isEncodingNeededForSerialization(nac)) {
        if (desc_flag) {
          ...
          col_node = new (generator->wHeap()) CompDecode(col_node, &col_node->getValueId().getType(), FALSE, TRUE);
        } else
          // col_node is already in encoded format.
          enode = new (generator->wHeap()) Cast(col_node, &col_node->getValueId().getType());
  ...
  generateContiguousMoveExpr(encode_val_id_list,
                             ...
                             atp, atp_index, tf, keyLen, encode_expr);
...
short ExpGenerator::generateDeserializedMoveExpr(const ValueIdList &valIdList, int atp, int atpIndex,
  ...
  for (int i = 0; i < valIdList.entries(); i++) {
    ...
    ItemExpr *ie = NULL;
    if (nac && CmpSeabaseDDL::isEncodingNeededForSerialization(nac) && !alreadyDeserialized.contains(vid)) {
      ie = new (generator->wHeap()) CompDecode(vid.getItemExpr(), &vid.getType(), FALSE, TRUE);
    } else {
  ...
  return generateContiguousMoveExpr(deserVIDlist,
                                    0,  // no conv nodes
                                    atp, atpIndex, tdataF, tupleLength, moveExpr, tupleDesc, tdescF);
}

short ExpGenerator::generateExtractKeyColsExpr(const ValueIdList &colVidList,  // const IndexDesc * indexDesc,
                                               int atp, int atp_index, int &keyLen, ex_expr **key_cols_expr) {
  ...
  generateContiguousMoveExpr(val_id_list,
                             1,  // add convert nodes,
                             atp, atp_index, ExpTupleDesc::SQLARK_EXPLODED_FORMAT, keyLen, key_cols_expr);
...
short ExpGenerator::generateKeyColValueExpr(const ValueId vid, int atp, int atp_index, int &len, ex_expr **colValExpr) {
  ...
  if (NOT((generateNarrow) || (targetType->getVarLenHdrSize() > 0))) {
    ...
    if (HbaseAccess::isEncodingNeededForSerialization(keycol)) {
      cv = new (generator->wHeap()) CompEncode(cv, FALSE, -1, CollationInfo::Sort, TRUE);
    }
    ...
    generateContiguousMoveExpr(val_id_list,
                               0,  // no conv nodes
                               atp, atp_index, ExpTupleDesc::SQLARK_EXPLODED_FORMAT, len, colValExpr);
  }
...
ItemExpr *ExpGenerator::generateKeyCast(const ValueId vid, ItemExpr *dataConversionErrorFlag, NABoolean desc_flag,
  ...
  switch (tf) {
    case ExpTupleDesc::SQLMX_KEY_FORMAT: {
      ...
      if (targetType->getTypeQualifier() == NA_CHARACTER_TYPE) {
        ItemExpr *keycol_tgt = keycol;
        if (keycol->getOperatorType() == ITM_TRANSLATE) keycol_tgt = keycol->child(0);

        const CharType &char_t = (CharType &)(keycol_tgt->getValueId().getType());
        ...
        if ((keycol_tgt->getValueId().getType().getVarLenHdrSize() > 0) || (char_t.isUpshifted())) {
          if (!CollationInfo::isSystemCollation(char_t.getCollation())) {
            if ((castVarcharToAnsiChar) && (keycol_tgt->getValueId().getType().getVarLenHdrSize() > 0)) {
              targetType =
                  new (wHeap()) ANSIChar(wHeap(), char_t.getDataStorageSize(), char_t.supportsSQLnull(), FALSE, FALSE,
                                         char_t.getCharSet(), char_t.getCollation(), char_t.getCoercibility());
            } else if (DFS2REC::isCharacterString(keycol_tgt->getValueId().getType().getFSDatatype())) {
              targetType =
                  new (wHeap()) SQLChar(wHeap(), CharLenInfo(char_t.getStrCharLimit(), char_t.getDataStorageSize()),
                                        char_t.supportsSQLnull(), FALSE, char_t.isCaseinsensitive(), FALSE,
                                        char_t.getCharSet(), char_t.getCollation(), char_t.getCoercibility());
              if (char_t.isVarchar2()) {
                doVarcharRPad = TRUE;
              }
            } else if (DFS2REC::isBinaryString(keycol_tgt->getValueId().getType().getFSDatatype())) {
              targetType = new (wHeap()) SQLBinaryString(wHeap(), keycol_tgt->getValueId().getType().getNominalSize(),
                                                         keycol_tgt->getValueId().getType().supportsSQLnull(), FALSE);
            }
          }
        }
      }

      if (keycol->getOperatorType() == ITM_HBASE_ROWID) {
        // convertfromhex for ROWID search.
        if (sourceType->getTypeQualifier() == NA_CHARACTER_TYPE && keyval->getOperatorType() != ITM_CONVERTFROMHEX) {
          ItemExpr *convert = new (wHeap()) ConvertHex(ITM_CONVERTFROMHEX, keyval);

          if (convert) {
            keyval = convert;

            if (targetType->getTypeQualifier() == NA_CHARACTER_TYPE) {
              HbaseRowid *rowidCol = (HbaseRowid *)keycol;
              targetType = rowidCol->rowidValsType();
            }
          }
        }
        cnode = new (wHeap()) Narrow(keyval, dataConversionErrorFlag, targetType, ITM_NARROW, desc_flag);
        knode = cnode;
      } else {
        ...
        if (generateNarrow) {
          cnode = new (wHeap()) Narrow(keyval, dataConversionErrorFlag, targetType, ITM_NARROW, desc_flag);
          if (doVarcharRPad) ((Narrow *)cnode)->setPadUseZero(TRUE);
        } else {
          if (*sourceType == *targetType) {
            ...
            if (doVarcharRPad) ((Cast *)cnode)->setPadUseZero(TRUE);
          }
        }
  ...
  if ((possibleErrorCount > 0) && (NOT allChosenPredsAreEqualPreds)) {
    ItemExpr *relativeMin =
        new (wHeap()) ConstValue(targetType->newCopy(wHeap()), !desc_flag, targetType->supportsSQLnull());
    ...
    knode = createExprTree("CASE WHEN @A1 = 0 THEN @A2 WHEN @A1 < 0 THEN @A3 ELSE @A4 END", 0, 4,
  ...
  if (generateNarrow) possibleErrorCount++;
...
short ExpGenerator::generateKeyExpr(const NAColumnArray &indexKeyColumns, const ValueIdList &val_id_list, int atp,
  ...
  if (val_id_list.entries() > 0) {
    ...
    for (int i = 0; i < val_id_list.entries(); i++) {
      ItemExpr *knode = NULL;

      if ((indexKeyColumns[i]->getNATable()->isHbaseCellTable()) ||
          (indexKeyColumns[i]->getNATable()->isHbaseRowTable()) || (indexKeyColumns[i]->isPrimaryKeyNotSerialized()) ||
        ...
        knode = generateKeyCast(val_id_list[i], dataConversionErrorFlag, !indexKeyColumns.isAscending(i), tf,
                                possibleErrorCount /* in/out */, allChosenPredsAreEqualPreds, FALSE);
      }
    ...
    generateContiguousMoveExpr(key_val_id_list,
                               0,  // don't add convert nodes,
                               atp, atp_index, tf, keyLen, key_expr);
  }
...
short ExpGenerator::generateExclusionExpr(ItemExpr *expr, int atp, int atpindex, ex_expr **excl_expr) {
  ...
  if (expr) {
    ...
    generateContiguousMoveExpr(exclusionResult,
                               -1,  // add convert node
...
short ExpGenerator::generateInputExpr(const ValueIdList &val_id_list, ex_expr::exp_node_type /* node_type */,
  ...
  *expr = ioExpr;
  ...
  for (CollIndex i = 0; i < val_id_list.entries(); i++) {
    ...
    if ((item_expr->getOperatorType() == ITM_HOSTVAR) || (item_expr->getOperatorType() == ITM_DYN_PARAM)) {
      ...
      if (getShowplan())
        attr[1] = new (wHeap()) ShowplanAttributes(val_id, convertNAString(item_expr->getText(), generator->wHeap()));
      ...
      NABoolean isDynamicParam = (item_expr->getOperatorType() == ITM_DYN_PARAM ? TRUE : FALSE);
      ...
      // temporary until all issues related to returned length in chars
      // are identified and fixed
      if (isDynamicParam) {
        DynamicParam *param = (DynamicParam *)item_expr;
        // max inline len in chars
        input_clause->lobInlinedDataMaxLen() = param->lobInlinedDataMaxLen() / bpc;
    ...
    } else if ((item_expr->isAUserSuppliedInput()) ||  // evaluate once functions
               (item_expr->getOperatorType() == ITM_CURRENT_TIMESTAMP) ||
               (item_expr->getOperatorType() == ITM_UNIX_TIMESTAMP) || (item_expr->getOperatorType() == ITM_SLEEP) ||
               (item_expr->getOperatorType() == ITM_CURRENT_USER) ||
               (item_expr->getOperatorType() == ITM_CURRENT_TENANT) ||
               (item_expr->getOperatorType() == ITM_SESSION_USER) || (item_expr->getOperatorType() == ITM_EXEC_COUNT) ||
  ...
  while ((clause) && (allInoutClauses)) {
    if (clause->getType() != ex_clause::INOUT_TYPE) allInoutClauses = FALSE;

    clause = clause->getNextClause();
  }

  if (allInoutClauses) ioExpr->setNoEval(TRUE);
...
short ExpGenerator::generateListExpr(const ValueIdList &val_id_list, ex_expr::exp_node_type node_type, ex_expr **expr,
  ...
  for (CollIndex i = 0; i < vidList->entries(); i++) {
    ...
    if (getenv("GEN_ENABLE_SF_PROTECT_ALL")) itmExpr->protectiveSequenceFunctionTransformation(generator);
#endif
...
short ExpGenerator::genGuardedListExpr(const ValueIdSet guard, const ValueIdList &val_id_list,
                                       ex_expr::exp_node_type node_type, ex_expr **expr) {
  ...
  if (val_id_list.entries() == 0 && guard.entries() != 0) {
    ...
    guardedTree = new (wHeap()) BoolResult(
        new (wHeap()) ItemList(val_id_list.rebuildExprTree(ITM_ITEM_LIST), new (wHeap()) BoolVal(ITM_RETURN_TRUE)));
  } else {
    ItemExpr *trueListTree =
        new (wHeap()) ItemList(val_id_list.rebuildExprTree(ITM_ITEM_LIST), new (wHeap()) BoolVal(ITM_RETURN_TRUE));

    guardedTree =
        new (wHeap()) BoolResult(new (wHeap()) BiLogic(ITM_AND, guard.rebuildExprTree(ITM_AND), trueListTree));
  }

  guardedTree->bindNode(generator->getBindWA());

  generateExpr(guardedTree->getValueId(), ex_expr::exp_SCAN_PRED, expr);
...
short ExpGenerator::generateOutputExpr(const ValueIdList &val_id_list, ex_expr::exp_node_type /* node_type */,
  ...
  *expr = ioExpr;
  ...
  if (isCallStatement && (isOdbc || isJdbc)) empty_colname = "";
  ...
  for (CollIndex i = 0; i < val_id_list.entries(); i++) {
    ...
    const NAString *colNameFromCaller = (colNamesForExpr ? colNamesForExpr[i] : NULL);

    // colNameFromRETDesc is the column name from the RETDesc
    const NAString colNameFromRETDesc(ret_desc->getColRefNameObj(i).getColNameAsAnsiString());
    ...
    ItemExpr *outNode = new (wHeap()) NATypeToItem((NAType *)&temp->getValueId().getType());
    outNode->synthTypeAndValueId();

    Attributes **attr;
    if (genItemExpr(outNode, &attr, 1, -1) == 1) return 0;

    // if table is a hbase map table, the added column will
    // causing some unexpected errors, such as the judgment of bulk_move
    if ((((ItemExpr *)temp)->getOperatorType() == ITM_INDEXCOLUMN ||
         ((ItemExpr *)temp)->getOperatorType() == ITM_BASECOLUMN) &&
        ((ItemExpr *)temp)->getValueId().getNAColumn(TRUE) &&
        ((ItemExpr *)temp)->getValueId().getNAColumn(TRUE)->getNATable() &&
        ((ItemExpr *)temp)->getValueId().getNAColumn(TRUE)->getNATable()->isHbaseMapTable())
      attr[0]->setIsHbaseMapTableCol();

    attr[0]->copyLocationAttrs(generator->getMapInfo(((ItemExpr *)temp)->getValueId())->getAttr());

    if (genConvertNodes) {  // bulk move may cause an issue where the values are overriden
    ...
    const QualifiedName *tblNameFromCaller = (tblNamesForExpr ? tblNamesForExpr[i] : NULL);
    ...
    if (tblNamesForExpr) {
      if (tblNameFromCaller) tableNameShouldBeSet = TRUE;
    } else if (((ret_desc->getValueId(i)).getItemExpr()->getOperatorType() == ITM_BASECOLUMN) ||
               ((ret_desc->getValueId(i)).getItemExpr()->getOperatorType() == ITM_INDEXCOLUMN)) {
    ...
    if (tableNameShouldBeSet) {
      ...
      if (qaname == NULL) {
        ...
        if (column) qaname = (const QualifiedName *)column->getTableName(TRUE);
      }
...
short ExpGenerator::generateSetExpr(const ValueIdSet &val_id_set, ex_expr::exp_node_type node_type, ex_expr **expr,
                                    short gen_last_clause, ExpHdrInfo *hdrInfo) {
  ValueId val_id;
  val_id = val_id_set.init();
  if (!val_id_set.next(val_id)) return 0;  // empty set
...
//
short ExpGenerator::generateSequenceExpression(const ValueIdSet &sequenceItems, ex_expr *&expr) {
  ...
  //
  ValueId valId;
  for (valId = sequenceItems.init(); sequenceItems.next(valId); sequenceItems.advance(valId)) {
...
short ExpGenerator::generateUnionExpr(const ValueIdList &val_id_list, ex_expr::exp_node_type node_type,
                                      ex_expr **left_expr, ex_expr **right_expr) {
...
short ExpGenerator::processAttributes(int numAttrs, Attributes **attrs, ExpTupleDesc::TupleDataFormat tdataF,
                                      int &tupleLength, int atp, int atpIndex, ExpTupleDesc **tupleDesc,
                                      ExpTupleDesc::TupleDescFormat tdescF, int startOffset, ExpHdrInfo *hdrInfo,
                                      Attributes **offsets) {
  int tuppDescFlags = 0x0;

  ExpTupleDesc::computeOffsets(numAttrs, attrs, tdataF, tupleLength, startOffset, &tuppDescFlags, &tdataF, hdrInfo,
                               NULL,  // headerSizePtr
                               NULL,  // varSizePtr
                               offsets);
  ...
  if (tupleDesc) {
    *tupleDesc = new (getSpace()) ExpTupleDesc(numAttrs, attrs, tupleLength, tdataF, tdescF, getSpace());
...
short ExpGenerator::processValIdList(ValueIdList valIdList, ExpTupleDesc::TupleDataFormat tdataF, int &tupleLength,
  ...
  for (CollIndex i = 0; i < valIdList.entries(); i++) {
    ...
    if (valIdList[i].getItemExpr()->isAGenericUpdateOutputFunction() && placeGuOutputFunctionsAtEnd)
      attrs[i]->setGuOutput();
    ...
    if (alignedFormat && !isIndex && (colArray == NULL) && ((col = valIdList[i].getNAColumn(TRUE)) != NULL) &&
        col->isAddedColumn())
      attrs[i]->setAddedCol();
  }
  ...
  if (returnedAttrs) {
    ...
    *returnedAttrs = attrs;
    ...
    // deallocate the attributes array only when attrs is not used to
    // construct tupleDesc.
    //
    // Don't deallocate the attributes
    // pointed to by the array entries since we didn't allocate them.
    if (!tupleDesc || (*tupleDesc)->attrs() != (AttributesPtr *)attrs) NADELETEBASIC(attrs, wHeap());
  }
...
short ExpGenerator::computeTupleSize(const ValueIdList &valIdList, ExpTupleDesc::TupleDataFormat tdataF,
                                     int &tupleLength, int startOffset, UInt32 *varCharSize, UInt32 *headerSize) {
  MapTable *myMapTable = generator->appendAtEnd();
  UInt32 vcSize = 0;
  ...
  UInt32 rtnFlags = 0x0;
  if (varCharSize) *varCharSize = 0;

  ExpTupleDesc::computeOffsets((int)numAttrs, attrs, tdataF, tupleLength, startOffset, &rtnFlags, NULL, NULL,
                               headerSize, varCharSize);

  // deallocate the attributes array.
  NADELETEBASIC(attrs, wHeap());

  if (myMapTable != NULL) generator->unlinkMe(myMapTable);
...
short ExpGenerator::assignAtpAndAtpIndex(ValueIdList valIdList, int atp, int atpIndex) {
  for (CollIndex i = 0; i < valIdList.entries(); i++) {
    ...
    if (atpIndex >= 0) attr->setAtpIndex(atpIndex);
  }
...
void ExpGenerator::initExprGen() {
  ...
  if ((CmpCommon::getDefault(GEN_LEANER_EXPRESSIONS) == DF_ON) && !(pCodeMode_ & ex_expr::PCODE_NO_LEANER_EXPR) &&
      (pCodeMode_ & ex_expr::PCODE_ON) && !(pCodeMode_ & ex_expr::PCODE_EVAL) && (NOT getShowplan()) &&
      (NOT generator->hasCompositeExpr())) {
...
short ExpGenerator::startExprGen(ex_expr **expr, ex_expr::exp_node_type node_type) {
  if (*expr == 0) *expr = new (getSpace()) ex_expr(node_type);
  (*expr)->setLength(getSpace()->getAllocatedSpaceSize());
...
short ExpGenerator::endExprGen(ex_expr **expr, short gen_last_clause) {
  ...
  //
  char *persistentInitializationArea = placeConstants(getPersistentList(), persistentLength());
  (*expr)->persistentInitializationArea() = persistentInitializationArea;
  (*expr)->persistentLength() = persistentLength();
  (*expr)->persistentArea() = getSpace()->allocateAlignedSpace(persistentLength());

  // Set the temporary expression storage length.
  //
  if (getTempsLength() > INT_MAX) {
    *CmpCommon::diags() << DgSqlCode(-7007)
                        << DgString0("Detail: computed sum of temp storage length greater than INT_MAX");
    GenExit();
  }
  ...
  if (generator->genNoFloatValidatePCode()) ex_expr_base::setNotValidateFloat64(f, TRUE);
...
#ifdef NEW_LEAN_EXPR
  if (generator->hasCompositeExpr()) {
    // do not generate lean expressions for composite columns.
  ...
  if (tempSpace) {
    ...
    for (ex_clause *clause = (*expr)->getClauses(); clause; clause = clause->getNextClause()) {
      if ((clause->getClassID() == ex_clause::FUNC_HBASE_COLUMN_CREATE) ||
          (clause->getClassID() == ex_clause::FUNC_HBASE_COLUMNS_DISPLAY) ||
          (clause->getClassID() == ex_clause::FUNC_COMPOSITE_CONCAT) ||
          (clause->getClassID() == ex_clause::FUNC_COMPOSITE_CREATE) ||
          (clause->getClassID() == ex_clause::FUNC_COMPOSITE_DISPLAY) ||
          (clause->getClassID() == ex_clause::FUNC_COMPOSITE_EXTRACT)) {
        tooHard = TRUE;
        break;
      }
    }
    ...
    if (!tooHard && ((*expr)->getPCodeSegment() != NULL)) {
      ...
      newExpr = (ex_expr_base *)new (generator->getSpace()) char[totalExprLen];
      ...
      if ((*expr)->getPCodeSegment()->containsClauseEval() || saveClauses) {
        for (ex_clause *clause = (*expr)->getClauses(); clause; clause = clause->getNextClause()) {
          ...
          clauseSize = (clause->getClassID() == ex_clause::FUNC_RAND_SELECTION_ID) ? sizeof(ExFunctionRandomSelection)
                                                                                   : clause->getClassSize();
          ...
          if (branchFound) {
            for (ex_clause *tempClause = newExpr->getClauses(); tempClause; tempClause = tempClause->getNextClause()) {
          ...
          if (saveClauses) len1 += (afterSize - beforeSize);
      ...
      newExpr->getPCodeSegment()->replaceAttributesPtr(newExpr->getClauses(), getSpace());
      ...
      *expr = (ex_expr *)newExpr;
      ...
      newExpr = (ex_expr_base *)new (getSpace()) char[tempSpace->getAllocatedSpaceSize()];
      ...
      tempSpace->makeContiguous((char *)newExpr, tempSpace->getAllocatedSpaceSize());
  ...
  if (genLeanExpr()) {
    ...
    ex_expr_base *newExpr = NULL;
    if (((*expr)->getPCodeSegment() == NULL) || ((*expr)->getPCodeSegment()->containsClauseEval())) {
      // make full copy of expr in fragment space
      newExpr = (ex_expr_base *)new (generator->getSpace()) char[tempSpace->getAllocatedSpaceSize()];
      (*expr)->pack(tempSpace);
      tempSpace->makeContiguous((char *)newExpr, tempSpace->getAllocatedSpaceSize());
      *expr = (ex_expr *)newExpr;
      ...
      newExpr = new (generator->getSpace()) ex_expr_lean();
      (*expr)->makeLeanCopy((ex_expr_lean *)newExpr, generator->getSpace());
      *expr = (ex_expr *)newExpr;

      totalExprLen = getSpace()->getAllocatedSpaceSize() - len1;
...
void ExpGenerator::initClauseList() { clause_list = new (wHeap()) AListNode(wHeap()); };
...
void ExpGenerator::linkClause(ItemExpr *node, ex_clause *clause) {
  ...
  if ((clause->getType() == ex_clause::FUNCTION_TYPE) && node) {
    if (((ex_function_clause *)clause)->getOperType() != node->origOpType()) {
      ((ex_function_clause *)clause)->setDerivedFunction(TRUE);
      ((ex_function_clause *)clause)->setOrigFunctionOperType(node->origOpType());
    }

    if (!node->getUserTextStr().isNull()) {
      ((ex_function_clause *)clause)->setUserTextStr(node->getUserTextStr().data());
    }
  }
};
...
AListNode *ExpGenerator::getConstantList() { return constant_list; };

AListNode *ExpGenerator::getPersistentList() { return persistentList_; };

void ExpGenerator::linkConstant(void *expr_tree) { constant_list->linkElement(expr_tree); };
...
char *ExpGenerator::placeConstants(AListNode *list, int length) {
  ...
  MapTable *startMapTable = (forceLinkConstant() ? getExprMapTable() : NULL);
  ...
  while (current->getNext()) {
    item = (ConstValue *)(((ExprNode *)(current->getElement()))->castToItemExpr());

    if (!generator->getMapInfoAsIs(item->getValueId(), startMapTable)) {
      *CmpCommon::diags() << DgSqlCode(-7007) << DgString0("Detail: ValueId must exist in map table");
      GenExit();
    }

    Attributes *attr = generator->getMapInfo(item->getValueId(), startMapTable)->getAttr();

    if ((attr->getNullFlag()) && (item->isNull()))  // null constant
    {
      ExpTupleDesc::setNullValue(area + attr->getNullIndOffset(), attr->getNullBitIndex(), attr->getTupleFormat());
    } else {
      if (attr->getNullFlag()) {
        ExpTupleDesc::clearNullValue(area + attr->getNullIndOffset(), attr->getNullBitIndex(), attr->getTupleFormat());
      }

      if (attr->getVCIndicatorLength() > 0) {
        str_cpy_all(&area[attr->getVCLenIndOffset()],
                    (char *)item->getConstValue() + item->getValueId().getType().getSQLnullHdrSize(),
                    attr->getVCIndicatorLength());
      }

      str_cpy_all(&area[attr->getOffset()],
                  (char *)item->getConstValue() + item->getValueId().getType().getSQLnullHdrSize() +
                      attr->getVCIndicatorLength(),
                  attr->getLength());
    }
...
ex_expr::exp_return_type ExpGenerator::genEvalPredicate(ItemExpr *rootPtr, ComDiagsArea *diagsArea) {
  ...
  // we use these expressions only once, therefore don't generate PCode
  expGen.setPCodeMode(ex_expr::PCODE_NONE);

  expGen.generateExpr(rootPtr->getValueId(), ex_expr::exp_SCAN_PRED, &expr);

  // create a DP2 expression and initialize it with the key encode expr.
  ExpDP2Expr *dp2Expr = new (space) ExpDP2Expr(expr, workCriDesc, space);

  dp2Expr->getExpr()->fixup(0, 0, 0, space, bindWA.wHeap(), FALSE, NULL);
...
short ExpGenerator::generateSamplingExpr(const ValueId &valId, ex_expr **expr, int &returnFactorOffset) {
  ...
  GenAssert(attr->getAtpIndex() == 1, "Sampling result must be a persistent expression variable.");
...
MapInfo *ExpGenerator::addTemporary(ValueId val, MapTable *mapTable) {
  ...
  //
  MapInfo *mapInfo = generator->addMapInfoToThis(generator->getLastMapTable(), val, NULL);
  ...
  /*
  if ((DFS2REC::isComposite(mapAttr->getDatatype())) &&
      (CmpCommon::getDefaultString(TRAF_COMPOSITE_DATATYPE_FORMAT) == "ALIGNED2"))
    tdataF = ExpTupleDesc::SQLMX_ALIGNED_FORMAT;
  */
  ExpTupleDesc::computeOffsets(mapAttr, tdataF, len, getTempsLength());
  addTempsLength(len);
  return mapInfo;
}
...
MapInfo *ExpGenerator::addPersistent(ValueId val, MapTable *mapTable) {
  ...
  int len;
  ExpTupleDesc::TupleDataFormat tdataF = ExpTupleDesc::SQLARK_EXPLODED_FORMAT;
  ExpTupleDesc::computeOffsets(mapAttr, tdataF, len, persistentLength());
...
/////////////////////////////////////////////////////////////////////////////
{
  ...
  if ((map_info) && (map_info->isCodeGenerated())) return 1;
  ...
  } else if (map_info->getAttr()->getAtpIndex() == 1) {
    int len = map_info->getAttr()->getLength();
    addTempsLength(len);
  }
  ...
  if (getShowplan()) {
    attr[0 + num_attrs] = new (wHeap())
        ShowplanAttributes(item_expr->getValueId(), convertNAString(item_expr->getText(), generator->wHeap()));

    attr[0]->setShowplan();

    for (short i = 0; i < num_attrs - 1; i++) {
      attr[i + 1 + num_attrs] = new (wHeap()) ShowplanAttributes(
          item_expr->child(i)->getValueId(), convertNAString(item_expr->child(i)->getText(), generator->wHeap()));
    }
  }
...
#ifdef _DEBUG
  if (getenv("NO_COMMON_SUBEX_ELIM")) enableCommonSubexpressionElimination_ = FALSE;
#endif
}
...
ItemExpr *ExpGeneratorCache::findEquivalentGeneratedExpr(ItemExpr *ie) {
  ...
  if ((ie) && (enableCommonSubexpressionElimination_)) {
    switch (ie->getOperatorType()) {
      case ITM_CAST: {
        for (CollIndex i = 0; (i < cachedCasts_.entries()) && (rc == 0); i++) {
          Cast *c = cachedCasts_[i];
          if (c->isEquivalentForCodeGeneration(ie)) rc = c;
        }
        break;
      }
      case ITM_PLUS:
      ...
      case ITM_DIVIDE: {
        for (CollIndex i = 0; (i < cachedBiAriths_.entries()) && (rc == 0); i++) {
          ...
          if (b->isEquivalentForCodeGeneration(ie)) rc = b;
        }
        break;
      }
      default:
...
NABoolean ExpGeneratorCache::expressionCanBeEliminated(ItemExpr *ie) {
  ...
  if (equiv_expr) {
    ...
    substitutions_.insert(new (generator_->wHeap()) ExpGeneratorCacheSubstitution(ie, temp2));
...
void ExpGeneratorCache::incrementLevel(void) { level_++; }
...
int ExpGenerator::foldConstants(ItemExpr *inExpr, ItemExpr **outExpr) {
  ...
  rc = ValueIdList::evaluateExpr(dummy, inExpr->getValueId(), -1,
                                 ...
                                 outExpr, CmpCommon::diags());
  if ((rc < 0) || (CmpCommon::diags()->getNumber() > 0)) {
    if (outExpr) *outExpr = NULL;
...
/////////////////////////////////////////////////////////////////////////////
ExpGeneratorCacheSubstitution::ExpGeneratorCacheSubstitution(ItemExpr *ie, ValueId &newValueId)
...
void ExpGeneratorCacheSubstitution::undo(void) {
  ...
  if (ie_->getValueId() == newValueId_) ie_->setValueId(oldValueId_);
}
