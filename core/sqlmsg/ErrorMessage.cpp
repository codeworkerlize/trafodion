  ...
  if (errcb->getErrParamCount() > 0) {
    ...
    for (int i = 0; i < msgBufOrigLen; i++) {
      if (msgBuf_[i] == ERRORPARAM_BEGINMARK) {
        ...
        NAWchar *p = paramName;
        while (++i < msgBufOrigLen && (isalnum(msgBuf_[i]) || msgBuf_[i] == ERRORPARAM_TYPESEP)) *p++ = msgBuf_[i];
        *p = NAWchar('\0');
        ...
        if (paramPos >= 0) {
          ...
          if (param) switch (param->getNAErrorParamType()) {
    ...
    if (truncation) tmp[tmpLen - 3] = tmp[tmpLen - 2] = tmp[tmpLen - 1] = NAWchar('.');
...
void ErrorMessage::printErrorMessage(NAError *errcb) {
  ...
  NAErrorCode erc_signed = (errcb->getErrType() == NAError::NAERROR_WARNING) ? erc_abs : -erc_abs;

  NABoolean msgNotFound = GetErrorMessage(MAIN_ERROR, erc_signed, tmp);
  ...
  */
  ...
  if (forceParamSubst) {
    ...
    // dbg: NAWstrcat(msgBuf_, WIDE_(" $ $$ $ab $9 $~ $~~ $~0 $0~ $~a $a~ $0x $0x~int0 $int0~x # $0~int0 $int0~0 $0 $00
    // $0$0")); dbg: NAWstrcat(msgBuf_, WIDE_(" $Int0~0$int0~1 $0~Int0$1~int0 #")); dbg: NAWstrcat(msgBuf_, WIDE_("
    // $Int0~0$int0~0 $0~Int0$0~int0 #")); dbg: NAWstrcat(msgBuf_, WIDE_(" $Int0~0$0~int0 $0~Int0$int0~0 #"));
  }
  ...
  for (int i = 0; i++ < numDiags;) {
    ...
    if (verbose || (!verbose) && (*diags)[i].getSQLCODE() <= 0) {
      ...
      if (fp)  // if a logfile is open, mirror messages to it
      {
        ...
        UnicodeStringToLocale(terminal_cs, msg, NAWstrlen(msg), mbstr, DEST_BUF_SIZE);
...
void NAWriteConsole(const char *str, ostream &outStream, NABoolean newline, NABoolean comment) {
  if (!str) return;
...
void NAWriteConsole(const NAWchar *str, ostream &outStream, NABoolean newline, NABoolean comment,
                    CharInfo::CharSet terminal_cs) {
  if (!str) return;
  char mbstr[DEST_BUF_SIZE + 16];
