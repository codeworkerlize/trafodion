...
ExSqlComp::ReturnStatus ExSqlComp::changePriority(IpcPriority priority, NABoolean isDelta) {
  ReturnStatus ret = SUCCESS;
  if (!server_) return ret;
...
ExSqlComp::ReturnStatus ExSqlComp::createServer() {
  ...
  if (!(sc_ = new (h_) IpcServerClass(env_, IPC_SQLCOMP_SERVER, allocMethod_, compilerVersion_, nodeName_))) {
    //
    *diagArea_ << DgSqlCode(-CLI_OUT_OF_MEMORY) << DgString0("IpcServerClass");
    ret = ERROR;
    ...
    if (cliGlobals_->currContext()->getSessionDefaults()->getMxcmpPriority() > 0)
      priority = cliGlobals_->currContext()->getSessionDefaults()->getMxcmpPriority();
    else if (cliGlobals_->currContext()->getSessionDefaults()->getMxcmpPriorityDelta() != 0)
      priority =
          //	  env_->getMyProcessPriority() +
          cliGlobals_->myPriority() + cliGlobals_->currContext()->getSessionDefaults()->getMxcmpPriorityDelta();
    if ((priority > 200) || (priority < 1)) priority = IPC_PRIORITY_DONT_CARE;

    ComDiagsArea *diags = 0;
    NAWNodeSetWrapper nodeSet(cliGlobals_->currContext()->getAvailableNodes(), cliGlobals_->getNAClusterInfo());
    if (!(server_ = sc_->allocateServerProcess(&diags, h_, nodeName_, IPC_CPU_DONT_CARE, priority, 1, TRUE, TRUE, 2,
                                               NULL, NULL, FALSE, NULL, &nodeSet)))
      ret = ERROR;
    if (diags) {
  ...
  if (ret == ERROR) {
    ...
    if (getenv("DEBUG_SERVER")) MessageBox(NULL, "ExSqlComp:createServer", "error ", MB_OK | MB_ICONINFORMATION);
  }
...
ExSqlComp::ReturnStatus ExSqlComp::establishConnection() {
  ...
  if (!(sqlcompMessage_ = new (h_) CmpMessageStream(env_, this))) {
    *diagArea_ << DgSqlCode(-CLI_OUT_OF_MEMORY) << DgString0("CmpMessageStreaam");
    ret = ERROR;
  } else {
    ...
    if (badConnection_) ret = ERROR;
  }
  //
  if (ret == ERROR) error(arkcmpErrorConnection);
...
ExSqlComp::ReturnStatus ExSqlComp::startSqlcomp(void) {
  ...
  if ((ret = createServer()) == ERROR) return ret;
  if ((ret = establishConnection()) == ERROR) return ret;

  if ((ret = resendControls()) == ERROR) return ret;

  // on NT, the environment is not shipped to mxcmp when the process
  // is created as an NSK lite process.
  // Send it now.
  if ((ret = refreshEnvs()) == ERROR) return ret;
...
ExSqlComp::ReturnStatus ExSqlComp::resendRequest() {
  ...
  IpcMessageType typ =
      outstandingSendBuffers_.message_ ? outstandingSendBuffers_.message_->getType() : CmpMessageObj::NULL_REQUEST;
...
#endif
      sqlCode || outstandingSendBuffers_.resendCount_ >= 1 || currentISPRequest_ ||  // an ISP request
      typ == CmpMessageObj::INTERNALSP_REQUEST || typ == CmpMessageObj::INTERNALSP_GETNEXT) {
  ...
  // restore request.
  outstandingSendBuffers_ = tempRequests;
  if (outstandingSendBuffers_.message_) ret = sendR(outstandingSendBuffers_.message_, outstandingSendBuffers_.waited_);

  return ret;
}

inline ExSqlComp::ReturnStatus ExSqlComp::sendR(CmpMessageObj *c, NABoolean w) {
  ReturnStatus ret;
  ...
  if (badConnection_) return ERROR;
  ...
  // can still be run).
  ContextCli::ArkcmpFailMode fm = cliGlobals_->currContext()->arkcmpInitFailed(indexIntoCliCompilerArray);
...
// and the CLI.
inline ExSqlComp::ReturnStatus ExSqlComp::waitForReply() {
...
static NABoolean pairLenTxt(int &len, const char *&txt, const char *&cqd) {
  ...
  for (int i = 3; i--; cqd++) {
    ...
    if (isDigit8859_1((unsigned char)*cqd))
      len = (10 * len) + *cqd - '0';
    else if (isSpace8859_1((unsigned char)*cqd))
      len = (10 * len);
    else
      return TRUE;  // error
  }
...
void ExSqlComp::appendControls(ExControlArea *dest, ExControlArea *src) {
  ...
  for (srcList->position(); ctl = (ExControlEntry *)srcList->getNext();) {
    dest->addControl(ctl->type(), ctl->getReset(), ctl->getSqlText(), ctl->getSqlTextLen(), ctl->getValue(1),
                     ctl->getLen(1), ctl->getValue(2), ctl->getLen(2), ctl->getValue(3), ctl->getLen(3),
                     ctl->getActionType(), ctl->getResendType(), ctl->isNonResettable());
  }
}
//
static ExSqlComp::ReturnStatus saveControls(ExControlArea *ca, const char *cqd) {
  ...
  ca->addControl(DEFAULT_, 2, sqlText, lenP, 0, 0, 0, 0, 0, 0, ComTdbControl::NONE_,
...
ExSqlComp::ReturnStatus ExSqlComp::loadQueryCache() {
  ...
  ReturnStatus ret = sendRequest(EXSQLCOMP::PROCESSDDL, data, dataLen);
...
ExSqlComp::ReturnStatus ExSqlComp::resetAllDefaults() {
  ...
  for (int i = 0; i < sizeof(buf) / sizeof(char *); i++) {
    ...
    ReturnStatus ret = sendRequest(EXSQLCOMP::SQLTEXT_STATIC_COMPILE, data, dataLen);
...
ExSqlComp::ReturnStatus ExSqlComp::resendControls(NABoolean ctxSw)  // Genesis 10-981211-5986
{
  ...
  if (resendingControls_) return SUCCESS;
  ...
  if (ctxt->arkcmpInitFailed(indexIntoCliCompilerArray)) {
    ctxt->arkcmpInitFailed(indexIntoCliCompilerArray) = ContextCli::arkcmpERROR_;
    error(-CLI_ARKCMP_INIT_FAILED);
  ...
  if (ret != ERROR) {
    ...
    if (ctxt->getAvailableNodes()) ctxt->getAvailableNodes()->serialize(serializedNodes);

    char userMessage[2 * MAX_AUTHID_AS_STRING_LEN + 2 + 2 * MAX_USERNAME_LEN + 2 + serializedNodes.size() + 2 + 12 +
                     COL_MAX_SCHEMA_LEN + 1];
    int bytesWritten = snprintf(userMessage, sizeof(userMessage), "%d,%d,%s,%d,%s,%s,%s", authOn, userAsInt,
                                ctxt->getDatabaseUserName(), ctxt->getTenantID(), ctxt->getTenantName(),
                                serializedNodes.c_str(), ctxt->getTenantDefaultSchema());
    assert(bytesWritten > 0 && bytesWritten < sizeof(userMessage));
    ...
    if (q->isEmpty()) {
      ...
      if (sharedCtrlList->isEmpty()) {
        ...
        if (ret != ERROR && ABS(mainSqlcode) == ABS(EXE_INFO_CQD_NAME_VALUE_PAIRS)) {
          ...
          if (ret != ERROR) diagArea_->clear();
        }
        if ((ret == ERROR) && (!breakReceived_)) {
          ctxt->arkcmpInitFailed(indexIntoCliCompilerArray) = ContextCli::arkcmpWARN_;
          error(+CLI_ARKCMP_INIT_FAILED);
        }
      ...
      if (ret != ERROR) {
        ...
        ReturnStatus ret = sendRequest(EXSQLCOMP::SET_TRANS, (char *)ta->getUserTransMode(), sizeof(TransMode));

        if (ret != ERROR)
          if (waitForReply() == ERROR) ret = ERROR;
      }
      ...
      for (q->position(); ctl = (ExControlEntry *)q->getNext();) {
        ...
        cerr << "resend " << ctl->getSqlTextLen() << "\t" << ctl->getSqlText() << endl;
#endif
#ifdef _DEBUG
        if (getenv("SQLCOMP_DEBUG")) cerr << ctl->getSqlTextLen() << "\t" << ctl->getSqlText() << endl;
#endif
        ...
        if (ret != ERROR) {
          //
          if (((*diagArea_).contains(-2050) || (*diagArea_).contains(-2055)) &&
              (compilerVersion_ < COM_VERS_COMPILER_VERSION)) {
            diagArea_->clear();
          } else {
            if (diagArea_->getNumber() > 0) {
              if (loopDiags == NULL) loopDiags = ComDiagsArea::allocate(h_);
  ...
  if (ret != SUCCESS || diagArea_->getNumber() || loopDiags != NULL) {
    ...
    if (ret != ERROR) ret = diagArea_->getNumber(DgSqlCode::ERROR_) ? ERROR : WARNING;
    if (ret == ERROR) *diagArea_ << DgSqlCode(-CLI_SEND_REQUEST_ERROR) << DgString0("resendControls");
  }
...
ExSqlComp::ReturnStatus ExSqlComp::refreshEnvs() {
  ...
  CmpMessageEnvs envs(CmpMessageEnvs::EXGLOBALS, cliGlobals_->getEnvVars(), FALSE, getHeap());
  ...
  if (ret == ERROR) *diagArea_ << DgSqlCode(-CLI_SEND_REQUEST_ERROR) << DgString0("SETENV");
...
ExSqlComp::ExSqlComp(void *ex_environment, CollHeap *h, CliGlobals *cliGlobals, ExStoredProcTcb *storedProcTcb,
                     short version, char *nodeName, IpcEnvironment *env)
...
ExSqlComp::~ExSqlComp() {
  ...
  if (diagArea_) diagArea_->deAllocate();
}
//
...
ExSqlComp::ReturnStatus ExSqlComp::preSendRequest(NABoolean doRefreshEnvs) {
  ...
  if (doRefreshEnvs)
    ...
    if (!sqlcompMessage_) ret = startSqlcomp();
  }

  return ret;
}

ExSqlComp::ReturnStatus ExSqlComp::sendRequest(CmpMessageObj *request, NABoolean waited) {
  ReturnStatus ret = SUCCESS;
...
ExSqlComp::ReturnStatus ExSqlComp::sendRequest(Operator op, const char *const_input_data, int size, NABoolean waited,
  ...
  switch (op) {
    ...
    case EXSQLCOMP::SQLTEXT_COMPILE:  // we might be static or dynamic mode
      request = new (h_) CmpMessageSQLText(input_data, (CmpMsgBufLenType)size, h_, charset, op);

      break;

    case EXSQLCOMP::SQLTEXT_STATIC_RECOMPILE:
    case EXSQLCOMP::SQLTEXT_STATIC_COMPILE:  // force to static compilation mode
      request = new (h_) CmpMessageCompileStmt(input_data, (CmpMsgBufLenType)size, op, h_, charset);
      break;

    case EXSQLCOMP::UPDATE_HIST_STAT:
      request = new (h_) CmpMessageUpdateHist(input_data, (CmpMsgBufLenType)size, h_, charset);
      break;

    case EXSQLCOMP::PROCESSDDL:
      request = new (h_) CmpMessageDDL(input_data, (CmpMsgBufLenType)size, h_, charset, parentQid, parentQidLen);
      break;

    case EXSQLCOMP::DESCRIBE:
      request = new (h_) CmpMessageDescribe(input_data, (CmpMsgBufLenType)size, h_, charset);
      break;

    case EXSQLCOMP::XN_OPER:
      request = new (h_) CmpMessageXnOper(input_data, (CmpMsgBufLenType)size, h_);
      break;

    case EXSQLCOMP::SET_TRANS:
      request = new (h_) CmpMessageSetTrans(input_data, (CmpMsgBufLenType)size, h_);
      break;

    case EXSQLCOMP::DDL_NATABLE_INVALIDATE:
      request = new (h_) CmpMessageDDLNATableInvalidate(input_data, (CmpMsgBufLenType)size, h_);
      break;

    case EXSQLCOMP::DDL_COMMIT_SHARED_CACHE:
      request = new (h_) CmpMessageDDLCommitSharedCache(input_data, (CmpMsgBufLenType)size, h_);
      break;

    case EXSQLCOMP::DDL_ROLLBACK_SHARED_CACHE:
      request = new (h_) CmpMessageDDLRollbackSharedCache(input_data, (CmpMsgBufLenType)size, h_);
      break;

    case EXSQLCOMP::DATABASE_USER:
      request = new (h_) CmpMessageDatabaseUser(input_data, (CmpMsgBufLenType)size, h_);
      break;

    case EXSQLCOMP::END_SESSION:
      request = new (h_) CmpMessageEndSession(input_data, (CmpMsgBufLenType)size, h_);
      break;

    case EXSQLCOMP::DDL_WITH_STATUS:
      request = new (h_) CmpMessageDDLwithStatus(input_data, (CmpMsgBufLenType)size, h_);
      break;
...
ExSqlComp::ReturnStatus ExSqlComp::sendRequest(const char *procName, void *inputExpr,
  ...
  CmpMessageObj *request = new (h_)
      CmpMessageISPRequest((char *)procName,  //## for now, cast away constness...
                           inputExpr, inputExprSize, outputExpr, outputExprSize, keyExpr, keyExprSize, inputData,
                           inputDataSize, outputRecSize, outputTotalSize, h_, parentQid, parentQidLen);
...
ExSqlComp::ReturnStatus ExSqlComp::getNext(int bufSize, long id, NABoolean waited, const char *parentQid,
  ...
  clearDiags();
  CmpMessageObj *request = new (h_) CmpMessageISPGetNext(bufSize, ispRequest, 0, h_, parentQid, parentQidLen);
...
ExSqlComp::ReturnStatus ExSqlComp::getReply(char *&reply, int &size, int maxSize, long reqId,
  ...
  switch (sqlcompMessage_->getNextObjType()) {
    ...
    case CmpMessageObj::REPLY_ISP: {
      ...
      if (retval_ == SUCCESS) currentISPRequest_ = 0;
      break;
    }
    default:
      break;
...
CmpMessageStream::CmpMessageStream(IpcEnvironment *env, ExSqlComp *sqlcomp)
    : IpcMessageStream(env, CmpMessageObj::EXE_CMP_MESSAGE, EXECMPIPCVERSION, 0, TRUE) {
  sqlcomp_ = sqlcomp;
  waited_ = TRUE;
}
...
void CmpMessageStream::actOnReceive(IpcConnection *) {
  if ((getState() == ERROR_STATE) || (getState() == BREAK_RECEIVED)) {
    ...
    if (getState() == BREAK_RECEIVED) sqlcomp_->breakReceived_ = TRUE;
    ...
    if (sqlcomp_->currentISPRequest_ ||  // an ISP request
        typ == CmpMessageObj::INTERNALSP_REQUEST || typ == CmpMessageObj::INTERNALSP_GETNEXT) {
      if (getState() == ERROR_STATE) sqlcomp_->error(arkcmpErrorResend);
    ...
    if ((getState() != BREAK_RECEIVED) && sqlcomp_->getDiags() && sqlcomp_->getDiags()->getNumber() == 0) {
    ...
    if (sqlcomp_->outstandingSendBuffers_.message_) {
      ...
      sqlcomp_->outstandingSendBuffers_.requestId_ = sqlcomp_->outstandingSendBuffers_.message_->id();
