...
static int getIntervalCodeLocal(short datatype) {
  switch (datatype) {
    ...
    case REC_INT_MONTH:
      return SQLINTCODE_MONTH;
    case REC_INT_YEAR_MONTH:
      return SQLINTCODE_YEAR_MONTH;
    case REC_INT_DAY:
      return SQLINTCODE_DAY;
    case REC_INT_HOUR:
      return SQLINTCODE_HOUR;
    case REC_INT_DAY_HOUR:
      return SQLINTCODE_DAY_HOUR;
    case REC_INT_MINUTE:
      return SQLINTCODE_MINUTE;
    case REC_INT_HOUR_MINUTE:
      return SQLINTCODE_HOUR_MINUTE;
    case REC_INT_DAY_MINUTE:
      return SQLINTCODE_DAY_MINUTE;
    case REC_INT_SECOND:
      return SQLINTCODE_SECOND;
    case REC_INT_MINUTE_SECOND:
      return SQLINTCODE_MINUTE_SECOND;
    case REC_INT_HOUR_SECOND:
      return SQLINTCODE_HOUR_SECOND;
...
ex_expr::exp_return_type InputOutputExpr::describeOutput(void *output_desc_, UInt32 flags) {
  ...
  while (clause) {
    if (clause->getType() == ex_clause::INOUT_TYPE) {
      ex_inout_clause *inout_clause = (ex_inout_clause *)clause;

      Attributes *operand = clause->getOperand(0);
      if (isCall() && output_desc->isDescTypeWide())
        entry = inout_clause->getParamIdx();
      else
        entry++;

      output_desc->setDescItem(entry, SQLDESC_TYPE_FS, operand->getDatatype(), 0);

      length = operand->getLength();

      if ((operand->getDatatype() >= REC_MIN_INTERVAL) && (operand->getDatatype() <= REC_MAX_INTERVAL)) {
        //
        ...
        //
        output_desc->setDescItem(entry, SQLDESC_DATETIME_CODE, getIntervalCodeLocal(operand->getDatatype()), 0);
        output_desc->setDescItem(entry, SQLDESC_PRECISION, operand->getScale(), 0);
        ...
        // PRECISION = fractional precision
        //
        output_desc->setDescItem(entry, SQLDESC_DATETIME_CODE, operand->getPrecision(), 0);
        output_desc->setDescItem(entry, SQLDESC_PRECISION, operand->getScale(), 0);
        output_desc->setDescItem(entry, SQLDESC_SCALE, 0, 0);
      ...
      output_desc->setDescItem(entry, SQLDESC_NAME, 0, ((ex_inout_clause *)clause)->getName());
      if (((ex_inout_clause *)clause)->getHeading() != 0)
        output_desc->setDescItem(entry, SQLDESC_HEADING, 0, ((ex_inout_clause *)clause)->getHeading());

      if (((ex_inout_clause *)clause)->getTableName() != 0)
        output_desc->setDescItem(entry, SQLDESC_TABLE_NAME, 0, ((ex_inout_clause *)clause)->getTableName());

      if (((ex_inout_clause *)clause)->getSchemaName() != 0)
        output_desc->setDescItem(entry, SQLDESC_SCHEMA_NAME, 0, ((ex_inout_clause *)clause)->getSchemaName());
      if (((ex_inout_clause *)clause)->getCatalogName() != 0)
        output_desc->setDescItem(entry, SQLDESC_CATALOG_NAME, 0, ((ex_inout_clause *)clause)->getCatalogName());
      ...
      if (operand->getRowsetSize() > 0) {
        output_desc->setDescItem(0, SQLDESC_ROWSET_SIZE, operand->getRowsetSize(), 0);
        output_desc->setDescItem(entry, SQLDESC_ROWSET_VAR_LAYOUT_SIZE, operand->getLength(), 0);
        output_desc->setDescItem(entry, SQLDESC_ROWSET_IND_LAYOUT_SIZE, operand->getNullIndicatorLength(), 0);
      } else {
        output_desc->setDescItem(0, SQLDESC_ROWSET_SIZE, 0, 0);
        output_desc->setDescItem(entry, SQLDESC_ROWSET_VAR_LAYOUT_SIZE, 0, 0);
        output_desc->setDescItem(entry, SQLDESC_ROWSET_IND_LAYOUT_SIZE, 0, 0);
      }
      ...
      BriefAssertion((paramMode == PARAMETER_MODE_INOUT) || (paramMode == PARAMETER_MODE_OUT) ||
      ...
      if (isCall()) {
        ...
        output_desc->setDescItem(entry, SQLDESC_PARAMETER_MODE, PARAMETER_MODE_OUT, 0);
        output_desc->setDescItem(entry, SQLDESC_PARAMETER_INDEX, entry, 0);
        output_desc->setDescItem(entry, SQLDESC_ORDINAL_POSITION, 0, 0);
      }
      ...
      // in the descriptor.
      //
      output_desc->setDescItem(entry, SQLDESC_IND_LENGTH, operand->getNullIndicatorLength(), 0);
      output_desc->setDescItem(entry, SQLDESC_VC_IND_LENGTH, operand->getVCIndicatorLength(), 0);
      ...
      output_desc->setDescItemInternal(entry, SQLDESC_DATA_OFFSET, dataOffset, 0);
      output_desc->setDescItemInternal(entry, SQLDESC_NULL_IND_OFFSET, nullIndOffset, 0);

      output_desc->setDescItemInternal(entry - 1, SQLDESC_ALIGNED_LENGTH, currEntryStartOffset - prevEntryStartOffset,
                                       0);
      ...
      // the other tuples.
      //
      if (tempAtp == -1) tempAtp = operand->getAtpIndex();
...
// End testing logic  *****
  ...
  if (firstEntryProcessed) {
    ...
    output_desc->setDescItemInternal(entry, SQLDESC_ALIGNED_LENGTH, currAlignedOffset - prevEntryStartOffset, 0);
  }

  if (!(isCall() && output_desc->isDescTypeWide()) || (entry > output_desc->getUsedEntryCount()))
...
//
Descriptor::BulkMoveStatus Descriptor::checkBulkMoveStatusV1(short entry, Attributes *op, Long &varPtr,
                                                             NABoolean isInputDesc, NABoolean isRWRS,
  ...
  if ((rowsetSize > 0) ||
      //      ((NOT rowwiseRowset()) &&
      (descItem.var_ptr == 0) || (op->getVCIndicatorLength() > 0) || (DFS2REC::isAnyVarChar(op->getDatatype())) ||
      (((DFS2REC::isDateTime(op->getDatatype())) || (DFS2REC::isInterval(op->getDatatype()))) &&
       (NOT isInternalFormatIO)) ||
      (op->isAddedCol()) || (!op->isBulkMoveable()))
    return BULK_MOVE_OFF;
  ...
  // character limits.
           ...
           ((NOT DFS2REC::isAnyCharacter(op->getDatatype())) && (descItem.scale != op->getScale())) ||
           (descItem.length != op->getLength()) ||
           ((descItem.datatype >= REC_MIN_BINARY_NUMERIC) && (descItem.datatype <= REC_MAX_BINARY_NUMERIC) &&
            (((isInputDesc) && (descItem.precision > 0)) ||
             ((NOT isInputDesc) && (descItem.precision > 0) &&
              ((op->getPrecision() == 0) || (op->getPrecision() > descItem.precision))))) ||
           (op->getNullFlag()) || (descItem.nullable != 0))
    return BULK_MOVE_DISALLOWED;
...
// items bulk move start address.
//
Descriptor::BulkMoveStatus Descriptor::checkBulkMoveStatusV2(short entry, Attributes *op, Long &varPtr,
                                                             NABoolean isInputDesc, NABoolean isOdbc, NABoolean isRWRS,
                                                             NABoolean isInternalFormatIO) {
#ifdef _DEBUG
  //  if (! getenv("DOSLOWBULKMOVE"))
  //    return BULK_MOVE_OFF;
  ...
  // if any of the first set of conditions is true, bulk move cannot
  // be done for any value of this row. Turn off bulk move completely.
  if ((NOT isRWRS) && ((rowsetSize > 0) || (descItem.var_ptr == 0) || (op->isAddedCol()))) return BULK_MOVE_OFF;
  ...
  //
      ...
      || ((NOT DFS2REC::isAnyCharacter(op->getDatatype())) && (NOT DFS2REC::isDateTime(op->getDatatype())) &&
          (NOT DFS2REC::isInterval(op->getDatatype())) && (descItem.scale != op->getScale()))
      ...
      //   precision is zero.
      || ((descItem.datatype >= REC_MIN_BINARY_NUMERIC) && (descItem.datatype <= REC_MAX_BINARY_NUMERIC) &&
          (((isInputDesc) && (NOT isRWRS) && (descItem.precision > 0)) ||
           ((NOT isInputDesc) && (descItem.precision > 0) &&
            ((op->getPrecision() == 0) || (op->getPrecision() > descItem.precision)))))
  ...
  if (descItem.nullable != 0) {
    ...
    if (opIndOffset != descIndOffset) {
      ...
      // correctly aligned.
      // BriefAssertion((isOdbc == FALSE) , "Indicator/var pointer alignment problem for rowwise rowset type 2 with
      // MXCS");
      if ((NOT isRWRS) || (NOT isOdbc)) return BULK_MOVE_DISALLOWED;
    }
  }
...
void InputOutputExpr::setupBulkMoveInfo(void *desc_, CollHeap *heap, NABoolean isInputDesc, UInt32 flags) {
  NABoolean isOdbc = isODBC(flags);
  NABoolean isDbtr = isDBTR(flags);
  NABoolean isRwrs = isRWRS(flags);
  NABoolean isIFIO = (isInternalFormatIO(flags) || (isDbtr && isRwrs));
  ...
  if (desc->bulkMoveDisabled()) return;
  ...
  while (clause) {
    if (clause->getType() == ex_clause::INOUT_TYPE) {
      ...
      if (entry == 1) firstDescStartPtr = desc->getRowsetVarPtr(entry);

      Descriptor::BulkMoveStatus bms =
          desc->checkBulkMoveStatus(entry, op, descVarPtr, isInputDesc, isOdbc, isRwrs, isIFIO);

      if (bms == Descriptor::BULK_MOVE_OFF) {  // It is not possible to do a bulk move
      ...
      if (op->getDatatype() >= REC_MIN_V_CHAR_H && op->getDatatype() <= REC_NCHAR_V_UNICODE) opIsVarchar = TRUE;
      ...
      if ((isInputDesc) && (isOdbc) && (isRwrs)) {
        ...
        if (firstDescStartPtr < 0) firstDescStartPtr = descVarPtr;
      }
      ...
      } else if ((op->getAtpIndex() != currAtpIndex) || (opOffset < currExeStartOffset) ||
                 (descVarPtr < currDescStartPtr) ||
                 ((descVarPtr - currDescStartPtr) != (opOffset - currExeStartOffset)) ||
                 (!isInputDesc && (opIsVarchar || currIsVarchar)))
      // (descVarPtr - currDescStartPtr) != currLength)
      {
        desc->bulkMoveInfo()->addEntry(
            currLength,
            (((desc->rowwiseRowset()) && (NOT desc->rowwiseRowsetDisabled())) ?
            ...
            currAtpIndex, (currAtpIndex <= 1 ? TRUE : FALSE), currExeStartOffset, currFirstEntryNum,
            (short)(bmEntry - 1), currIsVarchar, currIsNullable);
        ...
        if (op->getNullFlag() != 0)
          currLength = opOffset - currExeStartOffset + (op->getOffset() - op->getNullIndOffset()) + op->getLength();
        else if (opIsVarchar)
  ...
  if ((desc->bulkMoveInfo()) && (bmEntry > 0))  // found at least one entry which does bulk move.
    desc->bulkMoveInfo()->addEntry(
        currLength,
        (((desc->rowwiseRowset()) && (NOT desc->rowwiseRowsetDisabled())) ?
             ...
             (char *)(currDescStartPtr - firstDescStartPtr)
                                                                          ...
                                                                          (char *)currDescStartPtr),
        currAtpIndex, (currAtpIndex <= 1 ? TRUE : FALSE), currExeStartOffset, currFirstEntryNum, bmEntry, currIsVarchar,
        currIsNullable);

  desc->setBulkMoveSetup(TRUE);

  if (getenv("BULKMOVE") && getenv("BULKMOVEINFO") && desc->bulkMoveInfo()) {
    if (isInputDesc)
      cout << "InputDesc, Used Entries " << desc->bulkMoveInfo()->usedEntries() << endl;
    else
      cout << "OutputDesc, Used Entries " << desc->bulkMoveInfo()->usedEntries() << endl;

    for (UInt32 i = 0; i < desc->bulkMoveInfo()->usedEntries(); i++) {
      cout << "Entry #" << i + 1 << endl;
      cout << "desc ptr = " << desc->bulkMoveInfo()->getDescDataPtr(i)
           << ", exe offset = " << desc->bulkMoveInfo()->getExeOffset(i)
           << ", length = " << desc->bulkMoveInfo()->getLength(i)
           << ", atpindex = " << desc->bulkMoveInfo()->getExeAtpIndex(i) << endl;
      cout << "firstEntryNum = " << desc->bulkMoveInfo()->getFirstEntryNum(i)
           << ", lastEntryNum = " << desc->bulkMoveInfo()->getLastEntryNum(i) << endl;
      cout << endl;
    }
  }
}

ex_expr::exp_return_type InputOutputExpr::doBulkMove(atp_struct *atp, void *desc_, char *tgtRowPtr,
  ...
  for (int i = 0; i < (int)bmi->usedEntries(); i++) {
    ...
    if (NOT bmi->isExeDataPtr(i)) {
      ...
      if ((isInputDesc) && (tgtRowPtr))
        ...
        dataPtr = atp->getTupp(bmi->getExeAtpIndex(i)).getDataPointer() + bmi->getExeOffset(i);
    } else
...
ex_expr::exp_return_type InputOutputExpr::outputValues(atp_struct *atp, void *output_desc_, CollHeap *heap,
                                                       UInt32 flags) {
  NABoolean isOdbc = isODBC(flags);
  NABoolean isIFIO = isInternalFormatIO(flags);
  NABoolean isMxci = isMXCI(flags);
  ...
  if (!useParamIdx && (getNumEntries() != output_desc->getUsedEntryCount()) &&
  ...
  if (NOT output_desc->bulkMoveDisabled()) {
    if (NOT output_desc->bulkMoveSetup()) {
      if (output_desc->rowwiseRowset()) output_desc->setRowwiseRowsetDisabled(FALSE);

#ifdef _DEBUG
      if (getenv("DISABLE_ROWWISE_ROWSET")) output_desc->setRowwiseRowsetDisabled(TRUE);
#endif
      ...
      if (
          ...
          (((output_desc->rowwiseRowset()) && ((output_desc->bulkMoveDisabled()) || (output_desc->doSlowMove()))) &&
           (output_desc->rowwiseRowsetV1() == TRUE)) ||
          ((output_desc->rowwiseRowset()) && ((output_desc->bulkMoveDisabled())) &&
    ...
    if (NOT output_desc->bulkMoveDisabled()) {
      ...
      if (NOT output_desc->doSlowMove()) return ex_expr::EXPR_OK;
    }
  }
  ...
  while (clause) {
    if (clause->getType() == ex_clause::INOUT_TYPE) {
      ...
      if ((NOT output_desc->bulkMoveDisabled()) && (NOT output_desc->doSlowMove(entry))) goto next_clause;
      ...
      switch (operand->getAtpIndex()) {
        ...
        case 1:
          ...
          if (operand->getNullFlag() &&
              ExpTupleDesc::isNullValue(sourceNullInd, operand->getNullBitIndex(), operand->getTupleFormat())) {
          ...
          if (operand->getOffset() < 0) {
            ...
            if (dataPtr) {
              ...
              if (operand->getNullFlag()) {
                ...
                if (ExpTupleDesc::isNullValue(sourceNullInd, operand->getNullBitIndex(), operand->getTupleFormat())) {
                  sourceNullInd = 0;  // it is a null value
                }
              }
              if (operand->getVCIndicatorLength()) sourceVCLenInd = dataPtr + operand->getVCLenIndOffset();
            } else {
              sourceNullInd = 0;
              sourceVCLenInd = 0;
            }
      ...
      output_desc->getDescItem(entry, SQLDESC_ROWSET_IND_LAYOUT_SIZE, &indSize, 0, 0, 0, 0);
      ...
      if ((targetType >= REC_MIN_INTERVAL) && (targetType <= REC_MAX_INTERVAL)) {
        output_desc->getDescItem(entry, SQLDESC_INT_LEAD_PREC, &targetPrecision, 0, 0, 0, 0);

        // SQLDESC_PRECISION gets the fractional precision.
        output_desc->getDescItem(entry, SQLDESC_PRECISION, &targetScale, 0, 0, 0, 0);
      } else if (targetType == REC_DATETIME) {
        output_desc->getDescItem(entry, SQLDESC_DATETIME_CODE, &targetPrecision, 0, 0, 0, 0);
        output_desc->getDescItem(entry, SQLDESC_PRECISION, &targetScale, 0, 0, 0, 0);
      } else {
        output_desc->getDescItem(entry, SQLDESC_PRECISION, &targetPrecision, 0, 0, 0, 0);
        output_desc->getDescItem(entry, SQLDESC_SCALE, &targetScale, 0, 0, 0, 0);
      }
      ...
      if ((targetType >= REC_MIN_CHARACTER) && (targetType <= REC_MAX_CHARACTER)) {
        ...
        if (targetType == REC_BYTE_V_ANSI && CharInfo::is_NCHAR_MP(targetCharSet)) treatAnsivByteAsWideChar = TRUE;
      ...
      for (int RowNum = 0; RowNum < sourceNumProcessed; RowNum++) {
        ...
        if (sourceCompiledWithRowsets) output_desc->setDescItem(0, SQLDESC_ROWSET_HANDLE, RowNum, 0);
        output_desc->getDescItem(entry, SQLDESC_IND_PTR, &tempTarget, 0, 0, 0, 0);
        ...
        output_desc->getDescItem(entry, SQLDESC_VAR_PTR, &tempTarget, 0, 0, 0, 0);
        ...
        if (!nullMoved) {
          ...
          if ((DFS2REC::isSQLVarChar(targetType)) || (DFS2REC::isLOB(targetType)) ||
              (DFS2REC::isComposite(targetType))) {
            targetVCLen = target;
            if (targetVarPtr) {
          ...
          if (NOT areCompatible(sourceType, targetType)) {
            if ((NOT skipTypeCheck()) ||
                ((restrictedSkipTypeCheck()) && (NOT implicitConversionSupported(sourceType, targetType)))) {
...

            {
              ...
              *diagsArea << DgString0(CharInfo::getCharSetName((CharInfo::CharSet)targetCharSet))
          ...
          if ((targetType == REC_DECIMAL_LS) && (operand->getDatatype() != REC_DECIMAL_LSE) &&
            ...
            if (source) {
              ...
              if (::convDoIt(source, sourceLen, sourceType, sourcePrecision, sourceScale, intermediate, targetLen - 1,
                             REC_DECIMAL_LSE, targetPrecision, targetScale, NULL, 0, heap,
                             &diagsArea) != ex_expr::EXPR_OK) {
                if (diagsArea != atp->getDiagsArea()) atp->setDiagsArea(diagsArea);

                NADELETEBASIC(intermediate, heap);
                if (!targetVarPtr) NADELETEBASIC(realTarget, heap);
                return ex_expr::EXPR_ERROR;
              };
          ...
          //////////////////////////////////////////////////////////////////
          if ((sourceScale > targetScale) && (isExactNumeric(sourceType)) && (isExactNumeric(targetType))) {
            ...
            if (source) {
              // convert to largeint
              if (convDoIt(source, sourceLen, sourceType, sourcePrecision, sourceScale, intermediate,
                           (DFS2REC::isBigNum(sourceType) ? 57 : 8),
                           (DFS2REC::isBigNum(sourceType) ? REC_NUM_BIG_SIGNED : REC_BIN64_SIGNED), targetPrecision,
                           targetScale, NULL, 0, heap, &diagsArea) != ex_expr::EXPR_OK) {
                if (diagsArea != atp->getDiagsArea()) atp->setDiagsArea(diagsArea);
              ...
              // scale it
              if (scaleDoIt(intermediate, (DFS2REC::isBigNum(sourceType) ? 57 : 8),
                            (DFS2REC::isBigNum(sourceType) ? REC_NUM_BIG_SIGNED : REC_BIN64_SIGNED), sourceScale,
                            targetScale, sourceType, heap) != ex_expr::EXPR_OK) {
            ...
            if (targetType == REC_DECIMAL_LS) {
              if (source) {
                ...
                if (::convDoIt(intermediate, 8, REC_BIN64_SIGNED, targetPrecision, targetScale, intermediate2,
                               targetLen - 1, REC_DECIMAL_LSE, targetPrecision, targetScale, NULL, 0, heap,
                               &diagsArea) != ex_expr::EXPR_OK) {
                  if (diagsArea != atp->getDiagsArea()) atp->setDiagsArea(diagsArea);
              ...
              sourceType = (DFS2REC::isBigNum(sourceType) ? REC_NUM_BIG_SIGNED : REC_BIN64_SIGNED);
            }

            if (source) source = intermediate;
          ...
          if (implicitConversion) {
            if ((targetType == REC_BYTE_V_ANSI) &&
                ((sourceType < REC_MIN_CHARACTER) || (sourceType > REC_MAX_CHARACTER))) {
              if (source) {
                intermediate = new (heap) char[targetLen];
                short intermediateLen;
                if (::convDoIt(source, sourceLen, sourceType, sourcePrecision, sourceScale, intermediate, targetLen,
                               REC_BYTE_V_ASCII, targetPrecision, targetScale, (char *)&intermediateLen, sizeof(short),
            ...
            if (((targetType == REC_DATETIME) || (DFS2REC::isInterval(targetType))) &&
          ...
          if ((source) && (NOT implicitConversion) &&
            ...
            if (sourceType == REC_DATETIME) {
              if ((sourcePrecision != targetPrecision) || (sourceScale != targetScale)) {
                ...
                if (((NOT isOdbc) &&
                     (!isCall() || !output_desc->isDescTypeWide() || !output_desc->rowwiseRowsetV2())) ||
                    (isOdbc && !output_desc->rowwiseRowsetV1() && !output_desc->rowwiseRowsetV2())) {
                  targetType = REC_BYTE_F_ASCII;
            ...
            } else if ((sourceType == targetType) && (sourcePrecision == targetPrecision) &&
              ...
              if (::convDoIt(source, sourceLen, sourceType, sourcePrecision, sourceScale, intermediate, intermediateLen,
                             targetType, targetPrecision, targetScale, NULL, sizeof(short), heap,
                             &diagsArea) != ex_expr::EXPR_OK) {
                if (diagsArea != atp->getDiagsArea()) atp->setDiagsArea(diagsArea);
          ...
          if (source) {
            ...
            rc = convDoIt(source, sourceLen, sourceType, sourcePrecision, sourceScale, target, targetLen, targetType,
                          targetPrecision, targetScale, targetVCLen, targetVCLenSize, heap, &diagsArea, case_index, 0,
                          convFlags);
            ...
            if (diagsArea) {
              ...
              if (DFS2REC::isAnyCharacter(targetType) && DFS2REC::isAnyCharacter(operand->getDatatype()) &&
                  diagsArea->containsWarning(numOfWarningsFoundSoFar, EXE_STRING_OVERFLOW)) {
                if (targetIndPtr) {
                  ...
                  if (DFS2REC::isANSIVarChar(sourceType)) {
                    ...
                    *tempVCInd = (short)sourceLen;
                    ...
                    if (CharInfo::maxBytesPerChar(sourceCharSet) == 2) *tempVCInd /= SQL_DBCHAR_SIZE;
                  }
                }
            ...
            if (targetType >= REC_MIN_NUMERIC && targetType <= REC_MAX_NUMERIC && sourceScale != targetScale &&
                ::scaleDoIt(target, targetLen, targetType, sourceScale, targetScale, sourceType, heap) !=
                    ex_expr::EXPR_OK) {
              ExRaiseSqlError(heap, &diagsArea, EXE_NUMERIC_OVERFLOW);
              if (diagsArea != atp->getDiagsArea()) atp->setDiagsArea(diagsArea);
            ...
            if (!targetVarPtr) {
              ...
              if (targetVCLenSize > 0)
                ...
                length = targetLen;
              output_desc->setDescItem(entry, SQLDESC_VAR_DATA, length, realTarget);
        ...
        if (source != NULL) {
          ...
          if ((operand->getVCIndicatorLength() > 0) && (sourceVCLenInd)) sourceVCLenInd += operand->getStorageLength();
          if ((operand->getNullFlag()) && (sourceNullInd)) sourceNullInd += operand->getStorageLength();
        }
  ...
  if (totalNumOfRowsets) output_desc->setDescItem(0, SQLDESC_ROWSET_NUM_PROCESSED, sourceNumProcessed, 0);

  return ex_expr::EXPR_OK;
}

ex_expr::exp_return_type InputOutputExpr::describeInput(void *input_desc_, void *rwrs_info, UInt32 flags) {
  NABoolean isOdbc = isODBC(flags);
  NABoolean isDbtr = isDBTR(flags);
  NABoolean isRwrs = isRWRS(flags);
  NABoolean isIFIO = (isInternalFormatIO(flags) || (isDbtr && isRwrs));
  ...
  while (clause) {
    if (clause->getType() == ex_clause::INOUT_TYPE) {
      ...
      if ((dataType >= REC_MIN_INTERVAL) && (dataType <= REC_MAX_INTERVAL)) {
        ...
        // DATETIME_INTERVAL_PRECISION = leading precision
        //
        input_desc->setDescItem(entry, SQLDESC_DATETIME_CODE, getIntervalCodeLocal(dataType), 0);
        input_desc->setDescItem(entry, SQLDESC_PRECISION, operand->getScale(), 0);
        input_desc->setDescItem(entry, SQLDESC_SCALE, 0, 0);
        input_desc->setDescItem(entry, SQLDESC_INT_LEAD_PREC, operand->getPrecision(), 0);
        if (NOT isIFIO) {
          displayLength = ExpInterval::getDisplaySize(dataType, operand->getPrecision(), operand->getScale());
        ...
        // PRECISION = fractional precision
        //
        input_desc->setDescItem(entry, SQLDESC_DATETIME_CODE, operand->getPrecision(), 0);
        input_desc->setDescItem(entry, SQLDESC_PRECISION, operand->getScale(), 0);
        input_desc->setDescItem(entry, SQLDESC_SCALE, 0, 0);
        input_desc->setDescItem(entry, SQLDESC_INT_LEAD_PREC, 0, 0);

        if (NOT isIFIO) {
          displayLength = ExpDatetime::getDisplaySize(operand->getPrecision(), operand->getScale());
          length = displayLength;
        }

      } else {
        input_desc->setDescItem(entry, SQLDESC_DATETIME_CODE, 0, 0);
        input_desc->setDescItem(entry, SQLDESC_PRECISION, operand->getPrecision(), 0);
        input_desc->setDescItem(entry, SQLDESC_SCALE, operand->getScale(), 0);
        input_desc->setDescItem(entry, SQLDESC_INT_LEAD_PREC, 0, 0);
      }

      // Use SQLDESC_CHAR_SET_NAM (one-part name) for charset
      if ((((dataType >= REC_MIN_CHARACTER) && (dataType <= REC_MAX_CHARACTER))) || (dataType == REC_CLOB)) {
      ...
      input_desc->setDescItem(entry, SQLDESC_NAME, 0, ((ex_inout_clause *)clause)->getName());

      if (((ex_inout_clause *)clause)->getHeading() != 0)
        input_desc->setDescItem(entry, SQLDESC_HEADING, 0, ((ex_inout_clause *)clause)->getHeading());

      if (((ex_inout_clause *)clause)->getTableName() != 0)
        input_desc->setDescItem(entry, SQLDESC_TABLE_NAME, 0, ((ex_inout_clause *)clause)->getTableName());

      if (((ex_inout_clause *)clause)->getSchemaName() != 0)
        input_desc->setDescItem(entry, SQLDESC_SCHEMA_NAME, 0, ((ex_inout_clause *)clause)->getSchemaName());
      if (((ex_inout_clause *)clause)->getCatalogName() != 0)
        input_desc->setDescItem(entry, SQLDESC_CATALOG_NAME, 0, ((ex_inout_clause *)clause)->getCatalogName());
      ...
      if (operand->getRowsetSize() > 0) {
        input_desc->setDescItem(0, SQLDESC_ROWSET_SIZE, operand->getRowsetSize(), 0);

        // Use the operand length if the operand is not INTERVAL/DATETIME.
        if (displayLength == 0) displayLength = operand->getLength();
        ...
        if (DFS2REC::isAnyCharacter(dataType) && !DFS2REC::isSQLVarChar(dataType)) {
          ...
          displayLength += CharInfo::maxBytesPerChar(operand->getCharSet());
                   ...
                   (dataType >= REC_MIN_INTERVAL && dataType <= REC_MAX_INTERVAL_MP) || (dataType == REC_DATETIME)) {
        ...
        input_desc->setDescItem(entry, SQLDESC_ROWSET_VAR_LAYOUT_SIZE, displayLength, 0);

        input_desc->setDescItem(entry, SQLDESC_ROWSET_IND_LAYOUT_SIZE, operand->getNullIndicatorLength(), 0);
      } else {
        input_desc->setDescItem(entry, SQLDESC_ROWSET_VAR_LAYOUT_SIZE, 0, 0);
        input_desc->setDescItem(entry, SQLDESC_ROWSET_IND_LAYOUT_SIZE, 0, 0);
      }
      ...
      BriefAssertion((paramMode == PARAMETER_MODE_IN) || (paramMode == PARAMETER_MODE_INOUT) ||
                         (paramMode == PARAMETER_MODE_UNDEFINED),
                     "invalid param mode");
      BriefAssertion(paramIdx >= 0, "invalid param index");
      BriefAssertion(ordPos >= 0, "invalid param ordinal position");
      ...
      if (isCall()) {
        ...
        input_desc->setDescItem(entry, SQLDESC_PARAMETER_MODE, PARAMETER_MODE_IN, 0);
        input_desc->setDescItem(entry, SQLDESC_PARAMETER_INDEX, entry, 0);
        input_desc->setDescItem(entry, SQLDESC_ORDINAL_POSITION, 0, 0);
      }
      ...
      // in the descriptor.
      //
      input_desc->setDescItem(entry, SQLDESC_IND_LENGTH, operand->getNullIndicatorLength(), 0);
      input_desc->setDescItem(entry, SQLDESC_VC_IND_LENGTH, operand->getVCIndicatorLength(), 0);
      ...
      //
      if ((rwrsInfo) && ((entry == rwrsInfo->rwrsInputSizeIndex()) || (entry == rwrsInfo->rwrsMaxInputRowlenIndex()) ||
                         (entry == rwrsInfo->rwrsBufferAddrIndex()) || (entry == rwrsInfo->rwrsPartnNumIndex()))) {
      ...
      input_desc->setDescItemInternal(entry, SQLDESC_DATA_OFFSET, dataOffset, 0);
      input_desc->setDescItemInternal(entry, SQLDESC_NULL_IND_OFFSET, nullIndOffset, 0);

      input_desc->setDescItemInternal(entry - 1, SQLDESC_ALIGNED_LENGTH, currEntryStartOffset - prevEntryStartOffset,
                                      0);
  ...
  if (firstEntryProcessed) {
    currAlignedOffset = ADJUST(currAlignedOffset, firstEntryStartAlignment);
    input_desc->setDescItemInternal(entry, SQLDESC_ALIGNED_LENGTH, currAlignedOffset - prevEntryStartOffset, 0);
  }

  if (!(isCall() && input_desc->isDescTypeWide()) || (entry > input_desc->getUsedEntryCount()))
...
ex_expr::exp_return_type InputOutputExpr::inputSingleRowValue(atp_struct *atp, void *inputDesc_, char *tgtRowPtr,
                                                              CollHeap *heap, UInt32 flags) {
  NABoolean isOdbc = isODBC(flags);
  NABoolean isDbtr = isDBTR(flags);
  NABoolean isRwrs = isRWRS(flags);
  NABoolean isIFIO = (isInternalFormatIO(flags) || (isDbtr && isRwrs));
  ...
  if (NOT inputDesc->bulkMoveDisabled()) {
    ...
    if (NOT inputDesc->bulkMoveDisabled()) {
      ...
      if (NOT inputDesc->doSlowMove()) return ex_expr::EXPR_OK;
    }
  }
  ...
  while (clause) {
    if (clause->getType() == ex_clause::INOUT_TYPE) {
      ...
      if ((NOT inputDesc->bulkMoveDisabled()) && (NOT inputDesc->doSlowMove(entry))) goto next_clause;
      ...
      char *dataPtr = (tgtRowPtr ? tgtRowPtr : (atp->getTupp(operand->getAtpIndex())).getDataPointer());

      target = (char *)(dataPtr + operand->getOffset());

      if (operand->getVCIndicatorLength() > 0) targetVCLenInd = (char *)(dataPtr + operand->getVCLenIndOffset());

      if (operand->getNullFlag()) targetNullInd = (char *)(dataPtr + operand->getNullIndOffset());
      ...
      if ((sourceType >= REC_MIN_INTERVAL) && (sourceType <= REC_MAX_INTERVAL)) {
        inputDesc->getDescItem(entry, SQLDESC_INT_LEAD_PREC, &sourcePrecision, 0, 0, 0, 0);

        // SQLDESC_PRECISION gets the fractional precision
        inputDesc->getDescItem(entry, SQLDESC_PRECISION, &sourceScale, 0, 0, 0, 0);
      } else if (sourceType == REC_DATETIME) {
        inputDesc->getDescItem(entry, SQLDESC_DATETIME_CODE, &sourcePrecision, 0, 0, 0, 0);
        inputDesc->getDescItem(entry, SQLDESC_PRECISION, &sourceScale, 0, 0, 0, 0);
      } else {
        inputDesc->getDescItem(entry, SQLDESC_PRECISION, &sourcePrecision, 0, 0, 0, 0);
        inputDesc->getDescItem(entry, SQLDESC_SCALE, &sourceScale, 0, 0, 0, 0);
      };
      ...
      if ((targetType == REC_NCHAR_F_UNICODE || targetType == REC_NCHAR_V_UNICODE)) {
        // charset can be retrieved as a long INTERNALLY
        // using programatic interface by calling getDescItem(),
        // and can only be retrieved as a character string EXTERNALLY
        // using "get descriptor" syntax.
      ...
      // strings
      inputDesc->getDescItem(entry, SQLDESC_OCTET_LENGTH, &sourceLen, 0, 0, 0, 0);

      inputDesc->getDescItem(entry, SQLDESC_IND_DATA, &indData, 0, 0, 0, 0);
      if (indData == 0) {
        inputDesc->getDescItem(entry, SQLDESC_IND_PTR, &tempSource, 0, 0, 0, 0);
      } else
        tempSource = (char *)&indData;

      if ((isOdbc) && (isRwrs)) {
        if (operand->getNullIndOffset() >= 0)
          sourceIndPtr = (char *)(inputDesc->getCurrRowPtrInRowwiseRowset() + operand->getNullIndOffset());
        else
          sourceIndPtr = NULL;
      } else
      ...
      if (sourceIndPtr) {
        ...
        if (operand->getNullFlag()) {
          ...
          str_cpy_all(targetNullInd, sourceIndPtr, operand->getNullIndicatorLength());
        } else {
          if (temp < 0) {
      ...
      if ((isOdbc) && (isRwrs)) {
        if (operand->getOffset() >= 0)
          source = (char *)(inputDesc->getCurrRowPtrInRowwiseRowset() + operand->getOffset());
        else
          source = NULL;
      } else
      ...
      if (!nullMoved) {
        ...
        if (NOT areCompatible(sourceType, targetType)) {
          if ((NOT skipTypeCheck()) ||
              ((restrictedSkipTypeCheck()) && (NOT implicitConversionSupported(sourceType, operand->getDatatype())))) {
          ...
          if (DFS2REC::isAnyCharacter(sourceType) &&
            ...
            *diagsArea << DgString0(CharInfo::getCharSetName((CharInfo::CharSet)sourceCharSet))
        ...
        if (DFS2REC::isAnyCharacter(sourceType) && sourceCharSet == CharInfo::UNICODE) {
          ...
          if (sourceType == REC_NCHAR_V_ANSI_UNICODE) {
            ...
            if (realSourceLen > sourceLen) realSourceLen = sourceLen;
          }
          realSourceLen /= SQL_DBCHAR_SIZE;

          if (CharInfo::checkCodePoint((wchar_t *)source, realSourceLen, CharInfo::UNICODE) == FALSE) {
        ...
        if ((implicitConversion) && ((sourceType == REC_BYTE_V_ANSI) &&
                                     ((targetType < REC_MIN_CHARACTER) || (targetType > REC_MAX_CHARACTER)))) {
          intermediate = new (heap) char[sourceLen];
          if (::convDoIt(source, sourceLen, sourceType, sourcePrecision, sourceScale, intermediate, sourceLen,
                         REC_BYTE_F_ASCII, sourcePrecision, sourceScale, NULL, 0, heap,
                         &diagsArea) != ex_expr::EXPR_OK) {
            NADELETEBASIC(intermediate, heap);
            errorCode = EXE_OK;
            goto error_return;
        ...
        if ((sourceType >= REC_MIN_BINARY_NUMERIC) && (sourceType <= REC_MAX_BINARY_NUMERIC) &&
            (sourceType == operand->getDatatype()) && (sourcePrecision > 0) &&
        ...
        if ((NOT implicitConversion) &&
          ...
          if (sourceType == REC_DATETIME) {
            if ((sourcePrecision != operand->getPrecision()) || (sourceScale != operand->getScale())) {
              // source and target must have the same datetime_code,
              // they must both be date, time or timestamp.
              errorCode = EXE_CONVERT_NOT_SUPPORTED;
              goto error_return;
              ...
              sourceType = REC_BYTE_F_ASCII;
          } else if ((sourceType == targetType) && (sourcePrecision == operand->getPrecision()) &&
            ...
            intermediate = new (heap) char[intermediateLen];
            convFlags |= CONV_ALLOW_SIGN_IN_INTERVAL;
            if (::convDoIt(source, sourceLen, REC_BYTE_F_ASCII,
                           0,  // sourcePrecision,
                           0,  // sourceScale,
                           intermediate, intermediateLen, sourceType, sourcePrecision, sourceScale, NULL, 0, heap,
                           &diagsArea, CONV_UNKNOWN, 0, convFlags) != ex_expr::EXPR_OK) {
              NADELETEBASIC(intermediate, heap);
        ...
        retcode = ::convDoIt(source, sourceLen, sourceType, sourcePrecision, sourceScale, target, operand->getLength(),
                             operand->getDatatype(), operand->getPrecision(), operand->getScale(), targetVCLenInd,
                             operand->getVCIndicatorLength(), heap, &diagsArea, CONV_UNKNOWN, 0, convFlags);
        ...
        // up- or down-scale target.
        if ((operand->getDatatype() >= REC_MIN_NUMERIC) && (operand->getDatatype() <= REC_MAX_NUMERIC) &&
            (sourceScale != operand->getScale()) &&
            ::scaleDoIt(target, operand->getLength(), operand->getDatatype(), sourceScale, operand->getScale(),
                        sourceType, heap) != ex_expr::EXPR_OK) {
...
error_return:
  if (errorCode != EXE_OK) ExRaiseSqlError(heap, &diagsArea, errorCode);
  if (diagsArea != atp->getDiagsArea()) atp->setDiagsArea(diagsArea);

  return ex_expr::EXPR_ERROR;
}
ex_expr::exp_return_type InputOutputExpr::inputRowwiseRowsetValues(atp_struct *atp, void *inputDesc_, void *rwrs_info,
                                                                   NABoolean tolerateNonFatalError, CollHeap *heap,
                                                                   UInt32 flags) {
  NABoolean isOdbc = isODBC(flags);
  NABoolean isDbtr = isDBTR(flags);
  ...
  while (clause) {
    if (clause->getType() == ex_clause::INOUT_TYPE) {
      ...
      if ((entry == rwrsInfo->rwrsInputSizeIndex()) || (entry == rwrsInfo->rwrsMaxInputRowlenIndex()) ||
          (entry == rwrsInfo->rwrsBufferAddrIndex()) || (entry == rwrsInfo->rwrsPartnNumIndex())) {
        operand = clause->getOperand(0);

        target = (char *)((atp->getTupp(operand->getAtpIndex())).getDataPointer() + operand->getOffset());
  ...
  if ((inputRowsetSize < 0) || (inputRowsetSize > rwrsInfo->rwrsMaxSize())) {
    // raise error
  ...
  if (inputRowsetSize == 0) return ex_expr::EXPR_OK;
  ...
  if (NOT inputDesc->bulkMoveDisabled()) {
    if (NOT inputDesc->bulkMoveSetup()) {
      ...
      if ((NOT inputDesc->bulkMoveDisabled()) && (NOT inputDesc->doSlowMove()) &&
  ...
  if (rwrsInfo->useUserRWRSBuffer()) {
    inputDesc->setDescItem(0, SQLDESC_ROWWISE_ROWSET_SIZE, inputRowsetSize, 0);
    inputDesc->setDescItem(0, SQLDESC_ROWWISE_ROWSET_ROW_LEN, maxInputRowlen, 0);
    inputDesc->setDescItem(0, SQLDESC_ROWWISE_ROWSET_PTR, (Long)bufferAddr, 0);
    inputDesc->setDescItem(0, SQLDESC_ROWWISE_ROWSET_PARTN_NUM, partnNum, 0);

    inputDesc->setDescItem(0, SQLDESC_ROWSET_NUM_PROCESSED, inputRowsetSize, NULL);
    ...
    *rwrsMaxInputRowlenLoc = maxInputRowlen;
    ...
    while (numRowsProcessed < inputRowsetSize) {
      inputDesc->setDescItem(0, SQLDESC_ROWSET_NUM_PROCESSED, numRowsProcessed, NULL);

      setIsRWRS(flags, TRUE);
      if (inputSingleRowValue(atp, inputDesc, (char *)tgtRowAddr, heap, flags) == ex_expr::EXPR_ERROR) {
        errorCode = EXE_OK;
        goto error_return;
      }
    ...
    inputDesc->setDescItem(0, SQLDESC_ROWSET_NUM_PROCESSED, 0, NULL);

    // move the address of the internal rowwise rowset buffer so it could
    // be passed on to other operators.
    *rwrsInternalBufferAddrLoc = rwrsInfo->getRWRSInternalBufferAddr();

    // move the max length of each row so it could be passed on to other
    // operators.
    *rwrsMaxInputRowlenLoc = rwrsInfo->rwrsMaxInternalRowlen();
  }  // use internal buffer
...
error_return:
  if (errorCode != EXE_OK)
    ExRaiseSqlError(heap, &diagsArea, errorCode, NULL, (intParam1 != 0 ? &intParam1 : NULL),
                    (intParam2 != 0 ? &intParam2 : NULL), (intParam3 != 0 ? &intParam3 : NULL));
  if (diagsArea != atp->getDiagsArea()) atp->setDiagsArea(diagsArea);
  return ex_expr::EXPR_ERROR;
}

ex_expr::exp_return_type InputOutputExpr::inputValues(atp_struct *atp, void *inputDesc_,
                                                      NABoolean tolerateNonFatalError, CollHeap *heap, UInt32 flags,
                                                      NABoolean optLargVar, NABoolean syncQuery) {
  NABoolean isOdbc = isODBC(flags);
  NABoolean isIFIO = isInternalFormatIO(flags);
  ...
  if (!inputDesc) {
    ...
    if (!byPassCheck && !inputDesc->thereIsACompoundStatement() && getNumEntries() != inputDesc->getUsedEntryCount()) {
      ExRaiseSqlError(heap, &diagsArea, CLI_STMT_DESC_COUNT_MISMATCH);
      if (diagsArea != atp->getDiagsArea()) atp->setDiagsArea(diagsArea);
      return ex_expr::EXPR_ERROR;
    }
  ...
  CharInfo::CharSet sourceCharSet = CharInfo::UnknownCharSet;
  CharInfo::CharSet targetCharSet = CharInfo::UnknownCharSet;
  // 65535 means short size
  short charsets[65535];
  int presisions[65535];
  int scales[65535];
  ...
  while (clause) {
    if (clause->getType() == ex_clause::INOUT_TYPE) {
      ...
      if (operand->getVCIndicatorLength() > 0)
        targetVCLenInd =
            (char *)((atp->getTupp(operand->getAtpIndex())).getDataPointer() + operand->getVCLenIndOffset());

      if (operand->getNullFlag())
        targetNullInd = (atp->getTupp(operand->getAtpIndex())).getDataPointer() + operand->getNullIndOffset();
      ...
      // pack the paramters into a bytes[]
      charsets[entry] = sourceCharSet;
      presisions[entry] = sourcePrecision;
      scales[entry] = sourceScale;
      entries[entry] = 1;
      ...
      if (targetRowsetSize > 0) {
        ...
        inputDesc->getDescItem(entry, SQLDESC_ROWSET_VAR_LAYOUT_SIZE, &rowsetVarLayoutSize, 0, 0, 0, 0);
      ...
      for (int RowNum = 0; RowNum < numToProcess; RowNum++) {
        ...
        if ((targetRowsetSize > 0) && (RowNum > 0)) {
          ...
          if (operand->getVCIndicatorLength() > 0) targetVCLenInd += operand->getStorageLength();
          if (operand->getNullFlag()) targetNullInd += operand->getStorageLength();
        ...
        if (targetRowsetSize > 0) inputDesc->setDescItem(0, SQLDESC_ROWSET_HANDLE, RowNum, 0);
        inputDesc->getDescItem(entry, SQLDESC_IND_DATA, &indData, 0, 0, 0, 0);
        if (indData == 0) {
          inputDesc->getDescItem(entry, SQLDESC_IND_PTR, &tempSource, 0, 0, 0, 0);
          inputDesc->getDescItem(entry, SQLDESC_ROWSET_IND_LAYOUT_SIZE, &rowsetIndLayoutSize, 0, 0, 0, 0);
        ...
        if (sourceIndPtr) {
          short temp = 0;
          str_cpy_all((char *)&temp, sourceIndPtr, operand->getNullIndicatorLength());
          if (inputDesc && inputDesc->thereIsACompoundStatement() && (temp > 0)) {
        ...
        if (!nullMoved) {
          ...
          if (NOT areCompatible(sourceType, targetType)) {
            if ((NOT skipTypeCheck()) || ((restrictedSkipTypeCheck()) &&
                                          (NOT implicitConversionSupported(sourceType, operand->getDatatype())))) {
              ExRaiseSqlError(heap, &diagsArea, EXE_CONVERT_NOT_SUPPORTED);
              if (diagsArea != atp->getDiagsArea()) atp->setDiagsArea(diagsArea);
          ...
          if (targetRowsetSize > 0 && !optLargVar) {
            ...
            if (DFS2REC::isSQLVarChar(sourceType)) {
              if (sourceLen < operand->getLength()) memset(target + sourceLen, 0, operand->getLength() - sourceLen);
            }
          }
          ...
          if (DFS2REC::isAnyCharacter(sourceType) && sourceCharSet == CharInfo::UNICODE) {
            ...
            if (CharInfo::checkCodePoint((NAWchar *)source, realSourceLen, CharInfo::UNICODE) == FALSE) {
              // Error code 3400 falls in CLI error code area, but it is
              // a perfect fit to use here (as an exeutor error).
              ExRaiseSqlError(heap, &diagsArea, (ExeErrorCode)3400);
          ...
          if ((implicitConversion) && ((sourceType == REC_BYTE_V_ANSI) &&
            ...
            if (index1 == CONV_UNKNOWN) {
              ...
              index1 = tempClause.findInstruction(sourceType, sourceLen, REC_BYTE_F_ASCII, sourceLen, 0);
            }

            if (::convDoIt(source, sourceLen, sourceType, sourcePrecision, sourceScale, intermediate, sourceLen,
                           REC_BYTE_F_ASCII, sourcePrecision, sourceScale, NULL, 0, heap, &diagsArea,
          ...
          if ((sourcePrecision > 0) &&
              ((sourceType >= REC_MIN_BINARY_NUMERIC) && (sourceType <= REC_MAX_BINARY_NUMERIC) &&
                   (sourceType == operand->getDatatype()) && (sourcePrecision == operand->getPrecision()) ||
          ...
          if ((NOT implicitConversion) && (((sourceType == REC_DATETIME) ||
            ...
            if (sourceType == REC_DATETIME) {
              ...
              if (index2 == CONV_UNKNOWN) {
                ex_conv_clause tempClause;
                index2 = tempClause.findInstruction(REC_BYTE_F_ASCII, sourceLen, sourceType, intermediateLen,
                                                    0 - sourceScale);
              }
              ...
              if (::convDoIt(source, sourceLen, REC_BYTE_F_ASCII,
                             ...
                             0,  // sourceScale,
                             intermediate, intermediateLen, sourceType, sourcePrecision, sourceScale, NULL, 0, heap,
                             &diagsArea, index2, 0, convFlags) != ex_expr::EXPR_OK) {
                if (diagsArea != atp->getDiagsArea()) atp->setDiagsArea(diagsArea);
          ...
          if (index3 == CONV_UNKNOWN) {
            ex_conv_clause tempClause;
            index3 = tempClause.findInstruction(sourceType, sourceLen, operand->getDatatype(), operand->getLength(),
                                                sourceScale - operand->getScale());
          }
          ...
          retcode =
              ::convDoIt(source, sourceLen, sourceType, sourcePrecision, sourceScale, target, operand->getLength(),
                         operand->getDatatype(), operand->getPrecision(), operand->getScale(), targetVCLenInd,
                         operand->getVCIndicatorLength(), heap, &diagsArea, index3, 0, convFlags);
          ...
          if (retcode != ex_expr::EXPR_OK) {
            ...
            if (counter) {
              ...
              // only warning will get there, if it is a error, the upper if clause will
              // catch it
              diagsArea->deleteWarning(warningMark2 - counter);
            ...
            if (tolerateNonFatalError && diagsArea->canAcceptMoreErrors())
              ...
              *diagsArea << DgString1(str_len(operand->getColName()) ? NAString("Column Name: ") + operand->getColName()
                                                                     : "");
              return ex_expr::EXPR_ERROR;
            }
          }
          ...
          // from <var>
          if (operand->getHVRowsetForInputSize() || operand->getHVRowsetLocalSize()) {
            short targetType = operand->getDatatype();
            switch (targetType) {
  ...
  if (syncQuery) {
    int srcLents[entry + 1];
    // Calculate the total length
    for (short i = 1; i <= entry; i++) {
      if (entries[i] == 0) continue;

      int srcLent = inputDesc->getVarDataLength(i);
      sourceType = inputDesc->getVarDataType(i);
      if (sourceType == 64)
        if (srcLent > SHRT_MAX)
          srcLent += 4;
        else
          srcLent += 2;
      srcLents[i] = srcLent;
      memused += (sizeof(ParamPack) + srcLent);
    }

    // allock a char bufer len is memused.
    // more 4 bytes to desc rowset size
    sourDataParams_ = new (heap) unsigned char[memused + 4];
    sourDataParamsLen_ = memused + 4;
    unsigned char *pm = sourDataParams_;
    memcpy(pm, &rowsetsize, 4);
    pm += 4;
    for (int ii = 1; ii <= entry; ii++) {
      if (entries[ii] == 0) continue;

      short *const buffer1 = reinterpret_cast<short *>(pm);
      buffer1[0] = inputDesc->getVarDataType(ii);  // sourceType
      buffer1[1] = charsets[ii];                   // sourceCharSet
      pm += 4;
      int *const buffer2 = reinterpret_cast<int *>(pm);
      buffer2[0] = srcLents[ii];    // sourceLen
      buffer2[1] = presisions[ii];  // sourcePrecision
      buffer2[2] = scales[ii];      // sourceScale
      pm += 12;
      unsigned char *const buffer3 = reinterpret_cast<unsigned char *>(pm);
      memcpy(buffer3, inputDesc->getVarData(ii), srcLents[ii]);  // source
      pm += srcLents[ii];
    }
  }
...
ex_expr::exp_return_type InputOutputExpr::addDescInfoIntoStaticDesc(Descriptor *desc, NABoolean isInput) {
  ex_clause *clause = getClauses();
  short entry = 0;
  ...
  if (isCall() && desc->isDescTypeWide()) {
    while (clause) {
      if (clause->getType() == ex_clause::INOUT_TYPE) {
        entry = ((ex_inout_clause *)clause)->getParamIdx();
        if (entry > desc->getUsedEntryCount()) continue;
        desc->setDescItem(entry, SQLDESC_NAME, 0, ((ex_inout_clause *)clause)->getName());
        short paramMode = ((ex_inout_clause *)clause)->getParamMode();
        short ordPos = ((ex_inout_clause *)clause)->getOrdPos();
    ...
    while (clause && entry < desc->getUsedEntryCount()) {
      if (clause->getType() == ex_clause::INOUT_TYPE) {
        ...
        desc->setDescItem(entry, SQLDESC_NAME, 0, ((ex_inout_clause *)clause)->getName());
        if (isInput)
          desc->setDescItem(entry, SQLDESC_PARAMETER_MODE, PARAMETER_MODE_IN, 0);
        else
          desc->setDescItem(entry, SQLDESC_PARAMETER_MODE, PARAMETER_MODE_OUT, 0);
        desc->setDescItem(entry, SQLDESC_PARAMETER_INDEX, entry, 0);
