...
#include "common/ComDefs.h"          // pick up ROUND8 macro
...
class SqlBufferHeader {
 public:
  enum BufferType { NORMAL_ = 0, DENSE_ = 1, OLT_ = 2, OLT_SMALL_ = 3 };
  ...
  enum moveStatus { BUFFER_FULL, MOVE_SUCCESS, MOVE_ERROR };

  SqlBufferHeader(BufferType bufType) : bufType_(bufType) { init(); };

  void init() { sendFlags_ = 0; };

  NABoolean denseBuffer() { return ((bufType_ == DENSE_) != 0); };

  void setDenseBuffer(NABoolean v) { (v ? bufType_ = DENSE_ : bufType_ = NORMAL_); }

  NABoolean oltBuffer() { return ((bufType_ == OLT_) != 0); };

  void setOltBuffer(NABoolean v) { (v ? bufType_ = OLT_ : bufType_ = NORMAL_); }
  ...
  void setStatsEnabled(NABoolean v) { (v ? sendFlags_ |= STATS_ENABLED : sendFlags_ &= ~STATS_ENABLED); };
...
class SqlBufferBase : public SqlBufferHeader {
 public:
  ...
  virtual moveStatus moveInSendOrReplyData(NABoolean isSend,         // TRUE = send
                                           ...
                                           atp_struct *destAtp = NULL, unsigned short tuppIndex = 0,
                                           NABoolean doMoveStats = FALSE,  // if TRUE & statsArea
                                                                           // is passed in,
                                                                           // move in stats
                                           ExStatisticsArea *statsArea = NULL, tupp_descriptor **statsDesc = NULL,
                                           NABoolean useExternalDA = FALSE, NABoolean callerHasExternalDA = FALSE,
...
class SqlBuffer : public SqlBufferBase {
 public:
  SqlBuffer(BufferType bufType) : SqlBufferBase(bufType){};
  ...
  short atEOTD() const {
    if ((tuppDescIndex_ < maxTuppDesc_) || (flags_ & IGNORE_CONTROL_FLAG))
      return 0;
    else
      return -1;
  }
  ...
  void setVarLenRows(NABoolean v) { (v ? flags_ |= VAR_LEN_ROWS : flags_ &= ~VAR_LEN_ROWS); }

  virtual SqlBuffer::moveStatus moveInSendOrReplyData(
      ...
      NABoolean doMoveStats = FALSE,  // if TRUE & statsArea
                                      // is passed in,
                                      // move in stats
      ExStatisticsArea *statsArea = NULL, tupp_descriptor **statsDesc = NULL, NABoolean useExternalDA = FALSE,
      NABoolean callerHasExternalDA = FALSE, tupp_descriptor *defragTd = NULL
#if (defined(_DEBUG))
      ,
...
class SqlBufferNormal : public SqlBuffer {
 ...
 public:
  ...
  void advanceNormal() { tuppDescIndex_++; }
...
// class SqlBufferDense
//
// An SqlBufferDense is similar to an SqlBuffer, except that the
// tupp_descriptor objects and their associated data are alternating
// and grow in the same direction, rather than growing from opposite
// ends.
//
//      +-----------------------------+
//      | SqlBufferDense              |
...
//      +-----------------------------+
//
// This has the advantage that we can shorten the SqlBufferDense if
// there is a lot of free space.
//
// Each tupp_descriptor as well as the data for each tupp_descriptor
// begins on an 8 byte boundary, so there may be fillers between
// the items shown in the picture above. The tupp_descriptors are
// also connected in a doubly-linked list, using their
// nextTDIOffset_ and prevTDIOffset_ fields.
//
/////////////////////////////////////////////////////////////////////////
class SqlBufferDense : public SqlBuffer {
 ...
 private:
  ...
  TupleDescInfo *firstTupleDesc() { return (TupleDescInfo *)((char *)this + ROUND8(sizeof(*this))); }
...
class SqlBufferOltSmall : public SqlBufferHeader {
 public:
  SqlBufferOltSmall() : SqlBufferHeader(OLT_SMALL_) { str_pad(filler_, 2, '\0'); };
  ...
  void setSendData(NABoolean v) { (v ? sendFlags() |= SEND_DATA_ : sendFlags() &= ~SEND_DATA_); }

  NABoolean replyData() { return (replyFlags() & REPLY_DATA_) != 0; };

  void setReplyData(NABoolean v) { (v ? replyFlags() |= REPLY_DATA_ : replyFlags() &= ~REPLY_DATA_); }
  ...
  void setReplyStats(NABoolean v) { (v ? replyFlags() |= REPLY_STATS_ : replyFlags() &= ~REPLY_STATS_); }

  NABoolean replyStatsOnly() { return (replyFlags() & REPLY_STATS_ONLY_) != 0; };

  void setReplyStatsOnly(NABoolean v) { (v ? replyFlags() |= REPLY_STATS_ONLY_ : replyFlags() &= ~REPLY_STATS_ONLY_); }

  char *sendDataPtr() { return (sendData() ? (char *)((char *)this + sizeof(SqlBufferOltSmall)) : NULL); };

 private:
  enum SendFlags { SEND_DATA_ = 0x01 };

  enum ReplyFlags { REPLY_DATA_ = 0x01, REPLY_ROW_AFFECTED_ = 0x02, REPLY_STATS_ = 0x04, REPLY_STATS_ONLY_ = 0x08 };
...
class SqlBufferOlt : public SqlBufferBase {
 private:
  enum OltBufFlags { STATS_ = 0x01, CONTAINS_OLT_SMALL_ = 0x02, DATA_PROCESSED_ = 0x04 };
  ...
  enum Contents {
    ...
    NODATA_ROWCOUNT_,
    NODATA_WARNING_,
    NODATA_ROWAFFECTED_WARNING_,
    NODATA_ROWCOUNT_WARNING_,
    STATSONLY_
  };
 ...
 public:
  ...
  void setIsStats(NABoolean v) { (v ? oltBufFlags_ |= STATS_ : oltBufFlags_ &= ~STATS_); }

  NABoolean dataProcessed() { return (oltBufFlags_ & DATA_PROCESSED_) != 0; };

  void setDataProcessed(NABoolean v) { (v ? oltBufFlags_ |= DATA_PROCESSED_ : oltBufFlags_ &= ~DATA_PROCESSED_); }
  ...
  tupp_descriptor *getNextTuppDesc(tupp_descriptor *tdesc, int rowlen = -1) {
    ...
    if (tdesc) {
      nextTdesc = (tupp_descriptor *)(tdesc->getTupleAddress() + ROUND8(tdesc->getAllocatedSize()));
    } else {
      nextTdesc = (tupp_descriptor *)((char *)this + ROUND8(sizeof(SqlBufferOlt)));
    ...
    if (rowlen < 0)
      ...
      nextTdesc->init(rowlen, 0, (char *)nextTdesc + ROUND8(sizeof(tupp_descriptor)));
  ...
  char *getReplyDataPtr() {
    if (containsOltSmall()) {
      if ((dataProcessed()) && (NOT isStats()))
        return (char *)this + ROUND8(sizeof(SqlBufferOlt)) + ROUND8(sizeof(SqlBufferOltSmall)) -
...
class sql_buffer_pool : public ExGod {
 public:
  ...
  void setStaticMode(NABoolean v) { (v ? flags_ |= STATIC_MODE : flags_ &= ~STATIC_MODE); }

  NABoolean dynamicUnlimited() { return ((flags_ & DYNAMIC_UNLIMITED) != 0); }
