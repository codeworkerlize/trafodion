...
class SwapSpace : public NABasicObject {
 public:
  SwapSpace(Allocator &memory, const ExExeStmtGlobals *stmtGlobals, UInt16 scratchThresholdPct);
  ~SwapSpace(void);
...
inline bool SwapSpace::isIoPending(void) { return (ioPending_ != NONE); }

// Test ioPending_ in-line so that the ioPending_ == NONE case is efficient
inline IoStatus SwapSpace::checkIO(bool *readCompleted) {
  if (readCompleted) {
    *readCompleted = false;
  }
...
inline bool SwapSpace::empty(void) const { return ((numSwap_ == 0) && (ioPending_ == NONE)); }

inline char *SwapSpace::getReadBuffer(void) const { return atEOF_ ? NULL : readBuffer_; }

inline bool SwapSpace::haveFirstBuffer(void) const {
