...
inline void HashBuffer::setPrev(HashBuffer *prev) { prev_ = prev; };

inline HashBuffer *HashBuffer::getNext() const { return next_; };

inline void HashBuffer::setNext(HashBuffer *next) { next_ = next; };

inline char *HashBuffer::getDataPointer() const { return data_; };
...
inline NABoolean HashBuffer::considerBufferDefrag() const { return considerBufferDefrag_; };

inline char *HashBuffer::getFirstRow() const { return rows_; };

inline HashBufferHeader *HashBuffer::getHeader() const { return header_; };

// Cast this instance to a pointer to a HashBufferSerial in order to
// use the Serial Interface
inline HashBufferSerial *HashBuffer::castToSerial() const { return (HashBufferSerial *)this; };
...
// Methods for accessing rows in series based on an internal cursor (currRow_)
inline void HashBufferSerial::positionOnFirstRow() { currRow_ = (char *)getFirstRow(); };
...
// Return a pointer to the first row,
inline HashRow *HashBufferSerial::getFirstRow() const { return (HashRow *)rows_; };
...
inline Cluster *Bucket::getOuterCluster() const { return outerCluster_; };

inline void Bucket::setInnerCluster(Cluster *innerCluster) { innerCluster_ = innerCluster; };

inline void Bucket::setOuterCluster(Cluster *outerCluster) { outerCluster_ = outerCluster; };
...
class ClusterDB : public NABasicObject {
 ...
 public:
  ...
  void setBMOMaxMemThresholdMB(UInt16 mm) { bmoMaxMemThresholdMB_ = mm; }
...
/////////////////////////////////////////////////////////////////////////////
inline Cluster *ClusterDB::getClusterList() const { return clusterList_; };

inline void ClusterDB::setClusterList(Cluster *clusterList) { clusterList_ = clusterList; };
inline Cluster *ClusterDB::getClusterToFlush() const { return clusterToFlush_; };

inline Cluster *ClusterDB::getClusterToProbe() const { return clusterToProbe_; };

inline void ClusterDB::setClusterToProbe(Cluster *cluster) { clusterToProbe_ = cluster; };

inline Cluster *ClusterDB::getClusterToRead() const { return clusterToRead_; };

inline void ClusterDB::setClusterToRead(Cluster *cluster) { clusterToRead_ = cluster; };

inline Cluster *ClusterDB::getClusterReturnRightRows() const { return clusterReturnRightRows_; };

inline void ClusterDB::setClusterReturnRightRows(Cluster *cluster) { clusterReturnRightRows_ = cluster; };

inline HashScratchSpace *ClusterDB::getScratch() const { return tempFile_; };
...
class Cluster : public NABasicObject {
 ...
 public:
  ...
  void positionOnFirstRowInBuffer() {  // position on the first row in main memory
  ...
  NABoolean insert(atp_struct *newEntry, ex_expr *moveExpr, SimpleHashValue hashValue, ExeErrorCode *rc,
                   NABoolean skipMemoryCheck = FALSE);

  void positionOnFirstRowInFirstOuterBuffer() {  // position on the first row in the first outer buffer in memory

    for (HashBuffer *tmpB = clusterDb_->outerReadBuffers_; tmpB; tmpB = tmpB->getNext()) tmpB->castToSerial()->fixup();
  ...
  // hash group by)
  NABoolean spill(ExeErrorCode *rc, NABoolean noAggregates = FALSE);
  ...
  inline char *getDefragBuffer() { return defragBuffer_; }

  // ClusteredBitmapForIntegers& getClusteredBitMap()
  //  { return bitMap_; }

  // void getCombinedBitMap(ClusteredBitmapForIntegers& finalMap) const;
 ...
 private:
  ...
  inline NABoolean IONotComplete(ExeErrorCode *rc) { return IONotReady(rc, 0, TRUE); };
  ...
  // ClusteredBitmapForIntegers bitMap_;
};

/////////////////////////////////////////////////////////////////////////////
// inline functions of Cluster
/////////////////////////////////////////////////////////////////////////////
inline Cluster::ClusterState Cluster::getState() const { return state_; };
...
inline NABoolean Cluster::hasBitMap() { return (outerBitMap_ != NULL); };
...
inline NABoolean Cluster::endOfCluster() {
  ...
  return returnOverflowRows_ && seqIDIndex_ == 0 && readHandle_->endOfSequence();
};

inline NABoolean Cluster::IONotReady(ExeErrorCode *rc, UInt32 readSeqID, NABoolean checkAll) {
...
inline NABoolean Cluster::bitMapEnable() const { return bitMapEnable_; };

inline NABoolean Cluster::isInner() const { return isInner_; };

inline Cluster *Cluster::getNext() const { return next_; };
...
inline NABoolean Bucket::insert(atp_struct *newEntry, ex_expr *moveExpr, SimpleHashValue hashValue, ExeErrorCode *rc,
                                NABoolean skipMemoryCheck) {
  NABoolean isInserted = innerCluster_->insert(newEntry, moveExpr, hashValue, rc, skipMemoryCheck);
  if (isInserted) rowCount_++;
  return isInserted;
};
