...
class VersioningSPContextBase {
 public:
  VersioningSPContextBase(void) : rowIndex_(0), eValue_(NULL) {
    // set the type to a null string
    eType_[0] = 0;
  };

  virtual ~VersioningSPContextBase(void) {
    if (eValue_ != NULL) NADELETEBASIC(eValue_, STMTHEAP);
  };

  // Accessors
  const char *getInputType(void) const { return eType_; };
  const char *getInputValue(void) const { return eValue_; };

  CollIndex getRowIndex(void) const { return rowIndex_; };

  // Mutators
  void setRowIndex(const CollIndex rowIndex) { rowIndex_ = rowIndex; };
...
class VersionInfoSPContext : public VersioningSPContextBase {
 public:
  VersionInfoSPContext(void) : outputVersion_(COM_VERS_UNKNOWN), nodeSet_(CatProcess.getGlobalNodeSet()){};
  virtual ~VersionInfoSPContext(void){};

  // Accessors
  COM_VERSION getOutputVersion(void) const { return outputVersion_; };
  ComVersion_DerivedNodeSet &getNodeSet(void) { return nodeSet_; };

  // Mutators
  void setOutputVersion(const COM_VERSION version) { outputVersion_ = version; };

  // Input validation
  VersionInfoSPInputType validateInputType(void);

  // Generic input handling
  const CatROObject *handleInputValue(const ComAnsiNameSpace nameSpace, const ComObjectType expectedObjectType,
...
class RelatednessSPContext : public VersioningSPContextBase {
 public:
  ...
  // Accessors
  CatRelatedItemSet &getRelatedItemSet(void) { return relatedItemSet_; };
