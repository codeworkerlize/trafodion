...
class QRMessageObj : public IpcMessageObj {
 public:
  virtual ~QRMessageObj() {
    QRLogger::log(CAT_SQL_COMP_QR_IPC, LL_DEBUG, "---QRMessageObj dealloc: allocCount_ = %d", --allocCount_);
  }
   ...
   */
  void setRequestType(QRMessageTypeEnum requestType) { requestType_ = requestType; }
 ...
 protected:
   ...
   */
  QRMessageObj(IpcMessageObjType objType, IpcMessageObjVersion version) : IpcMessageObj(objType, version) {
    QRLogger::log(CAT_SQL_COMP_QR_IPC, LL_DEBUG, "+++QRMessageObj alloc: allocCount_ = %d", ++allocCount_);
  }
...
class QRSimpleMessageObj : public QRMessageObj {
 public:
   ...
   */
  virtual IpcMessageObjSize packedLength() { return sizeof(*this); }
...
class QRXmlMessageObj : public QRMessageObj {
 public:
   ...
   * @return
   */
  QRXmlMessageObj(const NAString *xml = NULL, IpcMessageObjType objType = UNSPECIFIED_QR_MESSAGE,
                  IpcMessageObjVersion version = QR_XML_MSG_VERSION)
      : QRMessageObj(objType, version) {
    setData(xml);
  }
   ...
   */
  IpcMessageObjSize getLength() { return strlen(xmlText_); }

  /**
   * Returns a pointer to the XML text contained by this object.
   * @return XML text.
   */
  char *getData() { return xmlText_; }

  /**
   * Writes the content of this message object to the log file.
   * @param logger The logger to use.
   */
  void log(QRLogger &logger) { logger.log(CAT_SQL_COMP_QR_IPC, LL_DEBUG, xmlText_); }
...
class QRStatusMessageObj : public QRMessageObj {
 public:
   ...
   * @param version The object's version.
   */
  QRStatusMessageObj(QRRequestResult result = Success, IpcMessageObjVersion version = QR_STATUS_MSG_VERSION)
      : QRMessageObj(STATUS_RESPONSE, version), statusCode_(result) {}
   ...
   */
  QRRequestResult getStatusCode() const { return statusCode_; }
   ...
   */
  void setStatusCode(QRRequestResult statusCode) { statusCode_ = statusCode; }
...
class QRMessageStream : public IpcMessageStream {
 public:
   ...
   */
  QRMessageStream(IpcEnvironment *env, const NAString &thisEnd, NAMemory *heap = NULL,
      ...
      : IpcMessageStream(env, msgType, QR_MSG_STREAM_VERSION, 0, TRUE),
        ...
        otherEnd_(heap) {
    QRLogger::log(CAT_SQL_COMP_QR_IPC, LL_DEBUG, "+++QRMessageStream alloc: allocCount_ = %d", ++allocCount_);
  }
