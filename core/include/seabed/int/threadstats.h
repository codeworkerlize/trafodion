...
class SB_Thread_Lock_Stats {
 public:
  SB_Thread_Lock_Stats()
      : iv_mutex_count(0),
        iv_mutex_time_get_lock(0),
        iv_spin_count(0),
        iv_spin_time_get_lock(0),
        iv_mutex_times_inx_get_lock(0),
        iv_mutex_times_inx_hold_lock(0),
        iv_spin_times_inx_get_lock(0),
        iv_spin_times_inx_hold_lock(0) {}

  ~SB_Thread_Lock_Stats() {}

  SB_Thread_Lock_Counter inc(SB_Thread_Lock_Counter *pp_counter) { return __sync_fetch_and_add_8(pp_counter, 1); }
  ...
  void print_bt() {
    ...
    SB_backtrace2(MAX_PRINT_COUNT, MAX_PRINT_SIZE, &lv_count, (char *)la_recs);
    for (lv_inx = 0; lv_inx < lv_count; lv_inx++) printf("%s\n", la_recs[lv_inx]);
  }
    ...
    if (!lv_readlink) {
      ...
      if (lv_size != -1) {
        ...
        if (lp_p != NULL)
          ...
          lp_p = la_self;
        if (strcmp(lp_p, EXE) == 0) lv_match = true;
      }
    }
    return lv_match;
  }
#else
  bool check_exe() { return true; }
#endif
  ...
  SB_Thread_Lock_Time get_lock_time_set_mutex_get_lock(SB_Thread_Lock_Time pv_start) {
    ...
    lv_stop = static_cast<SB_Thread_Lock_Time>(lv_tod.tv_sec) * 1000000 + lv_tod.tv_usec;
    lv_elapsed = lv_stop - pv_start;
    lv_slot = __sync_fetch_and_add_4(&iv_mutex_times_inx_get_lock, 1);
    lv_slot %= MAX_COUNT;
    ia_mutex_times_get_lock[lv_slot] = (static_cast<SB_Thread_Lock_Time>(GETTID()) << 32) + lv_elapsed;
  ...
  void get_lock_time_set_mutex_hold_lock(SB_Thread_Lock_Time pv_start) {
    ...
    lv_slot %= MAX_COUNT;
    ia_mutex_times_hold_lock[lv_slot] = (static_cast<SB_Thread_Lock_Time>(GETTID()) << 32) + lv_elapsed;
  ...
  SB_Thread_Lock_Time get_lock_time_set_spin_get_lock(SB_Thread_Lock_Time pv_start) {
    ...
    lv_slot %= MAX_COUNT;
    ia_spin_times_get_lock[lv_slot] = (static_cast<SB_Thread_Lock_Time>(GETTID()) << 32) + lv_elapsed;
  ...
  void get_lock_time_set_spin_hold_lock(SB_Thread_Lock_Time pv_start) {
    ...
    lv_slot %= MAX_COUNT;
    ia_spin_times_hold_lock[lv_slot] = (static_cast<SB_Thread_Lock_Time>(GETTID()) << 32) + lv_elapsed;
  ...
  SB_Thread_Lock_Time time_start() {
    ...
    gettimeofday(&lv_tod, NULL);
    lv_ret = static_cast<SB_Thread_Lock_Time>(lv_tod.tv_sec) * 1000000 + lv_tod.tv_usec;
    return lv_ret;
  }
