...
//# pragma GCC diagnostic ignored "-Wuninitialized"
   ...
   */
  inline static void *smartRealloc(void *p, const size_t currentSize, const size_t currentCapacity,
    ...
    fbstring_core(const fbstring_core &rhs, NAMemory *h) : heap_(h) {
      ...
      if (rhs.category() == isSmall) {
        ...
        } else  // do not share large string in different heap
        {
          ...
          auto const newRC = RefCounted::create(rhs.data(), &effectiveCapacity, heap_, false);
   ...
   */
    ...
    fbstring_core(const Char *const data, const size_t size, NAMemory *h) : heap_(h) {
      ...
      if (size <= maxSmallSize) {
        ...
        auto const newRC = RefCounted::create(data, &effectiveCapacity, heap_, false);
    ...
    Char *mutable_data() {
      ...
      if (c == isLarge && RefCounted::refs(ml_.data_) > 1) {
        ...
        fbstring_detail::pod_copy(ml_.data_, ml_.data_ + ml_.size_ + 1, newRC->data_);
        RefCounted::decrementRefs(ml_.data_, heap_);
        ml_.data_ = newRC->data_;
...
#ifdef FBSTRING_PERVERSE
      if (c == isSmall) {
        assert(small_[smallSize()] == TERMINATOR || smallSize() == maxSmallSize || small_[smallSize()] == '\0');
        small_[smallSize()] = '\0';
        return small_;
      }
      assert(/*c == isMedium ||*/ c == isLarge);
      ...
      if (category() == isLarge) {
        ...
        if (RefCounted::refs(ml_.data_) > 1) {
          ...
          if (minCapacity > ml_.capacity()) {
            ...
            auto const newRC = RefCounted::reallocate(ml_.data_, ml_.size_, ml_.capacity(), minCapacity, heap_);
            ml_.data_ = newRC->data_;
            ml_.capacity_ = minCapacity | isLarge;
            writeTerminator();
          }
    ...
    size_t size() const { return category() == isSmall ? smallSize() : ml_.size_; }
    ...
    bool isShared() const { return category() == isLarge && RefCounted::refs(ml_.data_) > 1; }
  ...
  enum { TERMINATOR = '\0' };
   ...
   private:
    ...
    struct MediumLarge {
     ...
     public:
      size_t capacity() const { return capacity_ & capacityExtractMask; }
    };
    ...
    struct RefCounted {
      ...
      static RefCounted *fromData(Char *p) {
        return static_cast<RefCounted *>(
            static_cast<void *>(static_cast<unsigned char *>(static_cast<void *>(p)) - sizeof(refCount_)));
      }
      ...
      static RefCounted *create(size_t *size, NAMemory *h, bool failureIsFatal = true) {
        // Don't forget to allocate one extra Char for the terminating
        // null. In this case, however, one Char is already part of the
        // struct.
        const size_t allocSize = goodMallocSize(sizeof(RefCounted) + *size * sizeof(Char));
        ...
        } else if (failureIsFatal)
          abort();

        return result;
      }

      static RefCounted *create(const Char *data, size_t *size, NAMemory *h, bool failureIsFatal = true) {
        const size_t effectiveSize = *size;
        auto result = create(size, h, failureIsFatal);
        if (result) {
      ...
      static RefCounted *reallocate(Char *const data, const size_t currentSize, const size_t currentCapacity,
        ...
        // null. In this case, however, one Char is already part of the
        // struct.
        auto result = static_cast<RefCounted *>(smartRealloc(dis, sizeof(RefCounted) + currentSize * sizeof(Char),
   ...
   public:
    ...
    size_t smallSize() const {
      ...
      return static_cast<size_t>(maxSmallSize) - static_cast<size_t>(small_[maxSmallSize]);
    }
...
template <typename E, class T = std::char_traits<E>, class A = std::allocator<E>, class Storage = fbstring_core<E> >
  ...
  class basic_fbstring {
    static void enforce(bool condition, void (*throw_exc)(const char *), const char *msg) {
    ...
    struct Invariant {
#ifndef NDEBUG
      explicit Invariant(const basic_fbstring &s) : s_(s) { assert(s_.isSane()); }
      ~Invariant() { assert(s_.isSane()); }
   ...
   public:
    ...
    //}
    explicit basic_fbstring(NAMemory *h = 0) : store_(h) {}

    basic_fbstring(const basic_fbstring &str, NAMemory *h = 0) : store_(str.store_, h) {}
    ...
    */
    ...
    /* implicit */ basic_fbstring(const std::string &str, NAMemory *h = 0) : store_(str.data(), str.size(), h) {}
#endif

    basic_fbstring(const basic_fbstring &str, size_type pos, size_type n = npos, NAMemory *h = 0) : store_(h) {
      assign(str, pos, n);
    }
    ...
    basic_fbstring(const value_type *s, size_type n, NAMemory *h = 0) : store_(s, n, h) {}

    basic_fbstring(size_type n, value_type c, NAMemory *h = 0) : store_(h) {
      assert(n >= 0);
    ...
    template <class InIt>
    basic_fbstring(InIt begin, InIt end, NAMemory *h) : store_(h) {
      assign(begin, end);
    }

    // Specialization for const char*, const char*
    basic_fbstring(const value_type *b, const value_type *e, NAMemory *h = 0) : store_(b, e - b, h) {}
    ...
    */
    ...
    basic_fbstring &operator=(const std::string &rhs) { return assign(rhs.data(), rhs.size()); }
  ...
  const basic_fbstring &toStdString() const { return *this; }
#endif
    const Storage &store() const { return store_; }

    NAMemory *heap() const { return store_.heap(); }

    size_t get_alloc_size() const { return store_.get_alloc_size(); }
    ...
    basic_fbstring &operator=(std::initializer_list<value_type> il) { return assign(il.begin(), il.end()); }
    ...
    basic_fbstring &append(const value_type *s) { return append(s, traits_type::length(s)); }
    ...
    basic_fbstring &append(std::initializer_list<value_type> il) { return append(il.begin(), il.end()); }
    ...
    basic_fbstring &assign(const value_type *s) { return assign(s, traits_type::length(s)); }

    basic_fbstring &assign(std::initializer_list<value_type> il) { return assign(il.begin(), il.end()); }
    ...
    basic_fbstring &insert(size_type pos1, const basic_fbstring &str) { return insert(pos1, str.data(), str.size()); }
    ...
    basic_fbstring &insert(size_type pos, const value_type *s) { return insert(pos, s, traits_type::length(s)); }
   ...
   private:
    ...
    iterator insertImplDiscr(const_iterator p, size_type n, value_type c, Selector<1>) {
      Invariant checker(*this);
      (void)checker;
      ...
      if (n < size_type(oldEnd - p)) {
        ...
        fbstring_detail::pod_move(&*p, &*oldEnd - n, begin() + pos + n);
    ...
    iterator insertImpl(const_iterator i, FwdIterator s1, FwdIterator s2, std::forward_iterator_tag) {
      ...
      if (pos + n2 <= size()) {
        ...
        std::copy(const_reverse_iterator(tailBegin), const_reverse_iterator(i), reverse_iterator(tailBegin + n2));
        ...
        fbstring_detail::pod_copy(data() + pos, data() + old_size, begin() + old_size + newElems);
   ...
   private:
    basic_fbstring &replaceImplDiscr(iterator i1, iterator i2, const value_type *s, size_type n, Selector<2>) {
      assert(i1 <= i2);
    ...
    basic_fbstring &replaceImplDiscr(iterator i1, iterator i2, size_type n2, value_type c, Selector<1>) {
    ...
    template <class InputIter>
    basic_fbstring &replaceImplDiscr(iterator i1, iterator i2, InputIter b, InputIter e, Selector<0>) {
   ...
   private:
    ...
    bool replaceAliased(iterator i1, iterator i2, FwdIterator s1, FwdIterator s2, P *) {
      return false;
    }

    template <class FwdIterator>
    bool replaceAliased(iterator i1, iterator i2, FwdIterator s1, FwdIterator s2, value_type *) {
   ...
   public:
    ...
    size_type find(const basic_fbstring &str, size_type pos = 0) const { return find(str.data(), pos, str.length()); }
    ...
    size_type find(const value_type *s, size_type pos = 0) const { return find(s, pos, traits_type::length(s)); }
    ...
    size_type rfind(const value_type *s, size_type pos = npos) const { return rfind(s, pos, traits_type::length(s)); }
  ...
  // C++11 21.4.8.1/2
  template <typename E, class T, class A, class S>
  inline basic_fbstring<E, T, A, S> operator+(basic_fbstring<E, T, A, S> &&lhs, const basic_fbstring<E, T, A, S> &rhs) {
    return std::move(lhs.append(rhs));
  }

  // C++11 21.4.8.1/3
  template <typename E, class T, class A, class S>
  inline basic_fbstring<E, T, A, S> operator+(const basic_fbstring<E, T, A, S> &lhs, basic_fbstring<E, T, A, S> &&rhs) {
  ...
  // C++11 21.4.8.1/4
  template <typename E, class T, class A, class S>
  inline basic_fbstring<E, T, A, S> operator+(basic_fbstring<E, T, A, S> &&lhs, basic_fbstring<E, T, A, S> &&rhs) {
    return std::move(lhs.append(rhs));
  }

  template <typename E, class T, class A, class S>
  inline basic_fbstring<E, T, A, S> operator+(const typename basic_fbstring<E, T, A, S>::value_type *lhs,
                                              const basic_fbstring<E, T, A, S> &rhs) {
    //
    basic_fbstring<E, T, A, S> result;
    const typename basic_fbstring<E, T, A, S>::size_type len = basic_fbstring<E, T, A, S>::traits_type::length(lhs);
  ...
  template <typename E, class T, class A, class S>
  inline basic_fbstring<E, T, A, S> operator+(typename basic_fbstring<E, T, A, S>::value_type lhs,
                                              const basic_fbstring<E, T, A, S> &rhs) {
    basic_fbstring<E, T, A, S> result;
  ...
  template <typename E, class T, class A, class S>
  inline bool operator==(const basic_fbstring<E, T, A, S> &lhs, const basic_fbstring<E, T, A, S> &rhs) {
  ...
  inline std::basic_istream<typename basic_fbstring<E, T, A, S>::value_type,
                            typename basic_fbstring<E, T, A, S>::traits_type>
      &getline(std::basic_istream<typename basic_fbstring<E, T, A, S>::value_type,
                                  typename basic_fbstring<E, T, A, S>::traits_type> &
                   is,
               basic_fbstring<E, T, A, S> & str, typename basic_fbstring<E, T, A, S>::value_type delim) {
  ...
  const typename basic_fbstring<E1, T, A, S>::size_type basic_fbstring<E1, T, A, S>::npos =
  ...
  template <typename E, class T, class A, class S>
  inline bool operator==(const basic_fbstring<E, T, A, S> &lhs, const std::string &rhs) {
    return lhs.compare(0, lhs.size(), rhs.data(), rhs.size()) == 0;
  }

  template <typename E, class T, class A, class S>
  inline bool operator==(const std::string &lhs, const basic_fbstring<E, T, A, S> &rhs) {
    return rhs == lhs;
  }
