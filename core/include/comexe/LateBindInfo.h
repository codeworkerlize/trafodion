...
class LateNameInfo : public NAVersionedObject {
 public:
  LateNameInfo() : NAVersionedObject(-1) {
    flags_ = 0;
    runtimeFlags_ = 0;
    varName_[0] = 0;
 ...
 private:
  void setCompileNameCompMode(short v) { (v ? flags_ |= COMP_NAME_STR_PTR : flags_ &= ~COMP_NAME_STR_PTR); };
  ...
  enum RunTimeFlags {
    ANSI_NAME_CHANGE = 0x0001,
    IGNORE_TS = 0x0002,
    VIEW_NAME_CHANGE = 0x0004,
...
class LateNameInfoList : public NAVersionedObject {
 public:
  LateNameInfoList() : NAVersionedObject(-1) {
    flags_ = 0;
    numEntries_ = 0;
  };
  ...
  void setViewPresent(short v) { (v ? flags_ |= VIEW_PRESENT : flags_ &= ~VIEW_PRESENT); };

  NABoolean envvarsPresent() { return (flags_ & ENVVARS_PRESENT) != 0; };
  void setEnvvarsPresent(short v) { (v ? flags_ |= ENVVARS_PRESENT : flags_ &= ~ENVVARS_PRESENT); };

  NABoolean variablePresent() { return (flags_ & VARIABLE_PRESENT) != 0; };
  void setVariablePresent(short v) { (v ? flags_ |= VARIABLE_PRESENT : flags_ &= ~VARIABLE_PRESENT); };
...
class TrafQuerySimilarityInfo : public NAVersionedObject {
 public:
  ...
  NABoolean disableAutoRecomp() { return ((flags_ & DISABLE_AUTO_RECOMP) != 0); };
  void setDisableAutoRecomp(NABoolean v) { (v ? flags_ |= DISABLE_AUTO_RECOMP : flags_ &= ~DISABLE_AUTO_RECOMP); };

 private:
  enum Flags { DISABLE_SIM_CHECK = 0x0002, DISABLE_AUTO_RECOMP = 0x0004 };
