...
class SqlTableOpenInfo : public NAVersionedObject {
 public:
  enum AccessMode { READ_, WRITE_, READWRITE_ };
  ...
  NABoolean isMXMetadataTable() { return (otherFlags_ & IS_MX_METADATA_TABLE) != 0; }

  NABoolean isValidateViewsAtOpenTime() { return (otherFlags_ & VALIDATE_VIEWS_AT_OPEN_TIME) != 0; }

  NABoolean validateNumIndexesAtOpenTime() { return (otherFlags_ & VALIDATE_NUM_INDEXES_AT_OPEN_TIME) != 0; }

  NABoolean isHbase() { return (otherFlags_ & IS_HBASE) != 0; }

  NABoolean ddlValidationRequired() { return (otherFlags_ & DDL_VALIDATION_REQUIRED) != 0; }

  NABoolean isUserBaseTable() { return (otherFlags_ & USER_BASE_TABLE) != 0; }

  void setUserBaseTable(NABoolean v) { (v ? otherFlags_ |= USER_BASE_TABLE : otherFlags_ &= ~USER_BASE_TABLE); }

  void setShareOpens(NABoolean v) { (v ? openFlags_ |= SHARE_OPENS : openFlags_ &= ~SHARE_OPENS); }
  void setExecutorOpen(NABoolean v) { (v ? openFlags_ |= EXECUTOR_OPEN : openFlags_ &= ~EXECUTOR_OPEN); }
  void setOpenAllParts(NABoolean v) { (v ? openFlags_ |= OPEN_ALL_PARTS : openFlags_ &= ~OPEN_ALL_PARTS); }
  void setUtilityOpen(NABoolean v) { (v ? openFlags_ |= UTILITY_OPEN : openFlags_ &= ~UTILITY_OPEN); }
  void setDistributeOpens(NABoolean v) { (v ? openFlags_ |= DISTRIBUTE_OPENS : openFlags_ &= ~DISTRIBUTE_OPENS); }
  void setReuseOpens(NABoolean v) { (v ? openFlags_ |= REUSE_OPENS : openFlags_ &= ~REUSE_OPENS); }
  ...
  void setLocationSpecified(NABoolean v) { (v ? otherFlags_ |= LOCATION : otherFlags_ &= ~LOCATION); }
  void setIsView(NABoolean v) { (v ? otherFlags_ |= IS_MX_VIEW : otherFlags_ &= ~IS_MX_VIEW); }
  void setValidateTimestamp(NABoolean v) { (v ? otherFlags_ |= VALIDATE_TS : otherFlags_ &= ~VALIDATE_TS); }
  void setNoWaited(NABoolean v) { (v ? otherFlags_ |= NO_WAIT : otherFlags_ &= ~NO_WAIT); }
  void setLockWait(NABoolean v) { (v ? otherFlags_ |= LOCK_WAIT : otherFlags_ &= ~LOCK_WAIT); }
  void setCheckSecurity(NABoolean v) { (v ? otherFlags_ |= SECURITY : otherFlags_ &= ~SECURITY); }
  void setIsAudited(NABoolean v) { (v ? otherFlags_ |= AUDITED : otherFlags_ &= ~AUDITED); }
  void setIsIndex(NABoolean v) { (v ? otherFlags_ |= IS_MX_INDEX : otherFlags_ &= ~IS_MX_INDEX); }
  void setSpecialTable(NABoolean v) { (v ? otherFlags_ |= SPECIAL_TABLE : otherFlags_ &= ~SPECIAL_TABLE); }
  void setIsUDRSurrogate(NABoolean v) { (v ? otherFlags_ |= IS_UDR_SURROGATE : otherFlags_ &= ~IS_UDR_SURROGATE); }
  void setIsReverseScan(NABoolean v) { (v ? otherFlags_ |= REVERSE_SCAN : otherFlags_ &= ~REVERSE_SCAN); }
  void setIsNSAOperation(NABoolean v) { (v ? otherFlags_ |= IS_NSA_OPERATION : otherFlags_ &= ~IS_NSA_OPERATION); }
  ...
  inline void setSubjectTable(NABoolean v) { (v ? otherFlags_ |= SUBJECT_TABLE : otherFlags_ &= ~SUBJECT_TABLE); }
  void setValidateViewsAtOpenTime(NABoolean v) {
    (v ? otherFlags_ |= VALIDATE_VIEWS_AT_OPEN_TIME : otherFlags_ &= ~VALIDATE_VIEWS_AT_OPEN_TIME);
  }

  void setValidateNumIndexesAtOpenTime(NABoolean v) {
    (v ? otherFlags_ |= VALIDATE_NUM_INDEXES_AT_OPEN_TIME : otherFlags_ &= ~VALIDATE_NUM_INDEXES_AT_OPEN_TIME);
  }

  void setIsHbase(NABoolean v) { (v ? otherFlags_ |= IS_HBASE : otherFlags_ &= ~IS_HBASE); }

  void setDDLValidationRequired(NABoolean v) {
    (v ? otherFlags_ |= DDL_VALIDATION_REQUIRED : otherFlags_ &= ~DDL_VALIDATION_REQUIRED);
  }
  ...
  void setSelectAccess(UInt32 flag = 1) { accessFlags_.select_ = flag ? 1 : 0; }
  void setInsertAccess(UInt32 flag = 1) { accessFlags_.insert_ = flag ? 1 : 0; }
  void setUpdateAccess(UInt32 flag = 1) { accessFlags_.update_ = flag ? 1 : 0; }
  void setDeleteAccess(UInt32 flag = 1) { accessFlags_.delete_ = flag ? 1 : 0; }
  ...
  char *getPartNameArray() { return partNameArray_.getPointer(); }
  void setPartNameArray(char *buf) { partNameArray_ = buf; }

  void setNumEntriesInPartNameArray(unsigned short numEntries) { numPartsInPartNameArray_ = numEntries; }

  unsigned short getNumEntriesInPartNameArray(void) { return numPartsInPartNameArray_; }
  ...
  UInt32 getExpectedFlags() { return expectedFlags_; }
 ...
 private:
  enum OpenFlags {
    SHARE_OPENS = 0x0001,
    EXECUTOR_OPEN = 0x0002,
    OPEN_ALL_PARTS = 0x0004,
    SQL_SECURED = 0x0008,
    PRIV_OPEN = 0x0010,
    LICENSED_PROC = 0x0020,
    UTILITY_OPEN = 0x0040,
  ...
  enum OtherFlags {
    ...
    IS_HBASE = 0x100000,
    DDL_VALIDATION_REQUIRED = 0x200000,
    USER_BASE_TABLE = 0x400000
  };
  ...
  char fillersSqlTableOpenInfo_[8];  // 104-111
};
...
// --------------------------------------------------------------------------
typedef NAVersionedObjectPtrArrayTempl<SqlTableOpenInfoPtr> SqlTableOpenInfoPtrPtr;
