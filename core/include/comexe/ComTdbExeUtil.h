...
// class ComTdbExeUtil
////////////////////////////////////////////////////////////////////
static const ComTdbVirtTableColumnInfo exeUtilVirtTableColumnInfo[] = {{"UTIL_OUTPUT",
                                                                        0,
                                                                        COM_USER_COLUMN,
                                                                        REC_BYTE_V_ASCII,
                                                                        2000,
...
class ComTdbExeUtil : public ComTdbGenericUtil {
 ...
 public:
  ...
  void setExplOptionsStr(char *expl) { explOptionsStr_ = expl; }
  ...
  void setLobV2(NABoolean v) { (v ? flags_ |= LOB_VERSION2 : flags_ &= ~LOB_VERSION2); };
...
static const ComTdbVirtTableColumnInfo exeUtilDisplayExplainVirtTableColumnInfo[] = {{"EXPLAIN OUTPUT",
                                                                                      0,
                                                                                      COM_USER_COLUMN,
                                                                                      REC_BYTE_V_ASCII,
                                                                                      4000,
...
class ComTdbExeUtilDisplayExplain : public ComTdbExeUtil {
 ...
 public:
  ...
  void setOptionE(NABoolean v) { (v ? flags_ |= OPTION_E : flags_ &= ~OPTION_E); };
  void setOptionF(NABoolean v) { (v ? flags_ |= OPTION_F : flags_ &= ~OPTION_F); };
  void setOptionM(NABoolean v) { (v ? flags_ |= OPTION_M : flags_ &= ~OPTION_M); };
  void setOptionN(NABoolean v) { (v ? flags_ |= OPTION_N : flags_ &= ~OPTION_N); };
  void setOptionC(NABoolean v) { (v ? flags_ |= OPTION_C : flags_ &= ~OPTION_C); };
  void setOptionP(NABoolean v) { (v ? flags_ |= OPTION_P : flags_ &= ~OPTION_P); };
...
class ComTdbExeUtilDisplayExplainComplex : public ComTdbExeUtil {
 ...
 public:
  enum ExplainType { CREATE_TABLE_, CREATE_INDEX_, CREATE_MV_, CREATE_TABLE_AS };
  ...
  virtual const char *getNodeName() const { return "DISPLAY_EXPLAIN_COMPLEX"; };

  void setIsVolatile(NABoolean v) { (v ? flags_ |= IS_VOLATILE : flags_ &= ~IS_VOLATILE); };
  NABoolean isVolatile() { return (flags_ & IS_VOLATILE) != 0; };

  void setIsShowddl(NABoolean v) { (v ? flags_ |= IS_SHOWDDL : flags_ &= ~IS_SHOWDDL); };
  ...
  void setLoadIfExists(NABoolean v) { (v ? flags_ |= LOAD_IF_EXISTS : flags_ &= ~LOAD_IF_EXISTS); };
...
class ComTdbExeUtilMaintainObject : public ComTdbExeUtil {
 ...
 public:
  enum ObjectType { TABLE_ = 0, INDEX_, MV_, MVGROUP_, MV_INDEX_, MV_LOG_, CATALOG_, SCHEMA_, CLEAN_MAINTAIN_, NOOP_ };
...
#define MAX_MULTI_TABLES 100
  ...
  void setParams(NABoolean reorgTable, NABoolean reorgIndex, NABoolean updStatsTable, NABoolean updStatsMvlog,
                 ...
                 NABoolean reorgMvgroup, NABoolean reorgMvs, NABoolean reorgMvsIndex, NABoolean continueOnError,
                 NABoolean cleanMaintainCIT, NABoolean getSchemaLabelStats, NABoolean getLabelStats,
                 NABoolean getTableLabelStats, NABoolean getIndexLabelStats, NABoolean getLabelStatsIncIndexes,
                 NABoolean getLabelStatsIncInternal, NABoolean getLabelStatsIncRelated);

  void setOptionsParams(char *reorgTableOptions, char *reorgIndexOptions, char *updStatsTableOptions,
                        char *updStatsMvlogOptions, char *updStatsMvsOptions, char *updStatsMvgroupOptions,
                        char *refreshMvgroupOptions, char *refreshMvsOptions, char *reorgMvgroupOptions,
                        char *reorgMvsOptions, char *reorgMvsIndexOptions, char *cleanMaintainCITOptions);

  void setLists(Queue *indexList, Queue *refreshMvgroupList, Queue *refreshMvsList, Queue *reorgMvgroupList,
                Queue *reorgMvsList, Queue *reorgMvsIndexList, Queue *updStatsMvgroupList, Queue *updStatsMvsList,
  ...
  virtual const char *getNodeName() const { return "MAINTAIN_OBJECT"; };

  void setInitializeMaintain(NABoolean v) { (v ? flags_ |= INITIALIZE_MAINTAIN : flags_ &= ~INITIALIZE_MAINTAIN); };
  ...
  void setDropMaintain(NABoolean v) { (v ? flags_ |= DROP_MAINTAIN : flags_ &= ~DROP_MAINTAIN); };
  NABoolean dropMaintain() { return (flags_ & DROP_MAINTAIN) != 0; };

  void setCreateView(NABoolean v) { (v ? flags_ |= CREATE_VIEW : flags_ &= ~CREATE_VIEW); };
  NABoolean createView() { return (flags_ & CREATE_VIEW) != 0; };

  void setDropView(NABoolean v) { (v ? flags_ |= DROP_VIEW : flags_ &= ~DROP_VIEW); };
  NABoolean dropView() { return (flags_ & DROP_VIEW) != 0; };

  void setReorgTable(NABoolean v) { (v ? flags_ |= REORG_TABLE : flags_ &= ~REORG_TABLE); };
  NABoolean reorgTable() { return (flags_ & REORG_TABLE) != 0; };

  void setReorgIndex(NABoolean v) { (v ? flags_ |= REORG_INDEX : flags_ &= ~REORG_INDEX); };
  NABoolean reorgIndex() { return (flags_ & REORG_INDEX) != 0; };

  void setUpdStatsTable(NABoolean v) { (v ? flags_ |= UPD_STATS_TABLE : flags_ &= ~UPD_STATS_TABLE); };
  NABoolean updStatsTable() { return (flags_ & UPD_STATS_TABLE) != 0; };

  void setUpdStatsMvlog(NABoolean v) { (v ? flags_ |= UPD_STATS_MVLOG : flags_ &= ~UPD_STATS_MVLOG); };
  NABoolean updStatsMvlog() { return (flags_ & UPD_STATS_MVLOG) != 0; };

  void setRefreshMvs(NABoolean v) { (v ? flags_ |= REFRESH_MVS : flags_ &= ~REFRESH_MVS); };
  NABoolean refreshMvs() { return (flags_ & REFRESH_MVS) != 0; };

  void setRefreshMvgroup(NABoolean v) { (v ? flags_ |= REFRESH_MVGROUP : flags_ &= ~REFRESH_MVGROUP); };
  NABoolean refreshMvgroup() { return (flags_ & REFRESH_MVGROUP) != 0; };

  void setReorgMvgroup(NABoolean v) { (v ? flags_ |= REORG_MVGROUP : flags_ &= ~REORG_MVGROUP); };
  NABoolean reorgMvgroup() { return (flags_ & REORG_MVGROUP) != 0; };

  void setReorgMvs(NABoolean v) { (v ? flags_ |= REORG_MVS : flags_ &= ~REORG_MVS); };
  NABoolean reorgMvs() { return (flags_ & REORG_MVS) != 0; };

  void setReorgMvsIndex(NABoolean v) { (v ? flags_ |= REORG_MVS_INDEX : flags_ &= ~REORG_MVS_INDEX); };
  NABoolean reorgMvsIndex() { return (flags_ & REORG_MVS_INDEX) != 0; };

  void setUpdStatsMvs(NABoolean v) { (v ? flags_ |= UPD_STATS_MVS : flags_ &= ~UPD_STATS_MVS); };
  NABoolean updStatsMvs() { return (flags_ & UPD_STATS_MVS) != 0; };

  void setUpdStatsMvgroup(NABoolean v) { (v ? flags_ |= UPD_STATS_MVGROUP : flags_ &= ~UPD_STATS_MVGROUP); };
  NABoolean updStatsMvgroup() { return (flags_ & UPD_STATS_MVGROUP) != 0; };

  void setContinueOnError(NABoolean v) { (v ? flags_ |= CONTINUE_ON_ERROR : flags_ &= ~CONTINUE_ON_ERROR); };
  NABoolean continueOnError() { return (flags_ & CONTINUE_ON_ERROR) != 0; };

  void setDisplay(NABoolean v) { (v ? flags_ |= DISPLAY : flags_ &= ~DISPLAY); };
  NABoolean display() { return (flags_ & DISPLAY) != 0; };

  void setDisplayDetail(NABoolean v) { (v ? flags_ |= DISPLAY_DETAIL : flags_ &= ~DISPLAY_DETAIL); };
  NABoolean displayDetail() { return (flags_ & DISPLAY_DETAIL) != 0; };

  void setDoSpecifiedTask(NABoolean v) { (v ? flags_ |= DO_SPECIFIED_TASK : flags_ &= ~DO_SPECIFIED_TASK); };
  NABoolean doSpecifiedTask() { return (flags_ & DO_SPECIFIED_TASK) != 0; };

  void setSkipRefreshMvs(NABoolean v) { (v ? flags_ |= SKIP_REFRESH_MVS : flags_ &= ~SKIP_REFRESH_MVS); };
  NABoolean skipRefreshMvs() { return (flags_ & SKIP_REFRESH_MVS) != 0; };

  void setSkipReorgMvs(NABoolean v) { (v ? flags_ |= SKIP_REORG_MVS : flags_ &= ~SKIP_REORG_MVS); };
  NABoolean skipReorgMvs() { return (flags_ & SKIP_REORG_MVS) != 0; };

  void setSkipUpdStatsMvs(NABoolean v) { (v ? flags_ |= SKIP_UPD_STATS_MVS : flags_ &= ~SKIP_UPD_STATS_MVS); };
  NABoolean skipUpdStatsMvs() { return (flags_ & SKIP_UPD_STATS_MVS) != 0; };

  void setGetStatus(NABoolean v) { (v ? flags_ |= GET_STATUS : flags_ &= ~GET_STATUS); };
  NABoolean getStatus() { return (flags_ & GET_STATUS) != 0; };

  void setGetDetails(NABoolean v) { (v ? flags_ |= GET_DETAILS : flags_ &= ~GET_DETAILS); };
  NABoolean getDetails() { return (flags_ & GET_DETAILS) != 0; };

  void setRun(NABoolean v) { (v ? flags_ |= RUN : flags_ &= ~RUN); };
  NABoolean run() { return (flags_ & RUN) != 0; };

  void setIfNeeded(NABoolean v) { (v ? flags_ |= IF_NEEDED : flags_ &= ~IF_NEEDED); };
  NABoolean ifNeeded() { return (flags_ & IF_NEEDED) != 0; };

  void setAllSpecified(NABoolean v) { (v ? flags_ |= ALL_SPECIFIED : flags_ &= ~ALL_SPECIFIED); };
  ...
  void setTableLabelStats(NABoolean v) { (v ? flags2_ |= GET_TABLE_LABEL_STATS : flags2_ &= ~GET_TABLE_LABEL_STATS); };
  NABoolean getTableLabelStats() { return (flags2_ & GET_TABLE_LABEL_STATS) != 0; }
  void setIndexLabelStats(NABoolean v) { (v ? flags2_ |= GET_INDEX_LABEL_STATS : flags2_ &= ~GET_INDEX_LABEL_STATS); };
  ...
  void setResetReorgMvs(NABoolean v) { (v ? controlFlags2_ |= RESET_REORG_MVS : controlFlags2_ &= ~RESET_REORG_MVS); };
  ...
  void setCleanMaintainCIT(NABoolean v) { (v ? flags_ |= CLEAN_MAINTAIN_CIT : flags_ &= ~CLEAN_MAINTAIN_CIT); };
  ...
  void setNoControlInfoTable(NABoolean v) { (v ? flags_ |= NO_CONTROL_INFO_TABLE : flags_ &= ~NO_CONTROL_INFO_TABLE); };
  NABoolean noControlInfoTable() { return (flags_ & NO_CONTROL_INFO_TABLE) != 0; };

  void setNoOutput(NABoolean v) { (v ? flags_ |= NO_OUTPUT : flags_ &= ~NO_OUTPUT); };
  ...
  void setShortFormat(NABoolean v) { (v ? formatFlags_ |= SHORT_ : formatFlags_ &= ~SHORT_); };
  NABoolean shortFormat() { return (formatFlags_ & SHORT_) != 0; };

  void setLongFormat(NABoolean v) { (v ? formatFlags_ |= LONG_ : formatFlags_ &= ~LONG_); };
  NABoolean longFormat() { return (formatFlags_ & LONG_) != 0; };

  void setDetailFormat(NABoolean v) { (v ? formatFlags_ |= DETAIL_ : formatFlags_ &= ~DETAIL_); };
  NABoolean detailFormat() { return (formatFlags_ & DETAIL_) != 0; };

  void setTokenFormat(NABoolean v) { (v ? formatFlags_ |= TOKEN_ : formatFlags_ &= ~TOKEN_); };
  NABoolean tokenFormat() { return (formatFlags_ & TOKEN_) != 0; };

  void setCommandFormat(NABoolean v) { (v ? formatFlags_ |= COMMAND_ : formatFlags_ &= ~COMMAND_); };
 ...
 protected:
  ...
  enum GetStatsFormat { SHORT_ = 0x0001, LONG_ = 0x0002, DETAIL_ = 0x0004, TOKEN_ = 0x0008, COMMAND_ = 0x0010 };
...
class ComTdbExeUtilLoadVolatileTable : public ComTdbExeUtil {
 ...
 public:
  ...
  virtual const char *getNodeName() const { return "LOAD_VOLATILE_TABLE"; };
...
class ComTdbExeUtilCleanupVolatileTables : public ComTdbExeUtil {
 ...
 public:
  ...
  virtual const char *getNodeName() const { return "CLEANUP_VOLATILE_TABLES"; };
...
class ComTdbExeUtilGetVolatileInfo : public ComTdbExeUtil {
 ...
 public:
  ComTdbExeUtilGetVolatileInfo() : ComTdbExeUtil() {}
  ...
  virtual const char *getNodeName() const { return "GET_VOLATILE_INFO"; };

  void setAllSchemas(NABoolean v) { (v ? flags_ |= ALL_SCHEMAS : flags_ &= ~ALL_SCHEMAS); };
  NABoolean allSchemas() { return (flags_ & ALL_SCHEMAS) != 0; };

  void setAllTables(NABoolean v) { (v ? flags_ |= ALL_TABLES : flags_ &= ~ALL_TABLES); };
  ...
  void setVTCatSpecified(NABoolean v) { (v ? flags_ |= VT_CAT_SPECIFIED : flags_ &= ~VT_CAT_SPECIFIED); };
...
class ComTdbExeUtilGetErrorInfo : public ComTdbExeUtil {
 ...
 public:
  ...
  virtual const char *getNodeName() const { return "GET_ERROR_INFO"; };
...
class ComTdbExeUtilCreateTableAs : public ComTdbExeUtil {
 ...
 public:
  ...
  virtual const char *getNodeName() const { return "CREATE_TABLE_AS"; };

  void setLoadIfExists(NABoolean v) { (v ? flags_ |= LOAD_IF_EXISTS : flags_ &= ~LOAD_IF_EXISTS); };
  NABoolean loadIfExists() { return (flags_ & LOAD_IF_EXISTS) != 0; };

  void setNoLoad(NABoolean v) { (v ? flags_ |= NO_LOAD : flags_ &= ~NO_LOAD); };
  NABoolean noLoad() { return (flags_ & NO_LOAD) != 0; };

  void setIsVolatile(NABoolean v) { (v ? flags_ |= IS_VOLATILE : flags_ &= ~IS_VOLATILE); };
  NABoolean isVolatile() { return (flags_ & IS_VOLATILE) != 0; };

  void setDeleteData(NABoolean v) { (v ? flags_ |= DELETE_DATA : flags_ &= ~DELETE_DATA); };
...
class ComTdbExeUtilGetStatistics : public ComTdbExeUtil {
 ...
 public:
  ComTdbExeUtilGetStatistics() : ComTdbExeUtil(), host_(NULL), port_(0), path_(NULL), queryHash_(0L) {}
  ...
  virtual const char *getNodeName() const { return "GET_STATISTICS"; };

  void setCompilerStats(NABoolean v) { (v ? flags_ |= COMPILER_STATS : flags_ &= ~COMPILER_STATS); };
  NABoolean compilerStats() { return (flags_ & COMPILER_STATS) != 0; };

  void setExecutorStats(NABoolean v) { (v ? flags_ |= EXECUTOR_STATS : flags_ &= ~EXECUTOR_STATS); };
  NABoolean executorStats() { return (flags_ & EXECUTOR_STATS) != 0; };

  void setOtherStats(NABoolean v) { (v ? flags_ |= OTHER_STATS : flags_ &= ~OTHER_STATS); };
  NABoolean otherStats() { return (flags_ & OTHER_STATS) != 0; };

  void setDetailedStats(NABoolean v) { (v ? flags_ |= DETAILED_STATS : flags_ &= ~DETAILED_STATS); };
  NABoolean detailedStats() { return (flags_ & DETAILED_STATS) != 0; };

  void setOldFormat(NABoolean v) { (v ? flags_ |= OLD_FORMAT : flags_ &= ~OLD_FORMAT); };
  NABoolean oldFormat() { return (flags_ & OLD_FORMAT) != 0; };

  void setShortFormat(NABoolean v) { (v ? flags_ |= SHORT_FORMAT : flags_ &= ~SHORT_FORMAT); };
  NABoolean shortFormat() { return (flags_ & SHORT_FORMAT) != 0; };

  void setTokenizedFormat(NABoolean v) { (v ? flags_ |= TOKENIZED_FORMAT : flags_ &= ~TOKENIZED_FORMAT); };
  NABoolean tokenizedFormat() { return (flags_ & TOKENIZED_FORMAT) != 0; };

  void setDataUsedStats(NABoolean v) { (v ? flags_ |= DATA_USED_STATS : flags_ &= ~DATA_USED_STATS); };
  NABoolean dataUsedStats() { return (flags_ & DATA_USED_STATS) != 0; };

  void setSingleLineFormat(NABoolean v) { (v ? flags_ |= SINGLELINE_FORMAT : flags_ &= ~SINGLELINE_FORMAT); };
...
class ComTdbExeUtilGetProcessStatistics : public ComTdbExeUtilGetStatistics {
 public:
  ComTdbExeUtilGetProcessStatistics() : ComTdbExeUtilGetStatistics() {}
  ...
  virtual const char *getNodeName() const { return "PROCESS_STATISTICS"; };
...
///////////////////////////////////////////////////////////////////////////
static const ComTdbVirtTableColumnInfo exeUtilGetUIDVirtTableColumnInfo[] = {{"GET_UID_OUTPUT",
                                                                              0,
                                                                              COM_USER_COLUMN,
                                                                              REC_BIN64_SIGNED,
                                                                              8,
...
///////////////////////////////////////////////////////////////////////////
static const ComTdbVirtTableColumnInfo exeUtilGetQIDVirtTableColumnInfo[] = {{"GET_QID_OUTPUT",
                                                                              0,
                                                                              COM_USER_COLUMN,
                                                                              REC_BYTE_V_ASCII,
                                                                              160,
...
class ComTdbExeUtilGetQID : public ComTdbExeUtil {
 ...
 public:
  ComTdbExeUtilGetQID() : ComTdbExeUtil() {}
...
class ComTdbExeUtilPopulateInMemStats : public ComTdbExeUtil {
 ...
 public:
  ...
  virtual const char *getNodeName() const { return "POP_IN_MEM_STATS"; };
  ...
  inline const char *getInMemHistintsTableName() const { return inMemHistintsTableName_.getPointer(); };

  inline const char *getSourceTableCatName() const { return sourceTableCatName_.getPointer(); };

  inline const char *getSourceTableSchName() const { return sourceTableSchName_.getPointer(); };

  inline const char *getSourceTableObjName() const { return sourceTableObjName_.getPointer(); };

  inline const char *getSourceHistogramsTableName() const { return sourceHistogramsTableName_.getPointer(); };
...
class ComTdbExeUtilAqrWnrInsert : public ComTdbExeUtil {
 ...
 public:
  ...
  virtual const char *getNodeName() const { return "AQRWNR_INSERT_UTIL"; };
...
class ComTdbExeUtilLongRunning : public ComTdbExeUtil {
 public:
  ...
  virtual const char *getNodeName() const { return "LONG_RUNNING"; };
  ...
  void setLongRunningUpdate(NABoolean v) { (v ? flags_ |= LR_UPDATE : flags_ &= ~LR_UPDATE); };
  NABoolean longRunningUpdate() { return (flags_ & LR_UPDATE) != 0; };

  void setLongRunningInsertSelect(NABoolean v) { (v ? flags_ |= LR_INSERT_SELECT : flags_ &= ~LR_INSERT_SELECT); };
  NABoolean longRunningInsertSelect() { return (flags_ & LR_INSERT_SELECT) != 0; };

  void setLongRunningQueryPlan(NABoolean v) { (v ? flags_ |= LR_QUERY_PLAN : flags_ &= ~LR_QUERY_PLAN); };
  NABoolean longRunningQueryPlan() { return (flags_ & LR_QUERY_PLAN) != 0; };

  void setUseParserflags(NABoolean v) { (v ? flags_ |= LR_PARSERFLAGS : flags_ &= ~LR_PARSERFLAGS); };
...
class ComTdbExeUtilShowSet : public ComTdbExeUtil {
 ...
 public:
  enum ShowSetType { ALL_ = 1, EXTERNALIZED_, SINGLE_ };

  ComTdbExeUtilShowSet() : ComTdbExeUtil() {}
  ...
  virtual const char *getNodeName() const { return "SHOWSET"; };
...
class ComTdbExeUtilGetMetadataInfo : public ComTdbExeUtil {
 ...
 public:
  ...
  ComTdbExeUtilGetMetadataInfo() : ComTdbExeUtil() {}
  ...
  virtual const char *getNodeName() const { return "GET_METADATA_INFO"; };
  ...
  void setUserObjs(NABoolean v) { (v ? flags_ |= USER_OBJS : flags_ &= ~USER_OBJS); };
  NABoolean userObjs() { return (flags_ & USER_OBJS) != 0; };

  void setSystemObjs(NABoolean v) { (v ? flags_ |= SYSTEM_OBJS : flags_ &= ~SYSTEM_OBJS); };
  NABoolean systemObjs() { return (flags_ & SYSTEM_OBJS) != 0; };

  void setAllObjs(NABoolean v) { (v ? flags_ |= ALL_OBJS : flags_ &= ~ALL_OBJS); };
  NABoolean allObjs() { return (flags_ & ALL_OBJS) != 0; };

  void setExternalObjs(NABoolean v) { (v ? flags_ |= EXTERNAL_OBJS : flags_ &= ~EXTERNAL_OBJS); };
  NABoolean externalObjs() { return (flags_ & EXTERNAL_OBJS) != 0; };

  void setGroupBy(NABoolean v) { (v ? flags_ |= GROUP_BY : flags_ &= ~GROUP_BY); };
  NABoolean groupBy() { return (flags_ & GROUP_BY) != 0; };

  void setOrderBy(NABoolean v) { (v ? flags_ |= ORDER_BY : flags_ &= ~ORDER_BY); };
  NABoolean orderBy() { return (flags_ & ORDER_BY) != 0; };

  void setGetVersion(NABoolean v) { (v ? flags_ |= GET_VERSION : flags_ &= ~GET_VERSION); };
  NABoolean getVersion() { return (flags_ & GET_VERSION) != 0; };

  void setGetObjectUid(NABoolean v) { (v ? flags_ |= GET_OBJECT_UID : flags_ &= ~GET_OBJECT_UID); };
  ...
  void setIsMv(NABoolean v) { (v ? flags_ |= IS_MV : flags_ &= ~IS_MV); };
  NABoolean isMv() { return (flags_ & IS_MV) != 0; };

  void setIsHbase(NABoolean v) { (v ? flags_ |= IS_HBASE : flags_ &= ~IS_HBASE); };
  NABoolean isHbase() { return (flags_ & IS_HBASE) != 0; };

  void setCascade(NABoolean v) { (v ? flags_ |= CASCADE : flags_ & CASCADE) != 0; };
  NABoolean cascade() { return (flags_ & CASCADE) != 0; };

  void setWithNamespace(NABoolean v) { (v ? flags_ |= WITH_NAMESPACE : flags_ & WITH_NAMESPACE) != 0; };
...
class ComTdbExeUtilHBaseBulkLoad : public ComTdbExeUtil {
 ...
 public:
  ...
  virtual const char *getNodeName() const { return "HBASE_BULK_LOAD"; };

  char *getErrCountTable() { return errCountTable_; }
  char *getLoggingLocation() { return loggingLocation_; }

  void setPreloadCleanup(NABoolean v) { (v ? flags_ |= PRE_LOAD_CLEANUP : flags_ &= ~PRE_LOAD_CLEANUP); };
  NABoolean getPreloadCleanup() { return (flags_ & PRE_LOAD_CLEANUP) != 0; };

  void setPreparation(NABoolean v) { (v ? flags_ |= PREPARATION : flags_ &= ~PREPARATION); };
  NABoolean getPreparation() { return (flags_ & PREPARATION) != 0; };

  void setKeepHFiles(NABoolean v) { (v ? flags_ |= KEEP_HFILES : flags_ &= ~KEEP_HFILES); };
  NABoolean getKeepHFiles() { return (flags_ & KEEP_HFILES) != 0; };

  void setTruncateTable(NABoolean v) { (v ? flags_ |= TRUNCATE_TABLE : flags_ &= ~TRUNCATE_TABLE); };
  NABoolean getTruncateTable() { return (flags_ & TRUNCATE_TABLE) != 0; };

  void setNoRollback(NABoolean v) { (v ? flags_ |= NO_ROLLBACK : flags_ &= ~NO_ROLLBACK); };
  NABoolean getNoRollback() { return (flags_ & NO_ROLLBACK) != 0; };

  void setLogErrorRows(NABoolean v) { (v ? flags_ |= LOG_ERROR_ROWS : flags_ &= ~LOG_ERROR_ROWS); };
  NABoolean getLogErrorRows() { return (flags_ & LOG_ERROR_ROWS) != 0; };

  void setContinueOnError(NABoolean v) { (v ? flags_ |= CONTINUE_ON_ERROR : flags_ &= ~CONTINUE_ON_ERROR); };
  NABoolean getContinueOnError() { return (flags_ & CONTINUE_ON_ERROR) != 0; };

  void setSecure(NABoolean v) { (v ? flags_ |= SECURE : flags_ &= ~SECURE); };
  NABoolean getSecure() { return (flags_ & SECURE) != 0; };

  void setNoDuplicates(NABoolean v) { (v ? flags_ |= NO_DUPLICATES : flags_ &= ~NO_DUPLICATES); };
  NABoolean getNoDuplicates() { return (flags_ & NO_DUPLICATES) != 0; };

  void setRebuildIndexes(NABoolean v) { (v ? flags_ |= REBUILD_INDEXES : flags_ &= ~REBUILD_INDEXES); };
  NABoolean getRebuildIndexes() { return (flags_ & REBUILD_INDEXES) != 0; };

  void setConstraints(NABoolean v) { (v ? flags_ |= CONSTRAINTS : flags_ &= ~CONSTRAINTS); };
  NABoolean getConstraints() { return (flags_ & CONSTRAINTS) != 0; };

  void setNoOutput(NABoolean v) { (v ? flags_ |= NO_OUTPUT : flags_ &= ~NO_OUTPUT); };
  NABoolean getNoOutput() { return (flags_ & NO_OUTPUT) != 0; };

  void setIndexTableOnly(NABoolean v) { (v ? flags_ |= INDEX_TABLE_ONLY : flags_ &= ~INDEX_TABLE_ONLY); };
  NABoolean getIndexTableOnly() { return (flags_ & INDEX_TABLE_ONLY) != 0; };
  void setUpsertUsingLoad(NABoolean v) { (v ? flags_ |= UPSERT_USING_LOAD : flags_ &= ~UPSERT_USING_LOAD); };
  NABoolean getUpsertUsingLoad() { return (flags_ & UPSERT_USING_LOAD) != 0; };

  void setForceCIF(NABoolean v) { (v ? flags_ |= FORCE_CIF : flags_ &= ~FORCE_CIF); };
  NABoolean getForceCIF() { return (flags_ & FORCE_CIF) != 0; };

  void setUpdateStats(NABoolean v) { (v ? flags_ |= UPDATE_STATS : flags_ &= ~UPDATE_STATS); };
...
static const ComTdbVirtTableColumnInfo comTdbRegionStatsVirtTableColumnInfo[] = {{"CATALOG_NAME",
                                                                                  0,
                                                                                  COM_USER_COLUMN,
                                                                                  REC_BYTE_F_ASCII,
                                                                                  256,
...
class ComTdbExeUtilRegionStats : public ComTdbExeUtil {
 ...
 public:
  ...
  void setDisplayFormat(NABoolean v) { (v ? flags_ |= DISPLAY_FORMAT : flags_ &= ~DISPLAY_FORMAT); };
  NABoolean displayFormat() { return (flags_ & DISPLAY_FORMAT) != 0; };

  void setSummaryOnly(NABoolean v) { (v ? flags_ |= SUMMARY_ONLY : flags_ &= ~SUMMARY_ONLY); };
  NABoolean summaryOnly() { return (flags_ & SUMMARY_ONLY) != 0; };

  void setClusterView(NABoolean v) { (v ? flags_ |= CLUSTER_VIEW : flags_ &= ~CLUSTER_VIEW); };
  ...
  virtual const char *getNodeName() const { return (clusterView() ? "GET_CLUSTER_STATS" : "GET_REGION_STATS"); };

  int getVirtTableNumCols() {
    return (clusterView() ? sizeof(comTdbClusterStatsVirtTableColumnInfo) / sizeof(ComTdbVirtTableColumnInfo)
  ...
  ComTdbVirtTableColumnInfo *getVirtTableColumnInfo() {
    return (ComTdbVirtTableColumnInfo *)(clusterView() ? comTdbClusterStatsVirtTableColumnInfo
                                                       : comTdbRegionStatsVirtTableColumnInfo);
  }

  int getVirtTableNumKeys() { return 0; }

  ComTdbVirtTableKeyInfo *getVirtTableKeyInfo() { return NULL; }

 private:
  enum { IS_INDEX = 0x0001, DISPLAY_FORMAT = 0x0002, SUMMARY_ONLY = 0x0004, CLUSTER_VIEW = 0x0008 };
...
static const ComTdbVirtTableColumnInfo comTdbLobInfoVirtTableColumnInfo[] = {{"CATALOG_NAME",
                                                                              0,
                                                                              COM_USER_COLUMN,
                                                                              REC_BYTE_F_ASCII,
                                                                              256,
...
class ComTdbExeUtilCompositeUnnest : public ComTdbExeUtil {
 ...
 public:
  ComTdbExeUtilCompositeUnnest() : ComTdbExeUtil() {}

  ComTdbExeUtilCompositeUnnest(ex_expr *scanExpr, ex_expr *extractColExpr, UInt16 extractColAtpIndex,
                               UInt32 extractColRowLen, UInt16 elemNumAtpIndex, UInt32 elemNumLen,
...
class ComTdbExeUtilUpdataDelete : public ComTdbExeUtil {
 ...
 public:
  ...
  virtual const char *getNodeName() const { return "SNAPSHOT_UPDATE_DELETE"; };
