...
class CostMethodIsolatedScalarUDF;
class QueryInvalidationFunc;
...
class RelRoutine : public RelExpr {
 public:
  ...
  RelRoutine(ItemExpr *params = NULL, OperatorTypeEnum otype = REL_ROUTINE,
  ...
  RelRoutine(RelExpr *child0, ItemExpr *params = NULL, OperatorTypeEnum otype = REL_ROUTINE,
             CollHeap *oHeap = CmpCommon::statementHeap())
  ...
  RelRoutine(QualifiedName name, ItemExpr *params = NULL, OperatorTypeEnum otype = REL_ROUTINE,
             CollHeap *oHeap = CmpCommon::statementHeap())
      : RelExpr(otype, NULL, NULL, oHeap),
        procAllParamsTree_(params),
  ...
  RelRoutine(QualifiedName name, ValueIdList outParams, ItemExpr *params = NULL, OperatorTypeEnum otype = REL_ROUTINE,
             CollHeap *oHeap = CmpCommon::statementHeap())
      : RelExpr(otype, NULL, NULL, oHeap),
        procAllParamsTree_(params),
        routineName_(name),
  ...
  RelRoutine(const RelRoutine &other, CollHeap *oHeap = CmpCommon::statementHeap());
  ...
  inline const NARoutine *getNARoutine() const { return routined_ ? routined_->getNARoutine() : NULL; }
  ...
  */
  ...
  inline const ValueIdList &getProcOutputParamsVids() const { return procOutputParamsVids_; }

  //! getProcOutputParamsVids acessor method
  // return a reference to the output parameters
  inline ValueIdList &getProcOutputParamsVids() { return procOutputParamsVids_; }

  //! getProcInputParamsVids acessor method
  // return a constant reference to the input parameters
  inline const ValueIdList &getProcInputParamsVids() const { return procInputParamsVids_; }

  //! getProcInputParamsVids acessor method
  // return a reference to the input parameters
  inline ValueIdList &getProcInputParamsVids() { return procInputParamsVids_; }

  //! getProcAllParamsVids acessor method
  // return a reference to all parameters
  inline ValueIdList &getProcAllParamsVids() { return procAllParamsVids_; }

  //! getProcAllParamsVids acessor method
  // return a constant reference to all parameters
  inline const ValueIdList &getProcAllParamsVids() const { return procAllParamsVids_; }

  //! getProcAllParamsTree acessor method
  // return a constant reference to the ItemExpr parameter tree
  inline const ItemExpr *getProcAllParamsTree() const { return procAllParamsTree_; }

  //! getProcAllParamsTree acessor method
  // return a reference to the ItemExpr parameter tree
  inline ItemExpr *getRWProcAllParamsTree() { return procAllParamsTree_; }
  ...
  */
  ...
  inline void setProcAllParamsVids(ValueIdList &paramIds) { procAllParamsVids_ = paramIds; }

  //! setProcAllParamsTree mutator method
  //  initializes the procAllParamsTree member to the given parameter.
  inline void setProcAllParamsTree(ItemExpr *paramTree) { procAllParamsTree_ = paramTree; }

  //! addProcOutputParamsVids mutator method
  //  add a ValueId to the procOutputParamsVids list
  inline void addProcOutputParamsVid(ValueId vId) { procOutputParamsVids_.insert(vId); }
  ...
  virtual const char *getInputExpressionName() const { return "function_parameters"; }
...
class TableValuedFunction : public RelRoutine {
 public:
  ...
  TableValuedFunction(ItemExpr *params = NULL, OperatorTypeEnum otype = REL_TABLE_VALUED_FUNCTION,
                      CollHeap *oHeap = CmpCommon::statementHeap())
      : RelRoutine(params, otype, oHeap), userTableName_("", oHeap), tabId_(0) {}

  //! TableValuedFunction Constructor with 1 child
  //  can be instantiated without any parameters
  TableValuedFunction(RelExpr *child0, ItemExpr *params = NULL, OperatorTypeEnum otype = REL_TABLE_VALUED_FUNCTION,
                      CollHeap *oHeap = CmpCommon::statementHeap())
      : RelRoutine(child0, params, otype, oHeap), userTableName_("", oHeap), tabId_(0) {}

  //! TableValuedFunction Constructor
  //  wants at least a name for the routine.
  TableValuedFunction(const CorrName &name, ItemExpr *params = NULL, OperatorTypeEnum otype = REL_TABLE_VALUED_FUNCTION,
                      CollHeap *oHeap = CmpCommon::statementHeap())
      : RelRoutine(name.getQualifiedNameObj(), params, otype, oHeap), userTableName_(name, oHeap), tabId_(0) {}

  //! TableValuedFunction Copy Constructor
  //  wants at least a name for the routine.
  TableValuedFunction(const TableValuedFunction &other, CollHeap *oHeap = CmpCommon::statementHeap())
      : RelRoutine(other, oHeap), tabId_(other.tabId_), userTableName_(other.userTableName_) {}
  ...
  virtual const char *getInputExpressionName() const { return "TableValuedFunction inputs"; }
  ...
  void setUserTableName(const CorrName &userTableName) { userTableName_ = userTableName; }
  // used only in the parser to override udf name is AS clause was used
  void setCorrName(const NAString &corrName) { getUserTableName().setCorrName(corrName); }
...
class BuiltinTableValuedFunction : public TableValuedFunction {
 public:
  ...
  BuiltinTableValuedFunction(ItemExpr *params = NULL, OperatorTypeEnum otype = REL_BUILTIN_TABLE_VALUED_FUNCTION,
                             CollHeap *oHeap = CmpCommon::statementHeap())
  ...
  //  Copy constructor
  BuiltinTableValuedFunction(const BuiltinTableValuedFunction &other, CollHeap *h = CmpCommon::statementHeap())
...
class TableMappingUDFChildInfo : public NABasicObject {
 ...
 public:
  TableMappingUDFChildInfo(const NAString &name, NAColumnArray &inputCols, TMUDFInputPartReq partType,
                           ValueIdSet &partBy, ValueIdList &orderBy, ValueIdList &outputs,
                           CollHeap *oHeap = CmpCommon::statementHeap())
      : inputTabName_(name),
  ...
  TableMappingUDFChildInfo(TMUDFInputPartReq partType, ValueIdSet &partBy, ValueIdList &orderBy,
                           CollHeap *oHeap = CmpCommon::statementHeap())
      : partType_(partType), partitionBy_(partBy), orderBy_(orderBy) {}
...
class TableMappingUDF : public TableValuedFunction {
 public:
  ...
  TableMappingUDF(int arity, ItemExpr *params = NULL, CollHeap *oHeap = CmpCommon::statementHeap())
  ...
  //! TableMappingUDF Constructor
  //  expects at least a name
  TableMappingUDF(int arity, const CorrName &name, ItemExpr *params = NULL,
                  CollHeap *oHeap = CmpCommon::statementHeap())
  ...
  TableMappingUDF(const CorrName &name, ItemExpr *params, int arity, OperatorTypeEnum otype,
                  CollHeap *oHeap = CmpCommon::statementHeap())
      : TableValuedFunction(name, params, otype, oHeap),
        childInfo_(oHeap),
        scalarInputParams_(NULL),
  ...
  static OperatorTypeEnum getTMUDFOpType(int arity) {
    ...
    return (arity == 0 ? REL_LEAF_TABLE_MAPPING_UDF
                       : (arity == 1 ? REL_UNARY_TABLE_MAPPING_UDF : REL_BINARY_TABLE_MAPPING_UDF));
  }
  ...
  TMUDFDllInteraction *getDllInteraction() { return dllInteraction_; }

  tmudr::UDRInvocationInfo *getInvocationInfo() { return invocationInfo_; }

  int getNextPlanInfoNum() { return numPlanInfos_++; }
  ...
  inline const NAColumnArray &getScalarInputParams() const { return *scalarInputParams_; }

  inline ComSInt32 getScalarInputParamCount() const { return scalarInputParams_->entries(); }

  inline void setScalarInputParams(NAColumnArray *val) { scalarInputParams_ = val; }

  inline const NAColumnArray &getOutputParams() const { return *outputParams_; }

  inline void setOutputParams(NAColumnArray *val) { outputParams_ = val; }
  ...
  inline ComSInt32 getOutputParamCount() const { return outputParams_->entries(); }

  inline void setInvocationInfo(tmudr::UDRInvocationInfo *invocationInfo) { invocationInfo_ = invocationInfo; }

  inline void setConstParamBuffer(char *buf, int len) {
    constParamBuffer_ = buf;
    constParamBufferLen_ = len;
  }

  inline void setRoutineHandle(CliRoutineHandle handle) { routineHandle_ = handle; }

 protected:
  virtual NARoutine *getRoutineMetadata(QualifiedName &routineName, CorrName &tmfuncName, BindWA *bindWA);
...
class PredefinedTableMappingFunction : public TableMappingUDF {
 ...
 public:
  PredefinedTableMappingFunction(int arity, const CorrName &name, ItemExpr *params, OperatorTypeEnum otype,
                                 CollHeap *oHeap = CmpCommon::statementHeap());
...
class PhysicalTableMappingUDF : public TableMappingUDF {
 public:
  ...
  PhysicalTableMappingUDF(int arity, CollHeap *oHeap = CmpCommon::statementHeap());
 ...
 private:
  ...
  CostMethodTableMappingUDF *pCostMethod_;
};  // class PhysicalTableMappingUDF
...
class TableValuedUDF : public TableValuedFunction {
  ...
  TableValuedUDF(ItemExpr *params = NULL, OperatorTypeEnum otype = REL_TABLE_VALUED_UDF,
                 CollHeap *oHeap = CmpCommon::statementHeap())
      : TableValuedFunction(params, otype, oHeap) {}

  //! TableValuedUDF Constructor
  //  expects at least a name
  TableValuedUDF(const CorrName &name, ItemExpr *params = NULL, OperatorTypeEnum otype = REL_TABLE_VALUED_UDF,
                 CollHeap *oHeap = CmpCommon::statementHeap())
      : TableValuedFunction(name, params, otype, oHeap) {}
...
class ExplainFunc : public BuiltinTableValuedFunction {
 public:
  ...
  ExplainFunc(ItemExpr *params = NULL, CollHeap *oHeap = CmpCommon::statementHeap())
...
class PhysicalExplain : public ExplainFunc {
 public:
  ...
  PhysicalExplain(CollHeap *oHeap = CmpCommon::statementHeap()) : ExplainFunc(NULL, oHeap) {}
...
class StatisticsFunc : public BuiltinTableValuedFunction {
 public:
  ...
  StatisticsFunc(ItemExpr *params = NULL, CollHeap *oHeap = CmpCommon::statementHeap())
...
class ProxyFunc : public BuiltinTableValuedFunction {
 public:
  ...
  ProxyFunc(OperatorTypeEnum otype, ElemDDLNode *columnsFromParser, CollHeap *heap);
...
class SPProxyFunc : public ProxyFunc {
 public:
  ...
  SPProxyFunc(ElemDDLNode *columnsFromParser, ItemExprList *optionalStrings, CollHeap *heap);
  ...
  // Copy constructor
  SPProxyFunc(const SPProxyFunc &other, CollHeap *h = CmpCommon::statementHeap());
  // destructors
...
class PhysicalSPProxyFunc : public SPProxyFunc {
 public:
  ...
  PhysicalSPProxyFunc(CollHeap *oHeap = CmpCommon::statementHeap()) : SPProxyFunc(oHeap) {}
...
class ExtractSource : public ProxyFunc {
 public:
  ...
  ExtractSource(ElemDDLNode *columnsFromParser, NAString &espPhandle, NAString &securityKey, CollHeap *heap);
  ...
  // Copy constructor without heap is not implemented nor allowed.
  ExtractSource(const ExtractSource &other, CollHeap *h = CmpCommon::statementHeap());
...
class PhysicalExtractSource : public ExtractSource {
 public:
  ...
  PhysicalExtractSource(CollHeap *oHeap = CmpCommon::statementHeap()) : ExtractSource(oHeap) {}
...
class IsolatedNonTableUDR : public RelRoutine {
 public:
  ...
  IsolatedNonTableUDR(QualifiedName routineName, ItemExprList *inParams, ValueIdList outParams, CollHeap *outHeap,
                      OperatorTypeEnum spType = REL_ISOLATED_NON_TABLE_UDR)
      : RelRoutine(routineName, outParams, inParams->convertToItemExpr(), spType, outHeap) {}
  ...
  inline void setNeededValueIds(ValueIdSet neededValueIds) { neededValueIds_ = neededValueIds; }
  ...
  virtual void populateAndBindItemExpr(ItemExpr *param, BindWA *bindWA);

  //! setInOrOutParam method
  //  add the ItemExpr to the appropriate list of input and output params
  virtual void setInOrOutParam(ItemExpr *expr, ComColumnDirection mode, BindWA *bindWA);
...
class CallSP : public IsolatedNonTableUDR {
 ...
 private:
  ...
  //! setInOrOutParam method
  //  add the ItemExpr to the appropriate list of input and output params
  void setInOrOutParam(ItemExpr *expr, ComColumnDirection mode, BindWA *bindWA);
...
class IsolatedScalarUDF : public IsolatedNonTableUDR {
 public:
  ...
  IsolatedScalarUDF(const QualifiedName &routineName, ItemExprList *inParams, ValueIdList outParams, CollHeap *h);
 ...
 private:
  ...
  CostMethodIsolatedScalarUDF *pCostMethod_;
};  // class IsolatedScalarUDF
...
// -----------------------------------------------------------------------
/*!
 *  \brief QueryInvalidationFunc Class.
 *        The QueryInvalidation class represents a table containing the info
 *        of QI key.
 *        This is both a logical and a physical operator.
 */
// -----------------------------------------------------------------------
class QueryInvalidationFunc : public BuiltinTableValuedFunction {
 public:
  // constructor
  QueryInvalidationFunc(ItemExpr *params = NULL, CollHeap *oHeap = CmpCommon::statementHeap())
      : BuiltinTableValuedFunction(params, REL_QUERYINVALIDATION, oHeap) {}
