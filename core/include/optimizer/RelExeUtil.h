...
class GenericUtilExpr : public RelExpr {
 public:
  GenericUtilExpr(char *stmtText, CharInfo::CharSet stmtTextCharSet, ExprNode *exprNode, RelExpr *child,
                  OperatorTypeEnum otype, CollHeap *oHeap = CmpCommon::statementHeap())
  ...
  GenericUtilExpr(OperatorTypeEnum otype) : RelExpr(otype, NULL, NULL, NULL){};
  ...
  char *getStmtText() { return stmtText_; }

  CharInfo::CharSet getStmtTextCharSet() const { return stmtTextCharSet_; }

  void setStmtText(char *stmtText, CharInfo::CharSet stmtTextCS) {
    if (stmtText_) NADELETEBASIC(stmtText_, heap_);
...
class DDLExpr : public GenericUtilExpr {
 ...
 public:
  DDLExpr(ExprNode *ddlNode, char *ddlStmtText = NULL, CharInfo::CharSet ddlStmtTextCharSet = CharInfo::UnknownCharSet,
      ...
      : GenericUtilExpr(ddlStmtText, ddlStmtTextCharSet, ddlNode, NULL, REL_DDL, oHeap),
        ...
        nodeIdArray_(heap_) {
    setDDLXns(TRUE);
    if (CmpCommon::getDefault(MAINTENANCE_WINDOW) == DF_ON) setMaintenanceWindow(TRUE);
  };
  ...
  virtual NABoolean dontUseCache() { return (showddlExplain() ? FALSE : TRUE); }
  ...
  void setCreateMDViews(NABoolean v) { (v ? flags_ |= CREATE_MD_VIEWS : flags_ &= ~CREATE_MD_VIEWS); }
  NABoolean createMDViews() { return (flags_ & CREATE_MD_VIEWS) != 0; }

  void setDropMDViews(NABoolean v) { (v ? flags_ |= DROP_MD_VIEWS : flags_ &= ~DROP_MD_VIEWS); }
  NABoolean dropMDViews() { return (flags_ & DROP_MD_VIEWS) != 0; }

  void setGetMDVersion(NABoolean v) { (v ? flags_ |= GET_MD_VERSION : flags_ &= ~GET_MD_VERSION); }
  NABoolean getMDVersion() { return (flags_ & GET_MD_VERSION) != 0; }

  void setCreateLibmgr(NABoolean v) { (v ? flags_ |= CREATE_LIBMGR : flags_ &= ~CREATE_LIBMGR); }
  NABoolean createLibmgr() { return (flags_ & CREATE_LIBMGR) != 0; }

  void setDropLibmgr(NABoolean v) { (v ? flags_ |= DROP_LIBMGR : flags_ &= ~DROP_LIBMGR); }
  NABoolean dropLibmgr() { return (flags_ & DROP_LIBMGR) != 0; }

  void setUpgradeLibmgr(NABoolean v) { (v ? flags_ |= UPGRADE_LIBMGR : flags_ &= ~UPGRADE_LIBMGR); }
  NABoolean upgradeLibmgr() { return (flags_ & UPGRADE_LIBMGR) != 0; }

  void setCreateRepos(NABoolean v) { (v ? flags_ |= CREATE_REPOS : flags_ &= ~CREATE_REPOS); }
  NABoolean createRepos() { return (flags_ & CREATE_REPOS) != 0; }

  void setDropRepos(NABoolean v) { (v ? flags_ |= DROP_REPOS : flags_ &= ~DROP_REPOS); }
  NABoolean dropRepos() { return (flags_ & DROP_REPOS) != 0; }

  void setUpgradeRepos(NABoolean v) { (v ? flags_ |= UPGRADE_REPOS : flags_ &= ~UPGRADE_REPOS); }
  NABoolean upgradeRepos() { return (flags_ & UPGRADE_REPOS) != 0; }

  void setCreateBackupRepos(NABoolean v) { (v ? flags_ |= CREATE_BACKUP_REPOS : flags_ &= ~CREATE_BACKUP_REPOS); }
  NABoolean createBackupRepos() { return (flags_ & CREATE_BACKUP_REPOS) != 0; }

  void setDropBackupRepos(NABoolean v) { (v ? flags_ |= DROP_BACKUP_REPOS : flags_ &= ~DROP_BACKUP_REPOS); }
  NABoolean dropBackupRepos() { return (flags_ & DROP_BACKUP_REPOS) != 0; }

  void setUpgradeBackupRepos(NABoolean v) { (v ? flags_ |= UPGRADE_BACKUP_REPOS : flags_ &= ~UPGRADE_BACKUP_REPOS); }
  NABoolean upgradeBackupRepos() { return (flags_ & UPGRADE_REPOS) != 0; }

  void setCleanupAuth(NABoolean v) { (v ? flags_ |= CLEANUP_AUTH : flags_ &= ~CLEANUP_AUTH); }
  NABoolean cleanupAuth() { return (flags_ & CLEANUP_AUTH) != 0; }

  void setForceOption(NABoolean v) { (v ? flags_ |= FORCE_OPTION : flags_ &= ~FORCE_OPTION); }
  NABoolean forceOption() { return (flags_ & FORCE_OPTION) != 0; }

  void setLoadTrafMetadataInCache(NABoolean v) { (v ? flags_ |= LOAD_METADATA : flags_ &= ~LOAD_METADATA); }
  NABoolean loadTrafMetadataInCache() { return (flags_ & LOAD_METADATA) != 0; }

  void setLoadTrafMetadataIntoSharedCache(NABoolean v) {
    (v ? flags_ |= LOAD_METADATA_INTO_SHARED_CACHE : flags_ &= ~LOAD_METADATA_INTO_SHARED_CACHE);
  }
  ...
  void setLoadTrafDataIntoSharedCache(NABoolean v) {
    (v ? flags_ |= LOAD_DATA_INTO_SHARED_CACHE : flags_ &= ~LOAD_DATA_INTO_SHARED_CACHE);
  }
  NABoolean loadTrafDataIntoSharedCache() { return (flags_ & LOAD_DATA_INTO_SHARED_CACHE) != 0; }

  void setLoadLocalIfEmpty(NABoolean v) { (v ? flags_ |= LOAD_LOCAL_IFEMPTY : flags_ &= ~LOAD_LOCAL_IFEMPTY); }
  NABoolean loadLocalIfEmpty() { return (flags_ & LOAD_LOCAL_IFEMPTY) != 0; }

  void setAlterTrafMetadataSharedCache(NABoolean v) {
    (v ? flags_ |= ALTER_METADATA_SHARED_CACHE : flags_ &= ~ALTER_METADATA_SHARED_CACHE);
  }
  ...
  void setAlterTrafDataSharedCache(NABoolean v) {
    (v ? flags_ |= ALTER_DATA_SHARED_CACHE : flags_ &= ~ALTER_DATA_SHARED_CACHE);
  }
  NABoolean alterTrafDataSharedCache() { return (flags_ & ALTER_DATA_SHARED_CACHE) != 0; }

  NABoolean isTrafSharedCacheOp() {
    return loadTrafMetadataIntoSharedCache() || alterTrafMetadataSharedCache() || loadTrafDataIntoSharedCache() ||
  ...
  void setCreateTenant(NABoolean v) { (v ? flags_ |= CREATE_TENANT : flags_ &= ~CREATE_TENANT); }
  NABoolean createTenant() { return (flags_ & CREATE_TENANT) != 0; }
  void setDropTenant(NABoolean v) { (v ? flags_ |= DROP_TENANT : flags_ &= ~DROP_TENANT); }
  NABoolean dropTenant() { return (flags_ & DROP_TENANT) != 0; }
  void setUpgradeTenant(NABoolean v) { (v ? flags_ |= UPGRADE_TENANT : flags_ &= ~UPGRADE_TENANT); }
  NABoolean upgradeTenant() { return (flags_ & UPGRADE_TENANT) != 0; }

  void setUpgradeNamespace(NABoolean v) { (v ? flags_ |= UPGRADE_NAMESPACE : flags_ &= ~UPGRADE_NAMESPACE); }
  ...
  NABoolean useReservedNamespace() { return (flags_ & RESERVED_NAMESPACE) != 0; }

  void setInitHbase(NABoolean v) { (v ? flags_ |= INIT_HBASE : flags_ &= ~INIT_HBASE); }
  NABoolean initHbase() { return (flags_ & INIT_HBASE) != 0; }

  void setDropHbase(NABoolean v) { (v ? flags_ |= DROP_HBASE : flags_ &= ~DROP_HBASE); }
  NABoolean dropHbase() { return (flags_ & DROP_HBASE) != 0; }

  void setInitAuth(NABoolean v) { (v ? flags_ |= INIT_AUTH : flags_ &= ~INIT_AUTH); }
  NABoolean initAuth() { return (flags_ & INIT_AUTH) != 0; }

  void setDropAuth(NABoolean v) { (v ? flags_ |= DROP_AUTH : flags_ &= ~DROP_AUTH); }
  NABoolean dropAuth() { return (flags_ & DROP_AUTH) != 0; }

  void setUpdateVersion(NABoolean v) { (v ? flags_ |= UPDATE_VERSION : flags_ &= ~UPDATE_VERSION); }
  NABoolean updateVersion() { return (flags_ & UPDATE_VERSION) != 0; }

  void setUpdateMDIndexes(NABoolean v) { (v ? flags_ |= UPDATE_MD_INDEXES : flags_ &= ~UPDATE_MD_INDEXES); }
  NABoolean updateMDIndexes() { return (flags_ & UPDATE_MD_INDEXES) != 0; }

  void setAddSchemaObjects(NABoolean v) { (v ? flags_ |= ADD_SCH_OBJS : flags_ &= ~ADD_SCH_OBJS); }
  ...
  void setMinimal(NABoolean v) { (v ? flags_ |= MINIMAL : flags_ &= ~MINIMAL); }
  NABoolean minimal() { return (flags_ & MINIMAL) != 0; }

  void setPurgedata(NABoolean v) { (v ? flags_ |= PURGEDATA : flags_ &= ~PURGEDATA); }
  NABoolean purgedata() { return (flags_ & PURGEDATA) != 0; }

  void setPurgedataIfExists(NABoolean v) { (v ? flags_ |= PURGEDATA_IF_EXISTS : flags_ &= ~PURGEDATA_IF_EXISTS); }
  ...
  void setShowddlExplain(NABoolean v) { (v ? flags_ |= SHOWDDL_EXPLAIN : flags_ &= ~SHOWDDL_EXPLAIN); }
  NABoolean showddlExplain() { return (flags_ & SHOWDDL_EXPLAIN) != 0; }

  void setShowddlExplainInt(NABoolean v) { (v ? flags_ |= SHOWDDL_EXPLAIN_INT : flags_ &= ~SHOWDDL_EXPLAIN_INT); }
  NABoolean showddlExplainInt() { return (flags_ & SHOWDDL_EXPLAIN_INT) != 0; }

  void setNoLabelStats(NABoolean v) { (v ? flags_ |= NO_LABEL_STATS : flags_ &= ~NO_LABEL_STATS); }
  NABoolean noLabelStats() { return (flags_ & NO_LABEL_STATS) != 0; }

  void setCreateLobMD(NABoolean v) { (v ? flags_ |= CREATE_LOBMD : flags_ &= ~CREATE_LOBMD); }
  NABoolean createLobMD() { return (flags_ & CREATE_LOBMD) != 0; }

  void setDropLobMD(NABoolean v) { (v ? flags_ |= DROP_LOBMD : flags_ &= ~DROP_LOBMD); }
  ...
  void setNodeList(ConstStringList *x = NULL) { nodeList_ = (x) ? new (heap_) ConstStringList(*x, heap_) : NULL; }
  ...
  void setMaintenanceWindow(NABoolean v) { (v ? flags_ |= DDL_MAINTENANCE_WINDOW : flags_ &= ~DDL_MAINTENANCE_WINDOW); }

  NABoolean isMaintenanceWindowOFF() const { return ((flags_ & DDL_MAINTENANCE_WINDOW) == 0); }

  void setCreateXDCMetadata(NABoolean v) { (v ? flags_ |= CREATE_XDC_METADATA : flags_ &= ~CREATE_XDC_METADATA); }
  NABoolean createXDCMetadata() { return (flags_ & CREATE_XDC_METADATA) != 0; }

  void setDropXDCMetadata(NABoolean v) { (v ? flags_ |= DROP_XDC_METADATA : flags_ &= ~DROP_XDC_METADATA); }
  NABoolean dropXDCMetadata() { return (flags_ & DROP_XDC_METADATA) != 0; }

  void setUpgradeXDCMetadata(NABoolean v) { (v ? flags_ |= UPGRADE_XDC_METADATA : flags_ &= ~UPGRADE_XDC_METADATA); }
  NABoolean upgradeXDCMetadata() { return (flags_ & UPGRADE_XDC_METADATA) != 0; }

  void setCreateMDPartTables(NABoolean v) { (v ? flags_ |= CREATE_MD_PARTTABLES : flags_ &= ~CREATE_MD_PARTTABLES); }
  NABoolean createMDPartTables() { return (flags_ & CREATE_MD_PARTTABLES) != 0; }

  void setDropMDPartTables(NABoolean v) { (v ? flags_ |= DROP_MD_PARTTABLES : flags_ &= ~DROP_MD_PARTTABLES); }
  NABoolean dropMDPartTables() { return (flags_ & DROP_MD_PARTTABLES) != 0; }

  void setUpgradeMDPartTables(NABoolean v) { (v ? flags_ |= UPGRADE_MD_PARTTABLES : flags_ &= ~UPGRADE_MD_PARTTABLES); }
  NABoolean upgradeMDPartTables() { return (flags_ & UPGRADE_MD_PARTTABLES) != 0; }

  void doDBAccountAuthPwdCheck(NABoolean v) { (v ? flags_ |= INIT_AUTHENTICATION : flags_ &= ~INIT_AUTHENTICATION); }

  NABoolean needDBAccountAuthPwdCheck() const { return ((flags_ & INIT_AUTHENTICATION) != 0); }
 ...
 protected:
  enum Flags {
    ...
    LOAD_METADATA_INTO_SHARED_CACHE = 0x200000000,
    CREATE_LOBMD = 0x400000000,
    DROP_LOBMD = 0x800000000,
    ALTER_METADATA_SHARED_CACHE = 0x1000000000,
    LOAD_LOCAL_IFEMPTY = 0x2000000000,
    ...
    LOAD_DATA_INTO_SHARED_CACHE = 0x400000000000,
...
class RelBackupRestore : public DDLExpr {
 public:
  RelBackupRestore(char *stmtText, CharInfo::CharSet stmtTextCharSet, CollHeap *oHeap = CmpCommon::statementHeap())
  ...
  void setBackup(NABoolean v) { (v ? flags_ |= BACKUP : flags_ &= ~BACKUP); }
  NABoolean backup() { return (flags_ & BACKUP) != 0; }

  void setRestore(NABoolean v) { (v ? flags_ |= RESTORE : flags_ &= ~RESTORE); }
  NABoolean restore() { return (flags_ & RESTORE) != 0; }

  void setBRMetadata(NABoolean v) { (v ? flags_ |= BR_METADATA : flags_ &= ~BR_METADATA); }
  NABoolean brMetadata() { return (flags_ & BR_METADATA) != 0; }

  void setBRUserdata(NABoolean v) { (v ? flags_ |= BR_USERDATA : flags_ &= ~BR_USERDATA); }
  NABoolean brUserdata() { return (flags_ & BR_USERDATA) != 0; }

  void setBackupDatabase(NABoolean v) { (v ? flags_ |= BACKUP_DATABASE : flags_ &= ~BACKUP_DATABASE); }
  NABoolean backupDatabase() { return (flags_ & BACKUP_DATABASE) != 0; }

  void setBRSchemas(NABoolean v) { (v ? flags_ |= BR_SCHEMAS : flags_ &= ~BR_SCHEMAS); }
  NABoolean brSchemas() { return (flags_ & BR_SCHEMAS) != 0; }

  void setBRTables(NABoolean v) { (v ? flags_ |= BR_TABLES : flags_ &= ~BR_TABLES); }
  NABoolean brTables() { return (flags_ & BR_TABLES) != 0; }

  void setBRViews(NABoolean v) { (v ? flags_ |= BR_VIEWS : flags_ &= ~BR_VIEWS); }
  NABoolean brViews() { return (flags_ & BR_VIEWS) != 0; }

  void setBRProcs(NABoolean v) { (v ? flags_ |= BR_PROCS : flags_ &= ~BR_PROCS); }
  NABoolean brProcs() { return (flags_ & BR_PROCS) != 0; }

  void setBRLibs(NABoolean v) { (v ? flags_ |= BR_LIBS : flags_ &= ~BR_LIBS); }
  NABoolean brLibs() { return (flags_ & BR_LIBS) != 0; }

  void setDropBackupMD(NABoolean v) { (v ? flags_ |= DROP_BACKUP_MD : flags_ &= ~DROP_BACKUP_MD); }
  ...
  void setRestoreToTS(NABoolean v) { (v ? flags_ |= RESTORE_TO_TS : flags_ &= ~RESTORE_TO_TS); }
  ...
  void setDropBackup(NABoolean v) { (v ? flags_ |= DROP_BACKUP : flags_ &= ~DROP_BACKUP); }
  NABoolean dropBackup() { return (flags_ & DROP_BACKUP) != 0; }

  void setDropAllBackups(NABoolean v) { (v ? flags_ |= DROP_ALL_BACKUPS : flags_ &= ~DROP_ALL_BACKUPS); }
  NABoolean dropAllBackups() { return (flags_ & DROP_ALL_BACKUPS) != 0; }

  void setUnlockTraf(NABoolean v) { (v ? flags_ |= UNLOCK_TRAF : flags_ &= ~UNLOCK_TRAF); }
  NABoolean unlockTraf() { return (flags_ & UNLOCK_TRAF) != 0; }

  void setReturnStatus(NABoolean v) { (v ? flags_ |= RETURN_STATUS : flags_ &= ~RETURN_STATUS); }
  NABoolean returnStatus() { return (flags_ & RETURN_STATUS) != 0; }

  void setGetBackupSnapshot(NABoolean v) { (v ? flags_ |= GET_BACKUP_SNAPSHOT : flags_ &= ~GET_BACKUP_SNAPSHOT); }
  ...
  void setGetBackupMD(NABoolean v) { (v ? flags_ |= GET_BACKUP_MD : flags_ &= ~GET_BACKUP_MD); }
  NABoolean getBackupMD() { return (flags_ & GET_BACKUP_MD) != 0; }

  void setGetBackupTag(NABoolean v) { (v ? flags_ |= GET_BACKUP_TAG : flags_ &= ~GET_BACKUP_TAG); }
  NABoolean getBackupTag() { return (flags_ & GET_BACKUP_TAG) != 0; }

  void setGetAllBackupTags(NABoolean v) { (v ? flags_ |= GET_ALL_BACKUP_TAGS : flags_ &= ~GET_ALL_BACKUP_TAGS); }
  ...
  void setGetLockedObjects(NABoolean v) { (v ? flags_ |= GET_LOCKED_OBJECTS : flags_ &= ~GET_LOCKED_OBJECTS); }
  NABoolean getLockedObjects() { return (flags_ & GET_LOCKED_OBJECTS) != 0; }

  void setGetVersionOfBackup(NABoolean v) { (v ? flags_ |= GET_VERSION_OF_BACKUP : flags_ &= ~GET_VERSION_OF_BACKUP); }
  NABoolean getVersionOfBackup() { return (flags_ & GET_VERSION_OF_BACKUP) != 0; }

  void setShowObjects(NABoolean v) { (v ? flags_ |= SHOW_OBJECTS : flags_ &= ~SHOW_OBJECTS); }
  NABoolean showObjects() { return (flags_ & SHOW_OBJECTS) != 0; }

  void setExportBackup(NABoolean v) { (v ? flags_ |= EXPORT_BACKUP : flags_ &= ~EXPORT_BACKUP); }
  NABoolean exportBackup() { return (flags_ & EXPORT_BACKUP) != 0; }

  void setExportAllBackups(NABoolean v) { (v ? flags_ |= EXPORT_ALL_BACKUPS : flags_ &= ~EXPORT_ALL_BACKUPS); }
  NABoolean exportAllBackups() { return (flags_ & EXPORT_ALL_BACKUPS) != 0; }

  void setImportBackup(NABoolean v) { (v ? flags_ |= IMPORT_BACKUP : flags_ &= ~IMPORT_BACKUP); }
  NABoolean importBackup() { return (flags_ & IMPORT_BACKUP) != 0; }

  void setImportAllBackups(NABoolean v) { (v ? flags_ |= IMPORT_ALL_BACKUPS : flags_ &= ~IMPORT_ALL_BACKUPS); }
  NABoolean importAllBackups() { return (flags_ & IMPORT_ALL_BACKUPS) != 0; }

  void setOverride(NABoolean v) { (v ? flags_ |= OVERRIDE : flags_ &= ~OVERRIDE); }
  NABoolean getOverride() { return (flags_ & OVERRIDE) != 0; }

  void setCleanupObjects(NABoolean v) { (v ? flags_ |= CLEANUP_OBJECTS : flags_ &= ~CLEANUP_OBJECTS); }
  NABoolean cleanupObjects() { return (flags_ & CLEANUP_OBJECTS) != 0; }

  void setIncremental(NABoolean v) { (v ? flags_ |= INCREMENTAL : flags_ &= ~INCREMENTAL); }
  NABoolean getIncremental() { return (flags_ & INCREMENTAL) != 0; }

  void setCreateTags(NABoolean v) { (v ? flags_ |= CREATE_TAGS : flags_ &= ~CREATE_TAGS); }
  NABoolean getCreateTags() { return (flags_ & CREATE_TAGS) != 0; }

  void setBackupSystem(NABoolean v) { (v ? flags_ |= BACKUP_SYSTEM : flags_ &= ~BACKUP_SYSTEM); }
  NABoolean backupSystem() { return (flags_ & BACKUP_SYSTEM) != 0; }

  void setRestoreSystem(NABoolean v) { (v ? flags_ |= RESTORE_SYSTEM : flags_ &= ~RESTORE_SYSTEM); }
  NABoolean restoreSystem() { return (flags_ & RESTORE_SYSTEM) != 0; }

  void setCascade(NABoolean v) { (v ? flags_ |= CASCADE : flags_ &= ~CASCADE); }
  NABoolean getCascade() { return (flags_ & CASCADE) != 0; }

  void setForce(NABoolean v) { (v ? flags_ |= FORCE : flags_ &= ~FORCE); }
  NABoolean getForce() { return (flags_ & FORCE) != 0; }

  void setNoHeader(NABoolean v) { (v ? flags_ |= NO_HEADER : flags_ &= ~NO_HEADER); }
  NABoolean getNoHeader() { return (flags_ & NO_HEADER) != 0; }

  void setReturnTag(NABoolean v) { (v ? flags_ |= RETURN_TAG : flags_ &= ~RETURN_TAG); }
  NABoolean returnTag() { return (flags_ & RETURN_TAG) != 0; }

  void setSkipLock(NABoolean v) { (v ? flags_ |= SKIP_LOCK : flags_ &= ~SKIP_LOCK); }
  NABoolean getSkipLock() { return (flags_ & SKIP_LOCK) != 0; }

  void setCleanupLock(NABoolean v) { (v ? flags_ |= CLEANUP_LOCK : flags_ &= ~CLEANUP_LOCK); }
  NABoolean getCleanupLock() { return (flags_ & CLEANUP_LOCK) != 0; }

  void setProgressStatus(NABoolean v) { (v ? flags_ |= PROGRESS_STATUS : flags_ &= ~PROGRESS_STATUS); }
  NABoolean getProgressStatus() { return (flags_ & PROGRESS_STATUS) != 0; }

  void setDropProgressTable(NABoolean v) { (v ? flags_ |= DROP_PROGRESS_TABLE : flags_ &= ~DROP_PROGRESS_TABLE); }
  ...
  void setFastRecovery(NABoolean v) { (v ? flags_ |= FAST_RECOVERY : flags_ &= ~FAST_RECOVERY); }
...
class ExeUtilExpr : public GenericUtilExpr {
 public:
  ...
  ExeUtilExpr(ExeUtilType type, const CorrName &name, ExprNode *exprNode, RelExpr *child, char *stmtText,
              CharInfo::CharSet stmtTextCharSet, CollHeap *oHeap = CmpCommon::statementHeap())
  ...
  ExeUtilExpr() : GenericUtilExpr(REL_EXE_UTIL){};
  ...
  void setUtilTableDesc(TableDesc *newId) { tableId_ = newId; }
...
class ExeUtilDisplayExplain : public ExeUtilExpr {
 public:
  ExeUtilDisplayExplain(ExeUtilType opType, char *stmtText, CharInfo::CharSet stmtTextCharSet, char *moduleName = NULL,
                        char *stmtName = NULL, char *optionsStr = NULL, ExprNode *exprNode = NULL,
                        CollHeap *oHeap = CmpCommon::statementHeap());
...
class ExeUtilDisplayExplainComplex : public ExeUtilDisplayExplain {
 public:
  enum ComplexExplainType { CREATE_TABLE_, CREATE_INDEX_, CREATE_MV_, CREATE_TABLE_AS };

  ExeUtilDisplayExplainComplex(char *stmtText, CharInfo::CharSet stmtTextCharSet, char *optionsStr = NULL,
                               ExprNode *exprNode = NULL, CollHeap *oHeap = CmpCommon::statementHeap());
...
class ExeUtilWnrInsert : public ExeUtilExpr {
 public:
  ExeUtilWnrInsert(const CorrName &name, RelExpr *child, CollHeap *oHeap = CmpCommon::statementHeap());
...
class ExeUtilLoadVolatileTable : public ExeUtilExpr {
 public:
  ExeUtilLoadVolatileTable(const CorrName &name, ExprNode *exprNode, CollHeap *oHeap = CmpCommon::statementHeap())
      : ExeUtilExpr(LOAD_VOLATILE_, name, exprNode, NULL, NULL  // in - char * stmt
                    ,
...
class ExeUtilProcessVolatileTable : public DDLExpr {
 public:
  ExeUtilProcessVolatileTable(ExprNode *exprNode, char *stmtText, CharInfo::CharSet stmtTextCharSet,
                              CollHeap *oHeap = CmpCommon::statementHeap())
...
class ExeUtilProcessExceptionTable : public DDLExpr {
 public:
  ExeUtilProcessExceptionTable(ExprNode *exprNode, char *stmtText, CharInfo::CharSet stmtTextCharSet,
                               CollHeap *oHeap = CmpCommon::statementHeap())
      : DDLExpr(exprNode, stmtText, stmtTextCharSet, oHeap){};
...
class ExeUtilCleanupVolatileTables : public ExeUtilExpr {
 public:
  ...
  ExeUtilCleanupVolatileTables(CleanupType type = OBSOLETE_TABLES_IN_DEFAULT_CAT, const NAString &catName = "",
                               CollHeap *oHeap = CmpCommon::statementHeap())
      : ExeUtilExpr(CLEANUP_VOLATILE_TABLES_, CorrName("dummyName"), NULL, NULL, NULL  // in - char * stmt
                    ,
                    CharInfo::UnknownCharSet  // in - CharInfo::CharSet stmtCharSet
                    ,
                    oHeap  // in - CollHeap * heap
...
class ExeUtilGetVolatileInfo : public ExeUtilExpr {
 public:
  enum InfoType { ALL_SCHEMAS, ALL_TABLES, ALL_TABLES_IN_A_SESSION };

  ExeUtilGetVolatileInfo(InfoType type, const NAString &sessionId = "", CollHeap *oHeap = CmpCommon::statementHeap())
      : ExeUtilExpr(GET_VOLATILE_INFO_, CorrName("dummyName"), NULL, NULL, NULL  // in - char * stmt
                    ,
                    CharInfo::UnknownCharSet  // in - CharInfo::CharSet stmtCharSet
                    ,
                    oHeap  // in - CollHeap * heap
...
class ExeUtilCreateTableAs : public ExeUtilExpr {
 ...
 public:
  ExeUtilCreateTableAs(const CorrName &name, ExprNode *exprNode, char *stmtText, CharInfo::CharSet stmtTextCharSet,
                       CollHeap *oHeap = CmpCommon::statementHeap())
...
class ExeUtilConnectby : public ExeUtilExpr {
 ...
 public:
  ExeUtilConnectby(const CorrName &TableName, char *stmtText, CharInfo::CharSet stmtTextCharSet, RelExpr *scan,
                   CollHeap *oHeap = CmpCommon::statementHeap())
      : ExeUtilExpr(CONNECT_BY_, TableName, NULL, scan, stmtText, stmtTextCharSet, oHeap) {
    hasStartWith_ = TRUE;
    myselection_ = NULL;
    noCycle_ = FALSE;
    nodup_ = FALSE;
  ...
  void setHasIsLeaf(NABoolean v) { v ? flags_ |= HAS_IS_LEAF : flags_ &= ~HAS_IS_LEAF; }

  void setHasConnectByPath(NABoolean v) { v ? flags_ |= HAS_CONNECT_BY_PATH : flags_ &= ~HAS_CONNECT_BY_PATH; }

  void setHasDynParamInStart(NABoolean v) { v ? flags_ |= HAS_DYN_PARAM_IN_START : flags_ &= ~HAS_DYN_PARAM_IN_START; }

  NABoolean hasIsLeaf() const { return (flags_ & HAS_IS_LEAF) != 0; }

  void setDual(NABoolean v = TRUE) { isDual_ = TRUE; }
  NABoolean isDual() { return isDual_; }

  NABoolean hasConnectByPath() const { return (flags_ & HAS_CONNECT_BY_PATH) != 0; }
...
class ExeUtilMaintainObject : public ExeUtilExpr {
 public:
  enum MaintainObjectOptionType {
    ...
    RUN_,
    IF_NEEDED_,
    GET_SCHEMA_LABEL_STATS_,
    GET_LABEL_STATS_,
    GET_TABLE_LABEL_STATS_,
    GET_INDEX_LABEL_STATS_,
    GET_LABELSTATS_INC_INDEXES_,
    GET_LABELSTATS_INC_INTERNAL_,
    GET_LABELSTATS_INC_RELATED_
  };
   ...
   public:
    MaintainObjectOption(MaintainObjectOptionType option, int numericVal1, const char *stringVal1, int numericVal2 = 0,
        ...
        : option_(option),
          ...
          numericVal2_(numericVal2),
          stringVal2_(stringVal2) {}
  ...
  ExeUtilMaintainObject(enum MaintainObjectType type, const CorrName &name, QualNamePtrList *multiTablesNames,
  ...
  void setParams(NABoolean all, NABoolean reorgTable, NABoolean reorgIndex, NABoolean updStatsTable,
                 NABoolean updStatsMvlog, NABoolean updStatsMvs, NABoolean updStatsMvgroup, NABoolean updStatsAllMvs,
                 NABoolean refreshAllMvgroup, NABoolean refreshMvgroup, NABoolean refreshAllMvs, NABoolean refreshMvs,
                 NABoolean reorgMvgroup, NABoolean reorgMvs, NABoolean reorgMvsIndex, NABoolean cleanMaintainCIT,
                 NABoolean continueOnError, NABoolean returnSummary, NABoolean returnDetailOutput, NABoolean noOutput,
                 NABoolean display, NABoolean displayDetail, NABoolean getLabelStats, NABoolean getTableLabelStats,
                 NABoolean getIndexLabelStats, NABoolean getLabelStatsIncIndexes, NABoolean getLabelStatsIncInternal,
                 NABoolean getLabelStatsIncRelated, NABoolean getSchemaLabelStats);

  void setOptionsParams(const NAString &reorgTableOptions, const NAString &reorgIndexOptions,
                        ...
                        const NAString &reorgMvsIndexOptions, const NAString &cleanMaintainCITOptions);

  void setControlParams(NABoolean disableReorgTable, NABoolean enableReorgTable, NABoolean disableReorgIndex,
                        NABoolean enableReorgIndex, NABoolean disableUpdStatsTable, NABoolean enableUpdStatsTable,
                        NABoolean disableUpdStatsMvs, NABoolean enableUpdStatsMvs, NABoolean disableRefreshMvs,
                        NABoolean enableRefreshMvs, NABoolean disableReorgMvs, NABoolean enableReorgMvs,
                        NABoolean resetReorgTable, NABoolean resetUpdStatsTable, NABoolean resetUpdStatsMvs,
                        NABoolean resetRefreshMvs, NABoolean resetReorgMvs, NABoolean resetReorgIndex,
                        NABoolean enableUpdStatsMvslog, NABoolean disableUpdStatsMvslog, NABoolean resetUpdStatsMvslog,
                        NABoolean enableReorgMvsIndex, NABoolean disableReorgMvsIndex, NABoolean resetReorgMvsIndex,
                        NABoolean enableRefreshMvgroup, NABoolean disableRefreshMvgroup, NABoolean resetRefreshMvgroup,
                        NABoolean enableReorgMvgroup, NABoolean disableReorgMvgroup, NABoolean resetReorgMvgroup,
                        NABoolean enableUpdStatsMvgroup, NABoolean disableUpdStatsMvgroup,
                        NABoolean resetUpdStatsMvgroup, NABoolean enableTableLabelStats,
                        NABoolean disableTableLabelStats, NABoolean resetTableLabelStats,
  ...
  void setMvLogTable(NATable *newTable) { mvLogTable_ = newTable; }
...
class ExeUtilGetObjectEpochStats : public ExeUtilExpr {
 public:
  ExeUtilGetObjectEpochStats(const CorrName *objectName, short cpu, bool locked, CollHeap *oHeap)
      : ExeUtilExpr(GET_OBJECT_EPOCH_STATS_, objectName != NULL ? *objectName : CorrName("dummyName"), NULL, NULL, NULL,
...
class ExeUtilGetObjectLockStats : public ExeUtilExpr {
 public:
  ExeUtilGetObjectLockStats(const CorrName *objectName, short cpu, bool entry, CollHeap *oHeap)
      : ExeUtilExpr(GET_OBJECT_LOCK_STATS_, objectName != NULL ? *objectName : CorrName("dummyName"), NULL, NULL, NULL,
                    CharInfo::UnknownCharSet, oHeap),
        cpu_(cpu),
        objectNameSpecified_(objectName != NULL),
        entry_(entry) {}
...
class ExeUtilGetStatistics : public ExeUtilExpr {
 public:
  ExeUtilGetStatistics(NAString statementName = "", char *optionsStr = NULL,
                       CollHeap *oHeap = CmpCommon::statementHeap(), short statsReqType = SQLCLI_STATS_REQ_STMT,
...
class ExeUtilGetProcessStatistics : public ExeUtilGetStatistics {
 public:
  ExeUtilGetProcessStatistics(NAString pid = "", char *optionsStr = NULL, CollHeap *oHeap = CmpCommon::statementHeap());
...
class ExeUtilGetMetadataInfo : public ExeUtilExpr {
 public:
  ...
  ExeUtilGetMetadataInfo(NAString &ausStr, NAString &infoType, NAString &iofStr, NAString &objectType,
                         CorrName &objectName, NAString *pattern, NABoolean returnFullyQualNames, NABoolean getVersion,
                         NAString *param1, CollHeap *oHeap = CmpCommon::statementHeap());
...
class ExeUtilShowSet : public ExeUtilExpr {
 public:
  enum ShowSessionDefaultType { ALL_DEFAULTS_, EXTERNALIZED_DEFAULTS_, SINGLE_DEFAULT_ };

  ExeUtilShowSet(ShowSessionDefaultType type, const NAString &ssdName = "",
                 CollHeap *oHeap = CmpCommon::statementHeap())
      : ExeUtilExpr(SHOWSET_DEFAULTS_, CorrName("dummyName"), NULL, NULL, (char *)NULL  // in - char * stmt
                    ,
                    CharInfo::UnknownCharSet  // in - CharInfo::CharSet stmtCharSet
                    ,
...
class ExeUtilAQR : public ExeUtilExpr {
 public:
  enum AQRTask { NONE_ = -1, GET_ = 0, ADD_, DELETE_, UPDATE_, CLEAR_, RESET_ };

  enum AQROptionType { SQLCODE_, NSKCODE_, RETRIES_, DELAY_, TYPE_ };
  ...
  ExeUtilAQR(AQRTask task, NAList<AQROption *> *aqrOptionsList, CollHeap *oHeap = CmpCommon::statementHeap());

  virtual RelExpr *copyTopNode(RelExpr *derivedNode = NULL, CollHeap *outHeap = 0);

  virtual RelExpr *bindNode(BindWA *bindWAPtr);

  virtual NABoolean producesOutput() { return (task_ == GET_ ? TRUE : FALSE); }
...
class ExeUtilRegionStats : public ExeUtilExpr {
 public:
  ExeUtilRegionStats(const CorrName &objectName, NABoolean summaryOnly, NABoolean isIndex, NABoolean forDisplay,
                     NABoolean clusterView, RelExpr *child, CollHeap *oHeap = CmpCommon::statementHeap());

  ExeUtilRegionStats() : summaryOnly_(FALSE), isIndex_(FALSE), displayFormat_(FALSE), clusterView_(FALSE) {}

  ExeUtilRegionStats(NABoolean clusterView)
      : summaryOnly_(FALSE), isIndex_(FALSE), displayFormat_(FALSE), clusterView_(clusterView) {}
  ...
  virtual const char *getVirtualTableName();
  static const char *getVirtualTableNameStr() { return "EXE_UTIL_REGION_STATS__"; }

  virtual TrafDesc *createVirtualTableDesc();

  static const char *getVirtualTableClusterViewNameStr() { return "EXE_UTIL_CLUSTER_STATS__"; }
...
class ExeUtilLongRunning : public ExeUtilExpr {
 public:
  ...
  void setNoLogOperation(NABoolean isNoLoggingOperation = FALSE) { isNoLogOperation_ = isNoLoggingOperation; }
...
class ExeUtilGetUID : public ExeUtilExpr {
 public:
  ExeUtilGetUID(const CorrName &name, enum ExeUtilMaintainObject::MaintainObjectType type,
                CollHeap *oHeap = CmpCommon::statementHeap());
...
class ExeUtilGetQID : public ExeUtilExpr {
 public:
  ExeUtilGetQID(NAString &statement, CollHeap *oHeap = CmpCommon::statementHeap());
...
class ExeUtilPopulateInMemStats : public ExeUtilExpr {
 public:
  ExeUtilPopulateInMemStats(const CorrName &inMemTableName, const CorrName &sourceTableName,
                            const SchemaName *sourceStatsSchemaName, CollHeap *oHeap = CmpCommon::statementHeap());
  ...
  const SchemaName &getSourceStatsSchemaName() const { return sourceStatsSchemaName_; }
...
class ExeUtilHbaseDDL : public ExeUtilExpr {
 public:
  enum DDLtype { INIT_MD_, DROP_MD_, CREATE_, DROP_ };

  ExeUtilHbaseDDL(const CorrName &hbaseTableName, DDLtype type, ConstStringList *csl,
                  CollHeap *oHeap = CmpCommon::statementHeap())
      : ExeUtilExpr(HBASE_DDL_, hbaseTableName, NULL, NULL, NULL, CharInfo::UnknownCharSet, oHeap),
        type_(type),
        csl_(csl){};
...
class ExeUtilMetadataUpgrade : public ExeUtilExpr {
 public:
  ExeUtilMetadataUpgrade(CollHeap *oHeap = CmpCommon::statementHeap())
      : ExeUtilExpr(METADATA_UPGRADE_, CorrName("dummyName"), NULL, NULL, NULL  // in - char * stmt
                    ,
                    CharInfo::UnknownCharSet  // in - CharInfo::CharSet stmtCharSet
                    ,
                    oHeap  // in - CollHeap * heap
  ...
  void setGetMDVersion(NABoolean v) { (v ? myFlags_ |= GET_MD_VERSION : myFlags_ &= ~GET_MD_VERSION); }
  NABoolean getMDVersion() { return (myFlags_ & GET_MD_VERSION) != 0; }

  void setGetSWVersion(NABoolean v) { (v ? myFlags_ |= GET_SW_VERSION : myFlags_ &= ~GET_SW_VERSION); }
  NABoolean getSWVersion() { return (myFlags_ & GET_SW_VERSION) != 0; }

 private:
  enum Flags { GET_MD_VERSION = 0x0001, GET_SW_VERSION = 0x0002 };
...
class ExeUtilHBaseBulkLoad : public ExeUtilExpr {
 public:
  ...
  ExeUtilHBaseBulkLoad(const CorrName &hBaseTableName, ExprNode *exprNode, char *stmtText,
                       CharInfo::CharSet stmtTextCharSet, RelExpr *queryExpression,
                       CollHeap *oHeap = CmpCommon::statementHeap())
      : ExeUtilExpr(HBASE_LOAD_, hBaseTableName, exprNode, NULL, stmtText, stmtTextCharSet, oHeap),
  ...
  NABoolean getKeepHFiles() const { return keepHFiles_; }

  void setKeepHFiles(NABoolean keepHFiles) { keepHFiles_ = keepHFiles; }
  NABoolean getContinueOnError() const { return continueOnError_; }

  void setContinueOnError(NABoolean v) { continueOnError_ = v; }

  NABoolean getLogErrorRows() const { return logErrorRows_; }

  void setLogErrorRows(NABoolean v) { logErrorRows_ = v; }

  NABoolean getNoRollback() const { return noRollback_; }
  ...
  NABoolean getTruncateTable() const { return truncateTable_; }

  void setTruncateTable(NABoolean truncateTable) { truncateTable_ = truncateTable; }

  NABoolean getNoDuplicates() const { return noDuplicates_; }

  void setNoDuplicates(NABoolean v) { noDuplicates_ = v; }
  NABoolean getConstraints() const { return constraints_; }

  void setConstraints(NABoolean constraints) { constraints_ = constraints; }

  NABoolean getRebuildIndexes() const { return rebuildIndexes_; }

  void setRebuildIndexes(NABoolean indexes) { rebuildIndexes_ = indexes; }

  NABoolean getNoOutput() const { return noOutput_; }

  void setNoOutput(NABoolean noOutput) { noOutput_ = noOutput; }

  NABoolean getIndexTableOnly() const { return indexTableOnly_; }

  void setIndexTableOnly(NABoolean indexTableOnly) { indexTableOnly_ = indexTableOnly; }

  NABoolean getHasUniqueIndexes() const { return hasUniqueIndexes_; }

  void setHasUniqueIndexes(NABoolean uniqIndex) { hasUniqueIndexes_ = uniqIndex; }

  NABoolean getUpsertUsingLoad() const { return upsertUsingLoad_; }

  void setUpsertUsingLoad(NABoolean upsertUsingLoad) { upsertUsingLoad_ = upsertUsingLoad; }

  NABoolean getUpdateStats() const { return updateStats_; }

  void setUpdateStats(NABoolean updateStats) { updateStats_ = updateStats; }
  void setMaxErrorRows(UInt32 v) { maxErrorRows_ = v; }
  UInt32 getMaxErrorRows() { return maxErrorRows_; }
  void setLogErrorRowsLocation(char *str) { logErrorRowsLocation_ = str; }
  ...
  short setOptions(NAList<ExeUtilHBaseBulkLoad::HBaseBulkLoadOption *> *hBaseBulkLoadOptionList, ComDiagsArea *da);
...
class ExeUtilHBaseBulkLoadTask : public ExeUtilExpr {
 public:
  ...
  ExeUtilHBaseBulkLoadTask(const CorrName &hBaseTableName, ExprNode *exprNode, char *stmtText,
                           CharInfo::CharSet stmtTextCharSet, TaskType ttype,
                           CollHeap *oHeap = CmpCommon::statementHeap(), NABoolean bWithUstats = FALSE)
      : ExeUtilExpr(HBASE_LOAD_TASK_, hBaseTableName, exprNode, NULL, stmtText, stmtTextCharSet, oHeap),
        taskType_(ttype),
...
class ExeUtilCompositeUnnest : public ExeUtilExpr {
 public:
  enum AdditionalColsType { NONE, KEY_COLUMNS, ALL_COLUMNS };

  ExeUtilCompositeUnnest(ItemExpr *val1Ptr, NABoolean isOuter,

                         // REL_JOIN, REL_LEFT_JOIN, REL_RIGHT_JOIN, REL_FULL_JOIN
                         // Valid is isOuter is TRUE.
                         OperatorTypeEnum outerType,

                         NABoolean showPos, AdditionalColsType addnlCols, const NAString &virtTableName,
                         ItemExpr *colNames, CollHeap *oHeap = CmpCommon::statementHeap());
...
class RelDumpLoad : public DDLExpr {
 ...
 public:
  RelDumpLoad(char *stmtText, ComObjectType dlObjType, CharInfo::CharSet stmtTextCharSet,
              CollHeap *oHeap = CmpCommon::statementHeap())
      : DDLExpr(NULL, stmtText, stmtTextCharSet, oHeap), heap_(oHeap) {
    if (dlObjType == COM_BASE_TABLE_OBJECT)
      dlObjType_ = DL_OBJ_TABLE;
    else if (dlObjType == COM_LIBRARY_OBJECT)
      dlObjType_ = DL_OBJ_LIBRARY;
    else if (dlObjType == COM_VIEW_OBJECT)
      dlObjType_ = DL_OBJ_VIEW;
    else if (dlObjType == COM_USER_DEFINED_ROUTINE_OBJECT)
      dlObjType_ = DL_OBJ_UDRO;
    else if (dlObjType == COM_SEQUENCE_GENERATOR_OBJECT)
      dlObjType_ = DL_OBJ_SGO;
    else if (dlObjType == (ComObjectType)DL_OBJ_SCHEMA)
      dlObjType_ = DL_OBJ_SCHEMA;
    else if (dlObjType == COM_STORED_PROCEDURE_OBJECT)
      dlObjType_ = DL_OBJ_PROCEDURE;
    else if (dlObjType == COM_TRIGGER_OBJECT)
      dlObjType_ = DL_OBJ_TRIGGER;
    else if (dlObjType == COM_PACKAGE_OBJECT)
      dlObjType_ = DL_OBJ_PACKAGE;
    else
      dlObjType_ = DL_OBJ_UNKNOWN;
  };
  ...
  virtual RelExpr *copyTopNode(RelExpr *derivedNode = NULL, CollHeap *outHeap = 0);
  virtual const NAString getText();
  virtual void unparse(NAString &result, PhaseEnum /*phase*/
                       ,
  ...
  void setDBObjectName(SchemaName &name) {
    ...
    if (dlObjType_ != DL_OBJ_SCHEMA) objName_.setObjectName(((QualifiedName *)&name)->getObjectName());
  }
...
class RelGenLoadQueryCache : public DDLExpr {
 public:
  ...
  RelGenLoadQueryCache(char *stmtText, GenLoadQueryCacheType type, CharInfo::CharSet stmtTextCharSet,
                       CollHeap *oHeap = CmpCommon::statementHeap())
      : DDLExpr(NULL, stmtText, stmtTextCharSet, oHeap), heap_(oHeap), type_(type), offset_(-1){};
...
class ExeUtilSnapShotUpdataDelete : public ExeUtilExpr {
 public:
  ...
  ExeUtilSnapShotUpdataDelete(const CorrName &tableName, char *stmtText, ExeUtilUpdataDeleteType type,
                              CharInfo::CharSet stmtTextCharSet, CollHeap *oHeap = CmpCommon::statementHeap())
