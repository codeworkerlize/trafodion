...
class Histograms : public NABasicObject {
 public:
  ...
  CollIndex entries() const { return colStatDescList_.entries(); }

  // get the ColStatDesc in the i-th position:
  const ColStatDesc &operator[](CollIndex i) const { return *(colStatDescList_[i]); }

  // Check whether a ColStatDesc for a given column already exists:
  NABoolean contains(ValueId &column) const { return colStatDescList_.contains(column); }
  ...
  // always return FALSE.
  NABoolean isEmpty() const { return colStatDescList_.isEmpty(); }
  ...
  inline void setCSDLUecList(const MultiColumnUecList *other) { colStatDescList_.insertIntoUecList(other); }
  ...
  // -----------------------------------------------------------------------
  NABoolean isAnIndexJoin(const EstLogProp &inputEstLogProp, LIST(CollIndex) *innerIndexColumnListPtr = NULL) const;
  ...
  void applyPredicates(const ValueIdSet &predicates, const RelExpr &scan, const SelectivityHint *selHint = NULL,
                       const CardinalityHint *cardHint = NULL, OperatorTypeEnum opType = ITM_FIRST_ITEM_OP);
  // Apply a single predicate to this histogram
  void applyPredicate(const ValueId &predicate, const RelExpr &scan, const SelectivityHint *selHint = NULL,
                      const CardinalityHint *cardHint = NULL, OperatorTypeEnum opType = ITM_FIRST_ITEM_OP);
  ...
  // -----------------------------------------------------------------------
  void applyPredicatesWhenMultipleProbes(const ValueIdSet &predicates, const EstLogProp &inputEstLogProp,
                                         const ValueIdSet &inputValues, const NABoolean isMDAM,
                                         const SelectivityHint *selHint = NULL, const CardinalityHint *cardHint = NULL,
                                         NABoolean *isAnIndexJoin = NULL, OperatorTypeEnum opType = ITM_FIRST_ITEM_OP);
...
class IndexDescHistograms : public Histograms {
 public:
  ...
  IndexDescHistograms(const IndexDesc &indexDesc, const CollIndex columnPosition);
  ...
  CostScalar computeFailedProbes(const Histograms &outerHistograms, const ValueIdSet &keyPreds,
                                 const ValueIdSet &inputValues, const ValueIdSet &operatorValues) const;
  ...
  //--------------------------------------------------------------------------
  NABoolean estimateUecUsingMultiColUec(const ColumnOrderList &keyPredsByCol, /*in*/
...
class ScanOptimizer : public NABasicObject  // Abstract class
{
 ...
 public:
  ...
  ScanOptimizer(const FileScan &associatedFileScan, const CostScalar &resultSetCardinality, const Context &myContext,
  ...
  const CostScalar getEstRowsAccessed() const { return estRowsAccessed_; }
  ...
  //
  static ScanOptimizer *getScanOptimizer(const FileScan &associatedFileScan, const CostScalar &resultSetCardinality,
  ...
  //
  static NABoolean useSimpleFileScanOptimizer(const FileScan &associatedFileScan, const Context &myContext,
                                              const ValueIdSet &externalInputs);

  static NABoolean canStillConsiderMDAM(const ValueIdSet partKeyPreds, const ValueIdSet nonKeyColumnSet,
  ...
  const CostScalar getDuplicateSuccProbes() const { return duplicateSuccProbes_; };
 ...
 protected:
  ...
  // -----------------------------------------------------------------------
  Cost *computeCostObject(const SimpleCostVector &firstRow, const SimpleCostVector &lastRow) const;
  ...
  CostScalar scmRowSizeFactor(CostScalar rowSize, ncmRowSizeFactorType rowSizeFactoryType = TUPLES_ROWSIZE_FACTOR);
  ...
  //              not a correctness issue, it is an efficiency issue.
  // -----------------------------------------------------------------------
  virtual void categorizeProbes(CostScalar &successfulProbes /* out */
                                ,
                                ...
                                CostScalar *dataRows = NULL) const;

  // Accesors:

  const CostScalar &getSingleSubsetSize() const { return singleSubsetSize_; }

  const CostScalar getResultSetCardinality() const { return resultSetCardinality_; }

  const CostScalar getIndexLevelsSeeks() const;

  NABoolean getInOrderProbesFlag() const { return inOrderProbes_; }

  NABoolean getProbesForceSynchronousAccessFlag() const { return probesForceSynchronousAccess_; }

  const Context &getContext() const { return context_; }

  // Mutators:

  void setSingleSubsetSize(const CostScalar &singleSubsetSize) { singleSubsetSize_ = singleSubsetSize; }

  void setInOrderProbesFlag(NABoolean probesAreInOrder) { inOrderProbes_ = probesAreInOrder; }

  void setProbesForceSynchronousAccessFlag(NABoolean probesForceSynchronousAccess) {
  ...
  void setEstRowsAccessed(CostScalar rows) { estRowsAccessed_ = rows; }

  // With overflow checks
  void setNumberOfBlocksToReadPerAccess(const CostScalar &blocks);

  const RelExpr &getRelExpr() const { return fileScan_; }

  const FileScan &getFileScan() const { return fileScan_; }

  const IndexDesc *getIndexDesc() const { return fileScan_.getIndexDesc(); }

  NABoolean isForwardScan() const { return (NOT fileScan_.getReverseScan()); }

  NABoolean getMdamFlag() const { return fileScan_.getMdamFlag(); }

  const Disjuncts &getDisjuncts() const { return fileScan_.getDisjuncts(); }

  NABoolean isMdamForced() const { return isMdamForced(fileScan_, getContext()); }

  // Static version, used by useSimpleFileScanOptimizer()
  //
  static NABoolean isMdamForced(const FileScan &fileScan, const Context &myContext);

  // Static method, used by useSimpleFileScanOptimizer()
  // Determine whether MDAM is Forced ON, Forced OFF, or ENABLED.
  //
  ScanForceWildCard::scanOptionEnum static getMdamStatus(const FileScan &fileScan, const Context &myContext);
  ...
  void printCostObject(const Cost *costPtr) const;
#endif
...
class FileScanOptimizer : public ScanOptimizer {
 ...
 public:
  FileScanOptimizer(const FileScan &associatedFileScan, const CostScalar &resultSetCardinality,
                    const Context &myContext, const ValueIdSet &externalInputs)
      : ScanOptimizer(associatedFileScan, resultSetCardinality, myContext, externalInputs),
  ...
  const IndexDescHistograms &getRawInnerHistograms() const { return rawInnerHistograms_; }
 ...
 private:
  ...
  void computeNumberOfBlocksToReadPerAccess(const Cost &scanCost, NABoolean &isMDAM, CostScalar numKBytes);

  void computeIOForFullCacheBenefit(CostScalar &seeks /* out */
                                    ,
  ...
  void computeSeekForDp2ReadAheadAndProbeOrder(CostScalar &seekComputedWithDp2ReadAhead, const CostScalar &finalRows,
                                               const CostScalar &uniqueProbes, const CostScalar &beginBlocksLowerBound,
                                               const CostScalar &totalBlocksLowerBound,
                                               const CostScalar &innerBlocksUpperBound, const CostScalar &dp2CacheSize,
                                               const NABoolean inOrderProbes) const;

  void computeIOForRandomCase(CostScalar &seeks /* out */
                              ,
                              CostScalar &sequential_io /* out */
                              ,
                              const CostScalar &blksPerSuccProbe, const CostScalar &beginBlocksLowerBound,
                              const CostScalar &totalBlocksLowerBound, const CostScalar &successfulProbes,
                              const CostScalar &failedProbes, const CostScalar &probes) const;

  void computeIOForFullTableScan(CostScalar &dataRows /* out */
                                 ,
  ...
  void computeCostVectors(SimpleCostVector &firstRow /* out */
                          ,
                          SimpleCostVector &lastRow /* out */
                          ,
                          CostScalar &seqKBytesPerScan /* out */
                          ,
                          const CostScalar &totalRows, const CostScalar &subsetRequests,
                          const CostScalar &successfulSubsetRequests, const CostScalar &seeks,
                          const CostScalar &sequential_io, const ValueIdSet &keyPredicates, const ValueIdSet &exePreds,
                          const CostScalar &incomingProbes  // probes incoming to the operator
  ) const;

  void computeCostVectorsForMultipleSubset(SimpleCostVector &firstRow /* out */
                                           ,
                                           SimpleCostVector &lastRow /* out */
                                           ,
                                           CostScalar &seqKBytesPerScan /* out */
                                           ...
                                           const CostScalar &totalRows, const CostScalar &subsetRequests,
                                           const CostScalar &successfulSubsetRequests, const CostScalar &seeks,
                                           const CostScalar &sequential_io, const ValueIdSet &keyPredicates,
  ...
  Cost *computeCostForSingleSubset(SearchKey &searchKey /* in/out */
                                   ,
                                   const NABoolean &breakOnConflict, CostScalar &numKBytes);
  ...
  Cost *computeCostForMultipleSubset(MdamKey *mdamKeyPtr /* in/out */
                                     ,
                                     const Cost *costBoundPtr, NABoolean mdamForced, CostScalar &numKBytes,
                                     NABoolean checkExePreds, NABoolean mdanTypeIsCommon, MdamKey *&sharedMdamKeyPtr);
  Cost *oldComputeCostForMultipleSubset(MdamKey *mdamKeyPtr /* in/out */
                                        ,
                                        const Cost *costBoundPtr, NABoolean mdamForced, CostScalar &numKBytes,
                                        NABoolean checkExePreds, NABoolean mdanTypeIsCommon,
                                        MdamKey *&sharedMdamKeyPtr);

  Cost *newComputeCostForMultipleSubset(MdamKey *mdamKeyPtr, const Cost *costBoundPtr, NABoolean mdamForced,
                                        CostScalar &numKBytes, ValueIdSet exePreds, NABoolean checkExePreds,
                                        NABoolean mdamTypeIsCommon, MdamKey *&sharedMdamKeyPtr);

  Cost *scmComputeCostForSingleSubset();

  Cost *scmRewrittenComputeCostForMultipleSubset(MdamKey *mdamKeyPtr, const Cost *costBoundPtr, NABoolean mdamForced,
                                                 CostScalar &numKBytes, ValueIdSet exePreds, NABoolean checkExePreds,
                                                 MdamKey *&sharedMdamKeyPtr);

  Cost *scmComputeCostForMultipleSubset(MdamKey *mdamKeyPtr, const Cost *costBoundPtr, NABoolean mdamForced,
                                        CostScalar &numKBytes, ValueIdSet exePreds, NABoolean checkExePreds,
                                        NABoolean mdamTypeIsCommon, MdamKey *&sharedMdamKeyPtr);

  Cost *scmComputeMDAMCostForHbase(CostScalar &totalRows, CostScalar &seeks, CostScalar &sequential_io,
                                   CostScalar &incomingProbes);

#ifndef NDEBUG
  void runMdamTests(const MdamKey *mdamKeyPtr, const Cost *costBoundPtr, NABoolean mdamForced, ValueIdSet exePreds,
  ...
  // -----------------------------------------------------------------------
  NABoolean exceedsBound(const Cost *costBoundPtr, const SimpleCostVector &firstRow,
...
class FileScanBasicCost : public NABasicObject {
 public:
  ...
  SimpleCostVector &getFRBasicCostSingleSubset() { return basicFRCostSingleSubset_; }

  SimpleCostVector &getLRBasicCostSingleSubset() { return basicLRCostSingleSubset_; }

  SimpleCostVector &getFRBasicCostMdamCommon() { return basicFRCostMdamCommon_; }

  SimpleCostVector &getLRBasicCostMdamCommon() { return basicLRCostMdamCommon_; }

  SimpleCostVector &getFRBasicCostMdamDisjuncts() { return basicFRCostMdamDisjuncts_; }

  SimpleCostVector &getLRBasicCostMdamDisjuncts() { return basicLRCostMdamDisjuncts_; }

  MdamKey *getMdamKeyPtr(NABoolean mdamType) const { return (mdamType ? mdamCommonKeyPtr_ : mdamDisjunctsKeyPtr_); }

  void setMdamKeyPtr(MdamKey *mdamKeyPtr, NABoolean mdamType) {
    mdamType ? mdamCommonKeyPtr_ = mdamKeyPtr : mdamDisjunctsKeyPtr_ = mdamKeyPtr;
  }
  ...
  void setSingleSubsetNumKBytes(CostScalar numKBytes) { singleSubsetNumKBytes = numKBytes; }

  void setEstRowsAccessed(CostScalar estRows) { estRowsAccessed_ = estRows; }

  void setMdamCommonNumKBytes(CostScalar numKBytes) { mdamCommonNumKBytes = numKBytes; }

  void setMdamDisjunctsNumKBytes(CostScalar numKBytes) { mdamDisjunctsNumKBytes = numKBytes; }

  // To get the kbytes from singlesubset, common * disjunt

  CostScalar getSingleSubsetNumKBytes() { return singleSubsetNumKBytes; }

  CostScalar getEstRowsAccessed() { return estRowsAccessed_; }

  CostScalar getMdamCommonNumKBytes() { return mdamCommonNumKBytes; }

  CostScalar getMdamDisjunctsNumKBytes() { return mdamDisjunctsNumKBytes; }
...
class FileScanCostList : public LIST(FileScanBasicCost *) {
 public:
  FileScanCostList(NAMemory *h) : LIST(FileScanBasicCost *)(h){};
};
