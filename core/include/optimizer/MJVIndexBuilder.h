...
class ColIndSetBucketVector : public ARRAY(ColIndSetBucket *) {
 public:
  ...
  // default ctors
  //
  ColIndSetBucketVector(CollHeap *heap) : maxNonEmptyEntrySize_(0), heapPtr_(heap), ARRAY(ColIndSetBucket *)(heap){};
...
class ColIndSetMatrix : public ARRAY(EnumArray *) {
 public:
  ...
  // ctors
  //
  ColIndSetMatrix(const ColIndSetBucketVector &src, size_t size, CollHeap *heap);
...
class NestingStack : public LIST(ColIndSet *) {
 public:
  ...
  // ctors
  //
  NestingStack(CollHeap *heap) : heapPtr_(heap), LIST(ColIndSet *)(heap){};

  NestingStack(const NestingStack &other, CollHeap *heap) : heapPtr_(heap), LIST(ColIndSet *)(heap){};
...
class MJVIndCook : public LIST(NestingStack *) {
 public:
  ...
  // ctors
  //
  MJVIndCook(CollHeap *heap) : heapPtr_(heap), LIST(NestingStack *)(heap){};

  MJVIndCook(const MJVIndCook &other, CollHeap *heap) : heapPtr_(heap), LIST(NestingStack *)(heap){};
  ...
  //
  MJVIndCook &insertOnto(const ColIndSet &setToBeCovered, const ColIndSet &newSet);
...
class MJVIndexBuilder {
 public:
  ...
  // ctors
  //
  MJVIndexBuilder(CollHeap *heap) : heap_(heap){};
