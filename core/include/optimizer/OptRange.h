...
class OptRangeSpec : public RangeSpec {
 public:
   ...
   */
  static OptRangeSpec *createRangeSpec(QRDescGenerator *descGenerator, ItemExpr *predExpr, CollHeap *heap,
                                       NABoolean createForNormalizer = FALSE);

  void setID(ValueId vid) { vid_ = vid; }

  ValueId getID() const { return vid_; }
   ...
   * @see #placeSubrange
   */
  void addSubrange(ConstValue *start, ConstValue *end, NABoolean startInclusive, NABoolean endInclusive);
  ...
  QRValueId getRangeExprValueId() const { return (rangeExpr_ ? rangeExpr_->getValueId() : NULL_VALUE_ID); }
   ...
   */
  NABoolean rangeSubjectIsSet() { return rangeColValueId_ != NULL_VALUE_ID || rangeExpr_ != NULL; }
  ...
  void setRangeSubject(OptRangeSpec *otherRange) {
    assertLogAndThrow(CAT_SQL_COMP_RANGE, logLevel_, !rangeSubjectIsSet(), QRLogicException,
  ...
  NABoolean sameRangeSubject(OptRangeSpec *otherRange) {
    return rangeSubjectIsSet() && rangeColValueId_ == otherRange->rangeColValueId_ &&
           rangeExpr_ == otherRange->rangeExpr_;
  }
  ...
  NAList<SubrangeBase *> &getSubranges() { return subranges_; }
 ...
 protected:
   ...
   */
  ItemExpr *makeSubrangeORBackbone(SubrangeBase *subrange, ItemExpr *subrangeItem) const;
   ...
   */
  ItemExpr *makeSubrangeItemExpr(SubrangeBase *subrange, ItemExpr *subrangeItem) const;
...
class OptNormRangeSpec : public OptRangeSpec {
 public:
  OptNormRangeSpec(QRDescGenerator *descGenerator, CollHeap *heap = NULL)
      : OptRangeSpec(descGenerator, heap, LL_ERROR), originalItemExpr_(NULL) {}

  virtual ~OptNormRangeSpec() {}

  virtual NABoolean forNormalizer() const { return TRUE; }
   ...
   *         from.
   */
  virtual const ItemExpr *getOriginalItemExpr() const { return originalItemExpr_; }

  virtual void setOriginalItemExpr(ItemExpr *itemExpr) { originalItemExpr_ = itemExpr; }
...
class RangeInfo : public NABasicObject {
 public:
  RangeInfo(OptRangeSpec *range, QRJBBPtr jbbElem) : rangeSpec_(range), jbbElem_(jbbElem) {
  ...
  const QRRangePredListPtr getOwningList() const { return owningList_; }

  const QRJBBPtr getJbbElem() const { return jbbElem_; }
