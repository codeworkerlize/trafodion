...
class CatalogName : public NABasicObject {
 public:
  // constructors
  CatalogName(const NAString &catalogName = "", CollHeap *h = 0, StringPos namePos = 0)
      : catalogName_(catalogName, h), isDelimited_(FALSE) {
    if (catalogName.isNull() && namePos == 0)
      namePos_ = 0;
    else {
  ...
  CatalogName(const CatalogName &cat, NAMemory *h)
      : catalogName_(cat.catalogName_, h), namePos_(cat.namePos_), isDelimited_(cat.isDelimited_) {}

  // accessors
  const NAString &getCatalogName() const { return catalogName_; }
  const NAString getCatalogNameAsAnsiString() const { return ToAnsiIdentifier(catalogName_); }
  ...
  void display() const;
  virtual void print(FILE *ofd = stdout, const char *indent = DEFAULT_INDENT, const char *title = "CatalogName") const;
...
class SchemaName : public CatalogName {
 public:
  ...
  SchemaName(CollHeap *h) : CatalogName("", h), schemaName_(h) {}

  SchemaName(const NAString &schemaName = "", const NAString &catalogName = "", CollHeap *h = 0, StringPos namePos = 0);

  // copy ctor
  SchemaName(const SchemaName &sch, NAMemory *h) : CatalogName(sch, h), schemaName_(sch.schemaName_, h) {}

  // accessors
  const NAString &getSchemaName() const { return schemaName_; }
  const NAString getSchemaNameAsAnsiString() const;
  const NAString getUnqualifiedSchemaNameAsAnsiString() const { return ToAnsiIdentifier(schemaName_); }

  // mutator
  void setSchemaName(const NAString &schName) { schemaName_ = schName; }

  // Display/print, for debugging.
  virtual void print(FILE *ofd = stdout, const char *indent = DEFAULT_INDENT, const char *title = "SchemaName") const;
...
class QualifiedName : public SchemaName {
 public:
  ...
  //
  QualifiedName(CollHeap *h = 0) : SchemaName(h), objectName_(h), objectNameSpace_(COM_UNKNOWN_NAME), flagbits_(0) {}

  QualifiedName(const NAString &objectName /* = "" */, const NAString &schemaName = "",
                const NAString &catalogName = "", CollHeap *h = 0, StringPos namePos = 0)
  ...
  const NAString getQualifiedNameAsAnsiString(size_t *lenArray /* array[5] */) const;
  const NAString getQualifiedNameAsAnsiNTFilenameString() const;
  const NAString &getObjectName() const { return objectName_; }
  const NAString getUnqualifiedObjectNameAsAnsiString() const { return ToAnsiIdentifier(objectName_); }
  ...
  void setIsVolatile(NABoolean v) { (v ? flagbits_ |= IS_VOLATILE : flagbits_ &= ~IS_VOLATILE); }

  NABoolean isGhost() const { return (flagbits_ & IS_GHOST) != 0; }
  void setIsGhost(NABoolean v) { (v ? flagbits_ |= IS_GHOST : flagbits_ &= ~IS_GHOST); }

  // If a routine name has 2 or 3 parts, it is ambigous because it can
  // be interpreted as a standalone routine ([cat.]sch.routine) or a
  // package routine ([sch.]pkg.routine)
  NABoolean isAmbiguous() { return (flagbits_ & IS_AMBIGUOUS) != 0; }
  void setIsAmbiguous(NABoolean v) { (v ? flagbits_ |= IS_AMBIGUOUS : flagbits_ &= ~IS_AMBIGUOUS); }
  void convertToPackageRoutineName() {
    if (!isAmbiguous()) return;
    assert(!getSchemaName().isNull());
    objectName_.prepend(".");
    objectName_.prepend(getSchemaName());
    schemaName_.clear();
    if (!getCatalogName().isNull()) {
      setSchemaName(getCatalogName());
      catalogName_.clear();
    }
 ...
 private:
  enum FlagBits { IS_VOLATILE = 0x01, IS_GHOST = 0x02, IS_AMBIGUOUS = 0x04 };
...
class PartitionClause : public NABasicObject {
 public:
  ...
  PartitionClause(ItemExpr *itemList, CollHeap *h = 0)
      : partName_("", h),
        beginPartNumber_(-1),
        endPartNumber_(-1),
        locationName_("", h),
  ...
  PartitionClause(const PartitionClause &pClause, CollHeap *h = 0)
      : partName_(pClause.partName_, h),
        ...
        partitionV2_(pClause.partitionV2_),
        partnValsSpecified_(pClause.partnValsSpecified_),
        partnVals_(pClause.partnVals_) {}
  ...
  const NABoolean partnValsSpecified() const { return partnValsSpecified_; }

  // mutator
  void setLocationName(const NAString &locName) { locationName_ = locName; }

  void setIsPartitionV2(NABoolean v) { partitionV2_ = v; }
  const NABoolean isPartitionV2() const { return partitionV2_; }
  ItemExpr *getPartnVals() { return partnVals_; }
 ...
 private:
  ...
  NABoolean partnValsSpecified_;

  NABoolean partitionV2_;
  ItemExpr *partnVals_;
};  // PartitionClause
...
class ExtendedQualName : public NABasicObject {
 public:
  ...
  // constructors
  ExtendedQualName(NAString objectName = "", NAString schemaName = "", NAString catalogName = "", CollHeap *h = 0,
                   NAString locName = "", StringPos namePos = 0)
  ...
  ExtendedQualName(const QualifiedName &qualName, CollHeap *h = 0, NAString locName = (const char *)"")
      : qualName_(qualName, h), type_(NORMAL_TABLE), utilityOpenId_(-1), isNSAOperation_(FALSE), partnClause_(h) {
    setLocationName(locName);
  }
  ...
  NABoolean isPartitionNameSpecified() const {
    return (getPartnClause().partnNameSpecified() || getPartnClause().partnNumSpecified());
  }
  NABoolean isPartitionRangeSpecified() const { return (getPartnClause().partnRangeSpecified()); }
  NABoolean isPartitionV2() const { return (getPartnClause().isPartitionV2()); }
  ...
  NABoolean isSpecialTable() const {
    return (type_ != NORMAL_TABLE) && (type_ != TRIGTEMP_TABLE) && (type_ != EXCEPTION_TABLE) &&
           (type_ != IUD_LOG_TABLE) && (type_ != RANGE_LOG_TABLE) && (type_ != GHOST_TABLE) &&
           (type_ != GHOST_IUD_LOG_TABLE);
  }

  NABoolean isGhost() const {
    return (type_ == GHOST_TABLE) || (type_ == GHOST_INDEX_TABLE) || (type_ == GHOST_MV_TABLE) ||
           (type_ == GHOST_IUD_LOG_TABLE);
  }

  // can this type of object be put in the NATable cache
  NABoolean isCacheable() const {
    return (type_ == NORMAL_TABLE) || (type_ == TRIGTEMP_TABLE) ||
           (type_ == EXCEPTION_TABLE)
           //|| (type_ == IUD_LOG_TABLE)
           //|| (type_ == RANGE_LOG_TABLE)
           || (type_ == INDEX_TABLE) || (type_ == SCHEMA_TABLE) || (type_ == SG_TABLE) || ((type_ == GHOST_TABLE));
  }
  ...
  static NABoolean isDescribableTableType(SpecialTableType type) {
    return (type == NORMAL_TABLE || type == INDEX_TABLE || type == IUD_LOG_TABLE || type == RANGE_LOG_TABLE ||
            type == MVS_UMD || type == MV_TABLE || type == TRIGTEMP_TABLE || type == EXCEPTION_TABLE ||
            type == GHOST_TABLE || type == GHOST_INDEX_TABLE || type == GHOST_MV_TABLE || type == GHOST_IUD_LOG_TABLE ||
            type == SG_TABLE || type == SCHEMA_TABLE);
  }
  ...
  void display() const;
  void print(FILE *ofd = stdout, const char *indent = DEFAULT_INDENT, const char *title = "ExtendedQualName") const;
...
class CorrName : public NABasicObject {
 private:
  enum FlagBits { OFF = 0, ISFABRICATED = 0x1, IS_VOLATILE = 0x2, IS_EXTERNAL = 0x4 };

 public:
  // constructors
  CorrName(const NAString &objectName = "", CollHeap *h = 0, const NAString &schemaName = "",
           const NAString &catalogName = "", const NAString &corrName = "", const NAString &locName = "",
           StringPos namePos = 0, HostVar *prototype = NULL)
      : qualName_(objectName, schemaName, catalogName, h, namePos),
  ...
  CorrName(const QualifiedName &qualName, CollHeap *h = 0, const NAString &corrName = "", const NAString &locName = "",
  ...
  CorrName(const NAString &corrName, NABoolean isFabricated, CollHeap *h = 0, HostVar *prototype = NULL)
      : qualName_("", "", h),
        corrName_(corrName, h),
        // ct-bug-10-030102-3803 -Begin
        ugivenName_("", h),
  ...
  ExtendedQualName::SpecialTableType getSpecialType() const { return qualName_.getSpecialType(); }
  ComAnsiNameSpace getAnsiNameSpace() const { return qualName_.getAnsiNameSpace(); }
  void setSpecialType(ExtendedQualName::SpecialTableType stt) { qualName_.setSpecialType(stt); }
  void setSpecialType(const CorrName &cn) { qualName_.setSpecialType(cn.getSpecialType()); }
  ...
  const QualifiedName &getQualifiedNameObj() const { return qualName_.getQualifiedNameObj(); }
  ...
  const NAString getExposedNameAsString(NABoolean debug = FALSE, NABoolean formatForDisplay = FALSE) const;
  const NAString getExposedNameAsAnsiString(NABoolean debug = FALSE, NABoolean formatForDisplay = FALSE) const;
  ...
  NABoolean isLocationNameSpecified() const { return qualName_.isLocationNameSpecified(); }
  NABoolean isPartitionNameSpecified() const { return qualName_.isPartitionNameSpecified(); }
  NABoolean isPartitionRangeSpecified() const { return qualName_.isPartitionRangeSpecified(); }
  NABoolean isPartitionV2() const { return qualName_.isPartitionV2(); }
  // ct-bug-10-030102-3803 -Begin
  ...
  void setPartnClause(const PartitionClause &pClause) { qualName_.setPartnClause(pClause); }
  ...
  void setIsExternal(NABoolean v) { (v ? flagbits_ |= IS_EXTERNAL : flagbits_ &= ~IS_EXTERNAL); }

  // Display/print, for debugging.
  const NAString getTextWithSpecialType() const { return qualName_.getTextWithSpecialType(); }

  const NAString getText() const;  // see RelExpr.C
  void display() const;
  void print(FILE *ofd = stdout, const char *indent = DEFAULT_INDENT, const char *title = "CorrName") const;
...
class ColRefName : public NABasicObject {
 public:
  ...
  ColRefName(CollHeap *h = 0) : colName_("", h), corrName_("", h), isStar_(NO_STAR), isOLAPInternal_(FALSE) {}
  ...
  ColRefName(const NAString &col, CollHeap *h = 0)
      : colName_(col, h), corrName_("", h), isStar_(NO_STAR), isOLAPInternal_(FALSE) {}

  ColRefName(const NAString &col, const CorrName &corr, CollHeap *h = 0)
      : colName_(col, h), corrName_(corr, h), isStar_(NO_STAR), isOLAPInternal_(FALSE) {}
  ...
  ColRefName(const ColRefName &colref, CollHeap *h = 0)
      : colName_(colref.colName_, h),
        ...
        isOLAPInternal_(colref.isOLAPInternal_) {}
  ...
  NABoolean getStarWithSystemAddedCols() const { return isStar_ == STAR_WITH_SYSTEM_ADDED_COLS; }
  ...
  const NAString getColRefAsAnsiString(NABoolean debug = FALSE, NABoolean formatForDisplay = FALSE,
                                       NABoolean removeIntCorrName = FALSE) const;
  const NAString getColRefAsString(NABoolean debug = FALSE, NABoolean formatForDisplay = FALSE,
                                   NABoolean removeIntCorrName = FALSE) const;
  static const NAString getColRefAsString(const NAString &col, const NAString &corr,
                                          const NABoolean removeIntCorrName = FALSE);
  NABoolean isDelimited() const { return corrName_.getQualifiedNameObj().isDelimited(); }

  // mutators
  void setSpecialType(ExtendedQualName::SpecialTableType stt) { corrName_.setSpecialType(stt); }
  void setNamePosition(StringPos pos, NABoolean setD = TRUE) { corrName_.setNamePosition(pos, setD); }
  ...
  // Display/print, for debugging.
  void display() const;
  void print(FILE *ofd = stdout, const char *indent = DEFAULT_INDENT, const char *title = "ColRefName",
             NABoolean brief = FALSE) const;

  NABoolean isQualified() const;
  void setOLAPInternal(NABoolean b) { isOLAPInternal_ = b; }
  NABoolean isOLAPInternal() const { return isOLAPInternal_; }
 ...
 private:
  ...
  NAString colName_;
  starType isStar_;
  // OLAP function column with no name
  NABoolean isOLAPInternal_;
};  // ColRefName
...
class TableRefName : public NABasicObject {
 public:
  ...
  TableRefName(const NAString &tableName, const NAString &refName, CollHeap *h = CmpCommon::statementHeap())
      : tableName_(tableName, h), refName_(refName, h), tableCorr_(tableName_, h), columnList_(NULL) {}

  TableRefName(const TableRefName &other, CollHeap *h = CmpCommon::statementHeap())
      : tableName_(other.tableName_, h),
        refName_(other.refName_, h),
        tableCorr_(other.tableCorr_, h),
        columnList_(other.columnList_) {}

  // operators
  NABoolean operator==(const TableRefName &other) const {
    return ((getTableName() == other.getTableName()) && (getRefName() == other.getRefName()));
  }

  NABoolean operator!=(const TableRefName &other) const { return !(*this == other); }
...
class UsingMvInfo : public NABasicObject {
 public:
  UsingMvInfo(NAString mvAnsiName, ComMVRefreshType refreshType, NABoolean isRewriteEnabled, NABoolean isInitialized,
              CollHeap *heap)
...
inline NABoolean QualifiedName::operator==(const QualifiedName &other) const {
  return getObjectName() == other.getObjectName() && getSchemaName() == other.getSchemaName() &&
...
inline NABoolean QualifiedName::operator!=(const QualifiedName &other) const { return NOT operator==(other); }
...
inline NABoolean CorrName::operator!=(const CorrName &other) const { return NOT operator==(other); }

// Comparison operators just for coding simplicity in callers
// ( Remove the cutesy == and != operators overloaded for NAString& ? )
inline NABoolean CorrName::isEmpty() const {
...
inline NABoolean CorrName::operator==(const NAString &other) const {
  CMPASSERT(other == (const char *)"");
  return isEmpty();
}

inline NABoolean CorrName::operator!=(const NAString &other) const { return NOT operator==(other); }
...
inline NABoolean PartitionClause::operator==(const PartitionClause &other) const {
  return (getPartitionName() == other.getPartitionName() &&
          getBeginPartitionNumber() == other.getBeginPartitionNumber() &&
          getEndPartitionNumber() == other.getEndPartitionNumber() && getLocationName() == other.getLocationName());
}
...
inline NABoolean PartitionClause::isEmpty() const {
...
#ifdef OPT_ANALYSIS
    ++count_;
    if (count_ > waterMark_) waterMark_ = count_;
    ++totalAllocations_;
#endif /* OPT_ANALYSIS */
  };
