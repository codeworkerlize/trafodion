...
enum SUBSET_LEGALITY { NOT_KNOWN, LEGAL, ILLEGAL };
...
class CANodeId {
 ...
 public:
  ...
  inline operator CollIndex() const { return id_; }

  // Comparison operators
  inline NABoolean operator==(const CANodeId &other) { return id_ == other.id_; }

  inline NABoolean operator!=(const CANodeId &other) { return id_ != other.id_; }

  NABoolean isValid() { return (id_ != NULL_CA_ID_VALUE); };
  ...
  const NAString getText() const { return NAString("CANodeId: ") + istring(id_); }
...
const CANodeId NULL_CA_ID((CollIndex)NULL_CA_ID_VALUE);
...
class CANodeIdSet : public SUBARRAY(NodeAnalysis *) {
 public:
  ...
  // Copy constructor
  CANodeIdSet(const CANodeIdSet &other, CollHeap *outHeap = CmpCommon::statementHeap());

  CANodeIdSet(const CANodeId &id, CollHeap *outHeap = CmpCommon::statementHeap());

  // Destructor
  virtual ~CANodeIdSet() {}

  // Comparison operators
  inline NABoolean operator==(const CANodeIdSet &other) const { return SUBARRAY(NodeAnalysis *)::operator==(other); }

  CANodeIdSet operator+(const CANodeIdSet &) const;

  inline NABoolean operator!=(const CANodeIdSet &other) const { return NOT operator==(other); }
  ...
  // ---------------------------------------------------------------------
  CANodeId init() const { return CANodeId((CollIndex)0); }

  NABoolean next(CANodeId &x) const { return nextUsed(x.id_); }

  void advance(CANodeId &x) const { x.id_++; }

  NABoolean containsThisId(CANodeId &x) const {
    CANodeId id;
    for (id = init(); next(id); advance(id)) {
      if (id.operator==(x)) return TRUE;
    }
    return FALSE;
  }
...
class CANodeIdSetMap : public HASHDICTIONARY(CANodeIdSet, JBBSubsetAnalysis) {
 public:
  ...
  inline JBBSubsetAnalysis *get(const CANodeIdSet &key) const { return getFirstValue(&key); }
...
class QueryAnalysis : public NABasicObject {
 public:
  ...
  QueryAnalysis(CollHeap *outHeap = CmpCommon::statementHeap(), NABoolean analysis = TRUE);
  ...
  inline static QueryAnalysis *Instance() {
    ...
    return (CmpCommon::statement()) ? CmpCommon::statement()->getQueryAnalysis() : NULL;
  }

  // List all JBBs in the query
  inline const ARRAY(JBB *) & getJBBs() { return jbbArray_; }

  // get all CANodes in the query tree that are JBBCs
  inline const CANodeIdSet &getJBBCs() { return allJBBCs_; }
  ...
  inline const CANodeIdSet &getGBs() { return allGBs_; }

  inline const CANodeIdSet &getLeftJoinJBBCs() { return leftJoinJBBCs_; }

  inline const CANodeIdSet &getSemiJoinJBBCs() { return semiJoinJBBCs_; }

  inline const CANodeIdSet &getRoutineJoinJBBCs() { return routineJoinJBBCs_; }

  inline const CANodeIdSet &getInnerNonSemiNonTSJJBBCs() { return innerNonSemiNonTSJJBBCs_; }

  // direct access to the ASM of the singleton
  inline static AppliedStatMan *ASM() { return Instance()->getASM(); }

  // get the ASM of this object
  inline AppliedStatMan *getASM() const { return appStatMan_; }

  // direct access to the TableConnectivityGraph of the singleton
  inline static TableConnectivityGraph *TCG() { return Instance()->getTableConnectivityGraph(); }

  // get the TableConnectivityGraph of this object
  inline TableConnectivityGraph *getTableConnectivityGraph() { return tCGraph_; }

  // get the query complexity vector
  inline QueryComplexityVector *getQueryComplexityVector() { return queryComplexityVector_; }
  ...
  inline NodeAnalysis *getNodeAnalysis(CollIndex id) {
    if (nodeAnalysisArray_.used(id)) return nodeAnalysisArray_[id];
    return NULL;
  }

  // Fast lookup (array lookup) for ColAnalysis using ValueId
  inline ColAnalysis *getColAnalysis(CollIndex id) {
    if (colAnalysisArray_.used(id)) return colAnalysisArray_[id];
    return NULL;
  }

  // Fast lookup (array lookup) for ColAnalysis using ValueId
  inline CANodeIdSet *getProducingJBBCs(CollIndex id) {
    if (outputToJBBCsMap_.used(id)) return outputToJBBCsMap_[id];
    return NULL;
  }
  ...
  inline PredAnalysis *getPredAnalysis(CollIndex id) {
    if (predAnalysisArray_.used(id))
      return predAnalysisArray_[id];
    else
      return findPredAnalysis(id);
  }
  ...
  inline JBB *getJBB(CollIndex id) { return jbbArray_[id]; }
  ...
  inline NABoolean isOriginalJBBTopExpr(RelExpr *expr) { return origJBBTopExprs_.contains(expr); }
  ...
  // Is Analysis ON?
  NABoolean isAnalysisON() { return analysisON_; }

  // Find a nodemap that was created for this degree of paralleism
  inline NodeMap *getNodeMap(CollIndex degreeOfParallelism) {
    if (nodeMapArray_.used(degreeOfParallelism)) return nodeMapArray_[degreeOfParallelism];
    return NULL;
  }

  // Insert the current node map in the global node map array
  inline void setNodeMap(NodeMap *map, CollIndex degree) { nodeMapArray_.insertAt(degree, map); }

  // Did MultiJoin Rewrite take place
  NABoolean multiJoinsUsed() { return multiJoinsUsed_; }

  NABoolean isCompressedHistsViable() { return compressedHistsViable_; }

  void disableCompressedHistsViable() { compressedHistsViable_ = FALSE; }

  // Should we optimize for FirstN optimization goal
  NABoolean optimizeForFirstNRows() { return optimizeForFirstNRows_; }

  // Use the join order as specified in SQL query FROM clause
  NABoolean joinOrderByUser() { return joinOrderByUser_; }

  void setJoinOrderByUser() { joinOrderByUser_ = TRUE; }

  // WaveFix Begin
  NABoolean dontSurfTheWave() { return dontSurfTheWave_; }

  void setDontSurfTheWave(NABoolean value) { dontSurfTheWave_ = value; }
  // WaveFix End
  ...
  inline TaskMonitor &pilotPhaseMonitor() { return pilotPhaseMon_; }
  inline TaskMonitor &jbbSetupMonitor() { return jbbSetupMon_; }
  inline TaskMonitor &tcgSetupMonitor() { return tcgSetupMon_; }
  inline TaskMonitor &synthLogPropMonitor() { return synthLogPropMon_; }
  inline TaskMonitor &asmPrecompMonitor() { return asmPrecompMon_; }
  inline TaskMonitor &queryGraphMonitor() { return queryGraphMon_; }
  inline TaskMonitor &compilerMonitor() { return compilerMon_; }
  inline TaskMonitor &parserMonitor() { return parserMon_; }
  inline TaskMonitor &compCleanupMonitor() { return compCleanupMon_; }

  inline TaskMonitor &tempMonitor() { return tempMon_; }
  ...
  inline CompilerPhaseEnum getCompilerPhase() { return compilerPhase_; }

  inline void setCompilerPhase(CompilerPhaseEnum phase) { compilerPhase_ = phase; }

  inline TableAnalysis *getLargestTable() { return largestTable_; }

  inline void setLargestTable(TableAnalysis *largestTable) { largestTable_ = largestTable; }

  inline const CANodeIdSet &getTablesJoinedToLargestTable() { return joinedToLargestTable_; }
  ...
  inline NABoolean isMvCreation() { return isMvCreation_; }

  inline void setSkippedSomeJoins() { skippedSomeJoins_ = TRUE; }
...
class QueryComplexityVector : public NABasicObject {
 public:
  QueryComplexityVector(CollHeap *outHeap = CmpCommon::statementHeap())
      : n_(-1), n2_(-1), n3_(-1), n4_(-1), exhaustive_(-1), heap_(outHeap) {}
...
class GroupAnalysis : public NABasicObject {
 ...
 public:
  ...
  GroupAnalysis(GroupAttributes *groupAttr, CollHeap *outHeap = CmpCommon::statementHeap())
      : groupAttr_(groupAttr), localJBBView_(NULL), caNode_(NULL), heap_(outHeap) {}

  // copy constructor.
  GroupAnalysis(const GroupAnalysis &other, CollHeap *outHeap = CmpCommon::statementHeap());
  ...
  inline NodeAnalysis *getCANodeAnalysis() const { return caNode_; }
  inline NodeAnalysis *getNodeAnalysis() const { return caNode_; }
  inline void setNodeAnalysis(NodeAnalysis *node) { caNode_ = node; }

  // The local JBB view of the JBBSubset
  inline const JBBSubset *getLocalJBBView() { return localJBBView_; }
  inline void setLocalJBBView(JBBSubset *jbbSubset) { localJBBView_ = jbbSubset; }
  ...
  inline const CANodeIdSet &getAllSubtreeTables() const { return allSubtreeTables_; }
  inline void setSubtreeTables(const CANodeIdSet &tables) { allSubtreeTables_ = tables; }
...
class NodeAnalysis : public NABasicObject {
 ...
 public:
  ...
  inline GBAnalysis *getGBAnalysis() { return groupBy_; }

  // get the RoutineAnalysis for this node. This returns NULL if this
  // node is not a routine
  inline RoutineAnalysis *getRoutineAnalysis() { return routine_; }

  // This returns a copy of the original rel expression that the NodeAnalysis
  // was computed for.
  RelExpr *getOriginalExpr() { return originalExpr_; }

  // This returns a copy of the original modified expression after the
  // tree was modified.
  RelExpr *getModifiedExpr() { return modifiedExpr_; }

  void setModifiedExpr(RelExpr *expr) { modifiedExpr_ = expr; }
  ...
  inline RowSize getRecordSize() const { return recordSize_; }
  ...
  EstLogPropSharedPtr getJBBInput() const { return jbbInputLP_; };

  void setJBBInput(EstLogPropSharedPtr &inLP) { jbbInputLP_ = inLP; };
 ...
 private:
  ...
  NodeAnalysis(CollIndex id, RelExpr *expr, CollHeap *outHeap = CmpCommon::statementHeap()) {
  ...
  inline void setJBBC(JBBC *jbbc) { jbbc_ = jbbc; }

  inline void setTableAnalysis(TableAnalysis *tab) { table_ = tab; }

  inline void setGBAnalysis(GBAnalysis *gb) { groupBy_ = gb; }

  inline void setRoutineAnalysis(RoutineAnalysis *routine) { routine_ = routine; }

  inline void setExtraHub(NABoolean eh) { isExtraHub_ = eh; }
...
class TableAnalysis : public NABasicObject {
 ...
 public:
  enum coverageCriteria { INDEX, PART_KEY };
  ...
  inline NodeAnalysis *getNodeAnalysis() const { return caNodeAnalysis_; }

  // Return the TableDesc
  inline TableDesc *getTableDesc() const { return tableDesc_; }

  // List all access paths
  inline const LIST(AccessPathAnalysis *) & getAccessPaths() const { return accessPaths_; }

  // List index-only access paths
  inline const LIST(AccessPathAnalysis *) & getIndexOnlyAccessPaths() const { return indexOnlyAccessPaths_; }

  // Return AccessPathAnalysis for base table
  inline AccessPathAnalysis *getApaForBaseTable() { return apaForBaseTable_; }

  // Get the set of used Cols
  inline const ValueIdSet &getUsedCols() const { return usedCols_; }

  // Get all cols that equivalent to constant value (via a veg pred)
  inline const ValueIdSet &getConstCols() const { return constCols_; }

  // Get local predicates on this table
  inline const ValueIdSet &getLocalPreds() const { return localPreds_; }

  // check if other appears in the local pred
  NABoolean referencedInLocalPred(ItemExpr *other);

  // Get all columns directly connected to a column in this table
  // (via a VEG predicates)
  inline const ValueIdSet &getConnectedCols() const { return connectedCols_; }

  // Get all tables that have columns directly connected
  // (via a VEG predicates) to a column in this table.
  inline const CANodeIdSet &getConnectedTables() const { return connectedTables_; }
  ...
  inline const ValueIdSet &getReferencingPreds() const { return referencingPreds_; }

  inline const ValueIdSet &getVegPreds() const { return vegPreds_; }
  ...
  // t1.a = t1.b
  inline void addEqualityRelation(ValueId colId, ValueId pred, ValueId equalityExpr, CANodeIdSet connectedJBBCs) {
  ...
  inline const CANodeIdSet *getEqualityConnectedJBBCs(ValueId pred) {
    ...
    if (index != NULL_COLL_INDEX) {
      ...
      CANodeIdSet *connectedJBBCs = QueryAnalysis::Instance()->getProducingJBBCs(equalityExpression);
  ...
  inline const ValueIdSet &getEqualityConnectedCols() const { return equalityConnectedCols_; }

  inline const ValueIdSet &getEqualityConnectingPreds() const { return equalityConnectingPreds_; }

  inline const CANodeIdSet &getEqualityConnectedJBBCs() const { return equalityConnectedJBBCs_; }
  ...
  getCoveringAccessPaths(NABoolean indexOnly, coverageCriteria cc, const ValueIdSet &vidSet, NABoolean exactMatch);
  ...
  LIST(AccessPathAnalysis *)
  accessPathsCoveringIndex(const ValueIdSet &vidSet, NABoolean indexOnly, NABoolean exactMatch);
  ...
  LIST(AccessPathAnalysis *)
  accessPathsCoveringPartKey(const ValueIdSet &vidSet, NABoolean indexOnly, NABoolean exactMatch);

  /////////////////////////////////////////////////

  // Get all JBBCs that are connected to this set of columns and the join preds
  // This table must be JBBC
  CANodeIdSet getJBBCsConnectedToCols(const CANodeIdSet &jbbcs, const ValueIdSet &cols, ValueIdSet &joinPreds /*OUT*/,
                                      ValueIdSet &localPreds /*OUT*/, ValueIdSet *predCols = NULL /*OUT*/);
  ...
  // of the table
  ValueIdSet getLocalPredsOnColumns(const ValueIdSet &cols, ValueIdSet *colsWithLocalPreds = NULL /*OUT*/);
  ...
  inline EstLogPropSharedPtr getStatsAfterLocalPredsOnCKPrefix() const { return statsAfterLocalPredsOnCKPrefix_; }
  ...
  inline RowSize getRecordSizeOfBaseTable() const { return recordSizeOfBaseTable_; }
  ...
  void setFactTableNJAccessCost(CostScalar factNJAccessCost) { factTableNJAccessCost_ = factNJAccessCost; }

  CostScalar getFactTableNJAccessCost() const { return factTableNJAccessCost_; }

  CostScalar computeDataAccessCostForTable(CostScalar probes, CostScalar tableRowsToScan) const;
  // finalized analysis
  void finishAnalysis();
 ...
 private:
  ...
  TableAnalysis(NodeAnalysis *nodeAnalysis, TableDesc *tableDesc, CollHeap *outHeap = CmpCommon::statementHeap())
  ...
  TableAnalysis() : accessPaths_(STMTHEAP), indexOnlyAccessPaths_(STMTHEAP), uniqueIndexes_(STMTHEAP) {
    // should never be called
    // use a constructor that passes NodeAnalysis* and TableDesc*
  }

  // set the usedCols_. to be used by QueryAnalysis
  void setUsedCols(const ValueIdSet &usedCols) { usedCols_ = usedCols; }

  // set the localPreds_. to be used by QueryAnalysis
  void setLocalPreds(const ValueIdSet &localPreds) { localPreds_ = localPreds; }

  // Set AccessPathAnalysis for base table.
  inline void setApaForBaseTable(AccessPathAnalysis *apa) { apaForBaseTable_ = apa; }
...
class AccessPathAnalysis : public NABasicObject {
 ...
 public:
  ...
  inline IndexDesc *getIndexDesc() const { return indexDesc_; }

  // Return TableAnalysis associated with this AccessPathAnalysis
  inline TableAnalysis *getTableAnalysis() const { return tableAnalysis_; }
  ...
  NABoolean keyCoveredByEqualityPreds(const ValueIdSet &vidSet, NABoolean *hasPrefix = NULL);
 ...
 private:
  ...
  AccessPathAnalysis(IndexDesc *indexDesc, TableAnalysis *tableAnalysis,
...
class GBAnalysis : public NABasicObject {
 ...
 public:
  ...
  inline GroupByAgg *getOriginalGBExpr() { return originalGBExpr_; }
  ...
  const CANodeIdSet &getPullableJBBCs() const { return pullableJBBCs_; }

  // R1: The other sibling jbbcs that are not part of pullableJBBCs
  const CANodeIdSet &getRequiredJBBCs() const { return requiredJBBCs_; }
  ...
  inline CANodeId getGroupingNodeId() const { return groupingNodeId_; }

  // get and JBB
  inline JBB *getJBB() { return jbb_; }

  const NAString getText() const { return "GBAnalysis"; }

 private:
  // construct a GBAnalysis. Called only by QueryAnalysis
  GBAnalysis(CANodeId id, GroupByAgg *gbExpr, CollHeap *outHeap = CmpCommon::statementHeap())
      : groupingNodeId_(id), originalGBExpr_(gbExpr), jbb_(NULL), heap_(outHeap) {}
  ...
  inline void setJBB(JBB *jbb) { jbb_ = jbb; }
...
class RoutineAnalysis : public NABasicObject {
 public:
  ...
  RoutineAnalysis(NodeAnalysis *nodeAnalysis, RoutineDesc *routineDesc, CollHeap *outHeap = CmpCommon::statementHeap())
      : caNodeAnalysis_(nodeAnalysis), routineDesc_(routineDesc) {}

  // Get NodeAnalysis for this routine
  inline NodeAnalysis *getNodeAnalysis() const { return caNodeAnalysis_; }

  // Return the TableDesc
  inline RoutineDesc *getRoutineDesc() const { return routineDesc_; }
...
class JBBC : public JBBItem {
 ...
 public:
  ...
  // Refer to "Connectivity Analyzer PRD" document.
  virtual const CANodeIdSet &getJoinedJBBCs() const { return joinedJBBCs_; }

  // Get all the join predicates associated with getJoinedJBBCs
  virtual const ValueIdSet &getJoinPreds() const { return joinPreds_; }
  ...
  void setLeftJoinFilterPreds(ValueIdSet &leftJoinFilterPreds) { leftJoinFilterPreds_ = leftJoinFilterPreds; }

  ValueIdSet getLeftJoinFilterPreds() const { return leftJoinFilterPreds_; }

  void setPredsWithDependencies(const ValueIdSet &predsWithDependencies, const ValueIdSet &predsWithPredecessors);

  // Get all JBBCs that are joined to this JBBC and that are
  // dependent on this JBBC. For definition of join dependency
  // Refer to "Connectivity Analyzer PRD" document.
  virtual const CANodeIdSet &getSuccessorJBBCs() const { return successorJBBCs_; }

  // Get all the join predicates associated with getDependentJBBCs
  virtual const ValueIdSet &getPredsWithSuccessors() const { return predsWithSuccessors_; }

  // Get all JBBCs that are joined to this JBBC and that this
  // JBBC is dependent on. For definition of join dependency
  // Refer to "Connectivity Analyzer PRD" document.
  virtual const CANodeIdSet &getPredecessorJBBCs() const { return predecessorJBBCs_; }

  // Get all the join predicates associated with getPredecessorJBBCs
  virtual const ValueIdSet &getPredsWithPredecessors() const { return predsWithPredecessors_; }

  LIST(CANodeIdSet) *const getPredecessorJBBCsList() const { return predecessorJBBCsList_; }
  ...
  virtual const ValueIdSet &getConstPredsWithPredecessors() const { return constPredsWithPredecessors_; }

  void setRoutineJoinFilterPreds(const ValueIdSet &routineJoinFilterPreds) {
    routineJoinFilterPreds_ = routineJoinFilterPreds;
  }

  ValueIdSet getRoutineJoinFilterPreds() const { return routineJoinFilterPreds_; }

  // inputs need from sibling JBBCs i.e. jbbcs part of the same JBB as this JBBC
  const ValueIdSet &getInputsRequiredFromSiblings() const { return inputsRequiredFromSiblings_; }
  ...
  virtual const CANodeIdSet &getJBBCsRequiringInputFromMe() const { return jbbcsRequiringInputFromMe_; }

  // Get all JBBCs that this jbbc needs input from
  virtual const CANodeIdSet &getJBBCsProvidingInput() const { return jbbcsProvidingInput_; }
  ...
  CANodeIdSet getJoinedJBBCsOnThisColumn(const ValueId &col, ValueIdSet &joiningPreds /*OUT*/) const;
  /* Do not inspect this method */
  // Returns a subset of getJoinedJBBCs that are joined with this JBBC
  // on these particular columns
  CANodeIdSet getJoinedJBBCsOnTheseColumns(const ValueIdSet &cols, ValueIdSet &joiningPreds /*OUT*/) const;

  const CANodeIdSet &getSubtreeTables() const {
    return getNodeAnalysis()->getOriginalExpr()->getGroupAnalysis()->getAllSubtreeTables();
  }
  ...
  // with no preds.
  inline Join *getOriginalParentJoin() const { return origParentJoin_; }

  // Get NodeAnalysis for this JBBC
  inline NodeAnalysis *getNodeAnalysis() const { return caNodeAnalysis_; }

  // Get the CANodeId for this JBBC
  inline CANodeId getId() const { return getNodeAnalysis()->getId(); }

  // Get the parent JBB of this JBBC
  inline JBB *getJBB() const { return jbb_; }

  const ValueIdList &nullInstantiatedOutput() const { return nullInstantiatedOutput_; }

  inline NABoolean isFullOuterJoinOrTSJJBBC() const { return isFullOuterJoinOrTSJJBBC_; }
 ...
 private:
  ...
  JBBC(NodeAnalysis *nodeAnalysis, Join *parentJoin, CollHeap *outHeap = CmpCommon::statementHeap())
      : caNodeAnalysis_(nodeAnalysis),
        ...
        heap_(outHeap) {
    if (parentJoin) {
      if (parentJoin->isLeftJoin()) nullInstantiatedOutput_ = parentJoin->nullInstantiatedOutput();
  ...
  inline void setIsFullOuterJoinOrTSJJBBC() { isFullOuterJoinOrTSJJBBC_ = TRUE; }
...
// disadvantage for that is sharing the setter methods
{
 public:
  ...
  JBBSubsetAnalysis(const JBBSubset &subset, CollHeap *outHeap = CmpCommon::statementHeap());

  // destruct a JBBSubsetAnalysis
  virtual ~JBBSubsetAnalysis() {}

  // equality operator
  inline NABoolean operator==(const JBBSubsetAnalysis &other) const { return (allMembers_ == other.allMembers_); }
  ...
  virtual const ValueIdSet &getLocalJoinPreds() const { return localJoinPreds_; }

  // Get the inner join preds between my jbbcs
  virtual const ValueIdSet &getLocalInnerNonSemiJoinPreds() const { return localInnerNonSemiJoinPreds_; }

  // Get the dependent join preds between my jbbcs i.e. all
  // semi, anti-semi and outer-join preds
  virtual const ValueIdSet &getLocalDependentJoinPreds() const { return localDependentJoinPreds_; }
  ...
  inline MultiJoin *getSubsetMJ() const { return subsetMJ_; }

  // Get the Grouping Id for the group by in this JBBSubset
  inline CANodeId getGB() const { return gb_; }

  // union of both getJBBCs() and getGB()
  inline CANodeIdSet getJBBCsAndGB() const { return allMembers_; }

  inline NABoolean allJoinsInnerNonSemi() { return allJoinsInnerNonSemi_; }

  NABoolean coversExpr(ValueId vid) const;

  NAList<CANodeIdSet *> *getConnectedSubgraphs(NABoolean followSuccessors = TRUE, NABoolean pullInPredecessors = FALSE);
  ...
  void consolidateFringes(CANodeId factTable, NAList<CANodeIdSet> &leftDeepJoinSequence);
  ...
  void setMJStarJoinRuleWA(MJStarJoinRuleWA *mjStarJoinRuleWA) { mjStarJoinRuleWA_ = mjStarJoinRuleWA; }

  inline MJStarJoinRuleWA *getMJStarJoinRuleWA() { return mjStarJoinRuleWA_; }

  void setMJStarJoinIRuleWA(MJStarJoinIRuleWA *mjStarJoinIRuleWA) { mjStarJoinIRuleWA_ = mjStarJoinIRuleWA; }

  inline MJStarJoinIRuleWA *getMJStarJoinIRuleWA() { return mjStarJoinIRuleWA_; }

  void setMJStarBDRuleWA(MJStarBDRuleWA *mjStarBDRuleWA) { mjStarBDRuleWA_ = mjStarBDRuleWA; }

  inline MJStarBDRuleWA *getMJStarBDRuleWA() { return mjStarBDRuleWA_; }

  const NAString getText() const { return "JBBSubsetAnalysis"; }

  // find the fact table and the largest table for this JBBSubset
  // if a fact table is not found NULL_CA_ID is returned
  CANodeId findFactAndLargestTable(CostScalar &factTableCKPrefixCardinality, CANodeId &biggestTable);

  // find the fact table and the largest table for this childSet
  // if a fact table is not found NULL_CA_ID is returned
  CANodeId findFactTable(CANodeIdSet childSet, CostScalar &factTableCKPrefixCardinality, CANodeId &biggestTable);

  CANodeId computeCenterTable();

  CostScalar getCenterTableSize() { return centerTableDataScanned_; }

  CostScalar getCenterTableSizePerPartition() { return centerTableDataPerPartition_; }

  void setStarJoinTypeIFeasible() { starJoinTypeIFeasible_ = TRUE; }

  NABoolean starJoinTypeIFeasible() { return starJoinTypeIFeasible_; }

  void setFactTable(CANodeId factTable) { factTable_ = factTable; }
  ...
  void computeRequiredResources(MultiJoin *mjoin, RequiredResources &reqResouces, EstLogPropSharedPtr &inLP);

  const NAList<CANodeIdSet> *const getInitialPlanLeftDeepJoinSequence() { return &leftDeepJoinSequence_; }

  CostScalar getCenterTableRowsScanned() { return centerTableRowsScanned_; }
  CostScalar getCenterTableDataScanned() { return centerTableDataScanned_; }
  CostScalar getCenterTableDataPerPartition() { return centerTableDataPerPartition_; }
  CostScalar getCenterTablePartitions() { return centerTablePartitions_; }
  UInt32 getCenterTableConnectivity() { return centerTableConnectivity_; }
  UInt32 getMaxDimensionConnectivity() { return maxDimensionConnectivity_; }

  NAPtrList<MVMatchPtr> getMatchingMVs() { return matchingMVs_; };
 ...
 private:
  JBBSubsetAnalysis(CollHeap *outHeap = CmpCommon::statementHeap())
      : leftDeepJoinSequence_(outHeap), matchingMVs_(outHeap) {
    // should never be called
  }
  ...
  CostScalar computeDataAccessCostForTable(CostScalar probes, CostScalar tableRowsToScan, CANodeId table);
...
class JBBSubset : public JBBItem {
 public:
  ...
  // Comparison operators
  inline NABoolean operator==(const JBBSubset &other) const { return ((jbbcs_ == other.jbbcs_) && (gb_ == other.gb_)); }

  inline NABoolean operator!=(const JBBSubset &other) const { return NOT operator==(other); }

  // Get all JBBCs that are joined to this JBBSubset and do not have
  // dependency relation with this JBBSubset. For definition of join dependency
  // Refer to "Connectivity Analyzer PRD" document.
  virtual const CANodeIdSet &getJoinedJBBCs() const { return getJBBSubsetAnalysis()->getJoinedJBBCs(); }

  // Get all the join predicates associated with getJoinedJBBCs
  virtual const ValueIdSet &getJoinPreds() const { return getJBBSubsetAnalysis()->getJoinPreds(); }

  // Get all JBBCs that are joined to this JBBSubset and that are
  // dependent on this JBBSubset. For definition of join dependency
  // Refer to "Connectivity Analyzer PRD" document.
  virtual const CANodeIdSet &getSuccessorJBBCs() const { return getJBBSubsetAnalysis()->getSuccessorJBBCs(); }

  // Get all the join predicates associated with getSuccessorJBBCs
  virtual const ValueIdSet &getPredsWithSuccessors() const { return getJBBSubsetAnalysis()->getPredsWithSuccessors(); }

  // Get all JBBCs that are joined to this JBBSubset and that this
  // JBBSubset is dependent on. For definition of join dependency
  // Refer to "Connectivity Analyzer PRD" document.
  virtual const CANodeIdSet &getPredecessorJBBCs() const { return getJBBSubsetAnalysis()->getPredecessorJBBCs(); }
  ...
  // Get the join preds between my jbbcs
  virtual const ValueIdSet &getLocalJoinPreds() const { return getJBBSubsetAnalysis()->getLocalJoinPreds(); }
  ...
  // which JBBCs can I add to this subset while reserving its self-dpendency
  inline CANodeIdSet legalJBBCAdditions() const { return getJBBSubsetAnalysis()->legalJBBCAdditions(); }
  ...
  // Get the set of JBBCs in this JBBSubset
  virtual const CANodeIdSet &getJBBCs() const { return jbbcs_; }

  // set JBBSubset MultiJoin
  inline void setSubsetMJ(MultiJoin *subsetMJ) { getJBBSubsetAnalysis()->setSubsetMJ(subsetMJ); };

  // get MultiJoin representing JBBSubset
  MultiJoin *getSubsetMJ() const { return getJBBSubsetAnalysis()->getSubsetMJ(); };
  ...
  NABoolean coversExpr(ValueId vid) const { return getJBBSubsetAnalysis()->coversExpr(vid); }
  ...
  inline JBBSubsetAnalysis *getJBBSubsetAnalysis() const {
    if (!jbbSubsetAnalysis_) findJBBSubsetAnalysis();
    return jbbSubsetAnalysis_;
  }

  inline NABoolean allJoinsInnerNonSemi() const { return getJBBSubsetAnalysis()->allJoinsInnerNonSemi(); }
  ...
  // JBBSubset.
  inline void clearAnalysis() { jbbSubsetAnalysis_ = NULL; }
 ...
 private:
  void findJBBSubsetAnalysis() const {
    mutate()->jbbSubsetAnalysis_ = QueryAnalysis::Instance()->findJBBSubsetAnalysis(*this);
    return;
  }

  // mutate method. use carefully and keep private (hush hush)
  // used only for methods that may change computable members
  inline JBBSubset *mutate() const { return (JBBSubset *)this; }
...
class JBBWA : public NABasicObject {
 ...
 private:
  ...
  CostScalar getCardinalityAfterLocalKeyPrefixPreds(CANodeId node, EstLogPropSharedPtr &logProps);
  ...
  CostScalar getDataSizeAfterLocalKeyPrefixPreds(CANodeId node, EstLogPropSharedPtr &logProps);

  // Given a node, return it's cardinality after application of local preds.
  // If logProps is passed in is NULL, then ASM will be called to compute
  // the logical properties
  CostScalar getCardinalityAfterLocalPreds(CANodeId node, EstLogPropSharedPtr &logProps);

  // Given a node, return it's data-size (i.e. cardinality * rowsize)
  // after application of local preds. If logProps is passed in is NULL,
  // then ASM will be called to compute the logical properties
  CostScalar getDataSizeAfterLocalPreds(CANodeId node, EstLogPropSharedPtr &logProps);

  // Given a node, return it's base cardinality
  CostScalar getBaseCardinality(CANodeId node, EstLogPropSharedPtr &logProps);

  // Given a node, return it's size (i.e. rowcount * rowsize)
  CostScalar getBaseDataSize(CANodeId node, EstLogPropSharedPtr &logProps);

  // Given a node, return it's cardinality
  CostScalar getNodeCardinality(CANodeId node, EstLogPropSharedPtr &logProps);

  // Given a node, return it's size (i.e. rowcount * rowsize)
  CostScalar getNodeDataSize(CANodeId node, EstLogPropSharedPtr &logProps);

  // Given a node, return it's output data size (i.e. rowcount * outputrowsize)
  CostScalar getNodeOutputDataSize(CANodeId node, EstLogPropSharedPtr &logProps);
...
class JBB : public NABasicObject {
 ...
 public:
  ...
  const JBBSubset &getMainJBBSubset() const { return mainJBBSubset_; }

  // return the set of all jbbcs in this jbb
  inline const CANodeIdSet getJBBCs() const { return getMainJBBSubset().getJBBCs(); }

  // The GBAnalysis for the group by on the top of this JBB
  GBAnalysis *getGBAnalysis() const { return gbAnalysis_; }

  // Workspace for the JBB
  JBBWA *getJBBWA() const { return workArea_; }

  // Global Id for the JBB in the QueryAnalysis
  CollIndex getJBBId() const { return jbbId_; }
  ...
  inline const ValueIdSet &getNormInputs() const { return normInputs_; }

  // Get the char. outputs during Query Analysis time. These were
  // the outputs from the normalizer tree for the top expression
  // in the join backbone.
  inline const ValueIdSet &getNormOutputs() const { return normOutputs_; }
  ...
  EstLogPropSharedPtr getInputEstLP() { return inEstLP_; }
 ...
 private:
  ...
  JBB(CollIndex id, CollHeap *outHeap = CmpCommon::statementHeap())
      : jbbId_(id), heap_(outHeap), inEstLP_((*GLOBAL_EMPTY_INPUT_LOGPROP)) {
...
class PredAnalysis : public NABasicObject {
 public:
  ...
  PredAnalysis(ValueId predicate) : predicate_(predicate) {}

  // Destructor
  virtual ~PredAnalysis() {}

  // Get the predicate ValueId
  inline ValueId getPredicate() { return predicate_; }

  // Get all columns directly connected by this predicate
  inline ValueIdSet &getReferencedCols() { return referencedCols_; }

  // Add this column to the list of Referenced col by me
  inline void addToReferencedCols(ValueId col) { referencedCols_ += col; }
  inline void addToReferencedCols(const ValueIdSet &cols) { referencedCols_ += cols; }

  // Get all columns directly connected by this VEG predicate
  inline ValueIdSet &getVegConnectedCols() { return vegConnectedCols_; }

  // Add this column to the list of vegConnected col by me
  inline void addToVegConnectedCols(ValueId col) { vegConnectedCols_ += col; }
  inline void addToVegConnectedCols(const ValueIdSet &cols) { vegConnectedCols_ += cols; }

  // Get all columns directly connected by this Equality Predicate
  inline ValueIdSet &getEqualityConnectedCols() { return equalityConnectedCols_; }

  // Add this column to the list of vegConnected col by me
  inline void addToEqualityConnectedCols(ValueId col) { equalityConnectedCols_ += col; }
  inline void addToEqualityConnectedCols(const ValueIdSet &cols) { equalityConnectedCols_ += cols; }

  // Get all tables directly connected by this predicate
  inline CANodeIdSet &getConnectedTables() { return connectedTables_; }

  // Get all JBBCs referenced by this predicate
  inline CANodeIdSet &getReferencedJBBCs() { return referencedJBBCs_; }

  // Add this JBBC to my set of referenced JBBCs
  inline void addToReferencedJBBCs(const CANodeId jbbc) { referencedJBBCs_ += jbbc; }

  const NAString getText() const { return "PredAnalysis"; }
...
class ValueAnalysis : public NABasicObject {
 public:
  ...
  ValueAnalysis(ValueId value, CollHeap *outHeap = CmpCommon::statementHeap());

  // Destructor
  virtual ~ValueAnalysis() {}

  // Get the ValueId
  inline ValueId valueId() { return valueId_; }

  // Get all columns directly or indirectly referenced by this value
  inline const ValueIdSet &getReferencedCols() { return referencedCols_; }

  // Get all tables whose column(s) was directly or indirectly
  // referenced by this value
  inline const CANodeIdSet &getReferencedTables() { return referencedTables_; }

  inline void addToReferencedCols(ValueId c) { referencedCols_ += c; }

  const NAString getText() const { return "ValueAnalysis"; }
...
class ColAnalysis : public NABasicObject {
 ...
 public:
  ...
  inline ValueId getColumnId() { return column_; }

  // Get all columns directly connected to this column
  // (via VEG predicates)
  inline ValueIdSet &getConnectedCols() { return connectedCols_; }
  ...
  inline TableDesc *getTableDesc() { return table_->getTableDesc(); }

  // Add these predicates to the list of Referencing predicates for me
  inline void addToReferencingPreds(ValueId pred) { referencingPreds_ += pred; }
  inline void addToReferencingPreds(const ValueIdSet &preds) { referencingPreds_ += preds; }
  inline const ValueIdSet &getReferencingPreds() { return referencingPreds_; }

  // Add these predicates to the list of connecting veg predicates for me
  inline void addToVegPreds(ValueId pred) { vegPreds_ += pred; }
  inline void addToVegPreds(const ValueIdSet &preds) { vegPreds_ += preds; }

  inline const ValueIdSet &getVegPreds() { return vegPreds_; }
  ...
  inline void addEqualityRelation(ValueId colId, ValueId pred, ValueId equalityExpr, CANodeIdSet connectedJBBCs) {
    equalityConnectingPreds_ += pred;
    equalityConnectedJBBCs_ += connectedJBBCs;
    if (table_) table_->addEqualityRelation(colId, pred, equalityExpr, connectedJBBCs);
  }

  inline const ValueIdSet &getEqualityConnectingPreds() { return equalityConnectingPreds_; }

  inline const CANodeIdSet &getEqualityConnectedJBBCs() { return equalityConnectedJBBCs_; }

  // Add these to the list of equality predicates against constants
  // that did not become VEG i.e.
  // t1.a = extract_month(10212009);
  inline void addToEqualityCoveringPreds(ValueId preds) { equalityCoveringPreds_ += preds; }

  inline const ValueIdSet &getEqualityCoveringPreds() { return equalityCoveringPreds_; }

  // attribute maxFreq_ is used to cache the cardinality of the most frequently
  // occuring value.

  inline CostScalar getMaxFreq() { return maxFreq_; }

  inline void setMaxFreq(CostScalar val) { maxFreq_ = val; }
  ...
  inline CostScalar getFinalUec() { return finalUec_; }

  inline void setFinalUec(CostScalar val) { finalUec_ = val; }

  inline CostScalar getFinalRC() { return finalRC_; }

  inline void setFinalRC(CostScalar val) { finalRC_ = val; }
 ...
 private:
  ...
  ColAnalysis(ValueId col, TableAnalysis *table) : column_(col), table_(table) {
    maxFreq_ = csMinusOne;
    finalUec_ = csZero;
    finalRC_ = csZero;
  }
...
class TableConnectivityGraph : public NABasicObject {
 ...
 public:
  ...
  // get the set of all base columns in the query
  inline const ValueIdSet &getColumns() { return cols_; }

  // get the set of all base tables in the query
  inline const CANodeIdSet &getTables() { return tables_; }

  const NAString getText() const { return "TableConnectivityGraph"; }
...
class CqsWA : public NABasicObject {
 ...
 public:
  CqsWA();
  void setTableNodeIdPairList(TableCANodeIdPairLookupList *tc) { tableCANodeList_ = tc; }

  TableCANodeIdPairLookupList *getTableCANodeList() { return tableCANodeList_; }

  CQSRelExprCANodeIdMap *getcqsCANodeIdMap() { return cqsCANodeIdMap_; }
  ...
  RelExpr *checkAndProcessGroupByJBBC(RelExpr *, CANodeIdSet &, CANodeIdSet &, RelExpr *);
  ...
  void incrementNumberOfScanNodesinCQS() { numberOfScanNodesinCQS_++; }
...
NodeAnalysis *CANodeId::getNodeAnalysis() { return QueryAnalysis::Instance()->getNodeAnalysis(id_); }

const CANodeIdSet &QueryAnalysis::getTables() { return tCGraph_->getTables(); }

const ValueIdSet &QueryAnalysis::getColumns() { return tCGraph_->getColumns(); }
