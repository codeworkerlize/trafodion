...
class Subquery : public ItemExpr {
 public:
  Subquery(OperatorTypeEnum otype, RelExpr *tableExpr, ItemExpr *scalarExpr = NULL)
      : ItemExpr(otype, scalarExpr), avoidHalloweenR2_(NULL), tableExpr_(tableExpr) {}
  ...
  NABoolean isARowSubquery() const { return (getOperatorType() == ITM_ROW_SUBQUERY); };
  NABoolean isAnExistsSubquery() const {
    return (getOperatorType() == ITM_EXISTS || getOperatorType() == ITM_NOT_EXISTS);
  };
  ...
  void evaluateCandidateForUnnesting(NormWA &normWARef, Join *newJoin, GroupByAgg *newGrby);
  ...
  inline NABoolean avoidHalloweenR2() const { return avoidHalloweenR2_ ? TRUE : FALSE; }
  inline GenericUpdate *getAvoidHalloweenR2() const { return avoidHalloweenR2_; }
  inline void setAvoidHalloweenR2(GenericUpdate *gu) { avoidHalloweenR2_ = gu; }
...
class RowSubquery : public Subquery {
  // ITM_ROW_SUBQUERY
 public:
  RowSubquery(RelExpr *tableExpr) : Subquery(ITM_ROW_SUBQUERY, tableExpr) {}
  ...
  virtual const NAType *pushDownType(NAType &desiredType, enum NABuiltInTypeEnum defaultQualifier);
...
class QuantifiedComp : public Subquery {
 ...
 public:
  QuantifiedComp(OperatorTypeEnum otype, ItemExpr *scalarExpr, RelExpr *tableExpr, GenericUpdate *avoidHalloweenR2)
      : Subquery(otype, tableExpr, scalarExpr), createdFromINlist_(FALSE), createdFromALLpred_(FALSE) {
...
class InSubquery : public Subquery {
  // ITM_IN_SUBQUERY
 public:
  InSubquery(OperatorTypeEnum otype, ItemExpr *scalarExpr, RelExpr *tableExpr)
