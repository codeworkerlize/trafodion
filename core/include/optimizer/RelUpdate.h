...
class GenericUpdate : public RelExpr {
 public:
  enum RowsAffected {
    DEFAULT_ROWSAFFECTED,  // indeterminate/off
    COMPUTE_ROWSAFFECTED,  // definitely compute
    DO_NOT_COMPUTE_ROWSAFFECTED
  };  // ruled out/off

  enum NoLogType { NORMAL_LOGGING = FALSE, CONSISTENT_NOLOG, INCONSISTENT_NOLOG };

  GenericUpdate(const CorrName &name, TableDesc *tabId, OperatorTypeEnum otype, RelExpr *child, ItemExpr *newRecExpr,
      ...
      : RelExpr(otype, child, NULL, oHeap),
        ...
        flags_(0),
        sCacheKey4UpdateSetList_(oHeap),
        embeddedDeleteWithFirstN_(FALSE),
        checkCnstrntOfPartv2BaseTable_(NULL) {}
  ...
  ItemExpr *&recExprTree() { return newRecExprTree_; }
  ...
  void setNoLogOp(NoLogType val) { isNoLogOperation_ = val; }

  // triggers
  void setIgnoreTriggers(NABoolean ignoreTriggers = FALSE) { isIgnoreTriggers_ = ignoreTriggers; }
  ...
  void setNoFlow(NABoolean noFlow) { noFlow_ = noFlow; }

  // MTS - Multi Transaction Support
  NABoolean isMtsStatement() const { return mtsStatement_; }
  void setMtsStatement(NABoolean lastRow) { mtsStatement_ = lastRow; }

  // No rollback - Transaction Type support
  NABoolean isNoRollback() const { return noRollback_; }
  void setNoRollbackOperation(NABoolean flag = TRUE) { noRollback_ = flag; }

  // update using upsert
  NABoolean isUpdUsingUpsert() const { return (flags_ & UPD_USING_UPSERT) != 0; }
  void setUpdUsingUpsert(NABoolean flag = TRUE) { (flag ? flags_ |= UPD_USING_UPSERT : flags_ &= ~UPD_USING_UPSERT); }

  NABoolean getUpdateCKorUniqueIndexKey() const { return updateCKorUniqueIndexKey_; }
  void setUpdateCKorUniqueIndexKey(NABoolean val) { updateCKorUniqueIndexKey_ = val; }

  virtual NABoolean transformedUpdateNoDtmXn() const { return FALSE; }
  ...
  //
  ARRAY(ValueIdArray) & indexNewRecExprArrays() { return indexNewRecExprArrays_; }

  // Expressions for begin and end key for each index
  //
  ARRAY(ValueIdList) & indexBeginKeyPredArray() { return indexBeginKeyPredArray_; }
  ARRAY(ValueIdList) & indexEndKeyPredArray() { return indexEndKeyPredArray_; }

  void setNoIMneeded(NABoolean v) { noIMneeded_ = v; }
  NABoolean noIMneeded() { return noIMneeded_; }
  ...
  ItemExpr *getCheckCnstrntOfPartv2BaseTable() const { return checkCnstrntOfPartv2BaseTable_; }
  void setCheckCnstrntOfPartv2BaseTable(ItemExpr *expr) { checkCnstrntOfPartv2BaseTable_ = expr; }
  ...
  void setPotentialOutputValues(const ValueIdSet &vs) { potentialOutputs_ = vs; }
  void setPotentialOutputValues(const ValueIdList &vl) {
    potentialOutputs_.clear();
    potentialOutputs_.insertList(vl);
  }
  void addPotentialOutputValues(const ValueIdList &vl) { potentialOutputs_.insertList(vl); }
  //++MV - returns the GenericUpdateOutputFunction's that are in the
  // characteristic outputs
  NABoolean getOutputFunctionsForMV(ValueId &epochValueId, OperatorTypeEnum opType) const;
  ...
  virtual RelExpr *copyTopNode(RelExpr *derivedNode = NULL, CollHeap *outHeap = CmpCommon::statementHeap());
  ...
  short addSpecificExplainInfo(ExplainTupleMaster *explainTuple, ComTdb *tdb, Generator *generator,
  ...
  void finalizeRowsAffected(const GenericUpdate *before) {
    rowsAffected_ =
        (before->rowsAffected_ == DO_NOT_COMPUTE_ROWSAFFECTED) ? DO_NOT_COMPUTE_ROWSAFFECTED : COMPUTE_ROWSAFFECTED;
  ...
  void bindUpdateExpr(BindWA *bindWA, ItemExpr *recExpr, ItemExprList &RecExpr, RelExpr *boundView, Scan *scanNode,
                      SET(short) & stoiColumnSet, NABoolean onRollback = FALSE);
  ...
  // QSTUFF
  void setTransactionRequired(Generator *generator, NABoolean isNeededForAllFragments = FALSE);
  ...
  inline void setHalloweenCannotUseDP2Locks(NABoolean h) { halloweenCannotUseDP2Locks_ = h; }

  inline NABoolean getHalloweenCannotUseDP2Locks() const { return halloweenCannotUseDP2Locks_; }

  // Following methods compatible with Neo R2.
  inline NABoolean avoidHalloweenR2() const { return avoidHalloweenR2_ > 0; }

  inline void setAvoidHalloweenR2(UInt32 numHalloweenR2) { avoidHalloweenR2_ = numHalloweenR2; }
  ...
  NABoolean &canIMDeleteDirect() { return canIMDeleteDirect_; }
  ...
  NABoolean useMVCCorSSCC() {
    if (useSSCC_ || useMVCC_)
      return TRUE;
    else
      return FALSE;
  }
  ...
  inline ValueIdSet &getPrecondition() { return precondition_; }
  inline ValueIdSet &getPartQualPreCond() { return partQualPreCond_; }
  inline void setPreconditionTree(ItemExpr *pc) { preconditionTree_ = pc; }
  inline void setPrecondition(const ValueIdSet pc) {
    precondition_ = pc;
    exprsInDerivedClasses_ += precondition_;
  }
  inline void setPartQualPreCond(const ValueIdSet pc) {
    partQualPreCond_ = pc;
    exprsInDerivedClasses_ += partQualPreCond_;
  }
  inline const ValueId &getProducedMergeIUDIndicator() const { return producedMergeIUDIndicator_; }
  inline void setProducedMergeIUDIndicator(const ValueId &v) { producedMergeIUDIndicator_ = v; }
  inline const ValueId &getReferencedMergeIUDIndicator() const { return referencedMergeIUDIndicator_; }
  inline void setReferencedMergeIUDIndicator(const ValueId &v) { referencedMergeIUDIndicator_ = v; }

  virtual ItemExpr *insertValues() { return NULL; }

  void setCacheKey4UpdateSetList(const NAString &sCacheKey) { sCacheKey4UpdateSetList_ = sCacheKey; }
  const NAString &getCacheKey4UpdateSetList() { return sCacheKey4UpdateSetList_; }

  void setEmbeddedDeleteWithFirstN(NABoolean b) { embeddedDeleteWithFirstN_ = b; }
  NABoolean isEmbeddedDeleteWithFirstN() { return embeddedDeleteWithFirstN_; }
 ...
 private:
  enum GenericUpdateFlags { UPD_USING_UPSERT = 0x0004 };
  ...
  RETDesc *createOldAndNewCorrelationNames(BindWA *bindWA, NABoolean createRETDescOnly = FALSE);

  // helper function to method handleInlining use to check some currently
  // unsupported uses of triggers
  NABoolean checkNonSupportedTriggersUse(BindWA *bindWA, QualifiedName &subjectTable, ComOperation op,
  ...
  RelExpr *inlineIM(RelExpr *topNode, BindWA *bindWA, NABoolean isLastTSJ, UpdateColumns *updatedColumns,
                    CollHeap *heap, NABoolean useInternalSyskey, NABoolean rowTriggersPresent);

  // Handle inlining of RI, row triggers and the temp insert.
  RelExpr *inlinePipelinedActions(RelExpr *topNode, BindWA *bindWA, TriggerList *rowTriggers, RefConstraintList *riList,
                                  CollHeap *heap);

  // Create the sub-tree that inserts the affected set into the temporary table.
  RelExpr *inlineTempInsert(RelExpr *topNode, BindWA *bindWA, TriggersTempTable &tempTableObj,
  ...
  RelExpr *createTentativeSubTree(BindWA *bindWA, TriggerList *triggers, UpdateColumns *updatedColumns,
                                  TriggersTempTable &tempTableObj, CollHeap *heap);
  ...
  void removeRedundantInputsFromTempInsertTree(BindWA *bindWA, RelExpr *tentativeSubtree);

  RelExpr *inlineAfterOnlyBackbone(BindWA *bindWA, TriggersTempTable &tempTableObj, TriggerList *rowTriggers,
                                   TriggerList *stmtTriggers, RefConstraintList *riConstraints, NABoolean needIM,
                                   NABoolean isMVLoggingRequired, UpdateColumns *updatedColumns, CollHeap *heap);
  RelExpr *inlineAfterOnlyBackboneForUndo(BindWA *bindWA, TriggersTempTable &tempTableObj, TriggerList *rowTriggers,
                                          TriggerList *stmtTriggers, RefConstraintList *riConstraints, NABoolean needIM,
                                          NABoolean isMVLoggingRequired, UpdateColumns *updatedColumns, CollHeap *heap);

  RelExpr *inlineBeforeAndAfterBackbone(BindWA *bindWA, RelExpr *tentativeSubtree, TriggersTempTable &tempTableObj,
  ...
  RelExpr *createRISubtree(BindWA *bindWA, const NATable *naTable, const RefConstraint &refConstraint, CollHeap *heap);

  // Inline Referential Integrity constraints
  RelExpr *inlineRI(BindWA *bindWA, const RefConstraintList *refConstraints, CollHeap *heap);
  ...
  // range partition v2, check constraints of partition base table
  ItemExpr *checkCnstrntOfPartv2BaseTable_;  // should not be bound
  ...
  UInt32 flags_;
  NAString sCacheKey4UpdateSetList_;
  NABoolean embeddedDeleteWithFirstN_;
};
...
class Insert : public GenericUpdate {
 public:
  enum InsertType {
    ...
    UPSERT_INSERT,
    UPSERT_LOAD,
    UPSERT_NO_DUP  // converted to UPSERT_LOAD in binder. Used to Upsert
                   // into table with indexes when user guarantees there are
                   // no duplicates in rows to be inserted, between old and
                   // new rows and among new rows.
  };

  Insert(const CorrName &name, TableDesc *tabId, OperatorTypeEnum otype = REL_UNARY_INSERT, RelExpr *child = NULL,
         ItemExpr *insertCols = NULL, ItemExpr *orderBy = NULL, CollHeap *oHeap = CmpCommon::statementHeap(),
         InsertType insertType = SIMPLE_INSERT, NABoolean createUstatSample = FALSE);
  ...
  void setInsertType(const InsertType &insType) {
    if ((insType == UPSERT_INSERT) || (insType == UPSERT_LOAD) || (insType == UPSERT_NO_DUP) || (noCheck())) {
      if ((insType == UPSERT_LOAD) || (insType == UPSERT_NO_DUP)) insertType_ = insType;
  ...
  void setTargetUserColPosList(CollIndexList &colPos) { targetUserColPosList_ = &colPos; }
  void setTargetUserColPosList() { targetUserColPosList_ = NULL; }
  NABoolean canBindDefaultSpecification() const { return (targetUserColPosList_ != NULL); }
  const char *getColDefaultValue(BindWA *bindWA, CollIndex positionInTheColPosList) const;
  ...
  NABoolean systemGeneratesIdentityValue() const { return systemGeneratesIdentityValue_; };

  void setSystemGeneratesIdentityValue(NABoolean value) { systemGeneratesIdentityValue_ = value; };
  ...
  NABoolean getOverwriteHiveTable() { return overwriteHiveTable_; }
  void setOverwriteHiveTable(NABoolean v) { overwriteHiveTable_ = v; }

  NABoolean isSequenceFile() { return isSequenceFile_; }
  void setSequenceFile(NABoolean sf) { isSequenceFile_ = sf; }
  ...
  NABoolean getIsTrafLoadPrep() const { return isTrafLoadPrep_; }

  void setIsTrafLoadPrep(NABoolean isTrafLoadPrep) { isTrafLoadPrep_ = isTrafLoadPrep; }

  NABoolean getCreateUstatSample() const { return createUstatSample_; }

  void setCreateUstatSample(NABoolean val) { createUstatSample_ = val; }

  // No conflict check at commit
  NABoolean withNoConflictCheck() const { return noConflictCheck_; }
  void setWithNoConflictCheck(NABoolean flag = TRUE) { noConflictCheck_ = flag; }

  // for xformEfficientTreeUpsert, force get on target table to be
  // non-transactional. This get is non-transactional if autocommit is ON
  // else transactional by default. This flag overrides the default and
  // is enabled with UPSERT NO REPEATABLE READ ... syntax.
  NABoolean noRepeatableRead() const { return noRepeatableRead_; }
  void setNoRepeatableRead(NABoolean flag = TRUE) { noRepeatableRead_ = flag; }

  const ItemExprList &baseColRefs() const { return *baseColRefs_; }

  void setBaseColRefs(ItemExprList *val) { baseColRefs_ = val; }

  NABoolean xformedEffUpsert() const { return xformedEffUpsert_; }
  void setXformedEffUpsert(NABoolean x) { xformedEffUpsert_ = TRUE; }

  virtual NABoolean transformedUpdateNoDtmXn() const { return transformedUpdateNoDtmXn_; }
  void setTransformedUpdateNoDtmXn(NABoolean x) { transformedUpdateNoDtmXn_ = x; }

  NABoolean &isLastPart() { return isLastPart_; }
  NABoolean &isSpecifyPartition() { return isSpecifyPartition_; }

  NABoolean isUpsertThatNeedsTransformation(NABoolean isAlignedRowFormat, NABoolean omittedDefaultCols,
                                            NABoolean omittedCurrentDefaultCols, NABoolean &toMerge) const;
  RelExpr *xformUpsertToMerge(BindWA *bindWA);
  RelExpr *xformUpsertToEfficientTree(BindWA *bindWA);

  const ItemExprList *getPartColValList() const { return partColVals_; }
  void setPartColValList(ItemExprList *partColVals) { partColVals_ = partColVals; }
 ...
 private:
  ...
  // Insert into t1 select * from (select T1.a, T2.a from (values(1,2)) T1(a,b), (values(2,2)) T2(a,b) where T1.b =
  // T2.b)TT(a,b) ; -- insert-select
  ...
  ItemExpr *getOrderByTree() { return orderByTree_; }
  ...
  NABoolean transformedUpdateNoDtmXn_;
  ...
  // Transaction Type Support
  // this indicates that this is an upsert statement and no conflict
  // check need to be done at commit time.
  // Allowed only when autocommit is ON and there are no dependent objects
  NABoolean noConflictCheck_;

  // For upsert with index when we want the the Get on target table
  // to be non-transactional, even when autocommit is OFF
  NABoolean noRepeatableRead_;

  // list of 'col=val' specified as part of PARTITION clause of an
  // "insert overwrite table <tabname> partition (col=val...) ... " stmt.
  ItemExprList *partColVals_;
};
...
class LeafInsert : public Insert {
 public:
  LeafInsert(const CorrName &name, TableDesc *tabId, ItemExprList *afterColumns,
             OperatorTypeEnum otype = REL_LEAF_INSERT, ItemExpr *preconditionTree = NULL,
             CollHeap *oHeap = CmpCommon::statementHeap())
      : Insert(name, tabId, otype, NULL, NULL, NULL, oHeap) {
    setBaseColRefs(afterColumns);
    setPreconditionTree(preconditionTree);
  }
...
class Update : public GenericUpdate {
 public:
  Update(const CorrName &name, TableDesc *tabId, OperatorTypeEnum otype = REL_UNARY_UPDATE, RelExpr *child = NULL,
         ItemExpr *newRecExpr = NULL, ItemExpr *currOfCursorName = NULL, CollHeap *oHeap = CmpCommon::statementHeap());
...
class MergeUpdate : public Update {
 public:
  MergeUpdate(const CorrName &name, TableDesc *tabId,
              ...
              // above.
              OperatorTypeEnum otype = REL_UNARY_UPDATE, RelExpr *child = NULL, ItemExpr *setExpr = NULL,
              ItemExpr *insertCols = NULL, ItemExpr *insertValues = NULL, CollHeap *oHeap = CmpCommon::statementHeap(),
              ItemExpr *where = NULL, ItemExpr *andpred = NULL);
...
class Delete : public GenericUpdate {
 public:
  Delete(const CorrName &name, TableDesc *tabId, OperatorTypeEnum otype = REL_UNARY_DELETE, RelExpr *child = NULL,
         ItemExpr *newRecExpr = NULL, ItemExpr *currOfCursorName = NULL, ConstStringList *csl = NULL,
         CollHeap *oHeap = CmpCommon::statementHeap());
  ...
  inline NABoolean getWasFirstN() { return wasFirstN_; }
  inline void setWasFirstN(NABoolean v) { wasFirstN_ = v; }
  inline NABoolean &needConvertToAsj() { return needConvertToAsj_; }
...
class MergeDelete : public Delete {
 public:
  MergeDelete(const CorrName &name, TableDesc *tabId,
              // This is a problem, we shouldn't have two classes
              // that share the same OperatorTypeEnum value, but
              // REL_UNARY_DELETE is also used by the Delete class
              // above.
              OperatorTypeEnum otype = REL_UNARY_DELETE, RelExpr *child = NULL, ItemExpr *insertCols = NULL,
              ItemExpr *insertValues = NULL, CollHeap *oHeap = CmpCommon::statementHeap());
...
class LeafDelete : public Delete {
 public:
  LeafDelete(const CorrName &name, TableDesc *tabId, ItemExprList *beforeColumns, NABoolean isUndoUniqueIndex = FALSE,
             OperatorTypeEnum otype = REL_LEAF_DELETE, ItemExpr *preconditionTree = NULL,
             CollHeap *oHeap = CmpCommon::statementHeap())
...
class HbaseDelete : public Delete {
 public:
  HbaseDelete(CorrName &corrName, RelExpr *child = NULL, CollHeap *oHeap = CmpCommon::statementHeap());

  HbaseDelete(CorrName &corrName, TableDesc *tableDesc, CollHeap *oHeap = CmpCommon::statementHeap());
  ...
  virtual const char *getVirtualTableName() { return corrName_.getQualifiedNameObj().getObjectName().data(); }
 ...
 private:
  CostMethodHbaseDelete *pCostMethod_;
};  // class HbaseDelete
...
class InsertCursor : public Insert {
 public:
  InsertCursor(const CorrName &name, TableDesc *tabId, OperatorTypeEnum otype = REL_INSERT_CURSOR,
               RelExpr *child = NULL, CollHeap *oHeap = CmpCommon::statementHeap(),
...
class HiveInsert : public Insert {
 public:
  HiveInsert(const CorrName &name, TableDesc *tabId, OperatorTypeEnum otype = REL_HIVE_INSERT, RelExpr *child = NULL,
             CollHeap *oHeap = CmpCommon::statementHeap(), InsertType insertType = SIMPLE_INSERT)
      : Insert(name, tabId, otype, child, NULL, NULL, oHeap, insertType){};
...
class HbaseInsert : public Insert {
 public:
  HbaseInsert(const CorrName &name, TableDesc *tabId, OperatorTypeEnum otype = REL_HBASE_INSERT, RelExpr *child = NULL,
              CollHeap *oHeap = CmpCommon::statementHeap(), InsertType insertType = SIMPLE_INSERT)
      : Insert(name, tabId, otype, child, NULL, NULL, oHeap, insertType), returnRow_(FALSE), vsbbInsert_(FALSE) {
    if (oHeap)
      pCostMethod_ = new (oHeap) CostMethodHbaseInsert();
    else
      pCostMethod_ = new (CmpCommon::statementHeap()) CostMethodHbaseInsert();
  };
 ...
 private:
  ...
  CostMethodHbaseInsert *pCostMethod_;
};

class HBaseBulkLoadPrep : public Insert {
 public:
  HBaseBulkLoadPrep(const CorrName &name, TableDesc *tabId, OperatorTypeEnum otype = REL_HBASE_BULK_LOAD,
                    RelExpr *child = NULL, NABoolean createUstatSample = FALSE,
                    CollHeap *oHeap = CmpCommon::statementHeap(), InsertType insertType = SIMPLE_INSERT)
...
class UpdateCursor : public Update {
 public:
  UpdateCursor(const CorrName &name, TableDesc *tabId, OperatorTypeEnum otype = REL_UPDATE_CURSOR,
               RelExpr *child = NULL, CollHeap *oHeap = CmpCommon::statementHeap())
...
class HbaseUpdate : public UpdateCursor {
 public:
  HbaseUpdate(CorrName &corrName, RelExpr *child = NULL, CollHeap *oHeap = CmpCommon::statementHeap());

  HbaseUpdate(CorrName &corrName, TableDesc *tableDesc, CollHeap *oHeap = CmpCommon::statementHeap());
 ...
 private:
  CostMethodHbaseUpdate *pCostMethod_;
};  // class HbaseUpdate
...
class DeleteCursor : public Delete {
 public:
  DeleteCursor(const CorrName &name, TableDesc *tabId, OperatorTypeEnum otype = REL_DELETE_CURSOR,
               RelExpr *child = NULL, CollHeap *oHeap = CmpCommon::statementHeap())
