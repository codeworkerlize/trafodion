...
struct ComResWord {
 public:
  ...
  //
  inline NABoolean isReserved(UInt32 ifSetFlags) { return ((flags_ & RESWORD_) && !(allowOldAndNew(ifSetFlags))); };

  // Are the reserved words OLD and NEW allowed as non-reserved
  // identifiers.
  //
  inline NABoolean allowOldAndNew(UInt32 ifSetFlags) { return (ifSetFlags & (flags_ & ALLOWOLDNEW_)); };
...
class ComResWords : public NABasicObject {
 public:
  ...
  //
  static NABoolean isSqlReservedWord(const char *word, UInt32 ifSetFlags = 0);

 private:
  // Encapulates the searching of the reserved word table.
  //
  static inline ComResWord *searchResWordTbl(ComResWord *val) { return binarySearch(val); }
