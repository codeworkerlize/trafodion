...
enum StaticOnly { NOT_STATIC_ONLY = FALSE, STATIC_ONLY_VANILLA = TRUE, STATIC_ONLY_WITH_WORK_FOR_PREPROCESSOR };
...
class ExprNode : public NABasicObject {
 public:
  enum ChildCondition {
    ALL_CHILDREN,          // all children of this node
  ...
  inline void setOperatorType(OperatorTypeEnum newType) { operator_ = newType; }

  NABoolean isStringOutput();
  ...
  virtual const NAString getTextForError() const { return getTextUpper(); }
  ...
  enum Cacheability { MAYBECACHEABLE, NONCACHEABLE, CACHEABLE_PARSE, CACHEABLE_BIND };

  void setNonCacheable() { cacheable_ = NONCACHEABLE; }
  void setCacheable(Cacheability ca) { cacheable_ = ca; }
