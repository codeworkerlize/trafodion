...
template <class K, class V>
class NAHashDictionaryIterator;
template <class K, class V>
class NAHashDictionaryIteratorNoCopy;
template <class K, class V>
class NAKeyLookup;
...
#define IsComplete(x) (x && x->isComplete())

// -----------------------------------------------------------------------
// The generic collection template, base class of other collection types
// -----------------------------------------------------------------------
template <class T>
class NACollection : public NABasicObject

{
 ...
 public:
  ...
  NACollection(CollIndex initLen = 0, NABoolean failureIsFatal = TRUE)
      : heap_(NULL), failureIsFatal_(failureIsFatal), isComplete_(TRUE) {
    allocate(initLen);
  }

  // constructor for a collection with user-defined heap management
  NACollection(CollHeap *heap, CollIndex initLen = 0, NABoolean failureIsFatal = TRUE)
      : heap_(heap), failureIsFatal_(failureIsFatal), isComplete_(TRUE) {
    allocate(initLen);
  }
...
#if defined(_DEBUG)
    if ((ix >= usedLength_) OR(usages_[ix] == UNUSED_COLL_ENTRY))
  ...
  inline void setUsage(CollIndex pos, CollIndex newUsage) {
    if ((pos < usedLength_) AND(usages_[pos] != UNUSED_COLL_ENTRY) AND(newUsage != UNUSED_COLL_ENTRY))
  ...
  inline void clear() {
    for (CollIndex i = FIRST_COLL_INDEX; i < usedLength_; i++) usages_[i] = UNUSED_COLL_ENTRY;
  ...
  inline void remove(CollIndex posToDelete) {
    if ((posToDelete < usedLength_) && (usages_[posToDelete] != UNUSED_COLL_ENTRY)) {
      ...
      if (posToDelete == usedLength_ - 1) {
        while (usedLength_ > 0 AND usages_[usedLength_ - 1] == UNUSED_COLL_ENTRY) usedLength_--;

        // resize, if we gained back a substantial part of the array
        if (maxLength_ > 10 AND usedLength_ < maxLength_ / 4) resize(usedLength_);
      }
    }
    return;
  }

  void insert(CollIndex posToInsert, const T &newElement, CollIndex newUsage = NULL_COLL_INDEX,
              NABoolean *inserted = NULL);

  inline CollIndex find(const T &toFind) const {
    for (CollIndex i = FIRST_COLL_INDEX; i < usedLength_; i++) {
      if ((usages_[i] != UNUSED_COLL_ENTRY) && (arr_[i] == toFind)) return i;
    }
  ...
  NABoolean isComplete() const { return isComplete_; }
  void setIsComplete(NABoolean x) { isComplete_ = x; }

  NABoolean isFailureIsFatal() const { return failureIsFatal_; }
 ...
 private:
  ...
  NABoolean isComplete_;  // TRUE : everything is OK
                          // FALSE : memory allocation failed and the
...
template <class T>
class NASubCollection : public NABasicObject {
  // protected:  doesn't work for NT nor C89
 public:
  // constructor
  NASubCollection(NACollection<T> *superset, CollHeap *heap = 0, NABoolean failureIsFatal = TRUE);
  ...
  inline CollIndex resize(CollIndex newSize) {
    if (newSize > maxLength_) {
      ...
      if (heap_) {
        newBits = new (heap_, failureIsFatal_) WordAsBits[maxLength_];
        if (!newBits) {
          isComplete_ = FALSE;
          return 0;
        }
      ...
      *newBits = *pBits;
      ...
      for (i = wordSize_; i < maxLength_; i++) newBits[i] = 0x0;
  ...
  inline NABoolean nextUsed(CollIndex &start) const {
    ...
    if (start >= limit || entries_ == 0)
      ...
      CollIndex firstDWord = pBits_[wordNo(start)] & ((~0u) >> startOffset);
      ...
      if (firstDWord) {
        ...
        if (dWordsRemaining == 0) return (FALSE);
        ...
        start = ((((char *)pBits) - ((char *)pBits_)) >> LogBytesPerWord) << LogBitsPerWord;
  ...
  inline NASubCollection<T> &addElement(CollIndex elem, NABoolean *inserted = NULL) {
    CollIndex wordNumber = wordNo(elem);

    assert(superset_ == NULL OR(superset_->getUsage(elem) != UNUSED_COLL_ENTRY));

    if (wordNumber >= wordSize_) {
      extendWordSize(wordNumber + 1);

      if (inserted) *inserted = isComplete_;
      ...
      if (originalWord != (pBits_[wordNumber] |= SingleBitArray[bitNo(elem)])) entries_++;
    }
    // Record the last bit ever set.
    if (elem > lastStaleBit_) lastStaleBit_ = elem;
    return (*this);
  }
  ...
  inline NASubCollection<T> &subtractElement(CollIndex elem) {
    ...
    if (wordNumber < wordSize_) {
      ...
      if (originalWord != (pBits_[wordNumber] &= LNOT SingleBitArray[bitNo(elem)])) entries_--;
    }
  ...
  // ---------------------------------------------------------------------
  inline NASubCollection<T> &operator+=(const NASubCollection<T> &other) { return addSet(other); }
  inline NASubCollection<T> &operator-=(const NASubCollection<T> &other) { return subtractSet(other); }
  inline NASubCollection<T> &operator+=(CollIndex elem) { return addElement(elem); }
  inline NASubCollection<T> &operator-=(CollIndex elem) { return subtractElement(elem); }
 ...
 private:
  ...
  NABoolean failureIsFatal_;  // TRUE: memory allocation failure is fatal
  NABoolean isComplete_;      // FALAE: the object is not in a usable state.

  // allocate and deallocate the pBits_ vector
  inline WordAsBits *allocateBits(CollIndex words) {
    return (heap_ ? (new (heap_, failureIsFatal_) WordAsBits[words]) : (new WordAsBits[words]));
  }
...
inline void NASubCollection<T>::clearFast() {
  if (lastStaleBit_ >= 128) {  // call the regular clear.
    clear();
    return;
  }
...
inline CollIndex NASubCollection<T>::prevUsed(CollIndex start) const {
  if (start > lastStaleBit_) return NULL_COLL_INDEX;

  if (start >= 64) return prevUsedSlow(start);
...
CollIndex NASubCollection<T>::prevUsedSlow(CollIndex start) const {
  ...
  do {
    ...
    if (startWordBucket >= 0) startWord = pBits_[startWordBucket];
...
inline CollIndex NASubCollection<T>::prevUsedFast(CollIndex start) const {
  ...
  startDoubleWord = (((uint64_t)(*pBits_) << 32) | (uint64_t)(*(pBits_ + 1))) >> shiftAmount;

  // If double word is 0, no bits are set, so return.
  if (!startDoubleWord) return NULL_COLL_INDEX;

  // Return start bit if it is set.  Otherwise calculate ordinal position of
  // last bit set and return it.
  return ((startDoubleWord & 1) ? start : 64 - shiftAmount - FindLastOne(startDoubleWord));
}
...
inline NABoolean NASubCollection<T>::nextUsedFast(CollIndex &start) const {
  if (start > lastStaleBit_)  // no bit 1 entry after lastStaleBit_.
    return FALSE;
  if (start >= 64) return nextUsed(start);  // call the regular nextUsed.
  ...
  if (lastStaleBit_ < 64) return FALSE;  // lastStaleBit_ is in the first two words.
...
inline NASubCollection<T> &NASubCollection<T>::addElementFast(CollIndex elem) {
  if (elem >= 64) return addElement(elem);
  WordAsBits *w = pBits_;
  CollIndex j = elem;
  if (j >= 32) {
    w = &pBits_[1];
  ...
  if (((*w) << j) >> 31) return (*this);  // already set.
...
// -----------------------------------------------------------------------
template <class T>
class NASet : public NACollection<T> {
 public:
  // constructor with user-defined heap
  NASet(CollHeap *heap, CollIndex initSize = 0, NABoolean failureIsFatal = TRUE)
      : NACollection<T>(heap, initSize, failureIsFatal) {
    invalidateCache();
  }

  // copy ctor
  NASet(const SET(T) & other, CollHeap *heap) : NACollection<T>(other, heap) { invalidateCache(); }
  ...
  inline NABoolean contains(const T &elem) const { return (NACollection<T>::find(elem) != NULL_COLL_INDEX); }
  ...
  inline NABoolean insert(const T &elem, NABoolean *inserted = NULL) {
    ...
    if (NACollection<T>::find(elem) == NULL_COLL_INDEX) {
      NACollection<T>::insert(this->freePos(), elem, NULL_COLL_INDEX, inserted);
      return TRUE;
    } else
      return FALSE;
  }
  ...
  inline NABoolean insert(const NASet<T> &other, NABoolean *inserted = NULL) {
    CollIndex count = other.entries();
    for (CollIndex i = 0; i < count; i++) {
      insert(other[i], inserted);

      // If the insertion status can be retrieved and
      // is not successful, return TRUE.
 ...
 private:
  inline void invalidateCache() { userIndexCache_ = arrayIndexCache_ = NULL_COLL_INDEX; }
...
// -----------------------------------------------------------------------
template <class T>
class NAList : public NACollection<T> {
 public:
  // constructor with user-defined heap
  NAList(CollHeap *heap, CollIndex initLen = 0, NABoolean failureIsFatal = TRUE);

  NAList(CollHeap *heap, CollIndex initLen, NABoolean failureIsFatal, NABoolean useMutex);

  // copy ctor
  NAList(const NAList<T> &other, CollHeap *heap);

  // virtual destructor
  virtual ~NAList();

  static void fixupMyVTable(NAList *ptr);
  ...
  // insert a new entry
  inline void insert(const T &elem, NABoolean *inserted = NULL) { this->insertAt(this->entries(), elem, inserted); }

  // insert a set, array, or list
  // void insert(const SET(T) &other);

  void insert(const LIST(T) & other, NABoolean *inserted = NULL);

  // remove an element(the first that matches) that is given by its value
  //(returns whether the element was found and removed)
  inline NABoolean remove(const T &elem) { return removeCounted(elem, 1) > 0; }

  // remove elements(all that match) given by value
  //(returns the number of elements removed)
  inline CollIndex removeAll(const T &elem) { return removeCounted(elem, 0); }
  ...
  void reverseOrder() {
    CollIndex n = this->entries();
    CollIndex m = this->entries() / 2;
    T tmp;
    for (CollIndex i = 0; i < m; i++) {
      tmp = operator[](i);
      operator[](i) = operator[](n - 1 - i);
      operator[](n - 1 - i) = tmp;
    }
  }
  ...
  inline NABoolean removeAt(const CollIndex index) {
    ...
    if (index >= this->entries()) return FALSE;
    ...
    if (index == 0) {
      ...
      if (elemToRemove == last_) last_ = first_;
    } else {
      ...
      if (elemToRemove == last_) last_ = pred;
      NACollection<T>::remove(elemToRemove);
    }
  ...
  inline CollIndex append(const T &elem, NABoolean *inserted = NULL) {
    return insertAt(this->entries(), elem, inserted);
  }
  ...
  inline CollIndex insertAt(CollIndex i, const T &elem, NABoolean *inserted = NULL) {
    ...
    if (i == 0) {
      ...
      newUsage = first_;
      first_ = newIndex;
      if (last_ == NULL_COLL_INDEX) last_ = first_;
    } else if (i == this->entries()) {
      // add the new element at the tail of the list
      if (last_ != NULL_COLL_INDEX) this->setUsage(last_, newIndex);
      ...
      while (predIx < i - 1) {
        pred = this->getUsage(pred);
        predIx++;
        if (pred == NULL_COLL_INDEX) ABORT("Insert position in list is invalid");
      }
    ...
    NABoolean ok = TRUE;
    NACollection<T>::insert(newIndex, elem, newUsage, &ok);

    // Invalidate the cache upon a successful insert.
    if (ok && userIndexCache_ >= i) invalidateCache();

    // update the insertion status pointer if exist
  ...
  inline NABoolean getFirst(T &elem) {
    if (this->entries() > 0) {
      ...
      if (last_ == oldFirst) last_ = NULL_COLL_INDEX;
  ...
  void simulateBadIndex(NABoolean x) { simualteBadIndex_ = x; }
...
// -----------------------------------------------------------------------
template <class T>
class NAArray : public NACollection<T> {
 public:
  // constructor with user-defined heap
  NAArray(CollHeap *heap, CollIndex initialElements = 0, NABoolean failureIsFatal = TRUE)
      : NACollection<T>(heap, initialElements, failureIsFatal) {}

  // copy ctor
  NAArray(const NAArray &other, CollHeap *heap) : NACollection<T>(other, heap) {}

  // Resize the array to a new size(return new size)
  inline CollIndex resize(CollIndex newSize) { return NACollection<T>::resize(newSize); }

  // access a used entry(warning: accessing unused entries aborts!!)
  inline T &operator[](CollIndex index) { return this->usedEntry(index); }
  inline const T &operator[](CollIndex index) const { return this->constEntry(index); }
  inline T &at(CollIndex i) { return operator[](i); }
  inline const T &at(CollIndex i) const { return operator[](i); }

  // check whether a certain entry is used(prior to using operator [])
  inline NABoolean used(CollIndex index) const { return (this->getUsage(index) != UNUSED_COLL_ENTRY); }
  ...
  inline NABoolean operator!=(const NAArray<T> &other) const { return NOT operator==(other); };
  ...
  // NULL_COLL_INDEX
  inline CollIndex find(const T &toFind) const { return NACollection<T>::find(toFind); }

  // insert or overwrite array entry at a specified position
  inline void insertAt(CollIndex index, const T &newEntry, NABoolean *inserted = NULL) {
    NACollection<T>::insert(index, newEntry, NULL_COLL_INDEX, inserted);
  }
  ...
  static void fixupMyVTable(NAArray *ptr);

  // Intersect this with other and save the results in 'result'.
  // Return true if the operation is OK. return false if some element is not
  // inserted (say due to memory pressure).
  NABoolean intersect(const NAArray &other, NAArray &result) const;
...
// -----------------------------------------------------------------------
template <class T>
class NASubArray : public NASubCollection<T> {
 public:
  NASubArray(NAArray<T> *superset, CollHeap *heap, NABoolean failureIsFatal = TRUE)
      : NASubCollection<T>(superset, heap, failureIsFatal) {}

  NASubArray(const NASubArray<T> &other, CollHeap *heap) : NASubCollection<T>(other, heap) {}

  virtual ~NASubArray();

  inline CollIndex resize(CollIndex newSize) { return NASubCollection<T>::resize(newSize); }

  inline void clear() { NASubCollection<T>::clear(); }

  inline NABoolean operator==(const NASubArray<T> &other) const { return NASubCollection<T>::operator==(other); }
  inline NABoolean operator!=(const NASubArray<T> &other) const { return (NOT operator==(other)); }

  inline NASubArray<T> &operator=(const NASubArray<T> &other) {
    NASubCollection<T>::operator=(other);
    return *this;
  }

  // check whether a particular entry is part of the subarray
  inline NABoolean contains(CollIndex elem) const { return NASubCollection<T>::testBit(elem); }
  inline NABoolean contains(const NASubArray<T> &other) const { return NASubCollection<T>::contains(other); }

  // find the next used entry in the array or return false
  // e.g. in a for-loop:
  // for(CollIndex i = 0; nextUsed(i); i++) ...element(i)...
  inline NABoolean nextUsed(CollIndex &start) const { return NASubCollection<T>::nextUsed(start); }

  inline NASubArray<T> &insert(const NASubArray<T> &other) {
    NASubCollection<T>::addSet(other);
    return *this;
  }
  inline NASubArray<T> &intersectSet(const NASubArray<T> &other) {
    NASubCollection<T>::intersectSet(other);
    return *this;
  }
  inline NASubArray<T> &remove(const NASubArray<T> &other) {
    NASubCollection<T>::subtractSet(other);
    return *this;
  }
  inline NASubArray<T> &insert(CollIndex elem, NABoolean *inserted = NULL) {
    NASubCollection<T>::addElement(elem, inserted);
    return *this;
  }
  inline NASubArray<T> &remove(CollIndex elem) {
    NASubCollection<T>::subtractElement(elem);
    return *this;
  }
  ...
  inline NASubArray<T> &operator+=(const NASubArray<T> &other) { return insert(other); }
  inline NASubArray<T> &operator-=(const NASubArray<T> &other) { return remove(other); }
  inline NASubArray<T> &operator+=(CollIndex elem) { return insert(elem); }
  inline NASubArray<T> &operator-=(CollIndex elem) { return remove(elem); }

  inline WordAsBits hash() const { return NASubCollection<T>::hash(); }

  inline const T &element(CollIndex e) const { return NASubCollection<T>::element(e); }
...
/*                                                                          */
template <class T>
class NASimpleArray : private NAArray<T> {
 public:
  NASimpleArray(CollHeap *h, NABoolean failureIsFatal = TRUE) : NAArray<T>(h, 0, failureIsFatal) {}

  // copy ctor
  NASimpleArray(const NASimpleArray &orig, CollHeap *h) : NAArray<T>(orig, h) {}
  ...
  inline void clear() { NAArray<T>::clear(); }

  static void fixupMyVTable(NASimpleArray *ptr);

  T &operator[](CollIndex index) { return NAArray<T>::operator[](index); };

  void append(T newItem) { this->insertAt(entries(), newItem); }

  CollIndex entries() const { return NAArray<T>::entries(); }
  ...
  static inline void *operator new(size_t size, CollHeap *h) { return NAArray<T>::operator new(size, h); }
  static inline void operator delete(void *buffer) { NAArray<T>::operator delete(buffer); }

  inline void setHeap(CollHeap *heap) { NAArray<T>::setHeap(heap); }
};  // NASimpleArray
...
enum iteratorEntryType { EVERYTHING, ENABLED, DISABLED };
...
class NAHashBucketEntry : public NABasicObject {
 public:
  ...
  // --------------------------------------------------------------------
  NAHashBucketEntry(K *key, V *value) : key_(key), value_(value), isEnabled_(TRUE) {}

  // copy ctor
  NAHashBucketEntry(const NAHashBucketEntry<K, V> &other)
      : key_(other.key_), value_(other.value_), isEnabled_(other.isEnabled_) {}
  ...
  static void fixupMyVTable(NAHashBucketEntry *ptr);

  // test if isEnabled_ matches the type 'type'
  NABoolean matchEntryType(enum iteratorEntryType type) const;
 ...
 private:
  ...
  // This data member indicates the state of the entry. For some
  // applications of the hash dictionary, a key-value pair can be
  // enabled or disabled, with only the enabled entries can participate
  // in getFirstValue() or contains() etc lookup operations. The disabled
  // entries are hidden in the data structure until they are enabled.
  //
  // By default, all entries are enabled unless they are specifically
  // disabled.
  NABoolean isEnabled_;
...
class NAHashBucket : public NABasicObject {
 public:
  ...
  // Constructor function
  // --------------------------------------------------------------------
  NAHashBucket(CollHeap *heap, NABoolean failureIsFatal = TRUE)
      : bucket_(heap, 0, failureIsFatal), heap_(heap), failureIsFatal_(failureIsFatal) {}

  NAHashBucket(CollHeap *heap, NABoolean failureIsFatal, NABoolean useMutex)
      : bucket_(heap, 0, failureIsFatal, useMutex), heap_(heap), failureIsFatal_(failureIsFatal) {}
  // copy ctor
  ...
  static void fixupMyVTable(NAHashBucket *ptr);
  ...
  // key value pair.
  NABoolean contains(const K *key, bool checkAll = false) const;
  ...
  V *getFirstValueAll(const K *key) const;
  ...
  inline void insert(K *key, V *value, NABoolean *inserted = NULL) {
    NAHashBucketEntry<K, V> *newEntry = new (heap_, failureIsFatal_) NAHashBucketEntry<K, V>(key, value);

    if (!newEntry) {
      if (inserted) *inserted = FALSE;
    } else {
      bucket_.insertAt(0, newEntry, inserted);
    }
  }
  ...
  // bad index in bucket_[] when the argument 'x' is TRUE.
  void simulateBadIndex(NABoolean x) { bucket_.simulateBadIndex(x); };

  NABoolean isComplete() const { return bucket_.isComplete(); }

  NAList<NAHashBucketEntry<K, V> *> *getBucket() { return &bucket_; }
...
class NAHashDictionary : public NABasicObject {
 public:
  ...
  friend class NAHashDictionaryIteratorNoCopy<K, V>;
...
#define NAHashDictionary_Default_Size 11
  ...
  static void fixupMyVTable(NAHashDictionary *ptr);
  ...
  // Remove all bucket entries and optionally delete each
  // key value pair stored in each bucket entry (when deleteContents
  // is TRUE).
  // Does not delete all hash buckets.
  // Does not delete the hash table and the dictionary.
  void clear(NABoolean deleteContents = FALSE);

  // Remove all bucket entries and delete each
  // key value pair stored in each bucket entry.
  // Does not delete all hash buckets.
  // Does not delete the hash table and the dictionary.
  ...
  // Remove all bucket entries and delete each
  // key value pair stored in each bucket entry.
  // Delete all hash buckets and the hash table.
  //
  // Call this method when it is necessary to
  // delete all data structures allocated for this
  // hash dictionary from the heap.
  void clearAll() { destroyHashTable(); }
  ...
  NABoolean operator==(const NAHashDictionary<K, V> &other) const;

  // Detach all hash buckets. Used by the dstr of some objects
  // (such as NATable) where a hash dictionary is shared at the
  // hash bucket level.
  void detachHashBuckets();
 ...
 private:
  ...
  // FALSE: the data structure is not in a usable state due to
  // some allocation failure.
  NABoolean isComplete_;
...
// double * value ;
// int desiredKey = 3;
// NAHashDictionaryIterator<int,double> bobIter (bob, &desiredKey) ;
//
...
// }
//
// --> create an iterator for all entries of the hash dictionary
//     and use a while loop instead
// int * key ;
// double * value ;
// NAHashDictionaryIterator<int,double> bobIter2 (bob) ;
//
// --> loop over and print out everything in the iterator
// while (bobIter2.getNext(key, value))
// {
//   cout << *key << " : " << *value << endl ;
// }
//
...
class NAHashDictionaryIterator : public NABasicObject {
 public:
  ...
  // copy ctor
  NAHashDictionaryIterator<K, V>(const NAHashDictionaryIterator<K, V> &other, CollHeap *heap);
  ...
  // have been iterated over. The method returns TRUE if there is
  // another entry, FALSE otherwise.
  NABoolean getNext(K *&key, V *&value);
...
// A version of hash dictionary iterator that does not copy all
// key and value pairs to a buckter container. It remembers the
// current position via currentBucket_ and iteratorPosition_
// variables, and the begin and the end bucket through the
// beginBucket_ and endBucket_ variables. entries_ is the number
// of key and value pairs to iterate over with.
//
// Usage note: the iterator uses logic index in a hash bucket
// to keep track the position in that hash bucket. As such,
// the iterator will not work (in fact crash) if during the
// traversal of the hash dictionary with the NoCopy iterator,
// an entry is deleted. For such use case, please use the
// NAHashDictionaryIterator, which copies out the entries first.
template <class K, class V>
class NAHashDictionaryIteratorNoCopy : public NABasicObject {
 public:
  // basic ctor
  ...
  // dtor
  ~NAHashDictionaryIteratorNoCopy<K, V>();
  ...
  NABoolean isComplete() const { return TRUE; }

 protected:
  NAHashBucket<K, V> *currentBucket() { return (*dict_.hashTable_)[currentBucket_]; }

  // advance to next logical position where a key,value pair can be
  // found.
  void advance();

  // advance to next logical position in the current bucket
  // where a key, value pair of type_ can be found.  This method
  // is a help method to advance() and reset().
  NABoolean advanceInCurrentBucket(NABoolean newBucket);

 private:
  NAHashDictionaryIteratorNoCopy<K, V>();  // never create an uninitialized iterator!

  const NAHashDictionary<K, V> &dict_;
...
class NAKeyLookup : public NAHashDictionary<K, V> {
 public:
  ...
  // Constructor functions
  // --------------------------------------------------------------------
  NAKeyLookup(short initSize, NAKeyLookupEnums::KeyProvenance keyProvenance, CollHeap *heap)
      : NAHashDictionary<K, V>(&hashKey, initSize, TRUE, heap), keyProvenance_(keyProvenance) {}

  // copy ctor
  NAKeyLookup(const NAKeyLookup &nakl, CollHeap *heap)
      : NAHashDictionary<K, V>(nakl, heap), keyProvenance_(nakl.keyProvenance_) {}
  ...
  inline void insert(const V *vv)  // insert a tuple
  {
    ...
    if (rv == NULL && keyProvenance_ == NAKeyLookupEnums::KEY_INSIDE_VALUE) delete kk;  // avoid memleak!
  }
  ...
  void clearAndDestroyKeysOnly() {
    ...
    for (CollIndex i = 0; i < entries; i++) delete keys[i];
...
#include "../../common/Collections.cpp"
#endif
