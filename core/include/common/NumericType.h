...
class NumericType : public NAType {
 public:
  ...
  NABoolean isExact() const { return qualifier_ >= MIN_EXACT_NUMERIC_TYPE && qualifier_ <= MAX_EXACT_NUMERIC_TYPE; }

  NABoolean isAnyUnsignedInt() const { return isExact() AND isUnsigned() AND binaryPrecision(); }

  NABoolean isInteger() const;

  NABoolean isDecimal() const { return qualifier_ == SQLDecimal_TYPE || qualifier_ == LSDecimal_TYPE; }
  ...
  // ---------------------------------------------------------------------
  virtual NAType *newCopy(CollHeap *h = 0) const { return new (h) NumericType(*this, h); }
...
class SQLBPInt : public NumericType {
 public:
  ...
  SQLBPInt(NAMemory *heap, UInt32 declared, NABoolean allowSQLnull = TRUE, NABoolean allowNegValues = FALSE);

  short getFSDatatype() const { return REC_BPINT_UNSIGNED; }
...
class SQLTiny : public NumericType {
 public:
  ...
  // Constructor functions
  // ---------------------------------------------------------------------
  SQLTiny(NAMemory *heap, NABoolean allowNegValues = TRUE, NABoolean allowSQLnull = TRUE);
  ...
  // A virtual function to return a copy of the type.
  // ---------------------------------------------------------------------
  virtual NAType *newCopy(CollHeap *h = 0) const { return new (h) SQLTiny(h, !isUnsigned(), supportsSQLnull()); }
...
class SQLSmall : public NumericType {
 public:
  ...
  // Constructor functions
  // ---------------------------------------------------------------------
  SQLSmall(NAMemory *heap, NABoolean allowNegValues = TRUE, NABoolean allowSQLnull = TRUE);
  ...
  // A virtual function to return a copy of the type.
  // ---------------------------------------------------------------------
  virtual NAType *newCopy(CollHeap *h = 0) const { return new (h) SQLSmall(h, !isUnsigned(), supportsSQLnull()); }
...
class SQLInt : public NumericType {
 public:
  ...
  // Constructor functions
  // ---------------------------------------------------------------------
  SQLInt(NAMemory *heap, NABoolean allowNegValues = TRUE, NABoolean allowSQLnull = TRUE);
  ...
  // A virtual function to return a copy of the type.
  // ---------------------------------------------------------------------
  virtual NAType *newCopy(CollHeap *h = 0) const { return new (h) SQLInt(h, !isUnsigned(), supportsSQLnull()); }
...
class SQLLargeInt : public NumericType {
 public:
  ...
  virtual double getMinValue() const { return (isUnsigned() ? 0 : -9.2233720368547e+18); }
...
NAString SQLLargeInt::getClientDataTypeName() const {
  if (clientDataType_.isNull()) return getSimpleTypeName();
  return clientDataType_;
}

void SQLLargeInt::setClientDataType(NAString clientDataTypeName) { clientDataType_ = clientDataTypeName; }
...
class SQLBigInt : public SQLLargeInt {
 public:
  SQLBigInt(NAMemory *heap, NABoolean allowNegValues = TRUE, NABoolean allowSQLnull = TRUE);

  // ---------------------------------------------------------------------
  // A virtual function to return a copy of the type.
  // ---------------------------------------------------------------------
  virtual NAType *newCopy(CollHeap *h = 0) const { return new (h) SQLBigInt(h, !isUnsigned(), supportsSQLnull()); }
...
class SQLNumeric : public NumericType {
 public:
  ...
  // ---------------------------------------------------------------------
  virtual NAType *newCopy(CollHeap *h = 0) const {
    return new (h) SQLNumeric(h, getNominalSize(), getPrecision(), getScale(), !isUnsigned(), supportsSQLnull());
  }
...
class SQLDecimal : public NumericType {
 public:
  ...
  // ---------------------------------------------------------------------
  virtual NAType *newCopy(CollHeap *h = 0) const {
    return new (h) SQLDecimal(h, getNominalSize(), getScale(), !isUnsigned(), supportsSQLnull());
  }
...
class SQLBigNum : public NumericType {
 public:
  ...
  virtual NAType::SynthesisPrecedence getSynthesisPrecedence() const { return SYNTH_PREC_BIG_NUM; }

  // ---------------------------------------------------------------------
  // A virtual function to return a copy of the type.
  // ---------------------------------------------------------------------
  virtual NAType *newCopy(CollHeap *h = 0) const {
    return new (h) SQLBigNum(h, getPrecision(), getScale(), isARealBigNum(), !isUnsigned(), supportsSQLnull());
  }
...
class LSDecimal : public NumericType {
 public:
  ...
  NAType *equivalentType(CollHeap *h = 0) const {
    return new (h) SQLDecimal(h, getNominalSize() - 1, getScale(), TRUE, supportsSQLnull());
  }
  ...
  // ---------------------------------------------------------------------
  virtual NAType *newCopy(CollHeap *h = 0) const {
    return new (h) LSDecimal(h, getNominalSize(), getScale(), supportsSQLnull());
  }
...
class SQLFloat : public NumericType {
 public:
  ...
  virtual NAType::SynthesisPrecedence getSynthesisPrecedence() const { return SYNTH_PREC_FLOAT; }

  // ---------------------------------------------------------------------
  // A virtual function to return a copy of the type.
  // ---------------------------------------------------------------------
  virtual NAType *newCopy(CollHeap *h = 0) const {
    return new (h) SQLFloat(h, supportsSQLnull(), getNominalSize(), getPrecision(), getSimpleTypeName());
  }
...
class SQLReal : public SQLFloat {
 public:
  ...
  // A virtual function to return a copy of the type.
  // ---------------------------------------------------------------------
  virtual NAType *newCopy(CollHeap *h = 0) const { return new (h) SQLReal(h, supportsSQLnull()); }
