...
class RtsMessageObj : public IpcMessageObj {
 ...
 public:
  ...
  //
  RtsMessageObj(RtsMessageObjType objType, IpcMessageObjVersion objVersion, NAMemory *heap);

  //
  // Constructor for copyless receive
  //
  RtsMessageObj(IpcBufferedMsgStream *msgStream) : IpcMessageObj(msgStream), heap_(NULL) {}
...
class RtsStatsReq : public RtsMessageObj {
 public:
  ...
  // Constructor for copyless receive
  //
  RtsStatsReq(IpcBufferedMsgStream *msgStream) : RtsMessageObj(msgStream) {}
...
class RtsStatsReply : public RtsMessageObj {
 public:
  ...
  // Constructor for allocation on a heap
  //
  RtsStatsReply(NAMemory *heap) : RtsMessageObj(RTS_MSG_STATS_REPLY, currRtsStatsReplyVersionNumber, heap) {
    numSscpErrors_ = 0;
    numSqlProcs_ = 0;
    numCpus_ = 0;
  }
  ...
  // Constructor for copyless send
  //
  RtsStatsReply(const RtsHandle &h) : RtsMessageObj(RTS_MSG_STATS_REPLY, currRtsStatsReplyVersionNumber, NULL) {
    setHandle(h);
    numSscpErrors_ = 0;
    numSqlProcs_ = 0;
    numCpus_ = 0;
  ...
  // Constructor for copyless receive
  //
  RtsStatsReply(IpcBufferedMsgStream *msgStream) : RtsMessageObj(msgStream) {}
  ...
  void unpackObj(IpcMessageObjType objType, IpcMessageObjVersion objVersion, NABoolean sameEndianness,
                 IpcMessageObjSize objSize, IpcConstMessageBufferPtr buffer);
  void setNumSscpErrors(short errors) { numSscpErrors_ = errors; }
...
class RtsQueryId : public RtsMessageObj {
 public:
  ...
  // Constructor for allocation on a heap
  //
  RtsQueryId(NAMemory *heap) : RtsMessageObj(RTS_QUERY_ID, currRtsQueryIdVersionNumber, heap) {
    queryId_ = NULL;
    queryIdLen_ = 0;
  ...
  RtsQueryId(NAMemory *heap, char *nodeName, short cpu, UInt16 statsMergeType = SQLCLI_SAME_STATS,
  ...
  // Constructor for copyless send
  //
  RtsQueryId(const RtsHandle &h) : RtsMessageObj(RTS_QUERY_ID, currRtsQueryIdVersionNumber, NULL) {}

  //
  // Constructor for copyless receive
  //
  RtsQueryId(IpcBufferedMsgStream *msgStream) : RtsMessageObj(msgStream) {}
...
class RtsExplainFrag : public RtsMessageObj {
 public:
  ...
  // Constructor
  //
  RtsExplainFrag() : RtsMessageObj(RTS_EXPLAIN_FRAG, currRtsExplainFragVersionNumber, NULL) {
    explainFrag_ = NULL;
    explainFragLen_ = 0;
    topNodeOffset_ = 0;
  }

  RtsExplainFrag(NAMemory *heap) : RtsMessageObj(RTS_EXPLAIN_FRAG, currRtsExplainFragVersionNumber, heap) {
    explainFrag_ = NULL;
    explainFragLen_ = 0;
    topNodeOffset_ = 0;
  }
  ...
  virtual ~RtsExplainFrag() {
    if (explainFrag_ != NULL) NADELETEBASIC((char *)explainFrag_, getHeap());
  }
...
class QueryStartedReply : public RtsMessageObj {
 public:
  QueryStartedReply(const RtsHandle &h, NAMemory *heap)
      : RtsMessageObj(CANCEL_QUERY_STARTED_REPLY, CurrQueryStartedReplyVersionNumber, heap),
        nextAction_(INVALID),
 ...
 private:
  enum NextActionType { INVALID, COMPLETE, CANCEL };
...
class QueryFinished : public RtsMessageObj {
 public:
  QueryFinished(const RtsHandle &h, NAMemory *heap)
      : RtsMessageObj(CANCEL_QUERY_FINISHED_REQ, CurrQueryFinishedReqVersionNumber, heap) {
    setHandle(h);
  }
...
class RmsGenericReply : public RtsMessageObj {
 public:
  RmsGenericReply(NAMemory *heap) : RtsMessageObj(IPC_MSG_RMS_REPLY, CurrRmsReplyMessageVersion, heap) {}
...
class CancelQueryRequest : public RtsMessageObj {
 public:
  ...
  virtual ~CancelQueryRequest() {
    if (comment_) NADELETEBASIC(comment_, getHeap());
  }
...
class ControlQueryReply : public RtsMessageObj {
 public:
  ControlQueryReply(const RtsHandle &h, NAMemory *heap)
      : RtsMessageObj(CONTROL_QUERY_REPLY, CurrControlQueryReplyVersionNumber, heap), didAttemptControl_(false) {
    setHandle(h);
  }

  ControlQueryReply(const RtsHandle &h, NAMemory *heap, bool didAttemptControl)
      : RtsMessageObj(CONTROL_QUERY_REPLY, CurrControlQueryReplyVersionNumber, heap),
...
class SuspendQueryRequest : public RtsMessageObj {
 public:
  ...
  SuspendQueryRequest(const RtsHandle &h, NAMemory *heap, bool forced, bool suspendLogging)
      : RtsMessageObj(SUSPEND_QUERY_REQ, CurrSuspendQueryReqVersionNumber, heap),
        forced_(forced),
...
class ActivateQueryRequest : public RtsMessageObj {
 public:
  ActivateQueryRequest(const RtsHandle &h, NAMemory *heap)
      : RtsMessageObj(ACTIVATE_QUERY_REQ, CurrActivateQueryReqVersionNumber, heap), suspendLogging_(false) {
    setHandle(h);
  }

  ActivateQueryRequest(const RtsHandle &h, NAMemory *heap, bool suspendLogging)
      : RtsMessageObj(ACTIVATE_QUERY_REQ, CurrActivateQueryReqVersionNumber, heap), suspendLogging_(suspendLogging) {
    setHandle(h);
  }
...
class CancelQueryKillServersReply : public RtsMessageObj {
 public:
  CancelQueryKillServersReply(const RtsHandle &h, NAMemory *heap)
      : RtsMessageObj(CANCEL_QUERY_KILL_SERVERS_REPLY, CurrKillServersReplyVersionNumber, heap) {
    setHandle(h);
  }
...
class SuspendActivateServersRequest : public RtsMessageObj {
 public:
  ...
  SuspendActivateServersRequest(const RtsHandle &h, NAMemory *heap, bool requestIsToSuspend, bool suspendLogging)
      : RtsMessageObj(SUSPEND_QUERY_REQ, CurrSuspendQueryReqVersionNumber, heap),
...
class SnapshotLockRequest : public RtsMessageObj {
 public:
  SnapshotLockRequest(NAMemory *heap) : RtsMessageObj(SNAPSHOT_LOCK_REQ, CurrSnapshotLockVersionNumber, heap) {}

  virtual ~SnapshotLockRequest(){};

  IpcMessageObjSize packedLength() { return baseClassPackedLength(); }

  IpcMessageObjSize packObjIntoMessage(IpcMessageBufferPtr buffer) { return packBaseClassIntoMessage(buffer); }
...
class SnapshotUnLockRequest : public RtsMessageObj {
 public:
  SnapshotUnLockRequest(NAMemory *heap) : RtsMessageObj(SNAPSHOT_UNLOCK_REQ, CurrSnapshotUnLockVersionNumber, heap) {}
...
class ObjectEpochChangeRequest : public RtsMessageObj {
 public:
  ...
  enum Flags { NO_DDL_IN_PROGRESS = 0, DDL_IN_PROGRESS = 0x00000001, READS_DISALLOWED = 0x00000002 };
...
class ObjectEpochChangeReply : public RtsMessageObj {
 public:
  ...
  ObjectEpochChangeReply(NAMemory *heap)
      : RtsMessageObj(OBJECT_EPOCH_CHANGE_REPLY, CurrObjectEpochChangeReplyVersionNumber, heap),
        result_(SUCCESS),
  ...
  ObjectEpochChangeReply(Result result, UInt32 maxExpectedEpochFound, UInt32 maxExpectedFlagsFound, NAMemory *heap)
      : RtsMessageObj(OBJECT_EPOCH_CHANGE_REPLY, CurrObjectEpochChangeReplyVersionNumber, heap),
        result_(result),
...
class ObjectEpochStatsReply : public RtsMessageObj {
 public:
  ObjectEpochStatsReply(NAMemory *heap)
      : RtsMessageObj(OBJECT_EPOCH_STATS_REPLY, CurrObjectEpochStatsReplyVersionNumber, heap) {}
 ...
 public:
  ...
  static const char *lockStateLit(LockState state) {
    const char *lockStates[] = {
        "LOCK_UNKNOWN", "LOCK_OK", "LOCK_CONFLICT_DML", "LOCK_CONFLICT_DDL", "LOCK_OUT_OF_ENTRY",
    };
...
class ObjectLockStatsReply : public RtsMessageObj {
 public:
  ObjectLockStatsReply(NAMemory *heap)
      : RtsMessageObj(OBJECT_LOCK_STATS_REPLY, CurrObjectLockStatsReplyVersionNumber, heap) {}
...
class QueryInvalidStatsRequest : public RtsMessageObj {
 public:
  QueryInvalidStatsRequest(NAMemory *heap)
      : RtsMessageObj(QUERY_INVALIDATION_STATS_REQ, CurrQueryInvalidationStatsRequestVersionNumber, heap) {}
...
class QueryInvalidStatsReply : public RtsMessageObj {
 public:
  QueryInvalidStatsReply(NAMemory *heap)
      : RtsMessageObj(QUERY_INVALIDATION_STATS_REPLY, CurrQueryInvalidationStatsReplyVersionNumber, heap) {}
