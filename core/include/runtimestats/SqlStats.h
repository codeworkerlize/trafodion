...
class StmtStats {
 public:
  ...
  NABoolean isMaster() { return (flags_ & IS_MASTER_) != 0; }

  void setMaster(NABoolean v) { (v ? flags_ |= IS_MASTER_ : flags_ &= ~IS_MASTER_); }

  NABoolean canbeGCed() { return (flags_ & CAN_BE_GCED_) != 0; }

  void setTobeGCed(NABoolean v = TRUE) { (v ? flags_ |= CAN_BE_GCED_ : flags_ &= ~CAN_BE_GCED_); }

  void setMergeReqd(NABoolean v) { (v ? flags_ |= FORCE_MERGE_ : flags_ &= ~FORCE_MERGE_); }

  NABoolean mergeReqd() { return (flags_ & FORCE_MERGE_) != 0; }
  ...
  NABoolean isStmtStatsUsed() { return (refCount_ > 0); }
  ...
  NABoolean isDeleteError() { return (flags_ & IS_DELETE_ERROR_) != 0; }

  void setDeleteError(NABoolean v) { (v ? flags_ |= IS_DELETE_ERROR_ : flags_ &= ~IS_DELETE_ERROR_); }
  NABoolean aqrInProgress() { return (flags_ & AQR_IN_PROGRESS_) != 0; }

  void setAqrInProgress(NABoolean v) { (v ? flags_ |= AQR_IN_PROGRESS_ : flags_ &= ~AQR_IN_PROGRESS_); }
  NABoolean isWMSMonitoredCliQuery() { return (flags_ & WMS_MONITORED_CLI_QUERY_) != 0; }

  void setWMSMonitoredCliQuery(NABoolean v) {
    (v ? flags_ |= WMS_MONITORED_CLI_QUERY_ : flags_ &= ~WMS_MONITORED_CLI_QUERY_);
  }

  NABoolean calledFromRemoveQuery() { return (flags_ & CALLED_FROM_REMOVE_QUERY) != 0; }
...
class LockHolder {
 public:
  ...
  bool operator==(const LockHolder &other) const { return (other.getNid() == nid_ && other.getPid() == pid_); }
...
class ObjectLockEntry {
 public:
  ObjectLockEntry(const char *objectName = NULL, ComObjectType objectType = COM_UNKNOWN_OBJECT)
      : objectType_(objectType), holder_(), inUse_(false) {
  ...
  bool locked() const { return (inUse_ && objectName_[0] != 0 && holder_.valid()); }
  bool lockedBy(const LockHolder &holder) { return locked() && holder == holder_; }
...
class ObjectLockArray {
 public:
  ObjectLockArray(unsigned short maxEntries, NAHeap *heap)
      : entries_(0), maxEntries_(maxEntries), heap_(heap), mutex_(true, true, true), locks_(NULL) {
    if (maxEntries > 0) {
  ...
  ObjectLockEntry *findLock(const char *objectName, ComObjectType objectType);
  ObjectLockEntry *addLock(const char *objectName, ComObjectType objectType, const LockHolder &holder);
  bool lockedBy(const char *objectName, ComObjectType objectType, const LockHolder &holder);
  bool hasConflictLock(const char *objectName, ComObjectType objectType, const LockHolder &holder) {
    LockHolder tmpHolder;
    return hasConflictLock(objectName, objectType, holder, tmpHolder);
  }
  bool hasConflictLock(const char *objectName, ComObjectType objectType, const LockHolder &holder,
                       LockHolder &conflictHolder);
  bool acquireLock(const char *objectName, ComObjectType objectType, const LockHolder &holder,
                   LockHolder &conflictHolder, bool &outOfEntry);
  void releaseLock(const char *objectName, ComObjectType objectType, const LockHolder &holder);
  ...
  void deepRecycle();
  void appendToStats(ExStatisticsArea *statsArea, short cpu, pid_t pid, const char *objectName,
...
class ObjectLockController {
 public:
  ...
  static const char *resultString(Result result) { return ObjectLockReply::lockStateLit(result); }

  ObjectLockController(const char *objectName, ComObjectType objectType)
      : objectName_(objectName), objectType_(objectType) {}

  Result lockDML(LockHolder &conflictHolder /* OUT */);
  Result lockDDL(const LockHolder &holder, LockHolder &conflictHolder /* OUT */);
  ...
  bool hasConflictDDL(const LockHolder &holder, LockHolder &conflictHolder /* OUT */);
  ...
  static void appendToStats(ExStatisticsArea *statsArea, const char *objectName, ComObjectType objectType);
...
class ProcessStats {
 public:
  ProcessStats(NAHeap *heap, short nid, pid_t pid, unsigned short dmlLockArraySize, unsigned short ddlLockArraySize);
  ~ProcessStats();
  NAHeap *getHeap() { return heap_; }
  ...
  void setQueryIdInfo(QueryIdInfo *queryIdInfo) {
    if (queryIdInfo_ != NULL) NADELETE(queryIdInfo_, QueryIdInfo, heap_);
    queryIdInfo_ = queryIdInfo;
  }

  QueryIdInfo *getQueryIdInfo() { return queryIdInfo_; }
...
class StatsGlobals {
 public:
  ...
  void setStatsArea(pid_t pid, ExStatisticsArea *stats) {
    if (statsArray_[pid].processStats_ != NULL) statsArray_[pid].processStats_->setStatsArea(stats);
  }
  ...
  short removeQuery(pid_t pid, StmtStats *stmtStats, NABoolean removeAlways = FALSE, NABoolean globalScan = FALSE,
  ...
  int getStatsSemaphore(Long &semId, pid_t pid, NABoolean calledByVerifyAndCleanup = FALSE);
  void releaseStatsSemaphore(Long &semId, pid_t pid);
  int forceReleaseAndGetStatsSemaphore(Long &semId, pid_t pid);

  int releaseAndGetStatsSemaphore(Long &semId, pid_t pid, pid_t releasePid);
  ...
  void setSscpInitialized(NABoolean toggle) { isSscpInitialized_ = toggle; }
  NABoolean IsSscpInitialized() { return (isSscpInitialized_ == TRUE); }
  enum RTSEnvType { RTS_GLOBAL_ENV = 1, RTS_PRIVATE_ENV = 0 };
...
class ObjectEpochCacheKey {
 public:
  ...
  ObjectEpochCacheKey(const ObjectEpochCacheKey &key, NAHeap *heap = 0)
      : objectName_(key.objectName(), heap), hash_(key.hash()), objectType_(key.objectType()) {}
  ~ObjectEpochCacheKey() {}
...
class ObjectEpochCacheEntry {
 public:
  ObjectEpochCacheEntry(UInt32 epoch, UInt32 flags) : epoch_(epoch), flags_(flags) {}
  ...
  bool isLocked() { return flags_ & ObjectEpochChangeRequest::DDL_IN_PROGRESS; }

  // NOTE2ME: Required by NAHashDictionaryIterator, more precisely
  // NAHashBucket<K,V>::getKeyValuePair method, when param `value` is
  // also used.
  NABoolean operator==(const ObjectEpochCacheEntry &other) const { return (this == &other); }
...
class ObjectEpochCache {
 ...
 public:
  ...
  ObjectEpochCache(StatsGlobals *statsGlobals, NAHeap *heap);
  ~ObjectEpochCache(){};

  short createEpochEntry(ObjectEpochCacheEntry *&entry, const ObjectEpochCacheKey &key, UInt32 epoch, UInt32 flags);
  short getEpochEntry(ObjectEpochCacheEntry *&entry, const ObjectEpochCacheKey &key);
  short updateEpochEntry(ObjectEpochCacheEntry *&entry, const ObjectEpochCacheKey &key, UInt32 newEpoch,
                         UInt32 newFlags);
  ...
  void appendToStats(ExStatisticsArea *statsArea, const char *objectName, bool locked);
...
class ObjectKey {
 public:
  ObjectKey(const char *objectName, ComObjectType objectType, NAHeap *heap = 0) {
  ...
  ObjectKey(const ObjectKey &key, NAHeap *heap = 0) : hash_(key.hash()) {
  ...
  NABoolean operator==(const ObjectKey &other) const {
    return (hash_ == other.hash()) && (!strncmp(keyName_, other.keyName(), sizeof(keyName_)));
  }
...
class ObjectLockPointer {
 public:
  ObjectLockPointer(const char *objectName, ComObjectType objectType) : lock_(objectName, objectType) {}
  ~ObjectLockPointer() {}
