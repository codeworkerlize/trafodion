...
typedef int SQLCTX_HANDLE;

// #if !defined(_MSC_VER) && !defined(long long )
// /* long long  is an MSVC builtin definition, but may be used elsewhere */
...
#define LOB_V1_EXT_HANDLE_LEN 1024
#define LOB_V2_EXT_HANDLE_LEN 96
...
struct SQLMODULE_ID_t {
  ...
  /* of the module name. */
  int module_name_len; /* length of the module name (in bytes) */
};
...
enum SQLWHAT_DESC { SQLWHAT_INPUT_DESC = 1, SQLWHAT_OUTPUT_DESC = 2 };
...
enum SQLDATETIME_CODE { SQLDTCODE_DATE = 1, SQLDTCODE_TIME = 2, SQLDTCODE_TIMESTAMP = 3, SQLDTCODE_MPDATETIME = 4 };
/* #endif */
...
typedef struct {
  int item_id;                /* item id (SQLDIAG_COND_INFO_ITEM_ID) */
  int cond_number_desc_entry; /* cond. num's input descriptor entry num   */
...
typedef struct {
  long long compileStartTime;
  long long compileEndTime;
  char compilerId[COMPILER_ID_LEN];
  long long cmpCpuTotal;
  long long cmpCpuBinder;
  long long cmpCpuNormalizer;
  long long cmpCpuAnalyzer;
  long long cmpCpuOptimizer;
  long long cmpCpuGenerator;
  UInt32 metadataCacheHits;
  UInt32 metadataCacheLookups;
  int queryCacheState;
  UInt32 histogramCacheHits;
  UInt32 histogramCacheLookups;
  long long stmtHeapSize;
  long long cxtHeapSize;
  int optTasks;
  int optContexts;
  short isRecompile;
...
typedef struct {
  ...
  int dop; /* degree of parallel execution */
  short xnNeeded;
...
typedef struct {
  long long prepareTime;
  long long fixupTime;
  long long freeupTime;
  long long returnedRowsIOTime;
...
enum SQLATTRCURSOR_TYPE { SQL_READONLY_CURSOR = 0, SQL_UPDATABLE_CURSOR = 1 };
...
enum SQLATTR_TYPE {
  ...
  SQL_ATTR_SYNC_QUERY_FOR_BINGLOG = -18
};
...
enum SESSIONATTR_TYPE {
  ...
  SESSION_TENANT_NAME = 8,
  SESSION_TENANT_ID = 9,
  SESSION_TENANT_BY_NAME = 10,  // this does a lookup of tenants table
  SESSION_TENANT_NODES = 11,
  SESSION_TENANT_DEFAULT_SCHEMA = 12
};
...
enum SQLATTRATOMIC_TYPE { SQL_NOT_SPECIFIED = 0, SQL_ATOMIC = 1, SQL_NOT_ATOMIC = 2 };
...
enum SQLSTATS_TYPE {
  ...
  SQLCLI_OBJECT_EPOCH_STATS = 15,
  SQLCLI_OBJECT_LOCK_STATS = 16,
  SQLCLI_QUERY_INVALIDATION_STATS = 17
};
...
enum UNIQUEQUERYID_ATTR_TYPE {
  ...
  /* null terminated user name: max 15 bytes              */
  ...
  /* null terminated tenant id :          max 15 bytes */
  UNIQUEQUERYID_TENANTID = 11
};
...
enum STATS_REQ_TYPE {
  ...
  SQLCLI_STATS_REQ_OEC_INFO = 13,
  SQLCLI_STATS_REQ_OBJECT_EPOCH = 14,
};
...
enum SQLSTATS_DESC_STATS_TYPE {
  ...
  SQLSTATS_DESC_OBJECT_EPOCH_STATS = 26,
  SQLSTATS_DESC_OBJECT_LOCK_STATS = 27,
  SQLSTATS_DESC_QUERY_INVALIDATION_STATS = 28
};
...
enum SQLSTATS_ITEM_ID {
  ...
  SQLSTATS_SLA_NAME = 208,
  SQLSTATS_PROFILE_NAME = 209,
  ...
  SQLSTATS_ACT_DATA_USED = 48,
  SQLSTATS_ACT_ROWS_FILTERED = 49,
  ...
  SQLSTATS_EPOCH_HEAP_ALLOC = 134,
  SQLSTATS_EPOCH_HEAP_USED = 135,
  SQLSTATS_EPOCH_HEAP_HIGH_WM = 136,
  SQLSTATS_EPOCH_ENTRIES = 137,
...
enum PREPARE_FLAGS {
  ...
  /* set when we need an Apache Sentry priv re-check */
  PREPARE_SENTRY_PRIV_RECHECK = 0x0200,

  /* set if recomp is caused by update statistics */
  PREPARE_RECOMP_USE_NATABLE_CACHE = 0x0400
};
...
/* SQLSEC_GroupInfo describes attributes for groups assigned to a tenant      */
/* It is used by authentication code to verify that the user is assigned      */
/* one of the tenants groups.                                                 */
/*   group_names - name of the group                                          */
/*   group_id - id assigned by EsgynDB                                        */
/*   group_config_num - num associated with LDAP section where group defined  */
struct SQLSEC_GroupInfo {
  char *group_name;
  int group_id;
  int group_config_num;

  SQLSEC_GroupInfo() : group_name(NULL), group_id(0), group_config_num(0){};

  ~SQLSEC_GroupInfo() {
    if (group_name) delete group_name;
  }
};

/* SQLSEC_AuthDetails describes attributes used by authentication code to     */
/* verify that the user is able to connect to EsgynDB.                        */
/*   auth_id: identifies the user                                             */
/*   auth_is_valid: indicates if the user is OFFLINE                          */
/*   tenant_id: identifies the tenant                                         */
/*   tenant_is_valid: indicates if the tenant is OFFLINE                      */
/*   tenant_group_check: flag telling auth code to verify tenant group usage  */
/*   num_groups & group_list: returns groups assigned to the tenant           */
/*      setup if auth code checks tenant group usage                          */
/*   num_tenants: returns the number of tenants defined in the metadata       */
/*      set up if the tenant is system tenant DB__SYSTEMTENANT                */
struct SQLSEC_AuthDetails {
  int auth_id;
  bool auth_is_valid;
  int tenant_id;
  bool tenant_is_valid;
  bool tenant_group_check;
  int num_groups;
  SQLSEC_GroupInfo *group_list;
  int num_tenants;
...
int SQL_EXEC_AllocDescBasic(
    /*INOUT*/ SQLDESC_ID *desc_id,
    /*IN OPTIONAL*/ int max_entries);

int SQL_EXEC_AssocFileNumber(
...
int SQL_EXEC_ASSOCFILENUMBER(
    /*IN*/ SQLSTMT_ID *statement_id,
    /*IN*/ short file_number);
int SQL_EXEC_GetDiskMaxSize(
    /*IN*/ char *volname,
    /*OUT*/ long *totalCapacity,
    /*OUT*/ long *totalFreespace);
int SQL_EXEC_GetListOfDisks(
    /*IN/OUT*/ char *diskBuf,
    /* OUT */ int *numTSEs,
    /* OUT */ int *maxTSELength,
    /* IN/OUT */ int *diskBufLength);
...
int SQL_EXEC_ExecDirectDealloc(
    /*IN*/ SQLSTMT_ID *statement_id,
    /*IN*/ SQLDESC_ID *sql_source,
    /*IN OPTIONAL*/ SQLDESC_ID *input_descriptor,
    /*IN*/ int num_ptr_pairs,
...
int SQL_EXEC_ClearExecFetchClose(
    /*IN*/ SQLSTMT_ID *statement_id,
    /*IN OPTIONAL*/ SQLDESC_ID *input_descriptor,
    /*IN OPTIONAL*/ SQLDESC_ID *output_descriptor,
...
int SQL_EXEC_GetDescEntryCountBasic(
    /*IN*/ SQLDESC_ID *sql_descriptor,
    /*OUT*/ int *num_entries);
...
int SQL_EXEC_SetDescEntryCountBasic(
    /*IN*/ SQLDESC_ID *sql_descriptor,
    /*IN*/ int num_entries);
...
/*
  handleStr:        lob handle string returned by sql to caller
  handleLen:        length of lob handle string
  lobV2:            returns 1, if V2 lob. 0, otherwise
  objectUID:        returns object uid
