...
class SQLMXLoggingArea {
 public:
  ...
  static void logSQLMXAbortEvent(const char *filename, int lineno, const char *msg);

  static void logSQLMXAssertionFailureEvent(const char *filename, int lineno, const char *msg,
                                            const char *condition = NULL, const int *tid = NULL,
                                            const char *stackTrace = NULL);

  // TBD

  static void logSortDiskInfo(const char *diskname, short percentfreespace, short diskerror);

  static void logSQLMXEventForError(int eventId, const char *msgtxt, const char *sqlId = NULL,
                                    NABoolean isWarning = FALSE);

  // generate an EMS event for executor runtime informational message
  static void logExecRtInfo(const char *fileName, int lineNo, const char *msg, int explainSeqNum);

  // generate an EMS event for executor runtime debug message
  static void logExecRtDebug(const char *fileName, int lineNo, const char *msg, int explainSeqNum);

  // generate an event for privilege manager tree traversal
  static void logPrivMgrInfo(const char *filename, int lineNo, const char *msg, int level);

  static void logSQLMXDebugEvent(const char *msg, short errorcode, int lineno);
  // events that correspond to messages generated by CommonLogger or its subclasses.
  static void logCommonLoggerInfoEvent(int eventId, const char *msg);
  static void logCommonLoggerErrorEvent(int eventId, const char *msg);
  static void logCommonLoggerFailureEvent(int eventId, const char *msg);
