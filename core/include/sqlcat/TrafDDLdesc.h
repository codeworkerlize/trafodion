...
enum ConstraintType { UNIQUE_CONSTRAINT, PRIMARY_KEY_CONSTRAINT, REF_CONSTRAINT, CHECK_CONSTRAINT };
...
class TrafDesc : public NAVersionedObject {
 public:
  ...
  void setWasCopied(NABoolean v) { (v ? descFlags |= WAS_COPIED : descFlags &= ~WAS_COPIED); };
  ...
  virtual NABoolean operator==(const TrafDesc &other) const {
    ...
    return (nodetype == other.nodetype && version == other.version && descFlags == other.descFlags &&
...
class TrafCheckConstrntsDesc : public TrafDesc {
 public:
  ...
  NABoolean operator==(const TrafCheckConstrntsDesc &other) const {
    return (TrafDesc::operator==(other) && COMPARE_CHAR_PTRS(constrnt_text, other.constrnt_text));
  }
...
class TrafColumnsDesc : public TrafDesc {
 public:
  ...
  void setNullable(NABoolean v) { (v ? columnsDescFlags |= NULLABLE : columnsDescFlags &= ~NULLABLE); };
  NABoolean isNullable() { return (columnsDescFlags & NULLABLE) != 0; };

  void setAdded(NABoolean v) { (v ? columnsDescFlags |= ADDED : columnsDescFlags &= ~ADDED); };
  NABoolean isAdded() { return (columnsDescFlags & ADDED) != 0; };

  void setUpshifted(NABoolean v) { (v ? columnsDescFlags |= UPSHIFTED : columnsDescFlags &= ~UPSHIFTED); };
  ...
  void setOptional(NABoolean v) { (v ? columnsDescFlags |= OPTIONAL : columnsDescFlags &= ~OPTIONAL); };
  NABoolean isOptional() { return (columnsDescFlags & OPTIONAL) != 0; };

  rec_datetime_field datetimeStart() { return (rec_datetime_field)datetimestart; }
  rec_datetime_field datetimeEnd() { return (rec_datetime_field)datetimeend; }

  ComColumnDefaultClass defaultClass() { return (ComColumnDefaultClass)defaultClass_; }
  void setDefaultClass(ComColumnDefaultClass v) { defaultClass_ = (Int16)v; }

  CharInfo::CharSet characterSet() { return (CharInfo::CharSet)character_set; }
  CharInfo::CharSet encodingCharset() { return (CharInfo::CharSet)encoding_charset; }
  CharInfo::Collation collationSequence() { return (CharInfo::Collation)collation_sequence; }
  ...
  NABoolean operator==(const TrafColumnsDesc &other) const {
    return (TrafDesc::operator==(other) && COMPARE_CHAR_PTRS(colname, other.colname) && colnumber == other.colnumber &&
            datatype == other.datatype && offset == other.offset && length == other.length && scale == other.scale &&
            precision == other.precision && datetimestart == other.datetimestart && datetimeend == other.datetimeend &&
            datetimefractprec == other.datetimefractprec && intervalleadingprec == other.intervalleadingprec &&

            defaultClass_ == other.defaultClass_ && character_set == other.character_set &&
            encoding_charset == other.encoding_charset && collation_sequence == other.collation_sequence &&

            hbaseColFlags == other.hbaseColFlags && paramDirection_ == other.paramDirection_ &&
            colclass == other.colclass && colFlags == other.colFlags && columnsDescFlags == other.columnsDescFlags &&

            COMPARE_CHAR_PTRS(pictureText, other.pictureText) && COMPARE_CHAR_PTRS(defaultvalue, other.defaultvalue) &&
            COMPARE_CHAR_PTRS(initDefaultValue, other.initDefaultValue) && COMPARE_CHAR_PTRS(heading, other.heading) &&
            COMPARE_CHAR_PTRS(computed_column_text, other.computed_column_text) &&
            COMPARE_CHAR_PTRS(hbaseColFam, other.hbaseColFam) && COMPARE_CHAR_PTRS(hbaseColQual, other.hbaseColQual) &&
...
class TrafConstrntKeyColsDesc : public TrafDesc {
 public:
  enum ConsrntKeyDescFlags { SYSTEM_KEY = 0x0001 };
  // why almost no initializers? see note at top of file
  TrafConstrntKeyColsDesc() : TrafDesc(DESC_CONSTRNT_KEY_COLS_TYPE) { constrntKeyColsDescFlags = 0; }
  ...
  NABoolean operator==(const TrafConstrntKeyColsDesc &other) const {
    return (TrafDesc::operator==(other) && COMPARE_CHAR_PTRS(colname, other.colname) && position == other.position &&
...
class TrafConstrntsDesc : public TrafDesc {
 public:
  ...
  enum ConstrntsDescFlags { ENFORCED = 0x0001, NOT_SERIALIZED = 0x0002 };

  void setEnforced(NABoolean v) { (v ? constrntsDescFlags |= ENFORCED : constrntsDescFlags &= ~ENFORCED); };
  ...
  NABoolean operator==(const TrafConstrntsDesc &other) const {
    return (TrafDesc::operator==(other) && COMPARE_CHAR_PTRS(constrntname, other.constrntname) &&
            COMPARE_CHAR_PTRS(tablename, other.tablename) && type == other.type && colcount == other.colcount &&
...
class TrafFilesDesc : public TrafDesc {
 public:
  ...
  enum FilesDescFlags { AUDITED = 0x0001 };

  void setAudited(NABoolean v) { (v ? filesDescFlags |= AUDITED : filesDescFlags &= ~AUDITED); };
  NABoolean isAudited() { return (filesDescFlags & AUDITED) != 0; };

  NABoolean operator==(const TrafFilesDesc &other) const {
    return (TrafDesc::operator==(other) && filesDescFlags == other.filesDescFlags &&
...
class TrafHbaseRegionDesc : public TrafDesc {
 public:
  ...
  NABoolean operator==(const TrafHbaseRegionDesc &other) const {
    return (TrafDesc::operator==(other) && hbaseRegionDescFlags == other.hbaseRegionDescFlags &&
            COMPARE_VOID_PTRS(beginKey, beginKeyLen, other.beginKey, other.beginKeyLen) &&
...
class TrafHistogramDesc : public TrafDesc {
 public:
  ...
  NABoolean operator==(const TrafHistogramDesc &other) const {
    return (TrafDesc::operator==(other) && COMPARE_CHAR_PTRS(tablename, other.tablename) &&
            table_uid == other.table_uid && histogram_id == other.histogram_id && col_position == other.col_position &&
            column_number == other.column_number && colcount == other.colcount &&
            interval_count == other.interval_count && rowcount == other.rowcount && total_uec == other.total_uec &&
            COMPARE_CHAR_PTRS(low_value, other.low_value) && COMPARE_CHAR_PTRS(high_value, other.high_value) &&
            read_time == other.read_time && sample_secs == other.sample_secs && col_secs == other.col_secs &&
            read_count == other.read_count && sample_percent == other.sample_percent && reason == other.reason &&
            cv == other.cv && v1 == other.v1 && v2 == other.v2 && COMPARE_CHAR_PTRS(v5, other.v5) &&
...
class TrafHistIntervalDesc : public TrafDesc {
 public:
  ...
  NABoolean operator==(const TrafHistIntervalDesc &other) const {
    return (TrafDesc::operator==(other) && histogram_id == other.histogram_id &&
            interval_number == other.interval_number && interval_rowcount == other.interval_rowcount &&
            interval_uec == other.interval_uec && COMPARE_CHAR_PTRS(interval_boundary, other.interval_boundary) &&
            std_dev_of_freq == other.std_dev_of_freq && v1 == other.v1 && v2 == other.v2 &&
...
class TrafIndexesDesc : public TrafDesc {
 public:
  ...
  void setExplicit(NABoolean v) { (v ? indexesDescFlags |= EXPLICIT : indexesDescFlags &= ~EXPLICIT); };
  NABoolean isExplicit() { return (indexesDescFlags & EXPLICIT) != 0; };

  void setVolatile(NABoolean v) { (v ? indexesDescFlags |= VOLATILE : indexesDescFlags &= ~VOLATILE); };
  NABoolean isVolatile() { return (indexesDescFlags & VOLATILE) != 0; };

  void setInMemoryObject(NABoolean v) { (v ? indexesDescFlags |= IN_MEM_OBJ : indexesDescFlags &= ~IN_MEM_OBJ); };
  NABoolean isInMemoryObject() { return (indexesDescFlags & IN_MEM_OBJ) != 0; };

  void setUnique(NABoolean v) { (v ? indexesDescFlags |= UNIQUE : indexesDescFlags &= ~UNIQUE); };
  NABoolean isUnique() { return (indexesDescFlags & UNIQUE) != 0; };

  // for ngram
  void setNgram(NABoolean v) { (v ? indexesDescFlags |= NGRAM : indexesDescFlags &= ~NGRAM); };
  ...
  void setPartLocalIndex(NABoolean v) { (v ? indexesDescFlags |= PART_LOCAL : indexesDescFlags &= ~PART_LOCAL); };
  NABoolean isPartLocalIndex() { return (indexesDescFlags & PART_LOCAL) != 0; };

  void setPartGlobalIndex(NABoolean v) { (v ? indexesDescFlags |= PART_GLOBAL : indexesDescFlags &= ~PART_GLOBAL); };
  NABoolean isPartGlobalIndex() { return (indexesDescFlags & PART_GLOBAL) != 0; };

  ComPartitioningScheme partitioningScheme() { return (ComPartitioningScheme)partitioningScheme_; }
  void setPartitioningScheme(ComPartitioningScheme v) { partitioningScheme_ = (Int16)v; }
  ...
  NABoolean operator==(const TrafIndexesDesc &other) const {
    return (TrafDesc::operator==(other) && COMPARE_CHAR_PTRS(tablename, other.tablename) &&
            COMPARE_CHAR_PTRS(indexname, other.indexname) && indexUID == other.indexUID && keytag == other.keytag &&
            record_length == other.record_length && colcount == other.colcount && blocksize == other.blocksize &&
            partitioningScheme_ == other.partitioningScheme_ && rowFormat_ == other.rowFormat_ &&
            numInitialSaltRegions == other.numInitialSaltRegions && numReplicas == other.numReplicas &&
            indexesDescFlags == other.indexesDescFlags && COMPARE_CHAR_PTRS(hbaseSplitClause, other.hbaseSplitClause) &&
...
class TrafKeysDesc : public TrafDesc {
 public:
  ...
  enum KeysDescFlags { DESCENDING = 0x0001, ADDNL_COL = 0x0002 };

  void setDescending(NABoolean v) { (v ? keysDescFlags |= DESCENDING : keysDescFlags &= ~DESCENDING); };
  NABoolean isDescending() { return (keysDescFlags & DESCENDING) != 0; };

  void setAddnlCol(NABoolean v) { (v ? keysDescFlags |= ADDNL_COL : keysDescFlags &= ~ADDNL_COL); };
  NABoolean isAddnlCol() { return (keysDescFlags & ADDNL_COL) != 0; };

  NABoolean operator==(const TrafKeysDesc &other) const {
    return (TrafDesc::operator==(other) && COMPARE_CHAR_PTRS(keyname, other.keyname) &&
            keyseqnumber == other.keyseqnumber && tablecolnumber == other.tablecolnumber &&
            keysDescFlags == other.keysDescFlags && COMPARE_CHAR_PTRS(hbaseColFam, other.hbaseColFam) &&
...
class TrafLibraryDesc : public TrafDesc {
 public:
  ...
  NABoolean operator==(const TrafLibraryDesc &other) const {
    return (TrafDesc::operator==(other) && COMPARE_CHAR_PTRS(libraryName, other.libraryName) &&
            COMPARE_CHAR_PTRS(libraryFilename, other.libraryFilename) && libraryUID == other.libraryUID &&
            libraryVersion == other.libraryVersion && libraryOwnerID == other.libraryOwnerID &&
...
class TrafPartnsDesc : public TrafDesc {
 public:
  ...
  NABoolean operator==(const TrafPartnsDesc &other) const {
    return (TrafDesc::operator==(other) && COMPARE_CHAR_PTRS(tablename, other.tablename) &&
            primarypartition == other.primarypartition && COMPARE_CHAR_PTRS(partitionname, other.partitionname) &&
            COMPARE_CHAR_PTRS(logicalpartitionname, other.logicalpartitionname) &&
            COMPARE_VOID_PTRS(firstkey, firstkeylen, other.firstkey, other.firstkeylen) &&
            COMPARE_VOID_PTRS(encodedkey, encodedkeylen, other.encodedkey, other.encodedkeylen) &&
            COMPARE_CHAR_PTRS(lowKey, other.lowKey) && COMPARE_CHAR_PTRS(highKey, other.highKey) &&
            indexlevel == other.indexlevel && priExt == other.priExt && secExt == other.secExt &&
            maxExt == other.maxExt && COMPARE_CHAR_PTRS(givenname, other.givenname));
  }
...
class TrafRefConstrntsDesc : public TrafDesc {
 public:
  ...
  NABoolean operator==(const TrafRefConstrntsDesc &other) const {
    return (TrafDesc::operator==(other) && refConstrntsDescFlags == other.refConstrntsDescFlags &&
...
class TrafRoutineDesc : public TrafDesc {
 public:
  ...
  NABoolean operator==(const TrafRoutineDesc &other) const {
    return (TrafDesc::operator==(other) && objectUID == other.objectUID &&
            COMPARE_CHAR_PTRS(routineName, other.routineName) && COMPARE_CHAR_PTRS(externalName, other.externalName) &&
            COMPARE_CHAR_PTRS(librarySqlName, other.librarySqlName) &&
            COMPARE_CHAR_PTRS(libraryFileName, other.libraryFileName) &&
            COMPARE_CHAR_PTRS(signature, other.signature) && paramsCount == other.paramsCount &&
            COMPARE_DESC_SMART_PTR(params, other.params) && language == other.language && UDRType == other.UDRType &&
            sqlAccess == other.sqlAccess && transactionAttributes == other.transactionAttributes &&
            maxResults == other.maxResults && paramStyle == other.paramStyle &&
            isDeterministic == other.isDeterministic && isCallOnNull == other.isCallOnNull &&
            isIsolate == other.isIsolate && externalSecurity == other.externalSecurity &&
            executionMode == other.executionMode && stateAreaSize == other.stateAreaSize &&
            parallelism == other.parallelism && owner == other.owner && schemaOwner == other.schemaOwner &&
            COMPARE_DESC_SMART_PTR(priv_desc, other.priv_desc) && routineDescFlags == other.routineDescFlags &&
            libRedefTime == other.libRedefTime && COMPARE_CHAR_PTRS(libBlobHandle, other.libBlobHandle) &&
            COMPARE_CHAR_PTRS(libSchName, other.libSchName) && libVersion == other.libVersion &&
...
class TrafSequenceGeneratorDesc : public TrafDesc {
 public:
  ...
  ComSequenceGeneratorType sgType() { return (ComSequenceGeneratorType)sgType_; }
  void setSgType(ComSequenceGeneratorType v) { sgType_ = (Int16)v; }
  ...
  NABoolean operator==(const TrafSequenceGeneratorDesc &other) const {
    return (TrafDesc::operator==(other) && startValue == other.startValue && increment == other.increment &&
            sgType_ == other.sgType_ && sqlDataType == other.sqlDataType && fsDataType == other.fsDataType &&
            cycleOption == other.cycleOption && maxValue == other.maxValue && minValue == other.minValue &&
            cache == other.cache && objectUID == other.objectUID && sgLocation == other.sgLocation &&
            nextValue == other.nextValue && redefTime == other.redefTime && seqOrder == other.seqOrder &&
...
class TrafTableDesc : public TrafDesc {
 public:
  ...
  void setUMDTable(NABoolean v) { (v ? tableDescFlags |= UMD_TABLE : tableDescFlags &= ~UMD_TABLE); };
  NABoolean isUMDTable() { return (tableDescFlags & UMD_TABLE) != 0; };

  void setMVTable(NABoolean v) { (v ? tableDescFlags |= MV_TABLE : tableDescFlags &= ~MV_TABLE); };
  NABoolean isMVTable() { return (tableDescFlags & MV_TABLE) != 0; };

  void setIUDLog(NABoolean v) { (v ? tableDescFlags |= IUD_LOG : tableDescFlags &= ~IUD_LOG); };
  NABoolean isIUDLog() { return (tableDescFlags & IUD_LOG) != 0; };

  void setMVMetadataObject(NABoolean v) { (v ? tableDescFlags |= MV_MD_OBJECT : tableDescFlags &= ~MV_MD_OBJECT); };
  ...
  void setVolatileTable(NABoolean v) { (v ? tableDescFlags |= VOLATILE : tableDescFlags &= ~VOLATILE); };
  NABoolean isVolatileTable() { return (tableDescFlags & VOLATILE) != 0; };

  void setInMemoryObject(NABoolean v) { (v ? tableDescFlags |= IN_MEM_OBJ : tableDescFlags &= ~IN_MEM_OBJ); };
  NABoolean isInMemoryObject() { return (tableDescFlags & IN_MEM_OBJ) != 0; };

  void setDroppable(NABoolean v) { (v ? tableDescFlags |= DROPPABLE : tableDescFlags &= ~DROPPABLE); };
  NABoolean isDroppable() { return (tableDescFlags & DROPPABLE) != 0; };

  void setInsertOnly(NABoolean v) { (v ? tableDescFlags |= INSERT_ONLY : tableDescFlags &= ~INSERT_ONLY); };
  NABoolean isInsertOnly() { return (tableDescFlags & INSERT_ONLY) != 0; };

  void setXnReplSync(NABoolean v) { (v ? tableDescFlags |= XN_REPL_SYNC : tableDescFlags &= ~XN_REPL_SYNC); };
  NABoolean isXnReplSync() { return (tableDescFlags & XN_REPL_SYNC) != 0; };

  void setXnReplAsync(NABoolean v) { (v ? tableDescFlags |= XN_REPL_ASYNC : tableDescFlags &= ~XN_REPL_ASYNC); };
  ...
  NABoolean operator==(const TrafTableDesc &other) const {
    return (
        TrafDesc::operator==(other) && COMPARE_CHAR_PTRS(tablename, other.tablename) && redefTime == other.redefTime &&
        baseTableUID == other.baseTableUID && mvAttributesBitmap == other.mvAttributesBitmap &&
        record_length == other.record_length && colcount == other.colcount && constr_count == other.constr_count &&
        insertMode_ == other.insertMode_ && partitioningScheme_ == other.partitioningScheme_ &&
        rowFormat_ == other.rowFormat_ && objectType_ == other.objectType_ && storageType_ == other.storageType_ &&
        numLOBdatafiles_ == other.numLOBdatafiles_ && catUID == other.catUID && schemaUID == other.schemaUID &&
        objectUID == other.objectUID && objDataUID == other.objDataUID && owner == other.owner &&
        schemaOwner == other.schemaOwner && COMPARE_CHAR_PTRS(default_col_fam, other.default_col_fam) &&
        COMPARE_CHAR_PTRS(all_col_fams, other.all_col_fams) && objectFlags == other.objectFlags &&
        tablesFlags == other.tablesFlags && tableDescFlags == other.tableDescFlags &&
        ...
        COMPARE_DESC_SMART_PTR(files_desc, other.files_desc) && COMPARE_DESC_SMART_PTR(priv_desc, other.priv_desc) &&
        COMPARE_DESC_SMART_PTR(hbase_regionkey_desc, other.hbase_regionkey_desc) &&
...
class TrafTableStatsDesc : public TrafDesc {
 public:
  ...
  enum TableStatsDescFlags { FAST_STATS = 0x0001 };

  void setFastStats(NABoolean v) { (v ? tableStatsDescFlags |= FAST_STATS : tableStatsDescFlags &= ~FAST_STATS); };
  NABoolean isFastStats() { return (tableStatsDescFlags & FAST_STATS) != 0; };

  NABoolean operator==(const TrafTableStatsDesc &other) const {
    return (TrafDesc::operator==(other) && rowcount == other.rowcount && hbtBlockSize == other.hbtBlockSize &&
            hbtIndexLevels == other.hbtIndexLevels && numHistograms == other.numHistograms &&
            numHistograms == other.numHistograms && COMPARE_DESC_SMART_PTR(histograms_desc, other.histograms_desc) &&
            COMPARE_DESC_SMART_PTR(hist_interval_desc, other.hist_interval_desc) &&
            COMPARE_TRAFDESC_PTRS(currHistogramDesc, other.currHistogramDesc) &&
            COMPARE_TRAFDESC_PTRS(currHistintDesc, other.currHistintDesc));
  }
  ...
  short fetchHistogramCursor(void *histogram_id, void *column_number, void *colcount, void *interval_count,
                             void *rowcount, void *total_uec, void *stats_time, void *low_value, void *high_value,
                             void *read_time, void *read_count, void *sample_secs, void *col_secs, void *sample_percent,
                             void *cv, void *reason, void *v1, void *v2,
                             // v5, expression text
                             void *v5 = NULL);
  short fetchHistintCursor(void *histogram_id, void *interval_number, void *interval_rowcount, void *interval_uec,
                           void *interval_boundary, void *std_dev_of_freq, void *v1, void *v2, void *v5);
...
class TrafUsingMvDesc : public TrafDesc {
 public:
  ...
  void setRefreshType(ComMVRefreshType v) { refreshType_ = (Int16)v; }

  NABoolean operator==(const TrafUsingMvDesc &other) const {
    return (TrafDesc::operator==(other) && COMPARE_CHAR_PTRS(mvName, other.mvName) &&
            rewriteEnabled == other.rewriteEnabled && isInitialized == other.isInitialized &&
...
class TrafViewDesc : public TrafDesc {
 public:
  ...
  enum ViewDescFlags { UPDATABLE = 0x0001, INSERTABLE = 0x0002 };

  void setUpdatable(NABoolean v) { (v ? viewDescFlags |= UPDATABLE : viewDescFlags &= ~UPDATABLE); };
  NABoolean isUpdatable() { return (viewDescFlags & UPDATABLE) != 0; };

  void setInsertable(NABoolean v) { (v ? viewDescFlags |= INSERTABLE : viewDescFlags &= ~INSERTABLE); };
  NABoolean isInsertable() { return (viewDescFlags & INSERTABLE) != 0; };

  NABoolean operator==(const TrafViewDesc &other) const {
    return (TrafDesc::operator==(other) && COMPARE_CHAR_PTRS(viewname, other.viewname) &&
            COMPARE_CHAR_PTRS(viewfilename, other.viewfilename) && COMPARE_CHAR_PTRS(viewtext, other.viewtext) &&
            COMPARE_CHAR_PTRS(viewchecktext, other.viewchecktext) &&
            COMPARE_CHAR_PTRS(viewcolusages, other.viewcolusages) && viewDescFlags == other.viewDescFlags &&
...
class TrafPrivDesc : public TrafDesc {
 public:
  ...
  NABoolean operator==(const TrafPrivDesc &other) const {
    return (TrafDesc::operator==(other) && COMPARE_DESC_SMART_PTR(privGrantees, other.privGrantees));
  }
...
class TrafPrivGranteeDesc : public TrafDesc {
 public:
  ...
  NABoolean operator==(const TrafPrivGranteeDesc &other) const {
    return (TrafDesc::operator==(other) && grantee == other.grantee && schemaUID == other.schemaUID &&
...
class TrafPrivBitmapDesc : public TrafDesc {
 public:
  ...
  NABoolean operator==(const TrafPrivBitmapDesc &other) const {
    return (TrafDesc::operator==(other) && columnOrdinal == other.columnOrdinal && privBitmap == other.privBitmap &&
...
class TrafPartDesc : public TrafDesc {
 public:
  ...
  NABoolean operator==(const TrafPartDesc &other) const {
    ...
    return (parentUid == other.parentUid && partitionUid == other.partitionUid &&
            COMPARE_CHAR_PTRS(partitionName, other.partitionName) &&
            COMPARE_CHAR_PTRS(partitionEntityName, other.partitionEntityName) &&
            isSubPartition == other.isSubPartition && hasSubPartition == other.hasSubPartition &&
            partPosition == other.partPosition && COMPARE_CHAR_PTRS(partitionValueExpr, other.partitionValueExpr) &&
            partitionValueExprLen == other.partitionValueExprLen &&
            COMPARE_CHAR_PTRS(prevPartitionValueExpr, other.prevPartitionValueExpr) &&
            prevPartitionValueExprLen == other.prevPartitionValueExprLen && isValid == other.isValid &&
            isReadonly == other.isReadonly && isInMemory == other.isInMemory && defTime == other.defTime &&
            flags == other.flags && subpartitionCnt == other.subpartitionCnt &&
...
class TrafPartitionV2Desc : public TrafDesc {
 public:
  ...
  virtual NABoolean operator==(const TrafPartitionV2Desc &other) const {
    ...
    return (baseTableUid == other.baseTableUid && partitionType == other.partitionType &&
            COMPARE_CHAR_PTRS(partitionColIdx, other.partitionColIdx) && partitionColCount == other.partitionColCount &&
            ...
            partitionAutolist == other.partitionAutolist && subpartitionAutolist == other.subpartitionAutolist &&
            flags == other.flags && stlPartitionCnt == other.stlPartitionCnt &&
