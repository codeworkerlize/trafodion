...
class PrivMgrRowInfo {
 public:
  PrivMgrRowInfo() : grantorID_(NA_UserIdDefault), granteeID_(NA_UserIdDefault), columnOrdinal_(-1) {
    privsBitmap_.reset();
    grantableBitmap_.reset();
  }

  PrivMgrRowInfo(const int32_t grantorID) : grantorID_(grantorID), granteeID_(NA_UserIdDefault), columnOrdinal_(-1) {
    privsBitmap_.reset();
    grantableBitmap_.reset();
  }

  PrivMgrRowInfo(const int32_t grantorID, const std::string grantorName, const int32_t granteeID,
                 const std::string granteeName, const int32_t columnOrdinal, const PrivObjectBitmap privsBitmap,
...
class PrivMgrMDRow {
 public:
  ...
  virtual PrivMgrRowInfo getPrivRowInfo() {
    PrivMgrRowInfo row;
    return row;
  }
...
class PrivMgrMDTable {
 public:
  ...
  // -------------------------------------------------------------------
  PrivMgrMDTable(const std::string &tableName, const PrivMgrTableEnum myTableEnum_, ComDiagsArea *pDiags = NULL);
  ...
  // Public functions:
  // -------------------------------------------------------------------
  PrivStatus CLIFetch(ExeCliInterface &cliInterface, const std::string &SQLStatement);

  PrivStatus CLIImmediate(const std::string &SQLStatement);

  PrivStatus executeFetchAll(ExeCliInterface &cliInterface, const std::string &SQLStatement, Queue *&queue);

  virtual PrivStatus insert(const PrivMgrMDRow &row) = 0;

  virtual PrivStatus selectCountWhere(const std::string &whereClause, int64_t &rowCount);

  virtual PrivStatus selectWhereUnique(const std::string &whereClause, PrivMgrMDRow &row) = 0;

  virtual PrivStatus deleteWhere(const std::string &whereClause);

  virtual PrivStatus update(const std::string &setClause);

  virtual PrivStatus updateWhere(const std::string &setClause, const std::string &whereClause, int64_t &rowCount);
