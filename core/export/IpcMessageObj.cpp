...
IpcMessageObj::IpcMessageObj(IpcMessageObjType msgType, IpcMessageObjVersion version) {
...
void *IpcMessageObj::operator new(size_t size, IpcBufferedMsgStream &msgStream, IpcMessageObjSize appendDataLen) {
...
IpcMessageObjSize IpcMessageObj::packObjIntoMessage(IpcMessageBufferPtr buffer, NABoolean swapBytes) {
  ...
  IpcMessageObjSize derivedObjLen = packedLength() - packBaseClassIntoMessage(buffer, swapBytes);
...
IpcMessageObjSize IpcMessageObj::packObjIntoMessage32(IpcMessageBufferPtr buffer, NABoolean swapBytes) {
  // copy the base class (IpcMessageObj) data members and calculate how
  // many more bytes beyond the base class remain to be copied
  IpcMessageObjSize derivedObjLen = packedLength32() - packBaseClassIntoMessage32(buffer, swapBytes);
...
void IpcMessageObj::unpackObj(IpcMessageObjType /*objType*/, IpcMessageObjVersion /*objVersion*/,
...
void IpcMessageObj::unpackObj32(IpcMessageObjType /*objType*/, IpcMessageObjVersion /*objVersion*/,
                                NABoolean /*sameEndianness*/, IpcMessageObjSize objSize,
                                IpcConstMessageBufferPtr buffer) {
...
NABoolean IpcMessageObj::checkObj(IpcMessageObjType t, IpcMessageObjVersion v, NABoolean sameEndianness,
                                  IpcMessageObjSize size, IpcConstMessageBufferPtr buffer) const {
  return checkBaseClass(t, v, sameEndianness, size, buffer);
}

NABoolean IpcMessageObj::checkBaseClass(IpcMessageObjType t, IpcMessageObjVersion v, NABoolean sameEndianness,
                                        IpcMessageObjSize size, IpcConstMessageBufferPtr &buffer) const {
  NABoolean result = TRUE;
  const IpcConstMessageBufferPtr lastByte = buffer + size - 1;
  if (!checkBuffer(buffer, sizeof(IpcMessageObj), lastByte) || s_.objType_ != t || s_.objVersion_ != v) {
    result = FALSE;
...
IpcMessageRefCount IpcMessageObj::incrRefCount() { return ++s_.refCount_; }
...
IpcMessageObjSize IpcMessageObj::packBaseClassIntoMessage(IpcMessageBufferPtr &buffer) {
  return packBaseClassIntoMessage(buffer, FALSE);
}

IpcMessageObjSize IpcMessageObj::packBaseClassIntoMessage(IpcMessageBufferPtr &buffer, NABoolean swapBytes) {
  ...
  assert(copyLen == 48 AND sizeof(s_) == 40 AND(char *) this == ((char *)&s_ - sizeof(char *)));
...
IpcMessageObjSize IpcMessageObj::packBaseClassIntoMessage32(IpcMessageBufferPtr &buffer, NABoolean swapBytes) {
...
IpcMessageObjSize IpcMessageObj::packDependentObjIntoMessage(IpcMessageBufferPtr buffer) {
  return packDependentObjIntoMessage(buffer, FALSE);
}

IpcMessageObjSize IpcMessageObj::packDependentObjIntoMessage(IpcMessageBufferPtr buffer, NABoolean swapBytes) {
...
IpcMessageObjSize IpcMessageObj::packDependentObjIntoMessage32(IpcMessageBufferPtr buffer) {
  return packDependentObjIntoMessage32(buffer, FALSE);
}

IpcMessageObjSize IpcMessageObj::packDependentObjIntoMessage32(IpcMessageBufferPtr buffer, NABoolean swapBytes) {
  IpcMessageBufferPtr alignedBuffer = buffer;
  alignBufferForNextObj(alignedBuffer);
...
void IpcMessageObj::unpackDependentObjFromBuffer(IpcConstMessageBufferPtr &buffer, NABoolean sameEndianness) {
  ...
  unpackObj(objInBuffer->s_.objType_, objInBuffer->s_.objVersion_, sameEndianness, objLength, buffer);

  buffer += objLength;
}

void IpcMessageObj::unpackDependentObjFromBuffer32(IpcConstMessageBufferPtr &buffer, NABoolean sameEndianness) {
  alignBufferForNextObj(buffer);
...
NABoolean IpcMessageObj::checkDependentObj(IpcConstMessageBufferPtr &buffer, NABoolean sameEndianness) const {
  alignBufferForNextObj(buffer);
  IpcMessageObj *objInBuffer = (IpcMessageObj *)buffer;
  IpcMessageObjSize objLength = objInBuffer->s_.objLength_;

  if (!checkObj(objInBuffer->s_.objType_, objInBuffer->s_.objVersion_, sameEndianness, objLength, buffer)) {
    return FALSE;
  }
...
void IpcMessageObj::mergeNextPackedObj() {
  ...
  if (nextObj) {
    if (nextObj->s_.next_) {
      IpcMessageObj *offset = (IpcMessageObj *)((Long)(s_.next_) + (char *)(nextObj->s_.next_));
...
void IpcMessageObj::convertNextToOffset() {
  ...
  if (s_.next_ != NULL) s_.next_ = (IpcMessageObj *)((char *)s_.next_ - (char *)this);
}
...
IpcMessageObjSize packCharStarIntoBuffer(IpcMessageBufferPtr &buffer, NAWchar *strPtr, NABoolean swapBytes) {
  ...
  if (strPtr != NULL) na_wcscpy((NAWchar *)buffer, strPtr);
  buffer += length;
  return result + length;
}

void unpackBuffer(IpcConstMessageBufferPtr &buffer, char *&strPtr, CollHeap *collHeapPtr) {
  if (collHeapPtr != NULL)
