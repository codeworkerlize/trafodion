...
public class RootResource extends ResourceBase {
	...
	private void initializeCacheAsNeeded() {
		if(!cacheInitialized) {
		    try {
		    	RESTServlet servlet = RESTServlet.getInstance();
				if(servlet != null) {
					LOG.info("Loading cache...");
					cacheInitialized = servlet.initializeCache();
				}
		    } catch (Exception e) {
		    }
		}
  ...
  @Path("/v1/transactions")
   ...
   @Path("/v1/wms/tenants")
   public TenantResource getTenantResource() throws IOException {
       //To test:
       //curl -v -L -k -X GET -H "Accept: application/json" http://<Rest server IP address>:8080/v1/wma/tenants
       //curl -v -L -k -X POST -d "{"name":"default"}" -H "Content-type: application/json" http://<Rest server IP address>/v1/wms/tenants
       //
