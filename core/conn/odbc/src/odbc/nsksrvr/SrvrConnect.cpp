...
extern bool stInternalQueriesInRepo;
extern int myNid;
extern int myPid;
...
extern "C" const char* state2String(int state);
extern "C" const char* type2String(int state);
void query_manager_watcher(zhandle_t *zzh, int type, int state, const char *path, void* watcherCtx);
...
bool firstTime = true;
//============================================================================================
string znodeWmsResourcesRegistered = "";
string znodeWmsResourcesQueries = "";
string znodeWmsResourcesQueriesSla = "";
string znodeWmsResourcesQueriesSlaQID = "";
pthread_cond_t c  = PTHREAD_COND_INITIALIZER;
pthread_mutex_t m = PTHREAD_MUTEX_INITIALIZER;
int qcond = 0;
const char *myEnvVar = "";
extern CMdsDeamon Mds;
...
extern int clearSLA;
void checkZKHandle(zhandle_t *zkHandle) {
        char zkErrStr[100];
        memset(zkErrStr,'\0',sizeof(zkErrStr));

        if ( zkHandle == 0 ) {
           snprintf(zkErrStr, sizeof(zkErrStr)-1,"***** Zookeeper session expired. zk handle is null.... Exiting server");
...
extern void publishAggrStatsToTSDB(std::tr1::shared_ptr<SESSION_AGGREGATION> pAggr_info);
extern string GetClusterID(string sql);
CEE_handle_def StatisticsTimerHandle;
CEE_handle_def queryTimerHandle;
...
void preloadTables()
{
    ...
    SQLRETURN retCode;
    SRVR_STMT_HDL *preloadSrvrStmt = NULL;
    ...
    int pathLen;
    int SqlCount = 0;
    int SqlSuccessCount = 0;
    int limitCount = srvrGlobal->loadEnable;

    const char *tempPath = getenv("TRAF_CONF");
    char *preloadPath;
    char *preloadStr = (char *)malloc(SQLSTRING_MAX_LEN + 1);
    char *begin = preloadStr;
    char *ret = NULL;
    char eventMessage[SQLSTRING_MAX_LEN + 256];

    if(tempPath != NULL) {
        pathLen = strlen(tempPath);
    }
    preloadPath = (char *)malloc(pathLen + 256);
    strcpy(preloadPath, tempPath);
    strcat(preloadPath, "/preload_config");

    FILE* fp = NULL;
    std::string SQLString = "";
    fp = fopen(preloadPath, "r");

    if (fp != NULL)
    {
        if((preloadSrvrStmt = getSrvrStmt("PRELOAD_TABLES", FALSE)) != NULL) 
        {     
            preloadSrvrStmt->Close(SQL_DROP);
        }     
        if((preloadSrvrStmt = getSrvrStmt("PRELOAD_TABLES", TRUE)) == NULL) 
        {     
            SendEventMsg(MSG_SERVER_TRACE_INFO, 
                    ...
                    srvrObjRef, 
                    1, 
                    "Fail to getSrvrStmt from the stmtLabel of PRELOAD_TABLES");
            fclose(fp);
            free(preloadStr);
            preloadStr = NULL;
            free(preloadPath);
        ...
        while (!feof(fp) && SqlCount < limitCount)
        {
            preloadStr = begin;
            memset(preloadStr, 0, SQLSTRING_MAX_LEN);
            fgets(preloadStr, SQLSTRING_MAX_LEN, fp);
            preloadStr = trimSqlString(preloadStr);
            if(isspace(preloadStr[0]) || (preloadStr[0] == '#') || (preloadStr[0] == 0))
            {
                continue;   
            }

            if((ret = strchr(preloadStr, SEMICOLON)) != NULL)
            {
                SQLString.append(preloadStr);
                SqlCount++;
                retCode = preloadSrvrStmt->Prepare(SQLString.data(), INTERNAL_STMT, SQL_ASYNC_ENABLE_OFF, 0);
                if (retCode != SQL_SUCCESS)      
                {         
                    sprintf(eventMessage, "Error, can not preload or identify the sqlString:\n%s", SQLString.data());
                    SendEventMsg(MSG_SERVER_TRACE_INFO, 
                            EVENTLOG_ERROR_TYPE,           
                            processId,                     
                            ODBCMX_SERVER,                 
                ...
                if((strstr(SQLString.c_str(), "set schema") != NULL) || (strstr(SQLString.c_str(), "SET SCHEMA") != NULL))
                {
                    retCode = preloadSrvrStmt->Execute(SQLString.data(), 1, SET_SCHEMA, &inValueList, SQL_ASYNC_ENABLE_OFF, 0);
                    if (retCode == SQL_ERROR)
                    {     
                        sprintf(eventMessage, "Error, fail to execute the sqlString:\n%s", SQLString.data());
                        SendEventMsg(MSG_SERVER_TRACE_INFO, 
                                EVENTLOG_ERROR_TYPE,                 
                                processId,                           
                                ODBCMX_SERVER,                       
                    ...
                    {
                        sprintf(eventMessage, "Haven been successed execute the sqlString:\n%s", SQLString.data());
                        SendEventMsg(MSG_SERVER_TRACE_INFO,    
                ...
                SqlSuccessCount++;
                sprintf(eventMessage, "Haven been successed preloaded the sqlString:\n%s", SQLString.data());
                SendEventMsg(MSG_SERVER_TRACE_INFO, 
    ...
    {   
        sprintf(eventMessage, "Fail to open the preload file %s, Value of errno: %d, Error opening file: %s", preloadPath, errno, strerror(errno));
        SendEventMsg(MSG_SERVER_TRACE_INFO, 
                EVENTLOG_ERROR_TYPE,
                processId, 
                ODBCMX_SERVER,
...
PRELOAD_ERR:
    sprintf(eventMessage, "PRELOAD COUNT:%d, SUCCESS COUNT:%d\n", SqlCount, SqlSuccessCount);
    SendEventMsg(MSG_SERVER_TRACE_INFO, 
            EVENTLOG_WARNING_TYPE,     
            processId,                     
            ODBCMX_SERVER,                 
...
static void* SessionWatchDog(void* arg)
{
	...
	try
	{
        ...
        if (okToGo)
        {
            retcode = pSrvrStmt->ExecDirect(NULL, "SET SESSION DEFAULT CANCEL_QUERY_ALLOWED 'OFF'", INTERNAL_STMT, TYPE_UNKNOWN, SQL_ASYNC_ENABLE_OFF, 0);
            if (retcode < 0)
            {
                errMsg.str("");
                if(pSrvrStmt->sqlError.errorList._length > 0)
                    ...
                    p_buffer = pSrvrStmt->sqlWarning._buffer;
                if(p_buffer != NULL && p_buffer->errorText)
                    errMsg << "Failed to turn off cancel query allowed - " << p_buffer->errorText;
                else
                    errMsg << "Failed to turn off cancel query allowed - " << " no additional information";
        ...
        if (okToGo)
        {
            retcode = pSrvrStmt->ExecDirect(NULL, "CQD TRAF_SAVEPOINTS 'ON'", INTERNAL_STMT, TYPE_UNKNOWN, SQL_ASYNC_ENABLE_OFF, 0);
            if (retcode < 0)
            {
                errMsg.str("");
                if(pSrvrStmt->sqlError.errorList._length > 0)
                    ...
                    p_buffer = pSrvrStmt->sqlWarning._buffer;
                if(p_buffer != NULL && p_buffer->errorText)
                    errMsg << "Failed to turn on missing statistics warning - " << p_buffer->errorText;
                else
                    errMsg << "Failed to turn off missing statistics warning - " << " no additional information";
                errStr = errMsg.str();
                SendEventMsg(MSG_ODBC_NSK_ERROR, EVENTLOG_ERROR_TYPE,
        ...
        if (okToGo)
        {
            retcode = pSrvrStmt->ExecDirect(NULL, "CQD MODE_SPECIAL_4 'ON' ", INTERNAL_STMT, TYPE_UNKNOWN, SQL_ASYNC_ENABLE_OFF, 0);
            if (retcode < 0)
            {
                errMsg.str("");
                if(pSrvrStmt->sqlError.errorList._length > 0)
                    ...
                    p_buffer = pSrvrStmt->sqlWarning._buffer;
                if(p_buffer != NULL && p_buffer->errorText)
                    errMsg << "Failed to turn on MODE_SPECIAL_4 - " << p_buffer->errorText;
                else
                    errMsg << "Failed to turn on MODE_SPECIAL_4 - " << " no additional information";
                errStr = errMsg.str();
                SendEventMsg(MSG_ODBC_NSK_ERROR, EVENTLOG_ERROR_TYPE,
                        0, ODBCMX_SERVER, srvrGlobal->srvrObjRef,
                        1, errStr.c_str());
        ...
        while(!record_session_done && okToGo)
        {
			...
			if (repos_stats.m_pub_type == PUB_TYPE_SESSION_START || repos_stats.m_pub_type == PUB_TYPE_SESSION_END)
			{
				...
				if(NULL == pSessionInfo)
				{
			      ...
			      if ( (!mgbltyQueries) || (srvrGlobal->m_stInternalQueriesInRepo) ) {  
				ss << "(";
			...
			else if (repos_stats.m_pub_type == PUB_TYPE_STATEMENT_NEW_QUERYEXECUTION)
			{
				...
				if(NULL == pQueryAdd)
				{
			      ...
			      if ( (!mgbltyQueries) || (srvrGlobal->m_stInternalQueriesInRepo) ) {  
                                ...
                                pQueryAdd->m_query_signature_id = GetClusterID(pQueryAdd->m_query_text.c_str());

				//Inserting records into metric_query_table 
                                ss << "(";
			...
			else if (repos_stats.m_pub_type == PUB_TYPE_SESSION_START_AGGREGATION)
			{
				...
				ss << pAggrStat->m_throughput_per_sec << ")";
                                if (srvrGlobal->m_bPublishStatsToTSDB)
                                {
                                     publishAggrStatsToTSDB(pAggrStat);
                                }
...
//LCOV_EXCL_STOP
		...
		srvrGlobal->bMultiTenancyEnabled = msg_license_multitenancy_enabled();
    if (srvrGlobal->bMultiTenancyEnabled)
    {
      bzero(srvrGlobal->tenantName, sizeof(srvrGlobal->tenantName));
      srvrGlobal->tenantID = 0;
      srvrGlobal->affinityNum = 0;
      srvrGlobal->tenantRedefTime = 0;
    };
...
//LCOV_EXCL_START
	if (srvrInitParam)
	{
		const char * trace = getenv("MXOSRVR_TRACE");
	...
	if (srvrInitParam != NULL && srvrInitParam->srvrTrace == true)
	{
		...
		diagnostic_flags = 1;  // turns on Catalog API tracing
...
//	checkIfRowsetSupported();
	...
	srvrGlobal->m_stInternalQueriesInRepo = stInternalQueriesInRepo;
...
void __cdecl SRVR::ASTimerExpired(CEE_tag_def timer_tag)
{
	...
	if(toDisable != 1 &&srvrGlobal->srvrState != SRVR_AVAILABLE &&  srvrGlobal->srvrState != SRVR_UNINITIALIZED && access(dcsStartTimeFile,0)!=0)
	{
		toDisable = 1;
		inDcsStopMode =1;
		char* dummyWriteBuffer = "select loop next";
		GTransport.m_listener->writeDummy(dummyWriteBuffer, strlen(dummyWriteBuffer));
		MXO_WARN("mxosrvr is in dcsstop mode now it will exit after 10s if no io");
	}
//srvrGlobal->srvrState is a global variable.  strictly say it should be protected by lock
//if the client querys every 1s, the tcp/ip thread maybe always holds this lock
//then this thread awaked, it will return directly brcause tcp/ip hold this lock 
//this thread maybe can't hold this lock forever and this mxosrvr will never exit if client sends data every 1s
//so set the lock operation after disable
//because the operation about variable of srvrGlobal->srvrState is read only
//and it depending on api of 'access', wether this mxos should be disabled,
//so even if srvrGlobal->srvrState is in middle state if dosen't matter
...
{
   SESSION_DEBUG("odbc_SQLSvc_InitializeDialogue_ame_ entry");
   ...
   char            prevTenantName[MAX_APPLICATION_NAME_LEN + 1];
   int             prevTenantID = 0;
   int             prevAffinityNum = 0;
   __int64         prevTenantRedefTime = 0;
...
//    cleanupJniDLL();
   ...
   if (srvrGlobal->bMultiTenancyEnabled)
   {
      strcpy(prevTenantName,srvrGlobal->tenantName );
      prevTenantID = srvrGlobal->tenantID;
      prevAffinityNum = srvrGlobal->affinityNum;
      prevTenantRedefTime = srvrGlobal->tenantRedefTime;
   };
   ...
   bzero(srvrGlobal->ClientIpAddress, sizeof(srvrGlobal->ClientIpAddress));
   bzero(srvrGlobal->tenantName, sizeof(srvrGlobal->tenantName));
   ...
   //ClientUserName
   {
       const char* ptr = (NULL == inContext->clientUserName) ? "unkown" : inContext->clientUserName;
       strncpy(srvrGlobal->ClientUserName, ptr, sizeof(srvrGlobal->ClientUserName) - 1);
       srvrGlobal->ClientUserName[sizeof(srvrGlobal->ClientUserName) -1] = '\0';
   }
...
//   else
   ...
   if(srvrGlobal->lic.isModuleOpen(LM_LITTLE_SHEET_CACHE))
   {
       outContext.outContextOptions2 = (outContext.outContextOptions2|OUTCONTEXT_OPT2_LITTLE_SHEET_CACHE);
   }
   else 
   {
   ...
   Mds.InitialDialMsg();
   SESSION_DEBUG("[Initialize_Dialogue]zookeeper operation start!");
   checkZKHandle(zh);
   ...
   if( rc == ZOK )
   {
      ...
      while(!stateOk && cnt)
      {
         ...
         if( rc != ZOK )
         {
            ...
            SendEventMsg(MSG_PROGRAMMING_ERROR, EVENTLOG_ERROR_TYPE,
               srvrGlobal->nskProcessInfo.processId, ODBCMX_SERVER, srvrGlobal->srvrObjRef, 1, tmpString);
            exitServerProcess();
         }
         ...
         if( rc == ZOK )
         {
            ...
            tkn = strtok_r(zkData, ":", &saveDataPtr);
...
*/
         ...
         if(NULL == (tkn = strsep(&stringp, ":"))) { tokenFlag = 1 ;goto TNULL;}         //token #1
         if(NULL == (tkn = strsep(&stringp, ":"))) { tokenFlag = 2 ;goto TNULL;}         //token #2
         if(NULL == (tkn = strsep(&stringp, ":"))) { tokenFlag = 3 ;goto TNULL;}         //token #3

         if(NULL == (tkn = strsep(&stringp, ":"))) { tokenFlag = 4 ;goto TNULL;}          //token #4
         if(NULL == (tkn = strsep(&stringp, ":"))) { tokenFlag = 5 ;goto TNULL;}          //token #5
         else
            serverProcessId = string(tkn);
         if(NULL == (tkn = strsep(&stringp, ":"))) { tokenFlag = 6 ;goto TNULL;}          //token #6
         else
            serverProcessName = string(tkn);
         if(NULL == (tkn = strsep(&stringp, ":"))) { tokenFlag = 7 ;goto TNULL;}          //token #7
         else
            serverHost = string(tkn);
         if(NULL == (tkn = strsep(&stringp, ":"))){ tokenFlag = 8 ;goto TNULL;}          //token #8
         else
            serverPort = string(tkn);
         if(NULL == (tkn = strsep(&stringp, ":"))) { tokenFlag = 9 ;goto TNULL;}          //token #9

         if(NULL == (tkn = strsep(&stringp, ":"))) { tokenFlag = 10 ;goto TNULL;}          //token #10
         if(NULL == (tkn = strsep(&stringp, ":"))) { tokenFlag = 11 ;goto TNULL;}          //token #11
         if(NULL == (tkn = strsep(&stringp, ":"))) { tokenFlag = 12 ;goto TNULL;}          //token #12
         else
            appName = string(tkn);
         if(NULL == (tkn = strsep(&stringp, ":"))) { tokenFlag = 13 ;goto TNULL;}          //token #13
         else
            sla = string(tkn);
         if(NULL == (tkn = strsep(&stringp, ":"))) { tokenFlag = 14 ;goto TNULL;}          //token #14
         else
            cprofile = string(tkn);
         if(NULL == (tkn = strsep(&stringp, ":"))) { tokenFlag = 15 ;goto TNULL;}          //token #15
         else
            dprofile = string(tkn);
         if(NULL == (tkn = strsep(&stringp, ":"))) { tokenFlag = 16 ;goto TNULL;}         //token #16
         else
            lastUpdate = string(tkn);
         if(NULL == (tkn = strsep(&stringp, ":"))) { tokenFlag = 17 ;goto TNULL;}          //token #17
         if(NULL == (tkn = strsep(&stringp, ":"))) { tokenFlag = 18 ;goto TNULL;}          //token #18
         else
            tenantName = string(tkn);
         if(NULL == (tkn = strsep(&stringp, ":"))) { tokenFlag = 19 ;goto TNULL;}          //token #19
         else
            affinityNum = string(tkn);
TNULL:
         if( tkn == NULL )
         {
			char msg[1024] = {0};
			sprintf(msg,"No expected tokens in registered node. Server exiting,token %d is error.",tokenFlag);
            SendEventMsg(MSG_PROGRAMMING_ERROR, EVENTLOG_ERROR_TYPE,
               srvrGlobal->nskProcessInfo.processId, ODBCMX_SERVER, srvrGlobal->srvrObjRef,
               1, msg);
            exitServerProcess();
         ...
         if ((srvrGlobal->bMultiTenancyEnabled)
             && ( stricmp(tenantName.c_str(),"null") != 0)
            )
         {
            strcpy(srvrGlobal->tenantName, tenantName.c_str());
            srvrGlobal->affinityNum = atoi(affinityNum.c_str());
         }
         else
         {
            srvrGlobal->tenantName[0] = 0;
            srvrGlobal->affinityNum = 0;
         }
...
// build sla path from zk node /<user name>/wms/slas/sla
         ...
         delete[] cstr;
         znodeWmsResourcesRegistered = "/" + user + "/wms/resources/query_manager";
         znodeWmsResourcesQueries = "/" + user + "/wms/resources/queries";
         znodeWmsResourcesQueriesSla = "/" + user + "/wms/resources/queries/" + sla;
         checkZKHandle(zh);
         rc = zoo_exists(zh, znodeWmsResourcesQueriesSla.c_str(), false, &stat);
         if (rc != ZOK ){
            checkZKHandle(zh);
            rc = zoo_create(zh, znodeWmsResourcesQueriesSla.c_str(), "sla", 3, &ZOO_OPEN_ACL_UNSAFE, 0, NULL, 0);
         }
//SLA
         srvrGlobal->estCardinality = 0.0;
         srvrGlobal->estCost = 0.0;
         ...
         if (rc == ZOK )
         {
            ...
            if( rc == ZOK )
            {
               ...
               while(true)
               {
                  ...
                  else if(strcmp(tkn, "estCardinality") == 0)
                  {
                     testcardinality = strsep(&stringp,":");
                     srvrGlobal->estCardinality = atof(testcardinality);
                  }
                  else if(strcmp(tkn, "estCost") == 0)
                  {
                     testcost = strsep(&stringp,":");
                     srvrGlobal->estCost = atof(testcost);
                  }
                  else
                     tkn = strsep(&stringp,":");
                  tkn = strsep(&stringp,"=");
...
// end of Profile on disconnect
   ...
   SESSION_DEBUG("[Initialize_Dialogue]zookeeper operation finished");
...
//   }
   ...
   SESSION_DEBUG("[Initialize_Dialogue]security initialization starts!");
   if (!securitySetup)
   {
...
//   R2.5  - Allow old driver connection without password encryption if security policy allows it
   ...
   SESSION_DEBUG("[Initialize_Dialogue]security initialization finished!");
...
//
   SESSION_DEBUG("[Initialize_Dialogue]attrs reseting starts!");
   ...
   if (setConnectException.exception_nr != CEE_SUCCESS)
   {
      ...
      exception_.exception_nr = odbc_SQLSvc_InitializeDialogue_SQLError_exn_;
      errorBuffer = setConnectException.u.SQLError.errorList._buffer;
      if (errorBuffer)
      {
      ...
      {
        sqlError = "RESET_DEFAULTS failed ";
        errorBuffer = setConnectException.u.SQLError.errorList._buffer;
        if (errorBuffer)
            sqlError += ":" + string(errorBuffer->errorText);
      }
      ...
      {
        SETSRVRERROR(SQLERRWARN, NULL, NULL, (char *)sqlError.c_str(), &exception_.u.SQLError.errorList);
      }
      odbc_SQLSvc_InitializeDialogue_ts_res_(objtag_, call_id_, &exception_, &outContext);
      updateSrvrState(SRVR_CONNECT_REJECTED);
      return;
...
// Added to detect MODE_SPECIAL_1 CQD
   ...
   */
   ...
   if (setConnectException.exception_nr != CEE_SUCCESS)
   {
      ...
      exception_.exception_nr = odbc_SQLSvc_InitializeDialogue_SQLError_exn_;
      errorBuffer = setConnectException.u.SQLError.errorList._buffer;
      sqlError = "SQL_TXN_ISOLATION failed :";
      if (errorBuffer)
        sqlError += string(errorBuffer->errorText);
      SendEventMsg(MSG_SRVR_POST_CONNECT_ERROR, EVENTLOG_ERROR_TYPE,
         srvrGlobal->nskProcessInfo.processId, ODBCMX_SERVER, srvrGlobal->srvrObjRef,
   ...
   if (setConnectException.exception_nr != CEE_SUCCESS)
   {
      ...
      exception_.exception_nr = odbc_SQLSvc_InitializeDialogue_SQLError_exn_;
      errorBuffer = setConnectException.u.SQLError.errorList._buffer;
      sqlError = "CUT_CONTROLQUERYSHAPE failed :";
      if (errorBuffer)
        sqlError += string(errorBuffer->errorText);
      SendEventMsg(MSG_SRVR_POST_CONNECT_ERROR, EVENTLOG_ERROR_TYPE,
         srvrGlobal->nskProcessInfo.processId, ODBCMX_SERVER, srvrGlobal->srvrObjRef,
...
// collect information for auditing and repository
   SESSION_DEBUG("[Initialize_Dialogue]authentication starts!");
   ...
   if (exception_.exception_nr != 0)
   {
      ...
      if (srvrGlobal->bMultiTenancyEnabled)
      {
         strcpy(srvrGlobal->tenantName,prevTenantName );
         srvrGlobal->tenantID = prevTenantID;
         srvrGlobal->affinityNum = prevAffinityNum;
         srvrGlobal->tenantRedefTime = prevTenantRedefTime;
      };
      updateSrvrState(SRVR_CONNECT_REJECTED);
   ...
   SESSION_DEBUG("[Initialize_Dialogue]authentication finished!");
//======================================END of authentication================================

// perform CQDs that tell CLI what the client supports
//
// Note that this has to be done *after* authentication, since authentication
// may clear out any CQDs associated with the current context.
   ...
   if (setConnectException.exception_nr != CEE_SUCCESS)
   {
      ...
      exception_.exception_nr = odbc_SQLSvc_InitializeDialogue_SQLError_exn_;
      errorBuffer = setConnectException.u.SQLError.errorList._buffer;
      sqlError = "SET_ODBC_PROCESS failed :";
      if (errorBuffer)
         sqlError += string(errorBuffer->errorText);
   ...
   if (setConnectException.exception_nr != CEE_SUCCESS)
   {
      ...
      exception_.exception_nr = odbc_SQLSvc_InitializeDialogue_SQLError_exn_;
      errorBuffer = setConnectException.u.SQLError.errorList._buffer;
      sqlError = "SET_JDBC_PROCESS failed :";
      if (errorBuffer)
         sqlError += string(errorBuffer->errorText);
      SendEventMsg(MSG_SRVR_POST_CONNECT_ERROR, EVENTLOG_ERROR_TYPE,
         srvrGlobal->nskProcessInfo.processId, ODBCMX_SERVER, srvrGlobal->srvrObjRef,
   ...
   if (setConnectException.exception_nr != CEE_SUCCESS)
   {
      ...
      exception_.exception_nr = odbc_SQLSvc_InitializeDialogue_SQLError_exn_;
      errorBuffer = setConnectException.u.SQLError.errorList._buffer;
      sqlError = "RESET_RESET_DEFAULTS failed :";
      if (errorBuffer)
         sqlError += string(errorBuffer->errorText);
      SendEventMsg(MSG_SRVR_POST_CONNECT_ERROR, EVENTLOG_ERROR_TYPE,
         srvrGlobal->nskProcessInfo.processId, ODBCMX_SERVER, srvrGlobal->srvrObjRef,
...
//============================== Execute all CQDs from the Profile ======================================
   ...
   if (sqlError.length() != 0)
   {
      ...
      SendEventMsg(MSG_SRVR_POST_CONNECT_ERROR, EVENTLOG_ERROR_TYPE,
         srvrGlobal->nskProcessInfo.processId, ODBCMX_SERVER, srvrGlobal->srvrObjRef,
         2, "EXECUTE CQD from wms profile failed :", sqlError.c_str());
...
// Convert the default catalog set by old drivers to the current
   ...
   else
   {
      ...
      if (strstr(srvrGlobal->sessionId, "MGBLTY") != NULL)   // Eliminate MGBLTY queries
          mgbltyQueries = true;
   }
   ...
   if (setConnectException.exception_nr != CEE_SUCCESS)
   {
      ...
      exception_.exception_nr = odbc_SQLSvc_InitializeDialogue_SQLError_exn_;
      errorBuffer = setConnectException.u.SQLError.errorList._buffer;
      sqlError = "SQL_AUTOCOMMIT failed :";
      if (errorBuffer)
        sqlError += string(errorBuffer->errorText);
      SendEventMsg(MSG_SRVR_POST_CONNECT_ERROR, EVENTLOG_ERROR_TYPE,
         srvrGlobal->nskProcessInfo.processId, ODBCMX_SERVER, srvrGlobal->srvrObjRef,
      ...
      for (curRowNo = 0; curRowNo < len_length; curRowNo ++)  // scan through each RG policy
      {
         ...
         if ((pEnvDesc->VarType == ENV_SET) || (pEnvDesc->VarType == ENV_CONTROL))// Set & Control statements
         {
            ...
            if (setConnectException.exception_nr != CEE_SUCCESS)
            {
               ...
               exception_.exception_nr = odbc_SQLSvc_InitializeDialogue_SQLError_exn_;
               errorBuffer = setConnectException.u.SQLError.errorList._buffer;
               sqlError = "SET_SETANDCONTROLSTMTS failed :";
               if (errorBuffer)
                 sqlError += string(errorBuffer->errorText);
               SendEventMsg(MSG_SRVR_POST_CONNECT_ERROR, EVENTLOG_ERROR_TYPE,
                  srvrGlobal->nskProcessInfo.processId, ODBCMX_SERVER, srvrGlobal->srvrObjRef,
   ...
   SESSION_DEBUG("[Initialize_Dialogue]statements preparing starts!");
//============================================================================================================
   ...
   SRVR_STMT_HDL *beginSpSrvrStmt;
   SRVR_STMT_HDL *rollbackSpSrvrStmt;
   SRVR_STMT_HDL *commitSpSrvrStmt;

   retCode = EXECDIRECT("CQD TRAF_SAVEPOINTS HOLD");
   if (retCode == SQL_ERROR)
   {
       exception_.exception_detail = -1;
       exception_.exception_nr = 99;
       sqlError = "Error in CQD TRAF_SAVEPOINTS HOLD.";
       SendEventMsg(MSG_SRVR_POST_CONNECT_ERROR, EVENTLOG_ERROR_TYPE,
               srvrGlobal->nskProcessInfo.processId, ODBCMX_SERVER, srvrGlobal->srvrObjRef,
               2, "", sqlError.c_str());
       SETSRVRERROR(SQLERRWARN, -1, "HY000", (char*)sqlError.c_str(), &exception_.u.SQLError.errorList);
   ...
   retCode = EXECDIRECT("CQD TRAF_SAVEPOINTS 'ON'");
   if (retCode == SQL_ERROR)
   {
       exception_.exception_detail = -1;
       exception_.exception_nr = 99;
       sqlError = "Error in CQD TRAF_SAVEPOINTS 'ON'.";
       SendEventMsg(MSG_SRVR_POST_CONNECT_ERROR, EVENTLOG_ERROR_TYPE,
               srvrGlobal->nskProcessInfo.processId, ODBCMX_SERVER, srvrGlobal->srvrObjRef,
               2, "", sqlError.c_str());
       SETSRVRERROR(SQLERRWARN, -1, "HY000", (char*)sqlError.c_str(), &exception_.u.SQLError.errorList);
   ...
   if ((beginSpSrvrStmt = getSrvrStmt("STMT_BEGIN_SAVEPOINT_1", FALSE)) != NULL)
       beginSpSrvrStmt->Close(SQL_DROP);
   if ((beginSpSrvrStmt = getSrvrStmt("STMT_BEGIN_SAVEPOINT_1", TRUE)) == NULL)
   {
       exception_.exception_detail = -1;
       exception_.exception_nr = 99;
       sqlError = "Unable to allocate statement to begin savepoint. ";
       SendEventMsg(MSG_SRVR_POST_CONNECT_ERROR, EVENTLOG_ERROR_TYPE,
               srvrGlobal->nskProcessInfo.processId, ODBCMX_SERVER, srvrGlobal->srvrObjRef,
               2, "STMT_BEGIN_SAVEPOINT_1", sqlError.c_str());
       SETSRVRERROR(SQLERRWARN, -1, "HY000", (char*)sqlError.c_str(), &exception_.u.SQLError.errorList);
       odbc_SQLSvc_InitializeDialogue_ts_res_(objtag_, call_id_, &exception_, &outContext);
       updateSrvrState(SRVR_CONNECT_REJECTED);
       return;
   ...
   retCode = beginSpSrvrStmt->Prepare("BEGIN SAVEPOINT",INTERNAL_STMT,SQL_ASYNC_ENABLE_OFF, 0);
   if (retCode == SQL_ERROR)
   {
       exception_.exception_detail = -1;
       exception_.exception_nr = 99;
       sqlError = "Error in Preparing BEGIN SAVEPOINT.";
       SendEventMsg(MSG_SRVR_POST_CONNECT_ERROR, EVENTLOG_ERROR_TYPE,
               srvrGlobal->nskProcessInfo.processId, ODBCMX_SERVER, srvrGlobal->srvrObjRef,
               2, "STMT_BEGIN_SAVEPOINT_1", sqlError.c_str());
       SETSRVRERROR(SQLERRWARN, -1, "HY000", (char*)sqlError.c_str(), &exception_.u.SQLError.errorList);
   ...
   if ((rollbackSpSrvrStmt = getSrvrStmt("STMT_ROLLBACK_SAVEPOINT_1", FALSE)) != NULL)
       rollbackSpSrvrStmt->Close(SQL_DROP);
   if ((rollbackSpSrvrStmt = getSrvrStmt("STMT_ROLLBACK_SAVEPOINT_1", TRUE)) == NULL)
   {
       exception_.exception_detail = -1;
       exception_.exception_nr = 99;
       sqlError = "Unable to allocate statement to rollback savepoint.  ";
       SendEventMsg(MSG_SRVR_POST_CONNECT_ERROR, EVENTLOG_ERROR_TYPE,
               srvrGlobal->nskProcessInfo.processId, ODBCMX_SERVER, srvrGlobal->srvrObjRef,
               2, "STMT_ROLLBACK_SAVEPOINT_1", sqlError.c_str());
       SETSRVRERROR(SQLERRWARN, -1, "HY000", (char*)sqlError.c_str(), &exception_.u.SQLError.errorList);
       odbc_SQLSvc_InitializeDialogue_ts_res_(objtag_, call_id_, &exception_, &outContext);
       updateSrvrState(SRVR_CONNECT_REJECTED);
       return;
   }   
   retCode = rollbackSpSrvrStmt->Prepare("ROLLBACK SAVEPOINT",INTERNAL_STMT,SQL_ASYNC_ENABLE_OFF, 0);
   if (retCode == SQL_ERROR)
   {
       exception_.exception_detail = -1;
       exception_.exception_nr = 99;
       sqlError = "Error in Preparing rollback savepoint.";
       SendEventMsg(MSG_SRVR_POST_CONNECT_ERROR, EVENTLOG_ERROR_TYPE,
               srvrGlobal->nskProcessInfo.processId, ODBCMX_SERVER, srvrGlobal->srvrObjRef,
               2, "STMT_ROLLBACK_SAVEPOINT_1", sqlError.c_str());
       SETSRVRERROR(SQLERRWARN, -1, "HY000", (char*)sqlError.c_str(), &exception_.u.SQLError.errorList);
   ...
   if ((commitSpSrvrStmt = getSrvrStmt("STMT_COMMIT_SAVEPOINT_1", FALSE)) != NULL)
       commitSpSrvrStmt->Close(SQL_DROP);
   if ((commitSpSrvrStmt = getSrvrStmt("STMT_COMMIT_SAVEPOINT_1", TRUE)) == NULL)
   {
       exception_.exception_detail = -1;
       exception_.exception_nr = 99;
       sqlError = "Unable to allocate statement to commit savepoint.  ";
       SendEventMsg(MSG_SRVR_POST_CONNECT_ERROR, EVENTLOG_ERROR_TYPE,
               srvrGlobal->nskProcessInfo.processId, ODBCMX_SERVER, srvrGlobal->srvrObjRef,
               2, "STMT_COMMIT_SAVEPOINT_1", sqlError.c_str());
       SETSRVRERROR(SQLERRWARN, -1, "HY000", (char*)sqlError.c_str(), &exception_.u.SQLError.errorList);
       odbc_SQLSvc_InitializeDialogue_ts_res_(objtag_, call_id_, &exception_, &outContext);
       updateSrvrState(SRVR_CONNECT_REJECTED);
	   return;
   }
   retCode = commitSpSrvrStmt->Prepare("COMMIT SAVEPOINT",INTERNAL_STMT,SQL_ASYNC_ENABLE_OFF, 0);
   if (retCode == SQL_ERROR)
   {
	   exception_.exception_detail = -1;
	   exception_.exception_nr = 99;
	   sqlError = "Error in Preparing commit savepoint.";
	   SendEventMsg(MSG_SRVR_POST_CONNECT_ERROR, EVENTLOG_ERROR_TYPE,
			   srvrGlobal->nskProcessInfo.processId, ODBCMX_SERVER, srvrGlobal->srvrObjRef,
			   2, "STMT_COMMIT_SAVEPOINT_1", sqlError.c_str());
	   SETSRVRERROR(SQLERRWARN, -1, "HY000", (char*)sqlError.c_str(), &exception_.u.SQLError.errorList);
   ...
   retCode = EXECDIRECT(" CQD TRAF_SAVEPOINTS RESTORE");
   if (retCode == SQL_ERROR)
   {
       exception_.exception_detail = -1;
       exception_.exception_nr = 99;
       sqlError = "Error in CQD TRAF_SAVEPOINTS RESTORE.";
       SendEventMsg(MSG_SRVR_POST_CONNECT_ERROR, EVENTLOG_ERROR_TYPE,
               srvrGlobal->nskProcessInfo.processId, ODBCMX_SERVER, srvrGlobal->srvrObjRef,
               2, "", sqlError.c_str());
       SETSRVRERROR(SQLERRWARN, -1, "HY000", (char*)sqlError.c_str(), &exception_.u.SQLError.errorList);
   ...
   //============================== Execute all Sets from the Profile ======================================

   SESSION_DEBUG("[Initialize_Dialogue]statements preparing finished!");
   ...
   if (sqlError.length() != 0)
   {
      ...
      SendEventMsg(MSG_SRVR_POST_CONNECT_ERROR, EVENTLOG_ERROR_TYPE,
         srvrGlobal->nskProcessInfo.processId, ODBCMX_SERVER, srvrGlobal->srvrObjRef,
         2, "EXECUTE SET from wms profile failed :", sqlError.c_str());
      SETSRVRERROR(SQLERRWARN, -1, "HY000", (char*)sqlError.c_str(), &exception_.u.SQLError.errorList);
      odbc_SQLSvc_InitializeDialogue_ts_res_(objtag_, call_id_, &exception_, &outContext);
      updateSrvrState(SRVR_CONNECT_REJECTED);
      return;
...
//==========================================================================================
   ...
   {
      srvrGlobal->srvrContext.connIdleTimeout = inContext->idleTimeoutSec;
   }
...
{
	SESSION_DEBUG("odbc_SQLSvc_TerminateDialogue_ame_ entry");
...
bailout:
	...
	if(exitSessions == true){
		string msgStr = "Number of Sessions reached the Limit. Mxosrvr exits.";
		odbc_SQLSvc_StopServer_exc_ StopException;
	...
	if(exitLTime == true ) {
		string msgStr = "Live Time of Sessions reached the Limit. Mxosrvr exits.";
		odbc_SQLSvc_StopServer_exc_ StopException;
...
void __cdecl SRVR::SrvrSessionCleanup(void)
{
	...
	EXECDIRECT("SET TRANSACTION AUTOCOMMIT SAVEPOINT ON");
	...
	if ((srvrGlobal->numConnection+1 == srvrGlobal->cleanupByConnection) || (t > (double)srvrGlobal->cleanupByTime * 60))
	{
	   srvrGlobal->numConnection = 0;
	   srvrGlobal->lastCleanupTime = time(NULL);
	   EXECDIRECT("SET SESSION DEFAULT SQL_SESSION 'END:CLEANUP_ESPS:CLEANUP_CMPCONTEXT'");
	   // all ESPs are stopped
	   srvrGlobal->allocatedResources = 0;
	}
	else
	{
	   EXECDIRECT("SET SESSION DEFAULT SQL_SESSION 'END:CLEANUP_CMPCONTEXT'");
...
//LCOV_EXCL_STOP
        ...
        // TBD:  This call may not be necessary since cleanup is performed as
        //       qi processing in SQL
	...
	if (retcode != SQL_ERROR)
	{
		strcpy(CmpQuery,"DELETE ALL FROM TABLE(NATABLECACHE('remove'))");
		retcode = CmpStmt->ExecDirect(NULL, CmpQuery, INTERNAL_STMT, TYPE_UNKNOWN, SQL_ASYNC_ENABLE_OFF, 0);
	}
...
void __cdecl SRVR::BreakDialogue(CEE_tag_def monitor_tag)
{
	...
	if(srvrGlobal->stopTypeFlag == STOP_WHEN_DISCONNECTED || inDcsStopMode)
	{
		odbc_SQLSvc_StopServer_exc_ StopException;
		StopException.exception_nr=0;
		if (srvrGlobal->traceLogger != NULL)
		...
		if(inDcsStopMode)
		{
			MXO_WARN("exit now for dcsstop and no io after 10s AutoCommitOn is %d",int(srvrGlobal->bAutoCommitOn));
		}
		exitServerProcess();
	}
...
{
	SESSION_DEBUG("odbc_SQLSrvr_Close_ame_ entry");
...
{
    int rc = 0;
    rc = SRVR::WSQL_EXEC_CancelOperation(transid);
    return rc;
}
//LCOV_EXCL_STOP
  ...
  )
{
	...
	SESSION_DEBUG("odbc_SQLSrvr_Prepare_ame_ entry");
#ifdef PERF_TEST
	perf->init();
...
//LCOV_EXCL_STOP
	...
	odbc_SQLSrvr_Prepare_ts_res_(objtag_, call_id_, returnCode, sqlWarningOrErrorLength,
		...
		outputDescLength, outputDesc,pSrvrStmt->comp_stats_info.dop, (pSrvrStmt != NULL) ? strlen(pSrvrStmt->sqlUniqueQueryID) : 0,pSrvrStmt->sqlUniqueQueryID);
...
{
	SESSION_DEBUG("odbc_SQLSrvr_Fetch_ame_ entry");
	...
	if (srvrGlobal->fetchAhead && pSrvrStmt->sqlStmtType != TYPE_SELECT_CATALOG)
	{
		...
		if (!firstFetch)
		{
			...
			odbc_SQLSrvr_Fetch_ts_res_( objtag_
							...
							, outputDataValue._length
							, outputDataValue._buffer
                     , (pSrvrStmt != NULL) ? pSrvrStmt->isClosed : IDL_TRUE);
...
//LCOV_EXCL_STOP
	...
	if (pSrvrStmt->sqlNewQueryType == SQL_SP_RESULT_SET)
	{
		...
		{
			if (resStatStatement != NULL) {
                        	if (returnCode == SQL_ERROR && sqlWarningOrError != NULL) 
				{
...
{
	SESSION_DEBUG("odbc_SQLSrvr_ExecDirect_ame_ entry");
...
//LCOV_EXCL_STOP
			...
			else
			{
						...
						if(!executed)
						{
							if (rcPlan == false)
							{
                                ...
                                if(sqlQueryType == SQL_OTHER && srvrGlobal->executeApiType == EXECUTEQUERY)
                                {
                                    returnCode = SQL_ERROR;
                                    GETMXCSWARNINGORERROR(-1, "HY000","Non-Select SQL statement is invalid in executeQuery", &sqlWarningOrErrorLength, sqlWarningOrError);
                                    // Clear diagnostics
                                    WSQL_EXEC_ClearDiagnostics(NULL);
                                    goto cfgerrexit;
                                }
                                ...
                                if (pSrvrStmt != NULL) {
                                    ...
                                    {
                                        returnCode = SQL_ERROR;
                                        GETMXCSWARNINGORERROR(-1, "HY000", "Invalid param Values", &sqlWarningOrErrorLength, sqlWarningOrError);
                                        // Clear diagnostics
                                        WSQL_EXEC_ClearDiagnostics(NULL);
                                        goto cfgerrexit;
                                    }
								   ...
								   returnCode == SQL_SUCCESS_WITH_INFO)
								{
									...
									if (srvrGlobal->isShapeLoaded == false)
									{
                                        ...
                                        if (queryTimeout != 0)
                                        {
                                            ...
                                            CEE_TIMER_CREATE2(queryTimeout, 0, QueryTimerExpired, (CEE_tag_def)pSrvrStmt, &queryTimerHandle, srvrGlobal->receiveThrId);
                                        }

										DO_WouldLikeToExecute(pSrvrStmt, &returnCode, &sqlWarningOrErrorLength, sqlWarningOrError);
										if (returnCode == SQL_SUCCESS || returnCode == SQL_SUCCESS_WITH_INFO)
										{
											...
											else
											{
												if((inputRowCnt > 0) || sqlQueryType != SQL_UNKNOWN  || (inputRowCnt == 0 && pSrvrStmt != NULL &&
                                                    ...
                                                    odbc_SQLSvc_Execute2_sme_(objtag_, call_id_, dialogueId, sqlAsyncEnable, queryTimeout,
                                                            ...
                                                            &rowsAffected, &outValuesLength, outValues);
                                                //do fetch
                                                IDL_long fetchSize = (rowLength > 0 && rowLength <100)?rowLength:100;
                                                bool fetch = (Int64)fetchSize*pSrvrStmt->outputDescVarBufferLen <= (Int64)1024*1024*1024*2;
                                                if(tmpPrepareRC == SQL_SUCCESS && returnCode == SQL_SUCCESS &&
                                                        fetch && srvrGlobal->drvrVersion.componentId == JDBC_DRVR_COMPONENT &&
                                                        srvrGlobal->drvrVersion.majorVersion >= 6 && 
                                                        pSrvrStmt->sqlQueryType == SQL_SELECT_NON_UNIQUE && 
                                                        pSrvrStmt->comp_stats_info.dop <= 1)
                                                {
                                                    /*do first fetch*/
                                                    IDL_long  outValuesFormat         = UNKNOWN_DATA_FORMAT;
                                                    SQL_DataValue_def outputDataValueFirst = {0,0};
                                                    odbc_SQLSrvr_FetchPerf_sme_(objtag_,
                                                            call_id_,
                                                            &returnCode,
                                                            dialogueId,
                                                            pSrvrStmt->stmtName,
                                                            fetchSize,
                                                            0,              //max row len,fisrt time
                                                            sqlAsyncEnable,
                                                            queryTimeout,
                                                            &rowsAffected,
                                                            &outValuesFormat,
                                                            &outputDataValueFirst,
                                                            &sqlWarningOrErrorLength,
                                                            sqlWarningOrError,
                                                            false);

                                                    qrysrvc_ExecuteFinished(NULL, (long)pSrvrStmt, false, returnCode, true);
                                                    outValuesLength = outputDataValueFirst._length;
                                                    outValues = (BYTE *)outputDataValueFirst._buffer;

                                                    if (srvrGlobal->fetchAhead && pSrvrStmt->sqlStmtType != TYPE_SELECT_CATALOG){
                                                        //do for next time of fetch
                                                        //we need anthoer outputDataValue here to store the fetch result
                                                        SQL_DataValue_def outputDataValue = {0,0};
                                                        IDL_long delayedRowsAffected = 0;
                                                        if (returnCode == SQL_SUCCESS || returnCode == SQL_SUCCESS_WITH_INFO)
                                                        {
                                                            odbc_SQLSrvr_FetchPerf_sme_(objtag_,
                                                                    call_id_,
                                                                    ...
                                                                    pSrvrStmt->stmtName,
                                                                    fetchSize,
                                                                    pSrvrStmt->maxRowLen?pSrvrStmt->maxRowLen:0,
                                                                    sqlAsyncEnable,
                                                                    queryTimeout,
                                                                    &delayedRowsAffected,
                                                                    &outValuesFormat,
                                                        ...
                                                        {
                                                            pSrvrStmt->returnCodeForDelayedError = returnCode;
                                                            pSrvrStmt->delayedRowsAffected = delayedRowsAffected;
                                                            //Daniel - if ahead fetch got no data found return code, do not use old data buffer for sending useless data.
                                                            if(returnCode==SQL_NO_DATA_FOUND){
                                                                pSrvrStmt->delayedOutputDataValue._buffer=NULL;
                                                                pSrvrStmt->delayedOutputDataValue._length=0;
	...
	else
	{
		...
		if(returnCode == SQL_SUCCESS ||
		   returnCode == SQL_SUCCESS_WITH_INFO)
		{
            if (pSrvrStmt != NULL && pSrvrStmt->paramCount > 0)
            {
                returnCode = SQL_ERROR;
                GETMXCSWARNINGORERROR(-1, "HY000", "Invalid param Values", &sqlWarningOrErrorLength, sqlWarningOrError);
                // Clear diagnostics
...
cfgerrexit:
    ...
    if(pSrvrStmt != NULL){
		Mds.GetApiEndTs();
        IDL_long_long transId = -1;
        SQL_EXEC_GetTransactionId(&transId);
        Mds.ExecuteMsg(pSrvrStmt,pSrvrStmt->sqlQueryType,returnCode,sqlString, transId);
		//	MXO_WARN("ExecDirect api cost time:%lld ms sendMdsMsg api cost time:%lld ms queuesize:%d[returnCode:%d  sqlString:%s queryId:%s]",querycosttime,msgcosttime,msgQueue.size(),returnCode,sqlString,pSrvrStmt->sqlUniqueQueryID);
    }
  ...
  )
{
	...
	SESSION_DEBUG("odbc_SQLSrvr_Execute2_ame_ entry");
...
//LCOV_EXCL_STOP
				...
				if (srvrGlobal->isShapeLoaded == false)
				{
					...
					{	
						...
						else
						{
                            ...
                            /* if sqlQueryType == SQL_SELECT_NON_UNIQUE, outValues will be null in execute, here we expect to use this value
                             * to send the first fetch result for saving one client fetch request*/
                            /** ingore success with info here
                             *  if dop > 1 don't fetch at a time
			     *  only used in jdbc driver
			     */
                            IDL_long fetchSize = (rowLength > 0 && rowLength <100)?rowLength:100;
                            bool fetch = (Int64)fetchSize*pSrvrStmt->outputDescVarBufferLen <= (Int64)1024*1024*1024*2;
                            if(fetch && srvrGlobal->drvrVersion.componentId == JDBC_DRVR_COMPONENT && 
                            srvrGlobal->drvrVersion.majorVersion >= 4 && pSrvrStmt->sqlQueryType == SQL_SELECT_NON_UNIQUE && returnCode == SQL_SUCCESS && pSrvrStmt->comp_stats_info.dop <= 1){
                                    /*do first fetch*/
                                    IDL_long  outValuesFormat         = UNKNOWN_DATA_FORMAT;
                                    SQL_DataValue_def outputDataValueFirst = {0,0};
                                    odbc_SQLSrvr_FetchPerf_sme_(objtag_,
    ...
    if(pSrvrStmt != NULL) {
        ...
        IDL_long_long transId = -1;
        SQL_EXEC_GetTransactionId(&transId);
        Mds.ExecuteMsg(pSrvrStmt,pSrvrStmt->sqlQueryType,returnCode,pSrvrStmt->sqlString, transId);
			//MXO_WARN("execute2 api cost time:%lld ms sendMdsMsg api cost time:%lld ms queuesize:%d[returnCode:%d  sqlString:%s queryId:%s]",querycosttime,msgcosttime,msgQueue.size(),returnCode,pSrvrStmt->sqlString,pSrvrStmt->sqlUniqueQueryID);
    }
	qrysrvc_ExecuteFinished(NULL,stmtHandle, true, returnCode, false);
	if (noRepository == false) {
		if ((resStatStatement != NULL) && (pSrvrStmt->stmtType == EXTERNAL_STMT)) // if statement is on
...
//LCOV_EXCL_START
	...
	if (!isStatusRowsetDelayed)
	{
      ...
      {
			estimatedCost = pSrvrStmt->rowsAffectedHigherBytes; // combine both rowsAffected and rowsAffectedHigherBytes as __int64 when interface between drvr/srvr changes
         SRVR::WSQL_EXEC_GetStatementHeapSize(&pSrvrStmt->stmt, &heapSize);
      }
			odbc_SQLSrvr_Execute_ts_res_(objtag_,
		         call_id_,
                 returnCode,
  ...
  , /* In    */ IDL_string optionValueStr)
{
	...
	SESSION_DEBUG("odbc_SQLSrvr_SetConnectionOption_ame_  entry");
  ...
  )
{
	...
	SESSION_DEBUG("odbc_SQLSrvr_EndTransaction_ame_   entry");
	SRVRTRACE_ENTER(FILE_AME+20);
	odbc_SQLSvc_EndTransaction_exc_ exception_={0,0,0};
	ERROR_DESC_LIST_def   sqlWarning = {0,0};
	RETCODE rc = 0;
	Int64 trans_id;
	WSQL_EXEC_GetTransactionId(&trans_id);
	if (srvrGlobal->traceLogger != NULL)
	{
	...
	odbc_SQLSrvr_EndTransaction_ts_res_(objtag_, call_id_, &exception_, &sqlWarning, trans_id);
    ...
    if (srvrGlobal->exitAfterEndTran == true)
    {
        MXO_WARN("mxosrvr exit after endtran");
        exitServerProcess();
    }
        ...
        )
{
    ...
    SRVRTRACE_ENTER(FILE_AME+20);
    odbc_SQLSvc_Savepoint_exc_ exception_={0,0,0};
    ERROR_DESC_LIST_def   sqlWarning = {0,0};
    RETCODE rc = 0;
    if (srvrGlobal->traceLogger != NULL)
    {
        srvrGlobal->traceLogger->TraceSavepointEnter(dialogueId, savepointOpt,savepointNameLength,savepointName);
    }
    if (srvrGlobal != NULL && srvrGlobal->srvrType == CORE_SRVR)
    {
        if (srvrGlobal->srvrState == SRVR_CONNECTED)
        {
            if (dialogueId != srvrGlobal->dialogueId)
                exception_.exception_nr = odbc_SQLSvc_Savepoint_InvalidConnection_exn_;
            else
                odbc_SQLSvc_Savepoint_sme_(objtag_, call_id_, &exception_, dialogueId,
                        savepointOpt,savepointNameLength,savepointName,&sqlWarning);
        }
        else
            exception_.exception_nr = odbc_SQLSvc_Savepoint_InvalidConnection_exn_;
    }
        ...
        odbc_SQLSvc_Savepoint_sme_(objtag_, call_id_, &exception_, dialogueId,
                savepointOpt,savepointNameLength,savepointName,&sqlWarning);
    odbc_SQLSrvr_Savepoint_ts_res_(objtag_, call_id_, &exception_, &sqlWarning);
    if (srvrGlobal->traceLogger != NULL)
    {
        srvrGlobal->traceLogger->TraceSavepointExit(exception_, sqlWarning);
    }
    SRVRTRACE_EXIT(FILE_AME+20);
}
  ...
  , /* In   */ IDL_long_long    extractLen)
{
    ...
    ExceptionStruct exception_ = {0, 0};
...
bool isInfoSystem(char*& sqlString, const IDL_char *stmtLabel, short& error)
{
   ...
   // get product portfolio info
   productEdition  = getenv("TRAFODION_VER_PROD"); 
   productVersion  = getenv("ESGYN_PRODUCT_VER");

   // get Timezone and GMT offset
   time_t tim     = time(NULL);
   struct tm *now = localtime(&tim);

   string databaseEdition =  (msg_license_advanced_enabled() ? "EsgynDB Advanced" : "EsgynDB Enterprise");

   //see ComAuthenticationType on ComSmallDefs.h
   // 0 - LDAP
   // 1 - NOT AUTH
   // 2 - LOCAL
   // 3 - NO AUTH
   Int32 authenticationType = 0;
   char str_AuthenticationType[64] = {0};
   bool authorizationEnabled = false;
   bool authorizationReady = false;
   bool auditingEnabled = false;
 
   Int32 rc = SQL_EXEC_GetAuthState(authenticationType,
   ...
   case 0:
   {
       strcat(str_AuthenticationType,"LDAP");
   }
   break;
   case 1:
   {
       strcat(str_AuthenticationType, "NO AUTH");
   }
   break;
   case 2:
   {
       strcat(str_AuthenticationType, "LOCAL");
   }
   break;
   case 3:
   default:
   {
       strcat(str_AuthenticationType, "SKIP_AUTHENTICATION");
   }
   break;
   }
   ...
   char pattern[] = "SELECT [first 1]"
                    ...
                    "'%d' as \"INSTANCE_ID\","
                    "'%d' as \"CLUSTER_ID\","
                    "'%s' as \"PRODUCT_EDITION\","
                    "'%s' as \"PRODUCT_VERSION\""
	            "FROM (values(1)) X(A);";
   ...
   sprintf (buffer, pattern,
            ...
            str_AuthenticationType,
            authorizationEnabled ? "true" : "false",
            srvrGlobal->traf_cluster_name,
            srvrGlobal->traf_instance_name,
            srvrGlobal->traf_instance_id,
            srvrGlobal->traf_cluster_id,
            productEdition,
            productVersion);
...
//   const char* format="%x - %I:%M%p";
   ...
   myEnvVar = getenv("LANG");
   if (myEnvVar != NULL) {
     pathcharset = strdup(myEnvVar); 
   }

   tmp = localtime(&connectedTimestamp);
   if (strftime(timestrtmp, sizeof(timestrtmp), format, tmp) == 0) {
      timestrtmp[0]=0;
   }

   if (pathcharset != NULL && (strcmp(pathcharset, "zh_CN.GBK") == 0))
   {
      iconv_t conv;
      char *input = timestrtmp;
      char *output = connecttime;
      size_t inlen = sizeof(timestrtmp);
      size_t outlen = sizeof(connecttime);

      conv = iconv_open("UTF-8", "GBK");
      iconv(conv, &input, &inlen, &output, &outlen);
      iconv_close(conv);
   }
   else
   {
      strcpy(connecttime, timestrtmp);
   }
...
bool updateZKState(DCS_SERVER_STATE currState, DCS_SERVER_STATE newState, char *currData, int currDataLen)
{
   int rc = ZOK;
   stringstream ss;
   Stat stat;
   ...
   if( currState == INITIAL && newState == AVAILABLE )   // change stat from STARTING to AVAILABLE
   { 
      checkZKHandle(zh);
   ...
   else if( currState == INITIAL && newState == DISABLE )   // change stat from STARTING to DISABLE
   {
      checkZKHandle(zh);
      rc = zoo_exists(zh, dcsRegisteredNode.c_str(), 0, &stat);
      if( rc == ZOK )
   ...
   else if( currState == DISABLE && newState == AVAILABLE )   // change stat from DISABLE to AVAILABLE
   {
      checkZKHandle(zh);
      rc = zoo_exists(zh, dcsRegisteredNode.c_str(), 0, &stat);
      if( rc == ZOK )
   ...
   else if( currState == CONNECTING && newState == CONNECTED )
   {
...
*/
         ...
         if (inet_ntop(AF_INET, &clientaddr.sin_addr, str, sizeof(str)))
         {
            ...
            strncpy(srvrGlobal->ClientIpAddress, str, sizeof(srvrGlobal->ClientIpAddress) - 1);
            srvrGlobal->ClientIpAddress[sizeof(srvrGlobal->ClientIpAddress) -1] = '\0';
            srvrGlobal->ClientPort = clientaddr.sin_port;
            ss << "CONNECTED"
               << ":"
               << JULIANTIMESTAMP()
               << ":"
   ...
   if( currState == CANCEL_OPERATION && newState == CONNECTED)   // Move from cancel to connected
   {
       checkZKHandle(zh);
       rc = zoo_exists(zh, dcsRegisteredNode.c_str(), 0, &stat);
       if( rc == ZOK )
       {
           string data;
           if (currData != NULL && currDataLen > 0)
           {
               data = currData;
               size_t found = data.find_first_of(":");
               if (found != std::string::npos)
                 data.replace(0, found, "CONNECTED");
               else
                 data.replace(0, 6, "CONNECTED");
           }
           else
           {
               ss << "CONNECTED"
...
{
public:
	mException(const string m="mException"):msg(m){}
...
void query_watcher(zhandle_t *zzh, int type, int state, const char *path, void* watcherCtx){
	int rc;
	stringstream ss;
...
string cost_info(SRVR_STMT_HDL *pSrvrStmt){
	stringstream ss;

	ss << "{\"est_cpuTime\":\"" << pSrvrStmt->cost_info.cpuTime << "\",";
	ss << "\"est_ioTime\":\"" << pSrvrStmt->cost_info.ioTime << "\",";
	ss << "\"est_msgTime\":\"" << pSrvrStmt->cost_info.msgTime << "\",";
	ss << "\"est_idleTime\":\"" << pSrvrStmt->cost_info.idleTime << "\",";
	ss << "\"est_numSeqIOs\":\"" << pSrvrStmt->cost_info.numSeqIOs << "\",";
	ss << "\"est_numRandIOs\":\"" << pSrvrStmt->cost_info.numRandIOs << "\",";
	ss << "\"est_cost\":\"" << pSrvrStmt->cost_info.totalTime << "\",";
	ss << "\"est_totalTime\":\"" << pSrvrStmt->cost_info.totalTime << "\",";
	ss << "\"est_cardinality\":\"" << pSrvrStmt->cost_info.cardinality << "\",";
	ss << "\"est_estimatedTotalMem\":\"" << pSrvrStmt->cost_info.estimatedTotalMem << "\",";
	ss << "\"est_resourceUsage\":\"" << pSrvrStmt->cost_info.resourceUsage << "\",";
	ss << "\"est_maxCpuUsage\":\"" << pSrvrStmt->cost_info.maxCpuUsage << "\"}";
	return ss.str();
}

short isGoingThroughWms(SRVR_STMT_HDL *pSrvrStmt){

	Stat stat;

	if (pSrvrStmt->m_bSkipWouldLikeToExecute == true || srvrGlobal->goingWMSStatus == false)
		return FALSE;

 	// Added for short running queries workaround to get the compiler estimates
	// from SLA and bypass WMS for these estimates as well as skip generating 21036 messages.

	if( srvrGlobal->estCardinality != -1 && srvrGlobal->estCost != -1 )
	{
		if( pSrvrStmt->cost_info.cardinality <= srvrGlobal->estCardinality &&
		    pSrvrStmt->cost_info.totalTime <= srvrGlobal->estCost )
			return FALSE;
	}
	else
	if( (srvrGlobal->estCardinality != -1 &&
	     pSrvrStmt->cost_info.cardinality <= srvrGlobal->estCardinality) ||
		(srvrGlobal->estCost != -1 &&
		pSrvrStmt->cost_info.totalTime <= srvrGlobal->estCost) )
		return FALSE;
...
/*
  ...
  SQL_SET_SCHEMA = 12,
									...
									SQL_INSERT_RWRS = 16,
									SQL_CAT_UTIL = 17,				purgedata, modify, dup, transform... etc
									SQL_EXE_UTIL = 18,				maintain, reorg, cleanupVolTab
									SQL_SELECT_UNLOAD = 19

*/

	long sqlQryType;
	if( pSrvrStmt->sqlNewQueryType != SQL_UNKNOWN )
		sqlQryType = pSrvrStmt->sqlNewQueryType;
	else
		sqlQryType = pSrvrStmt->sqlQueryType;
	...
	switch(pSrvrStmt->sqlSubQueryType)
	{
	...
	char* tmp;
	for(tmp = pSrvrStmt->sqlString; tmp[0] && isspace(tmp[0]); tmp++);

	if (strnicmp(tmp,"showcontrol", 11) == 0) return FALSE;
	if (strnicmp(tmp,"values ", 7) == 0) return FALSE;
	if (strnicmp(tmp,"info ", 5) == 0) return FALSE;
	if (strnicmp(tmp,"get ", 4) == 0) return FALSE;
	if (strnicmp(tmp,"set ", 4) == 0) return FALSE;

        checkZKHandle(zh);
	int rc = zoo_exists(zh, znodeWmsResourcesRegistered.c_str(), NULL, &stat);
	if(rc != ZOK)
		return FALSE;
...
int BuildQueryRejectedMessage(
	...
	stringstream ss;
	ss << masg_header << msg_detail;
	EventLog(ss.str().c_str());
	GETMXCSWARNINGORERROR(-1, "HY000", (char*)ss.str().c_str(), sqlWarningOrErrorLength, sqlWarningOrError);
	*returnCode = SQL_ERROR;
	return false;
}
...
short DO_WouldLikeToExecute(
	...
	struct timespec to;
	int TIMEOUT = 30;
	bool etimedout = false;
	bool runagain = false;
	qcond = ZOO_NOTWATCHING_EVENT;
	...
	if (srvrGlobal->sqlPlan)
		getSQLInfo( EXPLAIN_PLAN, (long int)pSrvrStmt);
	if (pSrvrStmt->sqlUniqueQueryID == NULL || pSrvrStmt->sqlUniqueQueryID[0] == '\0' )
		return 0;

RUNAGAIN:
	ss.str("");
	try {
		if(isGoingThroughWms(pSrvrStmt)){
			TIMEOUT = (wmsTimeout <= 30 )? 30 : wmsTimeout;
			TIMEOUT = (TIMEOUT >= 180 )? 180 : TIMEOUT;
			ss << pSrvrStmt->sqlUniqueQueryID << ":" << srvrGlobal->HostName << ":" << serverProcessName << ":" << srvrGlobal->nskProcessInfo.nodeId << "." << srvrGlobal->nskProcessInfo.processId;
			znodeWmsResourcesQueriesSlaQID = znodeWmsResourcesQueriesSla + '/' + ss.str();
                        checkZKHandle(zh);
			rc = zoo_exists(zh, znodeWmsResourcesQueriesSlaQID.c_str(), NULL, &stat);
			ss.str("");
			if( rc == ZOK ){
                                checkZKHandle(zh);
				rc = zoo_delete(zh, znodeWmsResourcesQueriesSlaQID.c_str(), -1);
				if( rc != ZOK ){
					ss << "*** QID already exists and zoo_delete failed with error " << rc << " [" << znodeWmsResourcesQueriesSlaQID << "]";
					throw mException(ss.str());
				}
			}
			qcond = ZOO_NOTWATCHING_EVENT;
			//qidData = "STARTING:" + sla + ":" + "query_proc.py" + ":" + connectProfile + ":" + disconnectProfile + ":" + userName;
			qidData = "STARTING:" + cost_info(pSrvrStmt);
                        checkZKHandle(zh);
			rc = zoo_create(zh, znodeWmsResourcesQueriesSlaQID.c_str(), qidData.c_str(), qidData.length(), &ZOO_OPEN_ACL_UNSAFE, ZOO_EPHEMERAL, NULL, 0);
			if( rc != ZOK ){
				ss << "***** zoo_create() failed with error " << rc << " [" << znodeWmsResourcesQueriesSlaQID << "]";
				throw mException(ss.str());
			}
			zkDataLen = -1;
                        checkZKHandle(zh);
			rc = zoo_wget(zh, znodeWmsResourcesQueriesSlaQID.c_str(), query_watcher, NULL, NULL, &zkDataLen, &stat);
			if( rc != ZOK ){
				ss << "***** zoo_get() failed with error " << rc << "[" << znodeWmsResourcesQueriesSlaQID << "]";
				throw mException(ss.str());
			}
			rc = pthread_mutex_lock(&m);
			if( rc != 0){
				ss << "***** pthread_mutex_lock() failed with error " << rc ;
				throw mException(ss.str());
			}
			while (qcond == ZOO_NOTWATCHING_EVENT){
				to.tv_sec = time(NULL) + TIMEOUT;
				to.tv_nsec = 0;
				rc = pthread_cond_timedwait(&c, &m, &to);
				if( rc == 0){
					break;
					...
					if( rc == ZOK ){
						...
						if( rc != 0){
							...
							ss << "***** zoo_delete failed with error " << rc << " [" << znodeWmsResourcesQueriesSlaQID;
							throw mException(ss.str());
						}
					} else
						qcond = ZOO_DELETED_EVENT;
				} else {
					pthread_mutex_unlock(&m);
					ss << "***** pthread_cond_wait() failed with error " << rc;
					throw mException(ss.str());
				}
			}
			rc = pthread_mutex_unlock(&m);
			if( rc != 0){
				ss << "***** pthread_mutex_unlock() failed with error " << rc ;
				throw mException(ss.str());
			}
			if(qcond == ZOO_CHANGED_EVENT){

				zkDataLen = sizeof(zkData) - 1;
                                checkZKHandle(zh);
				rc = zoo_get(zh, znodeWmsResourcesQueriesSlaQID.c_str(), false, zkData, &zkDataLen, &stat);
				if( rc != ZOK ){
					ss << "***** zoo_get() failed with error " << rc << " [" << znodeWmsResourcesQueriesSlaQID + "]";
					throw mException(ss.str());
				}
				zkData[zkDataLen] = 0;
				if(strncmp(zkData, "REJECTED:", 9) == 0){
					return BuildQueryRejectedMessage(
							"Query rejected. "
							, &zkData[9]
							, pSrvrStmt
							, returnCode
				...
				if(strncmp(zkData, "WAITING:", 8) == 0){
                                        checkZKHandle(zh);
					rc = zoo_delete(zh, znodeWmsResourcesQueriesSlaQID.c_str(), -1);
					int sec = atoi( &zkData[8] );
					if (sec < 10) sec = 10;
					if (sec > 180) sec = 180;
					ss.str("");
					ss << znodeWmsResourcesQueriesSlaQID << ". Query waits " << sec << " seconds. After that will be sent again to the WMS for admition.";
					EventLog(ss.str().c_str());
					sleep(sec);
					runagain = true;
				}
			}
			else if( qcond == ZOO_DELETED_EVENT){
				if( etimedout == TRUE ){
                                        checkZKHandle(zh);
					rc = zoo_delete(zh, znodeWmsResourcesQueriesSlaQID.c_str(), -1);
					return BuildQueryRejectedMessage(
							"Query rejected. "
							, "WMS timeout occurred."
							, pSrvrStmt
							, returnCode
							, sqlWarningOrErrorLength
							, sqlWarningOrError);
		...
		} //End of if for isGoingThroughWms condition
	} catch(mException& e){
		SendEventMsg(MSG_ODBC_NSK_ERROR, EVENTLOG_ERROR_TYPE,
				0, ODBCMX_SERVER, srvrGlobal->srvrObjRef,
				1, e.what());
	...
	if (srvrGlobal->sqlPlan)
	{
		if (! getSQLInfo( EXPLAIN_PLAN, (long int)pSrvrStmt ))
		{
...
void dcsServerNodeWatcher(zhandle_t *zh, int type, int state, const char *path, void* watcherCtx)
{
    char zkData[1024] = {0};
    int zkDataLen = sizeof(zkData) - 1;
    Stat stat;

    int rc = zoo_get(zh, path, 0, zkData, &zkDataLen, &stat);
    if (rc != ZOK)
    {
        MXO_WARN("get data from dcsServer Znode error");
        if(!updateZKState(DISABLE, AVAILABLE))
            MXO_WARN("update zookeeper for node enable failed");
        return;
    }

    if (type == ZOO_CHANGED_EVENT)
    {
        if (zkDataLen != 0 && stricmp(zkData, "ENABLE") == 0)
        {
            if(!updateZKState(DISABLE, AVAILABLE))
                MXO_WARN("update zookeeper for node enable failed");
        }
        ...
        {
            zoo_wexists(zh, path, dcsServerNodeWatcher, NULL, &stat);
        }
    }
    else
    {
...
bool checkNodeEnable()
{
    Stat stat;
    bool rc = true;

    string dcsServerNode;
    ...
    stringstream ss;
    ss.str("");
    ss << zkRootNode << "/dcs/servers/running/" << srvrGlobal->HostName;
    dcsServerNode = ss.str();

    char zkData[1024] = {0};
    int zkDataLen = sizeof(zkData) - 1;

    if (zoo_get(zh, dcsServerNode.c_str(), false, zkData, &zkDataLen, &stat) == ZOK)
    {
        if (zkDataLen != 0 && strncmp(zkData, "DISABLE", 7) == 0)
        {
            free_String_vector(&children);
            rc = false;
        }
...
void deleteZnode(string znode){
	...
	try {
                checkZKHandle(zh);
		int rc = zoo_exists(zh, znode.c_str(), NULL, &stat);
		if( rc == ZOK ){
                        checkZKHandle(zh);
			rc = zoo_delete(zh, znode.c_str(), -1);
			if( rc != ZOK ){
				ss << "*** zoo_delete failed with error " << rc << " [" << znode << "]";
				throw mException(ss.str());
			}
		}
	} catch(mException& e){
		...
		)
{
	...
	if (pSrvrStmt->m_bqueryFinish)
	{
		...
		string znode;
                ...
                {
		     ss << pSrvrStmt->sqlUniqueQueryID << ":" << srvrGlobal->HostName << ":" << serverProcessName << ":" << srvrGlobal->nskProcessInfo.nodeId << "." << srvrGlobal->nskProcessInfo.processId;
		     znode = znodeWmsResourcesQueriesSla + '/' + ss.str();
		     deleteZnode(znode);
                }
...
void __cdecl SRVR::QueryTimerExpired(CEE_tag_def pstmt)
{
    SRVR_STMT_HDL *pSrvrStmt = (SRVR_STMT_HDL *)pstmt;
    if (pSrvrStmt != NULL)
    {
        Int64 trans_id = -1;
        SQL_EXEC_GetTransactionId(&trans_id);
        if (trans_id != -1)
            WSQL_EXEC_CancelOperation(trans_id);
        int rc = WSQL_EXEC_Cancel(&pSrvrStmt->stmt);
        MXO_WARN("cancel status:%d", rc);
    }
}
