  ...
  )
{
    ...
    CTCPIPSystemSrvr* ipnode = (CTCPIPSystemSrvr*)objtag_;
    ...
    IDL_long optVal = 0;
	IDL_long bodyLen = 0;
    IDL_char * optStrVal = NULL;
    IDL_long optStrValLen = 0;
    bodyLen = ipnode->m_rhdr.total_length;
...
//LCOV_EXCL_STOP
    ...
    if(bodyLen > inputPosition)
    {

        optNum = *(IDL_long *)(curptr + inputPosition);

        inputPosition += sizeof(optNum);


        // |---optNum---|--optType1---|---optVal1---|--optType2---|---optVal2---|......
        while(optNum--)
        {
            optType = *(IDL_long *)(curptr + inputPosition);
            inputPosition += sizeof(optType);
            switch(optType)
            {
                case XATTRS_SET_CLIPVARCHAR:
                    optVal = *(IDL_long *)(curptr + inputPosition);
                    inputPosition += sizeof(optVal);
                    srvrGlobal->clipVarchar = optVal;
		    if(optVal > 0)
			    MXO_WARN("clipVarchar is on! Value = %d",optVal);
                    break;
                case XATTRS_SET_TEST:
                    optStrValLen = *(IDL_long*)(curptr+inputPosition);
                    inputPosition += sizeof(optStrValLen);
                    if(optStrValLen > 0)
                    {
                        optStrVal = curptr+inputPosition;
                        inputPosition += optStrValLen;
                    }
                    break;
				case XATTRS_SET_SESSION_DEBUG:
                    optVal = *(IDL_long *)(curptr + inputPosition);
                    inputPosition += sizeof(optVal);
					sessionDebug = optVal;
					if(optVal > 0)
        ...
        // Do the default operation without extended attribute settings
        ...
        )
{
    ...
    IDL_long inputPosition = 0;
    DIALOGUE_ID_def dialogueId;
    IDL_unsigned_short savepointOpt;
    IDL_long      savepointNameLength = 0;
    IDL_char      *savepointName = NULL;
    curptr = pnode->r_buffer();
    dialogueId = *(IDL_long*)(curptr+inputPosition);
    inputPosition += sizeof(dialogueId);
    savepointOpt = *(IDL_unsigned_short*)(curptr+inputPosition);
    inputPosition += sizeof(savepointOpt);
    savepointNameLength     = *(IDL_long*)(curptr+inputPosition);
    inputPosition += sizeof(savepointNameLength);
    if (savepointNameLength > 0)
    {
        savepointName = curptr+inputPosition;
        inputPosition += savepointNameLength;
    }
...
{
    currentTimeMillis(enterMxoTimestamp);
  CInterface    *pnode = (CInterface*)objtag_;
  ...
  )
{
    ...
    curptr = pnode->r_buffer();
    totalLength = pnode->r_buffer_length();
    ...
    extractLen = *(IDL_long_long *)(curptr + inputPosition);
    inputPosition += sizeof(extractLen);

    stmtHandleKey = *(IDL_long *)(curptr + inputPosition);
    inputPosition += sizeof(stmtHandleKey);
  ...
  )
{
    ...
    IDL_char *restoreFlag = NULL;
    IDL_long  restoreFlagLength = 0;
    DIALOGUE_ID_def dialogueId = 0;
    ...
    // 2nd Param: Restore flag
    restoreFlagLength = *(IDL_long*)(curptr+inputPosition);
    inputPosition += sizeof(restoreFlagLength);

    if(restoreFlagLength > 0)
    {
        restoreFlag = curptr + inputPosition;
        inputPosition += restoreFlagLength;
    }


    odbc_SQLSrvr_CheckRestore_ts_res_(
			objtag_
		  , call_id_
		  , dialogueId
		  , restoreFlag
...
//LCOV_EXCL_STOP
    ...
    if(bodyLen > inputPosition)
    {
        int executeApiType  = *(IDL_long *)(curptr + inputPosition);
        inputPosition += sizeof(executeApiType);
        srvrGlobal->executeApiType = executeApiType;
    
    }
    else{
        srvrGlobal->executeApiType = 0;
    }
...
static Int32 costTh = -2;
static Int32 costThTrans = -2;
static pid_t pid = getpid();
static Int32 callCount[64] = {0};
static double sumCost[64] = {0};
static Int64 curTransId = -1;
static Int32 transCount = 0;
static Int64 transCost = 0;
static map<string, Int32> transCountMap;
static map<string, Int64> transCostMap;

Int32 getTransCount(char *transType) {
  Int32 count = 0;
  if (transType == NULL)
    return 0;

  string key(transType);
  map<string, Int32>::iterator it = transCountMap.find(key);
  if (it == transCountMap.end())
  {
    transCountMap.insert(pair<string, Int32>(key, 0));
  }
  else
  {
    count = it->second;
...
Int32 transCountPlus(char *transType) {
  Int32 count = 0;
  if (transType == NULL)
    return count;
  ...
  if (it == transCountMap.end()) 
  { 
    transCountMap.insert(pair<string, Int32>(key, 1));
    count = 1;
  }
  else
  {
    count = it->second;
    transCountMap.erase(it);
    count++;
    transCountMap.insert(pair<string, Int32>(key, count));
  }
  return count;
}

Int64 getTransCost(char *transType) {
  Int64 cost = 0;
  if (transType == NULL)
    return cost;

  string key(transType);
  map<string, Int64>::iterator it = transCostMap.find(key);
  if (it == transCostMap.end()) 
  { 
    transCostMap.insert(pair<string, Int64>(key, 0L));
  }
  else
  {
    cost = it->second;
...
Int64 transCostPlus(char *transType, Int64 increment) {
  Int64 cost = 0;
  if (transType == NULL)
    return increment;
  ...
  if (it == transCostMap.end())
  {
    transCostMap.insert(pair<string, Int64>(key, increment));
    cost = increment;
  }
  else
  {
    cost = it->second;
    transCostMap.erase(it);
    cost += increment;
    transCostMap.insert(pair<string, Int64>(key, cost));
  }
  return cost;
}

Int64 DISPATCH_GetTransactionId() {
  Int64 transId = -1;
  SQL_EXEC_GetTransactionId(&transId);
  if (transId == -1 || transId != curTransId)
    { 
      if (curTransId != -1 && costThTrans >= 0)
      {
        Int32 totalCount = 0, transCount1 = 0;
        double totalCost = 0, transCost1 = 0;
        
        char *tranType = SQL_EXEC_GetClientInfo();
        for (Int32 idx = 0; idx < 64; idx++)
        {
          totalCount += callCount[idx];
          totalCost += sumCost[idx];
        }
        ...
        {
          transCost += totalCost;
          transCost1 = transCost;
          transCount1 = ++transCount;
        }
        else
        {
          transCost1 = transCostPlus(tranType + 16, totalCost);
          transCount1 = transCountPlus(tranType + 16);
        }
        if (totalCost > costThTrans && costThTrans >= 0)
        {
          char logMsg[1000];
          sprintf(logMsg, "DISPATCH_TCPIPRequest THIS_TRAN PID %d txID %ld SCC %ld SATC %.2f STTC %.2f TC %d TATC %.2f TTC %.2f %s", 
                  pid, curTransId, totalCount, totalCost / (totalCount ? totalCount : 1), 
                  totalCost, transCount1, transCost1 / (transCount1 ? transCount1 : 1), 
                  transCost1, (tranType != NULL ? tranType + 16 : "NO_INFO")); 
          MXO_WARN(logMsg);
        }
      }
      curTransId = transId;
...
)
{
	...
	if (costTh == -2)
	{
	  ...
	  if (costTh == -2)
	    costTh = -1;
          ...
          costThreshold = getenv("RECORD_TIME_COST_TRANS");
          if (costThreshold != NULL)
             costThTrans = atoi(costThreshold);
	}
	...
	if(inDcsStopMode)
	{	
		if(srvrGlobal->bAutoCommitOn)
		{
			//if not in transaction and  sql type is select, wait for all data has been fetched. otherwise exit
			if(SRVR_API_SQLFETCH != operation_id && SRVR_API_SQLFETCH_ROWSET != operation_id)
			{
				MXO_WARN("exit now for dcsstop and not in transaction operation id is %d",(int)operation_id);
				exitServerProcess();
			}
		}
	}
	...
	case SRVR_API_SQLENDTRAN:
		...
		break;
    case SRVR_API_SQLSAVEPOINT:
        SQLSAVEPOINT_IOMessage(objtag_, call_id_);
        break;
    ...
    case SRVR_API_UPDATELOB:
        ...
        break;
    case SRVR_API_TORESTORE:
        CHECKRESTORE_IOMessage(objtag_, call_id_);
        break;
    default:
//LCOV_EXCL_START
        break;
...
*/
    ...
    if (costTh >= 0 && operation_id > SRVR_API_START && operation_id <= SRVR_API_TORESTORE) {
        int  idx = (operation_id - SRVR_API_START);
        DISPATCH_GetTransactionId();
        timeCost = (JULIANTIMESTAMP() - timeCost) / 1000;
        callCount[idx]++;
        sumCost[idx] += timeCost;
        if (timeCost >= costTh) {
            sprintf(logMsg, "DISPATCH_TCPIPRequest %s PID %d txID %ld CC %d ATC %.2f TTC %.2f STC %ld ", 
                    OPERATION_NAME[idx], pid, curTransId, callCount[idx], 
                    sumCost[idx]/callCount[idx], sumCost[idx], timeCost);
            MXO_WARN(logMsg);
        }
    }
	...
	if(inDcsStopMode)
	{
		//if in transaction and sql type is commit or rollback,exit
		if(!srvrGlobal->bAutoCommitOn)
		{
			if(SRVR_API_SQLENDTRAN == operation_id)
			{
				MXO_WARN("exit now for dcsstop and after end transaction operation id is %d",(int)operation_id);
				exitServerProcess();
			}
		}
	}
...
void currentTimeMillis(int tmp[])
{
    struct timeval tv;
