                          ...
                          )
{
  ...
  if (sts == CEE_SUCCESS)
    {
    sts = pnode->send_response(buffer, message_length, call_id_);
    SESSION_DEBUG("odbc_SQLSrvr_Fetch_ts_res_ out send message_length is %d", message_length);
    }
  SRVRTRACE_EXIT(FILE_OIOM+11);
  return sts;
                          ...
                          )
{
  ...
  if (sts == CEE_SUCCESS)
    sts = pnode->send_response(buffer, message_length, call_id_);
  SESSION_DEBUG("odbc_SQLSrvr_Execute_ts_res_ out send message_length is %d", message_length);
  SRVRTRACE_EXIT(FILE_OIOM+11);
  return sts;
...
CEE_status
odbc_SQLSrvr_Savepoint_ts_res_(
        /* In    */ CEE_tag_def objtag_
        , /* In    */ const CEE_handle_def *call_id_
...
odbc_SQLSrvr_ExtractLob_ts_res_(
    /* In   */ CEE_tag_def    objtag_
  , /* In   */ const CEE_handle_def *call_id_
  , /* In   */ const struct ExceptionStruct *exception_
