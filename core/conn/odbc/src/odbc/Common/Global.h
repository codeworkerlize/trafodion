...
#define SQL_ATTR_TRANSACTION_TYPE    2002
...
#define SQLSVC_EXCEPTION_INVALID_SAVEPOINT_OPT  "Invalid SavepointOpt"
...
typedef struct _SRVR_GLOBAL_Def
{
	_SRVR_GLOBAL_Def(){
		traf_instance_id = 0;
		traf_cluster_id = 0;
		bzero(traf_instance_name,sizeof(traf_instance_name));
		bzero(traf_cluster_name,sizeof(traf_cluster_name));
		...
		m_stInternalQueriesInRepo = false;
        ...
        goingWMSStatus = true;      // this used for closing the way that  sla query? go through wms:function isGoingThroughWms()
                shmLimit = 70; // the percentage of rms share memory limit
        ...
        exitAfterEndTran = false;
        syncQueryForBinlog = 0;
		bzero(srvrObjRef,sizeof(srvrObjRef));

	}

        //for multi-instance
	int	traf_instance_id;
	int	traf_cluster_id;
	char	traf_instance_name[32];
	char	traf_cluster_name[32];
...
//
	...
	char				ApplicationName[MAX_APPLICATION_NAME_LEN + 1];
    char                ClientIpAddress[MAX_ADDRESS_LEN + 1];
    char                ClientUserName[MAX_USERNAME_LEN + 1];
    int                 ClientPort;
	...
	char                tenantName[MAX_APPLICATION_NAME_LEN + 1];
	int                 tenantID;
	int                 affinityNum;
	__int64             tenantRedefTime; // Store the last time this tenant information
	                                     // was changed
	bool				DSG;
...
//
	...
	bool	m_stInternalQueriesInRepo;
	...
	long				portNumber;
    int                 loadEnable;
    int                 encryptBase64Enable;
    bool                exitAfterEndTran;
    // Mantis-18082 add for set cpu bind while starting
    int                 cpuLimitPercentWhileStart;
