...
SET_ERROR(long signature, char platform, char transport, int api, ERROR_TYPE error_type, char* process, OPERATION operation, FUNCTION function, int error, int errordetail)
{
	...
	if(signature == 0)
		...
		ierror = gerror();
	else if (*(void **)signature == *(void **)&tnode)
	{
		// if signature is a valid CTCPIPSystemSrvr pointer, then its vtable pointer would be same as tnode
		ierror = &((CTCPIPSystemSrvr *)signature)->m_error;
		ierror->signature = signature;
	}
	else
...
SET_INFO(long signature, char platform, char transport, int api, ERROR_TYPE error_type, char* process, OPERATION operation, FUNCTION function, int error, int errordetail)
{
	...
	CError *ierror = signature ? GTransport.m_error_list.ins_error(signature) : gerror();
	if (ierror != NULL)
	{
		ierror->platform = platform;
		ierror->transport = transport;
...
FORMAT_ERROR(CError* ierror)
{
	...
	case O_PIPE:
		...
		break;
	case O_LOAD_METADATA:
		strcat(s_buffer,"LOAD_METADATA");
		break;
	default:
...
FORMAT_LAST_ERROR()
{
	...
	// 	return "";
	// return FORMAT_ERROR(ierror);
	return gerror()->signature ? FORMAT_ERROR(gerror()) : (char *)EMPTY_STRING;
}
...
LAST_ERROR_TO_TEXT()
{
	...
	return gerror()->signature ? ERROR_TO_TEXT(gerror()) : (char *)EMPTY_STRING;
}
