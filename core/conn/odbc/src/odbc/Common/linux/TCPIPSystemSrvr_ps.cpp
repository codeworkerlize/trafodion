...
short int CTCPIPSystemSrvr::getMsgHeader(char* buf, int cread, int bufSize, int headerSize, long wtimeout)
{
	...
	while (already_read < headerSize )
	{
		...
		if (buseSSLEnabled)
 	       {
  wcount = SSL_read(openssl, pbuf, inp_size);
  switch (SSL_get_error(openssl, wcount))
                     {
                     case SSL_ERROR_NONE:
                          break;
		...
		wcount = recv(m_nSocketFnum, pbuf, inp_size, 0);
	 }
   		if (wcount <= 0)
		{
...
bool CTCPIPSystemSrvr::do_read(bool bfirstblock, HEADER*& hdr, char*& buffer, int& bufcount, long timeout)
{
	...
	else
	{
		int wcount = -1;
            if (buseSSLEnabled)
            {
	           wcount = SSL_read(openssl, m_IObuffer, MAX_TCP_BUFFER_LENGTH);
			switch (SSL_get_error(openssl, wcount)){
					case SSL_ERROR_NONE:
                          break;
                     case SSL_ERROR_ZERO_RETURN:
            ...
            {
                wcount = recv(m_nSocketFnum, m_IObuffer, MAX_TCP_BUFFER_LENGTH, 0);
            }
		if (wcount <= 0)
        {
...
bool CTCPIPSystemSrvr::do_write(char* buffer, int bufcount, short operation_id, long timeout)
{
	...
	for (bp = &buffer[0]; count > 0; count -= wcount)
	{
		...
		do
		{
			if (buseSSLEnabled)
                    {
                        wcount = SSL_write(openssl, bp, count);
						switch (SSL_get_error(openssl, wcount))
                     {
                     case SSL_ERROR_NONE:
                          break;
		...
		if (wcount <= 0)
		{
			...
			// error_cleanup() is a bad interface, which causes a dangling this.
			// The send failure state should be marked apparently to indicate error.
			// error_cleanup();
			m_state |= TCPIP_SEND_FAILURE;
			return false;
		}
		bp += wcount;
	}
