...
std::string MXOSRVR_COMPONENT = "MXOSRVR";
...
void send_to_eventlog (short evt_num, short EventLogType, char *ComponentName, char *ObjectRef, short nToken, va_list marker)
{
   ...
   for (i = 0, found = FALSE; eventDataMap[i].eventId != 0L ; i++)
   {
      ...
      {
         found = TRUE;
         sprintf(tmpBuf, "%.30s NID %d PID %d TID %ld txID  ", process_name, nid, pid, GETTID());
...
#endif
void _mxowarn(const char* file, int line, const char *msg)
{
	static char process_name[MS_MON_MAX_PROCESS_NAME];
	static int nid, pid;
	...
	snprintf(buf, sizeof(buf),
            "%.30s NID %d PID %ld TID %d txID  %s",
            process_name, nid, pid, GETTID(), msg);
	CommonLogger::log("MXOSRVR", LL_WARN, "%s", buf);
}
