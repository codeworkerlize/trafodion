...
// Traces entry of odbc_SQLSvc_Savepoint_ame
void ODBCMXTraceMsg::TraceSavepointEnter(DIALOGUE_ID_def    dialogueId,
        IDL_unsigned_short savepointOpt,
        IDL_long savepointNameLength,
        const IDL_char *savepointName)
{
    int   length  = 0;
    char *pBuffer = msgBuffer;
    seqNumber     = 0;
    sprintf(seqNumStr, "%d", seqNumber);
    strcpy(funcName, "EnterSavepoint");
    CheckCollectorError();
    length   = sprintf(pBuffer,"DialogueId:%d ", dialogueId);
    pBuffer += length;
    length   = sprintf(pBuffer,"SavepointOpt:%d ", savepointOpt);
    pBuffer += length;
    length   = sprintf(pBuffer,"SavepointNameLength:%d ", savepointNameLength);
    pBuffer += length;
    length   = sprintf(pBuffer,"SavepointName:%d ", savepointName);
    pBuffer += length;
    SendEventMsg(MSG_SERVER_TRACE_INFO, EVENTLOG_INFORMATION_TYPE, pid, ODBCMX_SERVICE,
            objectRef, 4, srvrGlobal->sessionId, funcName, seqNumStr, msgBuffer);
}

// Traces exit of odbc_SQLSvc_Savepoint_ame
void ODBCMXTraceMsg::TraceSavepointExit(odbc_SQLSvc_Savepoint_exc_ exception,
        ERROR_DESC_LIST_def         sqlWarning)
{
    int   length  = 0;
    char *pBuffer = msgBuffer;
					      ...
					      const IDL_char   *fktableNm)
{
	...
	// % will cause a core because of later use of printf and variants, so
	// let's escape it using a printf-style escape
