...
/* To avoid conflicting declaration of Int64 in core/sqf/export/include/sql/sqlcli.h */
#define Int64 __DISCARD_THIS_INT64
#include "NAMemory.h"
#undef Int64

NAHeap _net_heap("network allocator");

#define NET_ALLOCATE(type) new(&_net_heap)type
#define NET_DEALLOCATE(ptr) if(ptr){_net_heap.deallocateMemory((void*)ptr),ptr=0;}

extern  bool buseSSLEnabled;
...
CTCPIPSystemSrvr::CTCPIPSystemSrvr(short nSocketFnum)
{
	...
	m_IObuffer = NET_ALLOCATE(char[MAX_TCP_BUFFER_LENGTH]);
	if (m_IObuffer == NULL)
		exit(0);
	next = NULL;
	m_trans_begin_tag = 0;
...
char* CTCPIPSystemSrvr::w_allocate( int size)
{
	...
	w_release();
	m_wbuffer = NET_ALLOCATE(char[size]);
	if (m_wbuffer != NULL)
...
char* CTCPIPSystemSrvr::r_allocate( int size)
{
	...
	r_release();
	m_rbuffer = NET_ALLOCATE(char[size]);
	if (m_rbuffer != NULL)
...
bool DoExpand(CTCPIPSystemSrvr* pnode, HEADER& rheader, unsigned char* ibuffer, unsigned long& output_size)
{
	...
	if(rheader.compress_ind == COMP_YES )
	{
	   ...
	   */ 
	   int type = rheader.compress_type==0xff?-1:rheader.compress_type;
	   if((type < COMP_DEFAULT) || (type > COMP_BEST_COMPRESSION))
	   {
	...
	obuffer = (unsigned char *)NET_ALLOCATE(char[rheader.total_length + 512]); // +512 is just to be safe
