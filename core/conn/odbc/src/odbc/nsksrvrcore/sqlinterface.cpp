...
SQLRETURN SRVR::GetODBCValues(Int32 DataType, Int32 DateTimeCode, Int32 &Length, Int32 Precision, 
						...
						Int32 IntLeadPrec, char *ColHeading,Int32 LobInlineMaxLen)
{
	...
	switch (DataType)
	{
		...
		case SQLTYPECODE_BLOB:
            ...
            {
                if (LobInlineMaxLen == -1) // Lob V1
                    Length = LOB_V1_EXT_HANDLE_LEN;
                else
                    Length = LobInlineMaxLen + LOB_V2_EXT_HANDLE_LEN;
                // lob always use 4 bytes for len
                totalMemLen = ((totalMemLen + 4 - 1) >> 2) << 2;
                totalMemLen += Length + 4;
            }
			if (!bRWRS)
							...
							BYTE *&VarBuffer, SRVR_DESC_HDL	*implDesc)
{
	...
	totalMemLen = ((totalMemLen + 8 - 1) >> 3) << 3;
	STMT_DEALLOCATE(VarBuffer);
	markNewOperator, VarBuffer = STMT_ALLOCATE(BYTE[totalMemLen]);
	if (VarBuffer == NULL)
	{
						   ...
						   Int32 numEntries, Int32 &indMemLen, Int32 &dataMemLen, BYTE *&VarBuffer)
{
	...
	markNewOperator, VarBuffer = STMT_ALLOCATE(BYTE[quadLength]);
	if (VarBuffer == NULL)
	{
		// Handle Memory Overflow execption here
			SendEventMsg(MSG_MEMORY_ALLOCATION_ERROR, EVENTLOG_ERROR_TYPE,
				...
				BYTE *&VarBuffer)
{
	...
	STMT_DEALLOCATE(SQLValueList->_buffer);
	STMT_DEALLOCATE(VarBuffer);
	...
	markNewOperator, SQLValueList->_buffer = STMT_ALLOCATE(SQLValue_def[numValues]);
	...
	SQLValueList->_length = 0;
    memset(SQLValueList->_buffer, 0, sizeof(SQLValue_def) * numValues);
	

	// Allocate the Value Buffer
	totalRowMemLen = totalMemLen * maxRowCount;
	markNewOperator, VarBuffer = STMT_ALLOCATE(BYTE[totalRowMemLen]);
	if (VarBuffer == NULL)
	{
		// Handle Memory Overflow execption here
			SendEventMsg(MSG_MEMORY_ALLOCATION_ERROR, EVENTLOG_ERROR_TYPE,
    ...
    memset(VarBuffer, 0, sizeof(BYTE) * totalRowMemLen);
						...
						SRVR_DESC_HDL	*&implDesc)
{
	...
	Int32 LobInlineMaxLen;
	Int32 LobChunkMaxLen;
	STMT_DEALLOCATE(implDesc);

	if (numEntries > 0)
	{
		markNewOperator, implDesc = STMT_ALLOCATE(SRVR_DESC_HDL[numEntries]);
		if (implDesc == NULL)
		{
			SendEventMsg(MSG_MEMORY_ALLOCATION_ERROR, EVENTLOG_ERROR_TYPE,
					srvrGlobal->nskProcessInfo.processId, ODBCMX_SERVER, 
        ...
        memset(implDesc, 0, sizeof(SRVR_DESC_HDL) * numEntries);
	}
	...
	for (i = 1, totalMemLen = 0; i <= numEntries; i++)
	{
		...
		LobInlineMaxLen = gDescItems[15].num_val_or_len;
	    LobChunkMaxLen = gDescItems[16].num_val_or_len;
		...
		retcode = GetODBCValues(DataType, DateTimeCode, Length, Precision, ODBCDataType, 
					...
					IntLeadPrec, ColumnHeading,LobInlineMaxLen);
		HANDLE_ERROR(retcode, sqlWarning);
        if (ODBCDataType == SQL_NUMERIC && Precision == Scale)
            ODBCPrecision += 1;
		...
		kdsCopyToSQLDescSeq(SQLDesc, DescName, DataType, DateTimeCode, Length, 
				  ...
				  SchemaName, Heading, IntLeadPrec, paramMode,LobInlineMaxLen,LobChunkMaxLen);
						...
						DESC_HDL_LISTSTMT *&SqlDescInfo)
{
	...
	*/
	...
	if (numEntries > 0)
	{
        ...
        memset(implDesc, 0, sizeof(SRVR_DESC_HDL) * numEntries);
		markNewOperator, SqlDescInfo = STMT_ALLOCATE(DESC_HDL_LISTSTMT[numEntries]);
		if (SqlDescInfo == NULL)
		{
			SendEventMsg(MSG_MEMORY_ALLOCATION_ERROR, EVENTLOG_ERROR_TYPE,
					srvrGlobal->nskProcessInfo.processId, ODBCMX_SERVER, 
        ...
        memset(SqlDescInfo, 0, sizeof(DESC_HDL_LISTSTMT) * numEntries);

		tempDescLen	= sizeof(totalMemLen) + sizeof(numEntries) + (sizeof(ODBCDescriptors) * numEntries);
		markNewOperator, SQLDesc = STMT_ALLOCATE(BYTE[tempDescLen]);
		if (SQLDesc == NULL)
		{
			SendEventMsg(MSG_MEMORY_ALLOCATION_ERROR, EVENTLOG_ERROR_TYPE,
					srvrGlobal->nskProcessInfo.processId, ODBCMX_SERVER, 
...
//		sqlBulkFetchPossible = true;
	...
	for (i = 1, totalMemLen = 0; i <= numEntries; i++)
	{
		...
		TableName[gDescItems[12].num_val_or_len] = '\0';
		DescName[gDescItems[13].num_val_or_len] = '\0';
		ColumnHeading[gDescItems[14].num_val_or_len+1] = '\0';
...
//		}
		...
		*(Int32 *)(SQLDesc+totalDescLength) = paramMode;
        ...
        if(srvrGlobal->drvrVersion.majorVersion >= 4)
        {
            //	int LobInlineMaxLen = gDescItems[15].num_val_or_len;
            *(Int32 *)(SQLDesc+totalDescLength) = LobInlineMaxLen;
            totalDescLength += sizeof(LobInlineMaxLen);

            //	LobChunkMaxLen = gDescItems[16].num_val_or_len;
            *(Int32 *)(SQLDesc+totalDescLength) = LobChunkMaxLen;
            totalDescLength += sizeof(LobChunkMaxLen);
            if(LobChunkMaxLen > 0)
                pSrvrStmt->setLobChunkMaxLen(LobChunkMaxLen);
        }
	}

        if (bRWRS)
        {
            if (totalMemLen > srvrGlobal->m_FetchBufferSize)
            {
                // the fetch buffer size is less than total memory, disable RWR
                srvrGlobal->m_FetchBufferSize = ((totalMemLen + 8 - 1) >> 3) << 3;
            }
        } 
        else
	{
...
//	if (sqlBulkFetchPossible && (sqlQueryType == SQL_SELECT_NON_UNIQUE || sqlQueryType == SQL_SP_RESULT_SET))
	size_t buffer_size = bRWRS ? srvrGlobal->m_FetchBufferSize : totalMemLen;
	markNewOperator, varBuffer = STMT_ALLOCATE(BYTE[buffer_size]);
	if (varBuffer == NULL)
	{
		// Handle Memory Overflow execption here
		SendEventMsg(MSG_MEMORY_ALLOCATION_ERROR, EVENTLOG_ERROR_TYPE,
...
SQLRETURN SRVR::EXTRACTLOB(SRVR_STMT_HDL *pSrvrStmt, IDL_string lobHandle, BYTE *&extractData, IDL_long_long &extractLen)
{ 
	...
	SQLDESC_ID *pInputDesc = &pSrvrStmt->inputDesc;
	SQLDESC_ID *pOutputDesc = &pSrvrStmt->outputDesc;
	if (! pSrvrStmt->isClosed) { 
		*pSrvrStmt->getChunkLenPtr() = extractLen;
		retcode =  WSQL_EXEC_Fetch(pStmt, pOutputDesc, NULL); 
	}
	else {
		if (IPD[0].dataType == SQLTYPECODE_CHAR) {
			lobHandleParam = (char *)IPD[0].varPtr;
			snprintf(lobHandleParam, 1024, "%-1024s", lobHandle);
			lobHandleParam[1023] = ' '; 
		}
		if (IPD[1].dataType == SQLTYPECODE_LARGEINT) {
			*(IDL_long_long *)IPD[1].varPtr = (IDL_long_long)pSrvrStmt->getChunkBuffer();
		}
		if (IPD[2].dataType == SQLTYPECODE_LARGEINT) {
			*pSrvrStmt->getChunkLenPtr() = extractLen;
			*(IDL_long_long *)IPD[2].varPtr = (IDL_long_long)pSrvrStmt->getChunkLenPtr();
		}
		retcode =  WSQL_EXEC_Exec(pStmt, pInputDesc, NULL);
		HANDLE_ERROR2(retcode, sqlWarning);
		retcode =  WSQL_EXEC_Fetch(pStmt, pOutputDesc, NULL);
	}
	if(retcode == SQL_SUCCESS || retcode == SQL_EOF || retcode == SQL_SUCCESS_WITH_INFO){
		extractData = pSrvrStmt->getChunkBuffer();
		extractLen = pSrvrStmt->getChunkLen();
	}
	HANDLE_ERROR3(retcode, sqlWarning);
	if (sqlWarning)
		return SQL_SUCCESS_WITH_INFO;
	else
		return SQL_SUCCESS;
}
SQLRETURN SRVR::UPDATELOB(SRVR_STMT_HDL *pSrvrStmt, IDL_string lobHandle, BYTE *&updateData, int &updateLen, IDL_long_long &updatePos)
{ 
    SQLRETURN rc = SQL_SUCCESS;
    long retcode = SQL_SUCCESS;
    BOOL   sqlWarning = FALSE;
    ...
    if (IPD[1].dataType == SQLTYPECODE_LARGEINT) {
        //memcpy((char*)(pSrvrStmt->getChunkBuffer()),(char*) updateData, updateLen);
        *(IDL_long_long *)IPD[1].varPtr = (IDL_long_long)pSrvrStmt->getChunkBuffer();
    }
    if (IPD[2].dataType == SQLTYPECODE_LARGEINT) {
        *pSrvrStmt->getChunkLenPtr() = updateLen;
        *(IDL_long_long *)IPD[2].varPtr = (IDL_long_long)pSrvrStmt->getChunkLenPtr();
    }
    if (IPD[3].dataType == SQLTYPECODE_LARGEINT) {
        *pSrvrStmt->getLobPosPtr() = updatePos;
        *(IDL_long_long *)IPD[3].varPtr = (IDL_long_long)pSrvrStmt->getLobPosPtr();
    }
    // The updateLobStmt_ is also closed with CLI_ExecFetch call
    retcode = WSQL_EXEC_ExecFetch(pStmt, pInputDesc, NULL);
    HANDLE_ERROR2(retcode, sqlWarning);
...
//--------------------------------------------------------------------------
SQLRETURN SRVR::BuildSQLDesc2withRowsets(SRVR_STMT_HDL*pSrvrStmt
					, SQLDESC_ID          *pDesc
					, Int32                 sqlQueryType 
					, Int32                 maxRowsetSize
					, bool                &sqlBulkFetchPossible 
                                        ...
                                        )
{
  SRVRTRACE_ENTER(FILE_INTF+40);
	...
	if (numEntries > 1)
	{
		markNewOperator, implDesc = STMT_ALLOCATE(SRVR_DESC_HDL[numEntries - 1]);
		if (implDesc == NULL)
		{
			SendEventMsg(MSG_MEMORY_ALLOCATION_ERROR, EVENTLOG_ERROR_TYPE,
					srvrGlobal->nskProcessInfo.processId, ODBCMX_SERVER, 
        ...
        memset(implDesc, 0, sizeof(SRVR_DESC_HDL) * (numEntries - 1));
        ...
        memset(inputQuadList, 0, sizeof(SQLCLI_QUAD_FIELDS) * numEntries);
        ...
        memset(inputQuadList_recover, 0, sizeof(SQLCLI_QUAD_FIELDS) * numEntries);

		// Setup ind_layout in quad ptr list.
                for (i = 0; i < numEntries; i++)
                  inputQuadList[i].ind_layout = 2;  // 2 byte indicator

		markNewOperator, SqlDescInfo = STMT_ALLOCATE(DESC_HDL_LIST[numEntries - 1]);
		if (SqlDescInfo == NULL)
		{
			SendEventMsg(MSG_MEMORY_ALLOCATION_ERROR, EVENTLOG_ERROR_TYPE,
					srvrGlobal->nskProcessInfo.processId, ODBCMX_SERVER, 
        ...
        memset(SqlDescInfo, 0, sizeof(DESC_HDL_LIST) * (numEntries - 1));
	...
	STMT_DEALLOCATE(varBuffer);
	size_t buffer_size = (sqlBulkFetchPossible && sqlQueryType == SQL_SELECT_NON_UNIQUE)
							 ? srvrGlobal->m_FetchBufferSize
							 : totalMemLen;
	markNewOperator, varBuffer = STMT_ALLOCATE(BYTE[buffer_size]);
...
SQLRETURN SRVR::RECOVERY_FROM_ROWSET_ERROR2( SRVR_STMT_HDL *pSrvrStmt
  ...
  *totalrowsAffected = -1;
  ...
  while( currentRowCount < RowsetSize)
    ...
    case SQLTYPECODE_BLOB:
	case SQLTYPECODE_CLOB:
        sqlMaxLength = DataLen + 4;
        sqlMaxLength = ((sqlMaxLength + 4 - 1)>>2)<<2;
        break;
	default:
...
SQLRETURN SRVR::FREESTATEMENT(SRVR_STMT_HDL* pSrvrStmt)
{
	...
	*rowsAffected = -1;
	SESSION_DEBUG("\nSRVR::FREESTATEMENT enter:\n\tpSrvrStmt = %p,\n\tfreeResourceOpt = %d", pSrvrStmt, freeResourceOpt);
	...
	case SQL_CLOSE:
		if (! pSrvrStmt->isClosed)
		{
			...
			if (pSrvrStmt->stmtType != INTERNAL_STMT)
			{
				if (retcode == -8811)
				{
					WSQL_EXEC_ClearDiagnostics(pStmt);
					retcode = SQL_SUCCESS;
				}
				HANDLE_ERROR(retcode, sqlWarning);
			}
...
SQLRETURN SRVR::PREPARE(SRVR_STMT_HDL* pSrvrStmt)
{
  ...
  retcode = WSQL_EXEC_SetStmtAttr(pStmt,SQL_ATTR_SYNC_QUERY_FOR_BINGLOG,srvrGlobal->syncQueryForBinlog, NULL);
	HANDLE_ERROR(retcode, sqlWarning);
		...
		for (curColumnNo = 0; curColumnNo < columnCount ; curColumnNo++)
		{
			...
			if ((indPtr == NULL) || (indPtr != NULL && *((short *)indPtr) != -1))
			{
				...
				switch(dataType)
				{
					...
					case SQLTYPECODE_BLOB:
					case SQLTYPECODE_CLOB:
						dataLength = *(int *)pBytes;
						allocLength = dataLength+5;
						if (maxRowLen != 0)
						{
							allocLength = (allocLength>(UInt32)maxRowLen+5)?(UInt32)maxRowLen+5:allocLength;
							srcDataLength = *(int *)pBytes;
							srcDataLength = (srcDataLength>(UInt32)maxRowLen)?(UInt32)maxRowLen:srcDataLength;
							*(int *)pBytes=srcDataLength;
						}
						break;
  ...
  if (useRowsets == true)
    ...
    *outValuesLength = pSrvrStmt->outputDescVarBufferLen * maxRowCnt;
    ...
    if (pSrvrStmt->outputQuadList == NULL) 
      ...
      memset(pSrvrStmt->outputQuadList, 0, sizeof(SQLCLI_QUAD_FIELDS) * columnCount);
      }  // if (pSrvrStmt->outputQuadList == NULL) 
...

{
  ...
  retcode = WSQL_EXEC_SetStmtAttr(pStmt,SQL_ATTR_SYNC_QUERY_FOR_BINGLOG,srvrGlobal->syncQueryForBinlog, NULL);
	...
	if (pSrvrStmt->stmtType == EXTERNAL_STMT && srvrGlobal->srvrType == CORE_SRVR)
	{
			...
			if (pSrvrStmt->sqlQueryType == SQL_EXE_UTIL || pSrvrStmt->sqlQueryType == SQL_DDL_WITH_STATUS)
...
{ 
  ...
  retcode = WSQL_EXEC_SetStmtAttr(pStmt,SQL_ATTR_SYNC_QUERY_FOR_BINGLOG,srvrGlobal->syncQueryForBinlog, NULL);
	...
	if (pSrvrStmt->paramCount > 0)
	{
		tmpBuildID = srvrGlobal->drvrVersion.buildId; // should go way once we support rowwise rowsets
		srvrGlobal->drvrVersion.buildId = 0;
		retcode = BuildSQLDesc2(pSrvrStmt,pInputDesc, -9999, 0, pSrvrStmt->sqlBulkFetchPossible, pSrvrStmt->paramCount,
	...
	if (pSrvrStmt->columnCount > 0)
	{
		retcode = BuildSQLDesc2(pSrvrStmt,pOutputDesc, pSrvrStmt->sqlQueryType,  pSrvrStmt->maxRowsetSize, pSrvrStmt->sqlBulkFetchPossible,
...
*/
	...
	if (pSrvrStmt->paramCount > 0)
	{
		...
		kdsCreateSQLDescSeq(inputSQLDesc, pSrvrStmt->paramCount);
		retcode = BuildSQLDesc2withRowsets(pSrvrStmt
                                                  , pInputDesc
                                                  , -9999
                                                  , 0
...
SQLRETURN SRVR::EXECUTE2withRowsets(SRVR_STMT_HDL* pSrvrStmt)
{
	...
	SQLItemDescList_def *tmp = &pSrvrStmt->inputDescList;
	...
	SESSION_DEBUG("doClipVarchar EXECUTE2withRowsets = %d",doClipVarchar);
    if(doClipVarchar)
    {
        if (paramCount > 0)
        {

            BYTE * curPtr = pSrvrStmt->transportBuffer;
            ...
            for (int i = 1; i < paramCount; i++)
            {
				...
				bool isShort = true;
                long dataLength = pSrvrStmt->IPD[i-1].length;  // dataLen is length with varLen 
...
//					dataLength++;
                    ...
                    && pSrvrStmt->IPD[i-1].length >= varcharLimit)
                {
                    ...
                    if (pSrvrStmt->IPD[i-1].indPtr != 0){
						indBufLen = 2*inputRowCnt;
					   	
						indBuf = (BYTE *)malloc(indBufLen);
						memset(indBuf,0,indBufLen);
						freeStack.push(indBuf);

                        pSrvrStmt->inputQuadList[i].ind_ptr = curPtr + curOffset;
						memcpy(indBuf,curPtr + curOffset,indBufLen);
                        curOffset += indBufLen;
                    ...
                    long totalDataLen = (dataLength ) * (inputRowCnt);

                    BYTE * dataPtr = (BYTE *)malloc(totalDataLen);
                    memset(dataPtr,0,totalDataLen);
                    freeStack.push(dataPtr);
					long dataPtrOffset = 0;

                    for(int j = 0;j< inputRowCnt ;j++){

						if (pSrvrStmt->IPD[i-1].indPtr != 0){  // Nullable
							short nullval = *(short*)(indBuf+indBufOffset);
							indBufOffset += 2;
							if(nullval != -1) { // cell not null

								if(isShort) {

									short varcharLen = *(short*)(curPtr+curOffset);


									memcpy(dataPtr+dataPtrOffset,curPtr+curOffset,varcharLen+2);
									curOffset+= varcharLen+2;
								}
								else {


									int varcharLen = *(int*)(curPtr+curOffset);


									memcpy(dataPtr+dataPtrOffset,curPtr+curOffset,varcharLen+4);
									curOffset+= varcharLen+4;

								}
							}
							else {
								// cell is null dont copy
							}
							dataPtrOffset += dataLength;
						}
						else { // not nullable 
					...
					pSrvrStmt->inputQuadList[i].var_ptr = dataPtr;
                ...
                else
                {
                    ...
                    pSrvrStmt->inputQuadList[i].var_ptr = curPtr + curOffset;
					curOffset += (dataLength) * (inputRowCnt); // var buffer with 
...
SQLRETURN SRVR::FETCH2bulk(SRVR_STMT_HDL *pSrvrStmt)
{
	...
	if (pSrvrStmt->maxRowCnt > 0)
	{
		if (pSrvrStmt->outputDescVarBufferLen > 0)
		{
			...
			{
				// new a large BYTE[] and memset it will cost many time for select statement, 
				// If we could reuse this memory, we could save 90 percent of the time(In one test case to slect 10000 rows, the time was reduced from 23s to 1.2s)
				// will record the length of [pSrvrStmt->outputDescVarBufferLen * pSrvrStmt->maxRowCnt], 
				// and if the space required by this FETCH operation is greater than the last record value, we will apply for a new one
				recordedValue = pSrvrStmt->recordOutputDescVarBufferLen;
				SESSION_DEBUG("\tSRVR::FETCH2bulk:\n\tpSrvrStmt = %p,\n\trecordedValue = %d,\n\tnew buffer length = %d, ", pSrvrStmt, recordedValue, pSrvrStmt->maxRowCnt*pSrvrStmt->outputDescVarBufferLen);
				if (recordedValue < pSrvrStmt->maxRowCnt*pSrvrStmt->outputDescVarBufferLen)
				{
					STMT_DEALLOCATE(pSrvrStmt->outputDescVarBuffer);
					markNewOperator, pSrvrStmt->outputDescVarBuffer = STMT_ALLOCATE(BYTE[pSrvrStmt->maxRowCnt * pSrvrStmt->outputDescVarBufferLen]);
					pSrvrStmt->recordOutputDescVarBufferLen = pSrvrStmt->maxRowCnt*pSrvrStmt->outputDescVarBufferLen;
					...
					memset(pSrvrStmt->outputDescVarBuffer, 0, sizeof(BYTE) * pSrvrStmt->recordOutputDescVarBufferLen);
				}
				// memset every time, this is a relatively safe operation,
				// If anyone can guarantee that we don't do memset, then we can save %20 time here
				// 				NEW ADDED  QA has tested the scene without memset, and the result is good
				// memset(pSrvrStmt->outputDescVarBuffer, 0, sizeof(BYTE) * pSrvrStmt->recordOutputDescVarBufferLen);
	...
	while (retcode != 100)
	{
		...
		for (curColumnNo = 0; curColumnNo < columnCount ; curColumnNo++)
		{
			...
			if ((indPtr == NULL) || (indPtr != NULL && *((short *)indPtr) != -1))
			{
				...
				switch(dataType)
				{
					...
					case SQLTYPECODE_BLOB:
					case SQLTYPECODE_CLOB:
						dataLength = *(Int32 *)pBytes;

						allocLength = dataLength + 5;
						if (maxRowLen != 0)
						{
							allocLength = (allocLength>(UInt32)maxRowLen+5)?(UInt32)maxRowLen+5:allocLength;
							srcDataLength = *(Int32 *)pBytes;
							srcDataLength = (srcDataLength>(UInt32)maxRowLen)?(UInt32)maxRowLen:srcDataLength;
							*(Int32 *)pBytes=srcDataLength;
						}
						break;
					case SQLTYPECODE_CHAR:
					case SQLTYPECODE_BIT:
		...
		if (increstTotalLength - outputDataValue->_length < estRowLength)
		{
			if (outputDataValue->_buffer != NULL)
			{
				...
				markNewOperator, tempOutputDataValue._buffer = STMT_ALLOCATE(BYTE[increstTotalLength + 1]);
				...
				STMT_DEALLOCATE(tempOutputDataValue._buffer);
			}
			else
			{
				// Handle Memory Overflow execption here
...
// we need following ids: SQLCODE, SQLSTATE, ROW_NUMBER, MESSAGE TEXT
		...
		for (i=0; i < total_conds; i++)
		{
			...
			strncpy(sqlState,pCondInfoItems[4*i+1].string_val,sizeof(sqlState)-1);
			sqlState[sizeof(sqlState)-1]=0;
			row_number = (Int32)*(pCondInfoItems[4*i+2].num_val_or_len);
			strncpy(MessageText,pCondInfoItems[4*i+3].string_val,sizeof(MessageText)-1);
