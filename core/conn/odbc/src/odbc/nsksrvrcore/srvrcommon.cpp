...
void SRVR::initSqlCore()
{
    ...
    gDescItems[15].item_id = SQLDESC_LOB_INLINE_DATA_MAXLEN;
    gDescItems[16].item_id = SQLDESC_LOB_CHUNK_MAXLEN;
                    ...
                    )
{
    ...
    if(stmtLabel == NULL)
    {
        sprintf(tmpString, "stmtLabel passed in is NUll"); 
        SendEventMsg(MSG_ODBC_NSK_ERROR,
                     EVENTLOG_ERROR_TYPE,
                     0,   
                     ODBCMX_SERVER,
...
void SRVR::convertFuzzyCard(const char* columnName, const IDL_char *inName1, const IDL_char *inName2, IDL_char *outString)
{
    char *in1 = (char *)inName1;
    char *in2 = (char *)inName2;
    char *out = (char *)outString;
    BOOL isFuzzy = FALSE;
    ...
    if (isFuzzy)
        sprintf(out, "and %s LIKE '%s' ESCAPE '\\'", columnName, inName1);
    else
        sprintf(out, "and %s = '%s'", columnName, inName2);
    return;
}
  ...
  , /* Out   */ BYTE *&MXCSWarningOrError)
{
	...
	char tsqlState[6];
	static BYTE WarningOrError[2048];
...
extern "C" void GETMXCSWARNINGORERRORSINGLE(
   /* In    */ Int32 sqlcode
  , /* In    */ char *sqlState
  , /* In    */ char *msg_buf
  , /* Out   */ Int32 *MXCSWarningOrErrorLength
