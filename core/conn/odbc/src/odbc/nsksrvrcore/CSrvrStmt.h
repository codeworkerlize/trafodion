...
#define Int64 __DISCARD_THIS_INT64
#include "NAMemory.h"
#undef Int64

extern NAHeap _stmt_heap;

#define STMT_ALLOCATE(type) new(&_stmt_heap)type
#define STMT_DEALLOCATE(ptr) if(ptr){_stmt_heap.deallocateMemory((void*)ptr),ptr=0;}
...
public:
	...
	Int32					recordOutputDescVarBufferLen;	// a record length of outputDescVarBuffer
...
// and should not be deleted.
	bool reprepareWarn;
//
	timespec queryResponseStartTime;
    static int EXPECTED_RESPONSE_TIME_IN_MILLIS;
    void setLobChunkMaxLen(long lobChunkMaxLen) {
        lobChunkMaxLen_ = lobChunkMaxLen;
    } 

    long getLobChunkMaxLen() {
        return lobChunkMaxLen_;
    }
private: 
    SRVR_STMT_HDL *lobUpdateStmt_;
    SRVR_STMT_HDL *lobExtractStmt_;
    long lobTableUid_;
    bool errorInLobUpdateStmt_;
    bool errorInLobExtractStmt_;
    BYTE *chunk_;
    int64 chunkLen_;
    int64 lobPos_;
    int64 lobChunkMaxLen_; // This will be same for all columns in a SQL statement and it is based on 
    // CQD TRAF_LOB_CHUNK_MAXLEN
...
public:
	...
	//SRVR_STMT_HDL *getLobExtractStmt(long tableUid, ExceptionStruct  *exception);
    SRVR_STMT_HDL *getLobExtractStmt(long tableUid, long stmtId, ExceptionStruct  *exception);
    SRVR_STMT_HDL *getLobUpdateStmt(long tableUid, long stmtId, ExceptionStruct  *exception);
	int64 *getChunkLenPtr() { return &chunkLen_; }
	int64 getChunkLen() { return chunkLen_; }
    BYTE *getChunkBuffer()
    {
        if (chunk_ == NULL){
            chunk_ = new BYTE[lobChunkMaxLen_];
        }
        return chunk_;
    }
    void setChunkBuffer(BYTE *chunkBuf, int length)
    {
        getChunkBuffer();
        memcpy(chunk_, chunkBuf, length);
    }
    int64 *getLobPosPtr() { return &lobPos_; }
    int64 getLobPos() { return lobPos_; }
    SQLRETURN extractLob(IDL_string lobHandle, BYTE *&extractData, IDL_long_long &extractLen, ExceptionStruct *exception);  
    SQLRETURN updateLob(IDL_string lobHandle, BYTE *&updateData, int &updateLen, IDL_long_long &updatePos, ExceptionStruct *exception);  
    SQLRETURN closeExtractLob();
    SQLRETURN closeUpdateLob();
...
// Modules	
	...
	void logDelayedResponseMessage(char *operation); 
	inline void setQueryResponseStartTime()
	{
		if (queryTimeout > 0 || EXPECTED_RESPONSE_TIME_IN_MILLIS > 0)
			clock_gettime(CLOCK_MONOTONIC, &queryResponseStartTime); 
	}

	inline void setQueryTimeout(int inQueryTimeout)
	{ 
