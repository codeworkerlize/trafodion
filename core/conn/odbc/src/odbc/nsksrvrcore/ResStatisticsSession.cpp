...
void initialize_curl()
{
	     ...
	     /*filep = fopen("/tmp/curldump.txt", "a");
...
void ResStatisticsSession::sendSessionInfo(string session_status, long long startTime, long long endTime)
{
	...
	pSession_info->m_instance_id = srvrGlobal->traf_instance_id;
	pSession_info->m_tenant_id = srvrGlobal->tenantID;
...
std::tr1::shared_ptr<SESSION_AGGREGATION> ResStatisticsSession::getAggrStats()
{
	...
	*pAggr_info = {0};
	...
	pAggr_info->m_instance_id = srvrGlobal->traf_instance_id;
	pAggr_info->m_tenant_id = srvrGlobal->tenantID;
...
void publishAggrStatsToTSDB(std::tr1::shared_ptr<SESSION_AGGREGATION> pAggr_info)
{
        int total_completed_statements = 0;
        if (srvrGlobal->m_bPublishStatsToTSDB) 
        {
             total_completed_statements = pAggr_info->m_delta_total_selects + pAggr_info->m_delta_total_inserts + pAggr_info->m_delta_total_updates + pAggr_info->m_delta_total_deletes + pAggr_info->m_delta_total_util_stmts+ pAggr_info->m_delta_total_ddl_stmts;
	     ...
	     for (int ic = 0; ic < 11; ic++)
	         ...
	         cc.clear();
		 cc << "[{";
		 cc << "\"timestamp\":" << julian_to_millis(pAggr_info->m_aggregation_last_update_utc_ts) << ",";
		 switch (ic)
		 {
		     case 0:
		    ...
		    case 5:
		     ...
		     case 9:
		        ...
		        break;
		     case 10:
		        cc << "\"metric\":\"esgyndb.mxosrvr.aggrstat.total_completed_stmts\"" << ",";
		        cc << "\"value\":" << total_completed_statements << ",";
		        break;
		     default:
		        break;
		} //end of switch
                cc << "\"tags\":{\"host\":\"" << srvrGlobal->HostName << "\",\"SessionID\":\"" << pAggr_info->m_sessionId.c_str() << "\",\"ProfileName\":\"" << srvrGlobal->mappedProfileName << "\",\"SLAName\":\"" << srvrGlobal->mappedSLAName << "\"}";
		cc << "}]";
		ccStr = cc.str();
                tmpString[0]='\0';
                sprintf(tmpString, "Publication data::ThreadId::%lu::%s", GETTID(), ccStr.c_str());
		SendEventMsg(MSG_SERVER_TRACE_INFO,
                    EVENTLOG_INFORMATION_TYPE,
		    0,
		    ODBCMX_SERVER,
