...
#define SQL_BEGIN_SAVEPOINT   0
#define SQL_ROLLBACK_SAVEPOINT   1
#define SQL_COMMIT_SAVEPOINT   2
#define SQL_RELEASE_SAVEPOINT  3
#endif
...
void addExpansionWarn(SRVR_STMT_HDL *pSrvrStmt ) {
	Int32 RMSWarningOrErrorLength = 0; 
	char RMSWarningOrErrorMsg[256];
	BYTE *RMSWarningOrError;
	sprintf(RMSWarningOrErrorMsg, "WARNING Query cache size exceeded RMS limit set. Left RMS size : [%d], total RMS size [%d]", pSrvrStmt->comp_stats_info.leftShmSize, pSrvrStmt->comp_stats_info.shmSize);
    MXO_WARN("%s",RMSWarningOrErrorMsg);
    GETMXCSWARNINGORERRORSINGLE(30048, "01001", RMSWarningOrErrorMsg,&RMSWarningOrErrorLength , RMSWarningOrError);
	Int32 num = *(Int32 *)(pSrvrStmt->sqlWarningOrError);
	*(Int32 *)(pSrvrStmt->sqlWarningOrError) = num+1;
	BYTE * tmp = new BYTE[(pSrvrStmt->sqlWarningOrErrorLength) + RMSWarningOrErrorLength];
	memcpy(tmp,pSrvrStmt->sqlWarningOrError,pSrvrStmt->sqlWarningOrErrorLength);
	memcpy(tmp+pSrvrStmt->sqlWarningOrErrorLength,RMSWarningOrError,RMSWarningOrErrorLength);
	delete pSrvrStmt->sqlWarningOrError;
	pSrvrStmt->sqlWarningOrError = tmp;
	pSrvrStmt->sqlWarningOrErrorLength = pSrvrStmt->sqlWarningOrErrorLength + RMSWarningOrErrorLength;

}	

// Cut extra parts of varchar in outputDataValue  to make data compaction
long long  clipVarchar(SRVR_STMT_HDL *pSrvrStmt, bool calledByFetch)
{
    if(srvrGlobal->clipVarchar == 0
    || pSrvrStmt->sqlQueryType == SQL_CALL_NO_RESULT_SETS || pSrvrStmt->sqlQueryType == SQL_CALL_WITH_RESULT_SETS
    || (srvrGlobal->drvrVersion.majorVersion < 5 && (!calledByFetch)))
    {
        return pSrvrStmt->outputDescVarBufferLen*pSrvrStmt->rowsAffected;
    }
	SESSION_DEBUG("clipvarchar fetch start!");
    ...
    long startOffset = 0;
    long clipVarcharCount = 0; //counter of varchar column per row 
    BYTE * cpStart = VarPtr;
    BYTE * cpEnd   = cpStart;
    BYTE * colEnd  = cpStart;
    ...
    {
        if(pSrvrStmt->SqlDescInfo[j].DataType ==SQLTYPECODE_VARCHAR_WITH_LENGTH && pSrvrStmt->SqlDescInfo[j].Length>=srvrGlobal->clipVarchar)
            clipVarcharCount ++;
        if(j == numEntries -1 ){
    ...
    for( i = 0; i < rowsAffected ; i++){//do clip 
        ...
        {
            if ((pSrvrStmt->SqlDescInfo[j].DataType == SQLTYPECODE_VARCHAR_WITH_LENGTH || pSrvrStmt->SqlDescInfo[j].DataType == SQLTYPECODE_VARCHAR_LONG)
                && pSrvrStmt->SqlDescInfo[j].Length >= srvrGlobal->clipVarchar)
			{
				/*
				   |--------------column 1 ------|--------------column 2 ------|
...
*/
            ...
            {
                memmove(bufferOffset,cpStart,cpEnd-cpStart);
            }
    ...
    delete [] colBuferLen;
	SESSION_DEBUG("clipvarchar fetch return %lld",bufferOffset-VarPtr);
		...
		if (stmtType != TYPE_SMD)
		{
			...
			if (exception_->exception_nr == 0)
			{
				...
				if (!pSrvrStmt->isReadFromModule)
				{
					...
					pSrvrStmt->setQueryTimeout(queryTimeout);
  ...
  , /* In    */ bool isFromExecDirect = false)
{
	...
	if (*returnCode == 0)
	{
		...
		*stmtHandle = (Long)pSrvrStmt;
		...
		pSrvrStmt->setQueryTimeout(queryTimeout);
		currentTimeMillis(enterEngineTimestamp);
		rc = PREPARE2(pSrvrStmt,isFromExecDirect);
		pSrvrStmt->logDelayedResponseMessage("PREPARE2");
			...
			*outputDescLength = pSrvrStmt->outputDescBufferLength;
			...
			if (rc == SQL_SUCCESS_WITH_INFO)
			{
				GETSQLWARNINGORERROR2(pSrvrStmt);
                                //for mantis 23619
                                //if AQR happens during statement Prepare, shmSiz and leftShmSize will be populated with 0
                                //RMS memory will be taken as exhausted due to 0 <= 0 by mistake
                                //so use < instead of <=
				if(pSrvrStmt->comp_stats_info.leftShmSize *100 < pSrvrStmt->comp_stats_info.shmSize * srvrGlobal->shmLimit){ 
					addExpansionWarn(pSrvrStmt);
				}
		...
		case SQL_SUCCESS:
			...
			*outputDescLength = pSrvrStmt->outputDescBufferLength;
            ...
            if(pSrvrStmt->comp_stats_info.leftShmSize *100 < pSrvrStmt->comp_stats_info.shmSize * srvrGlobal->shmLimit){
                *returnCode = SQL_SUCCESS_WITH_INFO;
                char *RMSWarningOrError;
                RMSWarningOrError = new char[256];
                sprintf(RMSWarningOrError, "WARNING RMS remaining memory exceeded the limitation setting. Left RMS size : [%d], total RMS size [%d]", pSrvrStmt->comp_stats_info.leftShmSize, pSrvrStmt->comp_stats_info.shmSize);
                MXO_WARN("%s",RMSWarningOrError);
                GETMXCSWARNINGORERROR(30048, "01000", RMSWarningOrError, sqlWarningOrErrorLength, sqlWarningOrError);
                delete RMSWarningOrError;
            }
...
//	        || pSrvrStmt->sqlStmtType == TYPE_DELETE))
		...
		pSrvrStmt->logDelayedResponseMessage("PREPARE2withRowsets");
...
odbc_SQLSvc_Execute2_sme_(
  ...
  , /* Out   */ BYTE *&outValues)
{
	SESSION_DEBUG("odbc_SQLSvc_Execute2_sme_ inValuesLength = %lld",inValuesLength);
	...
	else
	{
		...
		if (*returnCode == 0 || *returnCode == 1)
		{
			...
			else
			{
                ...
                int varcharLimit = srvrGlobal->clipVarchar;
                bool doClipVarchar = false;
                int paramCount = pSrvrStmt->paramCount;

                BYTE * target = pSrvrStmt->inputDescVarBuffer;
                BYTE * src = inValues;
                int targetOffset = 0;
                int srcOffset = 0;
                if(inValuesLength && varcharLimit > 0)
                {
                    for(int i = 1;i<= paramCount ;i++)
                    {
                        if( (pSrvrStmt->IPD[i-1].dataType == SQLTYPECODE_VARCHAR_WITH_LENGTH || pSrvrStmt->IPD[i-1].dataType == SQLTYPECODE_VARCHAR_LONG)
                            && pSrvrStmt->IPD[i-1].length >= varcharLimit)
                            doClipVarchar = true;
                    }
                }
                   

				SESSION_DEBUG("Execute2 doClipVarchar = %d",doClipVarchar);
                if(doClipVarchar)
                {

              //      memset(pSrvrStmt->inputDescVarBuffer,0,pSrvrStmt->inputDescVarBufferLen);
                    if (paramCount > 0)
                    {
                        int curOffset = 0;
                        int varcharColOffset = 0;

                        for (int i = 1; i <= paramCount; i++)
                        {

							bool isShort  = true;
                            int dataLength = pSrvrStmt->IPD[i-1].length;
                            switch(pSrvrStmt->IPD[i-1].dataType){
                                case SQLTYPECODE_VARCHAR_WITH_LENGTH:
                                case SQLTYPECODE_VARCHAR_LONG:
									if(dataLength > SHRT_MAX) {
										dataLength = ((dataLength + 4 - 1) >> 2) << 2;
										dataLength += 4;
										isShort = false;
									}
									else {
										dataLength = ((dataLength + 2 - 1) >> 1) << 1;
										dataLength += 2;
									}
									break;
                                ...
                                && pSrvrStmt->IPD[i-1].length >= varcharLimit)
                            {
								...
								//copy ind buffer first
                                if (pSrvrStmt->IPD[i-1].indPtr != 0)
								{
									short nullvalue = *(short*)(src+srcOffset);
                                    if(nullvalue == -1)
                                    {
                                        // var is null only copy ind buffer
                                        memcpy(pSrvrStmt->IPD[i-1].indPtr,src+srcOffset,2);
                                        srcOffset +=2;
                                    }
                                    else{
									    // copy ind buffer
										...
										// copy var buffer
										if(isShort){
											short varcharLen = *(short*)(src+srcOffset);

											memcpy(pSrvrStmt->IPD[i-1].varPtr,src+srcOffset,varcharLen+2);

											srcOffset += varcharLen +2; // varLen and data buffer

										}
										else{

											int varcharLen = *(int*)(src+srcOffset);
											memcpy(pSrvrStmt->IPD[i-1].varPtr,src+srcOffset,varcharLen+4);


											srcOffset += varcharLen +4; // varLen and data buffer

										}
                                    }
                                }
								else
								{ // not nullable ,copy var buffer only
							...
							{
								if (pSrvrStmt->IPD[i-1].indPtr != NULL){

									memcpy(pSrvrStmt->IPD[i-1].indPtr,src+srcOffset,2); //  copy ind buf
									srcOffset+= 2;
								}
								{
                                    memcpy(pSrvrStmt->IPD[i-1].varPtr,src+srcOffset,dataLength); // without ind buf
			...
			pSrvrStmt->currentMethod = odbc_SQLSvc_ExecuteN_ldx_;
			pSrvrStmt->setQueryTimeout(queryTimeout);
            pSrvrStmt->logDDLQuery();
			if (enterEngineTimestamp[0] == 0 && enterEngineTimestamp[1] == 0) {
				currentTimeMillis(enterEngineTimestamp);
			}
			rc = EXECUTE2(pSrvrStmt);
			currentTimeMillis(leaveEngineTimestamp);
			pSrvrStmt->logDelayedResponseMessage("EXECUTE2");
...
//					srvrGlobal->srvrObjRef, 1, tmpString);
			...
			case SQL_SUCCESS_WITH_INFO:
				...
				*rowsAffected = pSrvrStmt->rowsAffected;
				...
				{
					tmpLength = clipVarchar(pSrvrStmt, false);
					*outValuesLength = tmpLength;
...
odbc_SQLSvc_Execute2withRowsets_sme_(
  ...
  , /* Out   */ BYTE *&outValues)
{
	SESSION_DEBUG("odbc_SQLSvc_Execute2withRowsets_sme_ inValuesLength = %lld",inValuesLength);
	SRVRTRACE_ENTER(FILE_SME+19);
			...
			if (*returnCode == 0 || *returnCode == 1)
			{
				...
				if ( (*returnCode == 0 && rc == 0) || (*returnCode == 1 && rc == 1) )
				{
					...
					if (!bStubExecute2WithRowsets) {
						...
						currentTimeMillis(leaveEngineTimestamp);
						pSrvrStmt->logDelayedResponseMessage("EXECUTE2WithRowsets");
					}    
					else {
						rc = SQL_SUCCESS;
  ...
  if ((pSrvrStmt = (SRVR_STMT_HDL *)stmtHandle) == NULL)
    ...
    *returnCode = SQL_NO_DATA_FOUND;
	...
	delete pSrvrStmt->outputDataValue._buffer;
      ...
      pSrvrStmt->logDelayedResponseMessage("FETCH2bulk");
	...
	*outValuesLength = 0;
      ...
      { 
        ...
        pSrvrStmt->logDelayedResponseMessage("FETCH2");
      }
    switch (rc)
      {
      case SQL_SUCCESS_WITH_INFO:
...
extern "C" void 
odbc_SQLSvc_Savepoint_sme_(
        /* In    */ CEE_tag_def objtag_
        , /* In    */ const CEE_handle_def *call_id_
        , /* Out   */ odbc_SQLSvc_Savepoint_exc_ *exception_
        , /* In    */ DIALOGUE_ID_def dialogueId
        , /* In    */ IDL_unsigned_short savepointOpt
        , /* In    */ IDL_long savepointNameLength
        , /* In    */ const IDL_char *savepointName
        , /* Out   */ ERROR_DESC_LIST_def *sqlWarning
        )
{
    SRVRTRACE_ENTER(FILE_SME+5);
    ...
    // no savepoint name specified
    if (savepointNameLength == 0)
    {
      switch (savepointOpt) {
          case SQL_BEGIN_SAVEPOINT:
              pSrvrStmt = getSrvrStmt("STMT_BEGIN_SAVEPOINT_1", FALSE);
              break;
          case SQL_ROLLBACK_SAVEPOINT:
              pSrvrStmt = getSrvrStmt("STMT_ROLLBACK_SAVEPOINT_1", FALSE);
              break;
          case SQL_COMMIT_SAVEPOINT:
              pSrvrStmt = getSrvrStmt("STMT_COMMIT_SAVEPOINT_1", FALSE);
              break;
          default:
              exception_->exception_nr = odbc_SQLSvc_Savepoint_ParamError_exn_;
              exception_->u.ParamError.ParamDesc = SQLSVC_EXCEPTION_INVALID_SAVEPOINT_OPT;
              return;
      }
      if (pSrvrStmt == NULL)
      {
      ...
      else
      {
          ...
          if (rc == SQL_ERROR)
          {
              ...
              if (pSrvrStmt->sqlError.errorList._length != 0 )
              {
                  ...
                  {
                      exception_->exception_nr = odbc_SQLSvc_Savepoint_SQLError_exn_;
                      exception_->u.SQLError.errorList._length = pSrvrStmt->sqlError.errorList._length;
                      exception_->u.SQLError.errorList._buffer = pSrvrStmt->sqlError.errorList._buffer;
                  }
              }
    ...
    {
      char savepointSql[128] = {0};
      switch (savepointOpt) {
          case SQL_BEGIN_SAVEPOINT:
              sprintf(savepointSql, "savepoint %s", savepointName);
              break;
          case SQL_ROLLBACK_SAVEPOINT:
              sprintf(savepointSql, "rollback to %s", savepointName);
              break;
          case SQL_RELEASE_SAVEPOINT:
              sprintf(savepointSql, "release savepoint %s", savepointName);
              break;
          default:
              exception_->exception_nr = odbc_SQLSvc_Savepoint_ParamError_exn_;
              exception_->u.ParamError.ParamDesc = SQLSVC_EXCEPTION_INVALID_SAVEPOINT_OPT;
      ...
      SRVR_STMT_HDL *savepointOp = getSrvrStmt("STMT_NAMED_SAVEPOINT_1", TRUE);

      rc = savepointOp->Prepare(savepointSql, INTERNAL_STMT,  SQL_ASYNC_ENABLE_OFF, 0);
      if (rc == SQL_ERROR)
      {
          GETSQLERROR(savepointOp->bSQLMessageSet, &savepointOp->sqlError);
          ERROR_DESC_def *error_desc_def = savepointOp->sqlError.errorList._buffer;
          if (savepointOp->sqlError.errorList._length != 0 )
          {
              exception_->exception_nr = odbc_SQLSvc_Savepoint_SQLError_exn_;
              exception_->u.SQLError.errorList._length = savepointOp->sqlError.errorList._length;
              exception_->u.SQLError.errorList._buffer = savepointOp->sqlError.errorList._buffer;
          }
          else
          {
              exception_->exception_nr = odbc_SQLSvc_Savepoint_ParamError_exn_;
      ...
      savepointOp->sqlStmtType = TYPE_UNKNOWN;
      savepointOp->cursorName[0] = '\0';
      savepointOp->cursorNameLen = 0;
      savepointOp->inputValueList._buffer = NULL;
      savepointOp->inputValueList._length = 0;

      rc = EXECUTE(savepointOp);
      if (rc == SQL_ERROR)
      {
          GETSQLERROR(savepointOp->bSQLMessageSet, &savepointOp->sqlError);
          ERROR_DESC_def *error_desc_def = savepointOp->sqlError.errorList._buffer;
      ...
      {
          GETSQLWARNING(savepointOp->bSQLMessageSet, &savepointOp->sqlWarning);
          exception_->exception_nr = 0;
          sqlWarning->_length = savepointOp->sqlWarning._length;
          sqlWarning->_buffer = savepointOp->sqlWarning._buffer;
      }
      //savepointOp->Close(SQL_DROP);
    }
    SRVRTRACE_EXIT(FILE_SME+5);
    return;
}
  ...
  )
{
    ...
    char    TypeString[] = " (VALUES ("
        ...
        "('BIGINT UNSIGNED', -5, 20, NULL, NULL, NULL, 1, 0, 2, 1, 0, 0, 'LARGEINT', NULL, NULL, 'UNSIGNED LARGEINT', 138, 10, 20, 21, -405, NULL, NULL, 0, 0, 3, 0), "
        "('BLOB', 2004, 16777216, '''', '''', 'max length', 1, 1, 3, NULL, 0, NULL, 'BINARY LARGE OBJECT', NULL, NULL, 'BINARY LARGE OBJECT', 160, NULL, -1, -1, -602, NULL, NULL, 0, 0, 3, 0), "
        "('BOOLEAN', 16, 1, NULL, NULL, NULL, 1, 0, 2, 0, 0, 0, 'BOOLEAN', NULL, NULL, 'BOOLEAN', 170, 0, NULL, -1, -701, NULL, NULL, 0, 0, 3, 0), "
        "('CHAR', 1, 16777216, '''', '''', 'max length', 1, 1, 3, NULL, 0, NULL, 'CHARACTER', NULL, NULL, 'CHARACTER', 0, NULL, NULL, -1, 1, NULL, NULL, 0, 0, 3, 0), "
        "('CLOB', 2005, 16777216, '''', '''', 'max length', 1, 1, 3, NULL, 0, NULL, 'CHARACTER LARGE OBJECT', NULL, NULL, 'CHARACTER LARGE OBJECT', 161, NULL, NULL, -1, -603, NULL, NULL, 0, 0, 3, 0), "
        ...
        "('DOUBLE PRECISION', 8, 15, NULL, NULL, NULL, 1, 0, 2, 0, 0, 0, 'DOUBLE', NULL, NULL, 'DOUBLE', 143, 10, 54, -1, 8, NULL, NULL, 0, 0, 3, 0), "
        ...
        "('INTERVAL', 101, 0, '{INTERVAL ''', ''' YEAR}', NULL, 1, 0, 2, 0, 0, NULL, 'INTERVAL', 0, 0, 'INTERVAL', 195, 10, 0, 34, 100, 1, 2, 1, 1, 3, 0), "
        "('INTERVAL', 102, 0, '{INTERVAL ''', ''' MONTH}', NULL, 1, 0, 2, 0, 0, NULL, 'INTERVAL', 0, 0, 'INTERVAL', 196, 10, 0, 34, 100, 2, 2, 2, 2, 3, 0), "
        "('INTERVAL', 103, 0, '{INTERVAL ''', ''' DAY}', NULL, 1, 0, 2, 0, 0, NULL, 'INTERVAL', 0, 0, 'INTERVAL', 198, 10, 0, 34, 100, 3, 2, 3, 3, 3, 0), "
        "('INTERVAL', 104, 0, '{INTERVAL ''', ''' HOUR}', NULL, 1, 0, 2, 0, 0, NULL, 'INTERVAL', 0, 0, 'INTERVAL', 199, 10, 0, 34, 100, 4, 2, 4, 4, 3, 0), "
        "('INTERVAL', 105, 0, '{INTERVAL ''', ''' MINUTE}', NULL, 1, 0, 2, 0, 0, NULL, 'INTERVAL', 0, 0, 'INTERVAL', 201, 10, 0, 34, 100, 5, 2, 5, 5, 3, 0), "
        "('INTERVAL', 106, 0, '{INTERVAL ''', ''' SECOND}', NULL, 1, 0, 2, 0, 0, NULL, 'INTERVAL', 0, 0, 'INTERVAL', 204, 10, 0, 34, 100, 6, 2, 6, 6, 3, 0), "
        "('INTERVAL', 107, 0, '{INTERVAL ''', ''' YEAR TO MONTH}', NULL, 1, 0, 2, 0, 0, NULL, 'INTERVAL', 0, 0, 'INTERVAL', 197, 10, 3, 34, 100, 7, 2, 1, 2, 3, 0), "
        "('INTERVAL', 108, 0, '{INTERVAL ''', ''' DAY TO HOUR}', NULL, 1, 0, 2, 0, 0, NULL, 'INTERVAL', 0, 0, 'INTERVAL', 200, 10, 3, 34, 100, 8, 2, 3, 4, 3, 0), "
        "('INTERVAL', 109, 0, '{INTERVAL ''', ''' DAY TO MINUTE}', NULL, 1, 0, 2, 0, 0, NULL, 'INTERVAL', 0, 0, 'INTERVAL', 203, 10, 6, 34, 100, 9, 2, 3, 5, 3, 0), "
        "('INTERVAL', 110, 0, '{INTERVAL ''', ''' DAY TO SECOND}', NULL, 1, 0, 2, 0, 0, NULL, 'INTERVAL', 0, 0, 'INTERVAL', 207, 10, 9, 34, 100, 10, 2, 3, 6, 3, 0), "
        "('INTERVAL', 111, 0, '{INTERVAL ''', ''' HOUR TO MINUTE}', NULL, 1, 0, 2, 0, 0, NULL, 'INTERVAL', 0, 0, 'INTERVAL', 202, 10, 3, 34, 100, 11, 2, 4, 5, 3, 0), "
        "('INTERVAL', 112, 0, '{INTERVAL ''', ''' HOUR TO SECOND}', NULL, 1, 0, 2, 0, 0, NULL, 'INTERVAL', 0, 0, 'INTERVAL', 206, 10, 6, 34, 100, 12, 2, 4, 6, 3, 0), "
        "('INTERVAL', 113, 0, '{INTERVAL ''', ''' MINUTE TO SECOND}', NULL, 1, 0, 2, 0, 0, NULL, 'INTERVAL', 0, 0, 'INTERVAL', 205, 10, 3, 34, 100, 13, 2, 5, 6, 3, 0), "
        "('LONG VARCHAR', -1, 2000, '''', '''', 'max length', 1, 1, 3, NULL, 0, NULL, 'LONG VARCHAR', NULL, NULL, 'VARCHAR', 70, NULL, -1, -1, -1, NULL, NULL, 0, 0, 3, 0), "
        "('NCHAR', -15, 8388608, '''', '''', 'max length', 1, 1, 3, NULL, 0, NULL, 'WCHAR', NULL, NULL, 'WCHAR', 2, NULL, -1, -1, -8, NULL, NULL, 0, 0, 3, 0),"
        "('NCHAR VARYING', -9, 8388608, '''', '''', 'max length', 1, 1, 3, NULL, 0, NULL, 'WCHAR VARYING', NULL, NULL, 'VARWCHAR', 66, NULL, -1, -1, -9, NULL, NULL, 0, 0, 3, 0),"
        ...
        "('REAL', 7, 6, NULL, NULL, NULL, 1, 0, 2, 0, 0, 0, 'REAL', NULL, NULL, 'REAL', 142, 10, 22, -1, 7, NULL, NULL, 0, 0, 3, 0), "
        ...
        "('TIMESTAMP', 93, 26, '{ts ''', '''}', NULL, 1, 0, 2, NULL, 0, NULL, 'TIMESTAMP', 0, 9, 'TIMESTAMP', 192, NULL, 19, 16, 9, 3, NULL, 1, 6, 3, 0), "
        "('VARCHAR', 12, 16777216, '''', '''', 'max length', 1, 1, 3, NULL, 0, NULL, 'VARCHAR', NULL, NULL, 'VARCHAR', 64, NULL, -1, -1, 12, NULL, NULL, 0, 0, 3, 0), "
        "('TINYINT', -6, 3, NULL, NULL, NULL, 1, 0, 2, 0, 0, 0, 'TINYINT', NULL, NULL, 'SIGNED TINYINT', 136, 10, 3, -1, -403, NULL, NULL, 0, 0, 3, 0), "
        "('TINYINT UNSIGNED', -6, 3, NULL, NULL, NULL, 1, 0, 2, 1, 0, 0, 'TINYINT', NULL, NULL, 'UNSIGNED TINYINT', 137, 10, 3, -1, -404, NULL, NULL, 0, 0, 3, 0) "
        " ) "
        ...
        char                          CatalogQuery[20000] = {0};
        SRVR_STMT_HDL                 *QryCatalogSrvrStmt = NULL;
        SRVR_STMT_HDL                 *setHashJoinSrvrStmt = NULL;
	...
	char catalogNmNoEsc[MAX_ANSI_NAME_LEN+1] = {0};
	char schemaNmNoEsc[MAX_ANSI_NAME_LEN+1] = {0};
	char tableNmNoEsc[MAX_ANSI_NAME_LEN+1] = {0};
	char columnNmNoEsc[MAX_ANSI_NAME_LEN+1] = {0};

	char expCatalogNm[MAX_ANSI_NAME_LEN+1] = {0};
	char expSchemaNm[MAX_ANSI_NAME_LEN+1] = {0};
	char expTableNm[MAX_ANSI_NAME_LEN+1] = {0};
	char expColumnNm[MAX_ANSI_NAME_LEN+1] = {0};
    ...
    char mapCatalog[MAX_ANSI_NAME_LEN + 36 + 1] = {0};
    char mapSchema[MAX_ANSI_NAME_LEN + 35 + 1] = {0};
    char mapTable[MAX_ANSI_NAME_LEN + 35 + 1] = {0};
    char mapColumn[MAX_ANSI_NAME_LEN + 35 + 1] = {0};
	...
	_itoa(srvrGlobal->appVersion.majorVersion, odbcAppVersion, 10);
        ...
        switch(APIType)
        {
               ...
               case SQL_API_SQLTABLES_JDBC :
                       if ((strcmp(catalogNm,"%") == 0) && (strcmp(schemaNm,"") == 0) && (strcmp(tableNm,"") == 0))
                       {
                         ...
                         Int32 Cataloglen, SchemaLen;
                         SQL_EXEC_GetCurrentCatalog(catalogNmNoEsc, MAX_ANSI_NAME_LEN, Cataloglen, NULL, 0, SchemaLen);
                         ...
                         inputParam[1] = inputParam[0];
                              ...
                              if (APIType == SQL_API_SQLTABLES)
                              {
                                 ...
                                 snprintf(CatalogQuery, sizeof(CatalogQuery),
                                         "select (cast('TRAFODION' as varchar(128))) TABLE_CAT, "
                                         ...
                                         "from dual "
                                         "where upper('%s') = 'TRAFODION' "
                                         "UNION "
                                         "SELECT "
                                         "CAST('HIVE' AS VARCHAR(128)) TABLE_CAT, "
                                         "CAST(NULL AS VARCHAR(128)) TABLE_SCHEM, "
                                         "CAST(NULL AS VARCHAR(128)) TABLE_NAME, "
                                         "CAST(NULL AS VARCHAR(128)), "
                                         "CAST(NULL AS VARCHAR(128)) REMARKS FROM dual "
                                         "where upper('%s') = 'HIVE';",
                       ...
                       if ((strcmp(schemaNm,"%") == 0) && (strcmp(tableNm,"") == 0))
                       {
                         convertWildcard(metadataId, TRUE, schemaNm, expSchemaNm);
                         if (!catalogNm || strcmp(catalogNm,"") == 0)
                          {
                            Int32 Cataloglen, SchemaLen;
                            SQL_EXEC_GetCurrentCatalog(catalogNmNoEsc, MAX_ANSI_NAME_LEN, Cataloglen, 
                                                       NULL, 0, SchemaLen);
                          }
                         else
                         ...
                         if (APIType == SQL_API_SQLTABLES)
                           {
                             ...
                             snprintf(CatalogQuery, sizeof(CatalogQuery),
                                     "select "
                                     "cast(CAT_NAME as varchar(128)) as TABLE_CAT, "
                                     "cast(SCH_NAME as varchar(128)) as TABLE_SCHEM, "
                                     "cast(NULL as varchar(128) ) TABLE_NAME, "
                                     "cast(NULL as varchar(128) ) TABLE_TYPE, "
                                     "cast(NULL as varchar(128)) REMARKS "
                                     "from "
                                     "(select "
                                     "substring(a, 1, position(_utf8'.' in a)-1) as CAT_NAME, "
                                     "substring(a, position(_utf8'.' in a) + 1) as SCH_NAME "
                                     "from (get schemas for user %s, no header) as X(a) "
                                     ") as gt "
                                     "where "
                                     "CAT_NAME = upper('%s') "
                                     "and trim(replace(SCH_NAME, '\"', ' ')) not like _utf8'VOLATILE_SCHEMA_%%' "
                                     "and trim(replace(SCH_NAME, '\"', ' ')) not like _utf8'|_BACKUP|_%%' escape _utf8'|' "
                                     "and trim(replace(SCH_NAME, '\"', ' ')) not like '|_HV|_%%|_' escape _utf8'|' "
                                     "and trim(replace(SCH_NAME, '\"', ' ')) <> _utf8'_HB_MAP_' "
                                     "and trim(replace(SCH_NAME, '\"', ' ')) <> _utf8'_HBASESTATS_' "
                                     "and trim(replace(SCH_NAME, '\"', ' ')) <> _utf8'_HIVESTATS_' "
                                     "FOR READ UNCOMMITTED ACCESS ORDER BY 4,1,2,3;",
                                     srvrGlobal->QSDBUserName, inputParam[0]);
                           }
                         else
                           {
                             // strcpy((char *)catStmtLabel, "SQL_JAVA_TABLES_ANSI_Q2");
                               snprintf(CatalogQuery,sizeof(CatalogQuery),
                                       "select "
                                       "cast(SCH_NAME as varchar(128)) as TABLE_SCHEM, "
                                       "cast(CAT_NAME as varchar(128)) as TABLE_CATALOG "
                                       "from "
                                       "(select "
                                       "substring(a, 1, position(_utf8'.' in a)-1) as CAT_NAME, "
                                       "substring(a, position(_utf8'.' in a) + 1) as SCH_NAME "
                                       ...
                                       "where "
                                       "CAT_NAME = upper('%s') "
                                       "and replace(SCH_NAME, '\"', '') not like _utf8'VOLATILE_SCHEMA_%%' "
                                       "and replace(SCH_NAME, '\"', '') not like _utf8'|_BACKUP|_%%' escape _utf8'|' "
                                       "and replace(SCH_NAME, '\"', '') not like '|_HV|_%%|_' escape _utf8'|' "
                                       "and replace(SCH_NAME, '\"', '') <> _utf8'_HB_MAP_' "
                                       "and replace(SCH_NAME, '\"', '') <> _utf8'_HBASESTATS_' "
                                       "and replace(SCH_NAME, '\"', '') <> _utf8'_HIVESTATS_' "
                                       "FOR READ UNCOMMITTED ACCESS ORDER BY 2,1;",
                                       srvrGlobal->QSDBUserName, inputParam[0]);
                             
                           }
                       }
                       else
                         if ((strcmp(catalogNm,"") == 0) && (strcmp(schemaNm,"") == 0) && 
                             (strcmp(tableNm,"") == 0) && (strcmp(tableTypeList,"%") == 0))
                         {
                        ...
                        else
                        {
                                ...
                                tableParam[1] = NULL;
                                convertWildcardNoEsc(metadataId, TRUE, catalogNm, catalogNmNoEsc);
                                convertWildcardNoEsc(metadataId, TRUE, catalogNm, expCatalogNm);
                                ...
                                if (APIType == SQL_API_SQLTABLES)
                                {
                                   ...
                                   snprintf(CatalogQuery, sizeof(CatalogQuery),
                                            ...
                                            "cast(CAT_NAME as varchar(128)) as TABLE_CAT, "
                                            "cast(SCH_NAME as varchar(128)) as TABLE_SCHEM, "
                                            "cast(OBJ_NAME as varchar(128)) as TABLE_NAME, "
                                            "case OBJECT_TYPE "
                                            "  when 'BT' then 'TABLE' "
                                            "  when 'VI' then 'VIEW' "
                                            "  else 'UNKNOWN' end TABLE_TYPE, "
                                            "cast(NULL as varchar(128)) REMARKS "
                                            "from TRAFODION.\"_MD_\".OBJECTS ob, "
                                            "(select "
                                            "substring(a,1,2) as OBJ_TYPE, "
                                            "substring(a, 4, position(_utf8'.' in a) -4) as CAT_NAME, "
                                            "substring(a, position(_utf8'.' in a) +1, position(_utf8'.' in substring(a, position(_utf8'.' in a) +1)) -1) as SCH_NAME, "
                                            "substring(a, 1+ position(_utf8'.' in substring(a, position('.' in substring(a, position(_utf8'.' in a))))) +1 "
                                            "  + char_length(substring(a, position(_utf8'.' in a) + 1, position('.' in substring(a, position(_utf8'.' in a) +1)) -1))) as OBJ_NAME "
                                            "from (get objects for user %s, no header) as X(a) "
                                            ") as gt "
                                            "where ob.CATALOG_NAME = gt.CAT_NAME "
                                            "and ob.SCHEMA_NAME = trim(replace (gt.SCH_NAME, '\"', ' ')) "
                                            "and ob.OBJECT_NAME = trim(replace (gt.OBJ_NAME, '\"', ' ')) "
                                            "and (ob.CATALOG_NAME = upper('%s') or "
                                            "trim(ob.CATALOG_NAME) LIKE '%s' ESCAPE '\\') "
                                            "and (ob.SCHEMA_NAME = '%s' or "
                                            "trim(ob.SCHEMA_NAME) LIKE '%s' ESCAPE '\\') "
                                            "and (ob.OBJECT_NAME = '%s' or "
                                            "trim(ob.OBJECT_NAME) LIKE '%s' ESCAPE '\\') "
                                            "and ((SCHEMA_NAME not like 'VOLATILE_SCHEMA_%%' "
                                            "  and SCHEMA_NAME not like '|_BACKUP|_%%' escape '|' "
                                            "  and SCHEMA_NAME not like '|_HV|_%%|_' escape '|' "
                                            "  and SCHEMA_NAME <> '_HB_MAP_' "
                                            "  and SCHEMA_NAME <> '_MD_' "
                                            "  and '%s'='UT' "
                                            "  and OBJECT_TYPE in ('%s', '%s')) "
                                            "or (SCHEMA_NAME = '_MD_' and '%s'='SM' and OBJECT_TYPE in ('%s'))) "
                                            "FOR READ UNCOMMITTED ACCESS ORDER BY 4,1,2,3;",
                                        srvrGlobal->QSDBUserName,
                                        catalogNmNoEsc, expCatalogNm,
                                        inputParam[0], inputParam[1],
                                ...
                                else
                                  {
                                   // strcpy((char *)catStmtLabel, "SQL_JAVA_TABLES_ANSI_Q7");
                                                 snprintf(CatalogQuery,sizeof(CatalogQuery),
                                                         ...
                                                         ") as gt "
                                                         "where ob.CATALOG_NAME = gt.CAT_NAME "
                                                         "and ob.SCHEMA_NAME = replace (gt.SCH_NAME, '\"','') "
                                                         "and ob.OBJECT_NAME = replace (gt.OBJ_NAME, '\"', '') "
                                                         "and (ob.CATALOG_NAME = upper('%s') or "
                                                         "trim(ob.CATALOG_NAME) LIKE '%s' ESCAPE '\\') "
                                                         "and (ob.SCHEMA_NAME = '%s' or "
                                                         "trim(ob.SCHEMA_NAME) LIKE '%s' ESCAPE '\\') "
		...
		//	strcpy((char *)catStmtLabel, "SQL_PROCEDURES_ANSI_Q4");
			...
			inputParam[2] = tableNmNoEsc;
			inputParam[3] = expTableNm;
			inputParam[4] = catalogNmNoEsc;
			inputParam[5] = expCatalogNm;
			inputParam[6] = NULL;
 ...
 "FOR READ UNCOMMITTED ACCESS ORDER BY 4, 1, 2, 3 ;",
			...
			else
			{
                snprintf(CatalogQuery,sizeof(CatalogQuery),
                        "select "
                        "cast (gt.CAT_NAME as varchar(128)) as PROCEDURE_CAT, "
                        "cast (gt.SCH_NAME as varchar (128)) as PROCEDURE_SCHEM, "
                        "cast (gt.OBJ_NAME as varchar (128)) as PROCEDURE_NAME, "
                        "cast(NULL as varchar(10)) R1, "
                        "cast(NULL as varchar(10)) R2, "
                        "cast(NULL as varchar(10)) R3, "
                        "cast(NULL as varchar(10)) REMARKS, "
                        "case when rtns.UDR_TYPE = 'P' then 1 "
                        "     when rtns.UDR_TYPE = 'F' then 2 "
                        "     when rtns.UDR_TYPE = 'T' then 3 "
                        "     else 0 "
                        "     end as PROCEDURE_TYPE, "
                        "ob.OBJECT_NAME SPECIFIC_NAME "
                        "from TRAFODION.\"_MD_\".OBJECTS ob left join TRAFODION.\"_MD_\".ROUTINES rtns on ob.OBJECT_UID = rtns.UDR_UID, "
                        "(select "
                        "   substring(a,1,2) as OBJ_TYPE, "
                        "   substring(a, 4, position(_utf8'.' in a) -4) as CAT_NAME, "
                        "   substring(a, position(_utf8'.' in a) + 1, position(_utf8'.' in "
                        "       substring(a, position(_utf8'.' in a) +1)) -1) as SCH_NAME, "
                        "   substring(a, 1+ position(_utf8'.' in substring(a, position(_utf8'.' in "
                        "       substring(a, position(_utf8'.' in a) ) ) ) )+1 + char_length( "
                        "       substring(a, position(_utf8'.' in a) + 1, position(_utf8'.' in "
                        "       substring(a, position(_utf8'.' in a) +1)) -1))) as OBJ_NAME "
                        "from (get objects for user %s, no header) as X(a) "
                        ") as gt "
                        "where ob.CATALOG_NAME = gt.CAT_NAME "
                        "and ob.SCHEMA_NAME = replace(gt.SCH_NAME, '\"', '') "
...
" FOR READ UNCOMMITTED ACCESS",
		...
		case SQL_API_SQLCOLUMNS :
			...
			//		strcpy(tableName1, catalogNm);
                ...
                convertFuzzyCard("ob.SCHEMA_NAME" , inputParam[0], inputParam[1], mapSchema);
                convertFuzzyCard("ob.OBJECT_NAME" , inputParam[2], inputParam[3], mapTable);
                convertFuzzyCard("co.COLUMN_NAME" , inputParam[4], inputParam[5], mapColumn);

                                if((strcmp(catalogNm,"hive") == 0) || (strcmp(catalogNm, "HIVE") == 0))
                                {
                                  if((strcmp(schemaNm,"%")!= 0) && (strcmp(schemaNm,"")!= 0) &&
                                     (strcmp(tableNm,"%")!= 0) && (strcmp(tableNm,"")!= 0)) 
                                  { // optimization for hivemd(columns) with schema+table names
                                    snprintf(CatalogQuery,sizeof(CatalogQuery),
  "select "
  "cast('HIVE' as varchar(128)) TABLE_CAT, "
  "cast(upper(trim(co.SCHEMA_NAME)) as varchar(128)) TABLE_SCHEM, "
  "cast(upper(trim(co.TABLE_NAME)) as varchar(128)) TABLE_NAME, "
  "cast(upper(trim(co.COLUMN_NAME)) as varchar(128)) COLUMN_NAME, "
  "cast(dt.DATA_TYPE as smallint) DATA_TYPE, "
  ...
  "cast((case when (trim('U') <> 'S') then co.column_number+1 else "
  ...
  "from "
  "TABLE(HIVEMD(COLUMNS, \"%s\", \"%s\")) co, "
  "%s "
  "where dt.FS_DATA_TYPE = co.FS_DATA_TYPE "
  "and (dt.DATETIMESTARTFIELD = co.DATETIME_START_FIELD) "
  "and (dt.DATETIMEENDFIELD = co.DATETIME_END_FIELD) "
  "and ('HIVE' = upper('%s') or 'HIVE' LIKE '%s' ESCAPE '\\') "
  "and (co.SCHEMA_NAME = lower('%s') or trim(co.SCHEMA_NAME) LIKE '%s' ESCAPE '\\') "
  "and (co.TABLE_NAME = lower('%s') or trim(co.TABLE_NAME) LIKE '%s' ESCAPE '\\') "
  "and (co.COLUMN_NAME = lower('%s') or trim(co.COLUMN_NAME) LIKE '%s' ESCAPE '\\') "
  "FOR READ UNCOMMITTED ACCESS order by 1, 2, 3, ORDINAL_POSITION; ",
    inputParam[0], inputParam[2],  //hive schema, table names
    TypeString,
    catalogNmNoEsc, expCatalogNm,
    inputParam[0], inputParam[1], //HIVE SCHEMA_NAME
    inputParam[2], inputParam[3], //HIVE TABLE_NAME
    inputParam[4], inputParam[5] //HIVE COLUMN_NAME
        );
                                  }
                                  else { // no optimization for hivemd(columns) when schema/table no specified. 
                                    snprintf(CatalogQuery,sizeof(CatalogQuery),
  "select "
  "cast('HIVE' as varchar(128)) TABLE_CAT, "
  "cast(upper(trim(co.SCHEMA_NAME)) as varchar(128)) TABLE_SCHEM, "
  "cast(upper(trim(co.TABLE_NAME)) as varchar(128)) TABLE_NAME, "
  "cast(upper(trim(co.COLUMN_NAME)) as varchar(128)) COLUMN_NAME, "
  "cast(dt.SQL_DATA_TYPE as smallint) DATA_TYPE, "
  "trim(dt.TYPE_NAME) TYPE_NAME, "
  "cast((case when co.FS_DATA_TYPE = 0 and co.character_set = 'UCS2' then co.COLUMN_SIZE/2 "
  "when co.FS_DATA_TYPE = 64 and co.character_set = 'UCS2' then co.COLUMN_SIZE/2 "
  "when dt.USEPRECISION = -1 then co.COLUMN_SIZE when dt.USEPRECISION = -2 then co.COLUMN_PRECISION "
  ...
  "FOR READ UNCOMMITTED ACCESS order by 1, 2, 3, ORDINAL_POSITION; ",
    ...
    inputParam[4], inputParam[5] //HIVE COLUMN_NAME
                                  ...
                                  } // end of else for hive alone, no schema+table names case
                                }// end of check for hive columns
                                  else 
                                    { // Trafodion tables come here
                                    snprintf(CatalogQuery,sizeof(CatalogQuery),
  "select "
  "cast('TRAFODION' as varchar(128) ) TABLE_CAT, "
  "cast(ob.SCHEMA_NAME as varchar(128) ) TABLE_SCHEM, "
  "cast(ob.OBJECT_NAME as varchar(128) ) TABLE_NAME, "
  "cast(co.COLUMN_NAME as varchar(128) ) COLUMN_NAME, "
  "cast((case when co.FS_DATA_TYPE = 0 and co.character_set = 'UCS2' then -8 "
  "when co.FS_DATA_TYPE = 64 and co.character_set = 'UCS2' then -9 else dt.DATA_TYPE end) as smallint) DATA_TYPE, "
  "dt.TYPE_NAME TYPE_NAME, "
  "cast((case when co.FS_DATA_TYPE = 0 and co.character_set = 'UCS2' then co.COLUMN_SIZE/2 "
    "when co.FS_DATA_TYPE = 64 and co.character_set = 'UCS2' then co.COLUMN_SIZE/2 "
    "when dt.USEPRECISION = -1 then co.COLUMN_SIZE when dt.USEPRECISION = -2 then co.COLUMN_PRECISION "
    "when co.FS_DATA_TYPE = 192 then dt.USEPRECISION + 1 "
...
"cast((case when dt.USELENGTH = -1 then co.COLUMN_SIZE when dt.USELENGTH = -2 then co.COLUMN_PRECISION "
   "when dt.USELENGTH = -3 then co.COLUMN_PRECISION + 2  "
   "else dt.USELENGTH end) as integer) BUFFER_LENGTH, "
  "(case when co.FS_DATA_TYPE = 161 or co.FS_DATA_TYPE = 160 then 0 else cast(co.COLUMN_SCALE as smallint) end) DECIMAL_DIGITS, "
"cast(dt.NUM_PREC_RADIX as smallint) NUM_PREC_RADIX, "
 "cast(co.NULLABLE as smallint) NULLABLE, "
 "cast('' as varchar(128)) REMARKS, "
 "co.DEFAULT_VALUE COLUMN_DEF, "
  "cast((case when co.FS_DATA_TYPE = 0 and co.character_set = 'UCS2' then -8 "
     "when co.FS_DATA_TYPE = 64 and co.character_set = 'UCS2' then -9 else dt.SQL_DATA_TYPE end) as smallint) SQL_DATA_TYPE, "
  "cast(dt.SQL_DATETIME_SUB as smallint) SQL_DATETIME_SUB, cast((case dt.DATA_TYPE when 1 then co.COLUMN_SIZE "
    "when -1 then co.COLUMN_SIZE when 12 then co.COLUMN_SIZE else NULL end) as integer) CHAR_OCTET_LENGTH, "
    "cast(co.column_number+1 as integer) ORDINAL_POSITION, "
  "cast((case when co.NULLABLE = 0 then 'NO' else 'YES' end) as varchar(3)) IS_NULLABLE  "
"from  "
...
"%s "
"where ob.CATALOG_NAME = 'TRAFODION' "
"and ob.OBJECT_UID = co.OBJECT_UID "
  ...
  "and (co.COLUMN_CLASS not in ('S', 'M')) "
 "FOR READ UNCOMMITTED ACCESS order by 1, 2, 3, ORDINAL_POSITION; ",
    TypeString,
    mapSchema,
    mapTable,
  ...
  "cast('NO' as varchar(8)) IS_AUTOINCREMENT, "
  "cast('NO' as varchar(8)) IS_GENERATEDCOLUMN "
  "from "
  "TABLE(HIVEMD(COLUMNS, \"%s\", \"%s\")) co, "
  "%s "
  "where dt.FS_DATA_TYPE = co.FS_DATA_TYPE "
 ...
 "cast(NULL as varchar(128)) SCOPE_CATALOG, "
  ...
  "and (dt.DATETIMEENDFIELD = co.DATETIME_END_FIELD) "
  "and ('HIVE' = upper('%s') or 'HIVE' LIKE '%s' ESCAPE '\\') "
  "and (co.SCHEMA_NAME = upper('%s') or trim(co.SCHEMA_NAME) LIKE '%s' ESCAPE '\\') "
  "and (co.TABLE_NAME = upper('%s') or trim(co.TABLE_NAME) LIKE '%s' ESCAPE '\\') "
  "and (co.COLUMN_NAME = upper('%s') or trim(co.COLUMN_NAME) LIKE '%s' ESCAPE '\\') "
  "FOR READ UNCOMMITTED ACCESS order by 1, 2, 3, ORDINAL_POSITION; ",
    TypeString,
    catalogNmNoEsc, expCatalogNm,
    inputParam[0], inputParam[1], //HIVE SCHEMA_NAME
    ...
    inputParam[4], inputParam[5] //HIVE COLUMN_NAME
                ...
                snprintf(CatalogQuery,sizeof(CatalogQuery),
                        ...
                        "cast((case when co.FS_DATA_TYPE = 0 and co.character_set = 'UCS2' then co.COLUMN_SIZE/2 "
                        "when co.FS_DATA_TYPE = 64 and co.character_set = 'UCS2' then co.COLUMN_SIZE/2 "
                        "when co.character_set = 'UTF8' and co.COLUMN_PRECISION <>0 then co.COLUMN_PRECISION "
                        "when co.character_set = 'UTF8' and co.COLUMN_PRECISION =0 then co.COLUMN_SIZE "
                        "when dt.USEPRECISION = -1 then co.COLUMN_SIZE when dt.USEPRECISION = -2 then co.COLUMN_PRECISION "
...
*/
 ...
 " trim(ob.OBJECT_NAME) LIKE '%s' ESCAPE '\\') "
  ...
  " AND ky.COLUMN_NAME != 'SYSKEY' "
   " AND ob.OBJECT_TYPE = 'BT' "
  " FOR READ UNCOMMITTED ACCESS order by 1, 2, 3, 5 ;",
        ...
        case SQL_API_SQLSTATISTICS:
            ...
            snprintf(CatalogQuery, sizeof(CatalogQuery),
                    ...
                    "cast(SUBSTR(trim(k.COLUMN_NAME) from 1 for CHAR_LENGTH (trim(k.COLUMN_NAME))-1) as varchar(128)) COLUMN_NAME, "
                    ...
                    "(select "
                    "replace(cast (substring(a, 1, position('.' in a) -1) as varchar(128) character set iso88591), '\"',' ') as CAT_NAME, "
                    "replace(cast (substring(a, position('.' in a) + 1, position('.' in substring(a, position('.' in a) +1)) -1) as varchar(128) character set iso88591), '\"', '') as SCH_NAME, "
                    "replace(cast (substring(a, 1+ position('.' in substring(a, position('.' in substring(a, position('.' in a) ) ) ) )+1 + char_length(substring(a, position('.' in a) + 1, position('.' in substring(a, position('.' in a) +1)) -1))) as varchar(128) character set iso88591 ), '\"', '') as OBJ_NAME "
                    "from (get indexes for user %s) as X(a)) as gi "
                    "where "
                    "obj_idx.CATALOG_NAME = gi.CAT_NAME "
                    "and obj_idx.SCHEMA_NAME = gi.SCH_NAME "
                    "and obj_idx.OBJECT_NAME = gi.OBJ_NAME "
                    "and idx.INDEX_UID = obj_idx.OBJECT_UID and idx.BASE_TABLE_UID = obj_tbl.OBJECT_UID and idx.index_uid = k.OBJECT_UID "
        ...
        if (exception_->exception_nr == 0)
        {
           if (mapTable[0] != '\0' && (APIType == SQL_API_SQLCOLUMNS || APIType == SQL_API_SQLCOLUMNS_JDBC))
           {
                if ((setHashJoinSrvrStmt = getSrvrStmt("MXOSRVR_CLOSE_HASH_JOIN", TRUE)) == NULL)
                {
                    SendEventMsg(MSG_MEMORY_ALLOCATION_ERROR,
                            EVENTLOG_ERROR_TYPE,
                            srvrGlobal->nskProcessInfo.processId,
                            ...
                            srvrGlobal->srvrObjRef,
                            1,
                            "getColumns set hash_joins");
                }
                else
                {
                    try
                    {
                        char cqsOn[] = "control query shape sort(nested_join(nested_join(nested_join("
                            "scan(TABLE 'OB', path 'TRAFODION.\"_MD_\".OBJECTS', forward"
                                    ", blocks_per_access 2 , mdam off),anything),"
                            "scan(TABLE 'CO', path 'TRAFODION.\"_MD_\".COLUMNS', forward"
                                    ", blocks_per_access 1 , mdam off)),"
                            "scan(TABLE 'CO1', path 'TRAFODION.\"_MD_\".COLUMNS', forward"
                                    ", blocks_per_access 1 , mdam off)));";
                        setHashJoinSrvrStmt->ExecDirect(NULL, cqsOn, EXTERNAL_STMT, TYPE_UNKNOWN, SQL_ASYNC_ENABLE_OFF, 0);
                    }
                    catch (...)
                    {
                    }
        ...
        if (setHashJoinSrvrStmt != NULL)
        {
            try
            {
                setHashJoinSrvrStmt->ExecDirect(NULL, "control query shape cut", INTERNAL_STMT, TYPE_UNKNOWN, SQL_ASYNC_ENABLE_OFF, 0);
            }
            catch (...)
            {
            }
  ...
  )
{
   ...
   short PWDLength = userDesc->password._length;

   struct sockaddr_in clientaddr;
   socklen_t addrlen = sizeof(clientaddr);
   int conn = ((CTCPIPSystemSrvr* )objtag_)->m_nSocketFnum;
   getpeername (conn, (struct sockaddr *) &clientaddr, &addrlen);
...
int lifetime = 0;
   ...
   if (userDesc->userDescType != AUTHENTICATED_USER_TYPE)
   {
      ...
      Int32 resetAttributes = 0;    // Currently used as a boolean, may be used in the future as a bitmap.
      ...
      if(srvrGlobal->encryptBase64Enable && (srvrGlobal->drvrVersion.buildId & PASSWORD_SECURITY_BASE64) > 0) {
          retcode = base64_decrypt_password(pPWD, PWDLength);
      } else {
      ...
      if (retcode == SECMXO_NO_ERROR)
      {
         ...
         length = strlen(srvrGlobal->tenantName);
         for (size_t i = 0; i < length; i++)
            srvrGlobal->tenantName[i] = toupper(srvrGlobal->tenantName[i]);
        ...
        {
            lifetime = authenticationInfo.error;
            authenticationInfo.error = 0;
        }
      }
      if (retcode == 0)
      {
         if(strlen(srvrGlobal->QSRoleName) ==0 )
            strcpy(srvrGlobal->QSRoleName, "NONE");
         if((srvrGlobal->bMultiTenancyEnabled) &&
            (srvrGlobal->tenantID == authenticationInfo.usersInfo.tenantID))
         {
               resetAttributes = (srvrGlobal->tenantRedefTime != authenticationInfo.usersInfo.tenantRedefTime);
         }

      }

      if (authenticationInfo.usersInfo.client_host_name)
      {
          delete authenticationInfo.usersInfo.client_host_name;
          authenticationInfo.usersInfo.client_host_name = NULL;
      }
      authenticationInfo.usersInfo.client_host_name = new char[MAX_CLIENT_HOSTNAME_LEN + 1];
      if (authenticationInfo.usersInfo.client_user_name)
      {
          delete authenticationInfo.usersInfo.client_user_name;
          authenticationInfo.usersInfo.client_user_name = NULL;
      }
      authenticationInfo.usersInfo.client_user_name = new char[MAX_CLIENT_OSNAME_LEN + 1];
      if (authenticationInfo.usersInfo.client_addr)
      {
          delete authenticationInfo.usersInfo.client_addr;
          authenticationInfo.usersInfo.client_addr = NULL;
      }
      authenticationInfo.usersInfo.client_addr = new char[INET6_ADDRSTRLEN];

      strncpy(authenticationInfo.usersInfo.client_host_name, client_info.client_name, MAX_CLIENT_HOSTNAME_LEN);
      strncpy(authenticationInfo.usersInfo.client_user_name, client_info.client_user_name, MAX_CLIENT_OSNAME_LEN);
      inet_ntop(AF_INET, &clientaddr.sin_addr, authenticationInfo.usersInfo.client_addr, INET6_ADDRSTRLEN);
      ...
      if (authenticationInfo.error == 0)
      {
         retcode = WSQL_EXEC_SetAuthID2(authenticationInfo.usersInfo,
                                        authenticationInfo.tokenKey,
                                        authenticationInfo.tokenKeySize,
                                        srvrGlobal->mappedSLAName,
                                        srvrGlobal->mappedProfileName,
                                        resetAttributes
                                        );
        if (retcode < 0)
        {
          authenticationInfo.error = retcode;
          authenticationInfo.errorDetail = UA_STATUS_ERR_SETAUTHID;
        } 
      }
      if (authenticationInfo.error == 0 && retcode == 0)
      {
        if (authenticationInfo.errorDetail == UA_STATUS_PASSWORD_WILL_EXPIRE ||
            authenticationInfo.errorDetail == UA_STATUS_PASSWORD_GRACE_COUNT)
        {
            bool bSQLMessageSet;
            exception_->exception_detail = (authenticationInfo.errorDetail == UA_STATUS_PASSWORD_WILL_EXPIRE) ? 8857 : 8837;

            setAuthenticationError(bSQLMessageSet,
                                    &(exception_->u.SQLError),
                                    userDesc->userName,
                                    lifetime,
                                    authenticationInfo.errorDetail,
                                    authenticationInfo.usersInfo.tenantName);
        }

         srvrGlobal->redefTime = authenticationInfo.usersInfo.redefTime;
         if (srvrGlobal->bMultiTenancyEnabled)
         {
            const char* ptr = (NULL == authenticationInfo.usersInfo.tenantName) ? "ESGYNDB"
            : authenticationInfo.usersInfo.tenantName;
            strcpy(srvrGlobal->tenantName, ptr);
            srvrGlobal->tenantID = authenticationInfo.usersInfo.tenantID;
            srvrGlobal->tenantRedefTime = authenticationInfo.usersInfo.tenantRedefTime;
         }
      }
      else
      {
         ...
         setAuthenticationError(bSQLMessageSet,
                                ...
                                authenticationInfo.errorDetail,
                                authenticationInfo.usersInfo.tenantName);
         if (authenticationInfo.errorDetail = UA_STATUS_ERR_SYSTEM)
         {
            exception_->exception_nr = odbc_SQLSvc_InitializeDialogue_SQLError_exn_;
 ...
 )
{
	...
	char *endPtr;
    char clientInfo[MAX_CLIENT_INFO_LEN] = "";
    Int32 clientInfoLength = 0;
		...
		case SQL_TXN_READ_COMMITTED:
			...
			break;
                case SQL_TXN_REPEATABLE_READ:
                        strcpy(sqlString, "SET TRANSACTION READ WRITE, ISOLATION LEVEL REPEATABLE READ");
                        break;
                case SQL_TXN_SERIALIZABLE:
                        strcpy(sqlString, "SET TRANSACTION READ WRITE, ISOLATION LEVEL SERIALIZABLE");
                        break;
		default:
			exception_->exception_nr = odbc_SQLSvc_SetConnectionOption_ParamError_exn_;
...
//Special Case//
	...
	case SET_JDBC_PROCESS:
		...
		break;
	case SET_SESSION_DEBUG:

		sqlStringNeedsExecution = false;
		sessionDebug = optionValueNum;
		if(optionValueNum > 0)
			MXO_WARN("sessionDebug is on!");
		else
			MXO_WARN("sessionDebug is off!");
		break;
	...
	case WMS_QUERY_MONITORING:
		...
		break;
    case SET_CLIPVARCHAR:
        srvrGlobal->clipVarchar = optionValueNum;
        if(optionValueNum > 0)
			MXO_WARN("clipVarchar is on! Value = %lld",optionValueNum);
		else 
			MXO_WARN("clipVarchar is off!");
		sqlStringNeedsExecution = false;
        break;
    case SQL_ATTR_TRANSACTION_TYPE:
        if (optionValueStr != NULL && strlen(optionValueStr) >= 0)
        {
            clientInfoLength = snprintf(clientInfo, MAX_CLIENT_INFO_LEN, "TransactionType:%s", optionValueStr);
            WSQL_EXEC_SetClientInfo(clientInfo, clientInfoLength);
        }
	return;
        break;
	default:
...
//			if (pSrvrStmt->sqlBulkFetchPossible && (pSrvrStmt->sqlQueryType == SQL_SELECT_NON_UNIQUE || pSrvrStmt->sqlQueryType == SQL_SP_RESULT_SET))
			if (srvrGlobal->drvrVersion.buildId & ROWWISE_ROWSET)
			{
				*outValuesFormat = ROWWISE_ROWSETS;
				...
				currentTimeMillis(leaveEngineTimestamp);
				pSrvrStmt->logDelayedResponseMessage("FETCH2bulk");
				tmpLength = clipVarchar(pSrvrStmt, calledByFetch);
				SESSION_DEBUG("dbc_SQLSrvr_FetchPerf_ reutrn %lld",tmpLength);
				pSrvrStmt->logDelayedResponseMessage("FETCH2bulk");
  ...
  )
{
    ...
    int lobV2 = 0;
    Int64 objectUID = 0;
    SQL_EXEC_ExtractFieldsFromLobHandleStr(lobHandle, lobLength,
                                           &lobV2, &objectUID, NULL, NULL);

    IDL_long_long inExtractLen = extractLen;
    if (lobV2)
    {
        if (stmtHandle == 0)
        {
        ...
        int stmtId = stmtHandleKey;

        if ((QryLobExtractSrvrStmt = pSrvrStmt->getLobExtractStmt(objectUID,stmtHandleKey, exception_)) == NULL) {
            return;
        }
        if(extractLobAPI == 2){
            pSrvrStmt->closeExtractLob();
            return;
        }
        rc = QryLobExtractSrvrStmt->extractLob(lobHandle, extractData, extractLen, exception_);
        switch (extractLobAPI) {
        case 0:
            lobLength =  *(IDL_long_long *)extractData;
            extractData = NULL;
            break;
        }
        ...
        if (rc != SQL_SUCCESS && rc != SQL_SUCCESS_WITH_INFO)
            pSrvrStmt->closeExtractLob();
        else if (extractLen == 0 ||
                (extractLen == QryLobExtractSrvrStmt->getLobChunkMaxLen() && extractLen < inExtractLen) ||
		        (extractLen < QryLobExtractSrvrStmt->getLobChunkMaxLen() && extractLen <= inExtractLen))
            pSrvrStmt->closeExtractLob();
        return;
    }
    ...
    case 0:
      if (lobV2)
        snprintf(LobExtractQuery, sizeof(LobExtractQuery), "EXTRACT LOBLENGTH(TABLE UID '%020ld', LOB'%s') LOCATION %Ld", objectUID, lobHandle, (Int64)&lobLength);
      else
    ...
    case 1:
        ...
        if (lobV2)
          snprintf(LobExtractQuery, sizeof(LobExtractQuery), "EXTRACT LOBTOBUFFER(TABLE UID '%020ld', LOB'%s', LOCATION %Ld, SIZE %Ld)", objectUID, lobHandle, (Int64)extractData, &extractLen);
        else
  ...
  )
{
    ...
    int lobV2 = 0;
    Int64 objectUID = 0;
    SQL_EXEC_ExtractFieldsFromLobHandleStr(lobHandle, length,
                                           &lobV2, &objectUID, NULL, NULL);
    ...
    if (lobV2) {
        ...
        if ((QryLobUpdateSrvrStmt = pSrvrStmt->getLobUpdateStmt(objectUID,stmtHandleKey, exception_)) == NULL) {
            return;
        }

        int len = length;
        QryLobUpdateSrvrStmt->setChunkBuffer(data, len);

        rc = QryLobUpdateSrvrStmt->updateLob(lobHandle, data, len, offset, exception_);
        if (rc != SQL_SUCCESS && rc != SQL_SUCCESS_WITH_INFO)
            pSrvrStmt->closeUpdateLob();
        return;
    }
...
{
   tenantName = (NULL == tenantName) ? "ESGYNDB" : tenantName;
   size_t msgHeaderLen = 100; // size of message text
   msgHeaderLen += (msgHeaderLen * 4) + TIMEBUFSIZE; // why multiply by 4?
   char buf [msgHeaderLen];
   ...
   // create message text
   int32_t stmtSize  = 0;
   if (errorDetail == UA_STATUS_ERR_INVALID)
     stmtSize = snprintf(buf, sizeof(buf),
                         "*** ERROR[8837] Invalid username or password.  User: %s %s",
                         externalUsername, strNow);
   else if (errorDetail == UA_STATUS_ERR_REGISTER)
     stmtSize = snprintf(buf, sizeof(buf),
                         "*** ERROR[8837] User %s is not defined in the metadata. %s",
                         externalUsername, strNow);
   else if (errorDetail == UA_STATUS_ERR_TENANT)
     stmtSize = snprintf(buf, sizeof(buf),
                         "*** ERROR[8837] User %s is not authorized for tenant %s.  %s",
                         externalUsername, tenantName, strNow);
   else if (errorDetail == UA_STATUS_ERR_REGISTER_FAILED)
      stmtSize = snprintf(buf, sizeof(buf),
                          "*** ERROR[8837] Auto registration of user %s failed with SQL error %d.",
                          externalUsername, error);
   else if (errorDetail == UA_STATUS_ERR_MANUALLY_REGISTER)
      stmtSize = snprintf(buf, sizeof(buf),
                          "*** ERROR[8837] Auto registration of user %s failed, %s is defined in "
                          "multiple AD/LDAP configurations.  Please manually register the user. %s",
                          externalUsername, externalUsername, strNow);
   else if (errorDetail == UA_STATUS_ERR_SETAUTHID)
      stmtSize = snprintf(buf, sizeof(buf),
                          "*** ERROR[8837] Internal error occurred.  SQL Error %d returned while caching credentials for user %s. %s",
                          error, externalUsername, strNow);
   else if (errorDetail == UA_STATUS_ERROR_BAD_AUTH)
      stmtSize = snprintf(buf, sizeof(buf),
                          "*** ERROR[8837] User (%s) or Tenant (%s) is invalid. %s ",
                          externalUsername, tenantName, strNow);
   else if (errorDetail == UA_STATUS_ERROR_SYSTEM_TENANT)
      stmtSize = snprintf(buf, sizeof(buf),
                          "*** ERROR[8837] Tenant %s is reserved for administrators. Please "
                          "retry the connection and specify a valid tenant name. %s",
                          tenantName, strNow);
   else if (errorDetail == UA_STATUS_PASSWORD_WILL_EXPIRE)
       stmtSize = snprintf(buf, sizeof(buf),
                           "*** WARN[8857] User password will expire in %d days.  User: %s %s",
                           error, externalUsername, strNow);
   else if (errorDetail == UA_STATUS_PASSWORD_EXPIRE)
       stmtSize = snprintf(buf, sizeof(buf),
                           "*** ERROR[8837] User password has expired.  User: %s %s",
                           externalUsername, strNow);
   else if (errorDetail == UA_STATUS_PASSWORD_ERROR_COUNT)
       stmtSize = snprintf(buf, sizeof(buf),
                           "*** ERROR[8837] User password input error times exceed maximum error times, must change the password!  User: %s %s",
                           externalUsername, strNow);
   else if (errorDetail == UA_STATUS_PASSWORD_GRACE_COUNT)
       stmtSize = snprintf(buf, sizeof(buf),
                           "*** WARN[8837] User password has expired. User can still login %d times.  User: %s %s",
                           error,externalUsername, strNow);
   else if (errorDetail == UA_STATUS_NOT_LICENSE)
       stmtSize = snprintf(buf, sizeof(buf),
                           "*** ERROR[8837] The module LOCAL-AUTH is not supported in this database version.");
   else
     stmtSize = snprintf(buf, sizeof(buf),
                         "*** ERROR[8837] Internal error occurred. User: %s %s",
                         externalUsername, strNow);

   char *message = new char[stmtSize + 1];
   strcpy(message,buf);

   if (errorDetail == UA_STATUS_PASSWORD_WILL_EXPIRE)
   {
       kdsCopySQLErrorExceptionAndRowCount(SQLError, message, 8857, "    ", -1);
   }
   else if (errorDetail == UA_STATUS_PASSWORD_GRACE_COUNT)
   {
