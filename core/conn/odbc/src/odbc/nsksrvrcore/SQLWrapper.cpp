...
Int32 SRVR::WSQL_EXEC_CancelOperation (Int64 transid)
{
        SRVRTRACE_ENTER(WSQL_Cancel);
        INIT
        retcode = SQL_EXEC_CancelOperation (transid)
        SRVRTRACE_EXIT(WSQL_Cancel);
        RETURN
}
...
Int32 SRVR::WSQL_EXEC_SetAuthID2(
   const USERS_INFO & userDesc,
   const char * authToken,
   Int32        authTokenLen,
   const char * slaName,
...
{
  SRVRTRACE_ENTER(WSQL_SetAuthID2);
  INIT
  retcode = SQL_EXEC_SetAuthID2(
                           userDesc,
                           authToken,
                           authTokenLen,
                           slaName,
                           profileName,
                           resetAttributes);
  SRVRTRACE_EXIT(WSQL_SetAuthID2);
  RETURN
}
...
Int32 SRVR::WSQL_EXEC_SetClientInfo(
        /*IN*/ const char *value,
        /*IN*/ int length,
        /*IN OPTIONAL*/ const char *characterset)
{
    SRVRTRACE_ENTER(WSQL_SetClientInfo);
    INIT
    retcode = SQL_EXEC_SetClientInfo(
        value,
        length,
        characterset);
    RETURN
...
Int32 SRVR::WSQL_EXEC_GetTransactionId(
        /*OUT*/ Int64* trans_id)
{
    SRVRTRACE_ENTER(WSQL_GetTransactionId);
    INIT
    retcode = SQL_EXEC_GetTransactionId(trans_id);
    SRVRTRACE_EXIT(WSQL_GetTransactionId);
    RETURN
}

Int32 SRVR::WSQL_EXEC_GetStatementHeapSize(
          /*IN*/ SQLSTMT_ID *statement_id,
          /*OUT*/ Int64* heapSize)
{
    SRVRTRACE_ENTER(WSQL_GetStatementHeapSize);
    INIT
    retcode = SQL_EXEC_GetStatementHeapSize(statement_id, heapSize);
    SRVRTRACE_EXIT(WSQL_GetStatementHeapSize);
    RETURN
}
