...
void SRVR::kdsCreateSQLDescSeq(SQLItemDescList_def *SQLDesc, short numEntries)
{
	...
	{
		markNewOperator, SQLDesc->_buffer = STMT_ALLOCATE(SQLItemDesc_def[numEntries]);
				  ...
				  int LobChunkMaxLen)
{
	...
	SQLItemDesc->lobInlineMaxLen = LobInlineMaxLen;
	SQLItemDesc->lobChunkMaxLen = LobChunkMaxLen;
	SQLDesc->_length++;
