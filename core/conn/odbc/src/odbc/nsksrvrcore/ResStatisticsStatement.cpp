...
string getQuerySubStateStringRes(unsigned short state)
{
	...
	case QUERY_COMPLETED_BY_CLIENT:			return FMT_NA;
...
//	if ((srvrGlobal->resourceStatistics & STMTSTAT_EXECUTE) && (inState == STMTSTAT_EXECUTE))
	if (inState == STMTSTAT_EXECUTE)
	{
		...
		if (stmtType == TYPE_INSERT || stmtType == TYPE_INSERT_PARAM || stmtType == TYPE_DELETE || stmtType == TYPE_UPDATE || inSqlQueryType == SQL_SELECT_UNIQUE || inSqlQueryType == SQL_OTHER)
		{
#ifdef RES_STATS_EVENT
			stringstream ssEvent;
...
#ifdef RES_STATS_EVENT
			stringstream ssEvent;
			ssEvent << "END:SQLExecute:in STMTSTAT_EXECUTE:: StatementId: " << statementId
					<< ", QueryId: " << queryId
					<< ", StatementType: " << getStatementType(inSqlNewQueryType)
					<< ", ClientId: " << resCollectinfo.clientId
					<< ", UserName: " << resCollectinfo.userName
...
void ResStatisticsStatement::SendQueryStats(bool bStart, SRVR_STMT_HDL *pSrvrStmt, char *inSqlError, Int32 inSqlErrorLength)
{
	...
	*pQuery_info = {0};
	...
	pQuery_info->m_instance_id = srvrGlobal->traf_instance_id;
	pQuery_info->m_tenant_id = srvrGlobal->tenantID;
	...
	if(pSrvrStmt->sqlString!=NULL) 
	{
		...
		UpdateStringText(pQuery_info->m_query_text);
                if (pQuery_info->m_query_text.length() > 50000)//max size for column QUERY_TEXT is 50000
                {
                  pQuery_info->m_query_text = pQuery_info->m_query_text.substr(0, 50000);
                  pQuery_info->m_query_text.replace(50000 - 7, 7, "......;");
                }
	}
	...
	pQuery_info->m_udr_process_busy_time = UdrCpuTime;
