...
public class ScriptsInterface
{
		      ...
		      String dsnName) throws IOException, SQLException, InstantiationException, IllegalAccessException, ClassNotFoundException
  {
   ...
   {
      String tenantName = "";
      openConnection( userName, password, roleName, serverName, portNumber, dsnName, tenantName, 0);
   }
   ...
   //     System.exit(0); 
	...
	public void interrupt() {
		if (sessObj != null) {
			...
			try {
				Statement stmt = (Statement)sessObj.getCurrentStmtObj();
				if(stmt != null)
					stmt.cancel();
			} catch (SQLException e) {
				System.out.println("Failed to cancel active statement : " + e.getMessage());
			}
		}
	}
