...
//   private boolean doTrace = Boolean.getBoolean("trafci.enableTrace");
   ...
   public Session createSession(Session session,
      ...
      if (!noConnectOption)
      {
         ...
         sessObj.setUnsignedCQDs();
         sessObj.setBooleanCQDs();
         sessObj.setLobHandleCQD();
         sessObj.setTrafciCQD();
         //sessObj.setTinyintCQDs();
        ...
        //store the previous query for history command
         ...
         try
         {
             ...
             if (!this.queryMode && !curQryConditionalAction)
             {
               if (!OS_EOF)
               {
            	   if(!sessObj.isSessionStartup() && caller == USERI ) {
            ...
            */
            ...
            if (qryLine != null) {
               if(sessObj.isSessionIgnoreComments()) {
                   qryObj.setQueryText(parser.ignoreComments(qryLine));
               }
               else {
                   qryObj.setQueryText(qryLine);
               }
   ...
   public Connection getConnection() throws SQLException, InstantiationException, IllegalAccessException, ClassNotFoundException
   {
      ...
      connProp.put("password",sessObj.getSessionPass() == null? "" : sessObj.getSessionPass());
      connProp.put("tenantName", sessObj.getSessionTenant());
      /*
      ...
      if (sessObj.isDebugOn())
      {
    	  System.out.println("getConnection::" +
    			  ...
    			  "::USR:" + sessObj.getSessionUser() +
     			  "::TENANT:"+ sessObj.getSessionTenant() + 
   ...
   //  get the connection to the database
   public Connection getConnection(String user, String password, String role, String server, String dataSource, String port, String tenantName) 
      throws SQLException, InstantiationException, IllegalAccessException, ClassNotFoundException
   {
      ...
      connProp.put("tenantName", (tenantName==null?sessObj.getSessionTenant():tenantName));
      /*
       *  Only set the connection timeout if its not the default.
       */
      int connTimeout = Session.getConnectionTimeout();
