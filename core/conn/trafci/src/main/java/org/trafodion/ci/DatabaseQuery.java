...
public class DatabaseQuery extends QueryWrapper
{
   ...
   public void execute() throws IOException, SQLException, UnKnownInterfaceCommand, UserInterruption
   {
	   try{
           ...
           if (details == null)
             {
               // the exception doesn't have a message (which makes it useless to us),
               // so let's include the exception class name and the top frame of the stack
               StackTraceElement[] stackTrace = ex.getStackTrace();
               details = ex.toString() + " exception thrown at " + stackTrace[0].toString();
             }
           ErrorObject internalError = new ErrorObject(SessionError.INTERNAL_ERR.errorCode(),
                   SessionError.INTERNAL_ERR.errorMessage() + " Details=" + details);
      ...
      */
      ...
      switch (qryObj.getQueryId())
      {
         ...
         default:
            ...
            //For commands beginning with SHOW (SHOWTRANSACTION,SHOWSET,DESCRIBE)
            //set the trimout flag to true

            if(qryObj.getQueryText().matches("(?is)DESCRIBE.+")){
...
//      PreparedStatement pStmt=null;
   ...
   /**
    * trafci describe. describe statment xx;
    */
   private void describeQuery() throws UnKnownInterfaceCommand, IOException {
      parser.getNextKeyToken();
      String secondToken = parser.getNextKeyToken();
      if (secondToken == null || !(secondToken.equalsIgnoreCase("statement"))) {
         writeln(ise.getSyntaxError(this.queryStr, parser.getRemainderStr()).errorMessage());
         throw uic;
      }
      String[] errParams = new String[1];
      String stmtName = parser.getNextKeyToken();
      if (stmtName == null || parser.getNextKeyToken() != null) {
         writeln(ise.getSyntaxError(this.queryStr, parser.getRemainderStr()).errorMessage());
         throw uic;
      }
      PreparedStatement pStmt = sessObj.getPrepStmtMap(stmtName.toUpperCase());
      if (pStmt == null) {
         errParams[0] = stmtName;
         writer.writeError(sessObj, 'D', SessionError.DESCRIBE_PSTMT_IS_EMPTY, errParams);
         throw uic;
      }
      writeDescribe(pStmt, errParams, stmtName);
   }

   private void writeDescribe(PreparedStatement pStmt, String[] errParams, String stmtName)
           throws IOException, UnKnownInterfaceCommand {
      TrafT4PreparedStatement preparedStatement = (TrafT4PreparedStatement) pStmt;
      //messagesType input 1  output 2
      List<String> describeMessagesIut = preparedStatement.getDescribeMessages(1);
      List<String> describeMessagesOut = preparedStatement.getDescribeMessages(2);
      if (describeMessagesIut == null && describeMessagesOut == null) {
         errParams[0] = stmtName;
         writer.writeError(sessObj, 'D', SessionError.DESCRIBE_MESSAGES_NOT_FOUND, errParams);
         throw uic;
      }

      StringBuilder stringBuilder = new StringBuilder("---Describing the INPUT entries---");
      stringBuilder.append("\n");
      if (describeMessagesIut != null && describeMessagesIut.size() != 0) {
         for (String str : describeMessagesIut) {
            stringBuilder.append(str);
         }
         stringBuilder.append("\n");
      }
      if (describeMessagesOut != null && describeMessagesOut.size() != 0) {
         stringBuilder.append("---Describing the OUTPUT entries---");
         stringBuilder.append("\n");
         for (String str : describeMessagesOut) {
            stringBuilder.append(str);
         }
         stringBuilder.append("\n");
      }
...
//            	   value=paramList.get(i).toString();
               ...
               try
               {
                  if (value!=null)
                  {
                     ...
                     {
                        if ((value.startsWith("N'") || value.startsWith("n'")) && value
                                .endsWith("'")) {
   ...
   private void execSet() throws SQLException, IOException, UnKnownInterfaceCommand, UserInterruption
   {
      ...
      else
      {
         ...
         switch (setId)
         {
            ...
            case SessionDefaults.SET_TRAFDES:
                List<String> deList=parser.getCSTList(parser.getRemainderStr());
                String deTmp = deList.get(0).toString().toUpperCase();
                if (deTmp.equals("OFF")) {
                    sessObj.setTrafDetails(false);
                }else if (deTmp.equals("ON")){
                    sessObj.setTrafDetails(true);
                }
                break;
            case SessionDefaults.SET_CLIENTINFO:
                String clientInfoValue = parser.getRemainderStr();
                int pos = clientInfoValue.indexOf(":");
                String key = null;
                String value = null;
                if(pos == -1){
                    //default
                    ...
                    key = clientInfoValue.substring(0,pos).trim();
                    value = clientInfoValue.substring(pos + 1, clientInfoValue.length());
                }
                conn.setClientInfo(key, value);
                sessObj.setClientInfoProp(key, value);
                break;
            default:
               executeQuery(true);
               break;
   ...
   private String writeData(ResultSet rs,int numColumns) throws SQLException, IOException
   {
      ...
      for (int i=1;i<=numColumns ;i++)
      {
         ...
         {
          	 value = (value.trim()+"000000000").substring(0,rsmd.getColumnDisplaySize(i));
         }
   ...
   private void writeOutParams(ResultSetMetaData rsmd,int numColumns,ParameterMetaData paramMetaData,CallableStatement callStmt) throws SQLException, IOException
   {
      ...
      writeData(callStmt,numColumns,paramMetaData,rsmd);
      qryObj.setRowCount("1");
   }

   private void writeData(CallableStatement callStmt,int numColumns, ParameterMetaData paramMetaData,ResultSetMetaData rsmd) throws SQLException, IOException
   {
      for (int i=1;i<=numColumns ;i++)
      {
         if (paramMetaData.getParameterMode(i) != ParameterMetaData.parameterModeIn &&
            paramMetaData.getParameterMode(i) != ParameterMetaData.parameterModeUnknown)
         {
            ...
            if (value != null && !value.equalsIgnoreCase("NULL")
                    && rsmd.getColumnType(i) == Types.TIMESTAMP && value.length() < rsmd
   ...
   private void loadDbKeyWordMap()
   {
      ...
      dbKeyMap.put("SET_TRAFDES",""+SessionDefaults.SET_TRAFDES);
      dbKeyMap.put("SET_CLIENTINFO",""+SessionDefaults.SET_CLIENTINFO);
   }
