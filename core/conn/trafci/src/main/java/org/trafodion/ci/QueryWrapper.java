...
public abstract class QueryWrapper
{
   ...
   */
   void readQuery() throws IOException, UserInterruption
   {
      ...
      matchSpsql = isMatchSpsql(qryObj.getQueryText());
      //This is needed if user does a repeat
      //on create trigger
      if ((matchSpsql) && (queryStr.indexOf(SessionDefaults.lineSeperator) != -1))
      {
         qryLineArr = queryStr.split(SessionDefaults.lineSeperator);
         if (qryLineArr[qryLineArr.length-1].equals(crSpsqlTerminator))
         {
            isMultiLine = false;
         }
      }
      ...
      while (isMultiLine)
      {
         if ((matchSpsql) && utils.rtrim(queryStr).equals(crSpsqlTerminator))
            break;
         else if ((!matchSpsql) && (queryStr.trim().toUpperCase().endsWith(sessObj.getSessionSQLTerminator()))) {
         ...
         if (sessObj.isSessionIgnoreComments() && !matchSpsql) {
      ...
      if (matchSpsql)
          this.queryStr = queryParser.modifySql(this.queryStr);
   ...
   boolean isMatchSpsql(String qryText) throws IOException
   {
       return queryParser.match(qryText);
   }
   ...
   */
   ...
   void formatOutput(String output,int colWidth,char fillchar,int align) throws IOException
   {
     ...
     int outputStrLen = 0;
      ...
      {
         if(output.length()-colWidth == 1 && output.startsWith("-0.")){
            colWidth++;
         }
