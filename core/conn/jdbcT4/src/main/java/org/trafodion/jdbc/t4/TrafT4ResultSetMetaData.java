...
public class TrafT4ResultSetMetaData implements java.sql.ResultSetMetaData {
    private final static org.slf4j.Logger LOG =
            LoggerFactory.getLogger(TrafT4ResultSetMetaData.class);

	// begin required methods
	public String getCatalogName(int column) throws SQLException {
		if (column > outputDesc_.length) {
			throw TrafT4Messages.createSQLException(connection_.getT4props(), "invalid_desc_index");
		}
	...
	TrafT4ResultSetMetaData(TrafT4Statement stmt, TrafT4Desc[] outputDesc) {
		if (stmt.getT4props().isLogEnable(Level.FINEST)) {
            T4LoggingUtilities.log(stmt.getT4props(), Level.FINEST, "ENTRY", stmt, outputDesc);
        }
		if (LOG.isTraceEnabled()) {
            LOG.trace("ENTRY. {}, {}", stmt, outputDesc);
        }
	...
	TrafT4ResultSetMetaData(TrafT4ResultSet resultSet, TrafT4Desc[] outputDesc) {
		if (resultSet.getT4props().isLogEnable(Level.FINEST)) {
            T4LoggingUtilities.log(resultSet.getT4props(), Level.FINEST, "ENTRY", resultSet);
        }
		if (LOG.isTraceEnabled()) {
            LOG.trace("ENTRY. {}", resultSet);
        }

		resultSet_ = resultSet;
		connection_ = resultSet.connection_;
		outputDesc_ = outputDesc;
