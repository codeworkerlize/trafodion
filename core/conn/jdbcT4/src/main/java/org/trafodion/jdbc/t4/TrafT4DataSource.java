...

{
	 ...
	 */
    private final static org.slf4j.Logger LOG = LoggerFactory.getLogger(TrafT4DataSource.class);
	...
	synchronized public Connection getConnection() throws SQLException {
		...
		// Table Cache
		try {
			if (t4Conn.isUseCacheTable() && getT4Properties().getCacheTable() != null) {
				TrafCachePool.getTrafCachePool().initH2DataBase(t4Conn, getT4Properties());
			}
		} catch (Exception e) {
			if (this.isLogEnable(Level.WARNING)) {
				T4LoggingUtilities.log(this, Level.FINEST, "ENTRY . " + e.getMessage(), e);
			}
			if (LOG.isWarnEnabled()) {
				LOG.warn("ENTRY . {}", e.getMessage(), e);
			}
			throw TrafT4Messages.createSQLException(null, "cacheTable_error", e.getMessage(), e);
		}

		conn = t4Conn;

        if (this.isLogEnable(Level.FINEST)) {
            T4LoggingUtilities.log(this, Level.FINEST, "LEAVE");
        }
        if (LOG.isTraceEnabled()) {
            LOG.trace("LEAVE");
        }
	...
	synchronized public Connection getConnection(String username, String password) throws SQLException {
        if (this.isLogEnable(Level.FINEST)) {
            T4LoggingUtilities.log(this, Level.FINEST, "ENTRY", username);
        }
        if (LOG.isTraceEnabled()) {
            LOG.trace("ENTRY. {}", username);
        }
	...
	public void setLogWriter(PrintWriter out) throws SQLException {
        ...
        if (this.isLogEnable(Level.FINEST)) {
            T4LoggingUtilities.log(this, Level.FINEST, "ENTRY", out);
        }
        if (LOG.isTraceEnabled()) {
            LOG.trace("ENTRY. {}", out);
        }
	...
	void setPoolManager(Context nameCtx, String dataSourceName) throws Exception {
        if (this.isLogEnable(Level.FINEST)) {
            T4LoggingUtilities.log(this, Level.FINEST, "ENTRY", nameCtx, dataSourceName);
        }
        if (LOG.isTraceEnabled()) {
            LOG.trace("ENTRY. {}, {}", nameCtx, dataSourceName);
        }
		Object pds;
	...
	public TrafT4DataSource(Properties info) {
        ...
        if (this.isLogEnable(Level.FINEST)) {
            T4LoggingUtilities.log(this, Level.FINE, "ENTRY");
        }
        if (LOG.isTraceEnabled()) {
            LOG.trace("ENTRY");
        }
