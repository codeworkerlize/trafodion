...
class ConnectReply {
	...
	ConnectReply(LogicalByteArray buf, InterfaceConnection ic) throws SQLException, UnsupportedCharsetException,
		...
		if (m_p1_exception.exception_nr == TRANSPORT.CEE_SUCCESS) {
            ...
            if((m_p6_versionList.list[0].buildId & InterfaceConnection.PASSWORD_SECURITY) > 0) {
				...
				if((m_p6_versionList.list[0].buildId & InterfaceConnection.PASSWORD_SECURITY_BASE64) > 0)
					securityEnabledBase64 = true;
				else
					securityEnabledBase64 = false;
	...
	// -------------------------------------------------------------
	void fixupSrvrObjRef(InterfaceConnection ic, String name) throws SQLException {
		//
		...
		if (m_p2_srvrObjRef != null) {
			...
			try {
				m_ncsAddr_ = new NCSAddress(ic.getT4props(), m_p2_srvrObjRef);
