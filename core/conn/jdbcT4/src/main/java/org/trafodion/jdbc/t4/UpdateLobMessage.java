	...
	static LogicalByteArray marshal(short type, String lobHandle, long totalLength, long offset, byte[] data, long pos,
		...
		try {
			...
			if (lobHandle.length() > 0) {
				wlength += TRANSPORT.size_bytesWithCharset(lobHandleByets); // 149
			}
			...
			buf = new LogicalByteArray(wlength, Header.sizeOf(), ic.getByteSwap()); // 40

			buf.insertInt(type); // 44
			buf.insertStringWithCharset(lobHandleByets, InterfaceUtilities.SQLCHARSETCODE_ISO88591); // 149
