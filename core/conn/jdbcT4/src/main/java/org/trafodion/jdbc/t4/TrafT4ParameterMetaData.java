...
public class TrafT4ParameterMetaData implements java.sql.ParameterMetaData {
    ...
    private final static org.slf4j.Logger LOG =
            LoggerFactory.getLogger(TrafT4ParameterMetaData.class);
    
	public String getParameterClassName(int param) throws SQLException {
		if (param > inputDesc.length) {
			throw TrafT4Messages.createSQLException(props, "invalid_desc_index");
		}
	...
	TrafT4ParameterMetaData(TrafT4PreparedStatement stmt, TrafT4Desc[] inputDesc) {
		this.props = stmt.connection_.getT4props();
		this.inputDesc = inputDesc;

        if (props.isLogEnable(Level.FINEST)) {
            T4LoggingUtilities.log(props, Level.FINEST, "ENTRY", stmt, inputDesc);
        }
        if (LOG.isTraceEnabled()) {
            LOG.trace("ENTRY. {}, {}", stmt, inputDesc);
        }
	}
