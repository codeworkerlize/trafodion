...
public class T4Properties {
    final static int MAX_PSTMT = Short.MAX_VALUE;
	...
	private float releaseStatementsPercentage;
	...
	//clipVarchar flag
	private int clipVarchar_ = 0;
    // mds flag 
	private short mds_ = 0;
	// Default catalog 
	
	private short sessionDebug_ = 0;
	// sessionDebug_ flag
	static final String DEFAULT_CATALOG = "TRAFODION";

	// propertiy queryTimeout_ for future use.
	private int ddlTimeout_ = 1 * 60 * 30;
	...
	private boolean _tokenAuth;
        private short _txnIsolationLevel = (short) InterfaceConnection.SQL_TXN_READ_COMMITTED;

	static String cachedUrl;
	private static int defaultPort = 23400;
	private List<String> _urlList = new ArrayList<String>();
	...
	private String sslTrustManagerProvider = null;
	private String sslTrustManagerAlgorithm = "SunX509";
	private String sslContextProtoco = "TLS";
	private String socketFactoryClass = null;
	private String jksPwd = null;
	private String jksPath = null;
	private boolean isSSLEnable = false;
	private String ipMapping = null;

    private String clientCharset;
    private String specifiedServer = null;
    private boolean batchSkipProblemRow = true;
    private boolean ignoreCancel = false;
    
    private String remoteProcess;

    private boolean lobCopySrc = true;
	private String cacheTable;
	private int keepAliveTime;

    private long linkThreshold;
    private int traceTransTime;
    private long stmtMaxUsedSize;

    private long dcsConnectTimeThreshold = 50;  //ms
    private long mxoConnectTimeThreshold =  500; //ms

	private static String[] addressArr = null;
	private static Map<String,String> addressMap;
	private int maxCachedRows = 0;
	private int cacheRefreshIntervalSecs = -1;
	private String t4Url;
	private int cancelTimeOut;
	private static final int CANCEL_TIMEOUT_DEFAULT = 10;
	private static final long DEFAULT_STMT_MAX_USED_SIZE = 200L;

	public boolean isLogEnable(Level level) {
		boolean logWrite = false;
		try {
			logWrite = this.getLogWriter() != null;
		} catch (SQLException e) {
		}

		return this.t4Logger_.isLoggable(level) || logWrite;
	}
	...
	void initializeLogging() {
		...
		try {
			...
			SQLException se = TrafT4Messages.createSQLException(null, "problem_with_logging", e.getMessage());
			sqlExceptionMessage_ = se.getMessage();
	...
	private String getProperty(String token) {
		...
		if (inprops_ != null) {
			Object re = inprops_.get(token);
			ret = re==null ? null : re.toString();
		}
		// props file next
		if (ret == null && defaults_ != null) {
			Object re = defaults_.get(token);
			ret = re==null ? null : re.toString();
		}
	...
	private void setProperties() {
		...
		if (propsFile != null) {
			...
			if (propsFile.length() != 0) {
				...
				try {
					...
					fis = null;
					sqlExceptionMessage_ =
							"Error while loading " + propPrefix_ + "properties file: " + ex.getMessage();
				} finally {
					try {
						if (fis != null) {
							fis.close();
		...
		setT4Url(getProperty("url"));
		setUrlList(getProperty("url"));
		setDefaultPort(getProperty("defaultPort"));
		...
		setReleaseStatementsPercentage(getProperty("releaseStatementsPercentage"));
		...
		setMds(getProperty("mds"));
		setSessionDebug(getProperty("sessionDebug"));
		setFetchBufferSize(getProperty("fetchBufferSize"));
		setQueryTimeout(getProperty("queryTimeout"));
		setDDLTimeout(getProperty("ddlTimeout"));
		setIgnoreCancel(getProperty("ignoreCancel"));
		...
		setTenantName(getProperty("tenantName"));
		...
		setSSLEnable(getProperty("isSSLEnable"));
		setJKSPath(getProperty("SSLJKSPath"));
		setJKSPwd(getProperty("JKSPwd"));
		setSocketFactoryClass(getProperty("socketFactoryClass"));
		setTrustAllCerts(getProperty("isSSLTrustAll"));
		setSSLContextProtoco(getProperty("SSLContextProtoco"));
		setTrustManagerAlgorithm(getProperty("TrustManagerAlgorithm"));
		setTrustManagerProvider(getProperty("TrustManagerProvider"));
		setKeystoreType(getProperty("KeystoreType"));
		...
		setIpMapping(getProperty("ipMapping"));
		setSpecifiedServer(getProperty("specifiedServer"));
		setBatchSkipProblemRow(getProperty("batchSkipProblemRow"));
		setIgnoreCancel(getProperty("ignoreCancel"));
		setLobCopySrc(getProperty("lobCopySrc"));
		setLinkThreshold(getProperty("linkThreshold"));
		setDcsConnectTimeThreshold(getProperty("dcsConnectTimeThreshold"));
		setMxoConnectTimeThreshold(getProperty("mxoConnectTimeThreshold"));
		setTxnIsolationLevel(getProperty("txnIsolationLevel"));
		setCacheTable(getProperty("cacheTable"));
		setMaxCachedRows(getProperty("maxCachedRows"));
		setCacheRefreshIntervalSecs(getProperty("cacheRefreshIntervalSecs"));
		setTraceTransTime(getProperty("traceTransTime"));
        setStmtMaxUsedSize(getProperty("stmtMaxUsedSize"));
		setCancelTimeOut(getProperty("cancelTimeOut"));
		setKeepAliveTime(getProperty("keepAliveTime"));
	}

	private void setSSLEnable(String isSSLEnable) {
		if (isSSLEnable != null) {
			this.isSSLEnable = "true".equalsIgnoreCase(isSSLEnable.trim());
		}
	}

	private void setKeystoreType(String sslKeystoreType) {
		if(sslKeystoreType != null){
			this.sslKeystoreType = sslKeystoreType;
		}
	}

	private void setTrustManagerProvider(String sslTrustManagerProvider) {
		if(sslTrustManagerProvider != null){
			this.sslTrustManagerProvider = sslTrustManagerProvider;
		}
	}

	private void setTrustManagerAlgorithm(String sslTrustManagerAlgorithm) {
		if(sslTrustManagerAlgorithm != null){
			this.sslTrustManagerAlgorithm = sslTrustManagerAlgorithm;
		}
	}

	private void setSSLContextProtoco(String sslContextProtoco) {
		if(sslContextProtoco != null){
			this.sslContextProtoco = sslContextProtoco;
		}
	}

	private void setSocketFactoryClass(String socketFactoryClass) {
		if(socketFactoryClass != null){
			this.socketFactoryClass = socketFactoryClass;
		}
	}

	private void setJKSPwd(String jksPwd) {
		if(jksPwd != null){
			this.jksPwd = jksPwd;
		}
	}

	private void setJKSPath(String jksPath) {
		if(jksPath != null){
			this.jksPath = jksPath;
		}
	}

	public void setTrustAllCerts(String isTrustAllCerts) {
		if (isTrustAllCerts != null) {
			this.isTrustAllCerts = "true".equalsIgnoreCase(isTrustAllCerts.trim());}
	}
	...
	public String getSocketFactoryClasss() {
	...
	public String getSSLContextProtoco() {
		return this.sslContextProtoco;
	}
	...
	public Properties getProperties() {
		...
		props.setProperty("releaseStatementsPercentage", String.valueOf(releaseStatementsPercentage));
		...
		props.put("isSSLEnable",String.valueOf(isSSLEnable));
		props.put("isSSLTrustAll",String.valueOf(isTrustAllCerts));
		...
		props.setProperty("ddlTimeout", String.valueOf(ddlTimeout_));
		...
		if (_tenantName != null)
		    props.setProperty("tenantName", _tenantName);
        ...
        props.setProperty("mds", String.valueOf(mds_));
        props.setProperty("sessionDebug", String.valueOf(sessionDebug_));
		...
		props.setProperty("ipMapping", String.valueOf(ipMapping));
        if (clientCharset != null)
            props.setProperty("clientCharset", clientCharset);
        if (specifiedServer != null) {
            props.setProperty("specifiedServer", specifiedServer);
        }
        props.setProperty("batchSkipProblemRow", String.valueOf(batchSkipProblemRow));
        props.setProperty("ignoreCancel", String.valueOf(this.ignoreCancel));

        props.setProperty("lobCopySrc", String.valueOf(lobCopySrc));
        props.setProperty("linkThreshold", String.valueOf(linkThreshold));
        props.setProperty("dcsConnectTimeThreshold", String.valueOf(dcsConnectTimeThreshold));
        props.setProperty("mxoConnectTimeThreshold", String.valueOf(mxoConnectTimeThreshold));

		if (cacheTable != null) {
			props.setProperty("cacheTable", String.valueOf(cacheTable));
		}
		props.setProperty("keepAliveTime", String.valueOf(keepAliveTime));
		props.setProperty("maxCachedRows", String.valueOf(maxCachedRows));
		props.setProperty("cacheRefreshIntervalSecs", String.valueOf(cacheRefreshIntervalSecs));
		props.setProperty("traceTransTime", String.valueOf(traceTransTime));
		props.setProperty("stmtMaxUsedSize", String.valueOf(stmtMaxUsedSize));
		props.setProperty("cancelTimeOut", String.valueOf(cancelTimeOut));
		return props;
	}
	...
	void setMaxStatements(String maxStatements) {
		...
		if (maxStatements != null) {
			try {
				...
				sqlExceptionMessage_ = "maxStatements : " + maxStatements +"."+ ex.getMessage();
				maxstmt = 0;
			}
		}
    ...
    void setMaxStatements(int maxStatements) {
        if (maxStatements < 0) {
            sqlExceptionMessage_ = "maxStatements : " + maxStatements;
            maxStatements_ = 0;
        } else if (maxStatements_ > MAX_PSTMT) {
            sqlExceptionMessage_ = "maxStatements : " + maxStatements + ", maximum is: " + MAX_PSTMT;
            maxStatements_ = MAX_PSTMT;
        } else {
	...
	void setT4Url(String url){
		if(url != null){
			t4Url = url;
		}
	}
	public String getT4Url(){
		return t4Url;
	}

	List<String> getUrlList() {
		if (null != T4Properties.cachedUrl && !"".equals(T4Properties.cachedUrl) && url_.indexOf(T4Properties.cachedUrl) > -1) {
			sortUrlList();
		}
		return _urlList;
	}

	private void sortUrlList() {
		if (_urlList.remove(T4Properties.cachedUrl)) {
			_urlList.add(0, T4Properties.cachedUrl);
		}
	}

	private void setUrlList(String url) {
		if (url == null) {
			return;
		}
		...
		try {
			_urlList = T4Address.extractUrlListFromUrl(url_);
			setAddressArr(T4Address.getAddressArr());
			setAddressMap(T4Address.getAddressMap());
		} catch (SQLException e) {
			return;
		}
	}
	...
	Properties setURL(String url) throws SQLException {
		...
		//if (t4addr_ == null) {
			t4addr_ = new T4Address(this, url);
		//}
	 ...
	 * login timeout value is set to 2 minutes.
	 *
	 * @param loginTimeout
    ...
    void setLinkThreshold(String linkThreshold) {
	    long tmp = 0;
	    if (linkThreshold != null) {
            try {
                tmp = Long.parseLong(linkThreshold);
            } catch (NumberFormatException e) {
                sqlExceptionMessage_ = "Incorrect value for linkThreshold set: " + linkThreshold + ". "
                    + e.getMessage();
                tmp = 0;
            }
        }
        setLinkThreshold(tmp);
    }

    void setLinkThreshold(long linkThreshold) {
	    this.linkThreshold = linkThreshold;
    }
    public long getLinkThreshold() {
	    return this.linkThreshold;
    }

    private final static int KEEPALIVE_DEFAULT_TIME = 2 * 50;
    void setKeepAliveTime(String keepAliveTime){
		int time = KEEPALIVE_DEFAULT_TIME;
		if(keepAliveTime != null){
			try{
				time = Integer.parseInt(keepAliveTime);
				if (time < KEEPALIVE_DEFAULT_TIME){
					time = KEEPALIVE_DEFAULT_TIME;
				}
			} catch (NumberFormatException e){
				sqlExceptionMessage_ = "Incorrect value for traceTransTime set: " + keepAliveTime + ". "
						+ e.getMessage();
				time = KEEPALIVE_DEFAULT_TIME;
			}
		}
    ...
    void setTraceTransTime(String traceTransTime) {
	    int tmp = -1;
	    if (traceTransTime != null) {
            try {
                tmp = Integer.parseInt(traceTransTime);
            } catch (NumberFormatException e) {
                sqlExceptionMessage_ = "Incorrect value for traceTransTime set: " + traceTransTime + ". "
                    + e.getMessage();
                tmp = -1;
            }
        }
        setTraceTransTime(tmp);
    }

    void setTraceTransTime(int traceTransTime) {
	    this.traceTransTime = traceTransTime;
    }

    void setCancelTimeOut(String cancelTimeOut) {
	    int tmp = CANCEL_TIMEOUT_DEFAULT;
	    if (cancelTimeOut != null) {
		    try {
			    tmp = Integer.parseInt(cancelTimeOut);
			    if (tmp <= 0) {
				    tmp = CANCEL_TIMEOUT_DEFAULT;
			    }
		    } catch (NumberFormatException e) {
			    sqlExceptionMessage_ = "Incorrect value for cancelTimeOUt set: " + cancelTimeOut + ". "
					    + e.getMessage();
			    tmp = CANCEL_TIMEOUT_DEFAULT;
		    }
	    }
	    setCancelTimeOut(tmp);
    }
    void setCancelTimeOut(int cancelTimeOut) {
	...
	boolean isTraceTransTime() {
	    if (this.traceTransTime >= 0) {
	        return true;
        }else {
	        return false;
        }
    ...
    public int getTraceTransTime() {
        return traceTransTime;
    }

    void setStmtMaxUsedSize(String stmtMaxUsedSize){
        long tmp = DEFAULT_STMT_MAX_USED_SIZE;
        if (stmtMaxUsedSize != null) {
            try {
                tmp = Long.parseLong(stmtMaxUsedSize);
            } catch (NumberFormatException e) {
                sqlExceptionMessage_ = "Incorrect value for stmtMaxUsedSize set: " + stmtMaxUsedSize + ". "
                    + e.getMessage();
                tmp = DEFAULT_STMT_MAX_USED_SIZE;
            }
        }
        setStmtMaxUsedSize(tmp);
    }


    void setStmtMaxUsedSize(long stmtMaxUsedSize){
	    this.stmtMaxUsedSize = stmtMaxUsedSize;
    }

    public long getStmtMaxUsedSize(){
	    return this.stmtMaxUsedSize * 1024 * 1024;
    }
	...
	void setClipVarchar(String clipVarchar) {
		...
		if (clipVarchar != null) {
			try {
				tmp = Integer.parseInt(clipVarchar);
			} catch (NumberFormatException ex) {
	...
	void setMds(String mds) {
		short tmp = 0;
		if (mds != null) {
			try {
				tmp = Short.parseShort(mds);
			} catch (NumberFormatException ex) {
				sqlExceptionMessage_ = "Incorrect value for mds set: " + mds + ". "
						+ ex.getMessage();
				tmp = 0;
			}
		}
	...
	void setSessionDebug(String sessionDebug) {
		short tmp = 0;
		if (sessionDebug != null) {
			try {
				tmp = Short.parseShort(sessionDebug);
			} catch (NumberFormatException ex) {
				sqlExceptionMessage_ = "Incorrect value for sessionDebug set: " + sessionDebug + ". "
						+ ex.getMessage();
				tmp = 0;
			}
		}
	...
	void setClipVarchar(int clipVarchar) {
		if (clipVarchar < 0) {
			sqlExceptionMessage_ = "Incorrect value for clipVarchar set: " + clipVarchar
					+ ", and The result cannot be negative.";
		}
		clipVarchar_=clipVarchar;
	}

	void setMds(short mds) {
		mds_=mds;
	}

	void setSessionDebug(short sessionDebug) {
		sessionDebug_=sessionDebug;
	}
	/**
	...
	void setT4LogLevel(String level) {
		...
		if (level != null) {
			try {
				...
				SQLException se = TrafT4Messages.createSQLException(null, "problem_with_logging", ex.getMessage());
				sqlExceptionMessage_ = se.getMessage();
				// throw se;
	...
	void setDDLTimeout(String ddlTimeout) {
		Integer tmpDTimeOut = 1 * 60 * 30;
		if (ddlTimeout != null) {
			try {
				tmpDTimeOut = Integer.parseInt(ddlTimeout);
			} catch (NumberFormatException ex) {
				sqlExceptionMessage_ = "Incorrect value for ddlTimeout set: " + ddlTimeout + ex.getMessage();
				tmpDTimeOut = 0;
			}
		}
		setDDLTimeout(tmpDTimeOut);
	}

	void setDDLTimeout(Integer ddlTimeout) {
		ddlTimeout_ = ddlTimeout;
	}
	...
	public void setTenantName(String tenantName) {
		if (tenantName != null && tenantName.length() > 0) {
			if (tenantName.length() > 128)
				sqlExceptionMessage_ = "Invalid tenant name.  Max length: 128";
		}
	...
	/**
	 * Returns the default port when the ip in connection url does not have port
	 *
	 * @return the default port.
	 * @see #setDefaultPort(String defaultPort)
 	 */
	...
	/**
	 * Sets the default port when the ip in connection url does not have port
	 * 
	 * @param defaultPort
	 * Sets the port for the ips which don't have port. The value is
	 * between 0-65535
	 * @see #getDefaultPort()
	 */
	public static void setDefaultPort(String defaultPort) {
        int tmpPort = T4Properties.defaultPort;
        try {
            tmpPort = Integer.parseInt(defaultPort);
        } catch (NumberFormatException e) {
        }
        T4Properties.defaultPort = tmpPort;
	}
	...
	Reference addReferences(Reference ref) {
		...
		ref.add(new StringRefAddr("ddlTimeout", Integer.toString(getDDLTimeout())));
	...
	DriverPropertyInfo[] getPropertyInfo(String url, Properties defaults) throws SQLException {
		...
		DriverPropertyInfo[] propertyInfo = {
				...
				setPropertyInfo("inlineLobChunkSize", props, false, "Specifies the LOB chunk size that can be inlined along with row.",
						null),
				setPropertyInfo("lobChunkSize", props, false, "Specifies the LOB chunk size for streaming.",
						null),
    ...
    public void setIpMapping(String ipMapping){
	    this.ipMapping = ipMapping == null ? "" : ipMapping;
    }
    ...
    public boolean isBatchSkipProblemRow() {
        return batchSkipProblemRow;
    }

    public void setBatchSkipProblemRow(String batchSkipProblemRow) {
        if (batchSkipProblemRow == null || batchSkipProblemRow.length() == 0) {
            this.batchSkipProblemRow = true;
        } else {
            this.batchSkipProblemRow = Boolean.valueOf(batchSkipProblemRow);
        }
    }

    public float getReleaseStatementsPercentage() {
        return releaseStatementsPercentage;
    }

    public void setReleaseStatementsPercentage(float releaseStatementsPercentage) {
        if (releaseStatementsPercentage < 0) {
            sqlExceptionMessage_ = "releaseStatementsPercentage: "
                    + releaseStatementsPercentage + ", minimum is: " + 0;
            this.releaseStatementsPercentage = 0f;
        } else if (releaseStatementsPercentage > 100) {
            sqlExceptionMessage_ = "releaseStatementsPercentage: "
                    + releaseStatementsPercentage + ", maximum is: " + 100;
            this.releaseStatementsPercentage = 0f;
        } else {
            this.releaseStatementsPercentage = releaseStatementsPercentage;
        }
    }

    public void setReleaseStatementsPercentage(String releaseStatementsPercentage) {
        float percentage = 0f;
        if (releaseStatementsPercentage != null && releaseStatementsPercentage.length() > 0) {
            try {
                percentage = Float.parseFloat(releaseStatementsPercentage);
            } catch (NumberFormatException e) {
                sqlExceptionMessage_ = "releaseStatementsPercentage: "
                        + releaseStatementsPercentage + ". " + e.getMessage();
            }
        }
        setReleaseStatementsPercentage(percentage);
    }
    ...
    public boolean isLobCopySrc() {
        return lobCopySrc;
    }

    public void setLobCopySrc(String lobCopySrc) {
        if (lobCopySrc == null || "null".equalsIgnoreCase(lobCopySrc)) {
            return;
        }
        this.lobCopySrc = Boolean.getBoolean(lobCopySrc);
    }

    public void setLobCopySrc(boolean lobCopySrc) {
        this.lobCopySrc = lobCopySrc;
    }

    public void setDcsConnectTimeThreshold(String dcsConnectTimeThreshold) {
			if (dcsConnectTimeThreshold != null && dcsConnectTimeThreshold.length() > 0) {
				setDcsConnectTimeThreshold(Long.parseLong(dcsConnectTimeThreshold));
			}
		}
    public void setDcsConnectTimeThreshold(long connectTimeThreshold) {
			this.dcsConnectTimeThreshold = connectTimeThreshold;
		}

		public long getDcsConnectTimeThreshold() {
			return dcsConnectTimeThreshold;
		}

	public void setMxoConnectTimeThreshold(String mxoConnectTimeThreshold) {
		if (mxoConnectTimeThreshold != null && mxoConnectTimeThreshold.length() > 0) {
			setMxoConnectTimeThreshold(Long.parseLong(mxoConnectTimeThreshold));
		}
	}
	public void setMxoConnectTimeThreshold(long mxoConnectTimeThreshold) {
		this.mxoConnectTimeThreshold = mxoConnectTimeThreshold;
	}

	public long getMxoConnectTimeThreshold() {
		return mxoConnectTimeThreshold;
	}

	public static String[] getAddressArr() {
		return addressArr;
	}

	public static void setAddressArr(String[] addressArr) {
		T4Properties.addressArr = addressArr;
	}
	...
	public static void setAddressMap(Map<String, String> addressMap) {
		T4Properties.addressMap = addressMap;
	}

        void setTxnIsolationLevel(String txnIsolationLevel) {
            short tmp = (short) InterfaceConnection.SQL_TXN_READ_COMMITTED;
            if (txnIsolationLevel != null) {
                try {
                        tmp = Short.parseShort(txnIsolationLevel);
                    } catch (NumberFormatException ex) {
                        sqlExceptionMessage_ = "Incorrect value for txnIsolationLevel set: " + txnIsolationLevel + ". "
                            + ex.getMessage();
                        tmp = 0;
                }
            }
            setTxnIsolationLevel(tmp);
        }

        void setTxnIsolationLevel(short txnIsolationLevel) {
            _txnIsolationLevel = txnIsolationLevel;
        }

        short getTxnIsolationLevel() {
            return _txnIsolationLevel;
        }
	...
	public void setCacheTable(String cacheTable) {
		if (cacheTable == null || "".equals(cacheTable.trim())) {
	...
	public void setMaxCachedRows(String maxCachedRows) {
		this.maxCachedRows = 1000;
		if (maxCachedRows == null) {
			return;
		}
		try {
			this.maxCachedRows = Integer.parseInt(maxCachedRows);
		} catch (NumberFormatException nu) {
			sqlExceptionMessage_ = "setMaxCachedRows[" + maxCachedRows + "]" + ". " + nu.getMessage();
		}
	}

	public void setCacheRefreshIntervalSecs(String cacheRefreshIntervalSecs) {
		this.cacheRefreshIntervalSecs = -1;
		if (cacheRefreshIntervalSecs == null) {
			return;
		}
		try {
		this.cacheRefreshIntervalSecs = Integer.parseInt(cacheRefreshIntervalSecs);
		} catch (NumberFormatException nu) {
			sqlExceptionMessage_ = "setCacheRefreshIntervalSecs[" + cacheRefreshIntervalSecs + "]" + ". " + nu.getMessage();
		}
	}

	public int getMaxCachedRows() {
		return maxCachedRows;
	}

	public int getCacheRefreshIntervalSecs() {
		return cacheRefreshIntervalSecs;
	}
}
