...
public class T4Driver extends T4Properties implements java.sql.Driver {
	 ...
	 *         false.
	 */
    private final static org.slf4j.Logger LOG = LoggerFactory.getLogger(T4Driver.class);
    
	public boolean acceptsURL(String url) throws SQLException {
        if (this.isLogEnable(Level.FINEST)) {
            T4LoggingUtilities.log(this, Level.FINEST, "ENTRY", url);
        }
        if (LOG.isTraceEnabled()) {
            LOG.trace("ENTRY. url:{}", url);
        }
	...
	public java.sql.Connection connect(String url, Properties info) throws SQLException {
        if (this.isLogEnable(Level.FINEST)) {
            Properties tempinfo = new Properties();
            tempinfo.putAll(info);
            tempinfo.remove("password");
            T4LoggingUtilities.log(this, Level.FINEST, "ENTRY", url, tempinfo);
        }
        if (LOG.isTraceEnabled()) {
            Properties tempinfo = new Properties();
            tempinfo.putAll(info);
            tempinfo.remove("password");
            LOG.trace("ENTRY. url:{}, tempinfo:{}", url, tempinfo);
        }
			 ...
			 synchronized(this) {
				...
				if (info != null) {
					...
					if (getSQLException() != null) {
						throw TrafT4Messages.createSQLException(null, "invalid_property", getSQLException());
					}
				}
				if (getMaxPoolSize() != -1) {
	...
	public int getMajorVersion() {
	    if (this.isLogEnable(Level.FINEST)) {
            T4LoggingUtilities.log(this, Level.FINEST, "ENTRY");
        }
	    if (LOG.isTraceEnabled()) {
	        LOG.trace("ENTRY");
	    }
    ...
    static {
        ...
        try {
            initialLog4j();
            KeepAliveFun.initKeepAliveFun();
			TrafCachePool.getTrafCachePool();
	...
	public static void initialLog4j() {
        try {
            Class<?> logManager = Class.forName("org.apache.logging.log4j.LogManager");
            Constructor<?> conLogManager = logManager.getDeclaredConstructor();
            conLogManager.setAccessible(true);
            Object logmgr = conLogManager.newInstance();
            ...
            Constructor<?> conLoggerContext = loggerContext.getConstructor(String.class);
            Object ctx = conLoggerContext.newInstance("");
            ctx = getContext.invoke(logmgr, false);
            
            Method getConfiguration = loggerContext.getMethod("getConfiguration");
            ...
            Method getLoggerConfig = configuration.getMethod("getLoggerConfig",String.class);
            Class<?> loggerConfig = Class.forName("org.apache.logging.log4j.core.config.LoggerConfig");
            Object logconf = loggerConfig.newInstance();
            logconf = getLoggerConfig.invoke(config, "");
            Method getAppenderRefs = loggerConfig.getMethod("getAppenderRefs");
            
            List<Object> appenderRefs = (List<Object>) getAppenderRefs.invoke(logconf);
            Object[] refs = (Object[]) appenderRefs.toArray();
            
            Class<?> log4jLevel = Class.forName("org.apache.logging.log4j.Level");
            Field realLevel = log4jLevel.getField("OFF");
            Method createLogger = loggerConfig.getMethod("createLogger", String.class,Class.forName("org.apache.logging.log4j.Level"),String.class,String.class,Class.forName("[Lorg.apache.logging.log4j.core.config.AppenderRef;")
                    ,Class.forName("[Lorg.apache.logging.log4j.core.config.Property;"),Class.forName("org.apache.logging.log4j.core.config.Configuration"),Class.forName("org.apache.logging.log4j.core.Filter"));
            logconf = createLogger.invoke(logconf, "false",realLevel.get(null),"org.trafodion.jdbc.t4", "true", refs, null, config, null);
            
            Method addLogger = configuration.getMethod("addLogger", String.class,Class.forName("org.apache.logging.log4j.core.config.LoggerConfig"));
            addLogger.invoke(config, "org.trafodion.jdbc.t4", logconf);
            Method updateLoggers = loggerContext.getMethod("updateLoggers");
