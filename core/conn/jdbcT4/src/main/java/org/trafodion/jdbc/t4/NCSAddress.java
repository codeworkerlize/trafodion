...
final class NCSAddress extends Address {
	 ...
	 * 
	 */
	NCSAddress(T4Properties t4props, String addr) throws SQLException {
		super(t4props, addr);
//		int index0;
//		int index1;
//		int index2;
//		int index3;
		...
		if (acceptsURL(addr) == true) {
			...
			if (addr.endsWith(ODBCServerSuffix) == false) {
				...
				SQLException se2 = TrafT4Messages.createSQLException(m_t4props, "odbc_server_suffix_error",
						ODBCServerSuffix);
			...
			if (addr.indexOf(",") > 0)
				interpretNEOAddress(t4props, addr);
			else
				interpretAddress(t4props, addr);

			if ((m_machineName == null && m_ipAddress == null) || m_processName == null || m_portNumber == null) {
				SQLException se = TrafT4Messages.createSQLException(m_t4props, "address_parsing_error", addr);
				SQLException se2 = TrafT4Messages.createSQLException(m_t4props, "address_format_1_error");
	...
	// ----------------------------------------------------------
	void interpretAddress(T4Properties t4props, String addr) throws SQLException {
		//
	...
	void interpretNEOAddress(T4Properties t4props, String addr) throws SQLException {
		//
