...
class ExecuteMessage {
	...
	static LogicalByteArray marshal(int dialogueId, int sqlAsyncEnable, int queryTimeout, int inputRowCnt,
			...
			InterfaceConnection ic,int executeApiType
...

	{
		...
		insertXAttrsBuf(buf,executeApiType);
		return buf;
	}
	static int getXAttrsLength() throws CharacterCodingException{
		int len =  TRANSPORT.size_int;  //executeApiType
		return len;

	}
	static void insertXAttrsBuf(LogicalByteArray buf, int executeApiType)
			throws CharacterCodingException{

		buf.insertInt(executeApiType);
	}
}
