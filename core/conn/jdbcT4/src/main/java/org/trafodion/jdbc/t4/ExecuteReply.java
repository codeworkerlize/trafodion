...
class ExecuteReply extends BaseReply{
	int returnCode;
	...
	ExecuteReply(LogicalByteArray buf, InterfaceConnection ic) throws CharacterCodingException,
        ...
        // tail contains the stmtHandle to support LobV2
        if (numResultSets == 0) {
            stmtHandle = buf.extractInt();
        }
        super.setLinkThreshold(ic.getT4props().getLinkThreshold() > 0);
        if(ic.getDcsVersion() > 5){
			heapSize = buf.extractLong();
        }
        if(ic.getMajorVersion() >= 13) {
			endOfData = buf.extractBoolean();
		}
