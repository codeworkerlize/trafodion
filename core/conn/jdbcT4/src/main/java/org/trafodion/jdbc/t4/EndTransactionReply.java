...
class EndTransactionReply extends BaseReply {
	...
	EndTransactionReply(LogicalByteArray buf, String addr, InterfaceConnection ic) throws CharacterCodingException,
        ...
        if (ic.getT4props().isTraceTransTime() && !ic.getAutoCommit()) {
            long transId = buf.extractLong();
            ic.getTrafT4Conn().getTraceQueryUsedTime().setTransId(transId);
	}
	}
}
