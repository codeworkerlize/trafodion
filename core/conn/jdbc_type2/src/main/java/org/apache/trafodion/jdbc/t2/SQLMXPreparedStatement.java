...
public class SQLMXPreparedStatement extends SQLMXStatement implements
	...
	public void addBatch() throws SQLException {
		...
		try {
			...
			//rowsValue_.add(paramContainer_.clone());
	...
	public boolean execute() throws SQLException {
		...
		// **
			...
			boolean currentAutoCommit = connection_.autoCommit_;
			try {
				synchronized (connection_) {
					...
					if (isAnyLob_) {
						...
						if (outputDesc_ != null)
							throw Messages.createSQLException(connection_.locale_, "lob_as_param_not_supported", 
								null);
						else {
							try {
				...
				if (isAnyLob_ && currentAutoCommit) {
					if (! exceptionFound)
						connection_.commit();
					else
		...
		try {
			...
			if (isSelect_)
				if (batchRowCount_ == null) {
					...
					for (int i = 0 ; i < paramRowCount_; i++)
						batchRowCount_[i] = Statement.SUCCESS_NO_INFO;
			} else if (batchRowCount_ == null) {
				se = Messages.createSQLException(connection_.locale_,
						"batch_command_failed", null);
				throw new BatchUpdateException(se.getMessage(), se
						.getSQLState(), new int[0]);
		...
		try {
			...
			case Types.CLOB:
				...
				if (x == null) 
					...
					int lobInlineMaxLen = getLobInlineMaxLen(parameterIndex);
					if (lobInlineMaxLen > 0 && length <= lobInlineMaxLen) {
						...
						isAnyLob_ = true;
						clob = new SQLMXClob(this, inputDesc_[parameterIndex-1], null, 
								x, length);
...
//					}
				...
				case Types.REAL:
					double indbl = x.doubleValue();
					BigDecimal bd = new BigDecimal(String.valueOf(Float.MAX_VALUE));
					if (x.compareTo(bd) > 0) {
						Object[] parmIndex = new Object[2];
						parmIndex[0] = Double.toString(indbl);
						parmIndex[1] = new Integer(parameterIndex);
						throw Messages.createSQLException(connection_.locale_,
								"numeric_out_of_range",parmIndex );
					} else {
				...
				if (x == null) 
					...
					if (length != (long) 0) {
						if (lobInlineMaxLen > 0 && length <= lobInlineMaxLen) {
							...
							isAnyLob_ = true;
							blob = new SQLMXBlob(this, inputDesc_[parameterIndex - 1], null,
									x, length);
							inputDesc_[parameterIndex - 1].paramValue_ = "";
							paramContainer_.setBytes(parameterIndex, new byte[0]);
						}
					} else {
						paramContainer_.setBytes(parameterIndex, getBuffer(buffer, len, x, length));
					}
				}
			...
			default:
				if (x == null) { 
					...
					paramContainer_.setString(parameterIndex,
							new String(getBuffer(buffer,len,x,length)));
				}
			}
			inputDesc_[parameterIndex - 1].isValueSet_ = true;
		} finally {
	...
	private byte[] getBuffer(byte[] buffer, int len, InputStream x, long length)
			throws SQLException {
		if (length != (long) 0) {
			buffer = new byte[(int) length];
			try {
		...
		try {
			...
			case Types.BLOB:
				...
				if (x == null) 
					...
					isAnyLob_ = true;
					blob = new SQLMXBlob(this, inputDesc_[parameterIndex-1], null, x);
					inputDesc_[parameterIndex - 1].paramValue_ = "";
					paramContainer_.setBytes(parameterIndex, new byte[0]);
				}
				addLobObjects(parameterIndex, blob);
		...
		try {
			...
			case Types.TIMESTAMP:
				throw Messages.createSQLException(connection_.locale_,
						"invalid_datatype_for_column", null);
			case Types.BOOLEAN:
				paramContainer_.setBoolean(parameterIndex,dataWrapper
		...
		try {
			...
			case Types.CLOB:
				...
				if (x == null) 
					...
					if (lobInlineMaxLen > 0 && x.length <= lobInlineMaxLen)
						...
						isAnyLob_ = true;
						clob = new SQLMXClob(this, inputDesc_[parameterIndex-1], null, x);
						inputDesc_[parameterIndex - 1].paramValue_ = "";
						paramContainer_.setBytes(parameterIndex, new byte[0]);
					}
				}
			...
			case Types.LONGVARCHAR:
				...
				if (dataCharSet == SQLMXDesc.SQLCHARSETCODE_UCS2)
					charSet = "UTF-16LE";
                else if (dataCharSet == SQLMXDesc.SQLCHARSETCODE_UTF8)
                    charSet = "UTF-8";
				try {
			...
			case Types.CLOB:
				...
				if (reader == null) 
					...
					if (lobInlineMaxLen > 0 && length <= lobInlineMaxLen) {
						...
						isAnyLob_ = true;
						clob = new SQLMXClob(this, inputDesc_[parameterIndex-1], null, reader, length);
						inputDesc_[parameterIndex - 1].paramValue_ = "";
						paramContainer_.setBytes(parameterIndex,new byte[0]);
					}
				}
			...
			if (x == null) {
				...
				case Types.NUMERIC:
					if (dataObj == dataWrapper) {
                        ...
                        else if (dataWrapper.getDataType(1) == DataWrapper.STRING)
                            setBigDecimal(parameterIndex, new BigDecimal(
                                          dataWrapper.getString(1)));
                        else if (dataWrapper.getDataType(1) == DataWrapper.LONG)
                            setBigDecimal(parameterIndex, new BigDecimal(
                                          dataWrapper.getLong(1,connection_.locale_)));
						else
...
//											connection_.locale_)));
			...
			if (x == null && dataType != Types.CLOB && dataType != Types.BLOB) {
				...
				case Types.TIME: // Need by SQLJ to pass QA tests
                    if (x.length() == 0) {
                        ...
                        String regDate = "\\d{2}:\\d{2}:\\d{2}(\\.\\d{1,6})?";
                        x = x.trim();
                        if (!x.matches(regDate)) {
                            Object[] errMsg = new Object[1];
                            errMsg[0] = x;
                            throw Messages.createSQLException(connection_.locale_,
                                "time_format_error", errMsg);
                        }
                        paramContainer_.setString(parameterIndex, x);
                    }
					break;
				...
				case Types.BLOB:
					...
					if (x == null) 
						...
						if (lobInlineMaxLen > 0 && x.length() <= lobInlineMaxLen) {
							...
							isAnyLob_ = true;
							blob = new SQLMXBlob(this, inputDesc_[parameterIndex-1], 
								null, x.getBytes());
							inputDesc_[parameterIndex - 1].paramValue_ = "";
							paramContainer_.setBytes(parameterIndex, new byte[0]);
						}
		...
		try {
			...
			if (x == null) {
				...
				case Types.TIMESTAMP:
					...
					break;
				case Types.TIME:
					t1 = new Time(adjustedTimestamp.getTime());
					paramContainer_.setString(parameterIndex, t1.toString());
					break;
				case Types.DATE:
					d1 = new Date(adjustedTimestamp.getTime());
					paramContainer_.setString(parameterIndex, d1.toString());
					break;
				default:
					throw Messages.createSQLException(connection_.locale_,
	...
	boolean isAnyLob_ = false;
	ArrayList<SQLMXLob> lobObjects_;
	ArrayList<String> lobColNames_;
	ArrayList<Integer> lobColIds_;
	String[] lobLocators_;
	boolean addLobColNames_ = true; // lobColNames_ are added for one row
	boolean resetLobObjects_ = false;
	...
	public Object unwrap(Class iface) throws SQLException {
		Messages.throwUnsupportedFeatureException(connection_.locale_, "PreparedStatement.isWrapperFor()");
		return null;
	}
	...
	public void setRowId(int parameterIndex, RowId x) throws SQLException {
		Messages.throwUnsupportedFeatureException(connection_.locale_, "PreparedStatement.setRowId()");
	}

	public void setNString(int parameterIndex, String value)
			throws SQLException {
		Messages.throwUnsupportedFeatureException(connection_.locale_, "PreparedStatement.setNString()");
	}
	
	public void setNCharacterStream(int parameterIndex, Reader value,
			long length) throws SQLException {
		Messages.throwUnsupportedFeatureException(connection_.locale_, "PreparedStatement.setNCharacterStream()");
	}

	public void setNClob(int parameterIndex, NClob value) throws SQLException {
		Messages.throwUnsupportedFeatureException(connection_.locale_, "PreparedStatement.setNClob()");
	}
	...
	public void setSQLXML(int parameterIndex, SQLXML xmlObject)
			throws SQLException {
		Messages.throwUnsupportedFeatureException(connection_.locale_, "PreparedStatement.setSQLXML()");
	}
	...
	public void setAsciiStream(int parameterIndex, InputStream x)
        ...
        try {
			...
			throw Messages.createSQLException(connection_.locale_, "io_exception", null);
        }
	...
	public void setBinaryStream(int parameterIndex, InputStream x)
			throws SQLException {
		setBinaryStream(parameterIndex, x, (long)0);
	}
...
//		setCharacterStream(parameterIndex, reader, (long)0);
		...
		} else if (reader instanceof SQLMXClobReader) {
			long length = ((SQLMXClobReader) reader).getLength();
			setCharacterStream(parameterIndex, reader, length);
		} else {
	...
	public void setBlob(int parameterIndex, InputStream inputStream)
			throws SQLException {
		setBlob(parameterIndex, inputStream, (long)0);
	}
	...
	public String getInputDescCatalogName(int parameterIndex) {
		return inputDesc_[parameterIndex-1].catalogName_;
    	}

	public String getInputDescTableName(int parameterIndex) {
		return inputDesc_[parameterIndex-1].tableName_;
    	}

	public String getLobInputDescName(int parameterIndex) {
		return inputDesc_[parameterIndex-1].columnLabel_;
	}
	...
	private void addLobObjects(int parameterIndex, SQLMXLob x)
	{
		...
		if (addLobColNames_) {	
			lobColNames_.add(getLobInputDescName(parameterIndex));
			lobColIds_.add(parameterIndex);
		}
		lobObjects_.add(x);
		resetLobObjects_ = false;
	}
	...
	private String getLobColumns() 
	{
		...
		//colNames.append("lobconverthandle(").append(lobColNames_.get(0)).append(", 1)");
		for (int i = 1; i < lobColNames_.size(); i++)
			//colNames.append(", lobconverthandle(").append(lobColNames_.get(i)).append(", 1)");
	...
	private String[] getLobLocators() throws SQLException
	{
	   ...
	   try {
		...
		int numLocators = ((paramRowCount_ == 0 ? 1 : paramRowCount_) * getNumLobColumns());
        ...
        int getLobInlineMaxLen(int parameterIndex)
	{
		return inputDesc_[parameterIndex - 1].lobInlineMaxLen_;
	}
}
