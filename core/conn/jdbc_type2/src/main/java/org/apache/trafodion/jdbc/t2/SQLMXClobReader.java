...
public class SQLMXClobReader extends Reader
{
	...
	public int read() throws IOException
	{
		...
		try
		{
			...
			if (currentPos_ == charsRead_)
				retValue = readChunkThrowIO();
			if (retValue != -1)
			{
				retValue = chunk_.charAt(currentPos_);
				currentPos_++;
			}
			return retValue;
		}
		...
		throws IOException
	{
		...
		try
		{
			...
			copyOffset = off;
			while (remainLen > 0) {
				availableLen = charsRead_ - currentPos_;
				...
				if (copyLen > 0) { 
					if (! skip) 
						chunk_.getChars(currentPos_, copyLen, cbuf, copyOffset);
					currentPos_ += copyLen;
					copyOffset += copyLen;
					copiedLen += copyLen;
					remainLen -= copyLen;
	...
	{
		// eor_ can be set to true before all the bytes are read from chunk
		// So, don't use eor_ in any other function
		...
		if (currentLobPos_ >= startLobPos_) {
			if (length_ > (currentLobPos_ + clob_.chunkSize_-1))
				readLen = clob_.chunkSize_;
			else
		...
		ByteBuffer chunkByteBuf = readChunk(conn_.server_, conn_.getDialogueId(), stmt_.getStmtId(), conn_.getTxid(), 
			extractMode, clob_.lobVersion_, clob_.getLobTableUid(), clob_.lobLocator_, readLen); 
		if (chunkByteBuf != null && chunkByteBuf.capacity() > 0) {
			String charsetName = SQLMXDesc.getCharacterSetName(clob_.sqlCharSet_);
			byte[] chunkByteArray = new byte[chunkByteBuf.capacity()];
			chunkByteBuf.get(chunkByteArray);
			try {
				chunk_ = new String(chunkByteArray, charsetName);
			} catch (java.io.UnsupportedEncodingException e) {
				throw new SQLException(e);
			}
			charsRead_ = chunk_.length();
			currentPos_ = 0;
			currentLobPos_ += charsRead_;
			if (chunkByteBuf.capacity()  < readLen)
				eor_ = true;
		} else {
			chunk_ = null;
			charsRead_ = -1;
	...
	long getBeginLobPos()
	{
		return currentLobPos_ - charsRead_;
	}

	native ByteBuffer readChunk(String server, long dialogueId, long stmtId, long txid,  int extractMode, short lobVersion, long lobTableUid, 
			String lobLocator, int chunkLen);

	// constructors
	SQLMXClobReader(SQLMXStatement stmt, SQLMXConnection conn, SQLMXClob clob, long startPos, long length)
	{
		...
		try
		{
			...
			if (clob_.inlineDataString_ != null) {
				int inlineLen = clob_.inlineDataString_.length();
				currentLobPos_ = inlineLen;
				if (inlineLen > startLobPos_) {
					chunk_ = clob_.inlineDataString_.substring((int)startLobPos_-1);
					charsRead_ = inlineLen;
