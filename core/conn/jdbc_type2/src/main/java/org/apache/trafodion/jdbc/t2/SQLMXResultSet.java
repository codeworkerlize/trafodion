...
public class SQLMXResultSet extends SQLMXHandle implements java.sql.ResultSet {
		...
		try {
			...
			if (wasNull_)
				...
				if (x instanceof Blob)
					...
					lobInlineData = (String)x; 
					data = new SQLMXBlob(stmt_, outputDesc_[columnIndex-1], lobInlineData);
					row.setObject(columnIndex, data);
					isAnyLob_ = true;
				} else if (x instanceof byte[]) {
					data = new SQLMXBlob(stmt_, outputDesc_[columnIndex-1], (byte[])x);
					row.setObject(columnIndex, data);
					isAnyLob_ = true;
				} else
					throw Messages.createSQLException(connection_.locale_,
		...
		try {
			...
			switch (dataType) {
				...
				case Types.BLOB: // HP Extension allows BLOB;
					...
					if (wasNull_) {
						return null;
					} else if (blob instanceof SQLMXBlob) {
						return ((SQLMXBlob) blob).getBytes();
					} else {
						throw Messages.createSQLException(connection_.locale_,
								"Not a SQLMXBlob", null);
					}
		...
		try {
			...
			if (wasNull_)
				...
				if (x instanceof Clob)
					...
					lobInlineData = (String) x;
					data = new SQLMXClob(stmt_, outputDesc_[columnIndex-1], lobInlineData);
					row.setObject(columnIndex, data);
					isAnyLob_ = true;
				} else if (x instanceof byte[]) {
					lobInlineData = new String((byte[])x);
					data = new SQLMXClob(stmt_, outputDesc_[columnIndex-1], lobInlineData);
					row.setObject(columnIndex, data);
					isAnyLob_ = true;
				} else
	...
	public Object getObject(int columnIndex) throws SQLException {
		...
		try {
			...
			validateGetInvocation(columnIndex);
			dataType = outputDesc_[columnIndex - 1].dataType_;
			int sqltype = getSQLDataType(columnIndex - 1);
		...
		try {
			...
			{ // Some of these types are not supported by SQL/MX, but here for
				...
				case Types.CLOB:
					...
					if ((wasNull_)) {
						return null;
					} else if (clob instanceof SQLMXClob) {
						return ((SQLMXClob) clob).getString();
					} else {
						throw Messages.createSQLException(connection_.locale_,
								"Not a SQLMXClob", null);
					}
				default:
					...
					try {
						if (x instanceof byte[]) {
							if (charsetname != null) {
								...
								// Mantis-12834
								// Remove possible extra paddings for UTF8 CHAR data
								if (charsetcode == SQLMXDesc.SQLCHARSETCODE_UTF8) {
									int sqlPrecision = outputDesc_[columnIndex - 1].sqlPrecision_;
									if (sqlPrecision > 0 && sqlPrecision < data.length()) {
		...
		try {
			...
			if (timeStr != null) {
				...
				case Types.TIME:
					...
					try{
						if (outputDesc_[columnIndex - 1].sqlPrecision_ > 0) {
							retValue = Time.valueOf(timeStr.substring(0, timeStr.indexOf(".")));
						} else {
							retValue = Time.valueOf(timeStr);
						}
					} catch (IllegalArgumentException e) {
						throw Messages.createSQLException(connection_.locale_, "invalid_cast_specification",null);
					}
					break;
				default:
					throw Messages.createSQLException(connection_.locale_,
	...
	public Timestamp getTimestamp(int columnIndex) throws SQLException {
		...
		try {
			...
			if (dataType != Types.CHAR && dataType != Types.VARCHAR
					&& dataType != Types.LONGVARCHAR && dataType != Types.DATE
					&& dataType != Types.TIMESTAMP && dataType != Types.TIME) {
				throw Messages.createSQLException(connection_.locale_,
						"restricted_data_type", null);
			}
						...
						try {
							...
							try {
								...
								try {
									...
									if (sqlPrecision > 0) {
										nano = Integer.parseInt(timestampStr
												.substring(timestampStr.indexOf(".") + 1));
										nano *= Math.pow(10, 9 - sqlPrecision);
										timestampStr = timestampStr
												.substring(0, timestampStr.indexOf("."));
									}

									timeValue = Time.valueOf(timestampStr);
...
//Venu changed stmtId from int to long for 64 bit
	...
	public Object unwrap(Class iface) throws SQLException {
		Messages.throwUnsupportedFeatureException(connection_.locale_, "ResultSet.unwrap()");
		return null;
	}

	public boolean isWrapperFor(Class iface) throws SQLException {
		Messages.throwUnsupportedFeatureException(connection_.locale_, "ResultSet.isWrapperFor()");
		return false;
	}

	public RowId getRowId(int columnIndex) throws SQLException {
		Messages.throwUnsupportedFeatureException(connection_.locale_, "ResultSet.getRowId()");
		return null;
	}
	...
	public void updateRowId(int columnIndex, RowId x) throws SQLException {
		Messages.throwUnsupportedFeatureException(connection_.locale_, "ResultSet.updateRowId()");
	}
	...
	public void updateNString(int columnIndex, String nString)
			throws SQLException {
		Messages.throwUnsupportedFeatureException(connection_.locale_, "ResultSet.updateNString()");
	}
	...
	public void updateNClob(int columnIndex, NClob nClob) throws SQLException {
		Messages.throwUnsupportedFeatureException(connection_.locale_, "ResultSet.updateNClob()");
	}
	...
	public NClob getNClob(int columnIndex) throws SQLException {
		Messages.throwUnsupportedFeatureException(connection_.locale_, "ResultSet.getNClob()");
		return null;
	}
	...
	public SQLXML getSQLXML(int columnIndex) throws SQLException {
		Messages.throwUnsupportedFeatureException(connection_.locale_, "ResultSet.getSQLXML()");
		return null;
	}
	...
	public void updateSQLXML(int columnIndex, SQLXML xmlObject)
			throws SQLException {
		Messages.throwUnsupportedFeatureException(connection_.locale_, "ResultSet.updateSQLXML()");
	}
	...
	public String getNString(int columnIndex) throws SQLException {
		Messages.throwUnsupportedFeatureException(connection_.locale_, "ResultSet.getNString()");
		return null;
	}
	...
	public Reader getNCharacterStream(int columnIndex) throws SQLException {
		Messages.throwUnsupportedFeatureException(connection_.locale_, "ResultSet.getNCharacterStream()");
		return null;
	}
	...
	public void updateNCharacterStream(int columnIndex, Reader x, long length)
			throws SQLException {
		Messages.throwUnsupportedFeatureException(connection_.locale_, "ResultSet.updateNCharacterStream()");
	}
	...
	public void updateAsciiStream(int columnIndex, InputStream x, long length)
			throws SQLException {
		Messages.throwUnsupportedFeatureException(connection_.locale_, "ResultSet.updateAsciiStream()");
	}

	public void updateBinaryStream(int columnIndex, InputStream x, long length)
			throws SQLException {
		Messages.throwUnsupportedFeatureException(connection_.locale_, "ResultSet.updateBinaryStream()");
	}

	public void updateCharacterStream(int columnIndex, Reader x, long length)
			throws SQLException {
		Messages.throwUnsupportedFeatureException(connection_.locale_, "ResultSet.updateCharacterStream()");
	}

	public void updateAsciiStream(String columnLabel, InputStream x, long length)
			throws SQLException {
		Messages.throwUnsupportedFeatureException(connection_.locale_, "ResultSet.updateAaciiStream()");
	}
	...
	public void updateBlob(int columnIndex, InputStream inputStream, long length)
			throws SQLException {
		Messages.throwUnsupportedFeatureException(connection_.locale_, "ResultSet.updateBlob()");
	}
	...
	public void updateClob(int columnIndex, Reader reader, long length)
			throws SQLException {
		Messages.throwUnsupportedFeatureException(connection_.locale_, "ResultSet.updateClob()");
	}
	...
	public Object getObject(int columnIndex, Class type) throws SQLException {
		Messages.throwUnsupportedFeatureException(connection_.locale_, "ResultSet.getObject()");
		return null;
	}
    ...
    short getLobVersion(int columnIndex) 
    {
	return outputDesc_[columnIndex - 1].lobVersion_;
    }

    int getLobChunkSize(int columnIndex) 
    {
	return outputDesc_[columnIndex - 1].lobChunkMaxLen_;
    }

    int getCharacterSet(int parameterIndex)
    {
	return outputDesc_[parameterIndex - 1].sqlCharset_;
    }

    String getLobHandle(int columnIndex) throws SQLException
    {
	String lobLocator = null;

	int dataType = outputDesc_[columnIndex - 1].dataType_;
	switch (dataType) 
	{
	case Types.CLOB:
		SQLMXClob clob = (SQLMXClob)getClob(columnIndex);
		wasNull_ = (clob == null);
		if (! wasNull_)
			lobLocator = clob.getLobLocator();
		break;
	case Types.BLOB:
		SQLMXBlob blob = (SQLMXBlob)getBlob(columnIndex);
		wasNull_ = (blob == null);
		if (! wasNull_)
			lobLocator = blob.getLobLocator();
		break;
	default:
		throw Messages.createSQLException(connection_.locale_,
						"restricted_data_type", null);
