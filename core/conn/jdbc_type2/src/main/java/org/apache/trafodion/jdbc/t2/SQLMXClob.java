...
public class SQLMXClob extends SQLMXLob implements Clob 
{
	...
	public Reader getCharacterStream(long pos, long length) throws SQLException
	{
		...
		try
		{
			...
			reader_ = new SQLMXClobReader(stmt_, conn_, this, pos, length);
			return reader_;
		}
		finally
		{
	...
	public long position(String searchStr, long start) throws SQLException
	{
		...
		SQLMXClobReader cr = (SQLMXClobReader)(getCharacterStream(start, Long.MAX_VALUE));
		int readLen;
		int index;
		long retPosition = -1;

		while ((readLen = cr.readChunk()) != -1) {
			if ((index = cr.chunk_.indexOf(searchStr, cr.currentPos_)) != -1) {
				retPosition =  cr.getBeginLobPos()  + index + cr.currentPos_;	 
				break;
			}
		}
		return retPosition;
	...
	public Writer setCharacterStream(long pos) throws SQLException
	{
		...
		try
		{
			...
			writer_ = new SQLMXClobWriter(stmt_, conn_, this, pos);
			return writer_;
		}
		finally
		{
	...
	{
		return getSubString(1, chunkSize_);
	}
	...
	String getString(int inlineLobLen) throws SQLException 
		...
		if (llength > Integer.MAX_VALUE) {
			Object[] messageArguments = new Object[1];
			messageArguments[0] = "Clob.getString(int)";
			throw Messages.createSQLException(conn_.locale_,"invalid_input_value", messageArguments);
		}
	...
	// Constructors
	public SQLMXClob(SQLMXConnection conn) throws SQLException
	{
		super(conn, false);
	}

	SQLMXClob(SQLMXStatement stmt, SQLMXDesc lobDesc, String inlineData, InputStream x, long length) throws SQLException
	{
		super(stmt, lobDesc, inlineData, x, length, false);
		sqlCharSet_ = lobDesc.getSqlCharSet();	
	}

	SQLMXClob(SQLMXStatement stmt, SQLMXDesc lobDesc, String inlineData, Reader x, long length) throws SQLException
	{
		super(stmt, lobDesc, inlineData, false);
		sqlCharSet_ = lobDesc.getSqlCharSet();	
		ir_ = x;
		length_ = length;
	}
	
	SQLMXClob(SQLMXStatement stmt, SQLMXDesc lobDesc, String inlineData, Clob inputLob) throws SQLException
	{
		super(stmt, lobDesc, inlineData, false);
		sqlCharSet_ = lobDesc.getSqlCharSet();	
		inputLob_ = inputLob;
	...
	SQLMXClob(SQLMXStatement stmt, SQLMXDesc lobDesc, String inlineData, String lobStr) throws SQLException
	{
		super(stmt, lobDesc, inlineData, false);
		sqlCharSet_ = lobDesc.getSqlCharSet();	
		inputLobStr_ = lobStr;
	...
	SQLMXClob(SQLMXStatement stmt, SQLMXDesc lobDesc, String inlineData) throws SQLException
	{
		super(stmt, lobDesc, inlineData, false);
		sqlCharSet_ = lobDesc.getSqlCharSet();	
	}

	SQLMXClob(SQLMXStatement stmt, SQLMXDesc lobDesc, String inlineData, byte[] data) throws SQLException
	{
		super(stmt, lobDesc, inlineData, false, data);
		sqlCharSet_ = lobDesc.getSqlCharSet();	
	}
