...
class SQLMXDesc 
{
	 ...
	 * or UNKNOWN.                                                    */
	static String getCharacterSetName(int sqlCharSet) throws SQLException
	{
	...
	SQLMXDesc(int dataType, short datetimeCode, int maxLen, short precision,
		...
		int intLeadPrec, int paramMode, String colLabel, int lobInlineMaxLen, int lobChunkMaxLen)
	{
		...
		try
		{
			...
			switch (dataType_)
			{
				...
				case Types.TIMESTAMP:
					...
					break;
				case Types.CHAR:
                    if (sqlDataType_ == SQLTYPECODE_BOOLEAN) {
                        dataType_ = Types.BOOLEAN;
                        displaySize_ = 1;
                    } else {
    ...
    public static final int SQLTYPECODE_BOOLEAN                = -701;
	public static final int SQLTYPECODE_TINYINT                = -403;
	public static final int SQLTYPECODE_TINYINT_UNSIGNED       = -404;
	...
	public static final String SQLCHARSETSTRING_UNICODE	    = "UTF-16LE";
	...
	int		displaySize_;
	String	columnLabel_;
	int	lobInlineMaxLen_; // -1 means V1 LOB 0 amd > 0 means V2 LOB
	int 	lobChunkMaxLen_;
        short   lobVersion_;
