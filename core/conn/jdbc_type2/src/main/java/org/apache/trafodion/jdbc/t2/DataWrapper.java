...
public class DataWrapper implements Cloneable
{
	...
	{
		DataWrapper cloneDW = new DataWrapper();
		cloneDW.totalColumns = totalColumns;
		if (byteValue != null)
			cloneDW.byteValue = Arrays.copyOf(byteValue, byteValue.length);
		if (shortValue != null)
			cloneDW.shortValue = Arrays.copyOf(shortValue, shortValue.length);
		if (intValue != null)
			cloneDW.intValue = Arrays.copyOf(intValue, intValue.length);
		if (longValue != null)
			cloneDW.longValue = Arrays.copyOf(longValue, longValue.length);
		if (floatValue != null)
			cloneDW.floatValue = Arrays.copyOf(floatValue, floatValue.length);
		if (doubleValue != null)
			cloneDW.doubleValue = Arrays.copyOf(doubleValue, doubleValue.length);
		if (booleanValue != null)
			cloneDW.booleanValue = Arrays.copyOf(booleanValue, booleanValue.length);
		if (dataType != null)
			cloneDW.dataType = Arrays.copyOf(dataType, dataType.length);
		if (bytesValue != null)
			cloneDW.bytesValue = Arrays.copyOf(bytesValue, byteValue.length);
		if (objectValue != null)
			cloneDW.objectValue = Arrays.copyOf(objectValue, objectValue.length);
		if (origValue != null)
			cloneDW.origValue = Arrays.copyOf(origValue, origValue.length);
		if (isNullValue != null)
			cloneDW.isNullValue = Arrays.copyOf(isNullValue, isNullValue.length);
		if (numericValid != null)
			cloneDW.numericValid = Arrays.copyOf(numericValid, numericValid.length);
		if (setNeeded != null)
			cloneDW.setNeeded = Arrays.copyOf(setNeeded, setNeeded.length);
		if (SQLbytesValue != null)
			cloneDW.SQLbytesValue = Arrays.copyOf(SQLbytesValue, SQLbytesValue.length);
		cloneDW.updated = updated;
		cloneDW.deleted = deleted;
		cloneDW.inserted = inserted;
		cloneDW.isInsertRow = isInsertRow;
		if (dataTruncation != null)
			cloneDW.dataTruncation = Arrays.copyOf(dataTruncation, dataTruncation.length);
		return cloneDW;
	}
...
//---------------------
	...
	public String convertSQLBigNumToBigDecimal2(byte[] sourceData,
        ...
        boolean negative = ((sourceData[sourceData.length - 1] & 0x80) > 0);
        sourceData[sourceData.length - 1] &= 0x7F; // force sign to 0, continue
		// normally
...
//			dataInShorts[i] = DataWrapper.extractUShort(sourceData, i * 2, false); // copy
            dataInShorts[i] = DataWrapper.extractUShort(sourceData, i * 2, true); // numeric was changed to little-end
		// the
		// data
...
//			targetData[i * 2 + 1] = (byte) (targetInShorts[i] & 0xFF);
            targetData[i * 2 ] = (byte) (targetInShorts[i] & 0xFF); // numeric was changed to little-end
	...
	void setString(int col_num, String s) throws SQLException
	{
		...
		try
		{
			...
			if (Vcvalue.getVproc().equals("1")) {
                isNullValue[col_num-1] = s==null;
            }else {
			    isNullValue[col_num-1] = (s==null || s.length()==0);
            }
			...
			if (!isNullValue[col_num-1])
			{
				...
				switch (dataType[col_num-1])
				{
					...
					case BYTES:
						...
						try {
							convertStringToNumeric(col_num, allowBoolean, locale,
									new String(save_bytes));
						} catch (SQLException e) {
							// If NumberFormatException
							StringBuffer result = new StringBuffer();
							Character ch = 0;
							for (int i = 0; i < save_bytes.length; i += 2) {
								if (save_bytes[i + 1] == 0) {
									ch = (char) save_bytes[i];
								} else {
									ch = (char) ((save_bytes[i] << 8) & 0xFF00 | save_bytes[i + 1]);
								}

								if (ch == 0) { // End of string \0
									break;
								}
								result.append(ch);
							}
		...
		try
		{
			...
			switch (dataType[col_num-1])
			{
				case STRING:{
                    BigDecimal bd;
                    try {
                        bd = new BigDecimal((String)objectValue[col_num - 1]);
                        if (bd.compareTo(BigDecimal.valueOf(Long.MAX_VALUE)) > 0
                         || bd.compareTo(BigDecimal.valueOf(Long.MIN_VALUE)) < 0) {
						    ...
						    throw Messages.createSQLException(locale,
						        "value_out_of_range", errorParmList);
                        }
                    // truncation will be solved after switch/case
				...
				case BIG_DECIMAL:{
					...
					if( ((BigDecimal)objectValue[col_num - 1]).compareTo(new BigDecimal(longMaxValue)) > 0
				        || ((BigDecimal)objectValue[col_num - 1]).compareTo(new BigDecimal(longMinValue)) < 0	) {
						Object[] errorParmList = new Object[2];
				        errorParmList[0] = getString(col_num);
				        errorParmList[1] = new Integer(col_num);
				        throw Messages.createSQLException(locale,
