...
public class SQLMXLobOutputStream extends OutputStream
{
	...
	public void write(int b) throws IOException
	{
		...
		try
		{
			if (isClosed_)
				throw new IOException("Output stream is in closed state");
			chunk_[currentPos_] = (byte)b;
			isFlushed_ = false;
			currentPos_++;
			if (currentPos_ == lob_.chunkSize_) {
				writeChunkThrowIO(chunk_, 0, currentPos_);
			}
		}
		finally
	...
	public void write(byte[] b, int off, int len) throws IOException
	{
		...
		try
		{
			...
			copyLen = len;
			while (true) {
				if ((copyLen+currentPos_) < lob_.chunkSize_) {
					System.arraycopy(b, srcOffset, chunk_, currentPos_, copyLen);
					currentPos_ += copyLen;
					isFlushed_ = false;
					break;
				} else {
					if (currentPos_ != 0) {
						tempLen = lob_.chunkSize_-currentPos_;		
						System.arraycopy(b, srcOffset, chunk_, currentPos_, tempLen);
						currentPos_ += tempLen;
						writeChunkThrowIO(chunk_, 0, currentPos_);
					} else {
	...
	void writeChunk(byte[] chunk, int off, int len) throws SQLException
	{
		...
		writeChunk(conn_.server_, conn_.getDialogueId(), stmt_.getStmtId(), conn_.getTxid(),
				lob_.lobVersion_, lob_.getLobTableUid(), lob_.lobLocator_, chunk, off, len, lobWritePos);
		currentChunkNo_++;
		currentPos_ = 0;
		isFlushed_ = true;
	}
	...
	void populate(InputStream is, long length) throws SQLException
	{
		...
		try
		{
			...
			try
			{
				while (true)
				{
					if (readLen > 0 && readLen <= lob_.chunkSize_)
						tempLen = (int)readLen;
					else
						tempLen = lob_.chunkSize_;
					if (tempLen == 0)
	...
	// constructors
	SQLMXLobOutputStream(SQLMXStatement stmt, SQLMXConnection conn, long startingPos, SQLMXLob lob) throws 
		SQLException
	{
	...
	native void writeChunk(String server, long dialogueId, long stmtId, long txid, short lobVersion, long lobTablueUid,
					String lobLocator, byte[] chunk, int off, int writeLength, long pos);
}
