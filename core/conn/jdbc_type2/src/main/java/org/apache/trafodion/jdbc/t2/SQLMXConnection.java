...
static final short TYPE_INSERT_PARAM = 0x0120; //Modified for CQDs filter from 0x0100 to 0x0120
...
public static boolean getSqlStmtType2(String str) {
    ...
    static short getSqlStmtType(String str) {
        ...
        if ((str3.equals("SELECT")) || (str3.equals("SHOWSHAPE"))
            ...
            rt1 = TYPE_DELETE;
        } else if (str3.equals("INSERT") || (str3.equals("UPSERT"))) {
            if (str.indexOf('?') == -1) {
                rt1 = TYPE_INSERT;
            } else {
...
private native void setSchema(String server, long dialogueId, String schema);

private native void setClientInfo(String server, long dialogueId, String name, String value);
...
public Object unwrap(Class iface) throws SQLException {
	Messages.throwUnsupportedFeatureException(locale_, "Connection.unwrap()");
        return null;
    }

public boolean isWrapperFor(Class iface) throws SQLException {
	Messages.throwUnsupportedFeatureException(locale_, "Connection.isWrapperFor()");
        return false;
    }
...
public NClob createNClob() throws SQLException {
	Messages.throwUnsupportedFeatureException(locale_, "Connection.createNClob()");
        return null;
    }

public SQLXML createSQLXML() throws SQLException {
	Messages.throwUnsupportedFeatureException(locale_, "Connection.createSQLXML()");
        return null;
    }
...
public void setClientInfo(String name, String value)
    throws SQLClientInfoException {
        ...
        if (isClosed_)
           return;
        try {
            setClientInfo(server_, getDialogueId(), name, value);
        } catch (SQLException e) {
...
public void setSchema(String schema) throws SQLException {
    ...
    if (isClosed_)
       throw Messages.createSQLException(locale_, "invalid_connection", null);
    if (schema != null) {
       setSchema(server_, getDialogueId(), schema);
       String tmpSchema = schema.trim();
       char[] schemaCharArray = tmpSchema.toCharArray();
       boolean dotFound = false;
       boolean quoteFound = false;
       boolean firstPartQuoted = false;
       boolean secondPartQuoted = false;
       int beginFirstPartIdx = 0;
       int endFirstPartIdx = 0; 
       int beginSecondPartIdx = 0;
       int endSecondPartIdx = 0;
       for (int i = 0; i < schemaCharArray.length; i++) {
           char c = schemaCharArray[i]; 
           if (quoteFound) {
		if (c == '\"') {
                   if (dotFound) { 
                      secondPartQuoted = true;
                      endSecondPartIdx = i;
                   }
                   else {
                      firstPartQuoted = true;
                      endFirstPartIdx = i;
                   }
                   quoteFound = false;
                }
               ...
               dotFound = true;
               endFirstPartIdx = i-1;
               beginSecondPartIdx = i+1;
            }
       }
       if (dotFound) {
          if (secondPartQuoted)
             schema_ = tmpSchema.substring(beginSecondPartIdx+1, endSecondPartIdx);
          else
             schema_ = tmpSchema.substring(beginSecondPartIdx).toUpperCase();
          if (firstPartQuoted) 
             catalog_ = tmpSchema.substring(1, endFirstPartIdx);
          else
             catalog_ = tmpSchema.substring(0, endFirstPartIdx+1).toUpperCase();
       }
       else {
          if (firstPartQuoted) 
             schema_ = tmpSchema.substring(1, endFirstPartIdx);
          else
             schema_ = tmpSchema.toUpperCase();
       }
       updateConnectionReusability(SQL_SET_SCHEMA);
    }
}
