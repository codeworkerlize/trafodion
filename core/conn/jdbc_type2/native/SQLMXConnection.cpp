...
JNIEXPORT void JNICALL Java_org_apache_trafodion_jdbc_t2_SQLMXConnection_setSchema
  (JNIEnv *jenv, jobject jobj, jstring server, jlong dialogueId, jstring schema)
{
    FUNCTION_ENTRY("Java_org_apache_trafodion_jdbc_t2_SQLMXConnection_setCatalog",("jenv=0x%08x, server=%s, dialogueId=0x%08x, schema=%s",
                    jenv,
                    DebugJString(jenv,server),
                    dialogueId,
    ...
    const char *nSchema = NULL;
    if (schema == NULL)
	return;
    nSchema = JNI_GetStringUTFChars(jenv, schema, NULL);

    setConnectAttr(jenv, jobj, server, dialogueId, SET_SCHEMA, 0, nSchema);
    if (schema)
...
JNIEXPORT void JNICALL Java_org_apache_trafodion_jdbc_t2_SQLMXConnection_setClientInfo
  (JNIEnv *jenv, jobject jobj, jstring server, jlong dialogueId, jstring key, jstring value)
{
    FUNCTION_ENTRY("Java_org_apache_trafodion_jdbc_t2_SQLMXConnection_setClientInfo",("jenv=0x%08x, server=%s, dialogueId=0x%08x, key=%s, value=%s",
                    jenv,
                    DebugJString(jenv,server),
                    dialogueId,
                    DebugJString(jenv,key),
                    DebugJString(jenv,value)));
    ...
    if (value == NULL)
        return;
    nkey = JNI_GetStringUTFChars(jenv, key, NULL);
    nvalue = JNI_GetStringUTFChars(jenv, value, NULL);
    char clientInfo[MAX_CLIENT_INFO_LEN] = {0};
    snprintf(clientInfo, MAX_CLIENT_INFO_LEN, "%s:%s", nkey, nvalue);
    SQL_EXEC_SetClientInfo(clientInfo, strlen(clientInfo), NULL);
    if (nkey)
       JNI_ReleaseStringUTFChars(jenv, key, nkey);
    if (nvalue)
       JNI_ReleaseStringUTFChars(jenv, value, nvalue);
        ...
        jint statisticsIntervalTime, jint statisticsLimitTime, jstring statisticsType, jstring programStatisticsEnabled, jstring statisticsSqlPlanEnabled)
{
    ...
    {
        throwSetConnectionException(jenv, &setConnectException);
        FUNCTION_RETURN_VOID(("SET_ODBC_PROCESS - setConnectException.exception_nr(%s) is not CEE_SUCCESS",
                        CliDebugSqlError(setConnectException.exception_nr)));
    }
