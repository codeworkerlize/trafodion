...

{
    ...
    DEBUG_OUT(DEBUG_LEVEL_ENTRY,("  stmtType=%s, sqlString='%s', holdability=%d",
  ...
  )
{
    ...
    if (lobVersion == 2) {
       ...
       if ((QryLobExtractSrvrStmt = pSrvrStmt->getLobExtractStmt(lobTableUid, stmtId, exception_)) == NULL)
          return;
...
SRVR_STMT_HDL *getLobUpdateStmt(short lobVersion, long dialogueId, long stmtId, long lobTableUid, ExceptionStruct *exception_)
{
    long sqlcode;
    SRVR_STMT_HDL *pSrvrStmt;
    SRVR_STMT_HDL * lobUpdateStmt = NULL;
    ...
    if (lobVersion == 2) {
       ...
       if ((lobUpdateStmt = pSrvrStmt->getLobUpdateStmt(lobTableUid, stmtId, exception_)) == NULL)
          return NULL;
   }
   else 
	abort();
   return lobUpdateStmt;
}   
  ...
  , BYTE *updateData)
{
    ...
    if (updateLobPos == 0) {
        snprintf(lobUpdateQuery, sizeof(lobUpdateQuery),  "UPDATE LOB (LOB'%s', LOCATION %Ld, SIZE %d)", lobHandle, (Int64)updateData, updateLen);
    } else {
        snprintf(lobUpdateQuery, sizeof(lobUpdateQuery),  "UPDATE LOB (LOB'%s', LOCATION %Ld, SIZE %d, APPEND)", lobHandle, (Int64)(updateData+offset), updateLen);
    }
