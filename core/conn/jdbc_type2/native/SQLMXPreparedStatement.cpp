 ...
 jobject resultSet, jboolean contBatchOnError)
{
	...
	*/
		...
		paramRowNumber += rowsExecuted)
	{
		...
		// Setup batch binding
		if (pSrvrStmt->batchSetup(totalRows) < SQL_SUCCESS) {
			throwSQLException(jenv, PROGRAMMING_ERROR, "batchSetup return an error", "HY000");
			return;
                }
			...
			*/
			if (jenv->ExceptionOccurred() != NULL)
			{
				if (exceptionHead != NULL)
					jenv->CallVoidMethod(exceptionHead, gJNICache.setNextExceptionMethodId, jenv->ExceptionOccurred());
				else
					exceptionHead = jenv->ExceptionOccurred();
				jenv->ExceptionClear();
				if (isSelect) // ignore contBatchOnError
					break;
				if (contBatchOnError)
				{
		...
		*/
		if (!contBatchOnError && ((exceptionHead != NULL) || (exception_.exception_nr!=CEE_SUCCESS)))
		{
			break;
		}
		if (errorRow > 0)
