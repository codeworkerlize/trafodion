...
long AdjustMaxLength(SQLItemDesc_def *SQLItemDesc)
{
	long ret_length = SQLItemDesc->maxLen;
	
	switch (SQLItemDesc->dataType)
	{
		case SQLTYPECODE_CLOB:
		case SQLTYPECODE_BLOB:
			if (SQLItemDesc->lobInlineMaxLen == -1) // Lob V1
				ret_length = LOB_V1_EXT_HANDLE_LEN;
			else
				ret_length = SQLItemDesc->lobInlineMaxLen + LOB_V2_EXT_HANDLE_LEN;
			break;
	}
	return ret_length;
}
						...
						char *ColHeading)
{
	...
	case SQLTYPECODE_LARGEINT:
		SQLItemDesc->ODBCDataType  = SQL_BIGINT;
		SQLItemDesc->ODBCPrecision = 19;
		SQLItemDesc->signType = TRUE;
		break;
	case SQLTYPECODE_LARGEINT_UNSIGNED:
		SQLItemDesc->ODBCDataType  = SQL_BIGINT;
		SQLItemDesc->ODBCPrecision = 20;
		SQLItemDesc->signType = FALSE;
		break;
	...
	case SQLTYPECODE_CLOB:
		...
		if (SQLItemDesc->lobInlineMaxLen == -1) // Lob V1
			SQLItemDesc->maxLen = LOB_V1_EXT_HANDLE_LEN;
		else
			SQLItemDesc->maxLen = SQLItemDesc->lobInlineMaxLen + LOB_V2_EXT_HANDLE_LEN;
	        break;	
	case SQLTYPECODE_BLOB:
...
SQLRETURN BuildSQLDesc(SRVR_STMT_HDL*pSrvrStmt, SRVR_STMT_HDL::DESC_TYPE descType)
{
	...
	*totalMemLen = 0;
	for (i = 0; i < numEntries; i++) {
		...
		SQLItemDesc->precision    = gDescItems[2].num_val_or_len;
		SQLItemDesc->scale        = gDescItems[3].num_val_or_len;
		...
		SQLItemDesc->lobInlineMaxLen = gDescItems[16].num_val_or_len;
		SQLItemDesc->lobChunkMaxLen = gDescItems[17].num_val_or_len;

		SQLItemDesc->maxLen = AdjustMaxLength(SQLItemDesc);
		...
		SQLDesc->_length++;
		if (SQLItemDesc->lobChunkMaxLen > 0)
			pSrvrStmt->setLobChunkMaxLen(SQLItemDesc->lobChunkMaxLen);
...
SQLRETURN PREPARE(SRVR_STMT_HDL* pSrvrStmt)
{
	...
	* ***************************************************************************** */
	...
	DEBUG_OUT(DEBUG_LEVEL_CLI,("SQL Query Statement Type=%s",
		...
		if ((SqlQueryStatementType == SQL_EXE_UTIL  || SqlQueryStatementType == SQL_DDL_WITH_STATUS) &&
...
SQLRETURN EXTRACTLOB(SRVR_STMT_HDL *pSrvrStmt, IDL_string lobHandle, BYTE *&extractData, IDL_long_long &extractLen)
{ 
   ...
   if (! pSrvrStmt->isClosed) { 
      ...
      *pSrvrStmt->getChunkLenPtr() = extractLen;
      retcode = CLI_Fetch(pStmt, pOutputDesc, NULL); 
   }
   else {
      DEBUG_ASSERT(paramCount == 3, "EXTRACTLOB prepared statement has less than 3 parameters");
      if (IPD[0].dataType == SQLTYPECODE_CHAR) {
         ...
         DEBUG_ASSERT(false, "EXTRACTLOB prepared statement parameter 0 is not SQLTYPECODE_CHAR");
      if (IPD[1].dataType == SQLTYPECODE_LARGEINT) {
         *(IDL_long_long *)IPD[1].varPtr = (IDL_long_long)pSrvrStmt->getChunkBufferPtr();
      }
      else
         DEBUG_ASSERT(false, "EXTRACTLOB prepared statement parameter 1 is not SQLTYPECODE_LARGEINT");
      if (IPD[2].dataType == SQLTYPECODE_LARGEINT) {
          *pSrvrStmt->getChunkLenPtr() = extractLen;
          *(IDL_long_long *)IPD[2].varPtr = (IDL_long_long)pSrvrStmt->getChunkLenPtr();
      }
      else
         DEBUG_ASSERT(false, "EXTRACTLOB prepared statement parameter 2 is not SQLTYPECODE_LARGEINT");    
      retcode = CLI_Exec(pStmt, pInputDesc, NULL);
      HANDLE_THREAD_ERROR(retcode, sqlWarning, pSrvrStmt);
      retcode = CLI_Fetch(pStmt, pOutputDesc, NULL);
   }
   HANDLE_THREAD_ERROR(retcode, sqlWarning, pSrvrStmt);
   extractData = pSrvrStmt->getChunkBufferPtr();
...
SQLRETURN UPDATELOB(SRVR_STMT_HDL *pSrvrStmt, IDL_string lobHandle, BYTE *&updateData, int &updateLen, IDL_long_long &updatePos)
{ 
   SQLRETURN rc = SQL_SUCCESS;
   long retcode = SQL_SUCCESS;
   BOOL	sqlWarning = FALSE;
   ...
   DEBUG_ASSERT(paramCount == 4, "UPDATELOB prepared statement has less than 4 parameters");
   if (IPD[0].dataType == SQLTYPECODE_CHAR) {
      lobHandleParam = (char *)IPD[0].varPtr;
      snprintf(lobHandleParam, 1024, "%-1024s", lobHandle);
      lobHandleParam[1023] = ' '; 
      ...
      DEBUG_ASSERT(false, "UPDATELOB prepared statement parameter 0 is not SQLTYPECODE_CHAR");
   if (IPD[1].dataType == SQLTYPECODE_LARGEINT) {
      *(IDL_long_long *)IPD[1].varPtr = (IDL_long_long)pSrvrStmt->getChunkBufferPtr();
   }
   else
      DEBUG_ASSERT(false, "UPDATELOB prepared statement parameter 1 is not SQLTYPECODE_LARGEINT");
   if (IPD[2].dataType == SQLTYPECODE_LARGEINT) {
       *pSrvrStmt->getChunkLenPtr() = updateLen;
       *(IDL_long_long *)IPD[2].varPtr = (IDL_long_long)pSrvrStmt->getChunkLenPtr();
   }
   else
      DEBUG_ASSERT(false, "UPDATELOB prepared statement parameter 2 is not SQLTYPECODE_LARGEINT");    
   if (IPD[3].dataType == SQLTYPECODE_LARGEINT) {
      ...
      DEBUG_ASSERT(false, "UPDATELOB prepared statement parameter 3 is not SQLTYPECODE_LARGEINT");    
   // The updateLobStmt_ is also closed with CLI_ExecFetch call
   retcode = CLI_ExecFetch(pStmt, pInputDesc, NULL);
   HANDLE_THREAD_ERROR(retcode, sqlWarning, pSrvrStmt);
   if (sqlWarning) 
      THREAD_RETURN(pSrvrStmt,SQL_SUCCESS_WITH_INFO);
   THREAD_RETURN(pSrvrStmt,SQL_SUCCESS);
...
SQLRETURN REALLOCSQLMXHDLS(SRVR_STMT_HDL* pSrvrStmt)
{
        ...
        if (! pSrvrStmt->isClosed)
        {
                retcode = SQL_EXEC_CloseStmt(pStmt);
                pSrvrStmt->isClosed = TRUE;
        }
        if (pSrvrStmt->moduleName[0] == '\0')
        {
                retcode = SQL_EXEC_DeallocStmt(pStmt);
        }
        ...
        if (pModule->module_name == NULL)
        {
                if (pSrvrStmt->sqlQueryType == SQL_SP_RESULT_SET)
                  {
                  retcode = SQL_EXEC_AllocStmtForRS(pSrvrStmt->callStmtId, pSrvrStmt->resultSetIndex, pStmt);
                  HANDLE_ERROR(retcode, sqlWarning);
                  }
                else
                  {
                  retcode = SQL_EXEC_AllocStmt(pStmt,(SQLSTMT_ID *)NULL);
                  HANDLE_ERROR(retcode, sqlWarning);
                  }
        }
        return retcode;
