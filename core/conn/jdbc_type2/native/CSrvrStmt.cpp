...
SRVR_STMT_HDL *SRVR_STMT_HDL::getLobExtractStmt(long tableUid, long stmtId, ExceptionStruct  *exception)
{
   ...
   if (lobExtractStmt_ != NULL && (! errorInLobExtractStmt_) && (tableUid == lobTableUid_)) 
    ...
    if (lobExtractStmt_ == NULL) {
       if ((lobExtractStmt_ = createSrvrStmt(dialogueId, lobExtractStmtName, &sqlcode,
    ...
    rc = lobExtractStmt_->Prepare(&sqlString, TYPE_UNKNOWN, SQL_NONHOLDABLE,0); 
    if (rc == SQL_ERROR) {
...
SRVR_STMT_HDL *SRVR_STMT_HDL::getLobUpdateStmt(long tableUid, long stmtId, ExceptionStruct  *exception)
{
   ...
   if (lobUpdateStmt_ != NULL && (! errorInLobUpdateStmt_) && (tableUid == lobTableUid_)) 
    ...
    if (lobUpdateStmt_ == NULL) {
       if ((lobUpdateStmt_ = createSrvrStmt(dialogueId, lobUpdateStmtName, &sqlcode,
                        NULL, 0, 0, TYPE_UNKNOWN, false,false)) == NULL) {
    ...
    rc = lobUpdateStmt_->Prepare(&sqlString, TYPE_UNKNOWN, SQL_NONHOLDABLE,0); 
    if (rc == SQL_ERROR) {
...
SQLRETURN SRVR_STMT_HDL::batchSetup(long paramRowCount)
{
    SQLRETURN retcode = SQL_SUCCESS;
    FUNCTION_ENTRY_LEVEL(DEBUG_LEVEL_ROWSET,"SRVR_STMT_HDL::batchSetup",("paramRowCount=%ld",
        paramRowCount));

    bool didBatchPrepare = false;
    if (paramRowCount <= 1)
       return retcode;
    if (batchMaxRowsetSize == 0) { 
       if ((retcode = rePrepareForRowset(paramRowCount)) < SQL_SUCCESS)
          return retcode;
       didBatchPrepare = true;
    }

    // Limit the size to the maximum
    if ((didBatchPrepare) || (paramRowCount > batchMaxRowsetSize) || (batchRowsetSize == 0))
    {
        DEBUG_OUT(DEBUG_LEVEL_DATA|DEBUG_LEVEL_ROWSET,("Batch area being reallocated"));
        if (batchQuadField != NULL) {
           MEMORY_DELETE_ARRAY(batchQuadField);
        }
        batchQuadEntries = 0;
        batchRowsetSize = 0;
        ...
        if (paramCount > 0 && paramRowCount > 0)
        {
            // We want to create a rowset
            batchRowsetSize = paramRowCount;
        ...
        retcode = SET_DATA_PTR(this, Input);
    } 
    return retcode;
}
      ...
      IDL_long_long &updatePos, ExceptionStruct *exception)  
{
   ...
   switch (rc)
   {
    ...
    case SQL_ERROR:
        ...
        break;
    default:
        exception->exception_nr = odbc_SQLSvc_UpdateLob_ParamError_exn_;
        break;
    }
    CLI_DEBUG_RETURN_SQL(rc);
}
...
SQLRETURN SRVR_STMT_HDL::rePrepareForRowset(long paramRowCount)
{
   SQLRETURN retcode = SQL_SUCCESS;
   retcode = REALLOCSQLMXHDLS(this);
   if (retcode < SQL_SUCCESS)
      return retcode;
   if ((retcode = setMaxBatchSize(paramRowCount)) < SQL_SUCCESS)
      return retcode;
   SQLValue_def tmpSqlString;  
   tmpSqlString.dataCharset = sqlString.dataCharset;
   tmpSqlString.dataInd = sqlString.dataInd;
   tmpSqlString.dataType = sqlString.dataType;
   tmpSqlString.dataValue._buffer = (unsigned char *)new char[sqlString.dataValue._length+1];
   strcpy((char *)tmpSqlString.dataValue._buffer, (const char *)sqlString.dataValue._buffer);
   tmpSqlString.dataValue._length = sqlString.dataValue._length;
   if ((retcode = Prepare(&tmpSqlString, stmtType, holdability, queryTimeout)) < SQL_SUCCESS) {
      delete tmpSqlString.dataValue._buffer;
      return retcode;
   }
}
