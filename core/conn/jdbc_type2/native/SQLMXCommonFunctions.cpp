	...
	jobjectArray NewDescArray(JNIEnv *jenv, jobject jobj, SQLItemDescList_def *desc, jint offset)
	{
		...
		for (index = offset; index < desc->_length ; index++)
		{
			...
			SQLMXDesc = jenv->NewObject(gJNICache.SQLMXDescClass, gJNICache.SQLMXDescConstructorId,
				...
				SQLDesc->intLeadPrec,
				SQLDesc->paramMode,
				colLabel, SQLDesc->lobInlineMaxLen,
				SQLDesc->lobChunkMaxLen);
				...
				if (!isNull)
				{
					...
					else
					{
                        ...
                        case SQLTYPECODE_BOOLEAN:
                            memcpy(dataPtr, (const BOOL *)byteValue, sizeof(BOOL));
                            break;
						case SQLTYPECODE_CHAR:
							...
							if (dataLengthNotExceeded(charSet, dataLen, allocLength))
							{
                                ...
                                if (charSet == SQLCHARSETCODE_UCS2)
                                {
                                    ...
                                    for (int i = (dataLen+1); i < allocLength; i+=2)
                                    {
                                        *(dataPtr+i) = 0;
                                    }
                                }
						...
						case SQLTYPECODE_BLOB:
							...
							else if ((IPD[paramNumber + paramOffset].vc_ind_length == 2
                                    ||IPD[paramNumber + paramOffset].vc_ind_length == 0) && (allocLength >= dataLen + sizeof(short)))
							{
...
//---------------------------------------------------dataPtr--------
		...
		jboolean isSigned, jint FSDataType, jint charSet, jstring iso88591Encoding, jint vcIndLength)
	{
		...
		case SQLTYPECODE_LARGEINT_UNSIGNED:
			*(unsigned long long *)targetDataPtr = getWrapperLong(wrapperInfo,paramNumber);
			DEBUG_OUT(DEBUG_LEVEL_DATA,("SQLTYPECODE_LARGEINT_UNSIGNED: targetData = '%Lu'",*(unsigned long long *)targetDataPtr));
			retcode                = TRUE;
			goto func_exit;
			break;
		...
		case SQLTYPECODE_INTERVAL:
			...
			break;
        case SQLTYPECODE_BOOLEAN:
            *(BOOL *)targetDataPtr = getWrapperBoolean(wrapperInfo,paramNumber);
            DEBUG_OUT(DEBUG_LEVEL_DATA,("SQLTYPECODE_BOOLEAN: targetData = '%hd'",*(BOOL *)targetDataPtr));
            retcode = TRUE;
            goto func_exit;
            break;
		default:
		...
		case SQLTYPECODE_CHAR:
			...
			if (dataLengthNotExceeded(charSet, dataLen, targetLength))
			{
				...
				{
					// Back fill target buffer with double byte 'space' characters (i.e. 0x00 0x20)
					for (i = (dataLen + 1); i < targetLength; i+=2)
					{
						*(targetDataPtr+i) = 0;
					}
				}
		...
		case SQLTYPECODE_INTERVAL:
			...
			else if ((vcIndLength == 2 || vcIndLength == 0) && targetLength >= dataLen + sizeof(short))
			{
...
func_exit:
	...
	BOOL cacheJNIObjects(JNIEnv *jenv)
	{
		...
		gJNICache.SQLMXDescConstructorId = JNI_GetMethodID(jenv,SQLMXDescClass, "<init>",
			"(ISISSZLjava/lang/String;ZISIILjava/lang/String;Ljava/lang/String;Ljava/lang/String;IIILjava/lang/String;II)V");
...
//------------
		...
		gJNICache.SPJRS_stmtIDFieldID  = JNI_GetFieldID(jenv, ResultSetInfoClass, "stmtID", "J");
...
//---------------------------------------
		...
		case SQLTYPECODE_LARGEINT:
			...
			break;
        case SQLTYPECODE_LARGEINT_UNSIGNED:
            usource = *(UInt64 *)SQLValue->dataValue._buffer;
            sprintf(cTmpBuf, "%lu", usource);
            strDataPtr = cTmpBuf;
            break;
		case SQLTYPECODE_NUMERIC:
		case SQLTYPECODE_NUMERIC_UNSIGNED:
		...
		case SQLTYPECODE_IEEE_FLOAT:
			...
			FUNCTION_RETURN_NUMERIC(FALSE,("SQLValue->dataType=%s (unsupported). DATATYPE_NOT_SUPPPORTED_ERROR thrown.",
				CliDebugSqlTypeCode(SQLValue->dataType)));
        case SQLTYPECODE_BOOLEAN:
            source = *(char *)SQLValue->dataValue._buffer;
            convertInt64ToWrapper(wrapperInfo,columnIndex,source,scale);
            DEBUG_OUT(DEBUG_LEVEL_DATA,("SQLTYPECODE_BOOLEAN: SQL datavalue = %Ld",
                source));
            break;
		case SQLTYPECODE_TINYINT:
			source = *(char *)SQLValue->dataValue._buffer;
            convertInt64ToWrapper(wrapperInfo,columnIndex,source,scale);
            DEBUG_OUT(DEBUG_LEVEL_DATA,("SQLTYPECODE_TINYINT: SQL datavalue = %Ld",
                source));
            break;
		case SQLTYPECODE_TINYINT_UNSIGNED:
			source = *(unsigned char *)SQLValue->dataValue._buffer;
            convertInt64ToWrapper(wrapperInfo,columnIndex,source,scale);
            DEBUG_OUT(DEBUG_LEVEL_DATA,("SQLTYPECODE_TINYINT_UNSIGNED: SQL datavalue = %Ld",
                source));
            break;
		default:
...
/*
	long copyDataToNative(JNIEnv *jenv, int chunkType, jobject jUpdateData, jint jOffset, jlong jLength, BYTE *chunkBuf)
	{
		long retLen = -1; 
		switch (chunkType) {
			case 0:
   				jenv->GetByteArrayRegion((jUpdateData, jOffset, jLength, chunkBuf); 
				retLen = jLength;
				break;
			case 1:
				jenv->GetStringUTFRegion(jUpdateData, jOffset, jLength, chunkBuf);
   				// Find the null pointer
   				int chunkLength = jLength;
   				for (int i = jLength ; i < utfCharLen ; i++) {
        				if (chunkBuf[i] == '\0') {
           					retLen = i;
           					break;
       		 			}
