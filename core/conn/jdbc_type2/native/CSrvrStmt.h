...
public:
    ...
    BYTE *getChunkBufferPtr() 
    { 
       if (chunk_ == NULL)
          getBlobChunkBuffer();
       return chunk_;
    }
    BYTE *setBlobChunkBuffer() 
    {
       if (blobChunk_ == NULL){
          blobChunk_ = new BYTE[lobChunkMaxLen_];
       }
       return blobChunk_;
    }

    BYTE *getBlobChunkBuffer() 
    {
       chunk_ = setBlobChunkBuffer();
       return chunk_;
    }

    void setClobChunkBuffer(BYTE *chunkBuf)
    {
       chunk_ = chunkBuf;
    }
    ...
    SQLRETURN batchSetup(long paramRowCount);
    ...
    SQLRETURN rePrepareForRowset(long paramRowCount);
private:
