...
void processALoadMessage(UdrGlobals *UdrGlob, UdrServerReplyStream &msgStream, UdrLoadMsg &request,
  ...
  NABoolean showLoadLogic = (UdrGlob->verbose_ && UdrGlob->traceLevel_ >= TRACE_IPMS && UdrGlob->showLoad_);
  ...
  if (!handleAlreadyExists) {
    if (!UdrHandleIsValid(request.getHandle())) {
      ...
      *diags << DgString0("Load Message");
      ...
      if (showLoadLogic) {
        ServerDebug("[UdrServ (%s)]  About to call LM::getRoutine", moduleName);
      }
      ...
      if (sp == NULL) {
        ...
        *diags << DgString0("SPInfo");
        ...
        LmRoutine *lmRoutine;
        LmResult lmResult;
        LmLanguageManager *lm = UdrGlob->getOrCreateLM(lmResult, sp->getLanguage(), diags);
        ...
        if (lm) {
          if (sp->getParamStyle() == COM_STYLE_JAVA_OBJ || sp->getParamStyle() == COM_STYLE_CPP_OBJ) {
            lmResult = lm->getObjRoutine(request.getUDRSerInvocationInfo(), request.getUDRSerInvocationInfoLen(),
                                         request.getUDRSerPlanInfo(), request.getUDRSerPlanInfoLen(), sp->getLanguage(),
                                         sp->getParamStyle(), sp->getExternalName(), sp->getContainerName(),
                                         sp->getExternalPathName(), sp->getLibrarySqlName(), &lmRoutine, diags);

            if (lmRoutine && lmResult == LM_OK) {
              if (sp->getParamStyle() == COM_STYLE_CPP_OBJ) {
                LmRoutineCppObj *cppRoutine = static_cast<LmRoutineCppObj *>(lmRoutine);

#ifndef NDEBUG
                int debugLoop = 2;

                if (cppRoutine->getInvocationInfo()->getDebugFlags() & tmudr::UDRInvocationInfo::DEBUG_LOAD_MSG_LOOP)
                  debugLoop = 1;
                ...
                LmRoutineJavaObj *javaObjRoutine = static_cast<LmRoutineJavaObj *>(lmRoutine);

                // avoid unpacking the UDRInvocationInfo to look
                // up row lengths and take those from the request
                // message instead
                javaObjRoutine->setRowLengths(request.getInputRowSize(), request.getOutputRowSize());
              ...
              // by tdm_udrserv
              lmResult = lmRoutine->setFunctionPtrs(SpInfoGetNextRow, SpInfoEmitRowCpp, diags);
              ...
              if (lmResult == LM_OK)
                ...
                // chance to change it)
                lmResult = lmRoutine->setRuntimeInfo(request.getParentQid(), request.getNumInstances(),
                                                     request.getInstanceNum(), diags);
            }
          } else
            lmResult = lm->getRoutine(
                sp->getNumParameters(), sp->getLmParameters(), sp->getNumTables(), sp->getLmTables(),
                sp->getReturnValue(), sp->getParamStyle(), sp->getTransactionAttrs(), sp->getSQLAccessMode(),
                sp->getParentQid(), sp->getClientInfo(), sp->getRequestRowSize(), sp->getReplyRowSize(),
                sp->getSqlName(), sp->getExternalName(), sp->getRoutineSig(), sp->getContainerName(),
                sp->getExternalPathName(), sp->getLibrarySqlName(), UdrGlob->getCurrentUserName(),
                UdrGlob->getSessionUserName(), sp->getExternalSecurity(), sp->getRoutineOwnerId(), &lmRoutine,
                (LmHandle)&SpInfoGetNextRow, emitRowFuncPtr, sp->getMaxNumResultSets(), diags);
        }
        ...
        if (showLoadLogic) {
          if (lmResult == LM_OK) {
            sprintf(errorText, "[UdrServ (%.30s)]  LM::getRoutine was successful.", moduleName);
          } else {
            sprintf(errorText, "[UdrServ (%.30s)]  LM::getRoutine resulted in error.", moduleName);
          }
          ServerDebug(errorText);
          doMessageBox(UdrGlob, TRACE_SHOW_DIALOGS, UdrGlob->showMain_, errorText);
        }
  ...
  UdrLoadReply *reply = new (UdrGlob->getIpcHeap()) UdrLoadReply(UdrGlob->getIpcHeap());

  if (reply == NULL) {  // no reply buffer build...
  ...
  if (diags && diags->getNumber() > 0) {
    ...
    if (showLoadLogic) dumpDiagnostics(diags, 2);
  }
  ...
  if (UdrGlob && UdrGlob->getJavaLM()) {
    sleepIfPropertySet(*(UdrGlob->getJavaLM()), "MXUDR_LOAD_DELAY", diags);
  }
#endif  // _DEBUG
...
SPInfo *processLoadParameters(UdrGlobals *UdrGlob, UdrLoadMsg &request, ComDiagsArea &d) {
  ...
  if (UdrGlob->traceLevel_ >= TRACE_DETAILS && UdrGlob->showLoad_) {
    displayLoadParameters(request);
  }

  doMessageBox(UdrGlob, TRACE_SHOW_DIALOGS, UdrGlob->showLoad_, "create SPInfo");

  oldDiags = d.getNumber();

  while (sp == NULL && numRetries < 1) {
    sp = new (UdrGlob->getUdrHeap()) SPInfo(
        UdrGlob, UdrGlob->getUdrHeap(), request.getHandle(), (char *)request.getSqlName(),
        (char *)request.getRoutineName(), (char *)request.getSignature(), (char *)request.getContainerName(),
        (char *)request.getExternalPath(), (char *)request.getLibrarySqlName(), request.getNumParameters(),
        request.getNumInValues(), request.getNumOutValues(), request.getMaxResultSets(), request.getTransactionAttrs(),
        request.getSqlAccessMode(), request.getLanguage(), request.getParamStyle(), request.isIsolate(),
        request.isCallOnNull(), request.isExtraCall(), request.isDeterministic(), request.getExternalSecurity(),
        request.getRoutineOwnerId(), request.getInBufferSize(), request.getOutBufferSize(), request.getInputRowSize(),
        request.getOutputRowSize(),
        /* ComDiagsArea */ d, (char *)request.getParentQid(), (char *)request.getClientInfo());
    if (sp == NULL) {  // memory problem
  ...
  // set the Linux cgroup, if used
  UdrGlob->getCurrCGroup().setOrAlterCGroup(request.getTenantName(), d);
  ...
  if (sp->getParamStyle() == COM_STYLE_SQLROW_TM || sp->getParamStyle() == COM_STYLE_CPP_OBJ ||
  ...
  if (request.getUdrJavaDebugPort() > 0 && sp->getLanguage() == COM_LANGUAGE_JAVA &&
      UdrGlob->getJavaLM() == NULL)  // Langman not started already
  {
    ...
    if (debugPort % 1000 == 0) debugPort += (int)getpid() % 1000;

    if (timeout > 0) {
      // suspend the JVM and wait for <timeout> milliseconds for the
      // debugger to attach
      snprintf(buf, sizeof(buf), "-agentlib:jdwp=transport=dt_socket,address=%d,server=y,timeout=%d", debugPort,
               timeout);
      printf("Debugging tdm_udrserv JVM instance %d of %d, attach debugger to port %d within %d seconds\n",
             request.getInstanceNum(), request.getNumInstances(), debugPort, timeout / 1000);
    } else {
      // don't suspend the JVM, attaching the debugger is optional
      snprintf(buf, sizeof(buf), "-agentlib:jdwp=transport=dt_socket,address=%d,server=y,suspend=n", debugPort);
      printf("Debugging tdm_udrserv JVM instance %d of %d, attach debugger to port %d\n", request.getInstanceNum(),
             request.getNumInstances(), debugPort);
    }
  ...
  if (UdrGlob->verbose_ && UdrGlob->traceLevel_ >= TRACE_DATA_AREAS && UdrGlob->showLoad_) {
  ...
  if (UdrGlob->verbose_ && UdrGlob->traceLevel_ >= TRACE_DATA_AREAS && UdrGlob->showLoad_) ServerDebug("");
...
void displayLoadParameters(UdrLoadMsg &request) {
  ...
  ServerDebug("  parent ClientInfo   : %s", request.getClientInfo());
...
// into LM.
void copyRoutineOptionalData(UdrLoadMsg &request, SPInfo *sp) {
  if (sp->getLanguage() != COM_LANGUAGE_C) return;
