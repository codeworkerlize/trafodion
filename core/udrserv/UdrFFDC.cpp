...
static void getActiveRoutineInfoMsg(char *msg, NABoolean &isRoutineActive) {
  ...
  if (UDR_GLOBALS) {
    getActiveRoutineInfo(UDR_GLOBALS, routineName, routineType, routineLanguage, isRoutineActive);
  }

  if (routineName[0] == '\0')
    sprintf(msg, "A user routine is not currently being processed");
  else {
    sprintf(msg, "User routine being processed : %s, Routine Type : %s, Language Type : %s", routineName, routineType,
            routineLanguage);
...
void logEMS(const char *msg) { SQLMXLoggingArea::logSQLMXAbortEvent("", 0, msg); }

// This method writes to stderr which gets redirected to monitor log file
inline void logStdErr(const char *msg) { write(2, msg, strlen(msg)); }

void comTFDS(const char *msg1, const char *msg2, const char *msg3, const char *msg4, const char *msg5,
             NABoolean dialOut, NABoolean writeToSeaLog) {
...
void makeTFDSCall(const char *msg, const char *file, UInt32 line, NABoolean dialOut, NABoolean writeToSeaLog) {
  ...
  comTFDS(dialOut ? msg1 : msg1a, msg2, msg3, msg4, NULL, dialOut, writeToSeaLog);
}
...
NABoolean restoreJavaSignalHandlers() {
  ...
  for (int i = 0; i < sizeof(signalAttrTable) / sizeof(SignalAttr); i++) {
    ...
    if (sigaction(signalAttrTable[i].sigType, &sigAct, (struct sigaction *)NULL)) {
      cerr << "[MXUDR DEBUG SIGNAL HANDLER] "
           << "sigaction() failed, cannot restore Java signal handler for " << signalAttrTable[i].sigName << endl;
      return FALSE;
    }
  }
  return TRUE;
...
NABoolean restoreUdrTrapSignalHandlers(NABoolean saveJavaSignalHandlers) {
  ...
  for (int i = 0; i < sizeof(signalAttrTable) / sizeof(SignalAttr); i++) {
    ...
    if (sigaction(signalAttrTable[i].sigType, &sigAct, (struct sigaction *)NULL)) {
      cerr << "[MXUDR DEBUG SIGNAL HANDLER] "
           << "sigaction() failed, cannot restore UDR trap signal handler for " << signalAttrTable[i].sigName << endl;
      return FALSE;
    } else {
