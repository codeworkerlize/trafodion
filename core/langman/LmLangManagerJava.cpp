...
NABoolean getDefineSetting(const char *defineName, char *defineValue, short *defineValueLen) { return FALSE; }
...
#define ERR_LM_PARAM_OVERFLOW(result, pos)               \
  if (result == LM_PARAM_OVERFLOW) {                     \
    *da << DgSqlCode(-LME_DATA_OVERFLOW) << DgInt0(pos); \
    result = LM_ERR;                                     \
  }
...
#define WARN_LM_PARAM_OVERFLOW(result, pos)                  \
  if (result == LM_PARAM_OVERFLOW) {                         \
    *da << DgSqlCode(LME_DATA_OVERFLOW_WARN) << DgInt0(pos); \
    result = LM_OK;                                          \
  }
...
LmLanguageManagerJava::LmLanguageManagerJava(LmResult &result, NABoolean commandLineMode, ComUInt32 maxLMJava,
                                             LmJavaOptions *userOptions, ComDiagsArea *diagsArea)
  ...
  if (jvmMessages && jvmMessages[0]) {
    if (diagsArea && result != LM_OK) {
      *diagsArea << DgSqlCode(-LME_JVM_CONSOLE_OUTPUT) << DgString0(jvmMessages);
    } else {
      fprintf(stderr, "%s", jvmMessages);
      fflush(stderr);
    }
  }
}

void LmLanguageManagerJava::initialize(LmResult &result, ComUInt32 maxLMJava, LmJavaOptions *userOptions,
  ...
  // non-NULL value.
  loadClassId_ = verifyMethodId_ = createCLId_ = utilityInitId_ = classCacheSizeId_ = classCacheEnforceId_ =
      systemGetPropId_ = systemSetPropId_ = systemClearPropId_ = bigdecCtorId_ = bigdecStrId_ = bigdecUnscaleId_ =
          bigintIntValueId_ = bigintLongValueId_ = dateStrId_ = dateValId_ = timeStrId_ = timeValId_ = stampStrId_ =
              stampValId_ = intCtorId_ = intValId_ = longCtorId_ = longValId_ = floatCtorId_ = floatValId_ =
                  doubleCtorId_ = doubleValId_ = bytesToUnicodeId_ = unicodeToBytesId_ = utilityGetRSInfoId_ =
                      utilityInitRSId_ = utilityGetT4RSInfoId_ = utilityInitT4RSId_ = utilityGetConnTypeId_ =
                          lmObjMethodInvokeClass_ = makeNewObjId_ = setRuntimeInfoId_ = routineMethodInvokeId_ =
                              routineReturnInfoClass_ = returnInfoStatusField_ = returnInfoSQLStateField_ =
                                  returnInfoMessageField_ = returnInfoRIIField_ = returnInfoRPIField_ = udrClass_ =
                                      udrQueueStateField_ = rsCloseId_ = rsBeforeFirstId_ = rsNextId_ = rsWasNullId_ =
                                          rsGetWarningsId_ = rsGetShortId_ = rsGetIntId_ = rsGetLongId_ =
                                              rsGetFloatId_ = rsGetDoubleId_ = rsGetStringId_ = rsGetObjectId_ =
                                                  rsGetBigDecimalId_ = rsGetDateId_ = rsGetTimeId_ = rsGetTimestampId_ =
  ...
  if (maxLMJava_ == 0) maxLMJava_ = maxLMJava;

  ++numLMJava_;

  if (numLMJava_ > maxLMJava_) {
    *diagsArea_ << DgSqlCode(-LME_INTERNAL_ERROR) << DgString0(": Max JVMs exceeded.");
    result = LM_ERR;
    return;
  }
  ...
  if (jrc != JNI_OK) {
    *diagsArea_ << DgSqlCode(-LME_INTERNAL_ERROR) << DgString0(": Unable to determine whether JVMs exist.");
    result = LM_ERR;
    return;
  }
  if (nVMs == 0) LmSqlJVM = NULL;  // JVM has not yet been started
  ...
  if (LmSqlJVM == NULL) {
    ...
    LM_DEBUG1("commandLineMode_ is %s", getCommandLineMode() ? "TRUE" : "FALSE");
    LM_DEBUG1("setDefaultCatSchFlag_ is %s", setDefaultCatSchFlag_ ? "TRUE" : "FALSE");
    ...
    if (debugTimeoutStr != NULL) debugTimeout = atoi(debugTimeoutStr);
    const char *suspendOnDebug = getenv("JVM_SUSPEND_ON_DEBUG");
    if (debugPort > 0) numHookOpts++;
#endif
    ...
    if (numHookOpts > 0) {
      ...
      if (debugPort > 0) {
        ...
        sprintf(debugOptions, "-agentlib:jdwp=transport=dt_socket,address=%d,server=y,timeout=%d", debugPort,
                debugTimeout);
    ...
    for (i = 0; i < numUserOpts; i++) {
      vmOptions[i + numHookOpts].optionString = (char *)(actualJvmOptions->getOption(i));
    }
  ...
  exceptionReporter_ = new (collHeap()) LmJavaExceptionReporter((LmHandle)jni, this, result, diagsArea_);
  if (result == LM_ERR) return;
  ...
  if (jc) {
    ...
    if (utilityClass_ != NULL) {
      verifyMethodId_ = jni->GetStaticMethodID(
          (jclass)utilityClass_, "verifyMethodSignature",
          "(Ljava/lang/Class;Ljava/lang/String;Ljava/lang/String;III[Ljava/lang/String;[I[Ljava/lang/String;)V");
      createCLId_ = jni->GetStaticMethodID((jclass)utilityClass_, "createClassLoader",
                                           "(Ljava/lang/String;I)Lorg/trafodion/sql/udr/LmClassLoader;");
      utilityInitId_ = jni->GetStaticMethodID((jclass)utilityClass_, "init", "()V");
      classCacheSizeId_ = jni->GetStaticFieldID((jclass)utilityClass_, "classCacheSizeKB_", "I");
      classCacheEnforceId_ = jni->GetStaticFieldID((jclass)utilityClass_, "classCacheEnforceLimit_", "I");
      utilityGetRSInfoId_ = jni->GetStaticMethodID((jclass)utilityClass_, "getRSInfo",
                                                   "(Ljava/lang/Object;[J[J[Ljava/lang/Object;[I[Ljava/lang/String;)V");
      utilityInitRSId_ = jni->GetStaticMethodID((jclass)utilityClass_, "initRS", "([J)Z");
      utilityGetConnTypeId_ =
          jni->GetStaticMethodID((jclass)utilityClass_, "getConnectionType", "(Ljava/lang/Object;)I");
      utilityGetT4RSInfoId_ = jni->GetStaticMethodID(
          (jclass)utilityClass_, "getT4RSInfo", "(Ljava/lang/Object;[J[Z[I[Z[Ljava/lang/String;[Ljava/lang/Object;)V");
      utilityInitT4RSId_ = jni->GetStaticMethodID((jclass)utilityClass_, "initT4RS", "()Z");

      if (jni->ExceptionOccurred() || verifyMethodId_ == NULL || createCLId_ == NULL || classCacheSizeId_ == NULL ||
          classCacheEnforceId_ == NULL || utilityInitId_ == NULL || utilityGetRSInfoId_ == NULL ||
          utilityInitRSId_ == NULL || utilityGetConnTypeId_ == NULL || utilityGetT4RSInfoId_ == NULL ||
          utilityInitT4RSId_ == NULL) {
        exceptionReporter_->insertDiags(diagsArea_, -LME_JVM_SYS_CLASS_ERROR, "org.trafodion.sql.udr.LmUtility");
        result = LM_ERR;
        return;
      }
    }
  ...
  jni->CallStaticVoidMethod((jclass)utilityClass_, (jmethodID)utilityInitId_);
  if (jni->ExceptionOccurred()) {
    exceptionReporter_->insertDiags(diagsArea_, -LME_INTERNAL_ERROR,
  ...
  if (jc != NULL) {
    loaderClass_ = jc;
    loadClassId_ = jni->GetMethodID(jc, "loadClass", "(Ljava/lang/String;)Ljava/lang/Class;");

    if (jni->ExceptionOccurred() || loadClassId_ == NULL) {
      result = exceptionReporter_->insertDiags(diagsArea_, -LME_JVM_SYS_CLASS_ERROR, name);
      return;
    }
  } else {
    result = LM_ERR;
  ...
  if (jc) {
    ...
    if (lmCharsetClass_ != NULL) {
      bytesToUnicodeId_ = jni->GetStaticMethodID((jclass)lmCharsetClass_, "getUnicodeStringFromBytes",
                                                 "([BLjava/lang/String;II)Ljava/lang/String;");

      unicodeToBytesId_ = jni->GetStaticMethodID((jclass)lmCharsetClass_, "getBytesFromUnicodeString",
                                                 "(Ljava/lang/String;Ljava/lang/String;)[B");

      if (jni->ExceptionOccurred() || bytesToUnicodeId_ == NULL || unicodeToBytesId_ == NULL) {
        exceptionReporter_->insertDiags(diagsArea_, -LME_JVM_SYS_CLASS_ERROR, "org.trafodion.sql.udr.LmCharsetCoder");
        result = LM_ERR;
        return;
      }
    }
  ...
  if (jc) {
    ...
    if (lmObjMethodInvokeClass_ != NULL) {
      makeNewObjId_ =
          jni->GetStaticMethodID((jclass)lmObjMethodInvokeClass_, "makeNewObj",
                                 "(Lorg/trafodion/sql/udr/UDR;[B[B)Lorg/trafodion/sql/udr/LmUDRObjMethodInvoke;");

      setRuntimeInfoId_ =
          jni->GetMethodID((jclass)lmObjMethodInvokeClass_, "setRuntimeInfo", "(Ljava/lang/String;II)V");

      routineMethodInvokeId_ = jni->GetMethodID((jclass)lmObjMethodInvokeClass_, "invokeRoutineMethod",
                                                "(I[B[BI[B)Lorg/trafodion/sql/udr/LmUDRObjMethodInvoke$ReturnInfo;");

      if (jni->ExceptionOccurred() || makeNewObjId_ == NULL || setRuntimeInfoId_ == NULL ||
          routineMethodInvokeId_ == NULL) {
        exceptionReporter_->insertDiags(diagsArea_, -LME_JVM_SYS_CLASS_ERROR,
    ...
    exceptionReporter_->insertDiags(diagsArea_, -LME_JVM_SYS_CLASS_ERROR, "org.trafodion.sql.udr.LmUDRObjMethodInvoke");
    result = LM_ERR;
    return;
  }
  ...
  if (jc) {
    ...
    if (routineReturnInfoClass_ != NULL) {
      returnInfoStatusField_ = jni->GetFieldID((jclass)routineReturnInfoClass_, "returnStatus_", "I");
      returnInfoSQLStateField_ =
          jni->GetFieldID((jclass)routineReturnInfoClass_, "returnedSQLState_", "Ljava/lang/String;");
      returnInfoMessageField_ =
          jni->GetFieldID((jclass)routineReturnInfoClass_, "returnedMessage_", "Ljava/lang/String;");
      returnInfoRIIField_ = jni->GetFieldID((jclass)routineReturnInfoClass_, "returnedInvocationInfo_", "[B");
      returnInfoRPIField_ = jni->GetFieldID((jclass)routineReturnInfoClass_, "returnedPlanInfo_", "[B");

      if (jni->ExceptionOccurred() || returnInfoStatusField_ == NULL || returnInfoSQLStateField_ == NULL ||
          returnInfoMessageField_ == NULL || returnInfoRIIField_ == NULL || returnInfoRPIField_ == NULL) {
        exceptionReporter_->insertDiags(diagsArea_, -LME_JVM_SYS_CLASS_ERROR,
  ...
  if (jc) {
    ...
    exceptionReporter_->insertDiags(diagsArea_, -LME_JVM_SYS_CLASS_ERROR, "org.trafodion.sql.udr.UDR");
    result = LM_ERR;
    return;
  }
  ...
  if (jc) {
    udrQueueStateField_ = jni->GetFieldID(jc, "queueState_", "I");

    if (jni->ExceptionOccurred() || udrQueueStateField_ == NULL) {
      exceptionReporter_->insertDiags(diagsArea_, -LME_JVM_SYS_CLASS_ERROR, "org/trafodion/sql/udr/UDR$QueueStateInfo");
      result = LM_ERR;
      jni->DeleteLocalRef(jc);
      return;
    }
  ...
  if (jdbcMxT2Driver_) {
    ...
    jboolean jdbcRS = jni->CallStaticBooleanMethod((jclass)utilityClass_, (jmethodID)utilityInitRSId_, ver);
    ...
    LM_DEBUG2("jdbcSPJRSVer_ is %d, Supported version is %d", jdbcSPJRSVer_, JDBC_SPJRS_VERSION);
  }

  if (jdbcMxT4Driver_) {
    // Initialize stuff needed for processing T4 result sets
    jboolean t4jdbcSupportsRS = jni->CallStaticBooleanMethod((jclass)utilityClass_, (jmethodID)utilityInitT4RSId_);
...
#define LM_DRIVER_CLASS      "org.trafodion.sql.udr.LmT2Driver"
  jc = (jclass)loadSysClass(LM_DRIVER_CLASS_PATH, LM_DRIVER_CLASS, diagsArea_);
  if (jc != NULL) {
    lmDrvrClass_ = jc;
    driverInitId_ = jni->GetStaticMethodID(
        jc, "init", "(ZZZLjava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;I)V");

    if (jni->ExceptionOccurred() || driverInitId_ == NULL) {
      exceptionReporter_->insertDiags(diagsArea_, -LME_JVM_SYS_CLASS_ERROR, LM_DRIVER_CLASS);
      result = LM_ERR;
      return;
    }
  } else {
  ...
  if (userOptions) {
    ...
    foundCat = userOptions->getSystemProperty("catalog", &sysCatalog_, collHeap());
    if (foundCat) {
      LM_ASSERT(sysCatalog_);
    } else {
      foundCat = userOptions->getSystemProperty("jdbcmx.catalog", &sysCatalog_, collHeap());
      if (foundCat) LM_ASSERT(sysCatalog_);
    }

    foundSch = userOptions->getSystemProperty("schema", &sysSchema_, collHeap());
    if (foundSch) {
      LM_ASSERT(sysSchema_);
    } else {
      foundSch = userOptions->getSystemProperty("jdbcmx.schema", &sysSchema_, collHeap());
      if (foundSch) LM_ASSERT(sysSchema_);
    }
  }
  ...
  if (userName_ != NULL) {
    ...
    if (userPassword_ != NULL) userPassword = (jstring)jni->NewStringUTF(userPassword_);
  }
  ...
  if (retStatus) {
    ...
    *diagsArea_ << DgSqlCode(-LME_INTERNAL_ERROR) << DgString0(errStr);
    result = LM_ERR;
    return;
  }
  ...
  LM_DEBUG4(
      "About to init LmSQLMXDriver, setDefaultCatSchFlag_ is %s, enableType2Conn_ is %s, mapDefaultConnToType2Conn_ is "
      "%s, port number for Type 4 connections is %d",
      setDefaultCatSchFlag_ ? "TRUE" : "FALSE", enableType2Conn_ ? "TRUE" : "FALSE",
      mapDefaultConnToType2Conn_ ? "TRUE" : "FALSE", portNumber);

  jni->CallStaticVoidMethod((jclass)lmDrvrClass_, (jmethodID)driverInitId_, (setDefaultCatSchFlag_) ? 1 : 0,
                            (enableType2Conn_) ? 1 : 0, (mapDefaultConnToType2Conn_) ? 1 : 0, sysName, userName,
                            userPassword, dsName, portNumber);
  jni->DeleteLocalRef(sysName);
  jni->DeleteLocalRef(userName);
  if (userPassword) jni->DeleteLocalRef(userPassword);
  ...
  jint maxHeapKB = jni->GetStaticIntField((jclass)utilityClass_, (jfieldID)classCacheSizeId_);
  if (maxHeapKB < 1) {
    maxHeapKB = 1;
  }
 ...
 */
LmHandle LmLanguageManagerJava::loadSysClass(const char *clName, const char *name, ComDiagsArea *diagsArea) {
  JNIEnv *jni = (JNIEnv *)jniEnv_;
...
void LmLanguageManagerJava::unloadSysClass(LmHandle obj) {
  JNIEnv *jni = (JNIEnv *)jniEnv_;
  if (obj) jni->DeleteGlobalRef((jobject)obj);
}
...
LmResult LmLanguageManagerJava::initJavaClasses() {
  ...
  if (jc != NULL) {
    stringClass_ = jc;
    stringSubstringId_ = jni->GetMethodID(jc, "substring", "(II)Ljava/lang/String;");

    if (jni->ExceptionOccurred() || stringSubstringId_ == NULL) {
      return exceptionReporter_->insertDiags(diagsArea_, -LME_JVM_SYS_CLASS_ERROR, "java.lang.String");
    }
  } else {
    return LM_ERR;
  }
  ...
  if (jc != NULL) {
    systemClass_ = jc;
    systemGetPropId_ = jni->GetStaticMethodID(jc, "getProperty", "(Ljava/lang/String;)Ljava/lang/String;");

    systemSetPropId_ =
        jni->GetStaticMethodID(jc, "setProperty", "(Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;");

    systemClearPropId_ = jni->GetStaticMethodID(jc, "clearProperty", "(Ljava/lang/String;)Ljava/lang/String;");

    if (jni->ExceptionOccurred() || systemGetPropId_ == NULL || systemSetPropId_ == NULL ||
        systemClearPropId_ == NULL) {
      return exceptionReporter_->insertDiags(diagsArea_, -LME_JVM_SYS_CLASS_ERROR, "java.lang.System");
    }
  } else {
    return LM_ERR;
  }
  ...
  if (jc != NULL) {
    ...
    if (jni->ExceptionOccurred() || bigdecCtorId_ == NULL || bigdecStrId_ == NULL || bigdecUnscaleId_ == NULL) {
      return exceptionReporter_->insertDiags(diagsArea_, -LME_JVM_SYS_CLASS_ERROR, "java.math.BigDecimal");
    }
  } else {
    return LM_ERR;
  }
  ...
  if (jc != NULL) {
    ...
    if (jni->ExceptionOccurred() || bigintIntValueId_ == NULL) {
      return exceptionReporter_->insertDiags(diagsArea_, -LME_JVM_SYS_CLASS_ERROR, "java.math.BigInteger");
    }
  ...
  if (jc != NULL) {
    ...
    if (jni->ExceptionOccurred() || dateStrId_ == NULL || dateValId_ == NULL) {
      return exceptionReporter_->insertDiags(diagsArea_, -LME_JVM_SYS_CLASS_ERROR, "java.sql.Date");
    }
  } else {
    return LM_ERR;
  }
  ...
  if (jc != NULL) {
    ...
    if (jni->ExceptionOccurred() || timeStrId_ == NULL || timeValId_ == NULL) {
      return exceptionReporter_->insertDiags(diagsArea_, -LME_JVM_SYS_CLASS_ERROR, "java.sql.Time");
    }
  } else {
    return LM_ERR;
  }
  ...
  if (jc != NULL) {
    ...
    stampValId_ = jni->GetStaticMethodID(jc, "valueOf", "(Ljava/lang/String;)Ljava/sql/Timestamp;");

    if (stampStrId_ == NULL || stampValId_ == NULL) {
      return exceptionReporter_->insertDiags(diagsArea_, -LME_JVM_SYS_CLASS_ERROR, "java.sql.Timestamp");
    }
  } else {
    return LM_ERR;
  }
  ...
  if (jc != NULL) {
    ...
    if (jni->ExceptionOccurred() || intCtorId_ == NULL || intValId_ == NULL) {
      return exceptionReporter_->insertDiags(diagsArea_, -LME_JVM_SYS_CLASS_ERROR, "java.lang.Integer");
    }
  } else {
    return LM_ERR;
  }
  ...
  if (jc != NULL) {
    ...
    if (jni->ExceptionOccurred() || longCtorId_ == NULL || longValId_ == NULL) {
      return exceptionReporter_->insertDiags(diagsArea_, -LME_JVM_SYS_CLASS_ERROR, "java.lang.Long");
    }
  } else {
    return LM_ERR;
  }
  ...
  if (jc != NULL) {
    ...
    if (jni->ExceptionOccurred() || floatCtorId_ == NULL || floatValId_ == NULL) {
      return exceptionReporter_->insertDiags(diagsArea_, -LME_JVM_SYS_CLASS_ERROR, "java.lang.Float");
    }
  } else {
    return LM_ERR;
  }
  ...
  if (jc != NULL) {
    ...
    if (jni->ExceptionOccurred() || doubleCtorId_ == NULL || doubleValId_ == NULL) {
      return exceptionReporter_->insertDiags(diagsArea_, -LME_JVM_SYS_CLASS_ERROR, "java.lang.Double");
    }
  } else {
    return LM_ERR;
  }
  ...
  resultSetClass_ = loadSysClass("java/sql/ResultSet", "java.sql.ResultSet", diagsArea_);
  if (resultSetClass_ == NULL) return LM_ERR;

  rsCloseId_ = jni->GetMethodID((jclass)resultSetClass_, "close", "()V");
  rsBeforeFirstId_ = jni->GetMethodID((jclass)resultSetClass_, "beforeFirst", "()V");
  rsNextId_ = jni->GetMethodID((jclass)resultSetClass_, "next", "()Z");
  rsWasNullId_ = jni->GetMethodID((jclass)resultSetClass_, "wasNull", "()Z");
  rsGetWarningsId_ = jni->GetMethodID((jclass)resultSetClass_, "getWarnings", "()Ljava/sql/SQLWarning;");
  ...
  rsGetStringId_ = jni->GetMethodID((jclass)resultSetClass_, "getString", "(I)Ljava/lang/String;");
  rsGetObjectId_ = jni->GetMethodID((jclass)resultSetClass_, "getObject", "(I)Ljava/lang/Object;");

  rsGetDateId_ = jni->GetMethodID((jclass)resultSetClass_, "getDate", "(I)Ljava/sql/Date;");
  rsGetTimeId_ = jni->GetMethodID((jclass)resultSetClass_, "getTime", "(I)Ljava/sql/Time;");
  rsGetTimestampId_ = jni->GetMethodID((jclass)resultSetClass_, "getTimestamp", "(I)Ljava/sql/Timestamp;");
  if (jni->ExceptionOccurred() || rsCloseId_ == NULL || rsGetWarningsId_ == NULL || rsGetShortId_ == NULL ||
      rsGetIntId_ == NULL || rsGetLongId_ == NULL || rsGetFloatId_ == NULL || rsGetObjectId_ == NULL ||
      rsGetStringId_ == NULL || rsGetBigDecimalId_ == NULL || rsWasNullId_ == NULL || rsGetDateId_ == NULL ||
      rsGetTimeId_ == NULL || rsBeforeFirstId_ == NULL || rsNextId_ == NULL || rsGetDoubleId_ == NULL ||
      rsGetTimestampId_ == NULL) {
    return exceptionReporter_->insertDiags(diagsArea_, -LME_JVM_SYS_CLASS_ERROR, "java.sql.ResultSet");
  }

  // Load java.sql.Connection
  connClass_ = loadSysClass("java/sql/Connection", "java.sql.Connection", diagsArea_);
  if (connClass_ == NULL) return LM_ERR;

  connCloseId_ = jni->GetMethodID((jclass)connClass_, "close", "()V");
  connIsClosedId_ = jni->GetMethodID((jclass)connClass_, "isClosed", "()Z");
  if (jni->ExceptionOccurred() || connCloseId_ == NULL || connIsClosedId_ == NULL) {
    return exceptionReporter_->insertDiags(diagsArea_, -LME_JVM_SYS_CLASS_ERROR, "java.sql.Connection");
  }
  ...
  jdbcMxT2Driver_ =
      loadSysClass("org/apache/trafodion/jdbc/t2/T2Driver", "org.apache.trafodion.jdbc.t2.T2Driver", diagsArea_);
  if (jdbcMxT2Driver_ == NULL) return LM_ERR;
...
LmLanguageManagerJava::~LmLanguageManagerJava() {
  ...
  if (userName_) NADELETEBASIC(userName_, collHeap());

  if (userPassword_) NADELETEBASIC(userPassword_, collHeap());

  if (datasourceName_) NADELETEBASIC(datasourceName_, collHeap());

  if (sysCatalog_) NADELETEBASIC(sysCatalog_, collHeap());
  if (sysSchema_) NADELETEBASIC(sysSchema_, collHeap());
...
//////////////////////////////////////////////////////////////////////
LmResult LmLanguageManagerJava::validateRoutine(ComUInt32 numSqlParam, ComFSDataType paramType[],
                                                ComUInt32 paramSubType[], ComColumnDirection direction[],
                                                const char *routineName, const char *containerName,
                                                const char *externalPath, char *sigBuf, ComUInt32 sigLen,
                                                ComFSDataType resultType, ComUInt32 resultSubType,
                                                ComUInt32 numResultSets, const char *metaContainerName,
                                                const char *optionalSig, ComDiagsArea *diagsArea) {
  ...
  if (startService(da) == LM_ERR) return LM_ERR;

  // Get the requested container from the CM.
  result = contManager_->getContainer(containerName, externalPath, &container, da);

  if (result == LM_ERR) return LM_ERR;
  ...
  result = javaSig.createSig(paramType, paramSubType, direction, numSqlParam, resultType, resultSubType, numResultSets,
                             optionalSig, isUdrForJavaMain, sigBuf, sigLen, da);
  if (result == LM_ERR) return LM_ERR;
  ...
  if (midChk == LM_ERR) return LM_ERR;
  ...
  jni->CallStaticVoidMethodA((jclass)utilityClass_, (jmethodID)verifyMethodId_, jval);
  ...
  if (jni->ExceptionOccurred() != NULL) {
    ...
    result = exceptionReporter_->insertDiags(da, -LME_ROUTINE_NOT_FOUND, signature, containerName);
...
LmResult LmLanguageManagerJava::getRoutine(
    ComUInt32 numSqlParam, LmParameter parameters[], ComUInt32 numTableInfo, LmTableInfo tableInfo[],
    LmParameter *returnValue, ComRoutineParamStyle paramStyle, ComRoutineTransactionAttributes transactionAttrs,
  ...
  *handle = NULL;
  ...
  if (startService(da) == LM_ERR) result = LM_ERR;
  ...
  if (result != LM_ERR) {
    NABoolean isSPSQL = FALSE;
    if (str_cmp_ne(containerName, SEABASE_SPSQL_CONTAINER) == 0 &&
        str_cmp_ne(externalName, SEABASE_SPSQL_CONTAINER "." SEABASE_SPSQL_CALL_FUNC) == 0) {
      // SPSQL function in disguise
      isSPSQL = TRUE;
      externalName = SEABASE_SPSQL_CALL_FUNC;
      // FIXME: need to set this because we need to execute SQL for
      // initializing SPSQL.
      sqlAccessMode = COM_CONTAINS_SQL;
      routineSig = "(Ljava/lang/String;[Ljava/lang/Object;)V";
    }

    // Get the method ID from the container for the requested routine.
    if (str_cmp_ne(containerName, SEABASE_SPSQL_CONTAINER) == 0 && str_cmp_ne(externalName, SEABASE_SPSQL_CALL) == 0) {
      // SPSQL procedure in disguise
      isSPSQL = TRUE;
      routineSig = "(Ljava/lang/String;[Ljava/lang/Object;)V";
    }

    jmethodID jm = jni->GetStaticMethodID((jclass)container->getHandle(), externalName, routineSig);

    // Handle the result of the method ID look-up.
    if (jm != NULL) {
      // allocate an LM handle for the Java method.
      LmRoutineJava *h = new (collHeap()) LmRoutineJava(
          sqlName, externalName, librarySqlName, numSqlParam, returnValue, maxResultSets, (char *)routineSig, isSPSQL,
          COM_STYLE_JAVA_CALL, transactionAttrs, sqlAccessMode, externalSecurity, routineOwnerId, parentQid, clientInfo,
          inputRowLen, outputRowLen, currentUserName, sessionUserName, parameters, this, jm, container, da);
    ...
    {
      ComSInt32 size = 0;
      LmJavaSignature *lmSig = new (collHeap()) LmJavaSignature(routineSig, collHeap());
      ...
      result = exceptionReporter_->checkGetMethodExceptions(signature, containerName, da);
...
LmResult LmLanguageManagerJava::getObjRoutine(const char *serializedInvocationInfo, int invocationInfoLen,
                                              const char *serializedPlanInfo, int planInfoLen,
                                              ComRoutineLanguage language, ComRoutineParamStyle paramStyle,
                                              const char *externalName, const char *containerName,
  ...
  *handle = NULL;
  ...
  if (result != LM_ERR) {
    // Get the method ID from the container for the requested routine.
    jmethodID jm = jni->GetMethodID((jclass)container->getHandle(), externalName, "()V");
    ...
    if (jm != NULL) {
      ...
      if (newUDRObj == NULL || jni->ExceptionOccurred()) {
        if (diagsArea)
          exceptionReporter_->insertDiags(diagsArea, -LME_UDR_METHOD_ERROR, "constructor",
      ...
      if (invocationInfoLen > 0 && result == LM_OK) {
        ...
        try {
          invocationInfo->deserializeObj(serializedInvocationInfo, invocationInfoLen);
        } catch (...) {
        ...
        if (result == LM_OK) {
          ...
          LM_ASSERT(invocationInfo->sqlTransactionType_ == tmudr::UDRInvocationInfo::REQUIRES_NO_TRANSACTION);
          LM_ASSERT(invocationInfo->sqlAccessType_ == tmudr::UDRInvocationInfo::CONTAINS_NO_SQL);
          LM_ASSERT(invocationInfo->sqlRights_ == tmudr::UDRInvocationInfo::INVOKERS_RIGHTS);

          // allocate an LM handle for the Java method.
          LmRoutineJavaObj *h =
              new (collHeap()) LmRoutineJavaObj(invocationInfo->getUDRName().c_str(), externalName, librarySqlName,
                                                transactionAttrs, sqlAccessMode, externalSecurity,
                                                0,  // routine owner id is 0 for now
                                                serializedInvocationInfo, invocationInfoLen, serializedPlanInfo,
                                                planInfoLen, this, newUDRObj, container, da);
        ...
        if (invocationInfo) delete invocationInfo;
    ...
    {
      result = exceptionReporter_->checkGetMethodExceptions(externalName, containerName, da);
      result = LM_ERR;
    }
  }
  ...
  if (status != TRUE) {
    if (diagsArea)
      *diagsArea_ << DgSqlCode(-LME_INTERNAL_ERROR)
                  << DgString0(
                         ": Could not restore UDR trap signal handlers after returning from Java in getObjRoutine");
    result = LM_ERR;
  }
  ...
  if (result != LM_OK && container) contManager_->putContainer(container);
...
//////////////////////////////////////////////////////////////////////
LmResult LmLanguageManagerJava::putRoutine(LmRoutine *handle, ComDiagsArea *diagsArea) {
  ComDiagsArea *da = NULL;
  da = ((diagsArea != NULL) ? diagsArea : diagsArea_);
  ...
  if (h == NULL) {
    *da << DgSqlCode(-LME_INTERNAL_ERROR) << DgString0(": Invalid Routine Handle.");
    result = LM_ERR;
  } else {
    // De-ref the container.
...
//////////////////////////////////////////////////////////////////////
LmResult LmLanguageManagerJava::invokeRoutine(LmRoutine *handle, void *inputRow, void *outputRow,
                                              ComDiagsArea *diagsArea) {
  JNIEnv *jni = (JNIEnv *)jniEnv_;
  ComDiagsArea *da = NULL;
  da = ((diagsArea != NULL) ? diagsArea : diagsArea_);
  ...
  if (routine->getParamStyle() == COM_STYLE_JAVA_OBJ) {
    ...
    result = javaObjRoutine->invokeRoutineMethod(tmudr::UDRInvocationInfo::RUNTIME_WORK_CALL,
    ...
    if (result == LM_ERR) {
      ...
      if (jni->ExceptionOccurred() != NULL) {
        exceptionReporter_->insertDiags(da, -LME_INTERNAL_ERROR, ": There is a pending exception.");
    ...
    if (result != LM_ERR) {
      ...
      result = processOutParameters(routine, lmParams, outputRow, javaException, da);
  ...
  if (status != TRUE) {
    if (diagsArea)
      *diagsArea_ << DgSqlCode(-LME_INTERNAL_ERROR)
                  << DgString0(
                         ": Could not restore UDR trap signal handlers after returning from Java in invokeRoutine");
    return LM_ERR;
  }
 ...
 *          NULL   on FAILURE
 */
LmHandle LmLanguageManagerJava::createLoader(const char *externalPath, ComDiagsArea *da) {
  ComUInt32 i;
  JNIEnv *jni = (JNIEnv *)jniEnv_;
  jobject jobj;
  jvalue jval[2];
  ...
  if (midChk == LM_ERR) return NULL;
  jval[1].i = 0;

  // Create an LmClassLoader
  jobj = jni->CallStaticObjectMethodA((jclass)utilityClass_, (jmethodID)createCLId_, jval);
 ...
 */
void LmLanguageManagerJava::deleteLoader(LmHandle loader) { unloadSysClass((jobject)loader); }
 ...
 *          NULL on FAILURE
 */
LmHandle LmLanguageManagerJava::loadContainer(const char *containerName, const char *externalPath, LmHandle extLoader,
                                              ComUInt32 *containerSize, ComDiagsArea *da) {
  JNIEnv *jni = (JNIEnv *)jniEnv_;
  jobject jobj, container;
  jstring jstr;

  if (extLoader == NULL) {
    *da << DgSqlCode(-LME_INTERNAL_ERROR) << DgString0(": Invalid ClassLoader handle is passed to load a class file.");
    return NULL;
  }
  ...
  while (trycount > 0) {
    ...
    jobj = jni->CallObjectMethod((jobject)extLoader, (jmethodID)loadClassId_, jstr);
    if (jni->ExceptionOccurred()) {
      sleep(30);
      trycount--;
    } else
  ...
  if (jni->ExceptionOccurred() == NULL) {
    ...
    *containerSize = 100;  // currently not implemented,
    ...
    exceptionReporter_->insertDiags(da, -LME_CONT_NOT_FOUND, containerName, externalPath);
  }
...
void LmLanguageManagerJava::unloadContainer(LmHandle container) {
  if (container) ((JNIEnv *)jniEnv_)->DeleteGlobalRef((jobject)container);
}
...
//
LmResult LmLanguageManagerJava::processInParameters(LmRoutineJava *handle, LmParameter params[], void *inputRow,
                                                    ComDiagsArea *da) {
  JNIEnv *jni = (JNIEnv *)jniEnv_;
  ...
  for (int i = 0; i < (int)handle->numSqlParam_; i++) {
    ...
    void *thisParamDataPtr = (void *)((char *)inputRow + p->inDataOffset(inputRow));
    ...
    switch (LmJavaType(p).getType()) {
      case LmJavaType::JT_SHORT: {
        if (valIsNull) {
          *da << DgSqlCode(-LME_NULL_NOT_ALLOWED) << DgInt0(i + 1);
          return LM_ERR;
        }
      ...
      case LmJavaType::JT_LANG_STRING: {
        ...
        if (!valIsNull) {
          ...
          if (result == LM_ERR) return result;
        }
      ...
      default: {
        ...
        *da << DgSqlCode(-LME_INTERNAL_ERROR) << DgString0(errStr);
        return LM_ERR;
      }
    }  // Switch
 ...
 */
LmResult LmLanguageManagerJava::processOutParameters(LmRoutineJava *handle, LmParameter params[], void *outputRow,
                                                     NABoolean uncaughtExp, ComDiagsArea *da) {
  JNIEnv *jni = (JNIEnv *)jniEnv_;
  jvalue *jval = (jvalue *)handle->javaParams_;
  jobject jobj;
  LmResult result = LM_OK;
  ...
  if (!uncaughtExp) {
    for (i = 0; i < (int)handle->numSqlParam_ && result == LM_OK; i++) {
      ...
      switch (LmJavaType(p).getType()) {
        case LmJavaType::JT_SHORT: {
          jni->GetShortArrayRegion((jshortArray)jval[i].l, 0, 1, (jshort *)thisParamDataPtr);
          LM_ASSERT(jni->ExceptionOccurred() == NULL);
        } break;

        case LmJavaType::JT_INT: {
          jni->GetIntArrayRegion((jintArray)jval[i].l, 0, 1, (jint *)thisParamDataPtr);
          LM_ASSERT(jni->ExceptionOccurred() == NULL);
        } break;

        case LmJavaType::JT_LONG: {
          jni->GetLongArrayRegion((jlongArray)jval[i].l, 0, 1, (jlong *)thisParamDataPtr);
          LM_ASSERT(jni->ExceptionOccurred() == NULL);
        } break;

        case LmJavaType::JT_FLOAT: {
          jni->GetFloatArrayRegion((jfloatArray)jval[i].l, 0, 1, (jfloat *)thisParamDataPtr);
          LM_ASSERT(jni->ExceptionOccurred() == NULL);
        } break;

        case LmJavaType::JT_DOUBLE: {
          jni->GetDoubleArrayRegion((jdoubleArray)jval[i].l, 0, 1, (jdouble *)thisParamDataPtr);
          LM_ASSERT(jni->ExceptionOccurred() == NULL);
        } break;
  ...
  if (handle->maxResultSets_ > 0) {
    for (i = (int)handle->numSqlParam_; i < (int)handle->numParamsInSig_; i++) {
      ...
      if (jobj != NULL && enableType2Conn_ && (!jdbcSupportsRS_ || jdbcRSVerMismatch())) {
        if (!jdbcSupportsRS_)
 ...
 */
void LmLanguageManagerJava::processParametersDone(LmRoutineJava *handle, LmParameter params[]) {
  JNIEnv *jni = (JNIEnv *)jniEnv_;
  jvalue *jval = (jvalue *)handle->javaParams_;
  ComBoolean isUdrForJavaMain = handle->isUdrForJavaMain();
 ...
 */
LmResult LmLanguageManagerJava::convertToString(LmParameter *param, void *inputRow, LmHandle *obj, ComDiagsArea *da) {
  JNIEnv *jni = (JNIEnv *)jniEnv_;
  jobject jobj;

  ComUInt32 len = param->actualInDataSize(inputRow);

  char *inputChars = ((char *)inputRow) + param->inDataOffset(inputRow);
  ...
  if (result == LM_ERR) {
    ...
    if (singleCharWorkAround) NADELETEBASIC(inputChars, collHeap());
  ...
  jobj = (jstring)jni->CallStaticObjectMethod((jclass)lmCharsetClass_, (jmethodID)bytesToUnicodeId_, javaBytes,
                                              inputCharset, (jint)0, (jint)len);
  ...
  if (singleCharWorkAround) jobj = jni->CallObjectMethod(jobj, (jmethodID)stringSubstringId_, 0, 1);

  *obj = jobj;
  return LM_OK;
}

LmResult LmLanguageManagerJava::convertFromString(LmParameter *param, void *outputRow, LmHandle obj, ComDiagsArea *da) {
  JNIEnv *jni = (JNIEnv *)jniEnv_;
  ...
  javaBytesArray = (jbyteArray)jni->CallStaticObjectMethod((jclass)lmCharsetClass_, (jmethodID)unicodeToBytesId_,
...
LmResult LmLanguageManagerJava::convertFromInterval(LmParameter *param, void *outputRow, LmHandle obj,
                                                    ComDiagsArea *da) {
  JNIEnv *jni = (JNIEnv *)jniEnv_;
  ...
  javaBytesArray = (jbyteArray)jni->CallStaticObjectMethod((jclass)lmCharsetClass_, (jmethodID)unicodeToBytesId_, jstr,
                                                           outputCharset);
  jni->DeleteLocalRef(outputCharset);
...
LmResult LmLanguageManagerJava::convertToBigdec(LmParameter *param, void *inputRow, LmHandle *obj, ComDiagsArea *da) {
  jobject jobj;
  JNIEnv *jni = (JNIEnv *)jniEnv_;
  LmResult result = LM_OK;
  char tempbuf[131];
...
LmResult LmLanguageManagerJava::convertFromBigdec(LmParameter *param, void *outputRow, LmHandle obj,
                                                  ComBoolean resultSet_decimal,  // useful to decide setting Numeric or
                                                                                 // Decimal type. The default value
                                                                                 // FALSE will set Numeric type.
...
LmResult LmLanguageManagerJava::convertToDate(LmParameter *param, void *inputRow, LmHandle *obj, ComDiagsArea *da) {
  jobject jobj;
  JNIEnv *jni = (JNIEnv *)jniEnv_;
  jstring jstr;
  LmResult result = LM_OK;
  ...
  jobj = jni->CallStaticObjectMethod((jclass)dateClass_, (jmethodID)dateValId_, jstr);
...
LmResult LmLanguageManagerJava::convertFromDate(LmParameter *param, void *outputRow, LmHandle obj, ComDiagsArea *da) {
  JNIEnv *jni = (JNIEnv *)jniEnv_;
  jobject jobj = (jobject)obj;
  jstring jstr;
  LmResult result = LM_OK;
...
LmResult LmLanguageManagerJava::convertToTime(LmParameter *param, void *inputRow, LmHandle *obj, ComDiagsArea *da) {
  jobject jobj;
  JNIEnv *jni = (JNIEnv *)jniEnv_;
  LmResult result = LM_OK;
  char tempbuf[64];
  ...
  jobj = jni->CallStaticObjectMethod((jclass)timeClass_, (jmethodID)timeValId_, jstr);
  result = exceptionReporter_->checkJVMException(da);
...
LmResult LmLanguageManagerJava::convertFromTime(LmParameter *param, void *outputRow, LmHandle obj, ComDiagsArea *da) {
  JNIEnv *jni = (JNIEnv *)jniEnv_;
  jobject jobj = (jobject)obj;
  jstring jstr;
  LmResult result = LM_OK;
...
LmResult LmLanguageManagerJava::convertToTimestamp(LmParameter *param, void *inputRow, LmHandle *obj,
                                                   ComDiagsArea *da) {
  jobject jobj;
  JNIEnv *jni = (JNIEnv *)jniEnv_;
  LmResult result = LM_OK;
  ...
  jobj = jni->CallStaticObjectMethod((jclass)stampClass_, (jmethodID)stampValId_, jstr);
  result = exceptionReporter_->checkJVMException(da);
...
LmResult LmLanguageManagerJava::convertFromTimestamp(LmParameter *param, void *outputRow, LmHandle obj,
                                                     ComDiagsArea *da) {
  JNIEnv *jni = (JNIEnv *)jniEnv_;
  jobject jobj = (jobject)obj;
  jstring jstr;
...
LmResult LmLanguageManagerJava::convertToInteger(LmParameter *param, void *inputRow, LmHandle *obj, ComDiagsArea *da) {
  jobject jobj;
  JNIEnv *jni = (JNIEnv *)jniEnv_;
  LmResult result = LM_OK;

  jint val = *(jint *)((char *)inputRow + param->inDataOffset(inputRow));
...
LmResult LmLanguageManagerJava::convertFromInteger(LmParameter *param, void *outputRow, LmHandle obj) {
  jobject jobj = (jobject)obj;
  JNIEnv *jni = (JNIEnv *)jniEnv_;
  LmResult result = LM_OK;
...
LmResult LmLanguageManagerJava::convertToLong(LmParameter *param, void *inputRow, LmHandle *obj, ComDiagsArea *da) {
  jobject jobj;
  JNIEnv *jni = (JNIEnv *)jniEnv_;
  LmResult result = LM_OK;

  jlong val = *(jlong *)((char *)inputRow + param->inDataOffset(inputRow));
...
LmResult LmLanguageManagerJava::convertFromLong(LmParameter *param, void *outputRow, LmHandle obj) {
  jobject jobj = (jobject)obj;
  JNIEnv *jni = (JNIEnv *)jniEnv_;
  LmResult result = LM_OK;
...
LmResult LmLanguageManagerJava::convertToFloat(LmParameter *param, void *inputRow, LmHandle *obj, ComDiagsArea *da) {
  jobject jobj;
  JNIEnv *jni = (JNIEnv *)jniEnv_;
  LmResult result = LM_OK;

  jfloat val = *(jfloat *)((char *)inputRow + param->inDataOffset(inputRow));

  jvalue args[1];
  args[0].f = val;
  jobj = jni->NewObjectA((jclass)floatClass_, (jmethodID)floatCtorId_, args);
  result = exceptionReporter_->checkNewObjectExceptions(jobj, da);
...
LmResult LmLanguageManagerJava::convertFromFloat(LmParameter *param, void *outputRow, LmHandle obj, ComDiagsArea *da) {
  jobject jobj = (jobject)obj;
  JNIEnv *jni = (JNIEnv *)jniEnv_;
  LmResult result = LM_OK;
...
LmResult LmLanguageManagerJava::convertToDouble(LmParameter *param, void *inputRow, LmHandle *obj, ComDiagsArea *da) {
  jobject jobj;
  JNIEnv *jni = (JNIEnv *)jniEnv_;
  LmResult result = LM_OK;

  jdouble val = *(jdouble *)((char *)inputRow + param->inDataOffset(inputRow));

  jvalue args[1];
  args[0].d = val;
  jobj = jni->NewObjectA((jclass)doubleClass_, (jmethodID)doubleCtorId_, args);
  result = exceptionReporter_->checkNewObjectExceptions(jobj, da);
  *obj = jobj;
  return result;
}

LmResult LmLanguageManagerJava::convertFromDouble(LmParameter *param, void *outputRow, LmHandle obj, ComDiagsArea *da) {
  jobject jobj = (jobject)obj;
  JNIEnv *jni = (JNIEnv *)jniEnv_;
  LmResult result = LM_OK;
...
LmResult LmLanguageManagerJava::startService(ComDiagsArea *da) {
  if (LmSqlJVM != NULL && jniEnv_ != NULL && threadId_ == threadId())
    ...
    *da << DgSqlCode(-LME_INTERNAL_ERROR) << DgString0(": Language Manager is not started yet.");
    return LM_ERR;
  }
}
 ...
 */
ComSInt32 LmLanguageManagerJava::threadId() { return GetCurrThreadId(); }
 ...
 */
LmResult LmLanguageManagerJava::getSystemProperty(const char *key, char *value, ComUInt32 bufferLen,
                                                  ComBoolean &propertyIsSet, ComDiagsArea *diagsArea) {
  JNIEnv *jni = (JNIEnv *)jniEnv_;
  ...
  jstrOut = (jstring)jni->CallStaticObjectMethod((jclass)systemClass_, (jmethodID)systemGetPropId_, jstrIn);
  jni->DeleteLocalRef(jstrIn);
  if (exceptionReporter_->checkJVMException(da, 0) == LM_ERR) return LM_ERR;

  if (jstrOut == NULL) return LM_OK;

  cstrOut = jni->GetStringUTFChars(jstrOut, NULL);
  if (cstrOut == NULL) {
    *da << DgSqlCode(-LME_INTERNAL_ERROR) << DgString0(" while getting system property.");
 ...
 */
LmResult LmLanguageManagerJava::setSystemProperty(const char *key, const char *value, ComDiagsArea *diagsArea) {
  JNIEnv *jni = (JNIEnv *)jniEnv_;
  ...
  if (midChk == LM_ERR) {
    if (jstrKey) jni->DeleteLocalRef(jstrKey);
    return LM_ERR;
  }
  ...
  if (midChk == LM_ERR) {
    ...
    if (jstrVal) jni->DeleteLocalRef(jstrVal);
    return LM_ERR;
  }

  jstrResult =
      (jstring)jni->CallStaticObjectMethod((jclass)systemClass_, (jmethodID)systemSetPropId_, jstrKey, jstrVal);
 ...
 */
LmResult LmLanguageManagerJava::clearSystemProperty(const char *key, ComDiagsArea *diagsArea) {
  JNIEnv *jni = (JNIEnv *)jniEnv_;
  ...
  jstrKey = jni->NewStringUTF(key);
  if (jstrKey == NULL) return LM_ERR;

  jstrResult = (jstring)jni->CallStaticObjectMethod((jclass)systemClass_, (jmethodID)systemClearPropId_, jstrKey);
...
void LmLanguageManagerJava::processJavaOptions(const LmJavaOptions &userOptions, LmJavaOptions &jvmOptions) {
  ...
  jvmOptions.removeSystemProperty("sqlmx.datasource", &datasourceName_, collHeap());
  ...
  NABoolean foundJreHome = jvmOptions.removeSystemProperty("sqlmx.udr.jrehome", &userJreHomeValue, collHeap());
  if (foundJreHome) {
    LM_ASSERT(userJreHomeValue);
    const char *envPrefix = "JREHOME=";

    char *envString = new (collHeap()) char[str_len(envPrefix) + str_len(userJreHomeValue) + 10];
  ...
  jvmOptions.addSystemProperty("sqlj.defaultconnection", "jdbc:default:connection");
