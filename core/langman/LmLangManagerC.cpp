...
LmLanguageManagerC::LmLanguageManagerC(LmResult &result, NABoolean commandLineMode, ComDiagsArea *diagsArea)
    : LmLanguageManager(commandLineMode), diagsArea_(diagsArea) {
  setRoutineIsActive(FALSE);
  contManager_ = new (collHeap()) LmContainerManagerSimple(this);
  result = LM_OK;
}

LmLanguageManagerC::~LmLanguageManagerC() { delete contManager_; }

LmResult LmLanguageManagerC::validateRoutine(ComUInt32 numSqlParam, ComFSDataType paramType[], ComUInt32 paramSubType[],
                                             ComColumnDirection direction[], const char *routineName,
                                             const char *containerName, const char *externalPath, char *sigBuf,
                                             ComUInt32 sigLen, ComFSDataType resultType, ComUInt32 resultSubType,
...
LmResult LmLanguageManagerC::getRoutine(
  ...
  *handle = NULL;
  ...
  if (paramStyle != COM_STYLE_CPP_OBJ) {
    ...
    if (routinePtr == NULL) {
      char *libraryName = new (collHeap()) char[str_len(externalPath) + str_len(containerName) + 2];
      sprintf(libraryName, "%s/%s", externalPath, containerName);

      *da << DgSqlCode(-LME_DLL_METHOD_NOT_FOUND) << DgString0(externalName) << DgString1(libraryName);
  ...
  if (paramStyle == COM_STYLE_SQL) {
    routineHandle = new (collHeap())
        LmRoutineCSql(sqlName, externalName, librarySqlName, numSqlParam, (char *)routineSig, maxResultSets,
                      transactionAttrs, sqlAccessMode, externalSecurity, routineOwnerId, parentQid, inputRowLen,
                      outputRowLen, currentUserName, sessionUserName, parameters, this, routinePtr, container, da);
  } else if (paramStyle == COM_STYLE_SQLROW) {
    routineHandle = new (collHeap())
        LmRoutineCSqlRow(sqlName, externalName, librarySqlName, numSqlParam, (char *)routineSig, maxResultSets,
                         transactionAttrs, sqlAccessMode, externalSecurity, routineOwnerId, parentQid, inputRowLen,
                         outputRowLen, currentUserName, sessionUserName, parameters, this, routinePtr, container, da);
  } else if (paramStyle == COM_STYLE_SQLROW_TM) {
    routineHandle = new (collHeap()) LmRoutineCSqlRowTM(
        sqlName, externalName, librarySqlName, numSqlParam, numTableInfo, tableInfo, (char *)routineSig, maxResultSets,
        transactionAttrs, sqlAccessMode, externalSecurity, routineOwnerId, parentQid, inputRowLen, outputRowLen,
        currentUserName, sessionUserName, parameters, this, routinePtr, getNextRowPtr, emitRowPtr, container, da);
  } else if (paramStyle == COM_STYLE_CPP_OBJ) {
    // need to call getObjRoutine for this parameter style
    LM_ASSERT(0);
  } else {
    // XXX LM_ASSERT(0);
    char *paramStyleMsg = new (collHeap()) char[100];
    sprintf(paramStyleMsg, "Unknown ParameterStyle(%d)", paramStyle);

    *da << DgSqlCode(-LME_VALIDATION_FAILED) << DgString0(externalName) << DgString1(paramStyleMsg);
...
LmResult LmLanguageManagerC::getObjRoutine(const char *serializedInvocationInfo, int serializedInvocationInfoLen,
                                           const char *serializedPlanInfo, int serializedPlanInfoLen,
  ...
  *handle = NULL;
  ...
  if (factoryMethodPtr == NULL) {
    *da << DgSqlCode(-LME_DLL_METHOD_NOT_FOUND) << DgString0(externalName) << DgString1(containerName);
  ...
  try {
    if (serializedInvocationInfoLen > 0) {
      ...
      invocationInfo->deserializeObj(serializedInvocationInfo, serializedInvocationInfoLen);
    }

    if (serializedPlanInfoLen > 0) {
      planInfo = new tmudr::UDRPlanInfo(invocationInfo, 0);
      planInfo->deserializeObj(serializedPlanInfo, serializedPlanInfoLen);
    }

    tmudr::CreateInterfaceObjectFunc fPtr = reinterpret_cast<tmudr::CreateInterfaceObjectFunc>(factoryMethodPtr);

    // call the factory method
    interfacePtr = (*fPtr)();

    if (interfacePtr == NULL) {
      *da << DgSqlCode(-LME_FACTORY_METHOD) << DgString0(externalName) << DgString1(containerName)
  ...
  if (result == LM_OK && invocationInfo != NULL) {
    ...
    *handle = new (collHeap())
        LmRoutineCppObj(invocationInfo, planInfo, interfacePtr, invocationInfo->getUDRName().c_str(), externalName,
                        librarySqlName, 0, transactionAttrs, sqlAccessMode, externalSecurity,
...
LmResult LmLanguageManagerC::putRoutine(LmRoutine *routine, ComDiagsArea *diagsArea) {
  if (routine == NULL) return LM_OK;

  // make a final call or the equivalent, if needed
  LmResult result = routine->handleFinalCall();

  // De-ref the container.
  if (routine->container()) contManager_->putContainer(routine->container());
...
LmResult LmLanguageManagerC::invokeRoutine(LmRoutine *handle, void *inputRow, void *outputRow,
  ...
  if (routine->getParamStyle() == COM_STYLE_CPP_OBJ) {
    ...
    result = cppRoutine->invokeRoutineMethod(tmudr::UDRInvocationInfo::RUNTIME_WORK_CALL,
                                             ...
                                             (char *)inputRow, invocationInfo->par().getRecordLength(),
                                             (char *)outputRow, invocationInfo->out().getRecordLength(), diagsArea);
  } else
...
LmHandle LmLanguageManagerC::createLoader(const char *externalPath, ComDiagsArea *da) {
...
LmHandle LmLanguageManagerC::loadContainer(const char *containerName, const char *externalPath, LmHandle extLoader,
                                           ComUInt32 *containerSize, ComDiagsArea *da) {
  return loadDll(containerName, externalPath, extLoader, containerSize, da, collHeap());
}

void LmLanguageManagerC::unloadContainer(LmHandle containerHandle) { unloadDll(containerHandle, diagsArea_); }

LmResult LmLanguageManagerC::getSystemProperty(const char *key, char *value, ComUInt32 bufferLen,
...
LmResult LmLanguageManagerC::setSystemProperty(const char *key, const char *value, ComDiagsArea *diagsArea) {
