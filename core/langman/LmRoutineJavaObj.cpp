...
LmResult LmRoutineJavaObj::setRuntimeInfo(const char *qid, int totalNumInstances, int myInstanceNum, ComDiagsArea *da) {
  ...
  jni->CallVoidMethod(udrInvokeObjectG_, (jmethodID)getLM()->setRuntimeInfoId_, jqid, (jint)totalNumInstances,
                      (jint)myInstanceNum);
  ...
  if (jni->ExceptionOccurred()) {
    ...
    if (da) {
      ...
      *da << DgSqlCode(-LME_OBJECT_INTERFACE_ERROR) << DgString0(getNameForDiags()) << DgString1("runtime")
          << DgString2("LmRoutineJavaObj::setRuntimeInfo()") << DgString3("Exception in JNI call");
    }
    result = LM_ERR;
  }
...
LmResult LmRoutineJavaObj::invokeRoutineMethod(
  ...
  LmResult result = LM_OK;
  JNIEnv *jni = (JNIEnv *)getLM()->jniEnv_;
  if (inputRowLen >= 0) inputRowLen_ = inputRowLen;
  if (outputRowLen >= 0) outputRowLen_ = outputRowLen;
  ...
  jni->SetByteArrayRegion(jii, (jsize)0, (jsize)invocationInfoLen, (const jbyte *)serializedInvocationInfo);
  jni->SetByteArrayRegion(jpi, (jsize)0, (jsize)planInfoLen, (const jbyte *)serializedPlanInfo);
  jni->SetByteArrayRegion(jir, (jsize)0, (jsize)inputRowLen_, (const jbyte *)inputRow);

  // call the Java method that will ultimately call the user code
  jobject jniResult = jni->CallObjectMethod(udrInvokeObjectG_, (jmethodID)getLM()->routineMethodInvokeId_, (jint)phase,
                                            jii, jpi, (jint)planNum, jir);
  ...
  if (jniResult == NULL || jni->ExceptionOccurred()) {
    ...
    if (da && da->mainSQLCODE() >= 0) {
      ...
      *da << DgSqlCode(-LME_OBJECT_INTERFACE_ERROR) << DgString0(getNameForDiags()) << DgString1(buf)
          << DgString2("LmRoutineJavaObj::invokeRoutineMethod()") << DgString3("JNI call returned a NULL object");
    }
    result = LM_ERR;
  }
  ...
  if (result == LM_OK) {
    ...
    jint returnStatus = jni->GetIntField(jniResult, (jfieldID)getLM()->returnInfoStatusField_);

    if (returnStatus != 0) {
      getLM()->exceptionReporter_->processJavaObjException(jniResult, returnStatus, static_cast<int>(phase),
                                                           "LmRoutineJavaObj::invokeRoutineMethod()", getNameForDiags(),
                                                           da);
      result = LM_ERR;
    } else {
      returnedIIL_ = static_cast<jbyteArray>(jni->GetObjectField(jniResult, (jfieldID)getLM()->returnInfoRIIField_));
      returnedPIL_ = static_cast<jbyteArray>(jni->GetObjectField(jniResult, (jfieldID)getLM()->returnInfoRPIField_));

      if (returnedIIL_) iiLen_ = jni->GetArrayLength(returnedIIL_);

      if (returnedPIL_) piLen_ = jni->GetArrayLength(returnedPIL_);
    }

    if (phase == tmudr::UDRInvocationInfo::RUNTIME_WORK_CALL && result == LM_OK) {
...
LmResult LmRoutineJavaObj::getRoutineInvocationInfo(
  ...
  if (returnedIIL_) {
    if (invocationInfoMaxLen < iiLen_ && serializedInvocationInfo) {
      if (da)
        *da << DgSqlCode(-LME_OBJECT_INTERFACE_ERROR) << DgString0(getNameForDiags()) << DgString1("unknown")
            << DgString2("getRoutineInvocationInfo()") << DgString3("Buffer too short for invocation info");
      iiLen_ = 0;
      result = LM_ERR;
    } else if (serializedInvocationInfo) {
      jbyte *returnedIIChar = jni->GetByteArrayElements(returnedIIL_, NULL);
      str_cpy_all(serializedInvocationInfo, reinterpret_cast<const char *>(returnedIIChar), iiLen_);
  ...
  if (invocationInfoLenOut) *invocationInfoLenOut = iiLen_;
  iiLen_ = 0;
  ...
  if (returnedPIL_) {
    if (planInfoMaxLen < piLen_ && serializedPlanInfo) {
      if (da)
        *da << DgSqlCode(-LME_OBJECT_INTERFACE_ERROR) << DgString0(getNameForDiags()) << DgString1("unknown")
            << DgString2("getRoutineInvocationInfo()") << DgString3("Buffer too short for plan info");
      piLen_ = 0;
      result = LM_ERR;
    } else if (serializedPlanInfo) {
      jbyte *returnedPIChar = jni->GetByteArrayElements(returnedPIL_, NULL);
      str_cpy_all(serializedPlanInfo, reinterpret_cast<const char *>(returnedPIChar), piLen_);
  ...
  if (planInfoLenOut) *planInfoLenOut = piLen_;
  piLen_ = 0;

  return result;
}

LmResult LmRoutineJavaObj::setFunctionPtrs(SQLUDR_GetNextRow getNextRowPtr, SQLUDR_EmitRow emitRowPtr,
                                           ComDiagsArea *da) {
  JNIEnv *jni = (JNIEnv *)getLM()->jniEnv_;
  LmResult result = LM_OK;
  ...
  const JNINativeMethod methods[2] = {
      ...
      {const_cast<char *>("SpInfoEmitRowJava"), const_cast<char *>("([BILorg/trafodion/sql/udr/UDR$QueueStateInfo;)V"),
  ...
  jint returnStatus = jni->RegisterNatives(reinterpret_cast<jclass>(getLM()->udrClass_), methods,
                                           static_cast<jint>(sizeof(methods) / sizeof(methods[0])));

  if (returnStatus != 0 || jni->ExceptionOccurred()) {
    getLM()->exceptionReporter_->insertDiags(da, -LME_INTERNAL_ERROR, ": Error in LmRoutineJavaObj::setFunctionPtrs()");

    if (da) {
      char buf2[80];
      snprintf(buf2, sizeof(buf2), "Return status %d from JNI call", returnStatus);

      *da << DgSqlCode(-LME_OBJECT_INTERFACE_ERROR) << DgString0(getNameForDiags()) << DgString1("runtime initial")
          << DgString2("LmRoutineJavaObj::setFunctionPtrs()") << DgString3(buf2);
    }
    result = LM_ERR;
  }
...
void LmRoutineJavaObj::setJNIFunctionPtrs(void *jniGetNextRowPtr, void *jniEmitRowPtr) {
  jniGetNextRowPtr_ = jniGetNextRowPtr;
  jniEmitRowPtr_ = jniEmitRowPtr;
}

void LmRoutineJavaObj::setRowLengths(int inputRowLen, int outputRowLen) {
...
LmRoutineJavaObj::LmRoutineJavaObj(const char *sqlName, const char *externalName, const char *librarySqlName,
    ...
    : LmRoutineJava(sqlName, externalName, librarySqlName, 0, NULL, 0, NULL,
                    FALSE,  // udrForSPSQL
                    COM_STYLE_JAVA_OBJ, transactionAttrs, sqlAccessMode, externalSecurity, routineOwnerId, NULL, NULL,
                    0, 0, NULL, NULL, NULL, lm, (LmHandle)NULL, container, diagsArea),
      udrObjectG_(NULL),
  ...
  if (!jni->ExceptionOccurred())
    ...
    udrInvokeObject = jni->CallStaticObjectMethod((jclass)getLM()->lmObjMethodInvokeClass_,
                                                  (jmethodID)getLM()->makeNewObjId_, udrObject, jii, jpi);

  jni->DeleteLocalRef(jii);
  if (planInfoLen > 0) jni->DeleteLocalRef(jpi);
  ...
  if (udrObjectG_ == NULL || udrInvokeObjectG_ == NULL || jni->ExceptionOccurred()) {
    getLM()->exceptionReporter_->insertDiags(diagsArea, -LME_INTERNAL_ERROR,
...
LmRoutineJavaObj::~LmRoutineJavaObj() {
  ...
  if (udrObjectG_ != NULL) jni->DeleteGlobalRef(udrObjectG_);
  if (udrInvokeObjectG_ != NULL) jni->DeleteGlobalRef(udrInvokeObjectG_);
  if (returnedIIL_) jni->DeleteLocalRef(returnedIIL_);
  if (returnedPIL_) jni->DeleteLocalRef(returnedPIL_);
}

ComBoolean LmRoutineJavaObj::isValid() const { return udrObjectG_ != NULL && udrInvokeObjectG_ != NULL; }
