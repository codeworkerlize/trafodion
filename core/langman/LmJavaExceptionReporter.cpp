...
LmJavaExceptionReporter::LmJavaExceptionReporter(LmHandle jniEnv, LmLanguageManagerJava *langMan, LmResult &result,
  ...
  if (loadThrowable(diagsArea) == LM_ERR || loadSQLException(diagsArea) == LM_ERR ||
...
LmJavaExceptionReporter::~LmJavaExceptionReporter() {
  ...
  if (throwableClass_) jni->DeleteGlobalRef((jobject)throwableClass_);

  if (exSQLClass_) jni->DeleteGlobalRef((jobject)exSQLClass_);

  if (exMetValFailedClass_) jni->DeleteGlobalRef((jobject)exMetValFailedClass_);
}

//
// loadThrowable: loads java.lang.Throwable and some of its methods.
//
LmResult LmJavaExceptionReporter::loadThrowable(ComDiagsArea *diagsArea) {
  JNIEnv *jni = (JNIEnv *)jniEnv_;
  jclass jc = NULL;
  ...
  if (jc != NULL) {
    ...
    if (throwableClass_ != NULL) {
      throwableToStringId_ = jni->GetMethodID((jclass)throwableClass_, "toString", "()Ljava/lang/String;");
      throwableGetCauseId_ = jni->GetMethodID((jclass)throwableClass_, "getCause", "()Ljava/lang/Throwable;");
    }
  }
  ...
  if (throwableClass_ == NULL || throwableToStringId_ == NULL) {
    *diagsArea << DgSqlCode(-LME_JVM_SYS_CLASS_ERROR) << DgString0("java.lang.Throwable");
    return LM_ERR;
  }
...
//
LmResult LmJavaExceptionReporter::loadSQLException(ComDiagsArea *diagsArea) {
  jclass jc = NULL;
  JNIEnv *jni = (JNIEnv *)jniEnv_;
  ...
  if (jc != NULL) {
    ...
    if (exSQLClass_ != NULL) {
      exSQLStateId_ = jni->GetMethodID((jclass)exSQLClass_, "getSQLState", "()Ljava/lang/String;");
      exErrorCodeId_ = jni->GetMethodID((jclass)exSQLClass_, "getErrorCode", "()I");
      exNextExceptionId_ = jni->GetMethodID((jclass)exSQLClass_, "getNextException", "()Ljava/sql/SQLException;");
    }
  }

  if (exSQLClass_ == NULL || exSQLStateId_ == NULL || exErrorCodeId_ == NULL || exNextExceptionId_ == NULL ||
...
//
LmResult LmJavaExceptionReporter::loadMethodValidationFailedException(ComDiagsArea *diagsArea) {
  jclass jc = NULL;
  JNIEnv *jni = (JNIEnv *)jniEnv_;

  jc = (jclass)jni->FindClass("org/trafodion/sql/udr/MethodValidationFailedException");
  ...
  if (jc != NULL) {
    ...
    if (exMetValFailedClass_ != NULL) {
      exGetMethodName_ = jni->GetMethodID((jclass)exMetValFailedClass_, "getMethodName", "()Ljava/lang/String;");
      exGetSignature_ = jni->GetMethodID((jclass)exMetValFailedClass_, "getMethodSignature", "()Ljava/lang/String;");
    }
  }

  if (exGetMethodName_ == NULL || exGetSignature_ == NULL || jni->ExceptionOccurred()) {
    insertDiags(diagsArea, -LME_JVM_SYS_CLASS_ERROR, "org.trafodion.sql.udr.MethodValidationFailedException");
    return LM_ERR;
  }
...
//
LmResult LmJavaExceptionReporter::checkJVMException(ComDiagsArea *da, LmHandle exception) {
  JNIEnv *jni = (JNIEnv *)jniEnv_;
  jthrowable jex = NULL;
...
//          LM_ERR otherwise
//
LmResult LmJavaExceptionReporter::checkNewObjectExceptions(LmHandle jobj, ComDiagsArea *da) {
  JNIEnv *jni = (JNIEnv *)jniEnv_;
  ...
  if (jt == NULL) {
    ...
    jstring jstr = (jstring)jni->CallObjectMethod(jt, (jmethodID)throwableToStringId_);
    if (jstr) {
      const char *msg = jni->GetStringUTFChars(jstr, NULL);
      if ((str_cmp(msg, "java.lang.OutOfMemoryError", 28) == 0)) {
        *da << DgSqlCode(-LME_JVM_OUT_OF_MEMORY) << DgString0(msg);
      } else {
        *da << DgSqlCode(-LME_JAVA_EXCEPTION) << DgString0(msg);
      }
...
// Returns  Nothing
//
void LmJavaExceptionReporter::addJavaExceptionToDiags(LmHandle throwable, ComDiagsArea &diags) {
  JNIEnv *jni = (JNIEnv *)jniEnv_;
  ...
  if (t) {
    if (!throwableToStringId_) {
      ...
      jstring jstr = (jstring)jni->CallObjectMethod(t, (jmethodID)throwableToStringId_);
...
//
LmHandle LmJavaExceptionReporter::getNextChainedException(LmHandle throwable) {
  if (throwable == NULL) {
    return NULL;
  }
  ...
  if (classRef) {
    ...
    if (exSQLClass_ != NULL && jni->IsInstanceOf(t, (jclass)exSQLClass_) == JNI_TRUE) {
      methodID = (jmethodID)exNextExceptionId_;
    } else {
      methodID = (jmethodID)jni->GetMethodID(classRef, "getNextException", "()Ljava/sql/SQLException;");
    }
    jni->ExceptionClear();
    ...
    if (methodID == NULL) {
      if (throwableClass_ != NULL) {
        ...
        methodID = (jmethodID)jni->GetMethodID(classRef, "getCause", "()Ljava/lang/Throwable;");
      }
      jni->ExceptionClear();
    }
    ...
    if (methodID == NULL) {
      methodID = (jmethodID)jni->GetMethodID(classRef, "getException", "()Ljava/lang/Throwable;");
      jni->ExceptionClear();
    }
...
LmResult LmJavaExceptionReporter::insertDiags(ComDiagsArea *diags, int errCode, const char *arg1, const char *arg2,
  ...
  switch (numArgs) {
    ...
    case 1: {
      *diags << DgSqlCode(errCode) << DgString0(arg1);
      break;
    }

    case 2: {
      *diags << DgSqlCode(errCode) << DgString0(arg1) << DgString1(arg2);
      break;
    }
  }
...
//
LmResult LmJavaExceptionReporter::checkGetMethodExceptions(const char *routineName, const char *className,
                                                           ComDiagsArea *da) {
  JNIEnv *jni = (JNIEnv *)jniEnv_;
  jthrowable jt;
  ...
  if (jstr != NULL) {
    ...
    *da << DgSqlCode(-LME_INTERNAL_ERROR) << DgString0(": Unknown error occurred.");
  }
...
//
LmResult LmJavaExceptionReporter::processUserException(LmRoutineJava *routine_handle, ComDiagsArea *da) {
  jthrowable jex;
  JNIEnv *jni = (JNIEnv *)jniEnv_;
  char errText[LMJ_ERR_SIZE_512];

  // Get Java exception if one occurred.
  if ((jex = jni->ExceptionOccurred()) == NULL) return LM_OK;

  // jni->ExceptionDescribe();
  jni->ExceptionClear();

  jstring jstr = (jstring)jni->CallObjectMethod(jex, (jmethodID)throwableToStringId_);
  ...
  if (jstr != NULL) {
    ...
    if (str_cmp(errText, "java.sql", 8) == 0 || str_cmp(errText, "com.hp.jdbc.HPT4Exception", 25) == 0) {
      reportUserSQLException(jex, errText, da);
    } else if (routine_handle->isInternalSPJ()) {
      reportInternalSPJException(jex, errText, da);
    } else {
      *da << DgSqlCode(-LME_JAVA_EXCEPTION) << DgString0(errText);
    }
...
//
void LmJavaExceptionReporter::reportUserSQLException(LmHandle jt, char *errText, ComDiagsArea *da) {
  JNIEnv *jni = (JNIEnv *)jniEnv_;
  ...
  if (jsstate != NULL) {
    ...
    if (sstate != NULL && str_len(sstate) >= 5 && str_cmp(&sstate[0], "38", 2) == 0 &&
      ...
      sqlState[5] = '\0';
      *da << DgSqlCode(-LME_CUSTOM_ERROR) << DgString0(errText) << DgString1(sqlState) << DgCustomSQLState(sqlState);
    } else {
    ...
    if (sstate != NULL) jni->ReleaseStringUTFChars(jsstate, sstate);
...
//
void LmJavaExceptionReporter::reportInternalSPJException(LmHandle jt, char *errText, ComDiagsArea *da) {
  JNIEnv *jni = (JNIEnv *)jniEnv_;
  jthrowable jex = (jthrowable)jt;
  ...
  if (exMetValFailedClass_ != NULL && jni->IsInstanceOf(jex, (jclass)exMetValFailedClass_)) {
    metNameStr = (jstring)jni->CallObjectMethod(jex, (jmethodID)exGetMethodName_);
    metSigStr = (jstring)jni->CallObjectMethod(jex, (jmethodID)exGetSignature_);
  }
  ...
  if (metNameStr != NULL && metSigStr != NULL) {
    ...
    *da << DgSqlCode(-LME_VALIDATION_FAILED) << DgString0(signature) << DgString1(errText);
...
void LmJavaExceptionReporter::processJavaObjException(LmHandle returnInfoObj, int returnStatus, int callPhase,
                                                      const char *errText, const char *udrName, ComDiagsArea *da) {
  if (da) {
    JNIEnv *jni = (JNIEnv *)jniEnv_;
    ...
    jstring returnedSQLState =
        static_cast<jstring>(jni->GetObjectField(jniResult, (jfieldID)langMan_->getReturnInfoSQLStateField()));
    jstring returnedMessage =
        static_cast<jstring>(jni->GetObjectField(jniResult, (jfieldID)langMan_->getReturnInfoMessageField()));

    if (returnedSQLState != NULL) sqlState = jni->GetStringUTFChars(returnedSQLState, NULL);
    if (returnedMessage != NULL) message = jni->GetStringUTFChars(returnedMessage, NULL);

    if (sqlState || message) {
      const char *diagsMessage = (message ? message : "no message provided");
      const char *diagsSQLState = (sqlState ? sqlState : "no SQLSTATE provided");

      // Check the returned SQLSTATE value and raise appropriate
      // SQL code. Valid SQLSTATE values begin with "38" except "38000"
      if (sqlState && (strncmp(diagsSQLState, "38", 2) == 0) && (strncmp(diagsSQLState, "38000", 5) != 0)) {
        *da << DgSqlCode(-LME_CUSTOM_ERROR) << DgString0(diagsMessage) << DgString1(diagsSQLState);
        *da << DgCustomSQLState(diagsSQLState);
      } else {
        *da << DgSqlCode(-LME_UDF_ERROR) << DgString0(udrName) << DgString1(diagsSQLState) << DgString2(diagsMessage);
      }
    } else {
      ...
      *da << DgSqlCode(-LME_OBJECT_INTERFACE_ERROR) << DgString0(udrName) << DgString1(buf1) << DgString2(errText)
          << DgString3(buf2);
    }
    if (sqlState) jni->ReleaseStringUTFChars(returnedSQLState, sqlState);
    if (message) jni->ReleaseStringUTFChars(returnedMessage, message);
    if (returnedSQLState) jni->DeleteLocalRef(returnedSQLState);
