...
void LmResultSetJava::initType4ResultSet(int paramPos, const char *routineName, LmResultSetInfoStatus &status,
  ...
  jobjectArray oArray1 = jni->NewObjectArray(1, (jclass)lmj_->stringClass_, NULL);
  jobjectArray oArray2 = jni->NewObjectArray(1, (jclass)lmj_->connClass_, NULL);

  // Call getT4RSInfo() of LmUtility java class
  jni->CallStaticVoidMethod((jclass)lmj_->utilityClass_, (jmethodID)lmj_->utilityGetT4RSInfoId_, (jobject)jdbcRSRef_,
                            lArray, bArray, iArray, bArray2, oArray1, oArray2);
  ...
  LM_ASSERT(jni->ExceptionOccurred() == NULL);
  if (lobData[0]) {
    *da << DgSqlCode(-LME_LOB_COL_IN_RS_ERROR) << DgString0(routineName);
  ...
  if (isScrollable()) {
    jni->CallVoidMethod((jobject)jdbcRSRef_, (jmethodID)lmj_->rsBeforeFirstId_);
    currentRowPosition_ = 0;
  }
  ...
  if (i == lmConnList.entries()) {
    ...
    lmConn_ = new (collHeap())
        LmConnection(lmj_, (jobject)jdbcConnRef, LmConnection::NON_DEFAULT_CONN, COM_NO_TRANSACTION_REQUIRED);
    lmConn_->incrRefCnt();
...
void LmResultSetJava::initType2ResultSet(int paramPos, const char *routineName, LmResultSetInfoStatus &status,
  ...
  if (jni->ExceptionOccurred() || rsInfoLArray_ == NULL || lArray_ == NULL || oArray_ == NULL || errCode_ == NULL ||
      errDetail_ == NULL) {
    // JVM Out of memory
  ...
  jni->CallStaticVoidMethod((jclass)lmj_->utilityClass_, (jmethodID)lmj_->utilityGetRSInfoId_, (jobject)jdbcRSRef_,
                            rsInfoLArray_, lArray_, oArray_, errCode_, errDetail_);
  ...
  jlong rsInfoLArray[arrSize];
  jlong la[1];

  jni->GetLongArrayRegion(rsInfoLArray_, 0, arrSize, rsInfoLArray);
  LM_ASSERT(jni->ExceptionOccurred() == NULL);
  ...
  NABoolean rsClosed = (rsInfoLArray[7] == 0 ? FALSE : TRUE);
  if (rsClosed) {
    status = RS_INFO_CLOSED;
    return;
  }

  NABoolean lobData = (rsInfoLArray[0] == 0 ? FALSE : TRUE);
  if (lobData) {
    *da << DgSqlCode(-LME_LOB_COL_IN_RS_ERROR) << DgString0(routineName);
  ...
  setCtxHandle((SQLCTX_HANDLE)rsInfoLArray[1]);
  setStmtID((SQLSTMT_ID *)rsInfoLArray[2]);

  firstBufferedRow_ = rsInfoLArray[3];
  lastBufferedRow_ = rsInfoLArray[4];
  currentRowPosition_ = rsInfoLArray[5];
  cursorType_ = (LmResultSetType)rsInfoLArray[6];
  rsCounter_ = la[0];
  CLIStmtClosed_ = (rsInfoLArray[8] == 0 ? FALSE : TRUE);
...
LmResultSetJava::~LmResultSetJava() {
  ...
  if (rsInfoLArray_) jni->DeleteLocalRef(rsInfoLArray_);

  if (lArray_) jni->DeleteLocalRef(lArray_);

  if (oArray_) jni->DeleteLocalRef(oArray_);

  if (errCode_) jni->DeleteLocalRef(errCode_);

  if (errDetail_) jni->DeleteLocalRef(errDetail_);

  if (jdbcRSRef_ != NULL) jni->DeleteGlobalRef((jobject)jdbcRSRef_);

  if (proxySyntax_) NADELETEBASIC(proxySyntax_, collHeap());
}
...
void LmResultSetJava::close(ComDiagsArea *da) {
  ...
  if (jdbcRSRef_ != NULL) {
    ...
    jni->CallVoidMethod((jobject)jdbcRSRef_, (jmethodID)lmj_->rsCloseId_);

    LM_DEBUG1("ResultSet.close() returned %s", jni->ExceptionOccurred() ? "an Exception" : "no Exception");
  ...
  if (lmConn_->decrRefCnt()) lmConnList_.removeAt(i);
...
// as jlong value.
LmResult LmResultSetJava::getValueAsJlong(jobject javaRS, ComUInt32 columnIndex, ComDiagsArea &da, NABoolean &wasNull,
                                          jlong &returnvalue) {
  JNIEnv *jni = (JNIEnv *)lmj_->jniEnv_;

  // Get value as BigDecimal object
  jobject bigdecObj = jni->CallObjectMethod(javaRS, (jmethodID)lmj_->rsGetBigDecimalId_, columnIndex);
  ...
  jobject bigintObj;
  bigintObj = jni->CallObjectMethod(bigdecObj, (jmethodID)lmj_->bigdecUnscaleId_);
  ...
  returnvalue = jni->CallLongMethod(bigintObj, (jmethodID)lmj_->bigintLongValueId_);
    ...
    for (ComUInt32 index = 0; index < numCols; index++) {
      ...
      switch (LmJavaType(col).getType()) {
        ...
        case LmJavaType::JT_FLOAT: {
          jfloat val = jni->CallFloatMethod(javaRS, (jmethodID)lmj_->rsGetFloatId_, index + 1);
          if (jni->ExceptionOccurred()) {
        ...
        case LmJavaType::JT_DOUBLE: {
          jdouble val = jni->CallDoubleMethod(javaRS, (jmethodID)lmj_->rsGetDoubleId_, index + 1);
          if (jni->ExceptionOccurred()) {
            insertIntoDiagnostic(da, index + 1);
            retcode = -1;
            break;
        ...
        case LmJavaType::JT_LANG_STRING: {
          ...
          jobject strObj = jni->CallObjectMethod(javaRS, (jmethodID)lmj_->rsGetObjectId_, index + 1);
        ...
        case LmJavaType::JT_MATH_BIGDEC: {
          ...
          jobject bigdecObj = jni->CallObjectMethod(javaRS, (jmethodID)lmj_->rsGetBigDecimalId_, index + 1);
          if (jni->ExceptionOccurred()) {
            insertIntoDiagnostic(da, index + 1);
            retcode = -1;
            break;
            ...
            if (col->isNumeric()) {
              if (col->isBigNum()) {
                ...
                if (jni->ExceptionOccurred()) {
                  ...
                  jlong jlval;
                  jlval = jni->CallLongMethod(bigintObj, (jmethodID)lmj_->bigintLongValueId_);
                  jni->DeleteLocalRef(bigintObj);
                  ...
                  if (jni->ExceptionOccurred()) {
                    ...
                    if ((col->fsType() == COM_SIGNED_BIN8_FSDT) || (col->fsType() == COM_UNSIGNED_BIN8_FSDT)) {
                      char sval = (char)jlval;
                      memcpy(thisColDataPtr, (char *)&sval, col->outSize());
                    } else if ((col->fsType() == COM_SIGNED_BIN16_FSDT) || (col->fsType() == COM_UNSIGNED_BIN16_FSDT)) {
                      short sval = (short)jlval;
                      memcpy(thisColDataPtr, (char *)&sval, col->outSize());
        ...
        case LmJavaType::JT_SQL_DATE: {
          jobject dateObj = jni->CallObjectMethod(javaRS, (jmethodID)lmj_->rsGetObjectId_, index + 1);
          if (jni->ExceptionOccurred()) {
            insertIntoDiagnostic(da, index + 1);
            retcode = -1;
            break;
        ...
        case LmJavaType::JT_SQL_TIME: {
          jobject timeObj = jni->CallObjectMethod(javaRS, (jmethodID)lmj_->rsGetTimeId_, index + 1);
          if (jni->ExceptionOccurred()) {
            insertIntoDiagnostic(da, index + 1);
            retcode = -1;
            break;
        ...
        case LmJavaType::JT_SQL_TIMESTAMP: {
          jobject timestampObj = jni->CallObjectMethod(javaRS, (jmethodID)lmj_->rsGetTimestampId_, index + 1);
          if (jni->ExceptionOccurred()) {
            insertIntoDiagnostic(da, index + 1);
            retcode = -1;
            break;
      ...
      if ((conv_ret == LM_PARAM_OVERFLOW) && (has_warning == 0)) has_warning = 1;
