...
SQLUDR_INT32 SQLUDR_INVOKE_SQLROWTM(sqlrow_func func_ptr, SQLUDR_CHAR *in_data, SQLUDR_CHAR *rowDataSpace1,
  ...
  return func_ptr(in_data, rowDataSpace1, rowDataSpace2, getRow, emitRow, sqlstate, msgtext, calltype, statearea,
                  udrinfo);
}
...
//
//////////////////////////////////////////////////////////////////////
LmRoutineCSqlRowTM::LmRoutineCSqlRowTM(const char *sqlName, const char *externalName, const char *librarySqlName,
                                       ComUInt32 numSqlParam, ComUInt32 numTableInfo, LmTableInfo *tableInfo,
    ...
    : LmRoutineC(sqlName, externalName, librarySqlName, numSqlParam, routineSig, maxResultSets, COM_LANGUAGE_C,
                 COM_STYLE_SQLROW_TM, transactionAttrs, sqlAccessMode, externalSecurity, routineOwnerId, parentQid,
  ...
  udrInfo_->table_inputs = (SQLUDR_TABLE_PARAM *)malloc(numTableInfo * sizeof(SQLUDR_TABLE_PARAM) + 32);
  ...
  for (ComUInt32 i = 0; i < udrInfo_->num_table_inputs; i++) {
    ...
    udrTInfo->params = (SQLUDR_PARAM *)malloc(lmTInfo.numInColumns_ * sizeof(SQLUDR_PARAM) + 32);
    ...
    for (ComUInt32 j = 0; j < udrTInfo->num_params; j++) {
      ...
      udrColumnInfo.datatype = (SQLUDR_INT16)getAnsiTypeFromFSType(p.fsType());
      ...
      // * scale
      if (p.isCharacter()) {
        udrColumnInfo.u1.character_set = (SQLUDR_INT16)p.encodingCharSet();
      } else if (p.isDateTime()) {
        // For datetime types: ANSI type will be SQLTYPECODE_DATETIME and
        // SQLUDR_PARAM stores a code to indicate date, time, or
        // timestamp.
        udrColumnInfo.u1.datetime_code = (SQLUDR_INT16)p.getDatetimeCode();
        ...
        udrColumnInfo.u1.interval_code = (SQLUDR_INT16)getIntervalCode(p.fsType());
      } else {
    ...
    if (lmTInfo.rowLength_ > maxInputRowLength) maxInputRowLength = lmTInfo.rowLength_;
  }
...
LmResult LmRoutineCSqlRowTM::invokeRoutine(void *inputRow, void *outputRow, ComDiagsArea *da) {
