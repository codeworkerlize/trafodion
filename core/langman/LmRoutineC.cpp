...
LmRoutineC::LmRoutineC(const char *sqlName, const char *externalName, const char *librarySqlName, ComUInt32 numSqlParam,
    ...
    : LmRoutine(container, routine, sqlName, externalName, librarySqlName, numSqlParam, maxResultSets, language,
                paramStyle, transactionAttrs, sqlAccessMode, externalSecurity, routineOwnerId, parentQid, NULL,
                inputParamRowLen, outputRowLen, currentUserName, sessionUserName, parameters, lm),
  ...
  if (LmRoutineC::host_data_ == NULL) {
    LmRoutineC::host_data_ = (SQLUDR_CHAR *)malloc(SQLUDR_STATEAREA_BUFFER_SIZE + 128);
    LM_ASSERT(LmRoutineC::host_data_);
    memset((char *)LmRoutineC::host_data_, 0, SQLUDR_STATEAREA_BUFFER_SIZE + 128);
  }
  ...
  stateArea_->stmt_data.data = (SQLUDR_CHAR *)malloc(SQLUDR_STATEAREA_BUFFER_SIZE + 128);
  LM_ASSERT(stateArea_->stmt_data.data);
  memset((char *)stateArea_->stmt_data.data, 0, SQLUDR_STATEAREA_BUFFER_SIZE + 128);
  ...
  if (qid) udrInfo_->query_id = copy_and_pad(qid, strlen(qid), 8);
  ...
  SQLUDR_PARAM *sqludr_params = (SQLUDR_PARAM *)malloc(numSqlParam_ * sizeof(SQLUDR_PARAM) + 32);
  ...
  for (i = 0; i < numSqlParam_; i++) {
    ...
    udfParamInfo.datatype = (SQLUDR_INT16)getAnsiTypeFromFSType(p.fsType());
    ...
    // * scale
    if (p.isCharacter()) {
      udfParamInfo.u1.character_set = (SQLUDR_INT16)p.encodingCharSet();
    } else if (p.isDateTime()) {
      // For datetime types: ANSI type will be SQLTYPECODE_DATETIME and
      // SQLUDR_PARAM stores a code to indicate date, time, or
      // timestamp.
      udfParamInfo.u1.datetime_code = (SQLUDR_INT16)p.getDatetimeCode();
      ...
      udfParamInfo.u1.interval_code = (SQLUDR_INT16)getIntervalCode(p.fsType());
    } else {
    ...
    if ((getParamStyle() == COM_STYLE_SQLROW) || (getParamStyle() == COM_STYLE_SQLROW_TM)) {
      switch (p.direction()) {
        case COM_INPUT_COLUMN:
  ...
  udrInfo_->return_values = (num_outputs > 0 ? &sqludr_params[num_inputs] : NULL);
  ...
  if (currentUserName) udrInfo_->current_user_name = copy_and_pad(currentUserName, strlen(currentUserName), 8);

  if (sessionUserName) udrInfo_->session_user_name = copy_and_pad(sessionUserName, strlen(sessionUserName), 8);
...
LmRoutineC::~LmRoutineC() {
  ...
  if (stateArea_ && stateArea_->stmt_data.data) free(stateArea_->stmt_data.data);

  if (stateArea_) free(stateArea_);

  if (udrInfo_) {
    if (udrInfo_->query_id) free(udrInfo_->query_id);
    if (udrInfo_->current_user_name) free(udrInfo_->current_user_name);
    if (udrInfo_->session_user_name) free(udrInfo_->session_user_name);
    deletePassThroughInputs();
    free(udrInfo_);
  }
}

void LmRoutineC::setNumPassThroughInputs(ComUInt32 numPassThroughInputs) {
  deletePassThroughInputs();
...
void LmRoutineC::setPassThroughInput(ComUInt32 index, void *data, ComUInt32 dataLen) {
...
LmResult LmRoutineC::processReturnStatus(ComSInt32 retcode, ComDiagsArea *diags) {
  char *returnMsgText = NULL, *noMsgText = NULL;

  if (retcode == SQLUDR_SUCCESS) return LM_OK;

  if (msgText_[0] != '\0') {
    returnMsgText = msgText_;
  } else {
    const char *text = "No SQL message text was provided by user-defined function ";
  ...
  if (noMsgText) NADELETEBASIC(noMsgText, collHeap());
