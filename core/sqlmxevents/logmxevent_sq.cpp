...
int SQLMXLoggingArea::logSQLMXEventForError(
  ...
  if (String0) sql_error_event.set_string0(String0);
  if (String1) sql_error_event.set_string1(String1);
  if (String2) sql_error_event.set_string2(String2);
  if (String3) sql_error_event.set_string3(String3);
  if (String4) sql_error_event.set_string4(String4);
  if (serverName) sql_error_event.set_server_name(serverName);
  if (connectionName) sql_error_event.set_connection_name(connectionName);
  if (constraintCatalog) sql_error_event.set_constraint_catalog(constraintCatalog);
  if (constraintSchema) sql_error_event.set_constraint_schema(constraintSchema);
  if (constraintName) sql_error_event.set_constraint_name(constraintName);
  if (triggerCatalog) sql_error_event.set_trigger_catalog(triggerCatalog);
  if (triggerSchema) sql_error_event.set_trigger_schema(triggerSchema);
  if (triggerName) sql_error_event.set_trigger_name(triggerName);

  if (catalogName) sql_error_event.set_catalog_name(catalogName);
  if (schemaName) sql_error_event.set_schema_name(schemaName);
  if (tableName) sql_error_event.set_table_name(tableName);
  if (columnName) sql_error_event.set_column_name(columnName);
  ...
  AMQPRoutingKey routingKey(SP_EVENT, SP_SQLPACKAGE, SP_INSTANCE, SP_PUBLIC, SP_GPBPROTOCOL, "error_event");
  try {
    rc = sendAMQPMessage(true, sql_error_event.SerializeAsString(), SP_CONTENT_TYPE_APP, routingKey);
    if (rc) throw 1;
  } catch (...) {
    closeAMQPConnection();
    establishedAMQPConnection_ = FALSE;
...
static void writeStackTrace(char *s, int bufLen) {
  ...
  while (i < size - 2) {
    ...
    if (cmdFP == NULL) {
      if (len + safetyMargin < bufLen) len += sprintf(s, "Error %d while popen() of %s\n", errno, buffer);
      break;
    } else {
      for (j = i; j < i + 5 && j < size - 2; j++) {
        ...
        if (len + safetyMargin < bufLen) len += sprintf(s, "%p: %s()\n", bt[j], buffer);
        fgets(buffer, sizeof(buffer), cmdFP);
        if (len + safetyMargin < bufLen) len += sprintf(s, "            %s", buffer);
      }
      fclose(cmdFP);
    }
    i = j;
...
void SQLMXLoggingArea::logSQLMXAssertionFailureEvent(const char *file, int line, const char *msg,
  ...
  sevent.setLineNumber(line);
  sevent.setMessageText((char *)msg);
  if (tid) sevent.setInt0(*tid);
  if (condition) sevent.setString0((char *)condition);
...
Int16 SqlSealogEvent::sendEvent(Int16 eventId, int slSeverity) {
  ...
  AMQPRoutingKey routingKey(SP_EVENT, SP_SQLPACKAGE, SP_INSTANCE, SP_PUBLIC, SP_GPBPROTOCOL, "info_event");
  try {
