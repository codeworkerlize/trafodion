...
short DP2LockFlags::transactionNeeded() {
  ...
  if (getSkipLockedRows() && (!getUpdateOrDeleteFlag())) return 0;
  // QSTUFF
...
void StmtLevelAccessOptions::updateAccessOptions(TransMode::AccessType at, LockMode lm) {
  if (at != TransMode::ACCESS_TYPE_NOT_SPECIFIED_)
    if (accessType_ < at) accessType_ = at;

  if (lm != LOCK_MODE_NOT_SPECIFIED_)
    if (lockMode_ < lm) lockMode_ = lm;
...
DP2LockFlags StmtLevelAccessOptions::getDP2LockFlags() {
  ...
  switch (accessType_) {
    ...
    case TransMode::READ_COMMITTED_ACCESS_:
      ...
      if (isUpdateOrDelete()) flags.setUpdateOrDeleteFlag();
      // QSTUFF
...
NABoolean TransMode::operator==(const TransMode &other) const {
  return (il_ == other.il_ && am_ == other.am_ && rm_ == other.rm_ && autoAbortInterval_ == other.autoAbortInterval_ &&
          multiCommit_ == other.multiCommit_ && autoCommit_ == other.autoCommit_ &&
          xnInProgress() == other.xnInProgress());
}

void TransMode::updateTransMode(TransMode *trans_mode) {
  if (trans_mode->isolationLevel() != TransMode::IL_NOT_SPECIFIED_) isolationLevel() = trans_mode->isolationLevel();

  if (trans_mode->accessMode() != TransMode::AM_NOT_SPECIFIED_) accessMode() = trans_mode->accessMode();

  if (trans_mode->autoCommit() != TransMode::AC_NOT_SPECIFIED_) autoCommit() = trans_mode->autoCommit();

  if (trans_mode->rollbackMode() != TransMode::ROLLBACK_MODE_NOT_SPECIFIED_)
    rollbackMode() = trans_mode->rollbackMode();

  if (trans_mode->diagAreaSize() >= 0) diagAreaSize() = trans_mode->diagAreaSize();
  ...
  } else if (trans_mode->getAutoBeginOff() != 0) {
    setAutoBeginOn(FALSE);  // to clear AUTO_BEGIN_ON_ bit
    setAutoBeginOff(trans_mode->getAutoBeginOff());
  }

  if (trans_mode->getAutoCommitSavepointOn())
    setAutoCommitSavepointOn();
  else if (trans_mode->getAutoCommitSavepointOff())
    setAutoCommitSavepointOff();
}

void TransMode::updateXnInProgress(TransMode *trans_mode) { setXnInProgress(trans_mode->xnInProgress()); }
...
TransMode::IsolationLevel TransMode::getIsolationLevel(const char *value) {
  ...
  if (str_len(value) > 5) switch (value[5]) {  //  012345.  value[5] is {C|U|T|L}.
      case 'C':
        il = TransMode::READ_COMMITTED_;
        break;
      case 'U':
        il = TransMode::READ_UNCOMMITTED_;
        break;
      case 'T':
        il = TransMode::REPEATABLE_READ_;
        break;
...
const char *getStrOfIsolationLevel(TransMode::IsolationLevel isl, NABoolean doNotReportNotSpecified) {
  switch (isl) {
