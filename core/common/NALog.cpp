...
// -------------------------------------------------------------------------
CNALogfile::~CNALogfile() {
  if (m_pLogName) delete[] m_pLogName;
}
...
char *CNALogfile::GetNextToken(char *pStrToScan, int &length) {
  ...
  if (pStartOfToken[0] == '\0') return NULL;

  // if the first character is the comma, point past it.
  if (pStartOfToken[0] == ',') pStartOfToken++;

  char *pEndOfStr = pStartOfToken + strlen(pStartOfToken);

  // Take care of any leading blanks before the options start
  while (pStartOfToken < pEndOfStr && *pStartOfToken == ' ') pStartOfToken++;

  if (pStartOfToken >= pEndOfStr) return NULL;
...
int CNALogfile::Open(void) {
  ...
  try {
    ...
    stdioMode = m_bClearLogAtOpen ? CNAStdioFile::eWrite : CNAStdioFile::eAppend;
    if (!inherited::Open(m_pLogName, stdioMode)) {
      TURNOFFLOGGING
#ifdef _DEBUG
      THROWERROREXCP
    ...
    if (m_bPrintLogHeader) Write(headerStr, headerStr.length());
...
NABoolean CNALogfile::Write(const char *pBuffer, int buflen) {
  ...
  NAString row;
  if (m_bPrintTime) row += GetTimeToLog();
  if (m_bPrintProcessID) row += GetProcessToLog();
...
NABoolean CNALogfile::isHeaderString(const char *strHeader) {
  NAString header("*************** ");
