...
ComplexObject::ComplexObject(NAMemory *heap, ComplexObjectType type) : heap_(heap), type_(type) {}
...
void ComplexObject::pack(Packer *packer, PackerInput *input, PackerOutput *output) {
  sharedOperationSequence(packer, input, output);
}

void ComplexObject::unPack(UnPacker *unPacker, UnPackerInput *input, UnPackerOutput *output) {
  sharedOperationSequence(unPacker, input, output);
}

void ComplexObject::length(Lengther *lengther, LengtherInput *input, LengtherOutput *output) {
  sharedOperationSequence(lengther, input, output);
}

void ComplexObject::baseOperation(MessageOperator *msgOp, InputContainer *input, OutputContainer *output) {
...
// return NULL for now.
ComplexObject *ComplexObject::manufacture(NAMemory *heap, char *bufferPtr, ComplexObjectFactory *factory) {
...
void Packer::execute(ObjectContainer *objWrap, InputContainer *input, OutputContainer *output) {
  objWrap->pack(this, (PackerInput *)input, (PackerOutput *)output);
}

void Packer::setInputOutputForNextOperation(InputContainer *input, OutputContainer *output) {
...
void PackerInput::setAccMsgSize(IpcMessageObjSize accMsgSize) { accMsgSize_ = accMsgSize; }

// PackerOutput
PackerOutput::PackerOutput() : bufferPtr_(NULL), currMsgSize_(0) {}
...
void PackerOutput::setCurrMsgSize(IpcMessageObjSize currMsgSize) { currMsgSize_ = currMsgSize; }

// UnPacker related class definitions
// UnPacker defintions
UnPacker::UnPacker(NAMemory *heap, ComplexObjectFactory *factory) : heap_(heap), factory_(factory) {}
UnPacker::~UnPacker() {}

void UnPacker::execute(ObjectContainer *objWrap, InputContainer *input, OutputContainer *output) {
  objWrap->unPack(this, (UnPackerInput *)input, (UnPackerOutput *)output);
}

void UnPacker::setInputOutputForNextOperation(InputContainer *input, OutputContainer *output) {
...
void Lengther::execute(ObjectContainer *objWrap, InputContainer *input, OutputContainer *output) {
  objWrap->length(this, (LengtherInput *)input, (LengtherOutput *)output);
}

void Lengther::setInputOutputForNextOperation(InputContainer *input, OutputContainer *output) {
...
LengtherInput::LengtherInput(IpcMessageObjSize accMsgSize) : accMsgSize_(accMsgSize) {}

LengtherInput::~LengtherInput() {}

IpcMessageObjSize LengtherInput::getAccMsgSize() { return accMsgSize_; }

void LengtherInput::setAccMsgSize(IpcMessageObjSize accMsgSize) { accMsgSize_ = accMsgSize; }
...
void LengtherOutput::setCurrMsgSize(IpcMessageObjSize currMsgSize) { currMsgSize_ = currMsgSize; }

// ComplexObjectPtrContainer definitions
ComplexObjectPtrContainer::ComplexObjectPtrContainer(ComplexObjectPtr *objectPtr) : objectPtr_(objectPtr) {
...
ComplexObjectPtr *ComplexObjectPtrContainer::getComplexObjectPtr() { return objectPtr_; }

void ComplexObjectPtrContainer::setComplexObjectPtr(ComplexObjectPtr *objectPtr) { objectPtr_ = objectPtr; }

void ComplexObjectPtrContainer::pack(Packer *packer, PackerInput *input, PackerOutput *output) {
...
void ComplexObjectPtrContainer::unPack(UnPacker *unPacker, UnPackerInput *input, UnPackerOutput *output) {
  ComplexObject *object = *objectPtr_;
  if (object) {
    object->unPack(unPacker, input, output);
  } else {
    ComplexObject *plexObj =
        ComplexObject::manufacture(unPacker->getHeap(), input->getBufferPtr(), unPacker->getFactory());
...
void ComplexObjectPtrContainer::length(Lengther *lengther, LengtherInput *input, LengtherOutput *output) {
  ComplexObject *object = *objectPtr_;
  if (object) {
...
CharPtrObjectContainer::CharPtrObjectContainer(CharPtr *charPtr) : charPtr_(charPtr) {
...
void CharPtrObjectContainer::pack(Packer *packer, PackerInput *input, PackerOutput *output) {
...
void CharPtrObjectContainer::unPack(UnPacker *unPacker, UnPackerInput *input, UnPackerOutput *output) {
...
void CharPtrObjectContainer::length(Lengther *lengther, LengtherInput *input, LengtherOutput *output) {
  IpcMessageObjSize accMsgSize = input->getAccMsgSize();
...
void Int16ObjectContainer::pack(Packer *packer, PackerInput *input, PackerOutput *output) {
  Int16 data = *value_;
  char *bufferPtr = input->getBufferPtr();
  IpcMessageObjSize accMsgSize = input->getAccMsgSize();
...
void Int16ObjectContainer::unPack(UnPacker *unPacker, UnPackerInput *input, UnPackerOutput *output) {
  Int16 data;
  const char *bufferPtr = input->getBufferPtr();
...
void Int16ObjectContainer::length(Lengther *lengther, LengtherInput *input, LengtherOutput *output) {
  IpcMessageObjSize accMsgSize = input->getAccMsgSize();
...
void IntegerObjectContainer::length(Lengther *lengther, LengtherInput *input, LengtherOutput *output) {
  IpcMessageObjSize accMsgSize = input->getAccMsgSize();
  accMsgSize += sizeof(*value_);
  output->setCurrMsgSize(accMsgSize);
}
...
void LongObjectContainer::length(Lengther *lengther, LengtherInput *input, LengtherOutput *output) {
  IpcMessageObjSize accMsgSize = input->getAccMsgSize();
  accMsgSize += sizeof(*value_);
  output->setCurrMsgSize(accMsgSize);
}
...
EnumObjectContainer::EnumObjectContainer(ComplexObjectType *value) : value_(value) {}
...
void EnumObjectContainer::pack(Packer *packer, PackerInput *input, PackerOutput *output) {
...
void EnumObjectContainer::unPack(UnPacker *unPacker, UnPackerInput *input, UnPackerOutput *output) {
...
void EnumObjectContainer::length(Lengther *lengther, LengtherInput *input, LengtherOutput *output) {
  IpcMessageObjSize accMsgSize = input->getAccMsgSize();
  accMsgSize += sizeof(*value_);
  output->setCurrMsgSize(accMsgSize);
}
...
void NopObjectContainer::pack(Packer *packer, PackerInput *input, PackerOutput *output) {
  char *bufferPtr = input->getBufferPtr();
  IpcMessageObjSize accMsgSize = input->getAccMsgSize();
  output->setBufferPtr(bufferPtr);
  output->setCurrMsgSize(accMsgSize);
...
void NopObjectContainer::unPack(UnPacker *unPacker, UnPackerInput *input, UnPackerOutput *output) {
  char *bufferPtr = input->getBufferPtr();
  output->setBufferPtr(bufferPtr);
}

void NopObjectContainer::length(Lengther *lengther, LengtherInput *input, LengtherOutput *output) {
  IpcMessageObjSize accMsgSize = input->getAccMsgSize();
  output->setCurrMsgSize(accMsgSize);
}
...
EmptyComplexObject::EmptyComplexObject(NAMemory *heap) : ComplexObject(heap, EmptyComplexObjectType) {}

EmptyComplexObject::EmptyComplexObject() : ComplexObject(NULL, EmptyComplexObjectType) {}

EmptyComplexObject::~EmptyComplexObject() {}

void EmptyComplexObject::freeSubObjects() {}

void EmptyComplexObject::sharedOperationSequence(MessageOperator *msgOp, InputContainer *input,
