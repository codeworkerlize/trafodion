...
SockSocket::SockSocket(IpcEnvironment *env) { ABORT("Stub for SockSocket::SockSocket()"); }

SockSocket::~SockSocket() { ABORT("Stub for SockSocket::~SockSocket()"); }

SockIPAddress::SockIPAddress() { ABORT("Stub for SockIPAddress::SockIPAddress()"); }

SockConnection::SockConnection(IpcEnvironment *env, const IpcProcessId &serviceProcId,
                               NABoolean thisIsTheControlConnection, const char *eye)
...
SockConnection::~SockConnection() { ABORT("Stub for SockConnection::~SockConnection()"); }
...
void SockConnection::populateDiagsArea(ComDiagsArea *&diags, CollHeap *diagsHeap) {
...
IpcConnection *IpcServerClass::createInternetProcess(
    ComDiagsArea **diags, CollHeap *diagsHeap, const char *nodeName, const char *className,
    IpcCpuNum /*cpuNum (sorry, no support for SMPs yet)*/,
    NABoolean /*usesTransactions (sorry, no transactions in cyberspace*/, SockPortNumber defaultPortNumber) {
  ABORT("Stub for IpcServerClass::createInternetProcess()");
  return NULL;
}

IpcConnection *IpcServerClass::forkProcess(ComDiagsArea **diags, CollHeap *diagsHeap, const char * /*nodeName*/,
...
SockService::SockService(const char *serviceName, SockPortNumber defaultPortNumber) {
...
SockSocket::SockSocket(IpcEnvironment *env) {
  environment_ = env;
  expectedBytes_ = receivedBytesSoFar_ = 0;
  ...
  if (fdesc_ < 0) setFromerrno("creating an unbound socket with socket()");
}
...
SockPortNumber SockSocket::bindOrConnect(const SockIPAddress &ipAddr, SockPortNumber port, NABoolean bindOnly) {
  ...
  if (retcode == SOCKET_ERROR) setFromerrno("::bind() or ::connect()");
  ...
  if (retcode == SOCKET_ERROR) setFromerrno("::getsockname()");
...
SockPortNumber SockSocket::listen(SockPortNumber port) {
  ...
  if (NOT lastError_.hasError()) {
    ...
    if (::listen(fdesc_, 5) < 0) setFromerrno("::listen()");
  } else {
...
SockPortNumber SockSocket::receiveListnerPortNum() {
  ...
  assert(receivedBytes == numCharsInListnerMsg AND temp == 0);
...
#ifdef LOG_IPC
  IpcSockLogTimestamp(fdesc_);
  IpcMessageObj *firstUserObj = (IpcMessageObj *)(message->data(sizeof(InternalMsgHdrInfoStruct)));
  cerr << "sent " << bytesSentSoFar << " bytes, objtype " << firstUserObj->getType() << endl;
#endif
...
NABoolean SockSocket::receive(IpcMessageBuffer *&message, IpcTimeout timeout) {
  ...
  if (partiallyReceivedBuffer_ == NULL AND message != NULL) partiallyReceivedBuffer_ = message;
  ...
  while (expectedBytes_ > receivedBytesSoFar_ AND NOT lastError_.hasError()) {
    ...
    if (receivedBytesThisTime <= 0) {
      ...
      if (receivedBytesSoFar_ + receivedBytesThisTime == sizeof(InternalMsgHdrInfoStruct)) {
        // a pointer to the message header
        InternalMsgHdrInfoStruct *headerPtr = (InternalMsgHdrInfoStruct *)sockReceiveBuffer;
        ...
        if (sockReceiveBuffer == header) {
          ...
          partiallyReceivedBuffer_ = IpcMessageBuffer::allocate(expectedBytes_,
                                                                environment_->getGuaMaxMsgIOSize(),  // for now
          ...
          str_cpy_all(sockReceiveBuffer, header, sizeof(InternalMsgHdrInfoStruct));
        } else {
          // if a buffer was supplied then it better be big enough
          assert(partiallyReceivedBuffer_->getBufferLength() >= expectedBytes_);
        }
  ...
  if (lastError_.hasError()) {
    // indicate the error by returning a zero-length message
    if (partiallyReceivedBuffer_ != NULL) partiallyReceivedBuffer_->setMessageLength(0);
  } else {
...
#ifdef LOG_IPC
  IpcSockLogTimestamp(fdesc_);
  IpcMessageObj *firstUserObj = (IpcMessageObj *)(partiallyReceivedBuffer_->data(sizeof(InternalMsgHdrInfoStruct)));
  cerr << "received " << receivedBytesSoFar_ << " bytes, objtype " << firstUserObj->getType() << endl;
#endif
...
NABoolean SockSocket::accept(SockFdesc &fdesc, IpcTimeout timeout) {
  ...
  if (retcode >= 0) {
    ...
    SockPortNumber clientPort = clientSockAddr.sa_data[0] * 256 + clientSockAddr.sa_data[1];
...
SockConnection::SockConnection(IpcEnvironment *env, const IpcProcessId &serviceProcId,
                               NABoolean thisIsTheControlConnection, const char *eye)
    : IpcConnection(env, serviceProcId, eye), sock_(env), isClient_(TRUE), lastReplyTag_(0), ioq_(env->getHeap()) {
  port_ = sock_.connect(serviceProcId.getIPAddress(), serviceProcId.getPortNumber());
  ...
  if (thisIsTheControlConnection) {
    ...
    setOtherEnd(IpcProcessId(serviceProcId.getIPAddress(), sock_.receiveListnerPortNum()));
  } else {
  ...
  if (NOT sock_.hasError()) {
    ...
    cerr << "connected to server " << serverPidAsString << " on port " << port_ << endl;
#endif
...
SockConnection::SockConnection(IpcEnvironment *env, SockFdesc fdesc, NABoolean isClient, const char *eye)
    : IpcConnection(env, IpcProcessId(), eye),
      sock_(fdesc, env),
      port_(NoSockPortNumber),
      isClient_(isClient),
      lastReplyTag_(0),
      ioq_(env->getHeap()) {
...
void SockConnection::send(IpcMessageBuffer *buffer) {
  ...
  if (isClient_) {
    ...
    lastReplyTag_++;
    if (lastReplyTag_ > 4711) lastReplyTag_ = 0;
...
void SockConnection::receive(IpcMessageStreamBase *msg) {
  ...
  if (isClient_) {
    ...
    assert(receiveEntry AND receiveEntry->sent_ AND NOT receiveEntry->receiving_ AND receiveEntry->sendBuffer_ == NULL);
  } else {
  ...
  for (int i = 0; i < (int)ioq_.entries(); i++) {
    ...
    if (e->receiving_) {
      ...
      if (completed) {
        ...
        short replyTag = ((InternalMsgHdrInfoStruct *)buffer->data())->sockReplyTag_;
        ...
        if (isClient_) {
          ...
          assert(found AND e->sent_ AND e->recvBuffer_ == NULL);
        } else {
          // in the server the buffer is matched with the current
          // I/O queue entry; we know it is in the receiving state
          assert(e->receiving_ AND NOT e->sent_ AND e->sendBuffer_ == NULL AND e->recvBuffer_ == NULL);
    ...
    if (e->sent_ AND e->sendBuffer_) {
      ...
      if (isClient_) {
        ...
        if (ioq_.entries() == 0) setState(REPLY_PENDING);
      }
      ...
      if (isClient_) {
        ...
        // if this was the last active I/O then set state to inactive
        if (ioq_.entries() == 0) setState(ESTABLISHED);
      } else {
...
void SockConnection::populateDiagsArea(ComDiagsArea *&diags, CollHeap *diagsHeap) {
  if (sock_.hasError()) {
    ...
    *diags << DgSqlCode(-2035) << DgInt0(sock_.getError().geterrno());
    getEnvironment()->getMyOwnProcessId(IPC_DOM_INTERNET).addProcIdToDiagsArea(*diags, 0);
...
SockPairConnection::SockPairConnection(IpcEnvironment *env, const char *eye)
    : SockConnection(env, InvalidFdesc, TRUE, eye) {
  ABORT("Not Implemented milestone 1");
}

SockPairConnection::SockPairConnection(IpcEnvironment *env, SockFdesc fd) : SockConnection(env, fd, FALSE) {
  otherEnd_ = NULL;
}
...
void SockPairConnection::doConnectNow() { port_ = connect(ipAddr_, port_); }

// -----------------------------------------------------------------------
// Methods for class SockListnerPort
// -----------------------------------------------------------------------

SockListnerPort::SockListnerPort(IpcEnvironment *env, SockFdesc fdesc, SockControlConnection *cc, const char *eye)
...
void SockListnerPort::send(IpcMessageBuffer *) { ABORT("SockListnerPort::send()"); }

void SockListnerPort::receive(IpcMessageStreamBase *) { ABORT("SockListnerPort::receive()"); }
...
WaitReturnStatus SockListnerPort::wait(IpcTimeout timeout, UInt32 *eventConsumed, IpcAwaitiox *ipcAwaitiox) {
  ...
  if (socket().accept(newFdesc, timeout)) {
    ...
    SockConnection *newConn = new (getEnvironment()->getHeap()) SockConnection(getEnvironment(), newFdesc, FALSE);
...
SockControlConnection::SockControlConnection(IpcEnvironment *env, const char *eye)
  ...
  listnerPort_ = new (env->getHeap()) SockListnerPort(env, listnerSocket_.getFdesc(), this);
...
IpcConnection *SockControlConnection::getConnection() const { return controlConnection_; }

SockControlConnection *SockControlConnection::castToSockControlConnection() { return this; }
...
IpcConnection *IpcServerClass::createInternetProcess(
  ...
  IpcConnection *result = new (environment_->getHeap())
      SockConnection(environment_, IpcProcessId(theNode.getIPAddress(), service.getPortNumber()), TRUE);

  // do an error check and set diags area if it's used
  if (diags) result->populateDiagsArea(*diags, diagsHeap);
...
IpcConnection *IpcServerClass::forkProcess(ComDiagsArea **diags, CollHeap *diagsHeap, const char * /*nodeName*/,
  ...
  if (serverPid < 0) ABORT("Couldn't fork the server process");
  ...
  if (serverPid != 0) {
    ...
    // variable is set
    if (getenv("DEBUG_SERVER")) argv[1] = "-fork -debug";
    if (getenv("DEBUG_FORK")) NADebug();
