...
static short getIntervalFSDatatype(rec_datetime_field startField, rec_datetime_field endField) {
  short fsDatatype = -1;
  if (startField > endField) return fsDatatype;
  switch (startField) {
    case REC_DATE_YEAR:
      ...
      break;
    case REC_DATE_MONTH:
      if (endField == REC_DATE_MONTH) fsDatatype = REC_INT_MONTH;
      break;
    case REC_DATE_DAY:
    ...
    case REC_DATE_SECOND:
      if (endField == REC_DATE_SECOND) fsDatatype = REC_INT_SECOND;
      break;
...
NABoolean IntervalType::validate(rec_datetime_field startField, UInt32 leadingPrecision, rec_datetime_field endField,
...
UInt32 IntervalType::getPrecision(rec_datetime_field startField, UInt32 leadingPrecision, rec_datetime_field endField,
                                  UInt32 fractionPrecision) {
  // See also file ../sqludr/sqludr.cpp, method TypeInfo::TypeInfo,
  // which implements this algorithm as well.
  if (!IntervalType::validate(startField, leadingPrecision, endField, fractionPrecision)) return 0;
  UInt32 precision;
...
UInt32 IntervalType::computeLeadingPrecision(rec_datetime_field startField, UInt32 precision,
...
size_t IntervalType::getStringSize(rec_datetime_field startField, UInt32 leadingPrecision, rec_datetime_field endField,
                                   UInt32 fractionPrecision) {
  ...
  if (startField == REC_DATE_FRACTION_MP) {
    ...
    result = 1 + 1 + leadingPrecision + IntervalFieldStringSize * (endField - startField);
...
short IntervalType::getFSDatatype() const { return getIntervalFSDatatype(getStartField(), getEndField()); }
...
const NAType *IntervalType::synthesizeType(enum NATypeSynthRuleEnum synthRule, const NAType &operand1,
                                           const NAType &operand2, CollHeap *h, UInt32 *flags) const {
  //
  // The Interval is unsupported if the start field is FRACTION.
  //
  if (!operand1.isSupportedType() || !operand2.isSupportedType()) return NULL;
  //
  ...
  switch (synthRule) {
    ...
    case SYNTH_RULE_UNION: {
      ...
      rec_datetime_field startField = MINOF(op1.getStartField(), op2.getStartField());
      UInt32 leadingPrecision = MAXOF(op1.getLeadingPrecision(), op2.getLeadingPrecision());
      if ((synthRule == SYNTH_RULE_ADD) || (synthRule == SYNTH_RULE_SUB))
        leadingPrecision = MINOF(leadingPrecision + 1, SQLInterval::MAX_LEADING_PRECISION);
      ...
      UInt32 totalPrecision = getPrecision(startField, leadingPrecision, endField, fractionPrecision);
      if (totalPrecision > SQLInterval::MAX_LEADING_PRECISION)
        leadingPrecision -= totalPrecision - SQLInterval::MAX_LEADING_PRECISION;
      return new (h) SQLInterval(h, op1.supportsSQLnull() || op2.supportsSQLnull(), startField, leadingPrecision,
    ...
    case SYNTH_RULE_DIV: {
      ...
      if ((op1.getLeadingPrecision() < leadingPrecision) || (op2.supportsSQLnull() && !op1.supportsSQLnull()))
        return new (h) SQLInterval(h, op1.supportsSQLnull() || op2.supportsSQLnull(), op1.getStartField(),
                                   leadingPrecision, op1.getEndField(), fractionPrecision);
      else
        return &operand1;
    }
    case SYNTH_RULE_MUL: {
      ...
      UInt32 leadingPrecision =
          MINOF((UInt32)(intervalOp->getLeadingPrecision() + (numericOp->getMagnitude() + 9) / 10),
                SQLInterval::MAX_LEADING_PRECISION);
      UInt32 totalPrecision = getPrecision(intervalOp->getStartField(), leadingPrecision, intervalOp->getEndField(),
                                           intervalOp->getFractionPrecision());
      if (totalPrecision > SQLInterval::MAX_LEADING_PRECISION)
        leadingPrecision -= totalPrecision - SQLInterval::MAX_LEADING_PRECISION;
      return new (h)
          SQLInterval(h, intervalOp->supportsSQLnull() || numericOp->supportsSQLnull(), intervalOp->getStartField(),
                      leadingPrecision, intervalOp->getEndField(), intervalOp->getFractionPrecision());
    }
    default:
      return NULL;
  }  // switch
...
void IntervalType::getRepresentableValue(char sign, void *bufPtr, int *bufLen, NAString **stringLiteral,
  ...
  *v = '\0';
  ...
  i = (digit == '0') ? 0 : 1;
  IntervalValue value(&valueString[i], getStartField(), getLeadingPrecision(), getEndField(), getFractionPrecision(),
                      sign);
...
NABoolean IntervalType::createSQLLiteral(const char *buf, NAString *&stringLiteral, NABoolean &isNull,
                                         CollHeap *h) const {
  ...
  ex_expr::exp_return_type ok =
      convDoIt((char *)valPtr, getNominalSize(), getFSDatatype(), getPrecision(), getFractionPrecision(), numVal,
               NUMVAL_LEN, REC_BYTE_V_ASCII, 0, SQLCHARSETCODE_UTF8, (char *)&numValLen, sizeof(numValLen), h, &diags,
               ConvInstruction::CONV_UNKNOWN, 0);

  if (ok != ex_expr::EXPR_OK || numValLen == 0) return FALSE;
  ...
  *stringLiteral += getFieldName(getStartField());
  ...
  if (getEndField() != getStartField()) {
    ...
    *stringLiteral += getFieldName(getEndField());
    // add fraction precision for endfield(only RECOND) if not the default of 6
...
IntervalValue::IntervalValue(const char *strValue, rec_datetime_field startField, UInt32 leadingPrecision,
                             rec_datetime_field endField, UInt32 fractionPrecision, char sign)
    : value_(NULL), valueLen_(0) {
  if (sign != '+' && sign != '-') return;
  //
  ...
  for (int field = startField + 1; field <= endField; field++) {
    ...
    if (!scanChar(strValue, precedingITPunc[index]))
      if (field != REC_DATE_HOUR || (!scanChar(strValue, ':'))) return;
    //
    // Scan the field.  It can have no more than two digits.  Check that
    // it doesn't exceed the maximum value for that field.
    //
    if ((!scanField(strValue, 2, fieldValue)) || (UInt32)fieldValue > maxITVal[index]) return;
  ...
  if (endField >= REC_DATE_SECOND && startField != REC_DATE_FRACTION_MP) {
    if ((scanChar(strValue, '.') || (endField == REC_DATE_FRACTION_MP)) && isdigit(*strValue)) {
      const char *fraction = strValue;
      if (!scanField(strValue, SQLInterval::MAX_FRACTION_PRECISION, fieldValue)) return;
      //
      ...
      if (currentPrecision < fractionPrecision)
        for (; currentPrecision++ < fractionPrecision;) fieldValue *= 10;
      else if (currentPrecision > fractionPrecision) {  // illegal
        return;
      }
    } else
      fieldValue = 0;
    ...
    //
    for (UInt32 i = 0; i < fractionPrecision; i++) intervalValue *= 10;
    intervalValue += fieldValue;
  }
  //
  // If the sign is '-', negate the interval value.
  //
  if (sign == '-') intervalValue = -intervalValue;
  //
  ...
  //
  if (*strValue == '\0') setValue(intervalValue, storageSize);
...
NABoolean IntervalValue::scanField(const char *&strValue, UInt32 maxFieldLen, int &value) const {
  ...
  value = 0;
  UInt32 i = 0;
  for (; i < maxFieldLen && isdigit(strValue[i]); i++) value = value * 10 + (strValue[i] - '0');
  //
  // If the field has no digits or too many digits, the field is invalid.
  //
  if (i == 0 || isdigit(strValue[i])) return FALSE;
  strValue += i;
  return TRUE;
}
...
NAString IntervalValue::getValueAsString(const IntervalType &dt) const {
  ...
  result.prepend(buffer);
  if (sign == '-') result.prepend(&sign, 1);
  return result;
}

void IntervalValue::print(const IntervalType &dt, FILE *ofd, const char *indent) const {
  if (isValid())
...
double SQLInterval::getValueInSeconds(double valInSecs) {
  ...
  switch (startField) {
    ...
    case REC_DATE_MINUTE:
      ...
      break;
    case REC_DATE_SECOND:
      for (i = 0; i < getFractionPrecision(); i++) valInSecs /= 10;
      break;
    default:
      break;
  }
