...
#define PAD_REQUEST(req) (((req) + sizeof(size_t) + FRAG_ALIGN_MASK) & ~FRAG_ALIGN_MASK)
...
#define ALIGN_OFFSET(A) \
  ((((size_t)(A)&FRAG_ALIGN_MASK) == 0) ? 0 : ((FRAG_BYTE_ALIGN - ((size_t)(A)&FRAG_ALIGN_MASK)) & FRAG_ALIGN_MASK))
...
inline NABoolean NABlock::blockHolds(void *addr) {
  return ((addr >= (void *)this) && ((char *)addr < (char *)this + size_));
}

// Find the NABlock that contains an address.
inline NABlock *NABlock::blockHolding(NABlock *firstBlock, void *addr) {
...
inline NABoolean NABlock::isExternSegment() { return (sflags_ & EXTERN_BIT) != 0; }

// Return whether this NABlock was allocated using mmap().
inline NABoolean NABlock::isMMapped() { return (sflags_ & MMAP_BIT) != 0; }

// Return the offset of the first fragment in the block
inline size_t NABlock::firstFragmentOffset() {
...
inline NAHeapFragment *NABlock::alignAsFragment() { return (NAHeapFragment *)((char *)this + firstFragmentOffset()); }

// Return the usable memory used for this NAHeapFragment.
inline void *NAHeapFragment::getMemory(NABoolean cleanUpPrevNext) {
...
inline NAHeapFragment *NAHeapFragment::memToFragment(void *mem) {
...
inline NABoolean NAHeapFragment::cinuse() { return (head_ & CINUSE_BIT) != 0; }

// Return whether the previous fragment is in use.
inline NABoolean NAHeapFragment::pinuse() { return (head_ & PINUSE_BIT) != 0; }

// Return whether both PINUSE_BIT and CINUSE_BIT bits are set.
inline NABoolean NAHeapFragment::cpinuse() { return (head_ & INUSE_BITS) == INUSE_BITS; }

// Return the size of the NAHeapFragment
inline size_t NAHeapFragment::fragmentSize() { return head_ & ~(INUSE_BITS | MMAPPED_BIT | FIRST_FRAGMENT_BIT); }

// Clear CINUSE_BIT.
inline void NAHeapFragment::clearCinuse() { head_ &= ~CINUSE_BIT; }

// Clear PINUSE_BIT.
inline void NAHeapFragment::clearPinuse() { head_ &= ~PINUSE_BIT; }
...
inline void NAHeapFragment::initializeFirstFragment(size_t s) {
  head_ |= FIRST_FRAGMENT_BIT;
  prevFoot_ = s;
}

// Return the status of FIRST_FRAGMENT_BIT.
inline size_t NAHeapFragment::getFirstFragmentBit() { return head_ & FIRST_FRAGMENT_BIT; }

// Set the head bits of a fragment by ORing them.
inline void NAHeapFragment::setHeadBits(size_t bits) { head_ |= bits; }

// Does this fragment occupy the whole NABlock?
inline NABoolean NAHeapFragment::occupiesCompleteNABlock() {
  return ((head_ & FIRST_FRAGMENT_BIT) != 0 && prevFoot_ == fragmentSize());
}

// Return the address of the header.
inline const size_t *NAHeapFragment::getHeadAddr() { return &head_; }

// Set the "next_" pointer in the NAHeapFragment
inline void NAHeapFragment::setNext(NAHeapFragment *next) { next_ = next; }

// Set the "prev_" pointer in the NAHeapFragment
inline void NAHeapFragment::setPrev(NAHeapFragment *prev) { prev_ = prev; }

// Return the "next_" pointer.
inline NAHeapFragment *NAHeapFragment::getNext() { return next_; }

// Return the "prev_" pointer.
inline NAHeapFragment *NAHeapFragment::getPrev() { return prev_; }

// Return the size of the previous fragment (only valid if free).
inline size_t NAHeapFragment::getPrevFoot() { return prevFoot_; }
...
inline NAHeapFragment *NAHeapFragment::fragmentMinusOffset(size_t s) { return (NAHeapFragment *)((char *)this - s); }

// Return a pointer to the next fragment
inline NAHeapFragment *NAHeapFragment::nextFragment() {
  return (NAHeapFragment *)((char *)this + (head_ & ~(INUSE_BITS | MMAPPED_BIT | FIRST_FRAGMENT_BIT)));
}

// Return a pointer to the previous fragment.  This code assumes
// that the caller has checked whether the previous fragment is
// not in use.
inline NAHeapFragment *NAHeapFragment::prevFragment() { return (NAHeapFragment *)((char *)this - prevFoot_); }

// Return whether the next fragment has PINUSE_BIT set.
inline NABoolean NAHeapFragment::nextPinuse() { return nextFragment()->pinuse(); }

// Set the size at the footer.
inline void NAHeapFragment::setFoot(size_t s) {
...
inline void NAHeapFragment::setSizeAndPinuseOfFreeFragment(size_t s) {
  head_ = (s | PINUSE_BIT);
  setFoot(s);
}

// Set size, pinuse bit, foot, and clear next pinuse
inline void NAHeapFragment::setFreeWithPinuse(size_t s, NAHeapFragment *next) {
  next->clearPinuse();
...
// fragment.
inline void NAHeapFragment::setInuseAndPinuse(size_t s) {
...
inline void NAHeapFragment::setSize(size_t s) { head_ = s; }

// Set the size and pinuse bits
inline void NAHeapFragment::setSizeAndPinuse(size_t s) { head_ = (s | PINUSE_BIT); }

// Set size, cinuse and pinuse bit of this fragment.
inline void NAHeapFragment::setSizeAndPinuseOfInuseFragment(size_t s) { head_ = (s | PINUSE_BIT | CINUSE_BIT); }

// Return whether the head is a fencepost
inline NABoolean NAHeapFragment::isFencePost() {
  return (head_ == FENCEPOST_HEAD ||
          (head_ == (sizeof(size_t) | CINUSE_BIT) && (fragmentPlusOffset(sizeof(size_t)))->head_ == FENCEPOST_HEAD));
}

// Set fenceposts after the top fragment as follows
//  this->
//       prevfoot_: do not care
//       head_:  1010 (sizeof(size_t)=sizeof(8)=1000, CINUSE=10), or 0xA
//       next_:  1011 as next_ is next->head_, or 0xB
//       prev_: do not care
inline void NAHeapFragment::setFencePosts() {
...
// using mmap().
inline NABoolean NAHeapFragment::isMMapped() { return (head_ & MMAPPED_BIT) != 0; }

// Set the size in the head with the MMAP bit and initialize the trailing
// fenceposts.
inline void NAHeapFragment::setSizeOfMMapFragment(size_t size) {
...
inline void NAHeapFragment::cleanFreePages(size_t fragSize) {}

// Get either the left or right child of an NATreeFragment
inline NATreeFragment *NATreeFragment::getChild(UInt32 childNo) { return child_[childNo]; }

// Get address of either the left or right child of an NATreeFragment
inline NATreeFragment **NATreeFragment::getChildAddr(UInt32 childNo) { return &child_[childNo]; }
...
inline void NATreeFragment::setParent(NATreeFragment *p) { parent_ = p; }

// Return the index of an NATreeFragment.  This is the index into
// NAHeap.treebins_[].
inline NAHeap::bindex_t NATreeFragment::getIndex() { return index_; }

// Set the index of the NATreeFragment.
inline void NATreeFragment::setIndex(NAHeap::bindex_t idx) { index_ = idx; }

// Get/Set the freedNSKMemory_ flag

inline short NATreeFragment::getFreedNSKMemory() { return freedNSKMemory_; }
...
int NAMemory::getVmSize() {
  ...
  lastVmSize_ = memSize;
  if (memSize > maxVmSize_) maxVmSize_ = memSize;
  return memSize;
}
...
NABlock *NAMemory::allocateMMapBlock(size_t s) {
  NABlock *blk;
  blk = (NABlock *)mmap(0, s, (PROT_READ | PROT_WRITE), (MAP_PRIVATE | MAP_ANONYMOUS), -1, 0);
  if (blk == (NABlock *)-1) {
    mmapErrno_ = errno;
    return NULL;
  } else
...
inline void NAMemory::sysFreeBlock(NABlock *blk) {
  if (blk->isMMapped())
    deallocateMMapBlock(blk);
  else if (getSharedMemory()) {
    if (segmentDB_) segmentDB_->freeSharedSegment(blk);
  } else
    free((void *)blk);
}
...
NAMemory::NAMemory(const char *name)
  ...
  NAStringRef *tmp = new ((void *)(&nullNAStringRep_[3])) NAStringRef(NAStringRef::NULL_CTOR, this);
}

NAMemory::NAMemory(const char *name, NAHeap *parent, size_t blockSize, size_t upperLimit)
    : type_(DERIVED_MEMORY),
      ...
      allocFailed_(FALSE),
      segmentDB_(NULL) {
  if (parent_->getSharedMemory()) setSharedMemory();

  if (parent->useSemaphore()) setUseSemaphore();
...
NAMemory::NAMemory(const char *name, NAMemoryType type, size_t blockSize, size_t upperLimit)
    : type_(type),
      maximumSize_((size_t)-1),  // no maximum
      parent_(NULL),
      firstBlk_(NULL),
...
void NAMemory::initMe(const char *name, SEG_ID segmentId, void *baseAddr, off_t heapStartOffset, size_t maxSize,
                      NABoolean failureIsFatal, NABoolean firstBlockIsSticky) {
  type_ = EXECUTOR_MEMORY, parent_ = NULL;
  firstBlk_ = NULL;
  allocSize_ = 0;
  upperLimit_ = 0;
  highWaterMark_ = 0;
  ...
  lastVmSize_ = 0l;
  maxVmSize_ = 0l;
  sharedMemory_ = TRUE;  // assume shared, since we are using a segmentId
  useSemaphore_ = FALSE;
  ...
  if (heapStartAddr_ != NULL) {
    ...
    if (tsize > (8 * sizeof(size_t))) {
      ...
      if (firstBlockIsSticky) firstBlk_->sflags_ |= NABlock::STICKY_BIT;
    }
  }
  upperLimit_ = maxSize;
  // need to initialize an NAStringRef object "on top" of the array
...
NAMemory::NAMemory(const char *name, SEG_ID segmentId, void *baseAddr, off_t heapStartOffset, size_t maxSize,
                   NABoolean failureIsFatal) {
  NAMemory::initMe(name, segmentId, baseAddr, heapStartOffset, maxSize, failureIsFatal);
}

NAMemory::NAMemory(const char *name, SEG_ID segmentId, void *baseAddr, off_t heapStartOffset, size_t maxSize,
                   NamedSemaphore *semaphore, NABoolean failureIsFatal) {
  NAMemory::initMe(name, segmentId, baseAddr, heapStartOffset, maxSize, failureIsFatal);

  namedSemaphore_ = semaphore;
  namedSem_ = NULL;
}

NAMemory::NAMemory(const char *name, SEG_ID segmentId, void *baseAddr, off_t heapStartOffset, size_t maxSize,
                   LockController *controller, NABoolean failureIsFatal, SharedSegmentDB *segmentDB) {
  NAMemory::initMe(name, segmentId, baseAddr, heapStartOffset, maxSize, failureIsFatal, !!segmentDB);

  lockController_ = controller;

  segmentDB_ = segmentDB;
}

void NAMemory::reInitialize() {
  // delete all blocks (except the sticky block) allocated
  // for this heap and re-set the heap to the initial values
    ...
    switch (type_) {
      ...
      case IPC_MEMORY: {
        while (p) {
          ...
          if (!p->isStickySegment() && !p->isExternSegment())
            sysFreeBlock(p);
          else {
            if (!externSegment) externSegment = p;
          }
      ...
      case DERIVED_MEMORY: {
        ...
        // memory segment.
        short semRetcode = 0;
        NABoolean namedSemaphoreLocked = FALSE;
        if (parent_->getType() == EXECUTOR_MEMORY && useSemaphore()) {
          namedSemaphoreLocked = checkIfNamedSemaphoreLocked();

          if (!namedSemaphoreLocked) {
        ...
        if (!namedSemaphoreLocked && semRetcode == 1) releaseRTSSemaphore();

        if (parent_->type_ == NAMemory::EXECUTOR_MEMORY && parent_->parent_ == NULL) {
          updateMemStats();
        }
        HEAPLOG_ON()
      } break;
  ...
  allocSize_ = highWaterMark_ = intervalWaterMark_ = allocCnt_ = totalSize_ = blockCnt_ = 0;
  crowdedTotalSize_ = 0ll;
  thBlockCnt_ = DEFAULT_THRESHOLD_BLOCK_COUNT;
  ...
  // firstBlk_ correctly.
  if (derivedClass_ == NAHEAP_CLASS) ((NAHeap *)this)->reInitializeHeap();
}

NAMemory::~NAMemory() {}

void NAMemory::setType(NAMemoryType type, size_t blockSize) {
  ...
  switch (type_) {
    ...
    case DERIVED_FROM_SYS_HEAP: {
      ...
      if (blockSize == 0) blockSize = (size_t)32768;
      initialSize_ = incrementSize_ = blockSize;
      maximumSize_ = (size_t)-1;  // no maximum
    } break;
    default:
...
inline void NAMemory::incrementStats(size_t size) {
  // if ((long)this <= 1342177280 && (long)this >= 1073741824 && size!= 136)
  //  QRINFO("NAMemory::incrementStats: the adress %p is %ld, increment %ld", this, allocSize_, size);
  allocSize_ += size;
  allocCnt_++;
  if (highWaterMark_ < allocSize_) highWaterMark_ = allocSize_;
  if (intervalWaterMark_ < allocSize_) intervalWaterMark_ = allocSize_;
}

// decrementStats() is called when memory is deallocated
inline void NAMemory::decrementStats(size_t size) {
  // if ((long)this <= 1342177280 && (long)this >= 1073741824 && size!= 136)
  //  QRINFO("NAMemory::decrementStats: the adress %p is %ld, decrement %ld",  this, allocSize_, size);
  allocSize_ -= size;
  allocCnt_--;
}
#endif  // !MUSE
...
inline NABoolean NAHeap::isAligned(void *addr) { return ((size_t)addr & (FRAG_ALIGN_MASK)) == 0; }

// Round a size up to the next byte alignment size.
inline size_t NAHeap::granularityAlign(size_t size) { return (size + FRAG_BYTE_ALIGN) & ~(FRAG_BYTE_ALIGN - 1); }

// Check if address is at least the minimum.
inline NABoolean NAHeap::okAddress(void *addr) { return ((char *)addr >= least_addr_); }

// Check if address of next fragment n is higher than base fragment p
inline NABoolean NAHeap::okNext(NAHeapFragment *p, NAHeapFragment *n) { return ((char *)p < (char *)n); }

// Check if p has its cinuse bit on
inline NABoolean NAHeap::okCinuse(NAHeapFragment *p) { return p->cinuse(); }

// Check if p has its pinuse bit on
inline NABoolean NAHeap::okPinuse(NAHeapFragment *p) { return p->pinuse(); }

// bit corresponding to given index
inline NAHeap::binmap_t NAHeap::idx2bit(bindex_t idx) { return (binmap_t)1 << idx; }
...
// bit is set.
inline NAHeap::bindex_t NAHeap::bit2idx(binmap_t x) {
#if defined(i386)
  UInt32 ret;
  __asm__("bsfl %1,%0\n\t" : "=r"(ret) : "rm"(x));
...
inline NABoolean NAHeap::isSmall(size_t size) { return (size >> SMALLBIN_SHIFT) < NSMALLBINS; }

// Return the small bin index for this size
inline NAHeap::bindex_t NAHeap::smallIndex(size_t size) { return (size >> SMALLBIN_SHIFT); }

// Return the maximum size for this small bin index
inline size_t NAHeap::smallIndex2Size(bindex_t idx) { return (idx << SMALLBIN_SHIFT); }

// Set a bit in the small map to indicate this bin contains free fragments
inline void NAHeap::markSmallmap(bindex_t idx) { smallmap_ |= idx2bit(idx); }

// Clear a bit in the small map to indicate this bin does not contain
// free fragments
inline void NAHeap::clearSmallmap(bindex_t idx) { smallmap_ &= ~idx2bit(idx); }

// Return whether the bit for this index is set in the small map.
inline NABoolean NAHeap::smallmapIsMarked(bindex_t idx) { return (smallmap_ & idx2bit(idx)) != 0; }

// Mark a bit in the tree map to indicate the tree contains free fragments
inline void NAHeap::markTreemap(bindex_t idx) { treemap_ |= idx2bit(idx); }

// Clear a bit in the tree map to indicate the tree does not contain
// free fragments
inline void NAHeap::clearTreemap(bindex_t idx) { treemap_ &= ~idx2bit(idx); }

// Return whether a bit for this index is set in the tree map
inline NABoolean NAHeap::treemapIsMarked(bindex_t idx) { return (treemap_ & idx2bit(idx)) != 0; }

// Compute the tree index for a given size
inline NAHeap::bindex_t NAHeap::computeTreeIndex(size_t size) {
...
inline NAHeap::binmap_t NAHeap::leastBit(binmap_t bits) { return bits & -(bits); }

// Mask with all bits to left of least bit of "bits" on
inline NAHeap::binmap_t NAHeap::leftBits(binmap_t bits) { return (bits << 1) | -(bits << 1); }
...
inline UInt32 NAHeap::leftshiftForTreeIndex(bindex_t idx) {
  if (idx == NTREEBINS - 1)
    return 0;
  else
...
inline size_t NAHeap::minsizeForTreeIndex(bindex_t idx) {
  return ((1 << ((idx >> 1) + TREEBIN_SHIFT)) | (((size_t)(idx & 1)) << ((idx >> 1) + TREEBIN_SHIFT - 1)));
}

// Return the address of a small bin for a particular index
inline NAHeapFragment *NAHeap::smallbinAt(bindex_t idx) { return (NAHeapFragment *)&smallbins_[idx << 1]; }

// Return the head of a tree bin for a particular index
inline NATreeFragment **NAHeap::treebinAt(bindex_t idx) { return &treebins_[idx]; }

#ifndef MUSE
// Initialize bins
void NAHeap::initBins() {
  ...
  for (i = 0; i < NTREEBINS; ++i) *treebinAt(i) = 0;
}

// Insert a small fragment into the small bins
inline void NAHeap::insertSmallFragment(NAHeapFragment *p, size_t size) {
...
inline void NAHeap::unlinkSmallFragment(NAHeapFragment *p, size_t size) {
...
inline void NAHeap::unlinkFirstSmallFragment(NAHeapFragment *binPtr, NAHeapFragment *fragPtr, bindex_t idx) {
...
inline void NAHeap::insertLargeFragment(NATreeFragment *treeFrag, size_t size) {
    ...
    *treebinPtr = treeFrag;
    ...
    for (;;) {
      if (treePtr->fragmentSize() != size) {
        NATreeFragment **childPtr = treePtr->getChildAddr((sizebits >> (SIZE_T_BITSIZE - 1)) & 1);
        sizebits <<= 1;
        if (*childPtr != 0)
          treePtr = *childPtr;
...
inline void NAHeap::unlinkLargeFragment(NATreeFragment *treeFrag) {
  ...
  if (treeFrag->getPrev() != treeFrag) {
    ...
    if (((prev = *(rightmostPtr = treeFrag->getChildAddr(1))) != 0) ||
      ...
      while ((*(childPtr = prev->getChildAddr(1)) != 0) || (*(childPtr = prev->getChildAddr(0)) != 0)) {
  ...
  if (parent != 0) {
    ...
    if (treeFrag == *treebinPtr) {
      if ((*treebinPtr = prev) == 0) clearTreemap(treeFrag->getIndex());
...
inline void NAHeap::insertFragment(NAHeapFragment *p, size_t size) {
  if (isSmall(size)) {
...
// large trees
inline void NAHeap::unlinkFragment(NAHeapFragment *p, size_t size) {
  if (isSmall(size)) {
...
inline void NAHeap::replaceDV(NAHeapFragment *p, size_t size) {
  if (dvsize_ != 0) insertSmallFragment(dv_, dvsize_);
...
inline void NAHeap::resizeTop(size_t newsize) {
  topsize_ = newsize;
...
inline void NAHeap::initTop(NABlock *block) {
...
void NAHeap::addBlock(NABlock *newBlock) {
  // Insert the old top into a bin as an ordinary free fragment, since
  // this method is called only after a new block is allocated.
  NAHeapFragment *next = NULL;
  if (topsize_ != 0) {
    size_t firstFragBit = top_->getFirstFragmentBit();
    next = top_->fragmentPlusOffset(topsize_);

    top_->setFreeWithPinuse(topsize_, next);

    // Set the fence post immediately after the old top fragment.
    next->setFencePosts();
...
// Even if the block was allocated externally, we still need to do
// the bookkeeping.
NABoolean NAHeap::deallocateFreeBlock(NAHeapFragment *p) {
  NABlock *prev = NULL;
  ...
  switch (type_) {
    ...
    case DERIVED_MEMORY:
      ...
      // memory segment.
      if (parent_->getType() == EXECUTOR_MEMORY && useSemaphore()) {
        NABoolean namedSemaphoreLocked = checkIfNamedSemaphoreLocked();
        ...
        if (!namedSemaphoreLocked) {
          ...
          if (retcode == 1) releaseRTSSemaphore();
...
NAHeapFragment *NAHeap::tmallocLarge(size_t nb) {
...
NAHeapFragment *NAHeap::tmallocSmall(size_t nb) {
...
void NAMemory::unRegisterMemory(NAMemory *child) {
  ...
  if (child == lastListEntry_) lastListEntry_ = q;
}
...
NABlock *NAHeap::allocateBlock(size_t size, NABoolean failureIsFatal) {
  assert(type_ != NO_MEMORY_TYPE);

  // if a limit is specified for this heap, make sure we don't pass it
  if ((IdentifyMyself::GetMyName() == I_AM_EMBEDDED_SQL_COMPILER) && (upperLimit_ != 0) &&
  ...
  if ((type_ == DERIVED_MEMORY || type_ == DERIVED_FROM_SYS_HEAP) && thBlockCnt_ == (blockCnt_ + 1) &&
      incrementSize_ < DEFAULT_MAX_INCREMENT) {
    // next adjustment when we get 10 more blocks.
    incrementSize_ *= 2;
    if (incrementSize_ > DEFAULT_MAX_INCREMENT) incrementSize_ = DEFAULT_MAX_INCREMENT;
    thBlockCnt_ += DEFAULT_THRESHOLD_BLOCK_COUNT;
  }
    ...
    case EXECUTOR_MEMORY: {
      if (getSharedMemory()) {
        ...
        if (!segmentDB_)
          ...
          // attempt to create a shared segment at least in size
          // blockSize, for use as the new block. The actual size
          // of the segment 'segSize' can be much large than blockSize.
          size_t segSize = 0;
          SharedSegment *segment = segmentDB_->createSharedSegment(blockSize);

          if (!segment) return NULL;

          blockCnt_++;
          totalSize_ += segment->getSize();

          // Initialize the returned NABlock information
          p = (NABlock *)(segment->getAddr());
          p->segmentId_ = segment->getShmid();
          p->size_ = segment->getSize();
          p->next_ = NULL;  // The caller is responsible for linking this
          p->sflags_ = NABlock::EXTERN_BIT;
        ...
        if (blockSize >= MIN_MMAP_ALLOC_SIZE && (p = allocateMMapBlock(blockSize)) != NULL) return p;

        // allocate a block from the OS memory
        p = (NABlock *)malloc(blockSize);
        if (p) allocationIncrement(blockSize);
      }
    } break;
...
void NAMemory::showStats(int level) {
  ...
  for (NAMemory *p = memoryList_; p != NULL; p = p->nextEntry_) p->showStats(level + 1);
}

// this method used to belong to CollHeap
#ifndef MUSE
void NAMemory::handleExhaustedMemory() { exhaustedMem_ = TRUE; }
#endif  // MUSE
...
NABoolean NAMemory::checkSize(size_t size, NABoolean failureIsFatal) {
  size_t limit = (upperLimit_ == 0) ? MAX_MEMORY_SIZE_IN_AN_ALLOC : upperLimit_;

  // If segmentDB_ is avaiable, assume that it can allocate some
  // memory.
  if (size > limit && !segmentDB_) {
    if (failureIsFatal && failureIsFatal_)
      abort();
    else {
      allocFailed_ = TRUE;
      return FALSE;
...
NAHeap::NAHeap(const char *name, NAHeap *parent, size_t blockSize, size_t upperLimit)
  ...
  derivedClass_ = NAHEAP_CLASS;
  if (parent != NULL) {
    NAMutexScope mutex(parent->mutex_);
...
NAHeap::NAHeap(const char *name, NAMemoryType type, size_t blockSize, size_t upperLimit)
...
NAHeap::NAHeap(const char *name, SEG_ID segmentId, void *baseAddr, off_t heapStartOffset, size_t maxSize,
               NABoolean failureIsFatal)
    : NAMemory(name, segmentId, baseAddr, heapStartOffset, maxSize, failureIsFatal),
      smallmap_(0),
      treemap_(0),
      dvsize_(0),
      topsize_(0),
...
NAHeap::NAHeap(const char *name, SEG_ID segmentId, void *baseAddr, off_t heapStartOffset, size_t maxSize,
               NamedSemaphore *semaphore, NABoolean failureIsFatal)
    : NAMemory(name, segmentId, baseAddr, heapStartOffset, maxSize, semaphore, failureIsFatal),
      smallmap_(0),
      treemap_(0),
      dvsize_(0),
      topsize_(0),
...
// Constructor that imposes the NAHeap struture on already allocated memory
NAHeap::NAHeap(const char *name, SEG_ID segmentId, void *baseAddr, off_t heapStartOffset,
               size_t maxSize,  // This is the initial size of the segment
                                // with segmentId.
               LockController *controller, NABoolean failureIsFatal, SharedSegmentDB *segmentDB)
    : NAMemory(name, segmentId, baseAddr, heapStartOffset, maxSize, controller, failureIsFatal, segmentDB),
      smallmap_(0),
      treemap_(0),
      dvsize_(0),
      topsize_(0),
...
void NAHeap::destroy() {
#ifdef _DEBUG
  if (la_ && TraceAllocSize > 0) {
    char header[120];
    sprintf(header, "Stacks when block of %d bytes was asked from heap (%s)", TraceAllocSize, getName());
  ...
  if (parent_) {
    NAMutexScope mutex(parent_->mutex_);
...
void NAHeap::reInitializeHeap() {
  ...
  if (debugLevel_ == 2) checkForOverflow();
#endif
...
// being allocated.
void NAHeap::setErrorCallback(void (*errCallback)(NAHeap *, size_t)) { errCallback_ = errCallback; }
...
void *NAHeap::allocateAlignedHeapMemory(size_t userSize, size_t alignment, NABoolean failureIsFatal) {
  ...
  if (retAddr == (size_t)NULL) return NULL;
  alignedAddr = retAddr & ~(alignment - 1);
  if (alignedAddr != retAddr) {
    alignedAddr += alignment;
    if ((alignedAddr - retAddr) < (sizeof(size_t) * 2)) {  // need to double the alignment to avoid override header info
      deallocateHeapMemory((void *)retAddr);
      retAddr = (size_t)allocateHeapMemory(userSize + 2 * alignment, failureIsFatal);
      if (retAddr == (size_t)NULL) return NULL;
      alignedAddr = retAddr & ~(alignment - 1);
      if (alignedAddr == retAddr) return (void *)alignedAddr;
      alignedAddr += alignment;  // move to aligned addr inside the fragment
      if ((alignedAddr - retAddr) < (sizeof(size_t) * 2)) alignedAddr += alignment;  // move to next alignment
    }
...
void *NAHeap::allocateHeapMemory(size_t userSize, NABoolean failureIsFatal) {
#if 0
   int topsize = (top_) ? top_->fragmentSize() : -1;

   cout << "Calling NAHeap::allocateHeapMemory():"
        << " requested size=" << userSize
        << ", small bins=" 
        << std::hex << smallmap_ << std::dec 
        << ", tree bins=" 
        << std::hex << treemap_ << std::dec 
        << ", dv_=" << static_cast<void*>(dv_)
        << ", dvsize_=" << dvsize_;

   if ( top_ ) {
    NAHeapFragment* next = top_->fragmentPlusOffset(topsize);

    cout << "; top frag: "
        << "size=" << topsize
        << ", addr=" << static_cast<void*>(top_)
        << ", end addr=" << static_cast<void*>(next);
   }
 
   cout << endl;

   checkAllSmallbins("When entering NAHeap::allocateHeapMemory()");
#endif
  ...
  if (!checkSize(userSize, failureIsFatal)) return NULL;

  // needToCheckIfLocked() check alone is enough since it will return for both
  // global and process stats heap. Leaving the rest of the condition here
  //
  if (needToCheckIfLocked()) {
    // Check if you are within semaphore
    if (!checkIfLocked()) NAExit(1);
  }
...
#if (defined(_DEBUG) || defined(NSK_MEMDEBUG))
  if (debugLevel_) dynamicAllocs.addEntry(userSize);
  ...
  if (additionalUserSize >= MIN_MMAP_ALLOC_SIZE && parent_ == NULL && (!getSharedMemory())) {
    ...
    if (mmappedBlock != NULL) {
      ...
      if (least_addr_ == NULL || (char *)mmappedBlock < least_addr_) least_addr_ = (char *)mmappedBlock;
  ...
  if (additionalUserSize <= MAX_SMALL_REQUEST) {
    nb = (additionalUserSize < MIN_REQUEST) ? MIN_FRAGMENT_SIZE : PAD_REQUEST(additionalUserSize);
    ...
    if ((smallbits & 0x3) != 0) {  // Remainderless fit to a smallbin.
      ...
      if (!(p->fragmentSize() == fragSize)) assert(p->fragmentSize() == fragSize);
      unlinkFirstSmallFragment(b, p, idx);
      p->setInuseAndPinuse(fragSize | firstFragBit);
#ifdef _DEBUG
      if (p->fragmentSize() == TraceAllocSize) saveTrafStack(getSAL(), p);
#endif  // _DEBUG
...
#if 0
   checkAllSmallbins("Allocation done in remainderless smallbin");
#endif
...
#if 0
         checkAllSmallbins("Allocation done in next nonempty remainderless smallbin");
#endif
...
#if 0
        checkAllSmallbins("Allocation done in smallest tree fragment.");
#endif
        return p->getMemory(cleanUpPrevNext);
      }
    }
...
#if 0
     checkAllSmallbins("Allocation done in tree fragment via tmallocLarge()");
#endif
...
#if 0
  cout << "Can not get " << nb << " bytes" 
       << " from dv or bins. Try the top frag." << endl;
#endif
  ...
  if (nb > topsize_) {
    ...
    if (newBlock == NULL) {
      ...
      if (errCallback_ != NULL) (*errCallback_)(this, userSize);
...
#if 0
   cout << "Top frag size is not big enough. Allocate a new block." 
        << ", block=" << static_cast<void*>(newBlock)
        << ", size=" << newBlock->size_
        << ", endPtr=" << static_cast<void*>((char*)newBlock+newBlock->size_-1)
        << endl;
#endif

    if (least_addr_ == NULL || (char *)newBlock < least_addr_) least_addr_ = (char *)newBlock;
  ...
  if (rsize >= MIN_FRAGMENT_SIZE) {
...
#if 0
    cout << "Top frag is big enough to split into two: P and R. " 
     << " P: addr=" << static_cast<void*>(p)
     << ", size =" << nb
     << " R: addr=" << static_cast<void*>(r)
     << ", size of R=" << rsize
     << endl;
#endif

    // top_ before the split
    // |
    // |       top_ after the split
    // |       |
    // v       v
    // |---------------------------|
    // |       |                   |
    ...
    // |       |
    // p       r
    // p points at the memory span to be used by the caller.
    // r points at the remaining memory span

    // set r->head_=rsize | P_BIT
    r->setSizeAndPinuse(rsize);

    //
    // firstFragmentBit=0 if not first fragment in the block
    //                  1 if first fragment in the block;
    //
    // set p->head_=rsize | P_BIT | C_BIT | firstFragmentBit
    p->setSizeAndPinuseOfInuseFragment(nb | firstFragmentBit);
  } else {
#if 0
    cout << "Top frag is not enough, use all of it" << endl;
#endif
...
void NAHeap::deallocateHeapMemory(void *addr) {
...
#ifdef _DEBUG
  if (TraceAllocSize > 0 && p->fragmentSize() == TraceAllocSize) delTrafStack(getSAL(), p);
#endif  // _DEBUG
  ...
  if (type_ == EXECUTOR_MEMORY && deallocTraceArray != NULL && (memcmp(name_, "Global Executor Memo", 20) == 0)) {
    deallocCount += 1;
  ...
  if ((!next->cinuse()) || ((next == top_) && (next->isFencePost()) && (topsize_ == 0))) {  // consolidate forward
  ...
  if (p->occupiesCompleteNABlock()) {
    ...
    if (deallocateFreeBlock(p)) return;
  }
...
void NAHeap::dumpHeapInfo(ostream *outstream, int indent, bool doCheck, bool dumpFragmentDetails) {
  ...
  if (doCheck) checkMallocState();

  if (!outstream) outstream = &cerr;
  *outstream << ind << "Dump of Heap             " << this << ':' << endl
             << ind << "Name:                    " << name_ << endl
             << ind << "Parent:                  " << ind << (parent_ ? parent_->name_ : " ") << endl
             ...
             << ind << "bitmap for small bins:   " << std::hex << smallmap_ << std::dec << endl
             << ind << "bitmap for large bins:   " << std::hex << treemap_ << std::dec << endl;

  MemoryStats freeStats;
  MemoryStats allocStats;
  NABlock *p = firstBlk_;
  while (p) {
    *outstream << "Block: start addr=" << static_cast<void *>(p)
               << ", end addr=" << static_cast<void *>(p + p->size_ - 1) << ", size=" << p->size_ << " bytes" << endl;
    p->dump(outstream, debugLevel_, freeStats, allocStats, top_, indent, dumpFragmentDetails);
    p = p->next_;
  }
  ...
  bool dumpStats = !dumpFragmentDetails;
  if (dumpStats) {
...
void NABlock::dump(ostream *outstream, int debugLevel, MemoryStats &freeStats, MemoryStats &allocStats,
  ...
  while (blockHolds(q) && q != top && !q->isFencePost()) {
    if (dumpFragmentDetails) {
      *outstream << "fragment: " << static_cast<void *>(q) << ", cinuse=" << q->cinuse() << ", pinuse=" << q->pinuse()
                 << ", size=" << q->fragmentSize() << ", nextFrag=" << static_cast<void *>(q->nextFragment()) << endl;
    }
...
void MemoryStats::dump(ostream *outstream, const char *name, int indent) {
  ...
  *outstream << ind << name << " units:" << endl
             ...
             << ind << "average size: " << (count_ ? (sum_ / count_) : 0.0) << endl
             << ind << "variance:     "
  ...
  for (int i = 0; i < 17; i++) {
    ...
    *outstream << ind << "# entries <= " << size << unit << " : " << histBuckets_[i] << endl;
  }
  *outstream << ind << "# entries >  " << size << unit << " : " << histBuckets_[17] << endl;
}
#endif  // (defined(_DEBUG) || defined(NSK_MEMDEBUG))
...
void NAHeap::doAllocDebugProcess(NAHeapFragment *p, size_t userSize) {
  ...
  if (debugLevel_ == 2 && p->fragmentSize() < INT_MAX) {
    ...
    // requested memory
    UInt32 *lastWord = (UInt32 *)((char *)p->getMemory() + ((userSize - 1) & ~(0x3)));
    ...
    lastWord[0] = (lastWord[0] & ~overflow_mask[remainder]) | (buf_overflow_val & overflow_mask[remainder]);
...
void NAHeap::doDeallocDebugProcess(NAHeapFragment *p) {
  if (debugLevel_ == 2 && p->fragmentSize() < INT_MAX) {
...
void NAHeap::doCheckAnyFragment(NAHeapFragment *p) {
  assert((isAligned(p->getMemory())) || (p->isFencePost()));
  assert(okAddress(p));
}

// Check properties of top fragment
void NAHeap::doCheckTopFragment(NAHeapFragment *p) {
  ...
  if (topsize_ == 0) {
    ...
    // Check that the fragment size plus its offset and MIN_FRAGMENT_SIZE
    // is no more than the block size. Note that 'sz' is rounded down to
    // a multiple of FRAG_BYTE_ALIGN (8 bytes for Linux). Please refer to
    // NAHeap::initTop() on how topSize_ is computed.
    //
    // |                   |         |                       |                  |
    // |    NABlock        |alignment|  firstFragment Object | memory in        |
    // |                   |         |                       |   firstFragment  |
    // |                   |         |                       |                  |
    // |<-  firstFragmentOffset    ->|<- MIN_FRAGMENT_SIZE ->|<-     sz       ->|
    // |                   |         |                       |                  |
    // |<---------------------    sp->size   ---------------------------------->|
    assert(sp->firstFragmentOffset() + MIN_FRAGMENT_SIZE + sz <= sp->size_);
...
void NAHeap::doCheckInuseFragment(NAHeapFragment *p) {
...
void NAHeap::doCheckFreeFragment(NAHeapFragment *p) {
  size_t sz = p->fragmentSize();
  ...
  if (p != dv_ && p != top_) {
    if (sz >= MIN_FRAGMENT_SIZE) {
      ...
      assert(next == top_ || next->cinuse() || next->isFencePost());
...
void NAHeap::doCheckMallocedFragment(NAHeapFragment *p, size_t s) {
  if (p != 0) {
    size_t sz = p->fragmentSize();
...
void NAHeap::doCheckTree(NATreeFragment *t) {
  ...
  do {  // traverse through chain of same-sized nodes
    ...
    if (u->getParent() == NULL) {
      ...
      assert(u->getParent()->getChild(0) == u || u->getParent()->getChild(1) == u ||
      ...
      if (u->getChild(0) != NULL && u->getChild(1) != NULL) {
        assert(u->getChild(0)->fragmentSize() < u->getChild(1)->fragmentSize());
      }
    }
...
void NAHeap::doCheckTreebin(bindex_t i) {
  NATreeFragment **tb = treebinAt(i);
  NATreeFragment *t = *tb;
  NABoolean empty = (treemap_ & (1U << i)) == 0;
  if (t == 0) assert(empty);
  if (!empty) doCheckTree(t);
}

// Check all the fragments in a smallbin
void NAHeap::doCheckSmallbin(bindex_t i) {
  NAHeapFragment *b = smallbinAt(i);
  ...
  if (!empty) {
    for (; p != b; p = p->getPrev()) {
      ...
      assert(p->getPrev() == b || p->getPrev()->fragmentSize() == p->fragmentSize());
      // fragment is followed by an inuse fragment
      q = p->nextFragment();
      if (!q->isFencePost()) doCheckInuseFragment(q);
    }
  }
}

void NAHeap::checkAllSmallbins(const char *msg) {
  cout << "Check all small bins " << msg << endl;

  for (int i = 0; i < NSMALLBINS; ++i) doCheckSmallbin(i);

  cout << "check done" << endl;
}

// Check all of the properties in NAMemory related to Doug Lea's malloc()
void NAHeap::doCheckMallocState() {
  bindex_t i;
  size_t total;

  // check bins
  for (i = 0; i < NSMALLBINS; ++i) doCheckSmallbin(i);
  for (i = 0; i < NTREEBINS; ++i) doCheckTreebin(i);
...
NABoolean NAHeap::binFind(NAHeapFragment *x) {
  ...
  if (isSmall(size)) {
    ...
    if (treemapIsMarked(tidx)) {
      ...
      if (t != 0) {
        NATreeFragment *u = t;
        do {
          if (u == (NATreeFragment *)x) return TRUE;
...
size_t NAHeap::traverseAndCheck(const char *msg) {
#if 0
  ...
  if (s != NULL) {
    ...
    while (s != NULL) {
#if 0
      cout << "NABlock=" << static_cast<void*>(s) << endl;
#endif
      ...
      // have the first fragment bit set. If the NABlock was allocated
...
#if 0
        cout << " fragment=" << static_cast<void*>(q);
        cout << ", cinuse()=" << q->cinuse();
        cout << ", pinuse()=" << q->pinuse();
        cout << ", size=" << q->fragmentSize() ;
        cout << ", nextFrag=" << static_cast<void*>(q->nextFragment());
        cout << endl;
#endif
...
#if 0
  out << "traverseAndCheck() done" << endl;
  out.close();
#endif
...
void NAHeapFragment::checkBufferOverflow() {
  ...
  UInt32 *lastWord = (UInt32 *)((char *)getMemory() + ((userSize - 1) & ~(0x3)));

  // Check for buffer overflow;
  UInt32 remainder = userSize & 0x3;
  if ((lastWord[0] & overflow_mask[remainder]) != overflow_val[remainder] || lastWord[1] != buf_overflow_val) {
    //    char buf[128];
  ...
  if (userSizePtr != &lastWord[2] && lastWord[2] != buf_overflow_val) assert(0);  // overflow detected
}

// Traverse over all of the blocks for this NAHeap to see if any of them contain
// buffers that have overflowed.
void NAHeap::checkForOverflow() {
...
DefaultIpcHeap::~DefaultIpcHeap() { destroy(); }
...
NABoolean NAHeap::isStmtHeap() {
  if (!strcmp(name_, "Cmp Statement Heap"))
    return true;
  else
    return false;
}

NABoolean NAHeap::isHQCHeap() {
  // The name is only 20 bytes long.
  if (!strncmp(name_, "hybrid query cache heap", 20))
    return true;
  else
    return false;
}

NABoolean NAHeap::isSQCHeap() {
  if (!strcmp(name_, "mxcmp cache heap"))
    return true;
  else
    return false;
}
...
void NAHeap::unLock() {
  if (mutex_.isEnabled()) mutex_.unlock();
}

void NAHeap::destoryAndInit() {
  if (mutex_.isEnabled()) mutex_.destoryAndInit();
}
...
NABoolean NAMemory::checkIfLocked() {
  if (checkIfDistributedLockLocked()) return TRUE;
  if (checkIfNamedSemaphoreLocked())
    return TRUE;
  else if (checkIfNamedSemLocked())
    return TRUE;
  else
    return checkIfRTSSemaphoreLocked();
}

NABoolean NAMemory::checkIfNamedSemaphoreLocked() { return (namedSemaphore_ && namedSemaphore_->semaphoreLocked()); }

NABoolean NAMemory::checkIfNamedSemLocked() { return (namedSem_ && namedSem_->semaphoreLocked()); }

NABoolean NAMemory::checkIfDistributedLockLocked() { return (lockController_ && lockController_->lockHeld()); }
