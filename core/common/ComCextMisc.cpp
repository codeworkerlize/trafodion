...
BOOL GetMicroseconds(long long *t, short type) {
  static THREAD_P BOOL initialized;

  static THREAD_P double counterResolutionToTimeResolution;

  static THREAD_P double microsecondsPerCounterTick;

  static THREAD_P double reasonableCounterDrift;

  static THREAD_P unsigned long long baseCounter;

  static THREAD_P unsigned long long baseMicroseconds;

  static THREAD_P unsigned long long coldLoadMicroseconds;

  static THREAD_P unsigned long long lastCounter;

  static THREAD_P unsigned long long lastTime;
  ...
  if (!initialized) {
    ...
    struct LargeInt counterFrequency;
    ...
    if (!GetSystemTimeAdjustment(&timeAdjustment, &timeAdjustmentPeriod, &adjusting)) return (FALSE);
    ...
    reasonableCounterDrift = ((double)(adjusting ? timeAdjustmentPeriod : 0)) * counterResolutionToTimeResolution;
    ...
    coldLoadMicroseconds =
        baseMicroseconds - ((long long)(((double)((long long)baseCounter)) / microsecondsPerCounterTick));
    ...
    *t = (long long)(currentCounter / microsecondsPerCounterTick);
    ...
    if (currentTime <= lastTime) {
      if ((currentTime < lastTime)  // Time must have been set back.
        ...
        coldLoadMicroseconds =
            baseMicroseconds - ((long long)(((double)((long long)currentCounter)) / microsecondsPerCounterTick));
    ...
    if (type == 0) {
      *t = currentTime / 10;
      ...
      if (type == 1) *t = coldLoadMicroseconds;
    }
...
extern "C" DLLEXPORT long INTERPRETTIMESTAMP(long long juliantimestamp, short *date_n_time)
...
#define mmss          _jdnhh.highpart
  ...
  const unsigned long DAYS_PER_400_YEARS = 146097L, DAYS_PER_4_YEARS = 1461L;
  ...
  temp2 = ((temp1 - ((century = temp1 / DAYS_PER_400_YEARS)) * DAYS_PER_400_YEARS) / 4) * 4 + 3L;

  yoc = (unsigned short)(temp2 / DAYS_PER_4_YEARS);

  temp3 = ((((unsigned short)(temp2 - (yoc * DAYS_PER_4_YEARS))) + 4) / 4) * 5 - 3;
...
extern "C" DLLEXPORT void CONTIME(short *a, short t, short t1, short t2)
...
long long LCTBias2(long long GMTtime, short *CC) {
  short ldt[8];
  long ljdn;
  long long loffset64;
  struct tm ltm;
...
extern "C" DLLEXPORT long long CONVERTTIMESTAMP(long long timestamp, short direction, short node, short *error) {
  DWORD timeZoneID = 0;
  ...
  if (direction == OMITSHORT) direction = 0;

  if (node == OMITSHORT) node = -1;
  ...
  if ((timestamp == OMIT__INT64) || (CHECK_LIMIT_JTS(timestamp))) {
...
void TIMESTAMP(short *a) {
  ...
  if (!GetMicroseconds(&timeunion.ftime, 0))
    ...
    // Convert to LCT.
    //
    timeunion.ftime = CONVERTTIMESTAMP((timeunion.ftime + MICROSOFT_EPOCH), 0, -1, &error);
...
extern "C" DLLEXPORT void TIME(short *a)
...
extern "C" DLLEXPORT long long TIME_SINCE_COLDLOAD(void)
//
...
extern "C" DLLEXPORT long long JULIANTIMESTAMP(short type, short *tuid, short *error, short node)
...
// long long  result            // output, the requested Julian timestamp
{
  ...
  if (type == OMITSHORT) type = 0;

  if (node == OMITSHORT) node = -1;

  if (node == -1) {  // local request
    switch (type) {
      case 0:  // GMT
      {
        if (!sv_envJulianTimestamp) {
          ...
          if (strUseLinuxJulianTimestamp != NULL) {
            if (atoi(strUseLinuxJulianTimestamp) == 0) sv_useLinuxJulianTimestamp = FALSE;
          }
    ...
    if (error != 0) *error = ((time == (long long)-1) ? -1 : 0);
  } else {
    time = (long long)-1;
...
extern "C" DLLEXPORT int INTERPRETINTERVAL(long long time,    // INPUT,  REQUIRED - time in microseconds
                                                              ...
                                                              // returned value   - days or -1 for error
{
  ...
  if (time < 0) return -1;  // time intervals can only be positive
  ...
  if (microsecs != NULL) *microsecs = (microsecs_ - milsecs_ * 1000);
  if (milsecs != NULL) *milsecs = (milsecs_);
  if (seconds != NULL) *seconds = (seconds_ - minutes_ * 60);
  if (minutes != NULL) *minutes = (minutes_ - hours_ * 60);
  if (hours != NULL) *hours = (hours_ - days_ * 24);
  return (days_);
}

extern "C" DLLEXPORT void INTERPRETJULIANDAYNO(long julianDayNo, short *year, short *month, short *day) {
  const unsigned long DAYS_PER_400_YEARS = 146097L, DAYS_PER_4_YEARS = 1461L;
  ...
  temp3 = ((((unsigned short)(temp2 - (year_of_century * DAYS_PER_4_YEARS))) + 4) / 4) * 5 - 3;
...
short RANGE_CHECKER(const short params[], const short lowlimit[], const short highlimit[], const short count) {
  short errors = 0;
  unsigned short errorbit = 0100000;  // Octal constant.

  for (short x = 0; (x < (count - 1)); x++) {
    if ((params[x] < lowlimit[x]) || (params[x] > highlimit[x])) {
...
extern "C" DLLEXPORT long long COMPUTETIMESTAMP(short *date_n_time, short *error)
// COMPUTETIMESTAMP computes a Julian timestamp from an integer array
// that represents a Gregorian date and time of day.

// const short date_n_time; // input,  required, array [8]
//         date_n_time [0] = year
//         date_n_time [1] = month
//         date_n_time [2] = day
//         date_n_time [3] = hours
...
{
  const short limits_set[16] = {1, 1, 1, 0, 0, 0, 0, 0, 10000, 12, 31, 23, 59, 59, 999, 999};
...
#define micros date_n_time[7]
  ...
  // zero.
  //
  if (date_error || (hour > limits_set[11]) || (minute > limits_set[12]) || (second > limits_set[13]) ||
      (millis > limits_set[14]) || (micros > limits_set[15])) {
    ...
    //
    if (error) {
      for (int i = 0; i < 16; i++) limits[i] = limits_set[i];

      *error = RANGE_CHECKER(date_n_time, limits, &limits[8], 8) | date_error;
    }
  ...
  return (((((long long)(jdn * 24L + ((unsigned long)(hour - 12)))) * 3600) + ((long long)(minute * 60 + second))) *
              1000000 +
...
extern "C" DLLEXPORT short NODENAME_TO_NODENUMBER_
...
extern "C" DLLEXPORT int_16 NODENUMBER_TO_NODENAME_(int sysnum,
#ifdef TDM_ROSETTA_COMPATIBILITY_
  ...
  *syslen = 4;
  ...
  if (maxlen < 4) return FEBUFTOOSMALL;
...
extern "C" DLLEXPORT int_16 PROCESSHANDLE_DECOMPOSE_(int_16 *prochand,     // INPUT
                                                     int *cpu,             // OUTPUT
                                                     int *pin,             // OUTPUT
                                                     int *node,            // OUTPUT  THE NODE NUMBER
  ...
  return XPROCESSHANDLE_DECOMPOSE_(phandle, cpu, pin, node, (char *)nn, nnml, nnl, (char *)name, nml, nl, seq);
}

extern "C" DLLEXPORT int_16 MYSYSTEMNUMBER() { return 0; }

extern "C" DLLEXPORT int_16 GETSYSTEMNAME(int_16 sysnum, int_16 *sysname) {
  if (sysnum) return 0;
...
extern "C" DLLEXPORT void DELAY(int nsktimeout) {
  DWORD timeout;
