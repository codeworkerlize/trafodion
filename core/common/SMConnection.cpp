...
// -----------------------------------------------------------------------
SMConnection::SMConnection(IpcEnvironment *env, sm_target_t &smTarget, UInt32 numReceiveBuffers,
                           IpcMessageObjSize maxBufSize, ex_tcb *tcb, ExMasterStmtGlobals *masterGlobals,
                           NABoolean isServer)
    : IpcConnection(env, IpcProcessId(),
                    isServer ? eye_SEAMONSTER_CONNECTION_TO_CLIENT : eye_SEAMONSTER_CONNECTION_TO_SERVER),
      scheduled_(0),
      isServer_(isServer),
  ...
  if (!isServer_) ExSMTag_SetReplyFlag(&targetForTask.tag);
  ...
  smTask_ = smGlobals->addTask(targetForTask, queueSize, &scheduled_, ipcEnv->getHeap(), tcb_, this);
  assert(smTask_);

  EXSM_TRACE(EXSM_TRACE_INIT, "CONN CTOR %s %p task %p", isServer_ ? "S" : "C", this, smTask_);
  EXSM_TRACE(EXSM_TRACE_INIT, "CONN CTOR %s %p max buf %d", isServer_ ? "S" : "C", this, (int)maxBufSize_);

  if (masterGlobals) masterGlobals->addSMConnection(this);
  ...
  if (myNode == smTarget_.node) intranode_ = true;
}

SMConnection::~SMConnection() {
  EXSM_TRACE(EXSM_TRACE_INIT, "CONN DTOR %s %p %s", isServer_ ? "S" : "C", this, getConnectionStateString(getState()));
  ...
  ExSMEvent::add(ExSMEvent::SMError, &smTarget_, smErrorNumber_, ccErrorNumber_, 0, (int64_t)this);
  ...
  if (masterGlobals_) masterGlobals_->removeSMConnection(this);
...
int32_t SMConnection::allocateReceiveBuffers() {
  ...
  while (inQueue->getLength() < numReceiveBuffers_) {
    ...
    IpcMessageBuffer *msgBuf = IpcMessageBuffer::allocate(maxBufSize_, env->getGuaMaxMsgIOSize(), NULL, heap, 0);
    assert(msgBuf);
    ...
    if (!isServer_) ExSMTag_SetReplyFlag(&target.tag);

    UInt32 maxSMChunkSize = ExSM_GetMaxChunkSize(intranode_);
    UInt32 bufferSize = msgBuf->getBufferLength();

    if (bufferSize > maxSMChunkSize) bufferSize = maxSMChunkSize;

    rc = ExSM_Post(target, bufferSize, msgBuf->data(), smTask_, isServer_);

    if (rc == SM_ERR_NOPEER || rc == SM_ERR_NOSERVICE || rc == SM_ERR_NODE_DOWN) return rc;

    exsm_assert_rc(rc, "ExSM_Post");
    postCount_++;
  }

  EXSM_TRACE(EXSM_TRACE_BUFFER, "CONN %p %s alloc recv bufs %d", this, isServer_ ? "server" : "client", count);
  return rc;
}
...
void SMConnection::send(IpcMessageBuffer *msgBuf) {
  ...
  EXSM_TRACE(EXSM_TRACE_BUFFER, "CONN %p BEGIN SEND %s state %s", this, (isServer_ ? "S" : "C"),
             getConnectionStateString(getState()));
  EXSM_TRACE(EXSM_TRACE_BUFFER, "CONN %p buf %p len %d ref %d", this, msgBuf, (int)msgBuf->getMessageLength(),
  ...
  EXSM_TRACE(EXSM_TRACE_BUFFER, "CONN %p END SEND state %s", this, getConnectionStateString(getState()));
}
...
bool SMConnection::tryToSendOneChunk() {
  EXSM_TRACE(EXSM_TRACE_BUFFER, "CONN %p TRY wait %d arr %d q_sz %d", this, (int)getWaitingForAck(),
             (int)getAckArrived(), (int)sendQueueEntries());
  ...
  if (getWaitingForAck()) {
    ...
    if (getAckArrived()) {
      ...
      if (chunk_nextOffset_ + chunk_size_ < msgLen) {
        ...
        exsm_assert(chunk_nextOffset_ < msgLen, "Chunk offset should not exceed message length");
      ...
      EXSM_TRACE(EXSM_TRACE_BUFFER, "CONN %p ACK ARRIVED buf %p offset %d bytes %d final %d", this, sendBuffer,
                 (int)sendBytes, (int)sendOffset, (int)finalSQLChunk);
    ...
    if (sendBuffer) {
      ...
      EXSM_TRACE(EXSM_TRACE_BUFFER, "CONN %p dequeue %p len %d", this, sendBuffer, (int)msgLen);
      ...
      if (msgLen <= maxBufSize_ && msgLen <= maxSMChunkSize) {
        ...
        UInt32 sqlChunkSize = (msgLen > maxSMChunkSize ? maxSMChunkSize : msgLen);
        ...
        if (isServer_) ExSMTag_SetReplyFlag(&targetToSend.tag);
  ...
  if (!isServer_) incrOutstandingSMRequests();
  ...
  ExSM_MessageType msgType = (isServer_ ? EXSM_MSG_REPLY : EXSM_MSG_REQUEST);
  ExSMGlobals *smGlobals = ExSMGlobals::GetExSMGlobals();

  sm_target_t targetToSend = smTarget_;
  if (sqlChunkMode) ExSMTag_SetSQLChunkFlag(&targetToSend.tag);
  if (isServer_) ExSMTag_SetReplyFlag(&targetToSend.tag);

  bool messageWasSent = false;
  rc = ExSM_Send(smGlobals, targetToSend, sendBuffer->data(sendOffset), sendBytes, msgType,
                 true,            // isPrepostRequired
                 messageWasSent,  // OUT
                 -1,              // retry forever
                 sendCount_, sendBuffer->data(0));
  ...
  sendCount_++;
  EXSM_TRACE(EXSM_TRACE_PROTOCOL, "CONN %p send count %lld", this, sendCount_);

  if (sqlChunkMode) {
    // Do not attempt to send more chunks if an error was encountered
    if (getState() == ERROR_STATE) finalSQLChunk = true;
...
void SMConnection::receive(IpcMessageStreamBase *msgStream) {
  ...
  if (isServer_) setState(RECEIVING);
  ...
  // server.
  if (isServer_) {
    EXSM_TRACE(EXSM_TRACE_PROTOCOL, "CONN %p receive, allocating buffers", this);
    int32_t rc = allocateReceiveBuffers();
    if (rc == SM_ERR_NOPEER || rc == SM_ERR_NOSERVICE || rc == SM_ERR_NODE_DOWN) NAExit(0);
  }
}

WaitReturnStatus SMConnection::wait(IpcTimeout timeout, UInt32 *eventConsumed, IpcAwaitiox *ipcAwaitiox) {
  ...
  if (smGlobals->getReaderThreadState() == ExSMGlobals::TERMINATED_DUE_TO_ERROR) {
    if (!isServer_) {
      if (getState() != ERROR_STATE) {
        EXSM_TRACE(EXSM_TRACE_PROTOCOL, "CONN %p Reader thread terminated", this);
      ...
      EXSM_TRACE(EXSM_TRACE_PROTOCOL, "CONN %p Reader thread terminated, exiting", this);
      NAExit(0);
    }
  }
...
WaitReturnStatus SMConnection::workOnArrivals() {
  ...
  while (!outQueue->isEmpty()) {
    ...
    // messages.
    InternalMsgHdrInfoStruct *infoStruct = (InternalMsgHdrInfoStruct *)msgBuf->data(0);
    ESPMessageTypeEnum streamType = (ESPMessageTypeEnum)infoStruct->getType();

    EXSM_TRACE(EXSM_TRACE_PROTOCOL, "CONN %p recv %p len %d tag %d %s", this, msgBuf, (int)msgBuf->getMessageLength(),
               (int)msgBuf->getReplyTag(), getESPStreamTypeString(streamType));
    if (!isServer_)
      EXSM_TRACE(EXSM_TRACE_PROTOCOL, "CONN %p lbatch %d seq curr %d incoming %d", this,
                 (int)infoStruct->getSMLastInBatch(), (int)getReplySeqNum(), (int)infoStruct->getSeqNum());
    ...
    EXSM_TRACE(EXSM_TRACE_PROTOCOL, "CONN %p set reply tag %d", this, (int)replyTag);
    ...
    if (isServer_) setState(ESTABLISHED);
...
void SMConnection::setState(IpcConnectionState s) {
  ...
  if (oldState != s)
    EXSM_TRACE(EXSM_TRACE_PROTOCOL, "CONN %p %s -> %s", this, getConnectionStateString(oldState),
...
void SMConnection::incrOutstandingSMRequests() {
  ...
  EXSM_TRACE(EXSM_TRACE_PROTOCOL, "CONN %p outstanding reqs %d", this, (int)outstandingSMRequests_);
  if (outstandingSMRequests_ == 1) setState(SENDING);
}
...
void SMConnection::decrOutstandingSMRequests() {
  ...
  if (outstandingSMRequests_ == 0) setState(ESTABLISHED);
}
...
void SMConnection::reportControlConnectionError(GuaErrorNumber error) {
  ...
  if (ccErrorNumber_ == 0) {
    ...
    ExSMEvent::add(ExSMEvent::ControlConnectionError, &smTarget_, ccErrorNumber_, smErrorNumber_, 0, (int64_t)this);
    EXSM_TRACE(EXSM_TRACE_PROTOCOL, "CONN %p reportControlConnectionError %d", this, (int)error);
  }

  // Also store the error in the general-purpose field smErrorNumber_
  if (smErrorNumber_ == 0) smErrorNumber_ = error;
...
void SMConnection::handleIOErrorForSM() {
  ...
  while (sendBuf = removeNextSendBuffer()) sendBuf->decrRefCount();

  IpcMessageBuffer *receiveBuf;
  while (receiveBuf = removeNextReceiveBuffer()) getEnvironment()->getAllConnections()->bumpCompletionCount();
...
//
void SMConnection::populateDiagsArea(ComDiagsArea *&diags, CollHeap *diagsHeap) {
  if (smErrorNumber_ == GuaOK) return;
  ...
  // Case (a)
  if (smErrorNumber_ == SM_ERR_NOPEER || smErrorNumber_ == SM_ERR_NOSERVICE || smErrorNumber_ == SM_ERR_NODE_DOWN) {
  ...
  // Case (b)
    ...
    if (diags && diags->contains(-EXE_SM_CONTROL_CONN_ERROR)) return;

    IpcAllocateDiagsArea(diags, diagsHeap);

    char targetBuf[128];
    sprintf(targetBuf, "%d:%d:%" PRId64 ":%d:0x%c", (int)smTarget_.node, (int)smTarget_.pid, smTarget_.id,
            (int)ExSMTag_GetTagWithoutQualifier(smTarget_.tag), (char)ExSMTag_GetQualifierDisplay(smTarget_.tag));

    *diags << DgSqlCode(-EXE_SM_CONTROL_CONN_ERROR) << DgString1(targetBuf) << DgInt0(smErrorNumber_);
...
// EXIT operations.
void SMConnection::enterChunkMode(IpcMessageBuffer *sendBuffer, UInt32 sqlChunkSize) {
  ...
  EXSM_TRACE(EXSM_TRACE_BUFFER, "CONN %p ENTER CHUNK buf %p sz %d", this, sendBuffer, (int)sqlChunkSize);
}

void SMConnection::moveToNextChunk() {
  chunk_nextOffset_ += chunk_size_;

  EXSM_TRACE(EXSM_TRACE_BUFFER, "CONN %p NEXT CHUNK offset %d", this, (int)chunk_nextOffset_);
}
...
void SMConnection::setAckArrived(bool arrived, bool waitForAnother) {
  ...
  if (smTask_) smTask_->sendChunk_SetAckArrived(arrived);
...
bool SMConnection::getAckArrived() {
  bool result = false;
  if (smTask_) result = smTask_->sendChunk_GetAckArrived();
  return result;
}
...
void SMConnection::dumpAndStopOtherEnd(bool doDump, bool doStop) const {
  ...
  if (XZFIL_ERR_OK == msg_mon_get_process_name2(smTarget_.node, smTarget_.pid, smTarget_.verifier, processName)) {
    sprintf(seqName, "%s:%d", processName, smTarget_.verifier);

    if (doDump) msg_mon_dump_process_name(NULL, seqName, coreFile);

    if (doStop) msg_mon_stop_process_name(seqName);
  }
}
