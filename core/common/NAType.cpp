...
NAType::NAType(NAMemory *h, const NAString &adtName, NABuiltInTypeEnum ev, int dataStorageSize, NABoolean nullable,
  ...
  ComASSERT((dataAlignment == 1) || (dataAlignment == 2) || (dataAlignment == 4) || (dataAlignment == 8));
...
NABoolean NAType::operator==(const NAType &other) const {
  if (typeName_ == other.typeName_ && qualifier_ == other.qualifier_ && dataStorageSize_ == other.dataStorageSize_ &&
      SQLnullFlag_ == other.SQLnullFlag_ && varLenFlag_ == other.varLenFlag_)
    return TRUE;
  else
    return FALSE;
}  // operator==()
...
NABoolean NAType::equalIgnoreLength(const NAType &other) const {
  if (typeName_ == other.typeName_ && qualifier_ == other.qualifier_ && SQLnullFlag_ == other.SQLnullFlag_ &&
...
NABoolean NAType::equalPhysical(const NAType &other) const {
  if (typeName_ == other.typeName_ && qualifier_ == other.qualifier_ && dataStorageSize_ == other.dataStorageSize_ &&
      supportsSQLnullPhysical() == other.supportsSQLnullPhysical() && varLenFlag_ == other.varLenFlag_)
    return TRUE;
  else
    return FALSE;
}
...
const NAType *NAType::synthesizeNullableType(NAMemory *h) const {
  if (this->supportsSQLnull()) return this;
  NAType *result = this->newCopy(h);
  result->setNullable(TRUE);
  return result;
}

const NAType *NAType::synthesizeType(enum NATypeSynthRuleEnum synthRule, const NAType &operand1, const NAType &operand2,
                                     NAMemory *h, UInt32 *flags) const {
  //
  ...
  //
  if (this == &operand1 AND this != &operand2) return operand2.synthesizeType(synthRule, operand1, operand2, h, flags);
  return NULL;
}

const NAType *NAType::synthesizeTernary(enum NATypeSynthRuleEnum synthRule, const NAType &operand1,
  ...
  //
  if (this == &operand1) return operand2.synthesizeTernary(synthRule, operand1, operand2, operand3, h);
  if (this == &operand2) return operand3.synthesizeTernary(synthRule, operand1, operand2, operand3, h);
  return NULL;
}
...
NABoolean NAType::computeNextKeyValue(NAString &keyValue) const {
  keyValue = "";
  NAString temp = "Derived class of NAType needs to define virtual method computeNextKeyValue";
...
void NAType::print(FILE *ofd, const char *indent) {
  fprintf(ofd, "%s type name=%s; size: nominal=%d, total=%d\n", indent, getTypeName().data(), getNominalSize(),
          getTotalSize());
}

// -- The external name for the type (text representation)

NAString NAType::getTypeSQLname(NABoolean) const { return "UNSUPPORTED TYPE"; }
...
NAString *NAType::getKey(NAMemory *h) const { return new (h) NAString(getTypeSQLname(), h); }
...
NABoolean NAType::errorsCanOccur(const NAType &target, NABoolean lax) const {
  if ((!supportsSQLnull()) || (target.supportsSQLnull())) return FALSE;
  return TRUE;
}
...
int NAType::getDisplayLengthStatic(int datatype, int length, int precision, int scale, int heading_len) {
  ...
  switch (datatype) {
    ...
    case REC_INT_FRACTION: {
      ...
      SQLInterval interval(NULL, FALSE, startField, (UInt32)precision, endField, (UInt32)scale);
...
short NAType::convertTypeToText(char *text,       // OUTPUT
  ...
  return convertTypeToText_basic(text, fs_datatype, length, precision, scale, datetimestart, datetimeend,
                                 datetimefractprec, intervalleadingprec, upshift, caseinsensitive, charSet,
                                 CharInfo::getCollationName(collation), displaydatatype, displayCaseSpecific,
                                 isVarchar2);
}
...
short NAType::getHiveTypeStr(int hiveType, int precision, int scale, NAString *outputStr /*out*/) const {
  if (hiveType == HIVE_DECIMAL_TYPE) {
    ...
    decStr.format("%s(%d,%d)", getHiveTypeName(hiveType).data(), precision, scale);
    outputStr->append(decStr);
  } else if ((hiveType == HIVE_STRING_TYPE) || (hiveType == HIVE_BINARY_TYPE)) {
...
short NAType::genHiveTypeStrFromMyType(NAString *outputStr /*out*/) const {
  ...
  if (((DFS2REC::isBinaryNumeric(fs_datatype) && (((NumericType *)this)->decimalPrecision())) ||
    ...
    switch (fs_datatype) {
      ...
      case REC_DATETIME: {
        DatetimeIntervalCommonType &dtiCommonType = (DatetimeIntervalCommonType &)*this;

        ComDateTimeStartEnd dtEndField = (ComDateTimeStartEnd)dtiCommonType.getEndField();
...
short NAType::getHiveTypeStrForMyType(NAString *outputStr /*out*/) const {
  ...
  return getHiveTypeStr(getHiveType(), precision, getScale(), outputStr);
}
...
short NAType::getMyTypeAsText(NAString *outputStr,  // output
  ...
  if (getTypeQualifier() == NA_NUMERIC_TYPE) {
    ...
    if (DFS2REC::isFloat(fs_datatype) || fs_datatype == REC_BPINT_UNSIGNED ||  // all the floats
  ...
  if (getTypeQualifier() == NA_DATETIME_TYPE || getTypeQualifier() == NA_INTERVAL_TYPE) {
    DatetimeIntervalCommonType &dtiCommonType = (DatetimeIntervalCommonType &)*this;
  ...
  if (getTypeQualifier() == NA_CHARACTER_TYPE) {
    ...
    if (addCollation) collationSequence = charType.getCollation();
  ...
  char text[100];
  short rc = NAType::convertTypeToText(
      text, fs_datatype, getNominalSize(), precision, scale, (rec_datetime_field)dtStartField,
      (rec_datetime_field)dtEndField, (short)dtTrailingPrecision, (short)dtLeadingPrecision, (short)isUpshifted,
      (short)isCaseinsensitive, characterSet, collationSequence, getDisplayDataType().data(), 0, isVarchar2);
  if (rc) return -1;
...
NABoolean NAType::useHashInFrequentValue() const { return (DFS2REC::isAnyCharacter(getFSDatatype())); }

NABoolean NAType::useHashRepresentation() const {
  if (DFS2REC::isAnyCharacter(getFSDatatype())) return TRUE;

  if (getTypeQualifier() == NA_NUMERIC_TYPE && getTypeName() == LiteralNumeric &&
...
NABoolean NAType::isSkewBusterSupportedType() const {
  ...
  if (getTypeQualifier() == NA_NUMERIC_TYPE && getTypeName() == LiteralNumeric) return TRUE;
...
CharInfo::CharSet NAType::getCharSet() const { return CharInfo::UnknownCharSet; };
...
NAType *NAType::getArrayNATypeForHive(const char *compTypeStr, NABoolean isORC, NAMemory *heap) {
  SQLArray *nat = NULL;

  if (!compTypeStr) return NULL;
  ...
  NAType *elemType = NAType::getNATypeForHive(elemStr.data(), isORC, TRUE, heap);
  if (!elemType) return NULL;

  // array of array is not supported
  if (elemType->getFSDatatype() == REC_ARRAY) return NULL;

  nat = new (heap) SQLArray(heap, elemType, 0, COM_SQLARK_EXPLODED_FORMAT);
  if ((!nat) || (!nat->validate(CmpCommon::diags()))) return NULL;
...
NAType *NAType::getStructNATypeForHive(const char *compTypeStr, NABoolean isORC, NAMemory *heap) {
  SQLRow *nat = NULL;
  ...
  while (currStr < endStr) {
    ...
    if (strncmp(currStr, "struct", 6) == 0) {
      ...
      if (!endStr) endStr = findToken(currStr, '>');
      if (!endStr) return NULL;
    }
...
NAType *NAType::getNATypeForHive(const char *hiveType, NABoolean isORC, NABoolean compositeElement, NAMemory *heap) {
  NAType *nat = NULL;
  if (!strcmp(hiveType, "tinyint")) {
    ...
    nat = new (heap) SQLVarChar(heap, CharLenInfo(maxNumChars, storageLen),
                                ...
                                CharInfo::getCharSetEnum(hiveCharset), CharInfo::DefaultCollation, CharInfo::IMPLICIT);
    ...
    nat = new (heap) SQLTimestamp(heap, TRUE /* allow NULL */,
  ...
  } else if ((!strncmp(hiveType, "varchar", 7)) || (!strncmp(hiveType, "char", 4))) {
    char maxLen[32];
    memset(maxLen, 0, 32);
    int i = 0, j = 0;
    int copyit = 0;
