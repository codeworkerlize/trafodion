    ...
    switch (wildcard) {
      ...
      case REL_ANY_ROUTINE:
        switch (op_) {
          ...
          case REL_TABLE_MAPPING_BUILTIN_GENERATE_NGRAM:  // built-in function to produce ngram string
...
ExprNode::ExprNode(OperatorTypeEnum otype)
    : operator_(otype),
      ...
      transformed_(FALSE),
      normalized_(FALSE),
      preCodeGenned_(FALSE),
      normalized4Cache_(FALSE),
      cacheable_(MAYBECACHEABLE),
      semanticQueryOptimized_(FALSE) {}
...
NABoolean ExprNode::isStringOutput() {
  switch (operator_) {
    case ITM_LOWER:
    case ITM_LPAD:
    case ITM_LTRIM:
...
ElemDDLNode *ExprNode::castToElemDDLNode() { return NULL; }

const ElemDDLNode *ExprNode::castToElemDDLNode() const { return NULL; }

RelExpr *ExprNode::castToRelExpr() { return NULL; }

const RelExpr *ExprNode::castToRelExpr() const { return NULL; }

ItemExpr *ExprNode::castToItemExpr() { return NULL; }

const ItemExpr *ExprNode::castToItemExpr() const { return NULL; }

StmtDDLNode *ExprNode::castToStmtDDLNode() { return NULL; }

const StmtDDLNode *ExprNode::castToStmtDDLNode() const { return NULL; }

StmtNode *ExprNode::castToStatementExpr() { return NULL; }
...
void ExprNode::print(FILE *f, const char *prefix, const char *) const {
  // print operator text
  fprintf(f, "%sExprNode(%s)\n", prefix, (const char *)getText());
}

void ExprNode::unparse(NAString &, PhaseEnum, UnparseFormatEnum, TableDesc *tabId) const {
  ABORT("ExprNode::unparse() must be redefined");
}

void ExprNode::addLocalExpr(LIST(ExprNode *) &, LIST(NAString) &) const {
