...
/****************************************************************************/
MVCandidate::MVCandidate(const MVDetailsPtr mvDetails, const DescriptorDetailsPtr query, const QRJBBPtr jbb,
                         MVCandidatesForJBBSubsetPtr jbbSubset, ADD_MEMCHECK_ARGS_DEF(CollHeap *heap))
  ...
  if (jbbSubset) jbbSubset->registerCandidate(this);
...
/****************************************************************************/
/****************************************************************************/
QRJoinSubGraphMapPtr MVCandidate::getMvSubGraphMap() { return mvSubGraphMap_; }

/****************************************************************************/
/****************************************************************************/
QRJoinSubGraphMapPtr MVCandidate::getQuerySubGraphMap() { return jbbSubset_->getQuerySubGraphMap(); }
...
void MVCandidate::initAggregateMatchingType() {
  ...
  if (mvDetails_->hasGroupBy()) {
    ...
    if (QueryHasGroupBy) {
      ...
      assertLogAndThrow(CAT_MVCAND, LL_MVQR_FAIL, FALSE, QRLogicException, "A MAV cannot match a join query.");
    }
  } else {
    // The MV is an MJV
    if (QueryHasGroupBy) {
...
NABoolean MVCandidate::matchPass1() {
  if (getAggregateMatchingType() == AMT_MAV_AQ_DG && mvDetails_->getJbbDetails()->hasHavingPredicates()) {
  ...
  if (rangeMatching_.matchPass1() == FALSE) return FALSE;

  if (residualMatching_.matchPass1() == FALSE) return FALSE;

  if (groupingMatching_.matchPass1() == FALSE) return FALSE;

  if (outputMatching_.matchPass1() == FALSE) return FALSE;

  // Must be last Pass1 test before checking extrahub tables.
  if (joinPredMatching_.matchPass1() == FALSE) return FALSE;

  if (CheckExtraHubTables() == FALSE) return FALSE;

  if (lojMatching_.matchPass1() == FALSE) return FALSE;
...
NABoolean MVCandidate::matchPass2() {
  if (rangeMatching_.matchPass2() == FALSE) return FALSE;

  if (residualMatching_.matchPass2() == FALSE) return FALSE;

  if (groupingMatching_.matchPass2() == FALSE) return FALSE;

  if (outputMatching_.matchPass2() == FALSE) return FALSE;

  if (joinPredMatching_.matchPass2() == FALSE) return FALSE;

  if (lojMatching_.matchPass2() == FALSE) return FALSE;
...
QRCandidatePtr MVCandidate::generateDescriptor() {
  ...
  if (allIsWell) allIsWell = groupingMatching_.generateDescriptor(resultDesc);

  // Add range predicates.
  if (allIsWell) allIsWell = rangeMatching_.generateDescriptor(resultDesc);

  // Add range predicates.
  if (allIsWell) allIsWell = residualMatching_.generateDescriptor(resultDesc);

  // Add the output list.
  if (allIsWell) allIsWell = outputMatching_.generateDescriptor(resultDesc);

  // Add the back-join table list and join predicates.
  if (allIsWell) allIsWell = joinPredMatching_.generateDescriptor(resultDesc);

  // Add any NOT NULL predicates for inner over outer joins.
  if (allIsWell) allIsWell = lojMatching_.generateDescriptor(resultDesc);

  if (allIsWell) {
    QRLogger::log(CAT_MVCAND, LL_DEBUG, "Added MV %s to result descriptor.", mvDetails_->getMVName().data());
...
void MVCandidate::logMVWasDisqualified(const char *reason) {
  ...
  if (wasDisqualified_) return;
  wasDisqualified_ = TRUE;

  char buffer[1024];
  const MVDetailsPtr mvDetails = getMvDetails();
  sprintf(buffer, "MV %s was disqualified because %s", mvDetails->getMVName().data(), reason);
...
const BaseTableDetailsPtr MVCandidate::getMvTableForQueryID(const NAString &id, NABoolean assertOnFailure) {
  ...
  if (!queryMap->contains(id)) {
    if (assertOnFailure) {
      assertLogAndThrow(CAT_MVCAND, LL_MVQR_FAIL, FALSE, QRLogicException,
...
NABoolean MVCandidate::CheckExtraHubTables() {
  if (extraHubTables_.entries() == 0) return TRUE;

  // Allocate a new subGraph and subGraphMap for the MV - the one we have is from MVMemo,
  // so its considered read-only.
  QRJoinSubGraphMapPtr mvSubGraphMap = new (heap_) QRJoinSubGraphMap(*getMvSubGraphMap(), ADD_MEMCHECK_ARGS(heap_));
  ...
  QRJoinSubGraphMapPtr querySubGraphMap =
      new (heap_) QRJoinSubGraphMap(*getJbbSubset()->getQuerySubGraphMap(), ADD_MEMCHECK_ARGS(heap_));
  ...
  do {
    ...
    if (!extraHubTableWasAdded && extraHubTables_.entries() > 0) {
      logMVWasDisqualified1("Extra hub table %s is needed but cannot be matched.", extraHubTables_[0]->data());
      return FALSE;
    }
...
//*****************************************************************************
//*****************************************************************************
NABoolean MVCandidate::CheckAnExtraHubTable(const NAString *tableID, QRJoinSubGraphMapPtr mvSubGraphMap,
  ...
  const QRTablePtr queryTableElement = getQueryDetails()->getElementForID(*tableID)->downCastToQRTable();
  const JoinGraphTablePtr queryGraphTable = querySubGraph->getParentGraph()->getTableByID(queryTableElement->getID());
  ...
  const JoinGraphTablePtr mvGraphTable = mvSubGraph->getParentGraph()->getTableByID(mvTableElement->getID());

  if (matchPredsFromTableToSubGraph(mvSubGraph, querySubGraph, mvGraphTable, queryGraphTable) == FALSE) {
    // Match failed.
  ...
  QRLogger::log(CAT_MVCAND, LL_DEBUG, "ExtraHub table %s was matched for MV %s.",
                queryTableElement->getTableName().data(), mvDetails_->getMVName().data());
...
NABoolean MVCandidate::isAUsedExtraHubTable(const NAString *tableID) {
  ...
  for (CollIndex i = 0; i < maxEntries; i++) {
    if (*tableID == *(extraHubTables_[i])) return TRUE;
  }
...
NABoolean MVCandidate::matchPredsFromTableToSubGraph(const QRJoinSubGraphPtr mvSubGraph,
  ...
  for (CollIndex i = 0; i < maxEntries; i++) {
    ...
    if (mvHalfPred->match(queryHalfPred) == FALSE) return FALSE;

    // Let the JoinPredMatching object continue with the matching from the EQ set to the subgraph.
    if (matchPredsFromEQSetToSubGraph(mvSubGraph, querySubGraph, mvEqSet, queryEqSet, mvHalfPred->getID()) == FALSE)
      return FALSE;
  }
...
/****************************************************************************/
/****************************************************************************/
void MVCandidate::cacheTableID(const NAString *tableID, NABoolean isOutside, const NAString *extraHubID) {
  if (extraHubID != NULL) {
...
/****************************************************************************/
/****************************************************************************/
NABoolean MVCandidate::probeCacheForTableID(const NAString *tableID, NABoolean &isOutside,
...
JBBDetailsPtr MVCandidate::getQueryJbbDetails() {
  if (queryJbbDetails_) return queryJbbDetails_;
...
MVCandidatesForJBBSubset::~MVCandidatesForJBBSubset() {
  ...
  for (CollIndex i = 0; i < candidateList_.entries(); i++) deletePtr(candidateList_[i]);
}
...
/****************************************************************************/
/****************************************************************************/
const NAString *MVCandidatesForJBBSubset::getKeyObject() const { return getSubGraph()->getFastKey(); }
...
/****************************************************************************/
/****************************************************************************/
QRJoinSubGraphPtr MVCandidatesForJBBSubset::getSubGraph() const { return querySubGraphMap_->getSubGraph(); }
...
int parseMVAge(const NAString &mvAge) {
  ...
  if (result == 0) {
    QRLogger::log(CAT_MVCAND, LL_ERROR, "Invalid setting for MV_AGE default value: %s, Using only fresh MVs.",
                  mvAge.data());
  }
...
void MVCandidatesForJBBSubset::insert(MVDetailsPtr mv, QRJBBPtr queryJbb, NABoolean isPreferredMatch,
                                      GroupingList *extraGroupingColumns, QRJoinSubGraphMapPtr mvMap, CollHeap *heap) {
  ...
  MVCandidatePtr newCandidate = new (heap) MVCandidate(mv, queryDetails, queryJbb, this, ADD_MEMCHECK_ARGS(heap));

  if (mvMap != NULL) newCandidate->setMvSubGraphMap(mvMap);
...
QRJbbSubsetPtr MVCandidatesForJBBSubset::generateDescriptor(CollHeap *heap) {
  ...
  if (candidateList_.entries() == 0) return NULL;
  ...
  if (hasGroupBy_) {
    ...
    const NAString &groupbyID = getJbbCandidates()->getQueryJbb()->getGroupBy()->getID();
...
/****************************************************************************/
/****************************************************************************/
MVCandidatesForJBB::MVCandidatesForJBB(QRJBBPtr queryJbb, MVCandidateCollectionPtr allCandidates,
...
void MVCandidatesForJBB::insert(MVCandidatesForJBBSubsetPtr jbbSubset) {
  CollIndex pos = 0;
  while (pos < jbbSubsets_.entries() && jbbSubsets_[pos]->getKeyObject()) pos++;
...
NABoolean MVCandidatesForJBB::contains(QRJoinSubGraphPtr subGraph) const {
  ...
  return (pos < jbbSubsets_.entries() && jbbSubsets_[pos]->getKeyObject() == subGraph->getFastKey());
}
...
MVCandidatesForJBBSubsetPtr MVCandidatesForJBB::getJbbSubsetFor(QRJoinSubGraphPtr subGraph) const {
  ...
  if (pos < jbbSubsets_.entries() && jbbSubsets_[pos]->getKeyObject() == subGraph->getFastKey())
...
// Add this MVCandidate into the master list of all the MVCandiates for this query.
/****************************************************************************/
void MVCandidatesForJBB::registerCandidate(MVCandidatePtr candidate) { allCandidates_->registerCandidate(candidate); }
...
QRResultDescriptorPtr MVCandidateCollection::generateResultDescriptor(CollHeap *heap) {
  ...
  for (CollIndex i = 0; i < jbbs_.entries(); i++) {
    ...
    if (jbbResult != NULL) resultDesc->addJbbResult(jbbResult);
  }
...
// Add this MVCandidate into the master list of all the MVCandiates for this query.
/*****************************************************************************/
void MVCandidateCollection::registerCandidate(MVCandidatePtr candidate) { allCandidates_.insert(candidate); }
...
void MVCandidateCollection::doMatching() {
  ...
  for (int i = allCandidates_.entries() - 1; i >= 0; i--) {
    ...
    if (!candidate->wasDisqualified()) {
      ...
      try {
        ...
        // for the rest.
        QRLogger::log(CAT_MVCAND, LL_MVQR_FAIL, "Disqualifying candidate %s due to exception on pass 1",
  ...
  for (int i = allCandidates_.entries() - 1; i >= 0; i--) {
    ...
    try {
      ...
      // Skip only this candidate on exception, and proceed with matching
      // for the rest.
      QRLogger::log(CAT_MVCAND, LL_MVQR_FAIL, "Disqualifying candidate %s due to exception on pass 2",
                    candidate->getMvDetails()->getMVName().data());
      matched = FALSE;
    }
...
/*****************************************************************************/
/*****************************************************************************/
MVPair::MVPair(const QRJoinSubGraphMapPtr firstMV, const QRJoinSubGraphMapPtr otherMV,
               ADD_MEMCHECK_ARGS_DECL(CollHeap *heap))
    : MVCandidate(otherMV->getMVDetails(), firstMV->getMVDetails(),
                  firstMV->getMVDetails()->getJbbDetails()->getJbbDescriptor(), NULL, ADD_MEMCHECK_ARGS_PASS(heap)),
      querySubGraphMap_(firstMV),
      mvSubGraphMap_(otherMV),
      rangePredList_(heap),
      residualPredList_(heap) {
  QRRangePredListPtr rangePredList = getMvDetails()->getJbbDetails()->getJbbDescriptor()->getHub()->getRangePredList();
  if (rangePredList) rangePredList_.insert(rangePredList->getList());

  QRResidualPredListPtr residualPredList =
      getMvDetails()->getJbbDetails()->getJbbDescriptor()->getHub()->getResidualPredList();
  if (residualPredList) residualPredList_.insert(residualPredList->getList());
}
...
/*****************************************************************************/
/*****************************************************************************/
void MVPair::intersectColumnBitmapsForTable(const NAString &queryTableID, XMLBitmap &rangeBitmap,
