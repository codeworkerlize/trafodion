...
QRLatticeIndexNode::QRLatticeIndexNode(QRLatticeIndexPtr lattice, NABoolean noNewEntry,
...
QRLatticeIndexNode::QRLatticeIndexNode(const char *const name, QRLatticeIndexPtr lattice, LatticeKeyArray *keyArray,
                                       ADD_MEMCHECK_ARGS_DEF(CollHeap *heap))
    : NAIntrusiveSharedPtrObject(ADD_MEMCHECK_ARGS_PASS(heap)),
      lattice_(lattice),
      parents_(heap),
...
void QRLatticeIndexNode::adopt(QRLatticeIndexNodePtr newChild, int newChildPos) {
  ...
  if (newChild == this) {
    QRLogger::log(CAT_GRP_LATTCE_INDX, LL_DEBUG, "A node is attempting to adopt itself! Skipping.");
    return;
  }
...
void QRLatticeIndexNode::markVisited() {
  assertLogAndThrow(CAT_GRP_LATTCE_INDX, LL_MVQR_FAIL, lattice_->inTraversal_, QRLogicException,
                    "Cannot mark node as visited when no traversal is in progress.");

  visitMarker_ = lattice_->visitSeqNum_;
}

void QRLatticeIndexNode::dumpNode(NAString &nodeContents, const LatticeKeyArray &keyArray) const {
 ...
 *****************************************************************************
 */
void QRLatticeIndexNode::dumpLattice(NAString &graphText, NAString &graphLabel) {
...
QRLatticeIndex::QRLatticeIndex(CollHeap *heap, ADD_MEMCHECK_ARGS_DEF(CollIndex maxNumKeys))
  ...
  topNode_ = new (heap) QRLatticeIndexNode(TOP_NODE_NAME, this, &keyArr_, ADD_MEMCHECK_ARGS(heap));
  bottomNode_ = new (heap) QRLatticeIndexNode(BOTTOM_NODE_NAME, this, &keyArr_, ADD_MEMCHECK_ARGS(heap));
...
QRLatticeIndex::~QRLatticeIndex() {
  ...
  for (CollIndex i = 0; i < maxEntries; i++) delete keyArr_[i];
  keyArr_.clear();
}

void QRLatticeIndex::addMV(LatticeKeyList &keys, QRJoinSubGraphMapPtr map) {
  QRTRACER("QRLatticeIndex::addMV()");
  assertLogAndThrow(CAT_GRP_LATTCE_INDX, LL_MVQR_FAIL, !inTraversal_, QRLogicException,
  ...
  QRLatticeIndexNodePtr newNode =
      new (heap_) QRLatticeIndexNode(keys, this, ++seqNum_, FALSE, ADD_MEMCHECK_ARGS(heap_));
...
void QRLatticeIndex::removeMV(LatticeKeyList &keys, MVDetailsPtr mvDetails) {
  ...
  if (nodeToRemove->getMVs().entries() > 0) return;

  // Don't remove the top and bottom nodes.
  if (nodeToRemove->isBottom_ || nodeToRemove->isTop_) return;
  ...
  // parents to children.
  NABoolean nodeToRemoveIsLeaf = (childList.entries() == 1 && childList[0]->isBottom_);
  NABoolean nodeToRemoveIsRoot = (parentList.entries() == 1 && parentList[0]->isTop_);
  ...
  for (i = 0; i < childList.entries(); i++) {
    for (j = 0; j < parentList.entries(); j++) {
      ...
      if (nodeToRemoveIsLeaf && parentList[j]->children_.entries() > 1) continue;
      if (nodeToRemoveIsRoot && childList[i]->parents_.entries() > 1) continue;
...
NABoolean QRLatticeIndex::addKeysToBitmap(const LatticeKeyList &keys, NABoolean noNewEntry,
  ...
  for (CollIndex i = 0; i < keys.entries(); i++) {
    ...
    if (keyInx == NULL_COLL_INDEX) {
      ...
      isOK = FALSE;
      assertLogAndThrow1(CAT_GRP_LATTCE_INDX, LL_MVQR_FAIL, noNewEntry, QRLogicException,
                         "Node invalid due to absence of key '%s' in "
                         "lattice index hash table",
                         keyElement->data());
      QRLogger::log(CAT_GRP_LATTCE_INDX, LL_DEBUG, "Key not found: %s", keyElement->data());
    } else {
...
void QRLatticeIndex::findSubsets(QRLatticeIndexNode &supersetNode, NAPtrList<QRLatticeIndexNodePtr> &subsets) {
  QRTRACER("QRLatticeIndex::findSubsets()");
  QRLatticeIndexLock lock(this);  // ctor/dtor handles lock for traversal
  QRLatticeIndexNodePtr exactNode = exactSetsHash_.getFirstValue(&supersetNode.keyBitmap_);
  if (!exactNode)
...
void QRLatticeIndex::findSubsets_(QRLatticeIndexNode &supersetNode, QRLatticeIndexNodePtr childNode,
...
void QRLatticeIndex::getSubsets(QRLatticeIndexNodePtr node, NAPtrList<QRLatticeIndexNodePtr> &subsets) {
  ...
  for (CollIndex i = 0; i < immediateSubsets.entries(); i++) {
    ...
    if (subsetNode->isBottom_ || subsetNode->visited()) continue;
    subsetNode->markVisited();
    getSubsets(subsetNode, subsets);
  }
}

void QRLatticeIndex::findSupersets(QRLatticeIndexNode &subsetNode, NAPtrList<QRLatticeIndexNodePtr> &supersets) {
  QRTRACER("QRLatticeIndex::findSupersets()");
  QRLatticeIndexLock lock(this);  // ctor/dtor handles lock for traversal
  QRLatticeIndexNodePtr exactNode = exactSetsHash_.getFirstValue(&subsetNode.keyBitmap_);
  if (!exactNode)
...
void QRLatticeIndex::findSupersets_(QRLatticeIndexNode &subsetNode, QRLatticeIndexNodePtr parentNode,
...
void QRLatticeIndex::getSupersets(QRLatticeIndexNodePtr node, NAPtrList<QRLatticeIndexNodePtr> &supersets) {
  ...
  for (CollIndex i = 0; i < immediateSupersets.entries(); i++) {
    ...
    if (supersetNode->isTop_ || supersetNode->visited()) continue;
    supersetNode->markVisited();
    getSupersets(supersetNode, supersets);
  }
}

void QRLatticeIndex::insertNode(QRLatticeIndexNodePtr nodeToInsert, QRLatticeIndexNodePtr nodeToCheck) {
  ...
  for (CollIndex i = 0; i < list.entries(); i++) {
    ...
    assertLogAndThrow(CAT_GRP_LATTCE_INDX, LL_MVQR_FAIL, currentChild != nodeToCheck, QRLogicException,
    ...
    // {1,2,3}, {1,2}.
    if (nodeToInsert->isEqual(*currentChild)) return;
    ...
    if (currentChild->isBottom_ || nodeToInsert->isSupersetOf(*currentChild)) {
      ...
      if (currentChild->isBottom_) {
        if (nodeToInsert->children_.isEmpty()) nodeToInsert->adopt(currentChild);
  ...
  if (nodeToInsertHasBeenAdopted) {
    assertLogAndThrow(CAT_GRP_LATTCE_INDX, LL_MVQR_FAIL, skip != NULL_COLL_INDEX, QRLogicException,
    ...
    // node.
    if (nodeToInsert->children_.isEmpty()) nodeToInsert->adopt(bottomNode_);
  }
}

void QRLatticeIndex::findAndAttachSubset(QRLatticeIndexNodePtr insertedNode, QRLatticeIndexNodePtr searchRoot) {
  QRTRACER("QRLatticeIndex::findAndAttachSubset()");
  if (searchRoot->visited()) return;
  ...
  for (CollIndex i = 0; i < list.entries(); i++) {
    currentChild = list[i];
    if (currentChild->visited()) continue;
    ...
    // different node.
    if (currentChild->isSubsetOf(*insertedNode) && !insertedNode->children_.contains(currentChild))
...
void QRLatticeIndex::resizeKeyBitmaps(QRLatticeIndexNodePtr node) {
  ...
  for (CollIndex i = 0; i < children.entries(); i++) resizeKeyBitmaps(children[i]);
}
  ...
  if (!ordinal) {
    ...
    *ordinal = keyArr_.getUsedLength();
    ...
    if (*ordinal >= maxNumKeys_) {
      ...
      assertLogAndThrow2(CAT_GRP_LATTCE_INDX, LL_MVQR_FAIL, maxNumKeys_ == keyArr_.getUsedLength(), QRLogicException,
...
NABoolean QRLatticeIndex::contains(LatticeIndexablePtr key) { return ordinalsHash_.contains(key); }
