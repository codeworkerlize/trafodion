...
RewriteInstructionsItem::RewriteInstructionsItem(const QRElementPtr queryElement, const QRElementPtr mvElement,
                                                 ResultCode resultCode, ResultStatus status,
...
// ***************************************************************************
// ***************************************************************************
RewriteInstructionsItem::~RewriteInstructionsItem() { deletePtr(subElements_); }
...
// ***************************************************************************
// ***************************************************************************
void RewriteInstructionsItem::addSubElements(const RewriteInstructionsItemList &subElements, NABoolean isSpecial) {
  if (subElements.entries() == 0) return;

  if (isSpecial) {
    // There are backjoin or extra hub tables involved - insert one by one.
    CollIndex maxEntries = subElements.entries();
    for (CollIndex i = 0; i < maxEntries; i++) addSubElement(subElements[i]);
  } else {
...
const char *RewriteInstructionsItem::getResultString(ResultCode rc) {
  switch (rc) {
    ...
    case RC_EXPR_REWRITE:
      return "Expression Rewrite";
    case RC_MATCH_RANGE:
      return "RangeSpec comparison";
    case RC_SKIP_ME:
      return "Skip me";
    case RC_ROLLUP:
      return "Rollup";
...
const char *RewriteInstructionsItem::getResultString() { return getResultString(getResultCode()); }
...
compositeMatchingResults::~compositeMatchingResults() {
  for (CollIndex i = 0; i < providedInputs_.entries(); i++) deletePtr(providedInputs_[i]);
  providedInputs_.clear();

  for (CollIndex i = 0; i < notProvidedInputs_.entries(); i++) deletePtr(notProvidedInputs_[i]);
  notProvidedInputs_.clear();

  for (CollIndex i = 0; i < outsideInputs_.entries(); i++) deletePtr(outsideInputs_[i]);
  outsideInputs_.clear();

  for (CollIndex i = 0; i < extrahubInputs_.entries(); i++) deletePtr(extrahubInputs_[i]);
  extrahubInputs_.clear();

  for (CollIndex i = 0; i < backJoinInputs_.entries(); i++) deletePtr(backJoinInputs_[i]);
  backJoinInputs_.clear();

  for (CollIndex i = 0; i < indirectInputs_.entries(); i++) deletePtr(indirectInputs_[i]);
...
void compositeMatchingResults::insert(RewriteInstructionsItemPtr item) {
  switch (item->getResultCode()) {
    ...
    default:
      assertLogAndThrow(CAT_MATCHTST_MVDETAILS, LL_MVQR_FAIL, FALSE, QRLogicException, "Unexpected return code.");
  }  // switch
}
...
MatchTest::~MatchTest() {
  ...
  for (CollIndex i = 0; i < maxEntries; i++) deletePtr(listOfFinalInstructions_[i]);
  listOfFinalInstructions_.clear();

  maxEntries = listOfPendingWork_.entries();
  for (CollIndex j = 0; j < maxEntries; j++) deletePtr(listOfPendingWork_[j]);
...
NABoolean MatchTest::verifyExtraHubAndBackJoinColumns(RewriteInstructionsItemPtr pending) {
  ...
  if (rc != RC_BACKJOIN && rc != RC_EXTRAHUB && pending->getSubElements()->backJoinInputs_.entries() == 0 &&
  ...
  if (rc == RC_EXTRAHUB) {
    ...
    if (pending->getSubElements()->indirectInputs_.entries() == 0 && pending->getSecondaryResultCode() != RC_ROLLUP) {
...
NABoolean MatchTest::generateDescriptor(QRCandidatePtr resultDesc) {
  ...
  for (CollIndex i = 0; i < rewriteList.entries(); i++) {
    ...
    switch (rewrite->getResultCode()) {
      case RC_PROVIDED:
        if (generateProvidedElement(resultDesc, rewrite) == FALSE) return FALSE;
        break;

      case RC_NOTPROVIDED:
        if (generateNotProvidedElement(resultDesc, rewrite) == FALSE) return FALSE;
        break;
      ...
      default:
        ...
        assertLogAndThrow(CAT_MATCHTST_MVDETAILS, LL_MVQR_FAIL, FALSE, QRLogicException,
...
// ***************************************************************************
// ***************************************************************************
RewriteInstructionsItemPtr MatchTest::matchColumn(const QRColumnPtr colElem, NABoolean fromExpr,
                                                  NABoolean isAnEqualitySet) {
  if (isAnEqualitySet) return matchSingleColumn(colElem, fromExpr);  // Avoid recursion.
...
// ***************************************************************************
RewriteInstructionsItemPtr MatchTest::matchSingleColumn(const QRColumnPtr colElem, NABoolean fromExpr) {
  RewriteInstructionsItemPtr rewrite = NULL;
  const NAString *extraHubID = NULL;
  if (isOutsideColumn(colElem, extraHubID)) {
    rewrite = new (heap_) RewriteInstructionsItem(colElem, NULL, RC_OUTSIDE, RS_FINAL, ADD_MEMCHECK_ARGS(heap_));
    return rewrite;
  } else if (extraHubID != NULL) {
    rewrite =
        new (heap_) RewriteInstructionsItem(colElem, NULL, RC_EXTRAHUB, RS_INTERMEDIATE, ADD_MEMCHECK_ARGS(heap_));
  ...
  if (isProvidedColumn(colElem, mvCol)) {
    rewrite = new (heap_) RewriteInstructionsItem(colElem, mvCol, RC_PROVIDED, RS_FINAL, ADD_MEMCHECK_ARGS(heap_));
    return rewrite;
  }

  if (isBackJoinableColumn(colElem)) {
    rewrite =
        new (heap_) RewriteInstructionsItem(colElem, NULL, RC_BACKJOIN, RS_INTERMEDIATE, ADD_MEMCHECK_ARGS(heap_));
...
NABoolean MatchTest::isOutsideColumn(const QRColumnPtr colElem, const NAString *&extraHubID) {
  ...
  if (candidate_->probeCacheForTableID(&tableID, isOutside, extraHubID)) return isOutside;
...
// ***************************************************************************
NABoolean MatchTest::isProvidedColumn(const QRColumnPtr colElem, QROutputPtr &mvCol) {
  const MVDetailsPtr mvDetails = candidate_->getMvDetails();
  ...
  mvCol = mvDetails->getOutputByColumnName(baseTable->getTableElement()->getID(), colElem->getColumnName(), heap_);
...
RewriteInstructionsItemPtr MatchTest::isIndirectColumn(const QRColumnPtr colElem) {
  ...
  const QROutputPtr mvOutput =
      mvDetails->getOutputByInputColumns(baseTable->getTableElement()->getID(), colElem->getColumnName(), heap_);
  ...
  if (mvOutput == NULL)
    ...
    RewriteInstructionsItemPtr rewrite =
        new (heap_) RewriteInstructionsItem(colElem, mvOutput, RC_INDIRECT, RS_INTERMEDIATE, ADD_MEMCHECK_ARGS(heap_));
    return rewrite;
  }
...
RewriteInstructionsItemPtr MatchTest::findExpressionInputs(const ElementPtrList &queryColList,
  ...
  if (queryColList.entries() == 0) {
    ...
    rewrite =
        new (heap_) RewriteInstructionsItem(queryElement, NULL, RC_NOTPROVIDED, RS_FINAL, ADD_MEMCHECK_ARGS(heap_));
    rewrite->setSecondaryResultCode(RC_NO_INPUTS);
    return rewrite;
  }

  // Analyze the expresion inputs
  compositeMatchingResultsPtr intermediateResults = new (heap_) compositeMatchingResults(ADD_MEMCHECK_ARGS(heap_));
...
RewriteInstructionsItemPtr MatchTest::matchEqualitySet(const QRJoinPredPtr joinPred) {
  ...
  // ANY semantics.
  ...
  } else if (intermediateResults->backJoinInputs_.entries() > 0 || intermediateResults->extrahubInputs_.entries() > 0) {
    ...
    rewrite =
        new (heap_) RewriteInstructionsItem(joinPred, NULL, RC_NOTPROVIDED, RS_INTERMEDIATE, ADD_MEMCHECK_ARGS(heap_));
...
// ***************************************************************************
void MatchTest::MatchInputColumnList(const ElementPtrList &queryColList, compositeMatchingResultsPtr matchingResults,
                                     NABoolean isAnEqualitySet) {
  ...
  for (CollIndex i = 0; i < numInputs; i++) {
    ...
    switch (queryElem->getElementType()) {
      ...
      case ET_JoinPred: {
        ...
        assertLogAndThrow(CAT_MATCHTST_MVDETAILS, LL_MVQR_FAIL, !isAnEqualitySet, QRLogicException,
    ...
    if (rewrite == NULL) {
      ...
      if (!isAnEqualitySet)
        ...
        QRLogger::log(CAT_MATCHTST_MVDETAILS, LL_INFO, "Ignore last disqualification message.");
        continue;
      }
    }
    ...
    if (rewrite->getResultCode() == RC_PROVIDED && isAnEqualitySet) return;
...
QRMVColumnPtr MatchTest::generateProvidedMvColumn(RewriteInstructionsItemPtr rewrite) {
  ...
  assertLogAndThrow(CAT_MATCHTST_MVDETAILS, LL_MVQR_FAIL, mvElem != NULL, QRLogicException,
...
QRColumnPtr MatchTest::generateNotProvidedColumn(RewriteInstructionsItemPtr rewrite) {
  ...
  assertLogAndThrow(CAT_MATCHTST_MVDETAILS, LL_MVQR_FAIL, queryElem != NULL, QRLogicException,
  ...
  if (queryElem->getElementType() == ET_Column) {
    ...
    assertLogAndThrow(CAT_MATCHTST_MVDETAILS, LL_MVQR_FAIL, queryElem->getElementType() == ET_JoinPred,
                      QRLogicException, "Expecting a either a Column or JoinPred here.");

    RewriteInstructionsItemPtr subRewrite = rewrite->getSubElements()->backJoinInputs_[0];
    assertLogAndThrow(CAT_MATCHTST_MVDETAILS, LL_MVQR_FAIL, subRewrite, QRLogicException,
...
RewriteInstructionsItemPtr MatchTest::matchExpression(const QRExprPtr expr) {
  ...
  if (candidate_->getAggregateMatchingType() != AMT_MAV_AQ_DG || expr->getExprRoot()->containsAnAggregate() == FALSE) {
  ...
  if (queryInputs.entries() == 0) {
    ...
    switch (candidate_->getAggregateMatchingType()) {
      case AMT_MAV_AQ_MG:
        ...
        candidate_->logMVWasDisqualified1("output expression %s is not Provided.", expr->getID().data());
        return NULL;

      case AMT_MJV_AQ:
        // If its on an MJV, its a Final Outside match, because it can be computed on the MJV.
        rewrite = new (heap_) RewriteInstructionsItem(expr, NULL, RC_OUTSIDE, RS_FINAL, ADD_MEMCHECK_ARGS(heap_));
        break;

      case AMT_MAV_AQ_DG:
        // If this is a rollup matching case, defer to Pass 2.
        rewrite =
            new (heap_) RewriteInstructionsItem(expr, NULL, RC_NOTPROVIDED, RS_INTERMEDIATE, ADD_MEMCHECK_ARGS(heap_));
        if (expr->getExprRoot()->containsAnAggregate()) rewrite->setSecondaryResultCode(RC_ROLLUP);
        break;
      case AMT_MJV_JQ:
        candidate_->logMVWasDisqualified1("output expression %s is not Provided.", expr->getID().data());
        break;
    ...
    if (expr->getExprRoot()->containsAnAggregate() && candidate_->getAggregateMatchingType() != AMT_MJV_AQ) {
      // Yes, its an aggregate expression. Deal with it in Pass 2.
      rewrite =
          new (heap_) RewriteInstructionsItem(expr, NULL, RC_NOTPROVIDED, RS_INTERMEDIATE, ADD_MEMCHECK_ARGS(heap_));
      if (candidate_->getAggregateMatchingType() == AMT_MAV_AQ_DG) rewrite->setSecondaryResultCode(RC_ROLLUP);
    } else {
  ...
  for (CollIndex j = 0; j < iterator.entries(); j++) {
    ...
    if (matchExpressionInputs(queryColList, mvExpr->getInputColumns(heap_))) {
      ...
      if (queryColList.entries() == 0) {
        ...
        rewrite = new (heap_) RewriteInstructionsItem(expr, mvOutput, RC_PROVIDED, RS_FINAL, ADD_MEMCHECK_ARGS(heap_));
      } else {
        ...
        if (rewrite == NULL) {
          ...
          rewrite =
              new (heap_) RewriteInstructionsItem(expr, mvOutput, RC_PROVIDED, RS_FINAL, ADD_MEMCHECK_ARGS(heap_));
        } else {
          // Yes, Combine the two.
...
NABoolean MatchTest::matchExpressionInputs(const ElementPtrList &queryInputColumns,
  ...
  if (queryInputColumns.entries() != mvInputColumns.entries()) return FALSE;
  ...
  for (CollIndex i = 0; i < maxEntries; i++) {
    ...
    if (query_jp == NULL) {
      ...
      if (mv_jp != NULL) return FALSE;

      if (matchColumnsByNameAndID(queryColumn, mvColumn) == FALSE) return FALSE;
    } else if (query_jp) {
...
NABoolean MatchTest::matchColumnsByNameAndID(const QRColumnPtr queryColumn, const QRColumnPtr mvColumn) {
  ...
  if (queryColumn->getColumnName() != mvColumn->getColumnName()) return FALSE;
...
void MatchTest::addInputsToOutputList(RewriteInstructionsItemPtr rewrite) {
  if (candidate_->getAggregateMatchingType() == AMT_MAV_AQ_DG) return;
  ...
  if (rewrite->getResultCode() == RC_PROVIDED) return;
  ...
  if (rewrite->getSubElements()->outsideInputs_.entries() > 0 ||
      (expr->getExprRoot()->containsAnAggregate() && candidate_->getAggregateMatchingType() == AMT_MJV_AQ)) {
...
RewriteInstructionsItemPtr MatchTest::findInternalAggregateFunctions(QRExplicitExprPtr treeExpr) {
  AggregateCollectorVisitorPtr visitor = new (heap_) AggregateCollectorVisitor(ADD_MEMCHECK_ARGS(heap_));
  treeExpr->treeWalk(visitor);

  RewriteInstructionsItemPtr rewrite =
      new (heap_) RewriteInstructionsItem(treeExpr, NULL, RC_NOTPROVIDED, RS_FINAL, ADD_MEMCHECK_ARGS(heap_));
...
RewriteInstructionsItemPtr MatchTest::findAggregateFunction(QRFunctionPtr func) {
  ...
  if (candidate_->getAggregateMatchingType() == AMT_MAV_AQ_DG) {
    ...
    if (func->getAggregateFunc() == QRFunction::AFT_COUNTSTAR) {
      rewrite = new (heap_) RewriteInstructionsItem(func, NULL, RC_NOTPROVIDED, RS_FINAL, ADD_MEMCHECK_ARGS(heap_));
    ...
    if (areAllInputsGroupingColumns(func, inputList)) {
      ...
      if (func->isDistinctAggregate()) {
        candidate_->logMVWasDisqualified1("DISTINCT aggregate function %s cannot be rolled up.", func->getID().data());
        return NULL;
      }
    }
  }  // if (rollup)
  ...
  if (func->getAggregateFunc() == QRFunction::AFT_COUNT && countStarOutput != NULL) {
    ...
    if (func->isCountStarEquivalent(heap_)) {
      ...
      rewrite =
          new (heap_) RewriteInstructionsItem(func, countStarOutput, RC_PROVIDED, RS_FINAL, ADD_MEMCHECK_ARGS(heap_));
      return rewrite;
    }
  }

  // Give up on this aggregate function.
  candidate_->logMVWasDisqualified1("aggregate function %s cannot be matched.", func->getID().data());
  return NULL;
}
...
NABoolean MatchTest::areAllInputsGroupingColumns(QRFunctionPtr func, ElementPtrList &inputList) {
  ...
  for (CollIndex i = 0; i < inputList.entries(); i++) {
    ...
    if (!isExtraGroupingColumn(inputCol)) return FALSE;
  }
...
QRExprPtr MatchTest::generateAggregateExpressionOutput(RewriteInstructionsItemPtr rewrite) {
  ...
  assertLogAndThrow(CAT_MATCHTST_MVDETAILS, LL_MVQR_FAIL,
                    ...
                    QRLogicException, "Expecting all subexpressions to be Provided, NotProvided, or back join.");
...
NABoolean MatchTest::isExtraGroupingColumn(QRColumnPtr col) {
  ...
  for (CollIndex i = 0; i < maxEntries; i++) {
    ...
    if (elem->getElementType() == ET_Column) {
      ...
      if (matchColumnsByNameAndID(col, groupingCol)) return TRUE;
    }
    ...
    if (elem->getElementType() == ET_JoinPred) {
      ...
      for (CollIndex j = 0; j < eqSetSize; j++) {
        ...
        if (eqElement->getElementType() != ET_Column) continue;
        const QRColumnPtr eqCol = eqElement->downCastToQRColumn();
        if (matchColumnsByNameAndID(col, eqCol)) return TRUE;
      }
    }
  }
...
QRFunctionPtr MatchTest::generateRollupAggregateFunction(RewriteInstructionsItemPtr rewrite) {
  ...
  if (queryElem->getElementType() == ET_Expr) {
    ...
    assertLogAndThrow(CAT_MATCHTST_MVDETAILS, LL_MVQR_FAIL, queryElem->getElementType() == ET_Function,
                      QRLogicException, "Expecting either a QRExpr or QRFunction.");
  ...
  assertLogAndThrow(CAT_MATCHTST_MVDETAILS, LL_MVQR_FAIL, !queryFunction->isDistinctAggregate(), QRLogicException,
  ...
  switch (queryFunction->getAggregateFunc()) {
    ...
    case QRFunction::AFT_ONE_ROW:
      ...
      assertLogAndThrow1(CAT_MATCHTST_MVDETAILS, LL_MVQR_FAIL, FALSE, QRLogicException,
...
QRFunctionPtr MatchTest::generateRollupOverGroupingAggregateFunction(RewriteInstructionsItemPtr rewrite) {
  ...
  QRElementPtr queryElem = rewrite->getActualQueryElement();
  QRFunctionPtr queryFunction;
  assertLogAndThrow(CAT_MATCHTST_MVDETAILS, LL_MVQR_FAIL, queryElem->getElementType() == ET_Function, QRLogicException,
  ...
  if (needActualArgument) {
    ...
    if (queryArg->getElementType() == ET_Column || queryArg->getElementType() == ET_JoinPred) {
      ...
      assertLogAndThrow(CAT_MATCHTST_MVDETAILS, LL_MVQR_FAIL, colRewrite != NULL, QRLogicException,
...
NABoolean MatchOutput::matchPass2OnElement(RewriteInstructionsItemPtr pending) {
  if (pending->getActualQueryElement()->getElementType() != ET_Expr) {
    ...
    candidate_->logMVWasDisqualified1("Column output %s got to Pass 2.", pending->getQueryElement()->getID());
    return FALSE;
  }
  ...
  if (pending->getActualQueryElement()->downCastToQRExpr()->getExprRoot()->containsAnAggregate()) {
    ...
    candidate_->logMVWasDisqualified1("Output expression %s cannot be Provided.", pending->getQueryElement()->getID());
...
NABoolean MatchOutput::matchAggregateExpressions(RewriteInstructionsItemPtr pending) {
  ...
  assertLogAndThrow(CAT_MATCHTST_MVDETAILS, LL_MVQR_FAIL, treeExpr->containsAnAggregate(), QRLogicException,
                    "Expecting Rollup aggreate functions.");

  RewriteInstructionsItemPtr rewrite = NULL;

  // 1. Is the top level an aggregate function?
  if (treeExpr->getElementType() == ET_Function && treeExpr->downCastToQRFunction()->isAnAggregate()) {
    // Look for the corresponding aggregate function as an MV output.
    RewriteInstructionsItemPtr funcRewrite = findAggregateFunction(queryExpr->getExprRoot()->downCastToQRFunction());
    if (funcRewrite == NULL) return FALSE;
    ...
    rewrite =
        new (heap_) RewriteInstructionsItem(queryOutputExpr, NULL, RC_NOTPROVIDED, RS_FINAL, ADD_MEMCHECK_ARGS(heap_));

    assertLogAndThrow(CAT_MATCHTST_MVDETAILS, LL_MVQR_FAIL, candidate_->getAggregateMatchingType() == AMT_MAV_AQ_DG,
                      QRLogicException, "Simple aggreate functions don't get here - they are Provided.");
    rewrite->addSubElement(funcRewrite);
    rewrite->setSecondaryResultCode(RC_ROLLUP_EXPR);
  } else {
...
// ***************************************************************************
NABoolean MatchOutput::generateProvidedElement(QRCandidatePtr resultDesc, RewriteInstructionsItemPtr rewrite) {
...
NABoolean MatchOutput::isFromOutside(RewriteInstructionsItemPtr rewrite) {
  ...
  if (queryElem->getElementType() == ET_Column) {
    ...
    if (isOutsideColumn(queryCol, extraHubID)) return TRUE;
  }
...
// ***************************************************************************
void MatchOutput::switchRewrites(RewriteInstructionsItemPtr newRewrite, RewriteInstructionsItemPtr firstRewrite,
...
void MatchOutput::addRewriteInstructions(RewriteInstructionsItemPtr rewrite) {
  if (rewrite->isFinal() == FALSE) {
    ...
    if (mvOutput) {
      ...
      if (mvElem->getElementType() == ET_Column) MVColName = mvElem->downCastToQRColumn()->getColumnName();
    }

    // If we need to avoid this ID, ignore it.
    if (OutputsToAvoidByID_.contains(&id) == TRUE) return;
    ...
    if (OutputRewriteInstructionsByID_.contains(&id) == FALSE) {
      ...
      if (mvOutput && mvOutput->getElementType() == ET_Output) {
        if (OutputRewriteInstructionsByMvColName_.contains(&MVColName)) {
          ...
          RewriteInstructionsItemPtr firstRewrite = OutputRewriteInstructionsByMvColName_.getFirstValue(&MVColName);
...
NABoolean MatchRangePredicates::matchPass1() {
  if (matchPredicateBitmaps() == FALSE) return FALSE;
  ...
  for (CollIndex i = 0; i < queryRangePreds.entries(); i++) {
    ...
    assertLogAndThrow(CAT_MATCHTST_MVDETAILS, LL_MVQR_FAIL,
                      (rangeElemType == ET_Column || rangeElemType == ET_JoinPred || rangeElemType == ET_Expr),
                      QRLogicException, "Expecting range items to be columns, equality sets or expressions.");

    if (rangeElemType == ET_Expr) {
      // Do the matching work for a range pred on an expression.
      if (matchPredOnExpr(queryRangePred) == FALSE) return FALSE;
      ...
      if (matchPredOnColumn(queryRangePred, rangeColumn) == FALSE) return FALSE;
    } else {
      // Range item must be a join pred.
      QRJoinPredPtr joinPred = rangeItem->downCastToQRJoinPred();

      if (matchPredOnEqualitySet(queryRangePred, joinPred) == FALSE) return FALSE;
...
// ***************************************************************************
NABoolean MatchRangePredicates::matchPredOnEqualitySet(const QRRangePredPtr queryRangePred,
                                                       const QRJoinPredPtr joinPred) {
  const ElementPtrList &equalitySet = joinPred->getEqualityList();
  ...
  for (CollIndex i = 0; i < maxEntries; i++) {
    ...
    if (eqMember->getElementType() != ET_Column) continue;

    QRColumnPtr eqColumn = eqMember->downCastToQRColumn();
    const NAString *extraHubID = NULL;
    if (isOutsideColumn(eqColumn, extraHubID)) continue;
    insideColFound = TRUE;

    if (matchPredOnColumn(queryRangePred, eqColumn)) return TRUE;
  }
...
NABoolean MatchRangePredicates::matchPredOnExpr(const QRRangePredPtr queryRangePred) {
  ...
  if (candidate_->getAggregateMatchingType() == AMT_MAV_AQ_DG && queryRangeExpr->getExprRoot()->containsAnAggregate()) {
    rewrite = new (heap_)
    ...
    if (mvPredsOnExpr != NULL) {
      ...
      for (CollIndex j = 0; j < mvPredsOnExpr->entries(); j++) {
        ...
        if (matchExpressionInputs(queryInputColumns, mvRangeExpr->getInputColumns(heap_)) == FALSE)
          ...
          rewrite = new (heap_) RewriteInstructionsItem(queryRangePred, mvRangePred, RC_MATCH_RANGE, RS_INTERMEDIATE,
  ...
  if (rewrite == NULL) {
    ...
    if (candidate_->getAggregateMatchingType() == AMT_MAV_AQ_MG &&
      ...
      if (rewrite && rewrite->isFinal() && rewrite->getSecondaryResultCode() == RC_INPUTS_PROVIDED &&
          queryRangeExpr->getExprRoot()->containsAnAggregate()) {
...
// ***************************************************************************
// ***************************************************************************
NABoolean MatchRangePredicates::matchPredOnColumn(const QRRangePredPtr queryRangePred, const QRColumnPtr rangeColumn) {
  ...
  if (mvRangePred == NULL) {
    ...
    if (columnRewrite == NULL) return FALSE;

    // The primary result code is NotPrvided.
    // Its final if the column rewrite is Final.
    rewrite = new (heap_) RewriteInstructionsItem(queryRangePred, NULL, RC_NOTPROVIDED,
                                                  columnRewrite->getResultStatus(), ADD_MEMCHECK_ARGS(heap_));
...
NABoolean MatchRangePredicates::matchPredicateBitmaps() {
  ...
  if (mvJbbDetails->hasNoRangePredicates() && mvJbbDetails->hasNoResidualPredicates()) return TRUE;
...
NABoolean MatchRangePredicates::matchPass2OnElement(RewriteInstructionsItemPtr pending) {
  ...
  if (pending->getResultCode() == RC_AGGR_EXPR || pending->getResultCode() == RC_ROLLUP_EXPR) {
    QRElementPtr treeElem =
        queryPred->getRangeItem()->getReferencedElement()->downCastToQRExpr()->getExprRoot()->getReferencedElement();
    ...
    if (pending->getSubElements()->indirectInputs_.entries() > 0) {
      if (queryPred->getRangeItem()->getElementType() == ET_Expr) {
        ...
        if (exprRewrite == NULL) {
          ...
          return NULL;
        } else if (exprRewrite->getResultStatus() == RS_FINAL && exprRewrite->getResultCode() == RC_PROVIDED) {
          ...
          for (CollIndex i = 0; i < indirect.entries(); i++) deletePtr(indirect[i]);
          indirect.clear();
  ...
  assertLogAndThrow1(CAT_MATCHTST_MVDETAILS, LL_MVQR_FAIL, pending->getResultCode() == RC_MATCH_RANGE, QRLogicException,
  ...
  if (queryRangeSpec->isEqual(*mvRangeSpec)) {
    ...
    if (mvRangeSpec->subsumes(queryRangeSpec)) {
        ...
        case ET_Expr: {
          ...
          if (columnRewrite != NULL) {
            ...
            if (columnRewrite->isFinal() == FALSE) {
              ...
              columnRewrite = NULL;
              candidate_->logMVWasDisqualified1("range predicate %s cannot be Provided.", queryPred->getID().data());
              return FALSE;
            }
      ...
      candidate_->logMVWasDisqualified1(
          "The query range predicate %s is not subsumed by the corresponding MV predicate.", queryPred->getID().data());
      return FALSE;
    }
  }
...
// query element.
// ***************************************************************************
NABoolean MatchRangePredicates::generateProvidedElement(QRCandidatePtr resultDesc, RewriteInstructionsItemPtr rewrite) {
...
NABoolean MatchRangePredicates::generateNotProvidedElement(QRCandidatePtr resultDesc,
  ...
  if (rewrite->getSecondaryResultCode() == RC_AGGR_EXPR || rewrite->getSecondaryResultCode() == RC_ROLLUP_EXPR) {
    RewriteInstructionsItemPtr exprRewrite = rewrite->getSubElements()->notProvidedInputs_[0];
    assertLogAndThrow(CAT_MATCHTST_MVDETAILS, LL_MVQR_FAIL, exprRewrite, QRLogicException,
    ...
    if (rewrite->getSecondaryResultCode() == RC_NO_INPUTS) {
      ...
      assertLogAndThrow(CAT_MATCHTST_MVDETAILS, LL_MVQR_FAIL, rangeItem != NULL, QRLogicException,
...
QRRangePredPtr MatchRangePredicates::checkPredicate(QRRangePredPtr querySidePred) {
  ...
  if (rangeElemType == ET_Expr) {
    ...
    const NAString &predText = queryRangeExpr->getExprText();
    RangePredPtrList *mvPredsOnExpr = candidate_->getMvDetails()->getJbbDetails()->getRangePredsOnExpression(predText);
    if (mvPredsOnExpr == NULL) return NULL;
  ...
  if (mvRangePred == NULL) return NULL;
...
NABoolean MatchResidualPredicates::matchPass1() {
  ...
  const QRResidualPredListPtr queryResidualPredList = candidate_->getQueryJbb()->getHub()->getResidualPredList();
  ...
  for (CollIndex i = 0; i < queryResidualPreds.entries(); i++) {
    ...
    RewriteInstructionsItemPtr rewrite = matchResidualPredicate(queryResidualPred, usedDuplicates);
    if (rewrite == NULL) {
...
RewriteInstructionsItemPtr MatchResidualPredicates::matchResidualPredicate(const QRExprPtr queryResidualPred,
  ...
  if (candidate_->getAggregateMatchingType() == AMT_MAV_AQ_DG && isAggregateExpr) {
  ...
  if (mvPredList != NULL) {
    ...
    for (CollIndex j = 0; j < mvPredList->entries(); j++) {
      ...
      if (matchExpressionInputs(queryResidualPred->getInputColumns(heap_), mvResidualPred->getInputColumns(heap_)) ==
          FALSE)
  ...
  if (rewrite == NULL) {
    ...
    if (candidate_->getAggregateMatchingType() == AMT_MAV_AQ_MG && isAggregateExpr) {
      // This is an aggregate query on a matching MAV,
      // Try to match aggregate functions, in Pass 2.
      rewrite = new (heap_)
          RewriteInstructionsItem(queryResidualPred, NULL, RC_NOTPROVIDED, RS_INTERMEDIATE, ADD_MEMCHECK_ARGS(heap_));
      ...
      // This is on an MJV. Try to find the input columns.
      const ElementPtrList &queryInputElements = queryResidualPred->getInputColumns(heap_);
...
NABoolean MatchResidualPredicates::matchPass2OnElement(RewriteInstructionsItemPtr pending) {
  if (pending->getResultCode() == RC_NOTPROVIDED) {
    ...
    if (rewrite == NULL) {
      candidate_->logMVWasDisqualified1("Residual expression %s is not Provided by the MV.", residExpr->getID().data());
      return FALSE;
    }
...
NABoolean MatchResidualPredicates::generateNotProvidedElement(QRCandidatePtr resultDesc,
  ...
  if (rewrite->getSecondaryResultCode() == RC_AGGR_EXPR || rewrite->getSecondaryResultCode() == RC_ROLLUP_EXPR) {
    ...
    try {
      ...
      candidate_->logMVWasDisqualified1("Residual predicate %s cannot be rewritten.", queryExpr->getID().data());
      return NULL;
    }
...
QRExprPtr MatchResidualPredicates::checkPredicate(QRExprPtr querySidePred) {
  ...
  if (mvPredList != NULL) {
    ...
    for (CollIndex j = 0; j < mvPredList->entries(); j++) {
      QRExprPtr mvResidualPred = (*mvPredList)[j];
      if (matchExpressionInputs(querySidePred->getInputColumns(heap_), mvResidualPred->getInputColumns(heap_)))
...
NABoolean MatchGroupingColumns::matchPass1() {
  ...
  if (candidate_->getJbbSubset()->isIndirectGroupBy()) {
    if (VerifyGroupingColumns() == FALSE) return FALSE;
  } else {
    switch (candidate_->getAggregateMatchingType()) {
      case AMT_MJV_JQ:
        // No Groupby involved.
...
NABoolean MatchGroupingColumns::VerifyGroupingColumns() {
  ...
  assertLogAndThrow(CAT_MATCHTST_MVDETAILS, LL_MVQR_FAIL, queryGroupBy != NULL, QRLogicException,
  ...
  if (queryGroupBy->isEmpty()) return TRUE;

  const GroupingList &groupingList = queryGroupBy->getPrimaryList()->getList();
  assertLogAndThrow(CAT_MATCHTST_MVDETAILS, LL_MVQR_FAIL, !groupingList.isEmpty(), QRLogicException,
  ...
  for (CollIndex i = 0; i < groupingList.entries(); i++) {
    ...
    if (verifyGroupingElement(groupingElem) == FALSE) return FALSE;
  }
...
NABoolean MatchGroupingColumns::verifyGroupingElement(const QRElementPtr groupingElem, NABoolean isRecursive) {
  ...
  if (groupingElem->getElementType() == ET_Expr) {
    ...
    if (rewrite == NULL || !rewrite->isFinal()) {
      candidate_->logMVWasDisqualified1("grouping expression %s is not provided.", groupingExpr->getID().data());
      return FALSE;
    }
  } else {
    assertLogAndThrow(CAT_MATCHTST_MVDETAILS, LL_MVQR_FAIL, groupingElem->getElementType() == ET_Column,
                      QRLogicException, "Expecting a QRColumn element.");
    ...
    if (candidate_->getQueryDetails()->isFromJoin(groupingCol) && !isRecursive) {
      ...
      for (CollIndex i = 0; i < maxEntries; i++) {
        ...
        if (eqElem->getElementType() == ET_Column && verifyGroupingElement(eqElem, TRUE)) return TRUE;
      }  // End of for loop.

      // If we got here, none of the eqSet's columns is Provided by the MV.
      candidate_->logMVWasDisqualified1("The needed grouping column %s is not Provided.", groupingElem->getID().data());
      return FALSE;
    }
    // Get the MV base table object from the query table ID.
    ...
    if (baseTable == NULL) {
      if (isRecursive)
        ...
        rewrite =
            new (heap_) RewriteInstructionsItem(groupingElem, NULL, RC_OUTSIDE, RS_FINAL, ADD_MEMCHECK_ARGS(heap_));
    } else {
      const QROutputPtr mvOutput = candidate_->getMvDetails()->getOutputByColumnName(
          baseTable->getTableElement()->getID(), groupingCol->getColumnName(), heap_);
      ...
      if (mvOutput == NULL) {
        ...
        if (candidate_->getQueryJbbDetails()->isConstColumn(&groupingCol->getID())) return TRUE;
      }
      ...
      if (!isRecursive) isBackJoinable = isBackJoinableColumn(groupingCol);
      ...
      rewrite =
          new (heap_) RewriteInstructionsItem(groupingCol, mvOutput, RC_PROVIDED, RS_FINAL, ADD_MEMCHECK_ARGS(heap_));
...
NABoolean MatchGroupingColumns::generateDescriptor(QRCandidatePtr resultDesc) {
  ...
  if (candidate_->getAggregateMatchingType() == AMT_MJV_AQ || candidate_->getAggregateMatchingType() == AMT_MJV_JQ)
    return TRUE;
  ...
  if (candidate_->getAggregateMatchingType() == AMT_MAV_AQ_MG) {
    ...
    for (CollIndex i = 0; i < rewriteList.entries(); i++) {
      ...
      if (rewrite->getResultCode() == RC_PROVIDED) {
        ...
        assertLogAndThrow(CAT_MATCHTST_MVDETAILS, LL_MVQR_FAIL, rewrite->getResultCode() == RC_OUTSIDE,
                          QRLogicException, "Expecting only Outside and Provided grouping Columns.");
...
NABoolean MatchJoinPreds::matchPass1() {
  ...
  if (queryHubJoinPreds != NULL) {
    ...
    for (CollIndex i = 0; i < maxEntries; i++) {
      ...
      if (analyzeJoinPredicate(joinPred, TRUE) == FALSE) return FALSE;
    }
  }
  ...
  if (ehTables > 0) {
    ...
    for (CollIndex k = 0; k < mvColumns.entries(); k++) {
      ...
      if (isProvidedColumn(hubCol, mvCol)) {
        ...
        RewriteInstructionsItemPtr rewrite =
            new (heap_) RewriteInstructionsItem(hubCol, mvCol, RC_PROVIDED, RS_FINAL, ADD_MEMCHECK_ARGS(heap_));
    ...
    if (!foundProvided && mvExprs.entries() > 0) {
      ...
      for (CollIndex m = 0; m < mvExprs.entries(); m++) {
        if (analyzeProvidedExpression(mvExprs[m])) foundProvided = TRUE;
      }
    }
...
void MatchJoinPreds::analyzeEQMember(QRElementPtr elem, int &mvTables, int &ehTables, int &outTables,
  ...
  if (isPass1 == FALSE && extraHubID != NULL && candidate_->isAUsedExtraHubTable(extraHubID)) {
  ...
  if (isOutside) {
    ...
    if (elem->getElementType() == ET_Column)
      ...
      assertLogAndThrow(CAT_MATCHTST_MVDETAILS, LL_MVQR_FAIL, elem->getElementType() == ET_Expr, QRLogicException,
...
NABoolean MatchJoinPreds::analyzeProvidedExpression(const QRExprPtr expr) {
  ...
  if (rewrite->getResultCode() == RC_PROVIDED) candidate_->addOutputColumn(rewrite, FALSE);
...
NABoolean MatchJoinPreds::generateDescriptor(QRCandidatePtr resultDesc) {
  ...
  if (backJoinTables.entries() == 0) return TRUE;
  ...
  for (CollIndex i = 0; i < backJoinTables.entries(); i++) {
    ...
    if (addBackJoinPredsForTable(table, resultDesc) == FALSE) {
      ...
      // Disqualify the MV.
      candidate_->logMVWasDisqualified1("table %s cannot be back-joined.", table->getTableName().data());
      return FALSE;
    }
...
// ***************************************************************************
// ***************************************************************************
NABoolean MatchJoinPreds::addBackJoinPredsForTable(const QRTablePtr table, QRCandidatePtr resultDesc) {
  ...
  for (CollIndex i = 0; i < maxEntries; i++) {
    ...
    if (!isProvidedColumn(keyCol, mvOutput)) return FALSE;
...
NABoolean MatchOuterJoins::matchPass1() {
  ...
  if (!candidate_->getMvDetails()->getJbbDetails()->hasLOJs() && !candidate_->getQueryJbbDetails()->hasLOJs()) {
  ...
  for (CollIndex i = 0; i < maxEntries; i++) {
    ...
    if (queryJBBC->getElementType() != ET_Table) continue;
  ...
  if (queryExtraHubTables) {
    ...
    for (CollIndex i = 0; i < maxEntries; i++) {
      ...
      if (candidate_->isAUsedExtraHubTable(&queryTable->getID())) allTables.insert(queryTable);
    }
  }
  ...
  for (CollIndex j = 0; j < maxEntries; j++) {
    ...
    if (mvTableDetails == NULL) continue;  // Its an Outside table (not used by the MV).
...
NABoolean MatchOuterJoins::addTheNotNullPred(const QRTablePtr queryTable, const BaseTableDetailsPtr mvTableDetails) {
  ...
  for (CollIndex i = 0; i < maxEntries; i++) {
    ...
    QROutputPtr mvCol = NULL;
    if (!isProvidedColumn(keyCol, mvCol)) continue;

    RewriteInstructionsItemPtr rewrite =
        new (heap_) RewriteInstructionsItem(keyCol, mvCol, RC_PROVIDED, RS_FINAL, ADD_MEMCHECK_ARGS(heap_));
    addRewriteInstructions(rewrite);
...
// Not used.
// ***************************************************************************
NABoolean MatchOuterJoins::matchPass2OnElement(RewriteInstructionsItemPtr pending) { return TRUE; }
