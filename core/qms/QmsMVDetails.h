...
class MapIDsVisitor : public Visitor {
 public:
  MapIDsVisitor(QRElementHash &idHash, ADD_MEMCHECK_ARGS_DECL(CollHeap *heap))
  ...
  virtual Visitor::VisitResult visit(QRElementPtr caller) {
    ...
    if (id->length() > 0) idHash_.insert(id, caller);
...
class RefFinderVisitor : public Visitor {
 public:
  RefFinderVisitor(DescriptorDetailsPtr descDetails, ADD_MEMCHECK_ARGS_DECL(CollHeap *heap))
      : Visitor(ADD_MEMCHECK_ARGS_PASS(heap)), descDetails_(descDetails) {}
...
class BaseTableDetails : public NAIntrusiveSharedPtrObject {
 public:
  BaseTableDetails(const QRTablePtr table, const NABoolean isHub, ADD_MEMCHECK_ARGS_DECL(CollHeap *heap))
    ...
    for (CollIndex i = 0; i < (CollIndex)table->getNumCols(); i++) rangeColumnPredicates_.insertAt(i, NULL);
  }

  virtual ~BaseTableDetails() {}

  void addRangePredicateOnColumn(const QRRangePredPtr rangePred, const QRColumnPtr rangeCol);
  ...
  NABoolean operator==(const BaseTableDetails &other) { return name_ == other.name_; }

  const XMLBitmap &getRangeBits() const { return tableElement_->getRangeBits(); }

  const XMLBitmap &getResidualBits() const { return tableElement_->getResidualBits(); }

  const QRRangePredPtr getRangeColumnPredicatesFor(CollIndex pos) { return rangeColumnPredicates_[pos]; }
...
class JBBDetails : public NAIntrusiveSharedPtrObject {
 public:
   ...
   */
  JBBDetails(const QRJBBPtr jbbDesc, DescriptorDetailsPtr descDetails, NABoolean isAnMV,
  ...
  const QRJBBPtr getJbbDescriptor() const { return jbbDesc_; }
   ...
   * @return
   */
  BaseTableDetailsPtr getBaseTableByID(const NAString &id) const { return baseTablesByID_.getFirstValue(&id); }

  RangePredPtrList *getRangePredsOnExpression(const NAString &text) const {
    return rangePredicates_.getFirstValue(&text);
  }

  ResidualPredPtrList *getResidualPreds(const NAString &text) const { return residualPredicates_.getFirstValue(&text); }

  NABoolean hasNoRangePredicates() { return hasNoRangePredicates_; }

  NABoolean hasNoResidualPredicates() { return hasNoResidualPredicates_; }

  NABoolean hasHavingPredicates() { return hasHavingPredicates_; }

  NABoolean hasLOJs() { return hasLOJs_; }

  NABoolean isConstColumn(const NAString *id) { return constColumns_.contains(id); }
...
class DescriptorDetails : public NAIntrusiveSharedPtrObject {
 public:
   ...
   * @param heap The heap from which to allocate memory.
   */
  DescriptorDetails(QRDescriptorPtr descriptor, NABoolean isAnMV, ADD_MEMCHECK_ARGS_DECL(CollHeap *heap))
      : NAIntrusiveSharedPtrObject(ADD_MEMCHECK_ARGS_PASS(heap)),
   ...
   */
  const QRElementPtr getElementForID(const NAString &id) { return idHash_.getFirstValue(&id); }
 ...
 protected:
  void addJBBDetails(JBBDetailsPtr jbb) { jbbDetailsList_.insert(jbb); }
...
class MVDetails : public DescriptorDetails {
 public:
   ...
   * @param heap The heap from which to allocate memory.
   */
  MVDetails(QRMVDescriptorPtr descriptor, ADD_MEMCHECK_ARGS_DECL(CollHeap *heap))
   ...
   */
  const QRMVDescriptorPtr getMvDescriptor() { return static_cast<const QRMVDescriptorPtr>(getDescriptor()); }
   ...
   * @param ic
   */
  void setIgnoreChanges(NABoolean ic) { hasIgnoreChanges_ = ic; }

  /**
   * Set the IGNORE CHANGES state.
   * @param ic
   */
  void setConsistent(NABoolean ic) { isConsistent_ = ic; }

  /**
   * Set this MV is user maintained.
   * @param ic
   */
  void setUMV(NABoolean umv) { isUMV_ = umv; }
   ...
   * @return
   */
  NABoolean operator==(const MVDetails &other) { return !name_.compareTo(other.name_); }

  /**
   * Does this MV have a groupBy in it (or rather, in its top JBB)?
   * @return
   */
  NABoolean hasGroupBy() { return jbbDetails_->hasGroupBy(); }

  /**
   * Get the MV output column that is COUNT(*).
   * @return
   */
  QROutputPtr getCountStar() { return countStar_; }

  /**
   * Set the join graph of this MV
   * @param joinGraph
   */
  void setJoinGraph(QRJoinGraphPtr joinGraph) { joinGraph_ = joinGraph; }
   ...
   */
  const QRElementHash &getOutputByExprTextHash() { return outputByExprText_; }
   ...
   * @return
   */
  void addMVMemoGroup(MVMemoGroupPtr group) { mvMemoGroups_.insert(group); }
