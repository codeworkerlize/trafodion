...
NABoolean QmpPublish::setTarget(PublishTarget target, const char *targetFilename) {
  ...
  switch (target) {
    case PUBLISH_TO_QMM:
      ...
      if (!server_) {
        QRLogger::log(CAT_QR_IPC, LL_ERROR, "Unable to obtain QMM server.");
        return FALSE;
      }
      targetName_ = "QMM";
      break;
    ...
    case PUBLISH_TO_QMS:
      ...
      if (!server_) {
        QRLogger::log(CAT_QR_IPC, LL_ERROR, "Unable to obtain QMS server.");
        return FALSE;
      }
      targetName_ = "QMS";
      break;
    ...
    case PUBLISH_TO_FILE:
      ...
      if (!outFile_->rdbuf()->is_open()) {
        QRLogger::log(CAT_QMP, LL_ERROR, "Can't open output file ", targetFilename);
        return FALSE;
      }
...
void QmpPublish::performRewritePublishReading() {
  ...
  try {
    ...
    while (!wasTimeout) {
      ...
      if (!wasTimeout) preparePublishRewritePublishRowToSend(&publishData);
    ...
    QRLogger::log(CAT_QMP, LL_ERROR, "Error reading PUBLISH table from database - exiting.");
    exit(0);
  }
...
void QmpPublish::preparePublishRewritePublishRowToSend(MVQR_Publish *publishData) {
  ...
  const NAString *catalogName = sqlInterface_.getCatalogName(publishData->catalogUID_);

  // Get the schema definition version for this catalog
  const NAString *version = sqlInterface_.getSchemaVersion(publishData->catalogUID_);
  if (version == NULL) {
    QRLogger::log(CAT_QMP, LL_ERROR, "Not expecting an empty catalog.");
    return;
  }
  ...
  NAString *textTable = sqlInterface_.prepareDefinitionSchemaName(catalogName, version);
  *textTable += ".TEXT";
  ...
  NAString *mvDescriptorText = sqlInterface_.getMvDescriptor(*textTable, convert);

  // prepare the XML
  QRPublishDescriptorPtr pubDesc = createPublishDescriptor(publishData, mvDescriptorText);
  ...
  if (target_ == PUBLISH_TO_FILE) {
    *outFile_ << pubDescString;
    ...
    QR::QRRequestResult resultCode =
        MvQueryRewriteServer::sendPublishMessage(&pubDescString, targetName_, server_, getNAHeap());
  }
...
// * and details.
// ***************************************************************
QRPublishDescriptorPtr QmpPublish::createPublishDescriptor(MVQR_Publish *publishData, NAString *mvDescriptorText) {
  NAMemory *heap = getNAHeap();
  QRPublishDescriptorPtr pubDesc = new (getNAHeap()) QRPublishDescriptor(ADD_MEMCHECK_ARGS(heap));
  ...
  const NAString &objectNewName = (publishData->nullindObjectNewName_ < 0 ? "" : publishData->objectNewName_);

  NABoolean ignoreChangesUsed = FALSE;
  if (publishData->ignoreChangesUsed_ && strcmp(publishData->ignoreChangesUsed_, "1") == 0) ignoreChangesUsed = TRUE;

  pubDesc->initialize(convertOperationType(publishData->operationType_), &redefTimestamp, mvDescriptorText, &objectName,
                      ignoreChangesUsed, &refreshedAtTimestamp, &objectNewName, heap);
...
ComPublishMVOperationType QmpPublish::convertOperationType(char *operation) {
  if (operation == NULL) return COM_PUBLISH_MV_UNKNOWN;
