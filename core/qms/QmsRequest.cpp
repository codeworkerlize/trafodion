...
QRRequestResult QRMessageRequest::openXmlStream() {
  ...
  if (state == IpcMessageStream::RECEIVED) {
    if (msgStream_.moreObjects()) {
      ...
      QRLogger::log(CAT_QMS_MAIN, LL_ERROR, "In QRMessageRequest::openXmlStream(): no objects in stream");
      return ProtocolError;
    }
  } else {
    QRLogger::log(CAT_QMS_MAIN, LL_INFO, "In QRMessageRequest::openXmlStream(): message stream state is %d", state);
    return ProtocolError;
  }
...
QRRequestResult QRCommandLineRequest::openXmlStream() {
  if (isInlined_) return Success;

  char xmlFileName[100];
  inFile_ >> xmlFileName;
  QRLogger::log(CAT_QMS_MAIN, LL_DEBUG, "XML file containing descriptor is %s", xmlFileName);
  ...
  if (xmlFile_.fail()) QRLogger::log(CAT_QMS_MAIN, LL_ERROR, "Failed to open %s", xmlFileName);

  if (xmlFile_.rdbuf()->is_open())
    return Success;
  else {
    QRLogger::log(CAT_QMS_MAIN, LL_ERROR, "Could not open XML file %s for request.", xmlFileName);
    return BadFile;
  }
}  // End of openXmlStream
...
NABoolean QRCommandLineRequest::readRequestType(QRMessageTypeEnum &type) {
  ...
  if (inFile_.fail() == TRUE) return FALSE;
...
QmsGuaReceiveControlConnection::QmsGuaReceiveControlConnection(IpcEnvironment *env) : GuaReceiveControlConnection(env) {
  ...
  if (retcode == 0) {
    ...
    if (!dollar) dollar = QMS_PROCESS_PREFIX;
...
void QmsGuaReceiveControlConnection::actOnSystemMessage(short messageNum, IpcMessageBufferPtr sysMsg,
  ...
  } else if (messageNum == ZSYS_VAL_SMSG_CLOSE || messageNum == ZSYS_VAL_SMSG_PROCDEATH) {
#ifdef NA_WINNT
    QRLogger::log(CAT_QR_IPC, LL_INFO, "Client gone, exiting QMS with %d requestors remaining", getNumRequestors());
    NAExit(0);
#else
    if (isPrivateQms_) {
      QRLogger::log(CAT_QR_IPC, LL_INFO, "Client gone, private QMS %s exiting", procName_);
      NAExit(0);
...
void QmsMessageStream::actOnReceive(IpcConnection *connection) {
  QRLogger::log(CAT_QR_IPC, LL_DEBUG, "Reached QmsMessageStream::actOnReceive()");
  ...
  if (msgType != PUBLISH_REQUEST) respond(responseObj);
}

static void error(const char *text) { cerr << text << endl; }

static void usage(char *progName) {
  cerr << "Usage: " << progName << " infile outfile" << endl;
  QRLogger::log(CAT_QR_IPC, LL_ERROR, "Usage: %s infile outfile", progName);
}

// These static member functions are called from QmsMain.cpp.

QRRequestResult QRRequest::parseXMLDoc(QRRequest &request, XMLElementPtr &descriptor) {
  ...
  try {
    ...
    if (!descriptor) {
      QRLogger::log(CAT_QMS_MAIN, LL_WARN, "XMLDocument.parse() returned NULL.");
      return XMLParseError;
    } else {
      QRLogger::log(CAT_QMS_MAIN, LL_INFO, "Parsed XML document successfully.");
    }
  } catch (XMLException &ex) {
    QRLogger::log(CAT_QMS_MAIN, LL_MVQR_FAIL, "An XMLException occurred: %s", ex.getMessage());
    return XMLParseError;
  } catch (QRDescriptorException &ex) {
    QRLogger::log(CAT_QMS_MAIN, LL_MVQR_FAIL, "A QRDescriptorException occurred: %s", ex.getMessage());
    return XMLParseError;
  } catch (...) {
    QRLogger::log(CAT_QMS_MAIN, LL_MVQR_FAIL, "An Unknown exception occurred");
...
QRRequestResult QRRequest::handlePublishRequest(QRRequest &request, QRMessageStream *msgStream,
  ...
  if (msgStream) msgStream->respond(new QRStatusMessageObj(QR::Success));
  ...
  if (publishDescriptor->getElementType() != ET_PublishDescriptor) {
    QRLogger::log(CAT_QMS_MAIN, LL_ERROR, "XML document parsed ok, but had wrong document element -- %s",
...
QRRequestResult QRRequest::handleMatchRequest(QRRequest &request, XMLFormattedString &resultXML) {
  Qms &qms = *Qms::getInstance();
  ...
  if (result == Success) {
    ...
    QRQueryDescriptorPtr queryDescriptor = static_cast<QRQueryDescriptorPtr>(descriptor);
    QRResultDescriptorPtr resultDesc = qms.match(queryDescriptor, QmsInitializer::getInstance()->getHeap());
...
QRRequestResult QRRequest::handleInitializeRequest() {
  ...
  try {
    ...
    QRLogger::log(CAT_QMS_MAIN, LL_ERROR, "QMS initialization failure: %s", ex.getMessage());
    return InternalError;
  } catch (...) {
...
QRMessageObj *QRMessageRequest::processRequestMessage(QRMessageStream *msgStream) {
  ...
  if (!msgStream->moreObjects()) {
    QRLogger::log(CAT_QMS_MAIN, LL_ERROR, "QMS received an empty message stream.");
    ...
    switch (msgStream->getNextObjType()) {
      case MATCH_REQUEST:
        ...
        if (result == QR::Success) {
          ...
          responseMsgPtr = new QRXmlMessageObj(&resultXML, MATCH_RESPONSE);
        } else {
          QRLogger::log(CAT_QMS_MAIN, LL_ERROR, "handleMatchRequest() returned error status: %d", result);
      ...
      case PUBLISH_REQUEST:
        result = handlePublishRequest(request, msgStream, ADD_MEMCHECK_ARGS(qmsHeap));
        // collect and log qms stats
        if (qmsInitializer.doCollectQMSStats()) {
          qmsInitializer.logQMSStats();
        }
        ...
        if (result == QR::Success)
          QRLogger::log(CAT_QMS_MAIN, LL_INFO, "PUBLISH was successful.");

        else
          QRLogger::log(CAT_QMS_MAIN, LL_ERROR, "PUBLISH failed, status is %d", result);
      ...
      case CLEANUP_REQUEST:
        Qms::deleteInstance();
        QRLogger::log(CAT_QMS_MAIN, LL_DEBUG, "Qms instance deleted per CLEANUP request");
        dummyMsgObj.setType(msgStream->getNextObjType());
        msgStream->extractNextObj(dummyMsgObj, FALSE);
      ...
      case CHECK_REQUEST:
        QRLogger::log(CAT_QMS_MAIN, LL_WARN, "'Check Load' request not yet handled");
        dummyMsgObj.setType(msgStream->getNextObjType());
        msgStream->extractNextObj(dummyMsgObj, FALSE);
      ...
      case DEFAULTS_REQUEST:
        QRLogger::log(CAT_QMS_MAIN, LL_ERROR, "'Defaults' request not valid for QMS");
        dummyMsgObj.setType(msgStream->getNextObjType());
        msgStream->extractNextObj(dummyMsgObj, FALSE);
        responseMsgPtr = new QRStatusMessageObj(QR::InvalidRequest);
        break;
      ...
      default:
        QRLogger::log(CAT_QMS_MAIN, LL_ERROR, "Unhandled message: %d", msgStream->getNextObjType());
        noBadRequest = FALSE;
        responseMsgPtr = new QRStatusMessageObj(QR::InvalidRequest);
        break;
    }  // switch
...
int QRCommandLineRequest::processCommandLine(int argc, char *argv[]) {
  ...
  if (stricmp(argv[1], "redescribe") == 0) {
    ...
    if (stricmp(argv[2], "all") != 0) mvName = new NAString(argv[2]);

    NABoolean rePublish = FALSE;
    if (argc == 4 && stricmp(argv[3], "publish") == 0) rePublish = TRUE;
  ...
  while (moreInput) {
    ...
    switch (requestType) {
      case PUBLISH_REQUEST:
        result = handlePublishRequest(request, NULL, ADD_MEMCHECK_ARGS(heap));
        break;
      ...
      default:
        ...
        if (requestType == ALLOCATE_REQUEST)
          QRLogger::log(CAT_QMS_MAIN, LL_WARN, "ALLOCATE request was directed to QMS rather than QMM");
        else
          QRLogger::log(CAT_QMS_MAIN, LL_ERROR, "Unknown request type -- %s", request.getWrongRequestName());
        break;
    }  // switch(RequestType)
