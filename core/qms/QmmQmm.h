...
enum ListenOpt { RECEIVE, WAITONALL, WAITCC };
};  // namespace QR
...
class QmmException : public QRException {
 public:
   ...
   */
  QmmException(const char *msgTemplate...) : QRException() { qrBuildMessage(msgTemplate, msgBuffer_); }
...
class QmmGuaReceiveControlConnection : public GuaReceiveControlConnection {
 public:
  QmmGuaReceiveControlConnection(IpcEnvironment *env, Qmm *qmm) : GuaReceiveControlConnection(env), qmm_(qmm){};
...
class Qmm : public NABasicObject {
 public:
  static Qmm *getInstance(CollHeap *heap = NULL) {
    if (!instance_) instance_ = new Qmm(heap);
    return instance_;
  }

  const IpcEnvironment *getEnvironment() const { return ipcEnv_; }
  ...
  void setListenOpt(ListenOpt opt) { listenOpt_ = opt; }
...
class QRProcessStub : public NABasicObject {
 public:
  ...
  static const NAList<QRProcessStub *> getRestartList() { return restartList_; }
...
class QmsStub : public QRProcessStub {
 public:
  ...
  static void setQmsServerClass(IpcServerClass *serverClass) { qmsServerClass_ = serverClass; }

  enum Status { UNINITIALIZED = -1, RUNNING, NOT_RUNNING, CPU_NOT_PRESENT, CPU_NOT_REACHABLE, SEGMENT_NOT_REACHABLE };

  QmsStub(short segmentNumber, char *segmentName, short cpuNumber, short segmentStatus, NABoolean cpuExists,
  ...
  short getCpuNumber() const { return cpuNumber_; }

  short getSegmentNumber() const { return segmentNumber_; }
...
class QmpStub : public QRProcessStub {
 public:
  QmpStub(IpcEnvironment &ipcEnv, StartOpt qmpStartOpt, short cpu, CollHeap *heap)
      : QRProcessStub(heap), ipcEnv_(ipcEnv), qmpStartOpt_(qmpStartOpt), cpu_(cpu), debugQmp_(FALSE) {
    start();
  }
...
class QmmMessageStream : public QRMessageStream {
 public:
   ...
   */
  QmmMessageStream(IpcEnvironment *env, const NAString &thisEnd, Qmm *qmm, NAMemory *heap = NULL,
                   IpcMessageType msgType = UNSPECIFIED_QR_MESSAGE)
      : QRMessageStream(env, thisEnd, heap, msgType), qmm_(qmm) {}
