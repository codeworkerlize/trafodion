...
int QmsInitializer::performInitialization() {
  ...
  for (CollIndex i = 0; i < maxEntries; i++) delete (*catalogNames)[i];
  delete catalogNames;

  // end the transaction, if we started it
  if (startedTran) sqlInterface_.commitTransaction();
...
void QmsInitializer::processCatalogs(const NAStringList *catalogNames) {
  ...
  for (CollIndex i = 0; i < numberOfCatalogs_; i++) {
    ...
    if (schemaVersion == NULL) {
      ...
      QRLogger::log(CAT_QMS_INIT, LL_INFO, "Cannot get catalog Schema Version, catalog <%s> is empty", catalog->data());

      continue;
    }

    NAString *definitionSchema = sqlInterface_.prepareDefinitionSchemaName(catalog, schemaVersion);
    NAString textTable(*definitionSchema, heap_);
    textTable += ".TEXT";

    // Get all the MVs for this catalog
    QRMVDefinitionList *mvDefinitions = sqlInterface_.getMvUIDAndDefinitions(*definitionSchema);
    ...
    if (!mvDefinitions->isEmpty()) {
      ...
      for (CollIndex j = 0; j < maxEntries; j++) {
        ...
        const NAString *mvDescriptorText = sqlInterface_.getMvDescriptor(textTable, (const char *)uid);
      ...
      for (CollIndex k = 0; k < maxEntries; k++) NADELETE((*mvDefinitions)[k], QRMVDefinition, heap_);
      mvDefinitions->clear();
    }
...
void QmsInitializer::processMvDescriptor(const QRMVDefinition *mvDef, const NAString *mvDescriptorText) {
  ...
  try {
    ...
    if (mvDescriptor->getElementType() != ET_MVDescriptor) {
      QRLogger::log(CAT_QMS_INIT, LL_WARN, "XML document parsed ok, but had wrong document element -- %s",
    ...
    assertLogAndThrow(CAT_QMS_INIT, LL_ERROR, mvTableName != NULL, QRLogicException,
    ...
    if (qms_->contains(mvName)) {
      QRLogger::log(CAT_QMS_INIT, LL_INFO, "Dropping older definition of %s before re-inserting it.", mvName.data());
      qms_->drop(mvName);
    } else {
      QRLogger::log(CAT_QMS_INIT, LL_INFO, "Reading MV %s from SMD.", mvName.data());
    }

    qms_->insert(mvDesc, mvDef);
  } catch (QRException ex) {
    QRLogger::log(CAT_QMS_INIT, LL_MVQR_FAIL, "Exception thrown processing MV Descriptor: %s. Skipping.",
                  ex.getMessage());
  } catch (...) {
    QRLogger::log(CAT_QMS_INIT, LL_MVQR_FAIL, "Unknown exception thrown processing MV Descriptor. Skipping.");
  }
...
void QmsInitializer::reDescriber(NAString *mvName, NABoolean rePublish) {
  ...
  if (mvName) {
    QRLogger::log(CAT_QMS_INIT, LL_INFO, "ReDescribing MV %s with rePublish %s", mvName->data(),
                  rePublish ? "ON" : "OFF");

    // Regenerate the descriptor.
    sqlInterface_.reDescribeMV(mvName, rePublish);
  } else {
    QRLogger::log(CAT_QMS_INIT, LL_INFO, "ReDescribing all MVs with rePublish %s", rePublish ? "ON" : "OFF");
    ...
    if (result) reDescribeMVs(mvNames, rePublish);
...
// ***************************************************************
NABoolean QmsInitializer::collectMVNames(const NAStringList *catalogNames, NAStringList &mvNames) {
  NABoolean result = 0;

  if (catalogNames->isEmpty()) return TRUE;
  ...
  for (CollIndex i = 0; i < numberOfCatalogs_; i++) {
    ...
    // Get all the MVs for this catalog
    result = sqlInterface_.collectCatalogMVs(*catalog, *definitionSchema, mvNames);
