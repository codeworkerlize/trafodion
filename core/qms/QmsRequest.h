...
class QRRequest : public NAIntrusiveSharedPtrObject {
 public:
  ...
  QRRequest(ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0))
      : NAIntrusiveSharedPtrObject(ADD_MEMCHECK_ARGS_PASS(heap)), wrongRequestName_(heap) {}
   ...
   */
  static QRRequestResult parseXMLDoc(QRRequest &request, XMLElementPtr &descriptor);
  /**
   ...
   * @return Return code.
   */
  static QRRequestResult handlePublishRequest(QRRequest &request, QRMessageStream *msgStream,
   ...
   * @return Return code.
   */
  static QRRequestResult handleMatchRequest(QRRequest &request, XMLFormattedString &resultXML);
  ...
  const char *getWrongRequestName() { return wrongRequestName_.data(); }
...
class QRMessageRequest : public QRRequest {
 public:
   ...
   */
  QRMessageRequest(IpcMessageStream &msgStream, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0))
   ...
   */
  IpcMessageStream &getMessageStream() const { return msgStream_; }
  ...
  IpcMessageType getType() const { return msgStream_.getType(); }
...
class QRCommandLineRequest : public QRRequest {
 public:
  ...
  QRCommandLineRequest(ifstream &inFile, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0))
      : QRRequest(ADD_MEMCHECK_ARGS_PASS(heap)), inFile_(inFile), isInlined_(FALSE), inlinedLineRead_(FALSE) {}
 ...
 protected:
  size_t readXmlFile(char *buffer, size_t bufferSize) {
    if ((!xmlFile_.rdbuf()->is_open()) || (xmlFile_.eof())) return 0;
  ...
  size_t readXmlInlined(char *buffer, size_t bufferSize) {
    if (!inFile_.rdbuf()->is_open() || inlinedLineRead_) return 0;
...
class QmsMessageStream : public QRMessageStream {
 public:
   ...
   */
  QmsMessageStream(IpcEnvironment *env, const NAString &thisEnd, NAMemory *heap = NULL,
