...
ShiftMatrix::ShiftMatrix(int size, ADD_MEMCHECK_ARGS_DECL(CollHeap *heap))
  ...
  theMatrix_ = new (heap) Array2D(combinations_, elements_, 9999, ADD_MEMCHECK_ARGS(heap));
...
void ShiftMatrix::init() {
  ...
  for (CollIndex i = 0; i < elements_; i++) usedValues.insert(i);  // Start with all bits set.
...
ShiftMatrixFactoryPtr ShiftMatrixFactory::getInstance(NAMemory *heap) {
  if (!instance_) instance_ = new (heap) ShiftMatrixFactory(heap);
  return instance_;
}
...
void SelfJoinHandler::addSegment(SelfJoinSegment::SegmentType type) {
  SelfJoinSegmentPtr newSegment =
      new (heap_) SelfJoinSegment(type, segmentStart_, segmentEnd_, ADD_MEMCHECK_ARGS(heap_));
...
void SelfJoinHandler::preparePermutationMatrix() {
  ...
  permMatrix_ = new (heap_) Array2D(matrixWidth, totalPermutations_, 9999, ADD_MEMCHECK_ARGS(heap_));
  ...
  for (int row = permMatrix_->getRows() - 1; row >= 0; row--) {
    ...
    for (UInt32 rep = 0; rep < setReps; rep++) {
      ...
      for (UInt32 num = 0; num < perms; num++) {
        ...
        for (UInt32 col = 0; col < dups; col++) permMatrix_->setElement(row, dupStart + col, num);
      }
    }
...
void SelfJoinHandler::getNextShiftVector(int *shiftVector) {
  ...
  for (UInt32 segmentNo = 0; segmentNo < segmentArray_.entries(); segmentNo++) {
    ...
    if (segment->isSelfJoinSegment()) {
      ...
      for (CollIndex i = segment->getStart(); i <= segment->getEnd(); i++) shiftVector[i] = 0;
    }
  }
}
...
void SelfJoinHandler::dump(NAString &text) {
  ...
  for (CollIndex i = 0; i < maxEntries; i++) {
    SelfJoinSegmentPtr seg = segmentArray_[i];
    sprintf(buffer, "[%c, %d - %d] ", seg->isSelfJoinSegment() ? 'S' : 'U', seg->getStart(), seg->getEnd());
    text += buffer;
  }
