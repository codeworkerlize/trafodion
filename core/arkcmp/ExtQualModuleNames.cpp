...
ExtQualModuleNames::tokenType ExtQualModuleNames::checkReserved() {
  if (currentToken_ == "MODULEGROUP") return MODULEGROUP;
  if (currentToken_ == "MODULETABLESET") return MODULETABLESET;
  if (currentToken_ == "MODULEVERSION") return MODULEVERSION;
  return ID;
}
  ...
  while (!atEnd()) {
    ...
    if (isspace((unsigned char)cc)) {  // For VS2003...
      ...
      if (ToInternalIdentifier(id) != 0) {
        *mxCUMptr << FAIL << DgSqlCode(-2215) << DgString0(currentToken_.c_str());
      }
    ...
    switch (cc) {
          ...
          if (c1 == '"') {
            if (currentChar() == '"') {
              ...
              if (ToInternalIdentifier(id, FALSE, TRUE) != 0) {
                *mxCUMptr << FAIL << DgSqlCode(-2209) << DgString0(currentToken_.c_str());
              }
              currentToken_ = id.data();
              return ID;
            }
      ...
      default:
        advanceChar();
        *mxCUMptr << FAIL << DgSqlCode(-2211) << DgString0(currentToken_.c_str());
        return SCANERROR;
    }
  }
  return SCANEOF;
...
const char *ExtQualModuleNames::currentTokenString() {
  if (currentToken_.length()) return currentToken_.c_str();

  if (atEnd()) return "end-of-string";
...
void ExtQualModuleNames::match(tokenType t) {
  if (nextToken() == t) {           // matches t. all is OK.
    ...
    *mxCUMptr << FAIL << DgSqlCode(-2212) << DgString0(tokenString(t)) << DgString1(currentTokenString());
  }
}
...
void ExtQualModuleNames::parseModule(std::string &cat, std::string &sch, std::string &mod) {
      ...
      if (nextToken() == ID) {
        ...
        if (nextToken() == DOT) {
          ...
          if (nextToken() == ID) {  // module is <cat>.<sch>.<mod>
            ...
            *mxCUMptr << FAIL << DgSqlCode(-2213) << DgString0(currentToken_.c_str());
          }
        } else {      // nextToken != DOT
...
void ExtQualModuleNames::parseQualifiers(std::string &grp, std::string &tgt, std::string &ver) {
...
void ExtQualModuleNames::composeEQMN(std::string &grp, std::string &cat, std::string &sch, std::string &mod,
                                     std::string &tgt, std::string &ver, ThreePartModuleName &eqmn) {
  if (cat.length()) {
    eqmn.catalog = cat;
  }
  if (sch.length()) {
...
void ExtQualModuleNames::usage() {
  cerr << "A module name list should be" << endl
