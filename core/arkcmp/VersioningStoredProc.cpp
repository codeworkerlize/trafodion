...
SP_STATUS VersionInfoStoredProcedure::sp_InputFormat(SP_FIELDDESC_STRUCT *inputFieldFormat, int numFields,
  ...
  strcpy(&((inputFieldFormat++)->COLUMN_DEF[0]),
         "rValue varchar(1544 bytes) character set utf8");  // Max length cat.sch.obj, external format
  // 1544 = (128*4+2)*3+2
  return SP_SUCCESS;
}
...
//
SP_STATUS
VersionInfoStoredProcedure::sp_Process(SP_PROCESS_ACTION action, SP_ROW_DATA inputData, SP_EXTRACT_FUNCPTR eFunc,
  ...
  switch (action) {
    case SP_PROC_OPEN: {
      ...
      if (!getVarcharInputParameter(0, eFunc, inputData, sizeof(inputType), inputType, error)) return SP_FAIL;
      ...
      *spProcHandle = context;
      ...
      try {
        ...
        switch (iType) {
          ...
          case iTableAll: {
            ...
            object = context->handleInputValue(COM_TABLE_NAME, COM_BASE_TABLE_OBJECT, TRUE  // Resolve synonym
                                               ,
                                               error);
            if (object == NULL) return SP_FAIL;
          ...
          case iIndexTable: {
            ...
            object = context->handleInputValue(COM_INDEX_NAME, COM_INDEX_OBJECT, FALSE  // Don't resolve synonym
                                               ,
                                               error);
            if (object == NULL) return SP_FAIL;
          ...
          case iSchema: {
            ...
            if (schemaName.isValid()) {
              ...
              if (!validateInputValue(schemaName, context, error)) return SP_FAIL;
            } else {
              // Could be 3-part object name, try that.
              ComObjectName tableName(context->getInputValue(), COM_TABLE_NAME, FALSE, STMTHEAP);
              if (!validateInputValue(tableName, context, error)) return SP_FAIL;
            ...
            // Produce output information
            context->setOutputVersion(cache.getSchemaNodeSet(schemaName, context->getNodeSet()));
          ...
          case iSystemSchema: {
            ...
            if (*context->getInputValue() == 0) nodeName.setLocal();

            if (!validateInputValue(nodeName, context, error)) return SP_FAIL;
          ...
          case iView: {
            ...
            object = context->handleInputValue(COM_TABLE_NAME, COM_VIEW_OBJECT, TRUE  // Resolve synonym
                                               ,
                                               error);
            if (object == NULL) return SP_FAIL;
          ...
          case iProcedure: {
            // A stored procedure
            object = context->handleInputValue(COM_TABLE_NAME, COM_USER_DEFINED_ROUTINE_OBJECT,
          ...
          case iConstraint: {
            // A constraint
            object = context->handleInputValue(COM_CONSTRAINT_NAME, COM_UNKNOWN_OBJECT  // don't care about object type
                                               ,
                                               FALSE  // Don't resolve synonym
                                               ,
                                               error);
          ...
          case iTrigger: {
            // A trigger
            object = context->handleInputValue(COM_TRIGGER_NAME, COM_TRIGGER_OBJECT, FALSE  // Don't resolve synonym
                                               ,
                                               error);
            if (object == NULL) return SP_FAIL;
          } break;
          ...
          case iSynonym: {
            // A synonym
            object = context->handleInputValue(COM_TABLE_NAME, COM_SYNONYM_OBJECT, FALSE  // Don't resolve synonym
                                               ,
                                               error);
            if (object == NULL) return SP_FAIL;
          } break;
      ...
      for (rowIndex = 0; rowIndex < nodeSet.entries(); rowIndex++) {
        ...
        if (element.getNodeAccessError() != FEOK) {
          ...
          CatWarning(CatErrorCode(DISTRIBUTION_NODE_IS_UNAVAILABLE), DgString0(element.getNodeName()),
...
void VersionInfoStoredProcedure::Initialize(SP_REGISTER_FUNCPTR regFunc) {
  regFunc((char *)VERSION_INFO, sp_Compile, sp_InputFormat, 0, sp_NumOutputFields, sp_OutputFormat, sp_Process, 0,
          CMPISPVERSION);
}
...
SP_STATUS RelatednessStoredProcedure::sp_InputFormat(SP_FIELDDESC_STRUCT *inputFieldFormat, int numFields,
  ...
  strcpy(&((inputFieldFormat++)->COLUMN_DEF[0]), "rType  varchar(32) character set iso88591");  // Input type
  strcpy(&((inputFieldFormat++)->COLUMN_DEF[0]),
         "rValue varchar(1029 bytes) character set utf8");  // Max length cat.sch, external format, in UTF8
  return SP_SUCCESS;
}
...
//
SP_STATUS
RelatednessStoredProcedure::sp_Process(SP_PROCESS_ACTION action, SP_ROW_DATA inputData, SP_EXTRACT_FUNCPTR eFunc,
                                       SP_ROW_DATA outputData, SP_FORMAT_FUNCPTR fFunc, SP_KEY_VALUE keys,
                                       SP_KEYVALUE_FUNCPTR kFunc, SP_PROCESS_HANDLE *spProcHandle, SP_HANDLE spObj,
                                       SP_ERROR_STRUCT *error) {
      ...
      *spProcHandle = context;
      ...
      try {
        ...
        switch (rType) {
          case rCatalog: {
            // A catalog
            ComAnsiNamePart catalogName(context->getInputValue(), ComAnsiNamePart::EXTERNAL_FORMAT, STMTHEAP);
            if (!validateInputValue(catalogName, context, error)) return SP_FAIL;
...
void RelatednessStoredProcedure::Initialize(SP_REGISTER_FUNCPTR regFunc) {
  regFunc((char *)RELATEDNESS, sp_Compile, sp_InputFormat, 0, sp_NumOutputFields, sp_OutputFormat, sp_Process, 0,
          CMPISPVERSION);
}
...
//
SP_STATUS
FeatureVersionInfoStoredProcedure::sp_Process(SP_PROCESS_ACTION action, SP_ROW_DATA inputData, SP_EXTRACT_FUNCPTR eFunc,
                                              SP_ROW_DATA outputData, SP_FORMAT_FUNCPTR fFunc, SP_KEY_VALUE keys,
    ...
    case SP_PROC_OPEN: {
      ...
      *spProcHandle = context;
      ...
      try {
        ...
        if (!validateInputVersion(inputVersion, context, error)) return SP_FAIL;

        // Do the job ...
        switch (fType) {
          case fCatalog:
          case fCatalogCascade: {
            if (fType == fCatalogCascade) doCascade = TRUE;  // Cascade option was specified, set boolean
            ...
            // Produce output information
            context->getFeatureVersionInfoSet().buildFeatureVersionInfoSet(catalogName, doCascade, inputVersion);
...
void FeatureVersionInfoStoredProcedure::Initialize(SP_REGISTER_FUNCPTR regFunc) {
  regFunc((char *)FEATURE_VERSION_INFO, sp_Compile, sp_InputFormat, 0, sp_NumOutputFields, sp_OutputFormat, sp_Process,
          0, CMPISPVERSION);
}
...
void VersioningSPContextBase::setInputValue(const varchar *inputValue) {
  ...
  if (eValue_ != NULL) NADELETEBASIC(eValue_, STMTHEAP);
...
const CatROObject *VersionInfoSPContext::handleInputValue(const ComAnsiNameSpace nameSpace,
                                                          const ComObjectType expectedObjectType,
                                                          const NABoolean resolveSynonym, SP_ERROR_STRUCT *error) {
  ComObjectName objName(getInputValue(), nameSpace, FALSE, STMTHEAP);
  if (!validateInputValue(objName, this, error)) return NULL;
  ...
  if (expectedObjectType != COM_UNKNOWN_OBJECT)  // don't care ...
  {
    ...
    if (objectObjectType == COM_MV_OBJECT) objectObjectType = COM_VIEW_OBJECT;
...
// Deallocation method
void VersionInfoSPContext::deleteMe(void) { NADELETE(this, VersionInfoSPContext, STMTHEAP); }
...
// Deallocation method
void RelatednessSPContext::deleteMe(void) { NADELETE(this, RelatednessSPContext, STMTHEAP); }
...
// Deallocation method
void FeatureVersionInfoSPContext::deleteMe(void) { NADELETE(this, FeatureVersionInfoSPContext, STMTHEAP); }
...
NABoolean getVarcharInputParameter(int fieldNo, SP_EXTRACT_FUNCPTR eFunc, SP_ROW_DATA inputData, size_t maxSize,
  ...
  // leading blanks
  size_t numBlanks = 0;
  while (varInput->val[numBlanks] == ' ') numBlanks++;

  if (numBlanks) {
    // strip leading blanks
    for (size_t i = 0; i < (varInput->len - numBlanks); i++) varInput->val[i] = varInput->val[i + numBlanks];
    while (numBlanks--) varInput->val[--(varInput->len)] = 0;
  }
...
void reportInputError(VersioningSPContextBase *context, SP_ERROR_STRUCT *error) {
  // Report error -19017
...
NABoolean validateInputValue(const ComObjectName &object, VersioningSPContextBase *context, SP_ERROR_STRUCT *error) {
  ...
  // in external format
  if (object.isValid() && object.getCatalogNamePart().isValid() && object.getSchemaNamePart().isValid()) return TRUE;
...
NABoolean validateInputValue(const ComSchemaName &schema, VersioningSPContextBase *context, SP_ERROR_STRUCT *error) {
  // Validate that the input schema name is a valid 2-part schema name
  // in external format
  if (schema.isValid() && schema.getCatalogNamePart().isValid()) return TRUE;
...
NABoolean validateInputValue(const ComAnsiNamePart &catalog, VersioningSPContextBase *context, SP_ERROR_STRUCT *error) {
  // Validate that the input catalog name is a valid ANSI identifier
  // in external format
  if (catalog.isValid()) return TRUE;
...
NABoolean validateInputVersion(int inVersion, VersioningSPContextBase *context, SP_ERROR_STRUCT *error) {
  ...
  if (ComVersion_GetMXVersionVector((short)inVersion, oldestSupportedMXV, oldestSupportedSchemaVersion,
                                    oldestSupportedSystemSchemaVersion, oldestSupportedPlanVersion))
    return TRUE;
