...
static void CmpSPERROR2Diags(const SP_ERROR_STRUCT *spError, ComDiagsArea *diags) {
  if (!diags) return;
  ...
  if (spError[0].error == arkcmpErrorISPMergeCatDiags) {
    ...
    *diags << DgSqlCode(-4222) << DgString0("CmpSPERROR2Diags");
    return;
  }
    ...
    if (pSET->error) {
      *diags << DgSqlCode(pSET->error);
      if (pSET->error == -20067) {  // IDS_UTILITIES_BADSYNTAX = 20067
        ...
        if (pSET->optionalString[0] && pSET->optionalString[0] != (char *)ComDiags_UnInitialized_Int)
          *diags << DgString0(pSET->optionalString[0]);
        if (pSET->optionalString[1] && pSET->optionalString[1] != (char *)ComDiags_UnInitialized_Int)
          *diags << DgString1(pSET->optionalString[1]);
        if (pSET->optionalString[2] && pSET->optionalString[2] != (char *)ComDiags_UnInitialized_Int)
          *diags << DgString2(pSET->optionalString[2]);
        if (pSET->optionalString[3] && pSET->optionalString[3] != (char *)ComDiags_UnInitialized_Int)
          *diags << DgString3(pSET->optionalString[3]);
        if (pSET->optionalString[4] && pSET->optionalString[4] != (char *)ComDiags_UnInitialized_Int)
          *diags << DgString4(pSET->optionalString[4]);
        if (pSET->optionalInteger[0] != ComDiags_UnInitialized_Int) *diags << DgInt0(pSET->optionalInteger[0]);
        if (pSET->optionalInteger[1] != ComDiags_UnInitialized_Int) *diags << DgInt1(pSET->optionalInteger[1]);
        if (pSET->optionalInteger[2] != ComDiags_UnInitialized_Int) *diags << DgInt2(pSET->optionalInteger[2]);
        if (pSET->optionalInteger[3] != ComDiags_UnInitialized_Int) *diags << DgInt3(pSET->optionalInteger[3]);
        if (pSET->optionalInteger[4] != ComDiags_UnInitialized_Int) *diags << DgInt4(pSET->optionalInteger[4]);
      }
    } else if (i == 0) {
...
static ElemDDLColDef *SPFieldDesc2ElemDDLColDef(SP_FIELDDESC_STRUCT *fd, CmpContext *context) {
  ElemDDLColDef *elemDDL = 0;

  if (!fd) return 0;

  Parser parser(context);
  if (!(elemDDL = parser.parseColumnDefinition(fd->COLUMN_DEF, strlen(fd->COLUMN_DEF), CharInfo::UTF8))) {
...
NABoolean CmpSPInputFormat::SetFormat(int ncols, SP_FIELDDESC_STRUCT *fields) {
  ...
  for (int i = 0; i < ncols; i++, fd++) {
    ...
    if (inputType_)
      inputType_ = new ((CollHeap *)context_->statementHeap()) ItemList(inputType_, item);
    else
      inputType_ = item;
  }
  return TRUE;
...
NABoolean CmpSPOutputFormat::getColumnDesc(SP_FIELDDESC_STRUCT *fDesc, TrafColumnsDesc *colsDesc) {
  ...
  if (!(elemDDL = SPFieldDesc2ElemDDLColDef(fDesc, context_)))
    ...
    if (!ElemDDLColDef2ColumnDescStruct(elemDDL, tableDesc_->tableDesc()->tablename, colsDesc)) return FALSE;
  }
...
NABoolean CmpSPOutputFormat::ElemDDLColDef2ColumnDescStruct(ElemDDLColDef *elem, const char *tableName,
  ...
  if (genericType->getTypeQualifier() == NA_NUMERIC_TYPE) {
    ...
    if (nt.isExact() && nt.binaryPrecision() && (nt.getFSDatatype() != REC_BPINT_UNSIGNED)) {
      colDesc->precision = 0;
    } else {
  ...
  if (genericType->getTypeQualifier() == NA_DATETIME_TYPE || genericType->getTypeQualifier() == NA_INTERVAL_TYPE) {
    DatetimeIntervalCommonType &dti = (DatetimeIntervalCommonType &)*genericType;
...
CmpSPExecDataItem::~CmpSPExecDataItem() {
  if (SPFuncsDiags_) SPFuncsDiags_->decrRefCount();
}

CollHeap *CmpSPExecDataItem::wHeap() { return context_->heap(); }
...
CmpSPExecDataItemReply::CmpSPExecDataItemReply(int exprSize, void *expr, int dataRowSize, int dataTotalSize,
  ...
  rowBuffer_ = new (outHeap_) char[rowLength_];
...
CmpSPExecDataItemReply::~CmpSPExecDataItemReply() {
  ...
  NADELETEBASIC((char *)rowBuffer_, outHeap_);
  diags_.clear();  // this is to avoid memory leak.
}

NABoolean CmpSPExecDataItemReply::prepare() { return (ExSPPrepareReplyBuffer(data_) == 0); }
...
NABoolean CmpSPExecDataItemReply::MergeDiags(ComDiagsArea *pDiags) {
  if (pDiags == NULL || pDiags->getNumber() <= 0) return FALSE;
...
short CmpSPExecDataItemReply::AddARow() {
  ...
  rowExist_ = TRUE;
  short status = 0;
  status = ExSPPutReplyRow(data(), inputData_->control(), (char *)rowBuffer_, rowLength_, 0);
...
short CmpSPExecDataItemReply::AddEOR() {
  ...
  EORExist_ = TRUE;
  if (diagsExist_) {
    status = ExSPPutReplyRow(data(), inputData_->control(), 0, 0, &diags_);
    if (status == 1)  // buffer is full
      return 1;
    diagsExist_ = FALSE;
    diags_.clear();
...
CmpISPDataObject::CmpISPDataObject(CmpMessageISPRequest *req, CmpInternalSP *isp, CollHeap *outheap,
                                   CmpContext *context)
    : input_(req->inputExprSize(), (void *)req->inputExpr(), req->inputDataSize(), (void *)req->inputData(), context),
      key_(req->keyExprSize(), (void *)req->keyExpr(), req->keyDataSize(), (void *)req->keyData(), context),
      output_(req->outputExprSize(), (void *)req->outputExpr(), req->outputRowSize(), req->outputTotalSize(), &input_,
              outheap, context),
      outHeap_(outheap),
      context_(context) {
  isp->SetCmpISPDataObject(this);
  ispRequest_ = req;
  CMPASSERT(ExSPUnpackIOExpr(input_.expr(), output_.expr(), context->statementHeap()) == 0);
}
...
CmpStoredProc::CmpStoredProc(const NAString &procName, CmpContext *cmpContext) : procName_(procName) {
...
CmpStoredProc::ExecStatus CmpStoredProc::open(CmpSPDataObject &) { return FAIL; }

CmpStoredProc::ExecStatus CmpStoredProc::fetch(CmpSPDataObject &) { return FAIL; }

CmpStoredProc::ExecStatus CmpStoredProc::close(CmpSPDataObject &) { return FAIL; }
...
NABoolean CmpISPFuncs::ValidPFuncs(const ProcFuncsStruct &pFuncs) const {
  return (pFuncs.procName_ && pFuncs.compileFunc_ && pFuncs.inFormatFunc_ && pFuncs.outNumFormatFunc_ &&
...
int CmpISPFuncs::RegFuncs(const char *procName,  // null terminated
  ...
  procFuncsArray_.insert(ProcFuncsStruct(procName, compileFunc, inFormatFunc, parseFunc, outNumFormatFunc,
                                         outFormatFunc, procFunc, spHandle));
  return 1;
}
...
CmpInternalSP::~CmpInternalSP() {
  if (state_ == COMPILE) {
    ...
    initSP_ERROR_STRUCT();
    if ((*(procFuncs_.compileFunc_))(SP_COMP_EXIT, &compHandle_, procFuncs_.spHandle_, &spError_[0]) != SP_SUCCESS)
      appendDiags();
  }
...
NABoolean CmpInternalSP::startCompileCycle() {
  ...
  if (!procFuncsLookupTable_.ValidPFuncs(procFuncs_)) {
    *(cmpContext()->diags()) << DgSqlCode(arkcmpErrorISPNotFound) << DgString0(procName().data());
    return FALSE;
  }
  if (state_ != COMPILE) {
    // needs to call the compileFunc_
    initSP_ERROR_STRUCT();
    if ((*(procFuncs_.compileFunc_))(SP_COMP_INIT, &compHandle_, procFuncs_.spHandle_, &spError_[0]) != SP_SUCCESS) {
      appendDiags();
      return FALSE;
    }
  }
...
void CmpInternalSP::appendDiags() { CmpSPERROR2Diags(spError_, cmpContext()->diags()); }
...
NABoolean CmpInternalSP::InputFormat(int num, CmpSPInputFormat &t) {
  ...
  initSP_ERROR_STRUCT();
  if ((*(procFuncs_.inFormatFunc_))(fields, num, compHandle_, procFuncs_.spHandle_, &spError_[0]) == SP_SUCCESS) {
    if (!(t.SetFormat(num, fields))) retStatus = FALSE;
  } else {
    retStatus = FALSE;
    appendDiags();
  }
  if (!retStatus) *(cmpContext()->diags()) << DgSqlCode(arkcmpErrorISPFieldDef);
...
NABoolean CmpInternalSP::OutputFormat(CmpSPOutputFormat &t) {
  ...
  if ((*(procFuncs_.outNumFormatFunc_))(&num, compHandle_, procFuncs_.spHandle_, &spError_[0]) == SP_SUCCESS) {
    if (num == 0) {
      ...
      if (!(t.SetFormat(1, outTableName.data(), &fields, 0, 0))) retStatus = FALSE;
    } else {
      SP_FIELDDESC_STRUCT *fields = allocSP_FIELDDESC_STRUCT(num);
...
NABoolean CmpInternalSP::ParseInput(const NAString &param) {
  ...
  initSP_ERROR_STRUCT();
  if ((*(procFuncs_.parseFunc_))((char *)param.data(), compHandle_, procFuncs_.spHandle_, &spError_[0]) != SP_SUCCESS) {
    appendDiags();
    return FALSE;
  }
  return TRUE;
...
CmpStoredProc::ExecStatus CmpInternalSP::open(CmpISPDataObject &data) {
  ...
  initSP_ERROR_STRUCT();
  SP_STATUS spStatus = (*(procFuncs_.procFunc_))(SP_PROC_OPEN, (SP_ROW_DATA)data.input(), CmpSPExtractFunc,
                                                 (SP_ROW_DATA)data.output(), CmpSPFormatFunc, (SP_KEY_VALUE)data.key(),
                                                 CmpSPKeyValueFunc, &procHandle_, procFuncs_.spHandle_, &spError_[0]);

  if (spStatus == SP_FAIL || spStatus == SP_SUCCESS_WARNING) {
    // Errors or warnings, go get them
    data.output()->MoveDiags(spError_, spStatus);
    if (spStatus == SP_FAIL) return FAIL;
  }
  state_ = PROCESS;
  return SUCCESS;
}
...
CmpStoredProc::ExecStatus CmpInternalSP::fetch(CmpISPDataObject &data) {
  ...
  if (procName() == "sp_partn" || procName() == "sp_populate" || procName() == "sp_purgedata" ||
      procName() == "sp_recover" || procName() == "sp_transform" || procName() == "sp_validate" ||
      procName() == "sp_purgedata" || procName() == "sp_refresh" || procName() == "sp_SchLevel") {
  ...
  if (Get_SqlParser_Flags(ALLOW_FUNNY_IDENTIFIER)) sendParserFlag(ALLOW_FUNNY_IDENTIFIER);

  initSP_ERROR_STRUCT();
  SP_STATUS spStatus = (*(procFuncs_.procFunc_))(SP_PROC_FETCH, (SP_ROW_DATA)data.input(), CmpSPExtractFunc,
                                                 (SP_ROW_DATA)data.output(), CmpSPFormatFunc, (SP_KEY_VALUE)data.key(),
                                                 CmpSPKeyValueFunc, &procHandle_, procFuncs_.spHandle_, &spError_[0]);
...
CmpStoredProc::ExecStatus CmpInternalSP::close(CmpISPDataObject &data) {
  ...
  initSP_ERROR_STRUCT();
  state_ = NONE;
  SP_STATUS spStatus = (*(procFuncs_.procFunc_))(SP_PROC_CLOSE, (SP_ROW_DATA)data.input(), CmpSPExtractFunc,
                                                 (SP_ROW_DATA)data.output(), CmpSPFormatFunc, (SP_KEY_VALUE)data.key(),
                                                 CmpSPKeyValueFunc, &procHandle_, procFuncs_.spHandle_, &spError_[0]);
