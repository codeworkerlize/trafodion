...
BaseException::BaseException() : lineNum_(0) { fileName_[0] = 0; }

UInt32 BaseException::getLineNum() { return lineNum_; }

const char *BaseException::getFileName() { return fileName_; }
...
void BaseException::hookDebugger() {
  ...
  if (invokeDebugger) hookDebuggerHelper();
}

void BaseException::hookDebuggerHelper() {
  MessageBox(NULL, "An Internal Error Found, Attach Debugger Now", "NonStop SQL/MX", MB_OK | MB_ICONINFORMATION);
...
UserException::UserException(const char *fileName, UInt32 lineNum) : BaseException(fileName, lineNum) {}
...
FatalException::FatalException(const char *msg, const char *fileName, UInt32 lineNum, const char *stackTrace)
...
const char *FatalException::getMsg() { return msg_; }

const char *FatalException::getStackTrace() { return stackTrace_; }
...
CmpInternalException::CmpInternalException(const char *msg, const char *fileName, UInt32 lineNum)
    : BaseException(fileName, lineNum) {
  if (msg) {
...
const char *CmpInternalException::getMsg() { return msg_; }
...
AssertException::AssertException(const char *condition, const char *fileName, UInt32 lineNum, const char *stackTrace)
    : BaseException(fileName, lineNum) {
  int len = 0;
...
AssertException::AssertException(AssertException &e) : BaseException(e.getFileName(), e.getLineNum()) {
  int len = 0;
...
const char *AssertException::getCondition() { return condition_; }

const char *AssertException::getStackTrace() { return stackTrace_; }
...
OsimLogException::OsimLogException(const char *errMsg, const char *srcFileName, UInt32 srcLineNum)
    : BaseException(srcFileName, srcLineNum) {
  if (errMsg) errMsg_ = errMsg;
}

void OsimLogException::throwException() { throw *this; }

// PassOneAssertFatalException Implementation

PassOneAssertFatalException::PassOneAssertFatalException(const char *condition, const char *fileName, UInt32 lineNum)
    : FatalException("Assertion occurred in optimizer pass one", fileName, lineNum) {
  if (condition) {
...
PassOneNoPlanFatalException::PassOneNoPlanFatalException(const char *fileName, UInt32 lineNum)
    : FatalException("Cannot produce a plan in optimizer pass one", fileName, lineNum) {}
...
PassOneSkippedPassTwoNoPlanFatalException::PassOneSkippedPassTwoNoPlanFatalException(const char *fileName,
                                                                                     UInt32 lineNum)
    : FatalException("Pass one skipped, but cannot produce a plan in pass two", fileName, lineNum) {}
...
void CmpExceptionCallBack::throwFatalException(const char *msg, const char *file, UInt32 line, const char *stackTrace) {
  FatalException(msg, file, line, stackTrace).throwException();
}

void CmpExceptionCallBack::throwAssertException(const char *cond, const char *file, UInt32 line,
...
void CmpExceptionEnv::registerCallBack() { NAInternalError::registerExceptionCallBack(&eCallBack_); }

void CmpExceptionEnv::unRegisterCallBack() { NAInternalError::unRegisterExceptionCallBack(); }
