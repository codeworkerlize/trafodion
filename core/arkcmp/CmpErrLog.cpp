...
void CmpErrLog::renameBigLogFile(const char *fileName) {
  struct stat sbuf;
  if (stat(fileName, &sbuf) != 0) return;
...
void CmpErrLog::openLogFile() {
  ...
  if (!logFileName || logFileName[0] == '\0' || strlen(logFileName) > MAX_LOGFILE_NAME_LEN) return;
...
void CmpErrLog::closeLogFile() {
  ...
  if (memPtr == NULL) memPtr = malloc(SUBSEQUENT_MEM_RESERVE_SIZE);
}
...
void CmpErrLog::writeMemoryStats() {
  ...
  if ((meminfoFD = open("/proc/meminfo", O_RDONLY)) == -1) {
    ...
    while ((numBytes = read(meminfoFD, buf, sizeof(buf))) > 0) fwrite(buf, sizeof(char), numBytes, fp);
    fputc('\n', fp);
  ...
  if ((procStatusFD = open(procStatusFile, O_RDONLY)) == -1) {
    ...
    while ((numBytes = read(procStatusFD, buf, sizeof(buf))) > 0) fwrite(buf, sizeof(char), numBytes, fp);
    fputc('\n', fp);
...
void CmpErrLog::writeAllHeapInfo(CollHeap *failedHeap) {
  ...
  if (failedHeap != NULL) writeHeapInfo(failedHeap);

  CmpContext *context = CmpCommon::context();

  // Write the context heap information
  CollHeap *contextHeap = context->heap();
  if (contextHeap != NULL && contextHeap != failedHeap) writeHeapInfo(contextHeap);

  // Write the statement heap information
  CollHeap *statementHeap = context->statementHeap();
  if (statementHeap != NULL && statementHeap != failedHeap) writeHeapInfo(statementHeap);
}
...
void CmpErrLog::writeQueryInfo() {
  ...
  if (statement->userSqlText() != NULL) fprintf(fp, "SQL Text:\n%s\n\n", statement->userSqlText());
}
...
CmpErrLog::CmpErrLog(const char *failureTxt, CollHeap *failedHeap, size_t size) : fp(0) {
