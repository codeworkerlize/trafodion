...
int ValidateAnsiList::validate(const char *value, const NADefaults *nad, int attrEnum, int errOrWarn, float *) const {
  ...
  while (token != NULL) {
    ...
    for (int i = 0; !anyError && i < (int)tokenObj.length(); i++) {
      if (ComSqlText.isDigit(token[i]) || ComSqlText.isSimpleLatinLetter(token[i]))
        ...
        if (ComSqlText.getPeriod() == token[i] &&  // it is a period
            countPeriods++ < 2) {
          if (inBetween == 0)
            anyError = TRUE;  // no CATALOG or SCHEMA
          else
            inBetween = 0;  // start counting the next ( SCHEMA or NAME )
        } else
          anyError = TRUE;
      }
    }
    ...
    if (anyError) {
      if (errOrWarn)
        *CmpCommon::diags() << DgSqlCode(ERRWARN(2055)) << DgString0(token) << DgString1("INVALID QUALIFIED NAME");
      return FALSE;
    }
...
int ValidateNumericRange::validate(const char *value, const NADefaults *nad, int attrEnum, int errOrWarn,
  ...
  if (alreadyHaveFloat || isValid) {
    ...
    if (rangeOK) {
      multipleOK = (multiple_ == 0 || ((int)flt) % multiple_ == 0);
      if (multipleOK) {
        return TRUE;  // valid
      }
    }
    ...
    *CmpCommon::diags() << DgSqlCode(ERRWARN(2055)) << DgString0(value)
    ...
    if (!rangeOK) {
      ...
      if (type_ == VALID_INT) {
        ...
        sprintf(minbuf, "%u", (int)min_);
        if (max_ == 2147483520)
          sprintf(maxbuf, "%u", (int)max_ + 64);
        else
          sprintf(maxbuf, "%u", (int)max_);
    ...
    if (multiple_ && !multipleOK) *CmpCommon::diags() << DgSqlCode(ERRWARN(2057)) << DgInt0(multiple_);
  }
    ...
    retry_as_other_format:
      ...
      if (nsk) {
        ...
        if (collMP.isValid(ComMPLoc::FILE)) {
          ok = TRUE;
          if (cdb) {
            ncThis->lastCoInserted_ = cdb->insert(collMP, defaultMPLoc, flags);
          }
        }
      } else {
        ...
        if (collMX.isValid()) {
          ...
          if (cdb) {
            ...
            ncThis->lastCoInserted_ = cdb->insert(collQN, defaultSchema, flags);
          }
        }
      }
      ...
      if (!ok && errOrWarn)
        *CmpCommon::diags() << DgSqlCode(ERRWARN(2055)) << DgString0(collStr)
                            << DgString1(nad->lookupAttrName(attrEnum, errOrWarn));
...
UInt32 ValidateCollationList::insertIntoCDB(SchemaDB *sdb, CollationDB *cdb, const char *value, NABoolean nsk) {
...
int ValidateOverrideSchema::validate(const char *value, const NADefaults *nad, int attrEnum, int errOrWarn,
  ...
  if ((fromSchema.isNull()) || (toSchema.isNull()))
    ...
    if (!targetSchema.isValid())
      ok = FALSE;
    else if (fromSchema != "*")  // reserve * for wildcard operation
    {
      ComSchemaName sourceSchema(fromSchema);
      if (!sourceSchema.isValid()) ok = FALSE;
    }
  }
...
int ValidateReplIoVersion::validate(const char *value, const NADefaults *nad, int attrEnum, int errOrWarn,
  ...
  if (alreadyHaveFloat || isValid) {
    if (Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL))
      ...
      min = min_;
    if (flt >= min && flt <= max_) return TRUE;  // valid
  }
