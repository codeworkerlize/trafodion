...
NamedSemaphore::NamedSemaphore(const char *semName) : semId_(NULL), semPid_(-1), lastOpenStatus_(-1) {
...
NamedSemaphore::~NamedSemaphore() { closeSemaphore(); }
...
int NamedSemaphore::openSemaphore() {
  ...
  if (error == 0) semId_ = ln_semId;
  return error;
}

int NamedSemaphore::closeSemaphore() {
  if (semId_ == NULL) return -2;
...
void NamedSemaphore::display(const char *msg) {
  ...
  out << msg << ", status of NamedSemaphore:"
      << " this=" << static_cast<void *>(this) << " semName_=" << semName_ << " semId_=" << semId_
      << ", semPid_=" << semPid_ << ", lastOpenStatus_=" << lastOpenStatus_ << ", errno (EINTR=" << EINTR
      << ", EINVAL=" << EINVAL << ", EAGAIN=" << EAGAIN << ", ETIMEDOUT=" << ETIMEDOUT << ")" << endl;
  out.close();
}
  ...
  for (int i = 0; i < n; i++) {
    ...
    switch (pid) {
      ...
      default:
        // parent process case
        pids[i] = pid;
        if (i == n - 1) visitCriticalSection(i);
    }
  }
...
NamedSem::NamedSem(key_t semKey) : semId_(-1), semPid_(-1), semKey_(semKey) { getSemaphore(true); }
...
int NamedSem::getSemaphore(NABoolean create) {
  ...
  if (semId_ == -1) {
    ...
    if (error == 0) semId_ = semid;
  }
  return error;
}

int NamedSem::removeSemaphore() {
  if (semId_ == -1) return -2;
...
int NamedSem::lockSemaphore(NABoolean lock) {
  ...
  if (lock)
    QRLogger::log(CAT_SQL_SHARED_CACHE, LL_TRACE,
                  "before lock: value is %d, semId_ is %d, sem_op.sem_op = %d, errno = %d\n",
                  semctl(semId_, 0, GETVAL, 0), semId_, sem_op.sem_op, errno);
  else
    QRLogger::log(CAT_SQL_SHARED_CACHE, LL_TRACE,
                  "before unlock: value is %d, semId_ is %d, sem_op.sem_op = %d, errno = %d\n",
                  semctl(semId_, 0, GETVAL, 0), semId_, sem_op.sem_op, errno);
