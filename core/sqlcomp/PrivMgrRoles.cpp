...
static bool isDependentObjectPrivPair(ComObjectType objectType, PrivMgrBitmap objectPrivs);

static bool isNeedToInvalidCache(std::map<int32_t, std::set<int32_t> *> &, int32_t, int32_t);

#define FREE_USER_ROLES_CACHE(userRolesCache)                                                                          \
  {                                                                                                                    \
    for (std::map<int32_t, std::set<int32_t> *>::iterator itor = userRolesCache.begin(); itor != userRolesCache.end(); \
         itor++) {                                                                                                     \
      if (NULL != itor->second) {                                                                                      \
        delete itor->second;                                                                                           \
        itor->second = NULL;                                                                                           \
...
#define NOT_NEED_GRANT_TO_GROUPS(noRoleGrantToGroups)                                                   \
  {                                                                                                     \
    noRoleGrantToGroups =                                                                               \
...
class MyRow : public PrivMgrMDRow {
 public:
  ...
  // -------------------------------------------------------------------
  MyRow(std::string tableName) : PrivMgrMDRow(tableName, ROLE_USAGES_ENUM), roleID_(0){};
...
class MyTable : public PrivMgrMDTable {
 public:
  MyTable(const std::string &tableName, ComDiagsArea *pDiags = NULL)
      : PrivMgrMDTable(tableName, ROLE_USAGES_ENUM, pDiags), lastRowRead_(tableName){};

  virtual PrivStatus insert(const PrivMgrMDRow &row);

  PrivStatus insertSelect(const std::string &authsLocation, const std::string &inClause);

  PrivStatus selectAllWhere(const std::string &whereClause, const std::string &orderByClause, std::vector<MyRow> &rows);

  PrivStatus selectAuthGrantees(std::string whereClause, std::vector<int32_t> &granteeIDs);
...
PrivMgrRoles::PrivMgrRoles()
    : PrivMgr(),
      fullTableName_(metadataLocation_ + "." + PRIVMGR_ROLE_USAGE),
      myTable_(*new MyTable(fullTableName_, pDiags_)){};

PrivMgrRoles::PrivMgrRoles(const std::string &trafMetadataLocation, const std::string &metadataLocation,
                           ComDiagsArea *pDiags)
...
// Destructor.
// -----------------------------------------------------------------------
PrivMgrRoles::~PrivMgrRoles() { delete &myTable_; }
...
// *                                                                           *
// *****************************************************************************
bool PrivMgrRoles::areRemainingGrantedPrivsSufficient(const std::string whereClause, PrivType privType)

{
  ...
  PrivStatus privStatus = objectPrivileges.getPrivBitmaps(whereClause, orderByClause, privBitmaps);
  ...
  for (size_t pb = 0; pb < privBitmaps.size(); pb++)
    if (privBitmaps[pb].test(privType)) return true;
...
// *                                                                           *
// *****************************************************************************
bool PrivMgrRoles::dependentObjectsExist(const int32_t userID, const int32_t roleID, PrivDropBehavior dropBehavior)

{
  ...
  // TBD:  return the object UIDs to caller and have caller report issue
  // in error message
  std::vector<int64_t> objectUIDs;
  bool hasComponentPriv = false;
  if (!isAuthIDGrantedPrivs(roleID, privClass, objectUIDs, hasComponentPriv)) return false;
  ...
  if (privStatus == STATUS_NOTFOUND || ownedUIDandTypes.size() == 0) return false;
  ...
  // If roleID not found in metadata, ComDiags is populated with
  // error 8732: Authorization ID <roleID> is not a registered user or role
  Int16 retCode = ComUser::getAuthNameFromAuthID(roleID, roleName, sizeof(roleName), length, FALSE, pDiags_);
  ...
  for (size_t u3 = 0; u3 < ownedUIDandTypes.size(); u3++) {
    ...
    switch (ownedUIDandTypes[u3].objectType) {
      case COM_VIEW_OBJECT: {
        ...
        if (privStatus == STATUS_ERROR || privStatus == STATUS_NOTFOUND || referencedObjectsList.size() == 0) {
          PRIVMGR_INTERNAL_ERROR("Could not fetch objects referenced by view");
          return true;
        }

        for (size_t obj = 0; obj < referencedObjectsList.size(); obj++) {
          whereClause = whereClauseHeader + UIDToString(referencedObjectsList[obj]->objectUID);
          ...
          if (areRemainingGrantedPrivsSufficient(whereClause, privType)) continue;
          ...
          privStatus = objects.fetchQualifiedName(referencedObjectsList[obj]->objectUID, referencedObjectName);
      ...
      case COM_STORED_PROCEDURE_OBJECT: {
        ...
        // for this UDR/SPJ.
        if (areRemainingGrantedPrivsSufficient(whereClauseHeader + selectSubClause, USAGE_PRIV)) continue;

        // TODO: for cascade, drop the object instead of reporting dependency error
        selectSubClause.insert(0, " WHERE OBJECT_UID = ");
        privStatus = objects.fetchQualifiedName(selectSubClause, referencedObjectName);
      ...
      case COM_REFERENTIAL_CONSTRAINT_OBJECT: {
        ...
        if (areRemainingGrantedPrivsSufficient(whereClauseHeader + selectSubClause, REFERENCES_PRIV)) continue;
    ...
    if (dependencyFound) {
      ...
      privStatus = objects.fetchQualifiedName(ownedUIDandTypes[u3].UID, dependentObjectName);
      ...
      *pDiags_ << DgSqlCode(-CAT_DEPENDENT_ROLE_PRIVILEGES_EXIST) << DgString0(roleName)
               << DgString1(dependentObjectName.c_str()) << DgString2(referencedObjectName.c_str());
      return true;
    }
  }
...
// *                                                                           *
// *  <dontFetchGroupRoles>         bool                           In          *
// *    I don't want to fetch roles which are inherit from user group.         *
// *                                                                           *
// *****************************************************************************
// *                                                                           *
// * Returns: PrivStatus                                                       *
...
// *                                                                           *
// *****************************************************************************
PrivStatus PrivMgrRoles::fetchRolesForAuth(const int32_t authID, std::vector<std::string> &roleNames,
                                           std::vector<int32_t> &roleIDs, std::vector<int32_t> &grantDepths,
...

{
  ...
  bool skipGroupCheck;
  NOT_NEED_GRANT_TO_GROUPS(skipGroupCheck);

  // add the groups for the user to authIDs
  if (fetchGroupRoles && !skipGroupCheck && isUserID(authID)) {
    CmpSeabaseDDLuser userInfo;
    if (userInfo.getUserDetails(authID) != CmpSeabaseDDLauth::STATUS_GOOD) {
      if (pDiags_->getNumber(DgSqlCode::ERROR_) == 0)
        PRIVMGR_INTERNAL_ERROR("I/O error fetching roles, unable to retrieve user details.");
      return STATUS_ERROR;
    }
    if (userInfo.getUserGroups(authIDs) == CmpSeabaseDDLauth::STATUS_ERROR) return STATUS_ERROR;
  }

  std::string whereClause(" WHERE GRANTEE_ID in (");
  for (size_t a = 0; a < authIDs.size(); a++) {
    if (a > 0) whereClause += ", ";
    whereClause += authIDToString(authIDs[a]);
  }
  whereClause += ")";
  ...
  for (size_t r = 0; r < rows.size(); r++) {
    ...
    if (hasValue(roleIDs, row.roleID_) && hasValue(grantees, row.granteeID_)) {
      if (grantDepths.back() == 0) grantDepths.back() = row.grantDepth_;
      continue;
    }
...
// *****************************************************************************
// Function: PrivMgrRoles::fetchSystemGrantee
//
//  Returns the grantee name and ID for a role
//   <roleID> is the role ID.
//   <granteeName> returns the grantee name
//   <granteeID> returns the grantee ID
//
// Grantee names can only be users and roles at this time.
//
// Returns: PrivStatus
//   STATUS_GOOD    : Grantee information found
//   STATUS_NOTFOUND: Grantee information not found
//                 *: Grantee information not returned due to SQL error.
//                    A CLI error is put into the diags area.
// *****************************************************************************
PrivStatus PrivMgrRoles::fetchSystemGrantee(const int32_t roleID, std::string &granteeName, int32_t &granteeID) {
  char buf[300];
  snprintf(buf, sizeof(buf),
           "where role_id = %d and grantor_id = -2 and "
           "((grantee_id between %d and %d) or (grantee_id between %d and %d))",
           roleID, MIN_USERID, MAX_USERID, MIN_ROLEID, MAX_ROLEID);
  ...
  // Only one row should be returned,  just return the first entry
  if (rows.size() > 0) {
    MyRow &row = rows[0];
    granteeName = row.granteeName_;
...
// *****************************************************************************
// *                                                                           *
// * Function: PrivMgrRoles::fetchGranteesForRole                              *
// *                                                                           *
...
// *    passes back a list of granteeIDs corresponding to granteeNames.        *
// *                                                                           *
...
// *                                                                           *
// *  <includeTenantGrantees>         bool                            In       *
// *    include any grantees that are tenants in addition to other authIDs     *
// *                                                                           *
// *****************************************************************************
// *                                                                           *
// * Returns: PrivStatus                                                       *
...
PrivStatus PrivMgrRoles::fetchGranteesForRole(const int32_t roleID, std::vector<std::string> &granteeNames,
                                              std::vector<int32_t> &granteeIDs, std::vector<int32_t> &grantorIDs,
                                              std::vector<int32_t> &grantDepths, bool includeTenantGrantees) {
  ...
  for (size_t r = 0; r < rows.size(); r++) {
    ...
    if (includeTenantGrantees || (!includeTenantGrantees && !isTenantID(row.granteeID_))) {
...
// *                                                                           *
// *****************************************************************************
PrivStatus PrivMgrRoles::fetchGranteesForRoles(const std::vector<int32_t> &roleIDs,
                                               std::vector<std::int32_t> &granteeIDs, bool includeSysGrantor,
                                               bool includeTenantGrantees) {
  ...
  if (!includeSysGrantor) whereClause += " AND GRANTOR_ID <> -2 ";
  ...
  for (size_t r = 0; r < rows.size(); r++) {
    ...
    if (includeTenantGrantees && isTenantID(row.granteeID_) || CmpSeabaseDDLauth::isUserID(row.granteeID_) ||
        CmpSeabaseDDLauth::isUserGroupID(row.granteeID_) || CmpSeabaseDDLauth::isRoleID(row.granteeID_))
      granteeIDs.push_back(row.granteeID_);
  }
...
// -----------------------------------------------------------------------------
// method: fetchTenantGrantees
//
// Returns the list of tenantIDs that have been granted to roles.
// In effect is returns all the detached tenants
// -----------------------------------------------------------------------------
PrivStatus PrivMgrRoles::fetchTenantGrantees(std::vector<int32_t> &tenantGrantees) {
  std::string whereClause("where grantee_id >= ");
  whereClause += authIDToString(ROOT_TENANTID);
  whereClause += " and grantee_id < ";
  whereClause += authIDToString(MAX_TENANTID);

  MyTable &myTable = static_cast<MyTable &>(myTable_);

  if (myTable.selectAuthGrantees(whereClause, tenantGrantees) == STATUS_ERROR) return STATUS_ERROR;
  return STATUS_GOOD;
}
...
// *                                                                           *
// *****************************************************************************
PrivStatus PrivMgrRoles::grantRole(const std::vector<int32_t> &roleIDs, const std::vector<std::string> &roleNames,
...

{
  ...
  bool noRoleGrantToGroups;
  NOT_NEED_GRANT_TO_GROUPS(noRoleGrantToGroups);
  std::map<int32_t, std::set<int32_t> *> userRolesMap;
  ...
  for (size_t r = 0; r < roleIDs.size(); r++) {
    ...
    if (roleID == PUBLIC_USER) {
      *pDiags_ << DgSqlCode(-CAT_IS_NOT_A_ROLE) << DgString0(PUBLIC_AUTH_NAME);
      FREE_USER_ROLES_CACHE(userRolesMap);
      return STATUS_ERROR;
    }
    ...
    for (size_t g = 0; g < granteeIDs.size(); g++) {
      ...
      if (granteeIDs[g] == grantorIDs[r] || granteeIDs[g] == ComUser::getRootUserID()) {
      ...
      if (grantExists(roleID, grantorIDs[r], granteeIDs[g], thisGrantDepth)) {
        ...
        if (thisGrantDepth == grantDepth || grantDepth == 0) continue;
      ...
      // if ALLOW_ROLE_GRANTS_TO_GROUPS is ON and this user has inherited roles from user group,
      // there is not need to invalid the query cache
      bool doInvalidCache = true;
      if (CmpSeabaseDDLauth::isUserID(granteeIDs[g]) && NOT noRoleGrantToGroups) {
        doInvalidCache = isNeedToInvalidCache(userRolesMap, granteeIDs[g], roleID);
      }
      ...
      if (privStatus != STATUS_GOOD) {
        if (pDiags_->getNumber(DgSqlCode::ERROR_) == 0) PRIVMGR_INTERNAL_ERROR("I/O error granting role");
        FREE_USER_ROLES_CACHE(userRolesMap);
        return STATUS_ERROR;
      }

      if (NOT doInvalidCache) {
        continue;
      }
...
PrivStatus PrivMgrRoles::grantRoleToCreator(const int32_t roleID, const std::string &roleName, const int32_t grantorID,
...

{
  ...
  // if row not inserted, insert row
  if (!grantExists(roleID, grantorID, granteeID, row.grantDepth_)) return myTable.insert(row);
  return STATUS_GOOD;
}
...
// *                                                                           *
// *****************************************************************************
bool PrivMgrRoles::hasRole(int32_t authID, int32_t roleID)
...
// *****************************************************************************
// *                                                                           *
// * Function: PrivMgrRoles::hasRoleAdmin                                      *
// *                                                                           *
// *    Determines if an authID has been granted a role WITH ADMIN option.     *
// *                                                                           *
// *****************************************************************************
// *                                                                           *
// *  Parameters:                                                              *
...
// *                                                                           *
// *  <roleIDs>                       const std::vector<int32_t>      In       *
// *     list of roles assigned to the granteeID                               *
// *                                                                           *
// *  <hasAdmin>                      bool                            Out      *
// *     returns true if user has admin option, false otherwise                *
// *                                                                           *
// *****************************************************************************
// *                                                                           *
// * Returns: PrivStatus                                                       *
// *                                                                           *
// * diags area contains error if authID does not have with admin option       *
// *****************************************************************************
PrivStatus PrivMgrRoles::hasRoleAdmin(const int32_t roleID, const int32_t granteeID, const std::string &roleName,
                                      const std::vector<int32_t> &roleIDs, bool &hasAdmin) {
  hasAdmin = false;

  // Check to see if user granted MANAGE_PRIVILEGES, these users have admin privileges
  NAList<SQLOperation> compPrivList(STMTHEAP);
  compPrivList.insert(SQLOperation::MANAGE_PRIVILEGES);
  PrivMgrComponentPrivileges componentPrivileges;
  if (componentPrivileges.hasSQLPriv(granteeID, compPrivList, hasAdmin) < 0) return STATUS_ERROR;

  if (hasAdmin) return STATUS_GOOD;

  // Read metadata to see if granteeID has been granted role with admin option.
  // GRANT_DEPTH < 0 indicates granteeID has with admin option.
  MyTable &myTable = static_cast<MyTable &>(myTable_);
  ...
  if (myTable.selectCountWhere(whereClause, rowCount) == STATUS_ERROR) return STATUS_ERROR;

  if (rowCount > 0) hasAdmin = true;

  return STATUS_GOOD;
}
//******************* End of PrivMgrRoles::hasRoleAdmin ************************
...
// *                                                                           *
// *****************************************************************************
bool PrivMgrRoles::hasWGO(int32_t authID, int32_t roleID) {
  MyTable &myTable = static_cast<MyTable &>(myTable_);
...
// *                                                                           *
// *****************************************************************************
bool PrivMgrRoles::grantExists(const int32_t roleID, const int32_t grantorID, const int32_t granteeID,
                               int32_t &grantDepth)
...
// *                                                                           *
// *  <granteeNames>                  std::string reference           Out      *
// *    returns the list of users that have been granted the role.             *
// *                                                                           *
// *****************************************************************************
// *                                                                           *
// * Returns: bool                                                             *
...
// *                                                                           *
// *****************************************************************************
bool PrivMgrRoles::isGranted(const int32_t roleID, const bool shouldExcludeGrantsBySystem, std::string &granteeNames) {
  MyRow row(fullTableName_);
  MyTable &myTable = static_cast<MyTable &>(myTable_);
  ...
  std::string orderByClause = " ORDER BY 1 ";
  ...
  if ((privStatus == STATUS_GOOD || privStatus == STATUS_WARNING) && rows.size() > 0) {
    for (size_t r = 0; r < rows.size(); r++) {
      if (r > 0) granteeNames += ", ";
      MyRow &row = rows[r];
      granteeNames += row.granteeName_;
    }
    return true;
  }
...

{
  ...
  int32_t numberRoles = sizeof(systemRoles) / sizeof(SystemAuthsStruct) - NUMBER_SPECIAL_SYSTEM_ROLES;
  ...
  char buf[500];
  if (ComUser::getSystemRoleList(buf, actualSize, 500)) {
  ...
  int32_t cliRC = cliInterface.executeImmediate(selectStmt.c_str(), (char *)&expectedRows, &length, FALSE);
...
// *                                                                           *
// *****************************************************************************
PrivStatus PrivMgrRoles::revokeAllForGrantor(const int32_t grantorID, const int32_t roleID, const bool isGOFSpecified,
...

{
  ...
  if (privStatus == STATUS_NOTFOUND) return STATUS_GOOD;
  ...
  // *****************************************************************************
  // *                                                                           *
  // *   Go ahead and drop any users granted the role as long as the user did    *
  // *   not create any objects based on this grant.  If users created objects   *
  // *   then these dependent objects need to be dropped.  When CASCADE is added *
  // *   then this code will remove these dependent objects                      *
  // *                                                                           *
  // *****************************************************************************
  ...
  // * call revokeRole (our likely caller). Revoke role returns an error if user *
  // * created a dependent object.                                               *
  // *                                                                           *
  // *****************************************************************************
  ...
  privStatus = revokeRole(roleIDs, granteeIDs, granteeClasses, grantorIDs, isGOFSpecified, newGrantDepth, dropBehavior);

  if (STATUS_GOOD == privStatus) {
    bool noRoleGrantToGroups;
    NOT_NEED_GRANT_TO_GROUPS(noRoleGrantToGroups);
    if (NOT noRoleGrantToGroups) {
      CmpSeabaseDDLauth authEntry;
      std::set<int32_t> members;  // is a set

      // Invalidated cache one by one
      for (vector<int32_t>::iterator itor = granteeIDs.begin(); itor != granteeIDs.end(); itor++) {
        int32_t authID = *itor;
        // is a group?
        if (CmpSeabaseDDLauth::isUserGroupID(authID)) {
          if (COM_LDAP_AUTH == CmpCommon::getAuthenticationType()) {
            // need LDAP username
            authEntry.getAuthDetails(authID);
          } else {
            authEntry.setAuthID(authID);
          }
          std::vector<int32_t> members_;  // is a vector
          if (CmpSeabaseDDLauth::STATUS_GOOD == authEntry.getUserGroupMembers(members_)) {
            members.insert(members_.begin(), members_.end());
          }
        }
      }
      if (NOT members.empty()) {
        std::map<int32_t, std::set<int32_t> *> userRolesCache;
        std::vector<int32_t> members_;
        for (set<int32_t>::iterator itor = members.begin(); itor != members.end(); itor++) {
          if (isNeedToInvalidCache(userRolesCache, *itor, roleID)) {
            members_.push_back(*itor);
          }
        }
        // roleIDs.push_back(roleID);
        CmpCommon::letsQuerycacheToInvalidated(members_, roleIDs);
        FREE_USER_ROLES_CACHE(userRolesCache);
      }
    }
  }
  return privStatus;
...
// *                                                                           *
// *****************************************************************************
PrivStatus PrivMgrRoles::revokeRole(const std::vector<int32_t> &roleIDs, const std::vector<int32_t> &granteeIDs,
...

{
  ...
  for (size_t r = 0; r < roleIDs.size(); r++) {
    ...
    for (size_t g = 0; g < granteeIDs.size(); g++) {
      ...
      if (!grantExists(roleID, grantorIDs[r], granteeIDs[g], grantDepth)) {
        ...
        // If roleID not found in metadata, just use value returned
        Int16 retCode = ComUser::getAuthNameFromAuthID(roleID, roleName, sizeof(roleName), length, TRUE, NULL);

        char grantorName[MAX_DBUSERNAME_LEN + 1];

        // If grantorID not found in metadata, just use value returned
        retCode = ComUser::getAuthNameFromAuthID(grantorIDs[r], grantorName, sizeof(grantorName), length, TRUE, NULL);
        char granteeName[MAX_DBUSERNAME_LEN + 1];

        // If granteeID not found in metadata, just use value returned
        retCode = ComUser::getAuthNameFromAuthID(granteeIDs[g], granteeName, sizeof(granteeName), length, TRUE, NULL);

        // return error
        *pDiags_ << DgSqlCode(-CAT_GRANT_NOT_FOUND) << DgString0(roleName) << DgString1(grantorName)
                 << DgString2(granteeName);
        continue;
      }

      if (hasWGO(granteeIDs[g], roleID)) {
        privStatus = revokeAllForGrantor(granteeIDs[g], roleID, isGOFSpecified, newGrantDepth, dropBehavior);
        if (privStatus != STATUS_GOOD) return STATUS_ERROR;
      }

      if (dependentObjectsExist(granteeIDs[g], roleID, dropBehavior)) return STATUS_ERROR;
    }
  }
  ...
  for (size_t r2 = 0; r2 < roleIDs.size(); r2++) {
    ...
    for (size_t g2 = 0; g2 < granteeIDs.size(); g2++) {
      ...
      if (privStatus != STATUS_GOOD) {
        if (pDiags_->getNumber(DgSqlCode::ERROR_) == 0) PRIVMGR_INTERNAL_ERROR("I/O error revoking role");
        FREE_USER_ROLES_CACHE(userRolesMap);
        return STATUS_ERROR;
      }
      ...
      // if ALLOW_ROLE_GRANTS_TO_GROUPS is ON and this user has inherited roles from user group,
      // there is not need to invalid the query cache
      if (CmpSeabaseDDLauth::isUserID(granteeIDs[g2]) && NOT noRoleGrantToGroups) {
        if (NOT isNeedToInvalidCache(userRolesMap, granteeIDs[g2], roleIDs[r2])) {
          continue;
        }
      }

      ComSecurityKey::QIType keyType =
          (isUserID(granteeIDs[g2])) ? ComSecurityKey::SUBJECT_IS_USER : ComSecurityKey::SUBJECT_IS_GROUP;
      ComSecurityKey secKey(granteeIDs[g2], roleIDs[r2], keyType);
...
// *                                                                           *
// *****************************************************************************
PrivStatus PrivMgrRoles::revokeRoleFromCreator(const int32_t roleID, const int32_t granteeID,
...
PrivStatus MyTable::insert(const PrivMgrMDRow &rowIn) {
  ...
  sprintf(insertStatement, "insert into %s values (%d, '%s', %d, '%s', '%s', %d, '%s', '%s', %d)", tableName_.c_str(),
          row.roleID_, row.roleName_.c_str(), row.granteeID_, row.granteeName_.c_str(), granteeAuthClass,
          row.grantorID_, row.grantorName_.c_str(), grantorAuthClass, row.grantDepth_);
...
// *                                                                           *
// *****************************************************************************
PrivStatus MyTable::insertSelect(const std::string &authsLocation, const std::string &inClause)

{
  char insertStatement[2000];

  sprintf(insertStatement,
          "INSERT INTO %s SELECT A1.AUTH_ID, A1.AUTH_DB_NAME, A1.AUTH_CREATOR,"
          ...
          tableName_.c_str(), authsLocation.c_str(), authsLocation.c_str(), 'U', authsLocation.c_str(),
          inClause.c_str());
...

{
  std::string selectStmt(
      "SELECT ROLE_ID, ROLE_NAME, GRANTEE_ID, GRANTEE_NAME, "
  ...
  if (tableQueue == NULL) return STATUS_ERROR;
...
// -----------------------------------------------------------------------------
// method: selectAuthGrantees
//
// returns grantee_id's from role usages that match the passed in where clause
// -----------------------------------------------------------------------------
PrivStatus MyTable::selectAuthGrantees(std::string whereClause, std::vector<int32_t> &granteeIDs) {
  std::string selectStmt("SELECT distinct GRANTEE_ID FROM ");
  selectStmt += tableName_;
  selectStmt += " ";
  selectStmt += whereClause;
  selectStmt += " order by 1 ";
  ...
  for (int r = 0; r < tableQueue->numEntries(); r++) {
    OutputInfo *pCliRow = (OutputInfo *)tableQueue->getNext();
    pCliRow->get(0, ptr, length);
    granteeIDs.push_back(*(reinterpret_cast<int32_t *>(ptr)));
  }
  return STATUS_GOOD;
}
...

{
  ...
  if (tableQueue->numEntries() != 1) return STATUS_ERROR;
...
static bool isNeedToInvalidCache(std::map<int32_t, std::set<int32_t> *> &userRolesCache, int32_t granteeID,
                                 int32_t roleID) {
  // fetch cache
  if (0 == userRolesCache.count(granteeID)) {
    // get all of role from user which are inherited roles from user group
    std::vector<int32_t> groups;
    CmpSeabaseDDLuser userInfo;
    // has group?
    if (COM_LDAP_AUTH == CmpCommon::getAuthenticationType()) {
      // need LDAP username
      userInfo.getAuthDetails(granteeID);
    } else {
      userInfo.setAuthID(granteeID);
    }
    std::set<int32_t> *gRoleIDs = NULL;  // is a set
    if (CmpSeabaseDDLauth::STATUS_NOTFOUND != userInfo.getAllJoinedUserGroup(groups) && NOT groups.empty()) {
      gRoleIDs = new std::set<int32_t>();
      std::vector<int32_t> gRoleIDs_;    // is a vector
      std::vector<int32_t> granteeIDs_;  // i don't care
      for (std::vector<int32_t>::iterator itor = groups.begin(); itor != groups.end(); itor++) {
        gRoleIDs_.clear();
        granteeIDs_.clear();
        // need roles from group inheritance
        userInfo.getRoleIDs(granteeID, gRoleIDs_, granteeIDs_);
        gRoleIDs->insert(gRoleIDs_.begin(), gRoleIDs_.end());
      }
    }
    userRolesCache.insert(std::pair<int32_t, std::set<int32_t> *>(granteeID, gRoleIDs));
  }
  std::set<int32_t> *roles = userRolesCache[granteeID];
  return (NULL == roles) || (0 == roles->count(roleID));
}
