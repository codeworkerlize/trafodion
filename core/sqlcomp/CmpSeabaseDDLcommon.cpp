...
static void createSeabaseComponentOperation(const std::string &systemCatalog,
                                            StmtDDLCreateComponentPrivilege *pParseNode);

static void dropSeabaseComponentOperation(const std::string &systemCatalog, StmtDDLDropComponentPrivilege *pParseNode);

static void grantSeabaseComponentPrivilege(const std::string &systemCatalog,
                                           StmtDDLGrantComponentPrivilege *pParseNode);

static void revokeSeabaseComponentPrivilege(const std::string &systemCatalog,
                                            StmtDDLRevokeComponentPrivilege *pParseNode);

static void grantRevokeSeabaseRole(const std::string &systemCatalog, StmtDDLRoleGrant *pParseNode);
...
void CmpSeabaseDDL::getPartitionTableName(char *buf, int bufLen, const NAString &baseTableName,
                                          const NAString &partitionName) {
  if (buf != NULL && bufLen > 0) {
    snprintf(buf, bufLen, "PART_%s_%s_", baseTableName.data(), partitionName.data());
  }
}

void CmpSeabaseDDL::getPartitionIndexName(char *buf, int bufLen, const NAString &baseIndexName,
...
const NAString CmpSeabaseDDL::systemSchemas[16] = {
    NAString("_CELL_"),       NAString("_ROW_"),        NAString("HIVE"),          NAString("SEABASE"),
    NAString("_HBASESTATS_"), NAString("_HB_MAP_"),     NAString("_HV_HIVE_"),     NAString("_LIBMGR_"),
    NAString("_MD_"),         NAString("_PRIVMGR_MD_"), NAString("_REPOS_"),       NAString("_DTM_"),
    NAString("_TENANT_MD_"),  NAString("_MAC_MD_"),     NAString("_RECYCLE_BIN_"), NAString("_XDC_MD_")};
...
// ----------------------------------------------------------------------------
NABoolean CmpSeabaseDDL::getMDtableInfo(const ComObjectName &name, ComTdbVirtTableTableInfo *&tableInfo,
  ...
  if (objName.isNull()) return FALSE;

  // If metadata tables have not yet been added to the compiler context,
  // return FALSE
  if (!CmpCommon::context()->getTrafMDDescsInfo()) return FALSE;

  // The first set of objects are system and tenant metadata.  Check the passed in
  // objName and objType for a match
  if (isSeabaseMD(name.getCatalogNamePartAsAnsiString(), name.getSchemaNamePartAsAnsiString(TRUE),
    ...
    for (int i = 0; i < end; i++) {
      ...
      if (objName == mdti->newName) {
        ...
        if (tableInfo->objUID == 0) {
          ...
          if (cmpSBD.getSpecialTableInfo(CTXTHEAP, catName, schName, objName, extTableName, objType, tableInfo))
            return FALSE;  // error
        }

        if ((tableInfo && tableInfo->tableNamespace == NULL) && (CmpCommon::context()->useReservedNamespace())) {
          tableInfo->tableNamespace = new (CTXTHEAP) char[strlen(TRAF_RESERVED_NAMESPACE1) + 1];
          strcpy((char *)tableInfo->tableNamespace, TRAF_RESERVED_NAMESPACE1);
        }

        if (tableInfo && tableInfo->tableNamespace && getenv("INIT_TRAF_DEF_NS")) {
      ...
      } else  // if (mdti.oldName && (objName == mdti.oldName))
      {
        ...
        int sizeOfoldDDL = 0;
        if (getOldMDInfo(*mdti, oldName, oldDDL, sizeOfoldDDL) == FALSE) return FALSE;
        ...
        if ((oldName) && (objName == oldName)) {
          ...
          if (CmpCommon::context()->useReservedNamespace()) nameSpace = TRAF_RESERVED_NAMESPACE1;
  ...
  if (isSeabasePrivMgrMD(name.getCatalogNamePartAsAnsiString(), name.getSchemaNamePartAsAnsiString(TRUE))) {
    ...
    for (size_t i = 0; i < sizeof(privMgrTables) / sizeof(PrivMgrTableStruct); i++) {
      ...
      else  // See if this is an old name (upgrade)
      {
        ...
        const QString *oldDDL = NULL;
        int sizeOfoldDDL = 0;
        if (getOldMDPrivInfo(mdti, oldName, oldDDL, sizeOfoldDDL) == FALSE) return FALSE;
...
short CmpSeabaseDDL::convertColAndKeyInfoArrays(int btNumCols,                              // IN
  ...
  for (int i = 0; i < btNumCols; i++) {
    ComTdbVirtTableColumnInfo &ci = btColInfoArray[i];

    int colnum, offset;
    TrafDesc *desc =
        TrafMakeColumnDesc(NULL, NULL, colnum, ci.datatype, ci.length, offset, ci.nullable, ci.charset, NULL);
...
static short resetCQDs(NABoolean hbaseSerialization, NAString hbVal, short retval) {
...
short CmpSeabaseDDL::processDDLandCreateDescs(Parser &parser, const QString *ddl, int sizeOfddl,

                                              const NAString &defCatName, const NAString &defSchName,
                                              ...
                                              ComTdbVirtTableIndexInfo *&indexInfo)  // OUT
{
  ...
  exprNode = parser.parseDML((const char *)gluedQuery, strlen(gluedQuery), CharInfo::ISO88591);

  NADELETEBASICARRAY(gluedQuery, STMTHEAP);

  if (!exprNode) return resetCQDs(hbaseSerialization, hbVal, -1);
  ...
  if (!rRoot) return resetCQDs(hbaseSerialization, hbVal, -1);
  ...
  if (!ddlNode) return resetCQDs(hbaseSerialization, hbVal, -1);

  int keyLength = 0;
  NAString extTableName;
  NABoolean alignedFormat = FALSE;
  if (ddlNode->getOperatorType() == DDL_CREATE_TABLE) {
    StmtDDLCreateTable *createTableNode = ddlNode->castToStmtDDLNode()->castToStmtDDLCreateTable();
    ...
    tableName.applyDefaults(ComAnsiNamePart(defCatName, ComAnsiNamePart::INTERNAL_FORMAT),
    ...
    if (buildColInfoArray(COM_BASE_TABLE_OBJECT,
                          ...
                          &colArray, colInfoArray, FALSE, FALSE, NULL, NULL, NULL, NULL, CTXTHEAP)) {
      return resetCQDs(hbaseSerialization, hbVal, -1);
    }

    if (buildKeyInfoArray(&colArray, NULL, &keyArray, colInfoArray, keyInfoArray, FALSE, &keyLength, CTXTHEAP)) {
      return resetCQDs(hbaseSerialization, hbVal, -1);
    }
    ...
    StmtDDLCreateIndex *createIndexNode = ddlNode->castToStmtDDLNode()->castToStmtDDLCreateIndex();

    ComObjectName tableName(createIndexNode->getTableName());
    tableName.applyDefaults(ComAnsiNamePart(NAString(TRAFODION_SYSCAT_LIT)),
                            ComAnsiNamePart(NAString(SEABASE_MD_SCHEMA), ComAnsiNamePart::INTERNAL_FORMAT));
    extTableName = tableName.getExternalName(TRUE);
    ...
    if (convertColAndKeyInfoArrays(btNumCols, btColInfoArray, btNumKeys, btKeyInfoArray, &btNAColArray, &btNAKeyArr))
      return resetCQDs(hbaseSerialization, hbVal, -1);
    ...
    if (createIndexColAndKeyInfoArrays(
            indexColRefArray, createIndexNode->getAddnlColRefArray(), createIndexNode->isUniqueSpecified(),
            createIndexNode->isNgramSpecified(),  // ngram index
            FALSE,                                // no syskey
            FALSE,                                // not alignedFormat
            defaultColFam, btNAColArray, btNAKeyArr, numIndexKeys, numIndexNonKeys, numIndexCols, indexColInfoArray,
            indexKeyInfoArray, selColList, keyLength, CTXTHEAP))
      return resetCQDs(hbaseSerialization, hbVal, -1);

    numIndexNonKeys = numIndexCols - numIndexKeys;

    if (numIndexNonKeys > 0) {
      indexNonKeyInfoArray = new (CTXTHEAP) ComTdbVirtTableKeyInfo[numIndexNonKeys];
    }

    int ink = 0;
    for (int i = numIndexKeys; i < numIndexCols; i++) {
  ...
  tableInfo->tableNamespace = NULL;
  if ((CmpCommon::context()->useReservedNamespace()) && (NOT nameSpace.isNull())) {
    tableInfo->tableNamespace = new (CTXTHEAP) char[nameSpace.length() + 1];
...
short CmpSeabaseDDL::createMDdescs(MDDescsInfo *&trafMDDescsInfo) {
  ...
  trafMDDescsInfo =
      (MDDescsInfo *)new (CTXTHEAP) char[(numMDTables + numPrivTables + numTenantTables) * sizeof(MDDescsInfo)];
  ...
  for (size_t i = 0; i < numMDTables; i++) {
    ...
    const QString *oldDDL = NULL;
    int sizeOfoldDDL = 0;
    if (getOldMDInfo(mdti, oldName, oldDDL, sizeOfoldDDL) == FALSE) goto label_error;

    MDDescsInfo &mddi = trafMDDescsInfo[i];

    if (!mdti.newDDL) continue;
    ...
    breadCrumb = 1;
    if (processDDLandCreateDescs(parser, mdti.newDDL, mdti.sizeOfnewDDL, NAString(TRAFODION_SYSCAT_LIT),
                                 NAString(SEABASE_MD_SCHEMA), NAString(TRAF_RESERVED_NAMESPACE1),
                                 (mdti.isIndex ? TRUE : FALSE), 0, NULL, 0, NULL, numCols, colInfoArray, numKeys,
                                 keyInfoArray, tableInfo, indexInfo))
      goto label_error;
    ...
    if (oldDDL) {
      ...
      numKeys = 0;
      keyInfoArray = NULL;
      if (processDDLandCreateDescs(parser, oldDDL, sizeOfoldDDL, NAString(TRAFODION_SYSCAT_LIT),
                                   NAString(SEABASE_MD_SCHEMA), NAString(TRAF_RESERVED_NAMESPACE1),
                                   (mdti.isIndex ? TRUE : FALSE), 0, NULL, 0, NULL, numCols, colInfoArray, numKeys,
                                   keyInfoArray, tableInfo, indexInfo))
        goto label_error;
    ...
    if (mdti.indexDDL) {
      ...
      breadCrumb = 3;
      if (processDDLandCreateDescs(parser, mdti.indexDDL, mdti.sizeOfIndexDDL, NAString(TRAFODION_SYSCAT_LIT),
                                   NAString(SEABASE_MD_SCHEMA), NAString(TRAF_RESERVED_NAMESPACE1), FALSE,
                                   mddi.numNewCols, mddi.newColInfo, mddi.numNewKeys, mddi.newKeyInfo, numIndexCols,
                                   indexColInfoArray, numIndexKeys, indexKeyInfoArray, tableInfo, indexInfo))
        goto label_error;
  ...
  for (size_t i = 0; i < numPrivTables; i++) {
    ...
    breadCrumb = 4;
    if (processDDLandCreateDescs(parser, &ddlString, sizeof(QString), NAString(TRAFODION_SYSCAT_LIT),
                                 NAString(SEABASE_PRIVMGR_SCHEMA), NAString(TRAF_RESERVED_NAMESPACE1), FALSE, 0, NULL,
                                 0, NULL, numCols, colInfoArray, numKeys, keyInfoArray, tableInfo, indexInfo))
      goto label_error;
  ...
  for (size_t i = 0; i < numTenantTables; i++) {
    ...
    breadCrumb = 6;
    if (processDDLandCreateDescs(parser, mdti.newDDL, mdti.sizeOfnewDDL, NAString(TRAFODION_SYSCAT_LIT),
                                 NAString(SEABASE_TENANT_SCHEMA), NAString(TRAF_RESERVED_NAMESPACE1),
                                 (mdti.isIndex ? TRUE : FALSE), 0, NULL, 0, NULL, numCols, colInfoArray, numKeys,
                                 keyInfoArray, tableInfo, indexInfo))
      goto label_error;
...
label_error:
  ...
  if (trafMDDescsInfo) NADELETEBASIC(trafMDDescsInfo, CTXTHEAP);
...
NABoolean CmpSeabaseDDL::isHbase(const NAString &catName) {
  if (NOT catName.isNull()) {
    ...
    if ((catName == HBASE_SYSTEM_CATALOG) || (catName == hbaseDefCatName)) return TRUE;
  }

  return FALSE;
}

ComBoolean CmpSeabaseDDL::isHbase(const ComObjectName &name) { return isHbase(name.getCatalogNamePartAsAnsiString()); }

bool CmpSeabaseDDL::isHistogramTable(const NAString &name) {
  if (name == HBASE_HIST_NAME || name == HBASE_HISTINT_NAME || name == HBASE_PERS_SAMP_NAME) return true;

  return false;
}

bool CmpSeabaseDDL::isSampleTable(const NAString &name) {
  if (name(0, min((sizeof(TRAF_SAMPLE_PREFIX) - 1), name.length())) == TRAF_SAMPLE_PREFIX) return true;
...
NABoolean CmpSeabaseDDL::isSeabase(const NAString &catName) {
  if (NOT catName.isNull()) {
    ...
    if (catName == seabaseDefCatName) return TRUE;
  }
...
NABoolean CmpSeabaseDDL::isSeabaseMD(const NAString &catName, const NAString &schName, const NAString &objName) {
  if (NOT catName.isNull()) {
    NAString seabaseDefCatName = "";
    CmpCommon::getDefault(SEABASE_CATALOG, seabaseDefCatName, FALSE);
    seabaseDefCatName.toUpper();

    if ((catName == seabaseDefCatName) && (schName == SEABASE_MD_SCHEMA || schName == SEABASE_TENANT_SCHEMA ||
...
NABoolean CmpSeabaseDDL::isSeabasePrivMgrMD(const NAString &catName, const NAString &schName) {
  if (NOT catName.isNull()) {
    NAString seabaseDefCatName = "";
    CmpCommon::getDefault(SEABASE_CATALOG, seabaseDefCatName, FALSE);
    seabaseDefCatName.toUpper();

    if ((catName == seabaseDefCatName) && (schName == SEABASE_PRIVMGR_SCHEMA)) {
      return TRUE;
    }
  }
...
ComBoolean CmpSeabaseDDL::isREPOSSchema(const ComObjectName &name) {
  return isREPOSSchema(name.getCatalogNamePartAsAnsiString(), name.getSchemaNamePartAsAnsiString(TRUE));
}
...
ComBoolean CmpSeabaseDDL::isXDCSchema(const ComObjectName &name) {
  return isXDCSchema(name.getCatalogNamePartAsAnsiString(), name.getSchemaNamePartAsAnsiString(TRUE));
}

NABoolean CmpSeabaseDDL::isSeabaseReservedSchema(const NAString &catName, const NAString &schName) {
  if (catName.isNull()) return FALSE;
...
NABoolean CmpSeabaseDDL::isSeabaseReservedSchema(const ComObjectName &name) {
...
NABoolean CmpSeabaseDDL::isSeabaseExternalSchema(const NAString &catName, const NAString &schName) {
  if (catName.isNull()) return FALSE;
  ...
  if (catName != seabaseDefCatName) return FALSE;

  return ComIsTrafodionExternalSchemaName(schName);
}

NABoolean CmpSeabaseDDL::isTrafBackupSchema(const NAString &catName, const NAString &schName) {
  if (catName.isNull()) return FALSE;
...
// ----------------------------------------------------------------------------
NABoolean CmpSeabaseDDL::isUserUpdatableSeabaseMD(const NAString &catName, const NAString &schName,
  ...
  if (NOT catName.isNull()) {
    ...
    if ((catName == seabaseDefCatName) && (schName == SEABASE_MD_SCHEMA) && (objName == SEABASE_DEFAULTS)) {
      return TRUE;
    }
  }
...
ExpHbaseInterface *CmpSeabaseDDL::allocEHI(const char *connectParam1, const char *connectParam2, NABoolean raiseError,
                                           ComStorageType storageType) {
  ExpHbaseInterface *ehi = NULL;
  NABoolean replSync = FALSE;
  ehi = ExpHbaseInterface::newInstance(heap_, connectParam1, connectParam2, storageType, replSync);

  int retcode = ehi->init(NULL);
  if (retcode < 0) {
    if (raiseError) {
      *CmpCommon::diags() << DgSqlCode(-8448) << DgString0((char *)"ExpHbaseInterface::init()")
...
ExpHbaseInterface *CmpSeabaseDDL::allocBRCEHI() {
  ...
  int retcode = ehi->initBRC(NULL);
  if (retcode != 0) {
    *CmpCommon::diags() << DgSqlCode(-8448) << DgString0((char *)"ExpHbaseInterface::initBRC()")
                        << DgString1(getHbaseErrStr(retcode)) << DgInt0(-retcode)
                        << DgString2((char *)GetCliGlobals()->getJniErrorStr());
...
ComBoolean CmpSeabaseDDL::isSeabaseMD(const ComObjectName &name) {
  return isSeabaseMD(name.getCatalogNamePartAsAnsiString(), name.getSchemaNamePartAsAnsiString(TRUE),
                     name.getObjectNamePartAsAnsiString());
}

ComBoolean CmpSeabaseDDL::isSeabasePrivMgrMD(const ComObjectName &name) {
  return isSeabasePrivMgrMD(name.getCatalogNamePartAsAnsiString(), name.getSchemaNamePartAsAnsiString(TRUE));
}

void CmpSeabaseDDL::getColName(const char *colFam, const char *colQual, NAString &colName) {
  char c;
...
short CmpSeabaseDDL::readAndInitDefaultsFromSeabaseDefaultsTable(NADefaults::Provenance overwriteIfNotYet,
  ...
  if (defs->seabaseDefaultsTableRead()) return 0;
  ...
  hbaseDefaultsStr = genHBaseObjName(
      getSystemCatalog(), NAString(SEABASE_MD_SCHEMA), NAString(SEABASE_DEFAULTS),
      (CmpCommon::context()->useReservedNamespace() ? NAString(TRAF_RESERVED_NAMESPACE1) : NAString("")));
  ...
  retcode = ehi->fetchAllRows(hbaseDefaults,
                              2,  // numCols
                              col1TextStr, col2TextStr, colUnused, col1ValueList, col2ValueList, listUnused);
  ...
  for (int i = 0; i < col1ValueList.entries(); i++) {
    ...
    if (attrName == "RISK_PREMIUM_NJ" || attrName == "RISK_PREMIUM_SERIAL" ||
        attrName == "PARTITIONING_SCHEME_SHARING" || attrName == "ROBUST_HJ_TO_NJ_FUDGE_FACTOR" ||
        attrName == "ROBUST_SORTGROUPBY" || attrName == "RISK_PREMIUM_MJ") {
  ...
  // deal delay cqds
  for (int i = 0; i < delayAttrNameList.entries(); i++) {
...
label_return:
  ...
  if (col1) heap_->deallocateMemory(col1);
  if (col2) heap_->deallocateMemory(col2);
...
short CmpSeabaseDDL::getSystemSoftwareVersion(long &softMajVers, long &softMinVers, long &softUpdVers) {
  //  int cmaj, cmin, cupd;
  int pmaj, pmin, pupd;
...
short CmpSeabaseDDL::getVersionInfo(VersionInfo &vInfo) {
  ...
  if (testPoint != NULL)
    ...
    vInfo.dbVersionStr_ = (msg_license_advanced_enabled() ? "EsgynDB Advanced " : "EsgynDB Enterprise ");
  ...
  char buf[300];
  snprintf(buf, sizeof(buf), "Metadata Version %ld.%ld.%ld", vInfo.mdMajorVersion_, vInfo.mdMinorVersion_,
...
short CmpSeabaseDDL::validateVersions(NADefaults *defs, ExpHbaseInterface *inEHI, long *mdMajorVersion,
                                      long *mdMinorVersion, long *mdUpdateVersion, long *sysSWMajorVersion,
                                      long *sysSWMinorVersion, long *sysSWUpdVersion, long *mdSWMajorVersion,
                                      long *mdSWMinorVersion, long *mdSWUpdateVersion, int *hbaseErrNum,
  ...
  long sysMajorVersion = 0;
  long sysMinorVersion = 0;
  long sysUpdVersion = 0;
  long versionMultiplier;
  ...
  if (!ehi) {
    ...
    while (NULL == (ehi = allocEHI(server, zkPort, TRUE, COM_STORAGE_HBASE))) {
      ...
      if ((CmpCommon::diags()) && (CmpCommon::diags()->getNumber() > 0)) {
        CmpCommon::diags()->clear();
      }
      if (--retryCount <= 0) {
  ...
  if (retcode < 0) {
    if (hbaseErrNum) *hbaseErrNum = retcode;

    if (hbaseErrStr) *hbaseErrStr = (char *)GetCliGlobals()->getJniErrorStr();
  ...
  hbaseVersionsStr = genHBaseObjName(
      getSystemCatalog(), NAString(SEABASE_MD_SCHEMA), NAString(SEABASE_VERSIONS),
      (CmpCommon::context()->useReservedNamespace() ? NAString(TRAF_RESERVED_NAMESPACE1) : NAString("")));
  ...
  if (sysSWMajorVersion || sysSWMinorVersion || sysSWUpdVersion) {
    if (sysSWMajorVersion) *sysSWMajorVersion = sysMajorVersion;
    if (sysSWMinorVersion) *sysSWMinorVersion = sysMinorVersion;
    if (sysSWUpdVersion) *sysSWUpdVersion = sysUpdVersion;
  }

  if (retcode == 0)  // not initialized
  {
    if ((sysMajorVersion != SOFTWARE_MAJOR_VERSION) || (sysMinorVersion != SOFTWARE_MINOR_VERSION) ||
  ...
  retcode = ehi->fetchAllRows(hbaseVersions,
                              3,  // numCols
                              col1TextStr, col2TextStr, col3TextStr, col1ValueList, col2ValueList, col3ValueList);
  if (retcode != HBASE_ACCESS_SUCCESS) {
    if (hbaseErrNum) *hbaseErrNum = retcode;
  ...
  if ((col1ValueList.entries() == 0) || (col2ValueList.entries() == 0) || (col3ValueList.entries() == 0)) {
  ...
  for (int i = 0; i < col1ValueList.entries(); i++) {
    ...
    long majorVersion = *(long *)col2ValueList[i].data();
    long minorVersion = *(long *)col3ValueList[i].data();
    ...
    if (temp == "METADATA") {
      ...
      long updateVersion = minorVersion - (minorVersion / versionMultiplier) * versionMultiplier;
      if (mdMajorVersion) *mdMajorVersion = majorVersion;
      if (mdMinorVersion) *mdMinorVersion = minorVersion / versionMultiplier;
      if (mdUpdateVersion) *mdUpdateVersion = updateVersion;

      mdVersionFound = TRUE;
      if ((majorVersion != METADATA_MAJOR_VERSION) || (minorVersion / versionMultiplier != METADATA_MINOR_VERSION) ||
    ...
    if (temp == "DATAFORMAT") {
      if ((majorVersion != DATAFORMAT_MAJOR_VERSION) || (minorVersion != DATAFORMAT_MINOR_VERSION)) {
        retcode = -1396;
        goto label_return;
      }
    }
    ...
    if (temp == "SOFTWARE") {
      ...
      if (mdSWMajorVersion) *mdSWMajorVersion = majorVersion;
      if (mdSWMinorVersion) *mdSWMinorVersion = minorVersion / versionMultiplier;
      if (mdSWUpdateVersion) *mdSWUpdateVersion = minorVersion - (minorVersion / versionMultiplier) * versionMultiplier;
  ...
  if ((NOT mdVersionFound) || (invalidMD)) {
    retcode = -1394;
    goto label_return;
  }
...
label_return:
  ...
  if (col1) heap_->deallocateMemory(col1);
  if (col2) heap_->deallocateMemory(col2);
  if (col3) heap_->deallocateMemory(col3);
  return retcode;
}
...
void CmpSeabaseDDL::restoreAllControlsAndFlags() {
  ...
  if (cmpctxt->getCIClass() != CmpContextInfo::CMPCONTEXT_TYPE_META) {
    ...
    do {
      cliRC =
          cliInterface.executeImmediate("control query shape restore;", NULL, NULL, TRUE, NULL, FALSE, &globalDiags);
    ...
    attempt = 10;
    do {
      cliRC = cliInterface.executeImmediate(cqdsRestore.data(), NULL, NULL, TRUE, NULL, FALSE, &globalDiags);
    } while (0 > cliRC && 0 < attempt--);
    if (cliRC < 0) {
      cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
      return;
...
void CmpSeabaseDDL::processReturn(int retcode) { return; }
...
//
short CmpSeabaseDDL::doNamespaceCheck(ExpHbaseInterface *ehi, NABoolean &isDef, NABoolean &isRes) {
  short retcode = 0;

  ExpHbaseInterface *ehil = ehi;
  if (ehil == NULL) {
    ehil = allocEHI(COM_STORAGE_HBASE);
    if (ehil == NULL) return -1;
  }
  ...
  hbaseDefVersionsStr =
      genHBaseObjName(getSystemCatalog(), NAString(SEABASE_MD_SCHEMA), NAString(SEABASE_VERSIONS), NAString(""));

  hbaseResVersionsStr = genHBaseObjName(getSystemCatalog(), NAString(SEABASE_MD_SCHEMA), NAString(SEABASE_VERSIONS),
...
short CmpSeabaseDDL::isMetadataInitialized(ExpHbaseInterface *ehi) {
  ...
  if (retcode != 0) {
    LOGERROR(CAT_SQL_EXE, "Failed to do namespace check, retcode: %d isDef: %s isRes: %s", retcode,
             (isDef ? "true" : "false"), (isRes ? "true" : "false"));
    abort();
  }
  ...
  if (NOT(isDef || isRes)) {
    ...
    if (!getenv("INIT_TRAF_DEF_NS")) CmpCommon::context()->setUseReservedNamespace(TRUE);
  } else if (isRes) {
    // internal traf system objects are created in reserved namespace.
    CmpCommon::context()->setUseReservedNamespace(TRUE);
  } else if (isDef) {
    if ((!getenv("INIT_TRAF_DEF_NS")) && (!CmpCommon::context()->isMxcmp())) return 3;  // namespace upgrade metadata
  }
  ...
  for (int i = 0; (((retcode == 0) || (retcode == -1)) && (i < sizeof(allMDtablesInfo) / sizeof(MDTableInfo))); i++) {
    ...
    hbaseTablesStr = genHBaseObjName(getSystemCatalog(), NAString(SEABASE_MD_SCHEMA), NAString(mdi.newName),
    ...
    if ((retcode == -1) && (mdi.neededForInitTraf)) numExists++;
  }
  ...
  if (numExists == 0) return 0;  // metadata not initialized

  if (numExists == numTotal) return 1;  // metadata is initialized

  if (numExists < numTotal) return 2;  // metadata is corrupt

  return -1;
}

NABoolean CmpSeabaseDDL::isAuthorizationEnabled() { return CmpCommon::context()->isAuthorizationEnabled(); }
...
// ----------------------------------------------------------------------------
short CmpSeabaseDDL::isPrivMgrMetadataInitialized(NADefaults *defs, NABoolean checkAllPrivTables) {
  CMPASSERT(defs != NULL);
  ...
  size_t numTables = (checkAllPrivTables) ? sizeof(privMgrTables) / sizeof(PrivMgrTableStruct) : 1;
  ...
  for (int ndx_tl = 0; ndx_tl < numTables; ndx_tl++) {
    ...
    hbaseObject = genHBaseObjName(
        getSystemCatalog(), NAString(SEABASE_PRIVMGR_SCHEMA), tableDef.tableName,
        (CmpCommon::context()->useReservedNamespace() ? NAString(TRAF_RESERVED_NAMESPACE1) : NAString("")));
...
//          0, if does not exist
//          other, error
short CmpSeabaseDDL::existsInHbase(const NAString &objName, ExpHbaseInterface *ehi) {
  ExpHbaseInterface *ehil = ehi;
  if (!ehi) {
    ehil = allocEHI(COM_STORAGE_HBASE);
    if (ehil == NULL) return -1;
  ...
  int retcode = ehil->exists(hbaseObj);
  ...
  if (retcode == 0) return 0;  // does not exist
...
//          0, if not empty
//          other, error
int CmpSeabaseDDL::tableIsEmpty(const NAString &objName, ExpHbaseInterface *ehi) {
  ExpHbaseInterface *ehil = ehi;
  if (!ehi) {
    ehil = allocEHI(COM_STORAGE_HBASE);
    if (ehil == NULL) return -1;
  ...
  hbaseObj.val = (char *)objName.data();
  hbaseObj.len = objName.length();
  int retcode = ehil->isEmpty(hbaseObj);
...
const char *CmpSeabaseDDL::getSystemCatalog() { return seabaseSysCat_.data(); }
...
short CmpSeabaseDDL::beginXnIfNotInProgress(ExeCliInterface *cliInterface, NABoolean &xnWasStartedHere,
  ...
  if (NOT xnInProgress(cliInterface)) {
    ...
    if (clearDDLList) cmpContext->ddlObjsList().clearList();
  ...
  LOGDEBUG(CAT_SQL_LOCK, "beginXnIfNotInProgress exit: wasStartedHere=%d", xnWasStartedHere);
  return 0;
}
...
short CmpSeabaseDDL::endXnIfStartedHere(ExeCliInterface *cliInterface, NABoolean &xnWasStartedHere, int cliRC,
  ...
  if (clearObjectLocks && !GetCliGlobals()->currContext()->getTransaction()->xnInProgress()) {
...
short CmpSeabaseDDL::ddlInvalidateNATables(NABoolean isAbort, int processSP, long svptId) {
  ...
  for (int i = 0; i < tgtTables.entries(); i++) {
    ...
    const long objUID = ddlObj.getObjUID();
    ...
    if (ot == COM_USER_DEFINED_ROUTINE_OBJECT)
      ActiveSchemaDB()->getNARoutineDB()->removeNARoutine(cn.getQualifiedNameObj(), qiScope, objUID, TRUE, TRUE);
    else
    ...
    if (qiScope == REMOVE_FROM_ALL_USERS && !ddlObj.isDDLOp()) {
      ...
      if (CmpCommon::getDefault(TRAF_OBJECT_LOCK) == DF_ON && !Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL) &&
          ot == COM_BASE_TABLE_OBJECT && ComIsUserTable(tableName)) {
        ...
        if (!lock->ddlLockedByMe()) {
          LOGERROR(CAT_SQL_LOCK, "DDL lock released before sending invalidation message for %s",
      ...
      LOGINFO(CAT_SQL_LOCK, "Adding QIKEY to invalidate %s %s (UID: %lu)", comObjectTypeName(ot),
              ddlObj.getObjName().data(), ddlObj.getObjUID());
...
// ----------------------------------------------------------------------------
short CmpSeabaseDDL::ddlResetObjectEpochs(NABoolean doAbort, NABoolean clearList, ComDiagsArea *inDiags) {
  ...
  for (int i = 0; i < numObjs; i++) {
    ...
    if ((ddlObj.getObjType() != COM_BASE_TABLE_OBJECT) || (!ddlObj.isDDLOp())) continue;

    char msgBuf[ddlObj.getObjName().length() + 200];
    snprintf(msgBuf, sizeof(msgBuf),
             "DDL End, Entry: %d, Name: %s, "
             "Epoch: %d, Do abort: %d",
    ...
    ObjectEpochCacheEntryName oecName(STMTHEAP, ddlObj.getObjName(), COM_BASE_TABLE_OBJECT, ddlObj.getRedefTime());
    if (doAbort)
    ...
    if (retcode != 0) {
      ...
      *diags << DgSqlCode(-CAT_INTERNAL_EXCEPTION_ERROR) << DgString0(__FILE__) << DgInt0(__LINE__)
             << DgString1(msg.data());
  ...
  if (clearList) cmpContext->ddlObjsList().clear();
...
short CmpSeabaseDDL::deleteFromSharedCache(ExeCliInterface *cliInterface, const NAString objName, ComObjectType objType,
                                           NAString cmd) {
  // Delete if CQD is turned on
  if (CmpCommon::getDefault(TRAF_ENABLE_METADATA_LOAD_IN_SHARED_CACHE) == DF_OFF) return 0;

  // Delete if shared cache is enabled
  if (SharedDescriptorCache::locate() == NULL) return 0;

  if (objType != COM_BASE_TABLE_OBJECT && objType != COM_INDEX_OBJECT) return 0;
...
short CmpSeabaseDDL::finalizeSharedCache() {
  ...
  if (sharedDescCache == NULL && sharedDataCache == NULL) return 0;
  ...
  for (int i = 0; i < numObjs; i++) {
    ...
    if (cacheEntry.isCacheDesc()) {
      if (cacheEntry.getObjType() == COM_PRIVATE_SCHEMA_OBJECT || cacheEntry.getObjType() == COM_SHARED_SCHEMA_OBJECT) {
        searchName.append('.');
    ...
    if (!objCached && (cacheEntry.getDDLOperation() == SharedCacheDDLInfo::DDL_DELETE)) continue;
    ...
    if (cacheEntry.isCacheData()) alterString += " internal ";
    ...
    if (cliRC < 0) {
      cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
      *CmpCommon::diags() << DgSqlCode(CAT_SHARED_CACHE_DDL_OP_FAILED) << DgString0(currentOp.data())
                          << DgString1(currentType.data()) << DgString2(cacheEntry.getObjName().data());
      if (cliRC == -8448 && (cacheEntry.isCacheData()) &&
          cacheEntry.getDDLOperation() == SharedCacheDDLInfo::DDL_UPDATE) {
  ...
  int errorCodeArray[] = {-CAT_SHARED_CACHE_LOAD_ERROR, -CAT_SHARED_CACHE_DUPLICATE_ESP, -2034, -2012};
...
// shared cache.
// ----------------------------------------------------------------------------
void CmpSeabaseDDL::insertIntoSharedCacheList(const NAString &catName, const NAString &schNameIn,
                                              const NAString &objNameIn, ComObjectType objType,
  ...
  if (SharedDescriptorCache::locate() == NULL && SharedTableDataCache::locate() == NULL) return;
  ...
  if (objNameIn[0] == '"') objName = objName.strip(NAString::both, '"');

  // Don't add histogram tables
  if (isHistogramTable(objName)) return;
  ...
  if (objType == COM_BASE_TABLE_OBJECT || objType == COM_PRIVATE_SCHEMA_OBJECT || objType == COM_SHARED_SCHEMA_OBJECT ||
      objType == COM_INDEX_OBJECT) {
    NAString cachedObjName = catName + ".\"" + schName;
    if (objType == COM_BASE_TABLE_OBJECT || objType == COM_INDEX_OBJECT) cachedObjName += NAString("\".\"") + objName;
    cachedObjName += '"';
    SharedCacheDDLInfo newDDLObj(cachedObjName, objType, ddlOperation);
...
short CmpSeabaseDDL::populateKeyInfo(ComTdbVirtTableKeyInfo &keyInfo, OutputInfo *oi, NABoolean isIndex) {
  // get the column name
  int len = strlen(oi->get(0));
  keyInfo.colName = new (STMTHEAP) char[len + 1];

  strcpy((char *)keyInfo.colName, (char *)oi->get(0));

  keyInfo.tableColNum = *(int *)oi->get(1);

  keyInfo.keySeqNum = *(int *)oi->get(2);

  keyInfo.ordering = *(int *)oi->get(3);

  keyInfo.nonKeyCol = *(int *)oi->get(4);
...
NABoolean CmpSeabaseDDL::enabledForSerialization(NAColumn *nac) {
  ...
  if ((nac) && ((NOT givenType->isEncodingNeeded()) || (nac && CmpSeabaseDDL::isSerialized(nac->getHbaseColFlags())))) {
    return TRUE;
  }
...
NABoolean CmpSeabaseDDL::isEncodingNeededForSerialization(NAColumn *nac) {
  const NAType *givenType = nac->getType();
  if ((nac) && (CmpSeabaseDDL::isSerialized(nac->getHbaseColFlags())) &&
      ((givenType->isEncodingNeeded()) && (NOT DFS2REC::isAnyVarChar(givenType->getFSDatatype())))) {
    return TRUE;
  }
...
static NABoolean isHBaseCompressionOption(const NAText &str) {
  if (str == "NONE" || str == "GZ" || str == "LZO" || str == "LZ4" || str == "SNAPPY") return TRUE;
  return FALSE;
}
static NABoolean isHBaseDurabilityOption(const NAText &str) {
  if (str == "ASYNC_WAL" || str == "FSYNC_WAL" || str == "SKIP_WAL" || str == "SYNC" || str == "USE_DEFAULT")
    return TRUE;
  return FALSE;
}
static NABoolean isHBaseEncodingOption(const NAText &str) {
  if (str == "NONE" || str == "PREFIX" || str == "PREFIX_TREE" || str == "DIFF" || str == "FAST_DIFF") return TRUE;
  return FALSE;
}
static NABoolean isHBaseBloomFilterOption(const NAText &str) {
  if (str == "NONE" || str == "ROW" || str == "ROWCOL") return TRUE;
  return FALSE;
}

// note: this function expects hbaseCreateOptionsArray to have
// HBASE_MAX_OPTIONS elements
short CmpSeabaseDDL::generateHbaseOptionsArray(NAText *hbaseCreateOptionsArray,
                                               NAList<HbaseCreateOption *> *hbaseCreateOptions) {
  if (!hbaseCreateOptions) return 0;
  ...
  for (CollIndex i = 0; i < hbaseCreateOptions->entries(); i++) {
    ...
    if (hbaseOption->key() != "NAME") std::transform(s.begin(), s.end(), s.begin(), ::toupper);

    NABoolean isError = FALSE;
    if (hbaseOption->key() == "NAME") {
      if (!hbaseCreateOptionsArray[HBASE_NAME].empty()) isError = TRUE;
      hbaseCreateOptionsArray[HBASE_NAME] = hbaseOption->val();
    } else if (hbaseOption->key() == "MAX_VERSIONS") {
      if ((str_atoi(hbaseOption->val().data(), hbaseOption->val().length()) == -1) ||
      ...
      if ((!isTrueFalseStr(hbaseOption->val())) || (!hbaseCreateOptionsArray[HBASE_BLOCKCACHE].empty())) isError = TRUE;
      hbaseCreateOptionsArray[HBASE_BLOCKCACHE] = hbaseOption->val();
    } else if (hbaseOption->key() == "IN_MEMORY") {
      if ((!isTrueFalseStr(hbaseOption->val())) || (!hbaseCreateOptionsArray[HBASE_IN_MEMORY].empty())) isError = TRUE;
      hbaseCreateOptionsArray[HBASE_IN_MEMORY] = hbaseOption->val();
    } else if (hbaseOption->key() == "COMPRESSION") {
      if ((!isHBaseCompressionOption(hbaseOption->val())) || (!hbaseCreateOptionsArray[HBASE_COMPRESSION].empty()))
        isError = TRUE;
      hbaseCreateOptionsArray[HBASE_COMPRESSION] = hbaseOption->val();
    } else if (hbaseOption->key() == "BLOOMFILTER") {
      if ((!isHBaseBloomFilterOption(hbaseOption->val())) || (!hbaseCreateOptionsArray[HBASE_BLOOMFILTER].empty()))
        isError = TRUE;
      ...
      if (hbaseOption->val() != "AES") isError = TRUE;
      hbaseCreateOptionsArray[HBASE_ENCRYPTION] = hbaseOption->val();
    } else if (hbaseOption->key() == "DATA_BLOCK_ENCODING") {
      if ((!isHBaseEncodingOption(hbaseOption->val())) || (!hbaseCreateOptionsArray[HBASE_DATA_BLOCK_ENCODING].empty()))
        isError = TRUE;
      hbaseCreateOptionsArray[HBASE_DATA_BLOCK_ENCODING] = hbaseOption->val();
    } else if (hbaseOption->key() == "CACHE_BLOOMS_ON_WRITE") {
      if ((!isTrueFalseStr(hbaseOption->val())) || (!hbaseCreateOptionsArray[HBASE_CACHE_BLOOMS_ON_WRITE].empty()))
        isError = TRUE;
      hbaseCreateOptionsArray[HBASE_CACHE_BLOOMS_ON_WRITE] = hbaseOption->val();
    } else if (hbaseOption->key() == "CACHE_DATA_ON_WRITE") {
      if ((!isTrueFalseStr(hbaseOption->val())) || (!hbaseCreateOptionsArray[HBASE_CACHE_DATA_ON_WRITE].empty()))
        isError = TRUE;
      hbaseCreateOptionsArray[HBASE_CACHE_DATA_ON_WRITE] = hbaseOption->val();
    } else if (hbaseOption->key() == "CACHE_INDEXES_ON_WRITE") {
      if ((!isTrueFalseStr(hbaseOption->val())) || (!hbaseCreateOptionsArray[HBASE_CACHE_INDEXES_ON_WRITE].empty()))
        isError = TRUE;
      hbaseCreateOptionsArray[HBASE_CACHE_INDEXES_ON_WRITE] = hbaseOption->val();
      ...
      if ((!isHBaseCompressionOption(hbaseOption->val())) ||
        ...
        isError = TRUE;
      hbaseCreateOptionsArray[HBASE_COMPACT_COMPRESSION] = hbaseOption->val();
    } else if (hbaseOption->key() == "PREFIX_LENGTH_KEY") {
      if ((str_atoi(hbaseOption->val().data(), hbaseOption->val().length()) == -1) ||
          (!hbaseCreateOptionsArray[HBASE_PREFIX_LENGTH_KEY].empty()))
        isError = TRUE;
      hbaseCreateOptionsArray[HBASE_PREFIX_LENGTH_KEY] = hbaseOption->val();
    } else if (hbaseOption->key() == "EVICT_BLOCKS_ON_CLOSE") {
      if ((!isTrueFalseStr(hbaseOption->val())) || (!hbaseCreateOptionsArray[HBASE_EVICT_BLOCKS_ON_CLOSE].empty()))
        isError = TRUE;
      hbaseCreateOptionsArray[HBASE_EVICT_BLOCKS_ON_CLOSE] = hbaseOption->val();
    } else if (hbaseOption->key() == "KEEP_DELETED_CELLS") {
      if ((!isTrueFalseStr(hbaseOption->val())) || (!hbaseCreateOptionsArray[HBASE_KEEP_DELETED_CELLS].empty()))
        isError = TRUE;
      hbaseCreateOptionsArray[HBASE_KEEP_DELETED_CELLS] = hbaseOption->val();
    } else if (hbaseOption->key() == "REPLICATION_SCOPE") {
      if ((str_atoi(hbaseOption->val().data(), hbaseOption->val().length()) == -1) ||
          (!hbaseCreateOptionsArray[HBASE_REPLICATION_SCOPE].empty()))
        isError = TRUE;
      hbaseCreateOptionsArray[HBASE_REPLICATION_SCOPE] = hbaseOption->val();
    } else if (hbaseOption->key() == "REGION_REPLICATION") {
      if ((str_atoi(hbaseOption->val().data(), hbaseOption->val().length()) == -1) ||
          (!hbaseCreateOptionsArray[HBASE_REGION_REPLICATION].empty()))
        isError = TRUE;
      hbaseCreateOptionsArray[HBASE_REGION_REPLICATION] = hbaseOption->val();
      ...
      if ((!isTrueFalseStr(hbaseOption->val())) || (!hbaseCreateOptionsArray[HBASE_COMPACT].empty())) isError = TRUE;
      hbaseCreateOptionsArray[HBASE_COMPACT] = hbaseOption->val();
    } else if (hbaseOption->key() == "DURABILITY") {
      if ((!isHBaseDurabilityOption(hbaseOption->val())) || (!hbaseCreateOptionsArray[HBASE_DURABILITY].empty()))
        isError = TRUE;
      ...
      if ((str_atoi(hbaseOption->val().data(), hbaseOption->val().length()) == -1) ||
        ...
        isError = TRUE;
      hbaseCreateOptionsArray[HBASE_MEMSTORE_FLUSH_SIZE] = hbaseOption->val();
    } else if (hbaseOption->key() == "CACHE_DATA_IN_L1") {
      if ((!isTrueFalseStr(hbaseOption->val())) || (!hbaseCreateOptionsArray[HBASE_CACHE_DATA_IN_L1].empty()))
        isError = TRUE;
      hbaseCreateOptionsArray[HBASE_CACHE_DATA_IN_L1] = hbaseOption->val();
    } else if (hbaseOption->key() == "PREFETCH_BLOCKS_ON_OPEN") {
      if ((!isTrueFalseStr(hbaseOption->val())) || (!hbaseCreateOptionsArray[HBASE_PREFETCH_BLOCKS_ON_OPEN].empty()))
        isError = TRUE;
      hbaseCreateOptionsArray[HBASE_PREFETCH_BLOCKS_ON_OPEN] = hbaseOption->val();
      ...
      if ((valInOrigCase == "org.apache.hadoop.hbase.regionserver.ConstantSizeRegionSplitPolicy" ||
          ...
          (hbaseCreateOptionsArray[HBASE_SPLIT_POLICY].empty()))
        hbaseCreateOptionsArray[HBASE_SPLIT_POLICY] = valInOrigCase;
      else
        isError = TRUE;
    } else if (hbaseOption->key() == "HDFS_STORAGE_POLICY") {
      hbaseCreateOptionsArray[HBASE_HDFS_STORAGE_POLICY] = hbaseOption->val();
    } else
      isError = TRUE;

    if (isError) {
      *CmpCommon::diags() << DgSqlCode(-8449) << DgString0(hbaseOption->key().data())
...
short CmpSeabaseDDL::createMonarchTable(ExpHbaseInterface *ehi, HbaseStr *table, const int tableType,
                                        NAList<HbaseStr> &cols, NAList<HbaseCreateOption *> *monarchCreateOptions,
                                        const int numSplits, const int keyLength, char **encodedKeysBuffer,
                                        NABoolean doRetry) {
  // TEMPTEMP Monarch
  //  return 0;
  // TEMPTEMP
  ...
  if (retcode == -1) {
    *CmpCommon::diags() << DgSqlCode(-1390) << DgString0(table->val);
    return -1;
  }

  if (retcode < 0) {
    *CmpCommon::diags() << DgSqlCode(-8448) << DgString0((char *)"ExpHbaseInterface::exists()")
                        << DgString1(getHbaseErrStr(-retcode)) << DgInt0(-retcode)
                        << DgString2((char *)GetCliGlobals()->getJniErrorStr());
  ...
  NABoolean noXn = (CmpCommon::getDefault(DDL_TRANSACTIONS) == DF_OFF) ? true : false;

  NAText monarchCreateOptionsArray[HBASE_MAX_OPTIONS];
  if (generateHbaseOptionsArray(monarchCreateOptionsArray, monarchCreateOptions) < 0) {
    // diags already set
    return -1;
  }

  retcode = ehi->create(*table, tableType, cols, monarchCreateOptionsArray, numSplits, keyLength,
                        (const char **)encodedKeysBuffer, noXn, isMVCC);
...
short CmpSeabaseDDL::createHbaseTable(ExpHbaseInterface *ehi, HbaseStr *table, std::vector<NAString> &colFamVec,
                                      NAList<HbaseCreateOption *> *hbaseCreateOptions, const int numSplits,
                                      const int keyLength, char **encodedKeysBuffer, NABoolean doRetry,
                                      NABoolean ddlXns, NABoolean incrBackupEnabled, NABoolean createIfNotExists,
                                      NABoolean isMVCC) {
  ...
  retcode = -1;
  int numTries = 0;
  int delaySecs = 500;  // 5 secs to start with
  if (doRetry) {
  ...
  if (retcode == -1)  // table exists
  {
    ...
    *CmpCommon::diags() << DgSqlCode(-1431) << DgString0(table->val);
    return -1;
  }
  ...
  if (generateHbaseOptionsArray(hbaseCreateOptionsArray, hbaseCreateOptions) < 0) {
    // diags already set
    return -1;
  }
  ...
  if (NOT hbaseCreateOptionsArray[HBASE_NAME].empty()) {
    ...
    *CmpCommon::diags() << DgSqlCode(-8448) << DgString0((char *)"CmpSeabaseDDL::generateHbaseOptionsArray()")
                        << DgString1(getHbaseErrStr(-retcode)) << DgInt0(-retcode) << DgString2((char *)"NAME");
  ...
  if (colFamVec.size() > 5) ddlXns = FALSE;
  retcode = ehi->create(*table, hbaseCreateOptionsArray, numSplits, keyLength, (const char **)encodedKeysBuffer,
                        (NOT ddlXns), isMVCC, incrBackupEnabled);
...
short CmpSeabaseDDL::createHbaseTable(ExpHbaseInterface *ehi, HbaseStr *table, const char *cf1,
                                      NAList<HbaseCreateOption *> *inHbaseCreateOptions, const int numSplits,
                                      const int keyLength, char **encodedKeysBuffer, NABoolean doRetry,
                                      NABoolean ddlXns, NABoolean incrBackupEnabled, NABoolean createIfNotExists,
                                      NABoolean isMVCC) {
  if (!cf1) return -1;
  ...
  if (!inHbaseCreateOptions) hbaseCreateOptions = &lHbaseCreateOptions;

  return createHbaseTable(ehi, table, colFamVec, hbaseCreateOptions, numSplits, keyLength, encodedKeysBuffer, doRetry,
                          ddlXns, incrBackupEnabled, createIfNotExists, isMVCC);
}

short CmpSeabaseDDL::alterHbaseTable(ExpHbaseInterface *ehi, HbaseStr *table, NAList<NAString> &allColFams,
                                     NAList<HbaseCreateOption *> *hbaseCreateOptions, NABoolean ddlXns) {
  short retcode = 0;
  NAText hbaseCreateOptionsArray[HBASE_MAX_OPTIONS];

  if (generateHbaseOptionsArray(hbaseCreateOptionsArray, hbaseCreateOptions)) {
    // diags already set
    retcode = -1;
  } else {
    ...
    if (retcode < 0) {
      *CmpCommon::diags() << DgSqlCode(-8448) << DgString0((char *)"ExpHbaseInterface::alter()")
                          << DgString1(getHbaseErrStr(-retcode)) << DgInt0(-retcode)
                          << DgString2((char *)GetCliGlobals()->getJniErrorStr());
      retcode = -1;
    }  // if
...
short CmpSeabaseDDL::dropHbaseTable(ExpHbaseInterface *ehi, HbaseStr *table, NABoolean asyncDrop, NABoolean ddlXns) {
  short retcode = 0;
...
short CmpSeabaseDDL::dropMonarchTable(HbaseStr *table, NABoolean asyncDrop, NABoolean ddlXns) {
  short retcode = 0;
...
short CmpSeabaseDDL::copyHbaseTable(ExpHbaseInterface *ehi, HbaseStr *srcTable, HbaseStr *tgtTable, NABoolean force) {
  short retcode = 0;
...
short CmpSeabaseDDL::checkDefaultValue(const NAString &colExtName, const NAType *colType, ElemDDLColDef *colNode) {
  short rc = 0;
  ...
  rc = Generator::genAndEvalExpr(CmpCommon::context(), buf, 1, evalExpr, NULL, CmpCommon::diags());

  if (rc) {
    NAString result;
    colNode->getDefaultValueExpr()->unparse(result, PARSER_PHASE);

    *CmpCommon::diags() << DgSqlCode(-CAT_INCOMPATIBLE_DATA_TYPE_IN_DEFAULT_CLAUSE) << DgColumnName(colExtName)
                        << DgString0(colType->getTypeSQLname(TRUE).data()) << DgString1(result.data());
  }
...
short CmpSeabaseDDL::getTypeInfo(const NAType *naType, NABoolean alignedFormat, int serializedOption, int &datatype,
                                 ...
                                 int &nullable, NAString &charset, CharInfo::Collation &collationSequence,
                                 int &hbaseColFlags) {
  short rc = 0;
  ...
  datatype = (int)naType->getFSDatatype();
  ...
  switch (naType->getTypeQualifier()) {
    case NA_CHARACTER_TYPE: {
      ...
      if (serializedOption == 1)  // option explicitly specified
      {
        setFlags(hbaseColFlags, NAColumn::SEABASE_SERIALIZED);
                 ...
                 (CmpCommon::getDefault(HBASE_SERIALIZATION) == DF_ON) && (NOT alignedFormat)) {
        setFlags(hbaseColFlags, NAColumn::SEABASE_SERIALIZED);
      }
    } break;
    ...
    case NA_NUMERIC_TYPE: {
      ...
      if (serializedOption == 1)  // option explicitly specified
      {
        if (DFS2REC::isBinaryNumeric(datatype))
          ...
          *CmpCommon::diags() << DgSqlCode(-1191) << DgString0(numericType->getSimpleTypeName());
          return -1;
        }
      } else if ((serializedOption == -1) &&  // not specified
                 (CmpCommon::getDefault(HBASE_SERIALIZATION) == DF_ON) && (DFS2REC::isBinaryNumeric(datatype)) &&
                 (NOT alignedFormat)) {
        setFlags(hbaseColFlags, NAColumn::SEABASE_SERIALIZED);
      }
    } break;
    ...
    case NA_INTERVAL_TYPE: {
      DatetimeIntervalCommonType *dtiCommonType = (DatetimeIntervalCommonType *)naType;
      ...
      if ((serializedOption == 1) && (dtiCommonType->isEncodingNeeded())) {
        *CmpCommon::diags() << DgSqlCode(-1191) << DgString0(dtiCommonType->getSimpleTypeName());
...
short CmpSeabaseDDL::getNAColumnFromColDef(ElemDDLColDef *colNode, NAColumn *&naCol) {
  NAString colFamily;
  NAString colName;
  int datatype, length, precision, scale, dt_start, dt_end;
  int nullable, upshifted;
  ...
  NABoolean alignedFormat = FALSE;
  if (getColInfo(colNode, FALSE, colFamily, colName, alignedFormat, datatype, length, precision, scale, dt_start,
  ...
  char *defV = NULL;
  if ((defaultClass != COM_NO_DEFAULT) && (!defVal.isNull())) {
    char *data = (char *)defVal.data();
    int len = defVal.length();
...
short CmpSeabaseDDL::getColInfo(ElemDDLColDef *colNode, NABoolean isMetadataHistOrReposColumn, NAString &colFamily,
                                ...
                                ComColumnClass &colClass, ComColumnDefaultClass &defaultClass, NAString &defVal,
                                NAString &heading, ComLobsStorageType &lobStorage, NAString &compDefnStr,
                                int &hbaseColFlags, long &colFlags) {
  short rc = 0;
  ...
  if (colNode->isHeadingSpecified()) heading = colNode->getHeading();

  int serializedOption = -1;  // not specified
  ...
  rc = getTypeInfo(naType, alignedFormat, serializedOption, datatype, length, precision, scale, dtStart, dtEnd,
  ...
  if (collationSequence != CharInfo::DefaultCollation) {
    ...
    *CmpCommon::diags() << DgSqlCode(-4069) << DgColumnName(ToAnsiIdentifier(colName))
  ...
  if ((!isMetadataHistOrReposColumn) && (naType->getTypeQualifier() == NA_CHARACTER_TYPE) &&
    ...
    *CmpCommon::diags() << DgSqlCode(-4247) << DgInt0(naType->getNominalSize())
  ...
  if (naType->getTypeQualifier() == NA_LOB_TYPE) lobStorage = colNode->getLobStorage();

  if (naType->getTypeQualifier() == NA_CHARACTER_TYPE) {
    CharType *charType = (CharType *)naType;
    if (charType->isVarchar2()) colFlags |= SEABASE_COLUMN_IS_TYPE_VARCHAR2;
  }
  ...
  if (naType->isComposite()) {
    if (!nullable) {
      *CmpCommon::diags() << DgSqlCode(-3242) << DgString0("ROW or ARRAY datatypes must be nullable.");
      return -1;
    }
    ...
    if ((colNode->getDefaultClauseStatus() == ElemDDLColDef::NO_DEFAULT_CLAUSE_SPEC) ||
      ...
      *CmpCommon::diags() << DgSqlCode(-3242) << DgString0("ROW or ARRAY datatypes must have a default value of NULL.");
      return -1;
    }
  }
  ...
  if (colNode->getDefaultClauseStatus() == ElemDDLColDef::NO_DEFAULT_CLAUSE_SPEC)
    ...
    if (colNode->getSGOptions()) {
      ...
      snprintf(datetime, sizeof(datetime), "%4d-%02d-%02d:%02d:%02d:%02d.%06d", timestamp[0], timestamp[1],
               timestamp[2], timestamp[3], timestamp[4], timestamp[5], timestamp[6] * 1000 + timestamp[7]);
      datetime[26] = '\0';
      ...
      defaultClass = COM_CURRENT_UT_DEFAULT;
               ...
               ((ie->getOperatorType() == ITM_CAST) && (ie->child(0)->getOperatorType() == ITM_UNIQUE_ID)) ||
               ((ie->getOperatorType() == ITM_CAST) && (ie->child(0)->getOperatorType() == ITM_CAST) &&
      ...
      rc = checkDefaultValue(ToAnsiIdentifier(name), genericType, colNode);
      if (rc) return -1;
    } else if (ie->getOperatorType() == ITM_UNIQUE_ID_SYS_GUID ||
               ((ie->getOperatorType() == ITM_CAST) && (ie->child(0)->getOperatorType() == ITM_CONVERTTOHEX) &&
      ...
      rc = checkDefaultValue(ToAnsiIdentifier(name), genericType, colNode);
      if (rc) return -1;
    } else if ((ie->getOperatorType() == ITM_USER) || (ie->getOperatorType() == ITM_CURRENT_USER) ||
      ...
      if (ie->castToConstValue(negateIt)->isNull()) {
        ...
        rc = checkDefaultValue(ToAnsiIdentifier(name), genericType, colNode);
        if (rc) return -1;
        if (colName == ElemDDLReplicateClause::getReplicaSysColName()) colFlags |= SEABASE_COLUMN_IS_REPLICA;
        ...
        if (cv->getType()->getTypeQualifier() == NA_CHARACTER_TYPE) {
          if (((CharType *)(cv->getType()))->getCharSet() == CharInfo::UNICODE) {
            ...
            NAString *nas = unicodeToChar(naws.data(), naws.length(), CharInfo::UTF8, STMTHEAP);
            if (nas) {
              defVal = "_UCS2'";
              defVal += *nas;
              defVal += "'";
            ...
            // convert iso to utf8
            NAString *nas = charToChar(CharInfo::UTF8, cvalue, cvlen, CharInfo::ISO88591, STMTHEAP);
            if (nas) {
              defVal = "_ISO88591'";
              defVal += *nas;
              defVal += "'";
...
short CmpSeabaseDDL::createRowId(NAString &key, NAString &part1, int part1MaxLen, NAString &part2, int part2MaxLen,
                                 NAString &part3, int part3MaxLen, NAString &part4, int part4MaxLen) {
  if (part1.isNull()) return 0;
  ...
  keyValPadded = part1;
  if (part1.length() < part1MaxLen) keyValPadded.append(' ', (part1MaxLen - part1.length()));

  if (NOT part2.isNull()) {
    keyValPadded += part2;
    if (part2.length() < part2MaxLen) keyValPadded.append(' ', (part2MaxLen - part2.length()));
  }

  if (NOT part3.isNull()) {
    keyValPadded += part3;
    if (part3.length() < part3MaxLen) keyValPadded.append(' ', (part3MaxLen - part3.length()));
  }

  if (NOT part4.isNull()) {
    keyValPadded += part4;
    if (part4.length() < part4MaxLen) keyValPadded.append(' ', (part4MaxLen - part4.length()));
  }
...
short CmpSeabaseDDL::isValidHbaseName(const char *str) {
  ...
  if (colonPos && strchr(colonPos + 1, ':')) return 0;

  for (int i = 0; i < strlen(str); i++) {
    // skip colon
    if (colonPos && i == (colonPos - str)) continue;

    char c = str[i];

    if (NOT(((c >= '0') && (c <= '9')) || ((c >= 'a') && (c <= 'z')) || ((c >= 'A') && (c <= 'Z')) ||
            ((c == '_') || (c == '-') || (c == '.'))))
...
NAString CmpSeabaseDDL::genHBaseObjName(const NAString &catName, const NAString &schName, const NAString &objName,
                                        const NAString &nameSpace, long dataUID) {
  NABoolean isHBaseMapTable = (catName == TRAFODION_SYSTEM_CATALOG && schName == HBASE_EXT_MAP_SCHEMA);
...
short CmpSeabaseDDL::existsInSeabaseMDTable(ExeCliInterface *cliInterface, const char *catName, const char *schName,
                                            const char *objName, const ComObjectType objectType,
                                            NABoolean checkForValidDef, NABoolean checkForValidHbaseName,
  ...
  char cfvd[100];
  strcpy(cfvd, " ");
  if (checkForValidDef) strcpy(cfvd, " and valid_def = 'Y' ");

  // Name must be a valid hbase name
  if ((checkForValidHbaseName) && ((objectType == COM_BASE_TABLE_OBJECT) || (objectType == COM_INDEX_OBJECT))) {
    if ((!isValidHbaseName(catName)) || (!isValidHbaseName(schName)) || (!isValidHbaseName(objName))) {
  ...
  if (objectType == COM_UNKNOWN_OBJECT)
    str_sprintf(
        buf,
        "select count(*) from %s.\"%s\".%s where catalog_name = '%s' and schema_name = '%s' and object_name = '%s' %s ",
        getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_OBJECTS, catName, quotedSchName.data(), quotedObjName.data(),
        cfvd);
  else
    str_sprintf(buf,
                "select count(*) from %s.\"%s\".%s where catalog_name = '%s' and schema_name = '%s' and object_name = "
                "'%s' and object_type = '%s' %s ",
                getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_OBJECTS, catName, quotedSchName.data(),
                quotedObjName.data(), objectTypeLit, cfvd);
  ...
  if (rowCount > 0)
    ...
    cliRC = getObjectValidDef(cliInterface, catName, schName, objName, objectType, validDef);
    if (cliRC < 0) return -1;

    if ((cliRC == 1) && (NOT validDef))  // found and not valid
    {
      // invalid object, return error.
      NAString extTableName = NAString(catName) + "." + NAString(schName) + "." + NAString(objName);
      CmpCommon::diags()->clear();
...
long CmpSeabaseDDL::getObjectTypeandOwner(ExeCliInterface *cliInterface, const char *catName, const char *schName,
  ...
  char buf[4000];
  str_sprintf(buf,
              "select object_type, object_owner, object_UID, flags from %s.\"%s\".%s "
              "where catalog_name = '%s' and schema_name = '%s' and object_name = '%s' ",
              getSystemCatalogStatic().data(), SEABASE_MD_SCHEMA, SEABASE_OBJECTS, catName, quotedSchName.data(),
              quotedObjName.data());
  ...
  if (objectFlags) *objectFlags = *(long *)ptr;
...
short CmpSeabaseDDL::getObjectName(ExeCliInterface *inCliInterface, long objUID, NAString &catName, NAString &schName,
                                   NAString &objName, char *outObjType, NABoolean lookInObjects,
  ...
  if (lookInObjectsIdx)
    str_sprintf(buf,
                "select catalog_name, schema_name, object_name, object_type from table(index_table %s.\"%s\".%s) where "
                "\"OBJECT_UID@\" = %ld ",
                TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_OBJECTS_UNIQ_IDX, objUID);
  else {
    str_sprintf(buf,
                "select catalog_name, schema_name, object_name, object_type from %s.\"%s\".%s where object_uid = %ld ",
                TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_OBJECTS, objUID);
  }

  if (lookInObjects) {
    char shapeBuf[1000];
    str_sprintf(shapeBuf, "control query shape scan (path '%s.\"%s\".%s')", TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA,
                SEABASE_OBJECTS);
...
long CmpSeabaseDDL::getObjectUID(ExeCliInterface *cliInterface, const char *catName, const char *schName,
                                 const char *objName, const char *inObjType, long *objDataUID, const char *inObjTypeStr,
                                 char *outObjType, NABoolean lookInObjectsIdx, NABoolean reportErrorNow,
                                 long *objectFlags) {
  int retcode = 0;
  int cliRC = 0;

  if (objectFlags) *objectFlags = 0;
  ...
  if (inObjType) {
    if (lookInObjectsIdx && (!objectFlags))  // dont look into index if object_flags are to be returned
      str_sprintf(buf,
                  "select \"OBJECT_UID@\", 0, object_type from table(index_table %s.\"%s\".%s) where catalog_name = "
                  "'%s' and schema_name = '%s' and object_name = '%s'  and object_type = '%s' ",
                  getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_OBJECTS_UNIQ_IDX, catName, quotedSchName.data(),
                  quotedObjName.data(), inObjType);
    else
      str_sprintf(buf,
                  "select object_uid, 0, object_type, flags from %s.\"%s\".%s where catalog_name = '%s' and "
                  "schema_name = '%s' and object_name = '%s'  and object_type = '%s' ",
                  getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_OBJECTS, catName, quotedSchName.data(),
                  quotedObjName.data(), inObjType);
  } else if (inObjTypeStr)
    str_sprintf(buf,
                "select object_uid, 0, object_type, flags from %s.\"%s\".%s where catalog_name = '%s' and schema_name "
                "= '%s' and object_name = '%s'  and ( %s ) ",
                getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_OBJECTS, catName, quotedSchName.data(),
                quotedObjName.data(), inObjTypeStr);
  else  // inObjType == NULL
  {
    if (lookInObjectsIdx && (!objectFlags))
      str_sprintf(buf,
                  "select \"OBJECT_UID@\", 0, object_type from table(index_table %s.\"%s\".%s) where catalog_name = "
                  "'%s' and schema_name = '%s' and object_name = '%s' ",
                  getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_OBJECTS_UNIQ_IDX, catName, quotedSchName.data(),
                  quotedObjName.data());
    else
      str_sprintf(buf,
                  "select object_uid, 0, object_type, flags from %s.\"%s\".%s where catalog_name = '%s' and "
                  "schema_name = '%s' and object_name = '%s' ",
                  getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_OBJECTS, catName, quotedSchName.data(),
                  quotedObjName.data());
  }
  cliRC = cliInterface->fetchRowsPrologue(buf, TRUE /*no exec*/);
  ...
  if (cliRC == 100)  // did not find the row
  {
    ...
    if (reportErrorNow) *CmpCommon::diags() << DgSqlCode(-1389) << DgString0(objName);
...
long CmpSeabaseDDL::getObjectUID(ExeCliInterface *cliInterface, long objDataUID, NABoolean objuid) {
  int retcode = 0;
  int cliRC = 0;
  ...
  if (objuid)
    sqlQuery.format("SELECT object_uid FROM %s.\"%s\".%s WHERE object_uid = %ld", getSystemCatalog(), SEABASE_MD_SCHEMA,
                    SEABASE_OBJECTS, objDataUID);
  else  // pass for internal sql, untill full support replace name by uid
    sqlQuery.format("SELECT object_uid FROM %s.\"%s\".%s WHERE DATA_OBJECT_UID = %ld", getSystemCatalog(),
                    SEABASE_MD_SCHEMA, SEABASE_OBJECTS, objDataUID);
...
long CmpSeabaseDDL::getSchemaUID(const char *catName, const char *schName) {
  ...
  str_sprintf(buf,
              "select object_uid from %s.\"%s\".%s where catalog_name = '%s' and schema_name = '%s' and object_name = "
              "'__SCHEMA__'",
              getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_OBJECTS, (char *)catName, (char *)schName);
...
short CmpSeabaseDDL::getSchemaName(const char *schUID, NAString &schName) {
  schName = "";
  ExeCliInterface cliInterface(STMTHEAP);
  int retcode = 0;
  int cliRC = 0;
  ...
  str_sprintf(buf, "select schema_name from %s.\"%s\".%s where  object_uid = %s", getSystemCatalog(), SEABASE_MD_SCHEMA,
              SEABASE_OBJECTS, (char *)schUID);
  cliRC = cliInterface.fetchRowsPrologue(buf, TRUE);
  if (cliRC < 0) {
    cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
    return -1;
...
long CmpSeabaseDDL::fetchObjectInfo(ExeCliInterface *cliInterface, const char *catName, const char *schName,
  ...
  char buf[4000];
  str_sprintf(buf,
              "select object_uid, object_owner, schema_owner, flags, 0, create_time,redef_time from %s.\"%s\".%s where "
              "catalog_name = '%s' and schema_name = '%s' and object_name = '%s'  and object_type = '%s' %s ",
              getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_OBJECTS, catName, quotedSchName.data(),
              quotedObjName.data(), objectTypeLit, cfvd);
  ...
  if (createTime) *createTime = *(long *)ptr;

  // return redef_time
  cliInterface->getPtrAndLen(7, ptr, len);
  if (redefTime) *redefTime = *(long *)ptr;
...
long CmpSeabaseDDL::getSystemObjectInfo(ExeCliInterface *cliInterface, const char *catName, const char *schName,
  ...
  if (systemObjectInfoCacheEnabled_) {
    if (systemObjectInfoCache_) {
      SystemObjectInfoKey key(catName, schName, objName, objectType, checkForValidDef);

      SystemObjectInfoValue *value = systemObjectInfoCache_->getFirstValue(&key);
      ...
      if (value) {
        ...
        if (createTime) *createTime = value->createTime;

#ifdef DEBUG_GET_SYSTEM_OBJECT_INFO
        cout << "System object found: name=" << objName << ", objectOwner=" << objectOwner
             << ", schemaOwner=" << schemaOwner << ", objectFlags =" << objectFlags << ", createTime=" << createTime
             << endl;
#endif
...
#ifdef DEBUG_GET_SYSTEM_OBJECT_INFO
        cout << "System object not found: name=" << objName << endl;
#endif
      }
    } else {
      ...
      systemObjectInfoCache_ = new SystemObjectInfoHashTableType(hashOnObjectInfoKey, 30, TRUE, /*uniqueness*/
                                                                 NULL, TRUE                     /*failure is fatal*/
      );
    }
  }

  long objUID = fetchObjectInfo(cliInterface, catName, schName, objName, objectType, objectOwner, schemaOwner,
                                objectFlags, objDataUID, reportErrorNow, checkForValidDef, createTime, NULL);

  if (objUID != -1 && createTime && systemObjectInfoCacheEnabled_) {
    // create a key object
    SystemObjectInfoKey *keyPtr = new SystemObjectInfoKey(catName, schName, objName, objectType, checkForValidDef);

    // create a value object
    SystemObjectInfoValue *value =
        new SystemObjectInfoValue(objUID, objectOwner, schemaOwner, objectFlags, *createTime);
...
long CmpSeabaseDDL::getObjectInfo(ExeCliInterface *cliInterface, const char *catName, const char *schName,
  ...
  if (NAString(catName) == getSystemCatalog() &&
    ...
    long oid = getSystemObjectInfo(cliInterface, catName, schName, objName, objectType, objectOwner, schemaOwner,
                                   objectFlags, objDataUID, reportErrorNow, checkForValidDef, createTime);
  ...
  return fetchObjectInfo(cliInterface, catName, schName, objName, objectType, objectOwner, schemaOwner, objectFlags,
                         objDataUID, reportErrorNow, checkForValidDef, createTime, redefTime);
}
...
short CmpSeabaseDDL::getObjectOwner(ExeCliInterface *cliInterface, const char *catName, const char *schName,
  ...
  if (cliRC == 100)  // did not find the row
  {
    ...
    strObjName += '.';
    strObjName += objName;
    *CmpCommon::diags() << DgSqlCode(-1389) << DgString0(strObjName.data());
    return -1;
  }
...
short CmpSeabaseDDL::getObjectValidDef(ExeCliInterface *cliInterface, const char *catName, const char *schName,
  ...
  if (objectType == COM_UNKNOWN_OBJECT)
    str_sprintf(
        buf,
        "select valid_def from %s.\"%s\".%s where catalog_name = '%s' and schema_name = '%s' and object_name = '%s' ",
        getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_OBJECTS, catName, quotedSchName.data(), quotedObjName.data());
  else
    str_sprintf(buf,
                "select valid_def from %s.\"%s\".%s where catalog_name = '%s' and schema_name = '%s' and object_name = "
                "'%s'  and object_type = '%s' ",
                getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_OBJECTS, catName, quotedSchName.data(),
                quotedObjName.data(), objectTypeLit);
...
long CmpSeabaseDDL::getConstraintOnIndex(ExeCliInterface *cliInterface, long btUID, long indexUID,
                                         const char *constrType, NAString &catName, NAString &schName,
                                         NAString &objName) {
  int retcode = 0;
  int cliRC = 0;
  ...
  str_sprintf(
      buf,
      "select C.constraint_uid, O.catalog_name, O.schema_name, O.object_name from %s.\"%s\".%s C, %s.\"%s\".%s O where "
      "C.table_uid = %ld and C.index_uid = %ld and C.constraint_type = '%s' and C.constraint_uid = O.object_uid",
      getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_TABLE_CONSTRAINTS, getSystemCatalog(), SEABASE_MD_SCHEMA,
      SEABASE_OBJECTS, btUID, indexUID, constrType);
...
short CmpSeabaseDDL::getUsingObject(ExeCliInterface *cliInterface, long objUID, NAString &usingObjName) {
  int retcode = 0;
  int cliRC = 0;
  ...
  str_sprintf(
      buf,
      "select trim(catalog_name) || '.' || trim(schema_name) || '.' || trim(object_name) from %s.\"%s\".%s T, "
      "%s.\"%s\".%s VU where VU.used_object_uid = %ld and T.object_uid = VU.using_view_uid  and T.valid_def = 'Y' ",
      getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_OBJECTS, getSystemCatalog(), SEABASE_MD_SCHEMA,
      SEABASE_VIEWS_USAGE, objUID);
  ...
  if (usingViewsQueue->numEntries() == 0) return 100;
...
short CmpSeabaseDDL::getBaseTable(ExeCliInterface *cliInterface, const NAString &indexCatName,
  ...
  str_sprintf(buf,
              "select trim(O.catalog_name), trim(O.schema_name),"
              ...
              getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_OBJECTS, getSystemCatalog(), SEABASE_MD_SCHEMA,
              SEABASE_INDEXES, getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_OBJECTS, (char *)indexCatName.data(),
              (char *)indexSchName.data(), (char *)indexObjName.data());
  ...
  if (usingTableQueue->numEntries() != 1) {
    ...
    // error
    *CmpCommon::diags() << DgSqlCode(-3251) << DgString0("This")
                        << DgString1(" Base table for the specified index '" + extIndexName +
...
short CmpSeabaseDDL::getUsingViews(ExeCliInterface *cliInterface, long objectUID, Queue *&usingViewsQueue) {
  int retcode = 0;
  int cliRC = 0;
  ...
  str_sprintf(buf,
              "select '\"' || trim(catalog_name) || '\"' || '.' || '\"' || trim(schema_name) || '\"' || '.' || '\"' || "
              "trim(object_name) || '\"' , object_uid "
...
short CmpSeabaseDDL::getUsingViews(ExeCliInterface *cliInterface, long objectUID, const NAString &colName,
  ...
  str_sprintf(buf,
              ...
              getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_OBJECTS, getSystemCatalog(), SEABASE_MD_SCHEMA,
              SEABASE_COLUMNS, getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_VIEWS_USAGE, getSystemCatalog(),
              SEABASE_MD_SCHEMA, SEABASE_VIEWS_USAGE, objectUID, getSystemCatalog(), SEABASE_MD_SCHEMA,
              SEABASE_VIEWS_USAGE, objectUID, colName.data());
  cliRC = cliInterface->fetchAllRows(usingViewsQueue, buf, 0, FALSE, FALSE, TRUE);
  if (cliRC < 0) {
    cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
    return cliRC;
...
short CmpSeabaseDDL::getAllUsingViews(ExeCliInterface *cliInterface, NAString &catName, NAString &schName,
                                      NAString &objName, Queue *&usingViewsQueue) {
  int retcode = 0;
  int cliRC = 0;
  ...
  str_sprintf(buf,
              ...
              "'\"', '') "
              " order by 2",
              getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_OBJECTS, catName.data(), schName.data(), objName.data());
...
short CmpSeabaseDDL::getSaltText(ExeCliInterface *cliInterface, const char *catName, const char *schName,
                                 const char *objName, const char *inObjType, NAString &saltText) {
  int cliRC = 0;
  ...
  str_sprintf(
      buf,
      "select object_uid, column_number, cast(default_value as varchar(512) character set iso88591) from %s.\"%s\".%s "
      "o, %s.\"%s\".%s c where o.catalog_name = '%s' and o.schema_name = '%s' and o.object_name = '%s'  and "
      "o.object_type = '%s'  and o.object_uid = c.object_uid and c.column_name = '%s' and c.default_class = %d",
      getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_OBJECTS, getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_COLUMNS,
      catName, quotedSchName.data(), quotedObjName.data(), inObjType, ElemDDLSaltOptionsClause::getSaltSysColName(),
  ...
  // did not find row
  if (saltQueue->numEntries() == 0) return 0;

  saltQueue->position();
  OutputInfo *vi = (OutputInfo *)saltQueue->getNext();
  objUID = *(long *)vi->get(0);
  colNum = *(int *)vi->get(1);
  defaultValue = vi->get(2);

  // this should be the normal case, salt text is stored in the TEXT table,
  // not the default value
  cliRC = getTextFromMD(cliInterface, objUID, COM_COMPUTED_COL_TEXT, colNum, saltText);
  if (cliRC < 0) {
    cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
    return -1;
  }

  if (saltText.isNull()) saltText = defaultValue;
...
short CmpSeabaseDDL::getTrafReplicaNum(ExeCliInterface *cliInterface, const char *catName, const char *schName,
                                       const char *objName, const char *inObjType, Int16 &numTrafReplicas) {
  int cliRC = 0;
  ...
  str_sprintf(buf,
              "select object_uid, column_number, cast(default_value as smallint) from %s.\"%s\".%s o, %s.\"%s\".%s c "
              "where o.catalog_name = '%s' and o.schema_name = '%s' and o.object_name = '%s'  and o.object_type = '%s' "
              " and o.object_uid = c.object_uid and c.column_name = '%s' and bitextract(c.flags,60,1) = 1",
              getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_OBJECTS, getSystemCatalog(), SEABASE_MD_SCHEMA,
              SEABASE_COLUMNS, catName, quotedSchName.data(), quotedObjName.data(), inObjType,
  ...
  // did not find row
  if (replicaQueue->numEntries() == 0) return 0;
...
short CmpSeabaseDDL::getAllIndexes(ExeCliInterface *cliInterface, long objUID, NABoolean includeInvalidDefs,
                                   Queue *&indexInfoQueue) {
  int cliRC = 0;

  char query[4000];
  str_sprintf(query,
              "select O.catalog_name, O.schema_name, O.object_name, O.object_uid from %s.\"%s\".%s I, %s.\"%s\".%s O , "
              " %s.\"%s\".%s T where I.base_table_uid = %ld and I.index_uid = O.object_uid %s and I.index_uid = "
              "T.table_uid and I.keytag != 0 ",
...
short CmpSeabaseDDL::genHbaseCreateOptions(const char *hbaseCreateOptionsStr,
                                           NAList<HbaseCreateOption *> *&hbaseCreateOptions, NAMemory *heap,
                                           size_t *beginPos, size_t *endPos) {
  hbaseCreateOptions = NULL;
  if (beginPos) *beginPos = 0;
  if (endPos) *endPos = 0;

  if (hbaseCreateOptionsStr == NULL) return 0;
  ...
  if (beginPos) *beginPos = hboStr - hbaseCreateOptionsStr;
  if (endPos)
...
short CmpSeabaseDDL::genHbaseOptionsMetadataString(const NAList<HbaseCreateOption *> &hbaseCreateOptions,
...
short CmpSeabaseDDL::updateHbaseOptionsInMetadata(ExeCliInterface *cliInterface, long objectUID,
  ...
  int textSubID = 0;                              // meaning, the text pertains to the object as a whole
  NAString metadataText(STMTHEAP);
  result = getTextFromMD(cliInterface, objectUID, textType, textSubID, metadataText /* out */);
  ...
  result = genHbaseCreateOptions(metadataText.data(), hbaseCreateOptions /* out */, STMTHEAP,
                                 &beginHBOTextPos /* out */, &endHBOTextPos /* out */);
  if (result != 0)
  ...
  if (!hbaseCreateOptions) hbaseCreateOptions = new (STMTHEAP) NAList<HbaseCreateOption *>(STMTHEAP);
  ...
  result = genHbaseOptionsMetadataString(*hbaseCreateOptions, hbaseOptionsMetadataString /* out */);
  if (result == 0) {
    // edit the old text, removing the present HBASE_OPTIONS=> text if any,
    // and putting the new text in the same spot

    metadataText.replace(beginHBOTextPos, endHBOTextPos - beginHBOTextPos, hbaseOptionsMetadataString);
    ...
    int cliRC = deleteFromTextTable(cliInterface, objectUID, textType, textSubID);
    if (cliRC < 0) {
      result = -1;
    } else {
      // insert the text back into the metadata

      result = updateTextTable(cliInterface, objectUID, textType, textSubID, metadataText);
    }
  }
...
void CmpSeabaseDDL::handleDDLCreateAuthorizationError(int32_t SQLErrorCode, const NAString &catalogName,
...
short CmpSeabaseDDL::updateSeabaseMDObjectsTable(ExeCliInterface *cliInterface, const char *catName,
                                                 ...
                                                 const ComObjectType &objectType, const char *validDef, int objOwnerID,
                                                 int schemaOwnerID, long objectFlags, long &inUID) {
  int retcode = 0;
  int cliRC = 0;
  ...
  if ((objectType == COM_BASE_TABLE_OBJECT ||                                                              // bt
      ...
      && USE_UUID_AS_HBASE_TABLENAME && ComIsUserTable(getSystemCatalog(), quotedSchName, quotedObjName))  // user table
    objDataUID = objUID;

  str_sprintf(buf, "insert into %s.\"%s\".%s values ('%s', '%s', '%s', '%s', %ld, %ld, %ld, '%s', '%s', %d, %d, %ld )",
              getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_OBJECTS, catName, quotedSchName.data(),
              quotedObjName.data(), objectTypeLit, objUID, createTime, createTime, (validDef ? validDef : COM_YES_LIT),
              COM_NO_LIT, objOwnerID, schemaOwnerID, objectFlags);
...
short CmpSeabaseDDL::deleteFromSeabaseMDObjectsTable(ExeCliInterface *cliInterface, const char *catName,
                                                     const char *schName, const char *objName,
  ...
  str_sprintf(buf,
              "delete from %s.\"%s\".%s where catalog_name = '%s' and schema_name = '%s' and object_name = '%s' and "
              "object_type = '%s' ",
              getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_OBJECTS, catName, quotedSchName.data(),
              quotedObjName.data(), objectTypeLit);
  cliRC = cliInterface->executeImmediate(buf);
...
static short AssignColEntry(ExeCliInterface *cliInterface, int entry, char *currRWRSptr, const char *srcPtr,
                            int firstColOffset) {
  int cliRC = 0;
  ...
  cliRC = cliInterface->getAttributes(entry, TRUE, fsDatatype, length, vcIndLen, &indOffset, &varOffset);
  if (cliRC < 0) {
    cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
  ...
  if (indOffset != -1) *(short *)&currRWRSptr[indOffset] = 0;
  ...
  if (DFS2REC::isAnyCharacter(fsDatatype)) {
    if (DFS2REC::isAnyVarChar(fsDatatype)) {
      if (SQL_VARCHAR_HDR_SIZE == sizeof(short))
        ...
        *(int *)&currRWRSptr[firstColOffset + varOffset] = strlen(srcPtr);
      str_cpy_all(&currRWRSptr[firstColOffset + varOffset + SQL_VARCHAR_HDR_SIZE], srcPtr, strlen(srcPtr));
    } else {
...
short CmpSeabaseDDL::updateSeabaseMDSecondaryIndexes(ExeCliInterface *cliInterface) {
  ...
  for (int i = 0; i < numTables; i++) {
    ...
    if ((mdti.isIndex) || (mddi.numIndexes == 0)) continue;

    const char *catName = TRAFODION_SYSCAT_LIT;
    const char *schName = SEABASE_MD_SCHEMA;
    const char *objName = mdti.newName;
    long baseTableUID = getObjectUID(cliInterface, catName, schName, objName, COM_BASE_TABLE_OBJECT_LIT);

    const ComTdbVirtTableIndexInfo *indexInfo = mddi.indexInfo;
    QualifiedName qn(indexInfo->indexName, 1);

    long indexUID = getObjectUID(cliInterface, qn.getCatalogName().data(), qn.getSchemaName().data(),
                                 qn.getObjectName().data(), COM_INDEX_OBJECT_LIT);

    str_sprintf(buf, "upsert into %s.\"%s\".%s values (%ld, %d, %d, %d, %d, %d, %ld, 0) ", getSystemCatalog(),
                SEABASE_MD_SCHEMA, SEABASE_INDEXES, baseTableUID, indexInfo->keytag, indexInfo->isUnique,
                indexInfo->keyColCount, indexInfo->nonKeyColCount, indexInfo->isExplicit, indexUID);
    cliRC = cliInterface->executeImmediate(buf);
    if (cliRC < 0) {
      cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
...
short CmpSeabaseDDL::updateSeabaseMDTable(ExeCliInterface *cliInterface, const char *catName, const char *schName,
                                          ...
                                          const ComTdbVirtTableIndexInfo *indexInfo, long &inUID, NABoolean updPrivs) {
  ...
  if (updateSeabaseMDObjectsTable(cliInterface, catName, schName, objName, objectType, validDef, objOwnerID,
                                  schemaOwnerID, objectFlags, inUID))
    return -1;
  ...
  int colPos = 0;
  for (int i = 0; i < numKeys; i++) {
    str_sprintf(buf, "upsert into %s.\"%s\".%s values (%ld, '%s', %d, %d, %d, %d, 0)", getSystemCatalog(),
                SEABASE_MD_SCHEMA, SEABASE_KEYS, objUID, keyInfo->colName, keyInfo->keySeqNum, keyInfo->tableColNum,
                keyInfo->ordering, keyInfo->nonKeyCol);
  ...
  ExeCliInterface rwrsCliInterface(STMTHEAP, 0, NULL, CmpCommon::context()->sqlSession()->getParentQid());
  if (useRWRS) {
    ExeCliInterface cqdCliInterface;
    cliRC = cqdCliInterface.holdAndSetCQD("ODBC_PROCESS", "ON");

    str_sprintf(buf,
                "upsert using rowset (max rowset size %d, input rowset size ?, input row max length ?, rowset buffer "
                "?) into %s.\"%s\".%s values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                numCols, getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_COLUMNS);
  ...
  for (int i = 0; i < numCols; i++) {
    ...
    if (useRWRS) {
      ...
      cliRC = rwrsCliInterface.getAttributes(4, TRUE, fsDatatype, length, vcIndLen, &indOffset, &firstColOffset);
      ...
      AssignColEntry(&rwrsCliInterface, entry++, inputRow, (colInfo->upshifted ? COM_YES_LIT : COM_NO_LIT),
                     firstColOffset);
      ...
      AssignColEntry(&rwrsCliInterface, entry++, inputRow,
                     CharInfo::getCharSetName((CharInfo::CharSet)colInfo->charset), firstColOffset);
      AssignColEntry(&rwrsCliInterface, entry++, inputRow, (char *)&colInfo->defaultClass, firstColOffset);
      AssignColEntry(&rwrsCliInterface, entry++, inputRow, (colInfo->defVal ? quotedDefVal.data() : " "),
                     firstColOffset);
      AssignColEntry(&rwrsCliInterface, entry++, inputRow, (colInfo->colHeading ? quotedColHeading.data() : " "),
                     firstColOffset);
      AssignColEntry(&rwrsCliInterface, entry++, inputRow, (colInfo->hbaseColFam ? colInfo->hbaseColFam : " "),
                     firstColOffset);
      AssignColEntry(&rwrsCliInterface, entry++, inputRow, (colInfo->hbaseColQual ? colInfo->hbaseColQual : " "),
                     firstColOffset);
      AssignColEntry(&rwrsCliInterface, entry++, inputRow, colInfo->paramDirection, firstColOffset);
      AssignColEntry(&rwrsCliInterface, entry++, inputRow, (colInfo->isOptional ? COM_YES_LIT : COM_NO_LIT),
                     firstColOffset);
      ...
      str_sprintf(buf,
                  "insert into %s.\"%s\".%s values (%ld, '%s', %d, '%s', %d, '%s', %d, %d, %d, %d, %d, '%s', %d, %d, "
                  "'%s', %d, '%s', '%s', '%s', '%s', '%s', '%s', %ld)",
                  getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_COLUMNS, objUID, colInfo->colName, colInfo->colNumber,
                  colClassLit, colInfo->datatype, getAnsiTypeStrFromFSType(colInfo->datatype), colInfo->length,
                  colInfo->precision, colInfo->scale, colInfo->dtStart, colInfo->dtEnd,
                  (colInfo->upshifted ? "Y" : "N"), colInfo->hbaseColFlags, colInfo->nullable,
                  CharInfo::getCharSetName((CharInfo::CharSet)colInfo->charset), (int)colInfo->defaultClass,
                  (colInfo->defVal ? quotedDefVal.data() : " "), (colInfo->colHeading ? quotedColHeading.data() : " "),
                  colInfo->hbaseColFam ? colInfo->hbaseColFam : " ",
                  colInfo->hbaseColQual ? colInfo->hbaseColQual : " ", colInfo->paramDirection,
                  colInfo->isOptional ? "Y" : "N", colInfo->colFlags);
    ...
    if (isComputedColumn) {
      cliRC =
          updateTextTable(cliInterface, objUID, COM_COMPUTED_COL_TEXT, colInfo->colNumber, computedColumnDefinition);
    ...
    if (colInfo->colFlags & SEABASE_COLUMN_IS_COMPOSITE) {
      ...
      if (colInfo->compDefnStr != NULL) {
        numCompCols++;
        snprintf(buf, sizeof(buf), "%08d%08d", colInfo->colNumber, (int)strlen(colInfo->compDefnStr));
        compDefnStr += buf;
    ...
    if ((!tableInfo) || (tableInfo->rowFormat != COM_ALIGNED_FORMAT_TYPE)) {
      rowTotalLength += colInfo->length + (colInfo->nullable ? 1 : 0) + keyLength + sizeof(long) /*timestamp*/ +
  ...
  if (numCompCols > 0) {
    ...
    if ((CmpCommon::getDefault(TRAF_COMPOSITE_DATATYPE_SUPPORT) == DF_OFF) &&
      ...
      *CmpCommon::diags() << DgSqlCode(-3242) << DgString0("Cannot specify composite datatypes for a table or view.");
      return -1;
    }
    ...
    cliRC = updateTextTable(cliInterface, objUID, COM_COMPOSITE_DEFN_TEXT, 0, compDefnStr);
    ...
    if (tableInfo && compColsInExplodedFormat) tableInfo->tablesFlags |= MD_TABLES_COMP_COLS_IN_EXPLODED_FORMAT;
  }

  if (tableInfo && tableInfo->rowFormat == COM_ALIGNED_FORMAT_TYPE) {
    // one cell contains the aligned row
    rowTotalLength = rowDataLength + keyLength + sizeof(long) /*timestamp*/ + strlen(SEABASE_DEFAULT_COL_FAMILY) +
                     2 /* 2 bytes for col qual #1*/;
  }
  ...
  if (objectType == COM_BASE_TABLE_OBJECT || objectType == COM_INDEX_OBJECT) {
    ...
    if (tableInfo) {
      ...
      if (tableInfo->xnRepl != COM_REPL_NONE) {
        if (tableInfo->xnRepl == COM_REPL_SYNC)
          CmpSeabaseDDL::setMDflags(flags, MD_TABLES_REPL_SYNC_FLG);
        else if (tableInfo->xnRepl == COM_REPL_ASYNC)
          CmpSeabaseDDL::setMDflags(flags, MD_TABLES_REPL_ASYNC_FLG);
      }

      if (tableInfo->storageType == COM_STORAGE_MONARCH) {
        CmpSeabaseDDL::setMDflags(flags, MD_TABLES_STORAGE_MONARCH_FLG);
      }
      if (tableInfo->storageType == COM_STORAGE_BIGTABLE) {
        CmpSeabaseDDL::setMDflags(flags, MD_TABLES_STORAGE_BIGTABLE_FLG);
      }
    }

    str_sprintf(buf, "upsert into %s.\"%s\".%s values (%ld, '%s', '%s', %d, %d, %d, %d, %ld) ", getSystemCatalog(),
                SEABASE_MD_SCHEMA, SEABASE_TABLES, objUID, rowFormat, (isAudited ? "Y" : "N"), rowDataLength,
                rowTotalLength, keyLength, numSaltPartns, flags);
    cliRC = cliInterface->executeImmediate(buf);
    if (cliRC < 0) {
      cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
      return -1;
    ...
    if (hbaseSplitClause && *hbaseSplitClause != '\0') {
      ...
      if (updateTextTable(cliInterface, objUID, COM_HBASE_SPLIT_TEXT, 0, nas)) return -1;
    ...
    if (nameSpace) {
      ...
      if (updateTextTable(cliInterface, objUID, COM_OBJECT_NAMESPACE, 0, nas)) return -1;
    }
  ...
  if (objectType == COM_INDEX_OBJECT && numIndexes > 0) {
    ...
    if (updateTextTable(cliInterface, objUID, COM_OBJECT_NGRAM_TEXT, 0, ngram)) return -1;

    ComObjectName baseTableName(indexInfo->baseTableName);
    const NAString catalogNamePart = baseTableName.getCatalogNamePartAsAnsiString();
    const NAString schemaNamePart = baseTableName.getSchemaNamePartAsAnsiString(TRUE);
    const NAString objectNamePart = baseTableName.getObjectNamePartAsAnsiString(TRUE);

    long baseTableUID = getObjectUID(cliInterface, catalogNamePart.data(), schemaNamePart.data(), objectNamePart.data(),
                                     COM_BASE_TABLE_OBJECT_LIT);

    str_sprintf(buf, "insert into %s.\"%s\".%s values (%ld, %d, %d, %d, %d, %d, %ld, %ld) ", getSystemCatalog(),
                SEABASE_MD_SCHEMA, SEABASE_INDEXES, baseTableUID, indexInfo->keytag, indexInfo->isUnique,
                indexInfo->keyColCount, indexInfo->nonKeyColCount, indexInfo->isExplicit, objUID,
  ...
  if ((isAuthorizationEnabled()) && (updPrivs)) {
    QualifiedName qn(objName, schName, catName);
    NAString fullName(qn.getQualifiedNameAsAnsiString());

    if (!insertPrivMgrInfo(objUID, fullName, objectType, objOwnerID, schemaOwnerID, ComUser::getCurrentUser())) {
      *CmpCommon::diags() << DgSqlCode(-CAT_UNABLE_TO_GRANT_PRIVILEGES) << DgTableName(objName);
      return -1;
    }
  }
  return 0;
...
short CmpSeabaseDDL::updateSeabaseMDLibrary(ExeCliInterface *cliInterface, const char *catName, const char *schName,
  ...
  str_sprintf(buf, "insert into %s.\"%s\".%s values ('%s', '%s', '%s', '%s', %ld, %ld, %ld, '%s', '%s', %d, %d, 0 )",
              getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_OBJECTS, catName, quotedSchName.data(),
              quotedLibObjName.data(), COM_LIBRARY_OBJECT_LIT, libObjUID, createTime, createTime, COM_YES_LIT,
  ...
  str_sprintf(buf, "insert into %s.\"%s\".%s values (%ld, '%s', NULL, %d, 0)", getSystemCatalog(), SEABASE_MD_SCHEMA,
              SEABASE_LIBRARIES, libObjUID, libPath, libVersion);
...
short CmpSeabaseDDL::updateSeabaseMDSPJ(ExeCliInterface *cliInterface, const long libObjUID, const int ownerID,
  ...
  if (updateSeabaseMDTable(cliInterface, catalogNamePart, schemaNamePart, quotedSpjObjName,
                           COM_USER_DEFINED_ROUTINE_OBJECT, "Y", tableInfo, numCols, colInfo, 0, NULL, 0, NULL,
                           objUID)) {
    return -1;
  }

  long spjObjUID = getObjectUID(cliInterface, catalogNamePart, schemaNamePart, quotedSpjObjName,
                                COM_USER_DEFINED_ROUTINE_OBJECT_LIT);
  if (spjObjUID == -1) return -1;

  str_sprintf(buf,
              "insert into %s.\"%s\".%s values (%ld, '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', %d, %d, '%s', "
              "'%s', '%s', '%s', '%s', %ld, '%s', 0 )",
              getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_ROUTINES, spjObjUID, routineInfo->UDR_type,
              routineInfo->language_type, routineInfo->deterministic ? "Y" : "N", routineInfo->sql_access,
              routineInfo->call_on_null ? "Y" : "N", routineInfo->isolate ? "Y" : "N", routineInfo->param_style,
              routineInfo->transaction_attributes, routineInfo->max_results, routineInfo->state_area_size,
              routineInfo->external_name, routineInfo->parallelism, routineInfo->user_version,
              routineInfo->external_security, routineInfo->execution_mode, libObjUID, routineInfo->signature);
  ...
  str_sprintf(buf, "insert into %s.\"%s\".%s values (%ld, %ld, 0)", getSystemCatalog(), SEABASE_MD_SCHEMA,
              SEABASE_LIBRARIES_USAGE, libObjUID, spjObjUID);
...
short CmpSeabaseDDL::addSeabaseMDPartition(ExeCliInterface *cliInterface, ElemDDLPartitionV2 *pPartition,
                                           const char *catName, const char *schName, const char *entityName, long btUid,
                                           int index, long partUid, const char *partName) {
  char buf[4000];
  ...
  long partitionUID = partUid;
  getPartitionTableName(partitionTableName, 256, NAString(entityName), pPartition->getPartitionName());
  ...
  if (pPartition->hasSubpartition()) {
    ...
    partitionUID = getObjectUID(cliInterface, catName, schName, partitionTableName, COM_BASE_TABLE_OBJECT_LIT);
  }
  ...
  // subpartitions
  if (pPartition->hasSubpartition()) {
    ElemDDLPartitionV2Array *subpartitionArray = pPartition->getSubpartitionArray();
    ...
    for (CollIndex j = 0; j < subpartitionArray->entries(); ++j) {
      ...
      getPartitionTableName(subpartitionTableName, 256, NAString(entityName), subPartition->getPartitionName());
      long subpUID = getObjectUID(cliInterface, catName, schName, subpartitionTableName, COM_BASE_TABLE_OBJECT_LIT);
...
short CmpSeabaseDDL::updateSeabaseMDPartition(ExeCliInterface *cliInterface, StmtDDLCreateTable *createTableNode,
                                              const char *catName, const char *schName, const char *entityName,
                                              long btUid, int *partitionColIdx, int *subpartitionColIdx) {
  const ElemDDLPartitionV2Array &partitionV2Array = createTableNode->getPartitionV2Array();
  ...
  if (useRWRS) {
    ...
    for (CollIndex i = 0; i < partitionV2Array.entries(); ++i) {
      ...
      if (pPartition->hasSubpartition()) partitionNum += pPartition->getSubpartitionArray()->entries();
    }
    ...
    str_sprintf(buf,
                "upsert using rowset (max rowset size %d, input rowset size ?, input row max length ?, rowset buffer "
                "?) into %s.\"%s\".%s values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                partitionNum, getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_PARTITIONS);
  ...
  str_sprintf(
      buf, "insert into %s.\"%s\".%s values (%ld, %d, '%s', '%s', %d, '%s', '%c', %d, '%s', '%s', %d, '%s', '%c', %ld)",
      getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_TABLE_PARTITION, btUid, (int)(createTableNode->getPartitionType()),
      columNames.data(), columIndex.data(), partitionColArray.entries(), dummyChar, 'N',
      createTableNode->getSubpartitionType(), (subpColNum == 0 ? COM_NULL_LIT : subpartitionColName.data()),
      (subpColNum == 0 ? COM_NULL_LIT : subcolumIndex.data()), subpColNum, dummyChar, 'N', (long)(0));
...
short CmpSeabaseDDL::updateSeabaseTablePartitions(ExeCliInterface *cliInterface, long parentUid, const char *partName,
  ...
  str_sprintf(buf,
              ...
              "'%s', '%s', %ld, %ld, '%s', '%s', '%s', '%s')",
              getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_PARTITIONS, parentUid, partName, partEntityName,
              partitionUid, partitionLevel, partitionPos, str_len(partitionExp), status == NULL ? COM_YES_LIT : status,
              readonly == NULL ? COM_NO_LIT : readonly, inMemory == NULL ? COM_NO_LIT : inMemory, NA_JulianTimestamp(),
              flags, dummyChar, dummyChar, dummyChar, dummyChar);

  cliRC = cliInterface->executeImmediateCEFC(buf, CONST_CAST(char *, partitionExp), str_len(partitionExp), NULL, NULL,
                                             NULL);
  if (cliRC < 0) {
    cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
    return cliRC;
...
short CmpSeabaseDDL::updateTablePartitionsPartitionName(ExeCliInterface *cliInterface, NAString &currCatName,
                                                        NAString &currSchName, NAString &oldPtNm, NAString &newPtNm,
                                                        long parentUid) {
  int cliRC = 0;

  char buf[4000];
  str_sprintf(buf,
              "update %s.\"%s\".%s set partition_name = '%s' "
              "where parent_uid = %ld and partition_name = '%s'",
              getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_PARTITIONS, newPtNm.data(), parentUid, oldPtNm.data());
  cliRC = cliInterface->executeImmediate(buf);
  if (cliRC < 0) {
    cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
    return -1;
...
NABoolean CmpSeabaseDDL::isExistsPartitionName(NATable *naTable, NAList<NAString> &partitionNames) {
  for (int i = 0; i < partitionNames.entries(); i++) {
    if (isExistsPartitionName(naTable, partitionNames[i])) return TRUE;
  }

  return FALSE;
}

NABoolean CmpSeabaseDDL::isExistsPartitionName(ExeCliInterface *cliInterface, const NAString &currCatName,
  ...
  query.format(
      "select PARTITION_ENTITY_NAME from %s.\"%s\".%s where PARTITION_NAME = '%s' and PARENT_UID = ("
      "select \"OBJECT_UID@\" from table(index_table %s.\"%s\".%s) where catalog_name = '%s' and schema_name = '%s' "
      "and object_name = '%s')",
      getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_PARTITIONS, partitionName.data(), getSystemCatalog(),
      SEABASE_MD_SCHEMA, SEABASE_OBJECTS_UNIQ_IDX, currCatName.data(), currSchName.data(), currObjName.data());
...
short CmpSeabaseDDL::deleteFromSeabaseMDTable(ExeCliInterface *cliInterface, const char *catName, const char *schName,
  ...
  long objUID = getObjectUID(cliInterface, catName, schName, objName, objectTypeLit);

  if (objUID < 0) return -1;

  cliRC = deleteFromSeabaseMDObjectsTable(cliInterface, catName, schName, objName, objType);
  if (cliRC < 0) {
    cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
  ...
  if (objType == COM_LIBRARY_OBJECT) {
    str_sprintf(buf, "delete from %s.\"%s\".%s where library_uid = %ld", getSystemCatalog(), SEABASE_MD_SCHEMA,
                SEABASE_LIBRARIES, objUID);
    cliRC = cliInterface->executeImmediate(buf);
    if (cliRC < 0) {
      cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
      return -1;
    ...
    str_sprintf(buf, "delete from %s.\"%s\".%s where text_uid = %ld", getSystemCatalog(), SEABASE_MD_SCHEMA,
                SEABASE_TEXT, objUID);
    cliRC = cliInterface->executeImmediate(buf);
    if (cliRC < 0) {
      cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
  ...
  if (objType == COM_SEQUENCE_GENERATOR_OBJECT) {
    str_sprintf(buf, "delete from %s.\"%s\".%s where seq_uid = %ld", getSystemCatalog(), SEABASE_MD_SCHEMA,
                SEABASE_SEQ_GEN, objUID);
    cliRC = cliInterface->executeImmediate(buf);
    if (cliRC < 0) {
      cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
      return -1;
  ...
  str_sprintf(buf, "delete from %s.\"%s\".%s where object_uid = %ld", getSystemCatalog(), SEABASE_MD_SCHEMA,
  ...
  if (objType == COM_USER_DEFINED_ROUTINE_OBJECT) {
    str_sprintf(buf, "delete from %s.\"%s\".%s where udr_uid = %ld", getSystemCatalog(), SEABASE_MD_SCHEMA,
    ...
    str_sprintf(buf, "delete from %s.\"%s\".%s where used_udr_uid = %ld", getSystemCatalog(), SEABASE_MD_SCHEMA,
  ...
  const NAString extLobMDV2name =
      genHBaseObjName(getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_LOB_COLUMNS, TRAF_RESERVED_NAMESPACE1);
  if (existsInHbase(extLobMDV2name, NULL) == 1) {
    str_sprintf(buf, "delete from %s.\"%s\".%s where table_uid = %ld ", getSystemCatalog(), SEABASE_MD_SCHEMA,
                SEABASE_LOB_COLUMNS, objUID);
    cliRC = cliInterface->executeImmediate(buf);
    if (cliRC < 0) {
      cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
      return -1;
  ...
  if (objType == COM_INDEX_OBJECT) {
    str_sprintf(buf, "delete from %s.\"%s\".%s where index_uid = %ld ", getSystemCatalog(), SEABASE_MD_SCHEMA,
                SEABASE_INDEXES, objUID);
    cliRC = cliInterface->executeImmediate(buf);
    if (cliRC < 0) {
      cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
  ...
  if (objType == COM_VIEW_OBJECT) {
    str_sprintf(buf, "delete from %s.\"%s\".%s where view_uid  = %ld ", getSystemCatalog(), SEABASE_MD_SCHEMA,
                SEABASE_VIEWS, objUID);
    cliRC = cliInterface->executeImmediate(buf);
    if (cliRC < 0) {
      cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
    ...
    str_sprintf(buf, "delete from %s.\"%s\".%s where using_view_uid  = %ld ", getSystemCatalog(), SEABASE_MD_SCHEMA,
                SEABASE_VIEWS_USAGE, objUID);
    cliRC = cliInterface->executeImmediate(buf);
    if (cliRC < 0) {
      cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
...
short CmpSeabaseDDL::deleteConstraintInfoFromSeabaseMDTables(ExeCliInterface *cliInterface, long tableUID,
  ...
  if (deleteFromSeabaseMDTable(cliInterface, constrCatName, constrSchName, constrObjName, constrType)) return -1;

  // delete data from table constraints MD
  str_sprintf(buf, "delete from %s.\"%s\".%s where table_uid  = %ld and constraint_uid = %ld", getSystemCatalog(),
              SEABASE_MD_SCHEMA, SEABASE_TABLE_CONSTRAINTS, tableUID, constrUID);
  cliRC = cliInterface->executeImmediate(buf);
  if (cliRC < 0) {
    cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
  ...
  str_sprintf(buf, "delete from %s.\"%s\".%s where ref_constraint_uid  = %ld and unique_constraint_uid = %ld",
              getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_REF_CONSTRAINTS, constrUID, otherConstrUID);
  cliRC = cliInterface->executeImmediate(buf);
  if (cliRC < 0) {
    cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
  ...
  str_sprintf(buf, "delete from %s.\"%s\".%s where unique_constraint_uid = %ld and foreign_constraint_uid = %ld",
              getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_UNIQUE_REF_CONSTR_USAGE, otherConstrUID, constrUID);
  cliRC = cliInterface->executeImmediate(buf);
  if (cliRC < 0) {
    cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
...
short CmpSeabaseDDL::checkAndGetStoredObjectDesc(ExeCliInterface *cliInterface, long objUID, TrafDesc **retDesc,
                                                 NABoolean checkStats, NAHeap *heap) {
  int cliRC = 0;

  NAString packedDesc;
  cliRC = getTextFromMD(cliInterface, objUID, COM_STORED_DESC_TEXT, 0, packedDesc, TRUE);
  if (cliRC < 0) {
    *CmpCommon::diags() << DgSqlCode(-4493) << DgString0("Error reading from metadata.");
  ...
  if (packedDesc.length() == 0) {
    ...
    *CmpCommon::diags() << DgSqlCode(-4493) << DgString0("Does not exist. It needs to be regenerated.");
  ...
  if (checkStats) {
    if ((!desc->tableDesc()->table_stats_desc || !desc->tableDesc()->table_stats_desc->tableStatsDesc()) ||
        ((!desc->tableDesc()->table_stats_desc->tableStatsDesc()->histograms_desc) &&
         (desc->tableDesc()->table_stats_desc->tableStatsDesc()->rowcount == -1))) {
      *CmpCommon::diags() << DgSqlCode(-4493) << DgString0("Stats do not exist and need to be generated.");
      ...
      *CmpCommon::diags() << DgSqlCode(4493) << DgString0("Full stats do not exist, fast stats current.");
    } else {
      *CmpCommon::diags() << DgSqlCode(4493) << DgString0("Stats uptodate and current.");
    }
  } else {
    // Check privs
    ...
    if (schemaPrivs == NULL || objectPrivs == NULL) {
      *CmpCommon::diags() << DgSqlCode(4493) << DgString0("Privileges do not exist and need to be regenerated.");
    }
...
short CmpSeabaseDDL::updateObjectRedefTime(ExeCliInterface *cliInterface, const NAString &catName,
                                           const NAString &schName, const NAString &objName, const char *objType,
                                           long rt, long objUID, NABoolean genDesc, NABoolean genStats,
                                           NABoolean flushData) {
  int retcode = 0;
  int cliRC = 0;
  ...
  long redefTime = ((rt == -1) ? NA_JulianTimestamp() : rt);
  ...
  long flags = 0;
  if ((objUID > 0) && (NOT isSeabaseReservedSchema(catName, schName)) &&
      (NOT((schName.length() >= strlen(COM_VOLATILE_SCHEMA_PREFIX)) &&
           (strncmp(schName.data(), COM_VOLATILE_SCHEMA_PREFIX, strlen(COM_VOLATILE_SCHEMA_PREFIX)) == 0))) &&
      ((strcmp(objType, COM_BASE_TABLE_OBJECT_LIT) == 0) || (strcmp(objType, COM_INDEX_OBJECT_LIT) == 0) ||
       (strcmp(objType, COM_VIEW_OBJECT_LIT) == 0) || (strcmp(objType, COM_PRIVATE_SCHEMA_OBJECT_LIT) == 0) ||
    ...
    if (rt != -2)  // update time
    {
      ...
      {
        str_sprintf(buf,
                    "update %s.\"%s\".%s set redef_time = %ld, flags = bitor(flags, %ld) where catalog_name = '%s' and "
                    "schema_name = '%s' and object_name = '%s' and object_type = '%s' ",
                    getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_OBJECTS, redefTime, flags, catName.data(),
                    quotedSchName.data(), quotedObjName.data(), objType);
      } else {
        str_sprintf(buf,
                    "update %s.\"%s\".%s set redef_time = %ld where catalog_name = '%s' and schema_name = '%s' and "
                    "object_name = '%s' and object_type = '%s' ",
                    getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_OBJECTS, redefTime, catName.data(),
                    quotedSchName.data(), quotedObjName.data(), objType);
      }
    } else  // not update time
    {
      ...
      {
        str_sprintf(buf,
                    "update %s.\"%s\".%s set flags = bitor(flags, %ld) where catalog_name = '%s' and schema_name = "
                    "'%s' and object_name = '%s' and object_type = '%s' ",
                    getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_OBJECTS, flags, catName.data(), quotedSchName.data(),
                    quotedObjName.data(), objType);
      }
    }
    ...
    if (genDesc) {
      ...
      cliRC = updateTextTableWithBinaryData(cliInterface, objUID, COM_STORED_DESC_TEXT, 0, (char *)ds, packedDescLen,
...
short CmpSeabaseDDL::updateObjectStats(ExeCliInterface *cliInterface, const NAString &catName, const NAString &schName,
                                       const NAString &objName, const char *objType, long rt, long objUID,
                                       NABoolean force, NABoolean flushData) {
  int retcode = 0;

  retcode = updateObjectRedefTime(cliInterface, catName, schName, objName, objType, rt, objUID, force,
                                  TRUE /*gen stats*/, flushData);

  return retcode;
}

short CmpSeabaseDDL::updateObjectValidDef(ExeCliInterface *cliInterface, const char *catName, const char *schName,
  ...
  str_sprintf(buf,
              "update %s.\"%s\".%s set valid_def = '%s' where catalog_name = '%s' and schema_name = '%s' and "
              "object_name = '%s' and object_type = '%s' ",
              getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_OBJECTS, validDef, catName, quotedSchName.data(),
              quotedObjName.data(), objType);
  cliRC = cliInterface->executeImmediate(buf);
  if (cliRC < 0) {
    cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
...
short CmpSeabaseDDL::updateObjectName(ExeCliInterface *cliInterface, long objUID, const char *catName,
                                      const char *schName, const char *objName) {
  int retcode = 0;
  int cliRC = 0;
  ...
  str_sprintf(
      buf,
      "update %s.\"%s\".%s set catalog_name = '%s', schema_name = '%s', object_name = '%s' where object_uid = %ld ",
      getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_OBJECTS, catName, quotedSchName.data(), quotedObjName.data(),
      objUID);
  cliRC = cliInterface->executeImmediate(buf);
  if (cliRC < 0) {
...
short CmpSeabaseDDL::updateObjectAuditAttr(ExeCliInterface *cliInterface, const char *catName, const char *schName,
  ...
  long objectUID = getObjectUID(cliInterface, catName, schName, objName, objType);

  if (objectUID < 0) return -1;
  str_sprintf(buf, "update %s.\"%s\".%s set is_audited = '%s' where  table_uid = %ld ", getSystemCatalog(),
              SEABASE_MD_SCHEMA, SEABASE_TABLES, (audited ? "Y" : "N"), objectUID);
  cliRC = cliInterface->executeImmediate(buf);
  if (cliRC < 0) {
    cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
...
short CmpSeabaseDDL::updateObjectFlags(ExeCliInterface *cliInterface, const long objUID, const long inFlags,
                                       NABoolean reset) {
  int retcode = 0;
  int cliRC = 0;
  ...
  long flags = inFlags;
  if (reset) flags = ~inFlags;

  if (reset)
    str_sprintf(buf, "update %s.\"%s\".%s set flags = bitand(flags, %ld) where object_uid = %ld", getSystemCatalog(),
                SEABASE_MD_SCHEMA, SEABASE_OBJECTS, flags, objUID);
  else
    str_sprintf(buf, "update %s.\"%s\".%s set flags = bitor(flags, %ld) where object_uid = %ld", getSystemCatalog(),
                SEABASE_MD_SCHEMA, SEABASE_OBJECTS, flags, objUID);
  cliRC = cliInterface->executeImmediate(buf);
  if (cliRC < 0) {
    cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
...
void CmpSeabaseDDL::cleanupObjectAfterError(ExeCliInterface &cliInterface, const NAString &catName,
                                            const NAString &schName, const NAString &objName,
                                            const ComObjectType objectType, const NAString *nameSpace,
  ...
  if (objectType == COM_BASE_TABLE_OBJECT)
    str_sprintf(buf, "cleanup table \"%s\".\"%s\".\"%s\" ", catName.data(), schName.data(), objName.data());
  else if (objectType == COM_INDEX_OBJECT)
    str_sprintf(buf, "cleanup index \"%s\".\"%s\".\"%s\" ", catName.data(), schName.data(), objName.data());
  else
    str_sprintf(buf, "cleanup object \"%s\".\"%s\".\"%s\" ", catName.data(), schName.data(), objName.data());
  ...
  long objDataUID;
  long indexUID = getObjectUID(&cliInterface, catName.data(), schName.data(), objName.data(), NULL, &objDataUID, NULL,
                               NULL, FALSE, FALSE);
  ...
  if (nameSpace && (NOT nameSpace->isNull())) {
    ...
    str_sprintf(buf, "cleanup object \"%s\" ", extNameForHbase.data());
...
int CmpSeabaseDDL::purgedataObjectAfterError(ExeCliInterface &cliInterface, const NAString &catName,
                                             const NAString &schName, const NAString &objName,
                                             const ComObjectType objectType, NABoolean dontForceCleanup) {
  int cliRC = 0;
  // if ddlXns are being used, no need of additional cleanup.
  // transactional rollback will take care of cleanup.
  if (dontForceCleanup) return cliRC;
  ...
  CmpCommon::diags()->clear();
  if (objectType == COM_BASE_TABLE_OBJECT)
    str_sprintf(buf, "purgedata \"%s\".\"%s\".\"%s\" ", catName.data(), schName.data(), objName.data());
  else if (objectType == COM_INDEX_OBJECT)
    str_sprintf(buf, "purgedata table(index_table \"%s\".\"%s\".\"%s\" ) ", catName.data(), schName.data(),
                objName.data());
  else
...
short CmpSeabaseDDL::processColFamily(NAString &inColFamily, NAString &trafColFamily,
                                      std::vector<NAString> *userColFamVec, std::vector<NAString> *trafColFamVec) {
  if (inColFamily.isNull()) return 0;
  ...
  if (trafColFamVec) {
    ...
    if (not found) trafColFamVec->push_back(trafColFamily);
  }

  return 0;
}

short CmpSeabaseDDL::buildColInfoArray(ComObjectType objType, NABoolean isMetadataHistOrReposObject,
                                       ElemDDLColDefArray *colArray, ComTdbVirtTableColumnInfo *colInfoArray,
                                       NABoolean implicitPK, NABoolean alignedFormat,
                                       int *identityColPos,                   // IN_OUT
  ...
  for (index = 0; index < colArray->entries(); index++) {
    ...
    ComLobsStorageType lobStorage;
    NAString compDefnStr;
    if (getColInfo(colNode, isMetadataHistOrReposObject, colFamily, colName, alignedFormat, datatype, length, precision,
    ...
    if ((identityColPos) &&
        ((defaultClass == COM_IDENTITY_GENERATED_BY_DEFAULT) || (defaultClass == COM_IDENTITY_GENERATED_ALWAYS))) {
      if ((objType == COM_BASE_TABLE_OBJECT) && (*identityColPos >= 0))  // previously found
      {
    ...
    colInfoArray[index].hbaseColFam = new ((heap ? heap : STMTHEAP)) char[storedColFamily.length() + 1];
    strcpy((char *)colInfoArray[index].hbaseColFam, (char *)storedColFamily.data());

    char idxNumStr[40];
    str_itoa(index + 1, idxNumStr);

    colInfoArray[index].hbaseColQual = new ((heap ? heap : STMTHEAP)) char[strlen(idxNumStr) + 1];
    ...
    if (!compDefnStr.isNull()) {
      char *comp_str = new ((heap ? heap : STMTHEAP)) char[compDefnStr.length() + 1];
      str_cpy_all(comp_str, (char *)compDefnStr.data(), compDefnStr.length());
      comp_str[compDefnStr.length()] = 0;
      colInfoArray[index].compDefnStr = comp_str;
    }
  }  // for

  if ((objType == COM_BASE_TABLE_OBJECT) || (objType == COM_VIEW_OBJECT)) {
    ...
    if (it != myvector.end()) {
      *CmpCommon::diags() << DgSqlCode(-1080) << DgColumnName(*it);
      return -1;
    }
  }
...
short CmpSeabaseDDL::buildColInfoArray(ElemDDLParamDefArray *paramArray, ComTdbVirtTableColumnInfo *colInfoArray) {
  size_t index = 0;
  for (index = 0; index < paramArray->entries(); index++) {
    ElemDDLParamDef *paramNode = (*paramArray)[index];
    ElemDDLColDef colNode(NULL, &paramNode->getParamName(), paramNode->getParamDataType(), NULL, STMTHEAP);
    ...
    ComLobsStorageType lobStorage;
    NAString compDefnStr;
    if (getColInfo(&colNode, FALSE, colFamily, colName, FALSE, datatype, length, precision, scale, dt_start, dt_end,
                   upshifted, nullable, charset, colClass, defaultClass, defVal, heading, lobStorage, compDefnStr,
                   hbaseColFlags, colFlags))
      return -1;
...
short CmpSeabaseDDL::buildKeyInfoArray(ElemDDLColDefArray *colArray, NAColumnArray *nacolArray,
                                       ElemDDLColRefArray *keyArray, ComTdbVirtTableColumnInfo *colInfoArray,
                                       ComTdbVirtTableKeyInfo *keyInfoArray, NABoolean allowNullableUniqueConstr,
  ...
  for (index = 0; index < keyArray->entries(); index++) {
    char *col_name = new ((heap ? heap : STMTHEAP)) char[strlen((*keyArray)[index]->getColumnName()) + 1];
    ...
    if ((!colArray) && (!nacolArray)) {
      ...
      *CmpCommon::diags() << DgSqlCode(-1009) << DgColumnName(keyInfoArray[index].colName);

      return -1;
    }

    NAString nas((*keyArray)[index]->getColumnName());
    keyInfoArray[index].tableColNum = (int)(colArray ? colArray->getColumnIndex((*keyArray)[index]->getColumnName())
    ...
    if (keyColList.contains(col_name)) {
      *CmpCommon::diags() << DgSqlCode(-1080) << DgColumnName(keyInfoArray[index].colName);
    ...
    keyInfoArray[index].ordering =
        ((*keyArray)[index]->getColumnOrdering() == COM_ASCENDING_ORDER ? ComTdbVirtTableKeyInfo::ASCENDING_ORDERING
    ...
    if ((colInfoArray) && (colInfoArray[keyInfoArray[index].tableColNum].nullable != 0) &&
        (NOT allowNullableUniqueConstr) && (NOT bAllowNullableFunc)) {
    ...
    if (keyLength) {
      NAType *colType = (*colArray)[keyInfoArray[index].tableColNum]->getColumnDataType();
...
short CmpSeabaseDDL::updateTextTable(ExeCliInterface *cliInterface, long objUID, ComTextType textType, int subID,
                                     NAString &textInputData, char *binaryInputData, int binaryInputDataLen,
  ...
  int dataLen = -1;
  if (binaryInputData) {
    int encodedMaxLen = str_encoded_len(binaryInputDataLen);
    char *encodedData = new (STMTHEAP) char[encodedMaxLen];
    int encodedLen = str_encode(encodedData, encodedMaxLen, binaryInputData, binaryInputDataLen);
  ...
  for (int i = 0; i < numRows; i++) {
    ...
    str_sprintf(
        queryBuf,
        "insert into %s.\"%s\".%s values (%ld, %d, %d, %d, 0, cast(? as char(%d bytes) character set utf8 not null))",
        getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_TEXT, objUID, textType, subID, i, currDataLen);
...
short CmpSeabaseDDL::updateTextTableWithBinaryData(ExeCliInterface *cliInterface, long objUID, ComTextType textType,
                                                   int subID, char *inputData, int inputDataLen, NABoolean withDelete) {
  NAString dummy;
  return updateTextTable(cliInterface, objUID, textType, subID, dummy, inputData, inputDataLen, withDelete);
}

short CmpSeabaseDDL::deleteFromTextTable(ExeCliInterface *cliInterface, long objUID, ComTextType textType, int subID) {
  int cliRC = 0;

  char buf[1000];
  str_sprintf(buf, "delete from %s.\"%s\".%s where text_uid = %ld and text_type = %d and sub_id = %d",
              getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_TEXT, objUID, static_cast<int>(textType), subID);
  cliRC = cliInterface->executeImmediate(buf);
...
ItemExpr *CmpSeabaseDDL::bindDivisionExprAtDDLTime(ItemExpr *expr, NAColumnArray *availableCols, NAHeap *heap) {
  ...
  for (CollIndex c = 0; c < nc; c++) {
    ItemExpr *boundChild = bindDivisionExprAtDDLTime(expr->child(c), availableCols, heap);
  ...
  if (retval) switch (expr->getOperatorType()) {
      case ITM_REFERENCE: {
        ...
        for (CollIndex c = 0; c < availableCols->entries() && colType == NULL; c++)
          if (colName == (*availableCols)[c]->getColName()) colType = (*availableCols)[c]->getType();

        if (colType) {
          retval = new (heap) NamedTypeToItem(colName.data(), colType->newCopy(heap), heap);
        } else {
          // column not found
          NAString unparsed;
...
short CmpSeabaseDDL::validateDivisionByExprForDDL(ItemExpr *divExpr) {
  ...
  switch (divExpr->getOperatorType()) {
    ...
    case ITM_EXTRACT_ODBC:  // for YEAR
    {
      ...
      if (ef == REC_DATE_YEAR || ef == REC_DATE_YEARQUARTER_EXTRACT || ef == REC_DATE_YEARMONTH_EXTRACT ||
          ef == REC_DATE_YEARQUARTER_D_EXTRACT || ef == REC_DATE_YEARMONTH_D_EXTRACT) {
        ...
        if (divExpr->child(0)->getOperatorType() != leafColType) {
          if ((divExpr->child(0)->getOperatorType() == ITM_PLUS || divExpr->child(0)->getOperatorType() == ITM_MINUS) &&
            ...
            if (addedValue->getOperatorType() == ITM_CAST) addedValue = addedValue->child(0);
    ...
    case ITM_DIVIDE: {
      ...
      if (topLevelCast) {
        topLevelCastIsOk = (topLevelCast->getValueId().getType().getTypeQualifier() == NA_NUMERIC_TYPE);
      }
    } break;
    ...
    case ITM_LEFT: {
      ...
      if (divExpr->child(0)->getOperatorType() != leafColType) {
        if (divExpr->child(0)->getOperatorType() == ITM_CAST &&
          ...
          // alter the data type
          const CharType &tgtType = (const CharType &)divExpr->child(0)->getValueId().getType();
          const CharType &srcType = (const CharType &)divExpr->child(0)->child(0)->getValueId().getType();

          if (NOT(srcType.getTypeQualifier() == NA_CHARACTER_TYPE && tgtType.getTypeQualifier() == NA_CHARACTER_TYPE &&
                  srcType.getCharSet() == tgtType.getCharSet() && srcType.getFSDatatype() == tgtType.getFSDatatype() &&
      ...
      if (exprIsValid) {
        if (divExpr->getOperatorType() == ITM_LEFT) {
          ...
          if (child1 && child1->canGetExactNumericValue()) child1Value = child1->getExactNumericValue();

          if (child1Value != 1 OR divExpr->getArity() != 3) {
            exprIsValid = FALSE;
            unsupportedExpr = divExpr;
  ...
  if (topLevelCast && !topLevelCastIsOk) {
    exprIsValid = FALSE;
    if (!missingConstHere && !missingColHere && !unsupportedExpr) unsupportedExpr = topLevelCast;
  }
  ...
  if (origDivType.getTypeQualifier() == NA_NUMERIC_TYPE && !((NumericType &)origDivType).isExact()) {
...
short CmpSeabaseDDL::validatePartitionByExpr(ElemDDLPartitionV2Array *partitionV2Array, TrafDesc *colDescs,
  ...
  for (CollIndex i = 0; i < partitionV2Array->entries(); ++i) {
    ...
    if (cva.entries() != partColCnt) {
      ...
      *CmpCommon::diags() << DgSqlCode(-1129) << DgString0(str1);
      return -1;
    }
    ...
    for (CollIndex k = 0; k < cva.entries(); ++k) {
      ...
      if (cva[k]->getOperatorType() == ITM_CONSTANT) {
        if (((ConstValue *)(cva[k]))->isNull()) continue;
    ...
    retcode = encodeKeyValues(colDescs, partitionColDescs,
                              pinputStrings,      // INPUT
                              0,                  // isIndex
...
process_return:
  if (encodedKeysBuffer) NADELETEBASIC(encodedKeysBuffer, STMTHEAP);
  if (pinputStrings) NADELETEARRAY(pinputStrings, partColCnt, NAStringPtr, STMTHEAP);
    ...
    if (i + 1 < partCnt) {
      ...
      for (int j = 0; j < colCnt; j++) {
        ...
        if (currValueArray[j]->getOperatorType() == ITM_CONSTANT)
          if (((ConstValue *)currValueArray[j])->isNull()) currIsMaxvalue = true;
...
short CmpSeabaseDDL::createEncodedKeysBuffer(char **&encodedKeysBuffer, int &numSplits, TrafDesc *colDescs,
                                             TrafDesc *keyDescs, int numSaltPartitions, int numSaltSplits,
                                             NAString *splitByClause, Int16 numTrafReplicas, int numKeys, int keyLength,
                                             NABoolean isIndex) {
  ...
  if (!usesSplitBy && !usesSaltSplits && !usesTrafReplicas) return 0;

  // make a list of NAStrings with the default values for start keys
  NAString **defaultSplits = createInArrayForLowOrHighKeys(colDescs, keyDescs, numKeys, FALSE, isIndex, STMTHEAP);
  ...
  if (usesSplitBy) {
    ...
    ElemDDLPartitionClause *rangeSplits =
        parser.parseSplitDefinition(splitByClause->data(), splitByClause->length(), CharInfo::UTF8);
    ...
    const ElemDDLColRefArray &splitByColRefs = splitByCols->getPartitionKeyColumnArray();
    ...
    if (numKeys < numSplitByCols) {
      ...
      *CmpCommon::diags() << DgSqlCode(-1209) << DgInt0(numKeys);
      return -1;
    }
    ...
    for (CollIndex c = 0; c < numSplitByCols; c++) {
      ...
      while (cd->columnsDesc()->colnumber != tableColNum && cd->next) cd = cd->next;

      if (splitByColRefs[c]->getColumnName() != cd->columnsDesc()->colname) {
        // SPLIT BY column is not the next key column in sequence
        *CmpCommon::diags() << DgSqlCode(-1210) << DgString0(splitByColRefs[c]->getColumnName()) << DgInt0(c + 1)
                            << DgString1(cd->columnsDesc()->colname);
        return -1;
      }

      NABoolean keyColIsAscending = (keyColDesc->keysDesc()->isDescending() == FALSE);
      NABoolean splitByColIsAscending = (splitByColRefs[c]->getColumnOrdering() != COM_DESCENDING_ORDER);
    ...
    pPartitionList = rangeSplits->getPartitionDefBody()->castToElemDDLPartitionList();
    if (!pPartitionList) pPartitionRange = rangeSplits->getPartitionDefBody()->castToElemDDLPartitionRange();
    ...
    for (int k = 0; k < numKeys; k++) splitValuesAsText[k] = new (STMTHEAP) NAString(STMTHEAP);
  } else {
    ...
    for (int k = startKeyNum; k < numKeys; k++) splitValuesAsText[k] = defaultSplits[k];
  }

  // allocate the result buffers, numSplits buffers of length keyLength
  encodedKeysBuffer = new (STMTHEAP) char *[numSplits];
  for (int i = 0; i < numSplits; i++) encodedKeysBuffer[i] = new (STMTHEAP) char[keyLength];
  ...
  for (int i = 0; i < numSplits; i++) {
    if (usesSplitBy) {
      ...
      const ItemConstValueArray &cva = pPartitionRange->getKeyValueArray();

      if (cva.entries() > numSplitByCols) {
        *CmpCommon::diags() << DgSqlCode(-1216) << DgInt0(i + 1);
        return -1;
      }

      int v;
      // copy the values specified in SPLIT BY ... ADD PARTITION
      for (v = 0; v < cva.entries(); v++) *(splitValuesAsText[v]) = cva[v]->getConstStr(FALSE);

      // fill up to numKeys with default values
      for (; v < numKeys; v++) *(splitValuesAsText[v]) = *(defaultSplits[v]);
    } else {
      if (usesTrafReplicas && !usesSaltSplits) {
      ...
      */
        snprintf(splitNumCharStr, sizeof(splitNumCharStr), "%d", ((i + 1) * numSaltPartitions) / (numSaltSplits + 1));
...
\x00\x01\x00\x00\x00\x03\x00\x00\x00\x00                                                         (1,3) */
    ...
    retVal = encodeKeyValues(colDescs, keyDescs,
    ...
    if (i > 0 && memcmp(encodedKeysBuffer[i - 1], encodedKeysBuffer[i], keyLength) >= 0) {
      *CmpCommon::diags() << DgSqlCode(-1211) << DgInt0(i + 1);
      return -1;
    }
...
short CmpSeabaseDDL::dropSeabaseObject(ExpHbaseInterface *ehi, const NAString &objName, NAString &currCatName,
                                       NAString &currSchName, const ComObjectType objType, NABoolean ddlXns,
                                       NABoolean dropFromMD, NABoolean dropFromStorage, NABoolean isMonarch,
                                       NAString nameSpace, NABoolean updPrivs, long *objectUID,
                                       NAString **objDataUIDName, Int16 recoderHbName) {
  int retcode = 0;
  ...
  if (dropFromMD) {
    if (isAuthorizationEnabled()) {
      ...
      if ((ComIsTrafodionExternalSchemaName(schemaNamePart, &isHive)) && (isHive)) {
        ...
        hiveNativeName = ComConvertTrafNameToNativeName(catalogNamePart, quotedSchemaName, objectNamePart);
      ...
      if (objUID < 0) {
        objUID = getObjectInfo(&cliInterface, catalogNamePart.data(), schemaNamePart.data(), objectNamePart.data(),
                               objType, objOwnerID, schemaOwnerID, objectFlags, objDataUID);
      }
      if (objUID < 0 || objOwnerID == 0) {  // TODO: Internal error?
        return -1;
      }
      ...
      if (updPrivs)
        if (!deletePrivMgrInfo(extTableName, objUID, objType)) return -1;
    }

    if (deleteFromSeabaseMDTable(&cliInterface, catalogNamePart, schemaNamePart, objectNamePart, objType)) return -1;
  }  // dropFromMD

  NAString extNameForHbase = genHBaseObjName(catalogNamePart, schemaNamePart, objectNamePart, nameSpace, objDataUID);
...
short CmpSeabaseDDL::dropSeabaseStats(ExeCliInterface *cliInterface, const char *catName, const char *schName,
                                      long tableUID) {
  int cliRC = 0;
  char buf[4000];
  ...
  if ((strcmp(catName, HIVE_STATS_CATALOG) == 0) && (strcmp(schName, HIVE_STATS_SCHEMA_NO_QUOTES) == 0)) {
    long objUID = getObjectUID(cliInterface, catName, schName, SEABASE_SCHEMA_OBJECTNAME, COM_PRIVATE_SCHEMA_OBJECT_LIT,
  ...
  str_sprintf(buf, "delete from %s.\"%s\".%s where table_uid = %ld", catName, schName, HBASE_HIST_NAME, tableUID);
  ...
  if (cliRC != -4082) {
    ...
    str_sprintf(buf, "delete from %s.\"%s\".%s where table_uid = %ld", catName, schName, HBASE_HISTINT_NAME, tableUID);
    cliRC = cliInterface->executeImmediate(buf);
  ...
  str_sprintf(buf,
              "select translate(sample_name using ucs2toutf8) from "
...
short CmpSeabaseDDL::updateSeabaseVersions(ExeCliInterface *cliInterface, const char *sysCat, int majorVersion) {
  int cliRC = 0;
  char buf[4000];

  str_sprintf(buf, "delete from %s.\"%s\".%s ", sysCat, SEABASE_MD_SCHEMA, SEABASE_VERSIONS);
  ...
  long initTime = NA_JulianTimestamp();

  str_sprintf(buf,
              "insert into %s.\"%s\".%s values ('METADATA', %d, %d, %ld, 'initialize trafodion'), ('DATAFORMAT', %d, "
              "%d, %ld, 'initialize trafodion'), ('SOFTWARE', %d, %d, %ld, 'initialize trafodion') ",
              sysCat, SEABASE_MD_SCHEMA, SEABASE_VERSIONS, (majorVersion != -1 ? majorVersion : METADATA_MAJOR_VERSION),
              (METADATA_MINOR_VERSION * VERSION_MULTIPLE_LARGE + METADATA_UPDATE_VERSION), initTime,
              DATAFORMAT_MAJOR_VERSION, DATAFORMAT_MINOR_VERSION, initTime, SOFTWARE_MAJOR_VERSION,
...
short CmpSeabaseDDL::updateSeabaseAuths(ExeCliInterface *cliInterface, const char *sysCat) {
  int cliRC = 0;
  char buf[4000];

  str_sprintf(buf, "delete from %s.\"%s\".%s ", sysCat, SEABASE_MD_SCHEMA, SEABASE_AUTHS);
  ...
  if (CmpCommon::diags()->getNumber(DgSqlCode::ERROR_)) return -1;
...
short CmpSeabaseDDL::createNamespace(ExpHbaseInterface *ehi, NAString &ns, NABoolean createReservedNS) {
  short retcode = 0;
  ...
  if (ns.length() >= strlen(TRAF_RESERVED_NAMESPACE_PREFIX)) {
    ...
    rns.toUpper();
    if (rns == TRAF_RESERVED_NAMESPACE_PREFIX) reservedNS = TRUE;

    if (ns == TRAF_RESERVED_NAMESPACE5) reservedNS5 = TRUE;
  }

  if ((NOT createReservedNS) && (reservedNS)) return 0;

  if ((createReservedNS) && (reservedNS5)) return 0;

  retcode = ehi->namespaceOperation(COM_CHECK_NAMESPACE_EXISTS, ns.data(), 0, NULL, NULL, NULL);
  if (retcode == -HBC_ERROR_NAMESPACE_NOT_EXIST) {
    retcode = ehi->namespaceOperation(COM_CREATE_NAMESPACE, ns.data(), 0, NULL, NULL, NULL);
    if (retcode < 0) {
      *CmpCommon::diags() << DgSqlCode(-8448) << DgString0((char *)"ExpHbaseInterface::namespaceOperation()")
                          << DgString1(getHbaseErrStr(-retcode)) << DgInt0(-retcode)
                          << DgString2((char *)GetCliGlobals()->getJniErrorStr());
...
short CmpSeabaseDDL::dropCreateReservedNamespaces(ExpHbaseInterface *ehi, NABoolean drop, NABoolean create) {
  short retcode = 0;

  // drop existing reserved namespace
  if (drop) {
    for (int i = 1; i <= NUM_RESERVED_NAMESPACE_SCHEMAS; i++) {
      ...
      if (!str_cmp(name.data(), TRAF_RESERVED_NAMESPACE5, name.length())) continue;

      retcode = ehi->namespaceOperation(COM_CHECK_NAMESPACE_EXISTS, name.data(), 0, NULL, NULL, NULL);
      if (retcode == 0)  // namespace exists
      {
        ehi->namespaceOperation(COM_DROP_NAMESPACE, name.data(), 0, NULL, NULL, NULL);
      }
    }  // for
  }
  ...
  if (create) {
    for (int i = 1; i <= NUM_RESERVED_NAMESPACE_SCHEMAS; i++) {
      ...
      if (createNamespace(ehi, name, TRUE)) return -1;
...
short dropTrafNamespaces(ExpHbaseInterface *ehi) {
  ...
  retcode = ehi->namespaceOperation(COM_GET_NAMESPACES, NULL, 0, NULL, NULL, &namespaceObjects);

  if ((retcode) || (!namespaceObjects) || (namespaceObjects->entries() == 0)) return 0;

  for (int i = 0; i < namespaceObjects->entries(); i++) {
    ...
    if (nameSpace.length() >= TRAF_NAMESPACE_PREFIX_LEN &&
        (strncmp(nameSpace.data(), TRAF_NAMESPACE_PREFIX, TRAF_NAMESPACE_PREFIX_LEN) == 0))
      isTraf = TRUE;

    if (NOT isTraf) continue;

    if (nameSpace == TRAF_RESERVED_NAMESPACE5) continue;

    retcode = ehi->namespaceOperation(COM_CHECK_NAMESPACE_EXISTS, nameSpace.data(), 0, NULL, NULL, NULL);
    if (retcode == 0)  // namespace exists
    {
      ehi->namespaceOperation(COM_DROP_NAMESPACE, nameSpace.data(), 0, NULL, NULL, NULL);
    }
  }
...
//////////////////////////////////////////////////////////////////////////////
short CmpSeabaseDDL::upgradeNamespace(ExeCliInterface *cliInterface) {
  int cliRC = 0;
  ...
  HbaseStr versTgt;
  NAString newVers = genHBaseObjName(getSystemCatalog(), NAString(SEABASE_MD_SCHEMA), NAString(SEABASE_VERSIONS),
                                     NAString(TRAF_RESERVED_NAMESPACE1));
  ...
  NAString hbaseResVersionsStr = genHBaseObjName(getSystemCatalog(), NAString(SEABASE_MD_SCHEMA),
  ...
  Queue *nsObjs = NULL;
  cliRC = cliInterface->fetchAllRows(nsObjs, buf, 0, FALSE, FALSE, TRUE);
  if (cliRC < 0) {
    cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
    return -1;
  }
  ...
  str_sprintf(buf,
              ...
              "or O.schema_name like 'VOLATILE|_SCHEMA|_%%' escape '|') "
              " and O.object_type in ( 'BT', 'IX' ) ",
              TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_TEXT, COM_OBJECT_NAMESPACE, TRAFODION_SYSCAT_LIT,
              SEABASE_MD_SCHEMA, SEABASE_OBJECTS, TRAFODION_SYSCAT_LIT);
  ...
  cliRC = copyHbaseTable(ehi, &ecSrc, &ecTgt, TRUE /*remove tgt before copy*/);
  if (cliRC < 0) {
    *CmpCommon::diags() << DgSqlCode(-8448) << DgString0((char *)"ExpHbaseInterface::copy()")
                        << DgString1(getHbaseErrStr(-cliRC)) << DgInt0(-cliRC)
                        << DgString2((char *)GetCliGlobals()->getJniErrorStr());
  ...
  cliRC = copyHbaseTable(ehi, &versSrc, &versTgt, TRUE /*remove tgt before copy*/);
  if (cliRC < 0) {
    *CmpCommon::diags() << DgSqlCode(-8448) << DgString0((char *)"ExpHbaseInterface::copy()")
                        << DgString1(getHbaseErrStr(-cliRC)) << DgInt0(-cliRC)
                        << DgString2((char *)GetCliGlobals()->getJniErrorStr());
  ...
  for (size_t i = 0; i < nsObjs->numEntries(); i++) {
    ...
    if (cliRC < 0) {
      if (endXnIfStartedHere(cliInterface, xnWasStartedHere, -1) < 0) return -1;
...
label_error:
  ...
  str_sprintf(buf,
              "delete from %s.\"%s\".%s "
              "where text_type = %d",
              TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_TEXT, COM_OBJECT_NAMESPACE);
...
short CmpSeabaseDDL::createDefaultSystemSchema(ExeCliInterface *cliInterface) {
  ...
  str_sprintf(buf, "create shared schema " TRAFODION_SYSTEM_CATALOG "." SEABASE_SYSTEM_SCHEMA
                   " namespace '" TRAF_RESERVED_NAMESPACE3 "' no encryption ");
...
short CmpSeabaseDDL::createLibrariesObject(ExeCliInterface *cliInterface) {
  ...
  str_sprintf(
      buf,
      "create table %s.\"%s\".LIBRARIES (library_uid largeint not null not serialized,library_filename varchar(512) "
      "character set iso88591 not null not serialized,library_storage  blob, version int not null not serialized, "
      "flags largeint not null not serialized)  primary key (library_uid) attribute hbase format ,namespace %s; ",
      getSystemCatalog(), SEABASE_MD_SCHEMA, TRAF_RESERVED_NAMESPACE1);
...

{
  ...
  str_sprintf(buf, "SELECT DISTINCT TRIM(CATALOG_NAME), TRIM(SCHEMA_NAME) FROM %s.%s ", getMDSchema(), SEABASE_OBJECTS);
  ...
  for (size_t s = 0; s < schemaNames.size(); s++) {
    ...
    if (existsInSeabaseMDTable(cliInterface, schemaEntry.catalogName, schemaEntry.schemaName,
    ...
    if (addSchemaObject(*cliInterface, schemaName, COM_SCHEMA_CLASS_SHARED, SUPER_USER, FALSE, NAString(""), FALSE,
                        FALSE, FALSE, FALSE)) {
...
// ----------------------------------------------------------------------------
short CmpSeabaseDDL::createPrivMgrRepos(ExeCliInterface *cliInterface, NABoolean ddlXns) {
  std::vector<std::string> tablesCreated;

  if (initSeabaseAuthorization(cliInterface, ddlXns, FALSE /*isUpgrade*/, tablesCreated) < 0) return -1;

  return 0;
}

void CmpSeabaseDDL::createSeabaseSequence(StmtDDLCreateSequence *createSequenceNode, NAString &currCatName,
  ...
  if (isSeabaseReservedSchema(seqName)) {
    *CmpCommon::diags() << DgSqlCode(-CAT_CREATE_TABLE_NOT_ALLOWED_IN_SMD) << DgTableName(extSeqName);
    processReturn();
    return;
  }

  retcode = existsInSeabaseMDTable(&cliInterface, catalogNamePart, schemaNamePart, seqNamePart);
  if (retcode < 0) {
    processReturn();
  ...
  if (retcode == 1)  // already exists
  {
    if (NOT createSequenceNode->ifNotExistsSet()) *CmpCommon::diags() << DgSqlCode(-1390) << DgString0(extSeqName);
  ...
  str_sprintf(buf, "insert into %s.\"%s\".%s values ('%s', '%s', '%s', '%s', %ld, %ld, %ld, '%s', '%s', %d, %d, 0)",
              ...
              COM_YES_LIT, COM_NO_LIT, objectOwnerID, schemaOwnerID);
  cliRC = cliInterface.executeImmediate(buf);
  ...
  */
  ...
  if (sgo->getReplType() == COM_REPL_SYNC)
    ...
    CmpSeabaseDDL::setMDflags(flags, MD_SEQGEN_REPL_ASYNC_FLG);
  str_sprintf(
      buf, "insert into %s.\"%s\".%s values ('%s', %ld, %d, %ld, %ld, %ld, %ld, '%s', %ld, %ld, %ld, %ld, %ld, %ld)",
      getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_SEQ_GEN, sgoTypeLit.data(), seqObjUID,
      sgo->getFSDataType(),  // REC_BIN64_SIGNED,
      sgo->getStartValue(), sgo->getIncrement(), sgo->getMaxValue(), sgo->getMinValue(),
      (sgo->getCycle() ? COM_YES_LIT : COM_NO_LIT), sgo->getCache(), sgo->getStartValue(), 0L, createTime, 0L, flags);
  ...
  if (order_option) {
    ...
    ExpHbaseInterface *ehi = allocEHI(COM_STORAGE_HBASE);
    int ret = ehi->getNextValue(tabName, rowId, famName, qualName, sgo->getStartValue(), nextVal, skipWAL);
    if (ret < 0) {
      deallocEHI(ehi);
      *CmpCommon::diags() << DgSqlCode(-1574) << DgString0(extSeqName);
      processReturn();
      return;
    }
    deallocEHI(ehi);
    ...
    if (sgo->getReplType() == COM_REPL_ASYNC) {
      long cache = (sgo->getCache() == 0) ? 1 : sgo->getCache();
      long nextVal = sgo->getStartValue() + sgo->getIncrement() * cache;
      nextVal = (nextVal > sgo->getMaxValue()) ? sgo->getMaxValue() : nextVal;
      str_sprintf(buf, "upsert into %s.\"%s\".%s values('%s', '%s', '%s', %ld);", getSystemCatalog(),
                  SEABASE_XDC_MD_SCHEMA, XDC_SEQUENCE_TABLE, catalogNamePart.data(), quotedSchName.data(),
  ...
  if (!insertPrivMgrInfo(seqObjUID, extSeqName, COM_SEQUENCE_GENERATOR_OBJECT, objectOwnerID, schemaOwnerID,
                         ComUser::getCurrentUser())) {
    *CmpCommon::diags() << DgSqlCode(-CAT_UNABLE_TO_GRANT_PRIVILEGES) << DgTableName(extSeqName);
  ...
  if (sgo->getReplType() == COM_REPL_ASYNC && sgo->getSGType() == SG_EXTERNAL) {
    cliRC = updateSeabaseXDCDDLTable(&cliInterface, catalogNamePart.data(), schemaNamePart.data(), seqNamePart.data(),
  ...
  ActiveSchemaDB()->getNATableDB()->removeNATable(cn, ComQiScope::REMOVE_FROM_ALL_USERS, COM_SEQUENCE_GENERATOR_OBJECT,
                                                  createSequenceNode->ddlXns(), FALSE);

  return;
}

void CmpSeabaseDDL::alterSeabaseSequence(StmtDDLCreateSequence *alterSequenceNode, NAString &currCatName,
                                         NAString &currSchName) {
  int cliRC;
  int retcode;
  ...
  if (retcode == 0)  // does not exist
  {
    ...
    *CmpCommon::diags() << DgSqlCode(-1389) << DgString0(extSeqName);
  ...
  long seqUID = getObjectInfo(&cliInterface, catalogNamePart.data(), schemaNamePart.data(), seqNamePart.data(),
                              COM_SEQUENCE_GENERATOR_OBJECT, objectOwnerID, schemaOwnerID, objectFlags, objDataUID);
  ...
  // Verify that the current user has authority to perform operation
  if (!isDDLOperationAuthorized(SQLOperation::ALTER_SEQUENCE, objectOwnerID, schemaOwnerID, seqUID,
  ...
  if (sgo->isRestartValueSpecified()) {
    str_sprintf(tmpBuf, " start_value = %ld, next_value = %ld,", sgo->getStartValue(), sgo->getStartValue());
  ...
  str_sprintf(buf,
              "select t.seq_type, t.flags from (update %s.\"%s\".%s %s where seq_uid = %ld return \"NEW\".seq_type, "
              "\"NEW\".flags) t(seq_type, flags);",
              getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_SEQ_GEN, setOptions, seqUID);
  ...
  if (isMDflagsSet(seqFlags, MD_SEQGEN_REPL_ASYNC_FLG) && sgType == COM_EXTERNAL_SG) {
    cliRC = updateSeabaseXDCDDLTable(&cliInterface, catalogNamePart.data(), schemaNamePart.data(), seqNamePart.data(),
                                     COM_SEQUENCE_GENERATOR_OBJECT_LIT);
    if (cliRC < 0) {
      cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
  ...
  if (updateObjectRedefTime(&cliInterface, catalogNamePart, schemaNamePart, seqNamePart,
                            COM_SEQUENCE_GENERATOR_OBJECT_LIT, redefTime, -1)) {
    processReturn();
  ...
  if (NOT sgo->isNextValSpecified())
    ActiveSchemaDB()->getNATableDB()->removeNATable(cn, ComQiScope::REMOVE_FROM_ALL_USERS,
                                                    COM_SEQUENCE_GENERATOR_OBJECT, alterSequenceNode->ddlXns(), FALSE);
  ...
  if (sgo->isNextValSpecified() && sgo->isOrder()) {
    ...
    long nextVal = 0, newNextValue = 0;

    ExpHbaseInterface *ehi = allocEHI(COM_STORAGE_HBASE);
    int ret = ehi->getNextValue(tabName, rowId, famName, qualName, 0, nextVal, 1);  // get current value
    if (ret < 0) goto error;
    ret = ehi->getNextValue(tabName, rowId, famName, qualName, sgo->getNextVal() - nextVal - sgo->getIncrement(),
                            newNextValue, 0);
    if (ret < 0) goto error;
    deallocEHI(ehi);
    return;
...
void CmpSeabaseDDL::dropSeabaseSequence(StmtDDLDropSequence *dropSequenceNode, NAString &currCatName,
                                        NAString &currSchName) {
  int cliRC = 0;
  int retcode = 0;
  ...
  if (isAuthorizationEnabled()) {
    ...
    if (!isDDLOperationAuthorized(SQLOperation::DROP_SEQUENCE, objectOwnerID, schemaOwnerID, seqUID,
                                  COM_SEQUENCE_GENERATOR_OBJECT, NULL)) {
      *CmpCommon::diags() << DgSqlCode(-CAT_NOT_AUTHORIZED);
    ...
    if (!deletePrivMgrInfo(objectNamePart, seqUID, COM_SEQUENCE_GENERATOR_OBJECT)) {
  ...
  if (deleteFromSeabaseMDTable(&cliInterface, catalogNamePart, schemaNamePart, objectNamePart,
  ...
  if (naTable) {
    ...
    if (orderOption) {
      long seqUID = naTable->getSGAttributes()->getSGObjectUID().get_value();
      ExpHbaseInterface *ehi = allocEHI(COM_STORAGE_HBASE);
      NAString tabName(ORDER_SEQ_MD_TABLE);
      NAString rowId = Int64ToNAString(seqUID);
  ...
  if (naTable && naTable->getSGAttributes()->getSGAsyncRepl() &&
  ...
  if (naTable) {
    ...
    if (naTable->getSGAttributes()->getSGAsyncRepl()) {
      ...
      str_sprintf(buf, "delete from %s.\"%s\".%s where catalog_name = '%s' and schema_name = '%s' and seq_name = '%s'",
                  getSystemCatalog(), SEABASE_XDC_MD_SCHEMA, XDC_SEQUENCE_TABLE, catalogNamePart.data(),
                  quotedSchName.data(), quotedObjName.data());
      cliRC = cliInterface.executeImmediate(buf);
      if (cliRC < 0) {
        cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
      }
...
short CmpSeabaseDDL::processNamespaceOperations(StmtDDLNamespace *ns) {
  ...
  if ((reservedName) &&
      ((oper == COM_CREATE_NAMESPACE) || (oper == COM_DROP_NAMESPACE) || (oper == COM_ALTER_NAMESPACE)) &&
      (!Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL))) {
    // cannot create/drop/alter metadata namespace
    *CmpCommon::diags() << DgSqlCode(-1072) << DgString0(name.data());
    return -1;
  }
  ...
  retcode = ehi->namespaceOperation(COM_CHECK_NAMESPACE_EXISTS, name.data(), 0, NULL, NULL,
  ...
  if ((oper == COM_CREATE_NAMESPACE) &&  // create
    ...
    *CmpCommon::diags() << DgSqlCode(-1066) << DgString0(name.data());
    return -1;
  }

  if (((oper == COM_DROP_NAMESPACE) || (oper == COM_ALTER_NAMESPACE)) && (NOT namespaceExists))  // does not exist
  {
    ...
    *CmpCommon::diags() << DgSqlCode(-1067) << DgString0(name.data());
    return -1;
  }

  if (oper == COM_DROP_NAMESPACE) {
    // make sure namespace is empty
    retcode = ehi->namespaceOperation(COM_GET_NAMESPACE_OBJECTS, name.data(), 0, NULL, NULL, &namespaceObjects);
    ...
    if (namespaceObjects && (namespaceObjects->entries() > 0)) {
      ...
      if (namespaceObjects->entries() > 3) refObject += " ... ";
      *CmpCommon::diags() << DgSqlCode(-1064) << DgString0(name.data()) << DgString1(refObject.data());
      return -1;
    }
  }

  int numKeyValEntries = 0;
  NAText *keyArray = NULL;
  NAText *valArray = NULL;
  NAList<HbaseCreateOption *> *qo = NULL;
  if (((oper == COM_CREATE_NAMESPACE) || (oper == COM_ALTER_NAMESPACE)) && ((qo = ns->getQuotaOptions()) != NULL) &&
      (qo->entries() > 0)) {
  ...
  retcode = ehi->namespaceOperation(oper, name.data(), numKeyValEntries, keyArray, valArray, NULL);
  if (retcode < 0) {
    *CmpCommon::diags() << DgSqlCode(-8448) << DgString0((char *)"ExpHbaseInterface::namespaceOperation()")
                        << DgString1(getHbaseErrStr(-retcode)) << DgInt0(-retcode)
                        << DgString2((char *)GetCliGlobals()->getJniErrorStr());
...
short CmpSeabaseDDL::dropSeabaseObjectsFromHbase(const char *pattern, NABoolean ddlXns) {
  ExpHbaseInterface *ehi = allocEHI(COM_STORAGE_HBASE);
  if (ehi == NULL) return -1;
  ...
  if (retcode < 0) {
    *CmpCommon::diags() << DgSqlCode(-8448) << DgString0((char *)"ExpHbaseInterface::dropAll()")
                        << DgString1(getHbaseErrStr(-retcode)) << DgInt0(-retcode)
                        << DgString2((char *)GetCliGlobals()->getJniErrorStr());
...
void CmpSeabaseDDL::dropSeabaseMD(NABoolean ddlXns, NABoolean forceOption) {
  ...
  if ((CmpCommon::context()->isUninitializedSeabase()) &&
      (CmpCommon::context()->uninitializedSeabaseErrNum() == -TRAF_NOT_INITIALIZED) && (!forceOption)) {
    *CmpCommon::diags() << DgSqlCode(-TRAF_NOT_INITIALIZED);
    return;
  }
  ...
  if (endXnIfStartedHere(&cliInterface, xnWasStartedHere, 0) < 0) return;
...
NABoolean CmpSeabaseDDL::appendErrorObjName(char *errorObjs, const char *objName) {
...
// The diags area is populated with any unexpected errors
// ----------------------------------------------------------------------------
short CmpSeabaseDDL::initSeabaseAuthorization(ExeCliInterface *cliInterface, NABoolean ddlXns, NABoolean isUpgrade,
                                              std::vector<std::string> &tablesCreated) {
  int cliRC = 0;
  NABoolean xnWasStartedHere = FALSE;
  ...
  if (privInterface.initializeAuthorizationMetadata(tablesCreated, isUpgrade, cliInterface) == STATUS_ERROR) {
    // make sure authorization status is FALSE in compiler context
    if (!isUpgrade) GetCliGlobals()->currContext()->setAuthStateInCmpContexts(FALSE, FALSE);
    ...
    if (CmpCommon::diags()->getNumber(DgSqlCode::ERROR_) == 0) SEABASEDDL_INTERNAL_ERROR("initialize authorization");
  ...
  if (adjustHiveExternalSchemas(cliInterface) != 0) warnings = TRUE;
  ...
  if (cliRC == 1)  // library exists
  {
    ...
    if (cliRC == -1) warnings = TRUE;
  }

  if (NOT ddlXns) endXnIfStartedHere(cliInterface, xnWasStartedHere, cliRC);
...
void CmpSeabaseDDL::dropSeabaseAuthorization(ExeCliInterface *cliInterface, NABoolean doCleanup) {
  if (!ComUser::isRootUserID()) {
    *CmpCommon::diags() << DgSqlCode(-CAT_NOT_AUTHORIZED);
    return;
  }
  ...
  if (retcode == STATUS_ERROR) {
    if (CmpCommon::diags()->getNumber(DgSqlCode::ERROR_) == 0) SEABASEDDL_INTERNAL_ERROR("drop authorization");
    return;
  }
...
NABoolean CmpSeabaseDDL::insertPrivMgrInfo(const long objUID, const NAString &objName, const ComObjectType objectType,
  ...
  if (objectType == COM_VIEW_OBJECT) {
    ...
    if (qn.getCatalogName() == TRAFODION_SYSCAT_LIT) return TRUE;
  ...
  Int16 status = ComUser::getAuthNameFromAuthID((int)schemaOwnerID, (char *)&username, MAX_USERNAME_LEN + 1, lActualLen,
  ...
  if (schemaOwnerID == objOwnerID)
    ...
    Int16 status = ComUser::getAuthNameFromAuthID((int)objOwnerID, (char *)&username, MAX_USERNAME_LEN + 1, lActualLen,
                                                  FALSE, CmpCommon::diags());
    if (status != 0) return FALSE;
  ...
  NAString privMgrMDLoc;
  CONCAT_CATSCH(privMgrMDLoc, getSystemCatalog(), SEABASE_PRIVMGR_SCHEMA);
  PrivMgrPrivileges privileges(objUID, std::string(objName), SYSTEM_USER, std::string(privMgrMDLoc.data()),
                               CmpCommon::diags());
  PrivStatus retcode = privileges.grantToOwners(objectType, schemaOwnerID, schemaOwnerGrantee, objOwnerID, ownerGrantee,
                                                creatorID, creatorGrantee);
  if (retcode != STATUS_GOOD && retcode != STATUS_WARNING) return FALSE;
  return TRUE;
}
...
//    FALSE if an error occurred (ComDiags is set up with the error)
// ----------------------------------------------------------------------------
NABoolean CmpSeabaseDDL::deletePrivMgrInfo(const NAString &objectName, const long objUID,
                                           const ComObjectType objectType) {
  if (!PrivMgr::isSecurableObject(objectType)) return TRUE;
  ...
  PrivStatus retcode = privInterface.revokeObjectPrivilege(objUID, objName, -2);
...
void CmpSeabaseDDL::updateVersion() {
  ...
  long softMajorVersion;
  long softMinorVersion;
  long softUpdVersion;
  getSystemSoftwareVersion(softMajorVersion, softMinorVersion, softUpdVersion);

  char queryBuf[5000];

  long updateTime = NA_JulianTimestamp();

  str_sprintf(queryBuf,
              "update %s.\"%s\".%s set major_version = %ld, minor_version = %ld, init_time = %ld, comment = 'update "
              "version'  where version_type = 'SOFTWARE' ",
              getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_VERSIONS, softMajorVersion, softMinorVersion, updateTime);
  ...
  if (endXnIfStartedHere(&cliInterface, xnWasStartedHere, cliRC) < 0) return;
}
...
short CmpSeabaseDDL::recreateHbaseTable(const NAString &catalogNamePart, const NAString &schemaNamePart,
                                        const NAString &objectNamePart, NATable *naTable, ExpHbaseInterface *ehi) {
  int retcode = 0;

  const NAString extNameForHbase = genHBaseObjName(catalogNamePart, schemaNamePart, objectNamePart,
  ...
  int numSaltPartns = naf->numSaltPartns();
  int numSaltSplits = numSaltPartns - 1;
  Int16 numTrafReplicas = naf->numTrafReplicas();
  int numSplits = 0;
  const int numKeys = naf->getIndexKeyColumns().entries();
  int keyLength = naf->getKeyLength();
  ...
  if (createEncodedKeysBuffer(encodedKeysBuffer /*out*/, numSplits /*out*/, colDescs, keyDescs, numSaltPartns,
                              numSaltSplits, NULL, numTrafReplicas, numKeys, keyLength, FALSE)) {
    deallocEHI(ehi);
  ...
  for (int i = 0; i < naTable->allColFams().entries(); i++) {
    processColFamily(naTable->allColFams()[i], outColFam, &userColFamVec, &trafColFamVec);
  }  // for

  NABoolean isMVCC = true;
  if (CmpCommon::getDefault(TRAF_TRANS_TYPE) == DF_SSCC) isMVCC = false;
  retcode = createHbaseTable(ehi, &hbaseTable, trafColFamVec, hbaseCreateOptions, numSplits, keyLength,
                             encodedKeysBuffer, FALSE, FALSE, FALSE, FALSE, isMVCC);
  if (retcode == -1) {
    deallocEHI(ehi);
...
short CmpSeabaseDDL::truncateHbaseTable(const NAString &catalogNamePart, const NAString &schemaNamePart,
  ...
  const NAString extNameForHbase =
      genHBaseObjName(catalogNamePart, schemaNamePart, objectNamePart, nameSpace, objDataUID);
  ...
  if (gEnableRowLevelLock) {
    if (lockRequired(ehi, extNameForHbase, HBaseLockMode::LOCK_X, FALSE /* useHbaseXN */, FALSE /* replSync */,
                     FALSE /* incrementalBackup */, FALSE /* asyncOperation */, TRUE /* noConflictCheck */,
                     FALSE /* registerRegion */)) {
      processReturn();
      return -2;
    }
  }
  ...
  if (retcode) {
    *CmpCommon::diags() << DgSqlCode(-8448) << DgString0((char *)"ExpHbaseInterface::truncate()")
                        << DgString1(getHbaseErrStr(-retcode)) << DgInt0(-retcode)
                        << DgString2((char *)GetCliGlobals()->getJniErrorStr());
  ...
  if (createSnapshotForIncrBackup(&cliInterface, incrBackupEnabled, nameSpace, catalogNamePart, schemaNamePart,
                                  objectNamePart, objDataUID))
    return -1;

  return 0;
}

void CmpSeabaseDDL::purgedataHbaseTable(DDLExpr *ddlExpr, NAString &currCatName, NAString &currSchName) {
  int cliRC;
  int retcode = 0;
  ...
  if ((purgedataTableName.isSpecialTable()) && (!Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL))) {
  ...
  if (cn.isHbase()) {
    *CmpCommon::diags() << DgSqlCode(-3242) << DgString0("Purgedata is not allowed for HBase tables.");
    return;
  }

  retcode = lockObjectDDL(cn.getQualifiedNameObj(), COM_BASE_TABLE_OBJECT, cn.isVolatile(), cn.isExternal());
  if (retcode < 0) {
    processReturn();
    return;
  }
  ...
  long objUid = 0;
  cliRC = isBRInProgress(&cliInterface, catalogNamePart.data(), schemaNamePart.data(), objectNamePart.data(), objUid);
  if (cliRC) {
    processReturn();
    return;
  }
  ...
  if (!isDDLOperationAuthorized(SQLOperation::ALTER, naTable->getOwner(), naTable->getSchemaOwner(),
                                naTable->objectUid().get_value(), COM_BASE_TABLE_OBJECT, naTable->getPrivInfo())) {
    ...
    if (!privs->hasSelectPriv() || !privs->hasDeletePriv()) {
      ...
      if (privCommand.getPrivileges(naTable->objectUid().get_value(), COM_BASE_TABLE_OBJECT, ComUser::getCurrentUser(),
      ...
      if (!updatedPrivInfo.hasSelectPriv()) {
        *CmpCommon::diags() << DgSqlCode(-4481) << DgString0("SELECT") << DgString1(extTableName);
        unlockTruncateTable(&cliInterface, objUid);
        deallocEHI(ehi);
        processReturn();
        return;
      ...
      if (!updatedPrivInfo.hasDeletePriv()) {
        *CmpCommon::diags() << DgSqlCode(-4481) << DgString0("DELETE") << DgString1(extTableName);
        unlockTruncateTable(&cliInterface, objUid);
        deallocEHI(ehi);
        processReturn();
        return;
    ...
    for (int i = 0; i < uniqueList.entries(); i++) {
      ...
      for (int i = 0; i < uniqConstr->getNumRefConstraintsReferencingMe(); i++) {
        ...
        if (cliRC == 0)  // referencing table has record raise an error
        {
          ...
          *CmpCommon::diags() << DgSqlCode(-1425) << DgTableName(extTableName) << DgString0(reason.data());
          unlockTruncateTable(&cliInterface, objUid);
          deallocEHI(ehi);
          processReturn();
          return;
  ...
  retcode = updateObjectValidDef(&cliInterface, catalogNamePart.data(), schemaNamePart.data(), objectNamePart.data(),
  ...
  if (naTable->isPartitionV2Table()) {
    ...
    for (int i = 0; i < naPartArry.entries(); i++) {
      if (naPartArry[i]->hasSubPartition()) {
        ...
        for (int j = 0; j < naSubPartArry->entries(); j++) {
          ...
          if (truncateHbaseTable(catalogNamePart, schemaNamePart, subPartiEntityName, naTable->getNamespace(),
                                 FALSE, /*hasSaltOrReplicaCol*/
                                 inTrx, naTable->incrBackupEnabled(), naTable->objDataUID().castToInt64(), ehi)) {
            unlockTruncateTable(&cliInterface, objUid);
            deallocEHI(ehi);
            processReturn();
            goto label_error;
        ...
        if (truncateHbaseTable(catalogNamePart, schemaNamePart, partiEntityName, partNATable->getNamespace(),
                               FALSE, /*hasSaltOrReplicaCol*/
                               inTrx, partNATable->incrBackupEnabled(), partNATable->objDataUID().castToInt64(), ehi)) {
          unlockTruncateTable(&cliInterface, objUid);
          deallocEHI(ehi);
          processReturn();
          goto label_error;
        ...
        for (int i = 0; i < partIndexList.entries(); i++) {
          ...
          if ((partNaf->getKeytag() == 0) || (partNaf->partGlobalIndex())) continue;
          ...
          if (USE_UUID_AS_HBASE_TABLENAME) {
            getObjectUID(&cliInterface, catalogNamePart.data(), schemaNamePart.data(), partIndexName.data(),
                         COM_INDEX_OBJECT_LIT, &indexDataUID);
          }
          if (truncateHbaseTable(catalogNamePart, schemaNamePart, partIndexName, partNATable->getNamespace(),
                                 partNATable->hasSaltedColumn() || partNATable->hasTrafReplicaColumn(),
                                 (partNATable->xnRepl() == COM_REPL_SYNC), partNATable->incrBackupEnabled(),
                                 indexDataUID, ehi)) {
            unlockTruncateTable(&cliInterface, objUid);
            deallocEHI(ehi);
            processReturn();
    ...
    if (truncateHbaseTable(catalogNamePart, schemaNamePart, objectNamePart, naTable->getNamespace(),
                           naTable->hasSaltedColumn() || naTable->hasTrafReplicaColumn(), inTrx,
                           naTable->incrBackupEnabled(), naTable->objDataUID().castToInt64(), ehi)) {
      unlockTruncateTable(&cliInterface, objUid);
      deallocEHI(ehi);
      processReturn();
      goto label_error;
  ...
  if (naTable->hasSecondaryIndexes() && (NOT naTable->isPartitionV2Table()))  // user indexes
  {
    ...
    for (int i = 0; i < indexList.entries(); i++) {
      ...
      // if USE_UUID_AS_HBASE_TABLENAME is true get index table uid from objects
      if (USE_UUID_AS_HBASE_TABLENAME) {
        getObjectUID(&cliInterface, catalogNamePart.data(), schemaNamePart.data(), idxName.data(), COM_INDEX_OBJECT_LIT,
                     &indexDataUID);
      }
      if (truncateHbaseTable(catName, schName, idxName, naTable->getNamespace(),
                             naTable->hasSaltedColumn() || naTable->hasTrafReplicaColumn(),
                             (inTrx ? TRUE : (naTable->xnRepl() == COM_REPL_SYNC)), naTable->incrBackupEnabled(),
                             indexDataUID, ehi)) {
        unlockTruncateTable(&cliInterface, objUid);
        deallocEHI(ehi);
        processReturn();
  ...
  if (identity_found) {
    ...
    char buf[2500];
    long start_value = 0;
    int value_len = sizeof(long);
    long rowsAffected = 0;
    str_sprintf(buf,
                "select s.start_value from %s.\"%s\".%s s, %s.\"%s\".%s o where s.seq_uid = o.object_uid and "
                "o.catalog_name = '%s' and o.schema_name = '%s' and o.object_name = '%s' and o.object_type = 'SG'",
                getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_SEQ_GEN, getSystemCatalog(), SEABASE_MD_SCHEMA,
                SEABASE_OBJECTS, catalogNamePart.data(), schemaNamePart.data(), seqName.data());
    retcode = cliInterface.executeImmediateCEFC(buf, NULL, 0, (char *)&start_value, &value_len, &rowsAffected);
    if (retcode < 0) {
      unlockTruncateTable(&cliInterface, objUid);
      deallocEHI(ehi);
      processReturn();
    ...
    str_sprintf(buf, "alter sequence %s.%s.\"%s\" set nextval %ld", catalogNamePart.data(), schemaNamePart.data(),
                seqName.data(), start_value);
...
label_error:
  updateObjectValidDef(&cliInterface, catalogNamePart.data(), schemaNamePart.data(), objectNamePart.data(),
                       COM_BASE_TABLE_OBJECT_LIT, COM_YES_LIT);
...
void CmpSeabaseDDL::alterSeabaseTableTruncatePartition(StmtDDLAlterTableTruncatePartition *alterTableTrunPartitionNode,
  ...
  // error check
  retcode = setupAndErrorChecks(tabName, alterTableTrunPartitionNode->getOrigTableNameAsQualifiedName(), currCatName,
                                ...
                                SQLOperation::ALTER_TABLE, FALSE, FALSE /*process hiatus*/);
  if (retcode < 0) {
    processReturn();
    return;
  }
  ...
  char queryStmt[2000];
  for (int i = 0; i < truncatePartEntityNameVec.size(); i++) {
    // cout << truncatePartEntityNameVec[i] << endl;
    str_sprintf(queryStmt, "truncate table %s.%s.%s", catalogNamePart.data(), schemaNamePart.data(),
                truncatePartEntityNameVec[i].data());
...
short CmpSeabaseDDL::executeSeabaseDDL(DDLExpr *ddlExpr, ExprNode *ddlNode, NAString &currCatName,
  ...
  if ((ddlExpr) &&
      ((ddlExpr->initHbase()) || (ddlExpr->createMDViews()) || (ddlExpr->dropMDViews()) || (ddlExpr->createRepos()) ||
       (ddlExpr->dropRepos()) || (ddlExpr->upgradeRepos()) || (ddlExpr->createBackupRepos()) ||
       (ddlExpr->dropBackupRepos()) || (ddlExpr->upgradeBackupRepos()) || (ddlExpr->addSchemaObjects()) ||
       (ddlExpr->createLibmgr()) || (ddlExpr->createTenant()) || (ddlExpr->castToRelDumpLoad()) ||
       (ddlExpr->castToRelGenLoadQueryCache()) ||
       (ddlExpr->castToRelBackupRestore() &&
        ((ddlExpr->castToRelBackupRestore()->restoreSystem()) || (ddlExpr->castToRelBackupRestore()->unlockTraf()) ||
         ...
         (ddlExpr->castToRelBackupRestore()->exportBackup()) || (ddlExpr->castToRelBackupRestore()->importBackup()))) ||
       (ddlExpr->loadTrafMetadataInCache()) || (ddlExpr->loadTrafMetadataIntoSharedCache()) ||
       (ddlExpr->loadTrafDataIntoSharedCache()) || (ddlExpr->updateVersion()) || (ddlExpr->upgradeNamespace()) ||
       (ddlExpr->createXDCMetadata()) || (ddlExpr->dropXDCMetadata()) || (ddlExpr->upgradeXDCMetadata()) ||
       (ddlExpr->createMDPartTables()) || (ddlExpr->dropMDPartTables()) || (ddlExpr->upgradeMDPartTables())))
    ignoreUninitTrafErr = TRUE;

  if ((Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL)) && (CmpCommon::context()->isUninitializedSeabase())) {
    ignoreUninitTrafErr = TRUE;
  }

  if (dws && dws->getInitTraf()) ignoreUninitTrafErr = TRUE;

  if ((CmpCommon::context()->isUninitializedSeabase()) && (NOT ignoreUninitTrafErr)) {
    *CmpCommon::diags() << DgSqlCode(CmpCommon::context()->uninitializedSeabaseErrNum());
    return -1;
  }
  ...
  if ((ddlExpr && ddlExpr->loadTrafMetadataInCache()) || (ddlExpr && ddlExpr->loadTrafMetadataIntoSharedCache()) ||
      (ddlExpr && ddlExpr->loadTrafDataIntoSharedCache()) ||
      (ddlNode && ddlNode->getOperatorType() == DDL_ALTER_TABLE_STORED_DESC) ||
      (ddlNode && ddlNode->getOperatorType() == DDL_ALTER_SHARED_CACHE) || (ddlExpr && ddlExpr->castToRelDumpLoad()) ||
      (ddlExpr && ddlExpr->castToRelGenLoadQueryCache()) || (ddlExpr && ddlExpr->castToRelBackupRestore())) {
    sendACF = FALSE;
    setFlags = TRUE;
  }
  ...
  if (ddlExpr && ddlExpr->ddlXnsInfo(ddlXns, startXn)) return -1;

  if (startXn) {
    if (beginXnIfNotInProgress(&cliInterface, xnWasStartedHere)) goto label_return;
  }

  if (dws) {
    if (dws->getMDcleanup()) {
      StmtDDLCleanupObjects *co = (ddlNode ? ddlNode->castToStmtDDLNode()->castToStmtDDLCleanupObjects() : NULL);
      ...
      if (ddlExpr) {
        dws->setDDLXns(TRUE);
        if (ddlExpr->minimal()) dws->setMinimalInitTraf(TRUE);
      }
  ...
  if (ddlExpr->initHbase()) {
    ...
    initSeabaseAuthorization(&cliInterface, ddlExpr->ddlXns(), TRUE /*isUpgrade*/, tablesCreated);

    NAString msg("Tables created in \"_PRIVMGR_MD_\" schema: ");
    for (size_t i = 0; i < tablesCreated.size(); i++) msg += tablesCreated[i].c_str() + NAString(" ");
  ...
  } else if ((ddlExpr->createRepos()) || (ddlExpr->dropRepos()) || (ddlExpr->upgradeRepos())) {
    if (!ComUser::isRootUserID())
      *CmpCommon::diags() << DgSqlCode(-CAT_NOT_AUTHORIZED);
    else
      processRepository(ddlExpr->createRepos(), ddlExpr->dropRepos(), ddlExpr->upgradeRepos());
  } else if ((ddlExpr->createBackupRepos()) || (ddlExpr->dropBackupRepos()) || (ddlExpr->upgradeBackupRepos())) {
    if (!ComUser::isRootUserID())
      *CmpCommon::diags() << DgSqlCode(-CAT_NOT_AUTHORIZED);
    else
      processBackupRepository(ddlExpr->createBackupRepos(), ddlExpr->dropBackupRepos(), ddlExpr->upgradeBackupRepos());
  } else if (ddlExpr->castToRelDumpLoad()) {
    if (xnInProgress(&cliInterface)) {
      *CmpCommon::diags() << DgSqlCode(-20123) << DgString0("This operation");
    } else {
    ...
    if (dl->getType() == RelGenLoadQueryCache::GEN_QUERYCACHE_USER) {
    ...
    if (xnInProgress(&cliInterface)) {
      *CmpCommon::diags() << DgSqlCode(-20123) << DgString0("This operation");
      ...
      if (br->backupSystem()) {
        backupSystem(ddlExpr->castToRelBackupRestore(), &cliInterface, dws);
      } else if (br->backup()) {
        backupSubset(ddlExpr->castToRelBackupRestore(), &cliInterface, currCatName, currSchName, dws);
      }  // backup
      else if (br->restore()) {
        restore(ddlExpr->castToRelBackupRestore(), &cliInterface, currCatName, currSchName, dws);
      }  // restore
      else if (br->restoreSystem()) {
        if ((!ddlExpr->castToRelBackupRestore()->showObjects()) && (!CmpCommon::context()->isUninitializedSeabase())) {
          *CmpCommon::diags() << DgSqlCode(-1021) << DgString0(".This operation is not permitted");
        }
        ...
        exportOrImportBackup(ddlExpr->castToRelBackupRestore(), &cliInterface);
        ...
        getBackupLockedObjects(ddlExpr->castToRelBackupRestore(), &cliInterface, dws);
      } else if (br->getVersionOfBackup()) {
        getVersionOfBackup(ddlExpr->castToRelBackupRestore(), &cliInterface, dws);
      } else if (br->cleanupObjects()) {
        cleanupLockedObjects(ddlExpr->castToRelBackupRestore(), &cliInterface);
      } else if (br->getCreateTags()) {
        createBackupTags(ddlExpr->castToRelBackupRestore(), &cliInterface);
      } else if (br->getProgressStatus()) {
        getProgressStatus(ddlExpr->castToRelBackupRestore(), &cliInterface, dws);
      ...
      long operEndTime = NA_JulianTimestamp();

      if (!CmpCommon::diags()->getNumber(DgSqlCode::ERROR_)) {
        updateBackupOperationMetrics(ddlExpr->castToRelBackupRestore(), &cliInterface, operStartTime, operEndTime);
      }
    }
  }  // RelBackupRestore
    ...
    if (xnInProgress(&cliInterface)) {
      *CmpCommon::diags() << DgSqlCode(-20123) << DgString0("This operation");
      ...
      loadTrafMetadataIntoSharedCache(ddlExpr->explObjName(), ddlExpr->loadLocalIfEmpty(), dws);
    }
  } else if (ddlExpr->loadTrafDataIntoSharedCache()) {
    if (xnInProgress(&cliInterface)) {
      *CmpCommon::diags() << DgSqlCode(-20123) << DgString0("This operation");
    } else {
      loadTrafDataIntoSharedCache(ddlExpr->explObjName(), ddlExpr->loadLocalIfEmpty(), dws);
    }
  ...
  } else if ((ddlExpr->createXDCMetadata()) || (ddlExpr->dropXDCMetadata()) || (ddlExpr->upgradeXDCMetadata())) {
    if (!ComUser::isRootUserID())
      *CmpCommon::diags() << DgSqlCode(-CAT_NOT_AUTHORIZED);
    else
      processXDCMeta(ddlExpr->createXDCMetadata(), ddlExpr->dropXDCMetadata(), ddlExpr->upgradeXDCMetadata());
  } else if ((ddlExpr->createMDPartTables()) || (ddlExpr->dropMDPartTables()) || (ddlExpr->upgradeMDPartTables())) {
    if (!ComUser::isRootUserID())
      *CmpCommon::diags() << DgSqlCode(-CAT_NOT_AUTHORIZED);
    else
      processMDPartTables(ddlExpr->createMDPartTables(), ddlExpr->dropMDPartTables(), ddlExpr->upgradeMDPartTables());
    ...
    if (ddlNode->getOperatorType() == DDL_CREATE_TABLE) {
      // create hbase table
      StmtDDLCreateTable *createTableParseNode = ddlNode->castToStmtDDLNode()->castToStmtDDLCreateTable();
      ...
      if ((createTableParseNode->getAddConstraintUniqueArray().entries() > 0) ||
        ...
        if (((getenv("SQLMX_REGRESS")) || (CmpCommon::getDefault(TRAF_AUTO_CREATE_SCHEMA) == DF_ON)) &&
          ...
          char query[1000];
          if ((getenv("SQLMX_REGRESS")) && (schemaNamePart == SEABASE_REGRESS_DEFAULT_SCHEMA)) {
            CmpCommon::diags()->clear();
            str_sprintf(query, "create shared schema %s.%s namespace ''", TRAFODION_SYSCAT_LIT,
                        SEABASE_REGRESS_DEFAULT_SCHEMA);
            cliRC = cliInterface.executeImmediate(query);
            if (cliRC >= 0) {
              str_sprintf(
                  query, "upsert into %s.\"%s\".%s values ('SCHEMA ', '%s.%s ', 'inserted during regressions run', 0);",
                  TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_DEFAULTS, TRAFODION_SYSCAT_LIT,
                  SEABASE_REGRESS_DEFAULT_SCHEMA);
              cliRC = cliInterface.executeImmediate(query);
              if (cliRC >= 0) {
                createSeabaseTable(createTableParseNode, currCatName, currSchName);
            ...
            // create this schema
            CmpCommon::diags()->clear();
            str_sprintf(query, "create schema %s.\"%s\";", TRAFODION_SYSCAT_LIT, schemaNamePart.data());
            cliRC = cliInterface.executeImmediate(query);
            if (cliRC >= 0) {
              createSeabaseTable(createTableParseNode, currCatName, currSchName);
            }
      ...
      // create hbase table
      StmtDDLCreateHbaseTable *createTableParseNode = ddlNode->castToStmtDDLNode()->castToStmtDDLCreateHbaseTable();
      ...
      // drop seabase table
      StmtDDLDropTable *dropTableParseNode = ddlNode->castToStmtDDLNode()->castToStmtDDLDropTable();

      dropSeabaseTable(dropTableParseNode, currCatName, currSchName);
    } else if (ddlNode->getOperatorType() == DDL_DROP_HBASE_TABLE) {
      // drop hbase table
      StmtDDLDropHbaseTable *dropTableParseNode = ddlNode->castToStmtDDLNode()->castToStmtDDLDropHbaseTable();

      dropNativeHbaseTable(&cliInterface, dropTableParseNode, currCatName, currSchName);
    } else if (ddlNode->getOperatorType() == DDL_CREATE_INDEX) {
      // create seabase index
      StmtDDLCreateIndex *createIndexParseNode = ddlNode->castToStmtDDLNode()->castToStmtDDLCreateIndex();
      ...
      StmtDDLPopulateIndex *populateIndexParseNode = ddlNode->castToStmtDDLNode()->castToStmtDDLPopulateIndex();

      populateSeabaseIndex(populateIndexParseNode, currCatName, currSchName);
    } else if (ddlNode->getOperatorType() == DDL_DROP_INDEX) {
      // drop seabase table
      StmtDDLDropIndex *dropIndexParseNode = ddlNode->castToStmtDDLNode()->castToStmtDDLDropIndex();

      dropSeabaseIndex(dropIndexParseNode, currCatName, currSchName);
    } else if (ddlNode->getOperatorType() == DDL_ALTER_TABLE_ADD_COLUMN) {
      StmtDDLAlterTableAddColumn *alterAddColNode = ddlNode->castToStmtDDLNode()->castToStmtDDLAlterTableAddColumn();

      alterSeabaseTableAddColumn(alterAddColNode, currCatName, currSchName);
    } else if (ddlNode->getOperatorType() == DDL_ALTER_TABLE_DROP_COLUMN) {
      StmtDDLAlterTableDropColumn *alterDropColNode = ddlNode->castToStmtDDLNode()->castToStmtDDLAlterTableDropColumn();

      alterSeabaseTableDropColumn(alterDropColNode, currCatName, currSchName);
      ...
      StmtDDLAddConstraint *alterAddConstraint = ddlNode->castToStmtDDLNode()->castToStmtDDLAddConstraint();

      alterSeabaseTableAddPKeyConstraint(alterAddConstraint, currCatName, currSchName);
    } else if (ddlNode->getOperatorType() == DDL_ALTER_TABLE_ADD_CONSTRAINT_UNIQUE) {
      StmtDDLAddConstraint *alterAddConstraint = ddlNode->castToStmtDDLNode()->castToStmtDDLAddConstraint();

      alterSeabaseTableAddUniqueConstraint(alterAddConstraint, currCatName, currSchName);
    } else if (ddlNode->getOperatorType() == DDL_ALTER_TABLE_ADD_CONSTRAINT_REFERENTIAL_INTEGRITY) {
      StmtDDLAddConstraint *alterAddConstraint = ddlNode->castToStmtDDLNode()->castToStmtDDLAddConstraint();

      alterSeabaseTableAddRIConstraint(alterAddConstraint, currCatName, currSchName);
    } else if (ddlNode->getOperatorType() == DDL_ALTER_TABLE_ADD_CONSTRAINT_CHECK) {
      StmtDDLAddConstraint *alterAddConstraint = ddlNode->castToStmtDDLNode()->castToStmtDDLAddConstraint();

      alterSeabaseTableAddCheckConstraint(alterAddConstraint, currCatName, currSchName);
    } else if (ddlNode->getOperatorType() == DDL_ALTER_TABLE_DROP_CONSTRAINT) {
      StmtDDLDropConstraint *alterDropConstraint = ddlNode->castToStmtDDLNode()->castToStmtDDLDropConstraint();

      alterSeabaseTableDropConstraint(alterDropConstraint, currCatName, currSchName);
    }
      ...
      if (!(allIndexes || allUniquesOnly))
        alterSeabaseTableDisableOrEnableIndex(ddlNode, currCatName, currSchName);
      else {
        alterSeabaseTableDisableOrEnableAllIndexes(ddlNode, currCatName, currSchName, (NAString &)tabNameNAS,
                                                   allUniquesOnly);
      }
    } else if (ddlNode->getOperatorType() == DDL_ALTER_TABLE_RENAME) {
      StmtDDLAlterTableRename *alterRenameTable = ddlNode->castToStmtDDLNode()->castToStmtDDLAlterTableRename();

      renameSeabaseTable(alterRenameTable, currCatName, currSchName);
      ...
      StmtDDLAlterTableHBaseOptions *athbo = ddlNode->castToStmtDDLNode()->castToStmtDDLAlterTableHBaseOptions();
      ...
      StmtDDLAlterIndexHBaseOptions *aihbo = ddlNode->castToStmtDDLNode()->castToStmtDDLAlterIndexHBaseOptions();

      alterSeabaseIndexHBaseOptions(aihbo, currCatName, currSchName);
    } else if (ddlNode->getOperatorType() == DDL_ALTER_TABLE_ATTRIBUTE) {
      StmtDDLAlterTableAttribute *ata = ddlNode->castToStmtDDLNode()->castToStmtDDLAlterTableAttribute();

      alterSeabaseTableAttribute(ata, currCatName, currSchName);
    } else if (ddlNode->getOperatorType() == DDL_ALTER_TABLE_RESET_DDL_LOCK) {
      StmtDDLAlterTableResetDDLLock *atl = ddlNode->castToStmtDDLNode()->castToStmtDDLAlterTableResetDDLLock();

      alterSeabaseTableResetDDLLock(&cliInterface, atl, currCatName, currSchName);
    } else if (ddlNode->getOperatorType() == DDL_CREATE_VIEW) {
      // create seabase view
      StmtDDLCreateView *createViewParseNode = ddlNode->castToStmtDDLNode()->castToStmtDDLCreateView();

      createSeabaseView(createViewParseNode, currCatName, currSchName);
    } else if (ddlNode->getOperatorType() == DDL_DROP_VIEW) {
      // drop seabase table
      StmtDDLDropView *dropViewParseNode = ddlNode->castToStmtDDLNode()->castToStmtDDLDropView();

      dropSeabaseView(dropViewParseNode, currCatName, currSchName);
    } else if (ddlNode->getOperatorType() == DDL_REGISTER_USER) {
      StmtDDLRegisterUser *registerUserParseNode = ddlNode->castToStmtDDLNode()->castToStmtDDLRegisterUser();
      StmtDDLRegisterUser::RegisterUserType ruType = registerUserParseNode->getRegisterUserType();
      switch (ruType) {
      ...
      StmtDDLUserGroup *groupParseNode = ddlNode->castToStmtDDLNode()->castToStmtDDLUserGroup();
      ...
      StmtDDLAlterUser *alterUserParseNode = ddlNode->castToStmtDDLNode()->castToStmtDDLAlterUser();
      ...
      StmtDDLResourceGroup *rGroupParseNode = ddlNode->castToStmtDDLNode()->castToStmtDDLResourceGroup();
      ...
      createSeabaseComponentOperation(getSystemCatalog(), createComponentOperationParseNode);
      ...
      dropSeabaseComponentOperation(getSystemCatalog(), dropComponentOperationParseNode);
      ...
      grantSeabaseComponentPrivilege(getSystemCatalog(), grantComponentPrivilegeParseNode);
      ...
      revokeSeabaseComponentPrivilege(getSystemCatalog(), revokeComponentPrivilegeParseNode);
    } else if (ddlNode->getOperatorType() == DDL_GRANT_ROLE) {
      StmtDDLRoleGrant *grantRoleParseNode = ddlNode->castToStmtDDLNode()->castToStmtDDLRoleGrant();
      grantRevokeSeabaseRole(getSystemCatalog(), grantRoleParseNode);
    } else if ((ddlNode->getOperatorType() == DDL_GRANT) || (ddlNode->getOperatorType() == DDL_REVOKE)) {
      // grant/revoke seabase table
      StmtDDLNode *stmtDDLParseNode = ddlNode->castToStmtDDLNode();

      seabaseGrantRevoke(stmtDDLParseNode, (ddlNode->getOperatorType() == DDL_GRANT ? TRUE : FALSE), currCatName,
                         currSchName);
    } else if ((ddlNode->getOperatorType() == DDL_GRANT_SCHEMA) || (ddlNode->getOperatorType() == DDL_REVOKE_SCHEMA)) {
      // grant/revoke on schema
      StmtDDLNode *stmtDDLParseNode = ddlNode->castToStmtDDLNode();

      grantRevokeSchema(stmtDDLParseNode, (ddlNode->getOperatorType() == DDL_GRANT_SCHEMA ? TRUE : FALSE), currCatName,
                        currSchName);
    } else if (ddlNode->getOperatorType() == DDL_GIVE_ALL) {
      StmtDDLGiveAll *giveAllParseNode = ddlNode->castToStmtDDLNode()->castToStmtDDLGiveAll();
      giveSeabaseAll(giveAllParseNode);
    } else if (ddlNode->getOperatorType() == DDL_GIVE_OBJECT) {
      StmtDDLGiveObject *giveObjectParseNode = ddlNode->castToStmtDDLNode()->castToStmtDDLGiveObject();
      giveSeabaseObject(giveObjectParseNode);
    } else if (ddlNode->getOperatorType() == DDL_GIVE_SCHEMA) {
      StmtDDLGiveSchema *giveSchemaParseNode = ddlNode->castToStmtDDLNode()->castToStmtDDLGiveSchema();
      giveSeabaseSchema(giveSchemaParseNode, currCatName);
    } else if (ddlNode->getOperatorType() == DDL_CREATE_SCHEMA) {
      StmtDDLCreateSchema *createSchemaParseNode = ddlNode->castToStmtDDLNode()->castToStmtDDLCreateSchema();

      createSeabaseSchema(createSchemaParseNode, currCatName);
    } else if (ddlNode->getOperatorType() == DDL_DROP_SCHEMA) {
      // drop all tables in schema
      StmtDDLDropSchema *dropSchemaParseNode = ddlNode->castToStmtDDLNode()->castToStmtDDLDropSchema();

      dropSeabaseSchema(dropSchemaParseNode);
    } else if (ddlNode->getOperatorType() == DDL_ALTER_SCHEMA) {
      StmtDDLAlterSchema *alterSchemaParseNode = ddlNode->castToStmtDDLNode()->castToStmtDDLAlterSchema();

      alterSeabaseSchema(alterSchemaParseNode);
    } else if (ddlNode->getOperatorType() == DDL_CREATE_LIBRARY) {
      // create seabase library
      StmtDDLCreateLibrary *createLibraryParseNode = ddlNode->castToStmtDDLNode()->castToStmtDDLCreateLibrary();
      if ((CmpCommon::getDefault(USE_LIB_BLOB_STORE) == DF_OFF))
        createSeabaseLibrary(createLibraryParseNode, currCatName, currSchName);
      else {
        createSeabaseLibrary2(createLibraryParseNode, currCatName, currSchName);
      }
    } else if (ddlNode->getOperatorType() == DDL_DROP_LIBRARY) {
      // drop seabase library
      StmtDDLDropLibrary *dropLibraryParseNode = ddlNode->castToStmtDDLNode()->castToStmtDDLDropLibrary();
      dropSeabaseLibrary(dropLibraryParseNode, currCatName, currSchName);
    } else if (ddlNode->getOperatorType() == DDL_ALTER_LIBRARY) {
      // create seabase library
      StmtDDLAlterLibrary *alterLibraryParseNode = ddlNode->castToStmtDDLNode()->castToStmtDDLAlterLibrary();
      if ((CmpCommon::getDefault(USE_LIB_BLOB_STORE) == DF_OFF))
        alterSeabaseLibrary(alterLibraryParseNode, currCatName, currSchName);
      else {
      ...
      StmtDDLCreatePackage *createPackageParseNode = ddlNode->castToStmtDDLNode()->castToStmtDDLCreatePackage();

      createSeabasePackage(&cliInterface, createPackageParseNode, currCatName, currSchName);
      ...
      StmtDDLCreateSequence *createSequenceParseNode = ddlNode->castToStmtDDLNode()->castToStmtDDLCreateSequence();

      if (createSequenceParseNode->isAlter())
        alterSeabaseSequence(createSequenceParseNode, currCatName, currSchName);
      else
        createSeabaseSequence(createSequenceParseNode, currCatName, currSchName);
    } else if (ddlNode->getOperatorType() == DDL_DROP_SEQUENCE) {
      // drop seabase sequence
      StmtDDLDropSequence *dropSequenceParseNode = ddlNode->castToStmtDDLNode()->castToStmtDDLDropSequence();
      ...
      alterSeabaseTableAlterIdentityColumn(alterIdentityColNode, currCatName, currSchName);
      ...
      alterSeabaseTableAlterColumnDatatype(alterColNode, currCatName, currSchName, ddlExpr->isMaintenanceWindowOFF());
      ...
      alterSeabaseTableHDFSCache(alterTableHdfsCache, currCatName, currSchName);
      ...
      alterSeabaseTableAlterColumnRename(alterColNode, currCatName, currSchName);
      ...
      StmtDDLCleanupObjects *co = ddlNode->castToStmtDDLNode()->castToStmtDDLCleanupObjects();
      ...
      processNamespaceOperations(ddlNode->castToStmtDDLNode()->castToStmtDDLNamespace());
    } else if (ddlNode->getOperatorType() == DDL_COMMENT_ON) {
      StmtDDLCommentOn *comment = ddlNode->castToStmtDDLNode()->castToStmtDDLCommentOn();

      doSeabaseCommentOn(comment, currCatName, currSchName);
    }

    else if (ddlNode->getOperatorType() == DDL_CREATE_TRIGGER) {
      StmtDDLCreateTrigger *CreateTrigger = ddlNode->castToStmtDDLNode()->castToStmtDDLCreateTrigger();
      createSeabaseTriggers(&cliInterface, CreateTrigger, currCatName, currSchName);
    } else if (ddlNode->getOperatorType() == DDL_DROP_TRIGGER) {
      StmtDDLDropTrigger *DropTrigger = ddlNode->castToStmtDDLNode()->castToStmtDDLDropTrigger();
      dropSeabaseTriggers(&cliInterface, DropTrigger, currCatName, currSchName);
...
label_return:
  ...
  if (CmpCommon::diags()->getNumber(DgSqlCode::ERROR_)) {
    ...
    if (CmpCommon::diags()->mainSQLCODE() != -CLI_VALIDATE_TRANSACTION_ERROR) {
      CollIndex i = CmpCommon::diags()->returnIndex(-CLI_VALIDATE_TRANSACTION_ERROR);
  ...
  if (dlockForStoredDesc_) delete dlockForStoredDesc_;
...

{
  ...
  if (!isDDLOperationAuthorized(SQLOperation::ALTER, NA_UserIdDefault, NA_UserIdDefault, -1,
                                COM_SEQUENCE_GENERATOR_OBJECT, NULL)) {
    *CmpCommon::diags() << DgSqlCode(-CAT_NOT_AUTHORIZED);
    return;
  }
  ...
  if (ComUser::getAuthIDFromAuthName(giveAllParseNode->getFromID().data(), fromOwnerID, CmpCommon::diags()) != 0)
    return;
  ...
  if (ComUser::getAuthIDFromAuthName(giveAllParseNode->getToID().data(), toOwnerID, CmpCommon::diags()) != 0) return;
  ...
  str_sprintf(buf,
              ...
              getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_OBJECTS, toOwnerID, fromOwnerID);
  cliRC = cliInterface.executeImmediate(buf);
  if (cliRC < 0) {
    cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
    return;
  ...
  str_sprintf(buf,
              ...
              "FROM %s.\"%s\".%s "
              "WHERE object_owner = %d ",
              getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_OBJECTS, fromOwnerID);
...
// *                                                                           *
// *****************************************************************************
bool CmpSeabaseDDL::dropOneTableorView(ExeCliInterface &cliInterface, const char *objectName, ComObjectType objectType,
                                       bool isVolatile)

{
  ...
  if (isVolatile) strcpy(volatileString, "VOLATILE");

  str_sprintf(buf, "DROP %s %s %s CASCADE", volatileString, objectTypeString, objectName);
  ...
  if (cliRC < 0 && cliRC != -CAT_OBJECT_DOES_NOT_EXIST_IN_TRAFODION) someObjectsCouldNotBeDropped = true;
...
int32_t CmpSeabaseDDL::verifyDDLCreateOperationAuthorized(ExeCliInterface *cliInterface, SQLOperation operation,
                                                          ...
                                                          int &schemaOwner, long *schemaUID, long *schemaObjectFlags)

{
  ...
  if (operation == SQLOperation::CREATE_SCHEMA) {
    ...
    if (currentUser == ComUser::getRootUserID()) return 0;

    if (!isAuthorizationEnabled()) return 0;

    if (componentPrivileges.hasSQLPriv(currentUser, SQLOperation::CREATE_SCHEMA, true)) return 0;
  ...
  long tempSchUID = getObjectTypeandOwner(cliInterface, catalogName.data(), schemaName.data(),
                                          SEABASE_SCHEMA_OBJECTNAME, objectType, schemaOwner, schemaObjectFlags);
  ...
  if (schemaUID) *schemaUID = tempSchUID;
  ...
  if (schemaPrivs.getPrivsOnSchemaForUser(tempSchUID, objectType, currentUser, privsOfTheUser) == STATUS_ERROR) {
    SEABASEDDL_INTERNAL_ERROR("Error retrieving schema privileges");
    objectOwner = schemaOwner = NA_UserIdDefault;
    return CAT_INTERNAL_EXCEPTION_ERROR;
  }

  if (privsOfTheUser.getSchemaPrivs().getPriv(CREATE_PRIV)) return 0;
  ...
  snprintf(msg, len,
           ...
           currentUser, ComUser::getCurrentUsername(), schemaOwner, schemaName.data(),
           PrivMgr::getSQLOperationCode(operation), comObjectTypeLit(objectType));
...
bool CmpSeabaseDDL::isDDLOperationAuthorized(SQLOperation operation, const int objOwner, const int schemaOwner,
  ...
  // full DDL authority.
  if (currentUser == ComUser::getRootUserID()) return true;
  ...
  if (PrivMgr::isSQLCreateOperation(operation) && operation != SQLOperation::CREATE_INDEX) {
  ...
  if (currentUser == objOwner || currentUser == schemaOwner) return true;

  // 2) schema owner is a role and current user is granted the role
  if (CmpSeabaseDDLauth::isRoleID(schemaOwner)) {
    if (ComUser::currentUserHasRole(schemaOwner, NULL /*don't update diags*/)) return true;
  }

  // 3) current user has been granted DDL privilege
  //  --> privInfo contains both schema and object level
  bool dropRequested = (operation != SQLOperation::DROP_SCHEMA && PrivMgr::isSQLDropOperation(operation));
  bool alterRequested = (operation != SQLOperation::ALTER_SCHEMA && PrivMgr::isSQLAlterOperation(operation));
  if (privInfo) {
    if (dropRequested && privInfo->hasDropPriv()) return true;

    if (alterRequested && privInfo->hasAlterPriv()) return true;
  }
  ...
  if ((dropRequested || alterRequested) && objectUID > 0) {
    PrivMgrCommands privCommand;
    PrivMgrUserPrivs updatedPrivInfo;
    if (privCommand.getPrivileges(objectUID, objectType, currentUser, updatedPrivInfo) == STATUS_ERROR) return false;

    if (dropRequested && updatedPrivInfo.hasDropPriv()) return true;

    if (alterRequested && updatedPrivInfo.hasAlterPriv()) return true;
  }

  // 4) current user been granted component privilege
  PrivMgrComponentPrivileges componentPrivileges;
  if (componentPrivileges.hasSQLPriv(currentUser, operation, true)) return true;
...

{
  ...
  retcode = componentOperations.createComponentOperation(componentName, operationName, operationCode, isSystem,
                                                         operationDescription);

  if (retcode == STATUS_ERROR && CmpCommon::diags()->getNumber(DgSqlCode::ERROR_) == 0)
...

{
  ...
  retcode = componentOperations.dropComponentOperation(componentName, operationName, privDropBehavior);
...
static void grantRevokeSeabaseRole(const std::string &systemCatalog, StmtDDLRoleGrant *pParseNode) {
  ...
  PrivMgrCommands roleCommand(std::string(trafMDLocation.data()), std::string(privMgrMDLoc.data()), CmpCommon::diags());
  ...
  if (grantedBy != NULL) {
    ...
    if (grantorID != ComUser::getRootUserID()) {
      PrivMgrComponentPrivileges componentPrivileges(std::string(privMgrMDLoc.data()), CmpCommon::diags());
      if (!componentPrivileges.hasSQLPriv(grantorID, SQLOperation::MANAGE_ROLES, true)) {
        *CmpCommon::diags() << DgSqlCode(-CAT_NOT_AUTHORIZED);
        return;
      }
    }
    ...
    if (ComUser::getUserIDFromUserName(grantedByName.data(), grantorID, CmpCommon::diags()) != 0) {
      *CmpCommon::diags() << DgSqlCode(-CAT_AUTHID_DOES_NOT_EXIST_ERROR) << DgString0("User or group")
    ...
    if (grantorID == ComUser::getRootUserID()) grantorIsRoot = true;
  }
  ...
  PrivMgrRoles roleUsage(std::string(trafMDLocation.data()), std::string(privMgrMDLoc.data()), CmpCommon::diags());
  ...
  for (size_t r = 0; r < roles.entries(); r++) {
    ...
    if (roleInfo.isAuthAdmin()) checkForAdminRole = true;

    // See if this role has already been specified.
    if (hasValue(roleIDs, roleID)) {
      *CmpCommon::diags() << DgSqlCode(-CAT_DUPLICATE_ROLES_IN_LIST) << DgString0(roleName.data());
      return;
    }
    ...
    if (grantorIsRoot) {
      if (roleInfo.isAuthInTenant()) {
        ...
        Int16 retCode = ComUser::getAuthNameFromAuthID(roleInfo.getAuthCreator(), GrantorNameString,
                                                       sizeof(GrantorNameString), length, FALSE, CmpCommon::diags());
  ...
  for (size_t g = 0; g < grantees.entries(); g++) {
    ...
    if (grantees[g]->isPublic()) {
      ...
      if (authStatus == CmpSeabaseDDLauth::STATUS_ERROR) return;
      ...
      if (authEntry.isTenant() || authEntry.isRole()) {
        *CmpCommon::diags() << DgSqlCode(-CAT_IS_NOT_CORRECT_AUTHID) << DgString1("user or group")
      ...
      if (noRoleGrantToGroups && authEntry.isUserGroup()) {
        *CmpCommon::diags() << DgSqlCode(-CAT_ROLE_TO_GROUP_NOT_ALLOWED) << DgString0(granteeName.data());
        return;
    ...
    if (hasValue(granteeIDs, granteeID)) {
      *CmpCommon::diags() << DgSqlCode(-CAT_DUPLICATE_USERS_IN_LIST) << DgString0(granteeName.data());
      return;
    }
  ...
  if (pParseNode->isWithAdminOptionSpecified()) {
    if (pParseNode->isGrantRole()) grantDepth = -1;
  ...
  if (pParseNode->isGrantRole()) {
    ...
    privStatus = roleCommand.grantRole(roleIDs, roleNames, grantorIDs, grantorNames, PrivAuthClass::USER, granteeIDs,
                                       granteeNames, granteeClasses, grantDepth);
  } else {
    commandString = "REVOKE ROLE";
    privStatus = roleCommand.revokeRole(roleIDs, granteeIDs, granteeClasses, grantorIDs, withAdminOptionSpecified,
                                        grantDepth, privDropBehavior);
  }

  if (privStatus == STATUS_ERROR && CmpCommon::diags()->getNumber(DgSqlCode::ERROR_) == 0) {
  ...
  str_sprintf(buf,
              "update %s.\"%s\".%s set auth_redef_time = %ld "
              "where auth_id in (%s)",
  ...
  // FIXME roleCommand.grantRole/roleCommand.revokeRole,which will be error with ERROR[1018],and partial operation has
  // been successful
  if (!granteeIDAndNameForGroupMember.empty()) {
    ...
    if (!granteeIDAndNameForGroupMember.empty()) {
      ...
      if (!memberIDs.empty()) {
        ...
        NAString privMgrMDLoc;
        CONCAT_CATSCH(privMgrMDLoc, systemCatalog.c_str(), SEABASE_PRIVMGR_SCHEMA);
        PrivMgrRoles role(std::string(trafMDLocation.data()), std::string(privMgrMDLoc.data()), CmpCommon::diags());
        // I don't care
        std::vector<std::string> roleNames;
        ...
        for (std::vector<int32_t>::iterator itor = memberIDs.begin(); itor != memberIDs.end(); itor++) {
          ...
          if (PrivStatus::STATUS_ERROR !=
              role.fetchRolesForAuth(*itor, roleNames, roleIDs_, roleDepths, grantees, true)) {
            if (!roleIDs_.empty()) {
...
// *                                                                           *
// *****************************************************************************
static void grantSeabaseComponentPrivilege(const std::string &systemCatalog, StmtDDLGrantComponentPrivilege *pParseNode)

{
  ...
  if (ComUser::getAuthIDFromAuthName(granteeName.data(), granteeID, CmpCommon::diags()) != 0) return;

  if (CmpSeabaseDDLauth::isTenantID(granteeID) || CmpSeabaseDDLauth::isUserGroupID(granteeID)) {
    *CmpCommon::diags() << DgSqlCode(-CAT_IS_NOT_CORRECT_AUTHID) << DgString0(granteeName.data())
  ...
  if (grantedBy != NULL) {
    ...
    if (ComUser::getAuthIDFromAuthName(grantedByName.data(), grantedByID, CmpCommon::diags()) != 0) return;

    // Verify that the grantedBy authID is valid (user or role)
    if (!CmpSeabaseDDLauth::isUserID(grantedByID) && !CmpSeabaseDDLauth::isRoleID(grantedByID)) {
      *CmpCommon::diags() << DgSqlCode(-CAT_IS_NOT_CORRECT_AUTHID) << DgString0(grantedByName.data())
                          << DgString1("user or non public role");
      return;
    }

    // If grantedByName is a user, grantor must be DB__ROOT (or MANAGE_COMPONENTS)
    if (CmpSeabaseDDLauth::isUserID(grantedByID) && (ComUser::getCurrentUser() != grantedByID)) {
      if (!ComUser::isRootUserID()) {
        PrivMgrComponentPrivileges componentPrivileges;
  ...
  if (pParseNode->isWithGrantOptionSpecified() && CmpSeabaseDDLauth::isRoleID(granteeID) &&
  ...
  retcode = privInterface.grantComponentPrivilege(componentName, operationNamesList, grantorID, grantorName, granteeID,
                                                  granteeName.data(), grantDepth);
...

{
  ...
  if (!CmpSeabaseDDLauth::isUserID(granteeID) && !CmpSeabaseDDLauth::isRoleID(granteeID) &&
  ...
  if (pParseNode->isGrantOptionForSpecified()) isGOFSpecified = true;
  ...
  retcode = privInterface.revokeComponentPrivilege(componentName, operationNamesList, grantorID, granteeNameStr,
                                                   isGOFSpecified, dropBehavior);
  if (retcode == STATUS_ERROR && CmpCommon::diags()->getNumber(DgSqlCode::ERROR_) == 0)
    SEABASEDDL_INTERNAL_ERROR("REVOKE COMPONENT PRIVILEGE command");
}
//****************** End of revokeSeabaseComponentPrivilege ********************

short CmpSeabaseDDL::setupHbaseOptions(ElemDDLHbaseOptions *hbaseOptionsClause, int numSplits, const NAString &objName,
                                       NAList<HbaseCreateOption *> &hbaseCreateOptions, NAString &hco) {
  ...
  int numHbaseOptions = 0;
  if (hbaseOptionsClause) {
    for (CollIndex i = 0; i < hbaseOptionsClause->getHbaseOptions().entries(); i++) {
      HbaseCreateOption *hbaseOption = hbaseOptionsClause->getHbaseOptions()[i];
  ...
  if (numSplits > 0 /* i.e. a salted table or SPLIT BY used */) {
    ...
    double maxFileSize = CmpCommon::getDefaultNumeric(HBASE_SALTED_TABLE_MAX_FILE_SIZE);
    NABoolean usePerTableSplitPolicy = (CmpCommon::getDefault(HBASE_SALTED_TABLE_SET_SPLIT_POLICY) == DF_ON);
    ...
    if (usePerTableSplitPolicy && !splitPolicyOptionSpecified) {
      const char *saltedTableSplitPolicy = "org.apache.hadoop.hbase.regionserver.ConstantSizeRegionSplitPolicy";
      hbaseOption = new (STMTHEAP) HbaseCreateOption("SPLIT_POLICY", saltedTableSplitPolicy);
      hbaseCreateOptions.insert(hbaseOption);
  ...
  NAString encryption = CmpCommon::getDefaultString(HBASE_ENCRYPTION_OPTION);
  NAString dataBlockEncoding = CmpCommon::getDefaultString(HBASE_DATA_BLOCK_ENCODING_OPTION);
  NAString compression = CmpCommon::getDefaultString(HBASE_COMPRESSION_OPTION);
  NAString flushSize = CmpCommon::getDefaultString(HBASE_MEMSTORE_FLUSH_SIZE_OPTION);
  HbaseCreateOption *hbaseOption = NULL;

  char optionStr[200];
  if (!encryption.isNull() && !encryptionOptionSpecified) {
    hbaseOption = new (STMTHEAP) HbaseCreateOption("ENCRYPTION", encryption.data());
    hbaseCreateOptions.insert(hbaseOption);

    if (ActiveSchemaDB()->getDefaults().userDefault(HBASE_ENCRYPTION_OPTION) == TRUE) {
      numHbaseOptions += 1;
  ...
  if (!dataBlockEncoding.isNull() && !dataBlockEncodingOptionSpecified) {
    hbaseOption = new (STMTHEAP) HbaseCreateOption("DATA_BLOCK_ENCODING", dataBlockEncoding.data());
    hbaseCreateOptions.insert(hbaseOption);

    if (ActiveSchemaDB()->getDefaults().userDefault(HBASE_DATA_BLOCK_ENCODING_OPTION) == TRUE) {
      numHbaseOptions += 1;
  ...
  if (!flushSize.isNull() && !memstoreFlushSizeOptionSpecified) {
    hbaseOption = new (STMTHEAP) HbaseCreateOption("MEMSTORE_FLUSH_SIZE", flushSize.data());
    hbaseCreateOptions.insert(hbaseOption);

    if (ActiveSchemaDB()->getDefaults().userDefault(HBASE_MEMSTORE_FLUSH_SIZE_OPTION) == TRUE) {
      numHbaseOptions += 1;
  ...
  if (!compression.isNull() && !compressionOptionSpecified) {
    hbaseOption = new (STMTHEAP) HbaseCreateOption("COMPRESSION", compression.data());
    hbaseCreateOptions.insert(hbaseOption);

    if (ActiveSchemaDB()->getDefaults().userDefault(HBASE_COMPRESSION_OPTION) == TRUE) {
      numHbaseOptions += 1;
  ...
  if (hco.length() > HBASE_OPTIONS_MAX_LENGTH) {
    *CmpCommon::diags() << DgSqlCode(-CAT_INVALID_HBASE_OPTIONS_CLAUSE) << DgString0(objName);
    return -1;
  }
  return 0;
}
...
//     0 - success
// ----------------------------------------------------------------------------
short CmpSeabaseDDL::checkAndFixupAuthIDs(ExeCliInterface *cliInterface, NABoolean checkOnly,
                                          NASet<NAString> &mappingList) {
  // Create conflict list
  AuthConflictList conflictList(STMTHEAP);
  if (conflictList.generateConflicts(cliInterface) < 0) return -2;

  // no conflicts, just return
  if (conflictList.entries() == 0) return 0;

  // update conflicts
  if (!checkOnly) {
    // objects
    if (conflictList.updateObjects(cliInterface) < 0) return -1;

    // schema_privileges
    if (conflictList.updatePrivs(cliInterface, SEABASE_PRIVMGR_SCHEMA, PRIVMGR_SCHEMA_PRIVILEGES) < 0) return -1;

    // column_privileges
    if (conflictList.updatePrivs(cliInterface, SEABASE_PRIVMGR_SCHEMA, PRIVMGR_COLUMN_PRIVILEGES) < 0) return -1;

    // object_privileges - should be the last step
    if (conflictList.updatePrivs(cliInterface, SEABASE_PRIVMGR_SCHEMA, PRIVMGR_OBJECT_PRIVILEGES) < 0) return -1;
  }
...
short CmpSeabaseDDL::setLockError(short retcode) {
  if (retcode == -HBASE_LOCK_ROLLBACK_ERROR) {
    ...
    *CmpCommon::diags() << DgSqlCode(EXE_ROW_LEVEL_LOCK_LOCKREQUIRED_ERROR) << DgString0(", NOT IN TRANSACTION");
    processReturn();
    return 0;
...
short CmpSeabaseDDL::lockRequired(ExpHbaseInterface *ehi, NATable *naTable, const NAString catalogNamePart,
  ...
  if (gEnableRowLevelLock && CmpCommon::getDefault(TRAF_OBJECT_LOCK) == DF_OFF) {
    ...
    short retcode = ehi->lockRequired(extNameForHbase, lockMode, FALSE /* useHbaseXN */, FALSE /* replSync */,
                                      FALSE /* incrementalBackup */, FALSE /* asyncOperation */,
                                      TRUE /* noConflictCheck */, registerRegion);
    return setLockError(retcode);
  }
  return 0;
}

short CmpSeabaseDDL::lockRequired(const NATable *naTable, const NAString catalogNamePart, const NAString schemaNamePart,
...
short CmpSeabaseDDL::lockRequired(ExpHbaseInterface *ehi, const NAString extNameForHbase, HBaseLockMode lockMode,
                                  NABoolean useHbaseXn, const NABoolean replSync, const NABoolean incrementalBackup,
                                  NABoolean asyncOperation, NABoolean noConflictCheck, NABoolean registerRegion) {
  if (gEnableRowLevelLock && CmpCommon::getDefault(TRAF_OBJECT_LOCK) == DF_OFF) {
    short retcode = ehi->lockRequired(extNameForHbase, lockMode, FALSE /* useHbaseXN */, FALSE /* replSync */,
...
NABoolean CmpSeabaseDDL::isSystemSchema(NAString &schName) {
  for (int i = 0; i < 16; i++) {
    if (systemSchemas[i] == schName) return TRUE;
  }

  // for backup schema
  NAString bachupSch = NAString(TRAF_BACKUP_MD_PREFIX);
  if (schName.index(bachupSch, bachupSch.length(), 0, NAString::exact) != NA_NPOS) return TRUE;
...
void CmpSeabaseDDL::enableSystemObjectCache() { systemObjectInfoCacheEnabled_ = TRUE; }

short CmpSeabaseDDL::deleteFromXDCDDLTable(ExeCliInterface *cliInterface, const char *catName, const char *schName,
  ...
  str_sprintf(buf,
              "delete from %s.\"%s\".%s where catalog_name = '%s' and schema_name = '%s' and object_name = '%s' and "
              "object_type = '%s'",
              getSystemCatalog(), SEABASE_XDC_MD_SCHEMA, XDC_DDL_TABLE, catName, schName, objName, objType);
  cliRC = cliInterface->executeImmediate(buf);
...
short CmpSeabaseDDL::updateSeabaseXDCDDLTable(ExeCliInterface *cliInterface, const char *catName, const char *schName,
                                              const char *objName, const char *objType) {
  if (ddlQuery_ == NULL) {
  ...
  for (int i = 0; i < numRows; i++) {
    ...
    str_sprintf(queryBuf,
                "insert into %s.\"%s\".%s values ('%s', '%s', '%s', '%s', %d, cast(? as char(%d bytes) character set "
                "utf8 not null))",
                getSystemCatalog(), SEABASE_XDC_MD_SCHEMA, XDC_DDL_TABLE, catName, quotedSchName.data(),
                quotedObjName.data(), objType, i, currDataLen);
    cliRC = cliInterface->executeImmediateCEFC(queryBuf, &dataToInsert[currPos], currDataLen, NULL, NULL, NULL);
...
int CmpSeabaseDDL::lockTruncateTable(ExeCliInterface *cliInterface, long objUid) {
  int cliRC = 0;
  ...
  char query[2000];
  str_sprintf(query, "update %s.\"%s\".%s set flags = bitor(flags, %d) where object_uid = %ld", TRAFODION_SYSCAT_LIT,
              SEABASE_MD_SCHEMA, SEABASE_OBJECTS, MD_OBJECTS_TRUNCATE_IN_PROGRESS, objUid);
  cliRC = cliInterface->executeImmediate(query);
  if (cliRC < 0) {
    cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
...
int CmpSeabaseDDL::unlockTruncateTable(ExeCliInterface *cliInterface, long objUid) {
  int cliRC = 0;
  ...
  char query[2000];
  str_sprintf(query, "update %s.\"%s\".%s set flags = bitand(flags, %d) where object_uid = %ld", TRAFODION_SYSCAT_LIT,
              SEABASE_MD_SCHEMA, SEABASE_OBJECTS, ~MD_OBJECTS_TRUNCATE_IN_PROGRESS, objUid);
  cliRC = cliInterface->executeImmediate(query);
  if (cliRC < 0) {
    cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
...
long CmpSeabaseDDL::getIndexFlags(ExeCliInterface *cliInterface, long objUid, long indexUID) {
  int cliRC = 0;
  long flags = -1;
...
int CmpSeabaseDDL::isBRInProgress(ExeCliInterface *cliInterface, const char *catalogName, const char *schemaName,
                                  const char *objectName, long &objUid)

{
  long objFlags = 0;
  objUid =
      getObjectUID(cliInterface, catalogName, schemaName, objectName, NULL, NULL, NULL, NULL, false, true, &objFlags);
  if (objUid < 0) {
    return -1;
  }
  if (isMDflagsSet(objFlags, MD_OBJECTS_BR_IN_PROGRESS)) {
    NAString brTag;
    NAString extTableName =
        NAString(catalogName) + "." + "\"" + NAString(schemaName) + "\"" + "." + NAString(objectName);
    getTextFromMD(cliInterface, objUid, COM_LOCKED_OBJECT_BR_TAG, 0, brTag);
    *CmpCommon::diags() << DgSqlCode(-4264) << DgString0(extTableName) << DgString1(brTag);
    return 1;
  }
...
int CmpSeabaseDDL::updateCachesAndStoredDesc(ExeCliInterface *cliInterface, const NAString &catName,
                                             const NAString &schName, const NAString &objName, ComObjectType objType,
                                             UInt32 operations, RemoveNATableParam *rmNATableParam,
  ...
  if (isCacheAndStoredDescOpSet(operations, UPDATE_SHARED_CACHE)) {
    insertIntoSharedCacheList(catName, schName, objName, objType, ddlOp);
  }

  if (isCacheAndStoredDescOpSet(operations, UPDATE_REDEF_TIME)) {
    if (updOjbRedefParam == NULL) return -1;

    rc = updateObjectRedefTime(cliInterface, catName, schName, objName, comObjectTypeLit(objType),
                               updOjbRedefParam->redefTime_, updOjbRedefParam->objUID_, updOjbRedefParam->genDesc_,
                               updOjbRedefParam->genStats_, updOjbRedefParam->flushData_);
    if (rc) return rc;
  }

  if (isCacheAndStoredDescOpSet(operations, UPDATE_XDC_DDL_TABLE)) {
    rc = updateSeabaseXDCDDLTable(cliInterface, catName.data(), schName.data(), objName.data(),
                                  comObjectTypeLit(objType));
    if (rc) return rc;
  }

  if (isCacheAndStoredDescOpSet(operations, REMOVE_NATABLE)) {
    if (rmNATableParam == NULL) return -1;

    CorrName cn(objName, STMTHEAP, schName, catName);
    ActiveSchemaDB()->getNATableDB()->removeNATable(cn, rmNATableParam->qiScope_, objType, rmNATableParam->ddlXns_,
                                                    rmNATableParam->atCommit_, rmNATableParam->objUID_,
...
int CmpSeabaseDDL::updateCachesAndStoredDescByNATable(ExeCliInterface *cliInterface, const NAString &catName,
                                                      const NAString &schName, const NAString &objName,
                                                      ComObjectType objType, UInt32 operations, NATable *natable,
                                                      RemoveNATableParam *rmNATableParam,
                                                      SharedCacheDDLInfo::DDLOperation ddlOp) {
  if (natable->incrBackupEnabled()) setCacheAndStoredDescOp(operations, UPDATE_XDC_DDL_TABLE);
  if (natable->isStoredDesc()) setCacheAndStoredDescOp(operations, UPDATE_SHARED_CACHE);

  UpdateObjRefTimeParam updObjRefParam(-1, natable->objectUid().get_value(), natable->isStoredDesc(),
                                       natable->storedStatsAvail());

  return updateCachesAndStoredDesc(cliInterface, catName, schName, objName, objType, operations, rmNATableParam,
                                   &updObjRefParam, ddlOp);
}

int CmpSeabaseDDL::checkPartition(ElemDDLPartitionNameAndForValuesArray *partArray, TableDesc *tableDesc,
                                  BindWA &bindWA, NAString &objectName, NAString &invalidPartName,
                                  std::vector<NAString> *partEntityNameVec,
                                  std::map<NAString, NAString> *partEntityNameMap) {
  if (!partArray || !tableDesc) return -1;

  NAString partEntityName;
  std::vector<NAString> *partEntityNameVecTmp = partEntityNameVec;
  if (!partEntityNameVec) partEntityNameVecTmp = new (STMTHEAP) std::vector<NAString>;

  std::map<NAString, NAString> *partEntityNameMapTmp = partEntityNameMap;
  if (!partEntityNameMap) partEntityNameMapTmp = new (STMTHEAP) std::map<NAString, NAString>;
  ...
  for (int i = 0; i < partArray->entries(); ++i) {
    if ((*partArray)[i]->isPartitionForValues()) {
      ...
      if (!partEntityNames || partEntityNames->entries() == 0) return 2;
  ...
  if (partEntityNameVecTmp->size() > 1) {
    ...
    if (it != partEntityNameVecTmp->end()) {
      ...
      if (itm != partEntityNameMapTmp->end()) invalidPartName = itm->second;
...
DDLScopeInterface::~DDLScopeInterface() { currentContext_->ddlObjsList().endDDLOp(allClear_, diags_); }

UpdateObjRefTimeParam::UpdateObjRefTimeParam(long rt, long objUID, NABoolean force, NABoolean genStats,
                                             NABoolean flushData) {
  redefTime_ = rt;
  objUID_ = objUID;
  genDesc_ = force;
...
RemoveNATableParam::RemoveNATableParam(ComQiScope qiScope, NABoolean ddlXns, NABoolean atCommit, long objUID,
                                       NABoolean noCheck) {
  qiScope_ = qiScope;
  ddlXns_ = ddlXns_;
  atCommit_ = atCommit_;
...
void CmpSeabaseDDL::processMDPartTables(NABoolean create, NABoolean drop, NABoolean upgrade) {
  ExeCliInterface cliInterface(STMTHEAP, 0, NULL, CmpCommon::context()->sqlSession()->getParentQid());
