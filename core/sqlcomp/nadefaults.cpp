...
ValidateUIntForOptimizedScan::ValidateUIntForOptimizedScan()
    : ValidateNumericRange(VALID_UINT, (float)BloomFilterRT::minPackedLength(), (float)UINTmax_) {}
...
#define DD(name, value, validator) \
  { name, "" #name "", value, validator }
#define FDD(name, value, validator, flags) \
  { name, "" #name "", value, validator, flags }
...
#define DD0_400000(name, value)     DD(name, value, &validate0_400000)
#define XDD0_200000(name, value)    XDD(name, value, &validate0_200000)
#define XDD0_524288(name, value)    XDD(name, value, &validate0_524288)
...
#define DDui0_2(name, value)        DD(name, value, &validateUIntFrom0To2)
...
#define DDui_OptScan(name, value)   DD(name, value, &validateUIntForOptimizedScan)
...
#define DD1_8192(name, value)       DD(name, value, &validate1_8192)
#define DD1_40000(name, value)      DD(name, value, &validate1_40000)
...
const Validate_0_400000 validate0_400000;                               // allows zero to 400000 (integer)
const Validate_0_524288 validate0_524288;                               // allows zero to 524288 (integer)
...
const ValidateUIntFrom0To2 validateUIntFrom0To2;                        // integer from 0 to 2
...
const ValidateUIntForOptimizedScan validateUIntForOptimizedScan;
...
const Validate_1_8192 validate1_8192;               // allows 1 to 8192 (integer) which is max character size supported.
const Validate_1_40000 validate1_40000;  // allows 1 to 40000 (integer) which is max character size supported.
...
static DefaultDefault defaultDefaults[] = {
    DDflt0_(ACCEPTABLE_INPUTESTLOGPROP_ERROR, "0.5"),

    DD1_40000(AGGR_PUSHDOWN_THRESHOLD, "4000"),
    ...
    DDkwd__(ALLOW_JSON_FUNCTIONS, "OFF"),
    ...
    // If ON, allow granting roles to groups
    // If OFF, don't allow granting roles to groups
    // If SYSTEM
    //    allow granting roles to groups unless
    //    SQLMX_REGRESS is turned on, then don't allow grants
    // Added to put less stress on LDAP during regression runs
    // And for customer who don't plan to use LDAP groups
    DDkwd__(ALLOW_ROLE_GRANTS_TO_GROUPS, "SYSTEM"),

    DDkwd__(ALLOW_SUBQ_IN_SET, "SYSTEM"),

    // Tenants can allocate node usage through resource groups (RG) or adaptive
    // segmentation (AS).  Use this cqd to turn off RG nod allocation.
    DDkwd__(ALLOW_TENANT_RESOURCE_GROUPS, "ON"), DDkwd__(ALLOW_TRUNCATE_IN_USER_XN, "OFF"),

    DDkwd__(ALLOW_UNEXTERNALIZED_MAINTAIN_OPTIONS, "OFF"),

    // Allow users to grant privileges to role using the with grant option
    DDkwd__(ALLOW_WGO_FOR_ROLES, "OFF"),

    DDSkwd__(ALTPRI_ESP, ""), DDSkwd__(ALTPRI_MASTER, ""),

    DDS_____(AQR_ENTRIES, ""), DDkwd__(AQR_WNR, "ON"), DDkwd__(AQR_WNR_DELETE_NO_ROWCOUNT, "OFF"),
    DDkwd__(AQR_WNR_EXPLAIN_INSERT, "OFF"), DDkwd__(AQR_WNR_INSERT_CLEANUP, "OFF"),
    DDkwd__(AQR_WNR_LOCK_INSERT_TARGET, "OFF"),

    DDkwd__(ARKCMP_FAKE_HW, "OFF"), DDkwd__(ASG_FEATURE, "ON"),
    // Set ASM cache
    DDkwd__(ASM_ALLOWED, "ON"),
    // Precompute statistics in ASM
    DDkwd__(ASM_PRECOMPUTE, "OFF"), DDkwd__(ASYMMETRIC_JOIN_TRANSFORMATION, "MAXIMUM"),

    // Adaptive segmentation
    DDint__(AS_AFFINITY_VALUE, "-2"), DDui___(AS_CYCLIC_ESP_PLACEMENT, "1"), DDui___(AS_FRAGMENT_SIZE_THRESHOLD, "2"),
    DDkwd__(AS_SHIFT_FULL_FRAGMENTS, "OFF"),

    DDkwd__(AS_TOLERATE_ILL_DEFINED_TENANTS, "SYSTEM"), DDkwd__(ATTEMPT_ASYNCHRONOUS_ACCESS, "ON"),
    XDDkwd__(ATTEMPT_ESP_PARALLELISM, "ON"), DDkwd__(ATTEMPT_REVERSE_SYNCHRONOUS_ORDER, "ON"),

    DDkwd__(AUTOMATIC_RECOMPILATION, "OFF"),

    DDkwd__(AUTO_ADJUST_CACHEROWS, "ON"), DDkwd__(AUTO_QUERY_RETRY, "SYSTEM"),
    XDDkwd__(AUTO_QUERY_RETRY_WARNINGS, "OFF"),

    DDkwd__(BASE_NUM_PAS_ON_ACTIVE_PARTS, "OFF"), DDint__(BEGIN_TRANSACTION_FOR_SELECT, "1"),

    // see comments in DefaultConstants.h
    DDkwd__(BIGNUM_IO, "SYSTEM"),

    DD_____(BIGTABLE_INSTANCE_ID, ""), DD_____(BIGTABLE_PROJECT_ID, ""),
    ...
    DDflte_(BMO_MEMORY_EQUAL_QUOTA_SHARE_RATIO, "0.5"), DDui___(BMO_MEMORY_ESTIMATE_OUTLIER_FACTOR, "10"),
    DDflte_(BMO_MEMORY_ESTIMATE_RATIO_CAP, "0.7"), DDui___(BMO_MEMORY_LIMIT_LOWER_BOUND_HASHGROUPBY, "25"),
    DDui___(BMO_MEMORY_LIMIT_LOWER_BOUND_HASHJOIN, "25"), DDui___(BMO_MEMORY_LIMIT_LOWER_BOUND_SORT, "200"),
    XDDui___(BMO_MEMORY_LIMIT_PER_NODE_IN_MB, "10240"), DDui___(BMO_MEMORY_LIMIT_UPPER_BOUND, "800"),
    ...
    DDkwd__(CACHE_HISTOGRAMS, "ON"), DDkwd__(CACHE_HISTOGRAMS_CHECK_FOR_LEAKS, "OFF"),
    DD0_200000(CACHE_HISTOGRAMS_IN_KB, "32768"), DDkwd__(CACHE_HISTOGRAMS_MONITOR_HIST_DETAIL, "OFF"),
    DDkwd__(CACHE_HISTOGRAMS_MONITOR_MEM_DETAIL, "OFF"), DD_____(CACHE_HISTOGRAMS_MONITOR_OUTPUT_FILE, ""),

    DD_____(CACHE_HISTOGRAMS_TRACE_OUTPUT_FILE, ""), DDkwd__(CALL_EMBEDDED_ARKCMP, "OFF"),
    ...
    DDkwd__(CAT_ANSI_PRIVS_FOR_TENANT, "OFF"),
    ...
    DDint__(CLIENT_MAX_STATEMENT_POOLING, "0"),
    ...
    DDint__(COMPILER_TRACKING_INTERVAL, "0"), DD_____(COMPILER_TRACKING_LOGFILE, "NONE"),
    DDkwd__(COMPILER_TRACKING_LOGTABLE, "OFF"),

    DDkwd__(COMPILE_TIME_MONITOR, "OFF"), DD_____(COMPILE_TIME_MONITOR_LOG_ALLTIME_ONLY, "OFF"),
    DD_____(COMPILE_TIME_MONITOR_OUTPUT_FILE, "NONE"),

    DDkwd__(COMPOSITE_LATERAL_VIEW_UNNEST_OPT, "OFF"),

    // Switch between new aligned internal format and exploded format
    DDkwd__(COMPRESSED_INTERNAL_FORMAT, "SYSTEM"), DDkwd__(COMPRESSED_INTERNAL_FORMAT_BMO, "SYSTEM"),
    DDkwd__(COMPRESSED_INTERNAL_FORMAT_BMO_AFFINITY, "ON"), DDkwd__(COMPRESSED_INTERNAL_FORMAT_BULK_MOVE, "ON"),
    DDint__(COMPRESSED_INTERNAL_FORMAT_BULK_VARCHAR_MAX, "20000"),
    DDflt0_(COMPRESSED_INTERNAL_FORMAT_DEFRAG_RATIO, "0.30"), DDkwd__(COMPRESSED_INTERNAL_FORMAT_EXPLAIN, "OFF"),
    DDui1__(COMPRESSED_INTERNAL_FORMAT_MIN_ROW_SIZE, "32"),
    DDkwd__(COMPRESSED_INTERNAL_FORMAT_ROOT_DOES_CONVERSION, "ON"),
    ...
    DDkwd__(COMP_BOOL_1, "OFF"), DDkwd__(COMP_BOOL_100, "OFF"), DDkwd__(COMP_BOOL_101, "OFF"),
    DDkwd__(COMP_BOOL_102, "OFF"), DDkwd__(COMP_BOOL_103, "OFF"), DDkwd__(COMP_BOOL_104, "OFF"),
    ...
    DDkwd__(COMP_BOOL_109, "ON"),  // Controls whether predicates of the form X < c OR X > c are represented as X <> c
                                   // in RangeSpecRef definitions

    DDkwd__(COMP_BOOL_11, "OFF"), DDkwd__(COMP_BOOL_110, "OFF"), DDkwd__(COMP_BOOL_111, "OFF"),
    DDkwd__(COMP_BOOL_112, "OFF"), DDkwd__(COMP_BOOL_113, "OFF"),
    DDkwd__(COMP_BOOL_114, "ON"),  // If set to "OFF", turns off JIRA TRAFODION-3296 fix
    DDkwd__(COMP_BOOL_115, "OFF"), DDkwd__(COMP_BOOL_116, "OFF"), DDkwd__(COMP_BOOL_117, "OFF"),
    DDkwd__(COMP_BOOL_118, "OFF"),  // soln 10-100508-0135 - allow undo of fix.
    DDkwd__(COMP_BOOL_119, "OFF"),

    DDkwd__(COMP_BOOL_12, "OFF"), DDkwd__(COMP_BOOL_120, "OFF"),
    DDkwd__(COMP_BOOL_122, "ON"),  // Solution 10-081203-7708 fix
    DDkwd__(COMP_BOOL_123, "OFF"), DDkwd__(COMP_BOOL_124, "OFF"), DDkwd__(COMP_BOOL_125, "ON"),
    DDkwd__(COMP_BOOL_126, "OFF"), DDkwd__(COMP_BOOL_127, "ON"), DDkwd__(COMP_BOOL_128, "ON"),
    DDkwd__(COMP_BOOL_129, "ON"),

    DDkwd__(COMP_BOOL_13, "OFF"), DDkwd__(COMP_BOOL_130, "ON"), DDkwd__(COMP_BOOL_131, "OFF"),
    DDkwd__(COMP_BOOL_132, "OFF"), DDkwd__(COMP_BOOL_133, "OFF"), DDkwd__(COMP_BOOL_134, "ON"),
    DDkwd__(COMP_BOOL_135, "ON"), DDkwd__(COMP_BOOL_136, "OFF"),
    ...
    DDkwd__(COMP_BOOL_14, "ON"), DDkwd__(COMP_BOOL_140, "ON"),
    DDkwd__(COMP_BOOL_141, "ON"),  // Used for testing MC UEC adjustment for uplifting join cardinality
    DDkwd__(COMP_BOOL_144,
            "OFF"),  // only Key columns usage as a part of materialization of disjuncts is controlled by the CQD
    ...
    DDkwd__(
        COMP_BOOL_15,
        "ON"),  // If ON, use runtime ROUND for BigNums; if OFF, rewrite ROUND for BigNums in terms of other functions
    DDkwd__(COMP_BOOL_150, "OFF"), DDkwd__(COMP_BOOL_151, "OFF"), DDkwd__(COMP_BOOL_152, "OFF"),
    DDkwd__(COMP_BOOL_153, "ON"),  // skew buster: ON == use round robin, else Co-located.
    DDkwd__(COMP_BOOL_154, "OFF"), DDkwd__(COMP_BOOL_155, "OFF"),
    DDkwd__(COMP_BOOL_156, "ON"),  // Used by RTS to turn on RTS Stats collection for ROOT operators
    DDkwd__(COMP_BOOL_158,
            "ON"),  // ON --> allows equijoins on VARCHAR/VARCHAR and CHAR/VARCHAR to be rewritten as VEGPreds
    DDkwd__(COMP_BOOL_159, "OFF"),

    DDkwd__(COMP_BOOL_160, "OFF"), DDkwd__(COMP_BOOL_161, "OFF"),
    DDkwd__(COMP_BOOL_162, "ON"),  // transform NOT EXISTS subquery using anti_semijoin instead of Join-Agg
    DDkwd__(COMP_BOOL_163, "OFF"), DDkwd__(COMP_BOOL_164, "OFF"),
    DDkwd__(COMP_BOOL_165, "ON"),   // set to 'ON' in M5 for SQ
    DDkwd__(COMP_BOOL_166, "OFF"),  // ON --> turn off fix for 10-100310-8659.
    DDkwd__(COMP_BOOL_167, "OFF"), DDkwd__(COMP_BOOL_168, "ON"), DDkwd__(COMP_BOOL_169, "OFF"),

    DDkwd__(COMP_BOOL_171, "OFF"),  // if ON don't turn off straw scan if there is no runtime partition elimination
    DDkwd__(COMP_BOOL_172, "OFF"), DDkwd__(COMP_BOOL_173, "OFF"),  //  fix: make odbc params nullable
    DDkwd__(COMP_BOOL_174, "ON"),                                  // internal usage: merge stmt
    DDkwd__(COMP_BOOL_175, "OFF"),                                 // internal usage: merge stmt
    DDkwd__(COMP_BOOL_176, "OFF"), DDkwd__(COMP_BOOL_177, "OFF"), DDkwd__(COMP_BOOL_178, "OFF"),

    DDkwd__(COMP_BOOL_18, "OFF"), DDkwd__(COMP_BOOL_183, "OFF"),
    DDkwd__(COMP_BOOL_184, "ON"),  // ON => use min probe size for mdam. Using min probe size of 1 or 2 currently has a
                                   // bug so this is not the default. OFF => use default probe size of 100
    DDkwd__(COMP_BOOL_185, "ON"),  // Fix, allows extract(year from current_date) to be treated as a userinput
    DDkwd__(COMP_BOOL_186, "OFF"), DDkwd__(COMP_BOOL_187, "OFF"),  // reserved for internal usage
    DDkwd__(COMP_BOOL_188, "OFF"), DDkwd__(COMP_BOOL_189, "OFF"),  // reserved for internal usage

    DDkwd__(COMP_BOOL_19, "OFF"), DDkwd__(COMP_BOOL_190, "OFF"),
    DDkwd__(COMP_BOOL_191, "OFF"),  // Temp for UDF metadata switch
    DDkwd__(COMP_BOOL_192, "OFF"), DDkwd__(COMP_BOOL_193, "OFF"),
    DDkwd__(COMP_BOOL_194, "ON"),  // If "OFF", turns off JIRA Trafodion 3325 fix
    DDkwd__(COMP_BOOL_196, "OFF"), DDkwd__(COMP_BOOL_197, "OFF"), DDkwd__(COMP_BOOL_198, "OFF"),
    DDkwd__(COMP_BOOL_199, "ON"),

    DDkwd__(COMP_BOOL_2, "OFF"), DDkwd__(COMP_BOOL_20, "OFF"),  // ON -> disable ability of stmt to be canceled.
    DDkwd__(COMP_BOOL_200, "OFF"), DDkwd__(COMP_BOOL_201, "OFF"),
    DDkwd__(COMP_BOOL_202, "ON"),                                  // For SQ:
                                                                   ...
                                                                   //   OFF: do include.
                                                                   // Change to ON in M5
    DDkwd__(COMP_BOOL_203, "OFF"), DDkwd__(COMP_BOOL_206, "OFF"),  // For ROS cardinality
    DDkwd__(COMP_BOOL_207, "OFF"),                                 // Internal Usage
    DDkwd__(COMP_BOOL_208, "OFF"),                                 // To control min numb of ESPS.
                                                                   // Internal test use.

    DDkwd__(COMP_BOOL_21, "OFF"), DDkwd__(COMP_BOOL_210, "ON"),
    DDkwd__(COMP_BOOL_211, "ON"),                                  // controls removing constants from group expression
    DDkwd__(COMP_BOOL_217, "OFF"), DDkwd__(COMP_BOOL_219, "OFF"),  // for InMem obj defn
    ...
    DDkwd__(COMP_BOOL_23, "ON"), DDkwd__(COMP_BOOL_24, "OFF"),  // AS enhancement to adjust maxDoP
    DDkwd__(COMP_BOOL_25, "OFF"),                               // Being used in Cardinality Estimation
    DDkwd__(COMP_BOOL_27, "OFF"), DDkwd__(COMP_BOOL_28, "OFF"), DDkwd__(COMP_BOOL_29, "OFF"),

    DDkwd__(COMP_BOOL_3, "OFF"), DDkwd__(COMP_BOOL_30, "ON"), DDkwd__(COMP_BOOL_31, "OFF"),
    DDkwd__(COMP_BOOL_32, "OFF"), DDkwd__(COMP_BOOL_33, "OFF"), DDkwd__(COMP_BOOL_34, "OFF"),
    DDkwd__(COMP_BOOL_35, "OFF"), DDkwd__(COMP_BOOL_36, "OFF"), DDkwd__(COMP_BOOL_37, "OFF"),
    DDkwd__(COMP_BOOL_38, "OFF"), DDkwd__(COMP_BOOL_39, "OFF"),

    DDkwd__(COMP_BOOL_4, "OFF"), DDkwd__(COMP_BOOL_40, "ON"), DDkwd__(COMP_BOOL_41, "OFF"), DDkwd__(COMP_BOOL_42, "ON"),
    DDkwd__(COMP_BOOL_44, "OFF"), DDkwd__(COMP_BOOL_45, "ON"), DDkwd__(COMP_BOOL_46, "OFF"),
    DDkwd__(COMP_BOOL_47, "ON"), DDkwd__(COMP_BOOL_48, "ON"),  // Turned "Off" because of Regression failure
    DDkwd__(COMP_BOOL_49, "OFF"),

    DDkwd__(COMP_BOOL_5, "ON"), DDkwd__(COMP_BOOL_51, "OFF"), DDkwd__(COMP_BOOL_52, "OFF"),
    DDkwd__(COMP_BOOL_53, "ON"),  // Turned "ON" for OCB Cost
    DDkwd__(COMP_BOOL_54, "OFF"), DDkwd__(COMP_BOOL_55, "OFF"), DDkwd__(COMP_BOOL_56, "OFF"),
    DDkwd__(COMP_BOOL_57, "ON"), DDkwd__(COMP_BOOL_59, "OFF"),
    ...
    // costing logic.
    DDkwd__(COMP_BOOL_60, "ON"), DDkwd__(COMP_BOOL_61, "OFF"), DDkwd__(COMP_BOOL_62, "OFF"),
    DDkwd__(COMP_BOOL_63, "OFF"), DDkwd__(COMP_BOOL_64, "OFF"), DDkwd__(COMP_BOOL_65, "OFF"),
    DDkwd__(COMP_BOOL_66, "OFF"), DDkwd__(COMP_BOOL_67, "ON"),  // Being used in Cardinality Estimation
    DDkwd__(COMP_BOOL_68, "ON"), DDkwd__(COMP_BOOL_69, "OFF"),

    DDkwd__(COMP_BOOL_7, "OFF"), DDkwd__(COMP_BOOL_70, "ON"), DDkwd__(COMP_BOOL_71, "OFF"),
    DDkwd__(COMP_BOOL_72, "OFF"), DDkwd__(COMP_BOOL_73, "OFF"), DDkwd__(COMP_BOOL_74, "ON"),
    DDkwd__(COMP_BOOL_75, "ON"), DDkwd__(COMP_BOOL_76, "ON"), DDkwd__(COMP_BOOL_77, "OFF"),
    DDkwd__(COMP_BOOL_78, "OFF"), DDkwd__(COMP_BOOL_79, "ON"),

    DDkwd__(COMP_BOOL_8, "OFF"), DDkwd__(COMP_BOOL_80, "OFF"), DDkwd__(COMP_BOOL_81, "OFF"),
    DDkwd__(COMP_BOOL_82, "OFF"), DDkwd__(COMP_BOOL_83, "ON"), DDkwd__(COMP_BOOL_84, "OFF"),
    DDkwd__(COMP_BOOL_85, "OFF"), DDkwd__(COMP_BOOL_86, "OFF"), DDkwd__(COMP_BOOL_87, "OFF"),
    DDkwd__(COMP_BOOL_88, "OFF"),

    DDkwd__(COMP_BOOL_9, "OFF"), DDkwd__(COMP_BOOL_90, "ON"), DDkwd__(COMP_BOOL_91, "OFF"),
    DDkwd__(COMP_BOOL_92, "OFF"),  // used by generator.
    DDkwd__(COMP_BOOL_93, "ON"),   // turn on pushdown for IUDs involving MVs. Default is off
    DDkwd__(COMP_BOOL_94, "OFF"), DDkwd__(COMP_BOOL_95, "OFF"), DDkwd__(COMP_BOOL_96, "OFF"),
    DDkwd__(COMP_BOOL_97, "OFF"), DDkwd__(COMP_BOOL_98, "ON"), DDkwd__(COMP_BOOL_99, "OFF"),
    DDkwd__(COMP_BOOL_990, "OFF"),

    DDflt0_(COMP_FLOAT_0, "0.002"), DDflt0_(COMP_FLOAT_1, "0.00002"), DDflt0_(COMP_FLOAT_2, "0"),
    DDflt0_(COMP_FLOAT_3, "0.01"), DDflt0_(COMP_FLOAT_4, "1.1"),
    ...
    DDflt__(COMP_FLOAT_7, "1.5"), DDflt__(COMP_FLOAT_8, "0.8"),
    // min expected #groups when HGB under right side of NLJ
    DDflt__(COMP_FLOAT_9, "1002.0"),

    DDint__(COMP_INT_0, "5000"), DDint__(COMP_INT_10, "3"), DDint__(COMP_INT_11, "-1"), DDint__(COMP_INT_12, "15"),
    DDint__(COMP_INT_14, "0"), DDint__(COMP_INT_15, "7"), DDint__(COMP_INT_16, "1000000"),
    DDint__(COMP_INT_17, "1000000"), DDint__(COMP_INT_18, "1"), DDint__(COMP_INT_19, "2"), DDint__(COMP_INT_2, "1"),
    DDint__(COMP_INT_20, "4"), DDint__(COMP_INT_21, "0"),
    DDint__(COMP_INT_22, "0"),  // used to control old parser based INLIST transformation
    // 0 ==> OFF, positive value implies ON and has the effect of implicitly shutting down much of OR_PRED
    // transformations this cqd has been retained as a fallback in case OR_PRED has bugs.
    DDint__(COMP_INT_23, "22"), DDint__(COMP_INT_24, "1000000000"), DDint__(COMP_INT_26, "1"), DDint__(COMP_INT_3, "5"),
    DDint__(COMP_INT_30, "5"), DDint__(COMP_INT_31, "5"), DDint__(COMP_INT_32, "100"),
    ...
    DDint__(COMP_INT_40, "10"),  // this defines the percentage of selectivity after applying equality predicates on
                                 // single column histograms beyond which the optimizer should use MC stats
    DDint__(COMP_INT_43, "3"),  // this is only for testing purposes. Once HIST_USE_SAMPLE_FOR_CARDINALITY_ESTIMATION is
                                // set to ON by default, the value of this CQD should be adjusted
    ...
    DDint__(COMP_INT_74, "0"), DDint__(COMP_INT_77, "0"), DDint__(COMP_INT_79, "0"),
    // this is used temporaraly as value for parallel threshold
    // in case ATTEMPT_ESP_PARALLELISM is set to MAXIMUM
    DDint__(COMP_INT_8, "20"), DDint__(COMP_INT_80, "3"),

    DDint__(COMP_INT_89, "2"), DDint__(COMP_INT_9, "0"), DDint__(COMP_INT_90, "0"), DDint__(COMP_INT_95, "0"),
    DDint__(COMP_INT_98, "512"), DDint__(COMP_INT_99, "10"),

    DD_____(COMP_STRING_1, "NONE"), DD_____(COMP_STRING_2, ""), DD_____(COMP_STRING_5, ""),

    // Configures test points. See the description in DefaultConstants.h.
    DD_____(CONFIGURE_TESTPOINT, ""),

    DDint__(CONNECTBY_MAX_DEEP, "200"), DDint__(CONNECTBY_MAX_SIZE, "100000"),

    DDkwd__(CONSTANT_FOLDING, "OFF"),

    DDkwd__(COSTING_SHORTCUT_GROUPBY_FIX, "ON"), DDflt0_(COST_PROBE_DENSITY_THRESHOLD, ".25"),
    ...
    DDflt0_(CPUCOST_DATARQST_OVHD, ".01"), DDflt0_(CPUCOST_ENCODE_PER_BYTE, ".002"),
    ...
    DDflt0_(CPUCOST_EVAL_FUNC_DEFAULT, "10."), DDflt0_(CPUCOST_EVAL_SIMPLE_PREDICATE, "1."),
    DDflt0_(CPUCOST_EXCHANGE_COST_PER_BYTE, ".002"), DDflt0_(CPUCOST_EXCHANGE_INTERNODE_COST_PER_BYTE, ".008"),
    ...
    DDflt0_(CPUCOST_HASH_PER_KEY, "1.29"), DDflt0_(CPUCOST_LIKE_COMPARE_OVERHEAD, "10."),
    DDflt0_(CPUCOST_LIKE_COMPARE_PER_BYTE, ".1"), DDflt0_(CPUCOST_LOCK_ROW, ".01"),
    ...
    DDflt0_(CPUCOST_SCAN_DSK_TO_DP2_PER_KB, "2.5"), DDflt0_(CPUCOST_SCAN_DSK_TO_DP2_PER_SEEK, "0.0"),
    ...
    DDflt0_(CPUCOST_SCAN_OVH_PER_KB, "0.984215"), DDflt0_(CPUCOST_SCAN_OVH_PER_ROW, "0.0"),
    ...
    DDflt0_(CPUCOST_SUBSET_OPEN, "7000"), DDflt0_(CPUCOST_SUBSET_OPEN_AFTER_FIRST, "1250"),
    ...
    DDflt0_(CSE_TEMP_TABLE_MAX_MAX_SIZE, "1E12"), DDflt0_(CSE_TEMP_TABLE_MAX_SIZE, "1E9"),
    ...
    // Required minimial number of tables in a multi-join to enable
    // data flow optimization.
    DDui___(DATA_FLOW_OPTIMIZATION_NUM_TABLES, "4"),

    DDkwd__(DDL_EXPLAIN, "OFF"), DDkwd__(DDL_TRANSACTIONS, "ON"),

    SDDkwd__(DEFAULT_CHARSET, (char *)SQLCHARSETSTRING_ISO88591), XDDui1__(DEFAULT_DEGREE_OF_PARALLELISM, "2"),

    SDDkwd__(DEFAULT_SCHEMA_ACCESS_ONLY, "OFF"), SDDkwd__(DEFAULT_SCHEMA_NAMETYPE, "SYSTEM"),
...
#define str_DEF_CHUNK_SIZE "5000000.0"
    //
    DDui2__(DEF_CHUNK_SIZE, str_DEF_CHUNK_SIZE), DD_____(DEF_CPU_ARCHITECTURE, ""), DDui1__(DEF_DISCS_ON_CLUSTER, ""),
    DDui1__(DEF_INSTRUCTIONS_SECOND, ""), DDui___(DEF_LOCAL_CLUSTER_NUMBER, ""), DDui___(DEF_LOCAL_SMP_NODE_NUMBER, ""),
    // DEF_MAX_HISTORY_ROWS made external RV 06/21/01 CR 10-010425-2440
    XDDui1__(DEF_MAX_HISTORY_ROWS, "1024"), DDui___(DEF_NUM_BM_CHUNKS, ""),
    DDui1__(DEF_NUM_NODES_IN_ACTIVE_CLUSTERS, ""), DDui1__(DEF_NUM_SMP_CPUS, ""), DDui2__(DEF_PAGE_SIZE, ""),
    DDui1__(DEF_PHYSICAL_MEMORY_AVAILABLE, ""), DDui1__(DEF_TOTAL_MEMORY_AVAILABLE, ""),
    ...
    DDkwd__(DIMENSIONAL_QUERY_OPTIMIZATION, "OFF"), DDkwd__(DISABLE_BUFFERED_INSERTS, "OFF"),
    DDkwd__(DISABLE_READ_ONLY, "OFF"),

    DD_____(DISPLAY_DATA_FLOW_GRAPH, "OFF"), XDDkwd__(DISPLAY_DIVISION_BY_COLUMNS, "OFF"),

    DDkwd__(DONOT_WRITE_XDC_DDL, "OFF"),

    // dop reduction:
    //   0 off
    //   2 by run-time stats
    DDui___(DOP_ADJUST, "0"), DDkwd__(DOP_ADJUST_DEBUG, "OFF"), DDflt0_(DOP_ADJUST_MIN_NUM_ESPS_PER_NODE, "1.0"),
    DDflt0_(DOP_ADJUST_ROWCOUNT_THRESHOLD, "0.0"), DD_____(DOP_ADJUST_RTSTATS_HDFS_PATH, "/user/trafodion/rtstats"),
    DD_____(DOP_ADJUST_RTSTATS_LOCAL_FILE, ""), DDflt0_(DOP_ADJUST_WORK_UNIT, "67108864"),

    // DP2 Cache defaults as of 06/08/98.
    DDui1__(DP2_CACHE_1024_BLOCKS, "152"), DDui1__(DP2_CACHE_16K_BLOCKS, "1024"), DDui1__(DP2_CACHE_2048_BLOCKS, "150"),
    DDui1__(DP2_CACHE_32K_BLOCKS, "512"), DDui1__(DP2_CACHE_4096_BLOCKS, "4096"), DDui1__(DP2_CACHE_512_BLOCKS, "152"),
    ...
    DDflte_(DP2_END_OF_BUFFER_HEADER_SIZE, ".0313"), DDflte_(DP2_EXCHANGE_REQUEST_SIZE, ".0469"),
    ...
    DDflte_(DP2_MESSAGE_HEADER_SIZE, ".0176"), DDui2__(DP2_MESSAGE_HEADER_SIZE_BYTES, "18"),
    ...
    DDkwd__(DYNAMIC_PARAM_DEFAULT_CHARSET, (char *)SQLCHARSETSTRING_ISO88591),
    DDui2__(DYN_PA_QUEUE_RESIZE_INIT_DOWN, "1024"), DDui2__(DYN_PA_QUEUE_RESIZE_INIT_UP, "1024"),
    DDui2__(DYN_QUEUE_RESIZE_FACTOR, "4"), DDui2__(DYN_QUEUE_RESIZE_INIT_DOWN, "4"),
    DDui2__(DYN_QUEUE_RESIZE_INIT_UP, "4"), DDui1__(DYN_QUEUE_RESIZE_LIMIT, "9"),
    ...
    DDkwd__(ESP_MULTI_FRAGMENTS, "ON"), DDui1500_4000(ESP_MULTI_FRAGMENT_QUOTA_VM, "4000"),
    DDui1_8(ESP_NUM_FRAGMENTS, "3"),

    DDSint__(ESP_PRIORITY, "0"), DDSint__(ESP_PRIORITY_DELTA, "0"),

    // Disable hints - if SYSTEM, enable on SSD, and disable only on HDD
    DDkwd__(EXE_BMO_DISABLE_CMP_HINTS_OVERFLOW_HASH, "SYSTEM"),
    DDkwd__(EXE_BMO_DISABLE_CMP_HINTS_OVERFLOW_SORT, "SYSTEM"), DDkwd__(EXE_BMO_DISABLE_OVERFLOW, "OFF"),
    DDui___(EXE_BMO_MIN_SIZE_BEFORE_PRESSURE_CHECK_IN_MB, "50"), DDkwd__(EXE_BMO_SET_BUFFERED_WRITES, "OFF"),

    SDDkwd__(EXE_DIAGNOSTIC_EVENTS, "OFF"), DDui1__(EXE_HGB_INITIAL_HT_SIZE, "262144"),  // == hash buffer
    ...
    DDui___(EXE_MEMORY_LIMIT_LOWER_BOUND_EXCHANGE, "10"), DDui___(EXE_MEMORY_LIMIT_LOWER_BOUND_MERGEJOIN, "10"),
    ...
    DDui___(EXE_TEST_FORCE_CLUSTER_SPLIT_AFTER_MB, "0"), DDui___(EXE_TEST_FORCE_HASH_LOOP_AFTER_NUM_BUFFERS, "0"),
    ...
    DDkwd__(EXPRESSION_HISTOGRAMS, "OFF"),  // if 'OFF', compiler will not use expression
    ...
    // 01/23/98: 50.
    // Original: .1
    DDflte_(EX_OP_ALLOCATE_BUFFER, "50."), DDflte_(EX_OP_ALLOCATE_BUFFER_POOL, ".1"),
    ...
    DDkwd__(FAKE_VOLUME_ASSIGNMENTS, "OFF"), DDui1__(FAKE_VOLUME_NUM_VOLUMES, "24"),

    // upper and lower limit (2,10) must be in sync with error values in
    DDkwd__(FAST_EXTRACT_DIAGS, "OFF"), DDui1_10(FAST_EXTRACT_IO_BUFFERS, "1"),
    DDui___(FAST_EXTRACT_IO_TIMEOUT_SEC, "60"), DDui___(FAST_EXTRACT_MAX_PARTITIONS, "64"),
    DDkwd__(FIND_COLUMN_SUBEXPRS_IN_OR, "ON"), DDkwd__(FIND_COMMON_SUBEXPRS_IN_OR, "ON"),
    // ratio of rows produced without predicate (before firstn) to that
    // produced with predicate. Larger value => feature used less often
    // -0 => predicate will not be created for FirstN, feature is OFF
    // -positive value causes predicate to be pushed down
    // -negative value will cause predicate to be generated and displayed using
    // WARNING 2997. Predicate will not be actually used in query.
    DDint__(FIRSTN_PREDICATE_SAFETY_FACTOR, "0"), DDui___(FLOAT_ESP_RANDOM_NUM_SEED, "0"),

    DDkwd__(FORCE_BUSHY_CQS, "ON"),

    DDkwd__(FORCE_PARALLEL_INSERT_SELECT, "OFF"), DDkwd__(FORCE_PASS_ONE, "OFF"), DDkwd__(FORCE_PASS_TWO, "ON"),
    ...
    DDkwd__(GENERATE_EXPLAIN, "ON"), DDkwd__(GENERATE_USER_QUERYCACHE, "OFF"),
    DDkwd__(GENERATE_USER_QUERYCACHE_AUTOCOMMIT_FILTER, "ON"), DDkwd__(GENERATE_USER_QUERYCACHE_DEBUG, "OFF"),
    DD_____(GENERATE_USER_QUERYCACHE_KEYWORD, "ALL"), DD_____(GENERATE_USER_QUERYCACHE_LOG, "NONE"),
    DD0_400000(GENERATE_USER_QUERYCACHE_MAX_FILE_IN_MB, "3900"),
    DD1_1024(GENERATE_USER_QUERYCACHE_MAX_PLANSIZE_IN_MB, "16"),
    DD1_40000(GENERATE_USER_QUERYCACHE_NUM_PRELOAD, "5000"), DDkwd__(GENERATE_USER_QUERYCACHE_PRIV_CHECK, "ON"),
    DDkwd__(GENERATE_USER_QUERYCACHE_REPLACE, "OFF"), DDui___(GEN_CS_BUFFER_SIZE, "0"),
    DDui___(GEN_CS_NUM_BUFFERS, "0"), DDui___(GEN_CS_SIZE_DOWN, "4"), DDui___(GEN_CS_SIZE_UP, "4"),
    DDkwd__(GEN_DBLIMITS_LARGER_BUFSIZE, "ON"), DDui1__(GEN_DDL_BUFFER_SIZE, "30000"),
    DDui1__(GEN_DDL_NUM_BUFFERS, "4"), DDui1__(GEN_DDL_SIZE_DOWN, "2"), DDui1__(GEN_DDL_SIZE_UP, "32"),
    DDui1__(GEN_DESC_BUFFER_SIZE, "10240"), DDui1__(GEN_DESC_NUM_BUFFERS, "4"), DDui1__(GEN_DESC_SIZE_DOWN, "2"),
    DDui1__(GEN_DESC_SIZE_UP, "16"), DDui1__(GEN_DP2I_BUFFER_SIZE, "10000"), DDui1__(GEN_DP2I_NUM_BUFFERS, "2"),
    DDui1__(GEN_DP2I_SIZE_DOWN, "32"), DDui1__(GEN_DP2I_SIZE_UP, "64"), DDui1__(GEN_DPSO_BUFFER_SIZE, "10240"),
    DDui1__(GEN_DPSO_SIZE_DOWN, "2048"), DDui1__(GEN_DPSO_SIZE_UP, "2048"), DDui1__(GEN_DPUO_NUM_BUFFERS, "4"),
    DDint__(GEN_EXCHANGE_MAX_MEM_IN_KB, "4000"), DDint__(GEN_EXCHANGE_MSG_COUNT, "80"),

    // Fast extract settings are for UDR method invocations
    DDui1__(GEN_FE_BUFFER_SIZE, "31000"), DDui1__(GEN_FE_NUM_BUFFERS, "2"), DDui1__(GEN_FE_SIZE_DOWN, "4"),
    DDui1__(GEN_FE_SIZE_UP, "4"),

    // Do not alter the buffer size; it must be 56K for SCRATCH_MGMT_OPTION == 5
    DDui1__(GEN_HGBY_BUFFER_SIZE, "262144"), DDui1__(GEN_HGBY_NUM_BUFFERS, "5"),
    DDui1__(GEN_HGBY_PARTIAL_GROUP_FLUSH_THRESHOLD, "100"), DDui___(GEN_HGBY_PARTIAL_GROUP_ROWS_PER_CLUSTER, "0"),
    DDui1__(GEN_HGBY_SIZE_DOWN, "2048"), DDui1__(GEN_HGBY_SIZE_UP, "2048"),
    // Do not alter the buffer size; it must be 56K for SCRATCH_MGMT_OPTION == 5
    ...
    DDkwd__(GEN_HSHJ_MIN_MAX_OPT_INLIST_OPT, "OFF"),

    DDui1__(GEN_HSHJ_NUM_BUFFERS, "1"), DDui1__(GEN_HSHJ_SIZE_DOWN, "2048"), DDui1__(GEN_HSHJ_SIZE_UP, "2048"),

    // Controls LeanEr Expression generation
    DDkwd__(GEN_LEANER_EXPRESSIONS, "ON"),

    DDui1__(GEN_MEM_PRESSURE_THRESHOLD, "10000"), DDui1__(GEN_MJ_BUFFER_SIZE, "32768"),
    DDui1__(GEN_MJ_NUM_BUFFERS, "1"), DDui1__(GEN_MJ_SIZE_DOWN, "2"), DDui1__(GEN_MJ_SIZE_UP, "2048"),
    DDui1__(GEN_ONLJ_BUFFER_SIZE, "5120"), DDui1__(GEN_ONLJ_LEFT_CHILD_QUEUE_DOWN, "4"),
    DDui1__(GEN_ONLJ_LEFT_CHILD_QUEUE_UP, "2048"), DDui1__(GEN_ONLJ_NUM_BUFFERS, "5"),
    DDui1__(GEN_ONLJ_RIGHT_SIDE_QUEUE_DOWN, "2048"), DDui1__(GEN_ONLJ_RIGHT_SIDE_QUEUE_UP, "2048"),
    DDkwd__(GEN_ONLJ_SET_QUEUE_LEFT, "ON"), DDkwd__(GEN_ONLJ_SET_QUEUE_RIGHT, "ON"),
    DDui1__(GEN_ONLJ_SIZE_DOWN, "2048"), DDui1__(GEN_ONLJ_SIZE_UP, "2048"), DDipcBu(GEN_PA_BUFFER_SIZE, "31000"),
    DDui1__(GEN_PA_NUM_BUFFERS, "5"), DDui1__(GEN_PROBE_CACHE_NUM_ENTRIES, "16384"),  // number of entries
    DDui___(GEN_PROBE_CACHE_NUM_INNER, "0"),                                          // 0 means compiler decides
    DDui1__(GEN_PROBE_CACHE_SIZE_DOWN, "2048"), DDui1__(GEN_PROBE_CACHE_SIZE_UP, "2048"),
    DDui1__(GEN_SAMPLE_SIZE_DOWN, "16"), DDui1__(GEN_SAMPLE_SIZE_UP, "16"), DDui1__(GEN_SEQFUNC_BUFFER_SIZE, "10240"),
    DDui1__(GEN_SEQFUNC_NUM_BUFFERS, "10"), DDui1__(GEN_SEQFUNC_SIZE_DOWN, "512"), DDui1__(GEN_SEQFUNC_SIZE_UP, "2048"),
    DDui1__(GEN_SGBY_BUFFER_SIZE, "5120"), DDui1__(GEN_SGBY_NUM_BUFFERS, "5"), DDui1__(GEN_SGBY_SIZE_DOWN, "2048"),
    DDui1__(GEN_SGBY_SIZE_UP, "2048"), DDui1__(GEN_SID_BUFFER_SIZE, "1024"), DDui1__(GEN_SID_NUM_BUFFERS, "4"),
    DDui1__(GEN_SNDB_NUM_BUFFERS, "4"), DDui1__(GEN_SNDB_SIZE_DOWN, "4"), DDui1__(GEN_SNDB_SIZE_UP, "128"),
    DDui___(GEN_SNDT_BUFFER_SIZE_DOWN, "0"), DDui___(GEN_SNDT_BUFFER_SIZE_UP, "0"), DDui1__(GEN_SNDT_NUM_BUFFERS, "2"),
    DDkwd__(GEN_SNDT_RESTRICT_SEND_BUFFERS, "ON"), DDui1__(GEN_SNDT_SIZE_DOWN, "4"), DDui1__(GEN_SNDT_SIZE_UP, "128"),
    DDui1__(GEN_SORT_MAX_BUFFER_SIZE, "5242880"), DDui1__(GEN_SORT_MAX_NUM_BUFFERS, "160"),
    DDui1__(GEN_SORT_NUM_BUFFERS, "2"), DDui1__(GEN_SORT_SIZE_DOWN, "8"), DDui1__(GEN_SORT_SIZE_UP, "1024"),
    DDkwd__(GEN_SORT_TOPN, "ON"), DDui1__(GEN_SORT_TOPN_THRESHOLD, "10000"), DDui1__(GEN_SPLT_SIZE_UP, "2048"),
    DDui1__(GEN_TFLO_BUFFER_SIZE, "5120"), DDui1__(GEN_TFLO_NUM_BUFFERS, "2"), DDui1__(GEN_TFLO_SIZE_DOWN, "8"),
    DDui1__(GEN_TFLO_SIZE_UP, "16"),

    DDui512(GEN_TIMEOUT_BUFFER_SIZE, "4096"), DDui1__(GEN_TIMEOUT_NUM_BUFFERS, "1"),
    DDui2__(GEN_TIMEOUT_SIZE_DOWN, "2"), DDui2__(GEN_TIMEOUT_SIZE_UP, "4"),

    DDui1__(GEN_TRAN_BUFFER_SIZE, "4096"), DDui1__(GEN_TRAN_NUM_BUFFERS, "1"), DDui1__(GEN_TRAN_SIZE_DOWN, "2"),
    DDui1__(GEN_TRAN_SIZE_UP, "4"), DDui1__(GEN_TRSP_BUFFER_SIZE, "10240"), DDui1__(GEN_TRSP_NUM_BUFFERS, "5"),
    DDui1__(GEN_TRSP_SIZE_DOWN, "2048"), DDui1__(GEN_TRSP_SIZE_UP, "2048"), DDui1__(GEN_TUPL_BUFFER_SIZE, "1024"),
    DDui1__(GEN_TUPL_NUM_BUFFERS, "4"), DDui1__(GEN_TUPL_SIZE_DOWN, "2048"), DDui1__(GEN_TUPL_SIZE_UP, "2048"),

    // GEN_UDRRS_ settings are for stored procedure result
    // set proxy plans
    DDui1__(GEN_UDRRS_BUFFER_SIZE, "31000"), DDui1__(GEN_UDRRS_NUM_BUFFERS, "2"), DDui1__(GEN_UDRRS_SIZE_DOWN, "4"),
    DDui1__(GEN_UDRRS_SIZE_UP, "128"),

    // GEN_UDR_ settings are for UDR method invocations
    DDui1__(GEN_UDR_BUFFER_SIZE, "31000"), DDui1__(GEN_UDR_NUM_BUFFERS, "2"), DDui1__(GEN_UDR_SIZE_DOWN, "4"),
    DDui1__(GEN_UDR_SIZE_UP, "4"),

    DDkwd__(GEN_UNIQUE_COLUMN_OPT, "OFF"),

    DDui1__(GEN_UN_BUFFER_SIZE, "10240"), DDui1__(GEN_UN_NUM_BUFFERS, "5"), DDui1__(GEN_UN_SIZE_DOWN, "2048"),
    ...
    DDint__(GROUP_BY_PARTIAL_ROOT_THRESHOLD, "5000"), DDkwd__(GROUP_BY_PUSH_BELOW_UNION, "SYSTEM"),
    ...
    DDkwd__(HBASE_ASYNC_DROP_TABLE, "OFF"), DDkwd__(HBASE_ASYNC_OPERATIONS, "ALL"),

    // HBASE_CACHE_BLOCKS, ON => cache every scan, OFF => cache no scan
    // SYSTEM => cache scans which take less than 1 RS block cache mem.
    DDui___(HBASE_BLOCK_SIZE, "65536"), DDkwd__(HBASE_CACHE_BLOCKS, "SYSTEM"), DD_____(HBASE_CATALOG, "HBASE"),
    DDkwd__(HBASE_CHECK_AND_UPDEL_OPT, "SYSTEM"),
    ...
    // exposure.
    DDkwd__(HBASE_DELETE_COSTING, "ON"), DDflt0_(HBASE_DOP_PARALLEL_SCANNER, "0."),
    DDkwd__(HBASE_ENCRYPTION_OPTION, ""), DDkwd__(HBASE_ESTIMATE_ROW_COUNT_VIA_COPROCESSOR, "OFF"),
    DDkwd__(HBASE_FILTER_PREDS, "OFF"), DDkwd__(HBASE_HASH2_PARTITIONING, "ON"), DDui___(HBASE_INDEX_LEVEL, "0"),
    DDui___(HBASE_MAX_COLUMN_INFO_LENGTH, "10000"), DDui___(HBASE_MAX_COLUMN_NAME_LENGTH, "100"),
    DDui___(HBASE_MAX_COLUMN_VAL_LENGTH, "1000"), DDui___(HBASE_MAX_ESPS, "9999"),
    DDui___(HBASE_MAX_NUM_SEARCH_KEYS, "512"), DDkwd__(HBASE_MEMSTORE_FLUSH_SIZE_OPTION, ""),
    DDui1__(HBASE_MIN_BYTES_PER_ESP_PARTITION, "67108864"),

    DDkwd__(HBASE_NATIVE_IUD, "ON"), DDui1__(HBASE_NUM_CACHE_ROWS_MAX, "1024"),
    DDui1__(HBASE_NUM_CACHE_ROWS_MIN, "100"),

    DDkwd__(HBASE_RANGE_PARTITIONING, "ON"), DDkwd__(HBASE_RANGE_PARTITIONING_MC_SPLIT, "ON"),
    DDkwd__(HBASE_RANGE_PARTITIONING_PARTIAL_COLS, "ON"), DDkwd__(HBASE_READ_REPLICA, "OFF"),
    DDui___(HBASE_REGION_SERVER_MAX_HEAP_SIZE, "1024"),  // in units of MB

    DDkwd__(HBASE_ROWSET_VSBB_OPT, "ON"), DDusht_(HBASE_ROWSET_VSBB_SIZE, "1024"),
    DDflt0_(HBASE_SALTED_TABLE_MAX_FILE_SIZE, "0"), DDkwd__(HBASE_SALTED_TABLE_SET_SPLIT_POLICY, "ON"),
    DDflt__(HBASE_SCAN_DOP_AS_PARTITIONS_THRESHOLD, "10.0"),  // in units of MB
    DDkwd__(HBASE_SERIALIZATION, "ON"),

    DD_____(HBASE_SERVER, ""), DDkwd__(HBASE_SMALL_SCANNER, "SYSTEM"), DDkwd__(HBASE_SQL_IUD_SEMANTICS, "ON"),
    ...
    DDkwd__(HBASE_UPDEL_CURSOR_OPT, "ON"), DDui___(HBASE_USE_FAKED_REGIONS, "0"),

    DDkwd__(HBASE_VISIBILITY, "OFF"),

    DD_____(HBASE_ZOOKEEPER_PORT, ""),

    DDui1__(HDFS_IO_BUFFERSIZE, "65536"), DDui___(HDFS_IO_BUFFERSIZE_BYTES, "0"),
    DDui___(HDFS_IO_INTERIM_BYTEARRAY_SIZE_IN_KB, "1024"),
    // The value 0 denotes RangeTail = max record length of table.
    DDui___(HDFS_IO_RANGE_TAIL, "0"), DDkwd__(HDFS_PREFETCH, "ON"), DDkwd__(HDFS_READ_CONTINUE_ON_ERROR, "OFF"),
    DDui1__(HDFS_REPLICATION, "1"), DDkwd__(HDFS_USE_CURSOR_MULTI, "OFF"),
    DDui___(HDFS_WRITE_LOCK_TIMEOUT_IN_SECS, "30"),

    DDkwd__(HGB_BITMUX, "OFF"), DDflt0_(HGB_CPUCOST_INITIALIZE, "1."), DDflt0_(HGB_DP2_MEMORY_LIMIT, "10000."),
    DDflte_(HGB_GROUPING_FACTOR_FOR_SPILLED_CLUSTERS, ".5"), DDflte_(HGB_MAX_TABLE_SIZE_FOR_CLUSTERS, "4E5"),
    DDflte_(HGB_MEMORY_AVAILABLE_FOR_CLUSTERS, "10"), DDflte_(HH_OP_ALLOCATE_HASH_TABLE, ".05"),
    ...
    DDkwd__(HIST_ASSUME_INDEPENDENT_REDUCTION, "ON"), DDflt0_(HIST_BASE_REDUCTION_FUDGE_FACTOR, "0.1"),
    DDflt0_(HIST_CONSTANT_ALPHA, "0.5"), DDflt_0_1(HIST_DEFAULT_BASE_SEL_FOR_LIKE_WILDCARD, "0.50"),
    DDui1__(HIST_DEFAULT_NUMBER_OF_INTERVALS, "50"), DDui1__(HIST_DEFAULT_SAMPLE_MAX, "1000000"),
    DDui1__(HIST_DEFAULT_SAMPLE_MIN, "10000"), DDflt_0_1(HIST_DEFAULT_SAMPLE_RATIO, "0.01"),
    DDflte_(HIST_DEFAULT_SEL_FOR_BOOLEAN, "0.3333"), DDflt_0_1(HIST_DEFAULT_SEL_FOR_IS_NULL, "0.01"),
    DDflt_0_1(HIST_DEFAULT_SEL_FOR_JOIN_EQUAL, "0.3333"), DDflt_0_1(HIST_DEFAULT_SEL_FOR_JOIN_RANGE, "0.3333"),
    DDflt_0_1(HIST_DEFAULT_SEL_FOR_LIKE_NO_WILDCARD, "1.0"), DDflt_0_1(HIST_DEFAULT_SEL_FOR_LIKE_WILDCARD, "0.10"),
    DDflt_0_1(HIST_DEFAULT_SEL_FOR_PRED_EQUAL, "0.01"), DDflt_0_1(HIST_DEFAULT_SEL_FOR_PRED_RANGE, "0.3333"),
    ...
    DDkwd__(HIST_FREQ_VALS_NULL_FIX, "ON"), DDkwd__(HIST_INCLUDE_SKEW_FOR_NON_INNER_JOIN, "ON"),
    DDkwd__(HIST_INTERMEDIATE_REDUCTION, "OFF"), DDflt0_(HIST_INTERMEDIATE_REDUCTION_FUDGE_FACTOR, "0.25"),
    DDflt_0_1(HIST_JOIN_CARD_LOWBOUND, "1.0"), DDui1__(HIST_LOW_UEC_THRESHOLD, "55"),
    DDui1__(HIST_MAX_NUMBER_OF_INTERVALS, "10000"),

    DDkwd__(HIST_MC_STATS_NEEDED, "ON"), DDkwd__(HIST_MERGE_FREQ_VALS_FIX, "ON"),
    ...
    DDflt1_(HIST_NO_STATS_ROWCOUNT, "100"), DDflt1_(HIST_NO_STATS_UEC, "2"), DDflt1_(HIST_NO_STATS_UEC_CHAR1, "10"),
    DDui1__(HIST_NUM_ADDITIONAL_DAYS_TO_EXTRAPOLATE, "4"), DDintN1__(HIST_ON_DEMAND_STATS_SIZE, "0"),
    DDui___(HIST_OPTIMISTIC_CARD_OPTIMIZATION, "1"), XDDkwd__(HIST_PREFETCH, "ON"),
    XDDkwd__(HIST_REMOVE_TRAILING_BLANKS, "ON"),  // should remove after verifying code is solid
    DDansi_(HIST_ROOT_NODE, ""), XDDflt1_(HIST_ROWCOUNT_REQUIRING_STATS, "500"), DD_____(HIST_SCRATCH_VOL, ""),
    // control the amount of data in each partition of the sample tble.
    DDflt1_(HIST_SCRATCH_VOL_THRESHOLD, "10240000"), DDflt_0_1(HIST_SKEW_COST_ADJUSTMENT, "0.2"),
    DDkwd__(HIST_SKIP_MC_FOR_NONKEY_JOIN_COLUMNS, "OFF"), DDui___(HIST_TUPLE_FREQVAL_LIST_THRESHOLD, "40"),
    DDkwd__(HIST_USE_HIGH_FREQUENCY_INFO, "ON"), XDDkwd__(HIST_USE_SAMPLE_FOR_CARDINALITY_ESTIMATION, "ON"),
    ...
    DDui___(HIVE_APPLY_LOCALITY_MIN_NODES, "10"), DDui___(HIVE_APPLY_LOCALITY_NODE_RATIO, "2"),

    DDkwd__(HIVE_ARRAY_INDEX_MODE, "OFF"),

    DD_____(HIVE_CATALOG, "HIVE"), DDkwd__(HIVE_COMPATIBILITY_MODE, "OFF"),

    DDkwd__(HIVE_COMPOSITE_DATATYPE_SUPPORT, "ON"),

    DDkwd__(HIVE_CREATE_TABLE_LIKE_PARTITION_NO_NULL, "OFF"), DDkwd__(HIVE_CTAS_IN_NATIVE_MODE, "OFF"),

    DD_____(HIVE_CTAS_TABLETYPE, "ORC"),

    DDkwd__(HIVE_DATA_MOD_CHECK, "ON"),

    DDkwd__(HIVE_DDL, "OFF"),

    DDkwd__(HIVE_DEFAULT_CHARSET, (char *)SQLCHARSETSTRING_UTF8), DD_____(HIVE_DEFAULT_SCHEMA, "HIVE"),
    DDkwd__(HIVE_DESCRIBE_VIRT_COLS, "OFF"), DDkwd__(HIVE_EXT_TABLE_INCLUDE_VIRT_COLS, "OFF"),
    DD_____(HIVE_FILE_CHARSET, ""), DD_____(HIVE_HDFS_STATS_LOG_FILE, ""),
    DDint__(HIVE_HDFS_STATS_MAX_SAMPLE_FILES, "2000"), DDkwd__(HIVE_HDFS_STATS_SAMPLE_LOB_INTFC, "ON"),
    DDui___(HIVE_INSERT_ERROR_MODE, "1"), DDint__(HIVE_LIB_HDFS_PORT_OVERRIDE, "-1"),
    DDint__(HIVE_LOCALITY_BALANCE_LEVEL, "3"), DDflt1_(HIVE_LOCALITY_MAX_OVERLOAD, "1.05"),
    DDint__(HIVE_LOCALITY_MAX_SECOND_CHECK_TGTS, "20"), DDint__(HIVE_LOCALITY_NUM_SECOND_LOOPS, "4"),
    ...
    DDflt0_(HIVE_MIN_NUM_ESPS_PER_DATANODE, "1"), DDkwd__(HIVE_NATABLE_CACHE_S3_TABLES, "OFF"),
    DDkwd__(HIVE_NO_REGISTER_OBJECTS, "OFF"), DDflte_(HIVE_NUM_ESPS_PER_DATANODE, "8.0"),
    DDpct__(HIVE_NUM_ESPS_ROUND_DEVIATION, "34"), DDkwd__(HIVE_PARTITION_AUTO_CREATE, "ON"),
    DDkwd__(HIVE_PARTITION_ELIMINATION_CT, "ON"), DDkwd__(HIVE_PARTITION_ELIMINATION_MM, "ON"),
    DDkwd__(HIVE_PARTITION_ELIMINATION_RT, "ON"), DDkwd__(HIVE_PARTITION_INSERT_DIST_LOCK, "SYSTEM"),
    DDui___(HIVE_PARTITION_INSERT_REPART, "0"), DDkwd__(HIVE_REPORT_FILESTATS_READ_ET, "OFF"),
    ...
    DDflt__(HJ_BROADCAST_INNER_THRESHOLD_MB, "300"),

    DDflt0_(HJ_CPUCOST_INITIALIZE, "1."), DDui1__(HJ_INITIAL_BUCKETS_PER_CLUSTER, "4."),
    DDkwd__(HJ_NEW_MCSB_PLAN, "OFF"), DDint__(HJ_SCAN_TO_NJ_PROBE_SPEED_RATIO, "2000"), DDkwd__(HJ_TYPE, "HYBRID"),
    DDkwd__(HQC_CONVDOIT_DISABLE_NUMERIC_CHECK, "OFF"), DDkwd__(HQC_LOG, "OFF"), DD_____(HQC_LOG_FILE, ""),
    DDui1_10(HQC_MAX_VALUES_PER_KEY, "5"), DDkwd__(HYBRID_QUERY_CACHE, "ON"), DDkwd__(IF_LOCKED, "WAIT"),

    DDkwd__(IMPLICIT_DATETIME_INTERVAL_HOSTVAR_CONVERSION, "FALSE"), DDkwd__(IMPLICIT_HOSTVAR_CONVERSION, "FALSE"),
    ...
    DDkwd__(INCORPORATE_SKEW_IN_COSTING, "ON"), DDkwd__(INDEX_ELIMINATION_LEVEL, "MAXIMUM"),
    DDui1__(INDEX_ELIMINATION_THRESHOLD, "50"), DDkwd__(INDEX_HINT_WARNINGS, "ON"), SDDkwd__(INFER_CHARSET, "OFF"),

    DDkwd__(INITIALIZE_DROP_FORCE, "OFF"),

    // UDF initial row cost CQDs
    DDui___(INITIAL_UDF_CPU_COST, "100"), DDui___(INITIAL_UDF_IO_COST, "1"), DDui___(INITIAL_UDF_MSG_COST, "2"),
    ...
    // 10-040621-7139-end
    DDflt__(INTERSECT_PUSH_GRBY_RATIO, "-1.0"), DDkwd__(IN_MEMORY_OBJECT_DEFN, "OFF"),

    // History:
    // 3/11/99 Changed to zero because in large tables the read-ahead
    //         seems negligible (and/or hard to simulate)
    // Before 3/11/99: 0.58
    DDflt0_(IO_TRANSFER_COST_PREFETCH_MISSES_FRACTION, "0."), XDDkwd__(ISOLATION_LEVEL, "READ_COMMITTED"),
    XDDkwd__(ISOLATION_LEVEL_FOR_UPDATES, "NONE"), SDDkwd__(ISO_MAPPING, (char *)SQLCHARSETSTRING_ISO88591),

    DDkwd__(IS_DB_TRANSPORTER, "OFF"),

    DDkwd__(IS_SQLCI, "FALSE"), DDkwd__(IS_TRAFCI, "FALSE"),
    ...
    // 16MB in size.
    DDui___(LOB_OUTPUT_SIZE, "16777216"),
    ...
    DDkwd__(MAINTENANCE_WINDOW, "ON"),

    DDkwd__(MARIAQUEST_PROCESS, "OFF"),

    DDSint__(MASTER_PRIORITY, "0"), DDSint__(MASTER_PRIORITY_DELTA, "0"),
    ...
    DDint__(MAX_DOP_FOR_LOAD, "16"),  // don't make it anything over 31
    ...
    //
    DDflt__(MAX_ESPS_PER_CPU_PER_OP, "0.03125"),
    // max ESPs allocated per node (in average) at runtime
    DDint__(MAX_ESPS_PER_NODE, "-1"), DDui1__(MAX_EXPRS_USED_FOR_CONST_FOLDING, "1000"),

    // used in hash groupby costing in esp/master
    DDui1__(MAX_HEADER_ENTREIS_PER_HASH_TABLE, "250000"),

    DDui1__(MAX_LONG_VARCHAR_DEFAULT_SIZE, "2000"), DDui1__(MAX_LONG_WVARCHAR_DEFAULT_SIZE, "2000"),

    DD18_128(MAX_NUMERIC_PRECISION_ALLOWED, "128"),

    // max number of unique constraints to try when synthesize
    // the logical property for a join.
    // -1: no limit
    DDint__(MAX_NUM_UNIQUE_CONSTRAINTS_FOR_JOIN, "-1"),
    ...
    DDflt0_(MDAM_TOTAL_UEC_CHECK_MIN_RC_THRESHOLD, "10000"), DDflt0_(MDAM_TOTAL_UEC_CHECK_UEC_THRESHOLD, "0.2"),
    ...
    XDDui___(MDAM_UNDER_NJ_PROBES_THRESHOLD, "1"),
    ...
    // values are in KB
    DDui___(MEMORY_LIMIT_CMPCTXT_UPPER_KB, "0"), DDui___(MEMORY_LIMIT_CMPSTMT_UPPER_KB, "0"),
    DDui___(MEMORY_LIMIT_HISTCACHE_UPPER_KB, "0"), DDui___(MEMORY_LIMIT_NATABLECACHE_UPPER_KB, "0"),
    ...
    DDkwd__(MEMORY_MONITOR, "OFF"), DDui1__(MEMORY_MONITOR_AFTER_TASKS, "30000"),
    DDkwd__(MEMORY_MONITOR_IN_DETAIL, "OFF"), DD_____(MEMORY_MONITOR_LOGFILE, "NONE"),
    DDkwd__(MEMORY_MONITOR_LOG_INSTANTLY, "OFF"), DDui1__(MEMORY_MONITOR_TASK_INTERVAL, "5000"),

    DDkwd__(MEMORY_TABLE_SCAN, "ON"),
    ...
    DDflt1_(MEMORY_USAGE_NICE_CONTEXT_FACTOR, "1"), DDflt1_(MEMORY_USAGE_OPT_PASS_FACTOR, "1.5"),
    // increase to 1500 (from 500) to help with Q64, Q88 and other queries
    // in TPCDs. Without the raise, certain subqueries in Q88 will compilied
    // into slow nested joins, and the 2nd instnace of cross_sales common
    // expression will compiled into a zig-zag tree.
    DDui1__(MEMORY_USAGE_SAFETY_NET, "1500"),

    // MERGE_JOINS ON means do MERGE_JOINS
    XDDkwd__(MERGE_JOINS, "ON"), DDkwd__(MERGE_JOIN_ACCEPT_MULTIPLE_NJ_PROBES, "ON"),
    DDkwd__(MERGE_JOIN_CONTROL, "OFF"), DDkwd__(MERGE_JOIN_WITH_POSSIBLE_DEADLOCK, "OFF"),

    // controls if merge/upsert is supported on table with a unique index
    DDkwd__(MERGE_WITH_UNIQUE_INDEX, "ON"),

    SDDui___(METADATA_CACHE_SIZE, "50"),
    //-------------------------------------------------------------------
    ...
    //-------------------------------------------------------------------
    DDui___(MINIMUM_ESP_PARALLELISM, "0"), DDui1__(MIN_LONG_VARCHAR_DEFAULT_SIZE, "1"),
    DDui1__(MIN_LONG_WVARCHAR_DEFAULT_SIZE, "1"), DDkwd__(MIN_MAX_OPTIMIZATION, "ON"),
    DD1_128(MIN_PASSWORD_LENGTH, "6"), DDpct__(MJ_BMO_QUOTA_PERCENT, "0"),

    // 0: max, 1: min, 2 geometric mean
    DDui___(MJ_COMPUTE_CPU_RESOURCE, "0"),

    DDflt0_(MJ_CPUCOST_CLEAR_LIST, ".01"), DDflt0_(MJ_CPUCOST_GET_NEXT_ROW_FROM_LIST, ".01"),
    // calibrated 01/16/98:
    ...
    DDflt0_(MJ_CPUCOST_INSERT_ROW_TO_LIST, ".0001"), DDflt0_(MJ_CPUCOST_REWIND_LIST, ".01"),
    DDflte_(MJ_LIST_NODE_SIZE, ".01"), DDkwd__(MJ_OVERFLOW, "ON"),

    DDkwd__(MODE_COMPATIBLE_1, "ON"),

    // This cqd is for compatibility with mysql
    DDkwd__(MODE_COMPATIBLE_2, "OFF"),
    ...
    DD_____(MONARCH_LOCATOR_ADDRESS, "127.0.0.1"), DD_____(MONARCH_LOCATOR_PORT, "10334"),
    ...
    // -------------------------------------------------------------------------
    DDflte_(MSCF_OV_IO, "0.5"), DDflte_(MSCF_OV_MSG, "0.5"),

    DDkwd__(MTD_GENERATE_CC_PREDS, "ON"), DDint__(MTD_MDAM_NJ_UEC_THRESHOLD, "100"),
    ...
    XDDkwd__(MULTI_THREADED_ESP, "OFF"),

    DDflt0_(MU_CPUCOST_INITIALIZE, ".05"), DDui___(MU_INITIAL_BUFFER_COUNT, "5."),
...
#endif
    DDkwd__(MVQR_LOG_QUERY_DESCRIPTORS, "OFF"), DDint__(MVQR_MAX_EXPR_DEPTH, "20"), DDint__(MVQR_MAX_EXPR_SIZE, "100"),
    DDkwd__(MVQR_PARAMETERIZE_EQ_PRED, "ON"), DDkwd__(MVQR_PRIVATE_QMS_INIT, "SMD"), DDkwd__(MVQR_PUBLISH_TO, "BOTH"),
    DDansi_(MVQR_REWRITE_CANDIDATES, ""), XDDkwd__(MVQR_REWRITE_ENABLED_OPTION, "OFF"),  // @ZX -- change to ON later
    XDDui0_5(MVQR_REWRITE_LEVEL, "0"), XDDkwd__(MVQR_REWRITE_SINGLE_TABLE_QUERIES, "ON"),
    DDkwd__(MVQR_USE_EXTRA_HUB_TABLES, "ON"), DDkwd__(MVQR_USE_RI_FOR_EXTRA_HUB_TABLES, "OFF"),
    DD_____(MVQR_WORKLOAD_ANALYSIS_MV_NAME, ""),

    XDDMVA__(MV_AGE, "0 MINUTES"), XDDkwd__(MV_ALLOW_SELECT_SYSTEM_ADDED_COLUMNS, "OFF"),
    DDkwd__(MV_AS_ROW_TRIGGER, "OFF"), DDkwd__(MV_DUMP_DEBUG_INFO, "OFF"),
    DDkwd__(MV_ENABLE_INTERNAL_REFRESH_SHOWPLAN, "OFF"),

    DDui___(MV_LOG_CLEANUP_SAFETY_FACTOR, "200"), DDui___(MV_LOG_CLEANUP_USE_MULTI_COMMIT, "1"),
    ...
    SDDui___(MV_REFRESH_MAX_PARALLELISM, "0"), DDui___(MV_REFRESH_MAX_PIPELINING, "0"),
    ...
    DDSint__(MXCMP_PRIORITY, "0"), DDSint__(MXCMP_PRIORITY_DELTA, "0"),

    DDkwd__(NAMETYPE, "ANSI"),

    DDkwd__(NAR_DEPOBJ_ENABLE, "ON"), DDkwd__(NAR_DEPOBJ_ENABLE2, "ON"),
    ...
    DDflt__(NCM_CACHE_SIZE_IN_BLOCKS, "52"), DDflt__(NCM_COSTLIMIT_FACTOR, "0.05"),  // change to 0.05
    DDint__(NCM_ESP_FIXUP_WEIGHT, "3000"), DDkwd__(NCM_ESP_STARTUP_FIX, "OFF"),
    ...
    DDkwd__(NCM_HBASE_LIMIT_DOP_IN_COSTING, "ON"), DDkwd__(NCM_HGB_OVERFLOW_COSTING, "ON"),
    DDkwd__(NCM_HJ_OVERFLOW_COSTING, "ON"), DDflt__(NCM_IND_JOIN_COST_ADJ_FACTOR, "1.0"),
    DDflt__(NCM_IND_JOIN_SELECTIVITY, "1.0"), DDflt__(NCM_IND_SCAN_COST_ADJ_FACTOR, "1.0"),
    DDflt__(NCM_IND_SCAN_SELECTIVITY, "1.0"), DDflt__(NCM_MAP_CPU_FACTOR, "4.0"), DDflt__(NCM_MAP_MSG_FACTOR, "4.0"),
    DDflt__(NCM_MAP_RANDIO_FACTOR, "4.0"), DDflt__(NCM_MAP_SEQIO_FACTOR, "4.0"),
    DDflt__(NCM_MDAM_COST_ADJ_FACTOR, "1.0"), DDflt__(NCM_MJ_TO_HJ_FACTOR, "0.6"),
    DDflt__(NCM_MP_ORC_SCAN_COST_ADJ_FACTOR, "1000.00"), DDflt__(NCM_MP_PARQUET_SCAN_COST_ADJ_FACTOR, "1000.00"),
    DDflt__(NCM_NJ_PC_THRESHOLD, "1.0"), DDflt0_(NCM_NJ_PROBES_MAXCARD_FACTOR, "10000"),
    DDkwd__(NCM_NJ_SEQIO_FIX, "ON"),  // change to ON
    DDint__(NCM_NUM_SORT_RUNS, "4"),

    DDkwd__(NCM_ORC_COSTING, "ON"), DDkwd__(NCM_ORC_COSTING_APPLY_SKIP_RATIO, "OFF"),
    DDkwd__(NCM_ORC_COSTING_DEBUG, "OFF"), DDansi_(NCM_ORC_COSTING_DEBUG_TABLE, ""),

    DDkwd__(NCM_PARQUET_COSTING, "ON"), DDkwd__(NCM_PARQUET_COSTING_APPLY_SKIP_RATIO, "OFF"),
    DDkwd__(NCM_PARQUET_COSTING_DEBUG, "OFF"), DDansi_(NCM_PARQUET_COSTING_DEBUG_TABLE, ""),

    DDflt__(NCM_PAR_ADJ_FACTOR, "0.10"), DDkwd__(NCM_PAR_GRPBY_ADJ, "ON"),

    DDkwd__(NCM_PRINT_ROWSIZE, "OFF"), DDflt__(NCM_RAND_IO_ROWSIZE_FACTOR, "0"), DDflt__(NCM_RAND_IO_WEIGHT, "3258"),
    DDflt__(NCM_SEQ_IO_ROWSIZE_FACTOR, "0"), DDflt__(NCM_SEQ_IO_WEIGHT, "543"), DDflt__(NCM_SERIAL_NJ_FACTOR, "2"),
    DDflt__(NCM_SGB_TO_HGB_FACTOR, "0.8"), DDkwd__(NCM_SKEW_COST_ADJ_FOR_PROBES, "OFF"),
    DDkwd__(NCM_SORT_OVERFLOW_COSTING, "ON"), DDflt__(NCM_TUPLES_ROWSIZE_FACTOR, "0.5"),
    ...
    DDkwd__(NESTED_JOINS_CHECK_LEADING_KEY_SKEW, "OFF"), DDkwd__(NESTED_JOINS_FULL_INNER_KEY, "OFF"),
    DDkwd__(NESTED_JOINS_KEYLESS_INNERJOINS, "ON"), DDui1__(NESTED_JOINS_LEADING_KEY_SKEW_THRESHOLD, "15"),
    ...
    DDkwd__(NESTED_JOIN_FOR_CROSS_PRODUCTS, "ON"), DDkwd__(NEW_MDAM, "ON"), DDkwd__(NEW_OPT_DRIVER, "ON"),

    // default value of NGRAM_DISABLE_IN_NATABLE
    DDkwd__(NGRAM_DISABLE_IN_NATABLE, "ON"),
    // default value of NGRAM_INDEX
    DDkwd__(NGRAM_INDEX, "OFF"),

    DDflt0_(NJ_CPUCOST_INITIALIZE, ".1"),

    DDflt0_(NJ_CPUCOST_PASS_ROW, ".02"),

    DDflte_(NJ_INC_AFTERLIMIT, "0.0055"), DDflte_(NJ_INC_MOVEROWS, "0.0015"), DDflte_(NJ_INC_UPTOLIMIT, "0.0225"),

    DDui___(NJ_INITIAL_BUFFER_COUNT, "5"), DDui1__(NJ_INITIAL_BUFFER_SIZE, "5"),

    DDui1__(NJ_MAX_SEEK_DISTANCE, "5000"),

    DD_____(NODE_ALLOCATIONS, ""),

    // UDF costing CQDs for processing a steady state row
    DDui___(NORMAL_UDF_CPU_COST, "100"), DDui___(NORMAL_UDF_IO_COST, "0"), DDui___(NORMAL_UDF_MSG_COST, "2"),
    ...
    // NSK DEBUG defaults
    DDansi_(NSK_DBG, "OFF"), DDansi_(NSK_DBG_COMPILE_INSTANCE, "USER"), DDkwd__(NSK_DBG_GENERIC, "OFF"),
    DDansi_(NSK_DBG_LOG_FILE, ""), DDkwd__(NSK_DBG_MJRULES_TRACKING, "OFF"), DDkwd__(NSK_DBG_PRINT_CHAR_INPUT, "OFF"),
    DDkwd__(NSK_DBG_PRINT_CHAR_OUTPUT, "OFF"), DDkwd__(NSK_DBG_PRINT_CONSTRAINT, "OFF"),
    DDkwd__(NSK_DBG_PRINT_CONTEXT, "OFF"), DDkwd__(NSK_DBG_PRINT_CONTEXT_POINTER, "OFF"),
    DDkwd__(NSK_DBG_PRINT_COST, "OFF"), DDkwd__(NSK_DBG_PRINT_INDEX_ELIMINATION, "OFF"),
    DDkwd__(NSK_DBG_PRINT_ITEM_EXPR, "OFF"), DDkwd__(NSK_DBG_PRINT_LOG_PROP, "OFF"),
    DDkwd__(NSK_DBG_PRINT_PHYS_PROP, "OFF"), DDkwd__(NSK_DBG_PRINT_TASK, "OFF"),
    DDkwd__(NSK_DBG_PRINT_TASK_STACK, "OFF"), DDkwd__(NSK_DBG_QUERY_LOGGING_ONLY, "OFF"),
    DDansi_(NSK_DBG_QUERY_PREFIX, ""), DDkwd__(NSK_DBG_SHOW_PASS1_PLAN, "OFF"), DDkwd__(NSK_DBG_SHOW_PASS2_PLAN, "OFF"),
    DDkwd__(NSK_DBG_SHOW_PLAN_LOG, "OFF"), DDkwd__(NSK_DBG_SHOW_TREE_AFTER_ANALYSIS, "OFF"),
    DDkwd__(NSK_DBG_SHOW_TREE_AFTER_BINDING, "OFF"), DDkwd__(NSK_DBG_SHOW_TREE_AFTER_CODEGEN, "OFF"),
    DDkwd__(NSK_DBG_SHOW_TREE_AFTER_NORMALIZATION, "OFF"), DDkwd__(NSK_DBG_SHOW_TREE_AFTER_PARSING, "OFF"),
    ...
    DDflt0_(NUM_OF_PARTS_DEVIATION_TYPE2_JOINS, "0.25"),

    DDflt0_(OCB_COST_ADJSTFCTR, "0.996"), DDui___(OCR_FOR_SIDETREE_INSERT, "1"),

    DDkwd__(ODBC_PROCESS, "FALSE"),

    DDflte_(OHJ_BMO_REUSE_SORTED_BMOFACTOR_LIMIT, "3.0"), DDflte_(OHJ_BMO_REUSE_SORTED_UECRATIO_UPPERLIMIT, "0.7"),
    DDflte_(OHJ_BMO_REUSE_UNSORTED_UECRATIO_UPPERLIMIT, "0.01"), DDflte_(OHJ_VBMOLIMIT, "5.0"),

    DDui1__(OLAP_BUFFER_SIZE, "262144"),  // Do not alter (goes to DP2)
    DDkwd__(OLAP_CAN_INVERSE_ORDER, "ON"), DDui1__(OLAP_MAX_FIXED_WINDOW_EXTRA_BUFFERS, "2"),
    DDui1__(OLAP_MAX_FIXED_WINDOW_FRAME, "50000"), DDui1__(OLAP_MAX_NUMBER_OF_BUFFERS, "100000"),
    DDui___(OLAP_MAX_ROWS_IN_OLAP_BUFFER, "0"), DDkwd__(OLAP_MULTIWINDOWS, "ON"),
    ...
    DDkwd__(OLT_QUERY_OPT, "ON"), DDkwd__(OLT_QUERY_OPT_LEAN, "OFF"), DDint__(ONLINE_BACKUP_TIMEOUT, "30"),
    // -----------------------------------------------------------------------
    // Optimizer pruning heuristics.
    // -----------------------------------------------------------------------
    DDkwd__(OPH_EXITHJCRCONTCHILOOP, "ON"), DDkwd__(OPH_EXITMJCRCONTCHILOOP, "ON"),
    DDkwd__(OPH_EXITNJCRCONTCHILOOP, "OFF"), DDkwd__(OPH_PRUNE_WHEN_COST_LIMIT_EXCEEDED, "OFF"),
    DDflt__(OPH_PRUNING_COMPLEXITY_THRESHOLD, "10.0"), DDflt__(OPH_PRUNING_PASS2_COST_LIMIT, "-1.0"),
    DDkwd__(OPH_REDUCE_COST_LIMIT_FROM_CANDIDATES, "OFF"), DDkwd__(OPH_REDUCE_COST_LIMIT_FROM_PASS1_SOLUTION, "ON"),
    DDkwd__(OPH_REUSE_FAILED_PLAN, "ON"), DDkwd__(OPH_REUSE_OPERATOR_COST, "OFF"),
    DDkwd__(OPH_SKIP_OGT_FOR_SHARED_GC_FAILED_CL, "OFF"), DDkwd__(OPH_USE_CACHED_ELAPSED_TIME, "ON"),
    DDkwd__(OPH_USE_CANDIDATE_PLANS, "OFF"), DDkwd__(OPH_USE_COMPARE_COST_THRESHOLD, "ON"),
    DDkwd__(OPH_USE_CONSERVATIVE_COST_LIMIT, "OFF"), DDkwd__(OPH_USE_ENFORCER_PLAN_PROMOTION, "OFF"),
    DDkwd__(OPH_USE_FAILED_PLAN_COST, "ON"), DDkwd__(OPH_USE_NICE_CONTEXT, "OFF"),
    DDkwd__(OPH_USE_ORDERED_MJ_PRED, "OFF"), DDkwd__(OPH_USE_PWS_FLAG_FOR_CONTEXT, "OFF"),

    DDkwd__(OPTIMIZATION_GOAL, "LASTROW"), XDDkwd__(OPTIMIZATION_LEVEL, "3"),
    DDpct__(OPTIMIZATION_LEVEL_1_CONSTANT_1, "50"), DDpct__(OPTIMIZATION_LEVEL_1_CONSTANT_2, "0"),
    DDui1__(OPTIMIZATION_LEVEL_1_IMMUNITY_LIMIT, "5000"), DDui1__(OPTIMIZATION_LEVEL_1_MJENUM_LIMIT, "20"),
    DDui1__(OPTIMIZATION_LEVEL_1_SAFETY_NET, "30000"), DDflt__(OPTIMIZATION_LEVEL_1_SAFETY_NET_MULTIPLE, "3.0"),
    DDui1__(OPTIMIZATION_LEVEL_1_THRESHOLD, "1000"), DDui1__(OPTIMIZATION_TASKS_LIMIT, "2000000000"),
    // Optimizer Graceful Termination:
    // 1=> randomProbabilistic pruning
    // > 1 pruning based on potential
    DDui1__(OPTIMIZER_GRACEFUL_TERMINATION, "2"), DDkwd__(OPTIMIZER_HEURISTIC_1, "OFF"),
    DDkwd__(OPTIMIZER_HEURISTIC_2, "OFF"), DDkwd__(OPTIMIZER_HEURISTIC_3, "OFF"), DDkwd__(OPTIMIZER_HEURISTIC_4, "OFF"),
    ...
    // (03/03/98)
    SDDkwd__(OPTIMIZER_PRUNING, "ON"), DDkwd__(OPTIMIZER_PRUNING_FIX_1, "ON"),  // change to ON

    DDkwd__(OPTIMIZER_SYNTH_FUNC_DEPENDENCIES, "ON"),

    DDkwd__(OPTIMIZE_LARGE_VARCHAR, "ON"),

    // OPTS_PUSH_DOWN_DAM made external RV 06/21/01 CR 10-010425-2440
    DDui___(OPTS_PUSH_DOWN_DAM, "0"),

    DDkwd__(ORC_AGGR_PUSHDOWN, "ON"), DDkwd__(ORC_COLUMNS_PUSHDOWN, "ON"), DD_____(ORC_HDFS_STATS_LOG_FILE, ""),
    DDint__(ORC_METADATA_READ_MAX_FILENUM, "5"), DDint__(ORC_METADATA_READ_MAX_THREADS, "1"),
    DDkwd__(ORC_METADATA_READ_WITH_JAVA_THREADS, "ON"), DDkwd__(ORC_NJS, "OFF"),
    // CPS is the abbreviation for "compute probe stats"
    DDkwd__(ORC_NJS_CPS_DEBUG, "OFF"), DDflt__(ORC_NJS_CPS_MAXCARDINALITY, "502000"), DDansi_(ORC_NJS_CPS_TABLE, ""),
    DDflt__(ORC_NJS_CPS_THRESHOLD, "-1"), DDui___(ORC_NJS_OUTER_TABLES_THRESHOLD, "1"),
    DDui___(ORC_NJS_PROBES_MAXCARD_FACTOR, "10"), DDint__(ORC_NJS_PROBES_THRESHOLD, "150"),
    DDkwd__(ORC_PARTITION_ONLY_PUSHDOWN, "ON"), DDkwd__(ORC_PRED_PUSHDOWN, "ON"),

    // Controls buffer allocation for ORC scans
    DDui___(ORC_READER_ALLOCATE_ROW_LENGTH, "0"),  // row size in bytes (default of zero means use generator estimate)
    DDui___(ORC_READER_MAX_ALLOCATION_MB, "32"),   // max total allocation in MB (max rows to fill is
                                                   // adjusted downward to fit below this cap)
    DDui___(ORC_READER_MAX_ROWS_TO_FILL, "128"),   // max num of rows per buffer

    DDkwd__(ORC_READ_NUM_ROWS, "ON"), DDkwd__(ORC_READ_STRIPE_INFO, "OFF"), DDint__(ORC_STATS_MAX_SAMPLE_FILES, "5000"),
    DDkwd__(ORC_VECTORIZED_SCAN, "ON"), DDui___(ORC_VECTOR_BATCH_SIZE, "1024"),

    DDui___(ORC_WRITER_BUFFER_SIZE, "262144"), DDkwd__(ORDERED_HASH_JOIN_CONTROL, "ON"),
    SDDkwd__(OR_OPTIMIZATION, "ON"), DDkwd__(OR_PRED_ADD_BLOCK_TO_IN_LIST, "ON"),
    ...
    DDkwd__(OSIM_READ_OLD_FORMAT, "OFF"),
    ...
    // Timeout for a sequence generator call to idtmsrvr,
    // "0" means no timeout, just check instead.
    // "negative" means never timeout.
    // "positive" means the number of milliseconds to wait before timing out.
    DDintN1__(OVERRIDE_GLOBAL_SEQ_TIMEOUT, "-1"),
    ...
    // Specify the max # of data files in a Hive table
    // above which parallel aggregate pushdown will be
    // allowed to compete with the serial aggregate pushdown
    // plan.
    DDui___(PARALLEL_AGGR_PUSHDOWN, "500"),

    DDui___(PARALLEL_ESP_NODEMASK, "0"),

    // by default all parallelism heuristics are switched ON.
    DDkwd__(PARALLEL_HEURISTIC_1, "ON"), DDkwd__(PARALLEL_HEURISTIC_2, "ON"), DDkwd__(PARALLEL_HEURISTIC_3, "ON"),
    ...
    DDkwd__(PARQUET_AGGR_PUSHDOWN, "ON"),
    // 0 will disable batch read. Postive number denotes maximum # of valid
    // files for which batch read is used. More than limit implies serial read
    DDint__(PARQUET_BATCH_READ_FILESTATS, "2000"), DDkwd__(PARQUET_COLUMNS_PUSHDOWN, "ON"),

    // directly copy the data from Arrow arrays to the
    // row buffer.
    DDkwd__(PARQUET_CPP_READER_DIRECT_COPY, "ON"),

    // Control the application of page filters
    DDkwd__(PARQUET_CPP_READER_FILTER_PAGES, "ON"),

    // Control the application of rowgroup filters
    DDkwd__(PARQUET_CPP_READER_FILTER_ROWGROUPS, "ON"),

    // Specify the number of threads to use to read in and
    // process columns:
    // = 1:  single thread;
    // > 1:  the actual number of threads, no more than the
    //       number of columns;
    // <= 0: the max parallelism (the number of columns).
    DDui___(PARQUET_CPP_READER_PARALLELISM, "1"),

    // skip correctness check for column chunk stats
    // Default is ON.
    DDkwd__(PARQUET_CPP_READER_SKIP_STATS_CHECK, "ON"),

    DD_____(PARQUET_HDFS_STATS_LOG_FILE, ""), DDkwd__(PARQUET_IN_SUPPORT, "OFF"),
    DDkwd__(PARQUET_LEGACY_TIMESTAMP_FORMAT, "ON"), DDkwd__(PARQUET_METADATA_READ_WITH_JAVA_THREADS, "ON"),
    DDkwd__(PARQUET_NJS, "OFF"),

    // CPS is the abbreviation for "compute probe stats"
    DDkwd__(PARQUET_NJS_CPS_DEBUG, "OFF"), DDflt__(PARQUET_NJS_CPS_MAXCARDINALITY, "502000"),
    DDansi_(PARQUET_NJS_CPS_TABLE, ""), DDflt__(PARQUET_NJS_CPS_THRESHOLD, "-1"),
    DDui___(PARQUET_NJS_OUTER_TABLES_THRESHOLD, "1"), DDui___(PARQUET_NJS_PROBES_MAXCARD_FACTOR, "10"),
    DDint__(PARQUET_NJS_PROBES_THRESHOLD, "150"),

    DDkwd__(PARQUET_PRED_PUSHDOWN, "ON"), DDkwd__(PARQUET_READ_NUM_ROWS, "ON"),
    DDkwd__(PARQUET_READ_ROWGROUP_INFO, "ON"), DDkwd__(PARQUET_STRINGS_SIGNED_MIN_MAX, "OFF"),
    DDkwd__(PARQUET_USE_CPP_READER, "OFF"),
    ...
    DDint__(PASSWORD_ERROR_COUNT, "5"), DDint__(PASSWORD_LIFE_TIME, "180"),
    ...
    SDDkwd__(POS, "DISK_POOL"), DD_____(POS_DISKS_IN_SEGMENT, ""), DD_____(POS_DISK_POOL, "0"),
    SDDui___(POS_NUM_DISK_POOLS, "0"), DDui___(POS_NUM_OF_PARTNS, "SYSTEM"),
    SDDint__(POS_NUM_OF_TEMP_TABLE_PARTNS, "SYSTEM"), SDDpos__(POS_TEMP_TABLE_SIZE, ""),
    ...
    DDui___(PRIV_SENTRY_RECHECK_INTERVAL, "60"), DDkwd__(PRIV_SENTRY_USE_USERNAME_API, "ON"),
    DDkwd__(PSHOLD_CLOSE_ON_ROLLBACK, "OFF"), DDkwd__(PSHOLD_UPDATE_BEFORE_FETCH, "OFF"),
    SDDpsch_(PUBLIC_SCHEMA_NAME, ""), DDkwd__(PUSHDOWN_LIMIT_OPT, "OFF"),
    ...
    // query_cache max should be 512 MB. Set it 0 to turn off query cache
    // XDD0_200000(QUERY_CACHE,                  "0"),
    XDD0_524288(QUERY_CACHE, "16384"),
    ...
    DD0_200000(QUERY_CACHE_MAX_VICTIMS, "10"), DD0_255(QUERY_CACHE_REQUIRED_PREFIX_KEYS, "255"),
    DDkwd__(QUERY_CACHE_RUNTIME, "ON"), DD0_255(QUERY_CACHE_SELECTIVITY_ABOSOLUTE, "25"),
    SDDflt0_(QUERY_CACHE_SELECTIVITY_TOLERANCE, "0.05"),

    // query cache statement pinning is off by default
    DDkwd__(QUERY_CACHE_STATEMENT_PINNING, "OFF"), DDkwd__(QUERY_CACHE_STATISTICS, "OFF"),
    DD_____(QUERY_CACHE_STATISTICS_FILE, "qcachsts"), DDkwd__(QUERY_CACHE_TABLENAME, "OFF"),
    ...
    // 0: turn off query logging
    // 1: log DML queries
    DDint__(QUERY_LOGGING, "0"),

    // controls various expr optimizations based on bit flags.
    // see enum QueryOptimizationOptions in DefaultConstants.h
    DDint__(QUERY_OPTIMIZATION_OPTIONS, "3"), DDkwd__(QUERY_STRATEGIZER, "ON"), DDkwd__(QUERY_TEMPLATE_CACHE, "ON"),
    ...
    // Range spec based scan filter.  Specify the amount of memory in bytes to
    // allocate for each filter.
    //
    // 38:      to disable the feature.
    DDui_OptScan(RANGE_OPTIMIZED_SCAN, "38"),

    // Max number of tables at the inner side of a HJ above which
    // no min/max/ROS will be generated.
    DDui___(RANGE_OPTIMIZED_SCAN_MAX_INNER_TABLES, "1"),

    // 0: no limit
    // positive number specifies the max # of filters
    DDui___(RANGE_OPTIMIZED_SCAN_MAX_NUM_FILTERS, "0"),

    // The threshold to control the max. number of keys to insert
    // into a bloom filter before disabling the filter. It takes
    // about 1 sec to evaluate min/max per record expression
    // (minMaxExpr()->perrecExpr()) for 273972 integer values.
    // This default CQD thus allows at most 1 sec to insert values
    // into a bloom filter in HJ operator.
    DDint__(RANGE_OPTIMIZED_SCAN_MAX_NUM_KEYS, "274000"),

    // 0: do not use uec of the inner table to decide BF's # of entries
    // positive value: treat as BF's minimal desirable # of entries.
    DDui___(RANGE_OPTIMIZED_SCAN_MIN_UEC, "0"),

    // A heuristics to remove min/max predicates, but keep
    // the filter predicate.
    DDkwd__(RANGE_OPTIMIZED_SCAN_REMOVE_MINMAX, "OFF"),

    // Define the cycle (# of rows inserted) before a sanity
    // check of the range of values will be performed.
    DDui___(RANGE_OPTIMIZED_SCAN_SANITY_CHECK_CYCLE, "2000"),

    // Specify the ratio of the number of matching rows
    // over the total number of rows in the scanned table.
    //
    // A value of 1.0 means all filters will be applied;
    // A value of 0.5 means only those filters that at most
    // select 50% of the rows  will be applied.
    // A value of 0.2 means only those filters that at most
    // select 20% of the rows  will be applied.
    // In general, a small value keeps a small number of
    // filters that are very selective.
    //
    DDflt_0_1(RANGE_OPTIMIZED_SCAN_SELECTIVITY, "1.0"),

    DDflt1_(RANGE_OPTIMIZED_SCAN_SURVIVE_ROWS_FUDGE_FACTOR, "3.0"),
    ...
    DDflte_(REFERENCE_MSG_REMOTE_TIME, "0.00125"), DDkwd__(REF_CONSTRAINT_NO_ACTION_LIKE_RESTRICT, "SYSTEM"),
    DDflt0_(REGEXP_REPLACE_RESULT_TYPE_FACTOR, "2.0"),
    ...
    DDkwd__(REPORT_SCAN_KEY_INFO, "OFF"),
    ...
    DDflt0_(RISK_PREMIUM_MJ, "1.15"), XDDflt0_(RISK_PREMIUM_NJ, "1.0"), XDDflt0_(RISK_PREMIUM_SERIAL, "1.0"),
    XDDui___(RISK_PREMIUM_SERIAL_SCALEBACK_MAXCARD_THRESHOLD, "10000"),

    DDflt0_(ROBUST_HJ_TO_NJ_FUDGE_FACTOR, "0.0"), DDflt0_(ROBUST_PAR_GRPBY_EXCHANGE_FCTR, "0.25"),
    ...
    DDkwd__(ROUTINE_JOINS_SPOIL_JBB, "OFF"), DDkwd__(ROWSET_ROW_COUNT, "OFF"), DDkwd__(SALT_ON_UPSHIFT_ALLOWED, "OFF"),
    DDint__(SAP_KEY_NJ_TABLE_SIZE_THRESHOLD, "10000000"), DDkwd__(SAP_PREFER_KEY_NESTED_JOIN, "OFF"),
    DDint__(SAP_TUPLELIST_SIZE_THRESHOLD, "5000"), XDDansi_(SCHEMA, "SEABASE"),
    ...
    DD_____(SCRATCH_DIRS, ""), DDkwd__(SCRATCH_DISK_LOGGING, "OFF"), SDDpct__(SCRATCH_FREESPACE_THRESHOLD_PERCENT, "1"),

    // This cqd is temporarily not used.
    ...
    // platforms.
    DDui___(SCRATCH_IO_VECTOR_SIZE_HASH, "8"), DDui___(SCRATCH_IO_VECTOR_SIZE_SORT, "1"),
    DDui___(SCRATCH_MAX_OPENS_HASH, "1"), DDui___(SCRATCH_MAX_OPENS_SORT, "1"), DDui___(SCRATCH_MGMT_OPTION, "11"),
    ...
    // trans. is favored.
    DDflt1_(SEMIJOIN_TO_INNERJOIN_REDUCTION_RATIO, "5.0"), SDDkwd__(SEMIJOIN_TO_INNERJOIN_TRANSFORMATION, "SYSTEM"),
    ...
    DDflt0_(SGB_CPUCOST_INITIALIZE, ".05"), DDui___(SGB_INITIAL_BUFFER_COUNT, "5."),
    DDui1__(SGB_INITIAL_BUFFER_SIZE, "5."), DDkwd__(SHARE_TEMPLATE_CACHED_PLANS, "ON"),
    DDui___(SHORT_OPTIMIZATION_PASS_THRESHOLD, "12"),

    SDDkwd__(SHOWCONTROL_SHOW_ALL, "OFF"), SDDkwd__(SHOWCONTROL_SHOW_SUPPORT, "OFF"),
    DDkwd__(SHOWDDL_DISPLAY_FORMAT, "EXTERNAL"), DDkwd__(SHOWDDL_DISPLAY_PRIVILEGE_GRANTS, "SYSTEM"),
    DDkwd__(SHOWDDL_DISPLAY_TENANT_GROUPS, "OFF"), DDkwd__(SHOWWARN_OPT, "ON"),

    DDkwd__(SHOW_CACHE_KEY, "OFF"), DDkwd__(SIMPLE_COST_MODEL, "ON"),

    XDDkwd__(SKEW_EXPLAIN, "ON"), XDDflt__(SKEW_ROWCOUNT_THRESHOLD, "1000000"),  // Column row count
                                                                                 // threshold below
                                                                                 // which skew
                                                                                 // buster is disabled.
    ...
    //  interbal use only
    DDkwd__(SKIP_CHECK_FOR_TABLE_IB, "OFF"), DDkwd__(SKIP_CONFLICT_CHECK_FOR_INDEX, "ON"),
    DDkwd__(SKIP_CONFLICT_FOR_NOUPDATE_SCAN, "ON"), DDkwd__(SKIP_QUERY_CACHE, "OFF"),
    DDkwd__(SKIP_UNAVAILABLE_PARTITION, "OFF"),

    DDui0_5(SOFT_REQ_HASH_TYPE, "2"), DDkwd__(SORT_ALGO, "HEAP"),
    // Calibration
    // 01/23/98: 10000
    // Original: 10.
    DDflt0_(SORT_CPUCOST_INITIALIZE, "10000."),

    DDui1__(SORT_EX_BUFFER_SIZE, "5."), DDkwd__(SORT_INTERMEDIATE_SCRATCH_CLEANUP, "ON"),
    DDui1__(SORT_IO_BUFFER_SIZE, "128."),

    DD1_200000(SORT_MAX_HEAP_SIZE_MB, "800"), DDkwd__(SORT_MEMORY_QUOTA_SYSTEM, "ON"),
    ...
    // SPJ/SPSQL use aligned format
    DDkwd__(SP_ALIGNED_FORMAT, "ON"),

    // Set SPJ/SPSQL default transaction attribute to NO
    DDkwd__(SP_DEFAULT_NO_TRANSACTION_REQUIRED, "OFF"),

    DDkwd__(SQLMX_REGRESS, "OFF"), SDDui___(SSD_BMO_MAX_MEM_THRESHOLD_IN_MB, "1200"),
    ...
    XDDkwd__(SUBQUERY_UNNESTING, "ON"), DDkwd__(SUBQUERY_UNNESTING_EARLY_GROUP, "OFF"),
    DDkwd__(SUBQUERY_UNNESTING_P2, "ON"), DDkwd__(SUBSTRING_TRANSFORMATION, "OFF"),

    DDkwd__(SUPPRESS_CHAR_LIMIT_CHECK, "OFF"), XDDkwd__(TABLELOCK, "SYSTEM"),
...
// It must be either "DEBUG" or "RELEASE"
    ...
    DDui___(TENANT_DEFAULT_SIZE, "4"), DDflt1_(TENANT_OVERSUBSCRIBE_ERR_NODE_RATIO, "1.0"),
    DDflt1_(TENANT_OVERSUBSCRIBE_WARN_NODE_RATIO, "1.0"), DDkwd__(TERMINAL_CHARSET, (char *)SQLCHARSETSTRING_ISO88591),

    DDint__(TEST_PASS_ONE_ASSERT_TASK_NUMBER, "-1"), DDint__(TEST_PASS_TWO_ASSERT_TASK_NUMBER, "-1"),

    XDDintN2(TIMEOUT, "6000"),

    DDflt0_(TMUDF_CARDINALITY_FACTOR, "1"), DDflt0_(TMUDF_LEAF_CARDINALITY, "1"),

    DDint__(TM_SNAPSHOT_TABLE_CREATES, "1"),

    DDkwd__(TOTAL_RESOURCE_COSTING, "ON"),

    DDkwd__(TRAF_ALIGNED_ROW_FORMAT, "ON"),

    DDkwd__(TRAF_ALLOW_DISABLE_ENABLE_INDEXES, "OFF"),

    DDkwd__(TRAF_ALLOW_ESP_COLOCATION, "OFF"),

    DDkwd__(TRAF_ALLOW_IUD_FOR_HBASE_REPLICA_TABLES, "OFF"),

    DDkwd__(TRAF_ALLOW_POPULATE_ON_ENABLE_INDEXES, "OFF"),

    DDkwd__(TRAF_ALLOW_PURGEDATA_ON_REPLICATED_TABLES, "OFF"),
    ...
    DDkwd__(TRAF_AUTO_REGISTER_GROUP, "OFF"), DDkwd__(TRAF_AUTO_REGISTER_USER, "OFF"),

    DDkwd__(TRAF_AVRO_AGGR_PUSHDOWN, "ON"), DDkwd__(TRAF_AVRO_COLUMNS_PUSHDOWN, "ON"),

    DDkwd__(TRAF_BACKUP_INCLUDE_BINLOG, "ON"),
    // Setting TRAF_BACKUP_PARALLEL to zero will default to
    // number of region servers in cluster.
    DDint__(TRAF_BACKUP_PARALLEL, "0"),

    DDkwd__(TRAF_BACKUP_REPOSITORY_SUPPORT, "OFF"),

    DDkwd__(TRAF_BACKUP_SKIP_GET_TAGS_OWNER_LOOKUP, "ON"),

    DDkwd__(TRAF_BINARY_INPUT, "OFF"), DDkwd__(TRAF_BINARY_OUTPUT, "OFF"), DDkwd__(TRAF_BINARY_SPJ_SUPPORT, "OFF"),
    ...
    DDint__(TRAF_BREI_PROGRESS_UPDATE_DELAY, "0"),

    DDint__(TRAF_BR_LOCKED_OBJECTS_ACCESS, "1"),

    DDint__(TRAF_BR_LOCKED_OBJECTS_QUERY_TYPE, "0"),

    DDkwd__(TRAF_BR_REMOVE_SM_LOCKS, "OFF"),

    DDkwd__(TRAF_CHECK_SHARED_CACHE_LOCKED_AT_LOOKUP, "OFF"),

    DDkwd__(TRAF_CLEAR_METADATA_CACHE, "OFF"),

    DDkwd__(TRAF_CLOB_AS_VARCHAR, "OFF"),  // set to OFF to enable Lobs support

    DDkwd__(TRAF_COL_LENGTH_IS_CHAR, "ON"),

    DD_____(TRAF_COMPOSITE_DATATYPE_FORMAT, "ALIGNED"),

    DDkwd__(TRAF_COMPOSITE_DATATYPE_SUPPORT, "OFF"),

    DDui___(TRAF_CONCURRENT_DDL_WAIT_RETRIES, "5"),
    ...
    DDui___(TRAF_DEFAULT_COMPOSITE_ARRAY_SIZE, "25"),

    DD_____(TRAF_DEFAULT_STORAGE_TYPE, "HBASE"),

    DDkwd__(TRAF_ENABLE_AVRO_FORMAT, "ON"),

    DDkwd__(TRAF_ENABLE_DATA_LOAD_IN_CACHE, "OFF"), DDkwd__(TRAF_ENABLE_DATA_LOAD_IN_SHARED_CACHE, "OFF"),

    DDkwd__(TRAF_ENABLE_METADATA_LOAD_IN_CACHE, "OFF"), DDkwd__(TRAF_ENABLE_METADATA_LOAD_IN_SHARED_CACHE, "OFF"),

    DDkwd__(TRAF_ENABLE_ORC_FORMAT, "ON"),

    DDkwd__(TRAF_ENABLE_PARQUET_FORMAT, "ON"),

    DDint__(TRAF_EXPORT_IMPORT_PARALLEL, "1"),

    DDkwd__(TRAF_EXT_PRED_PUSHDOWN_OPT, "ON"),

    DDkwd__(TRAF_GET_HIVE_OBJECTS, "ON"),

    DDui___(TRAF_HBASE_DROP_CREATE, "1"),

    DDkwd__(TRAF_HBASE_MAPPED_TABLES, "ON"), DDkwd__(TRAF_HBASE_MAPPED_TABLES_IUD, "OFF"),
    DDkwd__(TRAF_HBMT_TREAT_EMPTY_STRING_AS_NULL, "ON"),

    DD_____(TRAF_HIVEMD_SCHEMAS, ""), DDkwd__(TRAF_HIVEMD_SPECIAL_OBJECTNAMETYPE, "ON"),

    DDkwd__(TRAF_IGNORE_EXTERNAL_LDAP_EXISTS_CHECK, "OFF"),

    DDkwd__(TRAF_IGNORE_EXTERNAL_PASSWORD_POLICY_SYNTAX, "OFF"),

    DDkwd__(TRAF_INCREMENTAL_BACKUP, "OFF"),

    DDkwd__(TRAF_INDEX_ALIGNED_ROW_FORMAT, "ON"), DDkwd__(TRAF_INDEX_CREATE_OPT, "OFF"),

    DDkwd__(TRAF_LARGEINT_UNSIGNED_IO, "OFF"),

    DDkwd__(TRAF_LOAD_ALLOW_RISKY_INDEX_MAINTENANCE, "OFF"), DDkwd__(TRAF_LOAD_CONTINUE_ON_ERROR, "OFF"),
    DD_____(TRAF_LOAD_ERROR_COUNT_ID, ""), DD_____(TRAF_LOAD_ERROR_LOGGING_LOCATION, "/user/trafodion/bulkload/logs"),
    DDkwd__(TRAF_LOAD_FORCE_CIF, "ON"), DDkwd__(TRAF_LOAD_LOG_ERROR_ROWS, "OFF"),
    DDint__(TRAF_LOAD_MAX_ERROR_ROWS, "0"), DDint__(TRAF_LOAD_MAX_HFILE_SIZE, "10240"),  // in MB -->10GB by default

    DDkwd__(TRAF_LOAD_PREP_ADJUST_PART_FUNC, "ON"), DDkwd__(TRAF_LOAD_PREP_CLEANUP, "ON"),
    DDkwd__(TRAF_LOAD_PREP_KEEP_HFILES, "OFF"), DDkwd__(TRAF_LOAD_PREP_SKIP_DUPLICATES, "OFF"),

    // need add code to check if folder exists or not. if not issue an error and ask
    // user to create it
    DD_____(TRAF_LOAD_PREP_TMP_LOCATION, "/user/trafodion/bulkload/"), DDint__(TRAF_LOAD_ROWSET_SIZE, "10240"),
    DDkwd__(TRAF_LOAD_TAKE_SNAPSHOT, "OFF"), DDkwd__(TRAF_LOAD_USE_FOR_INDEXES, "ON"),
    DDkwd__(TRAF_LOAD_USE_FOR_STATS, "OFF"),

    // maxlength of chunks to be inserted of selected from clients. 128M
    DDint__(TRAF_LOB_CHUNK_MAXLEN, "134217728"),

    // 1M
    DDui___(TRAF_LOB_DATA_IN_HBASE_COL_LEN, "1048576"),

    // max size 4G
    DDint__(TRAF_LOB_HBASE_DATA_MAXLEN_DDL, "-1"),  // unlimited

    DDint__(TRAF_LOB_HBASE_DATA_MAXLEN_DML, "-1"),

    // max number of inlined bytes used to store inline data.
    // Actual number of chars will depend on
    // bytes per character for the character set.
    // For blobs, bpc = 1.  For clobs, it depends on the charset of clob cols.
    DDui___(TRAF_LOB_INLINED_DATA_MAXBYTES, "0"),

    // max number of inlined characters. Byte storage size will depend on the
    // bytes per character for the character set.
    // For blobs, bpc = 1.  For clobs, it depends on the charset of clob cols.
    DDui___(TRAF_LOB_INLINED_DATA_MAXCHARS, "0"),

    // this is the max size that can be specified for inlined data maxbytes/chars.
    DDui___(TRAF_LOB_INLINED_DATA_MAXLIMIT_IN_BYTES, "204800"),

    DDui___(TRAF_LOB_PARALLEL_DDL, "1"),

    DDkwd__(TRAF_LOB_VERSION2, "ON"),

    DDkwd__(TRAF_LOB_XN_OPER, "ON"),

    //  DDkwd__(TRAF_LOB_VERSION2,        "OFF"),

    DDkwd__(TRAF_LOCK_BACKUP_SCHEMA, "OFF"),

    // Lock DDL operations by adjusting object epochs
    DDkwd__(TRAF_LOCK_DDL, "OFF"),
    ...
    DDui___(TRAF_MAX_COMPOSITE_ARRAY_SIZE, "250"), DDui___(TRAF_MAX_COMPOSITE_LEVELS, "3"),
    ...
    DDint__(TRAF_MAX_TABLES_PER_QUERY, "-1"),
    //  DDint__(TRAF_MAX_TABLES_PER_SELECT,     "120"),
    DDint__(TRAF_MAX_TABLES_PER_SELECT, "-1"),

    DDkwd__(TRAF_MD_REPLICATION, "OFF"),

    DDkwd__(TRAF_METADATA_READ_REPLICA, "OFF"),

    DDkwd__(TRAF_MULTI_COL_FAM, "ON"),

    DDkwd__(TRAF_NON_XN_INCR_BACKUP_OPERATION, "ON"),

    DDkwd__(TRAF_NO_CONSTR_VALIDATION, "OFF"),

    DDkwd__(TRAF_NO_DTM_XN, "OFF"),

    DDint__(TRAF_NUM_OF_SALT_PARTNS, "-1"), DDint__(TRAF_NUM_OF_SALT_REGIONS, "-1"),

    DD_____(TRAF_OBJECT_ENCRYPTION, ""),

    // Behavior when ObjectEpochCache is full
    // 0 - raise 8151 error (object epoch cache is full)
    // 1 - raise 8151 warning
    // 2 - ignore the condition, but log it
    DDui0_2(TRAF_OBJECT_EPOCH_CACHE_FULL_BEHAVIOR, "0"),

    // Object Lock to prevent concurrent DML/DDL
    DDkwd__(TRAF_OBJECT_LOCK, "ON"), DDui1__(TRAF_OBJECT_LOCK_DDL_DIST_LOCK_TIMEOUT, "60000"),  // in MilliSeconds
    DDui1__(TRAF_OBJECT_LOCK_DDL_RETRY_DELAY, "1000"),                                          // in MilliSeconds
    DDui1__(TRAF_OBJECT_LOCK_DDL_RETRY_LIMIT, "30"),
    DDui1__(TRAF_OBJECT_LOCK_DML_DIST_LOCK_TIMEOUT, "1000"),                                  // in MilliSeconds
    DDui1__(TRAF_OBJECT_LOCK_DML_RETRY_DELAY, "1000"),                                        // in MilliSeconds
    DDui1__(TRAF_OBJECT_LOCK_DML_RETRY_LIMIT, "30"), DDkwd__(TRAF_OBJECT_LOCK_FORCE, "OFF"),  // internal use only
    DDkwd__(TRAF_OBJECT_LOCK_UNSAFE_MODE, "OFF"),

    DD_____(TRAF_OBJECT_NAMESPACE, ""),

    DDkwd__(TRAF_PARTITIONV2_CONSTRAINT_RAISE_ERROR, "ON"),

    DDint__(TRAF_PASSWORD_GRACE_COUNTER, "5"),

    // default syntax of password check policy
    DD_____(TRAF_PASSWORD_POLICY_SYNTAX, "000000000000"),

    DDkwd__(TRAF_READ_OBJECT_DESC, "ON"),

    DDkwd__(TRAF_RELOAD_NATABLE_CACHE, "OFF"),

    // Setting TRAF_RESTORE_PARALLEL to zero will default to
    // number of region servers in cluster.
    DDint__(TRAF_RESTORE_PARALLEL, "0"),

    DDkwd__(TRAF_RETURN_LOB_HANDLE_WITHOUT_INLINE_DATA, "OFF"),

    // TRAF_ROWNUM_DEFAULT_ORDER_LEVEL :
    // 0 : do not generate default order for ROWNUM
    // 1 : generate default order only when table is partitioned
    // >1 : always generate default order for ROWNUM
    DDui0_5(TRAF_ROWNUM_DEFAULT_ORDER_LEVEL, "1"),

    DD_____(TRAF_SAMPLE_TABLE_LOCATION, "/user/trafodion/sample/"),

    DDkwd__(TRAF_SAVEPOINTS, "ON"),

    DD0_64(TRAF_SAVEPOINT_BATCH_COMMIT_COUNT, "0"),

    DDint__(TRAF_SEQUENCE_CACHE_SIZE, "-1"),

    DDint__(TRAF_SEQUENCE_RETRY_TIMES, "100"),

    DDkwd__(TRAF_SEQUENCE_USE_DLOCK, "OFF"), DDkwd__(TRAF_SEQUENCE_USE_DTM, "OFF"),

    DD_____(TRAF_SET_SEQUENCE_ID, ""), DDkwd__(TRAF_SIMILARITY_CHECK, "ROOT"),

    DDkwd__(TRAF_SKIP_WAL_FOR_ORDER_SEQ, "OFF"),

    DDkwd__(TRAF_START_UDR, "OFF"),

    DDkwd__(TRAF_STORE_OBJECT_DESC, "OFF"),

    DDkwd__(TRAF_STRING_AUTO_TRUNCATE, "OFF"), DDkwd__(TRAF_STRING_AUTO_TRUNCATE_WARNING, "OFF"),

    DDkwd__(TRAF_SWITCH_COMPILER, "ON"),
    ...
    // FORCE  --> Snapshot scan enabled for queries, will create a new
    //           snapshot before scan, and will delete this new snapshot
    //           when scan finishs. Need to work with CQD TRAF_NO_DTM_XN 'on'
    DDkwd__(TRAF_TABLE_SNAPSHOT_SCAN, "NONE"), DD_____(TRAF_TABLE_SNAPSHOT_SCAN_SNAP_SUFFIX, "SNAP"),
    ...
    DDkwd__(TRAF_TENANT_DROP_MINIMAL, "OFF"), DDkwd__(TRAF_TENANT_GROUP_CHECK, "OFF"),

    DDkwd__(TRAF_TINYINT_INPUT_PARAMS, "OFF"), DDkwd__(TRAF_TINYINT_RETURN_VALUES, "OFF"),
    DDkwd__(TRAF_TINYINT_SPJ_SUPPORT, "OFF"), DDkwd__(TRAF_TINYINT_SUPPORT, "ON"),

    // DTM Transaction Type: MVCC, SSCC
    XDDkwd__(TRAF_TRANS_TYPE, "MVCC"),

    DDkwd__(TRAF_UNIQUE_IUD_XN_OPT, "OFF"),

    DD_____(TRAF_UNLOAD_DEF_DELIMITER, "|"), DD_____(TRAF_UNLOAD_DEF_RECORD_SEPARATOR, "\n"),
    DDint__(TRAF_UNLOAD_HDFS_COMPRESS, "0"), DDkwd__(TRAF_UPDATE_TO_UPSERT, "ON"),
    DDkwd__(TRAF_UPSERT_ADJUST_PARAMS, "OFF"), DDkwd__(TRAF_UPSERT_MODE, "MERGE"),
    DDkwd__(TRAF_UPSERT_THROUGH_PARTITIONS, "OFF"),  // internal use for merge range parititon
    DDkwd__(TRAF_UPSERT_TO_EFF_TREE, "ON"), DDint__(TRAF_UPSERT_WB_SIZE, "2097152"),
    DDkwd__(TRAF_UPSERT_WRITE_TO_WAL, "OFF"),

    DDkwd__(TRAF_USE_REGION_XN, "ON"),

    DDkwd__(TRAF_USE_RWRS_FOR_MD_INSERT, "ON"),

    DDkwd__(TRAF_USE_STORED_STATS, "ALL"), DDkwd__(TRAF_USTAT_GENERATE_STORED_STATS, "OFF"),
    DDkwd__(TRANSLATE_ERROR, "ON"), DDkwd__(TRANSLATE_ERROR_UNICODE_TO_UNICODE, "ON"),
    ...
    SDD_____(UDR_JAVA_OPTIONS, "OFF"), DD_____(UDR_JAVA_OPTION_DELIMITERS, " "), XDDui___(UDR_JVM_DEBUG_PORT, "0"),
    XDDui___(UDR_JVM_DEBUG_TIMEOUT, "0"),

    DDkwd__(UNAVAILABLE_PARTITION, "STOP"),  // "?" used?
    DDkwd__(UNC_PROCESS, "OFF"), DDkwd__(UNION_MAP_REDUCTION, "ON"), SDDkwd__(UNIQUE_HASH_JOINS, "SYSTEM"),
    SDDui___(UNIQUE_HASH_JOIN_MAX_INNER_SIZE, "1000"), SDDui___(UNIQUE_HASH_JOIN_MAX_INNER_SIZE_PER_INSTANCE, "100"),
    SDDui___(UNIQUE_HASH_JOIN_MAX_INNER_TABLES, "2"), DDkwd__(UNIQUE_USERID, "ON"),
    DDui___(UNOPTIMIZED_ESP_BUFFER_SIZE_DOWN, "31000"), DDui___(UNOPTIMIZED_ESP_BUFFER_SIZE_UP, "31000"),
    DDui1__(UPDATED_BYTES_PER_ESP, "400000"), DDkwd__(UPDATE_CLUSTERING_OR_UNIQUE_INDEX_KEY, "ON"),
    ...
    DD_____(USER_GROUPNAMES, ""),  // user groups to consider wheh gathering privileges
    ...
    DDkwd__(USE_HDFS_WRITE_LOCK, "ON"),
    ...
    DDint__(USE_ROUND_ROBIN_FOR_RANDOMNUM, "3"),  // hash2(RandomNum) ==> round-robin, and use thread id for seed
    DDkwd__(USE_TRIGGER, "ON"),
    DDkwd__(USTAT_ADD_SALTED_KEY_PREFIXES_FOR_MC,
            "ON"),  // When ON, generate MCs for primary key prefixes as well as full key
    ...
    DDui___(USTAT_AUTO_MISSING_STATS_LEVEL,
            "4"),  // Similar to HIST_MISSING_STATS_WARNING_LEVEL, but controls
    ...
    DDkwd__(USTAT_COLLECT_MC_SKEW_VALUES, "OFF"), DDkwd__(USTAT_COLLECT_VIRT_COL_STATS, "OFF"),
    ...
    DDflte_(USTAT_DSHMAX, "50.0"), DDkwd__(USTAT_ESTIMATE_HBASE_ROW_COUNT, "ON"),
    DDkwd__(USTAT_FETCHCOUNT_ACTIVE, "OFF"), DDkwd__(USTAT_FORCE_MOM_ESTIMATOR, "OFF"),
    DDkwd__(USTAT_FORCE_TEMP, "OFF"), DDflt0_(USTAT_FREQ_SIZE_PERCENT, "0.5"),  // >100 effectively disables
    DDflt0_(USTAT_GAP_PERCENT, "10.0"), DDflt0_(USTAT_GAP_SIZE_MULTIPLIER, "1.5"),
    DDkwd__(USTAT_GENERATE_ORC_HISTOGRAM, "OFF"),
    DDui___(USTAT_HBASE_SAMPLE_RETURN_INTERVAL,
            "10000000"),  // Avoid scanner timeout by including on average at
    ...
    DDflt_0_1(USTAT_IS_MEMORY_FRACTION, "0.6"), DDflt0_(USTAT_IUS_INTERVAL_ROWCOUNT_CHANGE_THRESHOLD, "0.05"),
    ...
    DDkwd__(USTAT_IUS_NO_BLOCK, "OFF"), DDansi_(USTAT_IUS_PERSISTENT_CBF_PATH, "SYSTEM"),

    DDflt0_(USTAT_IUS_TOTAL_ROWCOUNT_CHANGE_THRESHOLD, "0.05"), DDflt0_(USTAT_IUS_TOTAL_UEC_CHANGE_THRESHOLD, "0.05"),
    ...
    DDui___(USTAT_MAX_CHAR_COL_LENGTH_IN_BYTES,
            "256"),                                   // When computing UECs, char cols are limited to this many bytes
    ...
    DDflt0_(USTAT_MIN_ESTIMATE_FOR_ROWCOUNT, "10000000"), XDDui1__(USTAT_MIN_ROWCOUNT_FOR_LOW_SAMPLE, "1000000"),
    XDDui1__(USTAT_MIN_ROWCOUNT_FOR_SAMPLE, "10000"), DDflt0_(USTAT_MODIFY_DEFAULT_UEC, "0.05"),
    ...
    DDui1__(USTAT_NUM_MC_GROUPS_FOR_KEYS, "10"), DDkwd__(USTAT_PROCESS_GAPS, "ON"),
    DDkwd__(USTAT_REQUIRE_DISTRIBUTED_LOCK,
            "ON"),  // require a distributed lock on table name for write UPDATE STATS operations
    DD0_255(USTAT_RETRY_DELAY, "100"), DD0_255(USTAT_RETRY_LIMIT, "3"),
    DD0_255(USTAT_RETRY_NEC_COLS_LIMIT, "3"),  // by default, use retry for AddNecessaryColumns
    DDpct__(USTAT_SAMPLE_PERCENT_DIFF, "10"), DDansi_(USTAT_SAMPLE_TABLE_NAME, " "),
    DDansi_(USTAT_SAMPLE_TABLE_NAME_CREATE, " "), DDkwd__(USTAT_SHOW_MC_INTERVAL_INFO, "OFF"),
    DDkwd__(USTAT_SHOW_MFV_INFO, "OFF"), DDflte_(USTAT_UEC_HI_RATIO, "0.5"), DDflte_(USTAT_UEC_LOW_RATIO, "0.1"),
    DDkwd__(USTAT_USE_BACKING_SAMPLE, "OFF"), DDkwd__(USTAT_USE_BULK_LOAD, "OFF"),
    DDkwd__(USTAT_USE_GROUPING_FOR_SAMPLING, "ON"), DDkwd__(USTAT_USE_INTERNAL_SORT_FOR_MC, "ON"),
    ...
    DD1_8192(VARCHAR_PARAM_DEFAULT_SIZE, "255"),

    // this is the default length of a param in function which is typed as a VARCHAR.
    // currently only used in instr
    DD1_8192(VARCHAR_PARAM_DEFAULT_SIZE_IN_FUNCTION, "8000"),
    ...
    DDkwd__(VSBB_TEST_MODE, "OFF"), DDkwd__(WAIT_ON_SELECT_FOR_UPDATE, "ON"),
    XDDkwd__(WMS_CHILD_QUERY_MONITORING, "OFF"), XDDkwd__(WMS_QUERY_MONITORING, "OFF"),
    // amount of work we are willing to assign per CPU for any query
    // not running at full system parallelism
    SDDflte_(WORK_UNIT_ESP, "0.08"), SDDflte_(WORK_UNIT_ESP_DATA_COPY_COST, "0.001"),

    // ZIG_ZAG_TREES ON means do ZIG_ZAG_TREES
    // $$$ OFF for beta
    DDkwd__(ZIG_ZAG_TREES, "SYSTEM"), DDkwd__(ZIG_ZAG_TREES_CONTROL, "OFF")
...
class NADefaults::HeldDefaults {
 public:
  ...
  short pushDefault(const char *value);
...
NADefaults::HeldDefaults::HeldDefaults(void) : stackPointer_(0) {
  for (int i = 0; i < STACK_SIZE; i++) stackValue_[i] = NULL;
}
...
// RETURN: -1, if error. 0, if success
short NADefaults::HeldDefaults::pushDefault(const char *value) {
  if (stackPointer_ >= STACK_SIZE) return -1;
...
size_t NADefaults::numDefaultAttributes() { return (size_t)__NUM_DEFAULT_ATTRIBUTES; }

// Returns current defaults in alphabetic order (for SHOWCONTROL listing).
const char *NADefaults::getCurrentDefaultsAttrNameAndValue(size_t ix, const char *&name, const char *&value,
  ...
  if (ix < numDefaultAttributes()) {
    ...
    if (userDefaultsOnly) {
      ...
      get = (defaultDefaults[ix].flags & DEFAULT_IS_EXTERNALIZED) ||  // bit-AND
            ((defaultDefaults[ix].flags & DEFAULT_IS_FOR_SUPPORT) && (getToken(SHOWCONTROL_SHOW_SUPPORT) == DF_ON)) ||
...
void NADefaults::initGlobalEntries() {
  const size_t numAttrs = numDefaultAttributes();
  if (numAttrs != sizeof(defaultDefaults) / sizeof(DefaultDefault)) return;
...
void NADefaults::initCurrentDefaultsWithDefaultDefaults() {
  ...
  CMPASSERT_STRING(numAttrs == sizeof(defaultDefaults) / sizeof(DefaultDefault),
                   "Check sqlcomp/DefaultConstants.h for a gap in enum DefaultConstants or sqlcomp/nadefaults.cpp for "
                   "duplicate entries in array defaultDefaults[].");

  SqlParser_NADefaults_Glob = SqlParser_NADefaults_ = new NADHEAP SqlParser_NADefaults();
  ...
  for (i = 0; i < numAttrs; i++) {
    ...
    // defDefIx_ and defaultDefaults arrays must have been initialized
    // by the time this code is reached.
    if (defaultDefaults[defDefIx_[i]].attrEnum != i) {
    ...
    if (!defaultDefaults[i].validator->validate(defaultDefaults[i].value, this, defaultDefaults[i].attrEnum,
                                                +1 /*warning*/)) {
      SqlParser_NADefaults_ = NULL;

      cerr << "\nERROR: " << defaultDefaults[i].attrName << " has invalid value" << defaultDefaults[i].value << endl;
...
// for debugging only
    ...
    if (nadval) {  // additional sanity checking we want to do occasionally
      ...
      CMPASSERT(i == 0 || strcmp(defaultDefaults[i - 1].attrName, defaultDefaults[i].attrName) < 0);
      ...
      // validate initial default default values
      CMPASSERT(defaultDefaults[i].validator);
      defaultDefaults[i].validator->validate(defaultDefaults[i].value, this, defaultDefaults[i].attrEnum,
                                             +1 /*warning*/);
      ...
      if (defaultDefaults[i].validator == &validateUnknown)
        cerr << "\nWARNING: " << defaultDefaults[i].attrName << " has a NO-OP validator" << endl;
  ...
  // Cache all the Keywords such as "ON", "OFF" up front,
  ...
  for (i = 0; i < numAttrs; i++) {
    ...
    if (validator(i)->getType() == VALID_KWD && (i != NATIONAL_CHARSET) && (i != INPUT_CHARSET) && (i != ISO_MAPPING)) {
  ...
  SqlParser_NADefaults_->ORIG_DEFAULT_CHARSET_ = CharInfo::getCharSetEnum(currentDefaults_[DEFAULT_CHARSET]);
...
NADefaults::~NADefaults() { deleteMe(); }

void NADefaults::deleteMe() {
  if (resetToDefaults_) {
    for (size_t i = numDefaultAttributes(); i--;) NADELETEBASIC(resetToDefaults_[i], NADHEAP);
  ...
  if (currentDefaults_) {
    for (size_t i = numDefaultAttributes(); i--;)
      if (provenances_[i] > INIT_DEFAULT_DEFAULTS) NADELETEBASIC(currentDefaults_[i], NADHEAP);
    NADELETEBASIC(currentDefaults_, NADHEAP);
    currentDefaults_ = NULL;
  }

  if (currentFloats_) {
    for (size_t i = numDefaultAttributes(); i--;) NADELETEBASIC(currentFloats_[i], NADHEAP);
  ...
  if (currentTokens_) {
    for (size_t i = numDefaultAttributes(); i--;)
      if (provenances_[i] > INIT_DEFAULT_DEFAULTS) NADELETEBASIC(currentTokens_[i], NADHEAP);
    NADELETEBASIC(currentTokens_, NADHEAP);
    currentTokens_ = NULL;
  }

  if (heldDefaults_) {
    for (size_t i = numDefaultAttributes(); i--;) NADELETE(heldDefaults_[i], HeldDefaults, NADHEAP);
...
enum DefaultConstants NADefaults::lookupAttrName(const char *name, int errOrWarn, int *position) {
  ...
  if (position != 0) *position = split;

  // if the last comparison result was equal, return value at "split"
  if (cresult == 0) return defaultDefaults[split].attrEnum;
...
void NADefaults::updateSystemParameters(NABoolean updateDefaultDefaults) {
  static int arrayOfSystemParameters[] = {
      ...
      DEF_VIRTUAL_MEMORY_AVAILABLE, USTAT_IUS_PERSISTENT_CBF_PATH};  // returned in KB not bytes
  ...
  for (size_t i = 0; i < numElements; i++) {
    ...
    NAClusterInfo *nac = CURRCONTEXT_CLUSTERINFO;
    ...
    switch (arrayOfSystemParameters[i]) {
      ...
      case DEF_NUM_SMP_CPUS:
        utoa_(nac->numberOfCpusPerSMP(), valuestr);
        strcpy(newValue, valuestr);
        break;
      ...
      case MAX_ESPS_PER_CPU_PER_OP: {
        float espsPerCore;
        if (updateDefaultDefaults)
          espsPerCore = computeNumESPsPerCore();
        else if (CURRSTMT_OPTDEFAULTS && !CURRSTMT_OPTDEFAULTS->canUseAllClusterResources()) {
          espsPerCore = ActiveSchemaDB()->getDefaults().computeNumESPsPerCoreForTenant();
        }
      ...
      case DEF_NUM_NODES_IN_ACTIVE_CLUSTERS:
        utoa_(nac->getTotalNumberOfCPUs(), valuestr);
        strcpy(newValue, valuestr);
        break;

      case DEF_PHYSICAL_MEMORY_AVAILABLE:
        ultoa_(nac->physicalMemoryAvailableInKB(), valuestr);
        strcpy(newValue, valuestr);
        break;

      case DEF_TOTAL_MEMORY_AVAILABLE:
        ultoa_(nac->totalMemoryAvailableInKB(), valuestr);
        strcpy(newValue, valuestr);
        break;

      case DEF_VIRTUAL_MEMORY_AVAILABLE:
        utoa_(nac->virtualMemoryAvailable(), valuestr);
        strcpy(newValue, valuestr);
        break;

      case DEF_NUM_BM_CHUNKS: {
        UInt32 numChunks = (UInt32)(nac->physicalMemoryAvailableInKB() / def_DEF_CHUNK_SIZE / 4);
      ...
      default:
        cerr << "updateSystemParameters: no case for " << lookupAttrName(arrayOfSystemParameters[i]) << endl;
        break;
...
inline static NABoolean initializeSQLdone() { return FALSE; }
...
void NADefaults::readFromDefaultsTable(Provenance overwriteIfNotYet, int errOrWarn) {
  ...
  if (!readFromDefaultsTable_) {
    ...
    if (errNum == 0)  // seabase is initialized properly
    {
      ...
      cmpSBD.readAndInitDefaultsFromSeabaseDefaultsTable(overwriteIfNotYet, errOrWarn, this);
      ...
      CmpContext::useReservedNameSpace_ = CmpCommon::context()->useReservedNamespace();
    } else {
      ...
      if (errNum != -1393         // 1393 equals TRAF_NOT_INITIALIZED
              && errNum != -1399  // 1399 equals TRAF_COLUMN_VALUE_EMPTY
              && errNum != -1395 ||
    ...
    CmpCommon::context()->setUseReservedNamespace(CmpContext::useReservedNameSpace_);
...
int NADefaults::validateFloat(const char *value, float &result, int attrEnum, int errOrWarn) const {
  ...
  if (errOrWarn)
    *CmpCommon::diags() << DgSqlCode(ERRWARN(2055)) << DgString0(value)
                        << DgString1(lookupAttrName(attrEnum, errOrWarn));

  if (useSYSTEM) {  // programmer error
    CMPASSERT(
        "Numeric attr allows SYSTEM -- you need to call token() first to see if its current value is this keyword, and "
        "compute your system default value!" == NULL);
  }
...
NABoolean NADefaults::insert(int attrEnum, const NAString &value, int errOrWarn) {
  ...
  if (currentTokens_[attrEnum]) {
    if (provenances_[attrEnum] > INIT_DEFAULT_DEFAULTS) NADELETEBASIC(currentTokens_[attrEnum], NADHEAP);
  ...
  if (isSetOnceAttribute(attrEnum) && Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL)) {
...
float NADefaults::getNumOfESPsPerNodeInFloat() const {
  ...
  if (CmpCommon::context()) {
    if (CURRSTMT_OPTDEFAULTS->isFakeHardware()) {
      cores = getAsLong(DEF_NUM_SMP_CPUS);
    } else {
      if (!CURRSTMT_OPTDEFAULTS->canUseAllClusterResources()) {
        cores = NAWNODESET_CORES_PER_SLICE;
        // cout << "getNumOfESPsPerNodeInFloat(): tenant case" << endl;
        // cout << "maxEspPerCpuPerOp=" << maxEspPerCpuPerOp << endl;
        // cout << "coresInTenantUnit=" << cores << endl;
      } else {
        cores = CURRCONTEXT_CLUSTERINFO->numberOfCpusPerSMP();
      }
    }
  } else {
    if (CURRSTMT_OPTDEFAULTS->isFakeHardware())
...
int NADefaults::getTotalNumOfESPs(NABoolean &fakeEnv) const {
  ...
  if (fakeEnv) {
#if 0
   if ( CmpCommon::context()->getCIClass() != CmpContextInfo::CMPCONTEXT_TYPE_META
      ) 
   {
     cout << "NADefaults::getTotalNumOfESPs(): "
          << " fakeEnv, CQD PARALLEL_NUM_ESPS=" <<getAsLong(PARALLEL_NUM_ESPS) 
          << ", #esps=" <<getAsLong(PARALLEL_NUM_ESPS) 
          << endl;
   }
#endif
  ...
  if (CmpCommon::context()) {
    if (CURRSTMT_OPTDEFAULTS->isFakeHardware()) {
      ...
      if (!CURRSTMT_OPTDEFAULTS->canUseAllClusterResources()) {
        numOfNodes = CmpCommon::context()->getAvailableNodes()->getNumNodes();
      } else {
        numOfNodes = CmpCommon::context()->getNumOfSMPs();
      }
    }
  } else {
    numOfNodes = CmpCommon::context()->getNumOfSMPs();
...
NABoolean NADefaults::domainMatch(int attrEnum, int expectedType /*DefaultValidatorType*/, float *flt) const {
  ...
  if (flt) {
    ...
    if (expectedType == VALID_INT)
      validator = (DefaultValidator *)&validateInt;
    else if (expectedType == VALID_UINT)
      validator = (DefaultValidator *)&validateUI;
    ...
    if (validator)
      if (validator->validate(currentDefaults_[attrEnum], this, attrEnum, -1, flt) == TRUE)
  ...
  *CmpCommon::diags() << DgSqlCode(+2058)  // emit a mismatch WARNING
                      << DgString0(lookupAttrName(attrEnum)) << DgString1(validator(attrEnum)->getTypeText())
                      << DgString2(DefaultValidator::getTypeText(DefaultValidatorType(expectedType)));
#ifndef NDEBUG
  cerr << "Warning[2058] " << lookupAttrName(attrEnum) << " " << validator(attrEnum)->getTypeText() << " "
       << DefaultValidator::getTypeText(DefaultValidatorType(expectedType)) << " " << (flt ? *flt : 123.45) << endl;
#endif
...
void NADefaults::resetAll(NAString &value, short reset, int errOrWarn) {
  ...
  if (reset == 1) {  // CQD * RESET; (not RESET RESET)
    ...
    if ((getToken(DEFAULT_SCHEMA_NAMETYPE) == DF_USER) && schSetByNametype()) {
      setSchemaAsLdapUser();
    }
    setResetAll(FALSE);
  } else if (reset == 2) {
...
NABoolean NADefaults::isReadonlyAttribute(const char *attrName) const {
  ...
  if ((stricmp(attrName, "ISO_MAPPING") == 0) || (stricmp(attrName, "NATIONAL_CHARSET") == 0) ||
      (stricmp(attrName, "VALIDATE_VIEWS_AT_OPEN_TIME") == 0) || (stricmp(attrName, "USER_EXPERIENCE_LEVEL") == 0) ||
  ...
  if (strlen(attrName) > 0) {
    ...
    if ((v != __INVALID_DEFAULT_ATTRIBUTE) && (getFlags(v) & DEFAULT_IS_SSD)) return TRUE;
  }
...
NABoolean NADefaults::isNonResetableAttribute(const char *attrName) const {
  if ((stricmp(attrName, "IS_SQLCI") == 0) || (stricmp(attrName, "IS_TRAFCI") == 0) ||
      (stricmp(attrName, "SESSION_ID") == 0) || (stricmp(attrName, "LDAP_USERNAME") == 0) ||
      (stricmp(attrName, "VOLATILE_SCHEMA_IN_USE") == 0) || (stricmp(attrName, "SESSION_USERNAME") == 0))
    return TRUE;
...
void NADefaults::setMultiCQDSValue(DefaultConstants attrEnum, NAString &rqoValue, int errOrWarn) {
  ...
  switch (tk) {
    case DF_MAXIMUM: {
      insert(RISK_PREMIUM_NJ, "5.0", errOrWarn);
      insert(RISK_PREMIUM_SERIAL, "2.0", errOrWarn);
      insert(PARTITIONING_SCHEME_SHARING, "2", errOrWarn);
      insert(ROBUST_HJ_TO_NJ_FUDGE_FACTOR, "1.0", errOrWarn);
      insert(ROBUST_SORTGROUPBY, "2", errOrWarn);
      insert(RISK_PREMIUM_MJ, "2.0", errOrWarn);
    } break;
    case DF_HIGH: {
      insert(RISK_PREMIUM_NJ, "2.5", errOrWarn);
      insert(RISK_PREMIUM_SERIAL, "1.5", errOrWarn);
      insert(PARTITIONING_SCHEME_SHARING, "1", errOrWarn);
      insert(ROBUST_HJ_TO_NJ_FUDGE_FACTOR, "3.0", errOrWarn);
      insert(ROBUST_SORTGROUPBY, "2", errOrWarn);
      insert(RISK_PREMIUM_MJ, "1.5", errOrWarn);
    } break;
    case DF_MINIMUM: {
      insert(RISK_PREMIUM_NJ, "1.0", errOrWarn);
      insert(RISK_PREMIUM_SERIAL, "1.0", errOrWarn);
      insert(PARTITIONING_SCHEME_SHARING, "0", errOrWarn);
      insert(ROBUST_HJ_TO_NJ_FUDGE_FACTOR, "0.0", errOrWarn);
      insert(ROBUST_SORTGROUPBY, "0", errOrWarn);
      insert(RISK_PREMIUM_MJ, "1.0", errOrWarn);
    } break;
    case DF_SYSTEM: {
...
enum DefaultConstants NADefaults::validateAndInsert(const char *attrName, NAString &value, NABoolean reset,
  ...
  if (attrEnum >= 0) {  // valid attrName
    ...
    if (attrEnum == DEFAULT_SCHEMA_ACCESS_ONLY && getState() < SET_BY_CQD) return attrEnum;

    // do the following check when
    // this is the primary mxcmp
    // and INTERNAL_QUERY_FROM_EXEUTIL is not set

    if (!CmpCommon::context()->isSecondaryMxcmp() && !Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL)) {
      ...
      if ((isSetOnceAttribute(attrEnum)) && (!isResetAll()) &&  // no error msg for cqd * reset
      ...
      if (getState() >= SET_BY_CQD && getToken(DEFAULT_SCHEMA_ACCESS_ONLY) == DF_ON) {
        if (attrEnum == SCHEMA || attrEnum == PUBLIC_SCHEMA_NAME || attrEnum == DEFAULT_SCHEMA_NAMETYPE) {
...
#if 0
      // Only users with elevated privileges can set the following CQDs.
      // this is causing odbc connections to fail during reset of CQSs when
      // a user changes.  Need to figure out a different way of handling this
      // or change connection code to override access.
      // commenting out check for now
      if ( getState() >= SET_BY_CQD &&
           attrEnum == CAT_ANSI_PRIVS_FOR_TENANT ) 
      {
         if (!ComUser::isRootUserID())
         {
           *CmpCommon::diags() << DgSqlCode(-30051) << DgString0(attrName);
           return attrEnum;
         }
      }
#endif
      ...
      if (attrEnum == LAST0_MODE) return attrEnum;
    }
    ...
    if (reset) {  // CQD attr RESET;
      ...
      if ((isNonResetableAttribute(attrName)) && (reset != 3)) return attrEnum;

      if (!resetToDefaults_[attrEnum]) {
        if (overwrite) value = currentDefaults_[attrEnum];  // return actual val to caller
    ...
    if (attrEnum == CATALOG) {
      ...
      if (attrEnum == SCRATCH_MAX_OPENS_HASH || attrEnum == SCRATCH_MAX_OPENS_SORT) {
        if (NOT((value.length() == 1) && ((*value.data() == '1') || (*value.data() == '2') || (*value.data() == '3') ||
                                          (*value.data() == '4')))) {
          *CmpCommon::diags() << DgSqlCode(-2055) << DgString0(value) << DgString1(lookupAttrName(attrEnum));
      ...
      switch (attrEnum) {
        case TRAF_PASSWORD_POLICY_SYNTAX:
        case TRAF_IGNORE_EXTERNAL_PASSWORD_POLICY_SYNTAX:
        case MIN_PASSWORD_LENGTH:
        case TRAF_IGNORE_EXTERNAL_LDAP_EXISTS_CHECK:
        case UNIQUE_USERID:
        // No effect, just a warning
        case PASSWORD_ERROR_COUNT:
        ...
        case PASSWORD_LIFE_TIME: {
          ...
          if (NOT isInternalOperation && NOT ComUser::isManagerUserID(uid)) {
            ...
            CmpSeabaseDDLauth::AuthStatus authStatus = authInfo.getAdminRole(uid, adminRole);
            if (isAuthorization && (hasPriv || CmpSeabaseDDLauth::STATUS_GOOD == authStatus)) {
              // ok!
            } else {
              if (NOT authInfo.verifyAuthority(SQLOperation::MANAGE_SECURE)) {
                *CmpCommon::diags() << DgSqlCode(ERRWARN(1017));
                attrEnum = __INVALID_DEFAULT_ATTRIBUTE;
                break;
              }
            }
          }
          if (TRAF_PASSWORD_POLICY_SYNTAX == attrEnum) {
            // If it is on the step for initializing schemaDB, let it be warned on CmpCommon::loadPasswordCheckSyntax
            if (NOT isInternalOperation) {
              if ("OFF" == getValue(TRAF_IGNORE_EXTERNAL_PASSWORD_POLICY_SYNTAX)) {
                *CmpCommon::diags() << DgSqlCode(ERRWARN(1717))
                                    << DgString0("TRAF_IGNORE_EXTERNAL_PASSWORD_POLICY_SYNTAX")
                                    << DgString1("TRAF_PASSWORD_POLICY_SYNTAX");
                attrEnum = __INVALID_DEFAULT_ATTRIBUTE;
              } else {
                ComPwdPolicySyntax tmp;  // i don't care
                if (!CmpCommon::ConvertPasswordPolicyString(value.data(), tmp, "the CQD statement", false)) {
                  attrEnum = __INVALID_DEFAULT_ATTRIBUTE;
                }
              }
            }
          } else if (MIN_PASSWORD_LENGTH == attrEnum) {
            int num = atoi(value.data());
            if (6 > num) {
              value = "6";
            } else if (128 < num) {
              value = "128";
        ...
        case ALLOW_ROLE_GRANTS_TO_GROUPS: {
          NABoolean isInternalOperation = CmpCommon::context()->isInStepForInitSchemaDB();
          // ALLOW_ROLE_GRANTS_TO_GROUPS is dangerous commands that would destroy the query cache
          if (NOT isInternalOperation && NOT ComUser::isManagerUser()) {
            // only for jenkis
      ...
      if (attrEnum != __INVALID_DEFAULT_ATTRIBUTE) {
        ...
        if ((!isValid) && (isNonResetableAttribute(attrName)) && (reset == 3)) {
          isValid = TRUE;
        }
        ...
        if (!isValid)
          ...
          if (!insert(attrEnum, value, errOrWarn)) attrEnum = __INVALID_DEFAULT_ATTRIBUTE;
  ...
  if (attrEnum >= 0) {
    if (overwrite) {
      if ((!reset) && (currentState_ == SET_BY_CQD)) {
      ...
      switch (attrEnum) {
        ...
        case CONFIGURE_TESTPOINT: {
          if (reset) {
            CNATestPointArray *testPoints = CmpCommon::context()->getTestPointArray();
            if (testPoints) testPoints->resetAllTestPoints();
          } else {
            CNATestPointArray *testPoints = CmpCommon::context()->getOrCreateTestPointArray();
            CMPASSERT(testPoints);
            if (!testPoints->configureTestPoint(value)) {
              *CmpCommon::diags() << DgSqlCode(-2055) << DgString0(value) << DgString1(lookupAttrName(attrEnum));
            }
          }
        } break;
        ...
        case MODE_SPECIAL_4: {
          ...
          validateAndInsert("NCM_IND_JOIN_COST_ADJ_FACTOR", costVal1, FALSE, errOrWarn);
          validateAndInsert("NCM_IND_SCAN_COST_ADJ_FACTOR", costVal2, FALSE, errOrWarn);
        ...
        case SESSION_IN_USE: {
          CmpCommon::context()->sqlSession()->setSessionInUse((getToken(attrEnum) == DF_ON));
        } break;
        ...
        case VOLATILE_SCHEMA_IN_USE: {
          CmpCommon::context()->sqlSession()->setVolatileSchemaInUse((getToken(attrEnum) == DF_ON));
        } break;
        ...
        case QUERY_TEXT_CACHE: {
          ...
          if (pSchema != "") value = "OFF";
        } break;

        case PUBLIC_SCHEMA_NAME: {
          // when PUBLIC_SCHEMA is used, turn off Query Text Cache
          if ((value != "") && !(getToken(QUERY_TEXT_CACHE) == DF_OFF)) insert(QUERY_TEXT_CACHE, "OFF");
        ...
        case LDAP_USERNAME: {
          ...
          if (!value.isNull() && (getToken(DEFAULT_SCHEMA_NAMETYPE) == DF_USER) &&
        ...
        case TRAF_MAX_CHARACTER_COL_LENGTH: {
          ...
          if ((NOT override) && (NOT(d >= 0 && d <= MAX_CHAR_COL_LENGTH_IN_BYTES))) {
            *CmpCommon::diags() << DgSqlCode(-2055) << DgString0(value) << DgString1(lookupAttrName(attrEnum));
          }
        } break;
        ...
        case ROBUST_QUERY_OPTIMIZATION: {
          // ROBUST_QUERY_OPTIMIZATION set at defaults table should set these six cqds
          if (currentState_ == READ_FROM_SQL_TABLE) {
            setMultiCQDSValue(ROBUST_QUERY_OPTIMIZATION, value, errOrWarn);
          }
        } break;
        default:
          break;
    ...
    if (reset && overwrite) {
      ...
      *CmpCommon::diags() << DgSqlCode(ERRWARN(2200)) << DgString0(lookupAttrName(attrEnum, errOrWarn));
    }
  }  // valid attrName
...
void NADefaults::updateNumESPsPerCoreForTenant() {
  float espsPerCore = computeNumESPsPerCoreForTenant();

  char buf[100];
  snprintf(buf, sizeof(buf), "%0.2f", espsPerCore);

  NAString value(buf);

  validateAndInsert("MAX_ESPS_PER_CPU_PER_OP", value, FALSE);
}
...
float NADefaults::computeNumESPsPerCore() {
  ...
  //            dynamic_cast<NAClusterInfoLinux*>(CURRCONTEXT_CLUSTERINFO);
  //   assert(gpLinux);
  //
  //     // number of POS TSE
...
enum DefaultConstants NADefaults::holdOrRestore(const char *attrName, int holdOrRestoreCQD) {
  ...
  if (holdOrRestoreCQD == 1)  // hold cqd
  {
    ...
    if (!heldDefaults_[attrEnum]) heldDefaults_[attrEnum] = new NADHEAP HeldDefaults();
    if (heldDefaults_[attrEnum]->pushDefault(value)) {
      *CmpCommon::diags() << DgSqlCode(-2050) << DgString0(attrName);

      return __INVALID_DEFAULT_ATTRIBUTE;
    }
  } else  // value 2 to restore
  {
    // restore cqd from heldDefaults_ array, if it was held.
    if (!heldDefaults_[attrEnum]) return attrEnum;

    value = heldDefaults_[attrEnum]->popDefault();
    if (!value) return attrEnum;
...
static void setCatSchErr(NAString &value, int sqlCode, int errOrWarn, NABoolean catErr = FALSE) {
  ...
  *CmpCommon::diags() << DgSqlCode(ERRWARN(sqlCode)) << DgCatalogName(value) << DgSchemaName(value) << DgString0(value)
                      << DgString1(value);
  ...
  if (value.first('"') == NA_NPOS) {  // delimited names too complicated !
    ...
    if (IsSqlReservedWord(namepart)) {
      *CmpCommon::diags() << DgSqlCode(ERRWARN(3128)) << DgString0(namepart) << DgString1(namepart);
      return;
    }
  }
  ...
  if (c && c != '\"') {
    ...
    stmt += "\"";
    stmt += ";";
    parser.parseDML(stmt, stmt.length(), OBJECTNAMECHARSET);
  }
...
NABoolean NADefaults::setSchema(NAString &value, int errOrWarn, NABoolean overwrite, NABoolean alreadyCanonical) {
  ...
  if (nam.getSchemaNamePart().isEmpty()) {  // 0 name parts, if *any* error
    ...
    if (overwrite) insert(SCHEMA, nam.getSchemaNamePartAsAnsiString());

    // If 2 parts, overwrite any prior catalog default
    if (!nam.getCatalogNamePart().isEmpty()) {
      if (overwrite) {
        insert(CATALOG, nam.getCatalogNamePartAsAnsiString());
...
const char *NADefaults::keywords_[DF_lastToken] = {"ACCUMULATED",
                                                   "ADVANCED",
                                                   "AGGRESSIVE",
                                                   "ALL",
                                                   "ANSI",
...
DefaultToken NADefaults::token(int attrEnum, NAString &value, NABoolean valueAlreadyGotten, int errOrWarn) const {
  ...
  if (value.isNull())
    ...
    if ((attrEnum == TERMINAL_CHARSET) || (attrEnum == USE_HIVE_SOURCE) || (attrEnum == HIVE_FILE_CHARSET) ||
        (attrEnum == HBASE_DATA_BLOCK_ENCODING_OPTION) || (attrEnum == HBASE_MEMSTORE_FLUSH_SIZE_OPTION) ||
        (attrEnum == HBASE_COMPRESSION_OPTION) || (attrEnum == HBASE_MEMSTORE_FLUSH_SIZE_OPTION) ||
        (attrEnum == HBASE_ENCRYPTION_OPTION))
      return DF_USER;

    if (attrEnum == NATIONAL_CHARSET || attrEnum == DEFAULT_CHARSET || attrEnum == HIVE_DEFAULT_CHARSET ||
      ...
      if (!CharInfo::isCharSetSupported(cs)) {
        ...
        switch (attrEnum) {
          case NATIONAL_CHARSET:
            ...
            if ((cs != CharInfo::UNICODE) && (cs != CharInfo::ISO88591)) err_found = 1;
            break;
          case DEFAULT_CHARSET:
          ...
          case DYNAMIC_PARAM_DEFAULT_CHARSET:
            if ((cs != CharInfo::UTF8) && (cs != CharInfo::ISO88591)) err_found = 1;
            break;
          case ISO_MAPPING:
            if (cs != CharInfo::ISO88591) err_found = 1;
            break;
          default:
            break;
        }
    ...
    // PCODE_OPT_LEVEL
    if ((attrEnum == PCODE_OPT_LEVEL) && value.length() == 1) switch (*value.data()) {
        case '0':
          return DF_MINIMUM;
        case '1':
          return DF_MEDIUM;
    ...
    // HBASE_FILTER_PREDS
    if ((attrEnum == HBASE_FILTER_PREDS) && value.length() == 1) switch (*value.data()) {
        case '0':
          return DF_OFF;
        case '1':
          return DF_MINIMUM;
    ...
    if (attrEnum == MVQR_REWRITE_CANDIDATES || attrEnum == MVQR_WORKLOAD_ANALYSIS_MV_NAME ||
        attrEnum == HIST_SCRATCH_VOL)
      return DF_SYSTEM;

    const char *k = value.data();
    char *match = (char *)bsearch(&k, keywords_, DF_lastToken, sizeof(char *), stringCompare);
    if (match)
      ...
      if (*c == '\0')  // terminating nul '\0'
        ...
        if (value == "1" || value == "+1" || value == "-1") tok = DF_ON;
      }
...
#define CONVERT_SYNONYM(from, to)    \
  ...
  if (tok != DF_noSuchToken) switch (attrEnum) {
      case DEFAULT_SCHEMA_ACCESS_ONLY:
        if (tok == DF_ON || tok == DF_OFF) isValid = TRUE;
        break;

      case DEFAULT_SCHEMA_NAMETYPE:
        if (tok == DF_SYSTEM || tok == DF_USER) isValid = TRUE;
        break;

      case DETAILED_STATISTICS:
        if (tok == DF_ALL || tok == DF_ACCUMULATED || tok == DF_OPERATOR || tok == DF_PERTABLE || tok == DF_OFF)
          isValid = TRUE;
        break;

      case HBASE_CHECK_AND_UPDEL_OPT:
        if (tok == DF_SYSTEM || tok == DF_ON || tok == DF_OFF) isValid = TRUE;
        break;

      case HIDE_INDEXES:
        if (tok == DF_NONE || tok == DF_ALL || tok == DF_VERTICAL || tok == DF_INDEXES || tok == DF_KEYINDEXES)
          isValid = TRUE;
        break;

      case HIVE_USE_EXT_TABLE_ATTRS:
        if (tok == DF_ALL || tok == DF_OFF || tok == DF_ON) isValid = TRUE;
        break;

      case HIVE_COMPOSITE_DATATYPE_SUPPORT:
        if (tok == DF_ALL || tok == DF_OFF || tok == DF_ON) isValid = TRUE;
        break;

      case INDEX_ELIMINATION_LEVEL:
        if (tok == DF_MINIMUM || tok == DF_MEDIUM || tok == DF_MAXIMUM || tok == DF_AGGRESSIVE) isValid = TRUE;
        break;
      case IF_LOCKED:
        if (tok == DF_RETURN || tok == DF_WAIT) isValid = TRUE;
        break;

      case INSERT_VSBB:
        if (tok == DF_OFF || tok == DF_LOADNODUP || tok == DF_SYSTEM || tok == DF_USER) isValid = TRUE;
        break;
      case OVERFLOW_MODE:
        if (tok == DF_DISK || tok == DF_SSD || tok == DF_MMAP) isValid = TRUE;
        break;
      case SORT_ALGO:
        if (tok == DF_HEAP || tok == DF_IQS || tok == DF_REPSEL || tok == DF_QS) isValid = TRUE;
        break;
      ...
      case QUERY_TEXT_CACHE:
        if (tok == DF_ON || tok == DF_OFF || tok == DF_SYSTEM || tok == DF_SKIP) isValid = TRUE;
        break;
      ...
      case IUD_NONAUDITED_INDEX_MAINT:
        if (tok == DF_OFF || tok == DF_SYSTEM || tok == DF_WARN || tok == DF_ON) isValid = TRUE;
        break;
      ...
      case NAMETYPE:
        if (tok == DF_ANSI || tok == DF_SHORTANSI) isValid = TRUE;
        break;

      case OPTIMIZATION_GOAL:
        if (tok == DF_FIRSTROW || tok == DF_LASTROW || tok == DF_RESOURCES) isValid = TRUE;
        break;

      case USER_EXPERIENCE_LEVEL:
        if (tok == DF_ADVANCED || tok == DF_BEGINNER) isValid = TRUE;
        break;
      ...
      case ATTEMPT_ESP_PARALLELISM:
        if (tok == DF_SYSTEM || tok == DF_ON || tok == DF_OFF || tok == DF_MAXIMUM) isValid = TRUE;
        break;

      case OPTIMIZATION_LEVEL:
        if (tok == DF_MINIMUM || tok == DF_MEDIUM_LOW || tok == DF_MEDIUM || tok == DF_MAXIMUM) isValid = TRUE;
        break;

      case HBASE_FILTER_PREDS:
        if (tok == DF_OFF || tok == DF_ON) {
          if (tok == DF_ON) tok = DF_MINIMUM;  // to keep backward compatibility
          isValid = TRUE;
        }
        break;

      case ROBUST_QUERY_OPTIMIZATION:
        if (tok == DF_MINIMUM || tok == DF_SYSTEM || tok == DF_MAXIMUM || tok == DF_HIGH) isValid = TRUE;
        break;

      case REFERENCE_CODE:
      case TARGET_CODE:
        if (tok == DF_RELEASE || tok == DF_DEBUG) isValid = TRUE;
        break;
      ...
      case AUTO_QUERY_RETRY:
        if (tok == DF_ON || tok == DF_OFF || tok == DF_SYSTEM) isValid = TRUE;
        break;

      case HIVE_USE_HASH2_AS_PARTFUNCTION:
        if (tok == DF_ON || tok == DF_SYSTEM) isValid = TRUE;
        break;
      ...
      case EXE_PARALLEL_DDL:
        if (tok == DF_OFF || tok == DF_ON || tok == DF_EXTERNAL || tok == DF_INTERNAL) isValid = TRUE;
        break;

      case UNAVAILABLE_PARTITION:
        if (tok == DF_SKIP || tok == DF_STOP) isValid = TRUE;
        break;
      ...
      case HJ_TYPE:
        if (tok == DF_ORDERED || tok == DF_HYBRID || tok == DF_SYSTEM) isValid = TRUE;
        break;

      case REF_CONSTRAINT_NO_ACTION_LIKE_RESTRICT:
        if (tok == DF_OFF || tok == DF_ON || tok == DF_SYSTEM) isValid = TRUE;
        break;

      case POS:
        if (tok == DF_LOCAL_NODE || tok == DF_OFF || tok == DF_MULTI_NODE || tok == DF_DISK_POOL) isValid = TRUE;
        break;

      case USTAT_INTERNAL_SORT:
        if (tok == DF_ON || tok == DF_OFF || tok == DF_HYBRID) isValid = TRUE;
        break;
      ...
      case SUBQUERY_UNNESTING:
        if (tok == DF_OFF || tok == DF_ON || tok == DF_DEBUG) isValid = TRUE;
        break;

      case SUBQUERY_UNNESTING_P2:
        if (tok == DF_OFF || tok == DF_ON || tok == DF_INTERNAL) isValid = TRUE;
        break;
      ...
      case MDAM_SCAN_METHOD:
        if (tok == DF_ON || tok == DF_OFF || tok == DF_MAXIMUM) isValid = TRUE;
        break;

      case SHOWDDL_DISPLAY_FORMAT:
        if (tok == DF_INTERNAL || tok == DF_EXTERNAL || tok == DF_LOG) isValid = TRUE;
        break;
      ...
      case EXPLAIN_DISPLAY_FORMAT:
        if (tok == DF_INTERNAL || tok == DF_EXTERNAL || tok == DF_EXTERNAL_DETAILED) isValid = TRUE;
        break;

      case UPDATE_CLUSTERING_OR_UNIQUE_INDEX_KEY:
        if (tok == DF_ON || tok == DF_OFF || tok == DF_AGGRESSIVE) isValid = TRUE;
        break;
      ...
      case MVQR_LOG_QUERY_DESCRIPTORS:
        if (tok == DF_OFF || tok == DF_DUMP || tok == DF_DUMP_MV || tok == DF_LOG) isValid = TRUE;
        break;

      case MVQR_PRIVATE_QMS_INIT:
        if (tok == DF_SMD || tok == DF_XML || tok == DF_NONE) isValid = TRUE;
        break;

      case MVQR_PUBLISH_TO:
        if (tok == DF_PUBLIC || tok == DF_PRIVATE || tok == DF_BOTH || tok == DF_NONE) isValid = TRUE;
        break;
      ...
      case ELIMINATE_REDUNDANT_JOINS:
        if (tok == DF_OFF || tok == DF_ON || tok == DF_DEBUG || tok == DF_MINIMUM) isValid = TRUE;
        break;
      ...
      case CAT_DISTRIBUTE_METADATA:
        if (tok == DF_OFF || tok == DF_LOCAL_NODE || tok == DF_ON) isValid = TRUE;
        break;

      case MV_DUMP_DEBUG_INFO:
        if (tok == DF_OFF || tok == DF_ON) isValid = TRUE;
        break;

      case RANGESPEC_TRANSFORMATION:
        if (tok == DF_OFF || tok == DF_ON || tok == DF_MINIMUM) isValid = TRUE;
        break;

      case ASYMMETRIC_JOIN_TRANSFORMATION:
        if (tok == DF_MINIMUM || tok == DF_MAXIMUM) isValid = TRUE;
        break;
      ...
      case COMPRESSION_TYPE:
        if (tok == DF_NONE || tok == DF_HARDWARE || tok == DF_SOFTWARE) isValid = TRUE;
        break;
      ...
      case USTAT_INCREMENTAL_UPDATE_STATISTICS:
        if (tok == DF_OFF || tok == DF_SAMPLE || tok == DF_ON) isValid = TRUE;
        break;

      case REPLICATE_COMPRESSION_TYPE:
        if (tok == DF_NONE || tok == DF_HARDWARE || tok == DF_SOFTWARE || tok == DF_SOURCE || tok == DF_SYSTEM)
          isValid = TRUE;
        break;

      case REUSE_OPENS:
        if (tok == DF_ON || tok == DF_OFF || tok == DF_OPENS_FOR_WRITE) isValid = TRUE;
        break;
      ...
      case TRAF_SIMILARITY_CHECK:
        if (tok == DF_ROOT || tok == DF_LEAF || tok == DF_ON || tok == DF_OFF) isValid = TRUE;
        break;

      case TRAF_TABLE_SNAPSHOT_SCAN:
        if (tok == DF_NONE || tok == DF_SUFFIX || tok == DF_LATEST || tok == DF_FORCE) isValid = TRUE;
        break;

      case TRAF_TRANS_TYPE:
        if (tok == DF_MVCC || tok == DF_SSCC) isValid = TRUE;
        break;

      case HBASE_RANGE_PARTITIONING_PARTIAL_COLS:
        if (tok == DF_OFF || tok == DF_MINIMUM || tok == DF_MEDIUM || tok == DF_MAXIMUM || tok == DF_ON) isValid = TRUE;
        break;

      case TRAF_UPSERT_MODE:
        if (tok == DF_MERGE || tok == DF_REPLACE || tok == DF_OPTIMAL) isValid = TRUE;
        break;

      case TRAF_USE_STORED_STATS:
        if (tok == DF_ON || tok == DF_ALL || tok == DF_OFF || tok == DF_MINIMUM) isValid = TRUE;
        break;

      case TRAF_SAVEPOINTS:
        if (tok == DF_OFF || tok == DF_ON) isValid = TRUE;
        break;

      case DOP_ADJUST_DEBUG:
        if (tok == DF_FILE || tok == DF_OFF || tok == DF_STDOUT) isValid = TRUE;
        break;

      case TRAF_ENABLE_METADATA_LOAD_IN_CACHE:
      case HBASE_ASYNC_OPERATIONS:
        if (tok == DF_ALL || tok == DF_OFF || tok == DF_ON) isValid = TRUE;
        break;

      case TRAF_UPDATE_TO_UPSERT:
        if (tok == DF_ALL || tok == DF_OFF || tok == DF_ON || tok == DF_FORCE) isValid = TRUE;
        break;

      case GENERATE_USER_QUERYCACHE:
        if (tok == DF_OFF || tok == DF_ON || tok == DF_PRELOAD) isValid = TRUE;
        break;

      case GENERATE_USER_QUERYCACHE_DEBUG:
        // if (tok == DF_OFF || tok == DF_ON || tok == DF_REPLACE || tok == DF_READONLY)
        if (tok == DF_OFF || tok == DF_ON || tok == DF_ALL) isValid = TRUE;
        break;
  ...
  if (!isValid) {
    tok = DF_noSuchToken;
    if (errOrWarn)
      *CmpCommon::diags() << DgSqlCode(ERRWARN(2055)) << DgString0(value) << DgString1(lookupAttrName(attrEnum));
  }
...
NABoolean NADefaults::getIsolationLevel(TransMode::IsolationLevel &arg, DefaultToken tok) const {
  NABoolean specifiedOK = TRUE;

  if (tok == DF_noSuchToken) tok = getToken(ISOLATION_LEVEL);

  switch (tok) {
    case DF_READ_COMMITTED:
      arg = TransMode::READ_COMMITTED_;
      break;
    case DF_READ_UNCOMMITTED:
      arg = TransMode::READ_UNCOMMITTED_;
      break;
    case DF_REPEATABLE_READ:
      arg = TransMode::REPEATABLE_READ_;
      break;

    case DF_SERIALIZABLE:
    case DF_SYSTEM:
      arg = TransMode::SERIALIZABLE_;
      break;
    case DF_NONE:
      arg = TransMode::IL_NOT_SPECIFIED_;
      break;
    ...
    default:
      ...
      *CmpCommon::diags() << DgSqlCode(-2055) << DgString0(value) << DgString1("ISOLATION_LEVEL");
  }
...
NABoolean NADefaults::isSameCQD(int numEntriesInBuffer, char *buffer, int bufLen) {
  ...
  if (numCurrentDefaultAttrs != numEntriesInBuffer) return FALSE;
...
void NADefaults::setSchemaAsLdapUser(const NAString val) {
  ...
  if (ldapUsername.isNull()) ldapUsername = getValue(LDAP_USERNAME);
  if (ldapUsername.isNull()) return;
  ...
  if (!cSchName.getSchemaNamePart().isEmpty() && cSchName.getCatalogNamePart().isEmpty())  // should have no catalog
  {
    insert(SCHEMA, schName);
  } else {
    *CmpCommon::diags() << DgSqlCode(-2055) << DgString0(schName) << DgString1("SCHEMA");
  }
}
...
bool NADefaults::moduleCheck(const int attrEnum) const {
  ...
  switch (attrEnum) {
    case TRAF_OBJECT_LOCK:
      if (NOT isModuleOpen(LM_DDL_ONLINE)) ret = -1;
      break;
    default:
      break;
  }
