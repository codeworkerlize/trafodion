...
static bool checkSpecifiedPrivs(ElemDDLPrivActArray &privActsArray, const char *externalObjectName,
                                ComObjectType objectType, NATable *naTable, std::vector<PrivType> &objectPrivs,
                                std::vector<ColPrivSpec> &colPrivs);

static bool hasValue(const std::vector<ColPrivSpec> &container, PrivType value);

static bool hasValue(const std::vector<PrivType> &container, PrivType value);

static bool isMDGrantRevokeOK(const std::vector<PrivType> &objectPrivs, const std::vector<ColPrivSpec> &colPrivs,
                              bool isGrant);

static bool isValidPrivTypeForObject(ComObjectType objectType, PrivType privType);

void CmpSeabaseDDL::convertVirtTableColumnInfoToDescStruct(const ComTdbVirtTableColumnInfo *colInfo,
                                                           const ComObjectName *objectName, TrafDesc *column_desc) {
  ...
  column_desc->columnsDesc()->pictureText = (char *)STMTHEAP->allocateMemory(340);
  NAType::convertTypeToText(
      column_desc->columnsDesc()->pictureText,  // OUT
      column_desc->columnsDesc()->datatype, column_desc->columnsDesc()->length, column_desc->columnsDesc()->precision,
      column_desc->columnsDesc()->scale, column_desc->columnsDesc()->datetimeStart(),
      column_desc->columnsDesc()->datetimeEnd(), column_desc->columnsDesc()->datetimefractprec,
      column_desc->columnsDesc()->intervalleadingprec, column_desc->columnsDesc()->isUpshifted(),
      column_desc->columnsDesc()->isCaseInsensitive(), (CharInfo::CharSet)column_desc->columnsDesc()->character_set,
  ...
  column_desc->columnsDesc()->encoding_charset =
      (CharInfo::CharSet)column_desc->columnsDesc()
          ->character_set;  // not present in colInfo so we go with the column's charset here.
  column_desc->columnsDesc()->collation_sequence =
      (CharInfo::Collation)1;  // not present in colInfo, so we go with default collation here (used in buildEncodeTree
...
TrafDesc *CmpSeabaseDDL::convertVirtTableColumnInfoArrayToDescStructs(const ComObjectName *objectName,
...
TrafDesc *CmpSeabaseDDL::convertVirtTableKeyInfoArrayToDescStructs(const ComTdbVirtTableKeyInfo *keyInfoArray,
                                                                   const ComTdbVirtTableColumnInfo *colInfoArray,
                                                                   int numKeys) {
...
void CmpSeabaseDDL::createSeabaseTableLike(ExeCliInterface *cliInterface, StmtDDLCreateTable *createTableNode,
  ...
  NABoolean srcSchNameSpecified = (NOT createTableNode->getOrigLikeSourceTableName().getSchemaName().isNull());
  ...
  retcode = lookForTableInMD(cliInterface, srcCatNamePart, srcSchNamePart, srcObjNamePart, srcSchNameSpecified, FALSE,
  ...
  CorrName cn(srcObjNamePart, STMTHEAP, srcSchNamePart, srcCatNamePart);

  ElemDDLColRefArray &keyArray = (createTableNode->getIsConstraintPKSpecified()
  ...
  NAString keyClause;
  if ((keyArray.entries() > 0) && ((createTableNode->getIsConstraintPKSpecified()) ||
                                   (createTableNode->getStoreOption() == COM_KEY_COLUMN_LIST_STORE_OPTION))) {
    ...
    for (CollIndex i = 0; i < keyArray.entries(); i++) {
      ...
      if (i < (keyArray.entries() - 1)) keyClause += ", ";
    }
  ...
  if (NOT keyClause.isNull()) {
    *CmpCommon::diags() << DgSqlCode(-3111) << DgString0("PRIMARY KEY/STORE BY");
    return;
  }

  if (createTableNode->isPartitionSpecified() || createTableNode->isPartitionBySpecified()) {
    *CmpCommon::diags() << DgSqlCode(-3111) << DgString0("PARTITION BY");
    return;
  }

  if (createTableNode->isDivisionClauseSpecified()) {
    *CmpCommon::diags() << DgSqlCode(-3111) << DgString0("DIVISION BY");
    return;
  }

  if (createTableNode->isHbaseOptionsSpecified()) {
    *CmpCommon::diags() << DgSqlCode(-3111) << DgString0("HBASE table options");
    return;
  }
  ...
  if (NOT likeOptions.getLikeOptHiveOptions().isNull()) {
    *CmpCommon::diags() << DgSqlCode(-3242) << DgString0("Hive options cannot be specified for this table.");
    return;
  }
  ...
  retcode = CmpDescribeSeabaseTable(
      ...
      likeOptions.getIsWithHorizontalPartitions(), likeOptions.getIsWithoutSalt(), likeOptions.getIsWithoutDivision(),
      likeOptions.getIsWithoutRowFormat(), likeOptions.getIsWithoutLobColumns(), likeOptions.getIsWithoutNamespace(),
      likeOptions.getIsLikeOptWithoutRegionReplication(), likeOptions.getIsLikeOptWithoutIncrBackup(),
      likeOptions.getIsLikeOptColumnLengthLimit(), TRUE);
  if (retcode) return;

  NAString query = createTableNode->isPartition() ? "create partition table " : "create table ";
  ...
  while (NOT done) {
    ...
    NAString frag(&buf[curPos + sizeof(short)], len - ((buf[curPos + len - 1] == '\n') ? 1 : 0));

    query += frag;
    curPos += ((((len + sizeof(short)) - 1) / 8) + 1) * 8;

    if (curPos >= buflen) done = TRUE;
  }
  ...
  int retCode = sendAllControls(FALSE, FALSE, TRUE);
  ...
  ActiveSchemaDB()->getDefaults().validateAndInsert("traf_lob_parallel_ddl", value, FALSE);
  ...
  if ((NOT likeOptions.getIsWithoutIndexes()) && (naTable->getIndexList().entries() > 1) && naTable->isSeabaseTable()) {
    ...
    CorrName tgtcn(tgtObjNamePart, STMTHEAP, tgtSchNamePart, tgtCatNamePart);
    retcode = CmpDescribeSeabaseTable(
        cn, 3, indexBuf, indexBuflen, STMTHEAP, createTableNode->isPartition(), NULL, NULL,
        likeOptions.getIsWithHorizontalPartitions(), likeOptions.getIsWithoutSalt(), likeOptions.getIsWithoutDivision(),
        likeOptions.getIsWithoutRowFormat(), likeOptions.getIsWithoutLobColumns(), likeOptions.getIsWithoutNamespace(),
        likeOptions.getIsLikeOptWithoutRegionReplication(), likeOptions.getIsLikeOptWithoutIncrBackup(),
        likeOptions.getIsLikeOptColumnLengthLimit(), TRUE, FALSE, NULL, 0, NULL, NULL, NULL, FALSE, &tgtcn,
        1 /*get index*/);
    if (retcode) return;
    while (NOT finish1) {
      short len = *(short *)&indexBuf[curPos];
      NAString frag(&indexBuf[curPos + sizeof(short)], len - ((indexBuf[curPos + len - 1] == '\n') ? 1 : 0));
      indexQuery += frag;
      ...
      curPos += ((((len + sizeof(short)) - 1) / 8) + 1) * 8;
      if (curPos >= indexBuflen) finish1 = TRUE;
    }
  }
  ...
  if ((NOT likeOptions.getIsWithoutConstraints()) && naTable->isSeabaseTable()) {
    ...
    CorrName tgtcn(tgtObjNamePart, STMTHEAP, tgtSchNamePart, tgtCatNamePart);
    retcode = CmpDescribeSeabaseTable(
        cn, 3, constraintBuf, constraintBuflen, STMTHEAP, createTableNode->isPartition(), NULL, NULL,
        likeOptions.getIsWithHorizontalPartitions(), likeOptions.getIsWithoutSalt(), likeOptions.getIsWithoutDivision(),
        likeOptions.getIsWithoutRowFormat(), likeOptions.getIsWithoutLobColumns(), likeOptions.getIsWithoutNamespace(),
        likeOptions.getIsLikeOptWithoutRegionReplication(), likeOptions.getIsLikeOptWithoutIncrBackup(),
        likeOptions.getIsLikeOptColumnLengthLimit(), TRUE, FALSE, NULL, 0, NULL, NULL, NULL, FALSE, &tgtcn,
    ...
    while (NOT finish2) {
      ...
      NAString frag(&constraintBuf[curPos + sizeof(short)], len - ((constraintBuf[curPos + len - 1] == '\n') ? 1 : 0));
      ...
      curPos += ((((len + sizeof(short)) - 1) / 8) + 1) * 8;
      if (curPos >= constraintBuflen) finish2 = TRUE;
    }
  }

  {
    ...
    if (srcTable && srcTable->readOnlyEnabled()) {
      ...
      NAString tgtSchNamePart = tgtTableName.getSchemaNamePartAsAnsiString(TRUE);
      NAString tgtObjNamePart = tgtTableName.getObjectNamePartAsAnsiString(TRUE);
      insertIntoSharedCacheList(tgtCatNamePart, tgtSchNamePart, tgtObjNamePart, COM_BASE_TABLE_OBJECT,
                                SharedCacheDDLInfo::DDL_UPDATE, SharedCacheDDLInfo::SHARED_DATA_CACHE);
    }
  }
...
// ----------------------------------------------------------------------------
short CmpSeabaseDDL::createSeabaseTableExternal(ExeCliInterface &cliInterface, StmtDDLCreateTable *createTableNode,
  ...
  int cliRC = cliInterface.executeImmediate((char *)createSchemaStmt.data());
  if (cliRC < 0) {
    cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
    return -1;
  }
  ...
  retcode = verifyDDLCreateOperationAuthorized(&cliInterface, SQLOperation::CREATE_TABLE, catalogNamePart,
  ...
  if (createTableNode->mapToHbaseTable()) return 0;
  ...
  if (isHive) {
    ...
    if (createTableNode->isImplicitExternal()) tableInfo->objectFlags |= SEABASE_OBJECT_IS_IMPLICIT_EXTERNAL;
  }
  ...
  if (naTable == NULL || bindWA.errStatus()) {
    *CmpCommon::diags() << DgSqlCode(-4082) << DgTableName(cnSrc.getExposedNameAsAnsiString());
    return -1;
  }

  if ((naTable->isHiveTable()) && (naTable->getViewText()) && (!Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL))) {
    *CmpCommon::diags() << DgSqlCode(-3242) << DgString0("Cannot create external table on a native Hive view.");
  ...
  if (colArray.entries() > 0) {
    if (CmpCommon::getDefault(HIVE_USE_EXT_TABLE_ATTRS) == DF_OFF) {
      *CmpCommon::diags() << DgSqlCode(-3242) << DgString0("Cannot specify column attributes for external tables.");
      return -1;
    }

    extTableAttrsSpecified = TRUE;
    CmpSeabaseDDL::setMDflags(tableInfo->tablesFlags, MD_TABLES_HIVE_EXT_COL_ATTRS);
  }

  if (keyArray.entries() > 0) {
    if (CmpCommon::getDefault(HIVE_USE_EXT_TABLE_ATTRS) != DF_ALL) {
      *CmpCommon::diags() << DgSqlCode(-3242) << DgString0("Cannot specify key attribute for external tables.");
      return -1;
    }

    extTableAttrsSpecified = TRUE;
    CmpSeabaseDDL::setMDflags(tableInfo->tablesFlags, MD_TABLES_HIVE_EXT_KEY_ATTRS);
  }
  ...
  NABoolean includeVirtHiveCols = (CmpCommon::getDefault(HIVE_EXT_TABLE_INCLUDE_VIRT_COLS) == DF_ON);

  // eliminate Hive virtual columns, unless requested via CQD
  for (CollIndex c = 0; c < origColArray.entries(); c++)
    if (!origColArray[c]->isHiveVirtualColumn() || includeVirtHiveCols) naColArray.insert(origColArray[c]);
  ...
  if (colArray.entries() > 0) {
    for (CollIndex colIndex = 0; colIndex < colArray.entries(); colIndex++) {
      ...
      if (naColArray.getColumnPosition((NAString &)edcd->getColumnName()) < 0) {
        ...
        *CmpCommon::diags() << DgSqlCode(-1009) << DgColumnName(ToAnsiIdentifier(edcd->getColumnName()));
  ...
  for (CollIndex index = 0; index < numCols; index++) {
    ...
    int colIndex = -1;
    if ((colArray.entries() > 0) && ((colIndex = colArray.getColumnIndex(naCol->getColName())) >= 0)) {
      ElemDDLColDef *edcd = colArray[colIndex];
      const NAType *edcdType = edcd->getColumnDataType();

      if ((isHive) && (CmpCommon::getDefault(HIVE_COMPOSITE_DATATYPE_SUPPORT) != DF_ALL) && (edcdType->isComposite())) {
    ...
    retcode = getTypeInfo(type, alignedFormat, serializedOption, datatype, length, precision, scale, dtStart, dtEnd,
                          upshifted, nullable, charset, collationSequence, hbaseColFlags);

    if (retcode) return -1;

    if (length > CmpCommon::getDefaultNumeric(TRAF_MAX_CHARACTER_COL_LENGTH)) {
      *CmpCommon::diags() << DgSqlCode(-4247) << DgInt0(length)
    ...
    if (type->isComposite()) {
      ...
      char *comp_str = new ((STMTHEAP)) char[fieldsInfoStr.length() + 1];
      str_cpy_all(comp_str, (char *)fieldsInfoStr.data(), fieldsInfoStr.length());
      ...
      if (format == "EXPLODED") compColsInExplodedFormat = TRUE;
    }  // composite
  }    // for

  if (compColsInExplodedFormat || isHive) tableInfo->tablesFlags |= MD_TABLES_COMP_COLS_IN_EXPLODED_FORMAT;
  ...
  if (numKeys > 0) {
    if (isHive) {
      *CmpCommon::diags() << DgSqlCode(-4222) << DgString0("\"PRIMARY KEY on external hive table\"");

      return -1;
    }

    keyInfoArray = new (STMTHEAP) ComTdbVirtTableKeyInfo[numKeys];
    if (buildKeyInfoArray(NULL, (NAColumnArray *)&naColArray, &keyArray, colInfoArray, keyInfoArray, TRUE)) {
      *CmpCommon::diags() << DgSqlCode(-CAT_UNABLE_TO_CREATE_OBJECT) << DgTableName(extTgtTableName);

      return -1;
    }
  }

  NABoolean registerHiveObject = (cnSrc.isHive() && CmpCommon::getDefault(HIVE_NO_REGISTER_OBJECTS) == DF_OFF);

  // if source table is a hive table and not already registered, register
  // it in traf metadata
  if (registerHiveObject) {
    char buf[2000];
    str_sprintf(buf, "register internal hive table if not exists %s.\"%s\".\"%s\"", srcCatNamePart.data(),
                srcSchNamePart.data(), srcObjNamePart.data());
  ...
  if (updateSeabaseMDTable(&cliInterface, catalogNamePart, schemaNamePart, objectNamePart, COM_BASE_TABLE_OBJECT,
                           COM_NO_LIT, tableInfo, numCols, colInfoArray, numKeys, keyInfoArray, 0, NULL,
                           objUID /*returns generated UID*/, (NOT registerHiveObject))) {
    *CmpCommon::diags() << DgSqlCode(-CAT_UNABLE_TO_CREATE_OBJECT) << DgTableName(extTgtTableName);
    return -1;
  }

  cliRC = updateObjectValidDef(&cliInterface, catalogNamePart, schemaNamePart, objectNamePart,
  ...
  ActiveSchemaDB()->getNATableDB()->removeNATable(cnTgt, ComQiScope::REMOVE_MINE_ONLY, COM_BASE_TABLE_OBJECT,
                                                  createTableNode->ddlXns(), FALSE);

  return 0;
}

short CmpSeabaseDDL::updatePKeyInfo(StmtDDLAddConstraintPK *addPKNode, const char *catName, const char *schName,
                                    const char *objName, const int ownerID, const int schemaOwnerID, int numKeys,
                                    long *outPkeyUID, long *outTableUID, const ComTdbVirtTableKeyInfo *keyInfoArray,
  ...
  if (addPKNode && addPKNode->getConstraint()->castToElemDDLConstraint() &&
    ...
    namedPKey = TRUE;
  if (genUniqueName((namedPKey ? addPKNode : NULL), NULL, pkeyStr, catName, schName, objName)) {
    return -1;
  }
  ...
  ComUID comUID;
  comUID.make_UID();
  long pkeyUID = comUID.get_value();
  if (outPkeyUID) *outPkeyUID = pkeyUID;
  ...
  str_sprintf(buf, "insert into %s.\"%s\".%s values ('%s', '%s', '%s', '%s', %ld, %ld, %ld, '%s', '%s', %d, %d, 0)",
              ...
              quotedObjName.data(), COM_PRIMARY_KEY_CONSTRAINT_OBJECT_LIT, pkeyUID, createTime, createTime, " ",
              COM_NO_LIT, ownerID, schemaOwnerID);
  cliRC = cliInterface->executeImmediate(buf);

  if (cliRC < 0) {
    cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());

    *CmpCommon::diags() << DgSqlCode(-1423) << DgString0(SEABASE_OBJECTS);

    return -1;
  }

  long tableUID = getObjectUID(cliInterface, catName, schName, objName, COM_BASE_TABLE_OBJECT_LIT);

  if (outTableUID) *outTableUID = tableUID;

  long validatedTime = NA_JulianTimestamp();
  ...
  if ((addPKNode->getAlterTableAction()->castToElemDDLConstraintPK()->ser() ==
      ...
      (ComIsHbaseMappedSchemaName(schName) && (addPKNode->getAlterTableAction()->castToElemDDLConstraintPK()->ser() ==
                                               ComPkeySerialization::COM_SER_NOT_SPECIFIED)))
    notSerializedPK = TRUE;

  if (notSerializedPK) {
    CmpSeabaseDDL::setMDflags(flags, CmpSeabaseDDL::MD_TABLE_CONSTRAINTS_PKEY_NOT_SERIALIZED_FLG);
  }

  long indexUID = 0;
  str_sprintf(buf, "insert into %s.\"%s\".%s values (%ld, %ld, '%s', '%s', '%s', '%s', '%s', '%s', %ld, %d, %ld, %ld )",
              getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_TABLE_CONSTRAINTS, tableUID, pkeyUID,
              COM_PRIMARY_KEY_CONSTRAINT_LIT, COM_NO_LIT, COM_NO_LIT, COM_NO_LIT, COM_YES_LIT, COM_YES_LIT,
              validatedTime, numKeys, indexUID, flags);
  cliRC = cliInterface->executeImmediate(buf);

  if (cliRC < 0) {
    cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());

    *CmpCommon::diags() << DgSqlCode(-1423) << DgString0(SEABASE_TABLE_CONSTRAINTS);
  ...
  if (keyInfoArray) {
    for (int i = 0; i < numKeys; i++) {
      str_sprintf(buf, "insert into %s.\"%s\".%s values (%ld, '%s', %d, %d, %d, %d, 0)", getSystemCatalog(),
                  SEABASE_MD_SCHEMA, SEABASE_KEYS, pkeyUID, keyInfoArray[i].colName, i + 1, keyInfoArray[i].tableColNum,
                  0, 0);
      ...
      if (cliRC < 0) {
        ...
        *CmpCommon::diags() << DgSqlCode(-1423) << DgString0(SEABASE_KEYS);
...
// ----------------------------------------------------------------------------
short CmpSeabaseDDL::getPKeyInfoForTable(const char *catName, const char *schName, const char *objName,
  ...
  // get constraint info
  str_sprintf(query,
              "select O.object_name, C.constraint_uid "
              ...
              SEABASE_TABLE_CONSTRAINTS, COM_PRIMARY_KEY_CONSTRAINT_LIT, getSystemCatalog(), SEABASE_MD_SCHEMA,
              SEABASE_OBJECTS, catName, schName, objName);

  Queue *constrInfoQueue = NULL;
  int cliRC = cliInterface->fetchAllRows(constrInfoQueue, query, 0, FALSE, FALSE, TRUE);
  if (cliRC < 0) {
    cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
  ...
  if (constrInfoQueue->numEntries() == 0 && errorIfNotExists == FALSE) {
    constrUID = 0;
    return 0;
  }
  ...
  constrUID = *(long *)vi->get(1);

  return 0;
}

short CmpSeabaseDDL::constraintErrorChecks(ExeCliInterface *cliInterface, StmtDDLAddConstraint *addConstrNode,
                                           NATable *naTable, ComConstraintType ct, NAList<NAString> &keyColList) {
  const NAString &addConstrName = addConstrNode->getConstraintNameAsQualifiedName().getQualifiedNameAsAnsiString();

  if (naTable && naTable->isHbaseMapTable() &&
      ((ct == COM_UNIQUE_CONSTRAINT) || (ct == COM_FOREIGN_KEY_CONSTRAINT) || (ct == COM_PRIMARY_KEY_CONSTRAINT))) {
    *CmpCommon::diags() << DgSqlCode(-3242)
                        << DgString0(
                               "Cannot specify unique, referential or check constraint for an HBase mapped table.");
  ...
  for (int i = 0; i < checkList.entries(); i++) {
    ...
    const NAString &tableConstrName = checkConstr->getConstraintName().getQualifiedNameAsAnsiString();
  ...
  if (NOT foundConstr) {
    ...
    for (int i = 0; i < ariList.entries(); i++) {
      AbstractRIConstraint *ariConstr = (AbstractRIConstraint *)ariList[i];

      const NAString &tableConstrName = ariConstr->getConstraintName().getQualifiedNameAsAnsiString();
  ...
  if (NOT foundConstr) {
    const NAString &constrCatName = addConstrNode->getConstraintNameAsQualifiedName().getCatalogName();
    const NAString &constrSchName = addConstrNode->getConstraintNameAsQualifiedName().getSchemaName();
    const NAString &constrObjName = addConstrNode->getConstraintNameAsQualifiedName().getObjectName();

    // check to see if this constraint was defined on some other table and
    // exists in metadata
    int retcode = existsInSeabaseMDTable(cliInterface, constrCatName, constrSchName, constrObjName, COM_UNKNOWN_OBJECT,
                                         FALSE, FALSE);
    if (retcode == 1)  // exists
    {
      foundConstr = TRUE;
  ...
  if (foundConstr) {
    *CmpCommon::diags() << DgSqlCode(-1043) << DgConstraintName(addConstrName);
  ...
  if ((ct == COM_UNIQUE_CONSTRAINT) || (ct == COM_FOREIGN_KEY_CONSTRAINT) || (ct == COM_PRIMARY_KEY_CONSTRAINT)) {
    ...
    for (CollIndex i = 0; i < keyColList.entries(); i++) {
      ...
      if (!nac) {
        *CmpCommon::diags() << DgSqlCode(-1009) << DgColumnName(ToAnsiIdentifier(keyColList[i]));
        return -1;
      }
      ...
      if (seenIt.contains(colNumber)) {
        *CmpCommon::diags() << DgSqlCode(-CAT_REDUNDANT_COLUMN_REF_PK) << DgColumnName(ToAnsiIdentifier(keyColList[i]));
        return -1;
      }
...
short CmpSeabaseDDL::genUniqueName(StmtDDLAddConstraint *addConstrNode, int *unnamedConstrNum, NAString &uniqueName,
                                   const char *catName, const char *schName, const char *objName) {
  ComObjectName tableName((addConstrNode ? addConstrNode->getTableName() : ""), COM_TABLE_NAME);

  ElemDDLConstraint *constraintNode =
      (addConstrNode ? (addConstrNode->getConstraint())->castToElemDDLConstraint() : NULL);
  ...
  if (!constraintNode || constraintNode->getConstraintName().isNull()) {
    ...
    if (unnamedConstrNum && (*unnamedConstrNum > 0)) {
      str_sprintf(buf, "\"%s%d_%s\"", TRAF_UNNAMED_CONSTR_PREFIX, *unnamedConstrNum,
                  (tableName.isValid() ? tableName.getObjectNamePartAsAnsiString(TRUE).data() : objName));
      ...
      int status = ToInternalIdentifier(oName  // in/out - from external- to internal-format
                                        ,
                                        TRUE  // in - NABoolean upCaseInternalNameIfRegularIdent
                                        ,
                                        TRUE  // in - NABoolean acceptCircumflex
      );
      ComDeriveRandomInternalName(ComGetNameInterfaceCharSet(), /*internalFormat*/ oName  // in  - const ComString &
                                  ,
    ...
    // Check for it.
    const NAString &constrName = constraintNode->getConstraintNameAsQualifiedName().getObjectName();
    if ((constrName.length() >= (strlen(TRAF_UNNAMED_CONSTR_PREFIX))) &&
        (constrName.index(TRAF_UNNAMED_CONSTR_PREFIX) == 0) && (!Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL))) {
      *CmpCommon::diags()
          << DgSqlCode(-3242)
          << DgString0(
                 NAString("The prefix \"") + NAString(TRAF_UNNAMED_CONSTR_PREFIX) +
                 NAString("\" is reserved for unnamed constraints. It cannot be specified for a named constraint."));

      return -1;
    }

    specifiedConstraint = constraintNode->getConstraintNameAsQualifiedName().getQualifiedNameAsAnsiString();
  }
  ...
  if (unnamedConstrNum && (*unnamedConstrNum > 0) && (NOT constrName.isValid())) {
    ...
    *CmpCommon::diags()
        << DgSqlCode(-3242)
        << DgString0(NAString("An invalid name(") + uniqueName +
                     NAString(") was generated for this unnamed " + constrType +
                              "constraint. Reissue the query after providing an explicit name for this constraint. "));
    return -1;
  }
...
short CmpSeabaseDDL::updateConstraintMD(NAList<NAString> &keyColList, NAList<NAString> &keyColOrderList,
                                        NAString &uniqueStr, long tableUID, long constrUID, NATable *naTable,
                                        ComConstraintType ct, NABoolean enforced, ExeCliInterface *cliInterface) {
  int retcode = 0;
  int cliRC = 0;
  ...
  str_sprintf(
      buf, "insert into %s.\"%s\".%s values ('%s', '%s', '%s', '%s', %ld, %ld, %ld, '%s', '%s', %d, %d, 0)",
      getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_OBJECTS, catalogNamePart.data(), quotedSchName.data(),
      quotedObjName.data(),
      ((ct == COM_UNIQUE_CONSTRAINT) ? COM_UNIQUE_CONSTRAINT_OBJECT_LIT
                                     : ((ct == COM_FOREIGN_KEY_CONSTRAINT) ? COM_REFERENTIAL_CONSTRAINT_OBJECT_LIT
                                                                           : COM_CHECK_CONSTRAINT_OBJECT_LIT)),
      constrUID, createTime, createTime, " ", COM_NO_LIT, naTable->getOwner(), naTable->getSchemaOwner());
  cliRC = cliInterface->executeImmediate(buf);
  ...
  str_sprintf(buf, "insert into %s.\"%s\".%s values (%ld, %ld, '%s', '%s', '%s', '%s', '%s', '%s', %ld, %d, %ld, 0 )",
              getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_TABLE_CONSTRAINTS, tableUID, constrUID,
              ((ct == COM_UNIQUE_CONSTRAINT)
                   ? COM_UNIQUE_CONSTRAINT_LIT
                   : ((ct == COM_FOREIGN_KEY_CONSTRAINT) ? COM_FOREIGN_KEY_CONSTRAINT_LIT : COM_CHECK_CONSTRAINT_LIT)),
              COM_NO_LIT, COM_NO_LIT, COM_NO_LIT, (enforced ? COM_YES_LIT : COM_NO_LIT), COM_YES_LIT, createTime,
              keyColList.entries(), indexUID);
  cliRC = cliInterface->executeImmediate(buf);
  ...
  for (int i = 0; i < keyColList.entries(); i++) {
    NAColumn *nac = naColArray.getColumn(keyColList[i]);
    int colNumber = nac->getPosition();

    str_sprintf(buf, "insert into %s.\"%s\".%s values (%ld, '%s', %d, %d, %d, %d, 0)", getSystemCatalog(),
                SEABASE_MD_SCHEMA, SEABASE_KEYS, constrUID, keyColList[i].data(), i + 1, colNumber,
                (keyColOrderList[i] == "DESC" ? 1 : 0), 0);
...
short CmpSeabaseDDL::updateRIConstraintMD(long ringConstrUID, long refdConstrUID, ExeCliInterface *cliInterface) {
  int retcode = 0;
  int cliRC = 0;

  char buf[4000];

  str_sprintf(buf, "insert into %s.\"%s\".%s values (%ld, %ld, '%s', '%s', '%s', 0 )", getSystemCatalog(),
              SEABASE_MD_SCHEMA, SEABASE_REF_CONSTRAINTS, ringConstrUID, refdConstrUID, COM_FULL_MATCH_OPTION_LIT,
              COM_RESTRICT_UPDATE_RULE_LIT, COM_RESTRICT_DELETE_RULE_LIT);
  cliRC = cliInterface->executeImmediate(buf);
  ...
  str_sprintf(buf, "insert into %s.\"%s\".%s values (%ld, %ld, 0)", getSystemCatalog(), SEABASE_MD_SCHEMA,
              SEABASE_UNIQUE_REF_CONSTR_USAGE, refdConstrUID, ringConstrUID);
  cliRC = cliInterface->executeImmediate(buf);
...
short CmpSeabaseDDL::updateIndexInfo(NAList<NAString> &ringKeyColList, NAList<NAString> &ringKeyColOrderList,
                                     NAList<NAString> &refdKeyColList, NAString &uniqueStr, long constrUID,
                                     const char *catName, const char *schName, const char *objName, NATable *naTable,
                                     NABoolean isUnique,  // TRUE: uniq constr. FALSE: ref constr.
                                     NABoolean noPopulate, NABoolean isEnforced, NABoolean sameSequenceOfCols,
                                     ExeCliInterface *cliInterface) {
  ...
  if (naTable->getCorrespondingIndex(ringKeyColList,
                                     ...
                                     sameSequenceOfCols, &existingIndexName))
    createIndex = FALSE;

  // if constraint is not to be enforced, then do not create an index.
  if (createIndex && (NOT isEnforced)) return 0;
  ...
  long tableUID = naTable->objectUid().castToInt64();
  ...
  if (createIndex) {
    ...
    for (CollIndex i = 0; i < ringKeyColList.entries(); i++) {
      ...
      if (i + 1 < ringKeyColList.entries()) keyColNameStr += ", ";
    }
    ...
    if (naTable->isPartitionV2Table()) {
      ...
      if (isUnique)
        str_sprintf(buf, "create unique index \"%s\" on \"%s\".\"%s\".\"%s\" ( %s ) %s global", quotedObjName.data(),
                    catName, schName, objName, keyColNameStr.data(), noPopStr);
      else
        str_sprintf(buf, "create index \"%s\" on \"%s\".\"%s\".\"%s\" ( %s ) %s global", quotedObjName.data(), catName,
                    schName, objName, keyColNameStr.data(), noPopStr);
      /* end partition table create global index add by kai.deng */
    } else {
      if (isUnique)
        str_sprintf(buf, "create unique index \"%s\" on \"%s\".\"%s\".\"%s\" ( %s ) %s", quotedObjName.data(), catName,
                    schName, objName, keyColNameStr.data(), noPopStr);
      else
        str_sprintf(buf, "create index \"%s\" on \"%s\".\"%s\".\"%s\" ( %s ) %s", quotedObjName.data(), catName,
                    schName, objName, keyColNameStr.data(), noPopStr);
    }
    ...
    str_sprintf(
        buf,
        "update %s.\"%s\".%s set is_explicit = 0 where base_table_uid = %ld and index_uid = (select object_uid from "
        "%s.\"%s\".%s where catalog_name = '%s' and schema_name = '%s' and object_name = '%s' and object_type = 'IX') ",
        getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_INDEXES, tableUID, getSystemCatalog(), SEABASE_MD_SCHEMA,
        SEABASE_OBJECTS, catName, schemaNamePart.data(), objectNamePart.data());
    cliRC = cliInterface->executeImmediate(buf);
    if (cliRC < 0) {
      cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
  ...
  long indexUID = getObjectUID(cliInterface, catName, schemaNamePart, objectNamePart, COM_INDEX_OBJECT_LIT);
  if (indexUID < 0) {
  ...
  str_sprintf(buf,
              "update %s.\"%s\".%s set index_uid = %ld where table_uid = %ld and constraint_uid = %ld and "
              "constraint_type = '%s'",
              getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_TABLE_CONSTRAINTS, indexUID, tableUID, constrUID,
...
short CmpSeabaseDDL::isLockedForBR(long objUID, long schUID, ExeCliInterface *cliInterface, const CorrName &corrName) {
  if (objUID <= 0) return 0;

  const NAString &objName = corrName.getExposedNameAsAnsiString();

  Int16 brLockedObjectsAccess = CmpCommon::getDefaultNumeric(TRAF_BR_LOCKED_OBJECTS_ACCESS);
  Int16 brLockedObjectsQueryType = CmpCommon::getDefaultNumeric(TRAF_BR_LOCKED_OBJECTS_QUERY_TYPE);
      ...
      (brLockedObjectsQueryType < 2))  // DDL query is not allowed.
  {
    ...
    if (schUID <= 0) {
      schUID = getObjectUID(cliInterface, corrName.getQualifiedNameObj().getCatalogName().data(),
                            corrName.getQualifiedNameObj().getSchemaName().data(), SEABASE_SCHEMA_OBJECTNAME, NULL,
                            NULL, "object_type in ('SS', 'PS')", NULL, FALSE, FALSE);
    }

    if (schUID > 0) {
      // first check if schema is locked
      if (getTextFromMD(cliInterface, schUID, COM_LOCKED_OBJECT_BR_TAG, 0, brTag)) {
        return -1;
      }

      if (NAString(brTag).contains("(BACKUP)")) {
        *CmpCommon::diags() << DgSqlCode(-4264) << DgString0(objName) << DgString1(brTag);
        return -1;
      }
    }

    // and then check if object is locked
    if (getTextFromMD(cliInterface, objUID, COM_LOCKED_OBJECT_BR_TAG, 0, brTag)) {
      return -1;
    }
...
short CmpSeabaseDDL::setupAndErrorChecks(NAString &tabName, QualifiedName &origTableName, NAString &currCatName,
                                         NAString &currSchName, NAString &catalogNamePart, NAString &schemaNamePart,
                                         NAString &objectNamePart, NAString &extTableName, NAString &extNameForHbase,
                                         CorrName &cn, NATable **naTable, NABoolean volTabSupported,
                                         NABoolean hbaseMapSupported, ExeCliInterface *cliInterface,
                                         const ComObjectType objectType, SQLOperation operation, NABoolean isExternal,
                                         NABoolean processHiatus, NABoolean createHiatusIfNotExist) {
  int cliRC = 0;
  int retcode = 0;
  ...
  if ((tableName.getCatalogNamePartAsAnsiString() == HBASE_SYSTEM_CATALOG) &&
      (tableName.getSchemaNamePartAsAnsiString(TRUE) == HBASE_MAP_SCHEMA) && (hbaseMapSupported))
    hbmapExtTable = TRUE;

  if (isExternal || hbmapExtTable) {
    // Convert the native name to its Trafodion form
    tabName = ComConvertNativeNameToTrafName(tableName.getCatalogNamePartAsAnsiString(),
  ...
  if ((isSeabaseReservedSchema(tableName)) && (!Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL))) {
  ...
  if (((catalogNamePart == HBASE_SYSTEM_CATALOG) &&
       ((schemaNamePart == HBASE_CELL_SCHEMA) || (schemaNamePart == HBASE_ROW_SCHEMA))) &&
  ...
  retcode = lookForTableInMD(cliInterface, catalogNamePart, schemaNamePart, objectNamePart,
                             (schNameSpecified && (NOT hbmapExtTable)), hbmapExtTable, tableName, tabName, extTableName,
                             objectType);
  if (retcode < 0) {
    processReturn();
  ...
  if (naTable) {
    ...
    *naTable = bindWA.getNATableInternal(cn);
    ...
    if (!isDDLOperationAuthorized(operation, objectOwnerID, schemaOwnerID, (int64_t)(*naTable)->objectUid().get_value(),
    ...
    long schUID = (*naTable)->getSchemaUid().castToInt64();
    if (isLockedForBR((*naTable)->objectUid().castToInt64(), schUID, cliInterface, cn)) {
      processReturn();
      return -1;
    }
    ...
    if ((NOT hbaseMapSupported) && (naTable && (*naTable)->isHbaseMapTable())) {
      // not supported
      *CmpCommon::diags() << DgSqlCode(-3242) << DgString0("This feature not available for an HBase mapped table.");
  ...
  extNameForHbase = genHBaseObjName(catalogNamePart, schemaNamePart, objectNamePart,
...
static void resetHbaseSerialization(NABoolean hbaseSerialization, NAString &hbVal) {
...
void *CmpSeabaseDDLcreateTable__pthreadCreate(void *_args) {
  ...
  struct createTablePthreadArgs *args = (struct createTablePthreadArgs *)_args;

  rc = CmpSeabaseDDL::createHbaseTable(args->ehi, args->table, *args->colFamVec, args->hbaseCreateOptions,
                                       args->numSplits, args->keyLength, args->encodedKeysBuffer, args->doRetry,
                                       args->ddlXns, args->incrBackupEnabled, args->createIfNotExists, args->isMVCC);
...
void *CmpSeabaseDDLdropTable__pthreadDrop(void *_args) {
  ...
  ehi = ExpHbaseInterface::newInstance(heap, NULL, NULL, COM_STORAGE_HBASE, FALSE);
  ehi->init(NULL);
...
short CmpSeabaseDDL::createSeabaseTable2(ExeCliInterface &cliInterface, StmtDDLCreateTable *createTableNode,
                                         NAString &currCatName, NAString &currSchName, NABoolean isCompound,
                                         long &outObjUID, NABoolean &schemaStoredDesc, NABoolean &schemaIncBack) {
  int retcode = 0;
  int cliRC = 0;
  ...
  if ((createTableNode->isExternal()) && (NOT createTableNode->mapToHbaseTable())) {
    // The schema name of the target table, if specified,  must match the
    // schema name of the source table
    NAString origSchemaName = createTableNode->getOrigTableNameAsQualifiedName().getSchemaName();

    srcTableName = new (STMTHEAP) ComObjectName(createTableNode->getLikeSourceTableName(), COM_TABLE_NAME);
    ...
    if (srcTableName->getCatalogNamePartAsAnsiString() == HBASE_SYSTEM_CATALOG) {
      *CmpCommon::diags() << DgSqlCode(-3242)
                          << DgString0(
                                 "Cannot create external table on a native HBase table without the MAP TO option.");

      return -1;
    }

    // Convert the native table name to its trafodion name
    NAString tabName = ComConvertNativeNameToTrafName(srcTableName->getCatalogNamePartAsAnsiString(),
    ...
    if (tableName.getSchemaNamePartAsAnsiString(TRUE).length() > ComMAX_ANSI_IDENTIFIER_INTERNAL_LEN) {
      *CmpCommon::diags() << DgSqlCode(-CAT_EXTERNAL_SCHEMA_NAME_TOO_LONG) << DgString0(type.data())
    ...
    if ((origSchemaName.length() > 0) && (origSchemaName != srcTableName->getSchemaNamePart().getExternalName())) {
      *CmpCommon::diags() << DgSqlCode(-CAT_EXTERNAL_NAME_MISMATCH) << DgString0(type.data())
    ...
    if (tableName.getObjectNamePart().getExternalName() != srcTableName->getObjectNamePart().getExternalName()) {
      *CmpCommon::diags() << DgSqlCode(-CAT_EXTERNAL_NAME_MISMATCH) << DgString0(type.data())
  ...
  ParDDLFileAttrsCreateTable &fileAttribs = createTableNode->getFileAttributes();

  NAString objectNamePart = tableName.getObjectNamePartAsAnsiString(TRUE);

  if (fileAttribs.isReadOnlySpecified() && fileAttribs.readOnlyEnabled()) isCreatingReadOnlyTable = TRUE;
  ...
  if ((createTableNode->isExternal()) && (createTableNode->mapToHbaseTable())) {
    if (CmpCommon::getDefault(TRAF_HBASE_MAPPED_TABLES) == DF_OFF) {
      *CmpCommon::diags() << DgSqlCode(-3242) << DgString0("HBase mapped tables not supported.");

      return -1;
    }

    if (isCreatingReadOnlyTable) {
      *CmpCommon::diags() << DgSqlCode(-3242) << DgString0("HBase mapped tables can not build as a read only table");
    ...
    hbaseMapFormat = TRUE;
    hbaseMappedDataFormatIsString = createTableNode->isHbaseDataFormatString();

    ComAnsiNamePart trafCat(TRAFODION_SYSCAT_LIT);
    ComAnsiNamePart trafSch(NAString(HBASE_EXT_MAP_SCHEMA), ComAnsiNamePart::INTERNAL_FORMAT);
    ...
    if (NOT tableNameSpace.isNull()) {
      if ((mappedHbaseNameSpace.isNull()) || (mappedHbaseNameSpace != tableNameSpace)) {
        *CmpCommon::diags() << DgSqlCode(-3242)
                            << DgString0(
                                   "Namespace specified as part of external tablename must be same as the namespace "
                                   "specified in HBase tablename.");
    ...
    if (fileAttribs.isNamespaceSpecified() == TRUE) {
      ...
      if ((mappedHbaseNameSpace.isNull()) || (fileAttrNameSpace != mappedHbaseNameSpace)) {
        *CmpCommon::diags() << DgSqlCode(-3242)
                            << DgString0(
                                   "Namespace specified in the 'namespace' attribute must be the same as the namespace "
                                   "specified in HBase tablename.");
    ...
    if ((tableNameColonPos && (objectNamePart != srcNamePart)) ||
      ...
      *CmpCommon::diags() << DgSqlCode(-CAT_EXTERNAL_NAME_MISMATCH) << DgString0("HBASE")
                          << DgTableName(tableName.getObjectNamePart().getExternalName())
  ...
  if ((isSeabaseReservedSchema(tableName)) && (!Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL))) {
    *CmpCommon::diags() << DgSqlCode(-CAT_CREATE_TABLE_NOT_ALLOWED_IN_SMD) << DgTableName(extTableName);
    deallocEHI(ehi);
  ...
  if (useSwitchCompiler && switchCompiler(CmpContextInfo::CMPCONTEXT_TYPE_META)) {
    deallocEHI(ehi);
    processReturn();
    return -1;
  }
  ...
  retcode = existsInSeabaseMDTable(&cliInterface, catalogNamePart, schemaNamePart, objectNamePart,
                                   COM_BASE_TABLE_OBJECT, FALSE, checkForValidHBaseName);
  ...
  if (retcode == 1)  // already exists
  {
    if (NOT createTableNode->createIfNotExists()) {
      if (createTableNode->isVolatile())
        ...
        *CmpCommon::diags() << DgSqlCode(-1390) << DgString0(extTableName);
    }
  ...
  if (!Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL) && (!createTableNode->isVolatile()) &&
      (!createTableNode->isExternal())) {
    retcode = ddlSetObjectEpoch(createTableNode, STMTHEAP, false, /* allowReads */
                                false /* isContinue */);
    if (retcode < 0) {
      deallocEHI(ehi);
      processReturn();
  ...
  if (createTableNode->isVolatile() && isCreatingReadOnlyTable) {
    *CmpCommon::diags() << DgSqlCode(-3242) << DgString0("Volatile tables can not build as a read only table");
    return -1;
  }
  ...
  if (createTableNode->isExternal()) {
    if (isCreatingReadOnlyTable) {
      *CmpCommon::diags() << DgSqlCode(-3242) << DgString0("External tables can not build as a read only table");

      return -1;
    }
    retcode = createSeabaseTableExternal(cliInterface, createTableNode, tableName, *srcTableName);
  ...
  if (hbaseMapFormat) {
    ...
    if (NOT mappedHbaseNameSpace.isNull()) objNameWithNamespace = mappedHbaseNameSpace + ":";
    ...
    {
      *CmpCommon::diags() << DgSqlCode(-4260) << DgString0(objNameWithNamespace);
  ...
  if ((NOT((createTableNode->isExternal()) && (createTableNode->mapToHbaseTable()))) &&
      ((createTableNode->getIsConstraintPKSpecified()) && (createTableNode->getAddConstraintPK()) &&
  ...
  int schemaOwnerID = SUPER_USER;
  long schemaUID = -1;
  long schemaObjectFlags = 0;
  ComSchemaClass schemaClass;

  retcode =
      verifyDDLCreateOperationAuthorized(&cliInterface, SQLOperation::CREATE_TABLE, catalogNamePart, schemaNamePart,
                                         schemaClass, objectOwnerID, schemaOwnerID, &schemaUID, &schemaObjectFlags);
  if (retcode != 0) {
    handleDDLCreateAuthorizationError(retcode, catalogNamePart, schemaNamePart);
    deallocEHI(ehi);
    processReturn();
  ...
  // create them.
  if (ComIsTrafodionExternalSchemaName(schemaNamePart) && (!Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL)) &&
      (NOT hbaseMapFormat)) {
    // error.
    *SqlParser_Diags << DgSqlCode(-CAT_CREATE_TABLE_NOT_ALLOWED_IN_SMD) << DgTableName(extTableName.data());
    return -1;
  }

  // get namespace of this schema, if it was created in a non-default ns.
  NAString schemaNamespace;
  if (getTextFromMD(&cliInterface, schemaUID, COM_OBJECT_NAMESPACE, 0, schemaNamespace)) {
    processReturn();
    return -1;
  }
  ...
  if (isMDflagsSet(schemaObjectFlags, MD_OBJECTS_ENCRYPT_ROWID_FLG)) schemaEncryptRowid = TRUE;

  if (isMDflagsSet(schemaObjectFlags, MD_OBJECTS_ENCRYPT_DATA_FLG)) schemaEncryptData = TRUE;

  if (createTableNode->getIsLikeOptionSpecified()) {
    createSeabaseTableLike(&cliInterface, createTableNode, currCatName, currSchName);
    deallocEHI(ehi);
    processReturn();
    return -1;
  }
  ...
  if (isMDflagsSet(schemaObjectFlags, MD_OBJECTS_STORED_DESC)) schemaStoredDesc = TRUE;
  ...
  if (schemaClass == COM_SCHEMA_CLASS_SHARED && isHistogramTable(objectNamePart)) objectOwnerID = schemaOwnerID;
  ...
  for (int i = 0; i < colArray.entries(); i++) {
    if ((CmpCommon::getDefault(TRAF_ALLOW_RESERVED_COLNAMES) == DF_OFF) &&
      ...
      *CmpCommon::diags() << DgSqlCode(-CAT_RESERVED_COLUMN_NAME) << DgString0(colArray[i]->getColumnName());
  ...
  ElemDDLColDef syskeyColDef(NULL, &syskeyColName, syskeyType, NULL, STMTHEAP);
  ...
  ElemDDLColDef hbRowIdColDef(NULL, &hbRowIdColName, hbRowIdType, NULL, STMTHEAP);
  ...
  if (((createTableNode->getStoreOption() == COM_KEY_COLUMN_LIST_STORE_OPTION) &&
    ...
    for (int i = 0; ((NOT syskeySpecifiedInStoreByClause) && (i < keyArray.entries())); i++) {
      if (keyArray[i]->getColumnName() == syskeyColName) syskeySpecifiedInStoreByClause = TRUE;
    }
  ...
  int numSaltPartnsFromCQD = CmpCommon::getDefaultNumeric(TRAF_NUM_OF_SALT_PARTNS);
  ...
  if ((createTableNode->getSaltOptions()) ||
      ((numSaltPartnsFromCQD > 0) && (NOT implicitPK) && (NOT createTableNode->isSplitBySpecified()))) {
    if (hbaseMapFormat) {
    ...
    if (isCreatingReadOnlyTable) {
      *CmpCommon::diags() << DgSqlCode(-3242) << DgString0("Partition tables can not build as a read only table");
      return -1;
    }
    ...
    if (saltArray == NULL || saltArray->entries() == 0) {
      ...
      for (CollIndex s = 0; s < saltArray->entries(); s++)
        if (keyArray.getColumnIndex((*saltArray)[s]->getColumnName()) < 0) {
          *CmpCommon::diags() << DgSqlCode(-1195) << DgColumnName((*saltArray)[s]->getColumnName());
          deallocEHI(ehi);
          processReturn();
          return -1;
        }
    ...
    for (CollIndex i = 0; i < saltArray->entries(); i++) {
      ...
      int colIx = colArray.getColumnIndex(colName);
      if (colIx < 0) {
        *CmpCommon::diags() << DgSqlCode(-1009) << DgColumnName(colName);
      ...
      if (colName != "SYSKEY") {
        if (firstSaltCol)
          ...
          saltExprText += ",";
        saltExprText += "CAST(";
        if (NOT cnp.isDelimitedIdentifier()) saltExprText += "\"";
        saltExprText += cnp.getExternalName();
        if (NOT cnp.isDelimitedIdentifier()) saltExprText += "\"";
        saltExprText += " AS ";
        saltExprText += typeText;
        if (!colType->supportsSQLnull()) saltExprText += " NOT NULL";
        saltExprText += ")";
        if (colType->getTypeQualifier() == NA_NUMERIC_TYPE && !(((NumericType *)colType)->isExact())) {
        ...
        // this is an error
        *CmpCommon::diags() << DgSqlCode(-1195) << DgColumnName((*saltArray)[i]->getColumnName());
        deallocEHI(ehi);
        processReturn();
        return -1;
      }
    ...
    if (saltOptions) {
      ...
      numInitialSaltRegions = CmpCommon::getDefaultNumeric(TRAF_NUM_OF_SALT_REGIONS);
    }

    if (numInitialSaltRegions <= 0 || numInitialSaltRegions > numSaltPartns) numInitialSaltRegions = numSaltPartns;
    ...
    if (numSaltPartns <= 1 || numSaltPartns > 1024) {
      ...
      *CmpCommon::diags() << DgSqlCode(-CAT_INVALID_NUM_OF_SALT_PARTNS) << DgInt0(2) << DgInt1(1024);
      deallocEHI(ehi);
      processReturn();
      return -1;
    }
    if ((numInitialSaltRegions != numSaltPartns) && createTableNode->getReplicateClause()) {
    ...
    ElemDDLColDefault *saltDef = new (STMTHEAP) ElemDDLColDefault(ElemDDLColDefault::COL_COMPUTED_DEFAULT);
    saltDef->setComputedDefaultExpr(saltExprText);
    ElemDDLColDef *saltColDef = new (STMTHEAP) ElemDDLColDef(NULL, &saltColName, saltType, saltDef, STMTHEAP);

    ElemDDLColRef *edcrs = new (STMTHEAP) ElemDDLColRef(saltColName, COM_ASCENDING_ORDER);
  ...
  if (numTrafReplicas > 0) {
    ...
    ElemDDLColDefault *replicaDef = new (STMTHEAP) ElemDDLColDefault(ElemDDLColDefault::COL_DEFAULT, replicaItem);
    replicaDef->setDefaultExprString(replicaExprText);
    ElemDDLColDef *replicaColDef =
        new (STMTHEAP) ElemDDLColDef(NULL, &replicaColName, replicaType, replicaDef, STMTHEAP);

    ElemDDLColRef *edcrr = new (STMTHEAP) ElemDDLColRef(replicaColName, COM_ASCENDING_ORDER);
  ...
  if (hbaseMapFormat && hbaseMappedDataFormatIsString) {
    ...
    if (createTableNode->getAddConstraintPK()->getAlterTableAction()->castToElemDDLConstraintPK()->serialized()) {
      *CmpCommon::diags() << DgSqlCode(-3242)
    ...
    int tableColNum = (int)colArray.getColumnIndex(keyArray[0]->getColumnName());
    if (tableColNum < 0) {
      *CmpCommon::diags() << DgSqlCode(-3242) << DgString0("Specified primary key column does not exist.");
  ...
  if (createTableNode->isSplitBySpecified()) splitByClause = createTableNode->getSplitByClause();
  ...
  if (fileAttribs.isRowFormatSpecified() == TRUE) {
    ...
    if (NOT isSeabaseReservedSchema(tableName)) {
      ...
      if (NOT hbaseMapFormat) alignedFormat = TRUE;
    }
  }

  if (!alignedFormat && isCreatingReadOnlyTable) {
    *CmpCommon::diags() << DgSqlCode(-3242) << DgString0("Hbase format tables can not build as a read only table");
    deallocEHI(ehi);
    processReturn();
    return -1;
  }
  ...
  NAString tableNamespace;
  if ((fileAttribs.isNamespaceSpecified() == TRUE) && ((createTableNode->isVolatile()) || (isMonarch))) {
  ...
  } else if ((NOT ComIsTrafodionReservedSchemaName(schemaNamePart)) && (NOT isHistogramTable(objectNamePart))) {
    NAString ns = CmpCommon::getDefaultString(TRAF_OBJECT_NAMESPACE);
    if ((NOT ns.isNull()) && (NOT ComValidateNamespace(&ns, TRUE, tableNamespace))) {
      *CmpCommon::diags() << DgSqlCode(-1065);
      deallocEHI(ehi);
      processReturn();
      return -1;
    }
  } else if (ComIsTrafodionReservedSchemaName(schemaNamePart) && isHistogramTable(objectNamePart)) {
  ...
  else if (ComIsTrafodionReservedSchemaName(schemaNamePart) && ComIsPartitionMDTable(objectNamePart)) {
    tableNamespace = TRAF_RESERVED_NAMESPACE1;
  }

  if ((NOT tableNamespace.isNull()) && (isMonarch)) {
    // monarch tables cannot be created in a namespace.
    tableNamespace.clear();
  }

  if ((NOT tableNamespace.isNull()) && (tableNamespace.index(TRAF_RESERVED_NAMESPACE_PREFIX) == 0) &&
      (!Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL)) &&
      (NOT((schemaNamePart == SEABASE_SYSTEM_SCHEMA) || (createTableNode->isVolatile())))) {
    // cannot create in metadata namespace
    *CmpCommon::diags() << DgSqlCode(-1072) << DgString0(tableNamespace.data());
    return -1;
  }
  ...
  if (enabled) {
...

    {
      ...
      if (tenantSchema == -1) return -1;

      if (isAuthorizationEnabled() && tenantSchema && !ComUser::isRootUserID() && !createTableNode->mapToHbaseTable()) {
        ...
        if (!compPrivs.hasSQLPriv(ComUser::getCurrentUser(), SQLOperation::MANAGE_TENANTS, true)) {
          *CmpCommon::diags() << DgSqlCode(-CAT_WRONG_NAMESPACE) << DgString0(tableNamespace.data())
                              << DgString1(schemaNamePart.data()) << DgString2(schemaNamespace.data());
          return -1;
        }
      }
    }
  ...
  if (NOT tableNamespace.isNull()) {
    retcode = ehi->namespaceOperation(COM_CHECK_NAMESPACE_EXISTS, tableNamespace.data(), 0, NULL, NULL, NULL);
    if (retcode == -HBC_ERROR_NAMESPACE_NOT_EXIST) {
      *CmpCommon::diags() << DgSqlCode(-1067) << DgString0(tableNamespace.data());
      deallocEHI(ehi);
      processReturn();
      return -1;
    }
  ...
  if (xnRepl == COM_REPL_ASYNC) {
    *CmpCommon::diags() << DgSqlCode(-1425) << DgTableName(extTableName)
  ...
  if ((encryptSpec && encryptionOptions) && ((NOT alignedFormat) || (hbaseMapFormat))) {
    *CmpCommon::diags() << DgSqlCode(-3242)
  ...
  NABoolean incrBackupEnabledByUser =
      fileAttribs.isIncrBackupSpecified() ? fileAttribs.incrBackupEnabled() : schemaIncrBackupEnabled;
  NABoolean incrBackupEnabled =
      ((incrBackupEnabledByUser || (CmpCommon::getDefault(TRAF_INCREMENTAL_BACKUP) == DF_ON)) &&
       (((NOT ComIsTrafodionReservedSchemaName(schemaNamePart)) || schemaNamePart == SEABASE_XDC_MD_SCHEMA) &&
        (NOT isHistogramTable(objectNamePart))));

  if ((createTableNode->isVolatile()) && (incrBackupEnabled)) {
    if (fileAttribs.incrBackupEnabled()) {
      *CmpCommon::diags() << DgSqlCode(-3242) << DgString0("Cannot specify 'incremental backup' for volatile tables.");
      return -1;
    }
  ...
  if ((CmpCommon::getDefault(VOLATILE_TABLE_FIND_SUITABLE_KEY) != DF_OFF) && (createTableNode->isVolatile()))
    allowNullableUniqueConstr = TRUE;

  if ((createTableNode->getIsConstraintPKSpecified()) && (createTableNode->getAddConstraintPK()
                                                              ->getAlterTableAction()
                                                              ->castToElemDDLConstraintPK()
                                                              ->isNullableSpecified()))
    allowNullableUniqueConstr = TRUE;
  ...
  if (!allowNullableUniqueConstr && createTableNode->getChild(1 /*StmtDDLCreateTable::INDEX_ATTRIBUTE_LIST*/)) {
    ElemDDLNode *pCreateTableAttrList = createTableNode->getChild(1)->castToElemDDLNode();
    ...
    if (sbkcl && sbkcl->isNullableSpecified()) allowNullableUniqueConstr = TRUE;
  }
  ...
  if (hbaseMapFormat) {
    if (CmpCommon::getDefault(HBASE_SERIALIZATION) == DF_ON) {
      NAString value("OFF");
      hbVal = "ON";
      ActiveSchemaDB()->getDefaults().validateAndInsert("hbase_serialization", value, FALSE);
  ...
  for (int iter = 0; iter < numIterationsToCompleteColumnList; iter++) {
    ...
    if (buildColInfoArray(COM_BASE_TABLE_OBJECT,
                          ...
                          &colArray, colInfoArray, implicitPK, alignedFormat, &identityColPos,
                          (hbaseMapFormat ? NULL : &userColFamVec), &trafColFamVec, defaultColFam.data())) {
    ...
    if (buildKeyInfoArray(&colArray, NULL, &keyArray, colInfoArray, keyInfoArray, allowNullableUniqueConstr)) {
      resetHbaseSerialization(hbaseSerialization, hbVal);
    ...
    if (iter == 0 && createTableNode->isDivisionClauseSpecified()) {
      ...
      CmpSeabaseDDL::convertColAndKeyInfoArrays(numCols, colInfoArray, numKeys, keyInfoArray,
                                                naColArrayForBindingDivExpr, keyColArrayForBindingDivExpr);
      ...
      for (CollIndex d = 0; d < divExpr->entries(); d++) {
        ...
        ItemExpr *boundDivExpr = bindDivisionExprAtDDLTime((*divExpr)[d], keyColArrayForBindingDivExpr, STMTHEAP);
        if (!boundDivExpr) {
          resetHbaseSerialization(hbaseSerialization, hbVal);
        ...
        try {
          ...
          if (CmpCommon::diags()->getNumber(DgSqlCode::ERROR_) == 0)
            *CmpCommon::diags() << DgSqlCode(-4243) << DgString0("(expression with unknown type)");
          exceptionOccurred = TRUE;
        }

        if (exceptionOccurred || boundDivExpr->getValueId() == NULL_VALUE_ID) {
          resetHbaseSerialization(hbaseSerialization, hbVal);
        ...
        NAType *divColType = boundDivExpr->getValueId().getType().newCopy(STMTHEAP);
        ElemDDLColDefault *divColDefault = new (STMTHEAP) ElemDDLColDefault(ElemDDLColDefault::COL_COMPUTED_DEFAULT);
        ...
        ElemDDLColDef *divColDef = new (STMTHEAP) ElemDDLColDef(NULL, &divColName, divColType, divColDefault, STMTHEAP);

        ElemDDLColRef *edcrs = new (STMTHEAP) ElemDDLColRef(divColName, divKeyOrdering);
  ...
  if (numSaltPartns > 0 && keyLength + 2 >= MAX_HBASE_ROWKEY_LEN_SALT_PARTIONS) {
    *CmpCommon::diags() << DgSqlCode(-CAT_ROWKEY_LEN_TOO_LARGE_USING_SALT_PARTIONS) << DgInt0(keyLength + 2)
  ...
  if (hbaseMapFormat) {
    for (CollIndex i = 0; i < colArray.entries(); i++) {
      ...
      int colIx = keyArray.getColumnIndex(colName);
  ...
  if (numSaltSplits > 0 || numTrafReplicas > 0 || splitByClause) {
    TrafDesc *colDescs = convertVirtTableColumnInfoArrayToDescStructs(&tableName, colInfoArray, numCols);
    TrafDesc *keyDescs = convertVirtTableKeyInfoArrayToDescStructs(keyInfoArray, colInfoArray, numKeys);

    if (createEncodedKeysBuffer(encodedKeysBuffer /*out*/, numSplits /*out*/, colDescs, keyDescs, numSaltPartns,
                                numSaltSplits, &splitByClause, numTrafReplicas, numKeys, keyLength, FALSE)) {
      processReturn();
  ...
  int *partionColIdxArray = NULL;
  int *subpartionColIdxArray = NULL;
  if (isPartitionV2Spec) {
    // validate that partition by columns exist
    const ElemDDLColRefArray &partitionColArray = createTableNode->getPartitionV2ColRefArray();
    const ElemDDLColRefArray &subpartitionColArray = createTableNode->getSubpartitionColRefArray();
    int pColLength = 0, subpColLength = 0;
    int partionColCnt = partitionColArray.entries();
    int subpartionColCnt = subpartitionColArray.entries();
    if (partionColCnt > 0) partionColIdxArray = new (STMTHEAP) int[partionColCnt];
    if (subpartionColCnt > 0) subpartionColIdxArray = new (STMTHEAP) int[subpartionColCnt];
    ...
    if (buildKeyInfoArray(&colArray, NULL, (ElemDDLColRefArray *)&partitionColArray, colInfoArray, pColInfoArray, true,
                          &pColLength)) {
      processReturn();
      return -1;
    }
    TrafDesc *pColDescs =
        convertVirtTableKeyInfoArrayToDescStructs(pColInfoArray, colInfoArray, partitionColArray.entries());
    ...
    if (subpartitionColArray.entries() > 0) {
      ...
      if (buildKeyInfoArray(&colArray, NULL, (ElemDDLColRefArray *)&subpartitionColArray, colInfoArray,
                            subpColInfoArray, true, &subpColLength)) {
        processReturn();
        return -1;
      }
      subpColDescs =
          convertVirtTableKeyInfoArrayToDescStructs(subpColInfoArray, colInfoArray, subpartitionColArray.entries());
    }
    ...
    ElemDDLPartitionV2Array &partitionV2Array = createTableNode->getPartitionV2Array();
    NAHashDictionary<NAString, int> *partitionNameMap =
        new (STMTHEAP) NAHashDictionary<NAString, int>(NAString::hash, 101, TRUE, STMTHEAP);

    int rc = validatePartitionByExpr((&(partitionV2Array)), colDescs, pColDescs, partitionColArray.entries(),
                                     pColLength, partitionNameMap);
    if (rc) {
      if (partitionNameMap) delete partitionNameMap;
      return -1;
    }

    if (createTableNode->getPartitionType() == COM_RANGE_PARTITION) {
      rc = validatePartitionRange((&(partitionV2Array)), partitionColArray.entries());
      if (rc) {
        if (partitionNameMap) delete partitionNameMap;
        return -1;
      }
    ...
    if (subpartitionColArray.entries() > 0) {
      for (int i = 0; i < partitionV2Array.entries(); i++) {
        if (partitionV2Array[i]->getSubpartitionArray() && partitionV2Array[i]->getSubpartitionArray()->entries() > 0) {
          rc = validatePartitionByExpr(partitionV2Array[i]->getSubpartitionArray(), colDescs, subpColDescs,
                                       subpartitionColArray.entries(), subpColLength, partitionNameMap);
          if (rc) {
            if (partitionNameMap) delete partitionNameMap;
            return -1;
          }

          rc = validatePartitionRange(partitionV2Array[i]->getSubpartitionArray(), subpartitionColArray.entries());
          if (rc) {
            if (partitionNameMap) delete partitionNameMap;
            return -1;
          }
  ...
  if (fileAttribs.isOwnerSpecified()) {
    ...
    if (schemaClass == COM_SCHEMA_CLASS_PRIVATE && objectOwnerID != schemaOwnerID) {
  ...
  if (encryptSpec) {
    if (rowIdEncrypt) CmpSeabaseDDL::setMDflags(tableInfo->objectFlags, MD_OBJECTS_ENCRYPT_ROWID_FLG);

    if (dataEncrypt) CmpSeabaseDDL::setMDflags(tableInfo->objectFlags, MD_OBJECTS_ENCRYPT_DATA_FLG);
  }
  ...
  tableInfo->tableNamespace = (NOT tableNamespace.isNull() ? tableNamespace.data() : NULL);
  ...
  short retVal =
      setupHbaseOptions(createTableNode->getHbaseOptionsClause(), numSplits, extTableName, *hbaseCreateOptions, hco);
  if (retVal) {
    deallocEHI(ehi);
    processReturn();
    return -1;
  ...
  {
    const char *v = ActiveSchemaDB()->getDefaults().getValue(TRAF_CREATE_TABLE_WITH_UID);
    if ((v) and (strlen(v) > 0)) {
      objUID = str_atoi(v, strlen(v));
    }
  }

  if (isPartitionV2Spec) CmpSeabaseDDL::setMDflags(tableInfo->objectFlags, MD_PARTITION_TABLE_V2);

  if (createTableNode->isPartition()) CmpSeabaseDDL::setMDflags(tableInfo->objectFlags, MD_PARTITION_V2);

  int hbaseDropCreate = CmpCommon::getDefaultNumeric(TRAF_HBASE_DROP_CREATE);
  bool updPrivs = (hbaseDropCreate != 0);
  if (updateSeabaseMDTable(&cliInterface, catalogNamePart, schemaNamePart, objectNamePart, COM_BASE_TABLE_OBJECT,
                           COM_NO_LIT, tableInfo, numCols, colInfoArray, numKeys, keyInfoArray, 0, NULL, objUID,
                           updPrivs)) {
    *CmpCommon::diags() << DgSqlCode(-CAT_UNABLE_TO_CREATE_OBJECT) << DgTableName(extTableName);
  ...
  if (addToTextTab) {
    ...
    cliRC = updateTextTable(&cliInterface, objUID, COM_HBASE_COL_FAMILY_TEXT, 0, allColFams);
    if (cliRC < 0) {
      *CmpCommon::diags() << DgSqlCode(-CAT_UNABLE_TO_CREATE_OBJECT) << DgTableName(extTableName);
  ...
  if (createTableNode->getAddConstraintPK()) {
    if (updatePKeyInfo(createTableNode->getAddConstraintPK(), catalogNamePart, schemaNamePart, objectNamePart,
                       objectOwnerID, schemaOwnerID, keyArray.entries(), NULL, NULL, keyInfoArray, &cliInterface)) {
      return -1;
    }
  }
  ...
  if (identityColPos >= 0) {
    ...
    NAString seqName;
    SequenceGeneratorAttributes::genSequenceName(catalogNamePart, schemaNamePart, objectNamePart,
                                                 colDef->getColumnName(), seqName);
    ...
    str_sprintf(buf, "create internal sequence %s.\"%s\".\"%s\" %s", catalogNamePart.data(), schemaNamePart.data(),
                seqName.data(), idOptions.data());
    ...
    ActiveSchemaDB()->getNATableDB()->removeNATable(cn, ComQiScope::REMOVE_MINE_ONLY, COM_BASE_TABLE_OBJECT,
                                                    createTableNode->ddlXns(), FALSE);

    long flags = 0;
    if (xnRepl == COM_REPL_SYNC) CmpSeabaseDDL::setMDflags(flags, MD_SEQGEN_REPL_SYNC_FLG);

    // update datatype and replication flags for this sequence
    str_sprintf(buf,
                "update %s.\"%s\".%s set fs_data_type = %d, flags = bitor(flags, %ld) where seq_type = '%s' and "
                "seq_uid = (select object_uid from %s.\"%s\".\"%s\" where catalog_name = '%s' and schema_name = '%s' "
                "and object_name = '%s' and object_type = '%s') ",
                getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_SEQ_GEN, colDef->getColumnDataType()->getFSDatatype(),
                flags, COM_INTERNAL_SG_LIT, getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_OBJECTS,
                catalogNamePart.data(), schemaNamePart.data(), seqName.data(), COM_SEQUENCE_GENERATOR_OBJECT_LIT);
  ...
  if (CmpCommon::getDefault(TRAF_TRANS_TYPE) == DF_SSCC) isMVCC = FALSE;

  if ((NOT extNameForHbase.isNull()) && (hbaseDropCreate != 0) &&
      (NOT isPartitionV2Spec))  // for partitionv2 table, do not create hbase table
  {
    ...
    if (isMonarch) {
      ...
      for (int i = 0; i < colArray.entries(); i++) {
        ...
        NAString colQual;
        HbaseAccess::convNumToId(colInfoArray[i].hbaseColQual, strlen(colInfoArray[i].hbaseColQual), colQual);
        *nas = colFam + ":" + colQual;
      ...
      if (hbaseDropCreate == 2) {
        ...
        createTablePthreadArgs *args = new CTXTHEAP createTablePthreadArgs;
        ...
        *tcfv = trafColFamVec;
        ...
        s = pthread_create(&lob_ddl_thread_id, NULL, CmpSeabaseDDLcreateTable__pthreadCreate, (void *)args);
        if (s != 0) {
          *CmpCommon::diags() << DgSqlCode(-CAT_CREATE_OBJECT_ERROR) << DgTableName(extTableName);
          deallocEHI(ehi);
          processReturn();
        ...
        retcode = createHbaseTable(ehi, &hbaseTable, trafColFamVec, hbaseCreateOptions, numSplits, keyLength,
                                   encodedKeysBuffer, FALSE, ddlXns, snapShotOption, FALSE, isMVCC);
    }  // hbase
  ...
  if (!ComIsTrafodionReservedSchemaName(schemaNamePart) && !(createTableNode->isExternal()) &&
  ...
  if (!ComIsTrafodionReservedSchemaName(schemaNamePart) && incrBackupEnabled == true)
    retcode1 = updateSeabaseXDCDDLTable(&cliInterface, catalogNamePart.data(), schemaNamePart.data(),
                                        objectNamePart.data(), COM_BASE_TABLE_OBJECT_LIT);
  if (retcode < 0 || retcode1 < 0) {
    *CmpCommon::diags() << DgSqlCode(-8448) << DgString0((char *)"ExpHbaseInterface::create()")
                        << DgString1(retcode < 0 ? "cannot update binlog table info" : "cannot update XDCDDLTable");
    deallocEHI(ehi);
    processReturn();
    return -2;
  }
  ...
  if (isPartitionV2Spec) {
    ...
    if (partitionV2Array.entries() > 0) {
      for (CollIndex i = 0; i < partitionV2Array.entries(); ++i) {
        if (partitionV2Array[i]->hasSubpartition() && partitionV2Array[i]->getSubpartitionArray()->entries() > 0) {
          ElemDDLPartitionV2Array *subpartArray = partitionV2Array[i]->getSubpartitionArray();
          for (CollIndex j = 0; j < subpartArray->entries(); ++j) {
            ret = createSeabasePartitionTable(&cliInterface, (*subpartArray)[j]->getPartitionName(), catalogNamePart,
                                              schemaNamePart, objectNamePart);
            if (ret < 0) break;
          }
        } else {
          ret = createSeabasePartitionTable(&cliInterface, partitionV2Array[i]->getPartitionName(), catalogNamePart,
                                            schemaNamePart, objectNamePart);
        }
        if (ret < 0) {
          cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
      ...
      updateSeabaseMDPartition(&cliInterface, createTableNode, catalogNamePart.data(), schemaNamePart.data(),
                               objectNamePart.data(), (long)outObjUID, partionColIdxArray, subpartionColIdxArray);
      if (isStoredDesc) {
        UInt32 op = 0;
        ...
        for (CollIndex i = 0; i < partitionV2Array.entries(); ++i) {
          if (partitionV2Array[i]->hasSubpartition() && partitionV2Array[i]->getSubpartitionArray()->entries() > 0) {
            ...
            for (CollIndex j = 0; j < subpartArray->entries(); ++j) {
              ...
              getPartitionTableName(partitionTableName, 256, objectNamePart, (*subpartArray)[j]->getPartitionName());
              long objUid = getObjectUID(&cliInterface, catalogNamePart, schemaNamePart, partitionTableName,
                                         COM_BASE_TABLE_OBJECT_LIT, NULL, NULL, NULL, false, false);
              UpdateObjRefTimeParam updObjRef(-2, objUid, isStoredDesc, FALSE);
              ret = updateCachesAndStoredDesc(&cliInterface, catalogNamePart, schemaNamePart, partitionTableName,
                                              COM_BASE_TABLE_OBJECT, op, NULL, &updObjRef,
                                              SharedCacheDDLInfo::DDL_INSERT);
              if (ret < 0) break;
            }
          } else {
            char partitionTableName[256];
            getPartitionTableName(partitionTableName, 256, objectNamePart, partitionV2Array[i]->getPartitionName());
            long objUid = getObjectUID(&cliInterface, catalogNamePart, schemaNamePart, partitionTableName,
                                       COM_BASE_TABLE_OBJECT_LIT, NULL, NULL, NULL, false, false);
            UpdateObjRefTimeParam updObjRef(-2, objUid, isStoredDesc, FALSE);
            ret =
                updateCachesAndStoredDesc(&cliInterface, catalogNamePart, schemaNamePart, partitionTableName,
                                          COM_BASE_TABLE_OBJECT, op, NULL, &updObjRef, SharedCacheDDLInfo::DDL_INSERT);
          }
          if (ret < 0) {
            cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
            deallocEHI(ehi);
...
void CmpSeabaseDDL::createSeabaseTable(StmtDDLCreateTable *createTableNode, NAString &currCatName,
                                       NAString &currSchName, NABoolean isCompound, long *retObjUID,
                                       NABoolean *retGenStoredDesc, NABoolean *retIncBack) {
  int cliRC = 0;
  ...
  if (beginXnIfNotInProgress(&cliInterface, xnWasStartedHere)) return;

  long objUID = 0;
  NABoolean schemaStoredDesc = FALSE;
  NABoolean schemaIncBack = FALSE;
  short rc = createSeabaseTable2(cliInterface, createTableNode, currCatName, currSchName, isCompound, objUID,
                                 schemaStoredDesc, schemaIncBack);
  ActiveSchemaDB()->getDefaults().holdOrRestore("traf_lob_parallel_ddl", 2);

  if ((CmpCommon::diags()->getNumber(DgSqlCode::ERROR_)) && (rc < 0)) {
    endXnIfStartedHere(&cliInterface, xnWasStartedHere, -1);

    if (rc == -2)  // cleanup before returning error..
    {
      cleanupObjectAfterError(cliInterface, catalogNamePart, schemaNamePart, objectNamePart, COM_BASE_TABLE_OBJECT,
                              NULL, createTableNode->ddlXns());
    }

    return;
  }

  if (retObjUID) *retObjUID = objUID;
  ...
  NABoolean genStoredDesc =
      ((schemaStoredDesc || fileAttribs.storedDesc() || (CmpCommon::getDefault(TRAF_STORE_OBJECT_DESC) == DF_ON)) &&
       (NOT isHistogramTable(objectNamePart)));

  if (createTableNode->isVolatile()) {
    if (fileAttribs.storedDesc()) {
      *CmpCommon::diags() << DgSqlCode(-3242) << DgString0("Cannot specify 'stored desc' for volatile tables.");
  ...
  if (retGenStoredDesc) *retGenStoredDesc = genStoredDesc;

  if (retIncBack) *retIncBack = schemaIncBack;

  if (NOT isCompound) {
    if (updateObjectRedefTime(&cliInterface, catalogNamePart, schemaNamePart, objectNamePart, COM_BASE_TABLE_OBJECT_LIT,
                              -1, objUID, genStoredDesc, FALSE)) {
      endXnIfStartedHere(&cliInterface, xnWasStartedHere, -1);
    ...
    // Add entry to list of DDL operations
    if (genStoredDesc && (!createTableNode->isExternal()) && (!Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL)))
      insertIntoSharedCacheList(catalogNamePart, schemaNamePart, objectNamePart, COM_BASE_TABLE_OBJECT,
                                SharedCacheDDLInfo::DDL_INSERT);
...
int CmpSeabaseDDL::createSeabasePartitionTable(ExeCliInterface *cliInterface, const NAString &partName,
  ...
  char likeTable[1000];
  snprintf(likeTable, sizeof(likeTable), "%s.%s.%s like %s.%s.%s;", currCatName.data(), currSchName.data(),
           ToAnsiIdentifier(partitionTableName).data(), currCatName.data(), currSchName.data(),
           ToAnsiIdentifier(baseTableName).data());
...
int CmpSeabaseDDL::createSeabasePartitionTableWithInfo(ExeCliInterface *cliInterface, const NAString &partName,
                                                       const NAString &currCatName, const NAString &currSchName,
                                                       const NAString &baseTableName, long objUID, int partPositoin,
                                                       NAString &PKey, NAString &pEntityNmae, NAString &PKcolName,
                                                       NAString &PK, int i, char *partitionTableName) {
  int ret;

  getPartitionTableName(partitionTableName, 256, baseTableName, partName);

  char sqlBuff[1000];
  snprintf(sqlBuff, sizeof(sqlBuff), "%s.%s.%s like %s.%s.%s;", currCatName.data(), currSchName.data(),
           ToAnsiIdentifier(partitionTableName).data(), currCatName.data(), currSchName.data(),
           ToAnsiIdentifier(baseTableName).data());
...

  {
    cliInterface->holdAndSetCQD("TRAF_UPSERT_THROUGH_PARTITIONS", "ON");
    {
      ...
      str_sprintf(sqlBuff, "upsert using load into %s.\"%s\".\"%s\" select * from %s.\"%s\".\"%s\" where ",
                  currCatName.data(), currSchName.data(), partitionTableName, currCatName.data(), currSchName.data(),
                  pEntityNmae.data());
  ...
  long partitionUID =
      getObjectUID(cliInterface, currCatName.data(), currSchName.data(), partitionTableName, COM_BASE_TABLE_OBJECT_LIT);

  snprintf(sqlBuff, sizeof(sqlBuff),
           "update %s.\"%s\".%s set PARTITION_ORDINAL_POSITION = PARTITION_ORDINAL_POSITION + 1 "
           " where PARTITION_ORDINAL_POSITION >= %d and PARENT_UID = %ld;",
           getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_PARTITIONS, partPositoin, objUID);
  ...
  ret = updateSeabaseTablePartitions(cliInterface, objUID, partName.data(), partitionTableName, partitionUID,
                                     COM_PRIMARY_PARTITION_LIT, (partPositoin), PKey.data());
...
static int extractNumVal(const NAString &constrName) {
  int numVal = -1;
...
static int getNextNumVal(const NAString &constrName, int inNum) {
  int retNum = inNum;
  int numVal = extractNumVal(constrName);
  if ((numVal > 0) && (numVal > inNum)) retNum = numVal;

  return retNum;
}

int CmpSeabaseDDL::getNextUnnamedConstrNum(NATable *naTable) {
  int nextNum = 0;

  const CheckConstraintList &ccList = naTable->getCheckConstraints();
  for (int i = 0; i < ccList.entries(); i++) {
  ...
  for (int i = 0; i < ucList.entries(); i++) {
  ...
  for (int i = 0; i < rcList.entries(); i++) {
...
void CmpSeabaseDDL::addConstraints(ComObjectName &tableName, ComAnsiNamePart &currCatAnsiName,
                                   ComAnsiNamePart &currSchAnsiName, StmtDDLNode *ddlNode,
                                   StmtDDLAddConstraintPK *pkConstr, StmtDDLAddConstraintUniqueArray &uniqueConstrArr,
                                   StmtDDLAddConstraintRIArray &riConstrArr,
                                   StmtDDLAddConstraintCheckArray &checkConstrArr, int inUnnamedConstrNum,
                                   NABoolean isCompound, NABoolean isPartitionV2) {
  int cliRC = 0;
  ...
  int unnamedConstrNum = inUnnamedConstrNum;
  if (pkConstr) {
    NAString uniqueName;
    ...
    ElemDDLConstraintPK *constraintNode = (pkConstr->getConstraint())->castToElemDDLConstraintPK();
    ...
    for (CollIndex i = 0; i < keyColumnArray.entries(); i++) {
      ...
      if (i + 1 < keyColumnArray.entries()) keyColNameStr += ", ";
    }

    str_sprintf(buf, "alter table \"%s\".\"%s\".\"%s\" add constraint \"%s\".\"%s\".\"%s\" primary key %s (%s)",
                catalogNamePart.data(), schemaNamePart.data(), objectNamePart.data(), constrCatalogNamePart.data(),
                constrSchemaNamePart.data(), constrObjectNamePart.data(),
                (constraintNode->isNullableSpecified() ? " nullable " : ""), keyColNameStr.data());
  ...
  if (uniqueConstrArr.entries() > 0) {
    for (int i = 0; i < uniqueConstrArr.entries(); i++) {
      StmtDDLAddConstraintUnique *uniqConstr = uniqueConstrArr[i];
      ...
      ElemDDLConstraintUnique *constraintNode = (uniqConstr->getConstraint())->castToElemDDLConstraintUnique();
      ElemDDLColRefArray &keyColumnArray = constraintNode->getKeyColumnArray();
      ...
      str_sprintf(buf, "alter table \"%s\".\"%s\".\"%s\" add constraint \"%s\".\"%s\".\"%s\" unique (%s)",
                  catalogNamePart.data(), schemaNamePart.data(), objectNamePart.data(), constrCatalogNamePart.data(),
                  constrSchemaNamePart.data(), constrObjectNamePart.data(), keyColNameStr.data());
  ...
  if (riConstrArr.entries() > 0) {
    for (int i = 0; i < riConstrArr.entries(); i++) {
      ...
      ElemDDLConstraintRI *constraintNode = (refConstr->getConstraint())->castToElemDDLConstraintRI();
      ...
      if (cn2.getExposedNameAsAnsiString() != extTableName) {
        if (lockObjectDDL(cn2.getQualifiedNameObj(), COM_BASE_TABLE_OBJECT, cn2.isVolatile(), cn2.isExternal()) < 0) {
          processReturn();
          goto label_return;
        }
      ...
      str_sprintf(buf,
                  "alter table \"%s\".\"%s\".\"%s\" add constraint \"%s\".\"%s\".\"%s\" foreign key (%s) references "
                  "\"%s\".\"%s\".\"%s\" %s %s",
                  catalogNamePart.data(), schemaNamePart.data(), objectNamePart.data(), constrCatalogNamePart.data(),
                  constrSchemaNamePart.data(), constrObjectNamePart.data(), ringColNameStr.data(),
      ...
      if (NOT isCompound) {
        ...
        ActiveSchemaDB()->getNATableDB()->removeNATable(cn2, ComQiScope::REMOVE_FROM_ALL_USERS, COM_BASE_TABLE_OBJECT,
      ...
      if (refdNaTable && refdNaTable->isStoredDesc() && !Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL))
        insertIntoSharedCacheList(refdCatNamePart, refdSchNamePart, refdObjNamePart, COM_BASE_TABLE_OBJECT,
                                  SharedCacheDDLInfo::DDL_UPDATE);
      if (cliRC < 0) goto label_return;

      if (NOT constraintNode->isEnforced()) {
        *CmpCommon::diags() << DgSqlCode(1313) << DgString0(addConstrName);
      }
  ...
  if (checkConstrArr.entries() > 0) {
    for (int i = 0; i < checkConstrArr.entries(); i++) {
      ...
      str_sprintf(buf, "alter table \"%s\".\"%s\".\"%s\" add constraint \"%s\".\"%s\".\"%s\" check %s",
                  catalogNamePart.data(), schemaNamePart.data(), objectNamePart.data(), constrCatalogNamePart.data(),
                  constrSchemaNamePart.data(), constrObjectNamePart.data(), constrText.data());
...
label_return:
  ...
  if (NOT isCompound) {
    ...
    CorrName cn(objectNamePart.data(), STMTHEAP, schemaNamePart.data(), catalogNamePart.data());
...
void CmpSeabaseDDL::createSeabaseTableCompound(StmtDDLCreateTable *createTableNode, NAString &currCatName,
  ...
  if ((createTableNode->isVolatile()) && ((createTableNode->getAddConstraintUniqueArray().entries() > 0) ||
                                          (createTableNode->getAddConstraintRIArray().entries() > 0) ||
                                          (createTableNode->getAddConstraintCheckArray().entries() > 0))) {
  ...
  if ((createTableNode->mapToHbaseTable()) && ((createTableNode->getAddConstraintUniqueArray().entries() > 0) ||
                                               (createTableNode->getAddConstraintRIArray().entries() > 0) ||
                                               (createTableNode->getAddConstraintCheckArray().entries() > 0))) {
    *CmpCommon::diags() << DgSqlCode(-3242)
                        << DgString0(
                               "Cannot specify unique, referential or check constraints for an HBase mapped table.");
  ...
  createSeabaseTable(createTableNode, currCatName, currSchName, TRUE, &objUID, &genStoredDesc, &schemaIncBack);
  ...
  addConstraints(
      tableName, currCatAnsiName, currSchAnsiName, createTableNode, NULL,
      createTableNode->getAddConstraintUniqueArray(), createTableNode->getAddConstraintRIArray(),
      createTableNode->getAddConstraintCheckArray(),
      ((fileAttribs.isXnReplSpecified() || fileAttribs.isIncrBackupSpecified() || schemaIncBack == true) ? 1 : 0), TRUE,
      createTableNode->isPartitionByClauseV2Spec());
  ...
  if (CmpCommon::diags()->getNumber(DgSqlCode::ERROR_)) {
    ...
    *CmpCommon::diags() << DgSqlCode(-1029) << DgTableName(extTableName);
  ...
  if (genStoredDesc) {
    ...
    str_sprintf(buf,
                ...
                SEABASE_OBJECTS, getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_OBJECTS, catalogNamePart.data(),
    ...
    if (indexes && indexes->numEntries() > 0) {
      ...
      for (int i = 0; i < indexes->numEntries(); i++) {
        ...
        if (genStoredDesc && (!Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL)))
          insertIntoSharedCacheList(ixCatName, ixSchName, ixName, COM_INDEX_OBJECT, SharedCacheDDLInfo::DDL_INSERT);
      }
    }
  }
...
label_error:
  ...
  if (NOT createTableNode->isVolatile()) {
    cleanupObjectAfterError(cliInterface, catalogNamePart, schemaNamePart, objectNamePart, COM_BASE_TABLE_OBJECT, NULL,
...
short CmpSeabaseDDL::lookForTableInMD(ExeCliInterface *cliInterface, NAString &catNamePart, NAString &schNamePart,
                                      NAString &objNamePart, NABoolean schNameSpecified, NABoolean hbaseMapSpecified,
  ...
  if ((schNamePart == HBASE_EXT_MAP_SCHEMA) && (NOT hbaseMapSpecified) &&
      (!Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL))) {
    *CmpCommon::diags() << DgSqlCode(-4261) << DgSchemaName(schNamePart);

    return -1;
  }

  if (NOT hbaseMapSpecified)
    retcode = existsInSeabaseMDTable(cliInterface, catNamePart, schNamePart, objNamePart,
                                     objectType,  // COM_BASE_TABLE_OBJECT,
                                     (Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL) ? FALSE : TRUE),
                                     checkForValidHbaseName, TRUE);
  if (retcode < 0) {
    return -1;  // error
  }

  if (retcode == 1) return 1;  // exists

  if ((retcode == 0) &&  // does not exist
      (NOT schNameSpecified)) {
    retcode = existsInSeabaseMDTable(cliInterface, catNamePart, HBASE_EXT_MAP_SCHEMA, objNamePart, objectType,
                                     (Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL) ? FALSE : TRUE),
                                     checkForValidHbaseName, TRUE);
    if (retcode < 0) {
      return -1;  // error
    ...
    if (retcode != 0)  // exists
    {
      ...
      ComAnsiNamePart mapSchAnsiName(schNamePart, ComAnsiNamePart::INTERNAL_FORMAT);
...
// RETURN:  -1, no need to cleanup.  -2, caller need to call cleanup
//                  0, all ok.
short CmpSeabaseDDL::dropSeabaseTable2(ExeCliInterface *cliInterface, StmtDDLDropTable *dropTableNode,
                                       NAString &currCatName, NAString &currSchName) {
  int cliRC = 0;
  int retcode = 0;
  ...
  NABoolean catNameSpecified = (NOT dropTableNode->getOrigTableNameAsQualifiedName().getCatalogName().isNull());
  NABoolean schNameSpecified = (NOT dropTableNode->getOrigTableNameAsQualifiedName().getSchemaName().isNull());
  ...
  if (tableName.getCatalogNamePartAsAnsiString() == HIVE_SYSTEM_CATALOG) isHive = TRUE;
  ...
  if (((tableName.getCatalogNamePartAsAnsiString() == HBASE_SYSTEM_CATALOG) &&
    ...
    if (dropTableNode->isExternal() && (NOT isHive)) {
      ...
      if (ComIsTrafodionExternalSchemaName(tableName.getSchemaNamePartAsAnsiString(), &hiveName)) {
        if (hiveName) hbmapExtTable = FALSE;
      }
    }
  }
  ...
  if ((isSeabaseReservedSchema(tableName)) && (!Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL))) {
    *CmpCommon::diags() << DgSqlCode(-CAT_USER_CANNOT_DROP_SMD_TABLE) << DgTableName(extTableName);
  ...
  if ((dropTableNode->isVolatile()) && (CmpCommon::context()->sqlSession()->volatileSchemaInUse())) {
    volTabName = tableName;
    isVolatile = TRUE;
  }

  if ((NOT dropTableNode->isVolatile()) && (CmpCommon::context()->sqlSession()->volatileSchemaInUse())) {
    ...
    if (qn == NULL) {
      *CmpCommon::diags() << DgSqlCode(-CAT_UNABLE_TO_DROP_OBJECT)
                          << DgTableName(
                                 dropTableNode->getOrigTableNameAsQualifiedName().getQualifiedNameAsAnsiString(TRUE));
  ...
  retcode = lockObjectDDL(dropTableNode->getTableNameAsQualifiedName(), COM_BASE_TABLE_OBJECT,
                          dropTableNode->isVolatile(), dropTableNode->isExternal());
  if (retcode < 0) {
    processReturn();
    return -1;
  }
  ...
  if (retcode == 0)  // does not exist
  {
    if (NOT dropTableNode->dropIfExists()) {
      ...
      if (isVolatile)
        ...
        *CmpCommon::diags() << DgSqlCode(-1386) << DgString0(objectNamePart);
      else
        *CmpCommon::diags() << DgSqlCode(-CAT_OBJECT_DOES_NOT_EXIST_IN_TRAFODION) << DgString0(extTableName);
    }
  ...
  if (dropTableNode->isExternal()) bindWA.setExternalTableDrop(TRUE);
  ...
  if (naTable->isPartitionEntityTable() && NOT dropTableNode->isPartition()) {
    *CmpCommon::diags() << DgSqlCode(-4265) << DgString0(extTableName);
    processReturn();
    return -1;
  }
  ...
  if (naTable->isPartitionV2Table()) {
    ...
    for (int i = 0; i < partitionArray.entries(); i++) {
      ...
      cliRC = dropSeabasePartitionTable(cliInterface, catalogNamePart, schemaNamePart, partitionBtName,
                                        dropTableNode->getDropBehavior() == COM_CASCADE_DROP_BEHAVIOR);
      if (cliRC < 0) {
        cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());

        *CmpCommon::diags() << DgSqlCode(-4266) << DgString0(extTableName) << DgString1("drop partition base table");
        processReturn();
        return -1;
      }
    }
  ...
  long objUID = naTable->objectUid().castToInt64();
  long dataUID = naTable->objDataUID().castToInt64();
  long schUID = naTable->getSchemaUid().castToInt64();
  long originuid = objUID;
  if (isLockedForBR(objUID, schUID, cliInterface, cn)) {
    return -1;
  }
  ...
  if (!isDDLOperationAuthorized(SQLOperation::DROP_TABLE, naTable->getOwner(), naTable->getSchemaOwner(),
                                (int64_t)naTable->objectUid().get_value(), COM_BASE_TABLE_OBJECT,
                                naTable->getPrivInfo())) {
    *CmpCommon::diags() << DgSqlCode(-CAT_NOT_AUTHORIZED);
  ...
  const NAString extNameForHbase =
      genHBaseObjName(catalogNamePart, schemaNamePart, objectNamePart, tableNamespace, dataUID);

  if (!isSeabaseExternalSchema(catalogNamePart, schemaNamePart)) {
    HbaseStr hbaseTable;
    NAString nameForUID;

    hbaseTable.val = (char *)extNameForHbase.data(), hbaseTable.len = extNameForHbase.length();

    if ((NOT isVolatile) && (ehi->exists(hbaseTable) == 0) &&
        NOT naTable->isPartitionV2Table())  // does not exist in hbase
    {
      *CmpCommon::diags() << DgSqlCode(-4254) << DgString0(extTableName);
  ...
  if (naTable->isPartitionV2Table()) {
    ...
    if (cliRC < 0) {
      *CmpCommon::diags() << DgSqlCode(-4266) << DgString0(extTableName) << DgString1("delete partition property");
      processReturn();
      return -1;
    }
  }
  ...
  if (naTable->isPartitionEntityTable() && dropTableNode->isPartition()) {
    ...
    if (cliRC < 0) {
      *CmpCommon::diags() << DgSqlCode(-4266) << DgString0(extTableName) << DgString1("update MD partitions");
      processReturn();
      return -1;
    }
    ...
    if (cliRC < 0) {
      *CmpCommon::diags() << DgSqlCode(-4266) << DgString0(extTableName) << DgString1("delete MD partitions");
      processReturn();
      return -1;
    }
  }
  ...
  if (dropTableNode->getDropBehavior() == COM_RESTRICT_DROP_BEHAVIOR) {
    ...
    cliRC = getAllUsingViews(cliInterface, catalogNamePart, schemaNamePart, objectNamePart, usingViewsQueue);
    if (cliRC < 0) {
      deallocEHI(ehi);
      processReturn();
    ...
    for (int i = 0; i < uniqueList.entries(); i++) {
      ...
      if (uniqConstr->hasRefConstraintsReferencingMe()) {
        ...
        if (rc->getTableName() != naTable->getTableName()) {
          const NAString &constrName = (rc ? rc->getConstraintName().getObjectName() : " ");
          *CmpCommon::diags() << DgSqlCode(-1059) << DgConstraintName(constrName);
  ...
  str_sprintf(
      ...
      getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_OBJECTS, getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_ROUTINES,
      objUID);
  ...
  triggerss->position();
  {
    ...
    for (int i = 0; i < triggerss->numEntries(); i++) {
      ...
      memset(query, 0, 4000);
      str_sprintf(query, "drop trigger %s;", objname);
  ...
  for (int i = 0; i < uniqueList.entries(); i++) {
    ...
    if (uniqConstr->hasRefConstraintsReferencingMe()) {
      for (int j = 0; j < uniqConstr->getNumRefConstraintsReferencingMe(); j++) {
        const ComplementaryRIConstraint *rc = uniqConstr->getRefConstraintReferencingMe(j);

        // Update the epoch value of the referenced table to lock out other user access
        CorrName cn2(rc->getTableName().getObjectName().data(), STMTHEAP,
                     rc->getConstraintName().getSchemaName().data(), rc->getConstraintName().getCatalogName().data());

        if (lockObjectDDL(cn2.getQualifiedNameObj(), COM_BASE_TABLE_OBJECT, isVolatile, cn2.isExternal()) < 0) {
          deallocEHI(ehi);
          processReturn();
          return -1;
        }
        ...
        str_sprintf(query, "alter table \"%s\".\"%s\".\"%s\" drop constraint \"%s\".\"%s\".\"%s\"",
                    rc->getTableName().getCatalogName().data(), rc->getTableName().getSchemaName().data(),
                    rc->getTableName().getObjectName().data(), rc->getConstraintName().getCatalogName().data(),
                    rc->getConstraintName().getSchemaName().data(), rc->getConstraintName().getObjectName().data());
        ...
        if (!selfRef) {
          if (refdNaTable && refdNaTable->isStoredDesc() && !Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL))
            insertIntoSharedCacheList(rc->getTableName().getCatalogName(), rc->getTableName().getSchemaName().data(),
                                      rc->getTableName().getObjectName().data(), COM_BASE_TABLE_OBJECT,
                                      SharedCacheDDLInfo::DDL_UPDATE);
        }
      }  // for
    }    // if
  ...
  for (int i = 0; i < uniqueList.entries(); i++) {
    ...
    const NAString &constrCatName = uniqConstr->getConstraintName().getCatalogName();

    const NAString &constrSchName = uniqConstr->getConstraintName().getSchemaName();

    NAString constrObjName = (NAString)uniqConstr->getConstraintName().getObjectName();
    ...
    if (isSeabasePrivMgrMD(catalogNamePart, schemaNamePart) ||
      ...
      if (getPKeyInfoForTable(catalogNamePart.data(), schemaNamePart.data(), objectNamePart.data(), cliInterface,
                              constrObjName, constrUID) == -1) {
        deallocEHI(ehi);
        processReturn();
        return -1;
      }
    ...
    // Read the metadata to get the constraint UID
      ...
      constrUID = getObjectUID(cliInterface, constrCatName.data(), constrSchName.data(), constrObjName.data(),
                               (uniqConstr->isPrimaryKeyConstraint() ? COM_PRIMARY_KEY_CONSTRAINT_OBJECT_LIT
    ...
    if (deleteConstraintInfoFromSeabaseMDTables(
            cliInterface, naTable->objectUid().castToInt64(), 0, constrUID, 0, constrCatName, constrSchName,
            constrObjName,
            (uniqConstr->isPrimaryKeyConstraint() ? COM_PRIMARY_KEY_CONSTRAINT_OBJECT
  ...
  for (int i = 0; i < refList.entries(); i++) {
    ...
    if (refConstr->selfRef()) continue;

    const NAString &constrCatName = refConstr->getConstraintName().getCatalogName();

    const NAString &constrSchName = refConstr->getConstraintName().getSchemaName();

    const NAString &constrObjName = refConstr->getConstraintName().getObjectName();

    long constrUID = getObjectUID(cliInterface, constrCatName.data(), constrSchName.data(), constrObjName.data(),
    ...
    if (lockObjectDDL(otherCN.getQualifiedNameObj(), COM_BASE_TABLE_OBJECT, otherCN.isVolatile(),
    ...
    long schUID = otherNaTable->getSchemaUid().castToInt64();
    if (isLockedForBR(otherNaTable->objectUid().castToInt64(), schUID, cliInterface, otherCN)) {
      processReturn();
      return -1;
    }
    ...
    const NAString &otherSchName = otherConstr->getConstraintName().getSchemaName();

    const NAString &otherConstrName = otherConstr->getConstraintName().getObjectName();

    long otherConstrUID = getObjectUID(cliInterface, constrCatName.data(), otherSchName.data(), otherConstrName.data(),
                                       COM_UNIQUE_CONSTRAINT_OBJECT_LIT);
    if (otherConstrUID < 0) {
      CmpCommon::diags()->clear();
      otherConstrUID = getObjectUID(cliInterface, constrCatName.data(), otherSchName.data(), otherConstrName.data(),
    ...
    if (deleteConstraintInfoFromSeabaseMDTables(
            cliInterface, naTable->objectUid().castToInt64(), otherNaTable->objectUid().castToInt64(), constrUID,
            otherConstrUID, constrCatName, constrSchName, constrObjName, COM_REFERENTIAL_CONSTRAINT_OBJECT)) {
      deallocEHI(ehi);
      processReturn();
    ...
    if (updateObjectRedefTime(
            cliInterface, otherNaTable->getTableName().getCatalogName(), otherNaTable->getTableName().getSchemaName(),
            otherNaTable->getTableName().getObjectName(), COM_BASE_TABLE_OBJECT_LIT, -1,
            otherNaTable->objectUid().castToInt64(), otherNaTable->isStoredDesc(), otherNaTable->storedStatsAvail())) {
      processReturn();
      deallocEHI(ehi);
    ...
    // Add entry to list of DDL operations
    if (otherNaTable->isStoredDesc() && (!Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL))) {
      insertIntoSharedCacheList(
          otherNaTable->getTableName().getCatalogName(), otherNaTable->getTableName().getSchemaName(),
          otherNaTable->getTableName().getObjectName(), COM_BASE_TABLE_OBJECT, SharedCacheDDLInfo::DDL_UPDATE);
  ...
  if (NOT(dropTableNode->getDropBehavior() == COM_NO_CHECK_DROP_BEHAVIOR) &&
    ...
    for (int i = 0; i < checkList.entries(); i++) {
      ...
      const NAString &constrCatName = checkConstr->getConstraintName().getCatalogName();

      const NAString &constrSchName = checkConstr->getConstraintName().getSchemaName();

      const NAString &constrObjName = checkConstr->getConstraintName().getObjectName();
      ...
      if (deleteConstraintInfoFromSeabaseMDTables(cliInterface, naTable->objectUid().castToInt64(), 0, constrUID, 0,
  ...
  if (hbaseDropCreate != 2) {
    ...
    for (int idx = 0; idx < indexInfoQueue->numEntries(); idx++) {
      ...
      if (naTable->isStoredDesc() && (!Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL)))
        insertIntoSharedCacheList(idxCatName, idxSchName, idxObjName, COM_INDEX_OBJECT, SharedCacheDDLInfo::DDL_DELETE);

      if (dropSeabaseObject(ehi, ansiName, idxCatName, idxSchName, COM_INDEX_OBJECT, dropTableNode->ddlXns(), TRUE,
                            FALSE, isMonarch, tableNamespace, TRUE, NULL, &indexNameForUID, 1)) {
    ...
    if (indexInfoQueue->numEntries() > 0) SQL_EXEC_SetSecInvalidKeys(indexInfoQueue->numEntries(), qiKeys);
    NADELETEBASIC(qiKeys, STMTHEAP);
  }
  ...
  if (found) {
    NAString seqName;
    SequenceGeneratorAttributes::genSequenceName(catalogNamePart, schemaNamePart, objectNamePart, col->getColName(),
                                                 seqName);

    char buf[4000];
    str_sprintf(buf, "drop sequence %s.\"%s\".\"%s\"", catalogNamePart.data(), schemaNamePart.data(), seqName.data());
    ...
    ActiveSchemaDB()->getNATableDB()->removeNATable(seqn, ComQiScope::REMOVE_FROM_ALL_USERS,
                                                    COM_SEQUENCE_GENERATOR_OBJECT, dropTableNode->ddlXns(), FALSE);
  }
  ...
  // tables
  if (!(tableName.isExternalHive() || tableName.isExternalHbase() || (hbaseDropCreate == 2))) {
    if ((!ComIsTrafodionReservedSchemaName(schemaNamePart)) && objectNamePart != "SB_HISTOGRAMS" &&
        objectNamePart != "SB_HISTOGRAM_INTERVALS" && objectNamePart != "SB_PERSISTENT_SAMPLES" &&
        strncmp(objectNamePart.data(), TRAF_SAMPLE_PREFIX, sizeof(TRAF_SAMPLE_PREFIX)) != 0)

    {
      if (dropSeabaseStats(cliInterface, catalogNamePart.data(), schemaNamePart.data(), objUID)) {
        deallocEHI(ehi);
        processReturn();
        return -1;
      }
  ...
  if (indexInfoQueue) {
    ...
    for (int idx = 0; idx < indexInfoQueue->numEntries(); idx++) {
      ...
      if (dropSeabaseObject(ehi, ansiName, idxCatName, idxSchName, COM_INDEX_OBJECT, dropTableNode->ddlXns(), FALSE,
                            TRUE, isMonarch, tableNamespace, TRUE, NULL, &tmp, 2)) {
        deallocEHI(ehi);
        processReturn();
      ...
      if (naTable->readOnlyEnabled() &&
          (!Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL) || Get_SqlParser_Flags(CONTEXT_IN_DROP_SCHEMA))) {
        insertIntoSharedCacheList(qCatName, qSchName, qObjName, COM_BASE_TABLE_OBJECT, SharedCacheDDLInfo::DDL_DELETE,
                                  SharedCacheDDLInfo::SHARED_DATA_CACHE);

        if (!GetCliGlobals()->currContext()->getTransaction()->autoCommit() ||
            Get_SqlParser_Flags(CONTEXT_IN_DROP_SCHEMA)) {
          int32_t diagsMark = CmpCommon::diags()->mark();

          str_sprintf(query, "alter trafodion data shared cache for index %s delete internal", ansiName.data());
  ...
  if (hbaseDropCreate == 0) dropFromHbase = FALSE;

  // internal table add X lock
  if (!dropTableNode->isExternal() &&
      lockRequired(ehi, naTable, catalogNamePart, schemaNamePart, objectNamePart, HBaseLockMode::LOCK_X,
  ...
  if (dropSeabaseObject(ehi, tabName, currCatName, currSchName, COM_BASE_TABLE_OBJECT, dropTableNode->ddlXns(),
                        dropFromMD, dropFromHbase, isMonarch, tableNamespace, updPrivs, NULL)) {
    deallocEHI(ehi);
    processReturn();
  ...
  if (naTable->incrBackupEnabled() && !ComIsTrafodionReservedSchemaName(schemaNamePart)) {
  ...
  if (hbaseDropCreate == 2)  // pthread drop
  {
    ...
    const NAString extNameForHbase = genHBaseObjName(catalogNamePart, schemaNamePart, objectNamePart, tableNamespace);
    ...
    s = pthread_create(&lob_ddl_thread_id, NULL, CmpSeabaseDDLdropTable__pthreadDrop, (void *)args);
    if (s != 0) {
      *CmpCommon::diags() << DgSqlCode(-CAT_CREATE_OBJECT_ERROR) << DgTableName(extTableName);
      deallocEHI(ehi);
      processReturn();
  ...
  if ((dropTableNode->isExternal()) && (dropTableNode->getTableNameAsQualifiedName().isHive())) {
    CorrName hcn(dropTableNode->getTableNameAsQualifiedName());
    ActiveSchemaDB()->getNATableDB()->removeNATable(hcn, ComQiScope::REMOVE_FROM_ALL_USERS, COM_BASE_TABLE_OBJECT,
                                                    dropTableNode->ddlXns(), FALSE);
  }
  ...
  for (int i = 0; i < refList.entries(); i++) {
    ...
    ActiveSchemaDB()->getNATableDB()->removeNATable(otherCN, ComQiScope::REMOVE_FROM_ALL_USERS, COM_BASE_TABLE_OBJECT,
                                                    dropTableNode->ddlXns(), FALSE);
  }
  ...
  for (int i = 0; i < uniqueList.entries(); i++) {
    ...
    if (uniqConstr->hasRefConstraintsReferencingMe()) {
      for (int j = 0; j < uniqConstr->getNumRefConstraintsReferencingMe(); j++) {
        ...
        CorrName cnr(rc->getTableName().getObjectName().data(), STMTHEAP, rc->getTableName().getSchemaName().data(),
                     rc->getTableName().getCatalogName().data());
        ActiveSchemaDB()->getNATableDB()->removeNATable(cnr, ComQiScope::REMOVE_FROM_ALL_USERS, COM_BASE_TABLE_OBJECT,
                                                        dropTableNode->ddlXns(), FALSE);
      }  // for
  ...
  // Add entry to list of DDL operations
  if (naTable->isStoredDesc() && (!Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL))) {
    insertIntoSharedCacheList(tableName.getCatalogNamePartAsAnsiString(), tableName.getSchemaNamePartAsAnsiString(),
                              tableName.getObjectNamePartAsAnsiString(), COM_BASE_TABLE_OBJECT,
...
void CmpSeabaseDDL::dropSeabaseTable(StmtDDLDropTable *dropTableNode, NAString &currCatName, NAString &currSchName) {
  int cliRC = 0;
  ...
  short rc = dropSeabaseTable2(&cliInterface, dropTableNode, currCatName, currSchName);
  ActiveSchemaDB()->getDefaults().holdOrRestore("traf_lob_parallel_ddl", 2);
...
int CmpSeabaseDDL::dropSeabasePartitionTable(ExeCliInterface *cliInterface, NAString &currCatName,
                                             NAString &currSchName, const NAString &btTableName,
                                             NABoolean needCascade) {
  int ret;
  char buf[4000];
  memset(buf, 0, 4000);
  str_sprintf(buf, "DROP PARTITION TABLE %s.\"%s\".%s %s", currCatName.data(), currSchName.data(),
              ToAnsiIdentifier(btTableName).data(), needCascade ? "cascade" : "");
  ...
  if (ret >= 0) {
    ...
    insertIntoSharedCacheList(currCatName, currSchName, btTableName, COM_BASE_TABLE_OBJECT,
                              SharedCacheDDLInfo::DDL_DELETE);
  }
  return ret;
}

int CmpSeabaseDDL::dropSeabasePartitionTable(ExeCliInterface *cliInterface,
                                             StmtDDLAlterTableDropPartition *alterDropPartition, NAPartition *partInfo,
                                             NAString &currCatName, NAString &currSchName) {
  int ret = 0;
  ...
  if (!alterDropPartition->isSubpartition() && partInfo->hasSubPartition()) {
    ...
    for (int i = 0; i < subPartArray->entries(); ++i) {
      ret = dropSeabasePartitionTable(cliInterface, currCatName, currSchName,
                                      (*subPartArray)[i]->getPartitionEntityName());
      if (ret < 0) return ret;
    }
  }

  return dropSeabasePartitionTable(cliInterface, currCatName, currSchName, partInfo->getPartitionEntityName());
}

int CmpSeabaseDDL::updateSeabasePartitionbtMDDataPosition(ExeCliInterface *cliInterface, const long partitionUID) {
  int ret;
  char buf[4000];
  memset(buf, 0, 4000);
  str_sprintf(buf,
              ...
              SEABASE_PARTITIONS, partitionUID, getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_PARTITIONS,
              partitionUID);
...
int CmpSeabaseDDL::deleteSeabasePartitionbtMDData(ExeCliInterface *cliInterface, const long partitionUID) {
  int ret;
  char buf[4000];
  memset(buf, 0, 4000);
  char partitionUIDStr[20];
  str_sprintf(buf, "DELETE FROM %s.\"%s\".%s WHERE PARTITION_UID = %ld", getSystemCatalog(), SEABASE_MD_SCHEMA,
              SEABASE_PARTITIONS, partitionUID);

  ret = cliInterface->executeImmediate(buf);
  return ret;
}

int CmpSeabaseDDL::deleteSeabasePartitionMDData(ExeCliInterface *cliInterface, const long tableUID) {
  int ret;
  char buf[4000];
  memset(buf, 0, 4000);
  str_sprintf(buf, "DELETE FROM %s.\"%s\".%s WHERE TABLE_UID = %ld", getSystemCatalog(), SEABASE_MD_SCHEMA,
              SEABASE_TABLE_PARTITION, tableUID);

  ret = cliInterface->executeImmediate(buf);
  return ret;
}

short CmpSeabaseDDL::invalidateStats(long tableUID) {
  SQL_QIKEY qiKey;
...
int CmpSeabaseDDL::createObjectEpochCacheEntry(ObjectEpochCacheEntryName *name) {
  if (CmpCommon::getDefault(TRAF_LOCK_DDL) == DF_OFF) return 0;

  int retcode = -EXE_OEC_UNABLE_TO_GET_DIST_LOCK;  // assume failure
  WaitedLockController lock(name->getDLockKey(), 1);
  if (lock.lockHeld()) {
    int objectNameLength = name->objectName().length();
    ...
    retcode = SQL_EXEC_SetObjectEpochEntry(ObjectEpochChangeRequest::CREATE_OBJECT_EPOCH_ENTRY, objectNameLength,
                                           objectName, name->getRedefTime(), name->getHash(), expectedEpoch,
                                           expectedFlags, expectedEpoch, expectedFlags,
    ...
    if (retcode == -EXE_OEC_UNEXPECTED_STATE) {
      ...
      retcode = SQL_EXEC_SetObjectEpochEntry(
          ObjectEpochChangeRequest::FORCE, objectNameLength, objectName, name->getRedefTime(), name->getHash(),
          maxExpectedEpochFound, maxExpectedFlagsFound, maxExpectedEpochFound,
          maxExpectedFlagsFound,  // no, that's not a typo; this line repeats!
...
int CmpSeabaseDDL::modifyObjectEpochCacheStartDDL(ObjectEpochCacheEntryName *name, UInt32 expectedEpoch,
                                                  bool readsAllowed) {
  if (CmpCommon::getDefault(TRAF_LOCK_DDL) == DF_OFF) return 0;
  ...
  if (lock.lockHeld()) {
    ...
    if (!readsAllowed) newFlags |= ObjectEpochChangeRequest::READS_DISALLOWED;
    UInt32 maxExpectedEpochFound = 0;
    UInt32 maxExpectedFlagsFound = 0;
    // update the flags in the ObjectEpochCacheEntry; leave the epoch unchanged until operation complete
    retcode = SQL_EXEC_SetObjectEpochEntry(ObjectEpochChangeRequest::START_OR_CONTINUE_DDL_OPERATION, objectNameLength,
                                           objectName, name->getRedefTime(), name->getHash(), expectedEpoch,
                                           ObjectEpochChangeRequest::NO_DDL_IN_PROGRESS, expectedEpoch, newFlags,
                                           /* out */ &maxExpectedEpochFound, /* out */ &maxExpectedFlagsFound);
...
int CmpSeabaseDDL::modifyObjectEpochCacheContinueDDL(ObjectEpochCacheEntryName *name, UInt32 expectedEpoch) {
  if (CmpCommon::getDefault(TRAF_LOCK_DDL) == DF_OFF) return 0;
  ...
  if (lock.lockHeld()) {
    ...
    // update the flags in the ObjectEpochCacheEntry; leave the epoch unchanged until operation complete
    retcode = SQL_EXEC_SetObjectEpochEntry(
        ObjectEpochChangeRequest::START_OR_CONTINUE_DDL_OPERATION, objectNameLength, objectName, name->getRedefTime(),
        name->getHash(), expectedEpoch, ObjectEpochChangeRequest::DDL_IN_PROGRESS, expectedEpoch,
        ObjectEpochChangeRequest::DDL_IN_PROGRESS | ObjectEpochChangeRequest::READS_DISALLOWED,
        /* out */ &maxExpectedEpochFound, /* out */ &maxExpectedFlagsFound);
    if (retcode == EXE_OEC_CACHE_FULL) {
      // We got a "cache full" warning. This can happen only if
      // CQD TRAF_OBJECT_EPOCH_CACHE_BEHAVIOR is set to '1'. It happens
...
int CmpSeabaseDDL::modifyObjectEpochCacheEndDDL(ObjectEpochCacheEntryName *name, UInt32 expectedEpoch) {
  if (CmpCommon::getDefault(TRAF_LOCK_DDL) == DF_OFF) return 0;
  ...
  if (lock.lockHeld()) {
    ...
    retcode = SQL_EXEC_SetObjectEpochEntry(
        ObjectEpochChangeRequest::COMPLETE_DDL_OPERATION, objectNameLength, objectName, name->getRedefTime(),
        name->getHash(), expectedEpoch,
        ObjectEpochChangeRequest::DDL_IN_PROGRESS | ObjectEpochChangeRequest::READS_DISALLOWED, expectedEpoch + 1,
        ObjectEpochChangeRequest::NO_DDL_IN_PROGRESS,
        /* out */ &maxExpectedEpochFound, /* out */ &maxExpectedFlagsFound);
...
int CmpSeabaseDDL::modifyObjectEpochCacheAbortDDL(ObjectEpochCacheEntryName *name, UInt32 expectedEpoch, bool force) {
  if (CmpCommon::getDefault(TRAF_LOCK_DDL) == DF_OFF) return 0;
  ...
  if (lock.lockHeld()) {
    ...
    UInt32 maxExpectedEpochFound = 0;
    UInt32 maxExpectedFlagsFound = 0;
    int operation = ObjectEpochChangeRequest::ABORT_DDL_OPERATION;
    if (force) operation = ObjectEpochChangeRequest::FORCE;
    // reset the flags to show now DDL in progress; epoch stays the same as before
    retcode = SQL_EXEC_SetObjectEpochEntry(
        operation, objectNameLength, objectName, name->getRedefTime(), name->getHash(), expectedEpoch,
        ObjectEpochChangeRequest::DDL_IN_PROGRESS | ObjectEpochChangeRequest::READS_DISALLOWED, expectedEpoch,
        ObjectEpochChangeRequest::NO_DDL_IN_PROGRESS,
        /* out */ &maxExpectedEpochFound, /* out */ &maxExpectedFlagsFound);
    if (retcode == EXE_OEC_CACHE_FULL) {
      // We got a "cache full" warning. This can happen only if
...
// ----------------------------------------------------------------------------
void CmpSeabaseDDL::alterSeabaseTableResetDDLLock(ExeCliInterface *cliInterface,
                                                  StmtDDLAlterTableResetDDLLock *alterTableNode, NAString &currCatName,
                                                  NAString &currSchName) {
  ...
  if (!isDDLOperationAuthorized(SQLOperation::ALTER_TABLE, 0 /*tblOwner*/, 0 /*schOwner*/, 0 /*uid*/,
  ...
  if (alterTableNode->objectLock()) {
    ...
    LOGDEBUG(CAT_SQL_LOCK, "%s object lock for table %s", alterTableNode->force() ? "Force reset" : "Reset",
             extTableName.data());
    NAString one("1");
    ...
    ActiveSchemaDB()->getDefaults().validateAndInsert("TRAF_OBJECT_LOCK_DDL_RETRY_LIMIT", one, FALSE);
    if (alterTableNode->force()) {
      NAString on("ON");
      ActiveSchemaDB()->getDefaults().validateAndInsert("TRAF_OBJECT_LOCK_FORCE", on, FALSE);
    }

    if (lockObjectDDL(tableName, COM_BASE_TABLE_OBJECT, alterTableNode->isVolatile(), alterTableNode->isExternal()) <
        0) {
  ...
  ObjectEpochCacheEntryName oecName(STMTHEAP, extTableName, COM_BASE_TABLE_OBJECT, 0 /*redefTime*/);
  ...
  if (retcode == ObjectEpochCache::OEC_ERROR_OK) {
    ...
    snprintf(msgBuf, sizeof(msgBuf), "Reset DDL lock, Name: %s, Epoch: %d, Flags: %d", extTableName.data(),
             oecEntry->epoch(), oecEntry->flags());
...
// ----------------------------------------------------------------------------
short CmpSeabaseDDL::ddlSetObjectEpoch(const NATable *naTable, NAHeap *heap, bool allowReads, bool isContinue) {
  if (CmpCommon::getDefault(TRAF_LOCK_DDL) == DF_OFF) return 0;
  ...
  return ddlSetObjectEpochHelper(naTable->getTableName(), naTable->getObjectType(), naTable->objectUid().get_value(),
                                 naTable->getRedefTime(), naTable->isVolatileTable(), naTable->isTrafExternalTable(),
                                 heap, allowReads, isContinue);
}
...
//      1 - successful, DDL lock obtained
// ----------------------------------------------------------------------------
short CmpSeabaseDDL::ddlSetObjectEpoch(const StmtDDLCreateTable *createTableNode, NAHeap *heap, bool allowReads,
                                       bool isContinue) {
  short retcode = 0;

  CMPASSERT(createTableNode != NULL);

  return ddlSetObjectEpochHelper(
      createTableNode->getTableNameAsQualifiedName(), COM_BASE_TABLE_OBJECT, 0, /* objectUID */
      0,                                                                        /* redefTime */
      createTableNode->isVolatile(), createTableNode->isExternal(), heap, allowReads, isContinue);
}
...
//      1 - successful, DDL lock obtained
// ----------------------------------------------------------------------------
short CmpSeabaseDDL::ddlSetObjectEpochHelper(const QualifiedName &qualName, ComObjectType objectType, long objectUID,
                                             long redefTime, bool isVolatile, bool isExternal, NAHeap *heap,
                                             bool allowReads, bool isContinue) {
  if (CmpCommon::getDefault(TRAF_LOCK_DDL) == DF_OFF) return 0;
  ...
  if (isSeabaseReservedSchema(qualName.getCatalogName(), qualName.getSchemaName())) return 0;

  // If special tables, just return
  if (isLOBDependentNameMatch(qualName.getObjectName())) return 0;

  // if not TRAFODION catalog, just return
  if (qualName.getCatalogName() != TRAFODION_SYSTEM_CATALOG) return 0;

  // If not a base table, just return
  // TODO: support other object types (UDR)
  if (objectType != COM_BASE_TABLE_OBJECT) return 0;
  ...
  if (retcode == ObjectEpochCache::OEC_ERROR_OK) {
    ...
    if (retcode < 0) {
      ...
      if (retcode == ObjectEpochCache::OEC_ERROR_OK) {
        ...
        *CmpCommon::diags() << DgSqlCode(-4401) << DgString0(tableName.data()) << DgString1(buf);
        return -4401;
      }
    }
  } else
  ...
  int ddlListEntry = CmpCommon::context()->ddlObjsList().findEntry(tableName);
  if (ddlListEntry >= 0) {
    NABoolean disallowReadsSet = (currentFlags & ObjectEpochChangeRequest::READS_DISALLOWED);
    if (disallowReadsSet) {
      if (allowReads) {
        *CmpCommon::diags() << DgSqlCode(-CAT_DDL_OP_IN_PROGRESS) << DgString0(tableName.data());
  ...
  if (ddlListEntry < 0 && (currentFlags & ObjectEpochChangeRequest::DDL_IN_PROGRESS)) {
    if ((currentFlags & ObjectEpochChangeRequest::READS_DISALLOWED) || (!isContinue)) {
      *CmpCommon::diags() << DgSqlCode(-CAT_DDL_OP_IN_PROGRESS) << DgString0(tableName.data());
      return -CAT_DDL_OP_IN_PROGRESS;
    }
  }
  ...
  snprintf(msgBuf, sizeof(msgBuf),
           ...
           (isContinue ? "DDL Continue" : "DDL Start"), tableName.data(), currentEpoch, currentFlags, allowReads);
  QRLogger::log(CAT_SQL_EXE, LL_WARN, "%s", msgBuf);
#ifndef NDEBUG
  char *oec = getenv("TEST_OBJECT_EPOCH");
  if (oec) cout << msgBuf << endl;
...
short CmpSeabaseDDL::lockObjectDDL(const NATable *naTable) {
  ...
  return lockObjectDDL(naTable->getTableName(), naTable->getObjectType(), naTable->isVolatileTable(),
                       naTable->isTrafExternalTable());
}

short CmpSeabaseDDL::lockObjectDDL(const StmtDDLCreateTable *createTableNode) {
  CMPASSERT(createTableNode != NULL);
  return lockObjectDDL(createTableNode->getTableNameAsQualifiedName(), COM_BASE_TABLE_OBJECT,
                       createTableNode->isVolatile(), createTableNode->isExternal());
}

short CmpSeabaseDDL::lockObjectDDL(const StmtDDLAlterTable *alterTableNode) {
  CMPASSERT(alterTableNode != NULL);
  return lockObjectDDL(alterTableNode->getTableNameAsQualifiedName(), COM_BASE_TABLE_OBJECT,
                       alterTableNode->isVolatile(), alterTableNode->isExternal());
}

short CmpSeabaseDDL::lockObjectDDL(const ComObjectName &objectName, ComObjectType objectType, bool isVolatile,
                                   bool isExternal) {
  QualifiedName qualName(objectName);
  return lockObjectDDL(qualName, objectType, isVolatile, isExternal);
}

short CmpSeabaseDDL::lockObjectDDL(const QualifiedName &qualName, ComObjectType objectType, bool isVolatile,
                                   bool isExternal) {
  short retcode = 0;

  if (CmpCommon::getDefault(TRAF_OBJECT_LOCK) == DF_OFF) return 0;
  ...
  LOGDEBUG(CAT_SQL_LOCK, "LOCK OBJECT DDL: %s", qualName.getQualifiedNameAsAnsiString().data());
  ...
  int cliRC = SQL_EXEC_LockObjectDDL(tabName.data(), COM_BASE_TABLE_OBJECT);
  if (cliRC < 0) {
...
short CmpSeabaseDDL::unlockObjectDDL(const QualifiedName &qualName, ComObjectType objectType, bool isVolatile,
                                     bool isExternal) {
  short retcode = 0;
  ...
  LOGDEBUG(CAT_SQL_LOCK, "UNLOCK OBJECT DDL: %s", qualName.getQualifiedNameAsAnsiString().data());
  CMPASSERT(!qualName.getCatalogName().isNull());
  CMPASSERT(!qualName.getSchemaName().isNull());
  CMPASSERT(!qualName.getObjectName().isNull());
  ...
  NAString tabName = qualName.getQualifiedNameAsAnsiString();
  int cliRC = SQL_EXEC_UnLockObjectDDL(tabName.data(), COM_BASE_TABLE_OBJECT);
  if (cliRC < 0) {
...
// for partition v2
short CmpSeabaseDDL::lockObjectDDL(const NATable *naTable, NAString &catalogNamePart, NAString &schemaNamePart,
    ...
    for (int i = 0; i < partitionArray.entries(); i++) {
      if (partitionArray[i]->hasSubPartition()) {
        ...
        for (int j = 0; j < subpArray->entries(); j++) {
          QualifiedName qn((*subpArray)[j]->getPartitionEntityName(), schemaNamePart, catalogNamePart);

          retcode =
              lockObjectDDL(qn, COM_BASE_TABLE_OBJECT, naTable->isVolatileTable(), naTable->isTrafExternalTable());
          if (retcode < 0) return retcode;
        }
      } else {
        QualifiedName qn(partitionArray[i]->getPartitionEntityName(), schemaNamePart, catalogNamePart);

        retcode = lockObjectDDL(qn, COM_BASE_TABLE_OBJECT, naTable->isVolatileTable(), naTable->isTrafExternalTable());
        if (retcode < 0) return retcode;
      }
    }
  }
...
// ----------------------------------------------------------------------------
void CmpSeabaseDDL::renameSeabaseTable(StmtDDLAlterTableRename *renameTableNode, NAString &currCatName,
                                       NAString &currSchName) {
  int retcode = 0;
  int cliRC = 0;
  ...
  CorrName cn;
  retcode = setupAndErrorChecks(tabName, renameTableNode->getOrigTableNameAsQualifiedName(), currCatName, currSchName,
                                catalogNamePart, schemaNamePart, objectNamePart, extTableName, extNameForHbase, cn,
                                &naTable, FALSE, FALSE, &cliInterface, COM_BASE_TABLE_OBJECT, SQLOperation::ALTER_TABLE,
                                FALSE, TRUE /*process hiatus*/);
  if (retcode < 0) {
    processReturn();
  ...
  const NAString newExtNameForHbase =
      genHBaseObjName(catalogNamePart, schemaNamePart, newObjectNamePart, naTable->getNamespace());

  CorrName newcn(newObjectNamePart, STMTHEAP, schemaNamePart, catalogNamePart);

  retcode = lockObjectDDL(newcn.getQualifiedNameObj(), COM_BASE_TABLE_OBJECT, renameTableNode->isVolatile(),
  ...
  if (naTable && naTable->isStoredDesc()) {
    ...
    TrafDesc *trafDesc = ActiveSchemaDB()->getNATableDB()->getTableDescFromCacheOrText(
        naTable->getExtendedQualName(), naTable->objectUid().get_value(), &descHeap);
    ...
    if (statsDesc) hasStatsDesc = TRUE;
    if (trafDesc) {
  ...
  if ((isSeabasePrivMgrMD(catalogNamePart, schemaNamePart)) && (Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL))) {
    retcode = existsInSeabaseMDTable(&cliInterface, catalogNamePart, schemaNamePart, newObjectNamePart,
    ...
    if (retcode == 1) {
      ...
      *CmpCommon::diags() << DgSqlCode(-1390) << DgString0(newExtTableName);
    ...
    if (newNaTable != NULL && (NOT bindWA.errStatus())) {
      ...
      *CmpCommon::diags() << DgSqlCode(-1390) << DgString0(newExtTableName);
      ...
      return;
    } else if (newNaTable == NULL && bindWA.errStatus() &&
      ...
      for (CollIndex i = CmpCommon::diags()->returnIndex(-4082); i != NULL_COLL_INDEX;
  ...
  if (naTable->getViewText()) {
    *CmpCommon::diags() << DgSqlCode(-1427) << DgString0("Reason: Operation not allowed on a view.");
  ...
  if (renameTableNode->isCascade()) {
    *CmpCommon::diags() << DgSqlCode(-1427) << DgString0("Reason: Cascade option not supported.");
  ...
  if (checkList.entries() > 0) {
    *CmpCommon::diags() << DgSqlCode(-1427)
                        << DgString0(
                               "Reason: Operation not allowed if check constraints are present. Drop the constraints "
                               "and recreate them after rename.");
  ...
  for (int i = 0; i < uniqueList.entries(); i++) {
    ...
    if (uniqConstr->hasRefConstraintsReferencingMe()) {
      *CmpCommon::diags() << DgSqlCode(-1427)
                          << DgString0(
                                 "Reason: Operation not allowed if there are referential constraint referencing this "
                                 "table. Drop the referential constraints and recreate them after rename.");
    ...
    if (naTable->getUniqueConstraints().entries() > 0) {
      for (int i = 0; i < uniqueList.entries(); i++) {
        ...
        if ((constrName.length() >= (strlen(TRAF_UNNAMED_CONSTR_PREFIX))) &&
          ...
          sprintf(buf, "ALTER TABLE %s DROP CONSTRAINT %s", ansiTableName.data(), ansiConstrName.data());
          ...
          int constrNum = extractNumVal(constrName);
          if (constrNum <= 0) {
            // internal error. Assert.
            // Should DDL lock be removed here?
            CMPASSERT(0);
          ...
          NAString stringToSearch = NAString("\"") + NAString(TRAF_UNNAMED_CONSTR_PREFIX) +
                                    str_itoa(constrNum, constrNumBuf) + NAString("_") + objectNamePart + NAString("\"");
          NAString stringToReplace = NAString("\"") + NAString(TRAF_UNNAMED_CONSTR_PREFIX) +
                                     str_itoa(constrNum, constrNumBuf) + NAString("_") + newObjectNamePart +
                                     NAString("\"");

          size_t pos = nas.index(stringToSearch.data(), stringToSearch.length(), 0, NAString::exact);
          if (pos == NA_NPOS) {
            // internal error. Assert.
            // Should DDL lock be removed here?
            CMPASSERT(0);
          ...
          nas.replace(pos, stringToSearch.length(), stringToReplace, stringToReplace.length());
  ...
  if (!renameTableNode->skipViewCheck()) {
    ...
    if (usingViewsQueue->numEntries() > 0) {
      *CmpCommon::diags() << DgSqlCode(-1427)
                          << DgString0(
                                 "Reason: Operation not allowed if dependent views exist. Drop the views and recreate "
                                 "them after rename.");
  ...
  if (!renameTableNode->skipTransactionCheck()) {
    if (xnInProgress(&cliInterface) || (!GetCliGlobals()->currContext()->getTransaction()->autoCommit())) {
      *CmpCommon::diags() << DgSqlCode(-20125) << DgString0("This ALTER");
  ...
  if (!USE_UUID_AS_HBASE_TABLENAME && NOT naTable->isPartitionV2Table()) {
    // drop constraints
    for (int i = 0; i < dropConstrList.entries(); i++) {
    ...
    // add constraints
    for (int i = 0; i < addConstrList.entries(); i++) {
  ...
  cliRC =
      updateObjectName(&cliInterface, objUID, catalogNamePart.data(), schemaNamePart.data(), newObjectNamePart.data());
  if (cliRC < 0) {
    processReturn();
    goto label_error_2;
  }
  ...
  if (naTable->isPartitionV2Table()) {
    ...
    for (int i = 0; i < naPartArry.entries(); i++) {
      if (naPartArry[i]->hasSubPartition()) {
        ...
        retcode = lockObjectDDL(partCn.getQualifiedNameObj(), COM_BASE_TABLE_OBJECT, partCn.isVolatile(),
                                partCn.isExternal());
        if (retcode < 0) {
          processReturn();
          return;
        ...
        long partObjUID = getObjectUID(&cliInterface, catalogNamePart.data(), schemaNamePart.data(),
                                       oldPartEntityName.data(), COM_BASE_TABLE_OBJECT_LIT);
        if (partObjUID < 0) {
          processReturn();
          goto label_error_2;
        }
        cliRC = updateObjectName(&cliInterface, partObjUID, catalogNamePart.data(), schemaNamePart.data(),
                                 newPartEntityName);
        if (cliRC < 0) {
          processReturn();
          goto label_error_2;
        ...
        str_sprintf(
            buf,
            "update %s.\"%s\".%s set partition_entity_name = '%s' where parent_uid = %ld and partition_name = '%s'",
            getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_PARTITIONS, newPartEntityName, naPartArry[i]->getParentUID(),
            naPartArry[i]->getPartitionName());
        cliRC = cliInterface.executeImmediate(buf);
        if (cliRC < 0) {
          cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
          processReturn();
        ...
        if (!USE_UUID_AS_HBASE_TABLENAME) {
          oldExtPartNameForHbase =
              genHBaseObjName(catalogNamePart, schemaNamePart, oldPartEntityName, naTable->getNamespace());
          hbasePartTable.val = (char *)oldExtPartNameForHbase.data();
          hbasePartTable.len = oldExtPartNameForHbase.length();
          newExtPartNameForHbase =
              genHBaseObjName(catalogNamePart, schemaNamePart, newPartEntityName, naTable->getNamespace());
          ...
          if (retcode < 0) {
            *CmpCommon::diags() << DgSqlCode(-8448) << DgString0((char *)"ExpHbaseInterface::copy()")
                                << DgString1(getHbaseErrStr(-retcode)) << DgInt0(-retcode)
        ...
        cliRC = updateObjectRedefTime(&cliInterface, catalogNamePart, schemaNamePart, newPartEntityName,
                                      COM_BASE_TABLE_OBJECT_LIT, -1, partObjUID, naTable->isStoredDesc(),
                                      hasStatsDesc == FALSE ? naTable->storedStatsAvail() : TRUE);
        if (cliRC < 0) {
          processReturn();
          goto label_error;
        }
        ...
        if (naTable->isStoredDesc() && isSeabase(catalogNamePart) &&
            (!Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL))) {
          // remove old name
          insertIntoSharedCacheList(catalogNamePart, schemaNamePart, oldPartEntityName, COM_BASE_TABLE_OBJECT,
                                    SharedCacheDDLInfo::DDL_DELETE);

          // add new name
          insertIntoSharedCacheList(catalogNamePart, schemaNamePart, newPartEntityName, COM_BASE_TABLE_OBJECT,
                                    SharedCacheDDLInfo::DDL_INSERT);
        }
        // 5. remove natable
        ActiveSchemaDB()->getNATableDB()->removeNATable(partCn, ComQiScope::REMOVE_FROM_ALL_USERS,
                                                        COM_BASE_TABLE_OBJECT, renameTableNode->ddlXns(), FALSE);
      }
    }
  }
  ...
  if (!USE_UUID_AS_HBASE_TABLENAME && NOT naTable->isPartitionV2Table()) {
    ...
    if (found) {
      ...
      NAString newSeqName;
      SequenceGeneratorAttributes::genSequenceName(catalogNamePart, schemaNamePart, newObjectNamePart,
                                                   col->getColName(), newSeqName);

      long seqUID = getObjectUID(&cliInterface, catalogNamePart.data(), schemaNamePart.data(), oldSeqName.data(),
      ...
      cliRC = updateObjectName(&cliInterface, seqUID, catalogNamePart.data(), schemaNamePart.data(), newSeqName.data());
      if (cliRC < 0) {
        processReturn();
        goto label_error_2;
      }
  ...
  if (lockRequired(ehi, const_cast<NATable *>(naTable), catalogNamePart, schemaNamePart, objectNamePart,
                   HBaseLockMode::LOCK_X, FALSE /* useHbaseXN */,
                   FALSE
                   /* replSync */,
                   FALSE /* incrementalBackup */, FALSE /* asyncOperation */, TRUE /* noConflictCheck */,
                   TRUE /* registerRegion */)) {
    processReturn();
    cliRC = -1;
    goto label_error_2;
  ...
  if (naTable->incrBackupEnabled()) {
    ...
    cliRC = deleteFromXDCDDLTable(&cliInterface, catalogNamePart.data(), schemaNamePart.data(), objectNamePart.data(),
                                  COM_BASE_TABLE_OBJECT_LIT);
  ...
  if (!ComIsTrafodionReservedSchemaName(schemaNamePart) && naTable->isSeabaseTable() == TRUE) {
    ...
    if (retcode < 0) {
      *CmpCommon::diags() << DgSqlCode(-3242) << DgString0("Cannot getTableDefforBinlog.");
      processReturn();
      return;
    }
  ...
  cliRC = updateObjectRedefTime(&cliInterface, catalogNamePart, schemaNamePart, newObjectNamePart,
                                COM_BASE_TABLE_OBJECT_LIT, -1, objUID, naTable->isStoredDesc(),
                                hasStatsDesc == FALSE ? naTable->storedStatsAvail() : TRUE);
  if (cliRC < 0) {
    processReturn();
    goto label_error;
  ...
  if (naTable->isStoredDesc() && isSeabase(catalogNamePart) && (!Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL))) {
    // remove old name
    insertIntoSharedCacheList(catalogNamePart, schemaNamePart, objectNamePart, COM_BASE_TABLE_OBJECT,
                              SharedCacheDDLInfo::DDL_DELETE);

    // add new name
    insertIntoSharedCacheList(catalogNamePart, schemaNamePart, newObjectNamePart, COM_BASE_TABLE_OBJECT,
                              SharedCacheDDLInfo::DDL_INSERT);
  }
  ...
  if (createSnapshotForIncrBackup(&cliInterface, naTable->incrBackupEnabled(), naTable->getNamespace(), catalogNamePart,
                                  schemaNamePart, newObjectNamePart, naTable->objDataUID().castToInt64())) {
    processReturn();
    cliRC = -1;
  }
...
void CmpSeabaseDDL::alterSeabaseTableRenamePartition(StmtDDLAlterTableRenamePartition *alterTableRenamePartNode,
  ...
  // error check
  retcode = setupAndErrorChecks(tabName, alterTableRenamePartNode->getOrigTableNameAsQualifiedName(), currCatName,
                                currSchName, catalogNamePart, schemaNamePart, objectNamePart, extTableName,
                                extNameForHbase, cn, &naTable, FALSE, FALSE, &cliInterface, COM_BASE_TABLE_OBJECT,
                                SQLOperation::ALTER_TABLE, FALSE, TRUE /*process hiatus*/);
  if (retcode < 0) {
    processReturn();
    return;
  }
  ...
  long objectUid = naTable->objectUid().castToInt64();
  ...
  if (oldPtNm == newPtNm) {
    *CmpCommon::diags() << DgSqlCode(-8307) << DgString0(newPtNm) << DgString1("the old partition name:");
    processReturn();
    return;
  }

  if (isExistsPartitionName(naTable, newPtNm)) {
    *CmpCommon::diags() << DgSqlCode(-8307) << DgString0(newPtNm)
  ...
  CorrName cnPt(NAString(oldFullPartName), STMTHEAP, schemaNamePart, catalogNamePart);
  ...
  cliRC = updateTablePartitionsPartitionName(&cliInterface, currCatName, currSchName, oldPtNm, newPtNm, objectUid);
  if (cliRC < 0) {
    processReturn();
    goto label_error;
  }
  ...
  ActiveSchemaDB()->getNATableDB()->removeNATable(cnPt, ComQiScope::REMOVE_FROM_ALL_USERS, COM_BASE_TABLE_OBJECT,
                                                  alterTableRenamePartNode->ddlXns(), FALSE);

  setCacheAndStoredDescOp(op, UPDATE_REDEF_TIME);
  setCacheAndStoredDescOp(op, REMOVE_NATABLE);
  if (naTable->incrBackupEnabled()) setCacheAndStoredDescOp(op, UPDATE_XDC_DDL_TABLE);
  if (naTable->isStoredDesc()) setCacheAndStoredDescOp(op, UPDATE_SHARED_CACHE);

  cliRC =
      updateCachesAndStoredDesc(&cliInterface, catalogNamePart, schemaNamePart, objectNamePart, COM_BASE_TABLE_OBJECT,
                                op, &removeNATableParam, &updObjRefParam, SharedCacheDDLInfo::DDL_UPDATE);
  if (cliRC < 0) {
    cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
    goto label_rollback;
  }
...
#define doQueryInTX                                                 \
  ...
  // error check
  cliRC = setupAndErrorChecks(tabName, alterTableSplitPartNode->getOrigTableNameAsQualifiedName(), currCatName,
                              currSchName, catalogNamePart, schemaNamePart, objectNamePart, extTableName,
                              extNameForHbase, cn, &naTable, FALSE, FALSE, &cliInterface, COM_BASE_TABLE_OBJECT,
                              SQLOperation::ALTER_TABLE, FALSE, TRUE /*process hiatus*/);
  if (cliRC < 0) {
  ...
  long baseObjUID = naTable->objectUid().castToInt64();
  ...
  int partPosition = naPartition->getPartPosition();
  ...
  for (int i = 0; i < naPartition->getBoundaryValueList().entries(); i++) {
    ...
    if (i != naPartition->getBoundaryValueList().entries() - 1) partKey += ",";
  }
  ...
  cliInterface.holdAndSetCQD("TRAF_PARTITIONV2_CONSTRAINT_RAISE_ERROR", "OFF");
  {
    ...
    // create a new table
    queryStr.format("create partition table %s.\"%s\".%s like %s.\"%s\".%s;", catalogNamePart.data(),
                    schemaNamePart.data(), ToAnsiIdentifier(partitionTableName.data()).data(), catalogNamePart.data(),
                    schemaNamePart.data(), ToAnsiIdentifier(objectNamePart).data());

    doQueryInTX;

    partitionUID = getObjectUID(&cliInterface, catalogNamePart.data(), schemaNamePart.data(), partitionTableName.data(),
                                COM_BASE_TABLE_OBJECT_LIT);
    ...
    queryStr.format("upsert using load into %s.\"%s\".\"%s\" select * from %s.\"%s\".\"%s\" where (%s) %s (%s)",
                    catalogNamePart.data(), schemaNamePart.data(), partitionTableName.data(), catalogNamePart.data(),
                    schemaNamePart.data(), partEntityName.data(), PKcolName.data(), (baseID == 0 ? "<" : ">="),
                    splitkey.data());
    doQueryInTX;

    // Delete data that does not meet the criteria from the original table
    queryStr.format("delete from %s.\"%s\".\"%s\" where (%s) %s (%s)", catalogNamePart.data(), schemaNamePart.data(),
                    partEntityName.data(), PKcolName.data(), (baseID == 0 ? "<" : ">="), splitkey.data());
    doQueryInTX;
    ...
    queryStr.format(
        ...
        getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_PARTITIONS, (baseID == 0 ? "= " : " "), partPosition,
        baseObjUID);
    doQueryInTX;
    ...
    if (SPPOSITION[0] == SPPOSITION[1]) {
      queryStr.format("update %s.\"%s\".%s set PARTITION_NAME = '%s' where PARTITION_NAME = '%s' and PARENT_UID = %ld;",
                      getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_PARTITIONS, (*splitedPartNameList)[1].data(),
                      naPartition->getPartitionName(), baseObjUID);
      doQueryInTX;
    } else if (SPPOSITION[0] == 1) {
      queryStr.format(
          "update %s.\"%s\".%s set PARTITION_EXPRESSION = '%s' where PARTITION_NAME = '%s' and PARENT_UID = %ld;",
          getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_PARTITIONS, splitkey.data(), naPartition->getPartitionName(),
          baseObjUID);
      doQueryInTX;
    }

    cliRC = updateSeabaseTablePartitions(&cliInterface, baseObjUID, (*splitedPartNameList)[baseID].data(),
                                         partitionTableName.data(), partitionUID, COM_PRIMARY_PARTITION_LIT,
  ...
  updObjRefParamNewtable =
      UpdateObjRefTimeParam(-1, partitionUID, naTable->isStoredDesc(), naTable->storedStatsAvail());
  updObjRefParamSplitedtable =
      UpdateObjRefTimeParam(-1, naPartition->getPartitionUID(), naTable->isStoredDesc(), naTable->storedStatsAvail());
  ...
  cliRC = updateCachesAndStoredDesc(&cliInterface, catalogNamePart, schemaNamePart, NAString(partitionTableName),
                                    COM_BASE_TABLE_OBJECT, op, &removeNATableParam, &updObjRefParamNewtable,
                                    SharedCacheDDLInfo::DDL_UPDATE);
  if (cliRC < 0) {
    cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
    goto label_rollback;
  ...
  // splited table
  cliRC =
      updateCachesAndStoredDesc(&cliInterface, catalogNamePart, schemaNamePart, partEntityName, COM_BASE_TABLE_OBJECT,
                                op, &removeNATableParam, &updObjRefParamSplitedtable, SharedCacheDDLInfo::DDL_UPDATE);
  if (cliRC < 0) {
    cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
    goto label_rollback;
  }
  ...
  cliRC =
      updateCachesAndStoredDesc(&cliInterface, catalogNamePart, schemaNamePart, objectNamePart, COM_BASE_TABLE_OBJECT,
                                op, &removeNATableParam, &updObjRefParamBasetable, SharedCacheDDLInfo::DDL_UPDATE);
  if (cliRC < 0) {
    cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
    goto label_rollback;
  }
...
// This statement runs in a transaction
// ----------------------------------------------------------------------------
void CmpSeabaseDDL::alterSeabaseTableStoredDesc(StmtDDLAlterTableStoredDesc *alterStoredDesc, NAString &currCatName,
                                                NAString &currSchName) {
  int retcode = 0;
  int cliRC = 0;
  ...
  if (isSchemaObjectName(tabName)) {
    cliRC = alterSchemaTableDesc(&cliInterface, alterStoredDesc->getType(), tabName, alterStoredDesc->ddlXns());
    if (cliRC < 0) {
      processReturn();
    }
    return;
  ...
  CorrName cn;
  retcode =
      setupAndErrorChecks(tabName, alterStoredDesc->getOrigTableNameAsQualifiedName(), currCatName, currSchName,
                          catalogNamePart, schemaNamePart, objectNamePart, extTableName, extNameForHbase, cn, &naTable,
                          FALSE, TRUE, &cliInterface, COM_BASE_TABLE_OBJECT, SQLOperation::ALTER_TABLE, FALSE);
  if (retcode < 0) {
    processReturn();
    return;
  }
  ...
  if (addToSharedCache)
    insertIntoSharedCacheList(catalogNamePart, schemaNamePart, objectNamePart, COM_BASE_TABLE_OBJECT, op);
  ...
  if (naTable->hasSecondaryIndexes())  // user indexes
  {
    ...
    for (int i = 0; i < naFsList.entries(); i++) {
      ...
      if (addToSharedCache)
        insertIntoSharedCacheList(indexName.getCatalogNameAsAnsiString(), indexName.getSchemaNameAsAnsiString(),
                                  indexName.getObjectName(), COM_INDEX_OBJECT, op);
    }
  }
  ...
  if (naTable->isPartitionV2Table() && addToSharedCache) {
    ...
    for (int i = 0; i < partitionArray.entries(); i++) {
      if (partitionArray[i]->hasSubPartition()) {
        const NAPartitionArray *subpArray = partitionArray[i]->getSubPartitions();
        for (int j = 0; j < subpArray->entries(); j++) {
          insertIntoSharedCacheList(catalogNamePart, schemaNamePart, (*subpArray)[j]->getPartitionEntityName(),
                                    COM_BASE_TABLE_OBJECT, op);
        }
      } else {
        insertIntoSharedCacheList(catalogNamePart, schemaNamePart, partitionArray[i]->getPartitionEntityName(),
                                  COM_BASE_TABLE_OBJECT, op);
      }
    }
  }
  ...
  if (alterStoredDesc->getType() == StmtDDLAlterTableStoredDesc::GENERATE_DESC) {
    ...
    if (switchCompiler(CmpContextInfo::CMPCONTEXT_TYPE_META)) return;

    cliRC = updateObjectRedefTime(&cliInterface, catalogNamePart, schemaNamePart, objectNamePart,
                                  COM_BASE_TABLE_OBJECT_LIT, -1, objUID, TRUE, FALSE);
    ...
    if (naTable->hasSecondaryIndexes())  // user indexes
    {
      ...
      for (int i = 0; i < naFsList.entries(); i++) {
        ...
        const QualifiedName &indexName = naFS->getFileSetName();
        cliRC =
            updateObjectRedefTime(&cliInterface, indexName.getCatalogName().data(), indexName.getSchemaName().data(),
                                  indexName.getObjectName().data(), COM_INDEX_OBJECT_LIT, -1, indexUID, TRUE, FALSE);
        if (cliRC < 0) {
          switchBackCompiler();
          processReturn();
    ...
    if (naTable->isPartitionV2Table()) {
      ...
      for (int i = 0; i < partitionArray.entries(); i++) {
        if (partitionArray[i]->hasSubPartition()) {
          const NAPartitionArray *subpArray = partitionArray[i]->getSubPartitions();
          for (int j = 0; j < subpArray->entries(); j++) {
            cliRC = updateObjectRedefTime(&cliInterface, catalogNamePart, schemaNamePart,
                                          (*subpArray)[j]->getPartitionEntityName(), COM_BASE_TABLE_OBJECT_LIT, -1,
                                          (*subpArray)[j]->getPartitionUID(), TRUE, FALSE);
          ...
          cliRC = updateObjectRedefTime(&cliInterface, catalogNamePart, schemaNamePart,
                                        partitionArray[i]->getPartitionEntityName(), COM_BASE_TABLE_OBJECT_LIT, -1,
                                        partitionArray[i]->getPartitionUID(), TRUE, FALSE);
    ...
    cliRC = updateObjectStats(
        &cliInterface, catalogNamePart, schemaNamePart, objectNamePart, COM_BASE_TABLE_OBJECT_LIT,
        (alterStoredDesc->getType() == StmtDDLAlterTableStoredDesc::GENERATE_STATS_INTERNAL
             ? -2
             : -1),  // GENERATE_STATS_INTERNAL don't update redef time
        objUID, TRUE, (alterStoredDesc->getType() == StmtDDLAlterTableStoredDesc::GENERATE_STATS_FORCE ? TRUE : FALSE));
    ...
    if (naTable->hasSecondaryIndexes())  // user indexes
    {
      ...
      for (int i = 0; i < naFsList.entries(); i++) {
        ...
        const QualifiedName &indexName = naFS->getFileSetName();
        cliRC = updateObjectStats(
            &cliInterface, indexName.getCatalogName().data(), indexName.getSchemaName().data(),
            indexName.getObjectName().data(), COM_INDEX_OBJECT_LIT, -1, indexUID, TRUE,
    ...
    if (naTable->isPartitionV2Table()) {
      ...
      for (int i = 0; i < partitionArray.entries(); i++) {
        if (partitionArray[i]->hasSubPartition()) {
          ...
          for (int j = 0; j < subpArray->entries(); j++) {
            cliRC = updateObjectStats(
                &cliInterface, catalogNamePart, schemaNamePart, (*subpArray)[j]->getPartitionEntityName(),
                COM_BASE_TABLE_OBJECT_LIT,
          ...
          cliRC = updateObjectStats(
              &cliInterface, catalogNamePart, schemaNamePart, partitionArray[i]->getPartitionEntityName(),
              COM_BASE_TABLE_OBJECT_LIT,
              (alterStoredDesc->getType() == StmtDDLAlterTableStoredDesc::GENERATE_STATS_INTERNAL ? -2 : -1),
    ...
    if (!dlockForStoredDesc_->lockHeld()) {
      *CmpCommon::diags() << DgSqlCode(-EXE_OEC_UNABLE_TO_GET_DIST_LOCK) << DgString0(dLockName.data());
      return;
    }
  } else if (alterStoredDesc->getType() == StmtDDLAlterTableStoredDesc::DELETE_DESC) {
    cliRC = deleteFromTextTable(&cliInterface, objUID, COM_STORED_DESC_TEXT, 0);
    if (cliRC < 0) {
      processReturn();
      return;
    }
    ...
    if (naTable->hasSecondaryIndexes())  // user indexes
    {
      ...
      for (int i = 0; i < naFsList.entries(); i++) {
        ...
        long indexUID = naFS->getIndexUID();
        cliRC = deleteFromTextTable(&cliInterface, indexUID, COM_STORED_DESC_TEXT, 0);
        if (cliRC < 0) {
          processReturn();
          return;
        }
    ...
    if (naTable->isPartitionV2Table()) {
      ...
      for (int i = 0; i < partitionArray.entries(); i++) {
        if (partitionArray[i]->hasSubPartition()) {
          const NAPartitionArray *subpArray = partitionArray[i]->getSubPartitions();
          for (int j = 0; j < subpArray->entries(); j++) {
            cliRC = deleteFromTextTable(&cliInterface, (*subpArray)[j]->getPartitionUID(), COM_STORED_DESC_TEXT, 0);
            if (cliRC < 0) {
              processReturn();
              return;
            }
          ...
          cliRC = deleteFromTextTable(&cliInterface, partitionArray[i]->getPartitionUID(), COM_STORED_DESC_TEXT, 0);
          if (cliRC < 0) {
            processReturn();
            return;
          }
      ...
      for (int i = 0; i < partitionArray.entries(); i++) {
        if (partitionArray[i]->hasSubPartition()) {
          ...
          for (int j = 0; j < subpArray->entries(); j++) {
            ...
            ActiveSchemaDB()->getNATableDB()->removeNATable(pcn, ComQiScope::REMOVE_FROM_ALL_USERS,
                                                            COM_BASE_TABLE_OBJECT, alterStoredDesc->ddlXns(), FALSE);
          }
        } else {
...
// ----------------------------------------------------------------------------
void CmpSeabaseDDL::alterSeabaseTableHBaseOptions(StmtDDLAlterTableHBaseOptions *hbaseOptionsNode,
                                                  NAString &currCatName, NAString &currSchName) {
  int retcode = 0;
  int cliRC = 0;
  ...
  retcode =
      setupAndErrorChecks(tabName, hbaseOptionsNode->getOrigTableNameAsQualifiedName(), currCatName, currSchName,
                          catalogNamePart, schemaNamePart, objectNamePart, extTableName, extNameForHbase, cn, &naTable,
                          FALSE, FALSE, &cliInterface, COM_BASE_TABLE_OBJECT, SQLOperation::ALTER_TABLE);
  if (retcode < 0) {
    processReturn();
    return;
  }
  ...
  if (!isDDLOperationAuthorized(SQLOperation::ALTER_TABLE, naTable->getOwner(), naTable->getSchemaOwner(),
                                (int64_t)naTable->objectUid().get_value(), COM_BASE_TABLE_OBJECT,
                                naTable->getPrivInfo())) {
    *CmpCommon::diags() << DgSqlCode(-CAT_NOT_AUTHORIZED);
  ...
  hbaseTable.len = extNameForHbase.length();
  result =
      alterHbaseTable(ehi, &hbaseTable, naTable->allColFams(), &(edhbo->getHbaseOptions()), hbaseOptionsNode->ddlXns());
  if (result < 0) {
    if (!xnInProgress(&cliInterface)) {
      ddlResetObjectEpochs(TRUE /*doAbort*/, TRUE /*clearList*/);
  ...
  if (naTable->incrBackupEnabled()) {
    result = updateSeabaseXDCDDLTable(&cliInterface, catalogNamePart.data(), schemaNamePart.data(),
                                      objectNamePart.data(), COM_BASE_TABLE_OBJECT_LIT);
    if (result < 0) {
      if (!xnInProgress(&cliInterface)) {
        ddlResetObjectEpochs(TRUE /*doAbort*/, TRUE /*clearList*/);
  ...
  cliRC =
      updateObjectRedefTime(&cliInterface, catalogNamePart, schemaNamePart, objectNamePart, COM_BASE_TABLE_OBJECT_LIT,
                            -1, objUID, naTable->isStoredDesc(), naTable->storedStatsAvail());
  if (cliRC < 0) {
    cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
...
// This statement runs in a transaction
// ----------------------------------------------------------------------------
void CmpSeabaseDDL::alterSeabaseTableAttribute(StmtDDLAlterTableAttribute *alterTableNode, NAString &currCatName,
                                               NAString &currSchName) {
  int retcode = 0;
  int cliRC = 0;
  ...
  if (CmpCommon::context()->sqlSession()->volatileSchemaInUse()) {
    ...
    if (retcode == 1) {
      ...
      *CmpCommon::diags() << DgSqlCode(-1427) << DgString0("Reason: Operation not allowed on volatile tables.");
  ...
  retcode =
      existsInSeabaseMDTable(&cliInterface, catalogNamePart, schemaNamePart, objectNamePart, COM_BASE_TABLE_OBJECT,
  ...
  if ((NOT fileAttrs.isXnReplSpecified()) && (NOT fileAttrs.isIncrBackupSpecified()) &&
      (NOT fileAttrs.isReadOnlySpecified())) {
    *CmpCommon::diags() << DgSqlCode(-1425) << DgString0(extTableName)
  ...
  if (fileAttrs.isReadOnlySpecified()) isAlteringReadOnlyAttr = TRUE;

  if (isAlteringReadOnlyAttr && !naTable->isSeabaseTable()) {
    *CmpCommon::diags() << DgSqlCode(-3242) << DgString0("Can not alter attribute on not a trafodion seabase table");
    processReturn();
    return;
  }

  long objDataUID = naTable->objDataUID().castToInt64();
  ...
  if (naTable && fileAttrs.isIncrBackupSpecified()) {
    hiatusObjectName_ = genHBaseObjName(catalogNamePart.data(), schemaNamePart.data(), objectNamePart.data(),
  ...
  if (fileAttrs.isXnReplSpecified()) {
    ...
    str_sprintf(queryBuf, "select flags from %s.\"%s\".%s where table_uid = %ld", getSystemCatalog(), SEABASE_MD_SCHEMA,
                SEABASE_TABLES, objUID);
    long flags = 0;
    ...
    str_sprintf(queryBuf, "update %s.\"%s\".%s set flags = %ld where table_uid = %ld ", getSystemCatalog(),
                SEABASE_MD_SCHEMA, SEABASE_TABLES, newFlags, objUID);
    ...
    int idenColPos = naTable->getNAColumnArray().getIdentityColumnPosition();
    if (idenColPos >= 0) {
      NAColumn *nac = naTable->getNAColumnArray().getColumn(idenColPos);
      NAString seqName;
      SequenceGeneratorAttributes::genSequenceName(catalogNamePart, schemaNamePart, objectNamePart, nac->getColName(),
                                                   seqName);
      long seqUID = getObjectUID(&cliInterface, catalogNamePart.data(), schemaNamePart.data(), seqName.data(),
                                 COM_SEQUENCE_GENERATOR_OBJECT_LIT);
      ...
      str_sprintf(queryBuf,
                  "update %s.\"%s\".%s set flags = case when %ld != 0 then bitor(flags, %ld) else bitand(flags, "
                  "bitnot(%ld)) end where seq_type = '%s' and seq_uid = (select object_uid from %s.\"%s\".\"%s\" where "
                  "catalog_name = '%s' and schema_name = '%s' and object_name = '%s' and object_type = '%s') ",
                  getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_SEQ_GEN, seqFlags, seqFlags, seqFlags,
                  COM_INTERNAL_SG_LIT, getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_OBJECTS, catalogNamePart.data(),
                  schemaNamePart.data(), seqName.data(), COM_SEQUENCE_GENERATOR_OBJECT_LIT);
      cliRC = cliInterface.executeImmediate(queryBuf);
      if (cliRC < 0) {
        cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
        processReturn();
  ...
  if (fileAttrs.isIncrBackupSpecified() || isAlteringReadOnlyAttr) {
    // get current table flags
    ...
    if (fileAttrs.isIncrBackupSpecified()) {
      ...
      if (fileAttrs.incrBackupEnabled()) CmpSeabaseDDL::setMDflags(newFlags, MD_OBJECTS_INCR_BACKUP_ENABLED);
    }
    ...
    str_sprintf(queryBuf, "update %s.\"%s\".%s set flags = %ld where object_uid = %ld ", getSystemCatalog(),
                SEABASE_MD_SCHEMA, SEABASE_OBJECTS, newFlags, objUID);
    ...
    if (alterReadonlyOP != 0)
      insertIntoSharedCacheList(
          catalogNamePart, schemaNamePart, objectNamePart, COM_BASE_TABLE_OBJECT,
  ...
  if (naTable->hasSecondaryIndexes()) {
    ...
    for (int i = 0; i < naFsList.entries(); i++) {
      ...
      long indexUID = naFS->getIndexUID();
      const QualifiedName &indexName = naFS->getFileSetName();
      retcode = updateObjectRedefTime(&cliInterface, indexName.getCatalogName().data(),
                                      indexName.getSchemaName().data(), indexName.getObjectName().data(),
                                      COM_INDEX_OBJECT_LIT, -1, indexUID, naTable->isStoredDesc(), FALSE);
      if (fileAttrs.isReadOnlySpecified()) {
        CorrName indexCn = CorrName(indexName.getObjectName().data(), STMTHEAP, indexName.getSchemaName().data(),
                                    indexName.getCatalogName().data());
        ActiveSchemaDB()->getNATableDB()->removeNATable(indexCn, ComQiScope::REMOVE_FROM_ALL_USERS, COM_INDEX_OBJECT,
...
short CmpSeabaseDDL::createSeabaseTableLike2(CorrName &cn, const NAString &likeTableName, NABoolean withPartns,
                                             NABoolean withoutSalt, NABoolean withoutDivision,
...
short CmpSeabaseDDL::addIBAttrForTable(const NATable *naTable, ExeCliInterface *cliInterface) {
  int retcode = 0;
  int cliRC = 0;
...
NABoolean CmpSeabaseDDL::isTableHasIBAttr(const NATable *naTable, ExpHbaseInterface *ehi) {
...
short CmpSeabaseDDL::cloneHbaseTable(const NAString &srcTable, const NAString &clonedTable, ExpHbaseInterface *inEHI) {
  int retcode = 0;
...
short CmpSeabaseDDL::cloneSeabaseTable(const NAString &srcTableNameStr, long srcTableUID,
                                       const NAString &clonedTableNameStr, const NATable *naTable,
                                       ExpHbaseInterface *inEHI, ExeCliInterface *cliInterface, NABoolean withCreate) {
  int cliRC = 0;
  int retcode = 0;
  ...
  char buf[2000];
  if (withCreate) {
    retcode = createSeabaseTableLike2(srcCN, clonedTableNameStr, FALSE, FALSE, TRUE, FALSE);
    if (retcode) return -1;

    long clonedTableUID = getObjectUID(cliInterface, clonedCatNamePart.data(), clonedSchNamePart.data(),
                                       clonedObjNamePart.data(), COM_BASE_TABLE_OBJECT_LIT);
    ...
    str_sprintf(buf,
                "merge into %s.\"%s\".%s using (select column_name, column_class from %s.\"%s\".%s where object_uid = "
                "%ld) x on (object_uid = %ld and column_name = x.column_name) when matched then update set "
                "column_class = x.column_class;",
                getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_COLUMNS, getSystemCatalog(), SEABASE_MD_SCHEMA,
                SEABASE_COLUMNS, srcTableUID, clonedTableUID);
    cliRC = cliInterface->executeImmediate(buf);
    if (cliRC < 0) {
      cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
      processReturn();
  ...
  if (NOT withCreate) {
    ...
    if (gEnableRowLevelLock &&
        beginXnIfNotInProgress(cliInterface, xnWasStartedHere, FALSE /*clearDDLList*/, FALSE /*clearObjectLocks*/)) {
      return -1;
    }

    long objDataUID;
    long clonedTableUID = getObjectUID(cliInterface, clonedCatNamePart.data(), clonedSchNamePart.data(),
                                       clonedObjNamePart.data(), COM_BASE_TABLE_OBJECT_LIT, &objDataUID);

    // truncate cloned(tgt) table before upsert
    if (truncateHbaseTable(clonedCatNamePart, clonedSchNamePart, clonedObjNamePart,
                           ((NATable *)naTable)->getNamespace(),
                           (naTable->hasSaltedColumn() || naTable->hasTrafReplicaColumn()),
                           (naTable->xnRepl() == COM_REPL_SYNC), naTable->incrBackupEnabled(), objDataUID, inEHI)) {
      if (gEnableRowLevelLock) {
        endXnIfStartedHere(cliInterface, xnWasStartedHere, -1, FALSE /*modifyEpoch*/, FALSE /*clearObjectLocks*/);
      }
      return -1;
    }
    if (gEnableRowLevelLock &&
  ...
  ToQuotedString(quotedSrcCatName, NAString(srcCN.getQualifiedNameObj().getCatalogName()), FALSE);
  NAString quotedSrcSchName;
  ToQuotedString(quotedSrcSchName, NAString(srcCN.getQualifiedNameObj().getSchemaName()), FALSE);
  NAString quotedSrcObjName;
  ToQuotedString(quotedSrcObjName, NAString(srcCN.getQualifiedNameObj().getObjectName()), FALSE);
  ...
  str_sprintf(buf, "upsert using load into %s select * from %s.\"%s\".\"%s\"", clonedTableNameStr.data(),
              quotedSrcCatName.data(), quotedSrcSchName.data(), quotedSrcObjName.data());
...
// ----------------------------------------------------------------------------
void CmpSeabaseDDL::alterSeabaseTableHDFSCache(StmtDDLAlterTableHDFSCache *alterTableHdfsCache, NAString &currCatName,
                                               NAString &currSchName) {
  int cliRC = 0;
  int retcode = 0;
  ...
  if (retCode == HBC_ERROR_POOL_NOT_EXIST_EXCEPTION) {
    *CmpCommon::diags() << DgSqlCode(-4081) << DgString0(alterTableHdfsCache->poolName());
    processReturn();
    return;
  }
...
short CmpSeabaseDDL::cloneAndTruncateTable(const NATable *naTable,  // IN: source table
  ...
  const NAColumnArray &naColArr = naTable->getNAColumnArray();
  for (int c = 0; c < naColArr.entries(); c++) {
  ...
  if (cloneSeabaseTable(naTable->getTableName().getQualifiedNameAsAnsiString(), naTable->objectUid().castToInt64(),
                        tempTable, naTable, ehi, cliInterface, TRUE)) {
    cliRC = -1;
    goto label_drop;
  }
  ...
  retcode = truncateHbaseTable(catalogNamePart, schemaNamePart, objectNamePart, ((NATable *)naTable)->getNamespace(),
                               naTable->hasSaltedColumn() || naTable->hasTrafReplicaColumn(),
                               (naTable->xnRepl() == COM_REPL_SYNC), naTable->incrBackupEnabled(),
                               naTable->objDataUID().castToInt64(), ehi);
  if (retcode) {
    cliRC = -1;
    if (retcode == -2) {
      goto label_drop;
...
label_restore:
  if (cloneSeabaseTable(tempTable, -1, naTable->getTableName().getQualifiedNameAsAnsiString(), naTable, ehi,
                        cliInterface, FALSE)) {
    cliRC = -1;
    goto label_drop;
  }
...
label_return:
  ...
  if (identityGenAlways) cliInterface->restoreCQD("override_generated_identity_values");
...
void CmpSeabaseDDL::alterTableMountPartition(StmtDDLAlterTableMountPartition *alterMountPartition,
  ...
  retcode = setupAndErrorChecks(tgtPartEntityName, qn, currCatName, currSchName, catalogNamePart, schemaNamePart,
                                objectNamePart, extTableName, extNameForHbase, cnPart, &naTablePart, FALSE, TRUE,
                                &cliInterface, COM_BASE_TABLE_OBJECT, SQLOperation::ALTER_TABLE, FALSE,
                                TRUE /*process hiatus*/);
  if (retcode < 0) return;
  ...
  retcode = setupAndErrorChecks(tabName, alterMountPartition->getOrigTableNameAsQualifiedName(), currCatName,
                                currSchName, catalogNameBase, schemaNameBase, objectNameBase, extTableName,
                                extNameForHbase, cn, &naTableBase, FALSE, TRUE, &cliInterface, COM_BASE_TABLE_OBJECT,
                                SQLOperation::ALTER_TABLE, FALSE, TRUE /*process hiatus*/);
  if (retcode < 0) return;
  ...
  // check constraints
  if (naTablePart->getCheckConstraints().entries() > 0 || naTablePart->getRefConstraints().entries() > 0 ||
  ...
  if (isExistsPartitionName(naTableBase, tgtPartitionName)) {
    *CmpCommon::diags() << DgSqlCode(-8307) << DgString0(tgtPartitionName)
                        << DgString1("that of any other partition of the object");
    goto label_return;
  }
  ...
  for (int i = 0; i < naTableBase->getPartitionColCount(); i++) {
    ...
    pColLength +=
        naTableBase->getNAColumnArray()[naTableBase->getPartitionColIdxArray()[i]]->getType()->getEncodedKeyLength();
  ...
  retcode = validatePartitionByExpr(partitionV2Array, naTableBase->getColumnsDesc(), pColDescs,
                                    naTableBase->getPartitionColCount(), pColLength);
  if (retcode) goto label_return;
  ...
  for (int i = lastNAPartition->getBoundaryValueList().entries() - 1; i >= 0; i--) {
    ...
    if (i != lastNAPartition->getBoundaryValueList().entries() - 1) lastPartExpr += ", ";
  ...
  if (alterMountPartition->getValidation()) {
    ...
    if (unparsed == "MAXVALUE")
      str_sprintf(sqlBuf, "select count(*) from %s where (%s) < (%s);", tgtPartEntityName.data(), colName.data(),
                  unparsedLast.data());
    else
      str_sprintf(sqlBuf, "select count(*) from %s where (%s) >= (%s) or (%s) < (%s);", tgtPartEntityName.data(),
                  colName.data(), unparsed.data(), colName.data(), unparsedLast.data());
  ...
  cliRC = updateObjectFlags(&cliInterface, naTablePart->objectUid().castToInt64(), MD_PARTITION_V2, FALSE);
  if (cliRC < 0) goto label_rollback;

  cliRC = addSeabaseMDPartition(&cliInterface, tgtPartition, catalogNameBase.data(), schemaNameBase.data(),
                                objectNameBase.data(), btUid, naTableBase->FisrtLevelPartitionCount(), partUid,
                                tgtPartitionName);
  if (cliRC < 0) goto label_rollback;

  setCacheAndStoredDescOp(op_part, UPDATE_REDEF_TIME);
  setCacheAndStoredDescOp(op_part, REMOVE_NATABLE);
  if (naTablePart->incrBackupEnabled()) setCacheAndStoredDescOp(op_part, UPDATE_XDC_DDL_TABLE);
  if (naTablePart->isStoredDesc()) setCacheAndStoredDescOp(op_part, UPDATE_SHARED_CACHE);

  cliRC =
      updateCachesAndStoredDesc(&cliInterface, catalogNamePart, schemaNamePart, objectNamePart, COM_BASE_TABLE_OBJECT,
                                op_part, &removeNATableParam, &updObjRefParamPart, SharedCacheDDLInfo::DDL_UPDATE);
  if (cliRC < 0) {
    cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
    goto label_rollback;
  }
  ...
  if (naTableBase->incrBackupEnabled()) setCacheAndStoredDescOp(op_base, UPDATE_XDC_DDL_TABLE);
  if (naTableBase->isStoredDesc()) setCacheAndStoredDescOp(op_base, UPDATE_SHARED_CACHE);

  cliRC =
      updateCachesAndStoredDesc(&cliInterface, catalogNameBase, schemaNameBase, objectNameBase, COM_BASE_TABLE_OBJECT,
                                op_base, &removeNATableParam, &updObjRefParamBase, SharedCacheDDLInfo::DDL_UPDATE);
  if (cliRC < 0) {
    cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
    goto label_rollback;
  }
...
int CmpSeabaseDDL::unmountSeabasePartitionTable(ExeCliInterface *cliInterface, NAPartition *partInfo,
                                                NAString &currCatName, NAString &currSchName, NAString &nameSpace,
                                                ExpHbaseInterface *ehi, NABoolean ddlXns) {
  int cliRC;
  long uuid;
  char sqlBuf[2048];
  UInt32 op = 0;
  ...
  NATable *naTable = bindWA.getNATable(cn);
  if (naTable == NULL) {
    *CmpCommon::diags() << DgSqlCode(-4402) << DgString0(partInfo->getPartitionName());
    return -1;
  }
...
void CmpSeabaseDDL::alterTableUnmountPartition(StmtDDLAlterTableUnmountPartition *alterUnmountPartition,
  ...
  retcode = setupAndErrorChecks(tabName, alterUnmountPartition->getOrigTableNameAsQualifiedName(), currCatName,
                                currSchName, catalogNamePart, schemaNamePart, objectNamePart, extTableName,
                                extNameForHbase, cn, &naTable, FALSE, TRUE, &cliInterface, COM_BASE_TABLE_OBJECT,
                                SQLOperation::ALTER_TABLE, FALSE, TRUE /*process hiatus*/);
  if (retcode < 0) return;
  ...
  int ret = checkPartition(&partArray, tableDesc, bindWA, objectName, dupilcatePartName, &unmountPartEntityNameVec,
                           &partEntityNameMap);
  if (ret == -1) {
    // An internal executor error occurred.
    *CmpCommon::diags() << DgSqlCode(-8001);
  ...
  int firstLevelPartCnt = naTable->FisrtLevelPartitionCount();
  const NAPartitionArray &partitionArray = naTable->getPartitionArray();
  NAString partName;
  NAString partEntityName;
  ...
  for (int i = 0; i < firstLevelPartCnt; ++i) {
  ...
  for (int i = 0; i < partArray.entries(); ++i) {
    ...
    if (partInfoIt == allNAPartitionMap.end() && !part->isPartitionForValues()) {
      // The specified partition partName does not exist.
      *CmpCommon::diags() << DgSqlCode(-1097) << DgString0(part->getPartitionName());
      processReturn();
      return;
    }
  ...
  for (int i = 0; i < unmountNAPartitionVec.size(); ++i) {
    cliRC = unmountSeabasePartitionTable(&cliInterface, unmountNAPartitionVec[i], currCatName, currSchName, nameSpace,
                                         ehi, alterUnmountPartition->ddlXns());
    if (cliRC < 0) {
      cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
      goto label_rollback;
    }
...
// DDL lock.
// ----------------------------------------------------------------------------
void CmpSeabaseDDL::alterTableAddPartition(StmtDDLAlterTableAddPartition *alterAddPartition, NAString &currCatName,
                                           NAString &currSchName) {
  int cliRC = 0;
  int retcode = 0;
  NABoolean xnWasStartedHere = FALSE;
  ...
  NAString extNameForHbase;
  CorrName cn;
  retcode = setupAndErrorChecks(tabName, alterAddPartition->getOrigTableNameAsQualifiedName(), currCatName, currSchName,
                                catalogNamePart, schemaNamePart, objectNamePart, extTableName, extNameForHbase, cn,
                                &naTable, FALSE, TRUE, &cliInterface, COM_BASE_TABLE_OBJECT, SQLOperation::ALTER_TABLE,
                                FALSE, TRUE /*process hiatus*/);
  if (retcode < 0) return;
  ...
  long btUid = naTable->objectUid().castToInt64();
  if (isExistsPartitionName(naTable, partitionNameList)) {
    *CmpCommon::diags() << DgSqlCode(-8307) << DgString0(partitionNames)
                        << DgString1("that of any other partition of the object");
    return;
  }
  ...
  for (int i = 0; i < naTable->getPartitionColCount(); i++) {
    ...
    pColLength += naTable->getNAColumnArray()[naTable->getPartitionColIdxArray()[i]]->getType()->getEncodedKeyLength();
  ...
  NAHashDictionary<NAString, int> *partitionNameMap =
      new (STMTHEAP) NAHashDictionary<NAString, int>(NAString::hash, 101, TRUE, STMTHEAP);
  retcode = validatePartitionByExpr(partitionV2Array, naTable->getColumnsDesc(), pColDescs,
                                    naTable->getPartitionColCount(), pColLength, partitionNameMap);
  if (retcode) return;

  // new partition bound must be higher than that of the last partition
  NAString err("");
  int firstLevelPartitionCnt = naTable->FisrtLevelPartitionCount();
  ...
  for (int i = lastNAPartition->getBoundaryValueList().entries() - 1; i >= 0; i--) {
    ...
    if (i != 0) lastPartitionExpr += ",";
  }
  ItemExpr *lastPartitionValue = parser.getItemExprTree(lastPartitionExpr.data(), lastPartitionExpr.length());
  ElemDDLPartitionV2 *lastPartition = new (PARSERHEAP()) ElemDDLPartitionV2(lastPartitionName, lastPartitionValue);
  retcode = lastPartition->replaceWithConstValue();
  if (retcode) return;
  if (lastPartition->buildPartitionValueArray(false) < 0) return;
  ...
  for (int i = 1; i < partitionV2Array->entries(); i++) {
    ...
    retcode =
        createSeabasePartitionTable(&cliInterface, partitionName, catalogNamePart, schemaNamePart, objectNamePart);
    if (retcode < 0) {
      cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
      goto label_rollback;
    }

    cliRC = addSeabaseMDPartition(&cliInterface, tgtPartition, catalogNamePart.data(), schemaNamePart.data(),
...
void CmpSeabaseDDL::alterTableMergePartition(StmtDDLAlterTableMergePartition *mergePartition, NAString &currCatName,
                                             NAString &currSchName) {
  int cliRC = 0;
  int retcode = 0;
  long defTime = NA_JulianTimestamp();
  long flags = 0;
  ...
  retcode = setupAndErrorChecks(tabName, mergePartition->getOrigTableNameAsQualifiedName(), currCatName, currSchName,
                                catalogNamePart, schemaNamePart, objectNamePart, extTableName, extNameForHbase, cn,
                                &naTable, FALSE, TRUE, &cliInterface, COM_BASE_TABLE_OBJECT, SQLOperation::ALTER_TABLE,
                                FALSE, TRUE /*process hiatus*/);
  if (retcode < 0) {
  ...
  int isCommit = 0;  // 0 is commit, -1 is rollback
  ...
  if (tgtPartInfo != NULL) {
    ...
    if (tgtPartPosition < 0 && tgtPartInfo->getPartPosition() != sortedSrcPart[0]->getPartPosition() - 1 &&
        tgtPartInfo->getPartPosition() != sortedSrcPart[sortedSrcPart.entries() - 1]->getPartPosition() + 1) {
  ...
  if (sortedSrcPart.entries() == 1 && tgtPartInfo != NULL &&
  ...
  getPartitionTableName(tgtPartTableName, sizeof(tgtPartTableName), objectNamePart, tgtPartName);
  ...
  RemoveNATableParam removeNATableParam(ComQiScope::REMOVE_FROM_ALL_USERS, mergePartition->ddlXns(), FALSE);

  // do merge 01, prepare target partition
  if (tgtPartInfo == NULL) {
    // create tgt partition table if not exist
    retcode = createSeabasePartitionTable(&cliInterface, tgtPartName, catalogNamePart, schemaNamePart, objectNamePart);
    if (retcode < 0) {
      cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
      goto label_rollback;
    }
    ...
    snprintf(sqlBuf, sizeof(sqlBuf),
             "upsert into %s.\"%s\".%s values (%ld, '%s', '%s', %ld, '%s', %d, "
             " cast(? as char(%ld bytes) character set utf8 not null), '%s', "
             " '%s', '%s', %ld, %ld, '%s', '%s', '%s', '%s');",
             getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_PARTITIONS, baseObjUID, tgtPartName.data(),
             tgtPartTableName, tgtPartUID, COM_PRIMARY_PARTITION_LIT, newTgtPartTmpPosition, strlen(tgtPartValueStr),
             COM_YES_LIT, COM_NO_LIT, COM_NO_LIT, defTime, flags, dummyChar, dummyChar, dummyChar, dummyChar);
    cliRC = cliInterface.executeImmediateCEFC(sqlBuf, CONST_CAST(char *, tgtPartValueStr), str_len(tgtPartValueStr),
                                              NULL, NULL, NULL);
    if (cliRC < 0) {
      cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
      goto label_rollback;
    ...
    char addStr[100];
    str_itoa((long)newClonePart, addStr);
    ...
    *newClonePart += addStr;
    ...
    // do clone manually
    cliRC = createSeabaseTableLike2(tgtCN, *newClonePart, FALSE, FALSE, TRUE, FALSE);
    if (cliRC < 0) {
      cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
      goto label_rollback;
    }
    ...
    str_sprintf(sqlBuf, "upsert using load into %s select * from %s.\"%s\".\"%s\"", newClonePart->data(),
  ...
  if (naTgtPart != NULL && naTgtPart->hasSecondaryIndexes()) {
    snprintf(sqlBuf, sizeof(sqlBuf), "alter table %s.%s.%s disable all indexes;", catalogNamePart.data(),
             schemaNamePart.data(), tgtPartTableName);
    cliRC = cliInterface.executeImmediate(sqlBuf);
    if (cliRC < 0) {
      cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
      goto label_rollback;
  ...
  for (int idx = 0; idx < sortedSrcPart.entries(); idx++) {
    ...
    if (srcPartInfo->getPartPosition() != tgtPartPosition) {
      snprintf(sqlBuf, sizeof(sqlBuf), "upsert using load into %s.%s.%s select * from %s.%s.%s;",
               catalogNamePart.data(), schemaNamePart.data(), tgtPartTableName, catalogNamePart.data(),
               schemaNamePart.data(), srcPartInfo->getPartitionEntityName());
      cliRC = cliInterface.executeImmediate(sqlBuf);
      if (cliRC < 0) {
        cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
        goto label_rollback;
      ...
      cliRC = dropSeabasePartitionTable(&cliInterface, catalogNamePart, schemaNamePart,
  ...
  if (naTable->partitionType() == RANGE_PARTITION) {
    if (newPartCreated) {
      snprintf(sqlBuf, sizeof(sqlBuf),
               ...
               " where PARTITION_ORDINAL_POSITION>=%d and PARENT_UID=%ld;",
               getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_PARTITIONS, sortedSrcPart[0]->getPartPosition(),
               baseObjUID);
      cliRC = cliInterface.executeImmediate(sqlBuf);
      if (cliRC < 0) {
        cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
      ...
      snprintf(sqlBuf, sizeof(sqlBuf),
               ...
               getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_PARTITIONS, str_len(tgtPartValueStr), baseObjUID,
               tgtPartName.data());
      cliRC = cliInterface.executeImmediateCEFC(sqlBuf, CONST_CAST(char *, tgtPartValueStr), str_len(tgtPartValueStr),
                                                NULL, NULL, NULL);
      if (cliRC < 0) {
  ...
  if (naTgtPart != NULL && naTgtPart->hasSecondaryIndexes()) {
    ...
    for (int i = 0; i < naTgtFsList.entries(); i++) {
      const NAFileSet *naFS = naTgtFsList[i];

      // skip clustering index
      if (naFS->getKeytag() == 0) continue;

      snprintf(sqlBuf, sizeof(sqlBuf), "POPULATE INDEX %s ON %s.%s.%s;", naFS->getFileSetName().getObjectName().data(),
               catalogNamePart.data(), schemaNamePart.data(), tgtPartTableName);
      cliRC = cliInterface.executeImmediate(sqlBuf);
      if (cliRC < 0) {
        cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
    ...
    for (int i = 0; i < naFsList.entries(); i++) {
      ...
      for (int j = 0; j < naIndexColArr.entries(); j++) {
      ...
      snprintf(sqlBuf, sizeof(sqlBuf),
               "create index INDEX_%s_%s on "
               " %s.%s.%s(%s);",
               naFS->getFileSetName().getObjectName().data(), tgtPartTableName, catalogNamePart.data(),
               schemaNamePart.data(), tgtPartTableName, idxKeys.data());
      cliRC = cliInterface.executeImmediate(sqlBuf);
      if (cliRC < 0) {
        cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
        goto label_rollback;
  ...
  cliRC = updateCachesAndStoredDescByNATable(&cliInterface, catalogNamePart, schemaNamePart, tgtPartTableName,
                                             COM_BASE_TABLE_OBJECT, UPDATE_REDEF_TIME | REMOVE_NATABLE, naTgtPart,
                                             &removeNATableParam, SharedCacheDDLInfo::DDL_UPDATE);
  if (cliRC < 0) {
    cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
    goto label_rollback;
  }
  cliRC = updateCachesAndStoredDescByNATable(&cliInterface, catalogNamePart, schemaNamePart, objectNamePart,
                                             COM_BASE_TABLE_OBJECT, UPDATE_REDEF_TIME | REMOVE_NATABLE, naTable,
                                             &removeNATableParam, SharedCacheDDLInfo::DDL_UPDATE);
  if (cliRC < 0) {
    cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
    goto label_rollback;
...
label_rollback:
  ...
  if (newCloneCreated) {
    ...
    cliInterface.holdAndSetCQD("ALLOW_TRUNCATE_IN_USER_XN", "ON");
    snprintf(sqlBuf, sizeof(sqlBuf), "truncate %s.%s.%s;", catalogNamePart.data(), schemaNamePart.data(),
             tgtPartTableName);
    cliRC = cliInterface.executeImmediate(sqlBuf);
    if (cliRC < 0) {
      cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
    ...
    snprintf(sqlBuf, sizeof(sqlBuf), "upsert using load into %s.%s.%s select * from %s;", catalogNamePart.data(),
             schemaNamePart.data(), tgtPartTableName, newClonePart->data());
    cliRC = cliInterface.executeImmediate(sqlBuf);
    if (cliRC < 0) {
      cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
    }
...
label_return:
  ...
  endXnIfStartedHere(&cliInterface, xnWasStartedHere, isCommit, FALSE /*modifyEpochs*/, FALSE /*clearObjectLocks*/);

  processReturn();
  return;
}

void CmpSeabaseDDL::alterTableExchangePartition(StmtDDLAlterTableExchangePartition *exchangePartition,
                                                NAString &currCatName, NAString &currSchName) {
  int cliRC = 0;
  int retcode = 0;
  long defTime = NA_JulianTimestamp();
  ...
  NAString exchgTabName = exchangePartition->getExchangeTableName().getQualifiedNameAsAnsiString();
  ...
  retcode = setupAndErrorChecks(tabName, exchangePartition->getOrigTableNameAsQualifiedName(), currCatName, currSchName,
                                catalogNamePart, schemaNamePart, objectNamePart, extTableName, extNameForHbase, cn,
                                &naTable, FALSE, TRUE, &cliInterface, COM_BASE_TABLE_OBJECT, SQLOperation::ALTER_TABLE,
                                FALSE, TRUE /*process hiatus*/);
  if (retcode < 0) {
  ...
  retcode = setupAndErrorChecks(exchgTabName, exchangePartition->getExchangeTableName(), currCatName, currSchName,
                                exCatalogNamePart, exSchemaNamePart, exObjectNamePart, exExtTableName,
                                exExtNameForHbase, exchgCN, &exchgNaTable, FALSE, TRUE, &cliInterface,
                                COM_BASE_TABLE_OBJECT, SQLOperation::ALTER_TABLE, FALSE, TRUE /*process hiatus*/);
  if (retcode < 0) {
    processReturn();
    return;
  }
  ...
  if (exchgNaTable->isPartitionV2Table()) {
    *CmpCommon::diags() << DgSqlCode(-8306)
                        << DgString0("ALTER TABLE EXCHANGE requires a non-partitioned, non-clustered table");
    processReturn();
    return;
  }
  ...
  long exchgTableUID = exchgNaTable->objectUid().get_value();
  ...
  const int *partColPosition = naTable->getPartitionColIdxArray();
  ...
  RemoveNATableParam removeNATableParam(ComQiScope::REMOVE_FROM_ALL_USERS, exchangePartition->ddlXns(), FALSE);
  ...
  if (partValHigh == "MAXVALUE") {
    if (partValLow.length() > 0) {
      str_sprintf(sqlBuf, "select count(1) from %s.%s.%s where (%s) < (%s);", exCatalogNamePart.data(),
                  exSchemaNamePart.data(), exObjectNamePart.data(), partCols.data(), partValLow.data());
    } else {
    ...
    if (partValLow.length() > 0) {
      str_sprintf(sqlBuf, "select count(1) from %s.%s.%s where (%s) < (%s) or (%s) >= (%s);", exCatalogNamePart.data(),
                  exSchemaNamePart.data(), exObjectNamePart.data(), partCols.data(), partValLow.data(), partCols.data(),
                  partValHigh.data());
    } else {
      str_sprintf(sqlBuf, "select count(1) from %s.%s.%s where (%s) >= (%s);", exCatalogNamePart.data(),
                  exSchemaNamePart.data(), exObjectNamePart.data(), partCols.data(), partValHigh.data());
    }
  }
  if (doValidate) {
  ...
  if (naPartTable->hasSecondaryIndexes()) {
    snprintf(sqlBuf, sizeof(sqlBuf), "alter table %s.%s.%s disable all indexes;", catalogNamePart.data(),
             schemaNamePart.data(), partEntityName.data());
    cliRC = cliInterface.executeImmediate(sqlBuf);
    if (cliRC < 0) {
      cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
      goto label_rollback;
  ...
  if (exchgNaTable->hasSecondaryIndexes()) {
    snprintf(sqlBuf, sizeof(sqlBuf), "alter table %s.%s.%s disable all indexes;", exCatalogNamePart.data(),
             exSchemaNamePart.data(), exObjectNamePart.data());
    cliRC = cliInterface.executeImmediate(sqlBuf);
    if (cliRC < 0) {
      cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
      goto label_rollback;
  ...
  if (!USE_UUID_AS_HBASE_TABLENAME) {
    // exchange 01, create clones of both exchanging table and exchanging partition
    {
      ...
      *newClonePart += addStr;
      ...
      str_itoa((long)newCloneTable, addStr);
      ...
      *newCloneTable += addStr;
      ...
      // cloneSeabaseTable() is use CQD OVERRIDE_SYSKEY, may cause NATable inconsistent
      // do clone manually
      cliRC = createSeabaseTableLike2(exchgPartCN, *newClonePart, FALSE, FALSE, TRUE, FALSE);
      if (cliRC < 0) {
        cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
        goto label_rollback;
      }

      cliRC = createSeabaseTableLike2(exchgTableCN, *newCloneTable, FALSE, FALSE, TRUE, FALSE);
      if (cliRC < 0) {
        cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
        goto label_rollback;
      }
      ...
      str_sprintf(sqlBuf, "upsert using load into %s select * from %s.\"%s\".\"%s\"", newCloneTable->data(),
    ...
    // exchange 02, exchange data
    {
      ...
      snprintf(sqlBuf, sizeof(sqlBuf), "truncate %s.%s.%s;", exCatalogNamePart.data(), exSchemaNamePart.data(),
               exObjectNamePart.data());
      cliRC = cliInterface.executeImmediate(sqlBuf);
      if (cliRC < 0) {
        cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
        goto label_rollback;
      ...
      snprintf(sqlBuf, sizeof(sqlBuf), "upsert using load into %s.%s.%s select * from %s;", catalogNamePart.data(),
               schemaNamePart.data(), partEntityName.data(), newCloneTable->data());
      cliRC = cliInterface.executeImmediate(sqlBuf);
      if (cliRC < 0) {
        cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
        goto label_rollback;
      ...
      snprintf(sqlBuf, sizeof(sqlBuf), "upsert using load into %s.%s.%s select * from %s;", exCatalogNamePart.data(),
               exSchemaNamePart.data(), exObjectNamePart.data(), newClonePart->data());
      cliRC = cliInterface.executeImmediate(sqlBuf);
      if (cliRC < 0) {
        cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
        goto label_rollback;
    ...
    long uidPartition = naPartTable->objDataUID().castToInt64();
    ;
    long uidExchgTable = exchgNaTable->objDataUID().castToInt64();
    long rowsAffected = 0;

    str_sprintf(sqlBuf,
                "update %s.\"%s\".%s set DATA_OBJECT_UID=%ld "
                " where CATALOG_NAME='%s' and SCHEMA_NAME='%s' and OBJECT_NAME='%s' "
                " and OBJECT_TYPE='BT'; ",
                getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_OBJECTS, uidExchgTable, catalogNamePart.data(),
                schemaNamePart.data(), partEntityName.data());
    cliRC = cliInterface.executeImmediate(sqlBuf);
    if (cliRC < 0) {
      cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
    ...
    str_sprintf(sqlBuf,
                ...
                " where CATALOG_NAME='%s' and SCHEMA_NAME='%s' and OBJECT_NAME='%s' "
                " and OBJECT_TYPE='BT'; ",
                getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_OBJECTS, uidPartition, exCatalogNamePart.data(),
                exSchemaNamePart.data(), exObjectNamePart.data());
    cliRC = cliInterface.executeImmediate(sqlBuf);
    if (cliRC < 0) {
      cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
  ...
  if (naPartTable->hasSecondaryIndexes()) {
    ...
    for (int i = 0; i < naTgtFsList.entries(); i++) {
      ...
      cliRC = purgedataObjectAfterError(cliInterface, catalogNamePart, schemaNamePart,
                                        naFS->getFileSetName().getObjectName(), COM_INDEX_OBJECT, FALSE);
      if (cliRC < 0) goto label_return;
  ...
  if (exchgNaTable->hasSecondaryIndexes()) {
    ...
    for (int i = 0; i < exchgFsList.entries(); i++) {
      ...
      // truncate index, since "POPULATE INDEX PURGEDATA" is not working here
      cliRC = purgedataObjectAfterError(cliInterface, exCatalogNamePart, exSchemaNamePart,
                                        naFS->getFileSetName().getObjectName(), COM_INDEX_OBJECT, FALSE);
      if (cliRC < 0) goto label_return;
  ...
  cliRC = updateCachesAndStoredDescByNATable(&cliInterface, catalogNamePart, schemaNamePart, partEntityName,
                                             COM_BASE_TABLE_OBJECT, UPDATE_REDEF_TIME | REMOVE_NATABLE, naPartTable,
                                             &removeNATableParam, SharedCacheDDLInfo::DDL_UPDATE);
  if (cliRC < 0) {
    cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
    goto label_rollback;
  ...
  cliRC = updateCachesAndStoredDescByNATable(&cliInterface, exCatalogNamePart, exSchemaNamePart, exObjectNamePart,
                                             COM_BASE_TABLE_OBJECT, UPDATE_REDEF_TIME | REMOVE_NATABLE, exchgNaTable,
                                             &removeNATableParam, SharedCacheDDLInfo::DDL_UPDATE);
  if (cliRC < 0) {
    cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
    goto label_rollback;
...
label_rollback:
  ...
  if (newCloneCreated) {
    ...
    snprintf(sqlBuf, sizeof(sqlBuf), "upsert using load into %s.%s.%s select * from %s;", catalogNamePart.data(),
             schemaNamePart.data(), partEntityName.data(), newClonePart->data());
    cliRC = cliInterface.executeImmediate(sqlBuf);
    if (cliRC < 0) {
      cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
    }
    ...
    snprintf(sqlBuf, sizeof(sqlBuf), "upsert using load into %s.%s.%s select * from %s;", exCatalogNamePart.data(),
             exSchemaNamePart.data(), exObjectNamePart.data(), newCloneTable->data());
    cliRC = cliInterface.executeImmediate(sqlBuf);
    if (cliRC < 0) {
      cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
    }
...
// DDL lock.
// ----------------------------------------------------------------------------
void CmpSeabaseDDL::alterSeabaseTableAddColumn(StmtDDLAlterTableAddColumn *alterAddColNode, NAString &currCatName,
                                               NAString &currSchName) {
  int cliRC = 0;
  int retcode = 0;
  ...
  retcode = setupAndErrorChecks(tabName, alterAddColNode->getOrigTableNameAsQualifiedName(), currCatName, currSchName,
                                catalogNamePart, schemaNamePart, objectNamePart, extTableName, extNameForHbase, cn,
                                &naTable, FALSE, TRUE, &cliInterface, COM_BASE_TABLE_OBJECT, SQLOperation::ALTER_TABLE,
                                hbmapExtTable, TRUE /*process hiatus*/);
  if (retcode < 0) {
    processReturn();
    return;
  }
  ...
  if (pColDef->getIsConstraintNotNullSpecified()) {
    ...
    if ((pDefVal) && (pDefVal->origOpType() != ITM_CURRENT_USER) && (pDefVal->origOpType() != ITM_CURRENT_TIMESTAMP) &&
        (pDefVal->origOpType() != ITM_UNIX_TIMESTAMP) && (pDefVal->origOpType() != ITM_UNIQUE_ID) &&
        (pDefVal->origOpType() != ITM_CAST) && (pDefVal->origOpType() != ITM_CURRENT_CATALOG) &&
  ...
  NAString colFamily;
  NAString colName;
  int datatype, length, precision, scale, dt_start, dt_end, nullable, upshifted;
  ...
  retcode = getColInfo(pColDef,
                       ...
                       colFamily, colName, naTable->isSQLMXAlignedTable(), datatype, length, precision, scale, dt_start,
                       dt_end, upshifted, nullable, charset, colClass, defaultClass, defVal, heading, lobStorage,
                       compDefnStr, hbaseColFlags, colFlags);
  ...
  sprintf(binlogTableDefString, "%d-%d-%d-%s-%d-%d-%s-%s-%d-%d-%d-%s;", datatype, nullable, length, "A", precision,
          scale, charset.data(), stripedstr.data(), dt_start, dt_end, (int)nacolArr.entries(),
          colName.strip(NAString::trailing, ' ').data());
  ...
  if ((CmpCommon::getDefault(TRAF_ALLOW_RESERVED_COLNAMES) == DF_OFF) && (ComTrafReservedColName(colName))) {
    *CmpCommon::diags() << DgSqlCode(-CAT_RESERVED_COLUMN_NAME) << DgString0(colName);
  ...
  if ((naTable->isHbaseMapTable()) && ((!nullable) || (defaultClass != COM_NULL_DEFAULT))) {
    *CmpCommon::diags() << DgSqlCode(-3242)
                        << DgString0(
                               "Added column of an HBase mapped table must be nullable with default value of NULL.");
  ...
  if ((colFamily == SEABASE_DEFAULT_COL_FAMILY) || (naTable->isHbaseMapTable()))
  ...
  if (nacol) {
    ...
    if (NOT alterAddColNode->addIfNotExists()) {
      *CmpCommon::diags() << DgSqlCode(-CAT_DUPLICATE_COLUMNS) << DgColumnName(colName);
    }
    deallocEHI(ehi);
    processReturn();
  ...
  long objUID = naTable->objectUid().castToInt64();
  int newColNum = naTable->getColumnCount();
  for (int cc = nacolArr.entries() - 1; cc >= 0; cc--) {
    const NAColumn *nac = nacolArr[cc];

    if ((NOT naTable->isSQLMXAlignedTable()) && (nac->isComputedColumn())) {
      str_sprintf(
          query,
          "update %s.\"%s\".%s set column_number = column_number + 1 where object_uid = %ld and column_number = %d",
          getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_COLUMNS, objUID, nac->getPosition());
      ...
      str_sprintf(
          query,
          "update %s.\"%s\".%s set column_number = column_number + 1 where object_uid = %ld and column_number = %d",
          getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_KEYS, objUID, nac->getPosition());
      ...
      str_sprintf(
          query, "update %s.\"%s\".%s set sub_id = sub_id + 1 where text_uid = %ld and text_type = %d and sub_id = %d",
          getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_TEXT, objUID, COM_COMPUTED_COL_TEXT, nac->getPosition());
      ...
      if (naTable->hasSecondaryIndexes()) {
        ...
        for (int i = 0; i < naFsList.entries(); i++) {
          ...
          str_sprintf(query,
                      "update %s.\"%s\".%s set column_number = column_number + 1  where column_number = %d and "
                      "object_uid = (select object_uid from %s.\"%s\".%s where catalog_name = '%s' and schema_name = "
                      "'%s' and object_name = '%s' and object_type = 'IX') ",
                      getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_KEYS, nac->getPosition(), getSystemCatalog(),
                      SEABASE_MD_SCHEMA, SEABASE_OBJECTS, indexName.getCatalogName().data(),
                      indexName.getSchemaName().data(), indexName.getObjectName().data());
          cliRC = cliInterface.executeImmediate(query);
          if (cliRC < 0) {
            cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
  ...
  str_sprintf(query,
              "insert into %s.\"%s\".%s values (%ld, '%s', %d, '" COM_ADDED_USER_COLUMN_LIT
              "', %d, '%s', %d, %d, %d, %d, %d, '%s', %d, %d, '%s', %d, '%s', '%s', '%s', '%u', '%s', '%s', %ld )",
              getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_COLUMNS, objUID, col_name, newColNum, datatype,
              getAnsiTypeStrFromFSType(datatype), length, precision, scale, dt_start, dt_end, (upshifted ? "Y" : "N"),
              hbaseColFlags, nullable, (char *)charset.data(), (int)defaultClass,
              (quotedDefVal.isNull()
                   ? " "
                   : quotedDefVal.data()),  // column DEFAULT_VALUE can't be NULL,so we can't use ''(null) here
              (quotedHeading.isNull()
                   ? " "
                   : quotedHeading.data()),  // column COLUMN_HEADING can't be NULL,so we can't use ''(null) here
              trafColFam.data(), maxColQual + 1, COM_UNKNOWN_PARAM_DIRECTION_LIT, "N", colFlags);
  ...
  if (addFam) {
    ...
    if (getTextFromMD(&cliInterface, objUID, COM_HBASE_COL_FAMILY_TEXT, 0, currColFams)) {
      deallocEHI(ehi);
      processReturn();
      return;
    }

    int cliRC = deleteFromTextTable(&cliInterface, objUID, COM_HBASE_COL_FAMILY_TEXT, 0);
    if (cliRC < 0) {
      deallocEHI(ehi);
      processReturn();
      return;
    ...
    hbaseTable.val = (char *)extNameForHbase.data();
    hbaseTable.len = extNameForHbase.length();
    cliRC = alterHbaseTable(ehi, &hbaseTable, nal, &(edhbo.getHbaseOptions()), alterAddColNode->ddlXns());
    if (cliRC < 0) {
      deallocEHI(ehi);
      processReturn();
      return;
  ...
  if ((colFlags & SEABASE_COLUMN_IS_COMPOSITE) && (!compDefnStr.isNull())) {
    int numCompCols = 0;
    NAString textTableCompDefnStr;
    if (naTable->hasCompositeColumns()) {
      cliRC = getTextFromMD(&cliInterface, naTable->objectUid().castToInt64(), COM_COMPOSITE_DEFN_TEXT, 0,
    ...
    char buf[100];
    numCompCols++;
    snprintf(buf, sizeof(buf), "%08d%08d", newColNum, (int)compDefnStr.length());
    ...
    cliRC = updateTextTable(&cliInterface, naTable->objectUid().castToInt64(), COM_COMPOSITE_DEFN_TEXT, 0,
                            textTableCompDefnStr, NULL, -1, TRUE /*delete existing row*/);
    if (cliRC < 0) {
      cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
      deallocEHI(ehi);
      processReturn();
  ...
  if (alterAddColNode->getAddConstraintPK()) {
    ...
    if ((naTable->getClusteringIndex()) && (NOT naTable->getClusteringIndex()->hasOnlySyskey())) {
      *CmpCommon::diags() << DgSqlCode(-1256) << DgString0(extTableName);
  ...
  if ((alterAddColNode->getAddConstraintPK())OR(alterAddColNode->getAddConstraintCheckArray().entries() NEQ 0)
          OR(alterAddColNode->getAddConstraintUniqueArray().entries() NEQ 0)
              OR(alterAddColNode->getAddConstraintRIArray().entries() NEQ 0)) {
    if (naTable->isHbaseMapTable()) {
      *CmpCommon::diags() << DgSqlCode(-3242)
                          << DgString0(
                                 "Cannot specify unique, referential or check constraint for an HBase mapped table.");
    ...
    int constrNum = getNextUnnamedConstrNum(naTable);
    addConstraints(tableName, currCatAnsiName, currSchAnsiName, alterAddColNode, alterAddColNode->getAddConstraintPK(),
                   alterAddColNode->getAddConstraintUniqueArray(), alterAddColNode->getAddConstraintRIArray(),
                   alterAddColNode->getAddConstraintCheckArray(), constrNum, false, naTable->isPartitionV2Table());
    if (needRestoreIBAttr == true) {
      naTable->setIncrBackupEnabled(TRUE);
      addIBAttrForTable(naTable, &cliInterface);
    }
    if (CmpCommon::diags()->getNumber(DgSqlCode::ERROR_)) return;
  }

  // if sample table exist, add the same col in it.
  if (isSampleExist(catalogNamePart, schemaNamePart, objUID, sampleTableName, sampleTableUid)) {
    char dbuf[sampleTableName.length() + pColDef->getColDefAsText().length() + 100];
    str_sprintf(dbuf, "alter table %s add column %s", sampleTableName.data(), pColDef->getColDefAsText().data());
  ...
  if (naTable->isPartitionV2Table()) {
    ...
    for (int i = 0; i < pa.entries(); i++) {
      ...
      str_sprintf(buf, "alter table %s add column %s", partitionTableName.data(), pColDef->getColDefAsText().data());
      cliRC = cliInterface.executeImmediate(buf);
      if (cliRC < 0) {
        return;
      }
...
short CmpSeabaseDDL::dropSample(const NAString &catalogNamePart, const NAString &schemaNamePart, const long tableUID,
  ...
  str_sprintf(dbuf, "delete from %s.\"%s\".%s where table_uid = %ld", catalogNamePart.data(), schemaNamePart.data(),
...
NABoolean CmpSeabaseDDL::isSampleExist(const NAString &catName, const NAString &schName, const long tableUID,
  ...
  str_sprintf(buf,
              "select OBJECT_NAME, OBJECT_UID from "
  ...
  } else if ((triggerss->numEntries()) > 0)  // if we got a sample table name back
  {
    OutputInfo *trigger = (OutputInfo *)triggerss->getNext();
    char *objname = trigger->get(0);
    long sampleUid = *(long *)trigger->get(1);
...
short CmpSeabaseDDL::updateMDforDropCol(ExeCliInterface &cliInterface, const NATable *naTable, int dropColNum,
                                        const NAType *type) {
  int cliRC = 0;
  ...
  char buf[4000];
  str_sprintf(buf, "delete from %s.\"%s\".%s where object_uid = %ld and column_number = %d", getSystemCatalog(),
              SEABASE_MD_SCHEMA, SEABASE_COLUMNS, objUID, dropColNum);
  ...
  str_sprintf(
      buf, "update %s.\"%s\".%s set column_number = column_number - 1 where object_uid = %ld and column_number >= %d",
      getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_COLUMNS, objUID, dropColNum);
  ...
  str_sprintf(
      buf, "update %s.\"%s\".%s set column_number = column_number - 1 where object_uid = %ld and column_number >= %d",
      getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_KEYS, objUID, dropColNum);
  ...
  str_sprintf(buf, "delete from %s.\"%s\".%s where text_uid = %ld and text_type = %d and sub_id = %d ",
              getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_TEXT, objUID, COM_COLUMN_COMMENT_TEXT, dropColNum);
  cliRC = cliInterface.executeImmediate(buf);
  if (cliRC < 0) {
    cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
    return -1;
  ...
  str_sprintf(buf,
              "update %s.\"%s\".%s set sub_id = sub_id - 1 where text_uid = %ld and ( text_type = %d or text_type = %d "
              ") and sub_id > %d",
              getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_TEXT, objUID, COM_COMPUTED_COL_TEXT,
              COM_COLUMN_COMMENT_TEXT, dropColNum);
  ...
  str_sprintf(
      buf,
      "update %s.\"%s\".%s K set column_number = column_number - 1  where K.column_number >= %d and K.object_uid = "
      "(select C.constraint_uid from %s.\"%s\".%s C where C.table_uid = %ld and C.constraint_type = 'P')",
      getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_KEYS, dropColNum, getSystemCatalog(), SEABASE_MD_SCHEMA,
      SEABASE_TABLE_CONSTRAINTS, objUID);
  cliRC = cliInterface.executeImmediate(buf);
  if (cliRC < 0) {
    cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
    return -1;
  ...
  if (naTable->hasSecondaryIndexes()) {
    ...
    for (int i = 0; i < naFsList.entries(); i++) {
      ...
      str_sprintf(buf,
                  "update %s.\"%s\".%s set column_number = column_number - 1  where column_number >=  %d and "
                  "object_uid = (select object_uid from %s.\"%s\".%s where catalog_name = '%s' and schema_name = '%s' "
                  "and object_name = '%s' and object_type = 'IX') ",
                  getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_KEYS, dropColNum, getSystemCatalog(),
                  SEABASE_MD_SCHEMA, SEABASE_OBJECTS, indexName.getCatalogName().data(),
                  indexName.getSchemaName().data(), indexName.getObjectName().data());
      cliRC = cliInterface.executeImmediate(buf);
      if (cliRC < 0) {
  ...
  if (type->isComposite()) {
    ...
    const NAColumnArray &nacolArr = naTable->getNAColumnArray();
    NAString textTableCompDefnStr;
    for (int i = 0; i < nacolArr.entries(); i++) {
      const NAColumn *nacol = nacolArr[i];

      if ((nacol->getType()->isComposite()) && (nacol->getPosition() != dropColNum)) {
        ...
        if (!compDefnStr.isNull()) {
          ...
          snprintf(buf, sizeof(buf), "%08d%08d",
                   (nacol->getPosition() >= dropColNum ? nacol->getPosition() - 1 : nacol->getPosition()),
    ...
    if (numCompCols > 0) {
      ...
      cliRC = updateTextTable(&cliInterface, objUID, COM_COMPOSITE_DEFN_TEXT, 0, textTableCompDefnStr, NULL, -1,
                              TRUE /*delete existing row*/);

    } else {
      cliRC = deleteFromTextTable(&cliInterface, objUID, COM_COMPOSITE_DEFN_TEXT, 0);
    }
    if (cliRC < 0) {
      cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
...
//
///////////////////////////////////////////////////////////////////////
short CmpSeabaseDDL::alignedFormatTableDropColumn(const NAString &catalogNamePart, const NAString &schemaNamePart,
                                                  const NAString &objectNamePart, const NATable *naTable,
                                                  const NAString &dropColName, ElemDDLColDef *pColDef, NABoolean ddlXns,
  ...
  int dropColNum = altNaCol->getPosition();
  ...
  if (naTable->isStoredDesc()) {
    ...
    deleteFromSharedCache(&cliInterface,
                          ...
                          naTable->getTableName().getQualifiedNameAsAnsiString(), COM_BASE_TABLE_OBJECT, "drop column");
  ...
  // add X lock
  if (lockRequired(naTable, catalogNamePart, schemaNamePart, objectNamePart, HBaseLockMode::LOCK_X,
                   FALSE /* useHbaseXN */, FALSE /* replSync */, FALSE /* incrementalBackup */,
                   FALSE /* asyncOperation */, TRUE /* noConflictCheck */, TRUE /* registerRegion */)) {
    cliRC = -1;
    goto label_restore;
  ...
  if (NOT altNaCol->getType()->isLob()) {
    str_sprintf(buf, "update statistics for table %s on \"%s\" clear",
                naTable->getTableName().getQualifiedNameAsAnsiString().data(), dropColName.data());
    cliRC = cliInterface.executeImmediate(buf);
    if (cliRC < 0) {
      cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
      goto label_restore;
  ...
  if ((dropColNum != -1) && (dropColNum < (naColArr.entries() - 1))) {
    str_sprintf(
        buf, "update %s.\"%s\".%s set column_number = column_number - 1 where table_uid = %ld and column_number >= %d",
        naTable->getTableName().getCatalogName().data(), naTable->getTableName().getSchemaName().data(),
        HBASE_HIST_NAME, naTable->objectUid().castToInt64(), dropColNum);
  ...
  for (int c = 0; c < naColArr.entries(); c++) {
    const NAColumn *nac = naColArr[c];
    if (nac->getColName() == dropColName) continue;

    if (nac->isComputedColumn()) continue;
  ...
  if (tgtCols.isNull()) {
    *CmpCommon::diags() << DgSqlCode(-1424) << DgColumnName(dropColName);
  ...
  if ((cliRC = recreateUsingViews(&cliInterface, viewNameList, viewDefnList, ddlXns)) < 0) {
    NAString reason =
        "Error occurred while recreating views due to dependency on older column definition. Drop dependent views "
        "before doing the drop.";
    *CmpCommon::diags() << DgSqlCode(-1404) << DgColumnName(dropColName) << DgString0("dropped") << DgString1(reason);
    goto label_restore;
  }
...
label_restore:
  ...
  if ((cliRC < 0) && (NOT clonedTable.isNull()) &&
      (cloneSeabaseTable(clonedTable, -1, naTable->getTableName().getQualifiedNameAsAnsiString(), naTable, ehi,
                         &cliInterface, FALSE))) {
    cliRC = -1;
    goto label_drop;
  }
...
short CmpSeabaseDDL::hbaseFormatTableDropColumn(ExpHbaseInterface *ehi, const NAString &catalogNamePart,
                                                const NAString &schemaNamePart, const NAString &objectNamePart,
                                                const NATable *naTable, const NAString &dropColName,
                                                const NAColumn *nacol, NABoolean ddlXns, NAList<NAString> &viewNameList,
                                                NAList<NAString> &viewDefnList) {
  int cliRC = 0;

  long objDataUID = (naTable ? naTable->objDataUID().castToInt64() : 0);

  const NAString extNameForHbase =
      genHBaseObjName(catalogNamePart, schemaNamePart, objectNamePart,
                      (naTable ? ((NATable *)naTable)->getNamespace() : NAString("")), objDataUID);

  ExeCliInterface cliInterface(STMTHEAP, 0, NULL, CmpCommon::context()->sqlSession()->getParentQid());

  int dropColNum = nacol->getPosition();
  ...
  if (ComIsHBaseMappedIntFormat(catalogNamePart, schemaNamePart)) {
    NAString newCatName;
    NAString newSchName;
    ComConvertHBaseMappedIntToExt(catalogNamePart, schemaNamePart, newCatName, newSchName);
  ...
  str_sprintf(buf, "update statistics for table %s on \"%s\" clear",
              // naTable->getTableName().getQualifiedNameAsAnsiString().data(),
              objectName.data(), dropColName.data());
  cliRC = cliInterface.executeImmediate(buf);
  if (cliRC < 0) {
    cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
    goto label_error;
...

  {
    ...
    if (naTable->isHbaseMapTable()) {
      ...
      int colQualLen = nacol->getHbaseColQual().length();
    ...
    if (cliRC < 0) {
      *CmpCommon::diags() << DgSqlCode(-8448) << DgString0((char *)"ExpHbaseInterface::deleteColumns()")
                          << DgString1(getHbaseErrStr(-cliRC)) << DgInt0(-cliRC)
                          << DgString2((char *)GetCliGlobals()->getJniErrorStr());
...
// DDL lock.
// ----------------------------------------------------------------------------
void CmpSeabaseDDL::alterSeabaseTableDropColumn(StmtDDLAlterTableDropColumn *alterDropColNode, NAString &currCatName,
                                                NAString &currSchName) {
  int cliRC = 0;
  int retcode = 0;
  ...
  retcode = setupAndErrorChecks(tabName, alterDropColNode->getOrigTableNameAsQualifiedName(), currCatName, currSchName,
                                catalogNamePart, schemaNamePart, objectNamePart, extTableName, extNameForHbase, cn,
                                &naTable, FALSE, TRUE, &cliInterface, COM_BASE_TABLE_OBJECT, SQLOperation::ALTER_TABLE,
                                FALSE, TRUE /*process hiatus*/);
  if (retcode < 0) {
  ...
  if (!nacol) {
    ...
    if (NOT alterDropColNode->dropIfExists()) {
      *CmpCommon::diags() << DgSqlCode(-CAT_COLUMN_DOES_NOT_EXIST_ERROR) << DgColumnName(colName);
    }
  ...
  int datatype = nacol->getType()->getFSDatatype();
  ...
  if (naKeyColArr.getColumn(colName)) {
    ...
    *CmpCommon::diags() << DgSqlCode(-1420) << DgColumnName(colName);
      ...
      (naTable->getCheckConstraints().entries() > 0))     // check constraints
  {
    ...
    if (naTable->getRefConstraints().entries() > 0) {
      ...
      for (int i = 0; i < ariList.entries(); i++) {
        ...
        if (userDefinedRefConstr) break;
      }
    }

    if ((userDefinedUniqueConstr) || (userDefinedRefConstr) || (naTable->getCheckConstraints().entries() > 0)) {
      *CmpCommon::diags() << DgSqlCode(-1425) << DgTableName(extTableName)
                          << DgString0(
                                 "Reason: Cannot drop a column on a table with check, unique or referential "
                                 "constraints. Drop those constraints before dropping the column and then recreate "
                                 "them afterwards. Use SHOWDDL to find out the unique and referential constraints.");
  ...
  if (naTable->hasSecondaryIndexes()) {
    ...
    for (int i = 0; i < naFsList.entries(); i++) {
      ...
      if (naIndexColArr.getColumn(colName)) {
        ...
        *CmpCommon::diags() << DgSqlCode(-1421) << DgColumnName(colName) << DgTableName(naFS->getExtFileSetName());
  ...
  if (CmpCommon::getDefault(MODE_COMPATIBLE_1) != DF_ON) {
    ...
    if (usingViewsQueue->numEntries() != 0) {
      ...
      for (int idx = 0; idx < usingViewsQueue->numEntries(); idx++) {
        ...
        char *viewName = vi->get(0);
        viewNames += viewName;
        if (idx != usingViewsQueue->numEntries() - 1) viewNames += ',';
      }
      ...
      *CmpCommon::diags() << DgSqlCode(-1404) << DgColumnName(colName) << DgString0("dropped") << DgString1(reason);
      return;
    }
  }
  ...
  int colNumber = nacol->getPosition();
  char *col = NULL;
  // if partition table skip this step
  if (NOT naTable->isPartitionV2Table()) {
    if (naTable->isSQLMXAlignedTable()) {
      if (alignedFormatTableDropColumn(catalogNamePart, schemaNamePart, objectNamePart, naTable,
                                       alterDropColNode->getColName(), NULL, alterDropColNode->ddlXns(), viewNameList,
                                       viewDefnList)) {
        cliRC = -1;
        goto label_error;
      }
      ...
      ehi = allocEHI(naTable->storageType());
      if (hbaseFormatTableDropColumn(ehi, catalogNamePart, schemaNamePart, objectNamePart, naTable,
                                     alterDropColNode->getColName(), nacol, alterDropColNode->ddlXns(), viewNameList,
                                     viewDefnList)) {
        cliRC = -1;
        goto label_error;
      }
  ...
  // first , get the current string
  if (!ComIsTrafodionReservedSchemaName(schemaNamePart) && naTable->isSeabaseTable() == TRUE) {
    if (ehi == NULL) ehi = allocEHI(naTable->storageType());
    retcode = ehi->getTableDefForBinlog(binlogTableName, &defStringArray);
    if (retcode < 0) {
      *CmpCommon::diags() << DgSqlCode(-3242) << DgString0("Cannot getTableDefforBinlog.");
      processReturn();
    ...
    retcode = ehi->updateTableDefForBinlog(binlogTableName, fullColBufString, fullKeyBufString, ddlStartTs);
    if (retcode < 0) {
      *CmpCommon::diags() << DgSqlCode(-3242) << DgString0("Cannot updateTableDefForBinlog.");
      processReturn();
      return;
    }
  }
  ...
  if (isSampleExist(catalogNamePart, schemaNamePart, objUID, sampleTableName, sampleTableUid)) {
    cliRC = dropSample(catalogNamePart, schemaNamePart, objUID, sampleTableName);
    if (cliRC < 0) {
      goto label_error;
    }
    // Warning: recreate sample table
    if (CmpCommon::getDefault(SHOWWARN_OPT) == DF_ON) *CmpCommon::diags() << DgSqlCode(1718);
  }
  ...
  if (naTable->isPartitionV2Table()) {
    ...
    for (int i = 0; i < pa.entries(); i++) {
      ...
      str_sprintf(buf, "alter table %s drop column %s", partitionTableName.data(),
                  alterDropColNode->getColName().data());
      cliRC = cliInterface.executeImmediate(buf);
      if (cliRC < 0) {
...
label_return:
  ...
  endXnIfStartedHere(&cliInterface, xnWasStartedHere, cliRC, FALSE /*modifyEpochs*/, FALSE /*clearObjectLocks*/);
...
label_error:
  ...
  if ((cliRC = recreateUsingViews(&cliInterface, viewNameList, viewDefnList, alterDropColNode->ddlXns())) < 0) {
    NAString reason =
        "Error occurred while recreating views due to dependency on older column definition. Drop dependent views "
        "before doing the drop.";
    *CmpCommon::diags() << DgSqlCode(-1404) << DgColumnName(colName) << DgString0("dropped") << DgString1(reason);
...
void CmpSeabaseDDL::alterSeabaseTableDropPartition(StmtDDLAlterTableDropPartition *alterDropPartition,
                                                   NAString &currCatName, NAString &currSchName) {
  int cliRC = 0;
  int retcode = 0;
  ...
  retcode = setupAndErrorChecks(tabName, alterDropPartition->getOrigTableNameAsQualifiedName(), currCatName,
                                currSchName, catalogNamePart, schemaNamePart, objectNamePart, extTableName,
                                extNameForHbase, cn, &naTable, FALSE, TRUE, &cliInterface, COM_BASE_TABLE_OBJECT,
                                SQLOperation::ALTER_TABLE, FALSE, TRUE /*process hiatus*/);
  if (retcode < 0) {
  ...
  int ret = checkPartition(&partArray, tableDesc, bindWA, objectName, invalidPartName, &dropPartEntityNameVec,
                           &partEntityNameMap);
  if (ret == -1) {
    // An internal executor error occurred.
    *CmpCommon::diags() << DgSqlCode(-8001);
    ...
    // The DDL request has duplicate references to partition name.
    *CmpCommon::diags() << DgSqlCode(-8305) << DgString0(invalidPartName);
    processReturn();
    return;
  } else if (ret == 2) {
    *CmpCommon::diags() << DgSqlCode(-8306) << DgString0("The partition number is invalid or out-of-range");
  ...
  if (!isSubpartition) {
    ...
    for (int i = 0; i < dropPartEntityNameVec.size(); ++i) {
    ...
    if (firstLevelPartCnt == 1) {
      // Dropping the only partition of an object is not allowed.
      *CmpCommon::diags() << DgSqlCode(-8306) << DgString0("Dropping the only partition of an object is not allowed");
      processReturn();
      return;
    }
    ...
    if (firstLevelPartCnt - dropNAPartitionVec.size() < 1) {
      // Operation is not allowed because no partition exists after drop.
      *CmpCommon::diags() << DgSqlCode(-8306) << DgString0("No partition exists after drop");
      processReturn();
      return;
    }
  } else  // subpartition
  {
    ...
    std::map<NAString, int> subpartCntPerFirstpartMap;

    for (int i = 0; i < firstLevelPartCnt; ++i) {
      const NAPartitionArray *subPartInfo = partitionArray[i]->getSubPartitions();
      for (int j = 0; j < subPartInfo->entries(); ++j) {
        partEntityName = (*subPartInfo)[j]->getPartitionEntityName();
        allSubPartitionMap[partEntityName] =
            std::pair<NAPartition *, NAPartition *>(partitionArray[i], (*subPartInfo)[j]);
    ...
    for (int i = 0; i < dropPartEntityNameVec.size(); ++i) {
      ...
      if (subpartInfoIt == allSubPartitionMap.end()) {
        ...
        *CmpCommon::diags() << DgSqlCode(-8036) << DgString0(errMsg);
        processReturn();
        return;
      }
    ...
    for (std::map<NAString, int>::iterator it = subpartCntPerFirstpartMap.begin();
  ...
  // drop partition
  for (int i = 0; i < dropNAPartitionVec.size(); ++i) {
    cliRC = dropSeabasePartitionTable(&cliInterface, alterDropPartition, dropNAPartitionVec[i], catalogNamePart,
                                      schemaNamePart);
    if (cliRC < 0) {
      cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
      goto label_rollback;
    ...
    std::map<NAString, NAPartition *>::iterator it =
        nextNAPartitionMap.find(dropNAPartitionVec[i]->getPartitionEntityName());
    if (it != nextNAPartitionMap.end()) {
      NAPartition *nextNAPart = it->second;
      if (nextNAPart) {
        NAString nextObjName = nextNAPart->getPartitionEntityName();
        if (std::find(dropPartEntityNameVec.begin(), dropPartEntityNameVec.end(), nextObjName) !=
            dropPartEntityNameVec.end())
          continue;
        ...
        RemoveNATableParam nextRemoveNATableParam(ComQiScope::REMOVE_FROM_ALL_USERS, alterDropPartition->ddlXns(),
                                                  FALSE);

        cliRC = updateCachesAndStoredDesc(&cliInterface, catalogNamePart, schemaNamePart, nextObjName,
                                          COM_BASE_TABLE_OBJECT, op, &nextRemoveNATableParam, &nextUpdObjRefParam,
                                          SharedCacheDDLInfo::DDL_UPDATE);
        if (cliRC < 0) {
          cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
          goto label_rollback;
...
// This statement runs in a transaction
// ----------------------------------------------------------------------------
void CmpSeabaseDDL::alterSeabaseTableAlterIdentityColumn(StmtDDLAlterTableAlterColumnSetSGOption *alterIdentityColNode,
                                                         NAString &currCatName, NAString &currSchName) {
  int cliRC = 0;
  int retcode = 0;
  ...
  retcode =
      setupAndErrorChecks(tabName, alterIdentityColNode->getOrigTableNameAsQualifiedName(), currCatName, currSchName,
                          catalogNamePart, schemaNamePart, objectNamePart, extTableName, extNameForHbase, cn, &naTable,
                          FALSE, FALSE, &cliInterface, COM_BASE_TABLE_OBJECT, SQLOperation::ALTER_TABLE);
  if (retcode < 0) {
    processReturn();
  ...
  if (!nacol->isIdentityColumn()) {
    *CmpCommon::diags() << DgSqlCode(-1590) << DgColumnName(colName);
  ...
  NAString seqName;
  SequenceGeneratorAttributes::genSequenceName(catalogNamePart, schemaNamePart, objectNamePart,
                                               alterIdentityColNode->getColumnName(), seqName);
  ...
  if (sgo) {
    ...
    char buf[4000];
    str_sprintf(buf, "alter internal sequence %s.\"%s\".\"%s\" %s", catalogNamePart.data(), schemaNamePart.data(),
                seqName.data(), options.data());
  ...
  seqn.setSpecialType(ExtendedQualName::SG_TABLE);
  ActiveSchemaDB()->getNATableDB()->removeNATable(seqn, ComQiScope::REMOVE_FROM_ALL_USERS,
                                                  COM_SEQUENCE_GENERATOR_OBJECT, alterIdentityColNode->ddlXns(), FALSE);

  return;
}

short CmpSeabaseDDL::saveAndDropUsingViews(long objUID, ExeCliInterface *cliInterface, NAList<NAString> &viewNameList,
                                           NAList<NAString> &viewDefnList) {
  int cliRC = 0;

  NAString catName, schName, objName;
  cliRC = getObjectName(cliInterface, objUID, catName, schName, objName);
  if (cliRC < 0) {
    processReturn();
  ...
  Queue *usingViewsQueue = NULL;
  cliRC = getAllUsingViews(cliInterface, catName, schName, objName, usingViewsQueue);
  if (cliRC < 0) {
    processReturn();
  ...
  if (usingViewsQueue->numEntries() == 0) return 0;
  ...
  for (int idx = 0; idx < usingViewsQueue->numEntries(); idx++) {
    ...
    long viewUID = getObjectUID(cliInterface, catName.data(), schName.data(), objName.data(), COM_VIEW_OBJECT_LIT);
    if (viewUID < 0) {
      endXnIfStartedHere(cliInterface, xnWasStartedHere, -1, FALSE /*modifyEpochs*/, FALSE /*clearObjectLocks*/);
...
short CmpSeabaseDDL::recreateUsingViews(ExeCliInterface *cliInterface, NAList<NAString> &viewNameList,
                                        NAList<NAString> &viewDefnList, NABoolean ddlXns) {
  int cliRC = 0;

  if (viewDefnList.entries() == 0) return 0;
...
label_return:
  ...
  for (int i = 0; i < viewDefnList.entries(); i++) {
    ...
    CorrName cn(objectNamePart, STMTHEAP, schemaNamePart, catalogNamePart);
    ActiveSchemaDB()->getNATableDB()->removeNATable(cn, ComQiScope::REMOVE_MINE_ONLY, COM_VIEW_OBJECT, ddlXns, FALSE);
  }

  endXnIfStartedHere(cliInterface, xnWasStartedHere, cliRC, TRUE /* modifyEpochs */, FALSE /*clearObjectLocks*/);
...
//
///////////////////////////////////////////////////////////////////////
short CmpSeabaseDDL::alignedFormatTableAlterColumnAttr(const NAString &catalogNamePart, const NAString &schemaNamePart,
                                                       const NAString &objectNamePart, const NATable *naTable,
                                                       const NAString &altColName, ElemDDLColDef *pColDef,
                                                       NABoolean ddlXns, NAList<NAString> &viewNameList,
                                                       NAList<NAString> &viewDefnList) {
  int cliRC = 0;
  int cliRC2 = 0;
  ...
  int altColNum = altNaCol->getPosition();
  ...
  if (getColInfo(pColDef,
                 FALSE,  // not a metadata, histogram or repository column
                 colFamily, colName, naTable->isSQLMXAlignedTable(), datatype, length, precision, scale, dt_start,
                 dt_end, upshifted, nullable, charset, colClass, defaultClass, defVal, heading, lobStorage, compDefnStr,
  ...
  if (!ComIsTrafodionReservedSchemaName(schemaNamePart) && naTable->isSeabaseTable() == TRUE) {
    ...
    for (int i = 0; i < fields.entries(); i++) {
      if (i == altColNum) {
        ...
        if (thisfields[3] == "A")
          sprintf(binlogTableDefString, "%d-%d-%d-%s-%d-%d-%s-%s-%d-%d-%d-%s", datatype, nullable, length, "A",
                  precision, scale, charset.data(), stripedstr.data(), dt_start, dt_end, altColNum,
                  colName.strip(NAString::trailing, ' ').data());
        else
          sprintf(binlogTableDefString, "%d-%d-%d-%s-%d-%d-%s-%s-%d-%d-%d-%s", datatype, nullable, length, "U",
                  precision, scale, charset.data(), stripedstr.data(), dt_start, dt_end, altColNum,
                  colName.strip(NAString::trailing, ' ').data());
  ...
  str_sprintf(buf, "update statistics for table %s on \"%s\" clear",
              naTable->getTableName().getQualifiedNameAsAnsiString().data(), altColName.data());
  cliRC = cliInterface.executeImmediate(buf);
  if (cliRC < 0) {
    cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
    goto label_restore;
  ...
  str_sprintf(
      buf,
      "update %s.\"%s\".%s set (column_class, fs_data_type, sql_data_type, column_size, column_precision, "
      "column_scale, datetime_start_field, datetime_end_field, is_upshifted, nullable, character_set, default_class, "
      "default_value) = (case when column_class in ('" COM_ADDED_USER_COLUMN_LIT
      "', '" COM_ADDED_ALTERED_USER_COLUMN_LIT "') then '" COM_ADDED_ALTERED_USER_COLUMN_LIT
      "' else '" COM_ALTERED_USER_COLUMN_LIT
      "' end, %d, '%s', %d, %d, %d, %d, %d, '%s', %d, '%s', %d, '%s') where object_uid = %ld and column_number = %d",
      getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_COLUMNS, datatype, getAnsiTypeStrFromFSType(datatype), length,
      precision, scale, dt_start, dt_end, (upshifted ? "Y" : "N"), nullable, (char *)charset.data(), (int)defaultClass,
      (quotedDefVal.isNull()
           ? " "
           : quotedDefVal.data()),  // column DEFAULT_VALUE can't be NULL,so we can't use ''(null) here
      naTable->objectUid().castToInt64(), altColNum);
  ...
  str_sprintf(buf, "upsert using load into %s select * from %s",
              naTable->getTableName().getQualifiedNameAsAnsiString().data(), clonedTable.data());
  cliRC = cliInterface.executeImmediate(buf);
  ...
  if (cliRC < 0) {
    ...
    *CmpCommon::diags() << DgSqlCode(-1404) << DgColumnName(altColName) << DgString0("altered") << DgString1(reason);
...
/////////////////////////////////////////////////////////////////////
short CmpSeabaseDDL::mdOnlyAlterColumnAttr(const NAString &catalogNamePart, const NAString &schemaNamePart,
                                           const NAString &objectNamePart, const NATable *naTable,
                                           const NAColumn *naCol, NAType *newType,
                                           StmtDDLAlterTableAlterColumnDatatype *alterColNode,
                                           NAList<NAString> &viewNameList, NAList<NAString> &viewDefnList,
                                           NABoolean isAlterForSample, long sampleTableUid) {
  int cliRC = 0;
  ...
  int altColNum = naCol->getPosition();
  ...
  if (!ComIsTrafodionReservedSchemaName(schemaNamePart) && naTable->isSeabaseTable() == TRUE) {
    ...
    colBufNAString.split(';', fields);
    CharInfo::Collation collationSequence = CharInfo::DefaultCollation;
    getTypeInfo(newType, alignedFormat, -1, datatype, length, precision, scale, dtStart, dtEnd, upshifted, nullable,
    ...
    for (int i = 0; i < fields.entries(); i++) {
      if (i == altColNum) {
        ...
        if (thisfields[3] == "A")
          sprintf(binlogTableDefString, "%d-%d-%d-%s-%d-%d-%s-%s-%d-%d-%d-%s;", datatype, nullable, length, "A",
                  precision, scale, charset.data(), stripedstr.data(), dtStart, dtEnd, altColNum,
                  colName.strip(NAString::trailing, ' ').data());
        else
          sprintf(binlogTableDefString, "%d-%d-%d-%s-%d-%d-%s-%s-%d-%d-%d-%s", datatype, nullable, length, "U",
                  precision, scale, charset.data(), stripedstr.data(), dtStart, dtEnd, altColNum,
  ...
  while (cd->columnsDesc()->colnumber != colNumber && cd->next) cd = cd->next;
  ...
  char buf[4000];
  str_sprintf(buf,
              "update %s.\"%s\".%s set column_size = %d, column_precision = %d, "
              "column_class = case  when column_class in ('" COM_ADDED_USER_COLUMN_LIT
              "', '" COM_ADDED_ALTERED_USER_COLUMN_LIT "') then '" COM_ADDED_ALTERED_USER_COLUMN_LIT
              "' else '" COM_ALTERED_USER_COLUMN_LIT
              "' end , flags = %ld where object_uid = %ld and column_number = %d",
              getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_COLUMNS, newType->getNominalSize(),
              newType->getPrecisionOrMaxNumChars(), colFlags, objUID, colNumber);
  ...
  if (NOT isAlterForSample) {
    cliRC = recreateUsingViews(&cliInterface, viewNameList, viewDefnList, alterColNode->ddlXns());
    if (cliRC < 0) {
      NAString reason =
          "Error occurred while recreating views due to dependency on older column definition. Drop dependent views "
          "before doing the alter.";
      *CmpCommon::diags() << DgSqlCode(-1404) << DgColumnName(naCol->getColName().data()) << DgString0("altered")
                          << DgString1(reason);
...
//
///////////////////////////////////////////////////////////////////////
short CmpSeabaseDDL::hbaseFormatTableAlterColumnAttr(const NAString &catalogNamePart, const NAString &schemaNamePart,
                                                     const NAString &objectNamePart, const NATable *naTable,
                                                     const NAColumn *naCol, NAType *newType,
                                                     StmtDDLAlterTableAlterColumnDatatype *alterColNode) {
  ExeCliInterface cliInterface(STMTHEAP, 0, NULL, CmpCommon::context()->sqlSession()->getParentQid());
  ...
  NAColumn *nac = NULL;
  if (getNAColumnFromColDef(pColDef, nac)) return -1;

  dispBuf[0] = 0;
  if (cmpDisplayColumn(nac, (char *)tempCol.data(), newType, 3, NULL, dispBuf, ii, FALSE, identityCol, FALSE, FALSE,
                       UINT_MAX, NULL))
    return -1;
  ...
  char buf[4000];
  str_sprintf(buf, "alter table %s add column %s", naTable->getTableName().getQualifiedNameAsAnsiString().data(),
              dispBuf);
  cliRC = cliInterface.executeImmediate(buf);
  if (cliRC < 0) {
    cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
  ...
  str_sprintf(buf, "update %s set %s = %s", naTable->getTableName().getQualifiedNameAsAnsiString().data(),
              tempCol.data(), naCol->getColName().data());
  cliRC = cliInterface.executeImmediate(buf);
  if (cliRC < 0) {
    cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
  ...
  str_sprintf(buf, "delete from %s.\"%s\".%s where object_uid = %ld and column_number = %d", getSystemCatalog(),
              SEABASE_MD_SCHEMA, SEABASE_COLUMNS, tableUID, altColNum);
  ...
  str_sprintf(buf,
              "insert into %s.\"%s\".%s select object_uid, '%s', %d, '" COM_ALTERED_USER_COLUMN_LIT
              "', fs_data_type, sql_data_type, column_size, column_precision, column_scale, datetime_start_field, "
              "datetime_end_field, is_upshifted, column_flags, nullable, character_set, default_class, default_value, "
              "column_heading, '%s', '%s', direction, is_optional, flags from %s.\"%s\".%s where object_uid = %ld and "
              "column_number = (select column_number from %s.\"%s\".%s where object_uid = %ld and column_name = '%s')",
              getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_COLUMNS, naCol->getColName().data(), altColNum,
              altNaCol->getHbaseColFam().data(), altNaCol->getHbaseColQual().data(), getSystemCatalog(),
              SEABASE_MD_SCHEMA, SEABASE_COLUMNS, tableUID, getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_COLUMNS,
              tableUID, tempCol.data());
  ...
  str_sprintf(buf, "update %s set %s = %s", naTable->getTableName().getQualifiedNameAsAnsiString().data(),
              naCol->getColName().data(), tempCol.data());
  cliRC = cliInterface.executeImmediate(buf);
  if (cliRC < 0) {
    cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
    ...
    // column cannot be altered
    *CmpCommon::diags() << DgSqlCode(-1404) << DgColumnName(naCol->getColName()) << DgString0("altered")
                        << DgString1(reason);
  ...
  str_sprintf(buf, "alter table %s drop column %s", naTable->getTableName().getQualifiedNameAsAnsiString().data(),
              tempCol.data());
  cliRC = cliInterface.executeImmediate(buf);
  if (cliRC < 0) {
    cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
...
// DDL lock.
// ----------------------------------------------------------------------------
void CmpSeabaseDDL::alterSeabaseTableAlterColumnDatatype(StmtDDLAlterTableAlterColumnDatatype *alterColNode,
                                                         NAString &currCatName, NAString &currSchName,
  ...
  retcode = setupAndErrorChecks(tabName, alterColNode->getOrigTableNameAsQualifiedName(), currCatName, currSchName,
                                catalogNamePart, schemaNamePart, objectNamePart, extTableName, extNameForHbase, cn,
                                &naTable, FALSE, FALSE, &cliInterface, COM_BASE_TABLE_OBJECT, SQLOperation::ALTER_TABLE,
                                FALSE, TRUE /*process hiatus*/);
  if (retcode < 0) {
  ...
  if (nacol->isIdentityColumn()) {
    *CmpCommon::diags() << DgSqlCode(-CAT_IDENTITY_COLUMN_DATATYPE_ALTER) << DgColumnName(colName);
    processReturn();
    return;
  }
  ...
  if (NOT pColDef->getIsConstraintNotNullSpecified()) newType->setNullable(*currType);

  // if column charset is not specified, use the orignal charset as default charset
  if (NOT pColDef->isColCharsetSpecified() && (DFS2REC::isSQLVarChar(currType->getFSDatatype())) &&
    ...
    if (currType->getCharSet() != newType->getCharSet()) {
      int Prec = ((CharType *)newType)->getStrCharLimit();
      int len = ((CharType *)newType)->getStrCharLimit() * CharInfo::maxBytesPerChar(currType->getCharSet());
      newType =
          new (STMTHEAP) SQLVarChar(STMTHEAP, CharLenInfo(Prec, len), isNullable, ((CharType *)currType)->isUpshifted(),
                                    ((CharType *)currType)->isCaseinsensitive(), ((CharType *)currType)->getCharSet(),
                                    ((CharType *)currType)->getCollation(), ((CharType *)currType)->getCoercibility());
  ...
  if ((newType->getTypeQualifier() == NA_CHARACTER_TYPE) &&
    ...
    *CmpCommon::diags() << DgSqlCode(-4247) << DgInt0(newType->getNominalSize())
                        << DgInt1(CmpCommon::getDefaultNumeric(TRAF_MAX_CHARACTER_COL_LENGTH))
  ...
  if ((NOT currType->isCompatible(*newType)) && (NOT((currType->getTypeQualifier() == NA_CHARACTER_TYPE) &&
                                                     (newType->getTypeQualifier() == NA_CHARACTER_TYPE)))) {
    NAString reason = "Old and New datatypes must be compatible.";

    // column cannot be altered
    *CmpCommon::diags() << DgSqlCode(-1404) << DgColumnName(colName) << DgString0("altered") << DgString1(reason);
  ...
  if ((DFS2REC::isSQLVarChar(currType->getFSDatatype())) && (DFS2REC::isSQLVarChar(newType->getFSDatatype())) &&
    ...
    if (NOT pColDef->isColDefaultSpecified())
      ...
      ComLobsStorageType lobStorage;
      NAString compDefnStr;
      if (getColInfo(pColDef, FALSE, colFamily, colName, naTable->isSQLMXAlignedTable(), datatype, length, precision,
                     scale, dt_start, dt_end, upshifted, nullable, charset, colClass, defaultClass, defVal, heading,
                     lobStorage, compDefnStr, hbaseColFlags, colFlags)) {
        processReturn();
        return;
      }
      ...
      if (nacol->getDefaultClass() == defaultClass) {
        if (defaultClass == COM_NO_DEFAULT)
          ...
          ToQuotedString(quotedDefVal, defVal, FALSE);
          if ((strncmp(defVal.data(), nacol->getDefaultValue(), defVal.length()) == 0)) mdAlterOnly = TRUE;
  ...
  if (isMaintenanceWindowOFF) {
    if (!mdAlterOnly) {
      ...
      *CmpCommon::diags() << DgSqlCode(-CLI_CANNOT_EXECUTE_DDL) << DgString0(tmp.data());
      processReturn();
    }
  }

  if ((NOT mdAlterOnly) && (CmpCommon::getDefault(TRAF_ALTER_COL_ATTRS) == DF_OFF)) {
    NAString reason;
    if (NOT((DFS2REC::isSQLVarChar(currType->getFSDatatype())) && (DFS2REC::isSQLVarChar(newType->getFSDatatype()))))
  ...
  // this operation cannot be done if a xn is already in progress.
  if ((NOT mdAlterOnly) &&
      (xnInProgress(&cliInterface) || (!GetCliGlobals()->currContext()->getTransaction()->autoCommit()))) {
    *CmpCommon::diags() << DgSqlCode(-20125) << DgString0("This ALTER");
  ...
  CmpCommon::context()->getCliContext()->execDDLOptions() = TRUE;
  if (mdAlterOnly) {
    if (mdOnlyAlterColumnAttr(catalogNamePart, schemaNamePart, objectNamePart, naTable, nacol, newType, alterColNode,
    ...
    if (alignedFormatTableAlterColumnAttr(catalogNamePart, schemaNamePart, objectNamePart, naTable, colName, pColDef,
                                          alterColNode->ddlXns(), viewNameList, viewDefnList)) {
      cliRC = -1;
      goto label_error;
    }
  } else if (hbaseFormatTableAlterColumnAttr(catalogNamePart, schemaNamePart, objectNamePart, naTable, nacol, newType,
                                             alterColNode)) {
    cliRC = -1;
  ...
  if (isSampleExist(catalogNamePart, schemaNamePart, objUID, sampleTableName, sampleTableUid)) {
    if (mdAlterOnly) {
      if (mdOnlyAlterColumnAttr(catalogNamePart, schemaNamePart, sampleTableName, naTable, nacol, newType, alterColNode,
      ...
      ActiveSchemaDB()->getNATableDB()->removeNATable(sampleCn, ComQiScope::REMOVE_FROM_ALL_USERS,
                                                      COM_BASE_TABLE_OBJECT, alterColNode->ddlXns(), FALSE);
    } else {
      cliRC = dropSample(catalogNamePart, schemaNamePart, objUID, sampleTableName);
      if (cliRC < 0) {
        goto label_error;
...
label_error:
  ...
  if ((cliRC = recreateUsingViews(&cliInterface, viewNameList, viewDefnList, alterColNode->ddlXns())) < 0) {
    NAString reason =
        "Error occurred while recreating views due to dependency on older column definition. Drop dependent views "
        "before doing the alter.";
    *CmpCommon::diags() << DgSqlCode(-1404) << DgColumnName(colName) << DgString0("altered") << DgString1(reason);
...
int CmpSeabaseDDL::updatePKeysTable(ExeCliInterface *cliInterface, const char *renamedColName, long objUID,
                                    const char *colName, NABoolean isIndexColumn) {
  char buf[1500];
  if (isIndexColumn)
    str_sprintf(
        buf, "update %s.\"%s\".%s set column_name = '%s' || '@' where object_uid = %ld and column_name = '%s' || '@'",
        getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_KEYS, renamedColName, objUID, colName);
  else
...
//
///////////////////////////////////////////////////////////////////
void CmpSeabaseDDL::alterSeabaseTableAlterColumnRename(StmtDDLAlterTableAlterColumnRename *alterColNode,
                                                       NAString &currCatName, NAString &currSchName) {
  int cliRC = 0;
  int retcode = 0;
  ...
  if (retcode == 0)  // does not exist
  {
    *CmpCommon::diags() << DgSqlCode(-CAT_CANNOT_ALTER_WRONG_TYPE) << DgString0(extTableName);
  ...
  */
  ...
  if ((CmpCommon::getDefault(TRAF_ALLOW_RESERVED_COLNAMES) == DF_OFF) && (ComTrafReservedColName(renamedColName))) {
  ...
  char buf[4000];
  str_sprintf(buf,
              "update %s.\"%s\".%s set column_name = '%s', "
              " column_class = case when column_class in ('" COM_ADDED_USER_COLUMN_LIT
              "', '" COM_ADDED_ALTERED_USER_COLUMN_LIT "') then '" COM_ADDED_ALTERED_USER_COLUMN_LIT
              "' else '" COM_ALTERED_USER_COLUMN_LIT "' end where object_uid = %ld and column_number = %d",
              getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_COLUMNS, renamedColName.data(), objUID, colNumber);
  ...
  if (isPkeyCol) {
    int divColPos = -1;
    if ((naTable->hasDivisioningColumn(&divColPos)) || naTable->hasTrafReplicaColumn()) {
      NAString reason;
    ...
    if ((naTable->hasSaltedColumn(&saltColPos))) {
      NAString saltText;
      cliRC = getTextFromMD(&cliInterface, objUID, COM_COMPUTED_COL_TEXT, saltColPos, saltText);
      if (cliRC < 0) {
        processReturn();
        return;
      }
      ...
      cliRC = updateTextTable(&cliInterface, objUID, COM_COMPUTED_COL_TEXT, saltColPos, saltText, NULL, -1, TRUE);
      if (cliRC < 0) {
        processReturn();
        return;
      }
    ...
    cliRC = updatePKeysTable(&cliInterface, renamedColName.data(), objUID, colName.data());
    if (cliRC < 0) {
      processReturn();
      return;
    }
    ...
    // as PK
    cliRC = getPKeyInfoForTable(catalogNamePart.data(), schemaNamePart.data(), objectNamePart.data(), &cliInterface,
                                pkName, pkUid, FALSE);
    if (cliRC < 0) {
      processReturn();
      return;
    ...
    if (pkUid > 0) {
      cliRC = updatePKeysTable(&cliInterface, renamedColName.data(), pkUid, colName.data());
      if (cliRC < 0) {
        processReturn();
        return;
      }
  ...
  if (naTable->hasSecondaryIndexes()) {
    ...
    for (int i = 0; i < naFsList.entries(); i++) {
      ...
      str_sprintf(
          buf, "update %s.\"%s\".%s set column_name = '%s' || '@' where object_uid = %ld and column_name = '%s' || '@'",
          getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_COLUMNS, renamedColName.data(), naFS->getIndexUID(),
          colName.data());
      ...
      // update KEYS
      if (isPkeyCol) {
        cliRC = updatePKeysTable(&cliInterface, renamedColName.data(), naFS->getIndexUID(), colName.data(),
                                 TRUE /*index column*/);
        if (cliRC < 0) {
          processReturn();
          return;
  ...
  if (isSampleExist(catalogNamePart, schemaNamePart, objUID, sampleTableName, sampleTableUid)) {
    ...
    str_sprintf(dbuf, "alter table %s alter column %s rename to %s", sampleTableName.data(), colName.data(),
                renamedColName.data());
    cliRC = cliInterface.executeImmediate(dbuf);
    if (cliRC < 0) {
      return;
...
// ----------------------------------------------------------------------------
void CmpSeabaseDDL::alterSeabaseTableAddPKeyConstraint(StmtDDLAddConstraint *alterAddConstraint, NAString &currCatName,
                                                       NAString &currSchName) {
  int cliRC = 0;
  int cliRC2 = 0;
  int retcode = 0;
  ...
  retcode =
      setupAndErrorChecks(tabName, alterAddConstraint->getOrigTableNameAsQualifiedName(), currCatName, currSchName,
                          catalogNamePart, schemaNamePart, objectNamePart, extTableName, extNameForHbase, cn, &naTable,
                          FALSE, FALSE, &cliInterface, COM_BASE_TABLE_OBJECT, SQLOperation::ALTER_TABLE);
  if (retcode < 0) {
    processReturn();
  ...
  ElemDDLColRefArray &keyColumnArray =
      alterAddConstraint->getConstraint()->castToElemDDLConstraintPK()->getKeyColumnArray();
  ...
  pkeyColsStr += "(";
  for (int j = 0; j < keyColumnArray.entries(); j++) {
    ...
    if (j < (keyColumnArray.entries() - 1)) pkeyColsStr += ", ";
  }
  pkeyColsStr += ")";

  if (constraintErrorChecks(&cliInterface, alterAddConstraint->castToStmtDDLAddConstraintPK(), naTable,
  ...
  Queue *usingViewsQueue = NULL;
  cliRC = getUsingViews(&cliInterface, naTable->objectUid().castToInt64(), usingViewsQueue);
  if (cliRC < 0) {
    processReturn();
  ...
  if ((naTable->hasSecondaryIndexes()) ||                           // user indexes
      (naTable->getUniqueConstraints().entries() > 0) ||            // unique constraints
      (naTable->getRefConstraints().entries() > 0) ||               // ref constraints
      (usingViewsQueue->entries() > 0) || (naTable->hasTrigger()))  // exists trigger
  {
  ...
  if (retcode < 0) {
    *CmpCommon::diags() << DgSqlCode(-8448) << DgString0((char *)"ExpHbaseInterface::isEmpty()")
                        << DgString1(getHbaseErrStr(-retcode)) << DgInt0(-retcode)
                        << DgString2((char *)GetCliGlobals()->getJniErrorStr());
  ...
  if (addPkeyAsUniqConstr) {
    ...
    if (naTable->isStoredDesc()) {
      int32_t diagsMark = CmpCommon::diags()->mark();
      deleteFromSharedCache(&cliInterface, naTable->getTableName().getQualifiedNameAsAnsiString(),
                            COM_BASE_TABLE_OBJECT, "add primary key column as unique constraint");
    ...
    NAString cliQuery;
    cliQuery = "alter table " + extTableName + " add constraint " + uniqueStr + " unique " + pkeyColsStr + ";";
    cliRC = cliInterface.executeImmediate((char *)cliQuery.data());
    if (cliRC < 0) {
      cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
    }
    ...
    *CmpCommon::diags() << DgSqlCode(-3242)
                        << DgString0(
                               "Cannot alter/add primary key constraint on a table with dependencies. Drop all "
                               "dependent objects (views, indexes, unique, trigger and referential constraints) on the "
                               "specified table and recreate them after adding the primary key.");
  ...
  long tableUID = getObjectUID(&cliInterface, catalogNamePart.data(), schemaNamePart.data(), objectNamePart.data(),
                               COM_BASE_TABLE_OBJECT_LIT);
  ...
  str_sprintf(cqdbuf, "cqd traf_create_table_with_uid '%ld';", tableUID);
  cliRC = cliInterface.executeImmediate(cqdbuf);
  if (cliRC < 0) {
    cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
  ...
  if (updateObjectRedefTime(&cliInterface, catalogNamePart, schemaNamePart, objectNamePart, COM_BASE_TABLE_OBJECT_LIT,
                            -1, tableUID, naTable->isStoredDesc(), naTable->storedStatsAvail())) {
    processReturn();
...
// DDL lock.
// ----------------------------------------------------------------------------
void CmpSeabaseDDL::alterSeabaseTableAddUniqueConstraint(StmtDDLAddConstraint *alterAddConstraint,
                                                         NAString &currCatName, NAString &currSchName) {
  int cliRC = 0;
  int retcode = 0;
  ...
  ElemDDLColRefArray &keyColumnArray =
      alterAddConstraint->getConstraint()->castToElemDDLConstraintUnique()->getKeyColumnArray();
  ...
  if (constraintErrorChecks(&cliInterface, alterAddConstraint->castToStmtDDLAddConstraintUnique(), naTable,
                            COM_UNIQUE_CONSTRAINT, keyColList)) {
    return;
  }
  ...
  int constrNum =
      ((naTable->xnRepl() != COM_REPL_NONE || naTable->incrBackupEnabled()) ? getNextUnnamedConstrNum(naTable) : 0);
  ...
  ComUID comUID;
  comUID.make_UID();
  long uniqueUID = comUID.get_value();

  if (updateConstraintMD(keyColList, keyColOrderList, uniqueStr, tableUID, uniqueUID, naTable, COM_UNIQUE_CONSTRAINT,
                         TRUE, &cliInterface)) {
    *CmpCommon::diags() << DgSqlCode(-1043) << DgTableName(uniqueStr);
  ...
  if (updateIndexInfo(keyColList, keyColOrderList, emptyKeyColList, uniqueStr, uniqueUID, catalogNamePart,
                      schemaNamePart, objectNamePart, naTable, TRUE,
                      (CmpCommon::getDefault(TRAF_NO_CONSTR_VALIDATION) == DF_ON), TRUE, FALSE, &cliInterface)) {
    *CmpCommon::diags() << DgSqlCode(-1029) << DgTableName(uniqueStr);
    // restore the flag before using it
    if (needRestoreIBAttr == true) {
      naTable->setIncrBackupEnabled(TRUE);
      addIBAttrForTable(naTable, &cliInterface);
  ...
  if (naTable->isPartitionV2Table()) {
    RemoveNATableParam removeNATableParam(ComQiScope::REMOVE_FROM_ALL_USERS, alterAddConstraint->ddlXns(), FALSE);
    UInt32 operation = 0;
    if (naTable->isStoredDesc()) {
    ...
    for (int i = 0; i < partArray.entries(); i++) {
      if (partArray[i]->hasSubPartition()) {
        ...
        for (int j = 0; j < subPartAry->entries(); j++) {
          UpdateObjRefTimeParam updObjRefParam(-1, (*subPartAry)[j]->getPartitionUID(), naTable->isStoredDesc(),
                                               naTable->storedStatsAvail());
          cliRC = updateCachesAndStoredDesc(
              &cliInterface, catalogNamePart, schemaNamePart, NAString((*subPartAry)[j]->getPartitionEntityName()),
              COM_BASE_TABLE_OBJECT, operation, &removeNATableParam, &updObjRefParam, SharedCacheDDLInfo::DDL_UPDATE);
          if (cliRC < 0) {
            processReturn();
            return;
          }
        ...
        UpdateObjRefTimeParam updObjRefParam(-1, partArray[i]->getPartitionUID(), naTable->isStoredDesc(),
                                             naTable->storedStatsAvail());
        cliRC = updateCachesAndStoredDesc(
            &cliInterface, catalogNamePart, schemaNamePart, NAString(partArray[i]->getPartitionEntityName()),
            COM_BASE_TABLE_OBJECT, operation, &removeNATableParam, &updObjRefParam, SharedCacheDDLInfo::DDL_UPDATE);
        if (cliRC < 0) {
          processReturn();
          return;
...
// return -1, if error.
short CmpSeabaseDDL::isCircularDependent(CorrName &ringTable, CorrName &refdTable, CorrName &origRingTable,
                                         BindWA *bindWA) {
  ...
  for (int i = 0; i < refList.entries(); i++) {
    ...
    short rc = isCircularDependent(cn, cn, origRingTable, bindWA);
    if (rc) return rc;
...
// DDL lock.
// ----------------------------------------------------------------------------
void CmpSeabaseDDL::alterSeabaseTableAddRIConstraint(StmtDDLAddConstraint *alterAddConstraint, NAString &currCatName,
                                                     NAString &currSchName) {
  int cliRC = 0;
  int retcode = 0;
  ...
  if ((isSeabaseReservedSchema(referencingTableName)) && (!Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL))) {
    *CmpCommon::diags() << DgSqlCode(-CAT_CANNOT_ALTER_DEFINITION_METADATA_SCHEMA);
    processReturn();
    return;
  }
  ...
  BindWA bindWA(ActiveSchemaDB(), CmpCommon::context(), TRUE /*inDDL*/);
  CorrName cn(referencingTableName.getObjectNamePart().getInternalName(), STMTHEAP,
  ...
  if (!isDDLOperationAuthorized(SQLOperation::ALTER_TABLE, ringNaTable->getOwner(), ringNaTable->getSchemaOwner(),
                                (int64_t)ringNaTable->objectUid().get_value(), COM_BASE_TABLE_OBJECT,
                                ringNaTable->getPrivInfo())) {
    *CmpCommon::diags() << DgSqlCode(-CAT_NOT_AUTHORIZED);
  ...
  if (ringNaTable->isHbaseMapTable()) {
    // not supported
    *CmpCommon::diags() << DgSqlCode(-3242) << DgString0("This alter option cannot be used for an HBase mapped table.");
  ...
  const ElemDDLConstraintRI *constraintNode = alterAddConstraint->getConstraint()->castToElemDDLConstraintRI();
  ComObjectName referencedTableName(constraintNode->getReferencedTableName(), COM_TABLE_NAME);
  referencedTableName.applyDefaults(currCatAnsiName, currSchAnsiName);

  if ((isSeabaseReservedSchema(referencedTableName)) && (!Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL))) {
    *CmpCommon::diags() << DgSqlCode(-CAT_CANNOT_ALTER_DEFINITION_METADATA_SCHEMA);
    processReturn();
    return;
  }

  retcode = lockObjectDDL(referencedTableName, COM_BASE_TABLE_OBJECT, alterAddConstraint->isVolatile(),
  ...
  retcode =
      existsInSeabaseMDTable(&cliInterface, referencedTableName.getCatalogNamePart().getInternalName(),
                             referencedTableName.getSchemaNamePart().getInternalName(),
                             referencedTableName.getObjectNamePart().getInternalName(), COM_BASE_TABLE_OBJECT, TRUE);
  if (retcode < 0) {
    processReturn();
  ...
  CorrName cn2(referencedTableName.getObjectNamePart().getInternalName(), STMTHEAP,
               referencedTableName.getSchemaNamePart().getInternalName(),
               referencedTableName.getCatalogNamePart().getInternalName());

  NATable *refdNaTable = bindWA.getNATable(cn2);
  if (refdNaTable == NULL || bindWA.errStatus()) {
    *CmpCommon::diags() << DgSqlCode(-4082) << DgTableName(cn2.getExposedNameAsAnsiString());
  ...
  if (refdNaTable->getViewText()) {
    *CmpCommon::diags() << DgSqlCode(-1127) << DgTableName(cn2.getExposedNameAsAnsiString());
  ...
  if ((CmpCommon::getDefault(TRAF_ALLOW_SELF_REF_CONSTR) == DF_OFF) && (referencingTableName == referencedTableName)) {
  ...
  if (isAuthorizationEnabled()) {
    ...
    if (!ComUser::isRootUserID() && !privs->hasReferencePriv()) noObjPriv = TRUE;
  }

  ElemDDLColNameArray &ringCols =
      alterAddConstraint->getConstraint()->castToElemDDLConstraintRI()->getReferencingColumns();
  ...
  for (int j = 0; j < ringCols.entries(); j++) {
    ...
    ringColListForValidation += "\"";
    if (j < (ringCols.entries() - 1)) ringColListForValidation += ", ";
  ...
  if (constraintErrorChecks(&cliInterface, alterAddConstraint->castToStmtDDLAddConstraintRI(), ringNaTable,
                            COM_FOREIGN_KEY_CONSTRAINT,  // FALSE, // referencing constr
                            ringKeyColList)) {
    return;
  }

  const NAString &addConstrName = alterAddConstraint->getConstraintNameAsQualifiedName().getQualifiedNameAsAnsiString();
  ...
  const ElemDDLColNameArray &referencedColNode = constraintNode->getReferencedColumns();

  NAList<NAString> refdKeyColList(HEAP, referencedColNode.entries());
  NAString refdColListForValidation;
  for (int j = 0; j < referencedColNode.entries(); j++) {
    ...
    refdColListForValidation += "\"";
    if (j < (referencedColNode.entries() - 1)) refdColListForValidation += ", ";
  }
  ...
  if (referencedColNode.entries() == 0) {
    ...
    for (int i = 0; i < naf->getIndexKeyColumns().entries(); i++) {
      ...
      refdColListForValidation += "\"";
      if (i < (naf->getIndexKeyColumns().entries() - 1)) refdColListForValidation += ", ";
    }
  }

  if (ringKeyColList.entries() != refdKeyColList.entries()) {
    *CmpCommon::diags() << DgSqlCode(-1046) << DgConstraintName(addConstrName);
  ...
  for (int i = 0; i < ringKeyColList.entries(); i++) {
    ...
    if (!refdNAC) {
      *CmpCommon::diags() << DgSqlCode(-1009) << DgColumnName(refdColName);
      processReturn();
      return;
    }
    ...
    if (noObjPriv) {
      ...
      if (!privs->hasColReferencePriv(refdNAC->getPosition())) {
        *CmpCommon::diags() << DgSqlCode(-4481) << DgString0("REFERENCES")
  ...
  if (NOT refdNaTable->getCorrespondingConstraint(refdKeyColList,
                                                  TRUE,  // unique constraint
                                                  &constrName, &isPkey, &reorderList)) {
    *CmpCommon::diags() << DgSqlCode(-CAT_REFERENCED_CONSTRAINT_DOES_NOT_EXIST) << DgConstraintName(addConstrName);
  ...
  if (reorderList.entries() > 0) {
    ...
    CMPASSERT(ringKeyColOrderList.entries() == numEntries && refdKeyColList.entries() == numEntries &&
  ...
  {
    *CmpCommon::diags() << DgSqlCode(-CAT_RI_CIRCULAR_DEPENDENCY) << DgConstraintName(addConstrName)
  ...
  if (gEnableRowLevelLock) {
    lockRequired(ringNaTable, catalogNamePart.data(), schemaNamePart.data(), objectNamePart.data(),
                 HBaseLockMode::LOCK_X, FALSE, FALSE, FALSE, FALSE, TRUE, TRUE);
  }

  if ((CmpCommon::getDefault(TRAF_NO_CONSTR_VALIDATION) == DF_OFF) && (constraintNode->isEnforced())) {
    ...
    char *validQry = new (STMTHEAP) char[ringColListForValidation.length() + refdColListForValidation.length() +
                                         ringNullList.length() + 2000];
    const char *sql;
    if (gEnableRowLevelLock) {
      sql =
          "select count(*) from \"%s\".\"%s\".\"%s\" where not ((%s) in (select %s from \"%s\".\"%s\".\"%s\" for "
          "repeatable read access)) %s;";
    } else {
      sql = "select count(*) from \"%s\".\"%s\".\"%s\" where not ((%s) in (select %s from \"%s\".\"%s\".\"%s\")) %s;";
    }
    str_sprintf(validQry, sql, referencingTableName.getCatalogNamePart().getInternalName().data(),
                referencingTableName.getSchemaNamePart().getInternalName().data(),
                referencingTableName.getObjectNamePart().getInternalName().data(), ringColListForValidation.data(),
                refdColListForValidation.data(), referencedTableName.getCatalogNamePart().getInternalName().data(),
                referencedTableName.getSchemaNamePart().getInternalName().data(),
                referencedTableName.getObjectNamePart().getInternalName().data(), ringNullList.data());
    ...
    if (rowCount > 0) {
      *CmpCommon::diags() << DgSqlCode(-1143) << DgConstraintName(addConstrName)
  ...
  long refdConstrUID =
      getObjectUID(&cliInterface, refdCatName.data(), refdSchName.data(), refdObjName.data(),
                   (isPkey ? COM_PRIMARY_KEY_CONSTRAINT_OBJECT_LIT : COM_UNIQUE_CONSTRAINT_OBJECT_LIT));

  // create unnamed constraints in deterministic format if table was
  // created for xDC replication.
  int constrNum = ((ringNaTable->xnRepl() != COM_REPL_NONE || ringNaTable->incrBackupEnabled())
                       ? getNextUnnamedConstrNum(ringNaTable)
                       : 0);
  ...
  ComUID comUID;
  comUID.make_UID();
  long ringConstrUID = comUID.get_value();

  if (updateConstraintMD(ringKeyColList, ringKeyColOrderList, uniqueStr, tableUID, ringConstrUID, ringNaTable,
                         COM_FOREIGN_KEY_CONSTRAINT, constraintNode->isEnforced(), &cliInterface)) {
    *CmpCommon::diags() << DgSqlCode(-1029) << DgTableName(uniqueStr);

    return;
  }

  if (updateRIConstraintMD(ringConstrUID, refdConstrUID, &cliInterface)) {
    *CmpCommon::diags() << DgSqlCode(-1029) << DgTableName(uniqueStr);

    return;
  }

  if (updateIndexInfo(ringKeyColList, ringKeyColOrderList, refdKeyColList, uniqueStr, ringConstrUID, catalogNamePart,
                      schemaNamePart, objectNamePart, ringNaTable, FALSE,
                      (CmpCommon::getDefault(TRAF_NO_CONSTR_VALIDATION) == DF_ON), constraintNode->isEnforced(),
  ...
  if (updateObjectRedefTime(&cliInterface, catalogNamePart, schemaNamePart, objectNamePart, COM_BASE_TABLE_OBJECT_LIT,
                            -1, tableUID, ringNaTable->isStoredDesc(), ringNaTable->storedStatsAvail())) {
    processReturn();
  ...
  if (refdNaTable->isStoredDesc() && (!Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL))) {
    insertIntoSharedCacheList(referencedTableName.getCatalogNamePart().getInternalName(),
                              referencedTableName.getSchemaNamePart().getInternalName(),
                              referencedTableName.getObjectNamePart().getInternalName(), COM_BASE_TABLE_OBJECT,
                              SharedCacheDDLInfo::DDL_UPDATE);
  }
  ...
  if (updateObjectRedefTime(&cliInterface, referencedTableName.getCatalogNamePart().getInternalName(),
                            referencedTableName.getSchemaNamePart().getInternalName(),
                            referencedTableName.getObjectNamePart().getInternalName(), COM_BASE_TABLE_OBJECT_LIT, -1,
                            refdNaTable->objectUid().castToInt64(), refdNaTable->isStoredDesc(),
                            refdNaTable->storedStatsAvail())) {
    processReturn();
...
short CmpSeabaseDDL::getCheckConstraintText(StmtDDLAddConstraintCheck *addCheckNode, NAString &qualifiedText,
  ...
  for (size_t x = 0; x < nameLocList.entries(); x++) {
    ...
    const NAString &nameExpanded = useNoneExpandedName ? pNameLoc->getNoneExpandedName(FALSE /*no assert*/)
    ...
    const int max_bytes2cnv = addCheckNode->getEndPosition() - pNameLoc->getNamePosition() + 1;
    ...
    qualifiedText += ComString(&pInputStr[inputStrPos], pNameLoc->getNamePosition() - inputStrPos + nameAsIs);
    ...
    if (NOT nameAsIs)  // original name to be replaced with expanded
    {
      ...
      if (pInputStr[namePos] EQU '"' AND nameExpanded.data()[0] NEQ '"' AND namePos >
      ...
      if (pInputStr[namePos + nameLen - 1] EQU '"' AND nameExpanded.data()[nameExpanded.length() - 1] NEQ '"' AND
              pInputStr[namePos + nameLen] NEQ '\0' AND(pInputStr[namePos + nameLen] EQU '_' OR isAlNumIsoMapCS(
  ...
  qualifiedText += ComString(&pInputStr[inputStrPos], addCheckNode->getEndPosition() - inputStrPos + 1);
...
short CmpSeabaseDDL::getTextFromMD(ExeCliInterface *cliInterface, long textUID, ComTextType textType, int textSubID,
                                   NAString &outText, NABoolean binaryData) {
  short retcode = getTextFromMD(getSystemCatalog(), cliInterface, textUID, textType, textSubID, outText, binaryData);

  if (retcode) processReturn();

  return retcode;
}

// static version of this method
short CmpSeabaseDDL::getTextFromMD(const char *catalogName, ExeCliInterface *cliInterface, long textUID,
                                   ComTextType textType, int textSubID, NAString &outText, NABoolean binaryData) {
  int cliRC;

  char query[1000];

  str_sprintf(query,
              "select octet_length(text), text from %s.\"%s\".%s where text_uid = %ld and text_type = %d and sub_id = "
              "%d order by seq_num",
              catalogName, SEABASE_MD_SCHEMA, SEABASE_TEXT, textUID, static_cast<int>(textType), textSubID);
  ...
  // glue text together
  NAString binaryText;
  for (int idx = 0; idx < textQueue->numEntries(); idx++) {
    OutputInfo *vi = (OutputInfo *)textQueue->getNext();

    int len = *(int *)vi->get(0);
  ...
  if (binaryData) {
    int decodedMaxLen = str_decoded_len(binaryText.length());

    char *decodedData = new (STMTHEAP) char[decodedMaxLen];
    int decodedLen = str_decode(decodedData, decodedMaxLen, binaryText.data(), binaryText.length());
    if (decodedLen < 0) return -1;

    outText.append(decodedData, decodedLen);
    if (decodedData) NADELETEBASIC(decodedData, STMTHEAP);
  }

  return 0;
}

short CmpSeabaseDDL::getPartIndexName(ExeCliInterface *cliInterface, const NAString &schemaName,
                                      const NAString &index_name, const NAString &partitionName,
  ...
  sqlBuff.format(
      "select OBJECT_NAME from %s.\"%s\".%s where object_uid = (                       "
      ...
      " );",
      TRAFODION_SYSTEM_CATALOG, SEABASE_MD_SCHEMA, SEABASE_OBJECTS, TRAFODION_SYSTEM_CATALOG, SEABASE_MD_SCHEMA,
      SEABASE_PARTITIONS, TRAFODION_SYSTEM_CATALOG, SEABASE_MD_SCHEMA, SEABASE_INDEXES, TRAFODION_SYSTEM_CATALOG,
      SEABASE_MD_SCHEMA, SEABASE_OBJECTS, schemaName.data(), index_name.data(), partitionName.data());
  ...
  if (objectsInfo && objectsInfo->numEntries() > 0) {
    ...
    *CmpCommon::diags() << DgSqlCode(-3242)
                        << DgString0("The specified index or partition does not exist or does not match");
    return -1;
  }
  return -1;
}
...
// DDL lock.
// ----------------------------------------------------------------------------
void CmpSeabaseDDL::alterSeabaseTableAddCheckConstraint(StmtDDLAddConstraint *alterAddConstraint, NAString &currCatName,
                                                        NAString &currSchName) {
  StmtDDLAddConstraintCheck *alterAddCheckNode = alterAddConstraint->castToStmtDDLAddConstraintCheck();
  ...
  const ParCheckConstraintColUsageList &colList = alterAddCheckNode->getColumnUsageList();
  for (CollIndex cols = 0; cols < colList.entries(); cols++) {
    ...
    if ((colName EQU "SYSKEY") && (naTable->getClusteringIndex()->hasSyskey())) {
      *CmpCommon::diags() << DgSqlCode(-CAT_SYSKEY_COL_NOT_ALLOWED_IN_CK_CNSTRNT) << DgColumnName("SYSKEY")
  ...
  if (constraintErrorChecks(&cliInterface, alterAddConstraint->castToStmtDDLAddConstraintCheck(), naTable,
                            COM_CHECK_CONSTRAINT, keyColList)) {
    return;
  }
  ...
  if (CmpCommon::getDefault(TRAF_NO_CONSTR_VALIDATION) == DF_OFF) {
    ...
    str_sprintf(validQry, "select count(*) from \"%s\".\"%s\".\"%s\" where not %s", catalogNamePart.data(),
                schemaNamePart.data(), objectNamePart.data(), checkConstrText.data());
    ...
    if (rowCount > 0) {
      *CmpCommon::diags() << DgSqlCode(-1083) << DgConstraintName(uniqueStr);
      return;
    }
  }
  ...
  ComUID comUID;
  comUID.make_UID();
  long checkUID = comUID.get_value();
  ...
  if (updateConstraintMD(keyColList, emptyList, uniqueStr, tableUID, checkUID, naTable, COM_CHECK_CONSTRAINT, TRUE,
                         &cliInterface)) {
    *CmpCommon::diags() << DgSqlCode(-1029) << DgTableName(uniqueStr);
  ...
  if (updateTextTable(&cliInterface, checkUID, COM_CHECK_CONSTR_TEXT, 0, checkConstrText)) {
    processReturn();
    return;
  }
...
// This statement runs in a transaction
// ----------------------------------------------------------------------------
void CmpSeabaseDDL::alterSeabaseTableDropConstraint(StmtDDLDropConstraint *alterDropConstraint, NAString &currCatName,
                                                    NAString &currSchName) {
  int cliRC = 0;
  int retcode = 0;
  ...
  retcode =
      setupAndErrorChecks(tabName, alterDropConstraint->getOrigTableNameAsQualifiedName(), currCatName, currSchName,
                          catalogNamePart, schemaNamePart, objectNamePart, extTableName, extNameForHbase, cn, &naTable,
                          FALSE, FALSE, &cliInterface, COM_BASE_TABLE_OBJECT, SQLOperation::ALTER_TABLE);
  if (retcode < 0) {
    processReturn();
  ...
  const NAString &dropConstrName =
      alterDropConstraint->getConstraintNameAsQualifiedName().getQualifiedNameAsAnsiString();
  const NAString &constrCatName = alterDropConstraint->getConstraintNameAsQualifiedName().getCatalogName();
  const NAString &constrSchName = alterDropConstraint->getConstraintNameAsQualifiedName().getSchemaName();
  const NAString &constrObjName = alterDropConstraint->getConstraintNameAsQualifiedName().getObjectName();

  char outObjType[10];
  long constrUID = getObjectUID(
      &cliInterface, constrCatName.data(), constrSchName.data(), constrObjName.data(), NULL, NULL,
      "object_type = '" COM_PRIMARY_KEY_CONSTRAINT_OBJECT_LIT "' or object_type = '" COM_UNIQUE_CONSTRAINT_OBJECT_LIT
      "' or object_type = '" COM_REFERENTIAL_CONSTRAINT_OBJECT_LIT
      "' or object_type = '" COM_CHECK_CONSTRAINT_OBJECT_LIT "' ",
      outObjType);
  if (constrUID < 0) {
    *CmpCommon::diags() << DgSqlCode(-1005) << DgConstraintName(dropConstrName);
  ...
  NABoolean isUniqConstr = ((strcmp(outObjType, COM_UNIQUE_CONSTRAINT_OBJECT_LIT) == 0) ||
                            (strcmp(outObjType, COM_PRIMARY_KEY_CONSTRAINT_OBJECT_LIT) == 0));
  NABoolean isRefConstr = (strcmp(outObjType, COM_REFERENTIAL_CONSTRAINT_OBJECT_LIT) == 0);
  NABoolean isPkeyConstr = (strcmp(outObjType, COM_PRIMARY_KEY_CONSTRAINT_OBJECT_LIT) == 0);
  NABoolean isCheckConstr = (strcmp(outObjType, COM_CHECK_CONSTRAINT_OBJECT_LIT) == 0);
  ...
  if (isUniqConstr) {
    ...
    for (int i = 0; i < ariList.entries(); i++) {
      ...
      const NAString &tableConstrName = uniqueConstr->getConstraintName().getQualifiedNameAsAnsiString();

      if (dropConstrName == tableConstrName) {
        constrFound = TRUE;
        if (uniqueConstr->hasRefConstraintsReferencingMe()) {
          *CmpCommon::diags() << DgSqlCode(-1050);
    ...
    if (isPkeyConstr) {
      *CmpCommon::diags() << DgSqlCode(-1255) << DgString0(dropConstrName) << DgString1(extTableName);
  ...
  if (isRefConstr) {
    ...
    retcode = lockObjectDDL(otherCN.getQualifiedNameObj(), COM_BASE_TABLE_OBJECT, alterDropConstraint->isVolatile(),
    ...
    const NAString &otherCatName = otherConstr->getConstraintName().getCatalogName();

    const NAString &otherSchName = otherConstr->getConstraintName().getSchemaName();

    const NAString &otherConstrName = otherConstr->getConstraintName().getObjectName();

    otherConstrUID = getObjectUID(&cliInterface, otherCatName.data(), otherSchName.data(), otherConstrName.data(),
                                  COM_UNIQUE_CONSTRAINT_OBJECT_LIT);
    if (otherConstrUID < 0) {
      CmpCommon::diags()->clear();
      otherConstrUID = getObjectUID(&cliInterface, otherCatName.data(), otherSchName.data(), otherConstrName.data(),
  ...
  if ((isUniqConstr || isRefConstr) && (NOT isPkeyConstr)) {
    ...
    str_sprintf(
        query,
        "select I.keytag, I.is_explicit from %s.\"%s\".%s T, %s.\"%s\".%s I /*+ cardinality 1e9 */ where T.table_uid = "
        "%ld and T.constraint_uid = %ld and T.table_uid = I.base_table_uid and T.index_uid = I.index_uid ",
        getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_TABLE_CONSTRAINTS, getSystemCatalog(), SEABASE_MD_SCHEMA,
        SEABASE_INDEXES, naTable->objectUid().castToInt64(), constrUID);
    ...
    if (indexQueue->numEntries() == 1) {
      ...
      keytag = *(int *)oi->get(0);
      isExplicit = *(int *)oi->get(1);
    }
  }

  if (deleteConstraintInfoFromSeabaseMDTables(
          &cliInterface, naTable->objectUid().castToInt64(),
          (otherNaTable ? otherNaTable->objectUid().castToInt64() : 0), constrUID, otherConstrUID, constrCatName,
          constrSchName, constrObjName,
          (isPkeyConstr
               ? COM_PRIMARY_KEY_CONSTRAINT_OBJECT
               : (isUniqConstr ? COM_UNIQUE_CONSTRAINT_OBJECT
                               : (isRefConstr ? COM_REFERENTIAL_CONSTRAINT_OBJECT : COM_CHECK_CONSTRAINT_OBJECT))))) {
    processReturn();
  ...
  if (((indexFound) && (NOT isExplicit) && (keytag != 0)) &&
    ...
    str_sprintf(buf, "drop index \"%s\".\"%s\".\"%s\" no check", constrCatName.data(), constrSchName.data(),
                constrObjName.data());
    ...
    ActiveSchemaDB()->getNATableDB()->removeNATable(idxName, ComQiScope::REMOVE_FROM_ALL_USERS, COM_INDEX_OBJECT,
                                                    alterDropConstraint->ddlXns(), FALSE);
    ActiveSchemaDB()->getNATableDB()->removeNATable(idxName, ComQiScope::REMOVE_MINE_ONLY, COM_INDEX_OBJECT,
                                                    alterDropConstraint->ddlXns(), FALSE);

    insertIntoSharedCacheList(constrCatName, constrSchName, constrObjName, COM_INDEX_OBJECT,
                              SharedCacheDDLInfo::DDL_DELETE, SharedCacheDDLInfo::SHARED_DATA_CACHE);
  }
  // restore the flag before using it
  if (needRestoreIBAttr == true) {
  ...
  if (naTable->isPartitionV2Table()) {
    RemoveNATableParam removeNATableParam(ComQiScope::REMOVE_FROM_ALL_USERS, alterDropConstraint->ddlXns(), FALSE);
    UInt32 operation = 0;
    if (naTable->isStoredDesc()) {
      setCacheAndStoredDescOp(operation, UPDATE_REDEF_TIME);
      setCacheAndStoredDescOp(operation, UPDATE_SHARED_CACHE);
  ...
  if (isRefConstr && otherNaTable) {
    CorrName otherCn(otherNaTable->getExtendedQualName().getQualifiedNameObj(), STMTHEAP);

    if (updateObjectRedefTime(&cliInterface, otherCn.getQualifiedNameObj().getCatalogName(),
                              otherCn.getQualifiedNameObj().getSchemaName(),
                              otherCn.getQualifiedNameObj().getObjectName(), COM_BASE_TABLE_OBJECT_LIT, -1,
                              otherNaTable->objectUid().castToInt64(), otherNaTable->isStoredDesc(),
                              otherNaTable->storedStatsAvail())) {
      processReturn();
    ...
    if (naTable->isStoredDesc() && (!Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL))) {
      insertIntoSharedCacheList(
          otherCn.getQualifiedNameObj().getCatalogName(), otherCn.getQualifiedNameObj().getSchemaName(),
          otherCn.getQualifiedNameObj().getObjectName(), COM_BASE_TABLE_OBJECT, SharedCacheDDLInfo::DDL_UPDATE);
    }
...
void CmpSeabaseDDL::seabaseGrantRevoke(StmtDDLNode *stmtDDLNode, NABoolean isGrant, NAString &currCatName,
                                       NAString &currSchName, NABoolean internalCall) {
  int retcode = 0;
  ...
  CorrName cn(origTableName.getObjectNamePart().getInternalName(), STMTHEAP,
  ...
  if (objectType == COM_BASE_TABLE_OBJECT) {
    ...
    if (naTable->isView()) objectType = COM_VIEW_OBJECT;
  ...
  if ((catalogNamePart == HIVE_SYSTEM_CATALOG) && NATable::usingSentry()) {
    *CmpCommon::diags() << DgSqlCode(-CAT_SENTRY_NOT_SUPPORTED) << DgString0(isGrant ? "GRANT" : "REVOKE");
    return;
  }
  ...
  else if (!checkSpecifiedPrivs(privActsArray, extTableName.data(), objectType, naTable, objectPrivs, colPrivs)) {
    processReturn();
    return;
  }
  ...
  // return an error
  if (naTable && (naTable->getTableName().isHive() || naTable->getTableName().isHbase()) && (colPrivs.size() > 0)) {
    if (hasValue(colPrivs, INSERT_PRIV) || hasValue(colPrivs, UPDATE_PRIV) || hasValue(colPrivs, REFERENCES_PRIV)) {
      NAString text1("INSERT, UPDATE, REFERENCES");
      NAString text2("Hive columns on");
      *CmpCommon::diags() << DgSqlCode(-CAT_INVALID_PRIV_FOR_OBJECT) << DgString0(text1.data())
                          << DgString1(text2.data()) << DgTableName(extTableName);
      processReturn();
      return;
    }
  }
  ...
  NABoolean isMDTable = (isSeabaseMD(tableName) || isSeabasePrivMgrMD(tableName));
  ...
  if (objectUID == 0 && naTable && naTable->isHiveTable() &&
    ...
    snprintf(query, sizeof(query), "register internal hive %s if not exists %s.\"%s\".\"%s\"",
             (naTable->isView() ? "view" : "table"), catalogNamePart.data(), schemaNamePart.data(),
             objectNamePart.data());
    int retcode = cliInterface.executeImmediate(query);
    if (retcode < 0) {
      cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
      return;
    }
  ...
  if (objectUID == 0 && naTable && ((naTable->isHbaseCellTable()) || (naTable->isHbaseRowTable()))) {
    ...
    // register this hive table in traf metadata
    char query[(ComMAX_ANSI_IDENTIFIER_EXTERNAL_LEN * 4) + 100];
    snprintf(query, sizeof(query), "register internal hbase table if not exists %s.\"%s\".\"%s\"",
  ...
  if (objectUID == 0) {
    ExeCliInterface cliInterface(STMTHEAP, 0, NULL, CmpCommon::context()->sqlSession()->getParentQid());
    objectUID = getObjectInfo(&cliInterface, catalogNamePart.data(), schemaNamePart.data(), objectNamePart.data(),
                              objectType, objectOwnerID, schemaOwnerID, objectFlags, objDataUID);
  ...
  for (CollIndex j = 0; j < pGranteeArray.entries(); j++) {
    ...
    if (pGranteeArray[j]->isPublic()) {
      ...
      if (CmpSeabaseDDLauth::isTenantID(grantee) || CmpSeabaseDDLauth::isUserGroupID(grantee)) {
    ...
    if (isGrant) {
      PrivStatus result =
          command.grantObjectPrivilege(objectUID, objectName, objectType, grantee, granteeName, effectiveGrantorID,
                                       effectiveGrantorName, objectPrivs, colPrivs, allPrivs, isWGOSpecified);
    } else {
      PrivStatus result =
          command.revokeObjectPrivilege(objectUID, objectName, objectType, grantee, granteeName, effectiveGrantorID,
                                        effectiveGrantorName, objectPrivs, colPrivs, allPrivs, isWGOSpecified);
    }
  }
  ...
  if (isHbase(tableName)) hbaseGrantRevoke(stmtDDLNode, isGrant, currCatName, currSchName);

  // update xdc ddl
  updateSeabaseXDCDDLTable(&cliInterface, TRAFODION_SYSCAT_LIT, XDC_AUTH_SCHEMA, XDC_AUTH_TABLE,
  ...
  if (updateObjectRedefTime(&cliInterface, catalogNamePart, schemaNamePart, objectNamePart, objectTypeLit, -1,
                            objectUID, (naTable ? naTable->isStoredDesc() : FALSE),
...
void CmpSeabaseDDL::hbaseGrantRevoke(StmtDDLNode *stmtDDLNode, NABoolean isGrant, NAString &currCatName,
                                     NAString &currSchName) {
  int cliRC = 0;
  int retcode = 0;
  ...
  if (allPrivs) {
    ...
    for (int i = 0; i < pPrivActsArray.entries(); i++) {
      switch (pPrivActsArray[i]->getOperatorType()) {
        ...
        default: {
          ...
          *CmpCommon::diags() << DgSqlCode(-CAT_INVALID_PRIV_FOR_OBJECT) << DgString0(privType)
  ...
  retcode = ehi->close();
  if (retcode < 0) {
    *CmpCommon::diags() << DgSqlCode(-8448) << DgString0((char *)"ExpHbaseInterface::close()")
                        << DgString1(getHbaseErrStr(-retcode)) << DgInt0(-retcode)
                        << DgString2((char *)GetCliGlobals()->getJniErrorStr());
...
void CmpSeabaseDDL::createNativeHbaseTable(ExeCliInterface *cliInterface, StmtDDLCreateHbaseTable *createTableNode,
                                           NAString &currCatName, NAString &currSchName) {
  int retcode = 0;
  int cliRC = 0;
  ...
  if (isAuthorizationEnabled() && !ComUser::isRootUserID() && !ComUser::currentUserHasRole(ROOT_ROLE_ID) &&
      !ComUser::currentUserHasRole(HBASE_ROLE_ID)) {
    *CmpCommon::diags() << DgSqlCode(-CAT_NOT_AUTHORIZED);
    processReturn();
    return;
  ...
  // Verify that user has privilege to create HBase tables - must be DB__ROOT
  // or granted the DB__HBASEROLE
  if (isAuthorizationEnabled() && !ComUser::isRootUserID() && !ComUser::currentUserHasRole(HBASE_ROLE_ID)) {
    *CmpCommon::diags() << DgSqlCode(-CAT_NOT_AUTHORIZED);
    processReturn();
    return;
  }
  ...
  if (retcode) {
    *CmpCommon::diags() << DgSqlCode(CAT_TABLE_ALREADY_EXISTS) << DgTableName(objectNamePart.data());
    deallocEHI(ehi);
    processReturn();
    return;
  }
  ...
  std::vector<NAString> colFamVec;
  for (int i = 0; i < createTableNode->csl()->entries(); i++) {
  ...
  NAList<HbaseCreateOption *> hbaseCreateOptions(STMTHEAP);
  NAString hco;
  retcode = setupHbaseOptions(createTableNode->getHbaseOptionsClause(), 0, objectNamePart, hbaseCreateOptions, hco);
  if (retcode) {
    deallocEHI(ehi);
    processReturn();
  ...
  if (createHbaseTable(ehi, &hbaseTable, colFamVec, &hbaseCreateOptions) == -1) {
    deallocEHI(ehi);
  ...
  snprintf(query, sizeof(query), "register internal hbase table if not exists \"%s\"", objectNamePart.data());
  cliRC = cliInterface->executeImmediate(query);
  if (cliRC < 0) {
    cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
    return;
...
void CmpSeabaseDDL::dropNativeHbaseTable(ExeCliInterface *cliInterface, StmtDDLDropHbaseTable *dropTableNode,
                                         NAString &currCatName, NAString &currSchName) {
  int retcode = 0;
  int cliRC = 0;
  ...
  retcode = existsInHbase(objectNamePart, ehi);
  if (retcode == 0) {
    *CmpCommon::diags() << DgSqlCode(CAT_TABLE_DOES_NOT_EXIST_ERROR) << DgTableName(objectNamePart.data());
    deallocEHI(ehi);
    processReturn();
    return;
  }
  ...
  // unregister tables
  char query[(ComMAX_ANSI_IDENTIFIER_EXTERNAL_LEN * 4) + 100];
  snprintf(query, sizeof(query), "unregister hbase table %s", tableName.getObjectNamePart().getExternalName().data());
  cliRC = cliInterface->executeImmediate(query);
  ...
  snprintf(query, sizeof(query), "drop external table if exists %s ",
  ...
  if (colonPos) {
    ...
    mapTable = NAString(&objectNamePart.data()[colonPos - objectNamePart + 1]);
  } else
    mapTable = objectNamePart;

  snprintf(query, sizeof(query), "drop external table if exists %s ", mapTable.data());
  cliRC = cliInterface->executeImmediate(query);
  if (cliRC < 0) {
    cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
    deallocEHI(ehi);
  ...
  if (naCellTable) {
    ActiveSchemaDB()->getNATableDB()->removeNATable(cnCell, ComQiScope::REMOVE_FROM_ALL_USERS, COM_BASE_TABLE_OBJECT,
                                                    dropTableNode->ddlXns(), FALSE);
  }

  if (naRowTable) {
    ActiveSchemaDB()->getNATableDB()->removeNATable(cnRow, ComQiScope::REMOVE_FROM_ALL_USERS, COM_BASE_TABLE_OBJECT,
                                                    dropTableNode->ddlXns(), FALSE);
  }
...
short CmpSeabaseDDL::registerNativeTable(const NAString &catalogNamePart, const NAString &schemaNamePart,
                                         const NAString &objectNamePart, int objOwnerId, int schemaOwnerId,
  ...
  long flags = 0;
  if (isRegister && isInternal) flags = MD_OBJECTS_INTERNAL_REGISTER;

  long objUID = -1;
  retcode = updateSeabaseMDObjectsTable(&cliInterface, catalogNamePart.data(), schemaNamePart.data(),
                                        objectNamePart.data(), objType, NULL, objOwnerId, schemaOwnerId, flags, objUID);
  if (retcode < 0) return -1;
  ...
  if (isAuthorizationEnabled()) {
    ...
    if (!insertPrivMgrInfo(objUID, fullName, objType, objOwnerId, schemaOwnerId, ComUser::getCurrentUser())) {
      *CmpCommon::diags() << DgSqlCode(-CAT_UNABLE_TO_GRANT_PRIVILEGES) << DgTableName(objectNamePart);
      return -1;
    }
  }
...
short CmpSeabaseDDL::unregisterNativeTable(const NAString &catalogNamePart, const NAString &schemaNamePart,
  ...
  if (isAuthorizationEnabled()) {
    ...
    fullName += ".";
    fullName += objectNamePart;
    if (!deletePrivMgrInfo(fullName, objUID, objType)) {
      *CmpCommon::diags() << DgSqlCode(-CAT_PRIVILEGE_NOT_REVOKED) << DgTableName(objectNamePart);
      return -1;
    }
  }

  // delete hist stats, if HIST tables exist
  retcode = existsInSeabaseMDTable(&cliInterface, HIVE_STATS_CATALOG, HIVE_STATS_SCHEMA_NO_QUOTES, HBASE_HIST_NAME,
                                   COM_BASE_TABLE_OBJECT);
  if (retcode < 0) return -1;

  if (retcode == 1)  // exists
  {
    if (dropSeabaseStats(&cliInterface, HIVE_STATS_CATALOG, HIVE_STATS_SCHEMA_NO_QUOTES, objUID)) {
      return -1;
    }
  }

  // drop from metadata
  retcode = deleteFromSeabaseMDObjectsTable(&cliInterface, catalogNamePart.data(), schemaNamePart.data(),
                                            objectNamePart.data(), objType);

  return 0;
}

short CmpSeabaseDDL::unregisterHiveSchema(const NAString &catalogNamePart, const NAString &schemaNamePart,
  ...
  if (cascade) {
    ...
    for (int i = 0; i < objectsInfo->numEntries(); i++) {
      ...
      if (strcmp(objType, COM_BASE_TABLE_OBJECT_LIT) == 0) {
        retcode = unregisterNativeTable(catalogNamePart, schemaNamePart, objName, cliInterface, COM_BASE_TABLE_OBJECT);
      }
  ...
  retcode = unregisterNativeTable(catalogNamePart, schemaNamePart, SEABASE_SCHEMA_OBJECTNAME, cliInterface,
...
//////////////////////////////////////////////////////////////////////////
short CmpSeabaseDDL::getSpecialTableInfo(NAMemory *heap, const NAString &catName, const NAString &schName,
                                         const NAString &objName, const NAString &extTableName,
                                         const ComObjectType &objType, ComTdbVirtTableTableInfo *&tableInfo) {
  int cliRC = 0;
  ...
  if (isUninit)
    getUID = FALSE;
  else if ((CmpCommon::context()->isMxcmp()) && (ComIsTrafodionReservedSchemaName(schName)))
    getUID = FALSE;
  else if (CmpSeabaseDDL::getBootstrapMode())
    getUID = FALSE;
  if (getUID) {
    if (switchCompiler(CmpContextInfo::CMPCONTEXT_TYPE_META)) return -1;

    CmpSeabaseDDL::setBootstrapMode(TRUE);
    objUID = getObjectInfo(&cliInterface, catName.data(), schName.data(), objName.data(), objType, objectOwner,
                           schemaOwner, objectFlags, objDataUID);
  ...
  if (createTableInfo) {
    ...
    if (!tableInfo->tableNamespace) {
      if (NOT ComIsTrafodionReservedSchemaName(schName)) {
        if (objUID != 0) {
          ...
          if (getTextFromMD(&cliInterface, objUID, COM_OBJECT_NAMESPACE, 0, tableNamespace)) {
            goto label_error_return;
          }
    ...
    if (NOT tableNamespace.isNull()) {
      tableInfo->tableNamespace = new (heap) char[tableNamespace.length() + 1];
...
TrafDesc *CmpSeabaseDDL::getSeabaseMDTableDesc(const NAString &catName, const NAString &schName,
                                               const NAString &objName, const ComObjectType objType) {
  int cliRC = 0;
  ...
  int uniqueInfoSize = 0;
  ComTdbVirtTableConstraintInfo *constrInfo = NULL;

  int indexInfoSize = 0;
  const ComTdbVirtTableIndexInfo *indexInfo = NULL;
  NAString nameSpace;
  if (NOT getMDtableInfo(coName, tableInfo, colInfoSize, colInfo, keyInfoSize, keyInfo, indexInfoSize, indexInfo,
                         objType, nameSpace))
    return NULL;
  ...
  tableDesc = Generator::createVirtualTableDesc(
      (char *)extTableName.data(),
      NULL,  // let it decide what heap to use
      colInfoSize, (ComTdbVirtTableColumnInfo *)colInfo, keyInfoSize, (ComTdbVirtTableKeyInfo *)keyInfo, uniqueInfoSize,
      constrInfo, indexInfoSize, (ComTdbVirtTableIndexInfo *)indexInfo, 0, NULL, tableInfo, NULL /*seqinfo*/,
      NULL /*statsinfo*/, NULL /*endk*/, FALSE /*gendesc*/, NULL /*desclen*/, FALSE /*ss*/, NULL /*priv*/,
      (nameSpace.length() > 0) ? nameSpace.data() : NULL);

  return tableDesc;
}

TrafDesc *CmpSeabaseDDL::getSeabaseHistTableDesc(const NAString &catName, const NAString &schName,
  ...
  ComTdbVirtTableConstraintInfo *constrInfo = new (STMTHEAP) ComTdbVirtTableConstraintInfo[1];
  ...
  if (processDDLandCreateDescs(parser, ddl, sizeOfDDL, catName, schName, nameSpace, FALSE, 0, NULL, 0, NULL, numCols,
                               colInfo, numKeys, keyInfo, tableInfo, indexInfo))
    return NULL;

  ComObjectName coConstrName(catName, schNameL, constrName);
  NAString *extConstrName = new (STMTHEAP) NAString(coConstrName.getExternalName(TRUE));
  ...
  Assign_SqlParser_Flags(savedParserFlags);
  tableDesc =
      Generator::createVirtualTableDesc((char *)extTableName.data(),
                                        NULL,  // let it decide what heap to use
                                        numCols, colInfo, numKeys, keyInfo, 1, constrInfo, 0, NULL, 0, NULL, tableInfo);

  return tableDesc;
}

TrafDesc *CmpSeabaseDDL::getTrafBackupMDTableDesc(const NAString &catName, const NAString &schName,
                                                  const NAString &objName) {
  int cliRC = 0;
  ...
  for (i = 0; ((NOT found) && (i < numMDTables)); i++) {
    ...
    if (objName != mdti.newName) continue;

    if (processDDLandCreateDescs(parser, mdti.newDDL, mdti.sizeOfnewDDL, catName, schName,
                                 NAString(TRAF_RESERVED_NAMESPACE3), FALSE, 0, NULL, 0, NULL, numCols, colInfo, numKeys,
                                 keyInfo, tableInfo, indexInfo))
      return NULL;
  ...
  if (NOT found) {
    ...
    for (size_t i = 0; ((NOT found) && (i < numMDTables)); i++) {
      ...
      if (objName != tableDefinition.tableName) continue;
      ...
      if (processDDLandCreateDescs(parser, &ddlString, sizeof(QString), catName, schName,
                                   NAString(TRAF_RESERVED_NAMESPACE3), FALSE, 0, NULL, 0, NULL, numCols, colInfo,
                                   numKeys, keyInfo, tableInfo, indexInfo))
        return NULL;
      found = TRUE;
    }  // for
  }    // if
...
int CmpSeabaseDDL::getTableDefForBinlogOp(ElemDDLColDefArray *colArray, ElemDDLColRefArray *keyArray, NAString &cols,
                                          NAString &keys, NABoolean hasPk) {
  for (size_t index = 0; index < colArray->entries(); index++) {
    ...
    ComLobsStorageType lobStorage;
    NAString compDefnStr;
    if (getColInfo(colNode, FALSE, colFamily, colName,
                   TRUE,  // TODO, now I assume it is always algined format for async XDC
                   datatype, length, precision, scale, dt_start, dt_end, upshifted, nullable, charset, colClass,
                   defaultClass, defVal, heading, lobStorage, compDefnStr, hbaseColFlags, colFlags))
      return -1;
    ...
    NAString tmpansitype(ansitype);
    NAString stripedstr = tmpansitype.strip(NAString::trailing, ' ');
    sprintf(buf, "%d-%d-%d-%s-%d-%d-%s-%s-%d-%d-%d-%s;", datatype, nullable, length, colClassLit, precision, scale,
            charset.data(), stripedstr.data(), dt_start, dt_end, (int)index,
            colName.strip(NAString::trailing, ' ').data());
    cols += buf;
  }
  ...
  for (size_t index = 0; index < keyArray->entries(); index++) {
    ...
    int tableColNum = (int)colArray->getColumnIndex((*keyArray)[index]->getColumnName());

    ElemDDLColDef *colDef = (*colArray)[tableColNum];
    NAType *colType = colDef->getColumnDataType();
    int keyLen = colType->getEncodedKeyLength();
    ...
    if (getColInfo(colDef, FALSE, colFamily, colName,
                   TRUE,  // TODO, now I assume it is always algined format for async XDC
                   datatype, length, precision, scale, dt_start, dt_end, upshifted, nullable, charset, colClass,
                   defaultClass, defVal, heading, lobStorage, compDefnStr, hbaseColFlags, colFlags))
      return -1;
    ...
    NAString tmpansitype(ansitype);
    NAString stripedstr = tmpansitype.strip(NAString::trailing, ' ');
    sprintf(buf, "%d-%d-%d-%s-%d-%d-%s-%s-%d-%d-%d-%s-%d;", datatype, nullable, length, colClassLit, precision, scale,
            charset.data(), stripedstr.data(), dt_start, dt_end, tableColNum,
            colName.strip(NAString::trailing, ' ').data(), keyorder == COM_ASCENDING_ORDER ? 0 : 1);
    keys += buf;
...
int CmpSeabaseDDL::getSeabaseColumnInfo(ExeCliInterface *cliInterface, long objUID, const NAString &catName,
                                        const NAString &schName, const NAString &objName, char *direction,
                                        NABoolean *isTableSalted, Int16 *numTrafReplicas, int *identityColPos,
                                        int *partialRowSize, int *numCols,
                                        ComTdbVirtTableColumnInfo **outColInfoArray) {
  char query[3000];
  int cliRC;

  if (identityColPos) *identityColPos = -1;

  Queue *tableColInfo = NULL;
  str_sprintf(query,
              "select column_name, column_number, column_class, "
              ...
              "order by 2 ",
              getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_COLUMNS, objUID, direction);
  ...
  *numCols = tableColInfo->numEntries();
  ...
  if (partialRowSize) *partialRowSize = 0;
  ...
  for (int idx = 0; idx < *numCols; idx++) {
    ...
    colInfo.colNumber = *(int *)oi->get(1);
    ...
    colInfo.datatype = *(int *)oi->get(3);

    colInfo.length = *(int *)oi->get(4);

    colInfo.precision = *(int *)oi->get(5);

    colInfo.scale = *(int *)oi->get(6);

    colInfo.dtStart = *(int *)oi->get(7);

    colInfo.dtEnd = *(int *)oi->get(8);
    ...
    colInfo.charset = (SQLCHARSET_CODE)CharInfo::getCharSetEnum((char *)oi->get(12));

    colInfo.defaultClass = (ComColumnDefaultClass) * (int *)oi->get(13);
    ...
    if (colInfo.defaultClass == COM_USER_DEFINED_DEFAULT ||
      ...
      if (colInfo.defaultClass != COM_USER_DEFINED_DEFAULT) {
        ...
        cliRC = getTextFromMD(cliInterface, objUID, COM_COMPUTED_COL_TEXT, colInfo.colNumber, tempDefVal);
        if (cliRC < 0) {
          cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
          return -1;
        }
        if (strcmp(colInfo.colName, ElemDDLSaltOptionsClause::getSaltSysColName()) == 0) tableIsSalted = TRUE;
      }
      if (strcmp(colInfo.colName, ElemDDLReplicateClause::getReplicaSysColName()) == 0)
      ...
      NAString seqName;
      SequenceGeneratorAttributes::genSequenceName(catName, schName, objName, colInfo.colName, seqName);

      NAString fullyQSeq = catName + "." + "\"" + schName + "\"" + "." + "\"" + seqName + "\"";

      tempDefVal = userFunc + fullyQSeq + ")";

      if (identityColPos) *identityColPos = idx;
    ...
    if (colInfo.defaultClass != COM_NO_DEFAULT) {
      ...
      if (colInfo.defaultClass == COM_CURRENT_DEFAULT || colInfo.defaultClass == COM_CATALOG_FUNCTION_DEFAULT ||
    ...
    colInfo.colFlags = *(long *)oi->get(20);
    ...
    if (colInfo.defaultClass == COM_ALWAYS_COMPUTE_COMPUTED_COLUMN_DEFAULT &&
        strcmp(colInfo.colName, ElemDDLSaltOptionsClause::getSaltSysColName()) == 0)
      colInfo.colFlags |= SEABASE_COLUMN_IS_SALT;

    if (colInfo.colFlags & SEABASE_COLUMN_IS_COMPOSITE) compositeTypeExists = TRUE;
    if (colInfo.colFlags & SEABASE_COLUMN_IS_REPLICA) {
      if (tableTrafReplicas < 1) CMPASSERT(0);
    }

    // rowsize uses the same algorithm as
    // method NATable::computeHBaseRowSizeFromMetaData
    if (partialRowSize) *partialRowSize += strlen(colInfo.hbaseColFam) + strlen(colInfo.hbaseColQual) + colInfo.length;
  ...
  if (compositeTypeExists) {
    ...
    //          8 bytes  8 bytes    8 bytes  defnSize bytes
    //                   Repeat for numCols entries
    cliRC = getTextFromMD(cliInterface, objUID, COM_COMPOSITE_DEFN_TEXT, 0, compTypeStr);
    if (cliRC < 0) {
      cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
      return -1;
    }
    ...
    for (int idx = 0; idx < *numCols; idx++) {
      ...
      if (colInfo.colFlags & SEABASE_COLUMN_IS_COMPOSITE) {
        ...
        for (int i = 0; ((NOT found) && (i < numCompFields)); i++) {
          int colNumber = str_atoi(compEntry, 8);
          compEntry += 8;
          int defnSize = str_atoi(compEntry, 8);
          compEntry += 8;
  ...
  if (isTableSalted != NULL) *isTableSalted = tableIsSalted;
  if (numTrafReplicas != NULL) *numTrafReplicas = tableTrafReplicas;
  *outColInfoArray = colInfoArray;
  return *numCols;
}

ComTdbVirtTableSequenceInfo *CmpSeabaseDDL::getSeabaseSequenceInfo(const NAString &catName, const NAString &schName,
                                                                   const NAString &seqName, NAString &extSeqName,
                                                                   int &objectOwner, int &schemaOwner, long &seqUID) {
  int retcode = 0;
  int cliRC = 0;
  ...
  long objectFlags = 0;
  long objDataUID = 0;
  seqUID = getObjectInfo(&cliInterface, catName.data(), schName.data(), seqName.data(), COM_SEQUENCE_GENERATOR_OBJECT,
  ...
  str_sprintf(buf,
              "select fs_data_type, start_value, increment, max_value, min_value, cycle_option, cache_size, "
              "next_value, seq_type, redef_ts, flags from %s.\"%s\".%s  where seq_uid = %ld",
              getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_SEQ_GEN, seqUID);
  ...
  if ((seqQueue->numEntries() == 0) || (seqQueue->numEntries() > 1)) {
    *CmpCommon::diags() << DgSqlCode(-4082) << DgTableName(extSeqName);

    return NULL;
  }

  ComTdbVirtTableSequenceInfo *seqInfo = new (STMTHEAP) ComTdbVirtTableSequenceInfo();
  ...
  seqInfo->datatype = *(int *)vi->get(0);
  seqInfo->startValue = *(long *)vi->get(1);
  seqInfo->increment = *(long *)vi->get(2);
  seqInfo->maxValue = *(long *)vi->get(3);
  seqInfo->minValue = *(long *)vi->get(4);
  seqInfo->cycleOption = (memcmp(vi->get(5), COM_YES_LIT, 1) == 0 ? 1 : 0);
  seqInfo->cache = *(long *)vi->get(6);
  seqInfo->nextValue = *(long *)vi->get(7);
  seqInfo->seqType = sgType;
  seqInfo->seqUID = seqUID;
  seqInfo->redefTime = *(long *)vi->get(9);
  seqInfo->flags = *(long *)vi->get(10);
...
// ----------------------------------------------------------------------------
TrafDesc *CmpSeabaseDDL::getSeabaseSchemaDesc(const NAString &catName, const NAString &schName, const int ctlFlags,
                                              int &packedDescLen) {
  TrafDesc *tableDesc = NULL;
  ...
  ComObjectName objectName(catName, schName, SEABASE_SCHEMA_OBJECTNAME, COM_UNKNOWN_NAME,
  ...
  if (switchCompiler(CmpContextInfo::CMPCONTEXT_TYPE_META)) return NULL;

  schemaUID =
      getObjectInfo(&cliInterface, catName.data(), schName.data(), SEABASE_SCHEMA_OBJECTNAME, COM_PRIVATE_SCHEMA_OBJECT,
  ...
  if (schemaUID == -1 || objectOwner == 0) {
    ...
    schemaUID =
        getObjectInfo(&cliInterface, catName.data(), schName.data(), SEABASE_SCHEMA_OBJECTNAME,
                      COM_SHARED_SCHEMA_OBJECT, objectOwner, schemaOwner, objectFlags, objDataUID, FALSE /*no error*/);
      ...
      ((objectFlags & MD_OBJECTS_DISABLE_STORED_DESC) == 0))  // not disabled
  {
    ...
    retcode = checkAndGetStoredObjectDesc(&cliInterface, schemaUID, &tableDesc, FALSE,
    ...
    if (retcode == 0) {
      ...
      QRLogger::log(CAT_SQL_EXE, LL_DEBUG, "Schema definition for %s.%s retrieved from SQL descriptor store (TEXT)",
  ...
  ComObjectType objectType = isPrivateSchema ? COM_PRIVATE_SCHEMA_OBJECT : COM_SHARED_SCHEMA_OBJECT;
  ComTdbVirtTablePrivInfo *privInfo =
      getSeabasePrivInfo(&cliInterface, catName, schName, schemaUID, objectType, schemaOwner);
  ...
  tableInfo->objectFlags = objectFlags;
  tableInfo->tablesFlags = 0;
  tableInfo->tableNamespace = (NOT schemaNamespace.isNull() ? schemaNamespace.data() : NULL);

  tableDesc = Generator::createVirtualTableDesc((char *)extSchName.data(),
                                                NULL,     // let it decide what heap to use
                                                0, NULL,  // colInfo
                                                0, NULL,  // keyInfo
                                                0, NULL,  // constrInfo
                                                ...
                                                NULL,  // statsInfo
                                                NULL,  // endKeyArray,
                                                ((ctlFlags & GEN_PACKED_DESC) != 0), &packedDescLen, !isPrivateSchema,
                                                privInfo);

  QRLogger::log(CAT_SQL_EXE, LL_DEBUG, "Schema definition for %s.%s generated from SQL metadata", catName.data(),
                schName.data());
  return tableDesc;
}
...
// ****************************************************************************
ComTdbVirtTablePrivInfo *CmpSeabaseDDL::getSeabasePrivInfo(ExeCliInterface *cliInterface, const NAString &catName,
                                                           const NAString &schName, const long objUID,
                                                           const ComObjectType objType, const int schemaOwner) {
  if (!isAuthorizationEnabled()) return NULL;
  ...
  if (objType == COM_PRIVATE_SCHEMA_OBJECT || objType == COM_SHARED_SCHEMA_OBJECT)
    ...
    schUID = getObjectUID(cliInterface, catName.data(), schName.data(), SEABASE_SCHEMA_OBJECTNAME, NULL, NULL,
                          "object_type in ('SS', 'PS')", schObjTypeLit);

  if (schUID == -1) schUID = 0;
  ...
  PrivMgrCommands command(std::string(MDLoc.data()), std::string(privMgrMDLoc.data()), CmpCommon::diags());
  if (command.getPrivileges(objUID, schemaOwner, schUID, schObjType, objType, *privInfo->privmgr_desc_list) !=
      STATUS_GOOD) {
    *CmpCommon::diags() << DgSqlCode(-CAT_UNABLE_TO_RETRIEVE_PRIVS);
    return NULL;
  }
  ...
  if (getenv("NATABLE_DEBUG")) {
    cout << "Retrieved privs from metadata, "
         << "entries: " << privInfo->privmgr_desc_list->entries() << endl;
...
TrafDesc *CmpSeabaseDDL::getSeabaseLibraryDesc(const NAString &catName, const NAString &schName,
...

{
  ...
  if (switchCompiler(CmpContextInfo::CMPCONTEXT_TYPE_META)) return NULL;
  long libUID = getObjectInfo(&cliInterface, catName.data(), schName.data(), libraryName.data(), COM_LIBRARY_OBJECT,
  ...
  str_sprintf(buf,
              "SELECT library_filename, version "
              "FROM %s.\"%s\".%s "
  ...
  if (cliRC == 100)  // did not find the row
  {
    ...
    *CmpCommon::diags() << DgSqlCode(-CAT_OBJECT_DOES_NOT_EXIST_IN_TRAFODION) << DgString0(libraryName);
    switchBackCompiler();
    return NULL;
  }
  ...
  if (libraryInfo == NULL) return NULL;
  ...
  libraryInfo->library_version = *(int *)ptr;
  libraryInfo->object_owner_id = objectOwner;
  libraryInfo->schema_owner_id = schemaOwner;
  libraryInfo->library_UID = libUID;

  TrafDesc *library_desc = Generator::createVirtualLibraryDesc(libraryName.data(), libraryInfo, NULL);

  processReturn();
  return library_desc;
}

TrafDesc *CmpSeabaseDDL::getSeabaseSequenceDesc(const NAString &catName, const NAString &schName,
  ...
  ComTdbVirtTableSequenceInfo *seqInfo =
      getSeabaseSequenceInfo(catName, schName, seqName, extSeqName, objectOwner, schemaOwner, seqUID);
  ...
  ComTdbVirtTablePrivInfo *privInfo =
      getSeabasePrivInfo(&cliInterface, catName, schName, seqUID, COM_SEQUENCE_GENERATOR_OBJECT, schemaOwner);
  ...
  tableDesc = Generator::createVirtualTableDesc((char *)extSeqName.data(),
                                                NULL,     // let it decide what heap to use
                                                0, NULL,  // colInfo
                                                0, NULL,  // keyInfo
                                                0, NULL,  // constrInfo
...
void lcl_checkEndKeys(NAArray<HbaseStr> *endKeyArray, ExpHbaseInterface *ehi, const NAString &extNameForHbase) {
  if (!endKeyArray || !ehi) return;
  int entries = endKeyArray->entries();
  ...
  for (int i = 0; i < entries - 1; ++i) {
    int len = endKeyArray->at(i).len;
    if (0 == len)  // empty End Key
    {
      // use next region's Start Key instead.
      int startIdx = i + 1;
      if (!startKeyArray) startKeyArray = ehi->getRegionBeginKeys(extNameForHbase);
      if (startKeyArray) {
        int srclen = startKeyArray->at(startIdx).len;
        if (srclen) {
          if (endKeyArray->at(i).val) NADELETEBASIC(endKeyArray->at(i).val, ehi->getHeap());
...
short CmpSeabaseDDL::genHbaseRegionDescs(TrafDesc *desc, const NAString &catName, const NAString &schName,
                                         const NAString &objName, const NAString &nameSpace) {
  if (!desc) return -1;
  ...
  NAString extNameForHbase;
  if ((catName != HBASE_SYSTEM_CATALOG) || ((schName != HBASE_ROW_SCHEMA) && (schName != HBASE_CELL_SCHEMA)))
  ...
  TrafDesc *regionKeyDesc = Generator::assembleDescs(endKeyArray, NULL);
...
short CmpSeabaseDDL::generateStoredStats(long tableUID, long objDataUID, const NAString &catName,
                                         const NAString &schName, const NAString &objName, const NAString &nameSpace,
                                         int ctlFlags, int partialRowSize, int numCols, ExpHbaseInterface *ehi,
  ...
  if (!statsInfo) statsInfo = new (STMTHEAP) ComTdbVirtTableStatsInfo[1];
  ...
  if (ctlFlags & FLUSH_DATA) {
    ...
    if (retcode < 0) {
      *CmpCommon::diags() << DgSqlCode(-8448) << DgString0((char *)"ExpHbaseInterface::drop()")
                          << DgString1(getHbaseErrStr(-retcode)) << DgInt0(-retcode)
                          << DgString2((char *)GetCliGlobals()->getJniErrorStr());
  ...
  int hbtIndexLevels = (ActiveSchemaDB()->getDefaults()).getAsLong(HBASE_INDEX_LEVEL);
  int hbtBlockSize = (ActiveSchemaDB()->getDefaults()).getAsLong(HBASE_BLOCK_SIZE);

  // call getHbaseTableInfo if index level is set to 0
  if (hbtIndexLevels == 0) {
    retcode = ehi->getHbaseTableInfo(fqTblName, hbtIndexLevels, hbtBlockSize);
  }
  ...
  cliRC = cliInterface.fetchAllRows(histogramQueue, histogramQuery.data(), 0, FALSE, FALSE, TRUE);
  ...
  // Histogram query
  /* select histogram_id, column_number, colcount, interval_count, rowcount, total_uec, juliantimestamp(stats_time),
   * low_value, high_value, juliantimestamp(read_time), read_count, sample_Secs, col_secs, sample_percent, cv, reason,
   * v1, v2, TRANSLATE(V5 USING UCS2TOUTF8) from %s where table_uid = %ld and reason != ' ' %s order by table_uid,
   * histogram_id, col_position */

  ComTdbVirtTableHistogramInfo *histogramInfoArray = NULL;
  histogramInfoArray = new (STMTHEAP) ComTdbVirtTableHistogramInfo[histogramQueue->numEntries()];
  for (int idx = 0; idx < histogramQueue->numEntries(); idx++) {
    ...
    hdesc.histogram_id = *(long *)vi->get(0);
    hdesc.column_number = *(int *)vi->get(1);
    hdesc.colcount = *(int *)vi->get(2);
    hdesc.interval_count = *(Int16 *)vi->get(3);
    hdesc.rowcount = *(long *)vi->get(4);
    hdesc.total_uec = *(long *)vi->get(5);
    hdesc.stats_time = *(long *)vi->get(6);

    if (idx == 0) statsInfo->rowcount = hdesc.rowcount;

    char *ptr = NULL;
    int len = 0;
    vi->get(7, ptr, len);
    hdesc.low_value = new (STMTHEAP) char[sizeof(len) + len];
    *(int *)hdesc.low_value = len;
    str_cpy_all(&hdesc.low_value[sizeof(len)], ptr, len);

    vi->get(8, ptr, len);
    hdesc.high_value = new (STMTHEAP) char[sizeof(len) + len];
    *(int *)hdesc.high_value = len;
    str_cpy_all(&hdesc.high_value[sizeof(len)], ptr, len);

    hdesc.read_time = *(long *)vi->get(9);
    hdesc.read_count = *(Int16 *)vi->get(10);
    hdesc.sample_secs = *(long *)vi->get(11);
    hdesc.col_secs = *(long *)vi->get(12);
    hdesc.sample_percent = *(Int16 *)vi->get(13);
    hdesc.cv = *(Float64 *)vi->get(14);
    hdesc.reason = *(char *)vi->get(15);
    hdesc.v1 = *(long *)vi->get(16);
    hdesc.v2 = *(long *)vi->get(17);
  ...
  if (statsExist) {
    ...
    cliRC = cliInterface.fetchAllRows(histintsQueue, histintsQuery.data(), 0, FALSE, FALSE, TRUE);
    ...
    // histints query
    /*
      select histogram_id, interval_number, interval_rowcount, interval_uec, interval_boundary, cast(std_dev_of_freq as
      double precision), v1, v2, v5 from %s where table_uid = %ld order by table_uid, histogram_id, interval_number;
    */
    histintsInfoArray = new (STMTHEAP) ComTdbVirtTableHistintInfo[histintsQueue->numEntries()];
    for (int idx = 0; idx < histintsQueue->numEntries(); idx++) {
  ...
  if (!statsExist) {
    ...
    retcode = ehi->estimateRowCount(fqTblName, partialRowSize, numCols,
                                    5000,  // retry limit of 5 seconds (5000 milliseconds)
                                    useCoprocessor, estRowCount /* out */, breadCrumb /* out*/);
...
int CmpSeabaseDDL::getIndexInfo(ExeCliInterface &cliInterface, NABoolean includeInvalidDefs, long objUID,
                                Queue *&indexQueue) {
  int cliRC = 0;
  char query[4000];

  str_sprintf(query,
              "select O.catalog_name, O.schema_name, O.object_name, I.keytag, I.is_unique, I.is_explicit, "
              "I.key_colcount, I.nonkey_colcount, T.num_salt_partns, T.row_format, I.index_uid, I.FLAGS from "
              "%s.\"%s\".%s I, %s.\"%s\".%s O ,  %s.\"%s\".%s T where I.base_table_uid = %ld and I.index_uid = "
              "O.object_uid %s and I.index_uid = T.table_uid order by 1,2,3",
              getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_INDEXES, getSystemCatalog(), SEABASE_MD_SCHEMA,
              SEABASE_OBJECTS, getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_TABLES, objUID,
  ...
  cliInterface.holdAndSetCQDs("MERGE_JOINS, HASH_JOINS", "MERGE_JOINS 'OFF', HASH_JOINS 'OFF'");
...
TrafDesc *CmpSeabaseDDL::getSeabaseUserTableDesc(const NAString &catName, const NAString &schName,
                                                 const NAString &objName, const ComObjectType objType,
                                                 NABoolean includeInvalidDefs, short includeBRLockedTables,
                                                 int ctlFlags, int &packedDescLen, int cntxtType) {
  int retcode = 0;
  int cliRC = 0;
  ...
  if ((includeInvalidDefs) || (Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL)) || (objType == COM_INDEX_OBJECT))
  ...
  objUID = getObjectInfo(&cliInterface, catName.data(), schName.data(), objName.data(), objType, objectOwner,
                         schemaOwner, objectFlags, objDataUID, FALSE /*no error now */, checkForValidDef);
  ...
  if ((objUID < 0) && (objType == COM_BASE_TABLE_OBJECT)) {
    ...
    CmpCommon::diags()->clear();
    objUID = getObjectInfo(&cliInterface, catName.data(), schName.data(), objName.data(), COM_VIEW_OBJECT, objectOwner,
                           schemaOwner, objectFlags, objDataUID, FALSE /*no error now*/);
  }

  if (objUID < 0) {
    NABoolean validDef = FALSE;
    cliRC = getObjectValidDef(&cliInterface, catName.data(), schName.data(), objName.data(), objType, validDef);
    if ((cliRC == 1) && (NOT validDef))  // found and not valid
    {
      CmpCommon::diags()->clear();
      *CmpCommon::diags() << DgSqlCode(-4254) << DgString0(extName);
    }
  ...
  if (objType == COM_BASE_TABLE_OBJECT && (objectFlags & MD_PARTITION_TABLE_V2) != 0) isPartitionTableV2 = true;

  NABoolean isPartitionBaseTable = false;
  if (objType == COM_BASE_TABLE_OBJECT && (objectFlags & MD_PARTITION_V2) != 0) isPartitionBaseTable = true;

  schUID = getObjectUID(&cliInterface, catName.data(), schName.data(), SEABASE_SCHEMA_OBJECTNAME, NULL, NULL,
                        "object_type in ('SS', 'PS')", NULL, FALSE, FALSE, &schObjFlags);

  // if a backup or restore oper is in progress on this table or on the schema, return error.
  if (((schObjFlags & MD_OBJECTS_BR_IN_PROGRESS) != 0) || ((objectFlags & MD_OBJECTS_BR_IN_PROGRESS) != 0)) {
    NAString brTag;
    ...
    // an error.
    if (NOT((includeBRLockedTables == 1) && (NAString(brTag).contains("(BACKUP)")))) {
      *CmpCommon::diags() << DgSqlCode(-4264) << DgString0(extName) << DgString1(brTag);

      return NULL;
    }
  }

  //  long t2 = NA_JulianTimestamp();
      ...
      ((objectFlags & MD_OBJECTS_DISABLE_STORED_DESC) == 0))  // not disabled
  {
    ...
    if ((CmpCommon::getDefault(TRAF_ENABLE_METADATA_LOAD_IN_SHARED_CACHE) == DF_ON) && objType == COM_BASE_TABLE_OBJECT)

      QRLogger::log(CAT_SQL_EXE, LL_INFO,
                    "Object definition for %s.%s.%s not found in shared cache, checking TEXT table", catName.data(),
                    schName.data(), objName.data());

    // if good stored desc was retrieved, return it.
    // Otherwise, continue and generate descriptor the old fashioned way.
    retcode =
        checkAndGetStoredObjectDesc(&cliInterface, objUID, &desc, FALSE, ActiveSchemaDB()->getNATableDB()->getHeap());
    if (retcode == 0) {
      CmpCommon::diags()->clear();
      if (!desc) return NULL;

      QRLogger::log(CAT_SQL_EXE, LL_DEBUG, "Object definition for %s.%s retrieved from SQL descriptor store (TEXT)",
                    catName.data(), schName.data());
      return desc;
    }
  ...
  str_sprintf(query, "select is_audited, num_salt_partns, row_format, flags from %s.\"%s\".%s where table_uid = %ld ",
              getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_TABLES, objUID);
  ...
  if (cliRC == 0)  // read some rows
  {
    ...
    else if (CmpSeabaseDDL::isMDflagsSet(tablesFlags, MD_TABLES_STORAGE_BIGTABLE_FLG))
      storageType = COM_STORAGE_BIGTABLE;
    else
      storageType = COM_STORAGE_HBASE;

    if (getTextFromMD(&cliInterface, objUID, COM_HBASE_OPTIONS_TEXT, 0, *hbaseCreateOptions)) {
      processReturn();
      return NULL;
    }

    if (getTextFromMD(&cliInterface, objUID, COM_HBASE_COL_FAMILY_TEXT, 0, colFamStr)) {
      processReturn();
      return NULL;
    }
    if (getTextFromMD(&cliInterface, objUID, COM_HBASE_SPLIT_TEXT, 0, *hbaseSplitClause)) {
      processReturn();
      return NULL;
    }
  ...
  //  long t3 = NA_JulianTimestamp();

  if (getSeabaseColumnInfo(&cliInterface, objUID, catName, schName, objName, (char *)direction, &tableIsSalted,
                           &numTrafReplicas, &identityColPos, &partialRowSize, &numCols, &colInfoArray) <= 0) {
    processReturn();
    return NULL;
  }
  ...
  if (objType == COM_INDEX_OBJECT) {
    str_sprintf(query,
                "select k.column_name, c.column_number, k.keyseq_number, ordering, cast(0 as int not null)  from "
                "%s.\"%s\".%s k, %s.\"%s\".%s c where k.column_name = c.column_name and k.object_uid = c.object_uid "
                "and k.object_uid = %ld and k.nonkeycol = 0 order by keyseq_number",
                getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_KEYS, getSystemCatalog(), SEABASE_MD_SCHEMA,
                SEABASE_COLUMNS, objUID);
  } else {
    str_sprintf(query,
                "select column_name, column_number, keyseq_number, ordering, cast(0 as int not null)  from "
                "%s.\"%s\".%s where object_uid = %ld and nonkeycol = 0 order by keyseq_number",
                getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_KEYS, objUID);
  }
  ...
  if (tableKeyInfo->numEntries() > 0) {
    keyInfoArray = new (STMTHEAP) ComTdbVirtTableKeyInfo[tableKeyInfo->numEntries()];
  }
  ...
  if (isPartitionTableV2) {
    if (getSeabasePartitionV2Info(&cliInterface, objUID, &partitionV2Info)) {
      processReturn();
      return NULL;
    }
  } else if (isPartitionBaseTable) {
    if (getSeabasePartitionBoundaryInfo(&cliInterface, objUID, &partitionV2Info)) {
      processReturn();
      return NULL;
    }
  }
  ...
  if (objType == COM_INDEX_OBJECT) {
    str_sprintf(query,
                "select t.flags, t.table_uid from %s.\"%s\".%s I, %s.\"%s\".%s T where I.index_uid = %ld and "
                "I.base_table_uid = T.table_uid",
                getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_INDEXES, getSystemCatalog(), SEABASE_MD_SCHEMA,
                SEABASE_TABLES, objUID);

    long outVals[2];
    long flags = 0;
    int outValsLen = 2 * sizeof(long);
    long rowsAffected = 0;
    cliRC = cliInterface.executeImmediateCEFC(query, NULL, 0, (char *)&outVals, &outValsLen, &rowsAffected);
    if (cliRC < 0) {
      cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
      processReturn();
      return NULL;
    ...
    if (CmpSeabaseDDL::isMDflagsSet(flags, MD_TABLES_STORAGE_MONARCH_FLG))
      storageType = COM_STORAGE_MONARCH;
    else if (CmpSeabaseDDL::isMDflagsSet(flags, MD_TABLES_STORAGE_BIGTABLE_FLG))
      storageType = COM_STORAGE_BIGTABLE;
    else
      storageType = COM_STORAGE_HBASE;
  }
  ...
  if (isPartitionBaseTable && partitionV2Info) {
    long pUid = partitionV2Info->baseTableUid;
    ...
    for (int p = 0; p < globalIndexInfoQueue->numEntries(); p++) {
      ...
      if (*(long *)vi->get(11) & MD_IS_GLOBAL_PARTITION_FLG) {
  ...
  if (indexInfoQueue->numEntries() > 0) {
    indexInfoArray = new (STMTHEAP) ComTdbVirtTableIndexInfo[indexInfoQueue->numEntries()];
  }
  ...
  ComObjectName coName(qCatName, qSchName, qObjName);
  NAString *extTableName = new (STMTHEAP) NAString(coName.getExternalName(TRUE));
  ...
  for (int idx = 0; idx < indexInfoQueue->numEntries(); idx++) {
    ...
    int keyTag = *(int *)vi->get(3);
    int isUnique = *(int *)vi->get(4);
    int isExplicit = *(int *)vi->get(5);
    int keyColCount = *(int *)vi->get(6);
    int nonKeyColCount = *(int *)vi->get(7);
    int idxNumSaltPartns = *(int *)vi->get(8);
    char *format = vi->get(9);
    long indexUID = *(long *)vi->get(10);
    long indexFlags = *(long *)vi->get(11);
    // get the ngram index flag
    str_sprintf(query,
                "select case when X.text is not null then cast(trim(X.text) as int) else 0 end from %s.\"%s\".%s X "
                "where X.TEXT_uid = %ld AND X.TEXT_TYPE=%d AND X.SUB_ID=0 for read committed access",
                getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_TEXT, indexUID, COM_OBJECT_NGRAM_TEXT);
    ...
    else if (memcmp(format, COM_HBASE_FORMAT_LIT, 2) == 0)
      idxRowFormat = COM_HBASE_FORMAT_TYPE;
    else
      idxRowFormat = COM_UNKNOWN_FORMAT_TYPE;

    long idxUID = getObjectUID(&cliInterface, idxCatName, idxSchName, idxObjName, COM_INDEX_OBJECT_LIT);
    if (idxUID < 0) {
      processReturn();
    ...
    if (getTextFromMD(&cliInterface, idxUID, COM_HBASE_OPTIONS_TEXT, 0, *idxHbaseCreateOptions)) {
      processReturn();
    ...
    NAString *extIndexName = new (STMTHEAP) NAString(coIdxName.getExternalName(TRUE));
    ...
    indexInfoArray[idx].hbaseCreateOptions = (idxHbaseCreateOptions->isNull() ? NULL : idxHbaseCreateOptions->data());
    ...
    str_sprintf(query,
                "select column_name, column_number, keyseq_number, ordering, nonkeycol  from %s.\"%s\".%s where "
                "object_uid = %ld order by keyseq_number",
                getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_KEYS, idxUID);
    ...
    ComTdbVirtTableKeyInfo *keyInfoArray = new (STMTHEAP) ComTdbVirtTableKeyInfo[keyColCount];

    ComTdbVirtTableKeyInfo *nonKeyInfoArray = NULL;
    if (nonKeyColCount > 0) {
      nonKeyInfoArray = new (STMTHEAP) ComTdbVirtTableKeyInfo[nonKeyColCount];
    }

    keyInfoQueue->position();
    int jk = 0;
    int jnk = 0;
    for (int j = 0; j < keyInfoQueue->numEntries(); j++) {
      OutputInfo *vi = (OutputInfo *)keyInfoQueue->getNext();

      int nonKeyCol = *(int *)vi->get(4);
      if (nonKeyCol == 0) {
  ...
  // get constraint info
  if (isPartitionBaseTable && partitionV2Info) {
    long consObjUid = partitionV2Info->baseTableUid;
    str_sprintf(query,
                "select O.object_name, C.constraint_type, C.col_count, C.constraint_uid, C.enforced, C.flags from "
                "%s.\"%s\".%s O <<+cardinality 1e3>>, %s.\"%s\".%s C where O.catalog_name = '%s' and O.schema_name = "
                "'%s' and O.object_uid = C.constraint_uid and (C.table_uid = %ld and C.constraint_type <> '%s' or "
                "C.table_uid = %ld and C.constraint_type = '%s') order by 1",
                getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_OBJECTS, getSystemCatalog(), SEABASE_MD_SCHEMA,
                SEABASE_TABLE_CONSTRAINTS, catName.data(), schName.data(), partitionV2Info->baseTableUid,
                COM_PRIMARY_KEY_CONSTRAINT_LIT, objUID, COM_PRIMARY_KEY_CONSTRAINT_LIT);
  } else
    str_sprintf(query,
                "select O.object_name, C.constraint_type, C.col_count, C.constraint_uid, C.enforced, C.flags from "
                "%s.\"%s\".%s O <<+cardinality 1e3>>, %s.\"%s\".%s C where O.catalog_name = '%s' and O.schema_name = "
                "'%s' and C.table_uid = %ld and O.object_uid = C.constraint_uid order by 1",
                getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_OBJECTS, getSystemCatalog(), SEABASE_MD_SCHEMA,
                SEABASE_TABLE_CONSTRAINTS, catName.data(), schName.data(), objUID);
  ...
  //  long t6 = NA_JulianTimestamp();

  ComTdbVirtTableConstraintInfo *constrInfoArray = NULL;
  if (constrInfoQueue->numEntries() > 0) {
    constrInfoArray = new (STMTHEAP) ComTdbVirtTableConstraintInfo[constrInfoQueue->numEntries()];
  }
  ...
  extTableName = new (STMTHEAP) NAString(coTableName.getExternalName(TRUE));
  ...
  for (int idx = 0; idx < constrInfoQueue->numEntries(); idx++) {
    ...
    int colCount = *(int *)vi->get(2);
    long constrUID = *(long *)vi->get(3);
    char *enforced = (char *)vi->get(4);
    long flags = *(long *)vi->get(5);
    ...
    str_sprintf(query,
                "select column_name, column_number, keyseq_number, ordering , cast(0 as int not null) from "
                "%s.\"%s\".%s where object_uid = %ld order by keyseq_number",
                getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_KEYS, constrUID);
    cliRC = cliInterface.initializeInfoList(keyInfoQueue, TRUE);
    if (cliRC < 0) {
      cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
    ...
    ComTdbVirtTableKeyInfo *keyInfoArray = NULL;
    if (colCount > 0) {
      keyInfoArray = new (STMTHEAP) ComTdbVirtTableKeyInfo[colCount];
    ...
    if ((strcmp(constrType, COM_UNIQUE_CONSTRAINT_LIT) == 0) ||
      ...
      str_sprintf(query,
                  "control query shape sort(nested_join(nested_join(nested_join(scan('U'),scan('O')), "
                  "scan('T','TRAFODION.\"_MD_\".TABLE_CONSTRAINTS_IDX')),cut))");
      ...
      str_sprintf(query,
                  "select trim(O.catalog_name || '.' || '\"' || O.schema_name || '\"' || '.' || '\"' || O.object_name "
                  "|| '\"' ) constr_name, trim(O2.catalog_name || '.' || '\"' || O2.schema_name || '\"' || '.' || '\"' "
                  "|| O2.object_name || '\"' ) table_name from %s.\"%s\".%s U, %s.\"%s\".%s O, %s.\"%s\".%s O2, "
                  "%s.\"%s\".%s T where  O.object_uid = U.foreign_constraint_uid and O2.object_uid = T.table_uid and "
                  "T.constraint_uid = U.foreign_constraint_uid and U.unique_constraint_uid = %ld order by 2, 1",
                  getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_UNIQUE_REF_CONSTR_USAGE, getSystemCatalog(),
                  SEABASE_MD_SCHEMA, SEABASE_OBJECTS, getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_OBJECTS,
                  getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_TABLE_CONSTRAINTS, constrUID);
      ...
      if (ringInfoQueue->numEntries() > 0) {
        ringInfoArray = new (STMTHEAP) ComTdbVirtTableRefConstraints[ringInfoQueue->numEntries()];
      }

      ringInfoQueue->position();
      for (int i = 0; i < ringInfoQueue->numEntries(); i++) {
    ...
    if (strcmp(constrType, COM_FOREIGN_KEY_CONSTRAINT_LIT) == 0) {
      str_sprintf(query,
                  "select trim(O.catalog_name || '.' || '\"' || O.schema_name || '\"' || '.' || '\"' || O.object_name "
                  "|| '\"' ) constr_name, trim(O2.catalog_name || '.' || '\"' || O2.schema_name || '\"' || '.' || '\"' "
                  "|| O2.object_name || '\"' ) table_name from %s.\"%s\".%s R, %s.\"%s\".%s O, %s.\"%s\".%s O2, "
                  "%s.\"%s\".%s T where  O.object_uid = R.unique_constraint_uid and O2.object_uid = T.table_uid and "
                  "T.constraint_uid = R.unique_constraint_uid and R.ref_constraint_uid = %ld order by 2,1",
                  getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_REF_CONSTRAINTS, getSystemCatalog(), SEABASE_MD_SCHEMA,
                  SEABASE_OBJECTS, getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_OBJECTS, getSystemCatalog(),
                  SEABASE_MD_SCHEMA, SEABASE_TABLE_CONSTRAINTS, constrUID);
      ...
      if (refdInfoQueue->numEntries() > 0) {
        refdInfoArray = new (STMTHEAP) ComTdbVirtTableRefConstraints[refdInfoQueue->numEntries()];
      }
      ...
      for (int i = 0; i < refdInfoQueue->numEntries(); i++) {
    ...
    if (strcmp(constrType, COM_CHECK_CONSTRAINT_LIT) == 0) {
      NAString constrText;
      if (getTextFromMD(&cliInterface, constrUID, COM_CHECK_CONSTR_TEXT, 0, constrText)) {
        processReturn();
  ...
  if (viewInfoQueue->numEntries() > 0) {
    ...
    int isUpdatable = *(int *)vi->get(1);
    int isInsertable = *(int *)vi->get(2);
  ...
  if (identityColPos >= 0) {
    NAString seqName;
    SequenceGeneratorAttributes::genSequenceName(catName, schName, objName, colInfoArray[identityColPos].colName,
                                                 seqName);

    NAString extSeqName;
    int objectOwner;
    long seqUID;
    seqInfo = getSeabaseSequenceInfo(catName, schName, seqName, extSeqName, objectOwner, schemaOwner, seqUID);
  }

  ComTdbVirtTablePrivInfo *privInfo = getSeabasePrivInfo(&cliInterface, catName, schName, objUID, objType, schemaOwner);
  ...
  tableInfo->numTrafReplicas = numTrafReplicas;
  tableInfo->numInitialSaltRegions = numInitialSaltRegions;
  tableInfo->hbaseSplitClause = (hbaseSplitClause->isNull() ? NULL : hbaseSplitClause->data());
  tableInfo->hbaseCreateOptions = (hbaseCreateOptions->isNull() ? NULL : hbaseCreateOptions->data());
  if (alignedFormat)
    tableInfo->rowFormat = COM_ALIGNED_FORMAT_TYPE;
  ...
  const NAString extNameForHbase = genHBaseObjName(catName, schName, objName, tableNamespace, objDataUID);

  ComTdbVirtTableStatsInfo *statsInfo = NULL;
  if (ctlFlags & GEN_STATS) {
    if (generateStoredStats(objUID, objDataUID, catName, schName, objName, tableNamespace, ctlFlags, partialRowSize,
                            numCols, ehi, statsInfo)) {
      CmpSeabaseDDL::deallocEHI(ehi);
      return NULL;
    }
  }

  //  long t8 = NA_JulianTimestamp();
  tableDesc = Generator::createVirtualTableDesc(
      extTableName->data(),  // objName,
      NULL,                  // let it decide what heap to use
      numCols, colInfoArray,
      tableKeyInfo->numEntries(),  // keyIndex,
      keyInfoArray, constrInfoQueue->numEntries(), constrInfoArray, indexInfoQueue->numEntries(), indexInfoArray,
      viewInfoQueue->numEntries(), viewInfoArray, tableInfo, seqInfo, statsInfo, endKeyArray,
      ((ctlFlags & GEN_PACKED_DESC) != 0), &packedDescLen, TRUE /*user table*/, privInfo,
      NULL,  // namesapce
      partitionV2Info);

  //  long t9 = NA_JulianTimestamp();
  /*
  printf("t9-t8=%ld\n", t9-t8);
  printf("t8-t7=%ld\n", t8-t7);
  printf("t7-t6=%ld\n", t7-t6);
  ...
  */
  ...
  if (tableDesc) {
    ...
    if (!objectFlags & SEABASE_OBJECT_IS_EXTERNAL_HIVE && !objectFlags & SEABASE_OBJECT_IS_EXTERNAL_HBASE) {
  ...
  if (!tableDesc) processReturn();

  return tableDesc;
}  // getSeabaseUserTableDesc

TrafDesc *CmpSeabaseDDL::getSeabaseTableDesc(const NAString &catName, const NAString &schName, const NAString &objName,
  ...
  ExpHbaseInterface *ehi = NULL;
  TrafDesc *tDesc = NULL;
  NABoolean isMDTable = (isSeabaseMD(catName, schName, objName) || isSeabasePrivMgrMD(catName, schName));
  int ctlFlags = 0;
  if (isMDTable && (objName != SEABASE_SCHEMA_OBJECTNAME)) {
    ...
    if ((CmpCommon::getDefault(TRAF_READ_OBJECT_DESC) == DF_ON) && (NOT includeInvalidDefs) &&
        (!Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL))) {
  ...
  else if ((objName == HBASE_HIST_NAME) || (objName == HBASE_HISTINT_NAME) || (objName == HBASE_PERS_SAMP_NAME)) {
    ...
    if (ComIsTrafodionExternalSchemaName(schName)) return NULL;
  ...
  if (!tDesc) {
    if ((CmpCommon::context()->isUninitializedSeabase()) && (!Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL))) {
      ...
      if ((CmpCommon::getDefault(TRAF_READ_OBJECT_DESC) == DF_ON) && (NOT includeInvalidDefs) &&
          (!Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL)) && (objType != COM_SEQUENCE_GENERATOR_OBJECT) &&
      ...
      switch (objType) {
        ...
        default:
          ...
          tDesc = getSeabaseUserTableDesc(catName, schName, objName, objType, includeInvalidDefs, includeBRLockedTables,
                                          ctlFlags, packedDescLen, (CmpContextInfo::CMPCONTEXT_TYPE_META));
          break;
      }
...
TrafDesc *CmpSeabaseDDL::decodeStoredDesc(NAString &gluedText) {
  int decodedMaxLen = str_decoded_len(gluedText.length());
  ...
  int decodedLen = str_decode(packedDesc, decodedMaxLen, gluedText.data(), gluedText.length());
...
int CmpSeabaseDDL::createNATableFromStoredDesc(NATableDB *natdb, BindWA &bindWA, PreloadInfo &loadInfo,
  ...
  if (desc->nodetype != DESC_TABLE_TYPE) return -3;  // internal error
...
int CmpSeabaseDDL::createNATableForSystemObjects(BindWA &bindWA, NAString &catName, NAString &schName,
                                                 NAString &objName, NABoolean isIndex, NABoolean doNotReloadIfExists) {
  CorrName cn(objName, STMTHEAP, schName, catName);
  if (isIndex) cn.setSpecialType(ExtendedQualName::INDEX_TABLE);
...
void CmpSeabaseDDL::generateQueryToFetchUserTableDescriptors(QualifiedName &inSchName, char *buffer, int bufLen) {
  NAString schNameStr;
  if (NOT inSchName.getSchemaName().isNull()) {
    schNameStr = " and O.schema_name = '" + inSchName.getSchemaName() + "' ";
  } else {
    schNameStr =
        " and O.schema_name not like 'VOLATILE_SCHEMA%' and O.schema_name not in ('_MD_', '_REPOS_', '_HV_HIVE_', "
        "'_PRIVMGR_MD_', '_HIVESTATS_') ";
  }

  snprintf(buffer, bufLen,
           "select o.catalog_name, o.schema_name, o.object_name, o.object_type, t.seq_num, t.text from %s.\"%s\".%s O, "
           "%s.\"%s\".%s T where O.object_uid = T.text_uid %s and (O.object_type in ('BT', 'PS', 'SS')) and "
           "T.text_type = %d order by 1,2,3,5",
           getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_OBJECTS, getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_TEXT,
           (schNameStr.isNull() ? " " : schNameStr.data()), COM_STORED_DESC_TEXT);
}

int CmpSeabaseDDL::generatePreloadInfoForUserTables(QualifiedName &inSchName, ExeCliInterface &cliInterface,
  ...
  while (cliRC != 100) {
    ...
    if (cliRC != 100) {
      ...
      int seqNum = *(int *)ptr;
      if (seqNum == 0) {
        if ((NOT objName.isNull()) && (gluedText.length() > 0)) {
          ...
          if (objName == HBASE_HIST_NAME || objName == HBASE_HISTINT_NAME || objName == HBASE_PERS_SAMP_NAME)
  ...
  if ((cliRC == 100) && (NOT objName.isNull()) && (gluedText.length() > 0)) {
    cliInterface.clearGlobalDiags();
...
int CmpSeabaseDDL::loadTrafUserMetadataInCache(CmpContext *cmpContext, NAList<PreloadInfo> &tables,
                                               NABoolean doNotReloadIfExists) {
  int cliRC = 0;
  int retcode;
  int tablesLoaded = 0;
  ...
  for (int i = 0; i < tables.entries(); i++) {
    ...
    if (retcode == 0)
      ...
      QRLogger::log(CAT_SQL_EXE, LL_WARN, "NATable for %s.%s.%s already loaded ", info.catName.data(),
                    info.schName.data(), info.objName.data());
    else if (retcode < 0)
...
int CmpSeabaseDDL::loadTrafMDSystemTableIntoCache(NAString &tableName, CmpContext *context,
                                                  NABoolean doNotReloadIfExists) {
  int retcode = 0;
  int tablesLoaded = 0;
  ...
  for (size_t i = 0; i < numMDTables; i++) {
    ...
    if (!(mdti.newName == tableName)) continue;
    ...
    retcode = createNATableForSystemObjects(bindWA, catName, schName, tableName, mdti.isIndex, doNotReloadIfExists);

    // if error, log it and continue. Load as many tables as can be loaded.
    if (retcode == 0)
      tablesLoaded++;
    else if (retcode < 0)
      QRLogger::log(CAT_SQL_EXE, LL_WARN, "Creating NATable for %s.%s.%s failed with error code %d ", catName.data(),
                    schName.data(), tableName.data(), retcode);
...
int CmpSeabaseDDL::loadTrafSystemMetadataInCache(CmpContext *context, NABoolean doNotReloadIfExists) {
  int retcode = 0;
  int tablesLoaded = 0;
  ...
  for (size_t i = 0; i < numMDTables; i++) {
    ...
    retcode = createNATableForSystemObjects(bindWA, catName, schName, objName, mdti.isIndex, doNotReloadIfExists);
    ...
    if (retcode == 0)
      ...
      QRLogger::log(CAT_SQL_EXE, LL_WARN, "Creating NATable for %s.%s.%s failed with error code %d ", catName.data(),
                    schName.data(), objName.data(), retcode);
  }  // for
...
int CmpSeabaseDDL::loadTrafMetadataInCache(QualifiedName &inSchName) {
  int retcode = 0;

  CmpContext *userContext = CmpCommon::context();

  // if uninitialized, dont load metadata cache
  if (userContext->isUninitializedSeabase()) return 0;
  ...
  NABoolean doNotReloadIfExists = (((CmpCommon::getDefault(IS_SQLCI) == DF_ON) &&
  ...
  NABoolean useSharedCache = (CmpCommon::getDefault(TRAF_ENABLE_METADATA_LOAD_IN_SHARED_CACHE) == DF_ON);

  ExeCliInterface cliInterface(STMTHEAP, 0, NULL, userContext->sqlSession()->getParentQid());
  ...
  cout << "loadTrafMetadataInCache"
       ...
       << ", class=" << CmpCommon::context()->getCIClass() << ", NONE=" << CmpContextInfo::CMPCONTEXT_TYPE_NONE
       << ", META=" << CmpContextInfo::CMPCONTEXT_TYPE_META << ", USTATS=" << CmpContextInfo::CMPCONTEXT_TYPE_USTATS
       << endl;

  if (CmpCommon::context()->getCliContext()) CmpCommon::context()->getCliContext()->displayAllCmpContexts();
#endif
  ...
  if (!sharedDescCache || sharedDescCache->entries(inSchName) == 0) {
    ...
    retcode = generatePreloadInfoForUserTables(inSchName, cliInterface, userTables, statsTables);
    if (retcode < 0) return -1;

    // load user tables into the current instance
    retcode = loadTrafUserMetadataInCache(userContext, userTables, doNotReloadIfExists);

    // load stats tables into META instance
    if (switchCompiler(CmpContextInfo::CMPCONTEXT_TYPE_META)) return -1;

    retcode = loadTrafUserMetadataInCache(CmpCommon::context(), statsTables, doNotReloadIfExists);
...
#define emitErrorForSharedCacheOperation(reason, operation)                                                   \
  *CmpCommon::diags() << DgSqlCode(-CAT_SHARED_CACHE_LOAD_ERROR) << DgString0(reason) << DgString1(operation) \
...
int CmpSeabaseDDL::loadTrafMetadataIntoSharedCache(QualifiedName &inSchName, NABoolean loadLocalIfEmpty,
  ...
  // if uninitialized, dont load metadata cache
  if (CmpCommon::context()->isUninitializedSeabase()) return 0;
  ...
  {
    cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
    snprintf(msgBuf, sizeof(msgBuf), "cliInterface.fetchRowsProlog() returns %d", cliRC);
    reasonString = msgBuf;
  ...
  if (retcode != 0) {  // check the return code from d-lock
    snprintf(msgBuf, sizeof(msgBuf), "observing lock returns %d", retcode);
    reasonString = msgBuf;
    returnCode = -1;
    goto bailout_label;
  }
  ...
  while (cliRC != 100) {
    ...
    if (cliRC < 0) {
      ...
      snprintf(msgBuf, sizeof(msgBuf), "cliInterface.fetch() returns %d", cliRC);
      reasonString = msgBuf;
      returnCode = -1;
      goto bailout_label;
    }
    ...
    if (cliRC != 100) {
      ...
      if (seqNum == 0) {
        ...
        if ((NOT objName.isNull()) && (gluedText.length() > 0)) {
          key = new (sharedHeap, FALSE /*failure is not fatal*/) QualifiedName(objName, schName, catName, sharedHeap);

          value = new (sharedHeap, FALSE /*failure is not fatal*/) NAString(gluedText, sharedHeap);

          if (sharedHeap->allocFailed() || !key || !value) {
            QRLogger::log(CAT_SQL_SHARED_CACHE, LL_DEBUG,
                          "loadTrafMetadataIntoSharedCache(): allocate from shared heap failed: objName=%s, "
                          "schName=%s, catName=%s, value length=%d",
                          objName.data(), schName.data(), catName.data(), gluedText.length());
          ...
          if (sharedDescCache->insert(key, value, &inserted)) {
            if (inserted)
              ...
              QRLogger::log(CAT_SQL_SHARED_CACHE, LL_DEBUG,
                            "loadTrafMetadataIntoSharedCache(): sharedDescCache->insert() failed: objName=%s, "
                            "schName=%s, catName=%s, value length=%d",
                            objName.data(), schName.data(), catName.data(), gluedText.length());
              returnCode = 1;  // memory exhaust condition
              goto bailout_label;
  ...
  if ((cliRC == 100) && (NOT objName.isNull()) && (gluedText.length() > 0)) {
    ...
    if (sharedHeap->allocFailed() || !key || !value) {
      QRLogger::log(CAT_SQL_SHARED_CACHE, LL_DEBUG,
                    "loadTrafMetadataIntoSharedCache(): allocate from shared heap failed: objName=%s, schName=%s, "
                    "catName=%s, value length=%d",
                    objName.data(), schName.data(), catName.data(), gluedText.length());
      returnCode = 1;  // memory exhaust condition
      goto bailout_label;
    }
    ...
    if (sharedDescCache->insert(key, value, &inserted))
      if (inserted)
        ...
        QRLogger::log(CAT_SQL_SHARED_CACHE, LL_DEBUG,
                      "loadTrafMetadataIntoSharedCache(): sharedDescCache->insert() failed: objName=%s, schName=%s, "
                      "catName=%s, value length=%d",
                      objName.data(), schName.data(), catName.data(), gluedText.length());
        returnCode = 1;  // memory exhaust condition
        goto bailout_label;
...
int CmpSeabaseDDL::loadTrafDataIntoSharedCache(QualifiedName &inSchName, NABoolean loadLocalIfEmpty,
                                               CmpDDLwithStatusInfo *dws) {
  int cliRC = 0;
  int retcode = 0;
  int returnCode = 0;
  ...
  ExeCliInterface cliInterface_load(STMTHEAP, 0, NULL, CmpCommon::context()->sqlSession()->getParentQid());
  ...
  NAString query;
  query.format(
      "SELECT CATALOG_NAME, SCHEMA_NAME, OBJECT_NAME, OBJECT_TYPE FROM %s.\"%s\".%s WHERE BITAND(FLAGS, %d) != 0;",
  ...
  while (cliRC != 100) {
    ...
    if (cliRC != 100) {
      ...
      if (NAString(ptr, len) == "BT")
        loadQuery.format("select /*+index(%s)*/ [last 0]* from %s.\"%s\".%s fetch data", tab.data(), cat.data(),
                         sch.data(), tab.data());
      else if (NAString(ptr, len) == "IX")
        loadQuery.format("select [last 0]* from TABLE(INDEX_TABLE %s.\"%s\".%s) fetch data", cat.data(), sch.data(),
                         tab.data());
      else {
      ...
      if (ret < 0) {
        ...
        msgBuf.format("An error occurred while processing object %s.%s.%s , return %d", cat.data(), sch.data(),
                      tab.data(), ret);
        returnCode = -1;
        goto bailout_label;
      }
...
void CmpSeabaseDDL::generateQueryToFetchOneTableDescriptor(const QualifiedName &tableName, char *buffer, int bufLen) {
  NAString tableNameStr;

  if (NOT tableName.getCatalogName().isNull()) {
    tableNameStr = " and O.catalog_name = '" + tableName.getCatalogName() + "' ";
  } else
  ...
  if (objName[0] == '"') objName = objName.strip(NAString::both, '"');

  tableNameStr += " and O.object_name = '" + objName + "' ";

  snprintf(buffer, bufLen,
           "select o.catalog_name, o.schema_name, o.object_name, o.object_type, t.seq_num, t.text from %s.\"%s\".%s O, "
           "%s.\"%s\".%s T where O.object_uid = T.text_uid %s and (O.object_type in ('BT', 'SS', 'PS')) and "
           "T.text_type = %d order by 1,2,3,5",
           getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_OBJECTS, getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_TEXT,
           tableNameStr.data(), COM_STORED_DESC_TEXT);
}

int CmpSeabaseDDL::updateTrafMetadataSharedCacheForTable(const QualifiedName &tableName, NABoolean insertOnly,
  ...
  while (cliRC != 100) {
    ...
    if (cliRC < 0) {
      ...
      snprintf(msgBuf, sizeof(msgBuf), "The API cliInterface.fetch() returns %d", cliRC);
      reasonString = msgBuf;
  ...
  if ((cliRC == 100) && (NOT objName.isNull()) && (gluedText.length() > 0)) {
    ...
    if (sharedHeap->allocFailed() || !key || !value) {
      QRLogger::log(CAT_SQL_SHARED_CACHE, LL_DEBUG,
                    "updateTrafMetadataSharedCacheForTable(): allocate from shared heap failed: objName=%s, "
                    "schName=%s, catName=%s, value length=%d",
                    objName.data(), schName.data(), catName.data(), gluedText.length());
...
static int packIntoBlackBox(const char *msg, char *&newBuf) {
  int bufLen = 16 + MAX_SEGMENT_NAME_LEN + 1 + strlen(msg) + 1;
  ...
  *(int *)ptr = 1;
  ...
  int len = snprintf(ptr, bufLen, "%s: %s", myNodeName, msg);
...
#define checkTrafSharedCache(T, MSG, FUN)                                           \
  {                                                                                 \
    ...
    if (!sharedCache) {                                                             \
      ...
      if (summaryData) filledBlackBoxLen = packIntoBlackBox(summaryData, blackBox); \
    }                                                                               \
                                                                                    \
...
int CmpSeabaseDDL::checkTrafMetadataSharedCacheForTable(const QualifiedName &tableName, CmpDDLwithStatusInfo *dws)
    checkTrafSharedCache(SharedDescriptorCache, "descriptor", collectSummaryDataForTable(tableName));

int CmpSeabaseDDL::checkTrafMetadataSharedCacheForSchema(const QualifiedName &schemaName, CmpDDLwithStatusInfo *dws)
    checkTrafSharedCache(SharedDescriptorCache, "descriptor", collectSummaryDataForSchema(schemaName));

int CmpSeabaseDDL::checkTrafMetadataSharedCacheAll(CmpDDLwithStatusInfo *dws)
    checkTrafSharedCache(SharedDescriptorCache, "descriptor", collectSummaryDataForAll());

int CmpSeabaseDDL::checkTrafDataSharedCacheForTable(const QualifiedName &tableName, CmpDDLwithStatusInfo *dws)
    checkTrafSharedCache(SharedTableDataCache, "data", collectSummaryDataForTable(tableName));

int CmpSeabaseDDL::checkTrafDataSharedCacheForSchema(const QualifiedName &schemaName, CmpDDLwithStatusInfo *dws)
    checkTrafSharedCache(SharedTableDataCache, "descriptor", collectSummaryDataForSchema(schemaName));

#undef checkTrafSharedCache

int CmpSeabaseDDL::checkTrafDataSharedCacheAll(CmpDDLwithStatusInfo *dws, bool showDetails) {
  char *blackBox = NULL;
  const char *reasonString = NULL;
  int filledBlackBoxLen = 0;
...
int CmpSeabaseDDL::updateTrafDataSharedCacheForTable(const vector<pair<QualifiedName, int>> &tables, NABoolean insert,
                                                     NABoolean errorAsWarning) {
  int retcode = 0;
  int cliRC = 0;
  NAString msgBuf;
  ...
  if (sharedDataCache = SharedTableDataCache::locate()) {
    ...
    for (auto &&table : tables) {
      if (sharedDataCache->contains(table.first)) {
        if (insert) {
          msgBuf.format("Failed to insert data shared cache for table %s, the table cache exist",
                        table.first.getQualifiedNameAsAnsiString(FALSE, TRUE).data());
          goto error_return;
        } else {
          if (sharedDataCache->remove(table.first)) {
            ...
            if (cliRC < 0) {
              cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
              msgBuf.format(
                  "Error in load data into share cache %s, Check data cache and execute \"alter trafodion data shared "
                  "cache for [table|index] OBJECTS delete internal\" to clear the inconsistent cache",
                  table.first.getQualifiedNameAsAnsiString(FALSE, TRUE).data());
              goto error_return;
            }
          } else {
            msgBuf.format("Failed to update data shared cache for table %s befor load data into share cache",
                          table.first.getQualifiedNameAsAnsiString(FALSE, TRUE).data());
            goto error_return;
          }
        }
      } else if (sharedDataCache->contains(table.first, true)) {
        msgBuf.format("Failed to update data shared cache for table %s, the table cache is disabled",
                      table.first.getQualifiedNameAsAnsiString(FALSE, TRUE).data());
        goto error_return;
      } else {
        NAString tableName = table.first.getQualifiedNameAsAnsiString(FALSE, TRUE);
        ...
        if (cliRC < 0) {
          ...
          msgBuf.format(
              "Error in load data into share cache %s, Check data cache and execute \"alter trafodion data shared "
              "cache for [table|index] OBJECTS delete internal\" to clear the inconsistent cache if exist",
              table.first.getQualifiedNameAsAnsiString(FALSE, TRUE).data());
          goto error_return;
        }
      }
...
int CmpSeabaseDDL::alterSharedDescCache(StmtDDLAlterSharedCache *ascNode, CmpDDLwithStatusInfo *dws) {
  char msgBuf[200];
  const char *reasonString = NULL;
  ...
  if (ascNode->opForTable() || ascNode->opForIndex()) {
    ...
    if (ascNode->doUpdate()) return updateTrafMetadataSharedCacheForTable(name, FALSE, errorAsWarning);
    if (ascNode->doInsert()) return updateTrafMetadataSharedCacheForTable(name, TRUE, errorAsWarning);
    if (ascNode->doCheck()) return checkTrafMetadataSharedCacheForSchema(name, dws);
  } else {
    if (ascNode->checkAll()) return checkTrafMetadataSharedCacheAll(dws);
  }
  ...
  if (retcode != 0) {
    snprintf(msgBuf, sizeof(msgBuf), "Error in locking a name semaphore(%d)", retcode);
    reasonString = msgBuf;
    goto error_return;
  }
    ...
    switch (ascNode->getSubject()) {
      ...
      case StmtDDLAlterSharedCache::INDEX:
        ...
        switch (ascNode->getOptions()) {
          ...
          default:
            snprintf(msgBuf, sizeof(msgBuf), "Invalid option %s requested for table %s", op.data(),
        ...
        if (!ok) {
          snprintf(msgBuf, sizeof(msgBuf), "Failed to %s the shared cache entry for table %s", op.data(),
                   name.getQualifiedNameAsAnsiString(FALSE, TRUE).data());
          reasonString = msgBuf;
          goto error_return_after_release_semaphore;
        }
      ...
      case StmtDDLAlterSharedCache::SCHEMA:
        ...
        switch (ascNode->getOptions()) {
          ...
          default:
            snprintf(msgBuf, sizeof(msgBuf), "Invalid option %s requested for schema %s", op.data(),
                     name.getQualifiedNameAsAnsiString(FALSE, TRUE).data());
            reasonString = msgBuf;
            goto error_return_after_release_semaphore;
        }
        ...
        if (!ok) {
          snprintf(msgBuf, sizeof(msgBuf), "Failed to %s the shared cache entry for schema %s", op.data(),
                   name.getQualifiedNameAsAnsiString(FALSE, TRUE).data());
          reasonString = msgBuf;
          goto error_return_after_release_semaphore;
        }
      ...
      case StmtDDLAlterSharedCache::ALL:
        ...
        switch (ascNode->getOptions()) {
          ...
          default:
            snprintf(msgBuf, sizeof(msgBuf), "Invalid option %s requested", op.data());
            reasonString = msgBuf;
            goto error_return_after_release_semaphore;
        }

        if (!ok) {
          snprintf(msgBuf, sizeof(msgBuf), "Failed to %s the shared cache", op.data());
          reasonString = msgBuf;
          goto error_return_after_release_semaphore;
        }
...
int CmpSeabaseDDL::alterSharedDataCache(StmtDDLAlterSharedCache *ascNode, CmpDDLwithStatusInfo *dws) {
  NAString msgBuf;
  const char *reasonString = NULL;
  NAString op;
  bool ok = true;
  ...
  if (name.getObjectName() != SEABASE_SCHEMA_OBJECTNAME && !isInternalOp) {
    naTable = bindWA.getNATable(cn);
    if (naTable == NULL || bindWA.errStatus()) {
      *CmpCommon::diags() << DgSqlCode(-4082) << DgTableName(name.getQualifiedNameAsAnsiString(FALSE, TRUE));
      return -1;
    }
  ...
  if (ascNode->opForTable() || ascNode->opForIndex()) {
    ...
    *CmpCommon::diags() << DgSqlCode(-1229) << DgString0("load data cache for schema");
    return -1;
  } else {
    if (ascNode->checkAll())
  ...
  if (sharedDataCache = SharedTableDataCache::locate()) {
    switch (ascNode->getSubject()) {
      ...
      case StmtDDLAlterSharedCache::INDEX:
        switch (ascNode->getOptions()) {
          case StmtDDLAlterSharedCache::DISABLE:
            ...
            for (auto &&table : names) {
              if (ok) sharedDataCache->disable(table.first);
              if (!ok) break;
            }
            break;
          ...
          case StmtDDLAlterSharedCache::ENABLE:
            ...
            op = "enable";
            for (auto &&table : names) {
              if (ok) sharedDataCache->enable(table.first);
              if (!ok) break;
            }
            break;
          ...
          case StmtDDLAlterSharedCache::DELETE:
            ...
            op = "delete";
            for (auto &&table : names) {
              if (ok) sharedDataCache->remove(table.first);
              if (!ok) break;
            }
            break;

          default:
            msgBuf.format("Invalid option %s requested for table %s", op.data(),
                          name.getQualifiedNameAsAnsiString(FALSE, TRUE).data());
            reasonString = msgBuf.data();
            goto error_return_after_release_semaphore;
        }
        ...
        if (!ok) {
          msgBuf.format("Failed to %s the shared cache entry for table %s", op.data(),
                        name.getQualifiedNameAsAnsiString(FALSE, TRUE).data());
          reasonString = msgBuf.data();
          goto error_return_after_release_semaphore;
        }
...
TrafDesc *CmpSeabaseDDL::getSeabaseRoutineDesc(const NAString &catName, const NAString &schName,
                                               const NAString &objName) {
...
TrafDesc *CmpSeabaseDDL::getSeabaseRoutineDescInternal(const NAString &catName, const NAString &schName,
                                                       const NAString &objName) {
  int retcode = 0;
  int cliRC = 0;
  ...
  objectUID = getObjectInfo(&cliInterface, catName.data(), schName.data(), objName.data(), objectType, objectOwnerID,
                            schemaOwnerID, objectFlags, objDataUID);
  ...
  str_sprintf(buf,
              "select udr_type, language_type, deterministic_bool,"
              ...
              SEABASE_LIBRARIES, getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_OBJECTS, objectUID);
  ...
  if (cliRC == 100)  // did not find the row
  {
    ...
    *CmpCommon::diags() << DgSqlCode(-CAT_OBJECT_DOES_NOT_EXIST_IN_TRAFODION) << DgString0(objName);
    return NULL;
  }
  ...
  routineInfo->max_results = *(int *)ptr;
  cliInterface.getPtrAndLen(10, ptr, len);
  routineInfo->state_area_size = *(int *)ptr;
  ...
  routineInfo->library_version = *(int *)ptr;
  ...
  ComObjectName libSQLName(libCat, libSch, libObj, COM_UNKNOWN_NAME, ComAnsiNamePart::INTERNAL_FORMAT, STMTHEAP);
  NAString libSQLExtName = libSQLName.getExternalName();
  routineInfo->library_sqlname = new (STMTHEAP) char[libSQLExtName.length() + 1];
  str_cpy_and_null((char *)routineInfo->library_sqlname, libSQLExtName.data(), libSQLExtName.length(), '\0', ' ', TRUE);
  NAString naLibSch(libSch);

  cliInterface.getPtrAndLen(22, ptr, len);
  routineInfo->lib_redef_time = *(long *)ptr;
  ...
  routineInfo->lib_obj_uid = *(long *)ptr;
  ...
  int numParams;
  char direction[50];
  str_sprintf(direction, "'%s', '%s', '%s'", COM_INPUT_PARAM_LIT, COM_OUTPUT_PARAM_LIT, COM_INOUT_PARAM_LIT);
  // Params
  if (getSeabaseColumnInfo(&cliInterface, objectUID, catName, schName, objName, (char *)direction, NULL, NULL, NULL,
                           NULL, &numParams, &paramsArray) < 0) {
    processReturn();
    return NULL;
  }

  ComTdbVirtTablePrivInfo *privInfo =
      getSeabasePrivInfo(&cliInterface, catName, schName, objectUID, objectType, schemaOwnerID);

  TrafDesc *routine_desc = NULL;
  routine_desc =
      Generator::createVirtualRoutineDesc(objName.data(), routineInfo, numParams, paramsArray, privInfo, NULL);

  if (routine_desc == NULL) processReturn();
  return routine_desc;
}

int CmpSeabaseDDL::getSeabasePartitionV2Info(ExeCliInterface *cliInterface, long btUid,
  ...
  str_sprintf(query,
              "select pp.partition_type, pp.partition_column_pos, pp.partition_column_num,"
  ...
  ComTdbVirtTablePartitionV2Info *partitionV2Info = new (STMTHEAP) ComTdbVirtTablePartitionV2Info();
  ...
  partitionV2Info->partitionType = *(int *)oi->get(0);
  ...
  int colCnt = *(int *)oi->get(2);
  partitionV2Info->partitionColCount = colCnt;

  // subpartition_type
  partitionV2Info->subpartitionType = *(int *)oi->get(3);

  // subpartition_column
  partitionV2Info->subpartitionColCount = *(int *)oi->get(5);
  ...
  str_sprintf(
      query,
      "select p.partition_uid puid, p.partition_name pname, p.partition_entity_name pnname, "
      ...
      getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_PARTITIONS, btUid, COM_PRIMARY_PARTITION_LIT, getSystemCatalog(),
      SEABASE_MD_SCHEMA, SEABASE_PARTITIONS, COM_SUBPARTITION_LIT);
  ...
  int partCnt = partitionsInfo->numEntries();
  assert(partCnt > 0);

  // need partition count and its subpartition count in advance
  int partPosAry[partCnt];
  long subpartUidAry[partCnt];
  partitionsInfo->position();
  for (int i = 0; i < partCnt; i++) {
    OutputInfo *oi = (OutputInfo *)partitionsInfo->getNext();
    partPosAry[i] = *(int *)oi->get(3);
    subpartUidAry[i] = *(long *)oi->get(5);
  }

  // first level partition count
  int firstLPartCnt = partPosAry[partCnt - 1] + 1;
  int subPartCntMap[firstLPartCnt];
  memset(subPartCntMap, 0, sizeof(int) * firstLPartCnt);
  for (int i = 0; i < partCnt; i++) {
  ...
  for (int i = 0; i < partCnt; i++) {
    ...
    if (start) {
      ...
      partInfoArray[curPartPos].partitionUid = *(long *)oi->get(0);
      ...
      if (subPartCntMap[curPartPos] > 0) {
        partInfoArray[curPartPos].subPartArray = new (STMTHEAP) ComTdbVirtTablePartInfo[subPartCntMap[curPartPos]];
    ...
    {
      curSubpartPos = *(int *)oi->get(8);
      ComTdbVirtTablePartInfo &ptmp = partInfoArray[curPartPos].subPartArray[curSubpartPos];
      ptmp.parentUid = *(long *)oi->get(0);
  ...
  if (outPartionV2InfoArray) *outPartionV2InfoArray = partitionV2Info;

  return 0;
}

int CmpSeabaseDDL::getSeabasePartitionBoundaryInfo(ExeCliInterface *cliInterface, long btUid,
                                                   ComTdbVirtTablePartitionV2Info **outPartionV2InfoArray) {
  char query[3000];
  char *data = NULL;
  int len = 0, cliRC = 0;
  ...
  // get partition info
  str_sprintf(query,
              "select part.parent_uid, part.partition_name, part.partition_entity_name, part.partition_level,"
              ...
              getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_PARTITIONS, btUid, getSystemCatalog(), SEABASE_MD_SCHEMA,
              SEABASE_PARTITIONS);
  ...
  for (int k = 0; k < partCnt; k++) {
    ...
    partInfo[k].isSubPartition = strncmp((char *)oi->get(offset + 3), COM_PRIMARY_PARTITION_LIT, 1) == 0 ? false : true;

    partInfo[k].partPosition = *(int *)oi->get(offset + 4);
  ...
  if (partInfo[0].isSubPartition)
    str_sprintf(query,
                ...
                getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_TABLE_PARTITION, getSystemCatalog(), SEABASE_MD_SCHEMA,
                SEABASE_PARTITIONS, partInfo[0].parentUid);
  else
    str_sprintf(query,
                "select pp.partition_type, pp.partition_column_pos, pp.partition_column_num, "
                "pp.subpartition_type, pp.subpartition_column_pos, pp.subpartition_column_num, "
  ...
  // uid
  partitionV2Info->baseTableUid = *(long *)oi->get(6);
  partitionV2Info->partitionType = *(int *)oi->get(0);
...
NABoolean CmpSeabaseDDL::compareDDLs(ExeCliInterface *cliInterface, NATable *naTabA, NATable *naTabB,
                                     NABoolean ignoreClusteringKey, NABoolean ignoreSaltUsing, NABoolean ignoreSaltNum,
                                     NABoolean ignoreV2Partition,  // new range partition
                                     NABoolean ignoreIndex) {
  NABoolean ret = TRUE;

  if (naTabA == NULL || naTabB == NULL) return FALSE;

  if (naTabA == naTabB || naTabA->getTableName() == naTabB->getTableName() /* may happen?*/) return TRUE;

  //
  if (NOT ignoreV2Partition && naTabA->isPartitionV2Table() != naTabB->isPartitionV2Table()) return FALSE;
  ...
  if (userColArrayA.entries() != userColArrayB.entries()) return FALSE;
...
{
  for (int i = 0; i < privActsArray.entries(); i++) {
    PrivType privType;
    ...
    if (objectType != COM_BASE_TABLE_OBJECT && objectType != COM_VIEW_OBJECT) {
      *CmpCommon::diags() << DgSqlCode(-CAT_INCORRECT_OBJECT_TYPE) << DgTableName(externalObjectName);
      return false;
    }
...

{
  for (size_t index = 0; index < container.size(); index++)
    if (container[index].privType == value) return true;
...
{
  for (size_t index = 0; index < container.size(); index++)
    if (container[index] == value) return true;
...

{
  ...
  if (colPrivs.size() == 0) {
    ...
    // just in case.
    if (objectPrivs.size() == 0) return false;

    if (objectPrivs.size() > 1) return false;

    if (objectPrivs[0] == SELECT_PRIV) return true;

    if (isGrant) return false;

    if (objectPrivs[0] == ALL_PRIVS || objectPrivs[0] == ALL_DML) return true;
  ...
  for (size_t i = 0; i < colPrivs.size(); i++)
    if (colPrivs[i].privType != SELECT_PRIV) return false;
