...
extern "C" UDR *GENERATE_NGRAM() { return new generate_ngram(); }

/*  Step 3: Write the actual UDF code*/
void generate_ngram::describeParamsAndColumns(UDRInvocationInfo &info) {
  // We always expect one input parameter
  if (info.par().getNumColumns() != 1 || info.par().getType(0).getSQLTypeClass() != TypeInfo::CHARACTER_TYPE)
    throw UDRException(38001, "Expecting a single input parameter with a character type");
...
#define BUF_LEN 1024
void generate_ngram::processData(UDRInvocationInfo &info, UDRPlanInfo &plan) {
...
extern "C" UDR *GENERATE_WILDCARD_NGRAM() { return new generate_wildcard_ngram(); }

void generate_wildcard_ngram::describeParamsAndColumns(UDRInvocationInfo &info) {
  if (info.getNumTableInputs() != 0)
    throw UDRException(38001, "UDF %s does not support table-valued inputs", info.getUDRName().c_str());
  // We always expect one input parameter
  if (info.par().getNumColumns() != 1 || info.par().getType(0).getSQLTypeClass() != TypeInfo::CHARACTER_TYPE)
    throw UDRException(38002, "Expecting a single input parameter with a character type");
  info.addFormalParameter(info.par().getColumn(0));
  info.out().addVarCharColumn("NGRAM", TRIGRAM_LEN);
}

/*  Step 3: Write the actual UDF code*/
void generate_wildcard_ngram::processData(UDRInvocationInfo &info, UDRPlanInfo &plan) {
  string ngram = info.par().getString(0);
  char srcStr[BUF_LEN];
