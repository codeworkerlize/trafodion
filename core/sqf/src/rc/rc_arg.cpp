...
void arg_proc_args2(TAD2 *args, bool inv_arg_ok, int argc, char **argv) {
    ...
    for (argi = 1; 
         ((args[argi].ip_arg_str[0] != 0) && (args[argi].iv_arg_type != TA_End)); 
         argi++)
    ...
    for (arg = arg_start; arg < argc; arg++) {
        ...
        for (argi = 0; args[argi].iv_arg_type != TA_End; argi++) {
            ...
            if ((arg_str[0] == 0) || (strcmp(argv_str, arg_str) == 0)) {
                arg_ok = true;
                ...
                case TA_StrArray:
                    ...
                    *arg_ref_str = argv_str;
                    args[argi].ip_arg_ref = ((char *) args[argi].ip_arg_ref) + sizeof(char *);
                    *((char *) args[argi].ip_arg_ref) = 0;
                    break;
                default:
