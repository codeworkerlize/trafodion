...
extractLocations(char *pp_locations, int pv_overflowtype)
{
  ...
  char *lv_ptr2 = 0;
  bool lv_done = false;
  STFS_stub::stfsDirsHDD_.numLocations = 0;
  STFS_stub::stfsDirsHDD_.currIndex = -1;
  STFS_stub::stfsDirsHDD_.lastInstanceNum= -1;
  STFS_stub::stfsDirsHDD_.processedDirs= false;
  ...
  while (lv_ptr1 && *lv_ptr1 && !lv_done) {
    ...
    if (lv_ptr2) {
      if( STFS_stub::diskError_ == true)  // when there is disk error, we should do the check
      {
        char dirname[4096];
        memset(dirname, 0, 4096);
        strncpy(dirname, lv_ptr1, lv_ptr2 - lv_ptr1);
        if(checkDirGood(dirname) == 1)
        {
          TRACE_PRINTF3(1,"STFS dir %s is broken error no is %d\n", lv_ptr1, errno);
          lv_ptr1 = lv_ptr2 + 1;
          continue;
        }
      }
...
stfs_fhndl_t STFS_mkstemp_instnum (char *ctemplate, int pv_instnum, bool doDiskCheck)
{
  short lv_retcode = 0;
  stfs_fhndl_t lv_fh = -1;

  STFS_stub::diskError_ = doDiskCheck;
...
stfs_fhndl_t STFS_mkstemp (char *ctemplate)
{
  ...
  TRACE_PRINTF2(1, "STFS_mkstemp, template: %s\n",
		ctemplate);
  return STFS_mkstemp_instnum(ctemplate, 0, false);
}
