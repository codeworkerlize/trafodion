...
public class SsccTransactionalTable extends TransactionalTable {
    ...
    */
    ...
    public SsccTransactionalTable( final byte[] tableName, Connection connection) throws IOException {
        ...
        SsccTransactionalTable.connection = connection;
    }

    public int broadcastRequest(final int requestType, final boolean requestBool) throws IOException{
       if (LOG.isTraceEnabled()) LOG.trace("Enter SsccTransactionalTable.broadcastRequest");
     ...
     * @since 0.20.0
     */
    public Result get(final TransactionState transactionState, final long savepointId, final Get get) throws IOException {

       return get(transactionState, savepointId, /* readCommitted */ 10, /* skipConflictAccess */ false, get, /* migrate */ true);

    }

    public Result get(final TransactionState transactionState, final long savepointId,
             final boolean skipConflictAccess, final Get get) throws IOException {

       return get(transactionState, savepointId, /* readCommitted */ 10, skipConflictAccess, get, /* migrate */ true);

    }

    public Result get(final TransactionState transactionState, final long savepointId, final int isolationLevel,
          final boolean skipConflictAccess, final Get get, final boolean bool_addLocation) throws IOException {
      if (LOG.isTraceEnabled()) LOG.trace("Enter TransactionalTable.get skipConflictAccess " + skipConflictAccess);

      if (bool_addLocation) addLocation(transactionState, connection.getRegionLocator(this.getName()).getRegionLocation(get.getRow()));
          final String regionName = connection.getRegionLocator(this.getName()).getRegionLocation(get.getRow()).getRegionInfo().getRegionNameAsString();
     ...
     * @since 0.20.0
     */
    public void delete(final TransactionState transactionState, final long savepointId, final Delete delete) throws IOException {

       delete(transactionState, savepointId, delete, true);

    }

    public void delete(final TransactionState transactionState, final long savepointId, final Delete delete, final boolean bool_addLocation) throws IOException {
        SingleVersionDeleteNotSupported.validateDelete(delete);
        if (bool_addLocation) addLocation(transactionState, connection.getRegionLocator(this.getName()).getRegionLocation(delete.getRow()));
        final String regionName = connection.getRegionLocator(this.getName()).getRegionLocation(delete.getRow()).getRegionInfo().getRegionNameAsString();
    ...
    public void deleteRegionTx(final long tid, final Delete delete, final boolean autoCommit, final boolean recoveryToPitMode) throws IOException {
      SingleVersionDeleteNotSupported.validateDelete(delete);
      final String regionName = connection.getRegionLocator(this.getName()).getRegionLocation(delete.getRow()).getRegionInfo().getRegionNameAsString();
     ...
     * @since 0.20.0
     */
    public synchronized void put(final TransactionState transactionState, final long savepointId, final Put put) throws IOException {

      if (LOG.isTraceEnabled()) LOG.trace("TransactionalTable.put without location ENTRY");
      put(transactionState, savepointId, put, true);
      if (LOG.isTraceEnabled()) LOG.trace("TransactionalTable.put without location EXIT");

    }

    public synchronized void put(final TransactionState transactionState, final long savepointId, final Put put, final boolean bool_addLocation) throws IOException{
      validatePut(put);
      if (LOG.isTraceEnabled()) LOG.trace("TransactionalTable.put ENTRY adding location with transactionState: " + transactionState);

      if (bool_addLocation) addLocation(transactionState, connection.getRegionLocator(this.getName()).getRegionLocation(put.getRow()));
      final String regionName = connection.getRegionLocator(this.getName()).getRegionLocation(put.getRow()).getRegionInfo().getRegionNameAsString();
        ...
        if(resultArray[0].getStatus() != STATELESS_UPDATE_OK && resultArray[0].getStatus() != STATEFUL_UPDATE_OK){
  ...
  public synchronized ResultScanner getScanner(final TransactionState transactionState, final long savepointId, final int isolationLevel,
		  final boolean skipConflictAccess, final Scan scan) throws IOException {
    if (LOG.isTraceEnabled()) LOG.trace("Enter TransactionalTable.getScanner skipConflictAccess " + skipConflictAccess);
   ...
   * @since 0.20.0
   */
  public synchronized void putRegionTx(final long tsId, final Put put, final boolean autoCommit, final boolean recoveryToPitMode) throws IOException{
   ...
   public boolean checkAndDeleteRegionTx(final long tid, final byte[] row, final byte[] family,
          final byte[] qualifier, final byte[] value, final Delete delete, final boolean autoCommit, final boolean recoveryToPitMode) throws IOException {
  ...
  public boolean checkAndDelete(final TransactionState transactionState, final long savepointId, final byte[] row, final byte[] family,
           final byte[] qualifier, final byte[] value, final Delete delete, final boolean skipCheck) throws IOException {
	...
	public boolean checkAndPut(final TransactionState transactionState, final long savepointId,
			final byte[] row, final byte[] family, final byte[] qualifier,
			final byte[] value, final Put put, final boolean skipCheck) throws IOException {
...
if (LOG.isTraceEnabled()) LOG.trace("checkAndPut, seting request startid: " + transactionState.getStartId());
	...
	public boolean checkAndPutRegionTx(final long tsId,
			final byte[] row, final byte[] family, final byte[] qualifier,
			final byte[] value, final Put put, final boolean autoCommit, final boolean recoveryToPitMode) throws IOException {
	...
	/**
   	 ...
   	 * @throws IOException
   	 */
     public void delete(final TransactionState transactionState, final long savepointId, List<Delete> deletes) throws IOException {
         ...
         * @throws IOException
         */
        public void put_rowset(final TransactionState transactionState, final long savepointId,
                        final List<Put> puts) throws IOException {
                // call standard puts (no support for puts for regions in parallel for SSCC)
                put(transactionState, savepointId, puts);
        }
       ...
       public void put_nontxn_rowset(final long nonTransactionId, final long commitId, final long flags, final List<Put> puts) throws IOException {
                ...
                throw new IOException("ERROR no support for non transactional IBR DML operation for SSCC tables");
        }
	 ...
	 * @throws IOException
	 */
	public void put(final TransactionState transactionState, final long savepointId,
			final List<Put> puts) throws IOException {
	...
	public HRegionLocation getRegionLocation(byte[] row, boolean f)
                                  throws IOException {
        return connection.getRegionLocator(this.getName()).getRegionLocation(row, f);
    }
    ...
    public byte[][] getEndKeys()
                    throws IOException
    {
        return connection.getRegionLocator(this.getName()).getEndKeys();
    }
    public byte[][] getStartKeys() throws IOException
    {
        return connection.getRegionLocator(this.getName()).getStartKeys();
    }
    ...
    public ResultScanner getScanner(Scan scan, float DOPparallelScanner) throws IOException
    {
        if (scan.isSmall() || DOPparallelScanner == 0)
            ...
            return new TrafParallelClientScanner(SsccTransactionalTable.connection, scan, getName(), DOPparallelScanner);
    }
