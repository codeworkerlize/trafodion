...
public class HBaseBackedTransactionLogger implements TransactionLogger {
   ...
   static Object adminConnLock = new Object();
        ...
        if (rpcTimeout != null){
           synchronized(adminConnLock) {
              if (adminConnection == null) {
                 adminConf = new Configuration(connection.getConfiguration());
    ...
    public void setStatusForTransaction(final long transactionId, final TransactionStatus status)  {
        ...
        update.addColumn(INFO_FAMILY, STATUS_QUALIFIER, HConstants.LATEST_TIMESTAMP, Bytes.toBytes(status.name()));
