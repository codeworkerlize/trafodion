...
public class SsccTransactionState extends TransactionState{
    ...
    public SsccTransactionState(final long transactionId, final long rLogStartSequenceId, AtomicLong hlogSeqId,
       ...
       super(transactionId,rLogStartSequenceId,hlogSeqId,regionInfo,htd,hLog,logging,SsccSequenceId, /* region TX */ false, /* IBR */ false, regionInfo.getRegionNameAsString());
    ...
    public SsccTransactionState(final long transactionId, final long rLogStartSequenceId, AtomicLong hlogSeqId,
        ...
        super(transactionId,rLogStartSequenceId,hlogSeqId,regionInfo,htd,hLog,logging,SsccSequenceId,regionTx, false, regionInfo.getRegionNameAsString());
    ...
    */
    public boolean handleResult(Cell inputCell,List<Cell> statusList,List<Cell> versionList,List<Cell> colList, long gTransId,  boolean disableChecking) 
    {
        byte[] version = null;
        ...
        try{
            ...
            */
            if(thisTs > startId  && disableChecking == false)  //only for debug checking, like an assert
            {
            ...
            for(Cell v: versionList) {
                    ...
                    if (commitId > startId  && disableChecking == false) {
                    ...
                    if(colList != null)
                    {
                        ...
                        {
                            sameCommit = true;
                            if(LOG.isTraceEnabled()) LOG.trace("handleResult: sameCommit "
                                 + sameCommit + " check version item cid: " + commitId );
            ...
            if(disableChecking == true ) 
              { if(versionIsDelete == false ) finalret = true; else finalret = false;}
            else
