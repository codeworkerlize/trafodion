...
//   case RET_WALDOWN: return FEABORTEDTRANSID;
...
int HbaseTM_initialize (short pv_nid)
{
  ...
  char *costThStr = getenv("RECORD_TIME_COST_JNI");
  if (costThStr != NULL)
    costTh = atoi(costThStr);
...
int CHbaseTM::initJVM() 
{
  ...
  JavaMethods_[JM_PRECOMMIT  ].jm_signature = "(J[B)S";
  ...
  JavaMethods_[JM_REGREGION  ].jm_signature = "(JJI[BJ[BII)S";
  ...
  JavaMethods_[JM_CREATETABLE ].jm_signature = "(J[B[Ljava/lang/Object;I)S";
  JavaMethods_[JM_PUSHEPOCH ].jm_name      = "callPushOnlineEpoch";
  JavaMethods_[JM_PUSHEPOCH ].jm_signature = "(J[B)S";
  ...
  JavaMethods_[JM_SDNTRAN ].jm_name = "sdnTran";
  JavaMethods_[JM_SDNTRAN ].jm_signature = "()I";
  JavaMethods_[JM_COMMITSAVEPOINT].jm_name = "doCommitSavepoint";
  JavaMethods_[JM_COMMITSAVEPOINT].jm_signature = "(JJJ)S";
  JavaMethods_[JM_ABORTSAVEPOINT].jm_name = "abortSavepoint";
  JavaMethods_[JM_ABORTSAVEPOINT].jm_signature = "(JJJ)S";


  char className[]="org/trafodion/dtm/HBaseTxClient";
  return (HBTM_RetCode)JavaObjectInterfaceTM::init(className, javaClass_, (JavaMethodInitTM*)&JavaMethods_, (int)JM_TMLAST, false);
}
...
short CHbaseTM::prepareCommit(int64 pv_transid , char *pa_querycontext, int pv_querycontext_len, char *errstr, int &errstrlen) {
  jlong   jlv_transid = pv_transid;
  JOI_RetCode lv_joi_retcode = JOI_OK;
  lv_joi_retcode = initJNIEnv();
  if (lv_joi_retcode != JOI_OK) {
   ...
   if( pv_querycontext_len < 0  || pv_querycontext_len > TM_MAX_QUERYCONTEXT_STRING)
   {
      _tlp_jenv->PopLocalFrame(NULL);
      return RET_ADD_PARAM;
   }
     
   jbyteArray jba_querycontext = _tlp_jenv->NewByteArray(pv_querycontext_len);
   if (jba_querycontext == NULL) {
      _tlp_jenv->PopLocalFrame(NULL);
      return RET_ADD_PARAM;
   }
   _tlp_jenv->SetByteArrayRegion(jba_querycontext, 0, pv_querycontext_len, (const jbyte*) pa_querycontext);

  jshort jresult = _tlp_jenv->CallShortMethod(javaObj_, JavaMethods_[JM_PRECOMMIT].methodID, jlv_transid, jba_querycontext);
...
//   strncpy(la_hostname,pa_hostname,pv_hostname_Length);
  ...
  //printf("CHbaseTM::registerRegion : Txn ID (%ld), startid %ld, hostname: %s.\n",
  //				 pv_transid, jlv_startid, la_hostname);
  JOI_RetCode lv_joi_retcode = JOI_OK;
  lv_joi_retcode = initJNIEnv();
  if (lv_joi_retcode != JOI_OK) {
  ...
  jbyteArray jba_hostname = _tlp_jenv->NewByteArray(pv_hostname_Length);
...
//----------------------------------------------------------------------------
//CHbaseTM: sdnTran
//Purpose: returns number of SDN personality Trans.
//----------------------------------------------------------------------------
int CHbaseTM::sdnTran()
{
  jint  lv_jcount = 0; 
  int lv_count = 0;
  ...
  lv_joi_retcode = initJNIEnv();
  if (lv_joi_retcode != JOI_OK) {
   tm_log_write(DTM_SEA_SOFT_FAULT, SQ_LOG_CRIT, (char *)"CHbaseTM::sdnTran()", (char *)_tlp_error_msg->c_str(), -1LL);
   abort();
  }
  
  lv_jcount = _tlp_jenv->CallShortMethod(javaObj_,
                 JavaMethods_[JM_SDNTRAN].methodID);
                 
  if (getExceptionDetails(NULL)) {
    tm_log_write(DTM_TM_JNI_ERROR, SQ_LOG_ERR, (char *)"CHbaseTM::sdnTran()", (char *)_tlp_error_msg->c_str());
    _tlp_jenv->PopLocalFrame(NULL);
    return RET_EXCEPTION;
  }
  _tlp_jenv->PopLocalFrame(NULL);
...
//----------------------------------------------------------------------------
// CHbaseTM: pushOnlineEpoch
// Purpose: To push online epoch following create table transactional requests
//----------------------------------------------------------------------------
int CHbaseTM::pushOnlineEpoch(int64 pv_transid,
                           ...
                           int &errstrlen)
{
   ...
   HBASETrace(HBASETM_TraceExitError,
              (HDR "CHbaseTM::pushOnlineEpoch returning %d.\n", lv_error));

   JOI_RetCode lv_joi_retcode = JOI_OK;
   lv_joi_retcode = initJNIEnv();
   if (lv_joi_retcode != JOI_OK) {
      tm_log_write(DTM_SEA_SOFT_FAULT, SQ_LOG_CRIT, (char *)"CHbaseTM::pushOnlineEpoch()", (char *)_tlp_error_msg->c_str(), -1LL);
      abort();
   }
   ...
   lv_error = _tlp_jenv->CallShortMethod(javaObj_,
                    JavaMethods_[JM_PUSHEPOCH].methodID,
                    jlv_transid,
                    jba_tbldesc);
  ...
  if (getExceptionDetails(NULL)) {
     ...
     strncpy(errstr, _tlp_error_msg->c_str(), errstrlen);
     errstr[errstrlen] = '\0';
     tm_log_write(DTM_TM_JNI_ERROR, SQ_LOG_ERR, (char *)"CHbaseTM::pushOnlineEpoch()", (char *)_tlp_error_msg->c_str(), pv_transid);
     _tlp_jenv->PopLocalFrame(NULL);
     return RET_EXCEPTION;
  }

   HBASETrace(HBASETM_TraceExit, (HDR "CHbaseTM::pushOnlineEpoch : Error %d, Txn ID (%d,%d), hostname %s.\n",
...
short CHbaseTM::commitSavepoint(int64 pv_transid, int64 pv_svpt_id, int64 pv_psvpt_id)
{
  jlong jlv_transid = pv_transid;
  jlong jlv_svpt_id = pv_svpt_id;
  jlong jlv_psvpt_id = pv_psvpt_id;
  JOI_RetCode lv_joi_retcode = JOI_OK;
  lv_joi_retcode = initJNIEnv();
  if (lv_joi_retcode != JOI_OK) {
    tm_log_write(DTM_SEA_SOFT_FAULT, SQ_LOG_CRIT, (char *)"CHbaseTM::commitSavepoint()",
                 (char *)_tlp_error_msg->c_str(), -1LL);
    abort();
  }

  jshort jresult = _tlp_jenv->CallShortMethod(javaObj_, JavaMethods_[JM_COMMITSAVEPOINT].methodID,
                                              jlv_transid, jlv_svpt_id, jlv_psvpt_id);
  if (getExceptionDetails(NULL)) {
     tm_log_write(DTM_TM_JNI_ERROR, SQ_LOG_ERR, (char *)"CHbaseTM::commitSavepoint()",
...
short CHbaseTM::abortSavepoint(int64 pv_transid, int64 pv_svpt_id, int64 pv_psvpt_id)
{
  jlong  jlv_transid = pv_transid;
  jlong  jlv_svpt_id = pv_svpt_id;
  jlong  jlv_psvpt_id = pv_psvpt_id;
  ...
  lv_joi_retcode = initJNIEnv();
  if (lv_joi_retcode != JOI_OK) {
    tm_log_write(DTM_SEA_SOFT_FAULT, SQ_LOG_CRIT, (char *)"CHbaseTM::abortSavepoint()",
                 (char *)_tlp_error_msg->c_str(), -1LL);
    abort();
  }

  jshort jresult = _tlp_jenv->CallShortMethod(javaObj_, JavaMethods_[JM_ABORTSAVEPOINT].methodID,
                                              jlv_transid, jlv_svpt_id, jlv_psvpt_id);
  if (getExceptionDetails(NULL)) {
     tm_log_write(DTM_TM_JNI_ERROR, SQ_LOG_ERR, (char *)"CHbaseTM::abortSavepoint()",
                  (char *)_tlp_error_msg->c_str(), pv_transid);
     _tlp_jenv->PopLocalFrame(NULL);
     return RET_EXCEPTION;
  }
...
// ===== Class HashMapArray
// ===========================================================================
JavaMethodInitTM * HashMapArray::JavaMethods_ = NULL;
...
HMN_RetCode HashMapArray::init()
{
   ...
   {
      JavaMethods_ = new JavaMethodInitTM[JM_MAPLAST];
...
char* HashMapArray::getRegionInfo(int64 tid)
{
   ...
   const jbyte* jb_val = (jbyte*)_tlp_jenv->GetStringUTFChars(js_val, NULL);
   //jd_val must be freed by jvm
   //char* cp_val = (char *)jb_val;
   char *cp_val = NULL;
   size_t len = strlen((char *)jb_val);
   if (len > 0)
   {
       cp_val = new char[len + 1];
       strcpy(cp_val, (char *)jb_val);
   }
   _tlp_jenv->ReleaseStringUTFChars(js_val, (const char *)jb_val);
   _tlp_jenv->PopLocalFrame(NULL);
   return cp_val;
}
