...
static int do_init(JNIEnv *pp_j_env) {
    ...
    if (gv_inited)
        ...
        lv_argc = 0;
        la_argv[0] = NULL;
        //gv_inited = true; //set true if lv_ferr == XZFIL_ERR_OK before return
        try {
        ...
        if (lv_ferr == XZFIL_ERR_OK) {
            try {
                //lv_ferr = msg_mon_process_startup4(false, true, true);
                  lv_ferr = msg_mon_process_startup3(false, false);
            } catch (SB_Fatal_Excep &fatal_exc) {
                if (gv_verbose)
    ...
    if (lv_ferr == XZFIL_ERR_OK) {
        if (gv_oid < 0) {
            ...
            if (gv_verbose)
                printf("cli: open-err=%d, gv_oid = %d\n", lv_ferr, gv_oid);

            if (lv_ferr != XZFIL_ERR_OK && gv_oid == 0)
              gv_oid = -1;
        }
    }
