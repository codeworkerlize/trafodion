...
{
  CTmTxKey k(gv_tm_info.instance_clusterid(), node(), seqnum());
   cleanup();
   return k.id();
} //CTmTxBase::cleanPoolElement
                            ...
                            int32 pv_rm_wait_time)
{
   ...
   iv_transid.iv_instance_cluster_id = gv_tm_info.instance_clusterid();
...
void CTmTxBase::eventQ_push(CTmTxMessage * pp_msg, bool pv_threadAssociated)
{
   ...
   if (!pv_threadAssociated && !isThreadPendingOrAssociate())
   {
   ...
   else
   {
      ...
      else
      {
         ...
         // Check here to make sure that the transaction object
         // isn't already on the queue or we could end up with duplicates.
         // To ensure uniqueness of the queue, we also scan the queue and remove "duplicates"
         // if any when pick a entry from it.
         gv_tm_info.txdisassociatedQ()->lock();
         if (!isThreadPendingOrAssociate() &&
             this->iv_in_txdisassociatedQ == false)
           {
             gv_tm_info.txdisassociatedQ()->push_nolock(this);
             this->iv_in_txdisassociatedQ = true;
           }
...
void CTmTxBase::eventQ_push_top(CTmTxMessage * pp_msg)
{
   ...
   if(ip_Thread)
   {
      ...
      iv_CV.signal(false /*already locked*/);
   }
   else
...
void CTmTxBase::removeTimerEvent(CTmTimerEvent *pp_event)
{
   TMTrace (2, ("CTmTxBase::remoteTimerEvent BASE CLASS!: ID %d, current txn state %d\n",
      seqnum(), tx_state()));
}
...
bool CTmTxBase::req_commit_svpt(CTmTxMessage *pp_msg)
{
   TMTrace (2, ("CTmTxBase::req_commit_svpt BASE CLASS!: ID %d, error=%d\n", 
                    seqnum(), pp_msg->responseError()));
   abort();
   return false; //Leave transaction thread alive
}

bool CTmTxBase::req_abort_svpt(CTmTxMessage *pp_msg)
{
   TMTrace (2, ("CTmTxBase::req_abort_svpt BASE CLASS!: ID %d, error=%d\n", 
                    seqnum(), pp_msg->responseError()));
   abort();
   return false; //Leave transaction thread alive
}
...
{
    CTmTxKey lv_key(gv_tm_info.instance_clusterid(), pv_nid, pv_pid);
