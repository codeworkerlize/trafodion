...
void CTxThread::associate(TM_TX_Info * pp_txn)
{
   ...
   //no matter whether pp_txn is in txdisassociatedQ() or not
   //we set iv_in_txdisassociatedQ to false
   pp_txn->intxdisassociatedQ(false);
...
void CTxThread::eventQ_push(CTmEvent *pp_event)
{
   ...
   eventQ_CV()->signal(false /*already locked*/);
...
CTmEvent * CTxThread::eventQ_pop()
{
   ...
   while (eventQ()->empty())
   {
      ...
      eventQ_CV()->wait(false /*already locked*/);
   }
   eventQ_CV()->unlock();
