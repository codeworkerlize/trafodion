   ...
   :iv_stats(TM_STATS, TM_STATS_INTERVAL)
{
   ...
   assert(lv_instance_id < 256);
   //printf("Instance id: %d\n", lv_instance_id);
   
   int32 lv_cluster_id = 0;
   ms_getenv_int("TRAF_CLUSTER_ID", &lv_cluster_id);
   assert(lv_cluster_id < 256);
   //printf("Cluster id: %d\n", lv_cluster_id);

   iv_seqnum_mode = 0;
   ms_getenv_int("TRAF_SEQNUM_MODE", &iv_seqnum_mode);

   iv_instance_cluster_id = (short) ((lv_instance_id << 8) | lv_cluster_id);
   //printf("Instance and Cluster id: %d\n", iv_instance_cluster_id);
   ...
   iv_tm_refresh_mon_interval = TM_REFRESH_MON_DEFAULT * 1000; //sec to msec
   unlock();
}
...
*/   
    ...
    ms_getenv_int ("DTM_MAX_NUM_TRANS", &lv_max_num_trans);
    if (lv_max_num_trans)
    {   
      TMTrace (1, ("Enabling DTM_MAX_NUM_TRANS from env variable\n"));
    }
    else 
    {
      lv_max_num_trans = MAX_NUM_TRANS;
    }
    ms_getenv_int ("DTM_STEADYSTATE_LOW_TRANS", &lv_ss_low_trans);
    if (lv_ss_low_trans)
    {
      TMTrace (1, ("Enabling DTM_STEADYSTATE_LOW_TRANS from env variable\n"));
    }
    else 
    {
      lv_ss_low_trans = STEADYSTATE_LOW_TRANS;
    }
    ms_getenv_int ("DTM_STEADYSTATE_HIGH_TRANS", &lv_ss_high_trans);
    if (lv_ss_high_trans)
    {
      TMTrace (1, ("Enabling DTM_STEADYSTATE_HIGH_TRANS from env variable\n"));
    }
    else 
    {
      lv_ss_high_trans = STEADYSTATE_HIGH_TRANS;
    }
  ...
  // get perf stats mode DTM_PERF_STATS
    ...
    int lv_RMPartic;
    ms_getenv_int ("DTM_RM_PARTIC", &lv_RMPartic);
    if (lv_RMPartic)
       iv_RMPartic = (lv_RMPartic != 0);
    else
    {   
    ...
    // registry value, then the default.
    int lv_TLOGperTM;
    ms_getenv_int ("DTM_TLOG_PER_TM", &lv_TLOGperTM);
    if (lv_TLOGperTM)
      iv_TLOGperTM = (lv_TLOGperTM == 0);
    else
    {   
       // Check for registry value
       lv_error = tm_reg_get(MS_Mon_ConfigType_Cluster, 
    ...
    {   
     ...
     int32 tmRefreshInt = 0;
     ms_getenv_int("TM_REFRESH_MON_INTERVAL", &tmRefreshInt);
     if (tmRefreshInt)
     {
       iv_tm_refresh_mon_interval = tmRefreshInt * 1000; //sec to msec
       TMTrace (1, ("Setting iv_tm_refresh_mon_interval to %ds. \n", tmRefreshInt));
     }
                      ...
                      void * (*constructPoolElement)(int64))
{
   ...
   if (pv_node != -1 && pv_seqnum != -1)
     lv_txKey.set(gv_tm_info.instance_clusterid(), pv_node, pv_seqnum);
   else
      // Allocate a new sequence number
     //      lv_txKey.set(gv_tm_info.nid(), tm_new_seqNum());
     lv_txKey.set(gv_tm_info.instance_clusterid(), gv_tm_info.nid(), tm_new_seqNum());
...
// ------------------------------------------------------------
void * TM_Info::get_tx(int16 pv_instance_cluster_id, int16 pv_node, int32 pv_seq)
{
   CTmTxKey k(pv_instance_cluster_id, pv_node, pv_seq);
...
{
  CTmTxKey k(pv_transid->iv_instance_cluster_id, pv_transid->iv_node, pv_transid->iv_seq_num);
...
void TM_Info::remove_tx (CTmTxBase * pp_tx)
{
   ...
   CTmTxKey k(pp_tx->instance_clusterid(), pp_tx->node(), pp_tx->seqnum());
...
int32 TM_Info::accessSeqNumBlockFile(unsigned int *seqNum, bool accessMode, int *errNo)
{
   static char* filePath = getenv("TRAF_VAR");
   FILE *fileHandler = NULL;
   char *value, *checksum;
   unsigned int *tempSeqNum = NULL;
   char fileBuff[20] = {0};
   char fullPath[256];
   char tempPath[256];

   if (seqNum == NULL || errNo == NULL)
      return -1;

   value = fileBuff;
   checksum = fileBuff + sizeof(unsigned int);
   snprintf(fullPath, sizeof(fullPath), "%s/tid_seq.config", filePath);
   snprintf(tempPath, sizeof(tempPath), "%s/tid_seq.temp", filePath);
   if (accessMode)
   {
      bool readTmpFail = false;
      fileHandler = fopen(tempPath, "rb");
      if (fileHandler == NULL)
      {
      ...
      if (1 != fread(fileBuff, 2 * sizeof(unsigned int), 1, fileHandler)
          || (*errNo = ferror(fileHandler)))
      {
         fclose(fileHandler);
         readTmpFail = true;
         goto readConfig;
      ...
      while (value < fileBuff + sizeof(unsigned int))
      {
         if (*checksum != (*value ^ 32))
         {
            fclose(fileHandler);
            readTmpFail = true;
            goto readConfig;
      ...
      tempSeqNum = (unsigned int *)fileBuff;
      *seqNum = *tempSeqNum;
      fclose(fileHandler);
...
readConfig:
   ...
   else
   {
      ...
      *tempSeqNum = *seqNum;
      ...
      if (1 != fwrite(fileBuff, 2 * sizeof(unsigned int), 1, fileHandler)
          || (*errNo = ferror(fileHandler)))
         return -7;

      if (0 != fflush(fileHandler) || (*errNo = ferror(fileHandler)))
         return -8;

      if (fsync(fileno(fileHandler)) || (*errNo = ferror(fileHandler)))
         return -9;
...
unsigned int TM_Info::setNextSeqNumBlock()
{
   ...
   if (lv_firstTime)
   {
      ...
      if (iv_seqnum_mode)
         ...
         lv_error = accessSeqNumBlockFile(&lv_startSeqNum, true, &error_no);
      ...
      if (lv_error == 0)
      {
         ...
         lv_startSeqNum += iv_SeqNumInterval;
      }
      else if (0 == iv_seqnum_mode)
      {
         char la_buf[DTM_STRING_BUF_SIZE];
         sprintf(la_buf, "TM_Info::setNextSeqNumBlock : proc:%s, failed to get seqnum block:%u error: %d errorno %d \n",
                 la_tm_name, lv_startSeqNum, lv_error, error_no);
         tm_log_write(DTM_TM_REGISTRY_SET_ERROR, SQ_LOG_ERR, la_buf);
         abort ();
      }
   ...
   if (iv_seqnum_mode)
   {
      ...
      lv_error = accessSeqNumBlockFile(&iv_nextSeqNumBlockStart, false, &error_no);

   if (lv_error)
   {
        char la_buf[DTM_STRING_BUF_SIZE];
        tm_log_event(DTM_TM_REGISTRY_SET_ERROR, SQ_LOG_CRIT, "DTM_TM_REGISTRY_SET_ERROR", lv_error);
        sprintf(la_buf, "TM_Info::setNextSeqNumBlock : proc:%s, failed to set next seqnum block:%u error:%d errorno: %d\n",
                la_tm_name, iv_nextSeqNumBlockStart, lv_error, error_no);
        tm_log_write(DTM_TM_REGISTRY_SET_ERROR, SQ_LOG_ERR, la_buf);
        abort (); 
    }
...
unsigned int TM_Info::getSeqNumBlock(int32 pv_blockSize)
{
   ...
   if (lv_updateRegistry) {
      ...
      {
           char la_buf[DTM_STRING_BUF_SIZE];
           sprintf(la_buf, "TM_Info::setNextSeqNumBlock : proc:%s, failed to update seqnum block:%u error:%d errorno: %d\n", 
                   la_tm_name, iv_nextSeqNumBlockStart, lv_error, error_no);
           tm_log_write(DTM_TM_REGISTRY_SET_ERROR, SQ_LOG_ERR, la_buf);
           tm_log_event(DTM_TM_REGISTRY_SET_ERROR, SQ_LOG_CRIT, "DTM_TM_REGISTRY_SET_ERROR", lv_error);
           abort (); 
...
bool TM_Info::check_for_queued_requests(CTxThread *pp_thread)
{
   ...
   if (lp_txn)
   {
      ...
      iv_txdisassociatedQ.remove_all_nolock(lp_txn); //delete all entry
      lp_txn->threadPending(true);
      lp_txn->intxdisassociatedQ(false);
   }
...
void TM_Info::stopTimerEvent()
{
   ...
   if (tmTimer() && false == tmTimer()->isExited())
      tmTimer()->eventQ_push((CTmEvent *) lp_timerEvent);
   ...
   while (tmTimer()->state() != TmTimer_Down)
   {
      ...
      if (200 < lv_waitCount || tmTimer()->isExited())
         break;
...
//----------------------------------------------------------------------------
//TM_Info::sdnTrans
//Purpose : Send a sdnTrans message to all TMs.  Each TM will return
//number of SDN personality transactions. If none, then FEOK is returned 
//else FE_SDNTRAN is returned.
//----------------------------------------------------------------------------
void TM_Info::sdnTrans(CTmTxMessage * pp_msg)
{
  int32 lv_error = FEOK;
  int32 lv_retries = 0;
  int32 lv_sdn_tran = -1; //non zero to begin with.
 
  TMTrace (2, ("TM_Info::sdnTrans : ENTRY\n"));
  ...
  {     
    ...
    {
      lv_sdn_tran = -1;
      while ((lv_sdn_tran) && (lv_retries++ < 100))
      {
        lv_sdn_tran = gv_HbaseTM->sdnTran();
        if (lv_sdn_tran)
           SB_Thread::Sthr::sleep  (1000); // sleep 1 second
      }
      ...
      {
        lv_error = FE_SDNTRAN;
        TMTrace(1, ("TM_Info::sdnTrans - Error %d returned by sendAllTMs.\n", lv_error));
      }
      else
      {
        lv_error = FEOK;
        TMTrace (3, ("TM_Info::sdnTrans: TMs successfully SDN drained.\n"));
      }
    }
  }
  // non lead DTMs need to drain, hold, and reply back to lead
  ...
  TMTrace(2, ("TM_Info::sdnTrans : EXIT, error %d.\n", lv_error));
}
...

{
    ...
    TMTrace(2, ("TM_Info::link ENTRY : ID (%d,%d), linker tag %ld, linkopts %d.\n",
            pp_transid ? pp_transid->get_node() : -1, pp_transid ? pp_transid->get_seq_num() : -1,
            pv_linkertag, pv_linkopts));
    ...
    do {
       ...
       {  // Message Descriptor depletion.  This means we ran out of MDs.
          ...
          tm_log_event(DTM_TM_LINK_PAUSED, SQ_LOG_WARNING, "DTM_TM_LINK_PAUSED",
                       lv_ret, -1,
                       pp_transid ? pp_transid->get_node() : -1,
                       pp_transid ? pp_transid->get_seq_num() : -1,
                       -1, -1, -1, -1, lv_retries,
