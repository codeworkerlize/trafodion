...
const char *TMLIB::mrminterface_classname = "org/apache/hadoop/hbase/client/transactional/MRMInterface";

jclass TMLIB::hbasetxclient_class = NULL;
jclass TMLIB::RMInterface_class = NULL;
jclass TMLIB::MRMInterface_class = NULL;

JavaMethodInitTM* TMLIB::TMLibJavaMethods_ = NULL;
bool TMLIB::javaMethodsInitialized_ = false;
TM_Mutex *TMLIB::initMutex_ = NULL;
bool TMLIB::enableCleanupRMInterface_ = false;
bool TMLIB::enableCleanupMRMInterface_ = false;
...
int tmlib_callback2 (MS_Mon_Tmlib_Fun_Type pv_fun,
                    ...
                    SB_Int64_Type        pv_svptid,
                    SB_Int64_Type       *pp_svptid_out,
                    SB_Int64_Type        pv_psvptid,
                    SB_Int64_Type       *pp_psvptid_out)

{
    ...
    SB_Int64_Type lv_svptid;
    SB_Int64_Type lv_psvptid;
   ...
   TMlibTrace(("TMLIB_TRACE : tmlib_callback2 ENTRY with function %d \n",
    ...
    case TMLIB_FUN_GET_TX:
    {
         ...
         if (gp_trans_thr->get_current() != NULL)
         {
             ...
             else
             {
                 ...
                 // get savepoint id
                 *pp_svptid_out = gp_trans_thr->get_current()->getSvptId();
                 *pp_psvptid_out = gp_trans_thr->get_current()->getPSvptId();
    ...
    case TMLIB_FUN_REG_TX:
    {
        ...
        {
            gp_trans_thr->increase_current_depth();
            // always update savepoint id
            gp_trans_thr->get_current()->setSvptId(lv_svptid, lv_psvptid);
        }
        else
        {
            // if its not active, add it if need be
            lv_return = gv_tmlib.add_or_update(lv_transid, lv_startid, lv_svptid, lv_psvptid);
         }
        break;
    }
...
short REGISTERREGION(long transid, long startid, int pv_port, char *pa_hostname, int pv_hostname_length, long pv_startcode, char *pa_regionInfo, int pv_regionInfo_length, int pv_peerId, int pv_tmFlags)
{
   short lv_error = FEOK;
   ...
   // instantiate a gp_trans_thr object for this thread if needed.
   TMlibTrace(("TMLIB_TRACE : REGISTERREGION ENTRY: transid: %ld, txid: (%d,%d), startId: %ld, port: %d, hostname %s, length: %d, startcode: %ld, regionInfo: %s, length: %d peerId: %d tmFlags: %d.\n",
	       transid, lv_transid.get_node(), lv_transid.get_seq_num(), startid, pv_port, pa_hostname, pv_hostname_length, pv_startcode, pa_regionInfo, pv_regionInfo_length, pv_peerId, pv_tmFlags), 2);
   ...
   if (lp_currTrans != NULL)
   {
      ...
      lv_error =  lp_currTrans->register_region(lv_savedStartId, pv_port, pa_hostname, pv_hostname_length, pv_startcode, pa_regionInfo, pv_regionInfo_length, pv_peerId, pv_tmFlags);
   }
   // Create a temp TM_transaction object to pass the trans id to REGION SERVER 
      ...
      lv_error =  lp_trans->register_region(lv_startid, pv_port, pa_hostname, pv_hostname_length, pv_startcode, pa_regionInfo, pv_regionInfo_length, pv_peerId, pv_tmFlags);
      delete lp_trans;
   }
...
// PUSHEPOCH
//
// Purpose: send PUSHEPOCH message to the TM
// Params: pa_tabledesc, pv_tabledesc_length, pv_tblname, transid
// -------------------------------------------------------------------
short PUSHEPOCH(char *pa_tbldesc, int pv_tbldesc_length, char *pv_tblname, long transid,
                  char* &pv_err_str, int &pv_err_len)
{
    TM_Transid lv_transid((TM_Native_Type) transid);
    short lv_error = FEOK;

    TMlibTrace(("TMLIB_TRACE : PUSHEPOCH ENTRY: txid: (%d,%d), tablename: %s \n",
       lv_transid.get_node(), lv_transid.get_seq_num(), pv_tblname), 2);

    if (gp_trans_thr == NULL)
       gp_trans_thr = new TMLIB_ThreadTxn_Object();
    TM_Transaction *lp_trans = gp_trans_thr->get_current();
    lv_error =  lp_trans->push_epoch(pa_tbldesc, pv_tbldesc_length,
                pv_tblname, pv_err_str, pv_err_len);

    TMlibTrace(("TMLIB_TRACE : PUSHEPOCH EXIT: txid: (%d,%d), returning %d\n",
       lv_transid.get_node(), lv_transid.get_seq_num(), lv_error), 2);
...
short CREATETABLE(char *pa_tbldesc, int pv_tbldesc_length, char *pv_tblname, char** pv_keys, int pv_numsplits, int pv_keylen, long transid ,
                  int pv_options, char* &pv_err_str, int &pv_err_len)
{
    TM_Transid lv_transid((TM_Native_Type) transid);
    short lv_error = FEOK;
    ...
    lv_error =  lp_trans->create_table(pa_tbldesc, pv_tbldesc_length, 
                pv_tblname, pv_keys, pv_numsplits, pv_keylen, pv_options,
...
short ABORTTRANSACTION() 
{
    ...
    // Propagated tx does not rollback transaction
    if (lp_trans->getIsPropagateTX() == true)
        return FEOK;
...
short ENDTRANSACTION()
{
  ...
  int   errlen = 0;
  char spaceStr[2];
  spaceStr[0]='Q'; spaceStr[1]= '\0';
  short lv_error = ENDTRANSACTION_ERR_QUERY(errStr,errlen, spaceStr);
...
//ENDTRANSACTION
//
//Purpose   : end the current transaction with query id passed in
//Params    : none
//--------------------------------------------------------------------
short ENDTRANSACTION_QUERY(char *querycontext)
{
  char *errStr = NULL;
  int   errlen = 0;
  short lv_error = ENDTRANSACTION_ERR_QUERY(errStr,errlen, querycontext);
  DEALLOCATE_ERR(errStr);
  return lv_error;
}
// --------------------------------------------------------------------
...
// Params    : none
// --------------------------------------------------------------------
short ENDTRANSACTION_ERR_QUERY(char *&errStr, int &errlen, char *querycontext) 
{
    short lv_error = FEOK;
    short lv_retries = 0;
    // instantiate a gp_trans_thr object for this thread if needed.
    ...
    // Propagated tx does not commit transaction
    if (lp_trans->getIsPropagateTX() == true)
        return FEOK;

    if (strlen(querycontext) > (TM_MAX_QUERYCONTEXT_STRING-1)) {
        TMlibTrace(("TMLIB_TRACE : ENDTRANSACTION returning with query id too long error %d\n",
                     FEBOUNDSERR), 1);
        return FEBOUNDSERR;
    }
    ...
    do {
       if (lv_retries > 0)
       {
         if (lv_error == FETMLOCKED)
           TMlibTrace(("TMLIB_TRACE : ENDTRANSACTION error FETMLOCKED, sleeping 1 second and trying again\n"), 1);
         if (lv_error == FETRANSNOWAITOUT) 
         {
            TMlibTrace(("TMLIB_TRACE : ENDTRANSACTION error FETRANSNOWAITOUT, sleeping 1 second and trying again\n"), 1);
            char la_buf[DTM_STRING_BUF_SIZE];
            sprintf(la_buf,"Warning: ENDTRANSACTION error FETRANSNOWAITOUT, already sleeping %d and trying again\n",
            lv_retries);
            tm_log_write(DTM_LIB_INVALID_TRANS, SQ_LOG_WARNING, la_buf);
            if (lv_retries > 10)  //sleep 10s to wait for the incomplete outstanding ios, hopefully it works
              break;
         }
         SB_Thread::Sthr::sleep(1000); // in msec (1 second)
       }
         char buf[TM_MAX_QUERYCONTEXT_STRING];
         memset(buf, '\0', sizeof(buf));
         snprintf(buf,sizeof(buf),"%s", querycontext);
         lv_error =  lp_trans->end(errStr, errlen, buf, strlen(buf));
      }while ((lv_error == FETMLOCKED || lv_error == FETRANSNOWAITOUT) && (lv_retries++ < 60));
...
short SETSAVEPOINTIDWITHFLAG(int64 svptId, int64 pSvptId)
{
    // instantiate a gp_trans_thr object for this thread if needed.
    if (gp_trans_thr == NULL)
      gp_trans_thr = new TMLIB_ThreadTxn_Object();
    ...
    if (lp_trans == NULL)
    {
        TMlibTrace(("TMLIB_TRACE : SETSAVEPOINTIDWITHFLAG returning with error %d\n",
                     FENOTRANSID), 1);
        return FENOTRANSID;
    }
    ...
    TMlibTrace(("TMLIB_TRACE : SETSAVEPOINTIDWITHFLAG ENTRY: txid: %d, current svpt id :%ld\n",
               lp_trans->getTransid()->get_seq_num(), svptId), 1);

    lp_trans->setSvptId(svptId, pSvptId);
...
// COMMITSAVEPOINT
//
// Purpose  : commit(merge) the savepoint to Transaction or parent savepoint
// Params   : svptId  - Savepoint id
//            pSvptId - parent Savepoint id,
//                      will merge mutatiions to parent Savepoint when assigned.
// --------------------------------------------------------------
short COMMITSAVEPOINT(char *&errStr, int &errlen, int64 svptId, int64 pSvptId)
{
    short lv_error = FEOK;
    short lv_retries = 0;
    ...
    if (lp_trans == NULL)
    {
        TMlibTrace(("TMLIB_TRACE : COMMITSAVEPOINT returning with error %d\n",
                     FENOTRANSID), 1);
        return FENOTRANSID;
    }

    // Propagated tx does not commit savepoint
    if (lp_trans->getIsPropagateTX() == true)
        return FEOK;
    ...
    TMlibTrace(("TMLIB_TRACE : COMMITSAVEPOINT ENTRY: txid: %d, svpt id :%ld, parent svpt id :%ld\n",
               lp_trans->getTransid()->get_seq_num(), svptId, pSvptId), 1);
    ...
    do
    {
      ...
      {
        if (lv_error == FETMLOCKED)
          TMlibTrace(("TMLIB_TRACE : COMMITSAVEPOINT error FETMLOCKED, sleeping 1 second and trying again\n"), 1);
        if (lv_error == FETRANSNOWAITOUT) 
        {
          TMlibTrace(("TMLIB_TRACE : COMMITSAVEPOINT error FETRANSNOWAITOUT, sleeping 1 second and trying again\n"), 1);
           char la_buf[DTM_STRING_BUF_SIZE];
           sprintf(la_buf,"Warning: COMMITSAVEPOINT error FETRANSNOWAITOUT, already sleeping %d and trying again\n",
           lv_retries);
           tm_log_write(DTM_LIB_INVALID_TRANS, SQ_LOG_WARNING, la_buf);
           if (lv_retries > 10)  //sleep 10s to wait for the incomplete outstanding ios, hopefully it works
             break;
      ...
      lv_error = lp_trans->commitSvpt(errStr, errlen, svptId, pSvptId);
    }while ((lv_error == FETMLOCKED || lv_error == FETRANSNOWAITOUT) && (lv_retries++ < 10));

    TMlibTrace(("TMLIB_TRACE : COMMITSAVEPOINT EXIT: txid: %d, svpt id :%ld, parent svpt id :%ld, retcode: %d\n", 
               lp_trans->getTransid()->get_seq_num(), svptId, pSvptId, lv_error),
               1);

    return lv_error;
}

short ROLLBACKSAVEPOINT(int64 svptId, int64 pSvptId)
{
    // instantiate a gp_trans_thr object for this thread if needed.
    if (gp_trans_thr == NULL)
       gp_trans_thr = new TMLIB_ThreadTxn_Object();
    ...
    if (lp_trans == NULL)
    {
        TMlibTrace(("TMLIB_TRACE : ROLLBACKSAVEPOINT returning with error %d\n",
                     FENOTRANSID), 1);
        return FENOTRANSID;
    }

     // Propagated tx does not rollback savepoint
    if (lp_trans->getIsPropagateTX() == true)
        return FEOK;
    ...
    TMlibTrace(("TMLIB_TRACE : ROLLBACKSAVEPOINT ENTRY: txid: %d svpt id:%ld\n",
               lp_trans->getTransid()->get_seq_num(), svptId), 1);

    lv_error = lp_trans->abortSvpt(svptId, pSvptId);

    TMlibTrace(("TMLIB_TRACE : ROLLBACKSAVEPOINT EXIT: txid: %d, retcode: %d\n", 
                lp_trans->getTransid()->get_seq_num(), 
                lv_error), 
               1);
...
// GETTRANSID
//
// Purpose  : get TransId from local TM_Transaction, not sending
//            GETTRANSID message to TM
...
// GETSAVEPOINTID
//
// Purpose  : get SavepointId from local TM_Transaction, may be 
//            inner savepoint id
// Params   : pp_transid - pointer to a transid (out)
// --------------------------------------------------------------
short GETSAVEPOINTIDWITHFLAG(int64* pp_svptid, int64* pp_psvptid)
{
    SB_Int64_Type *lp_svptid_to_return = NULL;
    ...
    if (lv_error)
    {
        TMlibTrace(("TMLIB_TRACE : GETSAVEPOINTIDWITHFLAG returning with error %d\n",
                     lv_error), 2);
        return lv_error;
    }

    lv_error = tmlib_check_miss_param (pp_svptid);
    if (lv_error)
    {
        TMlibTrace(("TMLIB_TRACE : GETSAVEPOINTIDWITHFLAG returning with error %d\n",
                     lv_error), 1);
    ...
    lp_svptid_to_return = (TM_Native_Type *)pp_svptid;
    *lp_svptid_to_return = gp_trans_thr->get_current()->getSvptId();
    *pp_psvptid = gp_trans_thr->get_current()->getPSvptId();
...
bool GETISPROPAGATETX()
{
    bool result = false;

    TM_Transaction *lp_trans = gp_trans_thr->get_current();
    if (lp_trans)
        result = lp_trans->getIsPropagateTX();
...

{
    ...
    if ((lv_transid.get_node()!=0) || (lv_transid.get_seq_num()!=0))
    {
        ...
        if(pp_instance_clusterid != NULL) {
            memcpy(pp_instance_clusterid, &lv_rsp.u.iv_gettransinfo.iv_instance_clusterid, 
                (sizeof(lv_rsp.u.iv_gettransinfo.iv_instance_clusterid)));  
        }
...
{
    int32 lv_seq_num, lv_node, lv_instance_clusterid;
...
//-----------------------------------------------------------------
//DTM_IS_SDN_DRAINED
//Purpose - Request DTMs to check for SDN transactions
//Returns FEOK if no SDN transactions found.
//Else returns with FE_SDNTRAN error.
//        
//-----------------------------------------------------------------
short DTM_SDNTRANSACTIONS()
{
 short           lv_error = FEOK;
 TMlibTrace(("TMLIB_TRACE : DTM_SDNTRANSACTIONS ENTRY.\n"), 2);

 lv_error = lock_helper(TM_MSG_TYPE_SDNTRAN);
 
 TMlibTrace(("TMLIB_TRACE : DTM_SDNTRANSACTIONS EXIT with error %d\n", lv_error), 2);
 return lv_error;
} // DTM_SDNTRANSACTIONS
...
TMLIB::TMLIB() : JavaObjectInterfaceTM()
{
    ...
    ms_getenv_int("TRAF_CLUSTER_ID", &lv_cluster_id);    
    int32 lv_instance_id = 0;
    ms_getenv_int("TRAF_INSTANCE_ID", &lv_instance_id);    
    iv_my_cluster_id = (short) (lv_instance_id | lv_cluster_id);
    ...
    //Monarch/Ampool
    enableCleanupMRMInterface(false);
    ms_getenv_bool("TMLIB_ENABLE_MCLEANUP", &enableCleanupMRMInterface_);
...
short TMLIB::add_or_update (TM_Transid pv_transid, TM_Transseq_Type pv_startid,
                            SB_Int64_Type pv_svptid, SB_Int64_Type pv_psvptid,
                            bool pv_can_end, int pv_tag)
{
    ...
    pv_tag = pv_tag; //Intel compiler warning 869
     ...
     if (lp_trans)
     {
         lp_trans->setSvptId(pv_svptid, pv_psvptid);
         gp_trans_thr->set_current(lp_trans);
         if (lv_new_tx){
...
{
   jclass lv_javaClass;
   TMLibJavaMethods_ = new JavaMethodInitTM[JM_LAST];
   ...
   if (ret == JOI_OK) {
	 ...
	 lv_javaClass = _tlp_jenv->FindClass(mrminterface_classname); 
	 if (lv_javaClass != NULL) {
            MRMInterface_class = (jclass)_tlp_jenv->NewGlobalRef(lv_javaClass);
            TMLibJavaMethods_[JM_CLEAR_MTRANSACTIONSTATES].methodID =
                  _tlp_jenv->GetStaticMethodID(MRMInterface_class,
...
short TMLIB::replayEngine(long timestamp)
{
  ...
  _tlp_jenv->CallStaticVoidMethod(RMInterface_class, TMLibJavaMethods_[JM_REPLAYENGINE].methodID, jlv_timestamp);
  if (getExceptionDetails(NULL)) {
     tm_log_write(DTM_TM_JNI_ERROR, SQ_LOG_ERR, (char *)"TMLIB::replayEngine()", (char *)_tlp_error_msg->c_str(), -1);
     _tlp_jenv->PopLocalFrame(NULL);
  }
  return JOI_OK;
}
...
void TMLIB::cleanupMTransactionLocal(long transactionID)
{
  ...
  _tlp_jenv->CallStaticVoidMethod(MRMInterface_class, TMLibJavaMethods_[JM_CLEAR_MTRANSACTIONSTATES].methodID, jlv_transid);
  if (getExceptionDetails(NULL)) {
     tm_log_write(DTM_TM_JNI_ERROR, SQ_LOG_ERR, (char *)"TMLIB::cleanupMTransactionLocal()", (char *)_tlp_error_msg->c_str(), -1);
     _tlp_jenv->PopLocalFrame(NULL);
  }
  _tlp_jenv->PopLocalFrame(NULL);
  return;
