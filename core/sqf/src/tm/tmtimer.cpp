...
CTmTimerEvent * CTmTimerList::pop(CTmTime *pp_time)
{
   ...
   if (lp_event)
   {
      ...
      if (lv_count > 1)
      {
         ...
         CTmTimerEvent * lp_ev = (CTmTimerEvent *) get_first_unprotected();
         ...
         for (int lv_idx=0;lv_idx < lv_count; lv_idx++)
         {
             if (lp_ev == NULL)
             {
               //should never happen
               get_end();
               sprintf(errmsg, "CTmTimerList::pop : PROGRAMMING ERROR! lp_ev is NULL, lv_idx = %d, lv_count = %d.\n",
                               lv_idx, lv_count);
               tm_log_write(DTM_XATM_PROG_ERROR, SQ_LOG_CRIT, errmsg);
               abort();
             }
      ...
      CTmTimerEvent *lp_removedEvent = (CTmTimerEvent *) remove_first_unprotected((pp_time->get()));
      if (lp_event != lp_removedEvent)
      {
          get_end();
          sprintf(errmsg, "CTmTimerList::pop : PROGRAMMING ERROR! first event %p doesn't match removed event %p.\n", (void *) lp_event, (void *) lp_removedEvent);
          tm_log_write(DTM_XATM_PROG_ERROR, SQ_LOG_CRIT, errmsg);
...
void CTmTimer::addRefreshMonEvent(int32 pv_interval)
{
    TMTIME_TRACE (2, XATM_TraceTimerExit, ("CTmTimer::addRefreshMonEvent : ENTRY, interval %d\n",
                     pv_interval));

    //repeat it intervally to refresh monitor
    ip_Refresh_Mon_event = new CTmTimerEvent(TM_MSG_TXINTERNAL_REFRESH_MON, NULL,
                                             pv_interval, -1);
     eventQ_push((CTmEvent *) ip_Refresh_Mon_event); 
}
