...
JNIEXPORT jstring JNICALL Java_org_apache_hadoop_hbase_client_transactional_RMInterface_pushEpochReq
  (JNIEnv *pp_env, jobject pv_object, jbyteArray pv_tableDescriptor, jlong pv_transid, jbyteArray pv_tblname)
{
   ...
   if(lv_tbldesc_length > TM_MAX_TABLE_STRING)
   {
   ...
   lv_error  = PUSHEPOCH((char*) lp_tbldesc, lv_tbldesc_length,
                           (char *)lp_tblname, lv_transid, la_err_str, la_err_len);
   ...
   if(lv_error)
   {
     ...
     {
       char str[200];
       sprintf(str, "Push Epoch failed. Error is Unknown, lv_error = %d", lv_error);
       lv_err_str = pp_env->NewStringUTF(str);
     }
     return lv_err_str;
   }
   else
...
JNIEXPORT jstring JNICALL Java_org_apache_hadoop_hbase_client_transactional_RMInterface_createTableReq
  (JNIEnv *pp_env, jobject pv_object, jbyteArray pv_tableDescriptor, jobjectArray pv_keys, jint pv_numSplits, jint pv_keyLength, jlong pv_transid, jbyteArray pv_tblname, jint pv_options)
{
   ...
   lv_error  = CREATETABLE((char*) lp_tbldesc, lv_tbldesc_length,
                           ...
                           lv_keyLength, lv_transid, lv_options, la_err_str, la_err_len);
   ...
   if(lv_error)
   {
     ...
     {
       char str[200];
       sprintf(str, "Create Table failed. Error is Unknown, lv_error = %d", lv_error);
       lv_err_str = pp_env->NewStringUTF(str);
     }
     return lv_err_str;
   }
...
JNIEXPORT jstring JNICALL Java_org_apache_hadoop_hbase_client_transactional_RMInterface_dropTableReq
  (JNIEnv *pp_env, jobject pv_object, jbyteArray pv_tblname, jlong pv_transid) {
   ...
   if(lv_error)
   {
     ...
     {
       char str[200];
       sprintf(str, "Drop Table failed. Error is Unknown, lv_error = %d", lv_error);
       lv_err_str = pp_env->NewStringUTF(str);
     }
     return lv_err_str;	   
   }
...
JNIEXPORT jstring JNICALL Java_org_apache_hadoop_hbase_client_transactional_RMInterface_truncateOnAbortReq
  (JNIEnv *pp_env, jobject pv_object, jbyteArray pv_tblname, jlong pv_transid) {
   ...
   if(lv_error)
   {
     ...
     {
       char str[200];
       sprintf(str, "RegTruncateonAbort failed. Error is Unknown, lv_error = %d", lv_error);
       lv_err_str = pp_env->NewStringUTF(str);
     }
     return lv_err_str;    
   }
...
JNIEXPORT jstring JNICALL Java_org_apache_hadoop_hbase_client_transactional_RMInterface_alterTableReq
  (JNIEnv *pp_env, jobject pv_object, jbyteArray pv_tblName, jobjectArray pv_tableOptions, jlong pv_transID) {
   ...
   if(lv_error)
   {
     ...
     {
       char str[200];
       sprintf(str, "Alter Table failed. Error is Unknown, lv_error = %d", lv_error);
       lv_err_str = pp_env->NewStringUTF(str);
     }
     return lv_err_str;    
   }
