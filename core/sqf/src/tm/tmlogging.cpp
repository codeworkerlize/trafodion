...
static bool gv_log4cplus_initialized = false;

int tm_init_logging()
{
    // Log4cxx logging
    CommonLogger::instance().initLog4cplus("log4cplus.trafodion.tm.config");
    gv_log4cplus_initialized = true;
...
{ 
    ...
    if (exception_stack == NULL) 
         CommonLogger::log(TM_COMPONENT, ll_severity, "%s NID %d PID %d TID %Ld txID %Ld Message ID %d Message %s ",
               my_processName, my_nid, my_pid, GETTID(), transid, pv_event_type, err_string);
    else
        CommonLogger::log(TM_COMPONENT, ll_severity, "%s NID %d PID %d TID %Ld txID %Ld Message ID %d Message Error at %s caused by exception %s ", 
              my_processName, my_nid, my_pid, GETTID(), transid, pv_event_type, err_string, exception_stack);
    return 0;
}
...

{
    ...
    if (gv_dual_logging || (! gv_log4cplus_initialized))
    {   
    ...
    CommonLogger::log(TM_COMPONENT, ll_severity, "%s NID %d PID %d TID %d %d %d Message ID %d, Message %s %s %s", 
              my_processName, my_nid, my_pid, rmid, dtmid, seq_num, event_id, temp_string, (string1 == NULL ? "" : string1), la_buf);
    return rc;
}
