...
bool           gv_time_refresh = true;
char           ga_name[BUFSIZ];
char          *gp_shm;
unsigned long *gp_shml;
bool           gv_shook   = false;
bool           gv_verbose = false;
long           gv_UTC_offset = -2L;
...
void reset_time_counter() {
    ...
    if (gp_local_mon_io)
    {
      gp_local_mon_io->refreshIdtm(lv_existing_tsl);
    }
...
int timespec_to_str(char *buf, size_t max_len, struct timespec *ppv_timespec) {
    ...
    //Use gmtime to get time in UTC
    struct tm *gtm = gmtime(&ppv_timespec->tv_sec);
    // Now put it all together
    sprintf(tmp_buffer, "%d-%02d-%02d %02d:%02d:%02d.%ld" ,
        (1900 + gtm->tm_year), (1 + gtm->tm_mon), gtm->tm_mday, gtm->tm_hour, gtm->tm_min, gtm->tm_sec, ppv_timespec->tv_nsec);

    if (gv_verbose)
       printf("srv: gmtime returning %s \n", tmp_buffer);
...
int str_to_tm_id(char *buf, unsigned long *ppv_tm_id) {
    ...
    *ppv_tm_id = 0L;
    ...
    lv_ts.tv_sec = timegm(&lv_tm);
    if (lv_ts.tv_sec == -1){

      // Unsuccessful conversion
      return XZFIL_ERR_FSERR;
    }
    lv_ts.tv_nsec = atol(usec_buff);

    *ppv_tm_id = ((unsigned long) lv_ts.tv_sec << 20) |
                 ((unsigned long) lv_ts.tv_nsec);
...
void do_init(int pv_argc, char **ppp_argv) {
    ...
    for (lv_arg = 1; lv_arg < pv_argc; lv_arg++) {
        ...
        if (strcmp(lp_arg, "-attach") == 0)
            ...
            gv_shook = true;
        else if (strcmp(lp_arg, "-norefresh") == 0)
            gv_time_refresh = false;
    ...
    clock_gettime(CLOCK_REALTIME, &lv_new_ts);
    gv_UTC_offset = localtime(&lv_new_ts.tv_sec)->tm_gmtoff;
