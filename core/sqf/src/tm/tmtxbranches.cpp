                                      ...
                                      int64 pv_flags, CTmTxMessage * pp_msg)
{
   ...
   int64 time1 = 0L, time2 = 0L, time3 = 0L;
   ...
   {
     gettimeofday(&curtime, NULL);
     time3 = (curtime.tv_sec*1000 + curtime.tv_usec/1000);
     if ((time2 - time1) > costTh || (time3 - time2) > costTh)
     {
       sprintf(la_buf, "commit_branches commit transaction hbase TC %ld TSE TC %ld", (time2 - time1), (time3 - time2));
       tm_log_write(DTM_XATM_COMPLETEONE_FAILED, SQ_LOG_WARNING, la_buf, NULL, pp_txn->legacyTransid());
     }
   }

   if (lv_err1 != RET_OK){
      lv_err1 = (lv_err1 == RET_WALDOWN) ? XAER_WALDOWN : XAER_RMERR;
   }

   lv_err = (lv_err1 != RET_OK) ? lv_err1 :((lv_err2 != XA_OK) ? lv_err2 : XA_OK);
...
//------------------------------------------------------------------------------
// epochOperation
// Purpose - Handles epochOperation requests
// -----------------------------------------------------------------------------
int32 CTmTxBranches::epochOperation(CTmTxBase *pp_txn, CTmTxMessage * pp_msg)
{
   int32 lv_err = FEOK;

   TMTrace (2, ("CTmTxBranches::epochOperation, Txn ID (%d,%d), ENTRY \n",
                pp_txn->node(), pp_txn->seqnum()));

   lv_err = iv_HBASEBranches.hb_epoch_operation(pp_txn, pp_msg);

   TMTrace (2, ("CTmTxBranches::epochOperation, Txn ID (%d,%d), EXIT, UnResolved branches %d.\n",
                pp_txn->node(), pp_txn->seqnum(), num_rms_unresolved(pp_txn)));
...
int32 CTmTxBranches::commit_savepoint(CTmTxBase *pp_txn, CTmTxMessage * pp_msg)
{
   int32 lv_err = FEOK, lv_err1 = RET_OK;

   TMTrace (2, ("CTmTxBranches::commit_savepoint, Txn ID (%d,%d), ENTRY\n",
                pp_txn->node(), pp_txn->seqnum()));

   lv_err = iv_HBASEBranches.commit_savepoint(pp_txn, pp_msg);
   //iv_TSEBranches.rollback_branches(pp_txn, pp_msg);
   ...
   TMTrace (2, ("CTmTxBranches::commit_savepoint, Txn ID (%d,%d), EXIT, "
            "UnResolved branches %d, return error %d, HBase client error %d.\n",
            pp_txn->node(), pp_txn->seqnum(), num_rms_unresolved(pp_txn), lv_err, lv_err1));
   return lv_err;
}

int32 CTmTxBranches::rollback_savepoint(CTmTxBase *pp_txn, CTmTxMessage * pp_msg)
{
   int32 lv_err = FEOK;

   TMTrace (2, ("CTmTxBranches::rollback_savepoint, Txn ID (%d,%d), ENTRY\n",
                pp_txn->node(), pp_txn->seqnum()));

   lv_err = iv_HBASEBranches.rollback_savepoint(pp_txn, pp_msg);
   //iv_TSEBranches.rollback_branches(pp_txn, pv_flags, pp_msg, pv_error_condition);
   ...
   TMTrace (2, ("CTmTxBranches::rollback_savepoint, Txn ID (%d,%d), EXIT, "
            "UnResolved branches %d, return error %d.\n",
