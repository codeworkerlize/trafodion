...
typedef struct _tminfo_h_as_6
{
   typedef union 
   {
      typedef struct _tminfo_h_as_6
      {
         char iv_iid; // instance id
         char iv_cid; // cluster id
      }id_bytes;
      int16 iv_inst_clust;
   } inst_cluster;
} Tm_Cluster_Instance_Id;
...
{
    private:
       int16            iv_instance_cluster_id;
       int32            iv_seqnum_mode;
       int32            iv_run_mode;
       ...
       int32            iv_stall_phase_2; // 1  = ABORT, 2 = COMMIT, 3 = ABORT or COMMIT, 4 = PREPARED, before TLOG write
                                          // 5  = xDC remote commit written, but local not written
                                          // 6  = STALL_REMOTE_COMMIT; stall after the first remote commit is sent
                                          // 7  = STALL_LOCAL_COMMIT; stall after the first local commit is sent
                                          // 8  = STALL_BOTH_COMMIT; stall after the first remote
                                          //      and first local commits are sent
                                          // 9  = Trigger FailedServerException in local TLOG.putSingleRecord commit write
                                          // 10 = Trigger FailedServerException in local TLOG.doTlogWrite commit write
                                          // 11 = Trigger FailedServerException in remote TLOG.putSingleRecord commit  write
                                          // 12 = Trigger FailedServerException in remote TLOG.doTlogWrite commit write
                                          //
                                          // 13 = Trigger FailedServerException in local TLOG.putSingleRecord forgotten commit write
                                          // 14 = Trigger FailedServerException in remote TLOG.putSingleRecord forgotten commit write
                                          // 15 = Trigger FailedServerException in remote TLOG.doTlogWrite forgotten commit write
                                          //
                                          // 16 = Trigger FailedServerException in local TLOG.putSingleRecord abort write
                                          // 17 = Trigger FailedServerException in remote TLOG.putSingleRecord abort write
                                          // 18 = Trigger FailedServerException in remote TLOG.doTlogWrite abort write
                                          //
                                          // 19 = Trigger FailedServerException in local TLOG.putSingleRecord forgotten abort write
                                          // 20 = Trigger FailedServerException in remote TLOG.putSingleRecord forgotten abort write
                                          // 21 = Trigger FailedServerException in remote TLOG.doTlogWrite forgotten abort write
                                          //
                                          //
      ...
      // TM_Mutex         iv_msg_mon_mutex;  // Semaphore to serialize access to msg_mon_ procs
       ...
       int32            iv_tm_refresh_mon_interval;
       // DTM stats - end
    ...
    public:
       ...
       int32 accessSeqNumBlockFile(unsigned int *seqNum, bool accessMode, int *errNo);
       ...
       void  refresh_mon_interval(int32 pv_interval);
       int32 refresh_mon_interval();
       ...
       unsigned char instanceid_only() {return iv_instance_cluster_id & 0xFF;}
       unsigned char clusterid_only() {return (iv_instance_cluster_id >> 8);}
       int16 instance_clusterid() {return iv_instance_cluster_id;}
       ...
       void sdnTrans(CTmTxMessage * pp_msg);
...
inline void TM_Info::refresh_mon_interval(int32 pv_interval)
{
    lock();
    iv_tm_refresh_mon_interval = pv_interval; 
    unlock();
}

inline int32 TM_Info::refresh_mon_interval()
{
    return iv_tm_refresh_mon_interval;
}
