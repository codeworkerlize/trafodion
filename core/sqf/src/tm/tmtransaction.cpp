...
// --------------------------------------------------------------------------
TM_Transaction::TM_Transaction(TM_Transid pv_transid, bool pv_propagate_tx)
{
    TMlibTrace(("TMLIB_TRACE : TM_Transaction::Constructor2 ENTRY. transid "
        "(%d,%d), server flag %d.\n", pv_transid.get_node(), pv_transid.get_seq_num(), pv_propagate_tx), 4);
    iv_transid = pv_transid;
    iv_coordinator = false;
    iv_propagate = pv_propagate_tx;
    iv_last_error = 0;
    iv_abort_timeout = 0;
...
// --------------------------------------------------------------------------
TM_Transaction::TM_Transaction() 
   :iv_propagate(false), iv_transid(0)
{
...
// TOPL
short  TM_Transaction::register_region(long startid, int port, char *hostName, int hostname_Length, long startcode, char *regionInfo, int regionInfo_Length, int pv_peerId, int pv_tmFlags)
{
    ...
    lv_req.u.iv_register_region.iv_tmFlags = pv_tmFlags;
...
short TM_Transaction::push_epoch(char* pa_tbldesc, int pv_tbldesc_len,
                                   char* pa_tblname,
                                   char* &pv_err_str, int &pv_err_len)
{
    ...
    char *buffer = new char[len_aligned];

    TMlibTrace(("TMLIB_TRACE : TM_Transaction::push_epoch ENTRY tablename: %s\n", pa_tblname), 1);

    if (!gv_tmlib_initialized)
         gv_tmlib.initialize();

    tmlib_init_req_hdr(TM_MSG_TYPE_PUSHEPOCHREQUEST, &lv_req);
    iv_transid.set_external_data_type(&lv_req.u.iv_push_epoch.iv_transid);
    memcpy(lv_req.u.iv_push_epoch.epochreq, pa_tbldesc, pv_tbldesc_len);
    lv_req.u.iv_push_epoch.epochreq_len = pv_tbldesc_len;
    memcpy(buffer, (char*)&lv_req, len);

    iv_last_error = gv_tmlib.send_tm_link(buffer, len_aligned, &lv_rsp, iv_transid.get_node());
    delete buffer;
    if(iv_last_error)
    {
        TMlibTrace(("TMLIB_TRACE : TM_Transaction::push_epoch returning error %d\n", iv_last_error), 1);
        return iv_last_error;
    }
    ...
    if(iv_last_error)
    {
        int maxErrStrBufLen = sizeof(lv_rsp.u.iv_push_epoch_response.iv_err_str);
        pv_err_len = lv_rsp.u.iv_push_epoch_response.iv_err_str_len < maxErrStrBufLen ? lv_rsp.u.iv_push_epoch_response.iv_err_str_len : maxErrStrBufLen;
        pv_err_str = new char[pv_err_len];
        memcpy(pv_err_str, lv_rsp.u.iv_push_epoch_response.iv_err_str, pv_err_len);
    }
                                   ...
                                   char* &pv_err_str, int &pv_err_len)
{
    ...
    lv_req.u.iv_ddl_request.options = pv_options;
...
// --------------------------------------------------------------------------
short TM_Transaction::end(char* &pv_err_str, int &pv_err_len, char* pv_querycontext_buf, int pv_querycontext_len)
{
    Tm_Req_Msg_Type lv_req;
    Tm_Rsp_Msg_Type lv_rsp;
    ...
    if (gv_tmlib.localBegin()) {
       ...
       //do truncate
       if(pv_querycontext_len > TM_MAX_QUERYCONTEXT_STRING)
         pv_querycontext_len = TM_MAX_QUERYCONTEXT_STRING; 
       lv_req.u.iv_end_trans.iv_querycontext_len = pv_querycontext_len;     
       memcpy(lv_req.u.iv_end_trans.ia_querycontext, pv_querycontext_buf, pv_querycontext_len);        
...
short TM_Transaction::commitSvpt(char* &pv_err_str, int &pv_err_len, int64 pv_svpt_id, int64 pv_psvpt_id)
{
    Tm_Req_Msg_Type lv_req;
    Tm_Rsp_Msg_Type lv_rsp;

    TMlibTrace(("TMLIB_TRACE : TM_Transaction::commitSvpt (seq num %d, svpt id %ld, psvpt id %ld) ENTRY\n",
                iv_transid.get_seq_num(), pv_svpt_id, pv_psvpt_id),
               2);

    iv_last_error = tmlib_check_active_tx();
    if (iv_last_error)
    {
        TMlibTrace(("TMLIB_TRACE : TM_Transaction::commitSvpt returning error %d \n", iv_last_error), 1);
        return iv_last_error;
    }
    ...
    if (gv_tmlib.localBegin()) {
       TMlibTrace(("TMLIB_TRACE : TM_Transaction::commitSvpt local txn trying to commit savepoint, err %d.\n", FENOTRANSID), 1);
       return FENOTRANSID;
    }
    else {
       tmlib_init_req_hdr(TM_MSG_TYPE_COMMIT_SAVEPOINT, &lv_req);
       lv_req.u.iv_commit_svpt.iv_tag = iv_transid.get_seq_num();
       lv_req.u.iv_commit_svpt.iv_pid = gv_tmlib.iv_my_pid;
       lv_req.u.iv_commit_svpt.iv_nid = gv_tmlib.iv_my_nid;
       lv_req.u.iv_commit_svpt.iv_svpt_id = pv_svpt_id;
       lv_req.u.iv_commit_svpt.iv_psvpt_id = pv_psvpt_id;
       iv_transid.set_external_data_type(&lv_req.u.iv_commit_svpt.iv_transid);
       ...
       if(iv_last_error)
       {
           int maxErrStrBufLen = sizeof(lv_rsp.u.iv_commit_svpt.iv_err_str);
           pv_err_len = lv_rsp.u.iv_commit_svpt.iv_err_str_len < maxErrStrBufLen ?
                                     lv_rsp.u.iv_commit_svpt.iv_err_str_len : maxErrStrBufLen;
           pv_err_str = new char[pv_err_len];
           memcpy(pv_err_str, lv_rsp.u.iv_commit_svpt.iv_err_str, pv_err_len); 
       }
    }
    TMlibTrace(("TMLIB_TRACE : TM_Transaction::commitSvpt  (seq num %d) EXIT\n", iv_transid.get_seq_num()), 2);

    return iv_last_error;
}

short TM_Transaction::abortSvpt(int64 pv_svpt_id, int64 pv_psvpt_id)
{
    Tm_Req_Msg_Type lv_req;
    Tm_Rsp_Msg_Type lv_rsp;

    TMlibTrace(("TMLIB_TRACE : TM_Transaction::abortSvpt (seq num %d, svpt id %ld) ENTRY\n",
                iv_transid.get_seq_num(), pv_svpt_id), 2);

    iv_last_error = tmlib_check_active_tx();
    if (iv_last_error)
    {
        TMlibTrace(("TMLIB_TRACE : TM_Transaction::abortSvpt returning error %d \n", iv_last_error), 1);
        return iv_last_error;
    }
    ...
    if (gv_tmlib.localBegin())
    {
       TMlibTrace(("TMLIB_TRACE : TM_Transaction::abortSvpt local txn trying to rollback savepoint, err %d.\n", FENOTRANSID), 1);
       return FENOTRANSID;
    }
    else
    {
       tmlib_init_req_hdr(TM_MSG_TYPE_ROLLBACK_SAVEPOINT, &lv_req);

       lv_req.u.iv_abort_svpt.iv_tag =  iv_transid.get_seq_num();
       lv_req.u.iv_abort_svpt.iv_nid = gv_tmlib.iv_my_nid;
       lv_req.u.iv_abort_svpt.iv_pid = gv_tmlib.iv_my_pid;
       lv_req.u.iv_abort_svpt.iv_svpt_id = pv_svpt_id;
       lv_req.u.iv_abort_svpt.iv_psvpt_id = pv_psvpt_id;
       iv_transid.set_external_data_type (&lv_req.u.iv_abort_trans.iv_transid);
    ...
    TMlibTrace(("TMLIB_TRACE : TM_Transaction::abortSvpt (seq num %d, svpt id %ld) EXIT\n",
                iv_transid.get_seq_num(), pv_svpt_id), 2);
