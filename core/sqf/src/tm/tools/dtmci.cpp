    ...
    if (argc > 1)
    {
       ...
       if(lv_singleCommand) {
          ...
          for (int i=lv_startArg; i<=lv_commands; i++)
          {
             ...
             if( cmdlen >= INPUT_LENGTH  )
             {
               printf("input command len too long\n");
...
//            }
    ...
    /*
     * for mantis-17797 & mantis-17138 & 18516
     * reduce sleep time for speed-up release not exists transaction's lock
     * 1ms delay reduce the stress on monitor, else monitor should report the error MPI_ERR_ASSERT
     */
    usleep(1000);
