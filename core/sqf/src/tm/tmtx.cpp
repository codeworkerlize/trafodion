         ...
         case XA_OK:
         {
          ...
          case XA_HEURMIX:
          {
           ...
           case XAER_WALDOWN:
           {
              iv_tx_state = TM_TX_STATE_FORGETTING;
              lv_error = branches()->completeRequest_branches(this);
              pp_msg->responseError(XAER_WALDOWN);
              return false;
              break;
           }
           default:  // some unexpected error
...
bool TM_TX_Info::state_change_prepare_helper(CTmTxMessage * pp_msg)
{
   ...
   if (iv_mark_for_rollback == true)
   {
      ...
      case COMMIT_DOOMED:
         ...
         tm_log_event(TM_HBASE_COMMIT_DOOMED, SQ_LOG_INFO, "TM_HBASE_COMMIT_DOOMED", 
                      lv_error,-1,node(),seqnum(),-1,lv_replyErr);
         break;         
      case COMMIT_SHIELDED:
         lv_replyErr = FEABORTEDTRANSID;
         tm_log_event(TM_HBASE_COMMIT_SHIELDED, SQ_LOG_INFO, "TM_HBASE_COMMIT_SHIELDED",
                      lv_error,-1,node(),seqnum(),-1,lv_replyErr);
         break;
      default: 
         lv_replyErr = FEABORTEDTRANSID;
   ...
   case COMMIT_CONFLICT:
      ...
      state_change_abort_set(pp_msg, COMMIT_DOOMED);
      state_change_abort_set(pp_msg, COMMIT_SHIELDED);
                               ...
                               CTmTxMessage * pp_msg)
{
    ...
    while (lv_continue)
    {
        ...
        case TX_COMMIT:
            {
               ...
               while (lv_continue2)
               {
                    ...
                    case TM_TX_STATE_COMMITTING:
                    {
                     ...
                     {
                          TM_TEST_PAUSE(iv_tx_state);
                          bool lv_continue3 = false;
                          lv_continue3 = state_change_commit_helper(pp_msg, iv_read_only);
                          if (! lv_continue3){
                            TMTrace(3, ("TM_TX_INFO::state_change, TM_TX_STATE_COMMITTED lv_continue is false message error: %d\n", pp_msg->responseError()));
       ...
       // at recovery time
        ...
        case TX_COMMIT_SAVEPOINT:
            {
                if (iv_tx_state == TM_TX_STATE_ACTIVE)
                {
                    branches()->commit_savepoint(this, pp_msg);
                    lv_continue = false;
                }
            }
            break;
        case TX_ROLLBACK_SAVEPOINT:
            {
                if (iv_tx_state == TM_TX_STATE_ACTIVE)
                {
                    branches()->rollback_savepoint(this, pp_msg);
                    lv_continue = false;
                }
            }
            break;
...
// --------------------------------------------------------------
// req_pushepochoperation
// Purpose - Txn Thread specific processing for [ush epoch operations
// --------------------------------------------------------------
bool TM_TX_Info::req_pushepochoperation(CTmTxMessage *pp_msg)
{
   TMTrace (2, ("TM_TX_Info::req_pushepochoperation : ID (%d,%d) ENTRY.\n",
      node(), seqnum()));

   short lv_error = branches()->epochOperation(this, pp_msg);
   ...
   TMTrace (2, ("TM_TX_Info::req_pushepochoperation : EXIT error %d, Txn ID (%d,%d).\n",
      lv_error, node(), seqnum()));
...
/*	 if (! lp_msg->replyPending()) {
      ...
      case TM_MSG_TYPE_DDLREQUEST:
         ...
         break;
      case TM_MSG_TYPE_PUSHEPOCHREQUEST:
         lv_exit = req_ddloperation(lp_msg);
         break;
      ...
      case TM_MSG_TYPE_COMMIT_SAVEPOINT:
         lv_exit = req_commit_svpt(lp_msg);
         break;
      case TM_MSG_TYPE_ROLLBACK_SAVEPOINT:
         lv_exit = req_abort_svpt(lp_msg);
         break;
      default:
...
// --------------------------------------------------------------
// remoteTimerEvent
// Purpose - reset the pointer to the timer event which is going to be freed.
// --------------------------------------------------------------
void TM_TX_Info::removeTimerEvent(CTmTimerEvent *pp_event)
{
   if (ip_hung_event == pp_event)
   {
      ip_hung_event = NULL;
   }

   // we can also reset other timer event when neccessary
...
bool TM_TX_Info::req_end_complete(CTmTxMessage * pp_msg)
{
   ...
   else if (tx_state() != TM_TX_STATE_FORGOTTEN)
      state_change(TX_COMMIT, gv_tm_info.nid(), gv_tm_info.pid(), pp_msg);
   ...
   if (pp_msg->replyPending()){
      if(pp_msg->responseError() == XAER_WALDOWN){
        TMTrace (2, ("TM_TX_Info::req_end_complete : ID %d, REPLYING FEABORTEDTRANSID due to XAER_WALDOWN \n", seqnum()));
//        pp_msg->reply(FEABORTEDTRANSID);
        pp_msg->reply(FEOK);
      }
      else{
...
bool TM_TX_Info::req_commit_svpt(CTmTxMessage *pp_msg)
{
   short lv_error = FEOK;
   bool lv_terminate = false;

   TMTrace (2, ("TM_TX_Info::req_commit_svpt : ENTRY\n"));

   if (iv_ender_pid != pp_msg->request()->u.iv_commit_svpt.iv_pid)
   {
      TMTrace (1, ("TM_TX_Info::req_commit_svpt : ID (%d%d), pid (%d,%d) not ender (%d,%d).\n",
               node(), seqnum(), pp_msg->request()->u.iv_end_trans.iv_nid, 
               pp_msg->request()->u.iv_end_trans.iv_pid, ender_nid(), ender_pid()));
   ...
   {
      lv_error = FENOTRANSID;
      TMTrace (1,  ("TM_TX_Info::req_commit_svpt, ID %d, wrong txState %d, returning error %d.\n",
                    seqnum(), tx_state(), lv_error));
      pp_msg->reply(lv_error);
      return lv_terminate;
   }
   ...
   if (num_active_partic() > 1)
   {
      TMTrace (1, ("TM_TX_Info::req_commit_svpt :, ID (%d,%d), has %d active joiners.\n",
                       node(), seqnum(), num_active_partic()));
      pp_msg->reply(FEJOINSOUTSTANDING);
      reset_transactionBusy();
      return false;
   ...
   // change state
   state_change (TX_COMMIT_SAVEPOINT, gv_tm_info.nid(), gv_tm_info.pid(), pp_msg);

   TMTrace (2, ("TM_TX_Info::req_commit_svpt : EXIT\n"));
...
bool TM_TX_Info::req_abort_svpt(CTmTxMessage *pp_msg)
{
   short lv_error = FEOK;
   bool lv_terminate = false;

   TMTrace (2, ("TM_TX_Info::req_abort_svpt : ENTRY\n"));

   if (iv_ender_pid != pp_msg->request()->u.iv_commit_svpt.iv_pid)
   {
      TMTrace (1, ("TM_TX_Info::req_abort_svpt : ID (%d%d), pid (%d,%d) not ender (%d,%d).\n",
               node(), seqnum(), pp_msg->request()->u.iv_end_trans.iv_nid, 
               pp_msg->request()->u.iv_end_trans.iv_pid, ender_nid(), ender_pid()));
   ...
   {
      lv_error = FENOTRANSID;
      TMTrace (1,  ("TM_TX_Info::req_abort_svpt, ID %d, wrong txState %d, returning error %d.\n",
                    seqnum(), tx_state(), lv_error));
      pp_msg->reply(lv_error);
      return lv_terminate;
   }
   ...
   if (num_active_partic() > 1)
   {
      TMTrace (1, ("TM_TX_Info::req_abort_svpt :, ID (%d,%d), has %d active joiners.\n",
                       node(), seqnum(), num_active_partic()));
      pp_msg->reply(FEJOINSOUTSTANDING);
      reset_transactionBusy();
      return false;
   ...
   // change state
   state_change (TX_ROLLBACK_SAVEPOINT, gv_tm_info.nid(), gv_tm_info.pid(), pp_msg);

   TMTrace (2, ("TM_TX_Info::req_abort_svpt : EXIT\n"));
