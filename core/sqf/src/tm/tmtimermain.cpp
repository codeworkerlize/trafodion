...
void tmTimer_refreshMon()
{
  TMTrace(2, ("tmTimer_refreshMon : ENTRY.\n"));
  if (gp_local_mon_io)
  {
    struct timespec lv_ts;
    clock_gettime(CLOCK_REALTIME, &lv_ts);
    unsigned long ts = ((unsigned long) lv_ts.tv_sec << 20) | ((unsigned long) lv_ts.tv_nsec / 1000);
    gp_local_mon_io->refreshDTM(ts);
    TMTrace(1, ("tmTimer_refreshMon : gp_local_mon_io->refreshDTM() called, ts = %lu.\n", ts));
  }
  TMTrace(2, ("tmTimer_refreshMon : EXIT.\n"));
}
      ...
      if (lp_event)
      {
         ...
         case TmTimerCmd_Timer:
         {
            ...
            case TM_MSG_TXINTERNAL_SYSTEM_RECOVERY:
               ...
               lv_deleteEvent = true;
               break;
            case TM_MSG_TXINTERNAL_REFRESH_MON:
               tmTimer_refreshMon(); 
               break;
            default:
            {
                if (lp_event->transaction())
                {
                   ...
                   lp_event->transaction()->removeTimerEvent(lp_event);
