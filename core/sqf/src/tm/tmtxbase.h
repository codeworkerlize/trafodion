...
class CTmTxBase :public virtual CTmPoolElement
{
    ...
    public:
       ...
       virtual void removeTimerEvent(CTmTimerEvent *pp_event);
       ...
       // savepoints
       virtual bool req_commit_svpt(CTmTxMessage *pp_msg);
       virtual bool req_abort_svpt(CTmTxMessage *pp_msg);
       ...
       bool checkAndSetRequest()
       {
          bool result = false;
          if (iv_request_received)
             return result;
          lock();
          if (false == iv_request_received)
          {
             iv_request_received = true;
             result = true;
          }
          ip_mutex->unlock();
          return result;
       ...
       unsigned char clusterid_only() {return iv_transid.iv_instance_cluster_id >> 8;}
       unsigned char instanceid_only() {return iv_transid.iv_instance_cluster_id & 0xFF;}
       int32 instance_clusterid() {return iv_transid.iv_instance_cluster_id;}
       ...
       void setThread(CTxThread * pp_thread)
       {
          lock();
          ip_Thread = pp_thread;
       ...
       bool isThreadPendingOrAssociate()
       {
          bool noThreadAssociate;
          lock();
          noThreadAssociate = ((!iv_threadPending) && (!ip_Thread));
          unlock();
          return (!noThreadAssociate);
       }
       bool threadPending() {return iv_threadPending;}
       void threadPending(bool pv_set) {iv_threadPending = pv_set;}

       bool intxdisassociatedQ() {return iv_in_txdisassociatedQ;}
       void intxdisassociatedQ(bool iv_set) {iv_in_txdisassociatedQ = iv_set;}
