...
bool SB_Thread::Sthr::isExited(Sthr::Id_Ptr pp_tid) {
    pthread_t *lp_pthread_t;
    int ret = 0;

    lp_pthread_t = static_cast<pthread_t *>(pp_tid);
    ret = pthread_tryjoin_np(*lp_pthread_t, NULL);

    return (0 != ret && EBUSY == ret ? false : true);
}
...
SB_Thread::Mutex::~Mutex() {
    ...
    lv_status = destroy();
    // M-5274 - Disabling this assert:   SB_util_assert_ieq(lv_status, 0); // sw fault
    lv_status = lv_status; // touch (in case assert disabled)
}
#else
SB_Thread::Mutex::~Mutex() {
