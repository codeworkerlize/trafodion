...
#define BIND_TRY_COUNT 10
    int try_count;
    lv_done = false;
    for (try_count=0; try_count<BIND_TRY_COUNT; ++try_count) {
        for (lv_inx = 0;; lv_inx++) {
            ...
            if (lv_err == 0) {
                ...
                sleep(1);       // in the hope that there would be port available later.
            }
        }

        if (lv_done) {
            // break twice if lv_done is true
            break;
        }
    }
