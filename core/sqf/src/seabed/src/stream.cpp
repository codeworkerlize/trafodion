...
void SB_Trans::Trans_Stream::finish_abandon(MS_Md_Type *pp_md) {
    ...
    pp_md->iv_cv.signal(false); // already locked
    pp_md->iv_cv.unlock();
}
        ...
        for (int lv_inx = 0; lv_inx < lv_max; lv_inx++) {
            ...
            if (lp_md != NULL) {
                ...
                if (lp_md->iv_inline) {
                    ...
                    lp_md->iv_cv.signal(false); // already locked
                    lp_md->iv_cv.unlock();
                } else {
...
void SB_Trans::Trans_Stream::is_stopped_inc() {
    const char *WHERE = "Trans_Stream::is_stopped_inc";
    int         lv_ref_count;

    lv_ref_count = iv_stopped.add_and_fetch(1);
    if (gv_ms_trace_ref)
        trace_where_printf(WHERE, "ENTER stream=%p %s, md-ref=%d\n",
                           pfp(this), ia_stream_name, lv_ref_count);
}

int  SB_Trans::Trans_Stream::is_stopped_get() {
    const char *WHERE = "Trans_Stream::is_stopped_get";
    int         lv_ref_count;

    lv_ref_count = iv_stopped.add_and_fetch(0);
    if (gv_ms_trace_ref)
        trace_where_printf(WHERE, "ENTER stream=%p %s, md-ref=%d\n",
                           pfp(this), ia_stream_name, lv_ref_count);
    return lv_ref_count;
...
void SB_Trans::Trans_Stream::is_stopped_set(int pv_val) {
    const char *WHERE = "Trans_Stream::is_stopped_set";

    iv_stopped.set_val(pv_val);
    if (gv_ms_trace_ref)
        trace_where_printf(WHERE, "ENTER stream=%p %s, md-ref=%d\n",
                           pfp(this), ia_stream_name, pv_val);
}
...
bool SB_Trans::Trans_Stream::remove_comp_q_static(MS_Md_Type *pp_md) {
    ...
    if (pp_md->out.iv_opts & MS_OPTS_FSDONE)
        ...
        lp_q = NULL;
    if (lp_q != NULL)
        lv_ret = lp_q->remove_list_lock(&pp_md->iv_link, true);
    else
        lv_ret = false;
    return lv_ret;
}
...
void SB_Trans::Trans_Stream::wait_rep_done(MS_Md_Type *pp_md) {
    ...
    if (gv_ms_trace)
        trace_where_printf(WHERE, "ENTER stream=%p %s, msgid=%d, md=%p\n",
                           ...
                           pp_md->iv_link.iv_id.i, pfp(pp_md));
    pp_md->iv_cv.lock();
    while (!pp_md->iv_reply_done) {
        lv_status = pp_md->iv_cv.wait(false); // already locked
        SB_util_assert_ieq(lv_status, 0);
    }
    pp_md->iv_cv.unlock();
...
int SB_Trans::Trans_Stream::wait_req_done(MS_Md_Type *pp_md, int timeout) {
    return wait_req_done_static(pp_md, timeout);
}
...
int SB_Trans::Trans_Stream::wait_req_done_static(MS_Md_Type *pp_md, int timeout) {
    int lv_status;

    if (!pp_md->iv_reply_done) {
        lv_status = pp_md->iv_cv.wait(true, timeout, 0); // need lock
    }
    return lv_status;
}
