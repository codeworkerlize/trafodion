...
static const char     *gp_custom_config_file     = "ms.env.custom";
...
// a flag indicating whether this is dtmci
bool                   gv_is_dtmci              = false;
...
const char *ms_getenv_str(const char *pp_key) {
    if (!gv_ms_env_loaded) {
        if (!gv_ms_env.load(gp_config_arg_file)) { // try to load arg config
            ...
            if (!gv_ms_env.load(gp_custom_config_file)) { // try to load default config
                char *lp_var = getenv(gp_ms_env_sq_conf);
                if (lp_var != NULL) {
                    SB_Buf_Lline lv_file;
                    sprintf(&lv_file, "%s/%s", lp_var, gp_custom_config_file);
                    gv_ms_env.load(&lv_file);
                }
            }
        }
...
#ifdef SQ_PHANDLE_VERIFIER
        if (*pp_argc >= 10) {
            // "SQMON1.1" <pnid> <nid> <pid> <pname> <port> <ptype> <zid> <verif> <feature bits>
...
SB_Export short BMSG_BREAK_TIMEOUT_(int              pv_msgid,
                                    short           *pp_results,
                                    SB_Phandle_Type *pp_phandle,
                                    int              timeout) {
    SB_API_CTR (lv_zctr, BMSG_BREAK_);
    return xmsg_break_com_timeout(pv_msgid, pp_results, pp_phandle, NULL, false, timeout);
}
...
short xmsg_break_com_timeout(int       pv_msgid,
                     short            *pp_results,
                     SB_Phandle_Type  *pp_phandle,
                     MS_Md_Type      **ppp_md,
                     bool              pv_reply_ctrl,
                     int               timeout) {
    const char         *WHERE = "BMSG_BREAK_TIMEOUT_";
    MS_Md_Type         *lp_md;
    MS_Result_Type     *lp_results;
    MS_Result_Raw_Type *lp_results_raw;
    long long           lv_delta;
    ...
    lv_trace_delta = false;
    if (lp_stream == NULL) {
        lv_status = SB_Trans::Trans_Stream::wait_req_done_static(lp_md, timeout);
        if (lp_md->out.iv_opts & (SB_Trans::MS_OPTS_LDONE | SB_Trans::MS_OPTS_FSDONE))
            SB_Trans::Trans_Stream::remove_comp_q_static(lp_md);
        else if (gv_ms_trace_params)
            lv_trace_delta = true;
    } else {
        lv_status = lp_stream->wait_req_done(lp_md, timeout);
        if (lp_md->out.iv_opts & (SB_Trans::MS_OPTS_LDONE | SB_Trans::MS_OPTS_FSDONE))
            lp_stream->remove_comp_q(lp_md);
        else if (gv_ms_trace_params)
            lv_trace_delta = true;
    ...
    {
        return ms_err_rtn_msg(WHERE, " EXIT [break timed out]", XZFIL_ERR_TIMEDOUT);
    }
...
short msg_license_features_enabled() {
     const char      *WHERE = "msg_license_features_enabled";
     short lv_features = msg_mon_get_license_feature_set();
     if (gv_ms_trace_params)
        trace_where_printf(WHERE, "EXIT ret=%d\n", lv_features);
     return lv_features;
}

bool msg_license_advanced_enabled() {
     const char      *WHERE = "msg_license_advanced_enabled";
     short lv_features = msg_mon_get_license_feature_set();   
	  
     if (lv_features & LF_ADVANCED) {
         if (gv_ms_trace_params)
             trace_where_printf(WHERE, "EXIT ret=%d\n", true);
	 return true;
     } else {
...
bool  msg_license_enterprise_enabled(){
     const char      *WHERE = "msg_license_enterprise_enabled";
     short lv_features = msg_mon_get_license_feature_set();     
     
     // if advanced is enabled then we get the enterprise features by default
     if ((lv_features & LF_ENTERPRISE) || (msg_license_advanced_enabled())){
         if (gv_ms_trace_params)
             trace_where_printf(WHERE, "EXIT ret=%d\n", true);
	 return true;
     } else {
...
bool  msg_license_multitenancy_enabled() {
     const char      *WHERE = "msg_license_multitenancy_enabled";
     short lv_features = msg_mon_get_license_feature_set();     
     if ((lv_features & LF_MULTITENANCY) && lic.isModuleOpen(LM_MULTITENANCY)) {
         if (gv_ms_trace_params)
             trace_where_printf(WHERE, "EXIT ret=%d\n", true);
	 return true;
     } else {
