...
static short                  gv_ms_license_feature     = 0;
...
static void           msg_mon_recv_msg_close_or_death(int            pv_nid,
                                                      ...
                                                      int            pv_aborted,
                                                      bool           pv_close);
static void           msg_mon_recv_msg_feature(Mon_Msg_Type *pp_msg);
...
static void           msg_mon_trace_msg_close(const char   *pp_where,
                                              Mon_Msg_Type *pp_msg);
static void           msg_mon_trace_msg_feature(const char *pp_where, 
					      Mon_Msg_Type *pp_msg);
...
bool msg_mon_accept_sock_cbt(void *pp_sock) {
    ...
    nego_result = msg_mon_accept_sock_negotiate_id_srv(lp_sock,
                                                       &lv_nid,
                                                       &lv_pid,
#ifdef SQ_PHANDLE_VERIFIER
                                                       &lv_verif,
...
bool msg_mon_accept_sock_callback_create(SB_Trans::Sock_User  *pp_sock,
    ...
    if (lv_fserr != XZFIL_ERR_OK) {
        if (lv_fserr != XZFIL_ERR_INVALIDSTATE) {
            lp_stream->ref_dec(WHERE, NULL);
            // sock stream is already closed when lv_fserr is XZFIL_ERR_INVALIDSTATE 
    ...
    // lp_md is taking over lp_stream now
    lp_stream->ref_dec(WHERE, NULL);
...
int msg_mon_accept_sock_negotiate_id_srv(SB_Trans::Sock_User *pp_sock,
    ...
    ssize_t     lv_rc = 0;
    ssize_t     lv_rc2 = 0;
    ssize_t     buf_size = sizeof(lv_req);

    pp_pname[0] = '\0';
    readBuf = (char *)(&lv_req);
    lv_rc = pp_sock->read(readBuf, buf_size, &lv_errno);
    lv_rc2 = lv_rc;
    while ((lv_rc > 0 && lv_rc < buf_size) || (lv_rc2 < 0 && lv_errno == EWOULDBLOCK)) {
        lv_rc2 = pp_sock->read(readBuf + lv_rc, buf_size - lv_rc, &lv_errno);
        if (lv_rc2 > 0) {
            lv_rc += lv_rc2;
        } else if (lv_rc2 == 0 || lv_errno != EWOULDBLOCK) {
            break;
        }
    }
    if (lv_rc < buf_size || lv_req.iv_fserr != XZFIL_ERR_OK) {
        if (gv_ms_assert_error)
            SB_util_abort(WHERE);
        else
            return 1; //take the invalid message as network attack.
    }
...
int msg_mon_close_process_com(SB_Phandle_Type *pp_phandle,
    ...
    static int               slv_close_timouet_count=0;
    ...
    if (pv_free_oid) {
        if ((lp_od != NULL) && (!gv_ms_conn_reuse)) {
            ...
            if ((lp_stream != NULL) && (lv_ref == 0)) {
                ...
                if (lv_fserr == XZFIL_ERR_OK) {
                    int ret = lp_stream->wait_req_done(gp_ms_rsvd_md, 5); // 5s
                    if (ret == ETIMEDOUT) {
                        lv_fserr = XZFIL_ERR_TIMEDOUT;
                        slv_close_timouet_count++;
                    }
                    else {
...
int msg_mon_init_process_args(const char *pp_where,
    ...
    gv_ms_license_feature = (short)atoi((*pppp_argv)[10]);
#ifdef SQ_PHANDLE_VERIFIER
...
int msg_mon_open_process_com_ph2_sock(const char      *pp_where,
    ...
    lp_stream->ref_dec("msg_mon_open_process_com_ph2_sock", NULL);
    if (XZFIL_ERR_OK != lv_fserr)
      return lv_fserr;
    SB_Trans::Trans_Stream::map_nidpid_unlock();
...
int msg_mon_process_shutdown_ph1(const char                    *pp_where,
    ...
    // tell global sequence id server of shutdown
    // true means ignore boundsErr, gv_ms_od_mgr.ipp_table[iv_oid] == NULL is normal case
    msg_gsid_close_process(true);
...
SB_THROWS_FATAL {
    ...
    if (pv_attach) {
		...
		gv_ms_license_feature = lp_msg->msg.u.reply.u.startup_info.features;
#ifdef SQ_PHANDLE_VERIFIER
...
void msg_mon_recv_msg(MS_Md_Type *pp_md) {
    ...
    case MsgType_Close:
        break;
    case MsgType_Feature:
        msg_mon_recv_msg_feature(lp_msg);
        break;
...
void msg_mon_recv_msg_feature(Mon_Msg_Type *pp_msg)
{
   const char   *WHERE = "msg_mon_recv_msg_feature";
   gv_ms_license_feature =  pp_msg->u.request.u.feature.feature_set;
    if (gv_ms_trace_mon) {
        trace_where_printf(WHERE, "received feature notice from mon, feature = %d\n",
                               pp_msg->u.request.u.feature.feature_set);
    }	  
}
...
void msg_mon_trace_msg(const char *pp_where, Mon_Msg_Type *pp_msg) {
    ...
    case MsgType_Close:
        ...
        break;
    case MsgType_Feature:
        msg_mon_trace_msg_feature(pp_where, pp_msg);
        break;
    case MsgType_NodeDown:
...
void msg_mon_trace_msg_feature(const char *pp_where, Mon_Msg_Type *pp_msg) {
    trace_where_printf(pp_where, "mon-msg-feature, feature = %d\n",
                       pp_msg->u.request.u.feature.feature_set);
}
...
short msg_mon_get_license_feature_set() {
    return gv_ms_license_feature;
}
