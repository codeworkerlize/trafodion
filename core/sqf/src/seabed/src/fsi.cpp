        ...
        case FS_FS_WRITE:
            ...
            if (TRANSID_IS_VALID(lp_wr_ctrl_req->tcbref)) {
                ...
                lv_fserr = static_cast<short>(ms_transid_reg(lp_ru->iv_transid,
                                              lp_wr_ctrl_req->startid,
                                              lp_wr_ctrl_req->currSvptId,
                                              lp_wr_ctrl_req->parentSvptId));
            } else
...
short fs_int_fs_file_close(FS_Fd_Type *pp_fd) {
    ...
    int lv_close_time = 5;//in seconds
    lv_fserr = BMSG_BREAK_TIMEOUT_(lv_msgid,              // msgid
                           reinterpret_cast<short *>(&lv_results), // results
                           &pp_fd->iv_phandle,   // phandle
                           lv_close_time);   // timeout in seconds
    if (lv_fserr != XZFIL_ERR_OK)
        return fs_int_err_fd_rtn(WHERE, pp_fd, lv_fserr);
    return XZFIL_ERR_OK;
}
...
short fs_int_fs_file_writereadx(FS_Fd_Type  *pp_fd,
    ...
    lp_wr_ctrl_req->currSvptId = lv_svptid;
    lp_wr_ctrl_req->parentSvptId = lv_psvptid;
