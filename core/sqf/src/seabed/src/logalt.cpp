...
void SBX_log_write(int                     pv_log_type,
    ...
    sprintf(la_tl, "%04d-%02d-%02d %02d:%02d:%02d.%03d",
            lp_tx->tm_year + 1900, lp_tx->tm_mon + 1, lp_tx->tm_mday,
    ...
    if (pp_msg_prefix == NULL)
        pp_msg_prefix = "";
    pid_t lv_ppid = getppid();
    sprintf(la_log_msg, "%s: %s (name=%s/pid=%d/ppid=%d/tid=%d) (cmp=%d/ev=%d/fac=%d/sev=%d): %s",
            la_tl, pp_msg_prefix,
            pp_name, lv_pid, (int)lv_ppid, lv_tid,
