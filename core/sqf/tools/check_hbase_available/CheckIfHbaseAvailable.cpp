...
int test_createJVM()
{
  ...
  printf("thread_createJVM: Calling create jvm \n"); fflush(stdout);
  int ret = JNI_CreateJavaVM(&gp_jvm, (void**)&_tlp_jenv, &jvm_args);
  printf("thread_createJVM: Done Calling create jvm \n"); fflush(stdout);
  free(classPathArg);
...
void * thread_main (void *arg)
{
  ...
  if (isHBaseAvailable()) { printf("HBase is available\n"); } else { printf("HBase is not available\n");}
  if (isHBaseAvailable()) { printf("HBase is available\n"); } else { printf("HBase is not available\n");}
  if (isHBaseAvailable()) { printf("HBase is available\n"); } else { printf("HBase is not available\n");}
  if (isHBaseAvailable()) { printf("HBase is available\n"); } else { printf("HBase is not available\n");}
  if (isHBaseAvailable()) { printf("HBase is available\n"); } else { printf("HBase is not available\n");}
