...
public class TestHBaseClient {
	...
	*/
        ...
        HTableDescriptor desc = new HTableDescriptor("replicated_table");
        desc.setRegionReplication(2);
        desc.addFamily(new HColumnDescriptor("cf1"));
        HBaseAdmin admin = new HBaseAdmin(config);
	try {
            System.out.println ("Creating the table replicated_table");
	    admin.createTable(desc);
	}
	catch (TableExistsException e) {
            System.out.println("Table replicated_table already exists");
        }
