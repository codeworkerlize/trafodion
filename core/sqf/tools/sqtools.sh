...
function cmaph {
    ...
    head --quiet --lines $l_nl $TRAF_LOG/monitor.map.* 2>/dev/null | sort -k1
}

# tail of the monitor.map.* files in the cluster
function cmapt {
    l_nl=1
    if [ $# != 0 ]; then
	l_nl=$1
    fi
    tail --quiet --lines $l_nl $TRAF_LOG/monitor.map.* 2>/dev/null | sort -k1
}

#grep the monitor map file(s) on a node for the given string
function ngrepmm {
    l_lookup=$*
    grep -h "$l_lookup" $TRAF_LOG/monitor.map.*
}

#grep the monitor map file(s) on a node for the given string and then filters anything other than tdm_arkesp 
function ngrepmms {
    l_lookup=$*
    grep -h "$l_lookup" $TRAF_LOG/monitor.map.* | grep tdm_arkesp
}

# lookup the monitor.map.* files in the cluster
function cmapl {
    if [ $# == 0 ]; then
	echo "Enter a string to lookup in all the monitor.map.* files"
	return 1;
    fi
    l_lookup=$*
    ngrepmm $l_lookup 2>/dev/null
}
...
# ===== BEGIN ======= lookup the monitor.map.* files for ESPs 
function cmapls {
    if [ $# == 0 ]; then
	echo "Enter a string to lookup in all the monitor.map.* files"
	return 1;
...
# =======  END ======= lookup the monitor.map.* files for ESPs
...
function cmappc {
    ...
    grep -h $l_nl $TRAF_LOG/monitor.map.* | grep BEGIN | wc -l 2>/dev/null | sort -nk2
}


#### Begin MD5 related functions
function sqmd5b {
    cd $TRAF_HOME/export/bin${SQ_MBTYPE}; md5sum $1 2>/dev/null | cut -f2 -d: | sort -u
}

function sqmd5ba {
    cd $TRAF_HOME/export/bin${SQ_MBTYPE}; md5sum $1 2>/dev/null | sort
}

function sqmd5l {
    cd $TRAF_HOME/export/lib${SQ_MBTYPE}; md5sum $1 2>/dev/null | cut -f2 -d: | sort -u
}

function sqmd5la {
    cd $TRAF_HOME/export/lib${SQ_MBTYPE}; md5sum $1 2>/dev/null | sort -u
}

#### End MD5 related functions

function cmapc {
    grep $1 $TRAF_LOG/monitor.map* | grep $2 | wc -l
}
...
function espstats {
   ...
   echo "`date`; ESPs running: $lv_ncurr; Total started: $lv_nbegin; Total exited: $lv_nend;"
}
...
function sqlcistats {
   ...
   echo "`date`; SQLCIs running: $lv_ncurr; Total started: $lv_nbegin; Total exited: $lv_nend;"
}
...
function cmpstats {
   ...
   echo "`date`; CMPs running: $lv_ncurr; Total started: $lv_nbegin; Total exited: $lv_nend;"
}
...
function mxosstats {
   ...
   echo "`date`; MXOSRVRs running: $lv_ncurr; Total started: $lv_nbegin; Total exited: $lv_nend;"
}

function nprocpn {
    pstat | grep $1 | grep -v run_command | wc -l | sort -nk2
}

#unsorted nprocn
function nprocpn_us {
    pstat | grep $1 | grep -v run_command | wc -l
}

function ncmp {
    nprocpn_us tdm_arkcmp
}

function nesp {
    nprocpn_us tdm_arkesp 
}
...
function ndbm {
    df -h | grep database 2>/dev/null | wc -l
...
function sqchksl {
    cd $TRAF_LOG; grep Executing startup.log 2>/dev/null | sort -M -k3 -k4 -k5
}

function sqchkopt {
    df -h /opt 2>/dev/null | grep -v Avail | sort -rnk1
}

function sqchkvm {
    vmstat -a | head -2
    vmstat -a | grep -v swpd | grep -v memory | sort -rnk5
}
...
function sqlscore {
    ...
    cd /local/cores/$UID 2>/dev/null; ls -al 2>/dev/null | grep core | grep \"${lv_string}\" | cut -f 2 -d: | cut -b 4- 
}
...
function sqrmcore {
    ...
    cd /local/cores/$UID 2>/dev/null; ls -al 2>/dev/null | grep core | grep \"${lv_string}\" | cut -f 2 -d: | cut -b 4- | xargs rm
}
...
function sqchkmpi {
    cd $TRAF_LOG; egrep -i '(mpi bug|ibv_create)' *.log 2>/dev/null
}
...
#Collect hbase logs
function gethbaselogs {

    lv_logs_collection_dir=$1
    lv_hbaselogs_dir=${lv_logs_collection_dir}/$2
    mkdir -p ${lv_hbaselogs_dir}
    cp -p /var/log/hbase/hbase* ${lv_hbaselogs_dir} 
}
...
function sqsavelogs {
    ...
    cp -p $TRAF_LOG/check_peers.log ${lv_copy_to_dir}
    cp -p $TRAF_LOG/hbcheck.log ${lv_copy_to_dir}
    cp -p $TRAF_LOG/master_exec*.log ${lv_copy_to_dir}
    cp -p $TRAF_LOG/mon.*.log ${lv_copy_to_dir}
    cp -p $TRAF_LOG/monmem.*.log ${lv_copy_to_dir}
    cp -p $TRAF_LOG/mxlobsrvr*.log ${lv_copy_to_dir}
    cp -p $TRAF_LOG/pstart*.log ${lv_copy_to_dir}
    cp -p $TRAF_LOG/smstats.*.log ${lv_copy_to_dir}
    cp -p $TRAF_LOG/sqcheckmon.log ${lv_copy_to_dir}
    cp -p $TRAF_LOG/sqmo*.log ${lv_copy_to_dir}
    cp -p $TRAF_LOG/sscp*.log ${lv_copy_to_dir}
    cp -p $TRAF_LOG/ssmp*.log ${lv_copy_to_dir}
    cp -p $TRAF_LOG/startup.log ${lv_copy_to_dir}
    cp -p $TRAF_LOG/trafodion.*log* ${lv_copy_to_dir}
    cp -p $TRAF_LOG/tm*.log ${lv_copy_to_dir}
    cp -p $TRAF_LOG/udr*.log ${lv_copy_to_dir}
    cp -p $TRAF_LOG/wdg.*.log ${lv_copy_to_dir}
    cp -p $TRAF_LOG/xdc.log ${lv_copy_to_dir}
    cp -p $TRAF_LOG/dbsecurity*.log ${lv_copy_to_dir}
    cp -p $TRAF_LOG/krb5*.log ${lv_copy_to_dir}

    cp -p $TRAF_LOG/monitor.map.* ${lv_copy_to_dir}
    ...
    cp -p $TRAF_LOG/hs_err_pid*.log ${lv_stdout_dir_name}
    cp -p $TRAF_LOG/*.out ${lv_stdout_dir_name}
    cp -p $TRAF_HOME/hs_err_pid*.log ${lv_stdout_dir_name}

    lv_dcs_dir_name=${lv_copy_to_dir}/dcs_${lv_node}
    mkdir -p ${lv_dcs_dir_name}
    cp -p $TRAF_LOG/dcs/*.log* ${lv_dcs_dir_name}
    cp -p $TRAF_LOG/dcs/*.out* ${lv_dcs_dir_name}
    cp -p $TRAF_CONF/dcs/* ${lv_dcs_dir_name}
    cp -p $DCS_INSTALL_DIR/hs_err_pid*.log ${lv_dcs_dir_name}

    lv_rest_dir_name=${lv_copy_to_dir}/rest_${lv_node}
    mkdir -p ${lv_rest_dir_name}
    cp -p $TRAF_LOG/rest/*.log ${lv_rest_dir_name}
    cp -p $TRAF_LOG/rest/*.out ${lv_rest_dir_name}
    cp -p $TRAF_CONF/rest/* ${lv_rest_dir_name}
    cp -p $REST_INSTALL_DIR/hs_err_pid*.log ${lv_rest_dir_name}

    lv_dbmgr_dir_name=${lv_copy_to_dir}/dbmgr_${lv_node}
    mkdir -p ${lv_dbmgr_dir_name}
    cp -p $TRAF_LOG/dbmgr/*.log ${lv_dbmgr_dir_name}
    cp -p $TRAF_CONF/dbmgr/*.xml ${lv_dbmgr_dir_name}
...
# (for a particular node). sqsavelogs_compress
...
function sqcollectlogs {
    ...
    OPTIND=1
    while getopts "acdlzh" lv_arg $1 $2 $3 $4 $5 $6 $7 $8 $9
      do
      case $lv_arg in 
	  a)
	      lv_all=1
	  ...
	  h)
	      lv_help=1
	      echo
	      echo "Usage: sqcollectlogs {-a | -d | -l | -z | -h}"
	      echo
	      ...
	      echo "   Note: This option (-d) only applies if the -z option is also used."
              echo "-l Collect hbase logs. Defaults to /var/log/hbase folder"
    ...
    mkdir -p $LOGS_TARDIR/esgyndblogs
    pushd . > /dev/null
    cd $LOGS_TARDIR/esgyndblogs

    if [[ ${lv_hblog} == 1 ]]; then
       hbaselog_dir=hbaselogs
       gethbaselogs $PWD $hbaselog_dir
    fi
    
    lv_curr_time=`date +%Y%m%d_%H%M`
    lv_dir_name=sqinfo_`hostname -f`.$lv_curr_time
...
#   begin configuration info
    ...
    cp -p $TRAF_HOME/sql/scripts/gomon.cold . 2>/dev/null
    cp -p $TRAF_HOME/sql/scripts/gomon.warm . 2>/dev/null
    cp -p $TRAF_CONF/sqconfig . 2>/dev/null
    cp -p $TRAF_VAR/mon.env . 2>/dev/null
    cp -p $TRAF_CONF/monitor.env . 2>/dev/null
    cd ..
#   end configuration info
	...
	sqsavelogs $PWD $PWD 2>/dev/null
    else
	sqsavelogs_compress $lv_node_local_dir_name $PWD 2>/dev/null
    ...
    netstat -aep 2>/dev/null | sort  > netstat.out
    uptime 2>/dev/null | sort  > uptime.out
	...
	echo "Created esgyndb logs tar file: $PWD/${lv_dir_name}.tgz"
	rm -rf ${lv_dir_name}
        if [[ $lv_hblog == 1 ]]; then
	   tar -chzf ${hbaselog_dir}_`hostname`.tgz ${hbaselog_dir}
	   echo "Created hbase logs tar file: $PWD/${hbaselog_dir}_`hostname`.tgz"
	   rm -rf ${hbaselog_dir}
        fi
...
function sqcollectstacks {
    ...
    sqnpstack monitor $PWD/monitor 2>/dev/null &
    echo "Collecting mxosrvr pstacks"
    sqnpstack mxosrvr $PWD/mxosrvr 2>/dev/null &
    echo "Collecting tdm_arkcmp pstacks"
    sqnpstack tdm_arkcmp $PWD/tdm_arkcmp 2>/dev/null &
    echo "Collecting tdm_arkesp pstacks"
    sqnpstack tdm_arkesp $PWD/tdm_arkesp 2>/dev/null &
    echo "Collecting tm pstacks"
    sqnpstack tm $PWD/tm 2>/dev/null &
    wait
...
function sqgdb_doit {
    ...
    pids=`ps -C $prog -o uid,pid |grep $UID | awk '{ print $2 }'`
    echo "PIDs: $pids"
    ...
    let lv_count=0
    for pid in $pids; do
	echo "execute: gdb -p $pid --batch -x $my_script > $lv_dir_name/${host}_${pid}.${my_script_basename}.out "
	gdb -p $pid --batch -x $my_script > $lv_dir_name/${host}_${pid}.${my_script_basename}.out &
	let ++lv_count
	if [ $lv_count == 8 ]; then
	    let lv_count=0
	    wait
...
function dbspace {
    df -h | grep database | sort -rnk5
}
function tmpspace {
    df -h /tmp | grep -v Used | sort -rnk5
}
...
function cdo {
    cd $TRAF_HOME/../conn/odbc/src/odbc/nsksrvr
}
function cdl {
    cd $TRAF_LOG
}
...
function cdxs {
    cd $TRAF_HOME/src/seatrans/hbase-trx/src/main/java/org/apache/hadoop/hbase
}
function cdh {
...
function sb_regr_log_cleanup {
    pushd . > /dev/null
    cd $TRAF_HOME/rundir
#    find . -name runregr-sb.log | xargs $0 | sed -r 's/.{5}//' | rm
    find . -name runregr-sb.log |  rm xargs $0
    popd > /dev/null
}
...
function get_synchronous_tables {

sqlci <<EOF|  egrep -e '^[A-Z][A-Z]' | sed -e 's/ //g'
  select
  case when TX.text is not null then trim(TX.text) || ':' else '' end || 
  O.catalog_name || '.' || O.schema_name || '.' || O.object_name 
  from trafodion."_MD_".OBJECTS O 
       join trafodion."_MD_".TABLES T on O.object_uid = T.table_uid
       left join trafodion."_MD_".TEXT TX on O.object_uid = TX.text_uid and TX.text_type = 9  
  where O.schema_name not like '|_HV|_%%|_' escape '|'  
        and O.schema_name not like '|_HB|_%%|_' escape '|' 
        and schema_name not like 'VOLATILE_SCHEMA_%%' 
        and bitand(T.flags, 1) = 1 
        and O.object_type in ('BT')
...
select
   ...
   from trafodion."_MD_".OBJECTS O
        left join trafodion."_MD_".TEXT TX on O.object_uid = TX.text_uid and TX.text_type = 9, 
        trafodion."_MD_".INDEXES I ,
        trafodion."_MD_".tables T 
   where O.object_uid = I.INDEX_UID 
         and I.base_table_uid = T.table_uid
         and  O.schema_name not like '|_HV|_%%|_' escape '|'  
         and O.schema_name not like '|_HB|_%%|_' escape '|' 
         and O.schema_name not like 'VOLATILE_SCHEMA_%%' 
         and O.object_type in ('IX') 
         and bitand(T.flags,1) = 1

union

select
      'TRAF_RSRVD_1:TRAFODION._MD_.SEQ_GEN' from DUAL;
...
function get_synchronous_tables_without_namespace {
...
select
   ...
   from trafodion."_MD_".OBJECTS O
   left join trafodion."_MD_".TEXT TX on O.object_uid = TX.text_uid and TX.text_type = 9   
   where schema_name not like '|_HV|_%%|_' escape '|'  
         and schema_name not like '|_HB|_%%|_' escape '|' 
         and schema_name <> '_REPOS_' 
         and schema_name not like ('_MD_')  
         and schema_name not like 'VOLATILE_SCHEMA_%%' 
         and object_type in ('BT', 'IX');
EOF
...
function get_user_tables_without_namespace {
...
export -f get_synchronous_tables
export -f get_user_tables
export -f get_synchronous_tables_without_namespace
export -f get_user_tables_without_namespace
export -f run_sb_regr
export -f run_sb_test
export -f check_sb_regr
export -f status_sb_regr
export -f sb_regr_log_cleanup
