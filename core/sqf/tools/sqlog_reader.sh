...
function lmaph {
    ...
    head --quiet --lines $l_nl $MY_LOGROOT/monitor.map.[0-9]*.* | sort -k1
}
...
function lmapt {
    ...
    tail --quiet --lines $l_nl $MY_LOGROOT/monitor.map.[0-9]*.* | sort -k1
}
...
function lmapl {
    ...
    l_lookup=$*
    grep "$l_lookup" $MY_LOGROOT/monitor.map.[0-9]*.*
}
...
function lmapls {
    ...
    l_lookup=$*
    grep "$l_lookup" $MY_LOGROOT/monitor.map.[0-9]*.* | grep tdm_arkesp
}
...
function lmappc {
    ...
    grep -h $l_nl $MY_LOGROOT/monitor.map.[0-9]*.* | grep BEGIN | wc -l | sort -nk2
}


function lmapc {
    
    grep $1 $MY_LOGROOT/monitor.map* | grep $2 | wc -l
}
...
function lsqchksl {
    ...
    cd $MY_LOGROOT/; grep Executing startup*.log 2>/dev/null | sort -k4 -k5
    cd -
}

function lsqchkmpi {
    cd $MY_LOGROOT/; egrep -i '(mpi bug|ibv_create)' *.log
}
