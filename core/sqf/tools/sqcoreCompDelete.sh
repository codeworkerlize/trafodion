...
if [ ${mode} == "m" ] ; then
	...
	edb_pdsh -a "${scriptName} -s $(uname -n) ${redoMask}"
	...
	echo "      ${DIAG_LOG}"
	pdcp $(trafconf -wname) -p ${DIAG_LOG} ${DIAG_DIR}    # Ship unique list to all nodes.
	echo "============================================================"
	...
	if [ ${wantDelete} -eq 1 ] ; then
		...
		echo ""
		edb_pdsh -a "${scriptName} -s $(uname -n) -d ${deleteTooOld}"
	fi
