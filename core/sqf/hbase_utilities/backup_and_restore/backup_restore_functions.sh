...
validate_hbase_sudo_access()
{
   ...
   if [[ $which_env -eq 2  ]]; then
    ...
    then
      ...
      echo "***Restore operation requires sudo hbase access for trafodion user. *** "   | tee -a ${log_file}
      echo "***Please uncomment the following line in /etc/sudoers.d/trafodion file ***"  | tee -a ${log_file}
      echo "***  %trafodion ALL =(hbase) NOPASSWD: /usr/bin/hbase    ***"  | tee -a ${log_file}
      return 1
    fi
   fi
   return 0
...
validate_environment_and_users()
{
  ...
  #if user tag is present, it is archival of existing snapshots.
  #no need of shutting down trafodion
  if [[ -z $1 ]]; then
