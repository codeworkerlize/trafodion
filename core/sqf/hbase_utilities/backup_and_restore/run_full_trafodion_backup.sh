...
usage()
{
  ...
  echo "     confirm the selection afterwards."
  echo "-t user tag"
  echo "     (Optional) The backup user tag used during online backup. This tag is displayed"
  echo "     in response to 'get trafodion backups' command issued in sql prompt"
  echo "-m mappers"
...
hbase_trafodion_snapshot_list=${tmp_dir}/bckp_trafodion_snapshot_list_${date_str}.hbase
tmp_trafodion_snapshot_list_file=$tmp_dir/bckp_tmp_trafodion_snapshot_list_${date_str}.txt
tmptmp_trafodion_snapshot_list_file=$tmp_dir/bckp_tmptmp_trafodion_snapshot_list_${date_str}.txt

hbase_trafodion_namespace_list=${tmp_dir}/bckp_trafodion_namespace_list_${date_str}.hbase
trafodion_namespace_list_file=$tmp_dir/bckp_trafodion_namespace_list_${date_str}.txt
tmp_trafodion_namespace_list_file=${tmp_dir}/bckp_tmp_trafodion_namespace_list_${date_str}.txt
trafodion_namespace_list_file_name=trafodion_namespace_list.txt;
...
while getopts b:u:m:h:d:t:l:no arguments
do
  case $arguments in
...
validate_environment_and_users $user_tag
if [[ $? -ne 0 ]]; then
  exit 1
fi
...
echo "hbase_trafodion_namespace_list: ${hbase_trafodion_namespace_list}" | tee -a $log_file
cat <<EOF > ${hbase_trafodion_namespace_list}
list_namespace
exit
EOF
if [[ ! -f "${hbase_trafodion_namespace_list}" ]]; then
 echo "***[ERROR]: Cannot create the ${hbase_trafodion_namespace_list} file. Exiting ..."   | tee -a $log_file
 exit 1;
fi

echo "Getting list of Trafodion namespace from HBase "  | tee -a  $log_file
$(get_hbase_cmd) shell ${hbase_trafodion_namespace_list} | tee  ${tmp_trafodion_namespace_list_file}   | tee -a  $log_file
echo "grep for trafodion namespaces"      | tee -a  $log_file
#filter out anything other than Trafodion namespaces from the list
grep 'TRAF_*'   ${tmp_trafodion_namespace_list_file} | tee  ${trafodion_namespace_list_file}    | tee -a  $log_file
...
do
  name_space=$line
  echo "list '${name_space}.*'" | tee -a ${hbase_trafodion_table_list}
done < ${trafodion_namespace_list_file}
###Also list tables in no-namespace location
  echo "list 'TRAFODION.*'" | tee -a ${hbase_trafodion_table_list}
echo "exit" | tee -a ${hbase_trafodion_table_list}
...
grep 'TRAF*'   ${tmp_trafodion_table_list_file} | tee  ${trafodion_table_list_file}    | tee -a  $log_file
...
do
  table_name=$line
  table_name_with_replaced_colon=${table_name/:/_}
  snapshot_name=${table_name_with_replaced_colon}_${snap_suffix}_${date_str}
...
## End of listing tables, creating snapshots, restarting trafodion.
fi

if [[ ! -z $user_tag ]]; then
###get list of snapshots matching user provided tag
echo "hbase_trafodion_snapshot_list: ${hbase_trafodion_snapshot_list}" | tee -a $log_file
cat <<EOF > ${hbase_trafodion_snapshot_list}
list_snapshots  'TRAF*.*'
exit
EOF
if [[ ! -f "${hbase_trafodion_snapshot_list}" ]]; then
  echo "***[ERROR]: Cannot create the ${hbase_trafodion_snapshot_list} file. Exiting ..."   | tee -a $log_file
 exit 1;
fi
 
echo "Getting list of snapshots from HBase "  | tee -a  $log_file
$(get_hbase_cmd) shell ${hbase_trafodion_snapshot_list} | tee  ${tmp_trafodion_snapshot_list_file}   | tee -a  $log_file
echo "grep for trafodion user tag snapshots"      | tee -a  $log_file
 
echo "filter out anything other than user tag snapshots from the list" | tee -a $log_file
snapshot_tag=_SNAPSHOT_${user_tag}_
grep $snapshot_tag  ${tmp_trafodion_snapshot_list_file} | tee ${tmptmp_trafodion_snapshot_list_file} | tee -a $log_file
 
echo "Filtering table names from snapshot list output" | tee -a $log_file
sed 's/ //;s/ .*//;' ${tmptmp_trafodion_snapshot_list_file} | grep "TRAFODION*" | tee  ${trafodion_snapshot_list_path}  | tee -a  $log_file
fi
...
#Also copy the generated namespace list to the hdfs backup location
cp ${trafodion_snapshot_list_path} /tmp/${trafodion_snapshot_list_file}
cp ${trafodion_namespace_list_file} /tmp/${trafodion_namespace_list_file_name}
...
hbase_cmd="$(get_hadoop_cmd) fs -copyFromLocal /tmp/${trafodion_namespace_list_file_name}  ${hdfs_backup_location}"
echo "${hbase_cmd}"   | tee -a  ${log_file}
do_sudo  ${hbase_user}  "${hbase_cmd}"  2>&1   | tee -a  ${log_file}
if [[ ${PIPESTATUS[0]} -ne 0 ]] ; then
   echo "***[ERROR]: Error occurred while copying the ${trafodion_namespace_list_file} file to hdfs backup location ${hdfs_backup_location}."   | tee -a $log_file
   echo "For more information please check the logs at ${log_file}. "   | tee -a $log_file
   exit 1;
fi
...
hbase_cmd="$(get_hadoop_cmd) fs -ls -d ${hdfs_backup_location}/TRAF*"
var_backup_snapshot_count=$(do_sudo ${hbase_user}  "$hbase_cmd" | wc -l)
...
#delete all temp files created as part of this backup.
rm -f $tmp_log_file
rm -f $hbase_create_snapshots
rm -f $hbase_delete_snapshots
rm -f $hbase_disable_tables
rm -f $hbase_drop_tables
rm -f $hbase_trafodion_namespace_list
rm -f $trafodion_namespace_list_file
rm -f $tmp_trafodion_namespace_list_file
rm -f $hbase_trafodion_table_list
rm -f $trafodion_table_list_file
rm -f $tmp_trafodion_table_list_file
rm -f $trafodion_snapshot_list_path
rm -f $trafodion_snapshot_list_path
rm -f $hbase_trafodion_snapshot_list
rm -f $tmp_trafodion_snapshot_list_file
rm -f $tmptmp_trafodion_snapshot_list_file
