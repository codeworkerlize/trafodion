...
#ifndef NAMESERVER_PROCESS
void CProcess::deathInterestRemove(int nid, int pid) {
    deathInterestLock_.lock();
    nidPid_t target = { nid, pid };
    deathInterest_.remove(target);
    deathInterestLock_.unlock();
}
#endif
    ...
    if (( !Clone ) && (++results_count == Monitor->GetNumVerifiers()))
    {
        ...
        { // reply pending, so send reply
            ...
            if (verificationSuccess)
                memcpy(msg->u.reply.u.license.license, licenseStruct,LICENSE_NUM_BYTES_MAX);
                                       ...
                                       struct timespec *creation_time, int /*origPNidNs*/)
{
    ...
    if ( !Clone )
    {
        ...
        else if ( Type == ProcessType_DTM )
        {
            ...
            if (Recreated)
            {
                Nodes->SendNodeDownMessage(this);
                Recreated = false;
            }
        }
...
bool CProcess::Create (CProcess *parent, void* tag, int & result)
{
   ...
   // strcpy (mpiflags, "l,y0,Eon");
    ...
    // "SQMON1.0" <pnid> <nid> <pid> <pname> <port> <ptype> <zid> <verifier> <feature bits>
    ...
    argv[j + 9] = new char[6];
    sprintf (argv[j + 9], "%5.5d", HealthCheck.getLicenseFeature());
...
void CProcessContainer::AttachProcessCheck ( struct message_def *msg )
{
             ...
             (msg->u.request.u.startup.pid == -1)   )
    {
        ...
        if (!process)
        {
            ...
            if ( ! MyNode->IsSpareNode() && !IAmIntegrating )
            {
                ...
                else
                {
                    ...
                    msg->u.reply.u.startup_info.features = HealthCheck.getLicenseFeature();
            ...
            {
                snprintf( la_buf, sizeof(la_buf),
                        "[%s], Can't attach, node is integrating or is a spare node!\n",
                        method_name );
...
void CProcessContainer::Exit_Process (CProcess *process, bool abend, int downNode)
{
    ...
    if (process)
    {
        ...
        // We have trouble understanding following code when fixing M-18577,
        // and improve the fix of M-18577 in the future if necessary.
    ...
    default:
        ...
        case Zid_ALL:       // recreate in current up <nid> or next up <nid>
            ...
            if ((downNid != -1 && !currentLNode->GetNode()->IsSpareNode()) ||
                 currentLNode->GetState() == State_Down || process->GetType() == ProcessType_TMID)
            {
                newLNode = Nodes->GetLNodeNext( process->GetNid(), true);
                     ...
                     newLNode->GetNid() != downNid ) )
                {
                    ...
                    else
                    {
                        if (trace_settings &
                            (TRACE_SYNC_DETAIL | TRACE_REQUEST_DETAIL | TRACE_PROCESS_DETAIL))
                            trace_printf( "%s@%d - Preclone process from nid=%d to nid=%d""\n"
                                        , method_name, __LINE__
                                        , process->GetNid(), nid);
                        newNode = newLNode->GetNode();
                        CProcess * cloneProcess;
                        struct timespec createTime = process->GetCreationTime();
                        int origPNidNs = 0;
                        #ifdef NAMESERVER_PROCESS
                        origPNidNs = process->GetOrigPNidNs();
                        #endif
                        cloneProcess = newNode->CloneProcess(nid,
                                process->GetType(),
                                process->GetPriority(),
                                process->IsBackup(),
                                process->IsUnhooked(),
                                const_cast<char *>(process->GetName()), // process name
                                const_cast<char *>(process->GetPort()),  // port
                                -1, //osPid_
                                process->GetVerifier(),
                                process->GetParentNid(),
                                process->GetParentPid(),
                                process->GetParentVerifier(),
                                process->IsEventMessages(),
                                process->IsSystemMessages(),
                                process->pathStrId(),
                                process->ldPathStrId(),
                                process->programStrId(),
                                const_cast<char *>(process->infile()),  // infile
                                const_cast<char *>(process->outfile()), // outfile
                                &createTime,
                                origPNidNs);
                       if (cloneProcess)
                       {
                         cloneProcess->userArgs(process->argc(),
                                                process->userArgvLen(),
                                                process->userArgv());
                       }
                    }
                ...
                else
                {
                    if (trace_settings &
                        (TRACE_SYNC_DETAIL | TRACE_REQUEST_DETAIL | TRACE_PROCESS_DETAIL))
                    {
                       STATE node_state = (newLNode == NULL) ? State_Unknown : newLNode->GetState();
                       trace_printf( "%s@%d - Next possible node is not available, nid=%d, newNodeState = %d, downNid = %d\n"
                                    , method_name, __LINE__, nid, node_state, downNid);
                    }
                }
            }
            else
