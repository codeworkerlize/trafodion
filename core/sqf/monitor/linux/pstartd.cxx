        ...
        for ( ; persistConfig; persistConfig = persistConfig->GetNext() )
        {
            ...
            case ProcessType_SSMP:
                ...
                if (procName.length() != 0)
                {
                    ...
                    {
                        int retries = MyNid * 20;
                        int count = 0;
                        bool isSingleton = ((persistConfig->GetProcessType() == ProcessType_TMID) ||
                                            procName.compare("$CMON") == 0);//one per cluster
                        do  {
                          if ( ! monUtil.requestProcInfo( procName.c_str()
                                                        , procNid
                                                        , procPid))
                          {// Process does not exist
                           // for idtm and CMON, if MyNid is not 0, wait and check
                            if (count++ < retries && isSingleton &&
                                isIdtmStartedByMe_ == false)
                            {
                              char buf[MON_STRING_BUF_SIZE];
                              snprintf( buf, sizeof(buf), "Process %s does not start, "
                                        "wait 1s and check(Nid = %d, retry = %d)\n",
                                        procName.c_str(), MyNid, count);
...
int main (int argc, char *argv[])
{
    ...
    MonLog = new CMonLog( "log4cplus.monitor.psd.config", "PSD", "alt.pstartd", MyPNID, MyNid, MyPid, MyName );
