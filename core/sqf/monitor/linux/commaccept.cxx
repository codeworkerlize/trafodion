...
void CCommAccept::processNewComm(MPI_Comm interComm)
{
    ...
    {   // Handle error
        ...
        char buf[MON_STRING_BUF_SIZE];
        snprintf(buf, sizeof(buf), "[%s@%d], unable to obtain node id from new "
...
{
    if (thread_shutdown_) {
        // if the thread has not begun, or another thread has already shutdown this CCommAccept, just return.
        return;
    } else {
        thread_shutdown_ = true;
    }
...
void CCommAccept::start()
{
    ...
    if (rc != 0)
    {
        ...
        // now this thread_id_ can be shutdown
