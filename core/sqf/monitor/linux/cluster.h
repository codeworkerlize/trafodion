...
public:
...
#endif
    int  Connect( const char *portName, bool doRetries = true, bool doTimeout = false);
    ...
    bool sendPing(int timeout,int *socketerror=0);
#ifdef NAMESERVER_PROCESS
...
private:
    ...
    int  MkCltSock( unsigned char srcip[MAX_IP_SIZE_ARRAY],
                    unsigned char dstip[MAX_IP_SIZE_ARRAY], int port );

    int createSocket(char* host, int port, struct sockaddr_in &sockinfo);

};

extern bool Emulate_Down;

extern int SYNC_MAX_RESPONSIVE;
