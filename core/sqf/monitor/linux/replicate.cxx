...
#else
    return  max(max(max(max(max(max(max(max(max(max(max(max(max(max(max(max(max(max(max(max(max(max(max(max(sizeof(CReplNameServerAdd),
#endif
               ...
               , req_verifier_ (req_verifier)
{
    ...
    memcpy (license_, req_license, LICENSE_NUM_BYTES_MAX);
...
bool CReplLicense::replicate(struct internal_msg_def *&msg)
{
    ...
    if (trace_settings & (TRACE_SYNC | TRACE_PROCESS))
        trace_printf("%s@%d" " - Validate License\n", method_name, __LINE__);
    ...
    memcpy (msg->u.license.license, license_, LICENSE_NUM_BYTES_MAX);
...
bool CReplLicenseVerified::replicate(struct internal_msg_def *&msg)
{
    ...
    if (trace_settings & (TRACE_SYNC | TRACE_PROCESS))
        trace_printf("%s@%d" " - License Verified\n", method_name, __LINE__);
