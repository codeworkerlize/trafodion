...
#define ZCLIENT_DOWN_ZNODE_STATE            "/state"
...
public:
    ...
    bool    IsRunningZNodeExpired( const char *nodeName, int &zerr ,bool isLocal=false,int timeoutPing=0 );
    ...
    ZClientState_t StateGet( void ) {
        CAutoLock lock(getLocker());
        return terminateMonitor?ZC_SHUTDOWN:state_;
    }
    ...
    int     updateExceptZKState(string dataState, string path);
    int     ZNodeStateDataGet( string stateZnode);
    string  getDownStateZNodePath_(){return downStateZNodePath_;}
private:
    ...
    int     MyZooExistRetry(zhandle_t *zh, const char *path, int watch, struct Stat *stat,int *elapse);
    bool    sendPingSokcet(int timeoutPing, int elapse);
    pthread_t       threadId_;
