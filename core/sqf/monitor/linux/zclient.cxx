...
#define ZOOKEEPER_RETRY_COUNT                 2
#define ZOOKEEPER_RETRY_WAIT                  1 // seconds
#define ZOOKEEPER_LOG_THRESHOLD               2 // seconds
...
extern pthread_t gPthreadId;

extern bool exeScriptEnable;
static zhandle_t *ZHandle;
static int myzcDelay=3;
static int pingTimeout=1;
         ...
         ,zkSessionTimeout_(ZCLIENT_SESSION_TIMEOUT) // seconds
{
    ...
    char *zcDelayString;
    if ( (zcDelayString = getenv( "SQ_MON_ZCLIENT_MY_ZNODE_DELAY" )) )
    {
	    // in seconds
	    int  zcDelay = atoi( zcDelayString );
	    if((zcDelay>=0) && zcDelay<(zkSessionTimeout_/3))
		    myzcDelay=zcDelay;
    }  

    char *timeoutPingString;
    if ( (timeoutPingString = getenv( "SQ_MON_ZCLIENT_MY_ZNODE_PING" )) )
    {
	    int pingTimeout_ = atoi( timeoutPingString);
	    if((pingTimeout_>0) && pingTimeout_<(zkSessionTimeout_/3))
...
bool CZClient::IsRunningZNodeExpired( const char *nodeName, int &zerr ,bool isLocal,int timeoutPing )
{
    ...
    if(isLocal)
    {
    rc = MyZooExistRetry(ZHandle, monZnode.c_str( ), 0, &stat,&elapse);
    }
    else{
    ...
    if (trace_settings & (TRACE_INIT | TRACE_RECOVERY))
    {
        ...
        trace_printf("IsRunningZNodeExpire error=%s,cost[%ld]s \n"
                        , zerror(rc),(ed.tv_sec-st.tv_sec));
    }
    if ( rc == ZNONODE )
    {
        expired = true;
    ...
    else if ( rc == ZCONNECTIONLOSS || rc == ZOPERATIONTIMEOUT )
    {
        ...
        char buf[MON_STRING_BUF_SIZE];
        snprintf( buf, sizeof(buf)
                , "[%s], Loss or Timeout zoo_exists() for %s failed with error %s\n,sendPing For Check"
                ,  method_name, monZnode.c_str( ), zerror(rc));
        mon_log_write(MON_ZCLIENT_ISZNODEEXPIRED_1, SQ_LOG_ERR, buf);
	if(isLocal){
	  expired = sendPingSokcet(timeoutPing,elapse);
	}
        // to stop the RegionServer of HBase on THIS node(not the hbase cluster)
        /*CNode  *node = Nodes->GetNode(MyPNID);
        exec_rm_hbase_rs_zk_cmd(node->GetName(), __FUNCTION__);
        snprintf( buf, sizeof(buf)
                , "[%s@%d], invoke exec_rm_hbase_rs_zk_cmd to stop RegionServer on THIS node\n"
                ,  method_name, __LINE__);
        mon_log_write(MON_ZCLIENT_ISZNODEEXPIRED_1, SQ_LOG_ERR, buf);*/
    }
    else if ( rc == ZOK )
    ...
    else
    {
        ...
        case ZCLOSING:
            ...
            // we can't communicate with quorum server
...
bool  CZClient::sendPingSokcet(int timeoutPing,int elapse  ){
	bool  expired = true;
    ...
    char ebuff[256];
    char buf[MON_STRING_BUF_SIZE];
    snprintf( buf, sizeof(buf), "SendPing Info elapse[%d],timeoutPing[%d] \n",
                               elapse,timeoutPing );
    mon_log_write(MON_CLUSTER_ALLGATHERSOCK_3, SQ_LOG_INFO, buf );
    while(true){
       int socketerror=-1;
       isSuccess= Monitor->sendPing(timeoutPing,&socketerror);
       expired = (!isSuccess)&&(socketerror!=0);
       clock_gettime(CLOCK_REALTIME,&ct);
       if((isSuccess && socketerror==0) || ((ct.tv_sec-st.tv_sec)>=elapse)){
            break;
        }
    }
    return expired;
...
void CZClient::MonitorCluster()
{
    ...
    while (StateGet() != ZC_SHUTDOWN && !terminateMonitor)
    {
        lock();
        if ( !IsEnabled() )
        {
        ...
        else
        {
         ...
         trace_printf( "%s@%d" " - MonitorCluster  state_=%s,node[%s]\n"
                                        , method_name, __LINE__
                                        , ZClientStateStr(StateGet()),Node_name );
...
void CZClient::MyRunningZNodeCheck( void )
{
    ...
    char *zkTimeoutFactor;
    int timeoutFactorValue=3;
    if ( (zkTimeoutFactor = getenv( "SQ_MON_ZCLIENT_TIMEOUT_FACTOR" )) )
     {
        timeoutFactorValue = atoi(zkTimeoutFactor);
     }
 trace_printf( "MyRunningZNodeCheck timeoutFactorValue[%d],currenttime[%ld],myznodetime[%ld],reset[%d],sessiontimeout[%d],pingtimeout[%d],node[%s]\n", 
              timeoutFactorValue,currentTime.tv_sec, myZNodeFailedTime_.tv_sec,resetMyZNodeFailedTime_,SessionTimeoutGet(),pingTimeout,Node_name );
    if ( IsClusterWatchEnabled() )
    {
        if (resetMyZNodeFailedTime_)
        {
            ...
            myZNodeFailedTime_.tv_sec += (SessionTimeoutGet() * timeoutFactorValue);
#if 0
            if (trace_settings & (TRACE_INIT | TRACE_RECOVERY))
            {
        ...
        if ( ! IsRunningZNodeExpired( Node_name, zerr,true,pingTimeout ) )
        {    
             trace_printf( "currenttime[%ld],myznodetime[%ld],,node[%s],error[%s]\n", currentTime.tv_sec, myZNodeFailedTime_.tv_sec,Node_name,zerror(zerr) );
            ...
            if ( zerr == ZCONNECTIONLOSS || zerr == ZOPERATIONTIMEOUT )
            {
                ...
                if (currentTime.tv_sec > myZNodeFailedTime_.tv_sec)
                {
                    char buf[MON_STRING_BUF_SIZE];
                    snprintf( buf, sizeof(buf)
                            , "[%s], Zookeeper quorum comm error: %s - Handling my znode (%s) as expired! Node is going down,currenttime[%ld],myznodetime[%ld]\n"
                            , method_name, zerror(zerr), Node_name,currentTime.tv_sec, myZNodeFailedTime_.tv_sec );
        ...
        else if (zerr != ZOK)
        {
            char buf[MON_STRING_BUF_SIZE];
            snprintf( buf, sizeof(buf)
                    , "[%s], My znode (%s) expired! Node is going down. exeScriptEnable[%d]\n"
                    , method_name, Node_name,exeScriptEnable);
...
int CZClient::MyStateZNodeCreate( void )
{
    const char method_name[] = "CZClient::MyStateZNodeCreate";
    TRACE_ENTRY;
    ...
    stringstream newpath;
    newpath.str( "" );
    newpath << downStateZNodePath_.c_str() << "/"
            << Node_name;
    ...
    ss.str( "" );
    ss << NORMAL ;
    string stateData = ss.str( );
    
    if (trace_settings & (TRACE_INIT | TRACE_RECOVERY))
    {
        trace_printf( "%s@%d MyStateZNodeCreate(%s:%s)\n"
                    , method_name, __LINE__
                    , stateZnode.c_str()
                    , stateData.c_str() );
    }
    
    rcExist = ZooExistRetry( ZHandle, stateZnode.c_str( ), 0, &stat );
    
    if(rcExist == ZNONODE)
    { 

      rc = ZNodeCreate( stateZnode.c_str(), stateData.c_str(), 0 );

    }
    else if ( rcExist == ZOK || rcExist == ZNODEEXISTS )
    {
      rc = updateExceptZKState(stateData,stateZnode);
    }else
    {
      rc=rcExist;
    }
...
int CZClient::ZNodeStateDataGet( string stateZnode)
{
    const char method_name[] = "CZClient::ZNodeStateDataGet";
    TRACE_ENTRY;
    ...
    if (trace_settings & (TRACE_INIT | TRACE_RECOVERY))
    {
        trace_printf( "%s@%d monZnode=%s\n"
                    , method_name, __LINE__, stateZnode.c_str() );
    }
    rc = ZooExistRetry( ZHandle, stateZnode.c_str( ), 0, &stat );
    if ( rc == ZNONODE )
    {
        // return the error
        if (trace_settings & (TRACE_INIT | TRACE_RECOVERY))
    ...
    else if ( rc == ZOK )
    {
        ...
        rc = zoo_get( ZHandle, stateZnode.c_str( ), false, zkData, &zkDataLen, &stat );
        if ( rc == ZOK )
        {
            data=atoi(zkData);
        }
        else
        {
            ...
            snprintf( buf, sizeof(buf)
                    ...
                    ,  method_name, stateZnode.c_str(), zerror(rc));
...
int  CZClient::updateExceptZKState(string dataState, string path)
{
   const char method_name[] = "CZClient::updateExceptZKState";
   int rc = ZOK;
   Stat stat;
 
   if( dataState.empty() || path.empty() )   // change stat from STARTING to AVAILABLE
   {   
       return -1;
   }

   rc = zoo_set(ZHandle, path.c_str(), dataState.c_str(), dataState.length(), -1);
   
  if(rc != ZOK)
  {
      char buf[MON_STRING_BUF_SIZE];
      snprintf( buf, sizeof(buf)
                , "[%s], node update %s for %s failed with error %s\n"
                ,  method_name, dataState.c_str(),path.c_str(), zerror(rc));
      mon_log_write(MON_ZCLIENT_ZNODEDATAGET_2, SQ_LOG_ERR, buf);
  }
   return rc;
}
...
void CZClient::TriggerCheck( int type, const char *znodePath )
{
    ...
    if (trace_settings & (TRACE_INIT | TRACE_RECOVERY))
    {
        trace_printf( "%s@%d" " TriggerCheck - type- type=%s, path=%s\n"
                    , method_name, __LINE__
                    , ZooConnectionTypeStr( type )
                    , znodePath );
    }
...
int CZClient::ZNodesTreeCreate( void )
{
    ...
    ss << zkRootNode_.c_str() 
       << zkRootNodeInstance_.c_str() 
       << ZCLIENT_CLUSTER_ZNODE
       << ZCLIENT_DOWN_ZNODE_STATE;
    downStateZNodePath_ = ss.str();

    rc = ZooExistRetry( ZHandle, downStateZNodePath_.c_str( ), 0, &stat );
    switch (rc)
    {
    case ZOK:
        break;
    case ZNONODE:
        if (trace_settings & (TRACE_INIT | TRACE_RECOVERY))
        {
            trace_printf( "%s@%d Invoking ZNodeCreate(%s)\n"
                        , method_name, __LINE__
                        , downStateZNodePath_.c_str() );
        }
        rc = ZNodeCreate( downStateZNodePath_.c_str(), NULL, 0 );
        if ( rc && rc != ZNODEEXISTS )
        {
            return(rc); // Return the error
        }
    ...
    default:
        ...
        snprintf( buf, sizeof(buf)
                , "[%s], zoo_exists(%s) failed with error %s\n"
                , method_name, downStateZNodePath_.c_str(), zerror(rc) );
        mon_log_write(MON_ZCLIENT_ZNODESTREECREATE_8, SQ_LOG_ERR, buf);
        break;
    }
...
int CZClient::MyZooExistRetry(zhandle_t *zh, const char *path, int watch, struct Stat *stat,int *elapse)
{
	int retries = 0;
	int rc;
	long rectime=0;
        int timeout= zoo_recv_timeout(zh);
	struct timespec st;
	struct timespec ed;
	clock_gettime(CLOCK_REALTIME,&st);
	rc = zoo_exists(zh, path, watch, stat);
	clock_gettime(CLOCK_REALTIME,&ed);
	rectime=ed.tv_sec-st.tv_sec;
    if(rectime >= ZOOKEEPER_LOG_THRESHOLD)
    {
	char buf[MON_STRING_BUF_SIZE];
	snprintf( buf, sizeof(buf)
			, "MY ZooExistRetry retry =%s,cost[%ld]s,myzcdelay[%d],path=%s \n"
			, zerror(rc),rectime,myzcDelay,path);
	mon_log_write(MON_ZCLIENT_ZNODESTREECREATE_8, SQ_LOG_WARNING, buf);
    }	
	// retry when loss zconnection or timeout, this may be caused by one zookeeper server down
	if( (rc == ZCONNECTIONLOSS
				...
				|| rc == ZSESSIONMOVED))
	{
		int bias=timeout/1000-rectime-myzcDelay;
 		if(bias>0){
		     *elapse=bias;
		}
