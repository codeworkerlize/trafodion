...
const int SQ_LocalIOToClient::requestSize[] = {
   ...
   sizeof(REQTYPE) + sizeof( Feature_def ),           // MsgType_Feature
...
// in most cases, check to see if this is not NULL is made before
...
serialRequestThread( void * )
{
    ...
    if (trace_settings & TRACE_MLIO)
       trace_printf("%s@%d" " Thread started, terminateMonitor=%d\n", method_name, __LINE__, terminateMonitor);
    ...
    while (!terminateMonitor)
    {
        ...
        else if (!terminateMonitor && sig != SIGUSR1 )
        {
            if (trace_settings & TRACE_MLIO_DETAIL)
                   ...
                   , almostDeadPidsError_(0)
{
  ...
  ((SharedMemHdr*)clientBuffers)->lastIdtmRefresh = 0;
  ((SharedMemHdr*)clientBuffers)->lastDTMRefresh = 0;
  if (trace_settings & TRACE_INIT)
