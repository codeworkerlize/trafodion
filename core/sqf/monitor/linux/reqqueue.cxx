...
void CIntCloneProcReq::performRequest()
{
    ...
    CLNode *targetLNode = Nodes->GetLNode(nid_);
    if (targetLNode->GetState() != State_Up)
    {
      char buf[MON_STRING_BUF_SIZE];
      snprintf(buf, sizeof(buf),
               "[%s], Process (%d, %d) will not be "
               "cloned because it is not in State_Up, "
               "targetLNode->GetState() = %d \n",
               method_name, nid_, osPid_,
               (int)(targetLNode->GetState()));

      mon_log_write(MON_INTREQ_CLONEPROC_1, SQ_LOG_INFO, buf);
      return;
    }

    if ( priorPid_ != 0 && priorPid_ != -1)
...
void CIntChildDeathReq::performRequest()
{
    ...
    if ( process_ != NULL)
    {
        ...
        // Fix M-18577: if this process is deleted from pidMap and CRedirectStderr::handleHangup
        // is not called in time, Abended field will not be set true and then TM will not be restarted.
        if (!process_->IsAttached() && process_->GetState() != State_Down && !process_->IsAbended())
        {
            process_->SetAbended( true );
        }
                ...
                , req_verifier_ (req_verifier)
{
    ...
    memcpy (license_, license, LICENSE_NUM_BYTES_MAX);
}
...
void CIntLicenseReq::performRequest()
{
    ...
    if ((!licenseFile.getLicenseReady()) || (strncmp(licenseFile.getLicense(), license_,LICENSE_NUM_BYTES_MAX)!=0))
    {
...
void CIntLicenseVerifiedReq::performRequest()
{
    ...
    if (req_pid_ == myPid)
    {
      ...
      int expireDate;
      memcpy ((void*)&expireDate, &(license_[LICENSE_EXPIRE_OFFSET]), LICENSE_EXPIRE_LEN);
...
void CIntNodeDeleteReq::performRequest()
{
    ...
    string nodeName(pnodeConfig->GetName());
    if (clusterConfig->DeleteNodeConfig( pnid_ ))
    {
        ZClient->ConfiguredZNodeDelete( nodeName.c_str() );
...
const char * CReqQueue::intReqType[] = {
    ...
    , "License"           // InternalType_License
    , "LicenseVerified"   // InternalType_LicenseVerified
