...
bool terminateMonitor = false;

extern bool IsRealCluster;
extern int MyPNID;
extern CMonLog *MonLog;
pthread_t gPthreadId = 0;

pthread_mutex_t       MonLogMutex = PTHREAD_MUTEX_INITIALIZER;

void mon_failure_exit()
{
    terminateMonitor = true;

    char buf[MON_STRING_BUF_SIZE] = {0};
    snprintf( buf, sizeof(buf)
            , "[%s@%d] Global variable terminateMonitor is set to TRUE,"
            " to terminate this monitor\n"
            , __FUNCTION__, __LINE__);
    mon_log_write(MON_CLUSTER_ALLGATHERSOCK_8, SQ_LOG_CRIT, buf );
    if (gPthreadId != 0)
        pthread_join(gPthreadId, NULL);
    exit(0);
}
...
CMonLog::CMonLog( const char *log4cplusConfig
                , const char *log4cplusComponent
                , const char *logFilePrefix
                , int pnid
                , int nid
                , int pid
                , const char *processName )
       : log4cplusConfig_(log4cplusConfig)
       , log4cplusComponent_(log4cplusComponent)
       , myPNid_(pnid)
       , myNid_(nid)
       , myPid_(pid)
       ...
       , memLogBase_(NULL)
{
    ...
    CommonLogger::instance().initLog4cplus(log4cplusConfig_.c_str(), logFileSuffix);
}
...
void CMonLog::writeMonLog(int eventType, posix_sqlog_severity_t severity, char *msg)
{
    ...
    // Log4cplus logging
    CommonLogger::log( log4cplusComponent_
                     , llevel
                     , "%s, NID:%u, PID:%u, TID:%d, txID: , Message ID: %u, %s"
                     , myProcessName_.c_str(), myPNid_, myPid_,  GETTID(), eventType,  msg);
...
void CMonLog::writeMonProcLog(int eventType, posix_sqlog_severity_t severity, char *msg)
{
    ...
    CommonLogger::log( log4cplusComponent_
                     , llevel
                     , "%s, NID:%u, PID:%u, TID:%u, txID: Message ID: %u, %s"
                     , myProcessName_.c_str(), myPNid_, myPid_, GETTID(), eventType, msg);
