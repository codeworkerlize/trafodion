...
void exitWdg()
{
    const char method_name[] = "CWatchdog::exitWdg";
    int monitorPid = gp_local_mon_io->get_monitor_pid();

    bool isDefunct = false;
    FILE *fd;
    char line_buff[256] = {0};
    char file[64] = {0};
    sprintf(file, "/proc/%d/status", monitorPid);
    fd = fopen(file, "r");
    if (fd != NULL)
    {
        while (fgets(line_buff, sizeof(line_buff), fd) != NULL)
        {
            if (strstr(line_buff, "State:") != NULL)
            {
                if (strstr(line_buff, "Z") != NULL)
                {
                    isDefunct = true;
                }
                break;
    ...
    // monitor process is dead, bring the node down or monitor is defunct process
    if ((kill( monitorPid,0 ) == -1 && errno == ESRCH) || isDefunct == true)
    {
        ...
        {
            sprintf(la_buf, "[%s], execute 'pkillall -safekill' command success!\n", method_name);
        }
        trace_printf("%s@%d\n", la_buf, __LINE__);
    }
}

void sigExitWdg(int sig)
{
    const char method_name[] = "CWatchdog::sigExitWdg";
...
int main (int argc, char *argv[])
{
    ...
    act.sa_handler = sigExitWdg;
    sigemptyset(&act.sa_mask);
    sigaddset(&act.sa_mask, SIGABRT);  // abort
    sigaddset(&act.sa_mask, SIGSEGV);  // not right address
    sigaddset(&act.sa_mask, SIGBUS);   // not right address 
    act.sa_flags = 0;
    ...
    MonLog = new CMonLog( "log4cplus.monitor.wdg.config", "WDG", "alt.wdg", MyPNID, MyNid, MyPid, MyName  );
