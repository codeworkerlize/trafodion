...
public:
    ...
    void setLicenseInfo(int expireTime, bool success);
    short getLicenseFeature();
#endif
    ...
    void IdtmRefreshCheck();
    void DTMRefreshCheck();
...
private:
    ...
    bool licenseValidateSuccess_;       // Did validation succeed?
#endif
    bool cpuSchedulingDataEnabled_;     // monitors exchange CPU scheduling data when enabled
    struct timespec lastIdtmCheckTime_;
    unsigned long lastIdtmRefresh_;
    unsigned long lastDTMRefresh_;
    int idtmCheckInterval_;
};
