...
static char *envSleepTime = getenv("MON_SLEEP_TIME_AFTER_TM_RESTART");
static int sleepTime = (envSleepTime && isdigit(*envSleepTime) ? atoi(envSleepTime) : 10);
void CLNodeContainer::SendNodeDownMessage(CProcess *proc, int nidIgnored)
{
    const char method_name[] = "CLNodeContainer::SendNodeDownMessage";
    CLNode *lnode = head_;
    bool shouldSleep = (nidIgnored >= 0 ? false : true);
    TRACE_ENTRY;

    while (lnode)
    {
        if ( lnode->GetNid() != nidIgnored && lnode->GetState() == State_Down)
        {
            char buf[MON_STRING_BUF_SIZE];
            snprintf(buf, sizeof(buf), "%s - broadcast node (%d) down to process (%s, %d) \n",
                     method_name, lnode->GetNid(), proc->GetName(), proc->GetState());
            mon_log_write(MON_LNODE_MARKDOWN, SQ_LOG_WARNING, buf);

            if (proc->GetState() == State_Up)
            {
                const char * nodeName = lnode->GetNode()->GetName();
#ifndef NAMESERVER_PROCESS
                struct  message_def *msg = new struct message_def;
                msg->type = MsgType_NodeDown;
                msg->noreply = true;
                msg->u.request.type = ReqType_Notice;
                msg->u.request.u.down.nid = lnode->GetNid();
                msg->u.request.u.down.takeover = lnode->GetNode()->IsActivatingSpare();
#ifdef USE_SEQUENCE_NUM
                msg->u.request.u.down.seqnum = Monitor->GetTimeSeqNum();
#endif
                if (IsRealCluster)
                {
                    nodeName = lnode->GetNode()->GetFqdn();
                    STRCPY(msg->u.request.u.down.node_name, nodeName);
                }
                else
                {
                    sprintf(msg->u.request.u.down.node_name,"%s:%d", nodeName, lnode->GetNid());
                }
#ifndef NAMESERVER_PROCESS
               if ( NameServerEnabled )
                {
                    NameServer->ProcessNodeDown(lnode->GetNid(), msg->u.request.u.down.node_name );
                }
#endif
               if (shouldSleep)
               {
                   //wait for process to startup completely, and using sleep is not a good way.
                   sleep(sleepTime);
                   shouldSleep = false;
               }
               SQ_theLocalIOToClient->putOnNoticeQueue( proc->GetPid(), proc->GetVerifier(), msg, NULL ); 
#endif
            }
