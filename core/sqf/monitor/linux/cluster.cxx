...
extern CMeas Meas;
extern bool exeScriptEnable;
extern bool isProcessExcept;
...
int SYNC_MAX_RESPONSIVE;
bool CheckExpiredByZK=false;
...
//  * pv_node_name, node name
//  * reason, can the one of the followings:
//      * "IsRunningZNodeExpired",  zk connection is lost, or operation timeouts,
//      * "HardNodeDown", node is going to shutdown,
long exec_rm_hbase_rs_zk_cmd(const char* pv_node_name, const char* reason) {
  if (trace_settings )
    trace_printf( "exec_rm_hbase_rs_zk_cmd@%d - Executing \"exec_rm_hbase_rs_zk_cmd %s %s\"\n",
                __LINE__,
                pv_node_name,
                reason);


#define MAX_PIPE_BUF 5000  
  char lv_cmd[MAX_PIPE_BUF];
  lv_cmd[0] = 0;
  sprintf(lv_cmd, "rm_hbase_rs_zknode %s %s &", pv_node_name, reason);
...
void CCluster::HardNodeDown (int pnid, bool communicate_state)
{
    ...
    if ( MyPNID == pnid ) {
        ...
        newpath.str( "" );
        newpath << ZClient->getDownStateZNodePath_().c_str() << "/"
		    << Node_name;
        string stateZnode = newpath.str( ); 
        snprintf(buf, sizeof(buf), "[%s@%d],  node down ,pnid=%d ,mypid=%s ,name=%s\n"
                 , method_name, __LINE__,pnid,mynode->GetName(),Node_name);
        mon_log_write(MON_CLUSTER_MARKDOWN_1, SQ_LOG_INFO, buf);
        if(isProcessExcept){
               stringstream ss;
               ss.str("");
               ss << CZClient::PROCESSDOWN;
               string zdata(ss.str());
               ZClient->updateExceptZKState(zdata,stateZnode);
               snprintf(buf, sizeof(buf)
                        ,"[%s@%d], update node state of this node when node down ,pnid=%d ,statePath=%s m,state=%s ,name=%s\n"
                        , method_name, __LINE__,pnid,stateZnode.c_str(),zdata.c_str(),Node_name);
               mon_log_write(MON_CLUSTER_MARKDOWN_1, SQ_LOG_INFO, buf);
           }

    }else{
 
	    stringstream downpath;
	    downpath.str( "" );
	    downpath << ZClient->getDownStateZNodePath_().c_str() << "/"
		    << node->GetName() ;
	    string downstateZnode = downpath.str( );        
        
        data = ZClient->ZNodeStateDataGet(downstateZnode);
        switch (data)
        {
             case CZClient::PROCESSDOWN:
                  exeScriptEnable = false;
                  break;
             case CZClient::NETEXCEPT:
             case CZClient::NORMAL:
                 exeScriptEnable = true;
                  break;
             default:
                 exeScriptEnable = true;
        } 
       snprintf(buf, sizeof(buf)
                ,"[%s@%d], state of this node is DOWN ,downpnid=%s ,statePath=%s m,state=%d ,currentnode=%s\n"
                , method_name, __LINE__,node->GetName(),downstateZnode.c_str(),data,Node_name);
       mon_log_write(MON_CLUSTER_MARKDOWN_1, SQ_LOG_INFO, buf);   
    }
          ...
          MyNode->IsKillingNode() ) )
    {
        ...
        {
            snprintf(buf, sizeof(buf), "[%s@%d], state of this node is DOWN, just return\n",
                    method_name, __LINE__);
            mon_log_write(MON_CLUSTER_MARKDOWN_1, SQ_LOG_INFO, buf);
    ...
    {
        snprintf(buf, sizeof(buf), "[%s@%d], Node failure during shutdown, down nid = %d\n",
    ...
    if (node->GetState() != State_Down || !node->isInQuiesceState())
    {
            ...
            !MyNode->isInQuiesceState() )
        {
            ...
            case State_Shutdown:
                ...
                { // Terminate CommAccept thread, remote pings will fail
                    snprintf(buf, sizeof(buf), "[%s@%d], shutdown the CommAccept thread\n",
                            method_name, __LINE__);
                    mon_log_write(MON_CLUSTER_MARKDOWN_1, SQ_LOG_INFO, buf);
        ...
        else
        {
            if (node->GetState() != State_Down)
            {
                ...
                myProcess = MyNode->GetProcessByType(ProcessType_DTM);
                if (myProcess != NULL) {
                    snprintf(buf, sizeof(buf),
                             "[CCluster::HardNodeDown], Node %s (%d)is down, MyNode %s (%d) and its process name %s.\n",
                             node->GetName(), node->GetPNid(), MyNode->GetName(), MyNode->GetPNid(), myProcess->GetName());
                    mon_log_write(MON_CLUSTER_MARKDOWN_1, SQ_LOG_INFO, buf);

                    Nodes->SendNodeDownMessage(myProcess, node->GetPNid());
                }
            }
        }
    }
    ...
    else
    {
        ...
        snprintf(buf, sizeof(buf)
                , "[%s@%d] be about to execute exec_rm_hbase_rs_zk_cmd, IsMaster:[%d], MyPNID:[%d],exeScript[%d] \n"
                 , __FUNCTION__, __LINE__, IsMaster, MyPNID,exeScriptEnable);
        mon_log_write(MON_CLUSTER_MARKDOWN_4, SQ_LOG_INFO, buf);

        if (!IsMaster) {
          //Temporary solution(set IsMaster is better)
          srand((unsigned)MyPNID);
          long time =(long)((rand() % 10+1)*100*1000);
          //avoid concurrent exe
          usleep(time);
          trace_printf("badNode:[%s],sleep:[%ld] \n",node->GetName(),time);
        }
        if(exeScriptEnable){

           exec_rm_hbase_rs_zk_cmd(node->GetName(), __FUNCTION__);
           
           snprintf(buf, sizeof(buf)
                    , "[%s@%d]  execute exec_rm_hbase_rs_zk_cmd, IsMaster:[%d], MyPNID:[%d]\n"
                    , __FUNCTION__, __LINE__, IsMaster, MyPNID);
            mon_log_write(MON_CLUSTER_MARKDOWN_4, SQ_LOG_INFO, buf);
        }
    }
                                   ...
                                   int pnid)
{
    ...
    case InternalType_License:
        ...
        {
            if (trace_settings & (TRACE_SYNC | TRACE_REQUEST | TRACE_PROCESS))
                trace_printf("%s@%d - Queuing internal license request\n", method_name, __LINE__);
    ...
    case InternalType_LicenseVerified:
        ...
        {
            if (trace_settings & (TRACE_SYNC | TRACE_REQUEST | TRACE_PROCESS))
                trace_printf("%s@%d - Queuing internal license verified request\n", method_name, __LINE__);
...
bool CCluster::responsive()
{
    ...
    if  ( !barrierDiff && isMonInitComplete() )
    {
        ...
        cumulativeSyncDelay_ += SYNC_MAX_RESPONSIVE + 1;
...
bool CCluster::PingSockPeer( CNode *node, struct timespec &peerZnodeFailTime )
{
    ...
    for (int i = 0; i < (sv_connect_retry_count*sv_connect_wait_timeout); i++ )
    {
        ...
        pingSock = Monitor->Connect( node->GetCommPort(), false, true);
        if ( pingSock < 0 )
        {
            ...
            else if (currentTime.tv_sec > peerZnodeFailTime.tv_sec)
            {
                ...
                mon_log_write(MON_PINGSOCKPEER_2, SQ_LOG_WARNING, buf);
                 //temporary
                if (CheckExpiredByZK && ZClientEnabled && createErrorZNode)
                {
                    if (node->GetState() == State_Up)
                    {
    ...
    else
    {
        ...
        {   // Handle error
            ...
            char buf[MON_STRING_BUF_SIZE];
            snprintf( buf, sizeof(buf)
                    , "[%s@%d], unable to obtain node sync info from remote, "
                      "monitor: %d(%s).\n"
                    , method_name, __LINE__, rc, ErrorMsg(rc));
            mon_log_write(MON_PINGSOCKPEER_5, SQ_LOG_ERR, buf);
            return(false);
        }
        else
        {
            if (CheckExpiredByZK && ZClientEnabled)
            {
                int zerr;
    ...
    for ( int i = 0; i < GetConfigPNodesCount(); i++ )
    {
        ...
        for ( int j = i+1; j < GetConfigPNodesCount(); j++ )
        {
            ...
            { // Current indexToPnid_[i] node is my node, so connect to indexToPnid_[j] node
                ...
                if (trace_settings & (TRACE_INIT | TRACE_RECOVERY)) {
                            trace_printf( "%s@%d - Node %s(%d) state:%d\n",
                                        __FUNCTION__, __LINE__,
                                        node->GetName(), node->GetPNid(), node->GetState());
                }
                if (node->GetState() != State_Up && node->GetState() != State_Joining)
                {
                    if (socks_[indexToPnid_[idst]] != -1)
                    { // Peer socket is still active
                        if (trace_settings & (TRACE_INIT | TRACE_RECOVERY))
                        {
                            trace_printf( "%s@%d - Node %s (%d) is not up, state:%d, "
                                          "removing old socket from epoll set, "
                                          "socks_[%d]=%d\n"
                                        , method_name, __LINE__
                                        , node->GetName(), node->GetPNid(), node->GetState()
                ...
                {   
                    if ((CheckExpiredByZK && ZClientEnabled && ZClient->IsRunningZNodeExpired( node->GetName(), zerr ))
                     || MyNode->IsPendingNodeDown()
                     || (MyNode->GetState() != State_Up && MyNode->GetState() != State_Joining)
                     || (node->GetState()   != State_Up && node->GetState()   != State_Joining))
                    {
                        if (socks_[indexToPnid_[idst]] != -1)
                        {
                            if (trace_settings & (TRACE_INIT | TRACE_RECOVERY))
            ...
            { // Current indexToPnid_[j] is my node, accept connection from peer indexToPnid_[i] node
                ...
                {   //A temporary solution. Later we shuould solve a design problem by multi instances.
                    if ((CheckExpiredByZK && ZClientEnabled && ZClient->IsRunningZNodeExpired( node->GetName(), zerr ))
                     || MyNode->IsPendingNodeDown()
                     || (MyNode->GetState() != State_Up && MyNode->GetState() != State_Joining)
                     || (node->GetState()   != State_Up && node->GetState()   != State_Joining))
                    {
                        if (socks_[indexToPnid_[idst]] != -1)
                        {
                            if (trace_settings & (TRACE_INIT | TRACE_RECOVERY))
                            {
                                trace_printf("%s@%d - MyNode->GetState()=%d, node->GetState()=%d\n",
                                            __FUNCTION__, __LINE__, MyNode->GetState(),
                                            node->GetState());
                                   ...
                                   int sentChangeNid)
{
    ...
    for (int index = 0; index < GetConfigPNodesCount(); index++)
    {
        ...
        if (trace_settings & TRACE_SYNC)
        {
            trace_printf("%s@%d - nodestate[%d], node_state: %c , change_nid: %d , seq_num=%lld, monProcCount=%d\n",
                         method_name, __LINE__, indexToPnid_[index], nodestate[indexToPnid_[index]].node_state,
                         nodestate[indexToPnid_[index]].change_nid, nodestate[indexToPnid_[index]].seq_num,
#ifdef NAMESERVER_PROCESS
                         nodestate[indexToPnid_[index]].monConnCount
#else
...
#ifndef NAMESERVER_PROCESS
            Node[indexToPnid_[index]]->SetNumProcs(nodestate[indexToPnid_[index]].monProcCount);
#endif
            doShutdown = true;
            break;
        default:
...
int CCluster::Connect( const char *portName, bool doRetries, bool doTimeout)
{
    ...
    for ( ;; )
    {
        ...
        if ( sock < 0 )
        {
            char la_buf[MON_STRING_BUF_SIZE];
            int err = errno;
            sprintf( la_buf, "[%s@%d], socket() failed! errno=%d (%s)\n"
                   , method_name, __LINE__, err, strerror( err ));
        ...
        if (doTimeout) {
            // timeout for receiving, 2s by default
            int sq_mon_peer_socket_recv_timeout = 2;

            char *sq_mon_peer_socket_recv_timeout_env = getenv( "SQ_MON_PEER_SOCKET_RECV_TIMEOUT");
            trace_printf("%s@%d - environment variable SQ_MON_PEER_SOCKET_RECV_TIMEOUT is %s\n",
                            __FUNCTION__, __LINE__, sq_mon_peer_socket_recv_timeout_env);
            if (sq_mon_peer_socket_recv_timeout_env)
            {
                sq_mon_peer_socket_recv_timeout = atoi(sq_mon_peer_socket_recv_timeout_env);
            }

            struct timeval timeout = {sq_mon_peer_socket_recv_timeout, 0};
...
int CCluster::MkCltSock( unsigned char srcip[MAX_IP_SIZE_ARRAY],
                         unsigned char dstip[MAX_IP_SIZE_ARRAY], int port )
{
...
int CCluster::ReceiveSock(char *buf, int size, int sockFd, const char *desc)
{
    ...
    do
    {
        ...
        if (trace_settings & (TRACE_REQUEST | TRACE_INIT | TRACE_RECOVERY))
        {
            trace_printf( "%s@%d - recv(%d), sock=%d, return value=%d, desc=%s, errno=%d\n"
                        , method_name, __LINE__
                        , sizeCount
                        , sockFd
                        , readCount
        ...
        { // Got an error
            if (errno != EINTR)
            {
                // even if errno is EAGAIN(same as EWOULDBLOCK), which is the result of timeout,
                // do not try again, and treat it as a error
                error = errno;
                char la_buf[MON_STRING_BUF_SIZE];
                sprintf( la_buf, "[%s@%d], recv(), received=%d, sock=%d, error=%d(%s), desc=%s\n"
                       , method_name, __LINE__
                       , received
                       , sockFd
...
int CCluster::createSocket(char* host, int port, struct sockaddr_in &sockinfo)
{    

    const char method_name[] = "CCluster::createSocket";
    int sockfd = -1; 
    if (-1 == (sockfd = socket(AF_INET, SOCK_STREAM, 0)))
    {
	char ebuff[256];
	char buf[MON_STRING_BUF_SIZE];
	snprintf( buf, sizeof(buf), "[%s@%d] socket  error: %s\n",
			method_name, __LINE__,
			strerror_r( errno, ebuff, 256 ) );
	mon_log_write( MON_CLUSTER_ALLGATHERSOCK_3, SQ_LOG_CRIT, buf );
	mon_failure_exit();
	...
	char ebuff[256];
	char buf[MON_STRING_BUF_SIZE];
	snprintf( buf, sizeof(buf), "[%s@%d] gethostbyname error: %s\n",
			method_name, __LINE__,
			strerror_r( errno, ebuff, 256 ) );
	mon_log_write( MON_CLUSTER_ALLGATHERSOCK_3, SQ_LOG_CRIT, buf );
	mon_failure_exit();
    }
    int size = sizeof(sockinfo);
    memset( (char *) &sockinfo, 0, size );
    memcpy( (char *) &sockinfo.sin_addr, (char *) he->h_addr, 4 );
    sockinfo.sin_family = AF_INET;
    sockinfo.sin_port = htons( (unsigned short) port );
...
bool CCluster::sendPing(int timeoutPing,int *sockeerror) {
    const char method_name[] = "CCluster::sendPing";
    CNode *node;
    fd_set set;
    FD_ZERO(&set);
    int max_sock = 0;
    ...
    for (int i = 0; i < GetConfigPNodesCount(); i++) {
        ...
        struct sockaddr_in  sockinfo; // socket address info
        struct hostent *he;
        const char *portName = node->GetCommPort();
        char host[1000];
        const char *colon;
        unsigned int port;
        colon = strstr(portName, ":");
        ...
        port = atoi(&colon[1]);
        int size = sizeof(sockinfo);
        int sock = createSocket(host,port, sockinfo);
        char buf[MON_STRING_BUF_SIZE];
        snprintf( buf, sizeof(buf), "[%s@%d] portName[%s] sockfd[%d]\n",
                       method_name, __LINE__, portName,sock
                         );
        mon_log_write(MON_CLUSTER_ALLGATHERSOCK_3,SQ_LOG_INFO, buf);
        fd_socks.push_back(sock);
        max_sock = max_sock > sock ? max_sock : sock;
        int ret = connect(sock, (struct sockaddr *) &sockinfo, size);
        if(ret==0) {
            for (unsigned int i = 0; i < fd_socks.size(); i++) {
                close(fd_socks[i]);
            }
            return true;
    ...
    timeo.tv_sec = timeoutPing;
    timeo.tv_usec = 0;
    sleep(timeoutPing);
    int retval = select(max_sock + 1, NULL, &set, NULL, &timeo);           
    if(retval < 0)
    {
        char ebuff[256];
        char buf[MON_STRING_BUF_SIZE];
        snprintf( buf, sizeof(buf), "[%s@%d] select non [%d] error: %s\n",
                        method_name, __LINE__, timeoutPing,
                        strerror_r( errno, ebuff, 256 ) );
        mon_log_write(MON_CLUSTER_ALLGATHERSOCK_3, SQ_LOG_CRIT, buf );
        retValue = false;
    }
    else if(retval == 0) 
    {
        char ebuff[256];
        char buf[MON_STRING_BUF_SIZE];
        snprintf( buf, sizeof(buf), "[%s@%d] select timeout [%d] error: %s\n",
                        method_name, __LINE__, timeoutPing,
                        strerror_r( errno, ebuff, 256 ) );
        mon_log_write(MON_CLUSTER_ALLGATHERSOCK_3, SQ_LOG_WARNING, buf );
        retValue = false;
    } else {
      
	for (unsigned int i = 0; i < fd_socks.size(); i++) {
            if(FD_ISSET(fd_socks[i], &set)) {
                int error = 0;
                socklen_t len = sizeof(error);
	            if(getsockopt(fd_socks[i], SOL_SOCKET, SO_ERROR, &error, &len) < 0) {
		           retValue = false;	
		        }
               char ebuff[256];
               char buf[MON_STRING_BUF_SIZE];
               snprintf( buf, sizeof(buf), "[%s@%d] sockesSize[%ld],etsockopt [%d], errornum[%d],error[%s] error: %s\n",
                        method_name, __LINE__,fd_socks.size(),fd_socks[i], error,strerror(error),
                        strerror_r( errno, ebuff, 256 ) );
               mon_log_write(MON_CLUSTER_ALLGATHERSOCK_3, SQ_LOG_INFO, buf );
            if(error != 0){
               retValue = false;
