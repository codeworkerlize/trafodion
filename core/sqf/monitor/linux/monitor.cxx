...
bool exeScriptEnable = true;
bool isProcessExcept = false;
#ifndef NAMESERVER_PROCESS
...
{
public:
    ExecutePreOp() : backup_fd_(-1)
    {
        backup_fd_ = dup(STDIN_FILENO);
        if (backup_fd_ != -1)
        {
            close(STDIN_FILENO);
        }
...
// @TODO Replace fork & execve with pseudo-terminal (more efficient)
typedef CPKillAll CommandExecutor;

class MonitorPreOp
{
#define PREOPTAG "/tmp/MonitorPreOp"
#define FOOTPRINT PREOPTAG "@footprint"
#define ENABLEFOOTPRINT FOOTPRINT "@enable"
#define DISABLEPREOP PREOPTAG "@disable"
#define PREOPCMD "pkillall -safekill -preop"
#define FILEEXIST(x) (access(x, F_OK) == 0)
public:
    MonitorPreOp()
    {
        if (!FILEEXIST(DISABLEPREOP))
        {
            Cleanup();
        }
    }
...
protected:
    int Footprint(bool do_close = true)
    {
        int fd = creat(FOOTPRINT, 0600);
        if (fd != -1)
        {
            char cmd[] = PREOPCMD;
        ...
        return do_close ? -1 : fd;
    }

    void Cleanup()
    {
        if (FILEEXIST(ENABLEFOOTPRINT))
        {
            Footprint();
        }
        // It is the WATCHDOG's duty to kill all concerned processes
        // after checking the death of MONITOR, but the Tester may kill
        // WATCHDOG & MONITOR at the same time (one by one). This may
        // cause MONITOR's child processes into an orphan process and
        // leave MXOSRVR processes in an invalid state.
        CommandExecutor kill_all(PREOPCMD);
        kill_all.ExecuteCommand(0);
    }
#undef FILEEXIST
#undef PREOPCMD
#undef DISABLEPREOP
#undef ENABLEFOOTPRINT
#undef FOOTPRINT
#undef PREOPTAG
};
...
{
    // mantis-22109
    //  reset signal mask(inherited from shell process)
    ...
    // SYNC_MAX_RESPONSIVE is 1 by default
    SYNC_MAX_RESPONSIVE = 1;
    char *sq_mon_sync_max_responsive_env = getenv("SQ_MON_SYNC_MAX_RESPONSIVE");
    trace_printf("%s@%d - environment variable SQ_MON_SYNC_MAX_RESPONSIVE is %s\n",
            __FUNCTION__, __LINE__, sq_mon_sync_max_responsive_env);
    if (sq_mon_sync_max_responsive_env) {
        SYNC_MAX_RESPONSIVE = atoi(sq_mon_sync_max_responsive_env);
    }
...
#ifdef NAMESERVER_PROCESS
    MonLog = new CMonLog( "log4cplus.monitor.trafns.config", "NS", "alt.mon", MyPNID, -1, getpid(), "$TNS" );
#else
    MonLog = new CMonLog( "log4cplus.monitor.mon.config", "MON", "alt.mon", MyPNID, -1, getpid(), "$MONITOR" );
#endif
...
retryMaster:
            ...
            char    buf[MON_STRING_BUF_SIZE];
            snprintf( buf, sizeof(buf)
                           , "[%s@%d], Master Monitor is %s on node %d\n"
                           , method_name, __LINE__
