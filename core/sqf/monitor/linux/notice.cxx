        ...
        if( notify )
        {
            if (!notify->IsClone())
            {
                ...
                else if (notify->IsSystemMessages() )
                {
                    ...
                    if (notify->GetType() == ProcessType_DTM) {
                        Nodes->CancelDeathNotification(Process->GetNid(), Process->GetPid(), Process->GetVerifier(), null_trans());
                        notify->deathInterestRemove(Process->GetNid(), Process->GetPid());
                    }
