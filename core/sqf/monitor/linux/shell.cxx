...
void recv_notice_msg(struct message_def *recv_msg, int )
{
    ...
    case MsgType_Feature:
        if (! QuietShell) printf("[%s] %s - Feature notice received (%d)\n", 
                                 MyName, time_string(), recv_msg->u.request.u.feature.feature_set);
        break;
...
bool start_monitor( char *cmd_tail, bool warmstart, bool reintegrate )
{
    ...
    else
    {
        ...
        // mantis-22109
        //  The monitor process would kill its parent(hydra_pmi_proxy or ssh process)
        //  to take over its child processes, which causes the defunct processes(as
        //  mpirun process is alive and does not handle or ignore SIGCHLD). The shell
        //  process launches the mpirun by its child process, if shell ignore SIGCHLD,
        //  mpirun would inherit the signal mask(with execve), the zombies is ignored.
...
void license_cmd ()
{
    ...
    char   msgString[MAX_BUFFER] = { 0 };
    int    count;
    char   decrypt_string[LICENSE_NUM_BYTES_MAX+1];
    short  version;
    char   name [LICENSE_CUSTOMER_LEN+1];
    int    nodenumber = 0;
    time_t exday = 0;
        ...
        (count == sizeof (struct message_def)))
    {
            ...
            (msg->u.reply.type == ReplyType_License))
        {
            ...
            { 
                 memset(name, 0, LICENSE_CUSTOMER_LEN+1);
                 memcpy ((void*)decrypt_string, msg->u.reply.u.license.license,LICENSE_NUM_BYTES_MAX );

                 char display[LICENSE_NUM_BYTES_ENC2]; memset(display, 0, sizeof(display)); 
  
                  strncpy(display, (char*)decrypt_string+ LICENSE_VERSION_LEN,  LICENSE_CUSTOMER_LEN);
                  ...
                  memcpy(&nodenumber, (char*)decrypt_string+ LICENSE_VERSION_LEN + LICENSE_CUSTOMER_LEN ,  sizeof(int));
                  printf("Licensed for :  %d node(s)\n",  nodenumber);
                  memcpy(&packageinstalled, (char*)decrypt_string+ LICENSE_VERSION_LEN + LICENSE_CUSTOMER_LEN + LICENSE_NODENUM_LEN + LICENSE_EXPIRE_LEN,  sizeof(int));
                  switch(packageinstalled)
                  {
                      case LICENSE_PACKAGE_ENT:
                          printf("Licensed Package : Enterprise\n");
                      break;
                      case LICENSE_PACKAGE_ADV:
                          printf("Licensed Package : Enterprise Advanced\n");
                      break;
                      default:
                          printf("Licensed Package : UNKNOWN\n");
                  } 
                  memcpy(&installtype, (char*)decrypt_string+ LICENSE_VERSION_LEN + LICENSE_CUSTOMER_LEN + LICENSE_NODENUM_LEN + LICENSE_EXPIRE_LEN + LICENSE_PACKAGE_INSTALLED,  sizeof(int));
                  switch(installtype) 
                  {
                      case LICENSE_TYPE_DEMO:
                         printf("License Type : Demo\n");
                      break;
                      case LICENSE_TYPE_POC:
                         printf("License Type : POC\n");
                      break;
                      case LICENSE_TYPE_PRODUCT:
                         printf("License Type : Product\n");
                      break;
                      case LICENSE_TYPE_INTERNAL:
                          printf("License Type : Internal\n");
                      break;
                      default:
                          printf("License Type : UNKNOWN\n");
                  }

                  memcpy(&exday, (char*)decrypt_string+ LICENSE_VERSION_LEN + LICENSE_CUSTOMER_LEN + LICENSE_NODENUM_LEN,  sizeof(int));
                  char tmbuf[32];
                  ...
                  memcpy(&version, (char*)decrypt_string ,  sizeof(short));
              }
         }
         else
         {
