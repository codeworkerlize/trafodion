...
class CProcess
{
 ...
 public:
    ...
    typedef struct nidPid_s { int nid; int pid;
      bool operator==(const nidPid_s& nidPid) {
          if (nidPid.nid == this->nid && nidPid.pid == this->pid)
              return true;
         return false;
      }
    } nidPid_t;
    ...
    inline bool IsRecreated() { return Recreated; }
    inline void SetRecreated( bool isRecreated ) { Recreated = isRecreated; }
    ...
    void deathInterestRemove(int nid, int pid);
...
private:
    ...
    bool           Abended;              // true if process terminated abnormally(ABEND: ABnormal END)
    ...
    bool           Recreated;            // Process has been down and recreated by monitor
