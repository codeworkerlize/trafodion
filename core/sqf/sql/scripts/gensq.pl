...
my $EXIST_ENABLE_DOUBLE_ROUND_FIFTEEN_ENV = 0;
...
#    }
    ...
    # Read the ms.env to determine whether to add env "ENABLE_DOUBLE_ROUND_FIFTEEN"
    open (MSENV, "<$msenv") or die("unable to open $msenv");
    while ($line = <MSENV>) {
        if ($line =~ /ENABLE_DOUBLE_ROUND_FIFTEEN=.*/) {
           $EXIST_ENABLE_DOUBLE_ROUND_FIFTEEN_ENV = 1;
        }
    }
    close (MSENV);
    ...
    if ($EXIST_ENABLE_DOUBLE_ROUND_FIFTEEN_ENV == 0) {
        print ETC "ENABLE_DOUBLE_ROUND_FIFTEEN=1\n";
    }
