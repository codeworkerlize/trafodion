...
-- Allows you to create up to 2 tenants without having to set oversubscribe cqds
upsert into TRAFODION."_MD_".DEFAULTS
     values
     ('TENANT_OVERSUBSCRIBE_ERR_NODE_RATIO', '4.1', 'inserted during seabase regressions run', 0);
upsert into TRAFODION."_MD_".DEFAULTS
     values
     ('TENANT_OVERSUBSCRIBE_WARN_NODE_RATIO', '4.1', 'inserted during seabase regressions run', 0);

initialize trafodion, create xdc metadata; 
initialize trafodion, create partition tables; 
create shared schema trafodion.sch namespace '' no encryption;

initialize trafodion, upgrade tenant usage;
...
-- namespace for regressions
create trafodion namespace 'regr' if not exists;
create trafodion namespace '1500000' if not exists;

-- create lob metadata for V2 lobs
