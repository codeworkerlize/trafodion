 ...
 *
 */
public class UserPrivTest {
	public static SentryPolicyService sentryService = null;

	public static void main(String[] args) {
		if(args.length < 2){
			System.out.println("You need to specify the user name and table names as input");
			System.out.println("");
			System.out.println("Usage : UserPrivTest <user name> <tablename> [<sentry site path>");
			return;
		}
		try {
			...
			testSentry(userName, sentrySiteConfPath);
			parseSentryPrivileges(userName, tableName, sentrySiteConfPath);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	
	public static void parseSentryPrivileges(String userName, String tableName, String sentrySiteConfPath)
			throws Exception {

		sentryService = SentryPolicyService.getInstance(sentrySiteConfPath);
		EsgynDBPrivilege privilege = sentryService.getPrivBitMapByUserName(userName, tableName);
	...
	public static void testSentry(String userName, String sentrySiteConfPath)
			throws Exception {
		sentryService = SentryPolicyService.getInstance(sentrySiteConfPath);

		User user = new User(userName);
		List<TSentryRole> roles = sentryService.listAllRoles(user);
