...
// ****************************************************************************
// function: authEventExists
//
// This function searches for a specific event the current list
//
// DB_SECUITY_EVENTID - is the event ID to search for
// ****************************************************************************
bool authEventExists (
  std::vector<AuthEvent> & authEvents,
  DB_SECURITY_EVENTID eventID)
{
   vector<AuthEvent>::iterator it;
   for (it=authEvents.begin(); it!= authEvents.end(); ++it)
   {
      AuthEvent authEvent  = *it;
      if (authEvent.getEventID() == eventID)
        return true;
   }
   return false;
}
...
void authInitEventLog()
{
  ...
  snprintf( log_name_suffix, log_name_suffix_len, ".%s.%d.log", my_hostname, my_nid );
  CommonLogger::instance().initLog4cplus("log4cplus.trafodion.auth.config",log_name_suffix);
}
...
std::string getAuthOutcome(AUTH_OUTCOME outcome, int32_t error)
{
   ...
   switch (outcome)
   {
      ...
      case AUTH_TYPE_INCORRECT:
         char buf[100];
         sprintf (buf, "Unexpected authorization type detected, error %d", error);
         outcomeDesc = buf;
         break;
      case AUTH_NO_PASSWORD:
         ...
         break;
      case AUTH_REJECTED:
         outcomeDesc = "Invalid username/groupname or password";
         break;
      case AUTH_FAILED:
         outcomeDesc = "Unexpected error returned from LDAP";
         break;
      case AUTH_TENANT_INVALID:
         outcomeDesc = "Tenant is not valid";
         break;
       case AUTH_NO_TENANT:
         outcomeDesc = "Tenant ESGYNDB not allowed";
         break;
       case AUTH_USER_REGISTERED:
         outcomeDesc = "User has been automatically registered";
         break;
       case AUTH_PASSWORD_WILL_EXPIRE:
         sprintf (buf, "User password will expire in %d days", error);
         outcomeDesc = buf;
         break;
       case AUTH_PASSWORD_EXPIRE:
         outcomeDesc = "User password has expired";
         break;
       case AUTH_PASSWORD_GRACE_COUNT:
           outcomeDesc = "User password has expired,using grace login";
           break;
       case AUTH_PASSWORD_ERROR_COUNT:
         outcomeDesc = "User password input error times exceed maximum error times, must change the password!";
         break;
       case AUTH_NO_LICENSE:
         outcomeDesc = "The module LOCAL-AUTH is not supported in this database version.";
         break;
      default:
...
void AuthEvent::logAuthEvent()
{
    ...
    snprintf(buf, MAX_EVENT_MSG_SIZE, "  NID %u PID %u TID %d txID  %s", 
            my_nid, my_pid, syscall(SYS_gettid), eventText_.c_str());
