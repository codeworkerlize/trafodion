...
// LCOV_EXCL_STOP
// if changing any of these default values, reflect the changes in LdapHelper.java equivalent
...
const string DEFAULT_GROUP_OBJECTCLASS  = "groupOfNames";
const string DEFAULT_GROUP_MEMBER_ATTR  = "member";
const string DEFAULT_GROUPNAME_ATTR     = "cn";
...
LDAPConfigFileErrorCode returnCode = LDAPConfigFile_CantOpenFile;
   ...
   try
   {
      ...
      //
      // Need to reserve, somehow sectionConfig vector doesn't get allotted
      config.sectionConfig.reserve(MAX_CONFIG_SECTIONS);
...
struct LDAPHostConfig sectionConfig;


   if (config.sectionConfig.size() == 0)
      return Check_MissingHostName;

   if (config.section.size() != 0 &&
       config.section.size() != config.sectionConfig.size())
      return Check_MissingHostName;

   for (int i = 0; i < config.sectionConfig.size(); i++)
   {
      sectionConfig = config.sectionConfig[i];
      if (sectionConfig.sectionRead)
      {
         if (sectionConfig.hostName.size() == 0)
            return Check_MissingHostName;

         if (sectionConfig.uniqueIdentifier.size() == 0)
            return Check_MissingUniqueIdentifier;
      }
   }

//
// If only a section was defined and no default section was
// specified in the defaults section, change the default to the first
// section.
//

   if (!defaultSectionLineRead)
   {
      config.defaultSection = config.section[0];
      config.defaultSectionNumber = 0;
   }
...
vector<string>::iterator it;
      it = find(config.section.begin(), config.section.end(), config.defaultSection);
      if (it != config.section.end())
         config.defaultSectionNumber = it - config.section.begin();
      else
         return Check_MissingSection;
   }

// If there were no sections, mark the first section as read.

   if (config.section.size() == 0)
      config.sectionConfig[0].sectionRead = true;
...

{
   ...
   defaultSectionLineRead = false;
   config.defaultSection = "local";
   config.defaultSectionNumber = 0;
   config.section.clear();
   config.sectionConfig.clear();
...

{
   ...
   hostConfig.searchGroupBase.clear();
   hostConfig.searchGroupCustomFilter.clear();
   hostConfig.searchGroupObjectClass = DEFAULT_GROUP_OBJECTCLASS;
   hostConfig.searchGroupMemberAttr = DEFAULT_GROUP_MEMBER_ATTR;
   hostConfig.searchGroupNameAttr = DEFAULT_GROUPNAME_ATTR;
...
LDAPHostConfig currentDefinition;

   initializeLDAPHost(currentDefinition);
...
// than once, we use the last value.
   ...
   while (fgets(inBuf,sizeof(inBuf),fp) != NULL)
   {
      ...
      if (readingDefaultLines)
         status = parseDefaultsLine(config,currentDefinition,ptr);
      else
 ...
 //
 // Push the last section read
 //

   if (currentDefinition.sectionRead == true)
   {
      config.sectionConfig.push_back(currentDefinition);
      initializeLDAPHost(currentDefinition);
   }
...
char *valuePtr = NULL;
   ...
   if (icmpPrefix(inBuf,PREFIX_DEFAULT_SECTION))
   {
      ...
      config.defaultSection = valuePtr;
...
// Section: line.
   return parseSectionLine(config,configLDAP,inBuf);
...
// Attributes used to manage ldap group searches
#define PREFIX_SEARCH_GROUP_BASE       "ldapsearchgroupbase:"
#define PREFIX_SEARCH_GROUP_CUSTOM_FILTER   "ldapsearchgroupcustomfilter:"
#define PREFIX_SEARCH_GROUP_OBJECTCLASS     "ldapsearchgroupobjectclass:"
#define PREFIX_SEARCH_GROUP_MEMBER_ATTR     "ldapsearchgroupmemberattr:"
#define PREFIX_SEARCH_GROUPNAME_ATTR   "ldapsearchgroupnameattr:"
...
//
   ...
   if (icmpPrefix(inBuf,PREFIX_SEARCH_GROUP_BASE))
   {
      configLDAP.searchGroupBase = parseStringValue(inBuf,PREFIX_SEARCH_GROUP_BASE);
      return Parse_Next;
   }

   if (icmpPrefix(inBuf,PREFIX_SEARCH_GROUP_CUSTOM_FILTER))
   {
      configLDAP.searchGroupCustomFilter = parseStringValue(inBuf,PREFIX_SEARCH_GROUP_CUSTOM_FILTER);
      return Parse_Next;
   }

   if (icmpPrefix(inBuf,PREFIX_SEARCH_GROUP_OBJECTCLASS))
   {
      configLDAP.searchGroupObjectClass = parseStringValue(inBuf,PREFIX_SEARCH_GROUP_OBJECTCLASS);
      if (configLDAP.searchGroupObjectClass.empty())
         configLDAP.searchGroupObjectClass = DEFAULT_GROUP_OBJECTCLASS;
      return Parse_Next;
   }

   if (icmpPrefix(inBuf,PREFIX_SEARCH_GROUP_MEMBER_ATTR))
   {
      configLDAP.searchGroupMemberAttr = parseStringValue(inBuf,PREFIX_SEARCH_GROUP_MEMBER_ATTR);
      if (configLDAP.searchGroupMemberAttr.empty())
         configLDAP.searchGroupMemberAttr = DEFAULT_GROUP_MEMBER_ATTR;
      return Parse_Next;
   }

   if (icmpPrefix(inBuf,PREFIX_SEARCH_GROUPNAME_ATTR))
   {
      configLDAP.searchGroupNameAttr = parseStringValue(inBuf,PREFIX_SEARCH_GROUPNAME_ATTR);
      if (configLDAP.searchGroupNameAttr.empty())
         configLDAP.searchGroupNameAttr = DEFAULT_GROUPNAME_ATTR;
      return Parse_Next;
   }
...
//
// Section name that needs to be processed
//

//
// Check if this is the first section to be read
// else push the contents of the section already read
//
   if (configLDAP.sectionRead == true)
   {
      config.sectionConfig.push_back(configLDAP);
      initializeLDAPHost(configLDAP);
   }
...
char *valuePtr = parseStringValue(inBuf,PREFIX_SECTION);
   ...
   config.section.push_back(valuePtr);
   configLDAP.sectionRead = true;
   return Parse_SwitchSection;
}
