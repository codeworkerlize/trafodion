...
{ 
    ...
    int key_byte_len = RSA_size(EVP_PKEY_get1_RSA(cipher_key));
...
extern "C" int base64_decrypt_password(char *password, int length)
{
    if (password == NULL || length < 0)
        return SECMXO_DECRYPTION_ERROR;

    // check if password is a user token
    if ((password[0] == USERTOKEN_ID_1) && (password[1] == USERTOKEN_ID_2))
        return 0;
    ...
    {
        tmpPwd[j]=((char)table[password[i]])<<2 | (((char)table[password[i+1]])>>4);
        tmpPwd[j+1]=(((char)table[password[i+1]])<<4) | (((char)table[password[i+2]])>>2);
        tmpPwd[j+2]=(((char)table[password[i+2]])<<6) | ((char)table[password[i+3]]);
    }
    strncpy(password, tmpPwd, len);
    password[str_len] = '\0';
