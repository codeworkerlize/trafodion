...
void displayContext(const Context &context) { context.print(); }
...
NABoolean RelExpr::preventPushDownPartReq(const ReqdPhysicalProperty *rppForMe, const EstLogPropSharedPtr &inLogProp) {
  if (getArity() == 0) {
    const CascadesPlan *myPlan = (*CURRSTMT_OPTGLOBALS->memo)[getGroupId()]->getFirstPlan();
    if (myPlan AND myPlan->succeededInCurrentPass()
            AND((getGroupAttr()->outputLogProp(inLogProp))->getResultCardinality().value() <
  ...
  for (int i = 0; i < 2; i++) {
    ...
    if (childPlan AND childPlan->succeededInCurrentPass()
            AND((child(i).outputLogProp(inLogProp))->getResultCardinality().value() <
                CURRSTMT_OPTDEFAULTS->numberOfRowsParallelThreshold())) {
      childIsPartitioned = (childPlan->getPhysicalProperty())->isPartitioned();
    }
...
RelExpr *RelExpr::optimize(NABoolean exceptionMode, Guidance *guidance, ReqdPhysicalProperty *rpp,
                           CostLimit *costLimit) {
  ...
  if (!exceptionMode && CURRSTMT_OPTDEFAULTS->optLevel() != OptDefaults::MINIMUM) {
  ...
  if ((CmpCommon::getDefault(COMP_BOOL_9) == DF_OFF)
          AND(child(0)->getOperatorType() == REL_DDL OR((RelRoot *)this)
                                                 ->accessOptions()
                                                 .isUpdateOrDelete() OR((RelRoot *)this)
                                                 ->isRootOfInternalRefresh()))
  ...
  CURRSTMT_OPTGLOBALS->forceParallelInsertSelect = (CmpCommon::getDefault(FORCE_PARALLEL_INSERT_SELECT) == DF_ON)
                                                       AND((RelRoot *)this)
                                                           ->accessOptions()
                                                           .isUpdateOrDelete();

  if (((CmpCommon::getDefault(COMP_BOOL_69) == DF_OFF)
           AND(child(0)->getOperatorType() == REL_DDL OR((RelRoot *)this)->isRootOfInternalRefresh())))
  ...
  Context *context =
      (*CURRSTMT_OPTGLOBALS->memo)[root]->shareContext(rpp, NULL, costLimit, NULL, (*GLOBAL_EMPTY_INPUT_LOGPROP));
  ...
  NABoolean skipShortOptimizationPass = (!exceptionMode && !CURRSTMT_OPTDEFAULTS->needShortPassOptimization() &&
                                         CURRSTMT_OPTDEFAULTS->optLevel() != OptDefaults::MINIMUM);

  if (CmpCommon::getDefault(FORCE_PASS_ONE) == DF_ON) skipShortOptimizationPass = FALSE;
  ...
  try {
    while (GlobalRuleSet->nextPass()) {
      if (CURRSTMT_OPTDEFAULTS->compileTimeMonitor()) (*CURRSTMT_OPTGLOBALS->cascadesPassMonitor).enter();

      if (CURRSTMT_OPTDEFAULTS->optimizerPruning() AND GlobalRuleSet->getCurrentPassNumber() == 2) {
        CostScalar forcedPass2costLimit = ActiveSchemaDB()->getDefaults().getAsDouble(OPH_PRUNING_PASS2_COST_LIMIT);
      ...
      CURRSTMT_OPTGLOBALS->task_list->push(new (CmpCommon::statementHeap())
                                               OptimizeGroupTask(context, guidance, FALSE, 0, ++stride));
      ...
      while (NOT CURRSTMT_OPTGLOBALS->task_list->empty()) {
        ...
        CascadesTask::cascadesTaskTypeEnum next_task_type = next_task->taskType();
      ...
      //
      if (NOT GlobalRuleSet->inLastPass()) context->setPreviousSolution();

      if (CURRSTMT_OPTDEFAULTS->compileTimeMonitor()) (*CURRSTMT_OPTGLOBALS->cascadesPassMonitor).exit();

      if (printCounters) {
        char string[200];
        printf("\n");
        *CmpCommon::diags() << DgSqlCode(arkcmpOptimizerCountersWarning);
        sprintf(string, "pass %d complete:\n", GlobalRuleSet->getCurrentPassNumber());
        report(string);
        *CmpCommon::diags() << DgString0(string);
        sprintf(string, " %d groups, %d tasks, %d rules,\n", CURRSTMT_OPTGLOBALS->memo->getCountOfUsedGroups(),
                task_count, GlobalRuleSet->getRuleApplCount());
        report(string);
        *CmpCommon::diags() << DgString1(string);
        sprintf(string, "%d groups merged, %d expr. cleaned up, %d tasks pruned\n",
                CURRSTMT_OPTGLOBALS->group_merge_count, CURRSTMT_OPTGLOBALS->garbage_expr_count,
                CURRSTMT_OPTGLOBALS->pruned_tasks_count);
        report(string);
        *CmpCommon::diags() << DgString2(string);
        sprintf(string, "%d/%d/%d/%d log/phys/plans/dupl expressions in CascadesMemo\n",
                CURRSTMT_OPTGLOBALS->logical_expr_count, CURRSTMT_OPTGLOBALS->physical_expr_count,
                CURRSTMT_OPTGLOBALS->plans_count, CURRSTMT_OPTGLOBALS->duplicate_expr_count);
        report(string);
        *CmpCommon::diags() << DgString3(string);
#ifdef _DEBUG
        sprintf(string, "%d asm hits and %d asm misses\n", CURRSTMT_OPTGLOBALS->asm_count,
      ...
      if (context->getSolution() && (CmpCommon::getDefault(NSK_DBG) == DF_ON)) {
        ...
        if ((passNumber == 1 && CmpCommon::getDefault(NSK_DBG_SHOW_PASS1_PLAN) == DF_ON)
          ...
          CURRCONTEXT_OPTDEBUG->showTree(context->getSolution()->getPhysicalExpr(), context->getSolution(), "  ");
    ...
    if (CURRSTMT_OPTDEFAULTS->isJoinToTSJRuleNeededOnPass1()) {
      GlobalRuleSet->disable(JoinToTSJRuleNumber, GlobalRuleSet->getFirstPassNumber(),
    ...
    if (currentPass == 1) {
      ...
      msg =
          "SQL compiler : Pass one optimization failed. "
  ...
  if (didNotCompleteFinalOptimizationPass) {
    ...
    if (plan == NULL) {
      ...
      if (NOT(skipShortOptimizationPass && GlobalRuleSet->getCurrentPassNumber() == 2)) {
        plan = context->bindSolutionTree(TRUE);  // TRUE == use prevSolution_
      }
    }
  }
  ...
  if (compilation_was_not_successful && !CmpCommon::diags()->getNumber(DgSqlCode::ERROR_)) {
    // QSTUFF
    ...
    if (ActiveControlDB()->getRequiredShape() && ActiveControlDB()->getRequiredShape()->getShape() &&
      ...
      const LIST(const NAString *) *reasonList = CmpCommon::statement()->getDetailsOnRefusedRequirements();

      // add up to 5 possible reasons to the diags area
      for (CollIndex i = 0; i < reasonList->entries() && i < 5; i++)
        *CmpCommon::diags() << DgSqlCode(-11153) << DgString0((*reasonList)[i]->data());
  ...
  // searching for an acceptable plan, e.g. generated when searching
  // for a suitable index  to satisfy an order by clause on a stream
  if (plan && context->getGroupAttr()->isStream()) clearFailedPlanWarningsForStream();
  // QSTUFF
...
Context::Context(CascadesGroupId groupId, const ReqdPhysicalProperty *const reqdPhys,
                 const InputPhysicalProperty *const inputPhys, const EstLogPropSharedPtr &inputLogProp)
    : reqdPhys_(reqdPhys),
      inputPhys_(inputPhys),
      costLimit_(NULL),
      groupId_(groupId),
      doneInPass_(-1),
      outstanding_(0),
      currentAncestor_(NULL),
      duplicateOf_(NULL),
      predecessorTask_(NULL),
      costLimitExceeded_(FALSE),
      currentPlanExceededCostLimit_(FALSE),
...
void Context::print(FILE *f, const char *prefix, const char *suffix) const {
  ...
  if (reqdPhys_ != NULL) reqdPhys_->print(f, CONCAT(prefix, " reqd phys: "));
  if (costLimit_) fprintf(f, CONCAT(prefix, " cost limit: %g"), costLimit_->getValue(getReqdPhysicalProperty()));
...
NAString Context::getRequirementsString() const {
  ...
  return NAString(thisptr) + NAString("Cost limit: ") + getCostString() + ", RPP: " + getRPPString() + IPPString +
         ", ILP: " + getILPString();
}
...
NAString Context::getCostString() const {
  ...
  if (getCostLimit()) {
    ...
    sprintf(costLimitStr, "%g", getCostLimit()->getValue(reqdPhys_));
...
NAString Context::getRPPString() const {
  ...
  if (reqdPhys_ != NULL) {
    ...
    if (reqdPhys_->getArrangedCols() != NULL) {
      ...
      ((ValueIdSet *)(reqdPhys_->getArrangedCols()))->unparse(propString, DEFAULT_PHASE, EXPLAIN_FORMAT);
      propString += ")";
    }
    if (reqdPhys_->getSortKey() != NULL) {
      propString += ", sort order (";
      ((ValueIdList *)(reqdPhys_->getSortKey()))->unparse(propString, DEFAULT_PHASE, EXPLAIN_FORMAT);
      propString += ")";
    }
    ...
    if ((reqdPhys_->getArrangedCols() != NULL) OR(reqdPhys_->getSortKey() != NULL)) {
    ...
    if (reqdPhys_->getLogicalPartRequirement()) {
      ...
      if (reqdPhys_->getLogicalPartRequirement()->getNumClientsReq() != ANY_NUMBER_OF_PARTITIONS) {
        char ncr[20];
        sprintf(ncr, "%d client(s) ", reqdPhys_->getLogicalPartRequirement()->getNumClientsReq());
        propString += ncr;
      }
      if (reqdPhys_->getLogicalPartRequirement()->getMustUsePapa()) propString += "must use PAPA ";
      if (reqdPhys_->getLogicalPartRequirement()->getLogReq())
        propString += reqdPhys_->getLogicalPartRequirement()->getLogReq()->getText();
    }
    ...
    if (reqdPhys_->getPartitioningRequirement() != NULL) {
      PartitioningRequirement *partReq = reqdPhys_->getPartitioningRequirement();

      if (partReq->isRequirementFuzzy())
        propString += (partReq->castToFuzzyPartitioningRequirement()->getSkewProperty()).getText();
      else if (partReq->isRequirementSkewed())
        propString += (partReq->castToRequireSkewed()->getSkewProperty()).getText();
    }
    ...
    if (pdr != NULL) {
      ...
      } else if (pdr->castToPushDownColocationRequirement()) {
...
NAString Context::getIPPString() const {
  ...
  if (inputPhys_ != NULL) {
    ...
    if (inputPhys_->getNjOuterOrder() != NULL) {
      ...
      ((ValueIdList *)(inputPhys_->getNjOuterOrder()))->unparse(propString, DEFAULT_PHASE, EXPLAIN_FORMAT);
      propString += ")";
    }
...
NABoolean Context::findBestSolution() {
  ...
  // ---------------------------------------------------------------------
  if (optimizedInCurrentPass()) return (TRUE);

  // If plan stealing is disabled, then we cannot allow this method to
  // do its job. Return FALSE now.
  if (CmpCommon::getDefault(PLAN_STEALING) == DF_OFF) return (FALSE);
      ...
      switch (comp) {
        ...
        case MORE:
          ...
          if (otherContext->hasOptimalSolution()) {
            ...
            // -----------------------------------------------------
            if (costLimit_ AND(
                    costLimit_->compareWithPlanCost((CascadesPlan *)otherContext->getSolution(), reqdPhys_) == LESS)) {
              ...
              // is better than ours
              if (CURRSTMT_OPTDEFAULTS->OPHuseFailedPlanCost() AND satisfied(otherContext->solution_)
                      AND(solution_ == NULL OR NOT solution_->succeededInCurrentPass())) {
            ...
            // -----------------------------------------------------
              ...
              if (solution_ == NULL OR NOT solution_->succeededInCurrentPass()) {
          ...
          // ---------------------------------------------------------
            ...
            //  to get "more"!)
            // -----------------------------------------------------
            if (costLimit_ AND otherContext->costLimit_ AND(otherContext->doneInPass_ ==
                                                            GlobalRuleSet->getCurrentPassNumber())
                    AND(costLimit_->compareCostLimits(otherContext->costLimit_, getReqdPhysicalProperty()) != MORE)) {
              costLimitExceeded_ = TRUE;
        ...
        case UNDEFINED:
          ...
          if ((NOT reqdPhys_->getMustMatch())OR(reqdPhys_->getMustMatch()->getArity() ==
                                                0))  // did little
                                                     ...
                                                     // arity and allow stealing if its zero)
          {
            for (j = 0; j < numOtherPlans; j++) {
              ...
              if (satisfied(plan)) addCandidatePlan(plan);
            }
          }
...
NABoolean Context::reoptimizeExprAndPlans() {
  ...
  if (grAttr->getNumBaseTables() == 1) {
    EstLogPropSharedPtr outputLogProp = grAttr->outputLogProp(getInputLogProp());

    // check if the group is "small" enough
    CostScalar groupRowCount = MIN_ONE(outputLogProp->getResultCardinality().value());
    if (groupRowCount.value() < CURRSTMT_OPTDEFAULTS->numberOfRowsParallelThreshold()) {
      ...
      if (rpp) {
        const PartitioningRequirement *pr = rpp->getPartitioningRequirement();
        ...
        if (pr) {
          if ((pr->getCountOfPartitions() > 1) AND NOT pr->isRequirementReplicateViaBroadcast()
                  AND NOT pr->isRequirementReplicateNoBroadcast()) {
            return FALSE;
          }
        } else {
          ...
          if (lpr) {
            ...
            if ((logreq->getCountOfPartitions() > 1) AND NOT logreq->isRequirementReplicateViaBroadcast()
                    AND NOT logreq->isRequirementReplicateNoBroadcast()) {
              return FALSE;
            }
          }
...
NABoolean Context::satisfied(const CascadesPlan *const plan) const {
  ...
  {
    NABoolean satisfied =
        (reqdPhys_->satisfied(getInputLogProp(), plan->getPhysicalExpr(), plan->getPhysicalProperty()));

    if (satisfied && plan->getPhysicalExpr()->getOperatorType() == REL_EXCHANGE) {
      const PhysicalProperty *sppForChild = getPhysicalPropertyOfSolutionForChild(0);
      ...
      if (sppForChild && !sppForChild->executeInDP2() && reqdPhys_->getNoEspExchangeRequirement())
  ...
  if (getInputLogProp()->compareEstLogProp(other.getInputLogProp()) == SAME) {
    ...
    if (inputPhys_ != other.inputPhys_) {
      if ((inputPhys_ != NULL) AND(other.inputPhys_ != NULL)) {
        result = combine_compare_results(result, inputPhys_->compareInputPhysicalProperties(*(other.inputPhys_)));
...
RelExpr *Context::bindSolutionTree(NABoolean getPrevSolution) const {
  ...
  const CascadesPlan *plan = (getPrevSolution) ? getPreviousSolution() : getSolution();
  ...
  if (plan != NULL) {
    ...
    if (NOT getPrevSolution AND NOT plan->succeededInCurrentPass()) return NULL;
    ...
    CostScalar numProbes =
        ((iLP->getResultCardinality() > (CostScalar)1) ? iLP->getResultCardinality() : (CostScalar)1);
    result->setInputCardinality(numProbes);
    result->setEstRowsUsed(
        (*CURRSTMT_OPTGLOBALS->memo)[getGroupId()]->getGroupAttr()->outputLogProp(iLP)->getResultCardinality() /
        numProbes);
    ...
    for (int i = 0; i < nc; i++) {
      ...
      if (plan->getContextForChild(i)) {
        ...
        if (result->child(i).getMode() == ExprGroupId::BINDING) result->child(i).releaseBinding();

        result->child(i) = plan->getContextForChild(i)->bindSolutionTree(getPrevSolution);
...
void Context::setCostLimit(CostLimit *cl) {
  ...
  if (costLimit_ != cl) {
    if (costLimit_ != NULL) delete costLimit_;
...
//
//==============================================================================
void Context::setSolution(CascadesPlan *plan, NABoolean checkAndAdjustCostLimit) {
  if (solution_ == plan) {
    return;
  }
  ...
  if (plan->exprOccursInChildTree(plan->getPhysicalExpr(), CURRSTMT_OPTDEFAULTS->maxDepthToCheckForCyclicPlan())) {
  ...
  // help pruning)
  //--------------------------------------------------------------------
  CMPASSERT(plan->getPhysicalExpr()->isPhysical() AND plan->getPhysicalProperty() != NULL);
  ...
  CMPASSERT(costLimit_ == NULL OR costLimit_->compareWithPlanCost(plan, reqdPhys_) != LESS);
  //---------------------------------------
...
NABoolean Context::requiresOrder() const {
  return (reqdPhys_ AND((reqdPhys_->getSortKey() AND reqdPhys_->getSortKey()->entries() > 0)
                            OR(reqdPhys_->getArrangedCols() AND reqdPhys_->getArrangedCols()->entries() > 0)));
}

NABoolean Context::requiresPartitioning() const { return (reqdPhys_ AND reqdPhys_->getPartitioningRequirement()); }
...
void Context::addCandidatePlan(CascadesPlan *plan) {
  ...
  if (CURRSTMT_OPTDEFAULTS->OPHuseCandidatePlans()) candidates_.insert(plan);

  if (plan->getPhysicalExpr()->isPhysical() AND plan->getPhysicalProperty() != NULL AND plan->getRollUpCost() !=
      NULL AND(solution_ == NULL OR solution_->getRollUpCost()->compareCosts(*plan->getRollUpCost(), reqdPhys_) ==
               MORE)) {
    ...
    if (plan->getCreationPassNumber() == GlobalRuleSet->getCurrentPassNumber()) {
...
const RelExpr *Context::getPhysicalExprOfSolutionForChild(int childIndex) const {
  if (getPlan() AND getPlan()
          ...
          ->getSolution())
    return getPlan()->getContextForChild(childIndex)->getSolution()->getPhysicalExpr();
  else
    return NULL;
...
const PhysicalProperty *Context::getPhysicalPropertyOfSolutionForChild(int childIndex) const {
  if (getPlan() AND getPlan()
          ...
          ->getContextForChild(childIndex)
          ->getSolution())
    return getPlan()->getContextForChild(childIndex)->getSolution()->getPhysicalProperty();
  else
    return NULL;
...
const Cost *Context::getCostOfSolutionForChild(int childIndex) const {
  if (getPlan() AND getPlan()
          ...
          ->getContextForChild(childIndex)
          ->getSolution())
    return getPlan()->getContextForChild(childIndex)->getSolution()->getRollUpCost();
  else
    return NULL;
...
const RelExpr *Context::getPhysicalExprOfSolutionForGrandChild(int childIndex, int grandChildIndex) const {
  if (getPlan() AND getPlan()
    ...
    const Context *grandChildContext =
        getPlan()->getContextForChild(childIndex)->getSolution()->getContextForChild(grandChildIndex);

    if (grandChildContext && grandChildContext->getSolution())
      return grandChildContext->getSolution()->getPhysicalExpr();
    else
      return NULL;
  } else
    return NULL;
...
const PhysicalProperty *Context::getPhysicalPropertyOfSolutionForGrandChild(int childIndex,
  ...
  if (getPlan() AND getPlan()
    ...
    if (grandChildContext && grandChildContext->getSolution())
      return grandChildContext->getSolution()->getPhysicalProperty();
    else
      return NULL;
...
const GroupAttributes *Context::getGroupAttr() const { return (*CURRSTMT_OPTGLOBALS->memo)[groupId_]->getGroupAttr(); }
...
CascadesPlan::~CascadesPlan() {
  ...
  delete rollUpCost_;
  delete physProp_;
  for (CollIndex index = 0; index < childContexts_.entries(); index++) delete childContexts_[index];
  CURRSTMT_OPTGLOBALS->cascadesPlanCounter->decrementCounter();

}  // CascadesPlan::~CascadesPlan

void CascadesPlan::setOperatorCost(Cost *cost) {
  if (operatorCost_) delete operatorCost_;
  operatorCost_ = cost;
}

void CascadesPlan::setRollUpCost(Cost *cost) {
  if (rollUpCost_) delete rollUpCost_;
  rollUpCost_ = cost;
}
  ...
  if (newExpr) {
    ...
    for (int i = 0; i < arity; i++) {
      ...
      if (cc AND cc->getSolution()) {
        ...
        //  the same expression)
        if (cc->getSolution()->getPhysicalExpr() == newExpr) return TRUE;
        ...
        if (maxDepth > 0 OR childIsInSameGroup) {
          if (cc->getSolution()->exprOccursInChildTree(newExpr, (childIsInSameGroup ? maxDepth : maxDepth - 1)))
            return TRUE;
        }
      }
    }
...
void PlanWorkSpace::storeChildContext(int childIndex, int planNumber, Context *childContext) {
  ...
  if (latestPlan_ != -1) prevPlan_ = latestPlan_;
...
NABoolean PlanWorkSpace::getScanLeaf(int childNumber, int planNumber, FileScan *&scanLeaf) const {
  Context *childContext = getChildContext(childNumber, planNumber);
  scanLeaf = NULL;
  int i = 0;

  while (++i < 20 /*guard against loops*/) {
    const CascadesPlan *plan = childContext ? childContext->getSolution() : NULL;
...
void PlanWorkSpace::initializeCost(Cost *operatorCost) {
  if (operatorCost_) delete operatorCost_;
  if (partialPlanCost_) delete partialPlanCost_;
...
//
//==============================================================================
void PlanWorkSpace::setPartialPlanCost(Cost *newCost) {
  if (partialPlanCost_ != 0) {
    delete partialPlanCost_;
  }
...
//
//==============================================================================
void PlanWorkSpace::setKnownChildrenCost(Cost *newCost) {
  if (knownChildrenCost_ != 0) {
    delete knownChildrenCost_;
  }
...
void PlanWorkSpace::updateBestCost(Cost *cost, int planNumber) {
  ...
  const ReqdPhysicalProperty *const rpp = myContext_->getReqdPhysicalProperty();
  ...
  if (bestRollUpCostSoFar_ == NULL) {
    ...
    if (cost->compareCosts(*bestRollUpCostSoFar_, rpp) == LESS) {
      delete bestRollUpCostSoFar_;
      delete bestOperatorCostSoFar_;
      if (bestSynthPhysPropSoFar_ != NULL) delete bestSynthPhysPropSoFar_;
      bestRollUpCostSoFar_ = cost;
      bestOperatorCostSoFar_ = operatorCost_->duplicate();
      bestPlanSoFar_ = planNumber;
      bestPlanSoFarIsBMO_ = isBMO_;
...
NABoolean PlanWorkSpace::findOptimalSolution(int &planNo) {
  ...
  if (CURRSTMT_OPTDEFAULTS->OPHuseFailedPlanCost()) {
    ...
    const ReqdPhysicalProperty *const rpp = myContext_->getReqdPhysicalProperty();
    if (costLimit AND costLimit->compareWithCost(*bestRollUpCost, rpp) == LESS) myPlan->setExceededCostLimit();
  }
...
Cost *PlanWorkSpace::getCostOfPlan(int planNumber) {
  ...
  if (opArity == 0) {
    ...
    for (int childIndex = 0; childIndex < opArity; childIndex++) {
      ...
      if (NOT childContext->hasOptimalSolution()) return NULL;
    }
    ...
    if (opArity == 1) {
      Cost *childCost;
      cm->getChildCostForUnaryOp(op, myContext_, this, planNumber, childCost);
    ...
    if (opArity == 2) {
      Cost *leftChildCost;
      Cost *rightChildCost;
      cm->getChildCostsForBinaryOp(op, myContext_, this, planNumber, leftChildCost, rightChildCost);

      cost->computePlanPriority(op, myContext_, leftChildCost, rightChildCost, this, planNumber);
  ...
  //-----------------------------------------
  CostLimit *costLimit = myContext_->getCostLimit();
  if (NOT CURRSTMT_OPTDEFAULTS->OPHuseFailedPlanCost()
          AND costLimit AND costLimit->compareWithCost(*cost, myContext_->getReqdPhysicalProperty()) == LESS) {
...
void NestedJoinPlanWorkSpace::transferParallelismReqsToRG(RequirementGenerator &rg) {
  ...
  rg.addNumOfPartitions(tempChildNumPartsRequirement, tempChildNumPartsAllowedDeviation);
}
...
void CascadesTaskList::print(FILE *f, const char *prefix, const char *suffix) const {
  ...
  else  // task list with some tasks in it
  {
    ...
    fprintf(f, "%s OPEN %s (end)\n%s", LINE_STRING, LINE_STRING, suffix);
...
void CascadesTaskList::insertOptimizeExprTask(CascadesTask *newTask, CascadesTask *beforeTask) {
  ...
  while (t != NULL) {
    ...
    if (t->next_) nextContext = t->next_->getContext();
    ...
    if ((t->next_ == beforeTask) ||
        ((myContext == nextContext) && (t->next_) && (t->next_->taskType() == CascadesTask::OPTIMIZE_EXPR_TASK) &&
         (t->next_->getExpr()) && (t->next_->getExpr()->getPotential() > newTask->getExpr()->getPotential()))) {
...
CascadesBinding::CascadesBinding(CascadesGroupId group_no, RelExpr *pattern, CascadesBinding *parent,
                                 NABoolean incl_log, NABoolean incl_phys)
...
//<pb>
CascadesBinding::CascadesBinding(RelExpr *expr, RelExpr *pattern, CascadesBinding *parent, NABoolean incl_log,
                                 NABoolean incl_phys)
    : children_(CmpCommon::statementHeap()) {
  state_ = START_EXPR;
...
CascadesBinding::~CascadesBinding() {
  ...
  if (state_ == VALID_BINDING OR state_ == ALMOST_EXHAUSTED) {
    ...
    if (copiedExpr_ != NULL) delete copiedExpr_;
...
RelExpr *CascadesBinding::extract_expr() {
  ...
  if (state_ != VALID_BINDING AND state_ != ALMOST_EXHAUSTED) ABORT("no binding available");
  ...
  if (pattern_ != NULL AND pattern_->isCutOp()) {
    ...
    if (fixed_) acut->setExpr(curExpr_);
  ...
  else  // general invocation of extract_expr
  {
    ...
    for (int childIndex = 0; childIndex < arity; ++childIndex) {
      ...
      if (curExpr_->child(childIndex).getMode() == ExprGroupId::BINDING) {
        ...
        copiedExpr_ = result = curExpr_->copyTopNode(0, CmpCommon::statementHeap());
  ...
  if (logExpr_) result->setBlockStmt(logExpr_->isinBlockStmt());
...
void CascadesBinding::release_expr() {
  // ensure that binding is in a suitable state
  if (state_ != VALID_BINDING AND state_ != ALMOST_EXHAUSTED) ABORT("trying to release a nonexistent binding");
    ...
    switch (state_) {
      case START_GROUP:
        ...
        if (pattern_ == NULL) {
          ...
          if (state_ != START_GROUP) break;  // something happened
        }
        ...
        if (curExpr_ == NULL) {
          ...
          if (pattern_ != NULL AND pattern_->isCutOp() OR inclPhys_) {
      ...
      case START_EXPR:
        ...
        if (NOT(  // first, verify suitable log/phys property
                (inclLog_ AND curExpr_->isLogical())OR(inclPhys_ AND curExpr_->isPhysical()))
                OR  // second, verify pattern conformance
            // not usable if there is a pattern that
            // isn't a SubtreeOp and doesn't match
            (pattern_ != NULL AND NOT pattern_->isSubtreeOp() AND NOT curExpr_->patternMatch(*pattern_))) {
          ...
          if (childGroupId != INVALID_GROUP_ID) {
            ...
            if (pattern_) {
              childBinding = new (CmpCommon::statementHeap()) CascadesBinding(
                  ...
                  IFX pattern_ == NULL OR pattern_->isSubtreeOp() THENX NULL ELSEX(*pattern_)[childIndex].getPtr(),
                  this, inclLog_, inclPhys_);
            } else {
              ...
              childBinding = new (CmpCommon::statementHeap())
                  CascadesBinding((*curExpr_)[childIndex].getGroupId(), newPattern, this, inclLog_, inclPhys_);
            }
          } else {
            ...
            childBinding = new (CmpCommon::statementHeap()) CascadesBinding(
                ...
                IFX pattern_ == NULL OR pattern_->isSubtreeOp() THENX NULL ELSEX(*pattern_)[childIndex].getPtr(), this,
                inclLog_, inclPhys_);
          }
        ...
        for (childIndex = 0; childIndex < arity; ++childIndex)
          if (NOT children_[childIndex]->advance()) break;  // terminate this loop

        // check whether all children found a binding
        if (childIndex == arity)  // successful!
        {
          // ensure proper bindings for SubtreeOp
          if (pattern_ != NULL AND pattern_->isSubtreeOp() AND NOT((SubtreeOp *)pattern_)->all_bindings()) {
        ...
        for (childIndex = arity; --childIndex >= 0;) {
          ...
          // found one more binding
          {
            ...
            for (int other_childIndex = childIndex; ++other_childIndex < arity;) {
              ...
              delete childBinding;
              children_[other_childIndex] = new (CmpCommon::statementHeap()) CascadesBinding(
                  (*curExpr_)[other_childIndex].getGroupId(),
                  IFX pattern_ ==
                      NULL OR pattern_->isSubtreeOp() THENX NULL ELSEX(*pattern_)[other_childIndex].getPtr(),
                  this, inclLog_, inclPhys_);
              if (NOT children_[other_childIndex]->advance()) ABORT("internal error");  // earlier bindings!
            }  // reset all siblings to the right
      ...
      case EXPR_FINISHED:
        ...
        if (NOT fixed_ AND(curExpr_ = curExpr_->getNextInGroup()) != NULL) {
          state_ = START_EXPR;
          break;
        }  // if possible, move to the next expression
...
OptDefaults::OptDefaults(CollHeap *h) : heap_(h) {
  ...
  tenantUnitMemory_ = 34359738368;  // 32GB in bytes
  tenantUnitCores_ = 4;             // 4 cores
  tenantTotalCores_ = 0;
  ...
  minMaxINlistOpt_ = FALSE;
  numESPsPerQuery_ = 0;
}

OptDefaults::~OptDefaults() { CleanupCostVariables(); }
...
NABoolean OptDefaults::pruneByOptDefaults(Rule *rule, RelExpr *relExpr) {
  ...
  if (!OPHuseNiceContext_ && (optLevel_ != OptDefaults::MAXIMUM) &&
    ...
    OPHuseEnforcerPlanPromotion_ =
        OPHuseNiceContext_ AND(CmpCommon::getDefault(OPH_USE_ENFORCER_PLAN_PROMOTION) == DF_ON);
  }

  // No pruning is allowed in the first optimization pass
  if (GlobalRuleSet->getCurrentPassNumber() == 1) return FALSE;
  ...
  if (optLevel_ == OptDefaults::MINIMUM) return TRUE;

  if (optLevel_ != OptDefaults::MAXIMUM) return pruneByOptLevel(rule, relExpr);
...
int OptDefaults::getRulePriority(Rule *rule) {
  ...
  if (rule->getNumber() == JoinToTSJRuleNumber) return 2;
  if (GlobalRuleSet->getPassNRules(1)->contains(rule->getNumber())) return 1;
  if (rule->isTransformationRule()) return 2;
  return 3;
}
...
NABoolean OptDefaults::pruneByOptLevel(Rule *rule, RelExpr *relExpr) {
  ...
  if (queryComplexity_ < level1Threshold_ AND taskCount_ < level1SafetyNet_ AND memUsed < memUsageSafetyNet_)
  ...
  if (taskCount_ <= level1ImmunityLimit_ AND memUsed < memUsageSafetyNet_) return FALSE;
...
NABoolean OptDefaults::pruneByRProbability(Rule *rule, RelExpr *relExpr) {
  ...
  if (rulePriority == 2) {
    ...
    if (relExpr->getOperator().match(REL_ANY_JOIN) && ((Join *)relExpr)->isJoinFromPTRule()) pruneRate = 0;
  }

  if (rulePriority == 3) pruneRate = double(level1Constant2_) / 100;
  ...
  if (memUsed > memUsageSafetyNet_) pruneMemAdj = (double(2 * memUsed) / memUsageSafetyNet_ - 2);

  if (taskCount_ > level1SafetyNet_) pruneTaskAdj = (double(taskCount_) / level1SafetyNet_ - 1);
...
NABoolean OptDefaults::pruneByPotential(Rule *rule, RelExpr *relExpr) {
  ...
  if (memUsed > memUsageSafetyNet_) pruneMemFactor = (double(2 * memUsed) / memUsageSafetyNet_ - 2);

  if (taskCount_ > level1SafetyNet_) pruneTaskFactor = (double(taskCount_) / level1SafetyNet_ - 1);
  ...
  if (adjustment) {
    if (combinedPotentialThreshold > 2) combinedPotentialThreshold = 3;
  ...
  if (combinedPotential > combinedPotentialThreshold) prune = TRUE;
...
void OptDefaults::initializeCostInfo() {
  ...
  if (CmpCommon::getDefault(COMP_BOOL_155) == DF_ON) {
    ...
    v = "690.27";
    defs_->validateAndInsert("CPUCOST_SUBSET_OPEN", v,
                             FALSE);  // Curently, it is High (Not much Effect, since Scan)
...
double OptDefaults::computeRecommendedNumCPUs(double cpuResourceRequired) {
  ...
  if (n >= 0) {
    ...
    if (cpuResourceRequired > cpuResourceFor_n_CPUs) {
      double penalty = (ActiveSchemaDB()->getDefaults()).getAsDouble(MDOP_CPUS_PENALTY);

      double additionalCPUs = (cpuResourceRequired - cpuResourceFor_n_CPUs) / (workPerCPU * penalty);

      mDoPc = n + additionalCPUs;
    }
  }

  if (!canUseAllClusterResources()) mDoPc = MAXOF(mDoPc, getTotalCoresInTenant());

  return mDoPc;
}

// This routine adjusts the memory DoP number using the similar idea for CPU.
double OptDefaults::computeRecommendedNumCPUsForMemory(double memoryResourceRequired) {
  ...
  if (n >= 0) {
    ...
    if (memoryResourceRequired > totalMemoryFor_N_CPU) {
      double penalty = (ActiveSchemaDB()->getDefaults()).getAsDouble(MDOP_MEMORY_PENALTY);

      double additionalCPUs = (memoryResourceRequired - totalMemoryFor_N_CPU) / (memoryPerCPU * penalty);
...
void OptDefaults::computeNumESPsForQuery(const RequiredResources *requiredResources) {
  float numESPsPerNode;
  NADefaults &defs = ActiveSchemaDB()->getDefaults();
  if (((RequiredResources *)requiredResources)->getNumOfHiveTables() == 0)
    numESPsPerNode = defs.getNumOfESPsPerNodeInFloat();
  else {
    if (!canUseAllClusterResources())
      // Use the value for queries involving Hbase tables only
      numESPsPerNode = defs.getNumOfESPsPerNodeInFloat();
    else
      numESPsPerNode = defs.getAsDouble(HIVE_NUM_ESPS_PER_DATANODE);
  }

  numESPsPerQuery_ = ceil(numESPsPerNode) * CmpCommon::context()->getNumOfSMPs();
}
...
RequiredResources *OptDefaults::estimateRequiredResources(RelExpr *rootExpr) {
  ...
  if (CmpCommon::getDefault(ASG_FEATURE) == DF_ON) {
    ...
    if (requiredResources->getNumOfHiveTables() > 0) {
      memoryPerCPU_ = getDefaultAsDouble(HIVE_MEMORY_UNIT_ESP) * 1000000;
      workPerCPU_ = getDefaultAsDouble(HIVE_WORK_UNIT_ESP) * 1000000;
    } else {
      memoryPerCPU_ = getDefaultAsDouble(MEMORY_UNIT_ESP) * 1000000;
      workPerCPU_ = getDefaultAsDouble(WORK_UNIT_ESP) * 1000000;
    }

    tenantUnitMemory_ = ((double)NAWNODESET_MEMORY_GB_PER_SLICE) * GB_IN_BYTES;
    tenantUnitCores_ = NAWNODESET_CORES_PER_SLICE;

    const NAWNodeSet *availableNodes = CmpCommon::context()->getAvailableNodes();

    if (!canUseAllClusterResources())
      tenantTotalCores_ = availableNodes->getTenantSize() * tenantUnitCores_;
    else
      tenantTotalCores_ = 0;

    defaultDegreeOfParallelism_ = getDefaultAsLong(DEFAULT_DEGREE_OF_PARALLELISM);
    ...
    NABoolean optForCluster = !isAdaptiveSegForHadoop();

    totalNumberOfCPUs_ = defs.getTotalNumOfESPs(fakeEnv);

    if (optForCluster) {
      totalNumberOfNodes_ = CmpCommon::context()->getNumOfSMPs();
    } else {
      DCMPASSERT(CmpCommon::context()->getAvailableNodes());
      totalNumberOfNodes_ = CmpCommon::context()->getAvailableNodes()->getNumNodes();
    }
    ...
    if (!fakeEnv) {
      ...
      if (optForCluster) {
        ...
        adjustedDegreeOfParallelism_ = smallestFactor(totalNumberOfCPUs_);
      }
      ...
      if (useOperatorMaxForComputations) {
        ...
        cpuToConsider = MINOF(cpuToConsider, (operatorResourceFactor * maxOperatorCpuEstimate_));
      }
      ...
      if (optForCluster) {
        ...
        // cout << "dDop,resourceDoP,numCPUs=";
        // cout  << dDoP << "," << resourceDoP << "," << numCPUs << endl;
      ...
      if (requiredResources && requiredResources->getNumOfHiveTables() > 0) {
        CollIndex numOfNodes = totalNumberOfNodes_;
        double minESPsPerNode = 0;
        minESPsPerNode = getDefaultAsDouble(HIVE_MIN_NUM_ESPS_PER_DATANODE);

        if (!optForCluster) {
          DCMPASSERT(CmpCommon::context()->getAvailableNodes());
          numOfNodes = CmpCommon::context()->getAvailableNodes()->getTenantSize();
        }
      ...
      if (optForCluster)
        ...
        maximumDegreeOfParallelism_ = closestFactor(maxDoP, numCPUs, totalNumberOfNodes_);
...
void OptDefaults::initialize(RelExpr *rootExpr) {
  ...
  if ((!QueryAnalysis::Instance()->isAnalysisON()) && (optLevel_ == OptDefaults::MEDIUM_LOW))
    optLevel_ = OptDefaults::MEDIUM;

  tok = CmpCommon::getDefault(INDEX_ELIMINATION_LEVEL);
  if (tok == DF_MINIMUM OR(ActiveControlDB()
  ...
  if (QueryAnalysis::Instance() && QueryAnalysis::Instance()->multiJoinsUsed() && level1Threshold_ < 5120)
    level1Threshold_ = 5120;

  level1SafetyNet_ = defs_->getAsLong(OPTIMIZATION_LEVEL_1_SAFETY_NET);
  maxDepthToCheckForCyclicPlan_ = defs_->getAsLong(MAX_DEPTH_TO_CHECK_FOR_CYCLIC_PLAN);
  ...
  CmpCommon::context()->validateAvailableNodes();
  ...
  if ((rootExpr->getInliningInfo()).isUsedForMvLogging() == TRUE AND CmpCommon::getDefault(COMP_BOOL_93) == DF_ON)
  ...
  queryComplexity_ = rootExpr->calculateSubTreeComplexity(enableJoinToTSJRuleOnPass1_);
  ...
  // -----------------------------------------------------------------------
  optimizerPruning_ = (CmpCommon::getDefault(OPTIMIZER_PRUNING) == DF_ON)
                          AND NOT pushDownDP2Requested_ AND(numTables_ > 0)
                              AND CURRSTMT_OPTGLOBALS->pruningIsFeasible AND(
                                  queryComplexity_ > defs_->getAsDouble(OPH_PRUNING_COMPLEXITY_THRESHOLD));

  level1SafetyNetMultiple_ =
      !optimizerPruning_ ? 0.0 : CmpCommon::getDefaultNumeric(OPTIMIZATION_LEVEL_1_SAFETY_NET_MULTIPLE);

  if (additiveResourceCosting_ AND optimizerPruning_ AND queryComplexity_ < 5120) optimizerPruning_ = FALSE;

  OPHpruneWhenCLExceeded_ = optimizerPruning_ AND(CmpCommon::getDefault(OPH_PRUNE_WHEN_COST_LIMIT_EXCEEDED) == DF_ON);
  OPHuseCandidatePlans_ = optimizerPruning_ AND(CmpCommon::getDefault(OPH_USE_CANDIDATE_PLANS) == DF_ON);
  ...
  OPHreduceCLfromPass1Solution_ =
      optimizerPruning_ AND(CmpCommon::getDefault(OPH_REDUCE_COST_LIMIT_FROM_PASS1_SOLUTION) == DF_ON);
  OPHreuseFailedPlan_ = optimizerPruning_ AND(CmpCommon::getDefault(OPH_REUSE_FAILED_PLAN) == DF_ON);
  OPHskipOGTforSharedGCfailedCL_ =
      optimizerPruning_ AND(CmpCommon::getDefault(OPH_SKIP_OGT_FOR_SHARED_GC_FAILED_CL) == DF_ON);
  OPHuseCompCostThreshold_ = optimizerPruning_ AND(CmpCommon::getDefault(OPH_USE_COMPARE_COST_THRESHOLD) == DF_ON);

  OPHuseConservativeCL_ = (optimizerPruning_ AND(CmpCommon::getDefault(OPH_USE_CONSERVATIVE_COST_LIMIT) == DF_ON))
      OR(optimizerPruning_ AND(defaultPerformanceGoal_ != NULL)
             AND defaultPerformanceGoal_->isOptimizeForResourceConsumption());

  OPHuseFailedPlanCost_ =
      optimizerPruning_ AND OPHreuseFailedPlan_ AND(CmpCommon::getDefault(OPH_USE_FAILED_PLAN_COST) == DF_ON);
  OPHuseNiceContext_ = optimizerPruning_ AND((CmpCommon::getDefault(OPH_USE_NICE_CONTEXT) == DF_ON) OR(
      (numTables_ > defs_->getAsLong(COMP_INT_0)) AND(optLevel_ < OptDefaults::MAXIMUM)));
  OPHuseEnforcerPlanPromotion_ =
      OPHuseNiceContext_ AND(CmpCommon::getDefault(OPH_USE_ENFORCER_PLAN_PROMOTION) == DF_ON);

  OPHreuseOperatorCost_ =
      optimizerPruning_ AND OPHuseFailedPlanCost_ AND(CmpCommon::getDefault(OPH_REUSE_OPERATOR_COST) == DF_ON);
  OPHusePWSflagForContext_ =
      optimizerPruning_ AND OPHreuseFailedPlan_ AND(CmpCommon::getDefault(OPH_USE_PWS_FLAG_FOR_CONTEXT) == DF_ON);
  OPHuseCachedElapsedTime_ = optimizerPruning_ AND(CmpCommon::getDefault(OPH_USE_CACHED_ELAPSED_TIME) == DF_ON);
  OPHexitNJcrContChiLoop_ =
      optimizerPruning_ AND OPHreuseFailedPlan_ AND(CmpCommon::getDefault(OPH_EXITNJCRCONTCHILOOP) == DF_ON);
  OPHexitMJcrContChiLoop_ =
      optimizerPruning_ AND OPHreuseFailedPlan_ AND(CmpCommon::getDefault(OPH_EXITMJCRCONTCHILOOP) == DF_ON);
  OPHexitHJcrContChiLoop_ =
      optimizerPruning_ AND OPHreuseFailedPlan_ AND(CmpCommon::getDefault(OPH_EXITHJCRCONTCHILOOP) == DF_ON);

  dataFlowOptimization_ = (CmpCommon::getDefault(DATA_FLOW_OPTIMIZATION) == DF_ON);
  attemptESPParallelism_ = CmpCommon::getDefault(ATTEMPT_ESP_PARALLELISM);

  if ((rootExpr->getOperatorType() == REL_ROOT) && (((RelRoot *)rootExpr)->disableESPParallelism()) &&
      (CmpCommon::getDefault(COMP_BOOL_187) == DF_OFF))
    attemptESPParallelism_ = DF_OFF;

  enableOrOptimization_ = (CmpCommon::getDefault(OR_OPTIMIZATION) != DF_OFF);

  // threshold for pruning based on a plans potential
  enumPotentialThreshold_ = (UInt32)(ActiveSchemaDB()->getDefaults()).getAsLong(COMP_INT_24);
  ...
  if (minimumESPParallelism_ == 0) {
    minimumESPParallelism_ = CmpCommon::context()->getNumOfSMPs();
  }

  if (minimumESPParallelism_ < 1) minimumESPParallelism_ = 1;
  if (minimumESPParallelism_ > maximumDegreeOfParallelism_) minimumESPParallelism_ = maximumDegreeOfParallelism_;
  ...
  maxParallelismIsFeasible_ =
      (CURRSTMT_OPTGLOBALS->forceParallelInsertSelect OR CURRSTMT_OPTGLOBALS->maxParIsFeasible)AND(
          attemptESPParallelism_ == DF_MAXIMUM);
  numberOfPartitionsDeviation_ =
      (maxParallelismIsFeasible_ ? 0 : (float)defs_->getAsDouble(NUMBER_OF_PARTITIONS_DEVIATION));
  numberOfRowsParallelThreshold_ =
      (maxParallelismIsFeasible_ ? (float)defs_->getAsLong(COMP_INT_8)
                                 : (float)defs_->getAsDouble(NUMBER_OF_ROWS_PARALLEL_THRESHOLD));

  updatedBytesPerESP_ = defs_->getAsDouble(UPDATED_BYTES_PER_ESP);

  numOfPartsDeviationType2Joins_ = (float)defs_->getAsDouble(NUM_OF_PARTS_DEVIATION_TYPE2_JOINS);

  parallelHeuristic1_ = (CmpCommon::getDefault(PARALLEL_HEURISTIC_1) == DF_ON) AND(attemptESPParallelism_ == DF_SYSTEM);
  parallelHeuristic2_ = (CmpCommon::getDefault(PARALLEL_HEURISTIC_2) == DF_ON) AND(attemptESPParallelism_ == DF_SYSTEM);
  parallelHeuristic3_ = (CmpCommon::getDefault(PARALLEL_HEURISTIC_3) == DF_ON) AND(attemptESPParallelism_ == DF_SYSTEM);
  parallelHeuristic4_ = (CmpCommon::getDefault(PARALLEL_HEURISTIC_4) == DF_ON) AND(attemptESPParallelism_ == DF_SYSTEM);

  // dataFlowOptimization_ overwrites heuristics 4 & 5 if its value is ON
  optimizerHeuristic4_ = optimizerHeuristic4_ OR dataFlowOptimization_;
  optimizerHeuristic5_ = optimizerHeuristic5_ OR dataFlowOptimization_;

  preferredProbingOrderForNJ_ = (CmpCommon::getDefault(PREFERRED_PROBING_ORDER_FOR_NESTED_JOIN) == DF_ON);

  orderedWritesForNJ_ = (CmpCommon::getDefault(UPD_ORDERED) == DF_ON);

  nestedJoinForCrossProducts_ = (CmpCommon::getDefault(NESTED_JOIN_FOR_CROSS_PRODUCTS) == DF_ON);
  ...
  reduction_to_push_gb_past_tsj_ = defs_->getAsDouble(REDUCTION_TO_PUSH_GB_PAST_TSJ);

  taskCount_ = 0;

  randomPruningOccured_ = FALSE;  // not yet
  optimizerGracefulTermination_ = defs_->getAsULong(OPTIMIZER_GRACEFUL_TERMINATION);

  // initialize random sequence;
  if (ranSeq_ == NULL)
    ranSeq_ = new (heap_) RandomSequence();  // this also does initialize
  else
    (*ranSeq_).initialize();
  ...
  if (ActiveControlDB()->getRequiredShape() && ActiveControlDB()->getRequiredShape()->getShape() &&
    ...
    ignoreExchangesInCQS_ = ActiveControlDB()->getRequiredShape()->getIgnoreExchange();
    ignoreSortsInCQS_ = ActiveControlDB()->getRequiredShape()->getIgnoreSort();
  ...
  // used for explain
  // BEGIN
  useStrategizer_ = ((CmpCommon::getDefault(QUERY_STRATEGIZER) == DF_ON) &&
  ...
  minMaxINlistOpt_ = (CmpCommon::getDefault(GEN_HSHJ_MIN_MAX_OPT_INLIST_OPT) == DF_ON);

  rangeOptimizedScan_ = (defs_->getAsLong(RANGE_OPTIMIZED_SCAN) > 38);
...
NABoolean OptDefaults::InitCostVariables() {
  ...
  if (defaultCostWeight_ == NULL && (defaultCostWeight_ = new (heap_) ResourceConsumptionWeight(1, 1, 1, 0, 0)) == NULL)
    return (FALSE);
  ...
  if (goalTok == DF_LASTROW) {
    ...
    *CmpCommon::diags() << DgSqlCode(+2055)  // warning only
                        << DgString0(goalText) << DgString1("OPTIMIZATION_GOAL");
  }
...
void OptDefaults::CleanupCostVariables() {
  if (defaultCostWeight_ != NULL) delete defaultCostWeight_;
  if (defaultPerformanceGoal_ != NULL) NADELETEBASIC(defaultPerformanceGoal_, heap_);
  if (resourcePerformanceGoal_ != NULL) NADELETEBASIC(resourcePerformanceGoal_, heap_);
...
void OptDefaults::recalibrateCPU() {
  // -----------------------------------------------------------------------
  ...
  if (defs_->getProvenance(TARGET_CPU_FREQUENCY) == NADefaults::INIT_DEFAULT_DEFAULTS) {
    // get  it from the registry:
    targetFrequency = (float)CURRCONTEXT_CLUSTERINFO->processorFrequency();
  } else {
  ...
  // code state:
  // -----------------------------------------------------------------------
  DefaultToken referenceCodeToken = defs_->getToken(REFERENCE_CODE), targetCodeToken = defs_->getToken(TARGET_CODE);
...
void OptDefaults::recalibrateIOSeqTransfer() {
  ...
  if (defs_->getProvenance(TARGET_IO_SEQ_READ_RATE) == NADefaults::INIT_DEFAULT_DEFAULTS) {
    // get  it from the registry:
    targetIORate = CURRCONTEXT_CLUSTERINFO->ioTransferRate();
  } else {
    // The user changed the defaults either through the SQL
    // defaults table or by using a CQD. Thus get what the
    // user wants:
...
void OptDefaults::recalibrateIOSeeks() {
  ...
  // Case when the user did not change the default.
  if (defs_->getProvenance(TARGET_IO_SEEK_TIME) == NADefaults::INIT_DEFAULT_DEFAULTS) {
    // get  it from the registry:
    targetSeekTime = (float)CURRCONTEXT_CLUSTERINFO->seekTime();
  } else {
    // The user changed the defaults either through the SQL
    // defaults table or by using a CQD. Thus get what the
    // user wants:
...
void OptDefaults::recalibrateLocalMsg() {
  calibratedMSCF_ET_NUM_LOCAL_MSGS_ =
      recalibrate(MSCF_ET_NUM_LOCAL_MSGS, REFERENCE_MSG_LOCAL_TIME, TARGET_MSG_LOCAL_TIME);
}

void OptDefaults::recalibrateLocalMsgTransfer() {
  calibratedMSCF_ET_LOCAL_MSG_TRANSFER_ =
      recalibrate(MSCF_ET_LOCAL_MSG_TRANSFER, REFERENCE_MSG_LOCAL_RATE, TARGET_MSG_LOCAL_RATE);
}

void OptDefaults::recalibrateRemoteMsg() {
  calibratedMSCF_ET_NUM_REMOTE_MSGS_ =
      recalibrate(MSCF_ET_NUM_REMOTE_MSGS, REFERENCE_MSG_REMOTE_TIME, TARGET_MSG_REMOTE_TIME);
}
...
NABoolean OptDefaults::cacheHistograms() {
  return ((CmpCommon::getDefault(CACHE_HISTOGRAMS) == DF_ON) &&
...
ostream &OptDebug::stream() {
  if (fileIsGood_) {
    ...
    if (CmpCommon::context() && ciClass_ != CmpCommon::context()->getCIClass())
...
// ---------------------------------------------------------------------
void OptDebug::showTree(const ExprNode *tree, const CascadesPlan *plan, const char *prefix, NABoolean showDetail) {
  if (tree == NULL && plan == NULL) return;

  NAString indent(prefix, CmpCommon::statementHeap());
  indent += "  ";

  const RelExpr *re = (plan != NULL) ? plan->getPhysicalExpr()->castToRelExpr() : tree->castToRelExpr();
  ...
  if (plan != NULL) {
    ...
    while (plan->getSolutionForChild(index)) {
      showTree(plan->getSolutionForChild(index)->getPhysicalExpr(), plan->getSolutionForChild(index), indent,
               showDetail);
      index++;
    }
  } else {
...
void OptDebug::showNode(const ExprNode *tree, const CascadesPlan *plan, const char *prefix, NABoolean showDetail) {
  if (tree == NULL) return;
  ...
  out << prefix << "SQL node(" << re->getText().data() << ")" << endl
      << prefix << "cost: " << getCost(re, plan) << " rows: " << getRows(re, plan) << BLANK_SPACE << getGroups(re)
      << " type: " << getType(re, plan) << endl;

  // Show my item expression.
  if (showDetail) {
    if (CmpCommon::getDefault(NSK_DBG_PRINT_ITEM_EXPR) == DF_ON) showItemExpr(tree, plan, prefix);
...
void OptDebug::showItemExpr(const ExprNode *tree, const CascadesPlan *plan, const char *prefix) {
  ostream &out = stream();
  ...
  while ((numEntries = localExpList.entries()) > 0 AND localExpList[numEntries - 1] == NULL)
  ...
  while (localExpList.getFirst(currExpr)) {
    ...
    if (currExpr) currExpr->print(fp(), CONCAT(indent, "  "), "");
...
void OptDebug::showProperties(const ExprNode *tree, const CascadesPlan *plan, const char *prefix) {
  const RelExpr *re = (plan != NULL) ? plan->getPhysicalExpr() : tree->castToRelExpr();

  // Print cost.
  if (CmpCommon::getDefault(NSK_DBG_PRINT_COST) == DF_ON) showCosts(re, plan, prefix);

  // Print logical properties.
  if (CmpCommon::getDefault(NSK_DBG_PRINT_LOG_PROP) == DF_ON) showLogicalProperties(re, plan, prefix);

  // Print physical properties.
  if (CmpCommon::getDefault(NSK_DBG_PRINT_PHYS_PROP) == DF_ON) showPhysicalProperties(re, plan, prefix);

  // Print characteristic inputs.
  if (CmpCommon::getDefault(NSK_DBG_PRINT_CHAR_INPUT) == DF_ON) showCharacteristicInputs(re, plan, prefix);

  // Print characteristic outputs.
  if (CmpCommon::getDefault(NSK_DBG_PRINT_CHAR_OUTPUT) == DF_ON) showCharacteristicOutputs(re, plan, prefix);

  // Print constraints.
  if (CmpCommon::getDefault(NSK_DBG_PRINT_CONSTRAINT) == DF_ON) showConstraints(re, plan, prefix);

  // Print context.
  if (CmpCommon::getDefault(NSK_DBG_PRINT_CONTEXT) == DF_ON) showContext(re, plan, prefix);
}
...
double OptDebug::getCost(const RelExpr *re, const CascadesPlan *plan) {
  ...
  if (ga != NULL) {
    const Cost *planCost = (plan != NULL) ? plan->getRollUpCost() : re->getRollUpCost();

    if (planCost) cost = planCost->convertToElapsedTime().getValue();
  }
...
double OptDebug::getRows(const RelExpr *re, const CascadesPlan *plan) {
  ...
  if (ga != NULL) {
    ...
    if (plan != NULL) {
      ...
      if (ga->existsLogExprForSynthesis()) numRows = ga->outputLogProp(inputLP)->getResultCardinality();
    }
...
const NAString OptDebug::getGroups(const RelExpr *re) {
  ...
  if (ga != NULL) {
    if (groupId == NULL_COLL_INDEX)
      sprintf(CURRSTMT_OPTGLOBALS->returnString, "in: %d, out: %d", ga->getCharacteristicInputs().entries(),
              ga->getCharacteristicOutputs().entries());
    else
      sprintf(CURRSTMT_OPTGLOBALS->returnString, "grp: %d, in: %d, out: %d", groupId,
              ga->getCharacteristicInputs().entries(), ga->getCharacteristicOutputs().entries());
  } else {
...
const NAString OptDebug::getType(const RelExpr *re, const CascadesPlan *plan) {
  if (re->isPhysical()) {
    if (plan != NULL) {
      if (plan->getPhysicalProperty() != NULL && plan->getRollUpCost() != NULL)
        sprintf(CURRSTMT_OPTGLOBALS->returnString, "plan");
      else
        sprintf(CURRSTMT_OPTGLOBALS->returnString, "incomplete plan");
    } else {
      if (re->getPhysicalProperty() != NULL && re->getRollUpCost() != NULL)
        sprintf(CURRSTMT_OPTGLOBALS->returnString, "plan");
      else
...
void OptDebug::showCharacteristicInputs(const RelExpr *re, const CascadesPlan *plan, const char *prefix) {
  const GroupAttributes *ga = re->getGroupAttr();
  if (ga == NULL) return;
  ...
  for (ValueId x = inputs.init(); inputs.next(x); inputs.advance(x)) {
    ...
    out << indent.data() << "ValId #" << (CollIndex)x << ": " << unparsed.data() << endl;
  }  // end for
...
void OptDebug::showCharacteristicOutputs(const RelExpr *re, const CascadesPlan *plan, const char *prefix) {
  const GroupAttributes *ga = re->getGroupAttr();
  if (ga == NULL) return;
...
void OptDebug::showConstraints(const RelExpr *re, const CascadesPlan *plan, const char *prefix) {
  const GroupAttributes *ga = re->getGroupAttr();
  if (ga == NULL) return;
  ...
  for (ValueId x = constraints.init(); constraints.next(x); constraints.advance(x)) {
    unparsed = "";
    x.getItemExpr()->unparse(unparsed);
...
void OptDebug::showContext(const RelExpr *re, const CascadesPlan *plan, const char *prefix) {
  ...
  out << prefix << "<Contexts>" << endl
      ...
      << " niceContext: " << context->isNice() << endl;
...
void OptDebug::showCosts(const RelExpr *re, const CascadesPlan *plan, const char *prefix) {
  ostream &out = stream();

  const Cost *rollUpCost = (plan != NULL) ? plan->getRollUpCost() : re->getRollUpCost();

  const Cost *operatorCost = (plan != NULL) ? plan->getOperatorCost() : re->getOperatorCost();
...
void OptDebug::showLogicalProperties(const RelExpr *re, const CascadesPlan *plan, const char *prefix) {
  const GroupAttributes *ga = re->getGroupAttr();
  if (ga == NULL) return;
  ...
  for (CollIndex i = 0; i < numEntries; i++) {
    ...
    out << indent.data() << "Input Est. Log. Props: #" << i
        << "; Estimated Rows = " << ga->getInputLogPropList()[i]->getResultCardinality().getValue()
    ...
    out << indent.data() << "Output Est. Log. Props: #" << i
        << "; Estimated Rows = " << ga->getOutputLogPropList()[i]->getResultCardinality().getValue()
        << "; Max cardinality = " << ga->getOutputLogPropList()[i]->getMaxCardEst().getValue() << endl;
...
void OptDebug::showPhysicalProperties(const RelExpr *re, const CascadesPlan *plan, const char *prefix) {
  const PhysicalProperty *pp = (plan != NULL) ? plan->getPhysicalProperty() : re->getPhysicalProperty();
  if (pp == NULL) return;
  ...
  if (sortKey.entries() > 0) {
    ...
    switch (pp->getSortOrderType()) {
      case NO_SOT:
        ...
        break;
      case ESP_NO_SORT_SOT:
        propText = "executor process sort order from an index";
        break;
      case ESP_VIA_SORT_SOT:
        propText = "executor process sort order from sorting";
        break;
      case DP2_SOT:
        propText = "DP2 process sort order from an index";
        break;
      default:
        // should never happen
        propText = "";
  ...
  if (pp->getPartitioningFunction()) {
    out << indent.data() << "partitioning function: " << pp->getPartitioningFunction()->getText().data() << endl;
  }
  ...
  if (pp->getIndexDesc()) {
    // Index levels
    out << indent.data() << "Index levels: " << pp->getIndexDesc()->getIndexLevels() << endl;

    // Record length
    out << indent.data() << "Record length: " << pp->getIndexDesc()->getRecordLength() << endl;

    // Record size in kb:
    out << indent.data() << "Record size in kb: " << pp->getIndexDesc()->getRecordSizeInKb().value() << endl;

    // block size:
    out << indent.data() << "Block size in kb: " << pp->getIndexDesc()->getBlockSizeInKb().value() << endl;

    // Records per blk:
    out << indent.data() << "Estimated records per blk: " << pp->getIndexDesc()->getEstimatedRecordsPerBlock().value()
        << endl;
  }
...
void OptDebug::showEstLogProp(const EstLogPropSharedPtr &estLogProp, const char *prefix) {
  ...
  for (int i = 0; i < numStats; i++) {
    ...
    if (colStats->isFakeHistogram()) out << prefix << "*** FAKE HISTOGRAM ***" << endl;

    out << prefix << "Histogram ID = " << colStats->getHistogramId() << ": uec = " << colStats->getTotalUec().getValue()
        << "; rowcount = " << colStats->getRowcount().getValue() << endl;

    out << prefix << "MinValue = " << colStats->getMinValue().getText().data() << endl;

    out << prefix << "MaxValue = " << colStats->getMaxValue().getText().data() << endl;

    out << prefix << "RowRedFactor = " << colStats->getRedFactor().getValue()
        << "; UecRedFactor = " << colStats->getUecRedFactor().getValue() << endl;
    ...
    if (hist != NULL) {
      for (CollIndex i = 0; i < hist->entries(); i++) {
        ...
        out << prefix << "Bound " << i << inclusiveBoundStr << (*hist)[i].getBoundary().getText().data()
            << ": rows = " << (*hist)[i].getCardinality().getValue() << ", uec = " << (*hist)[i].getUec().getValue()
            << endl;
      }
    }  // end if (hist)
...
void OptDebug::showCostInDetail(const NAString &header, const Cost *cost, const char *prefix) {
  ostream &out = stream();
  ...
  // Print the number plan fragments per cpu:
  out << prefix << "plan fragments per CPU: " << cost->getPlanFragmentsPerCPU() << endl;

  // Display header:
  out << prefix << "@@@@@@@@@@@@@@@@@@@@@@@@@@" << endl << prefix << header.data() << endl;

  // Print the elapsed time for this cost object:
  out << prefix << "elapsed time = " << cost->convertToElapsedTime().value() << endl;

  // Print Priority
  out << prefix << "priority: level = " << cost->getPlanPriority().getLevel()
      << " demotion = " << cost->getPlanPriority().getDemotionLevel()
      << " risk premium = " << cost->getPlanPriority().riskPremium().getValue() << endl;
...
void OptDebug::showSimpleCostVector(const char *header, const SimpleCostVector &scv, const char *prefix, ostream &out) {
  // showing all the five cost scalars of the given
  // Simple Cost Vector.

  out << prefix << header << "(" << scv.getDetailDesc(DF_OFF) << ")" << endl;
}
...
void OptDebug::showSimpleCostVectorDetail(const SimpleCostVector &scv, const char *prefix, ostream &out) {
...
void OptDebug::showTask(int pass, int groupId, int task_count, const CascadesTask *task, const ExprNode *tree,
  ...
  out << prefix << "Task: " << task_count << " ParentTask: " << task->getParentTaskId() << " Pass: " << pass
      << " GroupId: " << groupId;
    ...
    if (detailedStat) {
      ...
      for (int j = 0; j < grPlanCnt; j++) {
        ...
        if (planMonitorStat) {
          ...
          out << "PLMON," << i << "," << j << "," << curPlan->lastTaskId_ << "," << curPlan->planMonitor_.timer() << ","
              << curPlan->planMonitor_.count() << "," << curPlan->planMonitor_.goodcount() << "," << outStr << endl;
        }
      }
      failedPlanCnt += grFailedPlanCnt;

      out << prefix << "\tMemo Group: \t" << i << "\tContexts: \t" << groupPtr->getCountOfContexts() << "\tPlans: \t"
          << grPlanCnt << "\tFailed Plans: \t" << grFailedPlanCnt << endl;
    }
  }

  out << prefix << "\tMemo Groups: \t" << maxg << "\n\tMemo LogExprs: \t" << logExprCnt << "\n\tMemo PhyExprs: \t"
      << physExprCnt << "\n\tMemo Plans: \t" << planCnt << "\n\tMemo Plans: \t" << planCnt;
  if (detailedStat) out << "\tFailed Plans: " << failedPlanCnt;
...
RelExpr *RelExpr::optimize2() {
  ...
  if (level == DF_MINIMUM) {
    ...
    if (CURRSTMT_OPTDEFAULTS->needShortPassOptimization() || (CmpCommon::getDefault(FORCE_PASS_ONE) == DF_ON)) {
...
QueryOptimizerDriver::QueryOptimizerDriver(RelExpr *relExpr)
    : fatalExceptionOccurring_(FALSE), isStream_(FALSE), taskCount_(0), taskCountHold_(-1) {
  isCQS_ = (ActiveControlDB()->getRequiredShape() && ActiveControlDB()->getRequiredShape()->getShape() &&
            NOT ActiveControlDB()->getRequiredShape()->getShape()->isCutOp());

  // prunningIsFeasible is global
  if ((CmpCommon::getDefault(COMP_BOOL_9) == DF_OFF)
          AND(relExpr->child(0)->getOperatorType() == REL_DDL OR((RelRoot *)relExpr)
                                                          ->accessOptions()
                                                          .isUpdateOrDelete() OR((RelRoot *)relExpr)
                                                          ->isRootOfInternalRefresh()))
    CURRSTMT_OPTGLOBALS->pruningIsFeasible = FALSE;
  else
    // pruning can be forced by setting CDQ COMP_BOOL_9 to ON
  ...
  if (((CmpCommon::getDefault(COMP_BOOL_69) == DF_OFF)
           AND(relExpr->child(0)->getOperatorType() == REL_DDL OR((RelRoot *)relExpr)->isRootOfInternalRefresh())))
    CURRSTMT_OPTGLOBALS->maxParIsFeasible = FALSE;
  else
    // maximum parallelism can be forced by setting CDQ COMP_BOOL_69 to ON
    CURRSTMT_OPTGLOBALS->maxParIsFeasible = TRUE;
...
QueryOptimizerDriver::~QueryOptimizerDriver() {
  ...
  // searching for an acceptable plan, e.g. generated when searching
  // for a suitable index to satisfy an order by clause on a stream
  if (isStream_ && NOT fatalExceptionOccurring_) clearFailedPlanWarningsForStream();
}
...
Context *QueryOptimizerDriver::initializeOptimization(RelExpr *relExpr) {
  ...
  if (relExpr->getGroupAttr()->isStream() && relExpr->getGroupAttr()->isEmbeddedUpdateOrDelete()) {
    ValueIdSet dummy;
    dummy.clear();
  ...
  CascadesGroupId root = CURRSTMT_OPTGLOBALS->memo->include(relExpr, duplicateExpr, groupMerge)->getGroupId();
  ...
  // ensure that we will get a pass1 plan, since Hash Join does not
  // currently work for ASJ (or embedded delete)
  if (CURRSTMT_OPTDEFAULTS->isJoinToTSJRuleNeededOnPass1()) GlobalRuleSet->enable(JoinToTSJRuleNumber);
  ...
  Context *context = (*CURRSTMT_OPTGLOBALS->memo)[root]->shareContext(NULL,       /*rpp*/
                                                                      NULL, NULL, /*cost limit*/
                                                                      NULL, (*GLOBAL_EMPTY_INPUT_LOGPROP));
...
RelExpr *QueryOptimizerDriver::doPass1Only(RelExpr *relExpr, Context *context) {
  try {
    ...
    PassOneAssertFatalException(e.getCondition(), e.getFileName(), e.getLineNum()).throwException();
...
RelExpr *QueryOptimizerDriver::doPass1Pass2(RelExpr *relExpr, Context *context) {
  ...
  if (pass1HasPlan && context->getSolution()) {
    ...
    COMPARE_RESULT costComparison = pass2Cost->compareCosts(*pass1Cost, context->getReqdPhysicalProperty());
...
RelExpr *QueryOptimizerDriver::bindSavedPass1Solution(RelExpr *relExpr, Context *context) {
...
RelExpr *QueryOptimizerDriver::doPass2PerhapsPass1(RelExpr *relExpr, Context *context, RelExpr *original) {
  try {
    GlobalRuleSet->setCurrentPassNumber(2);
    setupPassTwoHeuristics(context);
    optimizeAPass(context);
  ...
  if (plan == NULL) {
    NABoolean tryPassOne = (CmpCommon::getDefault(TRY_PASS_ONE_IF_PASS_TWO_FAILS) == DF_ON);
    ...
    if (tryPassOne) {
      ...
      markFatalAndGenErrorNoPlan(relExpr, context);
      PassOneSkippedPassTwoNoPlanFatalException(__FILE__, __LINE__).throwException();
    }
  }
...
void QueryOptimizerDriver::setupPassTwoHeuristics(Context *context) {
  if (CURRSTMT_OPTDEFAULTS->optimizerPruning()) {
    ...
    if (forcedPass2costLimit > csZero)
      ((ElapsedTimeCostLimit *)(context->getCostLimit()))->setUpperLimit(forcedPass2costLimit);
  }
}
...
void QueryOptimizerDriver::optimizeAPass(Context *context) {
  try {
    ...
    if (!hasEnoughMemory) {
      // Warning: insufficient memory for pass 2.
      if (CmpCommon::getDefault(SHOWWARN_OPT) == DF_ON && !CmpCommon::diags()->containsWarning(6020))
...
void QueryOptimizerDriver::optimizeAPassHelper(Context *context) {
  ...
  // start optimization, push initial task
  CURRSTMT_OPTGLOBALS->task_list->push(new (CmpCommon::statementHeap()) OptimizeGroupTask(context, NULL, /*guidance*/
                                                                                          FALSE, 0, ++stride));
  ...
  while (NOT CURRSTMT_OPTGLOBALS->task_list->empty()) {
    ...
    if ((taskCount_ >= taskThreshold) && ((taskCount_ - taskThreshold) % taskInterval) == 0) {
    ...
    if (CURRSTMT_OPTDEFAULTS->compileTimeMonitor()) (*CURRSTMT_OPTGLOBALS->cascadesTasksMonitor[next_task_type]).exit();
...
void QueryOptimizerDriver::markFatalAndGenErrorNoPlan(RelExpr *relExpr, Context *context) {
  ...
  if (CmpCommon::diags()->getNumber(DgSqlCode::ERROR_) == 0) {
    ...
    if (relExpr->getOperatorType() == REL_ROOT) {
      ...
      if (root->isTrueRoot() && (!(root->reqdOrder().isEmpty()))) {
        ...
        if (context->getGroupAttr()->isEmbeddedUpdateOrDelete()) {
          *CmpCommon::diags() << DgSqlCode(4154)
                              << (relExpr->getGroupAttr()->isEmbeddedUpdate() ? DgString0("update")
                                                                              : DgString0("delete"));
        }
      }
    }
    ...
    if (relExpr->getOperatorType() == REL_ROOT && ((RelRoot *)relExpr)->getNumExtractStreams() > 0) {
...
void QueryOptimizerDriver::DEBUG_PRINT_COUNTERS() {
  ...
  if (printCounters_) {
    ...
    *CmpCommon::diags() << DgSqlCode(arkcmpOptimizerCountersWarning);
    ...
    report(string);
    *CmpCommon::diags() << DgString0(string);
    sprintf(string, " %d groups, %d tasks, %d rules,\n", CURRSTMT_OPTGLOBALS->memo->getCountOfUsedGroups(), taskCount_,
            GlobalRuleSet->getRuleApplCount());
    report(string);
    *CmpCommon::diags() << DgString1(string);
    sprintf(string, "%d groups merged, %d expr. cleaned up, %d tasks pruned\n", CURRSTMT_OPTGLOBALS->group_merge_count,
            CURRSTMT_OPTGLOBALS->garbage_expr_count, CURRSTMT_OPTGLOBALS->pruned_tasks_count);
    report(string);
    *CmpCommon::diags() << DgString2(string);
    sprintf(string, "%d/%d/%d/%d log/phys/plans/dupl expressions in CascadesMemo\n",
            CURRSTMT_OPTGLOBALS->logical_expr_count, CURRSTMT_OPTGLOBALS->physical_expr_count,
...
NABoolean OptDefaults::isAdaptiveSegForHadoop() const {
  const NAWNodeSet *availableNodes = CmpCommon::context()->getAvailableNodes();

  return defs_->getAsLong(AS_AFFINITY_VALUE) != -2 && availableNodes && availableNodes->castToNAASNodes();
}

NABoolean OptDefaults::canUseAllClusterResources() const {
  const NAWNodeSet *availableNodes = CmpCommon::context()->getAvailableNodes();

  return availableNodes == NULL || availableNodes->canUseAllAvailableUnits(CURRCONTEXT_CLUSTERINFO);
}

OptGlobals::OptGlobals(NAHeap *heap) : heap_(heap) {
  // -----------------------------------------------------------------------
...
NABoolean Context::hasOptimalSolution() const {
  return (solution_ AND optimizedInCurrentPass()
              AND(NOT costLimitExceeded_ OR CURRSTMT_OPTDEFAULTS->OPHuseFailedPlanCost()));
}

NABoolean Context::isNiceContextEnabled() const {
  return (niceContext_ AND CURRSTMT_OPTDEFAULTS->optimizerPruning())OR CURRSTMT_OPTDEFAULTS->OPHuseNiceContext();
}
