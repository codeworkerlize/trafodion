...
ItemExpr *QRExplicitExpr::findItemExpr() {
  ...
  assertLogAndThrow(CAT_SQL_COMP_QR_DESC_GEN, LL_MVQR_FAIL, id != 0, QRLogicException,
...
ItemExpr *QRColumn::toItemExpr(const NAString &mvName, CollHeap *heap, TableNameScanHash *scanhash) {
  ...
  if (scanhash->getFirstValue(&tableId_)) (corrNameStr = MVCandidates::BACKJOIN_CORRNAME_PREFIX).append(tableId_);
  CorrName tblCorrName(tblQualName, heap, corrNameStr);
  return new (heap) ColReference(new (heap) ColRefName(columnName_, tblCorrName, heap));
}
...
ItemExpr *QRMVColumn::toItemExpr(const NAString &mvName, CollHeap *heap, TableNameScanHash *scanhash) {
  ...
  return new (heap) ColReference(new (heap) ColRefName(getMVColName(), mvCorrName, heap));
}
...
ItemExpr *QRScalarValue::toItemExpr(const NAString &mvName, CollHeap *heap, TableNameScanHash *scanhash) {
  ...
  if (op == ITM_CACHE_PARAM)
    ...
    assertLogAndThrow(CAT_SQL_COMP_QR_DESC_GEN, LL_MVQR_FAIL, op == ITM_CONSTANT, QRLogicException,
                      "Expecting a ConstValue object.");
    cv = (ConstValue *)ie;
  }
...
ItemExpr *QRFunction::toItemExpr(const NAString &mvName, CollHeap *heap, TableNameScanHash *scanhash) {
  ...
  if (getRefNum() != 0) {
    ...
    NABoolean isScalarVariance =
        (getFunctionName() == "Scalar Variance Sample" || getFunctionName() == "Scalar Stddev Population" ||
         getFunctionName() == "Scalar Variance Population" || getFunctionName() == "Scalar Stddev Sample");
    ...
    for (CollIndex i = 0; i < args.entries(); i++) {
      if (args[i] != NULL) {
        ...
        if (isScalarVariance) argument = new (heap) Cast(argument, dbl);
    ...
    assertLogAndThrow(CAT_SQL_COMP_QR_DESC_GEN, LL_MVQR_FAIL, isAnAggregate(), QRLogicException,
    ...
    switch (getAggregateFunc()) {
      ...
      case AFT_SUM_ON_GROUPING: {
        ...
        if (getAggregateFunc() == AFT_COUNT_ON_GROUPING) {
          ...
          ieEnum = ITM_SUM;
          argument = new (heap) Case(NULL, new (heap) IfThenElse(new (heap) UnLogic(ITM_IS_NULL, argument),
                                                                 new (heap) SystemLiteral(0), countStar));
        } else {
