...
EstLogProp::EstLogProp(CostScalar card, ValueIdSet *preds, SemiTSJEnum inputForSemiTSJ, CANodeIdSet *nodeSet,
                       NABoolean cacheable, NAMemory *h)
  ...
  if (preds != NULL) unresolvedPreds_ = *preds;
  nodeSet_ = nodeSet;
  (*counter_).incrementCounter();
}

EstLogProp::EstLogProp(const EstLogProp &other, NAMemory *h)
    : columnStats_(h), isCardinalityEqOne_(other.isCardinalityEqOne_) {
...
EstLogProp::~EstLogProp() {
  ...
  if ((cacheable_) && (nodeSet_) && (qa) && (qa->isAnalysisON())) {
    AppliedStatMan *appStatMan = qa->getASM();
    if (appStatMan) appStatMan->removeEntryIfThisObjectIsCached(this);
...
COMPARE_RESULT EstLogProp::compareEstLogProp(const EstLogPropSharedPtr &other) const {
  ...
  if (resultCardinality_ ==
      other->resultCardinality_ AND(  // Check for the case where we have two "empty" input logical properties.
          (columnStats_.entries() == 0 AND other->columnStats_.entries() == 0)
              OR(columnStats_ == other->columnStats_ AND unresolvedPreds_ ==
                 other->unresolvedPreds_ AND inputForSemiTSJ_ == other->inputForSemiTSJ_)))
    return SAME;
...
//
// ---------------------------------------------------------------------
void EstLogProp::pickOutputs(ColStatDescList &columnStats, const EstLogPropSharedPtr &inputEstLogProp,
                             const ValueIdSet specifiedOutputs, const ValueIdSet predSet) {
  ...
  for (CollIndex i = 0; i < columnStats.entries(); i++) {
    ...
    for (CollIndex j = 0; j < outerColStatsList.entries(); j++) {
      if (columnId == outerColStatsList[j]->getVEGColumn() OR(CmpCommon::context()->showQueryStats())) {
        colStats().insert(colStatDesc);
        found = TRUE;
        if (!colStatDescAdded) colStatDescAdded = TRUE;
    ...
    if (NOT found AND(columnId != NULL_VALUE_ID)
            AND(colsRequiringHistograms.contains(columnId)
                    OR colsRequiringHistograms.referencesTheGivenValue(columnId, throwaway)
                        OR columnId.isInvolvedInJoinAndConst() OR CmpCommon::context()
                            ->showQueryStats())) {
      colStats().insert(colStatDesc);
      found = TRUE;
      if (!colStatDescAdded) colStatDescAdded = TRUE;
    ...
    // For now, always include expressions histograms. This is bad because
    // this method is attempting to minimize the set of histograms passed
    // up, in order to reduce compiler space and time overhead. This code
    // should be replaced with code that figures out whether the histogram
    // is needed by a parent node. That seems hard to do precisely. One
    // idea might be to see if the ValueIds of all columns referenced in
    // the expression have their histograms included, and if so, include
    // the expression histogram. Will do this later.
    if (colStatDesc->isExpressionHistogram()) {
      // TODO: would be better if insert method called
      // setHasExpressionHistogram, but the NAList
      // insert method isn't virtual
      colStats().insert(colStatDesc);
      found = TRUE;
      colStats().setHasExpressionHistogram();
      if (!colStatDescAdded) colStatDescAdded = TRUE;
    }
    // TODO: replace the above temporary code
    ...
    if (CURRSTMT_OPTDEFAULTS->incorporateSkewInCosting()) {
      ...
      if (!(stat->isVirtualColForHist()) && NOT found && !(stat->isExpressionHistogram()) &&
          !(stat->isOrigFakeHist())) {
  ...
  if (!colStatDescAdded && columnStats.entries() > 0) colStats().insert(columnStats[0]);
}  // pickOutputs

// -----------------------------------------------------------------
// This method is used to map colStats of my child to mine
// -----------------------------------------------------------------
void EstLogProp::pickOutputsForUpdate(ColStatDescList colStatsFromScan, const EstLogPropSharedPtr &inputEstLogProp,
                                      const RelExpr &relExpr, const ValueIdSet updateExprOutputs,
...
// that of mine
// ------------------------------------------------------------------------------
void EstLogProp::mapOutputsForUpdate(const GenericUpdate &updateExpr, const ValueIdMap &updateSelectValueIdMap) {
  ...
  for (CollIndex i = 0; i < colStats().entries(); i++) {
    ...
    if (updateColVEGOutputId != columnId) {
      ...
      for (ValueId column = baseColumns.init(); baseColumns.next(column); baseColumns.advance(column)) {
      ...
      ColStatDescSharedPtr colStatDescForUpdate(
          new (STMTHEAP) ColStatDesc(colStatsForUpdate,
...
CostScalar EstLogProp::getCardOfBusiestStream(const PartitioningFunction *partFunc, int numOfParts,
  ...
  if ((colStatsList.entries() == 0) || (groupAttr && groupAttr->getIsProbeCacheable())) {
    return (getResultCardinality() / numOfParts).minCsOne();
  }

  CostScalar cardinalityPerStream;
  if (NOT isUnderNestedJoin)
    cardinalityPerStream = colStatsList.getCardOfBusiestStream(partFunc, numOfParts, groupAttr, countOfCPUs);
  else {
    CANodeIdSet *outerNodeSet = getNodeSet();
    cardinalityPerStream =
        colStatsList.getCardOfBusiestStreamForUnderNJ(outerNodeSet, partFunc, numOfParts, groupAttr, countOfCPUs);
  }
  return cardinalityPerStream;

}  // EstLogProp::getCardOfBusiestStream

void EstLogProp::print(FILE *ofd, const char *prefix, const char *suffix) const {}  // EstLogProp::print()
...
void EstLogProp::display() const { print(); }
