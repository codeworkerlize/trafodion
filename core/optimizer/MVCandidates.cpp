...
void MVCandidates::buildFavoritesList(CollHeap *heap) {
  ...
  if (MVsStr.length() > 0)  // have any
  {
    ...
    if (mvqrFavoriteCandidates_ == NULL) mvqrFavoriteCandidates_ = new (heap) LIST(NAString)(heap);
...
GroupByAgg *MVCandidates::getGroupByAggNode(RelExpr *childNode, QRGroupByPtr groupBy, QRCandidatePtr candidate,
                                            CollHeap *heap) {
  GroupByAgg *groupByAgg = new (heap) GroupByAgg(childNode, REL_GROUPBY, NULL, NULL, heap);
  // Check for the Aggregate with no GroupBy case.
  if (groupBy->isEmpty()) return groupByAgg;
  ...
  for (CollIndex i = 0; i < groupItems.entries(); i++) {
    ...
    if (elemType == ET_MVColumn) {
      ...
      groupByAgg->addGroupExprTree(new (heap) ColReference(getColRefName(candidate, mvcol, heap)));
    } else if (elemType == ET_Column) {
      QRColumnPtr col = groupItem->getReferencedElement()->downCastToQRColumn();
      groupByAgg->addGroupExprTree(new (heap) ColReference(getColRefName(candidate, col, heap)));
    } else if (elemType == ET_Expr)
      groupByAgg->addGroupExprTree(
          parseExpression(groupItem->downCastToQRExpr(), candidate->getMVName()->getMVName(), heap));
    else
      assertLogAndThrow1(CAT_SQL_COMP_MVCAND, LL_MVQR_FAIL, FALSE, MVCandidateException,
                         "Unexpected element type for grouping item -- %d", elemType)
  }
...
void MVCandidates::getEqSetMembers(VEGPredicate *vegPred, ValueIdSet &vegMembers) {
  ...
  for (CollIndex i = 0; !found && i < eqSets.entries(); i++) {
    if (eqSets[i]->getJoinPredId() == vegrefVid) {
      ...
      for (CollIndex j = 0; j < eqSet.entries(); j++) vegMembers.insert(eqSet[j]->getValueId());
      found = TRUE;
    }
  }
}

ItemExpr *MVCandidates::rewriteVegPredicate(const ElementPtrList &mvColumns, VEGPredicate *vegPred,
                                            QRCandidatePtr candidate, CollHeap *heap) {
  ...
  for (ValueId memVid = vegMembers.init(); vegMembers.next(memVid); vegMembers.advance(memVid)) {
    ...
    for (CollIndex i = 0; i < mvColumns.entries() && !operand; i++) {
      switch (mvColumns[i]->getElementType()) {
        case ET_MVColumn:
          ...
          if (mvCol->hasRefTo(memVid)) operand = new (heap) ColReference(getColRefName(candidate, mvCol, heap));
          break;
  ...
  assertLogAndThrow(CAT_SQL_COMP_MVCAND, LL_MVQR_FAIL, andBackbone, MVCandidateException,
                    "rewriteVegPredicate returning NULL");
  return andBackbone;
}  // rewriteVegPredicate

ItemExpr *MVCandidates::rewriteItemExpr(const ElementPtrList &mvColumns, ItemExpr *itemExpr, QRCandidatePtr candidate,
                                        CollHeap *heap, CollIndex tblId) {
  ...
  if (numChildren == 0) {
    ...
    if (itemExpr->getOperatorType() == ITM_VEG_PREDICATE)
      return rewriteVegPredicate(mvColumns, static_cast<VEGPredicate *>(itemExpr), candidate, heap);
      ...
      switch (mvColumns[i]->getElementType()) {
        case ET_MVColumn:
          ...
          if (itemExpr->containsTheGivenValue(mvCol->getRefNum())) {
            replacement = new (heap) ColReference(getColRefName(candidate, mvCol, heap));
            OperatorTypeEnum agg = NO_OPERATOR_TYPE;
            if (isRollup(candidate)) agg = determineAggForRollup(mvCol);
            if (agg != NO_OPERATOR_TYPE) replacement = new (heap) Aggregate(agg, replacement);
          }
          break;
        ...
        case ET_JoinPred:
          ...
          if (colRefedElem->getElementType() == ET_Column) {
            ...
            if (itemExpr->containsTheGivenValue(col->getIDNum()))
              replacement = new (heap) ColReference(getColRefName(candidate, col, heap));
          } else {
            ...
            for (CollIndex i = 0; !foundRightOne && i < eqList.entries(); i++) {
              if (eqList[i]->getElementType() == ET_Column) {
                ...
                if (colRefedElem->getElementType() == ET_Column) {
                  ...
                  if (itemExpr->containsTheGivenValue(col->getIDNum())) {
                    foundOne = TRUE;
                    if (tblId == 0 || tblId == col->getTableIDNum()) {
                      foundRightOne = TRUE;
                      replacement = new (heap) ColReference(getColRefName(candidate, col, heap));
                    }
                  }
            ...
            assertLogAndThrow(CAT_SQL_COMP_MVCAND, LL_MVQR_FAIL, !foundOne || foundRightOne, MVCandidateException,
        ...
        default:
          assertLogAndThrow1(CAT_SQL_COMP_MVCAND, LL_MVQR_FAIL, FALSE, MVCandidateException,
                             "rewriteItemExpr() not ready for element of type %d", mvColumns[i]->getElementType());
          break;
      }
    }
      ...
      switch (mvColumns[i]->getElementType()) {
        ...
        case ET_Expr:
          if ((NumericID)itemExpr->getValueId() == mvColumns[i]->getRefNum())
            replacement = parseExpression(mvColumns[i]->downCastToQRExpr(), candidate->getMVName()->getMVName(), heap);
          break;
...
ItemExpr *MVCandidates::rewriteItemExpr(QRElementPtr mvCol,  // could be col or mvcol
                                        ItemExpr *itemExpr, QRCandidatePtr candidate, CollHeap *heap) {
...
static OperatorTypeEnum determineAggForRollup(QRMVColumnPtr mvCol) {
  ...
  if (result != ITM_NOT) return result;
  ...
  if (itemExpr->isAnAggregate()) {
    switch (itemExpr->getOperatorType()) {
      ...
      default:
        assertLogAndThrow1(CAT_SQL_COMP_MVCAND, LL_MVQR_FAIL, FALSE, MVCandidateException,
                           "Can't rewrite this agg func for rollup: %d", itemExpr->getOperatorType());
    }
  } else
...
void MVCandidates::rewriteRangePreds(QRCandidatePtr candidate, RelExpr *scan, RelExpr *groupBy, CollHeap *heap) {
    ...
    switch (rangePred->getResult()) {
      ...
      case QRElement::NotProvided:
        ...
        if (rangeItem->getElementType() == ET_MVColumn) {
          ...
          if (candidate->isIndirectGroupBy()) determineAggForRollup(mvCol);
          if (rangePred->getRefNum() == 0)  // If no ref attribute.
          {
            ...
            assertLogAndThrow(CAT_SQL_COMP_MVCAND, LL_MVQR_FAIL, isNotNull, MVCandidateException,
                              "Expecting only IS NOT NULL range predicates to have no ref attributes.");
            ItemExpr *mvCol = new (heap) ColReference(getColRefName(candidate, rangeItem, heap));
            rewrittenRangePredItemExpr = new (heap) UnLogic(ITM_IS_NOT_NULL, mvCol);
          } else {
            rewrittenRangePredItemExpr = rewriteItemExpr(rangeItem, rangePredValueId.getItemExpr(), candidate, heap);
          }
          ...
          if (!candidate->isIndirectGroupBy()) {
            QRColumnPtr col = rangeItem->getReferencedElement()->downCastToQRColumn();
            Scan *bjScan = bjScanHash_.getFirstValue(&col->getTableID());
            assertLogAndThrow(CAT_SQL_COMP_MVCAND, LL_MVQR_FAIL, bjScan, MVCandidateException,
            ...
            bjScan->addSelPredTree(rewriteItemExpr(rangeItem, rangePredValueId.getItemExpr(), candidate, heap));
          }
        } else if (rangeItem->getElementType() == ET_Expr) {
          rewrittenRangePredItemExpr = rewriteItemExpr(rangeItem, rangePredValueId.getItemExpr(), candidate, heap);
        } else
          assertLogAndThrow1(CAT_SQL_COMP_MVCAND, LL_MVQR_FAIL, FALSE, MVCandidateException,
                             "Not ready for range item with element type %d", rangeItem->getElementType())
      ...
      default:
        assertLogAndThrow1(CAT_SQL_COMP_MVCAND, LL_MVQR_FAIL, FALSE, MVCandidateException,
                           "Unknown value for result attribute -- %d", rangePred->getResult());
        break;
    }
  }
}
...
static RelExpr *getBackJoinNode(const ElementPtrList &inputList, RelExpr *bjRoot,
  ...
  for (CollIndex i = 0; i < inputList.entries(); i++) {
    ...
    if (elem->getElementType() == ET_MVColumn) {
      ...
      assertLogAndThrow1(CAT_SQL_COMP_MVCAND, LL_MVQR_FAIL, elem->getElementType() == ET_Column, MVCandidateException,
                         "getBackJoinNode() -- can't handle element of type %d", elem->getElementType());
      ...
      if (mvUsed)
        ...
        if (bjScan != bjOwningNode) return bjRoot;  // multinode pred, put in highest join node
      } else
        bjOwningNode = bjScan;
    }
  }
...
void MVCandidates::rewriteResidPreds(QRCandidatePtr candidate, RelExpr *scan, RelExpr *groupBy, RelExpr *bjRoot,
                                     CollHeap *heap) {
  QRExprPtr residPred;
  QRResidualPredListPtr residPreds = candidate->getResidualPredList();
  if (!residPreds) return;
  ...
  for (CollIndex rpInx = 0; rpInx < residPreds->entries(); rpInx++) {
    ...
    switch (residPred->getResult()) {
      ...
      case QRElement::NotProvided: {
        ...
        ElementFinderVisitorPtr visitor = new (heap) ElementFinderVisitor(inputTypes, FALSE, ADD_MEMCHECK_ARGS(heap));
        ...
        ItemExpr *rewrittenResidPredItemExpr =
            rewriteItemExpr(inputList, residPredValueId.getItemExpr(), candidate, heap);
        if (bjOwningNode)
      ...
      default:
        assertLogAndThrow1(CAT_SQL_COMP_MVCAND, LL_MVQR_FAIL, FALSE, MVCandidateException,
                           "Unknown value for result attribute -- %d", residPred->getResult());
        break;
    }
  }
}

ColRefName *MVCandidates::getColRefName(QRCandidatePtr candidate, QRElementPtr elem, CollHeap *heap) {
  ...
  if (elemType == ET_Column) {
    ...
    if (bjScanHash_.getFirstValue(&tblID)) (corrNameStr = BACKJOIN_CORRNAME_PREFIX).append(tblID);
    ...
    QualifiedName mvQualName(candidate->getMVName()->getMVName(), 3, heap, bindWA_);
    CorrName mvCorrName(mvQualName, heap);
    returnVal = new (heap) ColRefName(elem->downCastToQRMVColumn()->getMVColName(), mvCorrName, heap);
  } else
    throw QRDescriptorException("getColRefName() cannot be called for element %s", elem->getElementName());

  return returnVal;
}  // getColRefName()

static ItemExpr *addToList(ItemExpr *list, ItemExpr *item, CollHeap *heap) {
  if (list == NULL)
    list = item;
  else
...
void MVCandidates::buildOutputExprs(QRCandidatePtr candidate, RelRoot *root, ElementPtrList &topValElemList,
                                    CollHeap *heap) {
  ItemExpr *compExpr;
  ...
  for (CollIndex outInx = 0; outInx < outputList->entries(); outInx++) {
    ...
    switch (output->getResult()) {
      case QRElement::Provided: {
        ...
        assertLogAndThrow1(CAT_SQL_COMP_MVCAND, LL_MVQR_FAIL, outputItem->getElementType() == ET_MVColumn,
        ...
        if (!candidate->isIndirectGroupBy()) {
          ...
          compExpr = new (heap) ColReference(getColRefName(candidate, mvCol, heap));
      ...
      case QRElement::NotProvided: {
        ...
        if (outputItemList.entries() == 0) {
          ...
          assertLogAndThrow(CAT_SQL_COMP_MVCAND, LL_MVQR_FAIL, FALSE, MVCandidateException,
          ...
          assertLogAndThrow1(CAT_SQL_COMP_MVCAND, LL_MVQR_FAIL, outputItemList.entries() == 1, MVCandidateException,
          ...
          if (!candidate->isIndirectGroupBy()) {
            ...
            compExpr = new (heap) ColReference(getColRefName(candidate, col, heap));
            selectList = addToList(selectList, compExpr, heap);
          }
        } else  // The output item must be an <Expr>.
        {
          assertLogAndThrow1(CAT_SQL_COMP_MVCAND, LL_MVQR_FAIL,
                             outputItemList.entries() == 1 && outputItemList[0]->getElementType() == ET_Expr,
                             MVCandidateException,
          ...
          if (!isAggregate) topValElemList.insert(output);
          if (!candidate->isIndirectGroupBy() || hasAggregate) {
            ...
            if (hasAggregate) {
              ...
              char colName[20];
              sprintf(colName, "%.10s%d", AGG_NAME_PREFIX, output->getRefNum());
              compExpr = new (heap) RenameCol(compExpr, new (heap) ColRefName(colName, heap));
            }
            selectList = addToList(selectList, compExpr, heap);
          }
        }
      ...
      default:
        assertLogAndThrow1(CAT_SQL_COMP_MVCAND, LL_MVQR_FAIL, FALSE, MVCandidateException,
                           "Unknown value for result attribute -- %d", output->getResult());
        break;
    }
  }
...
void MVCandidates::analyzeCandidateMVs(QRJbbSubsetPtr qrJbbSubset) {
  ...
  if (qrJbbSubset->hasGroupBy()) jbbNodeSet.addElement(qrJbbSubset->getRefNum());

  JBBSubset *actualJbbSubset = jbbNodeSet.computeJBBSubset();

  // No jbb subset for single-table queries
  NABoolean handleRewriteOnSingleTable = (CmpCommon::getDefault(MVQR_REWRITE_SINGLE_TABLE_QUERIES) == DF_ON);

  if (actualJbbSubset->getJBBCs().entries() == 0 && !handleRewriteOnSingleTable) {
    QRLogger::log(CAT_SQL_COMP_MVCAND, LL_INFO, "JBBSubset has no JBBCs; skipping candidate analysis");
  ...
  if ((mvqrFavoriteCandidates_ == NULL) && (candidates->entries())) buildFavoritesList(heap);
  ...
  for (CollIndex i = 0; i < candidates->entries(); i++) {
    ...
    if (actualJbbSubset->getJBBCs().entries() == 0) {
      assertLogAndThrow(CAT_SQL_COMP_MVCAND, LL_MVQR_FAIL,
                        ...
                        MVCandidateException, "No JBBCs in jbbsubset analysis");
    ...
    try {
      analyzeCandidate(
          candidate,
          (candidate->isIndirectGroupBy() ? const_cast<JBBSubset &>(actualJbbSubset->getJBB()->getMainJBBSubset())
                                          : *actualJbbSubset),
          rewriteSingleNode, jbbNodeSet, heap);
      ...
      QRLogger::log(CAT_SQL_COMP_MVCAND, LL_MVQR_FAIL, "Skipping candidate MV %s due to exception",
...
ValueId MVCandidates::getRewriteVid(RETDesc *retDesc, QRElementPtr elem, QRCandidatePtr candidate, CollHeap *heap) {
  ...
  for (CollIndex i = 0; i < numCols; i++) {
    if (*colRefName == (*colDescList)[i]->getColRefNameObj()) return retDesc->getValueId(i);
  }
...
ValueId MVCandidates::getBaseColValueId(QRElementPtr referencingElem) {
  if (*referencingElem->getRef() == 'O') referencingElem = static_cast<QROutputPtr>(referencingElem)->getOutputItem();
  ...
  if (elemIdChar == 'X' || elemIdChar == 'S' || elemIdChar == 0) return NULL_VALUE_ID;

  if (elemIdChar != 'J') {
    assertLogAndThrow1(CAT_SQL_COMP_MVCAND, LL_MVQR_FAIL, elemIdChar == 'C', MVCandidateException,
                       "Reference %s does not start with 'J' or 'C'", ref.data());
    return referencingElem->getRefNum();
  }

  ValueId vegrefVid = referencingElem->getRefNum();
  ItemExpr *ie = vegrefVid.getItemExpr();
  assertLogAndThrow1(CAT_SQL_COMP_MVCAND, LL_MVQR_FAIL, ie->getOperatorType() == ITM_VEG_REFERENCE,
                     MVCandidateException, "Reference %s is not to a vegref", ref.data());

  const ValueIdSet &vegMembers = (static_cast<VEGReference *>(ie))->getVEG()->getAllValues();

  ValueId someMemberId = vegMembers.init();
  if (vegMembers.next(someMemberId) && someMemberId.getItemExpr()->getOperatorType() == ITM_BASECOLUMN)
    return someMemberId;

  assertLogAndThrow1(CAT_SQL_COMP_MVCAND, LL_MVQR_FAIL, FALSE, MVCandidateException,
                     "No base column found in vegref referenced by %s", ref.data());
...
ValueId MVCandidates::getVegrefValueId(QRElementPtr referencingElem) {
  ...
  const NAString &ref = referencingElem->getRef();
  if (*ref.data() != 'C') return referencingElem->getRefNum();
  ...
  if (referencedElem && referencedElem->getElementType() == ET_Column) {
    ...
    return (vrid != NULL_VALUE_ID ? vrid : (ValueId)referencingElem->getRefNum());
  } else {
    QRLogger::log(CAT_SQL_COMP_MVCAND, LL_DEBUG, "No vegref found for element refed by id %s, using ref id",
                  ref.toCharStar());
    return referencingElem->getRefNum();
  }
}
...
void MVCandidates::populateOneVidMap(RelExpr *root, ValueIdSet &vegRewriteVids, QRElementPtr elem,
  ...
  rewriteVid = getRewriteVid(root->getRETDesc(), elem, candidate, heap);
  ...
  if (rewriteIE->getOperatorType() == ITM_BASECOLUMN) {
    ...
    if (tableDesc) {
      ...
      if (ie->getOperatorType() == ITM_VEG_REFERENCE) {
        ...
        if (vegrefVid == NULL_VALUE_ID) return;
        if (vidMap.getTopValues().contains(vegrefVid)) return;
        vidMap.addMapEntry(vegrefVid, ie->getValueId());
      } else
        assertLogAndThrow(CAT_SQL_COMP_MVCAND, LL_MVQR_FAIL, FALSE, MVCandidateException,
                          "Unable to get vegref for map value id lower value") baseColVid = getBaseColValueId(elem);
      if (baseColVid != NULL_VALUE_ID) vegRewriteVids.addElement(baseColVid);
    }
  } else {
    assertLogAndThrow(CAT_SQL_COMP_MVCAND, LL_MVQR_FAIL, FALSE, MVCandidateException,
...
void MVCandidates::populateVidMap(RelExpr *root, ElementPtrList &vidMapTopElements, ValueIdSet &vegRewriteVids,
                                  QRCandidatePtr candidate, ValueIdMap &vidMap, CollHeap *heap) {
  QROutputPtr output;
  QRElementPtr elem;
...
void MVCandidates::addBackJoins(QRCandidatePtr candidate, QRTableListPtr bjTables, RelExpr *&node, CollHeap *heap) {
  QRTablePtr table;
  ...
  for (CollIndex jpInx = 0; jpInx < joinPredList->entries(); jpInx++) {
    const ElementPtrList &joinItems = (*joinPredList)[jpInx]->getEqualityList();
    numJoinItems = joinItems.entries();
    assertLogAndThrow(CAT_SQL_COMP_MVCAND, LL_MVQR_FAIL, numJoinItems > 1, MVCandidateException,
                      "Must have at least 2 items in a JoinPred");
    for (CollIndex jiInx = 1; jiInx < numJoinItems; jiInx++) {
      eq = new (heap) BiRelat(ITM_EQUAL, new (heap) ColReference(getColRefName(candidate, joinItems[jiInx - 1], heap)),
                              new (heap) ColReference(getColRefName(candidate, joinItems[jiInx], heap)));
      if (!backJoinPred)
        backJoinPred = eq;
      else
...
static NABoolean igbColMatchesTableNode(QRElementPtr elem, CANodeId tblNode) {
  ...
  assertLogAndThrow1(CAT_SQL_COMP_MVCAND, LL_MVQR_FAIL, refedElem->getElementType() == ET_JoinPred,
                     MVCandidateException, "Column refs element of type %d", refedElem->getElementType());

  NABoolean found = FALSE;
  const ElementPtrList &eqList = (static_cast<QRJoinPredPtr>(refedElem))->getEqualityList();
  for (CollIndex i = 0; !found && i < eqList.entries(); i++) {
    if (eqList[i]->getElementType() == ET_Column) {
      ...
      // handled.
      assertLogAndThrow(CAT_SQL_COMP_MVCAND, LL_MVQR_FAIL, refedElem->getElementType() == ET_Column,
                        MVCandidateException, "Column element of JoinPred refs a non-column");
...
template <class T>
void MVCandidates::addPredsFromList(QRCandidatePtr candidate, Scan *scan, CANodeId tableNodeId,
                                    const NAPtrList<T> &list, CollHeap *heap) {
  ItemExpr *itemExpr;
  ...
  for (CollIndex i = 0; i < list.entries(); i++) {
    ...
    ElementFinderVisitorPtr visitor = new (heap) ElementFinderVisitor(ET_Column, TRUE, ADD_MEMCHECK_ARGS(heap));
    ...
    if (igbColMatchesTableNode(colList[0], tableNodeId)) {
      itemExpr = rewriteItemExpr(colList, ((ValueId)elem->getIDNum()).getItemExpr(), candidate, heap, tableNodeId);
      scan->addSelPredTree(itemExpr);
    }
  }
}

void MVCandidates::addIGBPreds(QRCandidatePtr candidate, Scan *scan, CANodeId tableNodeId, QRJBBPtr jbb,
                               CollHeap *heap) {
...
// MV candidate.
ItemExpr *MVCandidates::getIGBJoinCondOp(QRElementPtr elem, QRCandidatePtr candidate, CANodeIdSet &jbbSubsetNodes,
                                         NABoolean &fromFactTable, CollHeap *heap) {
  if (elem->getElementType() != ET_Column)
    assertLogAndThrow1(CAT_SQL_COMP_MVCAND, LL_MVQR_FAIL, FALSE, MVCandidateException,
                       "Can only handle column join condition operands, not %d", elem->getElementType());
      ...
      for (CollIndex i = 0; i < outputs->entries(); i++) {
        ...
        if (outputItem->getElementType() == ET_MVColumn) {
          ...
          if (mvCol->getRefNum() == col->getIDNum())
            return new (heap) ColReference(getColRefName(candidate, mvCol, heap));
        }
      }
    assertLogAndThrow(CAT_SQL_COMP_MVCAND, LL_MVQR_FAIL, FALSE, MVCandidateException,
...
void MVCandidates::addIGBDimJoins(QRCandidatePtr candidate, CANodeIdSet &jbbSubsetNodes, QRJBBPtr jbbElem,
                                  RelExpr *&node, NodeIdScanHash &dimScanHash, CollHeap *heap) {
  ...
  for (CollIndex jpInx = 0; jpInx < joinPredList->entries(); jpInx++) {
    ...
    assertLogAndThrow(CAT_SQL_COMP_MVCAND, LL_MVQR_FAIL, numJoinItems > 1, MVCandidateException,
                      "Must have at least 2 items in a JoinPred");
    for (CollIndex jiInx = 1; jiInx < numJoinItems; jiInx++) {
      leftChild = getIGBJoinCondOp(joinItems[jiInx - 1]->getReferencedElement(), candidate, jbbSubsetNodes,
                                   op1FromFactTable, heap);
      rightChild =
          getIGBJoinCondOp(joinItems[jiInx]->getReferencedElement(), candidate, jbbSubsetNodes, op2FromFactTable, heap);
      if (op1FromFactTable && op2FromFactTable) continue;  // this join already represented in MV scan node
  ...
  if (joinPredItem) static_cast<Join *>(node)->setJoinPredTree(joinPredItem);
}

void MVCandidates::mapDimOutputs(QRJBBPtr jbbElem, CANodeIdSet &jbbSubsetNodes, NodeIdScanHash &dimScanHash,
                                 ValueIdMap &vidMap, CollHeap *heap) {
    ...
    switch (elem->getElementType()) {
        ...
        if (!jbbSubsetNodes.containsThisId(tblNodeId)) {
          ...
          for (CollIndex qColInx = 0; qColInx < qColList.entries() && !found; qColInx++) {
            if (qColList[qColInx] == colId) {
              ...
              assertLogAndThrow(CAT_SQL_COMP_MVCAND, LL_MVQR_FAIL, dimScan, MVCandidateException,
                                "Dimension scan not found in hash table");
              const ValueIdList &rColList = dimScan->getTableDesc()->getColumnList();
              vidMap.addMapEntry(colId, rColList[qColInx]);
              found = TRUE;
            }
          }
          assertLogAndThrow1(CAT_SQL_COMP_MVCAND, LL_MVQR_FAIL, found, MVCandidateException,
                             "Column value id not found in col desc list: %ud", colId.toUInt32());
        }
        break;
      ...
      case ET_JoinPred: {
        ...
        for (CollIndex i = 0; i < eqList.entries(); i++) {
          ...
          if (eqListElemType == ET_Column || eqListElemType == ET_Expr) elems.insert(eqListElem);
        }
      } break;

      default:
        assertLogAndThrow1(CAT_SQL_COMP_MVCAND, LL_MVQR_FAIL, FALSE, MVCandidateException,
                           "Unexpected element type in findDimOutputs: %d", elem->getElementType());
    }
  }
}
...
static void mapVidsForAggs(RETDesc *retDesc, ValueIdMap &vidMap, CollHeap *heap) {
  ...
  for (CollIndex i = 0; i < numCols; i++) {
    ...
    if (strlen(name) > prefixLen && !strncmp(name, AGG_NAME_PREFIX, prefixLen)) {
      vidMap.addMapEntry(atoi(name + prefixLen), retDesc->getValueId(i));
    }
  }
}

void MVCandidates::analyzeCandidate(QRCandidatePtr candidate, JBBSubset &jbbSubsetToReplace,
                                    NABoolean rewriteSingleNode, CANodeIdSet &jbbSubsetNodes, CollHeap *heap) {
  // If the MV is to be ignored - ignore it.
  const NAString &mvName = candidate->getMVName()->getMVName();
  if (isForbiddenMV(mvName)) {
    QRLogger::log(CAT_SQL_COMP_MVCAND, LL_DEBUG, "MV Candidate %s was ignored because this statement writes to it.",
                  mvName.data());
    return;
  }
  ...
  SetRefVisitorPtr visitor = new (heap) SetRefVisitor(qdescGenerator_.getColTblIdHash(), ADD_MEMCHECK_ARGS(heap));
  ...
  if (mvqrFavoriteCandidates_ != NULL) favoriteMV = isAfavoriteMV(match->getMvName());
  ...
  if (candidate->isIndirectGroupBy()) {
    ...
    for (CollIndex jbbInx = 0; jbbInx < jbbList.entries() && !jbbElem; jbbInx++) {
      if (jbbList[jbbInx]->getIDNum() == jbbId) jbbElem = jbbList[jbbInx];
    }
    assertLogAndThrow1(CAT_SQL_COMP_MVCAND, LL_MVQR_FAIL, jbbElem, MVCandidateException,
  ...
  RelRoot *root = new (heap) RelRoot(node, REL_ROOT, NULL, NULL, NULL, NULL, heap);
  ...
  populateVidMap(normalizedRoot, vidMapTopElements, vegRewriteVids, candidate, vidMap, heap);
  if (candidate->isIndirectGroupBy()) mapDimOutputs(jbbElem, jbbSubsetNodes, dimScanHash, vidMap, heap);
  ...
  if (!candidate->isIndirectGroupBy()) {
    ...
    mapVidNode->pushdownCoveredExpr(mapVidNode->getGroupAttr()->getCharacteristicOutputs(),
                                    mapVidNode->getGroupAttr()->getCharacteristicInputs(), mapVidNode->selectionPred());
    ...
    if (tops.entries() > mapVidNode->getMap().getTopValues().entries()) {
      mapVidNode->clear();
      for (CollIndex i = 0; i < tops.entries(); i++) mapVidNode->addMapEntry(tops[i], bottoms[i]);
    }
  }
  ...
  if (!rewriteSingleNode)
    ...
    assertLogAndThrow(CAT_SQL_COMP_MVCAND, LL_MVQR_FAIL, queryRoot_->child(0)->getOperator().match(REL_SCAN),
                      MVCandidateException,
...
void MVCandidates::findForbiddenMVs() {
  ...
  if (queryRoot_->seenIUD() == FALSE) return;
...
void MVCandidates::collectMVs(RelExpr *topNode, CollHeap *heap) {
  if (topNode->getOperator().match(REL_ANY_GEN_UPDATE)) {
    ...
    if (gu->getTableDesc()->getNATable()->isAnMV()) {
      ...
      if (forbiddenMVs_ == NULL) forbiddenMVs_ = new (heap) LIST(NAString)(heap);
...
NABoolean MVCandidates::isForbiddenMV(const NAString &mvName) {
  if (forbiddenMVs_ == NULL) return FALSE;

  for (CollIndex i = 0; i < forbiddenMVs_->entries(); i++)
    if ((*forbiddenMVs_)[i] == mvName) return TRUE;
