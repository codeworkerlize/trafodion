...
void ItemExprList::insertTree(ItemExpr *tree, OperatorTypeEnum backBoneType, NABoolean flattenSBQ,
  ...
  if (tree->getOperatorType() == backBoneType) {
    for (int i = 0; i < tree->getArity(); i++) {
      ...
      if (child) insertTree(tree->child(i), backBoneType, flattenSBQ, flattenUDF);
    }
  ...
  } else if ((flattenSBQ AND tree->isASubquery())OR(flattenUDF AND(tree->getOperatorType() == ITM_USER_DEF_FUNCTION))
                 AND(NOT tree->nodeIsTransformed()))
  ...
  // flag should be sufficient, but it never hurts to be safe.
  {
    ...
    if (haveRDesc == TRUE) {
...

      {
        ...
        ValueIdProxy *proxyOutput = new (CmpCommon::statementHeap()) ValueIdProxy(tree->getValueId(), proxyId, i);
...
//----------------------------------------------------------------------------
ItemExpr *ItemExprList::convertToItemExpr(ItemExprShapeEnum treeShape) const {
...
void ItemExprList::unparse(NAString &result, PhaseEnum phase, UnparseFormatEnum form, TableDesc *tabId) const {
...
void ItemExprList::display() const { ItemExprList::print(); }  // ItemExprList::display()
...
ItemExprTreeAsList::ItemExprTreeAsList(ExprValueId *treePtr, OperatorTypeEnum op, ItemExprShapeEnum shape) {
  treePtr_ = treePtr;
  operator_ = op;
  shape_ = shape;

  if (shape == BUSHY_TREE) ABORT("can't handle bushy trees for now");
}
...
int ItemExprTreeAsList::entries() const {
  ...
  while (aNode != NULL) {
    result++;
    if (aNode->getOperatorType() == operator_ AND aNode->getArity() >= 2) {
...
void ItemExprTreeAsList::insert(ItemExpr *treeToInsert) {
  if (treePtr_->getPtr() == NULL) {
    *treePtr_ = treeToInsert;
    ...
    switch (operator_) {
      case ITM_AND: {
        ItemExpr *leftExpr = NULL;
        ItemExpr *rightExpr = NULL;

        if (shape_ == RIGHT_LINEAR_TREE) {
          leftExpr = treeToInsert;
          rightExpr = treePtr_->getPtr();
        } else if (shape_ == LEFT_LINEAR_TREE) {
          leftExpr = treePtr_->getPtr();
          rightExpr = treeToInsert;
        } else
          ABORT("can only insert into left-linear or right-linear trees");

        *treePtr_ = new (CmpCommon::statementHeap()) BiLogic(operator_, leftExpr, rightExpr);
      } break;

      case ITM_ITEM_LIST: {
        if (shape_ != RIGHT_LINEAR_TREE) ABORT("can only insert into right-linear trees");

        ItemExpr *aNode = treePtr_->getPtr();
        ItemExpr *pNode = treePtr_->getPtr();
        if (aNode->getOperatorType() != operator_ AND aNode->getArity() < 2) {
          // case of current number of entries equal to 1.
          *treePtr_ = new (CmpCommon::statementHeap()) ItemList(treePtr_->getPtr(), treeToInsert);
        } else {
          ...
          pNode->child(1) = new (CmpCommon::statementHeap()) ItemList(pNode->child(1), treeToInsert);
        }
      }
...
void ItemExprTreeAsList::insertAtTop(ItemExpr *treeToInsert) {
  ...
  if (treePtr_->getPtr() == NULL) {
    ...
    *treePtr_ = new (CmpCommon::statementHeap()) ItemList(treeToInsert, treePtr_->getPtr());
  }
}
...
ItemExpr *ItemExprTreeAsList::remove(ItemExpr *treeToRemove) {
  ...
  if (shape_ != RIGHT_LINEAR_TREE) ABORT("delete is supported for right linear trees only");
  ...
  if (treeToRemove == andNodePtr) {
    ...
    *treePtr_ = NULL;
    ...
    while (andNodePtr != NULL AND andNodePtr->getOperatorType() == operator_ AND andNodePtr->getArity() ==
           2 AND NOT found) {
...
NABoolean ItemExprTreeAsList::contains(const ItemExpr *treeToCheck) {
  ...
  while (aNodePtr != NULL) {
    if (aNodePtr == treeToCheck) return TRUE;

    if (aNodePtr->getOperatorType() == operator_ AND aNodePtr->getArity() >= 2) {
      if (shape_ == RIGHT_LINEAR_TREE)
...
ItemExpr *ItemExprTreeAsList::operator[](CollIndex i) {
  ...
  if (aNodePtr->getOperatorType() != operator_ AND j == 0) return aNodePtr;  // case b
