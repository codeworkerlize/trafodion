...
int AppliedStatMan::hashASM(const CANodeIdSet &key) { return key.hash(); }  // AppliedStatMan::hashASM
...
EstLogPropSharedPtr AppliedStatMan::getStatsForLocalPredsOnGivenCols(CANodeId jbbc, const ValueIdSet colIdSet,
  ...
  // extract the base columns from colIdSet
  ValueIdSet usedCols;
  colIdSet.findAllReferencedBaseCols(usedCols);
  const ValueIdSet tableCols(tableAnalysis->getTableDesc()->getColumnList());
  usedCols.intersectSet(tableCols);

  // get local predicates for all extracted base columns
