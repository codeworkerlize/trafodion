...
CascadesTask::~CascadesTask() {
  if (guidance_ != NULL) guidance_->decrementReferenceCount();
...
NAString CascadesTask::taskText() const { return "Shouldn't ever see a generic task"; }

CascadesGroupId CascadesTask::getGroupId() const { return INVALID_GROUP_ID; }
//<pb>

RelExpr *CascadesTask::getExpr() { return NULL; }
...
OptimizeGroupTask::OptimizeGroupTask(Context *context, Guidance *guidance, NABoolean reoptimize, int parentTaskId,
    ...
    : CascadesTask(guidance, context, parentTaskId, stride),
      ...
      reoptimize_(reoptimize) {}  // OptimizeGroupTask::OptimizeGroupTask

void OptimizeGroupTask::print(FILE *f, const char *prefix, const char *suffix) const {
#ifndef NDEBUG
...
NAString OptimizeGroupTask::taskText() const {
  ...
  sprintf(returnText, "%d %d Optimize Group %d, ", getParentTaskId(), getSubTaskId(), groupId_);
  return NAString(returnText) + context_->getRequirementsString();
}

CascadesGroupId OptimizeGroupTask::getGroupId() const { return groupId_; }
...
NABoolean OptimizeGroupTask::canCauseCycle(const CascadesPlan *plan) const {
  if (CURRSTMT_OPTDEFAULTS->maxDepthToCheckForCyclicPlan() < 0) return FALSE;
  // enumerate below the situations that can cause cyclic plans:
  if (plan && plan->getPhysicalExpr() && plan->getPhysicalExpr()->getOperatorType() == REL_EXCHANGE &&
...
void OptimizeGroupTask::perform(int taskId) {
  ...
  if (NOT context_->findBestSolution() OR reoptimize_) {
    ...
    if (rpp AND(rpp->getSortKey() OR rpp->getArrangedCols() OR rpp->getPartitioningRequirement())) {
      partSortReqPresent = TRUE;
    }

    for (CollIndex ruleId = 0; applRules.nextUsed(ruleId); ++ruleId) {
      Rule *rule = GlobalRuleSet->rule(ruleId);
      if (rule->topMatch(NULL, context_) AND(rule->promiseForOptimization(NULL, guidance_, context_)) > 0) {
        CURRSTMT_OPTGLOBALS->task_list->push(new (CmpCommon::statementHeap())
                                                 ApplyRuleTask(rule, NULL, /* no assoc. expression */
                                                               NULL, guidance_, context_, taskId, ++stride));
    ...
    if (CURRSTMT_OPTDEFAULTS->parallelHeuristic2() AND NOT CURRSTMT_OPTDEFAULTS->pushDownDP2Requested()
            AND NOT context_->reoptimizeExprAndPlans()) {
      delete this;
      return;
    }
    ...
    if (expr != NULL) {
      ...
      for (; expr != NULL; expr = expr->getNextInGroup()) {
        CURRSTMT_OPTGLOBALS->task_list->push(new (CmpCommon::statementHeap())
                                                 OptimizeExprTask(expr, guidance_, context_, taskId, ++stride));
    ...
    if (context_->isNiceContextEnabled() AND partSortReqPresent AND(expr != NULL)
            AND(expr->getOperatorType() != REL_ROOT) AND(context_->getGroupAttr()->getNumBaseTables() > 1)
                AND(exchangeEnforcerWillFire OR sortEnforcerWillFire  // OR optExprTaskWillFire
                    )) {
      delete this;
      return;
    }
    ...
    for (int i = 0; i < countOfPlans; i++) {
      ...
      if (limit_for_CPT < 0) limit_for_CPT = 0;

      if (plan !=
          NULL AND((context_ == plan->getContext()) OR(context_->getSolution() == plan))
          ...
          ((NOT context_->isNiceContextEnabled())OR(NOT partSortReqPresent)
               OR(partSortReqPresent AND NOT(exchangeEnforcerWillFire OR sortEnforcerWillFire)) OR((expr != NULL) AND(
                   (expr->getOperatorType() == REL_ROOT) OR((CmpCommon::getDefault(COMP_BOOL_27) == DF_OFF) AND(
                       (expr->getOperatorType() == REL_EXCHANGE) OR(expr->getOperatorType() == REL_SORT))))))
              AND canCauseCycle(plan) ==
          FALSE
              // skip CPT if we're way over level1SafetyNet
              AND(limit_for_CPT == 0.0 OR(CURRSTMT_OPTDEFAULTS->getTaskCount() <=
        ...
        if (CURRSTMT_OPTDEFAULTS->OPHreuseFailedPlan()
                AND(plan->failedInCurrentPass() OR(plan->getRollUpCost() == NULL))) {
          ...
          if (NOT plan->failedInCurrentPass())
            CURRSTMT_OPTGLOBALS->task_list->push(new (CmpCommon::statementHeap()) CreatePlanTask(
                NULL, plan->getPhysicalExpr(), guidance_, context_, taskId, ++stride,
...
NAString OptimizeExprTask::taskText() const {
  ...
  if (expr_->getArity() == 1)
    ...
    sprintf(childrenGroup, "(%d,%d)", (expr_->child(0)).getGroupId(), (expr_->child(1)).getGroupId());
  childrenGroup[11] = '\0';
#endif

  sprintf(returnText, " of group %d, ", expr_->getGroupId());
  char prefixText[100];
  sprintf(prefixText, "%d %d Optimize Expression ", getParentTaskId(), getSubTaskId());
...
CascadesGroupId OptimizeExprTask::getGroupId() const { return expr_->getGroupId(); }
...
void OptimizeExprTask::perform(int taskId) {
  ...
  if (context_->isNiceContextEnabled()) {
    const ReqdPhysicalProperty *rpp = context_->getReqdPhysicalProperty();
    if (rpp AND(rpp->getSortKey() OR rpp->getArrangedCols() OR(
            rpp->getPartitioningRequirement() AND NOT rpp->getPartitioningRequirement()
                ->castToRequireReplicateNoBroadcast())) AND(context_->getGroupAttr()->getNumBaseTables() > 1)) {
  ...
  if (CURRSTMT_OPTDEFAULTS->optimizerHeuristic3()) {
    ...
    if (localView && localView->hasMandatoryXP() && !localView->getJBB()->hasMandatoryXP()) {
      implement = FALSE;
    }
  ...
  for (CollIndex ruleId = 0; applRules.nextUsed(ruleId); ++ruleId) {
    ...
    if (rule->topMatch(expr_, context_) AND(promise = rule->promiseForOptimization(expr_, guidance_, context_)) >
        0 AND
            ...
            NOT expr_->getContextInsensRules()
                .contains(ruleId) AND NOT expr_->getContextSensRules()
                .getTriedRules(context_)
                .contains(ruleId)) {
      ...
      NABoolean checkForce = ((context_ != NULL) AND(context_->getReqdPhysicalProperty() != NULL)
                                  AND(context_->getReqdPhysicalProperty()->getMustMatch() != NULL));

      if (!checkForce OR NOT rule->isContextSensitive()
               OR rule->canMatchPattern(context_->getReqdPhysicalProperty()->getMustMatch())) {
      ...
      if (rule->isContextSensitive())
        ...
        if (NOT(GlobalRuleSet->getCurrentPassNumber() == GlobalRuleSet->getFirstPassNumber() AND ruleId ==
                JoinToTSJRuleNumber AND expr_->getArity() > 1 AND(expr_->child(0)
                                                                      .getGroupAttr()
                                                                      ->isEmbeddedUpdateOrDelete() OR expr_->child(0)
                                                                      .getGroupAttr()
                                                                      ->isStream()))) {
  ...
  opt_qsort((char *)promisingMove, numberOfMoves, sizeof(RuleWithPromise), comparePromisingMoves);

  // determine how many of the promising moves to pursue
  numberOfMoves = expr_->computeOptimizeExprCutoff(promisingMove, numberOfMoves, guidance_, context_);
  ...
  while (--numberOfMoves >= 0) {
    ...
    CURRSTMT_OPTGLOBALS->task_list->push(new (CmpCommon::statementHeap())
                                             ApplyRuleTask(rule, expr_, NULL, guidance_, context_, taskId, ++stride));
    ...
    for (int childIndex = arity; --childIndex >= 0;) {
      ...
      if (NOT patternInput->isCutOp()
              AND NOT(patternInput->isSubtreeOp() AND NOT((SubtreeOp *)patternInput)->all_bindings())) {
  ...
  if (expr_->isPhysical())
    CURRSTMT_OPTGLOBALS->task_list->push(new (CmpCommon::statementHeap())
                                             CreatePlanTask(NULL, expr_, NULL, context_, taskId, ++stride));
...
ExploreGroupTask::ExploreGroupTask(CascadesGroupId group_no, RelExpr *pattern, Guidance *guidance, int parentTaskId,
    ...
    : CascadesTask(guidance, NULL, parentTaskId, stride),
      groupId_(group_no),
      pattern_(pattern) {}  // ExploreGroupTask::ExploreGroupTask
...
NAString ExploreGroupTask::taskText() const {
  ...
  sprintf(returnText, "%d %d Explore Group %d, ", getParentTaskId(), getSubTaskId(), groupId_);
  return returnText;
}
...
void ExploreGroupTask::perform(int taskId) {
  ...
  if (  //(*CURRSTMT_OPTGLOBALS->memo)[groupId_]->getExploredInPass() != currentPass OR
    ...
    for (RelExpr *expr = (*CURRSTMT_OPTGLOBALS->memo)[groupId_]->getFirstLogExpr(); expr != NULL;
         expr = expr->getNextInGroup())
      if (expr->isLogical())
        CURRSTMT_OPTGLOBALS->task_list->push(new (CmpCommon::statementHeap())
                                                 ExploreExprTask(expr, pattern_, guidance_, taskId, ++stride));

    // if this is a "vanilla" exploration, remember it in the group
    if (guidance_ == NULL) (*CURRSTMT_OPTGLOBALS->memo)[groupId_]->setExploredInPass(currentPass);
...
void ExploreExprTask::print(FILE *f, const char *, const char *suffix) const {
  ...
  expr_->print(f, "\t");
  if (pattern_ != NULL) pattern_->print(f, "\tpattern: ", suffix);
#endif  // NDEBUG
}  // ExploreExprTask::print

NAString ExploreExprTask::taskText() const {
  char returnText[100], prefix[100];
  sprintf(prefix, "%d %d Explore Expression \"", getParentTaskId(), getSubTaskId());
  sprintf(returnText, "\" of group %d", expr_->getGroupId());

  if (pattern_ != NULL)
    return NAString(prefix) + expr_->getText() + returnText + " for pattern \"" + pattern_->getText() + "\"";
  else
    return NAString(prefix) + expr_->getText() + returnText;
}

//<pb>
CascadesGroupId ExploreExprTask::getGroupId() const { return expr_->getGroupId(); }
...
void ExploreExprTask::perform(int taskId) {
  ...
  for (CollIndex ruleId = 0; applRules->nextUsed(ruleId); ruleId++) {
    ...
    if (rule->isTransformationRule()) {
      ...
      if (NOT expr_->getContextInsensRules().contains(ruleId)) {
        ...
        if (rule->topMatch(expr_, NULL) AND(promise = rule->promiseForExploration(expr_, pattern_, guidance_)) > 0) {
        ...
        // Otherwise, mark the transformation rule as a "tried" rule.
          ...
          // the JoinToTSJRule fire in Pass2, so don't mark the
          // JoinToTSJRule as a tried rule for them in Pass1.
          if (NOT((GlobalRuleSet->getCurrentPassNumber() == GlobalRuleSet->getFirstPassNumber())
                      AND(ruleId == JoinToTSJRuleNumber) AND expr_->getOperator()
                          .match(REL_ANY_NON_TSJ_JOIN))) {
            expr_->contextInsensRules() += ruleId;
          }
        }
  ...
  // determine how many of the promising moves to pursue
  numberOfMoves = expr_->computeExploreExprCutoff(promisingMove, numberOfMoves, pattern_, guidance_);
  ...
  while (--numberOfMoves >= 0) {
    ...
    // last task: apply the rule
    CURRSTMT_OPTGLOBALS->task_list->push(new (CmpCommon::statementHeap())
                                             ApplyRuleTask(rule, expr_, pattern_, guidance_, NULL, taskId, ++stride));
    ...
    for (int childIndex = arity; --childIndex >= 0;) {
      ...
      else if (patternInput->isSubtreeOp() AND NOT((SubtreeOp *)patternInput)->all_bindings())
        ;   // do nothing -- no expansion required
...
#ifndef NDEBUG
  fprintf(f, "%sapply rule \"%s\" on group %d -- parent task %d subtask %d\n", prefix, rule_->getName(), getGroupId(),
          parentTaskId_, stride_);
  if (expr_ != NULL) expr_->print(f, CONCAT(prefix, "\torig expr = "));
  if (pattern_ != NULL) pattern_->print(f, CONCAT(prefix, "\tpattern = "));
  if (context_ != NULL) context_->print(f, CONCAT(prefix, "\tcontext = "));
#endif  // NDEBUG
}  // ApplyRuleTask::print
//<pb>
NAString ApplyRuleTask::taskText() const {
  char returnText[100], prefix[100];
  sprintf(prefix, "%d %d Apply ", getParentTaskId(), getSubTaskId());
  sprintf(returnText, "\" on group %d", getGroupId());
  if (expr_ != NULL)
    return NAString(prefix) + "Rule \"" + rule_->getName() + "\" on expr \"" + expr_->getText() + returnText;
  else
    return NAString(prefix) + "Enforcer Rule \"" + rule_->getName() + returnText;
}
...
void ApplyRuleTask::perform(int taskId) {
  if (((expr_ == NULL AND NOT context_->getTriedEnforcerRules().contains(rule_->getNumber()))
           OR(expr_ != NULL AND((rule_->isContextSensitive() AND NOT expr_->getContextSensRules().applied(
                           context_, rule_->getNumber()))
                                    OR(NOT rule_->isContextSensitive() AND NOT expr_->getContextInsensRules().contains(
                                        rule_->getNumber())))))
          AND((GlobalRuleSet->getCurrentPassNumber() == 1) OR NOT taskNumberExceededLimit(taskId))) {
    //---------------------------------------------------------
    ...
    if (expr_ != NULL)
      binding = new (CmpCommon::statementHeap()) CascadesBinding(expr_, rule_->getPattern(), NULL, TRUE, FALSE);
    else {
      CMPASSERT(rule_->getPattern() != NULL AND rule_->getPattern()->isCutOp());
      binding = new (CmpCommon::statementHeap()) CascadesBinding(getGroupId(), rule_->getPattern(), NULL, TRUE, FALSE);
    }
    ...
    for (; binding->advance(); binding->release_expr()) {
      ...
      NABoolean memorySafe = CURRSTMT_OPTDEFAULTS->getMemUsed() < CURRSTMT_OPTDEFAULTS->getMemUsageSafetyNet();

      if ((checkForce AND memorySafe)OR NOT CURRSTMT_OPTDEFAULTS->pruneByOptDefaults(rule_, before)) {
        ...
        while (count == 0 OR ruleMemory != NULL) {
          ...
          CMPASSERT(after->getGroupAttr() == before->getGroupAttr() OR * (after->getGroupAttr()) ==
                    *(before->getGroupAttr()));
          ...
          new_expr = CURRSTMT_OPTGLOBALS->memo->include(after, duplicateExprFlag, groupMergeFlag,
                                                        FALSE,  // soln-10-070330-3667
                                                        getGroupId(), context_, before);
          ...
          while (new_expr != NULL) {
            ...
            {
              CURRSTMT_OPTGLOBALS->task_list->push(new (CmpCommon::statementHeap()) ExploreExprTask(
                  new_expr, pattern_, rule_->guidanceForExploringSubstitute(guidance_), taskId, ++stride));
            ...
            else  // optimizer is optimizing
            {
              ...
              if (new_expr->isLogical()) {
                  ...
                  for (int i = 0; i < numContexts; i++) {
                    ...
                    if (otherContext != NULL AND context_ !=
                      ...
                      if (CmpCommon::getDefault(COMP_BOOL_120) == DF_ON)
                        ...
                        CURRSTMT_OPTGLOBALS->task_list->insertTask(
                            new (CmpCommon::statementHeap()) OptimizeExprTask(
                                new_expr, rule_->guidanceForOptimizingSubstitute(guidance_, otherContext), otherContext,
                                taskId, ++stride),
                            otherContext->getPredecessorTask());
                      else
                        // insert this new OptimizeExprTask to be performed just
                        // before the stored predecessor task for this context.
                        CURRSTMT_OPTGLOBALS->task_list->insertOptimizeExprTask(
                            new (CmpCommon::statementHeap()) OptimizeExprTask(
                                new_expr, rule_->guidanceForOptimizingSubstitute(guidance_, otherContext), otherContext,
                                taskId, ++stride),
                            otherContext->getPredecessorTask());
                ...
                // for this context.
                CURRSTMT_OPTGLOBALS->task_list->push(new (CmpCommon::statementHeap()) OptimizeExprTask(
                    new_expr, rule_->guidanceForOptimizingSubstitute(guidance_, context_), context_, taskId, ++stride));
              ...
              if (new_expr->isPhysical()) {
                ...
                CURRSTMT_OPTGLOBALS->task_list->push(new (CmpCommon::statementHeap()) CreatePlanTask(
                    rule_, new_expr, guidance_, context_, taskId, ++stride));
              }  // for a physical operator, optimize the children
  ...
  if (context_ != NULL) context_->decrOutstanding();
...
CreatePlanTask::CreatePlanTask(Rule *rule, RelExpr *expr, Guidance *guidance, Context *context, int parentTaskId,
  ...
  if (failedPlan) {
    ...
    plan_ = new (CmpCommon::statementHeap()) CascadesPlan(expr, context);
...
CreatePlanTask::~CreatePlanTask() { delete workSpace_; }  // CreatePlanTask::~CreatePlanTask

CascadesGroupId CreatePlanTask::getGroupId() const { return plan_->getPhysicalExpr()->getGroupId(); }

RelExpr *CreatePlanTask::getExpr() { return NULL; }
...
void CreatePlanTask::perform(int taskId) {
  ...
  if ((CmpCommon::getDefault(NSK_DBG_PRINT_TASK) == DF_ON) && (CmpCommon::getDefault(NSK_DBG) == DF_ON)) {
    showThisPlan = TRUE;
    if (CmpCommon::getDefault(NSK_DBG_PRINT_COST) == DF_ON) showPlanCost = TRUE;
  }
...
#endif
  childContext = plan_->getPhysicalExpr()->createPlan(context_,        // in
                                                      workSpace_,      // in
                                                      rule_,           // in
                                                      guidance_,       // in
                                                      childGuidance);  // out
  ...
  if (childContext != NULL) {
    ...
    if (NOT CURRSTMT_OPTDEFAULTS->optimizerPruning()
            OR NOT(childContext->hasSolution() AND childContext->optimizedInCurrentPass()))
      CURRSTMT_OPTGLOBALS->task_list->push(new (CmpCommon::statementHeap())
                                               OptimizeGroupTask(childContext, childGuidance, FALSE, taskId, ++stride));
    ...
    if (CURRSTMT_OPTDEFAULTS->optimizerPruning() AND NOT workSpace_->allChildrenContextsConsidered()) {
    ...
    // properties.
    if (plan_->getRollUpCost() != NULL AND(planSatisfied = context_->satisfied(plan_))) {
      Cyes++;
      // Success!!!
      ...
      if ((oldSolution != NULL) AND oldSolution->succeededInCurrentPass()) {
        ...
        if (plan_->getRollUpCost()->compareCosts(*context_->getSolution()->getRollUpCost(),
                                                 context_->getReqdPhysicalProperty()) == LESS) {
          // found a better solution
          context_->setSolution(plan_);

          if (showThisPlan) out << prefix << "*** Chosen plan ***" << endl << endl;
          ...
          if (CURRSTMT_OPTDEFAULTS->OPHuseEnforcerPlanPromotion()) {
            OperatorTypeEnum op = plan_->getPhysicalExpr()->getOperatorType();
            if ((op == REL_SORT) OR(op == REL_EXCHANGE)) {
              CascadesPlan *planToPromote = (CascadesPlan *)plan_->getContextForChild(0)->getSolution();
              if (context_->satisfied(planToPromote)) {
                // child satisfies context without enforcer
                // and can be promoted
                context_->setSolution(planToPromote);
                if (showThisPlan) {
                  out << endl << prefix << "*** Promoted plan ***" << endl;
                  CURRCONTEXT_OPTDEBUG->showTree(planToPromote->getPhysicalExpr(), planToPromote, prefix, showPlanCost);
                }
              }
            }
          }
          ...
          oldSolution = plan_;
          if (showThisPlan) out << prefix << "*** Non-optimal plan ***" << endl << endl;
        }
      ...
      if (CURRSTMT_OPTDEFAULTS->OPHuseFailedPlanCost() AND plan_->exceededCostLimit())
      ...
      if ((oldSolution != NULL) AND NOT oldSolution->succeededInCurrentPass()) {
    ...
    if (context_->getOutstanding() == 0) {
      ...
      // cost limit
      if (CURRSTMT_OPTDEFAULTS->OPHusePWSflagForContext()
              AND CURRSTMT_OPTDEFAULTS->OPHuseFailedPlanCost() AND(NOT context_->hasSolution())
                  AND context_->currentPlanExceededCostLimit()) {
...
NAString CreatePlanTask::taskText() const {
  ...
  if (ex->getArity() == 1)
    ...
    sprintf(childrenGroup, "(%d,%d)", (ex->child(0)).getGroupId(), (ex->child(1)).getGroupId());
  childrenGroup[11] = '\0';
#endif

  sprintf(tmpText, "%d %d Create plan (%d) for ", getParentTaskId(), getSubTaskId(),
          workSpace_->getCountOfChildContexts());
...
//<pb>
GarbageCollectionTask::GarbageCollectionTask() : CascadesTask(NULL, NULL, 0, 0) { alreadyDone_ = FALSE; }

GarbageCollectionTask::~GarbageCollectionTask() {}

void GarbageCollectionTask::print(FILE *f, const char *prefix, const char * /*suffix*/) const {
...
void GarbageCollectionTask::garbageCollection(RelExpr * /* oldx */, RelExpr * /* newx */, int groupMergeCount) {
  ...
  if (groupMergeCount >= CURRSTMT_OPTGLOBALS->group_merge_count) alreadyDone_ = FALSE;
}
