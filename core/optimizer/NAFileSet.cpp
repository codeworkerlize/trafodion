...
NAFileSet::NAFileSet(const NAFileSet &other, CollHeap *h)
    : fileSetName_(other.fileSetName_, h),
      ...
      keysDesc_((other.keysDesc_) ? TrafDesc::copyDescList(other.keysDesc_, h) : NULL),
      ...
      hbaseCreateOptions_((other.hbaseCreateOptions_) ?  // shallow copied
                              new (h) NAList<HbaseCreateOption *>(*other.hbaseCreateOptions_, h)
                                                      : NULL),
...
NAFileSet::~NAFileSet() { delete partFunc_; }

NABoolean NAFileSet::operator==(const NAFileSet &other) const {
  if (this == &other) return TRUE;

  // Compare each data member.
  if (!(fileSetName_ == other.fileSetName_) || !(extFileSetName_ == other.extFileSetName_) ||
      !(extFileSetObj_ == other.extFileSetObj_) || fileOrganization_ != other.fileOrganization_ ||
      countOfFiles_ != other.countOfFiles_ || estimatedNumberOfRecords_ != other.estimatedNumberOfRecords_)
    return FALSE;

  if (recordLength_ != other.recordLength_ || keyLength_ != other.keyLength_ ||
      encodedKeyLength_ != other.encodedKeyLength_ || lockLength_ != other.lockLength_ ||
      fileCode_ != other.fileCode_ || blockSize_ != other.blockSize_ || packingScheme_ != other.packingScheme_ ||
      packingFactor_ != other.packingFactor_ || indexLevels_ != other.indexLevels_ ||
      !(allColumns_ == other.allColumns_) || !(indexKeyColumns_ == other.indexKeyColumns_) ||
      indexUID_ != other.indexUID_ || !COMPARE_TRAFDESC_PTRS(keysDesc_, other.keysDesc_) ||
      !(partitioningKeyColumns_ == other.partitioningKeyColumns_) ||
      !(hiveSortKeyColumns_ == other.hiveSortKeyColumns_))
    return FALSE;

  if (keytag_ != other.keytag_ || redefTime_ != other.redefTime_ || audited_ != other.audited_ ||
      auditCompressed_ != other.auditCompressed_ || compressed_ != other.compressed_ ||
      dcompressed_ != other.dcompressed_ || icompressed_ != other.icompressed_ || buffered_ != other.buffered_ ||
      clearOnPurge_ != other.clearOnPurge_ || isSystemTable_ != other.isSystemTable_ ||
      packedRows_ != other.packedRows_ || hasRemotePartition_ != other.hasRemotePartition_ ||
      setupForStatement_ != other.setupForStatement_ || resetAfterStatement_ != other.resetAfterStatement_ ||
      isDecoupledRangePartitioned_ != other.isDecoupledRangePartitioned_ || bitFlags_ != other.bitFlags_ ||
      thisRemoteIndexGone_ != other.thisRemoteIndexGone_ ||

      numSaltPartns_ != other.numSaltPartns_ || numInitialSaltRegions_ != other.numInitialSaltRegions_ ||
      numTrafReplicas_ != other.numTrafReplicas_ || numMaxVersions_ != other.numMaxVersions_ ||
      !COMPARE_PTRS(hbaseCreateOptions_, other.hbaseCreateOptions_) ||  // NAList<HbaseCreateOption*>
      rowFormat_ != other.rowFormat_)
    return FALSE;

  if (uniqueIndex() != other.uniqueIndex() || ngramIndex() != other.ngramIndex() ||
      isVolatile() != other.isVolatile() || isInMemoryObjectDefn() != other.isInMemoryObjectDefn() ||
      isNullablePkey() != other.isNullablePkey() || partLocalIndex() != other.partLocalIndex() ||
      partGlobalIndex() != other.partGlobalIndex() || partLocalBaseIndex() != other.partLocalBaseIndex())
    return FALSE;
...
NABoolean NAFileSet::isSyskeyLeading() const {
  return (indexKeyColumns_.entries() > 0 && indexKeyColumns_[0]->getPosition() == 0 &&
...
NABoolean NAFileSet::hasOnlySyskey() const {
  ...
  return ((indexKeyColumns_.entries() == 1) && (indexKeyColumns_[0]->isSyskeyColumn()));
}

NABoolean NAFileSet::hasSingleColVarcharKey() const {
  // clustering key is single column varchar
  return ((indexKeyColumns_.entries() == 1) && (indexKeyColumns_[0]->getType()->isVaryingLen()));
}
...
void NAFileSet::resetAfterStatement() {
  ...
  if (partFunc_) partFunc_->resetAfterStatement();

  for (UInt32 i = 0; i < allColumns_.entries(); i++) {
    // reset each NAColumn
    if (allColumns_[i]) allColumns_[i]->resetAfterStatement();
  }

  for (UInt32 j = 0; j < indexKeyColumns_.entries(); j++) {
    // reset each NAColumn
    if (indexKeyColumns_[j]) indexKeyColumns_[j]->resetAfterStatement();
  }
...
void NAFileSet::setupForStatement() {
  ...
  if (partFunc_) partFunc_->setupForStatement();
...
int NAFileSet::getCountOfColumns(NABoolean excludeNonKeyColumns, NABoolean excludeNonUserSpecifiedAlternateIndexColumns,
  ...
  for (CollIndex i = 0; i < colArray->entries(); i++) {
    ...
    if (NOT((excludeNonUserSpecifiedAlternateIndexColumns && nac->getIndexColName() == nac->getColName()) ||
            (excludeSystemColumns && nac->isSystemColumn()) ||
            (excludeAlwaysComputedSystemColumns && nac->isComputedColumnAlways() && nac->isSystemColumn())))
      numCols++;
  }
