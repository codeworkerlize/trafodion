...
RelRoutine::RelRoutine(const RelRoutine &other, CollHeap *h) : RelExpr(other.getOperatorType(), NULL, NULL, h) {
...
void RelRoutine::getPotentialOutputValues(ValueIdSet &outputValues) const {
  outputValues.clear();
  //
  ...
  if (rDesc != NULL) outputValues.insertList(getRoutineDesc()->getOutputColumnList());
...
NABoolean RelRoutine::duplicateMatch(const RelExpr &other) const {
  ...
  if (NOT(procAllParamsVids_ == o.procAllParamsVids_)) return FALSE;
  if (NOT(routineName_ == o.routineName_)) return FALSE;
  if (NOT(arity_ == o.arity_)) return FALSE;
...
RelExpr *RelRoutine::copyTopNode(RelExpr *derivedNode, CollHeap *outHeap) {
  ...
  if (derivedNode == NULL)
    result = new (outHeap) RelRoutine(NULL, REL_ROUTINE, outHeap);
  else
  ...
  if (nodeIsTransformed()) {
    ...
    if (procAllParamsTree_ != NULL)
      result->procAllParamsTree_ = procAllParamsTree_->copyTree(outHeap)->castToItemExpr();
  }
...
void RelRoutine::addLocalExpr(LIST(ExprNode *) & xlist, LIST(NAString) & llist) const {
  if (procAllParamsTree_ != NULL OR NOT procAllParamsVids_.isEmpty()) {
...
const NAString RelRoutine::getText() const { return "relroutine"; }
...
void TableValuedFunction::getPotentialOutputValues(ValueIdSet &outputValues) const {
  outputValues.clear();
  //
...
NABoolean TableValuedFunction::duplicateMatch(const RelExpr &other) const {
  if (!RelRoutine::duplicateMatch(other)) return FALSE;
...
RelExpr *TableValuedFunction::copyTopNode(RelExpr *derivedNode, CollHeap *outHeap)

{
  TableValuedFunction *result;

  if (derivedNode == NULL) {
    result = new (outHeap) TableValuedFunction(NULL, getOperatorType(), outHeap);
  } else
...
const NAString TableValuedFunction::getText() const { return "TableValuedFunction"; }

// -----------------------------------------------------------------------
// methods for class TableMappingUDFChildInfo
// -----------------------------------------------------------------------

TableMappingUDFChildInfo::TableMappingUDFChildInfo(const TableMappingUDFChildInfo &other) {
...
TableMappingUDF::TableMappingUDF(const TableMappingUDF &other) : TableValuedFunction(other), childInfo_(STMTHEAP) {
...
RelExpr *TableMappingUDF::copyTopNode(RelExpr *derivedNode, CollHeap *outHeap)

{
  TableMappingUDF *result;

  if (derivedNode == NULL) {
    result = new (outHeap) TableMappingUDF(getArity(), NULL, outHeap);
  } else
    result = (TableMappingUDF *)derivedNode;

  for (CollIndex i = 0; i < childInfo_.entries(); i++) {
    TableMappingUDFChildInfo *ci = new (outHeap) TableMappingUDFChildInfo(*(childInfo_[i]));
...
TableMappingUDF *TableMappingUDF::castToTableMappingUDF() { return this; }
...
const NAString TableMappingUDF::getText() const {
  ...
  NAString temp(CmpCommon::statementHeap());
  temp = " params: ";

  const ValueIdList &inVids = getProcInputParamsVids();
  NAString param(CmpCommon::statementHeap());
  inVids.unparse(param);

  return op + getUserTableName().getTextWithSpecialType() + temp + param;
}

PredefinedTableMappingFunction *TableMappingUDF::castToPredefinedTableMappingFunction() { return NULL; }

void TableMappingUDF::addLocalExpr(LIST(ExprNode *) & xlist, LIST(NAString) & llist) const {
...
void TableMappingUDF::transformNode(NormWA &normWARef, ExprGroupId &locationOfPointerToMe) {
...
void TableMappingUDF::primeGroupAnalysis() { RelExpr::primeGroupAnalysis(); };
...
void TableMappingUDF::pushdownCoveredExpr(const ValueIdSet &outputExprOnOperator, const ValueIdSet &newExternalInputs,
  ...
  if (!isNormalized_) {
    ...
    NABoolean status =
        dllInteraction_->describeDataflow(this, exprOnParent, selectionPred(), predsEvaluatedByUDF_, predsToPushDown);
    ...
    udfOutputToChildInputMap_.rewriteValueIdSetDown(predsToPushDown, childPredsToPushDown);

    // remaining selection predicates stay with the parent
    exprOnParent += selectionPred();
    if (nonPredNonOutputExprOnOperator) exprOnParent += *nonPredNonOutputExprOnOperator;

    RelExpr::pushdownCoveredExpr(outputExprOnOperator, newExternalInputs, childPredsToPushDown, &exprOnParent, childId);
    ...
    if (!childPredsToPushDown.isEmpty()) {
      ...
      udfOutputToChildInputMap_.mapValueIdSetUp(predsToPushDown, childPredsToPushDown);
...
void TableMappingUDF::finishSynthEstLogProp() { RelExpr::finishSynthEstLogProp(); };
...
void TableMappingUDF::synthEstLogProp(const EstLogPropSharedPtr &inputEstLogProp) {
  ...
  if (!udfSpecifiedCard) {
    ...
    if (arity < 1) {
      ...
      udfCard = ActiveSchemaDB()->getDefaults().getAsDouble(TMUDF_LEAF_CARDINALITY);
    } else {
      // use information about the function type (e.g. mapper, reducer) to
      // estimate the row count
      CostScalar udfCardFactor = dllInteraction_->getCardinalityScaleFactorFromFunctionType(this);

      // if the function type does not help, use a CQD
      if (udfCardFactor < 0) udfCardFactor = ActiveSchemaDB()->getDefaults().getAsDouble(TMUDF_CARDINALITY_FACTOR);
      ...
      for (CollIndex c = 0; c < getArity(); c++) {
        ...
        if (childCard > udfCard) udfCard = childCard;
      }
  ...
  for (CollIndex c = 0; c < tmudfOutputs.entries(); c++) {
    ...
    if (arity > 0) {
      ...
      if (inputValId != NULL_VALUE_ID) {
        for (CollIndex c = 0; c < arity && !inputHistFound; c++) {
          ...
          inputColStats = child(c).outputLogProp(inputEstLogProp)->getColStats().getColStatsPtrForColumn(inputValId);

          if (inputColStats != NULL) inputHistFound = TRUE;
        }
      }
    }
    ...
    if (!inputHistFound) {
      ...
      if (colUec < 1)
        colUec = udfCard.getValue() * ActiveSchemaDB()->getDefaults().getAsDouble(USTAT_MODIFY_DEFAULT_UEC);

      // create a fake histogram with the specified cardinality and UEC
      udfColStatDescList.addColStatDescForVirtualCol(colUec, udfCard, udfOutputCol, udfOutputCol, udfOutputCol, NULL,
                                                     TRUE);
    } else {
      ColStatDescSharedPtr outputColStatDescSharedPtr(new (HISTHEAP) ColStatDesc(inputColStats, udfOutputCol, HISTHEAP),
                                                      HISTHEAP);
  ...
  // apply predicates
  udfCard = udfColStatDescList.estimateCardinality(udfCard,                /*in*/
  ...
  // for the group.
  myEstProps->pickOutputs(udfColStatDescList, inputEstLogProp, getGroupAttr()->getCharacteristicOutputs(),
...
NABoolean TableMappingUDF::pilotAnalysis(QueryAnalysis *qa) { return RelExpr::pilotAnalysis(qa); };
void TableMappingUDF::jbbAnalysis(QueryAnalysis *qa) { RelExpr::jbbAnalysis(qa); };
...
void TableMappingUDF::predAnalysis(QueryAnalysis *qa) { RelExpr::predAnalysis(qa); };
RelExpr *TableMappingUDF::convertToMultiJoinSubtree(QueryAnalysis *qa) {
  return RelExpr::convertToMultiJoinSubtree(qa);
};
RelExpr *TableMappingUDF::expandMultiJoinSubtree() { return RelExpr::expandMultiJoinSubtree(); };
void TableMappingUDF::analyzeInitialPlan() { RelExpr::analyzeInitialPlan(); };
...
NABoolean TableMappingUDF::patternMatch(const RelExpr &other) const {
  ...
  return (RelExpr::patternMatch(other) && getArity() == other.getArity());
};
NABoolean TableMappingUDF::duplicateMatch(const RelExpr &other) const {
  if (!TableValuedFunction::duplicateMatch(other)) return FALSE;
...
void TableMappingUDF::checkAndCoerceScalarInputParamTypes(BindWA *bindWA) {
  ...
  for (int i = 0; i < getScalarInputParamCount(); i++) {
    ...
    if (!(paramType == inputType)) {
      ...
      Cast *castExpr = new (bindWA->wHeap()) Cast(inputTypeId.getItemExpr(), &paramType, ITM_CAST, TRUE);
...
void TableMappingUDF::createOutputVids(BindWA *bindWA) {
  ...
  for (int i = 0; i < getOutputParamCount(); i++) {
    ...
    NamedTypeToItem *paramTypeItem =
        new (bindWA->wHeap()) NamedTypeToItem(naColumn.getColName().data(), naColumn.mutateType(), bindWA->wHeap());
...
NARoutine *TableMappingUDF::getRoutineMetadata(QualifiedName &routineName, CorrName &tmfuncName, BindWA *bindWA) {
  NARoutine *result = NULL;
  CmpSeabaseDDL cmpSBD((NAHeap *)bindWA->wHeap());

  TrafDesc *tmudfMetadata = cmpSBD.getSeabaseRoutineDesc(routineName.getCatalogName(), routineName.getSchemaName(),
                                                         routineName.getObjectName());

  // IS req 5: If function name not found, output binder error.
  if (NULL == tmudfMetadata) {
    *CmpCommon::diags() << DgSqlCode(-4450) << DgString0(tmfuncName.getQualifiedNameAsString());
    bindWA->setErrStatus();
    return NULL;
  }
  ...
  if (udfType != COM_TABLE_UDF_TYPE) {
    *CmpCommon::diags() << DgSqlCode(-4457) << DgString0(tmfuncName.getQualifiedNameAsString())
...
PredefinedTableMappingFunction::PredefinedTableMappingFunction(int arity, const CorrName &name, ItemExpr *params,
...
OperatorTypeEnum PredefinedTableMappingFunction::nameIsAPredefinedTMF(const CorrName &name) {
  ...
  if (!qualName.getSchemaName().isNull()) return REL_ANY_TABLE_MAPPING_UDF;
  ...
  // for ngram
  else if (funcName == "GENERATE_NGRAM")
    return REL_TABLE_MAPPING_BUILTIN_GENERATE_NGRAM;
  else if (funcName == "GENERATE_WILDCARD_NGRAM")
    return REL_TABLE_MAPPING_BUILTIN_GENERATE_WILDCARD_NGRAM;
  else
...
const NAString PredefinedTableMappingFunction::getText() const {
  ...
  switch (getOperatorType()) {
    case REL_TABLE_MAPPING_BUILTIN_LOG_READER:
      op = "event_log_reader, params: ";
      break;
    case REL_TABLE_MAPPING_BUILTIN_TIMESERIES:
      op = "timeseries, params: ";
      break;
    ...
    case REL_TABLE_MAPPING_BUILTIN_GENERATE_NGRAM:
      op = "generate_ngram, params: ";
      break;
    case REL_TABLE_MAPPING_BUILTIN_GENERATE_WILDCARD_NGRAM:
      op = "generate_wildcard_ngram, params: ";
      break;
    default:
      CMPASSERT(0);
...
PredefinedTableMappingFunction *PredefinedTableMappingFunction::castToPredefinedTableMappingFunction() { return this; }

// a virtual function used to copy most of a Node
RelExpr *PredefinedTableMappingFunction::copyTopNode(RelExpr *derivedNode, CollHeap *outHeap) {
  RelExpr *result;

  if (derivedNode == NULL)
    result = new (outHeap) PredefinedTableMappingFunction(getArity(), getUserTableName(),
...
NARoutine *PredefinedTableMappingFunction::getRoutineMetadata(QualifiedName &routineName, CorrName &tmfuncName,
                                                              BindWA *bindWA) {
  NARoutine *result = NULL;
  ...
  switch (getOperatorType()) {
    ...
    case REL_TABLE_MAPPING_BUILTIN_JDBC:
      ...
      libraryPath += ".jar";
      break;
    // built-in function for ngram
    case REL_TABLE_MAPPING_BUILTIN_GENERATE_NGRAM:
      externalName = "GENERATE_NGRAM";
      libraryPath += getenv("SQ_MBTYPE");
      break;
    case REL_TABLE_MAPPING_BUILTIN_GENERATE_WILDCARD_NGRAM:
      externalName = "GENERATE_WILDCARD_NGRAM";
      libraryPath += getenv("SQ_MBTYPE");
      break;
    default:
      *CmpCommon::diags() << DgSqlCode(-4457) << DgString0(routineName.getQualifiedNameAsString())
  ...
  result = new (bindWA->wHeap()) NARoutine(routineName, bindWA->wHeap());
...
RelExpr *PhysicalTableMappingUDF::copyTopNode(RelExpr *derivedNode, CollHeap *outHeap) {
...
const NAString PhysicalTableMappingUDF::getText() const { return TableMappingUDF::getText(); }
...
const NAString BuiltinTableValuedFunction::getText() const { return "BuiltinTableValuedFunction"; }
...
RelExpr *TableValuedUDF::copyTopNode(RelExpr *derivedNode, CollHeap *outHeap) {
  TableValuedUDF *result;

  if (derivedNode == NULL) {
    result = new (outHeap) TableValuedUDF(NULL, getOperatorType(), outHeap);
  } else
...
const NAString TableValuedUDF::getText() const { return "TableValuedUDF"; }
...
const NAString IsolatedNonTableUDR::getText() const { return "IsolatedNonTableUDR"; }
...
// -----------------------------------------------------------------------
IsolatedScalarUDF::IsolatedScalarUDF(const QualifiedName &routineName, CollHeap *h)
    : IsolatedNonTableUDR(routineName, h, REL_ISOLATED_SCALAR_UDF) {
  if (h)
    pCostMethod_ = new (h) CostMethodIsolatedScalarUDF();
  else
    pCostMethod_ = new (CmpCommon::statementHeap()) CostMethodIsolatedScalarUDF();
}

IsolatedScalarUDF::IsolatedScalarUDF(const QualifiedName &routineName, ItemExprList *inParams, ValueIdList outParams,
                                     CollHeap *h)
    : IsolatedNonTableUDR(routineName, inParams, outParams, h, REL_ISOLATED_SCALAR_UDF) {
  if (h)
    pCostMethod_ = new (h) CostMethodIsolatedScalarUDF();
  else
    pCostMethod_ = new (CmpCommon::statementHeap()) CostMethodIsolatedScalarUDF();
...
const NAString IsolatedScalarUDF::getText() const {
  ...
  if (getRoutineDesc() && getRoutineDesc()->isUUDFRoutine() == TRUE && getRoutineDesc()->getActionNARoutine() &&
...
RelExpr *IsolatedScalarUDF::copyTopNode(RelExpr *derivedNode, CollHeap *outHeap) {
  ...
  if (derivedNode == NULL) {
    result = new (outHeap) IsolatedScalarUDF(getRoutineName(), outHeap);
  } else {
...
void IsolatedScalarUDF::addLocalExpr(LIST(ExprNode *) & xlist, LIST(NAString) & llist) const {
  RelRoutine::addLocalExpr(xlist, llist);
}
...
PhysicalIsolatedScalarUDF::PhysicalIsolatedScalarUDF(CollHeap *heap) : IsolatedScalarUDF(NULL, heap) {
  if (heap)
    pCostMethod_ = new (heap) CostMethodIsolatedScalarUDF();
  else
    pCostMethod_ = new (CmpCommon::statementHeap()) CostMethodIsolatedScalarUDF();
}
...
const NAString ExplainFunc::getText() const { return "explain"; }
...
const NAString StatisticsFunc::getText() const { return "statistics"; }
...
ProxyFunc::ProxyFunc(OperatorTypeEnum otype, ElemDDLNode *columnsFromParser, CollHeap *heap)
...
const char *ProxyFunc::getVirtualTableName() { return virtualTableName_.data(); }

//! ProxyFunc::getNumColumns method
ComUInt32 ProxyFunc::getNumColumns() const {
  ComUInt32 result = 0;
  if (columnListFromParser_) result = columnListFromParser_->entries();
  return result;
}
...
const ElemProxyColDef &ProxyFunc::getColumn(ComUInt32 i) const {
  ...
  ElemProxyColDef *result = (*columnListFromParser_)[i]->castToElemProxyColDef();
  CMPASSERT(result);
  return *result;
}
...
void ProxyFunc::populateColumnDesc(char *tableNam, TrafDesc *&colDescs, int &reclen) const {
  ...
  for (ComUInt32 colNum = 0; colNum < numCols; colNum++) {
    ElemProxyColDef *result = (*columnListFromParser_)[colNum]->castToElemProxyColDef();
  ...
  ComTdbVirtTableColumnInfo *colInfoArray =
      (ComTdbVirtTableColumnInfo *)new (STMTHEAP) char[numCols * sizeof(ComTdbVirtTableColumnInfo)];

  cmpSBD.buildColInfoArray(COM_USER_DEFINED_ROUTINE_OBJECT, FALSE, &colArray, colInfoArray, FALSE, FALSE, NULL, NULL,
                           NULL, NULL, CmpCommon::statementHeap());
  ...
  colDescs = cmpSBD.convertVirtTableColumnInfoArrayToDescStructs(&tableName, colInfoArray, numCols);
...
NABoolean ProxyFunc::duplicateMatch(const RelExpr &other) const {
  if (!BuiltinTableValuedFunction::duplicateMatch(other)) return FALSE;
  ProxyFunc &o = (ProxyFunc &)other;
  if (columnListFromParser_ != o.columnListFromParser_) return FALSE;
  if (virtualTableName_ != o.virtualTableName_) return FALSE;
  return TRUE;
}
...
SPProxyFunc::SPProxyFunc(ElemDDLNode *columnsFromParser, ItemExprList *optionalStrings, CollHeap *heap)
    : ProxyFunc(REL_SP_PROXY, columnsFromParser, heap), optionalStrings_(optionalStrings) {}

//! SPProxyFunc::SPProxyFunc Constructor
SPProxyFunc::SPProxyFunc(CollHeap *heap) : ProxyFunc(REL_SP_PROXY, NULL, heap), optionalStrings_(NULL) {}

//! SPProxyFunc::SPProxyFunc Copy Constructor
SPProxyFunc::SPProxyFunc(const SPProxyFunc &other, CollHeap *h)
    : ProxyFunc(other, h), optionalStrings_(other.optionalStrings_) {}
...
ComUInt32 SPProxyFunc::getNumOptionalStrings() const {
  ComUInt32 result = 0;
  if (optionalStrings_) result = optionalStrings_->entries();
  return result;
}
...
const char *SPProxyFunc::getOptionalString(ComUInt32 i) const {
  ...
  if (i < getNumOptionalStrings()) e = (*optionalStrings_)[i];

  if (e) cv = ((ItemExpr *)e)->castToConstValue(dummyNegate);

  if (cv) s = cv->getRawText();
...
const NAString SPProxyFunc::getText() const { return "sp_result_set"; }
...
NABoolean SPProxyFunc::duplicateMatch(const RelExpr &other) const {
  if (!ProxyFunc::duplicateMatch(other)) return FALSE;
  SPProxyFunc &o = (SPProxyFunc &)other;
  if (optionalStrings_ != o.optionalStrings_) return FALSE;
  return TRUE;
}
...
void CallSP::pushdownCoveredExpr(const ValueIdSet &outputExprOnOperator, const ValueIdSet &newExternalInputs,
  ...
  RelExpr::pushdownCoveredExpr(emptySet, newExternalInputs, predicatesOnOperator, &getNeededValueIds());
}
...
ExtractSource::ExtractSource(ElemDDLNode *columnsFromParser, NAString &espPhandle, NAString &securityKey,
                             CollHeap *heap)
...
ExtractSource::ExtractSource(CollHeap *heap)
    : ProxyFunc(REL_EXTRACT_SOURCE, NULL, heap), espPhandle_("", heap), securityKey_("", heap) {}
//! ExtractSource::ExtractSource Copy Constructor
ExtractSource::ExtractSource(const ExtractSource &other, CollHeap *h)
    : ProxyFunc(other, h), espPhandle_(other.espPhandle_), securityKey_(other.securityKey_) {}
...
const NAString ExtractSource::getText() const { return "extract_source"; }
...
NABoolean ExtractSource::duplicateMatch(const RelExpr &other) const {
  ...
  if (espPhandle_ != o.espPhandle_) return FALSE;
  if (securityKey_ != o.securityKey_) return FALSE;
  return TRUE;
}

// -----------------------------------------------------------------------
// methods for class QueryInvalidationFunc
// -----------------------------------------------------------------------
RelExpr *QueryInvalidationFunc::copyTopNode(RelExpr *derivedNode, CollHeap *outHeap) {
  QueryInvalidationFunc *result;

  if (derivedNode == NULL)
    result = new (outHeap) QueryInvalidationFunc(NULL, outHeap);
  else
    result = (QueryInvalidationFunc *)derivedNode;

  return BuiltinTableValuedFunction::copyTopNode(result, outHeap);
}

const NAString QueryInvalidationFunc::getText() const { return "queryinvalidation"; }
