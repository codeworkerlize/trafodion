...
OptRangeSpec::OptRangeSpec(QRDescGenerator *descGenerator, CollHeap *heap, logLevel ll)
  ...
  assertLogAndThrow(CAT_SQL_COMP_RANGE, logLevel_, descGenerator, QRLogicException,
                    "OptRangeSpec constructed for null QRDescGenerator");

  if (descGenerator->isDumpMvMode()) setDumpMvMode();
}
...
ValueId OptRangeSpec::getBaseCol(const ValueIdSet &vegMembers) {
  for (ValueId id = vegMembers.init(); vegMembers.next(id); vegMembers.advance(id)) {
    if (id.getItemExpr()->getOperatorType() == ITM_BASECOLUMN) return id;
  }
...
ValueId OptRangeSpec::getBaseCol(const ValueId vegRefVid) {
  ...
  if (opType == ITM_BASECOLUMN) return vegRefVid;

  // Get the value id of the first member that is a base column.
  assertLogAndThrow1(CAT_SQL_COMP_RANGE, logLevel_, opType == ITM_VEG_REFERENCE, QRDescriptorException,
                     "OptRangeSpec::getBaseCol() expected value id of a "
                     "vegref, not of op type -- %d",
                     opType);

  ValueId baseColVid = getBaseCol(static_cast<VEGReference *>(itemExpr)->getVEG()->getAllValues());
  assertLogAndThrow(CAT_SQL_COMP_RANGE, logLevel_, baseColVid != NULL_VALUE_ID, QRDescriptorException,
                    "Vegref contains no base columns");
  return baseColVid;
}

OptRangeSpec *OptRangeSpec::createRangeSpec(QRDescGenerator *descGenerator, ItemExpr *predExpr, CollHeap *heap,
  ...
  if (predExpr->getOperatorType() == ITM_RANGE_SPEC_FUNC) {
    assertLogAndThrow(CAT_SQL_COMP_RANGE, LL_ERROR, !createForNormalizer, QRDescriptorException,
    ...
    if (rcvid != NULL_VALUE_ID) {
      ...
      if (ie->getOperatorType() == ITM_VEG_REFERENCE) {
        if (descGenerator->isJoinPredId(rcvid)) {
          ...
          if (rcvid != NULL_VALUE_ID) range->setRangeColValueId(rcvid);
        }
      }
    }
  } else {
    if (createForNormalizer) {
      ...
      (static_cast<OptNormRangeSpec *>(range))->setOriginalItemExpr(predExpr);
    } else
...
OptRangeSpec::OptRangeSpec(const OptRangeSpec &other, CollHeap *heap)
  ...
  if (other.rangeExpr_) rangeExpr_ = other.rangeExpr_->copyTree(mvqrHeap_);
}

QRRangePredPtr OptRangeSpec::createRangeElem() {
  QRTRACER("createRangeElem");
  QRRangePredPtr rangePredElem = new (mvqrHeap_) QRRangePred(ADD_MEMCHECK_ARGS(mvqrHeap_));
  rangePredElem->setRangeItem(genRangeItem());

  NABoolean rangeIsOnCol = (rangeJoinPredId_ != NULL_VALUE_ID || rangeColValueId_ != NULL_VALUE_ID);

  assertLogAndThrow(CAT_SQL_COMP_RANGE, logLevel_, getID() > 0, QRDescriptorException,
  ...
  assertLogAndThrow(CAT_SQL_COMP_RANGE, logLevel_, typePtr, QRDescriptorException,
  ...
  for (CollIndex i = 0; i < numSubranges; i++) {
    ...
    if (subrange.startIsMin() || (i == 0 && rangeIsOnCol && subrange.isMinForType(type))) {
      assertLogAndThrow1(CAT_SQL_COMP_RANGE, logLevel_, i == 0, QRDescriptorException,
                         "Subrange other than 1st is unbounded on low side, "
                         "subrange index %d",
                         i);
      if (subrange.endIsMax() || (i == numSubranges - 1 && rangeIsOnCol && subrange.isMaxForType(type))) {
        ...
        assertLogAndThrow(CAT_SQL_COMP_RANGE, logLevel_, numSubranges == 1, QRDescriptorException,
                          "Range of all values must have a single Subrange.");
        if (nullIncluded_) {
          QRLogger::log(CAT_SQL_COMP_RANGE, LL_INFO, "Range predicate ignored because it spans entire range + NULL.");
          ...
          // remove it.
          QRElementPtr rangeItemElem = rangePredElem->getRangeItem()->getReferencedElement();
          if (rangeItemElem->getIDFirstChar() == 'C') {
            ValueId vid = rangeItemElem->getIDNum();
            if ((static_cast<BaseColumn *>(vid.getItemExpr()))->getNAColumn()->getNotNullNondroppable()) {
    ...
    } else if (subrange.endIsMax() || (i == numSubranges - 1 && rangeIsOnCol && subrange.isMaxForType(type))) {
      assertLogAndThrow1(CAT_SQL_COMP_RANGE, logLevel_, i == numSubranges - 1, QRDescriptorException,
      ...
      // subrange.
      if (!eqOp) eqOp = new (mvqrHeap_) QROpEQ(ADD_MEMCHECK_ARGS(mvqrHeap_));
      ...
      betweenOp->setStartValue(subrange.getStartScalarValElem(mvqrHeap_, type));
      betweenOp->setStartIncluded(subrange.startInclusive());
      betweenOp->setEndValue(subrange.getEndScalarValElem(mvqrHeap_, type));
  ...
  if (eqOp) {
    if (nullIncluded_) eqOp->setNullVal(new (mvqrHeap_) QRNullVal(ADD_MEMCHECK_ARGS(mvqrHeap_)));
    rangePredElem->addOperator(eqOp);
...
QRElementPtr OptRangeSpec::genRangeItem() {
  ...
  if (rangeColValueId_ != NULL_VALUE_ID)
    elem = descGenerator_->genQRColumn(rangeColValueId_, rangeJoinPredId_);
  else if (rangeExpr_)
    elem = descGenerator_->genQRExpr(rangeExpr_, rangeJoinPredId_);
  else {
    assertLogAndThrow(CAT_SQL_COMP_RANGE, logLevel_, rangeJoinPredId_ != NULL_VALUE_ID, QRDescriptorException,
                      "All range value ids are null");
    ValueId colVid = getBaseCol(rangeJoinPredId_);
    elem = descGenerator_->genQRColumn(colVid, rangeJoinPredId_);
  }
  return elem;
}
...
ConstValue *OptRangeSpec::getConstOperand(ItemExpr *predExpr, int constInx) {
  ...
  if (right->getOperatorType() != ITM_CONSTANT) {
    ...
    } else if (right->getOperatorType() == ITM_CAST) {
      Cast *c = (Cast *)right;
      if (c->getArity() == 1 && c->child(0).getPtr()->getOperatorType() == ITM_CONSTANT) {
        ConstValue *cv = (ConstValue *)c->child(0).getPtr();
        if (0 == strncmp(c->getType()->getTypeSQLname().data(), "DATE", 4)) {
          NAString *tmp = new NAString(cv->getText().data() + 1, cv->getText().length() - 2);
          right = literalOfDate(tmp);
          if (right == NULL) return NULL;
        } else if (0 == strncmp(c->getType()->getTypeSQLname().data(), "TIMESTAMP", 9)) {
          NAString *tmp = new NAString(cv->getText().data() + 1, cv->getText().length() - 2);
          right = literalOfTimestamp(tmp);
          if (right == NULL) return NULL;
        } else if (0 == strncmp(c->getType()->getTypeSQLname().data(), "TIME", 4)) {
          NAString *tmp = new NAString(cv->getText().data() + 1, cv->getText().length() - 2);
          right = literalOfTime(tmp);
          if (right == NULL) return NULL;
        } else
          return NULL;
      } else
  ...
  if (colVid != NULL_VALUE_ID) {
    ...
    if (rangeExpr_) return NULL;

    if (rangeColValueId_ == NULL_VALUE_ID) {
      rangeColValueId_ = colVid;
      EqualitySet *eqSet = descGenerator_->getEqualitySet(&rangeColValueId_);
      if (eqSet) {
    ...
    if (rangeColValueId_ != NULL_VALUE_ID) return NULL;

    if (!rangeExpr_) {
      // If more than one node is involved in an expression, it is a
      // residual pred instead of a range pred.
      if (descGenerator_->getExprNode(left) == NULL_CA_ID) return NULL;

      setRangeExpr(left);  // sets rangeExpr_, rangeExprText_
      EqualitySet *eqSet = descGenerator_->getEqualitySet(&rangeExprText_);
      if (eqSet) {
        rangeJoinPredId_ = (QRValueId)eqSet->getJoinPredId();
        setType(eqSet->getType());
      } else {
      ...
      if (rangeExprText_ != exprText) return NULL;
    }
  }

  if (!(QRDescGenerator::typeSupported(getType()))) return NULL;
...
void OptRangeSpec::addSubrange(ConstValue *start, ConstValue *end, NABoolean startInclusive, NABoolean endInclusive) {
  QRTRACER("addSubrange");
  const NAType *type = getType();
  assertLogAndThrow(CAT_SQL_COMP_RANGE, logLevel_, type, QRDescriptorException,
  ...
  if (isDumpMvMode()) {
    ...
    if (start) start->unparse(unparsedStart, OPTIMIZER_PHASE, QUERY_FORMAT);
    if (end) end->unparse(unparsedEnd, OPTIMIZER_PHASE, QUERY_FORMAT);
  }
  ...
  switch (typeQual) {
    ...
    case NA_BOOLEAN_TYPE:
      ...
      if (typeQual == NA_DATETIME_TYPE || typeQual == NA_INTERVAL_TYPE ||
        ...
        if (start) {
          ...
          if ((!startInclusive || valTruncated) && (!valTruncated || !valNegative))
        ...
        if (end) {
          ...
          if ((!endInclusive && !valTruncated) || (valTruncated && valNegative))
        ...
        if (!startOverflowed) sub->setStartInclusive(TRUE);
        if (!endOverflowed) sub->setEndInclusive(TRUE);
    ...
    case NA_CHARACTER_TYPE: {
      ...
      if (type->getDataAlignment() == 2) {
        ...
        sub->setUnparsedEnd(unparsedEnd);
        if (start) {
          headerBytes = startType->getVarLenHdrSize() + startType->getSQLnullHdrSize();
          sub->start.remove(0).append((const NAWchar *)start->getConstValue() + (headerBytes / sizeof(NAWchar)),
                                      (start->getStorageSize() - headerBytes) / sizeof(NAWchar));
        } else
          sub->setStartIsMin(TRUE);
        if (end) {
          headerBytes = endType->getVarLenHdrSize() + endType->getSQLnullHdrSize();
          sub->end.remove(0).append((const NAWchar *)end->getConstValue() + (headerBytes / sizeof(NAWchar)),
                                    (end->getStorageSize() - headerBytes) / sizeof(NAWchar));
        } else
          sub->setEndIsMax(TRUE);
        sub->setStartInclusive(startInclusive);
        sub->setEndInclusive(endInclusive);
        ...
        if (start) {
          headerBytes = startType->getVarLenHdrSize() + startType->getSQLnullHdrSize();
          sub->start.remove(0).append((const char *)start->getConstValue() + headerBytes,
                                      start->getStorageSize() - headerBytes);
        } else
          sub->setStartIsMin(TRUE);
        if (end) {
          headerBytes = endType->getVarLenHdrSize() + endType->getSQLnullHdrSize();
          sub->end.remove(0).append((const char *)end->getConstValue() + headerBytes,
    ...
    case NA_COMPOSITE_TYPE:
      // should not reach here. This type is not supported.
      assertLogAndThrow1(CAT_SQL_COMP_RANGE, logLevel_, FALSE, QRDescriptorException,
                         "Unhandled composite data type: %d", typeQual);
      break;

    default:
      assertLogAndThrow1(CAT_SQL_COMP_RANGE, logLevel_, FALSE, QRDescriptorException, "Unhandled data type: %d",
                         typeQual);
      break;
  }
...
ItemExpr *OptRangeSpec::getCheckConstraintPred(ItemExpr *checkConstraint) {
  if (checkConstraint->getOperatorType() != ITM_CASE) {
    QRLogger::log(CAT_SQL_COMP_RANGE, logLevel_, "Expected ITM_CASE but found operator %d.",
  ...
  if (itemExpr->getOperatorType() != ITM_IF_THEN_ELSE) {
    QRLogger::log(CAT_SQL_COMP_RANGE, logLevel_, "Expected ITM_IF_THEN_ELSE but found operator %d.",
...
void OptRangeSpec::intersectCheckConstraints(QRDescGenerator *descGen, ValueId colValId) {
  ...
  if (itemExpr->getOperatorType() == ITM_VEG_REFERENCE) {
    ...
    const ValueIdSet &vidSet = static_cast<VEGReference *>(itemExpr)->getVEG()->getAllValues();
    for (ValueId vid = vidSet.init(); vidSet.next(vid); vidSet.advance(vid)) {
      if (vid.getItemExpr()->getOperatorType() == ITM_BASECOLUMN) intersectCheckConstraints(descGen, vid);
    }
    return;
  ...
  for (CollIndex c = 0; c < checks.entries(); c++) {
    QRLogger::log(CAT_SQL_COMP_RANGE, LL_DEBUG, "Check constraint on table %s: %s",
                  tbl->getTableName().getObjectName().data(), checks[c]->getConstraintText().data());
  }
#endif

  OptRangeSpec *checkRange = NULL;
  ItemExpr *checkPred;
  const ValueIdList &checkConstraints = (static_cast<BaseColumn *>(itemExpr))->getTableDesc()->getCheckConstraints();
...
void OptRangeSpec::intersectTypeConstraint(QRDescGenerator *descGen, ValueId colValId) {
  ...
  switch (typeQual) {
    case NA_NUMERIC_TYPE: {
      ...
      if (isExact && numType.getFSDatatype() == REC_BIN64_SIGNED) return;  // No type restriction for 64-bit integers
    } break;
...
void OptRangeSpec::addConstraints(QRDescGenerator *descGen) {
  ...
  if (colValId == NULL_VALUE_ID) colValId = getRangeJoinPredId();
  if (colValId == NULL_VALUE_ID) return;
  ...
  if (descGen->getDescriptorType() == ET_QueryDescriptor) intersectCheckConstraints(descGen, colValId);
  //  else
...
void OptRangeSpec::addColumnsUsed(const QRDescGenerator *descGen) {
  if (descGen != descGenerator_) descGenerator_->mergeDescGenerator(descGen);
}
...
NABoolean OptRangeSpec::buildRange(ItemExpr *origPredExpr) {
  ...
  while (currIdx >= 0 && isRange) {
    ...
    switch (op) {
      case ITM_AND:
        ...
        if (leftOp == ITM_LESS || leftOp == ITM_LESS_EQ) {
          if (rightOp == ITM_GREATER || rightOp == ITM_GREATER_EQ) {
            ...
            if (endValue) {
              ...
              if (startValue)
                addSubrange(startValue, endValue, rightOp == ITM_GREATER_EQ, leftOp == ITM_LESS_EQ);
              else
                isRange = FALSE;
            } else
              isRange = FALSE;
          ...
          if (rightOp == ITM_LESS || rightOp == ITM_LESS_EQ) {
            ...
            if (startValue) {
              ...
              if (endValue)
                addSubrange(startValue, endValue, leftOp == ITM_GREATER_EQ, rightOp == ITM_LESS_EQ);
              else
                isRange = FALSE;
            } else
              isRange = FALSE;
        ...
        if (reprocessAND) {
          ...
          leftRange = createRangeSpec(descGenerator_, predExpr->child(0), mvqrHeap_, forNormalizer());
          if (!leftRange)
            isRange = FALSE;
          ...
          if (isRange) {
            rightRange = createRangeSpec(descGenerator_, predExpr->child(1), mvqrHeap_, forNormalizer());
      ...
      case ITM_IS_NOT_NULL: {
        ...
        if (child->getOperatorType() == ITM_VEG_REFERENCE) {
          ...
          if (rangeColValueId_ != NULL_VALUE_ID)
            ...
            if (rangeExprText_ != exprText) isRange = FALSE;
          }
        }
        ...
        if (isRange) {
          if (op == ITM_IS_NULL)
            ...
            assertLogAndThrow(CAT_SQL_COMP_RANGE, logLevel_, op == ITM_IS_NOT_NULL, QRLogicException,
                              "op must be ITM_IS_NOT_NULL here");
            const NAType *type = getType();
            NABuiltInTypeEnum typeQual = type->getTypeQualifier();
            if (typeQual == NA_LOB_TYPE) {
              isRange = FALSE;
              break;
            }
    ...
    if (state[currIdx] == AVR_STATE0) currIdx--;  // Go back to the parent node & continue working on it.
  }
...
ConstValue *OptRangeSpec::reconstituteInt64Value(NAType *type, long val) const {
  ...
  switch (typeQual) {
    ...
    case NA_DATETIME_TYPE: {
      ...
      switch (dtType->getSubtype()) {
        case DatetimeType::SUBTYPE_SQLDate:
          ...
          // proper value.
          DatetimeValue::decodeTimestamp((val + 1) * SubrangeBase::MICROSECONDS_IN_DAY - 1,
                                         dtType->getFractionPrecision(), tsFieldValues);
          dtv.setValue(REC_DATE_YEAR, REC_DATE_DAY, dtType->getFractionPrecision(), tsFieldValues);
          ...
          return new (mvqrHeap_) ConstValue(new (mvqrHeap_) SQLDate(mvqrHeap_, FALSE), (void *)dtv.getValue(),
                                            dtv.getValueLen(), &constValTextStr, mvqrHeap_);
          break;
        ...
        case DatetimeType::SUBTYPE_SQLTimestamp:
          ...
          DatetimeValue::decodeTimestamp(val, dtType->getFractionPrecision(), tsFieldValues);
          dtv.setValue(REC_DATE_YEAR, REC_DATE_SECOND, dtType->getFractionPrecision(), tsFieldValues);
          assertLogAndThrow(CAT_SQL_COMP_RANGE, logLevel_, dtv.isValid(), QRLogicException,
                            "Invalid timestamp value reconstructed from Julian timestamp");
          constValTextStr = dtv.getValueAsString(*dtType);
          return new (mvqrHeap_)
              ConstValue(new (mvqrHeap_) SQLTimestamp(mvqrHeap_, FALSE, dtType->getFractionPrecision()),
                         (void *)dtv.getValue(), dtv.getValueLen(), &constValTextStr, mvqrHeap_);
          break;

        default:
          assertLogAndThrow1(CAT_SQL_COMP_RANGE, logLevel_, FALSE, QRLogicException, "Unknown datetime subtype -- %d",
                             dtType->getSubtype());
          return NULL;
          break;
      }
    ...
    case NA_INTERVAL_TYPE: {
      ...
      switch (intvlType->getEndField()) {
        ...
        default:
          assertLogAndThrow1(CAT_SQL_COMP_RANGE, logLevel_, FALSE, QRLogicException,
                             "Invalid end field for interval type -- %d", intvlType->getEndField());
          break;
      }
      ...
      if (leadingPrec < 2) leadingPrec = 2;
      ...
      assertLogAndThrow2(CAT_SQL_COMP_RANGE, logLevel_, origVal == scaledVal, QRLogicException,
      ...
      return new (mvqrHeap_)
          ConstValue(new (mvqrHeap_) SQLInterval(mvqrHeap_, FALSE, intvlType->getEndField(), leadingPrec,
                                                 intvlType->getEndField(), intvlType->getFractionPrecision()),
                     (void *)intvlVal.getValue(), intvlVal.getValueLen(), &constValTextStr, mvqrHeap_);
    } break;

    default:
      assertLogAndThrow1(CAT_SQL_COMP_RANGE, logLevel_, FALSE, QRLogicException,
                         "Type not handled by reconstituteInt64Value() -- %d", typeQual);
      return NULL;
      break;
  }
...
ConstValue *OptRangeSpec::reconstituteDoubleValue(NAType *type, Float64 val) const {
  ...
  if (typeQual != NA_NUMERIC_TYPE) {
    assertLogAndThrow1(CAT_SQL_COMP_RANGE, logLevel_, FALSE, QRLogicException,
                       "Type not handled by reconstituteDoubleValue() -- %d", typeQual);
    return NULL;
  }
  ...
  return new (mvqrHeap_) ConstValue(constType, &val, constType->getNominalSize(), &constValTextStr, mvqrHeap_);
}  // reconstituteDoubleValue()

// Generate a left-linear OR backbone of equality predicates corresponding
// to the values within the subrange.
ItemExpr *OptRangeSpec::makeSubrangeORBackbone(SubrangeBase *subrange, ItemExpr *subrangeItem) const {
  ...
  assertLogAndThrow1(CAT_SQL_COMP_RANGE, logLevel_,
                     typeQual == NA_DATETIME_TYPE || typeQual == NA_INTERVAL_TYPE ||
                         (typeQual == NA_NUMERIC_TYPE && static_cast<const NumericType *>(type)->isExact()),
...
ItemExpr *OptRangeSpec::makeSubrangeItemExpr(SubrangeBase *subrange, ItemExpr *subrangeItem) const {
  ...
  if ((subrange->getSpecifiedValueCount() > 0)) return makeSubrangeORBackbone(subrange, subrangeItem);
  ...
  if (subrange->isSingleValue()) {
    ...
    itemExpr = parentOfStart = new (mvqrHeap_) BiRelat(ITM_EQUAL, subrangeItem);
  } else if (subrange->endIsMax())
    if (subrange->startInclusive() && subrange->getStartAdjustment() == 0)
      itemExpr = parentOfStart = new (mvqrHeap_) BiRelat(ITM_GREATER_EQ, subrangeItem);
    else
      itemExpr = parentOfStart = new (mvqrHeap_) BiRelat(ITM_GREATER, subrangeItem);
  else if (subrange->startIsMin())
    if (subrange->endInclusive() && subrange->getEndAdjustment() == 0)
      itemExpr = parentOfEnd = new (mvqrHeap_) BiRelat(ITM_LESS_EQ, subrangeItem);
    else
      itemExpr = parentOfEnd = new (mvqrHeap_) BiRelat(ITM_LESS, subrangeItem);
  else {
    itemExpr = new (mvqrHeap_) BiLogic(ITM_AND);
    if (subrange->startInclusive() && subrange->getStartAdjustment() == 0)
      itemExpr->child(0) = parentOfStart = new (mvqrHeap_) BiRelat(ITM_GREATER_EQ, subrangeItem);
    else
      itemExpr->child(0) = parentOfStart = new (mvqrHeap_) BiRelat(ITM_GREATER, subrangeItem);
    if (subrange->endInclusive() && subrange->getEndAdjustment() == 0)
      itemExpr->child(1) = parentOfEnd = new (mvqrHeap_) BiRelat(ITM_LESS_EQ, subrangeItem);
    else
      itemExpr->child(1) = parentOfEnd = new (mvqrHeap_) BiRelat(ITM_LESS, subrangeItem);
  }
  ...
  switch (typeQual) {
    ...
    case NA_INTERVAL_TYPE:
      if (typeQual == NA_DATETIME_TYPE || typeQual == NA_INTERVAL_TYPE ||
        ...
        if (parentOfStart) parentOfStart->child(1) = reconstituteDoubleValue(type, startVal);
        if (parentOfEnd) parentOfEnd->child(1) = reconstituteDoubleValue(type, endVal);
      }
      break;
    ...
    case NA_CHARACTER_TYPE: {
      ...
      if (type->getDataAlignment() == 2) {
        // Unicode string.
        Subrange<RangeWString> *wcharSubrange = (Subrange<RangeWString> *)subrange;
        if (parentOfStart) parentOfStart->child(1) = new (mvqrHeap_) ConstValue(wcharSubrange->start);
        if (parentOfEnd) parentOfEnd->child(1) = new (mvqrHeap_) ConstValue(wcharSubrange->end);
      } else {
        // Latin1 string.
        Subrange<RangeString> *charSubrange = (Subrange<RangeString> *)subrange;
        if (parentOfStart)
          parentOfStart->child(1) = new (mvqrHeap_) ConstValue(charSubrange->start, ((CharType *)type)->getCharSet());
        if (parentOfEnd)
          parentOfEnd->child(1) = new (mvqrHeap_) ConstValue(charSubrange->end, ((CharType *)type)->getCharSet());
      }
    } break;
...
void OptRangeSpec::intersectRange(OptRangeSpec *other) {
  assertLogAndThrow(CAT_SQL_COMP_RANGE, logLevel_, sameRangeSubject(other) || !other->rangeSubjectIsSet(),
                    QRDescriptorException,
...
ItemExpr *OptRangeSpec::getRangeItemExpr(NormWA *normWA) const {
  ...
  if (!subranges_.entries() && !nullIncluded_) rangeItemExpr = new (mvqrHeap_) BoolVal(ITM_RETURN_FALSE);

  // If all values are covered by the range, the underlying predicate is
  // necessarily true; return a TRUE itemexpr.
  if (coversFullRange()) rangeItemExpr = new (mvqrHeap_) BoolVal(ITM_RETURN_TRUE);
  ...
  // to 'a IS NOT NULL'
  if ((rangeItemExpr == NULL) && ((subranges_.entries() == 1) && subranges_[0]->coversFullRange()))
  ...
  if (rangeItemExpr == NULL) {
    ...
    for (CollIndex i = 0; i < subranges_.entries(); i++) {
      ...
      if (!rangeItemExpr)
        ...
        rangeItemExpr = new (mvqrHeap_) BiLogic(ITM_OR, rangeItemExpr, makeSubrangeItemExpr(subrange, subjectItemExpr));
    }

    // If the range permits the null value, OR in an isNull predicate.
    if (nullIncluded_) {
      ItemExpr *isNullItemExpr = new (mvqrHeap_) UnLogic(ITM_IS_NULL, subjectItemExpr);
      if (!rangeItemExpr)
        rangeItemExpr = isNullItemExpr;
      else
        rangeItemExpr = new (mvqrHeap_) BiLogic(ITM_OR, rangeItemExpr, isNullItemExpr);
    }
  }
...
ItemExpr *OptNormRangeSpec::getRangeItemExpr(NormWA *normWA) const {
  ...
  if ((op == ITM_AND) || (op == ITM_OR)) {
    ...
    if ((op == ITM_GREATER_EQ) || (op == ITM_GREATER) || (op == ITM_LESS) || (op == ITM_LESS_EQ)) {
      ...
      if ((op1 == ITM_GREATER_EQ) || (op1 == ITM_GREATER) || (op1 == ITM_LESS) || (op1 == ITM_LESS_EQ)) {
  ...
  } else if ((op == ITM_GREATER_EQ) || (op == ITM_GREATER) || (op == ITM_LESS) || (op == ITM_LESS_EQ)) {
  ...
  if (normWA) rangeItemExpr->normalizeNode(*normWA);
  return rangeItemExpr;
}
...
void OptNormRangeSpec::intersectRange(OptNormRangeSpec *other) {
  ...
  if (originalItemExpr_ && otherItemExpr) {
    originalItemExpr_ = new (mvqrHeap_) BiLogic(ITM_AND, originalItemExpr_, otherItemExpr);
    originalItemExpr_->synthTypeAndValueId(TRUE);
  } else {
    if (!originalItemExpr_) {
      QRLogger::log(CAT_SQL_COMP_RANGE, logLevel_, "OptNormRangeSpec::intersectRange -- originalItemExpr_ is NULL.");
    }
    if (!otherItemExpr) {
      QRLogger::log(CAT_SQL_COMP_RANGE, logLevel_, "OptNormRangeSpec::intersectRange -- other op has NULL ItemExpr.");
    }
  }
...
void OptNormRangeSpec::unionRange(OptNormRangeSpec *other) {
  ...
  ItemExpr *otherItemExpr = const_cast<ItemExpr *>(other->getOriginalItemExpr());
  if (originalItemExpr_ && otherItemExpr) {
    originalItemExpr_ = new (mvqrHeap_) BiLogic(ITM_OR, originalItemExpr_, otherItemExpr);
    originalItemExpr_->synthTypeAndValueId(TRUE);
  } else {
    if (!originalItemExpr_) {
      QRLogger::log(CAT_SQL_COMP_RANGE, logLevel_, "OptNormRangeSpec::unionRange -- originalItemExpr_ is NULL.");
    }
    if (!otherItemExpr) {
      QRLogger::log(CAT_SQL_COMP_RANGE, logLevel_, "OptNormRangeSpec::unionRange -- other op has NULL ItemExpr.");
    }
  }
...
static long getInt64ValueFromDateTime(ConstValue *val, const DatetimeType *rangeColType, const DatetimeType *constType,
  ...
  switch (constType->getSubtype()) {
    case DatetimeType::SUBTYPE_SQLDate:
      ...
      i64val = DatetimeType::julianTimestampValue(dateRep, 11, 0) / SubrangeBase::MICROSECONDS_IN_DAY;
      break;
    ...
    case DatetimeType::SUBTYPE_SQLTimestamp:
      i64val = DatetimeType::julianTimestampValue(valPtr, constValueFracSecPrec ? 11 : 7, constValueFracSecPrec);
      break;

    default:
      assertLogAndThrow1(CAT_SQL_COMP_RANGE, level, FALSE, QRDescriptorException, "Invalid datetime subtype -- %d",
                         constType->getSubtype());
  }
...
static long getInt64ValueFromInterval(ConstValue *constVal, const IntervalType *colIntvlType, NABoolean &truncated,
  ...
  switch (storageSize) {
    ...
    default:
      assertLogAndThrow1(CAT_SQL_COMP_RANGE, level, FALSE, QRDescriptorException,
                         "Invalid interval storage length -- %d", storageSize);
      break;
  }
  ...
  switch (constIntvlType->getEndField()) {
    ...
    default:
      assertLogAndThrow1(CAT_SQL_COMP_RANGE, level, FALSE, QRDescriptorException,
                         "Invalid end field for interval -- %d", constIntvlType->getEndField());
      break;
  }
  ...
  switch (colIntvlType->getEndField()) {
    ...
    default:
      assertLogAndThrow1(CAT_SQL_COMP_RANGE, level, FALSE, QRDescriptorException,
                         "Invalid end field for interval -- %d", colIntvlType->getEndField());
      break;
  }
...
long getInt64Value(ConstValue *val, const NAType *rangeColType, NABoolean &truncated, NABoolean &valWasNegative,
  ...
  if (constValType->getTypeQualifier() == NA_DATETIME_TYPE)
    return getInt64ValueFromDateTime(val, static_cast<const DatetimeType *>(rangeColType),
                                     static_cast<const DatetimeType *>(constValType), truncated, level);
  else if (constValType->getTypeQualifier() == NA_INTERVAL_TYPE)
    return getInt64ValueFromInterval(val, static_cast<const IntervalType *>(rangeColType), truncated, valWasNegative,
                                     level);
  ...
  if (constValNumType->isDecimal()) {
    ...
    switch (storageSize) {
      case 1: {
        assertLogAndThrow1(CAT_SQL_COMP_RANGE, level, isExactNumeric, QRDescriptorException,
      ...
      default:
        assertLogAndThrow1(CAT_SQL_COMP_RANGE, level, FALSE, QRDescriptorException,
                           "Numeric constant of unexpected size: %d bytes", storageSize);
...
double getDoubleValue(ConstValue *val, logLevel level) {
  ...
  switch (valueStorageSize) {
    ...
    {
      assertLogAndThrow1(CAT_SQL_COMP_RANGE, level, isExactNumeric, QRDescriptorException,
                         "const value of size 1 not exact numeric: %d", constValType->getTypeQualifier());
      Int8 i8val;
    ...
    {
      assertLogAndThrow1(CAT_SQL_COMP_RANGE, level, isExactNumeric, QRDescriptorException,
                         "const value of size 2 not exact numeric: %d", constValType->getTypeQualifier());
      Int16 i16val;
    ...
    default:
      assertLogAndThrow1(CAT_SQL_COMP_RANGE, level, FALSE, QRDescriptorException,
                         "Numeric constant of unexpected size: %d bytes", val->getStorageSize());
      return 0;  // avoid warning
  }
}
...
NABoolean RangeInfo::operator==(const RangeInfo &other) {
  if (rangeSpec_->getRangeJoinPredId() != NULL_VALUE_ID)
    return rangeSpec_->getRangeJoinPredId() == other.rangeSpec_->getRangeJoinPredId();
  else if (rangeSpec_->getRangeColValueId() != NULL_VALUE_ID)
    return rangeSpec_->getRangeColValueId() == other.rangeSpec_->getRangeColValueId();
  else
