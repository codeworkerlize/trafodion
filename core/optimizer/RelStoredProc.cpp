...
RelInternalSP::RelInternalSP(const NAString &procName, ItemExpr *params, OperatorTypeEnum otype, CollHeap *oHeap,
  ...
  if (oHeap)
    pCostMethod_ = new (oHeap) CostMethodStoredProc();
  else
    pCostMethod_ = new (CmpCommon::statementHeap()) CostMethodStoredProc();
}
...
RelInternalSP::RelInternalSP(const RelInternalSP &other, CollHeap *h)
  ...
  if (h)
    pCostMethod_ = new (h) CostMethodStoredProc();
  else
    pCostMethod_ = new (CmpCommon::statementHeap()) CostMethodStoredProc();
}
...
void RelInternalSP::addLocalExpr(LIST(ExprNode *) & xlist, LIST(NAString) & llist) const {
...
HashValue RelInternalSP::topHash() { return TableValuedFunction::topHash(); }
...
NABoolean RelInternalSP::duplicateMatch(const RelExpr &other) const {
  ...
  if (!(procName_ == o.procName_)) return FALSE;
...
NABoolean RelInternalSP::isDeleteCacheVirtualTable() const {
  // for delete cache entries, one entry, remove or reset cache
  return (getProcAllParamsVids().entries() == 1 &&
          (procName_.compareTo("NAROUTINECACHEDELETE", NAString::ignoreCase) == 0 ||
           procName_.compareTo("NATABLECACHEDELETE", NAString::ignoreCase) == 0));
}
...
RelExpr *RelInternalSP::bindNode(BindWA *bindWA) {
  ...
  CmpInternalSP *cmpInternalSP = new (bindWA->wHeap()) CmpInternalSP(procName_, bindWA->currentCmpContext());
  ...
  if (getProcAllParamsTree()) {
    ...
    if (isQueryCacheVirtualTable() || isHybridQueryCacheVirtualTable() || isNATableCacheVirtualTable() ||
        isNARoutineCacheVirtualTable()) {
    ...
    } else if (isDeleteCacheVirtualTable()) {
      // verify the cache delete type parameter
      // ?? - this only delete from the user cache.  Any requirement to
      //      delete from the META caches?
      const NAString cacheType = getProcAllParamsVids()[0].getItemExpr()->getText();
      if ((!cacheType.compareTo("'reset'", NAString::ignoreCase) == 0) &&
          (!cacheType.compareTo("'remove'", NAString::ignoreCase) == 0)) {
        bindWA->setErrStatus();
        *(bindWA->currentCmpContext()->diags())
            << DgSqlCode(-arkcmpErrorISPWrongInputType) << DgString0(cacheType.data()) << DgString1(procName_.data());
        return NULL;
      }
      arkcmpInfo_ |= executeInSameArkcmp;
      arkcmpInfo_ |= executeInLocalProcess;
      arkcmpInfo_ |= suppressDefaultSchema;
    } else {  // for other ISPs, without location parameter,
    ...
    for (int i = 0; i < nParams; i++) {
      ...
      Assign *tmpAssign = new (bindWA->wHeap()) Assign(procTypes_[i].getItemExpr(),               // target
                                                       getProcAllParamsVids()[i].getItemExpr());  // source
      const NAType *targetType = tmpAssign->synthesizeType(cmpInternalSP->procName(), i + 1);
      if (!targetType) {
        bindWA->setErrStatus();
        bindWA->currentCmpContext()->statement()->setStoredProc(0);
        return 0;
  ...
  NATable *outNaTable = schemaDB->getNATableDB()->get(&(outCorrName.getExtendedQualNameObj()));
  if (!outNaTable) {
    CmpSPOutputFormat outFormat(bindWA->currentCmpContext());
    if (!cmpInternalSP->OutputFormat(outFormat)) return 0;
  ...
  if (outNaTable) getGroupAttr()->addCharacteristicOutputs(getTableDesc()->getColumnList());
...
void RelInternalSP::transformNode(NormWA &normWARef, ExprGroupId &locationOfPointerToMe) {
  TableValuedFunction::transformNode(normWARef, locationOfPointerToMe);

  getProcAllParamsVids().transformNode(normWARef, locationOfPointerToMe, getGroupAttr()->getCharacteristicInputs());
}
...
RelExpr *RelUtilInternalSP::bindNode(BindWA *bindWA) {
  ...
  ((RelInternalSP *)boundExpr)->setRETDesc(new (bindWA->wHeap()) RETDesc(bindWA));
...
const literalAndEnumStruct SPFlagsValues[] = {
    ...
    {RelInternalSP::suppressDefaultSchema | RelInternalSP::executeInSameArkcmp | RelInternalSP::requiresTMFTransaction,
     "SpCatApiRequest"}};
...
UInt16 getTdmispArkcmpInfo(const char *actualSP) {
  ...
  // anyway.
  return (UInt16)literalToEnum(SPFlagsValues, occurs(SPFlagsValues), actualSP, found);
}
