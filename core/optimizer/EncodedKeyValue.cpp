...
NAString *getMinMaxValue(TrafDesc *column, TrafDesc *key, NABoolean highKey, CollHeap *h) {
  ...
  if (type->supportsSQLnullPhysical()) {
    ...
    for (int i = 0; i < nullHdrSize; i++) buf[i] = '\0';
    buflen -= nullHdrSize;
  }
  ...
  if (highKey == FALSE) {
    ...
    if (NOT key->keysDesc()->isDescending())  // ascending
      type->minRepresentableValue(buf + nullHdrSize, &buflen, &minMaxValue, h);
    else {
      if (type->supportsSQLnull()) {
        minMaxValue = new (h) NAString("NULL");
        nullValue = TRUE;
      } else {
        type->maxRepresentableValue(buf + nullHdrSize, &buflen, &minMaxValue, h);
      }
    }
  } else {
    // high key needed
...
ItemExpr *buildEncodeTree(TrafDesc *column, TrafDesc *key,
  ...
  if (column->columnsDesc()->isCaseInsensitive()) caseinsensitiveEncode = TRUE;

  // cannot have a NULL source value for a non-nullable column
  if (NOT column->columnsDesc()->isNullable() && dataBuffer->length() >= 4 && str_cmp(*dataBuffer, "NULL", 4) == 0) {
    return NULL;
  }

  if (column->columnsDesc()->isNullable() && dataBuffer->length() >= 4 && str_cmp(*dataBuffer, "NULL", 4) == 0) {
    nullValue = TRUE;
    ...
    itemExpr = expGen->createExprTree(ns, CharInfo::UTF8, ns.length(), 1, nullConst);
  } else {
    ns = "CAST ( ";
    ns += *dataBuffer;
    ns += " AS ";
    ...
    itemExpr = expGen->createExprTree(ns, CharInfo::UTF8, ns.length());
  }

  CMPASSERT(itemExpr != NULL);
  ItemExpr *boundItemExpr = itemExpr->bindNode(generator->getBindWA());
  if (boundItemExpr == NULL) return NULL;
  ...
  if (DFS2REC::isSQLVarChar(datatype)) {
    ...
    if (DFS2REC::isCharacterString(datatype))
      vc = "CAST (@A1 as CHAR(";
    else
      vc = "CAST (@A1 as BINARY(";
    ...
    ((NAType *)&(itemExpr->getValueId().getType()))->setNullable(column->columnsDesc()->isNullable());
  }
  ...
  itemExpr = new (expGen->wHeap())
      CompEncode(itemExpr, desc_flag, -1, CollationInfo::Sort, column->columnsDesc()->isNullable());
...
short encodeKeyValues(TrafDesc *column_descs, TrafDesc *key_descs,
  ...
  while (key) {
    ...
    ItemExpr *itemExpr = buildEncodeTree(column, key, inValuesArray[i], &generator, diagsArea);
    if (!itemExpr) return -1;
  ...
  keyEncodeExpr->getExpr()->fixup(0, expGen.getPCodeMode(), (ex_tcb *)space, space, h, FALSE, NULL);
  ...
  if (keyEncodeExpr->getExpr()->eval(workAtp, 0, space) == ex_expr::EXPR_ERROR) error = -1;

  if (deleteLater) delete[] inValuesArray;
