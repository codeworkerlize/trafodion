...
void displayPartitioningFunction(const PartitioningFunction &pf) { pf.display(); }

void displayPartitioningFunction(const PartitioningFunction *pf) {
  if (pf) pf->display();
}

void displayPartitionBoundaries(const RangePartitionBoundaries &pb) { pb.display(); }
...
// -----------------------------------------------------------------------
const LogPhysPartitioningFunction *PartitioningFunction::castToLogPhysPartitioningFunction() const { return NULL; }

const SinglePartitionPartitioningFunction *PartitioningFunction::castToSinglePartitionPartitioningFunction() const {
  return NULL;
}

const ReplicateViaBroadcastPartitioningFunction *PartitioningFunction::castToReplicateViaBroadcastPartitioningFunction()
    const {
  return NULL;
}

const ReplicateNoBroadcastPartitioningFunction *PartitioningFunction::castToReplicateNoBroadcastPartitioningFunction()
    const {
  return NULL;
}

const HashPartitioningFunction *PartitioningFunction::castToHashPartitioningFunction() const { return NULL; }

const TableHashPartitioningFunction *PartitioningFunction::castToTableHashPartitioningFunction() const { return NULL; }

const HashDistPartitioningFunction *PartitioningFunction::castToHashDistPartitioningFunction() const { return NULL; }

const Hash2PartitioningFunction *PartitioningFunction::castToHash2PartitioningFunction() const { return NULL; }

const RangePartitioningFunction *PartitioningFunction::castToRangePartitioningFunction() const { return NULL; }

const RoundRobinPartitioningFunction *PartitioningFunction::castToRoundRobinPartitioningFunction() const {
  return NULL;
}

const SkewedDataPartitioningFunction *PartitioningFunction::castToSkewedDataPartitioningFunction() const {
  return NULL;
};

const HivePartitioningFunction *PartitioningFunction::castToHivePartitioningFunction() const { return NULL; };
...
NABoolean PartitioningFunction::partKeyContainsFloatColumn() const {
  if (NOT getPartitioningKey().isEmpty()) {
    ...
    for (ValueId vid = partKeyColumns.init(); partKeyColumns.next(vid); partKeyColumns.advance(vid)) {
      const NAType &columnType = vid.getType();
      if ((columnType.getTypeQualifier() == NA_NUMERIC_TYPE) AND NOT((NumericType &)columnType).isExact()) {
        return TRUE;
      }
    }  // end for
...
// -----------------------------------------------------------------------
const NodeMap *PartitioningFunction::getNodeMap() const { return nodeMap_; }
...
void PartitioningFunction::useNodeMapFromReqOrChild(PartitioningRequirement *req, PartitioningFunction *childPF,
  ...
  if (req->isRequirementFullySpecified() && (CmpCommon::getDefault(COMP_BOOL_87) != DF_ON) && getNodeMap() &&
    ...
    myNodeMap = ((NodeMap *)childPF->getNodeMap())->synthesizeLogicalMap(partCnt, forESP);
  }
  CMPASSERT(myNodeMap);
...
// -----------------------------------------------------------------------
void PartitioningFunction::replaceNodeMap(NodeMap *nodeMap) {
  if (nodeMap_ != NULL) {
#ifndef NDEBUG
    ...
    if ((collHeap()) && (collHeap() != CmpCommon::statementHeap())) CMPASSERT(FALSE);
#endif  // NDEBUG
...
// -----------------------------------------------------------------------
PartitioningFunction *PartitioningFunction::copy(NAMemory *h) const {
...
void PartitioningFunction::normalizePartitioningKeys(NormWA &normWARef) {
  ...
  if (partitioningExpression_) partitioningExpression_ = partitioningExpression_->normalizeNode(normWARef);

  if (partitionSelectionExpr_) partitionSelectionExpr_ = partitionSelectionExpr_->normalizeNode(normWARef);
...
// --------------------------------------------------------------------
NABoolean PartitioningFunction::isKnownReplicaPartFunc() const {
  return ((isASkewedDataPartitioningFunction() &&
           ((SkewedDataPartitioningFunction *)this)->getSkewProperty().isBroadcasted()) ||
          isAReplicateViaBroadcastPartitioningFunction() || isAReplicateNoBroadcastPartitioningFunction());
}

COMPARE_RESULT
PartitioningFunction::comparePartFuncsForUnion(const PartitioningFunction &other) const {
  ...
  if (getCountOfPartitions() == other.getCountOfPartitions()) return SAME;
...
// -----------------------------------------------------------------------
COMPARE_RESULT
PartitioningFunction::comparePartFuncToFunc(const PartitioningFunction &other) const {
  if ((getPartitioningFunctionType() == other.getPartitioningFunctionType())
          AND(getCountOfPartitions() == other.getCountOfPartitions())
              AND(getPartitioningKey() == other.getPartitioningKey()))
    return SAME;
  else
    return INCOMPATIBLE;
}  // PartitioningFunction::comparePartFuncToFunc

static inline PartitioningFunction *getPhysPartFunc(const PartitioningFunction *x) {
  PartitioningFunction *phys = 0;
  if (x->isALogPhysPartitioningFunction()) {
    phys = x->castToLogPhysPartitioningFunction()->getPhysPartitioningFunction();
  }
  return phys;
}
...
PartitioningFunction *PartitioningFunction::scaleNumberOfPartitions(int &suggestedNewNumberOfPartitions,
  ...
  if (suggestedNewNumberOfPartitions != getCountOfPartitions()) {
    NodeMap *newNodeMap =
        new (heap_) NodeMap(heap_, suggestedNewNumberOfPartitions, NodeMapEntry::ACTIVE, getNodeMap()->type());
...
// -----------------------------------------------------------------------
PartitioningFunction *PartitioningFunction::copyAndRemap(ValueIdMap &map, NABoolean mapItUp) const {
  if (getPartitioningKey().entries() == 0) return (PartitioningFunction *)this;
...
// -----------------------------------------------------------------------
void PartitioningFunction::remapIt(const PartitioningFunction *opf, ValueIdMap &map, NABoolean mapItUp) {
  ...
  if (mapItUp) {
    map.rewriteValueIdSetUp(partitioningKeyColumns_, opf->partitioningKeyColumns_);
    map.rewriteValueIdSetUp(partitioningKeyPredicates_, opf->partitioningKeyPredicates_);
    // Note the pivs don't need mapping, they just need to be copied.
    map.rewriteValueIdSetUp(partitionInputValues_, opf->partitionInputValues_);
    map.rewriteValueIdListUp(partitionInputValuesLayout_, opf->partitionInputValuesLayout_);
  } else {
    map.rewriteValueIdSetDown(opf->partitioningKeyColumns_, partitioningKeyColumns_);
    map.rewriteValueIdSetDown(opf->partitioningKeyPredicates_, partitioningKeyPredicates_);
    // Note the pivs don't need mapping, they just need to be copied.
    map.rewriteValueIdSetDown(opf->partitionInputValues_, partitionInputValues_);
    map.rewriteValueIdListDown(opf->partitionInputValuesLayout_, partitionInputValuesLayout_);
  }

  if (partitioningExpression_)
    partitioningExpression_ = partitioningExpression_->mapAndRewrite(map, NOT mapItUp).getItemExpr();
...
// -----------------------------------------------------------------------
NABoolean PartitioningFunction::shouldUseSynchronousAccess(const ReqdPhysicalProperty *rpp,
  ...
  if (NOT rpp->getLogicalOrderOrArrangementFlag()) {
    ...
    if ((lpr != NULL) AND lpr->getMustUsePapa() AND(lpr->getNumClientsReq() == ANY_NUMBER_OF_PARTITIONS)) return FALSE;
    ...
    if ((CmpCommon::getDefault(ATTEMPT_ASYNCHRONOUS_ACCESS) == DF_OFF)
            OR((lpr != NULL) AND(lpr->getNumClientsReq() != ANY_NUMBER_OF_PARTITIONS)
                   AND(lpr->getNumClientsReq() < getCountOfPartitions()))) {
...
void PartitioningFunction::replacePivs(const ValueIdList &newPivs, const ValueIdSet &newPartKeyPreds) {
  // Redefine PartitioningFunction::replacePivs()
  CMPABORT;
}

PartitioningFunction *PartitioningFunction::createPartitioningFunctionForIndexDesc(IndexDesc *idesc) const {
...
void PartitioningFunction::createPartSelectionExprFromSearchKey(const ValueId beginPartSelId,
...
void PartitioningFunction::preCodeGen(const ValueIdSet &availableValues) {
  ...
  partitioningKeyColumns_.replaceVEGExpressions(availableValues, noExternalInputs, FALSE, NULL, TRUE);
  if (partitioningExpression_) {
    partitioningExpression_ =
        partitioningExpression_->replaceVEGExpressions(availableValues, noExternalInputs, FALSE, NULL,
...
const NAString PartitioningFunction::getText() const {
  CMPABORT;
  return NAString("some type of partitioning function", CmpCommon::statementHeap());
}
...
// -----------------------------------------------------------------------
void PartitioningFunction::print(FILE *ofd, const char *indent, const char *title) const {
  BUMP_INDENT(indent);
  ...
  if (NOT partitioningKeyColumns_.isEmpty()) partitioningKeyColumns_.print(ofd, NEW_INDENT, "Partitioning Key Columns");
  if (partKeyPredsCreated()) fprintf(ofd, "%sPartition Key Predicates Created\n", NEW_INDENT);

  if (NOT partitioningKeyPredicates_.isEmpty())
    partitioningKeyPredicates_.print(ofd, NEW_INDENT, "Partitioning Key Predicates");
  if (NOT partitionInputValues_.isEmpty()) partitionInputValues_.print(ofd, NEW_INDENT, "Partition Input Values");
  if (NOT partitionInputValuesLayout_.isEmpty())
    partitionInputValuesLayout_.print(ofd, NEW_INDENT, "Partition Input Values Layout");

  if (getPartitioningExpression()) {
    NAString partExpr("Partitioning Expression\n", CmpCommon::statementHeap());

    getPartitioningExpression()->unparse(partExpr, DEFAULT_PHASE, EXPLAIN_FORMAT);

    fprintf(ofd, "%s", partExpr.data());
  }

  if (assignPartition()) fprintf(ofd, "%sDoes Partition Assignment\n", NEW_INDENT);
...
SinglePartitionPartitioningFunction::castToSinglePartitionPartitioningFunction() const {
  return this;
}

PartitioningFunction *SinglePartitionPartitioningFunction::copy(NAMemory *h) const {
  return new (h) SinglePartitionPartitioningFunction(*this, h);
}
...
void SinglePartitionPartitioningFunction::replacePivs(const ValueIdList &newPivs, const ValueIdSet &newPartKeyPreds) {
...
// -----------------------------------------------------------------------
ItemExpr *SinglePartitionPartitioningFunction::createPartitioningExpression() {
...
NABoolean SinglePartitionPartitioningFunction::isAGroupingOf(const PartitioningFunction &other,
  ...
  return ((other.castToReplicateViaBroadcastPartitioningFunction() == NULL)
              AND(other.castToReplicateNoBroadcastPartitioningFunction() == NULL));
}

// -----------------------------------------------------------------------
// SinglePartitionPartitioningFunction::shouldUseSynchronousAccess()
// -----------------------------------------------------------------------
NABoolean SinglePartitionPartitioningFunction::shouldUseSynchronousAccess(const ReqdPhysicalProperty *rpp,
                                                                          const EstLogPropSharedPtr &inputLogProp,
                                                                          GroupAttributes *ga) const {
...
// partitioned.
// -----------------------------------------------------------------------
NABoolean SinglePartitionPartitioningFunction::partFuncAndFuncPushDownCompatible(
    const PartitioningFunction &other) const {
  if (other.castToSinglePartitionPartitioningFunction() == 0) return FALSE;

  return TRUE;
}

const NAString SinglePartitionPartitioningFunction::getText() const { return "exactly 1 partition"; }

void SinglePartitionPartitioningFunction::print(FILE *ofd, const char *indent, const char *title) const {
  PartitioningFunction::print(ofd, indent, "SinglePartitionPartitioningFunction");
}
...
PartitioningRequirement *ReplicateViaBroadcastPartitioningFunction::makePartitioningRequirement() {
  return new (CmpCommon::statementHeap()) RequireReplicateViaBroadcast(this);
}

PartitioningFunction *ReplicateViaBroadcastPartitioningFunction::copy(NAMemory *h) const {
  return new (h) ReplicateViaBroadcastPartitioningFunction(*this, h);
}

void ReplicateViaBroadcastPartitioningFunction::createPartitioningKeyPredicates() {
...
void ReplicateViaBroadcastPartitioningFunction::replacePivs(const ValueIdList &newPivs,
...
void ReplicateViaBroadcastPartitioningFunction::print(FILE *ofd, const char *indent, const char *title) const {
  PartitioningFunction::print(ofd, indent, "ReplicateViaBroadcastPartitioningFunction");
}
...
PartitioningRequirement *ReplicateNoBroadcastPartitioningFunction::makePartitioningRequirement() {
  return new (CmpCommon::statementHeap()) RequireReplicateNoBroadcast(this);
}

PartitioningFunction *ReplicateNoBroadcastPartitioningFunction::copy(NAMemory *h) const {
  return new (h) ReplicateNoBroadcastPartitioningFunction(*this, h);
}

void ReplicateNoBroadcastPartitioningFunction::createPartitioningKeyPredicates() {
  // Do nothing, the partitioning key preds of a replication "function"
  // are the empty set. By doing nothing we will return all data for
  // each partition.
  storePartitioningKeyPredicates(ValueIdSet());
...
void ReplicateNoBroadcastPartitioningFunction::replacePivs(const ValueIdList &newPivs,
                                                           const ValueIdSet &newPartKeyPreds) {
  // Do nothing, the pivs of a replication "function"
  // are the empty set.
}
...
void ReplicateNoBroadcastPartitioningFunction::print(FILE *ofd, const char *indent, const char *title) const {
  PartitioningFunction::print(ofd, indent, "ReplicateNoBroadcastPartitioningFunction");
}
...
PartitioningRequirement *HashPartitioningFunction::makePartitioningRequirement() {
  return new (CmpCommon::statementHeap()) RequireHash(this);
}

PartitioningFunction *HashPartitioningFunction::copy(NAMemory *h) const {
  return new (h) HashPartitioningFunction(*this, h);
}

// -----------------------------------------------------------------------
// PartitioningFunction::createBetweenPartitioningKeyPredicates()
// -----------------------------------------------------------------------
void PartitioningFunction::createBetweenPartitioningKeyPredicates(const char *pivLoName, const char *pivHiName,
                                                                  ItemExpr *partNumExpr, NABoolean useHash2Split) {
  if (NOT partKeyPredsCreated() || partNumExpr != NULL) {
    ...
    if (partNumExpr == NULL) partNumExpr = createPartitioningExpression();
    ...
    if (partitionInputValues_.isEmpty()) {
      ...
      loPart = new (CmpCommon::statementHeap())
          HostVar(pivLoName, new (CmpCommon::statementHeap()) SQLInt(CmpCommon::statementHeap(), FALSE, FALSE), TRUE);
      hiPart = new (CmpCommon::statementHeap())
          HostVar(pivHiName, new (CmpCommon::statementHeap()) SQLInt(CmpCommon::statementHeap(), FALSE, FALSE), TRUE);
    ...
    if (useHash2Split) {
      ...
      UInt32 numOfOrigPartns = castToHash2PartitioningFunction()->getCountOfOrigHashPartitions();
    ...
    // lower bound
    rootPtr = new (CmpCommon::statementHeap()) BiRelat(ITM_GREATER_EQ, partNumExpr, loPart, TRUE);
    rootPtr->synthTypeAndValueId();
    setOfPartKeyPredicates += rootPtr->getValueId();

    // upper bound
    rootPtr =
        new (CmpCommon::statementHeap()) BiRelat((useHash2Split ? ITM_LESS_EQ : ITM_LESS), partNumExpr, hiPart, TRUE);
    rootPtr->synthTypeAndValueId();
    setOfPartKeyPredicates += rootPtr->getValueId();
...
void HashPartitioningFunction::createPartitioningKeyPredicates() {
  createBetweenPartitioningKeyPredicates("_sys_HostVarLoHashPart", "_sys_HostVarHiHashPart");
}

// -----------------------------------------------------------------------
// HashPartitioningFunction::replacePivs()
// -----------------------------------------------------------------------
void HashPartitioningFunction::replacePivs(const ValueIdList &newPivs, const ValueIdSet &newPartKeyPreds) {
...
COMPARE_RESULT
HashPartitioningFunction::comparePartFuncToFunc(const PartitioningFunction &other) const {
...
COMPARE_RESULT
HashPartitioningFunction::comparePartKeyToKey(const PartitioningFunction &other) const {
  if (getPartitioningFunctionType() != other.getPartitioningFunctionType()) return INCOMPATIBLE;

  const HashPartitioningFunction &oth = (const HashPartitioningFunction &)other;

  if (keyColumnList_.entries() != oth.keyColumnList_.entries()) return INCOMPATIBLE;

  // compare the key columns and their order
  for (CollIndex i = 0; i < keyColumnList_.entries(); i++) {
    if (keyColumnList_[i] != oth.keyColumnList_[i]) return INCOMPATIBLE;

    if (NOT(originalKeyColumnList_[i].getType() == oth.originalKeyColumnList_[i].getType())) return INCOMPATIBLE;
  }
...
ItemExpr *HashPartitioningFunction::createPartitioningExpression() {
  ...
  for (CollIndex i = 0; i < keyColumnList_.entries(); i++) {
    ...
    ItemExpr *c = getCastedItemExpre(keyColumnList_[i].getItemExpr(), oType, CmpCommon::statementHeap());
  ...
  ItemExpr *partFunc = new (CmpCommon::statementHeap())
      Modulus(buildHashingExpressionForExpr(typedKeyCols.rebuildExprTree(ITM_ITEM_LIST)),
              //     Hash(typedKeyCols.rebuildExprTree(ITM_ITEM_LIST)),
              new (CmpCommon::statementHeap()) SystemLiteral(getCountOfPartitions()));
...
ItemExpr *HashPartitioningFunction::buildHashingExpressionForExpr(ItemExpr *expr) {
...
ItemExpr *HashPartitioningFunction::getHashingExpression() const {
  ...
  if (partExpr) {
    ...
    CMPASSERT(hashExpr AND hashExpr->getOperatorType() == ITM_HASH);
  }
  return hashExpr;
}
...
// -----------------------------------------------------------------------
void HashPartitioningFunction::remapIt(const PartitioningFunction *opf, ValueIdMap &map, NABoolean mapItUp) {
  ...
  if (mapItUp) {
    map.rewriteValueIdListUp(keyColumnList_, opf->castToHashPartitioningFunction()->keyColumnList_);
  } else {
    map.rewriteValueIdListDown(opf->castToHashPartitioningFunction()->keyColumnList_, keyColumnList_);
  }
...
void HashPartitioningFunction::preCodeGen(const ValueIdSet &availableValues) {
  ...
  keyColumnList_.replaceVEGExpressions(availableValues, noExternalInputs, FALSE, NULL, TRUE);
}  // HashPartitioningFunction::preCodeGen()
...
const NAString HashPartitioningFunction::getTextImp(const char *hashType) const {
  ...
  result += ")";
  if (result.contains("randomNum")) result = NAString("round robin partitioned");
  return result;
}

const NAString HashPartitioningFunction::getText() const { return getTextImp("hash"); }

void HashPartitioningFunction::print(FILE *ofd, const char *indent, const char *title) const {
...
static ItemExpr *getCastedSkewValueExpr(const EncodedValue &ev, const NAType &oType, CollHeap *heap) {
  double x = ev.getDblValue();
  return new (heap)
      Cast(new (heap) ConstValue(new (heap) SQLDoublePrecision(heap, FALSE /* no SQL NULL*/), (char *)&x, sizeof(x)),
           &oType);
}
...
// -----------------------------------------------------------------------
const TableHashPartitioningFunction *TableHashPartitioningFunction::castToTableHashPartitioningFunction() const {
  return this;
}
...
// -----------------------------------------------------------------------
// -----------------------------------------------------------------------
void TableHashPartitioningFunction::createPartitionSelectionExprInputs() {
  if (partitionSelectionExprInputs().entries() != 0) return;
  ...
  ItemExpr *numParts = new (heap) HostVar(hvFabricatedName,
                                          // int not null
  ...
  ItemExpr *partNum = new (heap) HostVar(hvFabricatedName,
                                         // int not null
                                         new (heap) SQLInt(CmpCommon::statementHeap(), FALSE, FALSE),
                                         // is system-supplied
                                         TRUE);
...
// key column belongs.
// -----------------------------------------------------------------------
void TableHashPartitioningFunction::normalizePartitioningKeys(NormWA &normWARef) {
...
// -----------------------------------------------------------------------
void TableHashPartitioningFunction::createPartitioningKeyPredicatesForSaltedTable(ValueId saltCol) {
  ...
  createBetweenPartitioningKeyPredicates(NULL, NULL, saltCol.getItemExpr(), isAHash2PartitioningFunction());
...
// -----------------------------------------------------------------------
void TableHashPartitioningFunction::replacePivs(const ValueIdList &newPivs, const ValueIdSet &newPartKeyPreds) {
  // Overwrite the old pivs, part key preds, and part expr. with the new ones.
  storePartitionInputValues(newPivs);
  storePartitioningKeyPredicates(newPartKeyPreds);
}  // TableHashPartitioningFunction::replacePivs()

PartitioningFunction *TableHashPartitioningFunction::createPartitioningFunctionForIndexDesc(IndexDesc *idesc) const {
  // Redefine PartitioningFunction::createPartitioningFunctionForIndexDesc()
  CMPABORT;
  return NULL;
}
...
// -----------------------------------------------------------------------
void TableHashPartitioningFunction::remapIt(const PartitioningFunction *opf, ValueIdMap &map, NABoolean mapItUp) {
  PartitioningFunction::remapIt(opf, map, mapItUp);

  // If we have arrived here, the original partitioning function (*opf)
  // MUST be a TableHashPartitioningFunction().
  //
  const TableHashPartitioningFunction *oth = opf->castToTableHashPartitioningFunction();
...
// are expressed using VEGReferences in terms of the available values.
// -----------------------------------------------------------------------
void TableHashPartitioningFunction::preCodeGen(const ValueIdSet &availableValues) {
  ValueIdSet noExternalInputs;
...
ItemExpr *TableHashPartitioningFunction::createPartitioningExpressionImp(NABoolean doVarCharCast) {
  // already constructed with the same doVarCharCast argument?
  if (getExpression() && doVarCharCast == doVarCharCast_) return getExpression();  // reuse it!
  ...
  for (CollIndex i = 0; i < keyColumnList.entries(); i++) {
    ...
    if (dataConversionErrorFlag == 0) {
      dataConversionErrorFlag =
          new (heap) HostVar("_sys_repartConvErrorFlg", new (heap) SQLInt(heap, TRUE, FALSE), TRUE);
    ...
    if (oType.getTypeQualifier() == NA_CHARACTER_TYPE && doVarCharCast == TRUE) {
      ...
      switch (vId.getItemExpr()->getOperatorType()) {
        ...
        case ITM_VEG_REFERENCE: {
          ...
          for (ValueId x = vidSet.init(); vidSet.next(x); vidSet.advance(x)) {
            ...
            if (maxCharType == NULL OR maxCharType->getNominalSize() <
                                   ctype.getNominalSize() OR(maxCharType->getNominalSize() ==
                                                             ctype.getNominalSize() AND maxCharType->getStrCharLimit() <
                                                                 ctype.getStrCharLimit()))
    ...
    c = new (heap) Narrow(keyColumnList[i].getItemExpr(), dataConversionErrorFlag, finalTypePtr, ITM_NARROW,
                          FALSE /* reverseDataErrorConversionFlag */, TRUE /* match child nullability */);
...
// -----------------------------------------------------------------------
ItemExpr *TableHashPartitioningFunction::createPartitionSelectionExpr(const SearchKey *partSearchKey,
                                                                      const ValueIdSet &availableValues) {
  // If it has already been created, return cached version.
  //
  if (partitionSelectionExpr()) return partitionSelectionExpr();
  ...
  if (!partSearchKey->isUnique()) {
    ...
    for (CollIndex i = 0; i < keyColumns.entries(); i++) {
      ...
      //
      NAType *finalTypePtr = oType.newCopy(heap);
      c = new (heap) Narrow(keyValues[i].getItemExpr(), dataConversionErrorFlag, finalTypePtr);
      c->synthTypeAndValueId();
      typedKeyCols.insert(c->getValueId());
    }
    ...
    //
    partitionSelectionExpr() = buildPartitioningSelectionExpr(typedKeyCols, numParts);
    ...
    //
    partitionSelectionExpr()->replaceVEGExpressions(availableValues, availableValues);
...
// -----------------------------------------------------------------------
SearchKey *TableHashPartitioningFunction::createSearchKey(const IndexDesc *indexDesc, ValueIdSet availInputs,
  ...
  if (indexDesc->getPrimaryTableDesc()->getNATable()->isHbaseTable()) {
    ...
    partSearchKey =
        new (CmpCommon::statementHeap()) SearchKey(indexDesc->getIndexKey(), indexDesc->getOrderOfKeyValues(),
                                                   availInputs, TRUE, preds, nonKeyColumnSet, indexDesc);
  } else {
    // Call this special constructor that constructs a search key for a
    // TableHashPartitioningFunction.
    //
    partSearchKey = new (CmpCommon::statementHeap())
        SearchKey(indexDesc->getPartitioningKey(), indexDesc->getOrderOfPartitioningKeyValues(), availInputs, preds,
                  this, nonKeyColumnSet, indexDesc);
  }

  return partSearchKey;
}  // TableHashPartitioningFunction::createSearchKey()

ItemExpr *TableHashPartitioningFunction::buildHashingExpressionForExpr(ItemExpr *expr) {
  CollHeap *heap = CmpCommon::statementHeap();
...
// -----------------------------------------------------------------------
const HashDistPartitioningFunction *HashDistPartitioningFunction::castToHashDistPartitioningFunction() const {
  return this;
}

PartitioningRequirement *HashDistPartitioningFunction::makePartitioningRequirement() {
  return new (CmpCommon::statementHeap()) RequireHashDist(this);
}

PartitioningFunction *HashDistPartitioningFunction::copy(NAMemory *h) const {
  return new (h) HashDistPartitioningFunction(*this, h);
}
...
const NAString HashDistPartitioningFunction::getText() const {
  ...
  char nparts[40];
  sprintf(nparts, "%d (%d) ways on (", numberOfPartitions_, numberOfOrigHashPartitions_);
  result += nparts;
...
void HashDistPartitioningFunction::print(FILE *ofd, const char *indent, const char *title) const {
...
// -----------------------------------------------------------------------
PartitioningFunction *HashDistPartitioningFunction::createPartitioningFunctionForIndexDesc(IndexDesc *idesc) const {
  ...
  // -----------------------------------------------------------------
  HashDistPartitioningFunction *partFunc = new (idesc->wHeap()) HashDistPartitioningFunction(
      partitioningKey, partitioningKeyList, getCountOfPartitions(), getNodeMap()->copy(idesc->wHeap()));
...
// -----------------------------------------------------------------------
COMPARE_RESULT
HashDistPartitioningFunction::comparePartFuncToFunc(const PartitioningFunction &other) const {
  COMPARE_RESULT c = PartitioningFunction::comparePartFuncToFunc(other);
  ...
  //
  if (getCountOfOrigHashPartitions() != oth->getCountOfOrigHashPartitions()) return INCOMPATIBLE;
...
PartitioningFunction *HashDistPartitioningFunction::scaleNumberOfPartitions(int &suggestedNewNumberOfPartitions,
  ...
  suggestedNewNumberOfPartitions =
      (suggestedNewNumberOfPartitions > numberOfPartitions_) ? numberOfPartitions_ : suggestedNewNumberOfPartitions;
...
// -----------------------------------------------------------------------
NABoolean HashDistPartitioningFunction::isAGroupingOf(const PartitioningFunction &other, int *maxPartsPerGroup) const {
  if (maxPartsPerGroup != NULL) *maxPartsPerGroup = 1;
  ...
  if (keyColumnList_.entries() != oth->keyColumnList_.entries()) return FALSE;

  // compare the key columns and their order
  for (CollIndex i = 0; i < keyColumnList_.entries(); i++) {
    if (keyColumnList_[i] != oth->keyColumnList_[i]) return FALSE;
    if (NOT(originalKeyColumnList_[i].getType() == oth->originalKeyColumnList_[i].getType())) return FALSE;
  }
  ...
  if ((oth->getCountOfPartitions() == oth->getCountOfOrigHashPartitions()) ||
    ...
    if (maxPartsPerGroup != NULL)
      *maxPartsPerGroup = ((oth->getCountOfPartitions() + getCountOfPartitions() - 1) / getCountOfPartitions());
...
// -----------------------------------------------------------------------
ItemExpr *HashDistPartitioningFunction::buildPartitioningExpression(const ValueIdList &keyCols) const {
  CollHeap *heap = CmpCommon::statementHeap();
  ...
  // partitions.
  ItemExpr *partFunc =
      new (heap) ProgDistrib(new (heap) HashDistPartHash(keyCols.rebuildExprTree(ITM_ITEM_LIST)), origNumParts);
...
// -----------------------------------------------------------------------
ItemExpr *HashDistPartitioningFunction::buildPartitioningSelectionExpr(const ValueIdList &keyCols,
                                                                       ItemExpr *numParts) const {
  CollHeap *heap = CmpCommon::statementHeap();

  ItemExpr *partFunc =
      new (heap) ProgDistrib(new (heap) HashDistPartHash(keyCols.rebuildExprTree(ITM_ITEM_LIST)), numParts);
...
// -----------------------------------------------------------------------
const Hash2PartitioningFunction *Hash2PartitioningFunction::castToHash2PartitioningFunction() const { return this; }

PartitioningRequirement *Hash2PartitioningFunction::makePartitioningRequirement() {
  return new (CmpCommon::statementHeap()) RequireHash2(this);
}

PartitioningFunction *Hash2PartitioningFunction::copy(NAMemory *h) const {
  return new (h) Hash2PartitioningFunction(*this, h);
}
...
void Hash2PartitioningFunction::print(FILE *ofd, const char *indent, const char *title) const {
...
// -----------------------------------------------------------------------
PartitioningFunction *Hash2PartitioningFunction::createPartitioningFunctionForIndexDesc(IndexDesc *idesc) const {
  const NAFileSet *fileSet = idesc->getNAFileSet();
  const NAColumnArray &allColumns = fileSet->getAllColumns();
  const NAColumnArray &partKeyColumns = fileSet->getPartitioningKeyColumns();
  ...
  for (CollIndex i = 0; i < partKeyColumns.entries(); i++) {
    // which column of the index is this (usually this will be == i)
    ixColNumber = allColumns.getColumnPosition(partKeyColumns[i]->getColName());
  ...
  // Allocate a new HashPartitioningFunction.
  // -----------------------------------------------------------------
  Hash2PartitioningFunction *partFunc = new (idesc->wHeap()) Hash2PartitioningFunction(
      partitioningKey, partitioningKeyList, getCountOfPartitions(), getNodeMap()->copy(idesc->wHeap()));
...
// -----------------------------------------------------------------------
void Hash2PartitioningFunction::createPartSelectionExprFromSearchKey(const ValueId beginPartSelId,
                                                                     const ValueId endPartSelId,
                                                                     ValueIdList &partSelectionValIds) const {
  ItemExpr *beginPartSelExpr = new (CmpCommon::statementHeap())
      Hash2Distrib(beginPartSelId.getItemExpr(), partitionSelectionExprInputs()[1].getItemExpr());
  beginPartSelExpr->synthTypeAndValueId();

  ItemExpr *endPartSelExpr = new (CmpCommon::statementHeap())
      Hash2Distrib(endPartSelId.getItemExpr(), partitionSelectionExprInputs()[1].getItemExpr());
...
// -----------------------------------------------------------------------
COMPARE_RESULT
Hash2PartitioningFunction::comparePartFuncToFunc(const PartitioningFunction &other) const {
  COMPARE_RESULT c = PartitioningFunction::comparePartFuncToFunc(other);
  ...
  if (getCountOfPartitions() != oth->getCountOfPartitions()) return INCOMPATIBLE;
...
PartitioningFunction *Hash2PartitioningFunction::scaleNumberOfPartitions(int &suggestedNewNumberOfPartitions,
  ...
  if (scaleByFactor) {
    ...
    while (numberOfPartitions_ % suggestedNewNumberOfPartitions != 0) suggestedNewNumberOfPartitions--;
  }
...
NABoolean Hash2PartitioningFunction::isAGroupingOf(const PartitioningFunction &other, int *maxPartsPerGroup) const {
  ...
  if (getCountOfPartitions() > oth->getCountOfPartitions() || oth->getCountOfPartitions() % getCountOfPartitions() != 0)
    return FALSE;
  ...
  if (maxPartsPerGroup != NULL) *maxPartsPerGroup = oth->getCountOfPartitions() / getCountOfPartitions();
...
// -----------------------------------------------------------------------
ItemExpr *Hash2PartitioningFunction::buildPartitioningExpression(const ValueIdList &keyCols) const {
  CollHeap *heap = CmpCommon::statementHeap();
  NAType *numPartsType = new (heap) SQLInt(heap, FALSE, FALSE);
  ...
  ItemExpr *partFunc =
      new (heap) Hash2Distrib(new (heap) HashDistPartHash(keyCols.rebuildExprTree(ITM_ITEM_LIST)), scaledNumParts);
...
//  - build the expression used during partition selection
// -----------------------------------------------------------------------
ItemExpr *Hash2PartitioningFunction::buildPartitioningSelectionExpr(const ValueIdList &keyCols,
                                                                    ItemExpr *numParts) const {
  CollHeap *heap = CmpCommon::statementHeap();

  ItemExpr *partFunc =
      new (heap) Hash2Distrib(new (heap) HashDistPartHash(keyCols.rebuildExprTree(ITM_ITEM_LIST)), numParts);
...
ItemExpr *PartitioningFunction::getCastedItemExpre(ItemExpr *iv, const NAType &oType, CollHeap *heap) {
  ItemExpr *dataConversionErrorFlag = getConvErrorExpr();
  if (dataConversionErrorFlag == 0) {
    dataConversionErrorFlag = new (CmpCommon::statementHeap())
        HostVar("_sys_repartConvErrorFlg",
                new (CmpCommon::statementHeap()) SQLInt(CmpCommon::statementHeap(), TRUE, FALSE), TRUE);
    storeConvErrorExpr(dataConversionErrorFlag);
  }
  ...
  ItemExpr *c = new (CmpCommon::statementHeap()) Narrow(iv, dataConversionErrorFlag, &oType, ITM_NARROW, FALSE,
...
SkewedDataPartitioningFunction::SkewedDataPartitioningFunction(PartitioningFunction *partFuncForUnskewed,
                                                               const skewProperty &sk, NAMemory *heap)
    : PartitioningFunction(SKEWEDDATA_PARTITIONING_FUNCTION, NULL, heap),
      partialPartFunc_(partFuncForUnskewed),
      skewProperty_(sk),
      skewHashList_(NULL) {
...
void SkewedDataPartitioningFunction::createPIV(ValueIdList &partInputValues) {
  ...
  ItemExpr *dummyPIV = new (CmpCommon::statementHeap())
      HostVar("_sys_dummySkewBusterPartNo",
              new (CmpCommon::statementHeap()) SQLInt(CmpCommon::statementHeap(), FALSE, FALSE), TRUE);
...
void SkewedDataPartitioningFunction::replacePivs(const ValueIdList &newPivs, const ValueIdSet &newPartKeyPreds) {
  partialPartFunc_->replacePivs(newPivs, newPartKeyPreds);
}

SkewedDataPartitioningFunction::SkewedDataPartitioningFunction(const SkewedDataPartitioningFunction &other,
                                                               NAMemory *heap)
...
PartitioningRequirement *SkewedDataPartitioningFunction::makePartitioningRequirement() {
  return new (CmpCommon::statementHeap()) RequireSkewed(this);
}

PartitioningFunction *SkewedDataPartitioningFunction::copy(NAMemory *h) const {
  SkewedDataPartitioningFunction *result;

  result = new (h) SkewedDataPartitioningFunction(*this, h);
...
const NAString SkewedDataPartitioningFunction::getText() const {
  ...
  if (CmpCommon::getDefault(SKEW_EXPLAIN) == DF_OFF) return result;
...
void SkewedDataPartitioningFunction::print(FILE *ofd, const char *indent, const char *title) const {
...
// -----------------------------------------------------------------------
COMPARE_RESULT
SkewedDataPartitioningFunction::comparePartFuncToFunc(const PartitioningFunction &other) const {
  // the other has to be a SkewedDataPartitioningFunction
  const SkewedDataPartitioningFunction *oth = other.castToSkewedDataPartitioningFunction();

  if (!oth) return INCOMPATIBLE;

  // compare the two partial partfuncs.
  COMPARE_RESULT c = partialPartFunc_->comparePartFuncToFunc(*(oth->getPartialPartitioningFunction()));

  if (c != SAME) return INCOMPATIBLE;

  // compare the skew property last.
  if (NOT(skewProperty_ == oth->skewProperty_)) return INCOMPATIBLE;
...
PartitioningFunction *SkewedDataPartitioningFunction::scaleNumberOfPartitions(int &suggestedNewNumberOfPartitions,
  ...
  PartitioningFunction *mayBeNewPartfunc =
      partialPartFunc_->scaleNumberOfPartitions(suggestedNewNumberOfPartitions, partGroupDist);

  if (mayBeNewPartfunc != partialPartFunc_) partialPartFunc_ = mayBeNewPartfunc;
...
// -----------------------------------------------------------------------
PartitioningFunction *SkewedDataPartitioningFunction::copyAndRemap(ValueIdMap &map, NABoolean mapItUp) const {
  SkewedDataPartitioningFunction *newPartFunc =
      (SkewedDataPartitioningFunction *)copy()->castToSkewedDataPartitioningFunction();
...
void SkewedDataPartitioningFunction::preCodeGen(const ValueIdSet &availableValues) {
  PartitioningFunction::preCodeGen(availableValues);
...
ItemExpr *SkewedDataPartitioningFunction::createPartitioningExpression() {
  ...
  ItemExpr *partExpr =
      ((TableHashPartitioningFunction *)partialPartFunc_)->createPartitioningExpressionImp(TRUE /*do varchar cast*/);
...
void RangePartitionBoundaries::defineUnboundBoundary(int partitionNumber, const ItemExpr *unboundBoundaryValue,
  ...
  CMPASSERT(partitionNumber > 0 AND partitionNumber <= partitionCount_ AND unboundBoundaryValue != NULL);
...
void RangePartitionBoundaries::bindAddBoundaryValue(int partitionNumber) {
  ...
  ItemExpr *unboundBoundaryValue =
      ((ItemExpr *)boundaryValuesList_[partitionNumber])->copyTree(CmpCommon::statementHeap());
  ...
  ItemExprList *boundBoundaryValue =
      new (CmpCommon::statementHeap()) ItemExprList(unboundBoundaryValue, CmpCommon::statementHeap());
...
void RangePartitionBoundaries::defineBoundary(int partitionNumber, const ItemExprList *boundaryValue,
  ...
  // entry, this gives us n insertion points for a table with n partitions,
  // new entry delimits partition <partitionNumber>
  CMPASSERT(partitionNumber > 0 AND partitionNumber <= partitionCount_ AND boundaryValue != NULL AND encodedKeyValue !=
            NULL);
...
void RangePartitionBoundaries::checkConsistency(const int numberOfPartitions) const {
  ...
  if (encodedBoundaryKeyLength_ > 0) {
    ...
    for (CollIndex index = 1; index < (CollIndex)numberOfPartitions; index++) {
...
NABoolean RangePartitionBoundaries::compareRangePartitionBoundaries(const RangePartitionBoundaries &other,
  ...
  CollIndex myix = 1;
  CollIndex otherix = 1;
  while (myix < (CollIndex)partitionCount_ AND otherix < (CollIndex)other.partitionCount_) {
    match = FALSE;
    ...
    if (*thisBoundaryValue == *otherBoundaryValue) {
      ...
      if (thisNumPartKeyCols != otherNumPartKeyCols) {
        ...
        for (i = 0; i < thisNumPartKeyCols AND match; i++) {
          ...
          thisConstValue = ((*thisBoundaryValue)[i])->castToConstValue(thisIsNegated);
          otherConstValue = ((*otherBoundaryValue)[i])->castToConstValue(otherIsNegated);
          if ((thisIsNegated != otherIsNegated) OR thisConstValue == NULL OR otherConstValue ==
              NULL OR(thisConstValue->getConstStr(FALSE) != otherConstValue->getConstStr(FALSE)))
            match = FALSE;
        }
      }
    }
    ...
    if (match) {
      ...
      myix++;
      otherix++;
      if ((maxPartsPerGroup != NULL) AND(currentPartsPerGroup > *maxPartsPerGroup)) {
  ...
  if ((maxPartsPerGroup != NULL) AND(numOfPartsInLastGroup > *maxPartsPerGroup)) {
...
//
//==============================================================================
RangePartitionBoundaries *RangePartitionBoundaries::merge(const RangePartitionBoundaries &other,
                                                          const NodeMap &thisNodeMap, NodeMap &resultNodeMap) const {
  // need binary boundary values to do a merge
  if (encodedBoundaryKeyLength_ == 0 OR other.encodedBoundaryKeyLength_ == 0) return NULL;
  ...
  while (thisix < maxthis OR otherix < maxother) {
    ...
    if (maxboth - thisix - otherix > 1) {
      ...
      resultNodeMap.setNodeMapEntry(resultix, *entry, CmpCommon::statementHeap());
    }
    ...
    if (otherix >= maxother) {
      ...
      if (thisix >= maxthis) {
        ...
        compResult =
            str_cmp(binaryBoundaryValues_[thisix], other.binaryBoundaryValues_[otherix], encodedBoundaryKeyLength_);
      }
    }
    ...
    if (compResult <= 0) {
      ...
      result->boundaryValues_.insertAt(resultix, boundaryValues_[thisix]);
      result->binaryBoundaryValues_.insertAt(resultix, binaryBoundaryValues_[thisix]);
      thisix++;

      // if the boundaries are identical, skip both of them
      if (compResult == 0) otherix++;
    } else {
      // "other" has the smaller boundary value
      result->boundaryValues_.insertAt(resultix, other.boundaryValues_[otherix]);
      result->binaryBoundaryValues_.insertAt(resultix, other.binaryBoundaryValues_[otherix]);
      otherix++;
    }
...
int RangePartitionBoundaries::getOptimizedNumberOfPartKeys() {
  ...
  for (CollIndex i = 1; i < (CollIndex)partitionCount_; i++) {
    ...
    if (boundaryValues_[i]) {
      ...
      if (numKeySpecs > numPartKeyCols) numPartKeyCols = numKeySpecs;
    } else {
...
int RangePartitionBoundaries::scaleNumberOfPartitions(int suggestedNewNumberOfPartitions, const NodeMap *nodeMap,
  ...
  if (suggestedNewNumberOfPartitions >= partitionCount_) return partitionCount_;
  ...
  if (useAPDistribution) {
    ...
    while ((partNum < partitionCount_) AND(newix != suggestedNewNumberOfPartitions)) {
      // Determine if the current partition is active
      partState = nodeMap->getNodeMapEntry((CollIndex)partNum)->getPartitionState();
      isPartActive = ((partState == NodeMapEntry::ACTIVE) OR(partState == NodeMapEntry::ACTIVE_NO_DATA));

      if ((remainder != 0) AND((newix - 1) + remainder == suggestedNewNumberOfPartitions)) {
  ...
  if (nodeMap->isMultiCluster(prevPartStart, partitionCount_, useAPDistribution)) {
    // More than one cluster in a group - undo.
    boundaryValues_ = undoBoundaryValues;
    binaryBoundaryValues_ = undoBinaryBoundaryValues;
    return partitionCount_;
...
NABoolean RangePartitionBoundaries::isAGroupingOf(const RangePartitionBoundaries &other, int *maxPartsPerGroup) const {
...
void RangePartitionBoundaries::completePartitionBoundaries(const ValueIdList &partitioningKeyOrder,
  ...
  ItemExprList *firstKey = new (CmpCommon::statementHeap()) ItemExprList(CmpCommon::statementHeap());
  ItemExprList *lastKey = new (CmpCommon::statementHeap()) ItemExprList(CmpCommon::statementHeap());
  ...
  for (i = 0; i < partKeyColumnCount_; i++) {
    ...
    ItemExpr *minVal =
        new (CmpCommon::statementHeap()) SystemLiteral(&prototypeType, TRUE /*want min*/, TRUE /*allow NULL*/);
    ItemExpr *maxVal = new (CmpCommon::statementHeap()) SystemLiteral(&prototypeType, FALSE, TRUE);
  ...
  if (binaryBoundaryValues_.entries()) {
    if (!setBinaryBoundaryFirstLastKey_) {
      ...
      char *binaryMin = new (heap_) char[encodedBoundaryKeyLength_];
      char *binaryMax = new (heap_) char[encodedBoundaryKeyLength_];
  ...
  if (useStringVersion) {
    for (UInt32 i = 0; i < boundaryStringsList_.entries(); i++) {
      if (!boundaryStringsList_.used(i) || !boundaryStringsList_[i]) {
        ...
        boundaryValuesList_.insertAt(i, NATable::getRangePartitionBoundaryValues(
                                            boundaryStringsList_[i]->data(), boundaryStringsList_[i]->length(),
...
// Method for debugging.
// -----------------------------------------------------------------------
void RangePartitionBoundaries::print(FILE *ofd, const char *indent, const char *title) const {
  BUMP_INDENT(indent);
  char S[500];
  int index;
  ...
  for (int index2 = 0; index2 < partitionCount_; index2++)
    if (binaryBoundaryValues_.used(index2) && binaryBoundaryValues_[index2]) {
      const char *binaryVal = binaryBoundaryValues_[index2];

      fprintf(ofd, "binary boundary[%d]: 0x",  // %#0*",
              index2);

      for (int b = 0; b < encodedBoundaryKeyLength_; b++) fprintf(ofd, "%02hhx", binaryVal[b]);
      fprintf(ofd, "\n");
    }
...
RangePartitioningFunction::RangePartitioningFunction(const RangePartitioningFunction &other, NAMemory *heap)
    : PartitioningFunction(other, heap),
      ...
      setupForStatement_(FALSE),
      resetAfterStatement_(FALSE) {
  partitionBoundaries_ = new (heap) RangePartitionBoundaries(*(other.partitionBoundaries_), heap);
}
...
PartitioningRequirement *RangePartitioningFunction::makePartitioningRequirement() {
  return new (CmpCommon::statementHeap()) RequireRange(this);
}

PartitioningFunction *RangePartitioningFunction::copy(NAMemory *h) const {
  return new (h) RangePartitioningFunction(*this, h);
}
...
// -----------------------------------------------------------------------
COMPARE_RESULT
RangePartitioningFunction::comparePartFuncToFunc(const PartitioningFunction &other) const {
  ...
  if (isAGroupingOf(other) AND getCountOfPartitions() == other.getCountOfPartitions())
    return SAME;
  else
    return INCOMPATIBLE;
...
NABoolean RangePartitioningFunction::isAGroupingOf(const PartitioningFunction &other, int *maxPartsPerGroup) const {
  ...
  // at this point, sorry)
  if (orderOfKeyValues_.entries() != oth->orderOfKeyValues_.entries()) return FALSE;
  ...
  for (CollIndex i = 0; i < orderOfKeyValues_.entries(); i++) {
    if (orderOfKeyValues_[i] != oth->orderOfKeyValues_[i]) {
      ...
      if (orderOfKeyValues_[i].getItemExpr()->getOperatorType() !=
          ITM_INVERSE OR oth->orderOfKeyValues_[i].getItemExpr()->getOperatorType() !=
          ITM_INVERSE OR orderOfKeyValues_[i].getItemExpr()->child(0) !=
  ...
  // -----------------------------------------------------------------
  return partitionBoundaries_->isAGroupingOf(*oth->partitionBoundaries_, maxPartsPerGroup);
}
  ...
  if (NOT partKeyPredsCreated()) {
    ...
    for (CollIndex beginEnd = 0; beginEnd < 2; beginEnd++)
      for (CollIndex i = 0; i < nCols; i++) {
        ...
        HostVar *hv = new (CmpCommon::statementHeap()) HostVar(S, &(vid.getType()), TRUE);
    ...
    ItemExpr *intervalExclusionIndicator = new (CmpCommon::statementHeap())
        HostVar("_sys_hostVarExclRange",
                new (CmpCommon::statementHeap()) SQLInt(CmpCommon::statementHeap(), TRUE, FALSE), TRUE);
    ...
    NABoolean nullable = keyColumnList_[i].getType().supportsSQLnullLogical();
    ...
    if (orderOfKeyValues_[i].getItemExpr()->getOperatorType() == ITM_INVERSE) {
      compOp = ITM_LESS_EQ;
    ...
    ItemExpr *beginPred = new (h) BiRelat(compOp, kc, loValues[i].getItemExpr(), nullable, TRUE);
    ...
    ItemExpr *extraEndPred = new (h) BiRelat(extraOp, kc, hiValues[i].getItemExpr(), nullable, TRUE);
    ...
    while (i--) {
      ...
      nullable = keyColumnList_[i].getType().supportsSQLnullLogical();

      beginPred = new (h) BiLogic(
          ITM_OR, new (h) BiRelat(compOp, kc, loValues[i].getItemExpr(), nullable, TRUE),
          new (h)
              BiLogic(ITM_AND, new (h) BiRelat(ITM_EQUAL, kc, loValues[i].getItemExpr(), nullable, TRUE), beginPred));

      extraEndPred = new (h)
          BiLogic(ITM_OR, new (h) BiRelat(extraOp, kc, hiValues[i].getItemExpr(), nullable, TRUE),
                  new (h) BiLogic(ITM_AND, new (h) BiRelat(ITM_EQUAL, kc, hiValues[i].getItemExpr(), nullable, TRUE),
                                  extraEndPred));
    }
    ...
    TriRelational *tr =
        new (h) TriRelational(compOp, kc, hiValues[i].getItemExpr(),
                              new (h) BiRelat(ITM_NOT_EQUAL, new (h) SystemLiteral(0), intervalExclusionIndicator));
    ...
    while (i--) {
      ...
      endPred = new (h) BiLogic(
          ITM_OR, new (h) BiRelat(compOp, kc, hiValues[i].getItemExpr(), nullable),
          new (h) BiLogic(ITM_AND, new (h) BiRelat(ITM_EQUAL, kc, hiValues[i].getItemExpr(), nullable), endPred));
    }
...
// -----------------------------------------------------------------------
void RangePartitioningFunction::replacePivs(const ValueIdList &newPivs, const ValueIdSet &newPartKeyPreds) {
  // Overwrite the old pivs, part key preds, and part expr. with the new ones.
  storePartitionInputValues(newPivs);
  storePartitioningKeyPredicates(newPartKeyPreds);
}  // RangePartitioningFunction::replacePivs()
...
PartitioningFunction *RangePartitioningFunction::scaleNumberOfPartitions(int &suggestedNewNumberOfPartitions,
  ...
  suggestedNewNumberOfPartitions =
      partitionBoundaries_->scaleNumberOfPartitions(suggestedNewNumberOfPartitions, getNodeMap(), partGroupDist);
...
// -----------------------------------------------------------------------
void RangePartitioningFunction::remapIt(const PartitioningFunction *opf, ValueIdMap &map, NABoolean mapItUp) {
  PartitioningFunction::remapIt(opf, map, mapItUp);
  ...
  if (mapItUp) {
    map.rewriteValueIdListUp(keyColumnList_, opf->castToRangePartitioningFunction()->keyColumnList_);
  } else {
    map.rewriteValueIdListDown(opf->castToRangePartitioningFunction()->keyColumnList_, keyColumnList_);
  }
  ...
  if (mapItUp) {
    map.rewriteValueIdListUp(orderOfKeyValues_, opf->castToRangePartitioningFunction()->orderOfKeyValues_);
  } else {
    map.rewriteValueIdListDown(opf->castToRangePartitioningFunction()->orderOfKeyValues_, orderOfKeyValues_);
  }
...
PartitioningFunction *RangePartitioningFunction::createPartitioningFunctionForIndexDesc(IndexDesc *idesc) const {
  const NAFileSet *fileSet = idesc->getNAFileSet();
  const NAColumnArray &allColumns = fileSet->getAllColumns();
  const NAColumnArray &partKeyColumns = fileSet->getPartitioningKeyColumns();
  ...
  for (i = 0; i < partKeyColumns.entries(); i++) {
    ...
    if (partKeyColumns.isAscending(i)) {
      ...
      InverseOrder *invExpr = new (idesc->wHeap()) InverseOrder(keyValueId.getItemExpr());
  ...
  // -----------------------------------------------------------------
  partBounds = new (idesc->wHeap()) RangePartitionBoundaries(*getRangePartitionBoundaries(), idesc->wHeap());
  ...
  // -----------------------------------------------------------------
  partBounds->completePartitionBoundaries(orderOfPartKeyValues, encodedBoundaryKeyLength);

  // -----------------------------------------------------------------
  // Allocate a new RangePartitioningFunction.
  // -----------------------------------------------------------------
  RangePartitioningFunction *partFunc = new (idesc->wHeap()) RangePartitioningFunction(
      partitioningKey, partitioningKeyList, orderOfPartKeyValues, partBounds, getNodeMap()->copy(idesc->wHeap()));
  partFunc->createPartitioningKeyPredicates();
...
ItemExpr *RangePartitioningFunction::createPartitioningExpression() {
  ...
  for (CollIndex i = 0; i < keyColumnList_.entries(); i++) {
    NABoolean descOrder = (orderOfKeyValues_[i].getItemExpr()->getOperatorType() == ITM_INVERSE);
    ItemExpr *dataConversionErrorFlag = getConvErrorExpr();
    if (dataConversionErrorFlag == 0) {
      dataConversionErrorFlag = new (CmpCommon::statementHeap())
          HostVar("_sys_repartConvErrorFlg",
    ...
    const NAType &oType = originalKeyColumnList_[i].getType();
    ItemExpr *c =
        new (CmpCommon::statementHeap()) Narrow(keyColumnList_[i].getItemExpr(), dataConversionErrorFlag, &oType);
  ...
  ItemExpr *partFunc = new (CmpCommon::statementHeap()) RangeLookup(encKey, this);
...
// -----------------------------------------------------------------------
NABoolean RangePartitioningFunction::shouldUseSynchronousAccess(const ReqdPhysicalProperty *rpp,
                                                                const EstLogPropSharedPtr &inputLogProp,
                                                                GroupAttributes *ga) const {
  NABoolean shouldUseSynchronousAccess = FALSE;
  ...
  // See if the user is trying to force synchronous access.
  if ((CmpCommon::getDefault(ATTEMPT_ASYNCHRONOUS_ACCESS) == DF_OFF) OR((lpr != NULL) AND(
          lpr->getNumClientsReq() != ANY_NUMBER_OF_PARTITIONS) AND(lpr->getNumClientsReq() < getCountOfPartitions()))) {
    shouldUseSynchronousAccess = TRUE;
  ...
  if (rpp->getLogicalOrderOrArrangementFlag()) {
    if ((rpp->getSortKey() != NULL) AND(rpp->getArrangedCols() != NULL)) {
      // We have both a required order and an arrangement.
      if ((partKey.satisfiesReqdOrder(*rpp->getSortKey()) == SAME_ORDER)
              AND partKey.satisfiesReqdArrangement(*rpp->getArrangedCols()))
        shouldUseSynchronousAccess = TRUE;
      else
  ...
  if (shouldUseSynchronousAccess AND NOT synchronousAccessForced) {
    ...
    if (rpp->getDp2SortOrderPartReq() != NULL) shouldUseSynchronousAccess = FALSE;
  }
  ...
  if (shouldUseSynchronousAccess AND NOT synchronousAccessForced) {
    ...
    if (getCountOfPartitions() > maxPAsPerProcess) {
      ...
      if (lpr != NULL) {
        ...
        if (logPartReq AND logPartReq->castToFullySpecifiedPartitioningRequirement())
          requiredPartFunc = logPartReq->castToFullySpecifiedPartitioningRequirement()->getPartitioningFunction();
      }
      ...
      if (logPartReq !=
          NULL AND requiredPartFunc AND requiredPartFunc->castToLogPhysPartitioningFunction()
              AND requiredPartFunc->castToLogPhysPartitioningFunction()
                  ->getLogPartType() == LogPhysPartitioningFunction::LOGICAL_SUBPARTITIONING)
        tooManyPAs = TRUE;
      else if ((logPartReq != NULL)
                   AND(getCountOfPartitions() > (logPartReq->getCountOfPartitions() * maxPAsPerProcess)))
        tooManyPAs = TRUE;
      else if ((getCountOfPartitions() > (rpp->getCountOfPipelines() * maxPAsPerProcess)))
        tooManyPAs = TRUE;
    }
    ...
    if (NOT tooManyPAs) {
      ...
      // record for overhead.
      CostScalar totalsize = (ga->outputLogProp(inputLogProp)->getResultCardinality() * (ga->getRecordLength() + 16.0));
      ...
      double numOutputBuffers = MAXOF(CmpCommon::getDefaultNumeric(GEN_PA_NUM_BUFFERS) - 1, 1);
      // Subtract 2000 bytes per buffer for headers.
      buffersize = (buffersize * numOutputBuffers) - (2000 * numOutputBuffers);

      if (totalsize < (getCountOfPartitions() * 2.0 * buffersize)) shouldUseSynchronousAccess = FALSE;
    }
...
SearchKey *RangePartitioningFunction::createSearchKey(const IndexDesc *indexDesc, ValueIdSet availInputs,
                                                      ValueIdSet additionalPreds) const {
  ValueIdSet preds(getPartitioningKeyPredicates());
  ...
  SearchKey *partSearchKey = new (CmpCommon::statementHeap())
      SearchKey(indexDesc->getPartitioningKey(), indexDesc->getOrderOfPartitioningKeyValues(), availInputs,
                TRUE,  // there isn't really a scan direction for partKey
                preds, this, nonKeyColumnSet, indexDesc);
...
/////////////////////////////////////////////////////////////
NABoolean RangePartitioningFunction::partFuncAndFuncPushDownCompatible(const PartitioningFunction &x) const {
  const RangePartitioningFunction *other = x.castToRangePartitioningFunction();
  ...
  // same boundaries
  if (getRangePartitionBoundaries()->compareRangePartitionBoundaries(*(other->getRangePartitionBoundaries())) == FALSE)
    return FALSE;
...
void RangePartitioningFunction::setupForStatement() {
  ...
  if (partitionBoundaries_) partitionBoundaries_->setupForStatement(TRUE /* use string */);
...
void RangePartitioningFunction::resetAfterStatement() {
  ...
  if (partitionBoundaries_) partitionBoundaries_->resetAfterStatement();
...
const NAString RangePartitioningFunction::getText() const {
  ...
  if (partitionBoundaries_) {
    ...
    for (int index = 0; index < partitionBoundaries_->getCountOfPartitions(); index++) {
      ...
      if (index < partitionBoundaries_->getCountOfPartitions() - 1) result += ";";
    }
...
void RangePartitioningFunction::print(FILE *ofd, const char *indent, const char *title) const {
  ...
  if (orderOfKeyValues_.entries() > 0) {
    ...
    for (CollIndex index = 0; index < orderOfKeyValues_.entries(); index++) {
      if (orderOfKeyValues_[index].getItemExpr()->getOperatorType() == ITM_INVERSE)
        fprintf(ofd, "D ");
      else
        fprintf(ofd, "A ");
      if (((index / 9) * 9 == index) OR(index == orderOfKeyValues_.entries() - 1)) fprintf(ofd, "\n");
    }
  }
...
int RangePartitionBoundaries::findBeginBoundary(char *encodedKey, int keyLen, compFuncPtrT compFunc) const {
  ...
  for (int i = 0; i <= partitionCount_ - 1; i++) {
    ...
    if ((*compFunc)(low, encodedKey, high, keyLen, i == partitionCount_ - 1)) return i;
  }
...
int RangePartitioningFunction::computeNumOfActivePartitions(SearchKey *skey, const TableDesc *tDesc) const {
  ...
  if ((naTable->isHiveTable()) || (naTable->isMonarch())) return origPartitions;
...
const LogPhysPartitioningFunction *LogPhysPartitioningFunction::castToLogPhysPartitioningFunction() const {
  return this;
}
...
// -----------------------------------------------------------------------
const NodeMap *LogPhysPartitioningFunction::getNodeMap() const {
  // --------------------------------------------------------------------------
...
NodeMap *LogPhysPartitioningFunction::getOrMakeSuitableNodeMap(NABoolean forESP) const {
...
PartitioningFunction *LogPhysPartitioningFunction::copy(NAMemory *h) const {
  return new (h) LogPhysPartitioningFunction(*this, h);
}

NABoolean LogPhysPartitioningFunction::canProducePartitioningKeyPredicates() const {
  ...
  return (logPartFunc_->canProducePartitioningKeyPredicates()
              OR logPartType_ == PA_PARTITION_GROUPING OR logPartType_ == PA_GROUPED_REPARTITIONING);
}
...
// -----------------------------------------------------------------------
void LogPhysPartitioningFunction::replacePivs(const ValueIdList &newPivs, const ValueIdSet &newPartKeyPreds) {
  // Overwrite the old pivs, part key preds, and part expr. with the new ones.
  storePartitionInputValues(newPivs);
  storePartitioningKeyPredicates(newPartKeyPreds);
...
PartitioningFunction *LogPhysPartitioningFunction::createRealPartitioningFunction() {
  if (realPartFunc_) return realPartFunc_;
  ...
  switch (logPartType_) {
    ...
    case LOGICAL_SUBPARTITIONING: {
      ...
      const RangePartitioningFunction *lpf = logPartFunc_->castToRangePartitioningFunction();
      const RangePartitioningFunction *ppf = physPartFunc_->castToRangePartitioningFunction();

      // Log and phys partitioning functions must have the same key
      // to be able to produce a common function
      if (lpf AND ppf AND lpf->getOrderOfKeyValues() == ppf->getOrderOfKeyValues()) {
        ...
        NodeMap *mergedMap = new (CmpCommon::statementHeap()) NodeMap(CmpCommon::statementHeap());
        RangePartitionBoundaries *rb = ppf->getRangePartitionBoundaries()->merge(*lpf->getRangePartitionBoundaries(),
                                                                                 *ppf->getNodeMap(), *mergedMap);

        // return the merged partitioning function if we could merge
        if (rb)
          realPartFunc_ = new (CmpCommon::statementHeap()) RangePartitioningFunction(
              ppf->getPartitioningKey(), ppf->getKeyColumnList(), ppf->getOrderOfKeyValues(), rb, mergedMap);
      } else {
        ...
        if (physPartFunc_->castToSinglePartitionPartitioningFunction()) {
          ...
          NodeMap *nodeMap = new (CmpCommon::statementHeap()) NodeMap(CmpCommon::statementHeap());
          ...
          for (CollIndex nodeIdx = 0; nodeIdx < (CollIndex)logPartFunc_->getCountOfPartitions(); nodeIdx++) {
            nodeMap->setNodeMapEntry(nodeIdx, *entry, CmpCommon::statementHeap());
          }
...
COMPARE_RESULT
LogPhysPartitioningFunction::comparePartFuncToFunc(const PartitioningFunction &other) const {
  COMPARE_RESULT c = PartitioningFunction::comparePartFuncToFunc(other);

  if (c != SAME) return INCOMPATIBLE;

  const LogPhysPartitioningFunction &lppfOther = (const LogPhysPartitioningFunction &)other;

  if (getLogPartType() != lppfOther.getLogPartType()) return INCOMPATIBLE;

  if (getNumOfClients() != lppfOther.getNumOfClients()) return INCOMPATIBLE;

  if (getUsePapa() != lppfOther.getUsePapa()) return INCOMPATIBLE;

  if (getSynchronousAccess() != lppfOther.getSynchronousAccess()) return INCOMPATIBLE;

  if (getLogPartitioningFunction()->comparePartFuncToFunc(*lppfOther.getLogPartitioningFunction()) != SAME)
    return INCOMPATIBLE;

  if (getPhysPartitioningFunction()->comparePartFuncToFunc(*lppfOther.getPhysPartitioningFunction()) != SAME)
    return INCOMPATIBLE;
...
NABoolean LogPhysPartitioningFunction::isAGroupingOf(const PartitioningFunction &other, int *maxPartsPerGroup) const {
  ...
  return (comparePartFuncToFunc(other) ==
          SAME OR(realPartFunc_ AND realPartFunc_->isAGroupingOf(other, maxPartsPerGroup)));
}

// -----------------------------------------------------------------------
// LogPhysPartitioningFunction::remapIt()
// -----------------------------------------------------------------------
void LogPhysPartitioningFunction::remapIt(const PartitioningFunction *opf, ValueIdMap &map, NABoolean mapItUp) {
...
//
NABoolean LogPhysPartitioningFunction::canMaintainSortOrder(const ValueIdList &sortOrder) const {
  ...
  //
  const SinglePartitionPartitioningFunction *sppf = physPartFunc_->castToSinglePartitionPartitioningFunction();
  ...
  const RangePartitioningFunction *rpf = physPartFunc_->castToRangePartitioningFunction();

  const TableHashPartitioningFunction *hdpf = physPartFunc_->castToTableHashPartitioningFunction();
  ...
  if (rpf) {
    ...
    if (partKeyCols.entries() <= numSortKeys) {
      ...
      if (oc == SAME_ORDER ||
          (CmpCommon::getDefault(ATTEMPT_REVERSE_SYNCHRONOUS_ORDER) == DF_ON && oc == INVERSE_ORDER))
...
const NAString LogPhysPartitioningFunction::getText() const {
  ...
  result += ", log=" + logPartFunc_->getText() + ", phys=" + physPartFunc_->getText() + ")";
  return result;
}
...
const NAString LogPhysPartitioningFunction::getPhysForSplitTop() const { return physPartFunc_->getText(); }

void LogPhysPartitioningFunction::print(FILE *ofd, const char *indent, const char *title) const {
...
PartitioningRequirement *RoundRobinPartitioningFunction::makePartitioningRequirement() {
  return new (CmpCommon::statementHeap()) RequireRoundRobin(this);
}

PartitioningFunction *RoundRobinPartitioningFunction::copy(NAMemory *h) const {
  return new (h) RoundRobinPartitioningFunction(*this, h);
}
...
// -----------------------------------------------------------------------
COMPARE_RESULT
RoundRobinPartitioningFunction::comparePartFuncToFunc(const PartitioningFunction &other) const {
  COMPARE_RESULT c = PartitioningFunction::comparePartFuncToFunc(other);
  ...
  // They must be based on the same physical partitioning.
  //
  if (getCountOfOrigRRPartitions() != oth->getCountOfOrigRRPartitions()) return INCOMPATIBLE;
...
// -----------------------------------------------------------------------
void RoundRobinPartitioningFunction::replacePivs(const ValueIdList &newPivs, const ValueIdSet &newPartKeyPreds) {
  // Overwrite the old pivs, part key preds, and part expr. with the new ones.
  storePartitionInputValues(newPivs);
  storePartitioningKeyPredicates(newPartKeyPreds);
}  // RoundRobinPartitioningFunction::replacePivs()
...
// -----------------------------------------------------------------------
PartitioningFunction *RoundRobinPartitioningFunction::createPartitioningFunctionForIndexDesc(IndexDesc *idesc) const {
  const NAFileSet *fileSet = idesc->getNAFileSet();
  const NAColumnArray &allColumns = fileSet->getAllColumns();
  const NAColumnArray &partKeyColumns = fileSet->getPartitioningKeyColumns();
  ...
  // -----------------------------------------------------------------
  RoundRobinPartitioningFunction *partFunc = new (idesc->wHeap())
      RoundRobinPartitioningFunction(getCountOfPartitions(), partitioningKey, getNodeMap()->copy(idesc->wHeap()));
  partFunc->createPartitioningKeyPredicates();
...
ItemExpr *RoundRobinPartitioningFunction::createPartitioningExpression() {
  ...
  //
  ItemExpr *partKey = new (heap) Cast(
      new (heap)
          Shift(ITM_SHIFT_RIGHT, new (heap) Cast(keyValue[0].getItemExpr(), desiredType), new (heap) ConstValue(32)),
...
// -----------------------------------------------------------------------
ItemExpr *RoundRobinPartitioningFunction::createPartitionSelectionExpr(const SearchKey *partSearchKey,
                                                                       const ValueIdSet &availableValues) {
  // If it has already been created, return cached version.
  //
  if (partitionSelectionExpr()) return partitionSelectionExpr();
  ...
  if (assignPartition()) {
    ...
    partitionSelectionExpr() =
        new (heap) Modulus(new (heap) Cast(new (heap) BiArith(ITM_PLUS, partNum, new (heap) SystemLiteral(1)),
    ...
    // (partition assignment).
    //
    partitionSelectionExpr() = new (heap) ProgDistrib(
        new (heap) Cast(new (heap) Shift(ITM_SHIFT_RIGHT, new (heap) Cast(keyValues[0].getItemExpr(), desiredType),
                                         new (heap) SystemLiteral(0x1000)),
                        new (heap) SQLInt(heap, FALSE, FALSE)),
        numParts);
...
PartitioningFunction *RoundRobinPartitioningFunction::scaleNumberOfPartitions(int &suggestedNewNumberOfPartitions,
  ...
  if (suggestedNewNumberOfPartitions != getCountOfPartitions()) {
    NodeMap *newNodeMap = new (heap_) NodeMap(heap_, suggestedNewNumberOfPartitions);
    replaceNodeMap(newNodeMap);
  }
...
NABoolean RoundRobinPartitioningFunction::isAGroupingOf(const PartitioningFunction &other,
  ...
  //
  if (!(getPartitioningKey() == oth->getPartitioningKey())) return FALSE;
...
// -----------------------------------------------------------------------
SearchKey *RoundRobinPartitioningFunction::createSearchKey(const IndexDesc *indexDesc, ValueIdSet availInputs,
                                                           ValueIdSet additionalPreds) const {
  ValueIdSet preds(getPartitioningKeyPredicates());
  ValueIdSet nonKeyColumnSet;  // empty set
  ...
  //
  SearchKey *partSearchKey = new (CmpCommon::statementHeap())
      SearchKey(indexDesc->getPartitioningKey(), indexDesc->getOrderOfPartitioningKeyValues(), availInputs, preds, this,
                nonKeyColumnSet, indexDesc);
...
void RoundRobinPartitioningFunction::print(FILE *ofd, const char *indent, const char *title) const {
...
NABoolean skewProperty::operator==(const skewProperty &other) const {
  if (indicator_ != other.indicator_) return FALSE;

  if (broadcastOneRow_ != other.broadcastOneRow_) return FALSE;

  if (skewValues_ == other.skewValues_) return TRUE;

  if (skewValues_ != NULL && other.skewValues_ != NULL && NOT(*skewValues_ == *(other.skewValues_))) return FALSE;

  if (numESPs_ == other.numESPs_) return TRUE;
  ...
  switch (getIndicator()) {
    case skewProperty::UNIFORM_DISTRIBUTE:
      ...
      if (abbre == FALSE) {
        ...
        if (skewValues_) result += skewValues_->getText();
      } else {
        result += "-ud";
      }
      break;
    ...
    case skewProperty::BROADCAST:
      ...
      if (abbre == FALSE) {
        ...
        if (getBroadcastOneRow()) result += " and one row ";
      } else {
        result += "-br";
      }
      break;
...
Int64List *SkewedDataPartitioningFunction::buildHashListForSkewedValues() {
  if (skewHashList_) return skewHashList_;
...
PartitioningRequirement *HivePartitioningFunction::makePartitioningRequirement() {
  return new (CmpCommon::statementHeap()) RequireHive(this);
}

PartitioningFunction *HivePartitioningFunction::copy(NAMemory *h) const {
  return new (h) HivePartitioningFunction(*this, h);
}
...
void HivePartitioningFunction::createPartitioningKeyPredicates() {
  createBetweenPartitioningKeyPredicates("_sys_HostVarLoHivePart", "_sys_HostVarHiHivePart");
...
ItemExpr *HivePartitioningFunction::buildHashingExpressionForExpr(ItemExpr *expr) {
  CollHeap *heap = CmpCommon::statementHeap();
  ...
  ItemExpr *narrowedNegatedHiveHash =
      new (heap) Narrow(negatedHiveHash, dataConversionErrorFlag, new (heap) SQLInt(heap, TRUE, FALSE), ITM_NARROW,
                        FALSE, TRUE /* nullability same as the child */
      );
...
// Method for debugging.
// -----------------------------------------------------------------------
const NAString HivePartitioningFunction::getText() const { return getTextImp("hive"); }

void HivePartitioningFunction::print(FILE *ofd, const char *indent, const char *title) const {
  PartitioningFunction::print(ofd, indent, "HivePartitioningFunction");
}  // HivePartitioningFunction::print()

PartitioningFunction *HivePartitioningFunction::createPartitioningFunctionForIndexDesc(IndexDesc *idesc) const {
  const NAFileSet *fileSet = idesc->getNAFileSet();
  const NAColumnArray &allColumns = fileSet->getAllColumns();
  const NAColumnArray &partKeyColumns = fileSet->getPartitioningKeyColumns();
  ...
  // Allocate a new HashPartitioningFunction.
  // -----------------------------------------------------------------
  HivePartitioningFunction *partFunc = new (idesc->wHeap()) HivePartitioningFunction(
      partitioningKey, partitioningKeyList, getCountOfPartitions(), getNodeMap()->copy(idesc->wHeap()));
...
NABoolean HivePartitioningFunction::isAGroupingOf(const PartitioningFunction &other, int *maxPartsPerGroup) const {
  ...
  // This is a grouping of.  Set the maxPartsPerGroup and return TRUE.
  if (maxPartsPerGroup != NULL) *maxPartsPerGroup = other.getCountOfPartitions() / getCountOfPartitions();

  return TRUE;
}

void HivePartitioningFunction::normalizePartitioningKeys(NormWA &normWARef) {
