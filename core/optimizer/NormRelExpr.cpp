...
static void tryToConvertFullOuterJoin(Join *fullOuterJoin, NormWA &normWARef) {
  ...
  if (leftChildMerged && !rightChildMerged) fullOuterJoin->setOperatorType(REL_LEFT_JOIN);

  if (rightChildMerged && !leftChildMerged) fullOuterJoin->setOperatorType(REL_RIGHT_JOIN);

  if (leftChildMerged && rightChildMerged) fullOuterJoin->setOperatorType(REL_JOIN);  // inner Join
  ...
  switch (fullOuterJoin->getOperatorType()) {
    case REL_LEFT_JOIN: {
      ...
      VEGRegion *rightChildVEGRegion = normWARef.locateVEGRegion(fullOuterJoin, 1 /* right child*/);

      VEGRegion *joinPredVEGRegion = normWARef.locateVEGRegion(fullOuterJoin, 2 /* join predicate */);
...
// -----------------------------------------------------------------------
void RelExpr::transformNode(NormWA &normWARef, ExprGroupId &locationOfPointerToMe) {
  ...
  for (int i = 0; i < arity; i++) {
    ...
    // ---------------------------------------------------------------------
    child(i)->getGroupAttr()->addCharacteristicInputs(getGroupAttr()->getCharacteristicInputs());
...
MapValueIds *GroupByAgg::buildMapValueIdNode(ValueIdMap *map) {
  ...
  mapNode->getGroupAttr()->addCharacteristicInputs(getGroupAttr()->getCharacteristicInputs());
...
RelExpr::rwErrorStatus RelRoot::checkReadWriteConflicts(NormWA &normWARef) {
  ...
  if (!(getGroupAttr()->isEmbeddedUpdateOrDelete()) && isTrueRoot()) return RWOKAY;
...
RelExpr::rwErrorStatus Scan::checkReadWriteConflicts(NormWA &normWARef) {
  NAString fileName(getTableDesc()->getNATable()->getClusteringIndex()->getFileSetName().getQualifiedNameAsString(),
...
RelExpr::rwErrorStatus GenericUpdate::checkReadWriteConflicts(NormWA &normWARef) {
  ...
  CollIndex i = 0;
  if (!getInliningInfo().isEffDeleteAsj())
    for (i = 0; i < normWARef.getReadList().entries(); i++)
      if (strcmp(normWARef.getReadList()[i], fileName) == 0) {
        *CmpCommon::diags() << DgSqlCode(-4152)
                            << DgTableName(getTableDesc()->getNATable()->getTableName().getQualifiedNameAsAnsiString());
...
int printTree(ItemExpr *ptrToTree, ItemExpr *parent, int depth, int l1) {
  if (ptrToTree != NULL) {
    ...
    if (depth == 0) {
      if (l1 == 0) cout << "root ";
      if (l1 == 1) cout << "left child ";
      if (l1 == 2) cout << "right child ";
      cout << " ValueId: " << ptrToTree->getValueId() << "  Value:" << ptrToTree->getText()
           << " Parent ValueId: " << parent->getValueId() << " Parent Value:" << parent->getText() << endl;
      return 1;
    }
...
NABoolean Scan::shrinkNewTree(OperatorTypeEnum op, ItemExpr *ptrToNewTree, RangeSpecRef *xref, NormWA &normWARef) {
  ...
  if (ptrToNewTree->getOperatorType() == ITM_RANGE_SPEC_FUNC) {
    ...
    if (op == ITM_OR) {
      if (destObj->getRangeExpr()->getValueId() == xref->getRangeObject()->getRangeExpr()->getValueId()) {
        destObj->unionRange(xref->getRangeObject());
        rangeIE->setChild(1, const_cast<ItemExpr *>(destObj->getRangeItemExpr(&normWARef)));
        status = true;
      }
    } else if (op == ITM_AND) {
      if (destObj->getRangeExpr()->getValueId() == xref->getRangeObject()->getRangeExpr()->getValueId()) {
   ...
   // RangeSpecRef(a,BiRel(=4))RangeSpecRef(b,BiRel(=6))
    if (op == ptrToNewTree->getOperatorType()) {
      ...
      if (!status) status = shrinkNewTree(op, ptrToNewTree->child(1), xref, normWARef);
    }
  }
  return status;
}
...
ItemExpr *Scan::applyAssociativityAndCommutativity(QRDescGenerator *descGenerator, CollHeap *heap,
                                                   ItemExpr *origPtrToOldTree, NormWA &normWARef,
                                                   NABoolean &transformationStatus) {
  if (CmpCommon::getDefault(SUBSTRING_TRANSFORMATION) != DF_OFF) return origPtrToOldTree;
  ...
  while (currIdx >= 0) {
    ...
    if ((ptrToOldTree->getOperatorType() != ITM_AND && ptrToOldTree->getOperatorType() != ITM_OR) ||
        (ptrToOldTree->getOperatorType() == ITM_OR && static_cast<BiLogic *>(ptrToOldTree)->createdFromINlist()) ||
        ptrToOldTree->isRangespecItemExpr()) {
      OptNormRangeSpec *range =
          static_cast<OptNormRangeSpec *>(OptRangeSpec::createRangeSpec(descGenerator, ptrToOldTree, heap, TRUE));

      // Transforms all Birel ItemExpression into RangeSpecRef ItemExpression
      if (range != NULL) {
        RangeSpecRef *refrange = new (heap)
            RangeSpecRef(ITM_RANGE_SPEC_FUNC, range, range->getRangeExpr(), range->getRangeItemExpr(&normWARef));
      ...
      if ((newLeftNode->getOperatorType() == ITM_RANGE_SPEC_FUNC) &&
        ...
        // where a = 10 or b =20
        // where a = 10 or a =20
        if (shrinkNewTree(ptrToOldTree->getOperatorType(), newLeftNode, (RangeSpecRef *)newRightNode, normWARef)) {
          newNode = (ItemExpr *)newLeftNode;
        } else {
          // where a = 10 or b =20
          newNode = new (heap)
              BiLogic(ptrToOldTree->getOperatorType(), (RangeSpecRef *)newLeftNode, (RangeSpecRef *)newRightNode);
        }
        ...
        if (!shrinkNewTree(ptrToOldTree->getOperatorType(), newLeftNode, (RangeSpecRef *)newRightNode, normWARef)) {
          newNode = new (heap) BiLogic(ptrToOldTree->getOperatorType(), newLeftNode, (RangeSpecRef *)newRightNode);
        } else
          newNode = (ItemExpr *)newLeftNode;
      }
      ...
      //                / \
       //            OrSet OrSet
        ...
        newNode = new (heap) BiLogic(ptrToOldTree->getOperatorType(), newLeftNode, newRightNode);
        ...
        if (!shrinkNewTree(ptrToOldTree->getOperatorType(), newRightNode, (RangeSpecRef *)newLeftNode, normWARef)) {
          newNode = new (heap) BiLogic(ptrToOldTree->getOperatorType(), (RangeSpecRef *)newLeftNode, newRightNode);
        } else
...
// -----------------------------------------------------------------------
void RelExpr::transformSelectPred(NormWA &normWARef, ExprGroupId &locationOfPointerToMe) {
  // ---------------------------------------------------------------------
  ...
  selectionPred().removeSubqueryOrIsolatedUDFunctionPredicates(subqueryOrIsolatedUDFunctionPredicates);
  ...
  // ---------------------------------------------------------------------
  if (selectionPred().transformNode(normWARef, locationOfPointerToMe, externalInputs, movePredicates,
                                    postJoinPredicates)) {
    // -----------------------------------------------------------------
  ...
  if (subqueryOrIsolatedUDFunctionPredicates.transformNode(normWARef, locationOfPointerToMe, externalInputs,
                                                           movePredicates, postJoinPredicates)) {
    // -----------------------------------------------------------------
  ...
  if (locationOfPointerToMe != (const RelExpr *)this) {
    ...
    // ---------------------------------------------------------------------
    if (normWARef.subqUnderExprTree()) normWARef.restoreSubqUnderExprTreeFlag();
...
NABoolean RelExpr::getMoreOutputsIfPossible(ValueIdSet &outputsNeeded) {
  ...
  // walk need not proceed any further.
  nc = getArity();
  for (i = 0; i < nc; i++) {
  ...
  fakeGA.coverTest(outputsNeeded,
                   ...
                   coveredExprs,
                   emptySet,  // additional inputs not provided
                   &coveredSubExprs, &newOutputsNeeded);
  ...
  if (NOT newOutputsNeeded.isEmpty()) {
    ...
    for (i = 0; i < nc; i++) {
      if (NOT gotOutputsNeeded) gotOutputsNeeded = child(i).getPtr()->getMoreOutputsIfPossible(newOutputsNeeded);
    }
  }
  ...
  for (i = 0; i < nc; i++) {
    ...
    outputsToAdd.accumulateReferencedValues(maxOutputs, outputsNeeded);
...
// Join::transformNode()
// -----------------------------------------------------------------------
void Join::transformNode(NormWA &normWARef, ExprGroupId &locationOfPointerToMe) {
  ...
  if (isTSJForMergeUpsert()) {
    ValueIdSet subqVids;
    for (ValueId vid = availableValues.init(); availableValues.next(vid); availableValues.advance(vid)) {
      if (vid.getItemExpr()->getOperatorType() == ITM_ROW_SUBQUERY) subqVids.insert(vid);
    }
  ...
  if (isFullOuterJoin()) normWARef.restoreOriginalVEGRegion();
  ...
  if (joinPred().transformNode(normWARef, child(1), externalInputs, movePredicates)) {
    ...
    ValueId vid;
    for (vid = neededInputs.init(); neededInputs.next(vid); neededInputs.advance(vid))
      neededInputs2.insert(vid.getItemExpr()->getReplacementExpr()->getValueId());

    ValueIdSet crossReferences2;
    for (vid = crossReferences.init(); crossReferences.next(vid); crossReferences.advance(vid))
      crossReferences2.insert(vid.getItemExpr()->getReplacementExpr()->getValueId());
  ...
  // ---------------------------------------------------------------------
  if (isLeftJoin() OR isFullOuterJoin() OR isAntiSemiJoin()) normWARef.restoreOriginalVEGRegion();
  ...
  DBGIF(unp = ""; selectionPred().unparse(unp); cerr << "Join selpred: " << unp << endl;)
  ...
  if (CmpCommon::getDefault(NOT_IN_OPTIMIZATION) == DF_ON) {
    if (isAntiSemiJoin()) {
      ...
      if (newSet.entries() > 0) {
        ...
        newSet.transformNode(normWARef, child(1), externalInputs, movePredicates);
  ...
  if (isTSJ()) {
    ...
    if (crossReferences2.isEmpty()) {
      ...
      for (vid = crossReferences.init(); crossReferences.next(vid); crossReferences.advance(vid)) {
        if (vid.getItemExpr()->getOperatorType() != ITM_BASECOLUMN) continue;
      ...
      for (vid1 = neededInputs.init(); neededInputs.next(vid1); neededInputs.advance(vid1)) {
        if (vid1.getItemExpr()->getOperatorType() != ITM_BASECOLUMN) continue;
      ...
      ValueIdSet rightChildInputs = child(1).getPtr()->getGroupAttr()->getCharacteristicInputs();

      for (vid = eis.init(); eis.next(vid); eis.advance(vid)) {
        if (vid.getItemExpr()->getOperatorType() != ITM_INDEXCOLUMN) continue;

        for (vid1 = eis1.init(); eis1.next(vid1); eis1.advance(vid1)) {
          if (vid1.getItemExpr()->getOperatorType() != ITM_INDEXCOLUMN) continue;

          if (doTwoVidsReferToSameColumn(vid, vid1)) {
            normWARef.addVEG(((IndexColumn *)vid.getItemExpr())->getDefinition(),
            ...
            rightChildInputs -= ((IndexColumn *)vid1.getItemExpr())->getDefinition();

            rightChildInputs += ((IndexColumn *)vid.getItemExpr())->getDefinition();
      ...
      child(1).getPtr()->getGroupAttr()->setCharacteristicInputs(rightChildInputs);
    }  // isEmpty()

    if (crossReferences2.isEmpty() && !isTSJForWrite() && !getInliningInfo().isDrivingPipelinedActions() &&
        !getInliningInfo().isDrivingTempInsert() &&  // Triggers -
        !(isRoutineJoin() && child(1).getGroupAttr()->getHasNonDeterministicUDRs()) &&
        !getInliningInfo().isEffUpsertLj()) {
...
void Join::pullUpPreds() {
  ...
  // ---------------------------------------------------------------------
  NABoolean needPullUp = TRUE;
  ValueIdSet preds1 = child(0)->getSelectionPred();
  if (preds1.entries() == 1) {
    ValueId pred;
    preds1.getFirst(pred);
    if (pred.getItemExpr()->getOperatorType() == ITM_CONSTANT) {
      needPullUp = FALSE;
    }
  }

  if (needPullUp && !getInliningInfo().isEffDeleteAsj()) {
  ...
  // ---------------------------------------------------------------------
  needPullUp = TRUE;
  ValueIdSet preds2 = child(1)->getSelectionPred();
  if (preds2.entries() == 1) {
    ValueId pred;
    preds2.getFirst(pred);
    if (pred.getItemExpr()->getOperatorType() == ITM_CONSTANT) {
      needPullUp = FALSE;
    }
  }
  ...
  {
    getGroupAttr()->coverTest(joinPred(), child(0)->getGroupAttr()->getCharacteristicOutputs(), predicatesToMove,
                              boringSet, &predicatesThatStay);
...
void Join::recomputeOuterReferences() {
  ...
  if (NOT getGroupAttr()->getCharacteristicInputs().isEmpty()) {
    ...
    if (isTSJForMergeUpsert()) {
      ValueIdSet exprSet2;
      ValueId vid;
      for (vid = exprSet.init(); exprSet.next(vid); exprSet.advance(vid))
        exprSet2.insert(vid.getItemExpr()->getReplacementExpr()->getValueId());
...
void Join::tryToRewriteJoinPredicate(NormWA &normWARef) {
  ...
  for (ValueId exprId = joinPred().init(); joinPred().next(exprId); joinPred().advance(exprId)) {
    ItemExpr *iePtr = exprId.getItemExpr();
    if (iePtr->getOperatorType() == ITM_NOT) {
      ItemExpr *grandkid, *kid = iePtr->child(0);
      if (kid && kid->getOperatorType() == ITM_IS_TRUE && (grandkid = kid->child(0)) != NULL &&
        ...
        if (preds.isNotNullable(grandkid->child(0)) && preds.isNotNullable(grandkid->child(1))) {
#ifndef NDEBUG
          FILE *logF = NULL;
          NABoolean logRewrites =
              CmpCommon::getDefault(COMP_BOOL_138) == DF_OFF && CmpCommon::getDefault(COMP_BOOL_137) == DF_ON;
          if (logRewrites && (logF = fopen("rewriteJoinPredicateLog.txt", "a")) != NULL) {
          ...
          // with "t1.c=t2.c".
          ItemExpr *eqpred =
              new (normWARef.wHeap()) BiRelat(ITM_EQUAL, grandkid->child(0).getPtr(), grandkid->child(1).getPtr());
          ((BiRelat *)eqpred)->specialMultiValuePredicateTransformation() = TRUE;
...
void Join::rewriteNode(NormWA &normWARef) {
  ...
  if (isAFullOuterJoin) {
    ...
    if (getOperatorType() != REL_FULL_JOIN) isAFullOuterJoin = FALSE;  // no longer a FULL OUTER
  }
  ...
  if (CmpCommon::getDefault(COMP_BOOL_138) == DF_OFF && CmpCommon::getDefault(NOT_IN_OPTIMIZATION) == DF_OFF) {
  ...
  // ---------------------------------------------------------------------
  if (isAFullOuterJoin) normWARef.locateAndSetVEGRegion(this, 0 /* first child */);
  ...
  // Restore the original VEGRegion.
  // -----------------------------------------------------------------
  if (isAFullOuterJoin) normWARef.restoreOriginalVEGRegion();
  ...
  if (isALeftJoin OR isAFullOuterJoin OR isAntiSemiJoin()) {
    ...
    // -----------------------------------------------------------------
    if (isAFullOuterJoin) normWARef.locateAndSetVEGRegion(this, 2 /* third child */);
  ...
  else {  // normalize the ON clause of the INNER Join
    child(1)->rewriteNode(normWARef);
    // -----------------------------------------------------------------
    // Rewrite expressions in the ON clause predicate.
    // -----------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------
  if (getInliningInfo().isDrivingMvLogInsert() && NULL != updateSelectValueIdMap_) {
    ...
    const ValueIdList &originalBottomValues = updateSelectValueIdMap_->getBottomValues();
    ...
    for (CollIndex i = 0; i < originalBottomValues.entries(); i++) {
      ...
      if (x && x->getOperatorType() == ITM_VALUEIDUNION &&
          ((ValueIdUnion *)x)->getLeftSource() == ((ValueIdUnion *)x)->getRightSource()) {
        newBottomValues[i] = ((ValueIdUnion *)x)->getRightSource();
      } else
        newBottomValues[i] = originalBottomValues[i];
    }

    updateSelectValueIdMap_ =
        new (CmpCommon::statementHeap()) ValueIdMap(updateSelectValueIdMap_->getTopValues(), newBottomValues);
  }
  ...
  if (isALeftJoin OR isAFullOuterJoin OR isAntiSemiJoin()) normWARef.saveLeftJoinChildVEGRegion(this, 0);

  ((ValueIdSet &)getGroupAttr()->getCharacteristicInputs()).normalizeNode(normWARef);

  if (isALeftJoin OR isAFullOuterJoin OR isAntiSemiJoin()) normWARef.resetLeftJoinChildVEGRegion();
...
void Join::normalizeNullInstantiatedOutput(NormWA &normWARef) {
  ...
  for (CollIndex index = 0; index < nullInstantiatedOutput().entries(); index++) {
...
void Join::normalizeNullInstantiatedForRightJoinOutput(NormWA &normWARef) {
  ...
  for (CollIndex index = 0; index < nullInstantiatedForRightJoinOutput().entries(); index++) {
...
//
//                          Inner Join #2 : p1 & p5 (we attempt to push down these predicates, so they may end up in the
//                          children)
//                         /              \
...
//
// -----------------------------------------------------------------------
Join *Join::leftLinearizeJoinTree(NormWA &normWARef, TransformationType transformationType,
                                  NABoolean includeIntersects) {
  ...
  if (CURRSTMT_OPTDEFAULTS->joinOrderByUser()) return this;

  // Condition for applying this rule:
  // I and my right child must both be an Inner Join.
  if ((getOperatorType() != REL_JOIN) OR((child(1)->getOperatorType() != REL_JOIN) AND(
          child(1)->getOperatorType() != REL_ROUTINE_JOIN) AND(child(1)->getOperatorType() != REL_LEFT_JOIN)))
    return this;

  // preserve intersects for the transformation, if applicable
  if (isIntersect_ && !includeIntersects && normWARef.getIntersectsToXformCount() > 0) return this;

  // R1 is my current right child
  Join *R1 = (Join *)(child(1).getPtr());

  // Left linearize R1
  R1->leftLinearizeJoinTree(normWARef, transformationType, includeIntersects);
  ...
  if ((!selectionPred().isEmpty() || !R1->joinPred().isEmpty()) &&
      (R1->child(0)->getOperatorType() == REL_UNION || R1->child(1)->getOperatorType() == REL_UNION))
  ...
  R1->getGroupAttr()->setCharacteristicInputs(getGroupAttr()->getCharacteristicInputs());
  R1->getGroupAttr()->setCharacteristicOutputs(getGroupAttr()->getCharacteristicOutputs());
  ...
  if (transformationType == UNNESTING) {
    ...
    R1->pushdownCoveredExprSQO(R1->getGroupAttr()->getCharacteristicOutputs(), availableInputs, R1->selectionPred(),
                               nonPredExpr,
    ...
    R1->pushdownCoveredExpr(R1->getGroupAttr()->getCharacteristicOutputs(), availableInputs, R1->selectionPred());
  ...
  // right child.
  //
  R1->child(0) = leftLinearizeJoinTree(normWARef, transformationType, includeIntersects);
...
RelExpr *Join::normalizeNode(NormWA &normWARef) {
  ...
  if (candidateForSubqueryUnnest() && (child(1)->getOperatorType() == REL_GROUPBY)) {
  ...
  if (isFullOuterJoin()) normWARef.locateAndSetVEGRegion(this, 0 /* first child */);
  ...
  if (isIntersect_ && ActiveSchemaDB()->getDefaults().getAsDouble(INTERSECT_PUSH_GRBY_RATIO) >= 0.0)
    // we want to check for the cardinality-based transformation in
    // the SQO phase that pushes a groupby down to the children of the
    // intersect
    normWARef.incrementIntersectsToXformCount();
  ...
  // ---------------------------------------------------------------------
  if (isATSJ AND NOT isTSJForWrite() AND NOT child(1)
          ->getGroupAttr()
          ->getCharacteristicInputs()
          .referencesOneValueFromTheSet(child(0)->getGroupAttr()->getCharacteristicOutputs()) &&
      !getInliningInfo().isDrivingPipelinedActions() && !getInliningInfo().isDrivingTempInsert()  // Triggers -
      && !(isRoutineJoin() && child(1).getGroupAttr()->getHasNonDeterministicUDRs())) {
    // Remember we used to be a RoutineJoin. This is used to determine
    // what type of contexts for partitioning we will try in OptPhysRel.
    if (isRoutineJoin()) setDerivedFromRoutineJoin();
...
void Join::createAFilterGrandChildIfNeeded(NormWA &normWARef) {
  ...
  if (oldRightGrandChild->getOperator().match(REL_ANY_SEMIJOIN) ||
    ...
    if (CmpCommon::getDefault(SUBQUERY_UNNESTING) == DF_DEBUG) {
      *CmpCommon::diags() << DgSqlCode(2997)
                          << DgString1(
                                 "Subquery was not unnested. Reason: Right grandchild of TSJ is a semijoin or has more "
                                 "than one group by");
    }
  }
  // -----------------------------------------------------------------------
  ...
  gbyNode->getGroupAttr()->getCharacteristicInputs().getOuterReferences(outerReferences);
  ...
  // 10-090206-8977.
  if ((doNotUnnest == FALSE) && oldRightGrandChild->getOperator().match(REL_ANY_LEFT_JOIN)) {
    Join *myself = (Join *)oldRightGrandChild;

    if (((Join *)oldRightGrandChild)->joinPred().getReferencedPredicates(outerReferences, nonLocalPreds)) {
      doNotUnnest = TRUE;
      if (CmpCommon::getDefault(SUBQUERY_UNNESTING) == DF_DEBUG) {
        *CmpCommon::diags()
            << DgSqlCode(2997)
            << DgString1(
                   "Subquery was not unnested. Reason: Filter child is leftJoin with outerreferences in joinPred ");
      }
    }
  }
  if (doNotUnnest == FALSE) {
    nonLocalPreds.clear();
    oldRightGrandChild->selectionPred().getReferencedPredicates(outerReferences, nonLocalPreds);
    if (nestedAggInSubQ) subQGby->selectionPred().getReferencedPredicates(outerReferences, nonLocalPreds);
    ...
    if (!nonLocalPreds.isEmpty()) {
      ...
      if (candidateForLeftJoin || oldRightGrandChild->getOperator().match(REL_ANY_LEFT_JOIN)) {
        for (ValueId filterVid = nonLocalPreds.init(); nonLocalPreds.next(filterVid);
      ...
      if ((doNotUnnest == FALSE) && candidateForLeftJoin &&
          ...
          ((normWARef.getLeftJoinConversionCount() >= ActiveSchemaDB()->getDefaults().getAsLong(COMP_INT_12)) ||
           nestedAggInSubQ)) {
        doNotUnnest = TRUE;
        // For phase 2 we only unnest COMP_INT_12 subqueries
        // containing NonNullRejecting Predicates. Later we will ensure
        // that these subqueries are not nested.

        if (CmpCommon::getDefault(SUBQUERY_UNNESTING) == DF_DEBUG) {
          if (!nestedAggInSubQ)
            *CmpCommon::diags() << DgSqlCode(2997)
                                << DgString1(
                                       "Skipping unnesting of Subquery due to NonNullRejecting Predicates in more than "
                                       "COMP_INT_12 subqueries");
          else
            *CmpCommon::diags() << DgSqlCode(2997)
                                << DgString1(
                                       "Skipping unnesting of Subquery since we have both NonNullRejecting predicate "
                                       "and nested aggregate in subquery.");
        }
      }

      // create the filter node
      if (doNotUnnest == FALSE) {
        predFilterNode = new (CmpCommon::statementHeap()) Filter(oldRightGrandChild);
        ...
        if (nestedAggInSubQ) {
          ...
          predFilterNode->getGroupAttr()->setCharacteristicInputs(subQGby->getGroupAttr()->getCharacteristicInputs());
        ...
        if (oldRightGrandChild->getGroupAttr()->getCharacteristicInputs() ==
          ...
          if (CmpCommon::getDefault(SUBQUERY_UNNESTING) == DF_DEBUG)
            *CmpCommon::diags() << DgSqlCode(2997)
                                << DgString1(
                                       "Skipping unnesting of Subquery due to subtree below filter requires same outer "
                                       "references as filter");
          ...
          if (candidateForLeftJoin) normWARef.incrementLeftJoinConversionCount();
      ...
      if (gbyNode->selectionPred().getReferencedPredicates(outerReferences, nonLocalPreds) ||
          gbyNode->aggregateExpr().getReferencedPredicates(outerReferences, nonLocalPreds)) {
        ...
        if (candidateForLeftJoin && (CmpCommon::getDefault(SUBQUERY_UNNESTING_P2) != DF_INTERNAL) &&
          ...
          if (CmpCommon::getDefault(SUBQUERY_UNNESTING) == DF_DEBUG)
            *CmpCommon::diags()
                << DgSqlCode(2997)
                << DgString1(
                       "Skipping unnesting of Subquery due to NonNullRejecting Predicates in more than one subquery");
        }
      } else {
...
RelExpr *Join::eliminateRedundantJoin(NormWA &normWARef) {
  if (getOperatorType() == REL_JOIN) {
    ...
    if (result) {
      ...
      for (ValueId id = constraints.init(); constraints.next(id) && NOT found; constraints.advance(id)) {
        if (id.getItemExpr()->getOperatorType() == ITM_COMP_REF_OPT_CONSTRAINT) {
          ComplementaryRefOptConstraint *compRIConstraint = (ComplementaryRefOptConstraint *)id.getItemExpr();
      ...
      for (CollIndex i = 0; i < allCols.entries(); i++) {
        ...
        for (ValueId eqVid = eic.init(); eic.next(eqVid); eic.advance(eqVid)) normWARef.deleteVEGMember(eqVid);
      }
...
RelExpr *Join::transformSemiJoin(NormWA &normWARef) {
  ...
  if ((getOperatorType() == REL_SEMITSJ) || getEquiJoinPredicates().isEmpty()) {
    return this;
  }

  // apply the transformation described in item a) above
  ValueIdSet equiJoinCols1 = getEquiJoinExprFromChild1();
  if ((NOT equiJoinCols1.isEmpty()) && child(1)->getGroupAttr()->isUnique(equiJoinCols1)) {
    ...
    selectionPred() += joinPred();
    joinPred().clear();
    linearizedExpr = leftLinearizeJoinTree(normWARef, SEMI_JOIN_TO_INNER_JOIN);
  ...
  if ((innerUec > 0) && (!noInnerStats) && (innerRowCount / innerUec > reductionThreshold))
    haveSignificantReduction = TRUE;
  CostScalar innerAllowance = ((ActiveSchemaDB()->getDefaults()).getAsDouble(SEMIJOIN_TO_INNERJOIN_INNER_ALLOWANCE));
  NABoolean haveSmallInner = FALSE;
  if ((innerRowCount < innerAllowance) && (!noInnerStats)) haveSmallInner = TRUE;

  if (preds.isEmpty() && ((child(1)->getGroupAttr()->getNumBaseTables() == 0) || haveSignificantReduction ||
                          haveSmallInner || (CmpCommon::getDefault(SEMIJOIN_TO_INNERJOIN_TRANSFORMATION) == DF_ON))) {
    CollHeap *stmtHeap = CmpCommon::statementHeap();

    setOperatorType(REL_JOIN);
    // we need a group by below the transformed join
    GroupByAgg *newGrby = new (stmtHeap) GroupByAgg(child(1)->castToRelExpr());
    newGrby->setGroupAttr(new (stmtHeap) GroupAttributes(*(child(1)->getGroupAttr())));
...
static RelExpr *copyNode(RelExpr *oldNode, CollHeap *heap) {
  ...
  newNode->setGroupAttr(new (heap) GroupAttributes(*(oldNode->getGroupAttr())));
...
NABoolean Join::pullUpPredsWithAggrs(GroupByAgg *grbyNode, MapValueIds *mapNode) {
  ...
  if (NOT joinPred().isEmpty()) {
    ...
    if (joinPred().getReferencedPredicates(grbyNode->aggregateExpr(), predicatesThatNeedsToBePulled)) {
  ...
  if (NOT selectionPred().isEmpty()) {
    if (mapNode == NULL) {
      ...
      if (selectionPred().getReferencedPredicates(grbyNode->aggregateExpr(), predicatesToPullUp)) {
        selectionPred() -= predicatesToPullUp;
        grbyNode->selectionPred() += predicatesToPullUp;
      }
    } else {
      ValueIdMap *copyOfMap = new (CmpCommon::statementHeap()) ValueIdMap(mapNode->getMap());

      for (ValueId vid = selectionPred().init(); selectionPred().next(vid); selectionPred().advance(vid)) {
        ValueId bottomMapId;
        ...
        if (vid != bottomMapId) {
          ...
          if (mapPullUpPred.getReferencedPredicates(grbyNode->aggregateExpr(), mapPredicatesToPullUp)) {
            selectionPred() -= vid;
            grbyNode->selectionPred() += mapPredicatesToPullUp;
            if (getGroupAttr()->getCharacteristicOutputs().referencesTheGivenValue(vid, ignoreVid, FALSE, FALSE)) {
...
// -----------------------------------------------------------------------
void GroupByAgg::computeGroupExpr(const ValueIdSet &seed, ValueIdSet &superSet, NormWA &normWARef) {
  ValueIdSet duplicates;
  ValueIdSet reqGrpValues = seed;
  reqGrpValues += leftUniqueExpr();
  reqGrpValues.accumulateReferencedValues(superSet, selectionPred());
  reqGrpValues.accumulateReferencedValues(superSet, getGroupAttr()->getCharacteristicOutputs());
...
GroupByAgg *Join::pullUpGroupByTransformation(NormWA &normWARef) {
  ...
  RelExpr *filterParent = nestedAggInSubQ ? oldGB->child(0)->castToRelExpr() : oldGB;
  ...
  if (NOT(child(0)->getGroupAttr()->findUniqueCols(leftUniqueCols))) {
    ...
    if (CmpCommon::getDefault(SUBQUERY_UNNESTING) == DF_DEBUG)
      *CmpCommon::diags() << DgSqlCode(2997)
                          << DgString1(
                                 "Subquery was not unnested. Reason: Left child does not have a unique constraint");
    ...
    if (NOT selectionPred().isEmpty()) {
      ...
      if (CmpCommon::getDefault(SUBQUERY_UNNESTING) == DF_DEBUG) {
        *CmpCommon::diags()
            << DgSqlCode(2997)
            << DgString1(
                   "Subquery was not unnested. Reason: Join with selectionPreds cannot be converted to LeftJoin.");
      }
      filterParent->eliminateFilterChild();
      return NULL;
    }
  ...
  if (nestedAggInSubQ) {
    newSubQGrby = (GroupByAgg *)copyNode(oldGB->child(0)->castToRelExpr(), stmtHeap);
  ...
  if (nestedAggInSubQ) {
    ...
    newSubQGrby->getGroupAttr()->addCharacteristicOutputs(getGroupAttr()->getCharacteristicOutputs());
    newSubQGrby->getGroupAttr()->setCharacteristicInputs(getGroupAttr()->getCharacteristicInputs());
    newSubQGrby->child(0) = newJoin;
  }

  // inputs and outputs of new GroupBy are same as the old
  // TSJ/Join that we are replacing
  newGrby->getGroupAttr()->setCharacteristicOutputs(getGroupAttr()->getCharacteristicOutputs());
  newGrby->getGroupAttr()->setCharacteristicInputs(getGroupAttr()->getCharacteristicInputs());
  ...
  ValueIdSet oldLeftChildOutputs(oldLeftChild->getGroupAttr()->getCharacteristicOutputs());
  newGrby->computeGroupExpr(leftUniqueCols, oldLeftChildOutputs, normWARef);
  if (nestedAggInSubQ) {
    newSubQGrby->getGroupAttr()->addCharacteristicOutputs(newGrby->groupExpr());
    newSubQGrby->computeGroupExpr(newGrby->groupExpr(), oldLeftChildOutputs, normWARef);
  }
  ...
  else if (nestedAggInSubQ && oldGB->child(0)->child(0)->getOperatorType() == REL_FILTER)
    newJoin->selectionPred() += oldGB->child(0)->child(0)->castToRelExpr()->selectionPred();
  ...
  ValueIdSet outerReferences;
  newGrby->getGroupAttr()->getCharacteristicInputs().getOuterReferences(outerReferences);
  if (NOT(outerReferences.isEmpty())) {
    if (!nestedAggInSubQ)
      ...
      if (CmpCommon::getDefault(SUBQUERY_UNNESTING) == DF_DEBUG)
        *CmpCommon::diags() << DgSqlCode(2997)
                            << DgString1(
                                   "Subquery was not unnested. Reason: More than 1 level of nested subquery and nested "
                                   "aggregate are both present");
      return NULL;
    }
  }
...
NABoolean Join::unnestingDoEarlyGrouping(NormWA &normWARef) {
  /*	Return TRUE only if ALL the following are satisfied
        1-level subq
        no outer reference in agg.
        no null rejecting predicate
        join pred is an equality pred.  */

  if (CmpCommon::getDefault(SUBQUERY_UNNESTING_EARLY_GROUP) == DF_OFF) return FALSE;

  if ((normWARef.getCorrelatedSubqCount() != 1) || candidateForSubqueryLeftJoinConversion()) return FALSE;

  NABoolean hasNonEquiPred = FALSE;
  ValueIdSet preds;

  if (child(1)->child(0)->getOperatorType() == REL_FILTER)
    preds = child(1)->child(0)->castToRelExpr()->selectionPred();
  else if (child(1)->child(0)->getOperatorType() == REL_GROUPBY &&
           child(1)->child(0)->child(0)->getOperatorType() == REL_FILTER)
    preds = child(1)->child(0)->child(0)->castToRelExpr()->selectionPred();
  else
    return FALSE;

  if (child(1)->getOperatorType() == REL_GROUPBY) {
    GroupByAgg *scalarGB = (GroupByAgg *)child(1)->castToRelExpr();
    ValueIdSet nonLocalAggrs;
    ValueIdSet outerReferences = scalarGB->getGroupAttr()->getCharacteristicInputs();
    outerReferences -= getGroupAttr()->getCharacteristicInputs();
    if (scalarGB->aggregateExpr().getReferencedPredicates(outerReferences, nonLocalAggrs)) return FALSE;
    if (scalarGB->groupExpr().entries() != 0) return FALSE;
  } else
    return FALSE;

  for (ValueId x = preds.init(); preds.next(x); preds.advance(x)) {
    if ((x.getItemExpr()->getOperatorType() != ITM_VEG_PREDICATE) &&
        (x.getItemExpr()->getOperatorType() != ITM_EQUAL)) {
      hasNonEquiPred = TRUE;
      break;
    }
  }
  if (hasNonEquiPred) return FALSE;
...
Join *Join::scalarAggrToGroupByTransformation(NormWA &normWARef) {
  CollHeap *stmtHeap = CmpCommon::statementHeap();

  // oldGB is verified to be a GB in unnestingDoEarlyGrouping
  RelExpr *oldGB = child(1)->castToRelExpr();
  ...
  RelExpr *oldGBgrandchild;
  RelExpr *filterNode;
  GroupByAgg *nestedAggGB = NULL;
  NABoolean nestedAggInSubQ = FALSE;
  if (oldGB->child(0)->getOperatorType() == REL_FILTER) {
    oldGBgrandchild = oldGB->child(0)->child(0)->castToRelExpr();
    filterNode = oldGB->child(0)->castToRelExpr();
  } else if (oldGB->child(0)->getOperatorType() == REL_GROUPBY &&
             oldGB->child(0)->child(0)->getOperatorType() == REL_FILTER) {
    oldGBgrandchild = oldGB->child(0)->castToRelExpr();
    filterNode = oldGB->child(0)->child(0)->castToRelExpr();
    nestedAggGB = (GroupByAgg *)oldGB->child(0)->castToRelExpr();
    nestedAggInSubQ = TRUE;
  } else
    return NULL;
  ...
  // copy the right child of Join
  RelExpr *newRightChild = copyNodeAndSetChildren(oldGBgrandchild, stmtHeap);
  if (nestedAggInSubQ) newRightChild->child(0) = filterNode->child(0);
  ...
  // inputs and outputs of new Join are same as the old
  // TSJ that we are replacing
  newJoin->getGroupAttr()->setCharacteristicOutputs(getGroupAttr()->getCharacteristicOutputs());
  newJoin->getGroupAttr()->setCharacteristicInputs(getGroupAttr()->getCharacteristicInputs());

  newGrby->child(0) = newRightChild;
  ValueIdSet preds = filterNode->selectionPred();
  ValueIdList filterChildOutputs;
  filterChildOutputs.insertSet(filterNode->child(0)->getGroupAttr()->getCharacteristicOutputs());
  ValueId gid;
  for (ValueId x = preds.init(); preds.next(x); preds.advance(x)) {
    if ((x.getItemExpr()->getOperatorType() == ITM_VEG_PREDICATE) ||
        (x.getItemExpr()->getOperatorType() == ITM_EQUAL)) {
      gid = filterChildOutputs.extractVEGRefForEquiPredicate(x);
      if (gid == NULL_VALUE_ID)
        return NULL;
      else {
        newGrby->groupExpr() += gid;
        if (nestedAggInSubQ && nestedAggGB->groupExpr().contains(gid))
          newRightChild->getGroupAttr()->addCharacteristicOutput(gid);
        else if (nestedAggInSubQ)
          return NULL;  // we have a GB below the scalar Agg, but the grouping
        // columns of this GB contains a column that is not in the filter
        // pred. This means the filter pred cannot be moved in the way
        // shown above, so we give up on this transformation.
      }
    } else
      return NULL;
  }
  ...
  newJoin->child(0) = newLeftChild;
  newJoin->child(1) = newGrby;
  newJoin->setOperatorType(REL_JOIN);
  newJoin->setRETDesc(getRETDesc());

  // pull up predicates in filter to newJoin
  // do not change the filter itself in case we decide to not unnest
  newJoin->selectionPred() += filterNode->selectionPred();

  newJoin->selectionPred() += newGrby->selectionPred();  // need some guards here
  newGrby->selectionPred().clear();
  newGrby->getGroupAttr()->setCharacteristicInputs(getGroupAttr()->getCharacteristicInputs());
  if (nestedAggInSubQ)
    newRightChild->getGroupAttr()->setCharacteristicInputs(getGroupAttr()->getCharacteristicInputs());

  return newJoin;
}  // Join::scalarAggrToGroupByTransformation()
...
//             the road.
------------------------------------------------------------------------------*/
RelExpr *GroupByAgg::nullPreservingTransformation(GroupByAgg *oldGB, NormWA &normWARef) {
  ...
  if ((NOT filterPreds.isEmpty()) &&
      filterPreds.referencesOneValueFromTheSet(oldGBgrandchild->getGroupAttr()->getCharacteristicOutputs()) == FALSE)

  {
    ...
    if (CmpCommon::getDefault(SUBQUERY_UNNESTING) == DF_DEBUG) {
      *CmpCommon::diags() << DgSqlCode(2997)
                          << DgString1(
                                 "Subquery was not unnested. Reason: Join predicate has no references to inner table.");
    }
  ...
  CMPASSERT((newJoin->getOperatorType() == REL_JOIN) || (newJoin->getOperatorType() == REL_TSJ));
  ...
  normWARef.getSqoWA()->insertChangedRelExpr(oldGB, newJoin, SQO_REASSIGNED_VREGION, oldGBRegion->getRegionId(), 0, 1);
...
RelExpr *GroupByAgg::nullPreserveMyExprs(NormWA &normWARef) {
  ...
  if (newJoin->getOperatorType() != REL_LEFT_JOIN) return this;
  ...
  if (newGrby->aggregateExpr().containsCount()) {
    ...
    for (ValueId vid = newGrby->aggregateExpr().init(); newGrby->aggregateExpr().next(vid);
  ...
  if (aggregateExpr().containsOneTrue(oneTrueVid)) {
    ...
    Aggregate *dummyAgg = new (stmtHeap) Aggregate(ITM_COUNT_NONULL, constVal);

    // Create the count(1)>0 predicate.
    BiRelat *fakeCountPred = new (stmtHeap) BiRelat(ITM_GREATER, dummyAgg, new (stmtHeap) SystemLiteral(0));
    ...
    normWARef.getSqoWA()->insertChangedItemExpr(oneTrueVid, SQO_REPLACED, oneTrueVid.getItemExpr());
  ...
  if (newGrby->aggregateExpr().containsAnyTrue(anyTrueVid)) {
    ...
    UnLogic *newIsNullPred = new (stmtHeap) UnLogic(ITM_IS_NULL, constVal);
    ...
    BiLogic *newPred = new (stmtHeap) BiLogic(ITM_AND, anyTrueExpr, newNotPred);

    newPred->synthTypeAndValueId(TRUE);

    // Remember what we changed, so it can be restored if we need to back out..
    normWARef.getSqoWA()->insertChangedItemExpr(anyTrueVid, SQO_NEWCHILD, anyTrueExpr, 0);
  ...
  for (ValueId exprId = currentOutputs.init(); currentOutputs.next(exprId); currentOutputs.advance(exprId)) {
  ...
  ValueIdMap *rightChildMap = new (stmtHeap) ValueIdMap(newJoin->rightChildMapForLeftJoin());
  ...
  for (ValueId topMapId = gbyOutputs.init(); gbyOutputs.next(topMapId); gbyOutputs.advance(topMapId)) {
    ValueId bottomMapId;
  ...
  // join.
  if (mapNeeded) {
    ValueIdSet child0Outputs = newJoin->child(0)->getGroupAttr()->getCharacteristicOutputs();
    ValueIdSet child1Outputs = newJoin->child(1)->getGroupAttr()->getCharacteristicOutputs();
    ValueIdSet vic = child0Outputs.intersect(child1Outputs);
    for (ValueId topMapId = vic.init(); vic.next(topMapId); vic.advance(topMapId)) {
      ValueId bottomMapId;
      rightChildMap->rewriteValueIdUp(bottomMapId, topMapId);
      if (bottomMapId != topMapId) map->addMapEntry(topMapId, bottomMapId);
    }
...
-------------------------------------------------------------------------------*/
GroupByAgg *Join::moveUpGroupByTransformation(GroupByAgg *topGrby, NormWA &normWARef) {
  ...
  while ((joinRightChild->getOperatorType() == REL_GROUPBY) && (((GroupByAgg *)joinRightChild)->requiresMoveUp()) ||
    ...
    if (joinRightChild->getOperatorType() == REL_MAP_VALUEIDS) {
      moveUpMap = (MapValueIds *)copyNodeAndSetChildren(joinRightChild, stmtHeap);
      moveUpGrby = (GroupByAgg *)copyNodeAndSetChildren(joinRightChild->child(0)->castToRelExpr(), stmtHeap);
      moveUpMap->child(0) = moveUpGrby;
      moveUpMap->getGroupAttr()->clearLogProperties();
    } else {
      moveUpMap = NULL;
      moveUpGrby = (GroupByAgg *)copyNodeAndSetChildren(joinRightChild, stmtHeap);
    }

    moveUpGrby->getGroupAttr()->clearLogProperties();

    joinRightChild = copyNodeAndSetChildren(moveUpGrby->child(0)->castToRelExpr(), stmtHeap);
    ...
    if (moveUpMap) {
      ...
      moveUpMap->pushdownCoveredExpr(moveUpMap->getGroupAttr()->getCharacteristicOutputs(),
                                     moveUpMap->getGroupAttr()->getCharacteristicInputs(), emptySet);
    }

    movedUpGrbyTail->pushdownCoveredExpr(movedUpGrbyTail->getGroupAttr()->getCharacteristicOutputs(),
    ...
    if (moveUpMap != NULL && moveUpMap->getGroupAttr()->getCharacteristicOutputs().isEmpty()) {
    ...
    ValueIdSet outerReferences;
    moveUpGrby->getGroupAttr()->getCharacteristicInputs().getOuterReferences(outerReferences);
    if (outerReferences.isEmpty()) moveUpGrby->setRequiresMoveUp(FALSE);
    ...
    moveUpGrby->computeGroupExpr(movedUpGrbyTail->groupExpr(), moveUpGrby->groupExpr(), normWARef);
...
-------------------------------------------------------------------------------*/
NABoolean GroupByAgg::subqueryUnnestFinalize(ValueIdSet &newGrbyGroupExpr, NormWA &normWARef) {
  Join *newJoin = NULL;
  ...
  for (int i = 0; i < 2; i++) {
    ...
    // ---------------------------------------------------------------------
    if (newJoin->child(i)->selectionPred().getReferencedPredicates(outerReferences, nonLocalPreds)) {
  ...
  newJoin->pushdownCoveredExprSQO(valuesReqdByParent, availableInputs, newJoin->selectionPred(), emptySet,
  ...
  newRightChild->getGroupAttr()->getCharacteristicInputs().getOuterReferences(outerReferences);
  if (NOT(outerReferences.isEmpty())) {
    ...
    if (CmpCommon::getDefault(SUBQUERY_UNNESTING) == DF_DEBUG)
      *CmpCommon::diags() << DgSqlCode(2997)
                          << DgString1(
                                 "Subquery was not unnested. Reason: Right child has outer references that cannot be "
                                 "removed by current unnesting.");
  ...
  if (newJoin->getMoreOutputsIfPossible(additionalOutputsNeeded)) {
    ...
    if (CmpCommon::getDefault(SUBQUERY_UNNESTING) == DF_DEBUG)
      *CmpCommon::diags()
          << DgSqlCode(2997)
          << DgString1(
                 "Subquery was not unnested. Reason: Left subtree cannot produce output values required for grouping.");
    return FALSE;
  }
  return TRUE;
}  // GroupByAgg::subqueryUnnestFinalize()
...
/*----------------------------------------------------------------------------
// Join::subqueryUnnestFinalize()
// set up inputs/outputs of the new Join and its children
// move selection predicates to the appropriate nodes.
// return FALSE if any outer references remain
//
// Expects: this pointer to be newJoin
// Sideffects: recomputed inputs and outputs of the join child's children
//             recomputed inputs and outputs of the join.
//             pushes any of the groupBy's predicates down that can go down.
//             pushes any of the join's predicates down that can go down.
...
-------------------------------------------------------------------------------*/
NABoolean Join::subqueryUnnestFinalize(NormWA &normWARef) {
  Join *newJoin = this;
  RelExpr *newLeftChild = newJoin->child(0)->castToRelExpr();
  RelExpr *newRightChild = newJoin->child(1)->castToRelExpr();
  newLeftChild->primeGroupAttributes();
  ...
  ValueIdSet emptySet, valuesReqdByParent, availableInputs, outerReferences;
  availableInputs = newJoin->getGroupAttr()->getCharacteristicInputs();
  valuesReqdByParent = newJoin->getGroupAttr()->getCharacteristicOutputs();
  ...
  // check if right child still contains outer references. If so
  // this subquery level cannot be unnested. Give up and return the
  // old TSJ. Note that other subquery levels may still be
  // successfully unnested.
  outerReferences.clear();
  newRightChild->getGroupAttr()->getCharacteristicInputs().getOuterReferences(outerReferences);
  outerReferences -= newJoin->getGroupAttr()->getCharacteristicInputs();
  if (NOT(outerReferences.isEmpty())) {
    // right child still has outer references
    // cannot unnest this subquery
    if (CmpCommon::getDefault(SUBQUERY_UNNESTING) == DF_DEBUG)
...
NABoolean Join::applyInnerKeyedAccessHeuristic(const GroupByAgg *newGrby, NormWA &normWARef) {
  RelExpr *oldGBgrandchild;
  ...
  if ((CmpCommon::getDefault(COMP_BOOL_168) == DF_OFF) && (oldGBgrandchild->getOperatorType() == REL_SCAN) &&
      ((normWARef.getCorrelatedSubqCount() == 1) || (newGrby && NOT(newGrby->requiresMoveUp()))) &&
      (getGroupAttr()->getNumJoinedTables() <= ActiveSchemaDB()->getDefaults().getAsLong(COMP_INT_46))) {
    RelExpr *oldGB = child(1)->castToRelExpr();

    const TableDesc *tableDesc = ((Scan *)oldGBgrandchild)->getTableDesc();
    ...
    for (CollIndex j = 0; j < ixlist.entries(); j++) {
      ...
      if (colIE->getOperatorType() == ITM_VEG_REFERENCE) {
        ...
        leadingKeyCols += ((VEGReference *)colIE)->getVEG()->getValueId();
      }
    }
    for (ValueId x = preds.init(); preds.next(x); preds.advance(x)) {
        ...
        if (leadingKeyCols.contains(id)) {
          ...
          if (CmpCommon::getDefault(SUBQUERY_UNNESTING) == DF_DEBUG)
            *CmpCommon::diags()
                << DgSqlCode(2997)
                << DgString1(
                       "Subquery was not unnested. Reason: Join predicate is on a leading key column of inner table.");
          return TRUE;
        }
      }
    }
...
RelExpr *Join::semanticQueryOptimizeNode(NormWA &normWARef) {
  ...
  if (reducedExpr != this) return reducedExpr;
  ...
  if (candidateForSubqueryUnnest()) {
    ...
    if ((CmpCommon::getDefault(COMP_BOOL_221) == DF_OFF) &&
        ((child(1)->getArity() != 1) || !(child(1)->castToRelExpr()->hasFilterChild()))) {
      if (CmpCommon::getDefault(SUBQUERY_UNNESTING) == DF_DEBUG)
        *CmpCommon::diags() << DgSqlCode(2997) << DgString1("Subquery was not unnested. Reason: No Correlation found");
    ...
    Join *newJoin = NULL;
    NABoolean hasNoErrors = TRUE;
    // if we enter this IF block and come back with something other than through return,
    // then continue on to do pullUpGroupby
    if (unnestingDoEarlyGrouping(normWARef)) {
      newJoin = scalarAggrToGroupByTransformation(normWARef);
      if (newJoin) {
        if (applyInnerKeyedAccessHeuristic(NULL, normWARef)) return this;
        hasNoErrors = newJoin->subqueryUnnestFinalize(normWARef);

        // this subquery level has been successfully unnested. Left linearize the
        // join backbone.
        if (hasNoErrors && ((ActiveSchemaDB()->getDefaults().getAsLong(COMP_INT_11) < 0) ||
        ...
        if (hasNoErrors) {
          if (CmpCommon::getDefault(SUBQUERY_UNNESTING) == DF_DEBUG)
            *CmpCommon::diags() << DgSqlCode(2997)
                                << DgString1("Subquery unnested using scalarAggrToGroupBy transformation.");
          return newJoin;
        }
      }
    }
    ...
    if (applyInnerKeyedAccessHeuristic((const GroupByAgg *)newGrby, normWARef)) return this;

    MapValueIds *newMap = NULL;

    if (candidateForSubqueryLeftJoinConversion()) {
      RelExpr *result = newGrby->nullPreservingTransformation((GroupByAgg *)child(1)->castToRelExpr(), normWARef);
      if (result == NULL) {
        normWARef.getSqoWA()->undoChanges(normWARef);
        return this;
      }
      if (result->getOperatorType() == REL_MAP_VALUEIDS) newMap = (MapValueIds *)result;
    }
    ...
    GroupByAgg *movedUpGrbyTail = newJoin->moveUpGroupByTransformation(newJoinParent, normWARef);
    if (movedUpGrbyTail != NULL) {
      hasNoErrors = movedUpGrbyTail->subqueryUnnestFinalize(newGrby->groupExpr(), normWARef);
    }
    ...
    if ((ActiveSchemaDB()->getDefaults().getAsLong(COMP_INT_11) < 0) ||
        (child(1)->getGroupAttr()->getNumJoinedTables() <= ActiveSchemaDB()->getDefaults().getAsLong(COMP_INT_11))) {
...
NABoolean Join::prepareMeForCSESharing(const ValueIdSet &outputsToAdd, const ValueIdSet &predicatesToRemove,
  ...
  if (isTSJForWrite() || isTSJForUndo() || isTSJForMerge() || getIsForTrafLoadPrep()) return FALSE;
...
// Union::transformNode()
// -----------------------------------------------------------------------
void Union::transformNode(NormWA &normWARef, ExprGroupId &locationOfPointerToMe) {
  CMPASSERT(this == locationOfPointerToMe);
  ...
  //++Triggers -
  if (getBlockedUnion()) normWARef.setInBlockedUnionCount();
  ...
  child(1)->getGroupAttr()->addCharacteristicInputs(getGroupAttr()->getCharacteristicInputs());
  ...
  if ((getIsTemporary()) && (getGroupAttr()->getCharacteristicOutputs().entries() == 0)) {
  ...
  //++Triggers -
  if (getBlockedUnion()) normWARef.restoreInBlockedUnionCount();
...
void Union::rewriteNode(NormWA &normWARef) {
  ...
  // normalizing outputs.
  if (getIsTemporary()) normWARef.locateAndSetVEGRegion(this, 0);
...
// -----------------------------------------------------------------------
// ConnectBy::normalizeNode()
// -----------------------------------------------------------------------
RelExpr *ConnectBy::normalizeNode(NormWA &normWARef) {
  if (nodeIsNormalized()) return this;
  markAsNormalized();
  ValueIdSet emptySet;
  pushdownCoveredExpr(getGroupAttr()->getCharacteristicOutputs(), getGroupAttr()->getCharacteristicInputs(), emptySet);
...
void ConnectBy::transformNode(NormWA &normWARef, ExprGroupId &locationOfPointerToMe) {
  CMPASSERT(this == locationOfPointerToMe);
  ...
  reqdOrder().transformNode(normWARef, child(1), getGroupAttr()->getCharacteristicInputs());
  pullUpPreds();

  // mark the SelectPred as isStartWithExpr
  for (ValueId x = selectionPred().init(); selectionPred().next(x); selectionPred().advance(x)) {
    x.getItemExpr()->setIsStartWithExpr(TRUE);
  }

  transformSelectPred(normWARef, locationOfPointerToMe);
}

RelExpr *ConnectBy::semanticQueryOptimizeNode(NormWA &normWARef) {
  condExpr() += selectionPred();
...
void ConnectBy::rewriteNode(NormWA &normWARef) {
  child(0)->rewriteNode(normWARef);
  ...
  if (leftPathItemList().normalizeNode(normWARef)) {
  }
  if (rightPathItemList().normalizeNode(normWARef)) {
  }
  getGroupAttr()->normalizeInputsAndOutputs(normWARef);
}
...
NABoolean Union::prepareTreeForCSESharing(const ValueIdSet &outputsToAdd, const ValueIdSet &predicatesToRemove,
                                          const ValueIdSet &commonPredicatesToAdd, const ValueIdSet &inputsToRemove,
                                          ValueIdSet &valuesForVEGRewrite, ValueIdSet &keyColumns, CSEInfo *info) {
  ...
  if (getSelectionPred().entries() > 0) {
    info->getConsumer(0)->emitCSEDiagnostics("Selection predicates on union node not supported");
    return FALSE;
  }
  ...
  for (CollIndex i = 0; i < 2 && result; i++) {
    ...
    if (locOutputsToAdd.removeUnCoveredExprs(availableValues)) {
      info->getConsumer(0)->emitCSEDiagnostics("Not able to add output values unknown to union operator");
      result = FALSE;
    }
    ...
    result = child(i)->prepareTreeForCSESharing(childOutputsToAdd, childPredsToRemove, childPredsToAdd, inputsToRemove,
                                                dummyValuesForVEGRewrite, childKeyColumns, info);
  ...
  if (result) {
    ...
    for (CollIndex u = 0; u < nu; u++) {
      if (unionMap_->leftColMap_.getBottomValues()[u].getItemExpr()->castToConstValue(dummy) ||
          unionMap_->rightColMap_.getBottomValues()[u].getItemExpr()->castToConstValue(dummy))
...
// ***********************************************************************
void GroupByAgg::transformNode(NormWA &normWARef, ExprGroupId &locationOfPointerToMe) {
  CMPASSERT(this == locationOfPointerToMe);
  ...
  // ---------------------------------------------------------------------
  if (groupExpr().transformNode(normWARef, child(0), getGroupAttr()->getCharacteristicInputs(),
  ...
  if (aggregateExpr().transformNode(normWARef, child(0), getGroupAttr()->getCharacteristicInputs(),
                                    FALSE /* Move predicates */)) {
...
void GroupByAgg::pullUpPreds() {
  ...
  // cannot be moved up.
  if (groupExpr().isEmpty() && NOT containsNullRejectingPredicates()) return;

  if (child(0)->getSelectionPred().isEmpty()) return;
  ...
  getGroupAttr()->coverTest(child(0)->selectionPred(),
                            ...
                            predicatesToPullUp, boringSet, &predicatesThatStay);
...
void GroupByAgg::rewriteNode(NormWA &normWARef) {
  ...
  // ---------------------------------------------------------------------
  if (needsNewVEGRegion) normWARef.restoreOriginalVEGRegion();
  // ---------------------------------------------------------------------
...
RelExpr *GroupByAgg::normalizeNode(NormWA &normWARef) {
  ...
  // ---------------------------------------------------------------------
  DBGSETDBG("TRANSFORM_DEBUG")
  DBGIF(unp = ""; unp += "sel:"; selectionPred().unparse(unp); unp += "\nagg:"; aggregateExpr().unparse(unp);)
      ...
      if (bottomOfTest->isAnAggregate())
        for (ValueId avid = agg.init(); agg.next(avid); agg.advance(avid))
          if (bottomOfTest == avid.getItemExpr()) {
            DBGIF(cerr << unp << endl; cerr << "Eliminating aggr " << svid << endl;)
            sel.subtractElement(svid);  // svid, not avid!
          } else {
            DBGIF(cerr << unp << endl; cerr << "Eliminating having-pred " << svid << endl;)
  ...
  // if this is a rollup groupby, then do not pushdown having pred of
  // form A is NULL or A is not NULL to child node. If pushdown is done,
  // then it might incorrectly process rows that
  ...
  //
  //  This fix is essential for Mantis #1629
  //  (Extra exchange nodes are added for TPCDS query Q18 and Q59) in that
  //  pushdownCoveredExpr() minimizes characteristics output at the scan level,
  //  in particular, the virtual column INPUT_FILE_NAME of char(4096). Without
  //  the minimization, this virtual column will contributes to the overal
  //  record length of the output of the scan and make the cost broadcast
  //  very high.
  //
  if (isRollup()) {
    //  We retain any A is NULL or A is NOT NULL predicate in this node.
    ValueIdSet pushdownPreds(selectionPred());
    ValueIdSet nullPreds;

    // Remove isNull or isNotNull predicates from selectionPred.
    pushdownPreds.removeIsNullIsNotNullExprs(groupExpr(), nullPreds);

    // Only push down non isNull or isNotNull predicates in selectionPred.
    pushdownCoveredExpr(getGroupAttr()->getCharacteristicOutputs(), getGroupAttr()->getCharacteristicInputs(),
                        pushdownPreds);

    // Modify selectPred accondingly
    selectionPred() = pushdownPreds;
    selectionPred() += nullPreds;
  } else {
    pushdownCoveredExpr(getGroupAttr()->getCharacteristicOutputs(), getGroupAttr()->getCharacteristicInputs(),
                        selectionPred());
  }
...
// VEGRegion we are currently in.
//
// INTERSECT GROUPBY PUSH-DOWN:
//
// Some intersects may produce many duplicates in their children, and a join
// of the children could multiply those. TPC-DS query 14 is a good example
// for this. This cardinality-based transformation checks the ratio of the
// cardinalities of the join and its children and, if the join
// causes an explosion of rows, does the following transformation
//
//   GroupBy                                 Join (intersect)
//      |                                    /  \
//    Join (intersect)                 GroupBy  GroupBy
//    /  \                ==>             |        |
//   X    Y                               X        Y
//
// -----------------------------------------------------------------------
RelExpr *GroupByAgg::semanticQueryOptimizeNode(NormWA &normWARef) {
  ...
  // Check for intersect groupby pushdown.
  // Do this before the child has left-linearized the join tree.
  if (  // there are intersects in this tree that should be transformed
      normWARef.getIntersectsToXformCount() > 0 &&
      // and this is a groupby/join combination of an INTERSECT
      child(0)->getOperatorType() == REL_JOIN && static_cast<Join *>(child(0).getPtr())->isIntersect() &&
      aggregateExpr().isEmpty() && groupExpr() == child(0).getGroupAttr()->getCharacteristicOutputs()) {
    Join *join = static_cast<Join *>(child(0).getPtr());
    double ratio = ActiveSchemaDB()->getDefaults().getAsDouble(INTERSECT_PUSH_GRBY_RATIO);

    // cardinalities of the join and its children
    CostScalar joinCard = join->getGroupAttr()->outputLogProp((*GLOBAL_EMPTY_INPUT_LOGPROP))->getResultCardinality();
    CostScalar joinChild0Card =
        join->child(0)->getGroupAttr()->outputLogProp((*GLOBAL_EMPTY_INPUT_LOGPROP))->getResultCardinality();
    CostScalar joinChild1Card =
        join->child(1)->getGroupAttr()->outputLogProp((*GLOBAL_EMPTY_INPUT_LOGPROP))->getResultCardinality();

    // Apply a CQD-based heuristic on whether to push a groupby down
    // to the children: Compare the cardinality of a 1:n join (the max
    // of the child cardinalities) with the actual cardinality of
    // the join representing the intersect. If the actual cardinality
    // is more than <ratio> times higher that that of a 1:n join,
    // then insert groupby nodes.
    // Effect of different values for CQD INTERSECT_PUSH_GRBY_RATIO:
    // < 0: Never make the transformation
    //   0: Always make the transformation
    // > 0: Make the transformation based on the cardinality ratio
    CostScalar oneToNJoinCard = MAXOF(joinChild0Card, joinChild1Card);

    if (joinCard > oneToNJoinCard * ratio && ratio >= 0) {
      for (int c = 0; c < 2; c++) {
        RelExpr *joinChild = join->child(c);

        // only add the child groupby if it is needed, i.e. the
        // child's outputs are not already unique
        if (!joinChild->getGroupAttr()->isUnique(joinChild->getGroupAttr()->getCharacteristicOutputs())) {
          GroupAttributes *joinChildGA = joinChild->getGroupAttr();

          GroupByAgg *newGrby = new (CmpCommon::statementHeap()) GroupByAgg(joinChild);

          // group on the characteristic outputs, the INTERSECT
          // applies uniqueness on all the output columns, similar
          // to a UNION
          newGrby->setGroupExpr(joinChildGA->getCharacteristicOutputs());

          newGrby->setGroupAttr(new (CmpCommon::statementHeap()) GroupAttributes(*joinChildGA));
          newGrby->getGroupAttr()->clearLogProperties();
          newGrby->synthLogProp(&normWARef);

          join->child(c) = newGrby;
        }
      }
      join->getGroupAttr()->setCharacteristicOutputs(getGroupAttr()->getCharacteristicOutputs());
      // the logical properties of the join need to be recomputed
      join->getGroupAttr()->clearLogProperties();

      // eliminate "this", the groupby, from the result
      result = join;
    } else {
      // left-linearize the join tree if we didn't push groupby
      // nodes, and request to include intersect nodes (default
      // is to exclude them)
      child(0) = join->leftLinearizeJoinTree(normWARef, Join::NO_TRANSFORMATION, TRUE);
    }
  }

  // ---------------------------------------------------------------------
  // UnNest the child or new result
  // ---------------------------------------------------------------------
  if (result == this)
    child(0) = child(0)->semanticQueryOptimizeNode(normWARef);
  else
    result = result->semanticQueryOptimizeNode(normWARef);

  if (needsNewVEGRegion) normWARef.restoreOriginalVEGRegion();

  if (result == this) eliminateCascadedGroupBy(normWARef);
...
void GroupByAgg::checkForCascadedGroupBy(NormWA &normWARef) {
  if (child(0)->getOperatorType() == REL_GROUPBY) {
    ...
    if (childGB->groupExpr().contains(groupExpr()) && childGB->selectionPred().isEmpty()) {
      NABoolean allExprsCanBeRolledup = TRUE;

      for (ValueId x = aggregateExpr().init(); aggregateExpr().next(x) && allExprsCanBeRolledup;
           aggregateExpr().advance(x)) {
        CMPASSERT(x.getItemExpr()->isAnAggregate());
        Aggregate *aggrExpr = (Aggregate *)x.getItemExpr();

        if (!aggrExpr->isDistinct() && aggrExpr->child(0)->isAnAggregate()) {
          Aggregate *childAggrExpr = (Aggregate *)aggrExpr->child(0)->castToItemExpr();

          if (!childAggrExpr->isDistinct()) {
            NABoolean aggInChildGroupExpr = (childGB->groupExpr().contains(childAggrExpr->getValueId())) ? true : false;
            switch (aggrExpr->getOperatorType()) {
              ...
              case ITM_MIN:
                if (aggrExpr->child(0)->getOperatorType() != ITM_MIN || aggInChildGroupExpr)
                  allExprsCanBeRolledup = FALSE;
                break;
              case ITM_MAX:
                if (aggrExpr->child(0)->getOperatorType() != ITM_MAX || aggInChildGroupExpr)
                  allExprsCanBeRolledup = FALSE;
                break;
          ...
          if (normWARef.compilingMVDescriptor() &&
              (aggrExpr->getOperatorType() == ITM_COUNT && aggrExpr->child(0)->getOperatorType() == ITM_CONSTANT))
            aggrExprsToBeDeleted() += x;
          else
            allExprsCanBeRolledup = FALSE;
        }
...
void GroupByAgg::eliminateCascadedGroupBy(NormWA &normWARef) {
  if (child(0)->getOperatorType() == REL_GROUPBY) {
    ...
    if (childGB->isMarkedForElimination()) {
      for (ValueId y = aggrExprsToBeDeleted().init(); aggrExprsToBeDeleted().next(y);
           aggrExprsToBeDeleted().advance(y)) {
        ItemExpr *constValue = new (CmpCommon::statementHeap()) SystemLiteral(&(y.getType()), &value, sizeof(short));
      ...
      for (ValueId x = aggregateExpr().init(); aggregateExpr().next(x); aggregateExpr().advance(x)) {
        CMPASSERT(x.getItemExpr()->isAnAggregate());
        ...
        if (aggrExpr->getOperatorType() == ITM_SUM && aggrExpr->child(0)->getOperatorType() == ITM_COUNT) {
...
NABoolean GroupByAgg::prepareMeForCSESharing(const ValueIdSet &outputsToAdd, const ValueIdSet &predicatesToRemove,
                                             const ValueIdSet &commonPredicatesToAdd, const ValueIdSet &inputsToRemove,
                                             ValueIdSet &valuesForVEGRewrite, ValueIdSet &keyColumns, CSEInfo *info) {
  ...
  NABoolean allCovered =
      outputsToAdd.isCovered(myAvailableValues, *(getGroupAttr()), referencedValues, myOutputsToAdd, unCoveredExpr);

  if (allCovered) myOutputsToAdd = outputsToAdd;
...
// Scan::transformNode()
// -----------------------------------------------------------------------
void Scan::transformNode(NormWA &normWARef, ExprGroupId &locationOfPointerToMe) {
  CMPASSERT(this == locationOfPointerToMe);
  ...
  for (CollIndex i = 0; i < allCols.entries(); i++) {
    ...
    newPtr = oldPtr;
    oldPtr->transformNode(normWARef, newPtr, locationOfPointerToMe, getGroupAttr()->getCharacteristicInputs());
    ...
    if (oldPtr->getOperatorType() == ITM_BASECOLUMN) {
      ...
      ValueIdList ckColumns = getTableDesc()->getClusteringIndex()->getIndexKey();

      for (CollIndex j = 0; j < ckColumns.entries(); j++) {
        if (allCols[i].getNAColumn()->getPosition() == ckColumns[j].getNAColumn()->getPosition()) {
      ...
      if (isClusteringKeyColumn && allCols[i].getType().supportsSQLnull() && eic.entries() > 0) {
        ItemExpr *vegrefPtr = normWARef.getVEGReference(allCols[i]);
        if (vegrefPtr) ((VEGReference *)vegrefPtr)->getVEG()->setSpecialNulls(TRUE);
      }
    } else
  ...
  NABoolean isHiveTable = getTableDesc()->getNATable()->isHiveTable();
  if (!isHiveTable)
    // Allow creation of redundant predicates from OR backbones
    // only for Hive scans; we may be able to push such predicates
    // down to Hive. For Trafodion, it is just redundant processing
    // so it is of no help.
    normWARef.setDontAddRedundantPredicatesFlag();

  transformSelectPred(normWARef, locationOfPointerToMe);

  if (!isHiveTable) normWARef.restoreDontAddRedundantPredicatesFlag();
...
void Scan::rewriteNode(NormWA &normWARef) {
  ...
  for (i = 0; i < (int)getTableDesc()->getIndexes().entries(); i++) {
    ...
    // ---------------------------------------------------------------------
    if (idesc->isPartitioned()) idesc->getPartitioningFunction()->normalizePartitioningKeys(normWARef);
  }
...
// Scan::normalizeNode()
// -----------------------------------------------------------------------
RelExpr *Scan::normalizeNode(NormWA &normWARef) {
  if (nodeIsNormalized()) return this;

  RelExpr::normalizeNode(normWARef);

  if (CmpCommon::getDefault(RANGESPEC_TRANSFORMATION) != DF_OFF && !normWARef.inMVQueryRewrite() &&
      (NOT getTableDesc()->getNATable()->hasCompositeColumns())) {
    ValueIdSet vs, vs1;
    ValueId exprId;
    ...
    if (inputItemExprTree != NULL) {
      ...
      result = applyAssociativityAndCommutativity(descGenerator, heap, inputItemExprTree, normWARef, transStatus);
      if (transStatus) {
        // result->synthTypeAndValueId(); // You can not remove it, it causes regression (however Bob told in Code
        // review) for case core/test029 delete from T29xv2 where j like 'f%';		-- ok ->ValueId of AND node is
        // not available.
  ...
  if (normWARef.getMergeUpdDelCount() == 0)
    for (exprId = preds.init(); preds.next(exprId); preds.advance(exprId)) {
      if (exprId.getItemExpr()->canTransformToSemiJoin(valuesListIE, getTableDesc(), numParams, colVid,
        ...
        TupleList *tl = new (normWARef.wHeap()) TupleList(valuesListIE.convertToItemExpr(RIGHT_LINEAR_TREE));
        tl->setCreatedForInList(TRUE);
        RelRoot *rr = new (normWARef.wHeap()) RelRoot(tl);
        retItemExpr = new (normWARef.wHeap()) QuantifiedComp(ITM_EQUAL_ANY, colVid.getItemExpr(), rr, FALSE);
        ...
        retItemExpr->transformNode(normWARef, nePtr, newJoin, getGroupAttr()->getCharacteristicInputs());
        if (!(newJoin->getOperator().match(REL_SEMITSJ))) continue;
  ...
  if ((RelExpr *)newJoin != this) {
    ((RelExpr *)newJoin)->getGroupAttr()->setCharacteristicOutputs(getGroupAttr()->getCharacteristicOutputs());
    ((RelExpr *)newJoin)->getGroupAttr()->setCharacteristicInputs(getGroupAttr()->getCharacteristicInputs());
...
NABoolean Scan::prepareMeForCSESharing(const ValueIdSet &outputsToAdd, const ValueIdSet &predicatesToRemove,
                                       const ValueIdSet &commonPredicatesToAdd, const ValueIdSet &inputsToRemove,
                                       ValueIdSet &valuesForVEGRewrite, ValueIdSet &keyColumns, CSEInfo *info) {
  // The caller of this method took care of most adjustments to
  ...
  // can produce here.
  outputsToAdd.isCovered(myColSet, *(getGroupAttr()), referencedCols, myOutputsToAdd, unCoveredExpr);
  ...
  keyColumns.insertList(((IndexDesc *)getTableDesc()->getClusteringIndex())->getHivePartCols());
...
NABoolean Scan::passSemiJoinHeuristicCheck(ValueId vid, int numValues, int numParams, ValueId colVid) const {
  ...
  if ((pcodeOptLevel == DF_OFF) || (pcodeOptLevel == DF_MINIMUM)) {
    noPCodeSupport = TRUE;
  }
  if (colVid.getType().getTypeQualifier() == NA_NUMERIC_TYPE) {
    const NumericType &ntype = (NumericType &)colVid.getType();
    if (ntype.isBigNum() || ntype.isDecimal() || (ntype.getScale() > 0)) unSupportedType = TRUE;
  }
  if (numValues > orPredToSemiJoin)  // num of in list values still has to be
  {                                  // greater than OR_PRED_TO_SEMIJOIN
    if (noPCodeSupport || (orPredToJumpTable == 0) ||
        (orPredToJumpTable < numValues) ||  // hash table imp. is OFF or In list VERY large
  ...
  if (totalRowCount > orPredToSemiJoinTableMinSize) isBigTable = TRUE;
  ...
  for (CollIndex ix = 0; ix < ixlist.entries(); ix++) {
    ...
    if (partKeyColIndex != NULL_COLL_INDEX)  // 'a' is a partitioning key column
    {
      ...
      if ((numValues < 0.5 * (idesc->getNAFileSet()->getCountOfPartitions())) && isBigTable &&
      ...
      for (CollIndex i = 0; (applyPartKeyHeuristic && (i < partKeyCols.entries())); i++) {
        if (i == partKeyColIndex) continue;
        if (!partKeyCols[i].getItemExpr()->doesExprEvaluateToConstant(
                FALSE, TRUE))  // equality preds on all part key columns, except 'a'
        {
          applyPartKeyHeuristic = FALSE;
        }
      }
      if (applyPartKeyHeuristic) return TRUE;
    }
    ...
    if ((keyColIndex != NULL_COLL_INDEX) && isBigTable)  // 'a' is a key column of this index
    {
      ...
      NABoolean predOnLoneSaltCol = FALSE;
      for (CollIndex i = 0; i < keyCols.entries(); i++) {
        if (i == keyColIndex) continue;
        if ((i == 0) && (keyCols[0].isSaltColumn())) {
          if ((partKeyCols.entries() == 1) && (partKeyCols[0] == colVid)) {
            predOnLoneSaltCol = TRUE;
            continue;
          }
        }
      ...
      if (fullKeyConstant) return TRUE;
      if (keyConstantUptoCol && ((numValues > orPredToSemiJoin) || predOnLoneSaltCol)) return TRUE;
      ...
      SearchKey *skey =
          new (CmpCommon::statementHeap()) SearchKey(idesc->getIndexKey(), idesc->getOrderOfKeyValues(), externalInputs,
                                                     TRUE, nonKeyPredicates, nonKeyColumnSet, idesc);
      ...
      if ((keyPredRowCount < ((CostScalar)orPredToSemiJoinMaxRatio) * totalRowCount) && (numValues > orPredToSemiJoin))
...
// ***********************************************************************
void GenericUpdate::transformNode(NormWA &normWARef, ExprGroupId &locationOfPointerToMe) {
  CMPASSERT(this == locationOfPointerToMe);
  ...
  if (newRecExpr().transformNode(normWARef, child(0), getGroupAttr()->getCharacteristicInputs(),
                                 FALSE /* Move predicates */)) {
  ...
  normWARef.setInGenericUpdateAssign(TRUE);
  // QSTUFF
  if (newRecBeforeExpr().transformNode(normWARef, child(0), getGroupAttr()->getCharacteristicInputs(),
                                       FALSE /* Move predicates */)) {
    normWARef.setInGenericUpdateAssign(origInGenericUpdateAssignFlag);
    // -----------------------------------------------------------------
    // Transform my new child.
  ...
  if (isMerge()) {
    normWARef.setInGenericUpdateAssign(TRUE);
    if (mergeInsertRecExpr().transformNode(normWARef, child(0), getGroupAttr()->getCharacteristicInputs(),
                                           FALSE /* Move predicates */)) {
      normWARef.setInGenericUpdateAssign(origInGenericUpdateAssignFlag);
      // -----------------------------------------------------------------
      // Transform my new child.
    ...
    if (mergeUpdatePred().transformNode(
            ...
            FALSE /* Move predicates */)) {  // restore previous "are we in mergeUpdateWhere?" flag
  ...
  for (exprId = mergeUpdatePred().init(); mergeUpdatePred().next(exprId); mergeUpdatePred().advance(exprId)) {
    ItemExpr *thisIE = exprId.getItemExpr();
    thisIE = thisIE->removeOneRowAggregate(thisIE, normWARef);
  }
  ...
  if (keyList.entries() > 0) {
    for (CollIndex i = 0; i < keyList.entries(); i++) {
      ...
      (*eqPtr)[1]->transformNode(normWARef, eqPtr->child(1), child(0), getGroupAttr()->getCharacteristicInputs());
  ...
  // ---------------------------------------------------------------------
  // ---------------------------------------------------------------------
  beginKeyPred().transformNode(normWARef, child(0), getGroupAttr()->getCharacteristicInputs());
  ...
  checkConstraints().transformNode(normWARef, child(0), getGroupAttr()->getCharacteristicInputs());
  ...
  if (child(0)) child(0)->recomputeOuterReferences();
...
void GenericUpdate::rewriteNode(NormWA &normWARef) {
  ...
  partQualPreCond_.normalizeNode(normWARef);
  ...
  if (getOperatorType() == REL_UNARY_INSERT) {
    Insert *ins = (Insert *)(this->castToRelExpr());
...
void GenericUpdate::recomputeOuterReferences() {
  ...
  for (vid = outerRefs.init(); outerRefs.next(vid); outerRefs.advance(vid)) {
    ...
    if (ie->getOperatorType() != ITM_CONSTANT) {
      if ((vid.getType().getTypeQualifier() == NA_ROWSET_TYPE) ||
          ((ie->getOperatorType() == ITM_DYN_PARAM) && (((DynamicParam *)ie)->getRowsetSize() != 0)))
        inRowsets += vid;
    }
  }
  ...
  allMyExpr += getPartQualPreCond();
  allMyExpr += getPrecondition();
  allMyExpr.weedOutUnreferenced(outerRefs);
...
RelExpr *GenericUpdate::normalizeNode(NormWA &normWARef) {
  ...
  if (isMerge()) normWARef.incrementMergeUpdDelCount();
  ...
  if (getOperator().match(REL_ANY_UNARY_GEN_UPDATE)) {
    ...
    if (scan && scan->requiresHalloweenForUpdateUsingIndexScan()) setAvoidHalloween(TRUE);
  }

  if (producedMergeIUDIndicator_ != NULL_VALUE_ID) {
    ValueId dummy;
    if (NOT getGroupAttr()->getCharacteristicOutputs().referencesTheGivenValue(producedMergeIUDIndicator_, dummy))
...
RelExpr *Insert::normalizeNode(NormWA &normWARef) {
  ...
  if (normalizedThis->getOperatorType() == REL_LEAF_INSERT) return normalizedThis;
  ...
  if ((reqdOrder().entries() > 0) && (child(0)->getOperatorType() == REL_FIRST_N)) {
  ...
  // If this is load-prep, skip single tuple optimization
  CMPASSERT(normalizedThis->getArity() > 0);
  if ((normalizedThis->child(0)->getOperatorType() != REL_TUPLE) || (getIsTrafLoadPrep())) return normalizedThis;
...
// -----------------------------------------------------------------------
NABoolean RelRoot::isUpdatableBasic(NABoolean isView, NABoolean &isInsertable) const {
  ...
  if (child(0)->getGroupAttr()->isEmbeddedUpdateOrDelete() && child(0)->getGroupAttr()->isGenericUpdateRoot()) {
  ...
  if (scan->getOperatorType() != REL_SCAN) return FALSE;

  if (scan->accessOptions().accessType() == TransMode::READ_UNCOMMITTED_ACCESS_)  // "read-only table"
    return FALSE;

  TransMode::IsolationLevel il;
  if ((NOT isView) || (CmpCommon::getDefault(ISOLATION_LEVEL_FOR_UPDATES) == DF_NONE))
  ...
  NATable *naTable = ActiveSchemaDB()->getNATableDB()->get(&scan->getTableDesc()->getNATable()->getExtendedQualName());
  CMPASSERT(naTable);

  if (naTable->getReferenceCount() > 1)
    // QSTUFF
    if (getGroupAttr()->isEmbeddedUpdateOrDelete()) {
      if (naTable->getReferenceCount() > 2) return FALSE;
    } else
      // QSTUFF
      // A subquery references the scan tbl
      return FALSE;

  if (naTable->isAnMV()) return FALSE;  // A materialized view is not updatable. -- MV
  ...
  if (isView) {
    for (CollIndex i = 0; i < compExpr().entries(); i++) {
      ...
      if (getGroupAttr()->isEmbeddedUpdateOrDelete()) {
        CMPASSERT(gu);
        for (CollIndex j = 0; j < gu->getTableDesc()->getColumnList().entries(); j++) {
          if (gu->getTableDesc()->getColumnList()[j].getItemExpr()->getValueId() == idcol) {
            idcol = scan->getTableDesc()->getColumnList()[j].getItemExpr()->getValueId();
          }
        }
      }
      // QSTUFF
  ...
  if (isView) {
    ...
    for (CollIndex i = 0; i < allCols.entries(); i++) {
      ...
      if (!selectCols.contains(idcol) && !nacol->getDefaultValue() && !nacol->isSystemColumn()) {
        isInsertable = FALSE;
        break;
      }
    }  // for allCols
...
NABoolean RelRoot::isUpdatableCursor()  // this is NOT const
{
  ...
  if (!updateCol().entries() || reqdOrder().entries()) {
    ...
    if (!upd.entries()) {
      const ColumnDescList &cols = *getScanNode()->getRETDesc()->getColumnList();
...
void RelRoot::transformNode(NormWA &normWARef, ExprGroupId &locationOfPointerToMe) {
  CMPASSERT(this == locationOfPointerToMe);
  ...
  if (getGroupAttr()->isEmbeddedUpdateOrDelete()) normWARef.setInEmbeddedUpdateOrDelete(TRUE);
  // QSTUFF
  ...
  if (getGroupAttr()->isEmbeddedInsert()) normWARef.setInEmbeddedInsert(TRUE);
  ...
  if ((isTrueRoot()) && (child(0)) &&
      ((child(0)->getOperatorType() == REL_SORT_LOGICAL) ||
       ((child(0)->getOperatorType() == REL_FIRST_N) &&
        ((child(0)->child(0)) && (child(0)->child(0)->getOperatorType() == REL_SORT_LOGICAL))))) {
    SortLogical *sl = NULL;
  ...
  if (compExpr().transformNode(newNormWA, child(0), getGroupAttr()->getCharacteristicInputs())) {
    // -----------------------------------------------------------------
    // Transform my new child.
    // -----------------------------------------------------------------
    child(0)->transformNode(newNormWA, child(0));
  ...
  // and constant values.
  // ---------------------------------------------------------------------
  if (inputVars().transformNode(newNormWA, child(0), getGroupAttr()->getCharacteristicInputs())) {
  ...
  // ---------------------------------------------------------------------
  if (reqdOrder().transformNode(newNormWA, child(0), getGroupAttr()->getCharacteristicInputs())) {
  ...
  normWARef.setContainsSemiJoinsToBeTransformed(newNormWA.containsSemiJoinsToBeTransformed());
  ...
  if (isTrueRoot()) {
    ...
    if (updatableSelect() == TRUE
      ...
      if (isUpdatableCursor()) {
        ...
        Scan *scan = NULL;
        if ((child(0)->castToRelExpr()->getOperatorType() == REL_FIRST_N) && (child(0)->child(0)))
          scan = (Scan *)child(0)->child(0)->castToRelExpr();
        else
          scan = (Scan *)child(0)->castToRelExpr();

        const ValueIdList *keyList = &(scan->getTableDesc()->getClusteringIndex()->getIndexKey());
        CollIndex i = 0;
        for (i = 0; i < keyList->entries(); i++) {
          ItemExpr *castNode = new (newNormWA.wHeap()) Cast((*keyList)[i].getItemExpr(), &((*keyList)[i].getType()));
        ...
        for (i = 0; i < nonKeyColList.entries(); i++) {
          ItemExpr *castNode =
              new (newNormWA.wHeap()) Cast(nonKeyColList[i].getItemExpr(), &(nonKeyColList[i].getType()));
...
void RelRoot::recomputeOuterReferences() {
  ...
  if (NOT getGroupAttr()->getCharacteristicInputs().isEmpty()) {
    ...
    child(0)->getGroupAttr()->getCharacteristicInputs().getLeafValuesForCoverTest(leafValues, emptyGA, emptySet);
    CMPASSERT(
        (getGroupAttr()->getCharacteristicInputs().contains(child(0)->getGroupAttr()->getCharacteristicInputs())) ||
...
void RelRoot::rewriteNode(NormWA &normWARef) {
  ...
  // ---------------------------------------------------------------------
  if ((updatableSelect() == TRUE) && (pkeyList().normalizeNode(normWARef))) {
  }
...
RelExpr *RelRoot::normalizeNode(NormWA &normWARef) {
  ...
  if (orderByList.entries() > 0) {
    ValueIdSet orderBySet(orderByList), coveredOrderBySet, inputsNeededForOrderBy, coveredOrderBySubExpr,
        uncoveredOrderByExpr;

    GroupAttributes *childGAPtr = child(0).getPtr()->getGroupAttr();

    childGAPtr->coverTest(orderBySet, myCharInput, coveredOrderBySet, inputsNeededForOrderBy, &coveredOrderBySubExpr);
  ...
  if (getFirstNRows() >= 0) {
    ...
    if ((safetyFactor != 0) && (reqdOrder().entries() > 0)) {
      ...
      if (firstOrderColVid.getItemExpr()->getOperatorType() == ITM_INVERSE) {
        firstOrderColVid = firstOrderColVid.getItemExpr()->child(0)->getValueId();
      }
      NABoolean isConstant = FALSE;
      NAColumn *nacol = NULL;
      ValueId baseVid = firstOrderColVid.getBaseColumn(&isConstant);
      if (baseVid != NULL_VALUE_ID) nacol = baseVid.getNAColumn(TRUE /*okIfNotColumn*/);
      if (nacol) {
        nacol->setReferencedForMultiIntHist();
        nacol->setNeedFullHistogram();
        normWARef.setCreatePredicateForFirstN(TRUE);
      }
    }
  }
  ...
  if (NOT normWARef.getExtraHubVertex()) normWARef.setExtraHubVertex(this);
...
RelExpr *RelRoot::semanticQueryOptimizeNode(NormWA &normWARef) {
  ...
  // for debugging
  if (normWARef.getCommonSubExprRefCount() > 0 && CmpCommon::getDefault(CSE_PRINT_DEBUG_INFO) == DF_ON)
    CommonSubExprRef::displayAll();

  // Delay the computation of the feasibleToPushDownAggr_ flag
  //
  // until this moment, since in TPCS Q41, the above transformation
  // does the following
  //
  // From
  //
  //       TSJ(SCAN(TPCDS_SF1000_ORC.ITEM),
  //           SCALAR_AGGR(SCAN(TPCDS_SF1000_ORC.ITEM))
  //
  //  into
  //
  //      GROUPBY(LEFT_JOIN(SCAN(TPCDS_SF1000_ORC.ITEM),
  //                        SCAN(TPCDS_SF1000_ORC.ITEM)
  //                       ))
  //
  // If we compute the flag at the end of the transformatoin stage,
  // we will incorrectly setup the flag for SCALAR_AGGR to TRUE
  // while in reality it should be set to FALSE due to the above
  // transformation.
  decideFeasibleToTransformForAggrPushdown(FALSE /*allow MAYBE answer*/);
...
RelExpr *RelRoot::inlineTempTablesForCSEs(NormWA &normWARef) {
  ...
  if (cses && cses->entries() > 0) {
    ...
    for (CollIndex i = 0; i < cses->entries(); i++)
      if (cses->at(i)->getInsertIntoTemp() != NULL) toDoVec += i;
    ...
    while (toDoVec.entries() > 0) {
      ...
      for (CollIndex r = 0; readyVec.nextUsed(r); r++) {
        ...
        if (thisLevelOfInserts == NULL)
          ...
          thisLevelOfInserts = CommonSubExprRef::makeUnion(thisLevelOfInserts, info->getInsertIntoTemp(), FALSE);
        }
      ...
      if (result == NULL)
        ...
        result = CommonSubExprRef::makeUnion(result, thisLevelOfInserts, TRUE);
  ...
  if (result) {
    const ValueIdSet &childOutputs(child(0).getGroupAttr()->getCharacteristicOutputs());
    ...
    Union *topUnion = CommonSubExprRef::makeUnion(result, child(0), TRUE);
    ...
    for (ValueId o = childOutputs.init(); childOutputs.next(o); childOutputs.advance(o)) {
      ItemExpr *leftFake =
          new (CmpCommon::statementHeap()) NATypeToItem(o.getType().newCopy(CmpCommon::statementHeap()));

      leftFake->synthTypeAndValueId();

      ValueIdUnion *vidUnion = new (CmpCommon::statementHeap())
          ValueIdUnion(leftFake->getValueId(), o, NULL_VALUE_ID, topUnion->getUnionFlags());
      vidUnion->synthTypeAndValueId();
      topUnion->addValueIdUnion(vidUnion->getValueId(), CmpCommon::statementHeap());
      outputValueList.insert(o);
      unionValueList.insert(vidUnion->getValueId());
      topUnion->getGroupAttr()->addCharacteristicOutput(vidUnion->getValueId());
    }
    ...
    result->getGroupAttr()->addCharacteristicInputs(topUnion->getGroupAttr()->getCharacteristicInputs());
...
RelExpr *Filter::normalizeNode(NormWA &normWARef) {
  ...
  if (selectionPred().getReferencedPredicates(outerReferences, nonLocalPreds)) {
    ...
    computeValuesReqdForPredicates(nonLocalPreds, valuesReqdByParent);
  }

  pushdownCoveredExpr(getGroupAttr()->getCharacteristicOutputs(), availableInputs, predsToPushDown,
                      &valuesReqdByParent);
...
NABoolean RelExpr::hasFilterChild() {
  if (getArity() == 1 && child(0)->getOperatorType() == REL_FILTER)
    return TRUE;
  else if (getArity() == 1 && child(0)->getArity() == 1 && child(0)->child(0)->getOperatorType() == REL_FILTER)
    return TRUE;
  else
    return FALSE;
}
...
void RelExpr::eliminateFilterChild() {
  if (child(0) && child(0)->getOperatorType() == REL_FILTER) {
    RelExpr *filterNode = child(0).getPtr();
    filterNode->pushdownCoveredExpr(filterNode->getGroupAttr()->getCharacteristicOutputs(),
                                    filterNode->getGroupAttr()->getCharacteristicInputs(), filterNode->selectionPred());
...
void RelExpr::recursivePushDownCoveredExpr(NormWA *normWAPtr, NABoolean doSynthLogProp) {
  ...
  if (getOperator().match(REL_ANY_JOIN)) {
    if ((NOT normWAPtr->getExtraHubVertex()) && !isExtraHub()) normWAPtr->setExtraHubVertex(this);
  }
...
NABoolean RelExpr::prepareTreeForCSESharing(const ValueIdSet &outputsToAdd, const ValueIdSet &predicatesToRemove,
                                            const ValueIdSet &newPredicatesToAdd, const ValueIdSet &inputsToRemove,
                                            ValueIdSet &valuesForVEGRewrite, ValueIdSet &keyColumns, CSEInfo *info) {
  NABoolean result = TRUE;
  ...
  for (CollIndex i = 0; i < nc && result; i++) {
    ...
    result = child(i)->prepareTreeForCSESharing(outputsToAdd, childPredsToRemove, childPredsToAdd, inputsToRemove,
                                                valuesForVEGRewrite, keyColumns, info);

    // if the child already had or has added any of the requested
    // outputs, then add them to our own char. outputs
    ValueIdSet childAddedOutputs(child(i).getGroupAttr()->getCharacteristicOutputs());
  ...
  // behavior).
  if (result)
    result = prepareMeForCSESharing(outputsToAdd, predicatesToRemove, newLocalPredicates, inputsToRemove,
                                    valuesForVEGRewrite, keyColumns, info);
...
//   in this node
NABoolean RelExpr::prepareMeForCSESharing(const ValueIdSet &outputsToAdd, const ValueIdSet &predicatesToRemove,
                                          const ValueIdSet &newPredicatesToAdd, const ValueIdSet &inputsToRemove,
                                          ValueIdSet &valuesForVEGRewrite, ValueIdSet &keyColumns, CSEInfo *info) {
  ...
  snprintf(buf, sizeof(buf), "Operator %s not supported", getText().data());
...
NABoolean RelExpr::createPredForFirstN(NormWA &normWARef) {
  ...
  ValueId firstOrderColVid;
  NABoolean isInverse = FALSE;
  if (getOperatorType() == REL_ROOT)  // FirstN with orderBy (should only happen with rowsets now)
  {
    RelRoot *rr = (RelRoot *)this;
    if (rr->reqdOrder().entries() == 0) return FALSE;
    firstOrderColVid = rr->reqdOrder()[0];
    if (firstOrderColVid.getItemExpr()->getOperatorType() == ITM_INVERSE) {
      firstOrderColVid = firstOrderColVid.getItemExpr()->child(0)->getValueId();
      isInverse = TRUE;
    }
  } else {
    FirstN *fn = (FirstN *)this;
    if (fn->reqdOrder().entries() > 0)  // FirstN with orderBy (non-rowsets case)
    {
      firstOrderColVid = fn->reqdOrder()[0];
      if (firstOrderColVid.getItemExpr()->getOperatorType() == ITM_INVERSE) {
        firstOrderColVid = firstOrderColVid.getItemExpr()->child(0)->getValueId();
        isInverse = TRUE;
      }
    } else  // FirstN without orderBy
    {
      getGroupAttr()->getCharacteristicOutputs().getFirst(firstOrderColVid);
      if (firstOrderColVid == NULL_VALUE_ID) return FALSE;
    }
  }
  if (firstOrderColVid.getItemExpr()->getOperatorType() == ITM_NOTCOVERED) {
    NotCovered *ncov = (NotCovered *)firstOrderColVid.getItemExpr();
    firstOrderColVid = ncov->getColumnRef()->getValueId();
  }

  QueryAnalysis *qa = QueryAnalysis::Instance();
  if (qa && qa->isCompressedHistsViable()) qa->disableCompressedHistsViable();

  EstLogPropSharedPtr childEstLogProp = child(0)->getGroupAttr()->outputLogProp((*GLOBAL_EMPTY_INPUT_LOGPROP));

  ColStatsSharedPtr firstOrderColStats = childEstLogProp->getColStats().getColStatsPtrForColumn(firstOrderColVid);
  if (!firstOrderColStats || firstOrderColStats->isFakeHistogram()) return FALSE;
  ...
  Interval iter;
  if (!isInverse)
    iter = firstOrderColStats->getHistogram()->getFirstInterval();
  else
    iter = firstOrderColStats->getHistogram()->getLastInterval();

  while (iter.isValid())  // we break out when we hit the last one
  {
    if (iter.isNull())  // we do not generate isNull predicates for now
      return FALSE;     // generating an pred will cause NULL to be filtered out
                        // For desc case OR ISNULL maybe required.
    rows += iter.getRowcount();
    if (rows > firstNValue * absSafetyFactor) {
      if (!isInverse)
        max = iter.hiBound().getDblValue();
      else
        max = iter.loBound().getDblValue();
      found = TRUE;
      break;
    }
    if (!isInverse)
  ...
  NAString *vStr = firstOrderColVid.getType().convertToString(max, STMTHEAP);
  if (!vStr || (vStr->length() == 0) || strchr(vStr->data(), '@')) return FALSE;

  ItemExpr *maxVal;
  maxVal = NATable::getRangePartitionBoundaryValues(vStr->data(), vStr->length(), STMTHEAP);
  if (!maxVal) return FALSE;
  ItemExpr *firstNPred;
  if (!isInverse)
    firstNPred = new (STMTHEAP) BiRelat(ITM_LESS_EQ, firstOrderColVid.getItemExpr(), maxVal);
  else
    firstNPred = new (STMTHEAP) BiRelat(ITM_GREATER_EQ, firstOrderColVid.getItemExpr(), maxVal);

  BindWA bindWA(ActiveSchemaDB(), CmpCommon::context());
  firstNPred->bindNode(&bindWA);
  if (bindWA.errStatus()) {
    CmpCommon::diags()->clear();
    bindWA.resetErrStatus();
    return FALSE;
  ...
  if (safetyFactor < 0) {
    ...
    firstNPred->unparse(unparsed, DEFAULT_PHASE, EXPLAIN_FORMAT);
    *CmpCommon::diags() << DgSqlCode(2997)
                        << DgString0("Consider adding this predicate to the query to improve performance.")
                        << DgString1(unparsed);
    return FALSE;
  }
  // if TopN sort is not enbled we cannot retry
  if (CmpCommon::getDefault(GEN_SORT_TOPN) != DF_ON) return FALSE;

  child(0)->selectionPred() += firstNPred->getValueId();
  normWARef.setRequiresRecursivePushdown(TRUE);
  if (getOperatorType() == REL_ROOT)  // FirstN with orderBy and rowsets
    ((RelRoot *)this)->setFirstNPredUsed(TRUE);
  else
    ((FirstN *)this)->setFirstNPredUsed(TRUE);
...
void Join::processCompRefOptConstraints(NormWA *normWAPtr) {
  if (CmpCommon::getDefault(ELIMINATE_REDUNDANT_JOINS) != DF_OFF) {
    ...
    if (normWAPtr && isInnerNonSemiJoin()) matchRIConstraint(leftGA, rightGA, normWAPtr);

    // Full Outer Join has a join pred that affect the rows that flow from the left
    if (NOT isFullOuterJoin()) myGA.addSuitableCompRefOptConstraints(leftConstraints, getSelectionPredicates(), this);
    // only non semi inner join rely solely on selection pred to control rows from
    // thr right. Other joins use a join pred also.
    if (isInnerNonSemiJoin()) myGA.addSuitableCompRefOptConstraints(rightConstraints, getSelectionPredicates(), this);
  }
}

void GroupByAgg::processCompRefOptConstraints(NormWA *normWAPtr) {
  if (CmpCommon::getDefault(ELIMINATE_REDUNDANT_JOINS) != DF_OFF) {
    getGroupAttr()->addSuitableCompRefOptConstraints(child(0).getGroupAttr()->getConstraints(),
...
void Filter::processCompRefOptConstraints(NormWA *normWAPtr) {
  if (CmpCommon::getDefault(ELIMINATE_REDUNDANT_JOINS) != DF_OFF) {
    GroupAttributes &myGA = *getGroupAttr();
    myGA.addSuitableCompRefOptConstraints(child(0).getGroupAttr()->getConstraints(), getSelectionPredicates(), this);
  }
}
...
// ***********************************************************************
void Rename::transformNode(NormWA &normWARef, ExprGroupId &locationOfPointerToMe) {
  CMPASSERT(this == locationOfPointerToMe);
  ...
  if (descendant->getOperatorType() == REL_ROOT) descendant = descendant->child(0);  // grandchild
...
//////////////////////////////////////////////////////////////////////////////
void BeforeTrigger::transformNode(NormWA &normWARef, ExprGroupId &locationOfPointerToMe) {
  ...
  if (parentTSJ_ != NULL)  // Is this the top most BeforeTrigger node?
  {
    ...
    const ValueIdSet &generatedValues = locationOfPointerToMe->getGroupAttr()->getCharacteristicOutputs();

    // The inputs of the TSJ node are the values needed from above:
    // transition variables and the executeId value.
    const ValueIdSet &externalInputs = tsjNode->getGroupAttr()->getCharacteristicInputs();
...
void RelRoutine::transformNode(NormWA &normWARef, ExprGroupId &locationOfPointerToMe) {
  if (nodeIsTransformed()) return;
...
void RelRoutine::rewriteNode(NormWA &normWARef) {
  ...
  if (child(0) != NULL) child(0)->rewriteNode(normWARef);
...
// ***********************************************************************
void Tuple::transformNode(NormWA &normWARef, ExprGroupId &locationOfPointerToMe) {
  CMPASSERT(this == locationOfPointerToMe);
  ...
  // tupleExpr() list
  tupleExpr().removeSubqueryOrIsolatedUDFunctionPredicates(subqueryOrIsolatedUDFunctionPredicates);
...
RelExpr *Tuple::normalizeNode(NormWA &normWARef) {
  ...
  if (rejectPredicates() && !selectionPred().isEmpty()) selectionPred().clear();
...
// ***********************************************************************
void TupleList::transformNode(NormWA &normWARef, ExprGroupId &locationOfPointerToMe) {
...
// -----------------------------------------------------------------------
void TupleList::rewriteNode(NormWA &normWARef) { Tuple::rewriteNode(normWARef); }  // TupleList::rewriteNode()
...
// transUnionVals().
//
void Transpose::transformNode(NormWA &normWARef, ExprGroupId &locationOfPointerToMe) {
  CMPASSERT(this == locationOfPointerToMe);
  ...
  for (CollIndex v = 0; v < transUnionVectorSize(); v++) {
    ...
    for (CollIndex i = 0; i < valIdList.entries(); i++) {
      ...
      for (CollIndex j = 0; j < numEntries; j++) {
        ...
        iePtr->transformNode(normWARef, nePtr, child(0), getGroupAttr()->getCharacteristicInputs());
...
// a subquery in it.
// -----------------------------------------------------------------------
void Pack::transformNode(NormWA &normWARef, ExprGroupId &locationOfPointerToMe) {
  CMPASSERT(this == locationOfPointerToMe);
  ...
  // child.
  // ---------------------------------------------------------------------
  if (packingExpr_.transformNode(normWARef, child(0), getGroupAttr()->getCharacteristicInputs())) {
    // -------------------------------------------------------------------
    // Transform my new child.
    // -------------------------------------------------------------------
    child(0)->transformNode(normWARef, child(0));
...
void CommonSubExprRef::transformNode(NormWA &normWARef, ExprGroupId &locationOfPointerToMe) {
  CMPASSERT(locationOfPointerToMe.getPtr() == this);
  ...
  if (isAnExpansionOf_) lexicalRefNumFromParent_ = isAnExpansionOf_->lexicalRefNumFromParent_;
...
void CommonSubExprRef::pushdownCoveredExpr(const ValueIdSet &outputExpr, const ValueIdSet &newExternalInputs,
  ...
  RelExpr::pushdownCoveredExpr(outputExpr, newExternalInputs, predicatesOnParent, setOfValuesReqdByParent, childIndex);
...
RelExpr *CommonSubExprRef::semanticQueryOptimizeNode(NormWA &normWARef) {
  ...
  if (result == NULL) emitCSEDiagnostics("Error in creating temp table or temp table insert", TRUE);

  return result;
}

NABoolean CommonSubExprRef::prepareMeForCSESharing(const ValueIdSet &outputsToAdd, const ValueIdSet &predicatesToRemove,
                                                   const ValueIdSet &commonPredicatesToAdd,
                                                   const ValueIdSet &inputsToRemove, ValueIdSet &valuesForVEGRewrite,
                                                   ValueIdSet &keyColumns, CSEInfo *info) {
...
CSEInfo::CSEAnalysisOutcome CommonSubExprRef::analyzeAndPrepareForSharing(CSEInfo &info) {
  ...
  if (info.getIdOfAnalyzingConsumer() >= 0) return info.getAnalysisOutcome(id_);
  ...
  ValueIdSet *nonCommonPredicatesArray = new (CmpCommon::statementHeap()) ValueIdSet[numConsumers];
  ValueIdMap *myColsToConsumerMaps = new (CmpCommon::statementHeap()) ValueIdMap[numConsumers];
  ...
  for (CollIndex c = 0; c < numConsumers && canShare; c++) {
    ...
    ValueIdSet requiredValues(consumer->getGroupAttr()->getCharacteristicOutputs());
    ...
    while (!ancestorIsTemped && infoToCheck->getNumLexicalRefs() == 1 && childToCheck &&
           childToCheck->parentRefId_ >= 0) {
      // look at the ancestor and what it is planning to do
      infoToCheck = CmpCommon::statement()->getCSEInfoById(childToCheck->parentCSEId_);
      CMPASSERT(infoToCheck);
      CommonSubExprRef *parent = infoToCheck->getConsumer(childToCheck->parentRefId_);
      CSEInfo::CSEAnalysisOutcome parentOutcome = infoToCheck->getAnalysisOutcome(parent->getId());

      if (parentOutcome == CSEInfo::CREATE_TEMP || parentOutcome == CSEInfo::TEMP) ancestorIsTemped = TRUE;

      childToCheck = parent;
    }

    if (!ancestorIsTemped) singleLexicalRefWithTempedAncestors = FALSE;

    requiredValues += cPreds;
    availableValues += consumer->getGroupAttr()->getCharacteristicInputs();
    ...
    if (requiredValues.removeUnCoveredExprs(availableValues)) {
      emitCSEDiagnostics("Characteristic outputs not covered by common subexpression");
      canShare = FALSE;
    }
    ...
    for (CollIndex i = 0; i < cCols.entries(); i++)
      if (requiredValues.referencesTheGivenValue(cCols[i], dummy, TRUE, TRUE)) neededColumnsBitmap += i;
    ...
    if (!cPreds.isEmpty())
      if (consumer->id_ == id_) {
        ...
        myColsToConsumerMaps[c].augmentForVEG(TRUE,   // add VEGPreds for existing VEGRefs
                                              ...
                                              &vegRefsWithDifferingConsts, &vegRefsWithDifferingInputs);
        ...
        if (vegRefsWithDifferingConsts.entries() > 0) {
          ...
          emitCSEDiagnostics("Encountered VEGs with different constants in different consumers");
          canShare = FALSE;
        }
        ...
        if (consumer->commonInputs_ != commonInputs_) {
          emitCSEDiagnostics("Differing inputs in CTE references, try CQD QUERY_CACHE '0'");
          canShare = FALSE;
        }
  ...
  if (singleLexicalRefWithTempedAncestors) {
    ...
    // it
    emitCSEDiagnostics("expression is only evaluated once because parent is materialized");
    canShare = FALSE;
  }
  ...
  // child tree
  makeValueIdListFromBitVector(tempTableColumns, columnList_, neededColumnsBitmap);
  ...
  for (CollIndex n = 0; n < numConsumers && canShare; n++) {
    ...
    commonPredicates.findCommonSubexpressions(nonCommonPredicatesArray[n], TRUE);

    if (nonCommonPredicatesArray[n].entries() > 0) {
      if (numORedPreds == n) {
        // build the ORed predicate
        ItemExpr *uncommonPreds = nonCommonPredicatesArray[n].rebuildExprTree();

        if (nonCommonPredicatesORed)
          nonCommonPredicatesORed =
              new (CmpCommon::statementHeap()) BiLogic(ITM_OR, nonCommonPredicatesORed, uncommonPreds);
        else
      ...
      // correct translation)
      myColsToConsumerMaps[n].rewriteValueIdSetDown(nonCommonPredicatesArray[n],
  ...
  if (canShare) {
    ...
    canShare = copyOfChildTree->prepareTreeForCSESharing(outputsToAdd, predicatesToRemove, newPredicatesToAdd,
                                                         inputsToRemove, nonVEGColumns_, childTreeKeyColumns, &info);

    if (!canShare)
      emitCSEDiagnostics("Failed to prepare child tree for materialization");
    else if (!copyOfChildTree->getGroupAttr()->getCharacteristicOutputs().contains(outputsToAdd)) {
      ...
      cseEstLogProps_ = copyOfChildTree->getGroupAttr()->outputLogProp((*GLOBAL_EMPTY_INPUT_LOGPROP));
      ...
      for (CollIndex k = 0; k < tempTableColumns.entries(); k++)
        if (childTreeKeyColumns.contains(tempTableColumns[k])) info.addCSEKeyColumn(k);
    }
  }

  if (canShare && CmpCommon::getDefault(CSE_USE_TEMP) != DF_ON) {
    ...
    if (maxTableSize > 0 && cseTempTableSize > maxTableSize) {
      ...
      snprintf(buf, sizeof(buf), "Temp table size %e exceeds limit %e", cseTempTableSize.getValue(), maxTableSize);
      emitCSEDiagnostics(buf);
      canShare = FALSE;
    } else if (maxTableSizeBasedOnMaxCard > 0 && cseTempTableMaxSize > maxTableSizeBasedOnMaxCard) {
      char buf[200];

      snprintf(buf, sizeof(buf), "Temp table size %e (based on max card) exceeds limit %e",
               cseTempTableMaxSize.getValue(), maxTableSizeBasedOnMaxCard);
      emitCSEDiagnostics(buf);
      canShare = FALSE;
    }
    ...
    for (CollIndex ncp = 0; ncp < numConsumers; ncp++) {
      ...
      tempRefCols.accumulateReferencedValues(childTreeKeyColumns, nonCommonPreds);
    ...
    if (totalKeyColPreds > (numConsumers * childTreeKeyColumns.entries() *
                            ActiveSchemaDB()->getDefaults().getAsDouble(CSE_PCT_KEY_COL_PRED_CONTROL) / 100.0)) {
      char buf[200];

      snprintf(buf, sizeof(buf), "Number of potential key predicates in consumers (%d) exceeds limit %f",
               totalKeyColPreds,
    ...
    if (commonKeyCols.entries() > ActiveSchemaDB()->getDefaults().getAsLong(CSE_COMMON_KEY_PRED_CONTROL)) {
      char buf[200];
      snprintf(buf, sizeof(buf), "All consumers have a predicate on %d common key columns, limit is %d",
               commonKeyCols.entries(), ActiveSchemaDB()->getDefaults().getAsLong(CSE_COMMON_KEY_PRED_CONTROL));
      emitCSEDiagnostics(buf);
      canShare = FALSE;
    }
  }
...
void CommonSubExprRef::determineTempTableType(CSEInfo &info) {
  NABoolean createHiveTable = (CmpCommon::getDefault(CSE_HIVE_TEMP_TABLE) == DF_ON);
...
NABoolean CommonSubExprRef::createTempTable(CSEInfo &info) {
  ...
  ValueIdList cols;
  NAString cseNamePrefix(internalName_.data(), MINOF(internalName_.length(), 16));
  ...
  if (cseNamePrefix.length() > maxCSENameLen) cseNamePrefix.remove(maxCSENameLen);
  ...
  for (int p = 0; p < cseNamePrefix.length(); p++) {
    ...
    if (!(c >= '0' && c <= '9' || c >= 'A' && c <= 'Z' || c == '_')) cseNamePrefix.replace(p, 1, "_");
  }

  tempTableName += cseNamePrefix;
  if (tempTableType == CSEInfo::HIVE_TEMP_TABLE) {
    tempTableName += "_";
    tempTableName += CmpCommon::context()->sqlSession()->getSessionId();
  }
  snprintf(buf, sizeof(buf), "_S%u_%d", CmpCommon::context()->getStatementNum(), info.getCSEId());
  tempTableName += buf;
  ...
  info.setTempTableName(QualifiedName(tempTableName, tempTableSchema, tempTableCatalog));

  // Step 2: Create the DDL for the temp table
  // -----------------------------------------

  tempTableDDL += "CREATE ";
  if (tempTableType == CSEInfo::VOLATILE_TEMP_TABLE) tempTableDDL += "VOLATILE ";
  tempTableDDL += "TABLE ";
  if (tempTableType == CSEInfo::HIVE_TEMP_TABLE && tempTableSchema == HIVE_SYSTEM_SCHEMA ||
    ...
    // (not yet supported)
    tempTableDDL += info.getTempTableName().getQualifiedNameAsAnsiString();
  }
  ...
  for (CollIndex c = 0; c < cols.entries(); c++) {
    ...
    if (tempTableType == CSEInfo::HIVE_TEMP_TABLE)
      cols[c].getType().genHiveTypeStrFromMyType(&colType);
    else
    ...
    if (colType == "unknown") {
      ...
      colType = "";
      cols[c].getType().getMyTypeAsText(&colType);
      snprintf(buf, sizeof(buf), "Unsupported data type for Hive temp table: %s", colType.data());
  ...
  if (result) info.setTempTableDDL(tempTableDDL);
  ...
  if (result) {
    ...
    NATable *tempNATable = ActiveSchemaDB()->getNATableDB()->get(cn, &bindWA, NULL, FALSE);
...
RelExpr *CommonSubExprRef::createInsertIntoTemp(CSEInfo &info, NormWA &normWARef) {
  ...
  TableDesc *tableDesc = bindWA.createTableDesc(info.getTempNATable(), cn, FALSE);
  ValueIdList srcValueList;

  { emitCSEDiagnostics("Unsupported temp table type in createInsertIntoTemp()", TRUE); }
...
RelExpr *CommonSubExprRef::createTempScan(CSEInfo &info, NormWA &normWARef)  //
{
  ...
  if (!info.getInsertIntoTemp()) return NULL;

  MapValueIds *result = NULL;
  BindWA bindWA(ActiveSchemaDB(), CmpCommon::context());
  CorrName cn(info.getTempTableName(), CmpCommon::statementHeap(), internalName_);
  TableDesc *tableDesc = bindWA.createTableDesc(info.getTempNATable(), cn, FALSE, getHint());

  Scan *scan = new (CmpCommon::statementHeap()) Scan(cn, tableDesc);
  ...
  result = new (CmpCommon::statementHeap()) MapValueIds(scan, outToTempMap, CmpCommon::statementHeap());

  result->setCSERef(this);
  result->addValuesForVEGRewrite(nonVEGColumns_);
  outToTempMap.rewriteValueIdSetDown(getGroupAttr()->getCharacteristicOutputs(), tempTableOutputs);
  ...
  scan->getGroupAttr()->setCharacteristicInputs(getGroupAttr()->getCharacteristicInputs());
...
void CommonSubExprRef::emitCSEDiagnostics(const char *message, NABoolean forceError) {
  ...
  if (CmpCommon::getDefault(CSE_DEBUG_WARNINGS) == DF_ON || forceError) {
    *CmpCommon::diags() << DgSqlCode(5001) << DgString0(internalName_.data()) << DgString1(message);
    if (forceError)
...
// -----------------------------------------------------------------------
void IsolatedNonTableUDR::transformNode(NormWA &normWARef, ExprGroupId &locationOfPointerToMe) {
  CMPASSERT(this == locationOfPointerToMe);
...
RelExpr *FirstN::normalizeNode(NormWA &normWARef) {
  ...
  if ((safetyFactor != 0) && (getFirstNRows() >= 0)) {
    ValueId firstVid;
    NAColumn *nacol = NULL;
    getGroupAttr()->getCharacteristicOutputs().getFirst(firstVid);
    if (firstVid != NULL_VALUE_ID) {
      NABoolean isConstant = FALSE;
      ValueId baseVid = firstVid.getBaseColumn(&isConstant);
      if (baseVid != NULL_VALUE_ID) nacol = baseVid.getNAColumn(TRUE /*okIfNotColumn*/);
      if (nacol) {
        nacol->setReferencedForMultiIntHist();
        nacol->setNeedFullHistogram();
...
// -----------------------------------------------------------------------
// FirstN::semanticQueryOptimizeNode()
// -----------------------------------------------------------------------
RelExpr *FirstN::semanticQueryOptimizeNode(NormWA &normWARef) {
  createPredForFirstN(normWARef);
  return RelExpr::semanticQueryOptimizeNode(normWARef);
}
...
//***********************************
CqsWA::CqsWA()
    : tableCANodeList_(new (CmpCommon::statementHeap()) TableCANodeIdPairLookupList(CmpCommon::statementHeap())),
      cqsCANodeIdMap_(new (CmpCommon::statementHeap()) CQSRelExprCANodeIdMap(30, CmpCommon::statementHeap())),
...

{
  ...
  if (nqtExpr->getArity() == 0) {
    if (nqtExpr->getOperatorType() == REL_SCAN) {
      ...
      TableCANodeIdPair *tableIdPair = new (CmpCommon::statementHeap()) TableCANodeIdPair();
      tableIdPair->Id_ = scan->getGroupAttr()->getGroupAnalysis()->getNodeAnalysis()->getId();
...
CANodeIdSet CQSRelExprCANodeIdMap::gatherNodeIdSetsForCQSTree(RelExpr *cqsExpr, CqsWA *cwa) {
  ...
  if ((arity == 0) && (cqsExpr->getOperatorType() == REL_FORCE_ANY_SCAN)) {
    ...
    if (cwa->isIndexJoin(cqsExpr)) {
      ...
      relExprNodeId->rightChildSet_ = gatherNodeIdSetsForCQSTree(cqsExpr->child(1), cwa);

    } else {
      relExprNodeId->leftChildSet_ = gatherNodeIdSetsForCQSTree(cqsExpr->child(0), cwa);
    ...
    return relExprNodeId->leftChildSet_ + relExprNodeId->rightChildSet_;
...
CANodeId CqsWA::findCANodeId(const NAString &tableName) {
  ...
  for (CollIndex i = 0; i < tcpairList->entries(); i++) {
    ...
    if ((tcpair->tabId_->getCorrNameObj().getQualifiedNameAsString() == tableNameAppend) ||
        (tcpair->tabId_->getCorrNameObj().getCorrNameAsString() == tableNameAppend))

    {
      if (tcpair->visited_) AssertException("", __FILE__, __LINE__).throwException();
...
//*****************************************************************
CANodeId CQSRelExprCANodeIdPair::populateReturnCANodeId(RelExpr *scan, CqsWA *cwa) {
  ScanForceWildCard *forcedScan = (ScanForceWildCard *)scan;

  NAString tableName(forcedScan->getExposedName(), CmpCommon::statementHeap());
...
//*********************************************************************
RelExpr *GroupByAgg::generateMatchingExpr(CANodeIdSet &lChildSet, CANodeIdSet &rChildSet, RelExpr *relExpr) {
  if (relExpr->getOperator().match(REL_ANY_GROUP)) {
    return CURRSTMT_CQSWA->checkAndProcessGroupByJBBC(relExpr, lChildSet, rChildSet, this);
  } else {
...
// child expression
//************************************************************
RelExpr *RelExpr::generateMatchingExpr(CANodeIdSet &lChildSet, CANodeIdSet &rChildSet, RelExpr *relExpr) {
  CANodeIdSet leftTableSet, rightTableSet;

  // throw an exception if the arity is not one
  if (getArity() != 1) AssertException("", __FILE__, __LINE__).throwException();

  RelExpr *wcChild = child(0);
  CURRSTMT_CQSWA->getTableSets(wcChild, leftTableSet, rightTableSet);

  return (child(0)->generateMatchingExpr(leftTableSet, rightTableSet, relExpr));
...
//
//*****************************************************************
RelExpr *JoinForceWildCard::generateMatchingExpr(CANodeIdSet &lChildSet, CANodeIdSet &rChildSet, RelExpr *relExpr) {
  Join *j = NULL;
  // check if the argument relExpr is a join; give an error if not ?

  if (relExpr->getOperator().match(REL_ANY_JOIN) || relExpr->getOperatorType() == REL_MULTI_JOIN) {
    j = (Join *)relExpr->generateLogicalExpr(lChildSet, rChildSet);
  } else {
    // index join?
    if ((relExpr->getOperator().match(REL_SCAN)) && (lChildSet.isEmpty() && rChildSet.entries() == 1)) return relExpr;
  ...
  if (j != NULL) {
    ...
    j->child(0) = wcLeftChild->generateMatchingExpr(leftTableSet, rightTableSet, lChild);

    CURRSTMT_CQSWA->getTableSets(wcRightChild, leftTableSet, rightTableSet);

    j->child(1) = wcRightChild->generateMatchingExpr(leftTableSet, rightTableSet, rChild);

    j->pushdownCoveredExpr(j->getGroupAttr()->getCharacteristicOutputs(), j->getGroupAttr()->getCharacteristicInputs(),
                           j->selectionPred());
...
// appropriately
//*******************************************************************
RelExpr *CqsWA::checkAndProcessGroupByJBBC(RelExpr *relExpr, CANodeIdSet &lChildSet, CANodeIdSet &rChildSet,
                                           RelExpr *cqsExpr) {
  if (!relExpr->getOperator().match(REL_ANY_GROUP)) {
    return relExpr;
  }
  ...
  NodeAnalysis *nodeAnalysis = relExpr->getGroupAttr()->getGroupAnalysis()->getNodeAnalysis();
  ...
  if (pGBAnalysis) {
    ...
    relExpr->child(0) = wcChild->generateMatchingExpr(lChildSet, rChildSet, childExpr);

    relExpr->primeGroupAttributes();
    relExpr->pushdownCoveredExpr(relExpr->getGroupAttr()->getCharacteristicOutputs(),
                                 relExpr->getGroupAttr()->getCharacteristicInputs(), relExpr->selectionPred());
...
// of its two children
//**************************************************************************
void CqsWA::getTableSets(RelExpr *cqsExpr, CANodeIdSet &leftSet, CANodeIdSet &rightSet) {
...
// it's child
//************************************************************************
RelExpr *ExchangeForceWildCard::generateMatchingExpr(CANodeIdSet &lChildSet, CANodeIdSet &rChildSet, RelExpr *relExpr) {
  // check the rChildSet is empty

  if (!rChildSet.isEmpty()) AssertException("", __FILE__, __LINE__).throwException();
...
//***********************************************************************
RelExpr *ScanForceWildCard::generateMatchingExpr(CANodeIdSet &lChildSet, CANodeIdSet &rChildSet, RelExpr *relExpr) {
  // check lChildSet and rChildSet are empty
  // check relExpr is a Scan
  if (relExpr->getOperator().match(REL_SCAN) && lChildSet.isEmpty() && rChildSet.isEmpty())
    return relExpr;
  else {
    AssertException("", __FILE__, __LINE__).throwException();
    return NULL;  // keep VisualC++ happy
...
RelExpr *RelExpr::generateLogicalExpr(CANodeIdSet &lChildSet, CANodeIdSet &rChildSet) {
  AssertException("", __FILE__, __LINE__).throwException();
  return NULL;  // keep VisualC++ happy
}
...
//**************************************************************
RelExpr *MultiJoin::generateLogicalExpr(CANodeIdSet &lChildSet, CANodeIdSet &rChildSet) {
  Join *j = splitByTables(lChildSet, rChildSet);

  // if the split is not possible, throw an exception
  if (j == NULL) AssertException("", __FILE__, __LINE__).throwException();
...
RelExpr *GroupByAgg::generateLogicalExpr(CANodeIdSet &lChildSet, CANodeIdSet &rChildSet) {
  AssertException("", __FILE__, __LINE__).throwException();
  return NULL;  // keep VisualC++ happy
}
...
NABoolean RelRoot::forceCQS(RelExpr *cqsExpr) {
  ...
  if (CmpCommon::getDefault(FORCE_BUSHY_CQS) != DF_ON) return FALSE;
  ...
  RelExpr *nqtCopyExpr = nqtExpr->child(0)->copyRelExprTree(CmpCommon::statementHeap());
  try {
    ...
    if (CqsWA::shouldContinue(nqtExpr, cqsExpr)) {
      ...
      while (!topJoin->getOperator().match(REL_ANY_JOIN) && !(topJoin->getOperatorType() == REL_MULTI_JOIN)) {
      ...
      RelExpr *childExpr = cqsExpr->generateMatchingExpr(leftTableSet, rightTableSet, topJoin);

      parentExpr->child(0) = childExpr;

      ValueIdList orderByList = reqdOrder();
      ValueIdSet valuesNeeded = parentExpr->getGroupAttr()->getCharacteristicOutputs();
      ...
      parentExpr->pushdownCoveredExpr(valuesNeeded, parentExpr->getGroupAttr()->getCharacteristicInputs(),
...
NABoolean CqsWA::shouldContinue(RelExpr *nqtExpr, RelExpr *cqsExpr) {
  ...
  if (CqsWA::containsNotSupportedOperator(nqtExpr)) return FALSE;

  // check if the CQS expression contains any cuts..
  if (CqsWA::containsCutOp(cqsExpr)) return FALSE;
  return TRUE;

}  // CqsWA::shouldContinue()

NABoolean CqsWA::containsNotSupportedOperator(RelExpr *nqtExpr) {
  if (nqtExpr->getOperatorType() == REL_COMPOUND_STMT || nqtExpr->getOperator().match(REL_ANY_GEN_UPDATE) ||
      nqtExpr->getOperatorType() == REL_UNION || nqtExpr->getOperatorType() == REL_DESCRIBE ||
      nqtExpr->getOperatorType() == REL_TUPLE_LIST || nqtExpr->getOperatorType() == REL_TUPLE ||
...
NABoolean CqsWA::isIndexJoin(RelExpr *cqsExpr) {
  ...
  if (cqsExpr->getOperator().match(REL_FORCE_JOIN) || cqsExpr->getOperator().match(REL_FORCE_NESTED_JOIN) ||
      cqsExpr->getOperator().match(REL_FORCE_HASH_JOIN) || cqsExpr->getOperator().match(REL_FORCE_MERGE_JOIN)) {
    JoinForceWildCard *jwc = (JoinForceWildCard *)cqsExpr;
    if (jwc->getPlan() == JoinForceWildCard::FORCED_INDEXJOIN) return TRUE;
...
NABoolean CqsWA::isMPTable(const NAString &tableName) {
  ...
  if (volumeLength < nameLength) {
    ...
    if (schemaLength < nameLength && volumeLength < schemaLength) {
      return TRUE;
    } else {
      AssertException("", __FILE__, __LINE__).throwException();
