...
//
// For debugging only
void TableOp::print(ostream &os) const {
  os << "Triggers on ";
  switch (operation_) {
    case COM_UPDATE:
      os << "Update";
  ...
  os << "of Table " << subjectTable_.getQualifiedNameAsAnsiString() << " : " << endl;
}
...
//
int TriggerDB::HashFunction(const TableOp &key) {
  int hval = key.subjectTable_.hash();
...
//
BeforeAndAfterTriggers *TriggerDB::getValidEntry(const TableOp *key, BindWA *bindWA) {
  BeforeAndAfterTriggers *result = NAHashDictionary<TableOp, BeforeAndAfterTriggers>::getFirstValue(key);
  ...
  if ((result != NULL) && (Trigger::Heap() == CmpCommon::contextHeap())) {
    ...
    ExtendedQualName exSubjectTable = ExtendedQualName(key->subjectTable_, CmpCommon::statementHeap());
    // fetch the subject table from the NATableDB
    NATable *subjectTable = bindWA->getSchemaDB()->getNATableDB()->get(&exSubjectTable);
...
//
BeforeAndAfterTriggers *TriggerDB::getTriggers(QualifiedName &subjectTable, ComOperation operation, BindWA *bindWA,
                                               bool allTrigger) {
...
BeforeAndAfterTriggers *TriggerDB::getTriggers(QualifiedName &subjectTable, ComOperation operation) {
...
void TriggerDB::putTriggers(QualifiedName &subjectTable, ComOperation operation, BeforeAndAfterTriggers *triggers) {
  TableOp *key = new (Trigger::Heap()) TableOp(subjectTable, operation);
  TriggerDB *triggerDB = GetCliGlobals()->currContext()->getTriggerDB();
...
void TriggerDB::removeTriggers(QualifiedName &subjectTable, ComOperation operation) {
  TableOp key(subjectTable, operation);
  TriggerDB *triggerDB = GetCliGlobals()->currContext()->getTriggerDB();
  if (triggerDB != NULL) {
...
BeforeAndAfterTriggers *TriggerDB::getAllTriggers(QualifiedName &subjectTable, long tableId, NABoolean useTrigger) {
  ...
  if (tableId == -1 || tableId == 0) {
    ...
    tableId =
        cmpSBD.getObjectUID(&cliInterface, catName, schemaName.data(), tableName.data(), COM_BASE_TABLE_OBJECT_LIT);
  ...
  memset(query, 0, 8192);
  str_sprintf(query,
              "select udr_uid, language_type, deterministic_bool, sql_access, max_results from %s.\"%s\".%s where "
              "udr_type='TR' and library_uid = %ld;",
  ...
  for (int i = 0; i < objectsInfo->numEntries(); i++) {
    ...
    memset(query, 0, 8192);
    str_sprintf(query,
                "select catalog_name, schema_name, object_name from %s.\"%s\".%s where object_uid = %ld and "
                "object_type = 'TR';",
                TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_OBJECTS, triggerid);
    flagBits = GetCliGlobals()->currContext()->getSqlParserFlags();
    GetCliGlobals()->currContext()->setSqlParserFlags(0x20000);
    ...
    trigger =
        new (Trigger::Heap()) Trigger(triggername, subjectTable, (ComOperation)op,
                                      ...
                                      triggerid, &text, CharInfo::ISO88591);
...
// Currently triggers are allocated from the statement heap.
// See method Trigger::Heap() in file Triggers.h for more details
void TriggerDB::clearAndDestroy() {
...
// Currently triggers are allocated from the statement heap.
// See method Trigger::Heap() in file Triggers.h for more details
static void ResetRecursionCounter(TriggerList *triggerList) {
...
// Currently triggers are allocated from the statement heap.
// See method Trigger::Heap() in file Triggers.h for more details
NABoolean TriggerDB::cleanupPerStatement() {
...
//
// not implemented
RefConstraintList *SchemaDB::getRIs(QualifiedName &subjectTable, ComOperation operation) { return NULL; }
