...
void HistInt::copy(const HistInt &other) {
...
void HistInt::setCardAndUec(CostScalar card, CostScalar uec) {
...
// ----------------------------------------------------------------------
CostScalar HistInt::mergeInterval(const HistInt &left, const HistInt &right, CostScalar scaleRowCount,
...

{
  ...
  switch (mergeMethod) {
    ...
    case LEFT_JOIN_OR_MERGE:
      ...
      if (leftUEC.isZero()) {
        ...
        numRows = leftRowCount + ((rightRowCount / numFudgedUec) * (numUec - leftUEC));
  ...
  if (numUec > numRows) numUec = numRows;

  this->setCardAndUec(numRows, numUec);
  return maxUEC;
}  // mergeInterval

void HistInt::display(FILE *f, const char *prefix, const char *suffix, CollHeap *c, char *buf) const {
  Space *space = (Space *)c;
  char mybuf[1000];
...
// -----------------------------------------------------------------------
NABoolean Interval::merge(Interval &other) {
...
// -----------------------------------------------------------------------
NABoolean Interval::canBeMerged() const {
  if (!OK()) return FALSE;
  ...
  if ((uec.getValue() < COSTSCALAR_EPSILON) && (row.getValue() < COSTSCALAR_EPSILON))
    return TRUE;
  else
    return FALSE;
...
NABoolean Interval::OK() const {
  ...
  if ((*hist_)[loIndex_].getBoundary() ==
      (*hist_)[loIndex_ + 1].getBoundary()) {  // isSingleValued()
...
CostScalar ColStats::getNullCount() const {
...
CostScalar ColStats::getNullUec() const {
  if (isNullInstantiated()) {
    Interval null = histogram_->getLastInterval();
    return null.getUec();
  } else {
...
void ColStats::setNullRowsAndUec(CostScalar nulls, CostScalar nullUec) {
  if (!isNullInstantiated()) {
...
void ColStats::setRowsAndUec(CostScalar rows, CostScalar uec, NABoolean allowMinusOne) {
  ...
  if (uec.isZero() && !rows.isZero()) uec = csOne;
...
//-----------------------------------------------------------------------
ColStatsSharedPtr ColStats::deepCopy(const ColStats &other, NAMemory *heap, NABoolean useColumnPositions,
...
ColStatsSharedPtr ColStats::deepCopySingleColHistFromCache(const ColStats &other, NAColumn &col, NAMemory *heap,
                                                           NABoolean copyIntervals) {
  CMPASSERT(other.columns_.entries() <= 1);
  ColStatsSharedPtr result = ColStats::deepCopy(other, heap, FALSE, copyIntervals);
...
//
// -----------------------------------------------------------------------
void ColStats::deleteIntervalsAbove(const Interval &boundary) {
...
// -----------------------------------------------------------------------
void ColStats::deleteIntervalsBelow(Interval &boundary) {
...
Interval Histogram::getNextInterval(const Interval &current) const {
...
Interval Histogram::getPrevInterval(const Interval &current) const {
...
// -----------------------------------------------------------------------
void Histogram::insertZeroInterval(const EncodedValue &loBound, const EncodedValue &hiBound,
...
void Histogram::insertZeroInterval(const CostScalar &loBound, const CostScalar &hiBound, NABoolean isNewBoundIncluded) {
  insertZeroInterval(EncodedValue(loBound.getValue()), EncodedValue(hiBound.getValue()), isNewBoundIncluded);
}

void Histogram::insertZeroInterval(const NormValueList &loBound, const NormValueList &hiBound,
                                   NABoolean isNewBoundIncluded) {
  insertZeroInterval(EncodedValue(loBound), EncodedValue(hiBound), isNewBoundIncluded);
}
...
// -----------------------------------------------------------------------
void Histogram::condenseToSingleInterval() {
  if (numIntervals() == 0) {
...
NABoolean Histogram::isNullInstantiated() const {
  if (numIntervals() == 0) {
    return FALSE;
  } else {
...
void Histogram::removeNullInterval() {
...
void Histogram::insertNullInterval() {
  if (isNullInstantiated()) {
...
// -----------------------------------------------------------------------
void Histogram::reduceNumHistInts(Criterion reductionCriterion, Source invokedFrom) {
  // if reduction criterion is none then return
  if (reductionCriterion == NONE) return;

  // interval object used to iterate of intervals
  Interval iter;
  // iterate over the intervals of this histogram
  for (iter = getFirstInterval(); iter.isValid() && !iter.isNull();
...
void Histogram::computeExtendedIntRange(Interval &currentInt, Criterion &reductionCriterion, EncodedValue &hiBound,
                                        EncodedValue &loBound, NABoolean &hiBoundInclusive,
                                        NABoolean &loBoundInclusive) {
  ...
  if (intervalExtended && hqc_ostream) {
    ...
    *hqc_ostream << "    Result Interval boundaries: " << endl
                 << "\t LOW:  [" << loBound.getDblValue() << "] with low bound" << (loBoundInclusive ? " " : " NOT ")
                 << "inclusive" << endl
                 << "\t HIGH: [" << hiBound.getDblValue() << "] with high bound" << (hiBoundInclusive ? " " : " NOT ")
                 << "inclusive" << endl;
  }
}
...
// -----------------------------------------------------------------------
void ColStats::divideHistogramAlongBoundaryValue(const EncodedValue &value, OperatorTypeEnum splitOperator) {
  ...
  if (histogram_->numIntervals() == 0) return;
  ...
  while (value > iter.hiBound() && !iter.isLast()) iter.next();
  ...
  Interval::distributeRowsAndUec(spanList, rows, uec, loBoundary, hiBoundary);
...
void ColStats::adjustRowcountforRollingColumns(ConstValue *constant) {
  ...
  if (encodedCurTime == UNINIT_ENCODEDVALUE) return;
  ...
  if ((timeEncompassedInNewInterval <= 0)) return;
...
// -----------------------------------------------------------------------
CollIndex Histogram::insertSingleValuedInterval(const EncodedValue &value, NABoolean distributeRowsAndUec) {
  // first, find the Interval that contains the
  // value for our soon-to-be-created SVI
  if (numIntervals() == 0) return NULL_COLL_INDEX;

  Interval iter = getFirstInterval();
  while (!iter.containsValue(value)) iter.next();

  if (!iter.containsValue(value)) return NULL_COLL_INDEX;  // something no good
...
// -----------------------------------------------------------------------
NABoolean Interval::spans(const Interval &other) const {
  ...
  if (hiBound.isNullValue() && loBound.isNullValue() && otherHiBound.isNullValue() && otherLoBound.isNullValue())
    return TRUE;
  ...
  //     |   |
  // other
  if (hiBound > otherHiBound && loBound < otherLoBound) return TRUE;  // this is always true
  ...
  //     |      |
  // other
  if (hiBound == otherHiBound && loBound < otherLoBound)
  ...
  // other
  if (hiBound == otherHiBound && loBound == otherLoBound)
    if (isOtherHiInclusive && !isHiInclusive || isOtherLoInclusive && !isLoInclusive)
      return FALSE;  // other is inclusive, I am not
    else
      return TRUE;
...
// -----------------------------------------------------------------------
void Interval::distributeRowsAndUec(LIST(Interval) & spanList, CostScalar rowsRemaining, CostScalar uecsRemaining,
                                    const EncodedValue &loBoundary, const EncodedValue &hiBoundary) {
  ...
  if (rowsRemaining.isZero() || uecsRemaining.isZero()) return;  // nothing to distribute!
  ...
  for (i = 0; i < spanListEntries; i++)
    if (spanList[i].isSingleValued()) singleCount++;
  ...
  if (singleCount > 0) {
    ...
    uecsPerSingle = MINOF(uecsRemaining / singleCount,                         // case where uecsRemaining < singleCount
                          1.0);                                                // "usual case" (we hope! :-)
    rowsPerSingle = MINOF(rowsRemaining / singleCount,                         // case where uecsRemaining < 1
  ...
  for (i = 0; i < spanListEntries; i++) {
    ...
    {       // to the size of the interval
      if (rowsRemaining.isZero() || uecsRemaining.isZero()) {
        ...
        factorHi = (CostScalar)spanList[i].hiBound().ratio(loBoundary, hiBoundary);
        factorLo = (CostScalar)spanList[i].loBound().ratio(loBoundary, hiBoundary);
...
NABoolean Interval::compare(Source invokedFrom, Criterion reductionCriterion, Interval &other) {
...
NABoolean Interval::satisfiesCriterion1(Source invokedFrom, Interval &other) {
  ...
  if ((myDistance < ourMin) || (myDistance > ourMax)) return FALSE;

  if ((neighborDistance < ourMin) || (neighborDistance > ourMax)) return FALSE;

  if ((myRowCount < ourMin) || (myRowCount > ourMax)) return FALSE;

  if ((neighborRowCount < ourMin) || (neighborRowCount > ourMax)) return FALSE;

  if ((myUEC < ourMin) || (myUEC > ourMax)) return FALSE;

  if ((neighborUEC < ourMin) || (neighborUEC > ourMax)) return FALSE;
  ...
  if (!(differenceSquared < adSquared)) return FALSE;
...
void Interval::display(FILE *f, const char *prefix, const char *suffix) const {
...
class HistIntVal {
 public:
  HistIntVal(const HistInt &init) : val_(init.getBoundary()), incl_(init.isBoundIncl()), hash_(init.getHash()) {}

  HistIntVal(const HistIntVal &other) : val_(other.val_), incl_(other.incl_), hash_(other.hash_) {}

  HistInt buildHistInt() { return HistInt(val_, incl_, hash_); }

  NABoolean operator==(const HistIntVal &rhs) const { return (val_ == rhs.val_ && incl_ == rhs.incl_); }

  NABoolean operator!=(const HistIntVal &rhs) const { return NOT(*this == rhs); }

  NABoolean operator<(const HistIntVal &rhs) const {
    if ((val_ < rhs.val_) || (val_ == rhs.val_ && incl_ == FALSE && rhs.incl_ == TRUE))
      return TRUE;
    else
      return FALSE;
  }
...
//  interest.
// -----------------------------------------------------------------------
HistogramSharedPtr Histogram::createMergeTemplate(const HistogramSharedPtr &otherHistogram, NABoolean equiMerge) const {
  ...
  if (this->entries() < 2 || otherHistogram->entries() < 2) {
    if (equiMerge)
      ...
      for (CollIndex i = 0; i < histTemplate->entries(); i++) (*histTemplate)[i].setCardAndUec(0, 0);
  ...
  while (1) {
    ...
    if (thisDone && otherDone) break;
  }

  NABoolean validHistTemp = TRUE;

  // sanity check
  if (histTemplate->entries() < 2 || histTemplate->entries() > thisEntries + otherEntries) {
    ...
    CCMPASSERT(histTemplate->entries() >= 2 && histTemplate->entries() <= thisEntries + otherEntries);
    validHistTemp = FALSE;
  }
  ...
  if (equiMerge) {
    ...
    if (validHistTemp) {
      ...
      while ((iT < histTemplate->entries()) && (overlapMax <= HistIntVal((*histTemplate)[iT])))
  ...
  if ((histTemplate->entries() == 1) || !validHistTemp) {
    // sanity check
...
// -----------------------------------------------------------------------
void ColStats::populateTemplate(const ColStatsSharedPtr &otherStats) {
  if (histogram_->numIntervals() == 0 || otherStats->getHistogram()->numIntervals() == 0) return;
  ...
  while (thisInterval.isValid()) {
    ...
    if (spanList.entries() > 0) {
      if (otherInterval.getUec().isGreaterThanZero() AND otherInterval.getRowcount().isGreaterThanZero()) {
        ...
        Interval::distributeRowsAndUec(spanList, iRows, iUec, otherInterval.loBound(), otherInterval.hiBound());
      }
    ...
    if (notIncrementedThisIter == TRUE) {
      ...
      if (!thisInterval.isLast() AND
           ...
           (otherInterval.hiBound() == thisInterval.hiBound() AND otherInterval.isHiBoundInclusive() ==
            TRUE AND thisInterval.isHiBoundInclusive() == FALSE))
        thisInterval.next();
  ...
  if (newRowcount < requiredMinimum && newRowcount.isGreaterThanZero()) {
    ...
    CostScalar calculatedUec =
        ColStatDesc::calculateCorrectResultUec(otherStats->getRowcount(), requiredMinimum, otherStats->getTotalUec());
...
NABoolean Histogram::condenseToPartitionBoundaries(const HistogramSharedPtr &partitionBoundaries) {
  ...
  while (this->entries() > 0 && HistIntVal((*this)[0]) < firstBoundary) this->removeAt(0);
  ...
  for (partIdx = 1; partIdx < partitionBoundaries->entries() && partIdx < this->entries(); partIdx++) {
  ...
  CCMPASSERT(this->entries() == partitionBoundaries->entries());
  CollIndex i;
  for (i = 0; i < this->entries(); i++) CCMPASSERT(HistIntVal((*this)[i]) == HistIntVal((*partitionBoundaries)[i]));
#endif
...
// ---------------------------------------------------------------------
void ColStats::insertZeroInterval() {
  if (histogram_ == NULL) histogram_ = HistogramSharedPtr(new (HISTHEAP) Histogram(HISTHEAP));
...
// --------------------------------------------------------------------
void ColStats::removeRedundantEmpties() {
  // if the NULL interval has zero rows, remove it
  if (getNullCount().isZero()) removeNullInterval();
  ...
  while (!iter.isLast()) {
    if (iter.getRowcount().isZero() AND iter.getLoIndex() == 0) {
    ...
    } else if (iter.getRowcount().isZero() AND next.getRowcount().isZero()) {
  ...
  iter = histogram_->getFirstInterval();
  iter.next();
  if (histogram_->numIntervals() == 2 && iter.isNull()) {
    histogram_->removeAt(0);
  }
  ...
  if (histogram_->numIntervals() > 0) {
    iter = histogram_->getFirstInterval();
    if (histogram_->numIntervals() == 1 && iter.getRowcount().isZero()) {
      clearHistogram();
    }
  }
...
void Histogram::display() const { Histogram::print(); }

void Histogram::print(FILE *f, const char *prefix, const char *suffix, CollHeap *c, char *buf) const {
  Space *space = (Space *)c;
  char mybuf[1000];
  snprintf(mybuf, sizeof(mybuf), "%sHistogram : %s\n", prefix, suffix);
  PRINTIT(f, c, space, buf, mybuf);
  if (entries() != 0) {
    for (CollIndex i = 0; i < entries(); i++) (*this)[i].display(f, "     ", "", c, buf);
  }
}
...
ComUID ColStats::nextFakeHistogramID() { return ComUID(++fakeHistogramIDCounter_); }
...
ColStats::ColStats(const ColStats &other, NAMemory *h, NABoolean assignColArray)
  ...
  if (assignColArray) columns_ = other.columns_;
...
void ColStats::populateColumnArray(const ColumnSet &cols, const NATable *table) {
  if (table) {
...
HistogramSharedPtr ColStats::getHistogramToModify() {
  if (NOT isModified()) {
    if (histogram_ != NULL) histogram_ = HistogramSharedPtr(new (heap_) Histogram(*histogram_, heap_));
...
void ColStats::createFakeHist() {
  ...
  setToSingleInterval(lowBound, highBound, getRowcount(), uec);
...
// --------------------------------------------------------------------
void ColStats::compressToSingleInt() {
...
// -----------------------------------------------------------------------
void ColStats::setRowsAndUecFromHistogram() {
  CostScalar newRowcount = 0;
  CostScalar newTotalUec = 0;

  for (Interval iter = histogram_->getFirstInterval(); iter.isValid();
...
// -----------------------------------------------------------------------
void ColStats::setMaxMinValuesFromHistogram() {
...
void ColStats::setStatColumn(NAColumn *column) {
...
void ColStats::overwrite(const ColStats &other) {
  HistogramSharedPtr otherCopy(new (other.heap_) Histogram(*(other.histogram_), other.heap_));
  FrequentValueList *otherFreqListCopy = new (STMTHEAP) FrequentValueList(other.getFrequentValues(), STMTHEAP);
...
// -----------------------------------------------------------------------
void ColStats::modifyStats(ItemExpr *pred, CostScalar *maxSelectivity) {
  ...
  if (pred->getArity() > 1) {
    ...
    if ((colType->getTypeQualifier() == NA_CHARACTER_TYPE) && ((CharType *)colType)->isCaseinsensitive() && constant &&
    ...
    if (constant == NULL) {
      if (rhs->getOperatorType() == ITM_VEG_REFERENCE) {
        ...
        if ((op == ITM_EQUAL) && (rhs->getOperatorType() == ITM_CACHE_PARAM)) {
          ItemExpr *constantExpr = ((ConstantParameter *)rhs)->getConstVal();

          if (constantExpr != NULL) constant = constantExpr->castToConstValue(negate);
        }  // cache_param
      }    // not aveg_reference
    }
...
void ColStats::simplestPreds(ItemExpr *pred) {
  // Begin Set-Up to perform the given Predicate........
  const ValueId predValueId = pred->getValueId();
  OperatorTypeEnum op = pred->getOperatorType();
...
void ColStats::populateTemplateOfFakeHist(const ColStatsSharedPtr &fakeHistogram,
  ...
  if (thisHist->numIntervals() == 0) return;
...
//     the RowCounts
// -----------------------------------------------------------------------
void ColStats::mergeColStats(const ColStatsSharedPtr &otherStats, MergeType mergeMethod, NABoolean isNumeric,
                             OperatorTypeEnum exprOpCode, NABoolean mergeFVs) {
  // look for the special case where histogram info is missing
  if (histogram_ == NULL || histogram_->entries() == 0 || otherStats->getHistogram() == NULL ||
  ...
  this->setMaxAndMinSetByPredFlags(otherStatsCopy, maxSetByPredFlag, minSetByPredFlag);
  ...
  if (((exprOpCode != REL_JOIN) && !useCompressedHistogramsForMerge) ||
      ((mergeMethod == SEMI_JOIN_MERGE) || (mergeMethod == ANTI_SEMI_JOIN_MERGE)))
    scaleFreq = FALSE;

  NABoolean isResultOrigAFakeHistogram = this->isOrigFakeHist() && otherStatsCopy->isOrigFakeHist();
  ...
  if ((CmpCommon::getDefault(HIST_INCLUDE_SKEW_FOR_NON_INNER_JOIN) == DF_ON) && !isResultOrigAFakeHistogram &&
      (exprOpCode == REL_JOIN) && (mergeMethod == INNER_JOIN_MERGE))
    adjRowCount = TRUE;

  if (mergeFVs || CmpCommon::getDefault(HIST_MERGE_FREQ_VALS_FIX) == DF_OFF)
  ...
  if ((CmpCommon::getDefault(COMP_BOOL_42) == DF_ON) && (qa && qa->isCompressedHistsViable()) &&
      ((exprOpCode == REL_JOIN) || useCompressedHistogramsForMerge) && (mergeMethod == INNER_JOIN_MERGE)) {
    // compute join cardinality using frequent values
    maxUecSum = this->mergeCompressedHistograms(otherStatsCopy, newRowCount, newUec, mergeMethod);
  } else {
    // do the actual join by merging histogram intervals
    maxUecSum = this->mergeWithExpandedHistograms(otherStatsCopy, isNumeric, newRowCount, newUec, mergeMethod);
    if (adjRowCount && isRCAdjusted && this->getFrequentValues().entries() > 0)
  ...
  if (isAJoinRelatedMerge(mergeMethod)) {
    ...
    if (mergeMethod == ANTI_SEMI_JOIN_MERGE) selAdj = csOne;
  ...
  NABoolean isResultAFakeHistogram =
      (this->isFakeHistogram() && otherStatsCopy->isFakeHistogram()) || isResultOrigAFakeHistogram;
  ...
  setSumOfMaxUec(MAXOF(
      sumOfMaxUec_, MAXOF(otherStatsCopy->sumOfMaxUec_, MAXOF(maxUecSum, MAXOF(baseUec_, otherStatsCopy->baseUec_)))));
...
void ColStats::setMaxAndMinSetByPredFlags(const ColStatsSharedPtr &otherStatsCopy, NABoolean &maxSetByPredFlag,
...
// ---------------------------------------------------------------------
void ColStats::recoverFromMergeColStats(const ColStatsSharedPtr &otherStats, NABoolean isNumeric,
                                        MergeType mergeMethod) {
  if (histogram_ == NULL) {
    CCMPASSERT(histogram_ != NULL);
    insertZeroInterval();
...
// ----------------------------------------------------------------------
CostScalar ColStats::mergeWithExpandedHistograms(const ColStatsSharedPtr &otherStats, NABoolean isNumeric,
                                                 CostScalar &newRowcount, CostScalar &newUec, MergeType mergeMethod) {
  // ------------------------------------------------------------------
  ...
  const NABoolean createTemplateWithEquimerge =
      (mergeMethod == UNION_MERGE || mergeMethod == OR_MERGE || mergeMethod == LEFT_JOIN_OR_MERGE ||
               mergeMethod == ANTI_SEMI_JOIN_MERGE
           ? FALSE
           : TRUE);

  HistogramSharedPtr leftHistogram =
      histogram_->createMergeTemplate(otherStats->getHistogram(), createTemplateWithEquimerge);
  ...
  isResultAFakeHistogram =
      this->populateLeftAndRightTemplates(otherStats, leftHistogram, rightHistogram, targetHistogram);
  ...
  while (i < targetHistogram->entries()) {
    maxUecSum +=
        (*targetHistogram)[i].mergeInterval((*leftHistogram)[i], (*rightHistogram)[i], scaleRowCount, mergeMethod);
...
// ----------------------------------------------------------------------
CostScalar ColStats::mergeCompressedHistograms(const ColStatsSharedPtr &otherStats, CostScalar &newRowcount,
                                               CostScalar &newUec, MergeType mergeMethod) {
  CostScalar maxUec = MAXOF(getSumOfMaxUec(), otherStats->getSumOfMaxUec());
  if (mergeMethod != INNER_JOIN_MERGE) return maxUec;

  if (!this->isCompressed()) this->compressToSingleInt();
  if (!otherStats->isCompressed()) otherStats->compressToSingleInt();
  ...
  if (maxAdjUC > 0) joinRCForContinuum = (adjRC1 * adjRC2) / maxAdjUC;
  ...
  if (iter.isValid()) iter.setRowsAndUec(newRowcount, newUec);
...
// ---------------------------------------------------------------------
CostScalar ColStats::adjustSelectivity(const ColStatsSharedPtr &otherStats, const CostScalar &newUec,
                                       MergeType mergeMethod) {
  // Make adjustments to the resulting UEC and rowcount if the UECs were
  // reduced due to independent predicates (preds not on this column)
  //
  ...
  //
  // Selectivity Adjustment (SA) =  SuperSet UEC based on correlated assumption / SuperSet UEC based on underlying
  // active assumption;
...
// children or not
// ------------------------------------------------------------------------
NABoolean ColStats::populateLeftAndRightTemplates(const ColStatsSharedPtr &otherStatsCopy,
                                                  HistogramSharedPtr &leftHistogram, HistogramSharedPtr &rightHistogram,
  ...
  } else if (otherOriginallyFake && !thisOriginallyFake) {
  ...
  if (leftHistogram->entries() != rightHistogram->entries() OR leftHistogram->entries() != targetHistogram->entries()) {
...
Criterion ColStats::decideReductionCriterion(Source invokedFrom, Criterion reductionCriterion, const NAColumn *column,
                                             NABoolean ignoreHistogramCachingFlag) {
  // cannot reduce multicolumn stats
  if (getStatColumns().entries() > 1) return NONE;

  // if invoked histograms for base tables
  if (invokedFrom == AFTER_FETCH) {
    // check if histogram caching is on
    if (CURRSTMT_OPTDEFAULTS->cacheHistograms() && (!ignoreHistogramCachingFlag)) {
...
void ColStats::reduceNumHistInts(Source invokedFrom, Criterion reductionCriterion) {
  ...
  histogram_->reduceNumHistInts(decideReductionCriterion(invokedFrom, reductionCriterion, column), invokedFrom);
}
...
void ColStats::reduceIntermediateHistInts(MergeType mergeMethod, NABoolean isNumeric) {
  if (CURRSTMT_OPTDEFAULTS->reduceIntermediateHistograms()) {
    if (isAJoinRelatedMerge(mergeMethod) || (mergeMethod == LEFT_JOIN_OR_MERGE)) {
      Criterion criterion;
      if (isNumeric)
...
// -----------------------------------------------------------------------
CostScalar ColStats::countFailedProbes(const ColStatsSharedPtr &otherStats) const {
  // look for the special case of missing/empty join Result.
  if (otherStats->getHistogram() == NULL OR otherStats->getHistogram()->entries() ==
      0 OR otherStats->getRowcount().isZero()) {
    return getRowcount();  // all probes failed.
  }

  // first create a template;  ( left = this; right = other )
  HistogramSharedPtr leftHistogram = histogram_->createMergeTemplate(otherStats->getHistogram(), FALSE);
  ...
  CostScalar totalFailedProbes = 0, failedProbesForInterval, leftUEC, leftRowCount, rightUEC, rightRowCount;
  CollIndex i = 1;
...
void ColStats::copyAndScaleHistogram(CostScalar scale) {
  if (getHistogram() == NULL) return;
...
void ColStats::scaleHistogram(CostScalar scale, CostScalar uecScale, NABoolean scaleFreqValList) {
  if (getHistogram() == NULL) return;
  ...
  if (scale != csOne) scaleFactor_ = scale;
  ...
  if ((scale == 1) && (uecScale == 1) && (rowRedFactor_ == 1) && (uecRedFactor_ == 1)) {
    return;
  } else {
    if (uecScale > csOne) {
    ...
    for (iter = hist->getFirstInterval(); iter.isValid(); iter.next())  // break when we've processed the last Interval
    {
  ...
  for (iter = hist->getFirstInterval(); iter.isValid() && !iter.isNull();  // do not merge NULL intervals!
    ...
    if (iter.canBeMerged()) {
      ...
      } else  // have to choose between neighbors to merge with
      {
        ...
        if (((loBound.getDblValue() - prevBound.getDblValue()) >= (nextBound.getDblValue() - hiBound.getDblValue())) &&
  ...
  if (newRowcount.isZero()) {
    if (hist->entries() > 1) hist->condenseToSingleInterval();
...
// --------------------------------------------------------------------
void ColStats::getAccRowCountAboveOrEqThreshold(CostScalar &accRowCnt, /* out */
...
void ColStats::setMaxFreq(CostScalar val) {
  if (CmpCommon::getDefault(COMP_BOOL_42) == DF_ON) {
...
CostScalar ColStats::getMaxFreq() const {
  if (CmpCommon::getDefault(COMP_BOOL_42) == DF_ON) {
    if (scaleFactor_ > csOne)
...
void ColStats::computeMaxFreqOfCol(NABoolean forced) {
  if ((forced == FALSE) && (getMaxFreq() > csZero)) return;
  ...
  if (freqList.entries() > 0) {
    ...
    if (maxFreqFromFreqList > maxFreq) maxFreq = maxFreqFromFreqList;
  } else {
    Interval iter = hist->getFirstInterval();
    ...
    while (iter.isValid() && !iter.isNull()) {
      ...
      if (iterUec == csZero) iterUec = csOne;

      CostScalar thisIterFreq = csZero;

      if (useHighFreq) thisIterFreq = iter.getRowcount2mfv();

      if (thisIterFreq == csZero) thisIterFreq = iter.getRowcount() / iterUec;

      if (maxFreq < thisIterFreq) maxFreq = thisIterFreq;
...
// -----------------------------------------------------------------------
void ColStats::reduceToMaxIntervalCount() {
  CollIndex maxIntervalCount = getMaxIntervalCount();
  if (histogram_->entries() == 0) return;
  ...
  if (intervalCount < 4 || maxIntervalCount < 4 || maxIntervalCount >= intervalCount) return;
  ...
  if ((additionalRows * 1.0) > (maxIntervalCount * 0.25)) reductionFactor++;
  ...
  for (iter = hist->getNextInterval(iter); iter.isValid() && !iter.isNull() && numKept < maxIntervalCount;
       /* no automatic increment */
  ) {
...
HistogramSharedPtr ColStats::transformOnIntervals(int numIntvs) {
  ...
  if (numIntvs > 1) {
    for (iter = hist->getFirstInterval(); iter.isValid() && !iter.isNull(); iter = hist->getNextInterval(iter)) {
      ...
      if (currentRcNew + rcInt < avgRcPerIntNew) {
        ...
        if (iter.getMFV(getFrequentValues(), mfv, freqMFV)) {
          ...
          if (r1 > 0.0) {
            ...
            availableRC = r1;
            iter.makeSplits(newHist, nt, avgRcPerIntNew, currentRcNew, availableRC, lowB, lowBInt, mfvInSC, TRUE);
          }

          if (freqMFV > 0.0) {
            // handle mfv
            availableRC = freqMFV;
            iter.makeSplits(newHist, nt, avgRcPerIntNew, currentRcNew, availableRC, lowB, mfvInSC, mfvInSC, FALSE);
          }

          CostScalar r2 = iter.getRowcount() - freqMFV - r1;
          r2.minCsZero();
          ...
          if (r2 > 0.0) {
            ...
            availableRC = r2;
            iter.makeSplits(newHist, nt, avgRcPerIntNew, currentRcNew, availableRC, lowB, mfvInSC, hiBInt, TRUE);
          }
          ...
          iter.makeSplits(newHist, nt, avgRcPerIntNew, currentRcNew, availableRC, lowB, lowBInt, hiBInt, TRUE);
        }
...
void Interval::makeSplits(HistogramSharedPtr &newHist, const NAType *nt, const CostScalar newHeight,
...
NABoolean Interval::getMFV(const FrequentValueList &list, EncodedValue &mfv, CostScalar &freq) {
  ...
  for (CollIndex index = 0; index < list.entries(); index++) {
    ...
    if (((isLoBoundInclusive() && lo <= mfv) || lo < mfv) && ((isHiBoundInclusive() && mfv <= hi) || mfv < hi)) {
      ...
      if (currentRcNew + rcInt < avgRcPerIntNew) {
        ...
        if (iter.getMFV(getMCSkewedValueList(), mfv, freqMFV)) {
          ...
          if (r1 > 0.0) {
            ...
            availableRC = r1;
            iter.makeSplitsForMC(newHist, avgRcPerIntNew, currentRcNew, availableRC, &lowbp, lowBInt, mfvInSC, TRUE);
          }

          if (freqMFV > 0.0) {
            // handle mfv
            availableRC = freqMFV;
            iter.makeSplitsForMC(newHist, avgRcPerIntNew, currentRcNew, availableRC, &lowbp, mfvInSC, mfvInSC, FALSE);
          }
          ...
          if (r2 > 0.0) {
            ...
            availableRC = r2;
            iter.makeSplitsForMC(newHist, avgRcPerIntNew, currentRcNew, availableRC, &lowbp, mfvInSC, hiBInt, TRUE);
          }
          ...
          iter.makeSplitsForMC(newHist, avgRcPerIntNew, currentRcNew, availableRC, &lowbp, lowBInt, hiBInt, TRUE);
        }
...
void Interval::makeSplitsForMC(HistogramSharedPtr &newHist, const CostScalar newHeight,
                               CostScalar &newRC,        // rc already filled;
                                                         // On exit, reset to 0 after a complete fill;
                                                         //          else, the partially filled RC
                               CostScalar &availableRC,  // on extry: rc available;
  ...
  if (availableRC < toFill) {
    ...
    *lowB = *hiBInt;
    ...
    if (allowSplits) {
      ...
      if (hiB.compare(hiBInt) == MORE) hiB = *hiBInt;  // and cap the value by hiBInt
  ...
  while (availableRC > newHeight) {
    ...
    if (split.compare(hiBInt) == MORE) split = *hiBInt;  // and cap the value by hiBInt
...
NABoolean Interval::getMFV(const MCSkewedValueList &list, MCboundaryValueList &mfv, CostScalar &freq) {
...
void Interval::getRCSmallerThanMFV(const MCboundaryValueList &mfv, const CostScalar &freqMFV, CostScalar &rc) {
...
// Guess the rowcount and uec of values smaller than mfv
void Interval::getRCSmallerThanMFV(const EncodedValue &mfv, const CostScalar &freqMFV, CostScalar &rc) {
...
// -----------------------------------------------------------------------
void ColStats::nullAugmentHistogram(CostScalar targetRowCount) {
...
void ColStats::makeGrouped() {
  ...
  for (iter = targetHistogram->getFirstInterval(); iter.isValid(); iter.next()) {
  ...
  if (targetHistogram->numIntervals() == 0) totalRowCount = MINOF(rowcount_ * rowRedFactor_, totalUec_ * uecRedFactor_);
...
// -----------------------------------------------------------------------
// To be called from the debugger
void ColStats::display() const { ColStats::print(); }

void ColStats::print(FILE *f, const char *prefix, const char *suffix, CollHeap *c, char *buf,
  ...
  snprintf(mybuf, sizeof(mybuf), "\n%s   RowRedFactor = %f;  UecRedFactor = %f %s\n", prefix, rowRedFactor_.value(),
           uecRedFactor_.value(), suffix);
  PRINTIT(f, c, space, buf, mybuf);
...
// -----------------------------------------------------------------------
void ColStats::mergeWithEmptyHistogram(const ColStatsSharedPtr &otherStats, MergeType mergeMethod) {
    ...
    case LEFT_JOIN_OR_MERGE:
      ...
      if (leftUEC.isZero()) {
        ...
        if (otherStats->getHistogram() == NULL)
          ...
          setHistogram(new (HISTHEAP) Histogram(*(otherStats->getHistogram()), HISTHEAP));
  ...
  if (isAJoinRelatedMerge(mergeMethod)) {
    ...
    if (totalUec_.isZero() && otherStats->totalUec_.isZero())
      ...
      {
        selAdj = ((baseUec_ / otherStats->baseUec_) * (otherStats->totalUec_ / MINOF(otherStats->totalUec_, totalUec_)))
                     .maxCsOne();
    ...
    } else  // baseUec_ >= otherStats->baseUec_
    {
      ...
      {
        selAdj = ((otherStats->baseUec_ / baseUec_) * (totalUec_ / MINOF(otherStats->totalUec_, totalUec_))).maxCsOne();
        selAdj *= ((numUec / MINOF(otherStats->totalUec_, totalUec_))).maxCsOne();
      }
    }
...
NABoolean ColStats::handleMergeTemplateWithZeroIntervals(const ColStatsSharedPtr &otherStats,
                                                         HistogramSharedPtr &leftHistogram) {
...
// -----------------------------------------------------------------------
void ColStats::newLowerBound(const EncodedValue &newLoBound, ConstValue *constExpr, NABoolean boundIncluded) {
  getHistogramToModify();
  ...
  //
  if (histogram_->numIntervals() == 0 OR getRowcount().isZero() OR getTotalUec().isZero()) {
    clearHistogram();
    return;
  }
  ...
  if (newLoBound > last.hiBound() ||
      ...
      // logical expression
      (newLoBound == last.hiBound() && (last.isHiBoundInclusive() == FALSE || boundIncluded == FALSE))) {
...
// -----------------------------------------------------------------------
void ColStats::newUpperBound(const EncodedValue &newUpBound, ConstValue *constExpr, NABoolean boundIncluded) {
  getHistogramToModify();
  ...
  //
  if (histogram_->numIntervals() == 0 || getRowcount().isZero() || getTotalUec().isZero()) {
  ...
  if (newUpBound < first.loBound() ||
      ...
      // logical expression
      (newUpBound == first.loBound() && (first.isLoBoundInclusive() == FALSE || boundIncluded == FALSE))) {
    // in all cases, the result is now fake
    setFakeHistogram(TRUE);
...
// -----------------------------------------------------------------------
void ColStats::setToSingleInterval(const EncodedValue &newLoBound, const EncodedValue &newUpBound, CostScalar numRows,
                                   CostScalar numUecs) {
  // want to be careful to keep track of the shape-changed flag
  Interval first = histogram_->getFirstInterval();

  if (first.isValid() && histogram_->numIntervals() == 1 && first.loBound() == newLoBound &&
      first.hiBound() == newUpBound && first.getRowcount() == numRows && first.getUec() == numUecs) {
...
void ColStats::adjustMaxSelectivity(const EncodedValue &normValue, ConstValue *constExpr, CostScalar *totalRows,
                                    CostScalar *maxSelectivity) {
  if (totalRows == NULL || *totalRows <= csZero || isVirtualColForHist() || histogram_->numIntervals() == 0 ||
  ...
  if (normValue < datatypeMinValue || normValue > datatypeMaxValue) return;

  if (normValue < first.loBound() || (normValue == first.loBound() && !first.isLoBoundInclusive())) return;

  if (normValue > last.hiBound() || (normValue == last.hiBound() && !last.isHiBoundInclusive())) return;
  ...
  if (hist->numIntervals() == 0) return;

  Interval iter = hist->getFirstInterval();
  while (!iter.containsValue(normValue)) iter.next();

  if (!iter.containsValue(normValue)) return;  // something no good
  ...
  if (useHighFreq && iter.getRowcount2mfv() > csZero)
    *maxSelectivity = MINOF(iter.getRowcount2mfv() / (*totalRows), *maxSelectivity);
  else {
...
// -----------------------------------------------------------------------
void ColStats::setToSingleValue(const EncodedValue &newValue, ConstValue *constExpr, CostScalar *totalRows,
                                FrequentValue *fv) {
  getHistogramToModify();
  ...
  //
  if (newValue < first.loBound() || (newValue == first.loBound() && !first.isLoBoundInclusive())) {
    ...
    if ((!isOrigFakeHist())) frequentValues_.clear();

    return;
  }

  if (newValue > last.hiBound() || (newValue == last.hiBound() && !last.isHiBoundInclusive())) {
  ...
  //
  FrequentValueList &frequentValueList = getModifableFrequentValues();
  NABoolean useMFVs = (((frequentValueList.entries() > 0) && CURRSTMT_OPTDEFAULTS->useHighFreqInfo()) ? TRUE : FALSE);
  ...
  if (useMFVs) distributeRowsAndUec = getTotalFreqInfoForIntervalWithValue(newValue, totalMfvRc, mfvCnt);
  ...
  // trailing spaces
  if ((CmpCommon::getDefault(HIST_REMOVE_TRAILING_BLANKS) == DF_ON) && constExpr &&
    ...
    if (typ->getCharSet() == CharInfo::UNICODE) {
      ...
      tempConstExpr =
          new (HISTHEAP) ConstValue(constString, typ->getCharSet(), typ->getCollation(), typ->getCoercibility());
    } else {
  ...
  if (useMFVs) {
    if ((frequentValueList.entries() > 0) &&
        (frequentValueList.getfrequentValueIndex(
             (fv) ? (*fv) : FrequentValue(newValue, constExpr, columns_[0]->getType()), index) == TRUE)) {
      ...
      setRowsAndUec(iterRC * rowRedFactor_, (iterRC.isGreaterThanZero() ? csOne : csZero) * uecRedFactor_);
    }
  } else {
    setRowsAndUec(theSVI.getRowcount() * rowRedFactor_, theSVI.getUec() * uecRedFactor_);
  }
...
// -----------------------------------------------------------------------
void ColStats::removeSingleValue(const EncodedValue &newValue, ConstValue *constExpr) {
  getHistogramToModify();
  ...
  //
  if ((newValue < first.loBound() || newValue > last.hiBound()) ||
  ...
  // set the s-c flag
  if (histogram_->entries() != entriesBefore || rowsRemoved.isGreaterThanZero() || uecsRemoved.isGreaterThanZero())
    setShapeChanged(TRUE);
...
void ColStats::clearHistogram() {
  if (histogram_->entries() > 0 OR getRowcount()
                                  .isGreaterThanZero() OR  // insurance: maybe some function (?!) which
                                      getTotalUec()
                                  .isGreaterThanZero())  // removed HistInts forgot to set this flag
    setShapeChanged(TRUE);
  setObsoleteHistogram(FALSE);
  ...
  setMinValue(UNINIT_ENCODEDVALUE);
  setMaxValue(UNINIT_ENCODEDVALUE);
  setToSingleInterval(UNINIT_ENCODEDVALUE, UNINIT_ENCODEDVALUE, 0, 0);  // avoid empty histograms!
  frequentValues_.clear();
  setIsCompressed(TRUE);
}
...
// -----------------------------------------------------------------------
void ColStats::isNull(NABoolean notFlag) {
  getHistogramToModify();

  //
  // if there aren't any Intervals, we're done
  //
  if (histogram_->entries() == 0 || getRowcount().isZero() || getTotalUec().isZero()) {
    clearHistogram();
    return;
  }
  ...
  if (notFlag == FALSE) {
    if (getNullCount().isZero())  // not any NULLs, we're probably done
    {
      ...
      // this sets all the flags except fake hist
      setToSingleInterval(NULL_ENCODEDVALUE, NULL_ENCODEDVALUE, getNullCount(), getNullUec());
...
void StatsList::reduceNumHistInts(Source invokedFrom, Criterion reductionCriterion) {
  ...
  for (UInt32 idx = 0; idx < entries(); idx++) {
    if ((*this)[idx]) (*this)[idx]->reduceNumHistInts(invokedFrom, reductionCriterion);
  }
}
...
void StatsList::reduceNumHistIntsAfterFetch(NATable &table) {
  NABoolean hbasePartitioning = table.isHbaseTable() && (CmpCommon::getDefault(HBASE_STATS_PARTITIONING) != DF_OFF);
  ...
  for (UInt32 idx = 0; idx < entries(); idx++) {
    ...
    if ((colStats) && (colStats->statColumns().entries() == 1) && (!colStats->isCompressed()) &&
        !colStats->isSingleIntHist()) {
      ...
      if (column) {
        ...
        if (hbasePartitioning && isAKeyColumn && colPos == leadingKeyColPos) continue;

        // check if this column requires full histograms
        NABoolean requiresHist = column->isReferencedForHistogram();
        NABoolean requiresFullHist = column->needFullHistogram();

        if (requiresHist && !requiresFullHist) {
          if (CURRSTMT_OPTDEFAULTS->reduceBaseHistograms()) {
            ...
            switch (colStats->decideReductionCriterion(AFTER_FETCH, CRITERION1, column, TRUE)) {
              case CRITERION1:
...
void StatsList::insertByPosition(const StatsList &other, const int position, SET(ColStats *) & dupList) {
  for (UInt32 i = 0; i < other.entries(); i++) {
    ...
    if (!otherColumns.getColumnByPos(position)) continue;
...
CostScalar StatsList::getSingleColumnUECCount(const int position) const {
  ...
  for (UInt32 i = 0; i < entries(); i++) {
    ...
    if (((*this)[i]->getStatColumns().entries() == 1) && ((*this)[i]->getStatColumns().getColumnByPos(position))) {
...
void StatsList::display() const { StatsList::print(); }

void StatsList::print(FILE *f, const char *prefix, const char *suffix, CollHeap *c, char *buf) const {
  Space *space = (Space *)c;
  char mybuf[1000];
  ...
  if (entries() != 0) {
    ...
    for (CollIndex i = 0; i < entries(); i++) {
      ...
      snprintf(mybuf, sizeof(mybuf), "%s   TotalUEC = %f \n", prefix, iter->getTotalUec().value());
      PRINTIT(f, c, space, buf, mybuf);

      sprintf(mybuf, "%s   Rowcount = %f \n", prefix, iter->getRowcount().value());
      PRINTIT(f, c, space, buf, mybuf);
      ...
      snprintf(mybuf, sizeof(mybuf), "\n%s     RowRedFactor = %f;  UecRedFactor = %f %s\n", prefix,
               iter->getRedFactor().value(), iter->getUecRedFactor().value(), suffix);
      PRINTIT(f, c, space, buf, mybuf);
...
NABoolean StatsList::allFakeStats() const {
  ...
  for (UInt32 i = 0; i < entries() AND allFake; i++) {
    if (!((*this)[i])->isFakeHistogram()) allFake = FALSE;
  }
  return allFake;
}
...
int StatsList::getSingleColumnCount() const {
  ...
  for (UInt32 i = 0; i < entries(); i++) {
    ...
    if (naca.entries() == 1) {
      const NAColumn *nac = naca[0];
      if (nac->getPosition() >= 0) count++;
    }
  }
  return count;
}
...
int StatsList::getExpressionsCount() const {
  ...
  for (UInt32 i = 0; i < entries(); i++) {
    ...
    if (naca.entries() == 1) {
      const NAColumn *nac = naca[0];
      if (nac->getPosition() < 0)  // negative position indicates expressions histogram
        count++;
    }
  }
  return count;
...
ColumnSet::ColumnSet(const NAColumnArray &colArray, NAMemory *heap) : ClusteredBitmap(heap) {
  for (CollIndex c = 0; c < colArray.entries(); c++) {
    addElement(colArray[c]->getPosition());
  }
}

void ColumnSet::display() const { ColumnSet::print(); }
...
void ColumnSet::printColsFromTable(FILE *ofd, NATable *table) const {
  ...
  CollIndex x;
  int i = 0, colCount = entries();
  if (!table) {
    for (x = init(); next(x); advance(x)) {
      fprintf(ofd, "%d", x);
      if ((++i < colCount) && (colCount > 1)) fprintf(ofd, ",");
    }
  } else {
    // declare a priority_queue and specify the order as <
    priority_queue<NAColumn, vector<NAColumn>, less<vector<NAColumn>::value_type> > pCols;
...
void MultiColumnHistogram::display() const { MultiColumnHistogram::print(); }
...
void MultiColumnHistogramList::addMultiColumnHistogram(const ColStats &mcStat, ColumnSet *singleColPositions) {
  if (mcStat.getStatColumns().entries() > 1) {
    ...
    if ((!singleColPositions) || (!(tempColumns.intersectSet(*singleColPositions).entries()))) {
      // get columns of this multi-column histogram
      ...
      MultiColumnHistogram *mcHistogram =
          new (heap_) MultiColumnHistogram(columns, uec, rows, id, mcSkewedValueList, mcStatsCopy, heap_);
...
void MultiColumnHistogramList::addMultiColumnHistograms(const StatsList &colStats)
...
void MultiColumnHistogramList::display() const { MultiColumnHistogramList::print(); }
...
// reduce the number of histogram intervals in the histogram
// referenced by this ColStats Object
void ColStats::compressColStatsForQueryPreds(ItemExpr *lowerBound, ItemExpr *upperBound, NABoolean hasJoinPred) {
  // if there is no histogram return
  if (!histogram_) return;
...
// ----------------------------------------------------------------------------
void Histogram::compressHistogramForQueryPreds(ItemExpr *lowerBound, ItemExpr *upperBound, NABoolean hasJoinPred) {
  // don't compress if less than 4 intervals
  if (numIntervals() < 4) return;
  ...
  if (lowerBound) {
    ...
    lowerBoundEncodedValue = new (CmpCommon::statementHeap()) EncodedValue(lowerBound, FALSE);
  } else {
    // a lower bound was not passed in

    // create an EncodedValue to represent the lower bound
    lowerBoundEncodedValue = new (CmpCommon::statementHeap()) EncodedValue(minEncodedValue);
  }

  // if a upper bound was passed in
  if (upperBound) {
    // create an EncodedValue to represent the upper bound
    upperBoundEncodedValue = new (CmpCommon::statementHeap()) EncodedValue(upperBound, FALSE);
  } else {
    // a upper bound was not passed in

    // create an EncodedValue to represent the upper bound
    upperBoundEncodedValue = new (CmpCommon::statementHeap()) EncodedValue(maxEncodedValue);
  }

  // if lowerBound is higher than upperBound
  // e.g. a > 3 and a < 2
  if ((*lowerBoundEncodedValue) > (*upperBoundEncodedValue)) compressToSingleInterval = TRUE;
  ...
  if ((state != 2) && (iter.containsValue(*lowerBoundEncodedValue))) {
  ...
  // iterate over the intervals of this histogram
  for (/* initialized above */; iter.isValid() && !iter.isNull();
       /* no automatic increment */) {
...
//         FALSE - otherwise
// -----------------------------------------------------------------------
NABoolean Histogram::computeSelectivityForEquality(ItemExpr *constVal, CostScalar totalRowcount, CostScalar totalUEC,
  ...
  if (constVal->getOperatorType() != ITM_CONSTANT) return FALSE;
...
const NAString SkewedValueList::getText() const {
  ...
  if (!needToComputeFinalHash()) {
    ...
    for (CollIndex i = 0; i < entries(); i++) result += ".";
  } else if (naType->useHashRepresentation() == FALSE) {
...
MCSkewedValue &MCSkewedValue::operator=(const MCSkewedValue &other) {
  if (this != &other) {
    ...
    hash_ = other.hash_;
    mcEncodedValue_ =
        new (heap_) EncodedValue(*(((MCSkewedValue &)other).mcEncodedValue_), ((MCSkewedValue &)other).heap_);
  }
  return *this;
}

void MCSkewedValue::print(FILE *f, const char *prefix, const char *suffix, CollHeap *c, char *buf) const {
  Space *space = (Space *)c;
  char mybuf[1000];
...
void MCSkewedValue::display() const { MCSkewedValue::print(); }

void MCSkewedValueList::print(FILE *f, const char *prefix, const char *suffix, CollHeap *c, char *buf) const {
  Space *space = (Space *)c;
  char mybuf[1000];
  ...
  for (CollIndex i = 0; i < entries(); i++) at(i)->print();
}

void MCSkewedValueList::display() const { MCSkewedValueList::print(); }

MCSkewedValueList::MCSkewedValueList(const MCSkewedValueList &mcsvl, NAMemory *h)
    : NAList<MCSkewedValue *>(h ? h : CmpCommon::statementHeap()), heap_(h ? h : CmpCommon::statementHeap()) {
...
MCSkewedValueList &MCSkewedValueList::operator=(const MCSkewedValueList &other) {
  if (this != &other) LIST(MCSkewedValue *)::operator=(other);
  return *this;
}
...
NABoolean MCSkewedValueList::operator==(const MCSkewedValueList &mcsvl) {
  if (entries() != mcsvl.entries())
    ...
    for (CollIndex i = 0; i < entries(); i++) {
      ...
      if (!(*thisMCSV == *otherMCSV)) return FALSE;
    }
  }
  return TRUE;
}

void MCSkewedValueList::mergeMCSkewedValueList(MCSkewedValueList *leftSide, MCSkewedValueList *rightSide,
  ...
  if (leftSide) leftSideEntries = leftSide->entries();
  CollIndex rightSideEntries = 0;
  if (rightSide) rightSideEntries = rightSide->entries();

  while (leftIndex < leftSideEntries || rightIndex < rightSideEntries) {
    if ((leftIndex < leftSideEntries) && (rightIndex < rightSideEntries)) {
    ...
    } else if ((leftIndex < leftSideEntries) && (rightIndex == rightSideEntries)) {
      if (mergeMethod == INNER_JOIN_MERGE || mergeMethod == OUTER_JOIN_MERGE) {
        MCSkewedValue *leftV = leftSide->at(leftIndex);
        newBound = (NAWchar *)leftV->getBoundary();
        newFreq = leftV->getFrequency() * avgRowcountForNonSkewValuesOnRightSide;
    ...
    if (newBound) {
      ...
      MCSkewedValue *newV = new (STMTHEAP) MCSkewedValue(newBound, newFreq, newEV, 0, STMTHEAP);
...
void MCSkewedValueList::addMCSkewedValue(MCSkewedValue *newValue) {
  addMCSkewedValue(newValue->getBoundary(), newValue->getFrequency(), *(newValue->getEncodedValue()),
                   newValue->getHash());
}

void MCSkewedValueList::addMCSkewedValue(const NAWchar *boundary, CostScalar frequency, const EncodedValue &eV,
                                         UInt32 hash) {
...
// to be called from the debugger
void FrequentValueList::display() const { FrequentValueList::print(); }

void FrequentValueList::print(FILE *f, const char *prefix, const char *suffix, CollHeap *c, char *buf) const {
  Space *space = (Space *)c;
  char mybuf[1000];
  snprintf(mybuf, sizeof(mybuf), "%sFrequent Values : %s\n", prefix, suffix);
  PRINTIT(f, c, space, buf, mybuf);
  if (entries() != 0) {
    for (CollIndex i = 0; i < entries(); i++) (*this)[i].print(f, "     ", "", c, buf);
  }
}

void FrequentValue::print(FILE *f, const char *prefix, const char *suffix, CollHeap *c, char *buf) const {
  Space *space = (Space *)c;
  char mybuf[1000];
...
FrequentValue::FrequentValue(UInt32 hashValue, CostScalar frequency, CostScalar probability, EncodedValue value) {
  hash_ = hashValue;
...
FrequentValue::FrequentValue(const EncodedValue &normValue, ConstValue *cv, const NAType *colType, CostScalar freq,
                             CostScalar prob)
    : hash_(0), frequency_(freq), probability_(prob), encodedValue_(normValue) {
  if (normValue.isNullValue())
    hash_ = 666654765;
  else {
    if (cv && colType->useHashRepresentation() && colType->useHashInFrequentValue()) {
      // const NAType* colType = columns[0]->getType();

      if ((colType->getTypeQualifier() == NA_CHARACTER_TYPE) && ((CharType *)colType)->isCaseinsensitive() &&
          (((CharType *)colType)->getCharSet() != CharInfo::UNICODE))
...
void ColStats::createAndAddSkewedValue(const wchar_t *boundary, Interval &iter) {
  HistogramSharedPtr hist = this->getHistogram();
  if ((hist == NULL) || (hist->numIntervals() == 0)) return;
...
void ColStats::createAndAddFrequentValue(const wchar_t *boundary, Interval &iter) {
  ...
  if (frequentValueList.isFull()) return;
  ...
  if (iter.isNull()) {
    ...
    EncodedValue boundaryEV;
    boundaryEV.setValueToNull();
...
NABoolean FrequentValueList::isFull() {
  const int maxSkewValues = CURRSTMT_OPTDEFAULTS->maxSkewValuesDetected();
...
NABoolean ColStats::mergeFrequentValues(ColStatsSharedPtr &otherStats, NABoolean scaleFreq, MergeType mergeMethod,
                                        NABoolean adjRowCount) {
  NABoolean isRCAdjusted = FALSE;
  ...
  if (CmpCommon::getDefault(COMP_BOOL_42) == DF_OFF) {
    ...
    if ((UF1 > 0) || (UF2 > 0)) {
      ...
      if (scaleFreq) scaleFactor = (getScaleFactor() * otherStats->getScaleFactor()).minCsOne();
      ...
      resultFreqValList->mergeFreqFreqValues(leftFrequentValueList, rightFrequentValueList, scaleFactor, mergeMethod,
                                             tmpLeftFreqValList, tmpRightFreqValList);
      ...
      if (UF1 > 0) adjProb1 = US2 / UF1;
      if (UF2 > 0) adjProb2 = US1 / UF2;
      ...
      if (UF1 > 0) {
        ...
        if ((RC2 > 0) && (UC2 > 0)) {
          ...
          if ((CmpCommon::getDefault(HIST_INCLUDE_SKEW_FOR_NON_INNER_JOIN) == DF_ON) && mergeMethod == OR_MERGE)
            resultFreqValList->scaleAndAppend(leftFrequentValueList, 1, 1, getScaleFactor());
          else
            resultFreqValList->scaleAndAppend(leftFrequentValueList, adjFreq1, adjProb1, getScaleFactor());
        }
      }
      ...
      if (UF2 > 0) {
        ...
        if ((UC1 > 0) && (RC1 > 0)) {
          ...
          // if OR_MERGE type, then simply add both sides frequent value lists
          if ((CmpCommon::getDefault(HIST_INCLUDE_SKEW_FOR_NON_INNER_JOIN) == DF_ON) && mergeMethod == OR_MERGE)
            resultFreqValList->scaleAndAppend(rightFrequentValueList, 1, 1, otherStats->getScaleFactor());
          else
            resultFreqValList->scaleAndAppend(rightFrequentValueList, adjFreq2, adjProb2, otherStats->getScaleFactor());
        }
      }
    ...
    if (tmpLeftFreqValList->entries() > 0 && tmpRightFreqValList->entries() > 0 && adjRowCount) {
      ...
      if ((value == leftMostFreqValue.getEncodedValue()) && (value == rightMostFreqValue.getEncodedValue())) {
        leftMaxFreq = leftMostFreqValue.getFrequency() * leftMostFreqValue.getProbability();

        HistogramSharedPtr hist = getHistogramToModify();
        Interval iter = hist->getFirstInterval();
        while (iter.isValid()) {
          if (iter.containsValue(value)) break;
          if (iter.isLast())
            break;
          else
            iter.next();
...
void FrequentValueList::mergeFreqFreqValues(FrequentValueList &leftFrequentValueList,
                                            FrequentValueList &rightFrequentValueList, CostScalar scaleFactor,
                                            MergeType mergeMethod, FrequentValueList *tmpLeftFreqValueList,
                                            FrequentValueList *tmpRightFreqValueList) {
  CollIndex leftIndex = 0;
  CollIndex rightIndex = 0;

  while (leftIndex < leftFrequentValueList.entries() && rightIndex < rightFrequentValueList.entries()) {
    ...
    if (leftV == rightV) {
      ...
      CostScalar probability = MINOF(leftV.getProbability(), rightV.getProbability());
...
void FrequentValueList::scaleAndAppend(FrequentValueList &sourceFrequentValueList, CostScalar adjFreq,
                                       CostScalar adjProb, CostScalar scaleFactor) {
...
NABoolean FrequentValueList::getfrequentValueIndex(const FrequentValue &key, CollIndex &index) const {
...
CostScalar FrequentValueList::getTotalFrequency() const {
...
CostScalar FrequentValueList::getMaxFrequency() const {
...
CostScalar FrequentValueList::getTotalProbability() const {
  CostScalar totalProbability = csZero;
  for (CollIndex index = 0; index < entries(); index++) totalProbability += (*this)[index].getProbability();

  return totalProbability;
}

void FrequentValueList::insertFrequentValue(const FrequentValue &key) {
  if ((key.getEncodedValue() == UNINIT_ENCODEDVALUE) || (key.getFrequency() <= csZero)) return;
...
void FrequentValueList::scaleFreqAndProbOfFrequentValues(CostScalar freqScale, CostScalar probScale) {
  if ((freqScale == 1) && (probScale == 1)) return;
  ...
  for (CollIndex j = 0; j < entries(); j++) {
    ...
    if (probScale < 1) newProb *= probScale.getValue();

    newProb = MINOF(newProb, newFreq);

    if (CmpCommon::getDefault(COMP_BOOL_42) == DF_ON) newFreq = MAXOF(newFreq, 1.0);
    thisFrequentValue.setFrequency(newFreq);
    thisFrequentValue.setProbability(newProb);
  }
}

void FrequentValueList::removeNULLAsFrequentValue() {
...
void FrequentValueList::deleteFrequentValuesAboveOrEqual(const EncodedValue &val, NABoolean include) {
  for (CollIndex j = 0; j < entries(); j++) {
...
void FrequentValueList::deleteFrequentValuesBelowOrEqual(const EncodedValue &val, NABoolean include) {
  for (CollIndex j = 0; j < entries(); j++) {
    EncodedValue value = (*this)[j].getEncodedValue();
    if (value < val) {
      this->removeAt(j);
...
void FrequentValueList::deleteAllButThisFreqVal(const FrequentValue &val) {
  for (CollIndex j = 0; j < entries(); j++) {
...
void FrequentValueList::deleteFrequentValue(const FrequentValue &val) {
  for (CollIndex j = 0; j < entries(); j++) {
    if ((*this)[j] == val) {
      this->removeAt(j);
      j--;
...
NABoolean ColStats::getTotalFreqInfoForIntervalWithValue(EncodedValue newValue, CostScalar &totalMfvRc,
                                                         CostScalar &mfvCnt) {
  totalMfvRc = csZero;
  mfvCnt = csZero;

  Interval iter = histogram_->getFirstInterval();
  while (!iter.containsValue(newValue)) iter.next();

  if (!iter.containsValue(newValue)) return TRUE;
...
CostScalar FrequentValueList::freqOfGivenEncodedVal(EncodedValue mfvEV, EncodedValue loBoundary,
                                                    EncodedValue hiBoundary, CostScalar &mfvCnt) const {
  CostScalar totalMfvRc = csZero;
  for (CollIndex i = 0; i < entries(); i++) {
...
FrequentValue FrequentValueList::getMostFreqValue() const {
  CostScalar maxFrequency = csZero;
...
FrequentValue FrequentValueList::getMostFreqValue(EncodedValue value) const {
  for (CollIndex index = 0; index < entries(); index++) {
    FrequentValue freqVal = (*this)[index];
...
void MCboundaryValueList::display() const { print(); }

void MCboundaryValueList::print(FILE *ofd, const char *indent, const char *title) const {
  char NEW_INDENT2[] = "   ";
