...
#ifndef NDEBUG
  BUMP_INDENT(indent);
  cout << title << " " << this << " NARoutine=" << (void *)routine_ << " actionNARoutine=" << (void *)action_
       << " invocation=" << invocationNum_ << " isUUDF_ =" << isUUDF_ << "actionNameAsGiven=" << actionNameAsGiven_
       << endl;
...
NABoolean RoutineDesc::createRoutineParams(BindWA *bindWA, RDParamHandling pHandling) {
  ...
  for (CollIndex i = 0; i < (CollIndex)routine->getParamCount(); i++) {
    if (createRoutineParam(bindWA, i, routine, inParamList, outputList) == FALSE) return FALSE;
  }
  return TRUE;
}
...
NABoolean RoutineDesc::createRoutineParam(BindWA *bindWA, CollIndex i, NARoutine *routine, ValueIdList *inParamList,
  ...
  RoutineParam *routineParam = new (bindWA->wHeap()) RoutineParam(params[i], i, this);

  if (routineParam == NULL) return FALSE;

  if ((params[i]->getColName() == "INVAL") || (params[i]->getColName() == "")) {
  ...
  if (routineParam->getParamMode() == COM_OUTPUT_COLUMN) {
    ...
    if (numUecs != 0 && (i <= numUecs)) {
      ...
      ColStatsSharedPtr outputStats(new (HISTHEAP) ColStats(id, routine->getUecValues()[i], fanOut, fanOut, FALSE,
                                                            FALSE, emptyHist, FALSE, 1.0, 1.0, 0, HISTHEAP,
                                                            TRUE  // allowMinusOne
                                                            ));
      ...
      ColStatDescSharedPtr tmpColStatDescPtr(
          new (HISTHEAP) ColStatDesc(outputStats, routineParam->getValueId(), HISTHEAP), HISTHEAP);
...
// the appropriate input/output list depending on the parameter type.
// -----------------------------------------------------------------------
NABoolean RoutineDesc::populateRoutineDesc(BindWA *bindWA, NABoolean createRETDesc) {
  ...
  if (createRETDesc == TRUE) {
    bindWA->getCurrentScope()->setRETDesc(new (bindWA->wHeap()) RETDesc(bindWA, this));
  }
...
void RoutineDesc::processRoutineCosting(SimpleCostVector *initial, SimpleCostVector *normal, int *fanOut,
  ...
  *fanOut = routine->getUdfFanOut();
  ...
  if (initial->getCPUTime() == csMinusOne) {
    CostScalar initCpuCost((double)(ActiveSchemaDB()->getDefaults()).getAsULong(INITIAL_UDF_CPU_COST));
    initial->setCPUTime(initCpuCost);
  }

  if (initial->getIOTime() == csMinusOne) {
    CostScalar initIOCost((double)(ActiveSchemaDB()->getDefaults()).getAsULong(INITIAL_UDF_IO_COST));
    initial->setIOTime(initIOCost);
  }

  if (initial->getMessageTime() == csMinusOne) {
    CostScalar initMsgCost((double)(ActiveSchemaDB()->getDefaults()).getAsULong(INITIAL_UDF_MSG_COST));
    initial->setMSGTime(initMsgCost);
  }

  if (normal->getCPUTime() == csMinusOne) {
    CostScalar initCpuCost((double)(ActiveSchemaDB()->getDefaults()).getAsULong(NORMAL_UDF_CPU_COST));
    normal->setCPUTime(initCpuCost);
  }

  if (normal->getIOTime() == csMinusOne) {
    CostScalar initIOCost((double)(ActiveSchemaDB()->getDefaults()).getAsULong(NORMAL_UDF_IO_COST));
    normal->setIOTime(initIOCost);
  }

  if (normal->getMessageTime() == csMinusOne) {
    CostScalar initMsgCost((double)(ActiveSchemaDB()->getDefaults()).getAsULong(NORMAL_UDF_MSG_COST));
