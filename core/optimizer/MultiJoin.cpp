...
MultiJoin::MultiJoin(const JBBSubset &jbbSubset, CollHeap *oHeap)
    : RelExpr(REL_MULTI_JOIN, NULL, NULL, oHeap), jbbSubset_(jbbSubset), childrenMap_(oHeap), scheduledLSRs_(oHeap) {
...
const NAString MultiJoin::getText() const { return NAString("multi_join"); }  // MultiJoin::getText()
...
NABoolean MultiJoin::duplicateMatch(const RelExpr &other) const {
  ...
  if (getArity() != other.getArity()) return FALSE;
  ...
  if (jbbSubset_ != o.jbbSubset_) return FALSE;
  ...
  if (scheduledLSRs_ != o.scheduledLSRs_) return FALSE;
...
Join *MultiJoin::splitSubset(const JBBSubset &leftSet, const JBBSubset &rightSet, NABoolean reUseMJ) const {
  // At this point assert that none of the subsets has a group by member
  CMPASSERT((jbbSubset_.getGB() == NULL_CA_ID) && (leftSet.getGB() == NULL_CA_ID) && (rightSet.getGB() == NULL_CA_ID));
#ifndef NDEBUG
  ...
  CMPASSERT((unionSet == jbbSubset_.getJBBCs()) && (intersectSet.entries() == 0));
#endif
  ...
  if (!leftSet.legal()) return NULL;

  if ((rightSet.getJBBCs().entries() > 1) && (!rightSet.legal())) return NULL;
  ...
  if (rightSet.getJBBCs().entries() == 1) {
    JBBC *rightChild = rightSet.getJBBCs().getFirst().getNodeAnalysis()->getJBBC();
    ...
    if (rightChildParentJoin) {
      if (rightChildParentJoin->derivedFromRoutineJoin()) derivedFromRoutineJoin = TRUE;

      if (rightChildParentJoin->isSemiJoin()) result = new (outHeap) Join(child0, child1, REL_SEMIJOIN, NULL);

      if (rightChildParentJoin->isAntiSemiJoin()) result = new (outHeap) Join(child0, child1, REL_ANTI_SEMIJOIN, NULL);
      ...
      if (rightChildParentJoin->isRoutineJoin()) {
        ...
        for (ValueId filterPred = routineJoinFilterPreds.init(); routineJoinFilterPreds.next(filterPred);
             routineJoinFilterPreds.advance(filterPred)) {
          if (jbbSubset_.coversExpr(filterPred)) predsToAddToRoutineJoin += filterPred;
        }
      ...
      if (result) {
        ...
        result->nullInstantiatedOutput().insert(rightChild->nullInstantiatedOutput());
      }
    }
  }

  // The join to be created is a regular InnerNonSemi join
  if (!result) result = new (outHeap) Join(child0, child1, REL_JOIN, NULL);

  // Make sure we carry the derivedFromRoutineJoin flag with us
  if (derivedFromRoutineJoin) result->setDerivedFromRoutineJoin();
  ...
  if (leftSet.getJBBCs().entries() == 1) result->setChild(0, getJBBCRelExpr(leftSet.getJBBCs().getFirst()));

  // We used CutOp as children, to avoid pushing predicates to JBBCs.
  // Now put the actual expression back in case the child is a JBBCs
  if (rightSet.getJBBCs().entries() == 1) result->setChild(1, getJBBCRelExpr(rightSet.getJBBCs().getFirst()));
  ...
  if (leftSet.getJBBCs().entries() > 1) result->child(0)->selectionPred().clear();
  if (rightSet.getJBBCs().entries() > 1) result->child(1)->selectionPred().clear();
...
MultiJoin *MultiJoin::createSubsetMultiJoin(const JBBSubset &subset, NABoolean reUseMJ) const {
  ...
  if (reUseMJ) result = subset.getSubsetMJ();
  ...
  result->getGroupAttr()->addCharacteristicInputs(getGroupAttr()->getCharacteristicInputs());
...
void MultiJoin::recomputeOuterReferences() {
  ...
  if (NOT getGroupAttr()->getCharacteristicInputs().isEmpty()) {
    ...
    // do not weed out the expressions for M17180
...
Join *MultiJoin::leftLinearize(NABoolean fixJoinOrder, NABoolean createPriviledgedJoins) const {
  ...
  NABoolean nonExpander =
      (left.jbbcsToJBBSubset()->isGuaranteedNonExpandingJoin((*jbbcRight.getNodeAnalysis()->getJBBC())));

  Join *result = splitSubset(*(left.jbbcsToJBBSubset()), *(right.jbbcsToJBBSubset()));
...
Join *MultiJoin::createLeftLinearJoinTree(const NAList<CANodeIdSet> *const leftDeepJoinSequence,
  ...
  for (UInt32 i = 0; i < (numJoinChildren - 1); i++) {
    ...
    if (CmpCommon::getDefault(COMP_BOOL_120) == DF_OFF) currentJoin->updatePotential(-3);

    // if this is the first iteration
    // set the result to be the top join
    if (i == 0) result = currentJoin;
    ...
    if (parentJoin) parentJoin->setChild(0, currentJoin);
  ...
  // synthesize_it
  if (result->child(1))
    if (result->child(1)->getOperatorType() == REL_MULTI_JOIN) result->child(1)->synthLogProp();
...
void MJJoinDirective::setupJoin(Join *join) {
  if (skipNestedJoin_) join->contextInsensRules() += JoinToTSJRuleNumber;

  if (skipMergeJoin_) join->contextInsensRules() += MergeJoinRuleNumber;

  if (skipHashJoin_) join->contextInsensRules() += HashJoinRuleNumber;

  if (skipJoinLeftShift_) join->contextInsensRules() += JoinLeftShiftRuleNumber;

  if (skipJoinCommutativity_) join->contextInsensRules() += JoinCommutativityRuleNumber;

  join->setSource(joinSource_);

  if (rightScheduledLSRs_.entries()) ((MultiJoin *)(join->getChild(1)))->scheduledLSRs() += rightScheduledLSRs_;

  if (leftScheduledLSRs_.entries()) ((MultiJoin *)(join->getChild(0)))->scheduledLSRs() += leftScheduledLSRs_;

  if (joinFromPTRule_) join->setJoinFromPTRule();
};
...
Join *MultiJoin::getPreferredJoin() {
  if (!getGroupAttr()->existsLogExprForSynthesis()) synthLogProp();
...
void MultiJoin::synthLogProp(NormWA *normWAPtr) {
  ...
  Join *join = splitSubset(*(left.jbbcsToJBBSubset()), *(right.jbbcsToJBBSubset()));
  ...
  CASortedList *synthLogPropPath =
      new (CmpCommon::statementHeap()) CASortedList(CmpCommon::statementHeap(), jbbcs.entries());
...
void MultiJoin::synthLogPropWithMJReuse(NormWA *normWAPtr) {
  ...
  if (getGroupAttr()->existsLogExprForSynthesis()) {
    Join *joinExprForSynth = (Join *)getGroupAttr()->getLogExprForSynthesis();

    if (joinExprForSynth->isJoinFromMJSynthLogProp()) return;
  }
  ...
  Join *join = splitSubset(*(left.jbbcsToJBBSubset()), *(right.jbbcsToJBBSubset()), reUseMJ);
  ...
  if (left.entries() > 1) {
    ...
    if (leftRelExpr && leftRelExpr->getOperator() == REL_MULTI_JOIN)
...
// -----------------------------------------------------------------------
Join *MultiJoin::splitByTables(const CANodeIdSet &leftTableSet, const CANodeIdSet &rightTableSet,
  ...
  CMPASSERT((unionSet == tables) && (intersectSet.entries() == 0));
#endif
