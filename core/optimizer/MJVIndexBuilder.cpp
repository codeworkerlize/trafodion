...
// equality operator
//---------------------------------------------------------------------------//
NABoolean ColIndSet::operator==(const ColIndSet &other) const {
  size_t size = entries();
...
//			FALSE - otherwise
//---------------------------------------------------------------------------//
NABoolean ColIndSet::isSubsetOf(const ColIndSet &other) const {
  size_t size = entries();
...
// virtual
#ifdef MJVIB_VERBOSE
void ColIndSetBucketVector::out() const {
...
//---------------------------------------------------------------------------//
// virtual
size_t ColIndSetBucketVector::getNumberOfEntities() const {
  size_t size = 0;
...
//---------------------------------------------------------------------------//
ColIndSetBucketVector &ColIndSetBucketVector::insert(ColIndSet &newSet) {
...
// virtual
#ifdef MJVIB_VERBOSE
void ColIndSetMatrix::out() const {
  // print setsList_ out
...
// ctor
//---------------------------------------------------------------------------//
ColIndSetMatrix::ColIndSetMatrix(const ColIndSetBucketVector &src, size_t size, CollHeap *heap)
    : ARRAY(EnumArray *)(heap, size + 1), currIndex_(0), lastIndex_(size + 1), setsList_(heap), settledList_(heap) {
  size += 1;
  size_t i;
...
//---------------------------------------------------------------------------//
void ColIndSetMatrix::setSettledByIndex(size_t i) {
...
// supersets
//---------------------------------------------------------------------------//
size_t ColIndSetMatrix::pickTheBestUnsettled() {
  size_t currMinSupersetsNum = entries(), currSupersetsNum, minSupersetsNumIndex = 0;

  for (currIndex_ = first(); currIndex_ > 0; currIndex_ = next()) {  // for each "unsettled" ColIndSet
    if ((currSupersetsNum = getSupersetsNum(currIndex_)) <
        currMinSupersetsNum) {  // update bestIndex, if the currColIndSet's supersets number
...
//---------------------------------------------------------------------------//
size_t ColIndSetMatrix::pickTheBestSettlement(size_t toPushIndex) {
  size_t currMinSubsetsNum = entries(), currSubsetsNum, minSubsetsNumIndex = 0;

  for (currIndex_ = 1; currIndex_ < lastIndex_; currIndex_++) {  // for each ColIndSet:
    if (at(toPushIndex)->at(currIndex_) == 1 &&
        (currSubsetsNum = getSubsetsNum(currIndex_)) <
            currMinSubsetsNum) {  // if it is the superset of the  ColIndSet #toPushIndex
...
//---------------------------------------------------------------------------//
void ColIndSetMatrix::update(size_t index) {
  // for each "unsettled" ColIndSet in the matrix do:
  for (currIndex_ = 1; currIndex_ > 0; currIndex_ = next()) {
    if (at(currIndex_)->at(index) == 1) {  // if ColIndSet #currIndex_ is-subset-of ColIndSet #index
...
// return top
//---------------------------------------------------------------------------//
ColIndSet &NestingStack::top() { return *at(entries() - 1); }
...
//---------------------------------------------------------------------------//
NABoolean NestingStack::isSubsetOfTop(const ColIndSet &newSet) {
  if (0 == entries()) {
    return TRUE;
  }
...
// push newSet
//---------------------------------------------------------------------------//
NestingStack &NestingStack::push(const ColIndSet &newSet) {
...
// produce the output.
//---------------------------------------------------------------------------//
void NestingStack::buildIndex(ColIndList &result) {
  ColIndSet *last;
  ColIndSet currSet;
  size_t currSetSize;
  ...
  while (!isEmpty())  // while there is a ColIndSet in the NestingStack
  {
    ...
    if (entries() > 0) {  // if there are more ColIndSets,
...
//---------------------------------------------------------------------------//
MJVIndCook &MJVIndCook::insert(const ColIndSet &newSet) {
...
//---------------------------------------------------------------------------//
MJVIndCook &MJVIndCook::insertOnto(const ColIndSet &setToBeCovered, const ColIndSet &newSet) {
  size_t numOfStacks = entries();
  for (size_t i = 0; i < numOfStacks; i++) {
...
//---------------------------------------------------------------------------//
MJVIndCook &MJVIndCook::addNewNestingStack(const ColIndSet &newSet) {
...
//---------------------------------------------------------------------------//
IndexList *MJVIndCook::buildIndex() {
...
//---------------------------------------------------------------------------//
IndexList *MJVIndexBuilder::buildIndex(const IndexList inputList) {
#ifdef MJVIB_VERBOSE
