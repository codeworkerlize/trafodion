...
ItemExpr *IndexJoinWithGroupbyRule::createCondPred(ItemExpr *selectionPred) {
  ...
  if (selectionPred->getOperatorType() == ITM_OR) {
    ...
    biPred1 = createCondPred(child1.getPtr());
    if (biPred0 == NULL && biPred1 == NULL)
      return NULL;
    else if (biPred0 == NULL)
      return biPred1;
                                                      ...
                                                      NABoolean &isDynStmt)     // out
{
  ...
  if (selectionPred->getOperatorType() == ITM_OR) {
    ...
    biPred1 = createOrPredicate(child1.getPtr(), pred1Count, isDynStmt);
    if (biPred0 == NULL && biPred1 == NULL)
      return NULL;
    else if (biPred0 == NULL)
      return biPred1;
...
RelExpr *IndexJoinWithGroupbyRule::makeSubstituteScan(Scan *s, RelExpr *partialResult) {
  ...
  newScan->setIsCreatedByNgram(TRUE);
  ...
  // predicates
  newUnionMap.rewriteValueIdSetDown(s->selectionPred(), vs);
  newPred = vs.rebuildExprTree(ITM_OR);
  newPred->synthTypeAndValueId();
  newScan->selectionPred() += newPred->getValueId();
  ...
  ValueIdList origCharOutputList = s->getGroupAttr()->getCharacteristicOutputs();
  ValueIdList newCharOutputList;
  ValueIdList valIdUnionList;
...
RelExpr *IndexJoinWithGroupbyRule::makeSubstituteFromIndexInfo(Scan *bef, ScanIndexInfo *ixi, NABoolean &isDynStmt) {
  ...
  ValueIdSet rightOutputSet = ixi->outputsFromRightScan_;
  rightOutputSet -= primaryKeySet;
  rightOutputSet += ixi->outputsFromRightScan_;

  // the char. outputs of the left and right scan are already
  // precomputed
  leftScan->setPotentialOutputValues(primaryKeySet);
  ...
  grpb->selectionPred() = havingPred->getValueId();
  grpb->groupExpr() = primaryKeySet;
  grpb->getGroupAttr()->setCharacteristicOutputs(primaryKeySet);
...
NABoolean JoinToTSJRule::topMatch(RelExpr *expr, Context *context) {
  ...
  */
  ...
  if (grpA1)
    if (grpA1->getAllSubtreeTables().entries() > (int)(ActiveSchemaDB()->getDefaults()).getAsLong(COMP_INT_10))
      return FALSE;
...
RelExpr *GroupBySplitRule::nextSubstitute(RelExpr *before, Context * /*context*/, RuleSubstituteMemory *&memory) {
  ...
  lowerGB->setIsFeasibleToTransformForAggrPushdown(bef->isFeasibleToTransformForAggrPushdown());

  //  transfer the parallel aggr pushdown flag to the upper the leaf
  upperGB->setParallelAggrPushdown(bef->parallelAggrPushdown());
  lowerGB->setParallelAggrPushdown(bef->parallelAggrPushdown());
...
// -----------------------------------------------------------------------
// methods for class GroupByOnUnionRule
// -----------------------------------------------------------------------
...
NABoolean GroupByOnUnionRule::topMatch(RelExpr *expr, Context * /*context*/) {
  ...
  //     union            ----->     union
  //     /   \                       /   \
  //   cut0  cut1                 grby1 grby2
  //                                |     |
  //                               cut0  cut1
  //
  // There are two cases where we can do this:
  //
  // a) We can guarantee that a group comes either from
  //    the left or the right child, but that a group
  //    never consists of rows from both left and right.
  //    One case where that happens is when the union
  //    contains a "label", a constant that identifies
  //    the child, and we group on that label.
  // b) If grby is a partial groupby leaf, then we can
...
RelExpr *GroupByOnUnionRule::nextSubstitute(RelExpr *before, Context * /*context*/,
  ...
  Union *befUnion = static_cast<Union *>(befGrby->child(0).getPtr());
  const ValueIdMap &befLeftMap = befUnion->getLeftMap();
  ...
  // more parts of the "before" tree
  ValueIdList parentGroupExpr(befGrby->groupExpr());
  ValueIdList parentAggrExpr(befGrby->aggregateExpr());

  // parts of the "after" tree
  const int arity = befUnion->getArity();
  MapValueIds *result = NULL;
  Union *afterUnion = NULL;
  GroupByAgg *childGBs[arity];
  ValueIdList childGroupExprs[arity];
  ValueIdList childAggrExprs[arity];
  ValueIdList vidUnionList;
  ...
  // The groupby and aggregates of the pushed groupbys were a rewrite
  // of the original groupbys and aggregates. Now we need to come up
  // with a list of ValueIdUnions, providing the values needed by the
  // result. For this, we just choose the groupby and aggregate
  // columns of the child groupbys combined. To make this easier, add
  // the three aggregate lists (parent, left and right child) to the
  ...
  // The before tree groupby produced the values in parentGroupExpr
  // (groupby + aggr now). vidUionList contains the equivalent
  // values produced by the new union node. So, this is what the
