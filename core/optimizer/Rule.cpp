...
Rule::Rule(const char *name, RelExpr *pattern, RelExpr *substitute) {
  ...
  if (pattern_) CMPASSERT(NOT pattern_->isSubtreeOp());
...
NABoolean Rule::topMatch(RelExpr *relExpr, Context *) {
...
RelExpr *Rule::nextSubstitute(RelExpr *before, Context *, RuleSubstituteMemory *& /*memory*/) {
  RelExpr *result;
...
Guidance *Rule::guidanceForExploringSubstitute(Guidance *) { return (NULL); }  // Rule::guidanceForExploringSubstitute

Guidance *Rule::guidanceForOptimizingSubstitute(Guidance *, Context *) {
  return (NULL);
...
void Rule::prepare() {
  ...
  // fill the "cuts" array
  if (pattern_) (void *)checkAndBindPattern(pattern_, cuts, wildcards, FALSE);

  // now replace all cut nodes in the substitute by the corresponding
  // cut nodes of the pattern
  if (substitute_) substitute_ = checkAndBindPattern(substitute_, cuts, wildcards, TRUE);

}  // Rule::prepare

RelExpr *Rule::checkAndBindPattern(RelExpr *patt, ARRAY(CutOp *) & cuts, ARRAY(WildCardOp *) & wildcards,
  ...
  if (patt->isCutOp()) {
    ...
    if (!isSubstitute) {
      ...
      if (NOT cuts.used(ix)) ABORT("cut in substitute doesn't match the pattern");
    ...
    if (!isSubstitute) {
      ...
      if (NOT wildcards.used(designator)) ABORT("wildcard in substitute has an invalid designator");
...
RuleSubset::RuleSubset(CollHeap *h) : SUBARRAY(Rule *)(&GlobalRuleSet->allRules_, h) {}

RuleSubset::RuleSubset(const RuleSubset &orig, CollHeap *h) : SUBARRAY(Rule *)(orig, h) {}
...
void RuleSet::insert(Rule *r) {
  ...
  if (r->isImplementationRule()) implRules_ += num;

  if (r->isTransformationRule()) transRules_ += num;

  if (r->isEnforcerRule()) enforcerRules_ += num;

  if (r->isContextSensitive()) contextSensitiveRules_ += num;

  if (r->isPassSensitive()) passSensitiveRules_ += num;

  if (r->isPatternSensitive()) patternSensitiveRules_ += num;
}
...
void RuleSet::setTotalPasses() {
  ...
  if (!CURRENTSTMT && (CmpCommon::getDefault(OPTIMIZATION_LEVEL) == DF_MINIMUM)) {
    totalPasses_ = 1;
  } else if (!CURRENTSTMT) {
    totalPasses_ = MAX_NUM_OF_PASSES;
  } else if ((CURRSTMT_OPTDEFAULTS->optLevel() == OptDefaults::MINIMUM) OR(
                 (CURRSTMT_OPTDEFAULTS->getQueryComplexity() > 3e13) AND NOT CURRSTMT_OPTDEFAULTS->joinOrderByUser())) {
    totalPasses_ = 1;
  } else {
    totalPasses_ = MAX_NUM_OF_PASSES;
  }
...
RelExpr *RuleSubstituteMemory::getNextSubstitute() {
  ...
  if (NOT getFirst(result)) result = NULL;
...
// -----------------------------------------------------------------------
RulesPerContext::RulesPerContext(const Context *const context, CollHeap *h) : context_(context), triedRules_(h) {
...
const RuleSubset &RulesPerContextList::getTriedRules(const Context *const context) const {
  ...
  RuleSubset *emptyRuleSubset = new (CmpCommon::statementHeap()) RuleSubset(CmpCommon::statementHeap());
...
void RulesPerContextList::addRule(const Context *const context, NAUnsigned ruleNumber) {
  ...
  RulesPerContext *newEntry = new (CmpCommon::statementHeap()) RulesPerContext(context, (CmpCommon::statementHeap()));
