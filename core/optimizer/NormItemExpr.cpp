...
// -----------------------------------------------------------------------
// -----------------------------------------------------------------------
static NABoolean canBeSQLUnknown(const ItemExpr *ie, NABoolean typeMustBeSQLBoolean = TRUE) {
  const NAType &typ = ie->getValueId().getType();
  if (typ.getTypeQualifier() == NA_BOOLEAN_TYPE) return ((SQLBooleanRelat &)typ).canBeSQLUnknown();
...
static void applyTruthTable(ItemExpr *ie, ExprValueId &locationOfPointerToMe) {
  enum truthResultEnum {
    TRU_ = ITM_RETURN_TRUE,   // true
  ...
  if (result != NOP_) {
    ...
    if (result == SIB_) {
      if (i == 1)
        ...
        CMPASSERT(FALSE);  // see for-loop test
...
// -----------------------------------------------------------------------
ItemExpr *buildComparisonPred(ItemExpr *rootPtr, ItemExpr *leftSubtreePtr, ItemExpr *rightSubtreePtr,
                              OperatorTypeEnum comparisonOpType = ITM_EQUAL,
                              NABoolean specialNulls = FALSE  //++MV - Irena
) {
  ItemExpr *eqPredPtr = new HEAP BiRelat(comparisonOpType, leftSubtreePtr, rightSubtreePtr,
...
// -----------------------------------------------------------------------
void ItemExpr::transformNode(NormWA &normWARef, ExprValueId &locationOfPointerToMe, ExprGroupId &introduceSemiJoinHere,
...
// ----------------------------------------------------------------------
NABoolean ItemExpr::predicateEliminatesNullAugmentedRows(NormWA & /* normWARef */, ValueIdSet & /* outerReferences */) {
  return FALSE;
...
// ----------------------------------------------------------------------
ItemExpr *ItemExpr::transformSubtreeOfNot(NormWA & /* normWARef */, OperatorTypeEnum /* falseOrNot */) {
  return this;
...
ItemExpr *ItemExpr::normalizeNode(NormWA &normWARef) {
  ...
  // ---------------------------------------------------------------------
    ...
    if (nodeIsNormalized()) return getReplacementExpr();
...
NABoolean ItemExpr::canTransformToSemiJoin(ItemExprList &valuesListIE, TableDesc *tdesc, int &numParams,
  ...
  if (ActiveSchemaDB()->getDefaults().getAsLong(OR_PRED_TO_SEMIJOIN) == 0) return FALSE;  // feature is turned OFF

  ItemExpr *predIE = (ItemExpr *)this;
  if (CmpCommon::getDefault(RANGESPEC_TRANSFORMATION) == DF_ON) predIE = revertBackToOldTree(h, (ItemExpr *)this);
  ...
  if (eqList[0].getItemExpr()->getOperatorType() == ITM_EQUAL) {
    ...
    isCovered = colExpr->isCovered(noCharacteristicInputs, scanGA, dummy1, dummy2, dummy3);
    ...
    if (rcTypeEnum == NA_UNKNOWN_TYPE) return FALSE;
  }

  numParams = 0;
  for (CollIndex i = 0; i < eqList.entries(); i++) {
    if (eqList[i].getItemExpr()->getOperatorType() != ITM_EQUAL) return FALSE;
...
// ***********************************************************************
void Aggregate::transformNode(NormWA &normWARef, ExprValueId &locationOfPointerToMe, ExprGroupId &introduceSemiJoinHere,
                              const ValueIdSet &externalInputs) {
  // ---------------------------------------------------------------------
  // Transform the operands of the Aggregate
  // ---------------------------------------------------------------------
  normWARef.setNullFlag();
  ItemExpr::transformNode(normWARef, locationOfPointerToMe, introduceSemiJoinHere, externalInputs);
  ...
  // ---------------------------------------------------------------------
  if (!isSensitiveToDuplicates()) setDistinct(FALSE);
...
ItemExpr *Aggregate::normalizeNode(NormWA &normWARef) {
  ...
  if (isDistinct())
    distinctId_ = distinctId_.getItemExpr()->getReplacementExpr()->normalizeNode(normWARef)->getValueId();
...
//----------------------------------------------------------------------------
//++ MV OZ
void Between::setDirectionVector(const IntegerList &directionVector, CollHeap *heap) {
...
ItemExpr *Between::transformIntoTwoComparisons() {
  ...
  NABoolean optimizeForEquals = (leftBoundryIncluded_ AND rightBoundryIncluded_ AND pDirectionVector_ == NULL);
  ...
  // ---------------------------------------------------------------------
  if (optimizeForEquals AND leftVal->getOperatorType() == ITM_ITEM_LIST) {
    ItemExprList leftList(leftVal, HEAP);
    ItemExprList startList(startVal, HEAP);
    ItemExprList endList(endVal, HEAP);

    while (leftList.entries() > 0 AND startList[0] == endList[0]) {
      ItemExpr *eqPred = new HEAP BiRelat(ITM_EQUAL, leftList[0], startList[0]);
      if (tfm)
  ...
  if (leftVal) {
    ...
    BiRelat *leftCondition = new HEAP BiRelat(leftBoundryOp, leftVal, startVal);

    BiRelat *rightCondition = new HEAP BiRelat(rightBoundryOp, leftVal, endVal);
...
ItemExpr *Between::transformMultiValuePredicate(NABoolean flattenSubqueries, ChildCondition condBiRelat) {
  ItemExpr *tfm = transformIntoTwoComparisons()->transformMultiValuePredicate(flattenSubqueries, condBiRelat);
...
void Between::transformNode(NormWA &normWARef, ExprValueId &locationOfPointerToMe, ExprGroupId &introduceSemiJoinHere,
                            const ValueIdSet &externalInputs) {
  DBGSETDBG("TRANSFORM_DEBUG")
  ...
  locationOfPointerToMe->transformNode(normWARef, locationOfPointerToMe, introduceSemiJoinHere, externalInputs);
...
ItemExpr *Between::transformSubtreeOfNot(NormWA &normWARef, OperatorTypeEnum falseOrNot) {
  ItemExpr *tfm = transformIntoTwoComparisons();
...
// ***********************************************************************
void BuiltinFunction::transformNode(NormWA &normWARef, ExprValueId &locationOfPointerToMe,
...
// ***********************************************************************
void UDFunction::transformNode(NormWA &normWARef, ExprValueId &locationOfPointerToMe,
                               ExprGroupId &introduceSemiJoinHere, const ValueIdSet &externalInputs) {
  if (nodeIsTransformed()) {
  ...
  if (inputVars_.transformNode(normWARef, introduceSemiJoinHere, externalInputs)) {
  ...
  // IsolatedScalarUDF
  // ---------------------------------------------------------------------
  transformToRelExpr(normWARef, locationOfPointerToMe, introduceSemiJoinHere, externalInputs);
...
// -----------------------------------------------------------------------
void UDFunction::transformToRelExpr(NormWA &normWARef, ExprValueId &locationOfPointerToMe,
                                    ExprGroupId &introduceSemiJoinHere, const ValueIdSet &externalInputs) {
  ...
  if (rdesc->getEffectiveNARoutine() != NULL && (!rdesc->getEffectiveNARoutine()->isIsolate()))
  ...
  switch (getRoutineDesc()->getEffectiveNARoutine()->getRoutineType()) {
    ...
    case COM_ACTION_UDF_TYPE: {
      ...
      isUdf = new (normWARef.wHeap()) IsolatedScalarUDF(getRoutineDesc()->getEffectiveNARoutine()->getSqlName(), params,
                                                        dummyOutputs, normWARef.wHeap());
      ...
      isUdf->getGroupAttr()->setHasNonDeterministicUDRs(!getRoutineDesc()->getEffectiveNARoutine()->isDeterministic());

      // Get the valueIds of the parameters
      isUdf->gatherParamValueIds(isUdf->getProcAllParamsTree(), isUdf->getProcAllParamsVids());
    ...
    default: {
      *CmpCommon::diags() << DgSqlCode(2997) << DgString1("Unsupported Function type");
  ...
  if (normWARef.inValueIdProxy()) {
    ...
    replacementExpr = udfDesc_->getOutputColumnList().rebuildExprTree(ITM_ITEM_LIST, FALSE, FALSE);
  }
  ...
  newJoin = new (normWARef.wHeap()) Join(introduceSemiJoinHere, newRightChild, REL_ROUTINE_JOIN);
...
// -----------------------------------------------------------------------
void InstantiateNull::transformNode(NormWA &normWARef, ExprValueId &locationOfPointerToMe,
                                    ExprGroupId &introduceSemiJoinHere, const ValueIdSet &externalInputs) {
  // Indicate that we are processing a complex scalar expression.
  normWARef.setComplexScalarExprFlag();
  // ---------------------------------------------------------------------
...
ItemExpr *InstantiateNull::initiateLeftToInnerJoinTransformation(NormWA &normWARef) {
  if (!NoCheckforLeftToInnerJoin) {
    ...
    // ---------------------------------------------------------------------
    setReplacementExpr(getExpr()->initiateLeftToInnerJoinTransformation(normWARef));
...
ItemExpr *InstantiateNull::getReplacementExpr() const {
  ...
  if (beginLOJTransform_) return iNull;

  ItemExpr *ie = ItemExpr::getReplacementExpr();

  if ((ItemExpr *)iNull == ie) return ie;

  while (ie != ie->getReplacementExpr() && ie->getOperatorType() == ITM_INSTANTIATE_NULL) {
...
ItemExpr *InstantiateNull::getReplacementExpr(NABoolean forceNavigate) const {
  ...
  if (NOT forceNavigate) return iNull->getReplacementExpr();
...
ItemExpr *InstantiateNull::normalizeNode(NormWA &normWARef) {
  ...
  if (getReplacementExpr() != this) setReplacementExpr(child(0).getPtr());
...
DisjunctArray *BoolVal::mdamTreeWalk() {
  DisjunctArray *disjunctArray = new HEAP DisjunctArray(new HEAP ValueIdSet(getValueId()));
...
// ***********************************************************************
ItemExpr *BiLogic::transformMultiValuePredicate(NABoolean flatten, ChildCondition condBiRelat) {
...
void BiLogic::transformNode(NormWA &normWARef, ExprValueId &locationOfPointerToMe, ExprGroupId &introduceSemiJoinHere,
                            const ValueIdSet &externalInputs) {
  if (nodeIsTransformed()) {
    locationOfPointerToMe = getReplacementExpr();
    return;
  ...
  switch (getOperatorType()) {
    ...
    case ITM_OR: {
      ...
      if (NOT normWARef.haveAnOrAncestor()) {
        ...
        if ((NOT normWARef.dontAddRedundantPredicatesFlag()) &&
            (CmpCommon::getDefault(FIND_COLUMN_SUBEXPRS_IN_OR) == DF_ON)) {
          // If there are predicates of the form
          //   (t1.a = 4 and t2.b = 6) or (t1.a = 5 and t2.b = 7),
          // this can't be pushed down to scan nodes. However,
          // we can introduce redundant predicates that can be
          // pushed down to scan nodes. In this example:
          //   (t1.a = 4 or t1.a = 5) and
          //   (t2.b = 6 or t2.b = 7) and
          //   (((t1.a = 4 and t2.b = 6) or (t1.a = 5 and t2.b = 7))
          // Such predicates occur, for example, in TPC-DS Query 48.
          // We attempt such a transformation here.
          locationOfPointerToMe = locationOfPointerToMe->applyInverseDistributivityLawByColumn();
          getValueId().replaceItemExpr(locationOfPointerToMe);
          locationOfPointerToMe->synthTypeAndValueId();
        }
        ...
        // distributivity law transformation a second time, but
        // we set a flag to prevent introducing more redundant
        // predicates)
        normWARef.setDontAddRedundantPredicatesFlag();
        locationOfPointerToMe->transformNode(normWARef, locationOfPointerToMe, introduceSemiJoinHere, externalInputs);
        normWARef.restoreDontAddRedundantPredicatesFlag();
      }
...
// -----------------------------------------------------------------------
ItemExpr *BiLogic::transformSubtreeOfNot(NormWA &normWARef, OperatorTypeEnum falseOrNot) {
  ItemExpr *rv = NULL;

  switch (getOperatorType()) {
    case ITM_AND:
      // NOT(A AND B) ==> NOT A OR NOT B
      rv = new (normWARef.wHeap()) BiLogic(ITM_OR, new (normWARef.wHeap()) UnLogic(falseOrNot, child(0).getPtr()),
                                           new (normWARef.wHeap()) UnLogic(falseOrNot, child(1).getPtr()));
      break;
    case ITM_OR:
      // NOT(A OR B) ==> NOT A AND NOT B
      rv = new (normWARef.wHeap()) BiLogic(ITM_AND, new (normWARef.wHeap()) UnLogic(falseOrNot, child(0).getPtr()),
                                           new (normWARef.wHeap()) UnLogic(falseOrNot, child(1).getPtr()));
      break;
    default:
...
// -----------------------------------------------------------------------
NABoolean BiLogic::predicateEliminatesNullAugmentedRows(NormWA &normWARef, ValueIdSet &outerReferences) {
  ...
  if (!normWARef.walkingAnExprTree() &&
      !normWARef.subqUnderExprTree()) {  // does NOT lie within an expression tree
                                         // -------------------------------------------------------------
    ...
    for (CollIndex i = 0; i < 2; i++) {
      if (child(i)->getOperatorType() == ITM_INSTANTIATE_NULL) {
        ...
        if ((normWARef.inSelectList()) && !(inst->NoCheckforLeftToInnerJoin)) {
          ...
          emptyGA->coverTest(*instNullValue, *emptySet, *coveredExpr, *emptySet1, coveredSubExpr);
        ...
        if ((!inst->NoCheckforLeftToInnerJoin) && !containsOuterReferencesInSelectList) {
          child(i) = child(i)->initiateLeftToInnerJoinTransformation(normWARef);
          returnValue = TRUE;
        }
        // we have to dig deeper
      } else if (child(i)->predicateEliminatesNullAugmentedRows(normWARef, outerReferences) == TRUE) {
        returnValue = TRUE;
      }
...
DisjunctArray *BiLogic::mdamTreeWalk() {
  ...
  switch (getOperatorType())  // what node are we on?
  {
    case ITM_AND:  // we are at an AND node
    {
      ...
      // -----------------------------------------------------------------
      return mdamANDDisjunctArrays(leftDisjunctArray, rightDisjunctArray);
    }
    case ITM_OR:  // we are at an OR node
    {
      ...
      // up the expression tree.
      // -----------------------------------------------------------------
      return mdamORDisjunctArrays(leftDisjunctArray, rightDisjunctArray);
    }
    default:
...
static NABoolean switchLeftAndRightChildren(ItemExpr *leftChild, ItemExpr *rightChild) {
  // Fastpath for expression of the form
  //  <column> <op> <expression>, <expression> op <column>
  if (leftChild->getOperatorType() == ITM_BASECOLUMN || leftChild->getOperatorType() == ITM_INDEXCOLUMN)
    return FALSE;
  else if (rightChild->getOperatorType() == ITM_BASECOLUMN || rightChild->getOperatorType() == ITM_INDEXCOLUMN)
    return TRUE;
  ...
  for (exprId = leftLeafValues.init(); leftLeafValues.next(exprId); leftLeafValues.advance(exprId)) {
  ...
  for (exprId = rightLeafValues.init(); rightLeafValues.next(exprId); rightLeafValues.advance(exprId)) {
    if ((exprId.getItemExpr()->getOperatorType() == ITM_BASECOLUMN) ||
        (exprId.getItemExpr()->getOperatorType() == ITM_INDEXCOLUMN)) {
...
// -----------------------------------------------------------------------
static ItemExpr *dissectOutSubqueries(ItemExpr *thisIE, NABoolean flattenSubqueries,
                                      NABoolean considerDirectChildSubqueries) {
  if (CmpCommon::getDefault(COMP_BOOL_137) == DF_OFF) return NULL;
    ...
    if (thisIE->child(i)->getOperatorType() == ITM_ITEM_LIST) {
      ...
      for (CollIndex i = list.entries(); i--;)
        if (list[i]->isASubquery()) {
          ItemExpr *u = new HEAP UnLogic(ITM_IS_NOT_UNKNOWN, new HEAP UnLogic(ITM_IS_NULL, list[i]));
          newPred = (!newPred) ? u : new HEAP BiLogic(ITM_AND, u, newPred);
        }
    } else if (considerDirectChildSubqueries && thisIE->child(i)->isASubquery()) {
      ItemExpr *u = new HEAP UnLogic(ITM_IS_NOT_UNKNOWN, new HEAP UnLogic(ITM_IS_NULL, thisIE->child(i)));
      newPred = (!newPred) ? u : new HEAP BiLogic(ITM_AND, u, newPred);
    }
...
//                     (B = Y) AND (C > Z))
// -----------------------------------------------------------------------
static ItemExpr *transformMultiValueComparison(BiRelat *thisCmp, NABoolean flattenSubq, NABoolean flattenUDF,
                                               IntegerList *directionVector) {
  // Convert the child subtrees into lists.
  ItemExprList lhs(thisCmp->child(0).getPtr(), HEAP, ITM_ITEM_LIST, flattenSubq, flattenUDF);
  ItemExprList rhs(thisCmp->child(1).getPtr(), HEAP, ITM_ITEM_LIST, flattenSubq, flattenUDF);
  ...
  if (!flattenSubq) {
    ...
    while (i--) {
      ...
      if (lhs[i]->isASubquery()) ll = ((Subquery *)lhs[i])->getSubquery()->getDegree();
      if (rhs[i]->isASubquery()) rr = ((Subquery *)rhs[i])->getSubquery()->getDegree();
  ...
  if (thisCmp->getOperatorType() == ITM_EQUAL) {
    ...
    while (i--) tfm = new HEAP BiLogic(ITM_AND, new HEAP BiRelat(ITM_EQUAL, lhs[i], rhs[i], special), tfm);
  } else {
    ...
    thisOp = origOp;
    if (directionVector != NULL && directionVector->at(i) == -1) thisOp = origRevOp;
    ...
    while (i--)  // we MUST build THESE preds BACKWARDS
    {
      // If the direction is DESC flip the sign.
      thisOp = newOp;
      if (directionVector != NULL && directionVector->at(i) == -1) thisOp = newRevOp;
      ...
      if (newOp != ITM_NOT_EQUAL)
        tfm = new HEAP BiLogic(ITM_AND, new HEAP BiRelat(ITM_EQUAL, lhs[i], rhs[i], special), tfm);
    ...
    if ((tfm->getOperatorType() == ITM_OR) && (origOp != ITM_NOT_EQUAL)) {
      ...
      if (prefixCompOper == ITM_GREATER) prefixCompOper = ITM_GREATER_EQ;

      if (prefixCompOper == ITM_LESS) prefixCompOper = ITM_LESS_EQ;

      ItemExpr *compLeftChild = (ItemExpr *)prefixComparison->child(0);
      ItemExpr *compRightChild = (ItemExpr *)prefixComparison->child(1);

      BiRelat *addedComparison =
          new HEAP BiRelat(prefixCompOper, compLeftChild, compRightChild, prefixComparison->getSpecialNulls());

      if (thisCmp->isPreferredForSubsetScanKey()) addedComparison->setPreferForSubsetScanKey();

      addedComparison->setDerivedFromMCRP();
      addedComparison->setListOfComparisons(listOfComparisons);

      tfm = new HEAP BiLogic(ITM_AND, addedComparison, tfm);
    }
  }
...
ItemExpr *BiRelat::transformMultiValuePredicate(NABoolean flattenSubqueries,  // default TRUE
...

{
  ...
  if (directionVector_ != NULL)
    ...
    tfmNeeded = (child(0)->getOperatorType() == ITM_ITEM_LIST || child(1)->getOperatorType() == ITM_ITEM_LIST) ||
                (child(0)->getOperatorType() == ITM_ONE_ROW || child(1)->getOperatorType() == ITM_ONE_ROW);
  ...
  if (tfmIf == ANY_CHILD_RAW)
    ...
    tfm = transformMultiValueComparison(this, flattenSubqueries, flattenUDFs, directionVector_);

  return tfm;
}  // BiRelat::transformMultiValuePredicate()

void BiRelat::transformNode(NormWA &normWARef, ExprValueId &locationOfPointerToMe, ExprGroupId &introduceSemiJoinHere,
                            const ValueIdSet &externalInputs) {
  if (nodeIsTransformed()) {
    locationOfPointerToMe = getReplacementExpr();
    return;
  ...
  // ---------------------------------------------------------------------
  ChildCondition tfmIf = specialMultiValuePredicateTransformation() ? ALL_CHILDREN : ANY_CHILD;
  ...
  // ---------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------
    BiRelat *flipflop = new (normWARef.wHeap()) BiRelat(getReverseOperatorType(), child(1).getPtr(), child(0).getPtr());

    flipflop->specialMultiValuePredicateTransformation() = specialMultiValuePredicateTransformation();
    //++MV - Irena
    flipflop->setSpecialNulls(getSpecialNulls());
    flipflop->setCompatibleNulls(getCompatibleNulls());
    //--MV - Irena
    ...
    if (getOperatorType() == ITM_EQUAL && !normWARef.walkingAnExprTree()) {
      ...
      if (bothAreColumnsOrUserInput(normWARef, child(0), child(1))) {
        ...
        // in that case.
        // Irena
        if (child(0)->getValueId() == child(1)->getValueId() && NOT getSpecialNulls()  // ++MV - Irena
        ) {
          // the check for whether T1.A is nullable is done inside
          // UnLogic::transformIsNull(), which gets called by
          // UnLogic::transformNode()
          UnLogic *newNode = new (normWARef.wHeap()) UnLogic(ITM_IS_NOT_NULL, child(0));
          ...
          if (normWARef.isInEmbeddedUpdateOrDelete() || normWARef.isInEmbeddedInsert()) {
            if (child(0)->isAColumnReference() && child(1)->isAColumnReference()) {
            ...
            } else if (child(0)->isAUserSuppliedInput() || child(1)->isAUserSuppliedInput()) {
              putInVeg = FALSE;
            }
          }
          ...
          if (type1.getTypeQualifier() == NA_CHARACTER_TYPE && type2.getTypeQualifier() == NA_CHARACTER_TYPE) {
            if ((NOT DFS2REC::isAnyVarChar(type1.getFSDatatype()) && DFS2REC::isAnyVarChar(type2.getFSDatatype())) ||
                (DFS2REC::isAnyVarChar(type1.getFSDatatype()) && NOT DFS2REC::isAnyVarChar(type2.getFSDatatype())) ||
                (DFS2REC::isAnyVarChar(type1.getFSDatatype()) && DFS2REC::isAnyVarChar(type2.getFSDatatype()))) {
              if (CmpCommon::getDefault(COMP_BOOL_158) == DF_OFF) putInVeg = FALSE;
            }
          }
          ...
          if (externalInputs.contains(child(0)->getValueId()) && externalInputs.contains(child(1)->getValueId())) {
            putInVeg = FALSE;
          }
          ...
          if (isStartWithExpr() == TRUE || isConnectByExpr() == TRUE) {
            putInVeg = FALSE;
          }
...
// discarding null augmented rows produced by a left join.
// -----------------------------------------------------------------------
NABoolean BiRelat::predicateEliminatesNullAugmentedRows(NormWA &normWARef, ValueIdSet &outerReferences) {
  NABoolean returnValue = FALSE;
  // -----------------------------------------------------------------
  // An InstantiateNull is used by the LeftJoin for instantiating a
  // null value in place of a value that is produced as output by
...
ItemExpr *BiRelat::normalizeNode(NormWA &normWARef) {
  ...
  if (getOperatorType() == ITM_EQUAL && !normWARef.walkingAnExprTree() &&
    ...
    if (!(child(0)->getValueId() == child(1)->getValueId())) {
      ItemExpr *vegRef0Ptr = normWARef.getVEGReference(child(0)->getValueId());
      ItemExpr *vegRef1Ptr = normWARef.getVEGReference(child(1)->getValueId());
  ...
  ((VEGPredicate *)normalizedExpr)->setCompatibleNulls(getCompatibleNulls());
  //--MV - Irena
  return normalizedExpr;
...
// -----------------------------------------------------------------------
ItemExpr *BiRelat::transformSubtreeOfNot(NormWA &normWARef, OperatorTypeEnum falseOrNot) {
  if (falseOrNot != ITM_NOT) return this;
  ...
  BiRelat *newComp = new (normWARef.wHeap()) BiRelat(otype, child(0).getPtr(), child(1).getPtr());

  newComp->setDirectionVector(directionVector_);

  newComp->specialMultiValuePredicateTransformation() = specialMultiValuePredicateTransformation();

  //++MV - Irena
  newComp->setSpecialNulls(getSpecialNulls());
  newComp->setCompatibleNulls(getCompatibleNulls());
  //--MV - Irena
...
NABoolean PatternMatchingFunction::predicateEliminatesNullAugmentedRows(NormWA &normWARef,
  ...
  if (!normWARef.walkingAnExprTree() &&
    ...
    if (child(0)->getOperatorType() == ITM_INSTANTIATE_NULL) {
      ...
      if ((!inst->NoCheckforLeftToInnerJoin) && !containsOuterReferencesInSelectList) {
        child(0) = child(0)->initiateLeftToInnerJoinTransformation(normWARef);
        returnValue = TRUE;
      }
    }
  }
...
void NotIn::transformNode(NormWA &normWARef, ExprValueId &locationOfPointerToMe, ExprGroupId &introduceSemiJoinHere,
                          const ValueIdSet &externalInputs) {
  if (nodeIsTransformed()) {
    locationOfPointerToMe = getReplacementExpr();
    return;
  ...
  if (CmpCommon::getDefault(NOT_IN_ANSI_NULL_SEMANTICS) == DF_OFF) {
    ...
    tfm = new (normWARef.wHeap()) BiRelat(ITM_EQUAL, child(0), child(1));
    if (tfm) {
      getValueId().replaceItemExpr(tfm);
      tfm->synthTypeAndValueId(TRUE);
      locationOfPointerToMe = tfm;
...
ValueIdSet NotIn::rewriteMultiColNotInPredicate(ValueId ninValId, ValueIdSet jPred, ValueIdSet selPred) {
  ItemExpr *itmExpr = ninValId.getItemExpr();

  CMPASSERT(itmExpr->getOperatorType() == ITM_NOT_IN && itmExpr->child(0)->getOperatorType() == ITM_ITEM_LIST &&
  ...
  CMPASSERT(outer.entries() > 0 && outer.entries() == inner.entries());
  ...
  for (CollIndex i = 0; i < outer.entries(); i++) {
    ...
    if ((!innerType.supportsSQLnull() || jPred.isNotNullable(inner[i]->getValueId())) &&
        (!outerType.supportsSQLnull() || selPred.isNotNullable(outer[i]->getValueId()))) {
      ItemExpr *newPred = new (CmpCommon::statementHeap()) BiRelat(ITM_EQUAL, outer[i], inner[i]);

      newPred->synthTypeAndValueId(TRUE);
      predSet += newPred->getValueId();

    } else {
      ItemExpr *newPred = new (CmpCommon::statementHeap()) BiRelat(ITM_NOT_EQUAL, outer[i], inner[i]);

      if (!nonEquiPred) {
        nonEquiPred = newPred;
      } else {
        nonEquiPred = new (CmpCommon::statementHeap()) BiLogic(ITM_OR, nonEquiPred, newPred);
      }
    }
  }  // for (CollIndex i = 0 ; i < outer.entries() ; i++)

  if (nonEquiPred) {
    nonEquiPred = new (CmpCommon::statementHeap()) UnLogic(ITM_IS_TRUE, nonEquiPred);
    nonEquiPred = new (CmpCommon::statementHeap()) UnLogic(ITM_NOT, nonEquiPred);
...
// -----------------------------------------------------------------------
void Subquery::transformNode(NormWA &normWARef, ExprValueId &locationOfPointerToMe, ExprGroupId &introduceSemiJoinHere,
                             const ValueIdSet &externalInputs) {
  if (nodeIsTransformed()) {
    // Return the address of the expression that was used for replacing
    // this subquery in an earlier call.
  ...
  if (isAnAllSubquery() && normWARef.walkingAnExprTree()) {
    ExprValueId newRoot = new (normWARef.wHeap())
        UnLogic(ITM_NOT, new (normWARef.wHeap()) QuantifiedComp(getComplementOfOperatorType(), child(0).getPtr(),
                                                                getSubquery(), getAvoidHalloweenR2()));
    // replace the definition of this valueId
    getValueId().replaceItemExpr(newRoot);

    newRoot->synthTypeAndValueId(TRUE);

    ((QuantifiedComp *)newRoot->child(0)->castToItemExpr())->setCreatedFromALLpred(TRUE);
    // -----------------------------------------------------------------
    // Perform the subquery transformation.
    // -----------------------------------------------------------------
    newRoot->child(0)->transformNode(normWARef, newRoot->child(0), introduceSemiJoinHere, externalInputs);
  ...
  // ---------------------------------------------------------------------
  if (normWARef.walkingAnExprTree()) normWARef.setSubqUnderExprTreeFlag();
...
void Subquery::evaluateCandidateForUnnesting(NormWA &normWARef, Join *newJoin, GroupByAgg *newGrby) {
  ...
  if (isAnAnySubquery()) wasAnAllSubquery = ((QuantifiedComp *)this)->createdFromALLpred();
  ...
  if (NOT((isAnExistsSubquery() && normWARef.isChildOfANot()) ||               // NOT EXISTS
          ...
          normWARef.inGenericUpdateAssign() || normWARef.haveAnOrAncestor()))  // have an OR ancestor
  {
  ...
  //--------------------------------------------------------------------
  if ((CmpCommon::getDefault(SUBQUERY_UNNESTING) != DF_OFF) && (skipUnnesting == FALSE)) {
    // Give a hint for QA that we are trying to unnest
    if (CmpCommon::getDefault(SUBQUERY_UNNESTING) == DF_DEBUG)
      *CmpCommon::diags() << DgSqlCode(2997) << DgString1("Attempting to unnest Subquery");
...
// Perform the Join-Aggregate or SemiJoin subquery transformation.
// -----------------------------------------------------------------------
void Subquery::transformToRelExpr(NormWA &normWARef, ExprValueId &locationOfPointerToMe,
                                  ExprGroupId &introduceSemiJoinHere, const ValueIdSet &externalInputs) {
  if (nodeIsTransformed()) return;
  markAsTransformed();
  ...
  if (!isARowSubquery()) {
    ...
    if (childOfRoot->getOperatorType() == REL_GROUPBY) {
      ...
      if (!aggNode->groupExpr().isEmpty() && aggNode->aggregateExpr().isEmpty() && !aggNode->isRollup()) {
        ...
        if (childOfRoot->getOperator() == REL_ROOT) {
          ...
          childOfRoot->child(0)->selectionPred() += childOfRoot->selectionPred();
        ...
        childOfRoot->getGroupAttr()->addCharacteristicInputs(aggNode->getGroupAttr()->getCharacteristicInputs());
      }
    }
  }
  ...
  // == {always TRUE}.
  if (aggNode && aggNode->selectionPred().isEmpty() && isAnExistsSubquery()) {
    ItemExpr *tf;
  ...
  if (isARowSubquery() || normWARef.walkingAnExprTree()) {
    ...
    // for NOT EXISTS
    // -------------------------------------------------------------
    if (!aggNode || !aggNode->selectionPred().isEmpty() || isNotExists()) {
      // -------------------------------------------------------------
      ...
      // aggregateExpr contains the aggregate expressions.
      // -------------------------------------------------------------
      newGrby = new (normWARef.wHeap()) GroupByAgg(getSubquery(), aggregateExpr);

      newGrby->setGroupAttr(new (normWARef.wHeap()) GroupAttributes());
      ...
      newGrby->getGroupAttr()->addCharacteristicInputs(newGrby->child(0)->getGroupAttr()->getCharacteristicInputs());
      ...
      if (isAnExistsSubquery()) {
        ...
        BiRelat *pred = new (normWARef.wHeap()) BiRelat(otype, child(0).getPtr(), getSubquery()->selectList());
    ...
    // one value.
    // -----------------------------------------------------------------
    newJoin = new (normWARef.wHeap()) Join(introduceSemiJoinHere, newGrby, REL_TSJ);
    newJoin->setGroupAttr(new (normWARef.wHeap()) GroupAttributes());
    ...
    DBGSETDBG("TRANSFORM_DEBUG")
    DBGIF(unp = ""; aggregateExpr.unparse(unp); cerr << "QComp BlkBox: " << unp << " " << (void *)this << endl;)

    // -----------------------------------------------------------------
    // Allocate a new semijoin or anti semijoin
    // -----------------------------------------------------------------
    OperatorTypeEnum joinOp = REL_SEMITSJ;
    if (isAnAllSubquery() || isNotExists()) joinOp = REL_ANTI_SEMITSJ;

    newJoin = new (normWARef.wHeap()) Join(introduceSemiJoinHere, getSubquery(), joinOp);

    newJoin->setGroupAttr(new (normWARef.wHeap()) GroupAttributes());

    if (CmpCommon::getDefault(SEMIJOIN_TO_INNERJOIN_TRANSFORMATION) != DF_OFF) {
    ...
    if (isAnExistsSubquery()) {
      ...
      if (isAnAllSubquery() && !normWARef.walkingAnExprTree() && (CmpCommon::getDefault(COMP_BOOL_139) == DF_ON)) {
        ItemExpr *filt = new (normWARef.wHeap()) UnLogic(ITM_IS_NOT_NULL, child(0).getPtr());
...
// -----------------------------------------------------------------------
ItemExpr *Subquery::getBlackBoxExpr(NormWA & /*normWARef*/, ValueIdSet & /*blackBoxExpr*/) {
...
// in a subtree that is rooted in a NOT.
// -----------------------------------------------------------------------
ItemExpr *Subquery::transformSubtreeOfNot(NormWA &normWARef, OperatorTypeEnum falseOrNot) {
...
// $$$$ RowSubquery
// ***********************************************************************
ItemExpr *RowSubquery::getBlackBoxExpr(NormWA &normWARef, ValueIdSet &blackBoxExpr) {
  // ---------------------------------------------------------------------
  // The oneRow aggregate function implements the row subquery semantics.
  // ---------------------------------------------------------------------
  ItemExpr *aggExpr = new (normWARef.wHeap()) Aggregate(ITM_ONE_ROW, getSubquery()->selectList());
  ...
  if ((aggChild->getOperatorType() == ITM_CAST) || (aggChild->getOperatorType() == ITM_INSTANTIATE_NULL)) {
    ...
    getValueId().changeType(((Cast *)aggChild)->getType());
    return aggChild;
  } else {
...
// in a subtree that is rooted in a NOT.
// -----------------------------------------------------------------------
ItemExpr *Exists::transformSubtreeOfNot(NormWA &normWARef, OperatorTypeEnum falseOrNot) {
  ItemExpr *rv = this;
  ...
  if ((CmpCommon::getDefault(COMP_BOOL_162) == DF_ON) && (normWARef.getWalkingAnExprTreeCount() == 1)) {
...
// $$$$ QuantifiedComp
// -----------------------------------------------------------------------
ItemExpr *QuantifiedComp::transformMultiValuePredicate(NABoolean flattenSubqueries, ChildCondition tfmIf) {
  if (tfmIf == ANY_CHILD_RAW) return dissectOutSubqueries(this, flattenSubqueries, FALSE);
  return NULL;
}
...
// is the child of a NOT.
// -----------------------------------------------------------------------
ItemExpr *QuantifiedComp::transformSubtreeOfNot(NormWA &normWARef, OperatorTypeEnum falseOrNot) {
  if (falseOrNot != ITM_NOT) return this;
  ...
  {
    ItemExpr *newComp =
        new (normWARef.wHeap()) QuantifiedComp(otype, child(0).getPtr(), getSubquery(), getAvoidHalloweenR2());
    return newComp;
  } else
    return this;
...
ItemExpr *QuantifiedComp::getBlackBoxExpr(NormWA &normWARef, ValueIdSet &blackBoxExpr) {
  ...
  // ---------------------------------------------------------------------
  ItemExpr *pred = new (normWARef.wHeap()) BiRelat(otype, child(0).getPtr(), getSubquery()->selectList());
  // ---------------------------------------------------------------------
  ...
  if (isAnAllSubquery() && !normWARef.walkingAnExprTree()) {
    if (CmpCommon::getDefault(NOT_IN_OPTIMIZATION) == DF_ON && getOperatorType() == ITM_NOT_EQUAL_ALL) {
      // create a new NotIn with the child0 and child1 of birelat
      //
      pred = new (normWARef.wHeap()) NotIn(pred->child(0), pred->child(1));
    } else {
      pred = new (normWARef.wHeap()) UnLogic(ITM_IS_TRUE, pred);
      pred = new (normWARef.wHeap()) UnLogic(ITM_NOT, pred);
      ...
      if ((CmpCommon::getDefault(COMP_BOOL_139) == DF_ON) && (otype == ITM_NOT_EQUAL)) {
        ...
        while (selList) {
          if (selList->getOperatorType() == ITM_ITEM_LIST) {
            if (selList->child(1)->getValueId().getType().supportsSQLnullLogical()) {
              allNotNull = FALSE;
              break;
            }
        ...
        if (allNotNull) {
          pred = new (normWARef.wHeap()) BiRelat(ITM_EQUAL, child(0).getPtr(), getSubquery()->selectList());
  ...
  if (isAnAnySubquery() && normWARef.walkingAnExprTree()) {
    ...
    ItemExpr *aggExpr = new (normWARef.wHeap()) Aggregate(ITM_ANY_TRUE, pred);
    aggExpr->synthTypeAndValueId(TRUE);
    aggExpr->convertToValueIdSet(blackBoxExpr);
...
// ***********************************************************************
ItemExpr *UnLogic::transformMultiValuePredicate(NABoolean flattenSubqueries, ChildCondition condBiRelat) {
  ...
  if (getOperatorType() == ITM_IS_NULL || getOperatorType() == ITM_IS_NOT_NULL) return NULL;

  ItemExpr *tfm = child(0)->transformMultiValuePredicate(flattenSubqueries, condBiRelat);
...
ItemExpr *UnLogicMayBeAnEliminableTruthTest(ItemExpr *unlogic, NABoolean aggOK) {
  ...
  if ((notVal && itm->getOperatorType() == ITM_IS_UNKNOWN) ||
    ...
    if (!canBeSQLUnknown(itm, FALSE)) {
      ...
      while (itm->getOperatorType() == ITM_NOT || itm->getOperatorType() == ITM_IS_NULL ||
             itm->getOperatorType() == ITM_IS_NOT_NULL || itm->getOperatorType() == ITM_IS_TRUE ||
             itm->getOperatorType() == ITM_IS_FALSE || itm->getOperatorType() == ITM_IS_UNKNOWN ||
             itm->getOperatorType() == ITM_IS_NOT_UNKNOWN)
        itm = itm->child(0);

      if (!itm->isASubquery() && (aggOK || !itm->isAnAggregate())) {
...
//       recursive code.
//
void UnLogic::transformNode2(NormWA &normWARef, ExprValueId &locationOfPointerToMe, ExprGroupId &introduceSemiJoinHere,
                             const ValueIdSet &externalInputs) {
  DBGSETDBG("TRANSFORM_DEBUG")
  DBGIF(unp = ""; unparse(unp);
        cerr << (int)getOperatorType() << " " << (int)getValueId() << " " << (void *)this << " " << unp << endl;)
  ...
  //
  if (getOperatorType() == ITM_IS_FALSE && !normWARef.inConstraints() &&
      (!normWARef.walkingAnExprTree() || !canBeSQLUnknown(child(0)))) {
  ...
  if (!normWARef.isChildOfANot()) {
    ...
    if (descendant) {
      ...
      if (eliminate)
        ...
        locationOfPointerToMe = new (normWARef.wHeap()) BoolVal(ITM_RETURN_FALSE);
        locationOfPointerToMe->synthTypeAndValueId(TRUE);
      }
      setReplacementExpr(locationOfPointerToMe);
      return;
...
void UnLogic::transformNode(NormWA &normWARef, ExprValueId &locationOfPointerToMe, ExprGroupId &introduceSemiJoinHere,
                            const ValueIdSet &externalInputs) {
  if (nodeIsTransformed()) {
    locationOfPointerToMe = getReplacementExpr();
    return;
  ...
  UnLogic::transformNode2(normWARef, locationOfPointerToMe, introduceSemiJoinHere, externalInputs);
  switch (getOperatorType()) {
    case ITM_NOT:
    case ITM_IS_FALSE: {
      ...
      if (rv != this) {
        ...
        rv->transformNode(normWARef, locationOfPointerToMe, introduceSemiJoinHere, externalInputs);
      } else {
        // this must be a NOT or IS FALSE
        markAsTransformed();
        normWARef.setNotFlag();
        child(0)->getReplacementExpr()->transformNode(normWARef, child(0), introduceSemiJoinHere, externalInputs);
    ...
    case ITM_IS_UNKNOWN:
    case ITM_IS_NOT_UNKNOWN:
      transformIsNull(normWARef, locationOfPointerToMe, introduceSemiJoinHere, externalInputs);
      break;
    case ITM_IS_TRUE: {
      ...
      if (!canBeSQLUnknown(child(0)) || !normWARef.walkingAnExprTree()) {
        ...
        child(0)->getReplacementExpr()->transformNode(normWARef, locationOfPointerToMe, introduceSemiJoinHere,
                                                      externalInputs);
      } else {
        normWARef.setNotFlag();
        ItemExpr::transformNode(normWARef, locationOfPointerToMe, introduceSemiJoinHere, externalInputs);
...
// -----------------------------------------------------------------------
ItemExpr *UnLogic::transformSubtreeOfNot(NormWA &normWARef, OperatorTypeEnum falseOrNot) {
  ItemExpr *newChild = NULL;

  // Only NOT or IS FALSE is processed here
  if (getOperatorType() != ITM_NOT && getOperatorType() != ITM_IS_FALSE) return this;
      ...
      child(0)->getOperatorType() == ITM_OR)  // type queries. The jump table implementation of
  {
    ...
    if (!status) {
      ...
      for (CollIndex i = 0; (leaveAsOrExpr && (i < eqList.entries())); i++) {
        if (eqList[i].getItemExpr()->getOperatorType() != ITM_EQUAL) leaveAsOrExpr = FALSE;

        eqExpr = (BiRelat *)eqList[i].getItemExpr();
        if (i == 0) leftChildId = eqExpr->child(0)->getValueId();
        if (eqExpr->child(0)->getValueId() != leftChildId)
          leaveAsOrExpr = FALSE;  // left side of '=' has different expressions, a = 1 OR b = 1
      }
      if (leaveAsOrExpr) return this;  // do not apply DeMorganRule here as OR will handle them efficiently.
    }
  }
  ...
  if (child(0)->getOperatorType() == ITM_IS_FALSE && !normWARef.inConstraints() && !canBeSQLUnknown(child(0))) {
  ...
  if (child(0)->getOperatorType() == ITM_NOT) {
    ...
    if (falseOrNot == ITM_IS_FALSE) {
      newChild = new (normWARef.wHeap()) UnLogic(ITM_IS_TRUE, newChild);
    }
  } else if (child(0)->getOperatorType() == ITM_IS_NOT_NULL && child(0)->child(0)->getOperatorType() != ITM_ITEM_LIST) {
    // NOT (x) IS NOT NULL  ==> x IS NULL provided x is not a list
    newChild = child(0);
    newChild->setOperatorType(ITM_IS_NULL);
    setChild(0, NULL);
  } else if (child(0)->getOperatorType() == ITM_IS_NULL && child(0)->child(0)->getOperatorType() != ITM_ITEM_LIST) {
  ...
  if (newChild == child(0)) newChild = this;
...
// -----------------------------------------------------------------------
void UnLogic::transformIsNull(NormWA &normWARef, ExprValueId &locationOfPointerToMe, ExprGroupId &introduceSemiJoinHere,
                              const ValueIdSet &externalInputs) {
      ...
      child(0)->getOperatorType() == ITM_NOT_EXISTS)  // i.e. isAnExistsSubquery()
  {
    ...
    if (getOperatorType() == ITM_IS_NOT_UNKNOWN || getOperatorType() == ITM_IS_UNKNOWN) {
      ItemExpr *tf =
          new (normWARef.wHeap()) BoolVal(getOperatorType() == ITM_IS_NOT_UNKNOWN ? ITM_RETURN_TRUE : ITM_RETURN_FALSE);
  ...
  else if (child(0)->getOperatorType() == ITM_ITEM_LIST) {
    /////////////////////////////////////////////////////////////////
    ...
    ItemExprList childList(child(0).getPtr(), normWARef.wHeap(), ITM_ITEM_LIST, FALSE /*don't flatten subquery*/);

    ItemExpr *rootPtr = NULL;
    for (CollIndex i = 0; i < childList.entries(); i++) {
      ItemExpr *unLogPtr = new HEAP UnLogic(getOperatorType(), childList[i]);
      if (!rootPtr)
        rootPtr = unLogPtr;
      else
    ...
    if (saveLocationOfPointerToMe == locationOfPointerToMe && (op == ITM_IS_NOT_NULL || op == ITM_IS_NULL) &&
        !normWARef.inConstraints() && !normWARef.subqUnderExprTree() && !normWARef.isInBeforeTrigger()) {
      ItemExpr *tf = NULL;
      if (!operandType.supportsSQLnullLogical())  // check for non-nullability
        tf = new (normWARef.wHeap()) BoolVal(op == ITM_IS_NOT_NULL ? ITM_RETURN_TRUE : ITM_RETURN_FALSE);
      else if (operand->getOperator() == ITM_CONSTANT &&      // check for
               static_cast<ConstValue *>(operand)->isNull())  //   NULL constant
        tf = new (normWARef.wHeap()) BoolVal(op == ITM_IS_NOT_NULL ? ITM_RETURN_FALSE : ITM_RETURN_TRUE);
      if (tf) {
        getValueId().replaceItemExpr(tf);
        tf->synthTypeAndValueId(TRUE);
        locationOfPointerToMe = tf;
...
// discarding null augmented rows produced by a left join.
// -----------------------------------------------------------------------
NABoolean UnLogic::predicateEliminatesNullAugmentedRows(NormWA &normWARef, ValueIdSet &outerReferences) {
  NABoolean returnValue = FALSE;
  ...
  if (!normWARef.walkingAnExprTree() &&
    ...
    if (child(0)->getOperatorType() == ITM_INSTANTIATE_NULL) {
      ...
      // we have to dig deeper
    } else if (child(0)->predicateEliminatesNullAugmentedRows(normWARef, outerReferences) == TRUE) {
      returnValue = TRUE;
    }
  }
...
ItemExpr *ValueIdUnion::normalizeSpecificChild(NormWA &normWARef, int childIndex) {
  ...
  if (result_ != getValueId()) result_ = ((ItemExpr *)(result_.getItemExpr()->normalizeNode(normWARef)))->getValueId();
  return this;
}  // ValueIdUnion::normalizeSpecificChild()

// No transformation for this node.
void ZZZBinderFunction::transformNode(NormWA &normWARef, ExprValueId &locationOfPointerToMe,
                                      ExprGroupId &introduceSemiJoinHere, const ValueIdSet &externalInputs) {
...
//
void ItmSeqOffset::transformNode(NormWA &normWARef, ExprValueId &locationOfPointerToMe,
                                 ExprGroupId &introduceSemiJoinHere, const ValueIdSet &externalInputs) {
  if (nodeIsTransformed()) {
    locationOfPointerToMe = getReplacementExpr();
    return;
...
void ItmLagOlapFunction::transformNode(NormWA &normWARef, ExprValueId &locationOfPointerToMe,
                                       ExprGroupId &introduceSemiJoinHere, const ValueIdSet &externalInputs) {
  if (nodeIsTransformed()) {
    locationOfPointerToMe = getReplacementExpr();
    return;
...
void ItmLeadOlapFunction::transformNode(NormWA &normWARef, ExprValueId &locationOfPointerToMe,
                                        ExprGroupId &introduceSemiJoinHere, const ValueIdSet &externalInputs) {
  if (nodeIsTransformed()) {
    locationOfPointerToMe = getReplacementExpr();
    return;
...
ItemExpr *ItmSeqDiff1::transformDiff1() {
  ...
  if (getArity() == 2) {
    ...
    if (opType == NA_INTERVAL_TYPE || opType == NA_DATETIME_TYPE) {
      ItemExpr *castExpr =
          new HEAP Cast(tfm2, new HEAP SQLLargeInt(HEAP, TRUE, TRUE));  // (must be) signed; nulls allowed
      tfm2 = castExpr;
    }

    tfm = new HEAP BiArith(ITM_DIVIDE, tfm1, tfm2);
  }  // end getArity == 2
  CMPASSERT(tfm);
  return tfm;
}

void ItmSeqDiff1::transformNode(NormWA &normWARef, ExprValueId &locationOfPointerToMe,
                                ExprGroupId &introduceSemiJoinHere, const ValueIdSet &externalInputs) {
  if (nodeIsTransformed()) {
    locationOfPointerToMe = getReplacementExpr();
    return;
...
void ItmSeqDiff2::transformNode(NormWA &normWARef, ExprValueId &locationOfPointerToMe,
                                ExprGroupId &introduceSemiJoinHere, const ValueIdSet &externalInputs) {
  if (nodeIsTransformed()) {
    locationOfPointerToMe = getReplacementExpr();
    return;
...
// ***********************************************************************
void ItmSeqRunningFunction::transformNode(NormWA &normWARef, ExprValueId &locationOfPointerToMe,
                                          ExprGroupId &introduceSemiJoinHere, const ValueIdSet &externalInputs) {
  ...
  if (op == ITM_RUNNING_SDEV || op == ITM_RUNNING_VARIANCE || op == ITM_RUNNING_AVG || op == ITM_RUNNING_RANK ||
    ...
    switch (getOperatorType()) {
      ...
      case ITM_RUNNING_DRANK: {
        ItemExpr *change = new HEAP ItmSeqRunningFunction(ITM_RUNNING_CHANGE, child(0));
        ((ItmSeqRunningFunction *)change)->setIsOLAP(isOLAP());
        ItemExpr *pred = new HEAP BiRelat(ITM_EQUAL, change, new HEAP SystemLiteral(1));

        tfm = new HEAP IfThenElse(pred, new HEAP SystemLiteral(1), new HEAP SystemLiteral(0));
...
ItemExpr *ItmSeqRunningFunction::transformRunningVariance() {
  ...
  ItemExpr *result = new HEAP ScalarVariance(newOp, sumOfValSquared, sumOfVal, castDouble);
...
ItemExpr *ItmSeqRunningFunction::transformRunningRank() {
  ...
  if (inv && !(isTDFunction() || isOLAP())) {  // Using ASC/DESC with sequence functions is not supported
    *CmpCommon::diags() << DgSqlCode(-4362);
    return this;
  }

  ItemExpr *rcStar = new HEAP ItmSeqRunningFunction(ITM_RUNNING_COUNT, new HEAP SystemLiteral(1));
...
void ItmSeqOlapFunction::transformNode(NormWA &normWARef, ExprValueId &locationOfPointerToMe,
                                       ExprGroupId &introduceSemiJoinHere, const ValueIdSet &externalInputs) {
  ValueId cacheEquivTransSeqId;
  ItemExpr *thisItem = locationOfPointerToMe.getPtr();
  if (CmpCommon::getDefault(COMP_BOOL_201) == DF_ON) {
...
ItemExpr *ItmSeqOlapFunction::transformOlapVariance(CollHeap *wHeap) {
  ...
  ItemExpr *sumOfValSquared = new (wHeap) ItmSeqOlapFunction(ITM_OLAP_SUM, childDoubleSquared);
  ...
  ItemExpr *result = new (wHeap) ScalarVariance(newOp, sumOfValSquared, sumOfVal, castDouble);
...
ItemExpr *ItmSeqOlapFunction::transformOlapAvg(CollHeap *wHeap) {
  ...
  ItemExpr *tfm = new (wHeap) BiArith(ITM_DIVIDE, newSum, newCount);
...
ItemExpr *ItmSeqOlapFunction::transformOlapRank(CollHeap *wHeap) {
  ...
  ItemExpr *rcStar = new (wHeap) ItmSeqRunningFunction(ITM_RUNNING_COUNT, new (wHeap) SystemLiteral(1));
  ((ItmSeqRunningFunction *)rcStar)->setIsOLAP(isOLAP());

  ItemExpr *change = new (wHeap) ItmSeqRunningFunction(ITM_RUNNING_CHANGE, child(0));
  ((ItmSeqRunningFunction *)change)->setIsOLAP(isOLAP());
...
ItemExpr *ItmSeqOlapFunction::transformOlapDRank(CollHeap *wHeap) {
  ...
  ItemExpr *pred = new (wHeap) BiRelat(ITM_EQUAL, change, new (wHeap) SystemLiteral(1));

  ItemExpr *tfm = new (wHeap) IfThenElse(pred, new (wHeap) SystemLiteral(1), new (wHeap) SystemLiteral(0));
...
ItemExpr *ItmSeqOlapFunction::transformOlapFunction(CollHeap *heap) {
  ...
  } else if (getOperatorType() == ITM_OLAP_DRANK) {
      ...
      !isFrameEndUnboundedFollowing())     // frameEnd_ != INT_MAX)
  {
    ...
    ItmSequenceFunction *seqFunc = new (heap) ItmSeqRunningFunction(op, child(0));
    seqFunc->setOLAPInfo(getOlapPartitionBy(), getOlapOrderBy());

    if (frameEnd_ != 0) {
      seqFunc = new (heap) ItmSeqOffset(seqFunc, -frameEnd_, FALSE, TRUE, INT_MAX);
  ...
  } else if (frameStart_ <= frameEnd_ && !isFrameStartUnboundedPreceding() &&  // frameStart_ != -INT_MAX &&
             !isFrameEndUnboundedPreceding())                                  // frameEnd_ != -INT_MAX)
  {
    //-- ROWS BETWEEN  CURRENT ROW          AND  CURRENT ROW
    //-- ROWS BETWEEN  CURRENT ROW          AND  BOUNDED FOLLOWING
    ...
    if (getOperatorType() == ITM_OLAP_MIN || getOperatorType() == ITM_OLAP_MAX) {
      if (frameStart_ < 0 && frameEnd_ > 0) {
        ...
        ItmSeqOlapFunction *precFunc = new (heap) ItmSeqOlapFunction(getOperatorType(), child(0));

        precFunc->setOLAPInfo(getOlapPartitionBy(), getOlapOrderBy());
        precFunc->setOlapWindowFrame(frameStart_, 0);

        ItmSeqOlapFunction *follFunc = new (heap) ItmSeqOlapFunction(getOperatorType(), child(0));  // off);

        follFunc->setOLAPInfo(getOlapPartitionBy(), getOlapOrderBy());
        follFunc->setOlapWindowFrame(0, frameEnd_);

        ItemExpr *minmax = new (heap) ItmScalarMinMax(op, precFunc, follFunc);
        return minmax;
      } else {
        return this;
      }
    ...
    ItmSequenceFunction *newSeq = new (heap) ItmSeqRunningFunction(op, child(0));

    ItmSequenceFunction *newOffset1 = newSeq;

    if ((-frameStart_ + 1) != 0) {
      newOffset1 = new (heap) ItmSeqOffset(newSeq, -frameStart_ + 1, FALSE, FALSE, wSize);
    }

    ItmSequenceFunction *newOffset2 = newSeq;

    if (-frameEnd_ != 0) {
      newOffset2 = new (heap) ItmSeqOffset(newSeq, -frameEnd_, FALSE, TRUE, wSize);
    }

    tfm = new (heap) BiArith(ITM_MINUS, newOffset2, newOffset1);
             ...
             isFrameEndUnboundedFollowing())      // frameEnd_ == INT_MAX)
  {
    ...
    ItmSequenceFunction *seqOff = new (heap) ItmSeqOffset(seqFunc, -frameEnd_ /*+1*/, FALSE, TRUE, INT_MAX);
  ...
  if (tfm) {
    ...
    if (getOperatorType() == ITM_OLAP_SUM) {
      ...
      olapCount->synthTypeAndValueId(
          TRUE);  // valueId/type will be used later when we further transfornm this olap count function
...
///---------------
void ItmSeqRowsSince::transformNode(NormWA &normWARef, ExprValueId &locationOfPointerToMe,
                                    ExprGroupId &introduceSemiJoinHere, const ValueIdSet &externalInputs) {
  DBGSETDBG("TRANSFORM_DEBUG");
  DBGIF(unp = ""; unparse(unp);
        cerr << (int)getOperatorType() << " " << (int)getValueId() << " " << (void *)this << " " << unp << endl;);
  ...
  // ---------------------------------------------------------------------
  BuiltinFunction::transformNode(normWARef, locationOfPointerToMe, introduceSemiJoinHere, externalInputs);
...
// ***********************************************************************
void ItmSeqMovingFunction::transformNode(NormWA &normWARef, ExprValueId &locationOfPointerToMe,
                                         ExprGroupId &introduceSemiJoinHere, const ValueIdSet &externalInputs) {
  ValueId cacheEquivTransSeqId;
  ItemExpr *thisItem = locationOfPointerToMe.getPtr();
  if (CmpCommon::getDefault(COMP_BOOL_201) == DF_ON) {
  ...
  switch (op) {
    ...
    case ITM_MOVING_DRANK: {
      ...
      ItemExpr *change = new HEAP ItmSeqRunningFunction(ITM_RUNNING_CHANGE, child(0));
      ((ItmSeqRunningFunction *)change)->setIsOLAP(isOLAP());
      ItemExpr *pred = new HEAP BiRelat(ITM_EQUAL, change, constOne);

      ItemExpr *rowChanged = new HEAP IfThenElse(pred, constOne, constZero);

      rowChanged = new HEAP Case(NULL, rowChanged);

      ItemExpr *rsum = new HEAP ItmSeqRunningFunction(ITM_RUNNING_SUM, rowChanged);
      ...
      //
      ItemExpr *win = new HEAP BiArith(ITM_MINUS, child(1), constOne);
      ...
      ItemExpr *diff = new HEAP BiArith(ITM_MINUS, rsum, offRsum);

      diff->synthTypeAndValueId(TRUE);
      diff->getValueId().changeType(&getValueId().getType());

      diff = new HEAP BiArith(ITM_PLUS, diff, constOne);
      ...
      // M_DRANK is one
      pred = new HEAP BiRelat(ITM_LESS, win, constOne);
...

{
  ...
  OperatorTypeEnum runningOp = ((op == ITM_MOVING_MIN) ? ITM_RUNNING_MIN : ITM_RUNNING_MAX);
...
ItemExpr *ItmSeqMovingFunction::transformMovingAvg() {
  ...
  ItemExpr *newCount =
      new HEAP ItmSeqMovingFunction(ITM_MOVING_COUNT, child(0), child(1));  // new MOVINGCOUNT(x, size);
...
ItemExpr *ItmSeqMovingFunction::transformMovingSum() {
  ...
  ItemExpr *tfm = new HEAP BiArith(ITM_MINUS, newSum, newOffset);
...
static ItemExpr *createCastNodesonLeaves3(ItemExpr *tfm, NormWA &normref) {
  if (tfm->getOperatorType() != ITM_ITEM_LIST) {
    ...
    InstantiateNull *inst = new (normref.wHeap()) InstantiateNull(tfm, outType);
...
static ItemExpr *createCastNodesonLeaves2(ItemExpr *tfm, NormWA &normref) {
  if (tfm->getOperatorType() == ITM_ONE_ROW) {
    ...
    if (!agr->isOneRowTransformed_) agr->child(0) = createCastNodesonLeaves3(tfm->child(0)->castToItemExpr(), normref);
...
ItemExpr *ItemExpr::removeOneRowAggregate(ItemExpr *tfm, NormWA &normref) {
  if ((tfm->getOperatorType() == ITM_IS_NULL || tfm->getOperatorType() == ITM_IS_NOT_NULL) &&
    ...
    // Convert the child subtree into list WITHOUT FLATTENING SUBQUERIES,
    // so they will be transformed to ONE_ROW aggregates.
    ItemExprList childList(tfm->child(0)->castToItemExpr()->child(0).getPtr(), normref.wHeap(), ITM_ITEM_LIST, FALSE);

    ItemExpr *rootPtr = NULL;
    for (CollIndex i = 0; i < childList.entries(); i++) {
      ItemExpr *unLogPtr = new HEAP UnLogic(tfm->getOperatorType(), childList[i]);
      if (!rootPtr)
        rootPtr = unLogPtr;
      else
        rootPtr = new HEAP BiLogic(ITM_AND, rootPtr, unLogPtr);
    ...
    rootPtr->synthTypeAndValueId(TRUE);
    return rootPtr;
  } else if (tfm->getOperatorType() == ITM_ONE_ROW) {
    CMPASSERT(tfm->getArity() == 1);  // as of 11/25/98
                                      // this can only handle only one child

    if (tfm->child(0)->getOperatorType() != ITM_ITEM_LIST) tfm = tfm->child(0)->castToItemExpr();
...
void BiRelat::translateListOfComparisonsIntoValueIds() {
  if (!listOfComparisonExprs_) return;
...
// scalar expression to a canonical form
// -----------------------------------------------------------------------
void ValueIdProxy::transformNode(NormWA &normWARef, ExprValueId &locationOfPointerToMe,
                                 ExprGroupId &introduceSemiJoinHere, const ValueIdSet &externalInputs) {
  if (nodeIsTransformed()) {
    // Return the address of the expression that was used for replacing
    // this subquery in an earlier call.
  ...
  if (transformDerivedFromValueId_ == TRUE) {
    ...
    expr->transformNode(normWARef, locationOfPointerToMe, introduceSemiJoinHere, externalInputs);
...
// -----------------------------------------------------------------------
// ItmLeadOlapFunction::normalizeNode()
// -----------------------------------------------------------------------
ItemExpr *ItmLeadOlapFunction::normalizeNode(NormWA &normWARef) {
  if (nodeIsNormalized()) return this;

  // check LEAD's second parameter
  // if the value is negatve, then
  // transform LEAD to LAG.
  if (getArity() > 1) {
    ItemExpr *offsetExpr = child(1)->castToItemExpr();
    if (offsetExpr && ITM_MINUS == offsetExpr->getOperatorType()) {
      ...
      if (dummyNegate && cv->canGetExactNumericValue()) {
        ...
        pExpr->setOLAPInfo(getOlapPartitionBy(), getOlapOrderBy());
        setReplacementExpr(pExpr);
        pExpr->synthTypeAndValueId();
        pExpr->normalizeNode(normWARef);
        return pExpr;
      }
    }
  }
  return ItemExpr::normalizeNode(normWARef);
}

// -----------------------------------------------------------------------
// ItmLagOlapFunction::normalizeNode()
// -----------------------------------------------------------------------
ItemExpr *ItmLagOlapFunction::normalizeNode(NormWA &normWARef) {
  if (nodeIsNormalized()) return this;

  // check LAG's second parameter
  // if the value is negatve, then
  // transform LAG to LEAD.
  if (getArity() > 1) {
    ItemExpr *offsetExpr = child(1)->castToItemExpr();
    if (offsetExpr && ITM_MINUS == offsetExpr->getOperatorType()) {
      NABoolean dummyNegate = FALSE;
...
// Called during PhysicalSequence::preCodeGen().
// This method transform ItmOlapPivotGroup to ItmSeqRunningPivotGroup.
// Frames has no effect for OLAP PivotGroup for now.
ItemExpr *ItmOlapPivotGroup::transformOlapFunction(CollHeap *heap) {
  ItemExpr *tfm = NULL;

  OperatorTypeEnum op = mapOperTypeToRunning();
  CMPASSERT(op == ITM_RUNNING_PIVOT_GROUP)

  ItmSequenceFunction *seqFunc =
      new (heap) ItmSeqRunningPivotGroup(op, child(0), getOlapOrderBy(), getPivotDelimiter(), getPivotMaxLen());

  ItmSequenceFunction *seqOff = new (heap) ItmSeqOffset(seqFunc, -INT_MAX, FALSE, TRUE, INT_MAX);
