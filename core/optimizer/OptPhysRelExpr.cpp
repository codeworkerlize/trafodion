...
static DataSourceEnum combineDataSources(DataSourceEnum a, DataSourceEnum b) {
  if (a == b) return a;
  // there is kind of an order on the data source enums, just
  // take the one with the higher order
  if (a == SOURCE_ESP_DEPENDENT OR b == SOURCE_ESP_DEPENDENT) return SOURCE_ESP_DEPENDENT;
  if (a == SOURCE_ESP_INDEPENDENT OR b == SOURCE_ESP_INDEPENDENT) return SOURCE_ESP_INDEPENDENT;
  if (a == SOURCE_PERSISTENT_TABLE OR b == SOURCE_PERSISTENT_TABLE) return SOURCE_PERSISTENT_TABLE;
  if (a == SOURCE_TEMPORARY_TABLE OR b == SOURCE_TEMPORARY_TABLE) return SOURCE_TEMPORARY_TABLE;
  if (a == SOURCE_TRANSIENT_TABLE OR b == SOURCE_TRANSIENT_TABLE) return SOURCE_TRANSIENT_TABLE;
  if (a == SOURCE_VIRTUAL_TABLE OR b == SOURCE_VIRTUAL_TABLE) return SOURCE_VIRTUAL_TABLE;
  return a;
}
...
// -----------------------------------------------------------------------
CostMethod *RelExpr::costMethod() const {
...
// RelExpr::createPlan()
// -----------------------------------------------------------------------
Context *RelExpr::createPlan(Context *myContext, PlanWorkSpace *pws, Rule *rule, Guidance *guidance,
  ...
  if (myContext->getSolution() != NULL AND(limit_for_CPT > 0.0 AND CURRSTMT_OPTDEFAULTS->getTaskCount() >
  ...
  if (isParHeuristic4Feasible(myContext, rppForMe)) return NULL;
  ...
  // ---------------------------------------------------------------------
  if (pws->isEmpty() OR(CURRSTMT_OPTDEFAULTS->optimizerPruning()) AND(pws->getPlanChildCount() == getArity())) {
    ...
    */
    ...
    if ((myContext->isPruningEnabled() AND checkCostLimit)OR(getArity() == 0) OR(getOperatorType() == REL_ROOT)) {
      if (CmpCommon::getDefault(SIMPLE_COST_MODEL) == DF_ON)
        operatorCost = cm->scmComputeOperatorCost(this, pws, countOfStreams);
      else
        operatorCost = cm->computeOperatorCost(this, myContext, countOfStreams);
    } else
  ...
  if (checkCostLimit && (pws->isEmpty() || pws->getPlanChildCount() > 0)) {
    // ------------------------------------------------------------------
    ...
    if ((myContext->getCostLimit()->compareWithCost(*partialPlanCost, rppForMe) == LESS)) {
  ...
  //------------------------------------------------------------------------
  if ((NOT pws->isEmpty() && pws->getPlanChildCount() == 0) || getArity() == 0) {
    ...
    if (cost AND CmpCommon::getDefault(NSK_DBG_SHOW_PLAN_LOG) == DF_ON) {
      ...
      if (costBefore != NULL) costBefore = costBefore->duplicate();
      ...
      if (result AND checkCostLimit) {
        if (result->isCostLimitFeasible())
          ...
          if (CURRSTMT_OPTDEFAULTS->OPHuseFailedPlanCost()) {
            ...
            if (pws->getPlanChildCount() == getArity()) done = TRUE;
          }
  ...
  if (result) {
    ...
    // -----------------------------------------------------------------
    if (rule != NULL)
      guidanceForChild = rule->guidanceForOptimizingChild(guidance, myContext, childIndex);
    else
...
NABoolean RelExpr::isParHeuristic4Feasible(Context *myContext, const ReqdPhysicalProperty *rppForMe) {
  EstLogPropSharedPtr inLogProp = myContext->getInputLogProp();

  if (rppForMe AND CURRSTMT_OPTDEFAULTS->parallelHeuristic4() AND NOT CURRSTMT_OPTDEFAULTS->pushDownDP2Requested()
          AND(getOperatorType() != REL_EXCHANGE) AND((inLogProp->getColStats()).entries() == 0)) {
    NABoolean conditionToAbort = FALSE;
    const PartitioningRequirement *pr = rppForMe->getPartitioningRequirement();
    if (pr) {
      conditionToAbort = NOT((pr->getCountOfPartitions() < 2) OR pr->isRequirementReplicateViaBroadcast()
      ...
      if (lpr) {
        ...
        conditionToAbort =
            NOT((logreq->getCountOfPartitions() < 2) OR logreq->isRequirementReplicateViaBroadcast()
...
Context *RelExpr::createContextForAChild(Context *myContext, PlanWorkSpace *pws, int &childIndex) {
  ...
  if (NOT pws->isEmpty()) {
    ...
    // try one plan.
    // ------------------------------------------------------------------
    if (NOT(childContext AND childContext->hasOptimalSolution())) return NULL;

    if (NOT pws->isLatestContextWithinCostLimit()) return NULL;
  }
...
//
//==============================================================================
CostLimit *RelExpr::computeCostLimit(const Context *myContext, PlanWorkSpace *pws) {
  //----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------
NABoolean RelExpr::findOptimalSolution(Context *myContext, PlanWorkSpace *pws) {
  // ---------------------------------------------------------------------
...
PhysicalProperty *RelExpr::synthPhysicalProperty(const Context *myContext, const int planNumber, PlanWorkSpace *pws) {
  ...
  if (getArity() != 0) {
    ...
    const PhysicalProperty *sppForChild = childContext->getPhysicalPropertyForSolution();
  ...
  // ---------------------------------------------------------------------
  PhysicalProperty *sppForMe = new (CmpCommon::statementHeap()) PhysicalProperty();
...
NABoolean RelExpr::okToAttemptESPParallelism(const Context *myContext, /*IN*/
  ...
  if (isRowsetIterator()) return FALSE;

  NABoolean result = FALSE;

  DefaultToken parallelControlSettings =
      getParallelControlSettings(rppForMe, numOfESPs, allowedDeviation, numOfESPsForced);

  if (parallelControlSettings == DF_OFF) {
    result = FALSE;
  }

  else if ((parallelControlSettings == DF_MAXIMUM) AND CURRSTMT_OPTDEFAULTS->maxParallelismIsFeasible()) {
    ...
    if (CmpCommon::getDefault(COMP_BOOL_61) == DF_OFF) {
      ...
      const CostScalar child0RowCount = (child0OutputLogProp->getResultCardinality()).minCsOne();
      if (child0RowCount.getCeiling() < MINOF(numOfESPs, CURRSTMT_OPTDEFAULTS->numberOfRowsParallelThreshold())) {
    ...
    EstLogPropSharedPtr inLogProp = myContext->getInputLogProp();
    EstLogPropSharedPtr child0OutputLogProp = child(0).outputLogProp(inLogProp);
    CostScalar rowCount = (child0OutputLogProp->getResultCardinality()).minCsOne();
    ...
    if (CmpCommon::getDefault(COMP_BOOL_125) == DF_ON) {
      rowCount += (getGroupAttr()->outputLogProp(inLogProp)->getResultCardinality()).minCsOne();
    }
    ...
    if (rowCount > numberOfRowsThreshold) {
      numOfESPs = rppForMe->getCountOfPipelines();
      allowedDeviation = (float)MAXOF(1.001 - ceil((rowCount / numberOfRowsThreshold).value()) / numOfESPs, 0);
      result = TRUE;
    } else {
      result = FALSE;
    }
...
//==============================================================================
NABoolean RelExpr::rppRequiresEnforcer(const ReqdPhysicalProperty *const rppForMe) const {
  ...
  if ((partReqForMe != NULL)
          AND(partReqForMe->isRequirementReplicateViaBroadcast() OR partReqForMe->isRequirementSkewBusterBroadcast()))
    return TRUE;
  ...
  if (sortOrderTypeReq == ESP_VIA_SORT_SOT) return TRUE;
...
void RelExpr::replacePivs() {
  ...
  const PartitioningFunction *myPartFunc = sppForMe->getPartitioningFunction();
  ...
  for (int childIndex = 0; childIndex < getArity(); childIndex++) {
    ...
    PartitioningFunction *childPartFunc = (PartitioningFunction *)(sppOfChild->getPartitioningFunction());

    CMPASSERT(childPartFunc != NULL);

    const ValueIdSet &childPartKeyPreds = childPartFunc->getPartitioningKeyPredicates();
    const ValueIdList &childPivLayout = childPartFunc->getPartitionInputValuesLayout();
    ...
    if (NOT childPivLayout.isEmpty() AND !(myPivLayout == childPivLayout)) {
      ...
      ValueIdMap pivMap(myPartFunc->getPartitionInputValuesLayout(), childPartFunc->getPartitionInputValuesLayout());
      ...
      CMPASSERT(myPartFunc->getPartitioningFunctionType() == childPartFunc->getPartitioningFunctionType());
      CMPASSERT(myPartFunc->getCountOfPartitions() == childPartFunc->getCountOfPartitions());
      ...
      childPartFunc->replacePivs(myPivLayout, rewrittenChildPartKeyPreds);
...
// ---------------------------------------------------------------------
PartitioningFunction *RelExpr::mapPartitioningFunction(const PartitioningFunction *partFunc,
...
NABoolean Sort::isBigMemoryOperator(const PlanWorkSpace *pws, const int planNumber) {
  ...
  if (spp != NULL) {
    ...
    if (numOfStreams < cpuCount) cpuCount = numOfStreams;
  } else if ((partReq != NULL) AND(partReq->getCountOfPartitions() != ANY_NUMBER_OF_PARTITIONS)) {
  ...
  const double probeCount = MAXOF(1., inLogProp->getResultCardinality().value());

  EstLogPropSharedPtr child0LogProp = child(0).outputLogProp(inLogProp);
  const double child0RowCount = MAXOF(1., child0LogProp->getResultCardinality().value());
  ...
  const double fileSizePerCpu = ((rowsPerCpuPerProbe * (rowLength + 12)) / 1024.);

  if (spp != NULL && CmpCommon::getDefault(COMP_BOOL_51) == DF_ON) {
    CurrentFragmentBigMemoryProperty *bigMemoryProperty =
        new (CmpCommon::statementHeap()) CurrentFragmentBigMemoryProperty();
    ...
    PhysicalProperty *childSpp = (PhysicalProperty *)context->getPhysicalPropertyOfSolutionForChild(0);

    if (childSpp != NULL) {
      CurrentFragmentBigMemoryProperty *memProp =
          (CurrentFragmentBigMemoryProperty *)childSpp->getBigMemoryEstimationProperty();
      if (memProp != NULL) {
...
// to the cost estimation functions for nodes of this type.
// -----------------------------------------------------------------------
CostMethod *Sort::costMethod() const {
...
Context *Sort::createContextForAChild(Context *myContext, PlanWorkSpace *pws, int &childIndex) {
  ...
  if (okToAttemptESPParallelism(myContext, pws, childNumPartsRequirement, childNumPartsAllowedDeviation,
                                numOfESPsForced)) {
    if (NOT numOfESPsForced) rg.makeNumOfPartsFeasible(childNumPartsRequirement, &childNumPartsAllowedDeviation);
    rg.addNumOfPartitions(childNumPartsRequirement, childNumPartsAllowedDeviation);
...
PhysicalProperty *Sort::synthPhysicalProperty(const Context *myContext, const int planNumber, PlanWorkSpace *pws) {
  ...
  // ---------------------------------------------------------------------
  PhysicalProperty *sppTemp = RelExpr::synthPhysicalProperty(myContext, planNumber, pws);
  ...
  PhysicalProperty *sppForMe = new (CmpCommon::statementHeap())
      PhysicalProperty(*(myContext->getPhysicalPropertyOfSolutionForChild(0)), sortKey_, ESP_VIA_SORT_SOT,
...
void Sort::synthPartialSortKeyFromChild(const Context *myContext) {
  ...
  NABoolean parentRequiresOrder = rpp->getSortKey() != NULL;

  // Does parent require some arrangement of columns
  NABoolean parentRequiresArrangement = rpp->getArrangedCols() != NULL;
  ...
  if (parentRequiresArrangement) {
    ...
    ValueIdSet commonElements = parentArrangement.intersect(myChildSortKeyAsASet);
    ...
    if (!commonElements.isEmpty()) {
      ...
      for (UInt32 i = 0; i < childSortKey.entries(); i++) {
        if (commonElements.contains(childSortKey[i].getItemExpr()->removeInverseOrder()->getValueId())) {
          rewriteSortKey = TRUE;
    ...
    if (rewriteSortKey) {
      ...
      mySortkey.subtractSet(mySortkey.intersect(partialSortKey));

      sortKey_ = PartialSortKeyFromChild_.removeInverseOrder();
      for (ValueId x = mySortkey.init(); mySortkey.next(x); mySortkey.advance(x)) {
        sortKey_.insert(x);
      }
    }
    ...
    UInt32 size = childSortKey.entries() > parentSortKey.entries() ? parentSortKey.entries() : childSortKey.entries();
...
void Sort::produceFinalSortKey() {
  ...
  if (NOT arrangedCols_.isEmpty()) {
    ...
    //-------------------------------------------------
    for (ValueId x = arrangedCols_.init(); arrangedCols_.next(x); arrangedCols_.advance(x)) {
      //----------------------------------------------------------------
...
// to the cost estimation functions for nodes of this type.
// -----------------------------------------------------------------------
CostMethod *Exchange::costMethod() const {
...
Context *Exchange::createContextForAChild(Context *myContext, PlanWorkSpace *pws, int &childIndex) {
  ...
  // considered so far.
  // ---------------------------------------------------------------------
  if ((NOT pws->isEmpty())AND(NOT pws->isLatestContextWithinCostLimit())) pws->eraseLatestContextFromWorkSpace();
  ...
  while (planNumber < 2 AND result == NULL) {
    ...
    if (planNumber == 0) {
      ...
      if (((NOT(CURRSTMT_OPTDEFAULTS->parallelHeuristic1() OR myContext->isNiceContextEnabled()))
               OR CURRSTMT_OPTDEFAULTS->pushDownDP2Requested() OR(myContext->getGroupAttr()->getNumBaseTables() == 1))
              AND((partReqForMe == NULL) OR NOT partReqForMe->isRequirementReplicateViaBroadcast())) {
      ...
      if ((rppForMe->getSortOrderTypeReq() == DP2_OR_ESP_NO_SORT_SOT) AND(childContext != NULL)
              AND(childContext->getReqdPhysicalProperty()->getDp2SortOrderPartReq() != NULL)
                  AND NOT childContext->hasOptimalSolution()) {
        ...
        if ((partReqForMe != NULL) AND NOT partReqForMe->castToRequireReplicateNoBroadcast()
                AND NOT rppForMe->getNoEspExchangeRequirement()
                    AND((rppForMe->getCountOfPipelines() > 1)
          ...
          // plan.
          if (NOT(partReqForMe->getCountOfPartitions() <=
                      1 AND childContext AND childContext->hasOptimalSolution() AND rppForMe->getMustMatch() ==
                  NULL AND CmpCommon::getDefault(TRY_DP2_REPARTITION_ALWAYS) == DF_OFF)) {
    ...
    if (tryThisPlan) {
      ...
      rppForChild = new (CmpCommon::statementHeap())
          ReqdPhysicalProperty(*rppForMe, rppForMe->getArrangedCols(), rppForMe->getSortKey(), sortOrderTypeReq,
                               dp2SortOrderPartReq, NULL, plenum);

      // -------------------------------------------------------------
      // Add logical partitioning requirement if the child context
      // is for DP2 and the exchange was given a part requirement
      // -------------------------------------------------------------
      if ((plenum == EXECUTE_IN_DP2) AND(rppForMe->getPartitioningRequirement() != NULL))
        rppForChild->setLogicalPartRequirement(
            new (CmpCommon::statementHeap()) LogicalPartitioningRequirement(rppForMe->getPartitioningRequirement()));
      ...
      // -------------------------------------------------------------
      if ((plenum == EXECUTE_IN_DP2) AND((rppForMe->getArrangedCols() != NULL) OR(rppForMe->getSortKey() != NULL)))
    ...
    if (tryThisPlan AND rppForChild) {
      ...
      // physical properties.
      // -------------------------------------------------------------
      result = shareContext(childIndex, rppForChild, myContext->getInputPhysicalProperty(), costLimit, myContext,
...
// -----------------------------------------------------------------------
ReqdPhysicalProperty *Exchange::processCQS(const ReqdPhysicalProperty *rppForMe, ReqdPhysicalProperty *rppForChild) {
      ...
      if (rppForChild->executeInDP2()) {
        ...
        switch (mm->getWhichLogPart()) {
          ...
          case ExchangeForceWildCard::FORCED_SPLIT:
            logPartType = LogPhysPartitioningFunction::LOGICAL_SUBPARTITIONING;
            break;
          default:
        ...
        if (NOT rppForMe->requiresPartitioning() OR rppForMe->getPartitioningRequirement()
                ->castToRequireExactlyOnePartition()) {
          logPartType = LogPhysPartitioningFunction::ANY_LOGICAL_PARTITIONING;
        }

        if (logPartType != LogPhysPartitioningFunction::ANY_LOGICAL_PARTITIONING OR numClients !=
            ANY_NUMBER_OF_PARTITIONS OR mustUsePapa) {
          ...
          result->setLogicalPartRequirement(new (CmpCommon::statementHeap()) LogicalPartitioningRequirement(
              rppForMe->getPartitioningRequirement(), logPartType, numClients, mustUsePapa, numberOfPAsForced));
        }
        ...
        if (whichType == ExchangeForceWildCard::FORCED_PA OR whichType == ExchangeForceWildCard::FORCED_PAPA)
          return NULL;
...
void Exchange::storePhysPropertiesInNode(const ValueIdList &partialSortKey) {
  ...
  // It is redundant if it enforces neither location nor partitioning or
  // the two fragments connected by this Exchange are of dop of 1. Such
  // case is possible when the dop of the two framgnents are reduced.
  //
  // When an exchange is tasked to merge scan filter inputs, then
  // it is not redundent.
  // ---------------------------------------------------------------------
  isRedundant_ = (sppOfChild->getPlanExecutionLocation() ==
                  sppForMe->getPlanExecutionLocation() AND sppForMe->getPartitioningFunction()->comparePartFuncToFunc(
                      *sppOfChild->getPartitioningFunction()) == SAME AND getScanFilterInputs().entries() == 0) ||
                 (sppForMe->getPartitioningFunction()->getCountOfPartitions() == 1 &&
                  sppOfChild->getPartitioningFunction()->getCountOfPartitions() == 1);
  ...
  if (lpf) isChildSyncAccess = lpf->getSynchronousAccess();
  ...
  if ((CmpCommon::getDefault(ATTEMPT_ASYNCHRONOUS_ACCESS) == DF_OFF || isChildSyncAccess) &&
    ...
    while (ot != REL_SCAN && ot != REL_FILE_SCAN) {
      if (relExpr->getArity() > 1 || relExpr->getOperatorType() == REL_EXCHANGE) {
        noScanFound = TRUE;
        break;
      }
    ...
    FileScan *scan = NULL;
    if (relExpr->getOperatorType() == REL_FILE_SCAN || relExpr->getOperatorType() == REL_SCAN) {
      scan = (FileScan *)relExpr;
    } else {
      noScanFound = TRUE;
    }

    if (!noScanFound && scan->getReverseScan() && scan->getIndexDesc()->isPartitioned()) {
      ValueIdSet externalInputs = scan->getGroupAttr()->getCharacteristicInputs();
      ValueIdSet dummySet;
      ...
      partSearchKey_ = new (CmpCommon::statementHeap())
          SearchKey(scan->getIndexDesc()->getPartitioningKey(), scan->getIndexDesc()->getOrderOfPartitioningKeyValues(),
                    externalInputs, TRUE, selPreds,
                    dummySet,  // not used
                    scan->getIndexDesc());
    }
...
//==============================================================================
PhysicalProperty *Exchange::synthPhysicalPropertyDP2(const Context *myContext) {
  ...
  const LogPhysPartitioningFunction *logPhysChildPartFunc = childPartFunc->castToLogPhysPartitioningFunction();
  ...
  if (logPhysChildPartFunc) {
    ...
    // may be required.
    //
    canMaintainSortOrder = logPhysChildPartFunc->canMaintainSortOrder(sppOfChild->getSortKey());
    ...
    if (CmpCommon::getDefault(COMP_BOOL_82) == DF_ON) {
      myNodeMap = logPhysChildPartFunc->getOrMakeSuitableNodeMap(FALSE /*forDP2*/);
    } else {
    ...
    CMPASSERT(childPartFunc AND childPartFunc->castToSinglePartitionPartitioningFunction());
    ...
    myNodeMap = childPartFunc->getNodeMap()->copy(CmpCommon::statementHeap());
  ...
  if (sppOfChild->getSortKey().isEmpty())
    ...
    if (myContext->requiresOrder()) {
      ...
      // requirement.
      if (usePapa AND NOT usingSynchronousAccess AND((rppForMe->getSortOrderTypeReq() == DP2_SOT) OR(
              rppForMe->getSortOrderTypeReq() == DP2_OR_ESP_NO_SORT_SOT)) AND rppForMe->getDp2SortOrderPartReq()
    ...
    } else  // no required order or arrangement
    {
      ...
      if (((NOT usePapa)OR usingSynchronousAccess)AND canMaintainSortOrder) {
  ...
  return synthPhysicalPropertyFinalize(myContext, myPartFunc, sortOrderType, childPartFunc, sppOfChild, rppForMe,
                                       dp2SortOrderPartFunc);
...
// Helper method for Exchange::synthPhysicalProperty()
//==============================================================================
PhysicalProperty *Exchange::synthPhysicalPropertyESP(const Context *myContext) {
  const PhysicalProperty *sppOfChild = myContext->getPhysicalPropertyOfSolutionForChild(0);
  PartitioningFunction *childPartFunc = sppOfChild->getPartitioningFunction();
  ...
  NABoolean noNeedToCoverChildPartKey =
      myPartReq AND myPartReq->isRequirementFuzzy() AND myPartReq->getPartitioningKey().isEmpty();
  ...
  if (((CmpCommon::getDefault(COMP_BOOL_126) == DF_ON) AND noNeedToCoverChildPartKey)
          OR(childPartFunc->getPartialPartitioningKey().isCovered(newInputs, *(getGroupAttr()), referencedInputs,
                                                                  coveredSubExpr, uncoveredExpr))) {
    softPartRequirement = childPartFunc->makePartitioningRequirement();
  }

  PartitioningFunction *myPartFunc = myPartReq->realize(myContext, FALSE, softPartRequirement);
  ...
  // SkewedDataPartFunc
  if (myPartReq->isRequirementFuzzy() AND myPartFunc->castToSkewedDataPartitioningFunction()) {
    if (NOT(myPartReq->castToFuzzyPartitioningRequirement()->getSkewProperty()).isAnySkew()) {
      CMPASSERT(myPartFunc->castToSkewedDataPartitioningFunction()->getSkewProperty() ==
                myPartReq->castToFuzzyPartitioningRequirement()->getSkewProperty());
    } else {
  ...
  // --------------------------------------------------------------------
  if (CmpCommon::getDefault(COMP_BOOL_82) == DF_ON) {
    myPartFunc->useNodeMapFromReqOrChild(myPartReq, childPartFunc, TRUE /*forESP*/);
  } else {
    if (!(myPartReq->isRequirementFullySpecified() && (CmpCommon::getDefault(COMP_BOOL_87) != DF_ON) &&
      ...
      //
      NodeMap *myNodeMap = ((NodeMap *)(childNodeMap))->synthesizeLogicalMap(myPartitionCount, TRUE /*forESP*/);
  ...
  return synthPhysicalPropertyFinalize(myContext, myPartFunc, sppOfChild->getSortOrderType(), childPartFunc, sppOfChild,
                                       rppForMe, sppOfChild->getDp2SortOrderPartFunc());
...
//
//==============================================================================
PhysicalProperty *Exchange::synthPhysicalPropertyFinalize(const Context *myContext, PartitioningFunction *myPartFunc,
  ...
  NABoolean mustMergeSortedStreamsToPreserveOrder =
      ((sortOrderType != DP2_SOT) AND(((logPhysChildPartFunc != NULL) AND logPhysChildPartFunc->getUsePapa()) OR(
          (logPhysChildPartFunc == NULL) AND(childPartFunc->getCountOfPartitions() > 1))));

  NABoolean mergesSortedStreams = sppOfChild->isSorted() AND(rppForMe->getSortKey() OR rppForMe->getArrangedCols())
                                      AND mustMergeSortedStreamsToPreserveOrder;
  ...
  if (sppOfChild->isSorted()
          AND(NOT mustMergeSortedStreamsToPreserveOrder OR(rppForMe->getSortKey() OR rppForMe->getArrangedCols()))) {
  ...
  NABoolean maintainOrderAfterRepartitioning = (CmpCommon::getDefault(COMP_BOOL_164) == DF_ON);

  PartitioningFunction *childLogPartFunc = childPartFunc;

  if (logPhysChildPartFunc) childLogPartFunc = logPhysChildPartFunc->getLogPartitioningFunction();
  ...
  if (childLogPartFunc &&
      (myPartFunc->getPartitioningFunctionType() == childLogPartFunc->getPartitioningFunctionType()) &&
      myPartFunc->castToHash2PartitioningFunction() && maintainOrderAfterRepartitioning) {
    const Hash2PartitioningFunction *myHash2PartFunc = myPartFunc->castToHash2PartitioningFunction();

    const Hash2PartitioningFunction *childHash2PartFunc = childLogPartFunc->castToHash2PartitioningFunction();

    ValueIdList myPartKeyList = myHash2PartFunc->getKeyColumnList();
    ValueIdList childPartKeyList = childHash2PartFunc->getKeyColumnList();
    ...
    if (myPartKeyList.entries() == childPartKeyList.entries()) {
      ...
      for (CollIndex i = 0; i < myPartKeyList.entries(); i++) {
        if (myPartKeyList[i] != childPartKeyList[i]) hash2RepartitioningWithSameKey_ = FALSE;
  ...
  if (sppOfChild->executeInDP2()) {
    ...
    // -----------------------------------------------------------------
    if (mergesSortedStreams AND((myPartFunc->getCountOfPartitions() > 1 AND sppOfChild->getCountOfPartitions() > 1)
                                    OR sppOfChild->getDataSourceEnum() == SOURCE_ESP_DEPENDENT)
            AND(!hash2RepartitioningWithSameKey_)) {
      mySortKey.clear();
      // If we are not going to synthesize a sort key, then we must
      // make sure we don't synthesize a sort order type or a
      // dp2SortOrderPartFunc.
  ...
  if ((CmpCommon::getDefault(DESTROY_ORDER_AFTER_REPARTITIONING) == DF_ON)
          AND(myPartFunc->getCountOfPartitions() > 1 AND sppOfChild->getCountOfPartitions() == 1)) {
  ...
  // ---------------------------------------------------------------------
  PhysicalProperty *sppForMe = new (CmpCommon::statementHeap())
      PhysicalProperty(mySortKey, sortOrderType, dp2SortOrderPartFunc, myPartFunc, location, dataSource, NULL, NULL);
  ...
  if (sppOfChild->executeInDP2()) sppForMe->setAccessOnePartition(sppOfChild->getAccessOnePartition());
...
// ---------------------------------------------------------------------
JoinForceWildCard::forcedPlanEnum Join::getParallelJoinPlanToEnforce(const ReqdPhysicalProperty *const rppForMe) const {
...
DefaultToken Join::getParallelControlSettings(const ReqdPhysicalProperty *const rppForMe, /*IN*/
  ...
  if (rppForMe->getMustMatch()) {
    JoinForceWildCard *mustMatch = (JoinForceWildCard *)rppForMe->getMustMatch();
  ...
  {
    result = RelExpr::getParallelControlSettings(rppForMe, numOfESPs, allowedDeviation, numOfESPsForced);
...
// -----------------------------------------------------------------------
void Join::splitSortReqsForLeftChild(const ReqdPhysicalProperty *rppForMe, RequirementGenerator &rg,
                                     ValueIdList &reqdOrder1, ValueIdSet &reqdArr1) const {
  CMPASSERT(isNestedJoin()
                OR(getOperatorType() == REL_HYBRID_HASH_JOIN AND((HashJoin *)this)->isOrderedCrossProduct()));
  ...
  if (rppForMe->getSortKey() AND rppForMe->getSortKey()->entries() > 0) {
    ...
    if (splitOrderReq(*(rppForMe->getSortKey()), reqdOrder0, reqdOrder1)) {
      ...
      SortOrderTypeEnum childSortOrderTypeReq = rppForMe->getSortOrderTypeReq();
      PartitioningRequirement *childDp2SortOrderPartReq = rppForMe->getDp2SortOrderPartReq();
      rg.addSortKey(reqdOrder0, childSortOrderTypeReq, childDp2SortOrderPartReq);
    }
  }  // end if required order
  if (rppForMe->getArrangedCols() AND rppForMe->getArrangedCols()->entries() > 0) {
    ValueIdSet reqdArr0;
    if (splitArrangementReq(*(rppForMe->getArrangedCols()), reqdArr0, reqdArr1)) {
      rg.removeArrangement();
      // Sort Order type and dp2SortOrderPartReq might have been
      // removed by the call above, so get them again from the rpp
      SortOrderTypeEnum childSortOrderTypeReq = rppForMe->getSortOrderTypeReq();
      PartitioningRequirement *childDp2SortOrderPartReq = rppForMe->getDp2SortOrderPartReq();
      rg.addArrangement(reqdArr0, childSortOrderTypeReq, childDp2SortOrderPartReq);
    }
...
// -----------------------------------------------------------------------
void Join::splitSortReqsForRightChild(const ReqdPhysicalProperty *rppForMe, RequirementGenerator &rg,
  ...
  if (rppForMe->getSortKey() AND rppForMe->getSortKey()->entries() > 0) {
    ...
    if (splitOrderReq(*(rppForMe->getSortKey()), reqdOrder0, reqdOrder1)) {
      ...
      // to the left child.
      if ((childSortOrderTypeReq == DP2_SOT) OR(childSortOrderTypeReq == DP2_OR_ESP_NO_SORT_SOT)) {
...
// -----------------------------------------------------------------------
CostMethod *NestedJoin::costMethod() const {
...
// -----------------------------------------------------------------------
NABoolean NestedJoin::genLeftChildPartReq(Context *myContext,                            // IN
                                          PlanWorkSpace *pws,                            // IN
                                          ...
                                          PartitioningRequirement *&logicalPartReq)      // OUT
{
  ...
  if (CmpCommon::getDefault(BASE_NUM_PAS_ON_ACTIVE_PARTS) == DF_ON) baseNumPartsOnAP = TRUE;

  NABoolean isUpdateOfHBaseTable = updateTableDesc() && updateTableDesc()->getNATable()->isHbaseTable();

  // adjust plan for hbase bulk load:
  // -use range partitioning for salted tables
  // -use hash on primary key for non salted tables
  NABoolean adjustPFForTrafBulkLoadPrep = isUpdateOfHBaseTable && getIsForTrafLoadPrep() &&
  ...
  NABoolean okToAttemptESPParallelismFlag = okToAttemptESPParallelism(myContext, pws, childNumPartsRequirement,
                                                                      childNumPartsAllowedDeviation, numOfESPsForced);

  ValueIdList pkList;

  // notice if parent requires a particular number of partitions
  if ((partReqForMe != NULL) AND(partReqForMe->getCountOfPartitions() != ANY_NUMBER_OF_PARTITIONS)
          AND !adjustPFForTrafBulkLoadPrep) {
    if (NOT numOfESPsForced) childNumPartsRequirement = partReqForMe->getCountOfPartitions();
    ...
    } else if ((childNumPartsRequirement > physicalPartFunc->getCountOfPartitions()) OR rppForMe->executeInDP2()) {
      // If the user was forcing the number of ESPs, then we
      // cannot honor the request and so we must give up now.
      if (numOfESPsForced) return FALSE;
    ...
    // then give up now.
    if (NOT((childNumPartsRequirement == partReqForMe->getCountOfPartitions())
                OR((partReqForMe->castToRequireApproximatelyNPartitions() != NULL)
                       AND partReqForMe->castToRequireApproximatelyNPartitions()
    ...
    if (isUpdateOfHBaseTable) {
      if (adjustPFForTrafBulkLoadPrep) {
        if (baseNumPartsOnAP && numActivePartitions > 1) {
          ...
          physicalPartFunc =
              new (CmpCommon::statementHeap()) Hash2PartitioningFunction(pkList, pkList, childNumPartsRequirement);
          ...
          physicalPartFunc =
              new (CmpCommon::statementHeap()) Hash2PartitioningFunction(partKey, partKey, childNumPartsRequirement);
        }
      } else
      ...
      if (numOfESPsForced AND(childNumPartsRequirement != 1))
        ...
        childNumPartsRequirement = 1;
    } else if ((baseNumPartsOnAP AND(childNumPartsRequirement > numActivePartitions) AND NOT numOfESPsForced)
                   OR(childNumPartsRequirement > physicalPartFunc->getCountOfPartitions())) {
      // If the user was forcing the number of ESPs, then we
      // cannot honor the request and so we must give up now.
      if (numOfESPsForced)
        return FALSE;
  ...
  if (createPartReqForChild == FALSE)
    ...
    logicalPartReq = new (CmpCommon::statementHeap()) RequireExactlyOnePartition();
  } else {
...
ValueIdList NestedJoin::genWriteOpLeftChildSortReq() {
  ...
  //
  // to solve mantis 13474, add getReqdOrder() as the first order column.
  // if (getReqdOrder().isEmpty())
  {
    NABoolean requireOrderedWrites;
    // QSTUFF
    if (NOT(getGroupAttr()->isStream() || getGroupAttr()->isEmbeddedUpdateOrDelete()))
      // QSTUFF
    ...
    CostScalar noOfProbes = child(0).getGroupAttr()->getResultCardinalityForEmptyInput();
    ...
    if (!(avoidHalloweenR2() OR(getHalloweenForceSort() == FORCED)
              OR isTSJForSideTreeInsert() == TRUE OR enableTransformToSTI() == TRUE)
            AND(noOfProbes < 100 OR child(0).getGroupAttr()->getNumBaseTables() == 0)) {
      ...
      NABoolean underCQS = ActiveControlDB()->getRequiredShape() && ActiveControlDB()->getRequiredShape()->getShape() &&
                           NOT ActiveControlDB()->getRequiredShape()->getShape()->isCutOp();
      if (NOT underCQS) requireOrderedWrites = FALSE;
    }
    // for bulk load we require that data is sorted
    if (updateTableDesc() && updateTableDesc()->getNATable()->isHbaseTable() && getIsForTrafLoadPrep() &&
        !requireOrderedWrites)
      requireOrderedWrites = TRUE;
    ...
    if (requireOrderedWrites) {
      ...
      if (reqdOrder.isEmpty() AND(avoidHalloweenR2() OR(getHalloweenForceSort() == FORCED))) {
        const ValueIdSet &child0Outputs = child(0).getGroupAttr()->getCharacteristicOutputs();

        for (ValueId exprId = child0Outputs.init(); child0Outputs.next(exprId); child0Outputs.advance(exprId)) {
  ...
  // make sure reqdOrder has some column(s) to sort on
  if (reqdOrder.isEmpty() AND updateTableDesc()->getClusteringIndex()->getNAFileSet()->hasSyskey()
          AND(avoidHalloweenR2() OR(getHalloweenForceSort() == FORCED) OR(getHalloweenForceSort() == NOT_FORCED))) {
    reqdOrder = ValueIdList(child(0).getGroupAttr()->getCharacteristicOutputs());
  }
  if (!getReqdOrder().isEmpty()) {
    ValueIdList nestReqdOrder = ValueIdList(getReqdOrder());
    for (UInt32 i = 0; i < reqdOrder.entries(); ++i)
      if (!nestReqdOrder.contains(reqdOrder[i])) nestReqdOrder.insert(reqdOrder[i]);
    nestReqdOrder.removeUnCoveredExprs(child(0).getGroupAttr()->getCharacteristicOutputs());
    reqdOrder = ValueIdList(nestReqdOrder);
  }
...
PartitioningFunction *isOneToOneMatchHashPartitionJoinPossible(NestedJoin *nj, const PartitioningFunction *leftPartFunc,
  ...
  if (leftPartFunc->castToHashDistPartitioningFunction() != NULL) {
    ...
    if (leftHashPartFunc->getCountOfOrigHashPartitions() != leftHashPartFunc->getCountOfPartitions()) {
      return NULL;
    }
  } else {
    ...
    if (leftPartFunc->castToSkewedDataPartitioningFunction()) {
      leftPartFunc = leftPartFunc->castToSkewedDataPartitioningFunction()->getPartialPartitioningFunction();
    }
  ...
  for (CollIndex i = 0; i < availIndexes.entries(); i++) {
    ...
    if (iDesc->isClusteringIndex()) {
      ...
      if (rightPartFunc AND(
              ((allowGrouping ? leftPartFunc->isAGroupingOf(*rightPartFunc)
                              : (leftPartFunc->comparePartFuncToFunc(*rightPartFunc) == SAME)))
                  OR(mappedLeftPartFunc AND(
                      allowGrouping ? mappedLeftPartFunc->isAGroupingOf(*rightPartFunc)
        ...
        if (origLeftHashPartFunc->castToSkewedDataPartitioningFunction()) {
          ...
          result = new (CmpCommon::statementHeap())
              ReplicateNoBroadcastPartitioningFunction(rightPartFunc->getCountOfPartitions());
        } else {
...
// -----------------------------------------------------------------------
ReqdPhysicalProperty *NestedJoin::genRightChildReqs(const PhysicalProperty *sppForChild,   // IN
  ...
  PartitioningFunction *childPartFunc = sppForChild->getPartitioningFunction();
  ...
  // pass this along.
  if ((rppForMe->getSortKey() AND(rppForMe->getSortKey()->entries() > 0))
          OR(rppForMe->getArrangedCols() AND(rppForMe->getArrangedCols()->entries() > 0))) {
    rg.removeSortKey();
    rg.removeArrangement();
  ...
  if (childPartFunc->isASinglePartitionPartitioningFunction()) {
    ...
    if ((npf = isOneToOneMatchHashPartitionJoinPossible(this, childPartFunc, getOriginalEquiJoinExpressions())) &&
      ...
      if (updateTableDesc() && isTSJForWrite()) {
        mappedChildPartFunc = childPartFunc->copyAndRemap(*updateSelectValueIdMap(), TRUE);
      }

      if (!avoidNSquareOpens || updateTableDesc())
        partReqForChild = new (CmpCommon::statementHeap()) RequireReplicateNoBroadcast(mappedChildPartFunc, useNodeMap);
      else
        return NULL;
    }
  }
  ...
  if (sppForChild->getPushDownProperty())
    rg.addPushDownRequirement(sppForChild->getPushDownProperty()->makeRequirement());
...
// -----------------------------------------------------------------------
InputPhysicalProperty *NestedJoin::generateIpp(const PhysicalProperty *sppForChild, NABoolean isPlan0) {
  ...
  ValueIdSet mappedCharOutputs(child(0).getGroupAttr()->getCharacteristicOutputs());
  PartitioningFunction *mappedChildPartFunc = sppForChild->getPartitioningFunction();
  PartitioningFunction *mappedDp2SortOrderPartFunc = sppForChild->getDp2SortOrderPartFunc();

  if (updateTableDesc() != NULL AND NOT mappedLeftChildOrder.isEmpty()) {
        ...
        if (bottomValuesSet == childCharOutput) {
          ...
          if (keySet.entries() == childSortKey.entries()) {
            ...
            if (
              ...
              updateSelectValueIdMap()->rewriteValueIdListUpWithIndex(mappedCharOPs, bottomOutputValues);

              mappedCharOutputs = mappedCharOPs;

              updateSelectValueIdMap()->rewriteValueIdListUpWithIndex(mappedLeftChildOrder, keyFromBottomList);
              ...
              mappedLeftChildOrder = mappedLeftChildTmp.findCommonElementsFromList(targetSortKey);
            }
          }
        }
      }
    ...
    if (!useListInsteadOfSet) {
      ...
      updateSelectValueIdMap()->rewriteValueIdSetUp(mappedCharOutputs, childCharOutput);

      // map the sort key from left to right
      updateSelectValueIdMap()->rewriteValueIdListUp(mappedLeftChildOrder, sppForChild->getSortKey());
    }

    // map the partitioning function from the left to the right
    mappedChildPartFunc = mappedChildPartFunc->copyAndRemap(*updateSelectValueIdMap(), TRUE);
    if (mappedDp2SortOrderPartFunc != NULL) {
      // map the Dp2 sort order part func from the left to the right
      mappedDp2SortOrderPartFunc = mappedDp2SortOrderPartFunc->copyAndRemap(*updateSelectValueIdMap(), TRUE);
    }
  }  // end if this is a base table write op

  if (NOT mappedLeftChildOrder.isEmpty() && NOT isPlan0) {
    ValueIdList *leftChildOrder = new (CmpCommon::statementHeap()) ValueIdList(mappedLeftChildOrder);

    ipp = new (CmpCommon::statementHeap())
        InputPhysicalProperty(mappedCharOutputs, leftChildOrder, mappedChildPartFunc, mappedDp2SortOrderPartFunc, FALSE,
                              sppForChild->getexplodedOcbJoinProperty());
  } else {
    if (NOT isPlan0) return NULL;
...
PartitioningFunction *NestedJoin::getClusteringIndexPartFuncForRightChild() const {
  const IndexDesc *iDesc = getClusteringIndexIndexDescForRightChild();

  if (iDesc) return iDesc->getPartitioningFunction();

  return NULL;
}

const NATable *NestedJoin::getNATableForRightChild() const {
  const SET(IndexDesc *) &availIndexes = child(1).getGroupAttr()->getAvailableBtreeIndexes();
...
const IndexDesc *NestedJoin::getClusteringIndexIndexDescForRightChild() const {
...
NABoolean NestedJoin::checkCompleteSortOrder(const PhysicalProperty *sppForChild0) {
  ...
  mappedTargetTableOrder.removeCoveredExprs(getGroupAttr()->getCharacteristicInputs());

  if (mappedTargetTableOrder.isEmpty()) return FALSE;
  ...
  if (n < mappedTargetTableOrder.entries()) return FALSE;

  if (n > mappedTargetTableOrder.entries()) n = mappedTargetTableOrder.entries();

  for (CollIndex i = 0; i < n; i++) {
    ValueId targetKeyCol = mappedTargetTableOrder[i];
    ValueId outerOrderCol = sppForChild0->getSortKey()[i];

    if (outerOrderCol.getItemExpr()->sameOrder(targetKeyCol.getItemExpr()) != SAME_ORDER) return FALSE;
  }
  ...
  PartitioningFunction *mappedDp2SortOrderPartFunc = sppForChild0->getDp2SortOrderPartFunc();
  if (mappedDp2SortOrderPartFunc != NULL) {
    mappedDp2SortOrderPartFunc = mappedDp2SortOrderPartFunc->copyAndRemap(*updateSelectValueIdMap(), TRUE);
    PartitioningFunction *targetTablePartFunc = updateTableDesc()->getClusteringIndex()->getPartitioningFunction();
    ...
    if ((targetTablePartFunc == NULL)
            OR mappedDp2SortOrderPartFunc->comparePartFuncToFunc(*targetTablePartFunc) != SAME) {
      return FALSE;
    }
  }
...
Context *NestedJoin::createContextForAChild(Context *myContext, PlanWorkSpace *pws, int &childIndex) {
  ...
  if (njPws->isEmpty()) {
    ...
    if (((CURRSTMT_OPTDEFAULTS->preferredProbingOrderForNJ()
              OR((getSource() == Join::STAR_FACT) AND  // star fact join is always ppo
                 (CmpCommon::getDefault(COMP_BOOL_72) != DF_ON))) AND(getGroupAttr()->getNumBaseTables() > 1)
             AND                       // to avoid Index joins
         (updateTableDesc() == NULL))  // read
        OR((CURRSTMT_OPTDEFAULTS->orderedWritesForNJ() OR avoidHalloweenR2()
                OR  // Always want to sort if avoidHalloweenR2
                    getHalloweenForceSort() == FORCED OR CURRSTMT_OPTDEFAULTS->isSideTreeInsert() ==
            TRUE)                            // Always sort if halloween forces
           AND(updateTableDesc() != NULL)))  // write
      childPlansToConsider = 8;              // try plans 2 and 3 for this join

    CostScalar child0card = child(0).getGroupAttr()->getResultCardinalityForEmptyInput();

    const SET(IndexDesc *) &availIndexes1 = child(1).getGroupAttr()->getAvailableBtreeIndexes();

    Cardinality minrows, maxrows;
    child(0).getGroupAttr()->hasCardConstraint(minrows, maxrows);

    CostScalar maxOCBRowsLimit =
        defs.getAsLong(COMP_INT_30) * myContext->getReqdPhysicalProperty()->getCountOfPipelines();
                ...
                OR(                                       // heuristic1
                    ...
                    ((getSource() == Join::STAR_FACT) OR
                     ...
                     ((CmpCommon::getDefault(COMP_BOOL_135) == DF_ON) AND
                      ...
                      (child(0).getGroupAttr()->getNumBaseTables() <= defs.getAsLong(COMP_INT_31)) AND
                      // the size of inner child per probe is greater
                      // than a threshold
                      (getGroupAttr()->getResultCardinalityForEmptyInput() / child0card.minCsOne() *
                       (child(1).getGroupAttr()->getRecordLength() +
                        // this is, in fact, row header size in a message
                        // CQD name is a misnomer.
                        defs.getAsLong(DP2_MESSAGE_HEADER_SIZE_BYTES))) > defs.getAsLong(COMP_INT_32)))
                        AND(child0card <
                            maxOCBRowsLimit AND child(0).getGroupAttr()->getResultMaxCardinalityForEmptyInput() / 10 <
                            maxOCBRowsLimit)) OR(  // heuristic2 -  forced OCB
                                                   // When join predicates do not cover inner table partition key,
                                                   // We do OCB to avoid O(n^2) connections (from Joining ESPs to
                                                   // DP2s).
                    JoinPredicateCoversChild1PartKey() ==
                    FALSE AND availIndexes1.entries() >=
                        1 AND(availIndexes1[0]->getPrimaryTableDesc()->getIndexes()).entries() ==
                    1
    ...
    NABoolean useParallelism = okToAttemptESPParallelism(myContext, pws, childNumPartsRequirement,
                                                         childNumPartsAllowedDeviation, numOfESPsForced);
    ...
    if (  // CmpCommon::getDefault(NESTED_JOINS_OCR) == DF_ON AND
        OCBJoinIsConsidered == FALSE AND updateTableDesc() ==
        NULL /* is read case? */ AND OCRJoinIsFeasible(myContext) == TRUE AND useParallelism) {
    ...
    njPws->setParallelismItems(useParallelism, childNumPartsRequirement, childNumPartsAllowedDeviation,
                               numOfESPsForced);
  ...
  // the right child.
  const InputPhysicalProperty *ippForMyChild = myContext->getInputPhysicalProperty();
  ...
  NABoolean noN2JForRead = (CmpCommon::getDefault(NESTED_JOINS_NO_NSQUARE_OPENS) == DF_ON ||
                            (CmpCommon::getDefault(NESTED_JOINS_NO_NSQUARE_OPENS) == DF_SYSTEM &&
                             !(child(1).getGroupAttr()->allHiveColumnarTables())) &&
                                (updateTableDesc() == NULL));

  NABoolean noEquiN2J = (njPws->getOCRJoinIsConsidered() || njPws->getOCBJoinIsConsidered()) &&
                        (!(isLeftJoin() OR isSemiJoin() OR isAntiSemiJoin())) && (rppForMe->getMustMatch() == NULL) &&
                        getOriginalEquiJoinExpressions().getTopValues().entries() > 0 && noN2JForRead;

  NABoolean trySortPlanInPlan3 = (CmpCommon::getDefault(NESTED_JOINS_PLAN3_TRY_SORT) == DF_ON);

  // if this IUD, but UPD_ORDER is ON then don't try NJ plan 0
  NABoolean shutDownPlan0 = FALSE;
  if ((updateTableDesc() != NULL) && (rppForMe->getMustMatch() == NULL) && (!rppForMe->executeInDP2()) &&
      (!getReqdOrder().entries() || njPws->getFastLoadIntoTrafodion()) && CURRSTMT_OPTDEFAULTS->orderedWritesForNJ())
    shutDownPlan0 = TRUE;

  // if it is NJ into a partitioned hive table, shut down plan0
  const NATable *naTable = getNATableForRightChild();
  if (naTable && naTable->isHiveTable() && naTable->getClusteringIndex()->isPartitioned()) shutDownPlan0 = TRUE;
  ...
  // ---------------------------------------------------------------------
  while ((pws->getCountOfChildContexts() < njPws->getChildPlansToConsider()) AND(rppForChild == NULL)) {
    ...
    if (pws->getPlanChildCount() >= getArity()) pws->resetPlanChildCount();
    ...
    switch (pws->getCountOfChildContexts()) {
      case 0:
        ...
        // -------------------------------------------------------------------
        if (currentPlanIsAcceptable(
                planNumber, rppForMe) AND(NOT njPws->getOCBJoinIsConsidered() OR rppForMe->getMustMatch() !=
                                          NULL OR rppForMe->getSortOrderTypeReq() ==
                                          DP2_OR_ESP_NO_SORT_SOT OR(CmpCommon::getDefault(COMP_BOOL_131) == DF_ON))
                AND(CmpCommon::getDefault(NESTED_JOINS_PLAN0) == DF_ON AND njPws->getOCRJoinIsConsidered() == FALSE)) {
          ...
          if (!(avoidHalloweenR2() || getHalloweenForceSort() == FORCED)) {
            ...
            if (rppForMe->executeInDP2()) {
              if (rppForMe->getPushDownRequirement() == NULL) {
                ...
                rg.addPushDownRequirement(new (CmpCommon::statementHeap()) PushDownColocationRequirement());
              }
            }
            ...
            if (updateTableDesc() != NULL) {
              ...
              if (genLeftChildPartReq(myContext, pws, physicalPartFunc, logicalPartReq)) {
                ...
                if (physicalPartFunc !=
                    NULL AND rppForMe->executeInDP2()
                        AND partReqForMe AND partReqForMe->partReqAndFuncCompatible(physicalPartFunc))
                  rg.removeAllPartitioningRequirements();

                // Map the partitioning requirement partitioning key columns
                // from the right child to the left child.
                if (logicalPartReq) {
                  logicalPartReq = logicalPartReq->copyAndRemap(*updateSelectValueIdMap(), FALSE);
            ...
            else  // read
            {
              ...
              if (njPws->getUseParallelism()
                  ...
                  AND NOT(child0LogExpr AND child0LogExpr->getOperatorType() ==
                          REL_LEAF_INSERT AND((Insert *)child0LogExpr)->insertATuple()))
      ...
      case 1:
        ...
        if (currentPlanIsAcceptable(planNumber, rppForMe)
                AND isTSJForSideTreeInsert() == FALSE AND enableTransformToSTI() == FALSE AND !shutDownPlan0
        ...
        // if OCR is not feasible
        {
          ...
          // considered so far.
          // -----------------------------------------------------------------
          if (pws->getLatestChildContext() AND NOT pws->isLatestContextWithinCostLimit()
                  AND NOT CURRSTMT_OPTDEFAULTS->OPHuseFailedPlanCost())
          ...
          if ((childContext != NULL) AND childContext->hasOptimalSolution()) {
            ...
            if (isRowsetIterator()) {
              ...
              ippForMyChild = new (CmpCommon::statementHeap()) InputPhysicalProperty(NULL, NULL, NULL, NULL, TRUE);

            } else if (updateTableDesc() == NULL && CmpCommon::getDefault(COMP_BOOL_60) == DF_ON) {
              ...
              ippForMyChild = generateIpp(sppForChild, TRUE);
                ...
                if ((rppForChild != NULL) AND(rppForMe->getMustMatch() != NULL)) {
              JoinForceWildCard::forcedPlanEnum forcePlanToken = getParallelJoinPlanToEnforce(rppForMe);
              ...
              if (forcePlanToken == JoinForceWildCard::FORCED_PLAN1 AND njPws->getOCRJoinIsConsidered() == FALSE) {
                // ----------------------------------------------------------------
                // Get the sort order of my left child and pass this information to
                // the context for optimizing my right child.  (Costing the inner
                // table access has a dependency on the order from the outer table).
                ...
                if (ippForMyChild == NULL) ippForMyChild = myContext->getInputPhysicalProperty();
      ...
      case 2:
        ...
        // -------------------------------------------------------------------
        if (currentPlanIsAcceptable(planNumber, rppForMe)
                AND(NOT njPws->getOCBJoinIsConsidered() OR rppForMe->getMustMatch() !=
                    NULL OR rppForMe->getSortOrderTypeReq() ==
                    DP2_OR_ESP_NO_SORT_SOT OR(CmpCommon::getDefault(COMP_BOOL_131) == DF_ON))
                    AND(NOT derivedFromRoutineJoin())
                        AND isTSJForSideTreeInsert() == FALSE AND enableTransformToSTI() == FALSE) {
          ...
          if (!(avoidHalloweenR2() || getHalloweenForceSort() == FORCED)) {
            ...
            // -----------------------------------------------------------------
            if ((childContext != NULL) AND childContext->hasOptimalSolution()) {
              const PhysicalProperty *sppForChild0Plan0 = childContext->getPhysicalPropertyForSolution();
              ...
              if (NOT sppForChild0Plan0->getSortKey().isEmpty()) {
                if (updateTableDesc() != NULL)  // WRITE
                {
                  ...
                  if (sppForChild0Plan0->getDp2SortOrderPartFunc() == NULL)
                    ...
                    rg1.addSortKey(sppForChild0Plan0->getSortKey(), sppForChild0Plan0->getSortOrderType(),
                                   sppForChild0Plan0->getDp2SortOrderPartFunc()->makePartitioningRequirement());
                  ...
                  rg1.addPartRequirement(sppForChild0Plan0->getPartitioningFunction()->makePartitioningRequirement());
                  ...
                  // extra keys if any.
                  if (CmpCommon::getDefault(COMP_BOOL_188) == DF_OFF) rg1.removeExtraSortKeys();
                  ...
                  if (rg1.checkFeasibility()) {
                    ...
                    const ReqdPhysicalProperty *rppForChild1Plan1 = rg1.produceRequirement();
                    ...
                    ValueIdList preferredOrder = child(1).getGroupAttr()->recommendedOrderForNJProbing(
                        child(0).getGroupAttr(), ANY_NUMBER_OF_PARTITIONS, rg2, reqdOrder1, reqdArr1, ppoIDesc,
          ...
          if (OCRJoinIsConsideredInCase2) {
            ...
            if (naTable && getNATableForRightChild()->isHiveTable()) {
              NABoolean isRoundRobin = (ActiveSchemaDB()->getDefaults().getAsLong(USE_ROUND_ROBIN_FOR_RANDOMNUM) == 3);

              ItemExpr *randNum = NULL;
              ValueIdSet partKey;

              if (isRoundRobin) {
                // Produce a requirement of reqApproxN(round robin).
                randNum = new (CmpCommon::statementHeap()) RandomNum(NULL, TRUE);
                randNum->synthTypeAndValueId();
                partKey.insert(randNum->getValueId());
              } else {
                // Produce a requirement of reqApproxN(hash2 on join cols)
                const ValueIdSet &equiJoinExprFromChild0 = getOriginalEquiJoinExpressions().getTopValues();
                partKey.insert(equiJoinExprFromChild0);
              }
              ...
              PartitioningFunction *innerTablePartFunc = getClusteringIndexPartFuncForRightChild();

              ValueIdMap map(getOriginalEquiJoinExpressions());

              // remap the right part func to the left (up)
              PartitioningFunction *rightPartFunc = innerTablePartFunc->copyAndRemap(map, TRUE);
              ...
              if (antiskewSkewEsps > 0) {
                ...
                if (childNodeContainSkew(0, joinPreds, threshold, &skList)) {
                  ...
                  rightPartFunc = new (CmpCommon::statementHeap()) SkewedDataPartitioningFunction(
                      rightPartFunc, skewProperty(skewProperty::UNIFORM_DISTRIBUTE, skList, antiskewSkewEsps));
                }
              }
      ...
      case 3:
        ...
        if (currentPlanIsAcceptable(planNumber, rppForMe)) {
          ...
          if ((childContext != NULL) AND childContext->hasOptimalSolution()) {
            ...
            rppForChild = genRightChildReqs(sppForChild, rppForMe, !njPws->getOCRJoinIsConsidered() && noN2JForRead);
            ...
            if (rppForChild != NULL) {
              if (sppForChild->getSortKey().isEmpty()) {
                ...
                if (njPws->getOCRJoinIsConsidered()) {
                  ...
                  ippForMyChild =
                      new (CmpCommon::statementHeap()) InputPhysicalProperty(mappedCharOutputs, NULL, NULL, NULL, TRUE);
              ...
              if (ippForMyChild) {
                ...
                // get the OCR plan.
                if (njPws->getOCRJoinIsConsidered()) {
                  const PartitioningFunction *njDp2OuterOrderPartFunc = ippForMyChild->getNjDp2OuterOrderPartFunc();

                  PartitioningFunction *innerTablePartFunc = getClusteringIndexPartFuncForRightChild();

                  if ((njDp2OuterOrderPartFunc != NULL)
                          AND(njDp2OuterOrderPartFunc->comparePartFuncToFunc(*innerTablePartFunc) != SAME)) {
                    ippForMyChild = NULL;
                  }
                }
              } else
      ...
      case 4:
        ...
        */
        ...
        if (currentPlanIsAcceptable(planNumber, rppForMe)
                AND(NOT njPws->getOCBJoinIsConsidered() OR rppForMe->getMustMatch() !=
                    NULL OR rppForMe->getSortOrderTypeReq() ==
                    DP2_OR_ESP_NO_SORT_SOT OR(CmpCommon::getDefault(COMP_BOOL_132) == DF_ON))
                    AND(NOT derivedFromRoutineJoin())) {
          // -----------------------------------------------------------------
          ...
          if (rppForMe->getPushDownRequirement() == NULL AND rppForMe->executeInDP2()) {
            // Add a co-location requirement (type-1 join in DP2) if the parent
            // does not provide one, regardless we are in CS or not.
            rg.addPushDownRequirement(new (CmpCommon::statementHeap()) PushDownColocationRequirement());
          }
          ...
          if (updateTableDesc() != NULL)  // WRITE
          {
            ...
            if (preferredOrder.isEmpty() AND !(avoidHalloweenR2() || getHalloweenForceSort() == FORCED)) {
              ...
              if (isTSJForSideTreeInsert() == FALSE AND !shutDownPlan0) return NULL;

              // this is an update case. I leave this now but we might need to
              // review it later because we might want to try another plan. SP.
            }
            physicalPartFunc = updateTableDesc()->getClusteringIndex()->getPartitioningFunction();
          ...
          else  // READ
          {
            ...
            IndexDesc *ppoIDesc = NULL;
            preferredOrder = child(1).getGroupAttr()->recommendedOrderForNJProbing(
                child(0).getGroupAttr(), childNumPartsRequirement, rg, reqdOrder1, reqdArr1, ppoIDesc,
                partKeyColsAreMappable);
            ...
            physicalPartFunc = ppoIDesc->getPartitioningFunction();
            ...
            if ((partReqForMe == NULL) OR NOT partReqForMe->isRequirementReplicateNoBroadcast()) {
              // Generate the partitioning requirements.
              // If the requirements could not be generated because the user
              // is attempting to force something that is not possible,
              // the method returns FALSE. Otherwise, it returns TRUE.
              if (genLeftChildPartReq(myContext, pws, physicalPartFunc, logicalPartReq)) {
                if (logicalPartReq) rg.addPartRequirement(logicalPartReq);
              } else
                // Tried to force something that was not possible. Give up now.
                // return NULL;
                break;
          ...
          JoinForceWildCard::forcedPlanEnum forcePlanToken = JoinForceWildCard::ANY_PLAN;
          ...
          if (rppForMe->getMustMatch() != NULL) {
            ...
            if ((forcePlanToken != JoinForceWildCard::FORCED_PLAN3)
                    AND(rppForMe->getMustMatch()->child(childIndex)->getOperatorType() == REL_SORT))
          ...
          else if ((rppForMe->getSortOrderTypeReq() == ESP_VIA_SORT_SOT)
                       OR(forcePlanToken == JoinForceWildCard::FORCED_PLAN3) OR avoidHalloweenR2()
                           OR getHalloweenForceSort() == FORCED)
            preferredSortOrderTypeReq = ESP_VIA_SORT_SOT;
          else if ((physicalPartFunc == NULL) OR(logicalPartReq == NULL) OR(logicalPartReq->getCountOfPartitions() ==
                                                                            physicalPartFunc->getCountOfPartitions())
                       OR NOT partKeyColsAreMappable OR(CmpCommon::getDefault(ATTEMPT_ASYNCHRONOUS_ACCESS) == DF_OFF))
            preferredSortOrderTypeReq = ESP_NO_SORT_SOT;
          else {
            preferredSortOrderTypeReq = DP2_OR_ESP_NO_SORT_SOT;
            preferredDp2SortOrderPartReq = physicalPartFunc->makePartitioningRequirement();
            if (updateTableDesc() != NULL)  // WRITE
            {
              ...
              preferredDp2SortOrderPartReq =
                  preferredDp2SortOrderPartReq->copyAndRemap(*updateSelectValueIdMap(), FALSE);
            }
          }

          // Now add in a requirement that the data be sorted in the
          // preferred probing order with the preferred sort order type.
          rg.addSortKey(preferredOrder, preferredSortOrderTypeReq, preferredDp2SortOrderPartReq);
        ...
        if (currentPlanIsAcceptable(planNumber, rppForMe)
          ...
          if ((childContext != NULL) AND NOT rppForMe->executeInDP2()) {
            ...
            if (NOT childContext->hasOptimalSolution())
              ...
              const PhysicalProperty *sppForChild0Plan2 = childContext->getPhysicalPropertyForSolution();

              if (trySortPlanInPlan3) {
                // The new way: try the explicit sort plan when plan2 does not produce
                // such a sort plan.
                if (sppForChild0Plan2->getSortOrderType() != ESP_VIA_SORT_SOT) trySortedPlan = TRUE;
              } else {
                ...
                if (sppForChild0Plan2->getSortOrderType() == ESP_NO_SORT_SOT) trySortedPlan = TRUE;
              }
            }
            ...
            if (trySortedPlan AND childContext->getReqdPhysicalProperty()->getSortKey()) {
              RequirementGenerator rg(child(0), rppForMe);
              ...
              const ReqdPhysicalProperty *rppForChild0Plan2 = childContext->getReqdPhysicalProperty();
              PartitioningRequirement *partReqForChild = rppForChild0Plan2->getPartitioningRequirement();
              const ValueIdList *const reqdOrderForChild = rppForChild0Plan2->getSortKey();
              ...
              // have done it!
              CMPASSERT(
                  (partReqForChild != NULL OR(updateTableDesc() AND updateTableDesc()->getNATable()->isHbaseTable()))
                      AND reqdOrderForChild != NULL);
              if (partReqForChild) rg.addPartRequirement(partReqForChild);
      ...
      case 8:
        ...
        // -------------------------------------------------------------------
        if (currentPlanIsAcceptable(planNumber, rppForMe) AND njPws->getOCBJoinIsConsidered()
                AND(NOT derivedFromRoutineJoin())) {
          // -----------------------------------------------------------------
          // Cost limit exceeded? Erase the Context from the workspace.
          // Erasing the Context does not decrement the count of Contexts
          ...
          if (njPws->getUseParallelism()) njPws->transferParallelismReqsToRG(rg);
          if (CmpCommon::getDefault(COMP_BOOL_53) == DF_ON) rg.addOcbCostingRequirement();
          // Produce the requirements and make sure they are feasible.
          if (rg.checkFeasibility()) {
            // Produce the required physical properties.
            rg.addNoEspExchangeRequirement();
        ...
        if (currentPlanIsAcceptable(planNumber, rppForMe) AND njPws->getOCBJoinIsConsidered()) {
          ...
          if ((childContext != NULL) AND childContext->hasOptimalSolution()) {
            ...
            if (CmpCommon::getDefault(COMP_BOOL_82) == DF_ON) {
              ...
              partReqForChild = new (CmpCommon::statementHeap()) RequireReplicateViaBroadcast(childPartFunc, TRUE);
            } else {
              partReqForChild =
                  new (CmpCommon::statementHeap()) RequireReplicateViaBroadcast(childPartFunc->getCountOfPartitions());

              // Use the node map of the inner child partitioning function.
              NodeMap *myNodeMap = childPartFunc->getNodeMap()->copy(CmpCommon::statementHeap());

              partReqForChild->castToFullySpecifiedPartitioningRequirement()->getPartitioningFunction()->replaceNodeMap(
                  myNodeMap);
            }
    ...
    if (rppForChild != NULL) {
      ...
      if (childIndex == 0)
        ...
        inputLPForChild = child(0).outputLogProp(myContext->getInputLogProp());

        if (isSemiJoin() OR isAntiSemiJoin()) {
          // don't alter the estlogprop returned above, alter its copy.
          copyInputLPForChild = EstLogPropSharedPtr(new (CmpCommon::statementHeap()) EstLogProp(*inputLPForChild));
          if (isSemiJoin())
      ...
      if (childIndex == 0 || !isSemiJoin() || !isAntiSemiJoin())
        result = shareContext(childIndex, rppForChild, ippForMyChild, costLimit, myContext, inputLPForChild);
      else
        result = shareContext(childIndex, rppForChild, ippForMyChild, costLimit, myContext, copyInputLPForChild);

      if (NOT(pws->isLatestContextWithinCostLimit() OR result->hasSolution())) result = NULL;
    ...
    if (CURRSTMT_OPTDEFAULTS->optimizerPruning() AND(pws->getPlanChildCount() == getArity())
            AND CURRSTMT_OPTDEFAULTS->OPHexitNJcrContChiLoop()) {
  ...
  if (pws->getCountOfChildContexts() == njPws->getChildPlansToConsider()) pws->setAllChildrenContextsConsidered();
...
NABoolean NestedJoin::currentPlanIsAcceptable(int planNo, const ReqdPhysicalProperty *const rppForMe) const {
  ...
  if (rppForMe->getMustMatch() == NULL) return TRUE;

  // Check for the correct forced plan type.

  JoinForceWildCard::forcedPlanEnum forcePlanToken = getParallelJoinPlanToEnforce(rppForMe);

  NABoolean OCR_CQD_is_on = ActiveSchemaDB()->getDefaults().getAsLong(NESTED_JOINS_OCR_MAXOPEN_THRESHOLD) > 0;

  switch (forcePlanToken) {
    case JoinForceWildCard::FORCED_PLAN0:
      if (planNo != 0) return FALSE;
      break;
    ...
    case JoinForceWildCard::FORCED_TYPE1:
      ...
      if (planNo == 1 AND OCR_CQD_is_on == FALSE) return FALSE;
...
NABoolean NestedJoin::OCBJoinIsFeasible(const Context *myContext) const {
  ...
  if (rppForMe->executeInDP2() == TRUE) return FALSE;
  ...
  if ((myContext->requiresOrder() AND partReq AND partReq->isRequirementExactlyOne() == FALSE)
          OR(partReq AND partReq->isRequirementApproximatelyN() AND NOT partReq->partitioningKeyIsSpecified())) {
    ...
    if (naTable && naTable->isHiveTable()) return FALSE;
    ...
    for (CollIndex i = 0; i < availIndexes.entries(); i++) {
      ...
      if (rightPartFunc) {
        ...
        if (numOfparts == numOfESPs) continue;
...
NABoolean NestedJoin::OCRJoinIsFeasible(const Context *myContext) const {
  ...
  if (NOT((partReq == NULL) OR(partReq->isRequirementApproximatelyN() AND NOT partReq->partitioningKeyIsSpecified())
              OR(partReq->castToRequireHash2() != NULL)))
    return FALSE;

  // Do not consider OCR if the join predicates do not cover part key of the
  // inner (correctness).
  if (JoinPredicateCoversChild1PartKey() == FALSE) return FALSE;

  // If NJ into hive tables, allow OCR anyway.
  const NATable *naTable = getNATableForRightChild();

  if (naTable && naTable->isHiveTable()) return TRUE;
  ...
  if (rightPartFunc->getCountOfPartitions() == 1) return FALSE;

  // No Hive cases resume here.
  // Do not consider OCR if the inner is not hash2 partitioned
  if (rightPartFunc->isAHash2PartitioningFunction() == FALSE) return FALSE;
  ...
  if (CmpCommon::getDefault(NESTED_JOINS_NO_NSQUARE_OPENS) != DF_ON) {
    ...
    if (CURRSTMT_OPTDEFAULTS->getMaximumDegreeOfParallelism() * rightPartFunc->getCountOfPartitions() < threshold)
      return FALSE;
  }
  ...
  if (isProbeCacheApplicable(rppForMe->getPlanExecutionLocation()) == FALSE) {
    ...
    double threshold =
        (ActiveSchemaDB()->getDefaults().getAsDouble(SKEW_SENSITIVITY_THRESHOLD)) / rppForMe->getCountOfPipelines();

    SkewedValueList *skList = NULL;

    if (childNodeContainSkew(0, joinPreds, threshold, &skList) == TRUE) return FALSE;
  }
...
NABoolean NestedJoin::JoinPredicateCoversChild1PartKey() const {
  ...
  if (nt && nt->isHiveTable()) {
    ...
    if (idesc && JoinPredicateCoversChild1PartKey(((IndexDesc *)idesc)->getHivePartCols())) return TRUE;
  }

  PartitioningFunction *rightPartFunc = getClusteringIndexPartFuncForRightChild();

  if (rightPartFunc && JoinPredicateCoversChild1PartKey(rightPartFunc->getPartitioningKey())) return TRUE;
...
NABoolean NestedJoin::JoinPredicateCoversChild1PartKey(const ValueIdSet &child1PartKey) const {
  const ValueIdSet &equiJoinExprFromChild1AsSet = getOriginalEquiJoinExpressions().getBottomValues();

  if (equiJoinExprFromChild1AsSet.contains(child1PartKey)) return TRUE;
  ...
  if (indexCols.contains(child1PartKey)) return TRUE;
  ...
  if (unCoveredPartKey.entries() == 0) return TRUE;
...
NABoolean NestedJoin::okToAttemptESPParallelism(const Context *myContext, /*IN*/
  ...
  if (isTSJForMerge()) {
    if ((isTSJForMergeWithInsert()) && (CmpCommon::getDefault(COMP_BOOL_189) == DF_OFF)) return FALSE;
  }
  ...
  if (parallelControlSettings == DF_OFF) {
    ...
    if (NOT numOfESPsForced) {
      ...
      if (child0RowCount.getCeiling() < numOfESPs) {
        ...
        if (CURRSTMT_OPTDEFAULTS->deviationType2JoinsSystem()) {
          ...
          allowedDeviation = ((float)numOfESPs - 1.99f) / (float)numOfESPs;
        }
    ...
    EstLogPropSharedPtr inLogProp = myContext->getInputLogProp();
    EstLogPropSharedPtr child0OutputLogProp = child(0).outputLogProp(inLogProp);
    CostScalar child0Rows = (child0OutputLogProp->getResultCardinality()).minCsOne();
    ...
    if (updateTableDesc()) {
      ...
      // -----------------------------------------------------------------
      const PartitioningFunction *updPartFunc = updateTableDesc()->getClusteringIndex()->getPartitioningFunction();

      if (updPartFunc == NULL OR updPartFunc->getCountOfPartitions() == 1) {
        numOfESPs = 1;
        ...
        if (CmpCommon::getDefault(COMP_BOOL_30) == DF_ON) {
          const CostScalar rowCount =
              (CmpCommon::getDefault(COMP_BOOL_125) == DF_ON)
                  ? child0Rows + (getGroupAttr()->outputLogProp(inLogProp)->getResultCardinality()).minCsOne()
                  : child0Rows;
...
PhysicalProperty *NestedJoin::synthPhysicalProperty(const Context *myContext, const int planNumber,
  ...
  if (sppOfRightChild->isSorted() AND NOT rowsFromLeftHaveUniqueMatch()
          AND NOT(getGroupAttr()->getMaxNumOfRows() <= 1)) {
    ...
    if (leftGA->isUnique(leftSortCols)) {
      ...
      if (rppForMe->getSortKey() AND(rppForMe->getSortKey()->entries() > 0)) {
        ValueIdList reqdOrder0, reqdOrder1;
        if (splitOrderReq(*(rppForMe->getSortKey()), reqdOrder0, reqdOrder1)) {
          if (NOT reqdOrder0.isEmpty()) leftChildSortColsAreReq = TRUE;
          if (NOT reqdOrder1.isEmpty()) rightChildSortColsAreReq = TRUE;
        }
      }
      if ((NOT(leftChildSortColsAreReq AND rightChildSortColsAreReq))AND rppForMe->getArrangedCols()
              AND(rppForMe->getArrangedCols()->entries() > 0)) {
        ValueIdSet reqdArr0, reqdArr1;
        if (splitArrangementReq(*(rppForMe->getArrangedCols()), reqdArr0, reqdArr1)) {
          if (NOT reqdArr0.isEmpty()) leftChildSortColsAreReq = TRUE;
          if (NOT reqdArr1.isEmpty()) rightChildSortColsAreReq = TRUE;
        }
      }
      ...
      if ((leftGA->getMaxNumOfRows() <= 1) AND NOT newSortKey.isEmpty()) {
      ...
      if (newSortKey.isEmpty()) {
        ...
        if ((sppOfLeftChild->getSortOrderType() == sppOfRightChild->getSortOrderType()) AND

            ((sppOfLeftChild->getDp2SortOrderPartFunc() == sppOfRightChild->getDp2SortOrderPartFunc())
                 OR((sppOfLeftChild->getDp2SortOrderPartFunc() != NULL)
                        AND(sppOfRightChild->getDp2SortOrderPartFunc() != NULL)
                            AND(sppOfLeftChild->getDp2SortOrderPartFunc()->comparePartFuncToFunc(
  ...
  if (canAppendRightColumns AND(
          childSortOrderTypesAreSame OR(rightChildSortColsAreReq AND sppOfRightChild->getSortOrderType() != DP2_SOT))) {
    ...
    if ((sppOfLeftChild->getSortOrderType() == ESP_NO_SORT_SOT)
            AND(sppOfRightChild->getSortOrderType() == ESP_VIA_SORT_SOT))
  ...
  // But if it is forced using CB189, then parallelise it at user's own risk.
  if (isTSJForMerge()) {
    if ((isTSJForMergeWithInsert()) && (CmpCommon::getDefault(COMP_BOOL_189) == DF_OFF)) loc = EXECUTE_IN_MASTER;
  }
  ...
  if (planNumber < 4) {
    if (rppForMe->executeInDP2() && updateTableDesc() && rppForMe->getPushDownRequirement()) {
      // Push down IUD queries involving MVs.
      //
      ...
      sppForMe = new (CmpCommon::statementHeap()) PhysicalProperty(
          newSortKey, newSortOrderType, newDp2SortOrderPartFunc, sppOfRightChild->getPartitioningFunction(), loc,
          combineDataSources(sppOfLeftChild->getDataSourceEnum(), sppOfRightChild->getDataSourceEnum()),
          sppOfRightChild->getIndexDesc(), NULL, sppOfRightChild->getPushDownProperty());
    } else {
      sppForMe = new (CmpCommon::statementHeap()) PhysicalProperty(
          newSortKey, newSortOrderType, newDp2SortOrderPartFunc, sppOfLeftChild->getPartitioningFunction(), loc,
          combineDataSources(sppOfLeftChild->getDataSourceEnum(), sppOfRightChild->getDataSourceEnum()),
          sppOfLeftChild->getIndexDesc(), sppOfLeftChild->getPartSearchKey(), sppOfLeftChild->getPushDownProperty(),
    ...
    sppForMe = new (CmpCommon::statementHeap()) PhysicalProperty(
        newSortKey, newSortOrderType, newDp2SortOrderPartFunc, sppOfRightChild->getPartitioningFunction(), loc,
        combineDataSources(sppOfLeftChild->getDataSourceEnum(), sppOfRightChild->getDataSourceEnum()),
        sppOfRightChild->getIndexDesc(), sppOfRightChild->getPartSearchKey(), sppOfRightChild->getPushDownProperty(),
        expldocbjoin);
  }
...
// ---------------------------------------------------------------------
PartitioningFunction *NestedJoin::mapPartitioningFunction(const PartitioningFunction *partFunc,
                                                          NABoolean rewriteForChild0) {
  ValueIdMap map(getOriginalEquiJoinExpressions());
  PartitioningFunction *newPartFunc = partFunc->copyAndRemap(map, rewriteForChild0);

  SkewedDataPartitioningFunction *oldSKpf = NULL;
  SkewedDataPartitioningFunction *newSKpf = NULL;

  if (rewriteForChild0 ==
      FALSE /* map for child 1 */ AND(
          oldSKpf = (SkewedDataPartitioningFunction *)(partFunc->castToSkewedDataPartitioningFunction()))
          AND(newSKpf = (SkewedDataPartitioningFunction *)(newPartFunc->castToSkewedDataPartitioningFunction()))) {
...
// to the cost estimation functions for nodes of this class.
// -----------------------------------------------------------------------
CostMethod *NestedJoinFlow::costMethod() const {
...
// ---------------------------------------------------------------------
PartitioningFunction *MergeJoin::mapPartitioningFunction(const PartitioningFunction *partFunc,
                                                         NABoolean rewriteForChild0) {
...
// -----------------------------------------------------------------------
NABoolean MergeJoin::parentAndChildPartReqsCompatible(const ReqdPhysicalProperty *const rppForMe) const {
  PartitioningRequirement *partReq = rppForMe->getPartitioningRequirement();
  ...
  if (partReq != NULL) {
    ...
    if (partReq->isRequirementFullySpecified()) {
      ...
      // is the empty set.
      if (NOT(joinLeftPartKey.contains(reqPartKey) OR joinRightPartKey.contains(reqPartKey))) {
        ...
        if (reqPartCount > EXACTLY_ONE_PARTITION) return FALSE;
      }
    } else  // fuzzy requirement
    {
      ...
      if (reqPartKey.entries() > 0) {  // yes
        ...
        if (joinLeftPartKey.isEmpty() AND joinRightPartKey.isEmpty() AND(reqPartCount != ANY_NUMBER_OF_PARTITIONS)
                AND((reqPartCount -
                     (reqPartCount * partReq->castToRequireApproximatelyNPartitions()->getAllowedDeviation())) >
...
void MergeJoin::generateSortOrders(const ValueIdList &ordering, /* input */
  ...
  while (!done && i < (int)ordering.entries()) {
    ...
    if (preds.referencesTheGivenValue(referencedValId, predId)) {
      ...
      if (predExpr->isAnEquiJoinPredicate(child(0).getGroupAttr(), child(1).getGroupAttr(), getGroupAttr(),
                                          leftOrderValId, rightOrderValId, isOrderPreserving)) {
        //
        ...
        const ValueId &simpLeftOrderValId = leftOrderValId.getItemExpr()->simplifyOrderExpr(&order2)->getValueId();
        ...
        if (order1 != order2) {
          InverseOrder *leftInverseExpr = new (CmpCommon::statementHeap()) InverseOrder(leftOrderValId.getItemExpr());
        ...
        // expression that results in an inverse order.
        const ValueId &simpRightOrderValId = rightOrderValId.getItemExpr()->simplifyOrderExpr(&order2)->getValueId();
        // if the order expression and the right side equijoin expression
        // do not have the same order, then generate an INVERSE
        // expression on top of the right side equijoin expression.
        if (order1 != order2) {
          InverseOrder *rightInverseExpr = new (CmpCommon::statementHeap()) InverseOrder(rightOrderValId.getItemExpr());
...
// child first.
// -----------------------------------------------------------------------
void MergeJoin::genRightChildArrangementReq(const ReqdPhysicalProperty *const rppForMe,
  ...
  if (reqOrderExists) {
    ...
    generateSortOrders(*(rppForMe->getSortKey()), getEquiJoinPredicates(), leftChildOrder, rightChildOrder,
                       orderedMJPreds, reqOrderCompletelyCovered);
    ...
    if (orderedMJPreds.entries() > 0) {
      ...
      if (reqOrderCompletelyCovered AND NOT reqArrangementExists) rg.addArrangement(rightJoinColumns);
    }
  }
  ...
  if (reqArrangementExists AND((NOT reqOrderExists)OR(reqOrderExists AND reqOrderCompletelyCovered))) {
    ...
    for (vid = leftJoinColumns.init(); leftJoinColumns.next(vid); leftJoinColumns.advance(vid))
      simpleLeftJoinColumns += vid.getItemExpr()->simplifyOrderExpr()->getValueId();

    for (vid = reqArrangement.init(); reqArrangement.next(vid); reqArrangement.advance(vid))
      simpleReqArrangement += vid.getItemExpr()->simplifyOrderExpr()->getValueId();
    ...
    if (simpleReqArrangement.contains(simpleLeftJoinColumns))
      ...
      for (lvid = leftJoinColumns.init(); leftJoinColumns.next(lvid); leftJoinColumns.advance(lvid)) {
  ...
  if (NOT reqOrderExists AND NOT reqArrangementExists) {
    rg.addArrangement(rightJoinColumns);
  }
...
// to the cost estimation functions for nodes of this class.
// -----------------------------------------------------------------------
CostMethod *MergeJoin::costMethod() const {
...
Context *MergeJoin::createContextForAChild(Context *myContext, PlanWorkSpace *pws, int &childIndex) {
  ...
  if ((child0GA != NULL AND(child0GA->getMaxNumOfRows() <= 1))
          OR(child1GA != NULL AND(child1GA->getMaxNumOfRows() <= 1)))
    return NULL;
  ;
  ...
  if (((rppForMe->getSortKey() != NULL) AND(rppForMe->getSortKey()->entries() >= numJoinCols)) OR(numJoinCols == 1))
  ...
  if (NOT mustTryBothChildrenFirst AND((rppForMe->getCountOfPipelines() == 1)
                                           OR((partReqForMe != NULL) AND partReqForMe->isRequirementFullySpecified())))
  ...
  // invocations of this method.
  // ---------------------------------------------------------------------
  while ((pws->getCountOfChildContexts() < childPlansToConsider) AND(rppForChild == NULL)) {
    // If we stay in this loop because we didn't generate some
    // child contexts, we need to reset the child plan count when
    ...
    // -------------------------------------------------------------------
    if ((pws->getCountOfChildContexts() == 0) AND currentPlanIsAcceptable(planNumber, rppForMe)) {
    ...
    // -------------------------------------------------------------------
    else if ((pws->getCountOfChildContexts() == 1) AND currentPlanIsAcceptable(planNumber, rppForMe)) {
      ValueIdList leftChildOrder;
      ValueIdList rightChildOrder;
      ValueIdList orderedMJPreds;
      ...
      // considered so far.
      // ---------------------------------------------------------------
      if (NOT pws->isLatestContextWithinCostLimit() AND NOT CURRSTMT_OPTDEFAULTS->OPHuseFailedPlanCost())
        pws->eraseLatestContextFromWorkSpace();
      ...
      if ((childContext != NULL) AND childContext->hasOptimalSolution()) {
        ...
        ValueIdMap map(getEquiJoinExpressions());
        PartitioningFunction *childPartFuncRewritten = childPartFunc->copyAndRemap(map, rewriteForChild0);
        PartitioningRequirement *partReqForChild = childPartFuncRewritten->makePartitioningRequirement();
        ...
        // -----------------------------------------------------------
        generateSortOrders(sppForChild->getSortKey(), equiJoinPreds, leftChildOrder, rightChildOrder, orderedMJPreds,
                           completelyCovered);
    ...
    // -------------------------------------------------------------------
    else if ((pws->getCountOfChildContexts() == 2) AND currentPlanIsAcceptable(planNumber, rppForMe)) {
      ValueIdSet joinColumns;
    ...
    // -------------------------------------------------------------------
    else if ((pws->getCountOfChildContexts() == 3) AND currentPlanIsAcceptable(planNumber, rppForMe)) {
      ValueIdList leftChildOrder;
      ValueIdList rightChildOrder;
      ValueIdList orderedMJPreds;
      NABoolean completelyCovered = FALSE;
      ...
      if ((childContext != NULL) AND childContext->hasOptimalSolution()) {
        ...
        if (orderedMJPreds.entries() > 0) {
          ...
          if (myContext->requiresOrder()) rg.makeSortKeyFeasible(feasibleLeftChildOrder);

          if (feasibleLeftChildOrder.isEmpty() && CmpCommon::getDefault(COMP_BOOL_84) == DF_OFF) return NULL;
          ...
          if (feasibleLeftChildOrder.entries() != leftChildOrder.entries()) {
            ...
            generateSortOrders(feasibleLeftChildOrder, equiJoinPreds, leftChildOrder, rightChildOrder, orderedMJPreds,
                               completelyCovered);
    ...
    if (CURRSTMT_OPTDEFAULTS->optimizerPruning() AND(pws->getPlanChildCount() == getArity())
            AND CURRSTMT_OPTDEFAULTS->OPHexitMJcrContChiLoop()) {
      pws->resetAllChildrenContextsConsidered();
      break;
    }
  }  // end while loop

  if (pws->getCountOfChildContexts() == childPlansToConsider) pws->setAllChildrenContextsConsidered();
...
NABoolean MergeJoin::currentPlanIsAcceptable(int planNo, const ReqdPhysicalProperty *const rppForMe) const {
  ...
  if (deadLockIsPossible_) return FALSE;
...
PhysicalProperty *MergeJoin::synthPhysicalProperty(const Context *myContext, const int planNumber, PlanWorkSpace *pws) {
  ...
  if ((CmpCommon::getDefault(MERGE_JOIN_WITH_POSSIBLE_DEADLOCK) == DF_OFF)
          AND(sppOfLeftChild->getSortOrderType() == ESP_NO_SORT_SOT)
              AND(sppOfLeftChild->getDataSourceEnum() == SOURCE_ESP_DEPENDENT)
                  AND(sppOfRightChild->getSortOrderType() == ESP_NO_SORT_SOT)
                      AND(sppOfRightChild->getDataSourceEnum() == SOURCE_ESP_DEPENDENT)) {
  ...
  // ---------------------------------------------------------------------
  PhysicalProperty *sppForMe = new (CmpCommon::statementHeap())
      PhysicalProperty(sppOfLeftChild->getSortKey(), sppOfLeftChild->getSortOrderType(),
                       sppOfLeftChild->getDp2SortOrderPartFunc(), myPartFunc,
                       sppOfLeftChild->getPlanExecutionLocation(),  //||opt synth from both kids
                       combineDataSources(sppOfLeftChild->getDataSourceEnum(), sppOfRightChild->getDataSourceEnum()),
                       sppOfLeftChild->getIndexDesc(), myPartSearchKey);
...
// ---------------------------------------------------------------------
PartitioningFunction *HashJoin::mapPartitioningFunction(const PartitioningFunction *partFunc,
                                                        NABoolean rewriteForChild0) {
  ValueIdMap map(getEquiJoinExpressions());
  PartitioningFunction *newPartFunc = partFunc->copyAndRemap(map, rewriteForChild0);
...
NABoolean HashJoin::isBigMemoryOperatorSetRatio(const Context *context, const int planNumber, double &ratio) {
  ...
  EstLogPropSharedPtr inLogProp = context->getInputLogProp();
  const double probeCount = MAXOF(1., inLogProp->getResultCardinality().value());
  const double innerRowCount = child(1).outputLogProp(inLogProp)->getResultCardinality().value();
  ...
  if (spp != NULL && CmpCommon::getDefault(COMP_BOOL_51) == DF_ON) {
    ...
    for (int i = 0; i <= getArity(); i++) {
      ...
      if (childSpp != NULL) {
        CurrentFragmentBigMemoryProperty *memProp =
            (CurrentFragmentBigMemoryProperty *)((PhysicalProperty *)childSpp)->getBigMemoryEstimationProperty();
  ...
  if (memoryLimitPerCPU < 1) memoryLimitPerCPU = 1;
...
NABoolean HashJoin::isSkewBusterFeasible(SkewedValueList **skList, int countOfPipelines,
  ...
  double threshold = (ActiveSchemaDB()->getDefaults().getAsDouble(SKEW_SENSITIVITY_THRESHOLD)) / countOfPipelines;
  ...
  if (isFullOuterJoin()) return FALSE;

  ValueIdSet joinPreds = getEquiJoinPredicates();

  if (joinPreds.entries() > 1) {
    double mc_threshold =
        (ActiveSchemaDB()->getDefaults().getAsDouble(MC_SKEW_SENSITIVITY_THRESHOLD)) / countOfPipelines;

    if (mc_threshold < 0) return FALSE;

    if (!multiColumnjoinPredOKforSB(joinPreds)) return FALSE;
    ...
    NABoolean ok = CmpCommon::getDefault(HJ_NEW_MCSB_PLAN) == DF_ON &&
                   childNodeContainMultiColumnSkew(0, joinPreds, mc_threshold, countOfPipelines, skList);
    ...
    if (ok)
      ...
      // predicate.
      return childNodeContainMultiColumnSkew(0, joinPreds, mc_threshold, threshold, countOfPipelines, skList,
                                             vidOfEquiPredWithSkew);
  }

  // single column SB
  return (singleColumnjoinPredOKforSB(joinPreds) && childNodeContainSkew(0, joinPreds, threshold, skList));
}
...
// to the cost estimation functions for nodes of this class.
// -----------------------------------------------------------------------
CostMethod *HashJoin::costMethod() const {
...
Context *HashJoin::createContextForAChild(Context *myContext, PlanWorkSpace *pws, int &childIndex) {
  ...
  const CostScalar innerChildSize =
      child(1).getGroupAttr()->getResultCardinalityForEmptyInput() * child(1).getGroupAttr()->getRecordLength();
  ...
  if ((NOT isFullOuterJoin())AND(
          (rppForMe->getCountOfPipelines() == 1) OR getEquiJoinPredicates().isEmpty() OR((partReqForMe != NULL) AND(
              partReqForMe->isRequirementExactlyOne() OR partReqForMe->isRequirementReplicateNoBroadcast()))
              OR(CURRSTMT_OPTDEFAULTS->parallelHeuristic3() AND(pws->getParallelismIsOK() == FALSE))
                  OR(isOrderedCrossProduct() AND myContext->requiresOrder())
                      OR(  // New heuristics to limit repartitioning of small right child.
                          ...
                          ((getSource() == Join::STAR_FILTER_JOIN) OR(CmpCommon::getDefault(COMP_BOOL_68) == DF_ON)) AND
                          ...
                          (innerChildSize < CostScalar(ActiveSchemaDB()->getDefaults().getAsLong(COMP_INT_7)))
                              AND(rppForMe->getMustMatch() == NULL)))  // AND
      )                                                                // (NOT isFullOuterJoin())
  {
  ...
  } else if ((NOT isFullOuterJoin())AND(partReqForMe != NULL) AND partReqForMe->isRequirementFullySpecified()) {
    childPlansToConsider = 4;
  }

  // Force serial plans for full outer join.
  if (isFullOuterJoin() AND CmpCommon::getDefault(COMP_BOOL_197) == DF_ON) childPlansToConsider = 2;
  ...
  while ((pws->getCountOfChildContexts() < childPlansToConsider) AND(rppForChild == NULL)) {
    ...
    if (planNumber == 1 OR planNumber == 2) {
      if (isSkewBusterFeasible(&skList, MAXOF(1, rppForMe->getCountOfPipelines()), vidOfEquiPredWithSkew) == TRUE) {
        considerMixedHashJoin = TRUE;

        if (CmpCommon::getDefault(COMP_BOOL_101) == DF_OFF) considerHashJoinPlan2 = FALSE;
      }
      ...
      if (getIsNotInSubqTransform()) {
        ...
        if (CmpCommon::getDefault(NOT_IN_SKEW_BUSTER_OPTIMIZATION) == DF_ON) {
          if (skList->hasOnlyNonSkewedNull() && skList->getNAType()->getTypeQualifier() != NA_CHARACTER_TYPE)
    ...
    if ((pws->getCountOfChildContexts() == 0) AND currentPlanIsAcceptable(planNumber, rppForMe)) {
      if (NOT CURRSTMT_OPTDEFAULTS->optimizerPruning()
              OR pws->isLatestContextWithinCostLimit() OR NOT CURRSTMT_OPTDEFAULTS->OPHpruneWhenCLExceeded()) {
        RequirementGenerator rg(child(0), rppForMe);
        ...
        } else if (okToAttemptESPParallelism(myContext, pws, childNumPartsRequirement, childNumPartsAllowedDeviation,
                                             numOfESPsForced)) {
          if (NOT numOfESPsForced) rg.makeNumOfPartsFeasible(childNumPartsRequirement, &childNumPartsAllowedDeviation);
          rg.addNumOfPartitions(childNumPartsRequirement, childNumPartsAllowedDeviation);
        }  // end if ok to try parallelism
        else {
          if (CURRSTMT_OPTDEFAULTS->parallelHeuristic3()) pws->setParallelismIsOK(FALSE);
        }
      ...
      if ((childContext != NULL) AND childContext->hasOptimalSolution()) {
        ...
        if (childPartFunc->isASinglePartitionPartitioningFunction())
          ...
          CostScalar innerMaxSize = child(1).getGroupAttr()->getResultMaxCardinalityForEmptyInput() *
                                    child(1).getGroupAttr()->getRecordLength();
          CostScalar outerSize =
              child(0).getGroupAttr()->getResultCardinalityForEmptyInput() * child(0).getGroupAttr()->getRecordLength();

          CostScalar innerSizeMB =
              child(1).getGroupAttr()->getResultCardinalityForEmptyInput() * child(1).getGroupAttr()->getRecordLength();
          innerSizeMB /= 1048576;
          // disallow replicate broadcast only if:
          // 1) we are an optional zigzag join, and
          // 2) inner can be dangerously large, or
          // 3) the broadcasted size is bigger than a threshold and
          //    broadcasting is not the only plan to be tried.
          NABoolean isEquiJoin = FALSE;
          double broadcastInnerCap = ActiveSchemaDB()->getDefaults().getAsDouble(HJ_BROADCAST_INNER_THRESHOLD_MB);

          if (broadcastInnerCap >= 0) {
            ValueIdSet allPreds(selectionPred());
            allPreds += getJoinPred();
            isEquiJoin =
                ((allPreds.isEmpty() && getEquiJoinPredicates().entries() > 0) || getEquiJoinPredicates() == allPreds);
          }

          if ((isJoinForZigZag() && innerMaxSize >= outerSize * childPartFunc->getCountOfPartitions()) ||
              (isEquiJoin && innerSizeMB > broadcastInnerCap && childPlansToConsider > 2) || (broadcastInnerCap < 0)) {
        ...
        // Produce the requirements and make sure they are feasible.
        if (rg.checkFeasibility() && considerReplicateBroadcast) {
          // Produce the required physical properties.
          rppForChild = rg.produceRequirement();
        }
    ...
    // -----------------------------------------------------------------
      ...
      if (NOT CURRSTMT_OPTDEFAULTS->optimizerPruning()
        ...
        if (considerMixedHashJoin == TRUE && getEquiJoinExprFromChild0().entries() > 1) {
          if (vidOfEquiPredWithSkew != NULL_VALUE_ID) {
            ...
            ValueId vref = getEquiJoinExprFromChild0().extractVEGRefForEquiPredicate(vidOfEquiPredWithSkew);
    ...
    // -----------------------------------------------------------------
      ...
      if ((childContext != NULL) AND childContext->hasOptimalSolution()) {
        ...
        PartitioningFunction *childPartFunc = sppForChild->getPartitioningFunction();
        NABoolean rewriteForChild0 = FALSE;
        PartitioningFunction *childPartFuncRewritten = mapPartitioningFunction(childPartFunc, rewriteForChild0);

        RequirementGenerator rg(child(1), rppForMe);

        // ignore single part func case since run-time skew does not exist.
        if (notInLeftChildOptimization && !childPartFuncRewritten->isASinglePartitionPartitioningFunction()) {
        ...
        if (childPartFunc->getPartitioningFunctionType() ==
            childPartFuncRewritten
                ->getPartitioningFunctionType() AND(skpf = childPartFunc->castToSkewedDataPartitioningFunction())
                    AND skpf->getSkewProperty()
                .isUniformDistributed()) {
          const SkewedDataPartitioningFunction *otherSKpf = (SkewedDataPartitioningFunction *)childPartFuncRewritten;

          CMPASSERT(otherSKpf->getPartialPartitioningFunction()
                        ->canHandleSkew() AND otherSKpf->getSkewProperty()
                        .isBroadcasted());
    ...
    // are referenced in the equijoin predicates.
    // -----------------------------------------------------------------
    else if ((pws->getCountOfChildContexts() == 4) AND currentPlanIsAcceptable(planNumber, rppForMe)
                 AND considerHashJoinPlan2 == TRUE) {
      if (NOT CURRSTMT_OPTDEFAULTS->optimizerPruning()
              OR pws->isLatestContextWithinCostLimit() OR NOT CURRSTMT_OPTDEFAULTS->OPHpruneWhenCLExceeded()) {
        RequirementGenerator rg(child(1), rppForMe);
    ...
    // -----------------------------------------------------------------
    else if ((pws->getCountOfChildContexts() == 5) AND currentPlanIsAcceptable(planNumber, rppForMe)
                 AND considerHashJoinPlan2 == TRUE) {
      // -------------------------------------------------------------
    ...
    if (rppForChild != NULL) {
      ...
      if (childIndex == 1 AND isReuse()) {
        ...
        inputLogPropForChild =
            child(1).getGroupAttr()->materializeInputLogProp(myContext->getInputLogProp(), &multipleCalls_);
      ...
      if ((NOT(pws->isLatestContextWithinCostLimit() OR result->hasSolution()))AND(
    ...
    if (CURRSTMT_OPTDEFAULTS->optimizerPruning() AND(pws->getPlanChildCount() == getArity())
            AND CURRSTMT_OPTDEFAULTS->OPHexitHJcrContChiLoop()) {
      pws->resetAllChildrenContextsConsidered();
      break;
    }
  }  // end while loop
...
NABoolean HashJoin::okToAttemptESPParallelism(const Context *myContext, /*IN*/
  ...
  if (parallelControlSettings == DF_OFF) {
    ...
    if (parThreshold.isGreaterThanOne()) {
      numOfESPs = rppForMe->getCountOfPipelines();
      allowedDeviation = (float)MAXOF(1.001 - ceil(parThreshold.value() / numOfESPs), 0);
      result = TRUE;
...
//
//==============================================================================
CostLimit *HashJoin::computeCostLimit(const Context *myContext, PlanWorkSpace *pws) {
  //----------------------------------------------------------------------------
  //  Context contains no cost limit.  Interpret this as an infinite cost limit.
  // Returning NULL indicates an infinite cost limit.
  //----------------------------------------------------------------------------
...
PhysicalProperty *HashJoin::synthPhysicalProperty(const Context *myContext, const int planNumber, PlanWorkSpace *pws) {
  if (isNoOverflow()) {
    CMPASSERT((getOperatorType() == REL_ORDERED_HASH_JOIN) OR(getOperatorType() == REL_ORDERED_HASH_SEMIJOIN) OR(
        getOperatorType() == REL_ORDERED_HASH_ANTI_SEMIJOIN) OR(getOperatorType() == REL_LEFT_ORDERED_HASH_JOIN)
                  OR(getOperatorType() ==
                     REL_HYBRID_HASH_JOIN AND isOrderedCrossProduct()));  // hybrid_hash_join used to implement a
                                                                          // cross product should not overflow.
  } else
    CMPASSERT((getOperatorType() == REL_HYBRID_HASH_JOIN) OR(getOperatorType() == REL_HYBRID_HASH_SEMIJOIN)
                  OR(getOperatorType() == REL_HYBRID_HASH_ANTI_SEMIJOIN)
                      OR(getOperatorType() == REL_LEFT_HYBRID_HASH_JOIN)
                          OR(getOperatorType() == REL_FULL_HYBRID_HASH_JOIN));
  ...
  if (isOrderedCrossProduct()) {
    ...
    if (NOT(canAppendRightColumns AND childSortOrderTypesAreSame)) {
  ...
  if (isNoOverflow()) {
    ...
    sppForMe = new (CmpCommon::statementHeap())
        PhysicalProperty(newSortKey,
                         newSortOrderType,  // no sort key, so no sort order type either
                         newDP2SortOrderPartFunc, myPartFunc,
...
NABoolean Union::rppAreCompatibleWithOperator(const ReqdPhysicalProperty *const rppForMe) const {
  if (rppForMe->executeInDP2()) {
    if (!CURRSTMT_OPTDEFAULTS->pushDownDP2Requested()) return FALSE;

    // no push-down union if the containing CS is not pushed down
    if (isinBlockStmt() == TRUE) {
      if (NOT PushDownCSRequirement::isInstanceOf(rppForMe->getPushDownRequirement())) return FALSE;
    } else {
      // no push-down union if it is not the one introduced
      // for the IUD log table associated with a MV.
      if (getInliningInfo().isUsedForMvLogging() == FALSE) return FALSE;
    }
  }
  ...
  // ---------------------------------------------------------------------
  if (rppForMe->requiresPartitioning() AND NOT rppForMe->getPartitioningRequirement()->getPartitioningKey().isEmpty()
          AND NOT getUnionForIF()) {
    // -----------------------------------------------------------------
    ...
    // -----------------------------------------------------------------
    return partKeyForUnion.contains(rppForMe->getPartitioningRequirement()->getPartitioningKey());
...
// to the cost estimation functions for nodes of this class.
// -----------------------------------------------------------------------
CostMethod *MergeUnion::costMethod() const {
...
// ---------------------------------------------------------------------
PartitioningFunction *MergeUnion::mapPartitioningFunction(const PartitioningFunction *partFunc,
                                                          NABoolean rewriteForChild0) {
...
// -----------------------------------------------------------------------
void MergeUnion::synthUnionSortKeyFromChild(const ValueIdList &childSortKey,  /* input */
  ...
  for (CollIndex j = 0; j < unionSortKey.entries(); j++) {
    noInverseVid = unionSortKey[j].getItemExpr()->removeInverseOrder()->getValueId();
  ...
  while (!done && i < childSortKey.entries()) {
    ...
    sortKeySvid = childSortKey[i].getItemExpr()->simplifyOrderExpr(&order1)->getValueId();
    ...
    if (NOT childSortKeyProcessed.contains(sortKeySvid)) {
      ...
      for (reqVid = reqArrangement.init(); reqArrangement.next(reqVid); reqArrangement.advance(reqVid)) {
        // Get the value id for the simplified form of the required
        // arrangement expression.
        reqSvid = reqVid.getItemExpr()->simplifyOrderExpr(&order2)->getValueId();
        ...
        if (sortKeySvid == reqSvid) {
          ...
          if (NOT reqSortKeyCols.contains(reqVid)) {
            ...
            if (order1 != order2) {
              InverseOrder *inverseExpr = new (CmpCommon::statementHeap()) InverseOrder(reqVid.getItemExpr());
  ...
  for (reqVid = reqArrangementCopy.init(); reqArrangementCopy.next(reqVid); reqArrangementCopy.advance(reqVid)) {
    // Add the required arrangement expression if it is not part
    // of the required sort key.
    if (NOT reqSortKeyCols.contains(reqVid)) {
      unionSortKey.insert(reqVid);
...
NABoolean MergeUnion::finalizeUnionSortKey(const ValueIdList &knownSortKey, int knownSide, const ValueIdSet &arrCols,
  ...
  if (unionSortKey.entries() != arrCols.entries()) return FALSE;

  if (unionSortKey.entries() < arrCols.entries()) {
    CollIndex index = unionSortKey.entries();

    for (ValueId vid = arrCols.init(); arrCols.next(vid); arrCols.advance(vid)) {
      if (NOT simplifiedUnionSortKey.contains(vid)) {
        ...
        ValueId sourceVid = ((ValueIdUnion *)vid.getItemExpr())->getSource(knownSide);
        ...
        if (knownSortKey.contains(sourceVid))  // ASC
        {
          ...
          ItemExpr *inverseCol = new (CmpCommon::statementHeap()) InverseOrder(vid.getItemExpr());
...
Context *MergeUnion::createContextForAChild(Context *myContext, PlanWorkSpace *pws, int &childIndex) {
  ...
  // order.
  if ((rppForMe->getArrangedCols() != NULL) AND(rppForMe->getArrangedCols()->entries() > 1))
    mustTryBothChildrenFirst = TRUE;

  // If we don't need to try two plans (four child plans) for sort
  // purposes and we don't need to try two for parallel purposes,
  // then indicate we will only try one plan (two child plans).
  if (NOT mustTryBothChildrenFirst AND((rppForMe->getCountOfPipelines() == 1) OR((partReqForMe != NULL) AND(
          partReqForMe->isRequirementExactlyOne() OR partReqForMe->isRequirementReplicateNoBroadcast()))))
    childPlansToConsider = 2;
  ...
  while ((pws->getCountOfChildContexts() < childPlansToConsider) AND(rppForChild == NULL)) {
    ...
    if (pws->getCountOfChildContexts() == 0) {
      ...
      if (myContext->requiresOrder()) {
        ...
        if (childSortOrderTypeReq == DP2_OR_ESP_NO_SORT_SOT)
          ...
          NABoolean mapItUp = FALSE;
          dp2SortOrderPartReq = rppForMe->getDp2SortOrderPartReq()->copyAndRemap(getMap(childIndex), mapItUp);
        }
      }
      if (rppForMe->getArrangedCols() != NULL) {
        ValueIdSet newArrangedCols;
        getMap(childIndex).rewriteValueIdSetDown(*rppForMe->getArrangedCols(), newArrangedCols);
        rg.addArrangement(newArrangedCols, childSortOrderTypeReq, dp2SortOrderPartReq);
      }
      if (rppForMe->getSortKey() != NULL) {
        ValueIdList newSortKey;
        getMap(childIndex).rewriteValueIdListDown(*rppForMe->getSortKey(), newSortKey);
        rg.addSortKey(newSortKey, childSortOrderTypeReq, dp2SortOrderPartReq);
      }
      ...
      if (rppForMe->requiresPartitioning()) {
        ...
        NABoolean mapItUp = FALSE;
        PartitioningRequirement *parentPartReq = rppForMe->getPartitioningRequirement();
        PartitioningRequirement *parentPartReqRewritten = parentPartReq->copyAndRemap(getMap(childIndex), mapItUp);
    ...
    // -------------------------------------------------------------------
      ...
      // considered so far.
      // ---------------------------------------------------------------
      if (NOT pws->isLatestContextWithinCostLimit()) pws->eraseLatestContextFromWorkSpace();
      ...
      if ((childContext != NULL) AND childContext->hasOptimalSolution()) {
        ...
        // ---------------------------------------------------------------
        CMPASSERT(sppForChild != NULL);
        PartitioningFunction *leftPartFunc = sppForChild->getPartitioningFunction();
        ...
        PartitioningFunction *unionPartFunc = leftPartFunc->copyAndRemap(getMap(0), mapItUp);
        mapItUp = FALSE;
        PartitioningFunction *rightPartFunc = unionPartFunc->copyAndRemap(getMap(1), mapItUp);
        // Now transform this partitioning function into a partitioning
        // requirement.
        PartitioningRequirement *partReqForRight = rightPartFunc->makePartitioningRequirement();
        ...
        if ((CURRSTMT_OPTDEFAULTS->isSideTreeInsert() == FALSE) && CmpCommon::getDefault(COMP_BOOL_22) == DF_ON &&
          ...
          partReqForRight =
              new (CmpCommon::statementHeap()) RequireApproximatelyNPartitions(0.0, numOfPartitions, TRUE);
        }
        ...
        if (getUnionForIF() && sppForChild->getPushDownProperty()) {
          ...
          if (rppForMe->getArrangedCols() != NULL) {
            ...
            getMap(0).rewriteValueIdSetDown(*rppForMe->getArrangedCols(), reqArrangementForChild);
            ValueIdList leftSortKey;
            // Is there also a required sort key?
            if (rppForMe->getSortKey() != NULL) {
              // Initialize the left child sort key to the required sort key cols.
              getMap(0).rewriteValueIdListDown(*rppForMe->getSortKey(), leftSortKey);
            }
            synthUnionSortKeyFromChild(sppForChild->getSortKey(), reqArrangementForChild, leftSortKey);

            ValueIdList unionSortKey;
            // map left child sort key cols to their output ValueIdUnion
            // equivalents
            getMap(0).rewriteValueIdListUp(unionSortKey, leftSortKey);
            ...
            if (finalizeUnionSortKey(leftSortKey, 0, *rppForMe->getArrangedCols(), unionSortKey) == FALSE) return NULL;
            ;
            ...
            if (unionSortKey.entries() > 0)  // is usually true
            {
              ...
              if (alternateRightChildOrderExpr().isEmpty()) {
                ...
                // child ValueIds
                getMap(childIndex).rewriteValueIdListDown(unionSortKey, rightSortKey);
              } else {
                // MV specific optimization refer to MVRefreshBuilder.cpp
                // MultiTxnMavBuilder::buildUnionBetweenRangeAndIudBlocks()
                getMap(childIndex).rewriteValueIdListDown(alternateRightChildOrderExpr(), rightSortKey);
              }
              ...
              rg.addSortKey(rightSortKey, childSortOrderTypeReq, dp2SortOrderPartReq);
            }
          ...
          // the required order into the valueIds for the right child.
            ...
            if (alternateRightChildOrderExpr().isEmpty()) {
              ...
              // MV specific optimization refer to MVRefreshBuilder.cpp
              // MultiTxnMavBuilder::buildUnionBetweenRangeAndIudBlocks()
              getMap(childIndex).rewriteValueIdListDown(alternateRightChildOrderExpr(), newSortKey);
            }
            rg.addSortKey(newSortKey, childSortOrderTypeReq, dp2SortOrderPartReq);
          }
        }
    ...
    // -------------------------------------------------------------------
      ...
      if ((childContext != NULL) AND childContext->hasOptimalSolution()) {
        ...
        // ---------------------------------------------------------------
        CMPASSERT(sppForChild != NULL);
        PartitioningFunction *rightPartFunc = sppForChild->getPartitioningFunction();
        // Take the synthesized partitioning function from the right
        // child, translate it to be in terms of the ValueIdUnions of
        // the union, then translate this to be in terms of the valueids
        // of the left child.
        NABoolean mapItUp = TRUE;
        PartitioningFunction *unionPartFunc = rightPartFunc->copyAndRemap(getMap(1), mapItUp);
        mapItUp = FALSE;
        PartitioningFunction *leftPartFunc = unionPartFunc->copyAndRemap(getMap(0), mapItUp);
        // Now translate the partitioning function into a requirement
        PartitioningRequirement *partReqForLeft = leftPartFunc->makePartitioningRequirement();
        ...
        if (rppForMe->getArrangedCols() != NULL) {
          ...
          // (Taking inverse nodes into account, of course).
          ValueIdSet reqArrangementForChild;
          getMap(1).rewriteValueIdSetDown(*rppForMe->getArrangedCols(), reqArrangementForChild);
          ValueIdList rightSortKey;
          // Is there also a required sort key?
          if (rppForMe->getSortKey() != NULL) {
            // Initialize the right child sort key to the required sort key cols.
            getMap(1).rewriteValueIdListDown(*rppForMe->getSortKey(), rightSortKey);
          }
          synthUnionSortKeyFromChild(sppForChild->getSortKey(), reqArrangementForChild, rightSortKey);

          ValueIdList unionSortKey;
          // map right child sort key cols to their output ValueIdUnion
          // equivalents
          getMap(1).rewriteValueIdListUp(unionSortKey, rightSortKey);

          // Temporary fix - if the sorting order is different than the arrangement
          // force the sort above Union - See comment in finalizeUnionSortKey
          if (finalizeUnionSortKey(rightSortKey, 1, *rppForMe->getArrangedCols(), unionSortKey) == FALSE) return NULL;
          ...
          if (unionSortKey.entries() > 0)  // is usually true
          {
            ...
            // child ValueIds
            getMap(childIndex).rewriteValueIdListDown(unionSortKey, leftSortKey);
            ...
            rg.addSortKey(leftSortKey, childSortOrderTypeReq, dp2SortOrderPartReq);
          }
...
NABoolean MergeUnion::findOptimalSolution(Context *myContext, PlanWorkSpace *pws) {
  ...
  if (hasOptSol) {
    ...
    // be true.
    if (myOptimalSPP != NULL) setSortOrder(myOptimalSPP->getSortKey());
  }
...
PhysicalProperty *MergeUnion::synthPhysicalProperty(const Context *myContext, const int planNumber,
  ...
  if (rppForMe->getArrangedCols() != NULL) {
    ...
    if (planNumber == 0) {
      ...
      synthUnionSortKeyFromChild(sppOfLeftChild->getSortKey(), reqArrangementForChild, leftSortKey);
      ...
      synthUnionSortKeyFromChild(sppOfRightChild->getSortKey(), reqArrangementForChild, rightSortKey);
  ...
  } else  // neither child uses random partitioning.
  {
    ...
    NABoolean comparable =
        (leftPartFunc->comparePartFuncToFunc(*rightPartFunc) == SAME) OR(pdreq && pdreq->castToPushDownCSRequirement());
    ...
    } else  // union children have incompatible partitioning
    {
      ...
      myPartFunc = new (CmpCommon::statementHeap()) Hash2PartitioningFunction(
          partKey, partKey, leftPartFunc->getCountOfPartitions(), const_cast<NodeMap *>(leftPartFunc->getNodeMap()));
  ...
  if (NOT sortOrder_.isEmpty()) {
    ...
    NABoolean leftChildNotOrdered = sppOfLeftChild->getSortOrderType() ==
                                    NO_SOT OR NOT myContext->getPlan()->getContextForChild(0)->requiresOrder();
    NABoolean rightChildNotOrdered = sppOfRightChild->getSortOrderType() ==
                                     NO_SOT OR NOT myContext->getPlan()->getContextForChild(1)->requiresOrder();
    ...
    if (leftChildNotOrdered AND rightChildNotOrdered)
    ...
    else if (sppOfLeftChild->getSortOrderType() != sppOfRightChild->getSortOrderType()) {
      ...
      CMPASSERT(((sppOfLeftChild->getSortOrderType() == ESP_NO_SORT_SOT)
                     OR(sppOfLeftChild->getSortOrderType() == ESP_VIA_SORT_SOT))
                    AND((sppOfRightChild->getSortOrderType() == ESP_NO_SORT_SOT)
                            OR(sppOfRightChild->getSortOrderType() == ESP_VIA_SORT_SOT)));
      ...
      if (sppOfLeftChild->getDp2SortOrderPartFunc() != NULL) {
        NABoolean mapItUp = TRUE;
        unionDp2SortOrderPartFunc = sppOfLeftChild->getDp2SortOrderPartFunc()->copyAndRemap(getMap(0), mapItUp);
      }
    }
  ...
  if (alternateRightChildOrderExpr().isEmpty()) {
    ...
    // map left child sort key cols to their output ValueIdUnion equivalents
    getMap(0).rewriteValueIdListUp(unionSortKey, sppOfLeftChild->getSortKey());
  }

  PhysicalProperty *sppForMe = new (CmpCommon::statementHeap())
      PhysicalProperty(unionSortKey, unionSortOrderType, unionDp2SortOrderPartFunc, myPartFunc,
                       sppOfLeftChild->getPlanExecutionLocation(),  //||opt
                                                                    // synth from both child props
                       combineDataSources(sppOfLeftChild->getDataSourceEnum(), sppOfRightChild->getDataSourceEnum()),
                       sppOfLeftChild->getIndexDesc(), myPartSearchKey, sppOfLeftChild->getPushDownProperty());
...
// those rules.
// -----------------------------------------------------------------------
NABoolean GroupByAgg::rppAreCompatibleWithOperator(const ReqdPhysicalProperty *const rppForMe) const {
  PartitioningRequirement *partReq = rppForMe->getPartitioningRequirement();
  // ---------------------------------------------------------------------
  ...
  if (rppForMe->executeInDP2()) {
    ...
    if (isinBlockStmt()) {
      ...
      if (pdr == NULL OR NOT PushDownCSRequirement::isInstanceOf(pdr)) return FALSE;
    }

    // we only want leaf groupbys to execute in DP2
    if (isAPartialGroupByRoot() OR isAPartialGroupByNonLeaf()) return FALSE;
  } else {
    ...
    NABoolean partialGBLeafInESPOK =
        (rppForMe->getCountOfPipelines() > 1) AND(partReq == NULL) AND NOT gbAggPushedBelowTSJ();

    // we don't want leaf groupbys to execute outside of DP2
    if (isAPartialGroupByLeaf() AND NOT partialGBLeafInESPOK) return FALSE;  // partial GB leaf in ESP not allowed
  }
  ...
  // ---------------------------------------------------------------------
  if (isAPartialGroupByNonLeaf() AND(numberOfPipelines == 1)) return FALSE;
  ...
  if (partReq != NULL) {
    if (partReq->isRequirementFullySpecified()) {
      if (groupExpr().isEmpty()) {
        ...
        if (NOT(partReq->isRequirementExactlyOne() OR partReq->isRequirementReplicateNoBroadcast())) return FALSE;
      } else {
        // If there is a group by expression, then the required
        // partitioning key columns must be a subset of the gb cols.
        // Contains will always return TRUE if the required part key
        // is the empty set.
        if (NOT groupExpr().contains(partReq->getPartitioningKey())) return FALSE;
      }
    } else  // fuzzy requirement
    {
      CMPASSERT(partReq->isRequirementApproximatelyN());
      ...
      if (groupExpr().isEmpty()) {
        ...
        if (myPartKey.isEmpty() AND(reqPartCount != ANY_NUMBER_OF_PARTITIONS)
                AND((reqPartCount -
                     (reqPartCount * partReq->castToRequireApproximatelyNPartitions()->getAllowedDeviation())) >
                    EXACTLY_ONE_PARTITION))
          return FALSE;
...
Context *GroupByAgg::createContextForAChild(Context *myContext, PlanWorkSpace *pws, int &childIndex) {
  ...
  if ((isNotAPartialGroupBy() OR isAPartialGroupByRoot())) {
    if (NOT rppForMe->executeInDP2() OR NOT isinBlockStmt() OR((NOT groupExpr().isEmpty())AND(
            (rppForMe->getCountOfPipelines() > 1) OR(CmpCommon::getDefault(COMP_BOOL_36) == DF_OFF))))
  ...
  if (isRollup()) {
    ...
    rg.addNumOfPartitions(1);
             ...
             okToAttemptESPParallelism(myContext, pws, childNumPartsRequirement, childNumPartsAllowedDeviation,
                                       numOfESPsForced) AND NOT rppForMe->executeInDP2() AND NOT groupExpr()
                 .isEmpty()) {
    if (NOT numOfESPsForced) rg.makeNumOfPartsFeasible(childNumPartsRequirement, &childNumPartsAllowedDeviation);
    rg.addNumOfPartitions(childNumPartsRequirement, childNumPartsAllowedDeviation);
  ...
  if (!parallelAggrPushdown() && isFeasibleToTransformForAggrPushdown() == GroupByAgg::TVL_TRUE)
    rg.addNoEspExchangeRequirement();
  ...
  // group.
  if ((CmpCommon::getDefault(COMP_BOOL_33) == DF_ON) AND(result->getReqdPhysicalProperty()->getCountOfPipelines() > 1)
          AND(NOT groupExpr().isEmpty()) AND(isNotAPartialGroupBy() OR isAPartialGroupByRoot())
              AND(*CURRSTMT_OPTGLOBALS->memo)[myContext->getGroupId()]
                  ->isBelowRoot()) {
    result->setNice(TRUE);
  }
...
//<pb>
void GroupByAgg::addArrangementAndOrderRequirements(RequirementGenerator &rg) {
...
NABoolean GroupByAgg::okToAttemptESPParallelism(const Context *myContext, /*IN*/
  ...
  return RelExpr::okToAttemptESPParallelism(myContext, pws, numOfESPs, allowedDeviation, numOfESPsForced);
}
...
// to the cost estimation functions for nodes of this class.
// -----------------------------------------------------------------------
CostMethod *SortGroupBy::costMethod() const {
...
//<pb>
void SortGroupBy::addArrangementAndOrderRequirements(RequirementGenerator &rg) {
  ...
  // extraOrderExpr() is only for ITM_PIVOT_GROUP for now
  else if (NOT extraOrderExpr().isEmpty()) {
    ValueIdList groupExprCpy(groupExpr());
    groupExprCpy.insert(extraOrderExpr());
    rg.addSortKey(groupExprCpy);
  }
...
PhysicalProperty *SortGroupBy::synthPhysicalProperty(const Context *myContext, const int planNumber,
  ...
  PhysicalProperty *sppForMe = new (CmpCommon::statementHeap())
      PhysicalProperty((isRollup() ? ValueIdList() : sppOfChild->getSortKey()), sppOfChild->getSortOrderType(),
                       sppOfChild->getDp2SortOrderPartFunc(), sppOfChild->getPartitioningFunction(),
                       sppOfChild->getPlanExecutionLocation(), sppOfChild->getDataSourceEnum(),
                       sppOfChild->getIndexDesc(), sppOfChild->getPartSearchKey(), sppOfChild->getPushDownProperty());

  sppForMe->setCurrentCountOfCPUs(sppTemp->getCurrentCountOfCPUs());
  // aggregate stores entire result before it gets sent on
  if (groupExpr().isEmpty()) sppForMe->setDataSourceEnum(SOURCE_TRANSIENT_TABLE);

  // remove anything that's not covered by the group attributes
  sppForMe->enforceCoverageByGroupAttributes(getGroupAttr());

  DP2CostDataThatDependsOnSPP *dp2CostInfo = (DP2CostDataThatDependsOnSPP *)sppOfChild->getDP2CostThatDependsOnSPP();

  if (sppForMe->executeInDP2()) sppForMe->setDP2CostThatDependsOnSPP(dp2CostInfo);
...
// to the cost estimation functions for nodes of this class.
// -----------------------------------------------------------------------
CostMethod *PhysShortCutGroupBy::costMethod() const {
...
//<pb>
void PhysShortCutGroupBy::addArrangementAndOrderRequirements(RequirementGenerator &rg) {
  ...
  if (opt_for_min_) {
    ...
    ItemExpr *inverseCol = new (CmpCommon::statementHeap()) InverseOrder(rhs_anytrue_);
    inverseCol->synthTypeAndValueId();
...
PhysicalProperty *PhysShortCutGroupBy::synthPhysicalProperty(const Context *myContext, const int planNumber,
  ...
  PhysicalProperty *sppForMe = new (CmpCommon::statementHeap()) PhysicalProperty(
      ...
      NO_SOT,  // no sort key, so no sort order type either
      NULL,    // no dp2SortOrderPartFunc either
      sppOfChild->getPartitioningFunction(), sppOfChild->getPlanExecutionLocation(), sppOfChild->getDataSourceEnum(),
      sppOfChild->getIndexDesc(), sppOfChild->getPartSearchKey(), sppOfChild->getPushDownProperty());
...
NABoolean HashGroupBy::isBigMemoryOperator(const PlanWorkSpace *pws, const int planNumber) {
  ...
  EstLogPropSharedPtr inLogProp = context->getInputLogProp();
  const double probeCount = MAXOF(1., inLogProp->getResultCardinality().value());
  const double myGroupCount = getGroupAttr()->intermedOutputLogProp(inLogProp)->getResultCardinality().value();
  ...
  if (spp != NULL && CmpCommon::getDefault(COMP_BOOL_51) == DF_ON) {
    ...
    // get cumulative file size of the fragment; get the child spp??
    const PhysicalProperty *childSpp = context->getPhysicalPropertyOfSolutionForChild(0);
...
// to the cost estimation functions for nodes of this class.
// -----------------------------------------------------------------------
CostMethod *HashGroupBy::costMethod() const {
...
PhysicalProperty *HashGroupBy::synthPhysicalProperty(const Context *myContext, const int planNumber,
  ...
  // ---------------------------------------------------------------------
  PhysicalProperty *sppForMe = new (CmpCommon::statementHeap()) PhysicalProperty(
      emptySortKey, NO_SOT, NULL, sppOfChild->getPartitioningFunction(), sppOfChild->getPlanExecutionLocation(),
      sppOfChild->getDataSourceEnum(), sppOfChild->getIndexDesc(), sppOfChild->getPartSearchKey());

  sppForMe->setCurrentCountOfCPUs(sppTemp->getCurrentCountOfCPUs());
  // aggregate stores entire result before it gets sent on,
  // non-partial hash groupby also stores entire result
  if (groupExpr().isEmpty() OR isNotAPartialGroupBy() OR isAPartialGroupByRoot())
    sppForMe->setDataSourceEnum(SOURCE_TRANSIENT_TABLE);
...
Context *RelRoot::createContextForAChild(Context *myContext, PlanWorkSpace *pws, int &childIndex) {
  ...
  // ---------------------------------------------------------------------
  if (((planNumber == 1) AND((attESPPara != DF_SYSTEM)
#ifndef NDEBUG
  ...
  if (((CURRSTMT_OPTDEFAULTS->attemptESPParallelism() != DF_OFF) OR hasTMUDFsOrLRU)
      // QSTUFF
      // we don't support paralled execution in the get_next
      // protocol yet - but for streams there should not be
      // a problem.
      && NOT getGroupAttr()->isEmbeddedUpdateOrDelete() && NOT getGroupAttr()->isStream() && NOT getTriggersList() &&
    ...
    countOfCPUs = CURRSTMT_OPTDEFAULTS->getTotalNumberOfCPUs();
    ...
    if ((CmpCommon::getDefault(ASG_FEATURE) == DF_ON) && (childOpType != REL_EXE_UTIL) && (numExtractStreams_ == 0) &&
        !isFastLoadIntoTrafodion && !hasTMUDFsOrLRU) {
      countOfCPUs = CURRSTMT_OPTDEFAULTS->getMaximumDegreeOfParallelism();
    ...
    if (containsLRU()) {
      ...
      if (x && x->getOperatorType() == REL_EXE_UTIL) {
        PartitioningFunction *pf =
            ((ExeUtilExpr *)x)->getUtilTableDesc()->getClusteringIndex()->getPartitioningFunction();
    ...
    if (ocrControl > 0 && x && x->getOperatorType() == REL_UNARY_INSERT) {
      ...
      if (ins->getInsertType() == Insert::VSBB_LOAD) {
        ...
        if ((ocrControl == 1 && (((countOfCPUs < partns) && (partns % countOfCPUs != 0)) || (countOfCPUs > partns))) ||
            ocrControl == 2) {
          countOfCPUs = partns;
          pipelinesPerCPU = 1;
          CURRSTMT_OPTDEFAULTS->setRequiredESPs(partns);
    ...
    // when reaching this block of code.
    if (reqdShape_) {
      ...
      if (wildCardOp) {
        ...
        if (maxDoP > 0) {
          ...
          CURRSTMT_OPTDEFAULTS->setRequiredESPs(countOfCPUs);
          canAdjustDoP = FALSE;
        }
      }
    }
    ...
    if (canAdjustDoP) {
      ...
      if (tAnalysis) {
        CostScalar tableSize = tAnalysis->getCardinalityOfBaseTable() * tAnalysis->getRecordSizeOfBaseTable();
        ...
        if (esps < 0)
          ...
          if (esps < 1)
            ...
            if (esps < countOfCPUs) countOfCPUs = esps;
          }
        }
        pipelinesPerCPU = 1;
      }
  ...
  // for DDL "LOAD TRAFODION META INTO SHARED CACHE", force
  // a parallel plan where the DoP = <number of nodes>
  OperatorTypeEnum childOpType = child(0).getLogExpr()->getOperatorType();
  if (childOpType == REL_DDL) {
    RelExpr *x = child(0).getGroupAttr()->getLogExprForSynthesis();

    DDLExpr *ddlExp = (DDLExpr *)(x->castToRelExpr());

    if (ddlExp->isTrafSharedCacheParallelOp()) {
      pipelinesPerCPU = 1;

      // Must use the following member function to get
      // the total number of CPUs (including down CPUs)
      countOfCPUs = CURRCONTEXT_CLUSTERINFO->getTotalNumberOfCPUs();

      CURRSTMT_OPTDEFAULTS->setRequiredESPs(countOfCPUs);

      ddlExp->setExecuteInESP(countOfCPUs > 1);
    }
  }
  ...
  */
  ...
  // for each operator.
  if ((planNumber == 0) AND(attESPPara == DF_SYSTEM)
#ifndef NDEBUG
  ...
  if (orderByList.entries() > 0) {
    ...
    while (i < orderByList.entries()) {
      ...
      ValueId svid = orderByList[i].getItemExpr()->simplifyOrderExpr()->getValueId();
  ...
  if (rppForMe)
    rppForChild =
        new (CmpCommon::statementHeap()) ReqdPhysicalProperty(*rppForMe,
                                                              NULL,     // no arrangement of rows required
                                                              sortKey,  // user-specified sort order
                                                              sortOrderTypeReq,
                                                              NULL,  // never a dp2SortOrderPartReq at the root
                                                              partReq, loc, countOfCPUs, pipelinesPerCPU);
  else
    rppForChild = new (CmpCommon::statementHeap())
        ReqdPhysicalProperty(NULL,     // no arrangement of rows required
                             sortKey,  // user-specified sort order
                             sortOrderTypeReq,
                             NULL,   // never a dp2SortOrderPartReq at the root
                             ...
                             loc, countOfCPUs, pipelinesPerCPU, CURRSTMT_OPTDEFAULTS->getDefaultCostWeight(),
                             CURRSTMT_OPTDEFAULTS->getDefaultPerformanceGoal(), NULL, NULL);

  // Support for CONTROL QUERY SHAPE
  if (reqdShape_ AND reqdShape_->isCutOp()) reqdShape_ = NULL;
  ...
  // ---------------------------------------------------------------------
  Context *result = shareContext(childIndex, rppForChild, myContext->getInputPhysicalProperty(), costLimit, myContext,
                                 myContext->getInputLogProp(), reqdShape_);
...
NABoolean RelRoot::currentPlanIsAcceptable(int planNo, const ReqdPhysicalProperty *const rppForMe) const {
  ...
  if ((planNo == 0) AND(attESPPara == DF_SYSTEM)
#ifndef NDEBUG
      // set this env var to disable the "teaser" context
      AND getenv("SINGLE_ROOT_CONTEXT") == NULL
#endif
...
PhysicalProperty *RelRoot::synthPhysicalProperty(const Context *myContext, const int planNumber, PlanWorkSpace *pws) {
  ...
  PhysicalProperty *sppForMe =
      new (CmpCommon::statementHeap()) PhysicalProperty(NULL, EXECUTE_IN_MASTER, sppForChild->getDataSourceEnum());
...
// to the cost estimation functions for nodes of this class.
// -----------------------------------------------------------------------
CostMethod *MapValueIds::costMethod() const {
  /*static THREAD_P CostMethodFixedCostPerRow *m = NULL;
  if (m == NULL)
    m = new (GetCliGlobals()->exCollHeap())
...
// ---------------------------------------------------------------------
PartitioningFunction *MapValueIds::mapPartitioningFunction(const PartitioningFunction *partFunc,
                                                           NABoolean rewriteForChild0) {
  NABoolean mapItUp = FALSE;
...
Context *MapValueIds::createContextForAChild(Context *myContext, PlanWorkSpace *pws, int &childIndex) {
  ...
  PartitioningRequirement *dp2SortOrderPartReqForMe = rppForMe->getDp2SortOrderPartReq();
  PartitioningRequirement *dp2SortOrderPartReqForChild = dp2SortOrderPartReqForMe;
  ...
  if (rppForMe->getArrangedCols() != NULL) {
    ...
    map_.rewriteValueIdSetDown(*rppForMe->getArrangedCols(), *arrangedColsReqForChild);
  }

  if (rppForMe->getSortKey() != NULL) {
    sortKeyReqForChild = new (CmpCommon::statementHeap()) ValueIdList();
    map_.rewriteValueIdListDown(*rppForMe->getSortKey(), *sortKeyReqForChild);
  }

  if ((partReqForMe != NULL) AND(NOT partReqForMe->getPartitioningKey().isEmpty())) {
    // -------------------------------------------------------------------
    // Rewrite the partitioning key in terms of the values that appear
    // below this MapValueIds.
    // -------------------------------------------------------------------
    NABoolean mapItUp = FALSE;
    partReqForChild = partReqForMe->copyAndRemap(map_, mapItUp);
  }

  if ((dp2SortOrderPartReqForMe != NULL) AND(NOT dp2SortOrderPartReqForMe->getPartitioningKey().isEmpty())) {
    // -------------------------------------------------------------------
    // Rewrite the partitioning key in terms of the values that appear
    // below this MapValueIds.
    // -------------------------------------------------------------------
    NABoolean mapItUp = FALSE;
    dp2SortOrderPartReqForChild = dp2SortOrderPartReqForMe->copyAndRemap(map_, mapItUp);
  }

  rppForChild = new (CmpCommon::statementHeap())
      ReqdPhysicalProperty(*rppForMe, arrangedColsReqForChild, sortKeyReqForChild, rppForMe->getSortOrderTypeReq(),
                           dp2SortOrderPartReqForChild, partReqForChild);
...
PhysicalProperty *MapValueIds::synthPhysicalProperty(const Context *myContext, const int planNumber,
  ...
  ValueIdList newSortKey;
  PartitioningFunction *oldDp2SortOrderPartFunc = sppOfChild->getDp2SortOrderPartFunc();
  PartitioningFunction *newDp2SortOrderPartFunc = oldDp2SortOrderPartFunc;
  ...
  if ((oldDp2SortOrderPartFunc != NULL) AND(NOT oldDp2SortOrderPartFunc->getPartitioningKey().isEmpty())) {
    newDp2SortOrderPartFunc = oldDp2SortOrderPartFunc->copyAndRemap(map_, mapItUp);
  }

  actualPartFunc = sppOfChild->getPartitioningFunction()->copyAndRemap(map_, mapItUp);
  ...
  PhysicalProperty *sppForMe = new (CmpCommon::statementHeap()) PhysicalProperty(
      *sppOfChild, newSortKey, sppOfChild->getSortOrderType(), newDp2SortOrderPartFunc, actualPartFunc);
...
// -----------------------------------------------------------------------
static void computeDP2CostDataThatDependsOnSPP(PartitioningFunction &physicalPartFunc  // in/out
                                               ,
                                               DP2CostDataThatDependsOnSPP &dp2CostInfo  // out
                                               ,
                                               const IndexDesc &indexDesc, const ScanKey *partKey,
                                               GroupAttributes &scanGroupAttr, const Context &myContext, NAMemory *heap,
                                               const RelExpr &scan)

{
  ...
  CostScalar totalCPUsExecutingDP2s = CURRSTMT_OPTDEFAULTS->getTotalNumberOfCPUs();
  ...
  //  The number of cpus executing DP2's cannot be more than the number
  //  of DP2 volumes:
  if (!isHbaseTable && !isHiveTable) cpusExecutingDP2s = MINOF(cpusExecutingDP2s, numOfDP2Volumes);
  ...
  // if possible below
  // -----------------------------------------------------------------------
  dp2CostInfo.setRepeatCountForOperatorsInDP2((myContext.getInputLogProp()->getResultCardinality()).minCsOne());

  // check if we are doing updates
  if (scan.getOperator().match(REL_ANY_LEAF_GEN_UPDATE) || scan.getOperator().match(REL_ANY_UNARY_GEN_UPDATE))

    dp2CostInfo.setRepeatCountState(DP2CostDataThatDependsOnSPP::UPDATE_OPERATION);
  ...
  if (physicalPartFunc.getNodeMap()->getNumEntries() > 0) {
    ...
    if (physicalPartFunc.isARangePartitioningFunction()
            AND partKey AND((scan.getOperatorType() == REL_FILE_SCAN) OR(scan.getOperatorType() == REL_HBASE_ACCESS))) {
      // ------------------------------------------------------------
      ...
      ValueIdSet keyPredsForLeadingColumn = keyPredsByCol.getPredicatesForColumn(leadingColumn);
      ...
      if (keyPredsForLeadingColumn.isEmpty()) return;

      // Obtain  a new node map to decorate:
      NodeMap *newNodeMapPtr = physicalPartFunc.getNodeMap()->copy(heap);

      // Run estimation algorithm:

      CostScalar probes = myContext.getInputLogProp()->getResultCardinality();

      RangePartitioningFunction *prpfPtr = (RangePartitioningFunction *)&physicalPartFunc;
      ...
      const ColStatDescList &primaryTableCSDL = indexDesc.getPrimaryTableDesc()->getTableColStats();
      ...
      if (colStats->isOrigFakeHist()) return;
      ...
      if ((scan.getOperatorType() == REL_FILE_SCAN) OR(scan.getOperatorType() == REL_HBASE_ACCESS)) opType = REL_SCAN;

      if ((probes.isGreaterThanZero())AND(myContext.getInputLogProp()->getColStats().entries() > 0)) {
        // NJ case:
        leadColHist.applyPredicatesWhenMultipleProbes(keyPredsForLeadingColumn, *(myContext.getInputLogProp()),
                                                      scanGroupAttr.getCharacteristicInputs(),
                                                      FALSE  // MDAM irrelevant here
                                                      ,
                                                      selHint, cardHint, NULL, opType);
      } else {
      ...
      // ------------------------------------------------------------
      PartitionKeyDistribution partKeyDist(*prpfPtr, leadColHist.getColStatDescList());
      ...
      DCMPASSERT(numParts == newNodeMapPtr->getNumEntries());
      ...
      for (i = 0; i < numParts; i++) {
        ...
        if (partKeyDist.getRowsForPartition(i) > 0.)
          newNodeMapPtr->setPartitionState(i, NodeMapEntry::ACTIVE);
        else
          newNodeMapPtr->setPartitionState(i, NodeMapEntry::NOT_ACTIVE);
      }
      ...
      for (i = 0; i < partKeyList.entries(); i++) {
        ...
        ValueIdSet keyPredsForRC = keyPredsByCol.getPredicatesForColumn(currentColumn);
        if (keyPredsForRC.isEmpty()) break;

        // find a predicate in the set that is an equijoin pred:
        NABoolean found = FALSE;
        for (ValueId predId = keyPredsForRC.init(); NOT found AND keyPredsForRC.next(predId);
             keyPredsForRC.advance(predId)) {
          // does it cover the column?
          ItemExpr *predIEPtr = predId.getItemExpr();
          if (predIEPtr->isANestedJoinPredicate(inputValues, operatorValues)) {
            // is it en equijoin?
            if (predIEPtr->getOperatorType() == ITM_VEG_PREDICATE OR predIEPtr->getOperatorType() == ITM_EQUAL) {
              if (i == 0) {
      ...
      if (highestColumn > 0) {
        ...
        CostScalar fanoutRatio = CostScalar(highestColumn - 1) / (numPartKeyColumns - 1);
        CostScalar fanoutFactor = (fanout * (csOne - fanoutRatio)).minCsOne();
    ...
    else if ((physicalPartFunc.isATableHashPartitioningFunction()
                  AND partKey AND(scan.getOperatorType() == REL_FILE_SCAN))
                 OR(indexDesc.isPartitioned() AND(scan.getOperatorType() == REL_HBASE_ACCESS)
      ...
      for (CollIndex i = 0; i < partKeyList.entries(); i++) {
        ...
        ValueIdSet keyPredsForCurCol = keyPredsByCol.getPredicatesForColumn(currentColumn);
        ...
        for (ValueId predId = keyPredsForCurCol.init(); keyPredsForCurCol.next(predId);
          ...
          if ((predIEPtr->getOperatorType() == ITM_VEG_PREDICATE) OR(predIEPtr->getOperatorType() == ITM_EQUAL)) {
            ...
            if ((NOT foundConstPred)OR(NOT foundNJPredOrConstPred)) {
              ...
              if (predIEPtr->getOperatorType() == ITM_VEG_PREDICATE) {
                const VEG *predVEG = ((VEGPredicate *)predIEPtr)->getVEG();
                ...
                if (leftExpr->doesExprEvaluateToConstant(FALSE) OR rightExpr->doesExprEvaluateToConstant(FALSE)) {
                  if (NOT foundConstPred) {
                    keyColsCoveredByConst++;
                    foundConstPred = TRUE;
                  }
            ...
            if ((NOT foundNJPredOrConstPred) &&
                (isAConstPred || (predIEPtr->isANestedJoinPredicate(inputValues, operatorValues)))) {
      ...
      if (keyColsCoveredByConst == partKeyList.entries()) {
        ...
        dp2CostInfo.setRepeatCountState(DP2CostDataThatDependsOnSPP::KEYCOLS_COVERED_BY_CONST);
        ...
        if (CURRSTMT_OPTDEFAULTS->incorporateSkewInCosting() AND physicalPartFunc.isATableHashPartitioningFunction()) {
          CostScalar probesAtBusyStream = myContext.getInputLogProp()->getCardOfBusiestStream(
              &physicalPartFunc, numParts, &scanGroupAttr, numParts, TRUE);

          dp2CostInfo.setProbesAtBusiestStream(probesAtBusyStream);
        }

        dp2CostInfo.setRepeatCountForOperatorsInDP2(
            (myContext.getInputLogProp()->getResultCardinality() / numParts).getCeiling().minCsOne());

        dp2CostInfo.setRepeatCountState(DP2CostDataThatDependsOnSPP::KEYCOLS_COVERED_BY_PROBE_COLS_CONST);
        ...
        dp2CostInfo.setRepeatCountState(DP2CostDataThatDependsOnSPP::KEYCOLS_NOT_COVERED);
...
PhysicalProperty *RelExpr::synthDP2PhysicalProperty(const Context *myContext, const ValueIdList &sortOrder,
                                                    const IndexDesc *indexDesc, const SearchKey *partSearchKey) {
  ...
  ValueIdList physicalClusteringKey = indexDesc->getOrderOfKeyValues();
  ...
  if (physicalPartFunc == NULL) {
    NodeMap *nodeMap =
        indexDesc->getNAFileSet()->getPartitioningFunction()->getNodeMap()->copy(CmpCommon::statementHeap());
    physicalPartFunc = new (CmpCommon::statementHeap()) SinglePartitionPartitioningFunction(nodeMap);
  }
  ...
  if (lpr) {
    ...
    if (lpr->getNumClientsReq() == 1 && lpr->isNumberOfPAsForced() &&
      ...
      numPAs = MINOF(currentCountOfCPUs, physicalPartFunc->getCountOfPartitions());
    ...
    if ((physicalPartFunc->isARoundRobinPartitioningFunction() OR physicalPartFunc->partKeyContainsFloatColumn())AND(
            (logPartReq == NULL)
                OR NOT(logPartReq->isRequirementExactlyOne() OR logPartReq->isRequirementReplicateNoBroadcast())))
      return NULL;
    ...
    if (logPartReq) {
      logicalPartFunc =
          logPartReq->realize(myContext, considerPhysicalReqs, physicalPartFunc->makePartitioningRequirement());
    ...
    if (logicalPartFunc == NULL OR logicalPartFunc->isAGroupingOf(*physicalPartFunc)
                               OR(logPartReq AND logPartReq->castToRequireReplicateNoBroadcast())) {
      if (logPartType == LogPhysPartitioningFunction::ANY_LOGICAL_PARTITIONING) {
    ...
    } else if (logPartType == LogPhysPartitioningFunction::PA_PARTITION_GROUPING) {
    ...
    if (logPartType != LogPhysPartitioningFunction::PA_PARTITION_GROUPING AND logicalPartFunc AND
      ...
      if (logPartReq->castToRequireRange())
        rpfFromRequirement =
            logPartReq->castToRequireRange()->getPartitioningFunction()->castToRangePartitioningFunction();
      else
      ...
      if (rpfFromRequirement AND(physicalClusteringKey.satisfiesReqdOrder(rpfFromRequirement->getOrderOfKeyValues()) ==
                                 SAME_ORDER)
              AND(physicalPartFunc->isASinglePartitionPartitioningFunction()
                      OR(physicalPartFunc->isARangePartitioningFunction()
                             AND(physicalClusteringKey.satisfiesReqdOrder(
                                     physicalPartFunc->castToRangePartitioningFunction()->getOrderOfKeyValues()) ==
                                 SAME_ORDER)))) {
        logPartType = LogPhysPartitioningFunction::LOGICAL_SUBPARTITIONING;
        //                           =======================
      } else if (logPartType == LogPhysPartitioningFunction::LOGICAL_SUBPARTITIONING) {
  ...
  if (CmpCommon::getDefault(BASE_NUM_PAS_ON_ACTIVE_PARTS) == DF_ON) baseNumPAsOnAP = TRUE;
  ...
  if ((numPAs != ANY_NUMBER_OF_PARTITIONS) OR(CmpCommon::getDefault(ATTEMPT_ASYNCHRONOUS_ACCESS) == DF_OFF)) {
    ...
    if (numPAs < physicalPartFunc->getCountOfPartitions()) {
      if (physicalPartFunc->shouldUseSynchronousAccess(rppForMe, myContext->getInputLogProp(), getGroupAttr()))
        shouldUseSynchronousAccess = TRUE;
      else
        return NULL;
    }
  }  // end if forcing number of PAs or asynchronous access
  else if (physicalPartFunc->shouldUseSynchronousAccess(rppForMe, myContext->getInputLogProp(), getGroupAttr())) {
    shouldUseSynchronousAccess = TRUE;
  ...
  } else  // # of PAs not forced and no synchronous access
  {
    ...
    if (NOT baseNumPAsOnAP OR(rppForMe->getLogicalOrderOrArrangementFlag()
  ...
  mergeOfSortedStreams =
      rppForMe->getLogicalOrderOrArrangementFlag() AND(rppForMe->getDp2SortOrderPartReq() == NULL) AND(numPAs != 1);
  ...
  // -----------------------------------------------------------------------
  if (logicalPartFunc AND logPartReq AND(logPartReq->getCountOfPartitions() != ANY_NUMBER_OF_PARTITIONS)) {
    // the logical partitioning function, which was derived from
    // the logical partitioning requirement, has specified the
    // number of ESPs
    numEsps =
        ((CmpCommon::getDefault(COMP_BOOL_129) == DF_ON) AND(CURRSTMT_OPTDEFAULTS->attemptESPParallelism() == DF_SYSTEM)
             AND logPartReq->isRequirementApproximatelyN())
            ? logPartReq->castToRequireApproximatelyNPartitions()->getCountOfPartitionsLowBound()
    ...
    numEsps = 1;
    logicalPartFunc = new (CmpCommon::statementHeap()) SinglePartitionPartitioningFunction();
    logicalPartFunc->createPartitioningKeyPredicates();
  } else {
    ...
    if (NOT okToAttemptESPParallelism(myContext,
                                      ...
                                      numEsps, allowedDeviation, numOfESPsForced) OR(numEsps == 1)
      ...
      if ((CmpCommon::getDefault(BASE_NUM_PAS_ON_ACTIVE_PARTS) == DF_ON)
        ...
        if (scaleNumOfParts > numActivePartitions) scaleNumOfParts = numActivePartitions;
      }

      // Actually do the grouping now
      logicalPartFunc = logicalPartFunc->scaleNumberOfPartitions(scaleNumOfParts);
      // Was scale able to do it's job?
      if (scaleNumOfParts != numEsps)  // No
      {
        ...
        if (logicalPartFunc->getCountOfPartitions() <= rppForMe->getCountOfPipelines())
  ...
  if (NOT numPAsForced AND NOT shouldUseSynchronousAccess AND numEsps > 1) {
    ...
    if (logicalPartFunc->isAReplicateNoBroadcastPartitioningFunction()) {
      ...
      const RequireReplicateNoBroadcast *rnbReq = logPartReq->castToRequireReplicateNoBroadcast();
      ...
      if (((mergeOfSortedStreams) || (CmpCommon::getDefault(COMP_BOOL_67) == DF_OFF)) && !grouping)
        numPAs = numPAs * numEsps;
    } else if (logicalPartFunc->isAGroupingOf(*physicalPartFunc, &maxPartsPerGroup)
                   AND(logPartType == LogPhysPartitioningFunction::PA_PARTITION_GROUPING)) {
      ...
      if (mergeOfSortedStreams) {
        ...
        if ((maxPartsPerGroup > maxPAsPerProcess) AND((logPartReq == NULL) OR logPartReq->isRequirementFuzzy())
                AND(maxPartsPerUniformGroup <= maxPAsPerProcess)) {
          logicalPartFunc = logicalPartFunc->copy();  // just in case
          logicalPartFunc->scaleNumberOfPartitions(numEsps, UNIFORM_PHYSICAL_PARTITION_GROUPING);
          logicalPartFunc->createPartitioningKeyPredicates();
        } else
  ...
  if (numPAs < numEsps) {
    ...
    if (logPartType == LogPhysPartitioningFunction::PA_PARTITION_GROUPING) {
  ...
  if ((numPAs / numEsps) > maxPAsPerProcess) {
    ...
    if (mergeOfSortedStreams) return NULL;
  ...
  // ---------------------------------------------------------------------
  if (logPartType ==
      LogPhysPartitioningFunction::PA_PARTITION_GROUPING AND physicalPartFunc->isASinglePartitionPartitioningFunction()
          AND numPAs == 1 AND NOT usePapa) {
    ...
    logPhysPartFunc = new (CmpCommon::statementHeap())
        SinglePartitionPartitioningFunction(physicalPartFunc->getNodeMap()->copy(CmpCommon::statementHeap()));
    ...
    LogPhysPartitioningFunction *lpf = new (CmpCommon::statementHeap()) LogPhysPartitioningFunction(
        logicalPartFunc, physicalPartFunc, logPartType, numPAs, usePapa, shouldUseSynchronousAccess);
    ...
    if (NOT(logicalPartFunc->getPartitioningKeyPredicates().isEmpty() &&
      ...
      ValueIdSet availInputs(getGroupAttr()->getCharacteristicInputs());
      ValueIdSet dummy;

      SearchKey *newPartSearchKey = logicalPartFunc->createSearchKey(indexDesc, availInputs, dummy);
      ...
      if (newPartSearchKey) {
        ...
        CMPASSERT((partSearchKey->getKeyPredicates() == logicalPartFunc->getPartitioningKeyPredicates())
                      OR(logPartType != LogPhysPartitioningFunction::PA_PARTITION_GROUPING));
      }
    }
  }
  ...
  if (NOT sortOrder.isEmpty()) dp2SortOrderPartFunc = physicalPartFunc;
  ...
  if (pdr) {
    ...
    if (PushDownCSRequirement::isInstanceOf(pdr))
      pushDownProperty = new (CmpCommon::statementHeap()) PushDownCSProperty(physicalPartFunc, partSearchKey);
    else {
      if (PushDownColocationRequirement::isInstanceOf(pdr))
        pushDownProperty = new (CmpCommon::statementHeap()) PushDownColocationProperty(physicalPartFunc->getNodeMap());
      else
        CMPASSERT(1 == 0);
    }
  }
  ...
  // ---------------------------------------------------------------------
  sppForMe = new (CmpCommon::statementHeap())
      PhysicalProperty(sortOrder,
                       NO_SOT,  // Don't synthesize a sort order type until the exchange node
                       dp2SortOrderPartFunc, logPhysPartFunc, location, SOURCE_PERSISTENT_TABLE, indexDesc,
                       partSearchKey, pushDownProperty);

  DP2CostDataThatDependsOnSPP *dp2CostInfoPtr = new HEAP DP2CostDataThatDependsOnSPP(dp2CostInfo);
...
RangePartitioningFunction *FileScan::createRangePartFuncForHbaseTableUsingStats(
  ...
  if (partns == 1 ||
      (!useMCSplit && (partitioningKeyColumns.entries() != 1)))  // if partition key has more than one column but MC
                                                                 // stats based partitioning is disabled, then return
    return NULL;
  ...
  if (indexDesc_->getPartitioningKey().entries() > 0) {
    ...
    for (CollIndex j = 0; j < partitioningKeyColumns.entries(); j++) {
      partKeyColArray.insert(indexDesc_->getNAFileSet()->getPartitioningKeyColumns()[j]);
      partKeyColArray.setAscending(j, indexDesc_->getNAFileSet()->getPartitioningKeyColumns().isAscending(j));
    }
  } else {
    leadingColumn = indexDesc_->getIndexKey()[0];

    for (CollIndex j = 0; j < partitioningKeyColumns.entries(); j++) {
      partKeyColArray.insert(indexDesc_->getNAFileSet()->getIndexKeyColumns()[j]);
      partKeyColArray.setAscending(j, indexDesc_->getNAFileSet()->getIndexKeyColumns().isAscending(j));
    }
  }
  ...
  if (primaryTableCSDL.getColStatDescIndexForColumn(i,  // out
    ...
    if (!colStats->isOrigFakeHist()) {
      ...
      RangePartitionBoundaries *rpb = createRangePartitionBoundariesFromStats(
          indexDesc_, hist, partns, partKeyColArray, partitioningKeyColumnsOrder, colStats->getStatColumns().entries(),
          STMTHEAP);
      ...
      RangePartitioningFunction *newPartFunc = new (STMTHEAP) RangePartitioningFunction(
          partitioningKeyColumns, partitioningKeyColumnsList, partitioningKeyColumnsOrder, rpb, NULL, STMTHEAP);
...
PhysicalProperty *FileScan::synthHbaseScanPhysicalProperty(const Context *context, const int planNumber,
  ...
  if (partReq && partReq->castToRequireExactlyOnePartition()) {
    myPartFunc = new (CmpCommon::statementHeap()) SinglePartitionPartitioningFunction();
  } else {
    //////////////////////////////////////
    // Perform the scaling
    //////////////////////////////////////
    ...
    NABoolean fakeEnv = (defs.getToken(PARALLEL_NUM_ESPS, 0) != DF_SYSTEM);
    ...
    if (partReq) {
      ...
      if (partReq->castToRequireApproximatelyNPartitions()) {
        minESPs = partReq->castToRequireApproximatelyNPartitions()->getCountOfPartitionsLowBound();
        canFreelyAdjustDoP = TRUE;
      } else {
        if (partReq->castToFuzzyPartitioningRequirement() && ANY_NUMBER_OF_PARTITIONS == minESPs) {
          canFreelyAdjustDoP = TRUE;
        }
      }
      ...
      if (!fakeEnv) {
        ...
        // forced dop case. Get the value from
        // OptDefaults::totalNumberOfCPUs_ which is from
        // NADefaults::getTotalNumOfESPs().
        // We can't use CmpCommon::context()->getNumOfSMPs()
        // as the method does not honor CQD PARALLEL_NUM_ESPS.
        minESPs = maxESPs = CURRSTMT_OPTDEFAULTS->getTotalNumberOfCPUs();
      }
    }
    ...
    if (!(CURRSTMT_OPTDEFAULTS->attemptESPParallelism() == DF_OFF)) {
      ...
      CollIndex totalESPsAllowed = CURRSTMT_OPTDEFAULTS->getTotalNumberOfCPUs();

      if (!fakeEnv) {
        // compute the scan size at the scan level
        CostScalar rowCount = getTuplesProcessed();
        CostScalar scanSize = rowCount * (indexDesc_->getNAFileSet()->getRecordLength());

        // if the # of tuples processed is unknown, use the output at the scan
        if (rowCount == 0.0) {
          rowCount = (CostScalar)getGroupAttr()->getResultCardinalityForEmptyInput();
        ...
        if (canFreelyAdjustDoP) {
          if (numOfPartitions > 1 && numPartitionsAsDopThreshold >= 0 && scanSize >= numPartitionsAsDopThreshold) {
            ...
            if (CURRSTMT_OPTDEFAULTS->isAdaptiveSegForHadoop())
              maxESPsByUser = adjustToMultiple(maxESPsByUser, totalESPsAllowed);
            ...
            if (getDefaultAsLong(AS_AFFINITY_VALUE) != -2 && ixDescPartFunc) {
              ...
              if (numOfUniqueNodes > 0) minESPs = MINOF(minESPs, numOfUniqueNodes);
            }
          }
        }
      }
    ...
    if (CURRSTMT_OPTDEFAULTS->isAdaptiveSegForHadoop())
      numESPs = closestFactor(numESPs, maxESPs, CURRSTMT_OPTDEFAULTS->getTotalNumberOfNodes());

    NABoolean performStatsSplit = (CmpCommon::getDefault(HBASE_STATS_PARTITIONING) != DF_OFF &&
                                   !(ixDescPartFunc && ixDescPartFunc->isAHash2PartitioningFunction()));

    if (partReq && partReq->castToRequireReplicateNoBroadcast()) {
      myPartFunc = partReq->castToRequireReplicateNoBroadcast()->getPartitioningFunction()->copy();
      ...
      if ((myPartFuncAsRange = myPartFunc->castToRangePartitioningFunction())) {
        ...
        if (performStatsSplit)
          newPartFunc = createRangePartFuncForHbaseTableUsingStats(partns, myPartFuncAsRange->getPartitioningKey(),
      ...
      if (performStatsSplit) {
        ...
        RangePartitioningFunction *newPartFunc =
            createRangePartFuncForHbaseTableUsingStats(partns, keyColumnSet, keyColumnList, orderOfKeyValues);
        ...
        if (newPartFunc) {
          ...
          // Create and set the Searchkey for the partitioning key:
          partKeys_ = new (CmpCommon::statementHeap()) SearchKey(keyColumnList, orderOfKeyValues, externalInputs,
                                                                 NOT getReverseScan(), selectionPred(), *disjunctsPtr_,
  ...
  if (myPartFunc->getNodeMap() == NULL || partnsScaled) {
    NodeMap *myNodeMap = new (CmpCommon::statementHeap())
        NodeMap(CmpCommon::statementHeap(), myPartFunc->getCountOfPartitions(), NodeMapEntry::ACTIVE, NodeMap::HBASE);

    myPartFunc->replaceNodeMap(myNodeMap);
  }

  // colocated ESP logic
  if ((CmpCommon::getDefault(TRAF_ALLOW_ESP_COLOCATION) == DF_ON) AND ixDescPartFunc) {
  ...
  PhysicalProperty *sppForMe =
      new (CmpCommon::statementHeap()) PhysicalProperty(sortOrderVEG, ESP_NO_SORT_SOT, NULL, /* no dp2 part func*/
                                                        myPartFunc, EXECUTE_IN_MASTER_AND_ESP, SOURCE_VIRTUAL_TABLE);
...
PhysicalProperty *FileScan::synthPhysicalProperty(const Context *myContext, const int planNumber, PlanWorkSpace *pws) {
  ...
  if (!getTableDesc()->getNATable()->isORC() &&
    ...
    sortOrderVEG.complifyAndRemoveUncoveredSuffix(getGroupAttr()->getCharacteristicOutputs(), getGroupAttr());
  }
  ...
  if (getReverseScan()) {
    ...
    for (int i = 0; i < (int)sortOrderVEG.entries(); i++) {
      ...
      if (ix->getOperatorType() == ITM_INVERSE) {
        ...
        inverseCol = new (CmpCommon::statementHeap()) InverseOrder(ix);
        inverseCol->synthTypeAndValueId();
      }
  ...
  if (isHbaseTable()) return synthHbaseScanPhysicalProperty(myContext, planNumber, sortOrderVEG);

  // ---------------------------------------------------------------------
  // call a static helper method shared between scan and ins/upd/del
  // ---------------------------------------------------------------------
  if ((sppForMe = synthDP2PhysicalProperty(myContext, sortOrderVEG, indexDesc_, partKeys_)) == NULL) return NULL;

  // ---------------------------------------------------------------------
  // Apply partitioning key predicates if necessary
  // ---------------------------------------------------------------------
  if (sppForMe->getPartitioningFunction()->castToLogPhysPartitioningFunction()) {
...
const PartitioningFunction *FileScan::getPartFunc() const { return getPhysicalProperty()->getPartitioningFunction(); }

void FileScan::addPartKeyPredsToSelectionPreds(const ValueIdSet &partKeyPreds, const ValueIdSet &pivs) {
...
NABoolean FileScan::okToAttemptESPParallelism(const Context *myContext, /*IN*/
  ...
  if (parallelControlSettings == DF_OFF) {
    result = FALSE;
  } else if ((CmpCommon::getDefault(COMP_BOOL_64) == DF_ON) AND(parallelControlSettings == DF_MAXIMUM)
                 AND CURRSTMT_OPTDEFAULTS->maxParallelismIsFeasible()) {
    numOfESPs = rppForMe->getCountOfPipelines();
    ...
    if (NOT numOfESPsForced) {
      ...
      numOfESPs = (numOfESPs + optimalNumPAsPerEsp - 1) / optimalNumPAsPerEsp;
    ...
    const CostScalar rowCount = (outputLogProp->getResultCardinality()).minCsOne();
...
// -----------------------------------------------------------------------
CostMethod *DP2Scan::costMethod() const {
...
PhysicalProperty *Describe::synthPhysicalProperty(const Context *myContext, const int planNumber, PlanWorkSpace *pws) {
  ...
  //------------------------------------------------------------
  PartitioningFunction *myPartFunc = new (CmpCommon::statementHeap()) SinglePartitionPartitioningFunction(myNodeMap);

  PhysicalProperty *sppForMe =
      new (CmpCommon::statementHeap()) PhysicalProperty(myPartFunc, EXECUTE_IN_MASTER, SOURCE_VIRTUAL_TABLE);
...
PhysicalProperty *GenericUtilExpr::synthPhysicalProperty(const Context *myContext, const int planNumber,
  ...
  ConstStringList *nodeList = NULL;

  if (getOperatorType() == REL_DDL) {
    DDLExpr *ddlExpr = (DDLExpr *)(this);

    nodeList = ddlExpr->getNodeList();

    // Only allow parallel plan for 'load metadata into shared cache' and
    // all other update operations, due to the need to load in every
    // relevant node in the cluster.
    if (ddlExpr->isTrafSharedCacheParallelOp()) {
      NABoolean executeInESP = ddlExpr->isExecuteInESP();
      if (!executeInESP) {
        location = EXECUTE_IN_MASTER;
      } else {
        location = EXECUTE_IN_ESP;
        doSerialPlan = FALSE;
        countOfPartitions = rppForMe->getCountOfAvailableCPUs();
  ...
  // Figure out whether this is an AS case or a subset of nodes case and
  // use the finding to set the node map for the parallel shared cache
  // loading DDL.
  //
  // AS case: all nodes will be involved or loaded
  // subset of nodes case: a subset of nodes will be involved or loaded
  //
  const NAWSetOfNodeIds *nodeIdSet = NULL;

  if (doParallelDDL) {
    const NAWNodeSet *nodeSet = CmpCommon::context()->getAvailableNodes();

    if (nodeSet) nodeIdSet = nodeSet->castToNAWSetOfNodeIds();
  }

  NodeMap *myNodeMap = NULL;

  if (nodeIdSet) {
    // merge to nodeIdArray the node list specified
    // in "LOAD TRAFODION METADATA INTO
    // SHARED CACHE FOR NODE LIST "node1, node2, node3""

    // nodeIdArray is the array of node Ids contained in nodeIdSet (the max
    // node id set where we are allowed to load the shared cache).
    const ARRAY(short) &nodeIdArray = nodeIdSet->getNodeIdArray();

    // nodeIdsSpecifiedInDDL is the array of node Ids specified in the DDL (if any)
    DDLExpr *ddlExpr = (DDLExpr *)(this);
    const ARRAY(short) &nodeIdsSpecifiedInDDL = ddlExpr->getNodeIdsSpecified();

    // finalNodeIdArray is the result array after intersecting the above two.
    // First, assume that there are node nodes specified in the DDL.
    ARRAY(short) finalNodeIdArray(nodeIdArray, CmpCommon::statementHeap());

    if (nodeIdsSpecifiedInDDL.entries() > 0) {
      // There are nodes specified in the DDL.  Clear the array and set its
      // max entries to be that of the nodeIdArray.
      finalNodeIdArray.clear();
      finalNodeIdArray.resize(nodeIdArray.entries());

      // Perform the intersection.
      NABoolean ok = nodeIdArray.intersect(nodeIdsSpecifiedInDDL, finalNodeIdArray);

      assert(ok);
    }

    //----------------------------------------------------------
    // Create a node map with countOfPartitions entries, out of
    // which <m> are ACTIVE and the rest IDLE, where <m> is the
    // entries in the nodeId array (nodeIdSet->getNodeIdArray())
    //
    // The node Ids of all active entries are given in the nodeId
    // array.
    //----------------------------------------------------------
    myNodeMap = new (heap) NodeMap(CmpCommon::statementHeap(), countOfPartitions, finalNodeIdArray);

  } else if (nodeList) {
    // Not a resource based tenant case. But a node list may
    // exist and we need to restrict the node map based on it.
    DDLExpr *ddlExpr = (DDLExpr *)(this);

    myNodeMap = new (heap) NodeMap(CmpCommon::statementHeap(), countOfPartitions, ddlExpr->getNodeIdsSpecified());
  } else {
    //----------------------------------------------------------
    // Create a node map with countOfPartitions active,
    // wild-card entries.
    //----------------------------------------------------------
    myNodeMap = new (heap) NodeMap(CmpCommon::statementHeap(), countOfPartitions, NodeMapEntry::ACTIVE);
  }

  //------------------------------------------------------------
  // Synthesize a partitioning function with a counterOfPartition
  // partitions.
  //------------------------------------------------------------
  PartitioningFunction *myPartFunc = NULL;

  if (doSerialPlan)
    myPartFunc = new (heap) SinglePartitionPartitioningFunction(myNodeMap);
  else {
    // Use ReplicateNoBroadcastPartitioningFunction to perform
    // the DDL in countOfPartitions number of ESPs. Each such ESP
    // should have a one-to-one mapping to nodes in the cluster (
    // non tenant configuration), or a one-to-one mapping to
    // a proper set of nodes in the cluster (tenant configuration).
    //
    // See RelRoot::createContextForAChild() where countOfCPUs takes
    // the value from nadefaults::getTotalNumberOfCPUs(), and the
    // implementation of the method.
    myPartFunc = new (heap) ReplicateNoBroadcastPartitioningFunction(countOfPartitions, myNodeMap, heap);

    myPartFunc->setAllowRandomMapping(FALSE);
  }

  PhysicalProperty *sppForMe = new (heap) PhysicalProperty(myPartFunc, location, SOURCE_VIRTUAL_TABLE);
...
PhysicalProperty *RelLock::synthPhysicalProperty(const Context *myContext, const int planNumber, PlanWorkSpace *pws) {
  ...
  if (rpp->getCountOfAvailableCPUs() <= 1) parallelExecution_ = FALSE;

  if (parallelExecution_) {
    myPartFunc = tabIds_[0]->getClusteringIndex()->getPartitioningFunction();

    if (!myPartFunc)
      parallelExecution_ = FALSE;
    else {
      PhysicalProperty *sppForMe =
          new (CmpCommon::statementHeap()) PhysicalProperty(myPartFunc, EXECUTE_IN_ESP, SOURCE_VIRTUAL_TABLE);
      // remove anything that's not covered by the group attributes
      sppForMe->enforceCoverageByGroupAttributes(getGroupAttr());
      return sppForMe;
    }
...
// to the cost estimation functions for nodes of this class.
// -----------------------------------------------------------------------
CostMethod *Tuple::costMethod() const {
  static THREAD_P CostMethodTuple *m = NULL;
  if (m == NULL) m = new (GetCliGlobals()->exCollHeap()) CostMethodTuple();
  return m;
}  // Tuple::costMethod()
...
PhysicalProperty *Tuple::synthPhysicalProperty(const Context *myContext, const int planNumber, PlanWorkSpace *pws) {
  ...
  if (myPartReq) {
    ...
    if (NOT rppForMe->executeInDP2() &&
        (myPartFunc->castToHashPartitioningFunction() || myPartFunc->castToTableHashPartitioningFunction() ||
         myPartFunc->castToRangePartitioningFunction() || myPartFunc->castToRoundRobinPartitioningFunction()) &&
    ...
    if (rppForMe->executeInDP2()) {
      ...
      if (pdq AND PushDownCSRequirement::isInstanceOf(pdq))
        myPartFunc = (PartitioningFunction *)((pdq->castToPushDownCSRequirement())->getPartFunc());
    }

    if (myPartFunc == NULL) myPartFunc = new (CmpCommon::statementHeap()) SinglePartitionPartitioningFunction();
  }
  ...
  if (myPartFunc->getNodeMap() == 0) {
    NodeMap *myNodeMap = new (CmpCommon::statementHeap())
        NodeMap(CmpCommon::statementHeap(), myPartFunc->getCountOfPartitions(), NodeMapEntry::ACTIVE);
    myPartFunc->replaceNodeMap(myNodeMap);
  }
  ...
  if (isinBlockStmt() AND rppForMe->executeInDP2() AND NOT PushDownCSRequirement::isInstanceOf(pdr)) return 0;

  if (pdr) {
    // If the tuple is the first statement (left most node) or
    // immediately below an EXCHANGE, do not allow it.
    if (pdr->isEmpty() == TRUE) return 0;
    ...
    if (!myPartFunc->isASinglePartitionPartitioningFunction()) {
      ...
      if (!lpr) {
        lpr = new (CmpCommon::statementHeap()) LogicalPartitioningRequirement(rppForMe->getPartitioningRequirement());
      ...
      logPartReq = lpr->getLogReq();
      if (logPartReq) {
        logicalPartFunc = logPartReq->realize(myContext, FALSE, myPartFunc->makePartitioningRequirement());

        myPartFunc = new (CmpCommon::statementHeap())
            LogPhysPartitioningFunction(logicalPartFunc,  // logical
                                        myPartFunc,       // physical
                                        logPartType, numPAs, usePapa, shouldUseSynchronousAccess);
  ...
  PhysicalProperty *sppForMe =
      new (CmpCommon::statementHeap()) PhysicalProperty(myPartFunc, planExecutionLocation, SOURCE_TUPLE);

  PushDownProperty *pushDownProperty = 0;

  if (pdr && !pdr->isEmpty()) {
    const PushDownCSRequirement *pdcsr = pdr->castToPushDownCSRequirement();

    if (pdcsr) {
      // generate a CS push-down property.
      pushDownProperty =
          new (CmpCommon::statementHeap()) PushDownCSProperty(pdcsr->getPartFunc(), pdcsr->getSearchKey());
    } else {
      const PushDownColocationRequirement *pdclr = pdr->castToPushDownColocationRequirement();

      // generate a colocation push-down property.
      if (pdclr) {
        pushDownProperty = new (CmpCommon::statementHeap()) PushDownColocationProperty(pdclr->getNodeMap());
      } else
        CMPASSERT(1 == 0);
    }
  }
  ...
  countOfCPUs = (countOfStreams < countOfAvailableCPUs ? countOfStreams : countOfAvailableCPUs);
...
PhysicalProperty *InsertCursor::synthPhysicalProperty(const Context *myContext, const int planNumber,
  ...
  // remove anything that's not covered by the group attributes
  if (sppForMe) sppForMe->enforceCoverageByGroupAttributes(getGroupAttr());
...
NABoolean GenericUpdate::okToAttemptESPParallelism(const Context *myContext, /*IN*/
  ...
  if (isMerge()) {
    result = FALSE;
  } else if (parallelControlSettings == DF_OFF) {
    result = FALSE;
  } else if ((CmpCommon::getDefault(COMP_BOOL_65) == DF_ON) AND(parallelControlSettings == DF_MAXIMUM)
                 AND CURRSTMT_OPTDEFAULTS->maxParallelismIsFeasible()) {
    numOfESPs = rppForMe->getCountOfPipelines();
    allowedDeviation = 0.0;  // not used by leafs
    result = TRUE;
...
// Obtain a pointer to a CostMethod object providing access
// to the cost estimation functions for nodes of this type.
CostMethod *PhysTranspose::costMethod() const {
...
PhysicalProperty *PhysTranspose::synthPhysicalProperty(const Context *context, const int planNumber,
  ...
  PhysicalProperty *sppForMe =
      new (CmpCommon::statementHeap()) PhysicalProperty(*context->getPhysicalPropertyOfSolutionForChild(0));
...
CostMethod *RelInternalSP::costMethod() const {
...
CostMethod *HbaseDelete::costMethod() const {
...
PhysicalProperty *HbaseDelete::synthPhysicalProperty(const Context *myContext, const int planNumber,
  ...
  if (partReqForMe && partReqForMe->castToRequireReplicateNoBroadcast()) {
    myPartFunc = partReqForMe->castToRequireReplicateNoBroadcast()->getPartitioningFunction()->copy();
  } else {
    // Create a node map with a single, active, wild-card entry.
    NodeMap *myNodeMap = new (CmpCommon::statementHeap()) NodeMap(CmpCommon::statementHeap(), 1, NodeMapEntry::ACTIVE);
    myPartFunc = new (CmpCommon::statementHeap()) SinglePartitionPartitioningFunction(myNodeMap);
  ...
  NABoolean existTrigger = FALSE;
  if (CmpCommon::getDefault(USE_TRIGGER) == DF_ON &&
      (getTableDesc() != NULL && getTableDesc()->getNATable() != NULL && getTableDesc()->getNATable()->hasTrigger()))
    existTrigger = TRUE;

  PhysicalProperty *sppForMe = new (CmpCommon::statementHeap())
      PhysicalProperty(myPartFunc, existTrigger ? EXECUTE_IN_MASTER : EXECUTE_IN_MASTER_AND_ESP, SOURCE_VIRTUAL_TABLE);
...
CostMethod *HbaseUpdate::costMethod() const {
...
PhysicalProperty *HiveInsert::synthPhysicalProperty(const Context *myContext, const int planNumber,
  ...
  if (partReq->isRequirementFullySpecified()) {
    FullySpecifiedPartitioningRequirement *fpr =
        (FullySpecifiedPartitioningRequirement *)(partReq->castToFullySpecifiedPartitioningRequirement());
  ...
  PhysicalProperty *pp = new (CmpCommon::statementHeap()) PhysicalProperty(myPartFunc, location);

  return pp;

}  // HiveInsert::synthPhysicalProperty()

CostMethod *HbaseInsert::costMethod() const {
...
PhysCompoundStmt::PhysCompoundStmt(RelExpr *leftChild, RelExpr *rightChild, OperatorTypeEnum op /*= REL_COMPOUND_STMT*/,
                                   CollHeap *heap /*= CmpCommon::statementHeap()*/)
    : CompoundStmt(leftChild, rightChild, op, heap) {
  if (heap)
    pCostMethod_ = new (heap) CostMethodCompoundStmt();
  else
    pCostMethod_ = new (CmpCommon::statementHeap()) CostMethodCompoundStmt();
}

PhysCompoundStmt::PhysCompoundStmt(const CompoundStmt &other) : CompoundStmt(other) {
  pCostMethod_ = new (CmpCommon::statementHeap()) CostMethodCompoundStmt();
}
// -----------------------------------------------------------------------
...
// to the cost estimation functions for nodes of this class.
// -----------------------------------------------------------------------
CostMethod *PhysCompoundStmt::costMethod() const {
...
PhysicalProperty *PhysCompoundStmt::synthPhysicalProperty(const Context *context, const int /*unused*/,
  ...
  // ---------------------------------------------------------------------
  PhysicalProperty *sppForMe = new (CmpCommon::statementHeap()) PhysicalProperty(
      sppOfLeftChild->getSortKey(), sppOfLeftChild->getSortOrderType(), sppOfLeftChild->getDp2SortOrderPartFunc(),
      sppOfLeftChild->getPartitioningFunction(), sppOfLeftChild->getPlanExecutionLocation(),
      sppOfLeftChild->getDataSourceEnum(), sppOfLeftChild->getIndexDesc(), sppOfLeftChild->getPartSearchKey(),
...
Context *CompoundStmt::createContextForAChild(Context *myContext, PlanWorkSpace *pws, int &childIndex) {
  ...
  if (rppForMe->getPushDownRequirement() == NULL AND rppForMe->executeInDP2() AND childIndex == 0) {
    // This must be the top CS and we are optimizing the left child.
    // Add a null-state required push-down property.
    rg.addPushDownRequirement(new (CmpCommon::statementHeap()) PushDownCSRequirement());
  }
  ...
  if (childIndex ==
      ...
      NULL
          ...
          AND childContext->hasOptimalSolution()) {
    // clean the rg.
    rg.removeAllPartitioningRequirements();
    ...
    if (rppForMe->executeInDP2()) {
      ...
      PartitioningRequirement *partReqForChild = childPartFunc->makePartitioningRequirement();

      CMPASSERT(partReqForChild->castToFullySpecifiedPartitioningRequirement());
...
Context *Pack::createContextForAChild(Context *myContext, PlanWorkSpace *pws, int &childIndex) {
  ...
  if (NOT rppForMe->executeInDP2() OR NOT isinBlockStmt()) rg.addNumOfPartitions(1);
...
// to the cost estimation functions for nodes of this class.
// -----------------------------------------------------------------------
CostMethod *IsolatedScalarUDF::costMethod() const {
...
// to the cost estimation functions for nodes of this class.
// -----------------------------------------------------------------------
CostMethod *PhysicalIsolatedScalarUDF::costMethod() const {
  /*static THREAD_P CostMethodIsolatedScalarUDF *m = NULL;
  if (m == NULL)
    m = new (GetCliGlobals()->exCollHeap()) CostMethodIsolatedScalarUDF();
  return m;*/
...
PhysicalProperty *IsolatedScalarUDF::synthPhysicalProperty(const Context *myContext, const int planNumber,
  ...
  NABoolean canRunInParallel = (getRoutineDesc()->getEffectiveNARoutine()->getParallelism() == "AP");

  PlanExecutionEnum planExecutionLocation = canRunInParallel ? EXECUTE_IN_MASTER_AND_ESP : EXECUTE_IN_MASTER;
  PhysicalProperty *sppForMe =
      new (CmpCommon::statementHeap()) PhysicalProperty(myPartFunc, planExecutionLocation, SOURCE_TUPLE);
...
NABoolean TableMappingUDF::okToAttemptESPParallelism(const Context *myContext, /*IN*/
  ...
  NABoolean status = dllInteraction_->degreeOfParallelism(this, (TMUDFPlanWorkSpace *)pws, udfDoP);
  ...
  if (udfDoP != 0 && udfDoP != numOfESPs && !numOfESPsForced) {
    ...
    if (result)
      ...
      switch (parallelControlSetting) {
        ...
        default: {
          if (!numOfESPsForced)
            if (parallelControlSetting == DF_SYSTEM && reqdNumOfPartitions != ANY_NUMBER_OF_PARTITIONS ||
              ...
              numOfESPs = MINOF(udfDoP, 4 * maxDoP);
...
PartitioningFunction *TableMappingUDF::mapPartitioningFunction(const PartitioningFunction *partFunc,
                                                               NABoolean rewriteForChild0) {
...
NABoolean TableMappingUDF::isBigMemoryOperator(const PlanWorkSpace *pws, const int /*planNumber*/) {
  ...
  if (udfPWS->getUDRPlanInfo()) udfWriterDop = udfPWS->getUDRPlanInfo()->getDesiredDegreeOfParallelism();
...
PhysicalTableMappingUDF::PhysicalTableMappingUDF(int arity, CollHeap *oHeap /*= CmpCommon::statementHeap()*/)
    : TableMappingUDF(arity, NULL, oHeap), planInfo_(NULL) {
  if (oHeap)
    pCostMethod_ = new (oHeap) CostMethodTableMappingUDF();
  else
    pCostMethod_ = new (CmpCommon::statementHeap()) CostMethodTableMappingUDF();
}

PhysicalTableMappingUDF::PhysicalTableMappingUDF(const TableMappingUDF &other)
    : TableMappingUDF(other), planInfo_(NULL) {
  pCostMethod_ = new (CmpCommon::statementHeap()) CostMethodTableMappingUDF();
}
...
PlanWorkSpace *PhysicalTableMappingUDF::allocateWorkSpace() const {
  PlanWorkSpace *result = new (CmpCommon::statementHeap()) TMUDFPlanWorkSpace(getArity());
...
Context *PhysicalTableMappingUDF::createContextForAChild(Context *myContext, PlanWorkSpace *pws, int &childIndex) {
  ...
  NABoolean useAParallelPlan = okToAttemptESPParallelism(myContext, pws, childNumPartsRequirement,
                                                         childNumPartsAllowedDeviation, numOfESPsForced);
  ...
  if (useAParallelPlan)
    if (childPartReqType == SPECIFIED_PARTITIONING) {
      ...
      if (countOfPartitions > 1)
        partReqForChild = new (CmpCommon::statementHeap()) RequireReplicateViaBroadcast(countOfPartitions);
      else
        partReqForChild = new (CmpCommon::statementHeap()) RequireExactlyOnePartition();
  ...
  if (NOT rg.checkFeasibility()) {
    ...
    snprintf(reason, sizeof(reason), "%s, use %d parallel streams in context %s",
             getUserTableName().getCorrNameAsString().data(), childNumPartsRequirement,
...
PhysicalProperty *PhysicalTableMappingUDF::synthPhysicalProperty(const Context *myContext, const int planNumber,
  ...
  if (arity == 0) {
    ...
    NABoolean useAParallelPlan =
        okToAttemptESPParallelism(myContext, pws, numOfESPs, allowedDeviation, numOfESPsForced);
    ...
    while (!foundChildToUse && childToUse < arity) {
      sppOfChild = myContext->getPhysicalPropertyOfSolutionForChild(childToUse);
      childPartFunc = sppOfChild->getPartitioningFunction();

      if (childPartFunc && !childPartFunc->isAReplicationPartitioningFunction())
        foundChildToUse = TRUE;
      else
        childToUse++;
    }

    if (!foundChildToUse || childPartFunc->isASinglePartitionPartitioningFunction()) {
      createSinglePartFunc = TRUE;
    } else {
      ...
      if (passThruCols.contains(childPartFunc->getPartitioningKey())) {
        ...
        // output columns
        myPartFunc = childPartFunc->copyAndRemap(udfOutputToChildInputMap_, TRUE);
      } else {
  ...
  if (createSinglePartFunc) {
    ...
    //-----------------------------------------------------------
    NodeMap *myNodeMap =
        new (CmpCommon::statementHeap()) NodeMap(CmpCommon::statementHeap(), numOfESPs, NodeMapEntry::ACTIVE);

    // set node numbers in the entries, if we need to have one
    // ESP per node
    if (numOfESPs == CURRSTMT_OPTDEFAULTS->getTotalNumberOfCPUs())
      for (int i = 0; i < numOfESPs; i++) myNodeMap->getNodeMapEntry(i)->setNodeNumber(i);
    ...
    randNum->synthTypeAndValueId();
    partKey.insert(randNum->getValueId());
    myPartFunc = new (CmpCommon::statementHeap()) Hash2PartitioningFunction(partKey, partKey, numOfESPs, myNodeMap);
    myPartFunc->createPartitioningKeyPredicates();
  }

  PhysicalProperty *sppForMe =
      new (CmpCommon::statementHeap()) PhysicalProperty(myPartFunc, EXECUTE_IN_MASTER_AND_ESP, SOURCE_VIRTUAL_TABLE);

  sppForMe->setUDRPlanInfo(static_cast<TMUDFPlanWorkSpace *>(pws)->getUDRPlanInfo());
...
NABoolean RelExpr::isBigMemoryOperator(const PlanWorkSpace *pws, const int) {
  ...
  if (spp == NULL || CmpCommon::getDefault(COMP_BOOL_51) != DF_ON) return FALSE;
...
Context *ConnectBy::createContextForAChild(Context *myContext, PlanWorkSpace *pws, int &childIndex) {
  ...
  if (reqdOrder().entries() > 0) {
    ValueIdList newSortKey;
    getMap(childIndex).rewriteValueIdListDown(reqdOrder(), newSortKey);
    rg.addSortKey(newSortKey, ESP_SOT, dp2SortOrderPartReq);
  }
...
}  // QueryInvalidationFunc::synthPhysicalProperty()
