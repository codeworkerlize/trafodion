...
// an undefined symbol.
void displayCostMethod(const CostMethod &pf) { pf.display(); }
...
//
//==============================================================================
Cost *rollUpUnaryNonBlocking(const Cost &parentOnly, const Cost &childRollUp, const ReqdPhysicalProperty *const rpp) {
  //-----------------------
  ...
  //------------------------------------------------------------------------
  rollUp->cpfr() = blockingAdd(parentOnly.getCpfr(), childRollUp.getCpfr(), rpp);
  ...
  rollUp->cplr() =
      overlapAddUnary(parentOnly.getCplr(), childRollUp.getCplr() - childRollUp.getCpfr()) + childRollUp.getCpfr();
  ...
  rollUp->cpbc1() = childRollUp.getCpbc1().getNormalizedVersion(parentNumProbes);
  rollUp->cpbcTotal() = childRollUp.getCpbcTotal().getNormalizedVersion(parentNumProbes);
...
//
//==============================================================================
Cost *rollUpUnaryBlocking(const Cost &parentOnly, const Cost &childRollUp, const ReqdPhysicalProperty *const rpp) {
  //-----------------------
  //  Create an empty cost.
  //-----------------------
  Cost *rollUp = new STMTHEAP Cost();
  ...
  if (childRollUp.getCpbc1().isZeroVectorWithProbes()) {
    rollUp->cpbc1() = overlapAddUnary(parentOnly.getCpbc1(), childRollUp.getCplr() / parentNumProbes);
  }
  ...
  //
  //----------------------------------------------------------------------
  rollUp->cpbcTotal() = blockingAdd(overlapAddUnary(parentOnly.getCpbc1(), childRollUp.getCplr() / parentNumProbes),
                                    childRollUp.getCpbcTotal().getNormalizedVersion(parentNumProbes), rpp);
...
// Print
void CostMethod::print(FILE *ofd, const char *indent, const char *title) const {
  BUMP_INDENT(indent);
  fprintf(ofd, "%s ", NEW_INDENT);
  if (title) fprintf(ofd, "%s ", title);
  if (className_) fprintf(ofd, "%s ", className_);
  fprintf(ofd, "\n ");
}  // CostMethod::print()

void CostMethod::display() const { print(); }
...
// -----------------------------------------------------------------------
void CostMethod::cleanUpAllCostMethods() {
  if (!CURRENTSTMT) return;

  for (CostMethod *cm = CURRENTSTMT->getCostMethodHead(); cm != NULL; cm = cm->nextCostMethod_) cm->cleanUp();
}
...
Cost *CostMethod::generateZeroCostObject() {
  // A zero cost vector.
  SimpleCostVector cv(csZero,               // CPU TIME
                      csZero,               // IO TIME
                      csZero,               // MSG TIME
                      csZero,               // idle time
...
NABoolean CostMethod::isUnderNestedJoin(RelExpr *op, const Context *myContext) {
  ...
  isUnderNestedJoin_ = (hasOuterReferences_ OR noOfProbes_.isGreaterThanOne() /* > csOne */);
...
void CostMethod::cacheParameters(RelExpr *op, const Context *myContext) {
  ...
  countOfPipelinesPerCPU_ = (rpp_ ? rpp_->getCountOfPipelines() : 1) / countOfAvailableCPUs_;
...
void CostMethod::estimateDegreeOfParallelism() {
  if (partFunc_ != NULL) {
    ...
    if ((partKey.entries() == 1) AND(randomFix != 0)) {
      ...
      if (myPartKeyCol.getItemExpr()->getOperatorType() == ITM_RANDOMNUM) {
        ...
        (CostScalar) countOfStreams_ = MINOF(activeStreams, (CostScalar)countOfStreams_);
      }
    }
  }
  // ---------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------
    ...
    if ((partReq_ != NULL) AND(partReq_->getCountOfPartitions() != ANY_NUMBER_OF_PARTITIONS)) {
  ...
  if ((CURRSTMT_OPTDEFAULTS->incorporateSkewInCosting()) && (partFunc_ != NULL)) {
    noOfProbesPerStream_ = inLogProp_->getCardOfBusiestStream(partFunc_, countOfStreams_, NULL, countOfAvailableCPUs_);
  } else
...
void CostMethod::determineCpuCountAndFragmentsPerCpu(int &cpuCount, int &fragmentsPerCpu) {
  ...
  //------------------------------------------------------------------
  cpuCount = (countOfStreams_ < countOfAvailableCPUs_ ? countOfStreams_ : countOfAvailableCPUs_);
...
//
//==============================================================================
void CostMethod::computePartialPlanCost(const RelExpr *op, PlanWorkSpace *pws, const Context *myContext) {
  //----------------------------------------------------------------------
  // Defensive programming.
  //----------------------------------------------------------------------
  CMPASSERT(op != NULL);
  ...
  if (CmpCommon::getDefault(SIMPLE_COST_MODEL) == DF_ON)
    partialPlanCost = scmRollUp(parentCost, knownChildrenCost, NULL, myContext->getReqdPhysicalProperty());
  else
    partialPlanCost = rollUp(parentCost, knownChildrenCost, myContext->getReqdPhysicalProperty());
...
Cost *CostMethod::computePlanCost(RelExpr *op, const Context *myContext, const PlanWorkSpace *pws, int planNumber) {
  ...
  for (int childIdx = 0; childIdx < op->getArity(); childIdx++) {
    ...
    //---------------------------------------------
    mergedChildCost.mergeOtherChildCost(*childContext->getSolution()->getRollUpCost());
  }
  ...
  Cost *planCost = rollUp(parentCost, &mergedChildCost, myContext->getReqdPhysicalProperty());
...
void CostMethod::getChildCostsForBinaryOp(RelExpr *op, const Context *myContext, const PlanWorkSpace *pws,
  ...
  if (childContext == NULL) {
    ABORT("CostMethod::getChildCostsForBinaryOp(): Left child has a NULL context");
  }
  ...
  // coverity[var_deref_model]
  if (NOT childContext->hasOptimalSolution()) {
    ABORT("CostMethod::getChildCostsForBinaryOp(): Left child has no solution");
  }

  leftChildCost = new STMTHEAP Cost(*childContext->getSolution()->getRollUpCost());
  ...
  if (childContext == NULL) {
    ABORT("CostMethod::getChildCostsForBinaryOp(): Right child has a NULL context");
  }
  ...
  // coverity[var_deref_model]
  if (NOT childContext->hasOptimalSolution()) {
    ABORT("CostMethod::getChildCostsForBinaryOp(): Right child has no solution");
  }
...
void CostMethod::getChildCostForUnaryOp(RelExpr *op, const Context *myContext, const PlanWorkSpace *pws, int planNumber,
  ...
  if (childContext == NULL) {
    ABORT("CostMethod::getChildCostForUnaryOp():  child has a NULL context");
  }
  ...
  // coverity[var_deref_model]
  if (NOT childContext->hasOptimalSolution()) {
    ABORT("CostMethod::getChildCostForUnaryOp():  child has no solution");
  }

  childCost = new STMTHEAP Cost(*childContext->getSolution()->getRollUpCost());
}
...
//
//==============================================================================
Cost *CostMethod::rollUp(Cost *const parentCost, Cost *const childCost, const ReqdPhysicalProperty *const rpp) {
  //------------------------------------------------------------------
...
Cost *CostMethod::rollUpForBinaryOp(RelExpr *op, const Context *myContext, const PlanWorkSpace *pws, int planNumber) {
  ...
  if (leftChildCost->getCpbcTotal().isZeroVectorWithProbes()) {
    if (rightChildCost->getCpbcTotal().isZeroVectorWithProbes()) {
      ...
      //-------------------------------------------------------
      mergedChildCost = mergeNoLegsBlocking(leftChildCost, rightChildCost, rpp);
    } else {
      //----------------------------------------------------------
      ...
      mergedChildCost = mergeBothLegsBlocking(blockingLeftChildCost, rightChildCost, rpp);
    ...
    if (rightChildCost->getCpbcTotal().isZeroVectorWithProbes()) {
      ...
      mergedChildCost = mergeBothLegsBlocking(leftChildCost, blockingRightChildCost, rpp);
      delete blockingRightChildCost;
    } else {
      //----------------------------------------------------------
      //  Both children have blocking operators in their subtrees.
      //----------------------------------------------------------
      mergedChildCost = mergeBothLegsBlocking(leftChildCost, rightChildCost, rpp);
    }
  }
...
//
//==============================================================================
Cost *CostMethod::mergeNoLegsBlocking(const CostPtr leftChildCost, const CostPtr rightChildCost,
...
//
//==============================================================================
Cost *CostMethod::convertToBlocking(const CostPtr nonBlockingCost) {
  //------------------------------------------------------
...
//
//==============================================================================
Cost *CostMethod::mergeBothLegsBlocking(const CostPtr leftChildCost, const CostPtr rightChildCost,
                                        const ReqdPhysicalProperty *const rpp) {
...
Cost *CostMethodExchange::computeOperatorCostInternal(RelExpr *op, const Context *myContext, int &countOfStreams) {
  ...
  if (NOT sppForMe_) return computeExchangeCostGoingDown(rpp, numOfProbes, countOfStreams);
  ...
  const PartitioningFunction *const myPartFunc = myContext->getPlan()->getPhysicalProperty()->getPartitioningFunction();
  ...
  //------------------------------------------------------------------
  CostScalar downMessages = computeDownMessages(numOfProbes, executeInDP2, messageHeaderInKb, messageBufferSizeInKb,
                                                numOfPartitions, numOfConsumers, myContext, downMessageLength_);
  ...
  CostScalar upMessages = computeUpMessages(
      myContext, exch, myPartFunc, childPartFunc, sppForChild, messageSpacePerRecordInKb, messageHeaderInKb,
      messageBufferSizeInKb, numOfConsumers, executeInDP2, upRowsPerConsumer_, numOfContinueDownMessages_);

  // each continue message is about 60 bytes
  if (CmpCommon::getDefault(COMP_BOOL_60) == DF_ON) downMessages = downMessages + numOfContinueDownMessages_;

  downMessageLength_ = downMessageLength_ + CostScalar(60) * numOfContinueDownMessages_;
  ...
  //-------------------------------------------------------------------------
  CostScalar downIntraNodeMessages, downInterNodeMessages, downRemoteNodeMessages, upIntraNodeMessages,
      upInterNodeMessages, upRemoteNodeMessages;
  categorizeMessages(myPartFunc, childPartFunc, executeInDP2, downMessages, upMessages, downIntraNodeMessages,
                     downInterNodeMessages, downRemoteNodeMessages, upIntraNodeMessages, upInterNodeMessages,
  ...
  produceCostVectors(numOfProbes, numOfConsumers, numOfProducers, executeInDP2, myPartFunc, childPartFunc,
                     messageSpacePerRecordInKb, messageHeaderInKb, messageBufferSizeInKb, upRowsPerConsumer_,
                     downIntraNodeMessages, downInterNodeMessages, downRemoteNodeMessages, upIntraNodeMessages,
                     upInterNodeMessages, upRemoteNodeMessages, myFR, myLR, childFR, childLR);
  ...
  CostScalar ocbAdjustFactor_2 = (ActiveSchemaDB()->getDefaults()).getAsDouble(COMP_FLOAT_5);
  CostScalar ocbAdjustFactor_3 = (ActiveSchemaDB()->getDefaults()).getAsDouble(COMP_INT_34);
  CostScalar ocbAdjustFactor_4 = (ActiveSchemaDB()->getDefaults()).getAsDouble(COMP_INT_35);
  CostScalar ocbAdjustFactor_5 = (ActiveSchemaDB()->getDefaults()).getAsDouble(COMP_INT_36);
  ...
  NABoolean isUnderNestedJoin = (hasOuterReferences OR numOfProbes.isGreaterThanOne());
  ...
  //--------------------------------------------------------------
  Cost *exchangeCost = computeExchangeCost(myFR, myLR, childFR, childLR, numOfConsumers, numOfProducers);
...
//------------------------------------------------------------------------
CostScalar CostMethodExchange::computeESPCost(const NABoolean executeInESP, const CostScalar &numOfProbes) const {
...
Cost *CostMethodExchange::computeExchangeCostGoingDown(const ReqdPhysicalProperty *rpp, const CostScalar &numOfProbes,
  ...
  SimpleCostVector cv(cpuTime, csZero, /* no IOTime */
                      csZero,          /* no MSGTime */
                      csZero,          /* idle time */
                      numOfProbes      /* num probes */
  );
  ...
  if ((myPartReq != NULL) AND(myPartReq->getCountOfPartitions() != ANY_NUMBER_OF_PARTITIONS)) {
...
//
//==============================================================================
void CostMethodExchange::getDefaultValues(const NABoolean executeInDP2, const Exchange *exch,
                                          CostScalar &messageSpacePerRecordInKb, CostScalar &messageHeaderInKb,
  ...
  //-------------------------------------------------------------------------
  if (executeInDP2)
    messageBufferSizeInKb = CostPrimitives::getBasicCostFactor(DP2_MESSAGE_BUFFER_SIZE);
  else
    messageBufferSizeInKb = CostPrimitives::getBasicCostFactor(OS_MESSAGE_BUFFER_SIZE);
  ...
  //-----------------------------------------------------------------
  const CostScalar recordHeaderInBytes = defs.getAsLong(DP2_MESSAGE_HEADER_SIZE_BYTES);  // 18

  //--------------------------------------------------------------------
  //  Compute size (in KB) of a row including its associated row header.
  //--------------------------------------------------------------------
  const GroupAttributes *childGA = exch->child(0).getGroupAttr();
  messageSpacePerRecordInKb = (recordHeaderInBytes + childGA->getRecordLength()) / csOneKiloBytes;
  ...
  if (CmpCommon::getDefault(COMP_BOOL_140) == DF_OFF) {
    CMPASSERT((CostScalar)CostPrimitives::getBasicCostFactor(LOCAL_MESSAGE_BUFFER_SIZE) >=
...
//
//==============================================================================
CostScalar CostMethodExchange::computeDownMessages(const CostScalar &numOfProbes, const NABoolean executeInDP2,
  ...
  if (CmpCommon::getDefault(COMP_BOOL_60) == DF_ON) {
    return computeDownDataAndControlMessages(numOfProbes, executeInDP2, messageHeaderInKb, messageBufferSizeInKb,
                                             numOfPartitions, numOfConsumers, myContext, downMessageLength);
  }
  ...
  //----------------------------------------------------
  const CostScalar endOfBufferHeaderInKb = defs.getAsDouble(DP2_END_OF_BUFFER_HEADER_SIZE);  // 32./1024 == 0.0313

  const CostScalar requestInKb = defs.getAsDouble(DP2_EXCHANGE_REQUEST_SIZE);  // 48/1024 == 0.0469
  ...
  if (childPartFunc->isALogPhysPartitioningFunction())
    phys = childPartFunc->castToLogPhysPartitioningFunction()->getPhysPartitioningFunction();
  ...
  if ((CURRSTMT_OPTDEFAULTS->incorporateSkewInCosting()) && (phys != NULL)) {
    ...
    if (sppForChild->getDP2CostThatDependsOnSPP())
      probesFromBusStream = sppForChild->getDP2CostThatDependsOnSPP()->getProbesAtBusiestStream();
    probesToBeCosted = MAXOF(probesFromBusStream, probesToBeCosted);
  }

  const CostScalar numOfProbesPerPartitionInKb = probesToBeCosted * requestInKb;

  DP2CostDataThatDependsOnSPP *dp2CostInfo = (DP2CostDataThatDependsOnSPP *)sppForChild->getDP2CostThatDependsOnSPP();
  ...
  //------------------------------------------------------------
  CostScalar downMessagesPerPartition = numOfProbesPerPartitionInKb / (messageBufferSizeInKb - endOfBufferHeaderInKb);
...
//****************************************************************************
CostScalar CostMethodExchange::computeDownDataAndControlMessages(
    const CostScalar &numOfProbes, const NABoolean executeInDP2, const CostScalar &messageHeaderInKb,
    const CostScalar &messageBufferSizeInKb, const CostScalar &numOfPartitions, const CostScalar &numOfConsumers,
    const Context *myContext, CostScalar &downMessageLength) const {
  CostScalar inputRowSize = op_->getGroupAttr()->getCharacteristicInputs().getRowLength();
  if (NOT executeInDP2) {
    // if (NOT inputRowSize.isLessThanOne())
    // Assume minimum of 28 bytes with each probe
    inputRowSize = inputRowSize + CostScalar(28);

    CostScalar numConnections = MINOF(ActiveSchemaDB()->getDefaults().getAsLong(GEN_SNDT_NUM_BUFFERS),
                                      ActiveSchemaDB()->getDefaults().getAsLong(GEN_SNDB_NUM_BUFFERS));

    CostScalar numOfMessages = CostScalar(2) * numOfConsumers * numOfPartitions * numConnections;

    downMessageLength = CostScalar(64) * numOfPartitions * numOfConsumers +  // A
                        inputRowSize * numOfPartitions * numOfConsumers +    // B
  ...
  CostScalar inputSizeInKb = numOfProbes1 * requestInKb + numOfProbes1 * inputRowSize / 1024;

  // assume data messages are buffered when computing number of messages
  CostScalar downDataMessages = (inputSizeInKb / (messageBufferSizeInKb - endOfBufferHeaderInKb)).minCsOne();
  ...
  NABoolean isGroupingDone =
      isGroupedRepartitioning(childPartFunc, parentPartFunc, parentGroupsChildren, paPartsPerGroup);

  NABoolean inAType2Join = FALSE;
  if (parentPartFunc->isAReplicateNoBroadcastPartitioningFunction() && myContext->getInputPhysicalProperty()) {
    const PartitioningFunction *const njParentPartFunc =
        myContext->getInputPhysicalProperty()->getNjOuterOrderPartFuncForNonUpdates();

    if (njParentPartFunc != NULL) {
      isGroupingDone = isGroupedRepartitioning(childPartFunc, njParentPartFunc, parentGroupsChildren, paPartsPerGroup);
      if (isGroupingDone) inAType2Join = TRUE;
    }
  }
  ...
  if (NOT isUnderNestedJoin) {
    ...
    if (compressionFactor == csZero) compressionFactor = messageBufferSizeInKb;
    ...
    if (dp2CostInfo != NULL) {
      switch (dp2CostInfo->getRepeatCountState()) {
        case DP2CostDataThatDependsOnSPP::KEYCOLS_COVERED_BY_CONST: {
          ...
          downDataMessages = downDataMessages * (messageBufferSizeInKb / compressionFactor.minCsOne()).minCsOne();
          break;
        }
        case DP2CostDataThatDependsOnSPP::KEYCOLS_COVERED_BY_PROBE_COLS_CONST: {
          ...
          if (parentPartFunc->isAReplicateNoBroadcastPartitioningFunction()) {
            CostScalar actualMessages = MINOF(numOfPartitions * numOfConsumers, numOfProbes);

            if (inAType2Join) {
              actualMessages = MINOF(actualMessages, numOfConsumers * paPartsPerGroup);
        ...
        case DP2CostDataThatDependsOnSPP::UNKNOWN: {
          downControlMessageLength = CostScalar(10000) * numOfPartitions * numOfConsumers;
          downControlMessages = numOfPartitions * numOfConsumers;

          if (myContext->getReqdPhysicalProperty()->getOcbEnabledCostingRequirement()) {
            downControlMessages = MINOF(numOfPartitions * numOfConsumers, numOfProbes);
            CostScalar interMedLength = CostScalar(10000) * numOfProbes;
            downControlMessageLength = MINOF(downControlMessageLength, interMedLength);
          }
...
//
//==============================================================================
CostScalar CostMethodExchange::computeUpMessages(
    const Context *parentContext, Exchange *exch, const PartitioningFunction *parentPartFunc,
    ...
    const CostScalar &messageBufferSizeInKb, const CostScalar &numOfConsumers, const NABoolean executeInDP2,
    CostScalar &upRowsPerConsumer, CostScalar &numOfContinueDownMessages) const {
  //----------------------------------------------------------------------
  // Defensive programming.
  //----------------------------------------------------------------------
  ...
  if (CURRSTMT_OPTDEFAULTS->incorporateSkewInCosting()) {
    ...
    upRowsPerConsumer = childOutputLP->getResultCardinality() / numOfConsumers;
  ...
  CostScalar upMessagesPerConsumer =
      (upRowsPerConsumer / ((bufferLength - messageHeaderInKb) / messageSpacePerRecordInKb).getFloor()).getCeiling();

  numOfContinueDownMessages = csZero;

  if (NOT upMessagesPerConsumer.isLessThanOne())
    numOfContinueDownMessages = (upMessagesPerConsumer - csOne).getCeiling() * numOfConsumers;
  if (NOT executeInDP2) {
    //----------------------------------------------------------
...
//
//**************************************************************************
NABoolean CostMethodExchange::isGroupedRepartitioning(const PartitioningFunction *childPartFunc,
  ...
  if (isParentSinglePF && isChildSinglePF) return FALSE;

  LogPhysPartitioningFunction *logPhys =
      (LogPhysPartitioningFunction *)childPartFunc->castToLogPhysPartitioningFunction();
  ...
  return ((childGroupsParent OR parentGroupsChildren)AND partsPerGroup >= 1);
}

void CostMethodExchange::categorizeMessagesForDP2(
    const PartitioningFunction *parentPartFunc, const PartitioningFunction *childPartFunc,
    const CostScalar &downMessages, const CostScalar &upMessages, CostScalar &downIntraCpuMessages,
    CostScalar &downIntraSegmentMessages, CostScalar &downRemoteSegmentMessages, CostScalar &upIntraCpuMessages,
    CostScalar &upIntraSegmentMessages, CostScalar &upRemoteSegmentMessages) const

{
  ...
  NABoolean areThereMultiSegments = childNodeMap->isMultiCluster(0, childNodeMap->getNumEntries(), TRUE);
  if (!areThereMultiSegments)
    areThereMultiSegments = parentNodeMap->isMultiCluster(0, parentNodeMap->getNumEntries(), TRUE);

  // number of producers, number of consumers
  // can't use nodemaps here
  const CostScalar &numOfConsumers = ((NodeMap *)(parentPartFunc->getNodeMap()))->getNumActivePartitions();
  const CostScalar &numOfPartitions = ((NodeMap *)(childPartFunc->getNodeMap()))->getEstNumActivePartitionsAtRuntime();

  const CostScalar &numOfProducers = MINOF(numOfPartitions, sppForChild_->getCurrentCountOfCPUs());

  const LogPhysPartitioningFunction *childLppf = childPartFunc->castToLogPhysPartitioningFunction();
  ...
  NABoolean isPAPartGroup =
      (childLppf != NULL && (childLppf->getLogPartType() == LogPhysPartitioningFunction::PA_PARTITION_GROUPING));

  NABoolean parentGroupsChildren = FALSE;
  CostScalar paPartsPerGroup;

  if ((numOfConsumers == csOne) && NOT areThereMultiSegments && (numOfProducers >= csOne))
  ...
  } else if ((numOfConsumers == csOne) && areThereMultiSegments) {
    // case 2: serial plan, multi-segment; same as Case 1
    CostScalar numChildSegments = getNumberofSegments(childNodeMap);
    downIntraCpuMessages = downMessages / numOfProducers;
    downIntraSegmentMessages = downMessages / numChildSegments - downIntraCpuMessages;

    downRemoteSegmentMessages = downMessages - downIntraSegmentMessages - downIntraCpuMessages;

    upIntraCpuMessages = upMessages / numOfProducers;
    upIntraSegmentMessages = upMessages / numChildSegments - upIntraCpuMessages;
    upRemoteSegmentMessages = upMessages - upIntraSegmentMessages - upIntraCpuMessages;

  } else if ((areThereMultiSegments && parentPartFunc->isAReplicateNoBroadcastPartitioningFunction()) ||
             (!areThereMultiSegments && parentPartFunc->isAReplicateNoBroadcastPartitioningFunction()))
  ...
  {
    CostScalar localMessageWeight = computeLocalMessageWeight(childNodeMap, parentNodeMap);
    CostScalar intraNodeWeightFactor = csOne / maxDegree;

    if (areThereMultiSegments) {
      downIntraCpuMessages = downMessages * localMessageWeight * (csOne - intraSegmentFactor);

      downIntraSegmentMessages = downMessages * localMessageWeight * intraSegmentFactor;

      downRemoteSegmentMessages = downMessages - downIntraSegmentMessages - downIntraCpuMessages;

      upIntraCpuMessages = upMessages * localMessageWeight * (csOne - intraSegmentFactor);

      upIntraSegmentMessages = upMessages * localMessageWeight * intraSegmentFactor;
  ...
  } else if (areThereMultiSegments && numOfConsumers <= numOfProducers &&
             isGroupedRepartitioning(childPartFunc, parentPartFunc, parentGroupsChildren, paPartsPerGroup)) {
  ...
  // 2 x 16, 4x16, 8 x 16, 16x16 etc.
  {
    ...
    // work
    CostScalar intraSegmentWeight = computeLocalMessageWeight(childNodeMap, parentNodeMap);

    downIntraCpuMessages = downMessages * intraSegmentWeight * (csOne - intraSegmentFactor);

    downIntraSegmentMessages = downMessages * intraSegmentWeight * intraSegmentFactor;

    downRemoteSegmentMessages = downMessages - downIntraSegmentMessages - downIntraCpuMessages;

    upIntraCpuMessages = upMessages * intraSegmentWeight * (csOne - intraSegmentFactor);
    upIntraSegmentMessages = upMessages * intraSegmentWeight * intraSegmentFactor;

    upRemoteSegmentMessages = upMessages - upIntraCpuMessages - upIntraSegmentMessages;
  }
...
//****************************************************************************
CostScalar CostMethodExchange::getNumberofSegments(const NodeMap *childNodeMap) const {
...
//**************************************************************************
CostScalar CostMethodExchange::computeLocalMessageWeight(const NodeMap *childNodeMap,
...
void CostMethodExchange::categorizeMessagesForESP(
  ...
  const NodeMap *childNodeMap = childPartFunc->getNodeMap();
  NABoolean multiSegmentsChild = childNodeMap->isMultiCluster(0, childNodeMap->getNumEntries(), TRUE);

  const NodeMap *parentNodeMap = parentPartFunc->getNodeMap();
  NABoolean multiSegmentsParent = parentNodeMap->isMultiCluster(0, parentNodeMap->getNumEntries(), TRUE);
  ...
  if (areThereMultiSegments) {
    intraSegmentWeight = computeLocalMessageWeight(childNodeMap, parentNodeMap);
  }

  NABoolean parentGroupsChildren;
  CostScalar paPartsPerGroup;
  NABoolean isPAGrouping =
      isGroupedRepartitioning(childPartFunc, parentPartFunc, parentGroupsChildren, paPartsPerGroup);

  // we are not interested in groups of 1 for ESP Exchange
  if (paPartsPerGroup == csOne) isPAGrouping = FALSE;

  if (areThereMultiSegments && !isPAGrouping) {
    downIntraCpuMessages = downMessages * intraSegmentWeight * (csOne - intraSegmentFactor);
    ...
    const CostScalar &numOfPartitions = ((NodeMap *)(childPartFunc->getNodeMap()))->getNumActivePartitions();
...
//
//==============================================================================
void CostMethodExchange::categorizeMessages(const PartitioningFunction *parentPartFunc,
                                            const PartitioningFunction *childPartFunc, const NABoolean executeInDP2,
  ...
  // CQD, otherwise get the number of nodes per cluster from NAClusterInfo.
  //-------------------------------------------------------------------------

  CollIndex numOfNodesInActiveClusters =
      (CURRSTMT_OPTDEFAULTS->isFakeHardware() ? defs.getAsLong(DEF_NUM_NODES_IN_ACTIVE_CLUSTERS)
                                              : CmpCommon::context()->getNumOfSMPs());
  ...
  if (CmpCommon::getDefault(COMP_BOOL_60) == DF_ON) {
    if (executeInDP2)
      categorizeMessagesForDP2(parentPartFunc, childPartFunc, downMessages, upMessages, downIntraNodeMessages,
                               downInterNodeMessages, downRemoteNodeMessages, upIntraNodeMessages, upInterNodeMessages,
                               upRemoteNodeMessages);
    else
      categorizeMessagesForESP(parentPartFunc, childPartFunc, downMessages, upMessages, downIntraNodeMessages,
                               downInterNodeMessages, downRemoteNodeMessages, upIntraNodeMessages, upInterNodeMessages,
                               upRemoteNodeMessages);
    return;
  }
  ...
  // (when CB_60 on) is fully tested and will be used all the time.
    ...
    if (childLppf != NULL && (childLppf->getLogPartType() == LogPhysPartitioningFunction::PA_PARTITION_GROUPING ||
                              childLppf->getLogPartType() == LogPhysPartitioningFunction::LOGICAL_SUBPARTITIONING)) {
      useNodeMaps = TRUE;
  ...
  if (useNodeMaps && parentNodeMap->allNodesSpecified() && childNodeMap->allNodesSpecified() &&
    ...
    ((NodeMap *)childNodeMap)->deriveGrouping(parentNodeMap->getNumEntries(), groupStart, groupSize);
    ...
    for (CollIndex parentIdx = 0; parentIdx < parentNodeMap->getNumEntries(); parentIdx++) {
      //------------------------------------------------------------
      //  Only count messages that pertain to active parent entries.
      //------------------------------------------------------------
      if (parentNodeMap->isActive(parentIdx)) {
        for (CollIndex childIdx = groupStart[parentIdx]; childIdx < groupStart[parentIdx] + groupSize[parentIdx];
             childIdx++) {
          //-----------------------------------------------------------
          ...
          if (childNodeMap->isActive(childIdx)) {
            ...
            //------------------------------------------------------
            if (parentNodeMap->getClusterNumber(parentIdx) != childNodeMap->getClusterNumber(childIdx)) {
              remoteNode++;
            }
            //------------------------------------------------------
            ...
            else if (parentNodeMap->getNodeNumber(parentIdx) == childNodeMap->getNodeNumber(childIdx)) {
              intraNode++;
            } else {
              interNode++;
            }
  ...
  // active clusters.
  //----------------------------------------------------------------------
  const CollIndex activeChildNodes = ((NodeMap *)childNodeMap)->getEstNumActivePartitionsAtRuntime();
  const CollIndex activeParentNodes = ((NodeMap *)parentNodeMap)->getNumActivePartitions();
  ...
  CostScalar activeClusterInNetwork = CURRCONTEXT_CLUSTERINFO->getNumActiveCluster();
  if (CURRSTMT_OPTDEFAULTS->isFakeHardware()) activeClusterInNetwork = csOne;
  //-------------------------------------------------------------------------
...
void CostMethodExchange::produceCostVectors(
    ...
    const CostScalar &upInterNodeMessages, const CostScalar &upRemoteNodeMessages, CostVecPtr &parentFR,
    CostVecPtr &parentLR, CostVecPtr &childFR, CostVecPtr &childLR) const {
  if (CmpCommon::getDefault(COMP_BOOL_60) == DF_ON) {
    return produceCostVectorsWithControlDataMessages(
        numOfProbes, numOfConsumers, numOfProducers, childExecutesInDP2, myPartFunc, childPartFunc,
        messageSpacePerRecordInKb, messageHeaderInKb, messageBufferSizeInKb, upRowsPerConsumer_, downIntraNodeMessages,
        downInterNodeMessages, downRemoteNodeMessages, upIntraNodeMessages, upInterNodeMessages, upRemoteNodeMessages,
        parentFR, parentLR, childFR, childLR);
  }
  ...
  //-------------------------------------------------------------------------
  const CostScalar instrPerByte = CostPrimitives::getBasicCostFactor(CPUCOST_COPY_ROW_PER_BYTE) +
                                  CostPrimitives::getBasicCostFactor(CPUCOST_EXCHANGE_COST_PER_BYTE);

  const CostScalar interNodeInstrPerByte = CostPrimitives::getBasicCostFactor(CPUCOST_EXCHANGE_INTERNODE_COST_PER_BYTE);
  ...
  //---------------------------------------------------------------------------
  const CostScalar instrToCopyAMessage = (instrPerByte * messageBufferSizeInKb * csOneKiloBytes).getCeiling();
  ...
  const CostScalar intraNodeCopiesPerMessage = senderIntraNodeCopies + receiverIntraNodeCopies;
  const CostScalar interNodeCopiesPerMessage = senderInterNodeCopies + receiverInterNodeCopies;
  const CostScalar remoteNodeCopiesPerMessage = senderRemoteNodeCopies + receiverRemoteNodeCopies;
  ...
  CostScalar parentIntraNodeCopies =
      (downIntraNodeMessages * senderIntraNodeCopies) + (upIntraNodeMessages * receiverIntraNodeCopies);

  CostScalar parentInterNodeCopies =
      (downInterNodeMessages * senderInterNodeCopies) + (upInterNodeMessages * receiverInterNodeCopies);
  CostScalar parentRemoteNodeCopies =
      (downRemoteNodeMessages * senderRemoteNodeCopies) + (upRemoteNodeMessages * receiverRemoteNodeCopies);
  ...
  CostScalar interNode = csZero;
  CostScalar remoteNode = csZero;
  CostScalar downMessages = downIntraNodeMessages + downInterNodeMessages + downRemoteNodeMessages;
  ...
  //----------------------------------------------------------------
  CostScalar parentIntraNodeCopiesPerCPU = parentIntraNodeCopies / MIN_ONE_CS(intraNode);
  CostScalar parentInterNodeCopiesPerCPU = parentInterNodeCopies / MIN_ONE_CS(interNode);
  CostScalar parentRemoteNodeCopiesPerCPU = parentRemoteNodeCopies / MIN_ONE_CS(remoteNode);
  CostScalar downInterNodeMessagesPerCPU = downInterNodeMessages / MIN_ONE_CS(interNode);
  CostScalar downRemoteNodeMessagesPerCPU = downRemoteNodeMessages / MIN_ONE_CS(remoteNode);

  //-------------------------------------------------------
  //  Calculate number of bytes of down internode and intersystem messages.
  //-------------------------------------------------------
  CostScalar parentInterNodeMessagesInKbPerCPU = downInterNodeMessagesPerCPU * messageBufferSizeInKb;
  CostScalar parentRemoteNodeMessagesInKbPerCPU = downRemoteNodeMessagesPerCPU * messageBufferSizeInKb;

  //----------------------------------------------------------------
  //  Calculate number of total message copies and internode message
  // copies for producing the first row.
  //----------------------------------------------------------------
  const CostScalar &firstRowParentIntraNodeCopiesPerCPU = MINOF(parentIntraNodeCopiesPerCPU, intraNodeCopiesPerMessage);

  const CostScalar &firstRowParentInterNodeCopiesPerCPU = MINOF(parentInterNodeCopiesPerCPU, interNodeCopiesPerMessage);
  const CostScalar &firstRowParentRemoteNodeCopiesPerCPU =
      MINOF(parentRemoteNodeCopiesPerCPU, remoteNodeCopiesPerMessage);
  //----------------------------------------------------------------------------
  ...
  //----------------------------------------------------------------------------
  CostScalar parentMemory = messageBufferSizeInKb * (MINOF(parentInterNodeCopiesPerCPU, interNodeCopiesPerMessage) +
  ...
  parentFR->setInstrToCPUTime(firstRowParentIntraNodeCopiesPerCPU * instrToCopyAMessage);
  parentFR->addNumLocalToMSGTime(MINOF(csOne, downInterNodeMessagesPerCPU));
  parentFR->addKBLocalToMSGTime(MINOF(messageBufferSizeInKb, parentInterNodeMessagesInKbPerCPU));
  parentFR->addNumRemoteToMSGTime(MINOF(csOne, downRemoteNodeMessagesPerCPU));
  parentFR->addKBRemoteToMSGTime(MINOF(messageBufferSizeInKb, parentRemoteNodeMessagesInKbPerCPU));
  ...
  parentLR->setInstrToCPUTime((parentIntraNodeCopiesPerCPU * instrToCopyAMessage) +
                              (parentInterNodeCopiesPerCPU * interNodeInstrToCopyAMessage) +
                              (parentRemoteNodeCopiesPerCPU * remoteNodeInstrToCopyAMessage));
  ...
  if ((numOfConsumers == csOne) AND(numOfProducers > numOfConsumers) AND isOpBelowRoot_ AND upRowsPerConsumer_ > adj1) {
  ...
  CostScalar childIntraNodeCopies =
      (downIntraNodeMessages * receiverIntraNodeCopies) + (upIntraNodeMessages * senderIntraNodeCopies);

  CostScalar childInterNodeCopies =
      (downInterNodeMessages * receiverInterNodeCopies) + (upInterNodeMessages * senderInterNodeCopies);
  CostScalar childRemoteNodeCopies =
      (downRemoteNodeMessages * receiverInterNodeCopies) + (upRemoteNodeMessages * senderRemoteNodeCopies);

  // Divide total IntraNode messages by number of intra nodes.
  // Divide total InterNode messages by number of inter nodes.
  // Divide total RemoteNode messages by number of remote nodes.
  CostScalar upMessages = upIntraNodeMessages + upInterNodeMessages + upRemoteNodeMessages;
  if (CmpCommon::getDefault(COMP_BOOL_97) == DF_OFF) {
  ...
  //-------------------------------------------------------------------------
  CostScalar childIntraNodeCopiesPerCPU = childIntraNodeCopies / MIN_ONE_CS(intraNode);

  CostScalar childInterNodeCopiesPerCPU = childInterNodeCopies / MIN_ONE_CS(interNode);
  CostScalar childRemoteNodeCopiesPerCPU = childRemoteNodeCopies / MIN_ONE_CS(remoteNode);

  CostScalar upInterNodeMessagesPerCPU = upInterNodeMessages / MIN_ONE_CS(interNode);
  CostScalar upRemoteNodeMessagesPerCPU = upRemoteNodeMessages / MIN_ONE_CS(remoteNode);

  CostScalar childInterNodeMessagesInKbPerCPU = upInterNodeMessagesPerCPU * messageBufferSizeInKb;
  CostScalar childRemoteNodeMessagesInKbPerCPU = upRemoteNodeMessagesPerCPU * messageBufferSizeInKb;

  const CostScalar &firstRowChildIntraNodeCopiesPerCPU = MINOF(childIntraNodeCopiesPerCPU, intraNodeCopiesPerMessage);
  ...
  // for each copy of a message.
  //----------------------------------------------------------------------------
  CostScalar childMemory = messageBufferSizeInKb * (interNodeCopiesPerMessage + remoteNodeCopiesPerMessage);
  if (NOT childExecutesInDP2) {
  ...
  if (NOT childExecutesInDP2 AND numOfConsumers.isGreaterThanOne() /*> csOne*/
      AND(myPartFunc->isAHashPartitioningFunction() OR myPartFunc->isAHash2PartitioningFunction()
              OR myPartFunc->isAHashDistPartitioningFunction()
                  OR myPartFunc->isARangePartitioningFunction() OR myPartFunc->isASkewedDataPartitioningFunction())) {
    if (CmpCommon::getDefault(COMP_BOOL_97) == DF_OFF)
      cpuCostHashRow = CostPrimitives::cpuCostForHash(myPartFunc->getPartitioningKey());
    else
      cpuCostHashRow = CostPrimitives::cpuCostForHash(childPartFunc->getPartitioningKey());
  }

  // if the exchange is PAPA and for Hash2 and Hash1 pfs, we need to
  // cost hashing of incoming probes...
  if ((CmpCommon::getDefault(COMP_BOOL_57) == DF_OFF) AND childExecutesInDP2 AND(numOfProducers > numOfConsumers)
          AND childPartFunc->isALogPhysPartitioningFunction() AND numOfProbes.isGreaterThanOne()) {
    PartitioningFunction *phys = childPartFunc->castToLogPhysPartitioningFunction()->getPhysPartitioningFunction();
    ...
    if (phys->isAHash2PartitioningFunction() OR phys->isAHashDistPartitioningFunction()
            OR phys->isARangePartitioningFunction()) {
      Exchange *exch = (Exchange *)op_;
      NABoolean areProbesHashed = exch->areProbesHashed(childPartFunc->getPartitioningKey());
      if (areProbesHashed) {
        CostScalar cpuCostHashProbes = CostPrimitives::cpuCostForHash(childPartFunc->getPartitioningKey());
        parentLR->addInstrToCPUTime(cpuCostHashProbes * (numOfProbes / numOfConsumers).minCsOne());
      }
    }
  }

  if ((CmpCommon::getDefault(COMP_BOOL_57) == DF_OFF) AND isMergeNeeded_ AND numOfProducers > numOfConsumers) {
    CostScalar cpuCostCompareKeys = CostPrimitives::cpuCostForCompare(sppForMe_->getSortKey());
    parentLR->addInstrToCPUTime(cpuCostCompareKeys * upRowsPerConsumer_);
  }
  ...
  const CostScalar maxRowsPerMessage =
      ((messageBufferSizeInKb - messageHeaderInKb) / messageSpacePerRecordInKb).getFloor();

  const CostScalar &firstRowNumOfRows = MINOF(upRowsPerConsumer, maxRowsPerMessage);
  ...
  childFR->setInstrToCPUTime((firstRowChildIntraNodeCopiesPerCPU * instrToCopyAMessage) +
                             (firstRowNumOfRows * cpuCostHashRow));

  childFR->addNumLocalToMSGTime(MINOF(csOne, upInterNodeMessagesPerCPU));

  childFR->addKBLocalToMSGTime(MINOF(messageBufferSizeInKb, childInterNodeMessagesInKbPerCPU));
  childFR->addNumRemoteToMSGTime(MINOF(csOne, upRemoteNodeMessagesPerCPU));

  childFR->addKBRemoteToMSGTime(MINOF(messageBufferSizeInKb, childRemoteNodeMessagesInKbPerCPU));
  ...
  childLR->setInstrToCPUTime(
      (childIntraNodeCopiesPerCPU * instrToCopyAMessage) + (childInterNodeCopiesPerCPU * interNodeInstrToCopyAMessage) +
      (childRemoteNodeCopiesPerCPU * remoteNodeInstrToCopyAMessage) + (upRowsPerConsumer * cpuCostHashRow));
  ...
  CostScalar startupAdj = (ActiveSchemaDB()->getDefaults()).getAsDouble(COMP_INT_63);
  if (isOpBelowRoot_ && (NOT childExecutesInDP2) && (numOfProbes < 10) &&
      (CmpCommon::getDefault(COMP_BOOL_122) == DF_ON)) {
...
void CostMethodExchange::produceCostVectorsWithControlDataMessages(
    const CostScalar &numOfProbes, const CostScalar &numOfConsumers, const CostScalar &numOfProducers,
    const NABoolean childExecutesInDP2, const PartitioningFunction *myPartFunc,
    const PartitioningFunction *childPartFunc, const CostScalar &messageSpacePerRecordInKb,
    const CostScalar &messageHeaderInKb, const CostScalar &messageBufferSizeInKb, const CostScalar &upRowsPerConsumer,
  ...
  // cost of copying that buffer across a node or system boundary.
  //---------------------------------------------------------------------------
  const CostScalar instrToCopyA1KBMessage = (instrPerByte * csOneKiloBytes).getCeiling();
  const CostScalar intraSegmentInstrToCopyA1KBMessage = (intraSegmentInstrPerByte * csOneKiloBytes).getCeiling();
  const CostScalar remoteSegmentInstrToCopyA1KBMessage = (remoteSegmentInstrPerByte * csOneKiloBytes).getCeiling();

  CostScalar downMessages = downIntraCpuMessages + downRemoteSegmentMessages + downIntraSegmentMessages;
  ...
  downIntraSegmentMessagesLength = downMessageLength_ * downIntraSegmentMessages / normalizeFactor;

  downRemoteSegmentMessagesLength = downMessageLength_ * downRemoteSegmentMessages / normalizeFactor;

  downIntraCpuMessagesLength = downMessageLength_ * downIntraCpuMessages / normalizeFactor;
  /*else
  {
  ...
  const CostScalar intraCPUCopiesPerMessage = senderIntraCPUCopies + receiverIntraCPUCopies;
  const CostScalar interSegmentCopiesPerMessage = senderIntraSegmentCopies + receiverIntraSegmentCopies;
  const CostScalar remoteSegmentCopiesPerMessage = senderRemoteSegmentCopies + receiverRemoteSegmentCopies;
  ...
  //------------------------------------------------------------------
  CostScalar parentIntraCPUCopyMessageLength = (downIntraCpuMessagesLength * senderIntraCPUCopies) +
                                               (upIntraCPUMessages * upMessageBufferLength_ * receiverIntraCPUCopies);

  CostScalar parentIntraSegmentCopyMessageLength =
      (downIntraSegmentMessagesLength * senderIntraSegmentCopies) +
      (upIntraSegmentMessages * upMessageBufferLength_ * receiverIntraSegmentCopies);

  CostScalar parentRemoteSegmentCopyMessageLength =
      (downRemoteSegmentMessagesLength * senderRemoteSegmentCopies) +
      (upRemoteSegmentMessages * upMessageBufferLength_ * receiverRemoteSegmentCopies);
  ...
  // If so, then numOfConsumers <> no. of CPUs.-- Sunil
  //----------------------------------------------------------------
  CostScalar parentIntraCPUCopyMessageLengthPerConsumer = parentIntraCPUCopyMessageLength / intraNode.minCsOne();

  CostScalar parentIntraSegmentCopyLengthPerConsumer = parentIntraSegmentCopyMessageLength / interNode.minCsOne();

  CostScalar parentRemoteSegmentCopyLengthPerConsumer = parentRemoteSegmentCopyMessageLength / remoteNode.minCsOne();

  CostScalar downIntraSegmentMessagesPerConsumer = downIntraSegmentMessages / interNode.minCsOne();
  CostScalar downRemoteSegmentMessagesPerConsumer = downRemoteSegmentMessages / remoteNode.minCsOne();

  //-------------------------------------------------------
  //  Calculate number of kilobytes of down internode and intersegment
  //  messages. They are normalized
  //-------------------------------------------------------
  CostScalar parentIntraSegmentMessagesInKbPerConsumer = downIntraSegmentMessagesLength / interNode.minCsOne();

  CostScalar parentRemoteSegmentMessagesInKbPerConsumer = downRemoteSegmentMessagesLength / remoteNode.minCsOne();
  ...
  parentFR->setInstrToCPUTime(firstRowParentIntraNodeCopiesPerConsumer * instrToCopyA1KBMessage);

  parentFR->addNumLocalToMSGTime(MINOF(csOne, downIntraSegmentMessagesPerConsumer));

  parentFR->addKBLocalToMSGTime(MINOF(downMessageBufferLength_, parentIntraSegmentMessagesInKbPerConsumer));

  parentFR->addNumRemoteToMSGTime(MINOF(csOne, downRemoteSegmentMessagesPerConsumer));

  parentFR->addKBRemoteToMSGTime(MINOF(downMessageBufferLength_, parentRemoteSegmentMessagesInKbPerConsumer));
  ...
  CostScalar copyCost = parentIntraCPUCopyMessageLengthPerConsumer * instrToCopyA1KBMessage +
  ...
  //
  //------------------------------------------------------------------
  CostScalar childIntraCPUCopyMessageLength = (downIntraCpuMessagesLength * receiverIntraCPUCopies) +
                                              (upIntraCPUMessages * upMessageBufferLength_ * senderIntraCPUCopies);

  CostScalar childIntraSegmentCopyMessageLength =
      (downIntraSegmentMessagesLength * receiverIntraSegmentCopies) +
      (upIntraSegmentMessages * upMessageBufferLength_ * senderIntraSegmentCopies);

  CostScalar childRemoteSegmentCopyMessageLength =
      (downRemoteSegmentMessagesLength * receiverIntraSegmentCopies) +
      (upRemoteSegmentMessages * upMessageBufferLength_ * senderRemoteSegmentCopies);

  // Divide total IntraNode messages by number of intra nodes.
  // Divide total InterNode messages by number of inter nodes.
  // Divide total RemoteNode messages by number of remote nodes.
  CostScalar upMessages = upIntraCPUMessages + upIntraSegmentMessages + upRemoteSegmentMessages;
  if (CmpCommon::getDefault(COMP_BOOL_97) == DF_ON) {
  ...
  // MAXOF producers and senders. Normalize this to the number of CPUs in the child.
  //-------------------------------------------------------------------------
  CostScalar childIntraCPUCopyLengthPerProducer = childIntraCPUCopyMessageLength / intraNode.minCsOne();

  CostScalar childIntraSegmentCopyLengthPerProducer = childIntraSegmentCopyMessageLength / interNode.minCsOne();

  CostScalar childRemoteSegmentCopyLengthPerProducer = childRemoteSegmentCopyMessageLength / remoteNode.minCsOne();

  CostScalar childUpIntraSegmentMessagesPerProducer = upIntraSegmentMessages / interNode.minCsOne();

  CostScalar childUpRemoteSegmentMessagesPerProducer = upRemoteSegmentMessages / remoteNode.minCsOne();
  ...
  // for each copy of a message.
  //----------------------------------------------------------------------------
  CostScalar childMemory = messageBufferSizeInKb * (interSegmentCopiesPerMessage + remoteSegmentCopiesPerMessage);
  if (NOT childExecutesInDP2) {
    childMemory = childMemory * numOfConsumers;
  }
  ...
  //----------------------------------------------------------------------------
  const CostScalar maxRowsPerMessage =
      ((upMessageBufferLength_ - messageHeaderInKb) / messageSpacePerRecordInKb).getFloor();
  ...
  childFR->setInstrToCPUTime(firstRowChildIntraNodeCopiesPerProducer * instrToCopyA1KBMessage +
                             (firstRowNumOfRows * cpuCostHashRow));

  childFR->addNumLocalToMSGTime(MINOF(csOne, childUpIntraSegmentMessagesPerProducer));

  childFR->addKBLocalToMSGTime(MINOF(messageBufferSizeInKb, childIntraSegmentMessagesInKbPerProducer));
  childFR->addNumRemoteToMSGTime(MINOF(csOne, childUpRemoteSegmentMessagesPerProducer));

  childFR->addKBRemoteToMSGTime(MINOF(messageBufferSizeInKb, childRemoteSegmentMessagesInKbPerProducer));
  ...
  childLR->setInstrToCPUTime(copyCost + adjustment * cpuCostHashRow);
...
//
//==============================================================================
Cost *CostMethodExchange::computeExchangeCost(const CostVecPtr parentFR, const CostVecPtr parentLR,
                                              const CostVecPtr childFR, const CostVecPtr childLR,
  ...
  //------------------------------------------------------------------
  SimpleCostVector cvFR = childExchangeCost->getCpfr() + parentExchangeCost->getCpfr();

  SimpleCostVector cvLR = childExchangeCost->getCplr() + parentExchangeCost->getCplr();

  Cost *exchangeCost = new STMTHEAP Cost(&cvFR, &cvLR,
...
Cost *CostMethodFileScan::computeOperatorCostInternal(RelExpr *op, const Context *myContext, int &countOfStreams) {
  ...
  CostScalar resultSetCardinality =
      p->getGroupAttr()->outputLogProp(myContext->getInputLogProp())->getResultCardinality();
  // $$$ Due to a bug in histograms (up to tag A091197_1)
  // $$$ sometimes the cardinality is negative, if so, fix it
  // $$$ to pass regressions:
  if (resultSetCardinality.isLessThanZero() /* < csZero */) resultSetCardinality = CostScalar(100.0);

  ValueIdSet externalInputs(p->getGroupAttr()->getCharacteristicInputs());
  ...
  if (logPhysPartFunc != NULL) {
    ...
    if (logPartType == LogPhysPartitioningFunction::LOGICAL_SUBPARTITIONING OR logPartType ==
      ...
      externalInputs += logPhysPartFunc->getLogPartitioningFunction()->getPartitionInputValues();
    }
  }

  ScanOptimizer *scanOptimizer = ScanOptimizer::getScanOptimizer(*p /*in, not side-effected */
                                                                 ,
                                                                 resultSetCardinality /* in */
                                                                 ,
                                                                 *myContext, externalInputs, STMTHEAP);
  ...
  NABoolean isNestedJoin = (myContext->getInputLogProp()->getColStats().entries() > 0);

  // excluded for coverage because DEBUG only code
  if (CURRSTMT_OPTDEFAULTS->optimizerHeuristic2()) {  //#ifndef NDEBUG
    if (isNestedJoin) (*CURRSTMT_OPTGLOBALS->nestedJoinMonitor).enter();
  ...
  if (CURRSTMT_OPTDEFAULTS->optimizerHeuristic2()) {  //#ifndef NDEBUG
    ...
    if (isNestedJoin) (*CURRSTMT_OPTGLOBALS->nestedJoinMonitor).exit();
  }  //#endif
  ...
  // ------------------------------------------------------------------------
  const ReqdPhysicalProperty *rpp = myContext->getReqdPhysicalProperty();
  const LogicalPartitioningRequirement *lpr = rpp->getLogicalPartRequirement();
  PartitioningRequirement *logPartReq = NULL;
  if (lpr != NULL) logPartReq = lpr->getLogReq();
  const CostScalar &noOfProbes = (myContext->getInputLogProp()->getResultCardinality()).minCsOne();
  ValueIdSet outerRefs;
  externalInputs.getOuterReferences(outerRefs);

  if (logPartReq AND logPartReq
          ->isRequirementReplicateNoBroadcast()  // parallel n.j.
      AND(noOfProbes == 1)                       // 1 probe, but
...
// excluded for coverage because DEBUG only code
  ...
  if (CmpCommon::getDefault(OPTIMIZER_PRINT_COST) == DF_ON) {
    ...
    fprintf(pfp, "%f\n", costPtr->convertToElapsedTime(myContext->getReqdPhysicalProperty()).value());
    ...
    CollIndex HRSPartitions = scanOptimizer->getEstNumActivePartitionsAtRuntimeForHbaseRegions();
    CollIndex HCSPartitions = scanOptimizer->getEstNumActivePartitionsAtRuntime();
...
Cost *CostMethodDP2Scan::computeOperatorCostInternal(RelExpr *op, const Context *myContext, int &countOfStreams) {
  ...
  Cost *costPtr = CostMethodFileScan::computeOperatorCostInternal(op, myContext, countOfStreams);
...
Cost *CostMethodFixedCostPerRow::computeOperatorCostInternal(RelExpr *op, const Context *myContext,
  ...
  // ---------------------------------------------------------------------
  SimpleCostVector cvLR(cpu / countOfStreams_ * CURRSTMT_OPTDEFAULTS->getTimePerCPUInstructions(), csZero, csZero,
                        csZero, noOfProbesPerStream_);

  SimpleCostVector cvFR(cpu / countOfStreams_ / noOfProbesPerStream_              // converting CPU instr
                            * CURRSTMT_OPTDEFAULTS->getTimePerCPUInstructions(),  // into time
                        csZero, csZero, csZero, noOfProbesPerStream_);
  ...
  if (printCost) {
    fprintf(pfp, "Elapsed time: ");
    fprintf(pfp, "%f", costPtr->convertToElapsedTime(myContext->getReqdPhysicalProperty()).value());
    fprintf(pfp, "\n");
  }
#endif
...
// -----------------------------------------------------------------------
void CostMethodFixedCostPerRow::print(FILE *ofd, const char *indent, const char *title) const {
  BUMP_INDENT(indent);
  CostMethod::print(ofd, indent, title);
  fprintf(ofd, "%s ", NEW_INDENT);
  fprintf(ofd, "baseCpuCost=%g cpuCostPerChildRow=%g cpuCostPerOutputRow=%g", baseCpuCost_.value(),
          cpuCostPerChildRow_.value(), cpuCostPerOutputRow_.value());
...
// -----------------------------------------------------------------------
void CostMethodSort::cacheParameters(RelExpr *op, const Context *myContext) {
  ...
  // ---------------------------------------------------------------------
  cpuCostSendRow_ = CostPrimitives::cpuCostForEncode(sortKeyVis) + CostPrimitives::cpuCostForCopyRow(myRowLength);
  ...
  if (isBMO_) {
    ...
    // -------------------------------------------------------------------
    double memoryForEachRowAtRunGenPhase = (sortRecLength_ / 1024. + treeNodeSize_);
...
Cost *CostMethodSort::computeOperatorCostInternal(RelExpr *op, const Context *myContext, int &countOfStreams) {
  ...
  if ((CURRSTMT_OPTDEFAULTS->incorporateSkewInCosting()) && (partFunc_ != NULL)) {
    CostScalar myRowCountPerStream =
        myLogProp_->getCardOfBusiestStream(partFunc_, countOfStreams_, ga_, countOfAvailableCPUs_);
    ...
    rowCount = (myRowCount_ / countOfStreams_ / noOfProbesPerStream_).minCsOne();
  }
  ...
  if (((NOT isBMO_)OR(tableSize < memoryLimit_))) {
    // j memBK += tableSize;
    cpuBK += cpuCostCompareKeys_ * qsFludgeFactor_ * rowCount * log(rowCount.value());
  } else {
    ...
    if (maxMergeOrder_ < noOfRuns) {
      ...
      // -----------------------------------------------------------------
      CostScalar noOfIntPasses = log(noOfRuns.value()) / log(maxMergeOrder_.value()) - 1.;

      // -----------------------------------------------------------------
      // Sum of CPU costs for all merge sessions.
      // -----------------------------------------------------------------
      CostScalar h = log(maxMergeOrder_.value()) / log(2.);
      CostScalar cpuCostPerIntPass = cpuCostCompareKeys_ * rsFludgeFactor_ * h * rowCount;
  ...
  const CostScalar ff_seqIO = CostScalar(rwFudgeFactor_ * CURRSTMT_OPTDEFAULTS->getTimePerSeqKb());
  ...
  SimpleCostVector cvBK(cpuBK * ff_cpu, ioSeekBK * ff_seeks + ioByteBK * ff_seqIO,
                        ioSeekBK * ff_local_msgs + ioByteBK * ff_kblocal_msgs, csZero, noOfProbesPerStream_);

  SimpleCostVector cvFR(csZero, csZero, csZero, csZero, noOfProbesPerStream_);
  ...
  // j  diskLR = diskBK;
  SimpleCostVector cvLR(cpuLR * ff_cpu, ioSeekLR * ff_seeks + ioByteLR * ff_seqIO,
                        ioSeekLR * ff_local_msgs + ioByteLR * ff_kblocal_msgs, csZero, noOfProbesPerStream_);
  ...
  if (!sOp->getPrefixSortKey().isEmpty()) {
    ...
    CostScalar maxWindowSize(myRowCount_), temp;

    for (CollIndex i = 0; i < length; i++) {
      temp = myPartSortColStats.getMaxOfMaxFreqOfCol(
          sOp->getPrefixSortKey().at(i).getItemExpr()->removeInverseOrder()->getValueId());

      if ((temp.isGreaterThanOne()) && (maxWindowSize > temp)) maxWindowSize = temp;
    }

    CostScalar sortAdjstFactor(csZero);
    CostScalar sortFactor = (ActiveSchemaDB()->getDefaults()).getAsDouble(PARTIAL_SORT_ADJST_FCTR);

    if (maxWindowSize.isGreaterThanOne()) sortAdjstFactor = (maxWindowSize / myRowCount_) * sortFactor;

    if (sortAdjstFactor == csZero) sortAdjstFactor = sortFactor;
...
// -----------------------------------------------------------------------
void CostMethodGroupByAgg::cacheParameters(RelExpr *op, const Context *myContext) {
  // ---------------------------------------------------------------------
  ...
  if (inLogProp_->getResultCardinality().isGreaterThanOne() &&
    ...
    modInputLP = gb_->child(0).getGroupAttr()->materializeInputLogProp(inLogProp_, &multipleCalls);
  } else
  ...
  // ---------------------------------------------------------------------
  cpuCostInitNewGroup_ = CostPrimitives::getBasicCostFactor(EX_OP_ALLOCATE_TUPLE) +
...
void CostMethodGroupByAgg::estimateDegreeOfParallelism() {
  ...
  if (grbyVis.isEmpty()) {
    ...
    if (gb_->isNotAPartialGroupBy() OR gb_->isAPartialGroupByRoot()) {
      ...
      else if ((pr != NULL) AND(pr->isRequirementReplicateNoBroadcast()))
      ...
      if ((CURRSTMT_OPTDEFAULTS->incorporateSkewInCosting()) && (partFunc_ != NULL)) {
        ...
        rowCountPerStream_ =
            child0LogProp_->getCardOfBusiestStream(partFunc_, countOfStreams_, ga_, countOfAvailableCPUs_);
        // For input logical properties, we shall use all columns of the partitioning
        // key. Passing NULL will ensure that
        noOfProbesPerStream_ =
            inLogProp_->getCardOfBusiestStream(partFunc_, countOfStreams_, NULL, countOfAvailableCPUs_);
      } else {
      ...
      if (pf != NULL)
        ...
        // ---------------------------------------------------------------
        countOfStreams_ = pf->getCountOfPartitions();
      else if ((pr != NULL) AND(pr->getCountOfPartitions() != ANY_NUMBER_OF_PARTITIONS))
        countOfStreams_ = pr->getCountOfPartitions();
      else
        countOfStreams_ = rpp_->getCountOfPipelines();
        ...
        if (gb_->isNotAPartialGroupBy()) {
          if ((CURRSTMT_OPTDEFAULTS->incorporateSkewInCosting()) && (partFunc_ != NULL)) {
            ...
            groupCountPerStream_ =
                groupEstLogProp->getCardOfBusiestStream(partFunc_, countOfStreams_, ga_, countOfAvailableCPUs_);
            myRowCountPerStream_ =
                myLogProp_->getCardOfBusiestStream(partFunc_, countOfStreams_, ga_, countOfAvailableCPUs_);
            // groupAttr are passed to compute the columns of partitioning key
            // which belong to this operator. For group by, the subtree of this
            // node is same as that of the child
          ...
          if ((CURRSTMT_OPTDEFAULTS->incorporateSkewInCosting()) && (partFunc_ != NULL)) {
            ...
            // node is same as that of the child
            // so we shall pass this nodes group attributes for the child too
            rowCount = child0LogProp_->getCardOfBusiestStream(partFunc_, countOfStreams_, ga_, countOfAvailableCPUs_);
          } else {
            noOfProbesPerStream_ = (noOfProbes_ / countOfStreams_).minCsOne();
            groupCountPerStream_ = (groupCount_ / countOfStreams_).minCsOne();
            myRowCountPerStream_ = (myRowCount_ / countOfStreams_).minCsOne();
  ...
  if ((delta < 0) && (CURRSTMT_OPTGLOBALS->warningGiven == FALSE)) {
    *CmpCommon::diags() << DgSqlCode(CMP_OPT_WARN_FROM_DCMPASSERT) << DgString0("delta < 0");
    CURRSTMT_OPTGLOBALS->warningGiven = TRUE;
  }
#endif
  // recover in release
  if (groupCountPerStream_ > rowCountPerStream_) groupCountPerStream_ = rowCountPerStream_;
...
Cost *CostMethodSortGroupBy::computeOperatorCostInternal(RelExpr *op, const Context *myContext, int &countOfStreams) {
  ...
  if (CmpCommon::getDefault(COMP_BOOL_86) == DF_ON) {
    ...
    cpuCostReturnRow_ = CostPrimitives::cpuCostForCopyRow(groupKeyLength_ + aggregateLength_);
  }
  ...
  // CPU cost for comparing the group keys.
  // ---------------------------------------------------------------------
  cpuFR += cpuCostCompareGroupKeys_ * MINOF(child0RowCount_ / groupCount_, rowCountPerStream_);
  cpuLR += cpuCostCompareGroupKeys_ * rowCountPerStream_;

  // ---------------------------------------------------------------------
  // CPU cost for aggregating a row with an existing group.
  // ---------------------------------------------------------------------
  cpuFR += cpuCostAggrRowToGroup_ * MINOF(child0RowCount_ / groupCount_, rowCountPerStream_ - groupCountPerStream_);
  cpuLR += cpuCostAggrRowToGroup_ * (rowCountPerStream_ - groupCountPerStream_);

  // ---------------------------------------------------------------------
  // CPU cost for evaluating the having predicate on a group.
  // ---------------------------------------------------------------------
  cpuFR += cpuCostEvalHavingPred_ * MINOF(groupCount_ / myRowCount_, groupCountPerStream_);
  ...
  SimpleCostVector cvFR(cpuForcvFR, csZero, csZero, csZero, noOfProbesPerStream_);

  SimpleCostVector cvLR(cpuLR * ff_cpu, csZero, csZero, csZero, noOfProbesPerStream_);
...
// -----------------------------------------------------------------------
void CostMethodHashGroupBy::cacheParameters(RelExpr *op, const Context *myContext) {
  ...
  extGroupLength_ = groupKeyLength_ + aggregateLength_ + hashedRowOverhead_;
  ...
  if (myContext->getReqdPhysicalProperty()->executeInDP2() && (CmpCommon::getDefault(COMP_BOOL_52) == DF_ON)) {
    CostScalar Gdp2 = (memoryLimitInDP2_ * csOneKiloBytes * csOneKiloBytes / extGroupLength_);
    ...
    if (Gdp2 < groupCount_) {
      ...
      CostScalar denominator = (child0RowCount_ - groupCount_ + Gdp2).minCsOne();
...
void CostMethodHashGroupBy::deriveParameters() {
  ...
  // ---------------------------------------------------------------------
  CostScalar rowCount = (rowCountPerStream_ / noOfProbesPerStream_).minCsOne();
  CostScalar groupCount = (groupCountPerStream_ / noOfProbesPerStream_).minCsOne();
  ...
  // Old behaviour
  if ((CmpCommon::getDefault(COMP_BOOL_52) == DF_ON) && (NOT isBMO_ || groupedTableSize <= memoryLimit_)) {
    noOfClustersToBeAllocated_ = 1;
  } else {
    // Use estimates to compute how many clusters we allocate.
    noOfClustersToBeAllocated_ = computeCountOfClusters(memoryLimit_, groupedTableSize);

    // We should have allocated > 1 clusters if table doesn't fit.
    if (CmpCommon::getDefault(COMP_BOOL_52) == DF_ON) CMPASSERT(noOfClustersToBeAllocated_ > 1);
  }
...
//
//==============================================================================
void CostMethodHashGroupBy::computePartialGroupByLeafCost(CostScalar &cpuFR, CostScalar &cpuLR, CostScalar &cpuBK,
  ...
  if (rpp_->executeInDP2()) {
    groupCountInMemory = ActiveSchemaDB()->getDefaults().getAsLong(MAX_DP2_HASHBY_GROUPS);
  } else {
    // get Esp groupby memory limit (default 100MB).
    CostScalar memorySizeForEsp = ActiveSchemaDB()->getDefaults().getAsLong(EXE_MEMORY_FOR_PARTIALHGB_IN_MB);
  ...
  groupCountInMemory = MINOF(groupCountInMemory, groupCount);
  mem_ = groupCountInMemory * extGroupLength_ / csOneKiloBytes;
  ...
  //---------------------------------------------------------------------------
  cpu += cpuCostAggrRowToGroup_ * (rowsPerGroup - csOne) * groupCountInMemory * ff_cpu;
  ...
  if (NOT(groupingFactor.isLessThanOne())) {
    ...
    numRowsPreBlocking = groupCountInMemory + (rowsConsumedByPartialGB - groupCountInMemory) / 2;
  ...
  //------------------------------------------------------------------
  if (CmpCommon::getDefault(COMP_BOOL_52) == DF_ON || gb_->aggregateExpr().isEmpty()) {
    cpuBK = csZero;
    cpuLR = cpu;
  } else {
    ...
    cpuBK = ((cpuCostInitNewGroup_ * groupCountInMemory) +
             (cpuCostHashRow_ + cpuCostAggrRowToGroup_ + cpuCostPositionHashTableCursor_) * numRowsPreBlocking) *
            ff_cpu;
  ...
  if (CmpCommon::getDefault(COMP_BOOL_52) == DF_OFF && (NOT(gb_->aggregateExpr().isEmpty()))) {
    //-------------------------------------------------------------------
    // Cost to return all output rows. Some of these are grouped rows and
    // some are un-grouped.
    //-------------------------------------------------------------------
    cpuLR += cpuCostReturnRow_ * ff_cpu * (groupCountInMemory + rowCount - rowsConsumedByPartialGB);

    if (CmpCommon::getDefault(COMP_BOOL_90) == DF_OFF) {
      if (groupingFactor < CostScalar(1))
        cpuLR += (cpuCostHashRow_ + cpuCostAggrRowToGroup_ + cpuCostPositionHashTableCursor_) *
                 rowsConsumedByPartialGB / 2 * ff_cpu;
    } else {
      if (groupingFactor.isLessThanOne()) {
        cpuLR += (csOne - groupingFactor) * cpuCostHashRow_ * rowCount * ff_cpu;
...
//
//==============================================================================
NABoolean CostMethodHashGroupBy::computePassCost(NABoolean isFirstPass, SimpleCostVector &cvPassCurr,
  ...
  //-----------------------------------------------
  const CostScalar &cpuCostAggregation = (isFirstPass ? cpuCostAggrRowToGroup_ : cpuCostAggrGroupToGroup_);
  ...
  //------------------------------------
  CostScalar rowCountPerCluster = (rowCount / noOfClustersToBeAllocated_).minCsOne();
  CostScalar groupCountPerCluster = (groupCount / noOfClustersToBeAllocated_).minCsOne();
  ...
  //----------------------------------------------------------------------
  CostScalar memoryFor1stBufferOfClusters = bufferSize_ * noOfClustersToBeAllocated_;
  ...
  //----------------------------------------------------------------------
  CostScalar memoryLeft = CostScalar(memoryLimit_) - memoryFor1stBufferOfClusters;
  ...
  if (CmpCommon::getDefault(COMP_BOOL_52) == DF_OFF) {
    CostScalar averageChainLength =
        (groupCountPerCluster / ActiveSchemaDB()->getDefaults().getAsLong(MAX_HEADER_ENTREIS_PER_HASH_TABLE))
            .getFloor();

    CostScalar costToInsert = calculateCostToInsertIntoChain(averageChainLength);
  ...
  CostScalar rowCountPerSpilledCluster =
      rowCountPerCluster + (groupCountPerCluster - rowCountPerCluster) * groupingFactorForSpilledClusters_;

  //-----------------------------------------------------------------------
  //  Re-estimate spilled cluster size after adjusted for partial grouping.
  //-----------------------------------------------------------------------
  CostScalar spilledClusterSize = rowCountPerSpilledCluster * extGroupLengthInKB;
...
//------------------------------------------------------------------
CostScalar CostMethodHashGroupBy::calculateCostToInsertIntoChain(CostScalar &averageChainLength) {
  if (averageChainLength < 2) return CostScalar(0);
...
int CostMethodHashGroupBy::computeCountOfClusters(const CostScalar &memoryLimit, const CostScalar &tableSize) {
  ...
  if (CmpCommon::getDefault(COMP_BOOL_52) == DF_ON) {
    ...
    double maxTableSizeForNumberOfClusters =
        ActiveSchemaDB()->getDefaults().getAsDouble(HGB_MAX_TABLE_SIZE_FOR_CLUSTERS);
    ...
    double exeMemoryLimit = ActiveSchemaDB()->getDefaults().getAsDouble(HGB_MEMORY_AVAILABLE_FOR_CLUSTERS) *
...
Cost *CostMethodHashGroupBy::computeOperatorCostInternal(RelExpr *op, const Context *myContext, int &countOfStreams) {
  ...
  if ((sppForMe != NULL) && sppForMe->executeInESPOnly()) executeInEsp = TRUE;
  ...
  //--------------------------------------------------------------------
  if (rpp_->executeInDP2() || (groupByNode->isAPartialGroupByLeaf() && executeInEsp)) {
    ...
    if ((CmpCommon::getDefault(COMP_BOOL_52) == DF_OFF) && (gb_->aggregateExpr().isEmpty()))
      considerBlockingCost = FALSE;
    else
      considerBlockingCost = TRUE;

    CostScalar hashCost = cpuCostHashRow_ * rowCountPerStream_ / noOfProbesPerStream_ * ff_cpu;
    ...
    do {
      ...
      isSpilled = computePassCost(isFirstPass, cvPassCurr, isRowProducedInThisPass);
      ...
      if ((isFirstPass OR(NOT isFRproduced))AND considerBlockingCost) {
        ...
        //------------------------------------------------------------------
        SimpleCostVector cvBlockingSum = blockingAdd(cvPassPrev, cvPassCurr, rpp_);
  ...
  //  Synthesize the simple cost vectors.
  //-------------------------------------
  SimpleCostVector cvFR(cpuFR, ioFR, ioFR, csZero, noOfProbesPerStream_);

  SimpleCostVector cvLR(cpuLR, ioLR, ioLR, idleLR, noOfProbesPerStream_);

  SimpleCostVector cvBK(cpuBK, ioBK, ioBK, csZero, noOfProbesPerStream_);
  ...
  Cost *costPtr = new STMTHEAP HashGroupByCost(&cvFR, &cvLR, &cvBK, cpuCount, fragmentsPerCPU, groupingFactor);
...
Cost *CostMethodHashGroupBy::computePlanCost(RelExpr *hashGroupByOp, const Context *myContext, const PlanWorkSpace *pws,
  ...
  //   considered blocking
  //------------------------------------------------------------------------
  if (rpp->executeInDP2() && (CmpCommon::getDefault(COMP_BOOL_52) == DF_ON)) {
    return CostMethod::computePlanCost(hashGroupByOp, myContext, pws, planNumber);
  }

  if ((CmpCommon::getDefault(COMP_BOOL_52) == DF_OFF) && ((HashGroupBy *)hashGroupByOp)->aggregateExpr().isEmpty()) {
    return CostMethod::computePlanCost(hashGroupByOp, myContext, pws, planNumber);
  }
  ...
  //       be made const.
  //------------------------------------------------------------------------
  HashGroupByCost *parentCost = (HashGroupByCost *)((PlanWorkSpace *)pws)->getFinalOperatorCost(planNumber);
  ...
  // unary blocking.
  //-------------------------------------------------------------------------
  planCost->totalCost() = parentCost->getTotalCost() + childRollUpCost->getTotalCost();
  ...
  planCost->cplr() = overlapAddUnary(parentCost->getCplr(), childRollUpCost->getCplr() * (csOne - groupingFactor));
  ...
  if (childRollUpCost->getCpbc1().isZeroVectorWithProbes()) {
    ...
    //----------------------------------------------------------------------
    planCost->cpbc1() =
        overlapAddUnary(parentCost->getCpbc1(), childRollUpCost->getCplr() * groupingFactor / parentNumProbes);
  }
    ...
    //-------------------------------------------------------------
    planCost->cpbc1() = childRollUpCost->getCpbc1().getNormalizedVersion(parentNumProbes);
  }
  ...
  // represents this percentage.
  //-------------------------------------------------------------------------
  planCost->cpbcTotal() = blockingAdd(
      overlapAddUnary(parentCost->getCpbc1(), childRollUpCost->getCplr() * groupingFactor / parentNumProbes),
      childRollUpCost->getCpbcTotal().getNormalizedVersion(parentNumProbes), rpp);
...
Cost *CostMethodShortCutGroupBy::computeOperatorCostInternal(RelExpr *op, const Context *myContext,
  ...
  if ((optype == ITM_ANY_TRUE) OR(optype == ITM_ANY_TRUE_MAX)) {
    ...
    CostScalar cpuCostEvalAnyTruePred = CostPrimitives::cpuCostForEvalPred(anyTruePredVis);
...
Cost *CostMethodShortCutGroupBy::computePlanCost(RelExpr *op, const Context *myContext, const PlanWorkSpace *pws,
  ...
  // Accumulate this child's cost into PlanCost.
  //---------------------------------------------
  Cost mergedChildCost(*childContext->getSolution()->getRollUpCost());
...
void CostMethodJoin::cacheParameters(RelExpr *op, const Context *myContext) {
  ...
  if (inLogProp_->getInputForSemiTSJ() != EstLogProp::NOT_SEMI_TSJ) {
    copyInputEstProp = EstLogPropSharedPtr(new (HISTHEAP) EstLogProp(*inLogProp_));
    copyInputEstProp->setInputForSemiTSJ(EstLogProp::NOT_SEMI_TSJ);
  } else {
  ...
  // ---------------------------------------------------------------------
  if (jn_->isTSJ()) {
    if (jn_->isSemiJoin() || inLogProp_->getInputForSemiTSJ() == EstLogProp::SEMI_TSJ) {
      EstLogPropSharedPtr copyChild0LogProp(new STMTHEAP EstLogProp(*child0LogProp_));
      copyChild0LogProp->setInputForSemiTSJ(EstLogProp::SEMI_TSJ);
      child1LogProp_ = jn_->child(1).outputLogProp(copyChild0LogProp);
    } else if (jn_->isAntiSemiJoin() || inLogProp_->getInputForSemiTSJ() == EstLogProp::ANTI_SEMI_TSJ) {
      EstLogPropSharedPtr copyChild0LogProp(new STMTHEAP EstLogProp(*child0LogProp_));
  ...
  if (jn_->isHashJoin() AND myContext->getInputLogProp()->getResultCardinality().isGreaterThanOne() /* > 1 */) {
    ...
    if (hj->isNoOverflow() AND hj->isReuse()) {
      ...
      child1LogProp_ = jn_->child(1).getGroupAttr()->materializeOutputLogProp(copyInputEstProp, &multipleCalls);
      child1RowCount_ = (child1LogProp_->getResultCardinality()).minCsOne();
      ...
      if (CmpCommon::getDefault(COMP_BOOL_37) == DF_ON AND multipleCalls == 0) {
        ...
        if ((CURRSTMT_OPTDEFAULTS->incorporateSkewInCosting()) && (partFunc_ != NULL)) {
          noOfProbesPerStream_ =
              emptyLogProp->getCardOfBusiestStream(partFunc_, countOfStreams_, NULL, countOfAvailableCPUs_);
        } else
          noOfProbesPerStream_ = (noOfProbes_ / countOfStreams_).minCsOne();
      }
    }
  ...
  hasEquiJoinPred_ = (NOT jn_->isTSJ())AND(NOT jn_->getEquiJoinPredicates().isEmpty());
  ...
  const PhysicalProperty *sppOfChild0 = myContext->getPhysicalPropertyOfSolutionForChild(0);
  ...
  const PhysicalProperty *sppOfChild1 = myContext->getPhysicalPropertyOfSolutionForChild(1);
  if (sppOfChild1 != NULL)
...
void CostMethodJoin::estimateEquiJoinStats() {
  ...
  if (jn_->isMergeJoin() AND(NOT CURRSTMT_OPTDEFAULTS->optimizerPruning()
                                 OR CmpCommon::getDefault(OPH_USE_ORDERED_MJ_PRED) == DF_ON)) {
  ...
  for (ValueId pred = equiJoinPreds.init(); equiJoinPreds.next(pred); equiJoinPreds.advance(pred)) {
    ...
    if (predItemExpr->getOperatorType() != ITM_VEG_PREDICATE) {
      ...
      // -----------------------------------------------------------------
      ColStatsSharedPtr child0ColStats = child0ColStatDescList.getColStatsPtrForPredicate(pred);

      ColStatsSharedPtr child1ColStats = child1ColStatDescList.getColStatsPtrForPredicate(pred);
  ...
  if ((equiJoinVEGPreds.entries() > noOfVEGPredsWithMissingStats) && (noOfVEGPredsWithMissingStats < 2)) {
  ...
  if (inputCacheable) inLogProp_->setCacheableFlag(FALSE);

  // Tell the join to synthesize its estimated logical properties.
  join.synthEstLogProp(inLogProp_);

  // Store result of equi join.
  resultEquiJoinRowCount_ = (ga->outputLogProp(inLogProp_)->getResultCardinality()).minCsOne();

  if (inputCacheable) inLogProp_->setCacheableFlag(TRUE);
...
void CostMethodJoin::estimateDegreeOfParallelism() {
  ...
  if (jn_->isTSJ()) {
    ...
    if (((NestedJoin *)jn_)->isProbeCacheApplicable(rppForMe->getPlanExecutionLocation())) doNotPenalizeSkew = TRUE;

    // The uec's are not currently being used in NJ cost analysis.
    if ((CURRSTMT_OPTDEFAULTS->incorporateSkewInCosting()) && (partFunc_ != NULL) && (doNotPenalizeSkew == FALSE)) {
      child0RowCountPerStream_ =
          child0LogProp_->getCardOfBusiestStream(partFunc_, countOfStreams_, child0GA, countOfAvailableCPUs_);
      child1RowCountPerStream_ =
          child1LogProp_->getCardOfBusiestStream(partFunc_, countOfStreams_, child1GA, countOfAvailableCPUs_);
    } else {
    ...
    // -----------------------------------------------------------------
    {
      ...
      if (isColStatsMeaningful_) {
        ...
        if (NOT computeRepresentativeStream()) {
          ...
          if ((CURRSTMT_OPTDEFAULTS->incorporateSkewInCosting()) && (partFunc_ != NULL)) {
            ...
            child0RowCountPerStream_ = (child0RowCount_ / countOfStreams_).minCsOne();
          child0UecPerStream_ = (child0EquiJoinColStats_->getTotalUec() / countOfStreams_).minCsOne();
          ...
          if ((child1PartFunc_ == NULL) OR child1PartFunc_->isAReplicateViaBroadcastPartitioningFunction()) {
            child1RowCountPerStream_ = child1RowCount_;
            ...
            if ((CURRSTMT_OPTDEFAULTS->incorporateSkewInCosting()) && (partFunc_ != NULL)) {
              ...
              child1RowCountPerStream_ = (child1RowCount_ / countOfStreams_).minCsOne();
            child1UecPerStream_ = (child1EquiJoinColStats_->getTotalUec() / countOfStreams_).minCsOne();
          }
          ...
          if ((CURRSTMT_OPTDEFAULTS->incorporateSkewInCosting()) && (partFunc_ != NULL)) {
            ...
            equiJnRowCountPerStream_ = equiJoinColStatDescList.getCardOfBusiestStream(partFunc_, countOfStreams_,
                                                                                      child0GA, countOfAvailableCPUs_);
          } else
            equiJnRowCountPerStream_ = (mergedEquiJoinColStats_->getRowcount() / countOfStreams_).minCsOne();

          equiJnUecPerStream_ = mergedEquiJoinColStats_->getTotalUec() / countOfStreams_;
        } else {
          // -----------------------------------------------------------
      ...
      // ---------------------------------------------------------------
      {
        ...
        if ((CURRSTMT_OPTDEFAULTS->incorporateSkewInCosting()) && (partFunc_ != NULL)) {
          ...
          child0RowCountPerStream_ = (child0RowCount_ / countOfStreams_).minCsOne();
        child0UecPerStream_ = (child0EquiJoinColUec_ / countOfStreams_).minCsOne();
        // For replicate broadcast, each stream must process all the
        ...
        if (ActiveSchemaDB()->getDefaults().getAsLong(COMP_INT_95) == 0) {
          if (jn_->isHashJoin() AND planNumber ==
              PLAN0 AND(child1PartFunc_ == NULL OR child1PartFunc_->isAReplicateViaBroadcastPartitioningFunction()))
            type2Plan = TRUE;
          else
            type2Plan = FALSE;
        } else {
        ...
        if (type2Plan) {
          ...
          if ((CURRSTMT_OPTDEFAULTS->incorporateSkewInCosting()) && (partFunc_ != NULL)) {
            ...
            child1RowCountPerStream_ = (child1RowCount_ / countOfStreams_).minCsOne();
          child1UecPerStream_ = (child1EquiJoinColUec_ / countOfStreams_).minCsOne();
        }
        ...
        // $$$ yet to be devised.
        // -------------------------------------------------------------
        equiJnRowCountPerStream_ = (resultEquiJoinRowCount_ / countOfStreams_);
...
NABoolean CostMethodJoin::mergeHistogramsOnEquiJoinPred() {
  ...
  if (child0RefColStatsIndices.entries() != 1 OR child1RefColStatsIndices.entries() != 1) return FALSE;
  ...
  HistogramSharedPtr child0Template = child0Histogram->createMergeTemplate(child1Histogram, TRUE /*equiMerge*/);
  ...
  if (child0Template->entries() != child1Template->entries() OR child0Template->entries() !=
      mergedTemplate->entries()) {
  ...
  while (i < mergedTemplate->entries()) {
    ...
    if (uec.isGreaterThanZero() /* > csZero */)
      rowCount = (child0RowCount * child1RowCount) / MAXOF(child0Uec, child1Uec);
    else
      rowCount = csZero;
  ...
  mergedEquiJoinColStats_ = ColStatsSharedPtr(new HISTHEAP ColStats(id, totalUec, totalRowCount,
                                                                    // added baseRowCount for testing initialized
                                                                    // baseRowCount with totalRowCount 11/30 RV
                                                                    totalRowCount, FALSE, FALSE, mergedTemplate, FALSE,
                                                                    csOne,  // default row reduction factor
                                                                    csOne,  // default uec reduction factor
                                                                    -1,     // default avg VarChar size
                                                                    HISTHEAP));

  ColStatDescSharedPtr equiJoinStatDesc(new (HISTHEAP) ColStatDesc(mergedEquiJoinColStats_, child0Col), HISTHEAP);
...
// -----------------------------------------------------------------------
void CostMethodJoin::classifyPredicates(ValueIdSet &innerEquiJoinKeys, ValueIdSet &outerEquiJoinKeys,
                                        ValueIdSet &otherJoinPreds, ValueIdSet &otherSelPreds) {
  CMPASSERT(NOT jn_->isTSJ());
  ...
  while (1) {
    ...
    for (ValueId pred = predSet.init(); predSet.next(pred); predSet.advance(pred)) {
      ...
      if (itemPred->getOperatorType() == ITM_VEG_PREDICATE) {
        if (jn_->isLeftJoin() AND isConsideringJoinPred) {
          ...
          if (VEGGroup.referencesAConstValue(&constant)) {
            const ValueId &VEGGroupId = predVEG->getVEGReference()->getValueId();

            if (jn_->child(0).getGroupAttr()->isCharacteristicOutput(VEGGroupId)) {
              // -----------------------------------------------------------
...
// -----------------------------------------------------------------------
void CostMethodHashJoin::cacheParameters(RelExpr *op, const Context *myContext) {
  ...
  if (child1PartFunc_ == NULL) {
    ...
    if (planNumber > PLAN0) {
      ...
      if (childContext != NULL && childContext->hasOptimalSolution()) {
        const PhysicalProperty *sppOfChild = childContext->getPhysicalPropertyForSolution();
        if (sppOfChild != NULL)
  ...
  classifyPredicates(innerHashKeys, outerHashKeys, otherJoinPreds, otherSelPreds);
  ...
  if (innerHashKeys.isEmpty()) {
    ...
    cpuCostCompareHashKeys_ = CostPrimitives::cpuCostForCompare(innerHashKeys);
  }

  if (otherJoinPreds.isEmpty())
    cpuCostEvalOtherJoinPreds_ = csZero;
  else
    cpuCostEvalOtherJoinPreds_ = CostPrimitives::cpuCostForEvalPred(otherJoinPreds);
  if (otherSelPreds.isEmpty())
    cpuCostEvalOtherSelPreds_ = csZero;
  else
    cpuCostEvalOtherSelPreds_ = CostPrimitives::cpuCostForEvalPred(otherSelPreds);

  if (hj_->isLeftJoin()) {
    if (hj_->nullInstantiatedOutput().isEmpty())
      cpuCostNullInst_ = csZero;
    else
      cpuCostNullInst_ = CostPrimitives::cpuCostForCopyRow(hj_->nullInstantiatedOutput().getRowLength());
  }
  ...
  cpuCostCopyChild0Row_ = CostPrimitives::cpuCostForCopyRow(child0RowLength_);
  cpuCostCopyChild1Row_ = CostPrimitives::cpuCostForCopyRow(child1RowLength_);
...
void CostMethodHashJoin::deriveParameters() {
  ...
  if ((CmpCommon::getDefault(COMP_BOOL_37) == DF_OFF) AND hj_->isReuse() AND(hj_->multipleCalls() == 0))
    innerRowCount = child1RowCountPerStream_;
  else
    innerRowCount = (child1RowCountPerStream_ / noOfProbesPerStream_).minCsOne();
  CostScalar innerTableSize = innerRowCount / csOneKiloBytes * extChild1RowLength_;
  CostScalar outerRowCount = (child0RowCountPerStream_ / noOfProbesPerStream_).minCsOne();
  CostScalar outerTableSize = outerRowCount / csOneKiloBytes * extChild0RowLength_;

  // ---------------------------------------------------------------------
  // Simple case. Quick way out.
  // ---------------------------------------------------------------------
  if ((NOT isBMO_)OR(innerTableSize <= memoryLimit_) OR(hj_->isNoOverflow())) {
    noOfClusters_ = csOne;
  ...
  if (CmpCommon::getDefault(COMP_BOOL_54) == DF_ON) {
    ...
    // -------------------------------------------------------------------
    idealNoOfClusters = computeIdealCountOfClusters(memoryLimit_, innerTableSize);
  } else {
    // -------------------------------------------------------------------
    // Calculate number of clusters (at most four) and number of overflow
    // clusters
    // -------------------------------------------------------------------
    idealNoOfClusters = computeInitialCountOfClusters(memoryLimit_, innerTableSize);
  }
  ...
  if (NOT hasEquiJoinPred_)
    ...
    noOfInnerClustersOccupied_ = noOfClusters_ + estimatedNumberOfOverflowClusters_;
  ...
  clusterSizeAfterSplitsOverFlow_ = innerTableSize / (noOfClusters_ + estimatedNumberOfOverflowClusters_);
  ...
  if (noOfInnerClustersInMemory_ != noOfInnerClustersOccupied_ && estimatedNumberOfOverflowClusters_.isZero()) {
    estimatedNumberOfOverflowClusters_ = noOfInnerClustersOccupied_ - noOfInnerClustersInMemory_;
  ...
  if (NOT hasEquiJoinPred_)
    ...
    noOfOuterClustersOccupied_ = (noOfInnerClustersOccupied_ + estimatedNumberOfOverflowClusters_);
  ...
  noOfOuterClustersFlushed_ = MAXOF(noOfOuterClustersOccupied_ - noOfInnerClustersInMemory_, csZero);
  ...
  // ---------------------------------------------------------------------
  double fractionOfOuterRowProbingInStage2 = (noOfInnerClustersInMemory_ / noOfOuterClustersOccupied_).value();
  double fractionOfOuterRowProbingInStage3 = (noOfOuterClustersFlushed_ / noOfOuterClustersOccupied_).value();
  double fractionOfOuterRowsThrownAway = (1. - fractionOfOuterRowProbingInStage2 - fractionOfOuterRowProbingInStage3);

  // No of outer rows which finish their processing in Stage 2.
  CostScalar outerRowsDoneInStage2 =
      outerRowCount * (fractionOfOuterRowProbingInStage2 + fractionOfOuterRowsThrownAway);

  // No of outer rows which finish their processing in Stage 3.
  CostScalar outerRowsDoneInStage3 = outerRowCount * fractionOfOuterRowProbingInStage3;
...
int CostMethodHashJoin::computeInitialCountOfClusters(const CostScalar &memoryLimit, const CostScalar &tableSize) {
  ...
  if (memoryLimit > 2E6) myMemoryLimit = 2E6;

  estimatedNumberOfOverflowClusters_ = CostScalar(tableSize.getValue() / myMemoryLimit);
  ...
  CostScalar basicMemoryReqdPerCluster = bufferSize_ * initialBucketCountPerCluster_;
...
// computation.
// -----------------------------------------------------------------------
CostScalar CostMethodHashJoin::computeCreateHashTableCost(const CostScalar &rowCount) const {
  ...
  if (NOT isColStatsMeaningful_) {
    ...
    if (NOT hasEquiJoinPred_) {
      ...
      // OA mar-02
        ...
        CostScalar selOfOtherPreds = myRowCount_ / resultEquiJoinRowCount_;
        ...
        // ---------------------------------------------------------------
        CostScalar averageChainLength = child1RowCount_ / child1EquiJoinColUec_;

        // Use selectivity to estimate part of chain compared.
        CostScalar chainLengthTraversed = averageChainLength * (csOne - selOfOtherPreds);

        compareHashKeyOpCount = evalOtherJoinPredsOpCount = resultEquiJoinRowCount_ * chainLengthTraversed;
        ...
        // predicates.
        // ---------------------------------------------------------------
        evalOtherSelPredsOpCount = (resultEquiJoinRowCount_ + myRowCount_) * .5;
      }
    }
...
void CostMethodHashJoin::computeStage1Cost() {
  ...
  // Cost scalars to be computed.
  // ---------------------------------------------------------------------
  CostScalar cpu(csZero), ioSeek(csZero), ioByte(csZero), ioTimeForFirstRow(csZero), ioTimeForBlocking(csZero),
      ioFlushing(csZero);  // j disk(csZero);
  ...
  // row to the buffer.
  // ---------------------------------------------------------------------
  cpu = (cpuCostHashRow_ + cpuCostCopyChild1Row_) * (child1RowCountPerStream_ / noOfProbesPerStream_);
  ...
  if (noOfInnerClustersInMemory_ != noOfInnerClustersOccupied_) {
    ioByte = clusterSizeAfterSplitsOverFlow_ * estimatedNumberOfOverflowClusters_;

    ioSeek = ioByte / bufferSize_;

    ioFlushing = ioSeek * CURRSTMT_OPTDEFAULTS->getTimePerSeek() + ioByte * CURRSTMT_OPTDEFAULTS->getTimePerSeqKb();
  }
  ...
  if (noOfInnerClustersInMemory_ != 0) {
    CostScalar rowsInMemory =
        clusterSizeAfterSplitsOverFlow_ * noOfInnerClustersInMemory_ / extChild1RowLength_ * csOneKiloBytes;
    rowsInMemory = (rowsInMemory).minCsOne();
    cpu += (computeCreateHashTableCost(csZero) * noOfInnerClustersInMemory_) + computeCreateHashTableCost(rowsInMemory);
  }
  ...
  if (CmpCommon::getDefault(COMP_BOOL_37) == DF_ON AND hj_->isNoOverflow() AND isBMO_) {
    ...
    // No limit if zero
    double percentageOfPageFaults = (innerTableSize - memoryLimit_) / innerTableSize;
    double numPagesToScan = 1.0;
    double pageSize = CostPrimitives::getBasicCostFactor(DEF_PAGE_SIZE);

    const double seekCostFR = numPagesToScan * percentageOfPageFaults;

    ioTimeForFirstRow =
        seekCostFR * (CURRSTMT_OPTDEFAULTS->getTimePerSeek() + pageSize * CURRSTMT_OPTDEFAULTS->getTimePerSeqKb());
    ...
    double recordSize = (hj_->child(1).getGroupAttr()->getRecordLength() +
                         CostPrimitives::getBasicCostFactor(HH_OP_HASHED_ROW_OVERHEAD)) /
                        1024.0;
    ...
    ioTimeForBlocking = CostScalar((innerRowCount - numRowsThatFitInMemory)) * ioTimeForFirstRow;

  }  // pagefault costing for ordered hash joins OA feb02

  CostScalar cpuTimeForBlocking = cpu * CURRSTMT_OPTDEFAULTS->getTimePerCPUInstructions();
  ...
  if (CmpCommon::getDefault(COMP_BOOL_39) == DF_ON)
    stage1cvBK_ = new STMTHEAP SimpleCostVector(cpu * CURRSTMT_OPTDEFAULTS->getTimePerCPUInstructions(), io,
                                                csZero,  // msg time
                                                csZero,  // idle time
                                                noOfProbesPerStream_);
  else
    stage1cvBK_ = new STMTHEAP SimpleCostVector(cpuTimeForBlocking, io,
                                                csZero,  // msg time
                                                csZero,  // idle time
...
void CostMethodHashJoin::computeStage2Cost() {
  ...
  CostScalar probingFractionForStage2 =
      (noOfInnerClustersInMemory_ / (noOfInnerClustersInMemory_ + noOfOuterClustersFlushed_));

  if (CmpCommon::getDefault(COMP_BOOL_54) == DF_OFF) {
    cpuLR = (cpuCostHashRow_ * child0RowCountPerStream_ / noOfProbesPerStream_) * probingFractionForStage2;

    cpuBK =
        (cpuCostHashRow_ * child0RowCountPerStream_ / noOfProbesPerStream_) * (1 - probingFractionForStage2.getValue());
  } else
    cpuLR = (cpuCostHashRow_ * child0RowCountPerStream_ / noOfProbesPerStream_);

  // ---------------------------------------------------------------------
  // Outer rows that are hashed to an in-memory cluster needn't be stored
  // in buffers. They could probe the hash table directly.
  // ---------------------------------------------------------------------
  CostScalar rowsCopied = outerClusterSize_ * noOfOuterClustersFlushed_ / extChild0RowLength_ * csOneKiloBytes;
  ...
  if (noOfInnerClustersInMemory_ > 0) {
    ...
    // -------------------------------------------------------------------
    CostScalar cpuCostAverageProbing = (cpuCostTotalProbing_ / countOfStreams_ / noOfProbesPerStream_);
  ...
  // ---------------------------------------------------------------------
  if (CmpCommon::getDefault(COMP_BOOL_39) == DF_ON)
    stage2cvFR_ = new STMTHEAP SimpleCostVector(cpuFR * ff_cpu, csZero, csZero, csZero, noOfProbesPerStream_);
  else
    stage2cvFR_ = new STMTHEAP SimpleCostVector(cpuFR * ff_cpu, csZero, csZero, csZero, numberOfProbesForHJ);
  ...
  // no messages:
  if (CmpCommon::getDefault(COMP_BOOL_39) == DF_ON)
    stage2cvLR_ = new STMTHEAP SimpleCostVector(cpuLR * ff_cpu,
                                                csZero,  // iotime
                                                csZero,  // msgtime
                                                csZero,  // idletime
                                                noOfProbesPerStream_);
  ...
  if (CmpCommon::getDefault(COMP_BOOL_39) == DF_ON)
    stage2cvBK_ =
        new STMTHEAP SimpleCostVector(cpuBK * ff_cpu,
                                      ...
                                      csZero, csZero, noOfProbesPerStream_);
  else
    stage2cvBK_ = new STMTHEAP SimpleCostVector(
        cpuBK * ff_cpu,
        (ioSeekLR * CURRSTMT_OPTDEFAULTS->getTimePerSeek() + ioByteLR * CURRSTMT_OPTDEFAULTS->getTimePerSeqKb()) /
            numberOfProbesForHJ,
        csZero, csZero, numberOfProbesForHJ);
}  // CostMethodHashJoin::computeStage2Cost().
//<pb>
// -----------------------------------------------------------------------
...
void CostMethodHashJoin::computeStage3Cost() {
  ...
  if (noOfOuterClustersFlushed_ == 0) {
    stage3cvFR_ = new STMTHEAP SimpleCostVector(csZero, csZero, csZero, csZero, noOfProbesPerStream_);

    stage3cvLR_ = new STMTHEAP SimpleCostVector(csZero, csZero, csZero, csZero, noOfProbesPerStream_);
  ...
  // Cost scalars to be computed.
  // ---------------------------------------------------------------------
  CostScalar cpuBK(csZero), cpuLR(csZero), cpuFR(csZero), ioSeekBK(csZero), ioByteBK(csZero);
  ...
  // ---------------------------------------------------------------------
  CostScalar rowsChained = clusterSizeAfterSplitsOverFlow_ / extChild1RowLength_ * 1024.;

  cpuBK = computeCreateHashTableCost(rowsChained) * estimatedNumberOfOverflowClusters_;

  // ---------------------------------------------------------------------
  // Remaining part of probing cost. Cost of item C.
  // ---------------------------------------------------------------------
  CostScalar cpuCostAverageProbing = (cpuCostTotalProbing_ / countOfStreams_ / noOfProbesPerStream_);
  double probingFractionForStage3 =
      (noOfOuterClustersFlushed_ / (noOfInnerClustersInMemory_ + noOfOuterClustersFlushed_)).value();
  ...
  // ---------------------------------------------------------------------
  // no messages:
  stage3cvFR_ = new STMTHEAP SimpleCostVector(cpuFR * ff_cpu, csZero, csZero, csZero, noOfProbesPerStream_);
  ...
  stage3cvLR_ = new STMTHEAP SimpleCostVector(cpuLR * ff_cpu, csZero, csZero, csZero, noOfProbesPerStream_);
...
Cost *CostMethodHashJoin::computeOperatorCostInternal(RelExpr *op, const Context *myContext, int &countOfStreams) {
  ...
  if (printCost) {
    ...
    fprintf(pfp, "myRowCount=%g;child0RowCount=%g;child1RowCount=%g\n", myRowCount_.value(), child0RowCount_.value(),
            child1RowCount_.value());
    cvFR.print(pfp);
    cvLR.print(pfp);
  }
#endif
  ...
  // ---------------------------------------------------------------------
  Cost *costPtr = new STMTHEAP HashJoinCost(&cvFR, &cvLR, &cvBK, cpuCount, fragmentsPerCPU, stage2WorkFractionForFR_,
                                            stage3WorkFractionForFR_, stage2cvLR_, stage3cvLR_, stage1cvBK_,
                                            stage2cvBK_, stage3cvBK_);
...
Cost *CostMethodHashJoin::computePlanCost(RelExpr *hashJoinOp, const Context *myContext, const PlanWorkSpace *pws,
  ...
  CostPtr leftChildCost;
  CostPtr rightChildCost;
  getChildCostsForBinaryOp(hashJoinOp, myContext, pws, planNumber, leftChildCost, rightChildCost);
  ...
  //----------------------------------------------------------------------
  HashJoinCost *parentCost = (HashJoinCost *)((PlanWorkSpace *)pws)->getFinalOperatorCost(planNumber);
  ...
  // addition.
  //------------------------------------------------------------------------
  rollUpCost->totalCost() = leftChildCost->getTotalCost() + rightChildCost->getTotalCost() + parentCost->getTotalCost();
  ...
  // *************************************************************************
  rollUpCost->cpfr() =
      blockingAdd(overlapAddUnary(leftCplr - leftCpfr, stage2Cost) * stage2Fraction, stage3Cost * stage3Fraction, rpp);
  ...
  if (CmpCommon::getDefault(COMP_BOOL_56) == DF_OFF) leftChildlrCostWithOutfrCost.setIdleTime(leftCplr.getIdleTime());
  rollUpCost->cplr() = blockingAdd(overlapAddUnary(leftChildlrCostWithOutfrCost, stage2Cost), stage3Cost, rpp);
  ...
  if (CmpCommon::getDefault(COMP_BOOL_55) == DF_OFF) {
    rollUpCost->cpbcTotal() = computeNewBlockingCost(parentCost, leftChildCost, rightChildCost, rpp);
    ...
    // ************************************************************
    if (CmpCommon::getDefault(COMP_BOOL_56) == DF_OFF) {
      rollUpCost->cpfr() = etMINOF(rollUpCost->cpfr(), rollUpCost->cpbcTotal(), rpp);
    }
    ...
    if (CmpCommon::getDefault(COMP_BOOL_39) == DF_OFF) {
      ...
      rollUpCost->cpbcTotal() = overlapAddUnary(
          leftCpfr + leftChildTotBl * (leftChildTotBl.getNumProbes()),
          blockingAdd(overlapAddUnary(rightCplr - rightCpfr, parentTotBl * (parentTotBl.getNumProbes())) + rightCpfr,
      ...
      rollUpCost->cpbcTotal() = overlapAddUnary(
          leftCpfr + leftChildCost->getCpbcTotal(),
          blockingAdd(overlapAddUnary((rightCplr - rightCpfr) / numProbes, parentCost->getCpbcTotal()) + rightCpfr,
                      rightChildCost->getCpbcTotal(), rpp));
  }
  //----------------------------------------------------------------------------
  ...
  if (hj->isNoOverflow() AND hj->isReuse() AND hj->multipleCalls() == 0) {
    ...
    if (NOT hj->valuesGivenToChild().isEmpty()) {
      ...
      if (myContext->getPlan()) spp = myContext->getPlan()->getPhysicalProperty();

      CostScalar uniqueProbeCount = inLogProp_->getAggregateUec(hj_->valuesGivenToChild());
      if (uniqueProbeCount < noOfProbes_) uniqueProbeCount = noOfProbes_;
      ...
      if (!spp->isSorted()) {
        ...
        newNumProbes = csOne + (noOfProbes_ - csOne) * (csOne - csOne / uniqueProbeCount);
      }
    }
...
//
//*************************************************************************
SimpleCostVector CostMethodHashJoin::computeNewBlockingCost(HashJoinCost *parentCost, CostPtr leftChildCost,
                                                            CostPtr rightChildCost, const ReqdPhysicalProperty *rpp) {
  const SimpleCostVector &rightCplr = rightChildCost->getCplr();
  const SimpleCostVector &leftCplr = leftChildCost->getCplr();
  const SimpleCostVector &rightCpfr = rightChildCost->getCpfr();
  const CostScalar &RCnumProbes = rightCplr.getNumProbes();
  const CostScalar &LCnumProbes = leftCplr.getNumProbes();
  const CostScalar &parentNumProbes = parentCost->getCplr().getNumProbes();
  ...
  SimpleCostVector A = overlapAddUnary(rightCplr - rightCpfr, stage1BK * parentNumProbes) + rightCpfr;

  SimpleCostVector B = overlapAddUnary(rightChildTotBl * RCnumProbes, leftChildTotBl * LCnumProbes);
...
// -----------------------------------------------------------------------
void CostMethodMergeJoin::cacheParameters(RelExpr *op, const Context *myContext) {
  CostMethodJoin::cacheParameters(op, myContext);
...
//
// -----------------------------------------------------------------------
CostScalar CostMethodMergeJoin::computeIntervalMergingCost(CostScalar child0RowCount, CostScalar child0Uec,
                                                           CostScalar child1RowCount, CostScalar child1Uec,
                                                           CostScalar &mem) {
  ...
  // ---------------------------------------------------------------------
  CostScalar mergeJoinRowCount = uecMatched * child0RowCountPerUec * child1RowCountPerUec;
  CostScalar unmatchedChild0RowCount = uecUnmatched * child0RowCountPerUec;
  ...
  if (mj_->isSemiJoin() OR mj_->isAntiSemiJoin()) {
    ...
    if (mj_->nullInstantiatedOutput().isEmpty())
      ...
      cpuCostNullInst_ = CostPrimitives::cpuCostForCopyRow(mj_->nullInstantiatedOutput().getRowLength());
  }
  ...
  // ---------------------------------------------------------------------
  cpu += cpuCostCompareKeys_ *
         (mergeExprEvalCount + compExprEvalCount + leftCheckDupExprEvalCount + rightCheckDupExprEvalCount);
  ...
  // preds.
  // ---------------------------------------------------------------------
  if (NOT(mj_->isSemiJoin() OR mj_->isAntiSemiJoin())) cpu += cpuCostGetNextRowFromList_ * mergeJoinRowCount;
...
CostScalar CostMethodMergeJoin::computeTotalMergingCost(CostScalar &mem) {
  ...
  if (NOT isColStatsMeaningful_) {
    ...
    cpu = computeIntervalMergingCost(child0RowCountPerStream_, child0UecPerStream_, child1RowCountPerStream_,
    ...
    // sum up.
    // -------------------------------------------------------------------
    HistogramSharedPtr child0Histogram = child0EquiJoinColStats_->getHistogram();
    HistogramSharedPtr child1Histogram = child1EquiJoinColStats_->getHistogram();
    HistogramSharedPtr mergedHistogram = mergedEquiJoinColStats_->getHistogram();

    CostScalar child0Uec, child0RowCount, child1Uec, child1RowCount;
    CollIndex i(1);

    // Just caching some parameters needed later.
    const CostScalar &child0RedFactor = child0EquiJoinColStats_->getRedFactor();
    const CostScalar &child1RedFactor = child1EquiJoinColStats_->getRedFactor();

    CostScalar memInt;

    while (i < mergedHistogram->entries()) {
      child0RowCount = ((*child0Histogram)[i].getCardinality() * child0RedFactor);
      child1RowCount = ((*child1Histogram)[i].getCardinality() * child1RedFactor);

      child0Uec = MINOF((*child0Histogram)[i].getUec(), child0RowCount);
      child1Uec = MINOF((*child1Histogram)[i].getUec(), child1RowCount);

      cpu += computeIntervalMergingCost(child0RowCount, child0Uec, child1RowCount, child1Uec, memInt);
      i++;
...
Cost *CostMethodMergeJoin::computeOperatorCostInternal(RelExpr *op, const Context *myContext, int &countOfStreams) {
  ...
  // Synthesize the cost vectors.
  // ---------------------------------------------------------------------
  const SimpleCostVector cvFR(cpuFR * ff_cpu, csZero, csZero, csZero, noOfProbesPerStream_);

  const SimpleCostVector cvLR(cpuLR * ff_cpu, csZero, csZero, csZero, noOfProbesPerStream_);
...
//
//==============================================================================
Cost *CostMethodMergeJoin::mergeNoLegsBlocking(const CostPtr leftChildCost, const CostPtr rightChildCost,
                                               const ReqdPhysicalProperty *const rpp) {
  //-------------------------------------------------------------------------
  //  The vector idleT represents the amount of time during which exactly one
  ...
  // addition.
  //-------------------------------------------------------------------------
  mergedCost->totalCost() = leftChildCost->getTotalCost() + rightChildCost->getTotalCost();
  ...
  //------------------------------------------------------------------
  mergedCost->cplr() = overlapAdd(leftChildCost->getCplr(), rightChildCost->getCplr());
...
//
//==============================================================================
Cost *CostMethodMergeJoin::mergeBothLegsBlocking(const CostPtr leftChildCost, const CostPtr rightChildCost,
                                                 const ReqdPhysicalProperty *const rpp) {
  //-------------------------------------
  //  Create merged cost initially empty.
  //-------------------------------------
  ...
  //----------------------------------------------------------------------------
  mergedCost->cpfr() = overlapAdd(leftChildCost->getCpfr(), rightChildCost->getCpfr());
  ...
  // probes.
  //--------------------------------------------------------------------
  rightChildCost->cpbcTotal().normalize(leftChildCost->getCpbcTotal().getNumProbes());
  ...
  //-------------------------------------------------------------------------
  mergedCost->cpbcTotal() = overlapAdd(leftChildCost->getCpbcTotal(),
  ...
  //----------------------------------------------------------------------------
  mergedCost->cpbc1() = overlapAdd(etMINOF(leftChildCost->getCpbc1(), rightChildCost->getCpbc1(), rpp),
...
//
//==============================================================================
Cost *CostMethodNestedJoin::mergeNoLegsBlocking(const CostPtr leftChildCost, const CostPtr rightChildCost,
                                                const ReqdPhysicalProperty *const rpp) {
  //-------------------------------------
  //  Create merged cost initially empty.
  //-------------------------------------
  ...
  //------------------------------------------------------------------------
  mergedCost->cpfr() = blockingAdd(leftChildCost->getCpfr(), rightChildCost->getCpfr(), rpp);

  //------------------------------------------------
  //  Same formula as in non-blocking unary roll-up.
  //------------------------------------------------
  mergedCost->cplr() = overlapAdd(rightChildCost->getCplr(), (leftChildCost->getCplr() - leftChildCost->getCpfr())) +
...
//
//==============================================================================
Cost *CostMethodNestedJoin::mergeLeftLegBlocking(const CostPtr leftChildCost, const CostPtr rightChildCost,
                                                 const ReqdPhysicalProperty *const rpp) {
  //-------------------------------------
  //  Create merged cost initially empty.
  //-------------------------------------
  ...
  mergedCost->cpbc1() = leftChildCost->getCpbc1().getNormalizedVersion(rightNumProbes);
  mergedCost->cpbcTotal() = leftChildCost->getCpbcTotal().getNormalizedVersion(rightNumProbes);
...
//
//==============================================================================
Cost *CostMethodNestedJoin::mergeRightLegBlocking(const CostPtr leftChildCost, const CostPtr rightChildCost,
                                                  const ReqdPhysicalProperty *const rpp) {
  //-------------------------------------
  //  Create merged cost initially empty.
  //-------------------------------------
  ...
  //-------------------------------------------------------------------------
  const CostScalar &rightNumProbes = rightChildCost->getCpbcTotal().getNumProbes();

  SimpleCostVector blkLeftCpfr = leftChildCost->getCpfr() / rightNumProbes;

  mergedCost->cpbc1() = blockingAdd(rightChildCost->getCpbc1(), blkLeftCpfr, rpp);

  mergedCost->cpbcTotal() = blockingAdd(rightChildCost->getCpbcTotal(), blkLeftCpfr, rpp);
...
//
//==============================================================================
Cost *CostMethodNestedJoin::mergeBothLegsBlocking(const CostPtr leftChildCost, const CostPtr rightChildCost,
                                                  const ReqdPhysicalProperty *const rpp) {
  //-------------------------------------
  //  Create merged cost initially empty.
  //-------------------------------------
  ...
  //--------------------------------------------------------------------------
  SimpleCostVector normLeftCpbcTotal = leftChildCost->getCpbcTotal().getNormalizedVersion(rightNumProbes);

  SimpleCostVector blkLeftCpfr = leftChildCost->getCpfr() / rightNumProbes;

  mergedCost->cpbcTotal() =
      blockingAdd(blockingAdd(rightChildCost->getCpbcTotal(), blkLeftCpfr, rpp), normLeftCpbcTotal, rpp);
...
// -----------------------------------------------------------------------
void CostMethodNestedJoin::cacheParameters(RelExpr *op, const Context *myContext) {
  CostMethodJoin::cacheParameters(op, myContext);
  nj_ = (NestedJoin *)op;
  ...
  if (nj_->isLeftJoin() AND(NOT nj_->selectionPred().isEmpty())) {
    cpuCostEvalPred_ = CostPrimitives::cpuCostForEvalPred(nj_->selectionPred());
  } else {
  ...
  if (nj_->isLeftJoin() AND(NOT nj_->nullInstantiatedOutput().isEmpty())) {
    cpuCostNullInst_ = CostPrimitives::cpuCostForCopyRow(nj_->nullInstantiatedOutput().getRowLength());
  } else {
    cpuCostNullInst_ = csZero;
  }
}
...
Cost *CostMethodNestedJoin::computeOperatorCostInternal(RelExpr *op, const Context *myContext, int &countOfStreams) {
  ...
  // Phase 1 cost.
  // ---------------------------------------------------------------------
  CostScalar noOfProbesNeededToGetFirstRow = (noOfProbes_ / myRowCount_).minCsOne();
  if (nj_->isLeftJoin() AND nj_->selectionPred().isEmpty()) noOfProbesNeededToGetFirstRow = csOne;
  noOfProbesNeededToGetFirstRow = MINOF(noOfProbesNeededToGetFirstRow, noOfProbes_);
  ...
  // first row returned.
  // ---------------------------------------------------------------------
  CostScalar leftRowsNeededToGetFirstRow = MIN_ONE(child0RowCount_ / myRowCount_);
  if (nj_->isLeftJoin() AND nj_->selectionPred().isEmpty()) leftRowsNeededToGetFirstRow = csOne;
  leftRowsNeededToGetFirstRow = MINOF(leftRowsNeededToGetFirstRow, child0RowCount_);
  ...
  if (NOT cpuCostEvalPred_.isZero()) {
    CostScalar rightRowsNeededToGetFirstRow = (child1RowCount_ / myRowCount_).minCsOne();
    rightRowsNeededToGetFirstRow = MINOF(rightRowsNeededToGetFirstRow, child1RowCount_);
  ...
  if (NOT cpuCostNullInst_.isZero()) {
    if (child1RowCount_ < child0RowCount_) {
      ...
      cpuLR += cpuCostNullInst_ * (child0RowCount_ - child1RowCount_).minCsOne();
    }
...
Cost *CostMethodNestedJoin::computePlanCost(RelExpr *nestedJoinOp, const Context *myContext, const PlanWorkSpace *pws,
  ...
  CostPtr leftChildCost;
  CostPtr rightChildCost;
  getChildCostsForBinaryOp(nestedJoinOp, myContext, pws, planNumber, leftChildCost, rightChildCost);
  ...
  //--------------------------------------------------------------------------
  if ((nestedJoinOp->getGroupAttr()->getNumBaseTables() == 1)
          AND(nestedJoinOp->child(0).getGroupAttr()->getNumBaseTables() == 1)) {
    ...
    if (childContext != NULL) ((Cost *)(childContext->getSolution()->getRollUpCost()))->planPriority().resetToNormal();
  }
  ...
  if (leftChildCost->getCpbcTotal().isZeroVectorWithProbes()) {
    if (rightChildCost->getCpbcTotal().isZeroVectorWithProbes()) {
      ...
      //  Only right child has a blocking operator in its subtree.
      //----------------------------------------------------------
      mergedChildCost = mergeRightLegBlocking(leftChildCost, rightChildCost, rpp);
    }
  } else {
    if (rightChildCost->getCpbcTotal().isZeroVectorWithProbes()) {
      //---------------------------------------------------------
      //  Only left child has a blocking operator in its subtree.
      //---------------------------------------------------------
      mergedChildCost = mergeLeftLegBlocking(leftChildCost, rightChildCost, rpp);
    } else {
      //----------------------------------------------------------
      //  Both children have blocking operators in their subtrees.
      //----------------------------------------------------------
...
// -----------------------------------------------------------------------
void CostMethodMergeUnion::cacheParameters(RelExpr *op, const Context *myContext) {
  CostMethod::cacheParameters(op, myContext);
  ...
  cpuCostCompareKeys_ = (sortKeyVis.isEmpty() ? csZero : CostPrimitives::cpuCostForCompare(sortKeyVis));
}
//<pb>
// -----------------------------------------------------------------------
...
Cost *CostMethodMergeUnion::computeOperatorCostInternal(RelExpr *op, const Context *myContext, int &countOfStreams) {
  ...
  // ATP to its up queue.
  // ---------------------------------------------------------------------
  cpuLR += (cpuCostCopyRow_ + cpuCostCompareKeys_ + cpuCostCopyAtp_) * myRowCount_;
...
//
//==============================================================================
Cost *CostMethodMergeUnion::mergeNoLegsBlocking(const CostPtr leftChildCost, const CostPtr rightChildCost,
                                                const ReqdPhysicalProperty *const rpp) {
  //-------------------------------------
  //  Create merged cost initially empty.
  //-------------------------------------
  ...
  //---------------------------------------------------------------------------
  mergedCost->cpfr() = overlapAdd(etMINOF(leftChildCost->getCpfr(), rightChildCost->getCpfr(), rpp),
                                  vecMINOF(leftChildCost->getCpfr(), rightChildCost->getCpfr()));
...
//
//==============================================================================
Cost *CostMethodMergeUnion::mergeBothLegsBlocking(const CostPtr leftChildCost, const CostPtr rightChildCost,
                                                  const ReqdPhysicalProperty *const rpp) {
  Cost *mergedCost = new STMTHEAP Cost();
  ...
  //-------------------------------------------------------------------------
  CostScalar leftChildCpbcTotalET = leftChildCost->getCpbcTotal().getElapsedTime(rpp);
  CostScalar rightChildCpbcTotalET = rightChildCost->getCpbcTotal().getElapsedTime(rpp);

  const CostScalar &leftChildCpbcTotalNumProbes = leftChildCost->getCpbcTotal().getNumProbes();
  const CostScalar &rightChildCpbcTotalNumProbes = rightChildCost->getCpbcTotal().getNumProbes();
  ...
  if (leftChildCpbcTotalET <= rightChildCpbcTotalET) {
    ...
    //---------------------------------------------------------------------
    const CostScalar leftChildLastRowElapsedTime = leftChildCost->getCplr().getElapsedTime(rpp);  // div-by-zero fix
    ...
    const CostScalar overlapRatio = (leftChildLastRowElapsedTime.isZero())
                                        ? csOne
                                        : MINOF(csOne, ((rightChildCpbcTotalET * rightChildCpbcTotalNumProbes) -
    ...
    //----------------------------------------------------------------------
    SimpleCostVector rightBlockingAllProbes = rightChildCost->getCpbcTotal() * rightChildCpbcTotalNumProbes;
    ...
    overlapLeft = overlapAdd(overlapLeft.scaleByValue(overlapRatio), rightBlockingAllProbes) - rightBlockingAllProbes;
    ...
    //-----------------------------------------------------------------------
    mergedCost->cplr() = overlapAdd(rightChildCost->getCplr(), nonOverlapLeft + overlapLeft);
  } else {
    //----------------------------------------------------------------------
    ...
    //----------------------------------------------------------------------
    const CostScalar rightChildLastRowElapsedTime = rightChildCost->getCplr().getElapsedTime(rpp);  // div-by-zero fix

    // if rightChildLastRowElapsedTime is zero (for whatever reason ... this is
    // suspicious ... ), set the ratio to be one -- don't divide by zero!
    const CostScalar overlapRatio = (rightChildLastRowElapsedTime.isZero())
                                        ? csOne
                                        : MINOF(csOne, ((leftChildCpbcTotalET * leftChildCpbcTotalNumProbes) -
    ...
    // commensurate with last row activity.
    //---------------------------------------------------------------------
    SimpleCostVector leftBlockingAllProbes = leftChildCost->getCpbcTotal() * leftChildCpbcTotalNumProbes;
    ...
    overlapRight = overlapAdd(overlapRight.scaleByValue(overlapRatio), leftBlockingAllProbes) - leftBlockingAllProbes;
    ...
    //-----------------------------------------------------------------------
    mergedCost->cplr() = overlapAdd(leftChildCost->getCplr(), nonOverlapRight + overlapRight);
  }
  ...
  //--------------------------------------------------------------------------
  mergedCost->cpbcTotal() = overlapAdd(leftChildCost->getCpbcTotal(), rightChildCost->getCpbcTotal());
  ...
  //---------------------------------------------------------------------------
  mergedCost->cpbc1() = overlapAdd(etMINOF(leftChildCost->getCpbc1(), rightChildCost->getCpbc1(), rpp),
                                   vecMINOF(leftChildCost->getCpbc1(), rightChildCost->getCpbc1()));
...
Cost *CostMethodRelRoot::computeOperatorCostInternal(RelExpr *op, const Context *myContext, int &countOfStreams) {
  ...
  CostScalar cpuCopyRow = CostPrimitives::cpuCostForCopyRow(myVis().getRowLength());
  CostScalar cpuFR = cpuCopyRow;
  CostScalar cpuLR = cpuCopyRow * myRowCount_;

  CostScalar readMetadataOpenFirstPartitionCpu = CostPrimitives::getBasicCostFactor(CPUCOST_SUBSET_OPEN) *
  ...
  // Synthesize the cost vectors.
  // ---------------------------------------------------------------------
  SimpleCostVector cvFR(cpuFR * ff_cpu, csZero, csZero, readMetadataOpenFirstPartitionCpu, csOne);

  SimpleCostVector cvLR(cpuLR * ff_cpu, csZero, csZero, readMetadataOpenFirstPartitionCpu, csOne);
...
//
void CostMethodTranspose::cacheParameters(RelExpr *op, const Context *myContext) {
  ...
  for (CollIndex v = 0; v < transpose->transUnionVectorSize(); v++) {
    ...
    for (CollIndex vidu = 0; vidu < valIdList.entries(); vidu++) {
      const ValueIdUnion *valIdUnion = (ValueIdUnion *)valIdList[vidu].getValueDesc()->getItemExpr();
  ...
  CostScalar cpuCostToProduceOneRow =
      CostPrimitives::cpuCostForCopySet(moveValues) + CostPrimitives::getBasicCostFactor(EX_OP_ALLOCATE_TUPLE);
...
Cost *CostMethodTranspose::computeOperatorCostInternal(RelExpr *op, const Context *myContext, int &countOfStreams) {
  ...
  CostScalar cpuCostToProduceLastRow = cpuCostToProduceAllRows_ / countOfStreams_;

  CostScalar cpuCostToProduceFirstRow = cpuCostToProduceLastRow / myRowCount_ / noOfProbes_;

  // fudge factor for cputime
  const CostScalar ff_cpu = CURRSTMT_OPTDEFAULTS->getTimePerCPUInstructions();

  const SimpleCostVector cvFirstRow(cpuCostToProduceFirstRow * ff_cpu, csZero, csZero, csZero, noOfProbesPerStream_);

  const SimpleCostVector cvLastRow(cpuCostToProduceLastRow * ff_cpu, csZero, csZero, csZero, noOfProbesPerStream_);
...
// Compute common costing parameters.
//
void CostMethodCompoundStmt::cacheParameters(RelExpr *op, const Context *myContext) {
  // Just to make sure things are working as expected
  //
  ...
  CostScalar cpuCostToProduceOneRow = CostPrimitives::getBasicCostFactor(EX_OP_ALLOCATE_TUPLE);
...
Cost *CostMethodCompoundStmt::computeOperatorCostInternal(RelExpr *op, const Context *myContext, int &countOfStreams) {
  ...
  const SimpleCostVector cvFirstRow(cpuFR * ff_cpu, csZero, csZero, csZero, noOfProbesPerStream_);

  const SimpleCostVector cvLastRow(cpuLR * ff_cpu, csZero, csZero, csZero, noOfProbesPerStream_);
...
Cost *CostMethodStoredProc::computeOperatorCostInternal(RelExpr *op, const Context *myContext, int &countOfStreams) {
  ...
  //
  const SimpleCostVector cv(csOne * CURRSTMT_OPTDEFAULTS->getTimePerCPUInstructions(), csZero, csZero, csZero, csOne);
 ...
 **************************************************/
  ...
  SimpleCostVector cvFR(cpuTimeForFirstRow,
                        csZero,                 // no IO time
                        csZero,                 // no message time
                        csZero,                 // no idle time
                        noOfProbesPerStream_);  // num. of probes

  SimpleCostVector cvLR(cpuTimeForLastRow,
                        csZero,  // no IO time
                        csZero,  // no message time
...
Cost *CostMethodHbaseInsert::computeOperatorCostInternal(RelExpr *op, const Context *myContext, int &countOfStreams) {
  ...
  noOfProbesPerStream = (noOfProbes_ / countOfAsynchronousStreams_).minCsOne();
...
// Compute common costing parameters.
//
void CostMethodUnPackRows::cacheParameters(RelExpr *op, const Context *myContext) {
  // Just to make sure things are working as expected
  //
...
// Compute common costing parameters.
//
void CostMethodRelSequence::cacheParameters(RelExpr *op, const Context *myContext) {
  // Just to make sure things are working as expected
  //
  ...
  CostScalar cpuCostToProduceOneRow =
      ...
      CostPrimitives::cpuCostForCopyRow(relSequence->sequenceFunctions().getRowLength()) +
...
Cost *CostMethodRelSequence::computeOperatorCostInternal(RelExpr *op, const Context *myContext, int &countOfStreams) {
  ...
  if (isBMO_ AND memFRInKB > csZero AND memFRInKB > memoryLimit_) {
    double pageSizeInKB = CostPrimitives::getBasicCostFactor(DEF_PAGE_SIZE);
  ...
  const SimpleCostVector cvFR(cpuFR * ff_cpu,                                 // CPU Time.
                              seekFR * ff_seeks + transferFRInKB * ff_seqIO,  // IOTime
                              csZero,                                         // no messages
                              csZero,                                         // never Idle
                              noOfProbesPerStream_);                          // num probes

  const SimpleCostVector cvLR(cpuLR * ff_cpu,                                 // CPU Time.
...
Cost *CostMethodSample::computeOperatorCostInternal(RelExpr *op, const Context *myContext, int &countOfStreams) {
  ...
  CostScalar cpuCostToProcessOneRow =
      numBalanceExpr * (CostPrimitives::getBasicCostFactor(CPUCOST_EVAL_SIMPLE_PREDICATE) +
...
Cost *CostMethodIsolatedScalarUDF::computeOperatorCostInternal(RelExpr *op, const Context *myContext,
  ...
  CostScalar resultSetCardinality =
      udf->getGroupAttr()->outputLogProp(myContext->getInputLogProp())->getResultCardinality();
  // $$$ Due to a bug in histograms (up to tag A091197_1)
  // $$$ sometimes the cardinality is negative, if so, fix it
  // $$$ to pass regressions:
  if (resultSetCardinality.isLessThanZero() /* < csZero */) resultSetCardinality = CostScalar(fanOut);
  ...
  // ---------------------------------------------------------------------
  SimpleCostVector cvFR(cpu / countOfStreams_                                     // converting CPU instr
                            * CURRSTMT_OPTDEFAULTS->getTimePerCPUInstructions(),  // into time
                        io / countOfStreams_, msgs / countOfStreams_, csZero, noOfProbesPerStream_);
  ...
  // ---------------------------------------------------------------------
  SimpleCostVector cvLR(cpu / countOfStreams_ * CURRSTMT_OPTDEFAULTS->getTimePerCPUInstructions(), io / countOfStreams_,
                        msgs / countOfStreams_, csZero, noOfProbesPerStream_);
