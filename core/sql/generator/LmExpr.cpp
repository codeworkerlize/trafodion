...
//
static ItemExpr *CreateLmString(ItemExpr &source, const NAType &sourceType, ComRoutineLanguage language,
                                ComRoutineParamStyle style, CmpContext *cmpContext) {
  //
...
static ItemExpr *CreateIntervalExpr(ItemExpr &source, const NAType &target, CmpContext *cmpContext) {
  ...
  if (!target.supportsSQLnull()) T += " NOT NULL";
...
LmExprResult CreateLmOutputExpr(const NAType &formalType, ComRoutineLanguage language, ComRoutineParamStyle style,
                                CmpContext *cmpContext, ItemExpr *&normalizedValue, ItemExpr *&outputValue,
  ...
  if (replyType) {
    ...
    if (normalizedValue) {
      ...
      if (formalType.getTypeQualifier() == NA_INTERVAL_TYPE && isString)
        outputValue = CreateIntervalExpr(*normalizedValue, formalType, cmpContext);
      else {
        // Fix for bug 3137.
        // Set the parser flag to allow VARCHAR(0) as proxy column type
        // before entering the parser. See comment above for more detail.
        if (isResultSet && formalType.getTypeQualifier() == NA_CHARACTER_TYPE) inRSProxyStmt = TRUE;
        outputValue = CreateCastExpr(*normalizedValue, formalType, cmpContext);
        inRSProxyStmt = FALSE;
      }
...
NABoolean LmTypeIsString(const NAType &t, ComRoutineLanguage language, ComRoutineParamStyle style,
  ...
  if (t.getTypeQualifier() == NA_NUMERIC_TYPE) {
    ...
    if (nt.isExact()) {
      if (nt.binaryPrecision()) {
        ...
        if (isResultSet)
          result = FALSE;
        else if (language != COM_LANGUAGE_C && language != COM_LANGUAGE_CPP && style != COM_STYLE_JAVA_OBJ)
          result = TRUE;
        else if (nt.isBigNum())
          result = TRUE;
        else
  ...
  } else if (t.getTypeQualifier() == NA_INTERVAL_TYPE && (style == COM_STYLE_JAVA_OBJ || style == COM_STYLE_CPP_OBJ)) {
...
NABoolean LmTypeSupportsScale(const NAType &t) {
  ...
  switch (q) {
    ...
    case NA_DATETIME_TYPE: {
      ...
      if (subtype == DatetimeType::SUBTYPE_SQLTime || subtype == DatetimeType::SUBTYPE_SQLTimestamp) {
