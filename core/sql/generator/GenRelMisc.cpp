...
short RelDumpLoad::codeGen(Generator *generator)
{
  return DDLExpr::codeGen(generator);
}

short RelGenLoadQueryCache::codeGen(Generator* generator)
{
  return DDLExpr::codeGen(generator);
}

short RelBackupRestore::codeGen(Generator * generator)
{
  return DDLExpr::codeGen(generator);
}
...
short RelRoot::codeGen(Generator * generator)
{
  ...
  generator->setBloomFilterUseNativeByteOrder(
                this->decideAllScansAreORCScans() || useCppReader
                                             );
  ...
  if (compFragDir->containsESPLayer() &&
      availableNodes &&
      !availableNodes->canUseAllNodes(CURRCONTEXT_CLUSTERINFO))
    {
      ...
      serializedNodesBuf =
        space->allocateAndCopyToAlignedSpace(serializedNodes.c_str(),
                                             serializedNodes.length() + 1,
                                             0);
...
//      total = MINOF(operatorCost->convertToElapsedTime(), 1e32).getValue();
 ...
 OperatorTypeEnum child_op_type = childOperType();
    ...
    if (NATable::usingSentry())
      {
        ...
        Lng32 refreshIntervalInSeconds = 
          CmpCommon::getDefaultLong(PRIV_SENTRY_RECHECK_INTERVAL);
  ...
  if (CmpCommon::getDefault(HIVE_SUPPORTS_SUBDIRECTORIES) == DF_ON)
    root_tdb->setHiveAllowSubdirs();

  OperatorTypeEnum childOper = childOperType();
  OperatorTypeEnum currChildOper = 
    (child(0) ? child(0)->castToRelExpr()->getOperatorType() : childOper);
  ...
  if (currChildOper == REL_DDL)
   ...
   if (CmpCommon::getDefault(MAINTENANCE_WINDOW) == DF_ON)
      root_tdb->setMaintenanceWindow(TRUE);
  ...
  if ((updDelInsert) &&
            ...
            (child(0)->castToRelExpr()->getOperatorType() == REL_NESTED_JOIN_FLOW))
    {
      ...
      if(tuple_flow->child(1) && tuple_flow->child(1)->castToRelExpr() &&
         tuple_flow->child(1)->castToRelExpr()->getOperator().match(REL_ANY_HBASE_GEN_UPDATE) &&
         (NOT output_expr) && tuple_flow->child(0) && tuple_flow->child(0)->castToRelExpr() &&
  ...
  else
  {
	...
	root_tdb->setQueryType(ComTdbRoot::SQL_INSERT_NON_UNIQUE);
      ...
      root_tdb->setMayNotCancel(TRUE);
             ...
             REL_RENAME_TABLE == childOperType())
    { //for select 1 from dual
      ItemExpr *selectList = this->selectList();
      if (selectList)
        if (selectList->getOperatorType() == ITM_CONSTANT ||
            selectList->getOperatorType() == ITM_CACHE_PARAM)
          root_tdb->setMayNotCancel(TRUE);
        else if (selectList->getOperatorType() == ITM_CAST)
        {//trafci
          ItemExpr *child0 = selectList->child(0)->castToItemExpr();
          if (child0->getOperatorType() == ITM_CONSTANT ||
              child0->getOperatorType() == ITM_CACHE_PARAM)
            root_tdb->setMayNotCancel(TRUE);
        }
    }
  }
  ...
  if (updDelInsert && child(0) && child(0)->castToRelExpr() &&
      (NOT output_expr))
  {
    if (root_tdb->getQueryType() == ComTdbRoot::SQL_INSERT_NON_UNIQUE)
    {
      if (child(0)->isUniqueOper())
        root_tdb->setQueryType(ComTdbRoot::SQL_INSERT_UNIQUE);
    }
  ...
  if (child(0) && child(0)->castToRelExpr() &&
      root_tdb->getQueryType() == ComTdbRoot::SQL_SELECT_NON_UNIQUE)
  {
    if (child(0)->isUniqueOper())
      root_tdb->setQueryType(ComTdbRoot::SQL_SELECT_UNIQUE);
  }
  ...
  if (generator->useRegionXN())
    root_tdb->setUseRegionXN(TRUE);
  ...
  if(generator->hiveWriteAccess())
    root_tdb->setHiveWriteAccess(TRUE);
                        ...
                        NABoolean operatorCIF)
{
  ...
  if(sortRecLen >= sort_options->scratchIOBlockSize())
  {
    ...
    ULng32 padlen = ((sort_options->scratchIOBlockSize() & 511) == 0)? 0 :\
                    (512 - (sort_options->scratchIOBlockSize() & 511));
...
short ConnectBy::codeGen(Generator * generator)
{
  ...
  if(priorValues().entries()>0) {
	...
	expGen->processValIdList(priorValues(),
                               ExpTupleDesc::SQLARK_EXPLODED_FORMAT,
                               priortupleLength, 0, priorValsValsDownAtpIndex,
                               &priorValueExprDesc,
                               ExpTupleDesc::LONG_FORMAT);
  ...
  if( pseudoColList().entries() > 0)
  {
    Attributes **   attrs =
     new(generator->wHeap())
    Attributes * [pseudoColList().entries()];
    ...
    expGen->processAttributes(pseudoColList().entries(),
                           attrs, tupleFormat,
                           ptupleLength,
                           0, fixedPseudoColRowAtpIndex,
  ...
  {
    Attributes **   attrs =
     new(generator->wHeap()) Attributes * [1];
    ...
    expGen->processAttributes(1,
                           attrs, tupleFormat,
                           pathTupleLen,
                           0, pathPseudoColRowAtpIndex,
                           &pathtupleDescReturn, ExpTupleDesc::LONG_FORMAT);
  ...
  for(int i=0; i< childColList().entries(); i++)
  {
    ...
    newsnode = new(generator->wHeap())
       Cast(((childColList()[i] ).getValueDesc())->getItemExpr(),
         &parentColList()[i].getType());
    ...
    newcnode = new(generator->wHeap())
               Cast((( rchildColList()[i] ).getValueDesc())->getItemExpr(),
                    &parentColList()[i].getType());
  ...
  if (left_val_id_list.entries() > 0) {
    expGen->generateContiguousMoveExpr(left_val_id_list,
				       0,
  ...
  if(pathItemList().entries() > 0)
  {
    expGen->generateContiguousMoveExpr(leftPathItemList(),
                                       1,
                                       1,  //this is the connectby atp
                                       pathEvalAtpIndex,
                                       ExpTupleDesc::SQLARK_EXPLODED_FORMAT,
                                       ...
                                       &path_tuple_desc,
                                       ExpTupleDesc::LONG_FORMAT); 
    expGen->generateContiguousMoveExpr(rightPathItemList(),
                                       1,
                                       1,  //this is the connectby atp
                                       pathEvalAtpIndex,
                                       ExpTupleDesc::SQLARK_EXPLODED_FORMAT,
  ...
  if(priorLeftSrcList.entries()>0) {
    Attributes **   attrs =
         new(generator->wHeap())
         Attributes * [priorLeftSrcList.entries()];
    ...
    expGen->processAttributes(priorLeftSrcList.entries(),
                           attrs, ExpTupleDesc::SQLARK_EXPLODED_FORMAT,
                           priortupleLength,
                           0, returnRowAtpIndex,
                           &priorPredExprDesc, ExpTupleDesc::SHORT_FORMAT);
  ...
  for (i = 0; i < colMapTable().entries(); i++)
    {
      ...
      Attributes * attr =
       ...
       generator->addMapInfo( vidu_node->getValueId(),
  ...
  if (NOT selectionPred().isEmpty()) {
    ...
    expGen->generateExpr(condExp->getValueId(), 
  ...
  ComTdbConnectBy* connectBy_tdb
    = new(space) ComTdbConnectBy(
                              workCriDesc,
                              given_desc,
                              returned_desc,
                              s_child_tdb->getMaxQueueSizeDown(),
                              s_child_tdb->getMaxQueueSizeUp(),
                              20, 4096*2000, //the reason is to ensure one of the Agree query can have enough memory
                              s_child_tdb,
                              c_child_tdb,
                              leftRowLength,
                              ptupleLength, 
  ...
  if(!generator->explainDisabled()) {
    generator->setExplainTuple(
                addExplainInfo(connectBy_tdb, childExplainTuple, connExplainTuple, generator));
  }
...
short ConnectByTempTable::codeGen(Generator* generator)
{
  ...
  ItemExpr *probeHashAsIeInput = new (generator->wHeap())
  ...
  GenAssert(nTypInput.isSigned() == FALSE,
            "Unexpected signed HashDistPartHash.");

  GenAssert(probeHashAsIeInput->getValueId().getType().supportsSQLnullLogical()
            == FALSE, "Unexpected nullable HashDistPartHash.");

  ItemExpr *hvAsIeInput = new (generator->wHeap()) Cast(
       probeHashAsIeInput, 
       new (generator->wHeap()) 
  ...
  GenAssert(hvLengthInput == sizeof(Lng32),
  ...
  for (inputListIndex = 0; 
    ...
    CompEncode * enodeInput = new(generator->wHeap()) 
      CompEncode(inputIeInput, FALSE /* ascend/descend doesn't matter*/);
  ...
  expGen->generateContiguousMoveExpr(encodeInputAsListInput, 
                              0, //don't add conv nodes
                              work_atp, encodeInputProbeDataIdx,
                              ExpTupleDesc::SQLARK_EXPLODED_FORMAT,
  ...
  ComTdbConnectByTempTable  * connectByTempTable_tdb
    = new(space) ComTdbConnectByTempTable(
                              child_tdb,
                              hvExpr,
                              encodeInputExpr,
                              innerRecExpr,
                              ...
                              child_tdb->getMaxQueueSizeUp(),
                              1, 4096*10000,
                              encodeInputHostVarExpr,
                              hvInputExpr,
                              hashInputValIdx,
                              encodeInputProbeDataIdx,
                              selectPred);
  ...
  if(!generator->explainDisabled()) {
    generator->setExplainTuple(
                               addExplainInfo(connectByTempTable_tdb, childExplainTuple, 0, generator));
  }
...
const char * QueryInvalidationFunc::getVirtualTableName()
{return getVirtualTableNameStr();}
...
TrafDesc *QueryInvalidationFunc::createVirtualTableDesc()
{
  TrafDesc * table_desc =
    Generator::createVirtualTableDesc(getVirtualTableName(),
				      NULL, // let it decide what heap to use    
				      ComTdbQryInvalid::getVirtTableNumCols(),
				      ComTdbQryInvalid::getVirtTableColumnInfo(),
				      ComTdbQryInvalid::getVirtTableNumKeys(),
				      ComTdbQryInvalid::getVirtTableKeyInfo());
  return table_desc;
}
...
short QueryInvalidationFunc::codeGen(Generator* generator)
{
  ...
  Lng32 numBuffers = 3;
  Lng32 bufferSize = 10 * tupleLength;
  ComTdbQryInvalid *qiTdb = new(space) ComTdbQryInvalid
    (
	 tupleLength,			 // Length of stats Tuple
	 tupleLength,                    // Length of returned tuple
	 inputTupleLength,               // length of input tuple
  ...
  if(!generator->explainDisabled()) {
    generator->setExplainTuple(
       addExplainInfo(qiTdb, 0, 0, generator));
  }
  generator->setCriDesc(givenDesc, Generator::DOWN);
  generator->setCriDesc(returnedDesc, Generator::UP);
