...
{ 
  ...
  if (maybeInMD())
  {
    ...
    NATable * describedTable = bindWA->getSchemaDB()-> 
      getNATableDB()->get(descCorrName, bindWA, NULL, FALSE);
...
int HbaseAccess::createAsciiColAndCastExprForExtStorage
(Generator * generator,
 ...
 ItemExpr *&castValue)
{
  ...
  if (asciiType->isComposite())
    {
      NAType *newCompType = 
        ((CompositeType*)asciiType)->newCopy2(COM_SQLARK_EXPLODED_FORMAT, h);

      Int32 hiveSrcMaxLen = ((CompositeType*)asciiType)->getHiveSourceMaxLen();
      SQLVarChar * vcAsciiType = new(h) SQLVarChar(h, hiveSrcMaxLen,
                                                   TRUE, FALSE, FALSE,
      ...
      if (((CompositeType*)newCastType)->getCompFormat() != 
          compType1.getCompFormat())
        {
                              ...
                              NABoolean& parquetFullScan)
{
  ...
  LIST(ComCompressionInfo) compressionTypes(generator->wHeap());

  NABoolean isOrc = hTabStats->isOrcFile();
  NABoolean isParquet = hTabStats->isParquetFile();
  NABoolean isAvro = hTabStats->isAvroFile();
  NABoolean isExt = (isOrc || isParquet || isAvro);

  NAString nodeListForSplitUnit = NAString(generator->wHeap());
  ...
  if ((NOT isExt) && CmpCommon::getDefault(HDFS_USE_CURSOR_MULTI) == DF_ON)
    useCursorMulti = TRUE;
    
  const NodeMap* nmap = (mypart ? mypart->getNodeMap() : NULL);

  doSplitFileOpt = 
    ((useCursorMulti || isExt) ? FALSE : TRUE);
                  
  parquetFullScan = TRUE;

  if  ((nmap && nmap->type() == NodeMap::HIVE) &&
       (mypart))
    {
      Queue * parqQ = new(generator->wHeap()) Queue(generator->wHeap());
      Lng32 entryNum = 0;
      for (CollIndex i=0; i < nmap->getNumEntries(); i++ )
	{
	  ...
	  for (CollIndex j=0; j<scanInfo.entries(); j++ ) 
		...
		hfi.setFileIsLocal(TRUE);
	      ...
	      if ((NOT isExt) && offset > 0)
		hfi.setFileIsSplitBegin(TRUE);

	      if ((NOT isExt) && (offset + span != file->getTotalSize()))
		hfi.setFileIsSplitEnd(TRUE);

	      if (isParquet)
                 if ( !scanInfo[j].isFullScan_ )
                    parquetFullScan = FALSE;
	      ...
	      if (isStrawScan){
	    	  ...
	    	  nodeListForSplitUnit.clear();
	    	  Int64 blkNumber =  bestBlockNb(offset, span,file->getBlockSize());
	    	  char str[10];
	    	  for (Int32 k=0;k<file->getReplication();k++){
	    	      if (k!=0)
	    	    	 nodeListForSplitUnit = nodeListForSplitUnit.append(' ');
	    	      HostId hostId = file->getHostId(k,blkNumber);
	    	      if (hostId != HHDFSFileStats::InvalidHostId){
	    	    	  itoa(hostId,str,10);
	    	    	  nodeListForSplitUnit = nodeListForSplitUnit.append(str);
	    	      }else
	    	    	  nodeListForSplitUnit = nodeListForSplitUnit.append("-1");
	    	  }
		      char * splitUnitBlockLocations = space->allocateAndCopyToAlignedSpace(nodeListForSplitUnit.data(),
		    		  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	    strlen(nodeListForSplitUnit.data())
																					, 0);
	    	  hfi.splitUnitBlockLocations_ = splitUnitBlockLocations;
	    	  hfi.hdfsBlockNb_ = (Lng32)blkNumber;
	      }

              // BEGIN AVRO TEMP CODE
              // if this fname already exists in hdfsFileInfoList, set beginRangeNum
              // and numRanges to 0.
              // This is temporary until right/complete support to map parquet tables to nodes
              // and esp is added.
              if (isAvro)
                {
                  NABoolean found = FALSE;
                  parqQ->position();
                  while ((! parqQ->atEnd()) && (NOT found))
                    {
                      char * fn = (char *)parqQ->getNext();
              ...
              if ((partColValuesLen > 0) && (hiveSearchKey))
                {
                  hfi.partColValues_ =
                    space->allocateAndCopyToAlignedSpace(
              ...
              if (isExt) {
                hfi.compressionTypeIx_ = -1; // not used for ORC, parquet, avro
                hfi.compressionMethod_ = ComCompressionInfo::UNCOMPRESSED;
              } 
              else
                {
                  ...
                  hfi.compressionMethod_ = file->getCompressionInfo().getCompressionMethod(); 
                  if (file->getCompressionInfo().getCompressionMethod() != ComCompressionInfo::UNCOMPRESSED) {
                     isCompressedFile = TRUE;
                     // Ensure that the full file is read
                     // just not the compressed file size
                     if (CmpCommon::getDefault(USE_LIBHDFS) == DF_OFF)
                        hfi.bytesToRead_ = LONG_MAX; 
                  }
                }
...
short FileScan::genExtStoragePPI(Generator * generator,
                                 NAList<ExtPushdownPredInfo> *listOfExtPPI,
                                 const ColStatDescList& colStatsList,
                                 Queue * &tdbListOfExtPPI,
                                 ValueIdList &extOperVIDlist)
{
  Space * space          = generator->getSpace();

  tdbListOfExtPPI = NULL;
  if (listOfExtPPI && (listOfExtPPI->entries() > 0))
    {
      tdbListOfExtPPI = new(space) Queue(space);
      
      ComTdbExtPPI * tdbPPI = NULL;
              
      UInt64 rc = 0;
      UInt64 uec = 0;
      UInt64 rowsToSurvive = 0;

      for (Lng32 i = 0; i < listOfExtPPI->entries(); i++)
        {
          ExtPushdownPredInfo &ppi = (*listOfExtPPI)[i];
	  ...
	  NAString colType;
          ...
          if (colValId != NULL_VALUE_ID)
            {
              ...
              if (nac)
                {
                  NAString nacLC(nac->getColName());
                  nacLC.toLower();
                  colName =
                    space->allocateAndCopyToAlignedSpace
                    (nacLC.data(), nacLC.length(), 0);
                }

              // Setup the threshold
              if ( ppi.getType() == IN ) {
                 rowsToSurvive = (UInt64)
                   (generator->findRowsToSurvive(colValId).getValue());
              }

              // figure out the average frquency of a unique value, without 
              // any local predicates applied
              ColStatsSharedPtr colStats = 
                     colStatsList.getColStatsPtrForColumn(colValId);
    
              if ( !(colStats == NULL) ) {
                rc = (UInt64)((colStats->getBaseRowCount()).value());
                uec = (UInt64)((colStats->getBaseUec()).value());
              }
            }

          ValueId operValId = ppi.operValId();
  
          Lng32 operAttrIndex = extOperVIDlist.entries();

          Int32 items = 0;

          if ( ppi.getType() == IN && (items=ppi.operValIdList().entries()) > 0 ) {

             const ValueIdList& inList = ppi.operValIdList();

             for (Int32 i=0; i<inList.entries(); i++) {
                operValId = genExtStoragePPIExpression(generator, 
                                                    colValId, inList[i]);

                extOperVIDlist.insert(operValId);
             }
          ...
          const char* ft = ppi.getText().data();

          char* filterSignature = space->
               allocateAndCopyToAlignedSpace(ft, strlen(ft), 0);

          tdbPPI = 
                  new(space) ComTdbExtPPI(type, colName, operAttrIndex, 
                                       (operValId != NULL_VALUE_ID ? 
                                        operValId.getType().getTypeName() :
                                        NAString("")), items, 
                                        filterSignature,
                                        ppi.getFilterId(),
                                        rc, uec, rowsToSurvive);

          tdbListOfExtPPI->insert(tdbPPI);
...
ValueId 
FileScan::genExtStoragePPIExpression(Generator * generator, 
                                     ValueId colValId,
                                     ValueId operValId)
{
   NABoolean isRangeOfValuesVal = FALSE;

   ItemExpr* c = operValId.getItemExpr();

   if ( c->getOperatorType() == ITM_HOSTVAR ) {
     isRangeOfValuesVal = 
          ((HostVar*)(c))->isSystemGeneratedRangeOfValuesHV();
   }

   c = NULL;
 
   if ( !isRangeOfValuesVal )  {

     if (NOT (colValId.getType().equalIgnoreNull(operValId.getType())) )
     {
       c = new(generator->wHeap()) 
         Cast(operValId.getItemExpr(), &colValId.getType());
     ...
     // underlying hive column datatype could be overwritten by
     // a traf external table datatype.
     // if hive column is a TIMESTAMP but current type is not,
     // convert source to TIMESTAMP. This is needed so data could
     // be passed in correct format during pred pushdown.
     if ((colValId.getType().getHiveType() == HIVE_TIMESTAMP_TYPE) &&
         (colValId.getType().getTypeQualifier() == NA_DATETIME_TYPE) &&
         (operValId.getType().getTypeQualifier() == NA_DATETIME_TYPE) &&
         (((DatetimeType&)colValId.getType()).getRecDateTimeCode() != 
          REC_DTCODE_TIMESTAMP))
       {
         c = new(generator->wHeap()) 
           Cast(operValId.getItemExpr(), 
                new(generator->wHeap())
                SQLTimestamp(generator->wHeap(),
                             operValId.getType().supportsSQLnull(), 6));
         c = c->bindNode(generator->getBindWA());
         c = c->preCodeGen(generator);
     ...
     NAType * newTyp = new (generator->wHeap())
       SQLVarChar(generator->wHeap(),operValId.getType().getDisplayLength(), 
                             operValId.getType().supportsSQLnull());
              
     c = new(generator->wHeap()) Cast(operValId.getItemExpr(), newTyp);
   } else {

     // no need to put a cast() on top of the range spec object.
     c = new(generator->wHeap()) 
             Aggregate(ITM_RANGE_VALUES_COPY,
                       operValId.getItemExpr(),
                       FALSE);
   }
...
static short genParqColSchStr(Generator *generator, 
                              const NAString &inColName, 
                              const NAType *nat,
                              NAString &schStrNAS)
{
    ...
    case HIVE_DECIMAL_TYPE:
      {
        ...
        schStrNAS += NAString(" fixed_len_byte_array(") + 
          str_itoa(numBytes,buf1) + ") ";
        schStrNAS += colName + " (DECIMAL(" + str_itoa(precision, buf1) + "," +
          str_itoa(scale, buf2) + "))" + ";";
      }
      break;
    ...
    case HIVE_ARRAY_TYPE:
      {
        ...
        if (genParqColSchStr(generator, "array_element", elemType, schStrNAS))
          return -1;
        ...
        for (Int32 i = 0; i < sr->getNumElements(); i++)
          {
            ...
            if (genParqColSchStr(generator, elemName.data(), 
                                 elemType, schStrNAS))
              return -1;
          } // for
        schStrNAS += " }";        
...
short FileScan::createParqTableSchStr(Generator *generator,
                                      NAString &tableName,
                                      const NAColumnArray &hiveColArr,
                                      char* &schStr)
{
  Space * space = generator->getSpace();
  ...
  for (int i = 0; i < hiveColArr.entries(); i++)
    {
      ...
      if ((hiveNACol->isHiveVirtualColumn()) ||
          (hiveNACol->isHivePartColumn()))
        continue;

      if (genParqColSchStr(generator, hiveNACol->getColName(), 
                           hiveNAType, schStrNAS))
        return -1;
    }

  schStrNAS += "}";

  schStr = space->allocateAndCopyToAlignedSpace
    (schStrNAS.data(), schStrNAS.length(), 0);
...
short FileScan::createHiveColNameAndTypeLists(
     Generator * generator,
     const NAColumnArray &hiveColArr,
     Queue* &colNameList,
     Queue* &colTypeList)
{
  ...
  if (colTypeList == NULL)
    colTypeList = new(space) Queue(space);
  ...
  for (int i = 0; i < hiveColArr.entries(); i++)
    {
      ...
      char * ctypeInList = 
        space->allocateAndCopyToAlignedSpace
        ((char*)&typeInfo, sizeof(typeInfo), 0);
...
short FileScan::codeGenForHive(Generator * generator)
{
  ...
  for (CollIndex i = 0; i < allHdfsVals.entries(); i++)
  {
    ...
    if (NOT inExecutorPred)
      {
        inExecutorPred = 
          parquetPushdownPreds()
          .referencesTheGivenValue(allHdfsVals[i],dummyVal);
      }
    ...
    if (!isPartCol && !isVirtCol)
      {
        ...
        if ((isOrc && CmpCommon::getDefault(ORC_COLUMNS_PUSHDOWN) == DF_OFF) ||
            (isParquet && CmpCommon::getDefault(PARQUET_COLUMNS_PUSHDOWN) == DF_OFF) ||
            (isAvro && CmpCommon::getDefault(TRAF_AVRO_COLUMNS_PUSHDOWN) == DF_OFF))
          {
            inProjExpr = TRUE;
          }
  ...
  const Int32 extOperTuppIndex = 5;
  ...
  // external storage row is returned by hdfs as len/value pair for each column.
  ...
  for (int ii = 0; ii < (int)allHdfsVals.entries();ii++)
  {
    ...
    extColLen = sizeof(Lng32);
    extColLen += givenType.getDisplayLength();

    if ( maxExtColLen < extColLen )
      maxExtColLen  = extColLen;
    ...
    if (isExt)
      res = HbaseAccess::createAsciiColAndCastExprForExtStorage
        (
             generator,        // for heap
    ...
    extRowLen += sizeof(Lng32);
    extRowLen += givenType.getDisplayLength();
  ...
  Queue * tdbListOfExtPPI = NULL;
  NABoolean isStrawScan = FALSE;
  NABoolean isCompressedFile = FALSE;
  NABoolean parquetFullScan = TRUE;
  if (hTabStats->isTextFile() || hTabStats->isSequenceFile() || isExt)
    {
	  ...
	  isStrawScan = isExt && CmpCommon::getDefault(HIVE_STRAW_SCAN) == DF_ON
				&& getProbes()<=1 //the probe check is to disable strawScan for NJ
				...
				&& generator->getNbStrawScans()<=CmpCommon::getDefaultLong(HIVE_STRAW_SCAN_MAX_NODES); // turn off straw scan if too many nodes.
      ...
      //disable strawScan if number of stripes to process is lower than DOP, as it does not provide benefit, while paying extra
      //strawscan init cost
      if (isStrawScan){
        if (getPhysicalProperty()->getPartitioningFunction()->getCountOfPartitions() >= hdfsFileInfoList->entries())
    	  isStrawScan = FALSE;
      }
    }
  ...
  if (isExt)
    {
      if (createHiveColNameAndTypeLists(
               generator, 
               (btHiveColArr.entries() > 0 ? btHiveColArr : btColArr),
      ...
      if (isParquet)
        {
          NAString tab = 
            GenGetQualifiedName(getTableDesc()->getNATable()->getTableName());
          if (createParqTableSchStr(
                   generator, 
                   tab,
                   (btHiveColArr.entries() > 0 ? btHiveColArr : btColArr),
                   parqSchStr))
  ...
  if (isExt)
    {
      ...
      const ColStatDescList& myColList = 
              getGroupAttr()->
                 outputLogProp((*GLOBAL_EMPTY_INPUT_LOGPROP))->colStats();

      genExtStoragePPI(generator, 
                       &extListOfPPI(),
                       myColList,
                       tdbListOfExtPPI,
                       extOperVIDlist);
      ...
      for (int i = 0; i < neededHdfsVals.entries(); i++)
        {
          ...
          HdfsColInfo hco(cnameInList, nac->getPosition()+1, 
                          nac->getType()->getHiveType());
          char * hcoInList = space->allocateAndCopyToAlignedSpace
            ((char*)&hco, sizeof(HdfsColInfo));
          hdfsColInfoList->insert(hcoInList);
        }

      if (extOperVIDlist.entries() > 0)
        {
          ExpTupleDesc * extOperTupleDesc = NULL;
          exp_gen->generateContiguousMoveExpr(
               extOperVIDlist, 0,
               work_atp, extOperTuppIndex, 
               ExpTupleDesc::SQLARK_EXPLODED_FORMAT,
               extOperLength,
               &extOperExpr,
               &extOperTupleDesc,
          ...
          work_cri_desc->setTupleDescriptor(extOperTuppIndex, extOperTupleDesc);
        }

      if (extListOfPPI().entries() > 0)
        {
  ...
  if (hTabStats->isTextFile())
    ...
    type = (short)ComTdbHdfsScan::PARQUET_;
  else if (isAvro)
    type = (short)ComTdbHdfsScan::AVRO_;
  else {
  ...
  Int64 hdfsBufSize = 0;
  Int32 extMaxRowsToFill = 0;
  Int32 extMaxAllocationInMB = 0;

  // We set the use cpp reader flag in generator in RelRoot::codeGen(), before
  // descending to the child/grand child for codeGen() tasks. We must use
  // that flag here. This is to make sure all scans use either the Java
  // or the cpp/arrow reader. The mix situation is not allowed because there are 
  // two types of bloom filters generated at the HJ operators: the native 
  // type where the values are hashed in their native byte order, and
  // the big endian order type where the values are converted to the
  // big-endian order first before hashing. And at the HJ operator level, we
  // only generate one type of bloom filter for every scan below.
  NABoolean useNativeReaderForParquet = generator->parquetUseCppReader(); 

  if (isExt)
    {
      extMaxRowsToFill = 
        CmpCommon::getDefaultNumeric(ORC_READER_MAX_ROWS_TO_FILL);
      extMaxAllocationInMB = CmpCommon::getDefaultNumeric(ORC_READER_MAX_ALLOCATION_MB);
      Lng32 extRowLenForAllocation = 
        CmpCommon::getDefaultNumeric(ORC_READER_ALLOCATE_ROW_LENGTH);

      if (extRowLenForAllocation == 0)   // if CQD is 0
        extRowLenForAllocation = extRowLen;  // use generator estimate

      // For CPP parquet reader, request the buffer size to be
      // at least the max column size. This is necessary to handle
      // select count(*) query where the projection and where clause
      // may be empty, and we just need to select a column with 
      // least width.
      if ( useNativeReaderForParquet )
        extRowLenForAllocation = MAXOF(extRowLenForAllocation, maxExtColLen);

      extRowLenForAllocation += 100;  // allow an extra 100 bytes per row

      hdfsBufSize = extRowLenForAllocation; // hdfsBufSize is expected row length in bytes
    }
  else
   {
     // CQD default value is in K bytes
  ...
  char * serializeLib = NULL;
  if (hTabStats->getSerializeLib())
    {
      serializeLib = 
        space->allocateAndCopyToAlignedSpace(hTabStats->getSerializeLib(),
                                             strlen(hTabStats->getSerializeLib()),
                                             0);
    }
  ...
  if ((CmpCommon::getDefault(HIVE_DATA_MOD_CHECK) == DF_ON) &&
      ...
      (!getCommonSubExpr()) &&
	  (!hTabStats->isS3Hosted())) //data mod check on S3 is not possible
    {
  ...
  if (isExt) {
    ...
    defs.getFloat(RANGE_OPTIMIZED_SCAN_SELECTIVITY, maxSelectivity);

    Int32 numFilters = 
         (Int32)(defs.getAsLong(RANGE_OPTIMIZED_SCAN_MAX_NUM_FILTERS));

    Int32 cpp_reader_threads = 
        (Int32)(defs.getAsLong(PARQUET_CPP_READER_PARALLELISM));

    NABoolean skipColumnChunkStatscheck = 
         (CmpCommon::getDefault(PARQUET_CPP_READER_SKIP_STATS_CHECK) == DF_ON);

    NABoolean filterRowgroups =
         (CmpCommon::getDefault(PARQUET_CPP_READER_FILTER_ROWGROUPS) == DF_ON);

    NABoolean filterPages =
         (CmpCommon::getDefault(PARQUET_CPP_READER_FILTER_PAGES) == DF_ON);

    NABoolean directCopy =
         (CmpCommon::getDefault(PARQUET_CPP_READER_DIRECT_COPY) == DF_ON);
    ...
    hdfsscan_tdb = new(space)
      ...
      (
           ...
           colNameList, colTypeList,
           FALSE, // never remove MinMax from ScanFilter
           maxSelectivity, 
           numFilters,
           useNativeReaderForParquet,
           parquetFullScan,
           cpp_reader_threads,
           skipColumnChunkStatscheck,
           filterRowgroups,
           filterPages,
           directCopy,
           parqSchStr
    ...
    hdfsscan_tdb = new(space)
      ...
      (
           ...
           nullFormat,
           serializeLib,
           hTabStats->getSkipHeaderCount(), // lines to skip for text files
           hdfsBufSize,
           rangeTailIOSize,
           partCols.entries(),
           executorPredColsRecLength,
  ...
  hdfsscan_tdb->setRecordLength(getGroupAttr()->getRecordLength());

  generator->initTdbFields(hdfsscan_tdb);

  if (isOrc) 
  {
     if ((CmpCommon::getDefault(ORC_VECTORIZED_SCAN) == DF_ON)) 
     {
        ((ComTdbExtStorageScan*)hdfsscan_tdb)->setOrcVectorizedScan(TRUE);
        ((ComTdbExtStorageScan*)hdfsscan_tdb)->setVectorBatchSize((Int32)CmpCommon::getDefaultLong(ORC_VECTOR_BATCH_SIZE));
     }    
  }
  ...
  if (isParquet) 
    {
      if ((CmpCommon::getDefault(PARQUET_LEGACY_TIMESTAMP_FORMAT) == DF_ON))
        ((ComTdbExtStorageScan*)hdfsscan_tdb)->setParquetLegacyTS(TRUE);
      if ((CmpCommon::getDefault(PARQUET_STRINGS_SIGNED_MIN_MAX) == DF_ON))
        ((ComTdbExtStorageScan*)hdfsscan_tdb)->setParquetSignedMinMax(TRUE);
    }
  ...
  hdfsscan_tdb->setStrawScanMode(isStrawScan);

  char * strawscanWebserver = space->AllocateAndCopyToAlignedSpace(CURRCONTEXT_CLUSTERINFO->getActiveNodeNamesAsCommaSeparatedList(), 0);
  hdfsscan_tdb->setStrawscanWebServer(strawscanWebserver);
                             ...
                             const NABoolean isForGetUIDName)
{
  ...
  if (naTable && isForGetUIDName)
  {
        ...
        ComIsUserTable(tableName))
    {
      ...
      {
        NAString tabNameforUID = tabNS + tableName.getQualifiedNameObj().getCatalogName() + "__" + Int64ToNAString(objUID);
        gendTablename = space->AllocateAndCopyToAlignedSpace(tabNameforUID, 0);
      }
    }
    return 0;
  ...
  if (gendTablename == NULL)
    {
      if (tabNS.isNull())
        gendTablename = space->AllocateAndCopyToAlignedSpace(
             GenGetQualifiedName(tableName, FALSE, asAnsiString), 0);
      else
        {
          NAString tabName = 
            tabNS + 
            GenGetQualifiedName(tableName, FALSE, asAnsiString);
...
short HbaseAccess::codeGen(Generator * generator)
{
 ...
 if (genTableName(generator, space, getTableName(),
  ...
  if ( getTableDesc()->getNATable()->isPartitionEntityTable() &&
       getTableDesc()->getPreconditions().entries() > 0 )
  {
    ValueIdList &precond = getTableDesc()->getPreconditions();
    ItemExpr *scanPreCondTree = precond.rebuildExprTree(ITM_AND, TRUE, TRUE);
    expGen->generateExpr(scanPreCondTree->getValueId(),ex_expr::exp_SCAN_PRED,
                         &scanPrecondExpr);
  }
  ...
  if (! executorPred().isEmpty())
    {
      ...
      {
        for(ValueId vid = executorPred().init();
                          executorPred().next(vid);
                          executorPred().advance(vid))
        {
 ...
 // generate filter value expression, if present
  ...
  if (NOT hbRidVIDlist.isEmpty())
    expGen->assignAtpAndAtpIndex(hbRidVIDlist,
                                 0, returnedDesc->noTuples()-1);
  ...
  generator->setHBaseNumCacheRows(MAXOF(getEstRowsAccessed().getValue(),
                                  ...
                                  hbpa, hbaseRowSize, samplePerc, isNeedPushDownLimit()) ;
  ...
  if (scanPrecondExpr)
    hbasescan_tdb->setScanPreCondExpr(scanPrecondExpr);
      ...
      if (isHbaseMapFormat)
        {
          ...
          hbasescan_tdb->setTreatEmptyAsNull(
               (CmpCommon::getDefault(TRAF_HBMT_TREAT_EMPTY_STRING_AS_NULL) == DF_ON));
        }
  ...
  if ((Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL)) &&
      ((getTableName().isSeabaseMD()) ||
       (getTableName().isSeabasePrivMgrMD())))
    {
  ...
  if (skipReadConflict) {
     if (! generator->updatableSelect()) {
       if (accessType == TransMode::READ_UNCOMMITTED_ACCESS_ ||
          accessType == TransMode::READ_COMMITTED_ACCESS_)
              hbasescan_tdb->setSkipReadConflict(TRUE);
     }
  }
  else {
  ...
  if (searchKey() && searchKey()->isUnique())
  {
       if (accessOptions().accessType() == TransMode::SERIALIZABLE_ACCESS_
           && CmpCommon::getDefault(WAIT_ON_SELECT_FOR_UPDATE) == DF_ON
           && gEnableRowLevelLock == FALSE)
       {
  ...
  hbasescan_tdb->setSkipTransaction(skipTransactionForBatchGet() || 
				    skipTransactionForCursorUpdDelScan());
  ...
  if ((getTableDesc()->getNATable()->readOnlyEnabled()) &&
      (CmpCommon::getDefault(MEMORY_TABLE_SCAN) == DF_ON))
    hbasescan_tdb->setScanMemoryTable(true);
