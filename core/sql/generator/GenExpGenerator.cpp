  ...
  if (naType->isComposite())
    {
      ...
      CompositeAttributes * compAttr = 
        new(wHeap) CompositeAttributes(ct->getNominalSize());
      ...
      if (naType->getTypeQualifier() == NA_CHARACTER_TYPE) 
	{
	  ...
	  if (charType->isCaseinsensitive())
	    attr->setCaseinsensitive(1);
          ...
          if (charType->isSpaceSensitive())
              attr->setSpaceSensitive();
...
Attributes::DefaultClass ExpGenerator::getDefaultClass(const NAColumn * col)
{
  ...
  switch (col->getDefaultClass())
  {
    ...
    case COM_SYS_GUID_FUNCTION_DEFAULT:
      dc = Attributes::DEFAULT_SYS_GUID_FUNCTION; break;
    ...
    case COM_CATALOG_FUNCTION_DEFAULT:
      dc = Attributes::DEFAULT_CATALOG_FUNCTION; break;
    case COM_SCHEMA_FUNCTION_DEFAULT:
      dc = Attributes::DEFAULT_SCHEMA_FUNCTION; break;
    default:
				    ...
				    COM_VERSION   objectSchemaVersion)
{
  ...
  if ((dc == Attributes::DEFAULT_USER) && (col->getType()->getSimpleTypeName() == "VARCHAR"))
  {
    ...
    defaultValue = new(generator->getSpace()) char[ExpTupleDesc::NULL_INDICATOR_LENGTH + attr->getVCIndicatorLength() + defaultValueLen];
  }
  else
  ...
  if (attr->getVCIndicatorLength() > 0)
  {
    ...
    //adjust default length to actual length
    if((dc == Attributes::DEFAULT_USER) && (col->getType()->getSimpleTypeName() == "VARCHAR"))
      defaultValueAdj = new(generator->getSpace()) char[ExpTupleDesc::NULL_INDICATOR_LENGTH + attr->getVCIndicatorLength() + defaultValueLen];
    else
				     ...
				     ExpTupleDesc * srcTupleDesc)
{
  ...
  for (CollIndex i = 0; i < numAttrs; i++)
      ...
      if (srcAttr->getDefaultValue())
	{
       ...
       {
         //is varchar
         tgtDefLen = ExpTupleDesc::NULL_INDICATOR_LENGTH + tgtAttr->getVCIndicatorLength() + str_len(srcAttr->getDefaultValue());
         srcDefLen = ExpTupleDesc::NULL_INDICATOR_LENGTH + srcAttr->getVCIndicatorLength() + str_len(srcAttr->getDefaultValue());
       }
       else
       {
	...
	case ITM_PIVOT_GROUP:
	  {
            ...
            newExpr =
              new(wHeap()) PivotGroup(ITM_PIVOT_GROUP, item_expr->child(0), item_expr->child(1),
                                      pg->orderBy(), pg->reqdOrder(),
	  ...
	  if (nullSwitcharoo)
	    {
          ...
          attrs[i] = attr;
           ...
           ExpTupleDesc::TupleDataFormat currentTupleFormat = 
                    attr->getTupleFormat();
           ...
           if ( endOffset > tupleLength )
               tupleLength = endOffset;
                                ...
                                )
{
  ...
  Int32 maxVarcharBytes =
    (Int32) CmpCommon::getDefaultLong(COMPRESSED_INTERNAL_FORMAT_BULK_VARCHAR_MAX);
    ...
    if (srcAttr && srcAttr->getVCIndicatorLength())
      {
        ...
        if (varcharBytes > maxVarcharBytes)
          return 0;
...
short ExpGenerator::generateBulkMove(const ValueIdList &inValIdList,
				     ValueIdList &outValIdList,
				     ULng32 tupleLength,
              ...
              if (DFS2REC::isCharacterString(col_node->getValueId().getType().getFSDatatype()))
                {
                  ...
                  if (char_t.isVarchar2())
                    ((Cast*)col_node)->setPadUseZero(TRUE);
                }
              else
                {
                  ...
                  col_node = new(wHeap())
                    Cast (col_node,
                          (new(wHeap())
                           SQLBinaryString(wHeap(), char_t.getDataStorageSize(),
                                         ...
                                         NABoolean castVarcharToAnsiChar)
{
	...
	if (targetType->getTypeQualifier() == NA_CHARACTER_TYPE)
	  {
            ...
            if (keycol->getOperatorType() == ITM_TRANSLATE)
              keycol_tgt = keycol->child(0);

	    const CharType& char_t =
	      (CharType&)(keycol_tgt->getValueId().getType());
	    ...
	    if ((keycol_tgt->getValueId().getType().getVarLenHdrSize() > 0) ||
                (char_t.isUpshifted()))
	      {
		if (!CollationInfo::isSystemCollation(char_t.getCollation()))
		{
                  if ((castVarcharToAnsiChar) &&
                      (keycol_tgt->getValueId().getType().getVarLenHdrSize() > 0))
                    {
                  ...
                  else if (DFS2REC::isBinaryString(keycol_tgt->getValueId().getType().getFSDatatype()))                  
                    {
                      targetType = new(wHeap())
                        SQLBinaryString(wHeap(),
                                        keycol_tgt->getValueId().getType().getNominalSize(),
                                        keycol_tgt->getValueId().getType().supportsSQLnull(),
                                        FALSE
                                        );                
                    }
		}
  ...
  if (keycol->getOperatorType() == ITM_HBASE_ROWID)
  {
    ...
    if (sourceType->getTypeQualifier() == NA_CHARACTER_TYPE &&
         keyval->getOperatorType() != ITM_CONVERTFROMHEX)
    {
    ...
    cnode = new(wHeap()) Narrow(keyval, dataConversionErrorFlag,
                                           targetType, ITM_NARROW, desc_flag);
    knode = cnode;
  }
  else
  {
	...
	if (generateNarrow)
	  {
        ...
        if (doVarcharRPad)
          ((Narrow*)cnode)->setPadUseZero(TRUE);
	  }
	else
	  {
  ...
  for (CollIndex i = 0; i < val_id_list.entries(); i++)
    {
	  ...
	  attr[0] = generator->getAttr(item_expr);
          if (val_id.getType().getFSDatatype() == REC_CLOB)
            {
              if (val_id.getType().getTypeQualifier() == NA_ROWSET_TYPE)
                {
                  const NAType &rowsetType = val_id.getType();
                  const  SQLClob *elType = (SQLClob *)((const SQLRowset *)&rowsetType)->getElementType();
                  ((SimpleType *)attr[0])->setCharSet(elType->getDataCharSet());
                }
              else
                {
                  SQLClob lobType = (SQLClob &)val_id.getType();
                  ((SimpleType *)attr[0])->setCharSet(lobType.getDataCharSet());
                }
            }
	  if (getShowplan())
	    attr[1] =
          ...
          if (isDynamicParam)
            {
              ...
              input_clause->lobInlinedDataMaxLen() = 
                param->lobInlinedDataMaxLen() / bpc;
                                     ...
                                     ExpHdrInfo * hdrInfo)
{
  ...
  if (returnedAttrs)
  ...
  else
    {
      ...
      // pointed to by the array entries since we didn't allocate them.
      if ( !tupleDesc || (*tupleDesc)->attrs() != (AttributesPtr*)attrs )
         NADELETEBASIC( attrs, wHeap());
    }
...
char * ExpGenerator::placeConstants(AListNode *list, Int32 length)
{
  ...
  MapTable *startMapTable =
    (forceLinkConstant() ? getExprMapTable() : NULL);
  ...
  while (current->getNext())
	...
	(((ExprNode *)(current->getElement()))->castToItemExpr());
      ...
      if (! generator->getMapInfoAsIs(item->getValueId(), startMapTable))
        {
          *CmpCommon::diags() << DgSqlCode(-7007) 
                              << DgString0("Detail: ValueId must exist in map table");
          GenExit();
        }

      Attributes * attr
	= generator->getMapInfo(item->getValueId(), startMapTable)->getAttr();
