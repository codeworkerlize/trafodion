    ...
    case ITM_RANGE_VALUES_UNPACK:
      {
        ...
        Int32 maxEntries = (getOperatorType() == ITM_RANGE_VALUES_INSERT) ? 
                               ActiveSchemaDB()->getDefaults().
                                  getAsLong(RANGE_OPTIMIZED_SCAN_MAX_NUM_KEYS)
                             : 
                             -1;

	clause =
	  new(generator->getSpace()) ExFunctionRangeOfValues(
                                          getOperatorType(),
					  attr, 
                                          generator->getSpace(),
                                          maxLength,
...
short ConstValue::codeGen(Generator * generator)
{
  MapInfo * map_info = generator->getMapInfoAsIs(
       getValueId(),
       (generator->getExpGenerator()->forceLinkConstant() 
        ? generator->getExpGenerator()->getExprMapTable() : NULL));
...
short ItemExpr::codeGen(Generator * generator)
{
  ...
  txt = NAString("Details: Should never reach ItemExpr::codeGen for ") + 
    getText() + ".";
...
short RangeCompare::codeGen(Generator * generator)
{
  ...
  ExFunctionRangeOfValues* rv_in_clause =
    new(generator->getSpace()) ExFunctionRangeOfValues(getOperatorType(), attr,
					        generator->getSpace(),
                                                maxLength, 2,
                                                getFilterId(),
                                                dumpDebugData,
