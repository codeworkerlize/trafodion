...
void collectAttributes(ItemExpr *exp, int degree, int &counter, Attributes **attr, Generator *generator) {
  ...
  for (short i = 0; i < exp->getArity(); i++) {
    if (exp->child(i)->getOperatorType() != ITM_ITEM_LIST) {
      ++counter;
      attr[counter] = generator->getMapInfo(exp->child(i)->castToItemExpr()->getValueId())->getAttr();
    } else
      collectAttributes(exp->child(i)->castToItemExpr(), degree, counter, attr, generator);
  }
...
short Aggregate::codeGen(Generator *generator) {
  ...
  if (aggrMapInfo && aggrMapInfo->isCodeGenerated()) return 0;
  ...
  switch (getOperatorType()) {
    case ITM_ONE_ROW: {
      ...
      clause = new (generator->getSpace())
          ex_aggr_one_row_clause(getOperatorType(), (short)(1 + degree), attr, generator->getSpace());
    } break;

    case ITM_ANY_TRUE_MAX: {
      clause = new (generator->getSpace())
          ex_aggr_any_true_max_clause(getOperatorType(), (short)(1 + getArity()), attr, generator->getSpace());
    } break;
    ...
    case ITM_RANGE_VALUES_UNPACK: {
      const NAType &operand = getValueId().getType();
      ...
      clause =
          new (generator->getSpace()) ExFunctionRangeOfValues(getOperatorType(), attr, generator->getSpace(), maxLength,
                                                              ...
                                                              getFilterId(),  // filter Id
                                                              dumpDebugData,
                                                              TRUE,  // dateAsTimeStamp
                                                              nativeByteOrder, maxEntries);
    } break;
...
short Assign::codeGen(Generator *generator) {
  ...
  if (assignMapInfo && assignMapInfo->isCodeGenerated()) return 0;
  ...
  //
  MapInfo *leftChildMapInfo = generator->getMapInfoAsIs(child(0)->castToItemExpr()->getValueId());
  if (leftChildMapInfo) {
    if (!assignMapInfo)
      assignMapInfo =
          generator->addMapInfoToThis(generator->getLastMapTable(), getValueId(), leftChildMapInfo->getAttr());
  ...
  //
    ...
    generator->addMapInfoToThis(generator->getLastMapTable(), child(0)->castToItemExpr()->getValueId(), attr[0]);
  }
  ...
  ex_conv_clause *conv_clause =
      new (generator->getSpace()) ex_conv_clause(getOperatorType(), attr, generator->getSpace());
...
short BaseColumn::codeGen(Generator *generator) {
  ...
  if ((!mapInfo || !mapInfo->isOffsetAssigned()) && (!getEIC().isEmpty())) {
    MapInfo *index_col_map_info;
    short done = 0;
    for (ValueId val_id = getEIC().init(); !done && getEIC().next(val_id); getEIC().advance(val_id)) {
      if (index_col_map_info = generator->getMapInfoAsIs(val_id)) {
        Attributes *attr = generator->addMapInfo(getValueId(), 0)->getAttr();
...
short BiArith::codeGen(Generator *generator) {
  ...
  if (isStandardNormalization()) attr[0]->setlastdayonerrflag();
  if (isKeepLastDay()) attr[0]->setlastdaymonthflag();

  ex_arith_clause *arith_clause = new (generator->getSpace())
      ex_arith_clause(getOperatorType(), attr, generator->getSpace(), (short)getRoundingMode(), getDivToDownscale());
...
short BiArithSum::codeGen(Generator *generator) {
  ...
  ex_arith_sum_clause *arith_clause =
      new (generator->getSpace()) ex_arith_sum_clause(getOperatorType(), attr, generator->getSpace());
  if (eg->inSequenceFuncExpr()) arith_clause->setAugmentedAssignOperation(FALSE);
...
short BiArithCount::codeGen(Generator *generator) {
  ...
  ex_arith_count_clause *arith_clause =
      new (generator->getSpace()) ex_arith_count_clause(getOperatorType(), attr, generator->getSpace());
...
short UnArith::codeGen(Generator *generator) {
  ...
  ex_arith_clause *arith_clause =
      new (generator->getSpace()) ex_arith_clause(getOperatorType(), attr, generator->getSpace(), 0, FALSE);
...
void markGeneratedEntries(Generator *generator, ItemExpr *item, ValueIdSet &marks) {
  if (item) {
    MapInfo *mapInfo = generator->getMapInfoAsIs(item->getValueId());
...
void generateMarkedEntries(Generator *generator, ValueIdSet &marks) {
  for (ValueId vid = marks.init(); marks.next(vid); marks.advance(vid)) {
    MapInfo *mapInfo = generator->getMapInfoAsIs(vid);
    if (mapInfo) mapInfo->codeGenerated();
  }
}

void unGenerate(Generator *generator, ItemExpr *item) {
  if (item) {
    MapInfo *mapInfo = generator->getMapInfoAsIs(item->getValueId());
...
short BiLogic::codeGen(Generator *generator) {
  ...
  if (generator->getExpGenerator()->genItemExpr(this, &attr, (1 + getArity()), 0) == 1) return 0;
  ...
  ex_branch_clause *branch_clause = new (space) ex_branch_clause(getOperatorType(), branch_attr, space);
  ...
  attr[2] = generator->getMapInfo(rightMost->getValueId())->getAttr();

  ex_bool_clause *bool_clause = new (space) ex_bool_clause(getOperatorType(), attr, space);
...
short BiRelat::codeGen(Generator *generator) {
  if (child(0)->getOperatorType() == ITM_ITEM_LIST) {
    ...
    ex_comp_clause *comp_clause =
        new (generator->getSpace()) ex_comp_clause(getOperatorType(), attr, generator->getSpace(), getSpecialNulls());

    comp_clause->setCollationEncodeComp(getCollationEncodeComp());

    if (rollupColumnNum() >= 0) comp_clause->setRollupColumnNum(rollupColumnNum());
...
short RangeSpecRef::codeGen(Generator *generator) {
  ...
  attr[0]->copyLocationAttrs(generator->getMapInfo(child(1)->getValueId())->getAttr());
...
short ConstValue::codeGen(Generator *generator) {
  MapInfo *map_info = generator->getMapInfoAsIs(
      getValueId(),
      (generator->getExpGenerator()->forceLinkConstant() ? generator->getExpGenerator()->getExprMapTable() : NULL));

  // force const code gen even if it is found in an existing map table.
  // this is needed if expression code is generated within another
  // expression, for composite expressions.
  //  if (map_info && generator->getExpGenerator()->forceLinkConstant())
  //    map_info = NULL;

  if (map_info && map_info->isCodeGenerated()) return 0;
  ...
  if (!map_info) {
    ...
    map_info = generator->addMapInfoToThis(generator->getExpGenerator()->getExprMapTable(), getValueId(), NULL);
...
short IndexColumn::codeGen(Generator * /*generator*/) { return 0; }

short ItemExpr::codeGen(Generator *generator) {
  if (getOperatorType() == ITM_NATYPE || getOperatorType() == ITM_NAMED_TYPE_TO_ITEM) {
    Attributes **attr;
  ...
  txt = NAString("Details: Should never reach ItemExpr::codeGen for ") + getText() + ".";
  *CmpCommon::diags() << DgSqlCode(-7007) << DgString0(txt);
  GenExit();
...
short UnLogic::codeGen(Generator *generator) {
  ...
  ex_unlogic_clause *unlogic_clause =
      new (generator->getSpace()) ex_unlogic_clause(getOperatorType(), attr, generator->getSpace());
...
short ValueIdRef::codeGen(Generator *generator) {
  ...
  if (!map_info) {
    /* this value has not been added to the map table. Add it.*/
    map_info = generator->addMapInfo(getValueId(), generator->getMapInfo(isDerivedFrom())->getAttr());
  }

  return 0;
}

short ValueIdUnion::codeGen(Generator * /*generator*/) { return 0; }

short RangeCompare::codeGen(Generator *generator) {
  Attributes **attr;
  ...
  CMPASSERT(operand1.getTypeQualifier() == NA_CHARACTER_TYPE);
  ...
  ExFunctionRangeOfValues *rv_in_clause = new (generator->getSpace()) ExFunctionRangeOfValues(
      getOperatorType(), attr, generator->getSpace(), maxLength, 2, getFilterId(), dumpDebugData,
      TRUE,  // dateAsTimeStamp
      nativeByteOrder);

  generator->getExpGenerator()->linkClause(this, rv_in_clause);
