				    ...
				    Generator *generator)
{
  ...
  if (getTableDesc()->getNATable()->isSeabaseTable())
    {
      if (((ComTdbHbaseAccess *)tdb)->useEncryption())
        description += "encryption: used ";
    }
					      ...
					      Generator *generator)
{
 ...
 if (isHashJoin())
 {
     ...
     if(hj->getMinMaxHostvars().entries()) 
       {
         ...
         ExprNode *minMaxHostvars  = 
           hj->getMinMaxHostvars().rebuildExprTree(ITM_ITEM_LIST);
     ...
     if (((ComTdbHashj *)tdb)->beforePredOnOuterOnly())
...
NABoolean displayDuringRegressRun(DefaultConstants attr)
{
  if ((attr == TRAF_READ_OBJECT_DESC) ||
      ...
      (attr == TRAF_USE_STORED_STATS) ||
      (attr == TRAF_USTAT_GENERATE_STORED_STATS) ||
      (attr == TRAF_ENABLE_METADATA_LOAD_IN_CACHE) ||
  ...
  if (natable->isPartitionNameSpecified())
    {
      ...
      if (nmapentry && nmapentry->getGivenName())
        description += nmapentry->getGivenName();
  ...
  if (natable->isSeabaseTable())
    {
      ...
      if (natable->incrBackupEnabled())
        description += "incremental_backup: enabled ";
    }
