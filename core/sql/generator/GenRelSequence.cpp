...
void PhysSequence::processHistoryAttributesForLead(
                          ...
                          CollHeap *wHeap)
{
  ...
  for(ValueId valId = leadFunctions.init();
      leadFunctions.next(valId);
      leadFunctions.advance(valId)) 
  {
     ItemExpr *itmExpr = valId.getItemExpr();
     ...
     { 
        ...
        ItemExpr *castExpr = 
                   new (wHeap) Cast (lead->child(1),
                                     new (wHeap)
                                       SQLInt(wHeap, TRUE, 
                                         cType.supportsSQLnullLogical()));
     ...
     { 
        const NAType &cType = lead->child(0)->getValueId().getType();
          ...
          // (must be) signed; nulls allowed (if allowed by child1)   
        ItemExpr *castExpr  = new (wHeap) Cast (lead->child(2), 
                                                cType.newCopy(wHeap));
        castExpr->synthTypeAndValueId(TRUE);
...
PhysSequence::codeGen(Generator *generator) 
{
  ...
  processHistoryAttributesForLead(readSeqFunctions(), 
                                  returnSeqFunctions(),
                                  historyIds, wHeap);
  ...
  if(NOT readSeqFunctions().isEmpty())
    {
      ...
      if ( firstValue.entries() > 0 && 
           sequencedColumns().entries() == 0 )
