...
extern int CreateAllCharsExpr(const NAType &formalType, ItemExpr &actualValue, CmpContext *cmpContext,
                              ItemExpr *&newExpr);

inline static NABoolean getReturnRow(const GenericUpdate *gu, const IndexDesc *index) { return gu->producesOutputs(); }
...
static DP2LockFlags initLockFlags(GenericUpdate *gu, Generator *generator) {
  ...
  generator->verifyUpdatableTransMode(&gu->accessOptions(), generator->getTransMode(), &ilForUpd);
  DP2LockFlags lf;
  ...
  // read committed.
  if (lf.getConsistencyLevel() == DP2LockFlags::STABLE) lf.setConsistencyLevel(DP2LockFlags::READ_COMMITTED);

  if ((ilForUpd != TransMode::IL_NOT_SPECIFIED_) && (NOT gu->accessOptions().userSpecified())) {
    TransMode t(ilForUpd);
    lf.setConsistencyLevel((DP2LockFlags::ConsistencyLevel)t.getDP2LockFlags().getConsistencyLevel());
    lf.setLockState((DP2LockFlags::LockState)t.getDP2LockFlags().getLockState());
  }

  return lf;
}

void GenericUpdate::setTransactionRequired(Generator *generator, NABoolean isNeededForAllFragments) {
  if (!generator->isInternalRefreshStatement() || getIndexDesc()->getNAFileSet()->isAudited()) {
    generator->setTransactionFlag(TRUE, isNeededForAllFragments);
  } else {
    // Internal refresh statement and table is non-audited.
    if (!getTableDesc()->getNATable()->isAnMV() && !getTableDesc()->getNATable()->isMonarch() &&
                        ...
                        NABoolean updOpt)                // IN
{
  ...
  ExpTupleDesc::TupleDataFormat tupleFormat = generator->getTableDataFormat(tableDesc->getNATable(), indexDesc);
  ...
  for (CollIndex i = 0; i < colEntries; i++) {
    fetchedCol = (BaseColumn *)(((indexDesc->getIndexColumns())[i]).getItemExpr());
    fetchedColNum = fetchedCol->getColNumber();

    updtCol =
        (updtCol != NULL
             ? updtCol
             : (j < recEntries ? (BaseColumn *)(recExprArray[j].getItemExpr()->child(0)->castToItemExpr()) : NULL));
    ...
    if (((updOpt) && (fetchedColNum == updtColNum)) || (NOT updOpt)) {
      if (updOpt) {
        // assign the attributes of the fetched col to the
        // updated col.
        generator->addMapInfo(castNode->getValueId(), generator->getMapInfo(fetchedCol->getValueId())->getAttr());
      }

      if (alignedFormat && (col = updtValId.getNAColumn(TRUE)) && (col != NULL)) colArray.insert(col);
  ...
  if (NOT updOpt) {
    ...
    if (tupleFormat == ExpTupleDesc::SQLMX_FORMAT || tupleFormat == ExpTupleDesc::SQLMX_ALIGNED_FORMAT)
      expGen->setForInsertUpdate(TRUE);

    expGen->generateContiguousMoveExpr(updtRowVidList,
                                       ...
                                       ExpTupleDesc::LONG_FORMAT, NULL, NULL, 0, NULL, NULL, &colArray);
  } else {
    ...
    expGen->assignAtpAndAtpIndex(updtRowVidList, 1, updatedRowAtpIndex);
    ...
    // generate the update expression
    expGen->generateListExpr(updtRowVidList, ex_expr::exp_ARITH_EXPR, updateExpr);
...
static short genUpdConstraintExpr(Generator *generator, ItemExpr *constrTree, const ValueIdSet &constraintColumns,
                                  ValueIdArray &targetRecExprArray, ex_expr **targetExpr /* out */) {
  ExpGenerator *expGen = generator->getExpGenerator();
  ...
  for (ValueId sourceValId = constraintColumns.init(); constraintColumns.next(sourceValId);
  ...
  if (sourceToTarget.entries() > 0) {
    ...
    expGen->generateExpr(mappedConstrTree, ex_expr::exp_SCAN_PRED, targetExpr);
  } else {
    targetExpr = NULL;
  }
                                     ...
                                     const TableDesc *tableDesc)             // IN
{
  ...
  *updateExpr = NULL;
  ...
  if (indexDesc->getNAFileSet()->isSqlmxAlignedRowFormat()) isAligned = TRUE;
  ...
  if (updRecExprArray.entries() > 0) listOfUpdatedColNames = new (space) Queue(space);
  ...
  for (CollIndex ii = 0; ii < updRecExprArray.entries(); ii++) {
    ...
    if (isAligned) {
      ...
      if (col != NULL) colArray.insert(col);
    }
    ...
    BaseColumn *bc = (BaseColumn *)(updRecExprArray[ii].getItemExpr()->child(0)->castToItemExpr());

    GenAssert(bc->getOperatorType() == ITM_BASECOLUMN, "unexpected type of base table column");

    const NAColumn *nac = bc->getNAColumn();
    if ((NOT isAligned) && HbaseAccess::isEncodingNeededForSerialization(bc)) {
      ie = new (generator->wHeap()) CompEncode(ie, FALSE, -1, CollationInfo::Sort, TRUE);
    }
  ...
  if ((isAligned) && (listOfUpdatedColNames) && (updRecExprArray.entries() > 0)) {
  ...
  // Generate the update expression
  //
  expGen->generateContiguousMoveExpr(updRowVidList,
                                     ...
                                     ExpTupleDesc::LONG_FORMAT, NULL, NULL, 0, NULL, NULL,
  ...
  if ((isInsert) && (updRowVidList.entries() > 0)) {
    ...
    expGen->generateKeyEncodeExpr(indexDesc,
                                  1,  // (IN) Destination Atp
                                  mergeInsertRowIdTuppIndex, ExpTupleDesc::SQLMX_KEY_FORMAT, mergeInsertRowIdLen,
                                  mergeInsertRowIdExpr, FALSE, firstKeyColumnOffset, &updRowKeyVidList, TRUE);
  }
...
//
static void bindVPCols(Generator *generator, const ValueIdList &vpCols, ValueIdList &resList) {
  ...
  for (CollIndex colNo = 0; colNo < vpCols.entries(); colNo++) {
    ...
    GenAssert(vpColItem->getOperatorType() == ITM_INDEXCOLUMN, "unexpected type of vp column");

    // Get the corresponding base table column -- must be ITM_BASECOLUMN
    ItemExpr *tblColItem = vpColItem->getDefinition().getItemExpr();
    GenAssert(tblColItem->getOperatorType() == ITM_BASECOLUMN, "unexpected type of base table column");
...
short HiveInsert::codeGen(Generator *generator) { return 0; }
...
static void orderColumnsByAlignment(NAArray<BaseColumn *> columns, UInt32 numColumns,
  ...
  if (align4.entries() > 0)
    for (i = 0; i < align4.entries(); i++) orderedCols->insertAt(k++, align4[i]);

  if (align2.entries() > 0)
    for (i = 0; i < align2.entries(); i++) orderedCols->insertAt(k++, align2[i]);

  if (align1.entries() > 0)
    for (i = 0; i < align1.entries(); i++) orderedCols->insertAt(k++, align1[i]);

  if (addedCols.entries() > 0)
    for (i = 0; i < addedCols.entries(); i++) orderedCols->insertAt(k++, addedCols[i]);

  if (varCols.entries() > 0)
    for (i = 0; i < varCols.entries(); i++) orderedCols->insertAt(k++, varCols[i]);
}

short Delete::codeGen(Generator * /*generator*/) { return -1; }

short Insert::codeGen(Generator * /*generator*/) { return -1; }

short Update::codeGen(Generator * /*generator*/) { return -1; }

short MergeUpdate::codeGen(Generator * /*generator*/) { return -1; }

short MergeDelete::codeGen(Generator * /*generator*/) { return -1; }
...
short HbaseDelete::codeGen(Generator *generator) {
  ...
  ExpTupleDesc::TupleDataFormat hbaseRowFormat = ExpTupleDesc::SQLARK_EXPLODED_FORMAT;
  ...
  HbaseAccess::sortValues(
      retColRefSet_, columnList, srcVIDlist, dupVIDlist,
  ...
  if (!getPartQualPreCond().isEmpty()) {
    ItemExpr *partQualPreCondTree = getPartQualPreCond().rebuildExprTree(ITM_AND, TRUE, TRUE);
    expGen->generateExpr(partQualPreCondTree->getValueId(), ex_expr::exp_SCAN_PRED, &partQualPreCondExpr);
  }
  ...
  expGen->buildKeyInfo(&keyInfo,  // out
                       ...
                       getBeginKeyPred(), (getSearchKey() && getSearchKey()->isUnique() ? NULL : getEndKeyPred()),
                       getSearchKey(),
                       NULL,  // getMdamKeyPtr(),
                       FALSE, ExpTupleDesc::SQLMX_KEY_FORMAT);

  UInt32 keyColValLen = 0;
  char *keyColName = NULL;
  if ((canDoCheckAndUpdel()) && (getSearchKey() && getSearchKey()->isUnique()) && (getBeginKeyPred().entries() > 0)) {
    expGen->generateKeyColValueExpr(getBeginKeyPred()[0], work_atp, keyColValTuppIndex, keyColValLen, &keyColValExpr);
  ...
  for (CollIndex ii = 0; ii < numColumns; ii++) {
    ...
    res = HbaseAccess::createAsciiColAndCastExpr2(generator,  // for heap
                                                  col_node,
  ...
  {
    expGen->addDefaultValues(columnList, getIndexDesc()->getAllColumns(), tuple_desc, TRUE);
  ...
  if (getTableDesc()->getNATable()->isSeabaseTable()) {
    ...
    HbaseAccess::genRowIdExpr(generator, getIndexDesc()->getNAFileSet()->getIndexKeyColumns(), getHbaseSearchKeys(),
                              ...
                              rowIdAsciiTupleDesc, rowIdLength, rowIdExpr, encodeKeys);
  } else {
    HbaseAccess::genRowIdExprForNonSQ(generator, getIndexDesc()->getNAFileSet()->getIndexKeyColumns(),
                                      getHbaseSearchKeys(), work_cri_desc, work_atp, rowIdAsciiTuppIndex,
                                      rowIdTuppIndex, rowIdAsciiRowLen, rowIdAsciiTupleDesc, rowIdLength, rowIdExpr);
  }
  ...
  if (isAlignedFormat) {
    ...
    HbaseAccess::genListOfColNames(generator, getIndexDesc(), columnList, listOfFetchedColNames);
  }
  ...
  if (csl()) {
    ...
    for (int i = 0; i < csl()->entries(); i++) {
      ...
      colNameInList = space->AllocateAndCopyToAlignedSpace(*nas, 0);
  ...
  if (getTableDesc()->getNATable()->isSeabaseTable()) {
    if ((keyInfo && getSearchKey() && getSearchKey()->isUnique()) || (tdbListOfUniqueRows)) {
  ...
  if (returnRow) {
    ...
    expGen->assignAtpAndAtpIndex(getIndexDesc()->getIndexColumns(), 0, returnedDesc->noTuples() - 1);

    expGen->assignAtpAndAtpIndex(getScanIndexDesc()->getIndexColumns(), 0, returnedDesc->noTuples() - 1);
  }
  ...
  if (FileScan::genTableName(generator, space, getTableDesc()->getCorrNameObj(), getTableDesc()->getNATable(),
  ...
  if (CmpCommon::getDefault(HBASE_CACHE_BLOCKS) != DF_OFF) hbpa->setCacheBlocks(TRUE);
  ...
  ComTdbHbaseAccess *hbasescan_tdb = new (space) ComTdbHbaseAccess(
      ComTdbHbaseAccess::DELETE_, tablename, baseTableName,
      ...
      rowIdLength, convertRowLen, rowIdAsciiRowLen, (keyInfo ? keyInfo->getKeyLength() : 0), keyColValLen,
      ...
      rowIdTuppIndex, returnedDesc->noTuples() - 1, rowIdAsciiTuppIndex,
      0,  // keyTuppIndex,
      keyColValTuppIndex,
      ...
      tdbListOfRangeRows, tdbListOfUniqueRows, listOfFetchedColNames, listOfDeletedColNames, NULL,
  ...
  if (USE_UUID_AS_HBASE_TABLENAME) {
    ...
    if (FileScan::genTableName(generator, space, getTableName(), getTableDesc()->getNATable(),
                               getIndexDesc()->getNAFileSet(), FALSE, tablenameForUID, TRUE)) {
      GenAssert(0, "genTableName failed");
    }
  ...
  if ((CmpCommon::getDefault(HBASE_ASYNC_OPERATIONS) == DF_ALL) && getInliningInfo().isIMGU())
  ...
  if (getTableDesc()->getNATable()->isSeabaseTable()) {
    ...
    if (getUpdateCKorUniqueIndexKey()) hbasescan_tdb->setUpdateKey(TRUE);
    ...
    if ((CmpCommon::getDefault(HBASE_SQL_IUD_SEMANTICS) == DF_ON) && (NOT noCheck()))
    ...
    if (getTableDesc()->getNATable()->xnRepl() == COM_REPL_ASYNC) hbasescan_tdb->setReplAsync(TRUE);
  ...
  if (keyInfo && getSearchKey() && getSearchKey()->isUnique()) hbasescan_tdb->setUniqueKeyInfo(TRUE);

  if (returnRow) hbasescan_tdb->setReturnRow(TRUE);

  if (rowsAffected() != GenericUpdate::DO_NOT_COMPUTE_ROWSAFFECTED) hbasescan_tdb->setComputeRowsAffected(TRUE);

  if (!tdbListOfUniqueRows) {
    hbasescan_tdb->setSubsetOper(TRUE);
  }

  if (canDoCheckAndUpdel()) hbasescan_tdb->setCanDoCheckAndUpdel(TRUE);

  if (canIMDeleteDirect()) hbasescan_tdb->setIMDeleteDirect(TRUE);
  ...
  if (csl()) hbasescan_tdb->setUpdelColnameIsStr(TRUE);

  if (partQualPreCondExpr) hbasescan_tdb->setPartQualPreCondExpr(partQualPreCondExpr);

  if (preCondExpr) hbasescan_tdb->setInsDelPreCondExpr(preCondExpr);
...
short HbaseUpdate::codeGen(Generator *generator) {
  ...
  if (getProducedMergeIUDIndicator() != NULL_VALUE_ID) {
    ...
    iudIndicatorTupleDesc = new (generator->getSpace())
        ExpTupleDesc(1,                  // numAttrs
                     ...
                     ExpTupleDesc::SQLARK_EXPLODED_FORMAT, ExpTupleDesc::LONG_FORMAT, generator->getSpace());
    work_cri_desc->setTupleDescriptor(mergeIUDIndicatorTuppIndex, iudIndicatorTupleDesc);
  }
  ...
  NABoolean isHbaseMapFormat = getTableDesc()->getNATable()->isHbaseMapTable();
  NABoolean isTrafMDTable = getTableDesc()->getNATable()->isSeabaseMDTable();
  GenAssert(NOT isHbaseMapFormat, "HBase mapped table update should have been transformed to delete + insert.");
  ...
  if ((isMerge()) && (mergeInsertRecExprArray().entries() > 0)) {
    genHbaseUpdOrInsertExpr(generator, TRUE, mergeInsertRecExprArray(), mergeInsertTuppIndex, &mergeInsertExpr,
                            mergeInsertRowLen, &mergedRowTupleDesc, listOfMergedColNames, &mergeInsertRowIdExpr,
                            mergeInsertRowIdLen, mergeInsertRowIdTuppIndex, getIndexDesc(), getTableDesc());
  ...
  if (isMerge() && !mergeUpdatePred().isEmpty()) {
    ...
    // fetched row
    ItemExpr *updPredTree = mergeUpdatePred().rebuildExprTree(ITM_AND, TRUE, TRUE);
    expGen->generateExpr(updPredTree->getValueId(), ex_expr::exp_SCAN_PRED, &mergeUpdScanExpr);
    ...
    ItemExpr *constrTree = getCheckConstraints().rebuildExprTree(ITM_AND, TRUE, TRUE);
    ...
    genUpdConstraintExpr(generator, constrTree, constraintColumns, newRecExprArray(), &updConstraintExpr /* out */);

    if ((isMerge()) && (mergeInsertRecExprArray().entries() > 0)) {
      // Generate the insert constraint expression

      genUpdConstraintExpr(generator, constrTree, constraintColumns, mergeInsertRecExprArray(),
  ...
  if ((getTableDesc()->getNATable()->isSeabaseTable()) && (NOT isMerge())) {
    if ((keyInfo && getSearchKey() && getSearchKey()->isUnique()) || (tdbListOfUniqueRows)) {
      // Save node for later use by RelRoot in the case of UPDATE CURRENT OF.
      generator->updateCurrentOfRel() = (void *)this;
    }
  ...
  if (returnRow) {
    const ValueIdList &fetchedOutputs =
        ((getScanIndexDesc() != NULL) ? getScanIndexDesc()->getIndexColumns() : getIndexDesc()->getIndexColumns());
    ...
    expGen->generateContiguousMoveExpr(fetchedOutputs,
                                       1,  // add conv nodes
                                       0, returnedFetchedTuppIndex, generator->getInternalFormat(),
                                       returnedFetchedRowLen, &projExpr, &returnedFetchedTupleDesc,
                                       ExpTupleDesc::SHORT_FORMAT, &returnedFetchedMapTable);
    ...
    expGen->assignAtpAndAtpIndex(getScanIndexDesc()->getIndexColumns(), 0, returnedFetchedTuppIndex);
    expGen->assignAtpAndAtpIndex(getIndexDesc()->getIndexColumns(), 0, returnedFetchedTuppIndex);
    ...
    if (isMerge()) {
      ...
      for (CollIndex ii = 0; ii < colEntries; ii++) {
        fetchedCol = (BaseColumn *)(((getIndexDesc()->getIndexColumns())[ii]).getItemExpr());
        fetchedColNum = fetchedCol->getColNumber();

        updtCol =
            (updtCol != NULL
                 ? updtCol
                 : (j < recEntries ? (BaseColumn *)(newRecExprArray()[j].getItemExpr()->child(0)->castToItemExpr())
                                   : NULL));
      ...
      if (getProducedMergeIUDIndicator() != NULL_VALUE_ID) updatedOutputs.insert(getProducedMergeIUDIndicator());
    } else {
    ...
    if (getTableDesc()->getNATable()->hasSerializedEncodedColumn()) {
      ...
      expGen->generateDeserializedMoveExpr(updatedOutputs, 0,
                                           returnedUpdatedTuppIndex,  // projRowTuppIndex,
                                           generator->getInternalFormat(), returnedUpdatedRowLen, &returnUpdateExpr,
                                           &returnedUpdatedTupleDesc, ExpTupleDesc::SHORT_FORMAT, tgtConvValueIdList,
                                           alreadyDeserialized);
    } else {
      expGen->generateContiguousMoveExpr(updatedOutputs,
                                         1,  // add conv nodes
                                         0, returnedUpdatedTuppIndex, generator->getInternalFormat(),
                                         returnedUpdatedRowLen, &returnUpdateExpr, &returnedUpdatedTupleDesc,
                                         ExpTupleDesc::SHORT_FORMAT, &returnedUpdatedMapTable, &tgtConvValueIdList);
    }
    ...
    for (CollIndex ii = 0; ii < tgtConvValueIdList.entries(); ii++) {
      ...
      if (updatedOutputs[ii] == getProducedMergeIUDIndicator()) {
        tgtIUDMergeColConvAttr = (generator->getMapInfo(tgtColConvValueId, 0))->getAttr();
        continue;
      }
    ...
    //
    returnedDesc->setTupleDescriptor(returnedFetchedTuppIndex, returnedFetchedTupleDesc);
    returnedDesc->setTupleDescriptor(returnedUpdatedTuppIndex, returnedUpdatedTupleDesc);
    ...
    if (isMerge()) {
      ...
      for (CollIndex ii = 0; ii < mergeInsertRecExprArray().entries(); ii++) {
        ...
        if ((NOT isAlignedFormat) &&
            ...
            (((Assign *)assignExpr)->canBeSkipped()) && (NOT col->isSystemColumn()) && (NOT col->isClusteringKey()) &&
            (NOT col->isIdentityColumn())) {
          if (listOfOmittedColNames == NULL) listOfOmittedColNames = new (space) Queue(space);
          NAString cnInList;
      ...
      if (getProducedMergeIUDIndicator() != NULL_VALUE_ID) mergeInsertOutputs.insert(getProducedMergeIUDIndicator());
      ...
      if (getTableDesc()->getNATable()->hasSerializedEncodedColumn()) {
        // if serialized columns are present, then create a new row with
        // deserialized columns before returning it.
        expGen->generateDeserializedMoveExpr(mergeInsertOutputs, 0, returnedUpdatedTuppIndex,
                                             generator->getInternalFormat(), returnedMergeInsertedRowLen,
                                             &returnMergeInsertExpr, &returnedMergeInsertedTupleDesc,
                                             ExpTupleDesc::SHORT_FORMAT, tgtConvValueIdList, alreadyDeserialized);
      } else {
        expGen->generateContiguousMoveExpr(mergeInsertOutputs,
                                           1,  // add conv nodes
                                           0, returnedUpdatedTuppIndex, generator->getInternalFormat(),
                                           returnedMergeInsertedRowLen, &returnMergeInsertExpr,
                                           &returnedMergeInsertedTupleDesc, ExpTupleDesc::SHORT_FORMAT,
                                           &returnedMergeInsertedMapTable, &tgtConvValueIdList);
      }
      if (getProducedMergeIUDIndicator() != NULL_VALUE_ID) iudIndicatorAttr->copyLocationAttrs(tgtIUDMergeColConvAttr);
    }
  }
  ...
  // create hdfsscan_tdb
  ComTdbHbaseAccess *hbasescan_tdb = new (space) ComTdbHbaseAccess(
      (isMerge() ? ComTdbHbaseAccess::MERGE_ : ComTdbHbaseAccess::UPDATE_), tablename, baseTableName,

      convert_expr, scanExpr, rowIdExpr, updateExpr, mergeInsertExpr, mergeInsertRowIdExpr, mergeUpdScanExpr, projExpr,
      returnUpdateExpr, returnMergeInsertExpr,
      NULL,  // encodedKeyExpr
      keyColValExpr,
      NULL,  // hbaseFilterValExpr
      hbTagExpr,

      asciiRowLen, convertRowLen, updateRowLen, mergeInsertRowLen, returnedFetchedRowLen, returnedUpdatedRowLen,

      ((rowIdLength > 0) ? rowIdLength : mergeInsertRowIdLen), convertRowLen, rowIdAsciiRowLen,
      (keyInfo ? keyInfo->getKeyLength() : 0), keyColValLen,
      0,  // hbaseFilterValRowLen
      hbTagRowLen,

      asciiTuppIndex, convertTuppIndex, (updateExpr ? updateTuppIndex : 0), mergeInsertTuppIndex,
      mergeInsertRowIdTuppIndex, mergeIUDIndicatorTuppIndex, returnedFetchedTuppIndex, returnedUpdatedTuppIndex,
      ...
      tdbListOfRangeRows, tdbListOfUniqueRows, listOfFetchedColNames, listOfUpdatedColNames, listOfMergedColNames,
  ...
  if (getTableDesc()->getNATable()->isHbaseRowTable()) hbasescan_tdb->setRowwiseFormat(TRUE);

  if (updConstraintExpr) hbasescan_tdb->setUpdConstraintExpr(updConstraintExpr);

  if (insConstraintExpr) hbasescan_tdb->setInsConstraintExpr(insConstraintExpr);
  ...
  if (getTableDesc()->getNATable()->isSeabaseTable()) {
    ...
    if (CmpCommon::getDefault(HBASE_SQL_IUD_SEMANTICS) == DF_ON) hbasescan_tdb->setHbaseSqlIUD(TRUE);
...
short HbaseInsert::codeGen(Generator *generator) {
  ...
  NABoolean returnRow = getReturnRow(this, getIndexDesc());
  if (getIsTrafLoadPrep() || (isUpsert() && inlinedActions)) returnRow = isReturnRow();
  ...
  for (CollIndex ii = 0; ii < newRecExprArray().entries(); ii++) {
    ...
    if ((NOT isAlignedFormat) &&
        ((CmpCommon::getDefault(TRAF_UPSERT_MODE) == DF_MERGE) ||
         (CmpCommon::getDefault(TRAF_UPSERT_MODE) == DF_OPTIMAL)) &&
        (((Assign *)assignExpr)->canBeSkipped()) && (col) && (NOT col->isSystemColumn()) &&
        (NOT col->isClusteringKey()) && (NOT col->isIdentityColumn())) {
      if (listOfOmittedColNames == NULL) listOfOmittedColNames = new (space) Queue(space);
      NAString cnInList;
      HbaseAccess::createHbaseColId(col, cnInList);
      ...
      if (col && col->isClusteringKey() && !isAlignedFormat && colIndexOfPK1 == -1)
        colIndexOfPK1 = (listOfOmittedColNames == NULL) ? ii : ii - listOfOmittedColNames->entries();
    }
    if (col) colArray.insert(col);

    if (col && col->isSyskeyColumn()) {
      ItemExpr *child1Expr = assignExpr->child(1);
      if (((Assign *)assignExpr)->isUserSpecified()) userSpecifySyskeyInput = TRUE;
    }

    if (returnRow) returnRowVIDList.insert(tgtValueId);

    ItemExpr *child1Expr = assignExpr->child(1);
    const NAType &givenType = tgtValueId.getType();

    ItemExpr *ie = new (generator->wHeap()) Cast(child1Expr, &givenType);
    ...
    if ((NOT isAlignedFormat) &&
        HbaseAccess::isEncodingNeededForSerialization(assignExpr->child(0)->castToItemExpr())) {
      ie = new (generator->wHeap()) CompEncode(ie, FALSE, -1, CollationInfo::Sort, TRUE);
    }
  ...
  if ((!isTrafMDTable) && (!allowIudForHbaseReplicaTables) && (((NATable *)naTable)->getNumReplications() > 1)) {
    *CmpCommon::diags() << DgSqlCode(-7008) << DgTableName(naTable->getTableName().getQualifiedNameAsAnsiString());
    GenExit();
    return -1;
  }
  ...
  expGen->generateContiguousMoveExpr(insertVIDList,
                                     0,  // dont add convert nodes
                                     1,  // work atp
                                     insertTuppIndex, tupleFormat, insertRowLen, &insertExpr, &tupleDesc,
                                     ExpTupleDesc::LONG_FORMAT, NULL, NULL, 0, NULL, NULL, &colArray);
  ...
  if (CmpCommon::getDefault(TRAF_LOAD_LOG_ERROR_ROWS) == DF_ON) {
    ...
    for (CollIndex ii = 0; ii < newRecExprArray().entries(); ii++) {
      ...
      if (nCount > 0 && !bFromHiveTable) {  // find whether column ItemExpr is come from hive table
    ...
    if (bFromHiveTable) {
      for (CollIndex i = 0; i < insertVIDList.entries(); i++) {
        ...
        GenAssert(res == 0 && lmExpr != NULL, "Error building expression tree for LM child Input value");
        if (lmExpr2) {
        ...
        for (int i = 0; i < baseCols.entries(); ++i) {
          if (!loggingbaseCols.contains(baseCols[i])) {
            if (loggingbaseCols.entries() == 0)
              ...
              if (!bInserted) loggingbaseCols.addMember(baseCols[i]);
            }
          }
        }
      }
      ...
      if (loggingbaseCols.entries() > 0) {
        for (int i = 0; i < loggingbaseCols.entries(); ++i) {
          ...
          GenAssert(res == 0 && loggingbaseCols[i] != NULL, "Error building expression tree for LM child Input value");
          if (lmExpr2) {
            lmExpr2->bindNode(generator->getBindWA());
            loggingDataVids.insert(lmExpr2->getValueId());
    ...
    if (loggingDataVids.entries() == 0) {  // If there is no column in insertlist, record the const vale
      for (CollIndex i = 0; i < newRecExprArray().entries(); i++) {
        ...
        if (pExpr->getOperatorType() == ITM_BASECOLUMN) {
          if (((BaseColumn *)pExpr)->getNAColumn()->isSyskeyColumn()) continue;
        }
        ItemExpr *lmExpr = assignExpr->child(1);
        if (lmExpr) {
    ...
    if (loggingDataVids.entries() > 0) {
      expGen->generateContiguousMoveExpr(loggingDataVids,   // [IN] source ValueIds
                                         FALSE,             // [IN] add convert nodes?
                                         ...
                                         // delimiter added during execution assumes exploded format
  ...
  if (CmpCommon::getDefault(TRAF_UPSERT_THROUGH_PARTITIONS) == DF_OFF && getIndexDesc()->isClusteringIndex() &&
    ...
    expGen->generateExpr(constrTree->getValueId(), ex_expr::exp_SCAN_PRED, &insConstraintExpr);
  }
  ...
  if (NOT isAlignedFormat) {
    ...
    if (getIsTrafLoadPrep()) {
      ...
      for (std::vector<NAString>::iterator it = columNamesVec.begin(); it != columNamesVec.end(); ++it) {
        ...
        char *colNameInList = space->AllocateAndCopyToAlignedSpace(cnInList2, 0);
  ...
  expGen->processValIdList(newRecExprArray(), tupleFormat, tempInsertRowLen, 0,
                           ...
                           &tempTupleDesc, ExpTupleDesc::LONG_FORMAT, 0, NULL, &colArray);
  ...
  if (returnRow) {
    if (getTableDesc()->getNATable()->hasSerializedEncodedColumn()) {
      ...
      // if serialized columns are present, then create a new row with
      // deserialized columns before returning it.
      expGen->generateDeserializedMoveExpr(returnRowVIDList,
                                           0,                   // work_atp,
                                           returnRowTuppIndex,  // projRowTuppIndex,
                                           generator->getInternalFormat(), projRowLen, &projExpr, &projRowTupleDesc,
                                           ExpTupleDesc::SHORT_FORMAT, deserColVIDList, dummy);
      ...
      expGen->assignAtpAndAtpIndex(returnRowVIDList, 0, returnRowTuppIndex);
    } else {
      expGen->processValIdList(returnRowVIDList, tupleFormat, tempInsertRowLen, 0, returnRowTuppIndex);
    }
  }
  ...
  if (getIndexDesc()->getNAFileSet()->isKeySequenced()) {
    ...
    if ((column_array[0]->isSyskeyColumn()) && (column_array[0]->getType()->getNominalSize() >= 4)) {
  ...
  ComTdbHbaseAccess *hbasescan_tdb =
      new (space) ComTdbHbaseAccess(t, tablename, baseTableName, insertExpr, NULL, rowIdExpr,
                                    ...
                                    NULL, listOfUpdatedColNames, NULL,

                                    NULL, NULL,

                                    workCriDesc, givenDesc, returnedDesc,

                                    downqueuelength, upqueuelength, expectedRows, numBuffers, buffersize,
  ...
  if ((dt != DF_OFF) && getInliningInfo().isIMGU()) hbasescan_tdb->setAsyncOperations(TRUE);

  // this is for insert on base table
  if ((dt == DF_ALL) && getTableDesc()->getNATable()->hasSecondaryIndexes()) hbasescan_tdb->setAsyncOperations(TRUE);

  if (preCondExpr) hbasescan_tdb->setPartQualPreCondExpr(preCondExpr);
  ...
  if (getTableDesc()->getNATable()->isSeabaseTable()) {
    ...
    if ((isUpsert()) || (noCheck())) hbasescan_tdb->setHbaseSqlIUD(FALSE);
    ...
    if (((((getInsertType() == Insert::VSBB_INSERT_USER) || isUpsert()) &&
      ...
      if (!hbasescan_tdb->hbaseSqlIUD()) setVsbbInsert(TRUE);
    }
    ...
    if (xformedEffUpsert()) generator->setEffTreeUpsert(TRUE);
    ...
    if (hbasescan_tdb->getIsTrafodionLoadPrep()) {
      ...
      const NAString &defColFam = getTableDesc()->getNATable()->defaultTrafColFam();
      if (!defColFam.isNull()) {
        char *colFam = space->allocateAlignedSpace(defColFam.length() + 1);
    ...
    if ((isUpsert()) && (withNoConflictCheck())) {
      hbasescan_tdb->setNoConflictCheck(TRUE);
    }

    if (colIndexOfPK1 >= 0 && t == ComTdbHbaseAccess::INSERT_) hbasescan_tdb->setColIndexOfPK1(colIndexOfPK1);
  } else {
    if (getTableDesc()->getNATable()->isHbaseRowTable())  // rowwiseHbaseFormat())
      hbasescan_tdb->setRowwiseFormat(TRUE);
  }
  ...
  if ((stt == ComTdbDp2Oper::KEY_SEQ_WITH_SYSKEY_) && !transformedUpdateNoDtmXn() && userSpecifySyskeyInput == FALSE)
