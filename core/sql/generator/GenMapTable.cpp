...
void MapInfo::set(const ValueId &valueId, Attributes *attr, CollHeap *heap) {
  ...
  attr_ = ExpGenerator::convertNATypeToAttributes(l_val_id.getType(), heap);

  // add valid and colName to attr so that we can get colname at running time
  attr_->setValId(valueId);
  NAString colName;

  if (operatorType == ITM_CONSTANT) {
    colName = ((ConstValue *)iePtr)->getAssignColName();
  } else if (operatorType == ITM_CACHE_PARAM || operatorType == ITM_DYN_PARAM) {
    colName = ((Parameter *)iePtr)->getAssignColName();
  } else if (operatorType == ITM_TRANSLATE) {
    colName = ((Translate *)iePtr)->getAssignColName();
  } else if (operatorType == ITM_INDEXCOLUMN)  // use for sub query insert into t1 select * from t2
  {
    colName = ((IndexColumn *)iePtr)->getAssignColName();
  }

  if (colName) {
    attr_->setColName(colName.data());
  }

  // remember the address(buffer location at runtime)
  // attributes for this value id, if attr is passed in.
  if (attr) attr_->copyLocationAttrs(attr);
};
...
MapInfo *MapTable::addMapInfoToThis(const ValueId &value_id, Attributes *attr) {
  ...
  if (whichMap >= vidBitMapArraySize_) {
    ...
    short *newVidsInBitMapArray = (short *)&buffer[newBitMapArraySize * sizeof(MTBitmapUnit)];

    if (vidBitMapArraySize_ > 0) {
      // Copy the existing info to the new location.
      memcpy(newVidBitMapArray, vidBitMapArray_, vidBitMapArraySize_ * sizeof(MTBitmapUnit));
      memcpy(newVidsInBitMapArray, vidsInBitMapArray_, vidBitMapArraySize_ * sizeof(short));
  ...
  if (totalVids_ >= mapInfoPtrArraySize_) {
    ...
    if (mapInfoPtrArraySize_ > 0) {
      // Copy the existing info to the new location.
      memcpy(newMapInfoPtrArray, mapInfoPtrArray_, mapInfoPtrArraySize_ * sizeof(MapInfo *));
...
#define moveXMapInfoPtrsAtATime 8
    ...
    while (mapInfosLeftToMove > 0) {
      ...
      memcpy(&mapInfoPtrArray_[end + 1 - ptrsToMove], tmpMapInfoPtrArray, bytesToMove);
...
void MapTable::print() { printToFile(stdout); }
void MapTable::printToFile(FILE *f) {
  ...
  for (Int32 i = 0; i < totalVids_; i++) {
    ...
    if (attr) {
      ...
      fprintf(f, ", %s\n", (attr->getNullFlag() ? "Nullable" : "Not nullable"));
    }
  }
...
void MapTable::display(char *title) {
  if (title) cout << title << ", ";
  ...
  for (Int32 i = 0; i < totalVids_; i++) {
    ...
    cout << info->getValueId();
    cout << "(" << unparsed.data() << ")";

    if (i < totalVids_ - 1) cout << ", ";
  }
  cout << endl;
}
