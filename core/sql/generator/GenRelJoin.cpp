  ...
  if(getMinMaxHostvars().entries())
    {
      ...
      //  description.
      expGen->processValIdList(getMinMaxHostvars(),
...
//generator->displayMapTables(900000, "hj: before call child(0)::codeGen");
  ...
  if(getMinMaxHostvars().entries()) {
    ...
    for (CollIndex mmCol = 0; mmCol < getMinMaxCols().entries(); mmCol++) {
      ...
      if ( generator->isMinmaxOptWithRangeOfValues() ) {
         ...
         Aggregate* rangeValuesVal = new(generator->wHeap()) 
                         Aggregate(ITM_RANGE_VALUES_INSERT, 
                                   castedRightCol, FALSE);

         Int16 filterId = 
             getMinMaxHostvars()[((mmCol*step)+2)].getItemExpr()->
                getFilterId();
  ...
  if ( minHBufferSize > bufferSize ) {
    ...
    ULng32 padlen = ((hashBufferSize & 511) == 0)? 0 :\
                    (( 512 - (hashBufferSize & 511)) & 511);
  ...
  if(useUniqueHashJoin) {
   ...
   ULng32 sanityCheckCycle = 
               (ULng32)CmpCommon::getDefaultLong(
  ...
  if (useUniqueHashJoin) {
      UInt16 innerTableSizePerInstanceLimitInMB =
            (UInt16) getDefault(UNIQUE_HASH_JOIN_MAX_INNER_SIZE_PER_INSTANCE);
...
short NestedJoin::codeGen(Generator * generator)
{
  ...
  if(child(0) && child(1)) {
    ...
    if (needPushDownLimit &&
        childExpr0 && childExpr0->getOperatorType() == REL_HBASE_ACCESS &&
        childExpr1 && childExpr1->getOperatorType() == REL_HBASE_ACCESS &&
        (CmpCommon::getDefault(PUSHDOWN_LIMIT_OPT) == DF_ON))
    {
      ...
      if (isIndexJoin())
      {
        ...
        for(ValueId vid = ha1->executorPred().init();
                          ha1->executorPred().next(vid);
                          ha1->executorPred().advance(vid))
        ...
        for (CollIndex i =0 ; i < executorPred.entries() && needPushDownLimit; i ++)
        {
          ...
          if (!baseKeyCols.contains(colId))
            needPushDownLimit = FALSE;
        }

        if (!ha0->getBeginKeyPred().isEmpty() && !ha0->getEndKeyPred().isEmpty() && needPushDownLimit)
        {
          if (ha0->isMixedBeginKeyPred() || ha0->isMixedEndKeyPred())
            needPushDownLimit = FALSE;
        }
        else if (ha0->getListOfRangeRows().entries() > 0 && needPushDownLimit)
        {
          for (Int16 i = 0; i < ha0->getListOfRangeRows().entries() && needPushDownLimit; i++)
          {
            ...
            if (hs.beginKeyIsMixedExpr_ || hs.endKeyIsMixedRxpr_)
              needPushDownLimit = FALSE;
          }
        }
        else
...
short NestedJoinFlow::codeGen(Generator * generator)
{
  ...
  if (child(0)->getOperatorType() == REL_SORT &&
      child(1)->getOperatorType() == REL_HBASE_INSERT)
