...
short HashJoin::codeGen(Generator *generator) {
  ...
  GenAssert(!isSemiJoin() || !isAntiSemiJoin(), "Node can not be both semi-join and anti-semi-join");
  GenAssert(!isReuse() || isNoOverflow(), "Reuse of inner table requires no-overflow!");
  ...
  ex_expr *rangeSpecPackingExpr = 0;
  ...
  const ValueIdSet &rightOutputCols = child(1)->getGroupAttr()->getCharacteristicOutputs();
  ...
  for (valId = rightOutputCols.init(); rightOutputCols.next(valId); rightOutputCols.advance(valId))
  ...
  // return buffer
  const ValueIdSet &leftOutputCols = child(0)->getGroupAttr()->getCharacteristicOutputs();
  ...
  if (!useUniqueHashJoin) {
    ...
    // which are not part of the input.
    for (valId = leftOutputCols.init(); leftOutputCols.next(valId); leftOutputCols.advance(valId))
      if (NOT getGroupAttr()->getCharacteristicInputs().contains(valId)) leftOutputValIds.insert(valId);
  }
  ...
  ex_cri_desc *rightChildUpDesc = rightChildTdb->getCriDescUp();
  ...
  // values.  Later we will map the actual min/max values to the
  // same location in the min/max tuple.
  if (getMinMaxHostvars().entries()) {
    ...
    ExpTupleDesc *minmaxTupleDesc = 0;

    // Layout the min/max values in the min/max tuple and add
    // corresponding entries into the min/max mapTable
    // (myMapTableMM).
    //
    //  For type-1 HJ, the min max host vars are for passing min/max to
    //  the split bottom operators
    //  for non type-1 HJ, the min max host vars are for passing min/max to
    //  the scan node
    //  getMinMaxVals()(i.e., minMaxVals_) is populated according to the above
    //  description.
    expGen->processValIdList(getMinMaxHostvars(), ExpTupleDesc::SQLARK_EXPLODED_FORMAT, minMaxRowLength, 0,
                             minMaxValsDownAtpIndex, &minmaxTupleDesc, ExpTupleDesc::LONG_FORMAT);

    // Add the tuple descriptor to down Q desc
    leftDownDesc->setTupleDescriptor(minMaxValsDownAtpIndex, minmaxTupleDesc);
    ...
    // pop out myMapTableMM and make myMapTableMM1 the last one
    // on the stack of map tables
  ...
  // if ( getenv("mark1") ) {
  //   char buf[128];
  //   sprintf(buf, "Map tables, before codeGen for child(0) of HJ");
  //   generator->displayMapTables(INT32_MAX, buf);
  //}

  // Keep using the performMinMaxOptForType1 flag in generator.
  //

  // generator->displayMapTables(900000, "hj: before call child(0)::codeGen");
  ...
  if (useUniqueHashJoin) {
    ...
    if (isReuse()) prevInputTuppIndex = numOfATPIndexes++;
    ...
    if (leftOutputValIds.entries()) returnedLeftRowAtpIndex = returnedTuples++;

    if (rightOutputNeeded) {
      returnedRightRowAtpIndex = returnedTuples++;
      if (nullInstantiatedOutput().entries() > 0) returnedInstRowAtpIndex = returnedTuples++;
    }

    if (nullInstantiatedForRightJoinOutput().entries() > 0) returnedInstRowForRightJoinAtpIndex = returnedTuples++;
    ...
    if (nullInstantiatedOutput().entries() > 0) instRowForLeftJoinAtpIndex = numOfATPIndexes++;

    if (nullInstantiatedForRightJoinOutput().entries() > 0) instRowForRightJoinAtpIndex = numOfATPIndexes++;

    if (isReuse()) prevInputTuppIndex = numOfATPIndexes++;
  }

  if (getMinMaxHostvars().entries()) numOfATPIndexes++;

  ex_cri_desc *workCriDesc = new (space) ex_cri_desc(numOfATPIndexes, space);

  // If this HashJoin is doing min/max optimization (i.e., some scan
  // in the left child of this HJ can use the min/max value), then
  // generate the aggregate expressions to compute the Min and Max values.
  if (getMinMaxHostvars().entries()) {
    // add one more tuple for min/max values.
    minMaxValsAtpIndex = numOfATPIndexes - 1;
    ...
    for (CollIndex mmCol = 0; mmCol < getMinMaxCols().entries(); mmCol++) {
      ...
      ItemExpr *castedRightCol = new (generator->wHeap())
          Cast(rightCol, getMinMaxHostvars()[(mmCol * step)].getType().newCopy(generator->wHeap()));

      // The min and max aggregates
      ItemExpr *minVal = new (generator->wHeap()) Aggregate(ITM_MIN, castedRightCol, FALSE);
      ItemExpr *maxVal = new (generator->wHeap()) Aggregate(ITM_MAX, castedRightCol, FALSE);
      ...
      mapAttr = (generator->getMapInfo(getMinMaxHostvars()[(mmCol * step)]))->getAttr();
      ...
      // (system generated hostvar).  Set the ATP/ATPIndex to refer to
      // the min/max tuple in the workAtp
      mapAttr = (generator->getMapInfo(getMinMaxHostvars()[((mmCol * step) + 1)]))->getAttr();
      ...
      if (generator->isMinmaxOptWithRangeOfValues()) {
        Aggregate *rangeValuesVal = new (generator->wHeap()) Aggregate(ITM_RANGE_VALUES_INSERT, castedRightCol, FALSE);

        Int16 filterId = getMinMaxHostvars()[((mmCol * step) + 2)].getItemExpr()->getFilterId();

        rangeValuesVal->setFilterId(filterId);

        CostScalar uec = getInnerChildUecs()[mmCol];
        rangeValuesVal->setSourceColUec(uec);

        rangeValuesVal->bindNode(generator->getBindWA());

        ValueId rangesId = rangeValuesVal->getValueId();

        mapAttr = (generator->getMapInfo(getMinMaxHostvars()[((mmCol * step) + 2)]))->getAttr();
        mapAttr = (generator->addMapInfo(rangesId, mapAttr))->getAttr();
        mapAttr->setAtp(workAtpPos);
        mapAttr->setAtpIndex(minMaxValsAtpIndex);

        mmPairs.insert(rangesId);
      }
    }

    // Generate the min/max expression.
    ExpTupleDesc *minMaxWorkAtpDesc = NULL;
    expGen->generateAggrExpr(mmPairs, ex_expr::exp_AGGR, &minMaxExpr, 0, true, NULL, &minMaxWorkAtpDesc,
                             ExpTupleDesc::LONG_FORMAT);

    workCriDesc->setTupleDescriptor(minMaxValsAtpIndex, minMaxWorkAtpDesc);

    // No longer need the right child map table.
    generator->unlinkNext(lastMap);

  }  // end of if(getMinMaxVals().entries()
  ...
  if (!getEquiJoinPredicates().isEmpty()) {
    ...
    for (valId = getEquiJoinPredicates().init(); getEquiJoinPredicates().next(valId);
      ...
      if (NOT(buildType == probeType)) {
        ...
        const NAType *resultType =
            buildType.synthesizeType(SYNTH_RULE_UNION, buildType, probeType, generator->wHeap(), &flags);
      ...
      if (buildType.getTypeQualifier() == NA_CHARACTER_TYPE && probeType.getTypeQualifier() == NA_CHARACTER_TYPE) {
        const CharType &cBuildType = (CharType &)buildType;
        const CharType &cProbeType = (CharType &)probeType;

        if (cBuildType.isCaseinsensitive() != cProbeType.isCaseinsensitive()) doCasesensitiveHash = TRUE;
      }
      ...
      BuiltinFunction *hashFunction = new (generator->wHeap()) Hash(buildHashCol);
      if (buildHashTree) {
        buildHashTree = new (generator->wHeap()) HashComb(buildHashTree, hashFunction);
      } else {
      ...
      if (probeHashTree) {
        probeHashTree = new (generator->wHeap()) HashComb(probeHashTree, hashFunction);
      } else {
    ...
    // if need be.
    buildHashTree =
        new (generator->wHeap()) Cast(buildHashTree, new (generator->wHeap()) SQLInt(generator->wHeap(), FALSE, FALSE));
    probeHashTree =
        new (generator->wHeap()) Cast(probeHashTree, new (generator->wHeap()) SQLInt(generator->wHeap(), FALSE, FALSE));
    ...
    mapAttr = (generator->addMapInfo(probeHashTree->getValueId(), 0))->getAttr();

    mapAttr->copyLocationAttrs(generator->getMapInfo(buildHashTree->getValueId())->getAttr());

    // generate code to evaluate the hash expression
    expGen->generateArithExpr(buildHashTree->getValueId(), ex_expr::exp_ARITH_EXPR, &rightHashExpr);

    // generate the probe hash expression
    expGen->generateArithExpr(probeHashTree->getValueId(), ex_expr::exp_ARITH_EXPR, &leftHashExpr);
  };
  ...
  if (!(getCheckInnerNullExpr().isEmpty())) {
    ...
    expGen->generateExpr(newExprTree->getValueId(), ex_expr::exp_SCAN_PRED, &checkInnerNullExpression);
  }
  ...
  if (!(getCheckOuterNullExpr().isEmpty())) {
    ...
    expGen->generateExpr(newExprTree->getValueId(), ex_expr::exp_SCAN_PRED, &checkOuterNullExpression);
  }
  ...
  if (!bmo_affinity && getCachedTupleFormat() != ExpTupleDesc::UNINITIALIZED_FORMAT &&
    ...
    tupleFormat = determineInternalFormat(rightBufferValIds, leftOutputValIds, this, resizeCifRecord, generator,
                                          bmo_affinity, considerBufferDefrag, useUniqueHashJoin);
  ...
  if (rightBufferValIds.entries() > 0) {
    ...
    expGen->generateContiguousMoveExpr(rightBufferValIds,
                                       -1,  // add convert nodes
                                       workAtpPos, extRightRowAtpIndex1, tupleFormat, rightRowLength, &rightMoveInExpr,
                                       &rightMoveInDesc, ExpTupleDesc::LONG_FORMAT, &rightExtValMapTable,
                                       &rightMoveTargets1);

    // set up the desc for identifying and capturing integer-typed values
    // from the inner
  ...
  if (leftOutputValIds.entries() > 0) {
    ...
    expGen->generateContiguousMoveExpr(leftOutputValIds,
                                       -1,  // add convert nodes
                                       workAtpPos, extLeftRowAtpIndex, tupleFormat, leftRowLength, &leftMoveInExpr, 0,
  ...
  if (!getEquiJoinPredicates().isEmpty()) {
    ...
    expGen->generateExpr(newPredTree->getValueId(), ex_expr::exp_SCAN_PRED, &probeSearchExpr1);
  }
  ...
  if (!joinPred().isEmpty()) {
    ...
    expGen->generateExpr(newPredTree->getValueId(), ex_expr::exp_SCAN_PRED, &beforeJoinPred1);
  }
  ...
  if (!selectionPred().isEmpty()) {
    ...
    expGen->generateExpr(newPredTree->getValueId(), ex_expr::exp_SCAN_PRED, &afterJoinPred1);
  }
  ...
  if (!getEquiJoinPredicates().isEmpty()) {
    ...
    for (i = 0; i < rightBufferValIds.entries(); i++) {
      ...
      ItemExpr *newCol = new (generator->wHeap()) NATypeToItem((NAType *)&(rightBufferValIds[i].getType()));
      newCol->synthTypeAndValueId();

      // copy the attributes from the first entended row
      Attributes *originalAttr = generator->getMapInfo(rightMoveTargets1[i])->getAttr();

      Attributes *newAttr = generator->addMapInfo(newCol->getValueId(), 0)->getAttr();
    ...
    for (i = 0; i < hashKeyColumns.entries(); i++) {
      ...
      ItemExpr *eqNode = new (generator->wHeap())
          BiRelat(ITM_EQUAL, rightMoveTargets1[hashColNum].getItemExpr(), rightMoveTargets2[hashColNum].getItemExpr(),
    ...
    expGen->generateExpr(newPredTree->getValueId(), ex_expr::exp_SCAN_PRED, &rightSearchExpr);
  }
  ...
  if (!useUniqueHashJoin) {
    ...
    if (!getEquiJoinPredicates().isEmpty()) {
      ItemExpr *newPredTree;
      newPredTree = getEquiJoinPredicates().rebuildExprTree(ITM_AND, TRUE, TRUE);
      expGen->generateExpr(newPredTree->getValueId(), ex_expr::exp_SCAN_PRED, &probeSearchExpr2);
    }
  }
  ...
  if (!joinPred().isEmpty()) {
    ItemExpr *newPredTree;
    newPredTree = joinPred().rebuildExprTree(ITM_AND, TRUE, TRUE);
    expGen->generateExpr(newPredTree->getValueId(), ex_expr::exp_SCAN_PRED, &beforeJoinPred2);
  }
  ...
  if (!selectionPred().isEmpty()) {
    ItemExpr *newPredTree;
    newPredTree = selectionPred().rebuildExprTree(ITM_AND, TRUE, TRUE);
    expGen->generateExpr(newPredTree->getValueId(), ex_expr::exp_SCAN_PRED, &afterJoinPred2);
  }
  ...
  if (rightOutputValIds.entries() > 0 && rightOutputNeeded) {
    ...
    expGen->generateContiguousMoveExpr(*rmo,
                                       -1,  // add convert nodes
                                       workAtpPos, rightRowAtpIndex, tupleFormat, rightRowLength, &rightMoveOutExpr, 0,
                                       ExpTupleDesc::SHORT_FORMAT, &rightValMapTable,
  ...
  if (leftOutputValIds.entries() > 0) {
    expGen->generateContiguousMoveExpr(leftOutputValIds,
                                       -1,  // add convert nodes
                                       workAtpPos, leftRowAtpIndex, tupleFormat, leftRowLength, &leftMoveOutExpr, 0,
                                       ExpTupleDesc::SHORT_FORMAT, &leftValMapTable);
  }
  ...
  if (rightOutputNeeded) {
    ...
    if (nullInstantiatedOutput().entries() > 0) {
      instantiateValuesForLeftJoin(generator, workAtpPos, instRowForLeftJoinAtpIndex, &leftJoinExpr,
                                   &nullInstForLeftJoinExpr, &instRowLength, &instNullForLeftJoinMapTable, tupleFormat);
    };
    ...
    if (isLeftJoin() && !selectionPred().isEmpty()) {
      ...
      ItemExpr *newPredTree;
      newPredTree = selectionPred().rebuildExprTree(ITM_AND, TRUE, TRUE);
      expGen->generateExpr(newPredTree->getValueId(), ex_expr::exp_SCAN_PRED, &afterJoinPred3);
      ...
      //
      {
        ...
        newPredTree = selectionPred().rebuildExprTree(ITM_AND, TRUE, TRUE);
        expGen->generateExpr(newPredTree->getValueId(), ex_expr::exp_SCAN_PRED, &afterJoinPred4);
  ...
  if (nullInstantiatedForRightJoinOutput().entries() > 0) {
    instantiateValuesForRightJoin(generator, workAtpPos, instRowForRightJoinAtpIndex, &rightJoinExpr,
                                  &nullInstForRightJoinExpr, &instRowForRightJoinLength,
  ...
  if (isRightJoin() && !selectionPred().isEmpty()) {
    ...
    ItemExpr *newPredTree;
    newPredTree = selectionPred().rebuildExprTree(ITM_AND, TRUE, TRUE);
    expGen->generateExpr(newPredTree->getValueId(), ex_expr::exp_SCAN_PRED, &afterJoinPred5);
  ...
  if (leftOutputValIds.entries()) {
    for (CollIndex ix = 0; ix < leftOutputValIds.entries(); ix++) {
      ...
      MapInfo *map_info = generator->getMapInfoFromThis(leftValMapTable, valId);
      if (map_info) {
        Attributes *attr = map_info->getAttr();
        attr->setAtp(0);
  ...
  Cardinality innerExpectedRows =
      (Cardinality)child(1)->getGroupAttr()->getOutputLogPropList()[0]->getResultCardinality().value();
  ...
  if (saveNumEsps > 0 && getParallelJoinType() == 1) innerExpectedRows /= (Cardinality)saveNumEsps;

  // determine the expected size of the outer table
  Cardinality outerExpectedRows =
      (Cardinality)child(0)->getGroupAttr()->getOutputLogPropList()[0]->getResultCardinality().value();

  // If this HJ is performed within ESPs, then number of records
  // processed by each ESP is a subset of total records.
  if (saveNumEsps > 0) outerExpectedRows /= (Cardinality)saveNumEsps;
  ...
  if (isReuse()) {  // only if testing is needed
    ...
    if (!moveInputValues().isEmpty()) {
      ...
      expGen->generateContiguousMoveExpr(vid_list,
                                         ...
                                         workAtpPos, prevInputTuppIndex, tupleFormat, inputValuesLen, &moveInputExpr);
    }
    ...
    if (!checkInputValues().isEmpty()) {
      ItemExpr *newPredTree = checkInputValues().rebuildExprTree(ITM_AND, TRUE, TRUE);
      expGen->generateExpr(newPredTree->getValueId(), ex_expr::exp_SCAN_PRED, &checkInputPred);
    }
  }
  ...
  if (useUniqueHashJoin) {
    ...
    GenAssert(!nullInstForLeftJoinExpr, "Bad Unique Hash Join: nullInstForLeftJoinExpr");
    ...
    GenAssert(instRowForLeftJoinAtpIndex == -1, "Bad Unique Hash Join: instRowForLeftJoinAtpIndex");
    GenAssert(returnedLeftRowAtpIndex == -1, "Bad Unique Hash Join: returnedLeftRowAtpIndex");
    GenAssert(returnedInstRowAtpIndex == -1, "Bad Unique Hash Join: returnedInstRowAtpIndex");
    GenAssert(!rightJoinExpr, "Bad Unique Hash Join: rightJoinExpr");
    GenAssert(!nullInstForRightJoinExpr, "Bad Unique Hash Join: nullInstForRightJoinExpr");
    GenAssert(instRowForRightJoinAtpIndex == -1, "Bad Unique Hash Join: instRowForRightJoinAtpIndex");
    GenAssert(returnedInstRowForRightJoinAtpIndex == -1, "Bad Unique Hash Join: returnedInstRowForRightJoinAtpIndex");
    GenAssert(instRowForRightJoinLength == 0, "Bad Unique Hash Join: instRowForRightJoinLength");
  }
  ...
  ComTdbHashj *hashj_tdb = new (space) ComTdbHashj(
      ...
      checkInnerNullExpression, checkOuterNullExpression, hjGrowthPercent,

      // For min/max optimization.
      minMaxValsAtpIndex, minMaxRowLength, minMaxExpr, leftDownDesc, rangeSpecPackingExpr, sanityCheckCycle);

  hashj_tdb->setRecordLength(getGroupAttr()->getRecordLength());

  generator->initTdbFields(hashj_tdb);
  hashj_tdb->setOverflowMode(generator->getOverflowMode());
  if (CmpCommon::getDefault(EXE_BMO_SET_BUFFERED_WRITES) == DF_ON) hashj_tdb->setBufferedWrites(TRUE);
  if (CmpCommon::getDefault(EXE_DIAGNOSTIC_EVENTS) == DF_ON) hashj_tdb->setLogDiagnostics(TRUE);
  if (useUniqueHashJoin ||  // UHJ avoids check for early overflow
      ...
      || (((generator->getOverflowMode() == ComTdb::OFM_DISK) || (generator->getOverflowMode() == ComTdb::OFM_MMAP)) &&
          CmpCommon::getDefault(EXE_BMO_DISABLE_CMP_HINTS_OVERFLOW_HASH) == DF_SYSTEM))
  ...
  if (generator->getOverflowMode() == ComTdb::OFM_SSD)
    hashj_tdb->setBMOMaxMemThresholdMB(
        (UInt16)(ActiveSchemaDB()->getDefaults()).getAsLong(SSD_BMO_MAX_MEM_THRESHOLD_IN_MB));
  else
    hashj_tdb->setBMOMaxMemThresholdMB((UInt16)(ActiveSchemaDB()->getDefaults()).getAsLong(EXE_MEMORY_AVAILABLE_IN_MB));

  hashj_tdb->setScratchIOVectorSize((Int16)getDefault(SCRATCH_IO_VECTOR_SIZE_HASH));
  hashj_tdb->setForceOverflowEvery(
      (UInt16)(ActiveSchemaDB()->getDefaults()).getAsULong(EXE_TEST_HASH_FORCE_OVERFLOW_EVERY));

  hashj_tdb->setForceHashLoopAfterNumBuffers(
      (UInt16)(ActiveSchemaDB()->getDefaults()).getAsULong(EXE_TEST_FORCE_HASH_LOOP_AFTER_NUM_BUFFERS));

  hashj_tdb->setForceClusterSplitAfterMB(
      (UInt16)(ActiveSchemaDB()->getDefaults()).getAsULong(EXE_TEST_FORCE_CLUSTER_SPLIT_AFTER_MB));
  ...
  if (useUniqueHashJoin) {
    ...
    memQuota = innerTableSizePerInstanceLimitInMB;
    hashj_tdb->setMemoryQuotaMB(memQuota);
  } else if (mmu != 0) {
    memQuota = mmu;
  ...
  if (beforeJoinPredOnOuterOnly()) hashj_tdb->setBeforePredOnOuterOnly();
  ...
  if (generator->getRightSideOfFlow()) hashj_tdb->setPossibleMultipleCalls(TRUE);

  // Internal CQD -- if set, enforce a minimum number of clusters
  UInt16 nc = (UInt16)(ActiveSchemaDB()->getDefaults()).getAsDouble(EXE_HJ_MIN_NUM_CLUSTERS);
  if (nc != 0) hashj_tdb->setNumClusters(nc);
  ...
  if (!generator->explainDisabled()) {
    generator->setExplainTuple(addExplainInfo(hashj_tdb, leftExplainTuple, rightExplainTuple, generator));
  }
  ...
  hashj_tdb->setDelayLeftRequest(innerExpectedRows > 100 || outerExpectedRows > 100000);
  ...
  if (minMaxExpr) hashj_tdb->setDelayLeftRequest(true);
...
ExpTupleDesc::TupleDataFormat HashJoin::determineInternalFormat(const ValueIdList &rightList,
                                                                const ValueIdList &leftList, RelExpr *relExpr,
  ...
  rTupleFormat =
      generator->determineInternalFormat(rightList, relExpr, rResizeRecord, bmo_cif, bmo_affinity, rAlignedLength,
                                         rExplodedLength, rAlignedVarCharSize, rAlignedHeaderSize, rAvgVarCharUsage);

  lTupleFormat =
      generator->determineInternalFormat(leftList, relExpr, lResizeRecord, bmo_cif, bmo_affinity, lAlignedLength,
                                         lExplodedLength, lAlignedVarCharSize, lAlignedHeaderSize, lAvgVarCharUsage);

  if (rTupleFormat == ExpTupleDesc::SQLARK_EXPLODED_FORMAT && lTupleFormat == ExpTupleDesc::SQLARK_EXPLODED_FORMAT) {
    resizeCifRecord = FALSE;
    return ExpTupleDesc::SQLARK_EXPLODED_FORMAT;
  }
  ...
  if (rTupleFormat == ExpTupleDesc::SQLMX_ALIGNED_FORMAT && lTupleFormat == ExpTupleDesc::SQLMX_ALIGNED_FORMAT) {
  ...
  if ((rAlignedVarCharSize > rAlignedHeaderSize || lAlignedVarCharSize > lAlignedHeaderSize) &&
...
CostScalar HashJoin::getEstimatedRunTimeMemoryUsage(Generator *generator, NABoolean perNode, Lng32 *numStreams) {
  ...
  if (phyProp) {
    ...
    if (partFunc->isAReplicationPartitioningFunction() == TRUE) totalHashTableMemory *= numOfStreams;
  }
...
NABoolean HashJoin::canUseUniqueHashJoin() {
  ...
  if (!isLeftJoin() && !isRightJoin() && joinPred().isEmpty() && selectionPred().isEmpty() && !isAntiSemiJoin() &&
      rowsFromLeftHaveUniqueMatch() && !getEquiJoinPredicates().isEmpty() &&
    ...
    if (rightChildGA->getGroupAnalysis()) {
      const CANodeIdSet &nodes = rightChildGA->getGroupAnalysis()->getAllSubtreeTables();

      UInt32 innerTables = (UInt32)getDefault(UNIQUE_HASH_JOIN_MAX_INNER_TABLES);

      // Default is 1GB
      UInt32 innerTableSizeLimitInMB = (UInt32)getDefault(UNIQUE_HASH_JOIN_MAX_INNER_SIZE);

      // Default is 100MB
      UInt32 innerTableSizePerInstanceLimitInMB = (UInt32)getDefault(UNIQUE_HASH_JOIN_MAX_INNER_SIZE_PER_INSTANCE);
      ...
      if (getParallelJoinType() == 1) {
        ...
        numPartitions = partFunc->getCountOfPartitions();
      }

      if (nodes.entries() == 1) {
        TableAnalysis *tabAnalysis = nodes.getFirst().getNodeAnalysis()->getTableAnalysis();
        ...
        if (tabAnalysis) {
          ...
          if (numRows >= estRows) {
            ...
            CostScalar innerTableSizePerInstance = innerTableSize / numPartitions;
      ...
      if (nodes.entries() <= innerTables) {
        CostScalar maxRows = rightChildGA->getResultMaxCardinalityForEmptyInput();
        CostScalar innerTableMaxSize = maxRows * rowSize / (1024 * 1024);
        CostScalar innerTableMaxSizePerInstance = innerTableMaxSize / numPartitions;
...
void HashJoin::addCheckNullExpressions(CollHeap *wHeap) {
  ...
  for (valId = getEquiJoinPredicates().init(); getEquiJoinPredicates().next(valId);
    ...
    if ((itemExpr->getOperatorType() == ITM_EQUAL) && ((BiRelat *)itemExpr)->getIsNotInPredTransform()) {
      notinCount++;
      ...
      if (innerType.supportsSQLnull() && !((BiRelat *)itemExpr)->getInnerNullFilteringDetected()) {
      ...
      // outer column
      if (outerType.supportsSQLnull() && !((BiRelat *)itemExpr)->getOuterNullFilteringDetected()) {
...
void gatherValuesFromExpr(ValueIdList &vs, ItemExpr *ie, Generator *generator) {
...
short MergeJoin::codeGen(Generator *generator) {
  ...
  if ((!is_semijoin) && (!is_anti_semijoin) && (!is_leftjoin)) {
#ifdef _DEBUG
...
#endif
    if (isLeftUnique || isRightUnique) fastMJEval = TRUE;
  }
  ...
  for (i = 0; i < orderedMJPreds().entries(); i++) {
    ...
    // Do the right row.
    ValueId child1Vid = val_id.getItemExpr()->child(1)->castToItemExpr()->getValueId();
    ...
    if (!child1MapInfo) {
      ...
      if ((child1Vid.getItemExpr()->getOperatorType() == ITM_CAST) ||
        ...
        ValueId nextChild0Vid = child1Vid.getItemExpr()->child(0)->castToItemExpr()->getValueId();
        ...
        if (generator->getMapInfoAsIs(nextChild0Vid)) {
          ...
          //
          ph = new (generator->wHeap()) Cast(child1Vid.getItemExpr()->child(0), &(nextChild0Vid.getType()));
      ...
      // Create place holder node.
      //
      ph = new (generator->wHeap()) Cast(val_id.getItemExpr()->child(1), &(child1Vid.getType()));
    ...
    if (childAttr->getAtpIndex() > 1) {
      ...
      // child value
      //
      MapInfo *map_info = generator->addMapInfo(ph->getValueId(), childAttr);
    ...
    if (!child1MapInfo) {
      ...
      //
      ph = new (generator->wHeap()) Cast(ph, &(child1Vid.getType()));
    }

    BiRelat *bi_relat = new (generator->wHeap()) BiRelat(ITM_EQUAL, val_id.getItemExpr()->child(1), ph);
  ...
  if (NOT doEncodedKeyCompOpt) {
    ...
    right_list = new (generator->wHeap()) ItemExprTreeAsList(&right_tree, ITM_ITEM_LIST, RIGHT_LINEAR_TREE);
  }
  ...
  for (i = 0; i < orderedMJPreds().entries(); i++) {
    ...
    if (NOT(leftType == rightType)) {
      ...
      const NAType *resultType =
          leftType.synthesizeType(SYNTH_RULE_UNION, leftType, rightType, generator->wHeap(), &flags);
    ...
    if (getLeftSortOrder()[i].getItemExpr()->getOperatorType() == ITM_INVERSE) desc_flag = TRUE;

    CompEncode *encoded_left_val = new (generator->wHeap()) CompEncode(left_val, desc_flag);
    CompEncode *encoded_right_val = new (generator->wHeap()) CompEncode(right_val, desc_flag);
  ...
  const ValueIdSet &child1CharacteristicOutputs = child(1)->castToRelExpr()->getGroupAttr()->getCharacteristicOutputs();

  // create a list of all values returned from right child that are
  // not input to this node.
  ValueIdList rightChildOutput;

  for (val_id = child1CharacteristicOutputs.init(); child1CharacteristicOutputs.next(val_id);
  ...
  if (NOT doEncodedKeyCompOpt) {
    ...
    exp_gen->generateExpr(newPredTree->getValueId(), ex_expr::exp_SCAN_PRED, &merge_expr);

    // generate the comp expression. This expression is used
    // to look for a matching value in the hash table.
    exp_gen->generateExpr(compTree->getValueId(), ex_expr::exp_SCAN_PRED, &comp_expr);
  } else {
    ...
    exp_gen->generateContiguousMoveExpr(leftEncodedValIds,
                                        ...
                                        1,  // atp 1
                                        encoded_key_atp_index, ExpTupleDesc::SQLARK_EXPLODED_FORMAT, encoded_key_len,
                                        &left_encoded_key_expr, 0, ExpTupleDesc::SHORT_FORMAT);
    ...
    exp_gen->generateContiguousMoveExpr(rightEncodedValIds,
                                        ...
                                        0,  // atp 0
                                        encoded_key_atp_index, ExpTupleDesc::SQLARK_EXPLODED_FORMAT, encoded_key_len,
                                        &right_encoded_key_expr, 0, ExpTupleDesc::SHORT_FORMAT);
  }
  ...
  if (NOT fastMJEval) {
    ...
    exp_gen->generateContiguousMoveExpr(rightChildOutput,
                                        -1,  // add convert nodes
                                        1 /*atp*/, 2 /*atpindex*/, tupleFormat, right_row_len, &right_copy_dup_expr,
                                        NULL, ExpTupleDesc::SHORT_FORMAT, NULL, &resultValIdList);

    ValueIdList prevRightValIds;  // the secend operand of dup comparison
    CollIndex i = 0;
    for (i = 0; i < resultValIdList.entries(); i++) {
      // create the new item xpression
      ItemExpr *newCol = new (generator->wHeap()) NATypeToItem((NAType *)&(resultValIdList[i].getType()));
      newCol->synthTypeAndValueId();

      // copy the attributes
      Attributes *originalAttr = generator->getMapInfo(resultValIdList[i])->getAttr();
    ...
    for (i = 0; i < rightChildOfMJPList.entries(); i++) {
      ...
      BiRelat *bi_relat = new (generator->wHeap())
          BiRelat(ITM_EQUAL, rightChildOfMJPList[i].getItemExpr(), prevRightValIds[index].getItemExpr());
      // for the purpose of checking duplicates, nulls are equal
      // to other nulls. Mark them so.
      bi_relat->setSpecialNulls(-1);
    ...
    exp_gen->generateExpr(newPredTree->getValueId(), ex_expr::exp_SCAN_PRED, &right_check_dup_expr);
    ...
    for (i = 0; i < rightChildOfMJPList.entries(); i++) {
      ...
      BiRelat *bi_relat = new (generator->wHeap())
          BiRelat(ITM_EQUAL, leftChildOfMJPList[i].getItemExpr(), resultValIdList[index].getItemExpr());
      // for the purpose of checking duplicates, nulls are equal
      // to other nulls. Mark them so.
      bi_relat->setSpecialNulls(-1);
    ...
    exp_gen->generateExpr(newPredTree->getValueId(), ex_expr::exp_SCAN_PRED, &left_check_dup_expr);
  }
  ...
  if (!joinPred().isEmpty()) {
    ItemExpr *newPredTree;
    newPredTree = joinPred().rebuildExprTree(ITM_AND, TRUE, TRUE);
    exp_gen->generateExpr(newPredTree->getValueId(), ex_expr::exp_SCAN_PRED, &pre_join_expr);
  }
  ...
  if (!selectionPred().isEmpty()) {
    ...
    exp_gen->generateExpr(newPredTree->getValueId(), ex_expr::exp_SCAN_PRED, &post_join_expr);
  }

  bool isOverflowEnabled = (CmpCommon::getDefault(MJ_OVERFLOW) == DF_ON);
  UInt16 scratchThresholdPct = (UInt16)getDefault(SCRATCH_FREESPACE_THRESHOLD_PERCENT);
  ...
  if (CmpCommon::getDefaultLong(MJ_BMO_QUOTA_PERCENT) != 0) {
    ...
    if (mlimitPerNode || numBMOsInFrag > 1 ||
        (numBMOsInFrag == 1 && CmpCommon::getDefault(EXE_SINGLE_BMO_QUOTA) == DF_ON)) {
      quotaMB = (UInt16)computeMemoryQuota(generator->getEspLevel() == 0, mlimitPerNode,
                                           generator->getBMOsMemoryLimitPerNode().value(), generator->getTotalNumBMOs(),
                                           generator->getTotalBMOsMemoryPerNode().value(), numBMOsInFrag,
  ...
  ComTdbMj *mj_tdb = new (space) ComTdbMj(
      child_tdb1, child_tdb2, given_desc, returned_desc, (NOT doEncodedKeyCompOpt ? merge_expr : left_encoded_key_expr),
      (NOT doEncodedKeyCompOpt ? comp_expr : right_encoded_key_expr), left_check_dup_expr, right_check_dup_expr,
      lj_expr, 0, right_copy_dup_expr, right_row_len, rowlen, work_cri_desc, instantiated_row_atp_index,
      encoded_key_len, encoded_key_atp_index, pre_join_expr, post_join_expr, (queue_index)getDefault(GEN_MJ_SIZE_DOWN),
      (queue_index)getDefault(GEN_MJ_SIZE_UP),
      (Cardinality)getGroupAttr()->getOutputLogPropList()[0]->getResultCardinality().value(),
      getDefault(GEN_MJ_NUM_BUFFERS), getDefault(GEN_MJ_BUFFER_SIZE), is_semijoin, is_leftjoin, is_anti_semijoin,
      isLeftUnique, isRightUnique, isOverflowEnabled, scratchThresholdPct, quotaMB, quotaPct, yieldQuota);
  mj_tdb->setRecordLength(getGroupAttr()->getRecordLength());
  ...
  if (!generator->explainDisabled()) {
    generator->setExplainTuple(addExplainInfo(mj_tdb, leftExplainTuple, rightExplainTuple, generator));
  }
...
short NestedJoin::codeGen(Generator *generator) {
  ...
  NABoolean needPushDownLimit = isNeedPushDownLimit();
  ...
  //
  GenAssert((child(0) AND child(1))OR(NOT child(0) AND NOT(child(1))), "NestedJoin -- missing one child");
  ...
  if (child(0) && child(1)) {
    ...
    RelExpr *childExpr1 = child(1)->castToRelExpr();

    if (needPushDownLimit && childExpr0 && childExpr0->getOperatorType() == REL_HBASE_ACCESS && childExpr1 &&
        childExpr1->getOperatorType() == REL_HBASE_ACCESS && (CmpCommon::getDefault(PUSHDOWN_LIMIT_OPT) == DF_ON)) {
      // push limit down to control the access data from the upper node
      //  The following conditions must currently be met
      // * index join
      // * There are no columns in the query condition that are not belongs to the joined index
      // * max/min only appears at the end of the beginkey and endkey

      if (isIndexJoin()) {
        ValueIdList baseKeyCols;
        ValueIdList executorPred;
        ValueId colId;
        NABoolean isConstant = FALSE;
        HbaseAccess *ha0 = (HbaseAccess *)childExpr0;
        HbaseAccess *ha1 = (HbaseAccess *)childExpr1;
        ItemExpr *col_expr = NULL;
        const ValueIdList &keyColumns = ha0->getIndexDesc()->getIndexKey();
        ((ValueIdList &)keyColumns).getBaseColumnList(baseKeyCols);

        for (ValueId vid = ha1->executorPred().init(); ha1->executorPred().next(vid); ha1->executorPred().advance(vid))
          executorPred.insert(vid);

        for (CollIndex i = 0; i < executorPred.entries() && needPushDownLimit; i++) {
          col_expr = ((executorPred[i]).getValueDesc())->getItemExpr();
          if (col_expr->getChild(0)->getOperatorType() == ITM_BASECOLUMN)
            colId = ((BaseColumn *)col_expr->getChild(0))->getValueId().getBaseColumn(&isConstant);
          else if (col_expr->getChild(0)->getOperatorType() == ITM_INDEXCOLUMN)
            colId = ((IndexColumn *)col_expr->getChild(0))->getValueId().getBaseColumn(&isConstant);
          else {
            needPushDownLimit = FALSE;
            break;
          }

          if (!baseKeyCols.contains(colId)) needPushDownLimit = FALSE;
        }

        if (!ha0->getBeginKeyPred().isEmpty() && !ha0->getEndKeyPred().isEmpty() && needPushDownLimit) {
          if (ha0->isMixedBeginKeyPred() || ha0->isMixedEndKeyPred()) needPushDownLimit = FALSE;
        } else if (ha0->getListOfRangeRows().entries() > 0 && needPushDownLimit) {
          for (Int16 i = 0; i < ha0->getListOfRangeRows().entries() && needPushDownLimit; i++) {
            HbaseRangeRows &hs = ha0->getListOfRangeRows()[i];
            if (hs.beginKeyIsMixedExpr_ || hs.endKeyIsMixedRxpr_) needPushDownLimit = FALSE;
          }
        } else
          needPushDownLimit = FALSE;

        child(0)->setNeedPushDownLimit(needPushDownLimit);
      }
    }
    ...
    if (generator->getMakeOnljLeftQueuesBig()) {
      ...
      if (generator->getTopNRows() > 0 && needPushDownLimit && upSize > generator->getTopNRows())
        upSize = generator->getTopNRows();
  ...
  if (!joinPred().isEmpty()) {
    ItemExpr *newPredTree = joinPred().rebuildExprTree(ITM_AND, TRUE, TRUE);
    exp_gen->generateExpr(newPredTree->getValueId(), ex_expr::exp_SCAN_PRED, &before_expr);
  }
  ...
  if (is_semijoin || is_antisemijoin) save_map_table = generator->getLastMapTable();
  ...
  if (child(0) && child(1)) {
    ...
    NABoolean savComputeRowsetRowsAffected = generator->computeRowsetRowsAffected();
    if (getRowsetRowCountArraySize() > 0) generator->setComputeRowsetRowsAffected(TRUE);
  ...
  // first place.
  if (tolerateNonFatalError) generator->setTolerateNonFatalErrorInFlowRightChild(FALSE);
  ...
  if (!is_semijoin) {
    returned_tuples = right_child_desc->noTuples();
    if (nullInstantiatedOutput().entries() > 0) returned_instantiated_row_atp_index = (short)returned_tuples++;
  }
  ex_cri_desc *returned_desc = new (space) ex_cri_desc(returned_tuples, space);
  ...
  if (is_semijoin || is_antisemijoin) generator->removeAll(save_map_table);
  ...
  if (!afterPredicates.isEmpty()) {
    ...
    exp_gen->generateExpr(newPredTree->getValueId(), ex_expr::exp_SCAN_PRED, &after_expr);
  }

  //++Triggers
  // no characteristic output, remove values that where generated by the children
  if (!(getGroupAttr()->getCharacteristicOutputs().entries())) generator->removeAll(beforeLeftMapTable);
  //--Triggers
  ...
  ComTdbOnlj *nlj_tdb = new (space) ComTdbOnlj(
      tdb1, tdb2, given_desc, returned_desc, (queue_index)getDefault(GEN_ONLJ_SIZE_DOWN),
      (queue_index)getDefault(GEN_ONLJ_SIZE_UP),
      (Cardinality)getGroupAttr()->getOutputLogPropList()[0]->getResultCardinality().value(),
      getDefault(GEN_ONLJ_NUM_BUFFERS), bufferSize, before_expr, after_expr, lj_expr, 0, 0, 0, rowlen, is_semijoin,
      is_antisemijoin, is_leftjoin, is_undojoin, is_setnferror, isRowsetIterator(), isIndexJoin(),
      rightChildIsVsbbInsert, getRowsetRowCountArraySize(), tolerateNonFatalError, usedForMvLogging);
  // getRowsetRowCountArraySize() should return positive values
  // only if isRowsetIterator() returns TRUE.
  GenAssert((((getRowsetRowCountArraySize() > 0) && isRowsetIterator()) || (getRowsetRowCountArraySize() == 0)),
            "Incorrect value returned by getRowsetRowCountArray()");

  nlj_tdb->setRecordLength(getGroupAttr()->getRecordLength());
  ...
  if (!generator->explainDisabled()) {
    generator->setExplainTuple(addExplainInfo(nlj_tdb, leftExplainTuple, rightExplainTuple, generator));
  }
...
short NestedJoinFlow::codeGen(Generator *generator) {
  ...
  if ((numberOfInputRows > 1) && (child(1)) &&
      ((child(1)->getOperatorType() == REL_HBASE_DELETE) || (child(1)->getOperatorType() == REL_HBASE_UPDATE)) &&
  ...
  if (child(0)->getOperatorType() == REL_SORT && child(1)->getOperatorType() == REL_HBASE_INSERT)
    ((Sort *)(child(0)->castToRelExpr()))->doIgnoreTrailingBlank();
  ...
  ComTdbTupleFlow *tflow_tdb = new (space)
      ComTdbTupleFlow(tdb1, tdb2, given_desc, returned_desc,
                      0,  // no tgt Expr yet
                      0,  // no work cri desc yet
                      (queue_index)getDefault(GEN_TFLO_SIZE_DOWN), (queue_index)getDefault(GEN_TFLO_SIZE_UP),
                      (Cardinality)getGroupAttr()->getOutputLogPropList()[0]->getResultCardinality().value(),
                      getDefault(GEN_TFLO_NUM_BUFFERS), getDefault(GEN_TFLO_BUFFER_SIZE), generator->getVSBBInsert(),
                      isRowsetIterator(), tolerateNonFatalError);

  tflow_tdb->setRecordLength(getGroupAttr()->getRecordLength());
  ...
  if (!generator->explainDisabled())
    generator->setExplainTuple(addExplainInfo(tflow_tdb, leftExplainTuple, rightExplainTuple, generator));
...
short Join::instantiateValuesForLeftJoin(Generator *generator, short atp, short atp_index, ex_expr **lj_expr,
  ...
  exp_gen->generateContiguousMoveExpr(nullInstantiatedOutput(),
                                      0,  // don't add convert nodes
                                      atp, atp_index, tupleFormat, *rowlen, lj_expr,
                                      0,  // no need for ExpTupleDesc * tupleDesc
                                      ExpTupleDesc::SHORT_FORMAT, newMapTable);
  ...
  for (CollIndex i = 0; i < nullInstantiatedOutput().entries(); i++) {
    ...
    ItemExpr *ie = new (generator->wHeap()) Cast(const_value, &(val_id.getType()));
    ie->bindNode(generator->getBindWA());

    generator->addMapInfo(ie->getValueId(), generator->getMapInfo(val_id)->getAttr());
...
short Join::instantiateValuesForRightJoin(Generator *generator, short atp, short atp_index, ex_expr **rj_expr,
  ...
  exp_gen->generateContiguousMoveExpr(nullInstantiatedForRightJoinOutput(),
                                      0,  // don't add convert nodes
                                      atp, atp_index, tupleFormat, *rowlen, rj_expr,
                                      0,  // no need for ExpTupleDesc * tupleDesc
                                      ExpTupleDesc::SHORT_FORMAT, newMapTable);
