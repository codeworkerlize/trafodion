...
short ExtStoragePushdownAggr::codeGen(Generator * generator)
{
  Space * space          = generator->getSpace();
  ExpGenerator * expGen = generator->getExpGenerator();
  ...
  if (tableDesc_->getNATable()->isORC())
    type = (short)ComTdbHdfsScan::ORC_;
  else if (tableDesc_->getNATable()->isParquet())
    type = (short)ComTdbHdfsScan::PARQUET_;
  else if (tableDesc_->getNATable()->isAvro())
  ...
  for (ValueId valId = aggregateExpr().init();
       ...
       aggregateExpr().advance(valId), i++) 
    {
      ...
      const ValueId &extAggrVid = extAggrVal->getValueId();
      generator->addMapInfo(extAggrVid, 0);
      extAggrVidList.insert(extAggrVid);
    }

  // Create the descriptor describing row where aggr returned by external
  // storage will be added.
  ExpTupleDesc * extAggrTupleDesc = NULL;
  ULng32 extAggrRowLen = 0;
  expGen->processValIdList(extAggrVidList,
                           ExpTupleDesc::SQLARK_EXPLODED_FORMAT,
                           extAggrRowLen,
                           workAtp,
                           extAggrTuppIndex,
  ...
  work_cri_desc->setTupleDescriptor(extAggrTuppIndex, extAggrTupleDesc);
  ...
  for (Int32 j = 0; j < numAttrs; j++)
    {
      ...
      Aggregate *a = (Aggregate*)valId.getItemExpr();
      ComTdbExtStorageFastAggr::ExtStorageAggrType aggrType;
      char * aggrTypeInList = NULL;
      char * aggrColName = NULL;
      ...
      if (NOT ((a->getOperatorType() == ITM_COUNT) ||
               ...
               ((type == ComTdbHdfsScan::PARQUET_) &&
                (a->getOperatorType() == ITM_COUNT_NONULL))))
        {
	  GenAssert(0, "This aggr not yet supported for fast ORC/PARQUET execution.");
        }

      char * cnameInList = NULL;
      Lng32 position = 0;
      Lng32 hiveType = HIVE_UNKNOWN_TYPE;
      if (a->getOperatorType() == ITM_COUNT) // count(*)
	{
          cnameInList = 
               ...
               (a->getOperatorType() == ITM_ORC_SUM_NV))
        {
          ...
          position = nac->getPosition()+1; // ext col numbers are 1 based

          hiveType = nac->getType()->getHiveType();
          if (hiveType == HIVE_UNKNOWN_TYPE)
            {
              GenAssert(0, "Pushdown aggr: Unknown hive type.");
            }
       }

      if (a->getOperatorType() == ITM_COUNT_NONULL)
        aggrType = ComTdbExtStorageFastAggr::COUNT_NONULL_;
      else if (a->getOperatorType() == ITM_MIN)
        aggrType = ComTdbExtStorageFastAggr::MIN_;            
      else if (a->getOperatorType() == ITM_MAX)
        aggrType = ComTdbExtStorageFastAggr::MAX_;
      else if (a->getOperatorType() == ITM_COUNT)
        aggrType = ComTdbExtStorageFastAggr::COUNT_;
      else if (a->getOperatorType() == ITM_SUM)
        aggrType = ComTdbExtStorageFastAggr::SUM_;
      else if (a->getOperatorType() == ITM_ORC_MAX_NV)
        aggrType = ComTdbExtStorageFastAggr::ORC_NV_LOWER_BOUND_;
      else if (a->getOperatorType() == ITM_ORC_SUM_NV)
        aggrType = ComTdbExtStorageFastAggr::ORC_NV_UPPER_BOUND_;

      aggrTypeInList = space->allocateAndCopyToAlignedSpace
	((char*)&aggrType, sizeof(aggrType), 0);
      listOfAggrTypes->insert(aggrTypeInList);

      HdfsColInfo hco(cnameInList, position, hiveType);
  ...
  Int16 dummy4;
  NABoolean dummy5;
  NABoolean parquetFullRead = TRUE;
  ...
  ComTdbExtStorageFastAggr *aggr_tdb = new(space) 
