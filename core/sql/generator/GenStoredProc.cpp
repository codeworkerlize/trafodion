...
//
//////////////////////////////////////////////////////////////////////
short generateSPIOExpr(RelInternalSP *sp, Generator *generator, ExSPInputOutput *&inputExpr,
  ...
  inputExpr = (ExSPInputOutput *)(genSpace->allocateAndCopyToAlignedSpace((char *)expr, totalLen, 0));
  ...
  if (expGen->processValIdList(sp->getTableDesc()->getColumnList(), ExpTupleDesc::SQLARK_EXPLODED_FORMAT, recordLen, 0,
                               1, &tupleDesc, ExpTupleDesc::LONG_FORMAT) == -1)
    return -1;

  cia =
      (ConvInstruction *)space->allocateMemory(sp->getTableDesc()->getColumnList().entries() * sizeof(ConvInstruction));
  ...
  for (short i = 0; i < (short)tupleDesc->numAttrs(); i++) {
    ...
    cia[i] = tempClause.findInstruction(REC_BYTE_V_ASCII,
                                        -1,  // no op
                                        tupleDesc->getAttr(i)->getDatatype(), tupleDesc->getAttr(i)->getLength(), 0);
  }
  ...
  space->makeContiguous((char *)expr, totalLen);
  outputExpr = (ExSPInputOutput *)(genSpace->allocateAndCopyToAlignedSpace((char *)expr, totalLen, 0));
...
short RelInternalSP::codeGen(Generator *generator) {
  ...
  for (CollIndex i = 0; i < procTypes().entries(); i++) {
    ...
    if ((procTypes())[i].getType().getVarLenHdrSize() > 0) {
      ...
      const CharType &char_type = (CharType &)((procTypes())[i].getType());
      ...
      // blankpad. TBD.
      //
      cn = new (generator->wHeap()) Cast(cn, &((procTypes())[i].getType()));
    } else
      cn = new (generator->wHeap()) Cast((getProcAllParamsVids())[i].getItemExpr(), &((procTypes())[i].getType()));
  ...
  exp_gen->generateContiguousMoveExpr(getTableDesc()->getColumnList(), -1 /*add conv nodes*/, 0,
                                      ...
                                      outputRowlen_, &output_expr, &output_tuple_desc, ExpTupleDesc::LONG_FORMAT,
                                      &returnedMapTable);
  ...
  generateSPIOExpr(this, generator, extractInputExpr, moveOutputExpr);
  ...
  char *sp_name = space->allocateAndCopyToAlignedSpace(procNameAsNAString, procNameAsNAString.length(), 0);
  ...
  ComTdbStoredProc *sp_tdb = new (space)
      ComTdbStoredProc(sp_name, input_expr, inputRowlen_, output_expr, outputRowlen_, work_cri_desc, work_atp_index,
                       given_desc, returned_desc, extractInputExpr, moveOutputExpr, 2, 1024,
                       (Cardinality)getGroupAttr()->getOutputLogPropList()[0]->getResultCardinality().value(), 5,
                       64000,  // 10240
                       predExpr, (UInt16)arkcmpInfo_);

  generator->initTdbFields(sp_tdb);

  if (!generator->explainDisabled()) {
    generator->setExplainTuple(addExplainInfo(sp_tdb, 0, 0, generator));
  }
