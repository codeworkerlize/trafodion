...
static NABoolean isAMixedExprListFunc(const ValueIdList & listOfKeyValues)
{
  ...
  for (CollIndex keyNum = 0; keyNum < keyCount; keyNum++)
  {
    ...
    if (ie &&
        ...
        (((ConstValue *)ie)->isMinOrMax()))
      isFisrtMeet = FALSE;
    else
    {
      if (!isFisrtMeet)
        isAMixedExprList = TRUE;
    }
  }
  return isAMixedExprList;
}
static NABoolean generateKeyExpr(const ValueIdSet & externalInputs,
			    const ValueIdList & listOfKeyColumns,
			    const ValueIdList & listOfKeyValues,
                            ValueIdList & listOfKeyExpr,
			    Generator* generator,
                            const ValueIdList & listOfRowidCols,
                            const ValueIdList & listOfRowidValues,
      ...
      for (CollIndex keyNum = 0; keyNum < keyCount; keyNum++)
        {
          ...
          ItemExpr *ieKeyVal = tempItem->replaceVEGExpressions(externalInputs, externalInputs,
			      ...
			      ValueIdSet &pulledNewInputs)
{
  ...
  NABoolean staticHiveData = 
      (CmpCommon::getDefault(HIVE_DATA_MOD_CHECK) == DF_OFF);
  NABoolean hasTMUDFsOrLRU = (getGroupAttr()->getNumTMUDFs() > 0 ||
                              containsLRU());

  NABoolean canAdjustDoP = (staticHiveData && 
                            getReqdShape() == NULL &&  
                            !hasTMUDFsOrLRU);
  ...
  if ( canAdjustDoP ) {
     ...
     Lng32 dopReduction = 
           (ActiveSchemaDB()->getDefaults()).getAsLong(DOP_ADJUST);
     ...
     switch ( dopReduction ) {
       ...
       case 2: 
       {
        ...
        if ( queryHash != 0 ) {
          ...
          // get the hdfs host and port to read RT stats 
          NABoolean readStatus = getHDFSDefaultName(host, port);
   
          if ( readStatus ) {
   
             NAString path;
             NABoolean useLocalFile = RelExpr::findRtStatsStorePath(path);

             if ( useLocalFile ) {
                // Do the read from a debug file
                readStatus = readRTStats((char *)path.data(), rtStats);

                if ( out ) 
                   *out << "file=" << path.data() << endl;
                
             } else {

                char buf[100];
                sprintf(buf, "act_data.%lu", queryHash);

                path.append(buf);

                readStatus = readRTStats((char *)host.data(), port, (char *)path.data(), rtStats);
   
                if ( out ) {
                   *out << "hdfs host=" << host << ", port=" << port;
                   *out << ", path=" << path << endl;
                }
             }
          }
          ...
          if ( out ) {
            *out << "readStatus=" << readStatus << endl;
          }
   
          if ( readStatus ) 
           {
             // If the read is successful, assign the actual data
             // to each node in the current plan, starting with a
             // traversal order of 1. The order is the same as the
             // operator number seen in the explain output.
             Int32 order = 1;
             NABoolean assignStatus = assignRTStats(rtStats, order);
   
             if ( out ) 
               *out << "assignStatus=" << assignStatus << endl;
   
             if ( assignStatus ) 
             {
                // The assignment of the actual data to nodes is 
                // successful. Now do the real work for reduction.
                pcgEspFragment topFragment(this, CmpCommon::statementHeap());
                generator->setCurrentEspFragmentPCG(&topFragment);
   
                // Compute the total actual data for each fragment
                prepareDopReduction(generator, TRUE /*use run-time stats*/);
   
                // Recursively compute the new dop for each fragment.
                topFragment.computeDesirableDoP(TRUE /*isRoot*/); 
   
                // Recursively modify the new dop by altering the 
                // partitioning functions.
                topFragment.performDopReduction();
              } 
           } else
...
short DDLExpr::ddlXnsInfo(NABoolean &isDDLxn, NABoolean &xnCanBeStarted)
{
  ...
  { 
    if ((dropHbase()) ||
        ...
        (dropRepos()) ||
        (upgradeRepos()) ||
        (createLobMD()) || (dropLobMD()) ||
        (addSchemaObjects()) ||
        (castToRelDumpLoad()) ||
        (castToRelGenLoadQueryCache()) ||
        (castToRelBackupRestore()) ||
        (loadTrafMetadataInCache()) ||
        (isTrafSharedCacheUpdateOp()) ||
  ...
  else  // When the DDL transaction is turned on
  {
     ...
     if (purgedata() || upgradeRepos() ||
         dropHbase()  || upgradeNamespace() ||
         createLobMD() || dropLobMD() ||
         dropTenant())
       // transaction will be started and commited in called methods.
       xnCanBeStarted = FALSE;
     
     if ((castToRelBackupRestore()) ||
         (castToRelDumpLoad()) ||
         (castToRelGenLoadQueryCache()) ||
         (alterTrafMetadataSharedCache()) ||
         (alterTrafDataSharedCache()) ||
         (loadTrafMetadataInCache()) ||
         (loadTrafMetadataIntoSharedCache())||
     ...
     if ((ddlNode && ddlNode->castToStmtDDLNode() &&
          ddlNode->castToStmtDDLNode()->ddlXns()) &&
            ((ddlNode->getOperatorType() == DDL_CLEANUP_OBJECTS) ||
             ...
             (ddlNode->getOperatorType() == DDL_REGISTER_TENANT) ||
             (ddlNode->getOperatorType() == DDL_UNREGISTER_TENANT) ||
			      ...
			      ValueIdSet &pulledNewInputs)
{
  ...
  if (castToRelDumpLoad()||castToRelGenLoadQueryCache())
    generator->setAqrEnabled(FALSE);
                                 ...
                                 ValueIdSet &pulledNewInputs)
{
  ...
  if ( !isAntiSemiJoin() && isProbeCacheApplicable(
			       ...
			       ValueIdSet &pulledNewInputs)
{
  ...
  if (useMinMaxOpt) {
    ...
    const PartitioningFunction *pf1 =
      child(1)->getPhysicalProperty()->getPartitioningFunction();

    const PartitioningFunction *pf0 =
    ...
    hashTableIsNotPartitioned =
      (pf1==NULL || pf1->getCountOfPartitions() == 1 ||
       pf1->castToReplicateViaBroadcastPartitioningFunction());
	 ...
	 getEquiJoinPredicates().advance(valId)) {
      ...
      if (itemExpr->getOperatorType() == ITM_VEG_PREDICATE) {
        ...
        if (outerChildCO.contains(itemExpr->child(0).getValueId())) {
          ...
          // right val = left val
          mmType = itemExpr->child(0).getValueId().getType().newCopy(
               generator->wHeap());
      ...
      if(mmType) {
        ...
        ItemExpr *minValScan = new(generator->wHeap()) 
          HostVar(name,
                  mmType,
                  TRUE);
        ...
        ItemExpr *maxValScan = new(generator->wHeap())
          HostVar(name,
                  mmType,
                  TRUE);
        ...
        const ColStatDescList& child1ColList = child(1).
              outputLogProp((*GLOBAL_EMPTY_INPUT_LOGPROP))->colStats();

        ColStatsSharedPtr child1ColStats =
           child1ColList.getColStatsPtrForColumn(innerValId);
        ...
        Lng32 maxLength = (ActiveSchemaDB()->getDefaults()).
                     figureOutMaxLength((UInt32)(c1Uec.getValue()));

        SQLVarChar* rangeValueType = new (generator->wHeap())
                SQLVarChar(generator->wHeap(), maxLength);

        rangeValueType->setNullable(FALSE); // RS is never nullable

        ItemExpr *rangeValueScan = new(generator->wHeap())
          HostVar(name,
                  rangeValueType,
                  TRUE);
        ...
        NABoolean child1IsBroadcasting =
            pf1->isAReplicateViaBroadcastPartitioningFunction();
        ...
        if ( !vid2rowcountMap_.find(valId, rowsProducedByHJ) ) {
           ...
           const ColStatDescList& hjColList = getGroupAttr()->
               outputLogProp((*GLOBAL_EMPTY_INPUT_LOGPROP))->colStats();

           ColStatsSharedPtr hjColStats =
                hjColList.getColStatsPtrForPredicate(valId);

           if ( hjColStats != NULL )
              rowsProducedByHJ = hjColStats->getRowcount();
        }
        ...
        generator->addMinMaxVals(outerValId,
                                 innerValId,
                                 minValScan->getValueId(),
                                 maxValScan->getValueId(),
                                 rangeValueScan->getValueId(),
                                 c1Uec,
                                 hashTableIsNotPartitioned,
                                 child1IsBroadcasting,
                                 this,
                                 rowsProducedByHJ);
      }
    }
  ...
  for (int i = 0; i < tableColDescList.entries(); i++)
    {
      for (int j=0 ; j < innerColStatsList.entries() ; j++)
        if (innerColStatsList[j]->getVEGColumn().getItemExpr()->
            containsTheGivenValue(tableColDescList[i]->getVEGColumn()) )
	  {
   ...
   newStatsList.appendDeepCopy(innerColStatsList,
                             innerColStatsList.entries(),
                             innerScale, FALSE);

   // Compute input values
   const ValueIdSet & inputValues = 
       hj->getGroupAttr()->getCharacteristicInputs();
   ...
   // Do the real work
   CostScalar rowcount =
          newStatsList.estimateCardinality(initialRowcount,
                                           joinPreds,         /*in*/
                                           outerReferences,   /*in*/
                                           hj,
                                           NULL,   
...
void FileScan::processMinMaxKeysForPartitionAndStripeCols(
     ...
     ValueIdSet& pulledNewInputs,
     ValueIdSet& availableValues,
     NABoolean useExtPushDownPredicates,
     ValueIdSet &extReaderMinMaxPredicates)
{
  ...
  NABoolean rosRemoveMinMax = 
          (CmpCommon::getDefault(RANGE_OPTIMIZED_SCAN_REMOVE_MINMAX) == DF_ON);

  NABoolean callComputeRowcountForMinMaxHashJoin = 
          (CmpCommon::getDefault(COMP_BOOL_206) == DF_OFF);

  NABoolean doRangeValueOpt = generator->isMinmaxOptWithRangeOfValues();

  NADefaults &defs = ActiveSchemaDB()->getDefaults();
  double surviveFudgeFactor = 
         defs.getAsDouble(RANGE_OPTIMIZED_SCAN_SURVIVE_ROWS_FUDGE_FACTOR);

  float rosMaxScanSelectivity = 0.0;
  defs.getFloat(RANGE_OPTIMIZED_SCAN_SELECTIVITY, 
                          rosMaxScanSelectivity);

  // The row count of the table (this scan).
  CostScalar tableRowCount = getTableDesc()->
                               getNATable()->
                                  getOriginalRowCount();

  NABoolean debugRosSelectivity = 
                (getenv("DEBUG_ROS_SELECTIVITY") != NULL);
  ...
  for (CollIndex i = 0; mmKeyVec.nextUsed(i); i++) {
    ...
    if (partColMM || stripeColMM) {
          ...
          // An indicator about whether to apply ROS for this min/max column
          NABoolean doRangeValueOptForThisMM = doRangeValueOpt;

          CostScalar totalRowsToSurvive = 0;

          // Apply the compile-time selectivity-based ROS elimination
          // only to non partition columns. For partition columns, we 
          // will apply the ROS unconditionally. 
          if ( !partColMM && doRangeValueOptForThisMM ) {

            // Fetch the col stats for the outerValId
            ValueId outerValId = generator->getMinMaxKey(i);

            ColStatDescSharedPtr outerColStatsDesc =
               getIndexDesc()->getPrimaryTableDesc()->getTableColStats().
                  getColStatsDescPtrForColumn(outerValId);

            ColStatsSharedPtr outerColStats = outerColStatsDesc->getColStats();
  

            // Try to see if there are any local predicates. If so
            // we need to compute their impact so that we can estimate
            // the effect of the filtering better.
            const TableAnalysis * tableAnalysis = 
                  getIndexDesc()->getPrimaryTableDesc()->getTableAnalysis();
            ...
            if (tableAnalysis) {
              ...
              ValueIdSet outerValIdAsSet(outerValId);
              localPreds.removeUnCoveredExprs(outerValIdAsSet);
            }

            // Compute the row count for applying ROS filter
            // at this scan node. The cardinality is equal to
            // the row count of HJ without applying any local
            // predicates at the scan node.
            HashJoin* hj = (HashJoin*)generator->getCaller(i);

            CostScalar rowcountForMinMaxHJ = 
               ( callComputeRowcountForMinMaxHashJoin ) ?
                 computeRowcountForMinMaxHashJoin(hj)
                                :
                 generator->getRowsProducedByHJ(i); 

            CostScalar rowsFilteredByLocalPreds = 0;

            if (localPreds.entries() > 0) {

              // A local predicate exists. 
              
              // Find all referenced local base columns in the local preds.
              ValueIdSet localCols;
              localPreds.findAllReferencedBaseCols(localCols);

              // Collect all local columns.
              const ValueIdList& colList = getIndexDesc()
                                             ->getPrimaryTableDesc()
                                                ->getColumnList();

              // Intersect the two to have all columns of this
              // that appear in the local preds.
              ValueIdSet colSet(colList);
              ValueIdSet colSetOfThis = localCols.intersect(colSet);

              // If the set is not empty, find out the RC associated
              // with the first not-NULL colStatsPtr.
              if ( colSetOfThis.entries() > 0 )
              {
                 const ColStatDescList& colStatsDescListOfThis =
                    getGroupAttr()->
                      outputLogProp((*GLOBAL_EMPTY_INPUT_LOGPROP))->colStats();

                 for (ValueId x=colSetOfThis.init(); 
                                   colSetOfThis.next(x); 
                                      colSetOfThis.advance(x) ) 
                 {
                    ColStatsSharedPtr colStatsPtr = 
                      colStatsDescListOfThis.getColStatsPtrForColumn(x);
 
                    if ( colStatsPtr != NULL )
                    {
                       rowsFilteredByLocalPreds =
                             colStatsPtr->getRowcount();
                       break;
                    }
                 }
              }
           ...
           // Sum up the row count from the HJ due to the join predicate,
           // which is the same as ROS at the scan node, and the row 
           // count from the local predicate involving outerValId. This 
           // is the estimated # of values (on column outerValId) to 
           // be available at the scan operator. 
           //
           // Note that the row count from the HJ can be estimated
           // low when there exists a local predicate on non-join
           // column on the fact (outer) table side. It is quite 
           // hard to overcome this limitation since when the logic
           // property is estimated for join, the colStatsList used
           // for either child has already been influenced by the local
           // predicate.  In this situation, the expected 
           // row count to survive the filter will be low as well
           // and the filter probably will be disabled on the fly after
           // enough values survive the filtering.
           totalRowsToSurvive = 
               rowcountForMinMaxHJ + rowsFilteredByLocalPreds;

           // Cap rowsToFilter with the table row count
           totalRowsToSurvive = MINOF(totalRowsToSurvive, tableRowCount);

  
            // If the ratio of surving values over the total number of rows
            // in the table is > maxScanSelectivity, do not bother with 
            // this ROS filter.
            if ( tableRowCount > 0 &&
                 totalRowsToSurvive / tableRowCount > rosMaxScanSelectivity ) {
                doRangeValueOptForThisMM = FALSE;
            }

            if ( debugRosSelectivity ) {
              cout << "ROS filter RV" << i << endl
                      << "  rows(local preds, hj, total, table)=" 
                      << rowsFilteredByLocalPreds.getValue() 
                      << ", "
                      << rowcountForMinMaxHJ.getValue()
                      << ", "
                      << totalRowsToSurvive.getValue()
                      << ", "
                      << tableRowCount.getValue() << endl
                      << "  total/table=" << (totalRowsToSurvive / tableRowCount).getValue()
                      << ", threshold=" << rosMaxScanSelectivity;
            }
          } // not partition MM and doROS for this MM
          ...
          // Find the Exchange that will be the merging point
          // for this scan and add the host vars to the
          // Exchange. Later on in Exchange::codeGen(), 
          // we generate a merge expression for it.

          if ( doRangeValueOptForThisMM ) 
           {
             NABoolean espSeen = FALSE;
             NABoolean innerIsOK = FALSE;
             Exchange* mergePoint = 
                  findMergePointer(generator->getCaller(i), 
                                   col->getValueId(),
                                   generator->isChild1Broadcasting(i), 
                                   espSeen, innerIsOK);
   
             if ( mergePoint ) {
                // Exchange seen from the scan to the 
                // HJ and the inner is OK. Definitely allow it. 
                if ( innerIsOK ) {
                  ScanFilterInput input(
                         generator->getMinVal(i),
                         generator->getMaxVal(i),
                         generator->getRangeOfValueVal(i)
                                       );
                  mergePoint->addScanFilterInput(input);
                } else
                  doRangeValueOptForThisMM = FALSE; // inner is bad. does not allow it.
             }
             else if ( !espSeen ) {
                // No exchange seen from the scan to the 
                // HJ. This is a type-2 HJ. Allow it only
                // if the inner is OK.
                if ( !innerIsOK ) {
                   doRangeValueOptForThisMM = FALSE; // inner is bad. does not allow it.
                }
             } else {
                // The exchange is seen, but does not 
                // meet the requirement. Proceeed to process
                // next host var and will not mark the hostvar 
                // useful.
                continue;
             }
          }
                   
          // If the above logic decides not to do ROS, and the CQD 
          // RANGE_OPTIMIZED_SCAN_REMOVE_MINMAX is ON, we will elminiate
          // the min/max+ROS completely. Otherwise, we will keep min/max 
          // only.
          if ( !doRangeValueOptForThisMM && rosRemoveMinMax ) {
            continue;
          }
          ...
          ItemExpr* rvExpr = 
               generator->getRangeOfValueVal(i).getItemExpr();

          ItemExpr *rangesPred = new (gheap) RangeCompare(
               col,
               rvExpr,
               rvExpr->getFilterId()
               );
          ...
          rangesPred->synthTypeAndValueId();
          ...
          if (partColMM)
            {
              ...
              if ( doRangeValueOptForThisMM )
                 newPartPredicates += rangesPred->getValueId();
            }
          else if (stripeColMM)
            {
              extReaderMinMaxPredicates += minPred->getValueId();
              extReaderMinMaxPredicates += maxPred->getValueId();

              if ( doRangeValueOptForThisMM ) {
                 extReaderMinMaxPredicates += rangesPred->getValueId();
          ...
          if ( doRangeValueOptForThisMM ) {
             minMaxValues += generator->getRangeOfValueVal(i);

             // Set the expected # of rows to pass the filter. Used
             // by the run-time adaptive filtering logic.
             CostScalar thresholdForTotalRowsToSurvive = 
                MINOF(tableRowCount * rosMaxScanSelectivity, 
                      totalRowsToSurvive * surviveFudgeFactor);

             const PartitioningFunction* myPartFunc = getPartFunc();

             if ( myPartFunc && 
                  !(myPartFunc->
                    castToReplicateViaBroadcastPartitioningFunction())
                )
               // Normalize by # of partitions (or the dop of the scan)
               thresholdForTotalRowsToSurvive /= 
                  myPartFunc->getCountOfPartitions();

             generator->setOuterRowsToSurvive(i, thresholdForTotalRowsToSurvive);
             if ( debugRosSelectivity ) {
               cout << ", RT threshold=" 
                    << thresholdForTotalRowsToSurvive.getValue()
                    << ", to keep." << endl;
             }
          } else {
             if ( debugRosSelectivity ) {
                cout << ", to eliminate." << endl;
             }
          }
			       ...
			       ValueIdSet &pulledNewInputs)
{
  ...
  if (isHiveTable())
    generator->setHiveReadAccess(TRUE);
          ...
          CmpContextInfo::CMPCONTEXT_TYPE_META) )
  {
    ...
    if ( skey ) {
       ...
       if ( mkey )
         mkey->reportKeyAssignment(tname);
    }
  }
  ...
  if (getMdamKeyPtr() != NULL)
      ...
      if (!partKeyPredsHBase.isEmpty())
        {
          ...
          if (getSearchKey())
          {
            ...
            if (getSearchKey()->hasRowidPredicate())
	...
	(availableValues,
	 ...
	 replicatePredicates
      ...
      isMixedBeginKeyPred_ = generateKeyExpr(getGroupAttr()->getCharacteristicInputs(),
		      getIndexDesc()->getIndexKey(),
		      getSearchKey()->getBeginKeyValues(),
		      beginKeyPred_,
		      generator,
                      getSearchKey()->getRowidCols(),
                      getSearchKey()->getRowidValues(),
                      replicatePredicates);
      isMixedEndKeyPred_ = generateKeyExpr(getGroupAttr()->getCharacteristicInputs(),
		      getIndexDesc()->getIndexKey(),
    ...
    {
      ValueIdSet extPushdownPreds;
      ValueIdSet usedPreds;
      ...
      if (isHiveTable()) {
        ...
        NABoolean useExtPushDownPredicates =
          (((hTabStats->isOrcFile() && CmpCommon::getDefault(ORC_PRED_PUSHDOWN) == DF_ON)) ||
           ((hTabStats->isParquetFile() && CmpCommon::getDefault(PARQUET_PRED_PUSHDOWN) == DF_ON)));

        // currently no pred pushdown if composite columns
        if (getTableDesc()->getNATable()->hasCompositeColumns())
         useExtPushDownPredicates = FALSE;
        ...
        if (useExtPushDownPredicates) {
          ...
          extReaderMinMaxPredicates.replaceVEGExpressions (
               availableValues,
               getGroupAttr()->getCharacteristicInputs(),
               FALSE, // no need for key predicate generation here
               &vegPairs, // to be side-affected
          ...
          locals += extReaderMinMaxPredicates;
          
          locals -= hiveSearchKey_->getPartAndVirtColPreds();
   
          hiveSearchKey_->makeHiveExtPushdownPredicates(
               locals, 
               getGroupAttr()->getCharacteristicInputs(), 
               getIndexDesc(), 
               extPushdownPreds,
          ...
          extPushdownPreds.generatePushdownListForExtStorage(extListOfPPI_);
          ...
          if ( hTabStats->isParquetFile() )
          {
            ...
            {
              NABoolean cantPushdown = 
                extListOfPPI_.validatePushdownForParquet();
              ...
              if (cantPushdown)
                ...
                {
                 if ( useCppReader )
                   useCppReader = extListOfPPI_.
                           validateOperatorsForParquetCppReader();
               
                  selectionPred() -= usedPreds;
                  parquetPushdownPreds() = usedPreds;
                }
            } 

            setUseParquetCppReader(useCppReader);
          }
            

          if ((hTabStats->isOrcFile()) &&
              (extListOfPPI_.entries() > 0))
            {
              NABoolean cantPushdown = 
                extListOfPPI_.validatePushdownForOrc();
        ...
        // We have to duplicate the push-down predicate in 
        // executor predicate for ORC, since ORC reader can return
        // a superset of the rows satisfying the push-down predicates.
        if ( isHiveOrcTable() )
        {
          extReaderMinMaxPredicates.removeBitMapInPredicate();
          selectionPred() += extReaderMinMaxPredicates;
        }
  ...
  // check if this scan node would be strawscan enabled to count and disable straw scan if there is too many
  if (CmpCommon::getDefault(HIVE_STRAW_SCAN) == DF_ON && //don't bother computing nb of strawscan nodes if strawscan is not enabled
      isHiveExtTable()){ 								 // or if not orc, parquet or avro hive table
   // CollIndex cix = generator->getFragmentDir()->getCurrentId();
    ...
    NABoolean isStrawScan;
    isStrawScan =  getProbes()<=1 //the probe check is to disable strawScan for NJ
        // && fragtype == FragmentDir::ESP //disable strawscan on MASTER scans
         && getPhysicalProperty()->getPartitioningFunction() != NULL; // disable strawscan for DOP 1;

    if (CmpCommon::getDefault(COMP_BOOL_171) == DF_OFF &&
        !(hiveSearchKey_
          && !hiveSearchKey_->getPartAndVirtColPreds().isEmpty()) //disable strawscan if no runtime part elimination
	   )isStrawScan = FALSE;
    if (isStrawScan) generator->incNbStrawScans(); // increment strawscan node counter
  }
  ...
  if (isHbaseTable() || isSeabaseTable()) {
     ...
     switch (beginTransForSelect) {
        case 2: 
           if (accessType != TransMode::SKIP_CONFLICT_ACCESS_
             && accessType != TransMode::READ_UNCOMMITTED_ACCESS_)
				    ...
				    ValueIdSet &pulledNewInputs)
{
 ...
 if ((((NATable*)getTableDesc()->getNATable())->getTableType() == ExtendedQualName::GHOST_TABLE) &&
  ...
  if ((getTableDesc()->getNATable()->getNumTrafReplicas() > 0) &&
      (generator->getNumTrafReplicas() == 0))
   {
				  ...
				  ValueIdSet &pulledNewInputs)
{
  ...
  if ((csl()) && (getTableDesc()->getNATable()->isSeabaseTable()))
    {
      ...
      ConstStringList * newCSL = new(generator->wHeap()) 
        ConstStringList(generator->wHeap());
  ...
  NABoolean incrBackupEnabled = (getTableDesc()->getNATable()->incrBackupEnabled() && !withNoReplicate() && !skipCheckIBInternal);

  // for update/delete using snapshot, skip write mutation file
  if (skipCheckIBInternal)
    generator->setSkipWriteMutation(TRUE);
  ...
  if (getTableDesc()->getNATable()->isMonarch())
    ...
    {
      if ((incrBackupEnabled) &&
          (CmpCommon::getDefault(TRAF_NON_XN_INCR_BACKUP_OPERATION) == DF_OFF))
        {
      ...
      noDTMxn() = TRUE;
           ...
           (NOT generator->getTransMode()->xnInProgress()) &&
           (NOT generator->savepointEnabled()) &&
  ...
  else
    {
      ...
      if ((NOT generator->updAbortOnError()) &&
          (generator->savepointEnabled()))
        {
          generator->setUpdErrorOnError(FALSE);
        }
      else
  ...
  if (getTableDesc()->getNATable()->isHbaseMapTable())
    {
      *CmpCommon::diags() << DgSqlCode(-1425)
			  << DgTableName(getTableDesc()->getNATable()->getTableName().
					 getQualifiedNameAsAnsiString())
                          << DgString0("Reason: HBase mapped table update should have been transformed to delete + insert.");
      GenExit();
    }
  ...
  if (isUnique)
	...
	generator->setMayNotCancel(TRUE);
	  ...
	  ((executorPred().isEmpty()) ||
           ((CmpCommon::getDefault(HBASE_CHECK_AND_UPDEL_OPT) == DF_ON) &&
            (NOT isAlignedFormat) &&
            (Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL)))) &&
	  (NOT needToGetCols) &&
	  (NOT inlinedActions) &&
                              ...
                              ValueIdSet &pulledNewInputs)
{
  ...
  if (getTableDesc()->getNATable()->isMonarch())
  ...
  else if ((isUpsert()) && (insertType_ == UPSERT_LOAD))
    {
      ...
      // lob 'upsert using load' need to run as inserts without dtm xn.
      if (getTableDesc()->getNATable()->hasLobColumn())
        setInsertType(Insert::SIMPLE_INSERT);
...
void GroupByAgg::decideFeasibleToTransformForAggrPushdown(NABoolean checkAll)
{
  ...
  if (childRelExpr && 
      (NOT (childRelExpr->getOperatorType() == REL_FIRST_N)) &&
      ((childRelExpr->getOperatorType() == REL_SCAN) ||
       (childRelExpr->getOperatorType() == REL_FILE_SCAN) ||
       (childRelExpr->getOperatorType() == REL_HBASE_ACCESS)))
    {
      scan = (Scan *)childRelExpr;
      if ((scan->getOperatorType() == REL_FILE_SCAN) ||
      ...
      const HivePartitionAndBucketKey *hiveSearchKey = NULL;

      if (fileScan)
        hiveSearchKey = fileScan->getHiveSearchKey();

      if ((NOT aggregateExpr().isEmpty()) &&
          (groupExpr().isEmpty()) &&
          (!fileScan || fileScan->executorPred().isEmpty()) &&
          ...
          !scan->getTableName().isPartitionRangeSpecified())
        {
          if(isOrc)
          {
             //FOR ORC, postpone the push down decision to preCodeGen phase
             //Even there is predicate, if it is only using the parition column, we should still push down
             if( !checkAll && 
                 (CmpCommon::getDefault(ORC_PARTITION_ONLY_PUSHDOWN) == DF_ON)) {
                 feasibleToPushdownAggr_ = GroupByAgg::TVL_MAYBE;
                 return;
             } else
             {
               if (NOT scan->userSpecifiedPred() &&
  ...
  if (aggrPushdown)
    {      
      if (NOT (((naTable->getObjectType() == COM_BASE_TABLE_OBJECT) ||
               ...
               (((naTable->isSeabaseTable()) && (NOT naTable->isHbaseMapTable())) || 
                (isExt))))
        {
          aggrPushdown = FALSE;
        }
          ...
          if ((isSeabase) &&
              ...
              (ae->origOpType() == ITM_COUNT_STAR__ORIGINALLY) )
            {
              ...
              const ColStatDescList& csdl =
                scan->getTableDesc()->getTableColStats();
              ...
              if( (numRows / 1000000) < ActiveSchemaDB()->getDefaults().
                  getAsDouble(NUMBER_OF_COUNTSTAR_ROWS_PARALLEL_THRESHOLD))
                continue;
            }
          ...
          if (isExt)
            {
                ...
                case ITM_ORC_SUM_NV:
                  {
                    ...
                    if ( !nac ) 
                    {
                       ...
                       ItemExpr* ie = 
                          ae->child(0)->getValueId().getItemExpr();
                       ...
                       if ( baseCols.entries() == 1 ) {
                          ...
                          nac = 
                           ((BaseColumn*)(baseCol.getItemExpr())) ->getNAColumn();
                       }
                    }
              ...
              if (aggrType.getTypeQualifier() == NA_BOOLEAN_TYPE)
                continue;
              ...
              if (((DFS2REC::isBinaryNumeric(aggrType.getFSDatatype())) ||
                   (DFS2REC::isBigNum(aggrType.getFSDatatype()))) &&
                                              ...
                                              ValueIdSet &pulledNewInputs)
{
  ...
  if ( !isNotAPartialGroupBy() && !isAPartialGroupByLeaf() )
    return this;
  ...
  if ( isFeasibleToTransformForAggrPushdown() == TVL_MAYBE )
     decideFeasibleToTransformForAggrPushdown(TRUE /*check all*/ );

  NABoolean aggrPushdown =  
       (isFeasibleToTransformForAggrPushdown() == TVL_TRUE);
  ...
  if (scan) {
     if(isOrc) 
     {
      const HivePartitionAndBucketKey *hiveSearchKey = scan->getHiveSearchKey();

      if( hiveSearchKey->getPartAndVirtColPreds().entries() == 0 && hiveSearchKey->getCompileTimePartColPreds().entries() == 0 )
        aggrPushdown = FALSE;

      if( scan->selectionPred().isEmpty() && (NOT scan->userSpecifiedPred())) 
        aggrPushdown = TRUE;
     }  
  } else {
     aggrPushdown = FALSE;
  ...
  // for mantis 13765
  if (NOT selectionPred().isEmpty() && NOT isOrc)
    aggrPushdown = FALSE;
  ...
  if (childRelExpr->getOperatorType() == REL_HBASE_ACCESS)
  {
    ...
    if ((c->getListOfRangeRows().isEmpty()) ||
      (c->getHbaseSearchKeys().isEmpty()) ||
    ...
    {
      if (c->getMaxCardEst().getValue() <
        ActiveSchemaDB()->getDefaults().getAsLong(AGGR_PUSHDOWN_THRESHOLD))
        aggrPushdown = FALSE;
    }
      ...
      if (isExt)
        {
              ...
              if (origAgg->getOperatorType() == ITM_COUNT_NONULL)
                {
                  ...
                  extAggrType = 
                    new(generator->wHeap()) SQLLargeInt(generator->wHeap(),
                                                        TRUE,
                                                        FALSE);
                }
                      ...
                      if (nType->isExact())
                        {
                          ...
                          NABoolean nullable = 
                            (origAgg->getOperatorType() != ITM_COUNT);
                        ...
                        {
                          extAggrType = 
                            new(generator->wHeap()) SQLDoublePrecision(generator->wHeap(),
                                                                       TRUE);
                        }
                    }
                }
              else if (childType->getTypeQualifier() == NA_DATETIME_TYPE)
                {
                  ...
                  extAggrType =
                    new(generator->wHeap()) SQLChar(generator->wHeap(), dType->getDisplayLength(), TRUE);
                }
            }
          ...
          ItemExpr * extAggrArg = 
            new (generator->wHeap()) NATypeToItem((NAType*)extAggrType);
  ...
  if (isSeabase)
    {
      ...
      if (childRelExpr->getOperatorType() == REL_HBASE_ACCESS)
      {
        ...
        if(scan->getExecutorPredicates().isEmpty()
          && c->getListOfRangeRows()[0].beginRowId_.isNull()
				 ...
				 ValueIdSet &pulledNewInputs)
{
  ...
  RelExpr * apdExpr = 
    transformForAggrPushdown(generator, externalInputs, pulledNewInputs);
      ...
      if (upperValues[e].getItemExpr()->getOperatorType() == ITM_VEG_REFERENCE)
        enhancedUpperValues +=
          static_cast<VEGReference *>(
               upperValues[e].getItemExpr())->getVEG()->getAllValues();
    }
...
ItemExpr * BiRelat::preCodeGen(Generator * generator)
{
  ...
  if ((DFS2REC::isCharacterString(type1A.getFSDatatype())) &&
      (DFS2REC::isCharacterString(type2A.getFSDatatype())))
    {
	 ...
	 ItemExpr * newChild = NULL;
         ...
         if (tran_type != Translate::UNKNOWN_TRANSLATION &&
             isTrailingBlankSensitive &&
             atoi(isTrailingBlankSensitive) == 1 &&
             cType1A.getVarLenHdrSize() == 0 &&
             cType2A.getVarLenHdrSize() == 0) //Both are char
         {
           if (tran_type == Translate::ISO88591_TO_UTF8)
           {
             ...
             if (chld_to_trans == 0 && byte1 < byte2)
             {
               CharType *targetType = new(generator->wHeap())
                                      SQLChar(generator->wHeap(),
                                           CharLenInfo(0, byte2),
                                           cType1A.supportsSQLnull(),
                                           cType1A.isUpshifted(),
                                           cType1A.isCaseinsensitive(),
                                           FALSE,
                                           cType1A.getCharSet(),
                                           cType1A.getCollation(),
             ...
             else if (chld_to_trans == 0 && byte1 > byte2)
             {
               CharType *targetType = new(generator->wHeap())
                                      SQLChar(generator->wHeap(),
                                           CharLenInfo(0, byte1),
                                           cType2A.supportsSQLnull(),
                                           cType2A.isUpshifted(),
                                           cType2A.isCaseinsensitive(),
                                           FALSE,
                                           cType2A.getCharSet(),
                                           cType2A.getCollation(),
           ...
           else if (tran_type == Translate::UCS2_TO_UTF8 ||
                    ...
                    tran_type == Translate::ISO88591_TO_UNICODE)
           {
             ...
             if (chld_to_trans == 0 && char1 < char2)
             {
               CharType *targetType = new(generator->wHeap())
                                      SQLChar(generator->wHeap(),
                                           CharLenInfo(char2, char2 * cType1A.getBytesPerChar()),
                                           cType1A.supportsSQLnull(),
                                           cType1A.isUpshifted(),
                                           cType1A.isCaseinsensitive(),
                                           FALSE,
             ...
             else if (chld_to_trans == 1 && char1 > char2)
             {
               CharType *targetType = new(generator->wHeap())
                                      SQLChar(generator->wHeap(),
                                           CharLenInfo(char1, char1 * cType2A.getBytesPerChar()),
                                           cType2A.supportsSQLnull(),
                                           cType2A.isUpshifted(),
                                           cType2A.isCaseinsensitive(),
                                           FALSE,
...
ItemExpr * PivotGroup::preCodeGen(Generator * generator)
{
  ...
  for (Int32 i = 0; i < getArity(); i++)
  {
    ...
    if (childType.getTypeQualifier() != NA_CHARACTER_TYPE ||
        childType.getCharSet() != pivotType.getCharSet())
    {
      Lng32 displayLen = childType.getDisplayLength(
                                                  childType.getFSDatatype(),
                                                  childType.getNominalSize(),
                                                  childType.getPrecision(),
                                                  childType.getScale(),
                                                  0);

      CharLenInfo charLenInfo(
                         displayLen,
                         displayLen * CharInfo::minBytesPerChar(pivotType.getCharSet()));

      NAType * newType = new(generator->getBindWA()->wHeap())
                               SQLVarChar(generator->getBindWA()->wHeap(),
                                              charLenInfo,
                                              childType.supportsSQLnull(),
                                              FALSE, FALSE,
  ...
  if (getArity()==3)
    {
	  ...
	  child(2) = new (generator->wHeap())
	    Cast(child(2), new (generator->wHeap()) SQLInt(generator->wHeap(), TRUE,
							   typ.supportsSQLnullLogical()));
...
RelExpr * CallSP::preCodeGen(Generator * generator,
                               const ValueIdSet & externalInputs,
                               ValueIdSet & pulledNewInputs)
{
  if (nodeIsPreCodeGenned())
  ...
  if (!IsolatedNonTableUDR::preCodeGen(generator,externalInputs,pulledNewInputs))
    return NULL;
					     ...
					     ValueIdSet &pulledNewInputs)
{
  ...
  if (hiveNATable() && 
      (hiveNATable()->isORC() || hiveNATable()->isParquet()) &&
      (reqdOrder().entries() > 0))
    {
      Sort *sortNode = new(generator->wHeap()) Sort(child(0));
  ...
  if (isHiveInsert() && getIsPartitioned())
    {
      PartitioningFunction *partFunc =
        getPhysicalProperty()->getPartitioningFunction();

      if (partFunc &&
          partFunc->getCountOfPartitions() > 1 &&
          ActiveSchemaDB()->getDefaults().getAsLong(HIVE_PARTITION_INSERT_REPART) <= 0)
        concurrentHivePartitionWrites_ = TRUE;
    }
...
RelExpr * ExeUtilCompositeUnnest::preCodeGen(Generator * generator,
                                          const ValueIdSet & externalInputs,
                                          ValueIdSet &pulledNewInputs)
{
  if (nodeIsPreCodeGenned())
  ...
  colNameExpr_ = colNameExpr_->
    replaceVEGExpressions(availableValues,
                          getGroupAttr()->getCharacteristicInputs());

  addnlColsVIDList_.replaceVEGExpressions(
       availableValues,
       getGroupAttr()->getCharacteristicInputs());
				  ...
				  ValueIdSet &pulledNewInputs)
{
  ...
  if ( canDoMinMaxForNonType1 &&
        (NOT getSearchKey() || NOT getSearchKey()->hasRowidPredicate()) ) {
  ...
  if (getTableDesc()->getNATable()->isHbaseCellTable())
	...
	((QualifiedName*)&getTableDesc()->getNATable()->getTableName());
      ...
      for (Lng32 ij = 0; 
           (hbaseColNameSet && (ij < hbaseColNameSet->entries()));
            ij++)
	{
...
RelExpr * ConnectBy::preCodeGen(Generator * generator,
			     const ValueIdSet & externalInputs,
			     ValueIdSet &pulledNewInputs)
{
  if (nodeIsPreCodeGenned())
...
RelExpr * ConnectByTempTable::preCodeGen(Generator * generator,
                               const ValueIdSet & externalInputs,
                               ValueIdSet &pulledNewInputs)
{
  if (nodeIsPreCodeGenned())
...
RelExpr * QueryInvalidationFunc::preCodeGen(Generator * generator,
				     const ValueIdSet & externalInputs,
				     ValueIdSet &pulledNewInputs)
{
  if (nodeIsPreCodeGenned())
