...
short LOBinsert::codeGen(Generator * generator)
{
  ...
  if (obj_ == LOBoper::STRING_)
    ...
    {
      if (lobStorageType() != Lob_External)
        li->setFromLob(TRUE);
      else
  ...
  li->setSrcLobStorageLocation((char*)srcLobStorageLocation().data());
  li->setSrcLobV2(srcLobV2());
  ...
  li->setLobInlineLimit(getLobInlineLimit());
  li->setLobHdfsServer((char *)getLobHdfsServer().data());
  li->setLobHdfsPort(getLobHdfsPort());
  li->setNumLOBdatafiles(getNumLOBdatafiles());
  li->setSrcNumLOBdatafiles(getSrcNumLOBdatafiles());
  li->setLobV2(lobV2());

  li->setOptions(li->getSrcOptions(),
                 0, getLobMaxSize(), getLobMaxChunkMemSize(), getLobGCLimit(), 
                 getNumLOBdatafiles(), getSrcNumLOBdatafiles(), 
                 lobInlinedDataMaxLen(), lobHbaseDataMaxLen(),
                 lobDataInHbaseColLen(),
                 getSrcSchemaName().length(), (char*)getSrcSchemaName().data());
  li->setOptions(li->getOptions(),
                 0, getLobMaxSize(), getLobMaxChunkMemSize(), getLobGCLimit(), 
                 getNumLOBdatafiles(), getSrcNumLOBdatafiles(), 
                 lobInlinedDataMaxLen(), lobHbaseDataMaxLen(),
                 lobDataInHbaseColLen(),
                 insertedTableSchemaName().length(), 
                 (char*)insertedTableSchemaName().data());

  li->getOptions().setLobXnOper(CmpCommon::getDefault(TRAF_LOB_XN_OPER) == DF_ON);
...
short LOBdelete::codeGen(Generator * generator)
{
  ...
  ld->setOptions(ld->getOptions(),
                 0,0,0,0,0,0,0,0,0,
                 getSchemaName().length(), (char*)getSchemaName().data());
  
  ld->getOptions().setLobXnOper(CmpCommon::getDefault(TRAF_LOB_XN_OPER) == DF_ON);
...
short LOBupdate::codeGen(Generator * generator)
{
 ...
 if (append_)
  ...
  lu->setLobSize(lobSize());
  lu->setLobMaxSize(getLobMaxSize());
  lu->setLobInlineLimit(getLobInlineLimit());
  ...
  lu->setNumLOBdatafiles(getNumLOBdatafiles());
  lu->setLobV2(lobV2());

  lu->setOptions(lu->getOptions(),
                 0, getLobMaxSize(), getLobMaxChunkMemSize(), getLobGCLimit(), 
                 getNumLOBdatafiles(), getSrcNumLOBdatafiles(), 
                 lobInlinedDataMaxLen(), lobHbaseDataMaxLen(),
                 lobDataInHbaseColLen(), 
  ...
  lu->getOptions().setLobXnOper(CmpCommon::getDefault(TRAF_LOB_XN_OPER) == DF_ON);
...
short LOBconvertHandle::codeGen(Generator * generator)
{
  ...
  if (obj_ == LOB_)
    lch->setToLob(TRUE);
  else if ((obj_ == HANDLE_STRING_) || (obj_ == HANDLE_STRING_AS_CHAR_))
    lch->setToHandleString(TRUE);
  else if (obj_ == HANDLE_STRING_WITH_DATA_)
    lch->setToHandleStringWithData(TRUE);
    
  lch->lobNum() = lobNum();
  lch->setLobStorageType(lobStorageType());
  lch->setLobStorageLocation((char*)lobStorageLocation().data());
  lch->setLobHdfsServer((char *)getLobHdfsServer().data());
  lch->setLobHdfsPort(getLobHdfsPort());

  lch->setLobV2(lobV2());
  lch->setOptions(lch->getOptions(),
                  0,0,0,0,0,0,0,0,0,
                  getSchemaName().length(), (char*)getSchemaName().data());

  generator->getExpGenerator()->linkClause(this, lch);
...
short LOBconvert::codeGen(Generator * generator)
{
  ...
  lc->setNumLOBdatafiles(getNumLOBdatafiles());
  lc->setLobV2(lobV2());
  lc->setOptions(lc->getOptions(),
                 0, 0, 0, 0, getNumLOBdatafiles(), 0, 0, 0,0,
                 getSchemaName().length(), (char*)getSchemaName().data());

  lc->getOptions().position_ = getPosition();

  return 0;
}

short LOBlength::codeGen(Generator * generator)
{
  Attributes ** attr;
  ...
  ExpLOBlength * ll =
    new(generator->getSpace()) ExpLOBlength(getOperatorType(), 
                                            attr, 
                                            space);
  ...
  ll->setLobHdfsServer((char *)getLobHdfsServer().data());
  ll->setLobHdfsPort(getLobHdfsPort());
  ll->setNumLOBdatafiles(getNumLOBdatafiles());
  ll->setLobV2(lobV2());
  ll->setOptions(ll->getOptions(),
                 0, 0, 0, 0, getNumLOBdatafiles(), 0, 0, 0,0,
                 getSchemaName().length(), (char*)getSchemaName().data());
...
short SequenceValue::codeGen(Generator * generator)
{
  ...
  sv->setSkipWalForIncrColVal(CmpCommon::getDefault(TRAF_SKIP_WAL_FOR_ORDER_SEQ) == DF_ON);
