  ...
  else
  {
    ...
    if ( getScanFilterInputs().entries())
    {
      ...
      for (CollIndex i=0; i<inputs; i++) 
      {
            ...
            ItemExpr* minValExprSB = new(generator->wHeap()) 
                                      Aggregate(ITM_MIN, minVal.getItemExpr(), FALSE);
   
            ItemExpr* maxValExprSB = new(generator->wHeap()) 
                                      Aggregate(ITM_MAX, maxVal.getItemExpr(), FALSE);
   
            ItemExpr* rangeValueExprSB = new(generator->wHeap()) 
                                      Aggregate(ITM_RANGE_VALUES_MERGE, 
                                                rangeValueVal.getItemExpr(), FALSE);
            ...
            if ( generator->isMinmaxOptWithRangeOfValues() &&
                 generator->getMapInfoAsIs(rangeValueVal) ) {
      ...
      Attributes** offsets =  
           new(space) Attributes * [mmPairs.entries()];
...
//generator->reportMappingInfo(cout, mmPairs, "Exchange, before gen mmMove:");
      ...
      splitBottomWorkCriDesc->setTupleDescriptor(
                                    minMaxValsWorkAtpIndex,
                                    minMaxTupleDescriptor
                                                );
    ...
    if ((numTopEsps > 1) && (NOT isAnESPAccess()))
    {
      ...
      if (topPartExpr)
      {
        ...
        if (topPartExpr->getOperatorType() == ITM_HASH2_DISTRIB &&
          ...
          {
            RandomNum *randomNum = static_cast<RandomNum *>(
                 topPartExpr->child(0)->child(0)->child(0).getPtr());
            ...
            if (randomNum->isSimple())
              {
                ...
                topPartExpr = new(generator->wHeap()) Modulus(
                     randomNum,
                     new(generator->wHeap())
                        ConstValue(topPartFunc->getCountOfPartitions(),
                                   (NAMemory *) generator->wHeap()));
