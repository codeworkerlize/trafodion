...
//
//////////////////////////////////////////////////////////////////////
LmMetaContainer::LmMetaContainer(const char *path, LmContainerManager *contManager, ComDiagsArea *diagsArea)
    : path_(path, collHeap()), contManager_(contManager), isDir_(FALSE) {
  loader_ = getLM()->createLoader(getPath(), diagsArea);
  if (isDirectory(getPath())) isDir_ = TRUE;
}

LmMetaContainer::~LmMetaContainer() { getLM()->deleteLoader(loader_); }
...
//
//////////////////////////////////////////////////////////////////////
LmContainer::LmContainer(const char *name, LmHandle handle, const char *fileSystemName)
    : name_(name, collHeap()), handle_(handle), fileSystemName_(fileSystemName, collHeap()), fileModTime_(-1) {}
...
NABoolean LmContainer::isContainerCurrent(ComDiagsArea *diagsArea) const {
  time_t new_modified_time = getModifiedTime(getFileSystemName(), diagsArea);

  if (new_modified_time == -1) return FALSE;

  if (new_modified_time != 0 && new_modified_time != getFileModTime()) return FALSE;
...
//
//////////////////////////////////////////////////////////////////////
LmContainerMC::LmContainerMC(const char *name, LmHandle handle, LmMetaContainer *owner, const char *fileSystemName)
    : LmContainer(name, handle, fileSystemName), owner_(owner) {}
...
//
//////////////////////////////////////////////////////////////////////
LmContainerManagerCache::LmContainerManagerCache(LmLanguageManager *lm, ComUInt32 capacity, ComBoolean enforceLimits,
                                                 ComDiagsArea *diagsArea, ComUInt32 hashTableSize)
...
LmContainerManagerCache::~LmContainerManagerCache() {
  ...
  while ((mc = (LmMetaContainerCache *)metaContainers_->getNext()) != NULL) delete mc;
...
//////////////////////////////////////////////////////////////////////
LmResult LmContainerManagerCache::getContainer(const char *containerName, const char *externalPath,
  ...
  if (metaContainer == NULL) {
    if (curSize_ > maxSize_) {
      ...
      LM_DEBUG1("***   Cache limits are currently %sbeing enforced", (enforceLimits_ ? "" : "NOT"));

      if (enforceLimits_) {
        // No space for another MC. Right now we return LM out of memory
        // error. But we need to give control over cache to user.
        *diagsArea << DgSqlCode(-LME_OUT_OF_MEMORY) << DgString0(". Could not create metaContainer.");
        return LM_ERR;
      }
    }
    // Create a new MC
    metaContainer = addMetaContainer(externalPath, diagsArea);
    if (metaContainer == NULL) return LM_ERR;
  }
  ...
  if (*container != NULL) {
    ...
    if (result == FALSE) {
      ...
      *container = NULL;
      ...
      getContainer(containerName, externalPath, container, diagsArea);
    }
  }

  // Exclude the following lines for coverage as caching is not used.
  // Check the cache as required.
  if (updates_ % checkPeriod_ == 0) checkCache();
...
//////////////////////////////////////////////////////////////////////
LmMetaContainer *LmContainerManagerCache::addMetaContainer(const char *externalPath, ComDiagsArea *diagsArea) {
  LmMetaContainerCache *mc;
  ...
  // Create the MC.
  mc = new (collHeap()) LmMetaContainerCache(externalPath, this, diagsArea);
...
////////////////////////////////////////////////////////////////////////
NABoolean LmContainerManagerCache::deleteMetaContainer(LmMetaContainer *metaCont) {
  if (metaCont != NULL) {
...
LmMetaContainer *LmContainerManagerCache::getMetaContainer(const char *path) {
  ...
  while ((mc = (LmMetaContainerCache *)metaContainers_->getNext()) != NULL) {
    if (str_cmp(path, mc->getPath(), len) == 0) break;
  }
...
void LmContainerManagerCache::cleanCache(ComBoolean purge) {
  ...
  for (Int32 i = 0; i < attempts && curSize_ > midWater_; i++) {
    ...
    if (mr) {
      metaContainers_->position((char *)mr->getPath(), str_len(mr->getPath()));
  ...
  if (purge_ && !purge) cleanCache(purge_);
}
...
void LmContainerManagerCache::decayCache() {
  ...
  d = (curSize_ <= maxSize_) ? ((float)1 - (float)curSize_ / (float)maxSize_) : 0;

  while ((mc = (LmMetaContainerCache *)metaContainers_->getNext()) != NULL) mc->decay(d);
}
...
//
//////////////////////////////////////////////////////////////////////
LmMetaContainerCache::LmMetaContainerCache(const char *path, LmContainerManager *contManager, ComDiagsArea *diagsArea)
    : LmMetaContainer(path, contManager, diagsArea), capacity_(0), hits_(0), misses_(0), refcnt_(0), usecnt_(0) {
...
LmContainer *LmMetaContainerCache::getContainer(const char *containerName, ComDiagsArea *diagsArea) {
  ...
  h = getLM()->loadContainer(containerName, getPath(), loader_, &size, diagsArea);
  if (h == 0) {
  ...
  c = new (collHeap()) LmContainerMC(containerName, h, this, nameToCheck.data());
  if (!c) {
    *diagsArea << DgSqlCode(-LME_OUT_OF_MEMORY) << DgString0(". Could not create container.");
    return NULL;
  }
...
//
/////////////////////////////////////////////////////////////////////
LmContainerSimple::LmContainerSimple(const char *name, LmHandle handle, const char *fileSystemName)
    : LmContainer(name, handle, fileSystemName), refCount_(0) {}
...
//
/////////////////////////////////////////////////////////////////////
LmContainerManagerSimple::LmContainerManagerSimple(LmLanguageManager *lm) : LmContainerManager(lm) {
...
LmResult LmContainerManagerSimple::getContainer(const char *containerName, const char *externalPath,
                                                LmContainer **container, ComDiagsArea *diagsArea) {
  LM_ASSERT(container);
  *container = NULL;
  ...
  if (lmc == NULL) {
    ...
    if (modTime == -1) result = LM_ERR;

    if (result == LM_OK) {
      // Load the library
      h = getLM()->loadContainer(containerName, externalPath, NULL, NULL, diagsArea);
      if (h == 0) result = LM_ERR;
    }
    ...
    if (simple->getRefCount() > 0) {
      ...
      if (isCurrent) {
        ...
        // check.
        if (diagsArea->getNumber(DgSqlCode::ERROR_) > 0) result = LM_ERR;
        ...
        if (result == LM_OK) {
          ...
          delete lmc;
          lmc = NULL;
          result = getContainer(containerName, externalPath, &lmc, diagsArea);
        }
      }
