...
//
LmJavaType::LmJavaType(LmParameter *lmParam) : lmParam_(lmParam), type_(JT_NONE), index_(LM_JAVATYPE_INVALID_INDEX) {
  if (lmParam_ == NULL) {
    type_ = JT_VOID;
    ...
    switch (data_type) {
      ...
      default:
        type_ = (Type)(lmParam_->fsType() + (lmParam_->prec() == 0 ? 0 : lmParam_->prec() - 1));
        break;
    }
  }
...
//
const char *LmJavaType::getJavaTypeName(ComUInt32 &len) const {
...
//         FALSE if not
//
ComBoolean LmJavaType::isJavaTypeObject() const {
  switch (type_) {
    case JT_LANG_STRING:
    case JT_MATH_BIGDEC:
    case JT_SQL_DATE:
...
// this instance.
//
LmJavaType::TypeElement *LmJavaType::getTypeElement() const {
  if (index_ != LM_JAVATYPE_INVALID_INDEX)
