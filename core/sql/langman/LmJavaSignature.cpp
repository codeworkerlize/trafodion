...
LmResult LmJavaSignature::createSig(ComFSDataType paramType[], ComUInt32 paramSubType[], ComColumnDirection direction[],
  ...
  static const int resultSetLen = str_len(resultSet);
  ...
  if (isUdrForJavaMain) {
    ...
    idx += 19;
  } else if (((int)numParam == 0) && optype && (numResultSets > 0)) {
  ...
  for (int i = 1; i <= (int)numParam; i++) {
    ...
    if (optionalSig) {
      ...
      if (optc != NULL) {
        *optc = '\0';
        optype = strip_spaces(optype);
        if ((i == (int)numParam) && (numResultSets == 0)) {
          // Case: Params specified in opt sig are more than expected
          *da << DgSqlCode(-LME_SIGNATURE_INVALID2) << DgInt0((int)numParam);
          if (optsig) NADELETEBASIC(optsig, heap_);
          return LM_ERR;
      ...
      } else  // first if (optc != NULL)
      {
        ...
        if (optc != NULL) {
          *optc = '\0';
          optype = strip_spaces(optype);
          if ((i < (int)numParam) || ((numParam == 1) && !(*optype))) {
...
// generated by us.
//
int LmJavaSignature::unpackSignature(char *unpackedSignature) {
...
//
int LmJavaSignature::getUnpackedSignatureSize() {
