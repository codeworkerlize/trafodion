...
int unpackSignature(const char *encodedSignature /*input*/, char *unpackedSignature /*input and output*/) {
  int outparam = 0;
  const char *encodedSig = encodedSignature;
  char *out = unpackedSignature;
  int retVal = 0;

  if (*encodedSig != '(') return -1;
...
int getUnpackedSignatureSize(const char *encodedSignature, int *paramsInSig) {
  if (encodedSignature == NULL) return 0;
  ...
  while (*signature) {
    switch (*signature) {
      ...
      case ')': {
        ...
        if (paramsInSig != NULL) *paramsInSig = numParams;
