...
LmResult LmParameter::setOutNumeric(void *dataPtr, const char *source, ComBoolean copyBinary, CollHeap *heap,
                                    ComDiagsArea *diags) {
  ...
  if (copyBinary) {
    ...
    if (!callerWantsDiags && diags) diags->decrRefCount();
  } else {
...
LmResult LmParameter::setOutChar(void *dataPtr, const char *src, ComUInt32 lengthInBytes) {
  LmResult result = LM_OK;
...
LmResult LmParameter::setOutDecimal(void *dataPtr, const char *source, CollHeap *heap, ComDiagsArea *diags) {
  LmResult result = LM_OK;
...
LmResult LmParameter::setOutDate(void *dataPtr, const char *a) { return setOutChar(dataPtr, a, str_len(a)); }
...
LmResult LmParameter::setOutTime(void *dataPtr, const char *val) {
  ...
  if (outSize_ > valLen) *((char *)dataPtr + outDataOffset() + valLen) = '.';

  for (ComUInt32 index = valLen + 1; index < outSize_; index++) *((char *)dataPtr + outDataOffset() + index) = '0';
...
LmResult LmParameter::setOutTimestamp(void *dataPtr, const char *a) {
  ...
  if (valLen > outSize_) {
    ...
    for (ComUInt32 index = valLen; index < outSize_; index++) *((char *)dataPtr + outDataOffset() + index) = '0';

    return LM_OK;
  }
}

LmResult LmParameter::setOutInterval(void *dataPtr, const char *rawBytes, ComUInt32 len) {
  LmResult result = LM_OK;
...
#ifdef LANGMAN
  if (((char *)dataRow)[indOffset] & NEG_BIT_MASK) return TRUE;
#endif
  return FALSE;
}
...
// expected to be > 0.
void LmParameter::setNullValue(char *dataRow, ComUInt32 indOffset, ComUInt32 indSize, ComBoolean valueIsNull) const {
#ifdef LANGMAN
  str_pad(dataRow + indOffset, indSize, (valueIsNull ? '\377' : '\0'));
#endif
}

ComUInt32 LmParameter::actualInDataSize(void *data) const {
  if (isAlignedFormat() && inVCLenIndSize_ > 0 && inDataOffset_ == UINT_MAX) {
    UInt32 offset = ((ExpAlignedFormat *)data)->getVoaEntry(inVoaOffset_);
    return *((UInt32 *)(((char *)data) + offset));
  }
...
ComUInt32 LmParameter::actualOutDataSize(void *data) const {
...
ComUInt32 LmParameter::vcDataSize(char *data, ComSInt32 lenIndOffset, ComSInt16 lenIndSize) const {
  ComUInt32 len = 0;
#ifdef LANGMAN
  switch (lenIndSize) {
    case 0:
