				      ...
				      ComDiagsArea *da)
{
  ...
  if ((clientInfo = getClientInfo()) == NULL)
    clientInfo = "";
  if (getLM()->setSystemProperty("sqlmx.udr.clientInfo", clientInfo, da) == LM_ERR)
    return LM_ERR;
...
LmResult LmRoutineJava::voidRoutine(void *dataPtr, LmParameter *)
{
  ...
  if (isSPSQL())
  {
    ...
    jobjectArray args = (jobjectArray)jni->NewObjectArray(numParams,
    ...
    jni->CallStaticVoidMethod((jclass)getContainerHandle(),
			      (jmethodID)routine_,
			      name,
			      args);
    return LM_OK;
