...
//
//////////////////////////////////////////////////////////////////////
LmRoutineJava::LmRoutineJava(const char *sqlName, const char *externalName, const char *librarySqlName,
    ...
    : LmRoutine(container, routine, sqlName, externalName, librarySqlName, numSqlParam, maxResultSets,
                COM_LANGUAGE_JAVA, paramStyle, transactionAttrs, sqlAccessMode, externalSecurity, routineOwnerId,
                parentQid, clientInfo, inputRowLen, outputRowLen, currentUserName, sessionUserName, parameters, lm),
      udrForSPSQL_(udrForSPSQL),
      javaParams_(NULL),
  ...
  if (paramStyle == COM_STYLE_JAVA_OBJ) return;
  ...
  if (isUdrForJavaMain()) {
    ...
    ((jvalue *)javaParams_)->l = jni->NewObjectArray((jsize)numSqlParam_, (jclass)lm->stringClass_, NULL);
  ...
  for (i = 0; i < (Int32)numSqlParam_; i++) {
    ...
    switch (LmJavaType(&p).getType()) {
      ...
      default:
        ...
        sprintf(str, ": Unknown parameter type at parameter position %d.", i + 1);
        *da << DgSqlCode(-LME_INTERNAL_ERROR) << DgString0(str);
        return;
    }  // switch()

    if (ja != NULL) jval[i].l = ja;
...
LmRoutineJava::~LmRoutineJava() {
  ...
  if (javaParams_) NADELETEBASIC((jvalue *)javaParams_, collHeap());
}
...
const int DR_SPJ_TOKEN_LEN = DR_SPJ_TOKEN_PREFIX_LEN + DR_SPJ_TOKEN_SEPARATOR_LEN + DR_SPJ_TOKEN_USER_ID_LEN +
                             DR_SPJ_TOKEN_SEPARATOR_LEN + DR_SPJ_TOKEN_NODE_LEN + DR_SPJ_TOKEN_SEPARATOR_LEN +
                             DR_SPJ_TOKEN_PID_LEN + DR_SPJ_TOKEN_SEPARATOR_LEN + DR_SPJ_TOKEN_INVOKER_TOKEN_LEN;
...
LmResult LmRoutineJava::generateDefAuthToken(char *defAuthToken, ComDiagsArea *da) {
  ...
  sprintf(defAuthToken, "%s%s:%07d:%03u:%06u:%.68s", "\005", "\006", getRoutineOwnerId(), myNode, myPid,
          getLM()->userPassword_);
  return LM_OK;
}

LmResult LmRoutineJava::invokeRoutine(void *inputRow, void *outputRow, ComDiagsArea *da) {
  ...
  if (getDefaultCatSchFlag()) {
    ...
    // the UDR are used.
    const char *catalog = (getLM()->sysCatalog_) ? getLM()->sysCatalog_ : udrCatalog_;
    const char *schema = (getLM()->sysSchema_) ? getLM()->sysSchema_ : udrSchema_;

    if (getLM()->setSystemProperty("sqlmx.udr.catalog", catalog, da) == LM_ERR) return LM_ERR;
    if (getLM()->setSystemProperty("sqlmx.udr.schema", schema, da) == LM_ERR) return LM_ERR;
  }

  const char *parentQid;
  if ((parentQid = getParentQid()) == NULL) parentQid = "NONE";
  if (getLM()->setSystemProperty("sqlmx.udr.parentQid", parentQid, da) == LM_ERR) return LM_ERR;
  const char *clientInfo;
  if ((clientInfo = getClientInfo()) == NULL) clientInfo = "";
  if (getLM()->setSystemProperty("sqlmx.udr.clientInfo", clientInfo, da) == LM_ERR) return LM_ERR;
  ...
  if (externalSecurity_ == COM_ROUTINE_EXTERNAL_SECURITY_DEFINER) {
    ...
    if (result != LM_ERR) result = getLM()->setSystemProperty("sqlmx.udr.defAuthToken", defAuthToken, da);
    NADELETEBASIC(defAuthToken, collHeap());
    if (result == LM_ERR) {
      char errStr[LMJ_ERR_SIZE_256];
      sprintf(errStr, ": Error returned from setting System Property:  %s.", "sqlmx.udr.defAuthToken");
      *da << DgSqlCode(-LME_INTERNAL_ERROR) << DgString0(errStr);
      return LM_ERR;
    }
  }
  ...
  if (externalSecurity_ == COM_ROUTINE_EXTERNAL_SECURITY_DEFINER) {
    ...
    if (result == LM_ERR) {
      char errStr[LMJ_ERR_SIZE_256];
      sprintf(errStr, ": Error returned from Resetting System Property:  %s.", "sqlmx.udr.defAuthToken");
      *da << DgSqlCode(-LME_INTERNAL_ERROR) << DgString0(errStr);
      return LM_ERR;
    }
  }
  ...
  while (connList.entries()) {
    ...
    LmConnection *lmConn =
        new (collHeap()) LmConnection(getLM(), conn, LmConnection::DEFAULT_CONN, getTransactionAttrs());
...
void LmRoutineJava::cleanupLmResultSet(ComUInt32 index, ComDiagsArea *diagsArea) {
...
void LmRoutineJava::cleanupLmResultSet(LmResultSet *resultSet, ComDiagsArea *diagsArea) {
...
void LmRoutineJava::cleanupResultSets(ComDiagsArea *diagsArea) {
  ...
  while (resultSetList_.entries()) cleanupLmResultSet((ComUInt32)0, diagsArea);
...
void LmRoutineJava::closeDefConnWithNoRS(ComDiagsArea *diagsArea) {
  ...
  while (i < connectionList_.entries()) {
    ...
    if (lmConn->connType() == LmConnection::DEFAULT_CONN && lmConn->readyToClose()) {
...
NABoolean LmRoutineJava::isDuplicateRS(LmHandle newRS) {
  ...
  for (indx = 0; indx < resultSetList_.entries(); indx++) {
    ...
    // of it's parameters
    if (jni->IsSameObject((jobject)lmRS->getResultSet(), newRSRef)) return TRUE;
...
LmResult LmRoutineJava::populateResultSetInfo(LmHandle newRS, Int32 paramPos, ComDiagsArea *da) {
  ...
  if (newRSRef == NULL || isDuplicateRS(newRSRef)) return result;

  LmResultSetJava::LmResultSetInfoStatus rsInfoStatus;

  LmResultSetJava *newLmRS = new (collHeap())
      LmResultSetJava(getLM(), (LmHandle)newRSRef, paramPos, getNameForDiags(), rsInfoStatus, connectionList_, da);
  ...
  if (rsInfoStatus == LmResultSetJava::RS_INFO_OK) {
    ...
    for (i = 0; i < resultSetList_.entries(); i++) {
      ...
      if (newLmRS->rsCounter_ < lmRS->rsCounter_) break;
    }
...
void LmRoutineJava::deleteLmResultSetsOverMax(ComDiagsArea *diagsArea) {
  ...
  if (resultSetList_.entries() > maxResultSets_) toDelete = resultSetList_.entries() - maxResultSets_;

  while (toDelete--) cleanupLmResultSet(maxResultSets_, diagsArea);
...
ComBoolean LmRoutineJava::isValid() const {
  if (retType_ > LmJavaType::JT_NONE && retType_ <= LmJavaType::JT_LAST) return TRUE;
  return FALSE;
}
...
LmResult LmRoutineJava::voidRoutine(void *dataPtr, LmParameter *) {
  ...
  if (isSPSQL()) {
    ...
    for (int i = 0; i < numParams; i++) {
      jni->SetObjectArrayElement(args, i, (jobject)params[i].l);
    }
    jni->CallStaticVoidMethod((jclass)getContainerHandle(), (jmethodID)routine_, name, args);
    return LM_OK;
  }

  jni->CallStaticVoidMethodA((jclass)getContainerHandle(), (jmethodID)routine_, (jvalue *)javaParams_);
  return LM_OK;
}
