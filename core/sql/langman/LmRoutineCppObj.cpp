...
//
//////////////////////////////////////////////////////////////////////
LmRoutineCppObj::LmRoutineCppObj(tmudr::UDRInvocationInfo *invocationInfo, tmudr::UDRPlanInfo *planInfo,
    ...
    : LmRoutine(container, (LmHandle)interfaceObj, sqlName, externalName, librarySqlName, 0, maxResultSets,
                ...
                invocationInfo->getQueryId().c_str(), NULL, invocationInfo->par().getRecordLength(),
                invocationInfo->out().getRecordLength(), invocationInfo->getCurrentUser().c_str(),
                invocationInfo->getSessionUser().c_str(), NULL, lm),
      planInfos_(collHeap()),
      ...
      inputRows_(NULL),
      outputRow_(NULL) {
  if (planInfo) planInfos_.insertAt(0, planInfo);
...
LmRoutineCppObj::~LmRoutineCppObj() {
  ...
  LM_ASSERT(invocationInfo_ == NULL && interfaceObj_ == NULL && outputRow_ == NULL);
}
...
LmResult LmRoutineCppObj::dealloc(ComDiagsArea *diagsArea) {
  ...
  for (CollIndex i = 0; i < planInfos_.getUsedLength(); i++)
    if (planInfos_.used(i)) delete planInfos_[i];
  planInfos_.clear();
  ...
  if (inputRows_) {
    for (int i = 0; i < numInputTables_; i++)
      if (inputRows_[i]) NADELETEBASIC((inputRows_[i]), collHeap());
  ...
  try {
    ...
    *diagsArea << DgSqlCode(-LME_UDR_METHOD_ERROR) << DgString0("destructor") << DgString1(getNameForDiags())
...
const char *LmRoutineCppObj::getParentQid() { return invocationInfo_->getQueryId().c_str(); }

LmResult LmRoutineCppObj::setRuntimeInfo(const char *parentQid, int totalNumInstances, int myInstanceNum,
                                         ComDiagsArea *da) {
  ...
  if (numInputTables_ > 0) {
    ...
    for (int i = 0; i < numInputTables_; i++) {
      ...
      if (inputRowLength > 0) {
        ...
        const_cast<tmudr::TableInfo &>(invocationInfo_->in(i)).setRowPtr(inputRows_[i]);
      } else
...
LmResult LmRoutineCppObj::invokeRoutine(void *inputRow, void *outputRow, ComDiagsArea *da) {
  // This type of routine does not support the old C interface,
  // must call invokeRoutineMethod instead
  *da << DgSqlCode(-11111) << DgString0("Called LmRoutineCppObj::invokeRoutine()");
...
LmResult LmRoutineCppObj::invokeRoutineMethod(
  ...
  *planInfoLenOut = 0;
  ...
  if (invocationInfoLen <= 0 && invocationInfo_ == NULL) {
    // we need to have an Invocation info
    *da << DgSqlCode(-LME_OBJECT_INTERFACE_ERROR) << DgString0(getNameForDiags())
        << DgString1(tmudr::UDRInvocationInfo::callPhaseToString(tmudr::UDRInvocationInfo::COMPILER_INITIAL_CALL))
        << DgString2("LmRoutineCppObj::invokeRoutineMethod()") << DgString3("Missing UDRInvocationInfo");
  }

  try {
    if (invocationInfoLen > 0) {
      if (invocationInfo_ == NULL) invocationInfo_ = new tmudr::UDRInvocationInfo;

      // unpack the invocation info
      invocationInfo_->deserializeObj(serializedInvocationInfo, invocationInfoLen);
    }

    if (planInfoLen > 0) {
      if (!planInfos_.used(planNum)) planInfos_.insertAt(planNum, new tmudr::UDRPlanInfo(invocationInfo_, planNum));

      // unpack the invocation info
      planInfos_[planNum]->deserializeObj(serializedPlanInfo, planInfoLen);
    }

    // some parameter checks
    if (inputParamRowLen < inputParamRowLen_) return LM_ERR;
    ...
    switch (phase) {
      case tmudr::UDRInvocationInfo::COMPILER_INITIAL_CALL:
        if (invocationInfo_->getDebugFlags() & tmudr::UDRInvocationInfo::PRINT_INVOCATION_INFO_INITIAL)
          invocationInfo_->print();

        if (invocationInfo_->getDebugFlags() & tmudr::UDRInvocationInfo::DEBUG_INITIAL_COMPILE_TIME_LOOP)
      ...
      case tmudr::UDRInvocationInfo::COMPILER_DOP_CALL:
        interfaceObj_->describeDesiredDegreeOfParallelism(*invocationInfo_, *planInfos_[planNum]);
        break;

      case tmudr::UDRInvocationInfo::COMPILER_PLAN_CALL:
        interfaceObj_->describePlanProperties(*invocationInfo_, *planInfos_[planNum]);
        break;

      case tmudr::UDRInvocationInfo::COMPILER_COMPLETION_CALL:
        interfaceObj_->completeDescription(*invocationInfo_, *planInfos_[planNum]);
        if (invocationInfo_->getDebugFlags() & tmudr::UDRInvocationInfo::PRINT_INVOCATION_INFO_END_COMPILE) {
          invocationInfo_->print();
          printf("\n");
          for (CollIndex i = 0; i < planInfos_.getUsedLength(); i++)
      ...
      case tmudr::UDRInvocationInfo::RUNTIME_WORK_CALL: {
        if (invocationInfo_->getDebugFlags() & tmudr::UDRInvocationInfo::PRINT_INVOCATION_INFO_AT_RUN_TIME) {
          invocationInfo_->print();
          planInfos_[planNum]->print();
        }

        if ((invocationInfo_->getDebugFlags() & tmudr::UDRInvocationInfo::DEBUG_INITIAL_RUN_TIME_LOOP_ALL) ||
            (invocationInfo_->getDebugFlags() & tmudr::UDRInvocationInfo::DEBUG_INITIAL_RUN_TIME_LOOP_ONE &&
             invocationInfo_->getMyInstanceNum() == 0))
          interfaceObj_->debugLoop();

        interfaceObj_->processData(*invocationInfo_, *planInfos_[planNum]);
        ...
        if (result == LM_OK) {
          ...
          (*interfaceObj_->emitRowPtr_)(invocationInfo_->out().getRowPtr(), 0, &qstate);
        }
      } break;

      default:
        *da << DgSqlCode(-11111) << DgString0("Invalid call phase in LmRoutineCppObj::invokeRoutineMethod()");
    ...
    // compile-time phases
    if (invocationInfo_ && phase < tmudr::UDRInvocationInfo::RUNTIME_WORK_CALL) {
      *invocationInfoLenOut = invocationInfo_->serializedLength();
      if (planInfos_.used(planNum)) *planInfoLenOut = planInfos_[planNum]->serializedLength();
    }
  } catch (tmudr::UDRException e) {
    ...
    if ((strncmp(sqlState, "38", 2) == 0) && (strncmp(sqlState, "38000", 5) != 0)) {
      *da << DgSqlCode(-LME_CUSTOM_ERROR) << DgString0(e.getMessage().c_str()) << DgString1(sqlState);
      *da << DgCustomSQLState(sqlState);
    } else {
      *da << DgSqlCode(-LME_UDF_ERROR) << DgString0(invocationInfo_->getUDRName().c_str()) << DgString1(sqlState)
          << DgString2(e.getMessage().c_str());
    }
    result = LM_ERR;
  } catch (...) {
    *da << DgSqlCode(-LME_OBJECT_INTERFACE_ERROR) << DgString0(getNameForDiags())
        << DgString1(invocationInfo_->callPhaseToString(phase)) << DgString2("LmRoutineCppObj::invokeRoutineMethod()")
        << DgString3("general exception");
    result = LM_ERR;
  }

  invocationInfo_->callPhase_ = tmudr::UDRInvocationInfo::UNKNOWN_CALL_PHASE;
...
LmResult LmRoutineCppObj::getRoutineInvocationInfo(
  ...
  try {
    if (invocationInfo_ && invocationInfoMaxLen > 0) {
      ...
      *invocationInfoLenOut = invocationInfo_->serialize(tempiibuf, tempiilen);
    } else
      *invocationInfoLenOut = 0;
    ...
    if (planInfos_.used(planNum) && planInfoMaxLen > 0) {
      ...
      *planInfoLenOut = planInfos_[planNum]->serialize(temppibuf, temppilen);
    } else
      *planInfoLenOut = 0;
  } catch (tmudr::UDRException e) {
    ...
    *da << DgSqlCode(-LME_OBJECT_INTERFACE_ERROR) << DgString0(getNameForDiags())
        ...
        << DgString2("LmRoutineCppObj::getRoutineInvocationInfo()") << DgString3(e.getMessage().c_str());
    result = LM_ERR;
  } catch (...) {
    *da << DgSqlCode(-LME_OBJECT_INTERFACE_ERROR) << DgString0(getNameForDiags())
        << DgString1(invocationInfo_->callPhaseToString(invocationInfo_->getCallPhase()))
        << DgString2("LmRoutineCppObj::getRoutineInvocationInfo()") << DgString3("general exception");
    result = LM_ERR;
  }
...
LmResult LmRoutineCppObj::setFunctionPtrs(SQLUDR_GetNextRow getNextRowPtr, SQLUDR_EmitRow emitRowPtr,
                                          ComDiagsArea *da) {
...
LmResult LmRoutineCppObj::validateWall(char *userBuf, int userBufLen, ComDiagsArea *da, const char *bufferName) {
  if (memcmp(userBuf - WALL_STRING_LEN, WALL_STRING, WALL_STRING_LEN) != 0) {
    if (da)
      *da << DgSqlCode(LME_BUFFER_OVERWRITE) << DgString0(invocationInfo_->getUDRName().data()) << DgString1(bufferName)
          << DgString2("beginning");
    else
      throw tmudr::UDRException(38900, "UDR %s overwrote space before its %s buffer",
                                invocationInfo_->getUDRName().data(), bufferName);
  }
  ...
  if (memcmp(userBuf + userBufLen, WALL_STRING, WALL_STRING_LEN) != 0) {
    if (da)
      *da << DgSqlCode(LME_BUFFER_OVERWRITE) << DgString0(invocationInfo_->getUDRName().data()) << DgString1(bufferName)
      ...
      throw tmudr::UDRException(38900, "UDR %s overwrote space after its %s buffer",
                                invocationInfo_->getUDRName().data(), bufferName);
  }
...
LmResult LmRoutineCppObj::validateWalls(ComDiagsArea *da) {
  ...
  */
  ...
  if (validateWall(outputRow_, outputRowLen_, da, "output row buffer") != LM_OK) result = LM_ERR;
