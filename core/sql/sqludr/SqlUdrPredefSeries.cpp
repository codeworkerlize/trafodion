...
extern "C" UDR *SERIES() { return new series(); }

/*  Step 3: Write the actual UDF code*/
void series::describeParamsAndColumns(UDRInvocationInfo &info) {
  // We always expect one input parameter
  int paramCount = info.par().getNumColumns();
  if (paramCount != 2 && paramCount != 3) throw UDRException(38001, "Expecting two or three input parameters");

  if (info.par().getType(0).getSQLTypeClass() != info.par().getType(1).getSQLTypeClass())
    throw UDRException(38001, "The data type of the first two parameters should be same");

  if (info.par().getType(0).getSQLTypeClass() != TypeInfo::NUMERIC_TYPE)
    throw UDRException(38001, "Only support to generate series for exact numerics type");

  for (int i = 0; i < paramCount; i++) info.addFormalParameter(info.par().getColumn(i));
...
void series::processData(UDRInvocationInfo &info, UDRPlanInfo &plan) {
  ...
  if (0 == step) throw UDRException(38001, "Step cannot be a zero");
