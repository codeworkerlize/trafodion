...
static void setParseError(char err, std::string &parseErrorField) {
  if (err != ' ' && parseErrorField.size() < ParseStatusNumChars)
    if (parseErrorField.find_first_of(err) == std::string::npos) parseErrorField.append(1, err);
}
...
static void setIntOutputColumn(const UDRInvocationInfo &info, int paramNum, const char *src,
...
static void setCharOutputColumn(const UDRInvocationInfo &info, int paramNum, const char *src,
                                std::string &parseErrorField) {
...
bool validateCharsAndCopy(char *outBuf, int outBufLen, const char *inBuf, int inBufLen, int &resultLen) {
  bool result = true;
  int srcPos = 0;
  int tgtPos = 0;
  unsigned char c;
  ...
  while (srcPos < inBufLen && tgtPos < tgtLimit) {
    ...
    if (c < 0x80) {
      ...
      if (numBytes > inBufLen - srcPos) validUTF8Char = 0;  // incomplete sequence
      ...
      for (int p = 1; p < numBytes; p++) {
        ...
        if (s < 0x80 || s >= 0xc0) validUTF8Char = 0;
      }

      if (validUTF8Char) {
        for (int j = 0; j < numBytes; j++) outBuf[tgtPos++] = inBuf[srcPos++];
      } else {
        // U+FFFD in UTF-8
        const unsigned char replacementChar[] = {0xef, 0xbf, 0xbd};

        for (int k = 0; k < sizeof(replacementChar); k++) outBuf[tgtPos++] = (char)replacementChar[k];
        srcPos += numBytes;
        result = false;
      }
    }
...
class ReadCppEventsUDFInterface : public UDR {
 ...
 private:
  bool validateEvent(const UDRInvocationInfo &info, const char *currField, const ColNum colNum, const bool doTrace,
                     const int pid);
...
extern "C" UDR *TRAF_CPP_EVENT_LOG_READER(const UDRInvocationInfo *info) { return new ReadCppEventsUDFInterface(); }

void ReadCppEventsUDFInterface::describeParamsAndColumns(UDRInvocationInfo &info) {
  ...
  if (info.getNumTableInputs() != 0)
    throw UDRException(38220, "There should be no table-valued parameters to the call to %s, got %d",
                       info.getUDRName().data(), info.getNumTableInputs());

  if (info.par().getNumColumns() > 1)
    throw UDRException(38221, "There should be no more than one input parameters to the call to %s, got %d",
                       info.getUDRName().data(), info.par().getNumColumns());
  else if (info.par().getNumColumns() == 1) {
    if (!info.par().isAvailable(0) || !info.par().getSQLTypeClass(0) == TypeInfo::CHARACTER_TYPE)
      throw UDRException(38222, "Expecting a character constant as first parameter of the call to %s",
                         info.getUDRName().data());

    std::string options = info.par().getString(0);

    // add an additional formal parameter for the options value
    info.addFormalParameter(ColumnInfo("OPTIONS", info.par().getColumn(0).getType()));
    ...
    for (std::string::iterator it = options.begin(); it != options.end(); it++) {
      switch (*it) {
        ...
        default: {
          throw UDRException(38223, "Option %c not supported in first parameter of the call to %s", *it,
                             info.getUDRName().data());
        }
      }
    }
  ...
  outTable.addColumn(ColumnInfo("LOG_TS", TypeInfo(TypeInfo::TIMESTAMP, 0, true, 6)));
...
void ReadCppEventsUDFInterface::describeDesiredDegreeOfParallelism(UDRInvocationInfo &info, UDRPlanInfo &plan) {
...
void ReadCppEventsUDFInterface::processData(UDRInvocationInfo &info, UDRPlanInfo &plan) {
  // input parameters
  bool addFileColumns = false;
  bool doTrace = false;
  bool doStats = false;
  ...
  if (info.par().getNumColumns() >= 1) {
    ...
    for (std::string::iterator it = inputPar.begin(); it != inputPar.end(); it++) switch (*it) {
        case ' ':
  ...
  if (info.out().getNumColumns() != 10)
    if (info.out().getNumColumns() == 14)
      ...
      throw UDRException(38000, "Expecting 10 or 14 result columns, got %d", info.out().getNumColumns());
  ...
  for (int logLocationIndex = 0; logLocationIndex < numLogLocations; logLocationIndex++) {
    ...
    if (strlen(logrootdir) > 1000) throw UDRException(38001, "TRAF_LOG is longer than 1000 characters");
    ...
    if (logDir_ == NULL) {
      // rest server and wms logs are only on master node
      // If udr runs on diff node, ignore if rest or wms
      // log folder does not exist
      if (logLocationIndex == 2 || logLocationIndex == 3) continue;

      throw UDRException(38002, "Error %d on opening directory %s", (int)errno, logDirName.data());
    }
    ...
    while (1) {
      ...
      if (errno != 0) throw UDRException(38003, "Error %d on reading from directory %s", (int)errno, logDirName.data());
      ...
      if (!validateEvent(info, dirEntry->d_name, LOG_FILE_NAME_COLNUM, doTrace, pid)) continue;
      ...
      if (nameLen > expectedSuffixLenMax) suffix = &fileName[nameLen - expectedSuffixLenMax];
      ...
      if (suffix && strstr(suffix, expectedSuffixPart) != NULL &&
          (strstr(fileName, "master_exec_") == fileName || strstr(fileName, "trafodion.sql_") == fileName ||
           strstr(fileName, "trafodion.sql.") == fileName || strstr(fileName, eventLogFileName.data()) == fileName ||
           strstr(fileName, "tm_") == fileName || strstr(fileName, "mxlobsrvr_") == fileName ||
           strstr(fileName, "sscp") == fileName || strstr(fileName, "ssmp") == fileName ||
           strstr(fileName, "mon") == fileName || strstr(fileName, "pstartd") == fileName ||
           strstr(fileName, "wdg") == fileName || strstr(fileName, "udr_") == fileName ||
           strstr(fileName, "dcs-") == fileName || strstr(fileName, "trafodion.hdfs") == fileName ||
           strstr(fileName, "krb5check") == fileName || strstr(fileName, "rest-") == fileName ||
           strstr(fileName, "wms-") == fileName || strstr(fileName, "scheduled_br") == fileName)) {
        if (infile_ != NULL) {
          fclose(infile_);
          infile_ = NULL;
        }
        ...
        if (infile_ == NULL) throw UDRException(38001, "Error %d returned when opening log file %s", status, fileName);
        ...
        if (addFileColumns) info.out().setString(LOG_FILE_NAME_COLNUM, fileName);
        ...
        while ((ok = fgets(inputLine, sizeof(inputLine), infile_)) != NULL) {
          ...
          if (!validateCharsAndCopy(inputLineValidated, sizeof(inputLineValidated), inputLine, lineLength,
                                    lineLengthValidated)) {
            if (doTrace) {
          ...
          numItems = sscanf(currField, "%4d-%2d-%2d %2d:%2d:%2d %1[,.] %6d%n", &year, &month, &day, &hour, &minute,
          ...
          if (numItems == 8) {
            ...
            if (haveRowToEmit) {
              ...
              if (validateEvent(info, messageTextField.data(), MESSAGE_COLNUM, doTrace, pid))
                setCharOutputColumn(info, MESSAGE_COLNUM, messageTextField.data(), rowParseStatus);
              else
                haveRowToEmit = 0;

              if (addFileColumns) {
                if (validateEvent(info, rowParseStatus.c_str(), PARSE_STATUS_COLNUM, doTrace, pid))
                  setCharOutputColumn(info, PARSE_STATUS_COLNUM, rowParseStatus.c_str(), rowParseStatus);
                else
                  haveRowToEmit = 0;
              }
              if (haveRowToEmit) {
            ...
            if (*fractionSeparator == ',') fraction *= 1000;
            ...
            if (!meetsConstraint) {
              if (doTrace) {
                printf(
                    "(%d) EVENT_LOG_READER Event not returned for column number %d,  value %s does not meet "
                    "constraint\n",
            ...
            if (!haveRowToEmit) {
              ...
              for (std::string::iterator it = lineParseError.begin(); it != lineParseError.end(); it++)
          ...
          if (!haveRowToEmit) {
            ...
            if (currField)
              ...
              *currField = 0;
              messageTextField = (numChars < lineLengthValidated) ? currField + 1 : currField;
              setParseError(FieldParserError, rowParseStatus);
            }
          }
          ...
          for (columnNum = 2; (columnNum <= 9 && !haveRowToEmit); columnNum++) {
            ...
            if (endOfField != NULL) {
              ...
              if (endOfField != currField) endOfField--;

              // remove trailing blanks
              while (*endOfField == ' ') endOfField--;
              ...
              while (*startOfVal != ':' && *startOfVal != ',' && startOfVal != currField) startOfVal--;

              // skip the ":"
              if (startOfVal != currField) startOfVal++;

              // skip leading blanks
              while (*startOfVal == ' ') startOfVal++;
              ...
              *currField = 0;
              ...
              if (messageTextField.empty())
                messageTextField = (currField - inputLineValidated < lineLengthValidated) ? currField + 1 : currField;
              setParseError(FieldParserError, rowParseStatus);
            }
            ...
            switch (columnNum) {
              case 2:
                if (!validateEvent(info, startOfVal, SEVERITY_COLNUM, doTrace, pid))
                  ...
                  setCharOutputColumn(info, SEVERITY_COLNUM, startOfVal, rowParseStatus);
                break;
              ...
              case 3:
                if (!validateEvent(info, startOfVal, COMPONENT_COLNUM, doTrace, pid))
                  ...
                  setCharOutputColumn(info, COMPONENT_COLNUM, startOfVal, rowParseStatus);
                break;
              ...
              case 4:
                if (!validateEvent(info, startOfVal, NODE_NUMBER_COLNUM, doTrace, pid))
                  ...
                  setIntOutputColumn(info, NODE_NUMBER_COLNUM, startOfVal, rowParseStatus);
                break;
              ...
              case 5:
                if (!validateEvent(info, startOfVal, CPU_COLNUM, doTrace, pid))
                  ...
                  setIntOutputColumn(info, CPU_COLNUM, startOfVal, rowParseStatus);
                break;
              ...
              case 6:
                if (!validateEvent(info, startOfVal, PIN_COLNUM, doTrace, pid))
                  ...
                  setIntOutputColumn(info, PIN_COLNUM, startOfVal, rowParseStatus);
                break;
              ...
              case 7:
                if (!validateEvent(info, startOfVal, PROCESS_NAME_COLNUM, doTrace, pid))
                  ...
                  setCharOutputColumn(info, PROCESS_NAME_COLNUM, startOfVal, rowParseStatus);
                break;
              ...
              case 8:
                if (!validateEvent(info, startOfVal, SQL_CODE_COLNUM, doTrace, pid))
                  ...
                  setIntOutputColumn(info, SQL_CODE_COLNUM, startOfVal, rowParseStatus);
                break;
              ...
              case 9:
                if (!validateEvent(info, startOfVal, QUERY_ID_COLNUM, doTrace, pid))
                  ...
                  setCharOutputColumn(info, QUERY_ID_COLNUM, startOfVal, rowParseStatus);

                // we read all required fields,
                // next field is the message text
                if (messageTextField.empty()) messageTextField = nextField;
                break;
            }
  ...
  if (doStats) {
    printf(
        "(%d) EVENT_LOG_READER results: number log files opened: %ld, "
...
bool ReadCppEventsUDFInterface::validateEvent(const UDRInvocationInfo &info, const char *currField, const ColNum colNum,
                                              const bool doTrace, const int pid) {
  ...
  for (int i = 0; i < info.getNumPredicates(); i++) {
    ...
    if (info.getComparisonPredicate(i).getColumnNumber() != colNum) continue;
    ...
    // compare
    bool isNumeric = (colNum == NODE_NUMBER_COLNUM || colNum == CPU_COLNUM || colNum == PIN_COLNUM ||
                      colNum == SQL_CODE_COLNUM || colNum == LOG_FILE_NODE_COLNUM || colNum == LOG_FILE_LINE_COLNUM)
                         ? true
                         : false;
    ...
    if (doTrace) {
      printf(
          "(%d) EVENT_LOG_READER Constraint check - comparison operator: %d "
          "column: %d event value: %s predicate value: %s, iterator: %d \n",
          pid, (int)info.getComparisonPredicate(i).getOperator(), colNum, currField, constStr.c_str(), i);
      fflush(stdout);
    }
    ...
    if (colNum == LOG_TS_COLNUM) {
      ...
      if (pEnd == NULL || *pEnd != 0 || pEnd == constStr.c_str()) validConst = false;

      // Convert event value
      long eventLong = strtol(currField, &pEnd, 10);

      // If unable to convert currField into an int, set validEvent to false
      if (pEnd == NULL || *pEnd != 0 || pEnd == currField) validEvent = false;
      ...
      switch (info.getComparisonPredicate(i).getOperator()) {
        case PredicateInfo::EQUAL:
          ...
          break;
        case PredicateInfo::NOT_EQUAL:
          if (validConst && validEvent && constLong == eventLong) return false;
          break;
        case PredicateInfo::LESS:
    ...
    // All other comparisons are assumed to be string compares
      ...
      temp = constStr;
      constStr.clear();
      for (size_t j = 0; j < temp.size(); ++j) constStr += (std::toupper(temp[j]));
      ...
      temp = currField;
      std::string eventStr;
      for (size_t j = 0; j < temp.size(); ++j) eventStr += (std::toupper(temp[j]));
      ...
      switch (info.getComparisonPredicate(i).getOperator()) {
        case PredicateInfo::EQUAL:
          if (constStr != eventStr) return false;
          break;
        case PredicateInfo::NOT_EQUAL:
          if (constStr == eventStr) return false;
          break;
        case PredicateInfo::LESS:
          if (eventStr >= constStr) return false;
          break;
        case PredicateInfo::LESS_EQUAL:
          if (eventStr > constStr) return false;
          break;
...
ReadCppEventsUDFInterface::~ReadCppEventsUDFInterface() {
  if (logDir_ != NULL) closedir(logDir_);
  if (infile_ != NULL) fclose(infile_);
}
