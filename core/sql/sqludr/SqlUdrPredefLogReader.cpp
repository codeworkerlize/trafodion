                 ...
                 UDRPlanInfo &plan)
{
  ...
  for(int logLocationIndex = 0; logLocationIndex < numLogLocations; logLocationIndex++) 
  {
    ...
    if (logDir_ == NULL)
    {
      ...
      if(logLocationIndex == 2 || logLocationIndex == 3) 
        continue;
	...
	expectedSuffixLenMax += 11 ; // for "*.log.yyyy-mm-dd" rollover files
	  ...
	  (strstr(fileName, "master_exec_")          == fileName ||
	   ...
	   strstr(fileName, "trafodion.sql.")        == fileName ||
	   ...
	   strstr(fileName, "trafodion.hdfs")        == fileName ||
	   strstr(fileName, "krb5check")             == fileName ||
	   strstr(fileName, "rest-")                 == fileName ||
	   strstr(fileName, "wms-")                  == fileName || 
	   strstr(fileName, "scheduled_br")          == fileName 
	   ))
      {
        if (infile_ != NULL)
        {
