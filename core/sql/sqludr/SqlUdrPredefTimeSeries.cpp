...
extern "C" UDR *TRAF_CPP_TIMESERIES() { return new TimeSeries(); }
...
void TimeSeries::describeParamsAndColumns(UDRInvocationInfo &info) {
  ...
  for (int pc = 0; pc < numPartCols; pc++) info.addPassThruColumns(0, part.getColumnNum(pc), part.getColumnNum(pc));
  ...
  const TypeInfo &timeType = info.in().getColumn(internalCols.getTimeSliceInColNum()).getType();

  info.out().addColumn(ColumnInfo(info.par().getString(0).c_str(), timeType));
  ...
  for (int a = 0; a < internalCols.getNumAggrCols(); a++) {
    ...
    TypeInfo inColType(info.in().getColumn(tsa.getInputColNum()).getType());
    ...
    if (tsa.isConstInterpol())
      ...
      outColName += "L";
    if (tsa.isIgnoreNulls()) outColName += "I";
    ...
    if (tsa.isConstInterpol()) {
      ...
      info.out().addColumn(ColumnInfo(outColName.c_str(), TypeInfo(TypeInfo::DOUBLE_PRECISION, 0, true)));
  }
  ...
  for (int p = 0; p < info.par().getNumColumns(); p++) {
    ...
    info.addFormalParameter(ColumnInfo(parName, info.par().getColumn(p).getType()));
  }
}

void TimeSeries::processData(UDRInvocationInfo &info, UDRPlanInfo &plan) {
  InternalColumns internalCols(info);
    ...
    while (inSamePartition) {
      if (!haveRowFromPreviousLoop) {
        ...
        if (getNextRow(info)) {
          ...
          if ((inSamePartition = internalCols.isSamePartition()) == false) haveRowFromPreviousLoop = true;
        } else {
...
template <class T>
NullableTimedValue<double> NullableTimedValue<T>::interpolateLinear(time_t t,
                                                                    const NullableTimedValue<T> &nextHigherVal) {
  // return a NULL value if t is outside the range of
  // "this" and "nextHigherVal" or if "this" or
  // "nextHigherVal" is a NULL value
  if (t < t_ || t > nextHigherVal.t_ || isNull_ || nextHigherVal.isNull_)
    return NullableTimedValue<double>(true, t, 0.0);

  // this should not be common, but avoid division by zero for this unlikely case
  if (t_ == nextHigherVal.t_) return NullableTimedValue<double>(false, t, (v_ + nextHigherVal.v_) / 2);

  return NullableTimedValue<double>(
      false, t,
      static_cast<double>(v_) + (static_cast<double>(t) - t_) / (nextHigherVal.t_ - t_) * (nextHigherVal.v_ - v_));
}

TimeSeriesAggregate::TimeSeriesAggregate(const TupleInfo &inTup, const TupleInfo &outTup, int inputColNum,
                                         int outputColNum, bool isFirstVal, bool isConstInterpol, bool isIgnoreNulls)
    : inTup_(inTup),
      outTup_(outTup),
  ...
  useLong_ = (isConstInterpol &&
              inTup_.getColumn(inputColNum_).getType().getSQLTypeSubClass() == TypeInfo::EXACT_NUMERIC_TYPE);
}
...
void TimeSeriesAggregate::setOutputCol(time_t startTime, int sliceNum, time_t width) {
  ...
  while (currIx_ + 1 < numEntries() &&
         ((isFirstVal_ && getTime(currIx_ + 1) <= targetTime) || (!isFirstVal_ && getTime(currIx_ + 1) < targetTime)))
    currIx_++;

  if (currIx_ >= 0) currTime = getTime(currIx_);

  if (isConstInterpol_) {
    // currIx_ points to the entry with the last known constant
    // value to use or currIx_ is -1 (no last known value exists)
    if (currIx_ < 0 || (useLong_ && lValues_[currIx_].isNull()) || (!useLong_ && dValues_[currIx_].isNull()))
    ...
    if (currIx_ >= 0 && currTime == targetTime) {
      ...
      interpolatedVal = dValues_[currIx_].interpolateLinear(targetTime, dValues_[currIx_ + 1]);
    }
...
InternalColumns::InternalColumns(const UDRInvocationInfo &info) : info_(info) {
  // expect a single table-valued input
  if (info.getNumTableInputs() != 1) throw UDRException(38010, "TIMESERIES UDF: Expecting one table-valued input");
  ...
  if (info.getCallPhase() == UDRInvocationInfo::COMPILER_INITIAL_CALL) {
    ...
    if (ord.getNumEntries() != 1 || ord.getOrderType(0) == OrderInfo::DESCENDING)
      throw UDRException(
          38020,
          "TIMESERIES UDF: Must use ORDER BY with one column for its input table and the order must be ascending");
    TypeInfo::SQLTypeCode typeCode = info.in().getColumn(ord.getColumnNum(0)).getType().getSQLType();
    if (typeCode != TypeInfo::TIME && typeCode != TypeInfo::TIMESTAMP)
      throw UDRException(38030, "TIMESERIES UDF: Must use ORDER BY a TIME or TIMESTAMP column for the input table");

    // we need at least two parameters, time column name and width
    // of time slice
    if (info.par().getNumColumns() < 2)
      throw UDRException(38040, "TIMESERIES UDF: UDF needs to be called with at least 2 scalar parameters");

    // input parameter 0 (defined in the DDL) is the
    // name of the column containing the time values
    if (!info.par().isAvailable(0) || info.par().getColumn(0).getType().getSQLTypeClass() != TypeInfo::CHARACTER_TYPE)
      throw UDRException(38050, "TIMESERIES UDF: Expecting a character constant (timestamp alias) as first parameter");

    // check type and value of the time slice width, specified
    // as parameter 1
    if (!info.par().isAvailable(1))
      throw UDRException(38060, "TIMESERIES UDF: Expecting a constant for the time slice width as second parameter");

    // time slice width must be a day-second interval
    if (info.par().getColumn(1).getType().getSQLTypeSubClass() != TypeInfo::DAY_SECOND_INTERVAL_TYPE)
      throw UDRException(38070,
                         "TIMESERIES UDF: Second scalar parameter for time slice width must be an interval constant in "
                         "the day to second range");
    ...
    for (int p = 2; p < info.par().getNumColumns(); p++)
      if (!info.par().isAvailable(p))
        throw UDRException(38090, "TIMESERIES UDF: All parameters must be specified as literals");
  ...
  while (ip < info.par().getNumColumns()) {
    ...
    if (info.getCallPhase() == UDRInvocationInfo::COMPILER_INITIAL_CALL) {
      if (instr.size() < 2 || instr.size() > 3)
        throw UDRException(38100, "TIMESERIES UDF: Expecting instructions with 2 or 3 characters: %s", instr.c_str());
      ...
      switch (instr[0]) {
        ...
        default:
          throw UDRException(38110, "TIMESERIES UDF: Parameter %d should start with F or L for first or last value",
                             ip + 2);
      }
      ...
      switch (instr[1]) {
        ...
        default:
          throw UDRException(38120,
                             "TIMESERIES UDF: Parameter %d should have C or L as its second character, for constant or "
                             "linear interpolation",
                             ip + 2);
      }

      if (instr.size() == 3 && instr[2] != 'i' && instr[2] != 'I')
        throw UDRException(38130, "TIMESERIES UDF: Unexpected trailing characters in aggregate instructions: %s",
                           instr.c_str());
    }  // compile-time checks
    ...
    columns_.push_back(new TimeSeriesAggregate(info.in(), info.out(), info.in().getColNum(info.par().getString(ip)),
                                               getFirstAggrCol() + columns_.size(), isFirstVal, isConstInterpol,
                                               isIgnoreNulls));
    ip += 2;
  }
}
...
InternalColumns::~InternalColumns() {
  for (std::vector<TimeSeriesAggregate *>::iterator it = columns_.begin(); it != columns_.end(); it++) delete *it;
}
...
void InternalColumns::initializePartition() {
  ...
  partitionStartTime_ = convertToLocalTimeStamp(partitionStartTime_);
  ...
  for (std::vector<TimeSeriesAggregate *>::iterator it = columns_.begin(); it != columns_.end(); it++)
    (*it)->initPartition();
}
...
bool InternalColumns::isSamePartition() {
  ...
  for (int p = 0; p < numPartCols_; p++) {
    ...
    if (wasNull != currPartKeyNulls_[p] || (!wasNull && pkVal != currPartKey_[p])) return false;
  }
...
int InternalColumns::getNumTimeSlices() { return (partitionCurrTime_ - partitionStartTime_) / timeSliceWidth_ + 1; }
...
void InternalColumns::readInputCols() {
  ...
  partitionCurrTime_ = convertToLocalTimeStamp(partitionCurrTime_);
...
void InternalColumns::setOutputCols(int sliceNum) {
  ...
  info_.out().setTime(numPartCols_, timeSliceStartTime);

  // set the aggregate columns
  for (std::vector<TimeSeriesAggregate *>::iterator it = columns_.begin(); it != columns_.end(); it++)
    (*it)->setOutputCol(timeSliceStartTime, sliceNum, timeSliceWidth_);
}
