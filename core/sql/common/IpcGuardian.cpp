...
#define ESP_TRACE1(s1) \
  if (sv_trace) {      \
    trace_printf(s1);  \
  }
#define ESP_TRACE2(s1, s2) \
  if (sv_trace) {          \
    trace_printf(s1, s2);  \
  }
...
IpcNodeName::IpcNodeName(const GuaProcessHandle &phandle) {
  ...
  str_cpy(guardianNode_.nodeName_, &nodeNameWithBackslash[1], GuaNodeNameMaxLen, ' ');
}
...
GuaConnectionToServer::GuaConnectionToServer(IpcEnvironment *env, const IpcProcessId &procId,
  ...
  numChunksRequested_ = 0;
  numChunksReceived_ = 0;
  ...
  for (unsigned short i = 0; i < nowaitDepth_; i++) sendCallbackBufferList_[i] = NULL;
...
NABoolean GuaConnectionToServer::moreWaitsAllowed() { return !stopWait_; }
...
WaitReturnStatus GuaConnectionToServer::wait(IpcTimeout timeout, UInt32 *eventConsumed, IpcAwaitiox *ipcAwaitiox) {
  ...
  if (getState() != OPENING) {
...

    {
      ...
      if (getState() == ERROR_STATE AND numOutstandingIOs_ <= 0 AND numQueuedSendMessages() <=
                            0) {  // no more waits on this connection
        stopWait(TRUE);
        return WAIT_OK;
      }
      stopWait(FALSE);
      ...
      if (timeout != IpcImmediately || numOutstandingIOs_ == 0 || chunkBytesSent_ == maxIOSize_)
        while (tryToStartNewIO())
          ;

      // try to complete I/Os within the given time limit,
      // if there are outstanding I/Os
      if (numOutstandingIOs_ == 0) return WAIT_OK;
    }  // release mutex before calling AWAITIOX
    ...
    while (retry) {
      ...
      if (!ipcAwaitioxCompleted) {
        if (tscoOpen_)
          stat = BAWAITIOXTS(&openFile_, (void **)&bufferAddr, &countRead, &ioTag, timeout, OMIT);
        else
          stat = BAWAITIOX(&openFile_, (void **)&bufferAddr, &countRead, &ioTag, timeout, OMIT);
      } else
        stat = ipcAwaitiox->ActOnAwaitiox((void **)&bufferAddr, &countRead, &ioTag);
...

    {
      ...
      // Note that other threads may have completed other IO entries.
      // TODO: Handle messages arriving out of order
      NAMutexScope ms(mutex_);

      getEnvironment()->setEvent(TRUE, AEVENT);

      // If we got an error, the tag may or may  not be a valid one.
      // So check for both cases below. Also check for the case that
      // a concurrent thread has set guaErrorInfo_ in the meantime.
      if (guardianErrNum != GuaOK || guaErrorInfo_ != GuaOK) {
        ...
        //    reply, then:
        //
        //    c.1) - if this is the last chunk of the reply, then put the
        ...
        if (entry.receiveBufferSizeLeft_ == 0) {
          ...
          if (entry.offset_ == 0) {
            ...
            InternalMsgHdrInfoStruct *msgHdr =
                new ((IpcMessageObj *)(entry.buffer_->data(0))) InternalMsgHdrInfoStruct(NULL);
            // remember the real length of the message coming back
            // including the chunk headers, which are still in it
            IpcMessageObjSize msgLen =
                IpcMessageBuffer::getLengthWithChunkHeaders(maxIOSize_, msgHdr->getMsgLengthFromData());
            ...
            if (msgLen == (IpcMessageObjSize)countRead) {
              ...
              // heap.
              if (bytesSent > maxIOSize_) entry.buffer_ = entry.buffer_->resize(getEnvironment(), msgLen, maxIOSize_);
              ...
              if (msgLen > entry.buffer_->getBufferLength() || bytesSent > maxIOSize_) {
                ...
                entry.buffer_ = entry.buffer_->resize(getEnvironment(), msgLen, maxIOSize_);
                entry.buffer_->setMessageLength(msgLen);
              }

              // do some sanity checks, make sure we don't have two
              // partial buffers at a time
              if (partiallyReceivedBuffer_ != NULL) reportBadMessage();
              assert(partiallyReceivedBuffer_ == NULL);
              if (msgLen <= (IpcMessageObjSize)countRead) reportBadMessage();
              assert(msgLen > (IpcMessageObjSize)countRead);
              assert((IpcMessageObjSize)countRead == maxIOSize_);

              // move some information from the entry to data members
              // in the connection while the chunky protocol is going on
              partiallyRecvSet(entry.buffer_, 1, 1);
              getEnvironment()->getAllConnections()->setReceivedPartialMessage(TRUE);
            }
          }  // case b) - first (maybe only) chunk
          else {
            // case c) - this is not the first chunk
            if (partiallyReceivedBuffer_ != entry.buffer_) reportBadMessage();
            assert(partiallyReceivedBuffer_ == entry.buffer_);
            entry.buffer_->validateChunkHeader(entry.offset_, countRead, maxIOSize_, TRUE);
            numChunksReceived_++;

            if (numChunksReceived_ == DIVIDE_AND_ROUND_UP(entry.buffer_->getMessageLength(), maxIOSize_)) {
              // case c.1) - this is the last chunk
              // jdu 01/24/12 - need more work to get the message info right
              //                     env()->addIpcMsgTrace(this, IpcEnvironment::RECEIVE,
              //                                           (void *)entry.buffer_,
              //                                           numChunksReceived_, 1, 0);
              entry.buffer_->removeChunkHeadersAndTrailer(maxIOSize_, getEnvironment());
              queueReceiveMessage(partiallyReceivedBuffer_);
              partiallyRecvSet(NULL, 0, 0);
            } else {
        ...
        // this I/O completed
        if (getState() != ERROR_STATE && numOutstandingIOs_ == 0) setState(ESTABLISHED);

      }  // if (guardianErrNum) else ..
    }    // release mutex before doing new I/Os and calling callbacks
    ...
    if (aCallbackIsCalled && getState() == ERROR_STATE) setState(ERROR_STATE);
    ...
    if (_status_ne(stat)) {
      ...
      if (getinfoError != 0) guaErrorInfo_ = getinfoError;  // not even FILE_GETINFO_ worked
      ...
      if (guaErrorInfo_ == XZFIL_ERR_NOSUCHDEV && getState() == OPENING && getOpenRetries() < 8 &&
          dataConnectionToEsp_) {
        guaErrorInfo_ = XZFIL_ERR_OK;
        setState(INITIAL);
      ...
      if (openCompletionScheduled_ != NULL) *openCompletionScheduled_ = 1;
      return WAIT_OK;
    }
...
void GuaConnectionToServer::populateDiagsArea(ComDiagsArea *&diags, CollHeap *diagsHeap) {
  if (guaErrorInfo_ != GuaOK) {
    ...
    *diags << DgNskCode(guaErrorInfo_);
    getEnvironment()->getMyOwnProcessId(IPC_DOM_GUA_PHANDLE).addProcIdToDiagsArea(*diags, 0);
    ...
    if (guaErrorInfo_ == FETIMEDOUT) {
      ...
      if (!bugcatcherInitialized) {
        ...
        if (dbc && (*dbc != '1')) doBugCatcher = false;
      }
      if (doBugCatcher) {
...
// a read buffer used to receive the reply. The same buffer can be used as both
// the write buffer and the read buffer. Here we will refer to the write buffer
...
NABoolean GuaConnectionToServer::tryToStartNewIO() {
  ...
  if (sendQueueEntries() == 0 && !partiallySentBuffer_ && !partiallyReceivedBuffer_) return FALSE;

  // do not allow new send if a partial message is being received and we
  // already have requested all the reply data for it.
  // add timeout for partial message receiving to avoid blocking send queue
  if (partiallyReceivedBuffer_ &&
      DIVIDE_AND_ROUND_UP(partiallyReceivedBuffer_->getMessageLength(), maxIOSize_) == numChunksRequested_) {
    // if partially received buffer not handled, then block any other messages
    // to keep waiting for the rest messages
    if (NOT partiallyRecvProcessed()) return FALSE;

    if (sendQueueEntries() == 0) return FALSE;
  }
  ...
  if (numOutstandingIOs_ >=
      (IFX(partiallySentBuffer_ OR partiallyReceivedBuffer_) THENX nowaitDepth_ ELSEX(nowaitDepth_ - 1))) {
  ...
  short numMsgsActual;
  if (XMESSAGESYSTEMINFO(5, &numMsgsActual)) assert(0);
  if (numMsgsActual + 1 >= getEnvironment()->getMaxPerProcessMQCs()) return FALSE;
  ...
  if (partiallySentBuffer_) {
    ...
    entry.bytesSent_ = MINOF(maxIOSize_, entry.buffer_->getMessageLength() - chunkBytesSent_);
    ...
    entry.offset_ = numChunksRequested_ * maxIOSize_;
    entry.receiveBufferSizeLeft_ = maxIOSize_;
    numChunksRequested_++;
    lastSentBuffer_ = entry.buffer_;
    assert(entry.buffer_->getMessageLength() >= entry.offset_);
  } else {
    ...
    // insert chunk headers if needed (this may require a buffer copy)
    nextToSend = nextToSend->addChunkHeadersAndTrailer(maxIOSize_, getEnvironment());
    ...
    if (entry.buffer_->getRefCount() > 1) {
      // The send buffer is shared by multiple connections.
      // Therefore, allocate a different buffer for the reply,
      // since otherwise the replies would overwrite each
      // other. Note that the copy contains a (possibly partial)
      // copy of the message to send. Note also that we don't need
      // to do this for any additional chunks, since there won't
      // be any expected reply data for those.
      entry.readBuffer_ = entry.buffer_->copy(getEnvironment(), maxIOSize_, maxIOSize_, FALSE, TRUE);
    ...
    entry.receiveBufferSizeLeft_ = MINOF(maxIOSize_, entry.readBuffer_->getBufferLength());
  ...
  do {
    ...
    if (_status_ne(stat)) {
      ...
      if (retcode != 0) fsError = retcode;                  // not even FILE_GETINFO_ worked
      if ((fsError == FENOLCB) && (retryCount < 100 * 60))  // after 60 seconds (and 6000 retries),
                                                            // just give up.
      {
      ...
      if (entry.bytesSent_ >= sizeof(InternalMsgHdrInfoStruct)) {
        InternalMsgHdrInfoStruct *imhis = (InternalMsgHdrInfoStruct *)writeReadBuffer->data(entry.offset_);
        env()->addIpcMsgTrace(this, IpcEnvironment::SEND, (void *)writeReadBuffer->data(entry.offset_),
                              entry.bytesSent_, (imhis->isLastMsgBuf() ? 1 : 0), imhis->getSeqNum());
#if 0
          if (sentMsgHdrInd_ == 7)
            sentMsgHdrInd_ = 0;
          else
  ...
  if (isFirstChunk) addSendCallbackBuffer(entry.buffer_);
  ...
  if (fsError) {
    ...
    if (numOutstandingIOs_ == 0) setState(SENDING);
    numOutstandingIOs_++;
...
void GuaConnectionToServer::openPhandle(char *processName, NABoolean parallelOpen) {
  ...
  if (stateOnEntry == INITIAL) {
    ...
    NAProcessHandle phandle((SB_Phandle_Type *)&(getOtherEnd().getPhandle().phandle_));
    phandle.decompose();
    ...
    NABoolean isEsp = getEnvironment()->getAllConnections()->isEsp();
    ...
    if (strcmp(getEnvironment()->myProcessName(), procFileName) == 0) {
      ESP_TRACE1("SELF");
      ...
      // multi fragment esp
      guaErrorInfo_ = BFILE_OPEN_(procFileName, procFileNameLen, &openFile_,
                                  0,  // open for read/write access
                                  0,  // shared access
                                  nowaitDepth_,
                                  0,           // sync depth 0 (target proc is not NonStop)
  ...
  if (openFlags & 0x4000)  // Nowaited FILE_OPEN_
  {
    ...
    do {
      ...
      if (getState() == INITIAL || env->getAllConnections()->isEsp() == FALSE) {
        if (tscoOpen_)
      ...
      if (_status_ne(condCode)) {
        ...
        if (guaErrorInfo_ == GuaTimeoutErr && env->getAllConnections()->isEsp()) {
          guaErrorInfo_ = XZFIL_ERR_OK;
...
void GuaConnectionToServer::handleIOError(NABoolean cancelReq) {
  // connection no longer usable due to I/O error. abort all existing I/Os
  // on this connection.
  for (unsigned short i = 0; i < nowaitDepth_; i++) {
    ActiveIOQueueEntry &entry = activeIOs_[i];
    if (entry.inUse_) handleIOErrorForEntry(entry, cancelReq);
  }
...
void GuaConnectionToServer::handleIOErrorForStream(IpcMessageStreamBase *msgStream) {
  ...
  for (unsigned short i = 0; i < nowaitDepth_; i++) {
    ActiveIOQueueEntry &entry = activeIOs_[i];
    if (entry.inUse_ && entry.buffer_->getMessageStream() == msgStream) handleIOErrorForEntry(entry);
  }
}
...
//
void GuaConnectionToServer::handleIOErrorForEntry(ActiveIOQueueEntry &entry, NABoolean cancelReq) {
  ...
  for (unsigned short i = 0; i < nowaitDepth_; i++) {
    ...
    if (nextEntry.inUse_ && nextEntry.buffer_->getMessageStream() == entry.buffer_->getMessageStream()) {
      cleanUpActiveIOEntry(nextEntry, cancelReq);
      ...
      if (nextEntry.readBuffer_ && nextEntry.readBuffer_ != nextEntry.buffer_)
  ...
  if (partiallySentBuffer_ && partiallySentBuffer_->getMessageStream() == entry.buffer_->getMessageStream())
    partiallySentBuffer_ = NULL;
  else if (partiallyReceivedBuffer_ &&
           partiallyReceivedBuffer_->getMessageStream() == entry.buffer_->getMessageStream())
    partiallyReceivedBuffer_ = NULL;
  ...
  if (!sendSuccess) entry.buffer_->callSendCallback(this);
...
void GuaConnectionToServer::cleanUpActiveIOEntry(ActiveIOQueueEntry &entry, NABoolean cancelReq) {
  // abort if we are still waiting for message reply
  if (entry.ioTag_ >= 0 && cancelReq == TRUE) {
...
NABoolean GuaConnectionToServer::partiallyRecvProcessed() { return FALSE; }

// -----------------------------------------------------------------------
// Methods for class GuaConnectionToServerTimeout
// -----------------------------------------------------------------------
GuaConnectionToServerTimeout::GuaConnectionToServerTimeout(IpcEnvironment *env, const IpcProcessId &procId,
  ...
  partiallyReceiveTimeout_ = 15;  // in seconds

  const char *partialReceiveTimeout = getenv("CONNECTION_TO_SERVER_PARTIAL_RECEIVE_TIMEOUT");
  if (partialReceiveTimeout) {
    int timeout = atoi(partialReceiveTimeout);
    if (timeout > 0) partiallyReceiveTimeout_ = timeout;
  }
}
...
NABoolean GuaConnectionToServerTimeout::partiallyRecvProcessed() {
  // first time reach here for this partially message
  if (partiallyReceiveTime_.tv_sec == 0) {
    clock_gettime(CLOCK_REALTIME, &partiallyReceiveTime_);
  }

  // check if we are waiting partially receive message to finish for too long
  // haven't find any good way to redo quest for partially receive message
  struct timespec curr;
  clock_gettime(CLOCK_REALTIME, &curr);
  if (curr.tv_sec - partiallyReceiveTime_.tv_sec > partiallyReceiveTimeout_) {
    // give up current partially receive message to unblock sending queue
    partiallyRecvSet(NULL, 0, 0);
    handleIOError(FALSE);

    {
      char msg[256];
      sprintf(msg, "WARNING!! partially received data abandoned because partial receive progress exceeds timeout.");
...
GuaConnectionToClient::GuaConnectionToClient(IpcEnvironment *env, const IpcProcessId &clientProcId,
                                             GuaFileNumber clientFileNumber,
                                             GuaReceiveControlConnection *controlConnection, const char *eye)
    : IpcConnection(env, clientProcId, eye) {
  clientFileNumber_ = clientFileNumber;
  guaErrorInfo_ = GuaOK;
  controlConnection_ = controlConnection;
  chunkSize_ = env->getGuaMaxMsgIOSize();

  partiallyRepliedBuffer_ = NULL;
  chunkBytesReplied_ = 0;
  partiallyReceivedBuffer_ = NULL;
  numChunksReceived_ = 0, fileNumForIOCompletion_ = controlConnection->receiveFile_;
...
bool GuaConnectionToClient::isServerSide() { return true; }

void GuaConnectionToClient::send(IpcMessageBuffer *buffer) {
  if (buffer->getReplyTag() == GuaInvalidReplyTag) ABORT("Need to wait for a request before replying for now");
...
void GuaConnectionToClient::receive(IpcMessageStreamBase *msg) {
  ...
  while (receiveBuf = getNextReceiveQueueEntry()) {
    // yes, so just call its callback
#ifdef LOG_RECEIVE
    cerr << "Calling receive callback for queued request during receive()" << endl;
#endif
...
void GuaConnectionToClient::populateDiagsArea(ComDiagsArea *&diags, CollHeap *diagsHeap) {
  if (guaErrorInfo_ != GuaOK) {
    IpcAllocateDiagsArea(diags, diagsHeap);

    *diags << DgSqlCode(-2033) << DgInt0(guaErrorInfo_) << DgNskCode(guaErrorInfo_);
    getEnvironment()->getMyOwnProcessId(IPC_DOM_GUA_PHANDLE).addProcIdToDiagsArea(*diags, 0);
    getOtherEnd().addProcIdToDiagsArea(*diags, 1);
  }
}

NABoolean GuaConnectionToClient::thisIsMyClient(const GuaProcessHandle &phandle, GuaFileNumber fileNo) const {
  return (clientFileNumber_ == fileNo AND getOtherEnd().getPhandle() == phandle);
}

void GuaConnectionToClient::close(NABoolean withError, GuaErrorNumber gerr) {
...

  {
    ...
    if (getState() == CLOSED) getEnvironment()->getAllConnections()->incrDeleteCount();
...
NABoolean GuaConnectionToClient::startReplyingToNextRequest() {
  ...
  IpcMessageObjSize bytesToSend;
  NABoolean ioCompleted = TRUE;

  {
    NAMutexScope ms(mutex_);

    if (partiallyRepliedBuffer_) return FALSE;
    ...
    bytesToSend = buffer->getMessageLength();
    if (bytesToSend > buffer->getMaxReplyLength()) {
      ...
      assert(buffer->getMaxReplyLength() == chunkSize_);
      // add chunk headers to the message, this may require
      // resizing the buffer
      buffer = buffer->addChunkHeadersAndTrailer(chunkSize_, getEnvironment());
    ...
    // send it off
    controlConnection_->sendReplyData(buffer->data(0), bytesToSend, buffer->getReplyTag(), this, GuaOK);
    decrNumOutstandingRequests();

    if (partiallyRepliedBuffer_ AND NOT(getState() == ERROR_STATE)) {
      ioCompleted = FALSE;
...
void GuaConnectionToClient::acceptBuffer(IpcMessageBuffer *buffer, IpcMessageObjSize receivedDataLength) {
  IpcMessageBuffer *callbackBuffer = NULL;

  {
    ...
    if (receivedDataLength == 0) {
      if (partiallyRepliedBuffer_ != NULL AND partiallyReceivedBuffer_ == NULL)
        ;
      else {
      ...
      IpcMessageObjSize nextChunkSize =
          MINOF(partiallyRepliedBuffer_->getMessageLength() - chunkBytesReplied_, buffer->getMaxReplyLength());

      controlConnection_->sendReplyData(partiallyRepliedBuffer_->data(chunkBytesReplied_), nextChunkSize,
                                        buffer->getReplyTag(), this, GuaOK);
      decrNumOutstandingRequests();
      ...
      if (chunkBytesReplied_ >= partiallyRepliedBuffer_->getMessageLength()) {
        ...
        // buffer and call the callback (once we release the mutex)
        IOComplete();
        callbackBuffer = partiallyRepliedBuffer_;
        partiallyRepliedBuffer_ = NULL;
        chunkBytesReplied_ = 0;
      } else {
        // tell the control connection we need another request
        controlConnection_->initiateReceive();
    ...
    // ---------------------------------------------------------------------
      ...
      InternalMsgHdrInfoStruct *msgHdr = new ((IpcMessageObj *)(buffer->data(0))) InternalMsgHdrInfoStruct(NULL);

      // get the total message length for all chunks from the
      // message header and add adjustments for the data displaced
      // by chunk headers and for the chunk trailer
      totalMessageLength = buffer->getLengthWithChunkHeaders(chunkSize_, msgHdr->getMsgLengthFromData());
      ...
      if (totalMessageLength == receivedDataLength) {
        ...
        if (totalMessageLength <= receivedDataLength) reportBadMessage();
        ...
        partiallyReceivedBuffer_ = buffer->resize(getEnvironment(), totalMessageLength, chunkSize_);
        numChunksReceived_ = 1;
      ...
      if (  // received more chunks than expected
          numChunksReceived_ > numChunksExpected ||
          // the chunk number is out of range
          chunkNum >= numChunksExpected ||
          (  // a chunk other than the first or last that
             // is not of length maxIOSize_
              !lastChunk && receivedDataLength != chunkSize_) ||
          (  // a last chunk that does not have the expected length
              lastChunk && receivedDataLength != totalMessageLength - (numChunksExpected - 1) * chunkSize_)) {
        reportBadMessage();
        assert(0);
      }

      str_cpy_all(partiallyReceivedBuffer_->data(chunkNum * chunkSize_), buffer->data(0), receivedDataLength);

      // must reply with an empty message to secondary request chunks
      controlConnection_->sendReplyData(NULL, 0, buffer->getReplyTag(), this, GuaOK);
      decrNumOutstandingRequests();
      controlConnection_->recycleReceiveBuffer(buffer);

      // We are done receiving the entire message if all the data has arrived.
      if (lastChunk) {
        partiallyReceivedBuffer_->removeChunkHeadersAndTrailer(chunkSize_, env());
        queueReceiveMessage(partiallyReceivedBuffer_);
        setState(ESTABLISHED);
        partiallyReceivedBuffer_ = NULL;
        numChunksReceived_ = 0;
  ...
  if (callbackBuffer) {
    callbackBuffer->callSendCallback(this);
    callbackBuffer->decrRefCount();
  }

  // call receive callbacks for any matching message streams
  IpcMessageBuffer *receiveBuf;
  while (receiveBuf = getNextReceiveQueueEntry()) receiveBuf->callReceiveCallback(this);
...
// -----------------------------------------------------------------------
GuaReceiveControlConnection::GuaReceiveControlConnection(IpcEnvironment *env, short receiveDepth, const char *eye,
                                                         IpcThreadInfo *threadInfo,
                                                         GuaReceiveFastStart *guaReceiveFastStart)
    : IpcControlConnection(IPC_DOM_GUA_PHANDLE, eye, threadInfo),
      mutex_(true, env->isMultiThreaded()),
      clientConnections_(env),
      failedConnections_(env),
  ...
  if (guaReceiveFastStart_ != NULL && guaReceiveFastStart_->open_) {
    ...
    guaErrorInfo_ = BFILE_OPEN_((char *)"$RECEIVE", 8, &receiveFile_,
                                0,                                                      // read-write
                                0,                                                      // shared
                                (short)maxOutstandingIOs_, receiveDepth_, 0, 0, 0, 0);  // no options
  if (guaErrorInfo_ != 0) {
  ...
  // in a multi-threaded IPC environment, the control connection
  // needs to be associated with a thread that will be notified when
  // system messages arrive
  assert(!env_->isMultiThreaded() || threadInfo);
...
IpcConnection *GuaReceiveControlConnection::getConnection() const { return firstClientConnection_; }

GuaReceiveControlConnection *GuaReceiveControlConnection::castToGuaReceiveControlConnection() { return this; }

WaitReturnStatus GuaReceiveControlConnection::wait(IpcTimeout timeout, UInt32 *eventConsumed,
  ...
  // TODO: Check on this, is a problem with or without multi-threading?
  ...
  while (retry) {
    if (initialized_ || timeout != IpcInfiniteTimeout) {
      ...
      if (ipcAwaitiox == NULL || !ipcAwaitiox->getCompleted()) {
        // cms.release(); TODO: uncomment

        stat = BAWAITIOX(&receiveFile_, (void **)&bufferAddr, &countTransferred, &ioTag, timeout, OMIT);
        // cms.reAcquire(); TODO_ uncomment
      } else {
        stat = ipcAwaitiox->ActOnAwaitiox((void **)&bufferAddr, &countTransferred, &ioTag);
      }
      if (_status_ne(stat))
      ...
      while (!done) {
        ...
        if (guaReceiveFastStart_ != NULL && guaReceiveFastStart_->awaitiox_) {
          ...
          stat = BAWAITIOX(&receiveFile_, (void **)&bufferAddr, &countTransferred, &ioTag, newTimeOut);
        }
    ...
    if (!((guaErrorInfo_ == 4004) && !env_->breakEnabled())) {
      ...
      if (retcode != GuaOK) guaErrorInfo_ = retcode;  // not even FILE_GETINFO_ worked

      systemMessageReceived = (guaErrorInfo_ == GuaSysmsgReceived);
      if (systemMessageReceived) msgType = *((short *)bufferAddr);
      ...
      // ---------------------------------------------------------------
      if (guaErrorInfo_ != GuaOK AND guaErrorInfo_ != GuaSysmsgReceived) {
        // Are there any cases where we need to retry? $$$$
  ...
  for (CollIndex i = 0; i < activeReceiveBuffers_.entries() AND receivedBuffer == NULL; i++) {
  ...
  if (countTransferred > 0 && systemMessageReceived == FALSE &&
      // TODO: I see two major and one minor bug here:
      //  a) conn may be NULL (we probably don't get a core dump because
      //     newClientConnection() doesn't access any data members)
      //  b) newClientConnection() looks for an
      //     InternalMsgHdrInfoStruct in the data, but that is
      //     incorrect for multi-chunk messages
      //  c) newClientConnection() (ab)uses the sockReplyTag_ field
    ...
    if (conn) {
      ...
      receiveInfo.phandle_.decompose(cpu, pin, nodeNumber, seqNum);
      env_->closeTrace(__LINE__, receiveInfo.clientFileNumber_, cpu, pin, seqNum);  // Persistent open simulated close
      conn->close();
      clientConnections_ -= conn->getId();
      if (conn == firstClientConnection_) setFirstClientToNull = TRUE;
      // TODO: shouldn't we delete the connection here?
      // delete conn;
    }

    conn = new (env_->getHeap())
        GuaConnectionToClient(env_, IpcProcessId(receiveInfo.phandle_), receiveInfo.clientFileNumber_, this);
    if (firstClientConnection_ == NULL) firstClientConnection_ = conn;
    clientConnections_ += conn->getId();
    ...
    actOnSystemMessage(msgType,
                       NULL,  // buffer Address
                       0,     // count transferred
                       receiveInfo.clientFileNumber_, receiveInfo.phandle_, conn);
    if (threadInfo_) threadInfo_->resume();
  }
  ...
  if (systemMessageReceived) {
    ...
    switch (msgType) {
      case ZSYS_VAL_SMSG_CPUDOWN: {
        ...
        zsys_ddl_smsg_cpudown_def *msg = (zsys_ddl_smsg_cpudown_def *)bufferAddr;

        IpcNodeName myNodeName = IpcProcessId(MyGuaProcessHandle()).getNodeName();
        for (CollIndex i = 0; clientConnections_.nextUsed(i); i++) {
          GuaConnectionToClient *c = clientConnections_.element(i)->castToGuaConnectionToClient();

          if (c != NULL AND c->getOtherEnd().match(myNodeName, msg->z_cpunumber)) {
            ...
            if (c == firstClientConnection_) {
              ...
              if (env_->getAllConnections()->isEsp()) {
      ...
      case ZSYS_VAL_SMSG_REMOTECPUDOWN: {
        ...
        zsys_ddl_smsg_remotecpudown_def *msg = (zsys_ddl_smsg_remotecpudown_def *)bufferAddr;
        ...
        IpcNodeName remoteNodeName(IPC_DOM_GUA_PHANDLE, &msg->z_nodename[1]);

        for (CollIndex i = 0; clientConnections_.nextUsed(i); i++) {
          GuaConnectionToClient *c = clientConnections_.element(i)->castToGuaConnectionToClient();

          if (c != NULL AND c->getOtherEnd().match(remoteNodeName, remoteCpu)) {
            markAsDead(c, GuaClientCpuDown);
            if (c == firstClientConnection_) setFirstClientToNull = TRUE;
          }
        }
      } break;
      ...
      case ZSYS_VAL_SMSG_OPEN: {
        ...
        sendReplyData((IpcMessageBufferPtr)&openReply, controlReceived ? 0 : sizeof(openReply), receiveInfo.replyTag_,
                      NULL, GuaOK);
      ...
      case ZSYS_VAL_SMSG_NODEDOWN: {
        ...
        zsys_ddl_smsg_nodedown_def *msg = (zsys_ddl_smsg_nodedown_def *)bufferAddr;
        ...
        for (CollIndex i = 0; clientConnections_.nextUsed(i); i++) {
          ...
          if (c != NULL AND c->getOtherEnd().match(remoteNodeName)) {
    ...
    if (NOT repliedToSystemMessage) sendReplyData(NULL, 0, receiveInfo.replyTag_, NULL, sysMsgRetcode);
    ...
    if (!(msgType == ZSYS_VAL_SMSG_CLOSE && controlReceived == FALSE && conn == NULL)) {
      ...
      cms.release();
      actOnSystemMessage(msgType, bufferAddr, countTransferred, receiveInfo.clientFileNumber_, receiveInfo.phandle_,
                         conn);
      cms.reAcquire();
      if (threadInfo_) threadInfo_->resume();
    }
    ...
    if (conn != NULL) {
      ...
      if (countTransferred >= sizeof(InternalMsgHdrInfoStruct)) {
        InternalMsgHdrInfoStruct *imhis = (InternalMsgHdrInfoStruct *)bufferAddr;
        conn->env()->addIpcMsgTrace(conn, IpcEnvironment::ACCEPT, (void *)bufferAddr, countTransferred,
                                    (imhis->isLastMsgBuf() ? 1 : 0), imhis->getSeqNum());
#if 0
      ...
      sendReplyData(NULL, 0, receiveInfo.replyTag_, NULL, FEWRONGID);
      recycleReceiveBuffer(receivedBuffer);
      initiateReceive();
    } else {
      // couldn't find the connection to the client
      GuaConnectionToClient *failedConn = findFailedConnection(receiveInfo.clientFileNumber_, receiveInfo.phandle_);
      if (failedConn) {
        // connection is known but has failed, reply to the
        // client with a special error code, ignore errors
        sendReplyData(NULL, 0, receiveInfo.replyTag_, NULL, GuaIpcApplicationErr, FALSE);
        recycleReceiveBuffer(receivedBuffer);
        initiateReceive();
      } else {
        // connection is not known, stop both the client and
        // our own process and possibly take a core dump
        receiveInfo.phandle_.dumpAndStop(true, false);
        ABORT("Couldn't find connection to client");
      }
    }
  }
  if (setFirstClientToNull) firstClientConnection_ = NULL;
  return WAIT_INTERRUPT;
}

void GuaReceiveControlConnection::actOnSystemMessage(short messageNum, IpcMessageBufferPtr /*sysMsg*/,
  ...
  if (getNumRequestors() > 1) {
    if (firstClientConnection_) {
      ...
      if (firstClientConnection_->getOtherEnd() == connection->getOtherEnd().getPhandle())
        ;  // already have a core.
      else
...
void GuaReceiveControlConnection::sendReplyData(IpcMessageBufferPtr data, IpcMessageObjSize size, short replyTag,
#ifdef LOG_RECEIVE
                                                IpcConnection *conn,
#else
                                                IpcConnection *,  // avoid compiler warning
#endif
                                                GuaErrorNumber retcodeToClient, NABoolean failureIsFatal) {
  NAMutexScope ms(mutex_);
  ...
  if ((_status_ne(stat) OR(ULng32) countWritten != size) && failureIsFatal) {
    ...
    if (errcode2 != GuaOK) guaErrorInfo_ = errcode2;
  ...
  activeTransReplyTag_ = GuaInvalidReplyTag;
  if (replyTag == implicitTransReplyTag_) implicitTransReplyTag_ = GuaInvalidReplyTag;
  if (replyTag == userTransReplyTag_) userTransReplyTag_ = GuaInvalidReplyTag;
  ...
  cerr << "Replying with " << countWritten << " bytes, tag " << replyTag << ", err " << retcodeToClient << endl;
#endif
}
...
void GuaReceiveControlConnection::initiateReceive(NABoolean newReceive) {
  ...
  if (numOutstandingIOs_ >= maxOutstandingIOs_ OR numOutstandingIOs_ + numOutstandingRequests_ >= receiveDepth_) return;
  ...
  for (CollIndex i = 0; buffer == NULL AND i < receiveBufferPool_.entries(); i++) {
  ...
  if (buffer == NULL) {
    ...
    buffer = IpcMessageBuffer::allocate(maxIOSize_, maxIOSize_, NULL, heap, 0);
    if (buffer == NULL) {
  ...
  if (guaReceiveFastStart_ != NULL && guaReceiveFastStart_->readUpdate_) {
    ...
    stat = BREADUPDATEX(receiveFile_, (char *)buffer->data(0), (MINOF(buffer->getBufferLength(), maxIOSize_)),
                        &count_read);
  }
...
short *GuaReceiveControlConnection::getOriginalTransaction() { return (short *)&txHandle_; }
void GuaReceiveControlConnection::clearOriginalTransaction() { memset(&txHandle_, 0, sizeof(SB_Transid_Type)); }

GuaConnectionToClient *GuaReceiveControlConnection::findConnection(short openLabel) {
  ...
  if (clientConnections_.contains(openLabel)) {
    return clientConnections_.element(openLabel)->castToGuaConnectionToClient();
  }
  MXTRC("GRCC::findConnection false\n");
  return NULL;
}

GuaConnectionToClient *GuaReceiveControlConnection::findConnection(short clientFileNumber,
                                                                   const GuaProcessHandle &clientPhandle) {
  // search all connections for a match
  for (CollIndex i = 0; clientConnections_.nextUsed(i); i++) {
    GuaConnectionToClient *c = clientConnections_.element(i)->castToGuaConnectionToClient();

    if (c != NULL AND c->thisIsMyClient(clientPhandle, clientFileNumber)) return c;
  }

  return NULL;
}

GuaConnectionToClient *GuaReceiveControlConnection::findFailedConnection(short clientFileNumber,
                                                                         const GuaProcessHandle &clientPhandle) {
  // search all failed connections for a match
  for (CollIndex i = 0; failedConnections_.nextUsed(i); i++) {
    GuaConnectionToClient *c = failedConnections_.element(i)->castToGuaConnectionToClient();
...
void GuaReceiveControlConnection::markAsDead(GuaConnectionToClient *c, GuaErrorNumber gerr) {
  ...
  if (clientConnections_.isEmpty()) getEnv()->notifyNoOpens();
}
...
void GuaReceiveControlConnection::waitForMaster() {
  ...
  if (getConnection() == NULL) {
    char msg[256];
    sprintf(msg, "Server exiting after waiting %d seconds for initial open.", maxWaitTime);
    SQLMXLoggingArea::logExecRtInfo(__FILE__, __LINE__, msg, 0);
    NAExit(0);
  }
...
IpcGuardianServer::IpcGuardianServer(IpcServerClass *serverClass, ComDiagsArea ** /* diags */,
  ...
  parallelOpens_ = parallelOpens, guardianError_ = 0;
  procCreateError_ = 0;
  procCreateDetail_ = 0;
  activeMessage_ = NULL;
...
IpcGuardianServer *IpcGuardianServer::castToIpcGuardianServer() { return this; }

void IpcGuardianServer::stop() {
  if (getServerClass()->getServerType() == IPC_SQLBDRS_SERVER) return;
...
short IpcGuardianServer::workOnStartup(IpcTimeout timeout, NAWNodeSetWrapper *availableNodes, ComDiagsArea **diags,
  ...
  if (serverState_ == INITIAL) {
    ...
    if (allocMethod_ == IPC_LAUNCH_GUARDIAN_PROCESS) {
      ...
      // Guardian WRITEREADX
      launchProcess(availableNodes, diags, diagsHeap);
      ...
      useProcess(diags, diagsHeap, useTimeout);
    } else
    ...
    assert(waitedStartup_ == FALSE);
    launchProcess(availableNodes, diags, diagsHeap);  // Call it a second time
  }
  ...
  if (serverState_ == ERROR_STATE) {
    if (diags && (allocMethod_ != IPC_USE_PROCESS)) {
      if ((!(**diags).contains(-2013)) && (!(**diags).contains(-2012)))  // avoid generating redundant error
      {
        ...
        (**diags) << DgSqlCode(-2013) << DgString0(progFileName_) << DgInt0(guardianError_)
...
void IpcGuardianServer::acceptSystemMessage(const char *sysMsg, int sysMsgLength) {
  ...
  switch (*msgType) {
    case ZSYS_VAL_SMSG_PROCCREATE: {
      zsys_ddl_smsg_proccreate_def *processCreateNowaitMsg = (zsys_ddl_smsg_proccreate_def *)sysMsg;

      procCreateError_ = processCreateNowaitMsg->z_error;
      procCreateDetail_ = processCreateNowaitMsg->z_error_detail;

      if (processCreateNowaitMsg->z_error != 0 AND processCreateNowaitMsg->z_error != 14 /*undef. externals*/) {
        ...
        IpcProcessId serverProcId((const GuaProcessHandle &)processCreateNowaitMsg->z_phandle);
        NABoolean useGuaIpc = TRUE;
        if (useGuaIpc) {
          controlConnection_ = new (getServerClass()->getEnv()->getHeap())
              GuaConnectionToServer(getServerClass()->getEnv(), serverProcId, usesTransactions_, nowaitDepth_,
                                    eye_GUA_CONNECTION_TO_SERVER, parallelOpens_, NULL, FALSE);
    ...
    case ZSYS_VAL_SMSG_PROCSPAWN: {
      zsys_ddl_smsg_procspawn_def *processSpawnNowaitMsg = (zsys_ddl_smsg_procspawn_def *)sysMsg;
      ...
      if (processSpawnNowaitMsg->z_errno != 0) {
        ...
        // process was successfully created, now create a connection to it
        IpcProcessId serverProcId((const GuaProcessHandle &)processSpawnNowaitMsg->z_phandle);
...
void NewProcessCallback(SB_Phandle_Type *newPhandle, MS_Mon_NewProcess_Notice_def *newProcNotice) {
  ...
  *nowaitedStartupCompleted = TRUE;
  ...
  //   if (nowaitedEspServer->startTag_ == nowaitedEspServer->callbackCount_ && nowaitedEspServer->completionCount_ ==
  //   0)
  if (nowaitedEspServer->waiting_) pthread_cond_signal(&nowaitedEspServer->cond_cond_);
  pthread_mutex_unlock(&nowaitedEspServer->cond_mutex_);
  ESP_TRACE2("CB: Rls_m, tag: %p\n", nowaitedEspStartup);
}

void IpcGuardianServer::launchNSKLiteProcess(NAWNodeSetWrapper *availableNodes, ComDiagsArea **diags,
                                             CollHeap *diagsHeap) {
  ...
  // Some notes on retry loops:
  //
  // There are three levels of retry loops for process create failures.
  //
  // The lowest level is on waited process creation. If the process
  // cannot be created due to a file system error (that is, we cannot
  // access the executable), we will retry. This happens entirely
  // within a single call to this method.
  //
  // For nowait process creation (today used for ESPs only), there is a
  // race between this method and the call back function. The file system
  // error is detected in the call back function. It may happen that the
  // call back function executes before we check for a process creation
  // error in this method call, in which case the lowest level retry loop
  // is attempted. If the race turns out the other way, though, we don't
  // discover the file system error until this method is called a second
  // time to complete the process creation processing. (The second call
  // to this method is redriven some layers higher in the call stack, in
  // ExRtFragTable::assignEsps.) In that case, this method will call
  // itself to retry initiating a nowaited process creation and return.
  // The higher layer will redrive yet again, calling this method once
  // more to complete the retry.
  //
  // At this lowest and middle levels, we retry at most sv_retry_limit times.
  // For tdm_arkcmp and tdm_arkesp processes, we will attempt the retry on
  // a different node if one is available. For permanent file system
  // errors (e.g. the executable directory is damaged), this will allow
  // processing to continue. For other processes, there are considerations
  // that prevent us from using a different node. For example, tdm_udrserv
  // logic caches jar files on the target node before this method is
  // invoked; starting tdm_udrserv on another node may fail because the
  // jar has not been cached there. (Refactoring that design to allow a
  // callback for recaching would address that issue.) For another example,
  // the ssmp and sscp processes need to access a shared segment, and
  // that is node-specific. Starting this process on another node would
  // get the wrong shared segment. So, retries at these levels for
  // processes other than tdm_arkcmp and tdm_arkesp will at best be able
  // to recover from transient file system errors only.
  //
  // At the highest level, there is a retry loop in the CLI AQR logic.
  // Process creation failures at this level result in a SQL error 2012,
  // which is a retryable error. Error 2012 happens for file system
  // errors and for certain other monitor conditions that are often
  // transient.
  //
  // The number of retries at this highest level is determined by the AQR
  // table entry for error 2012.
  //
  // The lowest and middle loops together retry at most sv_retry_limit times
  // to this method. AQR retries happen independently, so the effect on the
  // total number of retries is multiplicative.
...
#define SET_ARGV(argv, argc, argval)                    \
  {                                                     \
    argv[argc] = (char *)calloc(strlen(argval) + 1, 1); \
    ...
    else if (serverState_ == RETRYING)
      p_pe = (short)actualCpuNum_;  // start with CPU we last tried
    else
      p_pe = (short)cpuNum_;
    ...
    // -----------------------------------------------------------------
    if (overridingDefineForProgFile_) environmentName = getenv(overridingDefineForProgFile_);
    if (environmentName == NULL) {
      ...
      if ((strcmp(className_, "arkesp") == 0) || (strcmp(className_, "arkespdbg") == 0)) {
        ...
        if (fastStartArg == NULL || *fastStartArg != '1') SET_ARGV(largv, largc, "-noespfaststart");
        strcpy(progFileName, "tdm_arkesp");
        retryOnAlternativeNodeOK_ = TRUE;
      } else if ((strcmp(className_, "arkcmp") == 0) || (strcmp(className_, "arkcmpdbg") == 0)) {
        ...
        if (!sv_launch_cmp_unhooked_checked) {
          ...
          if ((lv_launch_unhooked != NULL) && (*lv_launch_unhooked == '1')) sv_launch_cmp_unhooked = true;
          sv_launch_cmp_unhooked_checked = true;
        }
        launch_hooked_special = !sv_launch_cmp_unhooked;
        retryOnAlternativeNodeOK_ = TRUE;
      } else if ((strcmp(className_, "arkcat") == 0) || (strcmp(className_, "arkcatdbg") == 0))
        strcpy(progFileName, "arkcat.exe");
      else if ((strcmp(className_, "arkustat") == 0) || (strcmp(className_, "arkustatdbg") == 0))
        strcpy(progFileName, "arkustat.exe");
      else if ((strcmp(className_, "udrserv") == 0) || (strcmp(className_, "udrservdbg") == 0)) {
        SET_ARGV(largv, largc, "tdm_udrserv");
        strcpy(progFileName, "tdm_udrserv");
      } else if ((strcmp(className_, "qms") == 0) || (strcmp(className_, "qmsdbg") == 0)) {
        SET_ARGV(largv, largc, "tdm_arkqms");
        strcpy(progFileName, "tdm_arkqms");
      } else if ((strcmp(className_, "qmp") == 0) || (strcmp(className_, "qmpdbg") == 0)) {
        SET_ARGV(largv, largc, "tdm_arkqmp");
        strcpy(progFileName, "tdm_arkqmp");
      } else if ((strcmp(className_, "qmm") == 0) || (strcmp(className_, "qmmdbg") == 0)) {
        SET_ARGV(largv, largc, "tdm_arkqmm");
        strcpy(progFileName, "tdm_arkqmm");
      } else if (strcmp(className_, "bdrr") == 0) {
        ...
        if (cpuNum_ != IPC_CPU_DONT_CARE) {
          ...
          if (msg_mon_get_node_info_detail(cpuNum_, &info) != XZFIL_ERR_OK)
            p_pe = -1;
          else if (info.num_returned < 1 || !(info.node[0].type & MS_Mon_ZoneType_Edge))
            p_pe = -1;
          else
            p_pe = cpuNum_;
        } else
        ...
        if (diags) {
          IpcAllocateDiagsArea(*diags, diagsHeap);
          (**diags) << DgSqlCode(-2011) << DgInt0(FEBADNAME) << DgString0(className_) << DgNskCode(FEBADNAME);
        }
    ...
    if (debugServer_) SET_ARGV(largv, largc, "-debug");

    if (getServerClass()->isMultiThreaded()) SET_ARGV(largv, largc, "-multiThread");
    ...
    process_name[0] = 0;
    pwd = getenv("PWD");
    if (strlen(pwd) + 1 + strlen(progFileName) + 1 < MS_MON_MAX_PROCESS_PATH) {
      strcpy(prog, pwd);
      strcat(prog, "/");
    ...
    if (!sv_cmp_node_id_checked) {
      ...
      if ((lv_cmp_node_id != NULL) && (*lv_cmp_node_id == '1')) sv_cmp_node_id_mine = true;
    ...
    if (sv_retry_limit == -1) {
      sv_retry_limit = 2;  // set default value
      const char *lv_retry_limit = getenv("PROCESS_LAUNCH_RETRY_LIMIT");
      if (lv_retry_limit != NULL) {
        sv_retry_limit = atoi(lv_retry_limit);
        if (sv_retry_limit < 1) sv_retry_limit = 1;  // insure at least one retry
      }
    }
    ...
    if (launch_hooked_special)
      ...
      if (!sv_launch_unhooked_checked) {
        ...
        if ((lv_launch_unhooked != NULL) && (*lv_launch_unhooked == '1')) sv_launch_unhooked = true;
        sv_launch_unhooked_checked = true;
      }
      unhooked_ = sv_launch_unhooked;
    }

    bool retryStartProcess = true;
    while (retryStartProcess) {
      // if we are retrying, do so on a different node if that is allowed
      if ((serverState_ == RETRYING) && retryOnAlternativeNodeOK_) {
        if (availableNodes)  // if we have node information, find another node
          server_nid = availableNodes->findAlternativeNode(server_nid);
        else
      ...
      if (waitedStartup_ == FALSE) {
        ...
        returnValue = msg_mon_start_process_nowait_cb2(NewProcessCallback, prog, /* prog */
                                                       process_name,             /* name */
        ...
        actualCpuNum_ = server_nid;                                // save requested CPU (might be IPC_CPU_DONT_CARE)
        returnValue = msg_mon_start_process2(prog,                 /* prog */
                                             process_name,         /* name */
                                             process_name,         /* output process name */
                                             largc,                /* args */
                                             largv, &p_phandle, 0, /* open */
                                             &server_oid,          /* oid */
                                             processType,          /* process type */
                                             0,                    /* priority */
                                             0,                    /* debug */
      ...
      // if we could not create the process due to a file system error,
      // try again on a different node (unless we are already retrying recursively)
      if (((serverState_ == INITIAL) || ((serverState_ == RETRYING) && (numberOfRetries_ < sv_retry_limit))) &&
          (returnValue == XZFIL_ERR_FSERR) && (server_nid != -1)) {
        serverState_ = RETRYING;
        numberOfRetries_++;
    ...
    assert(serverState_ == CREATING_PROCESS && nowaitedEspStartup_.nowaitedEspServer_->waitedStartupArg_ != '1');
    ...
    while (nowaitedStartupCompleted_ == FALSE) {
      ...
      int sv_timeout = 30;  // in seconds
      const char *lv_timeout = getenv("PROCESS_LAUNCH_TIMEOUT");
      if (lv_timeout != NULL) {
        sv_timeout = atoi(lv_timeout);
        if (sv_timeout < 0) sv_timeout = 30;
      }

      struct timespec waitTime;
      clock_gettime(CLOCK_REALTIME, &waitTime);
      waitTime.tv_sec += sv_timeout;

      nowaitedEspStartup_.nowaitedEspServer_->waiting_ = TRUE;
      int condRet = pthread_cond_timedwait(&nowaitedEspStartup_.nowaitedEspServer_->cond_cond_,
                                           &nowaitedEspStartup_.nowaitedEspServer_->cond_mutex_, &waitTime);
      if (condRet == ETIMEDOUT) {
        ESP_TRACE1("MT: start process exceeds 30 seconds\n");

        if (diags) {
          IpcAllocateDiagsArea(*diags, diagsHeap);
          (**diags) << DgSqlCode(-2023) << DgString0(progFileName_) << DgString1("TIMEOUT OCCURRED");
        }
    ...
    if (nowaitedEspStartup_.nowaitedEspServer_->startTag_ == nowaitedEspStartup_.nowaitedEspServer_->completionCount_) {
      assert(nowaitedEspStartup_.nowaitedEspServer_->startTag_ ==
             nowaitedEspStartup_.nowaitedEspServer_->callbackCount_);
      nowaitedEspStartup_.nowaitedEspServer_->startTag_ = nowaitedEspStartup_.nowaitedEspServer_->callbackCount_ =
  ...
  if (waitedStartup_ OR(procCreateError_ != NO_ERROR) OR nowaitedStartupCompleted) {
    // if on a no-waited startup, we found a retryable error after callback,
    // do the retry via a recursive call
    if ((waitedStartup_ == FALSE) && (procCreateError_ == XZFIL_ERR_FSERR) && (numberOfRetries_ < sv_retry_limit)) {
      serverState_ = RETRYING;
      numberOfRetries_++;
      launchNSKLiteProcess(availableNodes, diags, diagsHeap);
      return;  // we will be redriven by our caller
    }
    ...
    acceptSystemMessage((const char *)&sysmsg, sizeof(sysmsg));
...
void IpcGuardianServer::launchProcess(NAWNodeSetWrapper *availableNodes, ComDiagsArea **diags, CollHeap *diagsHeap) {
  launchNSKLiteProcess(availableNodes, diags, diagsHeap);
  return;
}

void IpcGuardianServer::spawnProcess(ComDiagsArea **diags, CollHeap *diagsHeap) {
  launchNSKLiteProcess(NULL, diags, diagsHeap);
  return;
}

void IpcGuardianServer::useProcess(ComDiagsArea **diags, CollHeap *diagsHeap, NABoolean useTimeout) {
  ...
  while (i < 3) {
    ...
    if (rc != FEOK) {
      ...
      if (diags) {
        IpcAllocateDiagsArea(*diags, diagsHeap);
        (**diags) << DgSqlCode(-2024) << DgString0(processName_) << DgInt0(rc);
      }
      return;
    } else {
      // Phandle wrapper in porting layer
    ...
    if (useTimeout)
      controlConnection_ = new (getServerClass()->getEnv()->getHeap())
          GuaConnectionToServerTimeout(getServerClass()->getEnv(), serverProcId, usesTransactions_, nowaitDepth_);
    else
      controlConnection_ = new (getServerClass()->getEnv()->getHeap())
          GuaConnectionToServer(getServerClass()->getEnv(), serverProcId, usesTransactions_, nowaitDepth_);
    if (controlConnection_->getState() == IpcConnection::ERROR_STATE) {
      i++;
      guaError = controlConnection_->castToGuaConnectionToServer()->getGuardianError();
  ...
  if (controlConnection_ != NULL)
    ...
    if (diags) {
      IpcAllocateDiagsArea(*diags, diagsHeap);
      (**diags) << DgSqlCode(-2024) << DgString0(processName_) << DgInt0(guardianError_);
    }
  }
}

short IpcGuardianServer::changePriority(IpcPriority priority, NABoolean isDelta) { return 0; }
...
void IpcGuardianServer::populateDiagsAreaFromTPCError(ComDiagsArea *&diags, CollHeap *diagsHeap) {
  IpcAllocateDiagsArea(diags, diagsHeap);
  ...
  switch (procCreateError_) {
    ...
    case XZFIL_ERR_DEVERR:      // 190
      ...
      (*diags) << DgSqlCode(-2012) << DgInt0(guardianError_) << DgInt1(procCreateError_) << DgInt2(procCreateDetail_)
               << DgNskCode(guardianError_);
      break;

    default:
      (*diags) << DgSqlCode(-2013) << DgInt0(procCreateError_) << DgNskCode(procCreateError_);
      break;
  }
  ...
  if (interpretiveText) (*diags) << DgString2(interpretiveText);
}
...
void IpcGuaLogTimestamp(IpcConnection *conn) {
  ...
  if (!useGuaIpc AND conn AND conn->castToGuaConnectionToServer()) {
    fromto = " to ";
    fno = conn->castToGuaConnectionToServer()->getFileNumForLogging();
  } else if (conn AND conn->castToGuaConnectionToClient()) {
    fromto = " from ";
    fno = conn->castToGuaConnectionToClient()->getFileNumForLogging();
  } else
    fromto = " <-> ";
