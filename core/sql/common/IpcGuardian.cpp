...
//
WaitReturnStatus GuaConnectionToServer::wait(IpcTimeout timeout,
        ...
        if (_status_ne(stat))
          {
            ...
            if (retcode != 0)
              guardianErrNum = retcode; // not even FILE_GETINFO_ worked
              ...
              if (entry.offset_ == 0)
                {
                  ...
                  IpcMessageObjSize msgLen =
                    IpcMessageBuffer::getLengthWithChunkHeaders(
                         maxIOSize_,
                         msgHdr->getMsgLengthFromData());
                  ...
                  if (msgLen == (IpcMessageObjSize) countRead)
                    {
                      ...
                      if (bytesSent > maxIOSize_)
                        entry.buffer_ = entry.buffer_->resize(getEnvironment(),
                                                              msgLen,
                                                              maxIOSize_);
              ...
              else
                {
                  ...
                  entry.buffer_->validateChunkHeader(
                       entry.offset_,
                       countRead,
                       maxIOSize_,
                       TRUE);
                  ...
                  if (numChunksReceived_ ==
                      DIVIDE_AND_ROUND_UP(entry.buffer_->getMessageLength(),
                                          maxIOSize_))
                    {
...
NABoolean GuaConnectionToServer::tryToStartNewIO()
{
  ...
  if (partiallyReceivedBuffer_ &&
      DIVIDE_AND_ROUND_UP(partiallyReceivedBuffer_->getMessageLength(),
                          maxIOSize_) == numChunksRequested_)
  {
    // if partially received buffer not handled, then block any other messages
    // to keep waiting for the rest messages
    if (NOT partiallyRecvProcessed())
      return FALSE;

    if (sendQueueEntries() == 0)
      return FALSE;
  }
  ...
  if (partiallySentBuffer_)
  ...
  else
    {
      ...
      nextToSend =
        nextToSend->addChunkHeadersAndTrailer(maxIOSize_,
                                              getEnvironment());
      ...
      if (entry.buffer_->getRefCount() > 1)
        {
          ...
          entry.readBuffer_ = entry.buffer_->
            copy(getEnvironment(), maxIOSize_, maxIOSize_, FALSE, TRUE);
...
void GuaConnectionToServer::openPhandle(char * processName, NABoolean parallelOpen)
{
  ...
  if (openFlags & 0x4000) // Nowaited FILE_OPEN_
  {
    ...
    if (getState() == INITIAL ||
        env->getAllConnections()->isEsp() == FALSE)
    {
      if (tscoOpen_)
  ...
  for (unsigned short i = 0; i < nowaitDepth_; i++)
    {
      ActiveIOQueueEntry &entry = activeIOs_[i];
      if (entry.inUse_)
        handleIOErrorForEntry(entry, cancelReq);
    }
...
NABoolean GuaConnectionToServer::partiallyRecvProcessed()
{
  return FALSE;
}
...
// -----------------------------------------------------------------------
GuaConnectionToServerTimeout::GuaConnectionToServerTimeout(
     IpcEnvironment *env,
     const IpcProcessId &procId,
     NABoolean usesTransactions,
     unsigned short nowaitDepth,
     ...
     : GuaConnectionToServer(env,procId,usesTransactions, nowaitDepth, eye,
                             parallelOpen, openCompletionScheduled, dataConnectionToEsp)
{
  ...
  if (partialReceiveTimeout)
  {
     ...
     if (timeout > 0)
       partiallyReceiveTimeout_ = timeout;
  }
}

void GuaConnectionToServerTimeout::partiallyRecvSet(IpcMessageBuffer *msg,
                                                       Int32 requested,
                                                       Int32 received)
{
...
NABoolean GuaConnectionToServerTimeout::partiallyRecvProcessed()
{
  ...
  if ( curr.tv_sec - partiallyReceiveTime_.tv_sec > partiallyReceiveTimeout_ )
  {
...

    {
      char msg[256];
      sprintf(msg,
              "WARNING!! partially received data abandoned because partial receive progress exceeds timeout.");
    ...
    if (bytesToSend > buffer->getMaxReplyLength())
      {
        ...
        // resizing the buffer
        buffer = buffer->addChunkHeadersAndTrailer(chunkSize_,
                                                   getEnvironment());
...

  {
    ...
    if (receivedDataLength == 0)
      {
        if (partiallyRepliedBuffer_ != NULL AND
        ...
        else 
          {
            ...
            assert(partiallyRepliedBuffer_ != NULL AND
                   partiallyReceivedBuffer_ == NULL);
          }
    ...
    // ---------------------------------------------------------------------
    else if (partiallyReceivedBuffer_ == NULL)
      {
        ...
        totalMessageLength =
          buffer->getLengthWithChunkHeaders(
               chunkSize_,
               msgHdr->getMsgLengthFromData());
    ...
    else
      {
        ...
        Int32 numChunksExpected =
          DIVIDE_AND_ROUND_UP(totalMessageLength, chunkSize_);
        ...
        if (// received more chunks than expected
            ...
            (// a last chunk that does not have the expected length
             lastChunk &&
             receivedDataLength !=
                totalMessageLength - (numChunksExpected - 1) * chunkSize_))
          {
            reportBadMessage();
            assert(0);
          }

        str_cpy_all(partiallyReceivedBuffer_->data(chunkNum*chunkSize_),
                    buffer->data(0),
                    receivedDataLength);
        ...
        if (lastChunk)
          {
            partiallyReceivedBuffer_->removeChunkHeadersAndTrailer(
                 chunkSize_,
                 env());
            queueReceiveMessage(partiallyReceivedBuffer_);
            setState(ESTABLISHED);
...
WaitReturnStatus GuaReceiveControlConnection::wait(IpcTimeout timeout, UInt32 *eventConsumed, IpcAwaitiox *ipcAwaitiox)
{
  ...
  // ---------------------------------------------------------------------
  GuaConnectionToClient *conn = findConnection(receiveInfo.clientFileNumber_,
                                               receiveInfo.phandle_);
  if (countTransferred > 0 &&
      systemMessageReceived == FALSE &&
...
//
      ...
      else
	{
          ...
          GuaConnectionToClient *failedConn =
            findFailedConnection(receiveInfo.clientFileNumber_,
                                 receiveInfo.phandle_);
          if (failedConn)
            {
              ...
              sendReplyData(NULL,0,receiveInfo.replyTag_,
                            NULL,GuaIpcApplicationErr,FALSE);
              recycleReceiveBuffer(receivedBuffer);
              initiateReceive();
            }
          else
  ...
  if (buffer == NULL)
    {
      ...
      buffer = IpcMessageBuffer::allocate(maxIOSize_,
                                          maxIOSize_,
                                          NULL,
                                          heap,
                                          0);
...
GuaConnectionToClient * GuaReceiveControlConnection::findFailedConnection(
     short                  clientFileNumber,
     const GuaProcessHandle &clientPhandle)
{
  ...
  for (CollIndex i = 0; failedConnections_.nextUsed(i); i++)
    {
      GuaConnectionToClient *c =
	failedConnections_.element(i)->castToGuaConnectionToClient();
...
IpcGuardianServer::IpcGuardianServer(
     ...
     NABoolean      parallelOpens) : IpcServer(NULL,
					       serverClass), 
numberOfRetries_(0),retryOnAlternativeNodeOK_(FALSE)
{
...
short IpcGuardianServer::workOnStartup(IpcTimeout timeout,
				       NAWNodeSetWrapper *availableNodes,
				       ComDiagsArea **diags,
				       CollHeap *diagsHeap,
				       NABoolean useTimeout)
{
...
void IpcGuardianServer::launchNSKLiteProcess(NAWNodeSetWrapper * availableNodes,
				     ComDiagsArea **diags,
				     CollHeap *diagsHeap)
{
...
//NGG
  ...
  // nowait tag
    ...
    if (getServerClass()->isMultiThreaded())
      SET_ARGV(largv, largc, "-multiThread");
    ...
    if (sv_retry_limit == -1)
    {
      ...
      if (lv_retry_limit != NULL)
      {
        ...
        if (sv_retry_limit < 1)
          sv_retry_limit = 1;  // insure at least one retry
      }
    }
    ...
    {  
      ...
      else
      {
        ...
        actualCpuNum_ = server_nid;  // save requested CPU (might be IPC_CPU_DONT_CARE)
        returnValue = msg_mon_start_process2(
			    prog,           /* prog */
			    process_name,   /* name */
			    process_name,   /* output process name */
			    largc,          /* args */
   ...
   else
   {
       ...
       while (nowaitedStartupCompleted_ == FALSE)
       {
         ...
         int condRet = pthread_cond_timedwait(&nowaitedEspStartup_.nowaitedEspServer_->cond_cond_,
                                              &nowaitedEspStartup_.nowaitedEspServer_->cond_mutex_,
                                              &waitTime);
...
void IpcGuardianServer::launchProcess(NAWNodeSetWrapper *availableNodes, 
				      ComDiagsArea **diags,
				      CollHeap *diagsHeap)
{
  launchNSKLiteProcess(availableNodes,diags,diagsHeap);
  ...
  while (i < 3)
  {
    ...
    if (useTimeout)
      controlConnection_ = new(getServerClass()->getEnv()->getHeap())
        GuaConnectionToServerTimeout(getServerClass()->getEnv(),
                          serverProcId,
                          usesTransactions_,
                          nowaitDepth_);
    else
