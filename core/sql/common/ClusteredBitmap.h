...
{
public:
   ClusteredBitmapIterator(ClusteredBitmap& x): 
     bitMap_(x) 
     { 
        init(); 
     };
...
public:
   ...
   RangeOfValues& operator+= (const RangeOfValues& other)
      { 
         const ClusteredBitmapForIntegers* castedOther = 
             dynamic_cast<const ClusteredBitmapForIntegers*>(&other);
   ...
   { 
      ...
      return bitMapPos_ == castedOther->bitMapPos_ && 
             bitMapNeg_ == castedOther->bitMapNeg_; 
   }

   RangeOfValues* clone(UInt32 packedSpaceCap, CollHeap *heap)
     { return new (heap) 
              ClusteredBitmapForIntegers(packedSpaceCap, heap); }
   ...
   void clear() 
     { bitMapPos_.clear(); bitMapNeg_.clear(); };

   UInt32 totalSize();

   UInt32 getPackedLength()
     { return RangeOfValues::getPackedLength() + 
              bitMapPos_.getPackedLength() + bitMapNeg_.getPackedLength(); 
     };
...
public:
   ...
   RangeOfValues* clone(UInt32 packedSpaceCap, CollHeap *heap)
     { return new (heap) RangeSpecRT(packedSpaceCap, heap); }
   ...
   UInt32 getPackedLength() 
   { return RangeOfValues::getPackedLength() + rs_.getPackedLength(); };
   ...
   static UInt32 minPackedLength()
       { return RangeOfValues::minPackedLength() + 4; }
...
public:
   ...
   RangeOfValues* clone(UInt32 packedSpaceCap, CollHeap *heap)
     { return new (heap) BloomFilterRT(packedSpaceCap, heap); }
   ...
   UInt32 entries(NABoolean estimate=FALSE) 
      { return rbf_.entries(estimate); } ;

   void clear() { rbf_.clear(); }

   NABoolean isUseful(Int32 maxNumEntries = -1) 
       { return rbf_.isUseful(maxNumEntries); }

   UInt32 getPackedLength() 
   { return RangeOfValues::getPackedLength() + rbf_.getPackedLength(); }
   ...
   static UInt32 minPackedLength()
       { return RangeOfValues::minPackedLength() +
                  RegularBloomFilter::minPackedLength(); }
...
public:
   NativeBloomFilterRT(UInt32 packedSpaceCap, CollHeap *heap)
    : BloomFilterRT(packedSpaceCap, heap) {}
   ~NativeBloomFilterRT() {};
