...
{
   if ( idx_ < 0 || idx_ >= bitMap_.numBitmaps_ )
     return NULL;

   return &bitMap_.bitmapMap_[idx_];
}

void ClusteredBitmapIterator::advance()
{
   idx_++;
}

void 
ClusteredBitmap::dump(ostream& out, const bitsToBitmap& bmap, const char* title)
{
  if ( title )
     out << title;
  ...
  for (Int32 wordIdx=0; wordIdx < ClusteredBitmap::WORDS_PER_BITMAP; wordIdx++)
  {
      ...
      while (wordVal != 0)
      {
        ...
        UInt32 value = bmap.significantBits_ + (wordIdx << WORD_BITS_SHIFT) +
                bitIdx;
...
NABoolean
ClusteredBitmap::convertToList(std::string& text, const bitsToBitmap& bmap)
{
  char buf[100];
  for (Int32 wordIdx=0; wordIdx < ClusteredBitmap::WORDS_PER_BITMAP; wordIdx++)
  {
...
UInt32 ClusteredBitmap::getPackedLength()
{
  ...
  if ( numBitmaps_ > 0 ) {
     ...
     // objects.
     sz += numBitmaps_ * 
           (sizeof(bitmapMap_[0].significantBits_) + WORDS_PER_BITMAP*sizeof(cb_int_t));
  }
...
/////////////////////////////////////////////////////////
RangeOfValues::RangeOfValues(UInt32 cap, CollHeap *heap)
:  totalSizeCap_(cap), enabled_(TRUE), heap_(heap)
{
}
...
NABoolean
ClusteredBitmapForIntegers::insert(Int32 value)
{
   if ( value >= 0 )
...
NABoolean
ClusteredBitmapForIntegers::insert(UInt32 value)
{
   bitMapPos_.insert(value);
...
RangeOfValues& 
ClusteredBitmapForIntegers::remove(Int32 value)
{
   if ( value >= 0 )
...
UInt32 ClusteredBitmapForIntegers::entries(NABoolean estimate)
{
  return bitMapPos_.entries() + bitMapNeg_.entries();
}

UInt32 ClusteredBitmapForIntegers::totalSize()
{
  return bitMapPos_.totalSize() + bitMapNeg_.totalSize();
}
...
/////////////////////////////////////////////////////////
RangeSpecRT::RangeSpecRT(UInt32 cap, CollHeap *heap)
...
///////////////////////////////////////////////////////////////////
NABoolean RangeSpecRT::lookup(Int32 value)
{
   return rs_.lookup(value);
}

NABoolean RangeSpecRT::lookup(UInt32 value)
{
   return rs_.lookup(value);
}

NABoolean RangeSpecRT::lookup(Int64 value)
{
   return rs_.lookup(value);
}

NABoolean RangeSpecRT::lookup(char* ptr, Lng32 len)
{
   return rs_.lookup(ptr, len);
}

NABoolean RangeSpecRT::lookup(wchar_t* ptr, Lng32 len)
{
   return rs_.lookup(ptr, len);
}
...
RangeOfValues& 
RangeSpecRT::remove(Int32 value)
{
   return *this;
}
...
: RangeOfValues(cap, heap)
  , rbf_((NAHeap*)heap, 
         (cap > BloomFilterRT::minPackedLength()) ? 
             cap - BloomFilterRT::minPackedLength() : 0)
{
   if ( rbf_.numBytes() == 0 )
       setEnable(FALSE);
}
...
NABoolean BloomFilterRT::insertDate(char* ptr, Lng32 len)
{
   return rbf_.insert(ptr, len);
}

NABoolean BloomFilterRT::insertTime(char* ptr, Lng32 len)
{
   return rbf_.insert(ptr, len);
}

NABoolean BloomFilterRT::insertTimestamp(char* ptr, Lng32 len)
{
   return rbf_.insert(ptr, len);
}
...
NABoolean BloomFilterRT::lookup(char* ptr, Lng32 len)
{
   return rbf_.contain(ptr, len);
}
...
NABoolean BloomFilterRT::lookupDate(char* ptr, Lng32 len)
{
   return rbf_.contain(ptr, len);
}

NABoolean  BloomFilterRT::lookupTime(char* ptr, Lng32 len)
{
   return rbf_.contain(ptr, len);
}

NABoolean BloomFilterRT::lookupTimestamp(char* ptr, Lng32 len)
{
   return rbf_.contain(ptr, len);
}
////////////////////////////////////////////////////////////
...
NABoolean BloomFilterRT::operator==(const RangeOfValues& other) const
{ 
   return FALSE;
}
...
{ 
   return computeHashTableSizeInBytes(m, p) + 
...
NABoolean NativeBloomFilterRT::insert(UInt32 value)
{
   return rbf_.insert((char*)&value, sizeof(value));
}

NABoolean  NativeBloomFilterRT::insert(Int64 value)
{
   return rbf_.insert((char*)&value, sizeof(value));
}
...
NABoolean NativeBloomFilterRT::lookup(Int32 value)
{
   return rbf_.contain((char*)&value, sizeof(value));
}

NABoolean NativeBloomFilterRT::lookup(UInt32 value)
{
   return rbf_.contain((char*)&value, sizeof(value));
}

NABoolean NativeBloomFilterRT::lookup(Int64 value)
{
   return rbf_.contain((char*)&value, sizeof(value));
}
