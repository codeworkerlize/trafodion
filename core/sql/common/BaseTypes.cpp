...
void NAExit(int status) {
  ...
  if (status != 0) {
    ...
    if (abortOnError != NULL)
      abort();
    else if (IdentifyMyself::GetMyName() == I_AM_EMBEDDED_SQL_COMPILER)
...
//
void NAError_stub_for_breakpoints() { /*** delete cmpCurrentContext; cmpCurrentContext = NULL; ***/
}

// A helper function to convert a given datetime field to its Text
const char *dtFieldToText(rec_datetime_field field) {
  switch (field) {
    case REC_DATE_YEAR:
      return "YEAR";
    case REC_DATE_MONTH:
...
void getDatetimeQualifierAsString(char *text,                       // OUTPUT
...
short getDateTimeTypeText(char *text,                     // OUTPUT
                          rec_datetime_field startField,  // INPUT
                          rec_datetime_field endField,    // INPUT
                          UInt32 fractionPrecision)       // INPUT
{
    ...
    case REC_DATE_YEAR:
      switch (endField) {
        case REC_DATE_DAY:  // YEAR TO DAY == DATE
          ...
          text += str_len("DATE");
          getDatetimeQualifierAsString(text, startField, endField, fractionPrecision,
...
short getIntervalTypeText(char *text,                        // OUTPUT
                          ...
                          UInt32 fractionPrecision)          // INPUT
{
  if (datetimestart >= REC_DATE_SECOND) {
    str_sprintf(text, "INTERVAL %s(%u,%u)", dtFieldToText(datetimestart), intervalleadingprec, fractionPrecision);
  } else if (datetimestart == datetimeend) {
    str_sprintf(text, "INTERVAL %s(%u)", dtFieldToText(datetimestart), intervalleadingprec);
  } else {
    if (datetimeend < REC_DATE_SECOND) {
      str_sprintf(text, "INTERVAL %s(%u) TO %s", dtFieldToText(datetimestart), intervalleadingprec,
                  dtFieldToText(datetimeend));
    } else {
      str_sprintf(text, "INTERVAL %s(%u) TO %s(%u)", dtFieldToText(datetimestart), intervalleadingprec,
                  dtFieldToText(datetimeend), fractionPrecision);
    }
  }
...
short convertTypeToText_basic(char *text,         // OUTPUT
  ...
  const char *varcharStr = isVarchar2 ? "VARCHAR2" : "VARCHAR";
  ...
  switch (fs_datatype) {
    case REC_BIN64_SIGNED:
      if ((!precision) && (scale == 0))
        ...
        if (!precision)
          str_sprintf(text, "NUMERIC(%d, %d)", 18 /*MAX_NUMERIC_PRECISION*/, scale);
        else
    ...
    case REC_BIN64_UNSIGNED:
      if ((!precision) && (scale == 0))
        ...
        if (!precision)
          str_sprintf(text, "NUMERIC(%d, %d) UNSIGNED", 18 /*MAX_NUMERIC_PRECISION*/, scale);
        else
    ...
    case REC_BYTE_F_ASCII:
      ...
      if (CharInfo::is_NCHAR_MP(charSet))
        ...
        if (displaydatatype && strlen(displaydatatype) > 0) {
          str_sprintf(text, "%s", displaydatatype);
          if (str_str(displaydatatype, " CHARACTER SET ")) addCharSet = 0;
        } else {
...
#ifdef IS_MP
      if (CharInfo::is_NCHAR_MP(charSet))
        str_sprintf(text, "%s(%d)", varcharStr, length / 2);
      else
#endif
          if (charSet == CharInfo::UTF8 || charSet == CharInfo::SJIS) {
        if (displaydatatype && strlen(displaydatatype) > 0) {
          ...
          {
            if (precision /*i.e., charlen*/ == 1)
              str_sprintf(text, "%s(1 CHAR)", varcharStr);
            else
              str_sprintf(text, "%s(%d CHARS)", varcharStr, precision /*i.e., charlen*/);
          } else  // precision == 0 (char len unit is BYTE[S])
          {
            if (length /*in_bytes*/ == 1)
              str_sprintf(text, "%s(1 BYTE)", varcharStr);
            else
              str_sprintf(text, "%s(%d BYTES)", varcharStr, length /*in_bytes*/);
          }
        }
      } else
        str_sprintf(text, "%s(%d)", varcharStr, length);

      addCollate = 1;
      break;

    case REC_BYTE_V_DOUBLE:
      str_sprintf(text, "%s(%d)", varcharStr, length / SQL_DBCHAR_SIZE);
      addCharSet = 1;
      addCollate = 1;
      break;
    ...
    case REC_BYTE_V_ASCII_LONG:
      ...
      if (charSet == CharInfo::UTF8 || charSet == CharInfo::SJIS) {
        if (displaydatatype && strlen(displaydatatype) > 0) {
          ...
          {
            if (precision /*i.e., charlen*/ == 1)
              str_sprintf(text, "LONG %s(1 CHAR)", varcharStr);
            else
              str_sprintf(text, "LONG %s(%d CHARS)", varcharStr, precision /*i.e., charlen*/);
          } else  // precision == 0 (char len unit is BYTE[S])
          {
            if (length /*in_bytes*/ == 1)
              str_sprintf(text, "LONG %s(1 BYTE)", varcharStr);
            else
              str_sprintf(text, "LONG %s(%d BYTES)", varcharStr, length /*in_bytes*/);
          }
        }
      } else
        str_sprintf(text, "LONG %s(%d)", varcharStr, length);

      addCollate = 1;
      break;

    case REC_DATETIME:
      return getDateTimeTypeText(text, datetimestart, datetimeend, datetimefractprec);
    ...
    case REC_ARRAY:
      str_sprintf(text, "ARRAY");
      break;

    case REC_ROW:
      str_sprintf(text, "ROW");
      break;
  ...
  if (addCharSet) {
    str_sprintf(&text[str_len(text)], " CHARACTER SET %s", CharInfo::getCharSetName(charSet));
  }

  if (addCollate && (collation_name != NULL) && (strcmp(collation_name, SQLCOLLATIONSTRING_UNKNOWN) != 0)) {
    str_sprintf(&text[str_len(text)], " COLLATE %s", collation_name);
  }

  if (DFS2REC::isAnyCharacter(fs_datatype)) {
    if (upshift) str_sprintf(&text[str_len(text)], " UPSHIFT");
...
int getAnsiTypeFromFSType(int datatype) {
  ...
  switch (datatype) {
    ...
    case REC_ARRAY:
      numeric_value = SQLTYPECODE_ARRAY;

    case REC_ROW:
      numeric_value = SQLTYPECODE_ROW;
...
const char *getAnsiTypeStrFromFSType(int datatype) {
  switch (datatype) {
    ...
    case REC_ARRAY:
      return COM_ARRAY_SDT_LIT;

    case REC_ROW:
      return COM_ROW_SDT_LIT;
