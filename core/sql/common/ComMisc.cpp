...
#define ComDefXLateE2L(E2L, eType, array)               \
  void E2L(const eType e, NAString &l) {                \
    NABoolean found;                                    \
    char lit[100];                                      \
...
#define ComDefXLateL2E(L2E, eType, array)                                \
...
// Define both
#define ComDefXLateFuncs(L2E, E2L, eType, array) \
  ComDefXLateL2E(L2E, eType, array);             \
  ComDefXLateE2L(E2L, eType, array)

// systemCatalog: if passed in, is the name of traf system catalog.
//                         default is TRAFODION.
NABoolean ComIsTrafodionReservedSchema(const NAString &systemCatalog, const NAString &catName,
  ...
  if ((catName == trafSysCat) &&
      ((schName == SEABASE_DTM_SCHEMA) || (schName == SEABASE_MD_SCHEMA) || (schName == SEABASE_PRIVMGR_SCHEMA) ||
       (schName == SEABASE_REPOS_SCHEMA) || (schName == SEABASE_TENANT_SCHEMA) || (schName == SEABASE_UDF_SCHEMA) ||
...
NABoolean ComIsTrafodionReservedSchemaName(const NAString &schName) {
  if ((schName.data()[0] == '_') && (schName.data()[schName.length() - 1] == '_')) return TRUE;
...
NABoolean ComIsTrafodionExternalSchemaName(const NAString &schName, NABoolean *isHive) {
  ...
  if ((len > 2) && (schName(0) == '"') && (schName(len - 1) == '"')) {
    quoted = TRUE;
    len = len - 2;
  }
  ...
  if (len > prefixLen && (schName((quoted ? 1 : 0), prefixLen - 1) == HBASE_EXT_SCHEMA_PREFIX &&
...
NABoolean ComIsHbaseMappedSchemaName(const NAString &schName) { return (schName == HBASE_EXT_MAP_SCHEMA); }
...
NABoolean ComIsHBaseMappedExtFormat(const NAString &catalogNamePart, const NAString &schemaNamePart) {
  if ((catalogNamePart == HBASE_SYSTEM_CATALOG) && (schemaNamePart == HBASE_MAP_SCHEMA)) return TRUE;
...
NABoolean ComIsHBaseMappedIntFormat(const NAString &catalogNamePart, const NAString &schemaNamePart) {
  if ((catalogNamePart == TRAFODION_SYSCAT_LIT) && (schemaNamePart == HBASE_EXT_MAP_SCHEMA)) return TRUE;
...
NAString ComConvertNativeNameToTrafName(const NAString &catalogName, const NAString &schemaName,
  ...
  if (NOT((catalogName == HBASE_SYSTEM_CATALOG) || (catalogName == HIVE_SYSTEM_CATALOG)))
    return catalogName + NAString(".") + schemaName + NAString(".") + objectName;

  // generate new schema name
  NAString tempSchemaName;
  if ((catalogName == HBASE_SYSTEM_CATALOG) &&
      ((schemaName == HBASE_MAP_SCHEMA) || (schemaName == HBASE_MAP_SCHEMA_QUOTED)))
...
NAString ComConvertTrafHiveNameToNativeHiveName(const NAString &catalogName, const NAString &schemaName,
  ...
  if (schemaName.compareTo(HIVE_DEFAULT_SCHEMA_EXE, NAString::ignoreCase) == 0)  // matches  'default'
  {
    ...
    if (NOT objectName.isNull()) newHiveName += ".";
  ...
  if (NOT objectName.isNull()) newHiveName += objectName;

  return newHiveName;
}

NABoolean ComTrafReservedColName(const NAString &colName) {
  if ((colName == TRAF_SALT_COLNAME) || (colName == TRAF_REPLICA_COLNAME) || (colName == TRAF_SYSKEY_COLNAME) ||
...
NABoolean ComValidateNamespace(NAString *inNS, NABoolean isTraf, NAString &outNS) {
  outNS.clear();
  if (!inNS || inNS->isNull()) return FALSE;  // empty input string is an invalid namespace
  ...
  if (lns.length() >= TRAF_NAMESPACE_PREFIX_LEN) {
    ...
    if (nas == TRAF_NAMESPACE_PREFIX) {
      if (NOT isTraf) return FALSE;  // cannot start with 'TRAF_'
...
NAString ComGetReservedNamespace(NAString schName) {
  if ((schName == SEABASE_MD_SCHEMA) || (schName == SEABASE_PRIVMGR_SCHEMA) || (schName == SEABASE_TENANT_SCHEMA) ||
      (schName == SEABASE_XDC_MD_SCHEMA))
    return TRAF_RESERVED_NAMESPACE1;
  else if ((schName == SEABASE_REPOS_SCHEMA) || (schName == HIVE_STATS_SCHEMA_NO_QUOTES) ||
...
Int32 ComGenerateUdrCachedLibName(NAString libname, Int64 redeftime, NAString schemaName, NAString userid,
  ...
  if (userid.length() != 0) {
    ...
    if (stat(cachedLibPath, &statbuf) != 0) {
      if (mkdir(cachedLibPath,
                S_IRUSR | S_IWUSR | S_IXUSR))  // Only this user has
...
bool ComIsVolatileSchema(const NAString &cat, const NAString &sch) {
  ...
  if (!strncmp(sch.data(), COM_VOLATILE_SCHEMA_PREFIX, strlen(COM_VOLATILE_SCHEMA_PREFIX))) {
    return true;
  }
  return false;
}
...
bool ComIsReservedTable(const char *tblName) {
  char internalTables[][256] = {"EXE_UTIL_DISPLAY_EXPLAIN__",
                                "EXPLAIN__",
                                "HIVEMD__",
                                "DESCRIBE__",
                                "EXE_UTIL_EXPR__",
  ...
  for (int i = 0; i < sizeof(internalTables) / sizeof(internalTables[0]); i++) {
    if (!strncmp(tblName, internalTables[i], strlen(internalTables[i]))) {
      return true;
    }
  }
  return false;
...
bool ComIsUserTable(const NAString &cat, const NAString &sch, const NAString &obj) {
  if (!cat.compareTo(HIVE_SYSTEM_CATALOG, NAString::ignoreCase) ||
      !cat.compareTo(HBASE_SYSTEM_CATALOG, NAString::ignoreCase) || ComIsTrafodionReservedSchema("", cat, sch) ||
      ComIsTrafodionReservedSchemaName(sch) || ComIsVolatileSchema(cat, sch) || ComIsReservedTable(obj.data())) {
    LOGTRACE(CAT_SQL_LOCK, "NOT USER TABLE: %s.%s.%s", cat.data(), sch.data(), obj.data());
    return false;
  }
  LOGTRACE(CAT_SQL_LOCK, "USER TABLE: %s.%s.%s", cat.data(), sch.data(), obj.data());
  return true;
}

bool ComIsPartitionMDTable(const NAString &name) {
  if (name == SEABASE_PARTITIONS || name == SEABASE_TABLE_PARTITION) return true;
