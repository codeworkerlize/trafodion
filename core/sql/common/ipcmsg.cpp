...
extern "C" _priv _resident int_16 FS_GETTRANSID_(extaddr tubaddr,                // input
...
extern "C" _priv int_16 TMFLIBFS_ABORTTRANS_(int_16 *tcbref_ptr, short disposition);

// -----------------------------------------------------------------------
// Methods for class GuaMsgConnectionToServer
// -----------------------------------------------------------------------

GuaMsgConnectionToServer::GuaMsgConnectionToServer(IpcEnvironment *env, const IpcProcessId &procId,
...
NABoolean GuaMsgConnectionToServer::moreWaitsAllowed() { return !stopWait_; }
...
//
WaitReturnStatus GuaMsgConnectionToServer::wait(IpcTimeout timeout, UInt32 *eventConsumed = NULL,
  ...
  if (getState() ==
      ERROR_STATE AND numOutstandingIOs_ <= 0 AND numQueuedSendMessages() <= 0) {  // no more waits on this connection
    stopWait(TRUE);
    return WAIT_OK;
  }
  stopWait(FALSE);
  ...
  do {
    if ((activeIOs_[currentEntry_].inUse_) && (activeIOs_[currentEntry_].expectReply_)) {
    ...
    */
    if (currentEntry_ == nowaitDepth_) currentEntry_ = 0;
  ...
  if (a_message_is_done) {
    ...
    MSG_BREAK2_((NSK_msId2)entry.msgid_, &results, (NSK_PHandle _ptr64 *)getOtherEnd().getPhandle().phandle_);
        ...
        if (entry.offset_ == 0) {
          if (numOutstandingIOs_ > 0) {
            ...
            for (unsigned short i = 0; i < nowaitDepth_; i++) {
              ...
              if (nextEntry.inUse_ && nextEntry.buffer_ == entry.buffer_) {
...
NABoolean GuaMsgConnectionToServer::tryToStartNewIO() {
  ...
  if (partiallyReceivedBuffer_ && partiallyReceivedBuffer_->getMessageLength() == chunkBytesRequested_) return FALSE;
  ...
  // multithreaded.
  short numMsgsActual;
  if (MESSAGESYSTEMINFO(5, &numMsgsActual)) assert(0);
  ...
  if (partiallySentBuffer_) {
    ...
    entry.receiveBufferSizeLeft_ = MINOF(maxIOSize_, entry.buffer_->getMessageLength() - chunkBytesRequested_);
    ...
    if (entry.buffer_->getRefCount() > 1 || entry.buffer_->isShared()) {
      if (!entry.buffer_->initLockCount(getEnvironment()->getHeap(), maxIOSize_)) {
  ...
  if (NOT GuardianError) {
    ...
    GuardianError = ADDRESS_WIRE_((unsigned char *)entry.controlBuf_, sizeof(fs_fs_writeread), wireOptions);
  }
  ...
  if (NOT GuardianError) {
    ...
    do {
      ...
      GuardianError =
          MSG_LINK2_((NSK_PHandle _ptr64 *)getOtherEnd().getPhandle().phandle_, (NSK_msId2 _ptr64 *)&localMsgId,
                     (int16 _ptr64 *)&controlCBA, (NSK_msSize2)sizeof(fs_fs_writeread), (int16 _ptr64 *)&controlCBA,
      ...
      if (GuardianError == FENOLCB)  // too many MQCs.
      {
        ...
        needToRetry = FALSE;
    } while (needToRetry && retryCount < 100 * 60);  // after 60 seconds (and 6000 retries),
                                                     // just give up.
    getEnvironment()->setEvent(TRUE, AEVENT);
...
void GuaMsgConnectionToServer::openPhandle(char *processName) {
  ...
  if (!processName) {
    ...
    guaErrorInfo_ = PROCESSHANDLE_TO_FILENAME_((short *)getOtherEnd().getPhandle().phandle_, procFileName,
                                               IpcMaxGuardianPathNameLength, &procFileNameLen, 0);
    if (guaErrorInfo_ != GuaOK) {
      setErrorInfo(-1);
      setState(ERROR_STATE);
      return;
  ...
  _cc_status stat;
  while (1) {
    guaErrorInfo_ = FILE_OPEN_(procFileName, procFileNameLen, &openFile_,
                               0,  // open for read/write access
                               0,  // shared access
                               nowaitDepth_,
                               0,           // sync depth 0 (target proc is not NonStop)
                               openFlags);  // options
    if (guaErrorInfo_ != GuaOK) break;
  ...
  for (short i = 0; i < numOldOpens; i++) FILE_CLOSE_(oldOpens[i]);
...
void GuaMsgConnectionToServer::handleIOError() {
  ...
  for (unsigned short i = 0; i < nowaitDepth_; i++) {
    ActiveIOQueueEntry &entry = activeIOs_[i];
    if (entry.inUse_) handleIOErrorForEntry(entry);
  }
...
void GuaMsgConnectionToServer::handleIOErrorForEntry(ActiveIOQueueEntry &entry) {
  ...
  // clean up I/O entry
  if (entry.inUse_) cleanUpActiveIOEntry(entry);
  ...
  if (entry.readBuffer_ && entry.readBuffer_ != entry.buffer_) entry.readBuffer_->decrRefCount(getEnvironment());
...
short GuaMsgConnectionToServer::addressWire(ActiveIOQueueEntry &entry, short wireOptions) {
  //
  ...
  if (entry.buffer_->isShared()) {
    ...
    if (entry.buffer_->getLockCount(entry.offset_) == 0) {
      ...
      GuardianError = ADDRESS_WIRE_((unsigned char *)entry.buffer_->data(entry.offset_), entry.bytesSent_, wireOptions);
  ...
  if (entry.readBuffer_ && entry.readBuffer_ != entry.buffer_) {
    ...
    GuardianError =
        ADDRESS_WIRE_((unsigned char *)entry.readBuffer_->data(0), entry.receiveBufferSizeLeft_, wireOptions);
    if (GuardianError) {
      ...
      if (entry.buffer_->decrLockCount(entry.offset_) == 0)
        ...
        ADDRESS_UNWIRE_((unsigned char *)entry.buffer_->data(entry.offset_), entry.bytesSent_, wireOptions);
...
void GuaMsgConnectionToServer::addressUnwire(ActiveIOQueueEntry &entry) {
  ...
  //
  if (entry.readBuffer_ && entry.readBuffer_ != entry.buffer_)
    ADDRESS_UNWIRE_((unsigned char *)entry.readBuffer_->data(0), entry.receiveBufferSizeLeft_, wireOptions);
}
