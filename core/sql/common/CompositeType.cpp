...
//
// ***********************************************************************
CompositeType::CompositeType(NAMemory *heap, const NAString &adtName,
                             NABuiltInTypeEnum ev, 
                             Int32 compFormat)
     : NAType(heap, adtName,
              ev,
              0,
              TRUE, SQL_NULL_HDR_SIZE,
              TRUE/*varchar*/, SQL_VARCHAR_HDR_SIZE_4/*4 bytes len*/
              )
{
...
const NAString &CompositeType::getCompDefnStr() const 
{
  return ((CompositeType*)this)->getCompDefnStr();
}
...
//
// ***********************************************************************
SQLArray::SQLArray(NAMemory *heap, const NAType *elementType, 
                   const Lng32 arraySize, Int32 compFormat)
       ...
       arraySize_(arraySize)
{
  ...
  if (getCompFormat() == COM_SQLMX_ALIGNED_FORMAT)
    nominalSize += 4 * sizeof(Int32); // aligned format header size

  if (elementType_)
    {
      nominalSize += ROUND4(elementType_->getTotalAlignedSize() +
                            (elementType_->isVaryingLen() ? sizeof(Int32) : 0))
        * arraySize_;
    }
...
NAType * SQLArray::newCopy2(Int32 compFormat, CollHeap* h) const
{
  ...
  if (pType->isComposite() &&
      (compFormat != COM_UNINITIALIZED_FORMAT))
    newPType = ((CompositeType*)pType)->newCopy2(compFormat, h);
  else
    newPType = pType->newCopy(h);

  SQLArray *newType = new(h) SQLArray(h, newPType, getArraySize(),
                                      compFormat);

  return newType;
}

NAType * SQLArray::newCopy(CollHeap* h) const
{
  return newCopy2(COM_UNINITIALIZED_FORMAT, h);
}
...
{
  NAString nas;
  if (elementType_->getMyTypeAsText(outputStr, addNullability, addCollation))
    return -1;
...
// format:  array<...>
short SQLArray::getHiveTypeStr(Lng32 hiveType, Lng32 precision, Lng32 scale,
                               NAString * outputStr/*out*/) const
{
  (*outputStr).append("array<");

  if (getElementType()->getHiveTypeStr(getElementType()->getHiveType(), 
                                       getElementType()->getPrecisionOrMaxNumChars(),
                                       getElementType()->getScale(),
                                       outputStr))
    return -1;
...
short SQLArray::getFSDatatype() const 
{
  return REC_ARRAY;
}

NAString SQLArray::getSimpleTypeName() const
{
  return "ARRAY";
}
...
NABoolean SQLArray::validate(ComDiagsArea *diags)
{
  if (DFS2REC::isLOB(getElementType()->getFSDatatype()))
    {
      *diags <<  DgSqlCode(-3242)
             << DgString0("BLOB/CLOB types are not allowed as ARRAY elements.");
...
const NAType* SQLArray::synthesizeType(enum NATypeSynthRuleEnum synthRule,
                                       ...
                                       UInt32 *flags) const
{
  ...
  //
  if ((operand1.getFSDatatype() != REC_ARRAY) ||
      (operand2.getFSDatatype() != REC_ARRAY))
    return NULL;

  SQLArray &op1Array = (SQLArray&)operand1;
  SQLArray &op2Array = (SQLArray&)operand2;
  if (op1Array.getArraySize() != op2Array.getArraySize())
    return NULL;

  if (synthRule == SYNTH_RULE_UNION)
    {
      const NAType *resultType = op1Array.getElementType()->synthesizeType
        (SYNTH_RULE_UNION, 
         *op1Array.getElementType(), *op2Array.getElementType(), h);
...
{
  if (getFSDatatype() != other.getFSDatatype())
    return FALSE;
  ...
  if (NOT getElementType()->isCompatible(*otherElemType, flags))
    return FALSE;
...
{
  if (NOT isCompatible(other))
    return FALSE;

  if (NOT (getElementType()->equalIgnoreCoercibility(*((SQLArray&)other).getElementType())))
    return FALSE;
  
  if (getCompFormat() != ((CompositeType&)other).getCompFormat())
    return FALSE;
...
Lng32 SQLArray::getDisplayLength() const
{
  ...
  totalDL += totalDL + elemDL * getArraySize() +
    (getArraySize() - 1) * 3; // extra 3 for " , " separator between elements.
    ...
    case HIVE_DECIMAL_TYPE:
      {
        if (elemType->getPrecision() <= 18)
          ...
          len = NAType::getDisplayLengthStatic(
               elemType->getFSDatatype(), elemType->getNominalSize(),
...
//
// ***********************************************************************
SQLRow::SQLRow(NAMemory *heap) :
     CompositeType(heap, LiteralRow, NA_COMPOSITE_TYPE)
{
...
SQLRow::SQLRow(NAMemory *heap, 
               ...
               NAArray<NAType*> *fTypes,
               Int32 compFormat) :
     CompositeType(heap, LiteralRow, NA_COMPOSITE_TYPE, compFormat),
     ...
     fieldTypes_(fTypes)
{
  ...
  if (getCompFormat() == COM_SQLMX_ALIGNED_FORMAT)
   nominalSize += 4 * sizeof(Int32); // header size
  for (Int32 i = 0; i < fieldTypes().entries(); i++)
    {
      nominalSize += ROUND4(fieldTypes()[i]->getTotalAlignedSize() +
                            (fieldTypes()[i]->isVaryingLen() ? sizeof(Int32) : 0));
...
NAType * SQLRow::newCopy2(Int32 compFormat, CollHeap* h) const
{
  ...
  for (CollIndex i = 0; i < p->fieldTypes().entries(); i++)
    {
      if (fieldNames && (p->fieldNames().entries() > 0))
        fieldNames->insertAt(i, p->fieldNames()[i]);
...
NAType * SQLRow::newCopy(CollHeap* h) const
{
  return newCopy2(COM_UNINITIALIZED_FORMAT, h);
}
...
// format:  struct<...>
short SQLRow::getHiveTypeStr(Lng32 hiveType, Lng32 precision, Lng32 scale,
                             NAString * outputStr/*out*/) const
{
  ...
  for (Int32 i = 0; i < sr->getNumElements(); i++)
    {
      ...
      if (elemType->getHiveTypeStr(elemType->getHiveType(),
                                   elemType->getPrecisionOrMaxNumChars(),
                                   elemType->getScale(),
                                   outputStr))
        return -1;

      if (i < (sr->getNumElements()-1))
        (*outputStr).append(",");
    } // for
  ...
  while (curr < fieldTypes().entries())
    {
      ...
      if (DFS2REC::isLOB(type->getFSDatatype()))
        {
          *diags <<  DgSqlCode(-3242)
                 << DgString0("BLOB/CLOB types are not allowed within a ROW definition.");
          return FALSE; // BLOB/CLOB not allowed
        }
              ...
              if (fieldNames()[curr] == fieldNames()[next])
                {
                  ...
                  *diags <<  DgSqlCode(-3242)
                         << DgString0("Duplicate column names are not allowed within a ROW definition.");
...
short SQLRow::getFSDatatype() const 
{
  return REC_ROW;
}
...
NABoolean SQLRow::getElementInfo(const NAString &fieldName,
...
NAString SQLRow::getSimpleTypeName() const
{
  return "ROW";
}
...
const NAType* SQLRow::synthesizeType(enum NATypeSynthRuleEnum synthRule,
                                     const NAType& operand1,
                                     const NAType& operand2,
                                     CollHeap* h,
                                     UInt32 *flags) const
{
  ...
  //
  if ((operand1.getFSDatatype() != REC_ROW) ||
      (operand2.getFSDatatype() != REC_ROW))
    return NULL;

  SQLRow &op1Row = (SQLRow&)operand1;
  SQLRow &op2Row = (SQLRow&)operand2;
  if (op1Row.getNumElements() != op2Row.getNumElements())
    return NULL;
      ...
      for (UInt32 i = 0; i < op1Row.getNumElements(); i++)
        {
          const NAType *resultType = op1Row.getElementType(i+1)->synthesizeType
            (SYNTH_RULE_UNION, 
             *op1Row.getElementType(i+1), *op2Row.getElementType(i+1), h);
          if (! resultType)
            return NULL;
          fieldTypes->insertAt(i, (NAType*)resultType);
          if (op1Row.fieldNamesPtr())
            fieldNames->insertAt(i, op1Row.fieldNames()[i]);
        }
...
NABoolean SQLRow::isCompatible (const NAType& other, UInt32 * flags) const
{
  ...
  if (p->fieldTypes().entries() != newP.fieldTypes().entries())
    return FALSE;
  ...
  for (CollIndex i = 0; i < p->fieldTypes().entries(); i++)
    {
      ...
      if (NOT type->isCompatible(*otherType, flags))
        return FALSE;
    }
  ...
  for (CollIndex i = 0; i < p->fieldTypes().entries(); i++)
    {
      ...
      if (NOT (type->equalIgnoreCoercibility(*otherType)))
        return FALSE;
    }
