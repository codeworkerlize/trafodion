...
//    $<vol>.<svol>.<file>
//    \<node>.$<vol>.<svol>.<file>
NABoolean ComInterpretGuardianFileName(const char *fileName, ComNodeName &nodeNamePart, ComVolumeName &volumeNamePart,
...
const ComGuardianFileNamePart &ComGuardianFileNamePart::operator=(const char *part) {
  ...
  for (size_t i = 0; i < length; i++) ((char *)&namePart_)[i] = upshift1Char(part[i]);
...
void ComGuardianFileNamePart::trim(void) {
  ...
  while (castToConstChar()[i] == ' ') ((char *)&namePart_)[i--] = 0;
}

// Return an indication if a name part is valid. Optionally, the first character
// can be special (like '\' and '$').
NABoolean ComGuardianFileNamePart::isNamePartValid(const char requiredFirstChar, const NABoolean emptyIsOk,
  ...
  if (requiredFirstChar) {
    // Required first char
    if (*(ptr++) != requiredFirstChar) return FALSE;
  }
  ...
  while (*(++ptr)) {
    ...
    if (!((*ptr >= 'A' && *ptr <= 'Z') || (*ptr >= '0' && *ptr <= '9'))) return FALSE;
  }
...
NABoolean ComSubvolumeName::isMXSubvol(const size_t svMinLength) const {
  ...
  return (isNamePartValid('Z', FALSE, svMinLength) && castToConstChar()[1] == 'S' && castToConstChar()[2] == 'D');
}
...
NABoolean ComSubvolumeName::isMXMetaDataSubvol(void) const {
  ...
  return (isMXSubvol(4) && castToConstChar()[3] >= '0' && castToConstChar()[3] <= '9');
}

NABoolean ComSubvolumeName::isMXUserDataSubvol(void) const {
  // A valid MX metadata subvol starts with ZSD<letter> and is
  // exactly 8 char long
  return (isMXSubvol() && castToConstChar()[3] >= 'A' && castToConstChar()[3] <= 'Z');
}
...
NABoolean ComFileName::isMXFile(void) const {
  ...
  // ends with '00' or '01'
  return (isNamePartValid(0, FALSE, ComFILE_NAME_PART_CHAR_MAX_LEN) && castToConstChar()[6] == '0' &&
          castToConstChar()[7] >= '0' && castToConstChar()[7] <= '1');
}
