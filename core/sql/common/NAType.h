...
public:
  ...
  void resetVaryingLen() 
  {
    ...
    totalAlignment_ = 
      MAXOF(MAXOF(dataAlignment_,SQLnullHdrSize_),lengthHdrSize_);
  }
  ...
  static Lng32 getDisplayLengthStatic(Lng32 datatype,
                                      Lng32 length,
                                      Lng32 precision,
                                      Lng32 scale,
                                      Lng32 heading_len);
  ...
  static NAType* getNATypeForHive(const char* hiveType, NABoolean isORC,
                                  NABoolean compositeElement,
                                  NAMemory* heap);
  static NAType* getStructNATypeForHive(const char *compTypeStr,
                                        NABoolean isORC,
                                        NAMemory* heap);
  static NAType* getArrayNATypeForHive(const char *compTypeStr,
                                       NABoolean isORC,
                                       NAMemory* heap);
  
  void setHiveType(const HiveProtoTypeKind ht)
  {
    hiveType_ = ht;
  }
  ...
  const NAString getHiveTypeName() const 
  { 
    return getHiveTypeName(hiveType_);
  }
  static const NAString getHiveTypeName(Int32 hiveType)
  {
    return hiveProtoTypeKindStr[hiveType];
  }
