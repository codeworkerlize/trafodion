...
//
NAWcharBuf *sjisToUnicode(const charBuf &sjisString, CollHeap *heap, NAWcharBuf *&result, NABoolean addNullAtEnd) {
  //
...
#include "sjis_fa_fc.h"
  ...
  static const SJISCodeBoundT SJISLeadByteBounds[] = {
      {0x81, 0x84}, {0x87, 0x9F}, {0xE0, 0xEA}, {0xED, 0xEE}, {0xFA, 0xFC}, {0x0, 0xFF}  // catch all
  };

  // blocks determined by the trailing byte
  static const SJISCodeBoundT SJISTrailByteBounds[] = {
      {0x40, 0x7E}, {0x80, 0xFC}, {0x0, 0xFF}  // catch all
  };
  ...
  while (i < sourceLen) {
    ...
    if (isSingleByteSJIS1stBlock(c))
    ...
    else if (isSingleByteSJIS2ndBlock(c))
      ...
      for (lead = 0; lead <= 4; lead++)
        if (in_range(c, SJISLeadByteBounds[lead].lower, SJISLeadByteBounds[lead].upper)) break;

      // find the first or the second block for c.
      for (trail = 0; trail <= 2; trail++)
        if (in_range(d, SJISTrailByteBounds[trail].lower, SJISTrailByteBounds[trail].upper)) break;

      // out of range of SJIS legal code values
      if ((c == 0xEA && d >= 0xA4) || (c == 0xFC && d >= 0x4C) || (lead == 5) || (trail == 2)) {
        return 0;
      }
...
//
static NABoolean binarySearchU2STable(NAWchar u, NAWchar &sjis) {
  //
...
void sjis2Unicode2sjis() {
  ...
  for (i = 0; i < sizeof(leadbyte) / sizeof(int); i++)
    for (int j = 0x40; j <= 0xFF; j++) {
      ...
      if (remapped_sjis->data()[0] != sjis.data()[0] || remapped_sjis->data()[1] != sjis.data()[1]) {
        printf("Roundtrip mapping failed: sjis=0x%X%X  unicode=0x%X, remapped_sjis=0x%X%X\n", sjis.data()[0],
               sjis.data()[1], unicode->data()[0], remapped_sjis->data()[0], remapped_sjis->data()[1]);
        return;
      }
    }
