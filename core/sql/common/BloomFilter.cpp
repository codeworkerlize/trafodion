...
ULng32 computeHashTableSizeInBytes(
...
void BloomFilter::dump(ostream& out, const char* msg)
{
   ...
   out << "BloomFilter: m_(#bits)=" << m_
       << ", k_(#funcs)=" << k_
       << ", keyLenInfo_=" << keyLenInfo_
       << endl;;
}

UInt32 BloomFilter::computeFinalHash(UInt32 hash, Lng32 hashFunc)
{
   return computeFinalHash(hash, hashFunc, m_);
}

UInt32 
BloomFilter::computeFinalHash(UInt32 hash, Lng32 hashFunc, UInt32 bits)
{
   UInt32 h2 = 0;
   ...
   // of '0', '1', '2', '3' and '4'.
   static UInt32 preComputedH2s[5] = 
         { 0x905ebe29, 0x95ff0b84, 0xe5357ed6, 0x2cffae90, 0x8350b3f1 };
...
ULng32 BloomFilter::minPackedLength()
{
  return sizeof(m_) + sizeof(k_) + sizeof(keyLenInfo_);
}
...
ULng32 CountingBloomFilter::packIntoBuffer(char*& buffer)
{
  ...
  for ( CollIndex i=0; i<numBuckets_; i++ )
     sz += freq2sL_[i]->packIntoBuffer(buffer);
...
RegularBloomFilter::RegularBloomFilter(
                                       NAHeap* heap,
  ...
  BloomFilter(maxHashFuncs, 
              (m>RegularBloomFilter::minPackedLength()) ? 
                m - RegularBloomFilter::minPackedLength() : 0,
                p, heap),
...
RegularBloomFilter::RegularBloomFilter(NAHeap* heap):
  BloomFilter(heap), hashTable_(0, 1, heap_), 
  entries_(0)
{
}
...
{
   return BloomFilter::minPackedLength() + sizeof(entries_) +
             VarUIntArray::minPackedLength();
}

ULng32 RegularBloomFilter::getPackedLength()
{
   return BloomFilter::minPackedLength() + sizeof(entries_) +
             hashTable_.getPackedLength();
}
...
{
   if ( maxNumEntries > -1 && entries_ > maxNumEntries )
     return FALSE;

   return ( currentFalseProbability() <= threshold );
}
   
UInt32 RegularBloomFilter::entries(NABoolean estimate) 
{ 
   return entries_;
}
...
void RegularBloomFilter::testMultiHashInt(const char* dataFile, UInt32 maxFuncs, UInt32 bytes)
{
   ...
   while ( !input.eof() ) {
     ...
     UInt32 hashValueCommon = ExHDPHash::hash((char*)&x, 
                                          ExHDPHash::NO_FLAGS, 
                                          sizeof(x));
...
void 
RegularBloomFilter::test(const char* dataFile, UInt32 maxFuncs, UInt32 bytes, double prob)
{
