...
Int32 compareWcharWithBlankPadding(const NAWchar *wstr1, UInt32 len1, const NAWchar *wstr2, UInt32 len2) {
  ...
  if (rc == 0) {
    if (len1 < len2) {
      ...
      while ((shorterLen < len2) && (wstr2[shorterLen] == L' ')) shorterLen++;
      ...
      while ((shorterLen < len1) && (wstr1[shorterLen] == L' ')) shorterLen++;
...
NAWchar *na_wcschr(const NAWchar *wstr, NAWchar wc) {
  ...
  if (wc == 0) return p + na_wcslen(wstr);
