...
//         FALSE otherwise.
NABoolean ComEncryption::validateEncryptionOptions(NAString *encryptionOptions,
...
short ComEncryption::initEncryptionInfo(Int64 encryptionKeyID,
...
/////////////////////////////////////////////////////////////////////////////
short ComEncryption::getKeyFromKeyStore(
...
{
  if (getKeyFromKeyStore((char*)&ei.encryptionKeyID, sizeof(ei.encryptionKeyID),
                         ei.rowIdKey, ei.rowIdKeyLen))
    return -1;
...
short ComEncryption::setEncryptionKeys(EncryptionInfo &ei)
{
  if (isFlagSet(ei, ROWID_ENCRYPT))
    {
      if (setRowidEncryptionKey(ei))
        return -1;
    }

 if (isFlagSet(ei, DATA_ENCRYPT))
    {
      if (setDataEncryptionKey(ei))
        return -1;
    }
...
{
  if ((NOT isFlagSet(ei, ROWID_ENCRYPT)) ||
      (ei.rowidInitVecLen <= 0))
    return 0;
...
{
  if ((NOT isFlagSet(ei, DATA_ENCRYPT)) ||
      (ei.dataInitVecLen <= 0))
    return 0;
...
{
  if ((isFlagSet(ei, ROWID_ENCRYPT)) &&
      (setRowidInitVector(ei)))
    return -1;

  if ((isFlagSet(ei, DATA_ENCRYPT)) &&
      (setDataInitVector(ei)))
    return -1;
...
short ComEncryption::extractEncryptionInfo(
...
int ComEncryption::encryptRowId(unsigned char *plaintext, int plaintext_len, 
                                unsigned char *key,
...
int ComEncryption::decryptRowId(unsigned char *ciphertext, int ciphertext_len, 
...
int ComEncryption::encryptData(int dataCipherType,
...
int ComEncryption::decryptData(int dataCipherType,
...
int ComEncryption::encrypt_ConvertBase64_Data(int dataCipherType,
                                              unsigned char *plaintext, int plaintext_len,
                                              unsigned char *key, unsigned char *iv,
                                              unsigned char *converttext, int &convertLen)
{
  ...
  // convert base64
  convertLen = EVP_EncodeBlock(converttext,
                               (const unsigned char *)encryPassword,
                               cryptlen);
