...
void dump(ostream* outstream,
	  ...
	  MemoryStats& allocStats,
          NAHeapFragment *top,
	  Lng32 indent,
          bool dumpFragmentDetails = false);
#endif
...
public:
  ...
  { return useSemaphore() || getNamedSemaphore() ||
           getNamedSem() || getLockController(); }
...
public:
  ...
  NAHeap(const char * name, NAMemoryType type,
         size_t blockSize = 0, size_t upperLimit = 0);
  ...
  void * allocateAlignedHeapMemory(size_t userSize, size_t alignment, 
                                   NABoolean failureIsFatal = TRUE);
