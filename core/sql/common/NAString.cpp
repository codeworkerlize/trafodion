...
NABoolean IsNAStringSpaceOrEmpty(const NAString &ns) {
  ...
  for (StringPos i = 0; i < len; i++)
    if (!isSpace8859_1((unsigned char)ns[i])) return FALSE;
  return TRUE;
}
...
NABoolean NAStringHasOnly7BitAsciiChars(const NAString &ns) {
  StringPos len = ns.length();
  for (StringPos i = 0; i < len; i++)
    if (((unsigned char)ns[i]) > 127) return FALSE;
  return TRUE;
}
// -----------------------------------------------------------------------
...
NABoolean NAStringHasOnlyDecimalDigitAsciiChars(const NAString &ns) {
  StringPos len = ns.length();
  for (StringPos i = 0; i < len; i++)
    if (((unsigned char)ns[i]) < '0' OR((unsigned char)ns[i]) > '9') return FALSE;
  return TRUE;
}
...
// -----------------------------------------------------------------------
void NAStringUpshiftASCII(NAString &ns) { ns.toUpper(); }
...
NAString &replaceAll(NAString &source, const NAString &searchFor, const NAString &replaceWith) {
  ...
  if (indexOfReplace != NA_NPOS) {
    ...
    while (indexOfReplace != NA_NPOS) {
      source.replace(indexOfReplace, searchFor.length(), replaceWith);
      // Find index of next occurence to replace.
      indexOfReplace = source.index(searchFor, indexOfReplace + replaceWith.length());
    }
  }
...
// ---------------------------------------------------------------------
ULng32 hashKey(const NAString &str) { return str.hash(); }
...
NAString LookupDefineName(const NAString &ns, NABoolean iterate) {
  ...
  if ((mappedName[0] == '$' OR mappedName[0] == '=') AND mappedName[1] == '"') {
  ...
  while (mappedName AND(mappedName[0] == '$' OR mappedName[0] == '=') AND iterations++ < iterlimit) {
...
NABoolean IsSqlReservedWord(const char *sqlText) { return ComResWords::isSqlReservedWord(sqlText, 0); }
...
NABoolean IsCIdentifier(const char *id) {
  ...
  for (size_t i = 0; id[i] != 0; i++) {
    ...
    if (!(c >= 'A' && c <= 'Z' || c >= 'a' && c <= 'z' || c == '_' || c >= '0' && c <= '9' && i > 0)) return FALSE;
  }
  return TRUE;
}
...
int ToInternalIdentifier(NAString &ansiIdent, Int32 upCase,
  ...
  if (len == 0) return -3004;  // An ident must contain at least one character

  size_t countOfRemoved = i;
  if ((ansiIdent[0] != '"') && (ansiIdent[0] != '`')) {
    const char *chineseStr = ansiIdent.data();
    for (i = 0; i < len; i++) {
      if (((unsigned char)chineseStr[i]) & 0x80) {
        ansiIdent = "\"" + ansiIdent + "\"";
        len = ansiIdent.length();
        chineseStr = NULL;
        break;
    ...
    // include Chinese Character, need to upper the other character
    if (chineseStr == NULL) {
      for (i = 0; i < len; i++) {
        if (((unsigned char)ansiIdent[i]) < 0x80) {
          ansiIdent[i] = (char)toupper(ansiIdent[i]);
        }
      }
    }
  }
  ...
  if ((ansiIdent[i] != '"') && (ansiIdent[i] != '`')) {  // REGULAR identifier
    ...
    for (i = len; i > 0;) {
      i--;
      if (!isSpace8859_1((unsigned char)ansiIdent[i])) break;
    }
    if (++i < len) {
      ansiIdent.remove(i);
      len = ansiIdent.length();
    ...
    if (isLatin1) c = (unsigned char)latin1Buf[i];

    if (isAlphaIsoMapCS(c) || (c == '$' && ((pv_flags & NASTRING_REGULAR_IDENT_WITH_DOLLAR_PREFIX) != 0))) {
    ...
    } else if ((c == '\\') || (c == '$' && ((pv_flags & NASTRING_ALLOW_NSK_GUARDIAN_NAME_FORMAT) != 0) &&
    ...
    if ((c == '\\') || (c == '$' && ((pv_flags & NASTRING_ALLOW_NSK_GUARDIAN_NAME_FORMAT) != 0) &&
                        ((pv_flags & NASTRING_DELIM_IDENT_WITH_DOLLAR_PREFIX) == 0))) {
      ...
      if (c == '\\') {
        ...
        while ((i < len) &&
               (isUpperIsoMapCS((unsigned char)ansiIdent[i]) || isDigit8859_1((unsigned char)ansiIdent[i]))) {
          i++;
        }
      ...
      while (i < ansiIdent.length()) {
        ...
        if (i == 0) {  // first character
          if ((c == '^' AND NOT acceptCircumflex)
            ...
            return illegalCharInIdentifier(ansiIdent, i, countOfRemoved);
          if (NOT Get_SqlParser_Flags(ALLOW_FUNNY_IDENTIFIER)
                  AND isDelimitedIdentifierReservedForInternalUse(ansiIdent.data(), ansiIdent.length())) {
            for (; i <= ansiIdent.length(); i++) {  // look for the first '@'
              if (ansiIdent[i] == NON_SQL_TEXT_CHAR) break;
            }  // for
            return illegalCharInIdentifier(ansiIdent, i, countOfRemoved);
      ...
      if ((isDquote && (state != 2)) || (NOT isDquote && (state != 4)))
        return illegalCharInIdentifier(ansiIdent, i, countOfRemoved);
      ...
      if (empty) return -3004;  // A delimited ident must contain at least one character
      ansiIdent.resize(++i);
    }
  ...
  if (!Get_SqlParser_Flags(ALLOW_FUNNY_IDENTIFIER)) {
    if (ansiIdent.length() > ComMAX_1_PART_INTERNAL_UTF8_NAME_LEN_IN_BYTES) return -3118;  // Identifier too long.
...
// ---------------------------------------------------------------------
void ToQuotedString(NAString &quotedStr, const NAString &internalStr, NABoolean encloseInQuotes) {
...
static NABoolean tokIsFuncOrParenKeyword(const NAString &sqlText, size_t pos, size_t prevpos) {
  ...
  if (tok == " AND " || tok == " OR ") return FALSE;
  ...
  if (pos >= 2 && sqlText[pos - 1] == ' ') {
    if (sqlText[pos - 2] == ')' && tok != " FROM " && tok != " AS ") return TRUE;
    if (pos >= 5)
      if (sqlText[pos - 5] == ')' && sqlText[pos - 4] == ' ' && sqlText[pos - 3] == 'A' && sqlText[pos - 2] == 'S')
        return TRUE;
  }
  ...
  if (bsearch(tok.data() + 1, keywords, (sizeof(keywords) / sizeof(keywords[0])), sizeof(char *), bsearchStrcmp))
    return TRUE;
  ...
  if (pic) {
    if (tok == " B " || tok == " X " || tok == " S9 " || tok == " S 9 " || tok == " SV9 " || tok == " V9 ") return TRUE;
  }
...
#define prevResultCharIs(c) (prevResultChar == c)
  ...
  NAString result;
  enum TokType { SPACE, ALPHA, DELIM, DIGIT, LPAREN, PUNC, UNARYOP } toktype = SPACE, prevtoktype = SPACE;
  ...
  for (const char *s = sqlText.data(); *s; s++) {
    ...
    if (quote_seen || *s == '\'' || *s == '"')  // in quotes or on ending quote
    {
      ...
      if (prev == ' ') { /*throw away the subsequent spaces; remain in whatever toktype*/
      } else {
    ...
    } else  // unquoted and not a space
    {
      ...
      if ((UInt32)curr >= 0x80)  // is not a 7-bit ASCII character
      {
        ...
        } else if (isChineseCharacter(tmpBuf[0]))  // is a Chinese character
        {
          for (; *s; s++) {
            if ((UInt32)*s >= 0x80) {
              result.append(s, 1);
      ...
      if (isAlphaIsoMapCS((unsigned char)curr) || curr == '_' || curr == ':' || curr == '?' || curr == '$' ||
          curr == '\\')  // non-Ansi '\nsk.$vol' extension
      {
        if (toktype != ALPHA) {
          toktype = ALPHA;
        ...
        if (toktype != LPAREN) {
          ...
          if (prevResultCharIs(' '))  // note: wrong to test (prev==' ')
            if (tokIsFuncOrParenKeyword(result, alphapos, prevalphapos)) result.remove(result.length() - 1);
        }
      } else if (curr == '.') {
        ...
        if (toktype != PUNC) {
          ...
          if (prev != ' ' || (curr == '-' && prevResultCharIs('-'))) result.append(" ");
          ...
          if (curr == '+' || curr == '-') {
            if (efftoktype == ALPHA) {
              ...
              toktype = (efftoktype != DELIM && efftoktype != DIGIT) ? UNARYOP : PUNC;
          } else {
            toktype = PUNC;
            ...
            if (curr == '/' && efftoktype == ALPHA) {
              ...
              if (tok == "LOCATION ") {
                tok = result;
                tok.remove(7);
                if (tok == "CREATE ") quote_seen = ' ';  // will consume till ' '
              }
            }
          }
        } else if (curr == '+' || curr == '-') {
      ...
      if (curr == '.' || curr == '(' || curr == ':' || toktype == PUNC || toktype == UNARYOP) {
        if (curr != '*')
...
//
size_t adjustedMaxLen(const NAString &sqlText, size_t pos, size_t maxLineLen, const char *schemaName) {
  ...
  if (schemaNameLen > SCH_LEN) {
    ...
    size_t occurs = 0;
    while (TRUE) {
      if (maxSearchPos >= sqlTextLen) maxSearchPos = sqlTextLen - 1;
      pos = sqlText.index(schemaName, pos, NAString::ignoreCase);
      if ((pos == NA_NPOS) || (pos > maxSearchPos)) break;
      occurs++;
      pos += schemaNameLen;
...
//
size_t LineBreakSqlText(NAString &sqlText, NABoolean showddlView, size_t maxlen, size_t pfxlen, size_t pfxinitlen,
                        char pfxchar, const char *schemaName, NABoolean commentOut) {
  if (commentOut && pfxchar != '-') {
    // Make sure that the prefixes have enough room
    // for the leading "--" comment prefix.
    if (pfxlen < 2) pfxlen += (2 - pfxlen);
    if (pfxinitlen < 2) pfxinitlen += (2 - pfxinitlen);
  }
    ...
    if (!quote_seen)
      if (isSpace8859_1((unsigned char)*s))  // sentinel ensures we get here
      {
        if (showddlView)  // look for keyword "AS"
          if ((space && result.length() - space == 3) || (!space && cnt == 3))
            if (result[result.length() - 3] == 'A' && result[result.length() - 2] == 'S') showddlViewAS = TRUE;
        ...
        if (cnt < maxcurrlen) {
          ...
          if (cnt > maxcurrlen + 1 && space > 0)  // linebreak on space
          {
            ...
            for (; i < 3; i++)
              if (!dot[i])
                ...
                dot[i] += pfxlen;  // replaced 1 with p+1
            maxcurrlen = adjustedMaxLen(sqlText, sqlNextPos - cnt, maxlen, schemaName);
          ...
          if (cnt >= maxcurrlen) {
            ...
            result += pfx;
            cnt = space = 0;
            maxcurrlen = adjustedMaxLen(sqlText, sqlNextPos, maxlen, schemaName);
        ...
        {
          size_t i;
          for (i = result.length() - 1; result[i] == ' ' || result[i] == pfxchar; i--) result.remove(i);
          if (result[i] != '\n') result += "\n";  // newline, no pfx chars

          if (result.length() <= maxinitlen + 1) pfxlen = pfxinitlen;  // still the first line

          NAString queryText(++s);  // get past the current space
          cnt = LineBreakSqlText(queryText, FALSE, maxlen + pfxlen, pfxlen + 4, pfxlen + 2, pfxchar, schemaName);
          result += queryText;
      ...
      else if ((*s == '.') && (!isDigit8859_1((unsigned char)s[1])))  // ignore dots in numeric constants
      {
        dot[2] = dot[1];
        dot[1] = dot[0];
        dot[0] = result.length();
...
void GetSimplePosixFilename(NAString &filename, NABoolean doLower) {
  ...
  const char *fslash = strrchr(filename, '/'), *bslash = strrchr(filename, '\\'), *dirpathpunc;
  if (fslash && bslash)
...
NAString Latin1StrToUTF8(const NAString &latin1Str, NAMemory *heap) {
  if (latin1Str.isNull()) return NAString();
  ...
  if ((Int32)utf8StrLenInBytes >= CharInfo::minBytesPerChar(CharInfo::UTF8))
    ...
    utf8StrLenInBytes = 0;
  NAString result;
  if (utf8StrLenInBytes > 0) result.append(target, (size_t)utf8StrLenInBytes);
  if (isBufferAllocatedFromProcessHeap) NADELETEBASIC(target, heap);
  return result;
}
...
base64Decoder::base64Decoder() {
  for (int i = 0; i < 256; i++) T_[i] = -1;

  for (int i = 'A'; i <= 'Z'; i++) T_[i] = i - 'A';

  for (int i = 'a'; i <= 'z'; i++) T_[i] = i - 'a' + 26;

  for (int i = '0'; i <= '9'; i++) T_[i] = i - '0' + 52;
...
NABoolean base64Decoder::decode(const char *str, const size_t len, NAString &decoded) {
  ...
  for (Int32 i = 0; i < len; i++) {
    ...
    valb += 6;
    if (valb >= 0) {
      decoded.append(char((val >> valb) & 0xFF));
      valb -= 8;
    }
  }
  return TRUE;
