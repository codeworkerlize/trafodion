...
NABoolean NAClusterInfo::mapNodeIdToNodeName(Int32 nodeId,
                                             NAString &nodeName) const
{
...
Int32 NAClusterInfo::mapPhysicalToLogicalNodeId(Int32 ix)
{
  ...
  if (result >= cpuArray_.entries())
    result = cpuArray_.entries() - 1;
...
Int32 NAClusterInfo::numberOfTenantUnitsInTheCluster() const
{
  ...
  Int32 result =
    MAXOF(
      MINOF(numCores  / NAWNODESET_CORES_PER_SLICE,
            ((long) (physMemGB / (0.98 * NAWNODESET_MEMORY_GB_PER_SLICE)))),
      1);
...
NAClusterInfoLinux *NAClusterInfo::castToNAClusterInfoLinux()
{
  return NULL;
}
...
NAClusterInfoLinux::NAClusterInfoLinux(CollHeap * heap,
                                       NABoolean isOsim) :
     NAClusterInfo(heap, isOsim),
     nid_(0), pid_(0)
{
  if (!isOsim)
...
long NAClusterInfoLinux::totalMemoryAvailableInKB() const
{
  return totalMemoryAvailableInKB_;
}
...
NAClusterInfoLinux *NAClusterInfoLinux::castToNAClusterInfoLinux()
{
  return this;
}
