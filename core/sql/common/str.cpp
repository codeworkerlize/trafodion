...
int str_cpy(char *tgt, const char *src, int tgtlen, char padchar) {
  ...
  if (tgtlen > copy_len) str_pad(&tgt[copy_len], (tgtlen - copy_len), padchar);
...
int str_decode(void *tgt, int tgtMaxLen, const char *src, int srcLen) {
  ...
  while (srcix + 1 < srcLen) {
    ...
    if (NOT(src1[srcix] - minChar < 64 && src1[srcix + 1] - minChar < 64)) return -1;
    ...
    if (srcix + 2 < srcLen) {
      ...
      if (NOT(src1[srcix + 2] - minChar < 64)) return -1;
    ...
    if (srcix + 3 < srcLen) {
      ...
      if (NOT(src1[srcix + 3] - minChar < 64)) return -1;
...
int str_to_ansi_id(char *src, char *tgt, int &tgtLen, short mustValidate, char *allowedChars) {
  ...
  for (i = 0; i < tgtLen; i++) {
    if (dQuoteSeen) {
      ...
      if ((tgt[i] == '"') && (tgt[i + 1] == '"')) {
        ...
        for (j = i; j < tgtLen; j++) tgt[j] = tgt[j + 1];
        tgtLen--;
      }
    }  // if dQuoteSeen
    else {
      ...
      if (NOT isAlNum8859_1((unsigned char)(tgt[i])) && (tgt[i] != '_')) {
        if (allowedChars) {
          ...
          for (UInt32 j = 0; j < str_len(allowedChars); j++) {
            if (tgt[i] == allowedChars[j]) found = 1;
          }
  ...
  if ((!dQuoteSeen) && (mustValidate)) {
    ...
    if (IsSqlReservedWord(tgt)) return -1;
  }
...
char *(str_str)(const char *s1, const char *s2) {
  ...
  for (; (s1 = str_chr(s1, *s2)) != NULL; s1++)
    if (str_ncmp(s1, s2, s2len) == 0) return (char *)s1;
  return NULL;
}
...
char *(str_replace)(char *s1, const char *s2, const char *s3) {
  ...
  if ((*s2 == '\0') || (*s3 == '\0')) return (char *)s1;

  s2len = str_len(s2);
  s3len = str_len(s3);

  if (s2len != s3len) return NULL;
...
char *str_tok(char *inStr, char c, char **internal) {
  ...
  if (*tempPtr == '\0') {
    *internal = NULL;
    ...
    ptr = tempPtr;
  while (*tempPtr != '\0' && *tempPtr != c) tempPtr++;
  if (*tempPtr == '\0')
    *internal = NULL;
  else {
    *tempPtr = '\0';
...
// -----------------------------------------------------------------------
size_t str_computeHexAsciiLen(size_t srcByteLen) { return 2 * srcByteLen; }
                            ...
                            NABoolean addNullAtEnd)      // in - default is TRUE
{
  ...
  if (addNullAtEnd) result[computedHexAsciiStrLen] = '\0';
...
void printBrief(char *dataPointer, int len, NABoolean endLine) {
  ...
  if (dataPointer) {
    ...
    if (allNulls) {
      ...
      if (len < lengthToMove) lengthToMove = len;
      ...
      if (len < repdChars) repdChars = len;

      for (size_t i = 0; i < repdChars; i++) {
        unsigned char nibbles[2];
        nibbles[0] = ((unsigned char)dataPointer[i] & (unsigned char)0xf0) / 16;
    ...
    if (len == 2)  // if it might be a short
    {
      ...
      long value = 256 * dataPointer[0] + (unsigned char)dataPointer[1];
    ...
    } else if (len == 4)  // if it might be a long
    {
      ...
      // the value is big-endian hence the weird computation
      long value = 256 * 256 * 256 * dataPointer[0] + 256 * 256 * (unsigned char)dataPointer[1] +
                   256 * (unsigned char)dataPointer[2] + (unsigned char)dataPointer[3];
    ...
    } else if (len == 8)  // if it might be a 64-bit integer
    {
      ...
      // the value is big-endian hence the weird computation
      long long value = 256 * 256 * 256 * dataPointer[0] + 256 * 256 * (unsigned char)dataPointer[1] +
                        256 * (unsigned char)dataPointer[2] + (unsigned char)dataPointer[3];
      value = (long long)256 * 256 * 256 * 256 * value + 256 * 256 * 256 * (unsigned char)dataPointer[4] +
              256 * 256 * (unsigned char)dataPointer[5] + 256 * (unsigned char)dataPointer[6] +
              (unsigned char)dataPointer[7];
      sprintf(local + strlen(local), " (long long %lld)", value);
    } else if (len == 7)  // a TIMESTAMP(0) perhaps?
    {
      long year = 256 * dataPointer[0] + (unsigned char)dataPointer[1];
      if ((year >= 1900) && (year <= 2100)) {
        ...
        if ((month >= 1) && (month <= 12) && (day >= 1) && (day <= 31) && (hour >= 0) && (hour <= 23) &&
          ...
          sprintf(local + strlen(local), " (TIMESTAMP(0) %ld-%02ld-%02ld %02ld:%02ld:%02ld)", year, month, day, hour,
                  minute, second);
        }
      }
    }
