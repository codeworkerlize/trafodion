...
CharType::CharType( NAMemory *h,
      ...
      : NAType(h, adtName
        ...
        , (varLenFlag ? ((vcIndLen > 0) ? vcIndLen
                                        : CharType::determineVCHdrSize(
                                          maxLenInBytesOrNAWchars *
...
NABoolean SQLVarChar::operator==(const NAType& other) const
{
  ...
  if ((getHiveType() == HIVE_STRING_TYPE) ||
      ((other.getTypeName() == LiteralVARCHAR) &&
       (other.getHiveType() == HIVE_STRING_TYPE)))
...
{
  return ((len & 0xFFFF8000) ? SQL_VARCHAR_HDR_SIZE_4
...
SQLBlob::SQLBlob(NAMemory *h, 
  ...
  : SQLlob(h, NA_LOB_TYPE,
	   ...
	   lobStorage,
	   allowSQLnull,
	   (lobV2 ? LOB_V2_EXT_HANDLE_LEN : LOB_V1_EXT_HANDLE_LEN)
           + extFormatLen
           )
{
