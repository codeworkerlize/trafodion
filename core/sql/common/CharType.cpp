...
CharType::CharType(NAMemory *h, const NAString &adtName, int maxLenInBytesOrNAWchars, short maxBytesPerChar,
    ...
    : NAType(h, adtName, NA_CHARACTER_TYPE,
             ...
             // if not passed in
             ,
             (varLenFlag ? ((vcIndLen > 0)
                                ? vcIndLen
                                : CharType::determineVCHdrSize(maxLenInBytesOrNAWchars * CharInfo::minBytesPerChar(cs)))
                         : 0),
             CharInfo::minBytesPerChar(cs)),
      qualifier_(CHARACTER_STRING_TYPE),
      charLimitInUCS2or4chars_(maxLenInBytesOrNAWchars * CharInfo::minBytesPerChar(cs) / CharInfo::maxBytesPerChar(cs)),
      ...
      coercibility_(ce),
      isVarchar2_(FALSE),
      isSpaceSensitive_(TRUE) {
  ComASSERT(adtName == LiteralCHAR || adtName == LiteralVARCHAR || adtName == LiteralBYTE ||
            adtName == LiteralVARBYTE || adtName == LiteralSchema || adtName == LiteralCatalog);
...
CharType::CharType(NAMemory *h, const NAString &adtName, const CharLenInfo &maxLenInfo,
                   short maxBytesPerChar,  // is maxBytesPerChar when cs is SJIS or UTF8
                   NABoolean nullTerminated, NABoolean allowSQLnull, NABoolean isUpShifted, NABoolean isCaseInsensitive,
                   NABoolean varLenFlag, CharInfo::CharSet cs, CharInfo::Collation co, CharInfo::Coercibility ce,
                   CharInfo::CharSet encoding)
    : NAType(h, adtName, NA_CHARACTER_TYPE,
             (maxLenInfo.getMaxLenInBytes() + (nullTerminated ? CharInfo::minBytesPerChar(cs) : 0)), allowSQLnull,
             allowSQLnull ? SQL_NULL_HDR_SIZE : 0, varLenFlag,
             (varLenFlag ? (CharType::determineVCHdrSize(maxLenInfo.getMaxLenInBytes()))
...
NAString CharType::getCharSetAsPrefix() const { return getCharSetAsPrefix(getCharSet()); }

NAString CharType::getCollationName() const { return CharInfo::getCollationName(getCollation()); }
...
NAString CharType::getSimpleTypeName() const { return isVaryingLen() ? LiteralVARCHAR : LiteralCHAR; }

NAString CharType::getTypeSQLname(NABoolean terse) const {
  char lenbuf[30];
  char *sp = lenbuf;

  NAString rName((isVaryingLen() || DFS2REC::isAnyVarChar(getFSDatatype())) ? LiteralVARCHAR : LiteralCHAR);
  ...
  if (isCaseinsensitive()) rName += " NOT CASESPECIFIC ";

  if (!terse) getTypeSQLnull(rName, terse);
...
// EncVal_encodeString(const char * str, int strLen, CharType *cType).
...
//
//----------------------------------------------
NABoolean CharType::computeCoAndCo(const CharType &other, CharInfo::Collation &newCO,
  ...
  NABoolean sameCEdiffCO = (getCoercibility() == other.getCoercibility() && getCollation() != other.getCollation());
...
// -----------------------------------------------------------------------
const NAType *CharType::synthesizeType(NATypeSynthRuleEnum synthRule, const NAType &operand1, const NAType &operand2,
  ...
  //
  if (operand1.getTypeQualifier() != NA_CHARACTER_TYPE || operand2.getTypeQualifier() != NA_CHARACTER_TYPE) return NULL;
  ...
  //
  if (op1.getCharSet() != op2.getCharSet() && op1.getCharSet() != CharInfo::UnknownCharSet &&
      op2.getCharSet() != CharInfo::UnknownCharSet)
    return NULL;

  int res_len_in_Chars;
  int res_nominalSize;
  ...
  switch (synthRule) {
    case SYNTH_RULE_UNION:
      ...
      if (!(DFS2REC::isAnyVarChar(op1.getFSDatatype()) || DFS2REC::isAnyVarChar(op2.getFSDatatype()))) {
        if ((flags && ((*flags & NAType::MAKE_RESULT_VARCHAR) != 0)) &&
            ((op1.getStrCharLimit() != op2.getStrCharLimit()) || (op1.getNominalSize() != op2.getNominalSize()))) {
      ...
      if (!op1.computeCoAndCo(op2, co, ce)) return NULL;

      caseinsensitive = op1.isCaseinsensitive() AND op2.isCaseinsensitive();
      break;
  ...
  CharLenInfo res_CharLenInfo(res_len_in_Chars, res_nominalSize);

  if (DFS2REC::isAnyVarChar(op1.getFSDatatype()) OR DFS2REC::isAnyVarChar(op2.getFSDatatype())) makeTypeVarchar = TRUE;

  NABoolean makeTypeBinary = FALSE;
  if (DFS2REC::isBinaryString(op1.getFSDatatype()) OR DFS2REC::isBinaryString(op2.getFSDatatype()))
    makeTypeBinary = TRUE;

  if (makeTypeBinary)
    return new (h) SQLBinaryString(h, res_nominalSize, null, makeTypeVarchar);
  else if (makeTypeVarchar)
    return new (h) SQLVarChar(h, res_CharLenInfo, null, upshift, caseinsensitive, op1.getCharSet(), co, ce);
  else
    return new (h) SQLChar(h, res_CharLenInfo, null, upshift, caseinsensitive, FALSE, op1.getCharSet(), co, ce);
}  // synthesizeType()
...
NABoolean CharType::errorsCanOccur(const NAType &target, NABoolean lax) const {
  if (!NAType::errorsCanOccur(target) && getNominalSize() <= target.getNominalSize()) {
    return FALSE;
  } else {
    return TRUE;
  }
...
NABoolean CharType::isEncodingNeeded() const {
  switch (getCharSet()) {
    ...
    case CharInfo::UTF8:
      return TRUE;  // UTF8 need to convert
...
int CharType::getBlankCharacterValue() const {
  switch (getCharSet()) {
    case CharInfo::UNICODE:
      return unicode_char_set::space_char();
    default:
      return int(' ');  //##NCHAR: is this correct for Kanji etc?
  }
}

int CharType::getMinSingleCharacterValue() const { return 0; }

int CharType::getMaxSingleCharacterValue() const {
  ...
  switch (getCharSet()) {
    ...
    case CharInfo::UNICODE:
      switch (getCollation()) {
        case CharInfo::DefaultCollation:
          return 0xffff;  // max UCS-2 character
        case CharInfo::SJIS_COLLATION:
          return 0xfcfc;  // max SJIS 2-byte char
      }
      break;
  }  // switch (getCharSet())
...
NABoolean CharType::isCharSetAndCollationComboOK() const {
  if (getCollation() == CharInfo::CZECH_COLLATION && charSet_ != CharInfo::ISO88591) return FALSE;

  // An unknown collation is allowed if not used in any dyadic operation
  // or comparison -- the parser just issues a warning for it.
  return getCollation() == CharInfo::UNKNOWN_COLLATION || getMaxSingleCharacterValue() != INT_MIN;
}
...
NABoolean CharType::compareCharAttrs(const NAType &other) const {
  return caseinsensitive_ == ((CharType &)other).caseinsensitive_ &&
         nullTerminated_ == ((CharType &)other).nullTerminated_ && upshifted_ == ((CharType &)other).upshifted_ &&
         charSet_ == ((CharType &)other).charSet_ && collation_ == ((CharType &)other).collation_ &&
         coercibility_ == ((CharType &)other).coercibility_ && encodingCharSet_ == ((CharType &)other).encodingCharSet_;
}

NABoolean CharType::operator==(const NAType &other) const {
  return NAType::operator==(other) && compareCharAttrs(other);
}

NABoolean CharType::equalIgnoreCoercibility(const NAType &other) const {
  const CharType &ctype = (CharType &)other;

  return NAType::operator==(other) && caseinsensitive_ == ((CharType &)other).caseinsensitive_ &&
         nullTerminated_ == ((CharType &)other).nullTerminated_ && upshifted_ == ((CharType &)other).upshifted_ &&
         charSet_ == ((CharType &)other).charSet_ && collation_ == ((CharType &)other).collation_ &&
...
NABoolean CharType::isCompatible(const NAType &other, UInt32 *flags) const {
  return (NAType::isCompatible(other, flags) && getCharSet() == ((CharType &)other).getCharSet() &&
...
NABoolean CharType::isCompatibleAllowUnknownCharset(const NAType &other) const {
  return (NAType::isCompatible(other) &&
          (getCharSet() == CharInfo::UnknownCharSet || ((CharType &)other).getCharSet() == CharInfo::UnknownCharSet));
}

NABoolean CharType::createSQLLiteral(const char *buf, NAString *&stringLiteral, NABoolean &isNull, CollHeap *h) const {
  ...
  if (sourceCS == CharInfo::BINARY) *resultLiteral += " X";

  *resultLiteral += "'";

  switch (sourceCS) {
    case CharInfo::BINARY: {
      int hexlen = str_computeHexAsciiLen(valLen);

      tempBuf = new (h) char[hexlen + 1];
      int rc = str_convertToHexAscii(valPtr, valLen, tempBuf, hexlen + 1, TRUE /*append null*/);
    ...
    case CharInfo::UNICODE: {
      ...
      int retCode = LocaleToUTF8(cnv_version1, valPtr, valLen, tempBuf, tempBufSize, convertCharsetEnum(sourceCS),
                                 firstUntranslatedChar, &outputLength);
      if (retCode != 0) result = FALSE;
  ...
  *resultLiteral += "'";
  if (tempBuf) NADELETEBASIC(tempBuf, h);
  stringLiteral = resultLiteral;

  return result;
}

SQLChar::SQLChar(NAMemory *h, int maxLen, NABoolean allowSQLnull, NABoolean isUpShifted, NABoolean isCaseInsensitive,
                 NABoolean varLenFlag, CharInfo::CharSet cs, CharInfo::Collation co, CharInfo::Coercibility ce,
                 CharInfo::CharSet encoding)
    : CharType(h, varLenFlag ? LiteralVARCHAR : LiteralCHAR, maxLen, CharInfo::maxBytesPerChar(cs), FALSE, allowSQLnull,
               isUpShifted, isCaseInsensitive, varLenFlag, cs, co, ce, encoding) {}

SQLChar::SQLChar(NAMemory *h, const CharLenInfo &maxLenInfo, NABoolean allowSQLnull, NABoolean isUpShifted,
                 NABoolean isCaseInsensitive, NABoolean varLenFlag, CharInfo::CharSet cs, CharInfo::Collation co,
                 CharInfo::Coercibility ce, CharInfo::CharSet encoding)
    : CharType(h, varLenFlag ? LiteralVARCHAR : LiteralCHAR, maxLenInfo, CharInfo::maxBytesPerChar(cs), FALSE,
               allowSQLnull, isUpShifted, isCaseInsensitive, varLenFlag, cs, co, ce, encoding) {}

SQLVarChar::SQLVarChar(NAMemory *h, int maxLen, NABoolean allowSQLnull, NABoolean isUpShifted,
                       NABoolean isCaseInsensitive, CharInfo::CharSet cs, CharInfo::Collation co,
                       CharInfo::Coercibility ce, CharInfo::CharSet encoding, int vcIndLen)
    : CharType(h, LiteralVARCHAR, maxLen, CharInfo::maxBytesPerChar(cs), FALSE, allowSQLnull, isUpShifted,
               isCaseInsensitive, TRUE, cs, co, ce, encoding, vcIndLen),
      clientDataType_(collHeap())  // Get heap from NABasicObject. Can't allocate on stack.
{}

SQLVarChar::SQLVarChar(NAMemory *h, const CharLenInfo &maxLenInfo, NABoolean allowSQLnull, NABoolean isUpShifted,
                       NABoolean isCaseInsensitive, CharInfo::CharSet cs, CharInfo::Collation co,
                       CharInfo::Coercibility ce, CharInfo::CharSet encoding)
    : CharType(h, LiteralVARCHAR, maxLenInfo, CharInfo::maxBytesPerChar(cs), FALSE, allowSQLnull, isUpShifted,
               isCaseInsensitive, TRUE, cs, co, ce, encoding),
      clientDataType_(collHeap())  // Get heap from NABasicObject. Can't allocate on stack.
{}
...
NABoolean SQLVarChar::operator==(const NAType &other) const {
  ...
  if ((getHiveType() == HIVE_STRING_TYPE) ||
      ((other.getTypeName() == LiteralVARCHAR) && (other.getHiveType() == HIVE_STRING_TYPE)))
...
// -----------------------------------------------------------------------
ANSIChar::ANSIChar(NAMemory *h, int maxLen, NABoolean allowSQLnull, NABoolean isUpShifted,
                   NABoolean varLenFlag,  // *unused*
                   CharInfo::CharSet cs, CharInfo::Collation co, CharInfo::Coercibility ce, CharInfo::CharSet encoding)
    : CharType(h, LiteralCHAR, maxLen, CharInfo::maxBytesPerChar(cs), TRUE, allowSQLnull, isUpShifted, FALSE, FALSE, cs,
               co, ce, encoding)
//##
...
short SQLChar::getFSDatatype() const {
  switch (getBytesPerChar()) {
    ...
    case 4:
      if (getCharSet() == CharInfo::UTF8) return REC_BYTE_F_ASCII;
      // fall through
    default:
      ComASSERT(FALSE);
      return REC_BYTE_F_ASCII;
...
short SQLVarChar::getFSDatatype() const {
  switch (getBytesPerChar()) {
    ...
    case 4:
      if (getCharSet() == CharInfo::UTF8) return REC_BYTE_V_ASCII;
      // fall through
    default:
      ComASSERT(FALSE);
      return REC_BYTE_V_ASCII;
...
int CharType::getPrecisionOrMaxNumChars() const {
  if ((charLimitInUCS2or4chars_ > 0) && (charSet_ == CharInfo::UTF8) &&
      (charLimitInUCS2or4chars_ < getDataStorageSize()))
    return charLimitInUCS2or4chars_;
  return 0;  // no limits for now
}

int CharType::getScaleOrCharset() const { return (int)getCharSet(); }
...
// -----------------------------------------------------------------------
NAString *CharType::getKey(CollHeap *h) const { return new (h) NAString(getTypeSQLname(), h); }
...
void CharType::minMaxRepresentableValue(void *bufPtr, int *bufLen, NAString **stringLiteral, NABoolean isMax,
  ...
  switch (getCharSet()) {
    ...
    case CharInfo::UTF8:
      if (isMax)
        valLen = fillWithMaxUTF8Chars(valPtr, valBufLen, getPrecisionOrMaxNumChars());
      else
        valLen = fillWithMinUTF8Chars(valPtr, valBufLen, getPrecisionOrMaxNumChars());
      break;
    ...
    case CharInfo::UCS2:
      ...
      for (i = 0; i < valBufLen; i++) ((wchar_t *)valPtr)[i] = minmax_wchar;
      break;
...
void SQLChar::minRepresentableValue(void *bufPtr, int *bufLen, NAString **stringLiteral, CollHeap *h) const {
  minMaxRepresentableValue(bufPtr, bufLen, stringLiteral, FALSE, h);
}

void SQLChar::maxRepresentableValue(void *bufPtr, int *bufLen, NAString **stringLiteral, CollHeap *h) const {
...
double SQLChar::encode(void *bufPtr) const {
  ...
  if (supportsSQLnull()) charBufPtr += getSQLnullHdrSize();
...
void SQLVarChar::minRepresentableValue(void *bufPtr, int *bufLen, NAString **stringLiteral, CollHeap *h) const {
  minMaxRepresentableValue(bufPtr, bufLen, stringLiteral, FALSE, h);
}

void SQLVarChar::maxRepresentableValue(void *bufPtr, int *bufLen, NAString **stringLiteral, CollHeap *h) const {
  minMaxRepresentableValue(bufPtr, bufLen, stringLiteral, TRUE, h);
}
...
//
const CharType *CharType::findPushDownCharType(CharInfo::CharSet cs, const CharType *first, ...) {
  if (pushDownRequiredByODBC == FALSE) return 0;
  ...
  do {
    ...
    if (ctp->getCharSet() != CharInfo::UnknownCharSet) {
      ct++;
      if (sampleCharType == 0) sampleCharType = ctp;
    }
...
const CharType *CharType::desiredCharType(enum CharInfo::CharSet cs) {
  ...
  static SQLChar sjisChar(NULL, 0, TRUE, FALSE, FALSE, FALSE,
                          CharInfo::SJIS /* ... old kludge ... CharInfo::ISO88591 */, CharInfo::DefaultCollation,
                          CharInfo::COERCIBLE, CharInfo::SJIS /*encoding*/);
  // static SQLChar sjisUnicodeChar(NULL, 0, TRUE, FALSE, FALSE, FALSE, CharInfo::SJIS, CharInfo::DefaultCollation,
  // CharInfo::COERCIBLE, CharInfo::UNICODE/*encoding*/);
  static SQLChar utf8Char(NULL, 0, TRUE, FALSE, FALSE, FALSE,
                          CharInfo::UTF8 /* ... old kludge ... CharInfo::ISO88591 */, CharInfo::DefaultCollation,
                          CharInfo::COERCIBLE, CharInfo::UTF8 /*encoding*/);
...
static int quantize(int len) {
  static const int quanLen[] = {8, 16, 32, 64, 128, 256, 512, 1024};
  int x, limit = sizeof(quanLen) / sizeof(quanLen[0]);
  if (len <= 0) {
    return quanLen[0];
  }
  if (len >= quanLen[limit - 1]) {
    return len;
  }
  for (x = 0; x < limit - 1; x++) {
    if (len <= quanLen[x]) {
...
NAType *SQLVarChar::equivalentCharType(NAMemory *h, NABoolean quantizeLen) {
  ...
  // When we set CQD DEFAULT_CHARSET 'UTF8'
  // For Chinese characters, in trafci:
  // 1) getStrCharLimit()=7, after quantize, len_in_chars will be 8
  //    getNominalSize()=21, after quantize, len_in_bytes will be 32
  //
  // 2) getStrCharLimit()=10, after quantize, len_in_chars will be 16
  //    getNominalSize()=30, after quantize, len_in_bytes will be 32
  //
  // But the len_in_bytes of 2) is not correct since
  // max length in bytes of 10 UTF8 characters should be 40.
  // It will cause 1) and 2) will hit the same query cache,
  // and 10 UTF8 characters will be truncated.
  if (quantizeLen && getCharSet() == CharInfo::UTF8) {
    int tmp = quantize(getStrCharLimit() * CharInfo::maxBytesPerChar(getCharSet()));
    len_in_bytes = max(len_in_bytes, tmp);
  }
  CharLenInfo CLInfo(len_in_chars, len_in_bytes);
  return new (h) SQLChar(h, CLInfo, supportsSQLnull(), isUpshifted(), isCaseinsensitive(), FALSE, getCharSet(),
...
NAType *SQLChar::equivalentVarCharType(NAMemory *h, NABoolean quantizeLen) {
  ...
  CharLenInfo CLInfo(len_in_chars, len_in_bytes);
  return new (h) SQLVarChar(h, CLInfo, supportsSQLnull(), isUpshifted(), isCaseinsensitive(), getCharSet(),
                            getCollation(), getCoercibility());
}

// A virtual function to return a varchar equivalent of this type
NAType *ANSIChar::equivalentVarCharType(NAMemory *h, NABoolean quantizeLen) {
  int len = quantizeLen ? quantize(getStrCharLimit()) : getStrCharLimit();
  return new (h) SQLVarChar(h, len, supportsSQLnull(), isUpshifted(), isCaseinsensitive(), getCharSet(), getCollation(),
                            getCoercibility());
}
...
short SQLLongVarChar::getTrueFSDatatype() const { return REC_BYTE_V_ASCII_LONG; }

//////////////////////////////
// class SQLlob
//////////////////////////////
SQLlob::SQLlob(NAMemory *h, NABuiltInTypeEnum ev, long lobLength, ComLobsStorageType ls, NABoolean allowSQLnull,
               int extFormatLen)
    : NAType(h, (ev == NA_LOB_TYPE ? LiteralLOB : LiteralLOB), ev, extFormatLen, allowSQLnull,
             allowSQLnull ? SQL_NULL_HDR_SIZE : 0, TRUE, SQL_VARCHAR_HDR_SIZE_4),
      lobHandleLen_(extFormatLen),
      lobStorage_(ls),
...
// class SQLBlob
/////////////////////////////////////
SQLBlob::SQLBlob(NAMemory *h, long blobLength, ComLobsStorageType lobStorage, NABoolean allowSQLnull,
                 int extFormatLen,  // inline data max len or param len
                 NABoolean lobV2)
    : SQLlob(h, NA_LOB_TYPE, blobLength, lobStorage, allowSQLnull,
             (lobV2 ? LOB_V2_EXT_HANDLE_LEN : LOB_V1_EXT_HANDLE_LEN) + extFormatLen) {
  setCharSet(CharInfo::ISO88591);  // lobhandle can only be in ISO format
}

NAType *SQLBlob::newCopy(NAMemory *h) const { return new (h) SQLBlob(*this, h); }

// -----------------------------------------------------------------------
// Type synthesis for binary operators
// -----------------------------------------------------------------------
const NAType *SQLBlob::synthesizeType(NATypeSynthRuleEnum synthRule, const NAType &operand1, const NAType &operand2,
                                      CollHeap *h, UInt32 *flags) const {
  if (operand1.getFSDatatype() != REC_BLOB || operand2.getFSDatatype() != REC_BLOB) return NULL;
  ...
  if (synthRule == SYNTH_RULE_UNION) {
    return new (h) SQLBlob(h, MAXOF(op1.getLobLength(), op2.getLobLength()), op1.getLobStorage(), null,
                           MAXOF(op1.extFormatLen(), op2.extFormatLen()));
  }
...
// class SQLClob
/////////////////////////////////////
SQLClob::SQLClob(NAMemory *h, long clobLength, CharInfo::CharSet charset, ComLobsStorageType lobStorage,
                 NABoolean allowSQLnull, int extFormatLen, NABoolean lobV2)
    : SQLlob(h, NA_LOB_TYPE, clobLength, lobStorage, allowSQLnull,
             (lobV2 ? LOB_V2_EXT_HANDLE_LEN : LOB_V1_EXT_HANDLE_LEN) + extFormatLen) {
  setDataCharSet(charset);
}

NAType *SQLClob::newCopy(NAMemory *h) const { return new (h) SQLClob(*this, h); }

// -----------------------------------------------------------------------
// Type synthesis for binary operators
// -----------------------------------------------------------------------
const NAType *SQLClob::synthesizeType(NATypeSynthRuleEnum synthRule, const NAType &operand1, const NAType &operand2,
                                      CollHeap *h, UInt32 *flags) const {
  if (operand1.getFSDatatype() != REC_CLOB || operand2.getFSDatatype() != REC_CLOB) return NULL;
  ...
  if (synthRule == SYNTH_RULE_UNION) {
    return new (h) SQLClob(h, MAXOF(op1.getLobLength(), op2.getLobLength()), op1.getCharSet(), op1.getLobStorage(),
                           null, MAXOF(op1.extFormatLen(), op2.extFormatLen()));
  }
...
// ----------------------------------------------------------------------
SQLBinaryString::SQLBinaryString(NAMemory *h, int maxLen, NABoolean allowSQLnull, NABoolean varLenFlag)
    : CharType(h, varLenFlag ? LiteralVARCHAR : LiteralCHAR, maxLen, 1, FALSE, allowSQLnull, FALSE, FALSE, varLenFlag,
               CharInfo::BINARY, CharInfo::DefaultCollation, CharInfo::COERCIBLE, CharInfo::UnknownCharSet) {}
...
// Type synthesis for binary operators
// -----------------------------------------------------------------------
const NAType *SQLBinaryString::synthesizeType(NATypeSynthRuleEnum synthRule, const NAType &operand1,
  ...
  if (DFS2REC::isAnyVarChar(operand1.getFSDatatype()) OR DFS2REC::isAnyVarChar(operand2.getFSDatatype()))
    makeTypeVarchar = TRUE;

  return new (h) SQLBinaryString(h, res_nominalSize, null, makeTypeVarchar);
}  // synthesizeType()

void SQLBinaryString::minRepresentableValue(void *bufPtr, int *bufLen, NAString **stringLiteral, CollHeap *h) const {
  minMaxRepresentableValue(bufPtr, bufLen, stringLiteral, FALSE, h);
}

void SQLBinaryString::maxRepresentableValue(void *bufPtr, int *bufLen, NAString **stringLiteral, CollHeap *h) const {
  minMaxRepresentableValue(bufPtr, bufLen, stringLiteral, TRUE, h);
}
