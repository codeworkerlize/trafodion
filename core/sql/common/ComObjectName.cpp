...
//
ostream &operator<<(ostream &out, const ComObjectName &name) {
#ifndef NDEBUG
  ...
  out << endl;
  out << "        catalogNamePart_ = [" << name.catalogNamePart_.getExternalName().length() << "] "
      << name.catalogNamePart_.getExternalName() << endl;
  out << "         schemaNamePart_ = [" << name.schemaNamePart_.getExternalName().length() << "] "
...
ComBoolean ComRoutineActionName::extractUudfUidAndIntActNameFromFunnyIntActNameInOBJECTS_OBJECT_NAME(
  ...
  if (p[19] NEQ '_') return FALSE;  // wrong parameter value
  char uidStrBuf[20];
...
ComObjectName::ComObjectName(CollHeap *h)
    : nameSpace_(COM_UNKNOWN_NAME), catalogNamePart_(h), schemaNamePart_(h), objectNamePart_(h), heap_(h), flags_(0) {}

ComObjectName::ComObjectName(const ComObjectName &rhs, CollHeap *h  // default is NULL - i.e. use C/C+ runtime heap
                             )
...
ComObjectName::ComObjectName(const NAString &externalObjectName, const ComAnsiNameSpace nameSpace,
                             NABoolean createDropAlias, CollHeap *h, unsigned short toInternalIdentifierFlags)
    : nameSpace_(nameSpace), catalogNamePart_(h), schemaNamePart_(h), objectNamePart_(h), heap_(h), flags_(0) {
  if (nameSpace EQU COM_UUDF_ACTION_NAME)
    objectNamePart_.toInternalIdentifierFlags_ |=
        toInternalIdentifierFlags | NASTRING_REGULAR_IDENT_WITH_DOLLAR_PREFIX | NASTRING_DELIM_IDENT_WITH_DOLLAR_PREFIX;
...
//
ComObjectName::ComObjectName(const NAString &externalObjectName, size_t &bytesScanned, const ComAnsiNameSpace nameSpace,
                             CollHeap *h)
    : nameSpace_(nameSpace), catalogNamePart_(h), schemaNamePart_(h), objectNamePart_(h), heap_(h), flags_(0) {
  if (nameSpace EQU COM_UUDF_ACTION_NAME)
    objectNamePart_.toInternalIdentifierFlags_ |=
  ...
  scan(externalObjectName, bytesScanned, FALSE /*createDropAlias*/, objectNamePart_.toInternalIdentifierFlags_);
...
//
ComObjectName::ComObjectName(const NAString &catalogNamePart, const NAString &schemaNamePart,
                             const NAString &objectNamePart, const ComAnsiNameSpace nameSpace,
                             const ComAnsiNamePart::formatEnum format, CollHeap *h)
    : nameSpace_(nameSpace), catalogNamePart_(h), schemaNamePart_(h), objectNamePart_(h), heap_(h), flags_(0) {
  if (nameSpace EQU COM_UUDF_ACTION_NAME)
    objectNamePart_.toInternalIdentifierFlags_ |=
        NASTRING_REGULAR_IDENT_WITH_DOLLAR_PREFIX | NASTRING_DELIM_IDENT_WITH_DOLLAR_PREFIX;
  ...
  switch (format) {
    case ComAnsiNamePart::EXTERNAL_FORMAT: {
      ...
      ComAnsiNamePart obj(objectNamePart, ComAnsiNamePart::EXTERNAL_FORMAT, h,
                          objectNamePart_.toInternalIdentifierFlags_);
      objectNamePart_ = obj;
      break;
    }
    ...
    case ComAnsiNamePart::INTERNAL_FORMAT: {
      ...
      ComAnsiNamePart obj(objectNamePart, ComAnsiNamePart::INTERNAL_FORMAT, h,
                          objectNamePart_.toInternalIdentifierFlags_);
...
//
ComObjectName::ComObjectName(const NAString &catalogNamePart, const NAString &schemaNamePart,
                             const NAString &objectNamePart, const ComAnsiNameSpace nameSpace, NABoolean doNotParse)
    : nameSpace_(nameSpace), flags_(0) {
...
ComObjectName::ComObjectName(const ComAnsiNamePart &objectNamePart, const ComAnsiNameSpace nameSpace)
    : objectNamePart_(objectNamePart), nameSpace_(nameSpace), flags_(0) {
  if (nameSpace EQU COM_UUDF_ACTION_NAME)
    objectNamePart_.toInternalIdentifierFlags_ |=
        NASTRING_REGULAR_IDENT_WITH_DOLLAR_PREFIX | NASTRING_DELIM_IDENT_WITH_DOLLAR_PREFIX;
...
ComObjectName::ComObjectName(const ComAnsiNamePart &schemaNamePart, const ComAnsiNamePart &objectNamePart,
                             const ComAnsiNameSpace nameSpace)
    : schemaNamePart_(schemaNamePart), objectNamePart_(objectNamePart), nameSpace_(nameSpace), flags_(0) {
  if (nameSpace EQU COM_UUDF_ACTION_NAME)
    objectNamePart_.toInternalIdentifierFlags_ |=
        NASTRING_REGULAR_IDENT_WITH_DOLLAR_PREFIX | NASTRING_DELIM_IDENT_WITH_DOLLAR_PREFIX;
...
ComObjectName::ComObjectName(const ComAnsiNamePart &catalogNamePart, const ComAnsiNamePart &schemaNamePart,
                             const ComAnsiNamePart &objectNamePart, const ComAnsiNameSpace nameSpace)
    : catalogNamePart_(catalogNamePart),
      schemaNamePart_(schemaNamePart),
...
ComObjectName::ComObjectName(const ComUID &uudfUid  // UID of UUDF associating with UDF action
  ...
  initialize(uudfUid, catalogNamePart, schemaNamePart, routineActionNamePart, nameSpace, format, parseInputNameParts);
...
// virtual
const ComRoutineActionName *ComObjectName::castToComRoutineActionName() const { return NULL; }

// virtual
ComRoutineActionName *ComObjectName::castToComRoutineActionName() { return NULL; }

//
// assignment operator
//
ComObjectName &ComObjectName::operator=(const NAString &rhsObjectName) {
  clear();
  scan(rhsObjectName);
  return *this;
}

ComObjectName &ComObjectName::operator=(const ComObjectName &rhsObjectName) {
...
const NAString &ComObjectName::getCatalogNamePartAsAnsiString(NABoolean v) const {
  if (v == FALSE)
...
const NAString &ComObjectName::getSchemaNamePartAsAnsiString(NABoolean v) const {
  if (v == FALSE)
...
const NAString &ComObjectName::getObjectNamePartAsAnsiString(NABoolean v) const {
  if (v == FALSE)
...
/* virtual */ NAString ComObjectName::getExternalName(NABoolean formatForDisplay, NABoolean displayedExternally) const {
  ...
  if ((NOT isVolatile()) || (NOT formatForDisplay)) {
    if (NOT schemaNamePart_.isEmpty()) {
      extObjName = getSchemaNamePartAsAnsiString() + NAString(ComSqlText.getPeriod()) + extObjName;

      if (NOT catalogNamePart_.isEmpty() && NOT displayedExternally) {
#ifndef NDEBUG
        ok = 1;
#endif
        extObjName = getCatalogNamePartAsAnsiString() + NAString(ComSqlText.getPeriod()) + extObjName;
      }
    }
  }
...
NAString ComObjectName::getExternalNameWithNameSpace(NABoolean formatForDisplay) const {
  NAString extObjName;
...
void ComObjectName::applyDefaults(const ComAnsiNamePart &catalogNamePart, const ComAnsiNamePart &schemaNamePart) {
  if (getSchemaNamePart().isEmpty()) setSchemaNamePart(schemaNamePart);
  if (getCatalogNamePart().isEmpty()) setCatalogNamePart(catalogNamePart);
}
...
void ComObjectName::clearIfInvalid() {
  if (NOT catalogNamePart_.isEmpty()) {
    if (schemaNamePart_.isEmpty() OR objectNamePart_.isEmpty()) clear();
    // else okay
  } else  // catalog name part is empty
  {
    if (NOT schemaNamePart_.isEmpty() AND objectNamePart_.isEmpty()) clear();
    // else okay
  }
}
...
NABoolean ComObjectName::initialize(const ComUID &uudfUid  // UID of UUDF associating with UDF action
                                    ,
                                    const NAString &catalogNamePart, const NAString &schemaNamePart,
                                    const NAString &routineActionNamePart,
                                    const ComAnsiNameSpace nameSpace  // must be COM_UUDF_ACTION_NAME
  ...
  if (parseInputNameParts) {
    if (format EQU ComAnsiNamePart::EXTERNAL_FORMAT) {
      ...
      status = (ComSInt32)ToInternalIdentifier(workName  // in/out
                                               ,
    ...
    } else  // INTERNAL_FORMAT
    {
      ...
      ComRoutineActionNamePart actNP2(uudfUid, routineActionNamePart, ComAnsiNamePart::INTERNAL_FORMAT);
      if (actNP2.isValid()) {
  ...
  } else  // DO NOT parseInputNameParts
  {
    ...
    } else  // INTERNAL_FORMAT
    {
      ...
      catalogNamePart_.setExternalName(ToAnsiIdentifier(catalogNamePart, FALSE /*assertShort*/));
      schemaNamePart_.setInternalName(schemaNamePart);
      schemaNamePart_.setExternalName(ToAnsiIdentifier(schemaNamePart, FALSE /*assertShort*/));
      ComRoutineActionNamePart actNP4;
      actNP4.setUudfUID(uudfUid);
      actNP4.setInternalName(routineActionNamePart);
      actNP4.setExternalName(ToAnsiIdentifier(routineActionNamePart, FALSE /*assertShort*/));
...
//
NABoolean ComObjectName::scan(const NAString &externalObjectName, NABoolean createDropAlias,
                              unsigned short toInternalIdentifierFlags) {
  size_t bytesScanned = 0;  // initialize this to 'off' for scan()
  return scan(externalObjectName, bytesScanned, createDropAlias, toInternalIdentifierFlags);
}
...
//
NABoolean ComObjectName::scan(const NAString &externalObjectName, size_t &bytesScanned, NABoolean createDropAlias,
                              unsigned short toInternalIdentifierFlags) {
  size_t count;
  ...
  ComAnsiNamePart part1(externalObjectName, count, 0, createDropAlias, FALSE  // NABoolean acceptCircumflex
                        ,
  ...
  count = trailingJunkIsOk;
  ComAnsiNamePart part3(remainingName, count, 0, createDropAlias,
                        (nameSpace_ == COM_MODULE_NAME)  // NABoolean acceptCircumflex
                        ,
                        toInternalIdentifierFlags  // unsigned short
  );
  bytesScanned += count;
  if (NOT part3.isValid()) return FALSE;
...
void ComObjectName::convertToPackageRoutineName() {
  if (!isAmbiguous()) return;
  assert(!schemaNamePart_.getInternalName().isNull());
  NAString &objNameInt = const_cast<NAString &>(objectNamePart_.getInternalName());
  NAString &objNameExt = const_cast<NAString &>(objectNamePart_.getExternalName());
  objNameInt.prepend(".");
  objNameInt.prepend(schemaNamePart_.getInternalName());
  objNameExt.prepend(".");
  objNameExt.prepend(schemaNamePart_.getExternalName());
  objNameExt.prepend("\"");
  objNameExt.append("\"");
  schemaNamePart_.clear();
  if (!catalogNamePart_.getInternalName().isNull()) {
    schemaNamePart_.setInternalName(catalogNamePart_.getInternalName());
    schemaNamePart_.setExternalName(catalogNamePart_.getExternalName());
...
// ostream insertion operator
//
ostream &operator<<(ostream &out, const ComRoutineActionName &name) { return out; }
...
ComRoutineActionName::ComRoutineActionName(CollHeap *h)  // default of h is NULL - i.e. use C/C++ runtime heap
    : ComObjectName(h), routineActionNamePart_(h), uudfComObjectName_(h) {}
...
ComRoutineActionName::ComRoutineActionName(const ComUID &uudfUid  // e.g., UID of UUDF sas_score
    ...
    : ComObjectName(h),
      ...
      uudfComObjectName_(uudfNameAsAnsiName, COM_UDF_NAME  // const ComAnsiNameSpace nameSpace
                         ,
...
ComRoutineActionName::ComRoutineActionName(const ComUID &uudfUid  // e.g., UID of UUDF sas_score
                                           ,
                                           const ComObjectName &uudfObjName, const NAString &catalogNamePart,
                                           const NAString &schemaNamePart, const NAString &routineActionNamePart,
                                           const ComAnsiNamePart::formatEnum format,
                                           ComBoolean parseInputNameParts  // default is TRUE
                                           ,
                                           CollHeap *h  // default of h is 0 (i.e., NULL)
                                           )
    : ComObjectName(uudfUid, catalogNamePart, schemaNamePart, routineActionNamePart, COM_UUDF_ACTION_NAME, format,
                    parseInputNameParts, h),
      routineActionNamePart_(h),
      uudfComObjectName_(uudfObjName, h) {
  initializeRoutineActionNamePart(uudfUid, uudfObjName, catalogNamePart, schemaNamePart, routineActionNamePart, format,
                                  parseInputNameParts);
}
...
// virtual
const ComRoutineActionName *ComRoutineActionName::castToComRoutineActionName() const { return this; }

// virtual
ComRoutineActionName *ComRoutineActionName::castToComRoutineActionName() { return this; }

//
// accessors
//

const NAString &ComRoutineActionName::getRoutineActionNamePartAsAnsiString(NABoolean) const {
...
// returned.
//
/* virtual */ NAString ComRoutineActionName::getExternalName(NABoolean formatForDisplay,
  ...
  if (NOT formatForDisplay) {
    if (NOT getSchemaNamePart().isEmpty()) {
      extActName = getSchemaNamePartAsAnsiString() + NAString(ComSqlText.getPeriod()) + extActName;

      if (NOT getCatalogNamePart().isEmpty() AND NOT displayedExternally) {
#ifdef _DEBUG
        ok = 1;
#endif
        extActName = getCatalogNamePartAsAnsiString() + NAString(ComSqlText.getPeriod()) + extActName;
      }
    }
  }
...
#ifdef _DEBUG
  if (NOT ok) {
    if (extActName.isNull()) cerr << "ERROR: empty ComRoutineActionName" << endl;
    // else
...
NABoolean ComRoutineActionName::set(const ComUID &uudfUid  // e.g., UID of UUDF sas_score
                                    ,
  ...
  ComObjectName kludge(routineActionNameAsAnsiName, COM_UUDF_ACTION_NAME  // ComAnsiNameSpace
                       ,
                       FALSE  // NABoolean createDropAlias - not applicable
  );
  if (kludge.isValid()) {
    ComRoutineActionNamePart actNP(uudfUid, kludge.getObjectNamePart().getInternalName(),
                                   ComAnsiNamePart::INTERNAL_FORMAT);
    if (actNP.isValid()) {
      ComAnsiNamePart objNP;
      NABoolean isOK = actNP.getRoutineActionNameStoredInOBJECTS_OBJECT_NAME(objNP  // out
                                                                             ,
...
NABoolean ComRoutineActionName::set(const ComUID &uudfUid  // e.g., UID of UUDF sas_score
  ...
  clear();
  NABoolean isOkay = ComObjectName::initialize(uudfUid, catalogNamePart, schemaNamePart, routineActionNamePart,
                                               COM_UUDF_ACTION_NAME, format, parseInputNameParts);
  if (NOT isOkay) return FALSE;

  return initializeRoutineActionNamePart(uudfUid, uudfObjName, catalogNamePart, schemaNamePart, routineActionNamePart,
                                         format, parseInputNameParts);
}
...
NABoolean ComRoutineActionName::isEmpty() const {
  return (routineActionNamePart_.isEmpty() AND getSchemaNamePart().isEmpty() AND getCatalogNamePart().isEmpty());
}

// virtual
NABoolean ComRoutineActionName::isValid() const { return (NOT routineActionNamePart_.isEmpty()); }
...
NABoolean ComRoutineActionName::initializeRoutineActionNamePart(
  ...
  uudfComObjectName_ = uudfObjName;
  if (uudfComObjectName_.isValid() AND uudfComObjectName_.getNameSpace() EQU COM_UNKNOWN_NAME) {
    uudfComObjectName_.setNameSpace(COM_UDF_NAME);
  }

  if (parseInputNameParts) {
    ComRoutineActionNamePart actNP(uudfUid, routineActionNamePart, format);
    if (actNP.isValid()) routineActionNamePart_ = actNP;
    setNameSpace(COM_UUDF_ACTION_NAME);
    clearIfInvalid();
    return isValid();
  } else  // DO NOT check - assume all input parameters are valid
  {
    ...
    } else  // INTERNAL_FORMAT
    {
      ...
      routineActionNamePart_.setExternalName(ToAnsiIdentifier(routineActionNamePart, FALSE /*assertShort*/));
    }  // INTERNAL_FORMAT
    setNameSpace(COM_UUDF_ACTION_NAME);
...
void ComRoutineActionName::clearIfInvalid() {
  if (NOT getCatalogNamePart().isEmpty()) {
    if (getSchemaNamePart().isEmpty() OR routineActionNamePart_.isEmpty()) clear();
    // else okay
  } else  // catalog name part is empty
  {
    if (NOT getSchemaNamePart().isEmpty() AND routineActionNamePart_.isEmpty()) clear();
    // else okay
  }
}
