...

{
  ...
  if (divisorLengthInShorts > 1) return -1;  // Error.
...
short BigNumHelper::DivHelper(Lng32 dividendLength, Lng32 divisorLength, char *dividendData, char *divisorData,
  ...
  for (j = 0; j <= m; j++) {
    ...
    if (neg) {
      ...
      BigNumHelper::SubHelper(divisorLength + 2, tempDivisorData,
...
short BigNumHelper::RoundHelper(Lng32 sourceLength, Lng32 targetLength, char *sourceData, Int64 roundingValue,
  ...
  rc = ConvBigNumToBcdHelper(sourceLength, sizeof(tempBCDDataInShorts), sourceData, (char *)tempBCDDataInShorts,
                             NULL);  // TODO: need heap?
  ...
  if (rc == 0) {
    ...
    size_t limit = 0;
    if (roundingValue < 0) roundingValue = 0;  // don't go off the right end; roundingValue of 0 is a no-op

    if (MaxPrecision - 1 - roundingValue > 0)  // if not off left end
    {
      limit = MaxPrecision - 1 - roundingValue;                                  // lowest digit to retain
      if (roundingValue > 0) roundUp = (tempBCDDataInChars[limit + 1] >= 0x05);  // highest digit to zero out (if any)
    }
...

{
  ...
  if (!actualSourceLengthInShorts) {
    if (heap) NADELETEBASIC(tempSourceDataInShorts, (heap));
    return 0;
  }
...
short BigNumHelper::ConvBigNumWithSignToBcdHelper(Lng32 sourceLength, Lng32 targetLength, char *sourceData,
  ...
  short returnValue =
      BigNumHelper::ConvBigNumToBcdHelper(sourceLength, targetLength - 1, sourceData, targetData + 1, heap);
...
short BigNumHelper::ConvBigNumWithSignToAsciiHelper(Lng32 sourceLength, Lng32 targetLength, char *sourceData,
  ...
  short returnValue =
      BigNumHelper::ConvBigNumToAsciiHelper(sourceLength, targetLength - 1, sourceData, targetData + 1, heap);
...

{
  ...
  *finalTargetLength = BigNumHelper::ConvPrecisionToStorageLengthHelper(exponent);
  ...
  for (k = 2 * currentTargetLengthInShorts; k < *finalTargetLength; k++) targetData[k] = 0;
...

{
  ...
  *tgt64 = sourceData;
  ...
  if (isNeg) BIGN_SET_SIGN(tgt, targetLength);
...

{
  ...
  while (sourceDataInShorts[sourceLengthInShorts - 1] == 0 && sourceLengthInShorts > 4) sourceLengthInShorts--;
...
short BigNumHelper::ConvBigNumWithSignToInt64AndScaleHelper(Lng32 sourceLength, char *sourceData, Int64 *targetData,
  ...
  if (exponent < 0) {
    ...
    if (scaleFactorLengthInShorts > sourceLengthInShorts) {
      ...
      anyTruncation = BigNumHelper::SimpleDivHelper(sourceLength, 2, sourceData, scaleFactor, sourceDataAfterScaling);
      sourceLengthAfterScalingInShorts = sourceLengthInShorts;
    } else {
      anyTruncation = BigNumHelper::DivHelper(sourceLength, scaleFactorLength, sourceData, scaleFactor,
                                              sourceDataAfterScaling, tempData);
      sourceLengthAfterScalingInShorts = sourceLengthInShorts;
    }
  }
    ...
    BigNumHelper::MulHelper(sourceLength + scaleFactorLength,
                            ...
                            scaleFactorLength, sourceData, scaleFactor, sourceDataAfterScaling);
...
short BigNumHelper::ConvBigNumWithSignToBigNumWithSignHelper(Lng32 sourceLength, Lng32 targetLength, char *sourceData,
  ...
  while ((sourceDataInShorts[sourceLengthInShorts - 1] == 0) && (sourceLengthInShorts > 2)) sourceLengthInShorts--;
  ...
  for (k = 0; k < sourceLengthInShorts; k++) targetDataInShorts[k] = sourceDataInShorts[k];
