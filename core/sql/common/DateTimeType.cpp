					   ...
					   UInt32 *flags) const
{
  ...
  case SYNTH_RULE_SUB:
	...
	const DatetimeType* datetime2 = (DatetimeType*) &operand2;
        ...
        NABoolean modeCompatible1 = 
          ((flags) && ((*flags & NAType::MODE_COMPATIBLE_1) != 0));
                                  ...
                                  NABoolean validate) const
{
  ...
  if (containsField (fieldToScan, startField_, endField_))
   {
     if (! scanField(strValue, fieldToScan, values, validate))
      return FALSE;
    needSeparator = TRUE;
   }
...
NABoolean DatetimeValue::scanAndReturnFields(const char* &strValue,
                                             DatetimeFormat &format,
                                             ULng32 values[]) const
{
                                   ...
                                   NABoolean validate) const
{
  ...
  //
  if (validate &&
      (value < DatetimeType::getMinValue(field, digits) || 
       value > DatetimeType::getMaxValue(field, digits)))
    return FALSE;
  values[field] = value;
  return TRUE;
                                    ...
                                    UInt32* dtvFields)
{
  ...
  dtvFields[FRACTION] = (UInt32)((tsFields[6] * 1000 + tsFields[7]) 
                                 / (Int64)pow(10, 6 - MINOF(6, fracSec)));
}
...
int DatetimeValue::convertToUtcFromDatetime(const char * value, Int32 valueLen,
                                         Int32 fractionPrec, time_t &utcTime, Lng32 &nanoSecs)
{
  const char *datetimeOpData = value;
  short year;
  char month;
  ...
  if (utcTime == -1 && errno > 0)
     return errno;
  //fixup to convert from prolectic gregorian to hybrid (julian + gregorian that java expects) based on formula in https://en.wikipedia.org/wiki/Gregorian_calendar
  if (utcTime< -12219292800) { // this constant is the equivalent of 1582-10-15 00:00:00, cut off date of Gregorian calandar
	  utcTime += 86400 * (year/100 - year/400 - 2); 	//86400 is number of seconds in a day formula in parenthesis is "Difference between Gregorian and Julian calendar dates
  }
  nanoSecs = fraction;
