...
LongHeldLockController::LongHeldLockController(const char *lockKey, const char *commandText, bool isDedicated,
                                               NAHeap *heap)
  ...
  if (distLockInterface_) {
    ...
    if (rc != DL_OK) lockMethodFailure_ = true;
  } else
    lockMethodFailure_ = true;  // couldn't get distLockInterface
}

WaitedLockController::WaitedLockController(const char *lockKey, long timeout) : timeout_(timeout) {
  distLockInterface_ = DistributedLock_JNI::getInstance();
...
IndependentLockController::IndependentLockController(const char *lockKey, long timeout, bool isDedicated)
    : LockController(isDedicated, NULL), timeout_(timeout) {
  if (isDedicated) {
    DL_RetCode dlr = DL_OK;
    distLockInterface_ = DistributedLock_JNI::newInstance(heap_, dlr /* out, but ignored */);
  } else
...
DistributedLockObserver::DistributedLockObserver(const char *lockKey, UInt32 useCount)
    : LockController(), lockKey_(lockKey), useCount_(useCount), ct_(0) {
  distLockInterface_ = DistributedLock_JNI::getInstance();
}
...
bool DistributedLockObserver::lockHeld() {
  if (!distLockInterface_) return false;
