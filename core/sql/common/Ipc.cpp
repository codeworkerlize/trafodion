...
IpcConnection::IpcConnection(IpcEnvironment *env, const IpcProcessId &pid, const char *eye)
  ...
  // assign an id and insert this connection into the global
  // array of connections
  id_ = env->getAllConnections()->registerNewConnection(this);
...
NABoolean IpcAllConnections::unRegisterConnection(IpcConnection *conn) {
  ...
  assert(!pendingIOs_->contains(connId));
  ...
  while (NOT somethingCompleted) {
    if (totalWaitTime >= timeout AND timeout != IpcInfiniteTimeout) {
      ...
      if (seqNo != allc->getCompletionSeqenceNo()) {
        ...
        // environment. Do not leave this line uncommented for
        // production versions (performance penalty)!!
        env->wakeAllThreads();
      }
...
IpcMessageBuffer *IpcMessageBuffer::addChunkHeadersAndTrailer(IpcMessageObjSize chunkSize, IpcEnvironment *env) {
  ...
  int chunkHdrBytes = sizeof(InternalChunkHdrStruct) * (numChunks - 1);
  ...
  result->msgLength_ = newSize;
  result->getPayloadHeader()->setHasChunkHeaders();
...
void IpcMessageBuffer::removeChunkHeadersAndTrailer(IpcMessageObjSize chunkSize, IpcEnvironment *env) {
  ...
  assert(getPayloadHeader()->getHasChunkHeaders());

  // bring any chunks that arrived out-of-order back in line
  ...
  // exactly chunkHdrBytes bytes of data, not less, even if that means
  // we add some random bytes that go beyond the original message.
  assert(dataInLastChunk >= chunkHdrBytes);

  // Find the last piece of the displaced data
  IpcMessageBufferPtr chunkSrcData = data(msgLength_);
  ...
  // a sanity check to make sure we put Humpty Dumpty back together correctly
...
IpcMessageObjSize IpcMessageBuffer::getLengthWithChunkHeaders(IpcMessageObjSize chunkSize,
  ...
  IpcMessageObjSize result = msgLength + sizeof(InternalChunkMsgTrailerStruct);
...
IpcMessageObjSize IpcMessageBuffer::roundUpBufferLength(IpcMessageObjSize chunkSize, IpcMessageObjSize newMaxLen) {
  ...
  // same size. Also, if we have multiple chunks, the first chunk
  // (this IpcMessageBuffer) must be chunkSize bytes long, since we
  // don't support intermediate chunks that are smaller than
  // chunkSize.
  ...
  while (i < inUseBufList_.entries()) {
    ...
    if (msgHdr->getNextFromOffset() == NULL) {  // all objects are no longer inuse, free buffer
      ...
      while (!(msgHdr->isLastMsgBuf())) {  // skip over remaining buffers for this message.
        ...
        msgHdr = msgBuf->getPayloadHeader();
        i++;
      }
    }
  }
...
void IpcBufferedMsgStream::setSMLastInBatch() {
  ...
  InternalMsgHdrInfoStruct *hdr = msgBuf->getPayloadHeader(FALSE);
  hdr->setSMLastInBatch();
}
