...
NAMemory::sysFreeBlock(NABlock *blk)
{
  ...
  if ( getSharedMemory() ) {
    if ( segmentDB_ )
      segmentDB_->freeSharedSegment(blk);
  }
  else
    free((void*)blk);
}
...
void
NAMemory::initMe(const char * name,
           SEG_ID  segmentId,
           void  * baseAddr,
           off_t   heapStartOffset,
           size_t  maxSize,
           NABoolean failureIsFatal,
           NABoolean firstBlockIsSticky)
{
    type_ = EXECUTOR_MEMORY,
    parent_ = NULL;
    firstBlk_ = NULL;
  ...
  if (heapStartAddr_ != NULL) {
    ...
    if (tsize > (8 * sizeof(size_t))) {
      ...
      if ( firstBlockIsSticky )
        firstBlk_->sflags_ |= NABlock::STICKY_BIT;
    }
  }
  upperLimit_ = maxSize;
  // need to initialize an NAStringRef object "on top" of the array
...
NAMemory::NAMemory(const char * name,
           ...
           NABoolean failureIsFatal)
{
   NAMemory::initMe(name, segmentId, baseAddr,
          heapStartOffset, maxSize, failureIsFatal);
}
...
{
   NAMemory::initMe(name, segmentId, baseAddr,
          heapStartOffset, maxSize, failureIsFatal, !!segmentDB);
    ...
    case DERIVED_MEMORY:
      {
        ...
        if ( !namedSemaphoreLocked && semRetcode == 1 )
            releaseRTSSemaphore();
...
// - 3/22/2012.
      ...
      if ( !namedSemaphoreLocked && retcode == 1)
         releaseRTSSemaphore();
...
{
  size_t limit = ( upperLimit_ == 0 ) ? 
                 MAX_MEMORY_SIZE_IN_AN_ALLOC : upperLimit_;
...
void NAHeap::dumpHeapInfo(ostream* outstream, Lng32 indent, 
                          bool doCheck, bool dumpFragmentDetails)
{
  NAMutexScope mutex(mutex_);
  ...
  *outstream << ind << "Dump of Heap             " << this << ':' << endl
	     ...
	     << ind << "bitmap for small bins:   " <<
                       std::hex << smallmap_ << std::dec << endl
	     << ind << "bitmap for large bins:   " <<
                       std::hex << treemap_ << std::dec << endl;
  ...
  while (p) {
    *outstream << "Block: start addr=" << static_cast<void*>(p) 
               << ", end addr=" << static_cast<void*>(p+p->size_-1)
               << ", size=" << p->size_ << " bytes"
               << endl;
  ...
  while (blockHolds(q) && q != top && !q->isFencePost()) {
    ...
    if (dumpFragmentDetails) {
      *outstream << "fragment: "
                 ...
                 << ", pinuse=" << q->pinuse()
                 << ", size=" << q->fragmentSize()
                 << ", nextFrag=" << static_cast<void*>(q->nextFragment())
                 << endl;
    }
...
size_t
NAHeap::traverseAndCheck(const char* msg)
{
...
{
   if ( checkIfDistributedLockLocked() )
     return TRUE;
...
NABoolean NAMemory::checkIfNamedSemaphoreLocked()
{
   return ( namedSemaphore_ && 
            namedSemaphore_->semaphoreLocked() );
}

NABoolean NAMemory::checkIfNamedSemLocked()
{
   return ( namedSem_ && 
            namedSem_->semaphoreLocked() );
}

NABoolean NAMemory::checkIfDistributedLockLocked()
{
   return ( lockController_ && 
            lockController_ -> lockHeld() );
}
