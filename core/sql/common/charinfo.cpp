...
const char *CharInfo::getCharSetName(CharSet cs, NABoolean retUnknownAsBlank) {
  if (cs >= CHARSET_MIN && cs <= CHARSET_MAX) {
    if (cs != CharInfo::UnknownCharSet) return mapCSArray[cs - CHARSET_MIN].name;
  }
...
CharInfo::CharSet CharInfo::getCharSetEnum(const char *name) {
  ...
  for (size_t i = 0; i < SIZEOF_CS; i++) {
    ...
    if (name == map->name)  // fastpath: pointers identical
      ...
      if (strncmp(name, map->name, len) == 0 && (name[len] == '\0' || name[len] == ' '))  //##fails if "My Coll"!
        return map->cs;
    }
  }
  ...
  if (strncmp(name, alias, aliasLen) == 0 && (name[aliasLen] == '\0' || name[aliasLen] == ' ')) {
...
#ifdef _DEBUG
    if (IF_WIDE == FALSE && mapCSArray[cs - CHARSET_MIN].maxBytesPerChar > 1 &&  // SJIS or UNICODE
...
NABoolean CharInfo::isHexFormatSupported(CharSet cs) {
  return ((cs == CharInfo::ISO88591) || (cs == CharInfo::UNICODE) || (cs == CharInfo::UTF8) || (is_NCHAR_MP(cs)) ||
          (cs == CharInfo::BINARY));
}

NABoolean CharInfo::isTerminalCharSetSupported(CharSet cs) {
  return ((cs == CharInfo::ISO88591) || (cs == CharInfo::SJIS) || (cs == CharInfo::EUCJP) || (cs == CharInfo::BIG5) ||
          (cs == CharInfo::GB18030) || (cs == CharInfo::GB2312) || (cs == CharInfo::GBK) || (cs == CharInfo::KSC5601) ||
          (cs == CharInfo::UTF8));
}

NABoolean CharInfo::isMsgCharSetSupported(CharSet cs) { return ((cs == CharInfo::UTF8) || (cs == CharInfo::UNICODE)); }
...
NABoolean CharInfo::isVariableWidthMultiByteCharSet(CharSet cs) {
  if (cs >= CHARSET_MIN && cs <= CHARSET_MAX) {
    return (mapCSArray[cs - CHARSET_MIN].minBytesPerChar != mapCSArray[cs - CHARSET_MIN].maxBytesPerChar);
  }
...
CollationInfo::CollationInfo(CollHeap *h, CharInfo::Collation co, const char *name, CollationFlags flags,
...
//
CollationDB::CollationDB(CollHeap *h) : CollationDBSupertype(h), heap_(h), refreshNeeded_(TRUE) {
  if (this == CharInfo::builtinCollationDB_) return;
  if (cmpCurrentContext != NULL) cmpCurrentContext->getCollationDBList()->insert(this);
}
...
void CollationDB::Display() {
  ...
  for (i = 0; i < n; i++) (*CDBlist)[i]->display();
}
...
inline CollationDB *CollationDB::nextCDB() const {
...
const CollationInfo *CollationDB::getCollationInfo(CharInfo::Collation co) const {
  ...
  n = entries();
  for (i = 0; i < n; i++)
    if (co == at(i)->co_) return at(i);

  CollationDB *next = nextCDB();
  return next ? next->getCollationInfo(co) : NULL;
}

const char *CollationDB::getCollationName(CharInfo::Collation co, NABoolean retUnknownAsBlank) const {
...
//
CharInfo::Collation CollationDB::getCollationEnum(const char *name, NABoolean formatNSK, size_t namlen) const {
  ComASSERT(namlen);
  ...
  for (CollIndex i = 0; i < n; i++) {
    ...
    if (map->co_ >= CharInfo::FIRST_USER_DEFINED_COLLATION && formatNSK XOR HasMPLocPrefix(map->name_)) continue;
  ...
  CollationDB *next = nextCDB();
  return next ? next->getCollationEnum(name, formatNSK, namlen) : CharInfo::UNKNOWN_COLLATION;
...
static const CollationInfo mapCOArray[] = {
    CollationInfo(NULL, CharInfo::DefaultCollation, SQLCOLLATIONSTRING_DEFAULT, STATIC_STR),
    CollationInfo(NULL, CharInfo::CZECH_COLLATION, SQLCOLLATIONSTRING_CZECH, STATIC_STR),
    CollationInfo(NULL, CharInfo::CZECH_COLLATION_CI, SQLCOLLATIONSTRING_CZECH_CI, STATIC_STR),
    CollationInfo(NULL, CharInfo::UNKNOWN_COLLATION, SQLCOLLATIONSTRING_UNKNOWN, STATIC_NEG)};

#define SIZEOF_CO (sizeof(mapCOArray) / sizeof(CollationInfo))

const CollationDB *CharInfo::builtinCollationDB_ = NULL;

CharInfo::Collation CharInfo::getCollationEnum(const char *name, NABoolean formatNSK, size_t namlen) {
  if (namlen == 0)
    namlen = strlen(name);
  else {
...
const char *CharInfo::getCollationName(Collation co, NABoolean retUnknownAsBlank) {
...
const char *CharInfo::getCoercibilityText(Coercibility ce) {
  ...
  switch (ce) {
    ...
    case COERCIBLE:
      return "coercible";
    case NO_COLLATING_SEQUENCE:
      return "no-collating-sequence";
    default:
      return "unknown";
  }
}
...
Int32 CharInfo::getMaxConvertedLenInBytes(CharSet sourceCS, Int32 sourceLenInBytes, CharSet targetCS) {
  ...
  // treat UCS2 as UTF16.
  return ((sourceLenInBytes / minBytesPerChar(sourceCS)) * maxBytesPerChar(targetCS));
}

const CollationDB *CharInfo::builtinCollationDB() {
  if (CharInfo::builtinCollationDB_ != NULL) return CharInfo::builtinCollationDB_;
  globalSemaphore.get();
