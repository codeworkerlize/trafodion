...
static void addDelimiter(NAText &s)
{
  s.append(NAWNodeSetDelimStr);
}
...
static void addString(const char *v, NAText &s)
{
  s.append(v);
}
...
{
  while (*s != 0 && *s != NAWNodeSetDelim)
    s++;

  if (*s == NAWNodeSetDelim)
...
const NAASNodes *NAWNodeSet::castToNAASNodes() const
{
  return NULL;
}

NAASNodes *NAWNodeSet::castToNAASNodes()
{
  return NULL;
}

const NAWSetOfNodeIds *NAWNodeSet::castToNAWSetOfNodeIds() const
{
  return NULL;
}

NAWSetOfNodeIds *NAWNodeSet::castToNAWSetOfNodeIds()
{
  return NULL;
}

bool NAWNodeSet::canUseAllNodes(NAClusterInfo *nacl) const
{
  return (getNumNodes() >= nacl->getTotalNumberOfCPUs());
}
...
bool NAWNodeSet::canUseAllAvailableUnits(NAClusterInfo *nacl) const
{
  ...
  return
    w < 0 ||
    w >= nacl->numberOfTenantUnitsInTheCluster();
}

bool NAWNodeSet::usesTheSameNodes(const NAWNodeSet *other,
...
NAWNodeSet * NAWNodeSet::deserialize(const char *serializedString,
                                     CollHeap *heap)
{
...
NAASNodes::NAASNodes(int totalWeight,
                     int clusterSize,
                     int affinity) :
     size_(totalWeight),
     ...
     affinity_(affinity)
{
  assert(size_ >= -1 && clusterSize_ >= -1 && affinity_ >= -1 &&
         size_ != 0 && clusterSize_ != 0);
}

NAWNodeSet * NAASNodes::copy(CollHeap *outHeap) const
{
  return new(outHeap) NAASNodes(size_, clusterSize_, affinity_);
}
...
const NAASNodes * NAASNodes::castToNAASNodes() const
{
  return this;
}

NAASNodes * NAASNodes::castToNAASNodes()
{
  return this;
}
...
{
  return (size_ == -1 && affinity_ == -1 && clusterSize_ == -1 ||
          NAWNodeSet::canUseAllNodes(nacl));
}

bool NAASNodes::usesTheSameNodes(const NAWNodeSet *other,
                                 NAClusterInfo *nacl) const
{
  ...
  if (oth == NULL)
    return NAWNodeSet::usesTheSameNodes(other, nacl);

  return (getClusterSize() == oth->getClusterSize() &&
          getIdOfTenantSegment() == oth->getIdOfTenantSegment());
}

int NAASNodes::makeSizeFeasible(int proposedSize,
                                int subsetSize,
                                int round) const
{
  if (subsetSize <= 0)
    subsetSize = clusterSize_;

  if (subsetSize % proposedSize == 0 ||
      proposedSize % subsetSize == 0)
  ...
  if (proposedSize < subsetSize)
      ...
      if (round >= 0)
        {
          ...
          if (round > 0 ||
              (result2-proposedSize) <= (proposedSize-result))
            result = result2;
        }
      return result;
    }
  else
    {
      ...
      if (round < 0 ||
          (round == 0 && (proposedSize-result) < subsetSize/2))
...
bool NAASNodes::isComplete() const
{
  return affinity_ >= 0;
}
...
int NAASNodes::getWeightForNodeId(int /*nodeId*/) const
{
...
{
  if (size_ == -1 && clusterSize_ == -1 && affinity_ == -1)
    return NULL;
  if (clusterSize_ <= 0)
    return "Size must be a positive integer";
  if (affinity_ < -1)
    return "Affinity must be greater or equal to -1";
...
bool NAASNodes::tryToMakeValid(NAClusterInfo *nacl,
      ...
      for (int t=0; t<4 && t<numPhysNodes; t++)
        {
          ...
          int feasibleSize = makeSizeFeasible(
               size_,
               numPhysNodes - t,
               -1);
...
NAASNodes * NAASNodes::deserialize(const char *serializedString,
                                   CollHeap *heap)
{
  const char *s = serializedString;
  ...
  if (!getIntField(s, size))
    return NULL;

  if (!getIntField(s, clusterSize))
    return NULL;

  if (!getInt(s, affinity))
    return NULL;
...
{
  NAWSetOfNodeIds *result =
    new(outHeap) NAWSetOfNodeIds(outHeap, anticipatedTotalWeight_);

  for (CollIndex ix=0; ix<nodeIds_.entries(); ix++)
    result->addNode(nodeIds_[ix], getDistinctWeight(ix));

  return result;
}

int NAWSetOfNodeIds::getNumNodes() const
{
  return nodeIds_.entries();
}

int NAWSetOfNodeIds::getTotalWeight() const
{
  return (anticipatedTotalWeight_ > 0 ? anticipatedTotalWeight_
                                      : cachedTotalWeight_);
}
...
int NAWSetOfNodeIds::getNodeNumber(int ix) const
{
  ...
  if (commonWeight_ > 0)
  ...
  else
    {
      ...
      while ((unitsToSkip -= getDistinctWeight(resultIx)) >= 0)
        resultIx++;
...
const NAWSetOfNodeIds * NAWSetOfNodeIds::castToNAWSetOfNodeIds() const
{
  return this;
}

NAWSetOfNodeIds * NAWSetOfNodeIds::castToNAWSetOfNodeIds()
{
  return this;
}

bool NAWSetOfNodeIds::usesTheSameNodes(const NAWNodeSet *other,
                                       NAClusterInfo *nacl) const
{
  if (getNumNodes() != other->getNumNodes())
    return false;
  ...
  if (nodeIds_.entries() != oth->nodeIds_.entries())
    return false;

  // we asserted elsewhere that the lists are sorted
  for (CollIndex i=0; i<nodeIds_.entries(); i++)
    if (nodeIds_[i] != oth->nodeIds_[i])
      return false;

  return true;
}

int NAWSetOfNodeIds::makeSizeFeasible(int proposedSize,
                                      int subsetSize,
                                      int round) const
{
  return proposedSize;
...
bool NAWSetOfNodeIds::isComplete() const
{
  return anticipatedTotalWeight_ == 0;
}

bool NAWSetOfNodeIds::usesDenseNodeIds() const
{
  return false;
}
...
const char * NAWSetOfNodeIds::isInvalid(NAClusterInfo *nacl) const
{
  if (anticipatedTotalWeight_ < 0)
    return "Specified size must be a positive value";
  // assume it is always valid, we assert if we see basic
  // mistakes, like negative node ids or weights <= 0
  return NULL;
}

bool NAWSetOfNodeIds::tryToMakeValid(NAClusterInfo *nacl,
                                     bool updateCliContext)
{
  return true;
}
...
NAWSetOfNodeIds * NAWSetOfNodeIds::deserialize(const char *serializedString,
                                               CollHeap *heap)
{
  const char *s = serializedString;
  int size = 0;
  ...
  if (isEmpty(s))
    // missing second and subsequent fields
    return NULL;
  if (!getInt(s, size) || size <= 0)
    return NULL;

  s = skipToNextField(s);
  if (isEmpty(s))
    return NULL;
  if (!getInt(s, numNodes) || numNodes <= 0)
    return NULL;
  ...
  if (result->getTotalWeight() != size ||
      result->getNumNodes() != numNodes)
          ...
          if (!individualWeights_)
            {
              ...
              for (int i=0; i<nodeIds_.entries()-1; i++)
                individualWeights_->insertAt(i, commonWeight_);
              commonWeight_ = -2;
            }
...
int NAWSetOfNodeIds::getDistinctNodeId(int ix) const
{
  return nodeIds_[ix];
}
...
bool NAWSetOfNodeIds::describeTheCluster() 
{
   ...
   if ( !nac || nac->getTotalNumberOfCPUs() != n )
     return false;
   ...
   for ( int i=0; i<n; i++ ) {
      if ( weight < 0 )
        weight = getWeightForNodeId(i);

      if ( weight != getWeightForNodeId(i) )
        return false;
   }
...
void NAWSetOfNodeIds::display() const
{
   cout << "NAWSetOfNodeIds::display():" 
        << ", anticipatedTotalWeight_=" << anticipatedTotalWeight_
        << ", cachedTotalWeight_=" << cachedTotalWeight_ 
        << ", commonWeight_=" << commonWeight_ 
        << endl;
}
