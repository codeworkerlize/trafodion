...
public:
  ...
  IpcThreadInfo(const char *nameForDisplay,
...
public:
  ...
  inline bool operator !=(const IpcSetOfConnections& other) const
                                           { return !(*this == other); }
...
public:
  ...
  IpcWaitableSetOfConnections(IpcEnvironment *env,
                              NABoolean eventDriven=FALSE,
                              NABoolean esp=FALSE);
...
private:
  ...
  void init(NABoolean eventDriven,
            NABoolean esp);
...
public:
 ...
 void openRetryCleanup();
  ...
  struct ActiveIOQueueEntry
  {
   ...
   virtual void partiallyRecvSet(IpcMessageBuffer *msg, Int32 requested, Int32 receiveed);
...
public:
  ...
  GuaConnectionToServerTimeout(  IpcEnvironment *env
                        , const IpcProcessId &procId
                        , NABoolean usesTransactions
                        , unsigned short nowaitDepth
                        , const char *eye = eye_GUA_CONNECTION_TO_SERVER
...
private:
  Int32 partiallyReceiveTimeout_;
...
public:
  ...
  IpcControlConnection(IpcNetworkDomain domain,
                       const char *eye = eye_CONTROL_CONNECTION,
...
private:
  ...
  GuaConnectionToClient *findConnection(
       short                  clientFileNumber,
       const GuaProcessHandle &clientPhandle);
  GuaConnectionToClient *findFailedConnection(
       short                  clientFileNumber,
       const GuaProcessHandle &clientPhandle);
...
public:
  ...
  IpcMessageBuffer *addChunkHeadersAndTrailer(IpcMessageObjSize chunkSize,
                                              IpcEnvironment *env);
  void removeChunkHeadersAndTrailer(IpcMessageObjSize chunkSize,
                                    IpcEnvironment *env);
  void reorderChunks(IpcMessageObjSize chunkSize,
                     IpcEnvironment *env);
  void validateChunkHeader(IpcMessageObjSize chunkStart,
                           IpcMessageObjSize chunkLen,
                           IpcMessageObjSize maxChunkSize,
                           NABoolean allowOutOfOrderChunks);
  static Int32 getChunkNumFromHeader(IpcMessageBufferPtr buf);
  static IpcMessageObjSize getLengthWithChunkHeaders(
       IpcMessageObjSize chunkSize,
       IpcMessageObjSize msgLength);
  static IpcMessageObjSize roundUpBufferLength(IpcMessageObjSize chunkSize,
                                               IpcMessageObjSize newMaxLen);
...
public:
  ...
  NABoolean getHasChunkHeaders() const
  { return (flags_  & IPCMSG_HAS_CHUNK_HDRS ? TRUE : FALSE); }
  void setHasChunkHeaders()
  { flags_ |= IPCMSG_HAS_CHUNK_HDRS; }
...
struct InternalChunkMsgTrailerStruct
{
  Int32 eyeCatcher_;
  Int32 numChunks_;
  Int32 chunkLen_; // length of each chunk
  Int32 dataLen_;  // redundant, is (numChunks_-1) * sizeof(*this)
...
public:
  ...
  void send(NABoolean waited = TRUE,
            Int64 transid = (Int64)-1,
            IpcMessageObjSize *bytesSent = NULL);
...
const Int32 InitialNowaitRequestsPerEsp = 8;
const Int32 HighLoadNowaitRequestsPerEsp = 8;
