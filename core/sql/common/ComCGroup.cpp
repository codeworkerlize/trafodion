...
ComCGroup::ComCGroup(NAMemory *heap) :
     heap_(heap),
     cgroupName_(heap),
     assignWasDone_(FALSE) {}

int ComCGroup::setOrAlterCGroup(const char *newCGroupName,
                                ComDiagsArea &diags)
{
  int result = 0;
      ...
      for (int cg=0; cg<numCGroups && !result; cg++)
        {
          ...
          if (cgroupPath != prevPath)
            {
              prevPath = cgroupPath;
              if (cgroupName_.length() > 0 &&
                  cgroupName_ != DB__SYSTEMTENANT)
                {
                  cgroupPath.append("/");
              ...
              if (fd >= 0)
                {
                  ...
                  if (close(fd) < 0)
                    result = recordAssignError(diags, cgroupPath.data(), "close", errno);
                }
              else
                {
...
NABoolean ComCGroup::isSystemTenant() const
{
  return cgroupName_ == DB__SYSTEMTENANT;
}

int ComCGroup::recordAssignError(ComDiagsArea &diags,
                                 const char *pathName,
                                 const char *assignPhase,
                                 int linuxError)
{
  int result = -8748; // the SQLCODE created
  IpcProcessId myProcId = GetCliGlobals()->getEnvironment()->
  ...
  switch (linuxError)
    ...
    default:
      if (strcmp(assignPhase, "getenv") == 0)
        snprintf(buf, sizeof(buf), "Environment variable %s is not set up",
                 pathName);
      else if (strcmp(assignPhase, "retries") == 0)
        snprintf(buf, sizeof(buf), "Retries exhausted");
      else
        snprintf(buf, sizeof(buf), "Could not %s file, errno = %d",
                 assignPhase, linuxError);
      break;
    }
