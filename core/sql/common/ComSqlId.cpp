...
short ComSqlId::packNumIntoStr(Int64 num, 
                               char * outStr, Lng32 outStrLen,
                               NABoolean backwardCompatibility)
{
  // max number of packed chars is 10. Could be increased at some point.
  int outNumChars = outStrLen - 1;
  if ((outNumChars <= 0) || (outNumChars > 10))
    return -1;
  ...
  if ((NOT useBase10) && (backwardCompatibility))
    outStr[0] |= 0x40;
...
Int64 ComSqlId::unpackNumFromStr(const char * inStr, Lng32 inStrLen,
                                 NABoolean backwardCompatibility)
{
  if ((!inStr) || (inStrLen <= 0) || (inStrLen > 10))
    return -1;
 ...
 char * stringValue) // null terminated string returned here
{
  ...
  switch (attr)
	...
	Lng32 tenantIdLenOffset =
	  USERNAMELEN_OFFSET + USERNAMELEN_LEN + userNameLen;
	Lng32 tenantIdLen = 
	  (Lng32)str_atoi(&queryId[tenantIdLenOffset], TENANTIDLEN_LEN);
	if (tenantIdLen < 0)
	  return -CLI_INVALID_ATTR_VALUE;
	if (stringValue && value > tenantIdLen )
	  {
	    strncpy(stringValue, 
		    &queryId[tenantIdLenOffset+TENANTIDLEN_LEN], 
		    tenantIdLen);
 ...
 )
{
  ...
  if (userName)
    {
      Int64  dummy = 0;
      retcode = getSqlIdAttr(SQLQUERYID_USERNAME, sessionId,
			userNameLen, dummy, userName);
      if (retcode)
	return retcode;
    }
  ...
  if(tenantId)
    {
      Int64 dummy = 0;
      retcode = getSqlIdAttr(SQLQUERYID_TENANTID, sessionId,
			tenantIdLen, dummy, tenantId);
      if (retcode)
	return retcode;
    }
  if (userSessionName)
    {
      Int64 dummy = 0;
      retcode = getSqlIdAttr(SQLQUERYID_SESSIONNAME, sessionId,
			     userSessionNameLen, dummy, userSessionName);
      if (retcode)
	return retcode;
    }
  if (version != NULL)
...
UInt64 ComSqlId::computeQueryHash(char* input_str)
{
   return ComSqlId::computeQueryHash(input_str, strlen(input_str));
}
