...
template <class T>
void NACollection<T>::insert(CollIndex posToInsert, const T &newElement, CollIndex newUsage, NABoolean *inserted) {
  // is a valid position and usage given?
  assert((posToInsert < MAX_COLL_INDEX) AND(newUsage != UNUSED_COLL_ENTRY));
  ...
  // overwrite or insert?
  if (usages_[posToInsert] == UNUSED_COLL_ENTRY) entries_++;
...
#endif
template <class T>
void NACollection<T>::allocate(CollIndex initLen) {
  ...
  if (maxLength_ > 0) {
    ...
    if (heap_ == NABasicObject::systemHeapPtr()) {
      ...
      if (!arr_) {
        ...
        usages_ = (CollIndex *)heap_->allocateMemory(sizeof(CollIndex) * ((size_t)maxLength_), failureIsFatal_);
      ...
      if (isComplete_) {
        T temp;
        for (CollIndex i = 0; i < maxLength_; i++) memcpy((void *)&arr_[i], (void *)&temp, sizeof(T));
      }
    }
  } else {
    arr_ = NULL;
...
template <class T>
void NACollection<T>::clearFrom(CollIndex entry) {
  assert(entry >= FIRST_COLL_INDEX);

  for (CollIndex i = entry; i < usedLength_; i++) usages_[i] = UNUSED_COLL_ENTRY;
...
template <class T>
T &NACollection<T>::rawEntry(CollIndex ix) {
  // resize, if index lies out of the allocated portion
  if (ix >= maxLength_) resize(ix + 1);

  // adjust used part to contain ix
  if (ix >= usedLength_) {
    for (CollIndex i = usedLength_; i <= ix; i++) usages_[i] = UNUSED_COLL_ENTRY;
    usedLength_ = ix + 1;
  }
...
NABoolean NACollection<T>::sanityCheck() {
  ...
  if (usages_ && (void *)usages_ < (void *)heap_) return FALSE;
...
template <class T>
NASubCollection<T>::NASubCollection(NACollection<T> *superset, NAMemory *heap, NABoolean failureIsFatal)
    : builtin_(TRUE),
      entries_(0),
...
template <class T>
NASubCollection<T>::NASubCollection(const NASubCollection<T> &other, NAMemory *heap)
  ...
  if (other.entries_) {
    ...
    if (i) {
      do {
        ...
        if (w != (w LOR * pOtherBits++)) return (FALSE);
      } while (--i);
    }
...
NABoolean NASubCollection<T>::lastUsed(CollIndex &lastBit) const {
  ...
  if (word(lastNonZeroWord) == 0) return FALSE;  // return; there are none

  // we know we have the last non-zero word; find last set bit

  lastBit = (lastNonZeroWord << LogBitsPerWord) + BitsPerWord - 1;
...
NABoolean NASubCollection<T>::operator==(const NASubCollection<T> &other) const {
  ...
  if (other.entries_ != entries_)
    ...
    if (other.entries_ == 0 && entries_ == 0) return (TRUE);
  }
  ...
  if (i) {
    ...
    //
    do {
      if (*pBits++ != *pOtherBits++) return (FALSE);
    } while (--i);
  }
...
NASubCollection<T> &NASubCollection<T>::addSet(const NASubCollection<T> &other) {
  ...
  if (other.entries_) {
    if (wordSize_ < maxWords) extendWordSize(maxWords);
  ...
  if (other.lastStaleBit_ > lastStaleBit_) lastStaleBit_ = other.lastStaleBit_;
...
T &NASet<T>::operator[](CollIndex i) {
  ...
  if (i >= this->entries()) ABORT("Set index exceeds # of entries");
  ...
  if (userIndexCache_ <= i) {
    ...
    while (this->getUsage(arrayIndex) == UNUSED_COLL_ENTRY AND arrayIndex < this->getSize()) arrayIndex++;
  }
...
NABoolean NASet<T>::operator==(const NASet<T> &other) const {
  ...
  if (count != other.entries()) return FALSE;

  for (CollIndex i = 0; i < count; i++) {
    if (NOT this->contains(other[i])) return FALSE;
  }
...
template <class T>
NAList<T>::NAList(CollHeap *heap, CollIndex initLen, NABoolean failureIsFatal)
    : NACollection<T>(heap, initLen, failureIsFatal), mutex_(true, false, true) {
...
template <class T>
NAList<T>::NAList(CollHeap *heap, CollIndex initLen, NABoolean failureIsFatal, NABoolean useMutex)
    : NACollection<T>(heap, initLen, failureIsFatal), mutex_(true, useMutex, true) {
  first_ = last_ = userIndexCache_ = arrayIndexCache_ = NULL_COLL_INDEX;
  invalidValuePtr_ = (failureIsFatal) ? NULL : new (heap) T;
  simualteBadIndex_ = FALSE;
}
...
// copy ctor
template <class T>
NAList<T>::NAList(const NAList<T> &other, CollHeap *heap) : NACollection<T>(other, heap), mutex_(true, false, true) {
  first_ = other.first_;
  last_ = other.last_;
...
void NAList<T>::fixupMyVTable(NAList *ptr) {
  ...
  if (ptr) memcpy((char *)ptr, (char *)&sourceA, sizeof(void *));
}
...
NABoolean NAList<T>::getLast(T &elem) {
  if (this->entries() > 0) {
    ...
    // invalidate the cache, if necessary
    if (userIndexCache_ >= this->entries()) invalidateCache();
...
NAHashBucketEntry<K, V>::~NAHashBucketEntry() {}  //  NAHashBucketEntry<K,V>::~NAHashBucketEntry()
...
void NAHashBucketEntry<K, V>::fixupMyVTable(NAHashBucketEntry *ptr) {
  ...
  if (ptr) memcpy((char *)ptr, (char *)&sourceBE, sizeof(void *));
}

template <class K, class V>
NABoolean NAHashBucketEntry<K, V>::matchEntryType(enum iteratorEntryType type) const {
  if (type == iteratorEntryType::EVERYTHING) return TRUE;

  if (isEnabled() && type == iteratorEntryType::ENABLED) return TRUE;

  if (!isEnabled() && type == iteratorEntryType::DISABLED) return TRUE;
...
NAHashBucket<K, V>::NAHashBucket(const NAHashBucket<K, V> &other, NAMemory *heap)
    : heap_((heap == NULL) ? other.heap_ : heap),
      ...
      failureIsFatal_(other.failureIsFatal_) {}  // NAHashBucket<K,V>::NAHashBucket()
...
NABoolean NAHashBucket<K, V>::contains(const K *key, bool checkAll) const {
  ...
  for (CollIndex index = 0; index < ne; index++) {
    ...
    if ((checkAll || (bep->isEnabled())) && *bep->getKey() == *key) return true;
...
Int32 NAHashBucket<K, V>::printStatistics(char *buf) {
  ...
  if (ne > 0) {
    for (CollIndex index = 0; index < ne; index++) {
      ...
      if (index > 0 AND(index / 4 * 4 == index)) c += sprintf(buf + c, "\n");
    }
  } else {
    c = sprintf(buf, "*** empty ***\n");
  }
...
V *NAHashBucket<K, V>::getFirstValue(const K *key) const {
  ...
  for (CollIndex index = 0; index < ne; index++) {
    ...
    // ----------------------------------------------------------------
    // NOTE: uses K::operator==()
    if (bep->isEnabled() && *bep->getKey() == *key) return bep->getValue();
...
V *NAHashBucket<K, V>::getFirstValueAll(const K *key) const {
  ...
  for (CollIndex index = 0; index < ne; index++) {
    ...
    // ----------------------------------------------------------------
    // NOTE: uses K::operator==()
    if (*bep->getKey() == *key) return bep->getValue();
...
// -----------------------------------------------------------------------
template <class K, class V>
void NAHashBucket<K, V>::getKeyValuePair(const K *key, const V *value, NAHashBucket<K, V> &container,
                                         NABoolean *inserted) const {
  CollIndex ne = bucket_.entries();

  NABoolean isInserted = TRUE;
  if (key) {  // a key is given
    for (CollIndex index = 0; index < ne; index++) {
      // -------------------------------------------------------------
      // Index a hash bucket entry.
      // -------------------------------------------------------------
...
K *NAHashBucket<K, V>::remove(K *key, int &entriesEnabled, NABoolean removeKV) {
  ...
  for (CollIndex index = 0; index < ne; index++) {
    ...
    {
      if (bep->isEnabled()) entriesEnabled--;

      bucket_.removeAt(index);

      // remove the key and the value if needed
      if (removeKV) bep->clear(TRUE /*deleteContents*/);
...
NABoolean NAHashBucket<K, V>::isEnable(K *key, NABoolean enforceUniqueness) {
  ...
  for (CollIndex index = 0; index < ne; index++) {
    ...
    if (*bep->getKey() == *key) {
      if (enforceUniqueness) return bep->isEnabled();
...
template <class K, class V>
NAHashDictionary<K, V>::NAHashDictionary(const NAHashDictionary<K, V> &other, NAMemory *heap)
    : heap_((heap == NULL) ? other.heap_ : heap),
      hash_(other.hash_),
      entries_(other.entries_),
  ...
  for (CollIndex index = 0; index < hashSize_; index++) (*hashTable_)[index] = (*other.hashTable_)[index];
}
...
void NAHashDictionary<K, V>::createHashTable(ULng32 hashSize) {
  ...
  for (CollIndex index = 0; index < hashSize; index++) {
    NAHashBucket<K, V> *newBucket = new (heap_, failureIsFatal_) NAHashBucket<K, V>(heap_, failureIsFatal_);
...
void NAHashDictionary<K, V>::createHashTable(ULng32 hashSize, NABoolean useMutex) {
  ...
  NABoolean inserted = TRUE;
  for (CollIndex index = 0; index < hashSize; index++) {
    NAHashBucket<K, V> *newBucket = new (heap_, failureIsFatal_) NAHashBucket<K, V>(heap_, failureIsFatal_, useMutex);
...
K *NAHashDictionary<K, V>::insert(K *key, V *value, ULng32 (*ptr)(const K &), NABoolean *inserted) {
  ...
  if (inserted) *inserted = isInserted;
...
template <class K, class V>
NAHashDictionaryIteratorNoCopy<K, V>::NAHashDictionaryIteratorNoCopy(const NAHashDictionaryIteratorNoCopy<K, V> &other,
                                                                     NAMemory *heap) {
  dict_ = other.dict_;
  type_ = other.type_;
...
void NAHashDictionaryIteratorNoCopy<K, V>::advance() {
  ...
  for (; currentBucket_ <= endBucket_; currentBucket_++) {
    ...
    // bail out.
    if (advanceInCurrentBucket(newBucket)) return;
...
NABoolean NAHashDictionaryIteratorNoCopy<K, V>::advanceInCurrentBucket(NABoolean newBucket) {
  ...
  if (current->entries() == 0) return FALSE;
...
void NAHashDictionaryIteratorNoCopy<K, V>::reset() {
  for (currentBucket_ = beginBucket_; currentBucket_ <= endBucket_; currentBucket_++) {
    ...
    if (advanceInCurrentBucket(TRUE)) return;
  }
...
NABoolean NAHashDictionary<K, V>::operator==(const NAHashDictionary<K, V> &other) const {
  ...
  for (int i = 0; i < entries(); i++) {
    if (!walkThis.getNext(keyThis, valueThis)) return FALSE;

    if (!walkOther.getNext(keyOther, valueOther)) return FALSE;

    if (keyThis != keyOther || valueThis != valueOther) return FALSE;
  }
