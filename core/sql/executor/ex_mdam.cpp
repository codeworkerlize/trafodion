...
NABoolean MdamPredIterator::positionToNextOr(MdamPred **currentPred) {
  ...
  if (p) {
    while ((p) && (p->getDisjunctNumber() < currentDisjunctNumber_) && (!(p->firstInOrGroup()))) {
      p = p->getNext();
    }
    *currentPred = p;
    if ((p) && (p->getDisjunctNumber() == currentDisjunctNumber_)) rc = TRUE;
  }
...
MdamPred *MdamPredIterator::getNextPred(MdamPred **currentPred) {
  ...
  if (rc) {
    ...
    if ((rc->getDisjunctNumber() == currentDisjunctNumber_) && (!(returnedAPred_ && rc->firstInOrGroup()))) {
...
void MdamPredIterator::positionToCurrentDisjunct(MdamPred **currentPred) {
  MdamPred *p = *currentPred;
  while ((p) && (p->getDisjunctNumber() < currentDisjunctNumber_)) {
    p = p->getNext();
  }
  *currentPred = p;
}

MdamColumn::MdamColumn(MdamColumn *previous, MdamColumnGen *columnGenInfo, ex_globals *glob, sql_buffer_pool *pool,
...
void MdamColumn::setNextColumn(MdamColumn *next) { next_ = next; }

MdamIntervalList &MdamColumn::getIntervalList() { return intervals_; }
...
MdamColumn::getNextValueReturnType MdamColumn::getNextValue(ULng32 productiveFetchRangeCounter,
  ...
  while ((traversal_in_progress_) && (!found_value)) {
    ...
    if ((current_interval_) && (!current_is_subset_interval_)) {
      ...
      if ((useSparseProbes_) || (lastProductiveFetchRangeCounter_ == productiveFetchRangeCounter)) {
        ex_assert(sparseProbeNeeded_ == FALSE, "MdamColumn::getNextvalue called at wrong time.");
        ...
        } else  // need to do a sparse probe now
        {
          ...
          endExclFlag = 0;
          if (endpt->getInclusion() == MdamEnums::MDAM_EXCLUDED) endExclFlag = 1;
    ...
    if (!found_value) {
      ...
      if (previous_) {
        ...
        while ((current_interval_ != 0) &&
               (previous_->current_context_.intersectEmpty(*(current_interval_->getRefListPtr())))) {
          current_interval_ = interval_iterator_();
        }
      }
      ...
      // else if this is a subset interval...
                   ...
                   (previous_
                        ? !stop_list_.intersectEmpty(previous_->current_context_, *(current_interval_->getRefListPtr()))
      ...
      } else  // not a subset interval
      {
        ...
        if (current_interval_->getFirstValue(len, cv)) {
          ...
          if (previous_) {
            MdamRefList &intervalRefList = *(current_interval_->getRefListPtr());

            current_context_.intersect(intervalRefList, previous_->current_context_, mdamRefListEntryHeap);
          } else {
            MdamRefList &intervalRefList = *(current_interval_->getRefListPtr());

            current_context_.deleteEntries(mdamRefListEntryHeap);
            current_context_.copyEntries(intervalRefList, mdamRefListEntryHeap);
          }
...
void MdamColumn::reportProbeResult(char *keyData) {
  ex_assert(sparseProbeNeeded_, "MdamColumn::reportProbeResult called unexpectedly.");
...
NABoolean MdamColumn::buildDisjunct(MdamPredIterator &predIterator, sql_buffer_pool *pool, atp_struct *atp0,
  ...
  MdamInterval *accumulator =
      new (mdamIntervalHeap) MdamInterval(lo_, MdamEnums::MDAM_INCLUDED, hi_, MdamEnums::MDAM_INCLUDED);
  ...
  while ((predIterator.positionToNextOr(&currentPred_)) && (!tentative_intervals_.isEmpty())) {
#if defined(NA_MDAM_EXECUTOR_DEBUG_ILTF)
    ...
    while (predPtr = predIterator.getNextPred(&currentPred_)) {
      ...
      if (predPtr->getPredType() == MdamPred::MDAM_BETWEEN) pool->get_free_tuple(pv2, columnGenInfo_->getLength());
      ...
      if (errorCode == ex_expr::EXPR_OK)
        predType = predPtr->getTransformedPredType(dcErrFlag1, dcErrFlag2, startInclusion, endInclusion);
      ...
      switch (predType) {
        case MdamPred::MDAM_EQ: {
          or_accumulator =
              new (mdamIntervalHeap) MdamInterval(pv, MdamEnums::MDAM_INCLUDED, pv, MdamEnums::MDAM_INCLUDED);
          break;
        }
        case MdamPred::MDAM_LE: {
          or_accumulator =
              new (mdamIntervalHeap) MdamInterval(nonNullLo_, MdamEnums::MDAM_INCLUDED, pv, MdamEnums::MDAM_INCLUDED);
          break;
        }
        case MdamPred::MDAM_LT: {
          or_accumulator =
              new (mdamIntervalHeap) MdamInterval(nonNullLo_, MdamEnums::MDAM_INCLUDED, pv, MdamEnums::MDAM_EXCLUDED);
          break;
        }
        case MdamPred::MDAM_GE: {
          or_accumulator =
              new (mdamIntervalHeap) MdamInterval(pv, MdamEnums::MDAM_INCLUDED, nonNullHi_, MdamEnums::MDAM_INCLUDED);
          break;
        }
        case MdamPred::MDAM_GT: {
          or_accumulator =
              new (mdamIntervalHeap) MdamInterval(pv, MdamEnums::MDAM_EXCLUDED, nonNullHi_, MdamEnums::MDAM_INCLUDED);
          break;
        }
        case MdamPred::MDAM_BETWEEN: {
          ...
          if (predPtr->reverseEndpoints())
            or_accumulator = new (mdamIntervalHeap) MdamInterval(pv2, endInclusion, pv, startInclusion);
          else
            or_accumulator = new (mdamIntervalHeap) MdamInterval(pv, startInclusion, pv2, endInclusion);
          break;
        }
        case MdamPred::MDAM_ISNULL:  // IS NULL predicate on ASC column
        {
          ...
          // is the NULL value
          or_accumulator =
              new (mdamIntervalHeap) MdamInterval(hi_, MdamEnums::MDAM_INCLUDED, hi_, MdamEnums::MDAM_INCLUDED);
          break;
        }
        case MdamPred::MDAM_ISNULL_DESC:  // IS NULL predicate on DESC column
        {
          ...
          // is the NULL value
          or_accumulator =
              new (mdamIntervalHeap) MdamInterval(lo_, MdamEnums::MDAM_INCLUDED, lo_, MdamEnums::MDAM_INCLUDED);
          break;
        }
        case MdamPred::MDAM_ISNOTNULL: {
          or_accumulator = new (mdamIntervalHeap)
              MdamInterval(nonNullLo_, MdamEnums::MDAM_INCLUDED, nonNullHi_, MdamEnums::MDAM_INCLUDED);
          break;
        }
      ...
      if (or_accumulator) {
        if (or_result.isEmpty()) {
          ...
          or_result.unionSameDisjunct(or_temp1, columnGenInfo_->getLength(), mdamIntervalHeap, mdamRefListEntryHeap);
          or_temp1.deleteAllIntervals(mdamIntervalHeap, mdamRefListEntryHeap);
        }
      }
    }

    // AND the OR result to the accumulator
    tentative_intervals_.intersect(or_result, columnGenInfo_->getLength(), mdamIntervalHeap, mdamRefListEntryHeap);
    or_result.deleteAllIntervals(mdamIntervalHeap, mdamRefListEntryHeap);
  }

  if (((got_a_predicate) && (!rc)) || ((previous_ == 0) && (!rc))) {
...
NABoolean MdamColumn::disjunctIsEmpty() { return this->tentative_intervals_.isEmpty(); }

void MdamColumn::tossDisjunct(FixedSizeHeapManager &mdamIntervalHeap, FixedSizeHeapManager &mdamRefListEntryHeap) {
...
void MdamColumn::releaseNetwork(FixedSizeHeapManager &mdamIntervalHeap, FixedSizeHeapManager &mdamRefListEntryHeap) {
  intervals_.deleteAllIntervals(mdamIntervalHeap, mdamRefListEntryHeap);
...
keyMdamEx::keyMdamEx(const keyRangeGen &keyRangeGen, const short in_version, sql_buffer_pool *pool, ex_globals *glob,
                     const ex_tcb *tcb)
    : keyRangeEx(keyRangeGen, glob, in_version, pool),
      ...
      mdamRefListEntryHeap_(sizeof(MdamRefListEntry), getGenInfo().getMaxMdamRefs()),
      mdamRefListEntrysForStopListsHeap_(sizeof(MdamRefListEntry), getGenInfo().getMaxMdamRefsForStopLists()),
      mdamIntervalHeap_(sizeof(MdamInterval), getGenInfo().getMaxMdamIntervals()),
      complementKeysBeforeReturning_(getGenInfo().complementKeysBeforeReturning()) {
  number_of_key_cols_ = 0;
  first_column_ = last_column_ = 0;

  const keyMdamGen &keyM = (const keyMdamGen &)keyRangeGen;

  for (MdamColumnGen *cg = keyM.getFirst(); cg != 0; cg = cg->getNext()) {
    last_column_ = new (glob->getSpace()) MdamColumn(last_column_, cg, glob, pool,
                                                     workAtp_,  // $$$ will this work?
                                                     workAtp_, keyM.getValueAtpIndex(), tcb);
    if (first_column_ == 0) first_column_ = last_column_;
    number_of_key_cols_++;
  }
...
void keyMdamEx::destroyNetwork() {
  if (network_built_) {
    for (MdamColumn *c = first_column_; c != 0; c = c->nextColumn()) {
      c->releaseNetwork(mdamIntervalHeap_, mdamRefListEntryHeap_);
    }
    network_built_ = FALSE;
  }
...
ExeErrorCode keyMdamEx::buildNetwork(sql_buffer_pool *pool, atp_struct *atp0) {
  ...
  ExeErrorCode refListEntryHeapErrorCode = mdamRefListEntryHeap_.acquireHeapMemory(globals_->getDefaultHeap());
  if (refListEntryHeapErrorCode != (ExeErrorCode)0) return refListEntryHeapErrorCode;

  // Acquire memory for MdamInterval's.
  ExeErrorCode intervalHeapErrorCode = mdamIntervalHeap_.acquireHeapMemory(globals_->getDefaultHeap());
  if (intervalHeapErrorCode != (ExeErrorCode)0) return intervalHeapErrorCode;
  ...
  while ((disjunct_number = predIterator.getNextDisjunctNumber()) >= 0) {
    ...
    for (MdamColumn *m = last_column_; m != 0; m = m->previousColumn()) {
      ...
      disjunct_number_in_stop_list =
          m->buildDisjunct(predIterator, pool, atp0, workAtp_, getGenInfo().getValueAtpIndex(), disjunct_number,
                           disjunct_number_in_stop_list, mdamIntervalHeap_, mdamRefListEntryHeap_,
                           mdamRefListEntrysForStopListsHeap_, dataConvErrorFlag_);

      if (m->disjunctIsEmpty()) empty_disjunct = TRUE;
    ...
    ex_assert(disjunct_number_in_stop_list, "keyMdamEx::Mdam -- disjunct number not in any stop list");
    ...
    if (empty_disjunct) {
      for (MdamColumn *c = first_column_; c != 0; c = c->nextColumn()) {
        c->tossDisjunct(mdamIntervalHeap_, mdamRefListEntryHeap_);
      }
    } else {
      for (MdamColumn *c = first_column_; c != 0; c = c->nextColumn()) {
        c->mergeDisjunct(disjunct_number, mdamIntervalHeap_, mdamRefListEntryHeap_);
      }
    }
  }  // end while
  network_built_ = TRUE;
...
keyMdamEx::~keyMdamEx() {
  ...
  MdamColumn *current;
  MdamColumn *next;
  for (current = first_column_; current != 0; current = next) {
...
ExeErrorCode keyMdamEx::initNextKeyRange(sql_buffer_pool *pool, atp_struct *atp0) {
...
keyRangeEx::getNextKeyRangeReturnType keyMdamEx::getNextKeyRange(atp_struct *, NABoolean fetchRangeHadRows,
  ...
  if (fetchRangeHadRows) productiveFetchRangeCounter_++;
  ...
  while ((current_column_) && (not_done)) {
    MdamColumn::getNextValueReturnType status = current_column_->getNextValue(
        productiveFetchRangeCounter_, bktarget, ektarget, bkExcludeFlag_, ekExcludeFlag_, mdamRefListEntryHeap_);
    switch (status) {
      case MdamColumn::TRAVERSE_DOWN: {
        current_column_ = current_column_->nextColumn();
        current_column_index_++;
        ex_assert(current_column_ != 0, "keyMdamEx::getNextKeyRange() traversed off end");
        current_column_->initNextValue();
        break;
      }
      case MdamColumn::TRAVERSE_UP: {
        ...
        ex_assert(current_column_index_ >= -1, "keyMdamEx::getNextKeyRange() column index wrong");
        break;
      }
      ...
      case MdamColumn::PROBE: {
        ...
        ex_assert(index == number_of_key_cols_, "keyMdamEx::getNextKeyRange() completeKey failure");

        not_done = FALSE;

        rc = FETCH_RANGE;
        if (status == MdamColumn::PROBE) rc = PROBE_RANGE;
...
void keyMdamEx::reportProbeResult(char *keyData) {
  ex_assert(current_column_, "keyMdamEx::reportProbeResult() called outside traverse");
