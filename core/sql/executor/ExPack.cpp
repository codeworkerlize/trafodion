...
// Constructor.
ExPackRowsTcb::ExPackRowsTcb(const ExPackRowsTdb &packTdb, const ex_tcb &childTcb, ex_globals *glob)
  ...
  pool_ = new (space) ExSimpleSQLBuffer(packTdb.noOfBuffers_, packTdb.packTuppLen_, space);
  ...
  qParent_.down = new (space) ex_queue(ex_queue::DOWN_QUEUE, packTdb.fromParent_, packTdb.givenCriDesc_, space);
  ...
  qParent_.up = new (space) ex_queue(ex_queue::UP_QUEUE, packTdb.toParent_, packTdb.returnedCriDesc_, space);
...
ExWorkProcRetcode ExPackRowsTcb::workDown() {
  ...
  while (qParent_.down->entryExists(nextRqst_) && !qChild_.down->isFull()) start();
  ...
  while (qParent_.down->getHeadIndex() != nextRqst_) {
    ...
    while (!qParent_.up->isFull() && !qChild_.up->isEmpty()) {
      ...
      if (pState.packTupp_.getDataPointer() == NULL) {
        ...
        if (pool_->getFreeTuple(pState.packTupp_)) return WORK_POOL_BLOCKED;
