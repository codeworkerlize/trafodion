...
ExExeUtilGetStatisticsTcb::moveCompilationStatsToUpQueue(CompilationStatsData *cmpStats)
{
  if( NULL != cmpStats )
  {
    ...
    snprintf(startTime, sizeof(startTime), 
                        "%04d/%02d/%02d %02d:%02d:%02d.%03u%03u",                        
		                    timestamp[0], // year
                        timestamp[1], // month
                        timestamp[2], // day
			                  timestamp[3], // hour
...
void ExExeUtilGetRTSStatisticsTcb::formatOperStatItems(SQLSTATS_ITEM operStatsItems[])
{
  ...
  operStatsItems[12].str_value    = new (getGlobals()->getDefaultHeap())
                                              char[1001];
...
void ExExeUtilGetRTSStatisticsTcb::formatOperStats(SQLSTATS_ITEM* operStatsItems)
{
  ...
  if(!isHeadingDisplayed_ )
  {
    ...
    sprintf(statsBuf_, "%5s%5s%5s%5s%5s%5s %-25s%5s%13s%19s%19s%19s%19s %s", 
      "LC","RC","Id","PaId", "ExId","Frag","TDBName","DOP", "Dispatches","OperCPUTime","EstRowsUsed", 
      "ActRowsUsed", "ActDataUsed","Details");
    moveRowToUpQueue(statsBuf_);
    moveRowToUpQueue("");
  } 
    ...
    case GET_MASTER_STATS_ENTRY_:
      {
        if (masterStatsItems_ == NULL)
        {
          ...
          masterStatsItems_[33].str_value = new (getGlobals()->getDefaultHeap())
                          char[MAX_SLA_NAME_LEN+1];
          masterStatsItems_[33].str_max_len = MAX_SLA_NAME_LEN;
          masterStatsItems_[34].str_value = new (getGlobals()->getDefaultHeap())
                          char[MAX_PROFILE_NAME_LEN+1];
    ...
    case DISPLAY_HIVE_STATS_HEADING_:
      {
        ...
        if (singleLineFormat_) {
           sprintf(statsBuf_, "%5s%10s%15s%20s%15s%20s%20s%20s%20s%20s%20s%10s",
                "Id", "DOP",
                "EstRowsAccess", "ActRowsAccess", "EstRowUsed", "ActRowsUsed", "ActRowsFiltered", "SE_IOs",
           ...
           sprintf(statsBuf_, "%5s%10s %-20s", 
                "Id", "DOP","Table Name");
	   moveRowToUpQueue(statsBuf_);
           sprintf(statsBuf_, "%15s%20s%15s%20s%20s%20s%20s%20s%20s",
		"EstRowsAccess", "ActRowsAccess", "EstRowsUsed", "ActRowsUsed", "ActRowsFiltered", "SE_IOs", "SE_IO_KBytes", "SE_IO_SumTime", "SE_IO_MaxTime");
           moveRowToUpQueue(statsBuf_);
        }
        isHeadingDisplayed_ = TRUE;
...
{
  ex_tcb * tcb = new (glob->getSpace())
    ExExeUtilGetObjectEpochStatsTcb(*this, glob);
...
ExExeUtilGetObjectEpochStatsTcb::ExExeUtilGetObjectEpochStatsTcb(
     const ComTdbExeUtilGetObjectEpochStats & exe_util_tdb,
     ex_globals * glob)
     : ExExeUtilTcb( exe_util_tdb, NULL, glob)
{
  // Allocate the private state in each entry of the down queue
        ...
        case INITIAL_:
          {
            cliRC = SQL_EXEC_GetObjectEpochStats_Internal
              (getObjectEpochStatsTdb().getObjectName(),
               getObjectEpochStatsTdb().getObjectNameLen(),
               getObjectEpochStatsTdb().getCpu(),
               getObjectEpochStatsTdb().getLocked(),
               stats_);
            if (cliRC < 0)
              {
                step_ = HANDLE_ERROR_;
        ...
        case RETURN_RESULT_:
          {
            ...
            ex_assert(epochStats != NULL,
                      "Object other than ObjectEpochStats found");
...
{
  ex_tcb * tcb = new (glob->getSpace())
    ExExeUtilGetObjectLockStatsTcb(*this, glob);
  tcb->registerSubtasks();
  return tcb;
}
...
ExExeUtilGetObjectLockStatsTcb::ExExeUtilGetObjectLockStatsTcb(
     const ComTdbExeUtilGetObjectLockStats & exe_util_tdb,
     ex_globals * glob)
     : ExExeUtilTcb( exe_util_tdb, NULL, glob)
     , distinctStats_(NULL)
{
...
ExStatisticsArea* ExExeUtilGetObjectLockStatsTcb::distinctLockStats()
{
  ...
  ExStatisticsArea *statsArea = new (heap)
    ExStatisticsArea(heap, 0,
  ...
  while (stat1 = stats_->getNext()) {
    ...
    while (stat2 = statsArea->getNext()) {
      ...
      ex_assert(lockStat1 != NULL && lockStat1 != NULL,
                "Object other than ObjectLockStats found");
    ...
    if (!found) {
      lockStat1 = stat1->castToExObjectLockStats();
      ExOperStats *newStat = new (heap) ExObjectLockStats(heap,
                                                          lockStat1->getCpu(),
                                                          lockStat1->getObjectName(),
                                                          lockStat1->getObjectType(),
                                                          lockStat1->getPid(),
                                                          lockStat1->getHolderNid(),
                                                          lockStat1->getHolderPid(),
                                                          lockStat1->getDMLLocked(),
                                                          lockStat1->getDDLLocked());
  ...
  while (true)
    {
      switch (step_)
        ...
        case INITIAL_:
          {
            cliRC = SQL_EXEC_GetObjectLockStats_Internal
              (getObjectLockStatsTdb().getObjectName(),
               getObjectLockStatsTdb().getObjectNameLen(),
               getObjectLockStatsTdb().getCpu(),
               stats_);
            if (cliRC < 0)
              {
                step_ = HANDLE_ERROR_;
        ...
        case RETURN_RESULT_:
          {
            ...
            snprintf(outBuf, sizeof(outBuf),
                     "Lock Type:                    %s",
                     lockStats->getLockString());
            moveRowToUpQueue(outBuf);
            snprintf(outBuf, sizeof(outBuf),
                     "Locked By:                    %d %d",
                     lockStats->getHolderNid(), lockStats->getHolderPid());
            moveRowToUpQueue(outBuf);
            moveRowToUpQueue(" ");
            break;
          }
