...
// Destructor.
FixedSizeHeapManager::~FixedSizeHeapManager() { releaseHeapMemory(); }
...
void *FixedSizeHeapManager::allocateElement(size_t size) {
  ...
  ex_assert(elementSpaceAvailable(), "FixedSizeHeapManager::allocateElement: heap is full.");

  // Check that the item to be allocated will fit.
  ex_assert(size <= adjustedElementSize_, "FixedSizeHeapManager::allocateElement: item too large.");
...
ExeErrorCode FixedSizeHeapManager::acquireHeapMemory(CollHeap *defaultHeapPtr) {
  ...
  while (elementsStillNeeded > 0 && elementsRequested > 0) {
    ...
    } else {  // No, we didn't get the memory block.  Try again with half as many elements.
      elementsRequested /= 2;
    };
  };
...
void FixedSizeHeapManager::releaseHeapMemory() {
  ...
  for (tempBlockPtr = firstBlockPtr_; tempBlockPtr != 0; tempBlockPtr = nextTempBlockPtr) {
...
FixedSizeHeapManagerBlock::FixedSizeHeapManagerBlock(FixedSizeHeapManager &fixedSizeHeapManagerRef,
  ...
  firstElementPtr_ = (FixedSizeHeapElement *)(((char *)this) + fixedSizeHeapManagerRef.sizeBlockRounded_);
  ...
  // Build the free list.
  char *tempPtr = 0;
  char *lastPtr = (char *)firstElementPtr_ + ((numberOfElements_ - 1) * fixedSizeHeapManagerRef.adjustedElementSize_);
  for (tempPtr = (char *)firstElementPtr_; tempPtr < lastPtr; tempPtr += fixedSizeHeapManagerRef.adjustedElementSize_) {
    new (tempPtr)
        FixedSizeHeapElement((FixedSizeHeapElement *)(tempPtr + fixedSizeHeapManagerRef.adjustedElementSize_));
  };
