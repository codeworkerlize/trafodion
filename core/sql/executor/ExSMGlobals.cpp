...
// *** SM READER THREAD.
void ExSMGlobals::addDiags(const char *functionName, int32_t rc, ExExeStmtGlobals *stmtGlob) {
  if (stmtGlob) {
...
const char *ExSMGlobals::createSessionID(ExExeStmtGlobals *stmtGlob) {
  ...
  if (rc == 0) rc = ComSqlId::getSqlQueryIdAttr(ComSqlId::SQLQUERYID_PIN, sqlQueryId, sqlQueryIdLen, masterPID, NULL);
  if (rc == 0)
    rc = ComSqlId::getSqlQueryIdAttr(ComSqlId::SQLQUERYID_EXESTARTTIME, sqlQueryId, sqlQueryIdLen, masterStart, NULL);
...
ExSMGlobals *ExSMGlobals::InitSMGlobals(ExExeStmtGlobals *stmtGlob) {
  int32_t rc = 0;
  bool isMasterExecutor = (stmtGlob->castToExMasterStmtGlobals() ? true : false);
  ...
  if (smGlobals_ == NULL) {
    ...
    EXSM_TRACE(EXSM_TRACE_INIT | EXSM_TRACE_SM_CALLS, "BEGIN %s, pid %d, tid %d", isMasterExecutor ? "MASTER" : "ESP",
               getpid(), ExSM_GetThreadID());
    EXSM_TRACE(EXSM_TRACE_INIT | EXSM_TRACE_SM_CALLS, "Seaquest node %d, SM node %d", (int)cliNodeNum,
               (int)smGlobals_->sqNodeNum_);
    ...
    EXSM_TRACE(EXSM_TRACE_MAIN_THR | EXSM_TRACE_INIT, "Creating SM reader thread...");
    ...
    EXSM_TRACE(EXSM_TRACE_MAIN_THR | EXSM_TRACE_INIT, "pthread_create returned %d", rc);

    if (rc != 0) {
      ExSMGlobals::addDiags("InitSMGlobals: Error from pthread_create", rc, stmtGlob);
      delete smGlobals_;
      smGlobals_ = NULL;
      return NULL;
    }

    EXSM_TRACE(EXSM_TRACE_MAIN_THR | EXSM_TRACE_INIT, "pthread_t %p", (void *)smGlobals_->readerThreadThreadId_);
    ...
    if (smGlobals_->traceLevel_ != effectiveTraceLevel) smGlobals_->traceLevel_ = effectiveTraceLevel;
    ...
    if (newTracePrefix == true) {
      EXSM_TRACE(EXSM_TRACE_INIT, "BEGIN %s, pid %d, tid %d", isMasterExecutor ? "MASTER" : "ESP", getpid(),
                 ExSM_GetThreadID());
    }
  ...
  while (smGlobals_->readerThreadState_ == NOT_STARTED && rc == 0) {
    rc = pthread_cond_timedwait(&smGlobals_->readerThreadStateCond_, &smGlobals_->readerThreadStateLock_, &waitTime);
    EXSM_TRACE(EXSM_TRACE_MAIN_THR | EXSM_TRACE_INIT, "pthread_cond_timedwait returned %d", rc);
...
ExSMGlobals::~ExSMGlobals() {
  ...
  if (threadSafeHeap_) delete threadSafeHeap_;
}
...
const char *ExSMGlobals::getThreadStateString(ThreadState ts) const {
  switch (ts) {
    ...
    case PROCESSING_SHUTDOWN: {
      return "PROCESSING_SHUTDOWN";
    }
    case TERMINATED_DUE_TO_ERROR: {
...
// *** FROM THE MAIN SQL THREAD. DO NOT CALL THIS FUNCTION FROM THE
// *** SM READER THREAD.
ExSMTask *ExSMGlobals::addTask(const sm_target_t &tgt, uint32_t queueSize, int32_t *scheduledAddr, NAMemory *heap,
                               ex_tcb *tcb, SMConnection *smConnection) {
  ExSMTask *result = new ExSMTask(tgt, queueSize, scheduledAddr, heap, tcb, smConnection);
  ex_assert(result, "ExSMTask allocation failure");
  smTaskList_.addTask(result);

  EXSM_TRACE(EXSM_TRACE_PROTOCOL, "TASK ADD %p, tgt %d:%d:%" PRId64 ":%d:0x%c", result, (int)tgt.node, (int)tgt.pid,
             tgt.id, (int)ExSMTag_GetTagWithoutQualifier(tgt.tag), (char)ExSMTag_GetQualifierDisplay(tgt.tag));
  EXSM_TRACE(EXSM_TRACE_PROTOCOL, "TASK  queueSize %d, scheduledAddr %p", (int)queueSize, scheduledAddr);
...
void ExSMGlobals::setReaderThreadState(ThreadState s, bool doLocking, bool doSignal) {
  int rc = 0;
  ...
  EXSM_TRACE(EXSM_TRACE_RDR_THR, "Reader thread state %s -> %s", getThreadStateString(readerThreadState_),
...
void ExSMGlobals::handleReaderThreadError(int32_t rc, const char *function, sm_handle_t dataHandle,
                                          ExSMTaskList *smTaskList) {
  if (smTaskList) smTaskList->unlock();
  ...
  if (dataHandle) rc = SM_get_done(0, dataHandle);
