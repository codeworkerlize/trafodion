...
Lng32 ExHbaseAccessTdb::execStatementTrigger(bool isBefore)
{
  ...
  if (getAccessType() == ComTdbHbaseAccess::INSERT_ ||
	   ...
	   (!isBefore && triggers->getAfterStatementTriggers() != NULL)))
	{
	  ...
	  HBaseClient_JNI::getTriggerInfo(triggerInfo,
					  type,
					  isBefore,
					  triggers,
					  TRUE
	  ...
	  if (cliRC < 0) {
	    return -HBASE_ACCESS_ERROR;
          ...
          char *sql = NULL;
          if(getExMasterStmtGlobals() != NULL &&
             getExMasterStmtGlobals()->getStatement() != NULL)
              {
  ...
  , ddlValidator_(hbaseAccessTdb.getExpectedEpoch(),hbaseAccessTdb.getExpectedFlags())
{
  ...
  if (partQualPreCondExpr())
    partQualPreCondExpr()->fixup(0, getExpressionMode(), this,  space, heap, FALSE, glob);
  if (insDelPreCondExpr())
    insDelPreCondExpr()->fixup(0, getExpressionMode(), this,  space, heap, FALSE, glob);
  if (scanPreCondExpr())
    scanPreCondExpr()->fixup(0, getExpressionMode(), this,  space, heap, FALSE, glob);
...
short ExHbaseAccessTcb::setupError(Lng32 retcode, const char * str, const char * str2)
{
  ...
  if ((ABS(retcode) >= HBASE_MIN_ERROR_NUM) &&
      (ABS(retcode) <= HBASE_MAX_ERROR_NUM) ||
      ABS(retcode) == TRIGGER_EXECUTE_EXCEPTION ||
      ABS(retcode) == TRIGGER_PARAMETER_EXCEPTION)
    {
      ex_queue_entry *pentry_down = qparent_.down->getHeadEntry();
      ...
      if (g)
        {
          ...
          if (g->getStatement()->getQueryType() > ComTdbRoot::SQL_SELECT_NON_UNIQUE  &&
              ...
              ComMemLog::instance().memlog("%s, RowsAffected: %ld, %ld",
                 g->getStatement()->getUniqueStmtId(), g->getRowsAffected(), rowsAffected);
          }
        }
      else
        {
  ...
  if (hbaseAccessTdb().hbaseRowidTuppIndex_ > 0)
    {
      ...
      rowID_ = rowID;
      workAtp_->getTupp(hbaseAccessTdb().hbaseRowidTuppIndex_)
          .setDataPointer(rowID_.val);
    }
  ...
  for (int colNo= 0; colNo < numCells; colNo++)
  {
      ...
      if (timestamp == latestTimestampForCols_[idx]) 
      {
         ...
         retcode = ehi_->getColVal(colNo, colVal, colValLen, 
                                   (NOT hbmTab ? attr->getNullFlag() : FALSE), 
                                   nullVal, tag, tagLen);
         if (retcode != HBASE_ACCESS_SUCCESS)
         {
         ...
         if (!nullVal && hbmTab && hbaseAccessTdb().treatEmptyAsNull())
           {
             ...
             if ((convAttr) &&
                 (convertExpr()) &&
                 (colValLen == 0) &&
                 (NOT DFS2REC::isAnyCharacter(convAttr->getDatatype())))
               nullVal = 1;
           }
  ...
  if (loadDataIntoMemoryTable_)
  {
    ...
    if (!ehi_->memoryTableInsert(rowID_, val)) {
      ...
      snprintf(errBuf,sizeof(errBuf),
               "%s will not be cached beacause Shared Cache has reached more than 90 percent, The table may be too large or not have enough cache space, Check the free space or Remove the READ_ONLY attributes from table",
               hbaseAccessTdb().getTableName());
      ...
      if (doEvalClauses)
        {
          evalRetCode = 
            convertExpr()->evalClauses(convertExpr()->getClauses(),
                                       pentry_down->getAtp(), workAtp_, NULL,
                                       asciiRowLen, &rowLen,
                                       NULL, NULL);
        }
      else
...
short ExHbaseAccessTcb::evalPartQualPreCondExpr()
{
  ...
  exprRetCode =
    partQualPreCondExpr()->eval(pentry_down->getAtp(), workAtp_);
...
short ExHbaseAccessTcb::evalScanPreCondExpr()
{
  ...
  exprRetCode =
    scanPreCondExpr()->eval(pentry_down->getAtp(), workAtp_);
...
Lng32 ExHbaseAccessTcb::createSQRowDirectFromMemoryTable()
{
  ...
  retcode = ehi_->getColVal(0, colVal, asciiRowLen, FALSE,
                            nullVal, tag, tagLen);
  if (retcode != HBASE_ACCESS_SUCCESS)
    return retcode;
  ...
  if (tcb->hbaseAccessTdb().validateDDL())
    {
      ...
      ObjectEpochCache *objectEpochCache = statsGlobals->getObjectEpochCache();
      ObjectEpochCacheEntryName oecName(NULL,  // TODO: get right heap, e.g. tcb_->getHeap() (system heap OK for the moment)
                                        tcb->hbaseAccessTdb().getBaseTableName(),
                                        COM_BASE_TABLE_OBJECT,
                                        0);
      ObjectEpochCacheEntry *oecEntry = NULL;
      ObjectEpochCacheKey key(tcb->hbaseAccessTdb().getBaseTableName(), 
        oecName.getHash(),COM_BASE_TABLE_OBJECT);
    ...
    switch (step_)
    {
      ...
      case GET_NAME:
      {
        ...
        if(tmpSampleLc)
        {
          ...
          hBulkLoadSamplePrepPath_ = std::string(tmpSampleLc)
                                     + tmpSampleName;
        ...
        if (sampletable_.len >0)
          {
            ...
            retcode = ehi_->doBulkLoad(sampletable_,
                                       hBulkLoadSamplePrepPath_,
                                       sampeTabName,
                                       hbaseAccessTdb().getUseQuasiSecure(),
                                       FALSE);
          }
