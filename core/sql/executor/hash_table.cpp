  ...
  while (row != NULL) {
	...
	chainIdx = ++chainIdx % headerCount_; // next HT entry
        ...
        snprintf(msg, sizeof(msg),
                 "Encountered two rows with different values but same hash values %d",
                 newRowHV);
	...
	if ( (chainIdx + 1) % headerCount_ == origChainIdx ) {
	  ...
	  hashTableOverflow_ = TRUE; // remember that the HT still contains
          ...
          char msg[256];
          snprintf(msg, sizeof(msg),
                   "Hash table of size %d is full, allow duplicates",
                   headerCount_);
          // log an EMS event and continue
    ...
    if ((++numRowsScanned % logTrigger) == 0)
      {
        char msg[256];
        snprintf(msg, sizeof(msg),
                 "Traversed %ld chains in hash table insert %d, hash value %d",
                 numRowsScanned,
                 origChainIdx,
                 newRowHV);
        // log an EMS event and continue
      ...
      if (numRowsTraversed >= logTrigger)
        {
          char msg[256];
          snprintf(msg, sizeof(msg),
                   "Found %d matching rows after traversing %ld rows in chain %d for hash value %d",
  ...
  if (numRowsTraversed >= logTrigger)
    {
      char msg[256];
      snprintf(msg, sizeof(msg),
               "Did not find any matching rows after traversing %ld rows in chain %d for hash value %d",
