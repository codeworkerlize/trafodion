...
ExControlArea::ExControlArea(ContextCli *context, CollHeap *heap) : context_(context), heap_(heap) {
...
void ExControlArea::addControl(ControlQueryType type, int reset, const char *sqlText, int lenX, const char *value1,
  ...
  if (reset == -1) {
    ...
    if (type == DEFAULT_ && sysDefResetQueueEntry_ != NULL) {
      ...
      q->position();
    while ((e = (ExControlEntry *)q->getNext()) != NULL) {
      if (e->getActionType() != ComTdbControl::HOLD_ && e->match(type, value1, value2)) {
        if (!e->isNonResettable()) {
          q->remove(NULL);
          NADELETE(e, ExControlEntry, heap_);
        }
        ...
        //   CQT tbl attr RESET;
        if (len1)
          if (type != TABLE_ || len2) break;
      }
    }
  } else if (actionType == ComTdbControl::RESTORE_) {
  ...
  } else if (actionType == ComTdbControl::HOLD_)
    addToList = TRUE;
  else {
    addToList = TRUE;
    ...
    if (type == DEFAULT_ && sysDefResetQueueEntry_ != NULL && resetAllQueueEntry_ == NULL) {
      q->position(sysDefResetQueueEntry_);
      q->getNext();
    } else
      q->position();
  ...
  if (addToList) {
    ...
    e = new (heap_) ExControlEntry(heap_, type, reset, sX, lenX, SQLCHARSETCODE_UTF8, v1, len1, v2, len2, v3, len3,
