     ...
     displayInGui_(FALSE)
{
  ...
  if (asNodes)
    availableNodes_ = NAWNodeSet::deserialize(asNodes->getSerializedNodes(),
                                              glob->getDefaultHeap());
  else
  ...
  if (totalESPLimit >= 0)
    {
      ...
      if (glob_->getContext()->getAvailableNodes())
        numNodesToUse =
          glob_->getContext()->getAvailableNodes()->getTotalWeight();
      ...
      if (numNodesToUse <= 0)
        ...
        numNodesToUse = glob_->getCliGlobals()->
          getNAClusterInfo()->getTotalNumberOfCPUs();
      ...
      if (!espManager->checkESPLimitPerNodeEst(fragDir_,
                                               totalESPLimit,
                                               numESPsNeeded))
        {
          reportESPLimitViolation(totalESPLimit,
                                  numNodesToUse,
                                  numESPsNeeded,
                                  TRUE);
          state_ = ERROR;
      ...
      if (fragDir_->getType(i) == ExFragDir::ESP)
	{
      ...
      if (checkDuplicateEsps)
      {
        if (nodeMap->isNodeNumDuplicate(GetCliGlobals()->getNAClusterInfo()->getTotalNumberOfCPUs()))
        {
          ...
          *diags << DgSqlCode(-1239)
                 << DgString0(output.data());
          state_ = ERROR;
          return;
        }
      }
	  ...
	  Lng32 espLevel = fragDir_->getEspLevel(i);
          ...
          for (Lng32 e = 0; e < fragEntry.numEsps_ && state_ != ERROR; e++)
	    {
	      ...
	      assignEspArrays.instance_[e]->releasingWorkMsg_      = FALSE;
              ...
              {
                if (nac)
                  state = nac->getNodeStatus(cpuNum);
                if (state == MS_Mon_State_Down || state == MS_Mon_State_Stopped ||
                    state == MS_Mon_State_Shutdown)
                {
                  assignEspArrays.instance_[e]->needToWork_ = FALSE;
                  QRLogger::log(CAT_SQL_EXE, LL_WARN, "Esp on Node %d need not to work due to node state = %d",
                                cpuNum, state);
                }
              }
              else
          ...
          for (Lng32 e = 0; e < fragEntry.numEsps_; e++)
          {
                ...
                assignEspArrays.instance_[e]->usedEsp_)
            {
              ...
              else if (checkDuplicateEsps)
              {
                ...
                {
                  if (nac)
                    state = nac->getNodeStatus(expected_node);
                  if (state != MS_Mon_State_Down && state != MS_Mon_State_Stopped &&
                      state != MS_Mon_State_Shutdown)
                  {
                    if (!diags)
                    ...
                    *diags << DgSqlCode(-2036)
                           << DgString0(ipcs->getProgFileName());
                    ipcs->getControlConnection()->getOtherEnd().addProcIdToDiagsArea(*diags, 1);
                    *diags << DgInt0(expected_node)
                           << DgInt1(actual_node);
                    state_ = ERROR;
                  }
                  QRLogger::log(CAT_SQL_EXE, LL_ERROR, "Esp could not be created on Node %d "
        ...
        while (numLoadFixupMsgesOut_ > 0)
          {
                ...
                if (getState() == ERROR)
                  abortFixup = true;
                timedOut = FALSE;
                glob_->getIpcEnvironment()->getAllConnections()->waitOnAll(30000, FALSE, &timedOut); //wait for up to 5 minutes 
...
void ExRtFragTable::releaseTransaction(NABoolean allWorkRequests,
				       NABoolean alwaysSendReleaseMsg,
                                       NABoolean commitSavepoint,
                                       NABoolean rollbackSavepoint,
                                       Int64 savepointId)
{
#ifdef IPC_INTEGRITY_CHECKING
  checkIntegrity();
          ...
          if ( fragEntry->assignedEsps_.used( inst ) )
            {
              ...
              // release the ESP, if one is assigned
              if ( fragInst->usedEsp_ )
                glob_->getEspManager()->releaseEsp(fragInst->usedEsp_,
                                                   glob_->verifyESP(),
                                                   fragInst->usedEsp_->inUse());
              fragInst->deleteMe( );
              fragEntry->assignedEsps_.remove( inst );
            } // fragment instance is present
        } // for each ESP assigned to this fragment of type ESP
...
void ExRtFragTable::reportESPLimitViolation(Int32 totalESPLimit,
                                            Int32 numOfNodes,
                                            Int32 numESPsNeeded,
                                            NABoolean singleStmtExceededLimit)
{
  ...
  Int32 limitPerNode =
    (numOfNodes > 0 ? (totalESPLimit / numOfNodes) : totalESPLimit);
  ...
  (*diags) << DgSqlCode(sqlcode)
           << DgInt0(numESPsNeeded)
           << DgInt1(totalESPLimit/numOfNodes)
           << DgInt2(numOfNodes);
}
...
ExEspManager::ExEspManager(IpcEnvironment *env, ContextCli *context)
  : env_(env), context_(context)
{
     ...
     bool esp_multi_threaded)
{
  ...
  NAWNodeSetWrapper nodeSet(context_->getAvailableNodes(),
                            GetCliGlobals()->getNAClusterInfo());
  ...
  if (*creatingEsp == NULL) // Nowaited Creation of an ESP is not in progress
  {
    ...
    if ( cpuNum == IPC_CPU_DONT_CARE )
      cpuNum = getRoundRobinCPU(availableNodes);

    // look up the cache for esp to share
    NABoolean espServerError = FALSE;
    result = getEspFromCache(alreadyAssignedEsps, statementHeap, statement, clusterName, cpuNum, memoryQuota, user_id, tenantId, verifyESP, espLevel, idleTimeout, assignTimeWindow, nowaitDepth, espServerError, soloFragment, esp_multi_fragment, esp_num_fragments, esp_multi_threaded);
   ...
   if (ptrToClusterName == NULL)
    {
      ...
      NABoolean espServerError = FALSE;
      result = getEspFromCache(alreadyAssignedEsps, statementHeap, statement, ptrToClusterName, cpuNum, memoryQuota, user_id, tenantId, verifyESP, espLevel, idleTimeout, assignTimeWindow, nowaitDepth, espServerError, soloFragment, esp_multi_fragment, esp_num_fragments, esp_multi_threaded);
      if (espServerError == TRUE)
        // found error from ESP already assigned to prev segment
        {
	  if (diags) {
  ...
  result = new(env_->getHeap()) ExEspDbEntry(
      ...
      user_id,
      tenantId,
      espServerClass_->isMultiThreaded());
        ...
        if (e->inUse_)
          {
            ...
            SQLMXLoggingArea::logExecRtInfo(
                 __FILE__, __LINE__,
                 "Statements from multiple tenants in an executor context", 
                 0);
            continue;
          }
        else
...
IpcCpuNum ExEspManager::getRoundRobinCPU(const NAWNodeSet *availableNodes)
{
  ...
  if (usesDenseNumbers)
    ...
    result = cliGlobals->getNAClusterInfo()->
      mapLogicalToPhysicalNodeId(logCPUNum);
  else
    result = logCPUNum;
  ...
  for (CollIndex i = 0; i < iter.entries(); i++)
    {
	  ...
	  entry = espList->at(j);
          ...
          Int64 currentTimestamp = NA_JulianTimestamp();
          if (ignoreTimeout ||
              (currentTimestamp - entry->idleTimestamp_ >
...
NABoolean ExEspManager::checkESPLimitPerNodeEst(ExFragDir *fragDir,
                                                Int32 totalESPLimit,
                                                Int32 &numESPsNeeded)
{
  numESPsNeeded = 0;

  for (int f=0; f<fragDir->getNumEntries(); f++)
    numESPsNeeded += fragDir->getNumESPs(f);
  ...
  if (fragDir->espNumFragments() > 1)
    numESPsNeeded /= fragDir->espNumFragments();
