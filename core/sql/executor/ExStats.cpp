...
Lng32 ExOperStats::getStatsItem(SQLSTATS_ITEM* sqlStats_item)
{
  ...
  case SQLSTATS_ACT_DATA_USED:
    sqlStats_item->int64_value = 
        getActualRowsReturned() * getRecordLength();
    break;
...
Lng32 ExFragRootOperStats::getStatsItem(SQLSTATS_ITEM* sqlStats_item)
{
  ...
  Lng32 retcode = 0;
    ...
    case SQLSTATS_DETAIL:
      if (sqlStats_item->str_value != NULL)
      {
         snprintf(sqlStats_item->str_value, sqlStats_item->str_max_len, "%ld|%ld|%ld|%ld|",
...
void 
ExStorageEngineStats::addScanFilterStats(ScanFilterStats& stats,
       ...
       NABoolean appendAlways)
{
  ...
  while ((stat = other->getNext()) != NULL)
  {
    ...
    case SQLCLI_OBJECT_EPOCH_STATS:
      ...
      {
        ExObjectEpochStats *oeStats =
          new (getHeap()) ExObjectEpochStats((NAHeap *)getHeap());
    ...
    case SQLCLI_OBJECT_LOCK_STATS:
      ...
      {
        ExObjectLockStats *oeStats =
          new (getHeap()) ExObjectLockStats((NAHeap *)getHeap());
        oeStats->setCollectStatsType(getCollectStatsType());
...
Lng32 ExMasterStats::getStatsItem(SQLSTATS_ITEM* sqlStats_item)
{
  ...
  case SQLSTATS_PROFILE_NAME:
    ...
    if (sqlStats_item->statsItem_id == SQLSTATS_PARENT_QUERY_SYSTEM)
       tempName = (parentQid_ != NULL ? (parentQidSystem_[0] != '\0' ? 
                   parentQidSystem_ : "SAME") : "NONE");
...
void ExBMOStats::merge(ExBMOStats* other)
{
  ...
  if (other->phase_ > phase_)
     phase_ = other->phase_;
  if (other->estMemoryUsage_ > 0)
     estMemoryUsage_ = other->estMemoryUsage_; 
  scratchOverflowMode_ = other->scratchOverflowMode_;
  scratchFileCount_ += other->scratchFileCount_;
...
Lng32 ExBMOStats::getStatsItem(SQLSTATS_ITEM* sqlStats_item)
{
  ...
  case SQLSTATS_DETAIL:
   if (sqlStats_item->str_value != NULL)
   {
      snprintf(sqlStats_item->str_value, sqlStats_item->str_max_len, "%ld|%ld|%ld|", getNumVal(2), getNumVal(3), getNumVal(4)); 
      len = strlen(sqlStats_item->str_value);
...
: ex_tcb(qryInvalidTdb, 1, glob)
{
  ...
  pool_ = new(space) sql_buffer_pool(qryInvalidTdb.numBuffers_,
				     (Lng32)qryInvalidTdb.bufferSize_,
				     space);

  // Allocate the queues used to communicate with parent
  qparent_.down = new(space) ex_queue(ex_queue::DOWN_QUEUE,
				      qryInvalidTdb.queueSizeDown_,
				      qryInvalidTdb.criDescDown_,
				      space);
  ...
  qparent_.up = new(space) ex_queue(ex_queue::UP_QUEUE,
				    qryInvalidTdb.queueSizeUp_,
				    qryInvalidTdb.criDescUp_,
				    space);
  ...
  if (qryInvalidTdb.scanExpr_)
    (void) qryInvalidTdb.scanExpr_->fixup(0, getExpressionMode(), this,
		       		     space, heap, FALSE, glob);

  if (qryInvalidTdb.inputExpr_)
    (void) qryInvalidTdb.inputExpr_->fixup(0, getExpressionMode(), this,
				     space, heap, FALSE, glob);
 
  if (qryInvalidTdb.projExpr_)
    (void) qryInvalidTdb.projExpr_->fixup(0, getExpressionMode(), this,
				     space, heap, FALSE, glob);
...
short ExQryInvalidStatsTcb::work()
{
  ...
  ex_queue_entry * pentry_down = qparent_.down->getHeadEntry();
  ExQIStatsPrivateState * pstate = 
    (ExQIStatsPrivateState*) pentry_down->pstate;
  NABoolean found = FALSE;
  char * stmtName = NULL;
  ExStatisticsArea *stats = NULL;
  CliGlobals *cliGlobals = getGlobals()->castToExExeStmtGlobals()->
  ...
  while (1)
  {
    switch (pstate->step_)
    {
      case INITIAL_:
      {
        ...
        {
          ex_expr::exp_return_type evalRetCode;
          evalRetCode = statsTdb().getInputExpr()->eval(pentry_down->getAtp(),
                                                        workAtp_);
          if (evalRetCode == ex_expr::EXPR_ERROR)
          {
     ...
     case GET_LOCAL_QI_STATS_ENTRY_:
     {
      ...
      case APPLY_SCAN_EXPR_:
      {
        ...
        {
          ex_expr::exp_return_type evalRetCode;
          evalRetCode = statsTdb().scanExpr_->eval(pentry_down->getAtp(), 
                                                   workAtp_);
          if (evalRetCode == ex_expr::EXPR_FALSE)
            rowSelected = FALSE;
          else if (evalRetCode == ex_expr::EXPR_ERROR)
      ...
      case PROJECT_:
      {
        ...
        pstate->matchCount_++;
        str_cpy_all(pentry_up->getTupp(statsTdb().criDescUp_->noTuples()-1).getDataPointer(),
                    data_, statsTdb().tupleLen_);
        pentry_up->upState.downIndex = qparent_.down->getHeadIndex();
        pentry_up->upState.parentIndex = 
        pentry_down->downState.parentIndex;
...
void ExQryInvalidStatsTcb::getColumnValues(ExOperStats *operStat)
{
  ...
  for (UInt32 i = 0; i < tDesc->numAttrs(); i++)
  {
    ...
    if (::convDoIt(src, srcLen, srcType, 0, 0,
                   &data_[attr->getOffset()], 
                   attr->getLength(),
                   attr->getDatatype(),0,0,
                   0, 0, NULL) != ex_expr::EXPR_OK)
