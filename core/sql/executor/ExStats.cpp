...
void ExStatsBase::alignBufferForNextObj(const char *&buffer) { buffer = (const char *)ROUND8((Long)buffer); }
...
//////////////////////////////////////////////////////////////////
ExStatsCounter::ExStatsCounter() { init(); }
...
void ExStatsCounter::merge(ExStatsCounter *other) {
  ...
  if (min_ > other->min()) min_ = other->min();

  if (max_ < other->max()) max_ = other->max();
...
float ExStatsCounter::mean() {
  float result = 0.0;
  if (entryCnt_) result = ((float)sum_) / (float)entryCnt_;
  return result;
}
...
float ExStatsCounter::variance() {
  float result = 0.0;
  if (entryCnt_ > 1)
    result = (1.0F / (float)(entryCnt_ - 1)) * (sum2_ - (1.0F / (float)entryCnt_) * (float)sum_ * (float)sum_);
  return result;
}
...
void ExClusterStats::getVariableStatsInfo(char *dataBuffer, char *dataLen, int maxLen) {
  ...
  sprintf(buf, "NumBuckets: %u ActRows: %ld NumChains: %u MaxChain: %ld VarChain: %f ", bucketCnt_, actRows_,
          hashChains_.entryCnt(), hashChains_.max(), hashChains_.variance());
  buf += str_len(buf);
...
void ExClusterStats::getVariableStatsInfoAggr(char *dataBuffer, char *dataLen, int maxLen) {
  ...
  while (tempNext) {
    ...
    if (tempNext->writeIOCnt_ == 0) residentClusters++;
...
ExTimeStats::ExTimeStats(clockid_t clk_id) : version_(StatsCurrVersion), clockid_(clk_id) { reset(); }
...
long ExTimeStats::stop() {
  ...
  if (isStarted_) {
    ...
    incTime =
        ((endTime.tv_sec - startTime_.tv_sec) * 1000LL * 1000LL * 1000LL) + (endTime.tv_nsec - startTime_.tv_nsec);

    incTime /= 1000LL;
    if (incTime < 0) incTime = 0;
    sumTime_ += incTime;
    isStarted_ = FALSE;
  }
  return incTime;
...
UInt32 ExeSEStats::pack(char *buffer) {
  ...
  size += packIntoBuffer(buffer, filteredRows_);
...
void ExeSEStats::merge(ExeSEStats *other) {
  ...
  incFilteredRows(other->filteredRows_);
}
...
void ExeSEStats::unpack(const char *&buffer) {
  ...
  unpackBuffer(buffer, filteredRows_);
}
...
// class ExOperStats
//////////////////////////////////////////////////////////////////
ExOperStats::ExOperStats(NAMemory *heap, StatType statType, ex_tcb *tcb, const ComTdb *tdb)
  ...
  if (!tdb) {
    ...
    if (glob->castToExEidStmtGlobals()) setStatsInDp2(TRUE);
    setStatsInTcb(TRUE);
    ...
    recordLength_ = ((ComTdb *)tdb)->getRecordLength();
  }
  sprintf(processNameString_, "%03d,%05d", GetCliGlobals()->myCpu(), GetCliGlobals()->myPin());
...
void ExOperStats::subTaskReturn(ExWorkProcRetcode rc) {
  ...
  if ((rc > 0) || rc < -STATS_WORK_LAST_RETCODE) rc = -STATS_WORK_LAST_RETCODE - 1;
...
void ExOperStats::merge(ExOperStats *other) {
  if (version_ >= _STATS_RTS_VERSION_R22) {
    if (other->version_ >= _STATS_RTS_VERSION_R22) {
      if (other->u.est.estRowsAccessed_ > 0) u.est.estRowsAccessed_ = other->u.est.estRowsAccessed_;
      if (other->u.est.estRowsUsed_ > 0) u.est.estRowsUsed_ = other->u.est.estRowsUsed_;
    } else {
      if (other->u.estRowsReturned_ > 0) u.est.estRowsUsed_ = (float)other->u.estRowsReturned_;
    }
  } else
  ...
  if (other->hasSentMsgIUD()) setHasSentMsgIUD();
...
ExMeasStats *ExOperStats::castToExMeasStats() { return NULL; }

ExMeasBaseStats *ExOperStats::castToExMeasBaseStats() { return NULL; }

ExFragRootOperStats *ExOperStats::castToExFragRootOperStats() { return NULL; }

ExHdfsScanStats *ExOperStats::castToExHdfsScanStats() { return NULL; }

ExHbaseAccessStats *ExOperStats::castToExHbaseAccessStats() { return NULL; }

ExPartitionAccessStats *ExOperStats::castToExPartitionAccessStats() { return NULL; }

ExProbeCacheStats *ExOperStats::castToExProbeCacheStats() { return NULL; }

ExFastExtractStats *ExOperStats::castToExFastExtractStats() { return NULL; }

ExHashGroupByStats *ExOperStats::castToExHashGroupByStats() { return NULL; }

ExUDRStats *ExOperStats::castToExUDRStats() { return NULL; }

ExHashJoinStats *ExOperStats::castToExHashJoinStats() { return NULL; }

ExESPStats *ExOperStats::castToExESPStats() { return NULL; }
ExSplitTopStats *ExOperStats::castToExSplitTopStats() { return NULL; }

ExSortStats *ExOperStats::castToExSortStats() { return NULL; }

ExMasterStats *ExOperStats::castToExMasterStats() { return NULL; }
...
void ExOperStats::getVariableStatsInfo(char *dataBuffer, char *dataLen, int maxLen) {
  ...
  buf += str_len(buf);
  if (collectStatsType_ == ComTdb::OPERATOR_STATS || collectStatsType_ == ComTdb::ALL_STATS ||
...
int ExOperStats::getStatsItem(SQLSTATS_ITEM *sqlStats_item) {
  ...
  switch (sqlStats_item->statsItem_id) {
    ...
    case SQLSTATS_ACT_DATA_USED:
      sqlStats_item->int64_value = getActualRowsReturned() * getRecordLength();
      break;
...
NABoolean ExOperStats::operator==(ExOperStats *other) {
  if (statType() == ExOperStats::MEAS_STATS)
    return TRUE;
  else if (statType() == ExOperStats::BMO_STATS || statType() == ExOperStats::UDR_BASE_STATS ||
           statType() == ExOperStats::REPLICATOR_STATS || statType() == ExOperStats::REORG_STATS ||
           statType() == ExOperStats::HBASE_ACCESS_STATS || statType() == ExOperStats::HDFSSCAN_STATS)
  ...
  switch (tempStatsMergeType) {
    ...
    default:
      if (statType() == ExOperStats::MEAS_STATS)
        ...
        return (getId()->fragId_ + getId()->tdbId_ + getId()->instNum_ + getId()->subInstNum_);
  }
}
...
ExFragRootOperStats::ExFragRootOperStats(NAMemory *heap, ex_tcb *tcb, const ComTdb *tdb)
    : ExOperStats(heap, ROOT_OPER_STATS, tcb, tdb), flags_(0) {
  executionCount_ = 0;
  init(FALSE);
  initHistory();
  queryId_ = NULL;
  ...
  if (tdb && tcb && tcb->getGlobals()) {
    ...
    if ((glob && glob->castToExMasterStmtGlobals()) && (tdb->getNodeType() == ComTdb::ex_ROOT)) {
...
ExFragRootOperStats::ExFragRootOperStats(NAMemory *heap) : ExOperStats(heap, ROOT_OPER_STATS), flags_(0) {
  executionCount_ = 0;
  init(FALSE);
  initHistory();
  queryId_ = NULL;
...
ExFragRootOperStats::~ExFragRootOperStats() {
  ...
  if ((Int32)getCollectStatsType() == SQLCLI_CPU_OFFENDER_STATS && queryId_ != NULL) {
    ...
    if (processStats != NULL) processStats->setRecentQidToNull(queryId_);
  }
}
...
UInt32 ExFragRootOperStats::pack(char *buffer) {
  ...
  if (statsInDp2()) {
    ...
    packedLen += scanFilterStats_.pack(buffer);
  } else {
...
void ExFragRootOperStats::merge(ExFragRootOperStats *other) {
  ...
  if (scratchOverflowMode_ == -1) scratchOverflowMode_ = other->scratchOverflowMode_;
  ...
  if (other->maxWaitTime_ > maxWaitTime_) maxWaitTime_ = other->maxWaitTime_;
  flags_ |= other->flags_;

  scanFilterStats_.merge(other->scanFilterStats_, ScanFilterStats::MAX);
}
...
void ExFragRootOperStats::copyContents(ExFragRootOperStats *stat) {
  ...
  scanFilterStats_ = stat->scanFilterStats_;
}
...
void ExFragRootOperStats::getVariableStatsInfo(char *dataBuffer, char *dataLen, int maxLen) {
  ...
  if ((Int32)getCollectStatsType() == SQLCLI_CPU_OFFENDER_STATS) {
    sprintf(buf,
            ...
            statType(), (((txtVal = getTextVal()) != NULL) ? txtVal : "NULL"), ((queryId_ != NULL) ? queryId_ : "NULL"),
            cpuTime_, (UInt32)spaceUsage_, (UInt32)spaceAlloc_, (UInt32)heapUsage_, (UInt32)heapAlloc_,
            pagesInUse_ * 16, diffCpuTime_);
  } else {
    ...
    sprintf(buf,
            ...
            cpuTime_, (((txtVal = getTextVal()) != NULL) ? txtVal : "NULL"), stmtIndex_, timestamp_,
            (UInt32)spaceUsage_, (UInt32)spaceAlloc_, (UInt32)heapUsage_, (UInt32)heapAlloc_, (UInt32)heapWM_,
            newprocess_, newprocessTime_, reqMsgCnt_, reqMsgBytes_, replyMsgCnt_, replyMsgBytes_, pagesInUse_ * 16,
            scratchOverflowMode_, topN_, scratchFileCount_, spaceBufferSize_, spaceBufferCount_, scratchIOSize_,
            scratchWriteCount_, scratchReadCount_, scratchIOMaxTime_, interimRowCount_, udrCpuTime_, maxWaitTime_,
            getAvgWaitTime(), hdfsAccess() ? 1 : 0);

    buf += str_len(buf);

    scanFilterStats_.getVariableStatsInfo(buf, maxLen - (buf - dataBuffer), " filters: ");
  }
  buf += str_len(buf);
...
int ExFragRootOperStats::getStatsItem(SQLSTATS_ITEM *sqlStats_item) {
  ...
  switch (sqlStats_item->statsItem_id) {
    ...
    case SQLSTATS_DETAIL:
      if (sqlStats_item->str_value != NULL) {
        snprintf(sqlStats_item->str_value, sqlStats_item->str_max_len, "%ld|%ld|%ld|%ld|", cpuTime_ + espCpuTime_,
                 getNumVal(2), getNumVal(3), getNumVal(4));
        len = strlen(sqlStats_item->str_value);

        tmpBuf = sqlStats_item->str_value + len;
        scanFilterStats_.getVariableStatsInfo(tmpBuf, sqlStats_item->str_max_len - len);
        len = strlen(sqlStats_item->str_value);
...
NABoolean ExFragRootOperStats::filterForCpuStats() {
  ...
  if (histCpuTime_ == 0)
    retcode = FALSE;
  else if ((diffCpuTime_ = cpuTime_ - histCpuTime_) > 0)
    retcode = TRUE;
  else
    retcode = FALSE;
...
ExStorageEngineStats::ExStorageEngineStats(NAMemory *heap, ex_tcb *tcb, ComTdb *tdb)
    : ExOperStats(heap, HDFSSCAN_STATS, tcb, tdb), timer_(CLOCK_MONOTONIC) {
  const char *name;
...
ExStorageEngineStats::ExStorageEngineStats(NAMemory *heap)
    : ExOperStats(heap, SE_STATS), tableName_(NULL), timer_(CLOCK_MONOTONIC) {
  queryId_ = NULL;
  queryIdLen_ = 0;
  init(FALSE);
}
...
void ExStorageEngineStats::merge(ExStorageEngineStats *other) {
  ...
  scanFilterStats_.merge(other->scanFilterStats_, ScanFilterStats::ADD);
}
...
void ExStorageEngineStats::copyContents(ExStorageEngineStats *other) {
  ...
  char *destPtr = (char *)this + sizeof(ExOperStats);
  UInt32 srcLen = sizeof(ExStorageEngineStats) - sizeof(ExOperStats);
  memcpy((void *)destPtr, (void *)srcPtr, srcLen);
...
ExHdfsScanStats *ExHdfsScanStats::castToExHdfsScanStats() { return this; }
...
NABoolean ExStorageEngineStats::filterForSEstats(struct timespec currTimespec, int filter) {
  ...
  if (filter > 0) {
    ...
    if (blockTime_ >= filter) return TRUE;
  } else if (queryId_ != NULL && (sumIOTime = timer_.getTime()) > 0 &&
...
void ExStorageEngineStats::getVariableStatsInfo(char *dataBuffer, char *dataLen, int maxLen) {
  ...
  sprintf(buf,
          "AnsiName: %s  MessagesBytes: %ld AccessedRows: %ld UsedRows: %ld HiveIOCalls: %ld HiveSumIOTime: %ld "
          "HdfsMaxIOTime: %ld "
          "HbaseSumIOCalls: %ld HbaseSumIOTime: %ld HbaseMaxIOTime: %ld ",
          (char *)tableName_, numBytesRead(), rowsAccessed(), rowsUsed(), numIOCalls_, timer_.getTime(), maxIOTime_,
          hbaseCalls(), timer_.getTime(), maxHbaseIOTime());

  buf += str_len(buf);

  maxLen -= strlen(dataBuffer);

  scanFilterStats_.getVariableStatsInfo(buf, maxLen);
...
int ExStorageEngineStats::getStatsItem(SQLSTATS_ITEM *sqlStats_item) {
  ...
  switch (sqlStats_item->statsItem_id) {
    ...
    case SQLSTATS_ACT_ROWS_FILTERED:
      sqlStats_item->int64_value = filteredRows_;
      break;

    case SQLSTATS_ACT_DATA_USED:
      sqlStats_item->int64_value = usedRows_ * getRecordLength();
      break;
    ...
    case SQLSTATS_DETAIL:
      if (sqlStats_item->str_value != NULL) {
        ...
        scanFilterStats_.getVariableStatsInfo(tmpBuf, bufLen);

        sqlStats_item->str_ret_len = strlen(sqlStats_item->str_value);
      }
      break;
    default:
...
void ExStorageEngineStats::addScanFilterStats(ScanFilterStats &stats, ScanFilterStats::MergeSemantics semantics) {
  scanFilterStats_.addEntry(stats, semantics);
}
...
void ExOperStats::addMessage(const char *msg, int len) {
  ...
  snprintf(fname, sizeof(fname), "Tdb-%s-Inst-%d.data", getTdbName(), getInstNum());
  ofstream fileout(fname, ios::app);
  for (int i = 0; i < len; i++) fileout << msg[i];
  fileout.close();
}
...
ExProbeCacheStats::ExProbeCacheStats(NAMemory *heap)
    : ExOperStats(heap, PROBE_CACHE_STATS), bufferSize_(0), numCacheEntries_(0) {
  init(FALSE);
  longestChain_ = 0;
  numChains_ = 0;
  maxNumChains_ = 0;
...
UInt32 ExProbeCacheStats::pack(char *buffer) {
  UInt32 size = ExOperStats::pack(buffer);
  buffer += size;
...
void ExProbeCacheStats::merge(ExProbeCacheStats *other) {
  ...
  if (maxNumChains_ < other->maxNumChains_) maxNumChains_ = other->maxNumChains_;
}
...
void ExProbeCacheStats::getVariableStatsInfo(char *dataBuffer, char *dataLen, int maxLen) {
  ...
  sprintf(buf, "CacheHits: %d CacheMisses: %d ", cacheHits_, cacheMisses_);
  buf += str_len(buf);

  sprintf(buf, "CanceledHits: %d CanceledMisses: %d CanceledNotStarted: %d ", canceledHits_, canceledMisses_,
          canceledNotStarted_);
  buf += str_len(buf);

  sprintf(buf, "LongestChain: %d MaxNumChains: %d ", longestChain_, maxNumChains_);
  buf += str_len(buf);

  sprintf(buf, "HighestUseCount: %d ResultBufferSizeBytes: %d NumCacheEntries: %d", highestUseCount_, bufferSize_,
          numCacheEntries_);
  buf += str_len(buf);
...
ExPartitionAccessStats::ExPartitionAccessStats(NAMemory *heap)
    : ExOperStats(heap, PARTITION_ACCESS_STATS), ansiName_(NULL), fileName_(NULL), opens_(0), openTime_(0) {}

ExPartitionAccessStats::~ExPartitionAccessStats() {
  if (ansiName_) heap_->deallocateMemory((void *)ansiName_);
  if (fileName_) heap_->deallocateMemory((void *)fileName_);
}
...
UInt32 ExPartitionAccessStats::pack(char *buffer) {
...
void ExPartitionAccessStats::getVariableStatsInfo(char *dataBuffer, char *dataLen, int maxLen) {
  ...
  sprintf(buf,
          "AnsiName: %s PhysName: %s BuffersSize: %u BuffersSent: %u BuffersRcvd: %u NumMessages: %ld MsgBytes: %ld "
          "MsgBytesSent: %ld MsgBytesRcvd: %ld ",
          ansiName_, fileName_, bufferStats()->sendBufferSize(), bufferStats()->sentBuffers().entryCnt(),
          bufferStats()->recdBuffers().entryCnt(), exeSEStats()->getNumIOCalls(), exeSEStats()->getNumIOBytes(),
          bufferStats()->totalSentBytes(), bufferStats()->totalRecdBytes());
  buf += str_len(buf);

  sprintf(buf,
          "SendUtilMin: %ld SendUtilMax: %ld SendUtilAvg: %f RecvUtilMin: %ld RecvUtilMax: %ld RecvUtilAvg: %f Opens: "
          "%u OpenTime: %ld ",
          bufferStats()->sentBuffers().min(), bufferStats()->sentBuffers().max(), bufferStats()->sentBuffers().mean(),
          bufferStats()->recdBuffers().min(), bufferStats()->recdBuffers().max(), bufferStats()->recdBuffers().mean(),
          opens_, openTime_);
  buf += str_len(buf);
...
//////////////////////////////////////////////////////////////////
ExHashGroupByStats::ExHashGroupByStats(NAMemory *heap, ex_tcb *tcb, const ComTdb *tdb)
    : ExBMOStats(heap, GROUP_BY_STATS, tcb, tdb),
      partialGroups_(0),
      memSize_(0),
      ioSize_(0),
      clusterCnt_(0),
...
void ExHashGroupByStats::merge(ExHashGroupByStats *other) {
  ...
  partialGroups_ += other->partialGroups_;
  if (other->memSize_ > memSize_) memSize_ = other->memSize_;
  ioSize_ += other->ioSize_;
  ExClusterStats *clusterStats = other->clusterStats_;
  while (clusterStats) {
    addClusterStats(*clusterStats);
...
UInt32 ExHashGroupByStats::pack(char *buffer) {
...
ExHashGroupByStats *ExHashGroupByStats::castToExHashGroupByStats() { return this; }
...
void ExHashGroupByStats::getVariableStatsInfo(char *dataBuffer, char *dataLen, int maxLen) {
  ...
  if (clusterStats_) {
    clusterStats_->getVariableStatsInfoAggr(buf, dataLen, maxLen - (buf - dataBuffer));
    buf += *((short *)dataLen);
  } else {
    // display a empty cluster Statistics info
    sprintf(buf, "NumClusters: %u ResidentClusters: %u TotNumBuckets: %u ", 0, 0, 0);
    buf += str_len(buf);
  }
...
//////////////////////////////////////////////////////////////////
ExHashJoinStats::ExHashJoinStats(NAMemory *heap, ex_tcb *tcb, const ComTdb *tdb)
    : ExBMOStats(heap, HASH_JOIN_STATS, tcb, tdb),
      phase_(0),
      memSize_(0),
      ioSize_(0),
      emptyChains_(0),
...
ExHashJoinStats::ExHashJoinStats(NAMemory *heap)
    : ExBMOStats(heap, HASH_JOIN_STATS),
      phase_(0),
      memSize_(0),
      ioSize_(0),
      emptyChains_(0),
...
void ExHashJoinStats::init(NABoolean resetDop) {
  ExBMOStats::init(resetDop);
  for (short p = 0; p < 3; p++) phaseTimes_[p].reset();
  phase_ = 0;
  memSize_ = 0;
  ioSize_ = 0;
  emptyChains_ = 0;
...
UInt32 ExHashJoinStats::pack(char *buffer) {
  UInt32 size = ExBMOStats::pack(buffer);
  buffer += size;
  size += packIntoBuffer(buffer, memSize_);
  size += packIntoBuffer(buffer, ioSize_);
...
void ExHashJoinStats::copyContents(ExHashJoinStats *other) {
  ...
  for (short p = 0; p < 3; p++) phaseTimes_[p] = other->phaseTimes_[p];
...
void ExHashJoinStats::merge(ExHashJoinStats *other) {
  ...
  for (short p = 0; p < 3; p++) phaseTimes_[p] = phaseTimes_[p] + other->phaseTimes_[p];
  emptyChains_ += other->emptyChains_;
  ExClusterStats *clusterStats = other->clusterStats_;
  while (clusterStats) {
    addClusterStats(*clusterStats);
...
ExHashJoinStats *ExHashJoinStats::castToExHashJoinStats() { return this; }
...
void ExHashJoinStats::getVariableStatsInfo(char *dataBuffer, char *dataLen, int maxLen) {
  ...
  getScanFilterStats()->getVariableStatsInfo(buf, maxLen - (buf - dataBuffer), "Filters: ");
...
ExESPStats::ExESPStats(NAMemory *heap) : ExOperStats(heap, ESP_STATS), sendTopStatID_(-1) {}
...
void ExESPStats::getVariableStatsInfo(char *dataBuffer, char *dataLen, int maxLen) {
  ...
  sprintf(buf, "BuffersSent: %u MsgBytesSent: %ld SendUtilMin: %ld SendUtilMax: %ld SendUtilAvg: %f ",
          bufferStats()->sentBuffers().entryCnt(), bufferStats()->totalSentBytes(), bufferStats()->sentBuffers().min(),
          bufferStats()->sentBuffers().max(), bufferStats()->sentBuffers().mean());
  buf += str_len(buf);

  sprintf(buf, "RecvBufferSize: %u ", bufferStats()->recdBufferSize());
  buf += str_len(buf);

  sprintf(buf, "BuffersRcvd: %u MsgBytesRcvd: %ld RecvUtilMin: %ld RecvUtilMax: %ld RecvUtilAvg: %f ",
          bufferStats()->recdBuffers().entryCnt(), bufferStats()->totalRecdBytes(), bufferStats()->recdBuffers().min(),
          bufferStats()->recdBuffers().max(), bufferStats()->recdBuffers().mean());
  buf += str_len(buf);
...
////////////////////////////////////////////////////////////////
ExSplitTopStats::ExSplitTopStats(NAMemory *heap, ex_tcb *tcb, const ComTdb *tdb)
    : ExOperStats(heap, SPLIT_TOP_STATS, tcb, tdb), maxChildren_(0), actChildren_(0) {
  maxChildren_ = ((ComTdbSplitTop *)tdb)->getBottomNumParts();
}

ExSplitTopStats::ExSplitTopStats(NAMemory *heap)
    : ExOperStats(heap, SPLIT_TOP_STATS), maxChildren_(0), actChildren_(0) {}
...
void ExSplitTopStats::getVariableStatsInfo(char *dataBuffer, char *dataLen, int maxLen) {
  ...
  sprintf(buf, "MaxChildren: %u ActChildren: %u", maxChildren_, actChildren_);
  buf += str_len(buf);
...
// class ExSortStats
//////////////////////////////////////////////////////////////////
ExSortStats::ExSortStats(NAMemory *heap, ex_tcb *tcb, const ComTdb *tdb)
    : ExBMOStats(heap, SORT_STATS, tcb, tdb),
      runSize_(0),
      numRuns_(0),
      numCompares_(0),
      numDupRecs_(0),
...
ExSortStats *ExSortStats::castToExSortStats() { return this; }
...
void ExSortStats::getVariableStatsInfo(char *dataBuffer, char *dataLen, int maxLen) {
  ...
  sprintf(buf,
          "RunSize: %u NumRuns: %u NumCompares: %u NumDupRecs: %u ScratchBlockSize: %u NumScratchBlocks: %u "
          "NumScratchWrites: %u NumScratchReads: %u",
          runSize_, numRuns_, numCompares_, numDupRecs_, scrBlockSize_, scrNumBlocks_, scrNumWrites_, scrNumReads_);
  buf += str_len(buf);
...
//////////////////////////////////////////////////////////////////
ExMeasBaseStats::ExMeasBaseStats(NAMemory *heap, StatType statType, ex_tcb *tcb, const ComTdb *tdb)
    : ExOperStats(heap, statType, tcb, tdb) {}

ExMeasBaseStats::ExMeasBaseStats(NAMemory *heap, StatType statType) : ExOperStats(heap, statType, NULL, NULL) {}

ExMeasBaseStats *ExMeasBaseStats::castToExMeasBaseStats() { return this; }
...
UInt32 ExMeasBaseStats::pack(char *buffer) {
  UInt32 size = 0;
...
void ExMeasBaseStats::getVariableStatsInfo(char *dataBuffer, char *datalen, int maxLen) {
  ...
  sprintf(buf,
          "NumMessages: " PF64 " MessagesBytes: " PF64 " StatsBytes: %d  AccessedRows: " PF64 " UsedRows: " PF64
          " DiskIOs: " PF64 "  MaxIOTime: " PF64 " ",
          exeSEStats()->getNumIOCalls(), exeSEStats()->getNumIOBytes(), 0, exeSEStats()->getAccessedRows(),
          exeSEStats()->getUsedRows(), exeSEStats()->getNumIOCalls(), exeSEStats()->getMaxIOTime());
  buf += str_len(buf);
...
// class ExMeasStats
//////////////////////////////////////////////////////////////////
ExMeasStats::ExMeasStats(NAMemory *heap, ex_tcb *tcb, const ComTdb *tdb)
    : ExMeasBaseStats(heap, MEAS_STATS, tcb, NULL) {
  executionCount_ = 0;
  init(FALSE);
  initHistory();
  if (tdb != NULL) scratchOverflowMode_ = ((ComTdb *)tdb)->getOverFlowMode();
  queryId_ = NULL;
  queryIdLen_ = 0;
}

ExMeasStats::ExMeasStats(NAMemory *heap) : ExMeasBaseStats(heap, MEAS_STATS) {
  executionCount_ = 0;
  init(FALSE);
  initHistory();
  queryId_ = NULL;
...
void ExMeasStats::getVariableStatsInfo(char *dataBuffer, char *datalen, int maxLen) {
  ...
  if ((Int32)getCollectStatsType() == SQLCLI_CPU_OFFENDER_STATS) {
    ...
    sprintf(buf,
            ...
            statType(), "NULL", cpuTime_, (UInt32)spaceUsage_, (UInt32)spaceAlloc_, (UInt32)heapUsage_,
            (UInt32)heapAlloc_, (UInt32)heapWM_, reqMsgCnt_, reqMsgBytes_, replyMsgCnt_, replyMsgBytes_);
  } else

  {
    ...
    sprintf(buf,
            ...
            statType(), getNewprocess(), getNewprocessTime(), getTimeouts(), getNumSorts(), getSortElapsedTime(),
            spaceAlloc_ + espSpaceAlloc_, spaceUsage_ + espSpaceUsage_, heapAlloc_ + espHeapAlloc_,
            heapUsage_ + espHeapUsage_, heapWM_ + espHeapWM_, cpuTime_ + espCpuTime_, reqMsgCnt_, reqMsgBytes_,
            replyMsgCnt_, replyMsgBytes_, scratchOverflowMode_, topN_, scratchFileCount_, spaceBufferSize_,
            spaceBufferCount_, scratchIOSize_, scratchWriteCount_, scratchReadCount_, scratchIOMaxTime_,
            interimRowCount_, udrCpuTime_);
  }
  buf += str_len(buf);
...
ExUDRStats::ExUDRStats(NAMemory *heap) : ExUDRBaseStats(heap, UDR_STATS), UDRName_(NULL){};

ExUDRStats::~ExUDRStats() {
  if (UDRName_) heap_->deallocateMemory((void *)UDRName_);
}
...
const char *ExUDRStats::getTextVal() { return UDRName_; }
...
ExStatisticsArea::ExStatisticsArea(NAMemory *heap, int sendBottomNum, ComTdb::CollectStatsType cst,
  ...
  else if (collectStatsType_ == ComTdb::PERTABLE_STATS || collectStatsType_ == ComTdb::PROGRESS_STATS ||
           collectStatsType_ == ComTdb::OPERATOR_STATS)
    ...
    while ((stat = getNext()) != NULL) {
      ...
      switch (statType) {
        ...
        case ExOperStats::SE_STATS:
          ...
          break;
        case ExOperStats::OBJECT_EPOCH_STATS:
          NADELETE((ExObjectEpochStats *)stat, ExObjectEpochStats, heap_);
          break;
        case ExOperStats::OBJECT_LOCK_STATS:
          NADELETE((ExObjectLockStats *)stat, ExObjectLockStats, heap_);
          break;
        case ExOperStats::MASTER_STATS:
          NADELETE((ExMasterStats *)stat, ExMasterStats, heap_);
          break;
        case ExOperStats::PROCESS_STATS:
          NADELETE((ExProcessStats *)stat, ExProcessStats, heap_);
          break;
        case ExOperStats::QUERY_INVALIDATION_STATS:
          NADELETE((ExQryInvalidStats *)stat, ExQryInvalidStats, heap_);
          break;
        default:
          NADELETE(stat, ExOperStats, heap_);
      }
...
void ExStatisticsArea::initEntries() {
  ...
  if (masterStats_ != NULL) masterStats_->setRowsReturned(0);
}
...
NABoolean ExStatisticsArea::merge(ExOperStats *other, UInt16 statsMergeType) {
  ...
  switch (statsMergeType) {
    case SQLCLI_ACCUMULATED_STATS:
      if ((stat = getNext()) == NULL) return FALSE;
      ...
      while ((stat = getNext()) != NULL) {
        switch (other->statType()) {
          case ExOperStats::ROOT_OPER_STATS:
            ...
            break;
          case ExOperStats::BMO_STATS:
            if (statsMergeType == SQLCLI_PROGRESS_STATS && stat->statType() == ExOperStats::BMO_STATS &&
            ...
            break;
          case ExOperStats::UDR_BASE_STATS:
            if (statsMergeType == SQLCLI_PROGRESS_STATS && stat->statType() == ExOperStats::UDR_BASE_STATS &&
                stat->getTdbId() == other->getTdbId()) {
    ...
    case SQLCLI_OPERATOR_STATS:
      if (other->getVersion() < _STATS_RTS_VERSION_R25 && other->statType() != ExOperStats::EX_OPER_STATS &&
          other->statType() != ExOperStats::ROOT_OPER_STATS && other->statType() != ExOperStats::BMO_STATS &&
          other->statType() != ExOperStats::UDR_BASE_STATS && other->statType() != ExOperStats::HBASE_ACCESS_STATS &&
          other->statType() != ExOperStats::HDFSSCAN_STATS && other->statType() != ExOperStats::REORG_STATS)
...
NABoolean ExStatisticsArea::merge(ExStatisticsArea *otherStatsArea, UInt16 statsMergeType) {
  ...
  if (otherStatsArea == NULL) return TRUE;
  ...
  if (otherStatsArea->numEntries() == 0) return TRUE;  // nothing to merge
    ...
    if (merge(stat, tempStatsMergeType) == FALSE) {
      ...
      switch (tempStatsMergeType) {
        case SQLCLI_ACCUMULATED_STATS:
          ...
          insert(newStat);
          if (merge(stat, tempStatsMergeType) == FALSE) ex_assert(FALSE, "Failed to merge accumulated stats");
          break;
...
ExOperStats *ExStatisticsArea::getNext() { return (ExOperStats *)(entries_->getNext()); }
...
NABoolean ExStatisticsArea::anyHaveSentMsgIUD() {
  if ((collectStatsType_ == ComTdb::OPERATOR_STATS) || (collectStatsType_ == ComTdb::PERTABLE_STATS) ||
      (collectStatsType_ == ComTdb::ALL_STATS)) {
    entries_->position();

    ExOperStats *stat;
    while ((stat = getNext()) != NULL) {
      if (stat->hasSentMsgIUD()) return TRUE;
    }
  }
  return FALSE;
}
...
ExOperStats *ExStatisticsArea::get(ExOperStats::StatType type, ComTdb::ex_node_type tdbType) {
  entries_->position();

  ExOperStats *stat;
  while ((stat = getNext()) != NULL) {
    if ((stat->statType() == type) && (stat->getTdbType() == tdbType)) {
      // found it.
      return stat;
    }
  }
...
ExOperStats *ExStatisticsArea::get(ExOperStats::StatType type, int tdbId) {
  ...
  ExOperStats *stat;
  while ((stat = getNext()) != NULL) {
    if ((collectStatsType_ == ComTdb::PERTABLE_STATS || collectStatsType_ == ComTdb::PROGRESS_STATS) &&
        stat->statType() == type && stat->getPertableStatsId() == tdbId)
      return stat;
    if ((stat->statType() == type) && (stat->getTdbId() == tdbId)) return stat;
    if ((type == ExOperStats::NO_OP) && (stat->getTdbId() == tdbId)) return stat;
  }
...
IpcMessageObjSize ExStatisticsArea::packedLength() {
  ...
  if (statsInDp2()) {
    ...
    size += sizeof(unsigned char)  // version info
            + sizeof(detailedFlags_.smallFlags_) + sizeof(detailedFlags_.otherFlags_);
  } else {
...
IpcMessageObjSize ExStatisticsArea::packSmallObjIntoMessage(IpcMessageBufferPtr buffer) {
  IpcMessageObjSize size = 0;
...
IpcMessageObjSize ExStatisticsArea::packObjIntoMessage(IpcMessageBufferPtr buffer) {
  IpcMessageObjSize size = 0;
  ...
  for (int i = 0; i < counter; i++) {
    ...
    switch (statType) {
      ...
      case ExOperStats::SE_STATS:
        ...
        break;
      case ExOperStats::OBJECT_EPOCH_STATS:
        stat = new (heap_) ExObjectEpochStats((NAHeap *)heap_);
        break;
      case ExOperStats::OBJECT_LOCK_STATS:
        stat = new (heap_) ExObjectLockStats((NAHeap *)heap_);
        break;
      case ExOperStats::QUERY_INVALIDATION_STATS:
        stat = new (heap_) ExQryInvalidStats((NAHeap *)heap_);
        break;
      default:
        FAILURE;
    }
...
void ExStatisticsArea::unpackObj(IpcMessageObjType objType, IpcMessageObjVersion objVersion, NABoolean sameEndianness,
...
void ExStatisticsArea::updateSpaceUsage(Space *space, CollHeap *heap) {
  if (rootStats_ != NULL) {
    if (rootStats_->castToExFragRootOperStats()) {
      // determine the high water mark for space
      // and heap usage
      rootStats_->castToExFragRootOperStats()->updateSpaceUsage(space, heap);
    } else if (rootStats_->castToExMeasStats()) {
      // determine the high water mark for space
      // and heap usage
      rootStats_->castToExMeasStats()->updateSpaceUsage(space, heap);
    }
  }
}
  ...
  for (Int32 i = 0; i < no_of_stats_items; i++) {
    switch (sqlStats_items[i].stats_type) {
      ...
      case ExOperStats::MASTER_STATS:
        ...
        break;
      case ExOperStats::MEAS_STATS:
        if (measStats == NULL) measStats = (ExMeasStats *)get(ExOperStats::MEAS_STATS, _UNINITIALIZED_TDB_ID);
        if (measStats != NULL)
      ...
      case ExOperStats::ROOT_OPER_STATS:
        if (rootStats == NULL)
          rootStats = (ExFragRootOperStats *)get(ExOperStats::ROOT_OPER_STATS, sqlStats_items[i].tdb_id);
        if (rootStats != NULL)
        ...
        break;
      case ExOperStats::BMO_STATS:
        if (bmoStats == NULL) bmoStats = (ExBMOStats *)get(ExOperStats::BMO_STATS, sqlStats_items[i].tdb_id);
        if (bmoStats != NULL)
      ...
      case ExOperStats::UDR_BASE_STATS:
        ...
        break;
      case ExOperStats::ESP_STATS:
        if (espStats == NULL) espStats = (ExESPStats *)get(ExOperStats::ESP_STATS, sqlStats_items[i].tdb_id);
        if (espStats != NULL)
      ...
      case ExOperStats::PARTITION_ACCESS_STATS:
        if (partitionAccessStats == NULL)
          partitionAccessStats =
              (ExPartitionAccessStats *)get(ExOperStats::PARTITION_ACCESS_STATS, sqlStats_items[i].tdb_id);
        if (partitionAccessStats != NULL)
      ...
      case ExOperStats::FAST_EXTRACT_STATS:
        ...
        break;
      case ExOperStats::SORT_STATS:
        if (sortStats == NULL) sortStats = (ExSortStats *)get(ExOperStats::SORT_STATS, sqlStats_items[i].tdb_id);
        if (sortStats != NULL)
      ...
      case ExOperStats::SPLIT_TOP_STATS:
        ...
        break;
      case ExOperStats::UDR_STATS:
        if (udrStats == NULL) udrStats = (ExUDRStats *)get(ExOperStats::UDR_STATS, sqlStats_items[i].tdb_id);
        if (udrStats != NULL)
    ...
    if (tempRetcode != 0 && retcode == 0) retcode = tempRetcode;
  ...
  switch (getCollectStatsType()) {
    case SQLCLI_ACCUMULATED_STATS:
      if (numEntries() == 0)
        ...
        no_of_stats_desc = 2;
      if (no_returned_stats_desc != NULL) *no_returned_stats_desc = no_of_stats_desc;
      if (max_stats_desc < no_of_stats_desc) return -CLI_INSUFFICIENT_STATS_DESC;
    ...
    case SQLCLI_OPERATOR_STATS:
      ...
      currTdbId = 0;
      while ((stat = getNext()) != NULL) {
        if ((tdbId = stat->getTdbId()) > currTdbId) currTdbId = tdbId;
      }
    ...
    case SQLCLI_PERTABLE_STATS:
    case SQLCLI_PROGRESS_STATS:
      if (no_returned_stats_desc != NULL) *no_returned_stats_desc = numEntries() + 1;
      if (max_stats_desc < numEntries() + 1) return -CLI_INSUFFICIENT_STATS_DESC;
      i = 0;
      ...
      position();
      for (; (stat = getNext()) != NULL;) {
        if (stat->statType() == ExOperStats::HBASE_ACCESS_STATS || stat->statType() == ExOperStats::HDFSSCAN_STATS) {
          sqlstats_desc[i].tdb_id = (short)stat->getTdbId();
          sqlstats_desc[i].stats_type = stat->statType();
          sqlstats_desc[i].tdb_name[0] = '\0';
          i++;
    ...
    case SQLCLI_RMS_INFO_STATS:
      if (no_returned_stats_desc != NULL) *no_returned_stats_desc = numEntries();
      if (max_stats_desc < numEntries()) return -CLI_INSUFFICIENT_STATS_DESC;
      position();
    ...
    case SQLCLI_NO_STATS:
      ...
      // the stats type
      if (no_returned_stats_desc != NULL) *no_returned_stats_desc = 1;
      if (max_stats_desc < 1) return -CLI_INSUFFICIENT_STATS_DESC;
      sqlstats_desc[0].tdb_id = (short)_UNINITIALIZED_TDB_ID;
      sqlstats_desc[0].stats_type = ExOperStats::MASTER_STATS;
      sqlstats_desc[0].tdb_name[0] = '\0';
      break;
...
void ExStatisticsArea::setCpuStatsHistory() {
  ...
  while ((stat = getNext()) != NULL) {
    statType = stat->statType();
    if (statType == ExOperStats::MEAS_STATS || statType == ExOperStats::ROOT_OPER_STATS) {
...
NABoolean ExStatisticsArea::appendCpuStats(ExMasterStats *masterStats, NABoolean appendAlways, short subReqType,
  ...
  if (!append) append = masterStats->filterForCpuStats(subReqType, currTimestamp, etFilter);
  if (append) {
...
NABoolean ExStatisticsArea::appendCpuStats(ExOperStats *stat, NABoolean appendAlways, int filter,
  ...
  if (!append) {
    ...
    else if (statType == ExOperStats::HDFSSCAN_STATS)
...
NABoolean ExStatisticsArea::appendCpuStats(ExStatisticsArea *other, NABoolean appendAlways) {
    ...
    switch (appendStatsType) {
      ...
      case SQLCLI_QID_DETAIL_STATS: {
        switch (statType) {
          ...
          case ExOperStats::ROOT_OPER_STATS:
            tdbType = stat->getTdbType();
            if ((detailLevel_ == -1 && (tdbType == ComTdb::ex_SPLIT_BOTTOM || tdbType == ComTdb::ex_ROOT)) ||
      ...
      case SQLCLI_SE_OFFENDER_STATS:
        ...
        break;
      case SQLCLI_OBJECT_EPOCH_STATS:
        if (statType == ExOperStats::OBJECT_EPOCH_STATS) {
          ExObjectEpochStats *oeStats = new (getHeap()) ExObjectEpochStats((NAHeap *)getHeap());
          oeStats->setCollectStatsType(getCollectStatsType());
          oeStats->copyContents((ExObjectEpochStats *)stat);
          insert(oeStats);
          retcode = TRUE;
        }
        break;
      case SQLCLI_OBJECT_LOCK_STATS:
        if (statType == ExOperStats::OBJECT_LOCK_STATS) {
          ExObjectLockStats *oeStats = new (getHeap()) ExObjectLockStats((NAHeap *)getHeap());
          oeStats->setCollectStatsType(getCollectStatsType());
          oeStats->copyContents((ExObjectLockStats *)stat);
          insert(oeStats);
          retcode = TRUE;
        }
        break;
...
void ExStatisticsArea::incReqMsg(long msgBytes) {
  ...
  if (rootStats_ != NULL) {
    ...
    else if ((measStats = rootStats_->castToExMeasStats()) != NULL)
...
void ExStatisticsArea::setQueryId(char *queryId, int queryIdLen) {
  ...
  ExOperStats *stat;
  while ((stat = getNext()) != NULL) {
    if (stat->castToExFragRootOperStats()) stat->castToExFragRootOperStats()->setQueryId(queryId, queryIdLen);
  }
}
...
ExStatsTcb::ExStatsTcb(const ExStatsTdb &statsTdb, ex_globals *glob) : ex_tcb(statsTdb, 1, glob) {
  ...
  qparent_.up = new (space) ex_queue(ex_queue::UP_QUEUE, statsTdb.queueSizeUp_, statsTdb.criDescUp_, space);
  ...
  // fixup expressions
  if (statsTdb.scanExpr_) (void)statsTdb.scanExpr_->fixup(0, getExpressionMode(), this, space, heap, FALSE, glob);

  if (statsTdb.inputExpr_) (void)statsTdb.inputExpr_->fixup(0, getExpressionMode(), this, space, heap, FALSE, glob);

  if (statsTdb.projExpr_) (void)statsTdb.projExpr_->fixup(0, getExpressionMode(), this, space, heap, FALSE, glob);
    ...
    switch (pstate->step_) {
      case INITIAL_: {
        ...
        if (statsTdb().getInputExpr()) {
          ...
          evalRetCode = statsTdb().getInputExpr()->eval(pentry_down->getAtp(), workAtp_);
          if (evalRetCode == ex_expr::EXPR_ERROR) {
            ex_assert(0, "ExStatsTcb::work(). Error from inputExpr_->eval()");
          }

          char *inputTuple = workAtp_->getTupp(statsTdb().getInputTupleAtpIndex()).getDataPointer();
          Attributes *modAttr = statsTdb().getAttrModName();
          Attributes *stmtAttr = statsTdb().getAttrStmtName();

          if ((stmtAttr->getNullFlag()) && (str_cmp(&inputTuple[stmtAttr->getNullIndOffset()], "\377\377", 2) == 0))
          ...
          if (stmtName) {
            ...
            str_cpy_and_null(inputStmtName_, inputStmtName_, stmtAttr->getLength(), '\0', ' ');
          ...
          switch (statsParamType) {
            case SQLCLI_STATS_REQ_STMT: {
              ...
              if (stmtName != NULL && (strncasecmp(stmtName, "CURRENT", 7) != 0)) {
                while ((NOT found) && (stmt = (Statement *)stmtList->getNext())) {
                  const char *ident = stmt->getIdentifier();

                  if ((ident) && (str_len(stmtName) == str_len(ident)) &&
              ...
              if ((stats == NULL) || (NOT stats->statsEnabled()))
                pstate->step_ = DONE_;
              else {
                if (statsMergeType_ == SQLCLI_SAME_STATS || statsMergeType_ == stats->getCollectStatsType() ||
                    // if the collection type is ALL_STATS, ignore statsMergeType_
                    stats->getOrigCollectStatsType() == ComTdb::ALL_STATS) {
                  stats_ = stats;
                } else {
                  stats_ = new (getHeap()) ExStatisticsArea(getHeap(), 0, (ComTdb::CollectStatsType)statsMergeType_,
          ...
          if ((stats_ == NULL) || (NOT stats_->statsEnabled()))
            pstate->step_ = DONE_;
          else
            pstate->step_ = GET_MASTER_STATS_ENTRY_;
        }
      ...
      case SEND_TO_SSMP_: {
        ...
        if ((stats_ == NULL) || (NOT stats_->statsEnabled())) {
          if (stats_ == NULL && reqType_ == SQLCLI_STATS_REQ_QID && (!diagsArea_)) {
            IpcAllocateDiagsArea(diagsArea_, getHeap());
      ...
      case APPLY_SCAN_EXPR_: {
        workAtp_->getTupp(statsTdb().getStatsTupleAtpIndex()).setDataPointer(data_);
        NABoolean rowSelected = TRUE;
        if (statsTdb().scanExpr_) {
          ex_expr::exp_return_type evalRetCode;
          evalRetCode = statsTdb().scanExpr_->eval(pentry_down->getAtp(), workAtp_);
          if (evalRetCode == ex_expr::EXPR_FALSE)
            rowSelected = FALSE;
          else if (evalRetCode == ex_expr::EXPR_ERROR) {
            ex_assert(0, "ExStatsTcb::work(). Error from scanExpr_->eval()");
            rowSelected = FALSE;
          }
        }
      ...
      case PROJECT_: {
        ...
        if (pool_->get_free_tuple(pentry_up->getTupp(statsTdb().criDescUp_->noTuples() - 1), statsTdb().tupleLen_)) {
          return WORK_POOL_BLOCKED;
        }

        pstate->matchCount_++;

        str_cpy_all(pentry_up->getTupp(statsTdb().criDescUp_->noTuples() - 1).getDataPointer(), data_,
                    statsTdb().tupleLen_);
      ...
      case ERROR_: {
        ...
        if (diagsArea_) {
          ...
          // qparent's diags.
          ExHandleArkcmpErrors(qparent_, pentry_down, 0, getGlobals(), getDiagsArea(), (ExeErrorCode)(-2024));
  ...
  for (UInt32 i = 0; i < tDesc->numAttrs(); i++) {
    ...
    switch (i) {
      ...
      case STAT_STATEMENT_NAME:
        ...
        if (src) {
          ...
          if (strncasecmp(src, "QID=", 4) == 0) {
            ComSqlId::getSqlQueryIdAttr(ComSqlId::SQLQUERYID_STMTNAME, src + 4, (str_len(src) - 4), sNameLen, sName);
            src = sName;
      ...
      case STAT_VARIABLE_INFO: {
        stat->getVariableStatsInfo(&data_[attr->getOffset()], &data_[attr->getVCLenIndOffset()], attr->getLength());
        callConvDoit = FALSE;
      } break;
      default:
    ...
    if (!valIsNull) {
      if (callConvDoit && ::convDoIt(src, srcLen, srcType, 0, 0, &data_[attr->getOffset()], attr->getLength(),
...
ExStatisticsArea *ExStatsTcb::sendToSsmp() {
  ExStatisticsArea *stats = NULL;
  CliGlobals *cliGlobals = getGlobals()->castToExExeStmtGlobals()->castToExMasterStmtGlobals()->getCliGlobals();
  ...
  if (reqType_ == SQLCLI_STATS_REQ_CPU_OFFENDER || reqType_ == SQLCLI_STATS_REQ_MEM_OFFENDER ||
      reqType_ == SQLCLI_STATS_REQ_SE_OFFENDER || reqType_ == SQLCLI_STATS_REQ_ET_OFFENDER ||
      reqType_ == SQLCLI_STATS_REQ_RMS_INFO) {
    cpuStatsReq = new (cliGlobals->getIpcHeap())
        RtsCpuStatsReq(rtsHandle, cliGlobals->getIpcHeap(), nodeName_, cpu_, activeQueryNum_, reqType_);
    ...
    switch (reqType_) {
      ...
      case SQLCLI_STATS_REQ_CPU:
        rtsQueryId = new (cliGlobals->getIpcHeap())
            RtsQueryId(cliGlobals->getIpcHeap(), nodeName_, cpu_, statsMergeType_, activeQueryNum_);
        break;
      case SQLCLI_STATS_REQ_PID:
      case SQLCLI_STATS_REQ_PROCESS_INFO:
        rtsQueryId = new (cliGlobals->getIpcHeap())
            RtsQueryId(cliGlobals->getIpcHeap(), nodeName_, cpu_, pid_, statsMergeType_, activeQueryNum_, reqType_);
        rtsQueryId->setSubReqType(subReqType_);
        break;
      case SQLCLI_STATS_REQ_QID_INTERNAL:
        rtsQueryId =
            new (cliGlobals->getIpcHeap()) RtsQueryId(cliGlobals->getIpcHeap(), nodeName_, cpu_, pid_, timeStamp_,
                                                      queryNumber_, statsMergeType_, activeQueryNum_);
        break;
      default:
        rtsQueryId = NULL;
        break;
  ...
  if (!ssmpMsgStream->isReplyReceived()) {
    IpcAllocateDiagsArea(diagsArea_, getHeap());
    (*diagsArea_) << DgSqlCode(-EXE_RTS_TIMED_OUT) << DgString0(inputStmtName_) << DgInt0(RtsTimeout / 100);
    return NULL;
  }
  retryAttempts_ = 0;
  if (ssmpMsgStream->getRtsQueryId() != NULL) {
    rtsQueryId->decrRefCount();
    statsReq->decrRefCount();
    if (inputStmtName_) NADELETEBASIC(inputStmtName_, getHeap());
...
int ExStatsTcb::parse_stmt_name(char *string, int len) {
  ...
  reqType_ =
      (short)str_parse_stmt_name(string, len, nodeName_, &cpu_, &pid_, &timeStamp_, &queryNumber_, &idOffset, &idLen,
                                 &activeQueryNum_, &statsMergeType_, &detailLevel_, &subReqType_, &filter_);
  ...
  if (reqType_ == SQLCLI_STATS_REQ_QID_INTERNAL) nodeName_[0] = '\0';
  return reqType_;
}
...
ExQIStatsPrivateState::ExQIStatsPrivateState() {
  matchCount_ = 0;
  step_ = ExQryInvalidStatsTcb::INITIAL_;
}

ex_tcb_private_state *ExQIStatsPrivateState::allocate_new(const ex_tcb *tcb) {
  return new (((ex_tcb *)tcb)->getSpace()) ExQIStatsPrivateState();
};
...
void ExMasterStats::init(NABoolean resetDop) {
  ...
  validPrivs_ = true;
  numSIKeys_ = 0;
  if ((sIKeys_ != NULL) && (sIKeys_ != &preallocdSiKeys_[0])) NADELETEBASIC(sIKeys_, getHeap());
  sIKeys_ = &preallocdSiKeys_[0];
  memset(sIKeys_, 0, PreAllocatedSikKeys * sizeof(SQL_QIKEY));
  validDDL_ = true;
  validHistogram_ = false;
  numObjUIDs_ = 0;
...
void ExMasterStats::reuse() {
  if (childQid_) NADELETEBASIC(childQid_, getHeap());
  init(FALSE);
}
...
void ExMasterStats::initBeforeExecute(long currentTimeStamp) {
  ...
  isQuerySuspended_ = false;
  querySuspendedTime_ = -1;
  if (cancelComment_ != NULL) cancelComment_[0] = '\0';
  cancelCommentLen_ = 0;
}
...
ExMasterStats::ExMasterStats(NAHeap *heap) : ExOperStats(heap, MASTER_STATS) {
  sourceStr_ = NULL;
  storedSqlTextLen_ = 0;
...
ExMasterStats::ExMasterStats() : ExOperStats() {
  sourceStr_ = NULL;
  storedSqlTextLen_ = 0;
  originalSqlTextLen_ = 0;
  queryId_ = NULL;
...
ExMasterStats::ExMasterStats(NAHeap *heap, char *sourceStr, int storedSqlTextLen, int originalSqlTextLen,
  ...
  if (storedSqlTextLen != 0) {
    ...
    // for mantis11484, wipe out Chinese character at the end of sourceStr
    if (storedSqlTextLen == RMS_STORE_SQL_SOURCE_LEN) {
      for (int i = RMS_STORE_SQL_SOURCE_LEN - 1; i > 0; i--) {
        if (sourceStr_[i] & 0x80) {
          if (sourceStr_[i] & 0x40)  // first byte of multi-byte utf8 character
          {
            sourceStr_[i] = ' ';
            break;
          } else  // multi-byte utf8 character
          {
            sourceStr_[i] = ' ';
            continue;
          }
        } else  // single-byte utf8 character
          break;
      }
    }
...
ExMasterStats::~ExMasterStats() { deleteMe(); }
...
UInt32 ExMasterStats::pack(char *buffer) {
  ...
  buffer += size;
  if (queryIdLen_ != 0 && queryId_ != NULL) size += packStrIntoBuffer(buffer, queryId_, queryIdLen_);
  if (storedSqlTextLen_ != 0 && sourceStr_ != NULL) size += packStrIntoBuffer(buffer, sourceStr_, storedSqlTextLen_);
  if (parentQidLen_ != 0 && parentQid_ != NULL) size += packStrIntoBuffer(buffer, parentQid_, parentQidLen_);
  if (childQidLen_ != 0 && childQid_ != NULL) size += packStrIntoBuffer(buffer, childQid_, childQidLen_);
...
void ExMasterStats::getVariableStatsInfo(char *dataBuffer, char *dataLen, int maxLen) {
  ...
  if ((Int32)getCollectStatsType() == SQLCLI_ET_OFFENDER_STATS) {
    sprintf(buf,
            ...
            ((parentQid_ != NULL) ? (parentQidSystem_[0] != '\0' ? parentQidSystem_ : "SAME") : "NONE"),
            ((childQid_ != NULL) ? childQid_ : "NONE"), rowsReturned_, firstRowReturnTime_, getNumSqlProcs(), numCpus_,
            exePriority_, transId_, (isQuerySuspended_ ? "yes" : "no"), querySuspendedTime_, aqrLastErrorCode_,
            numAqrRetries_, delayBeforeAqrRetry_, reclaimSpaceCount_, timeSinceBlocking(0), timeSinceUnblocking(0),
            lastActivity_, slaName_, profileName_, originalSqlTextLen_, ((sourceStr_ != NULL) ? sourceStr_ : ""));
  } else {
    sprintf(buf,
            "statsRowType: %d Qid: %s CompStartTime: %ld "
            "CompEndTime: %ld "
            ...
            "sqlSrcLen: %d sqlSrc: \"%s\"",
            statType(), ((queryId_ != NULL) ? queryId_ : "NULL"), compStartTime_, compEndTime_, exeStartTime_,
            exeEndTime_, canceledTime_, rowsAffected_, sqlErrorCode_, statsErrorCode_, stmtState,
            compilerStatsInfo_.collectStatsType(), queryType_, subqueryType_, compilerStatsInfo_.dp2RowsAccessed(),
            compilerStatsInfo_.dp2RowsUsed(), compElapsedTime, exeElapsedTime,
            ((parentQid_ != NULL) ? parentQid_ : "NONE"),
            ((parentQid_ != NULL) ? (parentQidSystem_[0] != '\0' ? parentQidSystem_ : "SAME") : "NONE"),
            ((childQid_ != NULL) ? childQid_ : "NONE"), rowsReturned_, firstRowReturnTime_, getNumSqlProcs(), numCpus_,
            exePriority_, transId_, (isQuerySuspended_ ? "yes" : "no"), querySuspendedTime_, aqrLastErrorCode_,
            numAqrRetries_, delayBeforeAqrRetry_, reclaimSpaceCount_, timeSinceBlocking(0), timeSinceUnblocking(0),
            lastActivity_, slaName_, profileName_, exeTimes_.entryCnt(), exeTimes_.min(), exeTimes_.max(),
            exeTimes_.mean(), originalSqlTextLen_, ((sourceStr_ != NULL) ? sourceStr_ : ""));
  }
  buf += str_len(buf);
  *(short *)dataLen = (short)(buf - dataBuffer);
}
...
ExMasterStats *ExMasterStats::castToExMasterStats() { return this; }
...
void ExMasterStats::copyContents(ExMasterStats *other) {
  ...
  if (other->cancelComment_ != NULL) {
    cancelComment_ = new ((NAHeap *)getHeap()) char[other->cancelCommentLen_ + 1];
    str_cpy_all(cancelComment_, other->cancelComment_, other->cancelCommentLen_);
  ...
  // invalidation keys are not currently required for this operation
  // and are not moved
  sIKeys_ = NULL;
  numSIKeys_ = 0;
  objUIDs_ = NULL;
  ...
  strcpy(slaName_, other->slaName_);
  strcpy(profileName_, other->profileName_);
}

void ExMasterStats::setEndTimes(NABoolean updateExeEndTime) {
  if (compStartTime_ != -1 && compEndTime_ == -1) compEndTime_ = NA_JulianTimestamp();
  if (updateExeEndTime && exeStartTime_ != -1 && exeEndTime_ == -1) exeEndTime_ = NA_JulianTimestamp();
}
  ...
  switch (sqlStats_item->statsItem_id) {
    ...
    case SQLSTATS_PROFILE_NAME:
      ...
      if (sqlStats_item->statsItem_id == SQLSTATS_PARENT_QUERY_SYSTEM)
        tempName = (parentQid_ != NULL ? (parentQidSystem_[0] != '\0' ? parentQidSystem_ : "SAME") : "NONE");
      else if (sqlStats_item->statsItem_id == SQLSTATS_SLA_NAME)
        tempName = slaName_;
      else
        tempName = profileName_;
      if (sqlStats_item->str_value != NULL) {
        tempNameLen = str_len(tempName);
        if (tempNameLen <= sqlStats_item->str_max_len) {
          str_cpy(sqlStats_item->str_value, tempName, tempNameLen);
          sqlStats_item->str_ret_len = tempNameLen;
        } else
          sqlStats_item->error_code = -EXE_ERROR_IN_STAT_ITEM;
      }
      break;
    ...
    case SQLSTATS_COMP_TIME:
      if (compStartTime_ == -1)
        ...
        if (sqlStats_item->int64_value < 0) sqlStats_item->int64_value = 0;
      } else
    ...
    case SQLSTATS_EXECUTE_TIME_AVG:
      ...
      break;
    case SQLSTATS_QUERY_HASH:
      sqlStats_item->int64_value = queryHash_;
      break;
    default:
      sqlStats_item->error_code = -EXE_STAT_NOT_FOUND;
      break;
...
Int32 ExMasterStats::timeSinceBlocking(Int32 s) {
  ...
  if (isBlocking_) {
    ...
    if (exeEndTime_ != -1)
      ...
      if (r < s)
        r = 0;
      else if (r == s && (timeNow.tv_usec < blockOrUnblockSince_.tv_usec))
        r = 0;
    }
  }
  return r;
...

{
  ...
  if (subReqType != SQLCLI_STATS_REQ_UNMONITORED_QUERIES && subReqType != SQLCLI_STATS_REQ_QUERIES_IN_COMPILE &&
      (collectStatsType_ == (UInt16)ComTdb::ALL_STATS || collectStatsType_ == (UInt16)ComTdb::NO_STATS))
    return FALSE;
  if (subReqType == SQLCLI_STATS_REQ_DEAD_QUERIES) {
    ...
    if (collectStatsType_ == (UInt16)ComTdb::NO_STATS || collectStatsType_ == (UInt16)ComTdb::ALL_STATS) {
      tsToCompare = -1;
      if (exeEndTime_ != -1)
        tsToCompare = exeEndTime_;
      else {
        if (exeStartTime_ != -1) tsToCompare = exeStartTime_;
      }
      if (tsToCompare == -1) {
        if (compEndTime_ != -1)
          tsToCompare = compEndTime_;
  ...
  } else if (subReqType == SQLCLI_STATS_REQ_QUERIES_IN_CLIENT) {
    if (exeStartTime_ != -1 && exeEndTime_ == -1 && (NOT isBlocking_)) {
      if (timeSinceUnblocking(etTimeInSecs)) retcode = TRUE;
    }
  } else if (subReqType == SQLCLI_STATS_REQ_INACTIVE_QUERIES) {
    if (stmtState_ != Statement::PROCESS_ENDED_ && stmtState_ != Statement::DEALLOCATED_) {
  ...
  } else if (subReqType == SQLCLI_STATS_REQ_ACTIVE_QUERIES) {
    if (stmtState_ != Statement::PROCESS_ENDED_) {
      if (exeStartTime_ != -1) {
        if (exeEndTime_ != -1)
          tsToCompare = exeEndTime_;
  ...
  } else if (subReqType == SQLCLI_STATS_REQ_QUERIES_IN_SQL) {
    if (exeStartTime_ != -1 && exeEndTime_ == -1 && isBlocking_) {
      if (timeSinceBlocking(etTimeInSecs)) retcode = TRUE;
    }
  } else if (subReqType == SQLCLI_STATS_REQ_QUERIES_IN_COMPILE) {
    if (stmtState_ != Statement::PROCESS_ENDED_) {
...
void ExMasterStats::setInvalidationKeys(CliGlobals *cliGlobals, SecurityInvKeyInfo *sikInfo, Int32 numObjUIDs,
  ...
  if ((numSIKeys > PreAllocatedSikKeys) || (numObjUIDs > PreAllocatedObjUIDs)) {
    ...
    if (statsGlobals) {
      int error = statsGlobals->getStatsSemaphore(semId, cliGlobals->myPin());
    }

    if (numSIKeys > PreAllocatedSikKeys) {
      if (sIKeys_ != &preallocdSiKeys_[0]) NADELETEBASIC(sIKeys_, (NAHeap *)getHeap());
  ...
  if (numSIKeys_ > 0) {
    ...
    for (int i = 0; i < numSIKeys_; i++) {
      ...
      char sikOpLit[4];
      ComQIActionTypeEnumToLiteral(pComSecurityKey[i].getSecurityKeyType(), sikOpLit);
  ...
  if (numObjUIDs == 0 || Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL) ||
      CmpCommon::getDefault(TRAF_OBJECT_LOCK_UNSAFE_MODE) == DF_ON) {
    validDDL_ = true;
    validPrivs_ = true;
  }
}
int ExStatsTcb::str_parse_stmt_name(char *string, int len, char *nodeName, short *cpu, pid_t *pid, long *timeStamp,
  ...
  *detailLevel = 0;
  ...
  if (ptr == NULL) return SQLCLI_STATS_REQ_NONE;
  do {
    if (strncasecmp(ptr, "STMT", 4) == 0) {
      ...
      if (stmtTemp != NULL)
        ...
        *idLen = 0;
    } else if (strncasecmp(ptr, "QID_INTERNAL", 12) == 0) {
      ptr = str_tok(NULL, ',', &internal);
      cpuTemp = ptr;
      ptr = str_tok(NULL, ',', &internal);
      pidTemp = ptr;
    ...
    } else if (strncasecmp(ptr, "QID", 3) == 0) {
      ptr = str_tok(NULL, ',', &internal);
      qidTemp = ptr;
      if (qidTemp != NULL)
        *idLen = (short)str_len(ptr);
        ...
        *idLen = 0;
    } else if (strncasecmp(ptr, "SYSTEM", 6) == 0) {
      ptr = str_tok(NULL, ',', &internal);
      nodeNameTemp = ptr;
    } else if (strncasecmp(ptr, "CPU_OFFENDER", 12) == 0) {
      ptr = str_tok(NULL, ',', &internal);
      cpuTemp = ptr;
      cpuOffender = TRUE;
    } else if (strncasecmp(ptr, "HIGHWM_MEM_OFFENDER", 19) == 0) {
      ptr = str_tok(NULL, ',', &internal);
      memThreshold = ptr;
      *subReqType = (short)SQLCLI_STATS_REQ_MEM_HIGH_WM;
      memOffender = TRUE;
    } else if (strncasecmp(ptr, "ALLOC_MEM_OFFENDER", 18) == 0) {
      ptr = str_tok(NULL, ',', &internal);
      memThreshold = ptr;
      *subReqType = (short)SQLCLI_STATS_REQ_MEM_ALLOC;
      memOffender = TRUE;
    } else if (strncasecmp(ptr, "PFS_USE_PERCENT", 15) == 0) {
      ptr = str_tok(NULL, ',', &internal);
      memThreshold = ptr;
      *subReqType = (short)SQLCLI_STATS_REQ_PFS_USE;
      memOffender = TRUE;
    } else if ((strncasecmp(ptr, "CPU", 3) == 0) || (strncasecmp(ptr, "NODE", 4) == 0)) {
      ptr = str_tok(NULL, ',', &internal);
      cpuTemp = ptr;
    } else if (strncasecmp(ptr, "SE_BLOCKED", 10) == 0) {
      ptr = str_tok(NULL, ',', &internal);
      seTemp = ptr;
      diskOffender = TRUE;
    } else if (strncasecmp(ptr, "SE_OFFENDER", 10) == 0) {
      ptr = str_tok(NULL, ',', &internal);
      seOffendTemp = ptr;
      diskOffender = TRUE;
    } else if (strncasecmp(ptr, "QUERIES_IN_SQL", 14) == 0) {
      ptr = str_tok(NULL, ',', &internal);
      etTemp = ptr;
      etOffender = TRUE;
      *subReqType = (short)SQLCLI_STATS_REQ_QUERIES_IN_SQL;
      retcode = SQLCLI_STATS_REQ_ET_OFFENDER;
    } else if (strncasecmp(ptr, "QUERIES_IN_CLIENT", 17) == 0) {
      ptr = str_tok(NULL, ',', &internal);
      etTemp = ptr;
      etOffender = TRUE;
      *subReqType = (short)SQLCLI_STATS_REQ_QUERIES_IN_CLIENT;
      retcode = SQLCLI_STATS_REQ_ET_OFFENDER;
    } else if (strncasecmp(ptr, "INACTIVE_QUERIES", 16) == 0) {
      ptr = str_tok(NULL, ',', &internal);
      etTemp = ptr;
      etOffender = TRUE;
      *subReqType = (short)SQLCLI_STATS_REQ_INACTIVE_QUERIES;
      retcode = SQLCLI_STATS_REQ_ET_OFFENDER;
    } else if (strncasecmp(ptr, "ACTIVE_QUERIES", 14) == 0) {
      ptr = str_tok(NULL, ',', &internal);
      etTemp = ptr;
      etOffender = TRUE;
      *subReqType = (short)SQLCLI_STATS_REQ_ACTIVE_QUERIES;
      retcode = SQLCLI_STATS_REQ_ET_OFFENDER;
    } else if (strncasecmp(ptr, "DEAD_QUERIES", 12) == 0) {
      ptr = str_tok(NULL, ',', &internal);
      etTemp = ptr;
      etOffender = TRUE;
      *subReqType = SQLCLI_STATS_REQ_DEAD_QUERIES;
      retcode = SQLCLI_STATS_REQ_ET_OFFENDER;
    } else if (strncasecmp(ptr, "UNMONITORED_QUERIES", 22) == 0) {
      ptr = str_tok(NULL, ',', &internal);
      etTemp = ptr;
      etOffender = TRUE;
      *subReqType = (short)SQLCLI_STATS_REQ_UNMONITORED_QUERIES;
      retcode = SQLCLI_STATS_REQ_ET_OFFENDER;
    } else if (strncasecmp(ptr, "QUERIES_IN_COMPILE", 21) == 0) {
      ptr = str_tok(NULL, ',', &internal);
      etTemp = ptr;
      etOffender = TRUE;
      *subReqType = (short)SQLCLI_STATS_REQ_QUERIES_IN_COMPILE;
      retcode = SQLCLI_STATS_REQ_ET_OFFENDER;
    } else if (strncasecmp(ptr, "PID", 3) == 0) {
      ptr = str_tok(NULL, ',', &internal);
      pidTemp = ptr;
      pidStats = TRUE;
      retcode = SQLCLI_STATS_REQ_PID;
    } else if (strncasecmp(ptr, "PROCESS_STATS", 13) == 0) {
      ptr = str_tok(NULL, ',', &internal);
      pidTemp = ptr;
      processStats = TRUE;
      retcode = SQLCLI_STATS_REQ_PROCESS_INFO;
    } else if (strncasecmp(ptr, "ACTIVE", 6) == 0) {
      ptr = str_tok(NULL, ',', &internal);
      activeQueryTemp = ptr;
    } else if (strncasecmp(ptr, "MERGE", 5) == 0) {
      ptr = str_tok(NULL, ',', &internal);
      mergeTemp = ptr;
    } else if (strncasecmp(ptr, "DETAIL", 6) == 0) {
      ptr = str_tok(NULL, ',', &internal);
      detailTemp = ptr;
    } else if ((strncasecmp(ptr, "TDBID_DETAIL", 12) == 0) || (strncasecmp(ptr, "TCBID_DETAIL", 12) == 0)) {
      ptr = str_tok(NULL, ',', &internal);
      tdbIdDetailTemp = ptr;
    } else if (strncasecmp(ptr, "RMS_INFO", 7) == 0) {
      ptr = str_tok(NULL, ',', &internal);
      cpuTemp = ptr;
      rmsInfo = TRUE;
    } else if (strncasecmp(ptr, "RMS_CHECK", 9) == 0) {
      ptr = str_tok(NULL, ',', &internal);
      cpuTemp = ptr;
      rmsInfo = TRUE;
  ...
  *cpu = -1;
  ...
  if (nodeNameTemp != NULL) {
    ...
    if (tempLen > MAX_SEGMENT_NAME_LEN) tempLen = MAX_SEGMENT_NAME_LEN;
  ...
  if (mergeTemp != NULL) {
    ...
    if (tempNum == SQLCLI_ACCUMULATED_STATS || tempNum == SQLCLI_PERTABLE_STATS || tempNum == SQLCLI_PROGRESS_STATS)
...
NABoolean ExOperStatsId::compare(const ExOperStatsId &other, ComTdb::CollectStatsType cst) const {
  switch (cst) {
    case ComTdb::ACCUMULATED_STATS:
    case ComTdb::PERTABLE_STATS:
    case ComTdb::OPERATOR_STATS:
    case ComTdb::PROGRESS_STATS:
      ...
      break;
    default:
      return (fragId_ == other.fragId_ && tdbId_ == other.tdbId_ && instNum_ == other.instNum_ &&
              subInstNum_ == other.subInstNum_);
  }
}

ExRMSStats::ExRMSStats(NAHeap *heap) : ExOperStats(heap, RMS_STATS) {
  rmsVersion_ = 0;
  nodeName_[0] = '\0';
  cpu_ = -1;
  sscpPid_ = -1;
  ...
  epochHeapAlloc_ = 0;
  epochHeapUsage_ = 0;
  epochHeapWatermark_ = 0;
  currNoOfEpochEntries_ = 0;
}
...
void ExRMSStats::getVariableStatsInfo(char *dataBuffer, char *dataLen, int maxLen) {
  ...
  const char *statusStr[3] = {"OK", "WARNING", "ERROR"};

  if (subReqType() == SQLCLI_STATS_REQ_RMS_CHECK) {
    sprintf(buf, "statsRowType: %d nodeId: %d nodeName: %s ", SQLCLI_RMS_CHECK_STATS, cpu_, nodeName_);
    ...
    if (tempCValue != NULL) statsHeapFreePercent = atoi(tempCValue);
    if (statsHeapFreePercent <= 0) statsHeapFreePercent = 40;
    if (((double)currGlobalStatsHeapUsage_ * 100 / (double)currGlobalStatsHeapAlloc_) > (100 - statsHeapFreePercent)) {
      sprintf(tmpbuf, "statsHeapUsed: %ld statsHeapTotal: %ld statsHeapWM: %ld ", currGlobalStatsHeapUsage_,
              currGlobalStatsHeapAlloc_, globalStatsHeapWatermark_);
      if (status < 1) status = 1;
      strcat(buf, tmpbuf);
    }
    if ((currNoOfProcessStatsHeap_ - currNoOfRegProcesses_) > 100) {
      sprintf(tmpbuf, "noOfProcessRegd: %d  noOfProcessStatsHeaps: %d ", currNoOfRegProcesses_,
    ...
    sprintf(
        ...
        "epochHeapUsed: %ld epochHeapTotal: %ld epochHeapWM: %ld noOfEpochEntries: %d ",
        statType(), rmsVersion_, nodeName_, cpu_, cpu_, sscpPid_, sscpPriority_, sscpTimestamp_, ssmpPid_,
        ssmpPriority_, ssmpTimestamp_, storeSqlSrcLen_, rmsEnvType_, currGlobalStatsHeapUsage_,
        currGlobalStatsHeapAlloc_, globalStatsHeapWatermark_, currNoOfRegProcesses_, currNoOfProcessStatsHeap_,
        currNoOfStmtStats_, semPid_, sscpOpens_, sscpDeletedOpens_, stmtStatsGCed_, lastGCTime_, totalStmtStatsGCed_,
        ssmpReqMsgCnt_, ssmpReqMsgBytes_, ssmpReplyMsgCnt_, ssmpReplyMsgBytes_, sscpReqMsgCnt_, sscpReqMsgBytes_,
        sscpReplyMsgCnt_, sscpReplyMsgBytes_, rmsStatsResetTimestamp_, numQueryInvKeys_, configuredPidMax_,
        epochHeapUsage_, epochHeapAlloc_, epochHeapWatermark_, currNoOfEpochEntries_);
  }
  buf += str_len(buf);
  *(short *)dataLen = (short)(buf - dataBuffer);
}

ExRMSStats *ExRMSStats::castToExRMSStats() { return this; }
...
int ExRMSStats::getStatsItem(SQLSTATS_ITEM *sqlStats_item) {
  ...
  switch (sqlStats_item->statsItem_id) {
    ...
    case SQLSTATS_RMS_CONFIGURED_PID_MAX:
      ...
      break;
    case SQLSTATS_EPOCH_HEAP_ALLOC:
      sqlStats_item->int64_value = epochHeapAlloc_;
      break;
    case SQLSTATS_EPOCH_HEAP_USED:
      sqlStats_item->int64_value = epochHeapUsage_;
      break;
    case SQLSTATS_EPOCH_HEAP_HIGH_WM:
      sqlStats_item->int64_value = epochHeapWatermark_;
      break;
    case SQLSTATS_EPOCH_ENTRIES:
      sqlStats_item->int64_value = currNoOfEpochEntries_;
      break;
    default:
      sqlStats_item->error_code = -EXE_STAT_NOT_FOUND;
      break;
...
ExBMOStats::ExBMOStats(NAMemory *heap) : ExOperStats(heap, BMO_STATS) {
  init(FALSE);
  spaceBufferSize_ = -1;
  scratchIOSize_ = -1;
  scratchOverflowMode_ = -1;
...
ExBMOStats::ExBMOStats(NAMemory *heap, StatType statType) : ExOperStats(heap, statType) {
  init(FALSE);
  spaceBufferSize_ = -1;
  scratchIOSize_ = -1;
  scratchOverflowMode_ = -1;
...
ExBMOStats::ExBMOStats(NAMemory *heap, StatType statType, ex_tcb *tcb, const ComTdb *tdb)
    : ExOperStats(heap, statType, tcb, tdb) {
  init(FALSE);
  spaceBufferSize_ = -1;
  scratchIOSize_ = -1;
...
ExBMOStats::ExBMOStats(NAMemory *heap, ex_tcb *tcb, const ComTdb *tdb)
    : ExOperStats(heap, BMO_STATS, tcb, tdb), timer_(CLOCK_MONOTONIC) {
  init(FALSE);
  spaceBufferSize_ = -1;
  scratchIOSize_ = -1;
  if (tdb != NULL) {
...
void ExBMOStats::getVariableStatsInfo(char *dataBuffer, char *dataLen, int maxLen) {
  ...
  sprintf(buf,
          ...
          statType(), getExplainNodeId(), getBmoPhaseStr(), interimRowCount_, estMemoryUsage_, bmoHeapUsage_,
          bmoHeapAlloc_, bmoHeapWM_, spaceBufferSize_, spaceBufferCount_, scratchOverflowMode_, scratchFileCount_,
          scratchBufferBlockSize_, scratchBufferBlockRead_, scratchBufferBlockWritten_, scratchWriteCount_,
          scratchReadCount_, topN_, scratchIOSize_, timer_.getTime(), scratchIOMaxTime_);
  buf += str_len(buf);
...
ExBMOStats *ExBMOStats::castToExBMOStats() { return this; }
...
void ExBMOStats::merge(ExBMOStats *other) {
  ...
  if (other->spaceBufferSize_ != -1) spaceBufferSize_ = other->spaceBufferSize_;
  spaceBufferCount_ += other->spaceBufferCount_;
  if (other->scratchBufferBlockSize_ != -1) scratchBufferBlockSize_ = other->scratchBufferBlockSize_;
  if (other->scratchIOSize_ != -1) scratchIOSize_ = other->scratchIOSize_;
  if (other->topN_ != -1) topN_ = other->topN_;
  if (other->phase_ > phase_) phase_ = other->phase_;
  if (other->estMemoryUsage_ > 0) estMemoryUsage_ = other->estMemoryUsage_;
  scratchOverflowMode_ = other->scratchOverflowMode_;
  scratchFileCount_ += other->scratchFileCount_;
...
int ExBMOStats::getStatsItem(SQLSTATS_ITEM *sqlStats_item) {
  ...
  switch (sqlStats_item->statsItem_id) {
    ...
    case SQLSTATS_DETAIL:
      if (sqlStats_item->str_value != NULL) {
        snprintf(sqlStats_item->str_value, sqlStats_item->str_max_len, "%ld|%ld|%ld|", getNumVal(2), getNumVal(3),
                 getNumVal(4));
        len = strlen(sqlStats_item->str_value);

        tmpBuf = sqlStats_item->str_value + len;
        // May not get anything if the list is empty.
        scanFilterStats_.getVariableStatsInfo(tmpBuf, sqlStats_item->str_max_len - len);
        len = strlen(sqlStats_item->str_value);
...
ExUDRBaseStats::ExUDRBaseStats(NAMemory *heap) : ExOperStats(heap, UDR_BASE_STATS) { init(FALSE); }

ExUDRBaseStats::ExUDRBaseStats(NAMemory *heap, StatType statType) : ExOperStats(heap, statType) { init(FALSE); }

ExUDRBaseStats::ExUDRBaseStats(NAMemory *heap, StatType statType, ex_tcb *tcb, const ComTdb *tdb)
    : ExOperStats(heap, statType, tcb, tdb) {
  init(FALSE);
}

ExUDRBaseStats::ExUDRBaseStats(NAMemory *heap, ex_tcb *tcb, const ComTdb *tdb)
...
ExUDRBaseStats *ExUDRBaseStats::castToExUDRBaseStats() { return this; }
...
void ExUDRBaseStats::merge(ExUDRBaseStats *other) {
  ...
  udrCpuTime_ += other->udrCpuTime_;
  // UDRServerId_ ??
  if (other->recentReqTS_ > recentReqTS_) recentReqTS_ = other->recentReqTS_;
...
//////////////////////////////////////////////////////////////////
ExFastExtractStats::ExFastExtractStats(NAMemory *heap, ex_tcb *tcb, const ComTdb *tdb)
    : ExOperStats(heap, FAST_EXTRACT_STATS, tcb, tdb),
      buffersCount_(0),
...
void ExFastExtractStats::getVariableStatsInfo(char *dataBuffer, char *dataLen, int maxLen) {
  ...
  sprintf(buf,
          ...
          buffersCount_, processedRowsCount_, errorRowsCount_, readyToSendBuffersCount_, sentBuffersCount_,
          partitionNumber_, bufferAllocFailuresCount_, readyToSendBytes_, sentBytes_);
  buf += str_len(buf);

  *(short *)dataLen = (short)(buf - dataBuffer);
}

ExProcessStats::ExProcessStats(NAMemory *heap) : ExOperStats(heap, PROCESS_STATS) {
  nid_ = 0;
  pid_ = 0;
  startTime_ = -1;
  numESPsStarted_ = 0;
...
ExProcessStats::ExProcessStats(NAMemory *heap, short nid, pid_t pid) : ExOperStats(heap, PROCESS_STATS) {
  nid_ = nid;
  pid_ = pid;
  startTime_ = -1;
  numESPsStarted_ = 0;
...
void ExProcessStats::copyContents(ExProcessStats *stat) {
  ...
  if (stat->recentSourceStr_) {
    setSourceStr(stat->recentSourceStr_, stat->strLen_);
  }
}
...
const char *ExProcessStats::getTextVal() { return ExOperStats::getTextVal(); }
...
UInt32 ExProcessStats::packedLength() {
  ...
  if (recentQid_ != NULL) size += qidLen_;
  if (recentSourceStr_ != NULL) size += strLen_;
  return size;
}
...
UInt32 ExProcessStats::pack(char *buffer) {
  ...
  if (recentSourceStr_ != NULL) {
    packedLen += packStrIntoBuffer(buffer, recentSourceStr_, strLen_);
  }
  return packedLen + srcLen;
}
...
void ExProcessStats::unpack(const char *&buffer) {
  ...
  if (strLen_ != 0) {
    ...
    unpackStrFromBuffer(buffer, recentSourceStr_, strLen_);
...
void ExProcessStats::getVariableStatsInfo(char *dataBuffer, char *dataLen, int maxLen) {
  ...
  sprintf(buf,
          ...
          "recentQid: %s sqlStr: %s",
          statType(), nid_, pid_, startTime_, (exeMemHighWM_ >> 20), (exeMemAlloc_ >> 20), (exeMemUsed_ >> 20),
          (ipcMemHighWM_ >> 20), (ipcMemAlloc_ >> 20), (ipcMemUsed_ >> 20), pfsSize_, pfsCurUse_, pfsMaxUse_,
          sqlOpenCount_, arkfsSessionCount_, staticStmtCount_, dynamicStmtCount_, openStmtCount_, closeStmtCount_,
          reclaimStmtCount_, numESPsStarted_, numESPsStartupCompleted_, numESPsBad_, numESPsDeleted_, numESPsInUse_,
          numESPsFree_, (recentQid_ != NULL ? recentQid_ : "NULL"),
          (recentSourceStr_ != NULL ? recentSourceStr_ : "NULL"));
  buf += str_len(buf);
  *(short *)dataLen = (short)(buf - dataBuffer);
}
...
///////////////////////////////////////////////////////////////////
// Methods for ExQryInvalidStatsTdb and ExQryInvalidStatsTcb
///////////////////////////////////////////////////////////////////
ex_tcb *ExQryInvalidStatsTdb::build(ex_globals *glob) {
  ExQryInvalidStatsTcb *qryInvalidTcb = new (glob->getSpace()) ExQryInvalidStatsTcb(*this, glob);

  qryInvalidTcb->registerSubtasks();

  return qryInvalidTcb;
}

ExQryInvalidStatsTcb::ExQryInvalidStatsTcb(const ExQryInvalidStatsTdb &qryInvalidTdb, ex_globals *glob)
    : ex_tcb(qryInvalidTdb, 1, glob) {
  Space *space = (glob ? glob->getSpace() : 0);
  CollHeap *heap = (glob ? glob->getDefaultHeap() : 0);
  ...
  // Allocate the queues used to communicate with parent
  qparent_.down =
      new (space) ex_queue(ex_queue::DOWN_QUEUE, qryInvalidTdb.queueSizeDown_, qryInvalidTdb.criDescDown_, space);

  // Allocate the private state in each entry of the down queue
  ExQIStatsPrivateState *p = new (space) ExQIStatsPrivateState();
  qparent_.down->allocatePstate(p, this);
  delete p;

  qparent_.up = new (space) ex_queue(ex_queue::UP_QUEUE, qryInvalidTdb.queueSizeUp_, qryInvalidTdb.criDescUp_, space);

  pool_->get_free_tuple(tuppData_, qryInvalidTdb.tupleLen_);
  data_ = tuppData_.getDataPointer();

  if (qryInvalidTdb.workCriDesc_) {
    workAtp_ = allocateAtp(qryInvalidTdb.workCriDesc_, glob->getSpace());
  ...
  // fixup expressions
  if (qryInvalidTdb.scanExpr_)
    (void)qryInvalidTdb.scanExpr_->fixup(0, getExpressionMode(), this, space, heap, FALSE, glob);

  if (qryInvalidTdb.inputExpr_)
    (void)qryInvalidTdb.inputExpr_->fixup(0, getExpressionMode(), this, space, heap, FALSE, glob);

  if (qryInvalidTdb.projExpr_)
    (void)qryInvalidTdb.projExpr_->fixup(0, getExpressionMode(), this, space, heap, FALSE, glob);
...
ExQryInvalidStatsTcb::~ExQryInvalidStatsTcb() {
  delete qparent_.up;
  delete qparent_.down;
  freeResources();
}

void ExQryInvalidStatsTcb::freeResources() {
  if (stats_ != NULL && deleteStats()) {
    NADELETE(stats_, ExStatisticsArea, stats_->getHeap());
    stats_ = NULL;
    setDeleteStats(FALSE);
...
short ExQryInvalidStatsTcb::work() {
  // if no parent request, return
  if (qparent_.down->isEmpty()) return WORK_OK;

  ex_queue_entry *pentry_down = qparent_.down->getHeadEntry();
  ExQIStatsPrivateState *pstate = (ExQIStatsPrivateState *)pentry_down->pstate;
  NABoolean found = FALSE;
  char *stmtName = NULL;
  ExStatisticsArea *stats = NULL;
  CliGlobals *cliGlobals = getGlobals()->castToExExeStmtGlobals()->castToExMasterStmtGlobals()->getCliGlobals();
    ...
    switch (pstate->step_) {
      case INITIAL_: {
        ...
        if (statsTdb().getInputExpr()) {
          ...
          evalRetCode = statsTdb().getInputExpr()->eval(pentry_down->getAtp(), workAtp_);
          if (evalRetCode == ex_expr::EXPR_ERROR) {
            ex_assert(0, "ExQryInvalidStatsTcb::work(). Error from inputExpr_->eval()");
          }
          ...
          if (stmtName && stmtAttr->getLength() != 0) {
            inputStmtName_ = new (getHeap()) char[stmtAttr->getLength() + 1];
            str_cpy_all(inputStmtName_, stmtName, stmtAttr->getLength());
            inputStmtName_[stmtAttr->getLength()] = 0;
            // remove trailing blanks
            str_cpy_and_null(inputStmtName_, inputStmtName_, stmtAttr->getLength(), '\0', ' ');
            parse_stmt_name(inputStmtName_, str_len(inputStmtName_));
          } else {
            cpu_ = cliGlobals->myCpu();
          }

          if (cpu_ == cliGlobals->myCpu()) {
            pstate->step_ = GET_LOCAL_QI_STATS_ENTRY_;
          } else if (cpu_ >= 0) {
            pstate->step_ = SEND_TO_SSMP_;
          } else {
            IpcAllocateDiagsArea(diagsArea_, getHeap());
      ...
      case APPLY_SCAN_EXPR_: {
        ...
        if (statsTdb().scanExpr_) {
          ...
          if (evalRetCode == ex_expr::EXPR_FALSE)
            ...
            ex_assert(0, "ExQryInvalidStatsTcb::work(). Error from scanExpr_->eval()");
            rowSelected = FALSE;
          }
        }
...
void ExQryInvalidStatsTcb::getColumnValues(ExOperStats *operStat) {
  ExpTupleDesc *tDesc = statsTdb().workCriDesc_->getTupleDescriptor(statsTdb().getStatsTupleAtpIndex());
  ...
  for (UInt32 i = 0; i < tDesc->numAttrs(); i++) {
    ...
    switch (i) {
      case STAT_NID:
        src = (char *)&int32Val;
        int32Val = stat->getCpu();
        srcLen = 4;
        srcType = REC_BIN32_SIGNED;
        break;

      case STAT_OPERATION:
        src = CONST_CAST(char *, stat->getOperation());
        srcLen = 2;
        srcType = REC_BYTE_F_ASCII;
        break;

      case STAT_OBJECT_UID:
        int64Val = stat->getObjectUid();
        break;

      case STAT_SUBJECT_HASH:
        src = (char *)&uint32Val;
        uint32Val = stat->getSubjectHash();
        srcLen = 4;
        srcType = REC_BIN32_UNSIGNED;
        break;

      case STAT_OBJECT_HASH:
        src = (char *)&uint32Val;
        uint32Val = stat->getObjectHash();
        srcLen = 4;
        srcType = REC_BIN32_UNSIGNED;
        break;

      case STAT_REVOKE_TIME:
        int64Val = stat->getRevokeTime();
        break;

      default:
        ex_assert(0, "bad case index in ExQryInvalidStatsTcb::work");
        break;
    }  // switch i

    if (::convDoIt(src, srcLen, srcType, 0, 0, &data_[attr->getOffset()], attr->getLength(), attr->getDatatype(), 0, 0,
                   0, 0, NULL) != ex_expr::EXPR_OK) {
      ex_assert(0, "Error from ExQryInvalidStatsTcb::work::convDoIt.");
    }
  }  // for i
}

ExStatisticsArea *ExQryInvalidStatsTcb::sendToSsmp() {
  ExStatisticsArea *stats = NULL;
  CliGlobals *cliGlobals = getGlobals()->castToExExeStmtGlobals()->castToExMasterStmtGlobals()->getCliGlobals();
  if (cliGlobals->getStatsGlobals() == NULL) {
    IpcAllocateDiagsArea(diagsArea_, getHeap());
  ...
  qiStatsReq = new (cliGlobals->getIpcHeap()) QueryInvalidStatsRequest(cliGlobals->getIpcHeap());
  *ssmpMsgStream << *qiStatsReq;
...
void ExQryInvalidStatsTcb::parse_stmt_name(char *string, int len) {
  ...
  char *ptr = str_tok(temp, '=', &internal);

  if (ptr == NULL) return;

  if (strncasecmp(ptr, "CPU", 3) == 0 || strncasecmp(ptr, "NODE", 4) == 0) {
    ptr = str_tok(NULL, ',', &internal);
    cpu_ = str_atoi(ptr, str_len(ptr));
  }
  return;
}
