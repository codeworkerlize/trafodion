...
#define ex_assert_both_sides(assert_test, assert_msg)               \
  if (!(assert_test)) {                                             \
    if (connection_) connection_->dumpAndStopOtherEnd(true, false); \
...
ex_tcb *ex_send_top_tdb::buildInstance(ExExeStmtGlobals *glob, int myInstanceNum, int childInstanceNum) {
  ...
  result = new (glob->getSpace()) ex_send_top_tcb(*this, glob, sendBottomProcId, myInstanceNum, childInstanceNum);
...
ex_send_top_tcb::ex_send_top_tcb(const ex_send_top_tdb &sendTopTdb, ExExeStmtGlobals *glob,
  ...
  ExEspInstanceThread *threadInfo = glob->getThreadInfo();
  ...
  if (masterGlob != NULL && sendTopTdb.getExtractProducerFlag()) masterGlob->insertExtractEsp(sendBottomProcId);

  // Allocate the queues to communicate with parent
  qParent_.down = new (space) ex_queue(ex_queue::DOWN_QUEUE, sendTopTdb.queueSizeDown_, sendTopTdb.criDescDown_, space);

  // Allocate the private state in each entry of the down queue
  ex_send_top_private_state *p = new (space) ex_send_top_private_state(this);
  qParent_.down->allocatePstate(p, this);
  delete p;

  qParent_.up = new (space) ex_queue(ex_queue::UP_QUEUE, sendTopTdb.queueSizeUp_, sendTopTdb.criDescUp_, space);

  workAtp_ = allocateAtp(sendTopTdb.workCriDesc_, space);

  // fixup expressions
  if (moveInputValues())
    (void)moveInputValues()->fixup(0, getExpressionMode(), this, glob->getSpace(), glob->getDefaultHeap(), FALSE, glob);
  ...
  IpcMessageObjSize maxBufSize = sendBufferSize_ > receiveBufferSize_ ? sendBufferSize_ : receiveBufferSize_;
  maxBufSize += (sizeof(TupMsgBuffer) + sizeof(ExEspInputDataReqHeader) + sizeof(ExEspOpenReqHeader));
  ...
  msgStream_ = new (glob->getSpace())
      ExSendTopMsgStream(glob, getNumSendBuffers(), getNumRecvBuffers(), maxBufSize, this, threadInfo);

  if (sendTopTdb.getExchangeUsesSM()) msgStream_->setSMContinueProtocol(TRUE);
...
/////////////////////////////////////////////////////////////////////////////
// destructor
ex_send_top_tcb::~ex_send_top_tcb() { freeResources(); }
...
ExWorkProcRetcode ex_send_top_tcb::sCancel(ex_tcb *tcb) { return ((ex_send_top_tcb *)tcb)->processCancel(); }
...
int ex_send_top_tcb::fixup() {
  if (sendTopTdb().getExchangeUsesSM()) {
    ...
    EXSM_TRACE(EXSM_TRACE_INIT | EXSM_TRACE_TAG, "SNDT  frag %d inst %d", myFrag, myInstNum);
    EXSM_TRACE(EXSM_TRACE_INIT | EXSM_TRACE_TAG, "SNDT  child frag %d inst %d", childFrag, childInstNum);

    // regress/executor/TEST123 expects this line of output when
    // tracing is reduced to tags only
    EXSM_TRACE(EXSM_TRACE_INIT | EXSM_TRACE_TAG, "SNDT  tgt %d:%d:%" PRId64 ":%d", (int)smTarget_.node,
               (int)smTarget_.pid, smTarget_.id, (int)smTarget_.tag);

    EXSM_TRACE(EXSM_TRACE_INIT, "SNDT  work stream %p", msgStream_);
    EXSM_TRACE(EXSM_TRACE_INIT, "SNDT  cancel stream %p", cancelMessageStream_);
    EXSM_TRACE(EXSM_TRACE_INIT, "SNDT  send limit %d, in use limit %d", (int)msgStream_->getSendBufferLimit(),
               (int)msgStream_->getInUseBufferLimit());
    EXSM_TRACE(EXSM_TRACE_INIT, "SNDT  send size %d", (int)sendBufferSize_);
    EXSM_TRACE(EXSM_TRACE_INIT, "SNDT  recv size %d", (int)receiveBufferSize_);
    EXSM_TRACE(EXSM_TRACE_INIT, "SNDT  stream max buf %d", (int)msgStream_->getBufferSize());
    EXSM_TRACE(EXSM_TRACE_INIT, "SNDT  send bufs %d recv bufs %d", (int)getNumSendBuffers(), (int)getNumRecvBuffers());
    EXSM_TRACE(EXSM_TRACE_INIT, "SNDT  row len %d down, %d up", downRowLen, upRowLen);
    EXSM_TRACE(EXSM_TRACE_INIT, "SNDT  down queue %d, up queue %d", downQueueLen, upQueueLen);
...
short ex_send_top_tcb::work() {
  ...
  if (ipcBroken_) {
    ...
    result = checkReceive();
    EXSM_TRACE(EXSM_TRACE_WORK, "SNDT %p checkReceive rc %s", this, ExWorkProcRetcodeToString(result));

    if (result == WORK_OK) {
      result = checkSend();
      EXSM_TRACE(EXSM_TRACE_WORK, "SNDT %p checkSend rc %s", this, ExWorkProcRetcodeToString(result));
    }

    if (sendTopState_ != WAITING_FOR_OPEN_COMPLETION) {
      if (result == WORK_OK) {
        result = continueRequest();
        EXSM_TRACE(EXSM_TRACE_WORK, "SNDT %p continueRequest rc %s", this, ExWorkProcRetcodeToString(result));
      }
      ...
      if (qParent_.down->isEmpty()) {
        ...
        if (sendTopTdb().getExchangeUsesSM()) {
          ...
          EXSM_TRACE(EXSM_TRACE_WORK, "SNDT %p glob msgs %d cancels %d", this, (int)glob->numSendTopMsgesOut(),
                     (int)glob->numCancelMsgesOut());
          if (msgStream_)
            EXSM_TRACE(EXSM_TRACE_WORK, "SNDT %p data resp pend %d", this, (int)msgStream_->numOfResponsesPending());
  ...
  EXSM_TRACE(EXSM_TRACE_WORK, "SNDT %p END WORK %s %s", this, ExWorkProcRetcodeToString(result),
...
short ex_send_top_tcb::checkReceive() {
  ...
  while (NOT qParent_.up->isFull()) {  // process data from send bottom
    if (ipcBroken_) return WORK_BAD_ERROR;
    ...
    if (qParent_.down->isEmpty()) {
      ...
      do {
        if (currentReceiveBuffer_) {
          SqlBuffer *sqlBuf = currentReceiveBuffer_->get_sql_buffer();
          ex_assert_both_sides(sqlBuf->atEOTD(), "Receiving extra rows from send bottom");
        }
    ...
    ex_send_top_private_state &pstate = *((ex_send_top_private_state *)rentry->pstate);
    ...
    if (currentReceiveBuffer_ == NULL) {
      ...
      EXSM_TRACE(EXSM_TRACE_WORK, "SNDT %p tupps arrived %d", this, (int)sb->getTotalTuppDescs());
    ...
    while (NOT qParent_.up->isFull()) {  // move data from sql buffer to up queue
      ...
      currBufferIsEmpty = currSqlBuffer->moveOutSendOrReplyData(FALSE, &(pentry->upState), nextTupp, &ci,
      ...
      if (ci->getIsExtDiagsAreaPresent()) {
        ...
        ex_assert_both_sides(msgStream_->getNextObjType(msgType) AND msgType == IPC_SQL_DIAG_AREA,
      ...
      if ((rentry->downState.request == ex_queue::GET_NOMORE) && (pentry->upState.status != ex_queue::Q_NO_DATA)) {
      ...
      if (ci->getIsExtStatsAreaPresent()) {
        IpcMessageObjType msgType;
        ex_assert_both_sides(msgStream_->getNextObjType(msgType) AND msgType == IPC_SQL_STATS_AREA,
                             "no stats area in message");

        CollHeap *defaultHeap = getGlobals()->getDefaultHeap();

        ExStatisticsArea *statArea =
            new (defaultHeap) ExStatisticsArea(defaultHeap, 0, ((ComTdb *)getTdb())->getCollectStatsType());

        *msgStream_ >> *statArea;
        if (getGlobals()->statsEnabled()) mergeStats(statArea);
      ...
      if (pentry->upState.status == ex_queue::Q_NO_DATA) {
        ...
        ex_assert_both_sides((pstate.step_ == STARTED_) || (pstate.step_ == CANCELED_AFTER_SENT_),
    ...
    EXSM_TRACE(EXSM_TRACE_WORK, "SNDT %p queue entries returned: %d", this, numRowsReturned);
    if (qParent_.up->isFull()) EXSM_TRACE(EXSM_TRACE_WORK, "SNDT %p up queue is full", this);
...
TupMsgBuffer *ex_send_top_tcb::getReceiveBuffer() {
  ...
  while (msgStream_->getNextReceiveMsg(msgType)) {
    ex_assert_both_sides(msgType == IPC_MSG_SQLESP_DATA_REPLY, "received message from unknown message stream");

    while (msgStream_->getNextObjType(msgType)) {
      if (msgType == ESP_RETURN_STATUS_HDR) {  // reply to open message, save child int
        ...
        ExEspReturnStatusReplyHeader *statHdr =
            new (msgStream_->receiveMsgObj()) ExEspReturnStatusReplyHeader(msgStream_);
      ...
      // check for a diagnostics area, returned from server during OPEN
        ...
        if (objSize == sizeof(IpcMessageObj)) {
          ...
          ex_assert_both_sides(packedObj, "error receiving diags area from unbuffered stream");
          diagsArea->unpackObj(packedObj->getType(), packedObj->getVersion(), TRUE, packedObj->getObjLength(),
        ...
        if (getGlobals()->castToExExeStmtGlobals()->getDiagsArea()) {
          getGlobals()->castToExExeStmtGlobals()->getDiagsArea()->mergeAfter(*diagsArea);
          // clean up
          diagsArea->deAllocate();
        } else {
          getGlobals()->castToExExeStmtGlobals()->setGlobDiagsArea(diagsArea);
          diagsArea->decrRefCount();
        }
      } else if (msgType == ESP_RETURN_DATA_HDR) {  // reply data header, get sql buffer and return for processing
        ExEspReturnDataReplyHeader *dataHdr = new (msgStream_->receiveMsgObj()) ExEspReturnDataReplyHeader(msgStream_);

        ex_assert_both_sides(msgStream_->getNextObjType(msgType) AND msgType == ESP_OUTPUT_SQL_BUFFER,
        ...
        TupMsgBuffer *buffer = new (msgStream_->receiveMsgObj()) TupMsgBuffer(msgStream_);
...
short ex_send_top_tcb::checkSend() {
  ...
  if (sendTopState_ == SERVER_SATURATED OR sendTopState_ == WAITING_FOR_OPEN_REPLY)
    return WORK_OK;  // send continue messages only

  while (qParent_.down->entryExists(nextToSendDown_)) {  // build sql buffers and send to send bottom
    if (ipcBroken_) return WORK_BAD_ERROR;
    ...
    ex_send_top_private_state &pstate = *((ex_send_top_private_state *)pentry->pstate);

    ex_assert(pstate.step_ == NOT_STARTED_, "Message shoulda been NOT_STARTED_");

    if (espGlobals) espGlobals->setSendTopTcbActivated(mySendTopTcbIndex_);
    ...
    if (msgStream_->sendLimitReached()) return WORK_OK;
    if (msgStream_->inUseLimitReached()) return WORK_POOL_BLOCKED;
    ...
    do  // fill sql buffer with data from down queue
    {
      ...
      //
      tupp_descriptor *controlInfoAsTupp = sqlBuf->add_tuple_desc((unsigned short)sizeof(ControlInfo));

      NABoolean isInputRowToBeSent = (moveInputValues() != NULL);

      tupp_descriptor *rowToSend =
          isInputRowToBeSent ? sqlBuf->add_tuple_desc(sendTopTdb().getDownRecordLength()) : NULL;

      assert(!isInputRowToBeSent || (rowToSend != NULL));

      NABoolean isDiagsAreaToBeSent = pentry->getDiagsArea() != NULL;
      //
      // Check that there was sufficient space in buffer.
      //
      sendSpaceAvailable = (controlInfoAsTupp != NULL) AND(!isInputRowToBeSent OR rowToSend != NULL);
      if (!sendSpaceAvailable) {
        ...
        str_cpy_all(controlInfoAsTupp->getTupleAddress(), (char *)&msgControlInfo, sizeof(ControlInfo));
        ...
        if (isInputRowToBeSent) {
          ...
          // in the message buffer.
          //
          workAtp_->getTupp((unsigned short)sendTopTdb().moveExprTuppIndex_) = rowToSend;

          ex_expr::exp_return_type retval = moveInputValues()->eval(pentry->getAtp(), workAtp_);

          ex_assert(retval != ex_expr::EXPR_ERROR, "Add error handling");

          // release the tupp_descriptor in the buffer
          workAtp_->getTupp((unsigned short)sendTopTdb().moveExprTuppIndex_) = NULL;
        }
        //
        // If there is a diagnostics area to send, then add
        // it to the message following the sql buffer.
        //
        if (isDiagsAreaToBeSent) {  // construct a copy of ComDiagsArea in message
    ...
    } while (sendSpaceAvailable AND qParent_.down->entryExists(nextToSendDown_));

    // send sql buffer even if not completely full
    ex_assert(NOT currentSendBuffer_->get_sql_buffer()->isEmpty(), "Send top's sql buffer is empty");
    ...
    if ((statsEntry = getStatsEntry()) != NULL) {
      ...
      if (stats) {
        stats->bufferStats()->totalSentBytes() += sqlBuf->getSendBufferSize();
    ...
    EXSM_TRACE(EXSM_TRACE_WORK, "SNDT %p SEND data %d tupps", this, (int)sqlBuf->getTotalTuppDescs());
...
TupMsgBuffer *ex_send_top_tcb::getSendBuffer() {
  if (sendTopState_ == NOT_OPENED || sendTopState_ == WAITING_FOR_OPEN_COMPLETION) {
    ...
    if (sendTopState_ == NOT_OPENED) {
      if (!sendTopTdb().getExtractConsumerFlag()) {
        if (masterGlob AND masterGlob->getRtFragTable() AND masterGlob->getRtFragTable()->getState() !=
            ExRtFragTable::READY)
    ...
    EXSM_TRACE(EXSM_TRACE_BUFFER, "SNDT %p about to allocate msg buffer", this);

    msgStream_->addRecipient(connection_);

    // construct open request in message,
    // ask child ESP to respond with int
    ExEspOpenReqHeader *openReq = new (*msgStream_) ExEspOpenReqHeader((NAMemory *)NULL);
    ...
    if (sendTopTdb().getExtractConsumerFlag()) {
      ...
      IpcEnvironment *ipcEnv = getGlobals()->castToExExeStmtGlobals()->getIpcEnvironment();
  ...
  } else if ((sendTopState_ == OPEN_COMPLETE) && (msgStream_->getRecipients().isEmpty())) {
  ...
  ExEspInputDataReqHeader *hdr = new (*msgStream_) ExEspInputDataReqHeader((NAMemory *)NULL);
...
#ifdef _DEBUG
  if (getenv("TEST_ERROR_AT_QUEUE_NO_ESP")) hdr->injectErrorAtQueueFreq_ = 0;
#endif

  // construct sql buffer(TupMsgBuffer) directly in message to avoid copy
  TupMsgBuffer *result =
      new (*msgStream_, sendBufferSize_) TupMsgBuffer(sendBufferSize_, TupMsgBuffer::MSG_IN, msgStream_);
  if (result == NULL) ABORT("mismatched send top buffer size");
...
short ex_send_top_tcb::continueRequest() {
  if (qParent_.down->isEmpty() OR sendTopState_ == WAITING_FOR_OPEN_REPLY OR sendTopState_ ==
      NOT_OPENED OR sendTopState_ == WAITING_FOR_OPEN_COMPLETION)
    return WORK_OK;
  ...
  while (NOT msgStream_->sendLimitReached()) {
    ...
    if (msgStream_->inUseLimitReached()) {
      EXSM_TRACE(EXSM_TRACE_CONTINUE, "SNDT %p CREQ in use limit reached, rp %d", this,
                 (int)msgStream_->numOfResponsesPending());
    ...
    // include child int for fast routing
    ExEspContinueReqHeader *hdr = new (*msgStream_) ExEspContinueReqHeader((NAMemory *)NULL);
    if (hdr == NULL) return WORK_POOL_BLOCKED;
    hdr->handle_ = childFragHandle_;
    hdr->myInstanceNum_ = getMyInstanceNum();

    EXSM_TRACE(EXSM_TRACE_CONTINUE, "SNDT %p CREQ sending continue", this);
...
void ex_send_top_tcb::checkCancelReply() {
  if (!cancelMessageStream_) {
    ...
    while (cancelMessageStream_->getNextReceiveMsg(msgType)) {
      ex_assert_both_sides(msgType == IPC_MSG_SQLESP_CANCEL_REPLY, "received message from unknown message stream");
      ...
      while (cancelMessageStream_->getNextObjType(msgType)) {
        ...
        EXSM_TRACE(EXSM_TRACE_CANCEL, "SNDT %p recv ESP_RETURN_CANCEL_HDR", this);
        EXSM_TRACE(EXSM_TRACE_CANCEL, "SNDT %p state %s", this, getExSendTopStateString(sendTopState_));

        ExEspCancelReplyHeader *dataHdr =
            new (cancelMessageStream_->receiveMsgObj()) ExEspCancelReplyHeader(cancelMessageStream_);
...
ExWorkProcRetcode ex_send_top_tcb::processCancel() {
  ...
  if (sendTopTdb().getExchangeUsesSM()) {
    ExExeStmtGlobals *glob = getGlobals()->castToExExeStmtGlobals();
    EXSM_TRACE(EXSM_TRACE_CANCEL, "SNDT %p glob msgs %d cancels %d", this, (int)glob->numSendTopMsgesOut(),
               (int)glob->numCancelMsgesOut());
    if (cancelMessageStream_)
      EXSM_TRACE(EXSM_TRACE_CANCEL, "SNDT %p cancel resp pend %d", this,
                 (int)cancelMessageStream_->numOfResponsesPending());
    EXSM_TRACE(EXSM_TRACE_CANCEL, "SNDT %p END processCancel rc %s", this, ExWorkProcRetcodeToString(result));
  }
...
ExWorkProcRetcode ex_send_top_tcb::processCancelHelper() {
  ...
  while (pindex != nextToSendDown_) {
    ...
    if ((pentry->downState.request == ex_queue::GET_NOMORE) && (pstate.step_ == STARTED_)) {
      // Am I being asked to refrain from new I/O?
      if (getGlobals()->castToExExeStmtGlobals()->noNewRequest()) {
        ExEspStmtGlobals *espGlobals = getGlobals()->castToExExeStmtGlobals()->castToExEspStmtGlobals();
        if (espGlobals) {
        ...
        EXSM_TRACE(EXSM_TRACE_CANCEL, "SNDT %p must refrain from new I/O", this);
      ...
      if (cancelSendBuffer == NULL) {
        if (cancelMessageStream_)
          if (cancelMessageStream_->sendLimitReached() || cancelMessageStream_->inUseLimitReached()) {
            EXSM_TRACE(EXSM_TRACE_CANCEL, "SNDT %p stream limits prevent new I/O", this);
        ...
        if (cancelSendBuffer == NULL) {
          EXSM_TRACE(EXSM_TRACE_CANCEL, "SNDT %p send buffer not available", this);
      ...
      if (controlInfoAsTupp == NULL) {
        ...
        EXSM_TRACE(EXSM_TRACE_CANCEL, "SNDT %p no room in buffer for ControlInfo", this);
  ...
  if (cancelSendBuffer) {
    ...
    EXSM_TRACE(EXSM_TRACE_CANCEL, "SNDT %p SEND cancel %d tupps", this, (int)sqlBuf->getTotalTuppDescs());

    cancelMessageStream_->sendRequest();
  } else {
    if (sendTopTdb().getExchangeUsesSM() && pindex == nextToSendDown_)
      EXSM_TRACE(EXSM_TRACE_CANCEL, "SNDT %p no outstanding entries to cancel", this);
  }
...
short ex_send_top_tcb::createSMConnection() {
  ...
  ExMasterStmtGlobals *masterGlobals = getGlobals()->castToExExeStmtGlobals()->castToExMasterStmtGlobals();

  connection_ =
      new (env->getHeap()) SMConnection(env, smTarget_, numBufs, msgStream_->getBufferSize(), this, masterGlobals);

  EXSM_TRACE(EXSM_TRACE_PROTOCOL, "SNDT %p created conn %p", this, connection_);
  ex_assert(connection_, "SM connection allocation failed");

  SMConnection *smConn = (SMConnection *)connection_->castToSMConnection();
...
short ex_send_top_tcb::createIpcGuardianConnection(NABoolean nowaitedCompleted) {
  ...
  if (sendTopState_ == NOT_OPENED)  // added
  {
    ...
    if (!sv_env_multiple_fragments_checked) {
      ...
      if ((lv_espMultipleFragmentsEnvvar != NULL) && (*lv_espMultipleFragmentsEnvvar == '0')) {
    ...
    connection_ =
        bottomProcId_.createConnectionToServer(env,
                                               FALSE,  // transactions are sent via the control connection
                                               nowaitDepth, espParallelDcOpens, ioSubtask_->getScheduledAddr(),
  ...
  if (connection_ == NULL || (connection_ && connection_->getErrorInfo())) {
    ComDiagsArea *da = getGlobals()->castToExExeStmtGlobals()->getDiagsArea();
    ...
    if (connection_ == NULL) {
      if (sendTopTdb().getExtractConsumerFlag()) {
        ...
        *da << DgSqlCode(-EXE_PARALLEL_EXTRACT_CONNECT_ERROR) << DgString0(stringForDiags);
      } else {
...
/////////////////////////////////////////////////////////////////////////////
//
ex_queue_pair ex_send_top_tcb::getParentQueue() const { return qParent_; }
...
ExOperStats *ex_send_top_tcb::doAllocateStatsEntry(CollHeap *heap, ComTdb *tdb) {
  ...
  if (tdb->getCollectStatsType() == ComTdb::OPERATOR_STATS) {
    ...
    // a split top node).
    stat = getGlobals()->getStatsArea()->get(ExOperStats::EX_OPER_STATS, tdb->getTdbId());
    if (stat) {
      setStatsEntry(stat);
      stat->incDop();
      return NULL;
    ...
    stat = new (heap) ExESPStats(heap, sendTopTdb().getSendBufferSize(), sendTopTdb().getRecvBufferSize(),
                                 getChildInstanceNum(), this, tdb);
  }
  if (stat) {
...
short ex_send_top_tcb::notifyProducerThatWeCanceled() {
  ...
  if (connection_ == NULL)
    if (createConnectionToSendBottom(TRUE) != 0) return -1;
  ...
  ex_assert(espGlobals, "Master does not use notifyProducerThatWeCanceled() right now");

  // make sure we wait for the late cancel to complete
  espGlobals->setSendTopTcbLateCancelling();

  if (sendTopState_ == NOT_OPENED OR sendTopState_ == WAITING_FOR_OPEN_COMPLETION)
...
TupMsgBuffer *ex_send_top_tcb::getCancelSendBuffer(NABoolean lateCancel) {
  ...
  if (!cancelMessageStream_) {
    ...
    ExEspInstanceThread *threadInfo = getGlobals()->castToExExeStmtGlobals()->getThreadInfo();
    ...
    maxBufSize += MAXOF(sizeof(ExEspCancelReqHeader), sizeof(ExEspLateCancelReqHeader));
    IpcMessageBuffer::alignOffset(maxBufSize);
    ...
    ex_assert(!cancelMessageStream_->sendLimitReached() && !cancelMessageStream_->inUseLimitReached(),
              "getCancelSendBuffer called at the wrong time!");
  }

  // construct cancel request header in message,
  if (lateCancel) {
    ExEspLateCancelReqHeader *hdr = new (*cancelMessageStream_) ExEspLateCancelReqHeader((NAMemory *)NULL);
    if (hdr == NULL) return NULL;
    // a late cancel message may not know the int yet
    hdr->key_ = getGlobals()->castToExExeStmtGlobals()->getFragmentKey(sendTopTdb().getChildFragId());
    hdr->myInstanceNum_ = getMyInstanceNum();
  } else {
    // include child int for fast routing
    ExEspCancelReqHeader *hdr = new (*cancelMessageStream_) ExEspCancelReqHeader((NAMemory *)NULL);
    if (hdr == NULL) return NULL;
    hdr->handle_ = childFragHandle_;
    hdr->myInstanceNum_ = getMyInstanceNum();
  }
  ...
  if (lateCancel) sqlBufferLen = MAXOF(sqlBufferLen, 500);
  TupMsgBuffer *result =
      new (*cancelMessageStream_, sqlBufferLen) TupMsgBuffer(sqlBufferLen, TupMsgBuffer::MSG_IN, cancelMessageStream_);
  if (result == NULL) ABORT("mismatched send top buffer size");
...
/////////////////////////////////////////////////////////////////////////////
void ex_send_top_tcb::setStState(enum ExSendTopState newState, int linenum) {
  if (newState != sendTopState_) {
    if (stTidx_ >= NumSendTopTraceElements) stTidx_ = 0;
...
const char *ex_send_top_tcb::getExSendTopStateString(ExSendTopState s) {
  switch (s) {
    ...
    case NOT_OPENED:
      return "NOT_OPENED";
    case WAITING_FOR_OPEN_COMPLETION:
      return "WAITING_FOR_OPEN_COMPLETION";
    case WAITING_FOR_OPEN_REPLY:
      return "WAITING_FOR_OPEN_REPLY";
    case CANCELED_BEFORE_OPENED:
      return "CANCELED_BEFORE_OPENED";
    case OPEN_COMPLETE:
...
ExSendTopMsgStream::ExSendTopMsgStream(ExExeStmtGlobals *glob, int sendBufferLimit, int inUseBufferLimit,
    ...
    : IpcClientMsgStream(glob->getIpcEnvironment(), IPC_MSG_SQLESP_DATA_REQUEST, CurrEspRequestMessageVersion,
                         sendBufferLimit, inUseBufferLimit, bufferSize, threadInfo),
      sendTopTcb_(sendTopTcb) {}

///////////////////////////////////////////////////////////////////////////////
// method called upon send complete
void ExSendTopMsgStream::actOnSend(IpcConnection *connection) {
  EXSM_TRACE(EXSM_TRACE_PROTOCOL, "SNDT %p ACTS D s %p c %p", sendTopTcb_, this, connection);
  ...
  if (connection) {
    ...
    if (getErrorInfo()) {
      ComDiagsArea *da = sendTopTcb_->getGlobals()->castToExExeStmtGlobals()->getDiagsArea();
      ComDiagsArea *oldDa = da;
      connection->populateDiagsArea(da, sendTopTcb_->getHeap());
      if ((!oldDa) && (da)) {
        sendTopTcb_->getGlobals()->castToExExeStmtGlobals()->setGlobDiagsArea(da);
        da->decrRefCount();
      }
  ...
  ExExeStmtGlobals *stmtGlob = sendTopTcb_->getGlobals()->castToExExeStmtGlobals();
  stmtGlob->incrementSendTopMsgesOut();

  EXSM_TRACE(EXSM_TRACE_PROTOCOL, "SNDT %p ACTS D glob msgs %d", sendTopTcb_, (int)stmtGlob->numSendTopMsgesOut());
}

/////////////////////////////////////////////////////////////////////////////
// method called upon receive complete
void ExSendTopMsgStream::actOnReceive(IpcConnection *connection) {
  EXSM_TRACE(EXSM_TRACE_PROTOCOL, "SNDT %p ACTR D s %p c %p", sendTopTcb_, this, connection);
  ...
  if (getSMContinueProtocol()) {
    if (getSMBatchIsComplete()) stmtGlob->decrementSendTopMsgesOut();
  } else {
    stmtGlob->decrementSendTopMsgesOut();
  }

  EXSM_TRACE(EXSM_TRACE_PROTOCOL, "SNDT %p ACTR D glob msgs %d rp %d", sendTopTcb_, (int)stmtGlob->numSendTopMsgesOut(),
...
void ExSendTopCancelMessageStream::actOnSend(IpcConnection *connection) {
  EXSM_TRACE(EXSM_TRACE_PROTOCOL, "SNDT %p ACTS C %p c %p", sendTopTcb_, this, connection);
  ...
  EXSM_TRACE(EXSM_TRACE_PROTOCOL, "SNDT %p ACTS C glob cancel %d", sendTopTcb_, (int)stmtGlob->numCancelMsgesOut());
}

void ExSendTopCancelMessageStream::actOnReceive(IpcConnection *connection) {
  EXSM_TRACE(EXSM_TRACE_PROTOCOL, "SNDT %p ACTR %s %p c %p", sendTopTcb_, (lateCancel_ ? "LC" : "C"), this, connection);
  ...
  if (lateCancel_) {
    ...
    lateCancel_ = FALSE;
    ExEspStmtGlobals *espGlobals = sendTopTcb_->getGlobals()->castToExExeStmtGlobals()->castToExEspStmtGlobals();
    if (espGlobals) espGlobals->resetSendTopTcbLateCancelling();
  }

  EXSM_TRACE(EXSM_TRACE_PROTOCOL, "SNDT %p ACTR %s glob cancel %d rp %d", sendTopTcb_, (lateCancel_ ? "LC" : "C"),
             (int)stmtGlob->numCancelMsgesOut(), (int)numOfResponsesPending());
...
///////////////////////////////////////////////
ex_send_top_private_state::ex_send_top_private_state(const ex_send_top_tcb * /*tcb*/) {
  step_ = ex_send_top_tcb::NOT_STARTED_;
}

ex_send_top_private_state::~ex_send_top_private_state() {}

ex_tcb_private_state *ex_send_top_private_state::allocate_new(const ex_tcb *tcb) {
  return new (((ex_tcb *)tcb)->getSpace()) ex_send_top_private_state((ex_send_top_tcb *)tcb);
}
