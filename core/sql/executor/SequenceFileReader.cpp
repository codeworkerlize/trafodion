...
static const char *const sfrErrorEnumStr[] = {"No more data.",
...
//
//////////////////////////////////////////////////////////////////////////////
SequenceFileReader::~SequenceFileReader() { close(); }
...
SFR_RetCode SequenceFileReader::init() {
  ...
  if (javaMethodsInitialized_)
    ...
    //  JavaMethods_[JM_INITSERDE ].jm_signature =
...
//    return SFR_ERROR_INITSERDE_PARAMS;
//
//  jenv_->CallVoidMethod(javaObj_, JavaMethods_[JM_INITSERDE].methodID, js_numColumns, js_delim, js_columns,
//  js_colTypes, js_nullFormat);
//
...
SFR_RetCode SequenceFileReader::open(const char *path) {
  ...
  if (initJNIEnv() != JOI_OK) return SFR_ERROR_OPEN_PARAM;
...
SFR_RetCode SequenceFileReader::isEOF(bool &isEOF) {
  ...
  if (initJNIEnv() != JOI_OK) return SFR_ERROR_ISEOF_EXCEPTION;
  // boolean isEOF();
...
SFR_RetCode SequenceFileReader::close() {
  ...
  if (initJNIEnv() != JOI_OK) return SFR_ERROR_CLOSE_EXCEPTION;
...
SFR_RetCode SequenceFileReader::fetchRowsIntoBuffer(long stopOffset, char *buffer, long buffSize, long &bytesRead,
  ...
  QRLogger::log(CAT_SQL_HDFS_SEQ_FILE_READER, LL_DEBUG, "  =>Returning %d, read %ld bytes in %d rows.", retCode,
                bytesRead, rowsRead);
  return retCode;
}
...
static const char *const sfwErrorEnumStr[] = {"JNI NewStringUTF() in open() for writing.",
                                              "Java exception in open() for writing.", "JNI NewStringUTF() in write()",
                                              "Java exception in write()", "Java exception in close() after writing."};
...
//
//////////////////////////////////////////////////////////////////////////////
SequenceFileWriter::~SequenceFileWriter() { close(); }
...
SFW_RetCode SequenceFileWriter::open(const char *path, SFW_CompType compression) {
  ...
  if (initJNIEnv() != JOI_OK) return SFW_ERROR_OPEN_PARAM;
  jstring js_path = jenv_->NewStringUTF(path);
  if (js_path == NULL) {
    jenv_->PopLocalFrame(NULL);
...
SFW_RetCode SequenceFileWriter::write(const char *data) {
  ...
  if (initJNIEnv() != JOI_OK) return SFW_ERROR_WRITE_PARAM;
  ...
  do {
    ...
    if (rowEnd && rowEnd < buffEnd) {
      ...
      *rowEnd = '\0';
      ...
      if (result != SFW_OK) return result;
