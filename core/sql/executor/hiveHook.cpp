...
NABoolean hive_sd_desc::isAvroFile() const
{
  return strstr(inputFormat_, "Avro") && 
    strstr(outputFormat_, "Avro");
}
...
struct hive_tbl_desc* HiveMetaData::getFakedTableDesc(const char* tblName)
{
  ...
  hive_column_desc* c3 = new (h) hive_column_desc(h, 3, "C3", "float", 2);
   ...
   hive_sd_desc* sd1 = new (h)hive_sd_desc(h, 1, "loc", 0, 1, "ift", "oft", NULL, NULL,
                                                 ...
                                                 NABoolean readPartnParamInfo)
{
    ...
    while (ptr) {
      ...
      if ( !(strcmp(ptr->tblName_, tableNameInt.data())
        ...
        if ((NOT rereadFromMD) && (validate(ptr, allowSubdirs)))
           return ptr;
        else {
   ...
   hvcRetcode = hiveClient->getHiveTableInfo(schemaName, tblName, readPartnParamInfo);
...
NABoolean HiveMetaData::validate(hive_tbl_desc *hDesc, NABoolean allowSubdirs)
{
   Int64 currentRedefTime = 0;
   HVC_RetCode retCode = HiveClient_JNI::getRedefTime(hDesc->schName_, hDesc->tblName_,
						      allowSubdirs, currentRedefTime);
...
hive_sd_desc::~hive_sd_desc()
{
  ...
  if (nullFormat_)
    NADELETEBASIC(nullFormat_, heap_);
  if (serializeLib_)
    NADELETEBASIC(serializeLib_, heap_);
  if (partitionColValues_)
    NADELETEBASIC(partitionColValues_, heap_);
