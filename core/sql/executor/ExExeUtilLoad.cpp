...
///////////////////////////////////////////////////////////////
ExExeUtilCreateTableAsTcb::ExExeUtilCreateTableAsTcb(const ComTdbExeUtil &exe_util_tdb, ex_globals *glob)
    : ExExeUtilTcb(exe_util_tdb, NULL, glob), step_(INITIAL_), tableExists_(FALSE) {
    ...
    switch (step_) {
      case INITIAL_: {
        ...
        if (xnAlreadyStarted && !ctaTdb().noLoad()) {
          ExRaiseSqlError(getHeap(), &diagsArea_, -20123, NULL, NULL, NULL, "This DDL operation");
          step_ = ERROR_;
          break;
        }
      ...
      case CREATE_: {
        ...
        cliRC = cliInterface()->executeImmediate(ctaTdb().ctQuery_);
        if (cliRC < 0) {
        ...
        if (ta->xnInProgress()) doSidetreeInsert_ = FALSE;
      ...
      case TRUNCATE_TABLE_AND_ERROR_: {
        char *ddQuery = new (getMyHeap()) char[strlen("TRUNCATE TABLE; ") + strlen(ctaTdb().getTableName()) + 100];
      ...
      case INSERT_SIDETREE_: {
        ...
        if (redriveCTAS) {
          if (childQueryId_ == NULL) childQueryId_ = new (getHeap()) char[ComSqlId::MAX_QUERY_ID_LEN + 1];
          childQueryIdLen_ = ComSqlId::MAX_QUERY_ID_LEN;
          cliRC = cliInterface()->executeImmediatePrepare2(ctaTdb().siQuery_, childQueryId_, &childQueryIdLen_,
                                                           &childQueryCostInfo_, &childQueryCompStatsInfo_, NULL, NULL,
                                                           &rowsAffected, TRUE);
          if (cliRC >= 0) {
            childQueryId_[childQueryIdLen_] = '\0';
            Statement *ctasStmt = masterGlob->getStatement();
            cliRC = ctasStmt->setChildQueryInfo(diagsArea, childQueryId_, childQueryIdLen_, &childQueryCostInfo_,
                                                &childQueryCompStatsInfo_);
            if (cliRC < 0) {
              step_ = HANDLE_ERROR_;
              break;
            }
            if (outputBuf_ == NULL) outputBuf_ = new (getHeap()) char[ComSqlId::MAX_QUERY_ID_LEN + 100];  //
          ...
          cliRC = cliInterface()->executeImmediatePrepare(ctaTdb().siQuery_, NULL, NULL, &rowsAffected, TRUE);
        }
      ...
      case INSERT_SIDETREE_EXECUTE_: {
        cliRC = cliInterface()->executeImmediateExec(ctaTdb().siQuery_, NULL, NULL, TRUE, &rowsAffected);
        if (cliRC < 0) {
          cliInterface()->allocAndRetrieveSQLDiagnostics(diagsArea_);
          step_ = HANDLE_ERROR_;
        } else {
      ...
      case INSERT_VSBB_: {
        ...
        if (redriveCTAS) {
          if (childQueryId_ == NULL) childQueryId_ = new (getHeap()) char[ComSqlId::MAX_QUERY_ID_LEN + 1];
          cliRC = cliInterface()->executeImmediatePrepare2(ctaTdb().viQuery_, childQueryId_, &childQueryIdLen_,
                                                           &childQueryCostInfo_, &childQueryCompStatsInfo_, NULL, NULL,
                                                           &rowsAffected, TRUE);
          cliInterface()->setIsoMapping(savedIsoMapping);
          if (cliRC >= 0) {
          ...
          cliRC = cliInterface()->executeImmediate(ctaTdb().viQuery_, NULL, NULL, TRUE, &rowsAffected, TRUE);
          cliInterface()->setIsoMapping(savedIsoMapping);
          if (cliRC < 0) {
            cliInterface()->allocAndRetrieveSQLDiagnostics(diagsArea_);
            step_ = HANDLE_ERROR_;
      ...
      case INSERT_VSBB_EXECUTE_: {
        cliRC = cliInterface()->executeImmediateExec(ctaTdb().viQuery_, NULL, NULL, TRUE, &rowsAffected);
        if (cliRC < 0) {
          cliInterface()->allocAndRetrieveSQLDiagnostics(diagsArea_);
          step_ = HANDLE_ERROR_;
        } else {
      ...
      case UPD_STATS_: {
        if ((ctaTdb().threshold_ == -1) ||
            ((ctaTdb().threshold_ > 0) && (masterGlob->getRowsAffected() < ctaTdb().threshold_))) {
          step_ = DONE_;
          break;
        }

        // issue the upd stats command
        char *usQuery = new (getHeap()) char[strlen(ctaTdb().usQuery_) + 10 + 1];

        str_sprintf(usQuery, ctaTdb().usQuery_, masterGlob->getRowsAffected());
      ...
      case HANDLE_ERROR_: {
        if ((ctaTdb().ctQuery_) && (ctaTdb().loadIfExists())) {
      ...
      case DROP_AND_ERROR_: {
        if ((ctaTdb().ctQuery_) && (NOT tableExists_)) {
          // this is an error case, drop the table
          char *dtQuery =
              new (getMyHeap()) char[strlen("DROP TABLE CASCADE; ") + strlen(ctaTdb().getTableName()) + 100];
...
ex_tcb *ExExeUtilAqrWnrInsertTdb::build(ex_globals *glob) {
  ...
  exe_util_tcb = new (glob->getSpace()) ExExeUtilAqrWnrInsertTcb(*this, childTcb, glob);
...
ExExeUtilAqrWnrInsertTcb::ExExeUtilAqrWnrInsertTcb(const ComTdbExeUtilAqrWnrInsert &exe_util_tdb,
                                                   const ex_tcb *child_tcb, ex_globals *glob)
    : ExExeUtilTcb(exe_util_tdb, child_tcb, glob), step_(INITIAL_), targetWasEmpty_(false) {}
...
void ExExeUtilAqrWnrInsertTcb::setStep(Step newStep, int lineNum) {
  ...
  if (!sv_checked_yet) {
    ...
    if (logST && *logST == '1') sv_logStep = true;
  }
  if (!sv_logStep) return;

  if (NULL == getGlobals()->castToExExeStmtGlobals()->castToExMasterStmtGlobals()) return;
  ...
  while (!(qparent_.down->isEmpty() || qparent_.up->isFull())) {
    ...
    switch (step_) {
      case INITIAL_: {
        ...
        if (getDiagsArea()) getDiagsArea()->clear();
      ...
      case LOCK_TARGET_: {
        ...
        for (size_t i = 0; i < sizeof(dummyArg); i++) ex_assert(dummyArg[i] == i, "lock table returned data");
#endif
      ...
      case CLEANUP_CHILD_: {
        ...
        do {
          ...
          if (centry->upState.status == ex_queue::Q_NO_DATA) {
            ...
            if (targetWasEmpty_ && (pentry_down->downState.request != ex_queue::GET_NOMORE)) {
              ...
              if (cliRc < 0 || !statsArea) {
                ...
                cleanupTarget = true;
              } else if (!statsArea->getMasterStats() || statsArea->getMasterStats()->getStatsErrorCode() != 0) {
...
ExWorkProcRetcode ExExeUtilAqrWnrInsertTcb::workCancel() {
  if (!(qparent_.down->isEmpty()) && (ex_queue::GET_NOMORE == qparent_.down->getHeadEntry()->downState.request)) {
    switch (step_) {
      case INITIAL_:
        SETSTEP(DONE_);
        break;
      case LOCK_TARGET_:
        ex_assert(0, "work method doesn't return with step_ set to LOCK_TARGET_.");
        break;
      case IS_TARGET_EMPTY_:
        ex_assert(0,
      ...
      case ERROR_:
        ...
        break;
      case CLEANUP_TARGET_:
        ex_assert(0, "work method doesn't return with step_ set to CLEANUP_TARGET_,.");
        break;
      case DONE_:
        ex_assert(0, "work method doesn't return with step_ set to DONE_.");
        break;
    }
  }
  return WORK_OK;
...
///////////////////////////////////////////////////////////////
ExExeUtilHBaseBulkLoadTcb::ExExeUtilHBaseBulkLoadTcb(const ComTdbExeUtil &exe_util_tdb, ex_globals *glob)
...
NABoolean ExExeUtilHBaseBulkLoadTcb::generateTrafSampleTable(const char *cTableName, const char *cSampleTableName) {
  ...
  if (sName != sHbaseCatalog) sOptions.append(" WITH PARTITIONS ");

  sQuery = "CREATE TABLE ";
  sQuery.append(sSampleTableName_).append(" LIKE ").append(NAString(cTableName)).append(sOptions);
...
short ExExeUtilHBaseBulkLoadTcb::loadWithParams(ComDiagsArea *&diagsArea) {
  ...
  workAtp_->getTupp(hblTdb().workAtpIndex()).setDataPointer(inputParamsBuf);

  ex_expr::exp_return_type exprRetCode = hblTdb().inputExpr()->eval(pentry_down->getAtp(), workAtp_);
  if (exprRetCode == ex_expr::EXPR_ERROR) {
    diagsArea = moveDiagsAreaFromEntry(pentry_down);
    return -1;
  }

  ExpTupleDesc *inputExprTD = hblTdb().workCriDesc_->getTupleDescriptor(hblTdb().workAtpIndex());
  ...
  for (UInt32 i = 0; i < inputExprTD->numAttrs(); i++) {
    ...
    if (srcAttr->getNullFlag()) {
      ...
      if (srcNullVal[0]) srcIsNull = TRUE;
    }
    ...
    if (NOT srcIsNull) {
      ...
      memcpy(&cliInterface()->inputBuf()[tgtParamOffset + tgtParamVcIndLen], srcVal, srcLen);
    }  // !nullVal
  }    // for

  // input values are filled in input buffer.
  // execute the internal load query.
  cliRC = cliInterface()->executeImmediateExec(NULL, NULL, NULL, FALSE, &rowsAffected_);
  if (cliRC < 0) {
    cliInterface()->allocAndRetrieveSQLDiagnostics(diagsArea);
    return cliRC;
  }
  ...
  while (1) {
    switch (step_) {
      case INITIAL_: {
        ...
        if (setStartStatusMsgAndMoveToUpQueue(" LOAD", &rc)) return rc;

        if (hblTdb().getUpsertUsingLoad()) hblTdb().setPreloadCleanup(FALSE);
      ...
      case TRUNCATE_TABLE_: {
        if (setStartStatusMsgAndMoveToUpQueue(" PURGE DATA", &rc, 0, TRUE)) return rc;
        ...
        char *ttQuery = new (getMyHeap()) char[strlen("PURGEDATA  ; ") + strlen(hblTdb().getTableName()) + 100];
        ...
        if (parserFlagSet) masterGlob->getStatement()->getContext()->resetSqlParserFlags(0x20000);
      ...
      case LOAD_START_: {
        ...
        if (hblTdb().getPreloadCleanup())
          ...
          step_ = PREPARATION_;
          if (hblTdb().getRebuildIndexes() || hblTdb().getHasUniqueIndexes()) step_ = DISABLE_INDEXES_;
        }
      } break;

      case PRE_LOAD_CLEANUP_: {
        if (setStartStatusMsgAndMoveToUpQueue(" CLEANUP", &rc, 0, TRUE)) return rc;

        // Cleanup files
        char *clnpQuery =
            new (getMyHeap()) char[strlen("LOAD CLEANUP FOR TABLE ; ") + strlen(hblTdb().getTableName()) + 100];
        strcpy(clnpQuery, "LOAD ");
        if (hblTdb().getUpdateStats() && !ustatNonEmptyTable_) strcat(clnpQuery, "WITH SAMPLE ");
        strcat(clnpQuery, "CLEANUP FOR TABLE ");
        if (hblTdb().getIndexTableOnly()) strcat(clnpQuery, "TABLE(INDEX_TABLE ");
        strcat(clnpQuery, hblTdb().getTableName());
        if (hblTdb().getIndexTableOnly()) strcat(clnpQuery, ")");
        strcat(clnpQuery, ";");
      ...
      case DISABLE_INDEXES_: {
        if (setStartStatusMsgAndMoveToUpQueue(" DISABLE INDEXES", &rc, 0, TRUE)) return rc;
        ...
        char *diQuery = new (
            getMyHeap()) char[strlen("ALTER TABLE DISABLE ALL INDEXES   ; ") + strlen(hblTdb().getTableName()) + 100];
      ...
      case LOADING_DATA_: {
        if (!hblTdb().getUpsertUsingLoad()) {
          ...
          if (loggingLocation_ != NULL) cliRC = holdAndSetCQD("TRAF_LOAD_ERROR_LOGGING_LOCATION", loggingLocation_);
          if (cliRC < 0) {
            step_ = LOAD_END_ERROR_;
            break;
          }
          ...
          if (ustatNonEmptyTable_) {
            ...
            if (sampleOptPtr) memset(sampleOptPtr, ' ', strlen(sampleOpt));
          }
          ...
          if (!hblTdb().inputExpr()) {
            cliRC = cliInterface()->executeImmediate(loadQuery, NULL, NULL, TRUE, &rowsAffected_, FALSE, &diagsArea);
          } else {
          ...
          if (rowsAffected_ == 0) step_ = ENABLE_INDEXES_AND_LOAD_END_;
          ...
          if (setStartStatusMsgAndMoveToUpQueue(" UPSERT USING LOAD ", &rc, 0, TRUE)) return rc;
          ...
          if (hblTdb().getRebuildIndexes() || hblTdb().getHasUniqueIndexes()) step_ = POPULATE_INDEXES_;
      ...
      case COMPLETE_BULK_LOAD_: {
        if (setStartStatusMsgAndMoveToUpQueue(" COMPLETION", &rc, 0, TRUE)) return rc;
        ...
        if (hblTdb().getKeepHFiles() && !hblTdb().getSecure()) {
        ...
        // complete load query
        char *clQuery =
            new (getMyHeap()) char[strlen("LOAD COMPLETE FOR TABLE  ; ") + strlen(hblTdb().getTableName()) + 100];
        strcpy(clQuery, "LOAD ");
        if (hblTdb().getUpdateStats() && !ustatNonEmptyTable_) strcat(clQuery, "WITH SAMPLE ");
        strcat(clQuery, "COMPLETE FOR TABLE  ");
        if (hblTdb().getIndexTableOnly()) strcat(clQuery, "TABLE(INDEX_TABLE ");
        strcat(clQuery, hblTdb().getTableName());
        if (hblTdb().getIndexTableOnly()) strcat(clQuery, ")");
        strcat(clQuery, ";");
      ...
      case POPULATE_INDEXES_EXECUTE_: {
        char *piQuery =
            new (getMyHeap()) char[strlen("POPULATE ALL INDEXES ON  ; ") + strlen(hblTdb().getTableName()) + 100];
        if (hblTdb().getRebuildIndexes())
      ...
      case UPDATE_STATS_EXECUTE_: {
        if (ustatNonEmptyTable_) {
          ...
          step_ = LOAD_END_;
          sprintf(statusMsgBuf_, "       UPDATE STATISTICS not executed: table %s not empty before load. %c",
                  hblTdb().getTableName(), '\n');
          int len = strlen(statusMsgBuf_);
        ...
        char *ustatStmt = new (getMyHeap()) char[strlen("UPDATE STATS FOR TABLE  ON EVERY COLUMN; ") +
                                                 strlen(hblTdb().getTableName()) + 100];
        ...
        if (cliRC < 0) step_ = LOAD_END_ERROR_;

        setEndStatusMsg(" UPDATE STATS", 0, TRUE);
      } break;
      case UPDATE_STATS__END_: {
        if (sSampleTableName_.length() > 0) {
          if (setStartStatusMsgAndMoveToUpQueue(" DROP TABLE", &rc, 0, TRUE)) return rc;
      ...
      case RETURN_STATUS_MSG_: {
        if (moveRowToUpQueue(statusMsgBuf_, 0, &rc)) return rc;
      ...
      case ENABLE_INDEXES_AND_LOAD_END_: {
        if (hblTdb().getRebuildIndexes() || hblTdb().getHasUniqueIndexes()) {
          char *diQuery = new (
              getMyHeap()) char[strlen("ALTER TABLE ENABLE ALL INDEXES   ; ") + strlen(hblTdb().getTableName()) + 100];
          strcpy(diQuery, "ALTER TABLE  ");
          strcat(diQuery, hblTdb().getTableName());
          if (hblTdb().getRebuildIndexes())
...
short ExExeUtilHBaseBulkLoadTcb::setCQDs() {
  ...
  if (holdAndSetCQD("ALLOW_INCOMPATIBLE_OPERATIONS", "ON") < 0) {
    return -1;
  }
  ...
  if (holdAndSetCQD("TRAF_LOAD_MAX_ERROR_ROWS", strMaxRR) < 0) {
    return -1;
  }
  if (hblTdb().getContinueOnError()) {
    ...
    if (holdAndSetCQD("TRAF_LOAD_ERROR_COUNT_ID", pt) < 0) {
      return -1;
    }
  }
  return 0;
...
short ExExeUtilHBaseBulkLoadTcb::restoreCQDs() {
  ...
  if (restoreCQD("TRAF_LOAD_PREP_SKIP_DUPLICATES") < 0) {
    return -1;
  }
  if (restoreCQD("ALLOW_INCOMPATIBLE_OPERATIONS") < 0) {
    return -1;
  }
  if (hblTdb().getForceCIF()) {
  ...
  if (restoreCQD("TRAF_LOAD_LOG_ERROR_ROWS") < 0) {
    return -1;
  }
  if (restoreCQD("TRAF_LOAD_CONTINUE_ON_ERROR") < 0) {
    return -1;
  }
  if (restoreCQD("TRAF_LOAD_MAX_ERROR_ROWS") < 0) {
    return -1;
  }
  if (restoreCQD("TRAF_LOAD_ERROR_COUNT_ID") < 0) {
    return -1;
  }
  if (restoreCQD("TRAF_LOAD_ERROR_LOGGING_LOCATION") < 0) {
    return -1;
  }
...
short ExExeUtilHBaseBulkLoadTcb::setStartStatusMsgAndMoveToUpQueue(const char *operation, short *rc, int bufPos,
                                                                   NABoolean withtime) {
  if (hblTdb().getNoOutput()) return 0;
  ...
  getStatusString(operation, "Started", hblTdb().getTableName(), &statusMsgBuf_[bufPos], FALSE,
                  (withtime ? timeBuf : NULL));
  return moveRowToUpQueue(statusMsgBuf_, 0, rc);
}

void ExExeUtilHBaseBulkLoadTcb::setEndStatusMsg(const char *operation, int bufPos, NABoolean withtime) {
  if (hblTdb().getNoOutput()) return;
  ...
  if (withtime) {
    ...
    getStatusString(operation, "Ended", hblTdb().getTableName(), &statusMsgBuf_[bufPos], FALSE,
                    withtime ? timeBuf : NULL);
...
///////////////////////////////////////////////////////////////
ExExeUtilUpdataDeleteTcb::ExExeUtilUpdataDeleteTcb(const ComTdbExeUtil &exe_util_tdb, ex_globals *glob)
    : ExExeUtilTcb(exe_util_tdb, NULL, glob), step_(INITIAL_), nextStep_(INITIAL_), rowsAffected_(0) {
  ehi_ = ExpHbaseInterface::newInstance(getGlobals()->getDefaultHeap(), hblTdb().server_, hblTdb().zkPort_,
                                        hblTdb().storageType_, FALSE);
...
Lng32 ExExeUtilUpdataDeleteTcb::lockOperatingTable() {
  ...
  exequery.format("update %s.\"%s\".%s set flags = bitor(flags, %d) where object_uid = %ld", TRAFODION_SYSCAT_LIT,
                  SEABASE_MD_SCHEMA, SEABASE_OBJECTS, MD_OBJECTS_TRUNCATE_IN_PROGRESS, hblTdb().objectUid());
...
ExExeUtilUpdataDeleteTcb::~ExExeUtilUpdataDeleteTcb() { freeResources(); }

short ExExeUtilUpdataDeleteTcb::setCQDs() {
  if (holdAndSetCQD("TRAF_NO_DTM_XN", "ON") < 0) {
    return -1;
  }
  if (holdAndSetCQD("ATTEMPT_ESP_PARALLELISM", "ON") < 0) {
    return -1;
  }
  if (holdAndSetCQD("SKIP_CHECK_FOR_TABLE_IB", "ON") < 0) {
    return -1;
  }
  return 0;
}
...
short ExExeUtilUpdataDeleteTcb::restoreCQDs() {
  if (restoreCQD("TRAF_NO_DTM_XN") < 0) {
    return -1;
  }
  if (restoreCQD("ATTEMPT_ESP_PARALLELISM") < 0) {
    return -1;
  }
  if (restoreCQD("SKIP_CHECK_FOR_TABLE_IB") < 0) {
    return -1;
  }
  return 0;
}
    ...
    switch (step_) {
      case INITIAL_: {
        ...
        if ((retcode = ehi_->init(NULL)) != HBASE_ACCESS_SUCCESS) {
          ExHbaseAccessTcb::setupError((NAHeap *)getMyHeap(), qparent_, retcode, "ExpHbaseInterface_JNI::init");
          step_ = EXE_ERROR_;
          break;
        }
        ...
        if (hblTdb().incrBackupEnabled()) {
          ...
          if (lockedTag.length() != 0 && lockedTag != "?") {
            ...
            if (lockedTag.contains("NON_TX_SNAPSHOT")) {
              ExRaiseSqlError(getHeap(), &diagsArea_, -8146, NULL, NULL, NULL,
                              " Locked BACKUP TAG exist! Lock held by tag: " + lockedTag);
              step_ = EXE_ERROR_;
              break;
            }
          }
        ...
        if (!indexList) {
          ExHbaseAccessTcb::setupError((NAHeap *)getMyHeap(), qparent_, retcode, "listOfIndexesAndTable is empty");
          step_ = EXE_ERROR_;
          break;
        }
        indexList->position();
      ...
      case EXE_GET_OBJNAME_: {
        ...
        NAString time_buf;
        time_buf.format("%4d-%02d-%02d_%02d_%02d_%02d", ltm->tm_year + 1900, ltm->tm_mon + 1, ltm->tm_mday,
        ...
        ex_assert(snap.fullTableName.length() > 0 && snap.snapshotName.length() > 0,
      ...
      case EXE_CREATE_SNAPSHOT_: {
        for (int i = 0; i < snapshotsList.entries(); i++) {
          ...
          if (retcode != HBC_OK) {
            ExHbaseAccessTcb::setupError((NAHeap *)getMyHeap(), qparent_, retcode, "HBaseClient_JNI::createSnapshot",
      ...
      case EXE_RUNSQL_: {
        ...
        cliRC = cliInterface()->executeImmediate(exe_query, NULL, NULL, TRUE, &rowsAffected, FALSE, &diagsArea_);
      ...
      case EXE_RESTORE_SNAPSHOT_: {
        for (int i = 0; i < snapshotsList.entries(); i++) {
          ...
          if (retcode != HBC_OK) {
            ExHbaseAccessTcb::setupError((NAHeap *)getMyHeap(), qparent_, retcode, "HBaseClient_JNI::restoreSnapshot",
                                         snapshotsList.at(i).snapshotName.data());
            break;
          }
        }
      ...
      case EXE_CLEANUP_: {
        ...
        for (int i = 0; i < snapshotsList.entries(); i++) {
          ...
          if (retcode != HBC_OK) {
            ExHbaseAccessTcb::setupError((NAHeap *)getMyHeap(), qparent_, retcode, "HBaseClient_JNI::deleteSnapshot",
                                         snapshotsList.at(i).snapshotName.data());
            step_ = EXE_ERROR_;
            break;
          }
      ...
      case DONE_: {
        ...
        if (!isProcessError) masterGlob->setRowsAffected(rowsAffected);
        unLockOperatingTable();
