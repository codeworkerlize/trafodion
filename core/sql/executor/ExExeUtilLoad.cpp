    ...
    step_(EMPTY_)    
{
  ...
  requestTag_ = -1;
  lobLoc_[0] = '\0';
  exLobGlobals_ = ExpLOBoper::initLOBglobal((NAHeap *)glob->getDefaultHeap(), currContext, exe_util_tdb.useLibHdfs(), FALSE, 0, FALSE);

  extractCliInterface_ = NULL;

  lobPtr_ = NULL;
  localFileFD_ = -1;
  hdfsFileClient_ = NULL;
  localFileWriteOffset_ = 0;
  bytesWritten_ = 0;
  ...
  if (lobHandle_ && lobName_)
    retcode = ExpLOBInterfaceSelectCursor
	      (lobGlobs,
	       ...
	       lobDataLen_, dummyUncompressedLen, lobData_, 
               NULL,
	       3, // close
...
short ExExeUtilLobExtractTcb::workV2()
{
  if (lobTdb().getToType() == ComTdbExeUtilLobExtract::TO_FILE_)
    {
      return extractToFileV2();
    }
  ...
  CollHeap *contextHeap = currContext->exCollHeap();

  ComDiagsArea & diags       = currContext->diags();

  ExpLobOperV2::DMLOptions *options = 
    (ExpLobOperV2::DMLOptions *)lobTdb().getLobDmlOptions();

  ExLobGlobals * lobGlobs = getLobGlobals();

  // if cancel request, return DONE
  if (pentry_down->downState.request == ex_queue::GET_NOMORE)
    {
      if ((step_ == OPEN_CURSOR_) || (step_ == READ_CURSOR_))
        {
          step_ = CLOSE_CURSOR_;
        }
      else
  ...
  while (1)
	...
	case GET_LOB_HANDLE_:
	  { 
	    if (lobTdb().handleInStringFormat())
	      {
		if (ExpLOBoper::genLOBhandleFromHandleStringV2
		    (lobTdb().getHandle(),
		     lobTdb().getHandleLen(),
		     lobHandle_,
		     lobHandleLen_) < 0)
	    ...
	    else
	      {
                ...
                ex_expr::exp_return_type exprRetCode =
                  lobTdb().inputExpr_->eval(pentry_down->getAtp(),
                                            workAtp_);
                if (exprRetCode == ex_expr::EXPR_ERROR)
                  {
                    step_ = HANDLE_ERROR_;
                ...
                ExpTupleDesc * inputExprTD =
                  lobTdb().workCriDesc_->getTupleDescriptor
                  (lobTdb().workAtpIndex());
                
                // get pointer to handle
                Attributes * attr = inputExprTD->getAttr(0);
                inputHandle_ = &lobInputHandleBuf_[attr->getOffset()];
		...
		if (ExpLOBoper::genLOBhandleFromHandleStringV2
		     ...
		     lobHandleLen_) < 0)
		  {
		    ...
		    break;
                ...
                Int64 lobUID = 0;
                Int64 lobSeq = 0;
                ExpLOBoper::extractFromLOBhandleV2(NULL, NULL, NULL,
                                                   &objUID, 
                                                   NULL, NULL, 
                                                   &lobUID, &lobSeq,
                                                   lobHandle_);

                // get data buf addr where data will be moved
                attr = inputExprTD->getAttr(1);
                dataBufAddr_ = *(Int64*)&lobInputHandleBuf_[attr->getOffset()];

                if (lobTdb().getToType() == ComTdbExeUtilLobExtract::TO_BUFFER_)
                  {
                    if (inputExprTD->numAttrs() > 2)
                      {
                        // get size of data.
                        // On input size of data to be selected.
                        // On output, size of returned data.
                        attr = inputExprTD->getAttr(2);
                        extractSizeAddr_ = *(Int64*)&lobInputHandleBuf_[attr->getOffset()];
                      }

                    if (inputExprTD->numAttrs() > 3)
                      {
                        // get position at which data will be updated.
                        position_ = 1;
                        attr = inputExprTD->getAttr(3);
                        positionAddr_ = *(Int64*)&lobInputHandleBuf_[attr->getOffset()];
                        position_ = *(Int64*)positionAddr_;
                        
                        if (position_ != 1)
                          {
                            // not yet supported. Invalid position value.
                            ExRaiseSqlError(getMyHeap(),
                                            &diagsArea_,
                                            (ExeErrorCode)(EXE_STMT_NOT_SUPPORTED),
                                            NULL, NULL, NULL,
                                            "Invalid position value specified.");
                            step_ = HANDLE_ERROR_;
                            break;
                          }
                      } // if
            ...
            // reset inline data len, in case it was set.
            ExpLOBoper::updLOBhandleInlineData(lobHandle_, 0, NULL);
            
            strcpy(lobLoc_, lobTdb().getStringParam2());
	    if (lobTdb().getToType() == ComTdbExeUtilLobExtract::TO_BUFFER_)
              {
                if (extractSizeAddr_ && *(Int64*)extractSizeAddr_ == -1)
                  step_ = RETRIEVE_LOB_LENGTH_;
                else
                  step_ = OPEN_CURSOR_;
              }
	    else if (lobTdb().getToType() == ComTdbExeUtilLobExtract::RETRIEVE_LENGTH_)
	      step_ = RETRIEVE_LOB_LENGTH_;
            else
              {
                // invalid "toType"
	...
	case RETRIEVE_LOB_LENGTH_: 
	  {
            retcode = lobOperV2_.dmlInterface(
                 lobGlobs,
                 lobHandle_, lobHandleLen_,
                 ExpLobOperV2::LOB_DML_SELECT_LOBLENGTH,
                 *options,
                 lobLoc_,
                 getMyHeap(),
                 NULL, NULL,
                 NULL, (Int64*)&lobDataLen_,
                 NULL);
		...
		ExRaiseSqlError(getHeap(), &diagsArea_, 
				...
				&cliError, NULL, (char*)"SQLCLI_LOBv2cliInterface",
		...
		break;
            ...
            if (dataBufAddr_)
              str_cpy_all((char*)dataBufAddr_, (char*)&lobDataLen_, sizeof(Int64));
            str_sprintf(statusString_," LOB Length : %ld", lobDataLen_);
            step_ = RETURN_STATUS_AND_DONE_;
	  }
          break;
	...
	case OPEN_CURSOR_:
	  {
	    Int64 currLen = -1;
            retcode = lobOperV2_.dmlInterface(lobGlobs,
                                              lobHandle_, lobHandleLen_,
	...
	case READ_CURSOR_:
	  {
            lobData_ =  (char *)dataBufAddr_;
            lobDataSpecifiedExtractLen_ = (extractSizeAddr_ ? *(Int64*)extractSizeAddr_ : 0);
            ...
            lobDataOutputLen = lobDataSpecifiedExtractLen_;
            ...
            retcode = lobOperV2_.dmlInterface(lobGlobs,
                                              ...
                                              *options,
                                              ...
                                              getMyHeap(),
                                              NULL, NULL,
                                              (Int64*)&lobData_, (Int64*)&lobDataOutputLen,
                                              NULL);
	    if (retcode < 0)
	      {
		Lng32 cliError = 0;
		...
		break;
            ...
            memcpy((char *)extractSizeAddr_, (char*)&lobDataOutputLen, sizeof(Int64));
	    if ((lobDataOutputLen == 0) || (retcode == 100))
	      {
                step_ = CLOSE_CURSOR_;
                break;
	      }
	...
	case RETURN_STATUS_:
	case RETURN_STATUS_AND_DONE_:
	  {
	    if (qparent_.up->isFull())
	      return WORK_OK;

	    //Return to upqueue whatever is in the statusString_ data member
	    short rc; 
	    moveRowToUpQueue(statusString_, strlen(statusString_), &rc);

            if (step_ == RETURN_STATUS_AND_DONE_)
              {
                step_ = DONE_;
              }
            else
...
short ExExeUtilLobExtractTcb::extractToFileV2()
{
  Lng32 cliRC = 0;
  Lng32 retcode = 0;
  Int64 lobDataOutputLen = 0;
  ...
  while (1)
	...
	case GET_LOB_HANDLE_:
	  { 
            if (NOT lobTdb().handleInStringFormat())
              {
                ExRaiseSqlError(getMyHeap(),
                                &diagsArea_,
                                (ExeErrorCode)(EXE_INVALID_LOB_HANDLE));
            ...
            if (!lobPtr_)
              {
                lobPtr_ = new(lobGlobs->getHeap()) ExLobV2(lobGlobs->getHeap(), NULL);
                lobPtr_->lobGlobalHeap_ = lobGlobs->getHeap();
              }
            
            fileType_ = lobPtr_->fileType(lobTdb().getStringParam1());
            if (fileType_ == CURL_FILE)
              {
                ExRaiseSqlError(getMyHeap(),
                                &diagsArea_,
                                (ExeErrorCode)(EXE_STMT_NOT_SUPPORTED),
            ...
            dataBufAddr_ = (Int64)(new(getMyHeap()) char[options->lobMaxChunkMemSize]);
            bytesWritten_ = 0;
	...
	case OPEN_CURSOR_:
	  {
		...
		break;
            ...
            if (fileType_ == LOCAL_FILE)
              {
                retcode = lobPtr_->openLocalFileToWrite(lobTdb().getStringParam1(),
                                                        tgtFlags, localFileFD_);
              }
            else
              {
                retcode = lobPtr_->openHdfsFileToWrite(lobTdb().getStringParam1(),
                                                       tgtFlags, hdfsFileClient_);
              }
		...
		ExRaiseSqlError(getHeap(), &diagsArea_, 
				(ExeErrorCode)(8442), NULL, &intParam1, 
				&cliError, NULL, (char*)"openFileToWrite",
		                getLobErrStr(intParam1), (char*)getSqlJniErrorStr());
		step_ = HANDLE_ERROR_;
		break;
	      }
	...
	case READ_CURSOR_:
	  {
            lobData_ =  (char *)dataBufAddr_;
            lobDataOutputLen = options->lobMaxChunkMemSize;
            retcode = lobOperV2_.dmlInterface(lobGlobs,
                                              lobHandle_, lobHandleLen_,
                                              ExpLobOperV2::LOB_DML_SELECT_FETCH,
                                              *options,
	    ...
	    if (lobDataOutputLen == 0)
	      {
		str_sprintf(statusString_, "Success. Targetfile :%s  Length : %ld", 
                            lobTdb().getStringParam1(), bytesWritten_);
		...
		break;
            ...
            if (fileType_ == LOCAL_FILE)
              {
                retcode = lobPtr_->writeToLocalFile(localFileFD_, lobData_,
                                                    lobDataOutputLen, localFileWriteOffset_,
                                                    bytesWritten_);
                localFileWriteOffset_ += lobDataOutputLen;
              }
            else
              {
                retcode = lobPtr_->writeToHdfsFile(hdfsFileClient_, lobData_,
                                                   lobDataOutputLen, lobDataOutputLen,
                                                   bytesWritten_);
              }
		...
		ExRaiseSqlError(getHeap(), &diagsArea_, 
				(ExeErrorCode)(8442), NULL, &intParam1, 
				&cliError, NULL, (char*)"writeToFile",
		                getLobErrStr(intParam1), (char*)getSqlJniErrorStr());
		step_ = HANDLE_ERROR_;
		break;
	      }
	...
	case DONE_:
	  {
            if (fileType_ == LOCAL_FILE)
              {
                retcode = lobPtr_->closeLocalFile(localFileFD_);
              }
            else
              {
                retcode = lobPtr_->closeHdfsFile(hdfsFileClient_);
              }
	  ...
	  { 
	    ...
	    strcpy(lobLoc_, lobTdb().getStringParam2());
	      ...
	      step_ = EXTRACT_LOB_DATA_;
                     ...
                     (lobTdb().getToType() == ComTdbExeUtilLobExtract::TO_FILE_))
	      step_ = RETRIEVE_LOB_LENGTH_;
            else
              {
                // invalid "toType"
	    ...
	    lobName_ = ExpLOBoper::ExpGetLOBname(
                 uid, lobNum, descPartnKey, lobTdb().getNumLOBdatafiles(),
                 lobNameBuf_, 1000);	   
	...
	case RETRIEVE_LOB_LENGTH_ : 
	  {
	    ...
	    ExpLOBoper::extractFromLOBhandle(&flags, &lobType_, &lobNum, &uid,  
            ...
            //Temp TBD after JDBC changes are in.
            // Don't allow extract on inline LOB yet.

            if (lobType_ == Lob_Inline)
              {
                Lng32 intParam1 = -1;
                Lng32 cliError= -1;
                ExRaiseSqlError(getHeap(), &diagsArea_, 
                                (ExeErrorCode)(8442), NULL, &intParam1, 
                                &cliError, NULL, (char*)"LOB extract",
                                ":Extract not supported on inline lob", (char*)getSqlJniErrorStr());
                step_ = HANDLE_ERROR_;
                break;
              }
                lobName_ = ExpLOBoper::ExpGetLOBname(
    ...
    step_(EMPTY_)    
{
  ...
  exLobGlobals_ = ExpLOBoper::initLOBglobal((NAHeap *)glob->getDefaultHeap(), currContext, exe_util_lobupdate_tdb.useLibHdfs(), 
             exe_util_lobupdate_tdb.useHdfsWriteLock(), exe_util_lobupdate_tdb.hdfsWriteLockTimeout(), FALSE);
...
short ExExeUtilLobUpdateTcb::workV2()
{
  Lng32 cliRC = 0;
  Lng32 retcode = 0;
	...
	case GET_HANDLE_:
	  { 
		     ...
		     lobHandleLen_) < 0)
		  {
		    ...
		    break;
                ...
                if (inputExprTD->numAttrs() > 2)
                  {
                    // get size of data.
                    // On input size of data to be updated.
                    // On output, size of returned data.
                    attr = inputExprTD->getAttr(2);
                    extractSizeAddr_ = *(Int64*)&lobInputHandleBuf_[attr->getOffset()];
                    lobDataLen_ = *(Int64*)extractSizeAddr_;
                  }
            ...
            if (step_ == GET_HANDLE_) // step_ not already changed
              {
        ...
        case EMPTY_LOB_DATA_:
          {
            ...
            if (retcode < 0)
              {
                Lng32 intParam1 = -retcode;
                Lng32 cliError = 0;
                ExRaiseSqlError(getGlobals()->getDefaultHeap(), &diagsArea_, 
                                (ExeErrorCode)(8442), NULL, &intParam1, 
                                &cliError, NULL, (char*)"lobOperV2_.dmlInterface",
                                getLobErrStr(intParam1), (char*)getSqlJniErrorStr());
                step_ = HANDLE_ERROR_;
            ...
            retcode = lobOperV2_.dmlInterface(
                 ...
                 *options,
                 ...
                 getGlobals()->getDefaultHeap(),
                 NULL, NULL,
                 (step_ == EMPTY_LOB_DATA_ ? NULL : (Int64*)lobData_),
                 (step_ == EMPTY_LOB_DATA_ ? NULL : &lobDataLen_));
            if (retcode < 0)
              {
                Lng32 intParam1 = -retcode;
                Lng32 cliError = 0;
            ...
            str_sprintf(statusString_," Updated %ld bytes of LOB data ", lobDataLen_);
            step_ = RETURN_STATUS_;
            break;
          }
          break;
        ...
        case APPEND_LOB_DATA_:
          {
            ...
            str_sprintf(statusString_," Appended %ld bytes of LOB data ", lobDataLen_);
            step_ = RETURN_STATUS_;
            break;
          }
          break;

          // update lob at specified position
        case UPDATE_LOB_DATA_:
          {
            // --currLobLen = retrieve length of current lob data 
            // --lobDataLen = length of source data to be updated
            // --lobData    = source data that will be updated
            //
            // --position_  = position where data will be updated from
            //
            // --compute length of lob data after update. 
            //     maxof (position+lobDataLen, currLobLen)
            // --updatedData = allocate updated data space.
            //
            // --retrieve curr lob data in updatedData
            // --overwrite it at pos with with dataPtr
            // 
            // --replace current data with updatedData(LOB_DML_UPDATE_APPEND)
            //   -- this will delete current data and insert updated data
            //

            // retrieve current lob data len
            Int64 currLobDataLen = 0;
            retcode = lobOperV2_.dmlInterface(
                 lobGlobs,
                 lobHandle_, lobHandleLen_,
                 ExpLobOperV2::LOB_DML_SELECT_LOBLENGTH,
		...
		break;
            ...
            Int64 updLobDataLen = MAXOF(position_+lobDataLen_, currLobDataLen);
            char *updLobData = new(getMyHeap()) char[updLobDataLen];

            Int64 lobLen =  currLobDataLen;
            retcode = lobOperV2_.dmlInterface(lobGlobs,
                                              lobHandle_, lobHandleLen_,
            ...
            str_cpy_all(&updLobData[position_], lobData_, lobDataLen_);
            ...
            str_sprintf(statusString_," Updated %ld bytes of LOB data at position %ld. Result len is %ld",
                        lobDataLen_, position_, updLobDataLen);
            step_ = RETURN_STATUS_;
            break;
          }
          break;
	...
	case EMPTY_:
	  {
	    ...
	    lobType_ =  lobTdb().lobStorageType_;
