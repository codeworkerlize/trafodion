						 ...
						 NABoolean dedicated)
{
  ...
  ExUdrServer *udrServ =
    context->acquireUdrServer(runtimeOptions,
                              optionDelimiters,
                              getThreadInfo(),
                              dedicated);
...
{
  if ( cachedMyFragId_ == UINT_MAX )
     cachedMyFragId_ = 
      espFragInstanceDir_->findKey(myHandle_).getFragId();
...
Lng32 ExEspStmtGlobals::getMyInstanceNumber() const
{
  ...
  if (cachedMyInstanceNum_ >= 0)
    return cachedMyInstanceNum_;
  ...
  for (Int32 i = 0;
       ...
       i < numInstances AND cachedMyInstanceNum_ < 0;
       i++)
    {
...
ExEspInstanceThread * ExEspStmtGlobals::getThreadInfo()
{
  if (multiThreadedEsp_)
    {
      ExEspInstanceThread *result =
        espFragInstanceDir_->getEspInstanceThread(myHandle_);
