...
void *SqlBufferBase::operator new(size_t size, char *s) { return s; }

SqlBufferBase::moveStatus SqlBufferBase::moveInSendOrReplyData(
    NABoolean isSend,         // TRUE = send
...
NABoolean SqlBufferBase::moveOutSendOrReplyData(NABoolean isSend, void *currQState, tupp &outTupp,
                                                ControlInfo **controlInfo, ComDiagsArea **diagsArea,
                                                ExStatisticsArea **statsArea, long *numStatsBytes,
                                                NABoolean noStateChange, NABoolean unpackDA, CollHeap *heap) {
  return FALSE;
}

// positions to the "tupp_desc_num"th tupp descriptor.
void SqlBufferBase::position(int){};
...
tupp_descriptor *SqlBufferBase::add_tuple_desc(int tup_data_size) { return NULL; }
...
short SqlBuffer::freeBuffer() {
  ...
  if (bufferStatus_ == IN_USE) return 0;
  ...
  if ((memcmp(this, &sv_sb_normal, sizeof(char *)) == 0) || (memcmp(this, &sv_sb_dense, sizeof(char *)) == 0) ||
      (memcmp(this, &sv_sb_olt_small, sizeof(char *)) == 0) || (memcmp(this, &sv_sb_olt, sizeof(char *)) == 0)) {
    ;
  } else {
  ...
  if (freeState && (bufferStatus_ != EMPTY)) init(sizeInBytes_);
...
short SqlBuffer::space_available(int tup_data_size) {
  if (freeSpace_ >= (int)(tup_data_size + sizeof(tupp_descriptor)))
    return -1;
  else
    return 0;
};

tupp_descriptor *SqlBuffer::getTuppDescriptor(int tupp_desc_num) { return NULL; }

// print buffer info. For debugging
void SqlBuffer::printInfo() { printInfo(cerr); }

void SqlBuffer::printInfo(ostream &out) {
  out << "SqlBuffer=" << this << " ,Status: " << (int)bufferStatus_ << " type: " << std::hex << bufType() << std::dec
      << " maxTuppDesc: " << maxTuppDesc_ << " freeSpace: " << freeSpace_ << endl;
  int refcount = 0;
  int firstref = -1;
  ...
  out << "Ref tupps: " << refcount << " first: " << firstref << endl;
};
...
SqlBuffer::moveStatus SqlBuffer::moveInSendOrReplyData(
  ...
  if ((srFlags() == REPLY_GET_UNIQUE_REQUEST) && (NOT doMoveData)) {
    doMoveData = TRUE;
    projRowLen = 0;
  }

  if ((getTotalTuppDescs() == 0) || (diagsArea != NULL) ||
      ((isSend == TRUE) && (NOT(srControlInfo_.getDownState() == *((down_state *)currQState))))) {
    moveControlInfo = TRUE;
  }

  if (moveControlInfo == FALSE) {
    if (srFlags() == REPLY_GET_UNIQUE_REQUEST) {
      if ((((up_state *)currQState)->parentIndex - srControlInfo_.getUpState().parentIndex) == 1) {
        if (NOT doMoveData) {
          doMoveData = TRUE;
          projRowLen = 0;
        }
      } else if (((up_state *)currQState)->parentIndex != srControlInfo_.getUpState().parentIndex) {
        moveControlInfo = TRUE;
      } else {
        doMoveData = FALSE;
      }
    } else {
      if ((doMoveControl) || ((isSend == FALSE) && (NOT(srControlInfo_.getUpState() == *((up_state *)currQState)))))
        moveControlInfo = TRUE;
    }
  }
  ...
  if (doMoveData == TRUE) {
...
#if (defined(_DEBUG))
        char txt[] = "exchange";
        sql_buffer_pool::logDefragInfo(txt, SqlBufferGetTuppSize(projRowLen, bufType()),
                                       SqlBufferGetTuppSize(rowLen, bufType()), getFreeSpace(), this,
                                       this->getTotalTuppDescs(), tcb);
#endif
      }
    }
    ...
    if (!tdesc) {
      ...
      if (controlInfoMoved == TRUE) remove_tuple_desc();
    ...
    if (outTdesc != NULL) *outTdesc = tdesc;
  }

#ifndef UDRSERV_BUILD
  if ((expr != NULL) && (tdesc != NULL)) {
    destAtp->getTupp(tuppIndex) = tdesc;
    if (diagsArea != NULL) daMark = diagsArea->mark();
    ...
    if (defragLength != 0) {
      str_cpy_all(tdesc->getTupleAddress(), defragTd->getTupleAddress(), defragLength);
      ...
      if (expr_retcode == ex_expr::EXPR_ERROR) {
        if (isSend) {
          ...
          if (workAtp == destAtp) destAtp->release();
      ...
      {       // resize the row if needed
        if (rowLenPtr && (*rowLenPtr != projRowLen)) resize_tupp_desc(*rowLenPtr, tdesc->getTupleAddress());
      }
    }

    if (                                                                 // Has a new diags area appeared?
        (atp1->getDiagsArea() != NULL) && (atp1HadNoDiagsAreaBefore) &&  // And we are sending diags areas externally?
        (useExternalDA) &&                                               // And state transistion from no DA to yes DA?
    ...
    if (backoutAndRestart) {
      ...
      return moveInSendOrReplyData(isSend,
                                   ...
                                   doMoveData, currQState, controlInfoLen, controlInfo, projRowLen, outTdesc, NULL,
                                   diagsDesc, expr, atp1, workAtp, destAtp, tuppIndex, doMoveStats, statsArea,
                                   statsDesc, useExternalDA,
    ...
    if (expr_retcode == ex_expr::EXPR_ERROR) {
      if (isSend == FALSE) {
        ((ControlInfo *)(cdesc->getTupleAddress()))->getUpState().status = ex_queue::Q_SQLERROR;
  ...
  if ((diagsArea != NULL && useExternalDA == FALSE) && (diagsArea->getNumber(DgSqlCode::ERROR_) == 0) &&
    ...
    tupp_descriptor *tempDiagsDesc = add_tuple_desc(diagsArea->packedLength());
    if (tempDiagsDesc == NULL) {
      // don't have enough space in this input buffer for diags.
      // remove the tuple data desc, if any.
      if (tdesc != NULL) remove_tuple_desc();

      // remove the control desc.
      if (controlInfoMoved == TRUE) remove_tuple_desc();

      if (daMark >= 0) diagsArea->rewind(daMark);
    ...
    if (diagsDesc) *diagsDesc = tempDiagsDesc;
  }
  ...
  if (controlInfoMoved && currQState) {
    ...
    if (isSend)
      ...
      if (setUpStatusToError) srControlInfo_.getUpState().status = ex_queue::Q_SQLERROR;
    }
  } else if ((srFlags() == REPLY_GET_UNIQUE_REQUEST) && (doMoveData) && (NOT isSend) && (currQState)) {
    srControlInfo_.getUpState() = *(up_state *)currQState;
  }
...
NABoolean SqlBuffer::moveOutSendOrReplyData(NABoolean isSend, void *currQState, tupp &outTupp,
                                            ControlInfo **controlInfo, ComDiagsArea **diagsArea,
                                            ExStatisticsArea **statsArea, long *numStatsBytes, NABoolean noStateChange,
                                            NABoolean unpackDA, CollHeap *heap) {
  if (atEOTD()) return TRUE;
    ...
    switch (bufType()) {
      case NORMAL_: {
        ...
        if (tuppDesc->isControlTuple()) {
          if (flags_ & IGNORE_CONTROL_FLAG) break;  // case done and break out.
          if (setupSrControlInfo(isSend, tuppDesc) == DEFER_MORE_TUPPS) break;
          if (NOT noStateChange) tuppDesc->resetCommFlags();
          if (++tuppDescIndex_ < maxTuppDesc_)
            tuppDesc = tupleDesc(tuppDescIndex_);
          else
  ...
  switch (srFlags_) {
      ...
      if (dataTuppDesc) {
        if (srControlInfo_.getUpState().status == ex_queue::Q_OK_MMORE) {
          ...
          if (castToSqlBufferDense())  // DENSE_ buffer
            castToSqlBufferDense()->currTupleDesc() = dataTuppDescInfo;
          ...
          flags_ |= IGNORE_CONTROL_FLAG;
        } else if (srControlInfo_.getUpState().status == ex_queue::Q_NO_DATA) {
    ...
    case REPLY_GET_SUBSET_REQUEST: {
      if (dataTuppDesc) {
        ...
        if (srControlInfo_.getUpState().getMatchNo() != OverflowedMatchNo) {
          long matchNo64 = srControlInfo_.getUpState().getMatchNo();
    ...
    case REPLY_VSBB_INSERT_REQUEST: {
      ...
      if ((diagsTuppDesc) && (unpackDA) && (heap)) {
        ComDiagsArea *d = (ComDiagsArea *)(diagsTuppDesc->getTupleAddress());
        unpackedDiagsArea = ComDiagsArea::allocate(heap);
        unpackedDiagsArea->unpackObj(d->getType(), d->getVersion(),
                                     TRUE,  // sameEndianness,
                                     0,     // dummy for now...
      ...
      if (srControlInfo_.getUpState().matchNo > 0) {
        if (flags_ & RETURN_ROWCOUNT) {
          if (flags_ & ROWCOUNT_RETURNED)
            ...
            ((up_state *)currQState)->matchNo = (int)unpackedDiagsArea->getRowCount();
      ...
      if (srControlInfo_.getUpState().downIndex >= (queue_index)srControlInfo_.getUpState().matchNo) {
        flags_ &= ~IGNORE_CONTROL_FLAG;
      }
    } break;
...
NABoolean SqlBuffer::findAndCancel(queue_index pindex, NABoolean startFromBeginning) {
  ...
  int saveTuppDescIndex = tuppDescIndex_;
  NABoolean wasFound = FALSE;

  if (startFromBeginning) position();

  while (!atEOTD()) {
    if (getCurr()->isControlTuple()) {
      down_state *ds = &((ControlInfo *)(getCurr()->getTupleAddress()))->getDownState();
...
void SqlBuffer::setNumInvalidTuplesUptoNow(int val) { UdrExeAssert(0, "Do not call the base class method"); }

void SqlBuffer::incNumInvalidTuplesUptoNow() { UdrExeAssert(0, "Do not call the base class method"); }

void SqlBuffer::decNumInvalidTuplesUptoNow() { UdrExeAssert(0, "Do not call the base class method"); }
...
void SqlBufferNormal::init(ULng32 in_size_in_bytes, NABoolean clear) {
  ...
  if (clear) memset((char *)this + dataOffset_, 0, freeSpace_);
...
tupp_descriptor *SqlBufferNormal::allocate_tuple_desc(int tup_data_size) {
  ...
  int freeSpaceNeeded = rounded_size + sizeof(tupp_descriptor);
  ...
  if (bufferStatus_ == EMPTY) bufferStatus_ = PARTIAL;

  return td;
}

// adds a tuple descriptor
tupp_descriptor *SqlBufferNormal::add_tuple_desc(int tup_data_size) { return allocate_tuple_desc(tup_data_size); }
...
void SqlBufferNormal::remove_tuple_desc() {
  ...
  if (maxTuppDesc_ > 0) {
    ...
    if (maxTuppDesc_ == 0) bufferStatus_ = EMPTY;
  }
}
...
short SqlBufferNormal::isFree() {
  ...
  if (bufferStatus_ == EMPTY) return -1;
...
void SqlBufferNormal::position() { tuppDescIndex_ = 0; }

// positions to the "tupp_desc_num"th tupp descriptor.
void SqlBufferNormal::position(int tupp_desc_num) { tuppDescIndex_ = (unsigned short)tupp_desc_num; }

// print buffer info. For debugging
void SqlBufferNormal::printInfo(ostream &out) {
  out << this << " Status: " << bufferStatus_ << " maxTuppDesc: " << maxTuppDesc_ << " freeSpace: " << freeSpace_
...
tupp_descriptor *SqlBufferNormal::getTuppDescriptor(int tupp_desc_num) {
...
void SqlBufferNormal::pack() {
  ...
  while (i < maxTuppDesc_) {
    tupleDesc(i)->setTupleOffset((int)((char *)tupleDesc(i)->getTupleAddress() - (char *)this));
...
void SqlBufferNormal::unpack() {
  UInt32 i = 0;
  while (i < maxTuppDesc_) {
    tupleDesc(i)->setTupleAddress((char *)this + tupleDesc(i)->getTupleOffset());
...
unsigned short SqlBufferNormal::getNumInvalidTuplesUptoNow() { return numInvalidTuplesUptoCurrentPosition_; }

void SqlBufferNormal::setNumInvalidTuplesUptoNow(int val) {
...
void SqlBufferNormal::decNumInvalidTuplesUptoNow() { numInvalidTuplesUptoCurrentPosition_--; }
...
tupp_descriptor *SqlBufferDense::add_tuple_desc(int tup_data_size) {
  ...
  if (lastTupleDesc())
    tdi = (TupleDescInfo *)(lastTupleDesc()->tupleDesc()->getTupleAddress() +
                            ROUND8(lastTupleDesc()->tupleDesc()->getAllocatedSize()));
  else
    tdi = firstTupleDesc();

  tupp_descriptor *td = tdi->tupleDesc();
  td->init(tup_data_size, 0, (char *)td + td_size);

  setPrevTupleDesc(tdi, lastTupleDesc());

  if (lastTupleDesc()) setNextTupleDesc(lastTupleDesc(), tdi);
...
tupp_descriptor *SqlBufferDense::allocate_tuple_desc(int tup_data_size) { return add_tuple_desc(tup_data_size); }
...
void SqlBufferDense::remove_tuple_desc() {
  if (maxTuppDesc_ > 0) {
    ...
    maxTuppDesc_--;
    freeSpace_ += (ROUND8(sizeof(tupp_descriptor)) + ROUND8(lastTupleDesc()->tupleDesc()->getAllocatedSize()));
...
void SqlBufferDense::position(int tupp_desc_num) {
  position();
  while ((tuppDescIndex_ != tupp_desc_num) && (tuppDescIndex_ < maxTuppDesc_)) advanceDense();
}
...
// is tupp_desc_num = 1.
// Returns NULL, if tupp_desc_num is greater than maxTuppDesc_.
tupp_descriptor *SqlBufferDense::getTuppDescriptor(int tupp_desc_num) {
  if (tupp_desc_num > maxTuppDesc_)
    return 0;
  else
    return tupleDesc(tupp_desc_num - 1);
...
void TupleDescInfo::pack(Long base) { tupleDesc()->setTupleOffset((Long)tupleDesc()->getTupleAddress() - base); }

void TupleDescInfo::unpack(Long base) { tupleDesc()->setTupleAddress((char *)base + tupleDesc()->getTupleOffset()); }
...
void SqlBufferDense::pack() {
  ...
  if (lastTupleDesc()) lastTupleDesc() = (TupleDescInfo *)((char *)lastTupleDesc() - (char *)this);

  if (currTupleDesc()) currTupleDesc() = (TupleDescInfo *)((char *)currTupleDesc() - (char *)this);
}

void SqlBufferDense::unpack() {
  if (lastTupleDesc()) lastTupleDesc() = (TupleDescInfo *)((Long)lastTupleDesc() + (char *)this);
...
unsigned short SqlBufferDense::getNumInvalidTuplesUptoNow() { return numInvalidTuplesUptoCurrentPosition_; }

void SqlBufferDense::setNumInvalidTuplesUptoNow(int val) {
  UdrExeAssert(val < USHRT_MAX, "total invalid tupps in buffer is larger than USHRT_MAX");
  numInvalidTuplesUptoCurrentPosition_ = (unsigned short)val;
}
...
void SqlBufferDense::decNumInvalidTuplesUptoNow() { numInvalidTuplesUptoCurrentPosition_--; }

void SqlBufferDense::printInfo(ostream &out) {
...
void SqlBuffer::integrityCheck(NABoolean ignore_omm_check) {
  const int savedTuppDescIndex = getProcessedTuppDescs();
  tupp p;
  ControlInfo *ci = 0;
  ComDiagsArea *d = NULL;
  ExStatisticsArea *s = NULL;
  long numStatsBytes = 0;
  up_state upState;
  enum okMMoreType { NOT_KNOWN_ = 0, YES_ = 1, NO_ = 3 } okMMoreHasData = NOT_KNOWN_;

  position();

  while (atEOTD() != -1) {
    moveOutSendOrReplyData(FALSE,  // reply data
                           &upState, p, &ci, &d, &s, &numStatsBytes,
                           TRUE,   //  noStateChange
    ...
    switch (upState.status) {
      case ex_queue::Q_NO_DATA: {
        if (ci->getIsDataRowPresent()) FAILURE;
        break;
      }
      case ex_queue::Q_GET_DONE:
      case ex_queue::Q_REC_SKIPPED: {
        if (ci->getIsDataRowPresent()) FAILURE;
        if (ci->getIsDiagsAreaPresent()) FAILURE;
        break;
      }
      case ex_queue::Q_OK_MMORE: {
        if (ignore_omm_check) break;
        ...
        switch (okMMoreHasData) {
          ...
          case NO_: {
            if (p.isAllocated() == TRUE) FAILURE;
            break;
          }
          case YES_: {
            if (p.isAllocated() == FALSE) FAILURE;
            break;
          }
          default: {
            FAILURE;
...
////////////////////////////////////////////////////////////////////////////
sql_buffer_pool::sql_buffer_pool(int numberOfBuffers, int bufferSize, CollHeap *space,
                                 SqlBufferBase::BufferType bufType)
    : bufType_(bufType), defragBuffer_(NULL), defragTd_(NULL) {
...
SqlBufferBase *sql_buffer_pool::addBuffer(int totalBufferSize, SqlBufferBase::BufferType bufType, CollHeap *space) {
  ...
  int rounded_size = ROUND8(totalBufferSize);
...
SqlBufferBase *sql_buffer_pool::addBuffer(Queue *bufferList, int totalBufferSize, SqlBufferBase::BufferType bufType,
                                          bool failureIsFatal) {
  // allocate a size that is divisible by 8, otherwise the data that
  // is allocated from the end of the buffer won't be aligned properly
  int rounded_size = ROUND8(totalBufferSize);
  ...
  if (bufferList == dynBufferList_) currNumberOfBuffers_++;
...
tupp_descriptor *sql_buffer_pool::addDefragTuppDescriptor(int dataSize) {
  if (defragTd_) {
    return defragTd_;
  }
  if (!defragBuffer_) {
    int neededBufferSize = (int)SqlBufferNeededSize(1, dataSize);

    // allocate a size that is divisible by 8, otherwise the data that
    // is allocated from the end of the buffer won't be aligned properly
    int rounded_size = ROUND8(neededBufferSize);
...
SqlBufferBase *sql_buffer_pool::addBuffer(int totalBufferSize, bool failureIsFatal) {
...
SqlBufferBase *sql_buffer_pool::addBuffer(int numTupps, int recordLength) {
  return addBuffer((int)SqlBufferNeededSize(
      numTupps, recordLength, ((bufType_ == SqlBufferBase::DENSE_) ? SqlBuffer::DENSE_ : SqlBuffer::NORMAL_)));
}

SqlBufferBase *sql_buffer_pool::get_free_buffer(int freeSpace) { return getBuffer(freeSpace, -1); }

// RETURNS: 0, if tuple found. -1, if not found.
short sql_buffer_pool::get_free_tuple(tupp &tp, int tupDataSize, SqlBuffer **buf) {
  if (buf) *buf = NULL;
  ...
  if (tupDataSize <= 0) tupDataSize = 8;

  int neededSpace = SqlBufferGetTuppSize(tupDataSize, bufType_);
  ...
  {
    tupp_descriptor *tuple_desc = currBuffer->allocate_tuple_desc(tupDataSize);

    // failed to set tp to a tuple_descriptor from the pool
    if (!tuple_desc) return -1;

    tuple_desc->setReferenceCount(0);

    tp = tuple_desc;

    if (buf) *buf = currBuffer;
...
short sql_buffer_pool::currentBufferHasEnoughSpace(int tupDataSize) {
  SqlBuffer *currBuffer;
...
SqlBuffer *sql_buffer_pool::getCurrentBuffer() { return ((SqlBuffer *)(activeBufferList()->getCurr())); }
...
tupp_descriptor *sql_buffer_pool::get_free_tupp_descriptor(int tupDataSize, SqlBuffer **buf) {
  if (buf) *buf = NULL;
...
void sql_buffer_pool::printAllBufferInfo() {
  staticBufferList_->position();
  SqlBuffer *buf;
  while (buf = (SqlBuffer *)staticBufferList_->getNext()) buf->printInfo();
...
SqlBufferBase *sql_buffer_pool::getBuffer(int freeSpace, int mustBeEmpty) {
  ...
  if (!result) {
    if (staticMode()) {
      ...
      int neededStaticBufferSize;

      if (freeSpace > 0)
        neededStaticBufferSize = (int)SqlBufferNeededSize(10, freeSpace, bufType_);
      else
        neededStaticBufferSize = defaultBufferSize_;

      result = addBuffer(staticBufferList_, neededStaticBufferSize, bufType_);
    } else if (currNumberOfBuffers_ < maxNumberOfBuffers_ || dynamicUnlimited()) {
      // The requested size may be wrong if the buffer size isn't computed
      // correctly in codegen.
      int bufferHeaderSize = (int)SqlBufferHeaderSize(bufType_);
      ...
      int buffSize = MAXOF(freeSpace + bufferHeaderSize, defaultBufferSize_);
      ...
      UdrExeAssert((result != NULL) && (get_number_of_buffers() != 0), "Buffer size not computed correctly at codegen");
    }
  }
...
void sql_buffer_pool::getUsedMemorySize(UInt32 &staticMemSize, UInt32 &dynMemSize) {
...
SqlBufferHeader::moveStatus sql_buffer_pool::moveIn(atp_struct *atp1, atp_struct *atp2, UInt16 tuppIndex,
                                                    int tupDataSize, ex_expr_base *moveExpr,
                                                    NABoolean addBufferIfNeeded, int bufferSize) {
  if (defragTd_ && !currentBufferHasEnoughSpace(tupDataSize)) {
    ...
    if (!get_free_tuple(atp2->getTupp(tuppIndex), defRowLen)) {
      ...
      sql_buffer_pool::logDefragInfo(txt, SqlBufferGetTuppSize(tupDataSize, buf->bufType()),
                                     SqlBufferGetTuppSize(defRowLen, buf->bufType()), buf->getFreeSpace(), buf,
      ...
      str_cpy_all(dataPointer, defragDataPointer, defRowLen);
  ...
  if (rowLen != maxRowLen) {
    resizeLastTuple(rowLen, atp2->getTupp(tuppIndex).getDataPointer());
  }
  return SqlBufferHeader::MOVE_SUCCESS;
}

SqlBufferHeader::moveStatus sql_buffer_pool::moveIn(atp_struct *atp, UInt16 tuppIndex, int tupDataSize, char *srcData,
                                                    NABoolean addBufferIfNeeded, int bufferSize) {
  ...
  str_cpy_all(atp->getTupp(tuppIndex).getDataPointer(), srcData, tupDataSize);
...
SqlBufferBase::moveStatus SqlBufferOlt::moveInReplyData(NABoolean doMoveControl, NABoolean doMoveData, void *currQState,
  ...
  if ((((up_state *)currQState)->matchNo > 1) || (diagsArea) || (getContents() == ERROR_) ||
      (getContents() == DATA_WARNING_) || ((statsArea) && (NOT statsArea->smallStatsObj()))) {
    return moveInSendOrReplyData(FALSE,  // reply
                                 doMoveControl, doMoveData, currQState, 0, NULL, projRowLen, NULL, diagsArea, diagsDesc,
                                 expr, atp1, workAtp, destAtp, tuppIndex, doMoveStats, statsArea, statsDesc);
  }

  if (doMoveData) setDataProcessed(TRUE);

  setContainsOltSmall(TRUE);

  SqlBufferOltSmall *smallBuf = (SqlBufferOltSmall *)getReplyDataPtr();
  if ((doMoveData) || (doMoveControl && (NOT dataProcessed()))) {
    smallBuf->setBufType(SqlBufferHeader::OLT_SMALL_);
    smallBuf->init();
  ...
  if (((up_state *)currQState)->status == ex_queue::Q_STATS) {
    ...
    UdrExeAssert(doMoveStats && statsArea && statsDesc && *statsDesc, "Incorrect parameters for Q_STATS");
  ...
  if (doMoveData) {
    ...
    if (expr) {
      ...
      if (expr) rc = expr->eval(atp1, workAtp);
      ...
      if (atp1->getDiagsArea()) {
        ...
        return moveInSendOrReplyData(FALSE,  // reply
                                     doMoveControl, doMoveData, currQState, 0, NULL, projRowLen, NULL,
                                     atp1->getDiagsArea(), diagsDesc, expr, atp1, workAtp, destAtp, tuppIndex,
  ...
  if ((doMoveStats) && (statsArea) && (statsDesc) && (*statsDesc)) {
    ...
    *(short *)((char *)smallBuf + sizeInBytes_) = statsAreaLen;

    sizeInBytes_ += sizeof(short);

    char *statsAreaLoc = (statsArea->smallStatsObj() ? ((char *)smallBuf + sizeInBytes_)
...
SqlBufferBase::moveStatus SqlBufferOlt::moveInSendOrReplyData(
  ...
  if (isSend) return MOVE_ERROR;

  if ((!diagsArea) && (((up_state *)currQState)->matchNo <= 1) && (getContents() != ERROR_) &&
      (getContents() != DATA_WARNING_) && ((!statsArea) || (statsArea->smallStatsObj())))

  {
    return moveInReplyData(doMoveControl, doMoveData, currQState, projRowLen, diagsArea, diagsDesc, expr, atp1, workAtp,
                           destAtp, tuppIndex, doMoveStats, statsArea, statsDesc);
  }
  ...
  if ((!doMoveData) && (getContents() != NOTHING_YET_)) {
    ...
    if (getContents() == DATA_WARNING_) tdesc = getNextTuppDesc(tdesc);  // skip the warning tupp
  }
  ...
  if (doMoveData)  // (doMoveData) && (projLen > 0))
  {
    ...
    if (destAtp) destAtp->getTupp(tuppIndex) = tdesc;

    if ((expr != NULL) && ((expr->eval(atp1, workAtp)) == ex_expr::EXPR_ERROR)) {
      if ((destAtp) && (workAtp == destAtp)) destAtp->release();
    ...
    if (diagsArea == NULL) diagsArea = atp1->getDiagsArea();
  ...
  //  else if ((doMoveControl) && (NOT dataProcessed()))
  {
    if (((up_state *)currQState)->status == ex_queue::Q_STATS) {
      ...
      tdesc = getNextTuppDesc(tdesc, sizeof(int));
      *(int *)tdesc->getTupleAddress() = ((up_state *)currQState)->matchNo;
    }
  }
  ...
  if (diagsArea != NULL) {
    if (diagsArea->mainSQLCODE() < 0) {
      ...
      if (diagsArea->packedLength() + sizeInBytes_ > maxBufSize) {
        ...
        while (diagsArea->getNumber(DgSqlCode::WARNING_) > 1) diagsArea->deleteWarning(1);
...
NABoolean SqlBufferOlt::moveOutSendOrReplyData(NABoolean isSend, void *currQState, tupp &outTupp,
                                               ControlInfo **controlInfo, ComDiagsArea **diagsArea,
                                               ExStatisticsArea **statsArea, long *numStatsBytes,
                                               NABoolean noStateChange, NABoolean unpackDA, CollHeap *heap) {
  tupp_descriptor *tDesc = NULL;
  ...
  switch (getContents()) {
    ...
    case NODATA_ROWCOUNT_: {
      ...
      tDesc = getNextTuppDesc(tDesc);
      ((up_state *)currQState)->matchNo = *(int *)tDesc->getTupleAddress();
    } break;
...
// add a new tuple descriptor to the end of the buffer
tupp_descriptor *SqlBufferOlt::add_tuple_desc(int tup_data_size) { return NULL; }
...
NABoolean SqlBufferOltSmall::moveOutSendData(tupp &outTupp, ULng32 returnedRowLen) {
  if (sendData()) {
    ...
    tupp_descriptor *td = (tupp_descriptor *)((char *)this + sizeof(SqlBufferOltSmall) + ROUND8(returnedRowLen));
...
#if (defined(_DEBUG))
void sql_buffer_pool::logDefragInfo(char *txt, int neededSpace, int actNeededSpace, int freeBuffSpace, void *p,
                                    int NumRowsInBuff, ex_tcb *tcb) {
  ...
  if (envCifLoggingLocation) {
    ...
    fprintf(p2, "%s      %s explainId: %d buffer: %p Neededspace: %d AcualSpace: %d freeSapce: %d NumRowsInBuff: %d\n",
            asctime(timeinfo), tcb ? tcb->getTdb()->getNodeName() : txt, tcb ? tcb->getTdb()->getExplainNodeId() : -1,
