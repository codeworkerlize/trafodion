...
void SqlBuffer::printInfo(ostream& out) {
  out << "SqlBuffer=" << this << " ,Status: " << (int)bufferStatus_
    << " type: " << std::hex << bufType() << std::dec
    << " maxTuppDesc: " << maxTuppDesc_
    << " freeSpace: " << freeSpace_ << endl;
  Lng32 refcount = 0;
  Lng32 firstref = -1;
...
void SqlBufferDense::printInfo(ostream& out) 
{
  ...
  if (maxTuppDesc_ > 0)
    ...
    while (temp)
      {
        out << "tupp[" << i++ << "]: allocatedSize=" 
            << temp->tupleDesc()->getAllocatedSize() << endl;
