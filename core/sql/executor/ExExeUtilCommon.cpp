...
Int64 ExExeUtilTcb::getObjectFlags(Int64 objectUID)
{
  ...
  exeQuery.format("select flags from %s.\"%s\".%s where object_uid = %ld",
                  TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA,SEABASE_OBJECTS, objectUID);
 ...
 )
{
  ...
  if ((rc = extractedPartsObj_->extractParts(numParts, parts, noValidate)) != 0 ||
      (numParts != 3))
    {
