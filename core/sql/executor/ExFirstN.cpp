...
ex_tcb *ExFirstNTdb::build(ex_globals *glob) {
  ...
  ExFirstNTcb *firstn_tcb = new (glob->getSpace()) ExFirstNTcb(*this, *child_tcb, glob);
...
ExFirstNTcb::ExFirstNTcb(const ExFirstNTdb &firstn_tdb,
                         ...
                         ex_globals *glob)
    : ex_tcb(firstn_tdb, 1, glob), step_(INITIAL_) {
  childTcb_ = &child_tcb;
  ...
  pool_ = new (space) sql_buffer_pool(firstn_tdb.numBuffers_, firstn_tdb.bufferSize_, space);

  // get the queue that child use to communicate with me
  qchild_ = child_tcb.getParentQueue();

  // Allocate the queue to communicate with parent
  qparent_.down = new (space) ex_queue(ex_queue::DOWN_QUEUE, firstn_tdb.queueSizeDown_, firstn_tdb.criDescDown_, space);

  // Allocate the private state in each entry of the down queue
  ExFirstNPrivateState p(this);
  qparent_.down->allocatePstate(&p, this);

  qparent_.up = new (space) ex_queue(ex_queue::UP_QUEUE, firstn_tdb.queueSizeUp_, firstn_tdb.criDescUp_, space);
  ...
  if (firstn_tdb.workCriDesc_) {
    ...
    workAtp_->getTupp(firstn_tdb.workCriDesc_->noTuples() - 1).setDataPointer((char *)&firstNParamVal_);
  }

  if (firstn_tdb.firstNRowsExpr_) {
    firstn_tdb.firstNRowsExpr_->fixup(0, getExpressionMode(), this, space, heap, FALSE, glob);
  }
};

ExFirstNTcb::~ExFirstNTcb() { freeResources(); };
    ...
    switch (step_) {
      case INITIAL_: {
        ...
        if (firstnTdb().firstNRowsExpr_) {
          ex_expr::exp_return_type evalRetCode = firstnTdb().firstNRowsExpr_->eval(pentry_down->getAtp(), workAtp_);
        ...
        if (effectiveFirstN_ >= 0) {
          ...
          if ((pentry_down->downState.request != ex_queue::GET_N) ||
            ...
            centry->downState.requestValue = MINOF(pentry_down->downState.requestValue, effectiveFirstN_);
          }
      ...
      case PROCESS_FIRSTN_: {
        if ((qchild_.up->isEmpty()) || (qparent_.up->isFull())) return WORK_OK;
        ...
        switch (cUpEntry->upState.status) {
          case ex_queue::Q_OK_MMORE: {
            ...
            } else if (requestedLastNRows_ == 1)  // last 1
            {
              ...
              if (qchild_.up->getLength() < 1 + 1) return WORK_OK;

              queue_index headIndex = qchild_.up->getHeadIndex();

              ex_queue_entry *nextCupEntry = qchild_.up->getQueueEntry(headIndex + 1);
