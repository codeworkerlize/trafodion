       ...
       flags_(0)
{
  ...
  if (currContext_ == NULL)
     currContext_ = GetCliGlobals()->currContext();
}

ExeCliInterface::~ExeCliInterface()
{
   dealloc();
   if (needToDestroyHeap_)
     delete heap_;
}
                               ...
                               Int32 *retGenCodeSize)
{
	...
	*outputBuf = new (heap_) char[outputDatalen_];
      ...
      for (entry=1; entry <= num_output_entries; entry++)
	{
	  ...
	  if (retcode != SUCCESS)
        ...
        if (outputVarPtrList)
           outputVarPtrList->insert((char*) &(*outputBuf)[outputAttrs_[entry-1].varOffset_]);
        }
    }
  ...
  if (stmtStr)
    {
      str_cpy_and_null(sqlStmtStr_, stmtStr, MINOF(1000, strlen(stmtStr)),
                       '\0', ' ', TRUE);
    }
...
Lng32 ExeCliInterface::setInputValue(short entry, char *ptr, Lng32 len)
{
  ...
  Lng32 indOffset = -1;
  Lng32 varOffset = -1;
  getAttributes(entry, 1 /* forInput */,
                fsType, length, vcIndLen, &indOffset, &varOffset);
  if (indOffset >= 0) {
    if (ptr == NULL) {
				    ...
				    Lng32 *indOffset, Lng32 *varOffset)
{
  ...
  if (forInput)
	...
	{
	  return getAttributes(entry, forInput, temp, temp, temp,
			       indOffset, varOffset);
	}
    }
  else
                                                ...
                                                )
{
  ...
  if ((retGenCodeSize) && (retcode == -CLI_GENCODE_BUFFER_TOO_SMALL))
    return retcode;
...
Lng32 ExeCliInterface::getGeneratedCode(char *genCodeBuf,
                                        Int32 genCodeSize)
{
  Lng32 retcode = 0;

  retcode = SQL_EXEC_Prepare2(stmt_, NULL, 
...
Lng32 ExeCliInterface::beginXn()
{
  ...
  if (currContext_->getTransaction() != NULL) 
     currContext_->getTransaction()->disableAutoCommit();
  return retcode;
}
...
Lng32 ExeCliInterface::commitXn()
{
  ...
  if (currContext_->getTransaction() != NULL) 
     currContext_->getTransaction()->enableAutoCommit();
  return retcode;
}
...
Lng32 ExeCliInterface::holdAndSetCQD(const char * defaultName, 
...
Lng32 ExeCliInterface::holdAndSetCQDs(const char * holdStr,
                                      const char * setStr,
                                      ComDiagsArea *globalDiags)
{
  Lng32 cliRC;
...
Lng32 ExeCliInterface::restoreCQDs(const char * restoreStr,
                                   ComDiagsArea *globalDiags)
