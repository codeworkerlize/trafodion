	...
	case SCAN_OPEN:
	  {
		...
		tcb_->fixObjName4PartTbl(tcb_->hbaseAccessTdb().getTableName(), 
	                             tcb_->hbaseAccessTdb().getDataUIDName(), 
	    ...
	    retcode = tcb_->ehi_->scanOpen(tcb_->table_, 
					   ...
					   tcb_->hbaseAccessTdb().skipReadConflict(),
	    ...
	    if (recordCostTh_ >= 0)
	      {
	        ...
	        if (time2 >= recordCostTh_)
	          QRWARN("ExHbaseScanTaskTcb SCAN_OPEN PID %d txID %ld CC %d ATC %.2f TTC %.2f TC %ld  %s",
	                  pid, transId, callCount[0], (sumCost[0] / (callCount[0] ? callCount[0] : 1)), sumCost[0],  time2, tcb_->table_.val);
	      }
	    ...
	    retcode = tcb_->ehi_->nextRow();
            if (recordCostTh_ >= 0)
              { 
                ...
                if (time2 >= recordCostTh_)
                  QRWARN("ExHbaseScanTaskTcb NEXTROW PID %d txID %ld CC %d ATC %.2f TTC %.2f TC %ld  %s",
                          pid, transId, callCount[1], (sumCost[1] / (callCount[1] ? callCount[1] : 1)), sumCost[1],  time2, tcb_->table_.val);
              }
	       ...
	       tcb_->colName_, tcb_->colVal_, tcb_->colTS_);
            if (recordCostTh_ >= 0)
              { 
                ...
                if (time2 >= recordCostTh_)
                  QRWARN("ExHbaseScanTaskTcb NEXTCELL PID %d txID %ld CC %d ATC %.2f TTC %.2f TC %ld  %s",
                          pid, transId, callCount[2], (sumCost[2] / (callCount[2] ? callCount[2] : 1)), sumCost[2],  time2, tcb_->table_.val);
              }
	...
	case RETURN_ROW:
	  {
	    if (tcb_->moveRowToUpQueue(tcb_->convertRow_, 
                      tcb_->hbaseAccessTdb().optLargVar() ? tcb_->convertRowLen_ : tcb_->hbaseAccessTdb().convertRowLen(), &rc, FALSE))
	      return 1;
            ...
            if (recordCostTh_ >= 0)
              {
                ...
                if (time2 >= recordCostTh_)
                  QRWARN("ExHbaseScanTaskTcb SCAN_CLOSE PID %d txID %ld CC %d ATC %.2f TTC %.2f TC %ld  %s",
                          pid, transId, callCount[3], (sumCost[3] / (callCount[3] ? callCount[3] : 1)), sumCost[3],  time2, tcb_->table_.val);
              }
	    ...
	    if (recordCostTh_ >= 0)
	      {
	        ...
	        if (time2 >= recordCostTh_)
	          QRWARN("ExHbaseScanRowwiseTaskTcb SCAN_OPEN PID %d txID %ld CC %d ATC %.2f TTC %.2f TC %ld  %s",
	                  pid, transId, callCount[4], (sumCost[4] / (callCount[4] ? callCount[4] : 1)), sumCost[4], time2, tcb_->table_.val);
	      }
	    if (tcb_->setupError(retcode, "ExpHbaseInterface::scanOpen"))
	      step_ = HANDLE_ERROR;
	    else
            ...
            if (recordCostTh_ >= 0)
              {
                ...
                if (time2 >= recordCostTh_)
                  QRWARN("ExHbaseScanRowwiseTaskTcb NEXT_ROW PID %d txID %ld CC %d ATC %.2f TTC %.2f TC %ld  %s",
                          pid, transId, callCount[5], (sumCost[5] / (callCount[5] ? callCount[5] : 1)), sumCost[5], time2, tcb_->table_.val);
              }
	    if (retcode == HBASE_ACCESS_EOD || retcode == HBASE_ACCESS_EOR)
	    {
	       step_ = SCAN_CLOSE;
            ...
            if (recordCostTh_ >= 0)
              {
                ...
                if (time2 >= recordCostTh_)
                  QRWARN("ExHbaseScanRowwiseTaskTcb NEXT_CELL PID %d txID %ld CC %d ATC %.2f TTC %.2f TC %ld  %s",
                          pid, transId, callCount[6], (sumCost[6] / (callCount[6] ? callCount[6] : 1)), sumCost[6], time2, tcb_->table_.val);
              }
	    if (retcode == HBASE_ACCESS_EOD)
	    { 
	       step_ = CREATE_ROW;
	    ...
	    retcode = tcb_->ehi_->scanClose();
            if (recordCostTh_ >= 0)
              { 
                ...
                if (time2 >= recordCostTh_)
                  QRWARN("ExHbaseScanRowwiseTaskTcb SCAN_CLOSE PID %d txID %ld CC %d ATC %.2f TTC %.2f TC %ld  %s",
                          pid, transId, callCount[7], (sumCost[7] / (callCount[7] ? callCount[7] : 1)), sumCost[7], time2, tcb_->table_.val);
              }
	    if (tcb_->setupError(retcode, "ExpHbaseInterface::scanClose"))
	      step_ = HANDLE_ERROR;
	    else
...
ExHbaseScanSQTaskTcb::ExHbaseScanSQTaskTcb(ExHbaseAccessSelectTcb * tcb)
  :  ExHbaseTaskTcb(tcb)
  , step_(NOT_STARTED)
{
	...
	case SCAN_OPEN:
	  {
	    ...
	    retcode = tcb_->ehi_->scanOpen
	       ...
	       tcb_->hbaseAccessTdb().getHbaseSnapshotScanAttributes()->getSnapshotName(),
            ...
            if (tcb_->hbaseAccessTdb().getScanMemoryTable() &&
                tcb_->ehi_->isReadFromMemoryTable() == false &&
                tcb_->ehi_->isMemoryTableDisabled() == false &&
                tcb_->ehi_->ismemDBinitFailed() == false &&
                tcb_->beginRowId_.size() == 0 &&
                tcb_->endRowId_.size() == 0)
            {
	    ...
	    if (recordCostTh_ >= 0)
	      {
	        ...
	        if (time2 >= recordCostTh_)
	          QRWARN("ExHbaseScanSQTaskTcb SCAN_OPEN PID %d txID %ld CC %d ATC %.2f TTC %.2f TC %ld  %s",
	                  pid, transId, callCount[8], (sumCost[8] / (callCount[8] ? callCount[8] : 1)), sumCost[8], time2, tcb_->table_.val);
	      }
	    if (tcb_->setupError(retcode, "ExpHbaseInterface::scanOpen"))
	      step_ = HANDLE_ERROR;
	    else
	    ...
	    retcode = tcb_->ehi_->nextRow();
            if (recordCostTh_ >= 0)
              { 
                ...
                if (time2 >= recordCostTh_)
                  QRWARN("ExHbaseScanSQTaskTcb NEXT_ROW PID %d txID %ld CC %d ATC %.2f TTC %.2f TC %ld  %s",
                          pid, transId, callCount[9], (sumCost[9] / (callCount[9] ? callCount[9] : 1)), sumCost[9], time2, tcb_->table_.val);
              }
	    if (retcode == HBASE_ACCESS_EOD || retcode == HBASE_ACCESS_EOR)
	      {
		step_ = SCAN_CLOSE;
	...
	case CREATE_ROW:
	  {
	    ...
	    if (retcode < 0)
	    {
              ...
              if ((pentry_down) &&
                  (pentry_down->getDiagsArea()->mainSQLCODE() == -EXE_DEFAULT_VALUE_INCONSISTENT_ERROR) &&
                  (tcb_->hbaseAccessTdb().getTableName() ||
				   tcb_->hbaseAccessTdb().replaceNameByUID() && tcb_->hbaseAccessTdb().getDataUIDName()))
                {
                  char tabnam[400];
                  str_sprintf(tabnam, "%s:%s.%s.%s",
                              TRAF_RESERVED_NAMESPACE2,
				  ...
				  {
					if (NAString(tcb_->hbaseAccessTdb().getDataUIDName()) ==
                      NAString(tabnam))
                    {
				  ...
				  {
                  	if (NAString(tcb_->hbaseAccessTdb().getTableName()) ==
                      NAString(tabnam))
                    {
                      pentry_down->getDiagsArea()->clear();
                      step_ = NEXT_ROW;
	    ...
	    retcode = tcb_->ehi_->scanClose();
            if (recordCostTh_ >= 0)
              { 
                ...
                if (time2 >= recordCostTh_)
                  QRWARN("ExHbaseScanSQTaskTcb SCAN_CLOSE PID %d txID %ld CC %d ATC %.2f TTC %.2f TC %ld  %s",
...
Lng32 ExHbaseScanSQTaskTcb::getProbeResult(char* &keyData)
{
  ...
  retcode = tcb_->ehi_->scanOpen(tcb_->table_, 
				 ...
				 probeSize,
				 TRUE, NULL, NULL, NULL, tcb_->hbaseAccessTdb().getNumReplications());
  if (recordCostTh_ >= 0)
    {
      Int64 time2 = (JULIANTIMESTAMP() - time1) / 1000;
      if (transId > 0)
      ...
      if (time2 >= recordCostTh_)
        QRWARN("ExHbaseScanSQTaskTcb::getProbeResult SCAN_OPEN PID %d txID %ld CC %d ATC %.2f TTC %.2f TC %ld  %s",
  ...
  if (recordCostTh_ >= 0)
    { 
      ...
      if (time2 >= recordCostTh_)
        QRWARN("ExHbaseScanSQTaskTcb::getProbeResult NEXT_ROW PID %d txID %ld CC %d ATC %.2f TTC %.2f TC %ld  %s",
  ...
  if (recordCostTh_ >= 0)
    {
      ...
      if (time2 >= recordCostTh_)
        QRWARN("ExHbaseScanSQTaskTcb::getProbeResult SCAN_CLOSE PID %d txID %ld CC %d ATC %.2f TTC %.2f TC %ld  %s",
	    ...
	    if (recordCostTh_ >= 0)
	      {
	        ...
	        if (time2 >= recordCostTh_)
                  {
	            if (tcb_->rowIds_.entries() == 1)
	              QRWARN("ExHbaseGetTaskTcb GET_OPEN PID %d txID %ld CC %d ATC %.2f TTC %.2f TC %ld  %s",
	                      pid, transId, callCount[14], sumCost[14] / (callCount[14] ? callCount[14] : 1), sumCost[14], time2, tcb_->table_.val);
	            else
	              QRWARN("ExHbaseGetTaskTcb GET_OPEN_ROWS PID %d txID %ld CC %d ATC %.2f TTC %.2f TC %ld  %s",
	                      pid, transId, callCount[14], sumCost[14] / (callCount[14] ? callCount[14] : 1), sumCost[14], time2, tcb_->table_.val);	          
	          }
	      }
	  }
	    ...
	    retcode = tcb_->ehi_->nextRow();
            if (recordCostTh_ >= 0)
              {   
                ...
                if (time2 >= recordCostTh_)
                  {
                      QRWARN("ExHbaseGetTaskTcb NEXT_ROW PID %d txID %ld CC %d ATC %.2f TTC %.2f TC %ld  %s",
            ...
            if (recordCostTh_ >= 0)
              {
                ...
                if (time2 >= recordCostTh_)
                  {   
                    QRWARN("ExHbaseGetTaskTcb GET_CLOSE PID %d txID %ld CC %d ATC %.2f TTC %.2f TC %ld  %s",
	    ...
	    if (recordCostTh_ >= 0)
	      {
	        ...
	        if (time2 > recordCostTh_)
	          {
                    if (tcb_->rowIds_.entries() == 1)
	              QRWARN("ExHbaseGetRowwiseTaskTcb GET_OPEN PID %d txID %ld CC %d ATC %.2f TTC %.2f TC %ld  %s",
	                    pid, transId, callCount[18], sumCost[18] / (callCount[18] ? callCount[18] : 1), sumCost[18], time2, tcb_->table_.val);
	            else
	              QRWARN("ExHbaseGetRowwiseTaskTcb GET_OPEN_ROWS PID %d txID %ld CC %d ATC %.2f TTC %.2f TC %ld  %s",
	                      pid, transId, callCount[18], sumCost[18] / (callCount[18] ? callCount[18] : 1), sumCost[18], time2, tcb_->table_.val);
	          }
	      }
	  }
	    ...
	    retcode = tcb_->ehi_->nextRow();
            if (recordCostTh_ >= 0)
              {   
                ...
                if (time2 > recordCostTh_)
                  { 
                    QRWARN("ExHbaseGetRowwiseTaskTcb NEXT_ROW PID %d txID %ld CC %d ATC %.2f TTC %.2f TC %ld  %s",
	    ...
	    retcode = tcb_->ehi_->nextCell(tcb_->rowId_, tcb_->colFamName_, 
            ...
            if (recordCostTh_ >= 0)
              {   
                ...
                if (time2 > recordCostTh_)
                  {
                    QRWARN("ExHbaseGetRowwiseTaskTcb nextCell PID %d txID %ld CC %d ATC %.2f TTC %.2f TC %ld  %s",
	    ...
	    retcode = tcb_->ehi_->getClose();
            if (recordCostTh_ >= 0)
              {   
                ...
                if (time2 > recordCostTh_)
                  {
                    QRWARN("ExHbaseGetRowwiseTaskTcb GET_CLOSE PID %d txID %ld CC %d ATC %.2f TTC %.2f TC %ld  %s",
		...
		if (recordCostTh_ >= 0)
		  {
		    ...
		    {
		      if (tcb_->rowIds_.entries() == 1)
		        QRWARN("ExHbaseGetSQTaskTcb GET_OPEN PID %d txID %ld CC %d ATC %.2f TTC %.2f TC %ld  %s",
		                pid, transId, callCount[22], (sumCost[22] / (callCount[22] ? callCount[22] : 1)), sumCost[22], time2, tcb_->table_.val);
		      else
		        QRWARN("ExHbaseGetSQTaskTcb GET_OPEN_ROWS PID %d txID %ld CC %d ATC %.2f TTC %.2f TC %ld  %s",
		                pid, transId, callCount[22], (sumCost[22] / (callCount[22] ? callCount[22] : 1)), sumCost[22], time2, tcb_->table_.val);
		    }
		  }
		if (tcb_->setupError(retcode, "ExpHbaseInterface::getRowsOpen"))
            ...
            if (recordCostTh_ >= 0)
              {
                ...
                if (time2 >= recordCostTh_)
                  {
                    QRWARN("ExHbaseGetSQTaskTcb NEXT_ROW PID %d txID %ld CC %d ATC %.2f TTC %.2f TC %ld  %s",
	    ...
	    retcode = tcb_->ehi_->getClose();
            if (recordCostTh_ >= 0)
              {   
                ...
                if (time2 >= recordCostTh_)
                  {
                    QRWARN("ExHbaseGetSQTaskTcb GET_CLOSE PID %d txID %ld CC %d ATC %.2f TTC %.2f TC %ld  %s",
  ...
  if (hbaseAccessTdb.validateDDL())
    {
      ...
      ObjectEpochCacheEntryName oecName(NULL,
                                        hbaseAccessTdb.getBaseTableName(),
                                        COM_BASE_TABLE_OBJECT,
                                        0);
      ObjectEpochCacheEntry *oecEntry = NULL;
      ObjectEpochCacheKey key(hbaseAccessTdb.getBaseTableName(), 
	...
	case COPROC_EVAL:
	  {
	    ...
	    Text colFam;
	    Text colName;
            retcode = ExFunctionHbaseColumnLookup::extractColFamilyAndName(
                 col, 
                 -1,
                 TRUE,
                 colFam, colName);
		...
		break;
	      ...
	      hsr_ = (ComTdbHbaseAccess::HbaseScanRows*)hbaseAccessTdb().listOfScanRows()->getCurr();
				...
				endKey = endRowId_;
				if (endKey.empty() && (filterForNull == 1))
					endKey.assign(2, 0xFE);
	    ...
	    retcode = ehi_->coProcAggr(table_, 
				       ...
				       aggrVal, hbaseAccessTdb().getIsolationLevel(),
	    ...
	    if (recordCostTh_ >= 0)
	      {
	        ...
	        if (time2 >= recordCostTh_)
	          QRWARN("ExHbaseCoProcAggrTcb COPROC_EVAL PID %d txID %ld CC %d ATC %.2f TTC %.2f TC %ld  %s",
	                  pid, transId, callCount[25], sumCost[25] / (callCount[25] ? callCount[25] : 1), sumCost[25], time2, table_.val);
	      }
