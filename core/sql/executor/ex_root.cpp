...
ex_tcb * ex_root_tdb::build(CliGlobals *cliGlobals, ex_globals * glob)
{
  ...
  if(useDlockForSharedCache())
    checkDuplicateEsp = true;
...
// Do not need to cover debug code since customers and QA do no receive it.
  ...
  if (master_glob->getStatement()->syncQueryForBinglog() &&
      ...
      (qt == ComTdbRoot::SQL_INSERT_NON_UNIQUE ||
       qt == ComTdbRoot::SQL_UPDATE_NON_UNIQUE ||
  ...
  if (syncQuery)
  {
    ...
    if (inputExpr())
      parameterpack = inputExpr()->sourDataParams_,
      packlen = inputExpr()->sourDataParamsLen_;
    ...
    if (ret < 0)
    {
      // log this error
      NAString massge;
      massge.format("put sql->:%s to hbase, objschema = %s, eventID = %ld", master_glob->getStatement()->getSrcStr(), objschema.data(), eventID);
...
SCOPE_GUARD_FN(ServerContext, {if(p->server)p->serverClass->freeServerProcess(p->server); })
...
Int32 ex_root_tcb::passiveCancel()
{
  if (qchild.down->isEmpty() && qchild.up->isEmpty())
    return 1; // Query is already finished.
  ...
  // IpcServer *server = ssmpManager->getSsmpServer((NAHeap *)getHeap(), cliGlobals->myNodeName(), cliGlobals->myCpu(), diagsArea);
  st.serverClass = ssmpManager->getServerClass();
  st.server = st.serverClass->allocateServerProcess(&diagsArea, getHeap(),
                                                    cliGlobals->myNodeName(), cliGlobals->myCpu(),
                                                    IPC_PRIORITY_DONT_CARE, false);
  if (!st.server || !st.server->castToIpcGuardianServer()->isReady() || !st.server->getControlConnection())
    return 0;
  ...
  CancelMsgStream *stream SCOPE_GUARD(CancelMsgStream) = new (cliGlobals->getIpcHeap()) CancelMsgStream(cliGlobals->getEnvironment(), 0, 0);
  stream->addRecipient(st.server->getControlConnection());

  CancelQueryRequest msg(0, cliGlobals->getIpcHeap(),
                         cancelStartTime,
                         firstEscalationInterval, secondEscalationInterval,
                         cancelEscalationSaveabend,
                         (char *)CANCEL_COMMENT, sizeof(CANCEL_COMMENT),
                         cancelLogging,
                         0,
                         cliGlobals->myPin(),
                         60 /*ActiveSchemaDB()->getDefaults().getAsULong(CANCEL_MINIMUM_BLOCKING_INTERVAL)*/);
  RtsQueryId qid(cliGlobals->getIpcHeap(), glob->getStatement()->getUniqueStmtId(), glob->getStatement()->getUniqueStmtIdLen());
  *stream << msg << qid;
  stream->send(true);
  return 0;
}
...
short ex_root_tcb::retryDeregisterCB()
{
  ...
  if (NOT isCbFinishedMessageSent())
    return -1;
...
void ex_root_tcb::resetConnectionToSsmp()
{
    ...
    if (cbServer_ != NULL)
    {
      ...
      queryFinishedStream_  = new (context->getIpcHeap())
            QueryFinishedMsgStream(context->getEnvironment(), this, ssmpManager);
      queryFinishedStream_->addRecipient(cbServer_->getControlConnection());
    }
}
...
void ex_root_tcb::cbMessageWait(Int64 waitStartTime)
{
  ...
  while (isCbFinishedMessageSent())
  {
    ...
    Int64 timeSinceCBMsgSentMicroSecs = retryCount > 1 ? 0 :
                (waitStartTime ? NA_JulianTimestamp() - waitStartTime : 0);
    ...
    if (cbDidTimedOut)
    {
      ...
      {
        SQLMXLoggingArea::logExecRtInfo(__FILE__, __LINE__,
               "retry send QueryFinished message to MXSSMP.", 0);
      ...
      else
      {
        ...
        SQLMXLoggingArea::logExecRtInfo(__FILE__, __LINE__,
               "IPC timeout while communicating with MXSSMP.", 0);
        dumpCb();
        if (exitCBTimeout)
        {
