...
ex_tcb *ex_root_tdb::build(CliGlobals *cliGlobals, ex_globals *glob) {
  ...
  if (getQueryUsesSM() && cliGlobals->getEnvironment()->smEnabled()) {
    ...
    if (smGlobals && smGlobals->getReaderThreadState() == ExSMGlobals::TERMINATED_DUE_TO_ERROR) {
  ...
  rtFragTable = new (master_glob->getSpace()) ExRtFragTable(master_glob, fragDir_, (char *)this);

  // if this tdb is displayed in the GUI then enable GUI display for ESPs
  if (displayExecution() > 0) rtFragTable->displayGuiForESPs(TRUE);
  ...
  NABoolean checkDuplicateEsp = false;
  if (useDlockForSharedCache()) checkDuplicateEsp = true;
  rtFragTable->assignEsps(TRUE, numOfTotalEspsUsed, numOfNewEspsStarted, checkDuplicateEsp);
  ...
  if (getQueryUsesSM() && cliGlobals->getEnvironment()->smEnabled()) {
    ...
    if (diags && diags->getNumber(DgSqlCode::ERROR_) > 0) return NULL;
  } else {
    // Turn off tracing if it once was enabled and now we turned off
    // the SEAMONSTER CQD, which disables the SeaMonster trace.
    ExSMGlobals *smGlobals = ExSMGlobals::GetExSMGlobals();
    if (smGlobals && smGlobals->getTraceEnabled()) smGlobals->setTraceEnabled(false);
  }

  // build the child
  ex_tcb *child_tcb = childTdb->build(exe_glob);

  ex_root_tcb *root_tcb = new (exe_glob->getSpace()) ex_root_tcb(*this, *child_tcb, exe_glob);
  ...
  if (getCollectStats()) {
    ...
    if (getCollectStatsType() == ALL_STATS || stmtStats == NULL || stmtStats->getQueryId() == NULL) {
      // These ALL stats are too numerous to store in the
      // shared segment.
      statsArea =
          new (master_glob->getDefaultHeap()) ExStatisticsArea(master_glob->getDefaultHeap(), 0, getCollectStatsType());
      ...
      if (statsGlobals == NULL) {
        statsArea = new (master_glob->getDefaultHeap())
            ExStatisticsArea(master_glob->getDefaultHeap(), 0, getCollectStatsType());
        master_glob->setStatsArea(statsArea);
        root_tcb->allocateStatsEntry();
        if (stmtStats != NULL) stmtStats->setStatsArea(statsArea);
      } else {
        int error = statsGlobals->getStatsSemaphore(cliGlobals->getSemId(), cliGlobals->myPin());
        statsArea =
            new (cliGlobals->getStatsHeap()) ExStatisticsArea(cliGlobals->getStatsHeap(), 0, getCollectStatsType());
        if (stmtStats != NULL) {
          ...
          ExStatisticsArea *ctxStats = master_glob->getStatement()->getContext()->getStats();
          ExStatisticsArea *prevStats = stmtStats->getStatsArea();
          if (ctxStats == prevStats) cliGlobals->currContext()->setStatsArea(NULL, FALSE, FALSE, FALSE);
    ...
    ExMasterStats *masterStats = (stmtStats ? stmtStats->getMasterStats() : NULL);
    if (masterStats) {
      masterStats->numOfNewEspsStarted() = numOfNewEspsStarted;
      masterStats->setNumCpus(rtFragTable->countSQLNodes(cliGlobals->myCpu()));
    }
  }
  ...
  if (getUncProcess()) cliGlobals->setUncProcess();
  ...
  if (root_tcb->getStatsEntry()) {
    ...
    if (root_tcb->getStatsEntry()->castToExFragRootOperStats())
      root_tcb->getStatsEntry()->castToExFragRootOperStats()->setStmtIndex(
          master_glob->getStatement()->getStatementIndex());
  }
  ...
  if (root_tcb->getStatsEntry()) {
    ...
    } else if (root_tcb->getStatsEntry()->castToExFragRootOperStats()) {
  ...
  if (NOT noEspsFixup()) rtFragTable->downloadAndFixup();
...
//  TCB procedures
///////////////////////////////////////////////////////////////////////////
ex_root_tcb::ex_root_tcb(const ex_root_tdb &root_tdb, const ex_tcb &child_tcb, ExExeStmtGlobals *glob)
  ...
  if (root_tdb.inputExpr_) {
    (root_tdb.inputExpr_)->fixup(0, getExpressionMode(), this, glob->getSpace(), glob->getDefaultHeap(), FALSE, glob);
  }

  if (root_tdb.outputExpr_) {
    (root_tdb.outputExpr_)->fixup(0, getExpressionMode(), this, glob->getSpace(), glob->getDefaultHeap(), FALSE, glob);
  }

  if (root_tdb.pkeyExpr_) {
    (root_tdb.pkeyExpr_)->fixup(0, getExpressionMode(), this, glob->getSpace(), glob->getDefaultHeap(), FALSE, glob);
  }

  if (root_tdb.predExpr_) {
    (root_tdb.predExpr_)->fixup(0, getExpressionMode(), this, glob->getSpace(), glob->getDefaultHeap(), FALSE, glob);
  }
  ...
  if (root_tdb.inputVarsSize_ > 0) {
    ...
    char *dataPtr = (char *)glob->getSpace()->allocateMemory(root_tdb.inputVarsSize_);
  ...
  if (root_tdb.updateCurrentOfQuery()) {
    ...
    tupp_descriptor *tp = new (glob->getSpace()) tupp_descriptor;
    char *dataPtr = (char *)glob->getSpace()->allocateMemory(root_tdb.pkeyLen_);
  ...
  ex_assert(numTuples == root_tdb.criDesc_->noTuples(), "conflicting tuple numbers in root_tdb");
...
ex_root_tcb::~ex_root_tcb() { freeResources(); };
...
void ex_root_tcb::populateCancelDiags(ComDiagsArea &diags) {
  ...
  if (statsArea) {
    ...
    if (masterStats && masterStats->getCancelComment()) diags << DgString0(masterStats->getCancelComment());
  }
}
...
void ex_root_tcb::registerSubtasks() {
  ...
  getGlobals()->castToExExeStmtGlobals()->castToExMasterStmtGlobals()->getRtFragTable()->setSchedulerEvent(
      getGlobals()->getScheduler()->registerNonQueueSubtask(sWorkOnFragDir, this, "FD"));
}

char *ex_root_tcb::getPkeyRow() {
  if ((pkeyExpr()) && (pkeyAtp_))
...
NABoolean ex_root_tcb::needStatsEntry() { return TRUE; }
...
ExOperStats *ex_root_tcb::doAllocateStatsEntry(CollHeap *heap, ComTdb *tdb) {
  ...
  if ((statsType == ComTdb::ALL_STATS) || (statsType == ComTdb::OPERATOR_STATS) ||
    ...
    stat = new (heap) ExFragRootOperStats(heap, this, tdb);
    StmtStats *ss = getGlobals()->castToExExeStmtGlobals()->castToExMasterStmtGlobals()->getStatement()->getStmtStats();
    if (ss != NULL) ((ExFragRootOperStats *)stat)->setQueryId(ss->getQueryId(), ss->getQueryIdLen());
    ...
    stat = new (heap) ExMeasStats(heap, NULL, tdb);
    StmtStats *ss = getGlobals()->castToExExeStmtGlobals()->castToExMasterStmtGlobals()->getStatement()->getStmtStats();
    if (ss != NULL) ((ExMeasStats *)stat)->setQueryId(ss->getQueryId(), ss->getQueryIdLen());
  }
  if (stat) getGlobals()->getStatsArea()->setRootStats(stat);
  return stat;
}
...
void ex_root_tcb::inputPkeyRow(char *pkey_row) {
  char *dataPtr = workAtp_->getTupp(root_tdb().criDesc_->noTuples() - 1).getDataPointer();
...
///////////////////////////////////////////////////////////////////////////
void ex_root_tcb::getInputData(char *&inputData, int &inputDatalen) {
  inputData = NULL;
  inputDatalen = 0;
  if (root_tdb().inputVarsSize_ > 0) {
...
void ex_root_tcb::setInputData(char *inputData) {
  if (root_tdb().inputVarsSize_ > 0) {
    str_cpy_all(workAtp_->getTupp(2).getDataPointer(), inputData, root_tdb().inputVarsSize_);
  }
}
...
int ex_root_tcb::execute(CliGlobals *cliGlobals, ExExeStmtGlobals *glob, Descriptor *input_desc,
  ...
  } else if (root_tdb().getFirstNRows() < 0) {
  ...
  if (master_glob->getStatement()->syncQueryForBinglog() &&
      ...
      (qt == ComTdbRoot::SQL_INSERT_NON_UNIQUE || qt == ComTdbRoot::SQL_UPDATE_NON_UNIQUE ||
       qt == ComTdbRoot::SQL_DELETE_NON_UNIQUE))
    syncQuery = TRUE;
  ...
  if ((!reExecute) && (inputExpr())) {
    ...
    inputExpr()->setInternalFormatIO(
        flags, master_glob->getStatement()->getContext()->getSessionDefaults()->getInternalFormatIO());
    inputExpr()->setIsDBTR(flags, master_glob->getStatement()->getContext()->getSessionDefaults()->getDbtrProcess());

    // move the values from user area to internal area
    ex_expr::exp_return_type rc;
    if ((input_desc) && (root_tdb().singleRowInput()) && (NOT input_desc->isDescTypeWide()) &&
        (NOT input_desc->thereIsACompoundStatement())) {
      // fast single row input
      rc = inputExpr()->inputSingleRowValue(entry->getAtp(), input_desc, NULL, glob->getDefaultHeap(), flags);
    } else if (root_tdb().rowwiseRowsetInput()) {
      // rowwise rowset input
      rc = inputExpr()->inputRowwiseRowsetValues(entry->getAtp(), input_desc, root_tdb().getRWRSInfo(),
                                                 root_tdb().isNonFatalErrorTolerated(), glob->getDefaultHeap(), flags);
    } else {
      // columnwise rowset or fastsingle row input disabled.
      rc = inputExpr()->inputValues(entry->getAtp(), input_desc, root_tdb().isNonFatalErrorTolerated(),
                                    glob->getDefaultHeap(), flags, optLargVar_, syncQuery);
    }
    if (rc == ex_expr::EXPR_ERROR) {
      ...
      if (root_tdb().isNonFatalErrorTolerated() && !(diagsArea->canAcceptMoreErrors())) {
        ...
        *diagsArea << DgSqlCode(-EXE_NONATOMIC_FAILURE_LIMIT_EXCEEDED) << DgInt0(root_tdb().getNotAtomicFailureLimit());
      }
    ...
    if ((NOT inputExpr()->noEval()) && (inputExpr()->eval(entry->getAtp(), 0) == ex_expr::EXPR_ERROR)) {
      diagsArea = ex_root_tcb::moveDiagsAreaFromEntry(entry);
      return -1;
    }
  } else if (!reExecute && !inputExpr() && input_desc && input_desc->getUsedEntryCount() &&
             glob->getStmtType() != ExExeStmtGlobals::STATIC) {
    NABoolean byPassCheck = input_desc->isDescTypeWide();
    if (!byPassCheck && (input_desc->getCompoundStmtsInfo() == 0) && (diagsArea == NULL)) {
      diagsArea = ComDiagsArea::allocate(getHeap());
  ...
  if (getStatsEntry()) {
    ...
    if (rootStats) rootStats->setTimestamp(NA_JulianTimestamp());
  }
  ...
  if (checkTransBeforeExecute(master_glob->getStatement()->getContext()->getTransaction(), master_glob, mStats,
                              diagsArea))
    return -1;
  ...
  if (root_tdb().getUniqueExecuteIdOffset() >= 0
    ...
    ExecuteId *uniqueExecuteId =
        (ExecuteId *)(entry->getAtp()->getTupp(2).getDataPointer() + root_tdb().getUniqueExecuteIdOffset());

    uniqueExecuteId->cpuNum =
        glob->castToExExeStmtGlobals()->getIpcEnvironment()->getMyOwnProcessId(IPC_DOM_GUA_PHANDLE).getCpuNum();
  ...
  if ((root_tdb().getTriggersCount() > 0)
      ...
      // root_tdb version related problems during INIT_SQL
      && (entry->getAtp()->getCriDesc()->noTuples() > 2) && (getTriggerStatusVector() != NULL))
  ...
  // in this statement
  {
    unsigned char *triggersStatus =
        (unsigned char *)(entry->getAtp()->getTupp(2).getDataPointer() + root_tdb().getTriggersStatusOffset());
  ...
  getGlobals()->castToExExeStmtGlobals()->castToExMasterStmtGlobals()->getRtFragTable()->setActiveState();
  ...
  if (root_tdb().getQueryUsesSM() && cliGlobals->getEnvironment()->smEnabled()) {
    EXSM_TRACE(EXSM_TRACE_MAIN_THR, "query [%s]", master_glob->getStatement()->getSrcStr());

    EXSM_TRACE(EXSM_TRACE_MAIN_THR, "root tcb %p exec count %d", this, (int)master_glob->getExecutionCount());
  }

  ExHbaseAccessTdb *tdb = (ExHbaseAccessTdb *)(root_tdb().getTriggerTdb());
  if (tdb != NULL && tdb->getHasCallBeforeTrigger() == FALSE) {
    tdb->setHasCallBeforeTrigger(TRUE);
    tdb->setExMasterStmtGlobals(master_glob);

    if (tdb->execStatementTrigger(TRUE) != 0) {
  ...
  if (syncQuery) {
    ...
    CmpContext *cmpContext = master_glob->getStatement()->getContext()->getEmbeddedArkcmpContext();
    if (cmpContext) {
      NAString currCatName, currSchName;
      cmpContext->schemaDB_->getDefaults().getCatalogAndSchema(currCatName, currSchName);
      ComSchemaName currCatSchName(currCatName, currSchName);
      objschema = currCatSchName.getExternalName();
    }
    ExpHbaseInterface *ehi = ExpHbaseInterface::newInstance(STMTHEAP, "", "", COM_STORAGE_HBASE, FALSE);
    ret = ehi->putData(eventID, master_glob->getStatement()->getSrcStr(), qt, objschema.data(), parameterpack, packlen);
    if (ret < 0) {
      // log this error
      NAString massge;
      massge.format("put sql->:%s to hbase, objschema = %s, eventID = %ld", master_glob->getStatement()->getSrcStr(),
                    objschema.data(), eventID);
      LOGINFO(CAT_SQL_CI, massge.data());
    }
    NADELETEBASIC(parameterpack, getHeap());
    delete ehi;
  }
...
void ex_root_tcb::snapshotScanCleanup(ComDiagsArea *&diagsArea) {
  ...
  if (retcode != 0) {
    ...
    if (retcode != 0) setupWarning(retcode, "ExpHbaseInterface::cleanSnpTmpLocation", "", diagsArea);
    if (root_tdb().getListOfSnapshotScanTables()) root_tdb().getListOfSnapshotScanTables()->position();
...
int ex_root_tcb::fetch(CliGlobals *cliGlobals, ExExeStmtGlobals *glob, Descriptor *output_desc,
  ...
  if (output_desc != NULL) {
    if ((newOperation) && (NOT output_desc->rowwiseRowsetEnabled())) {
      output_desc->setDescItem(0, SQLDESC_ROWSET_NUM_PROCESSED, 0, 0);
      output_desc->setDescItem(0, SQLDESC_ROWSET_HANDLE, 0, 0);
    }

    output_desc->getDescItem(0, SQLDESC_ROWSET_SIZE, &outputRowsetSize, 0, 0, 0, 0);
  ...
  if (newOperation && root_tdb().isEmbeddedUpdateOrDelete() && !qchild.down->isEmpty()) {
  ...
  IpcEnvironment *ipcEnv = glob->castToExExeStmtGlobals()->getIpcEnvironment();
  ...
  while (1)  // return
  {
    ...
    if ((queueSize > outputRowsetSize) ||
      ...
      NABoolean dontReturn = ((root_tdb().getFirstNRows() == -2) && (numOfCliCalls == 1) &&
      ...
      if (compileRowsetSize == -1) {
        ...
        if (centry->upState.status == ex_queue::Q_OK_MMORE) {
          if (outputExpr()) {
            compileRowsetSize = outputExpr()->getCompiledOutputRowsetSize(centry->getAtp());
          }

          if (compileRowsetSize < 0) compileRowsetSize = 0;
      ...
      for (int i = 0; i < queueSize && (retcode == 0 || retcode == 1); i++) {
        ...
        if ((centry->upState.status != ex_queue::Q_OK_MMORE) && (centry->upState.status != ex_queue::Q_GET_DONE)) {
          ...
          nextIsQNoData = (centry->upState.status == ex_queue::Q_NO_DATA);
        ...
        switch (centry->upState.status) {
          case ex_queue::Q_OK_MMORE: {
            if (predExpr()) {
              ex_expr::exp_return_type exprRetCode = predExpr()->eval(centry->getAtp(), NULL);
            ...
            if (mStats) mStats->incRowsReturned();

            // row was returned from child.Evaluate output expression.
            if (outputExpr()) {
              if ((NOT outputExpr()->noEval()) && (outputExpr()->eval(centry->getAtp(), 0) == ex_expr::EXPR_ERROR)) {
                retcode = -1;
                break;
              }
              ...
              if (output_desc) {
                ...
                outputExpr()->setInternalFormatIO(flags, currContext.getSessionDefaults()->getInternalFormatIO());

                outputExpr()->setIsDBTR(
                    flags, master_glob->getStatement()->getContext()->getSessionDefaults()->getDbtrProcess());

                outputExpr()->setIsMXCI(
                    flags, master_glob->getStatement()->getContext()->getSessionDefaults()->getMxciProcess());

                if (outputExpr()->outputValues(centry->getAtp(), output_desc, getHeap(), flags) ==
                    ex_expr::EXPR_ERROR) {
              ...
              if (pkeyExpr()) {
                if (pkeyExpr()->eval(centry->getAtp(), pkeyAtp_) == ex_expr::EXPR_ERROR) {
                  retcode = -1;
                  break;
                }
              }
            ...
            if (output_desc != NULL) {
              if (outputRowsetSize > 0) {
                ...
                if (numLoopFetches > 1 && (numProcessed < outputRowsetSize)) {
                  ...
                  output_desc->setDescItem(0, SQLDESC_ROWSET_ADD_NUM_PROCESSED, 0, 0);
                }

                // Special case when rowset size is 1
                if ((numLoopFetches == 1) && (numProcessed == 0) && (outputRowsetSize == 1)) {
                  output_desc->setDescItem(0, SQLDESC_ROWSET_ADD_NUM_PROCESSED, 0, 0);
                }
                ...
                if (rowDiags != NULL) {
                  ...
                  for (index = 1; index <= rowDiags->getNumber(DgSqlCode::ERROR_); index++) {
                ...
                if (((numProcessed >= outputRowsetSize) || (numLoopFetches <= 1)) && (!dontReturn)) {
                  doneWithRowsets = TRUE;
                }
              }
            }
          ...
          case ex_queue::Q_NO_DATA: {
            ...
            getGlobals()->castToExExeStmtGlobals()->castToExMasterStmtGlobals()->getRtFragTable()->setInactiveState();
...
int ex_root_tcb::fetchMultiple(CliGlobals *cliGlobals, ExExeStmtGlobals *glob, Descriptor *output_desc,
  ...
  while ((output_desc->rowwiseRowsetEnabled()) && keepFetching && (numRowsProcessed < numTotalRows)) {
    retcode = fetch(cliGlobals, glob, output_desc, diagsArea, timeLimit, newOperation, closeCursorOnError);

    if (retcode < 0) keepFetching = FALSE;
...

    {
      ...
      if (diagsArea && diagsArea->getNumber(DgSqlCode::WARNING_) > 0) retcode = 1;
    ...
    else if (retcode == 1 && diagsArea && diagsArea->containsError(-EXE_STREAM_TIMEOUT)) {
...
int ex_root_tcb::oltExecute(ExExeStmtGlobals *glob, Descriptor *input_desc, Descriptor *output_desc,
  ...
  if (master_glob->getStatement()->syncQueryForBinglog() &&
      (NOT master_glob->getStatement()->getContext()->getTransaction()->xnInProgress()) &&
      (NOT(Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL))) &&
      (qt == ComTdbRoot::SQL_INSERT_UNIQUE || qt == ComTdbRoot::SQL_UPDATE_UNIQUE ||
  ...
  if (inputExpr()) {
    ...
    // move the values from user area to internal area
    if (inputExpr()->inputValues(entry->getAtp(), input_desc,
                                 FALSE,  // do not tolerate nonfatal errors
                                 glob->getDefaultHeap(), flags, optLargVar_, syncQuery) == ex_expr::EXPR_ERROR) {
      diagsArea = ex_root_tcb::moveDiagsAreaFromEntry(entry);
      return -1;
    };
  ...
  while (1) {
        ...
        case ex_queue::Q_OK_MMORE: {
          ...
          if (outputExpr()) {
            ...
            if (output_desc) {
              ...
              outputExpr()->setIsDBTR(
                  flags, master_glob->getStatement()->getContext()->getSessionDefaults()->getDbtrProcess());
              if (outputExpr()->outputValues(centry->getAtp(), output_desc, getHeap(), flags) == ex_expr::EXPR_ERROR) {
                diagsArea = ex_root_tcb::moveDiagsAreaFromEntry(centry);
          ...
          // update operator stats
          if (getStatsEntry()) getStatsEntry()->incActualRowsReturned();
    ...
    if (NOT setRetcode) {
      ...
      if (retcode > 0 && syncQuery) {
        ...
        ExpHbaseInterface *ehi = ExpHbaseInterface::newInstance(STMTHEAP, "", "", COM_STORAGE_HBASE, FALSE);
        ret = ehi->putData(eventID, master_glob->getStatement()->getSrcStr(), qt, objschema.data(), parameterpack,
                           packlen);
        if (ret < 0) {
          // log this error
          NAString massge;
          massge.format("put sql->:%s to hbase, objschema = %s, eventID = %ld",
                        master_glob->getStatement()->getSrcStr(), objschema.data(), eventID);
          LOGINFO(CAT_SQL_CI, massge.data());
        }
        NADELETEBASIC(parameterpack, getHeap());
        delete ehi;
...
#define SCOPE_GUARD_FN(type, dispose)   \
  static void guard_##type(void *arg) { \
    type *p = *(type **)arg;            \
    if (p) dispose                      \
  }
SCOPE_GUARD_FN(ComDiagsArea, { p->decrRefCount(); })
SCOPE_GUARD_FN(CancelMsgStream, { p->addToCompletedList(); })
SCOPE_GUARD_FN(ServerContext, {
  if (p->server) p->serverClass->freeServerProcess(p->server);
})
...
int ex_root_tcb::passiveCancel() {
  ...
  ServerContext st = {}, *_p SCOPE_GUARD(ServerContext) = &st;
  ComDiagsArea *diagsArea SCOPE_GUARD(ComDiagsArea) = 0;

  // IpcServer *server = ssmpManager->getSsmpServer((NAHeap *)getHeap(), cliGlobals->myNodeName(), cliGlobals->myCpu(),
  // diagsArea);
  st.serverClass = ssmpManager->getServerClass();
  st.server = st.serverClass->allocateServerProcess(&diagsArea, getHeap(), cliGlobals->myNodeName(),
  ...
  CancelMsgStream *stream SCOPE_GUARD(CancelMsgStream) =
      new (cliGlobals->getIpcHeap()) CancelMsgStream(cliGlobals->getEnvironment(), 0, 0);
  stream->addRecipient(st.server->getControlConnection());

  CancelQueryRequest msg(0, cliGlobals->getIpcHeap(), cancelStartTime, firstEscalationInterval,
                         secondEscalationInterval, cancelEscalationSaveabend, (char *)CANCEL_COMMENT,
                         sizeof(CANCEL_COMMENT), cancelLogging, 0, cliGlobals->myPin(),
                         60 /*ActiveSchemaDB()->getDefaults().getAsULong(CANCEL_MINIMUM_BLOCKING_INTERVAL)*/);
  RtsQueryId qid(cliGlobals->getIpcHeap(), glob->getStatement()->getUniqueStmtId(),
...
int ex_root_tcb::cancel(ExExeStmtGlobals *glob, ComDiagsArea *&diagsArea, NABoolean getQueueDiags) {
  if (fatalError_) {
    ...
    ExHbaseAccessTdb *tdb = (ExHbaseAccessTdb *)(root_tdb().getTriggerTdb());
    if (tdb != NULL && tdb->getHasCallAfterTrigger() == FALSE) {
      tdb->setHasCallAfterTrigger(TRUE);
      if (glob->castToExMasterStmtGlobals() != NULL) {
        CliGlobals *cliGlobals = glob->castToExMasterStmtGlobals()->getCliGlobals();
        ComDiagsArea *diags = cliGlobals->currContext()->getDiagsArea();
        if (diags->ErrSizeIsNull()) {
          tdb->setExMasterStmtGlobals(glob->castToExMasterStmtGlobals());
          if (tdb->execStatementTrigger(FALSE) != 0) {
    ...
    // make sure we wake our ESPs if this is the first fetch
    // after quiescing the executor
    glob->castToExMasterStmtGlobals()->getRtFragTable()->continueWithTransaction();
    ...
    while ((!qchild.up->isEmpty() || !qchild.down->isEmpty()) && (schedRetcode != WORK_BAD_ERROR)) {
      ...
      while (!qchild.up->isEmpty()) {
        if (getQueueDiags && qchild.up->getHeadEntry()->getDiagsArea()) {
          if (!diagsArea) diagsArea = ComDiagsArea::allocate(glob->getDefaultHeap());
      ...
      // wait for external events if the scheduler indicated
      // last time that it had nothing to do
      if (schedRetcode == WORK_OK || schedRetcode == WORK_POOL_BLOCKED || schedRetcode == WORK_NOBODY_WORKED) {
        if (CancelTimeout < 0) {
          // call getenv once per process
          char *sct = getenv("SQLMX_CANCEL_TIMEOUT");
          if (sct) {
            CancelTimeout = (IpcTimeout)str_atoi(sct, str_len(sct)) * 100;
            if (CancelTimeout < 100) CancelTimeout = 30 * 100;
          } else
            CancelTimeout = 30 * 100;
        }
        // Wait for any I/O to complete, this includes both
        // ARKFS and ESP connections. Use an infinite timeout.
        glob->castToExExeStmtGlobals()->getIpcEnvironment()->getAllConnections()->waitOnAll(CancelTimeout, FALSE,
                                                                                            &timedOut);
      }

      if (timedOut) {
        SQLMXLoggingArea::logExecRtInfo(__FILE__, __LINE__, "IPC timeout in ex_root_tcb::cancel.", 0);
...
short ex_root_tcb::work() {
  ...
  if ((master_glob->getCancelState() == CLI_CANCEL_REQUESTED) && !cancelStarted_) {
    ...
    cancelStarted_ = TRUE;
    if (!qchild.down->isEmpty()) qchild.down->cancelRequest();
  }
  ...
  if (!qchild.up->isEmpty()) {
    ...
    if (!cancelStarted_) return 1;

    // For async cancel.
    // Remove all entries except Q_NO_DATA for the cancel request.
    while (qchild.up->getLength() > 1) qchild.up->removeHead();

    if (!qchild.down->isEmpty()) {  // Cancel request has not been removed by child.
      qchild.up->removeHead();
      // WORK_OK will continue scheduler loop.
      return WORK_OK;
    } else {  // Async cancel completes.
              // Attach -EXE_CANCELED error to Q_NO_DATA entry.
              // Return 1 to exit the scheduler loop.
      ex_queue_entry *centry = qchild.up->getHeadEntry();

      ComDiagsArea *diagsArea = ex_root_tcb::moveDiagsAreaFromEntry(centry);
      if (diagsArea == NULL) {
        diagsArea = ComDiagsArea::allocate(getHeap());
...
void ex_root_tcb::completeOutstandingCancelMsgs() {
  ...
  while (!fatalError_ && (glob->getRtFragTable()->getState() != ExRtFragTable::ERROR) && glob->anyCancelMsgesOut()) {
    if (root_tdb().getQueryUsesSM() && glob->getIpcEnvironment()->smEnabled())
      EXSM_TRACE(EXSM_TRACE_MAIN_THR | EXSM_TRACE_CANCEL, "root tcb %p completeOutstandingCancelMsgs out %d", this,
...
int ex_root_tcb::checkTransBeforeExecute(ExTransaction *myTrans, ExMasterStmtGlobals *masterGlob,
  ...
  if (myTrans && myTrans->xnInProgress()) {
    ...
    if ((myTrans->autoCommit() == FALSE) || (myTrans->exeStartedXn() == FALSE)) {
      NABoolean doAutoCommitTest = TRUE;
#ifdef _DEBUG
      if (getenv("SKIP_AUTOCOMMIT_TEST")) doAutoCommitTest = FALSE;
#endif

      if (root_tdb().isCheckAutoCommit() && doAutoCommitTest) {
        if (diagsArea == NULL) diagsArea = ComDiagsArea::allocate(getHeap());
    ...
    if (root_tdb().getMayAlterDb()) myTrans->setMayAlterDb(TRUE);

    if (root_tdb().getSuspendMayHoldLock()) myTrans->setMayHoldLock(TRUE);
    ...
    if (masterStats) {
      ...
      if (myTrans->mayAlterDb()) masterStats->setSuspendMayHaveAuditPinned(TRUE);

      if (myTrans->mayHoldLock()) masterStats->setSuspendMayHoldLock(TRUE);
    }
...
static const char *const NCRenvvar = getenv("SQL_NO_REGISTER_CANCEL");
static const bool NoRegisterCancel = (NCRenvvar && *NCRenvvar == '1');

void ex_root_tcb::registerCB(ComDiagsArea *&diagsArea) {
  if (NoRegisterCancel) return;

  ExMasterStmtGlobals *glob = getGlobals()->castToExExeStmtGlobals()->castToExMasterStmtGlobals();

  CliGlobals *cliGlobals = glob->getCliGlobals();
  if (cliGlobals->getStatsGlobals() == NULL) return;
  ContextCli *context = cliGlobals->currContext();
  ...
  if (root_tdb().mayNotCancel()) return;
  ...
  if (sessionDefaults) {
    ...
    if (!sessionDefaults->getCancelQueryAllowed()) return;
  ...
  // for this object.
  RtsQueryId *rtsQueryId = new (context->getIpcHeap()) RtsQueryId(context->getIpcHeap(), qid, qidLen);

  RtsHandle rtsHandle = (RtsHandle)this;

  // Make the message
  QueryStarted *queryStarted =
      new (context->getIpcHeap()) QueryStarted(rtsHandle, context->getIpcHeap(), JULIANTIMESTAMP(), executionCount);
  ...
  if (queryStartedStream_ == NULL) {
    queryStartedStream_ =
        new (context->getIpcHeap()) QueryStartedMsgStream(context->getEnvironment(), this, ssmpManager);
...
void ex_root_tcb::deregisterCB() {
  ...
  if (!isCbStartedMessageSent()) return;

  // Holdable cursor is known to deregister more than once.
  if (isCbFinishedMessageSent()) return;

  // The stream's actOnSend method will delete (or call decrRefCount())
  // for this object.
  RtsQueryId *rtsQueryId = new (context->getIpcHeap()) RtsQueryId(
      context->getIpcHeap(), glob->getStatement()->getUniqueStmtId(), glob->getStatement()->getUniqueStmtIdLen());

  RtsHandle rtsHandle = (RtsHandle)this;

  // Make the message
  QueryFinished *queryFinished = new (context->getIpcHeap()) QueryFinished(rtsHandle, context->getIpcHeap());
  ...
  if (queryFinishedStream_ == NULL) {
    queryFinishedStream_ =
        new (context->getIpcHeap()) QueryFinishedMsgStream(context->getEnvironment(), this, context->getSsmpManager());
  ...
  *queryFinishedStream_ << *rtsQueryId;
  ...
  // send the no-wait request to let the cancel broker know this query has finished.
  queryFinishedStream_->send(FALSE);

  setCbFinishedMessageSent();
}

short ex_root_tcb::retryDeregisterCB() {
  ExMasterStmtGlobals *glob = getGlobals()->castToExExeStmtGlobals()->castToExMasterStmtGlobals();
  CliGlobals *cliGlobals = glob->getCliGlobals();
  ContextCli *context = cliGlobals->currContext();

  // only retry when deregisterCB() was called
  if (NOT isCbFinishedMessageSent()) return -1;

  // should reset connection to ssmp before retry
  resetConnectionToSsmp();
  // can not get ssmp server
  if (cbServer_ == NULL) return -1;
...
// reset GuaConnectionToServer(not real sock connection) to ssmp by reset both
// queryStartedStream_ and queryFinishedStream_
void ex_root_tcb::resetConnectionToSsmp() {
  ExMasterStmtGlobals *glob = getGlobals()->castToExExeStmtGlobals()->castToExMasterStmtGlobals();
  CliGlobals *cliGlobals = glob->getCliGlobals();
  ContextCli *context = cliGlobals->currContext();

  // abandon current pending IOs
  // this will put stream and ssmpServer to deleted list
  if (queryFinishedStream_ != NULL) {
    queryFinishedStream_->removeRootTcb();
    queryFinishedStream_->abandonPendingIOs();
    queryFinishedStream_ = NULL;
  }

  if (queryStartedStream_ != NULL) {
    queryStartedStream_->removeRootTcb();
    queryStartedStream_->abandonPendingIOs();
    queryStartedStream_ = NULL;
  }

  // free stream in deleted list
  context->getEnvironment()->deleteCompletedMessages();
  // free ssmp server in deleted list
  context->getSsmpManager()->cleanupDeletedSsmpServers();

  ComDiagsArea *tmpDiagArea = context->getDiagsArea();
  ExSsmpManager *ssmpManager = context->getSsmpManager();
  cbServer_ =
      ssmpManager->getSsmpServer((NAHeap *)getHeap(), cliGlobals->myNodeName(), cliGlobals->myCpu(), tmpDiagArea);

  if (cbServer_ != NULL) {
    // create new streams with new ssmp server connection
    queryStartedStream_ =
        new (context->getIpcHeap()) QueryStartedMsgStream(context->getEnvironment(), this, ssmpManager);
    queryStartedStream_->addRecipient(cbServer_->getControlConnection());

    queryFinishedStream_ =
        new (context->getIpcHeap()) QueryFinishedMsgStream(context->getEnvironment(), this, ssmpManager);
    queryFinishedStream_->addRecipient(cbServer_->getControlConnection());
  }
}

// Control the behavior when timeout occurs communicating with MXSSMP
//
// Default to exiting the sender process
// Default to dumping the sender process and MXSSMP

static const char *const ECTenvvar = getenv("MASTER_EXIT_ON_CB_TIMEOUT");
static const bool exitCBTimeout = ((ECTenvvar != NULL) && (*ECTenvvar != '0') ? true : false);
...
void ex_root_tcb::cbMessageWait(long waitStartTime) {
  ...
  if (NOT anyCbMessages()) return;

  if (needsDeregister()) deregisterCB();

  while (isCbFinishedMessageSent()) {
    ...
    master_glob->getIpcEnvironment()->getAllConnections()->waitOnAll(cbTimeout, FALSE, &cbDidTimedOut);
    if (cbDidTimedOut) {
      if (retryCount < maxRetryCount) {
        SQLMXLoggingArea::logExecRtInfo(__FILE__, __LINE__, "retry send QueryFinished message to MXSSMP.", 0);
        if (0 != retryDeregisterCB()) {
          // retry deregisterCB() failed, just ignore CB
          break;
        }
        retryCount++;
      } else {
        // unfinished streams should not be used any more
        queryStartedStream_->abandonPendingIOs();
        queryStartedStream_ = NULL;
        queryFinishedStream_->abandonPendingIOs();
        queryFinishedStream_ = NULL;

        SQLMXLoggingArea::logExecRtInfo(__FILE__, __LINE__, "IPC timeout while communicating with MXSSMP.", 0);
        dumpCb();
        if (exitCBTimeout) {
          ex_assert(0, "Timeout waiting for control broker.");
        }
        break;
      }
    }
  ...
  // force clear status here:
  //   1) we have successfully deregistered CB
  //   2) we killed a no-responding ssmp, and CB info also cleared by kill
  clearCbMessageStatus();
}
void ex_root_tcb::dumpCb() {
  ...
  if (doDump) {
    if (dumpCBTimeout)
      ...
      cbServer_->getServerId().getPhandle().dumpAndStop(false, false);
  }
}
...
void QueryStartedMsgStream::actOnSend(IpcConnection *conn) {
  if (conn->getErrorInfo() != 0) delinkConnection(conn);
}
...
void QueryStartedMsgStream::actOnReceive(IpcConnection *connection) {
  if (connection->getErrorInfo() == 0) {
    ...
    QueryStartedReply *reply = new (ipcHeap) QueryStartedReply(INVALID_RTS_HANDLE, ipcHeap);
    ...
    *this >> *reply;
    ...
    if (reply->isNextActionComplete())
      ...
      if (reply->cancelLogging()) {
        ExMasterStmtGlobals *glob = rootTcb_->getGlobals()->castToExExeStmtGlobals()->castToExMasterStmtGlobals();
        char *qid = glob->getStatement()->getUniqueStmtId();
        char msg[80 +  // the constant text
                 ComSqlId::MAX_QUERY_ID_LEN];

        str_sprintf(msg, "Master executor of query %s has begun internal SQL cancel.", qid);
...
void QueryFinishedMsgStream::actOnReceive(IpcConnection *connection) {
  if (connection->getErrorInfo() == 0) {
    ...
    RmsGenericReply *reply = new (ipcHeap) RmsGenericReply(ipcHeap);
