...
ex_tcb *ExTupleLeafTdb::build(ex_globals *glob) {
  ExTupleLeafTcb *tcb = new (glob->getSpace()) ExTupleLeafTcb(*this, glob);
...
ex_tcb *ExTupleNonLeafTdb::build(ex_globals *glob) {
  ...
  ExTupleNonLeafTcb *tcb = new (glob->getSpace()) ExTupleNonLeafTcb(*this, *childTcb, glob);
...
// class ExTupleTcb
//////////////////////////////////////////
ExTupleTcb::ExTupleTcb(const ComTdbTuple &tupleTdb, ex_globals *glob) : ex_tcb(tupleTdb, 1, glob) {
  Space *space = (glob ? glob->getSpace() : 0);
  CollHeap *heap = (glob ? glob->getDefaultHeap() : 0);

  // Allocate the buffer pool
  pool_ = new (space) sql_buffer_pool(tupleTdb.numBuffers_, tupleTdb.bufferSize_, space);
  ...
  if (tupleTdb.predExpr_) (void)tupleTdb.predExpr_->fixup(0, getExpressionMode(), this, space, heap, FALSE, glob);
...
///////////////////////////////////////////////
ExTupleLeafTcb::ExTupleLeafTcb(const ExTupleLeafTdb &tupleTdb, ex_globals *glob) : ExTupleTcb(tupleTdb, glob) {}
...
ExWorkProcRetcode ExTupleLeafTcb::work() {
  ...
  ExTuplePrivateState *pstate = (ExTuplePrivateState *)pentry_down->pstate;
    ...
    switch (pstate->step_) {
      ...
      case ExTupleTcb::RETURN_TUPLE: {
        ...
        if (tupleTdb().predExpr_) {
          ex_expr::exp_return_type retCode = (tupleTdb().predExpr_->eval(up_entry->getAtp(), 0));
          if (retCode == ex_expr::EXPR_ERROR) {
            // handle errors
            pstate->step_ = ExTupleTcb::HANDLE_SQLERROR;
            break;
          }
          if (retCode == ex_expr::EXPR_FALSE) isPredicateSatisfied = FALSE;
        }
        ...
        if (tupleExprList()->atEnd()) pstate->step_ = ExTupleTcb::ALL_DONE;
      } break;
      ...
      case ExTupleTcb::ALL_DONE: {
        ...
        up_entry2->upState.parentIndex = pentry_down->downState.parentIndex;
...
///////////////////////////////////////////////
ExTupleNonLeafTcb::ExTupleNonLeafTcb(const ExTupleNonLeafTdb &tupleTdb, const ex_tcb &tcbChild, ex_globals *glob)
...
ExWorkProcRetcode ExTupleNonLeafTcb::work() {
  ...
  while (qparent.down->entryExists(nextToSendDown_)) {
    ...
    ExTuplePrivateState &pstate1 = *((ExTuplePrivateState *)pentry1->pstate);
    ex_queue_entry *centry1 = qchild_.down->getTailEntry();

    ex_assert(pstate1.step_ == ExTupleTcb::TUPLE_EMPTY, "Uninitialized pstate object");
  ...
  while (TRUE)  // exit via return
  {
    ...
    switch (pstate->step_) {
      ...
      case ExTupleTcb::RETURN_TUPLE: {
        ...
        up_entry4->upState.parentIndex = pentry_down->downState.parentIndex;
      ...
      case ExTupleTcb::ALL_DONE: {
        ...
        up_entry5->upState.parentIndex = pentry_down->downState.parentIndex;
