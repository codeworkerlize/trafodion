...
// ===========================================================================
// ===== The ParquetFileReader class implements access to the Java 
// ===== ParquetFileReader class.
// ===========================================================================

typedef enum {
  PFR_OK     = JOI_OK
 ,PFR_NOMORE = JOI_LAST         // OK, last row read.
 ,PFR_ERROR_OPEN_PARAM          // JNI NewStringUTF() in open()
 ,PFR_ERROR_OPEN_EXCEPTION      // Java exception in open()
 ,PFR_ERROR_FETCHROW_EXCEPTION  // Java exception in fetchNextRow()
 ,PFR_ERROR_CLOSE_EXCEPTION     // Java exception in close()
 ,PFR_ERROR_GETSTRIPEINFO_EXCEPTION
 ,PFR_ERROR_FILE_NOT_FOUND_EXCEPTION
 ,PFR_UNKNOWN_ERROR
 ,PFR_LAST
} PFR_RetCode;
...
class ParquetFileReader : public TrafExtStorageUtils
{
public:
  // Default constructor - for creating a new JVM		
  ParquetFileReader(NAHeap *heap)
  ...
  ParquetFileReader(NAHeap *heap, JavaVM *jvm, JNIEnv *jenv)
  ...
  //  PFR_RetCode    open(const char* path, Int64 offset=0L, Int64 length=ULLONG_MAX);
   ...
   *******/
  PFR_RetCode    open(const char * tableName,
                      const char* path,
                      ...
                      const char *parqSchStr=NULL,
                      int flags=0);

  // Fetch the next row as a raw string into 'buffer'.
  PFR_RetCode fetchNextRow(char** buffer, 
			   long& array_length, 
			   long& rowNumber, 
			   int& num_columns,
  ...
  PFR_RetCode    fetchRowsIntoBuffer(Int64 stopOffset, char* buffer, Int64 buffSize, Int64& bytesRead, char rowDelimiter);

  NAArray<HbaseStr> *getColStats(NAHeap *heap, 
                                 int colNum, char * colName, int colType);
                                 
  static char*  getErrorText(PFR_RetCode errEnum);

  NAArray<HbaseStr>* getFileStats(NAHeap *heap, char * rootDir);
  NAArray<HbaseStr>* 
       getFileAndBlockAttributes(NAHeap *heap, char * rootDir);
  PFR_RetCode getFileSchema(NAHeap *heap, 
                            char * tableName,
                            char * rootDir, 
                            TextVec * colNameList, 
...
private:
  ...
  enum JAVA_METHODS {
    ...
    JM_GET_FILE_SCHEMA,
    JM_CLOSE,
    JM_GET_FILE_AND_BLOCK_ATTRIBUTES,
    JM_LAST
  };
...
private:
  PFR_RetCode getLongArray(JAVA_METHODS method, const char* msg, LIST(Int64)& resultArray);
