...
char* ExDDLTcb::getQueryIdInfo(Lng32& len)
{
   ...
   // esp case
   ExEspStmtGlobals* espStmtGlobals = 
       stmtGlobals->castToExEspStmtGlobals();
	...
	*((ExDDLPrivateState*) pentry_down->pstate);
      ...
      if (ddlTdb().inputExpr_)
	{
      ...
      if ( currContextCli->isEmbeddedArkcmpInitialized() &&
	   currContextCli->getSessionDefaults()->callEmbeddedArkcmp() 
	   && ddlTdb().hbaseDDL() &&
          ...
          if (cpStatus == ExSqlComp::SUCCESS)
            {
                  ...
                  (NOT (Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL))))
              {
                ...
                if (ret < 0)
                {
                  // log this error
                  NAString massge;
                  massge.format("put sql->:%s to hbase, objschema = %s, eventID = %ld", masterStmtGlobals->getStatement()->getSrcStr(), objschema.data(), eventID);
    ...
    endOfData:
	  ...
	  cmpStatus = 
	    cmp->changePriority(
				currContextCli->getSessionDefaults()->getMxcmpPriorityDelta(), TRUE);
	  if (cmpStatus != ExSqlComp::SUCCESS)
	    {
	      // Add a warning that change priority failed.
	      // Continue processing with the original default.
...
short ExDDLwithStatusTcb::work()
{
  ...
  if ( isEsp ) {
     ...
     const ExEspFragInstanceDir* espFragDir = 
	...
	case NOT_STARTED_:
	  {
        ...
        case CALL_EMBEDDED_CMP_:
          {
            ...
            if (currContextCli->getDiagsArea())
              currContextCli->getDiagsArea()->clear();
        ...
        case PROCESS_REPLY_:
          {            
            ...
            if ((replyDWS_) && 
                (replyDWS_->blackBoxLen() > 0) && (replyDWS_->blackBox()))
              {
	    ...
	    if (qparent_.up->isFull())
	      return WORK_OK;
            ...
            char buf[1000];
            if ((replyDWS_->msgLen() > 0) &&
                (strlen(replyDWS_->msg()) > 0))
              {
       ...
       case RETURN_DETAILS_:
	    ...
	    if (qparent_.up->isFull())
	      return WORK_OK;
            ...
            if (moveRowToUpQueue(&qparent_, ddlTdb().tuppIndex(), currValue, currEntrySize, &rc))
              return rc;
            ...
            if ( currContext->isEmbeddedArkcmpInitialized() &&
                 ...
                 )
              {
                ...
                if (embCmpStatus == ExSqlComp::SUCCESS)
                  ...
                  {
                    if (embCmpDiagsArea &&
                        embCmpDiagsArea->mainSQLCODE() == -CAT_SCHEMA_DOES_NOT_EXIST_ERROR)
                      {
