...
typedef enum {
 ...
 ,HDFS_CLIENT_ERROR_NUMFILES_PARAM
 ,HDFS_CLIENT_ERROR_NUMFILES_EXCEPTION
 ,HDFS_CLIENT_ERROR_TRUNCATE_PARAM
 ,HDFS_CLIENT_ERROR_TRUNCATE_EXCEPTION
 ,HDFS_CLIENT_LAST
} HDFS_Client_RetCode;
...
public:
  ...
  Int64                  hdfsWriteImmediate(const char* data, Int64 size, HDFS_Client_RetCode &hdfsClientRetcode, NABoolean doRetry,
  ...
  static HDFS_Client_RetCode    hdfsTruncateFile(const char* filename);
  ...
  // if allFiles is TRUE, return all files. 
  // If FALSE, return valid non-zero len files only
  static HDFS_Client_RetCode    getNumFiles(char * rootDir, NABoolean allFiles,
                                            Lng32& numFiles);
