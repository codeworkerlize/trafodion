...
const char *getESPStreamTypeString(ESPMessageTypeEnum t) {
  switch (t) {
    case IPC_MSG_SQLESP_CONTROL_REQUEST:
      return "IPC_MSG_SQLESP_CONTROL_REQUEST";
    case IPC_MSG_SQLESP_CONTROL_REPLY:
      return "IPC_MSG_SQLESP_CONTROL_REPLY";
    case IPC_MSG_SQLESP_DATA_REQUEST:
      return "IPC_MSG_SQLESP_DATA_REQUEST";
    case IPC_MSG_SQLESP_DATA_REPLY:
      return "IPC_MSG_SQLESP_DATA_REPLY";
    case IPC_MSG_SQLESP_CANCEL_REQUEST:
      return "IPC_MSG_SQLESP_CANCEL_REQUEST";
    case IPC_MSG_SQLESP_CANCEL_REPLY:
      return "IPC_MSG_SQLESP_CANCEL_REPLY";
    case IPC_MSG_SQLESP_SERVER_ROUTING:
      return "IPC_MSG_SQLESP_SERVER_ROUTING";
...
const char *getESPMessageObjTypeString(ESPMessageObjTypeEnum t) {
  switch (t) {
    case ESP_INPUT_DATA_HDR:
      return "ESP_INPUT_DATA_HDR";
    case ESP_CONTINUE_HDR:
      return "ESP_CONTINUE_HDR";
    case ESP_RETURN_DATA_HDR:
      return "ESP_RETURN_DATA_HDR";
    case ESP_LOAD_FRAGMENT_HDR:
      return "ESP_LOAD_FRAGMENT_HDR";
    case ESP_FIXUP_FRAGMENT_HDR:
      return "ESP_FIXUP_FRAGMENT_HDR";
    case ESP_RELEASE_FRAGMENT_HDR:
      return "ESP_RELEASE_FRAGMENT_HDR";
    case ESP_OPEN_HDR:
      return "ESP_OPEN_HDR";
    case ESP_PARTITION_INPUT_DATA_HDR:
      return "ESP_PARTITION_INPUT_DATA_HDR";
    case ESP_WORK_TRANSACTION_HDR:
      return "ESP_WORK_TRANSACTION_HDR";
    case ESP_RELEASE_TRANSACTION_HDR:
      return "ESP_RELEASE_TRANSACTION_HDR";
    case ESP_CANCEL_HDR:
      return "ESP_CANCEL_HDR";
    case ESP_LATE_CANCEL_HDR:
      return "ESP_LATE_CANCEL_HDR";
    case ESP_RETURN_STATUS_HDR:
      return "ESP_RETURN_STATUS_HDR";
    case ESP_RETURN_CANCEL_HDR:
      return "ESP_RETURN_CANCEL_HDR";
    case ESP_FRAGMENT_KEY:
      return "ESP_FRAGMENT_KEY";
    case ESP_INPUT_SQL_BUFFER:
      return "ESP_INPUT_SQL_BUFFER";
    case ESP_OUTPUT_SQL_BUFFER:
      return "ESP_OUTPUT_SQL_BUFFER";
    case ESP_PROCESS_IDS_OF_FRAG:
      return "ESP_PROCESS_IDS_OF_FRAG";
    case ESP_LATE_NAME_INFO:
      return "ESP_LATE_NAME_INFO";
    case ESP_DIAGNOSTICS_AREA:
      return "ESP_DIAGNOSTICS_AREA";
    case ESP_STATISTICS:
      return "ESP_STATISTICS";
    case ESP_FRAGMENT:
      return "ESP_FRAGMENT";
    case ESP_TRANSID:
      return "ESP_TRANSID";
    case ESP_TIMEOUT_DATA:
      return "ESP_TIMEOUT_DATA";
    case ESP_SM_DOWNLOAD_INFO:
      return "ESP_SM_DOWNLOAD_INFO";
    case ESP_RESOURCE_INFO:
      return "ESP_RESOURCE_INFO";
    case ESP_SECURITY_INFO:
      return "ESP_SECURITY_INFO";
...
void ExEspMsgObj::unpackObj(IpcMessageObjType objType, IpcMessageObjVersion objVersion, NABoolean sameEndianness,
...
ExEspRequestHeader::ExEspRequestHeader(ESPMessageObjTypeEnum objType, IpcMessageObjVersion objVersion, NAMemory *heap)
    : ExEspMsgObj(objType, objVersion, heap) {}

// constructor used to perform copyless receive, maps packed objects in place
ExEspRequestHeader::ExEspRequestHeader(IpcBufferedMsgStream *msgStream) : ExEspMsgObj(msgStream) {}

// -----------------------------------------------------------------------
// Methods for class ExEspReplyHeader
// -----------------------------------------------------------------------

ExEspReplyHeader::ExEspReplyHeader(ESPMessageObjTypeEnum objType, IpcMessageObjVersion objVersion, NAMemory *heap)
    : ExEspMsgObj(objType, objVersion, heap) {}

// constructor used to perform copyless receive, maps packed objects in place
ExEspReplyHeader::ExEspReplyHeader(IpcBufferedMsgStream *msgStream) : ExEspMsgObj(msgStream) {}
...
// constructor used to perform copyless receive, maps packed objects in place
ExEspInputDataReqHeader::ExEspInputDataReqHeader(IpcBufferedMsgStream *msgStream) : ExEspRequestHeader(msgStream) {
...
IpcMessageObjSize ExEspInputDataReqHeader::packedLength() { return sizeof(*this); }
...
// constructor used to perform copyless receive, maps packed objects in place
ExEspContinueReqHeader::ExEspContinueReqHeader(IpcBufferedMsgStream *msgStream) : ExEspRequestHeader(msgStream) {
  if (endianness_ != IpcMyEndianness) {
    ABORT("TO BE DONE: need to swap endianness!");
    endianness_ = IpcMyEndianness;
  }
...
IpcMessageObjSize ExEspContinueReqHeader::packedLength() { return sizeof(*this); }
...
// constructor used to perform copyless receive, maps packed objects in place
ExEspReturnDataReplyHeader::ExEspReturnDataReplyHeader(IpcBufferedMsgStream *msgStream) : ExEspReplyHeader(msgStream) {
  if (endianness_ != IpcMyEndianness) {
    ABORT("TO BE DONE: need to swap endianness!");
    endianness_ = IpcMyEndianness;
  }
...
IpcMessageObjSize ExEspReturnDataReplyHeader::packedLength() { return sizeof(*this); }
...
ExEspLoadFragmentReqHeader::ExEspLoadFragmentReqHeader(NAMemory *heap)
    : ExEspRequestHeader(ESP_LOAD_FRAGMENT_HDR, CurrLoadFragmentHdrVersion, heap), spare1_(0), spare2_(0) {}

IpcMessageObjSize ExEspLoadFragmentReqHeader::packedLength() { return sizeof(*this); }
...
ExEspFixupFragmentReqHeader::ExEspFixupFragmentReqHeader(NAMemory *heap)
    : ExEspRequestHeader(ESP_FIXUP_FRAGMENT_HDR, CurrFixupFragmentHdrVersion, heap),
      flags_(0),
...
IpcMessageObjSize ExEspFixupFragmentReqHeader::packedLength() { return sizeof(*this); }
...
ExEspReleaseFragmentReqHeader::ExEspReleaseFragmentReqHeader(NAMemory *heap)
    : ExEspRequestHeader(ESP_RELEASE_FRAGMENT_HDR, CurrReleaseFragmentHdrVersion, heap),
      idleTimeout_(0),
      flags_(0),
...
IpcMessageObjSize ExEspReleaseFragmentReqHeader::packedLength() { return sizeof(*this); }
...
// constructor used to perform copyless receive, maps packed objects in place
ExEspOpenReqHeader::ExEspOpenReqHeader(IpcBufferedMsgStream *msgStream) : ExEspRequestHeader(msgStream) {
  if (endianness_ != IpcMyEndianness) {
    ABORT("TO BE DONE: need to swap endianness!");
    endianness_ = IpcMyEndianness;
  }
...
IpcMessageObjSize ExEspOpenReqHeader::packedLength() { return sizeof(*this); }
...
ExEspPartInputDataReqHeader::ExEspPartInputDataReqHeader(NAMemory *heap)
    : ExEspRequestHeader(ESP_PARTITION_INPUT_DATA_HDR, CurrPartitionInputDataHdrVersion, heap),
      staticAssignment_(FALSE),
      askForMoreWorkWhenDone_(FALSE),
      spare1_(0),
      spare2_(0) {
  memset(clientInfo_, 0, 30);
}

IpcMessageObjSize ExEspPartInputDataReqHeader::packedLength() { return sizeof(*this); }
...
ExEspWorkReqHeader::ExEspWorkReqHeader(NAMemory *heap)
    : ExEspRequestHeader(ESP_WORK_TRANSACTION_HDR, CurrWorkTransactionHdrVersion, heap), spare1_(0), spare2_(0) {}

IpcMessageObjSize ExEspWorkReqHeader::packedLength() { return sizeof(*this); }
...
ExEspReleaseWorkReqHeader::ExEspReleaseWorkReqHeader(NAMemory *heap)
    : ExEspRequestHeader(ESP_RELEASE_TRANSACTION_HDR, CurrReleaseTransactionHdrVersion, heap),
      allWorkRequests_(FALSE),
      inactiveTimeout_(0),
      savepointId_(0),
      flags_(0) {}

IpcMessageObjSize ExEspReleaseWorkReqHeader::packedLength() { return sizeof(*this); }
...
// constructor used to perform copyless receive, maps packed objects in place
ExEspCancelReqHeader::ExEspCancelReqHeader(IpcBufferedMsgStream *msgStream) : ExEspRequestHeader(msgStream) {
  if (endianness_ != IpcMyEndianness) {
    ABORT("TO BE DONE: need to swap endianness!");
    endianness_ = IpcMyEndianness;
  }
...
IpcMessageObjSize ExEspCancelReqHeader::packedLength() { return sizeof(*this); }
...
ExEspCancelReplyHeader::ExEspCancelReplyHeader(NAMemory *heap)
    : ExEspReplyHeader(ESP_RETURN_CANCEL_HDR, CurrCancelReplyHdrVersion, heap), spare1_(0), spare2_(0) {}

// constructor used to perform copyless receive, maps packed objects in place
ExEspCancelReplyHeader::ExEspCancelReplyHeader(IpcBufferedMsgStream *msgStream) : ExEspReplyHeader(msgStream) {}

IpcMessageObjSize ExEspCancelReplyHeader::packedLength() { return sizeof(*this); }
...
// constructor used to perform copyless receive, maps packed objects in place
ExEspLateCancelReqHeader::ExEspLateCancelReqHeader(IpcBufferedMsgStream *msgStream) : ExEspRequestHeader(msgStream) {
  // if (endianness_ != IpcMyEndianness)
  //   ABORT("TO BE DONE: need to swap endianness!");
}

IpcMessageObjSize ExEspLateCancelReqHeader::packedLength() { return sizeof(*this); }
...
// constructor used to perform copyless receive, maps packed objects in place
ExEspReturnStatusReplyHeader::ExEspReturnStatusReplyHeader(IpcBufferedMsgStream *msgStream)
...
IpcMessageObjSize ExEspReturnStatusReplyHeader::packedLength() { return sizeof(*this); }
...
TupMsgBuffer::TupMsgBuffer(int bufferLen, InOut inOut, NAMemory *heap)
  ...
  theBuffer_->driveInit(bufferLen, FALSE, SqlBuffer::DENSE_);
...
TupMsgBuffer::TupMsgBuffer(IpcBufferedMsgStream *msgStream) : ExEspMsgObj(msgStream) {
...
void TupMsgBuffer::unpackObj(IpcMessageObjType objType, IpcMessageObjVersion objVersion, NABoolean sameEndianness,
                             IpcMessageObjSize objSize, IpcConstMessageBufferPtr buffer) {
  ex_assert((objType == ESP_INPUT_SQL_BUFFER OR objType == ESP_OUTPUT_SQL_BUFFER)
                    AND objVersion == 100 AND sameEndianness AND objSize > sizeof(IpcMessageObj),
  ...
  ex_assert(sqlBufferSize == theBuffer_->get_buffer_size(), "Buffer size mismatch");
}
...
IpcMessageObjSize ExProcessIdsOfFrag::packObjIntoMessage(IpcMessageBufferPtr buffer) {
  ...
  for (CollIndex i = 0; i < (CollIndex)np; i++) {
    ...
    IpcMessageObjSize pidsize = processIds_[i].packDependentObjIntoMessage(buffer);
    result += pidsize;
    buffer += pidsize;
  }
...
void ExProcessIdsOfFrag::unpackObj(IpcMessageObjType objType, IpcMessageObjVersion objVersion, NABoolean sameEndianness,
                                   IpcMessageObjSize objSize, IpcConstMessageBufferPtr buffer) {
  ex_assert(objType == ESP_PROCESS_IDS_OF_FRAG AND objVersion ==
                CurrProcessIdsOfFragVersion AND sameEndianness AND objSize >=
                    baseClassPackedLength() + sizeof(fragmentId_) + sizeof(CollIndex),
...
ExProcessIdsOfFrag *ExProcessIdsOfFragList::findEntry(ExFragId fragId) const {
...
ExMsgFragment::ExMsgFragment(NAMemory *heap) : ExEspMsgObj(ESP_FRAGMENT, CurrFragmentVersion, heap) {
  fragment_ = NULL;
  f_.fragType_ = 0;
  f_.parentId_ = 0;
  f_.topNodeOffset_ = 0;
...
ExMsgFragment::ExMsgFragment(const ExFragKey &key, ExFragDir::ExFragEntryType fragType, ExFragId parentId,
...
#ifdef _DEBUG
  if (getenv("TEST_ERROR_AT_EXPR_NO_ESP")) f_.injectErrorAtExprFreq_ = 0;
#endif
  ...
  if (tenantName) {
    f_.tenantName_ = new (getHeap()) char[tenantNameLen];
    str_cpy_all(f_.tenantName_, tenantName, tenantNameLen);
    f_.tenantNameLen_ = tenantNameLen;
  } else {
    f_.tenantName_ = NULL;
    f_.tenantNameLen_ = 0;
  }
  ...
  if (needToWorkVec) {
    f_.needToWorkVec_ = new (getHeap()) char[needToWorkVecLen];
    str_cpy_all(f_.needToWorkVec_, needToWorkVec, needToWorkVecLen);
    f_.needToWorkVecLen_ = needToWorkVecLen;
  } else {
    f_.needToWorkVec_ = NULL;
    f_.needToWorkVecLen_ = 0;
  }
...
ExMsgFragment::~ExMsgFragment() {
  if (f_.queryId_ != NULL) getHeap()->deallocateMemory(f_.queryId_);
  if (f_.userName_ != NULL) getHeap()->deallocateMemory(f_.userName_);
  if (f_.tenantName_ != NULL) getHeap()->deallocateMemory(f_.tenantName_);
  if (f_.needToWorkVec_ != NULL) getHeap()->deallocateMemory(f_.needToWorkVec_);
  if (f_.iOwnTheFragment_) getHeap()->deallocateMemory(fragment_);
}

IpcMessageObjSize ExMsgFragment::packedLength() {
  if (f_.compressedLength_ == 0) f_.compressedLength_ = str_compress_size(fragment_, f_.fragmentLength_);
  ...
  if (f_.queryId_ != NULL) result += f_.queryIdLen_;
  alignSizeForNextObj(result);

  if (f_.userName_ != NULL) result += f_.userNameLen_;
  alignSizeForNextObj(result);

  if (f_.tenantName_ != NULL) result += f_.tenantNameLen_;
  alignSizeForNextObj(result);

  if (f_.needToWorkVec_ != NULL) result += f_.needToWorkVecLen_;
  alignSizeForNextObj(result);
...
IpcMessageObjSize ExMsgFragment::packObjIntoMessage(IpcMessageBufferPtr buffer) {
  ...
  if (f_.tenantName_ != NULL) {
    str_cpy_all(buffer, f_.tenantName_, f_.tenantNameLen_);
    buffer += f_.tenantNameLen_;
    alignBufferForNextObj(buffer);
  }

  if (f_.needToWorkVec_ != NULL) {
    str_cpy_all(buffer, f_.needToWorkVec_, f_.needToWorkVecLen_);
    buffer += f_.needToWorkVecLen_;
    alignBufferForNextObj(buffer);
  }
  ...
  if (f_.compressedLength_ > 0 && f_.compressedLength_ < f_.fragmentLength_) {
    ...
    ex_assert(cmprSize == f_.compressedLength_, "Error during compress a fragment for download");
...
void ExMsgFragment::unpackObj(IpcMessageObjType objType, IpcMessageObjVersion objVersion, NABoolean sameEndianness,
                              IpcMessageObjSize objSize, IpcConstMessageBufferPtr buffer) {
  // See description of packed format in packedLength() method

  // get rid of existing data members
  if (fragment_ AND f_.iOwnTheFragment_) getHeap()->deallocateMemory(fragment_);

  ex_assert(objType == ESP_FRAGMENT AND objVersion == CurrFragmentVersion AND sameEndianness,
  ...
  if (f_.tenantName_ != NULL) {
    getHeap()->deallocateMemory(f_.tenantName_);
    f_.tenantName_ = NULL;
  }

  if (f_.needToWorkVec_ != NULL) {
    getHeap()->deallocateMemory(f_.needToWorkVec_);
    f_.needToWorkVec_ = NULL;
  }
  ...
  if (f_.tenantName_ != NULL) {
    f_.tenantName_ = new (getHeap()) char[f_.tenantNameLen_];
    str_cpy_all(f_.tenantName_, buffer, f_.tenantNameLen_);
    buffer += f_.tenantNameLen_;
    alignBufferForNextObj(buffer);
  }

  if (f_.needToWorkVec_ != NULL) {
    f_.needToWorkVec_ = new (getHeap()) char[f_.needToWorkVecLen_];
    str_cpy_all(f_.needToWorkVec_, buffer, f_.needToWorkVecLen_);
    buffer += f_.needToWorkVecLen_;
    alignBufferForNextObj(buffer);
  }
  ...
  if (f_.compressedLength_ > 0 && f_.compressedLength_ < f_.fragmentLength_) {
    ...
    ex_assert(decpSize == f_.fragmentLength_, "Error during uncompress a downloaded fragment");
    buffer += f_.compressedLength_;
  } else {
...
NABoolean ExMsgFragment::needToWork(ULng32 espIdx) {
  ...
  return (NABoolean)(f_.needToWorkVec_[espIdx]);
}

void ExMsgFragment::displayNeedToWorkVec(ostream &out) {
  out << "needToWorkVecLen=" << f_.needToWorkVecLen_ << endl;

  for (int i = 0; i < f_.needToWorkVecLen_; i++) {
    out << "needToWorkVec[" << i << "]=" << (int)(f_.needToWorkVec_[i]) << endl;
  }
  out << endl;
}
...
IpcMessageObjSize ExMsgTransId::packedLength() { return sizeof(*this); }

// -----------------------------------------------------------------------
// Methods for class ExMsgTimeoutData
// -----------------------------------------------------------------------

ExMsgTimeoutData::ExMsgTimeoutData(TimeoutData *toData, NAMemory *heap)
    : ExEspMsgObj(ESP_TIMEOUT_DATA, CurrTimeoutDataVersion, heap), timeoutData_(toData) {
...
IpcMessageObjSize ExMsgTimeoutData::packedLength() { return baseClassPackedLength() + timeoutData_->packedLength(); }

IpcMessageObjSize ExMsgTimeoutData::packObjIntoMessage(IpcMessageBufferPtr buffer) {
  IpcMessageBufferPtr start = buffer;
...
void ExMsgTimeoutData::unpackObj(IpcMessageObjType objType, IpcMessageObjVersion objVersion, NABoolean sameEndianness,
                                 IpcMessageObjSize objSize, IpcConstMessageBufferPtr buffer) {
  IpcConstMessageBufferPtr start = buffer;

  ex_assert(objType == ESP_TIMEOUT_DATA AND objVersion == 100 AND sameEndianness AND objSize > sizeof(IpcMessageObj),
...
ExSMDownloadInfo::~ExSMDownloadInfo() {
  if (smTraceFilePrefix_) getHeap()->deallocateMemory(smTraceFilePrefix_);
}
...
void ExSMDownloadInfo::unpackObj(IpcMessageObjType objType, IpcMessageObjVersion objVersion, NABoolean sameEndianness,
                                 IpcMessageObjSize objSize, IpcConstMessageBufferPtr buffer) {
  ex_assert(objType == ESP_SM_DOWNLOAD_INFO && objVersion == 100 && sameEndianness && objSize > sizeof(IpcMessageObj),
...
ExMsgResourceInfo::ExMsgResourceInfo(const ExScratchFileOptions *sfo, NAMemory *heap)
    : ExEspMsgObj(ESP_RESOURCE_INFO, CurrResourceInfoVersion, heap), sfo_(sfo), spare_(0) {
  if (sfo_)
...
IpcMessageObjSize ExMsgResourceInfo::packedLength() {
  return baseClassPackedLength() + sfo_->ipcPackedLength() + sizeof(totalNameLength_) + sizeof(spare_);
}

IpcMessageObjSize ExMsgResourceInfo::packObjIntoMessage(IpcMessageBufferPtr buffer) {
  IpcMessageBufferPtr start = buffer;
...
void ExMsgResourceInfo::unpackObj(IpcMessageObjType objType, IpcMessageObjVersion objVersion, NABoolean sameEndianness,
                                  IpcMessageObjSize objSize, IpcConstMessageBufferPtr buffer) {
  ExScratchFileOptions *sfo;
  IpcConstMessageBufferPtr start = buffer;

  ex_assert(objType == ESP_RESOURCE_INFO AND objVersion == 100 AND sameEndianness AND objSize > sizeof(IpcMessageObj),
  ...
  sfo->ipcUnpackObj(objSize - (buffer - start), buffer, getHeap(), totalNameLength_, bufferForDependentObjects_);
}

// Methods for ExMsgSecurityInfo

// constructor
ExMsgSecurityInfo::ExMsgSecurityInfo(NAMemory *heap) : ExEspMsgObj(ESP_SECURITY_INFO, CurrTimeoutDataVersion, heap) {
  securityKey_ = NULL;
  authid_ = NULL;
}
...
void ExMsgSecurityInfo::unpackObj(IpcMessageObjType objType, IpcMessageObjVersion objVersion, NABoolean sameEndianness,
                                  IpcMessageObjSize objSize, IpcConstMessageBufferPtr buffer) {
