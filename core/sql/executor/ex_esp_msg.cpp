...
IpcMessageObjSize ExMsgFragment::packedLength()
{
  ...
  if (f_.tenantName_ != NULL)
    result += f_.tenantNameLen_;
  alignSizeForNextObj(result);

  if (f_.needToWorkVec_ != NULL)
    result += f_.needToWorkVecLen_;
  alignSizeForNextObj(result);
...
{
  if ( f_.needToWorkVecLen_ == 0 )
    return TRUE;

  if( espIdx >= f_.needToWorkVecLen_ )
    return FALSE;
...
{
   out << "needToWorkVecLen=" << f_.needToWorkVecLen_
       << endl;

   for ( int i=0; i<f_.needToWorkVecLen_; i++ ) {
     out << "needToWorkVec[" << i << "]=" 
         << (int)(f_.needToWorkVec_[i]) 
         << endl;
   }
   out << endl;
}
