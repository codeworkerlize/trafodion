...
static const char* const hdfsClientErrorEnumStr[] = 
{
 ...
 ,"Error in HdfsClient::getNumFiles()."
 ,"Java exception in HdfsClient::getNumFiles()."
 ,"Error in HdfsClient::hdfsTruncateFile()."
 ,"Java exception in HdfsClient::hdfsTruncateFile()."
};
...
HDFS_Client_RetCode HdfsClient::init()
{
  ...
  else
  {
    ...
    JavaMethods_[JM_HDFS_WRITE_IMMEDIATE].jm_signature = "([BZZI)J";
    ...
    JavaMethods_[JM_HDFS_GET_NUM_FILES       ].jm_name      = "getNumFiles";
    JavaMethods_[JM_HDFS_GET_NUM_FILES       ].jm_signature = "(Ljava/lang/String;Z)I";
    JavaMethods_[JM_HDFS_TRUNCATE       ].jm_name      = "hdfsTruncateFile";
    JavaMethods_[JM_HDFS_TRUNCATE       ].jm_signature = "(Ljava/lang/String;)Z";
...
Int64 HdfsClient::hdfsWriteImmediate(const char* data, Int64 len, HDFS_Client_RetCode &hdfsClientRetcode, NABoolean doRetry, 
                            NABoolean useLock, int timeout)
{
  QRLogger::log(CAT_SQL_HDFS, LL_DEBUG, "HdfsClient::hdfsWriteImmediate(%ld) called.", len);
     ...
     jboolean j_useLock = useLock;
     jint j_timeout = timeout;
     ...
     // Java method returns the cumulative bytes written
     writeOffset = jenv_->CallLongMethod(javaObj_, JavaMethods_[JM_HDFS_WRITE_IMMEDIATE].methodID, jbArray,j_doRetry, j_useLock, j_timeout);

     if (hdfsStats_ != NULL) {
         hdfsStats_->incMaxHdfsIOTime(hdfsStats_->getHdfsTimer().stop());
         hdfsStats_->incHdfsCalls();
         hdfsStats_->incBytesRead(len);
     }
     
     if (jenv_->ExceptionCheck())
     {
        getExceptionDetails(__FILE__, __LINE__, "HdfsClient::hdfsWriteImmediate()");
        jenv_->PopLocalFrame(NULL);      
        hdfsClientRetcode = HDFS_CLIENT_ERROR_HDFS_WRITE_IMMEDIATE_EXCEPTION_RETRY;
        return -1;
     }
     jenv_->PopLocalFrame(NULL);
     hdfsClientRetcode = HDFS_CLIENT_OK;
...
HDFS_Client_RetCode HdfsClient::hdfsTruncateFile(const char* filename)
{
  QRLogger::log(CAT_SQL_HDFS, LL_DEBUG, "HdfsClient::hdfsTruncateFIle(%s) called.", filename);

  if (initJNIEnv() != JOI_OK)
     return HDFS_CLIENT_ERROR_TRUNCATE_PARAM;
  ...
  tsRecentJMFromJNI = JavaMethods_[JM_HDFS_TRUNCATE].jm_full_name;
  jboolean jresult = jenv_->CallStaticBooleanMethod(javaClass_, JavaMethods_[JM_HDFS_TRUNCATE].methodID, js_filename);

  if (jenv_->ExceptionCheck())
  {
    getExceptionDetails(__FILE__, __LINE__, "HdfsClient::hdfsTruncateFile()");
    jenv_->PopLocalFrame(NULL);
    return HDFS_CLIENT_ERROR_TRUNCATE_EXCEPTION;
  }

  if (jresult == false)
  {
    logError(CAT_SQL_HDFS, "HdfsClient::hdfsTruncateFile()", getLastError());
    jenv_->PopLocalFrame(NULL);
    return HDFS_CLIENT_ERROR_TRUNCATE_EXCEPTION;
  }
...
HDFS_Client_RetCode HdfsClient::hdfsCreateDirectory(const NAString &dirName)
{
  ...
  jboolean jresult = 
        (jboolean)jenv_->CallStaticBooleanMethod(javaClass_,
...
// getNumFiles 
//////////////////////////////////////////////////////////////////////////////
HDFS_Client_RetCode HdfsClient::getNumFiles(char * rootDir, NABoolean allFiles,
                                            Lng32& numFiles)
{
  QRLogger::log(CAT_SQL_HBASE, LL_DEBUG, "HdfsClient::getNumFiles() called.");

  if (initJNIEnv() != JOI_OK)
    return HDFS_CLIENT_ERROR_NUMFILES_PARAM;
  ...
  jboolean j_allFiles = allFiles;

  tsRecentJMFromJNI = JavaMethods_[JM_HDFS_GET_NUM_FILES].jm_full_name;
  jint j_numFiles = 
    (jint)jenv_->CallStaticIntMethod(javaClass_, 
				     JavaMethods_[JM_HDFS_GET_NUM_FILES].methodID,
				     js_rootDir,
                                     j_allFiles);

  if (jenv_->ExceptionCheck())
  {
    getExceptionDetails(__FILE__, __LINE__, "HdfsClient::getNumFiles()");
    jenv_->PopLocalFrame(NULL);
    return HDFS_CLIENT_ERROR_NUMFILES_EXCEPTION;
  }
