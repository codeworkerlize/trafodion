      ...
      if (!hFileParamsInitialized_)
      {
        if (hbaseAccessTdb().replaceNameByUID())
          importLocation_= std::string(((ExHbaseAccessTdb&)hbaseAccessTdb()).getLoadPrepLocation()) +
            ((ExHbaseAccessTdb&)hbaseAccessTdb()).getDataUIDName() ;
        else
          importLocation_= std::string(((ExHbaseAccessTdb&)hbaseAccessTdb()).getLoadPrepLocation()) +
            ((ExHbaseAccessTdb&)hbaseAccessTdb()).getTableName() ;
        familyLocation_ = std::string(importLocation_ + "/" +
                                      hbaseAccessTdb().getColumnFamily());
        replace(familyLocation_.begin(), familyLocation_.end(), ':', '_');
        Lng32 fileNum = getGlobals()->castToExExeStmtGlobals()->getMyInstanceNumber();
        ...
        Text importSampleLoc;
        if (samplingRate > 0)
          {
            std::string tmpSampleName = std::string(table_.val) + std::string("_BULK_LOAD_SAMPLE");
            importSampleLoc = std::string(((ExHbaseAccessTdb&)hbaseAccessTdb()).getSampleLocation()) +
                              tmpSampleName;
            importSampleLoc = std::string(importSampleLoc + "/" +
                                          hbaseAccessTdb().getColumnFamily());
            replace(importSampleLoc.begin(), importSampleLoc.end(), ':', '_');
          }

        retcode = ehi_->initHFileParams(table_, familyLocation_, hFileName_,
                                        hbaseAccessTdb().getMaxHFileSize(),
                                        importSampleLoc, hFileName_,samplingRate);
