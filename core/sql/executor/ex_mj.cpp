...
ex_tcb *ex_mj_tdb::build(ex_globals *glob) {
  ...
  if (isLeftUnique() || isRightUnique()) {
    mj_tcb = new (glob->getSpace()) ex_mj_unique_tcb(*this, *left_tcb, *right_tcb, glob);
  } else {
...
ex_mj_tcb::ex_mj_tcb(const ex_mj_tdb &mj_tdb, const ex_tcb &left_tcb, const ex_tcb &right_tcb, ex_globals *glob)
    : ex_tcb(mj_tdb, 1, glob),
      dupPool_(NULL),
      leftEncodedKeyPtr_(NULL),
      rightEncodedKeyPtr_(NULL),
      lookAheadState_(LA_LEFT_UNCHECKED),
      postIoStep_(ex_mj_tcb::MJ_CANCEL),
      prevRightAtp_(NULL),
  ...
  if (!mj_tdb.isLeftUnique() && !mj_tdb.isRightUnique()) {
    ...
    dupPool_ = new (space) ExDupSqlBuffer(nBuffers, bufSize, nReserveEntries, mj_tdb.rightDupRecLen_, space);
    ...
    tspace_ = new (space) TupleSpace(mj_tdb.rightDupRecLen_, TupleSpace::MIN_INITIAL_BUFFERS, tsBufferSize, quotaMB,
                                     rightCopyDupExpr(), mj_tdb.workCriDesc_, heap, isOverflowEnabled(), stmtGlobals,
                                     mj_tdb.getScratchThresholdPct(), ovMode, mj_tdb.getYieldQuota());
  }
  ...
  if (mj_tdb.encodedKeyCompOpt()) {
    ...
    workAtp_->getTupp(mj_tdb.encodedKeyWorkAtpIndex_) = new (space) tupp_descriptor();
  ...
  // fixup expressions
  if (mergeExpr())
    (void)mergeExpr()->fixup(0, getExpressionMode(), this, glob->getSpace(), glob->getDefaultHeap(), FALSE, glob);

  if (compExpr())
    (void)compExpr()->fixup(0, getExpressionMode(), this, glob->getSpace(), glob->getDefaultHeap(), FALSE, glob);

  if (leftEncodedKeyExpr())
    (void)leftEncodedKeyExpr()->fixup(0, getExpressionMode(), this, glob->getSpace(), glob->getDefaultHeap(), FALSE,
                                      glob);

  if (rightEncodedKeyExpr())
    (void)rightEncodedKeyExpr()->fixup(0, getExpressionMode(), this, glob->getSpace(), glob->getDefaultHeap(), FALSE,
                                       glob);

  if (postJoinExpr())
    (void)postJoinExpr()->fixup(0, getExpressionMode(), this, glob->getSpace(), glob->getDefaultHeap(), FALSE, glob);

  if (preJoinExpr())
    (void)preJoinExpr()->fixup(0, getExpressionMode(), this, glob->getSpace(), glob->getDefaultHeap(), FALSE, glob);

  if (leftCheckDupExpr())
    (void)leftCheckDupExpr()->fixup(0, getExpressionMode(), this, glob->getSpace(), glob->getDefaultHeap(), FALSE,
                                    glob);

  if (rightCheckDupExpr())
    (void)rightCheckDupExpr()->fixup(0, getExpressionMode(), this, glob->getSpace(), glob->getDefaultHeap(), FALSE,
                                     glob);

  if (ljExpr())
    (void)ljExpr()->fixup(0, getExpressionMode(), this, glob->getSpace(), glob->getDefaultHeap(), FALSE, glob);

  if (rightCopyDupExpr())
    (void)rightCopyDupExpr()->fixup(0, getExpressionMode(), this, glob->getSpace(), glob->getDefaultHeap(), FALSE,
                                    glob);
}
...
void ex_mj_tcb::checkInit(void) {
  ex_assert((rightCopyDupExpr()), "Regular merge join should always have a rightCopyDupExpr");
}

void ex_mj_tcb::createDiags(Int16 sqlCode) {
  Int16 sysError = 0;
...
bool ex_mj_tcb::processError(atp_struct *entryAtp) {
  ...
  if (entryAtp) upEntry->copyAtp(entryAtp);
...
bool ex_mj_tcb::reacquireResources(void) {
  if (tspace_) {
    ...
    if (mjTdb().getLogDiagnostics()) {
      ...
      str_sprintf(msg, "Merge join initial TupleSpace memory is %d bytes", memorySize);
      SQLMXLoggingArea::logExecRtInfo(__FILE__, __LINE__, msg, mjTdb().getExplainNodeId());
    }
  }  // if tspace_
...
void ex_mj_tcb::registerSubtasks() {
  ...
  if (tspace_) {
    ...
    ExSubtask *ioEventHandler = sched->registerNonQueueSubtask(sWork, this, "WK");
...
short ex_mj_tcb::stop(ex_mj_private_state &pstate) {
  ...
  if (qparent.up->isFull()) return WORK_OK;
  ...
  if (qparent.down->isEmpty()) {
    if (tspace_) {
      ...
      if (mjTdb().getLogDiagnostics()) {
        ...
        str_sprintf(msg, "Merge join released resources, max memory %d", maxMemory);
        SQLMXLoggingArea::logExecRtInfo(__FILE__, __LINE__, msg, mjTdb().getExplainNodeId());
      }
    }
    return WORK_OK;
...
ex_mj_tcb::Comparison ex_mj_tcb::compareTuples(ex_queue_entry *lentry, ex_queue_entry *rentry, bool doFullComparison) {
  Comparison cmpResult = CMP_ERROR;

  ex_expr::exp_return_type retCode = ex_expr::EXPR_FALSE;

  if (mjTdb().encodedKeyCompOpt()) {
    workAtp_->getTupp(mjTdb().encodedKeyWorkAtpIndex_).setDataPointer(leftEncodedKeyPtr_);
    retCode = leftEncodedKeyExpr()->eval(lentry->getAtp(), workAtp_);
    if (retCode != ex_expr::EXPR_ERROR) {
      workAtp_->getTupp(mjTdb().encodedKeyWorkAtpIndex_).setDataPointer(rightEncodedKeyPtr_);
  ...
  } else {  // Not using encoded key comparison optimization
...
ex_expr::exp_return_type ex_mj_tcb::returnRow(atp_struct *leftAtp, atp_struct *rightAtp, ex_mj_private_state &pstate,
                                              bool isUniqueMj, ExOperStats *statsEntry) {
  ...
  ex_assert((!qparent.up->isFull()), "returnRow() called when parent up queue was full");
  ...
  if (retCode == ex_expr::EXPR_TRUE) {  // satisfied pre-join predicate
    ...
    if (!isAntiJoin()) {  // not anti-semi-join
      ...
      if (isLeftJoin() && ljExpr()) {
        if (pool_->get_free_tuple(parentAtp->getTupp(pentry->numTuples() - 1), mjTdb().ljRecLen_)) {
          ...
          } else if (pool_->get_free_tuple(parentAtp->getTupp(pentry->numTuples() - 1), mjTdb().ljRecLen_)) {
...
short ex_mj_tcb::work() {
  ...
  if (qparent.down->isEmpty()) return WORK_OK;
  ...
  while (true) {
    ...
    switch (pstate.step_) {
      ...
      case ex_mj_tcb::MJ_GET_LEFT: {
        ...
        switch (lentry->upState.status) {
          case ex_queue::Q_OK_MMORE: {
            if (noSavedDups()) {
            ...
            } else {  // check for duplicate left row
              ...
              } else {  // get duplicate from TupleSpace
                atp_struct *currentAtp = NULL;
                IoStatus status = tspace_->current(currentAtp);
                ex_assert((status != END_OF_DATA), "Duplicate right row not in TupleSpace");
                if (status == OK) {
                  retCode = leftCheckDupExpr()->eval(leftAtp, currentAtp);
      ...
      case ex_mj_tcb::MJ_GET_RIGHT: {
        ...
        switch (rentry->upState.status) {
          case ex_queue::Q_OK_MMORE: {
            ...
            if (compare == CMP_EQUAL) {  // left and right rows matched
              ...
              if (lookAheadState_ == LA_LEFT_UNCHECKED) {  // next left not checked
                queue_index nextIndex = qleft.up->getHeadIndex() + 1;
                if (qleft.up->entryExists(nextIndex)) {
                  ex_queue_entry *nextLeft = qleft.up->getQueueEntry(nextIndex);
              ...
              if ((lookAheadState_ == LA_LEFT_DUPLICATE) || (lookAheadState_ == LA_LEFT_UNCHECKED)) {
      ...
      case ex_mj_tcb::MJ_RETURN_ONE_ROW: {
        ...
        if (!(isSemiJoin() && pstate.outerMatched_)) {  // semi-join test
          ...
          if (dupPool_->getTuple(*dupTupp_)) {
            retCode = rightCopyDupExpr()->eval(rightAtp, dupAtp_);
            if (retCode == ex_expr::EXPR_ERROR) {
          ...
          ex_expr::exp_return_type retCode = returnRow(leftAtp, dupAtp_, pstate, false, statsEntry);
          if (retCode == ex_expr::EXPR_ERROR) {
            continue;
          }
        ...
        switch (rentry->upState.status) {
          case ex_queue::Q_OK_MMORE: {
            if (savedRetCode_ == ex_expr::EXPR_OK) {
              retCode = rightCheckDupExpr()->eval(rightAtp, prevRightAtp_);
            } else {
              // Use result of comparison done in MJ_GET_RIGHT
              retCode = savedRetCode_;
              savedRetCode_ = ex_expr::EXPR_OK;
            }
            if (retCode == ex_expr::EXPR_TRUE) {  // retCode == EXPR_TRUE
      ...
      case ex_mj_tcb::MJ_RETURN_SAVED_DUP_ROWS: {
        ...
        if (dupPool_->current(*dupTupp_)) {
          ...
          pstate.step_ = !tspace_->empty() ? MJ_RETURN_OVERFLOW_ROWS : MJ_FINISH_LEFT;
        }
      ...
      case ex_mj_tcb::MJ_RETURN_OVERFLOW_ROWS: {
        ...
        if (status == OK) {
          if (dupPool_->getTuple(*dupTupp_)) {
            memcpy(dupTupp_->getDataPointer(), currentAtp->getTupp(DUP_ATP_INDEX).getDataPointer(),
        ...
        if (status == OK) {  // have a duplicate right row
          ex_queue_entry *lentry = qleft.up->getHeadEntry();
          atp_struct *leftAtp = lentry->getAtp();
          retCode = returnRow(leftAtp, dupAtp_, pstate, false, statsEntry);
          if (retCode == ex_expr::EXPR_ERROR) {
      ...
      case ex_mj_tcb::MJ_FINISH_LEFT: {
        ...
        if (!pstate.outerMatched_ && (isLeftJoin() || isAntiJoin())) {  // return left row to parent
        ...
        switch (lookAheadState_) {  // switch on lookAheadState_
...
///////////////////////////////////////////////////////////////////////
ex_mj_unique_tcb::ex_mj_unique_tcb(const ex_mj_tdb &mj_tdb,  //
...
short ex_mj_unique_tcb::work() {
  ...
  while (true) {
    ...
    switch (pstate.step_) {
      ...
      case ex_mj_tcb::MJ_GET_LEFT: {
        if (qleft.up->isEmpty()) return WORK_OK;
        ...
        switch (lentry->upState.status) {
          ...
          case ex_queue::Q_SQLERROR: {
            if (!processError(lentry->getAtp())) return WORK_OK;
          } break;

          case ex_queue::Q_INVALID:
            ex_assert(0,
                      "ex_mj_unique_tcb::work() "
                      "Invalid state returned by left child");
            break;
        }
      } break;
      ...
      case ex_mj_tcb::MJ_GET_RIGHT: {
        if (qright.up->isEmpty()) return WORK_OK;
        ...
        switch (rentry->upState.status) {
          case ex_queue::Q_OK_MMORE: {
            ...
            if (compare == CMP_EQUAL) {
              ...
              if (!processError(rentry->getAtp())) return WORK_OK;
            }
          } break;
      ...
      case ex_mj_tcb::MJ_CANCEL: {
        if (cancel(pstate) == WORK_OK) return WORK_OK;
      } break;
...
//
ex_mj_private_state::ex_mj_private_state() : matchCount_(0), step_(ex_mj_tcb::MJ_UNINITIALIZED), outerMatched_(false) {}
...
const char *const ex_mj_private_state::stateNames[] = {"MJ_UNINITIALIZED",
                                                       "MJ_EMPTY",
                                                       "MJ_GET_LEFT",
                                                       "MJ_GET_RIGHT",
                                                       "MJ_RETURN_ROW",
