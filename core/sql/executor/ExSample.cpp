...
// 3. Fixup expressions.
//
ExSampleTcb::ExSampleTcb(const ExSampleTdb &myTdb, const ex_tcb &child_tcb, ex_globals *glob) : ex_tcb(myTdb, 1, glob) {
  ...
  qparent_.down = new (space) ex_queue(ex_queue::DOWN_QUEUE, myTdb.queueSizeDown_, myTdb.criDescDown_, space);

  // Allocate the private state in each entry of the down queue
  ExSamplePrivateState *p = new (space) ExSamplePrivateState(this);
  qparent_.down->allocatePstate(p, this);
  delete p;

  qparent_.up = new (space) ex_queue(ex_queue::UP_QUEUE, myTdb.queueSizeUp_, myTdb.criDescUp_, space);
  ...
  //
  if (initExpr()) initExpr()->fixup(0, getExpressionMode(), this, space, heap, FALSE, glob);

  if (balanceExpr()) balanceExpr()->fixup(0, getExpressionMode(), this, space, heap, FALSE, glob);

  if (postPred()) postPred()->fixup(0, getExpressionMode(), this, space, heap, FALSE, glob);
}
  ...
  while (1) {
    ...
    switch (pstate->step_) {
      ...
      case ExSamp_CANCELLED: {
        ...
        //
        if (child_status == ex_queue::Q_NO_DATA) pstate->step_ = ExSamp_DONE;
      ...
      case ExSamp_WORKING: {
        ...
        switch (centry->upState.status) {
          ...
          case ex_queue::Q_OK_MMORE: {
            ...
            //
            if ((samplingFactor > 0) && (retCode == ex_expr::EXPR_OK) && postPred())
              retCode = postPred()->eval(centry->getAtp(), centry->getAtp());
...
//
//
ExSamplePrivateState::ExSamplePrivateState(const ExSampleTcb *tcb) {
  matchCount_ = 0;
...
ex_tcb_private_state *ExSamplePrivateState::allocate_new(const ex_tcb *tcb) {
  return new (((ex_tcb *)tcb)->getSpace()) ExSamplePrivateState((ExSampleTcb *)tcb);
};
