...
ExWorkProcRetcode ExOnljTcb::work_phase1()
{
  ...
  QRLogger::log(CAT_SQL_EX_ONLJ,
               LL_DEBUG,
               "ExOnljTcb::work_phase1: TopToLeft(this ExOnljTcb=%p, timestamp=%s)",
               this,
               reportTimestamp());
  ...
  for( ; phaseTwo_ != tail; phaseTwo_++)
  {
    ...
    case ex_queue::Q_OK_MMORE:
      ...
      if (pstate.leftStep_ == NLJ_LEFT_STARTED)
      {
        QRLogger::log(CAT_SQL_EX_ONLJ,
               LL_DEBUG,
               "ExOnljTcb::work_phase2: left_started(this ExOnljTcb=%p, timestamp=%s)",
               this,
               reportTimestamp());
...
ExWorkProcRetcode ExOnljTcb::work_phase3()
{
  ...
  QRLogger::log(CAT_SQL_EX_ONLJ,
               LL_DEBUG,
               "ExOnljTcb::work_phase3: (this ExOnljTcb=%p, timestamp=%s) called",
               this,
               reportTimestamp());
      ...
      qparent_.down->removeHead())
  {
	...
	pstate.rightRecSkipped_ = ExConstants::EX_TRUE;
      ...
      case ex_queue::Q_NO_DATA:

       {
         ...
         QRLogger::log(CAT_SQL_EX_ONLJ,
               LL_DEBUG,
               "ExOnljTcb::work_phase3: noMoreData(this ExOnljTcb=%p, phase3s=%ld, timestamp=%s)",
               this,
               phase3s_,
               reportTimestamp());
#endif
	...
	qparent_.up->insert();
     ...
     QRLogger::log(CAT_SQL_EX_ONLJ,
                  LL_DEBUG,
                  "ExOnljTcb::work_phase3: loop over rightQ over (this ExOnljTcb=%p, timest=%s, rows=%ld, phase3s=%d times, rQempty=%d, pst.leftMatches=%d, timeSpent=%s)",
                  this,
                  reportTimestamp(),
                  rows2upq,
                  phase3s_,
