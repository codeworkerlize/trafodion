...
static const char *const dlErrorEnumStr[] = {"Doing initialization.",
...
DL_RetCode DistributedLock_JNI::init() {
  ...
  if (initJNIEnv() != JOI_OK) return DL_ERROR_INIT_JNI_ENV;
...
DL_RetCode DistributedLock_JNI::lock(const char *lockName, long timeout) {
  QRLogger::log(CAT_SQL_HBASE, LL_DEBUG, "DistributedLock_JNI::lock(lockName:%s, timeout: %ld)", lockName, timeout);
  if (initJNIEnv() != JOI_OK) return DL_ERROR_INIT_JNI_ENV;
...
DL_RetCode DistributedLock_JNI::longHeldLock(const char *lockName, const char *data, char *returnedData /* out */,
                                             size_t returnedDataMaxLength) {
  QRLogger::log(CAT_SQL_HBASE, LL_DEBUG, "DistributedLock_JNI::longHeldLock(lockName: %s, data: %s)", lockName, data);
  if (initJNIEnv() != JOI_OK) return DL_ERROR_INIT_JNI_ENV;
  jstring js_lockName = jenv_->NewStringUTF(lockName);
  if (js_lockName == NULL) {
    GetCliGlobals()->setJniErrorStr(getErrorText(DL_ERROR_PARAM));
  ...
  jstring jresult =
      (jstring)jenv_->CallObjectMethod(javaObj_, JavaMethods_[JM_LONGHELDLOCK].methodID, js_lockName, js_data);
...
DL_RetCode DistributedLock_JNI::listNodes(const char *lockName) {
  QRLogger::log(CAT_SQL_HBASE, LL_DEBUG, "DistributedLock_JNI::listNodes(lockName:%s)", lockName);
  if (initJNIEnv() != JOI_OK) return DL_ERROR_INIT_JNI_ENV;
  jstring js_lockName = jenv_->NewStringUTF(lockName);
  if (js_lockName == NULL) {
    GetCliGlobals()->setJniErrorStr(getErrorText(DL_ERROR_PARAM));
