    ...
    sanityCheckCycle_(hashJoinTdb.rangeSpecSanityCheckCycle_)
{
  ...
  if((nullLength > 0) || (hashJoinTdb.minMaxRowLength_ > 0)) {
    ...
    if (hashJoinTdb.minMaxRowLength_ > 0) {
      ...
      memset(workAtp_->getTupp(hashJoinTdb.minMaxValsAtpIndex_).getDataPointer(), 
             0,
             (Lng32) hashJoinTdb.minMaxRowLength_);
      ...
      if (! rightHashExpr_)
      ...
      else
        {
          ...
          if (minMaxExpr() && minMaxExpr()->perrecExpr()) {
            ...
            char buf[200];
            genFullMessage(buf, sizeof(buf), "HASHJ", 
                           hashJoinTdb().getExplainNodeId());
            ...
            if ( sanityCheckCycle_ > 0 &&
                 totalRowsEvaledByMinMax_ % sanityCheckCycle_ == 0 )
            {
               ExFunctionRangeOfValues::conditionalSanityCheck( 
  ...
  // If so, the left request was delayed until here.
    ...
    ex_expr::exp_return_type retCode =
            ExFunctionRangeOfValues::conditionalPack(
                   ...
                   workAtp_,
                   OperatorTypeEnum::ITM_RANGE_VALUES_INSERT, 
                   "HASHJ: Pack RS with inserted values (B)",
                   TRUE,
                   filterStats, ScanFilterStats::COPY);
    ...
    QRLogger::log(CAT_SQL_EXE_MINMAX,
                  ...
                  totalMinmaxEvalET_,
                  reportTimeDiff(totalMinmaxEvalET_),
                  totalRowsEvaledByMinMax_
                 );
#endif
          ...
          if (minMaxExpr_ &&  pstate.delayedLeftRequest() ) {
            ...
            ex_expr::exp_return_type retCode =
               ExFunctionRangeOfValues::conditionalPack(
                  ...
                  workAtp_,
                  OperatorTypeEnum::ITM_RANGE_VALUES_INSERT, 
                  "Pack RS with inserted values (A)",
                  TRUE, filterStats, ScanFilterStats::COPY);
