...
MdamRefList::~MdamRefList() {
  ...
  ex_assert(isEmpty(), "MdamRefList::~MdamRefList() called for a non-empty reference list.");
}
...
MdamRefList &MdamRefList::insert(const int disjunctNum, FixedSizeHeapManager &mdamRefListEntryHeap) {
  if (lastEntryPtr_ == 0)  // Empty list?
  {
    ...
    lastEntryPtr_ = new (mdamRefListEntryHeap) MdamRefListEntry(disjunctNum);
  } else {
    // Non-empty list.  Position beforePtr for insertion.
    MdamRefListEntry *beforePtr = lastEntryPtr_->positionBeforePtr(disjunctNum);
    // A duplicate disjunct number is discarded without error.
    if (disjunctNum == beforePtr->getDisjunctNum()) {
      return *this;
    };
    // Allocate a new node.
    MdamRefListEntry *newPtr = new (mdamRefListEntryHeap) MdamRefListEntry(disjunctNum, beforePtr);
...
void MdamRefList::intersect(const MdamRefList &refList0Ref, const MdamRefList &refList1Ref,
...
NABoolean MdamRefList::intersectEmpty(const MdamRefList &refList1, const MdamRefList &refList2) {
...
void MdamRefList::unionx(const MdamRefList &refList0Ref, const MdamRefList &refList1Ref,
                         FixedSizeHeapManager &mdamRefListEntryHeap) {
