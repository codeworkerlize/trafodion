     ...
     minMaxInputsReceived_(NULL)
{
  ...
  if ( needToWaitForMinMaxInputs() ) {
     minMaxInputsReceived_ = 
          new (glob->getSpace()) NABoolean[numOfParentInstances_];
  ...
  ULng32 minMaxTuppSpaceNeeded = 
         splitBottomTdb.minMaxRowLength_ + splitBottomTdb.minValStartOffset_;

  minMaxDataTupp_.init(minMaxTuppSpaceNeeded,
		       NULL,
                       new(glob->getSpace()) char[minMaxTuppSpaceNeeded]);

  workAtp_->getTupp(splitBottomTdb.minMaxValsWorkAtpIndex_) = &minMaxDataTupp_;
      
  // Initial min/max tuple to all null values.
  memset(workAtp_->getTupp(splitBottomTdb.minMaxValsWorkAtpIndex_).getDataPointer(), 
         0,
         (Lng32)minMaxTuppSpaceNeeded);
  ...
  if (minMaxExpr_) {
    (void) minMaxExpr_->fixup(0, expMode, this, 
                            glob->getSpace(), glob->getDefaultHeap(), FALSE, glob);
  ...
  if (minMaxMoveOutExpr_) {
    (void) minMaxMoveOutExpr_->fixup(0, expMode, this, 
                            glob->getSpace(), glob->getDefaultHeap(), FALSE, glob);
  }
...
*/
   ...
   if ( (desiredHostName &&
         strcmp(desiredHostName, myHostName) == 0) &&
      desiredInsNum == instNum && desiredNodeId == nodeId )
    {
...
void ex_split_bottom_tcb::releaseWorkRequest()
{
  ...
  if (glob_->getContext()->getTransaction()->getSavepointState() == SP_TO_COMMIT)
    {  
      Int64 transId = -1;
      if (NOT NAExecTrans(&transId))
        transId = -1;
      glob_->getContext()->getTransaction()->
        commitSavepoint(
...
const char* ex_split_bottom_tcb::getWorkState(SplitBottomWorkState state)
{
  switch (state) {
    ...
    case WAIT_FOR_MINMAX_INPUT: return "WAIT_FOR_MINMAX_INPUT";
    case WORK_ON_REQUEST: return "WORK_ON_REQUEST";
    case WAIT_TO_REPLY: return "WAIT_TO_REPLY";
    case WAIT_FOR_MORE_REQUESTORS: return "WAIT_FOR_MORE_REQUESTORS";
    case WAIT_FOR_PARTITION_INPUT_DATA: return "WAIT_FOR_PARTITION_INPUT_DATA";
    case WAIT_FOR_LATE_CANCELS: return "WAIT_FOR_PARTITION_INPUT_DATA";
    case LIMITS_EXCEEDED_ERROR: return "LIMITS_EXCEEDED_ERROR";
...
ExWorkProcRetcode ex_split_bottom_tcb::workOnWaitMinMaxInputs()
{
   ...
   QRLogger::log(CAT_SQL_EXE_MINMAX, LL_DEBUG, 
                 ...
                 splitBottomTdb().getExplainNodeId(),
                 numSendNodes,
         ...
         if (NOT squeueDown->isEmpty())
         {
           ...
           if (sentry->downState.request == ex_queue::GET_ALL) 
           {
              ...
              if (minMaxExpr_)
              {
                 ...
                 char buf[200];
                 genFullMessage(buf, sizeof(buf), "split_bottom", 
                                splitBottomTdb().getExplainNodeId());
                 ...
                 retCode = minMaxExpr()->perrecExpr()->eval(
                                   sentry->getAtp(), workAtp_,
                                   0,
                                   -1,
                                   NULL,
                                   0,
...
#ifdef DEBUG_SCAN_FILTER
                  ExFunctionRangeOfValues::conditionalDisplay(
                           minMaxExpr()->perrecExpr()->getClauses(),
   ...
   if ( numOfMinMaxInputsReceived_ == numSendNodes ) {
      ...
      ScanFilterStatsList* filterStats = (fragStats)  ?  
                 fragStats->getScanFilterStats() : NULL;
      ...
      retCode = 
         ExFunctionRangeOfValues::conditionalPack( 
                              minMaxExpr()->perrecExpr()->getClauses(),
                              workAtp_,
                              OperatorTypeEnum::ITM_RANGE_VALUES_MERGE,
...
ExWorkProcRetcode ex_split_bottom_tcb::work()
{
  ...
  if (newMessage_ && newMessage_->hasRequest())
    newMessage_->workOnReceivedMessage();
            ...
            if ( minMaxMoveOutExpr_ ) 
              {
               ...
               ex_expr::exp_return_type retCode = 
                    minMaxMoveOutExpr_->eval(
                                             workAtp_, centry->getAtp(),
                                             0,
                                             -1,
                                             NULL,
                                             0,
  ...
  if (workState_ != WORK_ON_REQUEST) {
     ...
     snprintf(buf, sizeof(buf), 
              "Internal err, splb work state %s (should be WORK_ON_REQUEST)",
  ...
  while (NOT qChild_.up->isEmpty())  // may exit via return
      ...
      else
	{
      ...
      /*      QRLogger::log(CAT_SQL_EX_SPLIT_BOTTOM,
                 LL_DEBUG,
                 "ex_split_bottom_tcb::workUp(): partFunc=%p, partnum=%d, broadcast the row=%d, combinedRequests=%d, isMWayRepart=%d, isSB=%d\n",  
                 partFunction(),
                 partNumInfo_.calculatedPartNum_,
                 broadcastThisRow_,
      ...
      while (moreObjects())
	{
	  switch (getNextObjType())
		...
		*this >> receivedRequest;
		...
		GetCliGlobals()->currContext()->setClientInfo(receivedRequest.clientInfo_, strlen(receivedRequest.clientInfo_), NULL);
		...
		*this >> receivedRequest;
		...
		// request
		splitBottom_->glob_->setReplyTag(transid.getTransIdAsInt(),
                                                 transid.getSavepointId(),
                                                 transid.getPSavepointId(),
                                                 getReplyTag());
                
                if ((transid.getSavepointId() > 0) &&
                    (splitBottom_->glob_->getContext()) &&
                    (splitBottom_->glob_->getContext()->getTransaction()))
                  {
                    // set parent savepoint(user savepoint) first
                    splitBottom_->glob_->getContext()->getTransaction()->
                      setImplicitPSavepointId(transid.getPSavepointId());

                    // set inner implicit savepoint
                    splitBottom_->glob_->getContext()->getTransaction()->
                      setImplicitSavepointId(transid.getSavepointId());
                    splitBottom_->glob_->getContext()->getTransaction()->
                      setImplicitSavepointState(SP_GENERATED);
                  }
                else
                  {
                     splitBottom_->glob_->getContext()->getTransaction()->
		...
		*this >> receivedRequest;
		...
		// Reply to this message later.
                ...
                if ((receivedRequest.getSavepointId() > 0) &&
                    ((receivedRequest.savepointCommit()) ||
                     (receivedRequest.savepointRollback())))
                  {
                    splitBottom_->glob_->getContext()->getTransaction()->
                      setImplicitSavepointId(receivedRequest.getSavepointId());

                    if (receivedRequest.savepointCommit())
                      splitBottom_->glob_->getContext()->getTransaction()->
                        setImplicitSavepointState(SP_TO_COMMIT);
                    else
                      splitBottom_->glob_->getContext()->getTransaction()->
                        setImplicitSavepointState(SP_TO_ROLLBACK);
                  }
	      }
	      break;
...
SplitBottomSavedMessage::SplitBottomSavedMessage(
     IpcEnvironment *env,
     ExEspInstanceThread *threadInfo) :
     IpcMessageStream(env,
		      IPC_MSG_SQLESP_CONTROL_REPLY,
...
NABoolean ex_split_bottom_tcb::needToWaitForMinMaxInputs()
{
   return (minMaxExpr_ != NULL);
}
