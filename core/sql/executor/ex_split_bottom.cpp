...
ex_split_bottom_tcb *ex_split_bottom_tdb::buildESPTcbTree(ExExeStmtGlobals *glob, ExEspFragInstanceDir *espInstanceDir,
  ...
  // create my own tcb
  result = new (glob->getSpace())
      ex_split_bottom_tcb(*this, tcbChild, glob, espInstanceDir, myKey, parentKey, myHandle, numOfParentInstances);
  ...
  if (getCollectStats()) {
...
#ifdef _DEBUG
    if (getenv("DISABLE_STATS")) glob->setStatsEnabled(FALSE);
#endif
    ...
    if (statsGlobals == NULL || getCollectStatsType() == ALL_STATS ||
      ...
      statsArea = new (espInstanceDir->getLocalStatsHeap())
          ExStatisticsArea(espInstanceDir->getLocalStatsHeap(), numOfParentInstances, getCollectStatsType());
      statsArea->setStatsEnabled(glob->statsEnabled());
      glob->setStatsArea(statsArea);
      result->allocateStatsEntry();
    } else {
      int error = statsGlobals->getStatsSemaphore(espInstanceDir->getSemId(), espInstanceDir->getPid());
      statsArea = new (espInstanceDir->getStatsHeap())
          ExStatisticsArea(espInstanceDir->getStatsHeap(), numOfParentInstances, getCollectStatsType());
      statsArea->setStatsEnabled(glob->statsEnabled());
      StmtStats *ss = glob->castToExEspStmtGlobals()->setStmtStats();
      if (ss != NULL) ss->setStatsArea(statsArea);
...
ex_split_bottom_tcb::ex_split_bottom_tcb(const ex_split_bottom_tdb &splitBottomTdb, const ex_tcb *child_tcb,
    ...
    : ex_tcb(splitBottomTdb, 1, glob),
      ...
      numOfMinMaxInputsReceived_(0),
      currentMinMaxCheckIndex_(0),
      minMaxInputsReceived_(NULL) {
  // build the child tcb
  tcbChild_ = child_tcb;
  ...
  minMaxExpr_ = splitBottomTdb.minMaxExpr_;

  minMaxMoveOutExpr_ = splitBottomTdb.minMaxMoveOutExpr_;

  ex_assert(glob_, "Using split bottom node outside of an ESP");

  ExEspInstanceThread *threadInfo = glob_->getThreadInfo();
  ...
  if (splitBottomTdb.isMWayRepartition()) {
    numOfParentInstances_ =
        numOfTargetESPs(splitBottomTdb.topNumESPs_, splitBottomTdb.bottomNumESPs_, glob_->getMyInstanceNumber());
    firstParentNum_ =
        myFirstTargetESP(splitBottomTdb.topNumESPs_, splitBottomTdb.bottomNumESPs_, glob_->getMyInstanceNumber());
  } else {
    firstParentNum_ = 0;
  }
  ...
  if (needToWaitForMinMaxInputs()) {
    minMaxInputsReceived_ = new (glob->getSpace()) NABoolean[numOfParentInstances_];
  ...
  partNumTupp_.init(sizeof(partNumInfo_), NULL, (char *)(&partNumInfo_));
  workAtp_->getTupp(splitBottomTdb.partNoATPIndex_) = &partNumTupp_;
  convErrTupp_.init(sizeof(conversionErrorFlg_), NULL, (char *)(&conversionErrorFlg_));
  workAtp_->getTupp(splitBottomTdb.convErrorATPIndex_) = &convErrTupp_;
  partInputDataTupp_.init((unsigned short)splitBottomTdb.partInputDataLen_, NULL,
  ...
  if (partFunction()) {
    unsigned short expMode = getExpressionMode();
    if (splitBottomTdb.partFuncUsesNarrow_) expMode = 0;  // don't use PCODE with Narrow
    partFunction()->fixup(0, expMode, this, glob->getSpace(), glob->getDefaultHeap(), FALSE, glob);
  }

  if (minMaxExpr_) {
    (void)minMaxExpr_->fixup(0, expMode, this, glob->getSpace(), glob->getDefaultHeap(), FALSE, glob);

    // initialize the min/max tupp in workAtp
    minMaxExpr()->initializeAggr(workAtp_);
  }

  if (minMaxMoveOutExpr_) {
    (void)minMaxMoveOutExpr_->fixup(0, expMode, this, glob->getSpace(), glob->getDefaultHeap(), FALSE, glob);
  }

  // allocate message stream to receive partition input data
  newMessage_ = new (glob->getSpace()) SplitBottomRequestMessage(glob->getIpcEnvironment(), this, threadInfo);
  savedDataMessage_ = new (glob->getSpace()) SplitBottomSavedMessage(glob->getIpcEnvironment(), threadInfo);
  savedWorkMessage_ = new (glob->getSpace()) SplitBottomSavedMessage(glob->getIpcEnvironment(), threadInfo);
  savedReleaseMessage_ = new (glob->getSpace()) SplitBottomSavedMessage(glob->getIpcEnvironment(), threadInfo);
  ...
  if (splitBottomTdb.partInputDataLen_) {
    ...
    if (needToWaitForMinMaxInputs()) {
      setWorkState(WAIT_FOR_MINMAX_INPUT);
      numOfMinMaxInputsReceived_ = 0;
    } else {
...
void ex_split_bottom_tcb::enableHook() {
  char myHostName[128];
  ...
  cout << ", my frag Id=" << fragId;
  cout << ", my Node id=" << nodeId;
  cout << ", my host name=" << myHostName;
  cout << endl;
  */
  ...
  char *p = getenv("host_name");

  if (!p) return;

  char desiredHostName[128];

  memmove(desiredHostName, p, strlen(p));
  desiredHostName[strlen(p)] = 0;

  /*
  cout << "split bottom:";
  cout << ", my instance number=" << instNum;
  cout << ", my Node id=" << nodeId;
  cout << ", my host name=" << myHostName << endl;
  cout << ", desired inst num=" << desiredInsNum;
  cout << ", node id =" << desiredNodeId;
  cout << ", host name=" << desiredHostName;
  cout << endl;
  */

  if ((desiredHostName && strcmp(desiredHostName, myHostName) == 0) && desiredInsNum == instNum &&
      desiredNodeId == nodeId) {
    cout << "SplitBottom: calling msg_debug_hook() for " << endl;
    cout << "instance number=" << instNum;
    // cout << ", my frag Id=" << fragId;
    cout << ", query node id=" << nodeId;
    cout << ", host name=" << myHostName;
    cout << "" << endl;

    msg_debug_hook("tdm_arkesp", "esp.hook");
  }
}

ex_split_bottom_tcb::~ex_split_bottom_tcb() { freeResources(); }

void ex_split_bottom_tcb::freeResources() {
  // we allocated ATPs for the down queue for the child, which is not
  // the normal way
  if (qChild_.down) qChild_.down->deallocateAtps();
  ...
  if (needToWaitForMinMaxInputs()) {
    NADELETEBASIC(minMaxInputsReceived_, getSpace());
    minMaxInputsReceived_ = NULL;
  }
}
...
void ex_split_bottom_tcb::registerSubtasks() {
  ...
  // finally, make a separate event through which IPC messages
  // with partition input values, transactions, or any other
  // messages in a multi-threaded IPC environment can wake us up
  ioHandler_ = sched->registerNonQueueSubtask(sWork, this);

  // enableHook();
}

NABoolean ex_split_bottom_tcb::needStatsEntry() { return TRUE; }

ExOperStats *ex_split_bottom_tcb::doAllocateStatsEntry(CollHeap *heap, ComTdb *tdb) {
  ...
  if ((statsType == ComTdb::ALL_STATS) || (statsType == ComTdb::OPERATOR_STATS) ||
    ...
    stat = new (getGlobals()->getStatsArea()->getHeap())
        ExFragRootOperStats(getGlobals()->getStatsArea()->getHeap(), this, tdb);
    ...
    // TDB or TCB.
    stat =
        new (getGlobals()->getStatsArea()->getHeap()) ExMeasStats(getGlobals()->getStatsArea()->getHeap(), NULL, tdb);
    stat->setStatsInEsp(TRUE);
    getGlobals()->getStatsArea()->setStatsInEsp(TRUE);
    StmtStats *ss = getGlobals()->castToExExeStmtGlobals()->castToExEspStmtGlobals()->getStmtStats();
...
ExWorkProcRetcode ex_split_bottom_tcb::processInactiveSendTopsBelow(NABoolean &foundOne) {
...
void ex_split_bottom_tcb::releaseWorkRequest() {
  ...
  if (workMessageSaved_) {
    ...
    glob_->setReplyTag(ExInvalidInt64Transid, ExInvalidInt64Transid, ExInvalidInt64Transid, GuaInvalidReplyTag);
...
const char *ex_split_bottom_tcb::getWorkState(SplitBottomWorkState state) {
  switch (state) {
    case IDLE:
      return "IDLE";
    case WAIT_FOR_MINMAX_INPUT:
      return "WAIT_FOR_MINMAX_INPUT";
    case WORK_ON_REQUEST:
      return "WORK_ON_REQUEST";
    case WAIT_TO_REPLY:
      return "WAIT_TO_REPLY";
    case WAIT_FOR_MORE_REQUESTORS:
      return "WAIT_FOR_MORE_REQUESTORS";
    case WAIT_FOR_PARTITION_INPUT_DATA:
      return "WAIT_FOR_PARTITION_INPUT_DATA";
    case WAIT_FOR_LATE_CANCELS:
      return "WAIT_FOR_PARTITION_INPUT_DATA";
    case LIMITS_EXCEEDED_ERROR:
      return "LIMITS_EXCEEDED_ERROR";
    case CLEANUP:
      return "CLEANUP";
    default:
...
// From state WAIT_FOR_MINMAX_INPUT, we can only go to
// WAIT_FOR_MINMAX_INPUT or IDLE as follows.
//   WAIT_FOR_MINMAX_INPUT: not all send bottom nodes have sent
//                          us their min/max values
//                    IDLE: All send bottom nodes have sent us
//                          their min/max values. Proceed to
//                          regular processing mode.
ExWorkProcRetcode ex_split_bottom_tcb::workOnWaitMinMaxInputs() {
  CollIndex numSendNodes = sendNodes_.entries();

  QRLogger::log(
      CAT_SQL_EXE_MINMAX, LL_DEBUG,
      "In workOnWaitMinMaxInputs() for nodeId=%d, wait for a total of %d min/max inputs, currently %d received",
      splitBottomTdb().getExplainNodeId(), numSendNodes, numOfMinMaxInputsReceived_);

  ex_expr::exp_return_type retCode = ex_expr::EXPR_OK;

  // Make one pass through all send nodes and check their
  // min/max availability.
  for (CollIndex i = 0; i < numSendNodes; i++) {
    ex_queue *squeueDown = sendNodesDownQ_[currentMinMaxCheckIndex_];

    if (!minMaxInputsReceived_[currentMinMaxCheckIndex_]) {
      if (NOT squeueDown->isEmpty()) {
        ex_queue_entry *sentry = squeueDown->getHeadEntry();

        // check if it is canceled. If so, mark the data received.
        if (sentry->downState.request == ex_queue::GET_NOMORE) {
          minMaxInputsReceived_[currentMinMaxCheckIndex_] = TRUE;
          numOfMinMaxInputsReceived_++;
        }

        // GET_ALL indicates the arrival of min/max. Record the fact.
        if (sentry->downState.request == ex_queue::GET_ALL) {
          ...
          if (minMaxExpr_) {
            // The following tracing code does not work because the tuple
            // desc for TUPP #2 is not set in GenRelEnforcer.cpp, in the section
            // for min/max. Only the descriptor of the result in workAtp's desc
            // is saved in SplitBottomTdb.

#ifdef DEBUG_SCAN_FILTER
            char buf[200];
            genFullMessage(buf, sizeof(buf), "split_bottom", splitBottomTdb().getExplainNodeId());

            sentry->getAtp()->dumpMinMaxTupp(buf, -1);
#endif
            // the source data is the 2nd TUPP in sentry, and
            // the result is stored in the 5th TUPP in workATP.
            retCode = minMaxExpr()->perrecExpr()->eval(sentry->getAtp(), workAtp_, 0, -1, NULL, 0, NULL);
...
#ifdef DEBUG_SCAN_FILTER
            ExFunctionRangeOfValues::conditionalDisplay(minMaxExpr()->perrecExpr()->getClauses(),
                                                        OperatorTypeEnum::ITM_RANGE_VALUES_MERGE, buf);
#endif
          }
    ...
    // wrap around the idx if it is at the last valid element
    if (currentMinMaxCheckIndex_ == numSendNodes - 1)
      currentMinMaxCheckIndex_ = 0;
    else
      currentMinMaxCheckIndex_++;
  }  // end of for

  // All inputs are available, pack the final bitmap and
  // transition to IDLE state.
  if (numOfMinMaxInputsReceived_ == numSendNodes) {
#ifdef DEBUG_SCAN_FILTER
    char buf[200];
    genFullMessage(buf, sizeof(buf), "split_bottom", splitBottomTdb().getExplainNodeId());
    ...
    ExFragRootOperStats *fragStats = statsEntry ? statsEntry->castToExFragRootOperStats() : NULL;

    ScanFilterStatsList *filterStats = (fragStats) ? fragStats->getScanFilterStats() : NULL;

    // pack all range values objects in the per record expr into
    // the target VARCHAR arrray.
    retCode = ExFunctionRangeOfValues::conditionalPack(minMaxExpr()->perrecExpr()->getClauses(), workAtp_,
                                                       OperatorTypeEnum::ITM_RANGE_VALUES_MERGE,
                                                       "SPLIT_BOTTOM: Pack RS with merged data",
                                                       TRUE,  // clearWhenDone
                                                       filterStats, ScanFilterStats::ADD);

    if (retCode == ex_expr::EXPR_ERROR)
      setWorkState(CLEANUP);
    else
      setWorkState(IDLE);  // transition to IDLE state
  }
...
ExWorkProcRetcode ex_split_bottom_tcb::work() {
  ...
  if (newMessage_ && newMessage_->hasRequest()) newMessage_->workOnReceivedMessage();
  ...
  // -----------------------------------------------------------------
  if (workState_ == WAIT_FOR_MINMAX_INPUT) {
    workOnWaitMinMaxInputs();

    // Not all min/max inputs are ready yet.
    // Bail out and wait for another chance
    // to enter this work() method.
    if (workState_ == WAIT_FOR_MINMAX_INPUT) return WORK_OK;
  }
  ...
  if (workState_ == IDLE) {
    ...
    while (workState_ == IDLE AND i < numSendNodes) {
      ...
      if (splitBottomTdb().getExtractProducerFlag() && i == (numSendNodes - 1)) isExtractConsumer = TRUE;
      ...
      if (NOT squeueDown->isEmpty()) {
        ...
        if (isExtractConsumer && numCanceledRequests > 0) {
          ...
          // node.
          if (sentry->downState.request != ex_queue::GET_NOMORE) squeueDown->cancelRequest();
        }
        ...
        if (sentry->downState.request == ex_queue::GET_NOMORE) {
          ...
          if (!splitBottomTdb().combineRequests_ || numCanceledRequests >= numSendNodes)
          ...
          bool okToPush = true;
          if (tcbChild_->getParentQueue().down->isFull() || isExtractConsumer || !hasTransaction()) okToPush = false;
          ...
          if (okToPush) {
            ...
            if (minMaxMoveOutExpr_) {
              // Move the min of min and the max of max from workAtp_ to
              // the down queue entry. Please refer to the plan to find
              // out the index of the TUPP in workAtp_ and down Q entry.
              ex_expr::exp_return_type retCode =
                  minMaxMoveOutExpr_->eval(workAtp_, centry->getAtp(), 0, -1, NULL, 0, NULL);
    ...
    // still idle?
    if (workState_ == IDLE) return WORK_OK;
    ...
    if (splitBottomTdb().getAbendType() != ComTdbSplitBottom::NO_ABEND && glob_->getMyFragId() == 2 &&
      ...
      switch (splitBottomTdb().getAbendType()) {
        case ComTdbSplitBottom::SIGNED_OVERFLOW: {
          ...
          while (loopCnt--) willOverflow++;
          break;
        }
        case ComTdbSplitBottom::ASSERT: {
          ex_assert(splitBottomTdb().getAbendType() != ComTdbSplitBottom::ASSERT, "Abend/assertTest passed.");
          break;
        }
          ...
          for (CollIndex i = 0; i < sendNodes_.entries(); i++) {
            ...
            if (!squeueDown->isEmpty()) {
              ...
              if (!squeueUp->isFull()) {
                ...
                upEntry->upState.parentIndex = squeueDown->getHeadEntry()->downState.parentIndex;
  ...
  if (workState_ != WORK_ON_REQUEST) {
    ...
    snprintf(buf, sizeof(buf), "Internal err, splb work state %s (should be WORK_ON_REQUEST)",
             getWorkState(workState_));
...
ExWorkProcRetcode ex_split_bottom_tcb::workUp() {
  ...
  if (splitBottomTdb().getExtractProducerFlag() && numCanceledPartitions_ > 0) {
    ...
    for (CollIndex i = 0; i < numSendNodes; i++) {
      ex_queue *squeueDown = sendNodesDownQ_[i];
      if (!squeueDown->isEmpty()) {
        if (squeueDown->getHeadEntry()->downState.request != ex_queue::GET_NOMORE) squeueDown->cancelRequest();
      }
    }
  }
  ...
  while (NOT qChild_.up->isEmpty())  // may exit via return
  {
    ...
    NABoolean isErrorEntry = (centry->getDiagsArea() AND centry->getDiagsArea()->mainSQLCODE() < 0);
    ...
    if (partFunction() AND NOT isEofEntry) {
      ...
      if (!calcPartNumIsValid_) {
        ...
        if (partFunction()->eval(centry->getAtp(), workAtp_) == ex_expr::EXPR_ERROR) {
        ...
        if (partFuncUsesNarrow() && conversionErrorFlg_ != ex_conv_clause::CONV_RESULT_OK) {
        ...
        if (splitBottomTdb().useSkewBuster()) {
          ex_assert(splitBottomTdb().combineRequests_, "Skew buster used without combining requests.");
          ...
          if (skewedValueFound) {
            if (splitBottomTdb().doBroadcastSkew())
              ...
              if (uniformDistributionType_ == RoundRobin_) {
                ...
                const UInt32 initialRoundRobin = splitBottomTdb().getInitialRoundRobin();
                const UInt32 finalRoundRobin = splitBottomTdb().getFinalRoundRobin();

                if (initialRoundRobin <= finalRoundRobin) {
                  if (roundRobinRecipient_ > finalRoundRobin) roundRobinRecipient_ = initialRoundRobin;
                } else {
                  if (roundRobinRecipient_ >= sendNodes_.entries())
                    roundRobinRecipient_ = 0;
                  else if (roundRobinRecipient_ == finalRoundRobin + 1)
                    roundRobinRecipient_ = initialRoundRobin;
                }
              } else
                partNumInfo_.calculatedPartNum_ = localSendBotttom_;
            }
          }

          if (splitBottomTdb().doBroadcastSkew() && broadcastOneRow_) {
            broadcastThisRow_ = TRUE;
      ...
      if (isEofEntry AND NOT staticInputData_) {
        ...
        ex_assert(savedDataMessage_ AND savedDataMessage_->getState() == IpcMessageStream::RECEIVED,
    ...
    /*      QRLogger::log(CAT_SQL_EX_SPLIT_BOTTOM,
               LL_DEBUG,
               "ex_split_bottom_tcb::workUp(): partFunc=%p, partnum=%d, broadcast the row=%d, combinedRequests=%d,
       isMWayRepart=%d, isSB=%d\n", partFunction(), partNumInfo_.calculatedPartNum_, broadcastThisRow_,
               splitBottomTdb().combineRequests_,
               splitBottomTdb().isMWayRepartition(),
               splitBottomTdb().useSkewBuster()
                );
    */
    ...
    if (splitBottomTdb().combineRequests_) {
      if (broadcastThisRow_) {
        if (splitBottomTdb().isMWayRepartition()) {
          ...
          ex_assert(isEofEntry || splitBottomTdb().isAnESPAccess(), "M-Way repartition broadcasts non EOD entry!");
        ...
        ex_assert(calcPartNumIsValid_, "Negative or no partition number calculated");
    ...
    for (CollIndex i = partNumLow; i < partNumHigh; i++) {
      ...
      if (squeueUp->isFull() && (isEofEntry || squeueDown->getHeadEntry()->downState.request != ex_queue::GET_NOMORE)) {
    ...
    /*      QRLogger::log(CAT_SQL_EX_SPLIT_BOTTOM,
       LL_DEBUG,
       "ex_split_bottom_tcb::workUp(): actual pnum: low=%d, high=%d, firstParentNum_=%d\n",
       partNumLow, partNumHigh, firstParentNum_);
    */
    ...
    for (CollIndex p = partNumLow; p < partNumHigh; p++) {
      ...
      if (isEofEntry || (squeueDown->getHeadEntry()->downState.request != ex_queue::GET_NOMORE)) {
        ex_queue_entry *sentry = squeueUp->getTailEntry();

        sentry->upState = centry->upState;

        if (isErrorEntry AND sentry->upState.status == ex_queue::Q_OK_MMORE)
          sentry->upState.status = ex_queue::Q_SQLERROR;

        if (splitBottomTdb().getExtractProducerFlag()) {
          if (sentry->upState.status == ex_queue::Q_OK_MMORE && p != (partNumHigh - 1)) {
        ...
        sentry->upState.parentIndex = squeueDown->getHeadEntry()->downState.parentIndex;
        ...
        if (isEofEntry) {
          if (centry->getAtp()->getDiagsArea()) {
            ...
            if (!splitBottomTdb().getExtractProducerFlag()) centry->getAtp()->setDiagsArea(NULL);
          }
        } else if (statsEntry) {
    ...
    if (isEofEntry) {
      ...
      if (retcode != WORK_OK) {
        ex_assert(retcode == WORK_BAD_ERROR, "Unexpected return from processInactiveSendTopsBelow");
        return retcode;
      }
      if (foundOne)
...
ExWorkProcRetcode ex_split_bottom_tcb::processCancel() {
  ...
  if (workState_ == IDLE || workState_ == WAIT_FOR_PARTITION_INPUT_DATA) return WORK_OK;
  ...
  for (CollIndex i = sendNodeLo; i < sendNodeHi; i++) {
    ...
    if (!squeueDown->isEmpty()) {
      ex_queue_entry *sentry = squeueDown->getHeadEntry();
      if (sentry->downState.request == ex_queue::GET_NOMORE) numCanceledPartitions_++;
    }  // if isEmpty ... else ...
  }    // for all sendNodes.
...
ExWorkProcRetcode ex_split_bottom_tcb::workCancelBeforeSent() {
  ...
  for (CollIndex p = partNumLow; p < partNumHigh; p++) {
    ...
    ex_assert(!squeueDown->isEmpty(), "Should have a GET_NOMORE request");
    ex_assert(!squeueUp->isFull(), "Up queue should be empty at this point");
    if (useFix0318) bugCatcher3041_ = squeueDown->getHeadIndex();
...
void ex_split_bottom_tcb::replyToMessage(IpcMessageStream *sm) {
  ...
  ex_assert(sm->getState() == IpcMessageStream::EXTRACTING OR sm->getState() == IpcMessageStream::RECEIVED,
...
void ex_split_bottom_tcb::acceptNewPartInputValues(NABoolean moreWork) {
  ...
  ex_assert(NOT staticInputData_ OR moreWork, "Static input data assignment must have part input values");

  if (workState_ != WAIT_FOR_PARTITION_INPUT_DATA) return;

  if (NOT qChild_.up->isEmpty() AND qChild_.up->getHeadEntry()->upState.status == ex_queue::Q_NO_DATA) {
...
void ex_split_bottom_tcb::testAllQueues() {
  ...
  CollIndex numSendNodes = sendNodes_.entries();
  for (CollIndex i = 0; i < numSendNodes; i++) {
    if (NOT sendNodesUpQ_[i]->isEmpty()) ex_assert(0, "Orphan entries in split bottom's up queue!");

    if (NOT sendNodesDownQ_[i]->isEmpty()) ex_assert(0, "Orphan entries in split bottom's down queue!");
  }
}
...
void ex_split_bottom_tcb::setLocalSendBottom(CollIndex s) {
  if (splitBottomTdb().useSkewBuster() && splitBottomTdb().doBroadcastSkew() == FALSE &&
    ...
    if (localSendBotttom_ != NULL_COLL_INDEX) {
      ...
      uniformDistributionType_ = RoundRobin_;
      localSendBotttom_ = NULL_COLL_INDEX;
    } else if (splitBottomTdb().bottomNumESPs_ >= splitBottomTdb().topNumESPs_) {
...
void ex_split_bottom_tcb::cpuLimitExceeded() {
  if ((workState_ == WORK_ON_REQUEST) || (workState_ == WAIT_FOR_MORE_REQUESTORS) || (workState_ == WAIT_TO_REPLY)) {
...
void ex_split_bottom_tcb::setWorkState(SplitBottomWorkState s) { workState_ = s; }
...
#define ex_assert_both_sides(assert_test, assert_msg)                                                                  \
  if (!(assert_test)) {                                                                                                \
    if (environment_ && environment_->getControlConnection() &&                                                        \
        environment_->getControlConnection()->castToGuaReceiveControlConnection() &&                                   \
        environment_->getControlConnection()->castToGuaReceiveControlConnection()->getConnection())                    \
...
SplitBottomRequestMessage::SplitBottomRequestMessage(IpcEnvironment *env, ex_split_bottom_tcb *splitBottom,
                                                     ExEspInstanceThread *threadInfo)
    : IpcMessageStream(env, IPC_MSG_SQLESP_CONTROL_REPLY, CurrEspReplyMessageVersion, 0, TRUE, threadInfo) {
...
void SplitBottomRequestMessage::actOnReceive(IpcConnection *connection) {
  if (getState() == ERROR_STATE) {
    ...
    // In the single-threaded model, we do quite a bit of work
    // in the callback. In the multi-threaded IPC model, we move
    // this work into the thread that owns the message stream.
    // TODO: Switch to this new model in both environments.
    if (!getThreadInfo()) workOnReceivedMessage();

    splitBottom_->tickleScheduler();
  }
}

void SplitBottomRequestMessage::workOnReceivedMessage() {
  while (hasRequest_) {
      ...
      switch (getNextObjType()) {
        case ESP_PARTITION_INPUT_DATA_HDR: {
          ...
          *this >> receivedRequest;
          // QRINFO("receivedRequest.clientInfo_: %s", receivedRequest.clientInfo_);
          GetCliGlobals()->currContext()->setClientInfo(receivedRequest.clientInfo_,
                                                        strlen(receivedRequest.clientInfo_), NULL);
          splitBottom_->staticInputData_ = (NABoolean)receivedRequest.staticAssignment_;
          ...
          if (moreObjects() AND getNextObjType() == ESP_INPUT_SQL_BUFFER) {
            ...
            *this >> recBuffer;
            ...
            ex_assert_both_sides(inputTuple.getDataPointer(), "Empty sql_buffer received for part input data");

            if (splitBottom_->splitBottomTdb().getPartInputDataLen() > 0) {
              ex_assert_both_sides(splitBottom_->workState_ == ex_split_bottom_tcb::WAIT_FOR_PARTITION_INPUT_DATA,
                                   "Got part input data although not waiting for it");
              str_cpy_all(splitBottom_->partInputDataTupp_.getTupleAddress(), inputTuple.getDataPointer(),
          ...
          splitBottom_->acceptNewPartInputValues(
              receivedRequest.staticAssignment_ OR receivedRequest.askForMoreWorkWhenDone_);
          ...
          if (receivedRequest.askForMoreWorkWhenDone_) {
            ...
            ex_assert_both_sides(NOT splitBottom_->inputDataMessageSaved_,
                                 "Received multiple part input data requests");
            ex_assert_both_sides(NOT receivedRequest.staticAssignment_,
        ...
        case ESP_WORK_TRANSACTION_HDR: {
          splitBottom_->getGlobals()->castToExExeStmtGlobals()->castToExEspStmtGlobals()->setNoNewRequest(FALSE);
          ...
          *this >> receivedRequest;
          ...
          // request
          splitBottom_->glob_->setReplyTag(transid.getTransIdAsInt(), transid.getSavepointId(),
                                           transid.getPSavepointId(), getReplyTag());

          if ((transid.getSavepointId() > 0) && (splitBottom_->glob_->getContext()) &&
              (splitBottom_->glob_->getContext()->getTransaction())) {
            // set parent savepoint(user savepoint) first
            splitBottom_->glob_->getContext()->getTransaction()->setImplicitPSavepointId(transid.getPSavepointId());

            // set inner implicit savepoint
            splitBottom_->glob_->getContext()->getTransaction()->setImplicitSavepointId(transid.getSavepointId());
            splitBottom_->glob_->getContext()->getTransaction()->setImplicitSavepointState(SP_GENERATED);
          } else {
            splitBottom_->glob_->getContext()->getTransaction()->clearSavepoint();
            splitBottom_->glob_->getContext()->getTransaction()->setImplicitSavepointState(SP_SPNONE);
          }

          // don't reply now, save the message for a later reply
          ex_assert_both_sides(NOT splitBottom_->workMessageSaved_, "Received multiple transaction/work requests");
          saveWorkMsg = TRUE;
        } break;

        case ESP_RELEASE_TRANSACTION_HDR: {
          ExExeStmtGlobals *stmtGlobals = splitBottom_->getGlobals()->castToExExeStmtGlobals();
          ...
          *this >> receivedRequest;
          ...
          // if this request has a savepoint commit/abort, call dtm
          // with that info.
          if ((receivedRequest.getSavepointId() > 0) &&
              ((receivedRequest.savepointCommit()) || (receivedRequest.savepointRollback()))) {
            splitBottom_->glob_->getContext()->getTransaction()->setImplicitSavepointId(
                receivedRequest.getSavepointId());

            if (receivedRequest.savepointCommit())
              splitBottom_->glob_->getContext()->getTransaction()->setImplicitSavepointState(SP_TO_COMMIT);
            else
              splitBottom_->glob_->getContext()->getTransaction()->setImplicitSavepointState(SP_TO_ROLLBACK);
          }
        } break;

        default:
          ex_assert_both_sides(0, "Received invalid request for split bottom node");
      }  // switch on next object type
    }    // while more objects

    if (saveWorkMsg) {
      ex_assert_both_sides(NOT saveDataMsg, "Trans/work requests can't come with load balancing msg");
      ...
      // replies. Method releaseWorkRequest() will reply when
      // the instance has no more outstanding send top / send
      // bottom messages. In multi-threaded IPC, the call below
      // will wake up the master thread.
      splitBottom_->espInstanceDir_->hasReleaseRequest(splitBottom_->myHandle_);
    } else {
...
SplitBottomSavedMessage::SplitBottomSavedMessage(IpcEnvironment *env, ExEspInstanceThread *threadInfo)
    : IpcMessageStream(env, IPC_MSG_SQLESP_CONTROL_REPLY, CurrEspReplyMessageVersion, 0, TRUE, threadInfo) {}

void SplitBottomSavedMessage::actOnSend(IpcConnection *) {
  ex_assert(getState() != ERROR_STATE, "Error during reply to a saved split bottom message");
}

void SplitBottomSavedMessage::actOnReceive(IpcConnection *) {
  ex_assert(getState() != ERROR_STATE, "Error while saving a split bottom message");
}
...
NABoolean ex_split_bottom_tcb::needToWaitForMinMaxInputs() { return (minMaxExpr_ != NULL); }
