...
short ExExeUtilLongRunningTcb::work()
{
  ...
  if (lrTdb().getDefaultCatalogName() && lrTdb().getDefaultSchemaName())
    {
      Int32 bufSize = 100 + strlen(lrTdb().getDefaultCatalogName()) +
                      strlen(lrTdb().getDefaultSchemaName());
...
short ExExeUtilLongRunningTcb::processInitial(Lng32 &rc)
{
  ...
  rc = cliInterface()->executeImmediate(lruStmtAndPartInfo_,
                                        0,
                                        0,
                                        0,
                                        &rowsAffected);
...
ex_tcb * ExExeUtilCompositeUnnestTdb::build(ex_globals * glob)
{
  ...
  exe_util_tcb = 
    new(glob->getSpace()) ExExeUtilCompositeUnnestTcb(*this, childTcb, glob);
  exe_util_tcb->registerSubtasks();
  return (exe_util_tcb);
}

ExExeUtilCompositeUnnestTcb::ExExeUtilCompositeUnnestTcb(
     const ComTdbExeUtilCompositeUnnest & exe_util_tdb,
  ...
  if (exe_util_tdb.extractColExpr_)
    {
      ...
      (void)exe_util_tdb.extractColExpr_->fixup(0, getExpressionMode(), this,
  ...
  if (exe_util_tdb.returnColsExpr_)
    {
      ...
      (void)exe_util_tdb.returnColsExpr_->fixup(0, getExpressionMode(), this,
                                               space, heap, FALSE, glob);
    }
...
ex_tcb_private_state *  ExExeUtilCompositeUnnestTcb::allocatePstates(
     Lng32 &numElems,      // inout, desired/actual elements
     Lng32 &pstateLength)  // out, length of one element
{
      ...
      switch (step_)
	    ...
	    ex_queue_entry * centry = qchild_.up->getHeadEntry();
            ...
            if (unnestTdb().getExtractColExpr())
              {
                exprRetCode = unnestTdb().getExtractColExpr()->eval(centry->getAtp(),
                                                                    workAtp_);
            ...
            ExpTupleDesc * extractColRowTD =
              unnestTdb().workCriDesc_->getTupleDescriptor
              (unnestTdb().extractColAtpIndex_);
            ...
            if ((attr->getNullFlag()) &&
                (ExpTupleDesc::isNullValue(
                     extractColRow_ + attr->getNullIndOffset(),
	    ...
	    ex_queue_entry * centry = qchild_.up->getHeadEntry();
            ...
            if (unnestTdb().getReturnColsExpr())
              {
                exprRetCode = unnestTdb().getReturnColsExpr()->eval(centry->getAtp(),
                                                                   workAtp_);
                if (exprRetCode == ex_expr::EXPR_ERROR)
                  {
                    step_ = HANDLE_ERROR_;
	    ...
	    exprRetCode =
	      unnestTdb().getScanExpr()->eval(centry->getAtp(), workAtp_);
	    if (exprRetCode == ex_expr::EXPR_ERROR)
	      {
		step_ = HANDLE_ERROR_;
		break;
	    ...
	    short rc = 0;
	    if (moveRowToUpQueue(returnColsRow_, unnestTdb().returnColsRowLen_,
                                 &rc, FALSE))
	      return rc;
