							...
							short &rc)
{
  ...
  if ((getMItdb().queryType() == ComTdbExeUtilGetMetadataInfo::TABLES_IN_SCHEMA_) ||
    ...
    strcpy(wherePred, " where hive_table_type = 'VIRTUAL_VIEW' ");
           ...
           (getMItdb().queryType() == ComTdbExeUtilGetMetadataInfo::OBJECTNAMES_IN_SCHEMA_) ||
           (getMItdb().queryType() == ComTdbExeUtilGetMetadataInfo::OBJECTNAMES_IN_CATALOG_) ||
  ...
  if (getMItdb().queryType() == ComTdbExeUtilGetMetadataInfo::SCHEMAS_IN_CATALOG_)
    ...
    {
      if (getMItdb().queryType() == ComTdbExeUtilGetMetadataInfo::OBJECTNAMES_IN_SCHEMA_)
        str_sprintf(buf, "select rtrim(table_name) from table(hivemd(object names, \"%s\")) %s order by 1", 
                    getMItdb().getSch(),
                    wherePred);
      else
    ...
    {
      if (getMItdb().queryType() == ComTdbExeUtilGetMetadataInfo::OBJECTNAMES_IN_CATALOG_)
        str_sprintf(buf, "select trim(schema_name) || '.' || trim(table_name) from table(hivemd(object names)) %s order by 1", 
                    wherePred);
      else
...
short ExExeUtilLobShowddlTcb::work()
{
  ...
  while (1)
	...
	case RETURN_LOB_NAME_:
	  {
	    if ((qparent_.up->getSize() - qparent_.up->getLength())
                < (15 + (lobTdb().getNumLOBdatafiles() > 0 ? lobTdb().getNumLOBdatafiles() : 0)))
	      return WORK_OK;	//come back later
	    ...
	    moveRowToUpQueue(query_);
            ...
            if (lobTdb().getIsExternalLobCol(currLobNum_))
              ...
              {
                if (lobTdb().getNumLOBdatafiles() > 0)
                  {
                    HDFS_FileInfo *fileInfos;
                    HDFS_Client_RetCode hdfsClientRetcode;
                    
                    HdfsClient *hdfsClient = 
                      HdfsClient::newInstance((NAHeap *)getHeap(), NULL, 
                                              hdfsClientRetcode);
                    if (hdfsClientRetcode != HDFS_CLIENT_OK)
                      {
                        ExRaiseSqlError(getHeap(), &diagsArea_, 
                                        (ExeErrorCode)(8447), NULL,
                                        NULL, NULL, NULL, 
                                        "Internal error: HdfsClient::newInstance returned an error",
                                        NULL, NULL);
                        step_ = HANDLE_ERROR_;
                        break;
                      }
                    ...
                    Int32 numFiles = -1;
                    hdfsClientRetcode = hdfsClient->hdfsListDirectory
                      (lobTdb().getLOBloc(currLobNum_), &fileInfos, &numFiles);
                    ...
                    if (hdfsClientRetcode != HDFS_CLIENT_OK)
                      {
                        ExRaiseSqlError(getHeap(), &diagsArea_, 
                                        (ExeErrorCode)(8447), NULL,
                                        NULL, NULL, NULL, 
                                        "Internal error: hdfsClient->hdfsListDirectory returned an error",
                                        NULL, NULL);
                        step_ = HANDLE_ERROR_;
                        break;
                      }
                    ...
                    for (int f = 0; f < numFiles; f++)
                      {
                        char lobNumBuf[10];
                        str_sprintf(lobNumBuf, "_%04d_", lobTdb().getLOBnum(currLobNum_));
                        NAString fileName(fileInfos[f].mName);
                        if (fileName.contains(lobNumBuf))
                          {
                            size_t lastSlash = fileName.last('/');
                            fileName = fileName(lastSlash+1, fileName.length()-lastSlash-1);
                            str_sprintf(query_, "DataFile #%d: %s",
...
short ExExeUtilLobShowddlTcb::workV2()
{
  Lng32 cliRC = 0;
  short retcode = 0;
	...
	case FETCH_METADATA_SHOWDDL_:
	  {
	    ...
	    moveRowToUpQueue("============");
            ...
            str_sprintf(query_, "showddl %s.\"%s\".%s ",
	...
	case RETURN_METADATA_SHOWDDL_:
	  {
		...
		return retcode;
	      ...
	      {
		currLobNum_ = 1;
		step_ = FETCH_LOB_DESC_CHUNKS_SHOWDDL_;//RETURN_LOB_NAME_;
		return WORK_RESCHEDULE_AND_RETURN;
	      }
	  }
	  break;
	...
	case FETCH_LOB_DESC_CHUNKS_SHOWDDL_:
	  {
	    ...
	    moveRowToUpQueue("");	    
	    moveRowToUpQueue("LOB CHUNKS table");
	    moveRowToUpQueue("================");

	    char lobInfoTableNameBuf[1024];
            str_sprintf(lobInfoTableNameBuf, "%s.%s%020ld",
                        lobTdb().schName(),
	...
	case RETURN_LOB_NAME_:
	  {
		...
		break;
            ...
            NABoolean dirExists = FALSE; 
            HDFS_Client_RetCode hdfsClientRetcode;
            const char *path = lobTdb().getLOBloc(currLobNum_);
	    ...
	    moveRowToUpQueue("HDFS Data Storage");
	    moveRowToUpQueue("=================");
	    moveRowToUpQueue(" ");
	    ...
	    moveRowToUpQueue(query_);
            ...
            currLobNum_++;
	    step_ = RETURN_LOB_NAME_; //FETCH_LOB_DESC_CHUNKS_SHOWDDL_;
                                                            ...
                                                            char *charset)
{
  ...
  precision = -1;
  scale = -1;
  NAType * nat = NAType::getNATypeForHive(hiveType, isORC, FALSE, getHeap());
  if (nat)
    {
      ...
      strncpy(displayType, displayTypeNAS.data(), HIVEMD_DISPLAY_DATA_TYPE_LEN);
    ...
    {
      // clear diags area set during call to NAType::getNATypeForHive
      if (CmpCommon::diags())
...
short ExExeUtilHiveMDaccessTcb::work()
{
  ...
  while (1)
        ...
        case SETUP_SCHEMAS_:
          {
            ...
            if (hiveMDtdb().schemasSpecified())
              {
                std::stringstream ss(hiveMDtdb().getSchema());
                std::string item;
                while (getline(ss, item, ',')) 
                  {
                    NAString nas(item);
                    nas = nas.strip(NAString::both, ' ');
                    if ((nas == HIVE_SYSTEM_SCHEMA_LC) ||
                        (nas == HIVE_SYSTEM_SCHEMA))
                      nas = HIVE_DEFAULT_SCHEMA_EXE;
                    NAText * nat = new(getHeap()) NAText(nas);
                    schNames_.insert(nat);
                  }
              }
	    else if ((hiveMDtdb().mdType_ == ComTdbExeUtilHiveMDaccess::SCHEMAS_) ||
                     (! hiveMDtdb().getSchema()))
              {
        ...
        case GET_ALL_TABLES_IN_SCHEMA_:
          {
            ...
            if (! currObj)
              {
                HVC_RetCode retCode;
                if ((hiveMDtdb().mdType_ == ComTdbExeUtilHiveMDaccess::OBJECT_NAMETYPES_) &&
                    (NOT hiveMDtdb().specialObjectnametype()))
                  retCode = HiveClient_JNI::getAllTablesMeta((NAHeap *)getHeap(), currSch, tblNames_);
                else if (hiveMDtdb().mdType_ == ComTdbExeUtilHiveMDaccess::VIEWS_)
                  retCode = HiveClient_JNI::getAllViews((NAHeap *)getHeap(), currSch, tblNames_);
                else 
                  retCode = HiveClient_JNI::getAllTables((NAHeap *)getHeap(), currSch, tblNames_);
		    ...
		    ExRaiseSqlError(getHeap(), &diagsArea_, -1003, 
                ...
                else if ((retCode != HVC_OK) && (retCode != HVC_DONE)) 
                  {
                    char * methodName;
                    if (hiveMDtdb().mdType_ == ComTdbExeUtilHiveMDaccess::OBJECT_NAMETYPES_)
                      methodName = (char *) "HiveClient_JNI::getAllTablesMeta()";
                    else if (hiveMDtdb().mdType_ == ComTdbExeUtilHiveMDaccess::VIEWS_)
                      methodName = (char *) "HiveClient_JNI::getAllViews()";
                    else
                      methodName = (char *) "HiveClient_JNI::getAllTables()";
		    ExRaiseSqlError(getHeap(), &diagsArea_, -1190, 
                           (Lng32 *)&retCode, NULL, NULL, 
                           methodName,
                           HiveClient_JNI::getErrorText(retCode),
            ...
            if (NOT ((hiveMDtdb().mdType_ == ComTdbExeUtilHiveMDaccess::OBJECTNAMES_) ||
                     (hiveMDtdb().mdType_ == ComTdbExeUtilHiveMDaccess::OBJECT_NAMETYPES_) ||
                     (hiveMDtdb().mdType_ == ComTdbExeUtilHiveMDaccess::VIEWS_)))
              {
                ...
                while (i < tblNames_.entries())
                  {
                    hiveMD_->getTableDesc(schForHive_, tblNames_[i]->c_str(),
					  FALSE, FALSE, FALSE /* no subdir ts check */ , 
					  FALSE /*dont read partn info*/);
                    i++;
                  }
              }
	...
	case POSITION_:
	  {
	    ...
	    else if ((hiveMDtdb().mdType_ == ComTdbExeUtilHiveMDaccess::OBJECTNAMES_) ||
                     (hiveMDtdb().mdType_ == ComTdbExeUtilHiveMDaccess::OBJECT_NAMETYPES_) ||
                     (hiveMDtdb().mdType_ == ComTdbExeUtilHiveMDaccess::VIEWS_))                                    
	      {
                step_ = FETCH_TABLE_NAME_;
                tblNameIdx_ = 0;
	      }
	...
	case FETCH_TABLE_NAME_:
	  {
	    if (qparent_.up->isFull())
	      return WORK_OK;

            if (tblNameIdx_ == tblNames_.entries())
              {
                step_ = ADVANCE_SCHEMA_;
                break;
              }

            HiveMDObjectNameTypesColInfoStruct *s =(HiveMDObjectNameTypesColInfoStruct*)mdRow_;

	    str_cpy(s->catName, hiveCat_, 256, ' ');
	    str_cpy(s->schName, hiveSch_, 256, ' ');

            if ((hiveMDtdb().mdType_ == ComTdbExeUtilHiveMDaccess::OBJECTNAMES_) ||
                (hiveMDtdb().mdType_ == ComTdbExeUtilHiveMDaccess::VIEWS_))
              str_cpy(s->tblName, tblNames_[tblNameIdx_]->c_str(), 256, ' ');
            else if ((hiveMDtdb().mdType_ == ComTdbExeUtilHiveMDaccess::OBJECT_NAMETYPES_) &&
                     (hiveMDtdb().specialObjectnametype()))
              {
                str_cpy(s->tblName, tblNames_[tblNameIdx_]->c_str(), 256, ' ');
                str_cpy(s->hiveTableType, "TABLE", 128, ' '); 
              }
            else
            {
              // OBJECT_NAMETYPES_
              const char* tok = ", TableType: ";
              size_t foundB = tblNames_[tblNameIdx_]->find(tok);
              if (foundB == std::string::npos) {
                // error
		 ExRaiseSqlError(getHeap(), &diagsArea_, -1004, 
                           NULL, NULL, NULL, 
                           tblNames_[tblNameIdx_]->c_str());
                 step_ = HANDLE_ERROR_;
                 break;
              }
              
              str_cpy(s->tblName, tblNames_[tblNameIdx_]->substr(0,foundB).c_str(), 
                      256, ' ');
              str_cpy(s->hiveTableType, 
                      tblNames_[tblNameIdx_]->substr(foundB+strlen(tok)).c_str(),
                      128, ' ');
            }
	...
	case FETCH_TABLE_:
	  {
	    ...
	    str_cpy(s->schName, hiveSch_, 256, ' ');
            ...
            str_cpy(s->tblName, tblNames_[0]->c_str(), 256, ' ');
            ...
            if (htd->getSDs())
              {
                ...
                else if (htd->getSDs()->isParquetFile())
                  str_cpy(s->fileFormat, "PARQUET", 24, ' ');
                else if (htd->getSDs()->isAvroFile())
                  str_cpy(s->fileFormat, "AVRO", 24, ' ');
              }
	    ...
	    if (retcode < 0)
	      {
                if (! getenv("SQLMX_REGRESS"))
                  {
                    // add a warning and continue if not running regressions.
                    char strP[2500];
                    snprintf(strP, sizeof(strP), "Datatype %s for column '%s' in table %s.%s.%s is not supported. This table will be ignored.", 
...
///////////////////////////////////////////////////////////////////
// class ExExeUtilParquetStatsTdb/Tcb
///////////////////////////////////////////////////////////////////
ex_tcb * ExExeUtilParquetStatsTdb::build(ex_globals * glob)
{
  ExExeUtilParquetStatsTcb * exe_util_tcb;

  if (displayFormat())
    exe_util_tcb = new(glob->getSpace()) ExExeUtilParquetStatsFormatTcb(*this, glob);
  else
    exe_util_tcb = new(glob->getSpace()) ExExeUtilParquetStatsTcb(*this, glob);
...
////////////////////////////////////////////////////////////////
// Constructor for class ExExeUtilParquetStatsTcb
///////////////////////////////////////////////////////////////
ExExeUtilParquetStatsTcb::ExExeUtilParquetStatsTcb(
     const ComTdbExeUtilParquetStats & exe_util_tdb,
     ex_globals * glob)
     : ExExeUtilTcb( exe_util_tdb, NULL, glob)
{
  statsBuf_ = new(glob->getDefaultHeap()) char[sizeof(ComTdbParquetStatsVirtTableColumnStruct)];
  statsBufLen_ = sizeof(ComTdbParquetStatsVirtTableColumnStruct);

  stats_ = (ComTdbParquetStatsVirtTableColumnStruct*)statsBuf_;
  ...
  ehi_ = ExpParquetInterface::newInstance(glob->getDefaultHeap(),
                                          (char*)"", //exe_util_tdb.server(), 
                                          0); //exe_util_tdb.zkPort()
...
//////////////////////////////////////////////////////
// work() for ExExeUtilParquetStatsTcb
//////////////////////////////////////////////////////
Int64 ExExeUtilParquetStatsTcb::getEmbeddedNumValue
(char* &sep, char endChar, NABoolean adjustLen)
{
  Int64 num = -1;
  char * sepEnd = strchr(sep+1, endChar);
...
short ExExeUtilParquetStatsTcb::collectStats(char * rootDir)
{
  int retcode = 0;

  retcode = ehi_->getFileStats(rootDir, &fileStats_);
  if (retcode < 0)
    {
      return retcode;
    }
...
short ExExeUtilParquetStatsTcb::populateStats
(char * tableName, char * fileName, Int32 currIndex)
{
  // populate catName_, schName_, objName_.
  if (extractParts(tableName,
                   &catName_, &schName_, &objName_))
  ...
  str_pad(stats_->rootDir, sizeof(stats_->rootDir), ' ');
  if (getPStdb().getRootDirLoc() && strlen(getPStdb().getRootDirLoc()) > 0)
    str_cpy_all(stats_->rootDir, getPStdb().getRootDirLoc(), 
                strlen(getPStdb().getRootDirLoc()));

  str_pad(stats_->fileName, sizeof(stats_->fileName), ' ');
  str_pad(stats_->filePath, sizeof(stats_->filePath), ' ');
  str_pad(stats_->blockName, sizeof(stats_->blockName), ' ');
  
  char fileInfoBuf[5000];
  Int32 len = 0;
  char *fileInfo = fileInfoBuf;
  char *val = fileStats_->at(currIndex).val;
  len = fileStats_->at(currIndex).len; 
  if (len >= sizeof(fileInfoBuf))
     len = sizeof(fileInfoBuf)-1;
  strncpy(fileInfoBuf, val, len);
  fileInfoBuf[len] = '\0';

  stats_->fileNum              = 0;
  stats_->compressedSize       = 0;
  stats_->totalSize            = 0;
  stats_->rowCount             = 0;
  stats_->startingPos          = 0;

  char filePath[1000];

  char longBuf[30];
  char * sep1 = strchr(fileInfo, '|');
  char * sepStart = NULL;
  if (sep1)
    {
      str_cpy_all(stats_->filePath,
                  fileInfo, 
                  (Lng32)(sep1 - fileInfo)); 
    }
  ...
  if (sep1)
    {
      str_cpy_all(stats_->blockName, sepStart, 
                  (Lng32)(sep1 - sepStart)); 
    }

  sepStart = sep1 ;
  stats_->fileNum       = getEmbeddedNumValue(sepStart, '|', FALSE);
  stats_->blockNum      = getEmbeddedNumValue(sepStart, '|', FALSE);
  stats_->compressedSize = getEmbeddedNumValue(sepStart, '|', FALSE);
  stats_->totalSize = getEmbeddedNumValue(sepStart, '|', FALSE);
  stats_->rowCount = getEmbeddedNumValue(sepStart, '|', FALSE);
  stats_->startingPos = getEmbeddedNumValue(sepStart, '|', FALSE);
  
  return 0;
}

short ExExeUtilParquetStatsTcb::work()
{
  short retcode = 0;
  Lng32 cliRC = 0;
  ex_expr::exp_return_type exprRetCode = ex_expr::EXPR_OK;
  ...
  while (1)
	...
	case INITIAL_:
	  {
            ...
            if (getPStdb().inputExpr())
              {
                step_ = EVAL_INPUT_;
                break;
              }

            strcpy(tableName_, getPStdb().getTableName());
            hdfsFileName_ = NULL;
	...
	break;
        ...
        case EVAL_INPUT_:
          {
	    workAtp_->getTupp(getPStdb().workAtpIndex())
	      .setDataPointer(inputNameBuf_);

	    ex_expr::exp_return_type exprRetCode =
	      getPStdb().inputExpr()->eval(pentry_down->getAtp(), workAtp_);
	    if (exprRetCode == ex_expr::EXPR_ERROR)
	      {
		step_ = HANDLE_ERROR_;
		break;
        ...
        case COLLECT_STATS_:
          {
            retcode = collectStats(getPStdb().getRootDirLoc());
            if (retcode < 0) 
              {
                if (retcode != -1) 
                  {
	...
	case POPULATE_STATS_BUF_:
	  {
            if ((! fileStats_) || currIndex_ == fileStats_->entries())
              {
                step_ = DONE_;
                break;
              }
            
            if (populateStats(tableName_, hdfsFileName_, currIndex_))
              {
                step_ = HANDLE_ERROR_;
                break;
              }
	...
	case DONE_:
	  {
            if (fileStats_ != NULL) {
              deleteNAArray(getHeap(), fileStats_);
              fileStats_ = NULL;
            }
	    retcode = handleDone();
	    if (retcode == 1)
...
////////////////////////////////////////////////////////////////
// Constructor for class ExExeUtilParquetStatsFormatTcb
///////////////////////////////////////////////////////////////
ExExeUtilParquetStatsFormatTcb::ExExeUtilParquetStatsFormatTcb(
     const ComTdbExeUtilParquetStats & exe_util_tdb,
     ex_globals * glob)
     : ExExeUtilParquetStatsTcb( exe_util_tdb, glob)
{
  statsTotalsBuf_ = new(glob->getDefaultHeap()) char[sizeof(ComTdbParquetStatsVirtTableColumnStruct)];

  statsTotals_ = (ComTdbParquetStatsVirtTableColumnStruct*)statsTotalsBuf_;
...
short ExExeUtilParquetStatsFormatTcb::initTotals()
{
  statsTotals_->compressedSize         = 0;
  statsTotals_->totalSize              = 0;
  statsTotals_->rowCount               = 0;

  return 0;
}

short ExExeUtilParquetStatsFormatTcb::computeTotals()
{
  str_pad(statsTotals_->catalogName, sizeof(statsTotals_->catalogName), ' ');
  ...
  str_pad(statsTotals_->rootDir, sizeof(statsTotals_->rootDir), ' ');

  str_pad(statsTotals_->fileName, sizeof(statsTotals_->fileName), ' ');

  str_pad(statsTotals_->filePath, sizeof(statsTotals_->filePath), ' ');

  statsTotals_->fileNum = 0;

  statsTotals_->blockNum = 0;
  for (Int32 currIndex = 0; currIndex < fileStats_->entries(); currIndex++)
    {
      if (populateStats(tableName_, hdfsFileName_, currIndex))
        return -1;

      if (stats_->fileNum > statsTotals_->fileNum)
        statsTotals_->fileNum++;

      statsTotals_->blockNum++;

      statsTotals_->compressedSize      += stats_->compressedSize;
      statsTotals_->totalSize           += stats_->totalSize;
      statsTotals_->rowCount            += stats_->rowCount;
    }
  
  return 0;
}

short ExExeUtilParquetStatsFormatTcb::work()
{
  short retcode = 0;
  Lng32 cliRC = 0;
  ...
  while (1)
	...
	break;
        ...
        case COMPUTE_TOTALS_:
          {
            ...
            if ((getPStdb().summaryOnly()) ||
                (fileStats_->entries() == 0))
              {
                step_ = POPULATE_STATS_BUF_;
              }
            else
        ...
        case RETURN_DETAILS_:
          {
            ...
            // display heading before returning
            // any stats details.
            // of first file
            if (currFileNum_ > 1)
              {
	    ...
	    if (moveRowToUpQueue(buf, strlen(buf), &rc))
	      return rc;
            ...
            str_sprintf(buf, "File Stats Details");
	    if (moveRowToUpQueue(buf, strlen(buf), &rc))
	      return rc;
	    ...
	    if (moveRowToUpQueue(buf, strlen(buf), &rc))
	      return rc;
            ...
            str_sprintf(buf, "  ObjectName:        %s", tableName_);
	    if (moveRowToUpQueue(buf, strlen(buf), &rc))
	      return rc;

            strcpy(buf, "  FileFormat:        parquet"); 
	    if (moveRowToUpQueue(buf, strlen(buf), &rc))
	      return rc;

            str_sprintf(buf, "  RootDir:           %s", 
                        getPStdb().getRootDirLoc());
	    if (moveRowToUpQueue(buf, strlen(buf), &rc))
	      return rc;
	...
	case POPULATE_STATS_BUF_:
	  {
            if (currIndex_ == fileStats_->entries())
              {
                // Done with all stats entries.
                step_ = RETURN_SUMMARY_;
                break;
              }
	    ...
	    short rc = 0;
            ...
            str_sprintf(buf, "  FileNum:           %ld", stats_->fileNum);
	    if (moveRowToUpQueue(buf, strlen(buf), &rc))
	      return rc;

            str_sprintf(buf, "  FilePath:          %s", 
                        removeTrailingBlanks(stats_->filePath, STATS_REGION_NAME_MAX_LEN).data());
	    if (moveRowToUpQueue(buf, strlen(buf), &rc))
	      return rc;
            
            str_sprintf(buf, "  FileName:          %s", 
                        removeTrailingBlanks(stats_->fileName, STATS_NAME_MAX_LEN).data());
	    if (moveRowToUpQueue(buf, strlen(buf), &rc))
	      return rc;

            /*            
            str_sprintf(buf, "  BlockName:         %s", 
                        removeTrailingBlanks(stats_->blockName, STATS_NAME_MAX_LEN).data());
	    if (moveRowToUpQueue(buf, strlen(buf), &rc))
	      return rc;
            */

            str_sprintf(buf, "  BlockNum:          %ld", stats_->blockNum);
	    if (moveRowToUpQueue(buf, strlen(buf), &rc))
	      return rc;
            
            str_sprintf(buf, "  CompressedSize:    %ld Bytes", stats_->compressedSize);
	    if (moveRowToUpQueue(buf, strlen(buf), &rc))
	      return rc;

            str_sprintf(buf, "  ActualSize:        %ld Bytes", stats_->totalSize);
	    if (moveRowToUpQueue(buf, strlen(buf), &rc))
	      return rc;

            str_sprintf(buf, "  RowCount:          %ld", stats_->rowCount);
	    if (moveRowToUpQueue(buf, strlen(buf), &rc))
	      return rc;
	    ...
	    if (moveRowToUpQueue(buf, strlen(buf), &rc))
	      return rc;
            ...
            str_sprintf(buf, "File Stats Summary");
	    if (moveRowToUpQueue(buf, strlen(buf), &rc))
	      return rc;
	    ...
	    if (moveRowToUpQueue(buf, strlen(buf), &rc))
	      return rc;
            ...
            str_sprintf(buf, "  TotalNumFiles:           %ld", statsTotals_->fileNum);
            //                       fileStats_->entries());
	    if (moveRowToUpQueue(buf, strlen(buf), &rc))
	      return rc;

            str_sprintf(buf, "  TotalNumBlocks:          %ld", statsTotals_->blockNum);
	    if (moveRowToUpQueue(buf, strlen(buf), &rc))
	      return rc;

            str_sprintf(buf, "  TotalCompressedSize:     %ld", statsTotals_->compressedSize);
	    if (moveRowToUpQueue(buf, strlen(buf), &rc))
	      return rc;

            str_sprintf(buf, "  TotalActualSize:         %ld", statsTotals_->totalSize);
	    if (moveRowToUpQueue(buf, strlen(buf), &rc))
	      return rc;

            str_sprintf(buf, "  TotalRowCount:           %ld", statsTotals_->rowCount);
	    if (moveRowToUpQueue(buf, strlen(buf), &rc))
	      return rc;
...
///////////////////////////////////////////////////////////////////
// class ExExeUtilGetExtSchemaTdb/Tcb
///////////////////////////////////////////////////////////////////
ex_tcb * ExExeUtilGetExtSchemaTdb::build(ex_globals * glob)
{
  ExExeUtilGetExtSchemaTcb * exe_util_tcb;

  exe_util_tcb = new(glob->getSpace()) ExExeUtilGetExtSchemaTcb(*this, glob);
...
////////////////////////////////////////////////////////////////
// Constructor for class ExExeUtilGetExtSchemaTcb
///////////////////////////////////////////////////////////////
ExExeUtilGetExtSchemaTcb::ExExeUtilGetExtSchemaTcb(
     const ComTdbExeUtilGetExtSchema & exe_util_tdb,
     ex_globals * glob)
     : ExExeUtilTcb( exe_util_tdb, NULL, glob)
{
  exti_ = NULL;

  if (exe_util_tdb.isParquet())
    exti_ = ExpParquetInterface::newInstance(glob->getDefaultHeap(),
                                             (char*)"", 0);
  else if (exe_util_tdb.isAvro())
...
short ExExeUtilGetExtSchemaTcb::work()
{
  short retcode = 0;
  Lng32 cliRC = 0;
  ...
  while (1)
	...
	break;
        ...
        case GET_SCHEMAS_:
          {
            ...
            if ((getExtSchemaTdb().isParquet()) &&
                (getExtSchemaTdb().parquetLegacyTS()))
              {
                flags = PARQUET_LEGACY_TS;
              }

            retcode = exti_->getFileSchema(
                 getExtSchemaTdb().getTableName(),
                 getExtSchemaTdb().getRootDirLoc(),
                 &colNameVec,
                 &colTypeVec,
                 flags,
                 readSchema_, writeSchema_);
            if (retcode < 0) 
              {
                if (retcode != -1) 
                  {
            ...
            if (getExtSchemaTdb().isParquet())
              {
                schemas_[READ_SCHEMA_IDX_] = 
                  (getExtSchemaTdb().isRead() ? getExtSchemaTdb().parqSchStr() : NULL);
              }
            else
              {
                schemas_[READ_SCHEMA_IDX_] = 
                  (getExtSchemaTdb().isRead() ? readSchema_ : NULL);
              }
            schemas_[WRITE_SCHEMA_IDX_] = 
              (getExtSchemaTdb().isWrite() ? writeSchema_ : NULL);
            currSchemaIdx_ = READ_SCHEMA_IDX_;
            currSchema_ = schemas_[currSchemaIdx_];
		...
		return WORK_CALL_AGAIN; // come back later
            ...
            ULng32 neededSize = SqlBufferNeededSize(7, 250);
            if (! pool_->get_free_buffer(neededSize))
              {
                return WORK_CALL_AGAIN;
              }
	    ...
	    if (moveRowToUpQueue(buf, strlen(buf), &rc))
	      return rc;
            ...
            str_sprintf(buf, "%s %s schema", 
                        (getExtSchemaTdb().isParquet() ? "Parquet" : "Avro"),
                        (currSchemaIdx_ == READ_SCHEMA_IDX_ ? "Read" : "Write"));
	    if (moveRowToUpQueue(buf, strlen(buf), &rc))
	      return rc;
	    ...
	    if (moveRowToUpQueue(buf, strlen(buf), &rc))
	      return rc;
            ...
            sepStart_ = currSchema_;
            schemaLen_ = strlen(currSchema_);

            step_ = RETURN_ONE_LINE_;
          }
          break;

        case RETURN_ONE_LINE_:
          {
            char buf[1000];
	    short rc = 0;

            char * sep = strchr(sepStart_, '\n');
            if (sep == NULL)
              str_cpy_and_null(buf, sepStart_, 
                               (Lng32)(schemaLen_ - (sepStart_ - currSchema_)),
                               '\0', ' ', TRUE);
            else
              {
                str_cpy_and_null(buf, sepStart_, (Lng32)(sep - sepStart_),
                                 '\0', ' ', TRUE);
              }
	    ...
	    if (moveRowToUpQueue(buf, strlen(buf), &rc))
	      return rc;
            ...
            if ((sep == NULL) ||
                ((Lng32)(sepStart_- currSchema_) >= schemaLen_))
              step_ = NEXT_SCHEMA_;
          }
          break;
        ...
        case NEXT_SCHEMA_:
          {
            ...
            if (currSchemaIdx_ == NUM_SCHEMAS_)
              step_ = DONE_;
            else
              {
                currSchema_ = schemas_[currSchemaIdx_];
...
// Constructor and destructor for ExeUtil_private_state
/////////////////////////////////////////////////////////////////////////////
ExExeUtilParquetStatsPrivateState::ExExeUtilParquetStatsPrivateState()
{
}
...
///////////////////////////////////////////////////////////////////
// class ExExeUtilAvroStatsTdb/Tcb
///////////////////////////////////////////////////////////////////
ex_tcb * ExExeUtilAvroStatsTdb::build(ex_globals * glob)
{
  ExExeUtilAvroStatsTcb * exe_util_tcb;

  if (displayFormat())
    exe_util_tcb = new(glob->getSpace()) ExExeUtilAvroStatsFormatTcb(*this, glob);
  else
    exe_util_tcb = new(glob->getSpace()) ExExeUtilAvroStatsTcb(*this, glob);
...
////////////////////////////////////////////////////////////////
// Constructor for class ExExeUtilAvroStatsTcb
///////////////////////////////////////////////////////////////
ExExeUtilAvroStatsTcb::ExExeUtilAvroStatsTcb(
     const ComTdbExeUtilAvroStats & exe_util_tdb,
     ex_globals * glob)
     : ExExeUtilTcb( exe_util_tdb, NULL, glob)
{
  statsBuf_ = new(glob->getDefaultHeap()) char[sizeof(ComTdbAvroStatsVirtTableColumnStruct)];
  statsBufLen_ = sizeof(ComTdbAvroStatsVirtTableColumnStruct);

  stats_ = (ComTdbAvroStatsVirtTableColumnStruct*)statsBuf_;
  ...
  ehi_ = ExpAvroInterface::newInstance(glob->getDefaultHeap(),
                                          (char*)"", //exe_util_tdb.server(), 
                                          0); //exe_util_tdb.zkPort()
...
//////////////////////////////////////////////////////
// work() for ExExeUtilAvroStatsTcb
//////////////////////////////////////////////////////
Int64 ExExeUtilAvroStatsTcb::getEmbeddedNumValue
(char* &sep, char endChar, NABoolean adjustLen)
{
  Int64 num = -1;
  char * sepEnd = strchr(sep+1, endChar);
...
short ExExeUtilAvroStatsTcb::collectStats(char * rootDir)
{
  int retcode = 0;
...
short ExExeUtilAvroStatsTcb::populateStats
(char * tableName, char * fileName, Int32 currIndex)
{
  // populate catName_, schName_, objName_.
  if (extractParts(tableName,
  ...
  str_pad(stats_->fileName, sizeof(stats_->fileName), ' ');
  str_pad(stats_->filePath, sizeof(stats_->filePath), ' ');
  str_pad(stats_->modTime, sizeof(stats_->modTime), ' ');
  ...
  sepStart = sep1 ;
  stats_->fileNum        = getEmbeddedNumValue(sepStart, '|', FALSE);
  stats_->fileSize       = getEmbeddedNumValue(sepStart, '|', FALSE);
  stats_->fileMaxSize    = getEmbeddedNumValue(sepStart, '|', FALSE);
  stats_->numBlocks      = getEmbeddedNumValue(sepStart, '|', FALSE);
  stats_->rowCount       = getEmbeddedNumValue(sepStart, '|', FALSE);
...
short ExExeUtilAvroStatsTcb::work()
{
  short retcode = 0;
  Lng32 cliRC = 0;
  ex_expr::exp_return_type exprRetCode = ex_expr::EXPR_OK;
...
////////////////////////////////////////////////////////////////
// Constructor for class ExExeUtilAvroStatsFormatTcb
///////////////////////////////////////////////////////////////
ExExeUtilAvroStatsFormatTcb::ExExeUtilAvroStatsFormatTcb(
     const ComTdbExeUtilAvroStats & exe_util_tdb,
     ex_globals * glob)
     : ExExeUtilAvroStatsTcb( exe_util_tdb, glob)
{
  statsTotalsBuf_ = new(glob->getDefaultHeap()) char[sizeof(ComTdbAvroStatsVirtTableColumnStruct)];

  statsTotals_ = (ComTdbAvroStatsVirtTableColumnStruct*)statsTotalsBuf_;
...
short ExExeUtilAvroStatsFormatTcb::initTotals()
{
  statsTotals_->fileSize               = 0;
  statsTotals_->rowCount               = 0;
  statsTotals_->numBlocks              = 0;
  statsTotals_->fileMaxSize            = 0;

  return 0;
}

short ExExeUtilAvroStatsFormatTcb::computeTotals()
{
  str_pad(statsTotals_->catalogName, sizeof(statsTotals_->catalogName), ' ');
  ...
  str_pad(statsTotals_->modTime, sizeof(statsTotals_->modTime), ' ');
  ...
  for (Int32 currIndex = 0; currIndex < fileStats_->entries(); currIndex++)
    {
      ...
      statsTotals_->fileSize      += stats_->fileSize;
      statsTotals_->rowCount      += stats_->rowCount;
      statsTotals_->numBlocks     += stats_->numBlocks;
    }
  
  return 0;
}

short ExExeUtilAvroStatsFormatTcb::work()
{
  short retcode = 0;
  Lng32 cliRC = 0;
	...
	case POPULATE_STATS_BUF_:
	  {
	    ...
	    if (moveRowToUpQueue(buf, strlen(buf), &rc))
	      return rc;
            ...
            str_sprintf(buf, "  FileSize:          %ld Bytes", stats_->fileSize);
	    if (moveRowToUpQueue(buf, strlen(buf), &rc))
	      return rc;

            str_sprintf(buf, "  FileMaxSize:       %ld Bytes", stats_->fileMaxSize);
	    if (moveRowToUpQueue(buf, strlen(buf), &rc))
	      return rc;

            str_sprintf(buf, "  NumBlocks:         %ld", stats_->numBlocks);
	    if (moveRowToUpQueue(buf, strlen(buf), &rc))
	      return rc;
	    ...
	    if (moveRowToUpQueue(buf, strlen(buf), &rc))
	      return rc;
            ...
            str_sprintf(buf, "  ModificationTime:  %s UTC", 
                        removeTrailingBlanks(stats_->modTime, 24).data());
	    if (moveRowToUpQueue(buf, strlen(buf), &rc))
	      return rc;
	    ...
	    if (moveRowToUpQueue(buf, strlen(buf), &rc))
	      return rc;
            ...
            str_sprintf(buf, "  TotalNumFiles:           %d", 
                        fileStats_->entries());
	    if (moveRowToUpQueue(buf, strlen(buf), &rc))
	      return rc;

            str_sprintf(buf, "  TotalFileSize:           %ld Bytes", 
                        statsTotals_->fileSize);
	    if (moveRowToUpQueue(buf, strlen(buf), &rc))
	      return rc;

            str_sprintf(buf, "  TotalNumBlocks:          %ld", 
                        statsTotals_->numBlocks);
	    if (moveRowToUpQueue(buf, strlen(buf), &rc))
	      return rc;

            str_sprintf(buf, "  TotalRowCount:           %ld", 
                        statsTotals_->rowCount);
	    if (moveRowToUpQueue(buf, strlen(buf), &rc))
	      return rc;
...
// Constructor and destructor for ExeUtil_private_state
/////////////////////////////////////////////////////////////////////////////
ExExeUtilAvroStatsPrivateState::ExExeUtilAvroStatsPrivateState()
{
}
...
short ExExeUtilLobInfoTcb::collectAndReturnLobInfo(char * tableName,Int32 currLobNum, ContextCli *currContext)
{
  ...
  short rc = 0;
  if (isUpQueueFull(5 + (getLItdb().getNumLOBdatafiles() > 0 ? (getLItdb().getNumLOBdatafiles()*2+1) : 0)))
    {
      return WORK_CALL_AGAIN; // come back later
    }
...
if (getLItdb().getLobTypeList()[(currLobNum-1)*sizeof(Int32)] == Lob_External)
  ...
  Int32 fileNum = -1;
  if ((getLItdb().getNumLOBdatafiles() > 0) &&
      (getLItdb().getLobTypeList()[(currLobNum-1)*sizeof(Int32)] != Lob_External))
    fileNum = 1;
  ...
  while (NOT done)
    {
      ...
      char *lobDataFile = 
        ExpLOBoper::ExpGetLOBname
        (getLItdb().objectUID_, 
         *(Int16*)&getLItdb().getLobNumList()[(currLobNum-1)*sizeof(Int16)], 
      ...
      if (isExternal || (fileNum <= 0))
        done = TRUE;
      else
        {
          fileNum++;
          if (fileNum > getLItdb().getNumLOBdatafiles())
            done = TRUE;
        }
    } // while
  ...
  if (fileNum > 0)
    {
      str_sprintf(buf, "  Total EOD :  %ld", totalLobEOD);
      if (moveRowToUpQueue(buf, strlen(buf), &rc))
        return rc;
    }
  ...
  if ((len ==0) ||(getLItdb().getLobTypeList()[(currLobNum-1)*sizeof(Int32)] == Lob_External))
    outlen = 0;
...
short ExExeUtilLobInfoTcb::collectAndReturnLobInfoV2(char * tableName,Int32 currLobNum, ContextCli *currContext)
{
  char *catName = NULL;
  char *schName = NULL;
  char *objName = NULL;
  ...
  char buf[LOBINFO_MAX_FILE_LEN+500];
  short rc = 0;
  if (isUpQueueFull(10 + (getLItdb().getNumLOBdatafiles() > 0 ? (getLItdb().getNumLOBdatafiles()*2+1) : 0)))
    {
      return WORK_CALL_AGAIN; // come back later
    }
  ...
  strcpy(columnName, &((getLItdb().getLobColList())[offset]));
  removeTrailingBlanks(columnName, LOBINFO_MAX_FILE_LEN);
  str_sprintf(buf, "  Column Name:   %-s", columnName);
  if (moveRowToUpQueue(buf, strlen(buf), &rc))
    return rc;

  ExpLobOperV2::DMLOptions *options = 
    (ExpLobOperV2::DMLOptions *)getLItdb().getLobDmlOptions();

  short chunkSchNameLen = 0;
  char chunkSchName[512];
  if (options->chunkTableSchNameLen > 0)
    {
      chunkSchNameLen = options->chunkTableSchNameLen;
      strcpy(chunkSchName, options->chunkTableSch);
      strcat(chunkSchName, ".");
    }
  else
    strcpy(chunkSchName, "");

  char lobChunksFileBuf[LOBINFO_MAX_FILE_LEN*2];
  str_sprintf(lobChunksFileBuf, "%s%s%020ld",
              chunkSchName, LOB_CHUNKS_V2_PREFIX, getLItdb().objectUID_);

  Int64 inlinedDataLen = 0;
  Int64 hbaseDataLen = 0;
  Int64 hdfsDataLen = 0;
  ...
  if (getLItdb().getLobTypeList()[(currLobNum-1)*sizeof(Int32)] != Lob_External)
    {
      inlinedDataLen = *(Int32*)(&getLItdb().getLobInlineMaxLenList()[(currLobNum-1)*sizeof(Int32)]);
      str_sprintf(buf, "  Inline MaxLen: %-ld", inlinedDataLen);
      if (moveRowToUpQueue(buf, strlen(buf), &rc))
        return rc;   

      // Sum of all chunks stored in HBase
      str_sprintf (query,  "select cast (case when sum(chunklen) is null then 0 else sum(chunklen) end as largeint not null) from %s where lobnum = %d and hdfs_offset < 0", 
                   lobChunksFileBuf, currLobNum);
      // set parserflags to allow ghost table
      currContext->setSqlParserFlags(0x1);	
      
      Lng32 len = 0;
      Int32 cliRC = cliInterface()->executeImmediate(query,(char *)&hbaseDataLen, &len, FALSE);
      ...
      str_sprintf(buf, "  HBase DataLen: %-ld", hbaseDataLen);
      if (moveRowToUpQueue(buf, strlen(buf), &rc))
        return rc;   

      // Sum of all chunks stored in HDFS
      str_sprintf (query,  "select cast (case when sum(chunklen) is null then 0 else sum(chunklen) end as largeint not null) from %s where lobnum = %d and hdfs_offset >= 0", 
                   lobChunksFileBuf, currLobNum);
      // set parserflags to allow ghost table
      currContext->setSqlParserFlags(0x1);	
      
      cliRC = cliInterface()->executeImmediate(query,(char *)&hdfsDataLen, &len, FALSE);
      ...
      str_sprintf(buf, "  HDFS DataLen:  %-ld", hdfsDataLen);
      if (moveRowToUpQueue(buf, strlen(buf), &rc))
        return rc;   
    }
  ...
  NABoolean dirExists = FALSE; 
  HDFS_Client_RetCode hdfsClientRetcode;
  hdfsClientRetcode = HdfsClient::hdfsExists(lobLocation, dirExists);
  ...
  if (getLItdb().getLobTypeList()[(currLobNum-1)*sizeof(Int32)] == Lob_External)
    ...
    str_sprintf(buf, "  HDFS Location: %-s", lobLocation);
  if (moveRowToUpQueue(buf, strlen(buf), &rc))
    return rc;      
  ...
  while (NOT done)
    {
      ...
      char *lobDataFile = 
        ExpLOBoper::ExpGetLOBnameWithFilenum
        (getLItdb().objectUID_, 
         *(Int16*)&getLItdb().getLobNumList()[(currLobNum-1)*sizeof(Int16)], 
         (isExternal ? -1 : fileNum),
         tgtLobNameBuf, LOBINFO_MAX_FILE_LEN);
      ...
      if (isExternal)
        ...
        str_sprintf(buf, "  HDFS DataFile: %-s", lobDataFile);
      if (moveRowToUpQueue(buf, strlen(buf), &rc))
        return rc;  
      ...
      str_sprintf(buf, "  HDFS File EOD: %-ld", lobEOD);
      if (moveRowToUpQueue(buf, strlen(buf), &rc))
        return rc;
  ...
  if (fileNum > 0)
    {
      str_sprintf(buf, "  Total HDFS EOD:%ld", totalLobEOD);
      if (moveRowToUpQueue(buf, strlen(buf), &rc))
        return rc;
    }
        ...
        case COLLECT_LOBINFO_:
          {
            ...
            if (getLItdb().lobV2())
              retcode = collectAndReturnLobInfoV2(tableName_,currLobNum_, currContext);
            else
              retcode = collectAndReturnLobInfo(tableName_,currLobNum_, currContext);
            if (retcode == WORK_CALL_AGAIN)
              return WORK_CALL_AGAIN;
            else if (retcode)
              {
                step_ = HANDLE_ERROR_;
...
short ExExeUtilLobInfoTableTcb::collectAndReturnLobTableInfo(char * tableName,Int32 currLobNum, ContextCli *currContext)
{
  char *catName = NULL;
  char *schName = NULL;
  char *objName = NULL;
  ...
  if (isUpQueueFull(5 + (getLItdb().getNumLOBdatafiles() > 0 ? getLItdb().getNumLOBdatafiles() : 0)))
    {
      return WORK_CALL_AGAIN; // come back later
    }
  ...
  //Get the descriptor chunks table name
  char *lobDescChunksFile = NULL;
  if (getLItdb().lobV2())
    {
      lobDescChunksFile = lobDescChunkFileBuf;
      str_sprintf(lobDescChunksFile, "%s%020ld",
                  LOB_CHUNKS_V2_PREFIX,
                  getLItdb().objectUID_);
    }
  else
  ...
  while (NOT done)
    {
      ...
      lobInfo_->lobDataFileSizeEod = 0;
      lobInfo_->lobDataFileSizeUsed = 0;
      ...
      if (fileNum >= 0)
        {
          if (getLItdb().lobV2())
            {
              str_sprintf (query, "select sum(chunklen) from %s where hash2partfunc(lobuid for %d) +1 = %d ", 
                           lobDescChunksFile,
                           getLItdb().getNumLOBdatafiles(),
                           fileNum);
            }
          else
            {
              str_sprintf (query, "select sum(chunklen) from %s where hash2partfunc(descpartnkey for %d) +1 = %d ", 
                           lobDescChunksFile,
                           getLItdb().getNumLOBdatafiles(),
                           fileNum);
            }            
      ...
      currContext->setSqlParserFlags(0x1);
      Int64 outlen = 0;Lng32 len = 0;
      Int32 cliRC = cliInterface()->executeImmediate(
           query, (char *)&outlen, &len, FALSE);
      if ((len == 0) || (getLItdb().getLobTypeList()[(currLobNum-1)*sizeof(Int32)] == Lob_External))
        outlen = 0;
	...
	case INITIAL_:
	  {
	    ...
	    step_ = COLLECT_AND_RETURN_LOBINFO_;
	  }
	break;
        ...
        case COLLECT_AND_RETURN_LOBINFO_:
          {
            if (currLobNum_ == getLItdb().getNumLobs()+1)
              {
                step_ = DONE_;
            ...
            retcode = collectAndReturnLobTableInfo(tableName_,currLobNum_, currContext);
            if (retcode == WORK_CALL_AGAIN)
              return WORK_CALL_AGAIN;
            else if (retcode)
              {
