  ...
  if (!isDefinedInOptions(options, "-Xmx"))
  {
      ...
      if (sqlciProcess) {
        ...
        QRLogger::log(CAT_SQL_EXE, LL_INFO,
                    "Backup Restore JVM Max heap option -Xmx: %d",
                    maxHeapEnvvarMB);
      }
    ...
    if (pfp != NULL)
    {
       ...
       jvm_options[numJVMOptions].extraInfo = NULL;
       QRLogger::log(CAT_SQL_HDFS_JNI_TOP, LL_DEBUG,
                "Preserve FramePointer Option: %s",
                jvm_options[numJVMOptions].optionString);
        numJVMOptions++;
    }    
      ...
      if (debugPort > 0)
        {
          ...
          if (debugPort_ % 1000 == 0)
            debugPort_ += (cliGlobals->myPin() % 1000);
...
JOI_RetCode JavaObjectInterface::initJVM(LmJavaOptions *options)
{
  ...
  {
     lJavaClass =  (jclass)jenv_->FindClass("org/apache/hadoop/hbase/coprocessor/transactional/lock/exception/LockTimeOutException");
     if (lJavaClass != NULL)
     {
  ...
  if (gDeadLockExceptionClass == NULL)
    {
       lJavaClass =  (jclass)jenv_->FindClass("org/apache/hadoop/hbase/coprocessor/transactional/lock/exception/DeadLockException");
       if (lJavaClass != NULL)
       {
  ...
  if (gLockCancelOperationExceptionClass == NULL)
    {
      lJavaClass =  (jclass)jenv_->FindClass("org/apache/hadoop/hbase/coprocessor/transactional/lock/exception/LockCancelOperationException");
      if (lJavaClass != NULL)
        {
  ...
  {
      lJavaClass = (jclass)jenv_->FindClass("org/apache/hadoop/hbase/coprocessor/transactional/lock/exception/LockNotEnoughResourcsException");
      if (lJavaClass != NULL)
      {
...
// 
//////////////////////////////////////////////////////////////////////////////
JOI_RetCode JavaObjectInterface::init(const char      *className,
                                      jclass          &javaClass,
...
void JavaObjectInterface::logError(std::string &cat, const char* file, int line, const char *methodName, const char *tableName, const char *error_msg)
{
  if (tableName != NULL)
      QRLogger::log(cat, LL_ERROR, "Java exception in File:%s, Line:%d Method:%s TableName:%s %s", file, line, methodName, tableName, error_msg);
  else
      QRLogger::log(cat, LL_ERROR, "Java exception in File:%s, Line:%d Method:%s TableName:NA %s", file, line, methodName, error_msg);
}
                                                   ...
                                                   NABoolean noDetails)
{
   ...
   if ((illArgIndex != std::string::npos))
       illArgIndex = error_msg.index("Connection is null or closed.");

   if ((rsseIndex != std::string::npos) || (nsreIndex != std::string::npos) 
	  || (nrheIndex != std::string::npos) || (fseIndex != std::string::npos)
          || (cceIndex != std::string::npos) || (rsaeIndex != std::string::npos)
          || (mnreIndex != std::string::npos) || (illArgIndex != std::string::npos)) {
       hbaseClient = cliGlobals->getHBaseClient();
       if (hbaseClient != NULL && hbaseClient->isInitialized())
          hbaseClient->reconnectHBase();
   }
   return TRUE;
}
                                                       ...
                                                       NABoolean noDetails)
{
    ...
    if (gOOMErrorClass != NULL && jenv_->IsInstanceOf(a_exception, gOOMErrorClass) == JNI_TRUE)
       killProcess = TRUE; 
    jstring msg_obj =
