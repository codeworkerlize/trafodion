...
SavepointState ExTransaction::getSavepointState()
{
  return savepointStack_->getCurrentSvptState();
}

NABoolean ExTransaction::savepointInProgress()
{
  return (savepointStack_->getCurrentSvptState() != SP_SPNONE);
}
...
short ExTransaction::beginTransaction()
{
  ...
  if (costTh >= 0)
  {
    ...
    if (timeCost >= costTh)
      QRWARN("beginTransaction PID %d txID %ld TC %ld", pid, transid_, timeCost);
  }
...
short ExTransaction::commitTransaction()
{
  ...
  if (costTh >= 0)
  {
    ...
    timeCost = (JULIANTIMESTAMP() / 1000) - timeCost;
    if (timeCost >= costTh)
      QRWARN("commitTransaction PID %d txID %ld TC %ld", pid, transactionid, timeCost);
  }
...
Int64 ExTransaction::getSavepointIdWithFlag()
{
  return getSavepointId();
}

Int64 ExTransaction::getPSavepointIdWithFlag()
{
  return getPSavepointId();
}

Int64 ExTransaction::getPPSavepointIdWithFlag()
{
  return getPPSavepointId();
}
...
{
  if (xnInProgress() &&
      NOT implicitSavepointInProgress())
    {
        implicitSavepointId_ =  1 + (getStackLastSvptId() > implicitSavepointId_ ?
...
short ExTransaction::commitSavepoint(Int64 savepointId, Int64 parentSvptId)
{
  ...
  if (getSavepointState() != SP_TO_COMMIT)
    return 0;
...
short ExTransaction::commitImplicitSavepoint(Int64 implicitSvptId, Int64 parentSvptId)
{
  ...
  if (implicitSavepointState_ != SP_TO_COMMIT)
    return 0;
...
short ExTransaction::rollbackSavepoint(Int64 savepointId)
{
  ...
  if (getSavepointState() != SP_TO_ROLLBACK)
    return 0;
...
short ExTransaction::rollbackToSavepoint(NAString svptName)
{
  return savepointStack_->popToSvpt(svptName);
}
...
short ExTransaction::rollbackImplicitSavepoint(Int64 implicitSvptId)
{
    ...
    if (implicitSavepointState_ != SP_TO_ROLLBACK)
      return 0;
...
{
  return (getTransMode()->getAutoCommitSavepointOn() &&
          NOT GETISPROPAGATETX());
}

NABoolean ExTransaction::getIsPropropagateTX()
{
  return GETISPROPAGATETX();
}
...
short ExTransTcb::work()
{
  ...
  while (1) {
    ...
    if ((ta->getSavepointState() == SP_GENERATED)
         && (ta->getStackSavepointName() == DEFAULT_SAVEPOINT_NAME)
         && (transTdb().coverageSavePoint())
         && (transTdb().transType_ == BEGIN_SAVEPOINT_)
	...
	// is issued.
      ...
      case COMMIT_: {
        ...
        {
          if (ta->xnInProgress())
            ta->rollbackStatement(); // have to roll it back or we risk inconsistencies
          handleErrors(pentry_down, NULL, 
             (ExeErrorCode)(-CLI_CONFLICTING_CONCURRENT_DDL));
          break;
        }
        ...
        if (cliGlobals->currContext()->ddlStmtsExecuted())
          {
            ...
            ExSqlComp::ReturnStatus cmpStatus =
              cliGlobals->currContext()->sendXnMsgToArkcmp
              (NULL, 0,
               EXSQLCOMP::DDL_COMMIT_SHARED_CACHE,
               diagsArea);
            if (cmpStatus == ExSqlComp::ERROR)
              {
      ...
      case BEGIN_SAVEPOINT_: {
        ...
        {
          handleErrors(pentry_down, NULL, 
             (ExeErrorCode)(-EXE_SAVEPOINT_NEED_USER_XN));
          break;
        }
        ...
        if (CmpCommon::getDefault(TRAF_SAVEPOINTS) == DF_OFF)
          {
            handleErrors(pentry_down, NULL, 
                         (ExeErrorCode)(-EXE_SAVEPOINT_OPERATION_ERROR));
            break;
          }
        if (ta->getSavepointState() == SP_GENERATED &&
          ...
          {
            handleErrors(pentry_down, NULL,
                         (ExeErrorCode)(-EXE_BEGIN_SAVEPOINT_ERROR));
            break;
          }

        NAString svptName(DEFAULT_SAVEPOINT_NAME);
        if (transTdb().hasSavepointName_)
          svptName = transTdb().savepointName_;
      ...
      case COMMIT_SAVEPOINT_: {
        if (NOT ta->xnInProgress())
         ...
         if (CmpCommon::getDefault(TRAF_SAVEPOINTS) == DF_OFF  &&
             ta->getSavepointState() != SP_GENERATED)
           {
             handleErrors(pentry_down, NULL, 
                          (ExeErrorCode)(-EXE_SAVEPOINT_OPERATION_ERROR));
             break;
        ...
        if (ta->getSavepointState() != SP_GENERATED)
          {
            handleErrors(pentry_down, NULL, 
                         (ExeErrorCode)(-EXE_COMMIT_SAVEPOINT_ERROR));
            break;
          }
        ...
        if (cliGlobals->currContext()->ddlStmtsInSPExecuted())
        {
            ...
            cmpStatus = cliGlobals->currContext()->sendXnMsgToArkcmp(
                (char*)&xnOper,
                sizeof(xnOper),
                EXSQLCOMP::DDL_NATABLE_INVALIDATE,
                diagsArea
        ...
        if (transTdb().coverageSavePoint()
             && (transTdb().transType_ != COMMIT_SAVEPOINT_))
          continue;
        ...
        if (ta->getSavepointState() != SP_GENERATED)
          {
            if (transTdb().hasSavepointName_)
            {
              ...
              *diagsArea << DgSqlCode(-EXE_SAVEPOINT_NAME_NOT_FOUND)
                         << DgString0(transTdb().savepointName_);
            ...
            {
              handleErrors(pentry_down, NULL,
                          (ExeErrorCode)(-EXE_ROLLBACK_SAVEPOINT_ERROR));
              break;
            }
          }
