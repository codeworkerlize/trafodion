...
void OversubscriptionInfo::setFromJavaArray(JNIEnv *jenv,
                                            jintArray &javaArray)
{
  jboolean isCopy;
  ...
  for (int i=0; i<numMeasures; i++)
    measures_[i] = arrayElems[i];
  
  if (isCopy == JNI_TRUE)
     jenv->ReleaseIntArrayElements(javaArray, arrayElems, JNI_ABORT);
}

bool OversubscriptionInfo::addDiagnostics(ComDiagsArea *diags,
  ...
  if (generateError || generateWarning)
    {
      ...
      *diags << DgSqlCode(generateError ? -1093 : 1092)
             << DgString0(tenantName)
             << DgInt0(measures_[0])
             << DgInt1(measures_[1])
             << DgInt2(measures_[2])
             << DgInt3(measures_[3])
...
static const char* const thErrorEnumStr[] = 
{
  "Preparing parameters for registerTenant()."
...
TH_RetCode TenantHelper_JNI::init()
{
  ...
  if (javaMethodsInitialized_)
    return (TH_RetCode)JavaObjectInterface::init(className, javaClass_, JavaMethods_, (Int32)JM_LAST, javaMethodsInitialized_);
  else
    {
      ...
      JavaMethods_[JM_REGISTER_TENANT].jm_signature = "(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Z[I)Ljava/lang/String;";
      JavaMethods_[JM_REGISTER_TENANT].isStatic     = true;
      JavaMethods_[JM_ALTER_TENANT].jm_name         = "alterTenant";
      JavaMethods_[JM_ALTER_TENANT].jm_signature    = "(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Z[I)Ljava/lang/String;";
      ...
      rc = (TH_RetCode)JavaObjectInterface::init(className, javaClass_, JavaMethods_, (Int32)JM_LAST, javaMethodsInitialized_);
...
// 
//////////////////////////////////////////////////////////////////////////////
TH_RetCode TenantHelper_JNI::registerTenant(const NAString & tenantName,
                                            ...
                                            NAHeap *outHeap)
{
  QRLogger::log(CAT_SQL_HBASE,
                LL_DEBUG,
                "TenantHelper_JNI::registerTenant(%s, %s, %s) called.",
                tenantName.data(),
                sessionLimit.data(),
                defaultSchema.data());
  ...
  if (js_tenantName == NULL ||
      js_serializedNodes == NULL ||
      js_sessionLimit == NULL ||
      js_defaultSchema == NULL ||
      jia_oversub == NULL) 
    {
  ...
  jstring jresult = (jstring) jenv_->CallStaticObjectMethod(
    javaClass_, JavaMethods_[JM_REGISTER_TENANT].methodID, 
...
TH_RetCode TenantHelper_JNI::alterTenant(const NAString & tenantName,
                                         ...
                                         NAHeap *outHeap)
{
  QRLogger::log(CAT_SQL_HBASE,
                LL_DEBUG,
                "TenantHelper_JNI::alterTenant(%s, %s, %s, %s) called.",
                tenantName.data(),
                sessionLimit.data(),
                defaultSchema.data(),
                overrideWarning ? "TRUE" : "FALSE");
  ...
  if (js_tenantName == NULL ||
      js_oldSerializedNodes == NULL ||
      js_newSerializedNodes == NULL ||
      js_sessionLimit == NULL ||
      js_defaultSchema == NULL ||
      jia_oversub == NULL)
    {
  ...
  jstring jresult = (jstring) jenv_->CallStaticObjectMethod(
    javaClass_, JavaMethods_[JM_ALTER_TENANT].methodID,
    js_tenantName, js_oldSerializedNodes, js_newSerializedNodes,
    js_sessionLimit, js_defaultSchema, j_overrideWarning,
    jia_oversub);
...
TH_RetCode TenantHelper_JNI::unregisterTenant(const NAString & tenantName)
{
  QRLogger::log(CAT_SQL_HBASE,
                LL_DEBUG,
                "TenantHelper_JNI::unregisterTenant(%s) called.",
                tenantName.data());
  ...
  jenv_->CallStaticVoidMethod(javaClass_, JavaMethods_[JM_UNREGISTER_TENANT].methodID, 
    js_tenantName);
...
TH_RetCode TenantHelper_JNI::createLocalCGroup(const char *tenantName)
{
  QRLogger::log(CAT_SQL_HBASE,
                LL_DEBUG,
                "TenantHelper_JNI::createLocalCGroup(%s) called.",
                tenantName);
  ...
  jenv_->CallStaticVoidMethod(javaClass_, JavaMethods_[JM_CREATE_LOCAL_CGROUP].methodID, 
    js_tenantName);
