  ...
  step_(INITIAL_)
{
  ...
  if(exeUtilTdb().scanExpr_)
   exeUtilTdb().scanExpr_->fixup(0, getExpressionMode(), this,  space, heap, FALSE, glob); 
...
short ExExeUtilConnectbyTcb::emitRow(ExpTupleDesc * tDesc, int level, int isleaf, int iscycle, connectByStackItem *vi , NABoolean chkForStartWith)
{
  ...
  if(chkForStartWith == TRUE)
  {
     ...
     if(exeUtilTdb().hasDynParamsInStartWith_ ==TRUE )
     {
	   ...
	   .setDataPointer((char*)data_);
         evalRetCode =
	   exeUtilTdb().startwith_expr_->eval(pentry_down->getAtp(),workAtp_);
        }
      }	 
...
void ExExeUtilConnectbyTcb::replaceStartWithStringWithValue(Queue * vals, const NAString sqlstr, char *out, int newsize)
{
   //search for ?
...
short ExExeUtilConnectbyTcb::work()
{
  ...
  if(exeUtilTdb().hasStartWith_ == TRUE)
    {
      if(exeUtilTdb().hasDynParamsInStartWith_!= TRUE) 
        sprintf(q1,"SELECT %s , cast (%s as VARCHAR(60) ), * , cast( 1 as INT) FROM %s WHERE %s ;" ,(exeUtilTdb().parentColName_).data() , (exeUtilTdb().pathColName_).data(), (exeUtilTdb().connTableName_).data() ,(exeUtilTdb().startWithExprString_).data() );
      else
        {sprintf(q1,"SELECT %s , cast (%s as VARCHAR(60) ), * , cast( 1 as INT) FROM %s " ,(exeUtilTdb().parentColName_).data() , (exeUtilTdb().pathColName_).data(), (exeUtilTdb().connTableName_).data() );
       }
      nq11 = q1;
    }
  else
    {
      sprintf(q1,"SELECT %s ,  cast (%s as VARCHAR(60) ) , * , cast(1 as INT)  FROM %s WHERE %s is null ;" ,(exeUtilTdb().parentColName_).data(), (exeUtilTdb().pathColName_).data(), (exeUtilTdb().connTableName_).data(), (exeUtilTdb().childColName_).data());
      nq11 = q1;
    }   
  }
...
nq21cnt_=0;
       ...
       case EVAL_INPUT_:
         if(exeUtilTdb().hasDynParamsInStartWith_ == TRUE)
         {
           workAtp_->getTupp(exeUtilTdb().dynParamTuppIndex)
             .setDataPointer(inputDynParamBuf_);
           ex_expr::exp_return_type exprRetCode =
             exeUtilTdb().inputExpr()->eval(pentry_down->getAtp(), workAtp_);
       ...
       case EVAL_START_WITH_:
        {
         ...
         else
         {
		   ...
		   connectByStackItem *it = new(getHeap()) connectByStackItem(getHeap()); //delete when seedQueue_ was destroyed
		   cliInterface()->getPtrAndLen(1, ptr, len);
		   cliInterface()->getAttributes(1, FALSE,
                                                 fsDatatype, length, vcIndLen,
                                                 &indOffset, &varOffset);
      ...
      case DO_CONNECT_BY_:
       ...
       for(int i=currSeedNum_; i< seedNum; )  {
         sprintf(q1,"SELECT %s ,cast (%s as VARCHAR(60) ) , *, cast( %d as INT) FROM %s WHERE " ,
                    (exeUtilTdb().parentColName_).data(), (exeUtilTdb().pathColName_).data(), currLevel_+1,(exeUtilTdb().connTableName_).data() );
         nq21 = q1;
       ...
       case NEXT_ROOT_:
         {
            currRootId_++;
            currLevel_ = 1;
            currQueue_ = getCurrQueue(currRootId_, seedQueue_); //in fact, this is the queue for all level 1 seeds for this root
            currArray[0] = currQueue_;  
            ...
            {
              Queue* currQueue = new(getHeap()) Queue(getHeap()) ; //this is the level 2 queue, empty now, will fill in in nq21 run
