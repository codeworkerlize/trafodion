...
Allocator::Allocator(ByteCount bufferSize, UInt32 memoryQuotaMB, BufferCount nBuffers, BufferCount reserve,
                     NAMemory *heap, ExExeStmtGlobals *exeGlobals, bool yieldQuota)
    : maxAllocated_(0),
      memAllocated_(0),
      quota_(0),
      exeGlobals_(exeGlobals),
      heap_(heap),
      bufferSize_(bufferSize),
      nInitial_(nBuffers),
      nReserve_(reserve),
      reuseList_(heap),
      yieldQuota_(yieldQuota) {
  quotaInitial_ = memoryQuotaMB * ONE_MEGABYTE;

  if (quotaInitial_) {
    ex_assert(((bufferSize_ * nInitial_) <= quotaInitial_), "inconsistent constructor arguments");
  }
...
Allocator::~Allocator(void) { deallocateAll(); }

void Allocator::adjust(BufferCount bufferDemand) {
...
char *Allocator::allocate(bool failureIsFatal) {
...
void Allocator::deallocate(char *&buffer) {
  if (buffer) {
...
void Allocator::deallocateAll(void) {
  if (exeGlobals_) {
    ...
    ByteCount yieldMB = static_cast<ByteCount>(yieldBytes / ONE_MEGABYTE);
#else
    ByteCount yieldMB = (ByteCount)(yieldBytes / ONE_MEGABYTE);
#endif
    if (yieldMB) {
...
char *Allocator::getBuffer(bool useReserve) {
  char *buffer = NULL;
...
void Allocator::reinitialize(void) {
  if (!memAllocated_) {
    maxAllocated_ = 0;
    acquireMemory();
  }
...
void Allocator::acquireMemory(void) {
...
char *Allocator::allocateBuffer(bool failureIsFatal) {
  char *buffer = NULL;
