...
ex_tcb *ExCatpoundStmtTdb::build(ex_globals *glob) {
  ...
  ExCatpoundStmtTcb *cs = new (glob->getSpace()) ExCatpoundStmtTcb(*this, *left, *right, glob);
...
//////////////////////////////////////////////////////////////////////////////
ExCatpoundStmtTcb::ExCatpoundStmtTcb(const ExCatpoundStmtTdb &tdb, const ex_tcb &left, const ex_tcb &right,
                                     ex_globals *glob)
    : ex_tcb(tdb, 1, glob) {
  ...
  qparent_.down = new (space) ex_queue(ex_queue::DOWN_QUEUE, tdb.queueSizeDown_, criDown, space);
  qparent_.up = new (space) ex_queue(ex_queue::UP_QUEUE, tdb.queueSizeUp_, criUp, space);
...
ExWorkProcRetcode ExCatpoundStmtTcb::workDownLeft() {
  ...
  while (qparent_.down->entryExists(parent2leftx_) && !qleft_.down->isFull()) {
    startLeftChild();
    parent2leftx_++;
  }
  return WORK_OK;
...
void ExCatpoundStmtTcb::startLeftChild() {
  ...
  ExCatpoundStmtPrivateState &pstate = *((ExCatpoundStmtPrivateState *)pentry->pstate);
  ...
  for (; qleft_.up->entryExists(leftupx_); leftupx_++) {
    ...
    switch (lentry->upState.status) {
      case ex_queue::Q_NO_DATA:
        ...
        if (pstate.leftstate_ == CS_STARTED && expectingLeftRows()) {
        ...
        if (pstate.leftstate_ == CS_ERROR || pstate.leftstate_ == CS_CANCELLED) {
          if (qparent_.up->isFull()) return WORK_OK;  // try again later
          ...
          if (pentry->downState.request == ex_queue::GET_NOMORE) {
            ...
            qright_.down->cancelRequestWithParentIndex(lentry->upState.parentIndex);
    ...
    for (; !qright_.up->isEmpty() && !eod; qright_.up->removeHead()) {
      ...
      switch (rentry->upState.status) {
        case ex_queue::Q_NO_DATA:
          ...
          // then raise a warning and stop further execution of CS statements
          if (pstate.rightstate_ == CS_STARTED && expectingRightRows()) {
          ...
          if (pstate.rightstate_ == CS_ERROR || pstate.rightstate_ == CS_CANCELLED) {
        ...
        case ex_queue::Q_SQLERROR:
          ...
          if (pstate.rightstate_ == CS_ERROR || pstate.rightstate_ == CS_CANCELLED) continue;
...
ExWorkProcRetcode ExCatpoundStmtTcb::workCancel() {
  ...
  for (x = qparent_.down->getHeadIndex(); x != parent2leftx_; x++) {
    ...
    if (pentry->downState.request == ex_queue::GET_NOMORE) {
      ExCatpoundStmtPrivateState &pstate = *((ExCatpoundStmtPrivateState *)pentry->pstate);
      if (pstate.leftstate_ == CS_STARTED || pstate.leftstate_ == CS_NOT_EMPTY) {
        // cancel request to left child
        qleft_.down->cancelRequestWithParentIndex(x);
        pstate.leftstate_ = CS_CANCELLED;
      }
      if (pstate.rightstate_ == CS_STARTED || pstate.rightstate_ == CS_NOT_EMPTY) {
...
void ExCatpoundStmtTcb::processCardinalityError(ex_queue_entry *centry) {
  ...
  ComDiagsArea *da =
      ExRaiseSqlError(getGlobals()->getDefaultHeap(), centry, (ExeErrorCode)-EXE_BLOCK_CARDINALITY_VIOLATION);
...
ex_tcb_private_state *ExCatpoundStmtPrivateState::allocate_new(const ex_tcb *tcb) {
  return new (((ex_tcb *)tcb)->getSpace()) ExCatpoundStmtPrivateState((ExCatpoundStmtTcb *)tcb);
