...
ex_tcb *ex_sort_grby_tdb::build(ex_globals *glob) {
  ...
  if ((isRollup()) && (numRollupGroups() > 0))
    sort_grby_tcb = new (glob->getSpace()) ex_sort_grby_rollup_tcb(*this, *child_tcb, glob);
  else
    sort_grby_tcb = new (glob->getSpace()) ex_sort_grby_tcb(*this, *child_tcb, glob);
...
ex_sort_grby_tcb::ex_sort_grby_tcb(const ex_sort_grby_tdb &sort_grby_tdb,
                                   ...
                                   ex_globals *glob)
    : ex_tcb(sort_grby_tdb, 1, glob), pool_(NULL) {
  ...
  pool_ = new (space) ExSimpleSQLBuffer(sort_grby_tdb.numBuffers_, sort_grby_tdb.bufferSize_, recLen(), space);
  ...
  if (aggrExpr()) (void)aggrExpr()->fixup(0, getExpressionMode(), this, space, heap, glob->computeSpace(), glob);

  // fixup move expression
  if (moveExpr()) (void)moveExpr()->fixup(0, getExpressionMode(), this, space, heap, glob->computeSpace(), glob);

  // fixup grby expression
  if (grbyExpr()) (void)grbyExpr()->fixup(0, getExpressionMode(), this, space, heap, glob->computeSpace(), glob);

  // fixup having expression
  if (havingExpr()) (void)havingExpr()->fixup(0, getExpressionMode(), this, space, heap, glob->computeSpace(), glob);
}
...
short ex_sort_grby_tcb::handleError(sort_grby_step &step, short &rc) {
  ...
  ex_sort_grby_private_state *pstate = (ex_sort_grby_private_state *)pentry_down->pstate;
  ...
  if ((sort_grby_tdb().isNonFatalErrorTolerated()) && (SORT_GRBY_LOCAL_ERROR == step))
...
short ex_sort_grby_tcb::handleFinalize(sort_grby_step &step, short &rc) {
  ex_queue_entry *pentry_down = qparent_.down->getHeadEntry();
  ex_sort_grby_private_state *pstate = (ex_sort_grby_private_state *)pentry_down->pstate;
  ...
  if ((!havingExpr()) || ((havingExpr()) && (evalRetCode == ex_expr::EXPR_TRUE))) {
  ...
  if (step == SORT_GRBY_FINALIZE_CANCEL)
    step = SORT_GRBY_CANCELLED;
  else if (qchild_.up->getHeadEntry()->upState.status != ex_queue::Q_OK_MMORE)
    step = SORT_GRBY_DONE;
  else
...
short ex_sort_grby_tcb::handleDone(sort_grby_step &step, short &rc, NABoolean noAssert) {
  ex_queue_entry *pentry_down = qparent_.down->getHeadEntry();
  ex_sort_grby_private_state *pstate = (ex_sort_grby_private_state *)pentry_down->pstate;
  ...
  if (step != SORT_GRBY_NEVER_STARTED) {
    ex_queue_entry *centry = qchild_.up->getHeadEntry();
    if (NOT noAssert) {
      ex_assert(centry->upState.status == ex_queue::Q_NO_DATA, "ex_sort_grby_tcb::handleDone() expecting Q_NO_DATA");
      ex_assert(centry->upState.parentIndex == qparent_.down->getHeadIndex(),
...
short ex_sort_grby_tcb::work() {
  ...
  for (; (processedInputs_ != tail) && (!qchild_.down->isFull()); processedInputs_++) {
    ...
    ex_assert(pstate->step_ == ex_sort_grby_tcb::SORT_GRBY_EMPTY, "Invalid initial state in ex_sort_grby_tcb::work()");
  ...
  while (1)  // exit via return
  {
    ...
    switch (pstate->step_) {
      case ex_sort_grby_tcb::SORT_GRBY_CANCELLED: {
        if (handleCancel(pstate->step_, rc)) return rc;
      ...
      case ex_sort_grby_tcb::SORT_GRBY_NEW_GROUP: {
        ...
        if ((!grbyExpr()) || ((grbyExpr()) && (centry->upState.status == ex_queue::Q_OK_MMORE))) {
          ...
          if (pool_->getFreeTuple(workAtp_->getTupp(((ex_sort_grby_tdb &)tdb).tuppIndex_)))
          ...
          if (aggrExpr()) {
            if (aggrExpr()->initializeAggr(workAtp_) == ex_expr::EXPR_ERROR) {
              pstate->step_ = SORT_GRBY_LOCAL_ERROR;
              break;
            }
          ...
          if ((!grbyExpr()) && (centry->upState.status == ex_queue::Q_NO_DATA)) {
            if (aggrExpr()->finalizeNullAggr(workAtp_) == ex_expr::EXPR_ERROR) {
              pstate->step_ = SORT_GRBY_LOCAL_ERROR;
              break;
            }
          }
      ...
      case ex_sort_grby_tcb::SORT_GRBY_STARTED: {
        ...
        switch (child_status) {
          case ex_queue::Q_OK_MMORE: {
            ...
            if (grbyExpr()) {
              grbyExprRetCode = grbyExpr()->eval(centry->getAtp(), workAtp_);
            }

            if (grbyExpr() && ((grbyExprRetCode == ex_expr::EXPR_FALSE) || (grbyExprRetCode == ex_expr::EXPR_ERROR))) {
              ...
              if (aggrExpr() && aggrExpr()->perrecExpr()) {
                ...
                retcode = aggrExpr()->perrecExpr()->eval(centry->getAtp(), workAtp_);
                if (retcode == ex_expr::EXPR_OK && pstate->oneRowAggr_ && moveExpr() && !grbyExpr()) {
                  retcode = moveExpr()->eval(centry->getAtp(), workAtp_);
                }
              ...
              if (pstate->step_ != ex_sort_grby_tcb::SORT_GRBY_FINALIZE_CANCEL) qchild_.up->removeHead();
            }
          } break;
      ...
      case SORT_GRBY_LOCAL_ERROR: {
        if (handleError(pstate->step_, rc)) return rc;
      ...
      case ex_sort_grby_tcb::SORT_GRBY_FINALIZE_CANCEL: {
        if (handleFinalize(pstate->step_, rc)) return rc;
      ...
      case ex_sort_grby_tcb::SORT_GRBY_NEVER_STARTED: {
        if (handleDone(pstate->step_, rc)) return rc;
        ...
        ex_assert(pstate->step_ != ex_sort_grby_tcb::SORT_GRBY_EMPTY, "ex_sort_grby_tcb::work() invalid step");
      } break;
...
/////////////////////////////////////////////////////////////
ex_sort_grby_rollup_tcb::ex_sort_grby_rollup_tcb(const ex_sort_grby_tdb &sgby_tdb,
                                                 const ex_tcb &child_tcb,  // child queue pair
                                                 ex_globals *glob)
    : ex_sort_grby_tcb(sgby_tdb, child_tcb, glob), rollupGroupAggrArr_(NULL), step_(SORT_GRBY_EMPTY) {
  rollupGroupAggrArr_ = new (glob->getDefaultHeap()) char *[sort_grby_tdb().numRollupGroups()];
...
short ex_sort_grby_rollup_tcb::processRollupGrbyNulls(Int16 groupNum) {
  ...
  if (grbyData) {
    ...
    ExpTupleDesc *returnRowTD = sort_grby_tdb().getCriDescUp()->getTupleDescriptor(sort_grby_tdb().tuppIndex_);
    ...
    char *workDataPtr = workAtp_->getTupp(sort_grby_tdb().tuppIndex_).getDataPointer();
...
short ex_sort_grby_rollup_tcb::rollupGrbyMoveValue(ex_queue_entry *centry) {
  ...
  char *tempWorkDataPtr = workAtp_->getTupp(sort_grby_tdb().tuppIndex_).getDataPointer();

  for (Int16 i = 0; i < sort_grby_tdb().numRollupGroups(); i++) {
    workAtp_->getTupp(sort_grby_tdb().tuppIndex_).setDataPointer(rollupGroupAggrArr_[i]);
...
short ex_sort_grby_rollup_tcb::rollupAggrEval(ex_queue_entry *centry) {
  if (!aggrExpr() || !aggrExpr()->perrecExpr()) return 0;
...
short ex_sort_grby_rollup_tcb::work() {
  ...
  while (1)  // exit via return
  {
    ...
    switch (step_) {
      ...
      case SORT_GRBY_NEW_GROUP: {
        ...
        if (centry->upState.status == ex_queue::Q_OK_MMORE) {
          ...
          if (aggrExpr()) {
            ...
            char *tempWorkDataPtr = workAtp_->getTupp(sort_grby_tdb().tuppIndex_).getDataPointer();
            workAtp_->getTupp(sort_grby_tdb().tuppIndex_).setDataPointer(rollupGroupAggrArr_[0]);
      ...
      case SORT_GRBY_CHILD_ERROR:
      case SORT_GRBY_LOCAL_ERROR: {
        if (handleError(step_, rc)) return rc;
      ...
      case SORT_GRBY_CANCELLED: {
        if (handleCancel(step_, rc)) return rc;
      ...
      case SORT_GRBY_FINALIZE_CANCEL: {
        ...
        if (handleFinalize(step_, rc)) return rc;
      ...
      case SORT_GRBY_ROLLUP_GROUP_START: {
        ...
        // get space to hold the grouped/aggregated row.
        if (pool_->getFreeTuple(workAtp_->getTupp(sort_grby_tdb().tuppIndex_)))
          return WORK_POOL_BLOCKED;  // couldn't allocate, try again.
      ...
      case SORT_GRBY_NEVER_STARTED: {
        if (handleDone(step_, rc, TRUE /*no child entry assertion check*/)) return rc;
