		...
		if (controlTdb().getType() == DEFAULT_)
		  {
			...
			if (strcmp(value[2], "ON") == 0)
		      ...
		      {
			if (strcmp(value[2], "ON") == 0)
			  currContext->getSessionDefaults()->setTrafciProcess(TRUE);
		      }
			...
			if (stricmp(value[2], "OFF") == 0)
		    ...
		    else if (strcmp(value[1], "SCHEMA") == 0)
		      {
                  ...
                  if (catalog != NULL && schemaName.getCatalogNamePart().isValid()) {
                      currContext->getSessionDefaults()->
                          setCatalog(catalog, strlen(catalog));
                  }
                  currContext->getSessionDefaults()->
                      setSchema(schema, strlen(schema));
		      }
...
short ExSetSessionDefaultTcb::work()
{
  ...
  if ((strcmp(defaultName, "ALTPRI_MASTER") != 0) &&
      ...
      (strcmp(defaultName, "SKIP_END_VALID_DDL_CHECK") != 0) &&
