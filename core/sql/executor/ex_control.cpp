...
///////////////////////////////////////////////////////////////
ExControlTcb::ExControlTcb(const ExControlTdb &control_tdb, ex_globals *glob) : ex_tcb(control_tdb, 1, glob) {
  ...
  qparent_.down =
      new (space) ex_queue(ex_queue::DOWN_QUEUE, control_tdb.queueSizeDown_, control_tdb.criDescDown_, space);
  ...
  qparent_.up = new (space) ex_queue(ex_queue::UP_QUEUE, control_tdb.queueSizeUp_, control_tdb.criDescUp_, space);
}

ExControlTcb::~ExControlTcb() { freeResources(); };
...
#define MAXVALS 3  // value(1 to 3)
  ...
  Int32 cmpStatus = 0;
  {
    ...
    if (currCtxt->getSessionDefaults()->callEmbeddedArkcmp() && currCtxt->isEmbeddedArkcmpInitialized() &&
      ...
      cmpStatus = CmpCommon::context()->compileDirect((char *)data, dataLen, arkcmpHeap, SQLCHARSETCODE_UTF8,
                                                      EXSQLCOMP::SQLTEXT_STATIC_COMPILE, dummyReply, dummyLen,
                                                      currCtxt->getSqlParserFlags(), NULL, 0, da);
      if (cmpStatus != 0) {
        char *emsText = new (getHeap()) char[dataLen + 120];
        str_sprintf(emsText, "Set control to embedded arkcmp failed, return code %d, data: %s", cmpStatus, data);
        SQLMXLoggingArea::logExecRtInfo(__FILE__, __LINE__, emsText, 0);
        ExHandleArkcmpErrors(qparent_, pentry_down, 0, getGlobals(), da);

        // da->clear();
        getHeap()->deallocateMemory(emsText);
        if (da != NULL) da->decrRefCount();
      } else
    ...
    if (!currCtxt->getSessionDefaults()->callEmbeddedArkcmp() ||
      ...
      for (short i = 0; i < currCtxt->getNumArkcmps(); i++) {
        ...
        if (currCtxt->sendSettingsToCompiler(i)) {
          ...
          ExSqlComp::ReturnStatus cmpStatus = cmp->sendRequest(EXSQLCOMP::SQLTEXT_STATIC_COMPILE, data, dataLen, TRUE,
                                                               NULL, SQLCHARSETCODE_UTF8, TRUE /*resend, if needed*/
          );

          if (cmpStatus == ExSqlComp::SUCCESS) cmpStatus = cmp->getReply(dummyReply, dummyLen);
          if (dummyReply != NULL) cmp->getHeap()->deallocateMemory((void *)dummyReply);
          ...
          if (cmpStatus == ExSqlComp::ERROR) {
            ...
            if (((*(cmp->getDiags())).contains(-2050) || (*(cmp->getDiags())).contains(-2055)) &&
                (cmp->getVersion() < COM_VERS_COMPILER_VERSION))
              cmp->getDiags()->clear();
            else
              ExHandleArkcmpErrors(qparent_, pentry_down, 0, getGlobals(), cmp->getDiags());
          } else
            saveControl = TRUE;
    ...
    if (saveControl) {
      ...
      currCtxt->getControlArea()->addControl(
          controlTdb().getType(), controlTdb().reset_, buf, usedlen, value[1], len[1], value[2], len[2], value[3],
          len[3], controlTdb().getControlActionType(), ExControlEntry::UPON_ALL, controlTdb().isNonResettable());
      ...
      if (((currCtxt->getSqlParserFlags() & 0x400000) != 0) && (controlTdb().getType() == DEFAULT_) &&
          (controlTdb().reset_ == -1) && (value[1] != NULL) && (len[1] == 0) && (value[2] != NULL) &&
      ...
      if (controlTdb().getType() == DEFAULT_) {
        if (strcmp(value[1], "AUTO_QUERY_RETRY") == 0) {
          ...
          currContext->getSessionDefaults()->setCatalog(value[2], strlen(value[2]));
        } else if (strcmp(value[1], "DBTR_PROCESS") == 0) {
          if (strcmp(value[2], "ON") == 0) currContext->getSessionDefaults()->setDbtrProcess(TRUE);
        } else if (strcmp(value[1], "IS_SQLCI") == 0) {
          if (strcmp(value[2], "ON") == 0) currContext->getSessionDefaults()->setMxciProcess(TRUE);
        } else if (strcmp(value[1], "IS_TRAFCI") == 0) {
          if (strcmp(value[2], "ON") == 0) currContext->getSessionDefaults()->setTrafciProcess(TRUE);
          ...
          ComSchemaName schemaName(value[2]);
          const char *catalog = schemaName.getCatalogNamePart().getInternalName().data();
          const char *schema = schemaName.getSchemaNamePart().getInternalName().data();
          if (catalog != NULL && schemaName.getCatalogNamePart().isValid()) {
            currContext->getSessionDefaults()->setCatalog(catalog, strlen(catalog));
          }
          currContext->getSessionDefaults()->setSchema(schema, strlen(schema));
          ...
          currContext->getSessionDefaults()->setUEL(value[2], strlen(value[2]));
          ...
          char *prefix = value[2];
          if (prefix != NULL) currContext->getSessionDefaults()->setExSMTraceFilePrefix(prefix, strlen(prefix));
          ...
          if ((strcmp(value[2], "TRUE") == 0) || (strcmp(value[2], "ON") == 0)) {
            currContext->getSessionDefaults()->setCallEmbeddedArkcmp(TRUE);
          } else if ((strcmp(value[2], "FALSE") == 0) || (strcmp(value[2], "OFF") == 0)) {
            currContext->getSessionDefaults()->setCallEmbeddedArkcmp(FALSE);
          }
        } else if (strcmp(value[1], "ESP_IDLE_TIMEOUT") == 0) {
          int lvl = (int)strtoul(value[2], NULL, 10);
          currContext->getSessionDefaults()->setEspIdleTimeout(lvl);
        } else if (strcmp(value[1], "ONLINE_BACKUP_TIMEOUT") == 0) {
          int lvl = (int)strtoul(value[2], NULL, 10);
          currContext->getSessionDefaults()->setOnlineBackupTimeout(lvl);
        } else if (strcmp(value[1], "COMPILER_IDLE_TIMEOUT") == 0) {
          int lvl = (int)strtoul(value[2], NULL, 10);
          currContext->getSessionDefaults()->setCompilerIdleTimeout(lvl);
        } else if (strcmp(value[1], "MULTI_THREADED_ESP") == 0) {
          if ((strcmp(value[2], "ON") == 0) || (strcmp(value[2], "TRUE") == 0))
            currContext->getSessionDefaults()->setMultiThreadedEsp(TRUE);
          else
            currContext->getSessionDefaults()->setMultiThreadedEsp(FALSE);
        }
      }
    }
  }
...
short ExSetSessionDefaultTcb::work() {
  // if no parent request, return
  if (getParentQueue().down->isEmpty()) return WORK_OK;

  // if no room in up queue, won't be able to return data/status.
  // Come back later.
  if (getParentQueue().up->isFull()) return WORK_OK;
  ...
  IpcEnvironment *ipcEnv =
      getGlobals()->castToExExeStmtGlobals()->castToExMasterStmtGlobals()->getCliGlobals()->getEnvironment();
  ...
  if ((strcmp(defaultName, "ALTPRI_MASTER") != 0) && (strcmp(defaultName, "ALTPRI_MASTER_SEQ_EXE") != 0) &&
      (strcmp(defaultName, "ALTPRI_ESP") != 0) && (strcmp(defaultName, "IS_DB_TRANPORTER") != 0) &&
      (strcmp(defaultName, "AQR_ENTRIES") != 0) && (strcmp(defaultName, "MASTER_PRIORITY") != 0) &&
      (strcmp(defaultName, "MASTER_PRIORITY_DELTA") != 0) && (strcmp(defaultName, "ESP_PRIORITY") != 0) &&
      (strcmp(defaultName, "ESP_PRIORITY_DELTA") != 0) && (strcmp(defaultName, "ESP_FIXUP_PRIORITY") != 0) &&
      (strcmp(defaultName, "ESP_FIXUP_PRIORITY_DELTA") != 0) && (strcmp(defaultName, "ESP_ASSIGN_DEPTH") != 0) &&
      (strcmp(defaultName, "ESP_ASSIGN_TIME_WINDOW") != 0) && (strcmp(defaultName, "ESP_STOP_IDLE_TIMEOUT") != 0) &&
      (strcmp(defaultName, "ESP_IDLE_TIMEOUT") != 0) && (strcmp(defaultName, "COMPILER_IDLE_TIMEOUT") != 0) &&
      (strcmp(defaultName, "ESP_INACTIVE_TIMEOUT") != 0) && (strcmp(defaultName, "ESP_RELEASE_WORK_TIMEOUT") != 0) &&
      (strcmp(defaultName, "MAX_POLLING_INTERVAL") != 0) && (strcmp(defaultName, "PERSISTENT_OPENS") != 0) &&
      (strcmp(defaultName, "ESP_FREEMEM_TIMEOUT") != 0) && (strcmp(defaultName, "ESP_CLOSE_ERROR_LOGGING") != 0) &&
      (strcmp(defaultName, "INTERNAL_FORMAT_IO") != 0) && (strcmp(defaultName, "ISO_MAPPING") != 0) &&
      (strcmp(defaultName, "MXCMP_PRIORITY") != 0) && (strcmp(defaultName, "MXCMP_PRIORITY_DELTA") != 0) &&
      (strcmp(defaultName, "CLI_BULKMOVE") != 0) && (strcmp(defaultName, "SQL_SESSION") != 0) &&
      (strcmp(defaultName, "SQL_USER") != 0) && (strcmp(defaultName, "ROWSET_ATOMICITY") != 0) &&
      (strcmp(defaultName, "RTS_TIMEOUT") != 0) && (strncmp(defaultName, "SET_ENVVAR_", strlen("SET_ENVVAR_")) != 0) &&
      (strncmp(defaultName, "RESET_ENVVAR_", strlen("RESET_ENVVAR_")) != 0) &&
      (strcmp(defaultName, "SET_PARSERFLAGS") != 0) && (strcmp(defaultName, "RESET_PARSERFLAGS") != 0) &&
      (strcmp(defaultName, "PARENT_QID") != 0) && (strcmp(defaultName, "PARENT_QID_SYSTEM") != 0) &&
      (strcmp(defaultName, "CANCEL_ESCALATION_INTERVAL") != 0) &&
      (strcmp(defaultName, "CANCEL_ESCALATION_MXOSRVR_INTERVAL") != 0) &&
      (strcmp(defaultName, "CANCEL_ESCALATION_SAVEABEND") != 0) && (strcmp(defaultName, "CANCEL_QUERY_ALLOWED") != 0) &&
      (strcmp(defaultName, "CANCEL_UNIQUE_QUERY") != 0) && (strcmp(defaultName, "CANCEL_LOGGING") != 0) &&
      (strcmp(defaultName, "SUSPEND_LOGGING") != 0) && (strcmp(defaultName, "STATISTICS_VIEW_TYPE") != 0) &&
      (strcmp(defaultName, "SKIP_END_VALID_DDL_CHECK") != 0) && (strcmp(defaultName, "RECLAIM_MEMORY_AFTER") != 0) &&
      (strcmp(defaultName, "RECLAIM_FREE_MEMORY_RATIO") != 0) && (strcmp(defaultName, "CALL_EMBEDDED_ARKCMP") != 0) &&
      (strcmp(defaultName, "RECLAIM_FREE_PFS_RATIO") != 0) && (strcmp(defaultName, "EXSM_TRACE_FILE_PREFIX") != 0) &&
      (strcmp(defaultName, "EXSM_TRACE_LEVEL") != 0) && (strcmp(defaultName, "OSIM") != 0) &&
      (strcmp(defaultName, "STATS") != 0)) {
    ExHandleErrors(qparent_, pentry_down, 0, getGlobals(), NULL, (ExeErrorCode)(-EXE_INVALID_SESSION_DEFAULT), NULL,
                   defaultName);
    goto all_ok;
  }

  if ((strcmp(defaultName, "AQR_ENTRIES") == 0) || (strcmp(defaultName, "CLI_BULKMOVE") == 0) ||
      (strcmp(defaultName, "SQL_SESSION") == 0) || (strcmp(defaultName, "SQL_USER") == 0) ||
      (strcmp(defaultName, "ALTPRI_MASTER") == 0) || (strcmp(defaultName, "ALTPRI_MASTER_SEQ_EXE") == 0) ||
      (strcmp(defaultName, "ALTPRI_ESP") == 0) || (strcmp(defaultName, "IS_DB_TRANSPORTER") == 0) ||
      (strcmp(defaultName, "INTERNAL_FORMAT_IO") == 0) || (strcmp(defaultName, "ISO_MAPPING") == 0) ||
      ...
      (strcmp(defaultName, "CANCEL_ESCALATION_SAVEABEND") == 0) || (strcmp(defaultName, "CANCEL_QUERY_ALLOWED") == 0) ||
      (strcmp(defaultName, "CANCEL_UNIQUE_QUERY") == 0) || (strcmp(defaultName, "CANCEL_LOGGING") == 0) ||
      (strcmp(defaultName, "SUSPEND_LOGGING") == 0) || (strcmp(defaultName, "CALL_EMBEDDED_ARKCMP") == 0) ||
      (strcmp(defaultName, "STATISTICS_VIEW_TYPE") == 0) || (strcmp(defaultName, "SKIP_END_VALID_DDL_CHECK") == 0) ||
      (strcmp(defaultName, "EXSM_TRACE_LEVEL") == 0) || (strcmp(defaultName, "EXSM_TRACE_FILE_PREFIX") == 0) ||
  ...
  if (strcmp(defaultName, "MASTER_PRIORITY") == 0) {
    ...
    currContext->getSessionDefaults()->setEspFixupPriority(defaultValueAsLong);
  } else if (strcmp(defaultName, "ESP_FIXUP_PRIORITY_DELTA") == 0) {
    currContext->getSessionDefaults()->setEspFixupPriorityDelta(defaultValueAsLong);
  } else if (strcmp(defaultName, "ESP_ASSIGN_DEPTH") == 0) {
    currContext->getSessionDefaults()->setEspAssignDepth(defaultValueAsLong);
    ...
    currContext->getSessionDefaults()->setEspStopIdleTimeout(defaultValueAsLong);
  } else if (strcmp(defaultName, "ESP_IDLE_TIMEOUT") == 0) {
    currContext->getSessionDefaults()->setEspIdleTimeout(defaultValueAsLong);
  } else if (strcmp(defaultName, "ONLINE_BACKUP_TIMEOUT") == 0) {
    currContext->getSessionDefaults()->setOnlineBackupTimeout(defaultValueAsLong);
  } else if (strcmp(defaultName, "COMPILER_IDLE_TIMEOUT") == 0) {
    currContext->getSessionDefaults()->setCompilerIdleTimeout(defaultValueAsLong);
  } else if (strcmp(defaultName, "ESP_INACTIVE_TIMEOUT") == 0) {
    currContext->getSessionDefaults()->setEspInactiveTimeout(defaultValueAsLong);
  } else if (strcmp(defaultName, "ESP_RELEASE_WORK_TIMEOUT") == 0) {
    currContext->getSessionDefaults()->setEspReleaseWorkTimeout(defaultValueAsLong);
  }

  else if (strcmp(defaultName, "MAX_POLLING_INTERVAL") == 0) {
    if (defaultValueAsLong < 0 || defaultValueAsLong > 1000) {
      ExHandleErrors(qparent_, pentry_down, 0, getGlobals(), NULL, (ExeErrorCode)(-EXE_INVALID_SESSION_DEFAULT), NULL,
                     defaultName);
      goto all_ok;
    } else {
      currContext->getSessionDefaults()->setMaxPollingInterval(defaultValueAsLong);
      ipcEnv->setMaxPollingInterval(defaultValueAsLong);
    }
  } else if (strcmp(defaultName, "PERSISTENT_OPENS") == 0) {
    if (defaultValueAsLong < 0 || defaultValueAsLong > 2) {
      ExHandleErrors(qparent_, pentry_down, 0, getGlobals(), NULL, (ExeErrorCode)(-EXE_INVALID_SESSION_DEFAULT), NULL,
                     defaultName);
      goto all_ok;
    } else {
      currContext->getSessionDefaults()->setPersistentOpens(defaultValueAsLong);
    ...
    if (currContext->aqrInfo()->setAQREntriesFromInputStr(defaultValue, strlen(defaultValue))) {
      ExHandleErrors(qparent_, pentry_down, 0, getGlobals(), NULL, (ExeErrorCode)(-EXE_INVALID_SESSION_DEFAULT), NULL,
                     defaultName);
      goto all_ok;
    }
  } else if (strcmp(defaultName, "AQR_EMS_MSG") == 0) {
    if ((strcmp(defaultValue, "ON") == 0) || (strcmp(defaultValue, "ENABLE") == 0))
      currContext->getSessionDefaults()->setAqrEmsEvent(TRUE);
    else if ((strcmp(defaultValue, "OFF") == 0) || (strcmp(defaultValue, "DISABLE") == 0))
    ...
    if (strncmp(defaultValue, "BEGIN", strlen("BEGIN")) == 0) {
      if ((strlen(defaultValue) > strlen("BEGIN")) && (defaultValue[strlen("BEGIN")] == ':')) {
        if (strlen(&defaultValue[strlen("BEGIN") + 1]) > ComSqlId::MAX_SESSION_NAME_LEN) {
          ...
          ExHandleErrors(qparent_, pentry_down, 0, getGlobals(), NULL, (ExeErrorCode)(-EXE_INVALID_SESSION_DEFAULT),
                         NULL, defaultName);
          goto all_ok;
        }
      ...
      *da << DgSqlCode(EXE_CLEANUP_ESP)  // a warning.
      ...
      currContext->endSession(FALSE /*do not cleanup ESPs*/, FALSE /*not esps only*/, FALSE /*do not cleanup opens*/);
    }  // END
    else if (strcmp(defaultValue, "END:CLEANUP_CMPCONTEXT") == 0) {
      currContext->endSession(FALSE /*do not cleanup ESPs*/, FALSE /*not esps only*/, FALSE /*do not cleanup opens*/,
                              TRUE /*cleanup CmpContext*/);
    } else if (strcmp(defaultValue, "END:CLEANUP_ESPS") == 0) {
      currContext->endSession(TRUE /*cleanup ESPs*/, FALSE, /*not esps only*/
                              FALSE /*do not cleanup opens*/);
    }  // END:CLEANUP_ESPS
    else if (strcmp(defaultValue, "END:CLEANUP_ESPS:CLEANUP_CMPCONTEXT") == 0) {
      currContext->endSession(TRUE /*cleanup ESPs*/, FALSE, /*not esps only*/
                              FALSE,                        /*do not cleanup opens*/
                              TRUE /*cleanup CmpContext*/);
    }  // END:CLEANUP_ESPS
      ...
      currContext->endSession(FALSE /*do not cleanup ESPs*/, FALSE, /*not esps only*/
                              TRUE /* cleanup opens*/);
    }  // END:CLEANUP_OPENS
      ...
      currContext->endSession(TRUE /*cleanup ESPs*/, TRUE /*only esps*/, FALSE);
    }  // END:CLEANUP_ESPS
    else if (strcmp(defaultValue, "SWITCH") == 0) {
      currContext->endSession(FALSE /*do not cleanup ESPs*/, FALSE, FALSE /*do not cleanup opens*/);
    ...
    if (isRoot) {
      ...
      if (strlen(defaultValue) > maxLen) invalidInput = TRUE;
    }
  ...
  else if ((strcmp(defaultName, "SET_PARSERFLAGS") == 0) || (strcmp(defaultName, "RESET_PARSERFLAGS") == 0)) {
    ...
    if (rc) {
      ExHandleErrors(qparent_, pentry_down, 0, getGlobals(), NULL, (ExeErrorCode)rc, NULL, NULL);
    }
    ...
    currContext->getSessionDefaults()->setIsoMappingName(defaultValue, strlen(defaultValue));
    ...
    if ((strcmp(defaultValue, "ON") == 0) || (strcmp(defaultValue, "on") == 0) || (strcmp(defaultValue, "ENABLE") == 0))
      currContext->getSessionDefaults()->setCancelEscalationSaveabend(TRUE);
    else if ((strcmp(defaultValue, "OFF") == 0) || (strcmp(defaultValue, "off") == 0) ||
             (strcmp(defaultValue, "DISABLE") == 0))
      currContext->getSessionDefaults()->setCancelEscalationSaveabend(FALSE);
    ...
    if ((strcmp(defaultValue, "ON") == 0) || (strcmp(defaultValue, "on") == 0) || (strcmp(defaultValue, "ENABLE") == 0))
      currContext->getSessionDefaults()->setCancelUniqueQuery(TRUE);
             ...
             (strcmp(defaultValue, "DISABLE") == 0))
      currContext->getSessionDefaults()->setCancelUniqueQuery(FALSE);
  } else if (strcmp(defaultName, "CANCEL_LOGGING") == 0) {
    if ((strcmp(defaultValue, "ON") == 0) || (strcmp(defaultValue, "on") == 0) || (strcmp(defaultValue, "ENABLE") == 0))
      currContext->getSessionDefaults()->setCancelLogging(TRUE);
    else if ((strcmp(defaultValue, "OFF") == 0) || (strcmp(defaultValue, "off") == 0) ||
             (strcmp(defaultValue, "DISABLE") == 0))
      currContext->getSessionDefaults()->setCancelLogging(FALSE);
  } else if (strcmp(defaultName, "SUSPEND_LOGGING") == 0) {
    if ((strcmp(defaultValue, "ON") == 0) || (strcmp(defaultValue, "on") == 0) || (strcmp(defaultValue, "ENABLE") == 0))
      currContext->getSessionDefaults()->setSuspendLogging(TRUE);
    else if ((strcmp(defaultValue, "OFF") == 0) || (strcmp(defaultValue, "off") == 0) ||
             (strcmp(defaultValue, "DISABLE") == 0))
      currContext->getSessionDefaults()->setSuspendLogging(FALSE);
  } else if (strcmp(defaultName, "CANCEL_ESCALATION_INTERVAL") == 0) {
    currContext->getSessionDefaults()->setCancelEscalationInterval(defaultValueAsLong);
  } else if (strcmp(defaultName, "CANCEL_ESCALATION_MXOSRVR_INTERVAL") == 0) {
    currContext->getSessionDefaults()->setCancelEscalationMxosrvrInterval(defaultValueAsLong);
  } else if (strcmp(defaultName, "CALL_EMBEDDED_ARKCMP") == 0) {
    if ((stricmp(defaultValue, "TRUE") == 0) || (stricmp(defaultValue, "ENABLE") == 0))
      currContext->getSessionDefaults()->setCallEmbeddedArkcmp(TRUE);
  ...
  } else if (strcmp(defaultName, "SKIP_END_VALID_DDL_CHECK") == 0) {
    if ((strcmp(defaultValue, "ON") == 0) || (strcmp(defaultValue, "on") == 0))
      currContext->getSessionDefaults()->setSkipEndValidDDLCheck(TRUE);
    else if ((strcmp(defaultValue, "OFF") == 0) || (strcmp(defaultValue, "off") == 0))
      currContext->getSessionDefaults()->setSkipEndValidDDLCheck(FALSE);
  } else if (strcmp(defaultName, "EXSM_TRACE_FILE_PREFIX") == 0) {
    currContext->getSessionDefaults()->setExSMTraceFilePrefix(defaultValue, strlen(defaultValue));
  ...
  if (changeMxcmpPriority) {
    ...
    for (short i = 0; i < currContext->getNumArkcmps(); i++) {
      ExSqlComp::ReturnStatus retStatus = currContext->getArkcmp(i)->changePriority(
          (mxcmpIsDelta ? currContext->getSessionDefaults()->getMxcmpPriorityDelta()
                        : currContext->getSessionDefaults()->getMxcmpPriority()),
          mxcmpIsDelta);
      if (retStatus != ExSqlComp::SUCCESS) {
        ExHandleErrors(qparent_, pentry_down, 0, getGlobals(), NULL, (ExeErrorCode)(-15371), NULL, "MXCMP");
      }
    }
  }

  if (changeEspPriority) {
    IpcPriority p = (espIsDelta ? currContext->getSessionDefaults()->getEspPriorityDelta()
    ...
    short rc = currContext->getCliGlobals()->getEspManager()->changePriorities(p, espIsDelta,
                                                                               true);  // ignore idle timed out esps
    if (rc) {
      char errMsg[100];
      ...
      ComDiagsArea *da = ExRaiseSqlError(getGlobals()->getDefaultHeap(), up_entry, (ExeErrorCode)(15371), NULL, errMsg);
      up_entry->setDiagsArea(da);
    }
  }
...
short ExShowEnvvarsTcb::work() {
  ...
  NAList<SessionEnvvar> *sessionEnvvars = currContext->getSessionDefaults()->sessionEnvvars();
  ...
  while (1) {
    switch (step_) {
      ...
      case RETURN_HEADER_: {
        ...
        moveRowToUpQueue(showTdb().tuppIndex(), "Current SESSION DEFAULTs");
      ...
      case RETURNING_VALUE_: {
        ...
        char *formattedStr = new (getHeap()) char[showTdb().outputRowlen()];

        str_sprintf(formattedStr, "%s\t%s", se.envvarName(), se.envvarValue());
...
ExControlPrivateState::ExControlPrivateState(const ExControlTcb * /*tcb*/) { step_ = ExControlTcb::EMPTY_; }
