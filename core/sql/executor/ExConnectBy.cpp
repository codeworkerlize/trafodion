...
ex_tcb * ExConnectByTdb::build(ex_globals * glob)
{
  ...
  if(tdbCChild_)
    c_child_tcb = tdbCChild_->build(glob); 
  ExConnectByTcb * connectby_tcb = 
    new(glob->getSpace()) ExConnectByTcb(*this,*s_child_tcb, *c_child_tcb, glob);
...
//
ExConnectByTcb::ExConnectByTcb(const ExConnectByTdb & connectby_tdb, 
                         const ex_tcb & s_child_tcb, 
                         const ex_tcb & c_child_tcb, 
			 ex_globals *glob
			 ) : ex_tcb( connectby_tdb, 1, glob),
                             step_(INITIAL_)
{
  ...
  pool_ = new(space) sql_buffer_pool(connectby_tdb.numBuffers_,
				     connectby_tdb.bufferSize_,
				     space);

  // Allocate the queue to communicate with parent
  qparent_.down = new(space) ex_queue(ex_queue::DOWN_QUEUE,
				      connectby_tdb.queueSizeDown_,
				      connectby_tdb.criDescDown_,
				      space);
  // Allocate the private state in each entry of the down queue
  ExConnectByPrivateState p(this);
  qparent_.down->allocatePstate(&p, this);
  qparent_.up = new(space) ex_queue(ex_queue::UP_QUEUE,
				    connectby_tdb.queueSizeUp_,
				    connectby_tdb.criDescUp_,
				    space);
  ...
  if (connectby_tdb.leftMoveExpr_)
  {
     ...
     rc = connectby_tdb.leftMoveExpr_->fixup(0, getExpressionMode(), this,  space, heap, 
                                        FALSE, glob);
  }
  if (connectby_tdb.rightMoveExpr_)
  {
     ex_expr::exp_return_type rc;
     rc = connectby_tdb.rightMoveExpr_->fixup(0, getExpressionMode(), this,  space, heap, 
                                        FALSE, glob);
  }
  if (connectby_tdb.condExpr_)
    (void) connectby_tdb.condExpr_->fixup(0, getExpressionMode(), this,
			     space, heap, FALSE, glob);

  if (connectby_tdb.priorCondExpr_)
    (void) connectby_tdb.priorCondExpr_->fixup(0, getExpressionMode(), this,
			     space, heap, FALSE, glob);

  if (connectby_tdb.priorPredExpr_)
  {
     ex_expr::exp_return_type rc;
     rc = connectby_tdb.priorPredExpr_->fixup(0, getExpressionMode(), this,  space, heap, 
                                        FALSE, glob);
  }
  if (connectby_tdb.priorValMoveExpr1_)
  {
     ex_expr::exp_return_type rc;
     rc = connectby_tdb.priorValMoveExpr1_->fixup(0, getExpressionMode(), this,  space, heap, 
                                        FALSE, glob);
  }
  if (connectby_tdb.priorValMoveExpr2_)
  {
     ex_expr::exp_return_type rc;
     rc = connectby_tdb.priorValMoveExpr2_->fixup(0, getExpressionMode(), this,  space, heap, 
                                        FALSE, glob);
  }
  if (connectby_tdb.leftPathExpr_)
  {
     ex_expr::exp_return_type rc;
     rc = connectby_tdb.leftPathExpr_->fixup(0, getExpressionMode(), this,  space, heap, 
                                        FALSE, glob);
  }
  ...
  {
     ex_expr::exp_return_type rc;
     rc = connectby_tdb.rightPathExpr_->fixup(0, getExpressionMode(), this,  space, heap, 
                                        FALSE, glob);
  }
...
short ExConnectByTcb::moveRightChildDataToParent(ExConnectByTreeNode *node, short state)
{
    ...
    if(state == 1)
    {
      ...
      if(connectbyTdb().rightMoveExpr_)
        connectbyTdb().rightMoveExpr_->eval( node->getAtp(), pUpEntry->getAtp() );
      ...
      if(NOT connectbyTdb().priorValMoveExpr2_ )
      {
        ...
        if(rcc != ex_expr::EXPR_TRUE) ret = -1;
        else ret = 0;
      }
    }
    if(releaseUpEntry == 1)
...
short ExConnectByTcb::moveChildDataToParent(Int32 who, short state)
{
  ...
  if(who == 1) 
  {
    ...
    if(state == 1)
    {
      ...
      //move data to parent
      if(connectbyTdb().leftMoveExpr_)
      connectbyTdb().leftMoveExpr_->eval(cUpEntry->getAtp(),
		                pUpEntry->getAtp());
      pUpEntry->getAtp()->getTupp(connectbyTdb().fixedPseudoColRowAtpIndex_) =  pcoldata_;
      if(connectbyTdb().pathPseudoColRowAtpIndex_ >0) //has path required
      {
  ...
  if(who == 2) 
  {
    ...
    if(state == 1)
    {
      ...
      if(connectbyTdb().rightMoveExpr_)
      connectbyTdb().rightMoveExpr_->eval(cUpEntry->getAtp(),
                                pUpEntry->getAtp());
...
short ExConnectByTcb::work()
{
  ...
  while (1) // exit via return
          ...
          step_ != DONE_)
	{
      ...
      else if ( (pentry_down->downState.request == ex_queue::GET_ALL || pentry_down->downState.request ==  ex_queue::GET_N) &&
          step_ == INITIAL_)
        {
          if(cache_ != NULL && connectbyTdb().useCache() == TRUE && doUseCache_ == TRUE)
          {
            if(cache_->filled() == TRUE )
              step_ = RETURN_FROM_CACHE_;
          }
        }
      switch (step_)
	{
	case INITIAL_:
	  {
	    if (qStartWithchild_.down->isFull())
	      return WORK_OK;
            if (qparent_.down->isEmpty())
              return WORK_OK;
            currentRoot_ = NULL;
	  ...
	  break;
	      ...
	      case ex_queue::Q_OK_MMORE:
		{
                  ...
                  if(connectbyTdb().pathPseudoColRowAtpIndex_ >0) //has path required
                  {
                    if(connectbyTdb().leftPathExpr_)
                    {
                      ...
                      ex_expr::exp_return_type evalRetCode =
                         connectbyTdb().leftPathExpr_->eval(cUpEntry->getAtp(), workAtp_);
                  ...
                  if(tmpPathBuffer!=NULL)
                    rootNode->setPathItem(tmpPathBuffer, tmpPathLen);
                  currentRoot_->setRoot(rootNode);
                  if(tmpPathBuffer!=NULL)
                  {
                    currentPath_ = new(getHeap()) char[3000];
                    memset(currentPath_,0,3000);
                    currentRoot_->getPath(currentRoot_->getRoot(), currentRoot_->getRoot(),currentPath_,
                                          connectbyTdb().del_, TRUE);
                  }
                  else
                    currentPath_ = NULL;
        ...
        case PROCESS_CONNECT_BY_INIT_:
          {
            if (qConnectBychild_.down->isFull())
              return WORK_OK;
            ex_queue_entry * pentry_down = qparent_.down->getHeadEntry();
            currentLevel_ = 2;
            ex_queue_entry * centry = qConnectBychild_.down->getTailEntry();
            ...
            short downqueueindex = connectbyTdb().priorValsValsDownAtpIndex_;
            short workqueueindex = connectbyTdb().priorPredAtpIndex_;
            toRightChildAtp_->getTupp(downqueueindex) =
                  workAtp_->getTupp(workqueueindex);
            centry->passAtp(toRightChildAtp_);
            qConnectBychild_.down->insert();
            step_ = PROCESS_CONNECT_BY_NEXT_LEVEL_;
          }
              ...
              case ex_queue::Q_OK_MMORE:
                {
                  ...
                  ExConnectByTreeNode* newNode = new(getHeap()) ExConnectByTreeNode(newAtp, currentLevel_, getSpace(), getHeap());
           ...
           if(currentLevel_ > 2) 
           {
              ...
              if(prevNode) {
                if(connectbyTdb().pathPseudoColRowAtpIndex_ >0) //has path required
                {
                  currentPath_ = new(getHeap()) char[3000];
                  memset(currentPath_,0,3000);
                  currentRoot_->getPath(currentRoot_->getRoot(), prevNode, currentPath_,
                                          connectbyTdb().del_, TRUE);
                }
                else
                  currentPath_ = NULL;
            ...
            if(newNode != NULL)
            {
              ...
              {
                ex_expr::exp_return_type evalRetCode =
                  connectbyTdb().priorValMoveExpr2_->eval(newNode->getAtp(), workAtp_);
                if(evalRetCode != ex_expr::EXPR_OK)
                {
                  if(currentLevel_ >= 2) {
...
void ExConnectByTree::getPath(ExConnectByTreeNode* r, ExConnectByTreeNode *n , char *out, NAString del, NABoolean getPathStarted)
{
  Int32 pathItemLen = 0;
  //from root to node n
  ...
  //finish condition:
  if(r == n || level == n->getLevel()+1) return;
  else
  {
...
NABoolean ExConnectByTree::hasLoop( int n )
{
  ...
  for(int i = 0 ; i < n - 1; i++)
  {
    if( tuppIsSame(currTupp_[i], currTupp_[n - 1] ) == TRUE)
      return TRUE; 
  }
  return FALSE;   
}
