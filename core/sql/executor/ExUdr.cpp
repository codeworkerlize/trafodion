...
ExWorkProcRetcode ExUdrTcb::buildAndSendRequestBuffer()
{
         ...
         && qParent_.down->entryExists(nextToSend_))
  {
    ...
    if (sqlBuf->getTotalTuppDescs() > 0)
    {
      ...
      {
        ex_assert(sqlBuf->getFreeSpace() >= 0,
                  "Negative sql buffer free space size");
  ...
  switch (t)
    ...
    case UDR_MSG_LOAD:
    {
      ...
      UdrLoadMsg *loadMsg = new (h) UdrLoadMsg(
        ...
        parentQid,
        clientInfo,
        GetCliGlobals()->currContext()->getTenantName(),
  ...
  if (msg != NULL && verifyUdrServerProcessId())
  {
    ...
    s = new (getIpcHeap())
      UdrClientControlStream(myIpcEnv(),
                             ...
                             FALSE,
                             isTransactional,
                             myExeStmtGlobals()->getThreadInfo());
	...
	if (myTdb().isTmudf())
	  {
	    ...
	    if (dataStream_->numOfInputBuffers()> 0 || !replyBufferIsEmpty())
	      {
		UdrDebug0("Not sending continue request since input buffers is >0 or reply buffer has data available to consume ");
		result = WORK_OK;
		done = TRUE;
		break;
	      }  
		...
		else if (repBuf)
		  {
			...
			if (repBuf->sendMoreData() &&
			...
			else
			  {
				...
				UdrDebug0("[TMUDF WORK checkreceive] Received next reply buffer while processing one  stay in this method");
			      } 
			    attemptTransitionToWorkState();
                            break; 
