...
//
ex_tcb *ExUnPackRowsTdb::build(ex_globals *glob) {
  // Build the Tcb tree below the unPackRows node.
  //
  ex_tcb *childTcb = NULL;
  if (childTdb_) childTcb = childTdb_->build(glob);
  ...
  if (rowwiseRowset())
    unPackRowsTcb = new (glob->getSpace()) ExUnPackRowwiseRowsTcb(*this, glob);
  else
    unPackRowsTcb = new (glob->getSpace()) ExUnPackRowsTcb(*this, *childTcb, glob);
...
//
ExUnPackRowsTcb::ExUnPackRowsTcb(const ExUnPackRowsTdb &unPackRowsTdbLocal, const ex_tcb &childTcb, ex_globals *glob)
  ...
  //
  pool_ = new (space) ExSimpleSQLBuffer(unPackRowsTdb().queueSizeUp_ * 2, unPackRowsTdb().unPackColsTupleLen_, space);
  ...
  //
  qParent_.down =
      new (space) ex_queue(ex_queue::DOWN_QUEUE, unPackRowsTdb().queueSizeDown_, unPackRowsTdb().criDescDown_, space);
  ...
  //
  qParent_.up =
      new (space) ex_queue(ex_queue::UP_QUEUE, unPackRowsTdb().queueSizeUp_, unPackRowsTdb().criDescUp_, space);

  // fixup all expressions expressions
  //

  if (unPackColsExpr())
    unPackColsExpr()->fixup(0, getExpressionMode(), this, glob->getSpace(), glob->getDefaultHeap(), FALSE, glob);
  ...
  workAtp_ = allocateAtp(unPackRowsTdb().workCriDesc_, glob->getSpace());

  // Must always have a packingFactor expression
  //
  ex_assert(packingFactor(), "ExUnPackRowsTcb::ExUnPackRowsTcb(): Internal Error");

  packingFactor()->fixup(0, getExpressionMode(), this, glob->getSpace(), glob->getDefaultHeap(), FALSE, glob);
  ...
  numRowsAtp_ = allocateAtp(unPackRowsTdb().criDescUp_, glob->getSpace());
}
...
ExWorkProcRetcode ExUnPackRowsTcb::processCancel() {
...
ExWorkProcRetcode ExUnPackRowsTcb::workDown() {
  ...
  //
  while (qParent_.down->entryExists(processedInputs_) && !childQueue_.down->isFull()) start();
...
ExWorkProcRetcode ExUnPackRowsTcb::workUp() {
  ...
  while (qParent_.down->getHeadIndex() != processedInputs_) {
    ...
    numRowsAtp_->getTupp(unPackRowsTdb().criDescUp_->noTuples() - 1) = numRowsTupp_;
    ...
    //
    workAtp_->getTupp(unPackRowsTdb().indexValueAtpIndex_) = indexValueTupp_;
    ...
    while (!qParent_.up->isFull() && !childQueue_.up->isEmpty()) {
      ...
      if (cEntry->upState.status == ex_queue::Q_NO_DATA) {
        ...
        while (indexValue_ < pState.numRows_ && !qParent_.up->isFull()) {
          ...
          //
          if (needTuple) {
            if (pool_->getFreeTuple(workAtp_->getTupp(unPackRowsTdb().unPackColsAtpIndex_))) {
          ...
          retCode = (unPackColsExpr() ? unPackColsExpr()->eval(workAtp_, childAtp) : ex_expr::EXPR_TRUE);
          ...
          switch (retCode) {
            case ex_expr::EXPR_TRUE: {
              ...
              if (tolerateNonFatalError) pEntry->setDiagsArea(NULL);
          ...
          if (retCode == ex_expr::EXPR_ERROR && !tolerateNonFatalError) break;
        ...
        //
        if (retCode == ex_expr::EXPR_ERROR && !tolerateNonFatalError) continue;
...
void ExUnPackRowsTcb::processError(atp_struct *atp, NABoolean isNonFatalError, Lng32 markValue) {
  ...
  if (!isNonFatalError) {
    ...
    while (!childQueue_.up->isEmpty() && (childQueue_.up->getHeadEntry()->upState.status != ex_queue::Q_NO_DATA))
      childQueue_.up->removeHead();
  } else {
...
void ExUnPackRowsTcb::processSkippedRow(atp_struct *atp) {
  ex_queue_entry *pEntryDown = qParent_.down->getHeadEntry();
  ex_queue_entry *pEntry = qParent_.up->getTailEntry();
  ExUnPackRowsPrivateState &pState = *((ExUnPackRowsPrivateState *)pEntryDown->pstate);
...
//
ExUnPackRowsPrivateState::ExUnPackRowsPrivateState(const ExUnPackRowsTcb *) { init(); }
...
//
void ExUnPackRowsPrivateState::init() {
  matchCount_ = 0;
  unPackCount_ = 0;
  numRows_ = 0;
...
ex_tcb_private_state *ExUnPackRowsPrivateState::allocate_new(const ex_tcb *tcb) {
  return new (((ex_tcb *)tcb)->getSpace()) ExUnPackRowsPrivateState((ExUnPackRowsTcb *)tcb);
}

// ExUnPackRowwiseRowsTcb::ExUnPackRowwiseRowsTcb() -------------------------
//
ExUnPackRowwiseRowsTcb::ExUnPackRowwiseRowsTcb(const ExUnPackRowsTdb &unPackRowsTdbLocal, ex_globals *glob)
    : ex_tcb(unPackRowsTdbLocal, 1, glob) {
  CollHeap *space = glob->getSpace();

  pool_ = new (space) sql_buffer_pool(uprTdb().numBuffers_, uprTdb().bufferSize_, space);

  // Allocate the queue to communicate with parent
  // (Child allocates queue to communicate with Child)
  //
  qParent_.down = new (space) ex_queue(ex_queue::DOWN_QUEUE, uprTdb().queueSizeDown_, uprTdb().criDescDown_, space);
  ...
  // Allocate a queue to communicate with the parent node.
  //
  qParent_.up = new (space) ex_queue(ex_queue::UP_QUEUE, uprTdb().queueSizeUp_, uprTdb().criDescUp_, space);

  // fixup all expressions
  if (uprTdb().rwrsInputSizeExpr())
    uprTdb().rwrsInputSizeExpr()->fixup(0, getExpressionMode(), this, glob->getSpace(), glob->getDefaultHeap(), FALSE,
                                        glob);

  if (uprTdb().rwrsMaxInputRowlenExpr())
    uprTdb().rwrsMaxInputRowlenExpr()->fixup(0, getExpressionMode(), this, glob->getSpace(), glob->getDefaultHeap(),
                                             FALSE, glob);

  if (uprTdb().rwrsBufferAddrExpr())
    uprTdb().rwrsBufferAddrExpr()->fixup(0, getExpressionMode(), this, glob->getSpace(), glob->getDefaultHeap(), FALSE,
                                         glob);

  rwrsInputValuesTuppDesc_.init();
  // rwrsInputValuesTupp_.init();
  workAtp_ = allocateAtp(uprTdb().workCriDesc_, glob->getSpace());
  workAtp_->getTupp(uprTdb().rwrsWorkIndex()) = &rwrsInputValuesTuppDesc_;
...
ExWorkProcRetcode ExUnPackRowwiseRowsTcb::work() {
  ...
  ExUnPackRowwiseRowsPrivateState *pstate = (ExUnPackRowwiseRowsPrivateState *)pentry_down->pstate;
  ...
  while (TRUE)  // exit via return
  {
    switch (step_) {
      ...
      case GET_INPUT_VALUES_: {
        ...
        workAtp_->getTupp(uprTdb().rwrsWorkIndex()).setDataPointer((char *)&rwrsNumRows_);
        retCode = uprTdb().rwrsInputSizeExpr()->eval(pentry_down->getAtp(), workAtp_);
        if (retCode == ex_expr::EXPR_ERROR) {
          step_ = ERROR_;
          break;
        }
        ...
        if (rwrsNumRows_ < 0) {
          ExHandleErrors(qParent_, pentry_down, 0, getGlobals(), NULL, EXE_NUMERIC_OVERFLOW, NULL, NULL);
          step_ = DONE_;
          break;
        }

        // get max length of each row
        workAtp_->getTupp(uprTdb().rwrsWorkIndex()).setDataPointer((char *)&rwrsMaxInputRowlen_);
        retCode = uprTdb().rwrsMaxInputRowlenExpr()->eval(pentry_down->getAtp(), workAtp_);
        if (retCode == ex_expr::EXPR_ERROR) {
          step_ = ERROR_;
          break;
        }
        ...
        workAtp_->getTupp(uprTdb().rwrsWorkIndex()).setDataPointer((char *)&rwrsBufferAddr_);
        retCode = uprTdb().rwrsBufferAddrExpr()->eval(pentry_down->getAtp(), workAtp_);
        if (retCode == ex_expr::EXPR_ERROR) {
          step_ = ERROR_;
          break;
        }
      ...
      case RETURN_ROW_: {
        ...
        char *currRow = (char *)(rwrsBufferAddr_ + currentRowNum_ * rwrsMaxInputRowlen_);

        // allocate an empty tupp descriptor
        tupp_descriptor *td = pool_->get_free_tupp_descriptor(0);
        if (!td) return WORK_POOL_BLOCKED;  // couldn't allocate, try again later.
      ...
      case ERROR_: {
        ...
        if (diagsArea != up_entry->getDiagsArea()) {
          ...
          if (diagsArea != NULL) diagsArea->incrRefCount();
        }
        // insert into parent
        qParent_.up->insert();
...
// Constructor and destructor for ExUnPackRowsPrivateState
//
ExUnPackRowwiseRowsPrivateState::ExUnPackRowwiseRowsPrivateState(const ExUnPackRowwiseRowsTcb *) { init(); }
...
// it's private state is already initialized.
//
void ExUnPackRowwiseRowsPrivateState::init() {}

ExUnPackRowwiseRowsPrivateState::~ExUnPackRowwiseRowsPrivateState() {}

ex_tcb_private_state *ExUnPackRowwiseRowsPrivateState::allocate_new(const ex_tcb *tcb) {
  return new (((ex_tcb *)tcb)->getSpace()) ExUnPackRowwiseRowsPrivateState((ExUnPackRowwiseRowsTcb *)tcb);
}
