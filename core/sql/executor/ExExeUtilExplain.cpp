...
///////////////////////////////////////////////////////////////
ExExeUtilDisplayExplainTcb::ExExeUtilDisplayExplainTcb(const ComTdbExeUtilDisplayExplain &exe_util_tdb,
                                                       ex_globals *glob)
...
ExExeUtilDisplayExplainTcb::~ExExeUtilDisplayExplainTcb() {
  ...
  if (optFOutput) NADELETEBASIC(optFOutput, getMyHeap());
  if (explainQuery_) {
...
static const QueryString displayExplainOptionEQuery[] = {
    {" select * from table(explain(%s, %s)) "}, {" order by SEQ_NUM descending "}, {" ; "}};

short processExplainRowsWrapper(ExExeUtilDisplayExplainTcb *i) { return i->processExplainRows(); }

short ExExeUtilDisplayExplainTcb::processExplainRows() { return 0; }
...
short ExExeUtilDisplayExplainTcb::work() {
  ...
  ExExeUtilDisplayExplainPrivateState &pstate = *((ExExeUtilDisplayExplainPrivateState *)pentry_down->pstate);
  ...
  while (1)  // exit via return
  {
    switch (pstate.step_) {
      ...
      case PREPARE_: {
        ...
        retcode = cliInterface()->executeImmediate("control session 'EXPLAIN' 'ON';");
        if (retcode < 0) {
        ...
        retcode = cliInterface()->allocStuff(module_, stmt_, sql_src_, input_desc_, output_desc_, "__EXPL_STMT_NAME__");
        if (retcode < 0) {
          cliInterface()->allocAndRetrieveSQLDiagnostics(diagsArea_);
          pstate.step_ = HANDLE_ERROR_;
          break;
        ...
        retcode =
            cliInterface()->prepare(stmtStr, module_, stmt_, sql_src_, input_desc_, output_desc_, NULL /* outputBuf */,
                                    NULL /* outputVarPtrList */, NULL /* inputBuf */, NULL /* inputVarPtrList */,
                                    NULL /* uniqueStmtId */, NULL /* uniqueStmtIdLen */, NULL /* query_cost_info */,
                                    NULL /* comp_stats_info */, FALSE /* monitorThis */, TRUE /* doNotCachePlan */);
      ...
      case SETUP_EXPLAIN_: {
        ...
        if (optFlag_ == M_) {
          explain_qry_array_size = sizeof(displayExplainQuery) / sizeof(QueryString);

          explainQuery = displayExplainQuery;
        } else  // same query used for both E_ and N_ and F_
        {
          explain_qry_array_size = sizeof(displayExplainOptionEQuery) / sizeof(QueryString);
        ...
        explainQuery_ = new (getMyHeap()) char[gluedQuerySize + strlen(explArg1) + strlen(explArg2) + 10];
      ...
      case FETCH_FIRST_EXPLAIN_ROW_: {
        if (optFlag_ == E_ || optFlag_ == N_) {
          ...
          if ((qparent_.up->getSize() - qparent_.up->getLength()) < 5) return WORK_OK;  // come back later
        }
        ...
        if (retcode == 100) {
          ...
          // was not found.
          ExRaiseSqlError(
              getMyHeap(), &diagsArea_,
              (((exeUtilTdb().getModuleName()) || (exeUtilTdb().getStmtName())) ? (ExeErrorCode)CLI_STMT_NOT_EXISTS
      ...
      case FETCH_EPILOGUE_AND_RETURN_ERROR_: {
        ...
        retcode = cliInterface()->deallocStuff(module_, stmt_, sql_src_, input_desc_, output_desc_);
        if (retcode < 0) {
          cliInterface()->allocAndRetrieveSQLDiagnostics(diagsArea_);
        }
        ...
        retcode = cliInterface()->executeImmediate("control session reset 'EXPLAIN';");
        if (retcode < 0) {
          cliInterface()->allocAndRetrieveSQLDiagnostics(diagsArea_);
        }
...
short ExExeUtilDisplayExplainTcb::GetColumns() {
  ...
  for (int i = 5; i < 14; ++i)  // start with operator to check for root
  {
    ...
    if (rc != 0) return (short)rc;  // not sure what error is, the rc will indicate the error
...
void ExExeUtilDisplayExplainTcb::FormatForF() {
  ...
  if (str_cmp_c(operName_, "TRAFODION_INSERT") == 0 || str_cmp_c(operName_, "TRAFODION_UPSERT") == 0 ||
      str_cmp_c(operName_, "TRAFODION_VSBB_UPSERT") == 0 || str_cmp_c(operName_, "TRAFODION_LOAD") == 0 ||
      str_cmp_c(operName_, "TRAFODION_DELETE") == 0 || str_cmp_c(operName_, "TRAFODION_UPDATE") == 0) {
  ...
  if (remaining_width <= 0)
    ...
    for (; remaining_width > 0; --remaining_width) *current++ = ' ';
  ...
  if (str_cmp_c(operName_, "ROOT") == 0) {
  ...
  } else if (str_cmp_c(operName_, "INDEX_SCAN") == 0 || str_cmp_c(operName_, "INDEX_SCAN_UNIQUE") == 0 ||
             str_cmp_c(operName_, "TRAFODION_INDEX_SCAN") == 0) {
    ...
    if (index_loc != 0) {
      ...
      if (index_name == 0) {
        ...
        while (((*index_name) != '(') &&
               (remaining_width >
                1))  // ensure that the index name fits within the 20 bytes allocated for this description column
        {
          *current = *index_name;
          ++current;
          ++index_name;
    ...
    if (par_proc == 0) {  // error
      ...
      *current++ = '#';
      ...
      {
        ++par_proc;
        while ((*par_proc) != ' ' && remaining_width > 0) {
          *current++ = *par_proc++;
          --remaining_width;
        }
      } else {
                      ...
                      if (rep != 0 && rep < child_func) {
                        ...
                        if (rep != 0 && rep < child_func) {
                          ...
                          /*
                            ...
                            parent_partitioning_function: grouped x to y, <optional> PAPA with z PA(s), <optional> "our
    ...
    *current++ = ':';
    ...
    if (child_proc == 0) {  // error
      ...
      *current++ = '#';
      ...
      if (child_proc != 0) {
        ++child_proc;
        while ((*child_proc) != ' ' && remaining_width > 0) {
          *current++ = *child_proc++;
          --remaining_width;
        }
      } else {
    ...
    *current++ = ' ';
    ...
    char *par_proc = str_str(description_, "parent_processes");
    if (par_proc == 0) {  // error, there should be a token present
      *current++ = '*';
      --remaining_width;
    } else if (exeUtilTdb().isOptionC()) {
      *current++ = '#';
    ...
    *current++ = ':';
    --remaining_width;
    char *child_proc = str_str(description_, "child_processes");
    if (child_proc == 0) {  // error, there should be a token present
      *current++ = '*';
      --remaining_width;
    } else if (exeUtilTdb().isOptionC()) {
      *current++ = '#';
  ...
  } else  // another kind of operator
  {
    ...
    if (tname_loc != 0) {
      ...
      if (tname_loc != 0) ++tname_loc;  // remove the schema
    } else
      tname_loc = tName_;
...
void ExExeUtilDisplayExplainTcb::DoHeader() {
  ...
  FormatFloat(outStr, intSize, valSize, cardinality_, (optFlag_ == N_), (optFlag_ == E_));
  FormatLine("ROWS_OUT", outStr, 8, valSize, 0, intSize);

  // rework totalCost as estimated total cost
  FormatFloat(outStr, intSize, valSize, totalCost_, (optFlag_ == N_), (optFlag_ == E_));
  ...
  if (GetField(description_, "must_match: ", stmt, valSize) == 0) FormatLongLine("MUST_MATCH", stmt, 10, valSize);
...
void ExExeUtilDisplayExplainTcb::DoOperator() {
  ...
  FormatFloat((char *)outStr, intSize, valSize, operatorCost_, (optFlag_ == N_), (optFlag_ == E_));
  FormatLine("EST_OPER_COST", outStr, 13, valSize, 0, intSize);

  // rework totalCost as estimated total cost
  FormatFloat((char *)outStr, intSize, valSize, totalCost_, (optFlag_ == N_), (optFlag_ == E_));
  FormatLine("EST_TOTAL_COST", outStr, 14, valSize, 0, intSize);
  ...
  if (optFlag_ == E_)  // only print total cost details for expert users
  {
    ...
    if (i < 4) {
      FormatLine("(missing fields)", NULL, 16, 0, 2);
    }
  }
...
NABoolean ExExeUtilDisplayExplainTcb::filterKey(const char *key, int keySize, char *value, char *retVal,
  ...
  if ((strncmp(key, "esp_", 4) == 0) && (strstr(key, "_node_map"))) {
    strcpy(retVal, "###");
    decLoc = 3;
    return TRUE;
  } else if ((strcmp(key, "child_partitioning_function") == 0) || (strcmp(key, "parent_partitioning_function") == 0)) {
    ...
    if ((strcmp(key, "child_partitioning_function") == 0) && (strncmp(value, "hash1", 5) == 0) ||
...
void ExExeUtilDisplayExplainTcb::FormatLine(const char *key, const char *inval, int keySize, int valSize,
  ...
  if (exeUtilTdb().isOptionC() && key) {
    if (filterKey(key, keySize, val, valBuf, decLoc)) {
      ...
      if (exeUtilTdb().isOptionP()) return;  // prune
    }
  }
...
void ExExeUtilDisplayExplainTcb::FormatSQL(const char *key, char *val, int keySize, int valSize, int indent) {
  ...
  char DMLUins[6][12] = {"INSERT INTO", "VALUES", "SELECT", "FROM", "WHERE", "FOR"};
  char DMLLins[6][12] = {"insert into", "values", "select", "from", "where", "for"};
...
int ExExeUtilDisplayExplainTcb::IsNumberFmt(char *fieldptr) const {
  ...
  if (*(wrkptr + 1) == '\0') {
    return (0);
  }                                                      // no need to determine if one digit
...
void ExExeUtilDisplayExplainTcb::FormatNumber(char *outStr, int &intSize, int &fullSize, char *strVal) const {
  ...
  while ((c = *ptrwrk++) != '\0') {  // scan input watching for end
    ...
    switch (c) {
      case 'e':
        *(ptrwrk - 1) = 'E';  // upcase it and no break
      case 'E':
        expdig = cnt;
        break;
      case '.':
...
void ExExeUtilTcb::FormatFloat(char *outStr, int &intSize, int &fullSize, double floatVal, NABoolean normalMode,
  ...
  while (*ptr++ != '.') {
    intSize++;
  }  // see how long.int part is (always a dot)
...
void ExExeUtilTcb::AddCommas(char *outStr, int &intSize) const {
  ...
  if (intSize <= 3) {
    return;
  }  // if digit count is 3 or less, just return
...
ExExeUtilDisplayExplainComplexTcb::ExExeUtilDisplayExplainComplexTcb(
    const ComTdbExeUtilDisplayExplainComplex &exe_util_tdb, ex_globals *glob)
  ...
  while (1)  // exit via return
  {
    switch (step_) {
      ...
      case IN_MEMORY_CREATE_: {
        ...
        masterGlob->getStatement()->getContext()->setSqlParserFlags(0x20000);  // INTERNAL_QUERY_FROM EXEUTIL
        cliRC = cliInterface()->executeImmediate(exeUtilTdb().qry1_);
        masterGlob->getStatement()->getContext()->resetSqlParserFlags(0x20000);  // INTERNAL_QUERY_FROM EXEUTIL
        if (cliRC < 0) {
          ...
          cliInterface()->allocAndRetrieveSQLDiagnostics(diagsArea_);
          if (exeUtilTdb().loadIfExists() && getDiagsArea()->contains(-1055)) {
            SQL_EXEC_ClearDiagnostics(NULL);
          } else {
            // Return error.
            // Do not do 'DROP_AND_ERROR' as we don't want to drop
            // an existing table.
            ExRaiseSqlError(getHeap(), &diagsArea_, EXE_NO_EXPLAIN_INFO, NULL);
      ...
      case EXPLAIN_CREATE_: {
        cliRC = executeQuery(NULL, NULL, exeUtilTdb().qry2_, FALSE, FALSE, rc, NULL, NULL, FALSE);
        ...
        if (cliRC == 1) return rc;

        if (exeUtilTdb().explainType_ == ComTdbExeUtilDisplayExplainComplex::CREATE_TABLE_AS) {
      ...
      case EXPLAIN_INSERT_SELECT_: {
        ...
        cliRC = executeQuery(NULL, NULL, (exeUtilTdb().qry4_ ? exeUtilTdb().qry4_ : exeUtilTdb().qry3_), FALSE, FALSE,
                             rc, NULL, NULL, FALSE);
        if (cliRC < 0) {
          step_ = DROP_AND_ERROR_;
          break;
      ...
      case DROP_AND_ERROR_: {
        char *dtQuery = new (getMyHeap()) char[strlen("DROP TABLE ; ") + strlen(exeUtilTdb().getObjectName()) + 100];
        if (exeUtilTdb().explainType_ == ComTdbExeUtilDisplayExplainComplex::CREATE_INDEX_)
          strcpy(dtQuery, "DROP INDEX ");
        else if (exeUtilTdb().explainType_ == ComTdbExeUtilDisplayExplainComplex::CREATE_MV_)
...
// Constructor and destructor for ExeUtil_private_state
/////////////////////////////////////////////////////////////////////////////
ExExeUtilDisplayExplainComplexPrivateState::ExExeUtilDisplayExplainComplexPrivateState() { matches_ = 0; }
...
ExExeUtilDisplayExplainShowddlTcb::ExExeUtilDisplayExplainShowddlTcb(
    const ComTdbExeUtilDisplayExplainComplex &exe_util_tdb, ex_globals *glob)
    : ExExeUtilTcb(exe_util_tdb, NULL, glob), step_(EMPTY_), newQry_(NULL) {
  // Allocate the private state in each entry of the down queue
  qparent_.down->allocatePstate(this);
}

ExExeUtilDisplayExplainShowddlTcb::~ExExeUtilDisplayExplainShowddlTcb() {
  if (newQry_) NADELETEBASIC(newQry_, getMyHeap());
  newQry_ = NULL;
}
  ...
  while (1)  // exit via return
  {
    switch (step_) {
      ...
      case GET_LABEL_STATS_: {
        ...
        newQry_ = new (getMyHeap()) char[strlen(exeUtilTdb().qry2_) + 100];
      ...
      case EXPLAIN_CREATE_: {
        cliRC = executeQuery(NULL, NULL,
                             ...
                             FALSE, FALSE, rc, NULL, NULL, FALSE);
...
// Constructor and destructor for ExeUtil_private_state
/////////////////////////////////////////////////////////////////////////////
ExExeUtilDisplayExplainShowddlPrivateState::ExExeUtilDisplayExplainShowddlPrivateState() { matches_ = 0; }
