...
bool ExSM_TargetsEqual(const sm_target_t &a, const sm_target_t &b) {
  bool result = false;
  if (a.node == b.node && a.pid == b.pid && a.tag == b.tag && a.id == b.id) result = true;
  return result;
}
...
//-------------------------------------------------------------------------
int ExSM_GetThreadID() { return (int)GETTID(); }
...
uint32_t ExSM_GetMaxChunkSize(NABoolean intra) {
  if ((intra & EXSM_INTRA_MAX_CHUNK == 0) || (!intra & EXSM_INTER_MAX_CHUNK == 0)) {
    int actualMax = 0;
    int rc = 0;
    ...
    exsm_assert(actualMax > 0, "SeaMonster buffer size must be greater than 0");
    ...
    EXSM_TRACE(EXSM_TRACE_SEND | EXSM_TRACE_SM_CALLS, "SM_ctl sm max %d sql max %u", (int)actualMax,
...
int32_t ExSM_Initialize(ExSMGlobals *smGlobals, ExExeStmtGlobals *stmtGlobals) {
  ...
  EXSM_TRACE(EXSM_TRACE_INIT | EXSM_TRACE_SM_CALLS, "SM_init(%d) returned %s", (int)smGlobals->getSQNodeNum(), buf);
  if (rc != 0) {
                  ...
                  const void *firstChunk)     // IN (useful for queued buffers)
{
  ...
  const char *tagForTracing =
      (msgType == EXSM_MSG_REQUEST ? "DN" : msgType == EXSM_MSG_REPLY ? "UP" : msgType == EXSM_MSG_SHORT ? "SD" : "??");

  // The count variable is for tracing only and is based on a global
  // message counter in SM globals
  uint32_t count =
      (msgType == EXSM_MSG_REQUEST
           ? (1 + smGlobals->getSendRequestCount())
           : msgType == EXSM_MSG_REPLY ? (1 + smGlobals->getSendReplyCount())
                                       : msgType == EXSM_MSG_SHORT ? (1 + smGlobals->getSendShortCount()) : 0);
  ...
  while (!done) {
    EXSM_TRACE(EXSM_TRACE_SEND | EXSM_TRACE_SM_CALLS,
               ...
               ":%d:0x%c bytes %d "
               "prepost %d count %d",
               (int)count, tagForTracing, (int)target.node, (int)target.pid, target.id,
               (int)ExSMTag_GetTagWithoutQualifier(target.tag), (char)ExSMTag_GetQualifierDisplay(target.tag),
    ...
    chunk.size = numBytes;
    if (isPrepostRequired) chunk.flags |= SM_FLAG_SEND_TO_PREPOST;
    ...
    if (firstChunk) {
      ...
      if (chunk.buff != chunk.handle) originalBuf = (char *)firstChunk;
    }
    ...
    if (numRetries == 0)
      event = ExSMEvent::add(ExSMEvent::Send, &target, 0, chunk.size, chunk.flags, (int64_t)chunk.buff,
                             (int64_t)originalBuf);
    ...
    // new event)
    if (event) event->setOptional1(retcode);
    ...
    if (retcode != 0) {
      ...
      EXSM_TRACE(EXSM_TRACE_SEND, "SEND %d ERROR %s %s data %p", (int)count, buf1, buf2, data);
    }
    ...
    if (done && event == NULL) ExSMEvent::add(ExSMEvent::SendRetry, &target, retcode, numRetries);
...
int32_t ExSM_Post(const sm_target_t &target, size_t dataSize, const void *data, ExSMTask *smTask,
                  NABoolean isServer)  // IN (for tracing only)
{
  ...
  ExSMEvent::add(ExSMEvent::Post, &target, result, chunk.size, chunk.flags, (int64_t)chunk.buff);

  EXSM_TRACE(EXSM_TRACE_BUFFER | EXSM_TRACE_SM_CALLS, "PREPOST %s %d:%d:%" PRId64 ":%d:0x%c %p %d rc %d %d",
             (isServer ? "S" : "C"), (int)target.node, (int)target.pid, target.id,
             (int)ExSMTag_GetTagWithoutQualifier(target.tag), (char)ExSMTag_GetQualifierDisplay(target.tag), chunk.buff,
             (int)dataSize, (int)rc, (int)chunk.errcode);
...
int32_t ExSM_SendShortMessage(ExSMGlobals *smGlobals, const sm_target_t &target, const void *data, size_t bytesToSend) {
  ...
  int32_t result = ExSM_Send(smGlobals, target, data, bytesToSend, EXSM_MSG_SHORT,
...
void *ExSM_AllocateMessageBuffer(size_t dataBytes, NAMemory *threadSafeHeap) {
  IpcMessageBuffer *msgBuf = IpcMessageBuffer::allocate(dataBytes,
                                                        IpcDefGuaMaxMsgIOSize,  // for now use a constant
...
void *ExSM_GetMessageBufferAddr(void *prepostAddr) {
  void *result = NULL;
  if (prepostAddr) result = ((char *)prepostAddr - sizeof(IpcMessageBuffer));
...
int32_t ExSM_GetNodeID(int32_t nodeNum) {
  ...
  if (firstTime) {
    ...
    if (getenv("SQ_VIRTUAL_NODES")) {
      ...
      if (sm_envvar && sm_envvar[0] == '0') smVirtualNodes = false;
    }
  }
...
void ExSM_AddCompletedSendBuffer(void *buf) {
  ...
  ExSM_MessageList *newElem = (ExSM_MessageList *)heap->allocateMemory(sizeof(ExSM_MessageList));
...
bool ExSM_RemoveCompletedSendBuffer(void *&buf) {
  ...
  if (EXSM_COMPLETED_SENDS) {
    ...
    if (headNode) EXSM_COMPLETED_SENDS = headNode->next_;
...
int32_t ExSM_Register(const int64_t &smQueryID) {
  ...
  if (found) {
    EXSM_TRACE(EXSM_TRACE_SM_CALLS, "REGISTER %" PRId64 " already active, entries %d\n", smQueryID,
               (int)EXSM_IDS->numEntries());
  } else {
    ...
    EXSM_TRACE(EXSM_TRACE_CANCEL | EXSM_TRACE_SM_CALLS, "SM_register %" PRId64 " rc %d entries %d", smQueryID,
               (int)smResult, (int)EXSM_IDS->numEntries());

    if (smResult == 0) {
      // Add the ID to the global collection of active IDs
      EXSM_IDS->insert((char *)&smQueryID, sizeof(smQueryID), (void *)smQueryID);
    }
  }
...
int32_t ExSM_Cancel(const int64_t &smQueryID) {
  ...
  if (EXSM_IDS) {
    ...
    if (found) {
      ...
      EXSM_IDS->remove((char *)&smQueryID, sizeof(smQueryID), (void *)smQueryID);
      ...
      EXSM_TRACE(EXSM_TRACE_CANCEL | EXSM_TRACE_SM_CALLS, "SM_cancel %" PRId64 " rc %d entries %d", smQueryID,
                 (int)smResult, (int)EXSM_IDS->numEntries());
    } else {
      EXSM_TRACE(EXSM_TRACE_SM_CALLS, "CANCEL %" PRId64 " is not active, entries %d\n", smQueryID,
                 (int)EXSM_IDS->numEntries());
    }
