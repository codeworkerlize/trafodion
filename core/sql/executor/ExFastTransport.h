...
public:
  ...
  NABoolean isOpen() const                   { return (sequenceFileWriter_ != NULL); }
...
private:
  ...
  OrcFileVectorWriter *orcFileWriter_;
  ParquetFileWriter *parquetFileWriter_;
  AvroFileWriter *avroFileWriter_;
...
protected:
  ...
  NABoolean isOrcFile();
  void closePartitionError(const char *);
  NABoolean isParquetFile();
  NABoolean isAvroFile();
  void createOrcFileError(Int32 ofwRetCode);
  void createParquetFileError(Int32 ofwRetCode);
  void createAvroFileError(Int32 ofwRetCode);
