          ...
          if (costTh >= 0)
          {
            ...
            if (taskCost >= costTh)
            {
              QRWARN("ExScheduler subtask %d TN %s TC %d STC %d PID %d txID %ld TC %ld",
                     (subtcb ? subtcb->getNodeType() : -2), (subtcb ? typeid(*subtcb).name() : "null") , taskCount, sTaskCount, pid,
                     (GetCliGlobals() ? GetCliGlobals()->getTransactionId() : -1), taskCost);
            }
          }
		  ...
		  // Other return codes cause the scheduler to exit
                   if (rc != WORK_CALL_AGAIN)
		    {
		      ...
		      if (costTh >= 0)
		        ...
		        if (timeCost >= costTh)
		          QRWARN("ExScheduler total TC %d STC %d PID %d txID %ld TC %ld",
		                  taskCount, sTaskCount, pid, (GetCliGlobals() ? GetCliGlobals()->getTransactionId() : -1), timeCost);
                      }
