...
NABoolean compileTimeMonitorOn()
{
   ...
   return CURRSTMT_OPTDEFAULTS->compileTimeMonitor() ||
          getenv("COMPILE_TIME_MONITOR");
}
  ...
  *gen_code_len = 0;
  ...
  for (Int32 x=0; x < 3; x++) {
    ...
    if (useTextCache && phase == END) {
      ...
      if (compileFromCache(sqlText, input.charSet(), queryExpr, NULL,
                           cachewa, gen_code,
   ...
   while (sqlcode < 0)
   {
     ...
     else if ( returnedNumSiKeys > 0 )
     {
       ...
       qiInvalidationType(returnedNumSiKeys, sikKeyArray, 
                          ...
                          resetRoleList, updateCaches, resetSchemaCaches); 
       ...
       if (resetSchemaCaches)
         InvalidateNATableSchemaCacheEntries(returnedNumSiKeys, sikKeyArray);
...
void CmpMain::UpdateNATableCacheEntryStoredStats(Int32 returnedNumQiKeys,
                                                 SQL_QIKEY * qiKeyArray)
{
   ActiveSchemaDB()->getNATableDB()->update_entry_stored_stats_with_QI_key(
     returnedNumQiKeys, qiKeyArray);
   return ;
}
...
void CmpMain::InvalidateNATableSchemaCacheEntries(Int32 returnedNumQiKeys,
                                                  SQL_QIKEY * qiKeyArray)
{
   ActiveSchemaDB()->getNATableDB()->free_entries_with_schemaUID(
     returnedNumQiKeys, qiKeyArray);
   return ;
}
   ...
   TimeVal&   begTime)  //(IN):  start time for this compile
{
  ...
  if (CmpCommon::getDefault(SKIP_QUERY_CACHE) == DF_ON)
    return FALSE;
    ...
    if (phase == CmpMain::PREPARSE) {
      ...
      if (tkey) {
        ...
        if (CmpCommon::getDefault(GENERATE_USER_QUERYCACHE_DEBUG) != DF_OFF)
          QRLogger::log(CAT_SQL_EXE, LL_WARN, "false hit textkey due to aqrEnabled not true or hiveWriteAccess. "); 
		    bPatchOK = FALSE;
		    return FALSE;
		  }
	      }
