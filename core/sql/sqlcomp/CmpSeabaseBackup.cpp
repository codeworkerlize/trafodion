...
static const NativeHBaseTableInfo nativeHBaseTables[] = {
    {TRAFODION_SYSCAT_LIT, SEABASE_DTM_SCHEMA, SEABASE_BINLOG_READER},
    {ESG_SEQ_CAT, ESG_SEQ_SCHEMA, ESG_SEQ_TABLE}
};
...
static NABoolean isHBaseNativeTable(NAString& tableName) {
    ...
    for (int i = 0; i < sizeof(nativeHBaseTables)/sizeof(NativeHBaseTableInfo); i++) {
        ...
        if (fields.entries() == 3)
            nativeTable += NAString(nativeHBaseTables[i].catName) + ".";
        nativeTable += NAString(nativeHBaseTables[i].schName) + "." +
                       NAString(nativeHBaseTables[i].objName);
        if (tableName == nativeTable)
            return TRUE;
    }
    return FALSE;
}
...
short CmpSeabaseDDL::lockSQL()
{
  ...
  IpcServer *ssmpServer = ssmpManager->getSsmpServer(heap_,
...
short CmpSeabaseDDL::lockAll()
{
  if (isSQLLocked())
    {
      *CmpCommon::diags() << DgSqlCode(-5050)
                          << DgString0("This")
                          << DgString1("Reason: Another backup or restore operation is in progress and need to finish first.");
  ...
  if(rc)
    {
      *CmpCommon::diags() << DgSqlCode(-5051)
                          << DgString0("lock")
                          << DgInt0(rc);
...
short CmpSeabaseDDL::genLobChunksTableName(ExeCliInterface *cliInterface,
                                           const char *catName, 
                                           const char *schName,
                                           const char *objName, 
                                           const char *chunksSchName,
                                           NAString &fullyQualChunksName,
                                           NAString *chunksObjName)
{
  Int64 objUID = getObjectUID(cliInterface, 
                              (catName ? catName : TRAFODION_SYSCAT_LIT), 
                              schName, objName,
  ...
  char buf[1000];
  if (catName)
    str_sprintf(buf, "%s.\"%s\".%s%020ld",
                catName, chunksSchName,
                LOB_CHUNKS_V2_PREFIX, objUID);
  else
    str_sprintf(buf, "%s.%s.%s%020ld",
                TRAFODION_SYSCAT_LIT, chunksSchName,
                LOB_CHUNKS_V2_PREFIX, objUID);
...
// ----------------------------------------------------------------------------
static short unlockSnapshotMeta( 
  ...
  NAString &lockedTag) 
{
  ...
  if (lockedTag == backupTag)
    return ehi->operationUnlock(lockedTag, FALSE, FALSE);
...
static bool hasAccess(const char * owner, bool checkForShow)
{
  ...
  if (owner != NULL)
    {
      if (strcmp(ComUser::getCurrentUsername(), owner) == 0)
        return true;
    }

  // If elevated, okay
  if (ComUser::currentUserHasElevatedPrivs(NULL /*don't populate diags*/))
    return true;
...
short CmpSeabaseDDL::createProgressTable(ExeCliInterface * cliInterface)
{
  ...
  char psQuery[2000];
  str_sprintf(psQuery, "create table if not exists %s.\"%s\".\"%s\" (oper char(20) character set iso88591 not null, subOper varchar(20) character set iso88591, stepText char(40) character set iso88591 not null, state varchar(25) character set iso88591, tag char(64) character set iso88591 not null, startTime largeint unsigned not null not serialized, endTime largeint unsigned not null not serialized, param1 int unsigned not null not serialized, param2 int unsigned not null not serialized, perc int unsigned not null not serialized, queryId varchar(160) character set iso88591, stepSeq int not null, tag_oper_cnt char(8) character set iso88591 not null, primary key(oper, tag, stepText, tag_oper_cnt)) hbase_options (max_versions = '1') attribute hbase format",
...
short CmpSeabaseDDL::dropProgressTable(ExeCliInterface * cliInterface)
{
  ...
  char psQuery[2000];
  str_sprintf(psQuery, "drop table if exists %s.\"%s\".\"%s\" ",
              TRAFODION_SYSCAT_LIT, SEABASE_REPOS_SCHEMA, "BREI_PROGRESS_STATUS_TABLE");
  cliRC = cliInterface->executeImmediate(psQuery);
  if (cliRC < 0)
    {
...
short CmpSeabaseDDL::truncateProgressTable(ExeCliInterface * cliInterface)
{
  ...
  char psQuery[2000];
  str_sprintf(psQuery, "truncate table %s.\"%s\".\"%s\" ",
              TRAFODION_SYSCAT_LIT, SEABASE_REPOS_SCHEMA, "BREI_PROGRESS_STATUS_TABLE");
  cliRC = cliInterface->executeImmediate(psQuery);
  if (cliRC < 0)
    {
...
short CmpSeabaseDDL::deleteFromProgressTable(ExeCliInterface * cliInterface,
                                             RelBackupRestore *brExpr,
                                             const char * oper)
{
  Int32 cliRC = 0;
  char psQuery[2000];
  ...
  str_sprintf(psQuery, "delete from %s.\"%s\".\"%s\" where oper = '%s' and (tag = '%s' or tag in (select tag from %s.\"%s\".\"%s\" where oper = '%s' and (steptext = 'Finalize' or state = 'Failed' or starttime <= juliantimestamp(current_timestamp_utc) ) ) )",
              TRAFODION_SYSCAT_LIT, SEABASE_REPOS_SCHEMA, "BREI_PROGRESS_STATUS_TABLE",
              oper, brExpr->backupTag().data(),
              TRAFODION_SYSCAT_LIT, SEABASE_REPOS_SCHEMA, "BREI_PROGRESS_STATUS_TABLE",
              oper);
...
Int32 CmpSeabaseDDL::getBREIOperCount(ExeCliInterface * cliInterface,
                       const char * oper,
                       const char * tag) {
    Int32 cliRC = 0;
    char psQuery[300];
    ...
    str_sprintf(psQuery, "select count(tag_oper_cnt) from %s.\"%s\".\"%s\" where oper='%s' and tag='%s' and steptext='%s';",
              TRAFODION_SYSCAT_LIT, SEABASE_REPOS_SCHEMA, "BREI_PROGRESS_STATUS_TABLE",
              oper, tag, stepText.data());
...
short CmpSeabaseDDL::insertProgressTable(ExeCliInterface * cliInterface,
                                         RelBackupRestore *brExpr,
                                         const char * oper,
                                         const char *stepText,
                                         const char *state,
                                         ...
                                         Int32 param2)
{
  ...
  if (NAString(stepText) == "Initialize")
    {
      if (strcmp(oper, "BACKUP") == 0)
      ...
      else
        {
          ...
          if (retcode < 0)
            {
              populateDiags("GET BACKUP TAGS", 
                            "Error returned during list all backups.",
                            "ExpHbaseInterface::listAllBackups", 
          ...
          if (snapshotList && (snapshotList->entries() > 0))
            {
              ...
              NABoolean tagInSnapshotList = 
                containsTagInSnapshotList(snapshotList, (char*)backupTag.data(),
                                          backupOper, backupStatus);
              
              if ((backupOper == "REGULAR") || (backupOper == "INCREMENTAL"))
                subOper = backupOper;
            }
        }
    }
  ...
  char str_cnt[8];
  str_sprintf(psQuery, "insert into %s.\"%s\".\"%s\" values ('%s', '%s', '%s', '%s', '%s', juliantimestamp(current_timestamp_utc), juliantimestamp(current_timestamp_utc), %d, %d, 0, '%s', %d, '%s');",
              TRAFODION_SYSCAT_LIT, SEABASE_REPOS_SCHEMA, "BREI_PROGRESS_STATUS_TABLE",
              oper, 
              (subOper.isNull() ? " " : subOper.data()),
              stepText, state,
              backupTag.data(),
              param1, param2,
              CmpCommon::context()->sqlSession()->getParentQid(),
              stepSeq, str_itoa(operCnt, str_cnt));
...
short CmpSeabaseDDL::updateProgressTable(ExeCliInterface * cliInterface,
                                         ...
                                         Int32 perc)
{
  ...
  if ((CmpCommon::getDefaultNumeric(TRAF_BREI_PROGRESS_UPDATE_DELAY) == -1) &&
      (NAString(state) != "Completed"))
    return 0;

  char str_cnt[8];
  if (perc < 0)
    str_sprintf(psQuery, "update %s.\"%s\".\"%s\" set state = '%s', endTime = juliantimestamp(current_timestamp_utc) where oper = '%s' and stepText = '%s' and tag = '%s' and tag_oper_cnt = '%s';",
                TRAFODION_SYSCAT_LIT, SEABASE_REPOS_SCHEMA, "BREI_PROGRESS_STATUS_TABLE",
                state, oper, stepText, 
                backupTag, str_itoa(operCnt, str_cnt));
  else              
    str_sprintf(psQuery, "update %s.\"%s\".\"%s\" set state = '%s', endTime = juliantimestamp(current_timestamp_utc), param2 = %d, perc = %d where oper = '%s' and stepText = '%s' and tag = '%s' and tag_oper_cnt = '%s';",
                TRAFODION_SYSCAT_LIT, SEABASE_REPOS_SCHEMA, "BREI_PROGRESS_STATUS_TABLE",
                state, param2, perc, oper, stepText, 
                backupTag, str_itoa(operCnt, str_cnt));
...
short CmpSeabaseDDL::getProgressStatus(RelBackupRestore * brExpr, 
                                       ...
                                       CmpDDLwithStatusInfo *dws)
{
  ...
  str_sprintf(psQuery, "select [first 1] trim(tag), tag_oper_cnt from %s.\"%s\".\"%s\" where oper = '%s' and stepSeq = 1 and steptext = 'Initialize' order by starttime desc ",
              TRAFODION_SYSCAT_LIT, SEABASE_REPOS_SCHEMA, "BREI_PROGRESS_STATUS_TABLE",
              brExpr->getBREIoper().data());              
  ...
  str_sprintf(psQuery, "select stepText, state, tag, cast(case when endtime > starttime then endtime - starttime else 0 end as largeint not null), param1, param2, perc, cast(converttimestamp(startTime, to local) as varchar(20)), cast(converttimestamp(endTime, to local) as varchar(20)), startTime, endTime from %s.\"%s\".\"%s\" where oper = '%s' %s %s order by stepSeq",
              TRAFODION_SYSCAT_LIT, SEABASE_REPOS_SCHEMA, "BREI_PROGRESS_STATUS_TABLE",
              brExpr->getBREIoper().data(),
  ...
  if ((! psQueue) || (psQueue->numEntries() == 0))
    {
      ...
      populateBlackBox(cliInterface, returnQueue, 
                       blackBoxLen, blackBox);
  ...
  NAString startTime;
  NAString endTime;
  Int64 startTimeJTS = 0;
  Int64 endTimeJTS = 0;
  Int64 et = 0;
  Int32 hh,mm,ss;
  for (Lng32 idx = 0; idx < psQueue->numEntries(); idx++)
    {
      vi = (OutputInfo*)psQueue->getCurr();
      ...
      str_sprintf(retBuf, "%-18s %-12s %-9s %-15s",
                  step.data(), state.data(), intBuf/*et.data()*/, progressBuf);
...
short CmpSeabaseDDL::truncateBackupMD(NAString &backupSch,
                                      ExeCliInterface * cliInterface)
{
  Lng32 cliRC = 0;
  char query[2000];
  ...
  NAString qTableName;
  qTableName = NAString(TRAF_RESERVED_NAMESPACE3) + ":" +
  ...
  for (Lng32 i = 0; i < sizeof(backupMDs)/sizeof(BackupMDsInfo); i++)
    {
      ...
      if (backupMDs[i].hasLob)
        {
          if (genLobChunksTableName(cliInterface, 
                                    ...
                                    backupMDs[i].objName,
                                    backupSch.data(),
                                    lobChunksFullyQualObjName,
                                    &lobChunksObjName) < 0)
            return -1; // does not exist

          str_sprintf(query, "delete with no rollback from %s;",
                      lobChunksFullyQualObjName.data());
        }
      else
        {
          str_sprintf(query, "delete with no rollback from %s.\"%s\".%s;",
                      TRAFODION_SYSCAT_LIT, backupSch.data(), 
                      backupMDs[i].objName);
        }
      ...
      if (cliRC < 0)
        {
          qTableName = NAString(TRAF_RESERVED_NAMESPACE3) + ":" +
            NAString(backupMDs[i].catName) + "." +
            backupSch + "." +
            NAString(backupMDs[i].hasLob ? lobChunksObjName.data()
                     : backupMDs[i].objName);
...
static short extractFromOperTag(char * operTag,
                                NAString &oper,
                                NAString &tag)
{
...
//         2, not locked
//        -1, if error.  
short CmpSeabaseDDL::checkIfBackupMDisLocked(RelBackupRestore * brExpr,
                                             ExeCliInterface * cliInterface,
                                             const char * inOper,
                                             NAString &oper, NAString &tag)
{
  ...
  if (objsQueue->numEntries() > 1)
    return -1; // error, should only have max one entry

  if (objsQueue->numEntries() == 0)
    return 2; // doesnt exist in TEXT, not locked
  
  objsQueue->position();
  OutputInfo * vi = (OutputInfo*)objsQueue->getNext(); 
  
  if (extractFromOperTag((char*)vi->get(0), oper, tag))
    return -1;

  // backup MD is locked. 
  if (strcmp(inOper, "BACKUP") == 0) {
    updateProgressTable(cliInterface, brExpr->backupTag().data(), inOper,
                      "SetupMetadata", "Failed", -1, -1);
  }

  return 1;
}

short CmpSeabaseDDL::lockBackupMD(RelBackupRestore * brExpr,
                                  ExeCliInterface * cliInterface,
                                  const char * oper,
                                  NABoolean override)
{
  ...
  Int64 objUID = getObjectUID(cliInterface, 
                              ...
                              FALSE, FALSE);
  if (objUID < 0)
    return 0; // nothing to lock, object doesnt exist
  ...
  char query[2000];
  str_sprintf(query, "%s into %s.\"%s\".%s values (%ld, %d, 0, 0, 0, '%s')",
              (override ? "upsert" : "insert"),
              TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_TEXT, 
              objUID,
              COM_LOCKED_OBJECT_BR_TAG,
              operTag.data());
  cliRC = cliInterface->executeImmediate(query);
  if (cliRC < 0)
    {
 ...
 label_return:
  if (endXnIfStartedHere(cliInterface, xnWasStartedHere, 
                         (cliRC < 0 ? -1 : 0)))
    return -1;
...
short CmpSeabaseDDL::unlockBackupMD(RelBackupRestore * brExpr,
                                    ExeCliInterface * cliInterface,
                                    const char * oper)
{
  Lng32 cliRC = 0;
  ...
  Int64 objUID = getObjectUID(cliInterface, 
                              ...
                              FALSE, FALSE);
  if (objUID < 0)
    return 0; // nothing to unlock, object doesnt exist
  ...
  char query[2000];
  str_sprintf(query, "delete from %s.\"%s\".%s where text_uid = %ld and text_type = %d",
              TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_TEXT, 
              objUID, COM_LOCKED_OBJECT_BR_TAG);
  cliRC = cliInterface->executeImmediate(query);
  if (cliRC < 0)
    {
      cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
...
static short cleanupBackupSchema(ExeCliInterface * cliInterface,
                                 NAString &backupSch)
{
  Lng32 cliRC = 0;

  char query[1000];
  str_sprintf(query, "drop schema if exists %s.\"%s\" cascade;",
              TRAFODION_SYSCAT_LIT, backupSch.data());
  cliRC = cliInterface->executeImmediate(query);
  if (cliRC < 0)
    {
      str_sprintf(query, "cleanup schema %s.\"%s\";",
                  TRAFODION_SYSCAT_LIT, backupSch.data());
      cliRC = cliInterface->executeImmediate(query);
      if (cliRC < 0)
        {
  ...
  str_sprintf(query, "delete from %s.\"%s\".%s where catalog_name = '%s' "
...
short CmpSeabaseDDL::createBackupMDschema(RelBackupRestore * brExpr,
                                          ExeCliInterface * cliInterface,
                                          NAString &backupSch,
                                          const char *oper)
{
  ...
  if (backupSchExists && brExpr->dropBackupMD())
    {
      if (cleanupBackupSchema(cliInterface, backupSch))
        return -1;
  ...
  if (insertProgressTable(
           cliInterface, brExpr, oper, 
           (NOT backupSchExists ? "CreateMetadata" : "TruncateMetadata"), "Initiated", 
           numTables, 0) < 0)
    {
      retcode = -1;
      goto label_return;
  ...
  if (NOT backupSchExists)
    {
      ...
      if (createHistogramTables(cliInterface, 
                                NAString(TRAFODION_SYSCAT_LIT) + "." + SEABASE_SYSTEM_SCHEMA,
                                TRUE, tableNotCreated))
        {
      ...
      str_sprintf(query, "create schema if not exists %s.\"%s\" namespace '%s';",
                  TRAFODION_SYSCAT_LIT, backupSch.data(), TRAF_RESERVED_NAMESPACE3);
      cliRC = cliInterface->executeImmediate(query);
      if (cliRC < 0)
        {
          cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
      ...
      for (Lng32 i = 0; i < numTables; i++)
        {
          str_sprintf(query, "create table if not exists %s.\"%s\".%s like %s.\"%s\".%s without namespace;",
                      TRAFODION_SYSCAT_LIT, backupSch.data(), backupMDs[i].objName,
                      backupMDs[i].catName, backupMDs[i].schName, backupMDs[i].objName);
          ...
          if (updateProgressTable
              (cliInterface, brExpr->backupTag().data(), oper, "CreateMetadata",
               "Initiated", i,
               perc) < 0)
            {
              retcode = -1;
  ...
  if (updateProgressTable(cliInterface, brExpr->backupTag().data(), oper, 
                          (NOT backupSchExists ? "CreateMetadata" : "TruncateMetadata"), 
                          "Completed", numTables, 100) < 0)
    {
      retcode = -1;
      goto label_return;
    }
...
short CmpSeabaseDDL::createBackupTags(RelBackupRestore * brExpr, 
                                      ExeCliInterface * cliInterface)
{
  short retcode = 0;

  if (createProgressTable(cliInterface))
    return -1;
...
short CmpSeabaseDDL::genLobLocation(Int64 objUID,
                                    const char *schName,
                                    ExeCliInterface * cliInterface,
                                    TextVec &lobLocList,
                                    Int32 &numLOBfiles,
                                    NABoolean forDisplay)
{
  ...
  Lng32 retcode = existsInSeabaseMDTable(
       cliInterface,
       TRAFODION_SYSCAT_LIT, schName, oldLobMDName,
       COM_BASE_TABLE_OBJECT, TRUE, TRUE);
  if (retcode == 1) // exists. old lob name
    {
      if (forDisplay)  // get all lob files
        sprintf(query, "select location || '/LOBP_%020ld_' || cast(repeat('0', 4 - char_length(lobnum), 10) as varchar(4) character set iso88591 not null) || lobnum from %s.\"%s\".LOBMD__%020ld",
                objUID,
  ...
  else
    {
      // V2 lob
      if (forDisplay)  // get all lob files
        sprintf(query, "select location || '/LOBP_%020ld_' || cast(repeat('0', 4 - char_length(lobnum), 10) as varchar(4) character set iso88591 not null) || lobnum from %s.\"%s\".%s where table_uid = %ld and num_lob_files > 0",
                objUID,
                TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_LOB_COLUMNS,
                objUID);
      else // concatenate and get all distinct locations 
        sprintf(query, "select pivot_group(a) from (select distinct location from %s.\"%s\".%s where table_uid = %ld and num_lob_files > 0) x(a) having pivot_group(a) is not null",
                TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_LOB_COLUMNS,
                objUID);
    }
  ...
  for (Lng32 idx = 0; idx < lobLocsQueue->numEntries(); idx++)
    {
...
short CmpSeabaseDDL::genLobObjs(ElemDDLList * tableList,
                                char * srcSch,
                                ExeCliInterface * cliInterface,
                                Queue* &lobsQueue,
                                NAString &lobObjs)
{
  ...
  lobsQuery +=
    ...
    " and (O2.object_name like 'LOBDescChunks|_|_' || lpad(cast(O.object_uid as varchar(20)),20,'0') || '|_' || '%%' escape '|' or "
    " O2.object_name like 'LOBDescHandle|_|_' || lpad(cast(O.object_uid as varchar(20)),20,'0') || '|_' || '%%' escape '|' or "
  ...
  for (CollIndex i = 0; i < tableList->entries(); i++)
    {
      ...
      if (i < tableList->entries()-1)
        lobsQuery += ", ";
    }
  lobsQuery += ")";
  lobsQuery += ")";
  lobsQueue = NULL;
  cliRC = cliInterface->fetchAllRows(lobsQueue, lobsQuery, 
                                     0, FALSE, FALSE, TRUE);
  if (cliRC < 0)
    {
      cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
  ...
  if (lobsQueue->numEntries() == 0)
    return 0;

  for (Lng32 idx = 0; idx < lobsQueue->numEntries(); idx++)
    {
...
short CmpSeabaseDDL::genIndexObjs(ElemDDLList * tableList,
                                  char * srcSch,
                                  ExeCliInterface * cliInterface,
                                  Queue* &indexesQueue,
                                  NAString &indexesObjs)
{
  ...
  indexesQuery += 
    ...
    NAString(" from " TRAFODION_SYSTEM_CATALOG".") +
    "\"" + srcSch + "\"" + "." SEABASE_OBJECTS" where object_uid in " +
    " (select index_uid from " TRAFODION_SYSTEM_CATALOG"." +
    "\"" + srcSch + "\"" + "." SEABASE_INDEXES" where base_table_uid in " +
    " (select object_uid from " TRAFODION_SYSTEM_CATALOG"." +
  ...
  indexesQueue = NULL;
  cliRC = cliInterface->fetchAllRows(indexesQueue, indexesQuery, 
                                     0, FALSE, FALSE, TRUE);
  if (cliRC < 0)
    {
      cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
  ...
  if (indexesQueue->numEntries() == 0)
    return 0;

  for (Lng32 idx = 0; idx < indexesQueue->numEntries(); idx++)
    {
      ...
      indexesObjs += "'";
      if (idx < indexesQueue->numEntries() -1)
        indexesObjs += ", ";
    }
...
short CmpSeabaseDDL::genReferencedObjs(ElemDDLList * tableList,
                                       char * srcSch,
                                       ExeCliInterface * cliInterface,
                                       Queue* &refsQueue,
                                       NAString &refsObjs)
{
  ...
  refsQuery += 
    ...
    NAString(" from " TRAFODION_SYSTEM_CATALOG".") +
    "\"" + srcSch + "\"" + "." SEABASE_OBJECTS" where object_uid in " +
    " (select table_uid from " TRAFODION_SYSTEM_CATALOG"." + "\"" + srcSch + "\"" + "." SEABASE_TABLE_CONSTRAINTS" where constraint_uid in " +
    " (select unique_constraint_uid from " TRAFODION_SYSTEM_CATALOG"." + "\"" + srcSch + "\"" + "." SEABASE_REF_CONSTRAINTS" where ref_constraint_uid in " +
    " (select constraint_uid from " TRAFODION_SYSTEM_CATALOG"." + "\"" + srcSch + "\"" + "." SEABASE_TABLE_CONSTRAINTS" where constraint_type = 'F' and table_uid in " +
    " (select object_uid from " TRAFODION_SYSTEM_CATALOG"." + "\"" + srcSch + "\"" + "." SEABASE_OBJECTS" where trim(catalog_name) || '.' || trim(schema_name) || " +
    "'.' || trim(object_name) in ";
  ...
  refsQueue = NULL;
  cliRC = cliInterface->fetchAllRows(refsQueue, refsQuery, 
                                     0, FALSE, FALSE, TRUE);
  if (cliRC < 0)
    {
      cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
  ...
  if (refsQueue->numEntries() == 0)
    return 0;

  for (Lng32 idx = 0; idx < refsQueue->numEntries(); idx++)
    {
...
short CmpSeabaseDDL::genLibsUsedProcs(NAString libsToBackup,
                                      ...
                                      ExeCliInterface * cliInterface)
{
  ...
  str_sprintf(query, "select schema_name || '.' || object_name from %s.\"%s\".%s where object_uid in (select used_udr_uid from %s.\"%s\".%s where using_library_uid in (select object_uid from %s.\"%s\".%s where (schema_name || '.' || object_name in (%s))))",
              TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_OBJECTS, 
              TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_LIBRARIES_USAGE,
              TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_OBJECTS, 
              libsToBackup.data());

  Queue *procsQueue = NULL;
  cliRC = cliInterface->fetchAllRows(procsQueue, query,
                                     0, FALSE, FALSE, TRUE);
  if (cliRC < 0)
    {
      cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
  ...
  if (procsQueue->numEntries() == 0)
    return 0;
  
  for (Lng32 idx = 0; idx < procsQueue->numEntries(); idx++)
    {
...
short CmpSeabaseDDL::genProcsUsedLibs(NAString procsToBackup,
                                      ...
                                      ExeCliInterface * cliInterface)
{
  ...
  str_sprintf(query, "select schema_name || '.' || object_name from %s.\"%s\".%s where object_uid in (select library_uid from %s.\"%s\".%s where udr_uid in (select object_uid from %s.\"%s\".%s where (schema_name || '.' || object_name in (%s))))",
              TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_OBJECTS, 
              TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_ROUTINES,
              TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_OBJECTS, 
              procsToBackup.data());

  Queue *libsQueue = NULL;
  cliRC = cliInterface->fetchAllRows(libsQueue, query,
                                     0, FALSE, FALSE, TRUE);
  if (cliRC < 0)
    {
      cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
  ...
  if (libsQueue->numEntries() == 0)
    return 0;
  
  for (Lng32 idx = 0; idx < libsQueue->numEntries(); idx++)
    {
...
short CmpSeabaseDDL::genListOfObjs(ElemDDLList * objList,
...
short CmpSeabaseDDL::genBRObjUIDsWherePred(RelBackupRestore *brExpr,
                                           ElemDDLList * schemaList, 
                                           ElemDDLList * tableList,
                                           ElemDDLList * viewList,
                                           ElemDDLList * procList,
                                           ElemDDLList * libList,
                                           ...
                                           NABoolean &schemasOnlyBackup)
{
  ...
  if (genListOfObjs(tableList, tablesToBackup, prependOR, schPrivsList))
    return -1;

  // generate list of specified views
  NAString viewsToBackup;
  if (genListOfObjs(viewList, viewsToBackup, prependOR, schPrivsList))
    return -1;

  // generate list of specified procs
  NAString procsToBackup;
  if (genListOfObjs(procList, procsToBackup, prependOR, schPrivsList))
    return -1;
  ...
  if (!procsToBackup.isNull())
    {
      ...
      if (genProcsUsedLibs(procsToBackup, procsUsedLibs, cliInterface))
        return -1;
    }

  // generate list of specified libs
  NAString libsToBackup;
  if (genListOfObjs(libList, libsToBackup, prependOR, schPrivsList))
    return -1;
  ...
  if (!libsToBackup.isNull())
    {
      ...
      if (genLibsUsedProcs(libsToBackup, libsUsedProcs, cliInterface))
        return -1;
    }

  NAString objsToBackup;
  if (!tablesToBackup.isNull() || !viewsToBackup.isNull() ||
      !procsToBackup.isNull()  || !libsToBackup.isNull())
    {
      if (prependOR)
        objsToBackup = " or ";
  ...
  if (NOT tablesToBackup.isNull())
    {
      if (genLobObjs(tableList, srcSch, cliInterface, lobsQueue, lobObjs))
        return -1;
      ...
      if (genIndexObjs(tableList, srcSch, cliInterface, indexesQueue, indexesObjs))
        return -1;
      ...
      if (genReferencedObjs(tableList, srcSch, cliInterface, refsQueue, refsObjs))
        return -1;
  ...
  char wherePred[2000 + schsToBackup.length() + objsToBackup.length() +
                 lobObjs.length() + indexesObjs.length() + refsObjs.length()];
  if (schsToBackup.isNull() && objsToBackup.isNull())
    {
      NAString wherep = " ( O.catalog_name = '%s' "
    ...
    {
      str_sprintf(wherePred, " ( O.catalog_name = '%s' and (%s %s %s %s %s ) and O.object_name not like 'TRAF_SAMPLE%%' ) ",
                  TRAFODION_SYSCAT_LIT,
                  (schsToBackup.isNull() ? " " : schsToBackup.data()),
                  (objsToBackup.isNull() ? " " : objsToBackup.data()),
                  (lobObjs.isNull() ? " " : lobObjs.data()),
                  (indexesObjs.isNull() ? " " : indexesObjs.data()),
                  (refsObjs.isNull() ? " " : refsObjs.data())
                  );
    }
  ...
  if (((! tableList) || (tableList->entries() == 0)) &&
      ((! viewList) || (viewList->entries() == 0)) &&
      ((! procList) || (procList->entries() == 0)) &&
      ((! libList) || (libList->entries() == 0)))
    {
...
short CmpSeabaseDDL::cleanupTgtObjectsFromMD(ExeCliInterface * cliInterface,
                                             char * objUIDsWherePred,
                                             char * srcSch)
{
  Lng32 cliRC = 0;
  ...
  str_sprintf(query, " select catalog_name, schema_name, object_name, object_type from %s.\"%s\".%s O where %s ",
              TRAFODION_SYSCAT_LIT, srcSch, SEABASE_OBJECTS, 
              objUIDsWherePred);
  cliRC = cliInterface->fetchAllRows(objsQueue, query, 
                                     0, FALSE, FALSE, TRUE);
  if (cliRC < 0)
    {
      cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
  ...
  for (Lng32 idx = 0; idx < objsQueue->numEntries(); idx++)
    {
      OutputInfo * vi = (OutputInfo*)objsQueue->getNext(); 
      ...
      str_sprintf(query, "cleanup %s %s.\"%s\".\"%s\", no hbase drop",
                  (objType == COM_BASE_TABLE_OBJECT_LIT ? "table" :
                   (objType == COM_INDEX_OBJECT_LIT ? "index" : "object")),
...
short CmpSeabaseDDL::invalidateTgtObjects(ExeCliInterface * cliInterface,
                                          char * objUIDsWherePred,
                                          char * tgtSch,
                                          char * srcSch,
                                          SharedCacheDDLInfoList &sharedCacheList)
{
  ...
  str_sprintf(objsQry, " select O.catalog_name, O.schema_name, O.object_name, O.object_type, O.flags"
              ...
              TRAFODION_SYSCAT_LIT, tgtSch, SEABASE_OBJECTS,
              TRAFODION_SYSCAT_LIT, srcSch, SEABASE_OBJECTS,
              objUIDsWherePred);
  cliRC = cliInterface->fetchAllRows(objsQueue, objsQry, 
                                     0, FALSE, FALSE, TRUE);
  if (cliRC < 0)
    {
      cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
  ...
  for (Lng32 idx = 0; idx < objsQueue->numEntries(); idx++)
    {
      ...
      NAString objType((char*)vi->get(3));
      Int64 objectFlags = *(Int64*)vi->get(4);
      ...
      if ((CmpCommon::getDefault(TRAF_ENABLE_METADATA_LOAD_IN_SHARED_CACHE) == DF_ON) &&
          (isMDflagsSet(objectFlags, MD_OBJECTS_STORED_DESC)))
        {
          ...
          SharedCacheDDLInfo newDDLObj(cachedObjName,
                                       (ComObjectType) ((strcmp(objType, COM_BASE_TABLE_OBJECT_LIT) == 0) 
                                        ? COM_BASE_TABLE_OBJECT : ((strcmp(objType, COM_INDEX_OBJECT_LIT) == 0) 
                                        ? COM_INDEX_OBJECT : COM_PRIVATE_SCHEMA_OBJECT)),
      ...
      ActiveSchemaDB()->getNATableDB()->removeNATable
        (cn,
         ...
         (ComObjectType) ((strcmp(objType, COM_BASE_TABLE_OBJECT_LIT) == 0) ? COM_BASE_TABLE_OBJECT :
...
short CmpSeabaseDDL::deleteTgtObjectsFromMD(RelBackupRestore *brExpr,
                                            ExeCliInterface * cliInterface,
                                            NAString &objUIDsWherePred,
                                            char * tgtSch, char * tgtSch2,
                                            char * srcSch)
{
  Lng32 cliRC;

  char objUIDsQry[1000 + objUIDsWherePred.length()];
  str_sprintf(objUIDsQry, " ( select object_uid from %s.\"%s\".%s O where %s ) ",
              TRAFODION_SYSCAT_LIT, srcSch, SEABASE_OBJECTS, 
              objUIDsWherePred.data());

  char query[4000 + sizeof(objUIDsQry)];

  cliRC = cliInterface->holdAndSetCQDs("NESTED_JOINS, MERGE_JOINS, HASH_JOINS, TRAF_BR_LOCKED_OBJECTS_QUERY_TYPE",
  ...
  str_sprintf(query, "delete from %s.\"%s\".%s where object_uid in %s",
              TRAFODION_SYSCAT_LIT, tgtSch, SEABASE_OBJECTS,
              objUIDsQry);
  cliRC = cliInterface->executeImmediate(query);
  if (cliRC < 0)
  ...
  str_sprintf(query, "delete from %s.\"%s\".%s where object_uid in %s",
              TRAFODION_SYSCAT_LIT, tgtSch, SEABASE_COLUMNS,
              objUIDsQry);
  cliRC = cliInterface->executeImmediate(query);
  if (cliRC < 0)
    {
  ...
  str_sprintf(query, "delete from %s.\"%s\".%s where object_uid in %s",
              TRAFODION_SYSCAT_LIT, tgtSch, SEABASE_KEYS,
              objUIDsQry);
  cliRC = cliInterface->executeImmediate(query);
  if (cliRC < 0)
    {
  ...
  str_sprintf(query, "delete from %s.\"%s\".%s where table_uid in %s",
              TRAFODION_SYSCAT_LIT, tgtSch, SEABASE_TABLES,
              objUIDsQry);
  cliRC = cliInterface->executeImmediate(query);
  if (cliRC < 0)
    {
  ...
  str_sprintf(query, "delete from %s.\"%s\".%s where base_table_uid in %s",
              TRAFODION_SYSCAT_LIT, tgtSch, SEABASE_INDEXES,
              objUIDsQry);
  cliRC = cliInterface->executeImmediate(query);
  if (cliRC < 0)
    {
  ...
  str_sprintf(query, "delete from %s.\"%s\".%s where text_uid in %s ",
              TRAFODION_SYSCAT_LIT, tgtSch, SEABASE_TEXT,
              objUIDsQry);
  cliRC = cliInterface->executeImmediate(query);
  if (cliRC < 0)
    {
  ...
  str_sprintf(query, "delete from %s.\"%s\".%s where udr_uid in %s ",
              TRAFODION_SYSCAT_LIT, tgtSch, SEABASE_ROUTINES,
              objUIDsQry);
  cliRC = cliInterface->executeImmediate(query);
  if (cliRC < 0)
    {
  ...
  str_sprintf(query, "delete from %s.\"%s\".%s where library_uid in %s ",
              TRAFODION_SYSCAT_LIT, tgtSch, SEABASE_LIBRARIES,
              objUIDsQry);
  cliRC = cliInterface->executeImmediate(query);
  if (cliRC < 0)
    {
  ...
  str_sprintf(query, "delete from %s.\"%s\".%s where using_library_uid in %s ",
              TRAFODION_SYSCAT_LIT, tgtSch, SEABASE_LIBRARIES_USAGE,
              objUIDsQry);
  cliRC = cliInterface->executeImmediate(query);
  if (cliRC < 0)
    {
  ...
  // delete lob col info
  str_sprintf(query, "delete from %s.\"%s\".%s where table_uid in %s ",
              TRAFODION_SYSCAT_LIT, tgtSch, SEABASE_LOB_COLUMNS,
              objUIDsQry);
  cliRC = cliInterface->executeImmediate(query);
  if (cliRC < 0)
    {
  ...
  str_sprintf(query, "delete from %s.\"%s\".%s where text_uid in (select constraint_uid from %s.\"%s\".%s where table_uid in %s and constraint_type = 'C')",
              TRAFODION_SYSCAT_LIT, tgtSch, SEABASE_TEXT,
              TRAFODION_SYSCAT_LIT, srcSch, SEABASE_TABLE_CONSTRAINTS,
              objUIDsQry);
  cliRC = cliInterface->executeImmediate(query);
  if (cliRC < 0)
    {
  ...
  str_sprintf(query, "delete from %s.\"%s\".%s where table_uid in %s",
              TRAFODION_SYSCAT_LIT, tgtSch, SEABASE_TABLE_CONSTRAINTS,
              objUIDsQry);
  cliRC = cliInterface->executeImmediate(query);
  if (cliRC < 0)
    {
  ...
  str_sprintf(query, "delete from %s.\"%s\".%s where unique_constraint_uid in (select constraint_uid from %s.\"%s\".%s where table_uid in %s and constraint_type = 'U')",
              TRAFODION_SYSCAT_LIT, tgtSch, SEABASE_REF_CONSTRAINTS,
              TRAFODION_SYSCAT_LIT, srcSch, SEABASE_TABLE_CONSTRAINTS,
              objUIDsQry);
  cliRC = cliInterface->executeImmediate(query);
  if (cliRC < 0)
  ...
  str_sprintf(query, "delete from %s.\"%s\".%s where unique_constraint_uid in (select constraint_uid from %s.\"%s\".%s where table_uid in %s and constraint_type = 'U')",
              TRAFODION_SYSCAT_LIT, tgtSch, SEABASE_UNIQUE_REF_CONSTR_USAGE,
              TRAFODION_SYSCAT_LIT, srcSch, SEABASE_TABLE_CONSTRAINTS,
              objUIDsQry);
  cliRC = cliInterface->executeImmediate(query);
  if (cliRC < 0)
  ...
  str_sprintf(query, "delete from %s.\"%s\".%s where ref_constraint_uid in (select constraint_uid from %s.\"%s\".%s where table_uid in %s and constraint_type = 'F')",
              TRAFODION_SYSCAT_LIT, tgtSch, SEABASE_REF_CONSTRAINTS,
              TRAFODION_SYSCAT_LIT, srcSch, SEABASE_TABLE_CONSTRAINTS,
              objUIDsQry);
  cliRC = cliInterface->executeImmediate(query);
  ...
  str_sprintf(query, "delete from %s.\"%s\".%s where foreign_constraint_uid in (select constraint_uid from %s.\"%s\".%s where table_uid in %s and constraint_type = 'F')",
              TRAFODION_SYSCAT_LIT, tgtSch, SEABASE_UNIQUE_REF_CONSTR_USAGE,
              TRAFODION_SYSCAT_LIT, srcSch, SEABASE_TABLE_CONSTRAINTS,
              objUIDsQry);
  cliRC = cliInterface->executeImmediate(query);
  ...
  str_sprintf(query, "delete from %s.\"%s\".%s where object_uid in (select constraint_uid from %s.\"%s\".%s where table_uid in %s)",
              TRAFODION_SYSCAT_LIT, tgtSch, SEABASE_OBJECTS,
              TRAFODION_SYSCAT_LIT, srcSch, SEABASE_TABLE_CONSTRAINTS,
              objUIDsQry);
  cliRC = cliInterface->executeImmediate(query);
  ...
  str_sprintf(query, "delete from %s.\"%s\".%s where object_uid in (select index_uid from %s.\"%s\".%s where base_table_uid in %s)",
              TRAFODION_SYSCAT_LIT, tgtSch, SEABASE_OBJECTS,
              TRAFODION_SYSCAT_LIT, srcSch, SEABASE_INDEXES,
              objUIDsQry);
  cliRC = cliInterface->executeImmediate(query);
  if (cliRC < 0)
    {
  ...
  str_sprintf(query, "delete from %s.\"%s\".%s where table_uid in (select index_uid from %s.\"%s\".%s where base_table_uid in %s)",
              TRAFODION_SYSCAT_LIT, tgtSch, SEABASE_TABLES,
              TRAFODION_SYSCAT_LIT, srcSch, SEABASE_INDEXES,
              objUIDsQry);
  cliRC = cliInterface->executeImmediate(query);
  ...
  str_sprintf(query, "delete from %s.\"%s\".%s where seq_uid in "
              "(select O.object_uid from %s.\"%s\".%s O, %s.\"%s\".%s O2 "
              " where  (O2.catalog_name, O2.schema_name) = (O.catalog_name, O.schema_name) "
              " and O2.object_type = 'SG' "
              " and %s )",
              TRAFODION_SYSCAT_LIT, tgtSch, SEABASE_SEQ_GEN,
              TRAFODION_SYSCAT_LIT, tgtSch, SEABASE_OBJECTS,
              TRAFODION_SYSCAT_LIT, srcSch, SEABASE_OBJECTS,
              objUIDsWherePred.data());
  cliRC = cliInterface->executeImmediate(query);
  ...
  str_sprintf(query, "delete from %s.\"%s\".%s where object_uid in "
              "(select O.object_uid from %s.\"%s\".%s O, %s.\"%s\".%s O2 "
              " where  (O2.catalog_name, O2.schema_name) = (O.catalog_name, O.schema_name) "
              " and O2.object_type = 'SG' "
              " and %s )",
  ...
  if (isAuthorizationEnabled())
    {
      str_sprintf(query, "delete from %s.\"%s\".%s where object_uid in %s",
                  TRAFODION_SYSCAT_LIT, tgtSch2, PRIVMGR_OBJECT_PRIVILEGES,
                  objUIDsQry);
      cliRC = cliInterface->executeImmediate(query);
      if (cliRC < 0)
        {
      ...
      str_sprintf(query, "delete from %s.\"%s\".%s where object_uid in %s",
                  TRAFODION_SYSCAT_LIT, tgtSch2, PRIVMGR_COLUMN_PRIVILEGES,
                  objUIDsQry);
      cliRC = cliInterface->executeImmediate(query);
      if (cliRC < 0)
        {
...
short CmpSeabaseDDL::copyBRSrcToTgtBasic(RelBackupRestore *brExpr,
                                         ExeCliInterface * cliInterface,
                                         const char * objUIDsWherePred,
                                         const char * srcSch1, const char * srcSch2,
                                         const char * tgtSch1, const char * tgtSch2,
                                         NAString *schPrivsList)
{
  Lng32 cliRC;
  ...
  str_sprintf(objUIDsQry, " ( select O.object_uid from %s.\"%s\".%s O where %s for skip conflict access) ",
              TRAFODION_SYSCAT_LIT, srcSch1, SEABASE_OBJECTS, 
              objUIDsWherePred);
  ...
  str_sprintf(query, "upsert into %s.\"%s\".%s select * from %s.\"%s\".%s O where %s for skip conflict access",
              TRAFODION_SYSCAT_LIT, tgtSch1, SEABASE_OBJECTS,
              TRAFODION_SYSCAT_LIT, srcSch1, SEABASE_OBJECTS,
              objUIDsWherePred);
  ...
  str_sprintf(query, "upsert into %s.\"%s\".%s select * from %s.\"%s\".%s  where object_uid in %s  for skip conflict access ",
              TRAFODION_SYSCAT_LIT, tgtSch1, SEABASE_COLUMNS,
              TRAFODION_SYSCAT_LIT, srcSch1, SEABASE_COLUMNS,
              objUIDsQry);
  cliRC = cliInterface->executeImmediate(query);
  if (cliRC < 0)
    {
  ...
  str_sprintf(query, "upsert into %s.\"%s\".%s select * from %s.\"%s\".%s  where object_uid in %s  for skip conflict access ",
              TRAFODION_SYSCAT_LIT, tgtSch1, SEABASE_KEYS,
              TRAFODION_SYSCAT_LIT, srcSch1, SEABASE_KEYS,
              objUIDsQry);
  cliRC = cliInterface->executeImmediate(query);
  if (cliRC < 0)
    {
  ...
  str_sprintf(query, "upsert into %s.\"%s\".%s select * from %s.\"%s\".%s  where table_uid in %s  for skip conflict access ",
              TRAFODION_SYSCAT_LIT, tgtSch1, SEABASE_TABLES,
              TRAFODION_SYSCAT_LIT, srcSch1, SEABASE_TABLES,
              objUIDsQry);
  cliRC = cliInterface->executeImmediate(query);
  if (cliRC < 0)
    {
  ...
  str_sprintf(query, "upsert into %s.\"%s\".%s select * from %s.\"%s\".%s  where table_uid in %s  for skip conflict access ",
              TRAFODION_SYSCAT_LIT, tgtSch1, SEABASE_TABLE_CONSTRAINTS,
              TRAFODION_SYSCAT_LIT, srcSch1, SEABASE_TABLE_CONSTRAINTS,
              objUIDsQry);
  cliRC = cliInterface->executeImmediate(query);
  if (cliRC < 0)
    {
  ...
  str_sprintf(query, "upsert into %s.\"%s\".%s select * from %s.\"%s\".%s  where text_uid in %s and text_type != %d for skip conflict access ",
              TRAFODION_SYSCAT_LIT, tgtSch1, SEABASE_TEXT,
              TRAFODION_SYSCAT_LIT, srcSch1, SEABASE_TEXT,
              objUIDsQry, COM_USER_QUERYCACHE_TEXT);
  {
    Int64 numRows = 0;
  ...
  str_sprintf(query, "upsert into %s.\"%s\".%s select * from %s.\"%s\".%s  where table_uid in %s for skip conflict access ",
              TRAFODION_SYSCAT_LIT, tgtSch1, SEABASE_LOB_COLUMNS,
              TRAFODION_SYSCAT_LIT, srcSch1, SEABASE_LOB_COLUMNS,
              objUIDsQry);
  cliRC = cliInterface->executeImmediate(query);
  if (cliRC < 0)
    {
  ...
  str_sprintf(query, "upsert into %s.\"%s\".%s select * from %s.\"%s\".%s where object_uid in (select constraint_uid from %s.\"%s\".%s where table_uid in %s ) for skip conflict access ",
              TRAFODION_SYSCAT_LIT, tgtSch1, SEABASE_OBJECTS,
              TRAFODION_SYSCAT_LIT, srcSch1, SEABASE_OBJECTS,
              TRAFODION_SYSCAT_LIT, srcSch1, SEABASE_TABLE_CONSTRAINTS,
              objUIDsQry);
...
short CmpSeabaseDDL::copyBRSrcToTgt(RelBackupRestore *brExpr,
                                    ExeCliInterface * cliInterface,
                                    const char * objUIDsWherePred,
                                    const char * srcSch1, const char * srcSch2,
                                    const char * tgtSch1, const char * tgtSch2,
                                    NAString *schPrivsList)
{
  ...
  str_sprintf(backupSchPredBuf, " (O.catalog_name = '%s' and O.schema_name = '%s' and O.object_type = 'BT') ",
              TRAFODION_SYSCAT_LIT, 
              (brExpr->backup() ? tgtSch1 : srcSch1));
  backupSchPred = backupSchPredBuf;
  if (copyBRSrcToTgtBasic(brExpr, cliInterface, backupSchPred,
                          srcSch1, srcSch2, tgtSch1, tgtSch2, schPrivsList) < 0)
    {
      goto label_error;
    }

  // now copy regular objects from src to tgt.
  if (copyBRSrcToTgtBasic(brExpr, cliInterface, objUIDsWherePred,
                          srcSch1, srcSch2, tgtSch1, tgtSch2, schPrivsList) < 0)
    {
      goto label_error;
    }
  ...
  str_sprintf(query, "upsert into %s.\"%s\".%s select * from %s.\"%s\".%s where text_uid in (select constraint_uid from %s.\"%s\".%s where constraint_type = 'C' and table_uid in %s )  for skip conflict access ",
              TRAFODION_SYSCAT_LIT, tgtSch1, SEABASE_TEXT,
              TRAFODION_SYSCAT_LIT, srcSch1, SEABASE_TEXT,
              TRAFODION_SYSCAT_LIT, srcSch1, SEABASE_TABLE_CONSTRAINTS,
              objUIDsQry);
  ...
  str_sprintf(query, "upsert into %s.\"%s\".%s select * from %s.\"%s\".%s  where base_table_uid in %s for skip conflict access ",
              TRAFODION_SYSCAT_LIT, tgtSch1, SEABASE_INDEXES,
              TRAFODION_SYSCAT_LIT, srcSch1, SEABASE_INDEXES,
              objUIDsQry);
  cliRC = cliInterface->executeImmediate(query);
  if (cliRC < 0)
    {
  ...
  str_sprintf(query, "upsert into %s.\"%s\".%s select * from %s.\"%s\".%s  where seq_uid in %s for skip conflict access ",
              TRAFODION_SYSCAT_LIT, tgtSch1, SEABASE_SEQ_GEN,
              TRAFODION_SYSCAT_LIT, srcSch1, SEABASE_SEQ_GEN,
              objUIDsQry);
  cliRC = cliInterface->executeImmediate(query);
  if (cliRC < 0)
    {
  ...
  str_sprintf(query, "upsert into %s.\"%s\".%s select * from %s.\"%s\".%s  where view_uid in %s for skip conflict access ",
              TRAFODION_SYSCAT_LIT, tgtSch1, SEABASE_VIEWS,
              TRAFODION_SYSCAT_LIT, srcSch1, SEABASE_VIEWS,
              objUIDsQry);
  cliRC = cliInterface->executeImmediate(query);
  if (cliRC < 0)
    {
  ...
  str_sprintf(query, "upsert into %s.\"%s\".%s select * from %s.\"%s\".%s  where using_view_uid in %s for skip conflict access ",
              TRAFODION_SYSCAT_LIT, tgtSch1, SEABASE_VIEWS_USAGE,
              TRAFODION_SYSCAT_LIT, srcSch1, SEABASE_VIEWS_USAGE,
              objUIDsQry);
  cliRC = cliInterface->executeImmediate(query);
  if (cliRC < 0)
    {
  ...
  str_sprintf(query, "upsert into %s.\"%s\".%s select * from %s.\"%s\".%s  where udr_uid in %s for skip conflict access ",
              TRAFODION_SYSCAT_LIT, tgtSch1, SEABASE_ROUTINES,
              TRAFODION_SYSCAT_LIT, srcSch1, SEABASE_ROUTINES,
              objUIDsQry);
  cliRC = cliInterface->executeImmediate(query);
  if (cliRC < 0)
    {
  ...
  if (brExpr->restore())
  {
      ...
      str_sprintf(query, "update %s.\"%s\".%s set library_uid = (select T1.object_uid from %s.\"%s\".%s T1 where T1.object_name= '%s' and T1.object_type='%s') where external_name like 'org.trafodion.sql.udr.spsql.SPSQL.callSPSQL%%';",
              TRAFODION_SYSCAT_LIT, tgtSch1, SEABASE_ROUTINES,
              TRAFODION_SYSCAT_LIT, tgtSch1, SEABASE_OBJECTS,
  ...
  str_sprintf(query, "upsert into %s.\"%s\".%s select * from %s.\"%s\".%s  where library_uid in %s for skip conflict access ",
              TRAFODION_SYSCAT_LIT, tgtSch1, SEABASE_LIBRARIES,
              TRAFODION_SYSCAT_LIT, srcSch1, SEABASE_LIBRARIES,
              objUIDsQry);
  cliRC = cliInterface->executeImmediate(query);
  if (cliRC < 0)
    {
  ...
  str_sprintf(query, "upsert into %s.\"%s\".%s select * from %s.\"%s\".%s  where using_library_uid in %s for skip conflict access ",
              TRAFODION_SYSCAT_LIT, tgtSch1, SEABASE_LIBRARIES_USAGE,
              TRAFODION_SYSCAT_LIT, srcSch1, SEABASE_LIBRARIES_USAGE,
              objUIDsQry);
  cliRC = cliInterface->executeImmediate(query);
  if (cliRC < 0)
    {
  ...
  str_sprintf(query, "upsert into %s.\"%s\".%s select * from %s.\"%s\".%s where unique_constraint_uid in (select constraint_uid from %s.\"%s\".%s where constraint_type = 'U' and table_uid in %s )  for skip conflict access ",
              TRAFODION_SYSCAT_LIT, tgtSch1, SEABASE_REF_CONSTRAINTS,
              TRAFODION_SYSCAT_LIT, srcSch1, SEABASE_REF_CONSTRAINTS,
              TRAFODION_SYSCAT_LIT, srcSch1, SEABASE_TABLE_CONSTRAINTS,
              objUIDsQry);
  cliRC = cliInterface->executeImmediate(query);
  if (cliRC < 0)
  ...
  str_sprintf(query, "upsert into %s.\"%s\".%s select * from %s.\"%s\".%s where unique_constraint_uid in (select constraint_uid from %s.\"%s\".%s where constraint_type = 'U' and table_uid in %s ) for skip conflict access ",
              TRAFODION_SYSCAT_LIT, tgtSch1, SEABASE_UNIQUE_REF_CONSTR_USAGE,
              TRAFODION_SYSCAT_LIT, srcSch1, SEABASE_UNIQUE_REF_CONSTR_USAGE,
              TRAFODION_SYSCAT_LIT, srcSch1, SEABASE_TABLE_CONSTRAINTS,
              objUIDsQry);
  cliRC = cliInterface->executeImmediate(query);
  if (cliRC < 0)
  ...
  str_sprintf(query, "upsert into %s.\"%s\".%s select * from %s.\"%s\".%s where ref_constraint_uid in (select constraint_uid from %s.\"%s\".%s where constraint_type = 'F' and table_uid in %s ) for skip conflict access ",
              TRAFODION_SYSCAT_LIT, tgtSch1, SEABASE_REF_CONSTRAINTS,
              TRAFODION_SYSCAT_LIT, srcSch1, SEABASE_REF_CONSTRAINTS,
              TRAFODION_SYSCAT_LIT, srcSch1, SEABASE_TABLE_CONSTRAINTS,
              objUIDsQry);
  ...
  str_sprintf(query, "upsert into %s.\"%s\".%s select * from %s.\"%s\".%s where foreign_constraint_uid in (select constraint_uid from %s.\"%s\".%s where constraint_type = 'F' and table_uid in %s ) for skip conflict access ",
              TRAFODION_SYSCAT_LIT, tgtSch1, SEABASE_UNIQUE_REF_CONSTR_USAGE,
              TRAFODION_SYSCAT_LIT, srcSch1, SEABASE_UNIQUE_REF_CONSTR_USAGE,
              TRAFODION_SYSCAT_LIT, srcSch1, SEABASE_TABLE_CONSTRAINTS,
              objUIDsQry);
  ...
  str_sprintf(query, "upsert into %s.\"%s\".%s select * from %s.\"%s\".%s where object_uid in (select index_uid from %s.\"%s\".%s where base_table_uid in %s ) for skip conflict access ",
              TRAFODION_SYSCAT_LIT, tgtSch1, SEABASE_OBJECTS,
              TRAFODION_SYSCAT_LIT, srcSch1, SEABASE_OBJECTS,
              TRAFODION_SYSCAT_LIT, srcSch1, SEABASE_INDEXES,
              objUIDsQry);
  ...
  str_sprintf(query, "upsert into %s.\"%s\".%s select * from %s.\"%s\".%s where table_uid in (select index_uid from %s.\"%s\".%s where base_table_uid in %s ) for skip conflict access ",
              TRAFODION_SYSCAT_LIT, tgtSch1, SEABASE_TABLES,
              TRAFODION_SYSCAT_LIT, srcSch1, SEABASE_TABLES,
              TRAFODION_SYSCAT_LIT, srcSch1, SEABASE_INDEXES,
              objUIDsQry);
  ...
  str_sprintf(query, "upsert into %s.\"%s\".%s select * from %s.\"%s\".%s "
              " where seq_uid in "
  ...
  if (isAuthorizationEnabled())
    {
      str_sprintf(query, "upsert into %s.\"%s\".%s select * from %s.\"%s\".%s  where object_uid in %s for skip conflict access ",
                  TRAFODION_SYSCAT_LIT, tgtSch2, PRIVMGR_OBJECT_PRIVILEGES,
                  TRAFODION_SYSCAT_LIT, srcSch2, PRIVMGR_OBJECT_PRIVILEGES,
                  objUIDsQry);
      cliRC = cliInterface->executeImmediate(query);
      if (cliRC < 0)
        {
      ...
      str_sprintf(query, "upsert into %s.\"%s\".%s select * from %s.\"%s\".%s  where object_uid in %s for skip conflict access ",
                  TRAFODION_SYSCAT_LIT, tgtSch2, PRIVMGR_COLUMN_PRIVILEGES,
                  TRAFODION_SYSCAT_LIT, srcSch2, PRIVMGR_COLUMN_PRIVILEGES,
                  objUIDsQry);
      cliRC = cliInterface->executeImmediate(query);
      if (cliRC < 0)
        {
      ...
      str_sprintf(query, "upsert into %s.\"%s\".%s select * from %s.\"%s\".%s  where schema_uid in %s for skip conflict access ",
                  TRAFODION_SYSCAT_LIT, tgtSch2, PRIVMGR_SCHEMA_PRIVILEGES,
                  TRAFODION_SYSCAT_LIT, srcSch2, PRIVMGR_SCHEMA_PRIVILEGES,
                  objUIDsQry);
      cliRC = cliInterface->executeImmediate(query);
      if (cliRC < 0)
        {
      ...
      if (schPrivsList && schPrivsList->length() > 0)
        {
          str_sprintf(query, "upsert into %s.\"%s\".%s select * from %s.\"%s\".%s  where "
    ...
    if (brExpr->restore() && brExpr->tableList())
    {
      for (CollIndex i = 0; i < brExpr->tableList()->entries(); i++)
      {
        ...
        NAString schemaName;
        if (NOT dn->getQualifiedName().getSchemaName().isNull())
          schemaName = dn->getQualifiedName().getSchemaName();
        if (NOT schemaName.isNull())
        {
          ComObjectType objectType;
          Int32 schemaOwnerID = 0;
          Int64 schemaUID = getObjectTypeandOwner(cliInterface, TRAFODION_SYSCAT_LIT, schemaName.data(),
                                                  SEABASE_SCHEMA_OBJECTNAME, objectType, schemaOwnerID);

          if (schemaUID == -1)
          {
            // schema does not exist
            snprintf(query, sizeof(query), "upsert into %s.\"%s\".%s select * from %s.\"%s\".%s  where "
...
short CmpSeabaseDDL::backupHistStats(ExeCliInterface * cliInterface,
                                     ElemDDLList * tableList,
                                     char * backupSch)
{
  Lng32 cliRC = 0;
  ...
  for (CollIndex i = 0; i < tableList->entries(); i++)
    {
      ...
      str_sprintf(query, "upsert into %s.\"%s\".%s select * from %s.\"%s\".%s where table_uid = (select object_uid from %s.\"%s\".%s where catalog_name = '%s' and schema_name = '%s' and object_name = '%s')",
                  TRAFODION_SYSCAT_LIT, backupSch, HBASE_HIST_NAME,
                  TRAFODION_SYSCAT_LIT, 
                  dn->getQualifiedName().getSchemaName().data(), 
                  HBASE_HIST_NAME,
                  TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_OBJECTS, 
                  TRAFODION_SYSCAT_LIT, 
                  dn->getQualifiedName().getSchemaName().data(), 
      ...
      str_sprintf(query, "upsert into %s.\"%s\".%s select * from %s.\"%s\".%s where table_uid = (select object_uid from %s.\"%s\".%s where catalog_name = '%s' and schema_name = '%s' and object_name = '%s')",
                  TRAFODION_SYSCAT_LIT, backupSch, HBASE_HISTINT_NAME,
                  TRAFODION_SYSCAT_LIT, 
                  dn->getQualifiedName().getSchemaName().data(), 
                  HBASE_HISTINT_NAME,
                  TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_OBJECTS, 
      ...
      str_sprintf(query, "upsert into %s.\"%s\".%s select * from %s.\"%s\".%s where table_uid = (select object_uid from %s.\"%s\".%s where catalog_name = '%s' and schema_name = '%s' and object_name = '%s')",
                  TRAFODION_SYSCAT_LIT, backupSch, HBASE_PERS_SAMP_NAME,
                  TRAFODION_SYSCAT_LIT, 
                  dn->getQualifiedName().getSchemaName().data(), 
...
short CmpSeabaseDDL::expandAndValidate(
     ExeCliInterface *cliInterface,
     ElemDDLQualName * dn,
     const char * objectType,
     NABoolean validateObjects,
     NAString &currCatName, NAString &currSchName)
{
  ...
  if (dn->getQualifiedName().numberExpanded() < 3)
    {
      ...
      SchemaName sn(csn.getSchemaNamePart().getInternalName(),
  ...
  if (validateObjects)
    {
      if (CmpSqlSession::isValidVolatileSchemaName(
               (NAString&)dn->getQualifiedName().getSchemaName()))
        {
          *CmpCommon::diags() << DgSqlCode(-5050)
                              << DgString0("BACKUP")
      ...
      Int64 objUID =
        getObjectUID(cliInterface,
                     objName.getCatalogNamePartAsAnsiString(),
                     objName.getSchemaNamePartAsAnsiString(TRUE),
                     objName.getObjectNamePartAsAnsiString(TRUE),
                     objectType,NULL,
      ...
      if (objUID <= 0)
        {
          ...
          if (type == COM_BASE_TABLE_OBJECT_LIT)
            ...
            typeName = "view";
          else if ((type == COM_USER_DEFINED_ROUTINE_OBJECT_LIT) ||
                   (type == COM_STORED_PROCEDURE_OBJECT_LIT))
            typeName = "routine";
          ...
          if (typeName.isNull())
            ...
            reasonStr = NAString("Specified object ") + dn->getQualifiedName().getQualifiedNameAsString() + NAString(" does not exist or is not a ") + typeName + NAString(".");
...
short CmpSeabaseDDL::validateInputNames(ExeCliInterface * cliInterface,
                                        RelBackupRestore * brExpr,
                                        NABoolean validateObjects,
                                        NAString &currCatName, NAString &currSchName)
{
          ...
          if (validateObjects)
            {
              ...
              Int64 objUID = 
                getObjectUID(cliInterface,
                             dn->getQualifiedName().getSchemaName(),
                             dn->getQualifiedName().getObjectName(),
                             SEABASE_SCHEMA_OBJECTNAME,
                             COM_SHARED_SCHEMA_OBJECT_LIT,NULL,
                             NULL, NULL, FALSE, FALSE);
      ...
      for (CollIndex i = 0; i < tableList->entries(); i++)
        {
          ...
          if (isHBaseNativeTable(tableName))
              continue;
          if (expandAndValidate(cliInterface,
                                dn, COM_BASE_TABLE_OBJECT_LIT, validateObjects,
      ...
      for (CollIndex i = 0; i < viewList->entries(); i++)
        {
          ...
          if (expandAndValidate(cliInterface,
                                dn, COM_VIEW_OBJECT_LIT, validateObjects,
                                currCatName, currSchName))
            {
              return -1;
            }
      ...
      for (CollIndex i = 0; i < procList->entries(); i++)
        {
          ...
          if (expandAndValidate(cliInterface,
                                dn, COM_USER_DEFINED_ROUTINE_OBJECT_LIT, 
                                validateObjects,
                                currCatName, currSchName))
            {
              return -1;
...
short CmpSeabaseDDL::processBRShowObjects(RelBackupRestore * brExpr,
                                          NAArray<HbaseStr>* mdList,
                                          NAString &objUIDsWherePred,
                                          ExeCliInterface * cliInterface,
                                          CmpDDLwithStatusInfo * dws)
{
  ...
  if (brExpr->backup())
    {
      str_sprintf(objsQry, " select count(*) from %s.\"%s\".%s O where %s and O.object_type in ('" COM_LIBRARY_OBJECT_LIT"') ",
                  TRAFODION_SYSCAT_LIT, sourceSch.data(), SEABASE_OBJECTS, 
                  objUIDsWherePred.data());
      Int64 rowCount = 0;
      Int32 length = 0;
      ...
      for (Int32 i = 0; i < mdList->entries(); i++)
        {
          ...
          NAString row = nameSpace + catName + NAString(".") +
            schName + "." + objName;
  ...
  str_sprintf(objsQry, " select case object_type when 'BT' then 'TABLE:   ' when 'IX' then 'INDEX:   ' when 'VI' then 'VIEW:    ' when 'UR' then 'ROUTINE: ' when 'LB' then 'LIBRARY: ' else ' ' end, case when T.text is not null then trim(T.text) || ':' else '' end,  trim(O.catalog_name), trim(O.schema_name), trim(O.object_name) from %s.\"%s\".%s O left join %s.\"%s\".%s T on O.object_uid = T.text_uid and T.text_type = %d where %s and O.object_type in ('" COM_BASE_TABLE_OBJECT_LIT"', '" COM_INDEX_OBJECT_LIT"', '" COM_VIEW_OBJECT_LIT"', '" COM_USER_DEFINED_ROUTINE_OBJECT_LIT"', '" COM_LIBRARY_OBJECT_LIT"') order by case object_type when 'BT' then 1 when 'IX' then 2 when 'VI' then 3 when 'UR' then 4 when 'LB' then 5 else 6 end, 3,4,5 ",
              TRAFODION_SYSCAT_LIT, sourceSch.data(), SEABASE_OBJECTS, 
              TRAFODION_SYSCAT_LIT, sourceSch.data(), SEABASE_TEXT, 
              COM_OBJECT_NAMESPACE,
              objUIDsWherePred.data());

  cliRC = cliInterface->fetchAllRows(userObjsQueue, objsQry, 
                                     0, FALSE, FALSE, TRUE);
  if (cliRC < 0)
    {
      cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
  ...
  if (userObjsQueue->numEntries() == 0)
    {
      *CmpCommon::diags() << DgSqlCode(-5050)
                          << DgString0(brExpr->backup() ? "BACKUP" : "RESTORE")
                          << DgString1("Reason: Specified list contains no objects.");

      return -1;
    }

  for (Lng32 idx = 0; idx < userObjsQueue->numEntries(); idx++)
    {
      ...
      NAString fullRow(obj + (NOT nameSpace.isNull() ? nameSpace : NAString("")) + catName + "." + schName + "." + objName);
      updateQueueWithOI(fullRow.data(), objsQueue, STMTHEAP);      
    }

  Queue *lobLocQueue = NULL;
  str_sprintf(objsQry, "select O.object_uid, trim(O.schema_name), trim(O.catalog_name) || '.' || trim(O.schema_name) || '.' || trim(O.object_name) from %s.\"%s\".%s O, %s.\"%s\".%s C where O.object_uid = C.object_uid and C.fs_data_type in (160,161) and %s and O.object_type in ('" COM_BASE_TABLE_OBJECT_LIT"', '" COM_INDEX_OBJECT_LIT"') group by 1,2,3",
              TRAFODION_SYSCAT_LIT, sourceSch.data(), SEABASE_OBJECTS,
              TRAFODION_SYSCAT_LIT, sourceSch.data(), SEABASE_COLUMNS,
              objUIDsWherePred.data());
  
  cliRC = cliInterface->fetchAllRows(lobLocQueue, objsQry, 
                                     0, FALSE, FALSE, TRUE);
  if (cliRC < 0)
    {
      cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
  ...
  if (brExpr->backup())
    {
      TextVec lobLocList;
      for (Lng32 idx = 0; idx < lobLocQueue->numEntries(); idx++)
        {
          OutputInfo * vi = (OutputInfo*)lobLocQueue->getNext();
          char * ptr = NULL;
          Lng32 len = 0;
          
          Int64 objUID = *(Int64*)vi->get(0);
          vi->get(1, ptr, len);
          ...
          Int32 lobFiles = 0;
          cliRC = genLobLocation(objUID, sch.data(), cliInterface, 
                                 lobLocList, lobFiles, TRUE);
          if (cliRC < 0)
            {
              return -1;
            }
  ...
  char numBuf[20];
  Int32 totalObjs = numMDs+numTables+numIndexes+numViews+numRoutines+numLibs
    + numLOBfiles;
  updateQueueWithOI(NAString("  Total:     ") + str_itoa(totalObjs, numBuf), objsQueue, STMTHEAP);
  if (numMDs > 0)
    updateQueueWithOI(NAString("  Metadata:  ") + str_itoa(numMDs, numBuf), objsQueue, STMTHEAP);
  if (numTables > 0)
    updateQueueWithOI(NAString("  Tables:    ") + str_itoa(numTables, numBuf), objsQueue, STMTHEAP);
  if (numIndexes > 0)
    updateQueueWithOI(NAString("  Indexes:   ") + str_itoa(numIndexes, numBuf), objsQueue, STMTHEAP);
  if (numViews > 0)
    updateQueueWithOI(NAString("  Views:     ") + str_itoa(numViews, numBuf), objsQueue, STMTHEAP);
  if (numRoutines > 0)
    updateQueueWithOI(NAString("  Routines:  ") + str_itoa(numRoutines, numBuf), objsQueue, STMTHEAP);
  if (numLibs > 0)
    updateQueueWithOI(NAString("  Libraries: ") + str_itoa(numLibs, numBuf), objsQueue, STMTHEAP);
  if (numLOBfiles > 0)
  ...
  populateBlackBox(cliInterface, objsQueue, 
                   blackBoxLen, blackBox);
...
static NABoolean adjacentSpaces(char lhs, char rhs) 
{ 
  return (lhs == rhs) && (lhs == ' '); 
}
...
static void extractTokens(std::string &s, std::vector<string> &tokens)
{
  ...
  std::string::iterator new_end = 
    std::unique(s.begin(), s.end(), adjacentSpaces);
...
static NABoolean containsTag(const char * inputStr, Int32 inputStrLen,
                             char * brExprTag,
                             NAString &oper, NAString &status)
{
  if ((! inputStr) || (inputStrLen == 0) || (! brExprTag))
    return FALSE;
...
NABoolean CmpSeabaseDDL::containsTagInSnapshotList(
     NAArray<HbaseStr>* snapshotList,
     char * brExprTag,
     NAString &oper, NAString &status)
{
  if ((! snapshotList) || (snapshotList->entries() == 0))
  ...
  for (Int32 i = 0; (i < numEntries); i++)
    {
      ...
      std::string::iterator new_end = 
        std::unique(str.begin(), str.end(), adjacentSpaces);
      str.erase(new_end, str.end());  

      if (containsTag(str.c_str(), str.length(), brExprTag, oper, status))
        return TRUE;
    }
...
NABoolean CmpSeabaseDDL::getTimeStampFromSnapshotList(
        NAArray<HbaseStr>* snapshotList,
        char * brExprTag,
        char * timeStamp,
...
short CmpSeabaseDDL::populateDiags(const char * oper, const char * errMsg,
...
short CmpSeabaseDDL::backupSubset(RelBackupRestore * brExpr, 
                                  ExeCliInterface * cliInterface,
                                  NAString &currCatName, NAString &currSchName,
                                  CmpDDLwithStatusInfo *dws)
{
  ...
  Int64 time0, time1, time2, time3, time4, time5, time6, time7, time8, time9;
  ...
  if (insertProgressTable(cliInterface, brExpr, "BACKUP",
  ...
  if (backupInit("BACKUP", FALSE) ||
          validateInputNames(cliInterface, brExpr, TRUE, currCatName, currSchName)) {
    updateProgressTable(cliInterface, brExpr->backupTag().data(),
                        "BACKUP", "Initialize", "Failed", -1, -1);
    return -1;
  }
  ...
  retcode = genBRObjUIDsWherePred(brExpr,
                                  brExpr->schemaList(), brExpr->tableList(),
                                  brExpr->viewList(), brExpr->procList(),
                                  brExpr->libList(),
                                  currCatName, currSchName,
                                  objUIDsWherePred, 
                                  (char *)"_MD_", 
  ...
  retcode =  verifyBRAuthority (objUIDsWherePred.data(), cliInterface, brExpr->showObjects(), 
                                TRUE, SEABASE_MD_SCHEMA, TRUE, NULL/*owner*/);
  if (retcode < 0)
    {
      *CmpCommon::diags() << DgSqlCode(-5050)
                          << DgString0("BACKUP")
  ...
  if (dws && brExpr->showObjects())
    {
      retcode = processBRShowObjects(brExpr, NULL, objUIDsWherePred,
                                     cliInterface, dws);
      if (retcode < 0)
        return -1;
  ...
  if (lockedTag.length() > 0)
    {
      if (lockedTag == "?")
        populateDiags("BACKUP", "Error occurred while checking snapshot metadata. ",
                      "ExpHbaseInterface::backupSubset", NULL, -1);
      else
        {
          NAString lockedMsg = "Reason: Snapshot metadata is currently locked for Tag: " + lockedTag;
          *CmpCommon::diags() << DgSqlCode(-5050)
                              << DgString0("BACKUP")
                              << DgString1(lockedMsg.data());
        }
      updateProgressTable(cliInterface, brExpr->backupTag().data(),
                          "BACKUP", "Initialize", "Failed", -1, -1);
      return -1;
  ...
  if (updateProgressTable(cliInterface, brExpr->backupTag().data(),
                          "BACKUP", "Initialize", "Completed", 0, 100) < 0)
    {
      return -1;
    }
      ...
      if (brExpr->getOverride()) // override it
        {
          ...
          retcode = deleteSnapshot(ehi, (char*)"BACKUP",
                                 brExpr->backupTag().data(), FALSE,
                                 TRUE, dropAllDependentTags);
          //Diagnostic already populated.
          if (retcode < 0)
            {
              ehi->close();
  ...
  if (retcode == 1)
    backupSchExists = TRUE;

  Int32 totalSetupMDsteps = 4;
  Int32 currSetupMDstep = 0;
  if (insertProgressTable(cliInterface, brExpr, "BACKUP",
                          "SetupMetadata", "Initiated", 
                          totalSetupMDsteps, currSetupMDstep) < 0)
    {
      return -1;
    }
  currSetupMDstep++;
  ...
  if (retcode < 0)
    {
      *CmpCommon::diags() << DgSqlCode(-5050)
      ...
      updateProgressTable(cliInterface, brExpr->backupTag().data(), "BACKUP",
                          "SetupMetadata", "Failed", -1, -1);
      return -1;
    }
      ...
      if (NOT (tag == brExpr->backupTag()))
        {
          *CmpCommon::diags() << DgSqlCode(-5050)
                              << DgString0("BACKUP")
                              << DgString1(NAString("Reason: Stored tag in TEXT table does not match the tag that is part of Backup Metadata name. This should not happen and is an internal error. ") + details);
          return -1;
        }
      else if (NOT (oper == "BACKUP"))
        {
          *CmpCommon::diags() << DgSqlCode(-5050)
                              << DgString0("BACKUP")
                              << DgString1(NAString("Reason: Backup Metadata is locked by a different operation which need to finish first.") + details);
          return -1;
        }
      else if (NOT brExpr->getOverride())
        {
          *CmpCommon::diags() << DgSqlCode(-5050)
                              << DgString0("BACKUP")
                              << DgString1(NAString("Reason: Backup Metadata is locked by another BACKUP operation. It needs to finish first or OVERRIDE option needs to be specified.") + details);
          return -1;
        }
    }

  if (updateProgressTable(cliInterface, brExpr->backupTag().data(),
                          "BACKUP", "SetupMetadata", "Initiated", 
                          currSetupMDstep, 
                          (currSetupMDstep*100)/totalSetupMDsteps) < 0)
    {
      return -1;
    }
  currSetupMDstep++;
  ...
  Lng32 numUR = 0;
  retcode = getUsersAndRoles(cliInterface, objUIDsWherePred.data(), SEABASE_MD_SCHEMA,
                             numUR, userOrRoles);
  if (retcode < 0)
    {
      *CmpCommon::diags() << DgSqlCode(-5050)
  ...
  retcode = checkIfObjectsLockedInMD(cliInterface, (char *)objUIDsWherePred.data(),
                                     (char *)SEABASE_MD_SCHEMA,
                                     "BACKUP",
                                     (char *)brExpr->backupTag().data(),
                                     lockedObjs);
  if (retcode != 0)
    {
      if (retcode < 0)
      ...
      if (retcode == 1)
        {
          *CmpCommon::diags() << DgSqlCode(-5050)
                              << DgString0("BACKUP")
                              << DgString1("Reason: Some of the objects to be backed up are locked. This could be due to another backup or restore operation in progress.");
      ...
      else if (retcode == 2)
        {
          *CmpCommon::diags() << DgSqlCode(-5050)
                              << DgString0("BACKUP")
                              << DgString1("Reason: Some of the objects to be backed up are locked but are not part of the specified tag or were not locked for a BACKUP operation. They cannot be overridden. Use GET ALL LOCKED OBJECTS command to see the list of locked objects.");
  ...
  if (lockObjectsInMD(cliInterface, "BACKUP", (char *)objUIDsWherePred.data(),
                      (char *)SEABASE_MD_SCHEMA, (char *)brExpr->backupTag().data(),
                      schemasOnlyBackup))
    {
  ...
  if (updateProgressTable(cliInterface, brExpr->backupTag().data(), "BACKUP", "SetupMetadata", 
                          "Initiated", currSetupMDstep, 
                          (currSetupMDstep*100)/totalSetupMDsteps) < 0)
    {
      unlockBackupMD(brExpr, cliInterface, "BACKUP");
      return -1;
  ...
  str_sprintf(query, "select case when T.text is not null then trim(T.text) || ':' else '' end || trim(O.catalog_name) || '.' || trim(O.schema_name) || '.' || trim(O.object_name), O.flags, NVL(C.object_uid, 0), trim(O.schema_name) from %s.\"%s\".%s O left join %s.\"%s\".%s T on O.object_uid = T.text_uid and T.text_type = %d left join %s.\"%s\".%s C on O.object_uid = C.object_uid and C.fs_data_type in (160,161) where %s and O.object_type in ('" COM_BASE_TABLE_OBJECT_LIT"', '" COM_INDEX_OBJECT_LIT"') and O.object_name not like 'TRAF_SAMPLE%%' group by 1,2,3,4",
              TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_OBJECTS,
              TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_TEXT,
              COM_OBJECT_NAMESPACE,
              TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_COLUMNS,
              objUIDsWherePred.data());
  ...
  retcode = copyBRSrcToTgt(
       brExpr,
       cliInterface, objUIDsWherePred.data(),
  ...
  retcode = backupHistStats(cliInterface, brExpr->tableList(),
                            (char *)backupSch.data());
  if (retcode < 0)
    {
      *CmpCommon::diags() << DgSqlCode(-5050)
      ...
      for (Lng32 idx = 0; idx < tableQueue->numEntries(); idx++)
        {
          ...
          Int32 lobFiles = 0;
          retcode = genLobLocation(objUID, obj, cliInterface, 
                                   lobLocList, lobFiles, FALSE);
          if (retcode < 0)
            {
              goto label_return;
            }
  ...
  if (insertProgressTable(cliInterface, brExpr, "BACKUP",
                          "BackupObjects", "Initiated", 
                          allTablesList.size(), 0) < 0)
    {
      return -1;
    }
                          
  retcode = ehi->backupObjects(allTablesList, 
                               allTablesFlagsList,
                               lobLocList,
                               brExpr->backupTag(), 
                               bkAttributeString,
                               (brExpr->getIncremental() ? "INCREMENTAL" : "REGULAR"),
                               backupThreads,
                               CmpCommon::getDefaultNumeric(TRAF_BREI_PROGRESS_UPDATE_DELAY));
  time6 = NA_JulianTimestamp();
  if (retcode < 0)
    {
      populateDiags("BACKUP", "Error returned from backupObjects method.",
                    "ExpHbaseInterface::backupObjects", NULL, -1);
  ...
  if (updateProgressTable(cliInterface, brExpr->backupTag().data(), "BACKUP", "BackupObjects", 
                          "Completed", -1, -1) < 0)
    {
      return -1;
    }
 ...
 label_return:	    
  ...
  if ((retcode == 0) ||
      (retcode == -HBASE_BACKUP_NONFATAL_ERROR))
    {
      if (unlockObjectsInMD(cliInterface, (char *)objUIDsWherePred.data(),
                            (char *)SEABASE_MD_SCHEMA,
                            schemasOnlyBackup))
        {
  ...
  if ((retcode == 0) && (NOT brExpr->showObjects()))
    {
      retcode = ehi->finalizeBackup(brExpr->backupTag(), 
                                    (brExpr->getIncremental() ? "INCR" : "REGULAR"));
      if (retcode < 0)
        {
          populateDiags("BACKUP", 
                        "Error returned during finalization of backup.",
                        "ExpHbaseInterface::finalizeBackup", 
                        getHbaseErrStr(-retcode), -retcode);
        }
     }
  ...
  if ((retcode == 0) && (backupSchExists))
    {
      ...
      // backup time.
      str_sprintf(query, "update %s.\"%s\".%s set redef_time = %ld where catalog_name = '%s' and schema_name = '%s' and object_name = '__SCHEMA__'",
                  TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_OBJECTS,
                  operStartTime,
                  TRAFODION_SYSCAT_LIT, backupSch.data());
  ...
  if (updateProgressTable(cliInterface, brExpr->backupTag().data(), "BACKUP", "Finalize", 
                          "Completed", 0, 100) < 0)
    {
      return -1;
    }
  ...
  Lng32 exportRetcode = 0;
  if ((retcode == 0 ) && brExpr->exportBackup())
    {
      Int32 exportImportThreads =
        CmpCommon::getDefaultNumeric(TRAF_EXPORT_IMPORT_PARALLEL);

      retcode = ehi->exportOrImportBackup(brExpr->backupTag(), TRUE,
                                          brExpr->getOverride(),
                                          brExpr->exportImportLocation().data(),
                                          exportImportThreads,
                                          CmpCommon::getDefaultNumeric(TRAF_BREI_PROGRESS_UPDATE_DELAY));
      if (retcode < 0)
        {
          NAString reasonStr;
          reasonStr = "Backup completed but export failed due to an error returned from exportOrImportBackup method. To redo just the export, issue a separate 'export backup' command with tag '";
          reasonStr += brExpr->backupTag();
          reasonStr += "'";
          populateDiags("EXPORT BACKUP", reasonStr,
                        "ExpHbaseInterface::exportOrImportBackup", 
                        getHbaseErrStr(-retcode), -retcode);
        }
...
short CmpSeabaseDDL::updateBackupOperationMetrics(RelBackupRestore * brExpr, 
                                                  ExeCliInterface * cliInterface,
                                                  Int64 operStartTime,
                                                  Int64 operEndTime)
{
  ...
  Int64 objUID = getObjectUID(cliInterface, 
                              TRAFODION_SYSTEM_CATALOG,
                              SEABASE_REPOS_SCHEMA,
                              TRAF_BACKUP_OPERATION_METRICS,
                              COM_BASE_TABLE_OBJECT_LIT,
                              NULL, NULL,NULL,
                              FALSE, FALSE);
  if (objUID < 0)
      ...
      if (brExpr->dropAllBackups())
        {
          ...
          char dummyTag[40];
          backupTag = NAString("DROP_ALL_") + 
            str_ltoa(brExpr->brUID(), dummyTag);
        }
    }
  ...
  str_sprintf(query, "insert into " TRAFODION_SYSTEM_CATALOG".\"" SEABASE_REPOS_SCHEMA"\"." TRAF_BACKUP_OPERATION_METRICS" values ('%s', %ld, converttimestamp(%ld), converttimestamp(%ld), %ld, '%s', '%s', '%s', %ld, %ld, %ld) ",
              backupTag.data(),
              brExpr->brUID(),
              operStartTime, operEndTime, (operEndTime-operStartTime),
              oper_name,
              oper_type,
              OPER_STATUS_COMPLETED,
              0L, 0L, 0L);
  cliRC = cliInterface->executeImmediate(query);
  if (cliRC < 0)
    {
...
short CmpSeabaseDDL::updateBackupObjectsMetrics(RelBackupRestore * brExpr, 
                                                ExeCliInterface * cliInterface)
{
  Lng32 cliRC = 0;
...
short CmpSeabaseDDL::createSnapshotForIncrBackup(ExeCliInterface * cliInterface,
                                                 NABoolean incrBackupEnabled,
                                                 const NAString &btNamespace,
                                                 ...
                                                 Int64 objDataUID)
{
  ...
  NAString extNameForHbase = genHBaseObjName
    (catalogNamePart, schemaNamePart, objectNamePart, btNamespace);
...
short CmpSeabaseDDL::setHiatus(const NAString &hiatusObjectName,
                               NABoolean resetHiatus,
                               ...
                               NABoolean ignoreSnapIfNotExist)
{
  ...
  if(resetHiatus)
    ...
    retcode = ehi->setHiatus(hiatusObjectName,
                             false,
  ...
  if (retcode < 0)
  {
    populateDiags("BACKUP", "Error returned from setHiatus method.",
    "ExpHbaseInterface::setHiatus", NULL, -1);
...
short CmpSeabaseDDL::backupSystem(RelBackupRestore * brExpr, 
                                  ExeCliInterface * cliInterface,
                                  CmpDDLwithStatusInfo *dws)
{
  short error = 0;
  ...
  if ((NOT backupType.isNull()) &&
      (NOT brExpr->showObjects()) &&
      (NOT brExpr->getOverride()))
    {
      *CmpCommon::diags() << DgSqlCode(-5050)
                          << DgString0("BACKUP")
                          << DgString1("Reason: Specified tag already exists.");
  ...
  if (lockedTag.length() > 0)
    {
      if (lockedTag == "?")
        populateDiags("BACKUP", "Error occurred while checking snapshot metadata. ",
                      "ExpHbaseInterface::backupSystem", NULL, -1);
      else
        {
          NAString lockedMsg = "Reason: Snapshot metadata is currently locked for Tag: " + lockedTag;
          *CmpCommon::diags() << DgSqlCode(-5050)
  ...
  retcode = verifyBRAuthority(NULL, cliInterface, brExpr->showObjects(), 
                              TRUE, SEABASE_MD_SCHEMA, FALSE, NULL/*owner*/);
  if (retcode < 0)
    {
      *CmpCommon::diags() << DgSqlCode(-5050)
                          << DgString0("BACKUP")
  ...
  if (brExpr->brMetadata())
    {
      str_sprintf(query,"select case when T.text is not null then trim(T.text) || ':' when T.text is null and O.schema_name in ('" SEABASE_MD_SCHEMA"') then '" TRAF_RESERVED_NAMESPACE1":' else '' end || trim(catalog_name) || '.' ||  trim(schema_name) || '.' || trim(object_name) from %s.\"%s\".%s O left join %s.\"%s\".%s T on O.object_uid = T.text_uid and T.text_type = %d where O.catalog_name = '%s' and O.schema_name in('" SEABASE_MD_SCHEMA"', '" SEABASE_REPOS_SCHEMA"', '" SEABASE_PRIVMGR_SCHEMA"', '" SEABASE_TENANT_SCHEMA"') and O.object_type in ('" COM_BASE_TABLE_OBJECT_LIT"', '" COM_INDEX_OBJECT_LIT"') and O.object_name not like 'TRAF_SAMPLE%%'",
                  TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_OBJECTS,
                  TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_TEXT,
                  COM_OBJECT_NAMESPACE,
                  TRAFODION_SYSCAT_LIT);
    ...
    {
      str_sprintf(query, "select case when T.text is not null then trim(T.text) || ':' when T.text is null and O.schema_name in ('" SEABASE_MD_SCHEMA"') then '" TRAF_RESERVED_NAMESPACE1":'else '' end || trim(O.catalog_name) || '.' ||  trim(O.schema_name) || '.' || trim(O.object_name), NVL(C.object_uid, 0), trim(O.schema_name) from %s.\"%s\".%s O left join %s.\"%s\".%s T on O.object_uid = T.text_uid and T.text_type = %d left join %s.\"%s\".%s C on O.object_uid = C.object_uid and C.fs_data_type in (160,161) where O.catalog_name = '%s' and schema_name not like '|_HV|_%%|_' escape '|'  and O.schema_name not like '|_HB|_MAP|_' escape '|' and schema_name not like 'VOLATILE_SCHEMA_%%' and schema_name not like '|_BACKUP|_%%' escape '|' and O.object_type in ('" COM_BASE_TABLE_OBJECT_LIT"', '" COM_INDEX_OBJECT_LIT"') and O.object_name not like 'TRAF_SAMPLE%%' group by 1,2,3 union all select trim(a), 0, '' from (get external hbase objects, match 'TRAF_RSRVD_%%') x(a) where a not like 'TRAF_RSRVD_7:%%' order by 1",
                  TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_OBJECTS,
                  TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_TEXT,
                  COM_OBJECT_NAMESPACE,
                  TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_COLUMNS,
  ...
  retcode = ehi->backupObjects(tableList, tableFlagsList, lobLocList,
                               brExpr->backupTag(),
                               bkAttributeString,
                               backupType,
                               backupThreads,
  ...
  if (retcode < 0)
    {
      populateDiags("BACKUP", "Error returned from backupObjects method.",
                    "ExpHbaseInterface::backupObjects", 
                    getHbaseErrStr(-retcode), -retcode);
...
short CmpSeabaseDDL::showBRObjects(RelBackupRestore *brExpr,
                                   ...
                                   ExeCliInterface * cliInterface)
{
  ...
  if ((! dws) || (NOT brExpr->showObjects()))
    return 0;

  if ((!mdList) && (!objectsList))
    return 0;
      ...
      for (Int32 i = 0; i < numEntries; i++)
        {
          ...
          if ((NOT qualBackupSch.isNull()) &&
              (strstr(val, qualBackupSch.data()) != NULL))
            continue;
...
short CmpSeabaseDDL::createNamespacesForRestore(RelBackupRestore* brExpr, 
                                                ...
                                                Int32 restoreThreads)
{
  ...
  if (CmpCommon::getDefault(TRAF_BR_REMOVE_SM_LOCKS) == DF_ON)
    {
      if (unlockSnapshotMeta(ehi, brExpr->backupTag(), lockedTag) < 0)
        {
          populateDiags("RESTORE", "Error occurred while unlocking snapshot metadata. "
  ...
  if (! objectsList)
    {
      if ((schemaListTV && schemaListTV->size() > 0) || 
          (tableListTV && tableListTV->size() > 0))
        {
      ...
      for (Int32 i = 0; i < nsVector.size(); i++)
        {
          ...
          if (createNamespace(ehihbase, ns, createReservedNS))
            return -1;
        }
...
short CmpSeabaseDDL::restoreSystem(RelBackupRestore* brExpr, 
                                   ExeCliInterface * cliInterface,
                                   NAString &currCatName, NAString &currSchName,
                                   CmpDDLwithStatusInfo *dws,
                                   ExpHbaseInterface *ehi,
                                   Int32 restoreThreads)
{
  ...
  if ((NOT brExpr->showObjects()) &&
      (NOT CmpCommon::context()->isUninitializedSeabase()))
    {
      *CmpCommon::diags() << DgSqlCode(-5050)
                          << DgString0("RESTORE")
  ...
  if (lockedTag.length() > 0)
    {
      if (lockedTag == "?")
        populateDiags("RESTORE", "Error occurred while checking snapshot metadata. ",
                      "ExpHbaseInterface::restoreSystem", NULL, -1);
      else
        {
          NAString lockedMsg = "Reason: Snapshot metadata is currently locked for Tag: " + lockedTag;
          *CmpCommon::diags() << DgSqlCode(-5050)
  ...
  if (brExpr->showObjects())
    {
      NAString qualBackupSch;
      return showBRObjects(brExpr, qualBackupSch, NULL, objectsList, 
                           dws, cliInterface);
    }
...
//         -1, some other error.
short CmpSeabaseDDL::checkIfObjectsLockedInMD(ExeCliInterface * cliInterface,
                                              char * objUIDsWherePred,
                                              char * srcSch,
                                              ...
                                              NAString &lockedObjs)
{
  ...
  if (objUIDsWherePred)
    {
      str_sprintf(objsQry, " select trim(T.text), trim(O.schema_name) || '.' || trim(O.object_name) from %s.\"%s\".%s O, %s.\"%s\".%s T where O.object_uid = T.text_uid and bitand(O.flags, %d) != 0 and T.text_type = 10 and O.object_uid in (select object_uid from %s.\"%s\".%s where %s and object_type in ('BT', 'IX') and object_name not in ('" HBASE_HIST_NAME"', '" HBASE_HISTINT_NAME"', '" HBASE_PERS_SAMP_NAME"')) ",
                  TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_OBJECTS, 
                  TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_TEXT,
                  MD_OBJECTS_BR_IN_PROGRESS,
                  TRAFODION_SYSCAT_LIT, srcSch, SEABASE_OBJECTS, 
                  objUIDsWherePred);
    }
  else
    {
      str_sprintf(objsQry, " select trim(T.text), trim(O.schema_name) || '.' || trim(O.object_name) from %s.\"%s\".%s O, %s.\"%s\".%s T where O.object_uid = T.text_uid and bitand(O.flags, %d) != 0 and T.text_type = 10 and O.object_type in ('BT', 'IX') and object_name not in ('" HBASE_HIST_NAME"', '" HBASE_HISTINT_NAME"', '" HBASE_PERS_SAMP_NAME"') ",
                  TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_OBJECTS, 
                  TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_TEXT,
                  MD_OBJECTS_BR_IN_PROGRESS);
    }
  ...
  for (Lng32 idx = 0; idx < objsQueue->numEntries(); idx++)
    {
      ...
      if ((NOT (tag == backupTag)) ||
          (operation && (NOT (oper == operation))))
        {
      ...
      lockedObjs += objName;
      if (idx < (objsQueue->numEntries() - 1))
        lockedObjs += ", ";
    } // for

  // objects are locked.
  if (operation && strcmp(operation, "BACKUP") == 0) {
    updateProgressTable(cliInterface, backupTag, operation,
                      "SetupMetadata",
                      "Failed", -1, -1);
  }
...
short CmpSeabaseDDL::lockObjectsInMD(ExeCliInterface * cliInterface,
                                     const char * oper,
                                     char * objUIDsWherePred,
                                     char * srcSch,
                                     char * backupTag,
                                     NABoolean schemasOnlyBackup)
{
  ...
  if ((schemasOnlyBackup) && 
      (CmpCommon::getDefault(TRAF_LOCK_BACKUP_SCHEMA) == DF_OFF))
    {
      schemasOnlyBackup = FALSE;
    }
  ...
  str_sprintf(query, "update %s.\"%s\".%s set flags = bitor(flags, %d) where object_uid in (select object_uid from %s.\"%s\".%s O where %s and object_type in ('BT', 'IX' %s) and object_name not in ('" HBASE_HIST_NAME"', '" HBASE_HISTINT_NAME"', '" HBASE_PERS_SAMP_NAME"') %s )",
              TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_OBJECTS, 
              MD_OBJECTS_BR_IN_PROGRESS,
              TRAFODION_SYSCAT_LIT, srcSch, SEABASE_OBJECTS, 
              objUIDsWherePred,
  ...
  operTag += NAString("(") + oper + ")";
  str_sprintf(query, "upsert into %s.\"%s\".%s select object_uid, %d, 0, 0, 0, '%s' from %s.\"%s\".%s O where %s and object_type in ('BT', 'IX' %s) and object_name not in ('" HBASE_HIST_NAME"', '" HBASE_HISTINT_NAME"', '" HBASE_PERS_SAMP_NAME"') %s ",
              TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_TEXT, 
              COM_LOCKED_OBJECT_BR_TAG,
              operTag.data(),
              TRAFODION_SYSCAT_LIT, srcSch, SEABASE_OBJECTS, 
...
short CmpSeabaseDDL::unlockObjectsInMD(ExeCliInterface * cliInterface,
                                       char * objUIDsWherePred,
                                       char * srcSch,
                                       NABoolean schemasOnlyBackup)
{
  ...
  str_sprintf(query, "update %s.\"%s\".%s set flags = bitand(flags, %d) where object_uid in (select object_uid from %s.\"%s\".%s O where %s and object_type in ('BT', 'IX', 'PS' %s) and object_name not in ('" HBASE_HIST_NAME"', '" HBASE_HISTINT_NAME"', '" HBASE_PERS_SAMP_NAME"') %s )",
              TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_OBJECTS, 
              ~MD_OBJECTS_BR_IN_PROGRESS,
              TRAFODION_SYSCAT_LIT, srcSch, SEABASE_OBJECTS, 
              objUIDsWherePred,
  ...
  str_sprintf(query, "delete from %s.\"%s\".%s where text_uid in (select object_uid from %s.\"%s\".%s O where %s and object_type in ('BT', 'IX', 'PS' %s) %s ) and text_type = %d ",
              TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_TEXT, 
              TRAFODION_SYSCAT_LIT, srcSch, SEABASE_OBJECTS, 
              objUIDsWherePred, 
              (schemasOnlyBackup ? " , 'PS', 'SS' " : ""),
...
short CmpSeabaseDDL::isTruncateTableLockedInMD(ExeCliInterface * cliInterface,
                                NAString &lockedObjs)
{
  Lng32 cliRC = 0;
  Queue * objsQueue = NULL;
  ...
  char objsQry[2000];
  str_sprintf(objsQry, " select trim(O.schema_name) || '.' || trim(O.object_name) from %s.\"%s\".%s O where bitand(O.flags, %d) != 0 and bitand(O.flags, %d) != 0",
              TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_OBJECTS, 
              MD_OBJECTS_BR_IN_PROGRESS, MD_OBJECTS_TRUNCATE_IN_PROGRESS);
  cliRC = cliInterface->fetchAllRows(objsQueue, objsQry, 
                                     0, FALSE, FALSE, TRUE);
  if (cliRC < 0)
    {
...
short CmpSeabaseDDL::returnObjectsList(ExeCliInterface * cliInterface,
                                       const char * objUIDsWherePred,
                                       char * srcSch,
                                       NAArray<HbaseStr>* &objectsList)
{
  ...
  str_sprintf(query, "select case when T.text is not null then trim(T.text) || ':' else '' end || trim(O.catalog_name) || '.' || trim(O.schema_name) || '.' || trim(O.object_name), O.flags from %s.\"%s\".%s O left join %s.\"%s\".%s T on O.object_uid = T.text_uid and T.text_type = %d where %s and O.object_type in ('" COM_BASE_TABLE_OBJECT_LIT"', '" COM_INDEX_OBJECT_LIT"') ",
              TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_OBJECTS,
              TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_TEXT,
              COM_OBJECT_NAMESPACE,
              objUIDsWherePred);
...
short CmpSeabaseDDL::restoreSubset(RelBackupRestore* brExpr, 
                                   ExeCliInterface * cliInterface,
                                   NAString &currCatName, NAString &currSchName,
                                   CmpDDLwithStatusInfo *dws,
                                   ExpHbaseInterface * ehi,
                                   Int32 restoreThreads)
{
      ...
      if (restoreToTsTag.isNull()) // error case
        {
          ...
          ExpDatetime::convAsciiDatetimeToUtcOrLocal(const_cast<char*>(originTime.data()), originTime.length(), localStrBuf,
                originTime.length(), (Int64)CmpCommon::context()->gmtDiff() * 60 * 1000000,
                FALSE, STMTHEAP, &diags);
          localStrBuf[10] = ':'; // add ':' separator between date and time
          localStrBuf[originTime.length()] = 0;
          NAString lockedMsg = "Error returned from getRestoreToTsBackupTag while processing to_timestamp value '" + NAString(localStrBuf) + "'.";

          populateDiags("RESTORE TO TIMESTAMP", lockedMsg,
                        "getRestoreToTsBackupTag", NULL, -1);
  ...
  if (operCnt < 0)
    return -1;
  operCnt++;
  if (insertProgressTable(cliInterface, brExpr, "RESTORE",
                          "Initialize", "Initiated", 
                          0, 0) < 0)
    {
      return -1;
  ...
  if (validateInputNames(cliInterface, brExpr, FALSE,
                         currCatName, currSchName))
    return -1;
  ...
  if (lockedTag.length() > 0)
    {
      ...
      updateProgressTable(cliInterface, brExpr->backupTag().data(), "RESTORE", "Initialize",
                          "Failed", -1, -1);
      return -1;
    }
  ...
  if (retcode == 1) // Backup MD is locked
    {
      ...
      if (NOT (tag == brExpr->backupTag()))
      ...
      else
        {
          *CmpCommon::diags() << DgSqlCode(-5050)
                              << DgString0("RESTORE")
                              << DgString1(NAString("Reason: Backup Metadata is locked by another Backup or Restore operation which need to finish first.") + details);
        }
  ...
  if (updateProgressTable(cliInterface, brExpr->backupTag().data(), "RESTORE", "Initialize", 
                          "Completed", 0, 100) < 0)
    {
      return -1;
    }
  ...
  if (attrList->getAttrByKey(BackupAttr::BACKUP_ATTR_USER_ROLES) != NULL) {
      NAString lackedUR;
      retcode = checkUsersAndRoles(cliInterface,
                 attrList->getAttrByKey(BackupAttr::BACKUP_ATTR_USER_ROLES), lackedUR, unmatchedUserID);
      if (retcode < 0) {
  ...
  retcode = verifyBRAuthority(NULL, cliInterface, brExpr->showObjects(), FALSE, 
                              backupSch.data(), FALSE, 
  ...
  if ((NOT brExpr->showObjects()) &&
      (!brExpr->schemaList() && !brExpr->tableList() && !brExpr->viewList() &&
       !brExpr->procList() && !brExpr->libList())) 
    restoreAllObjs = TRUE;

  if (insertProgressTable(cliInterface, brExpr, "RESTORE",
                          (restoreAllObjs ? "RestoreObjects" : "RestoreMetadata"), 
                          "Initiated", 
                          0, 0) < 0)
    {
      return -1;
  ...
  mdList = ehi->restoreObjects(
       brExpr->backupTag(),
       (restoreAllObjs ? NULL : &mdListTV),
       NULL,
       (brExpr->restoreToTS() ? brExpr->getTimestampVal() : ""),
       brExpr->showObjects(),
       saveBeforeRestore,
  ...
  if (! mdList)
    {
      ...
      updateProgressTable(cliInterface, brExpr->backupTag().data(), "RESTORE",
                          "RestoreObjects",
                          "Failed", -1, -1);
  ...
  if ((brExpr->showObjects()) && (NOT restoreAllObjs))
    {
      ...
      if (! mdObjsList)
        {
          populateDiags("RESTORE", " Backup Metadata objects could not be restored.",
                        "ExpHbaseInterface::restoreObjects", NULL, -1);
    ...
    if (NOT brExpr->showObjects() && unmatchedUserID.size() > 0)
    {
      ...
      if (retcode < 0)
      {
        *CmpCommon::diags() << DgSqlCode(-5050)
        ...
        // unlock the BackupMD
        if (NOT brExpr->showObjects())
          unlockBackupMD(brExpr, cliInterface, "RESTORE");
        ...
        if (retcode < 0)
        {
          populateDiags("RESTORE",
                        "Error returned during finalization of restore.",
                        "ExpHbaseInterface::finalizeRestore",
                        getHbaseErrStr(-retcode), -retcode);
        }
  ...
  if (brExpr->schemaList())
    {
      for (CollIndex i = 0; i < brExpr->schemaList()->entries(); i++)
        {
          ElemDDLQualName * dn = 
            ((*brExpr->schemaList())[i])->castToElemDDLQualName();
  ...
  if (brExpr->tableList())
    {
      for (CollIndex i = 0; i < brExpr->tableList()->entries(); i++)
        {
          ElemDDLQualName *dn =
              ((*brExpr->tableList())[i])->castToElemDDLQualName();
          ...
          {
            n += NAString(".") + dn->getQualifiedName().getSchemaName() +
                 "." + dn->getQualifiedName().getObjectName();
          }
      ...
      if (retcode == 1)
        {
          *CmpCommon::diags() << DgSqlCode(-5050)
                              << DgString0("RESTORE")
                              << DgString1("Reason: Some of the objects being restored are locked. This could be due to another backup or restore operation in progress.");
        }
      else
        {
          *CmpCommon::diags() << DgSqlCode(-5050)
  ...
  if (NOT brExpr->showObjects())
    {
      ...
      if (NOT brExpr->fastRecovery())
      {
        ...
        if (invalidateTgtObjects(cliInterface, (char *)objUIDsWherePred.data(),
                                 (char *)SEABASE_MD_SCHEMA, (char *)backupSch.data(), sharedCacheList))
        {
          *CmpCommon::diags() << DgSqlCode(-5050)
                              << DgString0("RESTORE")
      ...
      if (getenv("BACKUP_CLEANUP"))
        retcode = cleanupTgtObjectsFromMD(cliInterface, (char *)objUIDsWherePred.data(),
                                          (char *)backupSch.data());
      else
        retcode = deleteTgtObjectsFromMD(brExpr,
                                         cliInterface, objUIDsWherePred,
                                         (char *)SEABASE_MD_SCHEMA, (char *)SEABASE_PRIVMGR_SCHEMA,
                                         (char *)backupSch.data());
      if (retcode < 0)
        goto label_return;
      ...
      if (lockObjectsInMD(cliInterface, "RESTORE", (char *)objUIDsWherePred.data(),
                          (char *)backupSch.data(), (char *)brExpr->backupTag().data(),
                          FALSE))
        {
          *CmpCommon::diags() << DgSqlCode(-5050)
                              << DgString0("RESTORE")
  ...
  if (NOT restoreAllObjs)
    {
      if (updateProgressTable(cliInterface, brExpr->backupTag().data(), "RESTORE", 
                              "RestoreMetadata", "Completed", -1, -1) < 0)
        {
          return -1;
        }
      
      if (insertProgressTable(cliInterface, brExpr, "RESTORE",
                              "RestoreObjects", "Initiated", 
                              0, 0) < 0)
        {
          return -1;
        }
  ...
  if ((NOT restoreAllObjs) &&
      (NOT brExpr->brMetadata()))
    {
      ...
      objectsList = ehi->restoreObjects(
           brExpr->backupTag(),
           (schemaListTV.size() > 0 ? &schemaListTV : NULL), 
  ...
  if (updateProgressTable(cliInterface, brExpr->backupTag().data(), "RESTORE", "RestoreObjects", 
                          "Completed", -1, -1) < 0)
    {
      return -1;
    }
  ...
  if (brExpr->brMetadata())
    {
      if (returnObjectsList(cliInterface, objUIDsWherePred,
                            NULL, objectsList))
        return -1;
    }
  ...
  if (brExpr->showObjects())
    {
      return processBRShowObjects(brExpr, mdList, objUIDsWherePred, 
                                  cliInterface, dws);
    }
  ...
  if (brExpr->fastRecovery())
  {
    ...
    *CmpCommon::diags() << DgSqlCode(5050)
                        << DgString0("RESTORE IN FAST MODE")
...
short CmpSeabaseDDL::restore(RelBackupRestore* brExpr, 
                             ExeCliInterface * cliInterface,
                             NAString &currCatName, NAString &currSchName,
                             CmpDDLwithStatusInfo *dws)
{
  ...
  if (xnInProgress(cliInterface))
    {
      *CmpCommon::diags() << DgSqlCode(-5050)
                          << DgString0("RESTORE")
                          << DgString1("Reason: This operation cannot be performed if a user-defined transaction has been started or AUTOCOMMIT is OFF.");
  ...
  if (backupInit("RESTORE", 
                 ((((backupType == "SYSTEM")) &&
                   NOT brExpr->showObjects()) ? TRUE : FALSE)))
    {
      retcode = -1;
      goto label_return;
    }
  ...
  if ((backupType == "SYSTEM") || (backupType == "SYSTEM_PIT"))
    {
      retcode = restoreSystem(brExpr, cliInterface, currCatName, currSchName, 
                              dws, ehi, restoreThreads);
    }
  else
    {
      retcode = restoreSubset(brExpr, cliInterface, currCatName, currSchName, 
                              dws, ehi, restoreThreads);
    }
  if (retcode < 0)
    {
  ...
  if (NOT brExpr->showObjects())
    {
      retcode = ehi->finalizeRestore(brExpr->backupTag(), 
                                     backupType);
      if (retcode < 0)
        {
          populateDiags("RESTORE", 
  ...
  if (updateProgressTable(cliInterface, brExpr->backupTag().data(), "RESTORE", "Finalize",
                        "Completed", 0, 100) < 0)
  {
     return -1;
  }
...
short CmpSeabaseDDL::deleteSnapshot(ExpHbaseInterface *ehi,
                                    const char * oper,
                                    const char *backupTag,
                                    NABoolean ifExists,
                                    ...
                                    NABoolean skipLock) // do not lock during drop
{
  ...
  if (unlockSnapshotMeta(ehi, backupTag, lockedTag) < 0)
    {
      populateDiags("DROP BACKUP SNAPSHOT", "Error occurred while unlocking snapshot metadata. "
                    "Perform GET ALL LOCKED OBJECTS to verify lock has been removed. ",
                    "ExpHbaseInterface::restoreSubset", NULL, -1);
      cliRC =  -1;
    }
...
short CmpSeabaseDDL::dropBackupSnapshots(RelBackupRestore * brExpr, 
                                         ExeCliInterface * cliInterface)
{
  short error = 0;
  short rc = 0;
  ...
  if (backupInit("DROP BACKUP SNAPSHOT", FALSE))
    return -1;
  ...
  if (brExpr->dropAllBackups())
    {
      ...
      if (verifyBRAuthority(NULL, cliInterface, brExpr->showObjects(), 
                            TRUE, SEABASE_MD_SCHEMA, FALSE, NULL) != 0)
        {
          *CmpCommon::diags() << DgSqlCode(-5050)
                              << DgString0("DROP ALL BACKUP SNAPSHOTS/TAGS")
                              << DgString1("Reason: No privilege.");
      ...
      if (retcode < 0)
        {
          populateDiags("DROP ALL BACKUP SNAPSHOTS", 
                        "Error returned during list all backups.",
                        "ExpHbaseInterface::listAllBackups", 
                        getHbaseErrStr(-retcode), -retcode);
          ehi->close();
          ...
          if (NOT brExpr->getMatchPattern().isNull())
            {
              ...
              Queue * tagQueue = NULL;
              cliRC = cliInterface->fetchAllRows(tagQueue, qry.data(), 
                                                 0, FALSE, FALSE, TRUE);
              if (cliRC < 0)
                {
                  cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
              ...
              for (Lng32 idx = 0; idx < tagQueue->numEntries(); idx++)
                {
          ...
          for (Int32 i = 0; i < backupList->entries(); i++)
            {
              ...
              if (NOT brExpr->getSkipLock())
                {
                  NAString lockedObjs;
                  retcode = checkIfObjectsLockedInMD(cliInterface, NULL, NULL, NULL,
                                                     delBuf, lockedObjs);
                  if (retcode < 0)
                    {
                      *CmpCommon::diags() << DgSqlCode(-5050)
                                          << DgString0("DROP ALL BACKUP SNAPSHOTS")
                  ...
                  if (retcode != 0)
                    {
                      *CmpCommon::diags() << DgSqlCode(5050)
                                          << DgString0("DROP ALL BACKUP SNAPSHOTS")
                                          << DgString1(NAString("Reason: Some objects for tag ") + delBuf + " are locked in Metadata and need to be unlocked before it could be dropped.");
                      continue;
                    }
                }
  ...
  else
    {
      ...
      retcode = verifyBRAuthority(NULL, cliInterface, brExpr->showObjects(), 
                                  TRUE, SEABASE_MD_SCHEMA, FALSE, ownerStr);
      if (retcode < 0)
        {
          *CmpCommon::diags() << DgSqlCode(-5050)
                              << DgString0("DROP BACKUP SNAPSHOT")
                              << DgString1("Reason: No privilege.");
          ...
          if (retcode != 0)
            {
              *CmpCommon::diags() << DgSqlCode(-5050)
                                  << DgString0("DROP BACKUP SNAPSHOT")
                                  << DgString1("Reason: Some objects are locked in Metadata and need to be unlocked before dropping this tag.");
      ...
      if (deleteSnapshot(ehi, (char *)"DROP BACKUP SNAPSHOT",
                        (char *)brExpr->backupTag().data(), FALSE,
                        brExpr->getCascade(), dropForce,
                        brExpr->getSkipLock())) {
           ehi->close();
           return -1;
      }
...
short CmpSeabaseDDL::dropBackupTags(RelBackupRestore * brExpr, 
                                    ExeCliInterface * cliInterface)
{
  short rc = 0;
  Int32 cliRC = 0;
  ...
  if (NOT brExpr->dropAllBackups()) {
      ...
      Int64 objUID = getObjectUID(cliInterface,
                                  ...
                                  FALSE, FALSE);
      if (objUID < 0)
          return 0; // does not exist, nothing to drop
  }
...
short CmpSeabaseDDL::exportOrImportBackup(RelBackupRestore * brExpr, 
                                          ExeCliInterface * cliInterface)
{
  short error = 0;
  short rc = 0;
  ...
  if (insertProgressTable(cliInterface, brExpr, oper.data(),
                          "Initialize", "Initiated", 
                          0, 0) < 0)
    {
      return -1;
  ...
  //No need of any other lock for this operation.
  if (backupInit((brExpr->exportBackup() ? "EXPORT BACKUP" : "IMPORT BACKUP"), 
                 FALSE))
    return -1;
  ...
  if (getExtendedAttributes(ehi, brExpr->backupTag(), attrList, errorMsg) < 0)
    {
      errorMsg.prepend("Reason: ");
      *CmpCommon::diags() << DgSqlCode(-5050)
                          << DgString0(brExpr->exportBackup() ? "EXPORT" : "IMPORT")
                          << DgString1(errorMsg.data());
      ehi->close();
      return -1;
    }
  ...
  const char *owner = (brExpr->exportAllBackups()) ? NULL :
                        attrList->getAttrByKey(BackupAttr::BACKUP_ATTR_OWNER);
  if (verifyBRAuthority(NULL, cliInterface, brExpr->showObjects(), 
                        TRUE, SEABASE_MD_SCHEMA, FALSE, owner) != 0)
    {
      *CmpCommon::diags() << DgSqlCode(-5050)
                          << DgString0(brExpr->exportBackup() ? "EXPORT" : "IMPORT")
                          << DgString1("Reason: No privilege.");
  ...
  if (brExpr->exportAllBackups())
    {
      ...
      if (retcode < 0)
      {
        populateDiags("EXPORT IMPORT BACKUP", 
                      "Error returned during list all backups.",
                      "ExpHbaseInterface::listAllBackups", 
                      getHbaseErrStr(-retcode), -retcode);
        ehi->close();
          ...
          for (Int32 i = 0; i < backupList->entries(); i++)
            {
              ...
              archBuf[len] = 0;
              retcode = 
                ehi->exportOrImportBackup(archBuf, TRUE,
                                          brExpr->getOverride(),
                                          brExpr->exportImportLocation().data(),
                                          exportImportThreads,
                                          CmpCommon::getDefaultNumeric(TRAF_BREI_PROGRESS_UPDATE_DELAY));
    ...
    {
      if (updateProgressTable(cliInterface, brExpr->backupTag().data(), oper.data(), "Initialize", 
                              "Completed", 0, 100) < 0)
        {
          return -1;
        }
      ...
      if (insertProgressTable(cliInterface, brExpr, oper.data(),
                              (oper == "EXPORT" ? "ExportObjects" : "ImportObjects"), "Initiated", 
                              0, 0) < 0)
        {
          return -1;
        }
        ...
        if (!brExpr->getOverride()) {
          *CmpCommon::diags() << DgSqlCode(-5050)
          ...
          if (updateProgressTable(cliInterface, brExpr->backupTag().data(), oper.data(),
                                (oper == "EXPORT" ? "ExportObjects" : "ImportObjects"), "Failed",
                                -1, -1) < 0)
          {
            return -1;
          }
      ...
      retcode = 
        ehi->exportOrImportBackup(brExpr->backupTag(), 
                                  (brExpr->exportBackup() ? TRUE : FALSE),
                                  brExpr->getOverride(),
                                  brExpr->exportImportLocation().data(),
                                  exportImportThreads,
                                  CmpCommon::getDefaultNumeric(TRAF_BREI_PROGRESS_UPDATE_DELAY));
      if (retcode < 0)
        {
          populateDiags((brExpr->exportBackup() ? "EXPORT BACKUP" : "IMPORT BACKUP"), 
                        "Error returned from exportOrImportBackup method.",
                        "ExpHbaseInterface::exportOrImportBackup", 
                        getHbaseErrStr(-retcode), -retcode);
          
          updateProgressTable(cliInterface, brExpr->backupTag().data(), oper.data(), 
                              (oper == "EXPORT" ? "ExportObjects" : "ImportObjects"), 
                              "Failed", -1, -1);
  ...
  if (updateProgressTable(cliInterface, brExpr->backupTag().data(), oper.data(), "Finalize", 
                          "Completed", 0, 100) < 0)
    {
      return -1;
    }
...
short CmpSeabaseDDL::getBackupSnapshots(RelBackupRestore * brExpr, 
                                        ExeCliInterface * cliInterface,
                                        CmpDDLwithStatusInfo *dws)
{
  Lng32 retcode = 0;
  ...
  if (retcode < 0)
  {
    populateDiags("GET ALL BACKUP SNAPSHOTS", 
                  "Error returned during list all backups.",
                  "ExpHbaseInterface::listAllBackups", 
                  getHbaseErrStr(-retcode), -retcode);
    ehi->close();
  ...
  Lng32 maxTagLen = strlen("BackupTag");
  if (backupList)
    {
      NAString matchClause;
      if (NOT brExpr->getMatchPattern().isNull())
        matchClause = " where a like '" + brExpr->getMatchPattern() + "%'";
      else if ((brExpr->getBackupSnapshot()) &&
               (NOT brExpr->backupTag().isNull()))
  ...
  if ((brExpr->getBackupSnapshot()) && 
      ((! backupList) || (backupList->entries() == 0)))
    {
      *CmpCommon::diags() << DgSqlCode(-5050)
                          << DgString0("GET BACKUP SNAPSHOT")
                          << DgString1("Reason: Specified tag does not exist.");
...
short CmpSeabaseDDL::dropBackupMD(RelBackupRestore * brExpr,
                                  ExeCliInterface * cliInterface,
                                  NABoolean checkPrivs)
{
  Lng32 retcode = 0;
  Lng32 cliRC = 0;
  ...
  //do not let dropBackup when backup in progress.
  //No need of any other lock for this operation.
  if (backupInit("DROP BACKUP METADATA", FALSE))
    return -1;
  ...
  if (checkPrivs)
    {
      ...
      if (retcode < 0)
        {
          *CmpCommon::diags() << DgSqlCode(-5050)
                              << DgString0("DROP BACKUP METADATA")
                              << DgString1("Reason: No privilege.");
          return -1;
        }
    }
  ...
  if (brExpr->backupTag().isNull())
    {
      ...
      str_sprintf(query, "select distinct schema_name from %s.\"%s\".%s where catalog_name = '%s' %s ",
                  ...
                  likeClause.data());
      Queue * tableQueue = NULL;
      cliRC = cliInterface->fetchAllRows(tableQueue, query, 
                                         0, FALSE, FALSE, TRUE);
      if (cliRC < 0)
        {
          cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
      ...
      for (Lng32 idx = 0; idx < tableQueue->numEntries(); idx++)
        {
          ...
          char backupTag[100];
          str_cpy_and_null(backupTag, &obj[strlen(TRAF_BACKUP_MD_PREFIX)], 
                           (objLen - strlen(TRAF_BACKUP_MD_PREFIX) - 1),
                            '\0', ' ', TRUE);
          NAString backupType;
  ...
  else
    {
      ...
      if (NOT backupType.isNull())
        {
          *CmpCommon::diags() << DgSqlCode(-5050)
                              << DgString0("CLEANUP BACKUP")
                              << DgString1("Reason: Backup corresponding to the specified tag exists and is not obsolete. It needs to be deleted before metadata could be removed.");
...
short CmpSeabaseDDL::getBackupMD(RelBackupRestore * brExpr, 
                                 ExeCliInterface * cliInterface,
                                 CmpDDLwithStatusInfo * dws)
{
  Lng32 retcode = 0;
  ...
  str_sprintf(query, "select distinct schema_name from %s.\"%s\".%s "
                     "where catalog_name = '%s' %s %s ",
              TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_OBJECTS,
              TRAFODION_SYSCAT_LIT,
              (likeClause.isNull() ? " " : likeClause.data()),
              privsClause.data());
  Queue * tableQueue = NULL;
  cliRC = cliInterface->fetchAllRows(tableQueue, query, 
                                     0, FALSE, FALSE, TRUE);
  ...
  for (Lng32 idx = 0; idx < tableQueue->numEntries(); idx++)
    {
      ...
      str_sprintf(mdBuf, "Metadata(%-10s): %s",
                   (backupType.isNull() ? "OBSOLETE" : "ACTIVE"),
                   obj);
...
short CmpSeabaseDDL::getBackupTags(RelBackupRestore * brExpr, 
                                   ExeCliInterface * cliInterface,
                                   CmpDDLwithStatusInfo * dws)
{
  Lng32 retcode = 0;
  ...
  if (snapshotList && (snapshotList->entries() > 0))
    {
      ...
      if (NOT brExpr->getMatchPattern().isNull())
        snapshotQry += "where tag like '" + brExpr->getMatchPattern() + "%' ";
  ...
  snprintf(schemaFilter, sizeof(schemaFilter),
              "and substring(schema_name, %d, char_length(schema_name) - %d) not in %s",
              trafBackupMDPrefixLen, trafBackupMDPrefixLen,
              tags.data());

  // YES indicates this tag has metadata.
  snprintf(query, sizeof(query), "%s select distinct substring(schema_name, %d, char_length(schema_name) - %d), 'NO', ' ' "
              "from %s.\"%s\".%s where catalog_name = '%s' %s and object_name = '__SCHEMA__' "
              "%s order by 2;",
              (snapshotQry.isNull() ? " " : snapshotQry.data()),
              trafBackupMDPrefixLen, trafBackupMDPrefixLen,
              TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_OBJECTS,
              TRAFODION_SYSCAT_LIT, likeClause.data(),
              (tags.isNull() ? "" : schemaFilter)
              );
  
  Queue * tagQueue = NULL;
  cliRC = cliInterface->fetchAllRows(tagQueue, query, 
                                     0, FALSE, FALSE, TRUE);
  if (cliRC < 0)
    {
      cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
  ...
  for (Lng32 idx = 0; idx < tagQueue->numEntries(); idx++)
    {
      ...
      if (showDetails)
        {
          if (CmpCommon::getDefault(TRAF_BACKUP_SKIP_GET_TAGS_OWNER_LOOKUP) == DF_ON)
          ...
          else
            {
              ...
              if (ownerAttr && strlen(ownerAttr) > maxOwnerLen)
                maxOwnerLen = strlen(ownerAttr);
             }
        }
      ...
      if (NOT showDetails)
        ...
        {
          qry += NAString("('") + tag + "', " + (isExtended ? "'YES'" : "'NO'") +
            NAString(", '") + NAString(obj) + NAString("', '");
          char * time = NULL;
          Lng32 timeLen = 0;
  ...
  for (Lng32 idx = 0; idx < tagQueue->numEntries(); idx++)
    {
      ...
      if (backupOper == "SYSTEM")
        continue; // skip system backups

      if (NOT showDetails)
        count = *(Int64*)vi->get(2);
      else
        {
          char * snap = NULL;
          Int32 snapLen = 0;
          ...
          if (strncmp(snap, "NO", 2) == 0) {
            ...
            if (backupOper.contains("IMPORTED"))
              hasMeta = FALSE;
          }
          vi->get(3, ts, tsLen);
        }
      ...
      if (NOT showDetails)
      ...
      else
        {
          ...
          str_sprintf(mdBuf, frmt,
                      tag,
                      localStrBuf,
                      (hasMeta ? backupStatus.data() : "NO_METADATA"),
...
// ----------------------------------------------------------------------------    
short CmpSeabaseDDL::getVersionOfBackup(RelBackupRestore * brExpr,
                                        ExeCliInterface * cliInterface,
                                        CmpDDLwithStatusInfo * dws)
{
  Lng32 retcode = 0;
  ...
  // Check for privilege
  retcode =  verifyBRAuthority (NULL, cliInterface, brExpr->showObjects(),
                                TRUE, SEABASE_MD_SCHEMA, FALSE,
                                attrList->getAttrByKey(BackupAttr::BACKUP_ATTR_OWNER));

  if (retcode < 0)
    {
      *CmpCommon::diags() << DgSqlCode(-5050)
                          << DgString0("GET VERSION OF BACKUP")
                          << DgString1("Reason: No privilege.");
      ehi->close();
      return -1;
    }
  ...
  if (testPoint != NULL)
    attrList->replaceAttr(BackupAttr::BACKUP_ATTR_VERSION_MD, testPoint);

  // BR_TESTPOINT_PRV:  change the PR version
  testPoint = getenv("BR_TESTPOINT_PRV");
  if (testPoint != NULL)
    attrList->replaceAttr(BackupAttr::BACKUP_ATTR_VERSION_PR, testPoint);
  ...
  else
    {
      ...
      if (retcode == 0)
      ...
      else
        {
          ...
          versionStr += (retcode == -1) ? "Database" :
                      ((retcode == -2) ? "Metadata" : "Product");
...
short CmpSeabaseDDL::getBackupLockedObjects(RelBackupRestore * brExpr, 
                                            ExeCliInterface * cliInterface,
                                            CmpDDLwithStatusInfo * dws)
{
  Lng32 cliRC = 0;
  ...
  char objsQry[2000];
  str_sprintf(objsQry, " select cast(trim(T.text) as char(64) character set iso88591), trim(schema_name) || '.' || trim(object_name) from %s.\"%s\".%s O, %s.\"%s\".%s T where O.object_uid = T.text_uid and (O.object_type = 'PS' or bitand(O.flags, %d) != 0) and T.text_type = 10 order by 1,2",
              TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_OBJECTS, 
              TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_TEXT,
              MD_OBJECTS_BR_IN_PROGRESS);
  cliRC = cliInterface->fetchAllRows(objsQueue, objsQry, 
...
short CmpSeabaseDDL::cleanupLockForTag(RelBackupRestore * brExpr, 
                                       ExeCliInterface * cliInterface,
                                       ExpHbaseInterface * ehi)
{
    ...
    snprintf(objsQryLocked, sizeof(objsQryLocked), " select O.object_uid from %s.\"%s\".%s O, %s.\"%s\".%s T where O.object_uid = T.text_uid and (O.object_type = 'PS' or bitand(O.flags, %d) != 0) and T.text_type = %d and substring(T.text, 1, position('(' in T.text) -1) = '%s' and O.object_type in ('PS', 'BT', 'IX') and object_name not in ('" HBASE_HIST_NAME "', '" HBASE_HISTINT_NAME "', '" HBASE_PERS_SAMP_NAME "') ",
             TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_OBJECTS,
             TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_TEXT,
             MD_OBJECTS_BR_IN_PROGRESS, COM_LOCKED_OBJECT_BR_TAG,
             brExpr->backupTag().data());

    char query[3000];
    snprintf(query, sizeof(query), "update %s.\"%s\".%s set flags = bitand(flags, %d) where object_uid in (%s)",
             TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_OBJECTS,
             ~MD_OBJECTS_BR_IN_PROGRESS, objsQryLocked);
    ...
    snprintf(query, sizeof(query), "delete from %s.\"%s\".%s where text_type=%d",
             TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_TEXT,
             COM_LOCKED_OBJECT_BR_TAG);
    ...
    NAString lockedTag;
    if (unlockSnapshotMeta(ehi, brExpr->backupTag(), lockedTag) < 0) {
        populateDiags("CLEANUP", "Error occurred while unlocking snapshot metadata. "
                    "Perform GET ALL LOCKED OBJECTS to verify lock has been removed. ",
    ...
    if (tagStatus == NAString("VALID") && internalTagValid) {
      NAString backupType = ehi->getBackupType(brExpr->backupTag());
      Lng32 retcode = ehi->finalizeRestore(brExpr->backupTag(), backupType + "_(CLEANUP)");
      if (retcode < 0) {
        populateDiags("CLEANUP", "Error occurred while delete _SAVE_ tag. ",
                      "CmpSeabaseDDL::cleanupLockForTag", NULL, -1);
        return -1;
      }
    }
...
short CmpSeabaseDDL::cleanupLockedObjects(RelBackupRestore * brExpr, 
                                          ExeCliInterface * cliInterface)
{
  Lng32 cliRC = 0;
  Lng32 retcode = 0;
  ...
  if (verifyBRAuthority(NULL, cliInterface, FALSE /*show op*/, FALSE,
                        SEABASE_MD_SCHEMA, FALSE, owner) != 0)
    {
      *CmpCommon::diags() << DgSqlCode(-5050)
                          << DgString0("CLEANUP LOCKED OBJECTS")
                          << DgString1("Reason: No privilege.");
      deallocBRCEHI(ehi);
      return -1;
    }
  ...
  str_sprintf(objsQryAll, " select O.object_uid from %s.\"%s\".%s O, %s.\"%s\".%s T where O.object_uid = T.text_uid and T.text_type = 10 and substring(T.text, 1, position('(' in T.text) -1) = '%s' and O.object_type in ('PS', 'BT', 'IX') and object_name not in ('" HBASE_HIST_NAME"', '" HBASE_HISTINT_NAME"', '" HBASE_PERS_SAMP_NAME"') ",
              TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_OBJECTS,
              TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_TEXT,
              brExpr->backupTag().data());

  str_sprintf(objsQryLocked, " select O.object_uid from %s.\"%s\".%s O, %s.\"%s\".%s T where O.object_uid = T.text_uid and (O.object_type = 'PS' or bitand(O.flags, %d) != 0) and T.text_type = 10 and substring(T.text, 1, position('(' in T.text) -1) = '%s' and O.object_type in ('PS', 'BT', 'IX') and object_name not in ('" HBASE_HIST_NAME"', '" HBASE_HISTINT_NAME"', '" HBASE_PERS_SAMP_NAME"') ",
              TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_OBJECTS,
              TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_TEXT,
              MD_OBJECTS_BR_IN_PROGRESS,
              brExpr->backupTag().data());
  ...
  if (internalTagStatus == NAString("INVALID"))
    {
      ...
      *CmpCommon::diags() << DgSqlCode(-5050)
                          << DgString0("CLEANUP LOCKED OBJECTS")
                          << DgString1(internalTagName.data());
      deallocBRCEHI(ehi);
      return -1;
    }
  ...
  if (sqlLocked)
    {
      str_sprintf(query, "select distinct substring(text, position('(' in text)+1, octet_length(text) - position('(' in text)-1) from %s.\"%s\".%s where text_type = 10 and text_uid in (%s)",
                  TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_TEXT,
                  objsQryLocked);
      cliRC = cliInterface->fetchAllRows(objsQueue, query,
                                         0, FALSE, FALSE, TRUE);
      ...
      objsQueue->position();
      OutputInfo * vi = (OutputInfo*)objsQueue->getNext();
      if ((strcmp((char*)vi->get(0), "RESTORE") == 0) && internalTagValid)
        isRestore = TRUE; 
    }
  ...
  if (isRestore && (! backupType.isNull()) && internalTagValid)
    {
      Int32 restoreThreads = 
        CmpCommon::getDefaultNumeric(TRAF_RESTORE_PARALLEL);
  ...
  if (objectsList)
    {
      ...
      cliRC = deleteTgtObjectsFromMD(brExpr,
                                     cliInterface, objUIDsWherePred,
                                     (char *)SEABASE_MD_SCHEMA, (char *)SEABASE_PRIVMGR_SCHEMA,
                                     (char *)backupSch.data());      
      if (cliRC < 0)
  ...
  // reset metadata
  str_sprintf(query, "update %s.\"%s\".%s set flags = bitand(flags, %d) where object_uid in (%s)",
              TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_OBJECTS, 
              ~MD_OBJECTS_BR_IN_PROGRESS,
              objsQryLocked);
  cliRC = cliInterface->executeImmediate(query);
  ...
  str_sprintf(query, "delete from %s.\"%s\".%s where text_uid in (%s) and text_type = 10",
              TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_TEXT, 
              objsQryAll);
  cliRC = cliInterface->executeImmediate(query);
  if (cliRC < 0)
...
// ----------------------------------------------------------------------------
short CmpSeabaseDDL::verifyBRAuthority (const char *privsWherePred, 
                                        ExeCliInterface *cliInterface,
                                        NABoolean allowShow,
                                        NABoolean isBackup,
                                        const char *schToUse,
                                        const NABoolean checkSpecificPrivs,
                                        const char *owner)
{
  if (hasAccess(owner, allowShow))
    return 0;
  ...
  str_sprintf(query, " select distinct schema_owner, object_owner, object_type, "
              "object_uid, schema_name, object_name from %s.\"%s\".%s O where %s ",
              TRAFODION_SYSCAT_LIT, schToUse, SEABASE_OBJECTS,
              privsWherePred);
  ...
  for (Lng32 idx = 0; idx < objsQueue->numEntries(); idx++)
    {
      ...
      if (userID == *(Int32*)vi->get(0) || userID == *(Int32*)vi->get(1))
        continue;
      ...
      // on schema objects
      if (objType == COM_SHARED_SCHEMA_OBJECT || objType == COM_PRIVATE_SCHEMA_OBJECT)
        continue;
      ...
      if (checkSpecificPrivs && objType == COM_BASE_TABLE_OBJECT)
        { 
          ...
          Int32 diagsMark = CmpCommon::diags()->mark();
          if (userCmd.getPrivileges(objUID, objType, userID, privInfo) == PrivStatus::STATUS_ERROR)
            return -1;
          CmpCommon::diags()->rewind(diagsMark);

          if (privInfo.hasPriv(SELECT_PRIV) && privInfo.hasPriv(INSERT_PRIV))
            continue;

          *CmpCommon::diags() << DgSqlCode(-CAT_INSUFFICIENT_PRIV_ON_OBJECT)
                              << DgTableName(objName);
          return -1;
        }
      else
...
// ----------------------------------------------------------------------------
short CmpSeabaseDDL::getExtendedAttributes(
  ExpHbaseInterface * ehi,
  const char * backupTag,
  BackupAttrList * attrList,
  NAString &errorMsg)
...
// ----------------------------------------------------------------------------
short CmpSeabaseDDL::checkCompatibility(
  BackupAttrList *attrList,
  NAString &errorMsg)
{
  if (attrList->entries() == 0)
  ...
  if (retcode < 0)
    {
      ...
      if (retcode == -1)
        ...
        {
          errorMsg += (retcode = -2) ? "Database" :
                      ((retcode = -3) ? "Metadata" : "Product");
...
Int32 CmpSeabaseDDL::getOwnerList(
  ExpHbaseInterface *ehi,
        ...
        { 
          ...
          if (CmpCommon::getDefault(TRAF_BACKUP_SKIP_GET_TAGS_OWNER_LOOKUP) == DF_ON)
          ...
          else
            {
              ...
              if (owner && strlen(owner)> maxOwnerLen)
                maxOwnerLen = strlen(owner);
            }
        }
    }
  return maxOwnerLen;
...
short CmpSeabaseDDL::getUsersAndRoles(
    ExeCliInterface * cliInterface,
    const char * objUIDsWherePred,
    const char * srcSch,
    Lng32 &numUR,
    NAString &userOrRoles)
{
    Lng32 cliRC = 0;
    char query[1000 + strlen(objUIDsWherePred)];
    str_sprintf(query, "select distinct trim(grantee_name), grantee_id from ("
                ...
                TRAFODION_SYSCAT_LIT, SEABASE_PRIVMGR_SCHEMA, PRIVMGR_SCHEMA_PRIVILEGES,
                TRAFODION_SYSCAT_LIT, SEABASE_PRIVMGR_SCHEMA, PRIVMGR_OBJECT_PRIVILEGES,
                TRAFODION_SYSCAT_LIT, SEABASE_PRIVMGR_SCHEMA, PRIVMGR_COLUMN_PRIVILEGES,
    ...
    for (Lng32 idx = 0; idx < urQueue->numEntries(); idx++) {
        OutputInfo * vi = (OutputInfo*)urQueue->getNext();
        userOrRoles += "('";
        userOrRoles += (char*)vi->get(0);
        userOrRoles += "':";
        userOrRoles += Int64ToNAString(*(Int64*)vi->get(1));
        userOrRoles += ")";
        if (idx < urQueue->entries()-1)
            userOrRoles += ":";
    }
...
short CmpSeabaseDDL::checkUsersAndRoles(
    ExeCliInterface *cliInterface,
    const char *userOrRoles, NAString &lackedUR,
    std::map<Int32, NAString> &userMap)
{
    ...
    */
    ...
    str_sprintf(query, "select trim(grantee_name) from ( "
                       "select grantee_name, grantee_id from (values "
                       "%s) ur(grantee_name, grantee_id)) as U left join %s.\"%s\".%s as A "
    ...
    for (Lng32 idx = 0; idx < userQueue->numEntries(); idx++) {
        ...
        lackedUR += name;
        if (idx < userQueue->entries()-1)
            lackedUR += ",";
    }

    if (NOT lackedUR.isNull())
        return -1;
...
short CmpSeabaseDDL::createBackupRepos(ExeCliInterface * cliInterface)
{
  ...
  str_sprintf(queryBuf, "create schema if not exists %s.\"%s\" namespace '%s' ; ",
              TRAFODION_SYSCAT_LIT,SEABASE_REPOS_SCHEMA,
              ComGetReservedNamespace(SEABASE_REPOS_SCHEMA).data());
  ...
  for (Int32 i = 0; i < sizeof(allBackupReposInfo)/sizeof(MDUpgradeInfo); i++)
    {
      ...
      str_sprintf(queryBuf, "drop table if exists %s.\"%s\".%s cascade; ",
                  TRAFODION_SYSCAT_LIT, SEABASE_REPOS_SCHEMA,
                  rti.newName);
...
void CmpSeabaseDDL::processBackupRepository(
...
short CmpSeabaseDDL::cleanupBackupMetadata(ExeCliInterface *cliInterface, NAString &backupTag)
{
    Lng32 cliRC = 0;
    char objectsUID[200];
    str_sprintf(objectsUID, "select object_uid from %s.\"%s\".%s where schema_name = '%s'",
            getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_OBJECTS, backupTag.data());

    char deleteSql[400];
    // delete from tables
    str_sprintf(deleteSql, "delete from %s.\"%s\".%s where table_uid in (%s)",
            getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_TABLES, objectsUID);
    cliRC = cliInterface->executeImmediate(deleteSql);
    if (cliRC < 0)
        goto label_return;
    // delete from keys
    str_sprintf(deleteSql, "delete from %s.\"%s\".%s where object_uid in (%s)",
            getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_KEYS, objectsUID);
    cliRC = cliInterface->executeImmediate(deleteSql);
    if (cliRC < 0)
        goto label_return;
    // delete from columns
    str_sprintf(deleteSql, "delete from %s.\"%s\".%s where object_uid in (%s)",
            getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_COLUMNS, objectsUID);
    cliRC = cliInterface->executeImmediate(deleteSql);
    if (cliRC < 0)
        goto label_return;
    // delete from text
    str_sprintf(deleteSql, "delete from %s.\"%s\".%s where text_uid in (%s)",
            getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_TEXT, objectsUID);
    cliRC = cliInterface->executeImmediate(deleteSql);
    if (cliRC < 0)
        goto label_return;
    // delete from indexes
    str_sprintf(deleteSql, "delete from %s.\"%s\".%s where index_uid in (%s)",
            getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_INDEXES, objectsUID);
    cliRC = cliInterface->executeImmediate(deleteSql);
    if (cliRC < 0)
        goto label_return;
    // delete from views
    str_sprintf(deleteSql, "delete from %s.\"%s\".%s where view_uid in (%s)",
            getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_VIEWS, objectsUID);
    cliRC = cliInterface->executeImmediate(deleteSql);
    if (cliRC < 0)
        goto label_return;
    // delete from view_usage
    str_sprintf(deleteSql, "delete from %s.\"%s\".%s where using_view_uid in (%s)",
            getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_VIEWS_USAGE, objectsUID);
    cliRC = cliInterface->executeImmediate(deleteSql);
    if (cliRC < 0)
        goto label_return;
    // delete from routines
    str_sprintf(deleteSql, "delete from %s.\"%s\".%s where udr_uid in (%s)",
            getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_ROUTINES, objectsUID);
    cliRC = cliInterface->executeImmediate(deleteSql);
    if (cliRC < 0)
        goto label_return;
    // delete from libraries
    str_sprintf(deleteSql, "delete from %s.\"%s\".%s where library_uid in (%s)",
            getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_LIBRARIES, objectsUID);
    cliRC = cliInterface->executeImmediate(deleteSql);
    if (cliRC < 0)
        goto label_return;
    // delete from libraries_usage
    str_sprintf(deleteSql, "delete from %s.\"%s\".%s where used_udr_uid in (%s)",
            getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_LIBRARIES_USAGE, objectsUID);
    cliRC = cliInterface->executeImmediate(deleteSql);
    if (cliRC < 0)
        goto label_return;
    // delete from lob_columns
    str_sprintf(deleteSql, "delete from %s.\"%s\".%s where table_uid in (%s)",
            getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_LOB_COLUMNS, objectsUID);
    cliRC = cliInterface->executeImmediate(deleteSql);
    if (cliRC < 0)
        goto label_return;
    // delete from seq_gen
    str_sprintf(deleteSql, "delete from %s.\"%s\".%s where seq_uid in (%s)",
            getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_SEQ_GEN, objectsUID);
    cliRC = cliInterface->executeImmediate(deleteSql);
    if (cliRC < 0)
        goto label_return;
    // delete from schema_privileges
    str_sprintf(deleteSql, "delete from %s.\"%s\".%s where schema_uid in (%s)",
            getSystemCatalog(), SEABASE_PRIVMGR_SCHEMA, PRIVMGR_SCHEMA_PRIVILEGES, objectsUID);
    cliRC = cliInterface->executeImmediate(deleteSql);
    if (cliRC < 0)
        goto label_return;
    // delete from object_privileges
    str_sprintf(deleteSql, "delete from %s.\"%s\".%s where object_uid in (%s)",
            getSystemCatalog(), SEABASE_PRIVMGR_SCHEMA, PRIVMGR_OBJECT_PRIVILEGES, objectsUID);
    cliRC = cliInterface->executeImmediate(deleteSql);
    if (cliRC < 0)
        goto label_return;
    // delete from column_privileges
    str_sprintf(deleteSql, "delete from %s.\"%s\".%s where object_uid in (%s)",
            getSystemCatalog(), SEABASE_PRIVMGR_SCHEMA, PRIVMGR_COLUMN_PRIVILEGES, objectsUID);
    cliRC = cliInterface->executeImmediate(deleteSql);
    if (cliRC < 0)
        goto label_return;
    // delete from objects
    str_sprintf(deleteSql, "delete from %s.\"%s\".%s where object_uid in (%s)",
            getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_OBJECTS, objectsUID);
    cliRC = cliInterface->executeImmediate(deleteSql);
    if (cliRC < 0)
        goto label_return;
 ...
 * Function used to check user_id conflicts
 */
short CmpSeabaseDDL::fixBackupAuthIDs(ExeCliInterface *cliInterface,
                                      NAString &backupScheName,
                                      std::map<Int32, NAString> &unMatchedUserIDs)
{
	...
	snprintf(updateAuths, sizeof(updateAuths), "select max(auth_id) from %s.\"%s\".%s "
											   "where auth_type = 'U';",
	...
	while (iter != unMatchedUserIDs.end())
	{
		...
		snprintf(updateAuths, sizeof(updateAuths), "update %s.\"%s\".%s set GRANTEE_ID = %d "
												   "where GRANTEE_NAME = '%s';",
				 getSystemCatalog(), backupScheName.data(), PRIVMGR_OBJECT_PRIVILEGES,
				 tempUserID, user_name.data());
		cliRC = cliInterface->executeImmediate(updateAuths);
		if (cliRC < 0)
			return -1;

		snprintf(updateAuths, sizeof(updateAuths), "update %s.\"%s\".%s set GRANTOR_ID = %d "
		...
		snprintf(updateAuths, sizeof(updateAuths), "update %s.\"%s\".%s set GRANTEE_ID = %d "
												   "where GRANTEE_NAME = '%s';",
				 getSystemCatalog(), backupScheName.data(), PRIVMGR_COLUMN_PRIVILEGES,
				 tempUserID, user_name.data());
		cliRC = cliInterface->executeImmediate(updateAuths);
		if (cliRC < 0)
			return -1;
		...
		snprintf(updateAuths, sizeof(updateAuths), "update %s.\"%s\".%s set GRANTEE_ID = %d "
												   "where GRANTEE_NAME = '%s';",
				 getSystemCatalog(), backupScheName.data(), PRIVMGR_SCHEMA_PRIVILEGES,
				 tempUserID, user_name.data());
		cliRC = cliInterface->executeImmediate(updateAuths);
		if (cliRC < 0)
			return -1;
		...
		snprintf(updateAuths, sizeof(updateAuths), "update %s.\"%s\".%s set object_owner = %d "
												   "where object_owner = %d;",
				 getSystemCatalog(), backupScheName.data(), SEABASE_OBJECTS,
				 tempUserID, user_id);
		cliRC = cliInterface->executeImmediate(updateAuths);
		if (cliRC < 0)
			return -1;

		snprintf(updateAuths, sizeof(updateAuths), "update %s.\"%s\".%s set schema_owner = %d "
    ...
    while (iter != unMatchedUserIDs.end())
    {
		...
		snprintf(updateAuths, sizeof(updateAuths), "select auth_id from %s.\"%s\".%s "
