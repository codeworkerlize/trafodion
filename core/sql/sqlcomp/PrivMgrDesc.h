...
class PrivMgrCoreDesc 
{
  ...
  bool isNullWgo() const {   return ( wgo_.none() ); }
   ...
   void AndNot( const PrivMgrCoreDesc& other );
    ...
    inline void setAllSchemaGrantPrivileges(
      const bool wgo,
      const bool all_dml,
      const bool all_ddl)
    {
...
public:
  ...
  // Complement -- take the complement of each priv setting in this.
   ...
   bool isNull() const
   {
      if (tableLevel_.isNull() && schemaLevel_.isNull())
  ...
  void setAllSchemaGrantPrivileges(const bool wgo,
      const bool all_dml,
      const bool all_ddl)
   {
   ...
   PrivMgrCoreDesc::PrivResult grantSchemaPrivs(PrivMgrCoreDesc& priv)
   { return schemaLevel_.grantPrivs(priv); }

   PrivMgrCoreDesc::PrivResult revokeSchemaPrivs(PrivMgrCoreDesc& priv)
   { return schemaLevel_.revokePrivs(priv); }
...
class PrivMgrDescList : public LIST(PrivMgrDesc *)
{
  ...
  // copy constructor
  PrivMgrDescList(const PrivMgrDescList& other, CollHeap *heap)
   : LIST(PrivMgrDesc *)(other, heap),
     heap_(heap)
  {}
