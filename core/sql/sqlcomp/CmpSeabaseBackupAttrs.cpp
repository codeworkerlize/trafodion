...
{
  if (strcmp(litVal, BACKUP_ATTR_ENTRIES_LIT) == 0)
    return BACKUP_ATTR_ENTRIES;
  if (strcmp(litVal, BACKUP_ATTR_OWNER_LIT) == 0)
    return BACKUP_ATTR_OWNER;
  if (strcmp(litVal, BACKUP_ATTR_VERSION_DB_LIT) == 0)
    return BACKUP_ATTR_VERSION_DB;
  if (strcmp(litVal, BACKUP_ATTR_VERSION_MD_LIT) == 0)
    return BACKUP_ATTR_VERSION_MD;
  if (strcmp(litVal, BACKUP_ATTR_VERSION_PR_LIT) == 0)
    return BACKUP_ATTR_VERSION_PR;
  if (strcmp(litVal, BACKUP_ATTR_USER_ROLES_LIT) == 0)
    return BACKUP_ATTR_USER_ROLES;
...
short BackupAttrList::addVersions()
{
  // get version string  
  VersionInfo vInfo;
  if (CmpSeabaseDDL::getVersionInfo(vInfo) != 0)
    return -1;

  BackupAttr *dbVersion = new (getHeap()) 
    BackupAttr(BackupAttr::BACKUP_ATTR_VERSION_DB, vInfo.dbVersionStr_.data(), getHeap());
  insert(dbVersion);

  // metadata version
  BackupAttr *mdVersion = new (getHeap()) 
    BackupAttr(BackupAttr::BACKUP_ATTR_VERSION_MD, vInfo.mdVersionStr_.data(), getHeap());
  insert(mdVersion);

  // product version
  BackupAttr *prVersion = new (getHeap()) 
    BackupAttr(BackupAttr::BACKUP_ATTR_VERSION_PR, vInfo.prVersionStr_.data(), getHeap());
  insert(prVersion);
...
// ----------------------------------------------------------------------------
void BackupAttrList::packIntoBuffer(
  NAString &buffer,
  BackupAttr *attr)
{
  // KEY+key+
...
// ----------------------------------------------------------------------------
char * BackupAttrList::unpackFromBuffer(
  const char *buffer, 
  BackupAttr *&attr)
{
  char * ptr = (char *)buffer;
  ...
  if (strncmp (ptr, KVKey, strlen(KVKey)) != 0)
    return NULL;
  ...
  if (strncmp(ptr, KVValue, strlen(KVValue)) != 0)
    return NULL;
  ptr = nxt + strlen(KVDelim); 
  nxt = strstr(ptr, KVDelim); 
  if (nxt == NULL)
...
short BackupAttrList::backupCompatible(VersionInfo vInfo)
{
  ...
  if (versionStr.contains("Advanced") && vInfo.dbVersionStr_.contains("Enterprise"))
     return -1;
  ...
  if (bkValue > vInfo.mdMajorVersion_) 
    return -2;
  ...
  if (tok == NULL)
    return -2;
  bkValue = atol(tok);
  if (bkValue > vInfo.mdMinorVersion_)
    return -2;
  ...
  if (tok == NULL)
    return -2;
  bkValue = atol(tok);
  if (bkValue > vInfo.mdUpdateVersion_)
    return -2;
...
void  BackupAttrList::replaceAttr(
  const BackupAttr::BACKUP_ATTR_KEY attrKey, 
  const char * value)
{
  for (CollIndex i = 0; i < entries(); i++)
  {
