...
void Parser::reset(NABoolean on_entry_reset_was_needed) {
  ...
  //
  if (!Get_SqlParser_Flags(DELAYED_RESET)) Set_SqlParser_Flags(0);
...
Parser::Parser(const CmpContext *cmpContext)
  ...
  if (cmpContext_ && ((wHeap_ = cmpContext_->statementHeap()) != NULL)) {
    ...
    size_t memLimit = 0;
    if (memLimitStr != NULL) memLimit = (size_t)1024 * atol(memLimitStr);

    // Allocate a heap for the parser to prevent memory leaks.
    hasInternalHeap_ = TRUE;
    wHeap_ = new NAHeap("Cmp Parser Heap", NAMemory::DERIVED_FROM_SYS_HEAP, 524288, memLimit);
  ...
  potentialOlapFunctions_.setHeap(wHeap_);
...
CmpContext *Parser::cmpContext() { return cmpContext_; }
...
size_t Parser::inputStrLen() {
  ...
  if (inputBuf_ == NULL || inputBuf_->getBufSize() <= 0 || inputBuf_->getStrLen() <= 0) return 0;
...
size_t Parser::wInputStrLen() {
  ...
  if (wInputBuf_ == NULL || wInputBuf_->getBufSize() <= 0 || wInputBuf_->getStrLen() <= 0) return 0;
...
NABoolean Parser::fixupParserInputBufAndAppendSemicolon() {
  ...
  if (inputBuf_ == NULL || inputBuf_->getBufSize() <= 0 || inputStrLen() <= 0) {
    ...
    inputBuf_->data()[0] = semicolon;
    inputBuf_->setStrLen(1);
...
Int32 Parser::SetAndHoldCqdsInHint() {
  ...
  if (pHint) {
    ...
    if (cqdsInHint && cqdsInHint->entries() > 0) {
      ...
      for (iter.getNext(key, value); key != NULL && value != NULL; iter.getNext(key, value)) {
        attrEnum = defs.validate(key->data(), *value, FALSE);
        if (attrEnum < 0) {
          nRet = -1;
          break;
        }
        defs.holdOrRestore(key->data(), 1 /*hold)*/);
        defs.validateAndInsert(key->data(), *value, FALSE);
        restoreList.insert(NAString(key->data()));
#ifndef NDEBUG
        debugStr += key->data();
        debugStr += ":";
        debugStr += value->data();
  ...
  if (nRet == -1 && restoreList.entries() > 0) {
    for (CollIndex i = 0; i < restoreList.entries(); i++) defs.holdOrRestore(restoreList[i].data(), 2 /*restore)*/);
  }
#ifndef NDEBUG
  if (restoreList.entries() > 0) QRLogger::log(CAT_SQL_EXE, LL_INFO, "Set CQDs in Hint :%s", debugStr.data());
#endif
  return nRet;
}
...
NABoolean Parser::fixupParserWInputBufAndAppendSemicolon() {
  ...
  if (wInputBuf_ == NULL || wInputBuf_->getBufSize() <= 0 || wInputStrLen() <= 0) {
    ...
    wInputBuf_->data()[0] = semicolon;
    wInputBuf_->setStrLen(1);
...
static NAWcharBuf *parserCharSetToUTF16(const charBuf &inCharBuf, CollHeap *heap, NAWcharBuf *&outNAWcharBuf,
  ...
  result = csetToUnicode(inCharBuf, heap, outNAWcharBuf, inStrCharSet, outErrorCode, addNullAtEnd, outCharCount,
                         outErrorByteOff);
  if (outErrorCode) {
    if (outErrorCode == CNV_ERR_INVALID_CHAR) {
      *CmpCommon::diags() << DgSqlCode(-2109) << DgString0(CharInfo::getCharSetName((CharInfo::CharSet)inStrCharSet))
                          << DgString1("UCS2") << DgInt0(*outCharCount) << DgInt1(*outErrorByteOff);
    } else if (outErrorCode == CNV_ERR_BUFFER_OVERRUN)
      *CmpCommon::diags() << DgSqlCode(-2110) << DgString0(CharInfo::getCharSetName((CharInfo::CharSet)inStrCharSet))
                          << DgString1("UCS2");
    else
      PARSERASSERT(false);
    outNAWcharBuf = NULL;
                              ...
                              Int32 *outErrorByteOff)           // default is NULL
{
  ...
  result = unicodeTocset(pr_UTF16StrBuf, heap, pr_pOutCharSetStrBuf, inStrCharSet, outErrorCode, addNullAtEnd,
                         allowInvalidCodePoint, outCharCount, outErrorByteOff);
  if (outErrorCode) {
    if (outErrorCode == CNV_ERR_INVALID_CHAR) {
      *CmpCommon::diags() << DgSqlCode(-2109) << DgString0(CharInfo::getCharSetName((CharInfo::CharSet)inStrCharSet))
                          << DgString1("UCS2") << DgInt0(*outCharCount) << DgInt1(*outErrorByteOff);
...
static NABoolean stringScanWillTerminateInParser(const NAWchar *str, Int32 internalExpr, Int32 sLen) {
  ...
  if (quote_seen) {
    // Unmatched quote
    *SqlParser_Diags << DgSqlCode(-SQLCI_INPUT_MISSING_QUOTE) << DgWString0(NAWString(WIDE_("\n")) + str);
    return FALSE;
  }
                       ...
                       ItemExprList *paramItemList)  // (IN) : assigned to SqlParser_ParamItemList
{
  ...
  // DDLExpr node now.
  if (!internalExpr) {
    if (processSpecialDDL(inputStr(), inputStrLen(), NULL, (CharInfo::CharSet)inputStrCharSet(), node)) {
  ...
  if (!internalExpr) {
    parseUtilISPCommand(inputStr(), inputStrLen(), (CharInfo::CharSet)inputStrCharSet(), &utilISPNode);
  }
  ...
  if (cmpContext()) {
    if (cmpContext()->getParserResetIsNeeded())
      ...
      cmpContext()->setParserResetIsNeeded(TRUE);
  } else {
    if (resetIsNeeded)
      reset(TRUE);
    else
  ...
  SqlParser_NADefaults_Glob = ActiveSchemaDB()->getDefaults().getSqlParser_NADefaults();
  ...
  if (cmpContext() && ((cmpContext()->internalCompile() == CmpContext::INTERNAL_MODULENAME) ||
                       (cmpContext()->statement() && cmpContext()->statement()->isSMDRecompile())))
    Set_SqlParser_Flags(ALLOW_SPECIALTABLETYPE);

  if (internalExpr == INTERNALEXPR_TOKEN) Set_SqlParser_Flags(ALLOW_UNKNOWN_CHARSET);

  try {
    if (wInputStr() && stringScanWillTerminateInParser(wInputStr(), internalExpr, wInputStrLen())) {
    ...
    if (!parseError && initialErrCnt < SqlParser_Diags->getNumber(DgSqlCode::ERROR_)) parseError = 1;  // error
  ...
  if (parseError && initialErrCnt >= SqlParser_Diags->getNumber(DgSqlCode::ERROR_))
  ...
  if (inputBuf_ && startWithStr) {
    NADELETE(startWithStr, NAArray, wHeap_);
    startWithStr = NULL;
  }
...
Int32 Parser::parseDML(const char *instr, Int32 inlen, CharInfo::CharSet charset, ExprNode **node, Int32 internalExpr,
  ...
  if (inputBuf_) {
    switch (charset) {
      ...
      case SQLCHARSETCODE_UTF8:
        ...
        // wInputBuf_ == NULL
        wInputBuf_ =
            parserCharSetToUTF16(*inputBuf_, wHeap(), wInputBuf_, charset, errorcode, TRUE, &charCount, &errorByteOff);
  ...
  if (wInputStrLen() > 0 && charset_ != CharInfo::UTF8) {
    ...
    inputBuf_ =
        parserUTF16ToCharSet(*wInputBuf_, wHeap(), inputBuf_, charset_, errorcode, TRUE  // NABoolean addNullAtEnd
                             ,
  ...
  if (inputStr() != NULL && inputStrLen() > 0) fixupParserInputBufAndAppendSemicolon();
  if (wInputStr() != NULL && wInputStrLen() > 0) fixupParserWInputBufAndAppendSemicolon();
...
Int32 Parser::parseDML(QueryText &txt, ExprNode **node, Int32 internalExpr, ItemExprList *paramItemList) {
  ...
  switch ((SQLCHARSET_CODE)charset_) {
    ...
    case SQLCHARSETCODE_UTF8:
      ...
      wInputBuf_ =
          parserCharSetToUTF16(*inputBuf_, wHeap(), wInputBuf_, charset_, errorcode, TRUE, &charCount, &errorByteOff);
      if (errorcode) return 1;
      break;
    default: {
      Int32 CharsetNotSupported = 0;
                              ...
                              Int32 internal_expr)  // getItemExprTree is caller
{
  ...
  while (newlen > 0 && str[newlen - 1] == 0) newlen--;
  ...
  if (num_params > 0 || (otherParams && otherParams->entries() > 0)) {
    ...
    if (otherParams && otherParams->entries() > 0) paramItemList->insert(*otherParams);
  }
  ...
  if (newstr != str) NADELETEBASIC(newstr, wHeap());
...
ItemExpr *Parser::getItemExprTree(const char *str, UInt32 len, CharInfo::CharSet strCharSet, Int32 num_params,
                                  ...
                                  ItemExprList *paramItemList) {
  ExprNode *et =
      getExprTree(str, len, strCharSet, num_params, p1, p2, p3, p4, p5, p6, paramItemList, INTERNALEXPR_TOKEN);

  PARSERASSERT(et == NULL || (et->getOperatorType() >= ITM_FIRST_ITEM_OP && et->getOperatorType() <= ITM_LAST_ITEM_OP));
  return (ItemExpr *)et;
}

ItemExpr *Parser::getItemExprTree(const char *str, UInt32 len, CharInfo::CharSet strCharSet,
  ...
  while (i < 6) firstSix[i++] = NULL;

  while (i < params.entries()) remainder.insert(params[i++]);

  return getItemExprTree(str, len, strCharSet, params.entries(), firstSix[0], firstSix[1], firstSix[2], firstSix[3],
...
NABoolean Parser::parseUtilISPCommand(const char *command, size_t cmdLen, CharInfo::CharSet cmdCharSet,
                                      ExprNode **node) {
  ...
  static const char *UtilISPToken[] = {"PURGEDATA", "POPULATE", "RECOVER",   "REFRESH", "UPGRADE",
                                       "DOWNGRADE", "VALIDATE", "TRANSFORM", ""};

  static const char *UtilISPName[] = {"sp_purgedata", "sp_populate",  "sp_recover",
                                      "sp_refresh",   "sp_SchLevel",  "sp_SchLevel",
                                      "sp_validate",  "sp_transform", ""};
  ...
  for (; *puc != '\0'; puc++) *puc = (unsigned char)(TOUPPER(*puc));
  ...
  if (p) {
    ...
    for (index = 0; !utilISPFound && strlen(UtilISPToken[index]) > 0; index++)
      if (_stricmp(p, UtilISPToken[index]) == 0) {
        ...
        if (_stricmp(p, "PURGEDATA") == 0) isPurgedata = TRUE;
      }
  }
...
NABoolean Parser::processSpecialDDL(const char *inputStr, size_t inputStrLen, ExprNode *childNode,
  ...
  NAString ns(wHeap());
  if (inputStr != NULL) ns.append(inputStr, (size_t)newStrLen);
  ...
  if (specialDDL) {
    *node = NULL;
    DDLExpr *ddlExpr = new (CmpCommon::statementHeap())
        DDLExpr(childNode, (char *)ns.data(), inputStrCharSet, CmpCommon::statementHeap());
    RelExpr *queryExpr = new (CmpCommon::statementHeap()) RelRoot(ddlExpr);
    ...
    if (displayFound) ((RelRoot *)queryExpr)->setDisplayTree(TRUE);
...
void HQCParseKey::addTokenToNormalizedString(Int32 &tokCod) {
  if (SqlParser_CurrentParser->getLexer()->isLiteral4HQC(tokCod)) {
    ...
    if (tok) {
      ...
      if (nOfTokens_ == 0 && (strncmp(tok->data(), "select", 6) == 0
                              ...
                              || strncmp(tok->data(), "update", 6) == 0 || strncmp(tok->data(), "delete", 6) == 0))
        setIsCacheable(TRUE);
