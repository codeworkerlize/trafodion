...
NAHashDictionary<NAString,NAString>* Parser::getOrNewCqdHashDictionaryInHint()
{
  if (pCqdsHashDictionaryInHint_ == NULL) 
    pCqdsHashDictionaryInHint_ = 
      new (wHeap_) NAHashDictionary<NAString,NAString>(&NAString::hash, 20, TRUE, wHeap_);
...
Int32 Parser::SetAndHoldCqdsInHint()
{
  ...
  if (nRet == -1 && restoreList.entries()>0)
    {
      for (CollIndex i = 0; i < restoreList.entries(); i++)
        defs.holdOrRestore(restoreList[i].data(),2/*restore)*/);
    }
  #ifndef NDEBUG
  if (restoreList.entries()>0)
    QRLogger::log(CAT_SQL_EXE, LL_INFO,"Set CQDs in Hint :%s",debugStr.data());
  #endif
  return nRet;
}
...
NAType* Parser::parseCompositeDefinition(const char* str, size_t strLen, 
                                         CharInfo::CharSet strCharSet)
{
  ExprNode* node;
  // If strLen is passed in, use it so non-null-terminated strings can be passed to parser.
