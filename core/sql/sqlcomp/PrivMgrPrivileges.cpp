   ...
   const bool isWGOSpecified)
{
  ...
  for (size_t i = 0; i < colPrivsToGrant.entries(); i++)
  {
    ...
    {
      int64_t rowCount = 0;
      if (columnPrivsTable.updateColumnRow(row,whereBase,rowCount) == STATUS_ERROR)
        return STATUS_ERROR;
      if (rowCount == 0)
      {
    ...
    {
      if (columnPrivsTable.insert(row) == STATUS_ERROR)
        return privStatus;
    }
      ...
      const bool isWGOSpecified)
{
  ...
  if (foundRow)
  {
    ...
    sprintf(buf," WHERE object_uid = %ld AND grantor_id = %d AND grantee_id = %d ",
      ...
      const PrivObjectBitmap grantableBitmap)
{
  ...
  if (ComUser::getAuthNameFromAuthID(grantorID_, tempName, sizeof(tempName), 
                                     length, FALSE, pDiags_) < 0)
    return STATUS_ERROR;
  std::string grantorName(tempName);

  if (ComUser::getAuthNameFromAuthID(granteeID, tempName, sizeof(tempName), 
                                     length, FALSE, pDiags_) < 0)
    return STATUS_ERROR;
...
// error 8732: <grantedByName> is not a registered user or role
  if (ComUser::getAuthIDFromAuthName(grantedByName.c_str(),effectiveGrantorID, pDiags_))
    return STATUS_ERROR;
  ...
  PrivMgrDesc &privsToProcess)
{
  ...
  if (isAllSpecified)
  {
    ...
    privsToProcess.setAllObjectPrivileges(objectType, !isGofSpecified, isWgoSpecified, true/*all_dml*/,true/*all_ddl*/);
    return STATUS_GOOD;
  }
  ...
  for (int32_t i = 0; i < privsList.size();++i)
  {
    switch (privsList[i])
    {
      ...
      case DROP_PRIV:
        ...
        break;
      case ALL_DDL:
        tableCorePrivs.setAllObjectPrivileges(objectType, !isGofSpecified, isWgoSpecified, 
                                              false/*all_dml*/,true/*all_ddl*/);
        break;
        
      case ALL_DML:
        tableCorePrivs.setAllObjectPrivileges(objectType, !isGofSpecified, isWgoSpecified, 
                                              true/*all_dml*/,false/*all_ddl*/);
        break;
      default:
      {
...
// ---------------------------------------------------------------------------- 
PrivStatus PrivMgrPrivileges::updateObjectPrivRows(
  const std::string &setClause,
  const std::string &whereClause,
  int64_t &rowCount)
{
...
// ----------------------------------------------------------------------------
PrivStatus ObjectPrivsMDTable::updateWhere(
  const std::string & setClause,
  const std::string & whereClause,
  int64_t &rowCount)
{
