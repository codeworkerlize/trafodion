...
class ObjectPrivsMDRow : public PrivMgrMDRow {
 public:
  ...
  // -------------------------------------------------------------------
  ObjectPrivsMDRow()
      : PrivMgrMDRow(PRIVMGR_OBJECT_PRIVILEGES, OBJECT_PRIVILEGES_ENUM), objectUID_(0), grantorID_(0), granteeID_(0){};

  ObjectPrivsMDRow(const ObjectPrivsMDRow &other) : PrivMgrMDRow(other) {
  ...
  bool isChanged() { return (current_ == PrivMgrCoreDesc(privsBitmap_, grantableBitmap_)); }
...
class ObjectPrivsMDTable : public PrivMgrMDTable {
 public:
  ObjectPrivsMDTable(const std::string &tableName, ComDiagsArea *pDiags = NULL)
  ...
  PrivStatus selectWhere(const std::string &whereClause, const std::string &orderByClause,
  ...
  PrivStatus updateWhere(const std::string &setClause, const std::string &whereClause, int64_t &rowCount);
  PrivStatus insertSelect(const std::string &objectsLocation, const std::string &authsLocation);
...
class ColumnPrivsMDRow : public PrivMgrMDRow {
 public:
  ...
  ColumnPrivsMDRow(const ColumnPrivsMDRow &other) : PrivMgrMDRow(other) {
    objectUID_ = other.objectUID_;
    objectName_ = other.objectName_;
    granteeID_ = other.granteeID_;
    granteeName_ = other.granteeName_;
  ...
  PrivMgrRowInfo getPrivRowInfo() {
    ...
    rowInfo.setGranteeID(granteeID_);
    rowInfo.setGranteeName(granteeName_);
    rowInfo.setColumnOrdinal(columnOrdinal_);
    rowInfo.setPrivsBitmap(privsBitmap_);
    rowInfo.setGrantableBitmap(grantableBitmap_);
  ...
  NABoolean anyNotVisited() {
    return current_.getPrivBitmap() != visited_.getPrivBitmap() || current_.getWgoBitmap() != visited_.getWgoBitmap();
  }
...
class ColumnPrivsMDTable : public PrivMgrMDTable {
 public:
  ColumnPrivsMDTable(const std::string &tableName, ComDiagsArea *pDiags = NULL)
  ...
  PrivStatus selectWhere(const std::string &whereClause, const std::string &orderByClause,
                         std::vector<PrivMgrMDRow *> &rowList);
  PrivStatus updateColumnRow(const ColumnPrivsMDRow &row, const std::string whereBase, int64_t &rowCount);

 private:
  ColumnPrivsMDTable();
  void setRow(OutputInfo *pCliRow, ColumnPrivsMDRow &rowOut);
};
...
static PrivMgrCoreDesc *findColumnEntry(NAList<PrivMgrCoreDesc> &colPrivsToGrant, const int32_t columnsOrdinal);

static void getColRowsForGranteeGrantor(const std::vector<PrivMgrMDRow *> &columnRowList, const int32_t granteeID,
                                        const int32_t grantorID, NAList<PrivMgrCoreDesc> &colPrivGrants);

static bool hasAllDMLPrivs(ComObjectType objectType, PrivObjectBitmap privBitmap);
...
// Default Constructor
// -----------------------------------------------------------------------
PrivMgrPrivileges::PrivMgrPrivileges() : PrivMgr(), objectUID_(0), grantorID_(0) {
...
// -----------------------------------------------------------------------
PrivMgrPrivileges::PrivMgrPrivileges(const int64_t objectUID, const std::string &objectName, const int32_t grantorID,
                                     const std::string &metadataLocation, ComDiagsArea *pDiags)
    : PrivMgr(metadataLocation, pDiags), objectUID_(objectUID), objectName_(objectName), grantorID_(grantorID) {
...
// ----------------------------------------------------------------------------
PrivMgrPrivileges::PrivMgrPrivileges(const PrivMgrObjectInfo &objectInfo, const std::string &metadataLocation,
                                     ComDiagsArea *pDiags)
...
// ----------------------------------------------------------------------------
PrivMgrPrivileges::PrivMgrPrivileges(const int64_t objectUID, const std::string &metadataLocation, ComDiagsArea *pDiags)
    : PrivMgr(metadataLocation, pDiags), objectUID_(objectUID), grantorID_(0) {
  objectTableName_ = metadataLocation + "." + PRIVMGR_OBJECT_PRIVILEGES;
  columnTableName_ = metadataLocation + "." + PRIVMGR_COLUMN_PRIVILEGES;
}
...
// ----------------------------------------------------------------------------
PrivMgrPrivileges::PrivMgrPrivileges(const std::string &metadataLocation, ComDiagsArea *pDiags)
    : PrivMgr(metadataLocation, pDiags), objectUID_(0), grantorID_(0) {
  objectTableName_ = metadataLocation + "." + PRIVMGR_OBJECT_PRIVILEGES;
  columnTableName_ = metadataLocation + "." + PRIVMGR_COLUMN_PRIVILEGES;
}

PrivMgrPrivileges::PrivMgrPrivileges(const std::string &trafMetadataLocation, const std::string &metadataLocation,
                                     ComDiagsArea *pDiags)
    : PrivMgr(trafMetadataLocation, metadataLocation, pDiags), objectUID_(0), grantorID_(0) {
  objectTableName_ = metadataLocation + "." + PRIVMGR_OBJECT_PRIVILEGES;
  columnTableName_ = metadataLocation + "." + PRIVMGR_COLUMN_PRIVILEGES;
}
...
// Copy constructor
// -----------------------------------------------------------------------
PrivMgrPrivileges::PrivMgrPrivileges(const PrivMgrPrivileges &other) : PrivMgr(other) {
  objectUID_ = other.objectUID_;
  objectName_ = other.objectName_;
  grantorID_ = other.grantorID_;
...
// *
// *****************************************************************************
PrivStatus PrivMgrPrivileges::getPrivsOnObject(const ComObjectType objectType, std::vector<PrivMgrDesc> &privDescs) {
  ...
  if (generateObjectRowList() == STATUS_ERROR) return STATUS_ERROR;

  // generate the list of privileges granted to the column and store in class
  if (generateColumnRowList() == STATUS_ERROR) return STATUS_ERROR;
  ...
  if (getDistinctIDs(objectRowList_, columnRowList_, userIDs, roleIDs) == STATUS_ERROR) return STATUS_ERROR;
  ...
  for (size_t i = 0; i < userIDs.size(); i++) {
    ...
    if (getUserPrivs(objectType, userID, emptyRoleIDs, privsOfTheUser, hasManagePrivPriv) != STATUS_GOOD)
      return STATUS_ERROR;

    if (!privsOfTheUser.isNull()) privDescs.push_back(privsOfTheUser);
  }
  ...
  for (size_t i = 0; i < roleIDs.size(); i++) {
    ...
    // all grantors.
    if (getUserPrivs(objectType, grantee, emptyRoleIDs, privsOfTheUser, hasManagePrivPriv) != STATUS_GOOD)
      return STATUS_ERROR;
...
//
// *****************************************************************************
void PrivMgrPrivileges::getColRowsForGranteeOrdinal(const int32_t granteeID, const int32_t columnOrdinal,
  ...
  for (size_t i = 0; i < columnRowList.size(); ++i) {
    ...
    if (row.columnOrdinal_ == columnOrdinal) {
      if (row.granteeID_ == granteeID || std::find(roleIDs.begin(), roleIDs.end(), row.granteeID_) != roleIDs.end()) {
...
// * object
// *****************************************************************************
PrivStatus PrivMgrPrivileges::getDistinctIDs(const std::vector<PrivMgrMDRow *> &objectRowList,
                                             const std::vector<PrivMgrMDRow *> &columnRowList,
                                             std::vector<int32_t> &userIDs, std::vector<int32_t> &roleIDs) {
  int32_t authID;
...
// *
// *****************************************************************************
PrivStatus PrivMgrPrivileges::getPrivRowsForObject(const int64_t objectUID,
...
// ----------------------------------------------------------------------------
void PrivMgrPrivileges::getTreeOfGrantors(const int32_t granteeID, std::set<int32_t> &listOfGrantors) {
...
// *
// *****************************************************************************
PrivStatus PrivMgrPrivileges::givePrivForObjects(const int32_t currentOwnerID, const int32_t newOwnerID,
...
// *
// *****************************************************************************
PrivStatus PrivMgrPrivileges::givePriv(const int32_t currentOwnerID, const int32_t newOwnerID,
                                       const std::string &newOwnerName, const int64_t objectUID)

{
  ...
  sprintf(buf, "WHERE object_uid = %ld AND grantee_ID = %d", objectUID, newOwnerID);

  privStatus = objectPrivsTable.deleteWhere(buf);

  if (privStatus != STATUS_GOOD) return privStatus;

  sprintf(buf, "WHERE object_uid = %ld AND (grantor_ID = %d OR grantor_ID = %d) ", objectUID, newOwnerID,
          currentOwnerID);
  ...
  sprintf(setClause, " SET GRANTEE_ID = %d, GRANTEE_NAME = '%s' ", newOwnerID, newOwnerName.c_str());
  sprintf(whereClause, " WHERE GRANTOR_ID = %d ", SYSTEM_USER);

  int64_t rowCount = 0;
  privStatus = objectPrivsTable.updateWhere(setClause, whereClause, rowCount);
  if (privStatus != STATUS_GOOD) return privStatus;
...
// *
// *****************************************************************************
PrivStatus PrivMgrPrivileges::grantColumnPriv(const ComObjectType objectType, const int32_t granteeID,
                                              const std::string &granteeName, const std::string &grantorName,
                                              const PrivMgrDesc &privsToGrant, const bool isWGOSpecified) {
  std::string traceMsg;
  ...
  if (objectType == COM_VIEW_OBJECT) {
    ...
    if (admin.getViewColUsages(myUsage) == STATUS_ERROR) return STATUS_ERROR;
    if (myUsage.viewColUsagesStr.empty()) {
      *pDiags_ << DgSqlCode(-CAT_COLUMN_PRIVILEGE_NOT_ALLOWED) << DgTableName(objectName_.c_str());
      return STATUS_ERROR;
    }
  }
  ...
  getColRowsForGranteeGrantor(columnRowList_, granteeID, grantorID_, grantedColPrivs);
  ...
  for (size_t i = 0; i < colPrivsToGrant.entries(); i++) {
    ...
    for (size_t i = 0; i < colPrivsToGrant.entries(); i++) {
      ...
      if (grantedColPriv) adjustedPrivs = *grantedColPriv;
    ...
    if (updateDependentObjects(objectUsage, PrivCommand::GRANT_COLUMN) == STATUS_ERROR) return STATUS_ERROR;
    ...
    if (updateOperation) {
      int64_t rowCount = 0;
      if (columnPrivsTable.updateColumnRow(row, whereBase, rowCount) == STATUS_ERROR) return STATUS_ERROR;
      if (rowCount == 0) {
        std::string msg("Failed to update privilege bitmaps for column grant command on .");
        msg += row.objectName_;
      ...
      if (columnPrivsTable.insert(row) == STATUS_ERROR) return privStatus;
    }
...
PrivStatus PrivMgrPrivileges::grantObjectPriv(const ComObjectType objectType, const int32_t granteeID,
  ...
  if (nameRequested == nameToCheck && grantorID_ == SYSTEM_USER) return STATUS_GOOD;
  ...
  retcode = initGrantRevoke(objectType, granteeID, grantorName, privsList, colPrivsArray, isAllSpecified,
                            isWGOSpecified, true, privsToGrant, privsOfTheGrantor, roleIDs);
  ...
  if (enabled && isRoleID(granteeID) && doTenantPrivChecks()) {
    ...
    retcode = roles.hasRoleAdmin(granteeID, grantorID_, granteeName, roleIDs, hasAdmin);
    if (retcode == STATUS_ERROR) return STATUS_ERROR;
    if (!hasAdmin) {
      *pDiags_ << DgSqlCode(-CAT_PRIV_NOT_GRANTED_TO_ROLE) << DgString0(granteeName.c_str());
      return STATUS_ERROR;
    }
  }
  ...
  if (privsToGrant.limitToGrantable(privsOfTheGrantor)) warnNotAll = true;
  ...
  if (privsToGrant.isNull()) {
    ...
    if (getRolesToCheck(grantorID_, roleIDs, objectType, rolesWithPrivs) == STATUS_GOOD) {
      if (rolesWithPrivs.size() > 0) {
        *pDiags_ << DgSqlCode(-CAT_PRIVILEGE_NOT_GRANTED) << DgString0(grantorName.c_str())
  ...
  if (!privsToGrant.isColumnLevelNull()) {
    retcode = grantColumnPriv(objectType, granteeID, granteeName, grantorName, privsToGrant, isWGOSpecified);
    if (retcode != STATUS_GOOD) return retcode;
  }
  ...
  if (privsToGrant.getTablePrivs().isNull()) {
    ...
    if (warnNotAll) reportPrivWarnings(origPrivsToGrant, privsToGrant, CAT_NOT_ALL_PRIVILEGES_GRANTED);
  ...
  if (updateDependentObjects(objectUsage, PrivCommand::GRANT_OBJECT) == STATUS_ERROR) return STATUS_ERROR;
  ...
  if (foundRow) {
    ...
    // update the row
    char buf[500];
    sprintf(buf, " SET privileges_bitmap = %ld, grantable_bitmap = %ld  ", row.privsBitmap_.to_ulong(),
            row.grantableBitmap_.to_ulong());
    std::string setClause(buf);

    sprintf(buf, " WHERE object_uid = %ld AND grantor_id = %d AND grantee_id = %d ", row.objectUID_, row.grantorID_,
            row.granteeID_);
    std::string whereClause(buf);

    int64_t rowCount = 0;
    if (objectPrivsTable.updateWhere(setClause, whereClause, rowCount) == STATUS_ERROR) return STATUS_ERROR;

    if (rowCount == 0) {
      std::string msg("Failed to update privilege bitmaps for grant command on .");
      msg += row.objectName_;
      PRIVMGR_INTERNAL_ERROR(msg.c_str());
      return STATUS_ERROR;
    }
...
PrivStatus PrivMgrPrivileges::grantObjectPriv(const ComObjectType objectType, const int32_t granteeID,
  ...
  if (ComUser::getAuthNameFromAuthID(grantorID_, tempName, sizeof(tempName), length, FALSE, pDiags_) < 0)
    return STATUS_ERROR;
  std::string grantorName(tempName);

  if (ComUser::getAuthNameFromAuthID(granteeID, tempName, sizeof(tempName), length, FALSE, pDiags_) < 0)
    return STATUS_ERROR;
  std::string granteeName(tempName);
...

{
  ...
  corePrivs.setAllObjectPrivileges(objectType, true /*priv*/, true /*wgo*/, true /*all_dml*/, true /*all_ddl*/);
...
// *****************************************************************************
PrivStatus PrivMgrPrivileges::initGrantRevoke(const ComObjectType objectType, const int32_t granteeID,
                                              const std::string &grantorName, const std::vector<PrivType> &privList,
                                              const std::vector<ColPrivSpec> &colPrivsArray, const bool isAllSpecified,
                                              const bool isGOSpecified, const bool isGrant, PrivMgrDesc &privsToApply,
                                              PrivMgrDesc &privsOfTheGrantor, std::vector<int32_t> &roleIDs) {
  if (objectUID_ == 0) {
  ...
  PrivStatus retcode = convertPrivsToDesc(objectType, isAllSpecified,
                                          (isGrant) ? isGOSpecified : true,   // WGO
                                          (isGrant) ? false : isGOSpecified,  // GOF
                                          privList, colPrivsArray, privsToApply);
  if (retcode != STATUS_GOOD) return retcode;
  ...
  retcode = getUserPrivs(objectType, grantorID_, roleIDs, privsOfTheGrantor, hasManagePrivPriv);
  if (retcode != STATUS_GOOD) return retcode;

  // get roleIDs for the grantor
  retcode = getRoleIDsForUserID(grantorID_, roleIDs);
  if (retcode == STATUS_ERROR) return retcode;
...
// *
// *****************************************************************************
PrivStatus PrivMgrPrivileges::insertPrivRowsForObject(const int64_t objectUID,
...
//
// ****************************************************************************
PrivStatus PrivMgrPrivileges::dealWithConstraints(const ObjectUsage &objectUsage,
  ...
  if (objectUsage.objectType != COM_BASE_TABLE_OBJECT) return STATUS_GOOD;
  ...
  for (size_t i = 0; i < objectList.size(); i++) {
    ...
    retcode = summarizeCurrentAndOriginalPrivs(objectUsage.objectUID, pObjectRef->objectOwner, grantorID_, roleIDs,
                                               listOfAffectedObjects, originalPrivs, currentPrivs);
    if (retcode != STATUS_GOOD) return retcode;
    ...
    if (!thePrivs.getPriv(REFERENCES_PRIV)) {
      ...
      summarizeColPrivs(*pObjectRef, pObjectRef->objectOwner, grantorID_, roleIDs, listOfAffectedObjects,
                        summarizedColRefs);
        ...
        for (size_t j = 0; j < summarizedColRefs.size(); j++) {
          ...
          if (existingRef->columnOrdinal == neededColRef->columnOrdinal) {
            ...
            if (!colPrivs.getPriv(REFERENCES_PRIV)) {
              ...
              if (!admin.getConstraintName(objectUsage.objectUID, pObjectRef->objectUID, neededColRef->columnOrdinal,
                                           referencingTable)) {
                referencingTable = "UNKNOWN, Referencing table ID is ";
                referencingTable += UIDToString(pObjectRef->objectUID);
              }

              *pDiags_ << DgSqlCode(-CAT_DEPENDENT_OBJECTS_EXIST) << DgString0(referencingTable.c_str());
      ...
      while (!summarizedColRefs.empty()) delete summarizedColRefs.back(), summarizedColRefs.pop_back();
    }
  }
...
//
// ****************************************************************************
PrivStatus PrivMgrPrivileges::dealWithUdrs(const ObjectUsage &objectUsage,
                                           std::vector<ObjectUsage *> &listOfAffectedObjects) {
  PrivStatus retcode = STATUS_GOOD;

  // udrs (functions and procedures) can only be defined for in libraries
  if (objectUsage.objectType != COM_LIBRARY_OBJECT) return STATUS_GOOD;
  ...
  if (objectList.size() > 0) {
    ...
    retcode = summarizeCurrentAndOriginalPrivs(objectUsage.objectUID, objectUsage.granteeID, grantorID_, roleIDs,
                                               listOfAffectedObjects, originalPrivs, currentPrivs);
    if (retcode != STATUS_GOOD) return retcode;
    ...
    if (!thePrivs.getPriv(USAGE_PRIV)) {
      ...
      *pDiags_ << DgSqlCode(-CAT_DEPENDENT_OBJECTS_EXIST) << DgString0(pObjectRef->objectName.c_str());
      return STATUS_ERROR;
    }
  }
...
// metadata everytime.
// ****************************************************************************
PrivStatus PrivMgrPrivileges::dealWithViews(const ObjectUsage &objectUsage, const PrivCommand command,
  ...
  traceMsg += privStatusEnumToLit(retcode);
  log(__FILE__, traceMsg, -1);
  if (retcode == STATUS_NOTFOUND) return STATUS_GOOD;
  if (retcode != STATUS_GOOD && retcode != STATUS_WARNING) return retcode;
...
//
// ----------------------------------------------------------------------------
PrivStatus PrivMgrPrivileges::gatherViewColUsages(ObjectReference *objectRef, ViewUsage &viewUsage,
...
//
// ----------------------------------------------------------------------------
PrivStatus PrivMgrPrivileges::gatherViewPrivileges(ViewUsage &viewUsage, const PrivCommand command,
                                                   const int32_t grantorID,
  ...
  summarizedOriginalPrivs.setAllTableGrantPrivileges(true /*priv*/, setWGOtrue, true /*all_dml*/, true /*all_ddl*/);
  PrivMgrDesc summarizedCurrentPrivs;
  summarizedCurrentPrivs.setAllTableGrantPrivileges(true /*priv*/, setWGOtrue, true /*all_dml*/, true /*all_ddl*/);
  ...
  for (size_t i = 0; i < objectList.size(); i++) {
    ...
    retcode = summarizeCurrentAndOriginalPrivs(pObjectRef->objectUID, viewUsage.viewOwner, grantorID, roleIDs,
                                               listOfAffectedObjects, originalPrivs, currentPrivs);
    if (retcode != STATUS_GOOD) return retcode;
    ...
    if (isRevokeCommand(command)) {
      if (pObjectRef->objectType == COM_SEQUENCE_GENERATOR_OBJECT) {
        PrivMgrCoreDesc thePrivs = currentPrivs.getTablePrivs();
        if (!thePrivs.getPriv(USAGE_PRIV)) {
          *pDiags_ << DgSqlCode(-CAT_DEPENDENT_OBJECTS_EXIST) << DgString0(viewUsage.viewName.c_str());
          return STATUS_ERROR;
        }
      }
    }
  ...
  for (size_t i = FIRST_DML_COL_PRIV; i <= LAST_DML_COL_PRIV; i++) {
    ...
    if (thePrivs.getPriv(privType) && thePrivs.getWgo(privType)) continue;
    checkCols = true;
    break;
  }
  ...
  originalPrivs.setAllTableGrantPrivileges(true /*priv*/, setWGOtrue, true /*all_dml*/, true /*all_ddl*/);
  currentPrivs.setAllTableGrantPrivileges(true /*priv*/, setWGOtrue, true /*all_dml*/, true /*all_ddl*/);
  ...
  for (size_t i = 0; i < objectList.size(); i++) {
    ...
    if (retcode == STATUS_NOTFOUND) {
      if (command == GRANT_COLUMN) {
        *pDiags_ << DgSqlCode(-CAT_COLUMN_PRIVILEGE_NOT_ALLOWED) << DgTableName(viewUsage.viewName.c_str());
        return STATUS_ERROR;
      }
      return STATUS_GOOD;
    }
    ...
    summarizeColPrivs(*pObjectRef, viewUsage.viewOwner, grantorID, roleIDs, listOfAffectedObjects, summarizedColRefs);
    ...
    for (size_t j = 0; j < viewColUsages.size(); j++) {
      ...
      if (colUsage.getRefdUID() != pObjectRef->objectUID) continue;
...
PrivStatus PrivMgrPrivileges::generateColumnRowList() {
  ...
  if (columnRowList_.size() > 0) return STATUS_GOOD;
...
PrivStatus PrivMgrPrivileges::getColumnRowList(const int64_t objectUID, std::vector<PrivMgrMDRow *> &columnRowList) {
  ...
  ColumnPrivsMDTable columnPrivsTable(columnTableName_, pDiags_);
  PrivStatus privStatus = columnPrivsTable.selectWhere(whereClause, orderByClause, columnRowList);
...
PrivStatus PrivMgrPrivileges::generateObjectRowList() {
  ...
  if (objectRowList_.size() > 0) return STATUS_GOOD;
...
//   STATUS_ERROR    - the diags area is populated with any errors
// ****************************************************************************
PrivStatus PrivMgrPrivileges::getObjectRowList(const int64_t objectUID, std::vector<PrivMgrMDRow *> &objectRowList) {
  std::string whereClause("where object_uid = ");
  whereClause += UIDToString(objectUID);
  std::string orderByClause(" order by grantor_id, grantee_id ");

  ObjectPrivsMDTable objectPrivsTable(objectTableName_, pDiags_);
  PrivStatus privStatus = objectPrivsTable.selectWhere(whereClause, orderByClause, objectRowList);
  if (privStatus == STATUS_ERROR) return privStatus;
...
//   listOfAffectedObjects - returns the list of affected objects
// ****************************************************************************
PrivStatus PrivMgrPrivileges::getAffectedObjects(const ObjectUsage &objectUsage, const PrivCommand command,
                                                 std::vector<ObjectUsage *> &listOfAffectedObjects) {
  PrivStatus retcode = STATUS_GOOD;
  std::string traceMsg;
...
// ----------------------------------------------------------------------------
PrivStatus PrivMgrPrivileges::getGrantedPrivs(const int32_t granteeID, PrivMgrMDRow &rowOut) {
...
// ----------------------------------------------------------------------------
PrivStatus PrivMgrPrivileges::getGranteesForViewUsage(const ViewUsage &viewUsage, std::set<int32_t> &granteeList) {
  PrivStatus privStatus = STATUS_GOOD;
...
// ----------------------------------------------------------------------------
PrivStatus PrivMgrPrivileges::getGrantorDetailsForObject(const bool isGrantedBySpecified,
                                                         const std::string grantedByName, const int objectOwner,
                                                         int &effectiveGrantorID, std::string &effectiveGrantorName)

{
  ...
  if (ComUser::getAuthIDFromAuthName(grantedByName.c_str(), effectiveGrantorID, pDiags_)) return STATUS_ERROR;
  ...
  if (ComUser::isRootUserID() || currentUser == objectOwner) return STATUS_GOOD;
  ...
  if (roles.hasRole(currentUser, effectiveGrantorID)) return STATUS_GOOD;
...
// *
// *****************************************************************************
PrivStatus PrivMgrPrivileges::revokeColumnPriv(const ComObjectType objectType, const int32_t granteeID,
                                               const std::string &granteeName, const std::string &grantorName,
                                               const PrivMgrDesc &privsToRevoke, const bool isWGOSpecified) {
  PrivStatus privStatus = STATUS_GOOD;
      ...
      if (colPrivToRevoke.anyNotSet(*grantedColPriv)) {
        ...
        for (size_t p = FIRST_DML_COL_PRIV; p <= LAST_DML_COL_PRIV; p++) {
          ...
          if (colPrivToRevoke.getPriv(type)) {
            if (!grantedColPriv->getPriv(type)) printWarning = true;
          } else {
          ...
          if (printWarning) {
            char buf[1000];
            sprintf(buf, "%s %s (columm number %d) on %s", PrivMgrUserPrivs::convertPrivTypeToLiteral(type).c_str(),
                    (colPrivToRevoke.getWgo(type)) ? "WITH GRANT OPTION" : "", colPrivToRevoke.getColumnOrdinal(),
                    objectName_.c_str());
            *pDiags_ << DgSqlCode(CAT_GRANT_NOT_FOUND) << DgString0(buf) << DgString1(grantorName.c_str())
      ...
      for (size_t p = FIRST_DML_COL_PRIV; p <= LAST_DML_COL_PRIV; p++) {
        ...
        if (colPrivToRevoke.getPriv((PrivType)p) || colPrivToRevoke.getWgo((PrivType)p)) {
          ...
          sprintf(buf, "%s %s (columm number %d) on %s",
                  ...
                  (colPrivToRevoke.getWgo((PrivType)p)) ? "WITH GRANT OPTION" : "", colPrivToRevoke.getColumnOrdinal(),
                  objectName_.c_str());
          *pDiags_ << DgSqlCode(CAT_GRANT_NOT_FOUND) << DgString0(buf) << DgString1(grantorName.c_str())
                   << DgString2(granteeName.c_str());
        }
  ...
  if (objectUsage.columnReferences->size() == 0) return STATUS_GOOD;
  ...
  if (updateDependentObjects(objectUsage, PrivCommand::REVOKE_COLUMN_RESTRICT) == STATUS_ERROR) return STATUS_ERROR;
  ...
  if (checkColumnRevokeRestrict(granteeID, colPrivsToRevoke, columnRowList_)) return STATUS_ERROR;
  ...
  for (size_t i = 0; i < colPrivsToRevoke.entries(); i++) {
    ...
    if (grantedColPriv) {
      ...
      if (!isWGOSpecified && ((colPrivToRevoke.getPrivBitmap() & grantedColPriv->getPrivBitmap()) == 0)) continue;

      // If all privileges are revoked, delete corresponding row
      if (!isWGOSpecified && (colPrivToRevoke.getPrivBitmap() == grantedColPriv->getPrivBitmap()))
        deleteRow = true;
      else
        updateRow = true;
      ...
      if (isWGOSpecified) adjustedPrivs.setPrivBitmap(grantedColPriv->getPrivBitmap());
    ...
    int64_t rowCount = 0;
    if (columnPrivsTable.updateColumnRow(row, whereBase, rowCount) == STATUS_ERROR) return privStatus;

    if (rowCount == 0) {
      std::string msg("Failed to update privilege bitmaps for column revoke command on .");
      msg += row.objectName_;
      PRIVMGR_INTERNAL_ERROR(msg.c_str());
      return STATUS_ERROR;
    }
...
// *
// *****************************************************************************
PrivStatus PrivMgrPrivileges::revokeObjectPriv(const ComObjectType objectType, const int32_t granteeID,
                                               const std::string &granteeName, const std::string &grantorName,
                                               const std::vector<PrivType> &privsList,
                                               const std::vector<ColPrivSpec> &colPrivsArray, const bool isAllSpecified,
  ...
  std::vector<int> roleIDs;
  retcode = initGrantRevoke(objectType, granteeID, grantorName, privsList, colPrivsArray, isAllSpecified,
                            isGOFSpecified, false, privsToRevoke, privsOfTheGrantor, roleIDs);
  if (retcode != STATUS_GOOD) return retcode;
  ...
  if (privsToRevoke.limitToGrantable(privsOfTheGrantor)) {
    ...
    if (!isAllSpecified) warnNotAll = true;  // Not all the specified privs can be revoked
  }
  ...
  if (!privsToRevoke.isColumnLevelNull()) {
    retcode = revokeColumnPriv(objectType, granteeID, granteeName, grantorName, privsToRevoke, isGOFSpecified);
    if (retcode != STATUS_GOOD) return retcode;
  }
  ...
  if (privsToRevoke.getTablePrivs().isNull()) {
    ...
    // report any privs not granted
    if (warnNotAll) reportPrivWarnings(origPrivsToRevoke, privsToRevoke, CAT_NOT_ALL_PRIVILEGES_REVOKED);
  ...
  if (retcode == STATUS_NOTFOUND) {
    ...
    if (isGOFSpecified) privListStr += " WITH GRANT OPTION";

    *pDiags_ << DgSqlCode(CAT_GRANT_NOT_FOUND) << DgString0(privListStr.c_str()) << DgString1(grantorName.c_str())
             << DgString2(granteeName.c_str());
  ...
  if (updateDependentObjects(objectUsage, PrivCommand::REVOKE_OBJECT_RESTRICT) == STATUS_ERROR) return STATUS_ERROR;

  // Go rebuild the privilege tree to see if it is broken
  // If it is broken, return an error
  if (checkRevokeRestrict(row, objectRowList_)) return STATUS_ERROR;
  ...
  if (privsToRevoke.getTablePrivs().isNull()) {
    ...
    if (rowCount == 0) {
      std::string msg("Failed to update privilege bitmaps for revoke command on .");
      msg += row.objectName_;
      PRIVMGR_INTERNAL_ERROR(msg.c_str());
      return STATUS_ERROR;
    }
...
// ----------------------------------------------------------------------------
bool PrivMgrPrivileges::checkRevokeRestrict(PrivMgrMDRow &rowIn, std::vector<PrivMgrMDRow *> &rowList) {
  ...
  for (int32_t i = 0; i < rowList.size(); i++) {
    ...
    if (updatedRow.granteeID_ == currentRow.granteeID_ && updatedRow.grantorID_ == currentRow.grantorID_) {
    ...
    if (privEntry.grantorID_ == grantor) {
      ...
      if (current.getPriv(pType)) {
        ...
        if (current.getWgo(pType)) {
          ...
          if (visited.getWgo(pType)) {  // Already processed this subtree.
          } else {
                                   ...
                                   const std::vector<PrivMgrMDRow *> &privsList)  // in
{
  ...
  for (size_t i = 0; i < privsList.size(); i++) {
    ...
    if (current.getPriv(pType)) {
      ...
      if (current.getWgo(pType)) visited.setWgo(pType, true);
    }
...
// The diags area is set up with where the tree was broken
// ----------------------------------------------------------------------------
bool PrivMgrPrivileges::checkColumnRevokeRestrict(int32_t granteeID, const NAList<PrivMgrCoreDesc> &colPrivsToRevoke,
                                                  std::vector<PrivMgrMDRow *> &rowList) {
  std::string traceMsg;
  ...
  for (int32_t i = 0; i < rowList.size(); i++) {
    ...
    if (currentRow.grantorID_ == grantorID_ && currentRow.granteeID_ == granteeID) {
  ...
  for (size_t i = 0; i < rowList.size(); i++) {
    ...
    for (std::set<int32_t>::iterator it = listOfColumnOrdinals.begin(); it != listOfColumnOrdinals.end(); ++it) {
...
// -----------------------------------------------------------------------------
void PrivMgrPrivileges::scanColumnBranch(const PrivType pType, const int32_t &grantor,
      ...
      if (it != listOfColumnOrdinals.end()) {
        if (current.getPrivBitmap().test(pType)) {
          ...
          if (current.getWgoBitmap().test(pType)) {
            // This grantee has wgo.
            if (currentRow.visited_.getWgoBitmap().test(pType)) {  // Already processed this subtree.
            } else {
                ...
                if (isRoleID(thisGrantee)) {
                  ...
                  for (size_t j = 0; j < userIDs.size(); j++) {
                    ...
                    scanColumnBranch(pType,  // Scan for this grantee as grantor.
                                     granteeAsGrantor, listOfColumnOrdinals, rowList);
                  }
                }
                granteeAsGrantor = thisGrantee;
                scanColumnBranch(pType,  // Scan for this grantee as grantor.
...
void PrivMgrPrivileges::scanColumnPublic(const PrivType pType,  // in
                                         const std::set<int32_t> &listOfColumnOrdinals,
  ...
  for (size_t i = 0; i < rowList.size(); i++) {
    ...
    if (it != listOfColumnOrdinals.end()) {
      if (current.getPrivBitmap().test(pType)) {
        ...
        // This grantee has wgo.
        if (currentRow.visited_.getWgoBitmap().test(pType)) currentRow.visited_.setWgo(pType, true);
      }
    }
  }  // end scan privsList over all Grantees/Grantors
}  // end scanPublic
...
// ****************************************************************************
PrivStatus PrivMgrPrivileges::sendSecurityKeysToRMS(const int32_t granteeID, const PrivMgrDesc &revokedPrivs) {
  NAList<int32_t> roleGrantees(STMTHEAP);

  // If the privilege is revoked from a role, then need to generate QI keys for
  // the users and groups associated with the role.
  if (PrivMgr::isRoleID(granteeID)) {
    // Get users and groups granted to role (granteeID)
    std::vector<int32_t> roleID;
    roleID.push_back(granteeID);
    std::vector<int32_t> granteeIDs;
    if (getGranteeIDsForRoleIDs(roleID, granteeIDs, false) == STATUS_ERROR) return STATUS_ERROR;

    for (size_t g = 0; g < granteeIDs.size(); g++) roleGrantees.insert(granteeIDs[g]);
  } else
  ...
  if (!buildSecurityKeys(roleGrantees, granteeID, objectUID_, false, /* isSchema */
                         false,                                      /* isColumn */
                         revokedPrivs.getTablePrivs(), keyList)) {
...
// *
// *****************************************************************************
PrivStatus PrivMgrPrivileges::populateObjectPriv(const std::string &objectsLocation, const std::string &authsLocation) {
...
// *
// *****************************************************************************
PrivStatus PrivMgrPrivileges::getPrivBitmaps(const std::string &whereClause, const std::string &orderByClause,
...
// *
// *****************************************************************************
PrivStatus PrivMgrPrivileges::getPrivTextForObject(const PrivMgrObjectInfo &objectInfo, std::string &privilegeText) {
  PrivStatus retcode = STATUS_GOOD;
  ...
  retcode = buildPrivText(columnRowList_, objectInfo, PrivLevel::COLUMN, pDiags_, privilegeText);
...
// *
// *****************************************************************************
PrivStatus PrivMgrPrivileges::getPrivsOnObjectForUser(const int64_t objectUID, ComObjectType objectType,
                                                      const int32_t userID, PrivMgrDesc &privsOfTheUser) {
  PrivStatus retcode = STATUS_GOOD;
  ...
  std::vector<int32_t> roleIDs;
  if (getRoleIDsForUserID(userID, roleIDs) == STATUS_ERROR) return STATUS_ERROR;

  // generate the list of privileges for the user
  privsOfTheUser.setGrantee(userID);
  bool hasManagePrivPriv = false;

  retcode = getUserPrivs(objectType, userID, roleIDs, privsOfTheUser, hasManagePrivPriv);
...
PrivStatus PrivMgrPrivileges::getUserPrivs(ComObjectType objectType, const int32_t granteeID,
  ...
  retcode = getPrivsFromAllGrantors(objectUID_, objectType, granteeID, roleIDs, temp, hasManagePrivPriv);
  if (retcode != STATUS_GOOD) return retcode;
...
// *
// *****************************************************************************
PrivStatus PrivMgrPrivileges::getPrivsFromAllGrantors(const int64_t objectUID, ComObjectType objectType,
                                                      const int32_t granteeID, const std::vector<int32_t> &roleIDs,
                                                      PrivMgrDesc &summarizedPrivs, bool &hasManagePrivPriv) {
  PrivStatus retcode = STATUS_GOOD;
  ...
  componentPrivileges.getSQLCompPrivs(granteeID, roleIDs, systemPrivs, hasManagePrivPriv, hasSelectMetadata,
                                      hasAnyManagePriv);
  ...
  // Accumulate object level privileges
    ...
    for (int32_t i = 0; i < rowList.size(); ++i) {
      ...
      if (ComUser::isPublicUserID(row.granteeID_)) hasPublicGrantee = true;
    ...
    if (hasManagePrivPriv) grantableBitmap = systemPrivs;
...
// ----------------------------------------------------------------------------
PrivStatus PrivMgrPrivileges::getRolesToCheck(const int32_t grantorID, const std::vector<int32_t> &roleIDs,
                                              const ComObjectType objectType, std::string &rolesWithPrivs) {
  int32_t length;
  char roleName[MAX_DBUSERNAME_LEN + 1];
  std::vector<int> emptyRoleIDs;
  bool hasManagePrivPriv = false;

  for (size_t r = 0; r < roleIDs.size(); r++) {
    PrivMgrDesc privsOfTheRole(roleIDs[r], true);
    if (getUserPrivs(objectType, roleIDs[r], emptyRoleIDs, privsOfTheRole, hasManagePrivPriv) != STATUS_GOOD)
      return STATUS_ERROR;
    ...
    if (!privsOfTheRole.isNull()) {
      ...
      if (rolesWithPrivs.size() > 0) rolesWithPrivs += ", ";
...
// *
// *****************************************************************************
PrivStatus PrivMgrPrivileges::getRowsForGrantee(const int64_t objectUID, const int32_t granteeID,
                                                const bool isObjectTable, const std::vector<int> &roleIDs,
                                                std::vector<PrivMgrMDRow *> &rowList) {
  PrivStatus retcode = STATUS_GOOD;
  ...
  for (size_t i = 0; i < privRowList.size(); i++) {
    ...
    if (it != authIDs.end()) rowList.push_back(privRowList[i]);
  }
...
// ****************************************************************************
void PrivMgrPrivileges::summarizeColPrivs(const ObjectReference &objReference, const int32_t granteeID,
                                          const int32_t grantorID, const std::vector<int32_t> &roleIDs,
                                          const std::vector<ObjectUsage *> &listOfAffectedObjects,
                                          std::vector<ColumnReference *> &summarizedColRefs) {
  std::string traceMsg;
  ...
  for (size_t i = 0; i < colRefs->size(); i++) {
    ...
    std::vector<PrivMgrMDRow *> rowList;
    getColRowsForGranteeOrdinal(granteeID, colRef->columnOrdinal, columnRowList_, roleIDs, rowList);
    ...
    if (rowList.size() > 0) {
      for (int32_t j = 0; j < rowList.size(); ++j) {
        ...
        for (size_t k = 0; k < listOfAffectedObjects.size(); k++) {
          ...
          if (currentObj->objectUID == row.objectUID_ && grantorID == row.grantorID_ &&
              currentObj->granteeID == row.granteeID_) {
            ColumnReference *changedRef = currentObj->findColumn(row.columnOrdinal_);
            if (changedRef) updated = changedRef->updatedPrivs;
          }
        }
...
PrivStatus PrivMgrPrivileges::summarizeCurrentAndOriginalPrivs(const int64_t objectUID, const int32_t granteeID,
  ...
  for (int32_t i = 0; i < rowList.size(); ++i) {
    ...
    for (size_t j = 0; j < listOfChangedPrivs.size(); j++) {
      ...
      if (pObjectUsage->objectUID == row.objectUID_ && grantorID == row.grantorID_ &&
...
// *
// *****************************************************************************
PrivStatus PrivMgrPrivileges::updateDependentObjects(const ObjectUsage &objectUsage, const PrivCommand command) {
  std::string traceMsg;
  ...
  for (i; i < listOfObjects.size(); i++) {
    ...
    sprintf(buf, "where grantee_id = %d and grantor_id =  %d and object_uid = %ld", theGrantee, theGrantor, theUID);
    std::string whereClause(buf);
    ...
    if (thePrivs.isNull()) {
      ...
      sprintf(buf, "set privileges_bitmap  = %ld, grantable_bitmap =  %ld", thePrivs.getPrivBitmap().to_ulong(),
              thePrivs.getWgoBitmap().to_ulong());
      std::string setClause(buf);
      int64_t rowCount = 0;
      if (objectPrivsTable.updateWhere(setClause, whereClause, rowCount) == STATUS_ERROR) {
        deleteListOfAffectedObjects(listOfObjects);
        return privStatus;
      }

      if (rowCount == 0) {
        std::string msg("Failed to update privilege bitmaps on dependent view object during a ");
        if (PrivMgr::isGrantCommand(command))
          msg += "grant command.";
        else
          msg += "revoke command.";
        PRIVMGR_INTERNAL_ERROR(msg.c_str());
...
// *
// *****************************************************************************
bool PrivMgrPrivileges::isAuthIDGrantedPrivs(const int32_t authID, std::vector<int64_t> &objectUIDs) {
  ...
  std::vector<PrivMgrMDRow *> rowList;
  ObjectPrivsMDTable objTable(objectTableName_, pDiags_);

  PrivStatus privStatus = objTable.selectWhere(whereClause, orderByClause, rowList);
  if (privStatus == STATUS_ERROR) {
    deleteRowList(rowList);
    return true;
  }
  ...
  ColumnPrivsMDTable colTable(columnTableName_, pDiags_);

  privStatus = colTable.selectWhere(whereClause, orderByClause, rowList);
  if (privStatus == STATUS_ERROR) {
    deleteRowList(rowList);
    return true;
  }
...
// *
// *****************************************************************************
PrivStatus PrivMgrPrivileges::convertPrivsToDesc(const ComObjectType objectType, const bool isAllSpecified,
                                                 const bool isWgoSpecified, const bool isGofSpecified,
                                                 const std::vector<PrivType> privsList,
  ...
  switch (objectType) {
    ...
    default: {
      ...
      *pDiags_ << DgSqlCode(-4219) << DgString1(objectTypeLit);
      return STATUS_ERROR;
    }
  }
  ...
  if (isAllSpecified) {
    ...
    privsToProcess.setAllObjectPrivileges(objectType, !isGofSpecified, isWgoSpecified, true /*all_dml*/,
                                          true /*all_ddl*/);
    return STATUS_GOOD;
  }
  ...
  for (int32_t i = 0; i < privsList.size(); ++i) {
    switch (privsList[i]) {
      ...
      case DROP_PRIV:
        ...
        break;
      case ALL_DDL:
        tableCorePrivs.setAllObjectPrivileges(objectType, !isGofSpecified, isWgoSpecified, false /*all_dml*/,
                                              true /*all_ddl*/);
        break;

      case ALL_DML:
        tableCorePrivs.setAllObjectPrivileges(objectType, !isGofSpecified, isWgoSpecified, true /*all_dml*/,
                                              false /*all_ddl*/);
        break;
      default: {
  ...
  for (size_t i = 0; i < colPrivsList.size(); i++) {
    ...
    PrivMgrCoreDesc *existingEntry = findColumnEntry(columnCorePrivs, colPrivSpec.columnOrdinal);
...
{
  while (!rowList.empty()) delete rowList.back(), rowList.pop_back();
}
...
// *                                                                           *
// *****************************************************************************
static PrivMgrCoreDesc *findColumnEntry(NAList<PrivMgrCoreDesc> &colPrivEntries, const int32_t columnOrdinal)

{
  for (size_t i = 0; i < colPrivEntries.entries(); i++)
    if (colPrivEntries[i].getColumnOrdinal() == columnOrdinal) return &colPrivEntries[i];
...
// *****************************************************************************
static void getColRowsForGranteeGrantor(const std::vector<PrivMgrMDRow *> &columnRowList, const int32_t granteeID,
                                        const int32_t grantorID, NAList<PrivMgrCoreDesc> &grantedColPrivs)
...

{
  switch (objectType) {
    case COM_BASE_TABLE_OBJECT:
    case COM_VIEW_OBJECT:
      if (privBitmap.test(DELETE_PRIV) && privBitmap.test(INSERT_PRIV) && privBitmap.test(REFERENCES_PRIV) &&
          privBitmap.test(SELECT_PRIV) && privBitmap.test(UPDATE_PRIV))
        return true;
      break;
    case COM_LIBRARY_OBJECT:
      if (privBitmap.test(UPDATE_PRIV) && privBitmap.test(USAGE_PRIV)) return true;
      break;
    case COM_STORED_PROCEDURE_OBJECT:
    case COM_USER_DEFINED_ROUTINE_OBJECT:
      if (privBitmap.test(EXECUTE_PRIV)) return true;
      break;
    case COM_SEQUENCE_GENERATOR_OBJECT:
      if (privBitmap.test(USAGE_PRIV)) return true;
      break;
    default:
      return false;
  }
...
// *****************************************************************************
void PrivMgrPrivileges::reportPrivWarnings(const PrivMgrDesc &origPrivs, const PrivMgrDesc &actualPrivs,
  ...
  if (!objPrivsNotApplied.isNull()) {
    for (size_t i = FIRST_DML_PRIV; i <= LAST_DML_PRIV; i++) {
      ...
      if (objPrivsNotApplied.getPriv(privType)) {
        *pDiags_ << DgSqlCode(warningCode) << DgString0(PrivMgrUserPrivs::convertPrivTypeToLiteral(privType).c_str());
      }
    }
  }
      ...
      for (size_t j = FIRST_DML_PRIV; j <= LAST_DML_PRIV; j++) {
        ...
        if (colPrivsNotApplied.getPriv(privType)) {
          ...
          snprintf(privStr, sizeof(privStr), "%s (column number %d) ",
                   PrivMgrUserPrivs::convertPrivTypeToLiteral(privType).c_str(), colPrivsNotApplied.getColumnOrdinal());
          *pDiags_ << DgSqlCode(warningCode) << DgString0(privStr);
        }
      }
    }
  }
...
// ----------------------------------------------------------------------------
// method: updateObjectPrivRows
//
// Updates rows in objects table based on the passed in set and where clause
// ----------------------------------------------------------------------------
PrivStatus PrivMgrPrivileges::updateObjectPrivRows(const std::string &setClause, const std::string &whereClause,
                                                   int64_t &rowCount) {
  ObjectPrivsMDTable objectPrivsTable(objectTableName_, pDiags_);
  return (objectPrivsTable.updateWhere(setClause, whereClause, rowCount));
}
...
void ObjectPrivsMDRow::describeRow(std::string &rowDetails) {
  ...
  rowDetails += ", privileges are ";
  rowDetails += privsBitmap_.to_string();
  rowDetails += ", grantable privileges are ";
  rowDetails += grantableBitmap_.to_string();
}
...
// *****************************************************************************
PrivStatus ObjectPrivsMDTable::selectWhereUnique(const std::string &whereClause, PrivMgrMDRow &rowOut) {
  ObjectPrivsMDRow &row = static_cast<ObjectPrivsMDRow &>(rowOut);
...
// *           *: Select failed. A CLI error is put into the diags area.
// *****************************************************************************
PrivStatus ObjectPrivsMDTable::selectWhere(const std::string &whereClause, const std::string &orderByClause,
                                           std::vector<PrivMgrMDRow *> &rowList) {
...
PrivStatus ObjectPrivsMDTable::insert(const PrivMgrMDRow &rowIn) {
  char insertStmt[MAX_DBUSERNAME_LEN * 2 + ComMAX_ANSI_NAME_EXTERNAL_LEN + 200];
  ...
  sprintf(insertStmt, "insert into %s values (%ld, '%s', '%s', %d, '%s', '%s', %d, '%s', '%s', %ld, %ld)",
          tableName_.c_str(), row.objectUID_, row.objectName_.c_str(), objectTypeLit, row.granteeID_,
          row.granteeName_.c_str(), row.granteeType_.c_str(), row.grantorID_, row.grantorName_.c_str(),
          row.grantorType_.c_str(), privilegesBitmapLong, grantableBitmapLong);
...

{
  ...
  sprintf(whereClause, " WHERE object_uid = %ld AND grantor_id = %d AND grantee_id = %d ", row.objectUID_,
          row.grantorID_, row.granteeID_);
...
// * STATUS_NOTFOUND: No rows were deleted
// *               *: Insert failed. A CLI error is put into the diags area.
// *****************************************************************************
PrivStatus ObjectPrivsMDTable::deleteWhere(const std::string &whereClause) {
  ...
  int64_t rowCount = 0;
  int32_t cliRC = cliInterface.executeImmediate(deleteStmt.c_str(), NULL, NULL, TRUE, &rowCount);
  if (cliRC < 0) {
    cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
    return STATUS_ERROR;
  }
...

{
  ...
  sprintf(setClause,
          " SET grantor_id = %d, grantor_name = '%s', "
...
// ----------------------------------------------------------------------------
PrivStatus ObjectPrivsMDTable::updateWhere(const std::string &setClause, const std::string &whereClause,
                                           int64_t &rowCount) {
...
// ----------------------------------------------------------------------------
PrivStatus ObjectPrivsMDTable::insertSelect(const std::string &objectsLocation, const std::string &authsLocation) {
  ...
  privDesc.setAllTableGrantPrivileges(true, true, true, true);
  int64_t tableBits = privDesc.getTablePrivs().getPrivBitmap().to_ulong();

  privDesc.setAllLibraryGrantPrivileges(true, true, true, true);
  int64_t libraryBits = privDesc.getTablePrivs().getPrivBitmap().to_ulong();

  privDesc.setAllUdrGrantPrivileges(true, true, true, true);
  int64_t udrBits = privDesc.getTablePrivs().getPrivBitmap().to_ulong();

  privDesc.setAllSequenceGrantPrivileges(true, true, true, true);
  ...
  sprintf(buf,
          "case when object_type = 'BT' then %ld "
          "     when object_type = 'VI' then 1 "
  ...
  sprintf(buf,
          "insert into %s select distinct object_uid, "
          ...
          tableName_.c_str(), authsLocation.c_str(), USER_GRANTEE_LIT, SYSTEM_USER, SYSTEM_AUTH_NAME,
          SYSTEM_GRANTOR_LIT, privilegesClause.c_str(), grantableClause.c_str(), objectsLocation.c_str());
...
{
  // tablename = 768 + grantee/or name 128 + stuff
  ...
  sprintf(insertStmt, "INSERT INTO %s VALUES (%ld, '%s', %d, '%s', %d, '%s', %d, %ld, %ld)", tableName_.c_str(),
          row.objectUID_, row.objectName_.c_str(), row.granteeID_, row.granteeName_.c_str(), row.grantorID_,
          row.grantorName_.c_str(), row.columnOrdinal_, privilegesBitmapLong, grantableBitmapLong);
...
// *                                                                           *
// *****************************************************************************
PrivStatus ColumnPrivsMDTable::selectWhere(const std::string &whereClause, const std::string &orderByClause,
                                           std::vector<PrivMgrMDRow *> &rowList) {
  std::string selectStmt(
      "SELECT object_uid,object_name,"
...
// *                                                                           *
// *****************************************************************************
PrivStatus ColumnPrivsMDTable::selectWhereUnique(const std::string &whereClause, PrivMgrMDRow &row)
...
// *                                                                           *
// *****************************************************************************
void ColumnPrivsMDTable::setRow(OutputInfo *pCliRow, ColumnPrivsMDRow &row)
...
// *****************************************************************************
// *                                                                           *
// * Function: ColumnPrivsMDTable::updateColumnRow                             *
// *                                                                           *
...
// *                                                                           *
// *****************************************************************************
PrivStatus ColumnPrivsMDTable::updateColumnRow(const ColumnPrivsMDRow &row, const std::string whereBase,
                                               int64_t &rowCount) {
  char buf[whereBase.size() + 200];
  int64_t privilegesBitmapLong = row.privsBitmap_.to_ulong();
  int64_t grantableBitmapLong = row.grantableBitmap_.to_ulong();

  sprintf(buf, " SET privileges_bitmap = %ld, grantable_bitmap = %ld  ", privilegesBitmapLong, grantableBitmapLong);
  std::string setClause(buf);

  sprintf(buf, " %s %d", whereBase.c_str(), row.columnOrdinal_);
  std::string whereClause(buf);
