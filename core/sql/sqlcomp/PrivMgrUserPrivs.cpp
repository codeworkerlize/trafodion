  ...
  NASet<ComSecurityKey> *secKeySet)
{
  ...
  if (ComUser::getCurrentUserRoles(roleIDs, grantees, NULL/*don't update diags*/) != 0)
    return false;
  ...
  for (int i = 0; i < descList.entries(); i++)
  {
    ...
    if (privs->isSchemaObject())
    {
      if (!buildSecurityKeys(roleGrantees, grantee, objectUID, 
                             privs->isSchemaObject(), false, 
                             privs->getSchemaPrivs(), *secKeySet))
        return false;
    }
    else
    {
      // add schema security keys
      if (!buildSecurityKeys(roleGrantees, grantee, privs->getSchemaUID(), 
                             true, false, privs->getSchemaPrivs(), *secKeySet))
        return false;
      
      // add object security keys
      if (!buildSecurityKeys(roleGrantees, grantee, objectUID, 
                             privs->isSchemaObject(), false, privs->getTablePrivs(), *secKeySet))
        return false;
      ...
      for (int k = 0; k < colPrivs.entries(); k++)
      {
        ...
        if (!buildSecurityKeys(roleGrantees, grantee, objectUID, 
                             privs->isSchemaObject(), true, colDesc, *secKeySet))
          return false;
      }
    }
  }
...
bool PrivMgrUserPrivs::isStale(struct timeval * timeBase, int32_t incrementInSeconds)
{
  ...
  if ((getPrivilegesTime_.tv_sec == 0) && (getPrivilegesTime_.tv_usec == 0))
    return TRUE;
