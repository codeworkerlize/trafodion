...
static short injectError(
  const char * errEnvvar,
  const char * details)
{
  char *injectError = getenv(errEnvvar);
  if (injectError)
    {
      if (strcmp(injectError, "assert") == 0)
        assert (0);
  ...
  ExeCliInterface *cliInterface)
{
  ...
  if (updatePrivMgrMetadata(populateObjectPrivs) == STATUS_ERROR)
    return STATUS_ERROR;
  ...
  if (initializeMD)
  {
    ...
    if (roles.grantRoleToCreator(roleID, DB__ADMINROLE, ROOT_USER_ID, DB__ROOT,
                                 ADMIN_USER_ID, DB__ADMIN) == STATUS_ERROR)
        return STATUS_ERROR;
    ...
    if (roles.grantRoleToCreator(roleID, DB__LIBMGRROLE, ROOT_USER_ID, DB__ROOT,
                                 ADMIN_USER_ID, DB__ADMIN) == STATUS_ERROR)
        return STATUS_ERROR;
  }
   ...
   const bool shouldPopulateObjectPrivs)
{
   ...
   int64_t rowCount = 0;
   if (updatePrivBitmap(DROP_PRIV, rowCount) < 0)
     return STATUS_ERROR;
   traceMsg = "updated privilege bitmaps: DROP_PRIV ";
   traceMsg += UIDToString(rowCount);
   if (updatePrivBitmap(ALTER_PRIV, rowCount) < 0)
     return STATUS_ERROR;
   traceMsg = " ALTER_PRIV ";
   traceMsg += UIDToString(rowCount);
   log(__FILE__, traceMsg, -1);
...
// ----------------------------------------------------------------------------
short PrivMgrMDAdmin::alterRenamePMTbl(
  ExeCliInterface * cliInterface,
  NABoolean origToOld)
{
  Lng32 cliRC = 0;
  ...
  for (Int32 i = 0; i < sizeof(allPrivMgrUpgradeInfo)/sizeof(MDUpgradeInfo); i++)
  {
    ...
    str_sprintf(queryBuf, "alter table %s.\"%s\".%s rename to %s skip view check; ",
                TRAFODION_SYSCAT_LIT, SEABASE_PRIVMGR_SCHEMA, 
                srcObject, tgtObject);
    ...
    if (tables_upgraded > 1)
    {
       if (origToOld)
         ...
         if (injectError("UPGRADE_PRIVMGR_INJECT_12", "undo alterRenamePMTbl."))
           return -1;
    }
  }
  return 0;
...
{
  Lng32 cliRC = 0;
  Int32 bufSize = (strlen(TRAFODION_SYSCAT_LIT) * 2) + strlen(SEABASE_MD_SCHEMA) +
                  strlen(SEABASE_OBJECTS) + strlen(SEABASE_PRIVMGR_SCHEMA) + (3*60);
  char queryBuf[bufSize];
  ...
  if (rowCount > 0)
  {
    PRIVMGR_INTERNAL_ERROR("PrivMgrMDAdmin::checkForOldPMTbl failed, tables with the _OLD_MD suffix exist in the _PRIVMGR_MD_ SCHEMA");
    return 1;
  }
  return 0;
}
...
short PrivMgrMDAdmin::copyPMOldToNew(ExeCliInterface * cliInterface)
{
  ...
  for (Int32 i = 0; i < sizeof(allPrivMgrUpgradeInfo)/sizeof(MDUpgradeInfo); i++)
  {
    ...
    str_sprintf(queryBuf, "upsert into %s.\"%s\".%s %s%s%s select "
                "%s from %s.\"%s\".%s SRC %s",
                TRAFODION_SYSCAT_LIT, SEABASE_PRIVMGR_SCHEMA, pmti.newName, 
                (pmti.insertedCols ? "(" : ""),
                (pmti.insertedCols ? pmti.insertedCols : ""),
                (pmti.insertedCols ? ")" : ""),
                (pmti.selectedCols ? pmti.selectedCols : "*"),
                TRAFODION_SYSCAT_LIT, SEABASE_PRIVMGR_SCHEMA, pmti.oldName,
                (pmti.wherePred ? pmti.wherePred : ""));
...
short PrivMgrMDAdmin::createPMTbl(ExeCliInterface * cliInterface)
{
  ...
  for (Int32 i = 0; i < sizeof(allPrivMgrUpgradeInfo)/sizeof(MDUpgradeInfo); i++)
  {
    ...
    if (!pmti.upgradeNeeded && !pmti.addedTable)
      continue;
...
// ----------------------------------------------------------------------------  
short PrivMgrMDAdmin::dropAndLogPMViews(
  ExeCliInterface * cliInterface,
  NABoolean & someViewSaved /* out */)
{
  ...
  for (Int32 i = 0; i < sizeof(allPrivMgrUpgradeInfo)/sizeof(MDUpgradeInfo); i++)
  {
    ...
    if ((! pmti.oldName) || (NOT pmti.upgradeNeeded))
      continue;

    Int64 tableUID = cmpDDL.getObjectUID(cliInterface,
    ...
    if (tableUID > 0)  // if we got it
    {
      ...
      if (cmpDDL.saveAndDropUsingViews(tableUID, cliInterface,
                                      viewNameList /* out */,
                                      viewDefnList /* out */) == -1)
...
// ----------------------------------------------------------------------------   
short PrivMgrMDAdmin::dropPMTbl(
  ExeCliInterface * cliInterface,
  NABoolean oldPMTbl,
  NABoolean inRecovery)
{
  ...
  for (Int32 i = 0; i < sizeof(allPrivMgrUpgradeInfo)/sizeof(MDUpgradeInfo); i++)
  {
    ...
    if (oldPMTbl)
    {
      ...
      if (pmti.addedTable || !pmti.upgradeNeeded)
        continue;
    }
    else
    {
      ...
      if (inRecovery && !pmti.upgradeNeeded)
        continue;
    }
    ...
    if (snprintf(queryBuf, bufSize,"drop table if exists %s.\"%s\".%s cascade; ",
                 TRAFODION_SYSCAT_LIT, SEABASE_PRIVMGR_SCHEMA, objectName) >= bufSize)
    {
...
//   cliInterface - class that manages CLI requests
// ----------------------------------------------------------------------------
short PrivMgrMDAdmin::grantPMTbl(
  ExeCliInterface * cliInterface,
  NABoolean inRecovery)
{
  Lng32 cliRC = 0;
  ...
  for (Int32 i = 0; i < sizeof(allPrivMgrUpgradeInfo)/sizeof(MDUpgradeInfo); i++)
  {
    ...
    Int64 objectUID = cmpDDL.getObjectUID(cliInterface,
                                          TRAFODION_SYSCAT_LIT, SEABASE_PRIVMGR_SCHEMA, 
                                          pmti.newName, COM_BASE_TABLE_OBJECT_LIT, NULL, NULL, 
                                          NULL, FALSE, FALSE /* ignore error */);
    if (objectUID <= 0)
    {
    ...
    PrivMgrPrivileges privileges(objectUID,objectName,SYSTEM_USER,
                                 std::string(privMgrMDLoc.data()),CmpCommon::diags());
    if (privileges.grantToOwners(COM_BASE_TABLE_OBJECT,
                                 ROOT_USER_ID, DB__ROOT,
                                 ROOT_USER_ID, DB__ROOT,
                                 ROOT_USER_ID, DB__ROOT) == STATUS_ERROR)
    {
      cliRC = -1;
      break;
    }
...
// ----------------------------------------------------------------------------
short PrivMgrMDAdmin::revokePMTbl (
   ExeCliInterface * cliInterface,
   NABoolean oldPMTbl,
   NABoolean inRecovery)
{
...
// ----------------------------------------------------------------------------  
short PrivMgrMDAdmin::upgradePMTbl(
  ExeCliInterface * cliInterface,
  CmpDDLwithStatusInfo *mdui)
{
  ...
  while (1) // exit via return stmt in switch
  {
    switch (mdui->subStep())
    {
      ...
      {
        // create new tables
        if (createPMTbl(cliInterface))
          return ERR_RESTORE_ORIG_TBLS;  // error, bring back the orig and drop old tables
...
//   CmpDDLwithStatusInfo - Upgrade details
// ----------------------------------------------------------------------------
short PrivMgrMDAdmin::upgradePMTblComplete(
  ExeCliInterface * cliInterface,
  CmpDDLwithStatusInfo *mdui)
{
  Lng32 cliRC = 0;
        ...
        case 2:
          {
            ...
            // in the logs.
            mdui->setMsg("One or more views on a PrivMgr Metadata table were dropped. "
...
//   CmpDDLwithStatusInfo - Upgrade details
// ----------------------------------------------------------------------------
short PrivMgrMDAdmin::upgradePMTblUndo(
  ExeCliInterface * cliInterface,
  CmpDDLwithStatusInfo *mdui)
{
  Lng32 cliRC = 0;
