...
short CmpSeabaseDDL::updateColAndKeyInfo
 ...
 NAMemory * heap)
{
  ...
  retcode = CmpSeabaseDDL::getTypeInfo
    (naType, alignedFormat, FALSE,
     colInfoEntry->datatype, colInfoEntry->length, 
     precision, scale, dtStart, dtEnd, upshifted, 
     colInfoEntry->nullable,
  ...
  if (naType->getTypeQualifier() == NA_CHARACTER_TYPE)
    {
      ...
      if (char_t->isVarchar2())
        colInfoEntry->colFlags |= SEABASE_COLUMN_IS_TYPE_VARCHAR2;
    }
     ...
     Int32 numSaltPartns)
{
  ...
  Lng32 totalKeyColCount = keyColCount + nonKeyColCount;
  Lng32 addnlColCount = addnlColRefArray.entries();
  ...
  for ( i = 0; i < indexColRefArray.entries(); i++ )
    {
      ...
      if (naType->getFSDatatype() == REC_BLOB || 
          naType->getFSDatatype() == REC_CLOB ||
          naType->getFSDatatype() == REC_ROW ||
          naType->getFSDatatype() == REC_ARRAY)
	//Cannot allow LOB, ROW or ARRAY in primary or clustering key 
	{
	  ...
	  *CmpCommon::diags() << DgSqlCode(-CAT_LOB_COL_CANNOT_BE_INDEX_OR_KEY)
                              << DgColumnName(nodeKeyCol->getColumnName())
                              << DgString0(type);
          processReturn();
          return -1;
	}
      ...
      if (baseTableKeyArr.getColumn(newColName))
        colNameSuffix += "@";
      retcode = updateColAndKeyInfo
        (tableCol, i, alignedFormat, defaultColFam, 0,
         (nodeKeyCol->getColumnOrdering() == COM_ASCENDING_ORDER ? 0 : 1),
         colNameSuffix.data(),
         &colInfoArray[i], &keyInfoArray[i], keyLength, heap);
  ...
  while (i < totalKeyColCount)
    {
      ...
      retcode = updateColAndKeyInfo
        (keyCol, i, alignedFormat, defaultColFam, 
         (isUnique ? 1 : 0), 
         (keyCol->getClusteringKeyOrdering() == ASCENDING ? 0 : 1),
         NULL,
         &colInfoArray[i], &keyInfoArray[i], 
         keyLength, heap);
  ...
  while (j < addnlColRefArray.entries())
    {
	  ...
	  *CmpCommon::diags() << DgSqlCode(-CAT_LOB_COL_CANNOT_BE_INDEX_OR_KEY)
      ...
      retcode = updateColAndKeyInfo
        (keyCol, i, alignedFormat, defaultColFam, 2/*addnl col*/, 
         0, NULL,
         &colInfoArray[i], &keyInfoArray[i], keyLength, heap);
      selColList.insert(keyCol->getColName());
...
Lng32 CmpSeabaseDDL::createSeabasePartitionIndex(ExeCliInterface * cliInterface,
                                           ...
                                           const NAString &baseIndexName)
{
  ...
  for (int i = 0; i < indexColRefArray.entries(); i++)
  {
    ...
    if (indexColRefArray[i]->getColumnOrdering() == COM_DESCENDING_ORDER)
      col += " DESC";
  }
...

  {
    ...
    snprintf(createindex, sizeof(createindex), "create partition local %s index %s on  %s.%s.%s(%s)",
          indexType.data(),
          partitionIndexName, 
          currCatName.data(),
          currSchName.data(),
				       ...
				       NAString &currCatName, NAString &currSchName)
{
  ...
  {
    NAString newBtname;
    if (isExistsPartitionName(&cliInterface, btCatalogNamePart, btSchemaNamePart, btObjectNamePart, createIndexNode->getPartitionName(), newBtname))
    {
  ...
  retcode = lockObjectDDL(tableName,
			  COM_BASE_TABLE_OBJECT,
			  createIndexNode->isVolatile(),
			  createIndexNode->isExternal());
  if (retcode < 0) {
    processReturn();
    return;
  }
  ...
  if (naTable == NULL || bindWA.errStatus())
    {
      if (! CmpCommon::diags()->contains(-4264))
        CmpCommon::diags()->clear();
  ...
  if (naTable->isPartitionV2Table() && createIndexNode->getPartitionIndexType() == StmtDDLNode::PARTITION_INDEX_GLOBAL)
  {
  ...
  if ((isPartitionV2Table || isPartitionEntityTable) && NOT createIndexNode->isPartition())
  {
    *CmpCommon::diags()
              << DgSqlCode(-1229)
              << DgString0("CREATE INDEX ON A PARTITION TABLE WITHOUT PARTITION INFO");
    processReturn();
    return;
  }
  ...
  if (NOT (isPartitionV2Table || isPartitionEntityTable) && createIndexNode->isPartition())
  {
    *CmpCommon::diags()
              << DgSqlCode(-1229)
              << DgString0("CREATE PARTITION INDEX ON NOT A PARTITION TABLE");
    processReturn();
    return;
  }
  ...
  if ((isPartitionV2Table || isPartitionEntityTable) && createIndexNode->isPartition() && errorToken)
  {
    *CmpCommon::diags()
              << DgSqlCode(-1097)
              << DgString0(createIndexNode->getPartitionName());
    processReturn();
    return;
  }
  ...
  if (!isDDLOperationAuthorized(SQLOperation::ALTER_TABLE,
                                ...
                                btObjUID, COM_BASE_TABLE_OBJECT,
                                naTable->getPrivInfo()) &&
  ...
  if (alignedFormat)
    trafColFam = SEABASE_DEFAULT_COL_FAMILY;
  ...
  for (Int32 i = 0; i < indexList.entries(); i++)
    {
      ...
      if (naf->ngramIndex() == createIndexNode->isNgramSpecified())
        continue;
          ...
          if (indexColumns[j]->getColName() == colName)
            {
              ...
              if (naTable->objectUid().castToInt64() == naf->getIndexUID())
                *CmpCommon::diags() << DgSqlCode(-CAT_NGRAM_INDEX_COL_CONFLICT_WITH_CLUSTERING_KEY)
                                    << DgString0(colName);
              else  // index
  ...
  if (numTrafReplicas > 0)
  {
    ...
    ElemDDLColRef * edcrr = 
      new(STMTHEAP) ElemDDLColRef(replicaColName, COM_ASCENDING_ORDER, STMTHEAP);
      ...
      createIndexNode->getSaltOptions()->getLikeTable())
  {
    ...
    else
    {
       ...
       *CmpCommon::diags() << DgSqlCode(CAT_INVALID_SALT_LIKE_CLAUSE)
                           << DgString0(createIndexNode->isVolatile() ?
                                        btObjectNamePart :
                                        extTableName)
                           << DgString1(createIndexNode->isVolatile() ?
                                        objectNamePart :
                                        extIndexName);
    }
  }
  ...
  if (isPartitionV2Table && createIndexNode->isUniqueSpecified() && (createIndexNode->getPartitionIndexType() != StmtDDLNode::PARTITION_INDEX_GLOBAL))
  {
    ...
    for (int i = 0; i < indexColRefArray.entries(); i++)
    {
      ...
      {
        *CmpCommon::diags() << DgSqlCode(-1009)
                            << DgColumnName(ToAnsiIdentifier(indexColRefArray[i]->getColumnName()));
        return;
      }
  ...
  if ((numSplits > 0)||(numTrafReplicas > 0)) {
    ...
    if (createEncodedKeysBuffer(encodedKeysBuffer/*out*/,
                                ...
                                nafs->numTrafReplicas(), keyColCount, keyLength, TRUE))
      {
        deallocEHI(ehi);
        processReturn();
        return;
  ...
  tableInfo->tableNamespace =
    (NOT btNamespace.isNull() ? btNamespace.data() : NULL);
  if (btRowIdEncryption)
  ...
  if (naTable->isPartitionV2Table() && ((createIndexNode->getPartitionIndexType() != StmtDDLNode::PARTITION_INDEX_LOCAL) &&
		  (createIndexNode->getPartitionIndexType() != StmtDDLNode::PARTITION_INDEX_GLOBAL)))
  {
    // not supported
    *CmpCommon::diags() << DgSqlCode(-3242)
                        << DgString0("Cannot create normal index on a partition table.");
    processReturn();
  ...
  else if (naTable->isPartitionEntityTable() && createIndexNode->getPartitionIndexType() == StmtDDLNode::PARTITION_INDEX_LOCAL)
  { // local partition index
    ...
    removePartNATable = TRUE;
           ...
           createIndexNode->getPartitionIndexType() == StmtDDLNode::PARTITION_INDEX_GLOBAL)
    CmpSeabaseDDL::setMDflags(ii->indexFlags, MD_IS_GLOBAL_PARTITION_FLG), removePartNATable = TRUE;                         // global partiion index
  else if (naTable->isPartitionEntityTable() && createIndexNode->getPartitionIndexType() == StmtDDLNode::PARTITION_INDEX_NORMAL)
  {
  ...
  extNameForHbase = genHBaseObjName
    (catalogNamePart, schemaNamePart, objectNamePart, btNamespace, objDataUID);
  ...
  if (isPartitionV2Table && createIndexNode->getPartitionIndexType() == StmtDDLNode::PARTITION_INDEX_LOCAL)
  {
    ...
    for (int i = 0; i < naTable->getNAPartitionArray().entries(); i++)
    {
      ...
      {
        ret = createSeabasePartitionIndex(
                          &cliInterface,
                          createIndexNode,
                          indexColRefArray,
                          partitionTableName,
  ...
  if (NOT createIndexNode->isNoPopulateOptionSpecified())
    {
      NABoolean useLoad = 
        ((CmpCommon::getDefault(TRAF_LOAD_USE_FOR_INDEXES) == DF_ON) && (numTrafReplicas < 2));
      ...
      if (indexOpt && NOT isPartitionV2Table)
        {
          ...
          NAString extTableNameForHbase = genHBaseObjName	
            (btCatalogNamePart, btSchemaNamePart, btObjectNamePart, 	
             btNamespace, btObjUID);	
      ...
      if (NOT indexOpt && (NOT isPartitionV2Table ||
				  (isPartitionV2Table && createIndexNode->getPartitionIndexType() == StmtDDLNode::PARTITION_INDEX_GLOBAL)))
        {
          if (createIndexNode->isNgramSpecified())
          {
            // populate index
            if (populateNgramIndexFromTable(&cliInterface,
                                              createIndexNode->isUniqueSpecified(),
                                              extIndexName, 
  ...
  if (naTable->incrBackupEnabled())
  {
    if (updateSeabaseXDCDDLTable(&cliInterface, btCatalogNamePart.data(),
                                  btSchemaNamePart.data(), btObjectNamePart.data(),
  ...
  if (naTable->isStoredDesc())
    {
      insertIntoSharedCacheList(btCatalogNamePart, btSchemaNamePart, btObjectNamePart,
					  ...
					  NABoolean useLoad)
{
  ...
  cliRC = cliInterface->holdAndSetCQDs("ALLOW_DML_ON_NONAUDITED_TABLE, ATTEMPT_ESP_PARALLELISM, HIDE_INDEXES", 
  ...
  LOGDEBUG(CAT_SQL_LOCK, "Populate index %s for table %s",
           indexName.data(), tableName.getExternalName().data());
...
short CmpSeabaseDDL::populateNgramIndexFromTable(
            ExeCliInterface * cliInterface,
            NABoolean isUnique,
            const NAString &indexName, 
            const ComObjectName &tableName,
...
int CmpSeabaseDDL::populateSeabasePartIndex(
            ...
            ExeCliInterface * cliInterface)
{
  ...
  if (ispopulateForAll)
    populateQuery.format("POPULATE ALL %s INDEXES ON %s %s;",
      isUnique ? "UNIQUE" : "",
      tableName,
      isPurgedataSpecified ? "PURGEDATA" : ""
    );
  else
    populateQuery.format("POPULATE INDEX %s ON %s %s;",
      indexName,
      tableName,
      isPurgedataSpecified ? "PURGEDATA" : ""
    );
					 ...
					 NAString &currCatName, NAString &currSchName)
{
  ...
  if (populateIndexNode->populateIndexOnPartition() && !isPartitionTable)
  {
    *CmpCommon::diags()
              << DgSqlCode(-3242)
              << DgString0("POPULATE PARTITION INDEX ON A REGULAR TABLE");
    goto label_return;
  }
  ...
  if (populateIndexNode->populateIndexOnPartition())
  {
    ...
    if (!isMatched)
    {
      ...
      *CmpCommon::diags() << DgSqlCode(-1097)
                          << DgString0(popuPartitionName.data());
      goto label_return;
    }
    retcode = populateSeabasePartIndex(
      FALSE,
      populateIndexNode->purgedataSpecified(), 
      FALSE,
      partEntityName.data(),
      objectNamePart.data(),
      &cliInterface);
  ...
  {
    if (beginXnIfNotInProgress(&cliInterface, xnWasStartedHere,
                                   FALSE /*clearDDLObjList*/, FALSE /*clearObjectLocks*/))
    {
      processReturn();
    ...
    for (Int32 i = 0; i < naTable->FisrtLevelPartitionCount(); i++)
    {
      ...
      retcode = populateSeabasePartIndex(
            ispopulateForAll,                                     // is populate all index
	...
	qn.getQualifiedNameAsAnsiString(TRUE);
	  ...
	  (extIndexName == nafIndexName))
      {
        ...
        if (isValid)
        {
          ...
          else
          {
            ...
            *CmpCommon::diags() << DgSqlCode(-20205)
                                << DgString0(extIndexName);
            processReturn();
            goto label_return;
          }
      ...
      Int64 indexUID = getObjectUID(&cliInterface,
        ...
        if (populateIndexNode->purgedataSpecified()){
          ...
          if (!Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL) &&
              continueObjectEpochCalled == FALSE)
            {
          ...
          retcode = purgedataObjectAfterError(cliInterface,
                                              catalogNamePart, schemaNamePart, objectNamePart, 
  ...
  if ((NOT (populateIndexNode->populateAll() || 
            populateIndexNode->populateAllUnique())) &&
      (NOT foundIndex) &&
  ...
  if (indexList.entries() > 0 || isPartitionTable)
    {
      ...
      Int64 btObjUID = naTable->objectUid().castToInt64();
      if (updateObjectRedefTime(&cliInterface,
                                btCatalogNamePart, btSchemaNamePart, 
                                btObjectNamePart,
                                COM_BASE_TABLE_OBJECT_LIT, -1, btObjUID,
                                naTable->isStoredDesc(), 
                                naTable->storedStatsAvail()))
...
Lng32 CmpSeabaseDDL::dropSeabasePartitionIndex(ExeCliInterface * cliInterface,
                                ...
                                const char * partitionTableName)
{
  ...
  char partitionIndexName[256];
  getPartitionIndexName(partitionIndexName, 256, baseIndexName, partitionTableName);
  str_sprintf(buf, "DROP LOCAL INDEX %s.\"%s\".%s", 
            currCatName.data(), currSchName.data(), partitionIndexName);
				 ...
				 NAString &currCatName, NAString &currSchName)
{
  ...
  if ((retcode < 0) &&
      (CmpCommon::diags()->mainSQLCODE() == -4254))
    {
      CmpCommon::diags()->clear();
      retcode = 1;
    }
  ...
  if (!Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL))
    {
      ...
      Lng32 len;
      char queryStr[1000];
      str_sprintf(queryStr, "SELECT COUNT(*) FROM %s.\"%s\".%s WHERE BITAND(FLAGS, %d) != 0 AND CATALOG_NAME= '%s' AND SCHEMA_NAME='%s' AND OBJECT_NAME='%s' AND OBJECT_TYPE='IX'",
                  TRAFODION_SYSTEM_CATALOG, SEABASE_MD_SCHEMA, SEABASE_OBJECTS,
                  MD_OBJECTS_READ_ONLY_ENABLED,
                  catalogNamePart.data(),schemaNamePart.data(),objectNamePart.data());
      ...
      if (numEntries != 0)
        {//read only index can not be dropped
          *CmpCommon::diags() << DgSqlCode(-8644);
          processReturn();
          return;
        }       
  ...
  if ((!isDDLOperationAuthorized(SQLOperation::DROP_INDEX, 
                                 btObjOwner, btSchemaOwner, 
                                 btUID, COM_BASE_TABLE_OBJECT,
                                 naTable->getPrivInfo())) &&
      (!isDDLOperationAuthorized(SQLOperation::ALTER_TABLE, 
                                 btObjOwner, btSchemaOwner, 
                                 btUID, COM_BASE_TABLE_OBJECT,
                                 naTable->getPrivInfo())))
  {
  ...
  Int64 indexUID = getObjectUID(&cliInterface, catalogNamePart.data(),
                     schemaNamePart.data(), objectNamePart.data(), COM_INDEX_OBJECT_LIT);
  if (indexUID < 0)
    {
      processReturn();
      deallocEHI(ehi);
  ...
  Int64 indexFlags = getIndexFlags(&cliInterface, btUID, indexUID);
  ...
  if (dropSeabaseObject(ehi, idxName,
			currCatName, currSchName, COM_INDEX_OBJECT,
                        dropIndexNode->ddlXns(), dropFromMD, dropFromStorage, 
                        naTable->isMonarch(),
                        tableNamespace))
    {
      processReturn();
  ...
  if (naTable->incrBackupEnabled())
  {
    if (updateSeabaseXDCDDLTable(&cliInterface, btCatName.data(),
  ...
  if (naTable->isStoredDesc())
    {
      insertIntoSharedCacheList(btCatName, btSchName, btObjName,
                                COM_BASE_TABLE_OBJECT, SharedCacheDDLInfo::DDL_UPDATE);
      insertIntoSharedCacheList(catalogNamePart, schemaNamePart, objectNamePart,
				     ...
				     NAString &currCatName, NAString &currSchName)
{
  ...
  const QualifiedName &entityName = 
  ...
  isProcessIndexOnPartition = (isDisable ? 
    alterDisableIndexNode->isProcessIndexOnPartition() :
  ...
  {
    NAString query;
    query.format(" select object_uid from  %s.\"%s\".%s where     "\
                 " object_uid = (                                 "\
    ...
    if (indexes->numEntries() == 0)
    {
      *CmpCommon::diags()
                << DgSqlCode(-3242)
                << DgString0("index is mismatch of table");
      processReturn();
      return;
    }
  }
  ...
  if ((btUID = getObjectInfo(&cliInterface,
                             btCatName, btSchName, btObjName, 
                             COM_BASE_TABLE_OBJECT,
                             btObjOwner, btSchemaOwner, btObjectFlags, btUID, objDataUID)) < 0)
    {
      processReturn();
  ...
  {
    const NAPartitionArray &partArray = naTable->getPartitionArray();
    NAString alterPartitionName = (isDisable ? alterDisableIndexNode->getPartitionName() : alterEnableIndexNode->getPartitionName());
    NABoolean isMatched = FALSE;
    int recode = -1;
    for (int i = 0; i < partArray.entries(); i++)
    {
    ...
    if (!isMatched)
    {
      // The specified partition partName does not exist.
      *CmpCommon::diags() << DgSqlCode(-1097)
                          << DgString0(alterPartitionName.data());
      processReturn();
      return;
    }

    if (alterSeabaseTableDisableOrEnableIndex(btCatName.data(), btSchName.data(), objectNamePart.data(), partArray[recode]->getPartitionEntityName(), isDisable))
    {
      processReturn();
      return;
    }
  ...
  else
  {
    ...
    {
      char buf[4000];
      sprintf (buf, " POPULATE INDEX \"%s\" ON \"%s\".\"%s\".\"%s\";",
                  objectNamePart.data(),
                  btCatName.data(),
                  btSchName.data(),
                  btObjName.data());
    ...
    if (isPartitionTable)
    {
      ...
      for (Int32 i = 0; i < naTable->FisrtLevelPartitionCount(); i++)
      {
        char partitionIndexName[256];
        getPartitionIndexName(partitionIndexName, 256, objectNamePart, partArray[i]->getPartitionEntityName());
        if (alterSeabaseTableDisableOrEnableIndex(btCatName.data(), btSchName.data(), partitionIndexName, partArray[i]->getPartitionEntityName(), isDisable))
        {
          processReturn();
          return;
        }
  ...
  if (naTable->incrBackupEnabled()) 
  {
    retcode = updateSeabaseXDCDDLTable(&cliInterface,
                                       btCatName.data(), btSchName.data(),
                                       btObjName.data(), COM_BASE_TABLE_OBJECT_LIT);
    if (retcode < 0)
    {
      processReturn();
      return;
  ...
  if (naTable->isStoredDesc())
    {
      if (isDisable)
        {
          insertIntoSharedCacheList(catalogNamePart, schemaNamePart, objectNamePart,
                                    COM_INDEX_OBJECT, SharedCacheDDLInfo::DDL_DISABLE);
        }
      else 
        {
          insertIntoSharedCacheList(catalogNamePart, schemaNamePart, objectNamePart,
                                    COM_INDEX_OBJECT, SharedCacheDDLInfo::DDL_ENABLE);
        }
                                             ...
                                             NABoolean allUniquesOnly)
{
  ...
  Lng32 retcode = lockObjectDDL(tableName,
			  COM_BASE_TABLE_OBJECT,
			  false /* isVolatile */,
			  false /* isExternal */);
  if (retcode < 0) {
  ...
  if (isPartitionTable)
  {
    ...
    {
      NAString queryBuf;
      queryBuf.format("ALTER TABLE %s.\"%s\".%s %s ALL %s INDEXES;",
          catalogNamePart.data(),
          schemaNamePart.data(),
          partArray[i]->getPartitionEntityName(),
      ...
      if (naTable->isStoredDesc())
        {
          if (isDisable)
            {
              insertIntoSharedCacheList(catName, schName, idxName,
                                        COM_INDEX_OBJECT, SharedCacheDDLInfo::DDL_DISABLE);
            }
          else
            {
...
short CmpSeabaseDDL::alterSeabaseIndexStoredDesc(ExeCliInterface &cliInterface,
    ...
    for (int i = 0; i < indexes->numEntries(); i++)
    {
      ...
      cliRC =
        updateObjectRedefTime(&cliInterface,
                              ...
                              idx->get(2), //ndxName
                              COM_INDEX_OBJECT_LIT,
                              -1, *(Int64*)idx->get(3), enable, FALSE);
      if (cliRC < 0)
        return -1;
    }
  }
                                       ...
                                       NAString &currCatName, NAString &currSchName)
{
  ...
  if (naTable->incrBackupEnabled())
  {
    if (updateSeabaseXDCDDLTable(&cliInterface, catalogNamePart.data(),
  ...
  if (naTable->isStoredDesc())
    {
      ...
      insertIntoSharedCacheList(catalogNamePart, schemaNamePart, objectNamePart,
                                COM_INDEX_OBJECT, SharedCacheDDLInfo::DDL_UPDATE);

      Int64 transid;
      CliGlobals *cliGlobals = GetCliGlobals();
      if (cliGlobals->currContext()->getTransaction()->getCurrentXnId(&transid) != 0)
        finalizeSharedCache();
    }
...
// -----------------------------------------------------------------------------
Int32 CmpSeabaseDDL::alterSeabaseIndexAttributes(
  ExeCliInterface *cliInterface,
              ...
              fileAttrs.isReadOnlySpecified())
            {
              ...
              if (cliRC < 0)
                ...
                if (alterReadonlyOP != 0)
                  insertIntoSharedCacheList(indexName.getCatalogName(),
