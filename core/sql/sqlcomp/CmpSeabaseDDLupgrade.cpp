...
NABoolean CmpSeabaseMDupgrade::isLibrariesUpgradeNeeded()
{
  ...
  // ignore errors
  cmpDDL.validateVersions(&ActiveSchemaDB()->getDefaults(), ehi,
                          &mdCurrMajorVersion,
                          &mdCurrMinorVersion,
                          &mdCurrUpdateVersion);

  cmpDDL.deallocEHI(ehi);
  if (mdCurrMajorVersion == 2 && mdCurrMinorVersion == 1 &&
      mdCurrUpdateVersion == 1 && METADATA_OLD_MINOR_VERSION == 1 &&
...
NABoolean CmpSeabaseDDL::getOldMDPrivInfo(const PrivMgrTableStruct &mdti,
                                          const char* &oldName,
                                          const QString* &oldDDL, Lng32 &sizeOfoldDDL)
{
  if (! mdti.tableName)
	...
	case GET_SW_VERSION:
	  {
	      ...
	      case 0:
		{
		  ...
		  str_sprintf(msgBuf, "  Software Version: %ld.%ld.%ld. Expected Version: %ld.%ld.%ld.",
                  ...
                  case 1:
                   {
                     ...
                     for (Lng32 i = 0; i < sizeof(allMDupgradeInfo)/sizeof(MDUpgradeInfo); i++)
                       ...
                       if (dropSeabaseObject(ehi, mdti.oldName, catName, schName,
                           ...
                           FALSE, FALSE, TRUE, isMonarch, nameSpace))
                         {
