...
// ----------------------------------------------------------------------------
PrivStatus PrivMgr::getEffectiveGrantor(
  ...
  std::string &effectiveGrantorName)
{
  ...
  if (!isGrantedBySpecified)
  {
    ...
    retcode = ComUser::getAuthNameFromAuthID(owner,authName,
  ...
  if (ComUser::isRootUserID() || currentUser == owner)
    return STATUS_GOOD;
  ...
  // to have authority.
  if (ComUser::currentUserHasRole(effectiveGrantorID))
    return STATUS_GOOD;
...
// *                                                               
// *****************************************************************************
PrivStatus PrivMgr::getRoleIDsForUserID(
   int32_t userID,
...

{
  ...
  {
    const ComponentOpStruct &opDefinition = sqlOpList[i];
    if ((int32_t)operation == opDefinition.operationID)
...
// new a new PrivClass is added. 
   if (privClasses.size() == 1 && privClasses[0] == PrivClass::ALL)
   {
      ...
      if (schemaPrivileges.isAuthIDGrantedPrivs(authID, objectUIDs))
        return true;
...

{
   ...
   if (operation == SQLOperation::MANAGE || 
       operation == SQLOperation::MANAGE_COMPONENTS ||
       operation == SQLOperation::MANAGE_LIBRARY ||
       operation == SQLOperation::MANAGE_LOAD ||
       operation == SQLOperation::MANAGE_PRIVILEGES ||
       operation == SQLOperation::MANAGE_RESOURCE_GROUPS ||
       operation == SQLOperation::MANAGE_ROLES ||
       operation == SQLOperation::MANAGE_STATISTICS ||
       operation == SQLOperation::MANAGE_TENANTS ||
       operation == SQLOperation::MANAGE_USERS ||
       operation == SQLOperation::MANAGE_GROUPS ||
       operation == SQLOperation::MANAGE_SECURE)
      return true;
...
bool PrivMgr::isMetadataObject (
  const std::string objectName, 
  std::string &internalSchName)
{
...
// *                    on the object.                                                            
// *****************************************************************************
PrivStatus PrivMgr::buildPrivText(
   ...
   for (int32_t i = 0; i < rowList.size();++i)
   {
      ...
      else
      {
         ...
         else
         {
            ...
            if (nextRow.getGrantorID() != row.getGrantorID() ||
                nextRow.getGranteeID() != row.getGranteeID())
               mergeStrings = true;
         }
...
// ----------------------------------------------------------------------------
short PrivMgr::updatePrivBitmap(
  ...
  int64_t &rowCount)
{
   ...
   char buf [200];
   snprintf (buf, sizeof(buf), "set privileges_bitmap = privileges_bitmap + %ld,"
                               " grantable_bitmap = grantable_bitmap + %ld ",
             bitmap.to_ulong(), bitmap.to_ulong());
   std::string setClause(buf);

   snprintf(buf, sizeof(buf), "where grantor_id = %d and "
   ...
   if (privs.updateObjectPrivRows(setClause, whereClause, rowCount) == STATUS_ERROR)
      return -1;
