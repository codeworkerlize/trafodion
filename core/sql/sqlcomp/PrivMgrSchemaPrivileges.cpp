...
class SchemaPrivsMDRow : public PrivMgrMDRow {
 public:
  ...
  SchemaPrivsMDRow()
      : PrivMgrMDRow(PRIVMGR_SCHEMA_PRIVILEGES, SCHEMA_PRIVILEGES_ENUM), schemaUID_(0), grantorID_(0), granteeID_(0){};

  SchemaPrivsMDRow(const SchemaPrivsMDRow &other) : PrivMgrMDRow(other) {
...
class SchemaPrivsMDTable : public PrivMgrMDTable {
 public:
  // constructors and destructors
  SchemaPrivsMDTable(const std::string &tableName, ComDiagsArea *pDiags = NULL)
  ...
  PrivStatus updateRow(const SchemaPrivsMDRow &row, int64_t &rowCount);
...
// Default Constructor
// -----------------------------------------------------------------------
PrivMgrSchemaPrivileges::PrivMgrSchemaPrivileges() : PrivMgr(), schemaUID_(0) {
...
// ----------------------------------------------------------------------------
PrivMgrSchemaPrivileges::PrivMgrSchemaPrivileges(const std::string &metadataLocation, ComDiagsArea *pDiags)
    : PrivMgr(metadataLocation, pDiags), schemaUID_(0) {
  schemaTableName_ = metadataLocation + "." + PRIVMGR_SCHEMA_PRIVILEGES;
}

PrivMgrSchemaPrivileges::PrivMgrSchemaPrivileges(const std::string &trafMetadataLocation,
                                                 const std::string &metadataLocation, ComDiagsArea *pDiags)
    : PrivMgr(trafMetadataLocation, metadataLocation, pDiags), schemaUID_(0) {
  schemaTableName_ = metadataLocation + "." + PRIVMGR_SCHEMA_PRIVILEGES;
}
...
// -----------------------------------------------------------------------
PrivMgrSchemaPrivileges::PrivMgrSchemaPrivileges(const int64_t schemaUID, const std::string &schemaName,
                                                 const std::string &metadataLocation, ComDiagsArea *pDiags)
    : PrivMgr(metadataLocation, pDiags), schemaUID_(schemaUID), schemaName_(schemaName) {
  schemaTableName_ = metadataLocation + "." + PRIVMGR_SCHEMA_PRIVILEGES;
}
...
// ----------------------------------------------------------------------------
PrivMgrSchemaPrivileges::PrivMgrSchemaPrivileges(const PrivMgrObjectInfo &objectInfo,
                                                 const std::string &metadataLocation, ComDiagsArea *pDiags)
...
// Copy constructor
// -----------------------------------------------------------------------
PrivMgrSchemaPrivileges::PrivMgrSchemaPrivileges(const PrivMgrSchemaPrivileges &other) : PrivMgr(other) {
  schemaUID_ = other.schemaUID_;
  schemaName_ = other.schemaName_;
...
// Destructor.
// -----------------------------------------------------------------------
PrivMgrSchemaPrivileges::~PrivMgrSchemaPrivileges() { deleteRowList(schemaRowList_); }
...
// *
// *****************************************************************************
PrivStatus PrivMgrSchemaPrivileges::getPrivsOnSchema(const int64_t schemaUID, const int32_t schemaOwner,
  ...
  if (generateSchemaRowList(schemaOwner) == STATUS_ERROR) return STATUS_ERROR;
  ...
  if (getDistinctIDs(schemaRowList_, userIDs, roleIDs) == STATUS_ERROR) return STATUS_ERROR;
  ...
  for (size_t i = 0; i < userIDs.size(); i++) {
    ...
    if (getUserPrivs(schemaType, userID, emptyRoleIDs, privsOfTheUser, hasManagePrivPriv) != STATUS_GOOD)
      return STATUS_ERROR;
  ...
  for (size_t i = 0; i < roleIDs.size(); i++) {
    ...
    // getUserPrivs returns schema privileges summarized across all grantors.
    if (getUserPrivs(schemaType, grantee, emptyRoleIDs, privsOfTheUser, hasManagePrivPriv) != STATUS_GOOD)
      return STATUS_ERROR;
...
// * schema
// *****************************************************************************
PrivStatus PrivMgrSchemaPrivileges::getDistinctIDs(const std::vector<PrivMgrMDRow *> &schemaRowList,
...
// ******************************************************************************
void PrivMgrSchemaPrivileges::getTreeOfGrantors(const int32_t granteeID, std::set<int32_t> &listOfGrantors) {
...
// *
// *****************************************************************************
PrivStatus PrivMgrSchemaPrivileges::grantSchemaPriv(const ComObjectType schemaType, const int32_t grantorID,
  ...
  if (isWGOSpecified) {
    for (int32_t i = 0; i < privsList.size(); ++i) {
      if (privsList[i] >= FIRST_DDL_PRIV && privsList[i] <= LAST_DDL_PRIV || (isAllSpecified)) {
  ...
  if (!convertPrivsToDesc(isAllSpecified, isWGOSpecified, false, privsList, privsToGrant)) return STATUS_ERROR;

  // generate the list of privileges granted to the schema and store in class
  if (generateSchemaRowList(schemaOwnerID) == STATUS_ERROR) return STATUS_ERROR;
  ...
  if (getUserPrivs(schemaType, grantorID, roleIDs, privsOfTheGrantor, hasManagePrivPriv) != STATUS_GOOD)
    return STATUS_ERROR;
  ...
  if (foundRow) {
    int64_t rowCount;
    if (schemaPrivsTable.updateRow(row, rowCount) == STATUS_ERROR) return STATUS_ERROR;
...
PrivStatus PrivMgrSchemaPrivileges::generateSchemaRowList(const int32_t schemaOwnerID) {
  ...
  if (schemaRowList_.size() > 0) return STATUS_GOOD;
...
//   STATUS_ERROR    - the diags area is populated with any errors
// ****************************************************************************
PrivStatus PrivMgrSchemaPrivileges::getSchemaRowList(const int64_t schemaUID, const int32_t schemaOwnerID,
  ...
  if (ComUser::getAuthNameFromAuthID(schemaOwnerID, tempName, sizeof(tempName), length, FALSE, pDiags_) < 0)
    return STATUS_ERROR;
  ...
  SchemaPrivsMDTable schemaPrivsTable(schemaTableName_, pDiags_);
  PrivStatus privStatus = schemaPrivsTable.selectWhere(whereClause, orderByClause, schemaRowList);
  if (privStatus == STATUS_ERROR) return privStatus;
...
// ******************************************************************************
PrivStatus PrivMgrSchemaPrivileges::getGrantedPrivs(const int32_t grantorID, const int32_t granteeID,
                                                    PrivMgrMDRow &rowOut) {
...
// *
// *****************************************************************************
PrivStatus PrivMgrSchemaPrivileges::revokeSchemaPriv(const ComObjectType schemaType, const int32_t grantorID,
                                                     const std::string &grantorName, const int32_t granteeID,
                                                     const std::string &granteeName, const int32_t schemaOwnerID,
                                                     const std::vector<PrivType> &privsList, const bool isAllSpecified,
  ...
  if (!convertPrivsToDesc(isAllSpecified, true, isGOFSpecified, privsList, privsToRevoke)) return STATUS_ERROR;
  ...
  if (checkRevokeRestrict(row, schemaRowList_)) return STATUS_ERROR;
  ...
  if (warnNotAll) reportPrivWarnings(origPrivsToRevoke, updatedPrivs, CAT_NOT_ALL_PRIVILEGES_REVOKED);
...
// ******************************************************************************
bool PrivMgrSchemaPrivileges::checkRevokeRestrict(PrivMgrMDRow &rowIn, std::vector<PrivMgrMDRow *> &rowList) {
...
// ******************************************************************************
void PrivMgrSchemaPrivileges::scanSchemaBranch(const PrivType pType,                          // in
...
void PrivMgrSchemaPrivileges::scanPublic(const PrivType pType,
                                         const std::vector<PrivMgrMDRow *> &privsList)  // in
{
...
// ****************************************************************************
PrivStatus PrivMgrSchemaPrivileges::sendSecurityKeysToRMS(const int32_t granteeID, const PrivMgrDesc &revokedPrivs) {
  ...
  if (!buildSecurityKeys(roleGrantees, granteeID, schemaUID_, true, false, revokedPrivs.getSchemaPrivs(), keyList)) {
  ...
  strncpy(siKeyList[numKeys - 1].operation, COM_QI_SCHEMA_REDEF_LIT, sizeof(siKeyList[numKeys - 1].operation));
...
// *
// *****************************************************************************
PrivStatus PrivMgrSchemaPrivileges::getPrivBitmaps(const std::string &whereClause, const std::string &orderByClause,
...
// *
// *****************************************************************************
PrivStatus PrivMgrSchemaPrivileges::getPrivTextForSchema(const PrivMgrObjectInfo &schemaInfo,
  ...
  if (generateSchemaRowList(ncSchemaInfo.getSchemaOwner()) == STATUS_ERROR) return STATUS_ERROR;

  // No failures possible for schemas, all information in rowList.
  buildPrivText(schemaRowList_, schemaInfo, PrivLevel::SCHEMA, pDiags_, privilegeText);
...
// *
// *****************************************************************************
PrivStatus PrivMgrSchemaPrivileges::getPrivsOnSchemaForUser(const int64_t schemaUID, ComObjectType schemaType,
  ...
  if (generateSchemaRowList(ROOT_USER_ID) == STATUS_ERROR) return STATUS_ERROR;
  ...
  retcode = getUserPrivs(schemaType, userID, roleIDs, privsOfTheUser, hasManagePrivPriv);
...
// *
// *****************************************************************************
PrivStatus PrivMgrSchemaPrivileges::getSchemasForRole(const int32_t roleID, std::vector<int64_t> &schemaList) {
  PrivStatus retcode = STATUS_GOOD;
...
// *
// *****************************************************************************
PrivStatus PrivMgrSchemaPrivileges::getUserPrivs(ComObjectType schemaType, const int32_t granteeID,
                                                 const std::vector<int32_t> &roleIDs, PrivMgrDesc &summarizedPrivs,
                                                 bool &hasManagePrivPriv) {
  PrivStatus retcode = STATUS_GOOD;
  PrivMgrDesc temp(granteeID, true);

  retcode = getPrivsFromAllGrantors(schemaUID_, granteeID, roleIDs, temp, hasManagePrivPriv);
...
// *
// *****************************************************************************
PrivStatus PrivMgrSchemaPrivileges::getPrivsFromAllGrantors(const int64_t schemaUID, const int32_t granteeID,
  ...
  componentPrivileges.getSQLCompPrivs(granteeID, roleIDs, systemPrivs, hasManagePrivPriv, hasSelect, hasAnyManagePriv);

  bool hasAllPrivs = false;
  if (systemPrivs.test(DELETE_PRIV) && systemPrivs.test(INSERT_PRIV) && systemPrivs.test(REFERENCES_PRIV) &&
      systemPrivs.test(SELECT_PRIV) && systemPrivs.test(UPDATE_PRIV) && systemPrivs.test(USAGE_PRIV) &&
      systemPrivs.test(EXECUTE_PRIV) && systemPrivs.test(CREATE_PRIV) && systemPrivs.test(ALTER_PRIV) &&
      systemPrivs.test(DROP_PRIV))
    hasAllPrivs = true;
  ...
  if (hasSelect || hasAnyManagePriv) {
    ...
    if (retcode == STATUS_GOOD) {
      if (isMetadataObject(objectName, schemaName)) {
        if (hasSelect ||
            (schemaName == std::string(SEABASE_PRIVMGR_SCHEMA) || schemaName == std::string(SEABASE_MD_SCHEMA) ||
...
// ----------------------------------------------------------------------------
PrivStatus PrivMgrSchemaPrivileges::getRolesToCheck(const int32_t grantorID, const std::vector<int32_t> &roleIDs,
                                                    const ComObjectType schemaType, std::string &rolesWithPrivs) {
  int32_t length;
  ...
  for (size_t r = 0; r < roleIDs.size(); r++) {
    ...
    if (getUserPrivs(schemaType, roleIDs[r], emptyRoleIDs, privsOfTheRole, hasManagePrivPriv) != STATUS_GOOD)
      return STATUS_ERROR;
...
// *
// *****************************************************************************
PrivStatus PrivMgrSchemaPrivileges::getRowsForGrantee(const int64_t schemaUID, const int32_t granteeID,
  ...
  for (size_t i = 0; i < schemaRowList_.size(); i++) {
    ...
    if (it != authIDs.end()) rowList.push_back(schemaRowList_[i]);
  }
...
// *****************************************************************************
PrivStatus PrivMgrSchemaPrivileges::updateDependentObjects(const ObjectUsage &objectUsage, const PrivCommand command) {
...
// *
// *****************************************************************************
bool PrivMgrSchemaPrivileges::convertPrivsToDesc(const bool isAllSpecified, const bool isWgoSpecified,
...
// *****************************************************************************
void PrivMgrSchemaPrivileges::reportPrivWarnings(const PrivMgrDesc &origPrivs, const PrivMgrDesc &actualPrivs,
...
// *****************************************************************************
PrivStatus SchemaPrivsMDTable::selectWhereUnique(const std::string &whereClause, PrivMgrMDRow &rowOut) {
  SchemaPrivsMDRow &row = static_cast<SchemaPrivsMDRow &>(rowOut);
...
// *           *: Select failed. A CLI error is put into the diags area.
// *****************************************************************************
PrivStatus SchemaPrivsMDTable::selectWhere(const std::string &whereClause, const std::string &orderByClause,
                                           std::vector<PrivMgrMDRow *> &rowList) {
...
PrivStatus SchemaPrivsMDTable::insert(const PrivMgrMDRow &rowIn) {
  ...
  sprintf(insertStmt, "insert into %s values (%ld, '%s', %d, '%s', %d, '%s', %ld, %ld)", tableName_.c_str(),
          row.schemaUID_, row.schemaName_.c_str(), row.granteeID_, row.granteeName_.c_str(), row.grantorID_,
          row.grantorName_.c_str(), privilegesBitmapLong, grantableBitmapLong);
...

{
  ...
  sprintf(whereClause, " WHERE schema_uid = %ld AND grantor_id = %d AND grantee_id = %d ", row.schemaUID_,
...
// *           *: Insert failed. A CLI error is put into the diags area.
// *****************************************************************************
PrivStatus SchemaPrivsMDTable::updateRow(const SchemaPrivsMDRow &row, int64_t &rowCount) {
  char buf[500];
  ...
  sprintf(buf, " WHERE schema_uid = %ld AND grantor_id = %d AND grantee_id = %d ", row.schemaUID_, row.grantorID_,
          row.granteeID_);
  std::string whereClause(buf);
...
// ******************************************************************************
PrivStatus SchemaPrivsMDTable::updateWhere(const std::string &setClause, const std::string &whereClause,
                                           int64_t &rowCount) {
...
// *
// *****************************************************************************
PrivStatus PrivMgrSchemaPrivileges::getPrivRowsForSchema(const int64_t schemaUID,
...
// *
// *****************************************************************************
bool PrivMgrSchemaPrivileges::isAuthIDGrantedPrivs(const int32_t authID, std::vector<int64_t> &objectUIDs) {
  std::string whereClause(" WHERE GRANTEE_ID = ");
  ...
  if (privStatus == STATUS_NOTFOUND || (privStatus == STATUS_GOOD && rowList.size() == 0)) {
    deleteRowList(rowList);
    return false;
  }
