...
ComObjectName objName(catalogNamePart,schemaNamePart,NAString(SEABASE_SCHEMA_OBJECTNAME), 
                      COM_TABLE_NAME,ComAnsiNamePart::INTERNAL_FORMAT);
...
Lng32 retcode = existsInSeabaseMDTable(&cliInterface,catalogNamePart,schemaNamePart, 
...
NAString quotedObjName;
  ...
  Int64 flags = 0;
  if (rowIdEncrypt)
    CmpSeabaseDDL::setMDflags(flags, MD_OBJECTS_ENCRYPT_ROWID_FLG);
  if (dataEncrypt)
    CmpSeabaseDDL::setMDflags(flags, MD_OBJECTS_ENCRYPT_DATA_FLG);
  if (storedDesc)
    CmpSeabaseDDL::setMDflags(flags, MD_OBJECTS_STORED_DESC);
  if (incrBackupEnabled)
    CmpSeabaseDDL::setMDflags(flags, MD_OBJECTS_INCR_BACKUP_ENABLED);
   ...
   if (NOT namespace1.isNull())
     {
       ...
       cliRC = updateTextTable(&cliInterface, schemaObjectUID, 
                               COM_OBJECT_NAMESPACE, 0,
                               namespace1);
       if (cliRC < 0)
         {
           cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
    ...
    if (!ComIsTrafodionReservedSchemaName(schemaNamePart) &&
        schemaNamePart != SEABASE_SYSTEM_SCHEMA &&
        incrBackupEnabled)
    {
    ...
    if ((storedDesc || cqdSet) && !ComIsTrafodionReservedSchemaName(schemaNamePart))
       {
          if (updateObjectRedefTime(&cliInterface,
                                     schemaName.getCatalogNamePartAsAnsiString(),
                                     schemaNamePart,
                                     SEABASE_SCHEMA_OBJECTNAME,
                                     schemaObjectLit, -1, 
                                     schemaUID.get_value(),
                                     TRUE, FALSE)) //genStoredDesc))
             return -1;

        // Add entry to list of DDL operations
        insertIntoSharedCacheList(schemaName.getCatalogNamePartAsAnsiString(), 
                                  schemaNamePart, SEABASE_SCHEMA_OBJECTNAME_EXT,
                                  objType, SharedCacheDDLInfo::DDL_INSERT);
   ...
   
{
   ...
   else
   {
      ...
      if (ComUser::getAuthIDFromAuthName(createSchemaNode->getAuthorizationID().data(),
                                         schemaOwnerID, CmpCommon::diags()) != 0)
         return;

      if (CmpSeabaseDDLauth::isTenantID(schemaOwnerID) ||
          CmpSeabaseDDLauth::isUserGroupID(schemaOwnerID))
      {
   ...
   NAString tenantNS = enabled ?
     NAString(TRAF_NAMESPACE_PREFIX + ComTenant::getCurrentTenantIDAsString()) : "";
   ...
   if (NOT createSchemaNode->useEncryption())
     {
       ...
       if ((NOT enc.isNull()) &&
           (NOT ComEncryption::validateEncryptionOptions(&enc, encryptRowid, encryptData)))
         {
   ...
   if (addSchemaObject(cliInterface,
                       ...
                       encryptRowid,
                       encryptData,
                       createSchemaNode->storedDesc(),
                       createSchemaNode->incrBackupEnabled()))
     return;
   ...
   std::vector<std::string> & outlines)
{
  ...
  PrivMgrUserPrivs *privs = naTable->getPrivInfo();
   ...
   ComUser::getAuthNameFromAuthID(naTable->getSchemaOwner(),
                                  ownerName,
                                  sizeof(ownerName),
                                  length,
  ...
  if (cmpSBD.getSeabaseObjectComment(naTable->objectUid().get_value(), naTable->getObjectType(), objCommentInfo, STMTHEAP))
    {
   ...
   
{
   ...
   str_sprintf(query,"SELECT distinct TRIM(object_name), TRIM(object_type) "
               ...
               getSystemCatalog(),SEABASE_MD_SCHEMA,SEABASE_OBJECTS,
               (char*)catName.data(),(char*)schName.data(), MD_PARTITION_V2);
       ...
       if (objType == COM_BASE_TABLE_OBJECT_LIT) 
	 {
	   ...
	   if (!isHistogramTable(objName) &&
               ...
               !isLOBDependentNameMatch(objName))
	     {
	       ...
	       short retcode = lockObjectDDL(cn.getQualifiedNameObj(),
   ...
   if (!ComIsTrafodionReservedSchemaName(schName) &&
       schName != SEABASE_SYSTEM_SCHEMA)
    {
      ...
      NATable *naTable = bindWA.getNATable(cn);
      if (naTable)
        isIncrBackupEnabled = naTable->incrBackupEnabled();
    }
   ...
   str_sprintf(buf,"DELETE FROM %s.\"%s\".%s "
               ...
               schemaUID, COM_OBJECT_NAMESPACE);
   cliRC = cliInterface.executeImmediate(buf);
   if (cliRC < 0)
   {
     NAString reason;
     reason = "Reason: Delete of namespace for object " + 
       NAString(SEABASE_SCHEMA_OBJECTNAME) + " failed.";
      *CmpCommon::diags() << DgSqlCode(-CAT_UNABLE_TO_DROP_SCHEMA)
                          << DgSchemaName(catName + "." + schName)
                          << DgString0(reason);
   ...
   if (msg_license_multitenancy_enabled())
   {
      ...
      if (tenantSchemaInfo)
      {
        if (tenantSchemaInfo->removeSchemaUsage() == -1)
          goto label_error;
      }
   ...
   if (!ComIsTrafodionReservedSchemaName(schName) &&
       ...
       isIncrBackupEnabled)
    {
      cliRC = updateSeabaseXDCDDLTable(&cliInterface, catName.data(),
                                       schName.data(), SEABASE_SCHEMA_OBJECTNAME,
  ...
  insertIntoSharedCacheList(schemaName.getCatalogNamePartAsAnsiString(), 
                            schemaName.getSchemaNamePartAsAnsiString(),
                            SEABASE_SCHEMA_OBJECTNAME_EXT,
                            objectType, SharedCacheDDLInfo::DDL_DELETE);
   ...
   
{
   ...
   cliRC = alterSchemaTableDesc (&cliInterface, sdo, schemaUID, objectType, 
                                 catName, schName, alterSchemaNode->ddlXns());
   if (cliRC < 0)
     {
   ...
   for (int idx = 0; idx < objectsQueue->numEntries(); idx++)
     {
       ...
       ComObjectName tblName(tblCatName, schName, tblObjName, COM_TABLE_NAME,
       ...
       if (sdo == StmtDDLAlterTableStoredDesc::GENERATE_DESC)
         ...
         {
           str_sprintf(query, "alter table %s generate stored stats ",
                       tblName.getExternalName().data());
           cliRC = cliInterface.executeImmediate(query);
           if (cliRC < 0)
             {
         ...
         {
           str_sprintf(query, "alter table %s delete stored desc ",
                       tblName.getExternalName().data());
           cliRC = cliInterface.executeImmediate(query);
           if (cliRC < 0)
             {
         ...
         {
           str_sprintf(query, "alter table %s delete stored stats ",
                       tblName.getExternalName().data());
           cliRC = cliInterface.executeImmediate(query);
           if (cliRC < 0)
             {
   ...
   if (someObjectsCouldNotBeAltered)
     {
       ...
       *CmpCommon::diags() << DgSqlCode(-CAT_UNABLE_TO_ALTER_SCHEMA)
       ...
       goto label_error;
            ...
            sdo == StmtDDLAlterTableStoredDesc::CHECK_STATS)
   ...
   if (!ComIsTrafodionReservedSchemaName(schName))
   {
     cliRC = updateSeabaseXDCDDLTable(&cliInterface, catName.data(), schName.data(),
                                      NAString(SEABASE_SCHEMA_OBJECTNAME).data(),
                                      COM_PRIVATE_SCHEMA_OBJECT_LIT);
     if (cliRC < 0)
     {
       cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
...
// *                                                                           *
// *****************************************************************************
Lng32 CmpSeabaseDDL::alterSchemaTableDesc (
   ExeCliInterface *cliInterface, 
      ...
      case StmtDDLAlterTableStoredDesc::GENERATE_DESC:
        {
          ...
          cliRC =
           updateObjectRedefTime(cliInterface,
                                 getSystemCatalog(), schName, SEABASE_SCHEMA_OBJECTNAME,
                                 objTypeLit,
                                 -1, schUID, TRUE);
          if (cliRC < 0  || rc < 0)
            cliRC = -1;
          ...
          insertIntoSharedCacheList(catName, schName,
                                    SEABASE_SCHEMA_OBJECTNAME_EXT,
                                    objType, SharedCacheDDLInfo::DDL_INSERT);
      ...
      case StmtDDLAlterTableStoredDesc::DELETE_DESC:
        {
          ...
          cliRC = deleteFromTextTable
           (cliInterface, schUID, COM_STORED_DESC_TEXT, 0);
          if (cliRC < 0 || rc < 0)
            cliRC = rc;
          else
            removeFromCache = TRUE;
          ...
          if ( sharedDescCache ) 
            {
              insertIntoSharedCacheList(catName, schName,
                                        SEABASE_SCHEMA_OBJECTNAME_EXT,
                                        objType, SharedCacheDDLInfo::DDL_DELETE);
  ...
  if (removeFromCache)
    {
      ...
      ActiveSchemaDB()->getNATableDB()->removeNATable
            (cn,
             ComQiScope::REMOVE_FROM_ALL_USERS, objType,
             ddlXns, FALSE);
    }
  ...
  const NABoolean ddlXns)
{
  ...
  Int32 schOwner = 0;
  Int64 schUID = getObjectTypeandOwner(cliInterface,
                                       catName.data(),
                                       schName.data(),
                                       SEABASE_SCHEMA_OBJECTNAME,objType,schOwner);
...
int32_t newOwnerID = -1;
   ...
   if (ComUser::getAuthIDFromAuthName(giveSchemaNode->getAuthID().data(),
                                      newOwnerID, CmpCommon::diags()) != 0)
      return;

   if (CmpSeabaseDDLauth::isTenantID(newOwnerID) ||
       CmpSeabaseDDLauth::isUserGroupID(newOwnerID))
   {
     *CmpCommon::diags() << DgSqlCode(-CAT_IS_NOT_CORRECT_AUTHID)
  ...
  NAString &tableNotCreated)
{
   ...
   if (cliInterface == NULL)
     {
       cli = new(STMTHEAP) ExeCliInterface 
         (STMTHEAP, 0, NULL,
          CmpCommon::context()->sqlSession()->getParentQid());
     }
   else
...
// *                                                                           *
// *****************************************************************************
bool CmpSeabaseDDL::dropOneTable(
   ExeCliInterface & cliInterface,
   const char * catalogName, 
   const char * schemaName, 
    ...
    if (CmpCommon::getDefault(TRAF_ENABLE_METADATA_LOAD_IN_SHARED_CACHE) == DF_ON)
      {
        ...
        insertIntoSharedCacheList(catalogName, schemaName, objectName,
...
// ****************************************************************************
void CmpSeabaseDDL::grantRevokeSchema(
  ...
  NAString    & currSchName)
{
  ...
  if (isGrant)
  {
    ...
    grantedByName = isGrantedBySpecified ? 
      grantNode->getByGrantor()->getAuthorizationIdentifier() : "";
  }
  else
  {
    ...
    grantedByName = isGrantedBySpecified ?
      revokeNode->getByGrantor()->getAuthorizationIdentifier() : "";
  }

  NAString catalogName = schemaQualName.getCatalogName();
  NAString schemaName = schemaQualName.getSchemaName();
  ComObjectName objName(catalogName,schemaName,NAString(SEABASE_SCHEMA_OBJECTNAME),
                        COM_TABLE_NAME,TRUE);
  ...
  Int64 objectUID = getObjectUID(&cliInterface,
                                 catalogName.data(), schemaName.data(), objectName.data(),
                                 NULL,NULL,
                                 "object_type = '" COM_PRIVATE_SCHEMA_OBJECT_LIT"' or "
  ...
  objectUID = getObjectInfo(&cliInterface,
                            catalogName.data(), schemaName.data(),
  ...
  else
  {
    ...
    {
      PrivType privType;
      if (!(privActsArray[k]->elmPrivToPrivType(privType)) ||
          (!isDMLPrivType(privType) && !isDDLPrivType(privType)))
      {
  ...
  else
  {
    ...
    if (!CmpSeabaseDDLauth::isUserID(grantee) && 
        !CmpSeabaseDDLauth::isRoleID(grantee) &&
  ...
  if (isGrant)
  {
    PrivStatus result = command.grantSchemaPrivilege(objectUID,
                                                     std::string(extSchName),
                                                      objectType,
                                                     effectiveGrantorID,
                                                     effectiveGrantorName,
                                                     grantee,
  ...
  {
    PrivStatus result = command.revokeSchemaPrivilege(objectUID,
                                                      std::string(extSchName),
                                                      objectType,
                                                      effectiveGrantorID,
                                                      effectiveGrantorName,
  ...
  if (updateObjectRedefTime(&cliInterface,
                        ...
                        objectTypeLit, -1, objectUID, genStoredDesc) > 0)
    {
       processReturn();
       return;
    }
   ...
   if (genStoredDesc)
    {
      insertIntoSharedCacheList(catalogName, schemaName,
                                SEABASE_SCHEMA_OBJECTNAME_EXT,
                                objectType, SharedCacheDDLInfo::DDL_UPDATE);
    }
}
