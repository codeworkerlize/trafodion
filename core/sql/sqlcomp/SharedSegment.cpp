...
//////////////////////////////////////////////////////////
SharedSegment::SharedSegment() : shmid_(INVALID_SHMID), errno_(-1), isDataCache_(false) {
      ...
      if (errno == ENOENT) {
        ...
        if ((shmid_ = shmget(key_, size_, shmFlag | IPC_CREAT)) == -1) {
          ...
          if (enableHugePages_) {
            ...
            if ((shmid_ = shmget(key_, size_, shmFlag | IPC_CREAT)) == -1) ok = false;
          } else
            ok = false;
        }
...
void *SharedSegment::attachVirtualAddress(const void *addr) {
  ...
  if (!isShmidValid() && !locateOrCreateSegment()) return NULL;
  ...
  if ((addr_ = shmat(shmid_, addr, flag)) == (void *)-1 || addr_ != addr) {
    errno_ = errno;
    addr_ = NULL;
  } else
    assert(addr_ == addr);
...
void SharedSegment::init(key_t key, size_t sz, bool hpages, void *addr) {
  key_ = key;
  size_ = sz;
...
key_t SharedSegment::getKeyForDefaultSegment(bool isCreatedForData) {
  ...
  if (isCreatedForData) key += SHARED_DATA_SEGMENT_KEY_OFFSET;
...
UInt64 SharedSegment::roundupToSHMLBA(UInt64 offset) { return SharedSegment::traf_roundup(offset, SHMLBA); }

void SharedSegment::display(ostream &out, const char *msg) {
  out << msg << " key=" << std::hex << getKey() << std::dec << ", shmid=" << getShmid()
      << ", addr=" << static_cast<void *>(getAddr()) << ", size=" << getSize() / (1024 * 1024) << "MB" << endl;
}

void SharedSegment::display(const char *msg) { display(cout, msg); }
...
//////////////////////////////////////////////////////////
DefaultSharedSegment::DefaultSharedSegment() : SharedSegment(), segmentDB_(NULL) {
...
DefaultSharedSegment::~DefaultSharedSegment() {
  ...
  if (segmentDB_) segmentDB_->detachDynamicSegments();
...
void *DefaultSharedSegment::computeDefaultSegmentAddr(bool isDataCache) {
  if (isDataCache) return (void *)SHARED_DATA_SEGMENT_MOMORY_ADDR;

  UInt64 offset = (UInt64)getRmsSharedMemoryAddr() + computeSegmentSizeForRMS() +
                  (isDataCache ? computeDefaultSegmentSize() : 0) + SHMLBA;  // add SHMLBA (4K on linux) as the gap
...
key_t SharedSegmentDB::getNextKey() {
  ...
  if (nextKey_ == RMS_SEGMENT_ID_OFFSET) nextKey_++;

  return nextKey_;
}

SharedSegment *SharedSegmentDB::createSharedSegment(size_t size) {
  ...
  for (int i = 0; i < MAX_NUM_SHARED_SEGMENTS; i++) {
    ...
    if (current->isShmidValid()) continue;
    ...
    if ((size + sizeof(NABlock) < currentSegSize) && current->isShmidValid()) {
...
void SharedSegmentDB::attachDynamicSegments() {
  ...
  for (int i = 0; i < MAX_NUM_SHARED_SEGMENTS; i++) {
    current = &sharedSegments_[i];
    if (current->isShmidValid()) current->attachVirtualAddress(current->getAddr());
  }
}
...
size_t SharedSegmentDB::computeTotalSize(bool validSegment) {
  ...
  for (int i = 0; i < MAX_NUM_SHARED_SEGMENTS; i++) {
    if (SIZE_MAX - size < sharedSegments_[i].getSize()) return SIZE_MAX;

    if (validSegment && !sharedSegments_[i].isShmidValid()) continue;
...
void SharedSegmentDB::createAndDestroyNSegments(int numSegments, size_t size) {
  ...
  cout << "createAndDestroyNSegments(): "
       << ", numSegments=" << numSegments << ", size=" << size << endl;
  ...
  for (int i = 0; i < numSegments; i++) {
    if (indicators[i]) {
      ...
      if (segment)
        cout << "re-create segment at " << i << ", addr=" << static_cast<void *>(segment->getAddr())
