...
class MyRow : public PrivMgrMDRow {
 public:
  ...
  // -------------------------------------------------------------------
  MyRow(std::string tableName) : PrivMgrMDRow(tableName, COMPONENT_OPERATIONS_ENUM), componentUID_(0){};
  ...
  bool lookupByCode(const int64_t componentUID, const std::string &operationCode, std::string &operationName,
                    PrivMgrComponentOperations::OperationType &operationType, std::string &operationDescription);

  bool lookupByName(const int64_t componentUID, const std::string &operationName, std::string &operationCode,
                    PrivMgrComponentOperations::OperationType &operationType, std::string &operationDescription);
...
class MyTable : public PrivMgrMDTable {
 public:
  MyTable(const std::string &tableName, PrivMgrTableEnum myTableEnum, ComDiagsArea *pDiags)
      : PrivMgrMDTable(tableName, COMPONENT_OPERATIONS_ENUM, pDiags), lastRowRead_(tableName){};

  inline void clear() { lastRowRead_.clear(); };

  PrivStatus fetchByCode(const int64_t componentUID, const std::string &operationCode, MyRow &row);

  PrivStatus fetchByName(const std::string &componentUIDString, const std::string &operationName, MyRow &row);

  PrivStatus fetchByName(const int64_t componentUID, const std::string &operationName, MyRow &row);
  ...
  PrivStatus update(const std::string &setClause, const std::string &whereClause);
...
// -----------------------------------------------------------------------
PrivMgrComponentOperations::PrivMgrComponentOperations(const std::string &metadataLocation, ComDiagsArea *pDiags)
...
PrivMgrComponentOperations::PrivMgrComponentOperations(const PrivMgrComponentOperations &other)
    : PrivMgr(other), myTable_(*new MyTable(fullTableName_, COMPONENT_OPERATIONS_ENUM, pDiags_)) {
...
// Destructor.
// -----------------------------------------------------------------------
PrivMgrComponentOperations::~PrivMgrComponentOperations() { delete &myTable_; }
...
// *                                                                           *
// *****************************************************************************
bool PrivMgrComponentOperations::codeExists(const int64_t componentUID, const std::string &operationCode)
...
// *                                                                           *
// *****************************************************************************
PrivStatus PrivMgrComponentOperations::createOperation(const std::string &componentName,
...

{
  ...
  if (operationCode.size() != 2 ||
      (operationCode.size() == 2 && (operationCode[0] == ' ' || operationCode[1] == ' '))) {
  ...
  if (nameExists(componentUID, operationName)) {
    if (existsErrorOK) return STATUS_GOOD;

    *pDiags_ << DgSqlCode(-CAT_COMPONENT_PRIVILEGE_NAME_EXISTS) << DgString0(operationName.c_str());
    return STATUS_ERROR;
  }
  ...
  if (codeExists(componentUID, operationCode)) {
    ...
    *pDiags_ << DgSqlCode(-CAT_COMPONENT_PRIVILEGE_CODE_EXISTS) << DgString0(operationCode.c_str());
    return STATUS_ERROR;
  }
  ...
  return componentPrivilege.grantPrivilegeToCreator(componentUID, operationCode, ComUser::getCurrentUser(),
...
PrivStatus PrivMgrComponentOperations::createOperationInternal(
    const int64_t componentUID, const std::string &operationName, const std::string &operationCode,
    const bool operationTypeUnused, const std::string &operationDescription, const int32_t granteeID,
    const std::string &granteeName, const int32_t grantDepth, const bool checkExistence) {
  PrivStatus privStatus = STATUS_GOOD;

  // If operation already created, no need to create
  if (checkExistence && nameExists(componentUID, operationName)) return STATUS_GOOD;
  ...
  privStatus = componentPrivileges.grantPrivilegeInternal(componentUID, operationCodes, SYSTEM_USER,
                                                          ComUser::getSystemUserName(), granteeID, granteeName,
                                                          grantDepth, checkExistence);
...
// *                                                                           *
// *****************************************************************************
PrivStatus PrivMgrComponentOperations::describeComponentOperations(const std::string &componentUIDString,
  ...
  if (privStatus == STATUS_GOOD) {
    for (int i = 0; i < rowList.size(); i++) {
      ...
      if (myRow->operationType_ == OP_TYPE_UNUSED) continue;
      ...
      if (myRow->operationType_ == OP_TYPE_SYSTEM) componentText += " SYSTEM";

      if (!myRow->operationDescription_.empty()) componentText += " DETAIL '" + myRow->operationDescription_ + "'";

      componentText += ";";
      outlines.push_back(componentText);
      outlines.push_back("");
      if (componentPrivileges)
        componentPrivileges->describeComponentPrivileges(componentUIDString, componentName, myRow->operationCode_,
                                                         myRow->operationName_, outlines);
      delete myRow;
      outlines.push_back("");
    }
  }
...
// *                                                                           *
// *****************************************************************************
PrivStatus PrivMgrComponentOperations::dropOperation(const std::string &componentName, const std::string &operationName,
...

{
  ...
  fetchByName(componentUIDString, operationName, operationCode, isSystemOperation, tempStr);
  ...
  PrivStatus privStatus = componentPrivileges.dropAllForOperation(componentUIDString, operationCode);
...
// *                                                                           *
// *****************************************************************************
PrivStatus PrivMgrComponentOperations::fetchByName(const std::string &componentUIDString,
                                                   const std::string &operationName, std::string &operationCode,
                                                   bool isSystem, std::string &operationDescription)

{
  ...
  if (privStatus == STATUS_NOTFOUND || privStatus == STATUS_ERROR) return STATUS_NOTFOUND;
...
// *                                                                           *
// *****************************************************************************
PrivStatus PrivMgrComponentOperations::getCount(const int64_t &componentUID, int32_t &numOps, int32_t &numUnusedOps) {
  char buf[getMetadataLocation().size() + 300];
  snprintf(buf, sizeof(buf),
           "select distinct is_system, count(is_system) over "
           "(partition by is_system) from %s.%s where component_uid = %ld",
           getMetadataLocation().c_str(), PRIVMGR_COMPONENT_OPERATIONS, componentUID);
  ...
  for (int idx = 0; idx < tableQueue->numEntries(); idx++) {
    ...
    numOps += opTypeCount;
    if (value[0] == 'U') numUnusedOps += opTypeCount;
  }
...
// *****************************************************************************
PrivStatus PrivMgrComponentOperations::updateOperationCodes(const int64_t &componentUID) {
  ...
  if (!firstUnusedOp) {
    ...
    if (myTable.update(setClause, unusedItems) == STATUS_ERROR) return STATUS_ERROR;
  }
  ...
  if (!firstSystemOp) {
    ...
    if (myTable.update(setClause, systemItems) == STATUS_ERROR) return STATUS_ERROR;
  }
  return STATUS_GOOD;
}
...
// *                                                                           *
// *****************************************************************************
bool PrivMgrComponentOperations::nameExists(const int64_t componentUID, const std::string &operationName)
...
// *                                                                           *
// *****************************************************************************
PrivStatus MyTable::fetchByCode(const int64_t componentUID, const std::string &operationCode, MyRow &row)

{
  // Check the last row read before reading metadata.
  if (lastRowRead_.lookupByCode(componentUID, operationCode, row.operationName_, row.operationType_,
...
// *                                                                           *
// *****************************************************************************
PrivStatus MyTable::fetchByName(const std::string &componentUIDString, const std::string &operationName, MyRow &row)
...
// *                                                                           *
// *****************************************************************************
PrivStatus MyTable::fetchByName(const int64_t componentUID, const std::string &operationName, MyRow &row)

{
  // Check the last row read before reading metadata.
  if (lastRowRead_.lookupByName(componentUID, operationName, row.operationCode_, row.operationType_,
                                row.operationDescription_)) {
    row.componentUID_ = componentUID;
    return STATUS_GOOD;
  }
...
PrivStatus MyTable::insert(const PrivMgrMDRow &rowIn) {
  ...
  sprintf(insertStatement, "insert into %s values (%ld, '%s', '%s', '%c', '%s')", tableName_.c_str(), row.componentUID_,
          row.operationCode_.c_str(), row.operationName_.c_str(), operationType, row.operationDescription_.c_str());
...

{
  ...
  std::string selectStmt(
      "SELECT COMPONENT_UID, OPERATION_CODE, OPERATION_NAME, IS_SYSTEM, OPERATION_DESCRIPTION FROM ");
...
// *****************************************************************************
PrivStatus MyTable::update(const std::string &setClause, const std::string &whereClause) {
  char updateStatement[setClause.size() + whereClause.size() + tableName_.size() + 100];

  sprintf(updateStatement, "update %s %s %s", tableName_.c_str(), setClause.c_str(), whereClause.c_str());
...
// *                                                                           *
// *****************************************************************************
bool MyRow::lookupByCode(const int64_t componentUID, const std::string &operationCode, std::string &operationName,
                         PrivMgrComponentOperations::OperationType &operationType, std::string &operationDescription)

{
  // If componentUID_ is zero, that means data is uninitialized.

  if (componentUID_ == 0 || componentUID != componentUID_ || operationCode != operationCode) return false;
...
// *                                                                           *
// *****************************************************************************
bool MyRow::lookupByName(const int64_t componentUID, const std::string &operationName, std::string &operationCode,
                         PrivMgrComponentOperations::OperationType &operationType, std::string &operationDescription)

{
  // If componentUID_ is zero, that means data is uninitialized.

  if (componentUID_ == 0 || componentUID != componentUID_ || operationName != operationName_) return false;
...
// *           *: Select failed. A CLI error is put into the diags area.
// *****************************************************************************
PrivStatus MyTable::selectWhere(const std::string &whereClause, std::vector<MyRow *> &rowList) {
  std::string selectStmt(
      "SELECT COMPONENT_UID, OPERATION_CODE, OPERATION_NAME, IS_SYSTEM, TRIM(OPERATION_DESCRIPTION) FROM ");
  selectStmt += tableName_;
  selectStmt += " ";
  selectStmt += whereClause;
