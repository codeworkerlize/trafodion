...
bool PrivMgrCoreDesc::operator==(const PrivMgrCoreDesc &other) const {
  ...
  return ((priv_ == other.priv_) && (wgo_ == other.wgo_));
}

void PrivMgrCoreDesc::setPriv(const PrivType which, const bool value) {
  switch (which) {
    case ALL_PRIVS:
...
void PrivMgrCoreDesc::setWgo(const PrivType which, const bool value) {
  switch (which) {
    case ALL_PRIVS:
...
//
// This method sets all the relevant DML/DDL privileges for the specified
...
//
// ----------------------------------------------------------------------------
void PrivMgrCoreDesc::setAllObjectPrivileges(const ComObjectType objectType, const bool priv, const bool wgo,
                                             const bool all_dml, const bool all_ddl) {
  switch (objectType) {
    case COM_BASE_TABLE_OBJECT:
      setAllTableGrantPrivileges(priv, wgo, all_dml, all_ddl);
      break;
    case COM_LIBRARY_OBJECT:
      setAllLibraryGrantPrivileges(priv, wgo, all_dml, all_ddl);
      break;
    case COM_SEQUENCE_GENERATOR_OBJECT:
      setAllSequenceGrantPrivileges(priv, wgo, all_dml, all_ddl);
      break;
...
PrivMgrCoreDesc::PrivResult PrivMgrCoreDesc::grantPrivs(PrivMgrCoreDesc &param) {
  if (this->isNull()) return NEUTRAL;  // Do nothing.  No privs to be granted.
  ...
  for (size_t i = 0; i < NBR_OF_PRIVS; i++) {
    ...
    PrivMgrCoreDesc::interpretChanges(inPriv.test(pType), priv_.test(pType), result);
    PrivMgrCoreDesc::interpretChanges(inWgo.test(pType), wgo_.test(pType), result);
  }
...
PrivMgrCoreDesc::PrivResult PrivMgrCoreDesc::revokePrivs(PrivMgrCoreDesc &param) {
  if (this->isNull()) return NEUTRAL;  // Do nothing.  No privs to be revoked.
...
bool PrivMgrCoreDesc::limitToGrantable(const PrivMgrCoreDesc &other) {
  ...
  return !(beforePriv.none() && beforeWgo.none());
}
...
bool PrivMgrCoreDesc::cascadeAnyNotSet(const PrivMgrCoreDesc &other) {
  ...
  if ((this->priv_ == priv) && (this->wgo_ == wgo))
...
bool PrivMgrDesc::limitToGrantable(const PrivMgrDesc &other) {
  bool result = false;
  if (schemaLevel_.limitToGrantable(other.schemaLevel_)) result = true;

  if (tableLevel_.limitToGrantable(other.tableLevel_)) result = true;
  ...
  for (int i = 0; i < columnLevel_.entries(); i++) {
    ...
    if (index >= 0) {
      ...
      if (columnLevel_[i].limitToGrantable(temp)) result = true;
    } else {
      if (columnLevel_[i].limitToGrantable(temp)) result = true;
    }
  }
...
NAString PrivMgrDesc::print() {
  char intStr[20];
  NAString msg("Descs for ");
  msg += str_itoa(grantee_, intStr);
  msg += ", is schema: ";
  msg += str_itoa(schemaObject_, intStr);
  msg += ", tblLevel: ";
  msg += tableLevel_.print();
  msg += ", colLevel: ";
  for (int i = 0; i < columnLevel_.entries(); i++) {
    msg += "column - ";
    msg += i;
    msg += columnLevel_[i].print();
  }
  msg += ", schLevel: ";
  msg += schemaLevel_.print();
  return msg;
}
