...
// ----------------------------------------------------------------------------
short CmpSeabaseDDL::createSeabaseTenant(
   ...
   NABoolean reportNotEnabled)
{
  ...
  Lng32 cliRC = 0;
  char queryBuf[strlen(getSystemCatalog()) + strlen(SEABASE_TENANT_SCHEMA) +
                strlen(DB__ROOT) + 100];

  if (CmpCommon::context()->useReservedNamespace())
    str_sprintf(queryBuf, "create schema if not exists %s.\"%s\" authorization %s namespace '%s' ",
                getSystemCatalog(),SEABASE_TENANT_SCHEMA, DB__ROOT,
                ComGetReservedNamespace(SEABASE_TENANT_SCHEMA).data());
  else
  ...
  if (cliRC < 0)
   ...
   for (int32_t i = 0; i < numberTenants; i++)
   {
    ...
    if (tenantDefinition.isSpecialAuth || tenant.authExists(tenantDefinition.authName))
      continue;

     if (tenant.registerStandardTenant(tenantDefinition.authName, tenantDefinition.authID) == -1)
       return -1;
   }
   ...
   str_sprintf(queryBuf, "create trafodion namespace '%d' if not exists", 
               SYSTEM_TENANT_ID);
   cliRC = cliInterface->executeImmediate(queryBuf);
   if (cliRC < 0)
   {
   ...
   if (addCompPrivs && isAuthorizationEnabled())
   {
     ...
     PrivMgrMDAdmin admin(std::string(MDLoc.data()),
                          std::string(privMgrMDLoc.data()),
                          CmpCommon::diags());

     if (admin.initializeComponentPrivileges() != STATUS_GOOD)
        return -1;
   }
...
short CmpSeabaseDDL::dropSeabaseTenant(ExeCliInterface *cliInterface)
{
  ...
  if (authInfo.getAdminRoles(adminRoles) == CmpSeabaseDDLauth::STATUS_ERROR)
    return -1;
  ...
  for (Int32 i = 0; i < adminRoles.size(); i++)
  {
    ...
    // Ignore otherwise.
    if (ComUser::getAuthNameFromAuthID(roleID,
                                       adminRoleName,
                                       MAX_DBUSERNAME_LEN,
                                       length) == 0)
    {
  ...
  if (tenantMDexists)
  {
    str_sprintf(queryBuf, "drop schema if exists %s.\"%s\" cascade ",
                getSystemCatalog(),SEABASE_TENANT_SCHEMA);
  ...
  if (hasError && CmpCommon::diags()->contains(-1064))
...
//  -----------------------------------------------------------------------------
short CmpSeabaseDDL::getTenantNSList(
  ExeCliInterface *cliInterface,
  NAList<NAString *> *&tenantNSList)
{
 // Caller manages memory for the list, must have allocated an empty list
  ...
  char getStmt[100];
  char percent('%');
  Int32 stmtSize = snprintf(getStmt, sizeof (getStmt),
                            "get trafodion namespaces, match '%s1%c'",
                            TRAF_NAMESPACE_PREFIX, percent);
...
//     0 - call successful (list may be empty)
// -----------------------------------------------------------------------------
short CmpSeabaseDDL::getTenantSchemaList(
  ExeCliInterface *cliInterface,
  TenantSchemaInfoList *&tenantSchemaList)
{
  ...
  Int32 stmtSize = snprintf(selectStmt, sizeof (selectStmt),
                            ...
                            "order by 1, 2, 3",
                            getSystemCatalog(), SEABASE_TENANT_SCHEMA, SEABASE_TENANTS,
                            getSystemCatalog(), SEABASE_TENANT_SCHEMA, SEABASE_TENANT_USAGE);
...
short CmpSeabaseDDL::isTenantMetadataInitialized(ExeCliInterface *cliInterface)
{
  ...
  if (tenantID != SYSTEM_TENANT_ID && tenantID != NA_UserIdDefault)
    return 1;

  // read metadata to see if tenant schema exists
  // returns: 1, exists. 0, does not exists. -1, error.
  return existsInSeabaseMDTable(cliInterface,
                                getSystemCatalog(), SEABASE_TENANT_SCHEMA, SEABASE_SCHEMA_OBJECTNAME);
};
...
//   >0 = a tenant schema
// -----------------------------------------------------------------------------
short CmpSeabaseDDL::isTenantSchema(
  ExeCliInterface *cliInterface,
  const Int64 &schemaUID)
{
  ...
  Int32 stmtSize = snprintf(selectStmt, sizeof (selectStmt),
                            ...
                            getSystemCatalog(), SEABASE_TENANT_SCHEMA,
                            SEABASE_TENANT_USAGE, schemaUID);
...
// ----------------------------------------------------------------------------
void CmpSeabaseDDL::registerSeabaseTenant (
  ExeCliInterface *cliInterface,
  StmtDDLTenant *pNode )
{
  CmpSeabaseDDLtenant reg_tenant(getSystemCatalog(),getMDSchema());
  Int32 adminRoleID = 0;
  if (reg_tenant.registerTenantPrepare(pNode, adminRoleID) == -1)
    return;
...
// ----------------------------------------------------------------------------
void CmpSeabaseDDL::unregisterSeabaseTenant(
  ExeCliInterface *cliInterface,
  StmtDDLTenant *pNode)
{
  CmpSeabaseDDLtenant reg_tenant(getSystemCatalog(),getMDSchema());
  if (reg_tenant.unregisterTenantPrepare(pNode) == -1)
    return;

  TenantInfo tenantInfo(STMTHEAP);
  if (reg_tenant.getTenantDetails(pNode->getTenantName(), tenantInfo) == CmpSeabaseDDLauth::STATUS_ERROR)
    return;
  ...
  if (!CURRCONTEXT_CLUSTERINFO->hasVirtualNodes() || nodeAllocationList.length() == 0)
  {
    ...
    if (!tenantFound || (reg_tenant.getAuthID() != SYSTEM_TENANT_ID))
    {
      ...
      {
        *CmpCommon::diags() << DgSqlCode(CAT_TENANT_ALLOCATIONS)
...
short CmpSeabaseDDL::upgradeSeabaseTenant(ExeCliInterface * cliInterface)
{
  ...
  for (int32_t i = 0; i < numberTenants; i++)
  {
    ...
    Int32 retcode = TenantInfo::getTenantInfo(authDefinition.authID,
                                              systemTenantInfo);
    ...
    else if (retcode == 0)
    {
      ...
      {
        systemTenantInfo.setTenantNodes(
             new(STMTHEAP) NAASNodes(-1, -1, -1));
    ...
    if (authDefinition.isSpecialAuth || tenant.authExists(authDefinition.authName))
      continue;

    if (tenant.registerStandardTenant(authDefinition.authName, authDefinition.authID) == -1)
      return -1;
  }
...
// -----------------------------------------------------------------------------
void CmpSeabaseDDL::createSeabaseRGroup(
  ExeCliInterface * cliInterface,
  StmtDDLResourceGroup *pNode)
{
  ...
  {
    *CmpCommon::diags() << DgSqlCode (-3127)
                        << DgString0(groupName.data());
    return;
  }
  ...
  if (pNode->getOwner())
  {
    ...
    if (!CmpSeabaseDDLauth::isRoleID(grantee) &&
        !CmpSeabaseDDLauth::isUserID(grantee))
    {
    ...
    {
      *CmpCommon::diags() << DgSqlCode(-CAT_AUTHORIZATION_CLAUSE_INCORRECT)
                          << DgString0(groupName.data())
  ...
  if (pNode->getNodeList())
  {
    ...
    if (!group.validNodeNames((ConstStringList *)pNode->getNodeList(), invalidNodeNames))
    {
      *CmpCommon::diags() << DgSqlCode(-CAT_NODE_NAME_INVALID)
                          << DgString0(invalidNodeNames.data());
      return;
    }
  }
  ...
  if (pNode->getNodeList())
  {
    for (CollIndex i = 0; i < pNode->getNodeList()->entries(); i++)
    {
      ...
      TenantResourceUsage *usage  = new ((NAMemory *)group.getHeap())
            TenantResourceUsage(group.getResourceUID(),
                               group.getResourceName(),
                               node.getResourceUID(),
                               *nodeName,
    ...
    // insert node usages
    if (usageList.insertUsages() == -1)
      return;
  }

  // Insert resource group
  if (group.insertRow() == -1)
    return;
}
...
// ComDiags is set up if an error occurs
// -----------------------------------------------------------------------------
void CmpSeabaseDDL::alterSeabaseRGroup(
  ExeCliInterface * cliInterface,
  StmtDDLResourceGroup *pNode)
{
  ...
  switch (pNode->getAlterType())
  {
    case  StmtDDLResourceGroup::ALTER_ADD_NODE:
    {
      ...
      {
        *CmpCommon::diags() << DgSqlCode(-CAT_NODE_NAME_INVALID)
                            << DgString0(invalidNodeName.data());
        return;
      }
      ...
      for (CollIndex i = 0; i < pNode->getNodeList()->entries(); i++)
      {
        ...
        const NAString *nodeName = (*pNode->getNodeList())[i];
        TenantResourceUsage *existingUsage = existingUsageList->findUsage(nodeName, TenantResourceUsage::RESOURCE_USAGE_NODE);
        if (existingUsage)
          continue;

        // generate node resource
        TenantResource node (cliInterface, STMTHEAP);
        if (node.generateNodeResource((NAString *)nodeName, group.getCreator()) == -1)
          return ;
      ...
      if (numUsages > 0)
      {
        if (addUsageList.insertUsages() == -1)
          return;
      }
     ...
     // If tenant associated with resource group, set up a warning
      ...
      if (group.getTenantsForResource(tenantNames) == -1)
        return;
      if (tenantNames.length() > 0)
      {
         tenantNames.prepend("(");
         tenantNames.append(")");
         *CmpCommon::diags() << DgSqlCode(CAT_BALANCE_TENANTS)
                             << DgString0(groupName.data())
    ...
    case StmtDDLResourceGroup::ALTER_DROP_NODE:
    {
      ...
      for (CollIndex i = 0; i < pNode->getNodeList()->entries(); i++)
      {
        ...
        const NAString *nodeName = (*pNode->getNodeList())[i];
        TenantResourceUsage *usage = existingUsageList->
          findUsage(nodeName, TenantResourceUsage::RESOURCE_USAGE_NODE);
        if (usage == NULL)
        {
          NAString node ("Node ");
          node += nodeName->data();
        ...
        stmtSize = snprintf(buf, sizeof(buf),
                            "(resource_uid = %ld and usage_uid = %ld)",
                            usage->getResourceUID(), usage->getUsageUID());
        assert(stmtSize < 100);
      ...
      {
        *CmpCommon::diags() << DgSqlCode(-CAT_LAST_NODE)
                            << DgString0(groupName.data());
        return;
      }
      ...
      NAString tenantNames;
      if (group.getTenantsForNodes(deletedNodes, tenantNames) == -1)
        return;

      if (tenantNames.length() > 0)
      {
        if (group.getTenantInfoList(tenantNames, tenantInfoList) == -1)
          return;
      ...
      Int64 rowsDeleted = 0;
      if (numUsages > 0)
        rowsDeleted = existingUsageList->deleteUsages(rgroupNodeClause);
      ...
      stmtSize = snprintf(deleteStmt, sizeof(deleteStmt),
                          ...
                          sysCat.data(), SEABASE_TENANT_SCHEMA, SEABASE_RESOURCES,
                          nodeObsoleteClause.data(),
                          sysCat.data(), SEABASE_TENANT_SCHEMA, SEABASE_RESOURCE_USAGE,
                          group.getResourceUID());
    ...
    case StmtDDLResourceGroup::ALTER_OFFLINE:
    {
      ...
      if (group.updateRows(setClause.data(), whereClause.data()) == -1)
        return;
      break;
    }
...
// ComDiags is set up if an error occurs
// -----------------------------------------------------------------------------
void CmpSeabaseDDL::dropSeabaseRGroup(
  ExeCliInterface * cliInterface,
  const StmtDDLResourceGroup *pNode)
{
  ...
  {
    *CmpCommon::diags() << DgSqlCode(-CAT_RESOURCE_GROUP_NOT_EXIST)
                        << DgString0(pNode->getGroupName().data());
    return;
  }
  ...
  if (tenantNames.length() > 0)
  {
    tenantNames.prepend("(");
    tenantNames.append(")");
    *CmpCommon::diags() << DgSqlCode(-CAT_DEPENDENT_OBJECTS_EXIST)
                        << DgString0(tenantNames.data());
    return;
  }
  ...
  whereClause += (to_string((long long int)group.getResourceUID())).c_str();
  if (group.getUsageList()->deleteUsages(whereClause) == -1)
    return;

  if (group.removeObsoleteNodes(group.getUsageList()) == -1)
    return;

  if (group.deleteRow() == -1)
    return;
}
...
// -----------------------------------------------------------------------------
Int64 CmpSeabaseDDL::authIDOwnsResources(
  Int32 authID,
  NAString &resourceNames)
{
  ...
  Int32 bufSize = 500;
  char buf[bufSize];
  snprintf(buf, bufSize, "SELECT [FIRST 5] resource_name FROM %s.\"%s\".%s"
...
{
  if (usageString == "S ")
    return TenantUsage::TENANT_USAGE_SCHEMA;
  if (usageString == "U ")
    return TenantUsage::TENANT_USAGE_USER;
  if (usageString == "R ")
    return TenantUsage::TENANT_USAGE_ROLE;
  if (usageString == "G ")
    return TenantUsage::TENANT_USAGE_GROUP;
  if (usageString == "RG")
    return TenantUsage::TENANT_USAGE_RESOURCE;
...
Int32 TenantUsageList::deleteUsages(const char *whereClause)
{
  ...
  ExeCliInterface cliInterface(heap_, 0, NULL,
...
// ----------------------------------------------------------------------------
NABoolean TenantUsageList::findTenantUsage ( 
...
// ----------------------------------------------------------------------------
Int32 TenantUsageList::getNumberUsages(
  ...
  bool skipObsolete) const
{
  ...
  for (Int32 i = 0; i < entries(); i++)
  {
    ...
    {
      if (skipObsolete && usage.isObsolete())
        continue;
      numUsages++;
    }
  }
...
// ----------------------------------------------------------------------------
Int32 TenantUsageList::selectUsages(
  ...
  NAHeap *heap)
{
  ...
  Int32 diagsMark = CmpCommon::diags()->mark();
  ExeCliInterface cliInterface(heap, 0, NULL,
    CmpCommon::context()->sqlSession()->getParentQid());
  Queue * tableQueue = NULL;
...
void TenantUsageList::obsoleteTenantRGroupUsages()
{
  for (Int32 i = 0; i < entries(); i++)
  {
    if (operator[](i).getUsageType() == TenantUsage::TENANT_USAGE_RESOURCE)
      operator[](i).setIsObsolete(true);
  }
}
...
// -----------------------------------------------------------------------------
void TenantUsageList::setTenantUsageObsolete (
  const Int64 tenantUsageID,
  const bool obsolete)
{
  for (Int32 i = 0; i < entries(); i++)
...
Int32 TenantUsageList::insertUsages()
{
  ...
  for (Int32 i = 0; i < entries(); i++)
  {
    ...
    maxSize = snprintf(stmt, sizeof(stmt),
                        "(%ld, %d, '%s', %ld)",
                        usage.getUsageID(),
                        usage.getTenantID(),
                        usageType.data(),
...
TenantInfo::TenantInfo( const Int32 tenantID,
...
{
  if (assignedNodes_ && assignedNodes_->castToNAASNodes())
    return assignedNodes_->castToNAASNodes()->getAffinity();
...
{
  if (assignedNodes_ && assignedNodes_->castToNAASNodes())
    return assignedNodes_->castToNAASNodes()->getClusterSize();
...
bool TenantInfo::validateTenantSize()
{
  if (!assignedNodes_)
  {
    *CmpCommon::diags() << DgSqlCode(-7033)
                        << DgInt0(-1)
                        << DgString0("No nodes have been assigned to this tenant");
    return false;
  }
  ...
  {
    *CmpCommon::diags() << DgSqlCode(-7033)
                        << DgInt0(assignedNodes_->getTenantSize())
                        << DgString0(errMsg);
    return false;
  }
  return true;
...
// ----------------------------------------------------------------------------
void TenantInfo::balanceTenantNodeAlloc(
  ...
  bool &registrationComplete)
{
  ...
  {
    rc = TenantHelper_JNI::getInstance()->alterTenant(
         getTenantName(),
         *origTenantNodeSet,
         *tenantNodeSet,
         NAString(buf),
  ...
  { 
    rc = TenantHelper_JNI::getInstance()->registerTenant(
         getTenantName(),
         *tenantNodeSet,
         NAString(buf),
         tenantDefaultSchema,
  ...
  if (tenantNodeSet->castToNAWSetOfNodeIds())
  {
    for (int u=0; u<tenantNodeSet->getNumNodes(); u++)
    {
      ...
      if (tenantNodeSet->usesDenseNodeIds())
        nodeId = nacl->mapLogicalToPhysicalNodeId(nodeId);
      ...
      {
        tenantRUsageList->updateUsageValue(nodeName,
                                           getTenantName(),
...
//----------------------------------------------------------------------------- 
void TenantInfo::allocTenantNode (
  ...
  bool skipInvalidWeights)
{
  ...
  else
  {
    ...
    else
    {
      ...
      for (CollIndex i = 0; i < nodeList->entries(); i++)
      {
        ...
        if (skipInvalidWeights && nodeWeight == -1)
          continue;
  ...
  {
    *CmpCommon::diags() << DgSqlCode(-7033)
                        << DgInt0(tenantNodeSet->getTotalWeight())
                        << DgString0(invalidMsg);
    UserException excp (NULL, 0);
    throw excp;
  }
}
...
Int32 TenantInfo::dropTenantInfo()
{
  ...
  if (deleteUsages(whereClause.c_str()) != 0)
    return -1;
...
Int32 TenantInfo::modifyTenantInfo(
  ...
  const TenantUsageList *usageList)
{
  ...
  if (usageList)
  {
    ...
    {
     whereClause += ")";
      if (deleteUsages(NAString(whereClause.c_str())) == -1)
        return -1;
    }

    if (insertUsageList.entries() > 0)
      if (insertUsageList.insertUsages() == -1)
        return -1;
  }
...
//   -1 = failed (diags area is set up with error)
// ----------------------------------------------------------------------------
Int32 TenantInfo::updateTenantInfo(
  const bool updateInfo,
  TenantUsageList *usageList,
  const Int32 tenantSize)
{
  // add usage, insert row into TENANT_USAGE table
  if (usageList && usageList->entries() > 0)
    if (usageList->insertUsages() != 0)
      return -1;

  // Update the TENANTS row
  if (updateInfo)
    if (updateRow(tenantSize) != 0)
      return -1;
...
Int32 TenantInfo::deleteRow()
{
  ...
  char deleteStmt[500];
  Int32 stmtSize = snprintf(deleteStmt, sizeof(deleteStmt),
                            "delete from %s where tenant_id = %d",
                            mdLocation.data(),
                            tenantID_);
...
Int32 TenantInfo::insertRow(const Int32 tenantSize)
{
  ...
  char insertStmt[500];
  Int32 stmtSize = snprintf(insertStmt, sizeof(insertStmt),
                            "insert into %s values (%d, %d, %ld, %d, %d, %d, '%s', %ld)",
                            mdLocation.data(),
                            tenantID_, adminRoleID_, defaultSchemaUID_, 
                            getAffinity(), getClusterSize(), 
                            ((tenantSize == -1) ? getTenantSize() : tenantSize),
                            details.data(), flags_);
  if (stmtSize >= 500)
  {
...
Int32 TenantInfo::selectRow(const std::string &whereClause)
{
  ...
  else if (affinity >= 0)
  {
    ...
    setTenantNodes(new(heap_) NAASNodes(tenantSize,
                                        *(Int32*)pCliRow->get(4),
                                        affinity));
  } 
  else
  ...
  if (assignedNodes_->castToNAWSetOfNodeIds())
  {
    ...
    if (CURRCONTEXT_CLUSTERINFO->hasVirtualNodes() &&
        CURRCONTEXT_CLUSTERINFO->getTotalNumberOfCPUs() == 1) 
    {
    ...
    else
    {
      for (CollIndex i = 0; i < resourceUsageList->entries(); i++)
      {
        ...
        {
          *CmpCommon::diags() << DgSqlCode(-CAT_NODE_NAME_INVALID)
                              << DgString0(nodeInfo->getResourceName().data());
          return -1;
        }
        if (nodeInfo->getUsageValue() > 0)
            nodeMap[nodeID]= nodeInfo->getUsageValue();
      }
...
Int32 TenantInfo::updateRow(const Int32 tenantSize)
{
  ...
  Int32 stmtSize = snprintf(updateStmt, sizeof(updateStmt),
                            ...
                            "where tenant_id = %d",
                            mdLocation.data(),
                            adminRoleID_, defaultSchemaUID_, 
...
const void TenantInfo::setTenantNodes(NAWNodeSet *assignedNodes)
{
  ...
  if (assignedNodes_ && (assignedNodes_ == assignedNodes))
    return;
  if (assignedNodes_)
    delete assignedNodes_;
...
{
  // If trying to set to itself, just return
  if (origAssignedNodes_ && (origAssignedNodes_ == assignedNodes))
    return;
  if (origAssignedNodes_)
    delete origAssignedNodes_;
  ...
  TenantResourceUsageList *& nodeList)
{
  ...
  snprintf (buf, sizeof(buf), "where usage_uid = %d and usage_type = 'T'",
            getTenantID());
  whereClause = buf;
...
TenantSchemaInfoList::~TenantSchemaInfoList()
{
  for (CollIndex i = 0; i < entries(); i++)
     NADELETE(operator[](i), TenantSchemaInfo, STMTHEAP); 
  clear();
}
...
void TenantSchemaInfoList::getSchemaList(
  Int32 tenantID,
  NAList<Int64> &schemaList)
{
  ...
  {
    TenantSchemaInfo *tenantSchemaInfo = operator[](i);
    if (tenantSchemaInfo->getTenantID() == tenantID)
      schemaList.insert(tenantSchemaInfo->getSchemaUID());
  }
}
...
Int32 TenantSchemaInfo::removeSchemaUsage()
{
  ...
  if (tenantDefinition.getTenantDetails(getTenantID(), tenantInfo) == CmpSeabaseDDLauth::STATUS_ERROR)
    return -1;
  ...
  if (isDefSch() && tenantInfo.getNumberSchemaUsages() == 2)
  {
    ...
    defSchUID = (firstUsage.getUsageID() == getSchemaUID()) ?
      lastUsage.getUsageID() : firstUsage.getUsageID();
  }

  tenantInfo.setDefaultSchemaUID(defSchUID);
  if (tenantInfo.dropTenantInfo(isDefSch(), &usageList) == -1)
    return -1;
  return 0;
}
...
// No errors are generated
// -----------------------------------------------------------------------------
TenantResource::TenantResourceType 
TenantResource::getResourceTypeAsEnum(NAString &resourceType)
{
  if (resourceType == "N ")
    return TenantResourceType::RESOURCE_TYPE_NODE;
  if (resourceType == "G ")
     return TenantResourceType::RESOURCE_TYPE_GROUP;
...
// -----------------------------------------------------------------------------
bool TenantResource::validNodeNames(
  ConstStringList *usageNameList,
  NAString &invalidNodeNames)
{
  // for development machines, don't verify
  NAString nodeAllocationList = CmpCommon::getDefaultString(NODE_ALLOCATIONS).data();
  if ((getenv("CLUSTERNAME")  == NULL) || nodeAllocationList.length() > 0)
    return true;
...
//   -1 - failed (ComDiags contains error) 
// -----------------------------------------------------------------------------
short TenantResource::alterNodesForRGroupTenants(
  ExeCliInterface *cliInterface,
  ...
  const NAList<NAString> &deletedNodes)
{
  ...
  for (CollIndex i = 0; i < tenantInfoList->entries(); i++)
  {
     ...
     if (getNodesToDropFromTenant(cliInterface, tenantInfo, deletedNodes, nodesNotFound ) == -1)
       return -1;

    // All nodes are available from other rgroups, go to next tenant
    if (nodesNotFound.entries() == 0)
      continue;
    ...
    try
    {
      ...
      for (CollIndex j = 0; j < resourceUsageList->entries(); j++)
      {
        ...
        else
        {
          ...
          TenantNodeInfo *newNodeInfo = new (STMTHEAP)
            TenantNodeInfo(rUsage->getResourceName(), nodeID, -1, 
                           rUsage->getUsageUID(), -1,
                           STMTHEAP);
      ...
      if (balanceRequired)
      {
        ...
        if (getenv("CLUSTERNAME") || nodeAllocationList.length() == 0)
        {
          ...
          tenantInfo->allocTenantNode(FALSE, -1, -1,
                                      tenantSize, origNodeList,
                                      origTenantNodeSet, true);
          tenantInfo->setOrigTenantNodes(origTenantNodeSet);

          NAWNodeSet *tenantNodeSet = NULL;
          tenantInfo->allocTenantNode(FALSE, -1, -1,
                                      tenantSize, newNodeList,
                                      tenantNodeSet, false);
          tenantInfo->setTenantNodes(tenantNodeSet);

          // adjust node allocations, etc
          tenantInfo->balanceTenantNodeAlloc(tenantInfo->getDefaultSchemaName(), -2, false,
                                            origTenantNodeSet, tenantNodeSet,
    ...
    catch (...)
    {
      ...
      if (registrationComplete)
      {
         try
         {
           ...
           tenantInfo->setTenantNodes(NULL);
           tenantInfo->setOrigTenantNodes(NULL);
           *CmpCommon::diags() << DgSqlCode(-CAT_RESOURCE_GROUP_NO_RESOURCES)
                               << DgString0(getResourceName().data())
      ...
      if (newNodeList)
        NADELETE(newNodeList, TenantNodeInfoList, STMTHEAP);
...
{
  NAString tenantNames;
  if (getTenantsForResource(tenantNames) == -1)
    return false;
  if (tenantNames.length() == 0)
    return false;
...
// -----------------------------------------------------------------------------
short TenantResource::backoutAssignedNodes(
  ExeCliInterface *cliInterface,
  const NAList<TenantInfo *> &tenantInfoList)
{
  for (CollIndex i = 0; i < tenantInfoList.entries(); i++)
  {
    ...
    if (tenantInfo->getOrigTenantNodes() && tenantInfo->getTenantNodes())
    {
      ...
      try
      {
        ...
        NAWNodeSet *origSet = (NAWNodeSet*)tenantInfo->getOrigTenantNodes();
        NAWNodeSet *newSet = (NAWNodeSet*)tenantInfo->getTenantNodes();
        tenantInfo->balanceTenantNodeAlloc(tenantInfo->getDefaultSchemaName(), -2, 
                                          true, newSet, origSet,
                                          resourceUsageList, registrationComplete);
        NADELETE (resourceUsageList, TenantResourceUsageList, STMTHEAP);
        *CmpCommon::diags() << DgSqlCode(-CAT_RESOURCE_GROUP_NO_RESOURCES)
...
short TenantResource::createStandardResources()
{
  ...
  NAString sysCat =  CmpSeabaseDDL::getSystemCatalogStatic();
  Int32 stmtSize = snprintf(deleteStmt, sizeof(deleteStmt),
                            "delete from %s.\"%s\".%s where resource_name = '%s'",
                            sysCat.data(), SEABASE_TENANT_SCHEMA, SEABASE_RESOURCES,
                            RGROUP_DEFAULT);
  assert (stmtSize < 500);
  ...
  NAString &groupText)
{
      ...
      (currentUser != getCreator()))
  {
    ...
    if (!componentPrivileges.hasSQLPriv(currentUser,SQLOperation::MANAGE_TENANTS,true) &&
        !componentPrivileges.hasSQLPriv(currentUser,SQLOperation::MANAGE_RESOURCE_GROUPS, true) &&
        !componentPrivileges.hasSQLPriv(currentUser,SQLOperation::SHOW, true) &&
        !assignedTenant(currentTenant))
    {
      *CmpCommon::diags() << DgSqlCode(-CAT_NOT_AUTHORIZED);
      return false;
    }
  ...
  if (usageList_)
  {
    ...
    for (Int32 i = 0; i < usageList_->entries(); i++)
    {
      ...
      if (usage->isNodeUsage())
        groupText += usage->getUsageName();
    }
    groupText += "')";
  }
  ...
  int32_t length;
  ComUser::getAuthNameFromAuthID(getCreator(),
                                 ownerName,
                                 MAX_DBUSERNAME_LEN,
                                 length,
  ...
  if (!isValid())
    groupText += "-- resource group is offline\n";
  return true;
}
...
//    0 - successful
// -----------------------------------------------------------------------------
short TenantResource::generateNodeResource(
  NAString *nodeName,
  const Int32 grantee)
{
...
//    0 - successful
// -----------------------------------------------------------------------------
short TenantResource::getNodesToDropFromTenant (
  ExeCliInterface *cliInterface,
  ...
  NAList<NAString> &nodesNotFound)
{
  ...
  if (rgroupsExist)
  {
    ...
    if (rgroupUsages.fetchUsages(whereClause, orderByClause) == -1)
      return -1;
  }
  ...
  for (int i = 0; i < deletedNodes.entries(); i++)
  {
    ...
    if (rgroupUsages.findUsage(&nodeName, TenantResourceUsage::RESOURCE_USAGE_NODE))
      continue;
...
// -----------------------------------------------------------------------------
short TenantResource::getTenantsForNodes(
...
//    0 - request successful
// -----------------------------------------------------------------------------
short TenantResource::getTenantsForResource(
  NAString &tenantNames)
{
  assert (tenantNames.isNull());
  ...
  Int32 stmtSize = snprintf(selectStmt, sizeof(selectStmt),
                            ...
                            sysCat.data(), SEABASE_MD_SCHEMA, SEABASE_AUTHS,
                            sysCat.data(), SEABASE_TENANT_SCHEMA, SEABASE_TENANT_USAGE,
                            getResourceUID());
  assert (stmtSize < bufSize);
...
// -----------------------------------------------------------------------------
short TenantResource::getTenantInfoList(
  ...
  NAList<TenantInfo *> &tenantInfoList)
{
  ...
  while (!done)
  {
    ...
    if (tenantInfo.getTenantNodes()->castToNAASNodes())
      continue;
    ...
    TenantInfo *newTenantInfo = new (STMTHEAP) 
      TenantInfo(tenantInfo.getTenantID(), tenantInfo.getAdminRoleID(),
                 tenantInfo.getDefaultSchemaUID(),
                 usageList, NULL, /*tenantNodes*/
                 tenantName, STMTHEAP); 
...
// -----------------------------------------------------------------------------
short TenantResource::fetchMetadata (
...
short TenantResource::insertRow()
{
  ...
  Int32 stmtSize = snprintf(insertStmt, sizeof(insertStmt),
                            ...
                            " '%s', %ld, %ld, '%s', '%s', %ld)",
                            sysCat.data(), SEABASE_TENANT_SCHEMA, SEABASE_RESOURCES,
                            UID_, name_.data(), type.data(), creator_, isValid.data(),
                            createTime_, redefTime_, 
                            details1_.data(), details2_.data(), flags_);
  if (stmtSize >= 500)
  { 
    SEABASEDDL_INTERNAL_ERROR("TenantInfo::insertRow failed, internal buffer size too small");
    return -1;
...
short TenantResource::selectRow( const NAString &whereClause)
{
  ...
  Int32 stmtSize = snprintf(selectStmt, sizeof(selectStmt),
                            ...
                            sysCat.data(), SEABASE_TENANT_SCHEMA,
                            SEABASE_RESOURCES, whereClause.data());
...
//     0 - successful
// -----------------------------------------------------------------------------    
short TenantResource::updateRows(
...
short TenantResource::updateRedefTime()
{
  ...
  Int32 stmtSize = snprintf(updateStmt, sizeof(updateStmt),
                            "update %s.\"%s\".%s SET resource_redef_time = %ld where resource_uid = %ld",
                            sysCat.data(), SEABASE_TENANT_SCHEMA, SEABASE_RESOURCES,
                            time, UID_);
  assert (stmtSize < 128);
...
// No errors are generated
// -----------------------------------------------------------------------------
TenantResourceUsage::ResourceUsageType 
TenantResourceUsage::getUsageTypeAsEnum (NAString &usageType)
{
  if (usageType == "N ")
    return TenantResourceUsage::RESOURCE_USAGE_NODE;  
  if (usageType == "T ")
    return TenantResourceUsage::RESOURCE_USAGE_TENANT;
...
{
  for (CollIndex i = 0; i < entries(); i++)
     NADELETE(operator[](i), TenantResourceUsage, getHeap());
  clear();
}
...
// -----------------------------------------------------------------------------
Int64 TenantResourceUsageList::deleteUsages(
  const NAString &whereClause)
{
  NAString sysCat = CmpSeabaseDDL::getSystemCatalogStatic();
  Int32 deleteStmtLen = whereClause.length() + 200; 
  char deleteStmt[deleteStmtLen];
  Int32 stmtSize = snprintf(deleteStmt, deleteStmtLen,
                           " delete from %s.\"%s\".%s %s ",
                           sysCat.data(), SEABASE_TENANT_SCHEMA, SEABASE_RESOURCE_USAGE,
                           whereClause.data());
...
//    0 - successful
// -----------------------------------------------------------------------------
short TenantResourceUsageList::fetchUsages(
  const NAString &whereClause,
  const NAString &orderByClause)
{
  NAString sysCat = CmpSeabaseDDL::getSystemCatalogStatic();
  ...
  Int32 stmtSize = snprintf(selectStmt, sizeof(selectStmt),
                            ...
                            "from %s.\"%s\".%s %s %s",
                            sysCat.data(), SEABASE_TENANT_SCHEMA, SEABASE_RESOURCE_USAGE,
                            whereClause.data(), orderByClause.data());
  assert(stmtSize < 500);
...
// -----------------------------------------------------------------------------
TenantResourceUsage * TenantResourceUsageList::findResource(
  ...
  const TenantResourceUsage::ResourceUsageType &resourceType)
{
  for (CollIndex i = 0; i < entries(); i++)
  {
    ...
    if ((usage->getResourceName() == *resourceName) &&
        (usage->getUsageUID() == usageUID) &&
...
// No errors are expected
// -----------------------------------------------------------------------------
TenantResourceUsage * TenantResourceUsageList::findUsage(
  ...
  const TenantResourceUsage::ResourceUsageType &usageType)
{
  ...
  {
    TenantResourceUsage *usage = operator[](i);
    if ((usage->getUsageName() == *usageName) &&
        (usage->getUsageType() == usageType))
      return usage;
  }
  return NULL;
}
...
Int32 TenantResourceUsage::insertRow(ExeCliInterface *cliInterface)
{
  ...
  Int32 stmtSize = snprintf(insertStmt, sizeof(insertStmt),
                          ...
                          "(%ld, '%s', %ld, '%s', '%s', %ld, %ld)",
                          sysCat.data(), SEABASE_TENANT_SCHEMA, SEABASE_RESOURCE_USAGE,
                          resourceUID_, resourceName_.data(),
                          usageUID_, usageName_.data(),
                          getUsageTypeAsString(usageType_).data(), 
                          usageValue_,flags_);
  if (stmtSize >= 500)
  {
...
Int32 TenantResourceUsage::updateRow(ExeCliInterface *cliInterface)
{
  ...
  Int32 stmtSize = snprintf(updateStmt, sizeof(updateStmt),
                            ...
                            sysCat.data(), SEABASE_TENANT_SCHEMA, SEABASE_RESOURCE_USAGE,
                            usageValue_, flags_, resourceUID_, usageUID_);
  if (stmtSize >= 500)
  {
...
short TenantResourceUsageList::insertUsages()
{
  ...
  for (Int32 i = 0; i < entries(); i++)
  {
    ...
    NAString usageType = usage->getUsageTypeAsString(usage->getUsageType());
    stmtSize = snprintf(stmt, sizeof(stmt),
                        "(%ld, '%s', %ld, '%s', '%s', %ld, %ld)",
                        usage->getResourceUID(), usage->getResourceName().data(),
                        usage->getUsageUID(), nodeName.data(),
                        usageType.data(), usage->getUsageValue(), usage->getFlags());
...
// value.
// -----------------------------------------------------------------------------
void TenantResourceUsageList::updateUsageValue(
  const NAString &resourceName,
  const NAString &usageName,
  ...
  Int64 newValue)
{
  for (CollIndex i = 0; i < entries(); i++)
  {
    TenantResourceUsage *usage = operator[](i);
    if ((usage->getUsageType() == usageType) && 
        (usage->getResourceName() == resourceName) &&
...
NAString TenantNodeInfoList::getNodeName(const Int64 logicalNodeID)
{
  ...
  {
     TenantNodeInfo *nodeInfo = (*this)[i];
     if (nodeInfo->getLogicalNodeID() == logicalNodeID)
...
Int64 TenantNodeInfoList::getNodeID(const NAString &nodeName)
{
  ...
  for (CollIndex i = 0; i < entries(); i++)
  {
     TenantNodeInfo *nodeInfo = (*this)[i];
     if (nodeInfo->getNodeName() == nodeName)
       return nodeInfo->getLogicalNodeID();
  }
  return nodeID;
}
...
void TenantNodeInfoList::orderedInsert(TenantNodeInfo *nodeInfo)
{
  ...
  for (i; i < entries(); i++)
  {
    ...
    if (thisNodeInfo->getLogicalNodeID() > nodeInfo->getLogicalNodeID())
       break;
  }
