...
#define FREE_USER_ROLES_CACHE(userRolesCache)                                                \
    {                                                                                        \
        for (std::map<int32_t, std::set<int32_t> *>::iterator itor = userRolesCache.begin(); \
             itor != userRolesCache.end(); itor++)                                           \
        {                                                                                    \
            if (NULL != itor->second)                                                        \
            {                                                                                \
                delete itor->second;                                                         \
...

{
   ...
   if (!isAuthIDGrantedPrivs(roleID,privClass, objectUIDs, hasComponentPriv))
      return false;
...

{
   ...
   if (fetchGroupRoles && !skipGroupCheck && isUserID(authID))
   {
     ...
     if (userInfo.getUserGroups(authIDs) == CmpSeabaseDDLauth::STATUS_ERROR)
      return STATUS_ERROR;
   }
...
// *****************************************************************************
PrivStatus PrivMgrRoles::fetchSystemGrantee(
   const int32_t roleID,
   std::string & granteeName,
   int32_t & granteeID)
{
   char buf [300];
   snprintf (buf, sizeof(buf), "where role_id = %d and grantor_id = -2 and "
   ...
   bool includeTenantGrantees)
{
  ...
  PrivStatus privStatus = myTable.selectAllWhere(whereClause,orderByClause,rows);
   ...
   for (size_t r = 0; r < rows.size(); r++)
   {
      ...
      if (includeTenantGrantees ||
          (!includeTenantGrantees && !isTenantID(row.granteeID_)))
      {
   ...
   bool includeTenantGrantees)
{
   ...
   for (size_t r = 0; r < rows.size(); r++)
   {
      ...
      if (includeTenantGrantees && isTenantID(row.granteeID_) ||
          CmpSeabaseDDLauth::isUserID(row.granteeID_) ||
          CmpSeabaseDDLauth::isUserGroupID(row.granteeID_) || 
          CmpSeabaseDDLauth::isRoleID(row.granteeID_))
         granteeIDs.push_back(row.granteeID_);
   }
...
// -----------------------------------------------------------------------------
PrivStatus PrivMgrRoles::fetchTenantGrantees(
   std::vector<int32_t> &tenantGrantees)
{
   ...
   if (myTable.selectAuthGrantees(whereClause,tenantGrantees) == STATUS_ERROR)
      return STATUS_ERROR;
  return STATUS_GOOD;
}
     ...
     
{
   ...
   if (!grantExists(roleID, grantorID, granteeID, row.grantDepth_))
...
// *****************************************************************************
PrivStatus PrivMgrRoles::hasRoleAdmin(
   const int32_t roleID,
   const int32_t granteeID,
   const std::string &roleName,
   const std::vector<int32_t> &roleIDs,
   bool &hasAdmin)
{
  ...
  if (componentPrivileges.hasSQLPriv(granteeID, compPrivList, hasAdmin) < 0)
    return STATUS_ERROR;
  ...
  if (myTable.selectCountWhere(whereClause,rowCount) == STATUS_ERROR)
    return STATUS_ERROR;
...
std::vector<int32_t> roleIDs;
   ...
   privStatus = revokeRole(roleIDs,granteeIDs,granteeClasses,grantorIDs,
  ...
  
{
   ...
   for (size_t r2 = 0; r2 < roleIDs.size(); r2++)
   {
      ...
      for (size_t g2 = 0; g2 < granteeIDs.size(); g2++)
      {
         ...
         ComSecurityKey::QIType keyType = (isUserID(granteeIDs[g2])) ? 
                                           ComSecurityKey::SUBJECT_IS_USER :
                                           ComSecurityKey::SUBJECT_IS_GROUP;
...
// -----------------------------------------------------------------------------
PrivStatus MyTable::selectAuthGrantees(
   std::string whereClause,
   std::vector<int32_t> &granteeIDs)
{
...
static bool isNeedToInvalidCache(std::map<int32_t, std::set<int32_t> *> &userRolesCache, int32_t granteeID, int32_t roleID)
{
    //fetch cache
