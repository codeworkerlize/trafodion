...
inline static bool validateExternalGroupname(
...
#define CHECK_LICENSE                                                                                   \
    ...
    {                                                                                                   \
        ...
        {                                                                                               \
            *CmpCommon::diags() << DgSqlCode(-CAT_NO_LICENSE_ERROR)                                     \
                                << DgString0("LOCAL-AUTH");                                             \
            return;                                                                                     \
        }                                                                                               \
    } while (0)
...
{
  int bufSize = strlen(TRAFODION_SYSCAT_LIT)*12 + strlen(SEABASE_PRIVMGR_SCHEMA)*6 +
                strlen(SEABASE_MD_SCHEMA)*6 + strlen(PRIVMGR_OBJECT_PRIVILEGES)*2 +
                strlen(PRIVMGR_COLUMN_PRIVILEGES)*2 + strlen(PRIVMGR_SCHEMA_PRIVILEGES)*2 +
                strlen(SEABASE_AUTHS)*6 + (18*100);
  char queryBuf[bufSize];

  Queue * tableQueue = NULL;

  str_sprintf(queryBuf, "select distinct grantee_id as sourceID, auth_id as targetID, auth_db_name, "
...
CollIndex AuthConflictList::getIndexForSourceID(Int32 sourceID )
{
  for (CollIndex i = 0; i < entries(); i++)
  {
     ...
     if (conflict.getSourceID() == sourceID)
       return i;
  }
  return NULL_COLL_INDEX;
}
...
short AuthConflictList::updateObjects(ExeCliInterface *cliInterface)
{
  ...
  int bufSize = strlen(TRAFODION_SYSCAT_LIT) + strlen(SEABASE_MD_SCHEMA) +
                strlen(SEABASE_OBJECTS) + (sourceIDList.length()*2) + 200; 
  char queryBuf[bufSize];
  ...
  str_sprintf(queryBuf, "select object_uid, schema_owner, object_owner from %s.\"%s\".%s "
              "where (schema_owner in (%s) or object_owner in (%s))",
              TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_OBJECTS,
              sourceIDList.data(), sourceIDList.data());
  ...
  for (Int32 i = 0; i < tableQueue->numEntries(); i++)
  {
    ...
    str_sprintf (queryBuf, "update %s.\"%s\".%s %s where object_uid = %ld",
                 TRAFODION_SYSCAT_LIT, SEABASE_MD_SCHEMA, SEABASE_OBJECTS,
                 setClause.data(), objectUID);
    ...
    if (updateSchOwner)
    {
      AuthConflict schConflict(schOwner, operator[](schIndex).getAuthName(), 
                               operator[](schIndex).getTargetID(),
    ...
    if (updateObjOwner)
    {
      AuthConflict objConflict(objOwner, operator[](objIndex).getAuthName(), 
                               operator[](objIndex).getTargetID(),
...
// Result: < 0 (error); 0 good
// ----------------------------------------------------------------------------
short AuthConflictList::updatePrivs (
  ExeCliInterface *cliInterface,
  const char * schName,
  const char * objName)
{
  int bufSize = strlen(TRAFODION_SYSCAT_LIT) + strlen(schName) + strlen(objName) +
                MAX_AUTHID_AS_STRING_LEN + MAX_DBUSERNAME_LEN + 200;
  char queryBuf[bufSize];
  ...
  for (Int32 i = 0; i < entries(); i++)
  {
    ...
    {
      str_sprintf(queryBuf, "update %s.\"%s\".%s set %s = %d where %s = '%s'",
                  TRAFODION_SYSCAT_LIT, schName, objName,
...
{
  if (authGroupList_)
    NADELETE(authGroupList_, TenantGroupInfoList, authGroupList_->getHeap());
  authGroupList_ = NULL;
}
...
// -----------------------------------------------------------------------------
CmpSeabaseDDLauth::AuthStatus CmpSeabaseDDLauth::getAdminRole(
  const Int32 authID, Int32 &adminRoleID)
{
   // If there is no current tenant, just return
   Int32 tenantID = ComTenant::getCurrentTenantID(); 
   if (tenantID == NA_UserIdDefault)
   ...
   for (size_t i = 0; i < adminRoles.size(); ++i)
   {
     if (std::find(myRoles.begin(), myRoles.end(), adminRoles[i]) != myRoles.end())
     {
       ...
       {
         if (roleForTenant.getTenantID() == tenantID)
           return STATUS_GOOD;
       }
     }
   }
...
CmpSeabaseDDLauth::getAdminRoles(std::vector<Int32> &roleIDs)
{
  Int64 authFlags = 0;
  authFlags |= SEABASE_IS_ADMIN_ROLE;

  char buf[200];
  str_sprintf(buf, "where flags = %ld and auth_type = 'R'"
                   " and auth_creator <> %d", authFlags, getAuthID());
...
// ----------------------------------------------------------------------------
CmpSeabaseDDLauth::AuthStatus 
CmpSeabaseDDLauth::getAuthsForCreator(
  Int32 authCreator, 
  ...
  std::vector<std::string> &roleNames)
{
  ...
  char buf[200];
  str_sprintf(buf, "where flags = %ld and auth_type in ('R', 'U')"
                   "  and auth_creator = %d", authFlags, authCreator);
  NAString whereClause(buf);
  ...
  bool fetchGroupRoles)
{
  ...
  short isUsingMeta = 
      switcher.switchCompiler(CmpContextInfo::CMPCONTEXT_TYPE_META);
...
// ----------------------------------------------------------------------------
TenantGroupInfoList * CmpSeabaseDDLtenant::getTenantGroupList(
  ...
  if (usages)
  {
    ...
    for (CollIndex i = 0; i < usages->entries(); i++)
    {
      ...
      if (usage.isGroupUsage())
      {
        ...
        TenantGroupInfo *tenantGroup  = new (heap) TenantGroupInfo(getAuthID(), 
                                                                   groupInfo.getAuthExtName(), 
                                                                   groupInfo.getAuthID(), 
                                                                   groupInfo.getAuthConfig());
  ...
  const Int32 maxValue)
{
  ...
  if (isUser())
  {
    ...
    len = snprintf(buf, 300,
                   "SELECT sub_id FROM %s.%s where text_uid = %d and text_type = %d;",
                    MDSchema_.data(), SEABASE_TEXT, specialID, COM_USER_PASSWORD);
    assert (len <= 300);
  ...
  if (isUser() && CmpCommon::getDefault(UNIQUE_USERID) == DF_ON && currentMaxUserID && newUserID > currentMaxUserID && newUserID < MAX_USERID)
  {
    memset(buf, 0, 300);
    len = snprintf(buf, 300,
                   "update %s.%s set sub_id = %d where text_uid = %d and text_type = %d; ",
                   MDSchema_.data(), SEABASE_TEXT, newUserID, specialID, COM_USER_PASSWORD);
    assert (len <= 300);
...
bool CmpSeabaseDDLauth::isAdminAuthID(const Int32 authCreator)
{
   ...
   if (authStatus != AuthStatus::STATUS_GOOD)
     return false;
   if (std::find(adminRoles.begin(), adminRoles.end(), authCreator) != adminRoles.end())
      return true;
   return false;
}
...
// Returns:  true if authID is a user ID, false otherwise
// ----------------------------------------------------------------------------
bool CmpSeabaseDDLauth::isTenantID(Int32 authID)
{
   return (authID >= MIN_TENANTID && authID <= MAX_TENANTID);
}
...
// Returns:  true if authID is a user ID, false otherwise
// ----------------------------------------------------------------------------
bool CmpSeabaseDDLauth::isUserGroupID(Int32 authID)
{
   return (authID >= MIN_USERGROUP && authID <= MAX_USERGROUP);
}
...
{ 
  ...
  if (!isSystem || isSpecialAuth)
  {
    ...
    errorMsg += getAuthType() == COM_TENANT_CLASS ? "tenant " :
                getAuthType() == COM_ROLE_CLASS ? "role " : "user ";
  ...
  if (isUser())
  {
    ...
    rc = ComEncryption::encrypt_ConvertBase64_Data(ComEncryption::AES_128_CBC,
						   (unsigned char *)(AUTH_DEFAULT_WORD),
						   strlen(AUTH_DEFAULT_WORD),
...
void CmpSeabaseDDLauth::deleteRow(const NAString &authName)
{
  ...
  // delete password
  str_sprintf(buf, "delete from %s.\"%s\".%s where text_uid in (select \
                    auth_id from %s.\"%s\".%s where auth_db_name = '%s') and text_type = %d; ",
              systemCatalog.data(), SEABASE_MD_SCHEMA,SEABASE_TEXT,
...
short CmpSeabaseDDLauth::updateSeabaseXDCDDLTable()
{
  ...
  for (Lng32 i = 0; i < numRows; i++)
  {
    ...
    str_sprintf(queryBuf, "upsert into %s.\"%s\".%s values ('%s', '%s', '%s', '%s', %d, cast(? as char(%d bytes) character set utf8 not null))",
                TRAFODION_SYSCAT_LIT, SEABASE_XDC_MD_SCHEMA, XDC_DDL_TABLE,
                TRAFODION_SYSCAT_LIT, quotedSchName.data(), quotedObjName.data(), COM_XDC_AUTH_OBJ_TYPE_LIT, i, currDataLen);
...
void CmpSeabaseDDLauth::insertRow()
{
  ...
  if (isAuthInTenant())
    authFlags |= SEABASE_IS_TENANT_AUTH;
  if (isAutoCreated())
    authFlags |= SEABASE_IS_AUTO_CREATED;

  NAString sysCat = CmpSeabaseDDL::getSystemCatalogStatic();
  str_sprintf(buf, "insert into %s.\"%s\".%s values (%d, '%s', '%s', '%s', %d, '%s', %ld, %ld, %ld)",
  ...
  if (cliRC > -1 && authType == COM_USER_CLASS_LIT)
  {
    ...
    str_sprintf(buf, "upsert into  %s.\"%s\".%s values (%d, %d, 0, 0, %ld, '%s')",
                sysCat.data(), SEABASE_MD_SCHEMA, SEABASE_TEXT,
                getAuthID(), COM_USER_PASSWORD,getAuthCreateTime(), getAuthPassword().data());
...
void CmpSeabaseDDLauth::updateRow(NAString &setClause)
{
  ...
  NAString sysCat = CmpSeabaseDDL::getSystemCatalogStatic();
  str_sprintf(buf, "update %s.\"%s\".%s %s auth_redef_time = %ld where auth_id = %d",
              sysCat.data(), SEABASE_MD_SCHEMA, SEABASE_AUTHS,
              setClause.data(), redefTime, getAuthID());
  ...
  if (cliRC > -1)
  {
    ...
    if (authPassword && !authPassword.isNull())
    {
      str_sprintf(buf, "update  %s.\"%s\".%s set text = '%s', flags = %ld where text_uid = %d and text_type = %d",
                  sysCat.data(), SEABASE_MD_SCHEMA, SEABASE_TEXT, 
                  getAuthPassword().data(), redefTime, getAuthID(),
                  COM_USER_PASSWORD);
    }
    cliRC = cliInterface.executeImmediate(buf);
  }
...
static Int32 getSeqnumFromText(Int32 userid)
{
    ...
    str_sprintf(buf, "select SEQ_NUM from %s.\"%s\".%s where TEXT_UID = %d and text_type = %d",
                sysCat.data(), SEABASE_MD_SCHEMA, SEABASE_TEXT,
                userid, COM_USER_PASSWORD);
    ...
    {
        memset(buf, 0, 1000);
        str_sprintf(buf, "insert into  %s.\"%s\".%s values (%d, %d, 0, 0, 0, '')",
                    sysCat.data(), SEABASE_MD_SCHEMA, SEABASE_TEXT,
                    userid, COM_USER_PASSWORD);
...
Int32 CmpSeabaseDDLauth::updateErrPwdCnt(Int32 userid, Int16 errcnt, bool reset)
{
    ...
    ExeCliInterface cliInterface(STMTHEAP);
    NAString sysCat = CmpSeabaseDDL::getSystemCatalogStatic();
    str_sprintf(buf, "update  %s.\"%s\".%s set SEQ_NUM = %d where text_uid = %d and text_type = %d",
                sysCat.data(), SEABASE_MD_SCHEMA, SEABASE_TEXT,
                val, userid, COM_USER_PASSWORD);
    Int32 cliRC = cliInterface.executeImmediate(buf);
    if (cliRC < 0)
    {
        cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
...
CmpSeabaseDDLauth::AuthStatus CmpSeabaseDDLauth::updateRedefTime()
{
  ...
  Int64 redefTime = NA_JulianTimestamp();
  NAString sysCat = CmpSeabaseDDL::getSystemCatalogStatic();
  str_sprintf(buf, "update %s.\"%s\".%s set auth_redef_time = %ld where auth_id = %d",
              sysCat.data(), SEABASE_MD_SCHEMA, SEABASE_AUTHS,
              redefTime, getAuthID());
...
// -----------------------------------------------------------------------------
CmpSeabaseDDLauth::AuthStatus CmpSeabaseDDLauth::selectAuthIDs(
  const NAString & whereClause,
  std::set<int32_t> &authIDs)
{
  char buf[whereClause.length() + 500];
  snprintf(buf, sizeof(buf), "select auth_id from %s.%s %s ",
  ...
  Int32 cliRC = cliInterface.fetchAllRows(queue,(char *)buf,0,
                                            false,false,true);
  if (cliRC < 0)
  {
    cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
...
{
  char buf[whereClause.length() + 500];
  snprintf(buf, sizeof(buf), "select distinct auth_id,auth_db_name from %s.%s %s ",
           MDSchema_.data(), SEABASE_AUTHS, whereClause.data());
...
CmpSeabaseDDLauth::AuthStatus CmpSeabaseDDLauth::selectAuthNames(
  const NAString & whereClause,
  std::vector<std::string> &userNames,
  std::vector<std::string> &roleNames,
  std::vector<std::string> &tenantNames)
{
  ...
  char buf[stmtSize];
  str_sprintf(buf, "select auth_db_name, auth_type from %s.%s %s" ,
              MDSchema_.data(),SEABASE_AUTHS,whereClause.data());
  ...
  int32_t cliRC = cliInterface.fetchAllRows(queue,(char *)buf,0,
                                            false,false,true);
  if (cliRC < 0)
  {
    cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
...
CmpSeabaseDDLauth::AuthStatus CmpSeabaseDDLauth::selectAllWhere(
  const NAString & whereClause,
  std::vector<Int32> & roleIDs)
{
  ...
  int stmtSize = whereClause.length() + 500;
  char buf[stmtSize];
  str_sprintf(buf, "select auth_id from %s.%s %s" ,
              MDSchema_.data(),SEABASE_AUTHS,whereClause.data());
...
CmpSeabaseDDLauth::selectExactRow(const NAString & whereClause)
{
  ...
  if (isUser())
  {
      ...
      str_sprintf(buf, "select text, flags from %s.%s where text_uid = %d and text_type = %d ",
                  MDSchema_.data(), SEABASE_TEXT, getAuthID(), COM_USER_PASSWORD);
      NAString cmd(buf);
...
// -----------------------------------------------------------------------------
Int32 CmpSeabaseDDLauth::getUserGroups(
  const char * userName, 
...
//    -1 - unexpected error (ComDiags is setup)
// -----------------------------------------------------------------------------
CmpSeabaseDDLauth::AuthStatus CmpSeabaseDDLauth::getUserGroups(
  std::vector<int32_t> &groupIDs)
{
  ...
  if (COM_LDAP_AUTH == currentAuthType_ &&
      Get_SqlParser_Flags(DISABLE_EXTERNAL_AUTH_CHECKS))
  {
  ...
  else
  {
      ...
      case COM_LDAP_AUTH:
      {
          ...
          DBUserAuth::CheckUserResult chkRslt =
              userSession->getGroupListForUser(getAuthDbName(), getAuthExtName(),
                                               ComUser::getCurrentExternalUsername(),
          ...
          case DBUserAuth::InvalidConfig:
              *CmpCommon::diags() << DgSqlCode(-CAT_LDAP_DEFAULTCONFIG)
                                  << DgString0("unknown");
              return STATUS_ERROR;
              break;
  ...
  for (size_t j = 0; j < authIDs.size(); j++)
  {
    ...
    if (groupID >= MIN_USERGROUP && groupID <= MAX_USERGROUP)
...
void CmpSeabaseDDLuser::registerUser(StmtDDLRegisterUser * pNode)
{
  ...
  Int32 retcode = 
        registerUserInternal (pNode->getExternalUserName(),
                              pNode->getDbUserName(),
                              pNode->getConfig().data(),
                              false, authCreator,
                              *userPwd);
  if (retcode)
  {
      NAString userName = (COM_LDAP_AUTH == CmpCommon::getAuthenticationType()) ? pNode->getExternalUserName() : pNode->getDbUserName();
      *CmpCommon::diags() << DgSqlCode(-CAT_REGISTER_USER_FAIL)
                          << DgString0(userName);
  }
...
// ----------------------------------------------------------------------------
Int32 CmpSeabaseDDLuser::registerUserInternal(
   const NAString &extUserName,
   const NAString &dbUserName,
   const char *config,
   bool  autoCreated,
   Int32 authCreator,
   const NAString &authPassword)
{
  ...
  if (autoCreated && !(COM_LDAP_AUTH == currentAuthType_ &&
                       CmpCommon::getDefault(TRAF_AUTO_REGISTER_USER) == DF_ON))
  {
    *CmpCommon::diags() << DgSqlCode (-CAT_AUTO_REGISTER_USER)
  ...
  try
  {
    ...
    if (authExists(getAuthDbName(), false))
    {
      *CmpCommon::diags() << DgSqlCode(-CAT_AUTHID_ALREADY_EXISTS)
             << DgString0("Authorization identifier")
    ...
    {
        Int16 foundConfigurationNumber = 
            DBUserAuth::DefaultConfiguration;

        // Verify that the external user exists in configured identity store
        if (!validateExternalUsername(extUserName.data(),
                                    config,
    ...
    if (NATable::usingSentry() && NATable::usingLinuxGroups() && !usingSentryUserApi)
    {
      ...
      struct passwd *pwName = getpwnam(linuxName.data());
       if (pwName == NULL) 
      {
        *CmpCommon::diags() << DgSqlCode(-CAT_SENTRY_NO_LINUX_UID)
               << DgString0(getAuthDbName().data())
    ...
    rc = ComEncryption::encrypt_ConvertBase64_Data(ComEncryption::AES_128_CBC,
                                              (unsigned char *)authPassword.data(),
                                              authPassword.length(),
                                              (unsigned char *)AUTH_PASSWORD_KEY,
                                              (unsigned char *)AUTH_PASSWORD_IV,
                                              (unsigned char *)dePassword,
...
void CmpSeabaseDDLuser::unregisterUser(StmtDDLRegisterUser * pNode)
{
  ...
  try
  {
    ...
    if (CmpCommon::context()->isAuthorizationEnabled())
    {
      ...
      if (role.fetchRolesForAuth(getAuthID(),roleNames,roleIDs,roleDepths,grantees,false) == PrivStatus::STATUS_ERROR)
        return;
      if (roleNames.size() > 0)
      {
         NAString roleList;
         ...
         *CmpCommon::diags() << DgSqlCode(-CAT_NO_UNREG_USER_GRANTED_ROLES)
                             << DgString0("user")
                             << DgString1(dbUserName.data())
                             << DgString2(roleList.data());
         return;
      }
    }
...
void CmpSeabaseDDLuser::alterUser (StmtDDLAlterUser * pNode)
{
  try
  {
    ...
    if ((!hasPriv && adminRole != getAuthCreator() && StmtDDLAlterUser::SET_USER_PASSWORD != cmdSubType) && isAuthorization)
    {
       // No authority.  We're outta here.
       *CmpCommon::diags() << DgSqlCode(-CAT_NOT_AUTHORIZED);
       return;
    ...
    switch (pNode->getAlterUserCmdSubType())
    {
        case StmtDDLAlterUser::SET_EXTERNAL_NAME:
        {
            ...
            if (getAuthExtName() != pNode->getExternalUsername())
            {
                ...
                if (COM_LDAP_AUTH == currentAuthType_)
                {
                    ...
                    (pNode->getConfig().isNull() ?  NULL : pNode->getConfig().data());

                    // Verify that the external user exists in configured identity store
                    if (!validateExternalUsername((pNode->getAlterUserCmdSubType() ==
                                                    StmtDDLAlterUser::SET_USER_PASSWORD
                                                    ? getAuthExtName().data()
        ...
        case StmtDDLAlterUser::SET_USER_PASSWORD:
        {
            ...
            rc = ComEncryption::encrypt_ConvertBase64_Data(ComEncryption::AES_128_CBC,
                            (unsigned char *)(pNode->getAuthPassword().data()),
                            pNode->getAuthPassword().length(),
                            (unsigned char *)AUTH_PASSWORD_KEY,
                            (unsigned char *)AUTH_PASSWORD_IV,
                            (unsigned char *)dePassword,
                            cryptlen);
        ...
        case StmtDDLAlterUser::SET_USER_JOIN_GROUP:
        {
            ...
            std::set<int32_t> joinedGroups;
            {
                ...
                if (CmpSeabaseDDLauth::STATUS_GOOD != userJoinedStatus && CmpSeabaseDDLauth::STATUS_NOTFOUND != userJoinedStatus)
                {
            ...
            if (TRUE == getAuthNameAndAuthID(pNode->getUserGroupList(), authNameAndID))
            {
                    ...
                    itor != authNameAndID.end();)
                {
                    ...
                    if (0 == authID)
                    {
                        *CmpCommon::diags() << DgSqlCode(-CAT_GROUP_NOT_EXIST)
                                            << DgString0(authName);
                    ...
                    {
                        *CmpCommon::diags() << DgSqlCode(-CAT_IS_NOT_A_GROUP)
                                            << DgString0(authName);
                        isErrorOccurred = true;
                        break;
                    }
                    if (joinedGroups.count(authID) == 0)
                    {
                        ...
                        {
                            *CmpCommon::diags() << DgSqlCode(-CAT_USER_IS_NOT_MEMBER_OF_GROUP)
                                                << DgString0(getAuthDbName().data())
                                                << DgString1(authName);
                            isErrorOccurred = true;
                            break;
                ...
                Int32 (*func)(Int32, Int32) =
                    (StmtDDLAlterUser::SET_USER_JOIN_GROUP == pNode->getAlterUserCmdSubType())
                ...
                std::vector<int32_t> groupIDs;
                for (std::map<const char *, int32_t, Char_Compare>::iterator itor = authNameAndID.begin(); itor != authNameAndID.end(); itor++)
                {
                    if (!isErrorOccurred)
                    {
...

{
   ...
   chkUserRslt = DBUserAuth::CheckExternalGroupnameDefined(externalGroupName,
                                                           configurationName,
                                                          foundConfigurationNumber);
   ...
   switch (chkUserRslt)
   {
      case DBUserAuth::UserDoesNotExist:
        *CmpCommon::diags() << DgSqlCode(-CAT_LDAP_AUTH_NOT_FOUND)
                          << DgString0("User group")
...
bool CmpSeabaseDDLuser::describe (const NAString &authName, NAString &authText)
{
  ...
  if (authName != ComUser::getCurrentUsername())
  {
    ...
    if (NATable::usingSentry() && !usingSentryUserApi)
    {
      ...
      if (NATable::usingLinuxGroups())
        getLinuxGroups(getAuthDbName().data(), myGroups);
      if (myGroups.size() > 0)
      {
        authText += "-- groups: ";
  ...
  authText = (COM_LDAP_AUTH == currentAuthType_)
                 ? "REGISTER USER \""
                 : "CREATE USER \"";
   ...
   try
   {
      ...
      if (!tenantName.isNull())
      {
         ...
         if (tenantInfo.getAdminRoleID() > NA_UserIdDefault)
         {
            ...
            int32_t length;
            ComUser::getAuthNameFromAuthID(tenantInfo.getAdminRoleID(),
                                           adminRoleName,
                                           MAX_DBUSERNAME_LEN,
                                           length, FALSE, CmpCommon::diags());
            *CmpCommon::diags() << DgSqlCode(-CAT_AUTH_HAS_ADMIN_ROLE)
                                << DgString0(tenantName.data())
      ...
      {
        if(!isRoleID(adminRole))
          SEABASEDDL_INTERNAL_ERROR("Unable to create role, invalid role ID detected");
...
// ----------------------------------------------------------------------------
Int32 CmpSeabaseDDLrole::createAdminRole(
    const NAString &roleName,
    const NAString &tenantName,
    const Int32 roleOwner,
   ...
   NAString &roleText)
{
  ...
  bool userIsAuthorized = ComUser::currentUserHasRole(getAuthID(), NULL/*don't update diags*/, 
                          resetRoleList);
  if (!userIsAuthorized)
    userIsAuthorized = verifyAuthority(SQLOperation::SHOW);
  ...
  if (getAuthCreator() != ComUser::getRootUserID())
  {
     ...
     ComUser::getAuthNameFromAuthID(getAuthCreator(),creatorName,
                                    sizeof(creatorName),length,
                                    TRUE, NULL /*don't update diags*/);
  ...
  if (isAuthAdmin())
  {
    ...
    if (TenantInfo::getTenantInfo(getAuthID(), tenantForRole) == 0)
    {
     ...
     // just display what getAuthNameFromAuthID returns
      ComUser::getAuthNameFromAuthID(tenantForRole.getTenantID(),
                                     tempName,
                                     sizeof(tempName),
                                     length, TRUE, NULL /*don't update diags*/);
  ...
  if (privStatus == PrivStatus::STATUS_ERROR)                                   
     SEABASEDDL_INTERNAL_ERROR("Could not fetch users/groups granted role.");
  ...
  for (size_t r = 0; r < granteeNames.size(); r++)
  {
         ...
         grantorIDs[r] != ComUser::getSystemUserID())
     {
        ...
        ComUser::getAuthNameFromAuthID(grantorIDs[r],
                                       grantorName,
                                       sizeof(grantorName),
                                       length, TRUE, NULL /*don't update diags*/);
...
// ----------------------------------------------------------------------------
Int32 CmpSeabaseDDLrole::dropAdminRole( 
  const NAString &roleName,
  const Int32 tenantID )
{
  ...
  if (role.revokeAllForGrantor(getAuthCreator(),getAuthID(),false,0,PrivDropBehavior::RESTRICT) == PrivStatus::STATUS_ERROR)
  {
...
// Set up a global try/catch loop to catch unexpected errors
   try
   {
      ...
      if (isAuthAdmin())
      {
        TenantInfo roleForTenant(STMTHEAP);
         ...
         if (cliRC != 1)
         {
           ...
           // just display what getAuthNameFromAuthID returns
           ComUser::getAuthNameFromAuthID(roleForTenant.getTenantID(),
                                          tenantName,
                                          MAX_DBUSERNAME_LEN,
                                          length, TRUE, NULL /* don't update diags*/);

           *CmpCommon::diags() << DgSqlCode(-CAT_CANNOT_DROP_ADMIN_ROLE)
                               << DgString0(roleName.data())
      ...
      if (CmpCommon::context()->isAuthorizationEnabled())
      {
         ...
         {
            *CmpCommon::diags() << DgSqlCode(-CAT_ROLE_IS_GRANTED_NO_DROP)
                                << DgString0 (existingGrantees.c_str());
            return;
         }
         ...
         if (msg_license_multitenancy_enabled())
         {
           ...
           if (rowCount > 0)
           {
              *CmpCommon::diags() << DgSqlCode(-CAT_ROLE_OWNS_RESOURCES)
                                  << DgString0 (roleName.data())
...
// Constructor
// ----------------------------------------------------------------------------
CmpSeabaseDDLtenant::CmpSeabaseDDLtenant(
   const NAString & systemCatalog,
   const NAString & MDSchema)
: CmpSeabaseDDLauth(systemCatalog,MDSchema)
...
//   STATUS_ERROR: error was returned, diags area populated
// ----------------------------------------------------------------------------
CmpSeabaseDDLauth::AuthStatus CmpSeabaseDDLtenant::getTenantDetails(
  Int32 tenantID,
  TenantInfo &tenantInfo)
{
  if (!msg_license_multitenancy_enabled())
...
//       >0 - tenantID to use
// -----------------------------------------------------------------------------   
short CmpSeabaseDDLtenant::findDetachedTenant(
  const StmtDDLTenant *pNode,
  Int32 &adminRoleID, 
  Int32 &tenantID)
{
  tenantID = 0;
  ...
  {
    *CmpCommon::diags() << DgSqlCode(-CAT_ROLE_UNAVAILABLE)
                        << DgString0(pNode->getRoleName()->data())
                        << DgString1(getAuthDbName().data());
    return -1;
  }
...
//     0 = operation successful
// -----------------------------------------------------------------------------
short CmpSeabaseDDLtenant::generateSchemaUsages (
  const Int32 &tenantID,
  ...
  TenantUsageList *&existingUsageList)
{
  ...
  char buf [1000];
  snprintf (buf, sizeof(buf), "where object_type in ('PS', 'SS') and "
            "object_owner = %d", adminRoleID);
...
// ----------------------------------------------------------------------------
CmpSeabaseDDLauth::AuthStatus 
CmpSeabaseDDLtenant::getSchemaDetails(
  ExeCliInterface *cliInterface,
  ...
  Int32 &schOwner)
{
  ...
  schUID = cmpSBD.getObjectTypeandOwner(cliInterface,catName.data(),schName.data(),
                                        objName.data(),schType,schOwner);
  if (schUID == -1)
  {
...
// ----------------------------------------------------------------------------- 
void CmpSeabaseDDLtenant::addGroups (
   const StmtDDLTenant *pNode,
   const Int32 tenantID,
   const TenantUsageList *tenantGroupList,
   TenantUsageList *&usageList)
{
  ...
  if (groupList == NULL || groupList->entries() == 0)
    return;
  ...
  for (CollIndex i = 0; i < groupList->entries(); i++)
  {
    ...
    if (groupID == NA_UserIdDefault) 
    {
      ...
      {
        *CmpCommon::diags() << DgSqlCode(-CAT_GROUP_NOT_EXIST)
                            << DgString0(group->getGroupName().data());
        UserException excp (NULL, 0);
        throw excp;
      }
    ...
    else 
    {
      if (tenantGroupList != NULL)
      {
        for (CollIndex j = 0; j < tenantGroupList->entries(); j++)
        {
          ...
          {
            *CmpCommon::diags() << DgSqlCode (-CAT_GROUP_ALREADY_DEFINED)
                                << DgString0(group->getGroupName().data())
...
// throws UserException if unsuccessful (ComDiags area is setup)
// ----------------------------------------------------------------------------- 
void CmpSeabaseDDLtenant::dropGroups (
   const StmtDDLTenant *pNode,
   TenantInfo &tenantInfo,
   TenantUsageList *&usageList)
{
  ...
  for (CollIndex i = 0; i < groupList->entries(); i++)
  {
    ...
    {
      *CmpCommon::diags() << DgSqlCode(-CAT_NOT_TENANT_USAGE)
                            << DgString0("User group")
                            << DgString1(group->getGroupName().data())
                            << DgString2(pNode->getTenantName().data());
      UserException excp (NULL, 0); 
      throw excp;
    }
  }
...
// throws UserException if unsuccessful (ComDiags area is setup)
// -----------------------------------------------------------------------------
void CmpSeabaseDDLtenant::addResources (
  const StmtDDLTenant *pNode,
  ExeCliInterface *cliInterface,
  TenantInfo &tenantInfo,
  ...
  TenantResourceUsageList *&resourceUsageList)
{
  ...
  if (tenantInfo.getUsageList()->entries() == 1)
  {
    ...
    retcode = getTenantResource(defGroup, false /*isNode*/,
                                false /*fetch usages*/, existingDefGroup);
  ...
  {
    *CmpCommon::diags() << DgSqlCode(-CAT_INVALID_TENANT_RESOURCE_GROUPS)
                        << DgString0(defGroup.data())
                        << DgString1(getAuthDbName().data());
    UserException excp (NULL, 0);
    throw excp;
  }
  ...
  if (!isReplace)
  {
      ...
      for (CollIndex i = 0; i < existingNodeList->entries(); i++)
    {
      ...
      if (!outNodeList->contains(nodeInfo->getNodeName()))
        outNodeList->orderedInsert(nodeInfo);
    }
    NADELETE(existingNodeList, TenantNodeInfoList, STMTHEAP);
  }
  ...
  for (CollIndex i = 0; i < rgroupNameList.entries(); i++)
  {
    ...
    retcode = getTenantResource(rgroupName, true /*isNode*/, 
                                true /*fetch usages*/, rgroup);
    if (retcode == -1)
    {
      UserException excp (NULL, 0);
      throw excp;
    ...
    if (retcode == 1)
    {
      *CmpCommon::diags() << DgSqlCode(-CAT_RESOURCE_GROUP_NOT_EXIST)
                          << DgString0(rgroupName.data());
      UserException excp (NULL, 0);
      throw excp;
    }
    ...
    { 
      ...
      TenantResourceUsage *resourceUsage = 
          resourceUsageList->findResource(&usageName, 
                                          getAuthID(),
                                          TenantResourceUsage::RESOURCE_USAGE_TENANT);

      // Usage found, may be part of another resource group
      if (resourceUsage)
        resourceUsage->setIsObsolete(false);

      // Usage not found, add it
      else
      {
        TenantResourceUsage *newUsage = new (STMTHEAP) TenantResourceUsage 
          (existingUsage->getUsageUID(), existingUsage->getUsageName(),
           getAuthID(), getAuthDbName(), TenantResourceUsage::RESOURCE_USAGE_TENANT);
    ...
    for (CollIndex i = 0; i < nodeList->entries(); i++)
    {
      ...
      TenantResourceUsage *existingUsage = resourceUsageList->
        findResource(&nodeName, getAuthID(), TenantResourceUsage::RESOURCE_USAGE_TENANT);
      if (!existingUsage)
      {
        TenantResourceUsage *newUsage = new (STMTHEAP) TenantResourceUsage (
           nodeInfo->getMetadataNodeID(), nodeInfo->getNodeName(),
           getAuthID(), getAuthDbName(),
...
// throws UserException if unsuccessful (ComDiags area is setup)
// -----------------------------------------------------------------------------
void CmpSeabaseDDLtenant::alterResources (
  ExeCliInterface *cliInterface,
  TenantInfo &tenantInfo,
  TenantNodeInfoList *&outNodeList,
  TenantResourceUsageList *&resourceUsageList)
{
  ...
  TenantResourceUsageList *updatedUsageList =
    new (STMTHEAP) TenantResourceUsageList(cliInterface, STMTHEAP);
  ...
  for (CollIndex i = 0; i < nodeList->entries(); i++)
  {
    ...
    TenantNodeInfo *nodeInfo = (*nodeList)[i];
    NAString nodeName = nodeInfo->getNodeName();
    if (!outNodeList->contains(nodeName))
      outNodeList->orderedInsert(nodeInfo);

    // If node is new, add it to the resourceUsageList
    TenantResourceUsage *rUsage = 
          resourceUsageList->findResource(&nodeName, getAuthID(),
                                         TenantResourceUsage::RESOURCE_USAGE_TENANT);
    if (rUsage)
    ...
    {
      TenantResourceUsage *newUsage = new (STMTHEAP) TenantResourceUsage
        (nodeInfo->getMetadataNodeID(), nodeName,
         getAuthID(), getAuthDbName(), TenantResourceUsage::RESOURCE_USAGE_TENANT);
      newUsage->setIsNew(true);
      resourceUsageList->insert(newUsage);
    }
...
// throws UserException if unsuccessful (ComDiags area is setup)
// -----------------------------------------------------------------------------
void CmpSeabaseDDLtenant::dropResources (
  const StmtDDLTenant *pNode,
  ExeCliInterface *cliInterface,
  TenantInfo &tenantInfo,
  TenantNodeInfoList *&outNodeList,
  ...
  TenantResourceUsageList *&resourceUsageList)
{
  ...
  for (CollIndex i = 0; i < rgroupInputList->entries(); i++)
  {
    ...
    ElemDDLTenantResourceGroup *rgroupInput = 
        (*rgroupInputList)[i]->castToElemDDLTenantResourceGroup();
    ...
    {
      *CmpCommon::diags() << DgSqlCode(-CAT_RESOURCE_GROUP_NOT_ASSIGNED)
                          << DgString0(rgroupName.data())
                          << DgString1(getAuthDbName().data());
      UserException excp (NULL, 0);
      throw excp;
  ...
  if (numRGroups == 0)
  {
      *CmpCommon::diags() << DgSqlCode(-CAT_LAST_RESOURCE_GROUP)
                          << DgString0(getAuthDbName().data());
      UserException excp (NULL, 0);
      throw excp;
  }
  ...
  for (CollIndex i = 0; i < resourceUsageList->entries(); i++)
  {
    ...
    if (nodeList->contains(rUsage->getResourceName()))
      rUsage->setUsageValue(-1);
...
// ----------------------------------------------------------------------------
short CmpSeabaseDDLtenant::getNodeDetails (
  ExeCliInterface *cliInterface,
  TenantNodeInfoList *& nodesToAdd)
{
  Int32 numNodes = CURRCONTEXT_CLUSTERINFO->hasVirtualNodes() ? 1 : 
      CURRCONTEXT_CLUSTERINFO->getTotalNumberOfCPUs();
  ...
  for (Int32 i = 0; i < numNodes; i++)
  {
    ...
    if (node.generateNodeResource(&nodeName, ComUser::getCurrentUser()) == -1)
      return -1;

    TenantNodeInfo *newNodeInfo = new (STMTHEAP)
        TenantNodeInfo(nodeName, nodeID, -1, node.getResourceUID(), -1, STMTHEAP);
...
// ----------------------------------------------------------------------------
short CmpSeabaseDDLtenant::getNodeDetails (
  const TenantResourceUsageList *nodesInRGroup,
  TenantNodeInfoList *& nodesToAdd)
{
  ...
  for (CollIndex i = 0; i < nodesInRGroup->entries(); i++)
  {
    ...
    if (useTempNodes)
    {
      ...
      if (nodeID == -1)
      {
        *CmpCommon::diags() << DgSqlCode(-CAT_NODE_NAME_INVALID)
    ...
    {
      TenantNodeInfo *newNodeInfo = new (STMTHEAP)
        TenantNodeInfo(nodeName, nodeID, -1, usage->getUsageUID(), usage->getUsageValue(), STMTHEAP);
...
// -----------------------------------------------------------------------------
short CmpSeabaseDDLtenant::getTenantResource (
  const NAString &resourceName,
  const bool isNodeResource,
  const bool &fetchUsages,
...
// throws UserException if unsuccessful (ComDiags area is setup)
// ----------------------------------------------------------------------------- 
void CmpSeabaseDDLtenant::createSchemas (
  ExeCliInterface *cliInterface,
  const StmtDDLTenant * pNode,
  const Int32 &tenantID,
  const TenantUsageList *origUsageList,
  NAString schemaOwner,
  Int64 &defSchUID,
  TenantUsageList *&usageList)
{
  ...
  if (pNode->getSchemaList() && pNode->getSchemaList()->entries() > 0)
  {
    ...
    for (Int32 i = 0; i < pNode->getSchemaList()->entries(); i++)
    {
      ...
      snprintf (buf, sizeof(buf), "create private schema %s authorization \"%s\" "
                " namespace '%d'", schemaName->getSchemaNameAsAnsiString().data(), 
                schemaOwner.data(), tenantID);
      ...
      if (defSchemaName &&
        defSchemaName->getSchemaNameAsAnsiString() == schemaName->getSchemaNameAsAnsiString())
        defSchUID = schUID;
    }
  }
...
// throws UserException if unsuccessful (ComDiags area is setup)
// ----------------------------------------------------------------------------- 
void CmpSeabaseDDLtenant::dropSchemas (
   ExeCliInterface *cliInterface,
   const Int32 tenantID,
...
NAString CmpSeabaseDDLtenant::getNodeName(const Int64 nodeID)
{
  ...
  if (retcode != 0 || usageList.entries() == 0)
    return NAString("?"); 
...
NAString  CmpSeabaseDDLtenant::getSchemaName(const Int64 schUID)
{
  ...
  {
    ComObjectName extName((const NAString &)catName,(const NAString &)schName,
                         (const NAString &)objName,COM_UNKNOWN_NAME, 
...
//     >0 - schemaUID
// -----------------------------------------------------------------------------
Int64 CmpSeabaseDDLauth::getSchemaUID(
  ExeCliInterface *cliInterface,
  const SchemaName *schemaName)
{
  ...
  Int32 stmtSize = snprintf(selectStmt, sizeof(selectStmt),
                            ...
                            "and object_name = '%s'",
                            CmpSeabaseDDL::getSystemCatalogStatic().data(),
                            SEABASE_MD_SCHEMA, SEABASE_OBJECTS,
                            schemaName->getCatalogNameAsAnsiString().data(),
                            schemaName->getSchemaName().data(),
...
// -----------------------------------------------------------------------------   
Int32 CmpSeabaseDDLtenant::getTenantSchemas(
  ExeCliInterface *cliInterface,
  NAList<Int64> *&schemaUIDList)
{
...
// Output: the global diags area is set up with the result
// ----------------------------------------------------------------------------
void CmpSeabaseDDLtenant::alterTenant(
  ExeCliInterface *cliInterface,
  StmtDDLTenant * pNode)
{
  Int32 retcode;
  ...
  if (!msg_license_multitenancy_enabled())
  {
    *CmpCommon::diags() << DgSqlCode(-3242)
                        << DgString0("Resource group feature is not enabled.");
    return;
  }
  ...
  {
    *CmpCommon::diags() << DgSqlCode (-CAT_AUTH_NAME_RESERVED)
                        << DgString0(pNode->getTenantName().data());
     return;
  }
  ...
  switch (pNode->getAlterType())
  {
    case StmtDDLTenant::ALTER_TENANT_OPTIONS:
    {
      ...
      if (pNode->getDefaultSchema() && pNode->isDefaultSchemaOpt() &&
          pNode->getSchemaList() == NULL)
      {
        Int64 defSchUID = 0;
        const SchemaName *defaultSchema = pNode->getDefaultSchema();
        int32_t schOwnerID;
        ComObjectType schType;
        if (getSchemaDetails(cliInterface, defaultSchema, defSchUID, schType, schOwnerID) != STATUS_GOOD)
          return;
        if (defSchUID > 0)
        {
          ...
          if (schOwnerID != tenantInfo.getAdminRoleID())
          {
            ...
            NAString privMgrMDLoc;
            CONCAT_CATSCH(privMgrMDLoc,systemCatalog_.data(),SEABASE_PRIVMGR_SCHEMA);
            PrivMgrSchemaPrivileges schemaPrivs(std::string(privMgrMDLoc.data()),
                                                CmpCommon::diags());
            ...
            if (std::find(schemaList.begin(), schemaList.end(), defSchUID) == schemaList.end())
            {
              ...
              // just display what getAuthNameFromAuthID returns
              retcode = ComUser::getAuthNameFromAuthID(tenantInfo.getAdminRoleID(),
                                                       roleName,
                                                       MAX_DBUSERNAME_LEN,
                                                       length, NULL /* don't update diags*/);

              *CmpCommon::diags() << DgSqlCode(-CAT_TENANT_DEFAULT_SCHEMA_INVALID )
                                  << DgString0 (defaultSchema->getSchemaName().data())
                                  << DgString1(dbTenantName.data())
                                  << DgString2(roleName); 
      ...
      {
        *CmpCommon::diags() << DgSqlCode(-CAT_RESERVED_TENANT)
                            << DgString0(dbTenantName.data());
        return;
      }
      ...
      if (pNode->getRoleName())
      {
        ...
        Int32 adminRoleID =  adminRole.createAdminRole(*pNode->getRoleName(),
                                                       pNode->getTenantName(),
      ...
      if (pNode->getSchemaList() && pNode->addSchemaList())
      {
        ...
        if (usageList == NULL)
          usageList = new (STMTHEAP) TenantUsageList(STMTHEAP);
        ...
        try
        {
          createSchemas(cliInterface, pNode, tenantInfo.getTenantID(), 
                        tenantInfo.getUsageList(), NAString(roleName), defSchUID, 
                        usageList);
      ...
      if (pNode->getSchemaList() && !pNode->addSchemaList())
      {
        ...
        catch (...)
        {
          ...
          if (schemaList)
            NADELETE(schemaList, NAList<NAString>, STMTHEAP);
      ...
      if (pNode->isZookeeperUpdate())
      {
        ...
        if (AStenant && 
             (pNode->addRGroupList() || 
              pNode->dropRGroupList() || 
              pNode->replaceRGroupList()))
        {
          *CmpCommon::diags() << DgSqlCode(-CAT_MISMATCH_AS_RG_TENANTS )
                              << DgString0 (dbTenantName.data())
                              << DgString1("adaptive segmentation")
                              << DgString2("resource groups");
          return;
        }
        ...
        if (pNode->isAffinitySizing() && !AStenant)
        {
          *CmpCommon::diags() << DgSqlCode(-CAT_MISMATCH_AS_RG_TENANTS )
                              << DgString0 (dbTenantName.data())
                              << DgString1("resource groups")
                              << DgString2("adaptive segmentation");
          return;
        }
        ...
        try
        {
          ...
          tenantSize = (pNode->isTenantSizeSpecified()) ? 
            pNode->getTenantSize() : origTenantSize;

          if (AStenant)
          {
            // Get updated values for affinity and cluster size
            affinity = (pNode->getAffinity() >= 0) ?
              pNode->getAffinity() : origAffinity;

            clusterSize = (pNode->getClusterSize() >= 0) ?
              pNode->getClusterSize() : origClusterSize;

            // Create NAASNodes, one with original values, one with new values
            tenantInfo.allocTenantNode(AStenant, origAffinity, origClusterSize,
                                       origTenantSize, origNodeList,
                                       origTenantNodeSet, false);
            tenantInfo.setOrigTenantNodes(origTenantNodeSet);

            // allocate the NAASNodes based on updated attributes
            tenantInfo.allocTenantNode(AStenant, affinity, clusterSize,
                                       tenantSize, nodeList, tenantNodeSet);
          ...
          else //RGtenant
          {
            ...
            tenantInfo.allocTenantNode(AStenant, affinity, clusterSize,
                                       origTenantSize, origNodeList, 
                                       origTenantNodeSet, true);
            tenantInfo.setOrigTenantNodes(origTenantNodeSet);

            // Reset allocated units; for replace, obsolete each entry
            if (pNode->doNodeBalance())
              resourceUsageList->resetValue(pNode->replaceRGroupList());
          ...
          else
          {
            ...
            tenantInfo.balanceTenantNodeAlloc(tenantInfo.getDefaultSchemaName(), 
                                              pNode->getSessionLimit(), false,
                                              origTenantNodeSet, 
                                              tenantNodeSet,
        ...
        catch (...)
        {
          ...
          if (registrationComplete)
          {
             try
             {
               ...
               tenantInfo.balanceTenantNodeAlloc(origDefaultSchema,
                                                  -2, /* use existing session limit */
                                                  true, /*backing out a previous balance op */
                                                  tenantNodeSet, origTenantNodeSet,
                                                  resourceUsageList, registrationComplete);
             }
             catch (...)
             {
          ...
          if (origNodeList)
            NADELETE(origNodeList, TenantNodeInfoList, STMTHEAP);

          if (resourceUsageList)
...
// -----------------------------------------------------------------------------
Int32 CmpSeabaseDDLtenant::getNodesForRGroups (
  const TenantUsageList *usageList,
  TenantResourceUsageList *& nodeList)
{
  assert (nodeList);
  char buf[200];
  ...
  if (nodeList->fetchUsages(whereClause, orderByClause) == -1)
    return -1;
  return 0;
}
...
//
// ----------------------------------------------------------------------------
Int64 CmpSeabaseDDLtenant::getDefSchemaUID(
  TenantInfo &tenantInfo,
  const TenantUsageList  *dropUsageList)
{
  ...
  if (defSchUID > 0 && !dropUsageList->contains(droppedUsage))
    return defSchUID;
...
//    0 - succeeded
// ----------------------------------------------------------------------------
Int32 CmpSeabaseDDLtenant::registerStandardTenant(
  const std::string tenantName,
  const int32_t tenantID)
{
  setAuthType(COM_TENANT_CLASS);  // we are a tenant
  if (!createStandardAuth(tenantName, tenantID))
    return -1;
...
// ----------------------------------------------------------------------------
Int32 CmpSeabaseDDLtenant::registerTenantPrepare(
  StmtDDLTenant * pNode,
  Int32 &adminRoleID)
{
  ...
  {
    *CmpCommon::diags() << DgSqlCode(-3242)
                        << DgString0("User group feature is not enabled.");
    return -1;
  }
  ...
  {
    *CmpCommon::diags() << DgSqlCode(-CAT_INVALID_ADMIN_ROLE)
                        << DgString0(getAuthDbName().data());
    return -1;
  }
  ...
  adminRoleID = 0;
  Int32 tenantID = 0;
  if (findDetachedTenant (pNode, adminRoleID, tenantID) == -1)
    return -1;

  // tenantID is zero, then no detached tenant exists, go generate an ID
  if (tenantID == 0)
    if (generateTenantID(tenantID) == -1)
      return -1;
  assert (tenantID > 0);
...
//   -1 = failed (ComDiags contains error)
// ----------------------------------------------------------------------------
Int32 CmpSeabaseDDLtenant::registerTenant(
  ExeCliInterface *cliInterface,
  StmtDDLTenant * pNode,
  Int32 adminRoleID)
{
  ...
  try
  {
    ...
    if (adminRoleID == 0)
    {
      ...
      adminRoleID =  roleInfo.createAdminRole(*pNode->getRoleName(),
                                               pNode->getTenantName(),
                                               ComUser::getCurrentUser(),
                                               ComUser::getCurrentUsername());
      if (adminRoleID <= 0)
    ...
    else
    {
      if (isDetached())
      {
        ...
        PrivMgrRoles role;
        PrivStatus privStatus = role.revokeRoleFromCreator(adminRoleID,
                                                           getAuthID(),
                                                           false /*remove tenantID only*/);
        if (privStatus == PrivStatus:: STATUS_ERROR)
        {
          UserException excp (NULL, 0);
          throw excp;
    ...
    {
      createSchemas(cliInterface, pNode, getAuthID(), existingUsageList,
                    (NAString )*pNode->getRoleName(), defSchUID, usageList);
    }

    if (existingUsageList)
      NADELETE(existingUsageList, TenantUsageList, STMTHEAP);
    ...
    if (!pNode->getDefaultSchema() && defSchUID == 0)
      defSchUID = usageList->possibleDefSchUID();
    ...
    tenantSize = (pNode->getTenantSize() <= 0 ? 
       CmpCommon::getDefaultLong(TENANT_DEFAULT_SIZE) : pNode->getTenantSize());

    clusterSize = (pNode->getClusterSize() == 0) ?
       CURRCONTEXT_CLUSTERINFO->getTotalNumberOfCPUs() : pNode->getClusterSize();
    ...
    TenantInfo tenantInfo(getAuthID(), adminRoleID, defSchUID, usageList, 
                          tenantNodeSet, pNode->getTenantName(), STMTHEAP);
    ...
    tenantInfo.allocTenantNode(AStenant, pNode->getAffinity(), clusterSize, 
                               tenantSize, nodeList, tenantNodeSet);
    tenantInfo.setTenantNodes(tenantNodeSet);
    ...
    else
    {
      ...
      tenantInfo.balanceTenantNodeAlloc(tenantDefaultSchema, pNode->getSessionLimit(), 
                                        false, NULL, tenantNodeSet, resourceUsageList,
...
// Output: the global diags area is set up with the result
// ----------------------------------------------------------------------------
Int32 CmpSeabaseDDLtenant::unregisterTenant(
  ExeCliInterface *cliInterface,
  StmtDDLTenant * pNode)
{
  Int32 cliRC;
  ...
  if (tenantInfo.getAdminRoleID() > NA_UserIdDefault) 
  {
    ...
    // If adminRoleID not found in metadata, ComDiags is populated with 
    // error 8732: Authorization ID <adminRoleID> is not a registered user or role
    cliRC = ComUser::getAuthNameFromAuthID(tenantInfo.getAdminRoleID(),
                                           roleName,
                                           MAX_DBUSERNAME_LEN,
                                           length,
  ...
  if (pNode->dropDependencies())
  {
    ...
    if (role.dropAdminRole(NAString(roleName), tenantInfo.getTenantID()) == -1)
      return -1; 
  }
  ...
  else
  {
    ...
    PrivMgrRoles roles;
    PrivStatus privStatus = roles.grantRoleToCreator(tenantInfo.getAdminRoleID(),
                                                     std::string(roleName),
                                                     SYSTEM_USER, SYSTEM_AUTH_NAME,
                                                     tenantInfo.getTenantID(),
                                                     std::string(tenantInfo.getTenantName()));
    if (privStatus != PrivStatus::STATUS_GOOD)
       return -1;
  }
  ...
  if (usageList.deleteUsages(whereClause) == -1)
    return -1;

  // drop tenant details
  if (tenantInfo.dropTenantInfo() != 0)
    return -1;
...
//   -1 = failed (ComDiags contains error)
// ----------------------------------------------------------------------------
Int32 CmpSeabaseDDLtenant::unregisterTenantPrepare(
  StmtDDLTenant * pNode)
{
  if (!verifyAuthority(SQLOperation::MANAGE_TENANTS))
  {
...
bool CmpSeabaseDDLtenant::describe (const NAString &tenantName, NAString &tenantText)
{
  ...
  try
  {
   ...
   if (!isTenant())
   {
    ...
    ComUser::getAuthNameFromAuthID(getAuthCreator(),
                                   nameStr,
                                   MAX_DBUSERNAME_LEN,
                                   length, TRUE, NULL /*don't update diags*/);
    ...
    if (PrivMgr::isDelimited(tenantName))
      tenantText += '"';
    ...
    {
      snprintf(intStr, sizeof(intStr), ", AFFINITY %d, CLUSTER SIZE %d",
               tenantInfo.getAffinity(), tenantInfo.getClusterSize());
      tenantText += intStr;
    }
    ...
    if (tenantInfo.getUsageList())
    {
      ...
      if (groupText.length() > 0)
        tenantText += groupText + NAString(")");

      if (schemaText.length() > 0)
        tenantText += schemaText + NAString(")");

      if (rgroupText.length() > 0)
        tenantText += rgroupText + NAString(")");
    }
    tenantText += ";\n";
    ...
    if (rgroupText.length() > 0)
    {
      ...
      { 
        if (usageList.entries() > 0)
          tenantText += "-- Tenant node assignments: ";
        for (int i = 0; i < usageList.entries(); i++)
        {
          TenantResourceUsage *usage = (usageList)[i];

          // skip nodes with unallocated units
          if (usage->getUsageValue() <= 0)
            continue;

          if (i > 0)
            snprintf(nameStr, sizeof(nameStr), ", node '%s' (%ld) units",
                    usage->getResourceName().data(), usage->getUsageValue());
          else
            snprintf(nameStr, sizeof(nameStr), " node '%s' (%ld) units",
                    usage->getResourceName().data(), usage->getUsageValue());
          tenantText += nameStr;
...
Int32 CmpSeabaseDDLtenant::predefinedNodeAllocations (
  const Int32 unitsToAllocate,
  TenantResourceUsageList *&resourceUsageList)
{
...
// Constructor
// ----------------------------------------------------------------------------
CmpSeabaseDDLgroup::CmpSeabaseDDLgroup(
   const NAString & systemCatalog,
   const NAString & MDSchema)
: CmpSeabaseDDLauth(systemCatalog,MDSchema)
{}
...
void CmpSeabaseDDLgroup::alterGroup(StmtDDLUserGroup *pNode)
{
  ...
  if (COM_LDAP_AUTH == currentAuthType_)
  {
    ...
    if (!validateExternalGroupname(pNode->getGroupName().data(),
                                    configSectionName,
                                    foundConfigurationNumber))
        return;
  ...
  case StmtDDLUserGroup::REMOVE_USER_GROUP_MEMBER:
  {
      ...
      if (TRUE == getAuthNameAndAuthID(pNode->getMembers(), authNameAndID))
      {
          ...
          Int32 (*func)(Int32, Int32) =
              (StmtDDLUserGroup::ADD_USER_GROUP_MEMBER == pNode->getUserGroupType())
                  ? &CmpSeabaseDDLauth::addGroupMember
                  : &CmpSeabaseDDLauth::removeGroupMember;
...
//
// ----------------------------------------------------------------------------
Int32 CmpSeabaseDDLgroup::registerGroupInternal(
  const NAString &groupName,
  const NAString &config,
  bool isAutoCreated)
{
  // Verify that the name does not include unsupported special characters
  ...
  if (COM_LDAP_AUTH == currentAuthType_)
  {
    ...
    const char * configSectionName =
        config.isNull() ? NULL :
          ...
          if (!validateExternalGroupname(groupName.data(),
                                         configSectionName,
                                         foundConfigurationNumber))
              return -1;
      if (foundConfigurationNumber == DBUserAuth::DefaultConfiguration)
...
// ----------------------------------------------------------------------------
CmpSeabaseDDLauth::AuthStatus 
CmpSeabaseDDLgroup::getGroupDetails(const char *pGroupName)
{
...
void CmpSeabaseDDLgroup::removeAllGroupMember()
{
    ...
    str_sprintf(str, "delete from %s.\"%s\".%s where SUB_ID = %d and text_type = %d",
                sysCat.data(), SEABASE_MD_SCHEMA, SEABASE_TEXT, getAuthID(), COM_USER_GROUP_RELATIONSHIP);
...
void CmpSeabaseDDLuser::exitAllJoinedUserGroup()
{
    ...
    str_sprintf(str, "delete from %s.\"%s\".%s where TEXT_UID = %d and text_type = %d",
                sysCat.data(),
                SEABASE_MD_SCHEMA, SEABASE_TEXT,
                getAuthID(), COM_USER_GROUP_RELATIONSHIP);
...
Int32 CmpSeabaseDDLauth::addGroupMember(Int32 groupid, Int32 userid)
{
    ...
    str_sprintf(str, "upsert into %s.\"%s\".%s values(%d,%d,%d,0,0,0)",
                sysCat.data(), SEABASE_MD_SCHEMA, SEABASE_TEXT,
...
Int32 CmpSeabaseDDLauth::removeGroupMember(Int32 groupid, Int32 userid)
{
    ...
    str_sprintf(str, "delete from %s.\"%s\".%s where TEXT_UID = %d and SUB_ID = %d and text_type = %d",
                sysCat.data(),
                SEABASE_MD_SCHEMA, SEABASE_TEXT,
                userid, groupid, COM_USER_GROUP_RELATIONSHIP);
...
Int32 CmpSeabaseDDLauth::getUserGroupMembers(const char *groupName, std::vector<int32_t> &memberIDs)
{
    ...
    if (STATUS_GOOD != retcode || !isUserGroupID(authInfo.getAuthID()))
        return -1;

    retcode = authInfo.getUserGroupMembers(memberIDs);
    if ((retcode != STATUS_GOOD) && (retcode != STATUS_NOTFOUND))
        return -1;
    return 0;
}
...
CmpSeabaseDDLauth::AuthStatus CmpSeabaseDDLauth::getUserGroupMembers(std::vector<int32_t> &memberIDs)
{
    ...
    case COM_LDAP_AUTH:
    {
        ...
        if (DBUserAuth::GroupListFound == ldapConn->getMembersOnLDAPGroup(
                                              NULL, configurationNumber, getAuthDbName(), foundConfigurationNumber, members_))
        {
            ...
            for (std::vector<char *>::iterator itor = members_.begin(); itor != members_.end(); itor = members_.erase(itor))
            {
    ...
    default:
        {
            ...
            ExeCliInterface cliInterface(STMTHEAP);
            NAString sysCat = CmpSeabaseDDL::getSystemCatalogStatic();
            str_sprintf(buf, "select distinct TEXT_UID from %s.\"%s\".%s where SUB_ID = %d and text_type = %d",
                        sysCat.data(), SEABASE_MD_SCHEMA, SEABASE_TEXT, this->getAuthID(),
                        COM_USER_GROUP_RELATIONSHIP);
            Queue *cntQueue = NULL;
            Int32 cliRC = cliInterface.fetchAllRows(cntQueue, buf, 0, false, false, true);
            if (cliRC < 0)
            {
...
//for local auth
NABoolean CmpSeabaseDDLauth::getAuthNameAndAuthID(ElemDDLList *nameList, std::map<const char *, int32_t, Char_Compare> &result)
{
    //AUTH_DB_NAME
    NAString whereClause = "where AUTH_DB_NAME in (";
    result.clear();
...
//for ldap auth
NABoolean CmpSeabaseDDLauth::getAuthNameAndAuthID(std::vector<char *> &nameList, std::map<const char *, int32_t, Char_Compare> &result)
{
    //AUTH_EXT_NAME
...
CmpSeabaseDDLauth::AuthStatus CmpSeabaseDDLuser::getAllJoinedUserGroup(std::vector<int32_t> &groupIDs)
{
    ...
    default:
    {
        ...
        ExeCliInterface cliInterface(STMTHEAP);
        NAString sysCat = CmpSeabaseDDL::getSystemCatalogStatic();
        str_sprintf(buf, "select distinct SUB_ID from %s.\"%s\".%s where TEXT_UID= %d and text_type = %d",
                    sysCat.data(), SEABASE_MD_SCHEMA, SEABASE_TEXT, this->getAuthID(), COM_USER_GROUP_RELATIONSHIP);
        Queue *cntQueue = NULL;
        Int32 cliRC = cliInterface.fetchAllRows(cntQueue, buf, 0, false, false, true);
        if (cliRC < 0)
        {
...
void CmpSeabaseDDLuser::reflashPasswordModTime()
{
    ...
    ExeCliInterface cliInterface(STMTHEAP);
    NAString sysCat = CmpSeabaseDDL::getSystemCatalogStatic();
    str_sprintf(buf, "update  %s.\"%s\".%s set flags = %ld where text_uid = %d and text_type = %d",
                sysCat.data(), SEABASE_MD_SCHEMA, SEABASE_TEXT,
                getPasswdModTime(), getAuthID(), COM_USER_PASSWORD);
    Int32 cliRC = cliInterface.executeImmediate(buf);
    if (cliRC < 0)
    {
        cliInterface.retrieveSQLDiagnostics(CmpCommon::diags());
...
NAString CmpSeabaseDDLuser::getDefAuthInfo(Int32 uid)
{
    ...
    {
        int cryptlen = 0;
        if(ComEncryption::encrypt_ConvertBase64_Data(ComEncryption::AES_128_CBC,
                            (unsigned char *)(AUTH_DEFAULT_WORD),
                            strlen(AUTH_DEFAULT_WORD),
                            (unsigned char *)AUTH_PASSWORD_KEY,
                            (unsigned char *)AUTH_PASSWORD_IV,
    ...
    str_sprintf(buf, "insert into %s.\"%s\".%s values (%d, %d, 0, 0, %ld, '%s')",
                sysCat.data(), SEABASE_MD_SCHEMA, SEABASE_TEXT,
                uid, COM_USER_PASSWORD, NA_JulianTimestamp(), dePassword);
END:
    return NAString(buf);
}
...
void CmpSeabaseDDLauth::initAuthPwd(ExeCliInterface *cliInterface)
{
    ...
    char buf[1024] = {0};
    NAString sysCat = CmpSeabaseDDL::getSystemCatalogStatic();
    str_sprintf(buf, "select AUTH_ID from %s.\"%s\".%s where AUTH_TYPE = '%s' except select TEXT_UID from %s.\"%s\".%s where TEXT_TYPE = %d",
                sysCat.data(), SEABASE_MD_SCHEMA, SEABASE_AUTHS, COM_USER_CLASS_LIT,
                sysCat.data(), SEABASE_MD_SCHEMA, SEABASE_TEXT, COM_USER_PASSWORD);
    Int32 cliRC = cliInterface->fetchAllRows(cntQueue, buf, 0, false, false, true);
    if (cliRC < 0)
    {
        cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
...
UPDATE_MAX_USERID:
    //update max userID in used
    {
        ...
        char buf[1024] = {0};
        str_sprintf(buf, "update %s.\"%s\".%s set SUB_ID = %d where TEXT_UID = %d and TEXT_TYPE = %d",
                    sysCat.data(), SEABASE_MD_SCHEMA, SEABASE_TEXT, maxUserID, ROOT_USER_ID, COM_USER_PASSWORD);
        cliRC = cliInterface->executeImmediate(buf);
        if (cliRC < 0)
        {
            cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
...
void CmpSeabaseDDLauth::getAuthDbNameByAuthIDs(std::map<int32_t, std::string *> &authMap)
{
    ...
    char buf[stmtSize];
    //get id and name
    str_sprintf(buf, "select distinct auth_id,auth_db_name from %s.\"%s\".%s where auth_id in %s",
                sysCat.data(), SEABASE_MD_SCHEMA, SEABASE_AUTHS, authIDs.c_str());
