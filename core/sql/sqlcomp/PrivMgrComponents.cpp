...
class MyRow : public PrivMgrMDRow {
 public:
  ...
  // -------------------------------------------------------------------
  MyRow(std::string tableName) : PrivMgrMDRow(tableName, COMPONENTS_ENUM), componentUID_(0){};
  ...
  bool lookupByName(const std::string &componentName, int64_t &componentUID, bool &isSystem,
...
class MyTable : public PrivMgrMDTable {
 public:
  MyTable(const std::string &tableName, ComDiagsArea *pDiags)
      : PrivMgrMDTable(tableName, COMPONENTS_ENUM, pDiags), lastRowRead_(tableName){};
  ...
  PrivStatus fetchByName(const std::string &componentName, MyRow &myRow);
...
// -----------------------------------------------------------------------
PrivMgrComponents::PrivMgrComponents(const std::string &metadataLocation, ComDiagsArea *pDiags)
...
// Destructor.
// -----------------------------------------------------------------------
PrivMgrComponents::~PrivMgrComponents() { delete &myTable_; }
...
// *                                                                           *
// *****************************************************************************
PrivStatus PrivMgrComponents::describeComponents(const std::string &componentName, std::string &componentUIDString,
...

{
  ...
  if (privStatus != STATUS_GOOD) {
    if (pDiags_->getNumber(DgSqlCode::ERROR_) == 0) PRIVMGR_INTERNAL_ERROR("Unable to drop operations for component");
    return STATUS_ERROR;
  }
...
// *                                                                           *
// *****************************************************************************
PrivStatus PrivMgrComponents::fetchByName(const std::string &componentName, std::string &componentUIDString,
                                          int64_t &componentUID, bool &isSystem, std::string &componentDescription)
...
int64_t PrivMgrComponents::getUniqueID() {
  ...
  // argument is NOT NULLABLE, not quite sure why or when NVL becomes
  // NULLABLE for this case. The usage of executeImmediate below
  // requires the type of the value retrieived to be NOT NULLABLE,
  // has to workaround this problem by casting the type to NOT NULL.
  strcpy(stmtBuf, "SELECT CAST (NVL(MAX(component_uid),0) AS LARGEINT NOT NULL) FROM ");
  ...
  // Component UID's start at USER_COMPONENT_START_UID
  // System component UID's are less than USER_COMPONENT_START_UID
  if (maxValue < USER_COMPONENT_START_UID) return USER_COMPONENT_START_UID;
  return ++maxValue;
}
//******************** End of PrivMgrComponents::getUniqueID *******************

bool PrivMgrComponents::isReservedComponent(const std::string &componentName) {
  ...
  for (int c = 0; c < numComps; c++) {
    ...
    std::string reservedName(compDefinition.componentName);
    if (componentName == reservedName) return true;
  }
  return false;
}
...
// *                                                                           *
// *****************************************************************************
PrivStatus PrivMgrComponents::registerComponent(const std::string &componentName, const bool isSystem,
                                                const std::string &componentDescription, const bool existsErrorOK)

{
  ...
  if (exists(componentName)) {
    ...
    *pDiags_ << DgSqlCode(-CAT_TABLE_ALREADY_EXISTS) << DgTableName(componentName.c_str());
    return STATUS_ERROR;
  }
...
// *                                                                           *
// *****************************************************************************
PrivStatus PrivMgrComponents::registerComponentInternal(const std::string &componentName, const int64_t componentUID,
...
// *                                                                           *
// *****************************************************************************
PrivStatus PrivMgrComponents::unregisterComponent(const std::string &componentName, PrivDropBehavior dropBehavior)

{
  ...
  if (isReservedComponent(componentName)) {
    *pDiags_ << DgSqlCode(-CAT_NOT_ALLOWED_FOR_SYSTEM_OBJECT) << DgString0("UNREGISTER")
...
// *                                                                           *
// *****************************************************************************
bool MyRow::lookupByName(const std::string &componentName, int64_t &componentUID, bool &isSystem,
                         std::string &componentDescription)

{
  // If componentUID_ is zero, that means data is uninitialized.

  if (componentUID_ == 0 || componentName != componentName_) return false;
...
// *                                                                           *
// *****************************************************************************
PrivStatus MyTable::fetchByName(const std::string &componentName, MyRow &row)

{
  // Check the last row read before reading metadata.
  if (lastRowRead_.lookupByName(componentName, row.componentUID_, row.isSystem_, row.componentDescription_))
    return STATUS_GOOD;
...
PrivStatus MyTable::insert(const PrivMgrMDRow &rowIn) {
  ...
  sprintf(insertStatement, "insert into %s values (%ld, '%s', '%s', '%s')", tableName_.c_str(), row.componentUID_,
          row.componentName_.c_str(), isSystem, row.componentDescription_.c_str());
