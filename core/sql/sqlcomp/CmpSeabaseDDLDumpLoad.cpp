 ...
 *****************************************************************************
 *
 * File:         CmpSeabaseDumpLoad.cpp
 * Description:  Implements  dump objects to file 
 *
 *
 * Created:    
 * Language:     C++
...
{
  {RelDumpLoad::DL_OBJ_SCHEMA,  "SHOWDDL SCHEMA   "},
  {RelDumpLoad::DL_OBJ_TABLE,   "SHOWDDL TABLE    "},
  {RelDumpLoad::DL_OBJ_LIBRARY, "SHOWDDL LIBRARY  "},
  {RelDumpLoad::DL_OBJ_VIEW,    "SHOWDDL          "},
  {RelDumpLoad::DL_OBJ_UDRO,    "SHOWDDL FUNCTION "},
  {RelDumpLoad::DL_OBJ_SGO,     "SHOWDDL SEQUENCE "},
  {RelDumpLoad::DL_OBJ_PROCEDURE,     "SHOWDDL PROCEDURE "},
  {RelDumpLoad::DL_OBJ_TRIGGER,     "SHOWDDL TRIGGER "},
  {RelDumpLoad::DL_OBJ_PACKAGE,     "SHOWDDL PACKAGE "},
  {RelDumpLoad::DL_OBJ_UNKNOWN, "SHOWDDL UNKNOWN  "}
};
  ...
  if ((rval) != 0 && (error != EEXIST)) //EEXIST is meaning 'File already exists'.
    {
      ...
      switch (error)
        {
          ...
          case ENOENT:
            snprintf(errMessage
                     , sizeof(errMessage)
                     , "Coule not create directory %s, component of the path does not exist."
                     , localDir.data());
            break;
          case EROFS:
            snprintf(errMessage
      ...
      *CmpCommon::diags() << DgSqlCode(-1110)
                          << DgString0(errMessage);
      return -1;
    }
  return 0; 
}
...
static void checkAndRemoveUnnecessaryFields(RelDumpLoad *ddlExpr, char *originalStr)
{
  ...
  const char *unnecessaryFields[] = {
  /*The result of 'SHOWDDL FUNCTION' syntax include this,
  ...
  for (Int32 i = 0;;i++)
    {
      ...
      if (pos != NULL)
      {
        ...
        str_cpy(pos, pos + keyStringLength
            , originalLength - (pos-originalStr) - keyStringLength); 
...
static short dumpObjectsDDL(ExeCliInterface *cliInterface
                            , RelDumpLoad *ddlExpr
                            ...
                            , RelDumpLoad::DumploadObjType dlType)
{
  ...
  const NAString name = qn.getQualifiedNameAsAnsiString().isNull() ? 
                          qn.getSchemaNameAsAnsiString()
...
static short dumpHiveSchemaDDL(ExeCliInterface *cliInterface
                               , RelDumpLoad *ddlExpr
                               , const SchemaName &sn
                               , ostream *file)
{
  ...
  snprintf(query, sizeof(query), "SELECT trim(table_name) FROM table(hivemd(tables, \"%s\")) "
                     "WHERE hive_table_type = 'MANAGED_TABLE' or hive_table_type = 'EXTERNAL_TABLE'", schemaName.data());
      ...
      for (int i = 0; i < numEntries; i++)
        {
          ...
          if ((type < REC_MIN_CHARACTER)
               || (type > REC_MAX_CHARACTER))
            return -1;
...
static short getPackagesInSchema(ExeCliInterface *cliInterface,
                                 const SchemaName &sn,
                                 Queue *&outQueue)
{
  short retcode = 0;
  ...
  snprintf(query, sizeof(query), "SELECT DISTINCT object_name FROM %s.\"%s\".%s "
...
static short dumpSeabaseSchemaDDL(ExeCliInterface *cliInterface
                           , RelDumpLoad *ddlExpr
                           , const SchemaName &sn
                           , ostream *file)
{
  ...
  if (getPackagesInSchema(cliInterface, sn, outPkgQueue))
      return -1;

  char query[1024];
  Queue *outQueue = NULL;
  const char *sysCat = ActiveSchemaDB()->getDefaults().getValue(SEABASE_CATALOG);
  snprintf(query, sizeof(query), "SELECT object_name, object_type FROM %s.\"%s\".%s "
  ...
  if (outQueue) 
    {
      ...
      for (int i = 0; i < outQueue->numEntries(); i++)
      {
        ...
        else if (!str_cmp(type, COM_USER_DEFINED_ROUTINE_OBJECT_LIT
              , str_len(COM_USER_DEFINED_ROUTINE_OBJECT_LIT))) {
            // ignore error
            if(CmpSeabaseDDL::isSPSQLPackageRoutine(name, cliInterface, sn, outPkgQueue) != 1) {
                dlType = RelDumpLoad::DL_OBJ_UDRO;
            }
        }
        else if (!str_cmp(type, COM_SEQUENCE_GENERATOR_OBJECT_LIT 
              , str_len(COM_SEQUENCE_GENERATOR_OBJECT_LIT)))
            dlType = RelDumpLoad::DL_OBJ_SGO;
        else if (!str_cmp(type, COM_TRIGGER_OBJECT_LIT
              , str_len(COM_TRIGGER_OBJECT_LIT)))
            dlType = RelDumpLoad::DL_OBJ_TRIGGER;
        else if (!str_cmp(type, COM_PACKAGE_OBJECT_LIT
              , str_len(COM_PACKAGE_OBJECT_LIT)))
            dlType = RelDumpLoad::DL_OBJ_PACKAGE;
 
        if (dlType == RelDumpLoad::DL_OBJ_UNKNOWN)
            continue;
...
short CmpSeabaseDDL::dumpMetadataOfObjects(RelDumpLoad *ddlExpr
                                  , ExeCliInterface *cliInterface)
{
  ...
  if (retcode < 0)
    {
      if (!CmpCommon::diags()->containsError(-CAT_NOT_AUTHORIZED))
  ...
  if (createDumpDir(localtionPath) < 0)
    return -1;

  NAString dumpFileName = localtionPath + '/'
     + qn.getCatalogName() + '_' + qn.getSchemaName();

  if (NOT objNamePart.isNull())
    dumpFileName = dumpFileName + '_' + objNamePart;
  ...
  retcode = dumpObjectsDDL(cliInterface
                               , ddlExpr 
                               , qn
                               , file
                               , dlObjType);
...
short CmpSeabaseDDL::loadMetadataOfObjects(RelDumpLoad *ddlExpr
                                  , ExeCliInterface *cliInterface)
{
...
short CmpSeabaseDDL::isSPSQLPackageRoutine(const char *routineName,
         ExeCliInterface *cliInterface, const SchemaName &sn, Queue *inPkgQueue)
{
     short retcode = 0;
     if (inPkgQueue == NULL) {
         NAString catalogName = sn.getCatalogName();
