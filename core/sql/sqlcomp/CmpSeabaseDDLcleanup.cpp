                                               ...
                                               Int64 *objDataUID)
{
  ...
  objUID = getObjectUID(cliInterface, catName, schName, objName, 
                        (strcmp(inObjType, COM_UNKNOWN_OBJECT_LIT) == 0 ? NULL : inObjType),objDataUID,
                        NULL, outObjType, FALSE, TRUE, objectFlags);
  ...
  objUID = getObjectUID(cliInterface, catName, schName, objName, 
                        (strcmp(inObjType, COM_UNKNOWN_OBJECT_LIT) == 0 ? NULL : inObjType),objDataUID,
                        NULL, outObjType, TRUE, TRUE, objectFlags);
  if (objUID != -1)
    return objUID;
...
short CmpSeabaseMDcleanup::getCleanupObjectName(
                                                ExeCliInterface *cliInterface,
                                                Int64 objUID,
                                                NAString &catName,
                                                NAString &schName,
...
short CmpSeabaseMDcleanup::hasInferiorPartitons(ExeCliInterface *cliInterface, Int64 objUID)
{
  ...
  str_sprintf(query, "select count(parent_uid) from %s.\"%s\".%s where parent_uid = %ld",
              getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_PARTITIONS, objUID);
                                               ...
                                               ExeCliInterface *cliInterface)
{
  ...
  if (NOT inObjType.isNull())
    {
      ...
      if (stmtCleanupNode->isPartition()) {
          ...
          getPartitionTableName(partitionTableName, 256,
                                qn->getObjectName(),
                                stmtCleanupNode->getPartitionName());
      ...
      if ((objUID_ == -1) && // not found
        ...
        if (objUID_ != -1 && stmtCleanupNode->getType() == StmtDDLCleanupObjects::INDEX_ && CmpSeabaseDDL::isMDflagsSet(objectFlags_, MD_OBJECTS_READ_ONLY_ENABLED) && !Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL))
        {
  ...
  else if (stmtCleanupNode->getType() == StmtDDLCleanupObjects::OBJECT_UID_)
    {
      ...
      cliRC = getCleanupObjectName(cliInterface, objUID_, 
                                   catName_, schName_, objName_,
                                   objType_, objectOwner_, &objectFlags_, &objDataUID_);
      if (cliRC < 0)
        return -1;
          ...
          if (NOT (catName_.isNull() || schName_.isNull() || objName_.isNull()))
            {
              ...
              if (objUID_ > 0)
                {
                  if (getTextFromMD(cliInterface, objUID_, COM_OBJECT_NAMESPACE,
                                    0, nameSpace))
                    {
                      return -1;
                    }
              ...
              extNameForHbase_ = genHBaseObjName
                (catName_, schName_, objName_, nameSpace, objDataUID_);
            }
        }
    }
  else if (isHive_)
  ...
  if (objType_ == COM_INDEX_OBJECT_LIT)
    {
      ...
      if (getBaseTable(cliInterface,
                       catName_, schName_, objName_,
                       btCatName, btSchName, btObjName_, btUID,
  ...
  if (!isDDLOperationAuthorized(SQLOperation::DROP_TABLE,
                                objectOwner_, -1, objUID_, 
                                PrivMgr::ObjectLitToEnum(objType_), NULL))
  ...
  if (stmtCleanupNode->noHBaseDrop())
    noHBaseDrop_ = TRUE;
      ...
      if (NOT extTableName.isNull())
        {
          ...
          Int64 extObjUID = 
            getObjectUID(cliInterface, 
                         ...
                         COM_BASE_TABLE_OBJECT_LIT,NULL,
                         NULL, NULL, FALSE,
                         FALSE);
          if (extObjUID > 0)
            {
...
short CmpSeabaseMDcleanup::deleteMDentries(ExeCliInterface *cliInterface)
{
  ...
  if (objType_ == COM_INDEX_OBJECT_LIT)
    str_sprintf(query, "delete from %s.\"%s\".%s where index_uid = %ld ",
                getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_INDEXES,
                objUID_);
  else
      ...
      for (size_t i = 0; i < uniqueConstrUIDlist_->numEntries(); i++)
        {
          ...
          str_sprintf(query, "select flags, trim(catalog_name) || '.\"' || "
          ...
          str_sprintf(query, "delete from %s.\"%s\".%s where constraint_uid in "
                      "(select foreign_constraint_uid from %s.\"%s\".%s "
                      "where unique_constraint_uid = %ld)",
                      getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_TABLE_CONSTRAINTS,
                      getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_UNIQUE_REF_CONSTR_USAGE,
              ...
              if (storedDesc)
                {
                  ...
                  str_sprintf(query, "alter table %s generate stored desc",
                              (char *) referencingTable.data());
                  cliRC = cliInterface->executeImmediate(query);
                  if (cliRC < 0)
                    {
                      cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
...
short CmpSeabaseMDcleanup::deletePrivs(ExeCliInterface *cliInterface)
{
  ...
  str_sprintf(query, "delete from %s.\"%s\".%s where object_uid = %ld",
              getSystemCatalog(), SEABASE_PRIVMGR_SCHEMA, PRIVMGR_OBJECT_PRIVILEGES,
              objUID_);
  cliRC = cliInterface->executeImmediate(query);
  if (cliRC < 0)
    {
  ...
  str_sprintf(query, "delete from %s.\"%s\".%s where object_uid = %ld",
              getSystemCatalog(), SEABASE_PRIVMGR_SCHEMA, PRIVMGR_COLUMN_PRIVILEGES,
              objUID_);
  cliRC = cliInterface->executeImmediate(query);
  if (cliRC < 0)
    {
...
short CmpSeabaseMDcleanup::deleteSchemaPrivs(ExeCliInterface *cliInterface)
{
  ...
  str_sprintf(query, "delete from %s.\"%s\".%s where schema_uid = %ld",
              getSystemCatalog(), SEABASE_PRIVMGR_SCHEMA, PRIVMGR_SCHEMA_PRIVILEGES,
              objUID_);
  cliRC = cliInterface->executeImmediate(query);
  if (cliRC < 0)
    {
...
short CmpSeabaseMDcleanup::deleteTenantSchemaUsages(ExeCliInterface *cliInterface)
{
  ...
  // Remove usage
  str_sprintf(query, "delete from %s.\"%s\".%s where usage_uid = %ld and usage_type = 'S'",
  ...
  str_sprintf(query, "update %s.\"%s\".%s set default_schema_uid = 0 "
...
short CmpSeabaseMDcleanup::deletePartitionEntries(ExeCliInterface *cliInterface)
{
  ...
  if (isMDflagsSet(objectFlags_, MD_PARTITION_TABLE_V2)) {
      str_sprintf(query, "delete from %s.\"%s\".%s where table_uid = %ld",
                  getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_TABLE_PARTITION, objUID_);
      cliRC = cliInterface->executeImmediate(query);
      if (cliRC < 0)
      {
          cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
      ...
      str_sprintf(query, "delete from %s.\"%s\".%s where partition_uid = %ld",
                  getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_PARTITIONS, objUID_);
      cliRC = cliInterface->executeImmediate(query);
      if (cliRC < 0)
      {
          cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
  ...
  for (size_t i = 0; i < schObjList->numEntries(); i++)
    {
      ...
      if (obj_type == COM_BASE_TABLE_OBJECT_LIT) {
        if (lockObjectDDL(cn.getQualifiedNameObj(),
                          COM_BASE_TABLE_OBJECT,
                          cn.isVolatile(),
                          cn.isExternal()) < 0) {
          return;
      ...
      if (obj_type == COM_BASE_TABLE_OBJECT_LIT || obj_type == COM_INDEX_OBJECT_LIT) {
        insertIntoSharedCacheList(catName_, schName_, obj_name,
                                  (obj_type == COM_BASE_TABLE_OBJECT_LIT) ? 
                                      COM_BASE_TABLE_OBJECT : COM_INDEX_OBJECT,
      ...
      if (obj_type == COM_BASE_TABLE_OBJECT_LIT) {
        if (unlockObjectDDL(cn.getQualifiedNameObj(),
                            COM_BASE_TABLE_OBJECT,
                            cn.isVolatile(),
                            cn.isExternal()) < 0) {
          return;
  ...
  if (NOT cannotDropSchema)
    {
      ...
      insertIntoSharedCacheList(catName_, schName_, NAString(SEABASE_SCHEMA_OBJECTNAME),
      ...
      ActiveSchemaDB()->getNATableDB()->removeNATable
        ( cn,
          ComQiScope::REMOVE_FROM_ALL_USERS,
          (objType_ == COM_SHARED_SCHEMA_OBJECT_LIT ? COM_SHARED_SCHEMA_OBJECT :
                       COM_PRIVATE_SCHEMA_OBJECT),
           FALSE, FALSE);
    }

   if (!ComIsTrafodionReservedSchemaName(schName_) &&
       isMDflagsSet(objectFlags_,MD_OBJECTS_INCR_BACKUP_ENABLED))
   {
     cliRC = updateSeabaseXDCDDLTable(cliInterface, catName_.data(),
                                      schName_.data(), objName_.data(),
                                      objType_);
     if (cliRC < 0)
     {
       if (processCleanupErrors(NULL, errorSeen))
...
short CmpSeabaseMDcleanup::addReturnDetailsEntryForText(ExeCliInterface *cliInterface, Queue* &list, 
                              Int64 objUID, Int32 objType, NABoolean init)
{
  ...
  str_sprintf(r, "    Entry #%d(%s):  %ld", currReturnEntry_,  
            textType.data(),
            objUID);
  ...
  for (size_t i = 0; i < fromList->numEntries(); i++)
    {
      ...
      if (processTextInfo)
      {
        ...
        if (addReturnDetailsEntryForText(cliInterface, toList, objUID, objType, FALSE))
          return -1;
      }
      else
      {
                                                       ...
                                                       CmpDDLwithStatusInfo *dws)
{
  ...
  str_sprintf(query, "select O.object_uid, 0, trim(O.catalog_name), trim(O.schema_name), trim(O.object_name), case when T.text is not null then trim(T.text) else '' end from %s.\"%s\".%s O left join %s.\"%s\".%s T on O.object_uid = T.text_uid and T.text_type = %d where O.catalog_name = '%s' and O.schema_name not in ( '_MD_', '_REPOS_', '_PRIVMGR_MD_') and O.schema_name not like '|_HV|_%%|_' escape '|'  and O.schema_name not like '|_HB|_%%|_' escape '|' and (O.object_type = 'BT' or O.object_type = 'IX') and (bitand(O.flags, %d) = 0) ",
             getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_OBJECTS,
             getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_TEXT,
             COM_OBJECT_NAMESPACE,
             getSystemCatalog(), MD_PARTITION_TABLE_V2);
          ...
          if (rc == 0) // does not exist
            {
              ...
              NAString trafName = genHBaseObjName(
                   catName, schName, objName, NAString(""));
  ...
  for (Int32 i = 0; i < listArray->entries(); i++)
    {
      ...
      if (numParts == 3)
      {
        ...
        extHbaseName = genHBaseObjName
        (catalogNamePart, schemaNamePart, objectNamePart, nameSpace);
...
short CmpSeabaseMDcleanup::cleanupInconsistentPartitionEntries(
    	ExeCliInterface *cliInterface,
        ExpHbaseInterface *ehi,
        CmpDDLwithStatusInfo *dws)
{
  ...
  str_sprintf(query, "select cast(tp.partition_uid as varchar(30) not null) from %s.\"%s\".%s tp "
  ...
  addReturnDetailsEntryFromList(cliInterface, obsoleteEntriesList_, 0/*0 based*/,
                                returnDetailsList_, TRUE/*UID*/);
...
short CmpSeabaseMDcleanup::cleanupInconsistentPrivEntries(
     ExeCliInterface *cliInterface,
     ExpHbaseInterface *ehi)
{
  Lng32 cliRC = 0;
  ...
  str_sprintf(query, "select grantee_id, grantee_name from %s.\"%s\".%s "
              "where grantee_id > 0 and "
              "grantee_id not in (select auth_id from %s.\"%s\".%s)",
              getSystemCatalog(), SEABASE_PRIVMGR_SCHEMA, PRIVMGR_COMPONENT_PRIVILEGES,
              getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_AUTHS);
  Queue * orphanPrivObjs = NULL;
  cliRC = cliInterface->fetchAllRows
    (orphanPrivObjs, query, 0, FALSE, FALSE, TRUE);
  if (cliRC < 0)
    {
      if (processCleanupErrors(cliInterface, errorSeen))
        return -1;
  ...
  for (size_t i = 0; i < orphanPrivObjs->numEntries(); i++)
    {
      ...
      if (addReturnDetailsEntry(cliInterface, returnDetailsList_, msg.data(), FALSE, TRUE))
        return -1;
  ...
  for (size_t i = 0; i < authIDList.entries(); i++)
    {
      if (addReturnDetailsEntry(cliInterface, returnDetailsList_, authIDList[i].data(), FALSE, TRUE))
        return -1;
    }
...
short CmpSeabaseMDcleanup::cleanupInconsistentGroupEntries(
     ExeCliInterface *cliInterface,
     ExpHbaseInterface *ehi)
{
  Lng32 cliRC = 0;
  ...
  str_sprintf(query, "select tenant_id, usage_uid from %s.\"%s\".%s "
  ...
  cliRC = cliInterface->fetchAllRows
    (orphanGroups, query, 0, FALSE, FALSE, TRUE);
  if (cliRC < 0)
    {
      if (processCleanupErrors(cliInterface, errorSeen))
        return -1;
  ...
  for (size_t i = 0; i < orphanGroups->numEntries(); i++)
    {
      ...
      if (addReturnDetailsEntry(cliInterface, returnDetailsList_, intStr, FALSE, TRUE))
        return -1;

      if (NOT checkOnly_)
        {
            str_sprintf(query, "delete from %s.\"%s\".%s where "
...
short CmpSeabaseMDcleanup::cleanupInconsistentTextEntries(ExeCliInterface *cliInterface,
                                                          ExpHbaseInterface *ehi,
                                                          CmpDDLwithStatusInfo *dws)
{
  Lng32 cliRC = 0;
  ...
  str_sprintf(query, "select TEXT_UID, TEXT_TYPE  from %s.\"%s\".%s  where TEXT_UID not in (select OBJECT_UID  from %s.\"%s\".%s) AND TEXT_TYPE != 1",
              getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_TEXT,
              getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_OBJECTS);
...
void CmpSeabaseMDcleanup::cleanupInferiorPartitionEntries(ExeCliInterface *cliInterface)
{
  ...
  str_sprintf(query, "select partition_uid from %s.\"%s\".%s where parent_uid = %ld ",
              getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_PARTITIONS, objUID_);
        ...
        case INCONSISTENT_PARTITIONS_ENTRIES:
          {
            switch (dws->subStep())
              ...
              case 1:
                {
                  if (cleanupInconsistentPartitionEntries(cliInterface, ehi, dws))
                    return;
                  
                  str_sprintf(buf, "  End:   Cleanup Inconsistent Partitions Entries (%d %s %s)",
                              numInconsistentPartitionEntries_,
                              (numInconsistentPartitionEntries_ == 1 ? "entry" : "entries"),
                              (checkOnly_ ? "found" : "cleaned up"));
              ...
              case 1:
                {
                  ...
                  {
                    if (cleanupInconsistentPrivEntries(cliInterface, ehi))
                      return;
                  }
                  else
                  {
                  ...
                  str_sprintf(buf, "  End:   Cleanup Inconsistent Privilege Entries (%d %s %s)",
                              numInconsistentPrivEntries_,
              ...
              case 1:
                {
                  ...
                  if (cleanupInconsistentGroupEntries(cliInterface, ehi) == -1)
                    return;

                  str_sprintf(buf, "  End:   Cleanup Inconsistent User Group Entries (%d %s %s)",
                              numInconsistentGroupEntries_,
                              (numInconsistentGroupEntries_ == 1 ? "entry" : "entries"),
                              (checkOnly_ ? "found" : "cleaned up"));
       ...
       case INCONSISTENT_TEXT_ENTRIES:
          {
            switch (dws->subStep())
              ...
              case 1:
                {
                  if (cleanupInconsistentTextEntries(cliInterface, ehi, dws))
                    return;
                  
                  str_sprintf(buf, "  End:   Cleanup Inconsistent TEXT Entries (%d %s %s)",
                              numInconsistentTextEntries_,
                                         ...
                                         CmpDDLwithStatusInfo *dws)
{
  ...
  if ((((objType_ == COM_BASE_TABLE_OBJECT_LIT) && (! hasInferiorPartitons_)) ||
       (objType_ == COM_INDEX_OBJECT_LIT)) &&
      (NOT isHive_) &&
  ...
  if ((!ComIsTrafodionReservedSchemaName(schName_)) &&
      (NOT (catName_.isNull() || schName_.isNull())) &&
      (objType_ == COM_BASE_TABLE_OBJECT_LIT ||
       objType_ == COM_INDEX_OBJECT_LIT)             &&
      ((stmtCleanupNode->getType() == StmtDDLCleanupObjects::TABLE_ &&
        NOT objName_.isNull()) ||
       (stmtCleanupNode->getType() == StmtDDLCleanupObjects::INDEX_) &&
        NOT btObjName_.isNull())                                     &&
      (NOT isHive_) &&
      (NOT isLOBDependentNameMatch(objName_)))
  {
    if ((objUID_ != -1 &&
         isMDflagsSet(objectFlags_,
         MD_OBJECTS_INCR_BACKUP_ENABLED)) ||
        objUID_ == -1)
    {
  ...
  if (NOT (catName_.isNull() || schName_.isNull() || objName_.isNull()))
    {
      ...
      if (ot == COM_BASE_TABLE_OBJECT || ot == COM_INDEX_OBJECT) {
        insertIntoSharedCacheList(catName_, schName_, objName_,
                                  ot, SharedCacheDDLInfo::DDL_DELETE);
        insertIntoSharedCacheList(catName_, schName_, objName_,
                                  ot,
                                  SharedCacheDDLInfo::DDL_DELETE,
                                  SharedCacheDDLInfo::SHARED_DATA_CACHE);
          ...
          if (retcode == ObjectEpochCache::OEC_ERROR_OK)
            {
              ...
              snprintf (msgBuf, sizeof(msgBuf), "DDL cleanup, Name: %s, Epoch: %d, "
                                " Flags: %d",
