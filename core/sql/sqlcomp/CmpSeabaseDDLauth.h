...
class AuthConflict
{
  ...
  NABoolean operator==(const AuthConflict& other)
  {
    if ((sourceID_ == other.sourceID_) &&
        ...
        (mdTable_  == other.mdTable_)  &&
        (mdColumn_ == other.mdColumn_))
      return TRUE;
    return FALSE;
  }
...
class AuthConflictList : public LIST(AuthConflict)
{
  ...
  // constructor
  AuthConflictList( NAHeap *heap = NULL)
   : LIST(AuthConflict)(heap),
     heap_(heap)
  {}
...
class CmpSeabaseDDLauth  
{
   ...
   public:
     ...
     CmpSeabaseDDLauth::AuthStatus getAdminRole(
        const Int32 authID, 
        Int32 &adminRoleID);

     CmpSeabaseDDLauth::AuthStatus getAuthsForCreator(
        Int32 authCreator, 
 ...
 protected:
    ...
    // mutators
    void setAuthConfig       (const Int16 authConfig)
    ...
    void setAuthPassword     (const NAString &authPassword)
      {authPassword_ = authPassword;}
     void setAuthRedefTime    (const Int64 authRedefTime)
       { authRedefTime_ = authRedefTime;}
     void setPasswdModTime    (const Int64 passwdModTime)
       { passwdModTime_ = passwdModTime;}
     void setAuthType        (ComIdClass authType)
       {authType_ = authType;}
     void setAuthValid       (bool isValid)
       {authValid_ = isValid;}
     void setAuthIsAdmin     (bool isAdmin)
       {authIsAdmin_ = isAdmin;}
     void setAuthInTenant    (bool inTenant)
       {authInTenant_ = inTenant;}
     void setAutoCreated     (bool autoCreated)
       {authAutoCreated_ = autoCreated;}
     void setAuthGroupList   (TenantGroupInfoList * groupList)
       {authGroupList_ = groupList;}
    ...
    AuthStatus selectAuthIDs(const NAString & whereClause,
    ...
    AuthStatus selectAuthNames (const NAString & whereClause,
                                std::vector<std::string> &userNames,
                                std::vector<std::string> &roleNames,
                                std::vector<std::string> &tenantNames);
    AuthStatus selectAllWhere (const NAString & whereClause,
...
class CmpSeabaseDDLtenant : public CmpSeabaseDDLauth
{
   public:
     ...
     Int32 registerTenant(
       ExeCliInterface *cliInterface,
       StmtDDLTenant * pNode,
       Int32 adminRoleID);

     Int32 registerTenantPrepare(
       StmtDDLTenant * pNode,
       Int32 &adminRoleID);

     Int32 unregisterTenant(
       ExeCliInterface *cliInterface,
       StmtDDLTenant * pNode);
     ...
     Int64 getDefSchemaUID(
       TenantInfo &tenantInfo,
       const TenantUsageList  *usageList);

     static short getNodeDetails (
       const TenantResourceUsageList *usageList,
       TenantNodeInfoList *& nodeInfoList);

     Int32 getNodesForRGroups (
       const TenantUsageList *usageList,
       TenantResourceUsageList *& nodeList);

     CmpSeabaseDDLauth::AuthStatus getTenantDetails(
     ...
     Int32 getTenantSchemas (
        ExeCliInterface *cliInterface,
        NAList <Int64> *& schemaUIDList);

     TenantGroupInfoList * getTenantGroupList(
        TenantInfo &tenantInfo,
        NAHeap *heap);

    static Int32 predefinedNodeAllocations (
      Int32 unitsToAllocate,
   ...
   protected:
    ...
    void dropGroups (
     ...
     void dropResourceGroup (
       const StmtDDLTenant *pNode,
       TenantUsageList *&usageList);

     short findDetachedTenant(
       const StmtDDLTenant *pNode,
       Int32 &adminRoleID,
       Int32 &tenantID);

     short getNodeDetails (
       ExeCliInterface *cliInterface,
       TenantNodeInfoList *& nodesToAdd);

     short getTenantResource (
    ...
    short generateSchemaUsages (
     ...
     CmpSeabaseDDLauth::AuthStatus getSchemaDetails(
       ExeCliInterface *cliInterface,
       const SchemaName *schemaName,
       Int64 &schUID,
