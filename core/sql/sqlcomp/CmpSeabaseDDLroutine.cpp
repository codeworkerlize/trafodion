                                       ...
                                       NAString &currSchName)
{
  ...
  if (!isDDLOperationAuthorized(SQLOperation::DROP_LIBRARY,
                                ...
                                objUID, COM_LIBRARY_OBJECT, NULL))
  {
     *CmpCommon::diags() << DgSqlCode(-CAT_NOT_AUTHORIZED);
     processReturn ();
     return;
					 ...
					 NAString &currSchName)
{
  ...
  if (!isDDLOperationAuthorized(SQLOperation::ALTER_LIBRARY,
				objectOwnerID,schemaOwnerID, 
                                libUID, COM_LIBRARY_OBJECT, NULL))
    {
      *CmpCommon::diags() << DgSqlCode(-CAT_NOT_AUTHORIZED);
      processReturn();
      return;
...
{
  Lng32 cliRC = 0;
  Int64 libUID = getObjectUID(cliInterface, getSystemCatalog(), SEABASE_MD_SCHEMA, 
  ...
  for (int i = 0; i < numSPSQLProcs; i++)
    {
      ...
      cliRC = existsInSeabaseMDTable(cliInterface,
                                     ...
                                     quotedSpjObjName, COM_USER_DEFINED_ROUTINE_OBJECT,
                                     TRUE, FALSE);
      if (cliRC < 0)            // Error
        {
          return -1;
      ...
      if (updateSeabaseMDSPJ(cliInterface,
                             libUID,
...
short buildQuery(NAString &sql,
                 const StmtDDLCreateRoutine* stmtDDLCreateRoutine,
                 NABoolean noCreate=FALSE, NABoolean noResultSet=FALSE)
{
  ...
  if (stmtDDLCreateRoutine->getRoutineType() == COM_SCALAR_UDF_TYPE)
  ...
  else if (!noResultSet)
    {
      ...
      char buf[100];
      sprintf(buf, " DYNAMIC RESULT SETS %d",
              stmtDDLCreateRoutine->getMaxResults());
      sql.append(buf);
    }
...
void CmpSeabaseDDL::createSeabasePackage(ExeCliInterface * cliInterface,
					 ...
					 NAString &currSchName)
{
  ...
  const NAString catalogNamePart = 
    packageName.getCatalogNamePartAsAnsiString();
  const NAString schemaNamePart = 
    packageName.getSchemaNamePartAsAnsiString(TRUE);
  const NAString objectNamePart = 
    packageName.getObjectNamePartAsAnsiString(TRUE);
  ...
  if (retcode == 1 && createPackageNode->isCreatePackageBody())
      ...
      if (objUID < 0)
	{
      ...
      char query[1000];
      str_sprintf(query, "select count(*) from %s.\"%s\".%s "
                  " where text_uid=%ld and sub_id=%d",
                  getSystemCatalog(), SEABASE_MD_SCHEMA,
                  SEABASE_TEXT, objUID, subID);
      Int64 rowCount = 0;
      Lng32 length = 0;
  ...
  if (retcode == 1) // already exists
    {
      if (!createPackageNode->isReplace())
        {
          if (! createPackageNode->createIfNotExists())
            *CmpCommon::diags() << DgSqlCode(-1390)
                                << DgString0(extPackageName);
          processReturn();
          return;
        }
      else
        {
          if (dropPackageFromMDAndSPSQL(cliInterface,
                                        currCatName,
                                        currSchName,
                                        createPackageNode->getPackageName(),
  ...
  if (updateTextTable(cliInterface, objUID,
                      COM_ROUTINE_TEXT, subID, sql, NULL, length, false))
    {
      processReturn();
      return;
    }
...
short CmpSeabaseDDL::dropPackageFromMDAndSPSQL(ExeCliInterface * cliInterface,
                                               NAString & currCatName,
                                               NAString & currSchName,
                                               const NAString & packageName,
                                               NABoolean ddlXns)
{
  ...
  char query[1000];
  str_sprintf(query, "select object_name from %s.\"%s\".%s"
              ...
              catalogNamePart.data(), schemaNamePart.data(),
              COM_USER_DEFINED_ROUTINE_OBJECT_LIT, objectNamePart.data());
  Queue *queue = NULL;
  ...
  for (size_t i=0; i<queue->numEntries(); i++)
    {
      ...
      char *name = ri->get(0);
      char buf[1000];
      str_sprintf(buf, "%s.%s.\"%s\"",
                  catalogNamePart.data(), schemaNamePart.data(), name);
      if (dropSeabaseObject(ehi, buf, currCatName, currSchName,
                            COM_USER_DEFINED_ROUTINE_OBJECT,
                            ddlXns,
                            TRUE, FALSE, FALSE, NAString(""), TRUE, &oldObjUID))
        {
          deallocEHI(ehi); 
          return -1;
        }
  ...
  if (dropSeabaseObject(ehi, packageName,
  ...
  str_sprintf(query, "call %s.\"%s\".%s('DROP PACKAGE %s;')",
              TRAFODION_SYSTEM_CATALOG,
              SEABASE_MD_SCHEMA,
              SEABASE_SPSQL_DROP_SPJ,
              packageName.data());
...
void CmpSeabaseDDL::dropSeabasePackage(ExeCliInterface * cliInterface,
				       ...
				       NAString &currSchName)
{
  ...
  retcode = existsInSeabaseMDTable(cliInterface,
				   catalogNamePart, schemaNamePart,
                                   objectNamePart, COM_PACKAGE_OBJECT,
                                   TRUE, FALSE);
  if (retcode < 0)
    {
      processReturn();
  ...
  if (retcode == 0) // does not exist
    {
      if (NOT dropPackageNode->dropIfExists())
        *CmpCommon::diags() << DgSqlCode(-1389)
                            << DgString0(extPackageName);
      processReturn();
      return;
  ...
  objUID = getObjectInfo(cliInterface,
                         catalogNamePart.data(), schemaNamePart.data(), 
                         objectNamePart.data(), COM_PACKAGE_OBJECT,
                         objectOwnerID,schemaOwnerID,objectFlags,objDataUID);
  if (objUID < 0 || objectOwnerID == 0 || schemaOwnerID == 0)
    {
      processReturn();
  ...
  if (!isDDLOperationAuthorized(SQLOperation::DROP_ROUTINE,
                                objectOwnerID,schemaOwnerID, 
                                objUID, COM_PACKAGE_OBJECT, NULL))
  {
     *CmpCommon::diags() << DgSqlCode(-CAT_NOT_AUTHORIZED);
     processReturn ();
     return;
  ...
  if (dropPackageFromMDAndSPSQL(cliInterface,
                                currCatName,
                                currSchName,
                                dropPackageNode->getPackageName(),
...
short CmpSeabaseDDL::createSPSQLRoutine(ExeCliInterface * cliInterface,
					NAString *sql)
{
  NAString quotedSql;
  ToQuotedString(quotedSql, *sql);
  char *query = new (STMTHEAP) char[quotedSql.length() + 1000];
  str_sprintf(query, "call %s.\"%s\".%s(%s)",
              getSystemCatalog(), SEABASE_MD_SCHEMA,
              SEABASE_SPSQL_CREATE_SPJ, quotedSql.data());
                                      ...
                                      NAString &currSchName)
{
  ...
  if (retcode == 1) // already exists
    {
	  ...
	  retcode = dropRoutineFromMDAndSPSQL(&cliInterface, ehi,
					      currCatName, currSchName,
					      createRoutineNode->getRoutineName(),
					      createRoutineNode->getRoutineType(),
					      createRoutineNode->ddlXns(),
					      oldObjUID);
          if (retcode < 0)
            {
              deallocEHI(ehi);
  ...
  libUID = getObjectInfo(&cliInterface,
                         ...
                         dummy32,dummy32,dummy64,objDataUID,TRUE,FALSE, &dummy64, &redefTime);
  ...
  ComRoutineTransactionAttributes transAttrs
    = createRoutineNode->getTransactionAttributes();
  if (isSPSQL
      && transAttrs == COM_UNKNOWN_ROUTINE_TRANSACTION_ATTRIBUTE
...
Lng32 isSPSQLRoutine(ExeCliInterface *cliInterface, Int64 objUID)
{
  char query[1000];
  str_sprintf(query,
              "select external_name from %s.\"%s\".%s where udr_uid = %ld",
              TRAFODION_SYSTEM_CATALOG, SEABASE_MD_SCHEMA, SEABASE_ROUTINES,
              objUID);
  char extName[1024];
  Lng32 length;
...
Lng32 isSPSQLPackageRoutine(ExeCliInterface *cliInterface, Int64 objUID)
{
  ...
  // check the name
  char query[1000];
  str_sprintf(query, "select object_name from %s.\"%s\".%s where object_uid = %ld",
              TRAFODION_SYSTEM_CATALOG, SEABASE_MD_SCHEMA, SEABASE_OBJECTS, objUID);
  char objName[1024];
  Lng32 length = 0;
...
inline Lng32 skipDropSPSQLRoutine(ExeCliInterface *cliInterface,
                                  ComRoutineType routineType,
                                  Int64 objUID)
{
  // SPSQL can only support PROCEDURE and SCALAR_UDF, so routines of
  // other types are always skipped
  if (routineType != COM_PROCEDURE_TYPE &&
      routineType != COM_SCALAR_UDF_TYPE)
    {
      return 1;
    }
...
Lng32 dropSPSQLRoutine(ExeCliInterface *cliInterface,
                       ...
                       ComRoutineType routineType)
{
  ...
  char query[1000];
  str_sprintf(query, "call %s.\"%s\".%s('DROP %s %s;')",
              TRAFODION_SYSTEM_CATALOG, SEABASE_MD_SCHEMA, SEABASE_SPSQL_DROP_SPJ,
              type, routineName.data());
  cliRC = cliInterface->fetchRowsPrologue(query, TRUE/*no exec*/);
  if (cliRC < 0)
    {
      cliInterface->retrieveSQLDiagnostics(CmpCommon::diags());
...
short CmpSeabaseDDL::dropRoutineFromMDAndSPSQL(ExeCliInterface * cliInterface,
                                       ...
                                       NAString &currSchName)
{
  ...
  if (!isDDLOperationAuthorized(SQLOperation::DROP_ROUTINE,
                                objectOwnerID,schemaOwnerID, 
                                objUID, COM_USER_DEFINED_ROUTINE_OBJECT, NULL))
  {
     *CmpCommon::diags() << DgSqlCode(-CAT_NOT_AUTHORIZED);
     deallocEHI(ehi);
     processReturn ();
  ...
  if (dropRoutineFromMDAndSPSQL(&cliInterface, ehi,
                                currCatName, currSchName,
                                dropRoutineNode->getRoutineName(),
                                dropRoutineNode->getRoutineType(),
                                     ...
                                     const char * optionalSig)
{
  ...
  if (str_cmp_ne(className, SEABASE_SPSQL_CONTAINER) == 0 &&
      str_cmp_ne(methodName, SEABASE_SPSQL_CALL) == 0)
  {
...
short CmpSeabaseDDL::createSeabaseLibmgr(ExeCliInterface * cliInterface)
{
  ...
  char queryBuf[strlen(getSystemCatalog()) + sizeof(SEABASE_LIBMGR_SCHEMA) +
                sizeof(SEABASE_LIBMGR_LIBRARY) + sizeof(DB__LIBMGRROLE) + 
                jarLocation.length() + 100];
  char nameSpaceBuf[strlen(TRAF_RESERVED_NAMESPACE1) + 100];

  if (CmpCommon::context()->useReservedNamespace())
    snprintf(nameSpaceBuf, sizeof(nameSpaceBuf),
             "namespace '%s'",
  ...
  snprintf(queryBuf, sizeof(queryBuf),
           "create schema if not exists %s.\"%s\" authorization %s %s",
           getSystemCatalog(),SEABASE_LIBMGR_SCHEMA, DB__ROOT, nameSpaceBuf);
  ...
  for (Int32 i = 0; i < sizeof(allLibmgrRoutineInfo)/sizeof(LibmgrRoutineInfo); i++)
    {
      ...
      // generated text.
      char queryBuf[strlen(getSystemCatalog())*2 + sizeof(SEABASE_LIBMGR_SCHEMA)*2 +
                    sizeof(SEABASE_LIBMGR_LIBRARY) + gluedQuerySize + 200]; 
...
short CmpSeabaseDDL::upgradeSeabaseLibmgr(ExeCliInterface * cliInterface)
{
  ...
  if (cliRC < 0)
    ...
    stmtSize = snprintf(queryBuf, sizeof(queryBuf), "update %s.\"%s\".%s set redef_time = %ld where object_name in ('%s', '%s') and object_type = 'LB'",
                        getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_OBJECTS,
                        NA_JulianTimestamp(),
                        SEABASE_LIBMGR_LIBRARY, SEABASE_LIBMGR_LIBRARY_CPP);
    CMPASSERT(stmtSize < sizeof(queryBuf));
    cliRC = cliInterface->executeImmediate(queryBuf);
    if (cliRC < 0)
    {
  ...
  if (cliRC == 0) // does not exist
    {
      ...
      // INITIALIZE TRAFODION, CREATE LIBRARY MANAGEMENT
      NAString libraryName(getSystemCatalog());
      libraryName += NAString( ".\"") + NAString(SEABASE_LIBMGR_SCHEMA) + NAString("\".") + NAString(SEABASE_LIBMGR_LIBRARY);
      *CmpCommon::diags() << DgSqlCode(-1389)
                          << DgString0(libraryName.data());
      return -1;
    }
...
void CmpSeabaseDDL::createSeabaseTriggers(ExeCliInterface *cliInterface,
                                          ...
                                          NAString &currCatName, NAString &currSchName)
{
  ...
  retcode = lockObjectDDL(tableName,
                          ...
                          CreateTrigger->isVolatile(),
                          CreateTrigger->isExternal());
  if (retcode < 0) {
    deallocEHI(ehi);
    processReturn();
    return;
  ...
  if (retcode == 1) // Already exists
      ...
      else
	{
	  ...
	  retcode = dropTriggerFromMDAndSPSQL(cliInterface, ehi,
					      currCatName, currSchName,
					      CreateTrigger->getTriggerName(),
					      CreateTrigger->ddlXns(),
					      -1);
  ...
  if (updateSeabaseMDTable(cliInterface,
			   catalogNamePart, schemaNamePart, triggerNamePart,
			   COM_TRIGGER_OBJECT,
			   COM_NO_LIT,
			   tableInfo,
			   0, NULL,
  ...
  Int64 tableUID = -1;
  tableUID = getObjectUID(cliInterface,
			  catalogNamePart.data(),
			  schemaNamePart.data(),
			  tableNamePart.data(),
   ...
   * max_results(ComOperation), //COM_UNKNOWN_IUD(0), COM_INSERT(1), COM_DELETE(2), COM_UPDATE(3), COM_SELECT(4), COM_ROUTINE(5),
   *                              COM_INSERT_UPDATE(6), COM_INSERT_DELETE(7), COM_INSERT_UPDATE_DELETE(8), COM_UPDATE_DELETE(9)
  ...
  str_sprintf(query1, "insert into %s.\"%s\".%s values (%ld, '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', %d, %d, '%s', '%s', '%s', '%s', '%s', %ld, '%s', 0)",
              getSystemCatalog(), SEABASE_MD_SCHEMA, SEABASE_ROUTINES,
              triggerUID,
              COM_TRIGGER_OBJECT_LIT,
              CreateTrigger->isAfter() ? "A" : "B",
              CreateTrigger->isStatement() ? "Y" : "N" ,
              subID == 1 ? "P" : "S",
              "N" ,
              ...
              tableUID,
              "(Ljava/lang/String;IILjava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;ILjava/lang/String;ILjava/lang/String;I)V" // not used signature now
	      ); // not used flags now
  ...
  Lng32 length = sql.length();
  if (updateTextTable(cliInterface, triggerUID,
		      COM_TRIGGER_TEXT, subID, sql, NULL, length, false))
    {
      processReturn();
      return;
    }
  ...
  if (updateObjectFlags(cliInterface, tableUID, flags, FALSE) < 0)
    goto bad;
  // remove store info from text table, reset restore flag
  flags = MD_OBJECTS_STORED_DESC;
  if (updateObjectFlags(cliInterface, tableUID, flags, TRUE) < 0)
    goto bad;
  if (deleteFromTextTable(cliInterface, tableUID, COM_STORED_DESC_TEXT, 0) < 0)
    goto bad;

  // insert into "_XDC_MD_".xdc_ddl
  if (!ComIsTrafodionReservedSchemaName(schemaNamePart))
  {
    Lng32 retcode = updateSeabaseXDCDDLTable(cliInterface,
                                             catalogNamePart.data(),
                                             schemaNamePart.data(),
                                             triggerNamePart.data(),
                                             COM_TRIGGER_OBJECT_LIT);
    if (retcode < 0)
      goto bad; 
  }
  ...
  {
      insertIntoSharedCacheList(currCatName, currSchName, tableNamePart,
...
Lng32 dropSPSQLTrigger(ExeCliInterface *cliInterface,
		       const NAString &triggerName)
{
  ...
  char query[1000];
  str_sprintf(query, "call %s.\"%s\".%s('DROP TRIGGER %s;')",
	      TRAFODION_SYSTEM_CATALOG, SEABASE_MD_SCHEMA, SEABASE_SPSQL_DROP_SPJ,
	      triggerName.data());
  cliRC = cliInterface->fetchRowsPrologue(query, TRUE/*no exec*/);
  if (cliRC < 0)
...
short CmpSeabaseDDL::dropTriggerFromMDAndSPSQL(ExeCliInterface * cliInterface,
					       ExpHbaseInterface * ehi,
					       NAString &currCatName,
					       NAString &currSchName,
					       const NAString &triggerName,
					       ...
					       Int64 objUID)
{
  ...
  //1) get table id
  char query[1000] = {0};
  str_sprintf(query, "SELECT R2.LIBRARY_UID, count(R2.LIBRARY_UID) FROM %s.\"%s\".%s R1, \
  ...
  // Removed routine from metadata
  if (dropSeabaseObject(ehi, triggerName,
			currCatName, currSchName,
			COM_TRIGGER_OBJECT,
			ddlXns,
			TRUE, FALSE, FALSE, NAString("")))
...
void CmpSeabaseDDL::dropSeabaseTriggers(ExeCliInterface *cliInterface,
                                        ...
                                        NAString &currCatName, NAString &currSchName)
{
  ...
  retcode = existsInSeabaseMDTable(cliInterface,
				   catalogNamePart, schemaNamePart,
				   triggerNamePart, COM_TRIGGER_OBJECT,
				   TRUE, FALSE);
  ...
  triggerUID = getObjectInfo(cliInterface,
                             catalogNamePart.data(), schemaNamePart.data(),
                             triggerNamePart.data(), COM_TRIGGER_OBJECT,
			     objectOwnerID, schemaOwnerID, objectFlags, objDataUID);
  ...
  // Verify user has privilege to drop package
  if (!isDDLOperationAuthorized(SQLOperation::DROP_TRIGGER, objectOwnerID, schemaOwnerID, triggerUID, COM_TRIGGER_OBJECT, NULL))
    {
      *CmpCommon::diags() << DgSqlCode(-CAT_NOT_AUTHORIZED);
      processReturn ();
      return;
  ...
  // remove this table from NATable
  char query[1000];
  str_sprintf(query, "SELECT CATALOG_NAME, SCHEMA_NAME, OBJECT_NAME FROM  %s.\"%s\".%s WHERE OBJECT_UID=(SELECT \
  ...
  for (int i = 0; i < objectsInfo->numEntries(); i++)
    {
      ...
      if (CmpCommon::getDefault(TRAF_ENABLE_METADATA_LOAD_IN_SHARED_CACHE) == DF_ON)
      {
          insertIntoSharedCacheList(CatName, SchName, TabName,
                                    COM_BASE_TABLE_OBJECT, SharedCacheDDLInfo::DDL_DELETE);
      }
      if (!xnInProgress(cliInterface))
      {
  ...
  if (dropTriggerFromMDAndSPSQL(cliInterface, ehi,
				currCatName, currSchName,
				DropTrigger->getTriggerName(),
				DropTrigger->ddlXns(),
				triggerUID))
    {
      deallocEHI(ehi);
  ...
  if (!ComIsTrafodionReservedSchemaName(schemaNamePart))
  {
    updateSeabaseXDCDDLTable(cliInterface,
                             catalogNamePart.data(),
                             schemaNamePart.data(),
                             triggerNamePart.data(),
                             COM_TRIGGER_OBJECT_LIT);
        ...
        case 2:
          {
            ...
            if (checkForOldLibraries(cliInterface))
...
{
  Lng32 cliRC = 0;
  Int32 bufSize = (strlen(TRAFODION_SYSCAT_LIT)*2) + strlen(SEABASE_MD_SCHEMA)*2 +
                  strlen(SEABASE_OBJECTS) + strlen(SEABASE_LIBRARIES_OLD) + (3*60);
  char queryBuf[bufSize];

  str_sprintf(queryBuf, "select count(*) from %s.\"%s\".%s "
              ...
              "object_type = 'BT' and object_name = '%s'",
               ...
               TRAFODION_SYSCAT_LIT,SEABASE_MD_SCHEMA, SEABASE_LIBRARIES_OLD);
  ...
  if (rowCount > 0)
  {
    SEABASEDDL_INTERNAL_ERROR("CmpSeabaseDDL::checkForOldLibraries failed, tables with the _OLD_MD suffix exist in the _MD_ SCHEMA");
    return 1;
  }
  return 0;
}
...
short CmpSeabaseDDL::copyOldLibrariesToNew(ExeCliInterface * cliInterface)
{
      ...
      for (size_t i = 0; i < userLibsQ->numEntries(); i++)
        {
          ...
          if (libNameNoPath == "trafodion-sql-currversion.jar")
            continue;

          str_sprintf(ubuf," update %s.\"%s\".%s set library_filename = '%s', "
  ...
  if (failedLibraries.length())
    SQLMXLoggingArea::logSQLMXPredefinedEvent(failedLibraries, LL_ERROR);
