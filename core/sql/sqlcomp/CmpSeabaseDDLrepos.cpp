...
short CmpSeabaseDDL::createRepos(ExeCliInterface *cliInterface) {
  ...
  cliRC = cliInterface->holdAndSetCQD("TRAF_MAX_CHARACTER_COL_LENGTH", "1000000");
  ...
  if (CmpCommon::context()->useReservedNamespace())
    str_sprintf(queryBuf, "create schema %s.\"%s\" namespace '%s' ; ", getSystemCatalog(), SEABASE_REPOS_SCHEMA,
                ComGetReservedNamespace(SEABASE_REPOS_SCHEMA).data());
  else
    str_sprintf(queryBuf, "create schema %s.\"%s\" ; ", getSystemCatalog(), SEABASE_REPOS_SCHEMA);
...
short CmpSeabaseDDL::dropRepos(ExeCliInterface *cliInterface, NABoolean oldRepos, NABoolean dropSchema,
  ...
  for (Int32 i = 0; i < sizeof(allReposUpgradeInfo) / sizeof(MDUpgradeInfo); i++) {
    ...
    if (!oldRepos && inRecovery && !rti.oldName) continue;

    if ((oldRepos && !rti.oldName) || (NOT oldRepos && !rti.newName)) continue;

    str_sprintf(queryBuf, "drop table %s.\"%s\".%s cascade; ", getSystemCatalog(), SEABASE_REPOS_SCHEMA,
  ...
  if (dropSchema) {
    ...
    str_sprintf(queryBuf, "drop schema %s.\"%s\" cascade; ", getSystemCatalog(), SEABASE_REPOS_SCHEMA);
...
short CmpSeabaseDDL::alterRenameRepos(ExeCliInterface *cliInterface, NABoolean newToOld) {
  ...
  for (Int32 i = 0; i < sizeof(allReposUpgradeInfo) / sizeof(MDUpgradeInfo); i++) {
    ...
    if (newToOld)
      str_sprintf(queryBuf, "alter table %s.\"%s\".%s rename to %s skip view check; ", getSystemCatalog(),
                  SEABASE_REPOS_SCHEMA, rti.newName, rti.oldName);
    else
      str_sprintf(queryBuf, "alter table %s.\"%s\".%s rename to %s skip view check; ", getSystemCatalog(),
                  SEABASE_REPOS_SCHEMA, rti.oldName, rti.newName);
...
short CmpSeabaseDDL::copyOldReposToNew(ExeCliInterface *cliInterface) {
  ...
  for (Int32 i = 0; i < sizeof(allReposUpgradeInfo) / sizeof(MDUpgradeInfo); i++) {
    ...
    str_sprintf(queryBuf, "upsert using load into %s.\"%s\".%s %s%s%s select %s from %s.\"%s\".%s SRC %s;",
                TRAFODION_SYSCAT_LIT, SEABASE_REPOS_SCHEMA, rti.newName, (rti.insertedCols ? "(" : ""),
                (rti.insertedCols ? rti.insertedCols : ""), (rti.insertedCols ? ")" : ""),
                (rti.selectedCols ? rti.selectedCols : "*"), TRAFODION_SYSCAT_LIT, SEABASE_REPOS_SCHEMA, rti.oldName,
...
short CmpSeabaseDDL::dropAndLogReposViews(ExeCliInterface *cliInterface, NABoolean &someViewSaved /* out */) {
  ...
  for (Int32 i = 0; i < sizeof(allReposUpgradeInfo) / sizeof(MDUpgradeInfo); i++) {
    ...
    if (tableUID != -1)  // if we got it
    {
      ...
      short retcode1 = saveAndDropUsingViews(tableUID, cliInterface, viewNameList /* out */, viewDefnList /* out */);
      if (retcode1)
  ...
  while (1)  // exit via return stmt in switch
  {
    switch (mdui->subStep()) {
      ...
      case 4: {
        ...
        if (alterRenameRepos(cliInterface, TRUE)) return -2;  // error, need to undo the rename only
      ...
      case 6: {
        ...
        if (createRepos(cliInterface)) return -1;  // error, need to drop new repository then undo rename
      ...
      case 8: {
        ...
        if (copyOldReposToNew(cliInterface)) return -1;  // error, need to drop new repository then undo rename
  ...
  while (1)  // exit via return stmt in switch
  {
    switch (mdui->subStep()) {
      ...
      case 2: {
        ...
        // in the logs.
        mdui->setMsg(
            "One or more views on a Repository table were dropped. View text was saved in the logs. Look for 'CREATE "
            "VIEW' to find it.");
        mdui->subStep()++;
        mdui->setEndStep(FALSE);
...
void CmpSeabaseDDL::processRepository(NABoolean createR, NABoolean dropR, NABoolean upgradeR) {
