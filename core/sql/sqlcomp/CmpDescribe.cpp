...
static short CmpDescribePackage(
     const CorrName   & corrName,
     char           * & outbuf,
     ULng32           & outbuflen,
     CollHeap         * heap);
...
static short CmpDescribeTrigger(
     const CorrName   & corrName,
     char           * & outbuf,
     ULng32           & outbuflen,
     CollHeap         * heap);
...
static short CmpDescribeShowViews(
       char         *&outbuf,
       ULng32 &outbuflen,
       NAMemory      *h);

static short CmpDescribeDescView(
       const CorrName  &dtName,
       char         *&outbuf,
       ULng32 &outbuflen,
       NAMemory      *h);
...
static short CmpDescribeShowEnv(
       char         *&outbuf,
       ULng32 &outbuflen,
       NAMemory      *h); 
...
static short CmpDescribePartitions(Int32 partType, NAPartition* naPartition,  Space &space, char * buf, NABoolean isFirstPartition, NABoolean isFirst);

// This method is to describe the normal index information of the partition entity table
static short CmpDescribePartIndexDesc(BindWA *bindWA, short type, NATable *naTable, Space &space, char * buf, const CorrName  &dtName, const CorrName *likeTabName);
...
void getPartitionIndexName(char* buf, int bufLen,
      ...
      if (!enabled)
        {
          *CmpCommon::diags() << DgSqlCode(-3242)
                              << DgString0("Advanced feature is not enabled.");
          return -1;
        }
       ...
       (d->getDescribedTableName().getSpecialType() == ExtendedQualName::VIRTUAL_TABLE)))
    {
      rc = 
        CmpDescribeSeabaseTable(d->getDescribedTableName(), 
                                (d->getFormat() == Describe::INVOKE_ ? 1 : 2),
                                outbuf, outbuflen, heap, FALSE, NULL, NULL, TRUE,
                                FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, UINT_MAX, FALSE, FALSE,
...
//                     during display
/////////////////////////////////////////////////////////////////
static Lng32 CmpFormatPlan(ULng32 flags,
                          char *rootTdbBuf, Lng32 rootTdbSize,
                          char *srcStrBuf, Lng32 srcStrSize,
   ...
   NAMemory      *heap)
{
  ...
  retcode = cliInterface.executeImmediatePrepare2
    (query,
     NULL, NULL, NULL, NULL, NULL, NULL, NULL, FALSE, &genCodeSize);
...
{
  CorrName dtName(inDtName);
  ((NAString&)dtName.getQualifiedNameObj().getSchemaName()).toUpper();
  ((NAString&)dtName.getQualifiedNameObj().getObjectName()).toUpper();

  const NAString& tableName =
    dtName.getQualifiedNameObj().getQualifiedNameAsAnsiString();
  ...
  if (!CmpDescribeIsAuthorized(heap, SQLOperation::UNKNOWN,
                               naTable->getPrivInfo(),
  ...
  if ((type == 2) && (isView))
    {
      ...
      NAString expViewtext(naTable->getHiveExpandedViewText());
      expViewtext = expViewtext.strip(NAString::trailing, ';');
      outputLongLine(space, expViewtext, 0);
      outputShortLine(space, " ");

      outputShortLine(space,"Normalized native Hive view text:");
  ...
  for (Int32 i = 0; i < (Int32)naTable->getColumnCount(); i++)
    {
      ...
      if(nac->isHivePartColumn() && type == 2)
          continue;
      ...
      if (!nac->isHiveVirtualColumn() || describeVirtCols)
        {
          ...
          if ((type == 1) || (type == 3))
            ...
            nat->getHiveTypeStrForMyType(&nas);
       ...
       == DF_ON))
  {
       ...
       if (CURRCONTEXT_CLUSTERINFO->getTotalNumberOfCPUs() > 1)
       {
	 NAString saltStr = "  SALT USING ";
	 char numNodes[20];
	 str_itoa(CURRCONTEXT_CLUSTERINFO->getTotalNumberOfCPUs(), numNodes);
	 saltStr += numNodes;
	 saltStr += " PARTITIONS ";
       ...
       == DF_ON))
  {
	 ...
	 NAString saltStr = "  SALT USING ";
	 char numNodes[20];
	 str_itoa(CURRCONTEXT_CLUSTERINFO->getTotalNumberOfCPUs(),
                  numNodes);
	 saltStr += numNodes;
	 saltStr += " PARTITIONS ";
	 outputShortLine(space, saltStr);
  ...
  if (hTabStats)
    {
      ...
      else if (hTabStats->isParquetFile())
        {
          if (type == 1)
            outputShortLine(space, "  /* stored as parquet */");
          else if (type == 2)
            outputShortLine(space, "  stored as parquet ");
        }
      else if (hTabStats->isAvroFile())
        {
          if (type == 1)
            outputShortLine(space, "  /* stored as avro */");
          else if (type == 2)
            outputShortLine(space, "  stored as avro ");
        }
      ...
      if ((type == 2) &&
          (hTabStats->hiveTblParams() && 
           hTabStats->hiveTblParams()->getTblParamsStr() &&
           (strlen(hTabStats->hiveTblParams()->getTblParamsStr()) > 0)))
        {
          NABoolean done = FALSE;
          NABoolean firstTime = TRUE;
          char * start = hTabStats->hiveTblParams()->getTblParamsStr();
          char * str = hTabStats->hiveTblParams()->getTblParamsStr();
          Int32 len = strlen(hTabStats->hiveTblParams()->getTblParamsStr());
          NAString nas;
          while (NOT done)
            {
              nas.clear();
            ...
            label_output:
              // for now, only show properties/parameters that start 
              // with "orc." or "parquet."
              if (NOT (((key.length() >= strlen("orc.")) &&
                        (memcmp(key.data(), "orc.", strlen("orc.")) == 0)) ||
                       ((key.length() >= strlen("parquet.")) &&
                        (memcmp(key.data(), "parquet.", strlen("parquet.")) == 0))))
                {
                  // skip
                  continue;
                }
              ...
              if (firstTime)
                {
                  firstTime = FALSE;
                  outputShortLine(space, "  tblproperties ");
                  outputShortLine(space, "  ( ");
                }

              outputShortLine(space, nas.data());
            } // while NOT done
...
short CmpGenUniqueConstrStr(AbstractRIConstraint * ariConstr,
                            Space *space, char *buf,
                            NABoolean doOutputLine,
                            NABoolean unnamed, const CorrName *likeCorrName)
{
  ...
  sprintf(buf,  "%sALTER TABLE %s ADD %s %s UNIQUE ",
          (doOutputLine ? "\n" : ""),
          (likeCorrName != NULL) ? likeCorrName->getQualifiedNameObj().getQualifiedNameAsAnsiString(TRUE).data() : ansiTableName.data(),
          (unnamed ? "" : "CONSTRAINT"),
          (unnamed ? "" : newUniqueName.data()));
  if (doOutputLine)
    outputLine(*space, buf, 0);
  else
    strcat(buf, " ");
...
// doOutputLine: output in 'space'. Used later to display for invoke/showddl
// unnamed: created unnamed constraint
short CmpGenRefConstrStr(AbstractRIConstraint * ariConstr,
                         Space *space, char *buf,
                         NABoolean doOutputLine,
                         NABoolean unnamed, const CorrName *likeCorrName)
{
  ...
  sprintf(buf,  "%sALTER TABLE %s ADD %s %s FOREIGN KEY ",
          (doOutputLine ? "\n" : ""),
          (likeCorrName != NULL) ? likeCorrName->getQualifiedNameObj().getQualifiedNameAsAnsiString(TRUE).data() : ansiTableName.data(),
          (unnamed ? "" : "CONSTRAINT"),
          (unnamed ? "" : newRefName.data()));
  if (doOutputLine)
    outputLine(*space, buf, 0);
  else
    strcat(buf, " ");
...
short CmpDescribeSeabaseTable ( 
                               ...
                               NABoolean withoutNamespace,
                               NABoolean withoutRegionReplication,
                               NABoolean withoutIncrBackup,
                               UInt32 columnLengthLimit,
                               ...
                               short subType)
{
  ...
  if (dtName.isPartitionV2() && dtName.getSpecialType() == ExtendedQualName::INDEX_TABLE)
  {
    ...
    cliRC = CmpSeabaseDDL::getPartIndexName(&cliInterface,
      dtName.getQualifiedNameObj().getSchemaName(),
      dtName.getQualifiedNameObj().getObjectName(),
      dtName.getPartnClause().getPartitionName(),
      partitionIndexName);
    ...
    const_cast<CorrName&>(dtName) = CorrName(partitionIndexName,
  ...
  //for debugging
  if (type == 2 && naTable->isPartitionEntityTable())
    naTable->displayPartitonV2();

  TableDesc *tdesc = bindWA.createTableDesc(naTable, (CorrName&)dtName);
  if (bindWA.errStatus())
    return -1;

  if ((naTable->getTableType() != ExtendedQualName::VIRTUAL_TABLE) &&
      (NOT naTable->isHbaseTable()))
    {
      if (CmpCommon::diags()->getNumber() == 0)
  ...
  if (type == 1)
    {
      NAString extNameForHbase = CmpSeabaseDDL::genHBaseObjName(
          ...
          dtName.getQualifiedNameObj().getSchemaName(), 
          dtName.getQualifiedNameObj().getObjectName(),
          naTable->getNamespace(), objectDataUID);
      ...
      if (isView)
        ...
        sprintf(buf,  "-- Definition of %s%stable %s\n"
                ...
                "-- Definition current  %s",
                ((storageType == COM_STORAGE_MONARCH) ? "Monarch" : ((storageType == COM_STORAGE_BIGTABLE) ? "Bigtable" : "Trafodion")),
                (isVolatile ? " volatile " : 
                 (isHbaseMapTable ? " HBase mapped " :
                  (isExternalTable ? " external " : " "))), 
                (isHbaseMapTable ? objectName.data() : 
                 tableName.data()),
                 (isPartitionV2Table ? "NO " : ""),
                 (isPartitionV2Table ? "" : extNameForHbase.data()),
                ctime(&tp));
      outputShortLine(*space, buf);
    }
  else if (type == 2)
  ...
  if (bCreateTableLikeOnlyTable && (pkeyStr))
  ...
  else
    {
      if ((naf) &&
          (nonSystemKeyCols > 0) &&
          (NOT isStoreBy) && subType != 1 && subType != 2)
        {
          ...
          if (type == 1)
          ...
          else
            {
              ...
              if (dontDisplay)
                ...
                sprintf(buf, " , CONSTRAINT %s PRIMARY KEY %s",
                        pkeyConstrName.data(),
                        (nullablePkey ? "NULLABLE " : ""));
            }
  ...
  if (type != 1 && subType != 1 && subType != 2)
    {
      ...
      if (((NOT isAudited) || (isAligned)) || encrypt || storedDesc ||
          ...
          ((NOT naTable->defaultUserColFam().isNull()) && 
           (naTable->defaultUserColFam() != SEABASE_DEFAULT_COL_FAMILY)) ||
          ((type == 2) && (NOT naTable->getNamespace().isNull())))
        {
          ...
          if ((NOT formatSet) && isAligned)
            strcat(attrs, "ALIGNED FORMAT ");
          if ((NOT naTable->defaultUserColFam().isNull()) &&
              (naTable->defaultUserColFam() != SEABASE_DEFAULT_COL_FAMILY))
            {
          ...
          if (encrypt)
            {
              sprintf(&attrs[strlen(attrs)], "USE ENCRYPTION OPTIONS '%s%s' ",
                      (naTable->useRowIdEncryption() ? "r" : ""),
      ...
      if (!isView && hbaseCreateOptions &&
          (hbaseCreateOptions->entries() > 0))
        {
          NABoolean bSkipHbaseOptions = FALSE;
          if (withoutRegionReplication && hbaseCreateOptions->entries() ==1)
            {//If we have only one hbase option and the option is REGION_REPLICATION,
             //if we specified without region replication, skip creating hbase options.
              HbaseCreateOption * hco = (*hbaseCreateOptions)[0];
              if (hco->key() == "REGION_REPLICATION")
                bSkipHbaseOptions = TRUE;
            }
                  ...
                  if (i < hbaseCreateOptions->entries() - 1)
                    {
                      ...
                      if (!((i+1 == hbaseCreateOptions->entries() - 1)
                            && withoutRegionReplication
                            && tmp && tmp->key() == "REGION_REPLICATION"))
                        comma = TRUE;
                    }
  ...
  if (((type == 1) && (NOT sqlmxRegr)) || (type == 2) || bCreateTableLikeGetIndexDesc || bCreateTableLikeGetConstraintDesc)
    {
      ...
      for (Int32 i = 0; i < indexList.entries(); i++)
	{
	  ...
	  if (type == 1)
		...
		strcat(vu, "unique ");
          ...
          "-- %sDefinition of hbase table %s\n"
		      ...
		      ((NOT naf->isCreatedExplicitly()) ? " implicit " : " "),
                      ((storageType == COM_STORAGE_MONARCH) ? "Monarch" : ((storageType == COM_STORAGE_BIGTABLE) ? "Bigtable" : "Trafodion")),		      vu,
		      indexName.data(),
          (isPartitionV2Table ? "NO " : ""),
          (isPartitionV2Table ? "" : extNameForHbase.data()),
		      ctime(&tp));
	  ...
	  else
	    {
	      char vu[40];
	      strcpy(vu, " ");
          if (naf->partLocalBaseIndex() && isCreateForPart && type == 3)
            strcat(vu, "PARTITION LOCAL ");
	      if (isVolatile)
		strcat(vu, "VOLATILE ");
	      if (naf->uniqueIndex())
		strcat(vu, "UNIQUE ");
    ...
    if (bCreateTableLikeGetIndexDesc && (NOT naf->isCreatedExplicitly()))
      continue;
    ...
    if(bCreateTableLikeGetIndexDesc)
    {
      ...
      newIndexName = ToAnsiIdentifier(newIndexName);
	      ...
	      sprintf(buf,  "%sCREATE%sINDEX %s ON %s",
		      ...
		      bCreateTableLikeGetIndexDesc ? likeTabName->getQualifiedNameObj().getQualifiedNameAsAnsiString(TRUE).data() : tableName.data());
	      outputLine(*space, buf, 0);
	    }
          ...
          if (type == 2 || bCreateTableLikeGetIndexDesc)
            {
              ...
              if (NOT colString.isNull())
                outputLine(*space, colString.data(), 2);
            } // type == 2
          ...
          for (Int32 i = 0; i < checkList.entries(); i++)
            {
              ...
              if (bCreateTableLikeGetConstraintDesc && likeTabName)
              {
                ...
                {
                  checkConstText = checkConstText.replace(pos, tableName.length(),likeTabName->getQualifiedNameObj().getQualifiedNameAsAnsiString(TRUE));
                  pos = checkConstText.index(tableName.data(), pos+likeTabName->getQualifiedNameObj().getQualifiedNameAsAnsiString(TRUE).length(),NAString::ignoreCase);
                }
              }
              sprintf(buf,  "\nALTER TABLE %s ADD CONSTRAINT %s CHECK %s",
                      bCreateTableLikeGetConstraintDesc ? likeTabName->getQualifiedNameObj().getQualifiedNameAsAnsiString(TRUE).data() : tableName.data(),
                      newCheckName.data(),
                      checkConstText.data());
              outputLine(*space, buf, 0);
              outputShortLine(*space, ";");
                               ...
                               NABoolean sqlCommentOut)
{
  ...
  if (sga->getSGOrder()) //order sequence
  {
    ...
    if (ehi != 0)
    {
      ...
      if (seqVal == 0)
      {
        *CmpCommon::diags() << DgSqlCode(-1582)
                            << DgString0(rowId);
        if (sga)
          NADELETEBASIC(sga, heap);
        return -1;
      }
  ...
  if (sga->getSGType() == COM_INTERNAL_SG)
    ...
    (sga->getSGType() == COM_EXTERNAL_SG)
    {
      strcpy(seqType, " ");
    }
  else if
    (sga->getSGType() == COM_SYSTEM_SG)
    {
   ...
   
{
  ...
  if (owner != NA_UserIdDefault)
    if (ComUser::getCurrentUser() == owner ||
        ComUser::currentUserHasRole(owner))
      return true;
  ...
  if (operation != SQLOperation::UNKNOWN)
    compPrivList.insert(operation);

  bool hasPriv = false;
  if (componentPrivileges.hasSQLPriv(ComUser::getCurrentUser(), compPrivList, hasPriv) < 0)
    return false;
...
PrivMgrCommands privInterface(privMgrMDLoc.data(),CmpCommon::diags());
   ...
   if (CmpCommon::context()->isAuthorizationEnabled())
   {
      ...
      if (!CmpDescribeIsAuthorized(heap, SQLOperation::MANAGE_LIBRARY,&privs,
                                   COM_LIBRARY_OBJECT))
         return false;
   }
...
short CmpDescribePackage (const CorrName   & cn,
                          char           * & outbuf,
                          ULng32           & outbuflen,
                          CollHeap         * heap)
{
  ...
  CmpSeabaseDDL cmpSBD((NAHeap*)heap);
  Int64 objUID = cmpSBD.getObjectUID(&cliInterface,
  ...
  if(CmpSeabaseDDL::getTextFromMD(CmpSeabaseDDL::getSystemCatalogStatic().data(),
                                  &cliInterface,
                                  objUID,
                                  COM_ROUTINE_TEXT,
                          ...
                          CollHeap         * heap)
{
  ...
  ExeCliInterface cliInterface(STMTHEAP, 0, NULL,
      CmpCommon::context()->sqlSession()->getParentQid());
  if (CmpSeabaseDDL::isSPSQLPackageRoutine(cn.getQualifiedNameObj().getObjectName().data(),
                                           &cliInterface, cn.getQualifiedNameObj()) == 1) {
      *CmpCommon::diags() << DgSqlCode(-4459) 
                          << DgString0(rName.data());
      return 0;
  }
...
short CmpDescribeTrigger (const CorrName   & cn,
			  char           * & outbuf,
			  ULng32           & outbuflen,
			  CollHeap         * heap)
{
  ...
  Int64 triggerId = cmpSBD.getObjectUID(&cliInterface,
					triggerName.getCatalogName().data(),
					triggerName.getSchemaName().data(),
					triggerName.getObjectName().data(),
  ...
  if (triggerId < 1)
    {
      *CmpCommon::diags() << DgSqlCode(-4082)
                          << DgTableName(triggerName.getQualifiedNameAsAnsiString(TRUE).data());
      return 0;
    }
			  ...
			  CollHeap         * heap)
{
  ...
  sprintf(buf, "SELECT OBJECT_TYPE, CATALOG_NAME ||'.'|| SCHEMA_NAME ||'.'|| OBJECT_NAME, 0 FROM %s.\"%s\".%s WHERE OBJECT_UID = %ld;", 
                CmpSeabaseDDL::getSystemCatalogStatic().data(),
                SEABASE_MD_SCHEMA,
                SEABASE_OBJECTS,
                uuid);
...
static short CmpDescribeTablePartitions(NATable *naTable, Space &space, char * buf, short type)
{
  ...
  // range list hash
  sprintf(buf, "  PARTITION BY %s (%s)", naTable->partitionSchemeToString((ComPartitioningSchemeV2)partType), colName.data());
  outputShortLine(space, buf);
  ...
  for (Int32 i = 0; i < naTable->FisrtLevelPartitionCount(); i++)
  {
    ...
    if (partArray[i]->hasSubPartition())
      CmpDescribePartitions(partType, partArray[i], space, buf, FALSE, (i == 0));
  }
  outputShortLine(space, "  ) ");
  return 0;
}
...
static short CmpDescribePartitions(Int32 partType, NAPartition* naPartition,  Space &space, char * buf, NABoolean isFirstPartition, NABoolean isFirst)
{
  ...
  if (partType == COM_RANGE_PARTITION)
    sprintf(buf, "  %s%s %s VALUES LESS THAN (%s)", !isFirst ? ", " : "  ", isFirstPartition ? "PARTITION" : "SUBPARTITION", ToAnsiIdentifier(naPartition->getPartitionName()).data(), valueText.data());
  else if (partType == COM_LIST_PARTITION)
    sprintf(buf, "  %s%s %s VALUES (%s)", !isFirst ? ", " : "  ", isFirstPartition ? "PARTITION" : "SUBPARTITION", ToAnsiIdentifier(naPartition->getPartitionName()).data(), valueText.data());
  else
  {
...
static short CmpDescribePartIndexDesc(BindWA *bindWA, short type, NATable *naTable, Space &space, char * buf, const CorrName  &dtName, const CorrName *likeTabName)
{
  ...
  for (Int32 i = 0; i < naTable->FisrtLevelPartitionCount(); i++)
  {
    ...
    for (Int32 j = 0; j < indexList.entries(); j++)
    {
      ...
      if ((naf->getKeytag() == 0) || (naf->partLocalIndex()) || (naf->partGlobalIndex()))
        continue;
      ...
      Lng32 numIndexCols = naf->getCountOfColumns(
                                      TRUE,
                                      TRUE,  // user-specified index cols only
                                      FALSE,
                                      FALSE);
