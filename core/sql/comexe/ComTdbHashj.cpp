...
void ComTdbHashj::displayContents(Space * space,ULng32 flag)
{
  ...
  if (flag & 0x00000008)
	...
	space->allocateAndCopyToAlignedSpace(buf, str_len(buf), sizeof(short));
      ...
      str_sprintf(buf,"extLeftRowLength = %d, extRightRowLength = %d",
                  extLeftRowLength_,extRightRowLength_);
      space->allocateAndCopyToAlignedSpace(buf, str_len(buf), sizeof(short));

      str_sprintf(buf,"minMaxValsAtpIndex = %d, minMaxRowLength = %d",
                  minMaxValsAtpIndex_, minMaxRowLength_);
      space->allocateAndCopyToAlignedSpace(buf, str_len(buf), sizeof(short));
