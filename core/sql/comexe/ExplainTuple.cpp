  ...
  if (e)
    {
      ...
      if (col == ExplainTupleCols::EX_COL_DESCRIPT &&
                 DFS2REC::isAnyCharacter(dataType))
        vcIndicatorLen = CharType::determineVCHdrSize(len *
                         CharInfo::minBytesPerChar(CharInfo::DefaultCharSet));
      if (vcIndicatorLen == 0)
        vcIndicatorLen = ExpTupleDesc::VC_ACTUAL_LENGTH; 
      UInt32 nullOffset = 0;
      UInt32 vcOffset = 0;
