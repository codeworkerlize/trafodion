...
public:
  ...
  enum DDLSafenessType
   {
    SQL_SAFE_DDL = 0,
    SQL_UNSAFE_DDL = 1,
    SQL_DDL_SAFENESS_UNKNOWN = 2
   };
  ...
  NABoolean isAutoCollectRTStats() const 
            { return ((rtFlags2_& AUTO_COLLECT_RTSTATS) != 0);};
  ...
  NABoolean savepointEnabled() 
      {return ((rtFlags1_ & SAVEPOINT_ENABLED) != 0);};
  ...
  { (v ? rtFlags1_ |= MAINTENANCE_WINDOW : rtFlags1_ &= ~MAINTENANCE_WINDOW); }

  NABoolean isMaintenanceWindowOFF() const 
    { return ((rtFlags1_ & MAINTENANCE_WINDOW) == 0);}
  ...
  NABoolean useHdfsWriteLock() const
    {return ((rtFlags2_ & USE_HDFS_WRITE_LOCK) != 0);};
 
  void setUseHdfsWriteLock(NABoolean v)
    { 
      (v ? rtFlags2_ |= USE_HDFS_WRITE_LOCK : rtFlags2_ &= ~USE_HDFS_WRITE_LOCK); 
    }

  void setAutoCollectRTStats(NABoolean v)
   {
     (v ? rtFlags2_ |= AUTO_COLLECT_RTSTATS:
          rtFlags2_ &= ~AUTO_COLLECT_RTSTATS);
   }
  
  NABoolean hiveWriteAccess()
  { return (rtFlags2_ & HIVE_WRITE_ACCESS) != 0; }
  void setHiveWriteAccess(NABoolean v)
  { (v ? rtFlags2_ |= HIVE_WRITE_ACCESS : rtFlags2_ &= ~HIVE_WRITE_ACCESS); }
  ...
  NABoolean useRegionXN()
    {return ((rtFlags2_ & USE_REGION_XN) != 0);}

  void setUseRegionXN(NABoolean v)
    {
      (v ? rtFlags2_ |= USE_REGION_XN : rtFlags2_ &= ~USE_REGION_XN);
    }
  ...
  NABoolean useDlockForSharedCache() 
      {return ((rtFlags5_ & USE_DLOCK_FOR_SHARED_CACHE) != 0);};
  ...
  enum DDLSafenessType getDDLSafeness() 
      { return ddlSafeness_; }
  void setDDLSafeness(enum DDLSafenessType x)
      { ddlSafeness_ = x; }
  ...
  NABoolean hiveAllowSubdirs() const
  { return (rtFlags4_ & HIVE_ALLOW_SUBDIRS) ? TRUE : FALSE; }
  void setHiveAllowSubdirs() { rtFlags4_ |= HIVE_ALLOW_SUBDIRS; }

  Int64 getQueryHash() const { return queryHash_; }

  void setClientMaxStatementPooling(int n) 
  { clientMaxStatementPooling_ = n; }

  Int32 getClientMaxStatementPooling() 
  {  return clientMaxStatementPooling_; }

  void setTriggerTdb(ComTdb* tdb) { triggerTdb_ = tdb; }
  ComTdb* getTriggerTdb() { return triggerTdb_; }

  void setNumTrafReplicas(Int16 n) 
  { numTrafReplicas_ = n; }

  Int16 getNumTrafReplicas() 
