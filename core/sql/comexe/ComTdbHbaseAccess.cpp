...
Long ComTdbHbaseAccess::pack(void * space)
{
  ...
  if (replaceNameByUID_)
    listOfIndexesAndTableForUID_.pack(space);
      ...
      if (getEncryptionInfo())
        {
          ComEncryption::EncryptionInfo &ei = 
            *(ComEncryption::EncryptionInfo*)getEncryptionInfo();

          strcpy(buf, "\nUse Encryption: ");
          space->allocateAndCopyToAlignedSpace(buf, str_len(buf), sizeof(short));

          str_sprintf(buf, "  rowidCipherType = %d, dataCipherType = %d",
                      ei.rowidCipherType, ei.dataCipherType);
          space->allocateAndCopyToAlignedSpace(buf, str_len(buf), sizeof(short));

          str_sprintf(buf, "  encryptionKeyID = %ld ",
                      ei.encryptionKeyID);
          space->allocateAndCopyToAlignedSpace(buf, str_len(buf), sizeof(short));
        }
...
ComTdbHbaseCoProcAccess::ComTdbHbaseCoProcAccess(
  ...
  : ComTdbHbaseAccess(COPROC_,
		      ...
		      baseTableName,
		      projExpr,
		      NULL, rowIdExpr, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
		      0, projRowLen, 0, 0, 0, 0, rowIdLength, 0, rowIdAsciiRowLen, 0, 0, 0, 0,
		      0, projTuppIndex, 0, 0, 0, 0, 0, 0, rowIdTuppIndex, returnedTuppIndex, rowIdAsciiTuppIndex, 
          0, 0, 0, 0, 0, 0, 0,
          tdbListOfRangeRows, NULL, listOfColNames, NULL, NULL,
		      NULL, NULL,
		      workCriDesc,
		      criDescParentDown,
		      criDescParentUp,
