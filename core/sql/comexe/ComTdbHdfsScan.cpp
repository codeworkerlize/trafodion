...
: ComTdb( ComTdb::ex_HDFS_SCAN,
  ...
  serializeLib_(serializeLib),
  skipHeaderCount_(skipHeaderCount),
...
Long ComTdbHdfsScan::pack(void * space)
{
  ...
  for (Lng32 i = 0; i < getHdfsFileInfoList()->numEntries(); i++)
    {
      ...
      hdf->splitUnitBlockLocations_.pack(space);
    }
  ...
  strawscanWebServer_.pack(space);
...
Lng32 ComTdbHdfsScan::unpack(void * base, void * reallocator)
{
  ...
  for (Lng32 i = 0; i < getHdfsFileInfoList()->numEntries(); i++)
    {
      ...
      if (hdf->splitUnitBlockLocations_.unpack(base)) return -1;
    }
  ...
  if (serializeLib_.unpack(base)) return -1;
  ...
  if (strawscanWebServer_.unpack(base)) return -1;
  ...
  if(flag & 0x00000008)
    {
      ...
      if (type_ == TEXT_)
        strcpy(typeStr, "TEXT");
      else if (type_ == SEQUENCE_)
        strcpy(typeStr, "SEQUENCE");
      else if (type_ == ORC_)
        strcpy(typeStr, "ORC");
      else if (type_ == PARQUET_)
        strcpy(typeStr, "PARQUET");
      else if (type_ == AVRO_)
        strcpy(typeStr, "AVRO");
          
      str_sprintf(buf, "type_ = %s", typeStr);
      space->allocateAndCopyToAlignedSpace(buf, str_len(buf), sizeof(short));
	  ...
	  str_sprintf(
	      ...
	      char splitInfo[250]; //increase from 200 to 250
	      strcpy(splitInfo, " ");
        ...
        if (hdfo->getSplitUnitBlockLocations()){
        	strcat(splitInfo, "->");
        	strcat(splitInfo, hdfo->getSplitUnitBlockLocations());
        }
