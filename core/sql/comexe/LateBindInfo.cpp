...
NABoolean LateNameInfo::makeSQLIdentifier(char *invalue, char *outvalue) {
  NABoolean dQuoteSeen = FALSE;

  UInt32 invalueLen = str_len(invalue);
  UInt32 i = 0;
  for (i = 0; i < invalueLen; i++) {
    if (invalue[i] == '"') dQuoteSeen = NOT dQuoteSeen;
                                     ...
                                     NABoolean dQuote)    // IN: if TRUE, parts are double quoted.
{
  ...
  if (strstr(inName, "TRAFODION__")) {
    int nptr = strstr(inName, "TRAFODION__") - inName + strlen("TRAFODION__");
    outBuffer[nptr - 2] = 0;
...
Int16 AnsiName::convertAnsiName(bool doCheck, bool noValidate) {
  ...
  // if noValidate is set, do not validate that a non-quoted part is a
  // valid name.
  if (isError_) return -1;
  ...
  if (*ptr != '\0') {
    ...
    {
      if ((!noValidate) && (!isAlpha8859_1((unsigned char)(*ptr)))) {
        if (*(ptr) != '\\' && *(ptr) != '$')
          return -1;
        else {
          if (*(ptr) == '$') dollarFound = TRUE;
        }
      }
    }
  } else
    ...
    switch (*ptr) {
        ...
        if (!inQuotes) {
          ...
          *partPtr = '\0';
          if (!delimited) {
            ...
            if (doCheck && IsSqlReservedWord(parts_[noOfParts_])) return -1;
          }
          delimited = FALSE;
          noOfParts_++;
          partPtr = parts_[noOfParts_];
          ...
          {
            if ((!noValidate) && (!isAlpha8859_1((unsigned char)(*ptr)))) return -1;
          }
        } else {
          if (partLen < partCheckLen) {
            *tgtPtr = *ptr;
      ...
      default:
        ...
        if (inQuotes)
          ...
          {
            if ((!noValidate) && (NOT isAlNum8859_1((unsigned char)(*ptr)) && (*ptr != '_'))) return -1;
          }
          if (doCheck) {
            c = (char)TOUPPER(*ptr);
          } else
...
Int16 AnsiName::extractParts(int &numParts, char *parts[], bool noValidate) {
  ...
  if (!isValid_) {
    if ((ret = convertAnsiName(FALSE, noValidate)) == -1) return ret;
  }
  numParts = noOfParts_;
  for (i = 0; i < noOfParts_; i++) parts[i] = parts_[i];
  return 0;
}
...
Int16 AnsiName::equals(AnsiName *inName) {
  ...
  if (str_cmp_ne(inName->extName_, extName_) == 0)
    ...
    if (!isValid_) {
      if (convertAnsiName(FALSE) != 0) return -1;
    }
    if (!inName->isValid_) {
      if (inName->convertAnsiName() != 0) return -1;
    }
...
char *AnsiName::getInternalName() {
  if (!isValid_) {
    if (convertAnsiName() != 0) return NULL;
  }
  return intName_;
}
...
Int16 AnsiName::fillInMissingParts(char *schemaName) {
  ...
  if (!isValid_) {
    if (convertAnsiName() != 0) return -1;
  }
  partsToFill = (Int16)(3 - noOfParts_);
  if (partsToFill == 0) return 1;
  ...
  *tgt = '\0';
  ...
  if (retCode != -1 && noOfParts_ != 3) retCode = -1;
  return retCode;
}
...
void LateNameInfo::resetRuntimeFlags() {
  ...
  else if (!isLastUsedNameCompEmbedded())
...
TrafSimilarityTableInfo::TrafSimilarityTableInfo(char *tableName, NABoolean isHive, char *hdfsRootDir, long modTS,
    ...
    : NAVersionedObject(-1),
      ...
      modTS_(modTS),
      numPartnLevels_(numPartnLevels),
      hdfsDirsToCheck_(hdfsDirsToCheck),
      hdfsHostName_(hdfsHostName),
      hdfsPort_(hdfsPort),
      flags_(0) {
  if (isHive) setIsHive(TRUE);
}
...
NABoolean TrafSimilarityTableInfo::operator==(TrafSimilarityTableInfo &o) {
  return ((isHive() == o.isHive()) && (strcmp(tableName(), ((TrafSimilarityTableInfo &)o).tableName()) == 0));
}
...
///////////////////////////////////////////////////////////////////
TrafQuerySimilarityInfo::TrafQuerySimilarityInfo(Queue *siList) : NAVersionedObject(-1), siList_(siList) {}

TrafQuerySimilarityInfo::TrafQuerySimilarityInfo() : NAVersionedObject(-1), siList_(NULL) {}
