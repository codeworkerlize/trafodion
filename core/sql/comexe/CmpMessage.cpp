...
IpcMessageObjSize CmpMessageObj::packIntoBuffer(IpcMessageBufferPtr &buffer, char *strPtr) {
  ...
  if (strPtr != NULL) str_cpy_all((char *)buffer, strPtr, length);
  buffer += length;
...
IpcMessageObjSize CmpMessageObj::packIntoBuffer(IpcMessageBufferPtr &buffer, void *strPtr, int sz) {
  ...
  if (strPtr != NULL) str_cpy_all((char *)buffer, (char *)strPtr, sz);
  buffer += sz;
  return result + sz;
}

void CmpMessageObj::unpackBuffer(IpcConstMessageBufferPtr &buffer, char *&strPtr, CollHeap *h) {
...
CmpMessageObj::CmpMessageObj(IpcMessageObjType t, CollHeap *h) : IpcMessageObj(t, EXECMPIPCVERSION) {
  h_ = h;
...
void CmpMessageObj::unpackMyself(IpcMessageObjType, IpcMessageObjVersion, NABoolean, IpcMessageObjSize,
...
CmpMessageRequestBasic::CmpMessageRequestBasic(MessageTypeEnum t, CollHeap *h) : CmpMessageObj(t, h) {}
...
IpcMessageObjSize CmpMessageRequestBasic::packMyself(IpcMessageBufferPtr &buffer) {
  IpcMessageObjSize size = CmpMessageObj::packMyself(buffer);
  return size;
}

void CmpMessageRequestBasic::unpackMyself(IpcMessageObjType objType, IpcMessageObjVersion objVersion,
                                          NABoolean sameEndianness, IpcMessageObjSize objSize,
                                          IpcConstMessageBufferPtr &buffer) {
  CmpMessageObj::unpackMyself(objType, objVersion, sameEndianness, objSize, buffer);
}

// -----------------------------------------------------------------------
// methods for CmpMessageReplyBasic
// -----------------------------------------------------------------------
CmpMessageReplyBasic::CmpMessageReplyBasic(MessageTypeEnum t, ID id, CollHeap *h) : CmpMessageObj(t, h), request_(id) {}
...
void CmpMessageReplyBasic::unpackMyself(IpcMessageObjType objType, IpcMessageObjVersion objVersion,
                                        NABoolean sameEndianness, IpcMessageObjSize objSize,
                                        IpcConstMessageBufferPtr &buffer) {
  CmpMessageObj::unpackMyself(objType, objVersion, sameEndianness, objSize, buffer);
...
void CmpCompileInfo::getUnpackedFields(char *&sqltext, char *&schemaName) {
  char *base = (char *)this;
...
const short CmpCompileInfo::getRowsetAtomicity() {
  ...
  if (!(flags_ & ROWSET_ATOMICITY_SPECIFIED)) return 0;
...
// ---------------------------------------------------------
CmpDDLwithStatusInfo::CmpDDLwithStatusInfo() : CmpCompileInfo() {
  statusFlags_ = 0;
  step_ = 0;
  substep_ = 0;
  statusFlags_ = 0;
...
void CmpMessageReply::destroyMe() {
  if (dataAllocated_) NADELETEBASIC(data_, outh_);
}

CmpMessageReply::~CmpMessageReply() { destroyMe(); }
...
void CmpMessageReply::unpackMyself(IpcMessageObjType objType, IpcMessageObjVersion objVersion, NABoolean sameEndianness,
                                   IpcMessageObjSize objSize, IpcConstMessageBufferPtr &buffer) {
  CmpMessageReplyBasic::unpackMyself(objType, objVersion, sameEndianness, objSize, buffer);
...
void CmpMessageReplyCode::destroyMe() {
  ...
  if (fragmentDir_) delete fragmentDir_;

  fragmentDir_ = NULL;
}

CmpMessageReplyCode::~CmpMessageReplyCode() { destroyMe(); }

IpcMessageObjSize CmpMessageReplyCode::mypackedLength() {
  if (fragmentDir_ == NULL) return CmpMessageReply::mypackedLength();
...
IpcMessageObjSize CmpMessageReplyCode::packMyself(IpcMessageBufferPtr &buffer) {
  if (fragmentDir_ == NULL) return CmpMessageReply::packMyself(buffer);
...
void CmpMessageRequest::copyToString(char *&dest, CmpMsgBufLenType &sz, char *source, CmpMsgBufLenType sz1) {
...
IpcMessageObjSize CmpMessageRequest::packMyself(IpcMessageBufferPtr &buffer) {
  ...
  if (parentQidLen_ != 0 && parentQid_ != NULL) size += packStrIntoBuffer(buffer, (char *)parentQid_, parentQidLen_);
  return size;
}

void CmpMessageRequest::unpackMyself(IpcMessageObjType objType, IpcMessageObjVersion objVersion,
                                     NABoolean sameEndianness, IpcMessageObjSize objSize,
                                     IpcConstMessageBufferPtr &buffer) {
...
CmpMessageEnvs::CmpMessageEnvs(EnvsOperatorEnum o, char **inEnviron, NABoolean emptyOne, CollHeap *h)
...
void CmpMessageEnvs::unpackMyself(IpcMessageObjType objType, IpcMessageObjVersion objVersion, NABoolean sameEndianness,
                                  IpcMessageObjSize objSize, IpcConstMessageBufferPtr &buffer) {
  CmpMessageObj::unpackMyself(objType, objVersion, sameEndianness, objSize, buffer);
...
CmpMessageEnvs::~CmpMessageEnvs() { destroyMe(); }
...
CmpMessageISPRequest::~CmpMessageISPRequest() { destroyMe(); }
...
void CmpMessageISPRequest::unpackMyself(IpcMessageObjType objType, IpcMessageObjVersion objVersion,
                                        NABoolean sameEndianness, IpcMessageObjSize objSize,
                                        IpcConstMessageBufferPtr &buffer) {
  allocated_ = TRUE;  // so that the allocated storages will be destroyed later.
  CmpMessageRequest::unpackMyself(objType, objVersion, sameEndianness, objSize, buffer);
...
void CmpMessageISPGetNext::unpackMyself(IpcMessageObjType objType, IpcMessageObjVersion objVersion,
                                        NABoolean sameEndianness, IpcMessageObjSize objSize,
                                        IpcConstMessageBufferPtr &buffer) {
  CmpMessageRequest::unpackMyself(objType, objVersion, sameEndianness, objSize, buffer);
...
void CmpMessageReplyISP::unpackMyself(IpcMessageObjType objType, IpcMessageObjVersion objVersion,
                                      NABoolean sameEndianness, IpcMessageObjSize objSize,
                                      IpcConstMessageBufferPtr &buffer) {
  CmpMessageReply::unpackMyself(objType, objVersion, sameEndianness, objSize, buffer);
...
CmpMessageConnectionType::CmpMessageConnectionType(ConnectionTypeEnum t, CollHeap *h)
...
void CmpMessageConnectionType::unpackMyself(IpcMessageObjType objType, IpcMessageObjVersion objVersion,
                                            NABoolean sameEndianness, IpcMessageObjSize objSize,
                                            IpcConstMessageBufferPtr &buffer) {
  CmpMessageRequestBasic::unpackMyself(objType, objVersion, sameEndianness, objSize, buffer);
  int t;
...
void CmpMessageEndSession::unpackMyself(IpcMessageObjType objType, IpcMessageObjVersion objVersion,
                                        NABoolean sameEndianness, IpcMessageObjSize objSize,
                                        IpcConstMessageBufferPtr &buffer) {
  CmpMessageObj::unpackMyself(objType, objVersion, sameEndianness, objSize, buffer);
  ::unpackBuffer(buffer, flags_);
