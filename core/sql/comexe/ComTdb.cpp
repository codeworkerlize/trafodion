  ...
  if(flag & 0x00000008)
    {
		...
		str_sprintf(buf,"Class Version = %d, Class Size = %d",
  		...
  		str_sprintf(buf, "estimatedRowUsed = %0.0f, estimatedRowsAccessed = %0.0f, expressionMode = %d", 
  		...
  		space->allocateAndCopyToAlignedSpace(buf, str_len(buf), sizeof(short));
                ...
                if (criDescDown_ && criDescUp_)
                  {
                    str_sprintf(buf, "criDescDown_->noTuples() = %d, criDescUp_->noTuples() = %d",
                                criDescDown_->noTuples(), criDescUp_->noTuples());
                    space->allocateAndCopyToAlignedSpace(buf, str_len(buf), sizeof(short));
                  }
