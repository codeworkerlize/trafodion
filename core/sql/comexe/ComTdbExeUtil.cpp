  ...
  if(flag & 0x00000008)
    {
      ...
      if (getStringParam1())
        {
          str_sprintf(buf, "stringParam1_ = %s", getStringParam1());
          space->allocateAndCopyToAlignedSpace(buf, str_len(buf), sizeof(short));
        }

      if (getStringParam2())
        {
          str_sprintf(buf, "stringParam2_ = %s", getStringParam2());
          space->allocateAndCopyToAlignedSpace(buf, str_len(buf), sizeof(short));
        }
    }
...
(
 ...
 ULng32 buffer_size)
  : ComTdbExeUtil(ComTdbExeUtil::LOB_SHOWDDL_,
    ...
    numLOBdatafiles_(numLOBdatafiles),
...
NABoolean ComTdbExeUtilLobShowddl::getIsExternalLobCol(short i)
{
  ...
  NABoolean isExternal = (*((Int32*)&getLOBtypeArray()[4*(i-1)]) == Lob_External);
...
////////////////////////////////////////////////////////////////
ComTdbExeUtilAvroStats::ComTdbExeUtilAvroStats
(
     char * tableName,
     ex_expr_base * input_expr,
     ULng32 input_rowlen,
     ...
     Lng32 num_buffers,
     ULng32 buffer_size)
     : ComTdbExeUtil(ComTdbExeUtil::AVRO_STATS_,
		     NULL, 0, (Int16)SQLCHARSETCODE_UNKNOWN,
		     tableName, strlen(tableName),
		     input_expr, input_rowlen,
		     NULL, 0,
       ...
       flags_(0)
{
  setNodeType(ComTdb::ex_AVRO_STATS);
}

Long ComTdbExeUtilAvroStats::pack(void * space)
{
  if (rootDirLoc_) 
    rootDirLoc_.pack(space);
...
Lng32 ComTdbExeUtilAvroStats::unpack(void * base, void * reallocator)
{
  if (rootDirLoc_.unpack(base)) return -1;
...
////////////////////////////////////////////////////////////////
ComTdbExeUtilGetExtSchema::ComTdbExeUtilGetExtSchema
(
     char * tableName,
     short type,
     Queue * colNameList,
     ...
     : ComTdbExeUtil(ComTdbExeUtil::AVRO_STATS_,
       ...
       rootDirLoc_(rootDirLoc),
       type_(type),
       colNameList_(colNameList),
       colTypeList_(colTypeList),
       parqSchStr_(parqSchStr),
       flags_(0)
{
  setNodeType(ComTdb::ex_GET_EXT_SCHEMA);
}

Long ComTdbExeUtilGetExtSchema::pack(void * space)
{
  if (rootDirLoc_) 
    rootDirLoc_.pack(space);

  // pack elements in colNameList
  if (getColNameList())
    {
      colNameList_.pack(space);
    }

  // pack elements in colTypeList
  if (getColTypeList())
    {
      colTypeList_.pack(space);
    }

  parqSchStr_.pack(space);

  return ComTdbExeUtil::pack(space);
}

Lng32 ComTdbExeUtilGetExtSchema::unpack(void * base, void * reallocator)
{
  if (rootDirLoc_.unpack(base)) return -1;

  if (colNameList_.unpack(base, reallocator)) return -1;
  if (colTypeList_.unpack(base, reallocator)) return -1;

  if (parqSchStr_.unpack(base)) return -1;

  return ComTdbExeUtil::unpack(base, reallocator);
}

///////////////////////////////////////////////////////
// class ComTdbExeUtilLobInfo
///////////////////////////////////////////////////////
...
(
     ...
     : ComTdbExeUtil(ComTdbExeUtil::LOB_INFO_,
       ...
       lobLocArray_(lobLocArray),
       lobTypeArray_(lobTypeArray),
       lobInlineMaxLenArray_(lobInlineMaxLenArray),
       hdfsPort_(0),
       hdfsServer_(hdfsServer),
       tableFormat_(tableFormat ? -1 : 0)
{
...
Long ComTdbExeUtilLobInfo::pack(void * space)
{
 ...
 if(lobInlineMaxLenArray_)
   lobInlineMaxLenArray_.pack(space);
...
Lng32 ComTdbExeUtilLobInfo::unpack(void * base, void * reallocator)
{
  ...
  if(lobInlineMaxLenArray_.unpack(base))
    return -1;
