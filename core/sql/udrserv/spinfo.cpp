...
//
//**********************************************************
SPInfo::SPInfo(UdrGlobals *udrGlobals, NAHeap *heapPtr, const UdrHandle &udrHandle, char *pSqlName, char *pExternalName,
               char *pRoutineSig, char *pContainerName, char *pExternalPathName, char *pLibrarySqlName,
               ComUInt32 pNumParams, ComUInt32 pNumInParams, ComUInt32 pNumOutParams, ComUInt32 pMaxRSets,
    ...
    : udrGlobals_(udrGlobals),
      ...
      clientInfo_(NULL),
      rowDiags_(NULL) {
  doMessageBox(udrGlobals_, TRACE_SHOW_DIALOGS, udrGlobals_->showSPInfo_, "SPInfo Constructor");
  ...
  assignStringMember(clientInfo_, clientInfo);
  ...
  sqlBufSize = MAXOF(getRequestBufferSize(), getReplyBufferSize());
  ...
  //
  dataStream_ = new (udrHeapPtr_) UdrServerDataStream(udrGlobals_->getIpcEnvironment(),
                                                      2,           // long sendBufferLimit
                                                      1,           // long inUseBufferLimit
                                                      maxBufSize,  // IpcMessageObjSize bufferSize
                                                      udrGlobals_, this);

  txStream_ = new (udrHeapPtr_) UdrServerReplyStream(udrGlobals_->getIpcEnvironment(), udrGlobals_,
                                                     UDR_STREAM_SERVER_REPLY, UdrServerReplyStreamVersionNumber);

}  // SPInfo::SPInfo()

SPInfo::SPInfo(UdrGlobals *udrGlobals, NAHeap *heapPtr, const UdrHandle &udrHandle)
    : udrGlobals_(udrGlobals),
      udrHandle_(udrHandle),
      lmHandle_(FALSE),
#ifdef UDR_MULTIPLE_CONTEXTS
...
SPInfo::~SPInfo() {
  ...
  if (udrHeapPtr_ == NULL) {
    ...
    if (parentQid_) delete[] parentQid_;
    if (clientInfo_) delete[] clientInfo_;
  } else {
    ...
    if (parentQid_) udrHeapPtr_->deallocateMemory(parentQid_);
    if (clientInfo_) udrHeapPtr_->deallocateMemory(clientInfo_);
  ...
  if (rowDiags_) rowDiags_->decrRefCount();

}  // SPInfo::~SPInfo()

void SPInfo::assignStringMember(char *&memberBuff, const char *const src) {
  if (udrHeapPtr_ != NULL) {
...
void SPInfo::setInParam(ComUInt32 i, const UdrParameterInfo &info) {
  UDR_ASSERT(numInParams_ > i, "An invalid index was passed to SPInfo::setInParam()");
  initLmParameter(info, TRUE);
}

void SPInfo::setOutParam(ComUInt32 i, const UdrParameterInfo &info) {
  UDR_ASSERT(numOutParams_ > i, "An invalid index was passed to SPInfo::setOutParam()");
  initLmParameter(info, FALSE);
}

// setUdrContext
NABoolean SPInfo::setUdrContext(ComDiagsArea &d) const { return TRUE; }  // setUdrContext

// Set corresponding LmParameter using given UdrParamterInfo object
void SPInfo::initLmParameter(const UdrParameterInfo &pInfo, NABoolean isInput) {
  ...
  if (pInfo.isInOut() && isInput == FALSE) {
    lmp.setOutDataInfo(pInfo.getDataOffset(), pInfo.getDataLength(), pInfo.getNullIndicatorOffset(),
                       pInfo.getNullIndicatorLength(), pInfo.getVCLenIndOffset(), pInfo.getVCIndicatorLength());
  }
    ...
    if (charset == CharInfo::ISO88591) charset = udrGlobals_->getIsoMapping();

    if (isInput) {
      lmp.init((ComFSDataType)pInfo.getFSType(), pInfo.getPrec(), pInfo.getScale(), charset,
               (CharInfo::Collation)pInfo.getCollation(), direction, pInfo.isLmObjType(), RS_NONE,
               pInfo.getTupleFormat(), pInfo.getDataOffset(), pInfo.getDataLength(), pInfo.getNullIndicatorOffset(),
               pInfo.getNullIndicatorLength(), pInfo.getNullBitIndex(), pInfo.getVCLenIndOffset(),
               pInfo.getVCIndicatorLength(), pInfo.getVoaOffset(), 0, 0, 0, 0, 0, 0,  // output offsets and lengths
               pInfo.getParamName());
    } else {
      lmp.init((ComFSDataType)pInfo.getFSType(), pInfo.getPrec(), pInfo.getScale(), charset,
               (CharInfo::Collation)pInfo.getCollation(), direction, pInfo.isLmObjType(), RS_NONE,
               pInfo.getTupleFormat(), 0, 0, 0, 0, 0, 0, 0, 0,  // input offsets and lengths
               pInfo.getDataOffset(), pInfo.getDataLength(), pInfo.getNullIndicatorOffset(),
               pInfo.getNullIndicatorLength(), pInfo.getVCLenIndOffset(), pInfo.getVCIndicatorLength(),
               pInfo.getParamName());
    }
...
int SPInfo::releaseSP(NABoolean reportErrors, ComDiagsArea &d) {
  ...
  if (lmr != NULL) {
    ...
    if (lmResult == LM_ERR) {
      if (reportErrors) {
        if (udrGlobals_->verbose_ && udrGlobals_->traceLevel_ >= TRACE_DATA_AREAS && udrGlobals_->showSPInfo_) {
          sprintf(errorText, "[UdrServ (%.30s)]  LM putRoutine call resulted in error.", moduleName);
...
const char *SPInfo::getSPInfoStateString() const {
  switch (spInfoState_) {
    ...
    case INVOKED_EMITROWS:
      return "INVOKED_EMITROWS";
    case INVOKED_GETROWS:
      return "INVOKED_GETROWS";
    case INVOKED_GETROWS_FAILED:
      return "INVOKED_GETROWS_FAILED";
    case UNLOADING:
      return "UNLOADING";
    default:
...
NABoolean SPInfo::setupUdrResultSets(ComDiagsArea &d) {
  ...
  for (ComUInt32 index = 0; index < numRS; index++) {
    ...
    if (rsList_.entries() < index + 1) {
      ...
      if (rsList_[index]->reInit(lmRS, d) != 0) return FALSE;
    }
  }
...
SQLSTMT_ID *SPInfo::executeSqlStmt(const char *sql_str, ComDiagsArea &d) {
  ...
  if (retcode < 0) {
    d << DgSqlCode(-UDR_ERR_INTERNAL_CLI_ERROR) << DgString0("SQL_EXEC_ClearDiagnostics") << DgInt0(retcode);
  ...
  if (retcode < 0) {
    d << DgSqlCode(-UDR_ERR_INTERNAL_CLI_ERROR) << DgString0("SQL_EXEC_AllocStmt") << DgInt0(retcode);
  ...
  if (retcode < 0) {
    d << DgSqlCode(-UDR_ERR_INTERNAL_CLI_ERROR) << DgString0("SQL_EXEC_AllocDesc") << DgInt0(retcode);
  ...
  if (retcode < 0) {
    d << DgSqlCode(-UDR_ERR_INTERNAL_CLI_ERROR) << DgString0("SQL_EXEC_SetDescItem2") << DgInt0(retcode);
  ...
  if (retcode < 0) {
    ...
    d << DgSqlCode(-UDR_ERR_INTERNAL_CLI_ERROR) << DgString0("SQL_EXEC_Prepare") << DgInt0(retcode);
  ...
  if (retcode < 0) {
    ...
    d << DgSqlCode(-UDR_ERR_INTERNAL_CLI_ERROR) << DgString0("SQL_EXEC_Exec") << DgInt0(retcode);
...
void SPInfo::loadUdrResultSet(ComUInt32 index, RSHandle handle, ComUInt32 numRSCols, ComUInt32 rowSize,
                              ComUInt32 bufferSize, UdrParameterInfo *columnDesc, ComDiagsArea &d) {
  const char *moduleName = "SPInfo::loadUdrResultSet";
  UdrResultSet *rs = rsList_[index];

  if (udrGlobals_->verbose_ && udrGlobals_->traceLevel_ >= TRACE_DETAILS && udrGlobals_->showRSLoad_)
    ServerDebug("[Udrserv (%s)] Loading ResultSet at position %d", moduleName, index + 1);

  NABoolean result = rs->load(handle, numRSCols, rowSize, bufferSize, columnDesc, d);

  if (udrGlobals_->verbose_ && udrGlobals_->traceLevel_ >= TRACE_DETAILS && udrGlobals_->showRSLoad_)
    ServerDebug("[Udrserv (%s)] ResultSet load %s", moduleName, (result) ? "succeded" : "failed");
...
UdrResultSet *SPInfo::getUdrResultSetByHandle(RSHandle handle) {
  ...
  for (ComUInt32 i = 0; i < entries; i++) {
    rs = rsList_.at(i);
    if (rs != NULL && rs->getRSHandle() == handle) return rs;
  }
...
NABoolean SPInfo::activateTransaction() {
  ...
  if (udrGlobals_->verbose_) {
    ...
    ServerDebug("           GETTRANSID returned %d. Transid is %Ld.", tmfRetcode, tx_id);
  }
#endif
...
void SPInfo::prepareToReply(UdrServerReplyStream &msgStream) {
  ...
  if (doTrace) ServerDebug("[UdrServ (%s)] Done.", moduleName);
...
void SPInfo::quiesceExecutor() {
  ...
  if (getParamStyle() == COM_STYLE_SQLROW || getParamStyle() == COM_STYLE_SQL) {
    if (doTrace) ServerDebug("         Quiescing is not needed for UDFs.");
  ...
  if (getParamStyle() == COM_STYLE_JAVA_CALL) {
    ...
    if (tmfCode == 0) {
      ...
      if (doTrace) ServerDebug("         Quiesce was successful");
      ...
      char msg[MAXERRTEXT];
      str_sprintf(msg, "Unable to get transaction state. TMF returned error %d", tmfCode);
      if (doTrace) {
        ServerDebug(msg);
...
void SPInfo::work() {
  ...
  if (udrGlobals_->verbose_ && udrGlobals_->showInvoke_ && udrGlobals_->traceLevel_ >= TRACE_DATA_AREAS)
    traceInvokeDataAreas = true;

  doMessageBox(udrGlobals_, TRACE_SHOW_DIALOGS, udrGlobals_->showInvoke_, moduleName);

  // Do we have a request to process?
  if (!currentRequest_) {
    if (traceInvokeIPMS) ServerDebug("[UdrServ (%s)]  No current request buffer to process", moduleName);

    return;
  }

  if (traceInvokeIPMS) ServerDebug("[UdrServ (%s)]  Processing Invoke Request", moduleName);
  ...
  if (reqSqlBuf == NULL) {
    if (traceInvokeIPMS) ServerDebug("[UdrServ (%s)]  Empty request buffer in the request", moduleName);

    dataErrorReply(udrGlobals_, *dataStream_, UDR_ERR_MESSAGE_PROCESSING, INVOKE_ERR_NO_REQUEST_BUFFER, NULL);
  ...
  if (reqSqlBuf->atEOTD()) {
    ...
    if (traceInvokeIPMS)
      ServerDebug(
          "[UdrServ (%s)]  No request rows to process in the"
          " request SQL buffer",
          moduleName);
  ...
  if (traceInvokeIPMS)
    ServerDebug("[UdrServ (%s)]  There are %d tupps in the request buffer", moduleName, initialNumRequestTupps);

  // Does stream have room for next buffer?
  if (dataStream_->sendLimitReached()) {
    if (traceInvokeIPMS) ServerDebug("[UdrServ (%s)]  Send limit reached in the data stream", moduleName);
    return;
  }

  // Allocate a reply buffer
  UdrDataBuffer *reply =
      new (*dataStream_, getReplyBufferSize()) UdrDataBuffer(getReplyBufferSize(), UdrDataBuffer::UDR_DATA_OUT, NULL);

  if (reply == NULL || reply->getSqlBuffer() == NULL) {
    dataErrorReply(udrGlobals_, *dataStream_, UDR_ERR_MESSAGE_PROCESSING, INVOKE_ERR_NO_REPLY_DATA_BUFFER, NULL);

    if (traceInvokeIPMS) ServerDebug("[UdrServ (%s)] Send Invoke Error Reply", moduleName);
  ...
  if (!activateTransaction()) dataStream_->activateCurrentMsgTransaction();
  ...
  if (status == ALL_ROWS_PROCESSED || reqSqlBuf->atEOTD()) {
    ...
    if (traceInvokeDataAreas) ServerDebug("[UdrServ (%s)] Processed all rows from the request buffer.", moduleName);
  ...
  if (numRowsProcessed > 0) {
    if (traceInvokeIPMS) ServerDebug("[UdrServ (%s)] Send Invoke Reply", moduleName);
...
RequestRowProcessingStatus SPInfo::processOneRequestRow(SqlBuffer *reqSqlBuf, SqlBuffer *replySqlBuf,
  ...
  if (!replyRowAllocated) {
    ...
    // allocation failed.
    if (numTuppsBefore == 0) {
      dataErrorReply(udrGlobals_, *dataStream_, UDR_ERR_MESSAGE_PROCESSING, INVOKE_ERR_NO_REPLY_ROW, NULL);
  ...
  tupp requestRow;
  NABoolean endOfData = reqSqlBuf->moveOutSendOrReplyData(TRUE,        // [IN] sending? (vs. replying)
  ...
  if (rowDiags_ == NULL) rowDiags_ = ComDiagsArea::allocate(udrGlobals_->getIpcHeap());
  UDR_ASSERT(rowDiags_, "Unable to allocate memory for SQL diagnostics area");

  if (traceInvokeIPMS) ServerDebug("[UdrServ (%s)]  Call LM invokeRoutine", moduleName);

  // Call into Language manager
  LmLanguageManager *lm = udrGlobals_->getLM(getLanguage());
  UDR_ASSERT(lm, "No Language Manager available to process this INVOKE message");
  UDR_ASSERT(getLMHandle(), "Missing LM Routine handle");

  LmResult lmResult = lm->invokeRoutine(getLMHandle(), requestData, replyData, rowDiags_);

  if (traceInvokeIPMS) {
    char errorText[MAXERRTEXT];
    if (lmResult == LM_OK)
      sprintf(errorText, "[UdrServ (%.30s)]  LM::invokeRoutine was successful.", moduleName);
    else
      sprintf(errorText, "[UdrServ (%.30s)]  LM::invokeRoutine resulted in error.", moduleName);

    ServerDebug(errorText);
    doMessageBox(udrGlobals_, TRACE_SHOW_DIALOGS, udrGlobals_->showInvoke_, errorText);
  }
  ...
  if (lmResult == LM_OK) {
    ...
    if (getParamStyle() != COM_STYLE_SQLROW) {
      ...
      if (getParamStyle() == COM_STYLE_JAVA_CALL) setupUdrResultSets(*rowDiags_);
    }
    ...
    NABoolean ok = convertReplyRowToErrorRow(replySqlBuf, numTuppsBefore, requestQueueIndex, *dataStream_, udrGlobals_);
  ...
  if (getNumResultSets() > 0) {
    ...
    if (!rsMoveOk) {
      ...
      dataErrorReply(udrGlobals_, *dataStream_, UDR_ERR_MESSAGE_PROCESSING, INVOKE_ERR_NO_REPLY_BUFFER, NULL);
...
void SPInfo::workTM() {
  ...
  if (spInfoState_ == INVOKED_EMITROWS) {
    ...
    if (traceInvokeIPMS) ServerDebug("[UdrServ (%s)]  Continue request Received", moduleName);
  ...
  if (traceInvokeDataAreas) {
    ...
    SqlBuffer *tempSqlBuffer = (SqlBuffer *)udrHeapPtr_->allocateMemory(getRequestBufferSize());
  ...
  if (sqlBufCopy == NULL) {
    ...
    sqlBufCopy = (SqlBuffer *)udrHeapPtr_->allocateMemory(getRequestBufferSize());
  }
  ...
  if (currentRequest_->isLastBuffer()) setLastReqSqlBuffer(currentRequest_->tableIndex());
  ...
  up_state upState;
  tupp requestRow;
  NABoolean endOfData = sqlBufCopy->moveOutSendOrReplyData(TRUE,        // [IN] sending? (vs. replying)
  ...
  LmResult lmResult = lm->invokeRoutine(getLMHandle(), requestData, NULL, rowDiags_);
  ...
  if (paramStyle_ != COM_STYLE_CPP_OBJ && paramStyle_ != COM_STYLE_JAVA_OBJ) {
...
NABoolean SPInfo::moveRSInfoIntoStream() {
  ...
  // improvement.
  UdrRSInfoMsg *rsInfoMsg = new (udrGlobals_->getIpcHeap()) UdrRSInfoMsg(getNumResultSets(), udrGlobals_->getIpcHeap());

  if (rsInfoMsg == NULL) return false;
  ...
  for (ComUInt32 i = 0; i < getNumResultSets(); i++) {
    ...
    ResultSetInfo rsInfo(udrRS->getProxySyntax(), udrRS->getStatementHandle(), udrRS->getContextHandle(),
...
void SPInfo::moveDiagsIntoStream(ComDiagsArea *diags, ControlInfo *replyControlInfo) {
  if (diags && diags->getNumber() > 0) {
    if (replyControlInfo) replyControlInfo->setIsExtDiagsAreaPresent(TRUE);
...
LmParameter &SPInfo::getLmParameter(ComUInt32 i) {
  UDR_ASSERT(i < (numParameters_ + maxNumResultSets_), "An invalid index was passed to SPInfo::getLmParameter()");
...
void SPInfo::setTableInputInfo(const UdrTableInputInfo info[]) {
  for (ComUInt32 i = 0; i < numTableInfo_; i++) {
    ...
    for (ComUInt32 j = 0; j < tableInfo->numInColumns_; j++) {
      ...
      tableInfo->inColumnParam_[j].init(
          (ComFSDataType)pInfo.getFSType(), pInfo.getPrec(), pInfo.getScale(), charset,
          (CharInfo::Collation)pInfo.getCollation(), COM_INPUT_COLUMN, pInfo.isLmObjType(), RS_NONE,
          pInfo.getTupleFormat(), pInfo.getDataOffset(), pInfo.getDataLength(), pInfo.getNullIndicatorOffset(),
          pInfo.getNullIndicatorLength(), pInfo.getNullBitIndex(), pInfo.getVCLenIndOffset(),
          pInfo.getVCIndicatorLength(), pInfo.getVoaOffset(), 0, 0, 0, 0, 0, 0,  // output offsets and lengths
          pInfo.getParamName());
...
ComUInt32 SPInfo::getInputRowLength(ComSInt32 tableIndex) { return tableInfo_[tableIndex].getRequestRowSize(); }
...
NABoolean SPInfo::isLastReqSqlBuffer(ComSInt32 tableIndex) { return tableInfo_[tableIndex].isLastReqSqlBuffer(); }
