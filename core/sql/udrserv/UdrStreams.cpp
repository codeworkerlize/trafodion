...
extern void processAnRSFetchMessage(UdrGlobals *udrGlob, UdrServerDataStream &msgStream, UdrHandle udrHandle,
                                    RSHandle rsHandle, UdrDataBuffer *request);

extern void processAnRSContinueMessage(UdrGlobals *udrGlob, UdrServerDataStream &msgStream, UdrRSContinueMsg &request);
...
void UdrServerDataStream::actOnReceive(IpcConnection *conn) {
    ...
    while (getNextObjType(t)) {
      switch (t) {
        ...
        case UDR_MSG_DATA_REQUEST: {
          ...
          if (udrGlob_->verbose_ && udrGlob_->traceLevel_ >= TRACE_DETAILS && udrGlob_->showMain_) {
            ServerDebug("");
        ...
        case UDR_MSG_TMUDF_DATA_HEADER: {
          ...
          UDR_ASSERT(t == UDR_MSG_DATA_REQUEST, "DATA_REQUEST must follow TMUDF DATA_HEADER");
        ...
        case UDR_MSG_RS_DATA_HEADER: {
          ...
          processAnRSFetchMessage(udrGlob_, *this, udrHandle, rsHandle, request);
          ...
          UDR_ASSERT(sqlBuf, "UDR request buffer is corrupt or contains no data");
          sqlBuf->bufferFull();
        } break;

        case UDR_MSG_RS_CONTINUE: {
          UdrRSContinueMsg *rsContinue = new (receiveMsgObj()) UdrRSContinueMsg(this);
        ...
        default: {
          UDR_ASSERT(FALSE, "Unknown message type arrived on UDR data stream");
        }  // default
        break;
  ...
  if (somethingArrived && numInputBuffers == 0 && spinfo_->getCurrentRequest() == NULL) {
    ...
    if (udrGlob_->verbose_ && udrGlob_->traceLevel_ >= TRACE_IPMS && udrGlob_->showMain_) {
...
void UdrGuaControlConnection::actOnSystemMessage(short messageNum, IpcMessageBufferPtr sysMsg,
  ...
  switch (messageNum) {
    case ZSYS_VAL_SMSG_OPEN: {
      if (udrGlob_->verbose_ && udrGlob_->traceLevel_ >= TRACE_IPMS && udrGlob_->showMain_) {
        ServerDebug("[UdrServ (%s)] A new connection %p is opened to the Server.", moduleName, connection);
      }

      // Create a new message stream. Link the connection.
      // Then call receive on it to bring it into receiving mode.
      UdrServerControlStream *newControlStream = new (udrGlob_->getIpcHeap()) UdrServerControlStream(
          udrGlob_->getIpcEnvironment(), udrGlob_, UDR_STREAM_SERVER_CONTROL, UdrServerControlStreamVersionNumber);
...
void UdrServerControlStream::actOnReceive(IpcConnection *conn) {
  const char *moduleName = "UdrServerControlStream::actOnReceive";

  NABoolean doTrace = (udrGlob_->verbose_ && udrGlob_->traceLevel_ >= TRACE_IPMS && udrGlob_->showMain_) ? TRUE : FALSE;
  if (doTrace) {
  ...
  UdrServerReplyStream *newReplyStream = new (udrGlob_->getUdrHeap()) UdrServerReplyStream(
      udrGlob_->getIpcEnvironment(), udrGlob_, UDR_STREAM_SERVER_REPLY, UdrServerReplyStreamVersionNumber);
  ...
  if (doTrace) {
    ...
    ServerDebug("        The connection %p will be placed on receive mode.", conn);
