...
LmLanguageManager *UdrGlobals::getOrCreateLM(LmResult &result, ComRoutineLanguage language, ComDiagsArea *diags) {
  switch (language) {
    case COM_LANGUAGE_JAVA:
...
JOI_RetCode UdrGlobalsJavaObj::initJVMForUDRServ(LmJavaOptions *options) { return initJVM(options); }

LmLanguageManagerJava *UdrGlobals::getOrCreateJavaLM(LmResult &result, ComDiagsArea *diags) {
  result = LM_OK;

  if (javaLanguageManager_) return javaLanguageManager_;
  ...
  if (!cliGlobals) cliGlobals = CliGlobals::createCliGlobals(TRUE);
  ...
  if (!javaLanguageManager_) result = LM_ERR;
...
LmLanguageManagerC *UdrGlobals::getOrCreateCLM(LmResult &result, ComDiagsArea *diags) {
  if (cLanguageManager_) {
    result = LM_OK;
    return cLanguageManager_;
  }

  cLanguageManager_ = new (getUdrHeap()) LmLanguageManagerC(result, commandLineMode_, diags);
  ...
  if (!cLanguageManager_) result = LM_ERR;
