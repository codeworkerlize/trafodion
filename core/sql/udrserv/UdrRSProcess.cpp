...
void processAnRSLoadMessage(UdrGlobals *udrGlob, UdrServerReplyStream &msgStream, UdrRSLoadMsg &request) {
  ...
  doMessageBox(udrGlob, TRACE_SHOW_DIALOGS, udrGlob->showRSLoad_, moduleName);

  NABoolean showRSLoadLogic = (udrGlob->verbose_ && udrGlob->traceLevel_ >= TRACE_IPMS && udrGlob->showRSLoad_);
  ...
  if (sp == NULL) {
    controlErrorReply(udrGlob, msgStream, UDR_ERR_MISSING_UDRHANDLE, 0, "RS Load Message");
    return;
  }

  // Check if RS index in the request is valid. The RS index above UDR
  // server is 1-based.
  if (request.getRsIndex() < 1 || request.getRsIndex() > sp->getNumResultSets()) {
    controlErrorReply(udrGlob, msgStream, UDR_ERR_INVALID_RS_INDEX, request.getRsIndex(), "Load");
    return;
  }
  ...
  sp->loadUdrResultSet(request.getRsIndex() - 1, request.getRSHandle(), request.getNumRSColumns(), request.getRowSize(),
                       request.getBufferSize(), request.getColumnDesc(), *diags);

  // Prepare reply and send it
  msgStream.clearAllObjects();

  UdrRSLoadReply *reply = new (udrGlob->getIpcHeap()) UdrRSLoadReply(udrGlob->getIpcHeap());

  if (reply == NULL) {  // no reply msg built...
    controlErrorReply(udrGlob, msgStream, UDR_ERR_MESSAGE_PROCESSING, RS_ERR_NO_REPLY_MSG, NULL);
    return;
  }
  ...
  if (diags && diags->getNumber() > 0) {
    ...
    msgStream << *diags;
    if (showRSLoadLogic) dumpDiagnostics(diags, 2);
  }
...
void processAnRSFetchOrContinueMessage(UdrGlobals *udrGlob, UdrServerDataStream &msgStream, UdrHandle udrHandle,
                                       RSHandle rsHandle, UdrDataBuffer *request, UdrIpcObjectType requestType) {
  ...
  NABoolean doTrace = (udrGlob->verbose_ && udrGlob->traceLevel_ >= TRACE_IPMS && show) ? TRUE : FALSE;

  if (doTrace) ServerDebug("[UdrServ (%s)]  Receive RS Invoke or Continue Request", moduleName);
  ...
  if (sp == NULL) {
    if (requestType == UDR_MSG_RS_DATA_HEADER)
      dataErrorReply(udrGlob, msgStream, UDR_ERR_MISSING_UDRHANDLE, 0, "RS Fetch Message");
    else
      dataErrorReply(udrGlob, msgStream, UDR_ERR_MISSING_UDRHANDLE, 0, "RS Continue Message");
  ...
  if (requestType == UDR_MSG_RS_CONTINUE && (udrRS->isClosed() || udrRS->isFetchComplete())) {
    if (diags) diags->decrRefCount();
  ...
  if ((requestType == UDR_MSG_RS_DATA_HEADER && (!udrRS->isLoaded() && !udrRS->isReInited())) ||
      (requestType == UDR_MSG_RS_CONTINUE && (!udrRS->isFetchStarted() && !udrRS->isEarlyClose()))) {
    // TBD: This does not work correctly. The error takes two char params
    // but we are sending only one. Need to fix this.
    dataErrorReply(udrGlob, msgStream, UDR_ERR_INVALID_RS_STATE, 0, udrRS->stateString());
  ...
  UdrDataBuffer *reply =
      new (msgStream, udrRS->getBufferSize()) UdrDataBuffer(udrRS->getBufferSize(), UdrDataBuffer::UDR_DATA_OUT, NULL);
  if (reply == NULL || reply->getSqlBuffer() == NULL) {  // could not allocate UdrDataBuffer
    dataErrorReply(udrGlob, msgStream, UDR_ERR_MESSAGE_PROCESSING, RS_ERR_NO_REPLY_DATA_BUFFER, NULL);

    sendDataReply(udrGlob, msgStream, NULL);

    if (reply) reply->decrRefCount();
  ...
  if (numRows == -1) {
    ...
    if (rowDiagsList) {
      for (ComUInt32 i = 0; i < rowDiagsList->entries(); i++) (*rowDiagsList)[i]->decrRefCount();
  ...
  if (diags && diags->getNumber() > 0) {
    msgStream << *diags;
    if (doTrace) dumpDiagnostics(diags, 2);

    if (diags->getNumber(DgSqlCode::ERROR_) > 0) {
      udrGlob->numErrUDR_++;
      udrGlob->numErrSP_++;
      if (requestType == UDR_MSG_RS_DATA_HEADER) udrGlob->numErrRSFetch_++;
      if (requestType == UDR_MSG_RS_CONTINUE) udrGlob->numErrRSContinue_++;
    }
  }
  ...
  if (udrRS->isFetchComplete()) reply->setLastBuffer(TRUE);
...
void processAnRSFetchMessage(UdrGlobals *udrGlob, UdrServerDataStream &msgStream, UdrHandle udrHandle,
                             RSHandle rsHandle, UdrDataBuffer *request) {
  processAnRSFetchOrContinueMessage(udrGlob, msgStream, udrHandle, rsHandle, request, UDR_MSG_RS_DATA_HEADER);
  return;
}

void processAnRSContinueMessage(UdrGlobals *udrGlob, UdrServerDataStream &msgStream, UdrRSContinueMsg &request) {
  ...
  processAnRSFetchOrContinueMessage(udrGlob, msgStream, request.getHandle(), request.getRSHandle(), NULL,
                                    UDR_MSG_RS_CONTINUE);
  return;
}
...
void processAnRSCloseMessage(UdrGlobals *udrGlob, UdrServerReplyStream &msgStream, UdrRSCloseMsg &request) {
  ...
  doMessageBox(udrGlob, TRACE_SHOW_DIALOGS, udrGlob->showRSClose_, moduleName);

  NABoolean doTrace = (udrGlob->verbose_ && udrGlob->traceLevel_ >= TRACE_IPMS && udrGlob->showRSClose_) ? TRUE : FALSE;
  if (doTrace) {
  ...
  SPInfo *sp = udrGlob->getSPList()->spFind(request.getHandle());
  if (sp == NULL) {
    controlErrorReply(udrGlob, msgStream, UDR_ERR_MISSING_UDRHANDLE, 0, "RS Close Message");
    return;
  }
  ...
  if (!sp->activateTransaction()) msgStream.activateCurrentMsgTransaction();
  ...
  UdrRSCloseReply *reply = new (udrGlob->getIpcHeap()) UdrRSCloseReply(udrGlob->getIpcHeap());
  if (reply == NULL) {
    controlErrorReply(udrGlob, msgStream, UDR_ERR_MESSAGE_PROCESSING, RS_ERR_NO_REPLY_MSG, NULL);
    return;
  }
...
void processAnRSUnloadMessage(UdrGlobals *udrGlob, UdrServerReplyStream &msgStream, UdrRSUnloadMsg &request) {
  ...
  doMessageBox(udrGlob, TRACE_SHOW_DIALOGS, udrGlob->showRSUnload_, moduleName);

  if (udrGlob->verbose_ && udrGlob->traceLevel_ >= TRACE_IPMS && udrGlob->showRSUnload_) {
  ...
  SPInfo *sp = udrGlob->getSPList()->spFind(request.getHandle());
  if (sp == NULL) {
    controlErrorReply(udrGlob, msgStream, UDR_ERR_MISSING_UDRHANDLE, 0, "RS Unload Message");
    return;
  }
  ...
  UdrRSUnloadReply *reply = new (udrGlob->getIpcHeap()) UdrRSUnloadReply(udrGlob->getIpcHeap());
