...
static void deleteDescItems(ComUInt32 numCols, SQLDESC_ITEM *desc_items, NAMemory *heapPtr) {
  ComUInt32 index = 0;
...
//
//////////////////////////////////////////////////////////////////////////////
char *TmpBuffer::getNextRow(ComDiagsArea *&rowDiags, NABoolean singleRowFetchEnabled) {
  char *retptr = NULL;
  ...
  if (moreRowsToCopy()) {
    ...
    while (cliDiags_->getNumber(DgSqlCode::WARNING_)) {
      ...
      if (rowNumberFromCli > rowNumber) {
        ...
        if (rowNumberFromCli == rowNumber || singleRowFetchEnabled) {
          if (rowDiags == NULL) rowDiags = ComDiagsArea::allocate(heap_);
    ...
    if (numRows_ == index_) numRows_ = index_ = 0;
  }
...
UdrResultSet::UdrResultSet(SPInfo *spInfo, SQLSTMT_ID *stmt, ComDiagsArea &d)
  ...
  UDR_DEBUG1("  Proxy syntax \"%s\"", (proxySyntax_ ? proxySyntax_->data() : ""));
}
...
UdrResultSet::UdrResultSet(SPInfo *spInfo, LmResultSet *lmRS, ComDiagsArea &d)
  ...
  UDR_DEBUG1("  SPInfo %p", spInfo_);
  UDR_DEBUG1("  LmResultSet %p", lmResultSet_);
  if (stmt_id_) UDR_DEBUG1("  Stmt name %s", stmt_id_->identifier);
  UDR_DEBUG1("  Proxy syntax \"%s\"", (proxySyntax_ ? proxySyntax_->data() : ""));
}
...
UdrResultSet::~UdrResultSet() {
  ...
  if (lmResultSet_) getSPInfo()->getLMHandle()->cleanupLmResultSet(lmResultSet_);
...
void UdrResultSet::deallocateUDRGeneratedFields() {
  ...
  if (prevContext != 0) SQL_EXEC_SwitchContext_Internal(prevContext, NULL, TRUE);
...
void UdrResultSet::deallocateExeGeneratedFields() {
  rsHandle_ = INVALID_RS_HANDLE;
  numColumns_ = 0;
  exeRowSize_ = 0;
  bufferSize_ = 0;
...
const char *UdrResultSet::stateString() {
  switch (state_) {
    case RS_INITIATED:
      return "RS_INITIATED";
    case RS_REINITIATED:
      return "RS_REINITIATED";
    case RS_LOADED:
      return "RS_LOADED";
    case RS_UNLOADED:
      return "RS_UNLOADED";
    case RS_FETCH:
      return "RS_FETCH";
    case RS_FETCH_COMPLETE:
      return "RS_FETCH_COMPLETE";
    case RS_CLOSED:
      return "RS_CLOSED";
    case RS_EARLY_CLOSE:
...
int UdrResultSet::reInit(LmResultSet *lmRS, ComDiagsArea &d, SQLSTMT_ID *stmt_id) {
  ...
  if (generateProxySyntax(d) != 0) return -1;
...
// by UDR invocation.
void UdrResultSet::prepareForReinvoke() {
  deallocateUDRGeneratedFields();
...
NABoolean UdrResultSet::load(RSHandle handle, ComUInt32 numRSCols, ComUInt32 rowSize, ComUInt32 bufferSize,
  ...
  if (!isInited() && !isReInited() && !isUnloaded()) {
    d << DgSqlCode(-UDR_ERR_INVALID_RS_STATE) << DgString0("Load") << DgString1(stateString());
  ...
  if (udrGlob->verbose_ && udrGlob->traceLevel_ >= TRACE_DETAILS && udrGlob->showRSLoad_) {
    ServerDebug("  UdrResultSet %p loaded ", this);
    ServerDebug("    SPInfo %p", spInfo_);
    ServerDebug("    LmResultSet %p", lmResultSet_);
    if (stmt_id_) ServerDebug("    Stmt name %s", stmt_id_->identifier);
    ServerDebug("    Proxy syntax \"%s\"", (proxySyntax_ ? proxySyntax_->data() : ""));
...
SQLSTMT_ID *UdrResultSet::copyStatementID(SQLSTMT_ID *srcStmtID, NABoolean resetStmtInfo) {
  if (srcStmtID == NULL) return NULL;
  ...
  str_cpy_all(module_name, srcModuleID->module_name, srcModuleID->module_name_len);
...
int UdrResultSet::resetContext(SQLCTX_HANDLE ctxHandle, ComDiagsArea &d) {
  ...
  UDR_ASSERT(tmpCtxHandle == (SQLCTX_HANDLE)getContextHandle(), "Wrong context Handle.");
...
int UdrResultSet::generateProxySyntax(ComDiagsArea &d) {
  ...
  if (state_ != RS_INITIATED && state_ != RS_REINITIATED) {
    ...
    d << DgSqlCode(-UDR_ERR_INVALID_RS_STATE) << DgString0("Initiate") << DgString1(stateString());

    return -1;
  }

  SQLCTX_HANDLE prevContext = 0;
  if (setContext(prevContext, d) != 0) return -1;

  if (stmt_id_ == NULL) {
    if (lmResultSet_ == NULL)
      d << DgSqlCode(-UDR_ERR_INTERNAL_ERROR) << DgString0("LmResultSet object is not set");
    else
      stmt_id_ = copyStatementID((SQLSTMT_ID *)lmResultSet_->getStmtID());
  }
  ...
  if (retcode < 0) {
    d << DgSqlCode(-UDR_ERR_INTERNAL_CLI_ERROR) << DgString0("SQL_EXEC_DescribeStmt") << DgInt0(retcode);
  ...
  if (numColumns <= 0) {
    d << DgSqlCode(-UDR_ERR_INVALID_RS_COLUMN_COUNT) << DgInt0(numColumns);
  ...
  SQLDESC_ITEM *desc_items = new (collHeap()) SQLDESC_ITEM[numColumns * NUMDESC_ITEMS];
  ...
  for (index = 0; index < numColumns; index++) {
    ...
    if (catalog_name.num_val_or_len != 0) proxy += ToAnsiIdentifier(catalog_name.string_val) + dot;

    if (schema_name.num_val_or_len != 0) proxy += ToAnsiIdentifier(schema_name.string_val) + dot;

    if (table_name.num_val_or_len != 0) proxy += ToAnsiIdentifier(table_name.string_val) + dot;
    ...
    if (heading.num_val_or_len > 0) ToQuotedString(quotedHeading, heading.string_val);
...
void UdrResultSet::populateColumnDesc(UdrParameterInfo *columnDesc) {
  // Store UdrParameterInfo objects
  rsColumnDesc_ = (UdrParameterInfo *)collHeap()->allocateMemory(numColumns_ * sizeof(UdrParameterInfo));
  memset(rsColumnDesc_, 0, numColumns_ * sizeof(UdrParameterInfo));

  rsColDescForLM_ = (LmParameter *)collHeap()->allocateMemory(numColumns_ * sizeof(LmParameter));
  ...
  for (ComUInt32 index = 0; index < numColumns_; index++) {
    ...
    CharInfo::CharSet charset = (CharInfo::CharSet)udrColumn->getEncodingCharSet();
    if (charset == CharInfo::ISO88591) charset = spInfo_->getUdrGlobals()->getIsoMapping();

    p->init((ComFSDataType)udrColumn->getFSType(), udrColumn->getPrec(), udrColumn->getScale(), charset,
            (CharInfo::Collation)udrColumn->getCollation(), COM_OUTPUT_COLUMN,
            FALSE,                                                         // objMap
            RS_NONE, udrColumn->getTupleFormat(), 0, 0, 0, 0, 0, 0, 0, 0,  // input lengths and offsets
            udrColumn->getDataOffset(), udrColumn->getDataLength(), udrColumn->getNullIndicatorOffset(),
            udrColumn->getNullIndicatorLength(), udrColumn->getVCLenIndOffset(), udrColumn->getVCIndicatorLength(),
            NULL);  // parameter name
  }
...
void UdrResultSet::deallocateColumnDesc() {
...
int UdrResultSet::fetchRows(UdrGlobals *udrGlob, SqlBuffer *requestBuffer, SqlBuffer *replyBuffer,
  ...
  SQLCTX_HANDLE prevContext = 0;
  if (setContext(prevContext, mainDiags) != 0) return -1;
  ...
  if (tmpBuffer_ == NULL) tmpBuffer_ = new (collHeap()) TmpBuffer(bufferSize_, exeRowSize_, collHeap());
  ...
  if (tmpBuffer_->moreRowsToCopy()) {
    copyRowsIntoSqlBuffer(replyBuffer, parentQueueIndex_, numBufferedRows, rowDiagsList);
  }
  ...
  if (!(tmpBuffer_->moreRowsToCopy())) {
    ...
    if (lmResultSet_->moreSpecialRows()) {
      fetchRowsFromJDBC(udrGlob, replyBuffer,
                        numBufferedRows,  // in/out value
                        eofEncountered, mainDiags, rowDiagsList);
    }
    ...
    if (mainDiags.getNumber(DgSqlCode::ERROR_) == 0 && !eofEncountered && !(tmpBuffer_->moreRowsToCopy())) {
      fetchRowsFromCLI(udrGlob, replyBuffer,
                       numBufferedRows,  // in/out value
                       mainDiags, rowDiagsList);
    }
  }
  // Restore the previous context
  if (prevContext != 0) {
    if (resetContext(prevContext, mainDiags) != 0) return -1;
  }
...
void UdrResultSet::fetchRowsFromJDBC(UdrGlobals *udrGlob, SqlBuffer *replyBuffer, ComUInt32 &numBufferedRows,
                                     NABoolean &eofEncountered, ComDiagsArea &mainDiags,
  ...
  while (lmResultSet_->moreSpecialRows()) {
    ...
    if (numRows == -1) {
      ...
      UDR_ASSERT((numRows == 1) || (numRows == 0),
                 "Language Manager returned more than one row, or found an exception or found other errors.");
    ...
    if (tmpBuffer_->moreRowsToCopy()) {
      ...
      copyRowsIntoSqlBuffer(replyBuffer, parentQueueIndex_, numRowsCopied, rowDiagsList);
      if (numRowsCopied == 0)
...
//         TRUE  for success
NABoolean UdrResultSet::setupQuadFields(ComDiagsArea &d) {
  // We can reuse the quad_fields_ that were setup earlier.
  if (quad_fields_ != NULL) return TRUE;
  ...
  for (ComUInt32 index = 0; index < numColumns_; index++) {
    ...
    if (vcIndLen > 0) {
      ...
      UDR_ASSERT((vcLenOffset + vcIndLen) == dataOffset, "VARCHAR indicator must immediately precede data");
...
void UdrResultSet::fetchRowsFromCLI(UdrGlobals *udrGlob, SqlBuffer *replyBuffer, ComUInt32 &numBufferedRows,
  ...
  if (setupQuadFields(mainDiags) == FALSE) return;
  ...
  if (stmt_id_->tag != 0) {
    ...
    if (retcode < 0) {
      mainDiags << DgSqlCode(-UDR_ERR_INTERNAL_CLI_ERROR) << DgString0("SQL_EXEC_DisassocFileNumber")
                << DgInt0(retcode);
  ...
  //  - tmpBuffer_ is empty
  while (!needToCopyEODRow_ && !tmpBuffer_->moreRowsToCopy()) {
    retcode = SQL_EXEC_ClearDiagnostics(stmt_id_);
    if (retcode < 0) {
      mainDiags << DgSqlCode(-UDR_ERR_INTERNAL_CLI_ERROR) << DgString0("SQL_EXEC_ClearDiagnostics") << DgInt0(retcode);
    ...
    if (retcode != 0 && retcode != 100) {
      ...
      if (retcode >= 0) {
        ...
        d.unpackObj(messageObjType, messageObjVersion, TRUE, bufSizeNeeded, bufForPackedDiags);
        ...
        if (fetchRetcode < 0 && d.getNumber(DgSqlCode::ERROR_) == 0) {
          ...
          mainDiags << DgSqlCode(-UDR_ERR_INTERNAL_CLI_ERROR) << DgString0("SQL_EXEC_Fetch") << DgInt0(fetchRetcode);
          needToCopyErrorRow_ = needToCopyEODRow_ = TRUE;
          continue;
        }
      } else {
        ...
        mainDiags << DgSqlCode(-UDR_ERR_INTERNAL_CLI_ERROR) << DgString0("SQL_EXEC_MergeDiagnostics_Internal")
                  << DgInt0(retcode);
    ...
    if (numRowsFetched > 0 && udrGlob->verbose_ && udrGlob->traceLevel_ >= TRACE_DATA_AREAS && udrGlob->showInvoke_) {
      ...
      ServerDebug("[UdrServ (%s)] N rows %ld, exe row size %u, buf size %u", moduleName, numRows, rowLen, tmpBufLen);

      // Display either the full buffer, or N+1 rows, whichever region
      // is smaller
      ComUInt32 displayBytes = (numRows + 1) * rowLen;
      if (displayBytes > tmpBufLen) displayBytes = tmpBufLen;
  ...
  if (!tmpBuffer_->moreRowsToCopy()) {
    ...
    if (needToCopyErrorRow_) {
      if (copyErrorRowIntoSqlBuffer(replyBuffer, parentQueueIndex_, mainDiags)) needToCopyErrorRow_ = FALSE;
    }
...
void UdrResultSet::copyRowsIntoSqlBuffer(SqlBuffer *replyBuffer, queue_index queueIndex, ComUInt32 &numRowsCopied,
                                         NAList<ComDiagsArea *> *rowDiagsList) {
  ...
  while (tmpBuffer_->moreRowsToCopy()) {
    ...
    if (result) {
      ...
      char *srcRowPtr = tmpBuffer_->getNextRow(rowDiags, singleRowFetchEnabled_);
...
NABoolean UdrResultSet::copyErrorRowIntoSqlBuffer(SqlBuffer *replyBuffer, queue_index queueIndex,
                                                  ComDiagsArea &mainDiags) {
  UdrGlobals *glob = getSPInfo()->getUdrGlobals();
  NABoolean errorRowAllocOK = allocateErrorRow(glob, *replyBuffer, queueIndex, TRUE);

  // Add error condition to main diags
  if (errorRowAllocOK) mainDiags.mergeAfter(*tmpBuffer_->getDiags());

  return errorRowAllocOK;
}  // UdrResultSet::copyErrorRowIntoSqlBuffer

NABoolean UdrResultSet::copyEODRowIntoSqlBuffer(SqlBuffer *replyBuffer, queue_index queueIndex) {
  UdrGlobals *glob = getSPInfo()->getUdrGlobals();
  NABoolean eodRowAllocOK = allocateEODRow(glob, *replyBuffer, parentQueueIndex_);
...
char *UdrResultSet::fixDataRow(char *rowPtr, ComDiagsArea *rowDiags) {
...
void UdrResultSet::close(ComDiagsArea *diags) {
  // release UDR resources
  deallocateUDRGeneratedFields();

  // Cleanup LmResultSet object
  if (lmResultSet_) getSPInfo()->getLMHandle()->cleanupLmResultSet(lmResultSet_, diags);
  lmResultSet_ = NULL;
