...
public class HBulkLoadClient
{
  ...
  HFile.Writer writer;
  HFile.Writer writer_sample;
  String hFileLocation;
  String hFileName;
  String hFileLocation_Sample;
  String hFileName_Sample;
  ...
  public boolean initHFileParams(String hFileLoc, String hFileNm, long userMaxSize /*in MBs*/, String tblName,
                                 String hFileLocSample, String hFileNmSample, float sampleRate)
  throws UnsupportedOperationException, IOException, SQLException, ClassNotFoundException
  {
    ...
    hFileName = hFileNm;
    hFileLocation_Sample="";
    hFileName_Sample="";
    fSampleRate = sampleRate;
    if ( sampleRate>0 )
      {
        hFileLocation_Sample = hFileLocSample;
        hFileName_Sample = hFileNmSample;
      }

    // hFileName cannot contain character ":".
    // This could happen if the provided name is for a namespace object.
    // Replace ":" by "_"
    hFileLocation = hFileLocation.replace(":", "_");
    hFileName = hFileName.replace(":", "_");
    if ( hFileLocation_Sample.length() > 0 )
      hFileLocation_Sample = hFileLocation_Sample.replace(":", "_");
    if ( hFileName_Sample.length() > 0 )
     hFileName_Sample = hFileName_Sample.replace(":", "_");
  ...
  public boolean doCreateHFile() throws IOException, URISyntaxException
  {
    ...
    if (hFileName_Sample.length()>0)
    {
      Path hfilePathSample = new Path(new Path(hFileLocation_Sample ), hFileName_Sample + "_" +  System.currentTimeMillis());
      hfilePathSample = hfilePathSample.makeQualified(hfilePathSample.toUri(), null);

      if (logger.isDebugEnabled()) logger.debug("HBulkLoadClient createSampleHFile Path: " + hfilePathSample);

      writer_sample = HFile.getWriterFactoryNoCache(config)
                           .withPath(fileSys, hfilePathSample)
                           .withFileContext(hfileContext)
                           .withComparator(KeyValue.COMPARATOR)
                           .create();
      if (logger.isDebugEnabled()) logger.debug("HBulkLoadClient.createSampleHFile Path: " + writer_sample.getPath() + "Created");
    }
    return true;
  }
                ...
                Object rows) throws IOException, URISyntaxException
  {
    ...
    if (isNewFileNeeded())
    {
     ...
     HTableClient htc = new HTableClient(bigTable);
     long now = System.currentTimeMillis();
     Random rand = new Random(now);
     int nRate = (int)(fSampleRate*10000);
     for (short rowNum = 0; rowNum < numRows; rowNum++) 
     {
        ...
        if ( nRate > 0 )
          bSample = (rand.nextInt(10000) <= nRate);
     ...
     MasterNotRunningException, IOException, SnapshotCreationException, InterruptedException, RestoreSnapshotException
  {
    ...
    String prepLocation2 = prepLocation.replace(":", "_");
    Path prepPath = new Path(prepLocation2);
