...
public class FunctionOra extends Function {
  ...
  public void register(Function f) {  
    ...
    f.map.put("DBMS_LOG.PUT_LINE", new FuncCommand() { public void run(HplsqlParser.Expr_func_paramsContext ctx) {
      dbmsLogPutLine(ctx); }});
  ...
  void dbmsOutputPutLine(HplsqlParser.Expr_func_paramsContext ctx) {
    if (ctx.func_param().size() > 0) {
      String msg = evalPop(ctx.func_param(0).expr()).toString();
      if (exec.conf.dbmsOutputToLog) {
        LOG.info(msg);
      }
      else {
  ...
  /**
   * Print a text message to the log
   */
  void dbmsLogPutLine(HplsqlParser.Expr_func_paramsContext ctx) {
    if (ctx.func_param().size() > 0) {
      LOG.info(evalPop(ctx.func_param(0).expr()).toString());
    }
  }
  ...
  void utlFileGetLine(HplsqlParser.Expr_func_paramsContext ctx) {
    ...
    Var file = null;
    Var str = null;
    int len = 65535;            // default line size
    ...
    if(cnt > 2) {
      visit(ctx.func_param(2).expr());
      len = exec.stackPop().intValue();
    }
