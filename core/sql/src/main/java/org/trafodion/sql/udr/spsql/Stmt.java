...
public class Stmt {
  ...
  private static final Logger LOG = Logger.getLogger(Stmt.class);
  ...
  public Integer declareCursor(HplsqlParser.Declare_cursor_itemContext ctx) { 
    ...
    Cursor cursor = new Cursor(name, query);
    if (ctx.create_routine_params() != null) {
      cursor.setCursorParametersContext(ctx.create_routine_params());
    }
   ...
   */
  public Integer createTemporaryTable(HplsqlParser.Create_temporary_table_stmtContext ctx) { 
    if (ctx.T_GLOBAL() != null) {
      trace(ctx, "CREATE GLOBAL TEMPORARY TABLE");
    } else {
  ...
  public Integer createTemporaryTable(HplsqlParser.IdentContext identCtx, HplsqlParser.Create_table_definitionContext defCtx,
    ...
    if (conf.tempTables == Conf.TempTables.NATIVE) {
      if (exec.getConnectionType() == Conn.Type.TRAFODION) {
        sql.append("CREATE VOLATILE TABLE " + name);
      } else {
  ...
  public Integer describe(HplsqlParser.Describe_stmtContext ctx) {
    ...
    String sql;
    if (exec.getConnectionType() == Conn.Type.TRAFODION) {
      sql = "SHOWDDL " + evalPop(ctx.table_name()).toString();
    } else {
      sql = "DESCRIBE " + evalPop(ctx.table_name()).toString();
    }
    trace(ctx, sql);
  ...
  public Integer drop(HplsqlParser.Drop_stmtContext ctx) { 
    ...
    String sql = null;    
    if (ctx.T_TABLE() != null) {
      if (ctx.T_TEMPORARY() != null || ctx.T_VOLATILE() != null) {
        if (exec.getConnectionType() == Conn.Type.TRAFODION) {
          sql = "DROP VOLATILE TABLE ";
        } else {
    ...
    else if (ctx.T_SCHEMA() != null) {
      sql = "DROP SCHEMA ";
      if (ctx.T_EXISTS() != null) {
        sql += "IF EXISTS ";
      }
      sql += evalPop(ctx.expr()).toString();
      if (ctx.T_CASCADE() != null) {
        sql += " CASCADE";
      }
      else if (ctx.T_RESTRICT() != null) {
        sql += " RESTRICT";
      }
    }
    else if (ctx.T_FUNCTION() != null) {
        exec.dropFunction(ctx.ident().getText());
    }
    else if (ctx.T_PROCEDURE() != null) {
        exec.dropProcedure(ctx.ident().getText());
    }
    else if (ctx.T_PACKAGE() != null) {
        exec.dropPackage(ctx.ident().getText());
    }
    else if (ctx.T_TRIGGER() != null) {
      exec.dropTrigger(ctx.ident().getText());
    }
    if (sql != null) {
      trace(ctx, sql);
      Query query = exec.executeSql(ctx, sql, exec.conf.defaultConnection);
  ...
  public Integer open(HplsqlParser.Open_stmtContext ctx) { 
    ...
    Query query = null;
    Var var = null;
    String cursor = ctx.ident().getText();
    String sql = null;
    if (ctx.T_FOR() != null) {                             // SELECT statement or dynamic SQL
      ...
      query = new Query(sql);
      Cursor cur = new Cursor(cursor, query);
      var = exec.findCursor(cursor);                      // Can be a ref cursor variable
      if (var == null) {
        var = new Var(cursor, Type.CURSOR, cur);
        exec.addVariable(var);
      }
      else {
        var.setValue(cur);
      ...
      if (var != null && var.type == Type.CURSOR) {
        Cursor cur = (Cursor)var.value;
        cur.generateQuery(exec, ctx.expr_func_params());
        query = cur.getQuery();
        if (query.isOpen()) {
          exec.signal(new SQLException("cursor " + cursor + " already open"));
          return 1;
        }
      }
    }
    if (query != null) {
      ...
      exec.signal(new SQLException("Cursor not found: " + cursor));
      return 1;
    }
    return 0; 
  }
  ...
  public Integer fetch(HplsqlParser.Fetch_stmtContext ctx) { 
    ...
    if (cursor == null) {
      exec.signal(new SQLException("Cursor not found: " + name));
      return 1;
    }  
    else if (cursor.value == null) {
      exec.signal(new SQLException("Cursor not open: " + name));
      return 1;
    }  
    else if (exec.getOffline()) {
      exec.setSqlCode(100);
    ...
    try {
      Query query = ((Cursor)cursor.value).getQuery();
      ResultSet rs = query.getResultSet();
      ...
      if(rs != null && rsm != null) {
        ...
        exec.signal(new SQLException("Cursor not open or invalid: " + name));
      }
    } 
    catch (SQLException e) {
      trace(ctx, "Cursor fetch failed with exception: " + e);
      exec.setSqlCode(e);
  ...
  public Integer close(HplsqlParser.Close_stmtContext ctx) { 
    ...
    if(var != null && var.type == Type.CURSOR) {
      exec.closeQuery(((Cursor)var.value).getQuery(), exec.conf.defaultConnection);
      exec.setSqlCode(0);
    }
    else {
      exec.signal(new SQLException("Cursor not found: " + name));
   ...
   */
  public Integer closePrepare(HplsqlParser.Close_prepare_stmtContext ctx) { 
    trace(ctx, "CLOSE PREPARE");
    String name = ctx.L_ID().toString();
    if (exec.conn.prepareSqls.containsKey(name)) {
      exec.closePrepare(name);
    }
    else {
      exec.signal(new SQLException("Prepared statement not found: " + name));
      return 1;
    }
    return 0; 
  }
  ...
  public Integer insertSelect(HplsqlParser.Insert_stmtContext ctx) { 
    trace(ctx, "INSERT OR UPSERT SELECT");
    StringBuilder sql = new StringBuilder();
    if (ctx.T_INSERT() != null) {
        sql.append(ctx.T_INSERT().getText() + " ");
    }
    else {
        sql.append(ctx.T_UPSERT().getText() + " ");
        if (ctx.T_USING() != null && ctx.T_LOAD() != null) {
            sql.append(ctx.T_USING().getText() + " " + ctx.T_LOAD().getText() + " ");
        }
    }
  ...
  public Integer insertValues(HplsqlParser.Insert_stmtContext ctx) { 
    trace(ctx, "INSERT OR UPSERT VALUES");
    ...
    for (int i = 0; i < rows; i++) {
      ...
      for (int j = 0; j < cols; j++) {         
        ...
        String value;
        try {
          value = evalPop(row.expr(j)).toString();
        } catch (Exception e) {
          throw e;
        } finally {
  ...
  public Integer use(HplsqlParser.Use_stmtContext ctx) {
    trace(ctx, "USE");
    return use(ctx, meta.normalizeIdentifierPart(evalPop(ctx.expr()).toString()));
  }

  public Integer use(ParserRuleContext ctx, String schema) {
    String sql = null;
    if (exec.getConnectionType() == Conn.Type.TRAFODION) {
  ...
  public Integer forCursor(HplsqlParser.For_cursor_stmtContext ctx) { 
    ...
    if (ctx.select_stmt() != null) {
      ...
      Var var = exec.findVariable(cursorName);
      if (var != null && var.type == Type.CURSOR) {
        Cursor cur = (Cursor)var.value;
        cur.generateQuery(exec, ctx.expr_func_params());
        Query query = cur.getQuery();
        if (query.isOpen()) {
          exec.signal(new SQLException("cursor " + cursor + " already open"));
          return 1;
        }
    ...
    try {
      ...
      if (rs != null) {
        ...
        Row row = new Row();
        for (int i = 1; i <= cols; i++) {
          row.addColumn(rm.getColumnName(i),
  ...
  public Integer exec(HplsqlParser.Exec_stmtContext ctx) { 
    if (exec.conf.executeIdentAsProcedure && execProc(ctx)) {
      return 0;
    }
    Query query = null;
    // exec prepared stmt
    ...
    if (exec.conn.prepareSqls.containsKey(name)) {
      trace(ctx, "EXECUTE PREPARED SQL");
      int usingListCount = 0;
      if (ctx.using_clause() != null) {
        usingListCount = ctx.using_clause().expr().size();
      }
      ArrayList paramList = new ArrayList();
      for (int i = 0; i < usingListCount; i++) {
        paramList.add(evalPop(ctx.using_clause().expr(i)).toString());
      }
      query = exec.executePreparedSql(ctx, name, paramList, exec.conf.defaultConnection);
      if (query.error()) {
        exec.signal(query);
        return 1;
      }
    ...
    if (rs != null) {
      try {
        ResultSetMetaData rm = rs.getMetaData();
        if (ctx.T_INTO() != null) {
          int cols = ctx.ident().size();
          exec.setRowCount(0);
          if (rs.next()) {
            if (!rs.isLast()) {
              exec.signal(Signal.Type.SQLEXCEPTION,
                          "more than one row was returned",
                          new SQLException("more than one row was returned"));
              return 1;
            }
            for (int i = 0; i < cols; i++) {
              String into_name = ctx.ident(i).getText();
            ...
            // No data found
            exec.setSqlCode(100);
            exec.signal(Signal.Type.NOTFOUND, "execute immediate into statement not found data.");
          }
        }
        // Print the results
        else {
   ...
   */
  public Integer beginTransaction(HplsqlParser.Begin_transaction_stmtContext ctx) {
    trace(ctx, "BEGIN TRANSACTION");
    String sql = null;
    if (exec.getConnectionType() == Conn.Type.TRAFODION) {
      sql = "BEGIN WORK";
    } else {
   ...
   * COMMIT statement
   */
  public Integer commit(HplsqlParser.Commit_stmtContext ctx) {
    trace(ctx, "COMMIT");
    String sql = "COMMIT";
    Query query = exec.executeSql(ctx, sql, exec.conf.defaultConnection);
    if (query.error()) {
   ...
   * ROLLBACK statement
   */
  public Integer rollback(HplsqlParser.Rollback_stmtContext ctx) {
    trace(ctx, "ROLLBACK");
    String sql = "ROLLBACK";
    Query query = exec.executeSql(ctx, sql, exec.conf.defaultConnection);
    if (query.error()) {
  ...
  public Integer update(HplsqlParser.Update_stmtContext ctx) {
    ...
    String table = evalPop(ctx.update_table().table_name()).toString();
    ...
    boolean oldBuildSql = exec.buildSql;
    exec.buildSql = true;
    sb.append(evalPop(ctx.update_assignment()).toString());
    if (ctx.where_clause() != null) {
      sb.append(" ");
      sb.append(evalPop(ctx.where_clause()).toString());
    }
    exec.buildSql = oldBuildSql;
  ...
  public Integer delete(HplsqlParser.Delete_stmtContext ctx) {
    ...
    if (ctx.delete_alias() != null) {
        table += " ";
        table += evalPop(ctx.delete_alias()).toString();
    }
    StringBuilder sql = new StringBuilder();
    if (ctx.T_ALL() == null) {
  ...
  public Integer print(HplsqlParser.Print_stmtContext ctx) { 
    ...
    // TODO: maybe we can mock a resultset for this
    String sql = "SELECT " + evalPop(ctx.expr()).toSqlString();
    if (ctx.expr() != null &&
        ctx.expr().expr_atom() != null &&
        ctx.expr().expr_atom().ident() != null) {
      sql += " AS ";
      String ident = ctx.expr().expr_atom().ident().getText();
      if (ident.startsWith("\"")) {
        sql += ident;
      } else {
        sql += "\"" + ident.toUpperCase() + "\"";
      }
    }
    sql += " FROM DUAL";
    Query query = exec.conn.executeSql(sql, exec.conf.defaultConnection);
    if (query.error()) {
      exec.signal(query);
      exec.closeQuery(query, exec.conf.defaultConnection);
      return 1;
   ...
   * PREPARE Statement
   */
  public Integer prepare(HplsqlParser.Prepare_stmtContext ctx) {
    trace(ctx, "PREPARE");
    String prepareSqlName = ctx.L_ID().toString();
    String sql = evalPop(ctx.expr()).toString();
    Query query = exec.prepareSql(ctx, sql, prepareSqlName, exec.conf.defaultConnection);
    if (query.error()) {
      exec.signal(query);
      return 1;
    }
