...
public class HDFSClient 
{
      ...
      public Object call() throws IOException 
      {
         ...
         if (sequenceFile_) {
            ...
            // native layer to process the data
             if ( (fs_ instanceof org.apache.hadoop.fs.LocalFileSystem) || 
                  ((! alluxioNotInstalled_) && fs_ instanceof alluxio.hadoop.FileSystem)) {
               savedBuf_ = buf_;
  ...
  //  c) An exception is encountered while reading the file
   ...
   boolean hdfsCreate(String fname , boolean overwrite, boolean append, boolean compress) throws IOException
   {
      ...
      if (! fileExists)
          {
              retcode = fs_.createNewFile(filepath_); 
              if (retcode)
                  logger_.info("HdfsFile " + filepath_ + " created");
          }
      else
          {
              retcode = true;
              logger_.info("HdfsFile " + filepath_ + " already exists");
          }
   ...
   boolean hdfsOpen(String fname , boolean compress) throws IOException
   {
    ...
    long hdfsWriteImmediate(byte[] buff, boolean doRetry, boolean useLock, int timeout) throws IOException, KeeperException
    {
        ...
        while (true)
        {
          ...
          { 
             ...
             if (lock != null) {
                if (lock.lock(timeout) == -1)   
                    {
                       if (logger_.isDebugEnabled()) logger_.debug("Could not get distributed lob lock :"+lockName);  
                        throw new IOException("HDFSClient.hdfsWriteImmediate -- lock " + lockName + "fail due to timeout"); 
                    }
                locked = true; 
             }
             if (fs_.exists(filepath_)) {
             ...
             fsOut.write(buff);
             fsOut.hflush();
             if (logger_.isDebugEnabled()) logger_.debug("HdfsWrite called with length : "+ buff.length); 
             writeEndOffset = fsOut.getPos(); 
          } catch (org.apache.hadoop.hdfs.protocol.AlreadyBeingCreatedException |org.apache.hadoop.hdfs.protocol.RecoveryInProgressException| org.apache.hadoop.ipc.RemoteException re) {
              logger_.info(re.getMessage(),re);
               try {
                   Thread.sleep(30000);
              ...
              if (logger_.isDebugEnabled())logger_.debug("IO exception from hflush ",e);
              logger_.info(e.getMessage(),e);
              throw e;
              }finally {
              try {
                  if (fsOut != null )
                  ...
                  if (logger_.isDebugEnabled()) logger_.debug("Exception from fsOut.close ",e);
                  logger_.info(e.getMessage(),e);
                   throw e;
              }
              finally {                 
          ...
          if (doRetry && (buff.length != writeEndOffset-writeOffset)) {
              trycount++;
              if (trycount == 3) {
                 throw new IOException("HDFSwriteImmediate did not succeed due to concurrent writes ");    
              }
           }
           else 
              break;
   ...
   public static boolean hdfsDeletePath(String pathStr) throws IOException
   {
      ...
      boolean retcode = fs.delete(delPath, true);
      if (retcode)
         logger_.info("HdfsFile " + delPath + " deleted");
      else
         logger_.info("HdfsFile " + delPath + " NOT deleted");
      return retcode;
   }

   public static boolean hdfsTruncateFile(String fname) throws IOException
   {
       boolean retcode = false;
       
       if (logger_.isDebugEnabled()) 
           logger_.debug("HDFSClient.hdfsTruncateFile() - start - File: " + fname);
       Path truncPath = new Path(fname);
       FileSystem fs = FileSystem.get(truncPath.toUri(), config_);
       if (fs.exists(truncPath))
           {
               fs.delete(truncPath, true);
           }
       
       retcode = fs.createNewFile(truncPath);
   ...
   {
      logger_.info("HDFSClient.hdfsDeleteFiles(" + dirPathStr + ", " + startingFileName +")");
   ...
   private static FileSystem getFileSystem(String path) throws IOException{
      FileSystem result = defaultFs_;  
      // For HDFS, please make sure that the following property:
      //  fs.hdfs.impl.disable.cache is set to false (default IS false)
      // else the FileSystem.get() call could take a long time.
      // 
      if ( path.startsWith("s3a://") || 
           path.startsWith("alluxio://") ||
           path.startsWith("s3://") ||
          ...
          try
          {
           result = FileSystem.get(new URI(path), config_);
          }
   ...
   // if levelDeep == -1, include all subdirectories under stableDirPaths, for HIVE_SUPPORTS_SUBDIRECTORIES
   static public long getHiveTableMaxModificationTs( String stableDirPaths, int levelDeep) throws FileNotFoundException, IOException
   {
       long result = 0;
       if (logger_.isDebugEnabled())
       ...
       String samplePath = tableDirPaths[0];
       FileSystem fs = getFileSystem(samplePath);
       // account for root dir
       for (int i=0; i<tableDirPaths.length; i++) {
           FileStatus r = fs.getFileStatus(new Path(tableDirPaths[i]));// super fast API, return in .2ms
           if (r != null && r.getModificationTime() > result)
               result = r.getModificationTime();
       }

       if ((levelDeep>0)||(levelDeep==-1))
       {
           ...
           long l;
	   if (levelDeep>0)
	       l = getHiveTableMaxModificationTs2(paths,levelDeep,fs);
	   else
	       l= getHiveTableMaxModificationTsSubdirs(paths,fs);
           if (l > result)
              result = l;
       }
       if (logger_.isDebugEnabled())
   ...
   private static long getHiveTableMaxModificationTs2(Path[] paths, int levelDeep, FileSystem fs)throws FileNotFoundException, IOException
   {
       long result = 0;
       ...
       if (levelDeep == 1){ // stop condition on recursive function
           ...
           for (Path path : paths){
               FileStatus r = fs.getFileStatus(path);// super fast API, return in .2ms
               if (r != null && r.getModificationTime()>result)
                   result = r.getModificationTime();
           }
           if (paths.length==1)
               fileStatuss = fs.listStatus(paths[0],filter);// minor optimization. avoid using list based API when not needed
           else
               fileStatuss = fs.listStatus(paths,filter);
           ...
           long l = getHiveTableMaxModificationTs2(pathList.toArray(new Path[pathList.size()]),levelDeep-1,fs);
    ...
    private static long getHiveTableMaxModificationTsSubdirs(Path[] paths, FileSystem fs)throws FileNotFoundException, IOException
   {
       long result = 0;
       PathFilter filter = new PathFilter(){
           public boolean accept(Path file){
	       return !file.getName().startsWith(".") && !file.getName().startsWith("_") ;//filter out hidden files and directories
           }
       };
       FileStatus[] fileStatuss=null;
       //check parent level (important for deletes):
       ...
       if (pathList.size() > 0) {
       long l = getHiveTableMaxModificationTsSubdirs(pathList.toArray(new Path[pathList.size()]),fs);
	if (l>result) result = l;
       }  
       return result;
   }
   ...
   public static boolean hdfsRename(String fromPathStr, String toPathStr) throws IOException 
   {
    ...
    public static int getNumFiles(String rootDir, boolean allFiles) 
	throws IOException
    {
        if (! allFiles)
            return getNumValidFiles(rootDir);
	...
	FileSystem fs = FileSystem.get(path.toUri(), config_);
        try {
            ...
            return result; // numFiles is zero
        }
   ...
   public static void main(String[] args) throws Exception
   {
      ...
      String x = client.getFsDefaultName();
      System.out.println("fs default name=" + x);
   }
