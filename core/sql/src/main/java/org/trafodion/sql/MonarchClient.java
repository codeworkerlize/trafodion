...
/*
        ...
        String logFile = System.getenv("TRAF_LOG") + "/trafodion.sql.java.log";
    	if (confFile == null) {
	...
	config.addResource("ampool-site.xml");
        config.set(Constants.MClientCacheconfig.ENABLE_META_REGION_CACHING, "true");
        config.set(Constants.MClientCacheconfig.MONARCH_CLIENT_LOG, logFile);
        clientCache = new MClientCacheFactory().create(config);
*/
    }
...
*/
    public boolean exists(String tblName)  {
            ...
            boolean result = true;
            try {
               MTable table = MConnection.getTable(tblName);
               if (table == null)
                  result = false;
            }
                  ...
                  boolean useTRex, boolean bSynchronized) throws IOException 
    {
       if (logger.isTraceEnabled()) 
          logger.trace("MonarchClient.getMTableClient(" + tblName + 
                     ...
                     + ") called.");
       MTableClient  mTable = new MTableClient();
