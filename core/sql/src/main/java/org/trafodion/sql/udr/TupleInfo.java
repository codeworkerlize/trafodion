            ...
            case DECIMAL_UNSIGNED:
            {
                ...
                else
                {
                    try { 
                        ...
                        if (parts.length == 2)
                            for (int i = parts[1].length(); i < t.getScale(); i++) {
                                parts[1] += "0";
                            }
    ...
    public void setTime(int colNum, Date val) throws UDRException {
        ...
        else
        {
            ...
            case DATE:
                // yyyy-mm-dd
                dfStr = "yyyy-MM-dd";
                df = new SimpleDateFormat (dfStr);
                resultDate = df.format(val);
                break;
                
            case TIME:
                dfStr = "HH:mm:ss" + scaleFormat;
                df = new SimpleDateFormat (dfStr);
                resultDate = df.format(val);
                break;
                
            case TIMESTAMP:
                dfStr = "yyyy-MM-dd HH:mm:ss" + scaleFormat;
                df = new SimpleDateFormat (dfStr);
                resultDate = df.format(val);
                break;
            ...
            if (resultDate.length() > dfStr.length())
                resultDate = resultDate.substring(0, dfStr.length());
