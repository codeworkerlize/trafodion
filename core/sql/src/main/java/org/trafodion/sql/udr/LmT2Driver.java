...
*/
	...
	stmt.executeUpdate("CONTROL QUERY DEFAULT WMS_CHILD_QUERY_MONITORING 'OFF'");
      ...
      String clientInfo = System.getProperty("sqlmx.udr.clientInfo");
      if (clientInfo != null)
      {
          String[] keyValue = clientInfo.split(":");
          if (keyValue.length == 2) {
              if (keyValue[0].equalsIgnoreCase("TransactionType")) {
                  conn.setClientInfo(keyValue[0], keyValue[1]);
                  }
          }
      }
      ...
      if (transId == -1) {
        System.setProperty("spsql.transaction.attribute",
                           Integer.toString(getTransactionAttrs()));
      }
/*
