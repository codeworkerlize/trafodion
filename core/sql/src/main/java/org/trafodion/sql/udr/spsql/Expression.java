...
public class Expression {
  ...
  public Integer execBoolUnary(HplsqlParser.Bool_expr_unaryContext ctx) {
    boolean val = false;
    if (ctx.T_IS() != null) {
      val = evalPop(ctx.expr(0)).isNull(exec.conf.emptyIsNull);
      if (ctx.T_NOT() != null) {
        val = !val;
      }
    }
      ...
      int cmp = v.compareTo(v1, exec.conf.emptyIsNull);
      if (cmp >= 0) {
        Var v2 = evalPop(ctx.expr(2));
        cmp = v.compareTo(v2, exec.conf.emptyIsNull);
        if (cmp <= 0) {
          val = true;
        }
      }
      ...
      // TODO: support select_stmt
      HplsqlParser.Bool_expr_single_inContext inCtx = ctx.bool_expr_single_in();
      Var v = evalPop(inCtx.expr(0));
      for (int i=1; i<inCtx.expr().size(); i++) {
        Var v1 = evalPop(inCtx.expr(i));
        if (v.equals(v1)) {
          val = true;
          break;
        }
  ...
  public void operatorAdd(HplsqlParser.ExprContext ctx) {
    ...
    if (v1.value == null || v2.value == null) {
      ...
      exec.stackPush(new Var(v1.decimalValue().add(v2.decimalValue())));
    }
    ...
    else if (v1.type == Type.TIME && v2.type == Type.INTERVAL) {
      exec.stackPush(new Var(((Interval)v2.value).timeChange((Time)v1.value, true /*add*/), v1.scale));
    }
  ...
  public void operatorSub(HplsqlParser.ExprContext ctx) {
    ...
    if (v1.value == null || v2.value == null) {
      ...
      exec.stackPush(new Var(v1.decimalValue().subtract(v2.decimalValue())));
    }
    else if (v1.type == Type.DATE && v2.type == Type.BIGINT) {
      ...
      exec.stackPush(new Var(((Interval)v2.value).timeChange((Time)v1.value, false /*subtract*/), v1.scale));
    } 
    else if (v1.type == Type.TIMESTAMP && v2.type == Type.INTERVAL) {
  ...
  public void operatorMultiply(HplsqlParser.ExprContext ctx) {
    ...
    if (v1.value == null || v2.value == null) {
      ...
      exec.stackPush(new Var(v1.decimalValue().multiply(v2.decimalValue())));
    }
    else {
  ...
  public void operatorDiv(HplsqlParser.ExprContext ctx) {
    ...
    if (v1.value == null || v2.value == null) {
      ...
      exec.stackPush(new Var(v1.decimalValue().divide(v2.decimalValue(), BigDecimal.ROUND_HALF_UP)));
    }
    else {
  ...
  public void operatorEqual(HplsqlParser.Bool_expr_binaryContext ctx, boolean equal) {
    Var v1 = evalPop(ctx.expr(0));
    Var v2 = evalPop(ctx.expr(1));
    boolean eq = v1.equals(v2, exec.conf.emptyIsNull);
    if (!equal) {
      eq = !eq;
    }
    exec.stackPush(eq);
  ...
  public void operatorCompare(HplsqlParser.Bool_expr_binaryContext ctx, HplsqlParser.Bool_expr_binary_operatorContext op) {
    Var v1 = evalPop(ctx.expr(0));
    Var v2 = evalPop(ctx.expr(1));
    if (v1.isNull(exec.conf.emptyIsNull) ||
        v2.isNull(exec.conf.emptyIsNull)) {
      exec.stackPush(Var.Null);
      return;
    }
    int cmp = v1.compareTo(v2, exec.conf.emptyIsNull);
    boolean bool = false;
  ...
  public void execSimpleCase(HplsqlParser.Expr_case_simpleContext ctx) { 
    ...
    while(i < cnt - 1) {
      ...
      if(val.compareTo(when, exec.conf.emptyIsNull) == 0) {
        visit(ctx.expr(i + 1));
        found = true;
        break;
      }
