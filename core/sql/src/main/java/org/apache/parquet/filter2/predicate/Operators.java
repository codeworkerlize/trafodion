...
public final class Operators {
  private static final org.apache.parquet.Log LOG = Log.getLog(Operators.class);
  ...
  public static abstract class ColumnFilterPredicate<T extends Comparable<T>> implements FilterPredicate, Serializable  {
    ...
    public boolean equals(Object o) {
      ...
      if (LOG.DEBUG) 
	  LOG.debug("CFP"
		     + ", column class: " + column.getClass().getName()
		     );
  ...
  public static final class Eq<T extends Comparable<T>> extends ColumnFilterPredicate<T> {
	...
	if (LOG.DEBUG) 
	    LOG.debug("Operators.eq"
		      + ", visitor class: " + visitor.getClass().getName()
		      + ", visitor: " + visitor
		      );
