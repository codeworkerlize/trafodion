...
abstract public class Binary implements Comparable<Binary>, Serializable {
  ...
  public    boolean isInt96Timestamp = false;
  ...
  private static class ByteArraySliceBackedBinary extends Binary {
	...
	return Binary.compareTwoByteArrays(value, offset, length, other, otherOffset, otherLength, isInt96Timestamp);
    }
  ...
  private static class ByteArrayBackedBinary extends Binary {
    ...
    @Override
    int compareTo(byte[] other, int otherOffset, int otherLength) {
	return Binary.compareTwoByteArrays(value, 0, value.length, other, otherOffset, otherLength, isInt96Timestamp);
    }
  ...
  private static class ByteBufferBackedBinary extends Binary {
    ...
    @Override
    int compareTo(ByteBuffer bytes, int otherOffset, int otherLength) {
      return Binary.compareTwoByteBuffers(value, offset, length, bytes, otherOffset, otherLength, isInt96Timestamp);
    }
  ...
  private static final int compareTwoByteBuffers(ByteBuffer buf1, int offset1, int length1,
    ...
    if (pv_isInt96 && (length1 == 12) && (length2 == 12)) {

        ByteOrder lv_buf1_order = buf1.order();
        ByteOrder lv_buf2_order = buf2.order();
	buf1.order(ByteOrder.LITTLE_ENDIAN);
	buf2.order(ByteOrder.LITTLE_ENDIAN);
	long lv_timeOfDayNanos_1 = buf1.getLong(offset1);
	int lv_julianDay_1 = buf1.getInt(offset1 + 8);
	long lv_timeOfDayNanos_2 = buf2.getLong(offset2);
	int lv_julianDay_2 = buf2.getInt(offset2 + 8);
	buf1.order(lv_buf1_order);
	buf2.order(lv_buf2_order);

	if (lv_julianDay_1 < lv_julianDay_2) {
	    return 1;
	}
	if (lv_julianDay_1 > lv_julianDay_2) {
	    return -1;
	}
	if (lv_timeOfDayNanos_1 < lv_timeOfDayNanos_2) {
	    return 1;
	}
	if (lv_timeOfDayNanos_1 > lv_timeOfDayNanos_2) {
	    return -1;
  ...
  private static final int compareTwoByteArrays(byte[] array1, int offset1, int length1,
      ...
      return compareTwoByteArrays(array1, offset1, length1,
				   array2, offset2, length2,
				   false);
  }
  ...
  private static final int compareTwoByteArrays(byte[] array1, int offset1, int length1,
    ...
    if (pv_isInt96 && (length1 == 12) && (length2 == 12)) {
	ByteBuffer lv_bb1 = ByteBuffer.wrap(array1, offset1, 12);
	ByteBuffer lv_bb2 = ByteBuffer.wrap(array2, offset2, 12);
	lv_bb1.order(ByteOrder.LITTLE_ENDIAN);
	lv_bb2.order(ByteOrder.LITTLE_ENDIAN);

	long lv_timeOfDayNanos_1 = lv_bb1.getLong();
	int lv_julianDay_1 = lv_bb1.getInt();
	long lv_timeOfDayNanos_2 = lv_bb2.getLong();
	int lv_julianDay_2 = lv_bb2.getInt();
	...
	for (int i = 0; i < min_length; i++) {
	    //	System.out.print(array1[i + offset1] + ":" +  array2[i + offset2] + ",");
	    if ((array1[i + offset1]) < (array2[i + offset2])) {
		//	    System.out.println("]");
		return 1;
	    }
