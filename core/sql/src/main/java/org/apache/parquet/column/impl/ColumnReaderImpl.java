...
public class ColumnReaderImpl implements ColumnReader {
    ...
    binding = type.convert(new PrimitiveTypeNameConverter<Binding, RuntimeException>() {
      ...
      public Binding convertINT32(PrimitiveTypeName primitiveTypeName) throws RuntimeException {
        return new Binding() {
          ...
          void writeValue() {
	      /*	      System.out.println("Binding.convertInt32.writeValue(): " + current
				 + ", this: " + this
				 );
	      */
	  ...
	  if (LOG.DEBUG) LOG.debug("convertINT96" + primitiveTypeName);
        return new Binding() {
          Binary current;
          void read() {
            current = dataColumn.readBytes();
	    if (LOG.DEBUG) LOG.debug("Binding.read"
				     + ", value: " + current);
          }
          public void skip() {
            current = null;
          ...
          void writeValue() {
	      if (LOG.DEBUG) LOG.debug("writeValue"
				       + ", value: " + current);
	      current.isInt96Timestamp = true;
  ...
  public ColumnReaderImpl(ColumnDescriptor path, PageReader pageReader, PrimitiveConverter converter, ParsedVersion writerVersion) {
      /**
       * Following output:
       * ColumnReaderImpl, converter: org.apache.parquet.filter2.recordlevel.FilteringPrimitiveConverter
       * System.out.println("ColumnReaderImpl" + ", converter: " + converter.getClass().getName();
       */
  ...
  private void readPage() {
    if (LOG.DEBUG) LOG.debug("loading page: " + this);
  ...
  private void initDataReader(Encoding dataEncoding, ByteBuffer bytes, int offset, int valueCount) {
      /*      System.out.println("CRI.initDataReader()"
			 + ", bytes: " + bytes
			 + ", offset: " + offset
