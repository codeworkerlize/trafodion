...
class LmContainerManager : public NABasicObject {
 public:
  LmContainerManager(LmLanguageManager *lm) : lm_(lm) {}
  ...
  //////////////////////////////////////////////////////////////////////
  virtual LmResult getContainer(const char *containerName, const char *externalPath, LmContainer **container,
...
class LmMetaContainer : public NABasicObject {
 public:
  LmMetaContainer(const char *path, LmContainerManager *contManager, ComDiagsArea *diagsArea);
...
class LmContainer : public NABasicObject {
 public:
  LmContainer(const char *name, LmHandle handle, const char *fileSystemName);
...
class LmContainerMC : public LmContainer {
 public:
  LmContainerMC(const char *name, LmHandle handle, LmMetaContainer *owner, const char *fileSystemName);
...
class LmContainerManagerCache : public LmContainerManager {
 public:
  LmContainerManagerCache(LmLanguageManager *lm, ComUInt32 capacity, ComBoolean enforceLimits, ComDiagsArea *diagsArea,
  ...
  ComUInt32 hashTableSize() { return metaContainers_->size(); }

 protected:
  virtual LmMetaContainer *addMetaContainer(const char *externalPath, ComDiagsArea *diagsArea);
...
class LmMetaContainerCache : public LmMetaContainer {
 ...
 public:
  LmMetaContainerCache(const char *path, LmContainerManager *contManager, ComDiagsArea *diagsArea);
  ...
  // Accessor/mutators.
  ComUInt32 capacity() { return capacity_; }

  ComBoolean clean() { return refcnt_ < 1; }

  void decay(float scale) { usecnt_ = (ComUInt16)(usecnt_ * scale); }

  ComUInt16 usecnt() { return usecnt_; }

 private:
  // Internal support methods.
  LmContainerManagerCache *cm() { return (LmContainerManagerCache *)contManager_; }
...
class LmContainerSimple : public LmContainer {
 public:
  LmContainerSimple(const char *name, LmHandle handle, const char *fileSystemName);
