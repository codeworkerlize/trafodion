...
class FragmentDir : public NABasicObject {
 public:
  enum FragmentTypeEnum { MASTER = 1, DP2 = 2, ESP = 3, EXPLAIN = 4 };
  ...
  inline Space *getCurrentSpace() const { return (*entries_)[currentFragmentId_]->space_; }
  ...
  inline void setTopObj(char *obj) { (*entries_)[currentFragmentId_]->topNode_ = obj; }
  ...
  // set the pointer to the generated top object for the current fragment
  inline void setTopObj(CollIndex fragIndex, char *obj) { (*entries_)[fragIndex]->topNode_ = obj; }

  // get the type of a fragment
  inline FragmentTypeEnum getType(CollIndex ix) const { return (FragmentTypeEnum)(*entries_)[ix]->type_; }
  ...
  inline CollIndex getParentId(CollIndex ix) const { return (*entries_)[ix]->parentIndex_; }
  ...
  inline NABoolean getSoloFragment(CollIndex ix) const { return (*entries_)[ix]->soloFragment_; }
  inline void setSoloFragment(CollIndex ix, NABoolean n = TRUE) { (*entries_)[ix]->soloFragment_ = n; }
  ...
  inline unsigned short getNumBMOs(CollIndex ix) { return (*entries_)[ix]->numBMOs_; }

  inline void setBMOsMemoryUsage(CollIndex ix, double memInMB) { (*entries_)[ix]->BMOsMemoryUsage_ = memInMB; }
  inline double getBMOsMemoryUsage() { return (*entries_)[currentFragmentId_]->BMOsMemoryUsage_; }
  inline double getBMOsMemoryUsage(CollIndex ix) { return (*entries_)[ix]->BMOsMemoryUsage_; }
