...
class ComTdbHashj : public ComTdb {
 ...
 public:
  ComTdbHashj();
  ComTdbHashj(ComTdb *leftChildTdb, ComTdb *rightChildTdb, ex_cri_desc *givenCriDesc, ex_cri_desc *returnedCriDesc,
  ...
  UInt16 forceHashLoopAfterNumBuffers() { return forceHashLoopAfterNumBuffers_; }
  void setForceHashLoopAfterNumBuffers(UInt16 numBuffers) { forceHashLoopAfterNumBuffers_ = numBuffers; }
  UInt16 forceClusterSplitAfterMB() { return forceClusterSplitAfterMB_; }
  void setForceClusterSplitAfterMB(UInt16 mb) { forceClusterSplitAfterMB_ = mb; }

  NABoolean bufferedWrites() { return (hjFlags_ & BUFFERED_WRITES_) != 0; }
  void setBufferedWrites(NABoolean v) { (v ? hjFlags_ |= BUFFERED_WRITES_ : hjFlags_ &= ~BUFFERED_WRITES_); }
  NABoolean logDiagnostics() { return (hjFlags_ & LOG_DIAGNOSTICS_) != 0; }
  void setLogDiagnostics(NABoolean v) { (v ? hjFlags_ |= LOG_DIAGNOSTICS_ : hjFlags_ &= ~LOG_DIAGNOSTICS_); }
  ...
  NABoolean isPossibleMultipleCalls() { return (hjFlags_ & POSSIBLE_MULTIPLE_CALLS) != 0; }
  void setPossibleMultipleCalls(NABoolean v) {
    (v ? hjFlags_ |= POSSIBLE_MULTIPLE_CALLS : hjFlags_ &= ~POSSIBLE_MULTIPLE_CALLS);
  }
  ...
  inline void setXproductPreemptMax(UInt32 m) { xProductPreemptMax_ = m; };
  ...
  NABoolean leftSideIUD() const { return (hjFlags_ & LEFT_SIDE_IUD_OPERATION) != 0; };
  void setLeftSideIUD() { hjFlags_ |= LEFT_SIDE_IUD_OPERATION; };

  NABoolean beforePredOnOuterOnly() const { return (hjFlags2_ & BEFORE_PRED_OUTER_ONLY) != 0; };
  void setBeforePredOnOuterOnly() { hjFlags2_ |= BEFORE_PRED_OUTER_ONLY; };

  AggrExpr *minMaxExpr() { return (AggrExpr *)((ex_expr *)minMaxExpr_); }

  ex_expr *rangeSpecPackingExpr() { return rangeSpecPackingExpr_; }
 ...
 private:
  ...
  Float32 bmoQuotaRatio_;         // 376-376
...
inline ComTdb *ComTdbHashj::getLeftChildTdb() { return leftChildTdb_; };
