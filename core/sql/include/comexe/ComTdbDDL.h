...
class ComTdbGenericUtil : public ComTdb {
 ...
 public:
  ComTdbGenericUtil() : ComTdb(ComTdb::ex_DDL, eye_DDL){};
  ...
  virtual ex_expr *getExpressionNode(Int32 pos) {
    switch (pos) {
      ...
      case 1:
        return (ex_expr *)outputExpr_.getPointer();
      default:
        return NULL;
    }
  }
...
// class ComTdbDDL
////////////////////////////////////////////////////////////////////
static const ComTdbVirtTableColumnInfo ddlVirtTableColumnInfo[] = {{"DDL_OUTPUT",
                                                                    0,
                                                                    COM_USER_COLUMN,
                                                                    REC_BYTE_V_ASCII,
                                                                    2000,
...
class ComTdbDDL : public ComTdbGenericUtil {
 ...
 public:
  ...
  NABoolean createTable() { return (flags_ & CREATE_TABLE_DDL) != 0; }
  void setCreateTable(NABoolean v) { (v ? flags_ |= CREATE_TABLE_DDL : flags_ &= ~CREATE_TABLE_DDL); }

  NABoolean createIndex() { return (flags_ & CREATE_INDEX_DDL) != 0; }
  void setCreateIndex(NABoolean v) { (v ? flags_ |= CREATE_INDEX_DDL : flags_ &= ~CREATE_INDEX_DDL); }

  NABoolean createMV() { return (flags_ & CREATE_MV_DDL) != 0; }
  void setCreateMV(NABoolean v) { (v ? flags_ |= CREATE_MV_DDL : flags_ &= ~CREATE_MV_DDL); }

  NABoolean hbaseDDL() { return (flags_ & HBASE_DDL) != 0; }
  void setHbaseDDL(NABoolean v) { (v ? flags_ |= HBASE_DDL : flags_ &= ~HBASE_DDL); }

  NABoolean hbaseDDLNoUserXn() { return (flags_ & HBASE_DDL_NO_USER_XN) != 0; }
  void setHbaseDDLNoUserXn(NABoolean v) { (v ? flags_ |= HBASE_DDL_NO_USER_XN : flags_ &= ~HBASE_DDL_NO_USER_XN); }
  ...
  NABoolean isExecuteInESP() { return (flags_ & EXECUTE_IN_ESP) != 0; }
...
class ComTdbDDLwithStatus : public ComTdbDDL {
 ...
 public:
  ...
  ComTdbDDLwithStatus(char *ddl_query, ULng32 ddl_querylen, Int16 ddl_querycharset, char *schemaName,
  ...
  void setGetMDVersion(NABoolean v) { (v ? flags2_ |= GET_MD_VERSION : flags2_ &= ~GET_MD_VERSION); }
  NABoolean getMDVersion() { return (flags2_ & GET_MD_VERSION) != 0; }

  void setGetSWVersion(NABoolean v) { (v ? flags2_ |= GET_SW_VERSION : flags2_ &= ~GET_SW_VERSION); }
  NABoolean getSWVersion() { return (flags2_ & GET_SW_VERSION) != 0; }

  void setMDupgrade(NABoolean v) { (v ? flags2_ |= MD_UPGRADE : flags2_ &= ~MD_UPGRADE); }
  NABoolean getMDupgrade() { return (flags2_ & MD_UPGRADE) != 0; }

  void setMDcleanup(NABoolean v) { (v ? flags2_ |= MD_CLEANUP : flags2_ &= ~MD_CLEANUP); }
  NABoolean getMDcleanup() { return (flags2_ & MD_CLEANUP) != 0; }

  void setCheckOnly(NABoolean v) { (v ? flags2_ |= CHECK_ONLY : flags2_ &= ~CHECK_ONLY); }
  NABoolean getCheckOnly() { return (flags2_ & CHECK_ONLY) != 0; }

  void setReturnDetails(NABoolean v) { (v ? flags2_ |= RETURN_DETAILS : flags2_ &= ~RETURN_DETAILS); }
  NABoolean getReturnDetails() { return (flags2_ & RETURN_DETAILS) != 0; }

  void setInitTraf(NABoolean v) { (v ? flags2_ |= INIT_TRAF : flags2_ &= ~INIT_TRAF); }
...
class ComTdbProcessVolatileTable : public ComTdbDDL {
 ...
 public:
  ComTdbProcessVolatileTable() : ComTdbDDL() {}

  ComTdbProcessVolatileTable(char *ddl_query, ULng32 ddl_querylen, Int16 ddl_querycharset, char *volTabName,
                             ULng32 volTabNameLen, NABoolean isCreate, NABoolean isTable, NABoolean isIndex,
                             NABoolean isSchema, char *schemaName, ULng32 schemaNameLen, ex_expr *input_expr,
  ...
  // Redefine virtual functions required for Versioning.
  //----------------------------------------------------------------------
  virtual short getClassSize() { return (short)sizeof(ComTdbProcessVolatileTable); }
  ...
  void setIsTable(NABoolean v) { (v ? flags_ |= IS_TABLE_ : flags_ &= ~IS_TABLE_); };
  NABoolean isTable() { return (flags_ & IS_TABLE_) != 0; };

  void setIsIndex(NABoolean v) { (v ? flags_ |= IS_INDEX_ : flags_ &= ~IS_INDEX_); };
  NABoolean isIndex() { return (flags_ & IS_INDEX_) != 0; };

  void setIsSchema(NABoolean v) { (v ? flags_ |= IS_SCHEMA_ : flags_ &= ~IS_SCHEMA_); };
  NABoolean isSchema() { return (flags_ & IS_SCHEMA_) != 0; };

 protected:
  enum { IS_CREATE_ = 0x0001, IS_TABLE_ = 0x0002, IS_INDEX_ = 0x0004, IS_SCHEMA_ = 0x0008 };
...
class ComTdbProcessInMemoryTable : public ComTdbDDL {
 ...
 public:
  ...
  ComTdbProcessInMemoryTable(char *ddl_query, ULng32 ddl_querylen, Int16 ddl_querycharset, char *objName,
                             ULng32 objNameLen, NABoolean isCreate, NABoolean isVolatile, NABoolean isTable,
                             NABoolean isIndex, NABoolean isMV, char *schemaName, ULng32 schemaNameLen,
                             ex_expr *input_expr, ULng32 input_rowlen, ex_expr *output_expr, ULng32 output_rowlen,
                             ex_cri_desc *work_cri_desc, const unsigned short work_atp_index,
                             ex_cri_desc *given_cri_desc, ex_cri_desc *returned_cri_desc, queue_index down,
                             queue_index up, int num_buffers, ULng32 buffer_size);
  ...
  // Redefine virtual functions required for Versioning.
  //----------------------------------------------------------------------
  virtual short getClassSize() { return (short)sizeof(ComTdbProcessInMemoryTable); }
  ...
  void setIsVolatile(NABoolean v) { (v ? flags_ |= IS_VOLATILE_ : flags_ &= ~IS_VOLATILE_); };
  NABoolean isVolatile() { return (flags_ & IS_VOLATILE_) != 0; };
  ...
  void setIsMV(NABoolean v) { (v ? flags_ |= IS_MV_ : flags_ &= ~IS_MV_); };
  NABoolean isMV() { return (flags_ & IS_MV_) != 0; };

 protected:
  enum { IS_CREATE_ = 0x0001, IS_VOLATILE_ = 0x0002, IS_TABLE_ = 0x0004, IS_INDEX_ = 0x0008, IS_MV_ = 0x0010 };
...
class ComTdbDescribe : public ComTdbDDL {
 ...
 public:
  enum DescribeType { INVOKE_ = 0, SHORT_, LONG_, PLAN_, LABEL_, LEAKS_, SHAPE_, SHOWSTATS_, TRANSACTION_, ENVVARS_ };

  ComTdbDescribe() : ComTdbDDL(){};
