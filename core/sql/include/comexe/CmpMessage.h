...
public:
  ...
  void setSentryPrivRecheck(NABoolean v)      
           { (v ? flags_ |= SENTRY_PRIV_RECHECK : flags_ &= ~SENTRY_PRIV_RECHECK); }
 ...
 NABoolean isHbaseDDL()
  ...
  NABoolean isGenLoadQueryCache()
  {
    return (flags_ & GEN_LOAD_QUERYCACHE) != 0;
  }
  void setGenLoadQueryCache(NABoolean v)
  {
    (v ? flags_ |= GEN_LOAD_QUERYCACHE : flags_ &= ~GEN_LOAD_QUERYCACHE);
  }
...
{
public:
  CmpMessageDDLwithStatus(char* stmt,CmpMsgBufLenType size,CollHeap* h=0,
                          Lng32 charset=SQLCHARSETCODE_UTF8,
                          const char *parentQid = NULL,
                          Lng32 parentQidLen = 0):
