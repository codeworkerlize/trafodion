...
#define SQLUDR_GETNULLIND(null_indicator) ((((char *)(null_indicator))[1] && 0x0080) ? SQLUDR_NULL : SQLUDR_NOTNULL)

/* To set the NULL indicator for a return value */
#define SQLUDR_SETNULLIND(null_indicator) (*((short *)(null_indicator)) = -1)

/* To clear the NULL indicator for a return value */
#define SQLUDR_CLEARNULLIND(null_indicator) (*((short *)(null_indicator)) = 0)
...
enum SQLUDR_PARAMSTYLE { SQLUDR_PARAMSTYLE_SQL = 1, SQLUDR_PARAMSTYLE_SQLROW = 2 };

/* UDR row formats */
enum SQLUDR_ROWFORMAT { SQLUDR_ROWFORMAT_EXPLODED = 1 };
...
#define SQLUDR_TRAIL_ARGS                                                                                      \
  SQLUDR_CHAR sqlstate[SQLUDR_SQLSTATE_SIZE], SQLUDR_CHAR msgtext[SQLUDR_MSGTEXT_SIZE], SQLUDR_INT32 calltype, \
      SQLUDR_STATEAREA *statearea, SQLUDR_UDRINFO *udrinfo

/* in case we need to call another function with SQLUDR_TRAIL_ARGS */
#define PASS_ON_SQLUDR_TRAIL_ARGS sqlstate, msgtext, calltype, statearea, udrinfo

/* SQLTMUDF Queue State, also defined in
   ../src/main/java/org/trafodion/sql/udr/UDR.java  */
typedef enum SQLUDR_Q_STATE { SQLUDR_Q_MORE = 1, SQLUDR_Q_EOD, SQLUDR_Q_CANCEL } SQLUDR_Q_STATE;
...
#define SQLUDR_TMUDF_TRAIL_ARGS                                                                             \
  SQLUDR_CHAR *rowDataSpace1, SQLUDR_CHAR *rowDataSpace2, SQLUDR_GetNextRow getRow, SQLUDR_EmitRow emitRow, \
      SQLUDR_TRAIL_ARGS

/* in case we need to call another function with SQLUDR_TMUDF_TRAIL_ARGS */
#define PASS_ON_SQLUDR_TMUDF_TRAIL_ARGS rowDataSpace1, rowDataSpace2, getRow, emitRow, PASS_ON_SQLUDR_TRAIL_ARGS
...
class TMUDRSerializableObject {
 public:
  enum Endianness { UNKNOWN_ENDIANNESS = 0, IS_LITTLE_ENDIAN = 1, IS_BIG_ENDIAN = 2 };
  ...
  TMUDRSerializableObject(TMUDRObjectType objectType, unsigned short version = 1,
  ...
  virtual int serialize(Bytes &outputBuffer, int &outputBufferLength);
  virtual int deserialize(ConstBytes &inputBuffer, int &inputBufferLength);
  ...
  int serializeInt8(uint8_t i, Bytes &outputBuffer, int &outputBufferLength);
  int serializeInt(int i, Bytes &outputBuffer, int &outputBufferLength);
  int serializeShort(short s, Bytes &outputBuffer, int &outputBufferLength);
  int serializeDouble(double d, Bytes &outputBuffer, int &outputBufferLength);
  int serializeLong(long i, Bytes &outputBuffer, int &outputBufferLength);
  int serializeString(const char *s, Bytes &outputBuffer, int &outputBufferLength);
  int serializeString(const char *s, int len, Bytes &outputBuffer, int &outputBufferLength);
  int serializeString(const std::string &s, Bytes &outputBuffer, int &outputBufferLength);
  int serializeBinary(const void *b, int len, Bytes &outputBuffer, int &outputBufferLength);

  int deserializeInt8(uint8_t &i, ConstBytes &inputBuffer, int &inputBufferLength);
  int deserializeInt(int &i, ConstBytes &inputBuffer, int &inputBufferLength);
  int deserializeShort(short &s, ConstBytes &inputBuffer, int &inputBufferLength);
  int deserializeDouble(double &d, ConstBytes &inputBuffer, int &inputBufferLength);
  int deserializeLong(long &i, ConstBytes &inputBuffer, int &inputBufferLength);

  int deserializeStringSafe(const char *&s, int &stringLength, bool makeACopy, ConstBytes &inputBuffer,
                            int &inputBufferLength);

  int deserializeString(const char *&s, int &stringLength, bool makeACopy, ConstBytes &inputBuffer,
                        int &inputBufferLength);
  int deserializeString(std::string &s, ConstBytes &inputBuffer, int &inputBufferLength);
  int deserializeBinary(const void **b, int &binaryLength, bool makeACopy, ConstBytes &inputBuffer,
                        int &inputBufferLength);
  TMUDRObjectType getNextObjectType(ConstBytes inputBuffer, int inputBufferLength);
...
class TypeInfo : public TMUDRSerializableObject {
 public:
  ...
  TypeInfo(SQLTypeCode sqltype = UNDEFINED_SQL_TYPE, int length = 0, bool nullable = false, int scale = 0,
           SQLCharsetCode charset = CHARSET_UTF8, SQLIntervalCode intervalCode = UNDEFINED_INTERVAL_CODE,
           int precision = 0, SQLCollationCode collation = SYSTEM_COLLATION);
  // this constructor should not be used by UDR writers
  TypeInfo(SQLTypeCode sqltype, bool nullable, int scale, SQLCharsetCode charset, SQLIntervalCode intervalCode,
  ...
  const char *getRaw(const char *row, bool &wasNull, int &byteLen) const;
 ...
 private:
  // flags
  enum { TYPE_FLAG_4_BYTE_VC_LEN = 0x00000001 };
...
class ConstraintInfo : public TMUDRSerializableObject {
 ...
 protected:
  ConstraintInfo(ConstraintTypeCode constraintType, unsigned short version);
...
class PredicateInfo : public TMUDRSerializableObject {
 public:
  ...
  virtual void toString(std::string &s, const TableInfo &ti) const = 0;
...
class ComparisonPredicateInfo : public PredicateInfo {
 public:
  ...
  virtual void toString(std::string &s, const TableInfo &ti) const;
...
class TupleInfo : public TMUDRSerializableObject {
 public:
  ...
  void getDelimitedRow(std::string &row, char delim = '|', bool quote = false, char quoteSymbol = '"',
                       int firstColumn = 0, int lastColumn = -1) const;
  ...
  const char *setFromDelimitedRow(const char *row, char delim = '|', bool quote = false, char quoteSymbol = '"',
                                  int firstColumnToSet = 0, int lastColumnToSet = -1, int numDelimColsToSkip = 0) const;
  ...
  void addCharColumn(const char *colName, int length, bool isNullable = false,
                     TypeInfo::SQLCharsetCode charset = TypeInfo::CHARSET_UTF8,
                     TypeInfo::SQLCollationCode collation = TypeInfo::SYSTEM_COLLATION);
  void addVarCharColumn(const char *colName, int length, bool isNullable = false,
                        TypeInfo::SQLCharsetCode charset = TypeInfo::CHARSET_UTF8,
                        TypeInfo::SQLCollationCode collation = TypeInfo::SYSTEM_COLLATION);
...
class UDRInvocationInfo : public TMUDRSerializableObject {
 public:
   ...
   */
  enum SQLAccessType { CONTAINS_NO_SQL, READS_SQL, MODIFIES_SQL };

  /**
   *  Type of transaction that is required, if any
   */
  enum SQLTransactionType { REQUIRES_NO_TRANSACTION, REQUIRES_SQL_TRANSACTION };
   ...
   */
  enum SQLRightsType { INVOKERS_RIGHTS, DEFINERS_RIGHTS };

  /**
   *  Indicates whether this UDR is trusted or not
   */
  enum IsolationType { ISOLATED, TRUSTED };
 ...
 private:
  ...
  void validateCallPhase(CallPhase start, CallPhase end, const char *callee) const;
