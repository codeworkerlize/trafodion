 ...
 */
enum RelativeLocation { rel_loc_before, rel_loc_within, rel_loc_after };
...
class RangeString : public NAString {
 public:
  ...
  ElementType getElementType() { return ET_StringVal; };
};
...
inline NABoolean operator==(const RangeString &s1, const RangeString &s2) { return RangeString::cmp(s1, s2) == 0; }
...
class RangeWString : public NAWString {
 public:
  RangeWString(NAMemory *heap = NULL) : NAWString(heap) {}

  RangeWString(const NAWchar *nawChrPtr, size_t len, NAMemory *heap = NULL) : NAWString(nawChrPtr, len, heap) {}
  ...
  ElementType getElementType() { return ET_WStringVal; };
};
...
inline NABoolean operator==(const RangeWString &s1, const RangeWString &s2) { return RangeWString::cmp(s1, s2) == 0; }
...
// Range date for SQL date data-type.
// Have to derive from RangeString to
// avoid compilation error.
class RangeDate : public RangeString {
 public:
  RangeDate() { memset(date_, 0, sizeof(date_)); };
  RangeDate(const char *date);
  RangeDate(short year, short month, short day);
  ~RangeDate();
  ...
  ElementType getElementType() { return ET_DateVal; };

  static void init(char *, short year, short month, short day);

 protected:
  char date_[DATE_LENGTH];
};

inline NABoolean operator==(const RangeDate &s1, const RangeDate &s2) { return RangeDate::cmp(s1, s2) == 0; }
...
// RangTime for SQL time data type
...
inline NABoolean operator==(const RangeTime &s1, const RangeTime &s2) { return RangeTime::cmp(s1, s2) == 0; }
...
class RangeTimestamp : public RangeString {
 ...
 protected:
  char timestamp_[TIMESTAMP_LENGTH];
};

inline NABoolean operator==(const RangeTimestamp &s1, const RangeTimestamp &s2) {
  return RangeTimestamp::cmp(s1, s2) == 0;
}
...
class SubrangeBase : public NABasicObject {
 ...
 public:
   ...
   * @return Pointer to the created scalar element.
   */
  static QRScalarValuePtr createScalarValElem(CollHeap *heap, double value, const NAType &type,
                                              const NAString &unparsedText);
   ...
   * @return Pointer to the created scalar element.
   */
  static QRScalarValuePtr createScalarValElem(CollHeap *heap, const NAString &value, const NAType &type,
                                              const NAString &unparsedText);
   ...
   * @return Pointer to the created scalar element.
   */
  static QRScalarValuePtr createScalarValElem(CollHeap *heap, const RangeWString &value, const NAType &type,
                                              const NAString &unparsedText);
  ...
  NABoolean operator!=(const SubrangeBase &other) const { return !(*this == other); }
   ...
   * @return \c TRUE if the two subranges are equal, otherwise \c FALSE.
   */
  NABoolean isEqual(const SubrangeBase &other) const { return *this == other; }
   ...
   */
  virtual void copyToRangeSpec(RangeSpec *range, CollHeap *heap = NULL) const = 0;
   ...
   *                 to the other subrange (before, within, or after).
   */
  virtual void compareTo(SubrangeBase *other, RelativeLocation &startLoc, RelativeLocation &endLoc) const = 0;
   ...
   */
  virtual SubrangeBase *splitAtEndOf(const SubrangeBase *other, CollHeap *heap) = 0;
   ...
   *         subrange.
   */
  virtual QRScalarValuePtr getStartScalarValElem(CollHeap *heap, const NAType &type) const = 0;
   ...
   *         subrange.
   */
  virtual QRScalarValuePtr getEndScalarValElem(CollHeap *heap, const NAType &type) const = 0;
  ...
  virtual NABoolean existsWithin(SubrangeBase *other) const = 0;
   ...
   */
  NABoolean coversFullRange() const { return startIsMin_ && endIsMax_; }

  NABoolean startInclusive() const { return startInclusive_; }

  NABoolean endInclusive() const { return endInclusive_; }

  NABoolean startIsMin() const { return startIsMin_; }

  NABoolean endIsMax() const { return endIsMax_; }

  void setStartInclusive(NABoolean inclusive) { startInclusive_ = inclusive; }

  void setEndInclusive(NABoolean inclusive) { endInclusive_ = inclusive; }

  void setStartIsMin(NABoolean isMin) { startIsMin_ = isMin; }
  ...
  // to be written when some of the data members are packed/unpacked.
  virtual UInt32 pack(char *buf) { return 0; };
  virtual UInt32 unpack(char *buf) { return 0; };

  virtual UInt32 getPackedLength() { return 0; };
  virtual UInt32 totalSize() { return sizeof(SubrangeBase); };

  static UInt32 unpackGeneric(char *buf, CollHeap *heap, logLevel level, SubrangeBase *&subRangeBase);

  ElementType getElementType() { return elemType_; };
  void setElementType(ElementType x) { elemType_ = x; };

  virtual void outputRangeForOrc(std::vector<std::string> &ppiVec, const char *colName, const char *colType) = 0;

  /*
      virtual
      void outputOrcSearchArgPredicate(
              std::string& text, ExtPushdownOperatorType opType,
              const char* colName, const char* colType) = 0;
  */

  ~SubrangeBase() {}

 protected:
  UInt32 packElementType(char *buf);
  static UInt32 unpackElementType(char *buf, ElementType &type);

 protected:
  SubrangeBase(ElementType et = ET_INVALID, NABoolean startInclusive = FALSE, NABoolean endInclusive = FALSE)
...
class Subrange : public SubrangeBase {
 public:
  Subrange(logLevel ll, ElementType et = ET_INVALID, NABoolean startInclusive = FALSE, NABoolean endInclusive = FALSE)
      : logLevel_(ll), SubrangeBase(et, startInclusive, endInclusive) {}
   ...
   */
  virtual Subrange *clone(CollHeap *heap = NULL) const { return new (heap) Subrange(*this); }
  ...
  virtual void copyToRangeSpec(RangeSpec *range, CollHeap *heap = NULL) const;
  ...
  virtual NABoolean existsWithin(SubrangeBase *other) const;

  virtual NABoolean adjacentTo(SubrangeBase *other) const;
  virtual void compareTo(SubrangeBase *other, RelativeLocation &startLoc, RelativeLocation &endLoc) const;
  ...
  virtual SubrangeBase *splitAtEndOf(const SubrangeBase *other, CollHeap *heap);

  virtual NABoolean isSingleValue() const { return !startIsMin_ && !endIsMax_ && start == end; }

  virtual void initSpecifiedValueCount();

  virtual QRScalarValuePtr getStartScalarValElem(CollHeap *heap, const NAType &type) const {
    return createScalarValElem(heap, start, type, unparsedStart_);
  }

  virtual QRScalarValuePtr getEndScalarValElem(CollHeap *heap, const NAType &type) const {
  ...
  void setUnparsedStart(const NAString &text) { unparsedStart_ = text; }

  void setUnparsedEnd(const NAString &text) { unparsedEnd_ = text; }

  virtual logLevel getLogLevel() const { return logLevel_; }
  ...
  void outputRangeForOrc(std::vector<std::string> &ppiVec, const char *colName, const char *colType);

  virtual void outputOrcSearchArgPredicate(std::vector<std::string> &ppiVec, ExtPushdownOperatorType opType,
                                           const char *colName, const char *colType, T value);
...
class RangeSpec : public NABasicObject {
  ...
  friend void Subrange<RangeDate>::copyToRangeSpec(RangeSpec *, CollHeap *) const;
  friend void Subrange<RangeTime>::copyToRangeSpec(RangeSpec *, CollHeap *) const;
  friend void Subrange<RangeTimestamp>::copyToRangeSpec(RangeSpec *, CollHeap *) const;
 ...
 public:
  ...
  NABoolean operator!=(const RangeSpec &other) const { return !(*this == other); }
   ...
   */
  NABoolean isEqual(const RangeSpec &other) const { return *this == other; }
   ...
   */
  QRValueId getRangeColValueId() const { return rangeColValueId_; }
   ...
   */
  QRValueId getRangeJoinPredId() const { return rangeJoinPredId_; }

  /**
   * Returns the expression text for the expression the range applies to.
   * @return The expression text.
   */
  const NAString &getRangeExprText() const { return rangeExprText_; }
   ...
   */
  virtual void setType(const NAType *type) { type_ = type->newCopy(mvqrHeap_); }
   ...
   */
  void setRangeColValueId(QRValueId colVid) { rangeColValueId_ = colVid; }
   ...
   */
  void setRangeJoinPredId(QRValueId joinPredId) { rangeJoinPredId_ = joinPredId; }
   ...
   */
  void addSubrange(QRScalarValuePtr startVal, QRScalarValuePtr endVal, NABoolean startInclusive,
  ...
  */
   ...
   * @return NULL.
   */
  virtual const ItemExpr *getOriginalItemExpr() const { return NULL; }
   ...
   * @return NULL.
   */
  virtual ItemExpr *getRangeItemExpr() const { return NULL; }
  ...
  NABoolean coversFullRange() const {
    ...
    return (subranges_.entries() == 1 ? nullIncluded_ && subranges_[0]->coversFullRange() : FALSE);
  }
   ...
   * @return
   */
  NABoolean isFalse() const { return !nullIncluded_ && subranges_.entries() == 0; }
  ...
  void testPackUnpack(NAHeap *heap);
 ...
 protected:
  ...
  template <class T>
  NABoolean lookup(Subrange<T> *sub);
   ...
   */
  void addDenormalizedSubrange(const char *typeText, ElementType rangeOpType, QRScalarValuePtr value);
...
inline ostream &operator<<(ostream &os, RangeSpec &rangeSpec) {
  ...
  if (rangeSpec.nullIncluded_) os << "NULL is included\n";
  return os;
}
