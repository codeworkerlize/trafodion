...
class QRDescriptorException : public QRException {
 public:
   ...
   */
  QRDescriptorException(const char *msgTemplate...) : QRException() {
...
class Visitor : public NAIntrusiveSharedPtrObject {
 public:
  Visitor(ADD_MEMCHECK_ARGS_DECL(CollHeap *heap)) : NAIntrusiveSharedPtrObject(ADD_MEMCHECK_ARGS_PASS(heap)) {}
...
#define VISIT_THIS(visitor, thisObj)                     \
  ...
  if (result == Visitor::VR_Stop)                        \
    return TRUE;                                         \
  else if (result == Visitor::VR_Skip)                   \
    return FALSE;
...
class AggregateFinderVisitor : public Visitor {
 public:
  AggregateFinderVisitor(ADD_MEMCHECK_ARGS_DECL(CollHeap *heap), NABoolean findAll = FALSE)
      : Visitor(ADD_MEMCHECK_ARGS_PASS(heap)), foundAggregate_(FALSE), findAll_(findAll), aggregatesFound_(heap) {}

  virtual VisitResult visit(QRElementPtr caller);

  NABoolean foundAggregate() const { return foundAggregate_; }

  const ElementPtrList &getAggregatesFound() const { return aggregatesFound_; }
...
class ElementFinderVisitor : public Visitor {
 public:
   ...
   */
  ElementFinderVisitor(ElementType elemType, NABoolean useRefedElem = TRUE, ADD_MEMCHECK_ARGS_DECL(CollHeap *heap = 0))
      : Visitor(ADD_MEMCHECK_ARGS_PASS(heap)), targetTypes_(heap), elementsFound_(heap), useRefedElem_(useRefedElem) {
   ...
   * @param heap Heap to use.
   */
  ElementFinderVisitor(NAList<ElementType> &targetTypes, NABoolean useRefedElem = TRUE,
  ...
  const ElementPtrList &getElementsFound() const { return elementsFound_; }
...
class QRElement : public XMLElement {
 public:
   ...
   */
  virtual NABoolean isExtraHub() { return FALSE; }
  ...
  virtual const NAString &getSortName() const {
    throw QRDescriptorException("getSortName() is not defined for Element %s", getElementName());
  }

  void serializeBoolAttr(const char *attrName, NABoolean attrVal, XMLString &xml);

  void deserializeBoolAttr(const char *attrName, const char *attrVal, NABoolean &attr);

  /**
   * Returns the unique identifier for this element
   * @return The element's unique id.
   */
  const NAString &getID() const { return id_; }
   ...
   */
  NumericID getIDNum() const { return (id_.length() < 2 ? 0 : atoi(id_.data() + 1)); }
  ...
  virtual char getIDFirstChar() {
    assertLogAndThrow(CAT_SQL_COMP_QR_DESC_GEN, LL_MVQR_FAIL, FALSE, QRDescriptorException,
   ...
   */
  char getRefFirstChar() const { return (ref_.length() < 2 ? ' ' : *ref_.data()); }
   ...
   */
  void setRef(const NAString &ref) { ref_ = ref; }
  ...
  virtual void serializeBody(XMLString &xml) { xml.appendCharData(charData_); }
   ...
   * @return
   */
  virtual enum ElementType getElementType() const { return eType_; }

  void setReferencedElement(QRElementPtr referenced) { referencedElement_ = referenced; }

  QRElementPtr getReferencedElement() { return referencedElement_; }

  const QRElementPtr getReferencedElement() const { return referencedElement_; }

#define THROW_DOWNCAST_EXCEPTION                                                     \
  assertLogAndThrow(CAT_SQL_COMP_QR_DESC_GEN, LL_MVQR_FAIL, FALSE, QRLogicException, \
                    "Down cast to wrong element type.");
  ...
  const QRElementPtr getParentElement() const { return static_cast<const QRElementPtr>(getParent()); }
 ...
 protected:
  ...
  virtual void startElement(void *parser, const char *elementName, const char **atts) {
    throw QRDescriptorException("Element %s cannot contain element content", getElementName());
  }
...
class QRList : public QRElement {
 public:
  ...
  QRList() : QRElement(ET_List, NULL, NULL) {}

  QRList(ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0))
      : QRElement(ET_List, NULL, ADD_MEMCHECK_ARGS_PASS(heap)), list_(heap) {}
   ...
   */
  QRList(XMLElementPtr parent, AttributeList atts, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0))
      : QRElement(ET_List, parent, ADD_MEMCHECK_ARGS_PASS(heap)), list_(heap) {}
   ...
   */
  virtual ~QRList() {
    for (CollIndex i = 0; i < list_.entries(); i++) deletePtr(list_[i]);
  }
   ...
   */
  CollIndex entries() { return list_.entries(); }
   ...
   */
  PTR_TO_TYPE(T) operator[](CollIndex i) { return list_[i]; }
   ...
   */
  const NAPtrList<PTR_TO_TYPE(T)> &getList() const { return list_; }
   ...
   * @return
   */
  NABoolean isEmpty() const { return list_.isEmpty(); }
   ...
   */
  void addItem(PTR_TO_TYPE(T) item) { list_.insert(item); }
...
class QRElementList : public QRElement {
 protected:
  QRElementList(ElementType type, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0))
      : QRElement(type, NULL, ADD_MEMCHECK_ARGS_PASS(heap)), list_(heap) {}

  QRElementList(ElementType type, XMLElementPtr parent, AttributeList atts, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0));
 ...
 public:
   ...
   */
  const ElementPtrList &getList() const { return list_; }
   ...
   * @return
   */
  const QRElementPtr getElement(CollIndex i) { return list_[i]; }
  ...
  void addElement(QRElementPtr elem) {
    if (!isAllowed(elem))
      throw QRDescriptorException("<%s> is not allowed in <%s>", elem->getElementName(), getElementName());
    list_.insert(elem);
  }
  ...
  CollIndex entries() const { return list_.entries(); }

  void removeElement(QRElementPtr elem) { list_.remove(elem); }
...
class QRDescriptor : public QRElement {
 public:
   ...
   */
  const NAPtrList<QRJBBPtr> &getJbbList() const { return jbbList_; }

  /**
   * Adds a JBB to the list for this MV.
   * @param jbb The JBB to add to the list.
   */
  void addJBB(QRJBBPtr jbb) { jbbList_.insert(jbb); }

 protected:
  QRDescriptor(ElementType eType, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap))
      : QRElement(eType, NULL, ADD_MEMCHECK_ARGS_PASS(heap)), jbbList_(heap) {}
  ...
  QRDescriptor(ElementType eType, XMLElementPtr parent, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap))
      : QRElement(eType, parent, ADD_MEMCHECK_ARGS_PASS(heap)), jbbList_(heap) {}
...
class QRQueryMisc : public QRElement {
 public:
  ...
  QRQueryMisc(XMLElementPtr parent, AttributeList atts, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0));
   ...
   * @return The user ID.
   */
  const NAString &getUserID() const { return userID_; }

  /**
   * Returns the MV age associated with the query.
   * @return The MV age.
   */
  const NAString &getMVAge() const { return mvAge_; }

  /**
   * Returns the optimization level in effect for the query.
   * @return The optimization level.
   */
  const NAString &getOptLevel() const { return optLevel_; }

  /**
   * Returns the rewrite level in effect.
   * @return The query rewrite level.
   */
  const MvqrRewriteLevel getRewriteLevel() const { return rewriteLevel_; }
   ...
   */
  QRForcedMVsPtr getForcedMVs() const { return forcedMVs_; }

  /**
   * Sets the user ID for the query.
   * @param id The user ID.
   */
  void setUserID(NAString &id) { userID_ = id; }

  /**
   * Sets the MV age associated with the query.
   * @param age The MV age.
   */
  void setMVAge(NAString &age) { mvAge_ = age; }

  /**
   * Sets the MV age associated with the query.
   * @param age The MV age.
   */
  void setMVAge(char *age) { mvAge_ = *age; }

  /**
   * Sets the optimization level to use for the query.
   * @param optLevel The optimization level.
   */
  void setOptLevel(NAString &optLevel) { optLevel_ = optLevel; }
   ...
   */
  void setRewriteLevel(MvqrRewriteLevel rewriteLevel) { rewriteLevel_ = rewriteLevel; }

  /**
   * Sets the list of MVs that QMS is not to disqualify.
   * @param forcedMVs The list of MVs.
   */
  void setForcedMVs(QRForcedMVsPtr forcedMVs) { forcedMVs_ = forcedMVs; }
...
class QRQueryDescriptor : public QRDescriptor {
 public:
   ...
   * @param[in] lineNum Line number at which this constructor was called.
   */
  QRQueryDescriptor(AttributeList atts, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0));
...
class QRJBB : public QRElement {
 public:
  ...
  QRJBB(XMLElementPtr parent, AttributeList atts, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0));
   ...
   */
  virtual NABoolean canBeJbbc() { return TRUE; }

  // Getters/setters

  virtual char getIDFirstChar() { return 'B'; }

  /**
   * Returns the hub of this JBB.
   * @return The JBB's hub.
   */
  QRHubPtr getHub() const { return hub_; }

  /**
   * Returns the extra-hub of this JBB.
   * @return The JBB's extra-hub.
   */
  QRExtraHubPtr getExtraHub() const { return extraHub_; }

  /**
   * Returns the select list items for this JBB.
   * @return Select list items of the JBB.
   */
  QROutputListPtr getOutputList() const { return outputList_; }

  /**
   * Sets the hub of this JBB
   * @param hub The JBB's hub.
   */
  void setHub(QRHubPtr hub) { hub_ = hub; }

  /**
   * Sets the extra-hub of this JBB.
   * @param xhub The JBB's extra-hub.
   */
  void setExtraHub(QRExtraHubPtr xhub) { extraHub_ = xhub; }
   ...
   */
  CollIndex getNodeId() const { return caNodeId_; }
  ...
  QRTablePtr *getTableArray() const { return tableArray_; }

  CollIndex getTableCount() const { return tableCount_; }

  GroupByAgg *getGbExpr() const { return gbExpr_; }

  void setGbExpr(GroupByAgg *gb) { gbExpr_ = gb; }
...
class QRInfo : public QRElement {
 public:
  ...
  QRInfo(ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0))
      : QRElement(ET_Info, NULL, ADD_MEMCHECK_ARGS_PASS(heap)), text_(charData_) {}

  QRInfo(XMLElementPtr parent, AttributeList atts, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0))
      : QRElement(ET_Info, parent, ADD_MEMCHECK_ARGS_PASS(heap)), text_(charData_) {}
   ...
   */
  void setText(const NAString &text) { text_ = text; }
   ...
   */
  virtual void serializeBody(XMLString &xml) { xml.append("<![CDATA[").append(charData_).append("]]>"); }
...
class QRHub : public QRElement {
 public:
  ...
  QRHub(XMLElementPtr parent, AttributeList atts, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0));
...
class QRJBBCList : public QRElementList {
 public:
  ...
  QRJBBCList(ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0)) : QRElementList(ET_JBBCList, ADD_MEMCHECK_ARGS_PASS(heap)) {}

  QRJBBCList(XMLElementPtr parent, AttributeList atts, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0));
 ...
 protected:
   ...
   */
  virtual NABoolean isAllowed(QRElementPtr elem) { return elem->canBeJbbc(); }
...
class QRTable : public QRElement {
 public:
  ...
  QRTable(XMLElementPtr parent, AttributeList atts, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0));

  virtual ~QRTable() {}

  virtual QRTablePtr downCastToQRTable() { return this; }
  ...
  virtual const NAString &getSortName() const { return tableName_; }

  // Getters/setters

  virtual char getIDFirstChar() { return 'T'; }
   ...
   */
  const NAString &getTimestamp() const { return redefTimestamp_; }
   ...
   */
  const NABoolean isKeyCovered() const { return isKeyCovered_; }
   ...
   *                   otherwise.
   */
  void setIsKeyCovered(NABoolean keyCovered) { isKeyCovered_ = keyCovered; }
  ...
  void setExtraHub(NABoolean eh) { isExtraHub_ = eh; }

  virtual NABoolean isExtraHub() { return isExtraHub_; }

  /**
   * Sets the bitmap of columns with range predicates.
   * @param bits
   */
  void setRangeBits(const XMLBitmap &bits) { rangeBits_ = bits; }
   ...
   */
  void setRangeBit(CollIndex index) { rangeBits_ += index; }

  /**
   * Sets the bitmap of columns with residual predicates.
   * @param bits
   */
  void setResidualBits(const XMLBitmap &bits) { residualBits_ = bits; }
   ...
   *              position in the table.
   */
  void setResidualBit(CollIndex index) { residualBits_ += index; }

  /**
   * Set this table as the right child of a left outer join.
   * @param hasLOJParent
   */
  void setLOJParent(NABoolean hasLOJParent) { hasLOJParent_ = hasLOJParent; }

  /**
   * Set the primary key of the table.
   * @param key
   */
  void setKey(QRKeyPtr key) { key_ = key; }

  void setCorrelationName(const NAString &name) { correlationName_ = name; }
...
class QRKey : public QRElementList {
 public:
  ...
  QRKey(ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0)) : QRElementList(ET_JBBCList, ADD_MEMCHECK_ARGS_PASS(heap)) {}

  QRKey(XMLElementPtr parent, AttributeList atts, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0));
 ...
 protected:
   ...
   */
  virtual NABoolean isAllowed(QRElementPtr elem) { return elem->getElementType() == ET_Column; }
...
class QRForcedMVs : public QRElement {
 public:
  ...
  QRForcedMVs(ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0))
      : QRElement(ET_ForcedMVs, NULL, ADD_MEMCHECK_ARGS_PASS(heap)), tableList_(heap) {}

  QRForcedMVs(XMLElementPtr parent, AttributeList atts, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0))
      : QRElement(ET_ForcedMVs, parent, ADD_MEMCHECK_ARGS_PASS(heap)), tableList_(heap) {}
   ...
   */
  const NAPtrList<QRTablePtr> &getTableList() const { return tableList_; }

  /**
   * Adds an MV to the list of forced MVs.
   * @param tbl MV to add to the list.
   */
  void addTable(QRTablePtr tbl) { tableList_.insert(tbl); }
...
class QRExplicitExpr : public QRElement {
 public:
   ...
   * @return
   */
  virtual NABoolean isAnAggregate() const { return FALSE; }
  ...
  virtual void getInputColumns(ElementPtrList &inputList, CollHeap *heap, NABoolean useRefedElem = TRUE);
  ...
  virtual ItemExpr *toItemExpr(const NAString &mvName, CollHeap *heap, TableNameScanHash *scanhash) = 0;

 protected:
  QRExplicitExpr(ElementType eType, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0))
      : QRElement(eType, NULL, ADD_MEMCHECK_ARGS_PASS(heap)), containsAnAggregate_(AGGREGATE_UNKNOWN) {}

  QRExplicitExpr(ElementType eType, XMLElementPtr parent, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0))
      : QRElement(eType, parent, ADD_MEMCHECK_ARGS_PASS(heap)), containsAnAggregate_(AGGREGATE_UNKNOWN) {}
   ...
   */
  QRExplicitExprPtr constructSubElement(void *parser, const char *elementName, const char **atts);
 ...
 private:
   ...
   */
  enum AggregateStatus { AGGREGATE_NO = FALSE, AGGREGATE_YES = TRUE, AGGREGATE_UNKNOWN };
...
class QRColumn : public QRExplicitExpr {
 public:
  ...
  QRColumn(XMLElementPtr parent, AttributeList atts, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0));
   ...
   * @return \c TRUE.
   */
  virtual NABoolean canBeInJoinPred() { return TRUE; }
   ...
   */
  virtual NABoolean canBeOutputItem() const { return TRUE; }
   ...
   *         <code>Group By</code> item.
   */
  virtual NABoolean canBeGroupByItem() const { return TRUE; }

  virtual const NAString &getSortName() const { return fqColumnName_; }

  // Getters/setters

  virtual char getIDFirstChar() { return 'C'; }
   ...
   */
  const NAString &getTableID() const { return tableId_; }
   ...
   */
  NumericID getTableIDNum() const { return (tableId_.length() < 2 ? 0 : atoi(tableId_.data() + 1)); }

  /**
   * Returns the fully-qualified name of this column.
   * @return The column's name.
   */
  const NAString &getFullyQualifiedColumnName() const { return fqColumnName_; }

  /**
   * Returns the name of this column.
   * @return The column's name.
   */
  const NAString &getColumnName() const { return columnName_; }
   ...
   * @return
   */
  NABoolean isNullable() const { return isNullable_; }
   ...
   */
  void setTableID(const NAString &tblId) { tableId_ = tblId; }
   ...
   * @param nullable
   */
  void setNullable(NABoolean nullable) { isNullable_ = nullable; }
  ...
  CollIndex getVegrefId() const { return vegrefVid_; }

  void setVegrefId(CollIndex vid) { vegrefVid_ = vid; }
  ...
  virtual ItemExpr *toItemExpr(const NAString &mvName, CollHeap *heap, TableNameScanHash *scanhash);
...
class QRMVColumn : public QRExplicitExpr {
 public:
  ...
  QRMVColumn(XMLElementPtr parent, AttributeList atts, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0));
  ...
  virtual QRMVColumnPtr downCastToQRMVColumn() { return this; }
   ...
   *         expression tree.
   */
  virtual NABoolean canBeExprRoot() const { return TRUE; }
   ...
   */
  const NAString &getMV() const { return mv_; }

  /**
   * Returns the unqualified name of this MV column.
   * @return The MV column name.
   */
  const NAString &getMVColName() const { return mvColName_; }

  /**
   * Sets the name of the MV owning this column.
   * @param mvName The MV name.
   */
  void setMV(const NAString &mvName) { mv_ = mvName; }

  /**
   * Sets the unqualified name of this MV column.
   * @param name Name of the MV column.
   */
  void setMVColName(const NAString &name) { mvColName_ = name; }

  /**
   * Create a text representation of the expression.
   * @param text String to append to
   */
  virtual void unparse(NAString &text, NABoolean useColumnName) { text += getMVColName(); }
   ...
   */
  OperatorTypeEnum getAggForRewrite() const { return aggForRewrite_; }
   ...
   */
  void setAggForRewrite(OperatorTypeEnum agg) { aggForRewrite_ = agg; }
...
class QROperator : public QRElement {
 public:
  ...
  QROperator(ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0)) : QRElement(ET_Operator, NULL, ADD_MEMCHECK_ARGS_PASS(heap)) {}

  QROperator(XMLElementPtr parent, AttributeList atts, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0));
...
class QRJoinPred : public QRElementList {
 public:
  ...
  QRJoinPred(ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0))
      : QRElementList(ET_JoinPred, ADD_MEMCHECK_ARGS_PASS(heap)), result_(INVALID_EXPR_RESULT) {}

  QRJoinPred(XMLElementPtr parent, AttributeList atts, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0));

  virtual ~QRJoinPred() {}

  virtual QRJoinPredPtr downCastToQRJoinPred() { return this; }
  ...
  virtual char getIDFirstChar() { return 'J'; }
   ...
   * @return
   */
  virtual const NAString &getSortName() const { return ((QRJoinPredPtr)this)->getFirstColumn()->getSortName(); }
...
class QRRangePred : public QRElement {
 public:
  ...
  QRRangePred(XMLElementPtr parent, AttributeList atts, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0));

  virtual ~QRRangePred();

  virtual QRRangePredPtr downCastToQRRangePred() { return this; }
   ...
   */
  QRElementPtr getRangeItem() const { return rangeItem_; }

  /**
   * Returns the list of operators defining this range predicate.
   * @return Operator list for this range predicate.
   */
  const NAPtrList<QRRangeOperatorPtr> &getOperatorList() const { return opList_; }
   ...
   */
  NABoolean getMustMatch() const { return mustMatch_; }
   ...
   */
  void setSqlType(const NAString &sqlType) { sqlType_ = sqlType; }
   ...
   */
  void setRangeItem(QRElementPtr item) { rangeItem_ = item; }

  /**
   * Adds a range operator to the list for this MV.
   * @param op The operator to add to the list.
   */
  void addOperator(QRRangeOperatorPtr op) { opList_.insert(op); }
   ...
   */
  void setMustMatch(NABoolean mustMatch) { mustMatch_ = mustMatch; }
...
class QROpEQ : public QRRangeOperator {
 public:
  ...
  QROpEQ(ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0))
      : QRRangeOperator(ET_OpEQ, ADD_MEMCHECK_ARGS_PASS(heap)), valueList_(heap), nullVal_(NULL) {}

  QROpEQ(QRElement *parent, AttributeList atts, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0))
      : QRRangeOperator(ET_OpEQ, parent, ADD_MEMCHECK_ARGS_PASS(heap)), valueList_(heap), nullVal_(NULL) {
    if (*atts) throw QRDescriptorException("Invalid attribute specified for element %s: %s", elemName, *atts);
  }
   ...
   */
  const NAPtrList<QRScalarValuePtr> &getValueList() const { return valueList_; }
   ...
   */
  NABoolean includesNull() const { return nullVal_ != NULL; }

  /**
   * Adds a value to those that are part of the range.
   * @param value The value to add.
   */
  void addValue(QRScalarValuePtr value) { valueList_.insert(value); }
   ...
   */
  void setNullVal(QRNullValPtr nullVal) { nullVal_ = nullVal; }
...
class QROpInequality : public QRRangeOperator {
 public:
   ...
   */
  QROpInequality(ElementType eType, QRElement *parent, AttributeList atts, const char *elemName,
   ...
   */
  QRScalarValuePtr getValue() const { return value_; }

  /**
   * Sets the value associated with this subrange.
   * @param val The value used for the subrange.
   */
  void setValue(QRScalarValuePtr val) { value_ = val; }
   ...
   */
  NABoolean isNormalized() const { return isNormalized_; }
   ...
   */
  void setNormalized(NABoolean isNormalized) { isNormalized_ = isNormalized; }
...
class QROpLS : public QROpInequality {
 public:
  ...
  QROpLS(ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0)) : QROpInequality(ET_OpLS, ADD_MEMCHECK_ARGS_PASS(heap)) {}

  QROpLS(QRElement *parent, AttributeList atts, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0))
...
class QROpLE : public QROpInequality {
 public:
  ...
  QROpLE(ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0)) : QROpInequality(ET_OpLE, ADD_MEMCHECK_ARGS_PASS(heap)) {}

  QROpLE(QRElement *parent, AttributeList atts, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0))
...
class QROpGT : public QROpInequality {
 public:
  ...
  QROpGT(ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0)) : QROpInequality(ET_OpGT, ADD_MEMCHECK_ARGS_PASS(heap)) {}

  QROpGT(QRElement *parent, AttributeList atts, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0))
...
class QROpGE : public QROpInequality {
 public:
  ...
  QROpGE(ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0)) : QROpInequality(ET_OpGE, ADD_MEMCHECK_ARGS_PASS(heap)) {}

  QROpGE(QRElement *parent, AttributeList atts, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0))
...
class QROpBT : public QRRangeOperator {
 public:
  ...
  QROpBT(ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0))
      : QRRangeOperator(ET_OpBT, ADD_MEMCHECK_ARGS_PASS(heap)),
        ...
        startValue_(NULL),
        endValue_(NULL),
        startIsIncluded_(TRUE),
        endIsIncluded_(TRUE) {}

  QROpBT(QRElement *parent, AttributeList atts, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0));
   ...
   */
  QRScalarValuePtr getStartValue() const { return startValue_; }
   ...
   */
  QRScalarValuePtr getEndValue() const { return endValue_; }
   ...
   *         otherwise.
   */
  NABoolean startIsIncluded() const { return startIsIncluded_; }
   ...
   */
  void setStartIncluded(NABoolean included) { startIsIncluded_ = included; }
   ...
   *                 value, \c FALSE otherwise.
   */
  void setEndIncluded(NABoolean included) { endIsIncluded_ = included; }
 ...
 protected:
  ...
  virtual void endElement(void *parser, const char *elementName) {
    if (valueCount_ != 2) throw QRDescriptorException("Two values must be contained in %s", elementName);
...
class QRScalarValue : public QRExplicitExpr {
 public:
   ...
   */
  virtual const NAString &getValue() const { return stringRep_; }
   ...
   * @param text String to append to
   */
  virtual void unparse(NAString &text, NABoolean useColumnName) { text += stringRep_; }

  virtual ItemExpr *toItemExpr(const NAString &mvName, CollHeap *heap, TableNameScanHash *scanhash);
  virtual void serializeAttrs(XMLString &xml);

  virtual char getIDFirstChar() { return 'S'; }

  void setSql(const NAString &sql) { sql_ = sql; }

  const NAString &getSql() const { return sql_; }

 protected:
  QRScalarValue(ElementType eType, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0))
      : QRExplicitExpr(eType, NULL, ADD_MEMCHECK_ARGS_PASS(heap)), stringRep_(charData_), sql_(heap) {}

  QRScalarValue(ElementType eType, QRElement *parent, AttributeList atts, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0));

  QRScalarValue(const QRScalarValue &other, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0))
      : QRExplicitExpr(other, ADD_MEMCHECK_ARGS_PASS(heap)), stringRep_(charData_), sql_(heap) {}

  /**
   * Sets the string representation of the value represented by this object.
   * @param value The value represented by this object.
   */
  void setStringRep(const NAString &value) { stringRep_ = value; }
 ...
 public:
  ...
  QRNumericVal(ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0))
      : QRScalarValue(ET_NumericVal, ADD_MEMCHECK_ARGS_PASS(heap)),
        unscaledNumericVal_(0),
        numericScale_(0),
        scale_(heap) {}

  QRNumericVal(QRElement *parent, AttributeList atts, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0));
   ...
   */
  const NAString &getScale() const { return scale_; }
...
class QRStringVal : public QRScalarValue {
 public:
  ...
  QRStringVal(ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0)) : QRScalarValue(ET_StringVal, ADD_MEMCHECK_ARGS_PASS(heap)) {}
  ...
  QRStringVal(QRElement *parent, AttributeList atts, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0))
  ...
  virtual void setValue(const NAString &value) { setStringRep(value); }
...
class QRWStringVal : public QRScalarValue {
 public:
  ...
  QRWStringVal(ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0))
      : QRScalarValue(ET_WStringVal, ADD_MEMCHECK_ARGS_PASS(heap)), wideStringRep_(heap), danglingCharCount_(0) {}
  ...
  QRWStringVal(QRElement *parent, AttributeList atts, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0))
      : QRScalarValue(ET_WStringVal, parent, atts, ADD_MEMCHECK_ARGS_PASS(heap)), danglingCharCount_(0) {}

  // Copy Ctor
  QRWStringVal(const QRWStringVal &other, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0))
      : QRScalarValue(ET_WStringVal, ADD_MEMCHECK_ARGS_PASS(heap)), danglingCharCount_(0) {
  ...
  void setWideValue(const NAWString &value) { setWStringRep(value); }
  ...
  const NAWString &getWideValue() const { return wideStringRep_; }
   ...
   * @param value The value represented by this object.
   */
  void setWStringRep(const NAWString &value) { wideStringRep_ = value; }
...
class QRFloatVal : public QRScalarValue {
 public:
  ...
  QRFloatVal(ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0))
      : QRScalarValue(ET_FloatVal, ADD_MEMCHECK_ARGS_PASS(heap)), floatVal_(0) {}

  QRFloatVal(double val, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0))
      : QRScalarValue(ET_FloatVal, ADD_MEMCHECK_ARGS_PASS(heap)), floatVal_(val) {
    sprintf(buf, "%g", val);
    stringRep_ = buf;
  }

  QRFloatVal(QRElement *parent, AttributeList atts, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0))
      : QRScalarValue(ET_FloatVal, parent, atts, ADD_MEMCHECK_ARGS_PASS(heap)), floatVal_(0) {}

  // Copy Ctor
  QRFloatVal(const QRFloatVal &other, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0))
      : QRScalarValue(ET_FloatVal, ADD_MEMCHECK_ARGS_PASS(heap)), floatVal_(other.floatVal_) {
    setValue(other.getValue());
  }
...
class QRNullVal : public QRExplicitExpr {
 public:
  ...
  QRNullVal(XMLElementPtr parent, AttributeList atts, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0))
   ...
   * @param text String to append to.
   */
  virtual void unparse(NAString &text, NABoolean useColumnName) { text += "NULL"; }
...
class QRPrimaryGroupBy : public QRElementList {
 public:
  ...
  QRPrimaryGroupBy(XMLElementPtr parent, AttributeList atts, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0));

  virtual ~QRPrimaryGroupBy() {}

  virtual const char *getElementName() const { return elemName; }

  virtual char getIDFirstChar() { return 'G'; }
 ...
 protected:
   ...
   */
  virtual NABoolean isAllowed(QRElementPtr elem) { return elem->canBeGroupByItem(); }
...
class QRDependentGroupBy : public QRElementList {
 public:
  ...
  QRDependentGroupBy(XMLElementPtr parent, AttributeList atts, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0));
...
class QRMinimalGroupBy : public QRElementList {
 public:
  ...
  QRMinimalGroupBy(XMLElementPtr parent, AttributeList atts, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0));
...
class QRGroupBy : public QRElement {
 public:
  ...
  QRGroupBy(XMLElementPtr parent, AttributeList atts, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0));
   ...
   */
  ExprResult getResult() const { return result_; }
   ...
   */
  const QRPrimaryGroupByPtr getPrimaryList() const { return primary_; }

  /**
   * Returns the list of dependent grouping columns/expressions.
   * @return List of grouping items.
   */
  const QRDependentGroupByPtr getDependentList() const { return dependent_; }

  /**
   * Returns the list of minimal grouping columns/expressions.
   * @return List of grouping items.
   */
  const QRMinimalGroupByPtr getMinimalList() const { return minimal_; }

  /**
   * Returns the list of tables that are outside the hub.
   * @return List of extra-hub tables.
   */
  QRTableListPtr getTableList() const { return tableList_; }

  void addPrimaryGroupingElement(QRElementPtr elem) { primary_->addElement(elem); }

  void addDependentGroupingElement(QRElementPtr elem) { dependent_->addElement(elem); }

  void addMinimalGroupingElement(QRElementPtr elem) { minimal_->addElement(elem); }

  /**
   * Adds a table to the list of extra-hub tables.
   * @param tbl The table to be added to the list.
   */
  void addTable(QRTablePtr tbl) { tableList_->addItem(tbl); }

  NABoolean isEmpty() { return primary_ == NULL || primary_->entries() == 0; }
...
class QROutput : public QRElementList {
 public:
  ...
  QROutput(XMLElementPtr parent, AttributeList atts, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0));

  virtual ~QROutput() {}

  virtual QROutputPtr downCastToQROutput() { return this; }

  virtual const char *getElementName() const { return elemName; }

  // Getters/setters

  virtual char getIDFirstChar() { return 'O'; }
   ...
   */
  QRElementPtr getOutputItem() const { return (entries() > 0 ? getList()[0] : NULL); }
   ...
   */
  const ElementPtrList &getOutputItems() const { return getList(); }
   ...
   */
  void setOutputItem(QRElementPtr elem) { addElement(elem); }
   ...
   * @return
   */
  virtual NABoolean isAllowed(QRElementPtr elem) { return elem->canBeOutputItem(); }
...
class QRExtraHub : public QRElement {
 public:
  ...
  QRExtraHub(XMLElementPtr parent, AttributeList atts, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0))
      : QRElement(ET_ExtraHub, parent, ADD_MEMCHECK_ARGS_PASS(heap)), tableList_(NULL), joinPredList_(NULL) {
    if (*atts) throw QRDescriptorException("Invalid attribute specified for element %s: %s", elemName, *atts);
  }
...
class QRBinaryOper : public QRExplicitExpr {
 public:
  ...
  QRBinaryOper(XMLElementPtr parent, AttributeList atts, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0));
   ...
   */
  const NAString &getOperator() const { return operator_; }
   ...
   */
  QRExplicitExprPtr getFirstOperand() const { return firstOperand_; }
   ...
   */
  QRExplicitExprPtr getSecondOperand() const { return secondOperand_; }

  /**
   * Sets the name of this operation.
   * @param[in] opType Name of the operation.
   */
  void setOperator(const NAString &opType) { operator_ = opType; }
   ...
   *               first operand.
   */
  void setFirstOperand(QRExplicitExprPtr op) { firstOperand_ = op; }
   ...
   *               second operand.
   */
  void setSecondOperand(QRExplicitExprPtr op) { secondOperand_ = op; }
...
class QRUnaryOper : public QRExplicitExpr {
 public:
  ...
  QRUnaryOper(ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0))
      : QRExplicitExpr(ET_UnaryOper, NULL, ADD_MEMCHECK_ARGS_PASS(heap)), operator_(heap), operand_(NULL) {}

  QRUnaryOper(XMLElementPtr parent, AttributeList atts, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0));

  // Copy Ctor
  QRUnaryOper(const QRUnaryOper &other, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0))
      : QRExplicitExpr(other, ADD_MEMCHECK_ARGS_PASS(heap)), operator_(other.operator_, heap), operand_(NULL) {}

  virtual ~QRUnaryOper() { deletePtr(operand_); }
   ...
   */
  QRExplicitExprPtr getOperand() const { return operand_; }
   ...
   *           operand.
   */
  void setOperand(QRExplicitExprPtr op) { operand_ = op; }
...
class QRParameter : public QRElement {
 public:
  ...
  QRParameter(ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0))
      : QRElement(ET_Parameter, NULL, ADD_MEMCHECK_ARGS_PASS(heap)), paramName_(heap), paramValue_(heap) {}

  QRParameter(XMLElementPtr parent, AttributeList atts, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0));
  ...
  const NAString &getName() const { return paramName_; }

  const NAString &getValue() const { return paramValue_; }

  void setName(const char *name) { paramName_ = name; }
...
class QRFunction : public QRExplicitExpr {
 public:
  ...
  QRFunction(XMLElementPtr parent, AttributeList atts, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0));

  // Copy Ctor
  QRFunction(const QRFunction &other, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0));

 public:
  virtual QRFunctionPtr downCastToQRFunction() { return this; }
   ...
   */
  const NAString &getFunctionName() const { return function_; }

  /**
   * Returns a list of all the function arguments.
   * @return List of function arguments.
   */
  const NAPtrList<QRExplicitExprPtr> &getArguments() const { return arguments_; }

  /**
   * Returns a list of the hidden (implicitly added) parameters.
   * @return List of hidden parameters.
   */
  const NAPtrList<QRParameterPtr> &getHiddenParams() const { return hiddenParams_; }

  /**
   * Sets the name of this function.
   * @param fnName Function name to use.
   */
  void setFunctionName(const NAString &fnName) { function_ = fnName; }

  /**
   * Adds an argument to the argument list for this function.
   * @param arg The argument to add to the list.
   */
  void addArgument(QRExplicitExprPtr arg) { arguments_.insert(arg); }
   ...
   */
  void addHiddenParam(QRParameterPtr param) { hiddenParams_.insert(param); }
   ...
   */
  void setAggregateFunc(AggregateFunctionType aft) { aggregateFunc_ = aft; }

  /**
   * Get the aggregate function type
   * @return
   */
  AggregateFunctionType getAggregateFunc() { return aggregateFunc_; }
   ...
   * @return
   */
  virtual NABoolean isAnAggregate() const { return aggregateFunc_ != AFT_NONE; }
...
class QRExpr : public QRElement {
 public:
  ...
  QRExpr(NABoolean isResidual = FALSE, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0))
  ...
  QRExpr(XMLElementPtr parent, AttributeList atts, NABoolean isResidual = FALSE,
         ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0));

  virtual ~QRExpr();

  virtual QRExprPtr downCastToQRExpr() { return this; }

  virtual const char *getElementName() const { return (isResidual_ ? residElemName : elemName); }
   ...
   */
  virtual NABoolean canBeInJoinPred() { return !isResidual_; }
   ...
   * @return \c FALSE for a residual predicate, TRUE for any other expression.
   */
  virtual NABoolean canBeOutputItem() const { return !isResidual_; }
   ...
   * @return \c FALSE for a residual predicate, TRUE for any other expression.
   */
  virtual NABoolean canBeGroupByItem() const { return !isResidual_; }

  // Getters/setters

  virtual char getIDFirstChar() { return (isResidual_ ? 'D' : 'X'); }
   ...
   */
  QRExplicitExprPtr getExprRoot() const { return exprRoot_; }
  ...
  void setExprRoot(QRExplicitExprPtr elem) {
    if (!elem->canBeExprRoot())
      throw QRDescriptorException("Element %s cannot be the root of an expression tree", elem->getElementName());
    exprRoot_ = elem;
  }
  ...
  CharInfo::CharSet getExprTextCharSet() { return unparsedTextCharSet_; }

  virtual const ElementPtrList &getInputColumns(CollHeap *heap, NABoolean useRefedElem = TRUE);
  ...
  ExprResult getResult() const {
    assertLogAndThrow(CAT_SQL_COMP_QR_DESC_GEN, LL_MVQR_FAIL, isResidual_, QRDescriptorException,
  ...
  void setResidual(NABoolean isResidual) { isResidual_ = isResidual; }

  void setInfo(QRInfoPtr info) { info_ = info; }

  const QRInfoPtr getInfo() const { return info_; }
...
class QRMVMisc : public QRElement {
 public:
  ...
  QRMVMisc(XMLElementPtr parent, AttributeList atts, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0));
   ...
   *                    \c FALSE otherwise.
   */
  void setIsIncremental(NABoolean incremental) { isIncremental_ = incremental; }
   ...
   *                    \c FALSE otherwise.
   */
  void setIsImmediate(NABoolean immediate) { isImmediate_ = immediate; }
   ...
   *                    \c FALSE otherwise.
   */
  void setIsUMV(NABoolean umv) { isUMV_ = umv; }

  /**
   * Tells whether the MV descriptor is for a query in workload analysis mode.
   * @return
   */
  NABoolean isFromQuery() const { return isFromQuery_; }

  /**
   * Set whether the MV descriptor is for a query in workload analysis mode.
   * @param incremental
   */
  void setFromQuery(NABoolean isFromQuery) { isFromQuery_ = isFromQuery; }
...
class QRVersion : public QRElement {
 public:
  ...
  QRVersion(ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0))
      : QRElement(ET_Version, NULL, ADD_MEMCHECK_ARGS_PASS(heap)), versionString_(charData_) {}

  QRVersion(QRElement *parent, AttributeList atts, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0))
      : QRElement(ET_Version, parent, ADD_MEMCHECK_ARGS_PASS(heap)), versionString_(charData_) {
    if (*atts) throw QRDescriptorException("Invalid attribute specified for element %s: %s", elemName, *atts);
  }
   ...
   */
  const NAString &getVersionString() const { return versionString_; }

  /**
   * Sets the version string for this descriptor.
   * @param version The descriptor's version.
   */
  void setVersionString(NAString &version) { versionString_ = version; }
...
class QRMVDescriptor : public QRDescriptor {
 public:
  ...
  QRMVDescriptor(XMLElementPtr parent, AttributeList atts, ADD_MEMCHECK_ARGS_DEF(NAMemory *heap))
      : QRDescriptor(ET_MVDescriptor, parent, ADD_MEMCHECK_ARGS_PASS(heap)), version_(NULL), table_(NULL), misc_(NULL) {
    if (*atts)
      throw QRDescriptorException("<%s> should have no attributes; attribute %s specified", getElementName(), *atts);
  }
...
class QRMVName : public QRElement {
 public:
  ...
  QRMVName(ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0))
      : QRElement(ET_MVName, NULL, ADD_MEMCHECK_ARGS_PASS(heap)), timestamp_(heap), fqMVName_(charData_) {}

  QRMVName(XMLElementPtr parent, AttributeList atts, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0));
...
class QRCandidate : public QRElement {
 public:
  ...
  QRCandidate(XMLElementPtr parent, AttributeList atts, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0));
   ...
   *                           \c FALSE otherwise.
   */
  void setPreferredMatch(NABoolean preferredMatch) { isPreferredMatch_ = preferredMatch; }
   ...
   */
  void setIndirectGroupBy(NABoolean indirectGroupBy) { isIndirectGroupBy_ = indirectGroupBy; }
   ...
   */
  void setStatsOnly(NABoolean statsOnly) { statsOnly_ = statsOnly; }

  /**
   * Sets the name of this candidate MV.
   * @param[in] mvName The fully-qualified name of the MV.
   */
  void setMVName(QRMVNamePtr mvName) { mvName_ = mvName; }
   ...
   * @param table
   */
  void addBackJoinTable(QRTablePtr table) { tableList_->addItem(table); }

  /**
   * Add a Join predicate
   * @param pred
   */
  void addJoinPred(QRJoinPredPtr pred) { joinPredList_->addItem(pred); }
...
class QRJbbSubset : public QRElement {
 public:
  ...
  QRJbbSubset(XMLElementPtr parent, AttributeList atts, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0));
...
class QRJbbResult : public QRElement {
 public:
  ...
  QRJbbResult(ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0))
      : QRElement(ET_JbbResult, NULL, ADD_MEMCHECK_ARGS_PASS(heap)), jbbSubsets_(heap), infoItems_(heap) {}

  QRJbbResult(XMLElementPtr parent, AttributeList atts, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0));
...
class QRResultDescriptor : public QRElement {
 public:
  QRResultDescriptor(ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0))
      : QRElement(ET_ResultDescriptor, NULL, ADD_MEMCHECK_ARGS_PASS(heap)), version_(NULL), jbbResults_(heap) {}
   ...
   */
  const NAPtrList<QRJbbResultPtr> &getJbbResults() const { return jbbResults_; }
   ...
   */
  void addJbbResult(QRJbbResultPtr jbbResult) { jbbResults_.insert(jbbResult); }
...
class QRPublishDescriptor : public QRElement {
 public:
   ...
   * @param[in] lineNum Line number at which this constructor was called.
   */
  QRPublishDescriptor(AttributeList atts, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0));
  ...
  void initialize(ComPublishMVOperationType opType, const NAString *redefTimestamp, NAString *mvDescText,
                  const NAString *mvName, NABoolean hasIgnoreChanges, const NAString *refreshTimestamp,
                  const NAString *newName, NAMemory *heap);
   ...
   */
  const NAString &getRedefTimestamp() const { return redefTimestamp_; }
  ...
  const NAString &getName() const {
    if (name_)
      ...
      assertLogAndThrow(CAT_SQL_COMP_QR_DESC_GEN, LL_MVQR_FAIL, mvDesc_ != NULL, QRDescriptorException,
   ...
   */
  const NAPtrList<QRUpdatePtr> &getUpdateList() const { return updateList_; }
   ...
   */
  const QRIncludePtr getIncludeFile() const { return includeFile_; }
   ...
   */
  const QRMVDescriptorPtr getMV() const { return mvDesc_; }
   ...
   */
  const NAString *getMVText() const { return mvDescText_; }
   ...
   */
  void setRedefTimestamp(const NAString &redefTimestamp) { redefTimestamp_ = redefTimestamp; }

  /**
   * Sets the name of the MV this publish message is for.
   * @param name The published MV's name.
   */
  void setName(const QRMVNamePtr name) { name_ = name; }

  /**
   * Adds an Update operation to the list for this Publish message.
   * @param update The update to add to the list.
   */
  void addUpdate(QRUpdatePtr update) { updateList_.insert(update); }
   ...
   */
  void setIncludeFile(QRIncludePtr includeFile) { includeFile_ = includeFile; }
   ...
   */
  void setMV(QRMVDescriptorPtr mvDesc) { mvDesc_ = mvDesc; }
  ...
  void setMVText(NAString *mvDescText, NAMemory *heap) {
    if (mvDescText) mvDescText_ = new (heap) NAString(*mvDescText);
  }
...
class QRUpdate : public QRElement {
 public:
  ...
  QRUpdate(XMLElementPtr parent, AttributeList atts, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0));
   ...
   */
  QRUpdate(UpdateType operationType, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0));
  ...
  // Getters/setters for attributes/subelements
  UpdateType getUpdateType() { return type_; }

  void setUpdateType(UpdateType type) { type_ = type; }

  const NAString &getRefreshTimestamp() {
    assertLogAndThrow(CAT_SQL_COMP_QR_DESC_GEN, LL_MVQR_FAIL, type_ == REFRESH || type_ == RECOMPUTE,
                      QRDescriptorException,
  ...
  const NAString &getNewName() {
    assertLogAndThrow(CAT_SQL_COMP_QR_DESC_GEN, LL_MVQR_FAIL, type_ == RENAME, QRDescriptorException,
  ...
  const NABoolean getIgnoreChanges() {
    assertLogAndThrow(CAT_SQL_COMP_QR_DESC_GEN, LL_MVQR_FAIL, type_ == ALTER, QRDescriptorException,
  ...
  void setDefaultValue(const NAString &name, const NAString &value) {
    assertLogAndThrow(CAT_SQL_COMP_QR_DESC_GEN, LL_MVQR_FAIL, type_ == DEFAULT, QRDescriptorException,
                      "Defaults can only be used in Default Update elements.");
    defaultName_ = name;
    defaultValue_ = value;
  }

  const NAString &getDefaultName() { return defaultName_; }

  const NAString &getDefaultValue() { return defaultValue_; }
...
class QRInclude : public QRElement {
 public:
  ...
  QRInclude(ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0))
      : QRElement(ET_Include, NULL, ADD_MEMCHECK_ARGS_PASS(heap)), fileName_(charData_) {}

  QRInclude(XMLElementPtr parent, AttributeList atts, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0))
      : QRElement(ET_Include, parent, ADD_MEMCHECK_ARGS_PASS(heap)), fileName_(charData_) {}
   ...
   */
  void setFileName(const NAString &fileName) { fileName_ = fileName; }
