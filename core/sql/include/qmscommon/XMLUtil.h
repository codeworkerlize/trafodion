...
class XMLString : public NAString {
 public:
   ...
   */
  XMLString(NAMemory *heap = NASTRING_UNINIT_HEAP_PTR) : NAString(heap) {}
   ...
   *         unformatted string.
   */
  virtual short getLevel() const { return 0; }
   ...
   */
  virtual XMLString &incrementLevel() { return *this; }
   ...
   * @return Reference to this object.
   */
  virtual XMLString &decrementLevel() { return *this; }
   ...
   * @return Reference to this object.
   */
  virtual XMLString &indent() { return *this; }
...
class XMLFormattedString : public XMLString {
 public:
   ...
   *         string.
   */
  virtual short getLevel() const { return level_; }
   ...
   */
  virtual XMLString &appendCharData(NAString &charData) {
    if (charData.length() == 0) return *this;
    indent();
    append(charData);
    endLine();
    return *this;
   ...
   */
  virtual XMLString &appendCharData(char *charData) {
    if (!charData || !*charData) return *this;
    indent();
    append(charData);
    endLine();
    return *this;
...
class XMLAttributeIterator {
 public:
   ...
   */
  XMLAttributeIterator(AttributeList atts) : atts_(atts), inx_(-2), inRange_(FALSE) {}
   ...
   */
  NABoolean hasNext() const { return *(atts_ + (inx_ + 2)) != NULL; }
  ...
  const char *getName() const {
    if (!inRange_) throw XMLException("Attribute index out of range");
    return *(atts_ + inx_);
  }
...
class XMLElementMapper {
 public:
   ...
   */
  virtual XMLElementPtr operator()(void *parser, char *elementName, AttributeList atts) = 0;
};  // XMLElementMapper
...
class XMLDocument {
 public:
   ...
   */
  static void startElementHandler(void *xmlParser, const char *elemName, const char **atts);
   ...
   */
  virtual ~XMLDocument() { XML_ParserFree(parser); }
...
class XMLElement : public NAIntrusiveSharedPtrObject {
 ...
 public:
   ...
   *         memory management.
   */
  virtual const char *className() const { return getElementName(); }
 ...
 protected:
  ...
  XMLElement(XMLElementPtr parent = NULL, ADD_MEMCHECK_ARGS_DECL(NAMemory *heap = 0))
      : NAIntrusiveSharedPtrObject(ADD_MEMCHECK_ARGS_PASS(heap)), parent_(parent) {}
  ...
  virtual void charData(void *parser, const char *data, Int32 len) {
    ...
    if (strspn(data, WHITESPACE) < (size_t)len)
      throw XMLException("Character content not allowed for element %s", getElementName());
  }
   ...
   */
  XMLElementPtr getParent() const { return parent_; }
   ...
   *                   object.
   */
  void setParent(XMLElementPtr parent) { parent_ = parent; }
...
class XMLBitmap : public NABitVector {
 public:
   ...
   */
  XMLBitmap(CollHeap *heap) : NABitVector(heap) { clear(); }
   ...
   * @param *heap Heap from to allocate internal data members.
   */
  XMLBitmap(size_t initSize, CollHeap *heap) : NABitVector(heap) {
    clear();
  ...
  virtual NABoolean isEmpty() const { return NABitVector::isEmpty() || getWordSize() == 0; }
