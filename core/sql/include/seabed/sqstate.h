...
//
SB_Export bool sqstateic_format_transid(SB_Transid_Type *transid_in, char *transid_out, int transid_out_len);

//
// ICs can call this to transid from ctrl.
// Returns non-NULL if there's a transid.
//
SB_Export SB_Transid_Type *sqstateic_get_transid(void *ctrl, int ctrl_len);
...
#define SQSTATE_IC_VAR_TABLE_BEGIN(v) \
  static Sqstate_Ic_Var v[] = {{"begin-" #v, NULL, NULL, sizeof(Sqstate_Ic_Var), SQSTATE_IC_VAR_FLAGS_NONE},
...
// IC format types
//
enum { SQSTATE_IC_FMT_TYPE_HEX = 0x1, SQSTATE_IC_FMT_TYPE_DEC = 0x2 };

//
// IC struct formatter signature
//
typedef int (*Sqstate_Ic_Struct_Fmt)(const char *data_name, void *data, const char *struct_name, void *struct_desc,
                                     int fmt_type, char *rsp);
//
// ICs can call this to add structs.
//
SB_Export int sqstateic_struct_add(const char *struct_name, Sqstate_Ic_Struct_Fmt formatter, void *struct_desc);

//
// ICs can call this to format a struct.
//
SB_Export int sqstateic_struct_format(const char *struct_name, const char *data_name, void *data, int fmt_type,
                                      char *rsp);

//
// ICs can call this to format a struct field.
//
SB_Export int sqstateic_struct_format_field(void *data, long data_size, long data_off, int fmt_type, char *rsp);
...
//
SB_Export int sqstateic_var_dyn_add(const char *var_str, void *var_addr, long var_size, bool var_ro);
...
//       return len;
//
SB_Export int sqstateic_set_ic_args_options(int ic_argc, char *ic_argv[], char *rsp, int rsp_len,
                                            Sqstate_Ic_Arg_Opt_Type *options);
...
//
SB_Export void sqstatepi_set_ctrl(char *ctrl, Sqstate_Pi_Info_Type *info, const char *lib, const char *module,
                                  const char *call);
