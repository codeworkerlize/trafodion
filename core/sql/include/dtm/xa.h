 ...
 */
#define TMNOFLAGS                              \
  0x00000000LL /* no resource manager features \
selected */
#define TMREGISTER                             \
  0x00000001LL /* resource manager dynamically \
registers */
#define TMUSEASYNC                          \
  0x00000004LL /* resource manager supports \
...
#define TMONEPHASE                                 \
  0x40000000LL /* caller is using one-phase commit \
optimisation */
#define TMFAIL                                                \
  0x20000000LL                /* dissociates caller and marks \
              transaction branch rollback-only */
#define TMNOWAIT 0x10000000LL /* return if blocking condition exists */
#define TMRESUME                                 \
  0x08000000LL /* caller is resuming association \
with suspended transaction branch */
#define TMSUCCESS                                    \
  0x04000000LL /* dissociate caller from transaction \
branch*/
#define TMSUSPEND                                                       \
  0x02000000LL                     /* caller is suspending, not ending, \
                association */
...
#define TMJOIN                                                           \
  0x00200000LL                 /* caller is joining existing transaction \
               branch */
...
#define XA_RBOTHER                                  \
  XA_RBBASE + 4 /* the resource manager rolled back \
...
resource manager */
#define XA_RBTIMEOUT                                                  \
  XA_RBBASE + 6                      /* a transaction branch took too \
             long */
...
#define XAER_WALDOWN -10 /* TLOG write error forces abort */
