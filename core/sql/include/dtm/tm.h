...
#define TM_TT_FORCE_CONSISTENCY \
  0x100  // Overrides NO_UNDO!
...
// Trafodion start //
#define TM_MAX_REGIONSERVER_STRING 3072
#define TM_MAX_DDLREQUEST_STRING   2048
#define TM_MAX_TABLE_STRING        2048
#define TM_MAX_ERROR_STRING        2048
#define TM_MAX_QUERYCONTEXT_STRING 256
extern "C" short REGISTERREGION(long transid, long startid, int port, char *hostname, int hostname_length,
                                long startcode, char *regionInfo, int regionInfo_Length, int peerId, int pv_tmFlags);
extern "C" short PUSHEPOCH(char *pa_tbldesc, int pv_tbldesc_length, char *pv_tblname, long transid, char *&pv_err_str,
                           int &pv_err_len);
extern "C" short CREATETABLE(char *pa_tbldesc, int pv_tbldesc_length, char *pv_tblname, char **pv_keys,
                             int pv_numsplits, int pv_keylen, long transid, int pv_options, char *&pv_err_str,
                             int &pv_err_len);
extern "C" short REGTRUNCATEONABORT(char *pv_tblname, int pv_tblname_len, long pv_transid, char *&pv_err_str,
                                    int &pv_err_len);
extern "C" short DROPTABLE(char *pv_tblname, int pv_tblname_len, long transid, char *&pv_err_str, int &pv_err_len);
extern "C" short ALTERTABLE(char *pv_tblname, int pv_tblname_len, char **pv_tbloptions, int pv_tbloptslen,
                            int pv_tbloptscnt, long pv_transid, char *&pv_err_str, int &pv_err_len);
...
extern "C" short ENDTRANSACTION_QUERY(char *querycontext);
...
extern "C" short ENDTRANSACTION_ERR_QUERY(char *&errStr, int &errlen, char *querycontext);
extern "C" void DEALLOCATE_ERR(char *&errStr);

extern "C" short SETSAVEPOINTIDWITHFLAG(int64 svptId, int64 pSvptId);
extern "C" short COMMITSAVEPOINT(char *&errStr, int &errlen, int64 svptId, int64 pSvptId = 0);
extern "C" short ROLLBACKSAVEPOINT(int64 svptId, int64 pSvptId);
extern "C" short GETSAVEPOINTIDWITHFLAG(int64 *svptId, int64 *pSvptId);
...
extern "C" bool GETISPROPAGATETX();
...
extern "C" short DTM_GETTRANSINFO(int64 pv_transid, int32 *pp_seq_num, int32 *pp_node, int32 *pp_clusterid,
                                  int16 *pp_incarnation_num, int16 *pp_tx_flags, TM_TT_Flags *pp_tt_flags,
                                  int16 *pp_version, int16 *pp_checksum, int64 *pp_timestamp);
extern "C" short DTM_GETTRANSINFO_EXT(TM_Transid_Type pv_transid, int32 *pp_seq_num, int32 *pp_node,
                                      int32 *pp_clusterid, int16 *pp_incarnation_num, int16 *pp_tx_flags,
                                      TM_TT_Flags *pp_tt_flags, int16 *pp_version, int16 *pp_checksum,
                                      int64 *pp_timestamp);
...
extern "C" short DTM_SDNTRANSACTIONS();
