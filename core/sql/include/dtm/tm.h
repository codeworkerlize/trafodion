...
extern "C" short REGISTERREGION(long transid, long startid, int port, char *hostname, int hostname_length, long startcode, char *regionInfo, int regionInfo_Length, int peerId, int pv_tmFlags);
extern "C" short PUSHEPOCH(char *pa_tbldesc, int pv_tbldesc_length, char *pv_tblname, long transid, char *&pv_err_str, int &pv_err_len);
