...
#define FOREACH_BLOCK(PCBlk, firstPCInst, lastPCInst, indx) FOREACH_BLOCK_ALL(PCBlk, firstPCInst, lastPCInst, indx)
...
#define FOREACH_BLOCK_FAST(PCBlk, firstPCInst, lastPCInst, indx)           \
  {                                                                        \
    ...
    for (indx = 0; indx < allBlocks_->entries(); indx++) {                 \
      ...
      firstPCInst = PCBlk->getFirstInst();                                 \
      lastPCInst = PCBlk->getLastInst();                                   \
      if (PCBlk->getPreds().isEmpty() && (PCBlk != entryBlock_)) continue; \
      if (firstPCInst == NULL) {                                           \
        assert(lastPCInst == NULL);                                        \
        /*continue;*/                                                      \
      }
...
#define RESTART_INST_IN_BLOCK _next = inst;
...
#define RESTART_INST_IN_BLOCK_AT _next = inst;
...
#define RESTART_INST_IN_BLOCK_BACKWARDS _prev = inst;
...
#define RESTART_INST_IN_BLOCK_BACKWARDS_AT _prev = inst;
...
class PCodePredicateGroup {
 ...
 public:
  PCodePredicateGroup(CollHeap *heap, PCodeBlock *member, NABoolean isStatic, PCodeCfg *cfg);
  ...
  // them accordingly.
  NABoolean findUnits(GROUPLIST &unit, GROUPLIST &workList, NABoolean &swapPerformed, NABoolean isStatic);
...
class PCodeConstants {
 public:
  ...
  NABoolean operator==(const PCodeConstants &other) const {
    ...
    if (other.align_ != align_) return FALSE;

    if ((other.data_ == data_) || (memcmp(other.data_, data_, len_) == 0)) return TRUE;
  ...
  // newOnes, and newNeg1.
  static void mergeConstantVectors(NABitVector &newZeroes, NABitVector &newOnes, NABitVector &newNeg1,
 ...
 public:
  ...
  NABoolean operator==(const PCodeOperand &other) const {
    return ((other.stackIndex_ == stackIndex_) && (other.offset_ == offset_) &&
            (other.nullBitIndex_ == nullBitIndex_) && (other.voaOffset_ == voaOffset_));
  }
  ...
  NABoolean hasSameLength(PCodeOperand *op1) {
    ...
    if (hasKnownLen() && op1->hasKnownLen()) return (getLen() == op1->getLen());
    if (isVarchar() && op1->isVarchar()) return (getVcMaxLen() == op1->getVcMaxLen());
    return FALSE;
  }
  ...
  PCodeOperand *copy(CollHeap *heap) {
    PCodeOperand *operand = new (heap) PCodeOperand(stackIndex_, offset_, nullBitIndex_, stackIndexPos_, offsetPos_,
                                                    bvIndex_, operandType_, len_, voaOffset_);
#if 0
...
printf("ZZZ In COPY Constructor(): allocated new PCodeOperand at %p, AM=%d\n", operand, operand->getType() );
  ...
  NABoolean isUnsignedType() {
    return (operandType_ == PCIT::MBIN16U) || (operandType_ == PCIT::MBIN32U) || (operandType_ == PCIT::MDECU) ||
  ...
  NABoolean isNonNativeTemp() {
    ...
    return (isTemp() && ((getType() == PCIT::MBIGS) || (getType() == PCIT::MATTR5)));
  }

#ifdef NA_LINUX_LLVMJIT
#ifdef COMING_FROM_NATIVE_EXPR_SOURCE_FILE
  jit_value_t getJitValue(PCodeCfg *cfg, IRBldr_t *Builder, jit_type_t type, PCodeBlock *b, PCodeOperand *orig,
                          NABoolean noSaveJitVals);

  void setJitValue(PCodeCfg *cfg, IRBldr_t *, jit_value_t val, PCodeBlock *b, NABoolean noSaveJitVals);

  void clearJitValues(PCodeCfg *cfg);

  jit_type_t getJitType(PCodeCfg *cfg, PCIT::AddressingMode oprTyp = PCIT::AM_NONE);

  void storeJitValue(PCodeCfg *cfg, IRBldr_t *Builder, jit_type_t type, jit_value_t val, PCodeBlock *, jit_value_t *len,
                     NABoolean noSaveJitVals);
  ...
  jit_value_t getJitValue(PCodeCfg *cfg, jit_function_t func, jit_type_t type, PCodeBlock *b, PCodeOperand *orig,
                          NABoolean noSaveJitVals);

  void setJitValue(PCodeCfg *cfg, jit_function_t, jit_value_t val, PCodeBlock *b, NABoolean noSaveJitVals);

  void clearJitValues(PCodeCfg *cfg);

  jit_type_t getJitType();

  void storeJitValue(PCodeCfg *, jit_function_t, jit_type_t, jit_value_t val, PCodeBlock *, jit_value_t *len,
                     NABoolean noSaveJitVals);

  void storeNullJitValueAndBranch(PCodeCfg *, jit_function_t, jit_value_t, jit_label_t *, NABoolean comp,
                                  PCodeBlock *b);
...
class PCodeInst {
 ...
 public:
  ...
  // Operator equals function to compare two operand bv elements
  NABoolean operator==(const PCodeInst &other) const { return (&other == this); }
  ...
  NABoolean isIndirectBranch() { return (getOpcode() == PCIT::BRANCH_INDIRECT_MBIN32S); }
 ...
 public:
  ...
  PCodeBlock(Int32 blockNum, PCodeInst *first, CollHeap *heap, PCodeCfg *cfg)
      : blockNum_(blockNum),
        ...
        cost_(1) {
    reachingDefsTableIn_ = new (heap_) NAHashDictionary<CollIndex, PCodeInst>(&collIndexHashFunc2, 10, TRUE, heap_);

    reachingDefsTableOut_ = new (heap_) NAHashDictionary<CollIndex, PCodeInst>(&collIndexHashFunc2, 10, TRUE, heap_);
  ...
  NABoolean isOperandLiveInSuccs(PCodeOperand *op) {
    ...
    for (i = 0; i < getSuccs().entries(); i++) {
      ...
      if (succ->getLiveVector().testBit(bvIndex)) return TRUE;
    }
  ...
  NABoolean branchesTo(PCodeBlock *block) {
    if (last_ && last_->isBranch() && (getTargetBlock() == block)) return TRUE;

    if (last_ && last_->isIndirectBranch()) {
      for (CollIndex i = 0; i < getSuccs().entries(); i++)
        if (getSuccs()[i] == block) return TRUE;
    }

    return FALSE;
  }

  PCodeBlock *determineTargetBlock(NABitVector &zeroes, NABitVector &ones, NABitVector &neg1);

  PCodeInst *reduceBranch(PCodeBlock *to, NABitVector &zeroes, NABitVector &ones, NABitVector &neg1);

  void fixupConstantVectors(PCodeBlock *targetBlock, NABitVector &newZeroes, NABitVector &newOnes,
                            NABitVector &newNeg1);

  void mergeConstantVectors(NABitVector &newZeroes, NABitVector &newOnes, NABitVector &newNeg1, NABitVector &zeroes,
...
#define INVALID_LONG INVALID_INT64
  ...
  PCodeCfg(ex_expr *expr, Int32 *atpMap, Int32 *atpIndexMap, CollHeap *heap, Space *space)
      : expr_(expr),
        ...
        origHeap_(heap),
        rDefsHeap_(NULL),
        operandToIndexMap_(NULL),
        targets_(NULL),
        entryBlock_(NULL),
        maxBVIndex_(0),
        zeroOffset_(-1),
        flags_(0),
        indexToOperandMap_(NULL),
        atpMap_(atpMap),
        NExTempsList_(0),
        atpIndexMap_(atpIndexMap),
        nullToPadMap_(NULL),
        newConstsAreaLen_(0),
        constToOffsetMap_(NULL),
        offsetToConstMap_(NULL) {
    ...
    if (NExDbgInfoPtr_ && (NExDbgInfoPtr_ > (NExDbgInfo *)((long)(expr->getConstsLength()))))
  ...
  PCodeCfg(ex_expr *expr, CollHeap *heap)
      : expr_(expr),
        space_(NULL),
        origHeap_(heap),
        rDefsHeap_(NULL),
  ...
  // Setup Routines
  NABoolean canPCodeBeOptimized(PCodeBinary *pCode, NABoolean &pExprCacheable /* OUT */
                                ,
  ...
  CollIndex *createLookupTableForSwitch(OPLIST &constants, BLOCKLIST &blocks, UInt32 &size, NABoolean forInList);
  ...
  PCodeBlock *shortCircuitOptForBlock(PCodeBlock *block, NABoolean assumeTaken, NABoolean overlapFound,
                                      const NABitVector &zeroesConst, const NABitVector &onesConst,
                                      const NABitVector &neg1Const);

  // Accessor Routines
  NAHashDictionary<CollIndex, PCodeOperand> *getMap() { return indexToOperandMap_; }
  NAHashDictionary<PCodeOperand, CollIndex> *getOpToIndexMap() { return operandToIndexMap_; }
...
#ifdef NA_LINUX_LIBJIT
  void jitProcessFloatExceptionCheck(jit_function_t f, PCodeOperand *fRes, jit_label_t *errorJitLabel,
                                     PCodeBlock *block);
  ...
  void IR_insn_branch_if_not_zero(IRBldr_t *Bldr, jit_value_t value, p_IR_block_t *tgt_IR_block);
