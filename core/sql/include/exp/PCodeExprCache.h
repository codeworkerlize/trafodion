...
class PCECacheEntry {  // Doubly-linked PCode Expr Cache Entry
 public:
  ...
  UInt32 getTotalMemorySize() const {
    return ((pcLenU_ + pcLenO_) * sizeof(PCodeBinary) + constsLenN_ + sizeof(PCECacheEntry));
  }
  ...
  // constructor
  PCECacheEntry(NAHeap *heap, PCodeBinary *unOptimizedPCode, PCodeBinary *optimizedPCode, char *constsPtr,
                UInt32 unOptPClen, UInt32 optPClen, UInt32 constsLenU, UInt32 constsLenO, UInt32 constsLenN,
                UInt32 tempsLenO
#if OPT_PCC_DEBUG == 1
                ,
                UInt64 optTime, UInt64 NEgenTime, UInt64 myUniqCtr
#endif /* OPT_PCC_DEBUG==1 */
                )
      : heap_(heap),
  ...
  ~PCECacheEntry() {
    ...
    if (constsLenN_ > 0) NADELETEBASIC(constsPtr_, heap_);
  };
...
class OptPCodeCache : public NABasicObject {  // Anchor for PCode Expr Cache
 public:
  ...
  PCECacheEntry *findPCodeExprInCache(PCodeBinary *unOptPCodePtr, char *unOptConstantsArea, UInt32 optFlags,
  ...
  // constructor
  OptPCodeCache()
      : heap_(new CTXTHEAP NABoundedHeap("optPCode cache heap", (NAHeap *)CTXTHEAP, 0, 0)),
  ...
  ~OptPCodeCache() {
    resizeCache(0);
#if OPT_PCC_DEBUG == 1
    if (logDirPath_) NADELETEBASIC(logDirPath_, heap_);
    logDirPath_ = NULL;
#endif /* OPT_PCC_DEBUG==1 */
  };
