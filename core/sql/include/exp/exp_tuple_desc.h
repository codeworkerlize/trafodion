...
class ExpTupleDesc : public NAVersionedObject {
 public:
  ...
  enum TupleDataFormat {
    UNINITIALIZED_FORMAT = COM_UNINITIALIZED_FORMAT,
    OBSOLETE_SIMULATOR_FORMAT = COM_OBSOLETE_SIMULATOR_FORMAT,
    PACKED_FORMAT = COM_PACKED_FORMAT,
    SQLMX_KEY_FORMAT = COM_SQLMX_KEY_FORMAT,
    SQLARK_EXPLODED_FORMAT = COM_SQLARK_EXPLODED_FORMAT,
    SQLMX_FORMAT = COM_SQLMX_FORMAT,
    SQLMX_ALIGNED_FORMAT = COM_SQLMX_ALIGNED_FORMAT
  };
  ...
  //
  enum TupleDescFormat { SHORT_FORMAT, LONG_FORMAT };
 ...
 private:
  ...
  Int16 packed() { return (Int16)(flags_ & PACKED); }

 public:
  ExpTupleDesc(UInt32 num_attrs, Attributes **attrs, UInt32 tupleDataLength, TupleDataFormat tdataF,
               TupleDescFormat tdescF, Space *space);

  ExpTupleDesc(UInt32 num_attrs, AttributesPtr *attrs, UInt32 tupleDataLength, TupleDataFormat tdataF,
  ...
  ////////////////////////////////////////////////////////
  static Int16 computeOffsets(UInt32 num_attr, Attributes **attrs, TupleDataFormat tf, UInt32 &datalen,
                              UInt32 startOffset = 0, UInt32 *rtnFlags = NULL, TupleDataFormat *outTf = NULL,
                              ExpHdrInfo *hdrInfo = NULL, UInt32 *headerSizePtr = NULL, UInt32 *VarSizePtr = NULL,
                              Attributes **offsets = NULL);
  ...
  // offsets computation to start there.
  ////////////////////////////////////////////////////////
  static Int16 computeOffsets(Attributes *attr, TupleDataFormat tf, UInt32 &datalen, UInt32 startOffset = 0);

  ////////////////////////////////////////////////////////
  // Class method that centralizes SQLARK_EXPLODED_FORMAT tuple
  // format offset calculations.  Returns aligned column offset.
  ////////////////////////////////////////////////////////
  static UInt32 sqlarkExplodedOffsets(UInt32 offset, UInt32 length, Int16 dataType, NABoolean isNullable,
                                      UInt32 *nullIndicatorOffset = NULL, UInt32 *vcIndicatorOffset = NULL,
  ...
  void setAddedField() { flags_ |= ADDED_COLUMN; }

  Int16 addedFieldPresent() { return (Int16)(flags_ & ADDED_COLUMN); }

  void setAddedFixed() { flags_ |= ADDED_FIXED_PRESENT; }

  Int16 addedFixedPresent() { return (Int16)(flags_ & ADDED_FIXED_PRESENT); }

  NABoolean variableFieldPresent() { return ((flags_ & (UInt32)VAR_FIELD_PRESENT) != 0); }

  NABoolean tandemFloatPresent() { return ((flags_ & (UInt32)TDM_FLOAT_PRESENT) != 0); }

  // Check if key fields may be shifted due to an added column or not.
  NABoolean isKeyShifted() { return ((flags_ & KEY_SHIFT) > 0); }
  ...
  NABoolean isSQLMXTable() { return tupleDataFormat_ == (Int16)ExpTupleDesc::SQLMX_FORMAT; }

  NABoolean isSQLMXAlignedTable() { return tupleDataFormat_ == (Int16)ExpTupleDesc::SQLMX_ALIGNED_FORMAT; }

  NABoolean isSQLPackedTable() { return tupleDataFormat_ == (Int16)ExpTupleDesc::PACKED_FORMAT; }
...

  {
    if (tdf == ExpTupleDesc::SQLMX_ALIGNED_FORMAT) {
      return ExpAlignedFormat::computeFirstFixedOffset(ffAlignSize, startOffset, voaIdxOffset, nullableFieldCount,
                                                       hdrInfo, headerSize, bitmapOffset);
    } else {
...
#ifdef NA_BIG_ENDIAN
    return (dataPtr[0] << 24) | (dataPtr[1] << 16) | (dataPtr[2] << 8) | (dataPtr[3]);
#else
  ...
  static void write4(char *dataPtr, UInt32 value) {
#ifdef NA_BIG_ENDIAN
  ...
  static void write2(char *dataPtr, unsigned short value) {
#ifdef NA_BIG_ENDIAN
  ...
  UInt32 getActualDataLength(char *dataPtr, UInt32 dataLen) {
    if (isSQLMXAlignedTable())
  ...
  static UInt32 getVoaSize(TupleDataFormat tdf) {
    return ((tdf == SQLMX_ALIGNED_FORMAT) ? ExpAlignedFormat::OFFSET_SIZE : ExpVoaSize);
  }

  UInt32 getVoaSize() { return getVoaSize((TupleDataFormat)tupleDataFormat_); }

  UInt32 adjustDataLength(char *dataPtr, UInt32 currLength) {
    if (isSQLMXAlignedTable())
      return ExpAlignedFormat::adjustDataLength(dataPtr, currLength, ExpAlignedFormat::ALIGNMENT);
    else
  ...
  static UInt32 getFirstFixedOffset(char *dataPtr, TupleDataFormat tdf) {
    if (tdf == ExpTupleDesc::SQLMX_ALIGNED_FORMAT)
  ...
  static void setFirstFixedOffset(char *dataPtr, UInt32 firstFixedOffset, TupleDataFormat tdf) {
    if (tdf == ExpTupleDesc::SQLMX_ALIGNED_FORMAT)
  ...
  // Determine which disk format we are working with and return the first
  // field offset based on the disk format.
  UInt32 getFirstVariableOffset(char *dataPtr, UInt32 firstFixedOffset) {
    UInt32 offset = 0;
    if (variableFieldPresent()) {
      if (isSQLMXAlignedTable()) {
        offset = ExpAlignedFormat::getFirstVariableOffset(dataPtr, firstFixedOffset);
      } else {
  ...
  static char getNullChar() { return (char)'\377'; }

  static char getZeroChar() { return (char)'\0'; }
  ...
  static void setNullValue(char *dataPtr, Int16 nullBitIndex, TupleDataFormat tdf) {
    if (tdf == ExpTupleDesc::SQLMX_ALIGNED_FORMAT) {
  ...
  static void setNullValue(char *dataPtr, Int16 nullBitIndex) {
    if (nullBitIndex >= 0) {
      ExpAlignedFormat::setNullBit(dataPtr, nullBitIndex);
    } else {
      dataPtr[0] = '\377';
  ...
  // Determine the data format we are working with and set the appropriate
  // null indicator correctly.
  static void clearNullValue(char *dataPtr, Int16 nullBitIndex, TupleDataFormat tdf) {
    if (tdf == ExpTupleDesc::SQLMX_ALIGNED_FORMAT) {
  ...
  // Determine the data format we are working with and set the appropriate
  // null indicator correctly.
  static void clearNullValue(char *dataPtr, Int16 nullBitIndex) {
    if (nullBitIndex >= 0) {
      ExpAlignedFormat::clearNullBit(dataPtr, nullBitIndex);
    } else {
      dataPtr[0] = '\0';
  ...
  static NABoolean isNullValue(char *nullDataPtr, Int16 nullBitIdx, TupleDataFormat tdf) {
    if (tdf == ExpTupleDesc::SQLMX_ALIGNED_FORMAT) {
  ...
  static NABoolean isNullValue(char *nullDataPtr, Int16 nullBitIdx) {
    if (nullBitIdx >= 0) {
      return (ExpAlignedFormat::isNullValue(nullDataPtr, nullBitIdx));
    } else {
  ...
  static NABoolean isValidVariableOffset(UInt32 offset) { return (offset != ExpOffsetMax); }

  static void setVoaValue(char *dataPtr, UInt32 voaEntryOffset, UInt32 voaEntryValue, TupleDataFormat tdf) {
  ...
  static void setVoaValue(char *dataPtr, UInt32 voaEntryOffset, UInt32 voaEntryValue, Int16 vcIndLen) {
    if (voaEntryOffset != ExpOffsetMax) {
  ...
  void setVoaValue(char *dataPtr, UInt32 voaEntryOffset, UInt32 voaEntryValue) {
    ExpTupleDesc::setVoaValue(dataPtr, voaEntryOffset, voaEntryValue, getTupleDataFormat());
  }

  // Get the offset value out of the VOA entry based on tuple data format.
  static UInt32 getVoaOffset(char *dataPtr, UInt32 voaEntryOffset, TupleDataFormat tdf) {
    UInt32 offset = 0;
    if (voaEntryOffset != ExpOffsetMax) {
  ...
  UInt32 getVoaOffset(char *dataPtr, UInt32 voaEntryOffset) {
    return ExpTupleDesc::getVoaOffset(dataPtr, voaEntryOffset, getTupleDataFormat());
  }
  ...
  static UInt32 getVarOffset(char *dataPtr, UInt32 offset, UInt32 voaEntryOffset, UInt32 vcIndLen, UInt32 nullIndLen) {
    UInt32 varLenIndOffset;

    if (offset == ExpOffsetMax) {
      // indirect varchar
      varLenIndOffset = getVoaOffset(dataPtr, voaEntryOffset, vcIndLen) + nullIndLen;
    } else {
  ...
  static NABoolean isDiskFormat(TupleDataFormat tdf) {
    return (((tdf == SQLMX_ALIGNED_FORMAT) || (tdf == SQLMX_FORMAT)) ? TRUE : FALSE);
  }

  static NABoolean isAlignedFormat(TupleDataFormat tdf) { return (SQLMX_ALIGNED_FORMAT == tdf); }

  static NABoolean isHeaderClauseNeeded(TupleDataFormat tdf) { return (tdf == SQLMX_ALIGNED_FORMAT); }
