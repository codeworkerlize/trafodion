...
class LikePatternString : public NABasicObject {
 public:
  ...
  // Constructor.
  //
  LikePatternString(const char *pattern, UInt16 patternLen, CharInfo::CharSet patternCS = CharInfo::ISO88591,
                    const char *escapeChar = NULL, UInt16 escapeChar_len = 0, const char *underscoreChar = "_",
                    UInt16 underscoreChar_len = 1, const char *percentChar = "%", UInt16 percentChar_len = 1)
      : pattern_(pattern),
...
class LikePatternStringIterator {
 public:
  ...
  // pattern.
  //
  LikePatternStringIterator(const LikePatternString &pattern) : pattern_(pattern), currentChar_(pattern.pattern_) {
    determineCharType();
  }
  ...
  //
  enum CharType { END_OF_PATTERN, NON_WILDCARD, UNDERSCORE, PERCENT, CharType_ERROR };
  void determineCharType() {
    if (currentChar_ >= pattern_.endOfPattern_)
      charType_ = END_OF_PATTERN;
    else if ((pattern_.escapeChar_ != NULL) AND thisCharIsEqualTo(pattern_.escapeChar_, pattern_.escapeChar_len_)) {
      ...
      } else if (thisCharIsEqualTo(pattern_.escapeChar_, pattern_.escapeChar_len_)
                     OR thisCharIsEqualTo(pattern_.underscoreChar_, pattern_.underscoreChar_len_)
                         OR thisCharIsEqualTo(pattern_.percentChar_, pattern_.percentChar_len_))
  ...
  const char *getCurrentChar() { return currentChar_; }
  ...
  Int16 getChar(char *ch) const {
    ...
    switch (pattern_.patternCS_) {
      ...
      default:  // For all multi-byte character sets
        ...
        UInt32 UCS4val = 0;
        chrLen = LocaleCharToUCS4(currentChar_, pattern_.bytesPerChar_, &UCS4val, cnv_cs);
        if (chrLen <= 0) chrLen = 1;  // Prevent going wild.
        break;
    }
    for (UInt16 i = 0; i < chrLen; i++) ch[i] = currentChar_[i];
    return (chrLen);
  }
  ...
  NABoolean thisCharIsEqualTo(const char *ch, const UInt16 ch_len) const {
    if (currentChar_ + ch_len > pattern_.endOfPattern_) return FALSE;
    for (UInt16 i = 0; i < ch_len; i++)
      if (currentChar_[i] != ch[i]) return FALSE;
    return TRUE;
  }
  //
...
class LikePatternClause : public NABasicObject {
 public:
  ...
  // Constructor.
  //
  LikePatternClause(LikePatternStringIterator::CharType charType, char *patternBuf)
      : charType_(charType),
        pattern_(patternBuf),
        length_(0),
  ...
  //
  void setType(LikePatternStringIterator::CharType charType) { charType_ = charType; }
  //
  // Append the given character to the clause.
  //
  void append(const LikePatternStringIterator &i) { length_ += i.getChar(getPattern() + getLength()); }
  //
  ...
  CharInfo::Collation getCollation() const { return co_; }

  void setCollation(CharInfo::Collation collation) { co_ = collation; }

  unsigned char *getEncodedPattern() { return encodedPattern_; }

  void setEncodedPattern(unsigned char *encodedPattern) { encodedPattern_ = encodedPattern; }
...
class LikePatternHeader : public LikePatternClause {
 public:
  ...
  // Constructor.
  //
  LikePatternHeader(LikePatternStringIterator::CharType charType, char *patternBuf, CollHeap *exHeap)
  ...
  ~LikePatternHeader() {
    ...
    if (this->getEncodedHeader()) NADELETEBASIC(this->getEncodedHeader(), getExHeap());
    //
  ...
  UInt16 getLength() const { return length_; }
  UInt16 getClauseLength() const { return ((LikePatternClause *)this)->getLength(); }
  ...
  void addNewClause(LikePatternStringIterator::CharType charType) {
    ...
    getLastClause()->append(new (exHeap_) LikePatternClause(charType, getPattern() + getLength()));
  ...
  // length.
  //
  void endClauses() { length_ += getLastClause()->getLength(); }
  //
  ...
  void setError(ExeErrorCode exeErrorCode) { error_ = exeErrorCode; }

  unsigned char *getEncodedHeader() { return encodedHeader_; }

  void setEncodedHeader(unsigned char *encodedHeader) { encodedHeader_ = encodedHeader; }
...
class LikePattern : public LikePatternHeader {
 public:
  ...
  // Constructor.
  //
  LikePattern(const LikePatternString &pattern, CollHeap *exHeap, CharInfo::CharSet cs = CharInfo::ISO88591,
  ...
  //
  NABoolean matches(const char *text, UInt16 textLen, CharInfo::CharSet cs = CharInfo::ISO88591);
