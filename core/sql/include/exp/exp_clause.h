...
class ex_clause : public NAVersionedObject {
 public:
  ...
  enum clause_type {
    ...
    FUNC_ISIP_ID = 122,
    FUNC_INETATON_ID = 123,
    FUNC_INETNTOA_ID = 124,
    AGGR_GROUPING_ID = 125,
    FUNC_CRC32_ID = 126,
    FUNC_MD5_ID = 127,
    FUNC_SHA1_ID = 128,
    FUNC_SHA2_ID = 129,
    ...
    FUNC_REVERSE_ID = 135,
    FUNC_ANSI_TENANT_ID = 136,
    FUNC_MAXBYTES_ID = 137,
    FUNC_RANGE_VALUES_MERGE_ID = 138,
    FUNC_RANGE_VALUES_INSERT_ID = 139,
    FUNC_RANGE_VALUES_IN_ID = 140,
    FUNC_RANGE_VALUES_COPY_ID = 141,
    FUNC_RANGE_VALUES_UNPACK_ID = 142,
    REGEXP_REPLACE = 143,
    FUNC_FIRSTNGRAM = 144,  // for ngram
    FUNC_NGRAMCOUNT = 145,

    FUNC_BLOOM_FILTER_MERGE_ID = 146,
    FUNC_BLOOM_FILTER_INSERT_ID = 147,
    FUNC_BLOOM_FILTER_IN_ID = 148,
    FUNC_BLOOM_FILTER_COPY_ID = 149,
    FUNC_BLOOM_FILTER_UNPACK_ID = 150,
    FUNC_SLEEP_ID = 151,
    FUNC_UNIX_TIMESTAMP_ID = 152,

    FUNC_SPLIT_PART_ID = 156,

    // COMPOSITE (ARRAY,ROW) expression
    FUNC_COMPOSITE_CONCAT = 157,
    FUNC_COMPOSITE_CREATE = 158,
    FUNC_COMPOSITE_DISPLAY = 159,
    FUNC_COMPOSITE_EXTRACT = 160,
    FUNC_COMPOSITE_ARRAY_CAST = 161,
    FUNC_COMPOSITE_ARRAY_LENGTH = 162,
    FUNC_COMPOSITE_HIVE_CAST = 163,
    FUNC_BASE64_ENC_DEC = 164,
    FUNC_HBASE_ROWID = 165,

    // for to_char
    FUNC_NUMBERFORMAT_ID = 166,
  ...
  //
  ex_clause() : NAVersionedObject(CLAUSE_ANCHOR), nextClause_(NULL), op_(NULL){};
  ex_clause(clause_type type_, OperatorTypeEnum operType_, short num_operands_, Attributes **op_, Space *space_);
  ~ex_clause();
  ...
  inline OperatorTypeEnum getOperType() { return (OperatorTypeEnum)operType_; };
  ...
  inline void setPCIList(PCILink *pciLink) { pciLink_ = pciLink; };
  ...
  void setHandleIndirectVC(NABoolean v) { (v ? flags_ |= HANDLE_INDIRECT_VC : flags_ &= ~HANDLE_INDIRECT_VC); };
  ...
  virtual ex_expr::exp_return_type fixup(Space *space = 0, CollHeap *exHeap = 0, char *constants_area = 0,
                                         char *temps_area = 0, char *persistentArea = 0, short = 0,
  ...
  static void setRowLength(Attributes *attr, atp_struct *atp, UInt32 *rowLen, UInt32 newLength) {
    if (attr->isSQLMXAlignedFormat()) {
      *rowLen = ExpAlignedFormat::adjustDataLength(atp->getTupp(attr->getAtpIndex()).getDataPointer(), newLength,
  ...
  static void setVoaData(char *dataPtr, UInt32 voaOffset, UInt32 value) {
    assert(dataPtr);
    if (voaOffset != ExpOffsetMax) str_cpy_all(dataPtr + voaOffset, (char *)&value, ExpVoaSize);
  }

  static void evalSetRowLength(Attributes *attr, atp_struct *atp, UInt32 *rowLen, UInt32 newLength) {
    ...
    if (attr && attr->isSQLMXAlignedFormat() && (*rowLen != newLength)) {
      *rowLen = ExpAlignedFormat::adjustDataLength((char *)atp->getTupp(attr->getAtpIndex()).getDataPointer(),
                                                   newLength, ExpAlignedFormat::ALIGNMENT);
    } else
      *rowLen = newLength;
  }
  ...
  void setExeGlobals(ex_globals *glob) { globals_ = glob; }

  void setTcb(const ex_tcb *tcb) { tcb_ = (ex_tcb *)tcb; }

  Int16 getInstrArrayIndex() { return instrArrayIndex_; }
  void setInstrArrayIndex(Int16 index) { instrArrayIndex_ = index; }

  virtual NABoolean evalNeedAtps() { return FALSE; }

 protected:
  ex_globals *getExeGlobals() { return globals_; }
...
inline ex_clause::clause_type ex_clause::getType() { return (clause_type)clauseType_; };

inline void ex_clause::setType(ex_clause::clause_type t) { clauseType_ = t; };

inline void ex_clause::setLastClause() { nextClause_ = 0; };

inline short ex_clause::getNumOperands() { return numOperands_; };

inline Attributes *ex_clause::getOperand(short operand_num) { return op_[operand_num]; };

inline AttributesPtr *ex_clause::getOperand() { return op_; };

inline void ex_clause::setOperand(AttributesPtrPtr &attr) { op_ = attr; };

inline void ex_clause::setNextClause(ex_clause *clause_) { nextClause_ = clause_; };
