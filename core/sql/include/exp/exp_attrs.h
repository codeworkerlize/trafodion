...
public:
  ...
  enum DefaultClass
  {
    ...
    DEFAULT_CATALOG_FUNCTION, DEFAULT_SCHEMA_FUNCTION, DEFAULT_SYS_GUID_FUNCTION,
    INVALID_DEFAULT
  };
  ...
  {
    if (((DFS2REC::isBinaryNumeric(getDatatype())) ||
         (DFS2REC::isBigNum(getDatatype()))) &&
        (getPrecision() > 0))
      return TRUE;
    else
      return FALSE;
  ...
  void copyLocationAttrs(Attributes * attr)
  {
    ...
    if (attr->isAddedCol() && !isHbaseMapTableCol())
      setAddedCol();
