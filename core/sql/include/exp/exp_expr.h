...
public:
  ...
  inline exp_return_type 
       eval(atp_struct*atp1,
	    ...
	    char *fetchedDataPtr
            ...
            ,Int32 clause2display = -1
            ,Int32 operand2display = -1
#endif
          )
  {
    ...
    if (evalPtr_)
      ...
      return evalClauses(getClauses(), atp1, atp2, atp3, datalen, rowLen, 
  ...
  {
    return eval(atp1, atp2, NULL, exHeap, datalen, rowLen, lastFldIndex, 
                fetchedDataPtr);
  }
...
public:
  ...
  NABoolean isMXCI(UInt32 flags) 
  { return (flags & IS_MXCI_) != 0; };

  void setIsMXCI(UInt32 &flags, NABoolean v)      
  { (v ? flags |= IS_MXCI_ : flags &= ~IS_MXCI_); }
  ...
  static NABoolean areCompatible(short source, short target)
  {
	...
	((source == REC_CLOB) && (target == REC_CLOB)) ||
        ...
        ((source == REC_ARRAY) && (target == REC_ARRAY)) ||
