...
/////////////////////////////////////////////////////////////////
#define NEG_BIT_MASK \
  0x0080  // the first bit in a byte - used for finding
          // out whether the negative bit in a number
          // is flipped

#define ExpAssert(p, msg)              \
  if (!(p)) {                          \
...
class ex_expr : public NAVersionedObject {
 ...
 public:
  ...
  //
  enum exp_return_type { EXPR_OK, EXPR_ERROR, EXPR_TRUE, EXPR_FALSE, EXPR_NULL, EXPR_PREEMPT };

  typedef exp_return_type (*evalPtrType)(PCodeBinary *, atp_struct *, atp_struct *, ex_expr *);

  // Read/Write mode for SQLMX disk format
  enum exp_mode { exp_UNKNOWN, exp_READ, exp_WRITE };
  ...
  exp_node_type getType() { return (exp_node_type)getClassID(); };
  ...
  ex_expr_lean *castToExExprLean() { return (ex_expr_lean *)this; }

  ex_expr *castToExExpr() { return this; }
  ...
  //
  ex_clause *getClauses() { return clauses_.getPointer(); };

  void setClauses(ex_clause *clause) { clauses_ = clause; };
  ...
  char *getMyConstantsArea() { return constantsArea_; }

  void setConstantsArea(char *constants_area) { constantsArea_ = constants_area; };

  inline char *getTempsArea() { return tempsArea_; }
  ...
  inline exp_return_type eval(atp_struct *atp1, atp_struct *atp2, atp_struct *atp3, CollHeap *exHeap, Lng32 datalen,
    ...
    if (evalPtr_)
      ...
      return evalPCode(pCode, atp1, atp2, atp3, datalen, rowLen);
    else
      return evalClauses(getClauses(), atp1, atp2, atp3, datalen, rowLen, lastFldIndex, fetchedDataPtr
#ifdef TRACE_EXPR_EVAL
                         ,
                         clause2display, operand2display
#endif
      );
  }
  ...
  // Moves
  static exp_return_type evalFast4(PCodeBinary *, atp_struct *, atp_struct *, ex_expr *);

  static exp_return_type evalFast202(PCodeBinary *, atp_struct *, atp_struct *, ex_expr *);

  static exp_return_type evalFast203(PCodeBinary *, atp_struct *, atp_struct *, ex_expr *);

  // Compares
  static exp_return_type evalFast33(PCodeBinary *, atp_struct *, atp_struct *, ex_expr *);

  static exp_return_type evalFast36(PCodeBinary *, atp_struct *, atp_struct *, ex_expr *);

  static exp_return_type evalFast37(PCodeBinary *, atp_struct *, atp_struct *, ex_expr *);

  static exp_return_type evalFast40(PCodeBinary *, atp_struct *, atp_struct *, ex_expr *);

  static exp_return_type evalFast117(PCodeBinary *, atp_struct *, atp_struct *, ex_expr *);

  static exp_return_type evalFast130(PCodeBinary *, atp_struct *, atp_struct *, ex_expr *);

  static exp_return_type evalFast131(PCodeBinary *, atp_struct *, atp_struct *, ex_expr *);

  static exp_return_type evalFast132(PCodeBinary *, atp_struct *, atp_struct *, ex_expr *);

  static exp_return_type evalFast133(PCodeBinary *, atp_struct *, atp_struct *, ex_expr *);

  static exp_return_type evalFast162(PCodeBinary *, atp_struct *, atp_struct *, ex_expr *);

  static exp_return_type evalFast262(PCodeBinary *, atp_struct *, atp_struct *, ex_expr *);

  static exp_return_type evalFast275(PCodeBinary *, atp_struct *, atp_struct *, ex_expr *);

  // Hash
  static exp_return_type evalFast94(PCodeBinary *, atp_struct *, atp_struct *, ex_expr *);

  static exp_return_type evalFast170(PCodeBinary *, atp_struct *, atp_struct *, ex_expr *);

  static exp_return_type evalFast171(PCodeBinary *, atp_struct *, atp_struct *, ex_expr *);

  static exp_return_type evalFast173(PCodeBinary *, atp_struct *, atp_struct *, ex_expr *);

  static exp_return_type evalFast223(PCodeBinary *, atp_struct *, atp_struct *, ex_expr *);
  ...
  //
  PCodeBinary *getPCodeBinary() { return (pCode_.getPointer() ? pCode_.getPointer()->getPCodeBinary() : NULL); };

  void setPCodeLean(PCodeBinary *pCode) { pCodeLean_ = pCode; }

  PCode *getPCodeObject() { return pCodeObject_; }
  ...
  UInt32 getPCodeOptFlags() { return pCodeOptFlags_; }
  void setPCodeOptFlags(UInt32 flags) { pCodeOptFlags_ = flags; }
  UInt32 getPCodeMaxOptBrCnt() { return pCodeMaxOptBrCnt_; }
  UInt32 getPCodeMaxOptInCnt() { return pCodeMaxOptInCnt_; }
  void setPCodeMaxOptBrCnt(UInt32 cnt) { pCodeMaxOptBrCnt_ = cnt; }
  void setPCodeMaxOptInCnt(UInt32 cnt) { pCodeMaxOptInCnt_ = cnt; }
  void setEvalPtr(NABoolean isSamePrimary);

  void setEvalPtr(evalPtrType e) { evalPtr_ = e; }

  void setNExDbgInfo(NExDbgInfo *NExDbgPtr) { NExDbgInfo_ = NExDbgPtr; }

  NExDbgInfo *getNExDbgInfoPtr() { return NExDbgInfo_; }

  Long getEvalPtrOffset() { return evalPtrOff_; }

  exp_return_type pCodeGenerate(ComSpace *space, CollHeap *heap, UInt32 flags);
  void pCodePrint();
  ...
  //
  inline char *getPersistentArea() { return persistentArea_; }
  ...
  NABoolean pCodeSpecialFields() { return (pCodeMode_ & PCODE_SPECIAL_FIELDS) != 0; };
  ...
  NABoolean getFixupConstsAndTemps() { return ((flags_ & FIXUP_CONSTS_AND_TEMPS) != 0); };

  NABoolean generateNoPCode() { return ((flags_ & GENERATE_NO_PCODE) != 0); }

  void setGeneratePcode(short v) {
    if (v != 0)
      flags_ |= GENERATE_NO_PCODE;
    else
      flags_ &= ~GENERATE_NO_PCODE;
  }

  NABoolean getPCodeGenCompile() { return ((flags_ & PCODE_GEN_COMPILE) != 0); }
  ...
  void setPCodeNative(NABoolean v) {
    if (v)
      flags_ |= PCODE_EVAL_NATIVE;
    else
      flags_ &= ~PCODE_EVAL_NATIVE;
  };

  NABoolean getPCodeNative() { return ((flags_ & PCODE_EVAL_NATIVE) != 0); };

  NABoolean getNEInShowplan() { return ((flags_ & PCODE_NE_IN_SHOWPLAN) != 0); };

  void setNEInShowplan(NABoolean v) { (v ? flags_ |= PCODE_NE_IN_SHOWPLAN : flags_ &= ~PCODE_NE_IN_SHOWPLAN); };

  void setPCodeMoveFastpath(NABoolean v) {
    if (v)
      flags_ |= PCODE_MOVE_FASTPATH;
    else
      flags_ &= ~PCODE_MOVE_FASTPATH;
  };
  NABoolean getPCodeMoveFastpath() { return ((flags_ & PCODE_MOVE_FASTPATH) != 0); };
  ...
  NABoolean usePCodeEvalAligned() { return ((flags_ & PCODE_EVAL_ALIGNED) != 0); };
  ...
  static void setForShowplan(UInt32 &f, NABoolean v) { (v ? f |= FOR_SHOWPLAN : f &= ~FOR_SHOWPLAN); }

  static NABoolean notValidateFloat64(UInt32 f) { return ((f & NOT_VALIDATE_FLOAT64) != 0); }
  static void setNotValidateFloat64(UInt32 &f, NABoolean v) {
    (v ? f |= NOT_VALIDATE_FLOAT64 : f &= ~NOT_VALIDATE_FLOAT64);
  }

  static int formatARow2(const char **srcFldsPtr, const int *srcFldsLength, const int *srcFieldsConvIndex,
  ...
  const ex_tcb *getMyTcb() const { return myTcb_; };
...
class AggrExpr : public ex_expr {
  enum Flags { ONE_ROW_AGGR = 0x0001 };
 ...
 public:
  ...
  AggrExpr(ex_expr *init_expr, ex_expr *perrec_expr, ex_expr *final_expr, ex_expr *final_null_expr,
  ...
  NABoolean isOneRowAggr() { return ((flags_ & ONE_ROW_AGGR) != 0); }
};
...
class InputOutputExpr : public ex_expr {
 ...
 public:
  unsigned char *sourDataParams_;
  long sourDataParamsLen_;
  ...
  InputOutputExpr()
      : ex_expr(ex_expr::exp_INPUT_OUTPUT),
        ...
        isCall_(0),
        numEntries_(-1),
        sourDataParams_(NULL),
        sourDataParamsLen_(0) {}

  InputOutputExpr(Generator *generator);
  ~InputOutputExpr() {
    if (sourDataParams_) delete sourDataParams_;
    sourDataParams_ = NULL;
  }
  ...
  ex_expr::exp_return_type inputRowwiseRowsetValues(atp_struct *, void *, void *, NABoolean tolerateNonFatalError,
                                                    CollHeap *exHeap, UInt32 flags);

  ex_expr::exp_return_type inputValues(atp_struct *atp, void *inputDesc_, NABoolean tolerateNonFatalError,
                                       CollHeap *heap, UInt32 flags, NABoolean optLargVar = FALSE,
  ...
  void setNoEval(NABoolean v) { (v ? flags_ |= NO_EVAL : flags_ &= ~NO_EVAL); }

  NABoolean skipTypeCheck() { return (flags_ & SKIP_TYPE_CHECK) != 0; };

  void setSkipTypeCheck(NABoolean v) { (v ? flags_ |= SKIP_TYPE_CHECK : flags_ &= ~SKIP_TYPE_CHECK); }

  NABoolean restrictedSkipTypeCheck() { return (flags_ & RESTRICTED_SKIP_TYPE_CHECK) != 0; };

  void setRestrictedSkipTypeCheck(NABoolean v) {
    (v ? flags_ |= RESTRICTED_SKIP_TYPE_CHECK : flags_ &= ~RESTRICTED_SKIP_TYPE_CHECK);
  }

  NABoolean suppressCharLimitCheck() { return (flags_ & SUPPRESS_CHAR_LENGTH_LIMIT_CHECKS); }

  void setSuppressCharLimitCheck(NABoolean v) {
    (v ? flags_ |= SUPPRESS_CHAR_LENGTH_LIMIT_CHECKS : flags_ &= ~SUPPRESS_CHAR_LENGTH_LIMIT_CHECKS);
  }

  NABoolean isDBTR(UInt32 flags) { return (flags & IS_DBTR_) != 0; };

  void setIsDBTR(UInt32 &flags, NABoolean v) { (v ? flags |= IS_DBTR_ : flags &= ~IS_DBTR_); }

  NABoolean isODBC(UInt32 flags) { return (flags & IS_ODBC_) != 0; };

  void setIsODBC(UInt32 &flags, NABoolean v) { (v ? flags |= IS_ODBC_ : flags &= ~IS_ODBC_); }

  NABoolean isRWRS(UInt32 flags) { return (flags & IS_RWRS_) != 0; };

  void setIsRWRS(UInt32 &flags, NABoolean v) { (v ? flags |= IS_RWRS_ : flags &= ~IS_RWRS_); }

  NABoolean isMXCI(UInt32 flags) { return (flags & IS_MXCI_) != 0; };

  void setIsMXCI(UInt32 &flags, NABoolean v) { (v ? flags |= IS_MXCI_ : flags &= ~IS_MXCI_); }

  NABoolean isInternalFormatIO(UInt32 flags) { return (flags & INTERNAL_FORMAT_IO_) != 0; };
  ...
  void setupBulkMoveInfo(void *desc_, CollHeap *heap, NABoolean isInputDesc, UInt32 flags);
  ex_expr::exp_return_type doBulkMove(atp_struct *atp, void *desc_, char *tgtRowPtr, NABoolean isInputDesc);
  ...
  static NABoolean areCompatible(short source, short target) {
    if ((((source >= REC_MIN_NUMERIC) && (source <= REC_MAX_NUMERIC)) &&
        ...
        ((DFS2REC::isCharacterString(source)) && (DFS2REC::isCharacterString(target))) ||
        ((DFS2REC::isBinaryString(source)) && (DFS2REC::isBinaryString(target))) ||
        ...
        ((source == REC_BLOB) && (target == REC_BLOB)) || ((source == REC_CLOB) && (target == REC_CLOB)) ||
        ((source == REC_BOOLEAN) && (target == REC_BOOLEAN)) || ((source == REC_ARRAY) && (target == REC_ARRAY)) ||
        ((source == REC_ROW) && (target == REC_ROW)))
      return TRUE;
    else
      return FALSE;
  }
