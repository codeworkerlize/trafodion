...
template <class K,class V> class NAHashDictionaryIteratorNoCopy;
...
public:
  ...
  NACollection(CollHeap *heap, CollIndex initLen = 0, NABoolean failureIsFatal = TRUE) : heap_(heap), failureIsFatal_(failureIsFatal), isComplete_(TRUE)
  { allocate(initLen); }
...
private:
  ...
  {
    return( heap_ ?
              (new( heap_, failureIsFatal_) WordAsBits[ words ])
                 :
...
public:
  ...
  NASet(CollHeap  *heap,
        CollIndex initSize = 0,
        NABoolean failureIsFatal = TRUE) : NACollection<T>(heap,initSize, failureIsFatal)
  { invalidateCache(); }
...
public:
  ...
  // insert a new entry
  inline void insert(const T &elem, NABoolean* inserted = NULL) 
   { this->insertAt(this->entries(),elem, inserted); }
...
public:
  ...
  { NASubCollection<T>::addElement(elem, inserted); return *this; }
...
public:
  NASimpleArray (CollHeap * h, NABoolean failureIsFatal = TRUE) 
    : NAArray<T>(h, 0, failureIsFatal) {}
...
public:
  ...
  inline void insert(K* key, V* value, NABoolean* inserted = NULL)
    { 
      NAHashBucketEntry<K, V>* newEntry = 
           new(heap_, failureIsFatal_) NAHashBucketEntry<K,V>(key, value);
...
//#endif
  ...
  K* insert(K* key, V* value, 
            ULng32(*hashFunction)(const K&) = NULL,
            NABoolean* inserted = NULL
           );
...
public:
  ...
  NAHashDictionaryIteratorNoCopy<K,V> (const NAHashDictionary<K,V> & dict, 
                                 enum iteratorEntryType type = iteratorEntryType::EVERYTHING,
                                 const K* key = NULL,
                                 const V* value = NULL,
                                 CollHeap* heap = NULL,
                                 ULng32(*hashFunc)(const K&) = NULL
  ...
  // copy ctor
  NAHashDictionaryIteratorNoCopy<K,V> (const NAHashDictionaryIteratorNoCopy<K,V> & other,
                                 CollHeap * heap) ; 
...
protected:
  NAHashBucket<K,V>* currentBucket() 
    { return (*dict_.hashTable_)[currentBucket_]; }
