...
#define ORIGINAL_ARG_PRESENT(T, V) \
  inline int _arg_present(T arg) { return (arg != V); }
...
#define REPLACEMENT_ARG_PRESENT(T) \
  inline int _arg_present(OptionalArgument<T> arg) { return arg.present; }
REPLACEMENT_ARG_PRESENT(short)
REPLACEMENT_ARG_PRESENT(int)
REPLACEMENT_ARG_PRESENT(__int64)
#endif  // ARG_PRESENT_REPLACEMENT

short _fixedtoascii(fixed_0 qvalue, void *buffer, unsigned short maxdigits);

short _asciitofixed(void *buffer, unsigned short maxdigits, unsigned short *remainingdigits, fixed_0 qvaluein,
                    fixed_0 *qvalueout, short *overflow);
