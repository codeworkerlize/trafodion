...
#define VARSTRING_SETOFFSET(msg, vars, ptr) (msg)->vars.offset = (int_16)_xlow((int_ptr)ptr - (int_ptr)msg)
...
//
enum {
  DIALECT_ZERO = 0,            // %0,  Unenlightened messages
...
#define MESSAGE_HEADER                                                                                   \
  ...
  _redefarrayafter(__request, request, 0, -1, minimum_interpretation_version); /* substruct request*/    \
                                                                               /* reply format*/         \
