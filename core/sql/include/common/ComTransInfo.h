...
enum TransStmtType
{
  ...
  BEGIN_SAVEPOINT_, COMMIT_SAVEPOINT_, ROLLBACK_SAVEPOINT_, RELEASE_SAVEPOINT_,
  SET_TRANSACTION_
};
...
public:
  ...
  enum AutoOption
  {
    AO_ON_, AO_OFF_, AO_NOT_SPECIFIED_
  };
  ...
  TransMode(IsolationLevel il = IL_NOT_SPECIFIED_,
    ...
    : il_(il), am_((short) am), autoCommit_((short) ac), diagAreaSize_(das),
      ...
      multiCommit_((short) mc), multiCommitSize_(mcs), 
  ...
  NABoolean xnInProgress() const
  { return (flags_ & XN_IN_PROGRESS_) != 0; }
  void setXnInProgress(NABoolean v)
  { if (v) flags_ |= XN_IN_PROGRESS_; 
    else   flags_ &= ~XN_IN_PROGRESS_; }

  NABoolean savepointInProgress() const
  { return (flags_ & SAVEPOINT_IN_PROGRESS_) != 0; }
  void setSavepointInProgress(NABoolean v)
  { if (v) flags_ |= SAVEPOINT_IN_PROGRESS_; 
    else   flags_ &= ~SAVEPOINT_IN_PROGRESS_; }
