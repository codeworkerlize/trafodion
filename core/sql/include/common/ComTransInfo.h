...
enum LockMode { SHARE_, EXCLUSIVE_, PROTECTED_, LOCK_MODE_NOT_SPECIFIED_ };
...
class DP2LockFlags {
 public:
  enum BounceLock { WAIT_LOCK = 0, BOUNCE_LOCK = 1 };
  ...
  enum LockSize { ROW_LOCK = 0, GROSS_FILE_LOCK = 1 };

  enum LockState { LOCK_CURSOR = 0, LOCK_SHARED = 1, LOCK_EXCLUSIVE = 2, LOCK_SELECT = 3 };

  DP2LockFlags() : value_(0) {}

  void setBounceLock(BounceLock bl) { flags_.bounceLockFlag_ = (unsigned char)bl; }
  short getBounceLock() const { return flags_.bounceLockFlag_; }

  void setConsistencyLevel(ConsistencyLevel cl) { flags_.consistencyLevel_ = (unsigned short)cl; }
  ...
  void setSelfReferencingUpdate(unsigned char v) { flags_.selfReferencingUpdate_ = v; }
  short getSelfReferencingUpdate() const { return flags_.selfReferencingUpdate_; }
...
class TransMode : public NAVersionedObject {
 public:
  ...
  static IsolationLevel ATtoIL(AccessType at) { return (IsolationLevel)at; }

  enum AccessMode { READ_ONLY_, READ_WRITE_, AM_NOT_SPECIFIED_, READ_ONLY_SPECIFIED_BY_USER_ };

  enum AutoOption { AO_ON_, AO_OFF_, AO_NOT_SPECIFIED_ };

  enum AutoCommit { ON_, OFF_, AC_NOT_SPECIFIED_ };

  enum MultiCommit { MC_NOT_SPECIFIED_, MC_ON_, MC_OFF_ };

  // whether to rollback waited or nowaited. Default is waited.
  enum RollbackMode {
    ROLLBACK_MODE_WAITED_,
    ROLLBACK_MODE_NOWAITED_,
    ROLLBACK_MODE_NOT_SPECIFIED_,
    NO_ROLLBACK_,
    NO_ROLLBACK_IN_IUD_STATEMENT_
  };
  ...
  enum Flags {
    ...
    XN_IN_PROGRESS_ = 0x20,
    SAVEPOINT_IN_PROGRESS_ = 0x40,
    AUTO_COMMIT_SAVEPOINT_ = 0x80
  };
  ...
  TransMode(IsolationLevel il = IL_NOT_SPECIFIED_, AccessMode am = AM_NOT_SPECIFIED_, AutoCommit ac = AC_NOT_SPECIFIED_,
      ...
      : il_(il),
        ...
        flags_(0),
        rm_((short)rm),
        autoAbortInterval_(aiVal),
        multiCommit_((short)mc),
        multiCommitSize_(mcs),
        autoCommitSP_(AO_ON_),
        savedAutoCommitSP_(AO_NOT_SPECIFIED_),
        NAVersionedObject(-1) {}
  ...
  void setIsolationLevel(Int16 il) { il_ = il; }
  void setAccessMode(Int16 am) { am_ = am; }
  void setAutoCommit(Int16 autoCommit) { autoCommit_ = autoCommit; }
  void setRollbackMode(Int16 rm) { rm_ = rm; }
  void setFlags(Int16 flags) { flags_ = flags; }
  ...
  NABoolean getAutoCommitOn() { return (autoCommit_ == ON_); }
  NABoolean getAutoCommitOff() { return (autoCommit_ == OFF_); }
  NABoolean getAutoCommitSavepointOn() { return (autoCommitSP_ == AO_ON_); }
  NABoolean getAutoCommitSavepointOff() { return (autoCommitSP_ == AO_OFF_); }
  void setAutoCommitSavepointOn() { autoCommitSP_ = AO_ON_; }
  void setAutoCommitSavepointOff() { autoCommitSP_ = AO_OFF_; }
  void setAutoCommitSavepointNotSpecified() { autoCommitSP_ = AO_NOT_SPECIFIED_; }
  ...
  void setUserTransMode(NABoolean v) {
    if (v)
      flags_ |= USER_TRANS_MODE_;
    else
      flags_ &= ~USER_TRANS_MODE_;
  }

  NABoolean xnInProgress() const { return (flags_ & XN_IN_PROGRESS_) != 0; }
  void setXnInProgress(NABoolean v) {
    if (v)
      flags_ |= XN_IN_PROGRESS_;
    else
      flags_ &= ~XN_IN_PROGRESS_;
  }

  NABoolean savepointInProgress() const { return (flags_ & SAVEPOINT_IN_PROGRESS_) != 0; }
  void setSavepointInProgress(NABoolean v) {
    if (v)
      flags_ |= SAVEPOINT_IN_PROGRESS_;
    else
      flags_ &= ~SAVEPOINT_IN_PROGRESS_;
  }

  NABoolean isolationLevelCompatible(const TransMode &o) const { return il_ <= o.il_; }
  ...
  void updateAccessModeFromIsolationLevel(IsolationLevel il, NABoolean affectIL = TRUE) {
    Int16 tempIL = il_;
    if (il != IL_NOT_SPECIFIED_) tempIL = (Int16)il;

    if (am_ != READ_ONLY_SPECIFIED_BY_USER_) am_ = (Int16)((tempIL == READ_UNCOMMITTED_) ? READ_ONLY_ : READ_WRITE_);

    if (affectIL) il_ = tempIL;
  }
  ...
  void print(ostream &out) {
    ...
    out << "AccessMode :" << am_ << endl;
    out << "autoCommit_ :" << autoCommit_ << endl;
    out << "flags_ :" << flags_ << endl;
    out << "diagAreaSize_ :" << diagAreaSize_ << endl;
    out << "RollbackMode :" << rm_ << endl;
    out << "multiCommit_ :" << multiCommit_ << endl;
    out << "autoAbortInterval_ :" << autoAbortInterval_ << endl;
    out << "multiCommitSize_ :" << multiCommitSize_ << endl;
    out << "savedAutoCommitSP_ :" << savedAutoCommitSP_ << endl;
    out << "filler0_ :" << (int)filler0_[0] << (int)filler0_[1] << (int)filler0_[2] << (int)filler0_[3] << endl;
  }
...
class StmtLevelAccessOptions {
 public:
  StmtLevelAccessOptions(TransMode::AccessType at = TransMode::ACCESS_TYPE_NOT_SPECIFIED_,
      ...
      : accessType_(at), lockMode_(lm), updateOrDelete_(FALSE), scanLockForIM_(FALSE) {}
  ...
  NABoolean userSpecified() {
    return (accessType_ != TransMode::ACCESS_TYPE_NOT_SPECIFIED_ || lockMode_ != LOCK_MODE_NOT_SPECIFIED_);
  }

  void updateAccessOptions(TransMode::AccessType at, LockMode lm = LOCK_MODE_NOT_SPECIFIED_);
