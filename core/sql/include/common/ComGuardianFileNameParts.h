...
//
const unsigned short ComFULLY_QUALIFIED_OSS_NAME_MAX_LEN =
    (4 * ComFILE_NAME_PART_CHAR_MAX_LEN) + 6;  // 4 name parts, 6 slashes
...
class ComGuardianFileNamePart {
 protected:
  ...
  ComGuardianFileNamePart(void) : namePart_(0), nullTerminator_(0){};

  // Copy constructor
  ComGuardianFileNamePart(const ComGuardianFileNamePart &rhs) : namePart_(rhs.namePart_), nullTerminator_(0){};

  // Assignment constructors
  ComGuardianFileNamePart(const char *namePart)  // Name part from string
      : namePart_(0), nullTerminator_(0) {
    (*this) = namePart;
  };

  ComGuardianFileNamePart(const long namePartAsInt64)  // Name part from 64 bit int
      : namePart_(namePartAsInt64), nullTerminator_(0){};
 ...
 public:
  ...
  const ComGuardianFileNamePart &operator=(const long rhs) {
    namePart_ = rhs;
    return *this;
  };

  // Comparison operators - between objects
  NABoolean operator==(const ComGuardianFileNamePart &rhs) const { return (namePart_ == rhs.namePart_); };
  NABoolean operator!=(const ComGuardianFileNamePart &rhs) const { return (namePart_ != rhs.namePart_); };

  // Comparison operators - between object and string
  NABoolean operator==(const char *rhs) const { return (!strcmp((char *)&namePart_, rhs)); };
  NABoolean operator!=(const char *rhs) const { return (strcmp((char *)&namePart_, rhs)); };
  NABoolean operator<=(const char *rhs) const { return (strcmp((char *)&namePart_, rhs) <= 0); };
  NABoolean operator<(const char *rhs) const { return (strcmp((char *)&namePart_, rhs) < 0); };
  NABoolean operator>=(const char *rhs) const { return (strcmp((char *)&namePart_, rhs) >= 0); };
  NABoolean operator>(const char *rhs) const { return (strcmp((char *)&namePart_, rhs) > 0); };

  // Type conversion operators
  operator const char *() const { return (const char *)&namePart_; };

  // Accessors
  NABoolean isEmpty(void) const { return (namePart_ == 0); };
  NABoolean isNull(void) const { return isEmpty(); };
  size_t length(void) const { return strlen(*this); };

  const char *castToConstChar() const { return (const char *)this; };
  ...
  NABoolean isNamePartValid(const char requiredFirstChar = 0, const NABoolean emptyIsOK = FALSE,
                            const size_t minLength = ComFILE_NAME_PART_CHAR_MAX_LEN) const;
  // Mutators
  void clear(void) { namePart_ = 0; };
  void trim(void);  // Remove trailing blanks

 protected:
  long namePart_;        // The name part, as a 64 bit integer
...
class ComNodeName : public ComGuardianFileNamePart {
 public:
  ...
  const ComNodeName &operator=(const char *rhs) {
    ComGuardianFileNamePart::operator=(rhs);
    return *this;
  };

  // Comparison operators
  NABoolean operator==(const ComNodeName &rhs) const { return ComGuardianFileNamePart::operator==(rhs); };
  NABoolean operator!=(const ComNodeName &rhs) const { return ComGuardianFileNamePart::operator!=(rhs); };
  NABoolean operator<=(const ComNodeName &rhs) const { return ComGuardianFileNamePart::operator<=(rhs); };
  NABoolean operator<(const ComNodeName &rhs) const { return ComGuardianFileNamePart::operator<(rhs); };
  NABoolean operator>=(const ComNodeName &rhs) const { return ComGuardianFileNamePart::operator>=(rhs); };
  ...
  // Accessors
  NABoolean isLocalNode(void) const { return (namePart_ == ComGetNodeNameAsInt64(-1)); };
  NABoolean isDefaultNode(void) const { return (namePart_ == ComGetNodeNameAsInt64(-2)); };
  ...
  void setDefault(void) { namePart_ = ComGetNodeNameAsInt64(-2); };
};
...
class ComVolumeName : public ComGuardianFileNamePart {
 public:
  ...
  // Accessors
  NABoolean isValid(const NABoolean emptyIsOK = FALSE) const { return isNamePartValid('$', emptyIsOK, 2); };
  const char *getVolumeNameWithoutDollar(void) const { return &((char *)(&namePart_))[1]; };
...
class ComSubvolumeName : public ComGuardianFileNamePart {
 public:
  ...
  // Accessors
  NABoolean isValid(const NABoolean emptyIsOK = FALSE) const { return isNamePartValid(0, emptyIsOK, 1); };
