...
#define MAX_CHAR_SET_STRING_LENGTH \
  128  // keep in sync with w:/cli/sqlcli.h
...
class CharInfo {
 public:
  ...
  static const char *getCharSetName(CharSet cs, NABoolean retUnkAsBlank = FALSE);
  static NABoolean isCharSetSupported(CharSet cs);
  static NABoolean isCharSetSupported(const char *name) { return isCharSetSupported(getCharSetEnum(name)); }

  static NABoolean isCharSetFullySupported(CharSet cs);
  static NABoolean isCharSetFullySupported(const char *name) { return isCharSetFullySupported(getCharSetEnum(name)); }
  ...
  static NABoolean isModuleCharSetSupported(CharSet cs) { return cs == CharInfo::ISO88591; };

  static NABoolean isHexFormatSupported(CharSet cs);
  static NABoolean isHexFormatSupported(const char *name) { return isHexFormatSupported(getCharSetEnum(name)); }
  ...
  static CharSet getCharSetEnumFromNSK_MBCS(int n) {
    switch (n) {
      ...
      case 12:
        return KSC5601_MP;
      default:
        return UnknownCharSet;  // we don't support other magic numbers
  ...
  static NABoolean isAssignmentCompatible(CharSet clientCS, CharSet mxCS) {
    return clientCS == mxCS || (clientCS == CharInfo::UNICODE &&  // relaxation
...
class CollationInfo : public NABasicObject {
 ...
 public:
  ...
  enum CollationMisc { MAX_NAME_PARTS = 4, SIZEARRAY_SIZE = MAX_NAME_PARTS + 1, OFFSETARRAY_SIZE = MAX_NAME_PARTS - 1 };

  enum Pass { FirstPass = 0, SecondPass = 1, ThirdPass = 2, FourthPass = 3 };
  ...
  enum SortDirection { DefaultDir = 0, Ascending = 1, Descending = 2 };

  CollationInfo(CollHeap *h, CharInfo::Collation co, const char *name, CollationFlags flags = NO_FLAGS,
...
class CollationDB : private CollationDBSupertype {
 public:
  ...
  CharInfo::Collation insert(ComMPLoc &loc, const ComMPLoc *defaultMPLoc,
                             CollationInfo::CollationFlags flags = CollationInfo::NO_FLAGS);

  CharInfo::Collation insert(QualifiedName &qn, const SchemaName *defaultSchema,
                             CollationInfo::CollationFlags flags = CollationInfo::NO_FLAGS);
