...
int LocaleToUTF8(const enum cnv_version version, const char *in_bufr, const int in_len, const char *out_bufr,
                 ...
                 unsigned int *output_data_len_p = NULL, const int addNullAtEnd_flag = FALSE,
...
int UTF8ToLocale(const enum cnv_version version, const char *in_bufr, const int in_len, const char *out_bufr,
                 const int out_len, enum cnv_charset charset, char *&first_untranslated_char,
                 unsigned int *output_data_len_p = NULL, const int addNullAtEnd_flag = FALSE,
                 const int allow_invalids = FALSE, unsigned int *translated_char_cnt_p = NULL,
...
int LocaleToUTF16(const enum cnv_version version, const char *in_bufr, const int in_len, const char *out_bufr,
                  const int out_len, enum cnv_charset charset, char *&first_untranslated_char,
                  unsigned int *output_data_len_p = NULL, const int cnv_flags = 0, const int addNullAtEnd_flag = FALSE,
                  unsigned int *translated_char_cnt_p = NULL, unsigned int max_chars_to_convert = 0xffffffff);
...
int gbkToUtf8(char *gbkString, size_t gbklen, char *result, size_t outlen, bool addNullAtEnd = FALSE);
...
#define CNV_ERR_TARGET_SIZE_INVALID \
  -7                             // Provided target buffer is not large enough
...
int lightValidateUTF8StrAndPad(char *bufr, int in_len, int max_chars = 0, int ignore_trailing_blanks = 1);
...
inline int fillWithMinMaxUTF8Chars(char *bufr,
                                   int in_len,  // in bytes
                                   int max_chars, int is_max) {
  if (is_max)
    return fillWithMaxUTF8Chars(bufr, in_len, max_chars);
  else
    return fillWithMinUTF8Chars(bufr, in_len, max_chars);
}
