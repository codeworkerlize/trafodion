...
#define TRAF_LOAD_ERROR_COUNT_TABLE    TRAF_RESERVED_NAMESPACE6":""ERRORCOUNTER"
...
enum ComOperation{COM_UNKNOWN_IUD, COM_INSERT, COM_DELETE, COM_UPDATE,
		  COM_SELECT, COM_ROUTINE, COM_INSERT_UPDATE, COM_INSERT_DELETE,
		  COM_INSERT_UPDATE_DELETE, COM_UPDATE_DELETE};
...
enum ComTableType {  COM_REGULAR_TABLE          = 0
...
enum ComColumnClass { COM_UNKNOWN_CLASS           = 0
                    , COM_SYSTEM_COLUMN           = 1
                    , COM_USER_COLUMN             = 2
...
enum ComNamespaceOper { COM_CREATE_NAMESPACE       = 1,
...
enum ComColumnDirection { COM_UNKNOWN_DIRECTION    = 0
                        , COM_INPUT_COLUMN         = 1
                        , COM_OUTPUT_COLUMN        = 2
                        , COM_INOUT_COLUMN         = 3
                        };
...
enum ComObjectType { COM_UNKNOWN_OBJECT                = 0
...
enum ComSequenceGeneratorType { COM_UNKNOWN_SG            = 0
                              , COM_INTERNAL_SG           = 1
                              , COM_EXTERNAL_SG           = 2
...
enum ComPartitioningScheme { COM_UNSPECIFIED_PARTITIONING = 0
...
enum ComPartitioningSchemeV2 { COM_NO_PARTITION = 0
...
enum ComRowFormat {  COM_UNKNOWN_FORMAT_TYPE     = 0
...
enum ComInsertMode  { COM_UNKNOWN_TABLE_INSERT_MODE = 0,
...
enum ComRoutineSQLAccess { COM_UNKNOWN_ROUTINE_SQL_ACCESS  = 0
                         , COM_NO_SQL                      = 1
                         , COM_CONTAINS_SQL                = 2
                         , COM_READS_SQL                   = 3
                         , COM_MODIFIES_SQL                = 4
...
enum ComRoutineType { COM_UNKNOWN_ROUTINE_TYPE    = 0
...
enum ComSQLDataType { COM_UNKNOWN_SDT                   = 0
                    , COM_CHARACTER_SDT                 = 1
                    , COM_VARCHAR_SDT                   = 2
...
static const char * const hiveProtoTypeKindStr[] =
  {
    "unknown",
    "boolean",
    "tinyint",
...
#define COMPARE_PTRS(x, y) ((x == NULL && y == NULL) ||  \
                            (x && y && (*x == *y)))

#define COMPARE_CHAR_PTRS(x, y) ((x == NULL && y == NULL) || \
                             (x && y && !strcmp(x, y)))

#define COMPARE_VOID_PTRS(x, xlen, y, ylen) \
                   ((x == NULL && y == NULL) || \
                    (x && y && xlen == ylen &&  \
                     !memcmp((char*)x, (char*)y, xlen)))

#define COMPARE_PART_FUNCS(x, y) \
           ((x == NULL && y == NULL) || \
            (x && y && \
             x->comparePartFuncToFunc(*y) == COMPARE_RESULT::SAME))
...
#define COMPARE_TRAFDESC_PTRS(x, y) \
           ((x == NULL && y == NULL) || \
            (x && y && x->deepCompare(*y)))
