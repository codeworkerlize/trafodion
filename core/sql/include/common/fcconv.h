...
#define CONV_FUNC_NEXT                                        \
  0x1000                   /* Try conversion function next if \
                              table lookup fails		     */
...
#define GET_VAL(cd, idx, val, io)                                                              \
  {                                                                                            \
    ...
    _row = ROW(idx);                                                                           \
    _col = COL(idx);                                                                           \
    if ((_row >= (cd)->##io##row_count) || ((_row = (cd)->##io##row_table[_row]) == UCS2_IDE)) \
      val = idx;                                                                               \
...
#define GET_VAL(cd, idx, val, io)                                                              \
  {                                                                                            \
    ...
    _row = ROW(idx);                                                                           \
    _col = COL(idx);                                                                           \
    if ((_row >= (cd)->##io##row_count) || ((_row = (cd)->##io##row_table[_row]) == UCS2_BAD)) \
      val = BAD;                                                                               \
    else if (IS_CELL4_ROW(_row))                                                               \
      val = (cd)->##io##cell4_tab[MASKROW(_row)][_col];                                        \
    else {                                                                                     \
...
typedef int (*cnvfunc_t)(_LC_fcconv_iconv_t *, uchar_t **, size_t *, uchar_t **, size_t *);
...
extern int __from_ucs_exec(_LC_fcconv_iconv_t *, uchar_t **, size_t *, uchar_t **, size_t *);
extern int __to_ucs_exec(_LC_fcconv_iconv_t *, uchar_t **, size_t *, uchar_t **, size_t *);
extern int __cs_to_ucs_exec(_LC_fcconv_iconv_t *, uchar_t **, size_t *, uchar_t **, size_t *);
extern int __sb_to_ucs_exec(_LC_fcconv_iconv_t *, uchar_t **, size_t *, uchar_t **, size_t *);
extern int __sb_to_sb_exec(_LC_fcconv_iconv_t *, uchar_t **, size_t *, uchar_t **, size_t *);
extern int __mb_to_mb_exec(_LC_fcconv_iconv_t *, uchar_t **, size_t *, uchar_t **, size_t *);
