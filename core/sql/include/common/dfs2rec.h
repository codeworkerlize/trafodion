...
enum { REC_SYSTEM_DEFAULT = -1, REC_NO_DEFAULT = -2, REC_NULL_DEFAULT = -3 };
...
class rec_field_struct {
 ...
 public:
  ...
  int getPrecision() {
    ...
    else if (type == REC_DATETIME) {
      return getRecDateTimeCode((rec_datetime_field)getBeginType(), (rec_datetime_field)getEndType());
    } else
      return 0;
  };
