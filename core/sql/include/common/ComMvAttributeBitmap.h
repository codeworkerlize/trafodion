...
class ComMvAttributeBitmap : public NABasicObject {
 public:
  enum Result { ATTRIBUTE_SET, NO_CHAGE, INTERNAL_ERROR };

  ComMvAttributeBitmap() : bitmap_(0) {}

  ComMvAttributeBitmap(const ComMvAttributeBitmap &other) : bitmap_(other.bitmap_) {}

  NABoolean operator==(const ComMvAttributeBitmap &other) const { return (bitmap_ == other.bitmap_); }
  ...
  inline NABoolean getIsAnMv() const { return IsBitSet(IS_AN_MV); }

  inline NABoolean getIsInsertLog() const { return IsBitSet(INSERTLOG); }

  inline NABoolean getIsLockOnRefresh() const { return IsBitSet(LOCKONREFRESH); }

  inline NABoolean getIsEnableRewrite() const { return IsBitSet(ENABLE_REWRITE); }

  inline NABoolean getIsMvUnInitialized() const { return !IsBitSet(MV_INITIALIZED); }

  inline NABoolean getIsMvUnAvailable() const { return IsBitSet(MV_UNAVAILABLE); }

  inline NABoolean getLogCreatedOnMVCreation() const { return IsBitSet(LOGS_ON_CREATION); }

  inline NABoolean getLoggingRequired() const { return IsBitSet(LOGGING_REQUIRED); }

  // Note: mixed is auto logging but not auto range logging.
  inline NABoolean getAutomaticRangeLoggingRequired() const { return (IsBitSet(LOGGING_REQUIRED | AUTO_RANGELOG)); }

  // exclude from coverage since this method is used for similarity
  // check and now we use AQR instead
  inline NABoolean getInitOnStmtMvOnMe() const { return (IsBitSet(INIT_ON_STMT_ON_ME)); }

  // exclude from coverage since Range Logging is not supported
  inline NABoolean getEnableMVLOGExecution() const {
    return (IsBitSet(MVS_ON_ME | MANUAL_RANGELOG) || IsBitSet(MVS_ON_ME | MIXED_RANGELOG));
  }
 ...
 private:
  ...
  enum BitmapMasks {
    ...
    BASE_TABLE_ATTRIBUTES_MASK = (RANGE_LOGGING_MASK | INSERT_LOGGING_MASK | LOCKONREFRESH_MASK | MVS_ALLOWED_MASK),

    MV_ATTRIBUTES_MASK = (REWRITE_MASK | MVSTATUS_MASK | MV_AUDIT_MASK),

    LAST_BITMAP_MASK = 0x77777777
  };

  inline NABoolean IsBitSet(ComSInt32 flag) const { return ((bitmap_ & flag) == flag) ? TRUE : FALSE; }
