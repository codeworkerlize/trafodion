...
class NABlock {
 public:
  // If more flags are added, muse.cpp may need to be changed, depending on
  // what info is collected by the utility. Currently, it collects the
  // block count only.
  enum {
    MMAP_BIT = 0x4,    // This NABlock was allocated with mmap.
    EXTERN_BIT = 0x8,  // Memory for this block was externally
                       // allocated
    STICKY_BIT = 0x10  // Memory for this block should not be
                       // freed during NAMemory::reInitialize().
  };
  ...
  NABoolean isStickySegment() { return (sflags_ & STICKY_BIT) != 0; }
...
class NAMemory : public NABasicObject {
 ...
 public:
  ...
  // programs.
  enum DerivedClass { NAHEAP_CLASS = 0, COMSPACE_CLASS = 1, DEFAULTIPCHEAP_CLASS = 2 };
  ...
  NAMemory(const char *name, NAMemoryType type, size_t blockSize, size_t upperLimit);

  // DERIVED_MEMORY
  NAMemory(const char *name, NAHeap *parent, size_t blockSize, size_t upperLimit);

  // an NAMemory of type EXECUTOR_MEMORY that imposes the NAMemory struture
  // on already allocated memory
  NAMemory(const char *name, SEG_ID segmentId, void *baseAddr, off_t heapStartOffset, size_t maxSize,
           // set to FALSE when it is necessary to not abort() upon
           // allocation failures, regardless of how the failureIsFatal
           // argument is passed to checkSize(),
           // allocateAlignedHeapMemory(), allocateHeapMemory(),
           // allocateIpcHeapMemory(). Setting to FALSE is thus
           // a master switch. If set to TRUE, then each failureIsFatal
           // argument determines whether to abort() or not.
  ...
  NAMemory(const char *name, SEG_ID segmentId, void *baseAddr, off_t heapStartOffset, size_t maxSize,
           ...
           // A pointer to the segment DB from which shared
           // memory segments can be allocated/deallocated.
           SharedSegmentDB *segmentDB = NULL);
  ...
  NABoolean containsAddress(void *addr) { return NABlock::blockHolding(firstBlk_, addr) != NULL; }

  NABoolean getSharedMemory() { return sharedMemory_; }

 private:
  void setSharedMemory() { sharedMemory_ = TRUE; }  // used only by copy constructor
 public:
  NABoolean useSemaphore() { return useSemaphore_; }
  void setUseSemaphore() { useSemaphore_ = TRUE; }

  NABoolean needToCheckIfLocked() {
    return useSemaphore() || getNamedSemaphore() || getNamedSem() || getLockController();
  }

  // get/set the embedded named semaphore.
  NamedSemaphore *getNamedSemaphore() { return namedSemaphore_; }
  void setNamedSemaphore(NamedSemaphore *x) { namedSemaphore_ = x; }

  // get/set the embedded System V named semaphore.
  NamedSem *getNamedSem() { return namedSem_; }
  void setNamedSem(NamedSem *x) { namedSem_ = x; }

  // get/set the distributed lock controller.
  LockController *getLockController() { return lockController_; }
  void setLockController(LockController *x) { lockController_ = x; }

  NABoolean isComSpace(void) { return derivedClass_ == COMSPACE_CLASS; };
  NABoolean isNAHeap(void) { return derivedClass_ == NAHEAP_CLASS; };

  NABoolean checkSize(size_t size, NABoolean failureIsFatal);

 protected:
  // init data member of this class. Called within some of the
  // cstrs of this class. When deligate cstr (a C++11 feature)is
  // supported by the C++ compiler, this method can be removed.
  void initMe(const char *name, SEG_ID segmentId, void *baseAddr, off_t heapStartOffset, size_t maxSize,
              // set to FALSE when it is necessary to not abort() upon
              // allocation failures, regardless of how the failureIsFatal
              // argument is passed to checkSize(),
              // allocateAlignedHeapMemory(), allocateHeapMemory(),
              ...
              // Set to true so that the first NABlock constructed on
              // at the address of 'heapStartOffset' can survive the
              // reInitialize() call and be reused afterwards.
              NABoolean firstBlockIsSticky = FALSE);

  // check if there is any lock (e.g., a semaphore) in place
  // to protect allocation/deallocation of memory. Such lock
  // could be either the RTS semaphore or the distributed lock.
  // See this method's implementation.
  NABoolean checkIfLocked();

  // Return true if the named semaphore exists (not NULL)
  // and is locked.
  NABoolean checkIfNamedSemaphoreLocked();
  NABoolean checkIfNamedSemLocked();

  // Return true if a distributed lock exists (not NULL)
  // and is locked.
  NABoolean checkIfDistributedLockLocked();
 ...
 protected:
  NABoolean failureIsFatal_;
  NABoolean allocFailed_;  // set to true if most recent alloc failed.
 ...
 protected:
  ...
  //
  // sharedMemory_ TRUE means the NAHeap is in a shared segment, and
  // should not be extended into non-shared memory. FALSE means that
  // the NAHeap is not in a shared segment.
  //
  // useSemaphore_ TRUE means that allocation/deallocation is guarded
  // by a semaphore (namedSemaphore_ if non-null, the RTS semaphore
  // otherwise). FALSE means that allocation/deallocation is not guarded
  // by a semaphore.
  //
  // These flags are orthogonal. For example, the RTS segment is shared,
  // so any heap in this segment will have sharedMemory_ == TRUE. However,
  // mutual exclusion to the heap might be provided by the heap itself
  // (in which case useSemaphore_ == TRUE), or at a layer above heap
  // access (in which case useSemaphore_ is FALSE).
  //
  // In a non-shared segment, sharedMemory_ is FALSE. Such a segment is
  // local to a single process. So long as the heap is used by a single
  // thread in the process, or access to it is guarded by a higher-level
  // mutex, useSemaphore_ is FALSE. However, if the process has multiple
  // threads accessing the heap with no higher layer mutex, a semaphore
  // at this level is useful, in which case useSemaphore_ would be TRUE.
  // At this writing, however, there are no known use cases of the latter.
  NABoolean sharedMemory_;  // if TRUE, heap will not be extended into non-shared memory
  NABoolean useSemaphore_;  // if TRUE, heap allocations will be guarded by semaphore

  // When set, this provides exclusive modifiction protection
  // with a NamedSemaphore.
  NamedSemaphore *namedSemaphore_;
  NamedSem *namedSem_;
...
class NAHeap : public NAMemory {
 public:
  ...
  enum { NSMALLBINS = 32, NTREEBINS = 32 };

  // see the corresponding NAMemory constructors for an explanation

  NAHeap();
  NAHeap(const char *name, NAHeap *parent, size_t blockSize = 0, size_t upperLimit = 0);
  NAHeap(const char *name, NAMemoryType type, size_t blockSize = 0, size_t upperLimit = 0);

  // Constructors that imposes the NAHeap struture on already allocated memory
  NAHeap(const char *name, SEG_ID segmentId, void *baseAddr, off_t heapStartOffset, size_t maxSize,
         NABoolean failureIsFatal = TRUE);

  NAHeap(const char *name, SEG_ID segmentId, void *baseAddr, off_t heapStartOffset, size_t maxSize,
         NamedSemaphore *semaphore, NABoolean failureIsFatal = TRUE);

  // Constructor that imposes the NAHeap struture on already allocated memory
  NAHeap(const char *name, SEG_ID segmentId, void *baseAddr, off_t heapStartOffset, size_t maxSize,
         LockController *controller, NABoolean failureIsFatal = TRUE,
         // A pointer to the segment DB from which shared
         // memory segments can be allocated/deallocated.
         SharedSegmentDB *segmentDB = NULL);
  ...
  void setThreadSafe();
  const inline bool getThreadSafe() { return mutex_.isEnabled(); }

#ifdef _DEBUG
  LIST(TrafAddrStack *) * getSAL() { return la_; }
#endif  // _DEBUG

  NABoolean isStmtHeap();  // stmt heap
  NABoolean isHQCHeap();   // hybrid query cache heap
  NABoolean isSQCHeap();   // domain/semantic query cache heap
 ...
 private:
  ...
  NAMutex mutex_;                          // Mutex to serialize thread safe access
 ...
 private:
  ...
  void unlinkFirstSmallFragment(NAHeapFragment *b, NAHeapFragment *p, bindex_t idx);
  ...
  void checkAllSmallbins(const char *msg = "");
 ...
 public:
#if (defined(_DEBUG) || defined(NSK_MEMDEBUG))
  size_t traverseAndCheck(const char *msg = NULL);
#endif  // (defined(_DEBUG) || defined(NSK_MEMDEBUG))
};
