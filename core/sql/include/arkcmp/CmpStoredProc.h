...
class CmpSPOutputFormat : public NABasicObject {
 public:
  ...
  static NABoolean ElemDDLColDef2ColumnDescStruct(ElemDDLColDef *elemDDL, const char *tableName,
...
class CmpISPDataObject : public CmpSPDataObject {
 public:
  CmpISPDataObject(CmpMessageISPRequest *, CmpInternalSP *isp, CollHeap *outHeap, CmpContext *context);
...
class CmpISPFuncs {
 public:
  ...
  struct ProcFuncsStruct {
    ...
    ProcFuncsStruct(const char *procName = 0, SP_COMPILE_FUNCPTR compileFunc = 0,
                    SP_INPUTFORMAT_FUNCPTR inFormatFunc = 0, SP_PARSE_FUNCPTR parseFunc = 0,
                    SP_NUM_OUTPUTFIELDS_FUNCPTR outNumFormatFunc = 0, SP_OUTPUTFORMAT_FUNCPTR outFormatFunc = 0,
                    SP_PROCESS_FUNCPTR procFunc = 0, SP_HANDLE spHandle = 0) {
      if (procName)
    ...
    NABoolean operator==(const ProcFuncsStruct &other) { return (strcmp(procName_, other.procName_) == 0); }
  };
