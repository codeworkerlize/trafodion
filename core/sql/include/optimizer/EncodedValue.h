...
class NormValue  // NOT a NABasicObject (memreduction)
{
 private:
  inline void copy(const NormValue &other) {
    theValue_ = other.theValue_;
    isNullFlag_ = other.isNullFlag_;
  }
 ...
 public:
  ...
  // copy constructor
  NormValue(const NormValue &other) { copy(other); }
  ...
  inline NABoolean operator>(const NormValue &other) const { return (theValue_ > other.theValue_); }
  inline NABoolean operator>=(const NormValue &other) const { return (theValue_ >= other.theValue_); }
  inline NABoolean operator<(const NormValue &other) const { return (theValue_ < other.theValue_); }
  inline NABoolean operator<=(const NormValue &other) const { return (theValue_ <= other.theValue_); }
  inline NABoolean operator==(const NormValue &other) const { return (theValue_ == other.theValue_); }
  ...
  inline void setNull() {
    theValue_ = _ENCODEDVALUE_NULL_VALUE_;
    isNullFlag_ = TRUE;
  }
...
class EncodedValue : public NABasicObject {
 public:
  ...
  EncodedValue(const NAWchar *theValue) : valueList_(NULL), heap_(HISTHEAP) {
    const NAColumnArray empty((CollHeap *)0 /* NULL CollHeap* */);
    constructorFunction(theValue, empty, FALSE);
  }

  EncodedValue(const wchar_t *theValue, const NAColumnArray &columns, ConstValue *cvPtrs[] = NULL)
      : valueList_(NULL), heap_(HISTHEAP) {
    constructorFunction(theValue, columns, TRUE, cvPtrs);
  }

  EncodedValue(const NAWchar *theValue, const NAColumn *column) : valueList_(NULL), heap_(HISTHEAP) {
 ...
 private:
  ...
  void constructorFunction(const wchar_t *theValue, const NAColumnArray &columns, NABoolean okToReportErrors,
                           ConstValue **cvPtr = NULL);
 ...
 public:
  ...
  inline NABoolean operator>(const EncodedValue &other) const { return (compare(other) == MORE); }
  inline NABoolean operator>=(const EncodedValue &other) const {
    return ((compare(other) == MORE) || (compare(other) == SAME));
  }
  inline NABoolean operator<(const EncodedValue &other) const { return (compare(other) == LESS); }
  inline NABoolean operator<=(const EncodedValue &other) const {
    return ((compare(other) == LESS) || (compare(other) == SAME));
  }
  inline NABoolean operator==(const EncodedValue &other) const { return (compare(other) == SAME); }
  inline NABoolean operator!=(const EncodedValue &other) const { return (compare(other) != SAME); }
 ...
 private:
  ...
  // ---------------------------------------------------------------------
  void addANormValue(EncodedValue *thisPtr, ItemExpr *itemPtr, NABoolean negate);
