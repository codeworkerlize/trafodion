...
class BaseColumn : public ItemExpr {
 public:
  ...
  NABoolean isEIC(ValueId id) const { return (getValueId() == id) OR equivalentIndexCols_.contains(id); }
  ...
  virtual NABoolean isAColumnReference() { return TRUE; }
  ...
  ItemExpr *copyTopNodeNonleaf(ItemExpr *, CollHeap *outHeap) { return copyTopNodeAndValueId(outHeap); };

  NABoolean appearInALocalPredTogether(ItemExpr *other);
...
class ColReference : public ItemExpr {
 public:
  ColReference(ColRefName *colRefName)
      : ItemExpr(ITM_REFERENCE),
        colRefName_(colRefName),
        starExpansion_(NULL),
        parent_(NULL),
        targetColumnClass_(USER_COLUMN),
        ignoreUnusedColErr_(false),
        isCompCol_(FALSE),
        isResolved_(FALSE),
        numCompParts_(0),
        numTotalParts_(0),
        compNames_(CmpCommon::statementHeap()),
        compArrIndexes_(CmpCommon::statementHeap()) {}
  ...
  void setStarExpansion(const ColumnDescList *columnList) { starExpansion_ = columnList; }
  ...
  void setCompColInfo(NABoolean isCompCol, NABoolean isResolved, UInt32 numCompParts, UInt32 numTotalParts,
                      NAList<NAString> &compNames, NAList<UInt32> &compArrIndexes) {
    isCompCol_ = isCompCol;
    isResolved_ = isResolved;
    numCompParts_ = numCompParts;
    numTotalParts_ = numTotalParts;
    compNames_ = compNames;
    compArrIndexes_ = compArrIndexes;
  }

  void resetCompColInfo() {
    isCompCol_ = FALSE;
    isResolved_ = FALSE;
    numCompParts_ = 0;
    numTotalParts_ = 0;
    compNames_.clear();
    compArrIndexes_.clear();
  }

  const NABoolean isCompCol() { return isCompCol_; }
  const NABoolean isResolved() { return isResolved_; }
  const NAList<NAString> getCompNames() { return compNames_; }
  const NAList<UInt32> &getCompArrIndexes() { return compArrIndexes_; }

  NABoolean isIgnoreUnusedColErr() { return ignoreUnusedColErr_; }

  void setIgnoreUnusedColErr(NABoolean ignore_err) { ignoreUnusedColErr_ = ignore_err; }
 ...
 private:
  ...
  NABoolean ignoreUnusedColErr_;

  // used withh composite columns
  NABoolean isCompCol_;
  NABoolean isResolved_;  // used with isCompCol_
  UInt32 numCompParts_;
  UInt32 numTotalParts_;
  NAList<NAString> compNames_;
  NAList<UInt32> compArrIndexes_;
...
class ConstValue : public ItemExpr {
 public:
  ...
  ConstValue(const NAWString &strval, enum CharInfo::CharSet charSet = CharInfo::UNICODE,
             enum CharInfo::Collation collation = CharInfo::DefaultCollation,
             enum CharInfo::Coercibility coercibility = CharInfo::COERCIBLE,
             NAMemory *outHeap = CmpCommon::statementHeap(),
  ...
  ConstValue(NAString strval, NAWString wstrval, enum CharInfo::Collation collation = CharInfo::DefaultCollation,
             enum CharInfo::Coercibility coercibility = CharInfo::COERCIBLE,
             NAMemory *outHeap = CmpCommon::statementHeap());
  ...
  // or excluding NULL)
  ConstValue(const NAType *type, const NABoolean wantMinValue, const NABoolean allowNull,
             NAMemory *outHeap = CmpCommon::statementHeap());

  ConstValue(OperatorTypeEnum otype, ConstValue *other, NAMemory *outHeap = CmpCommon::statementHeap());
  ...
  void setWasDefaultSpec() {
    if (isNull_) isNull_ = IS_NULL_WAS_DEFAULT;
  }
  ...
  void getOffsetsInBuffer(int &nullIndOffset, int &vcLenOffset, int &dataOffset);
 ...
 private:
  void initCharConstValue(const NAString &, enum CharInfo::CharSet charSet, enum CharInfo::Collation collation,
                          enum CharInfo::Coercibility coercibility, NABoolean isCaseInSensitive = FALSE,
                          NAMemory *outHeap = CmpCommon::statementHeap());

  void initCharConstValue(const NAWString &, enum CharInfo::CharSet charSet, enum CharInfo::Collation collation,
                          enum CharInfo::Coercibility coercibility, NABoolean isCaseInSensitive = FALSE,
                          NAMemory *outHeap = CmpCommon::statementHeap(),
                          enum CharInfo::CharSet strLitPrefixCharSet = CharInfo::UnknownCharSet);
...
class SystemLiteral : public ConstValue {
 public:
  ...
  SystemLiteral(const NAWString &strval, enum CharInfo::CharSet charSet = CharInfo::UNICODE,
                enum CharInfo::Collation collation = CharInfo::DefaultCollation,
                enum CharInfo::Coercibility coercibility = CharInfo::COERCIBLE)
      : ConstValue(strval, charSet, collation, coercibility) {
    isSystemSupplied_ = TRUE;
  ...
  SystemLiteral(const NAString &strval, const NAWString &wstrval,
                enum CharInfo::Collation collation = CharInfo::DefaultCollation,
                enum CharInfo::Coercibility coercibility = CharInfo::COERCIBLE)
  ...
  SystemLiteral(const NAType *type, const NABoolean wantMinValue, const NABoolean allowNull)
...
class DefaultSpecification : public ItemExpr {
 public:
  DefaultSpecification() : ItemExpr(ITM_DEFAULT_SPECIFICATION) {}
...
class HostVar : public ItemExpr {
 public:
  ...
  HostVar(const NAString &varName, const NAType *type, NABoolean isSystemGen = FALSE)
      : ItemExpr(ITM_HOSTVAR),
        varName_(varName, CmpCommon::statementHeap()),
        varType_(type),
        isSystemGenerated_(isSystemGen),
        isEnvVar_(FALSE),
        isDefine_(FALSE),
        isParam_(FALSE),
        isCachedParam_(FALSE),
  ...
  HostVar(OperatorTypeEnum otype, const NAString &varName, const NAType *type, NABoolean isSystemGen = FALSE)
      : ItemExpr(otype),
        varName_(varName, CmpCommon::statementHeap()),
        varType_(type),
        isSystemGenerated_(isSystemGen),
  ...
  HostVar(const NAString &varName, const NAString &indicatorName, const NAType *type, NABoolean isSystemGen = FALSE)
      : ItemExpr(ITM_HOSTVAR),
        varName_(varName, CmpCommon::statementHeap()),
        varType_(type),
        isSystemGenerated_(isSystemGen),
  ...
  // copy ctor
  HostVar(const HostVar &, NAMemory *h = CmpCommon::statementHeap());
  ...
  void setIsCachedParam(NABoolean isCachedParam = TRUE) { isCachedParam_ = isCachedParam; }

  NABoolean isSystemGeneratedOutputHV() const;
  NABoolean isSystemGeneratedMinMaxHV() const;
  NABoolean isSystemGeneratedRangeOfValuesHV() const;

  // extract the filter id from the range of values HV
  Int16 extractFilterId();
  ...
  void setSpecialType(ExtendedQualName::SpecialTableType type) { specialSyntaxType_ = type; }
  ExtendedQualName::SpecialTableType getSpecialType() const { return specialSyntaxType_; }
  ...
  inline void setUseTotalRowsetSize() { rowsetInfo_ |= USE_TOTAL_ROWSET_SIZE; }

  inline void setHVRowsetForInputSize() { rowsetInfo_ |= HV_ROWSET_FOR_INPUT_SIZE; }

  inline void setHVRowsetForOutputSize() { rowsetInfo_ |= HV_ROWSET_FOR_OUTPUT_SIZE; }

  inline void setHVRowsetLocalSize() { rowsetInfo_ |= HV_ROWSET_LOCAL_SIZE; }

  inline void setHVInputAssignment() { rowsetInfo_ |= HV_INPUT_ASSIGNMENT; }

  inline NABoolean isHVInputAssignment() { return ((rowsetInfo_ & HV_INPUT_ASSIGNMENT) != 0); }
  ...
  void setParamHeading(NAString head) { heading_ = head; }
...
class Parameter : public ItemExpr {
 public:
  // constructor
  Parameter(OperatorTypeEnum otype) : ItemExpr(otype) {}

  // copy ctor
  Parameter(const Parameter &orig) : ItemExpr(orig) {}
...
class ConstantParameter : public Parameter {
 ...
 public:
  ...
  ConstantParameter(const ConstValue &v, NAMemory *h, NABoolean quantizeLen = FALSE, UInt32 p = 0);
...
struct SelParamType {
  ...
  SelParamType(NAType *t, const Selectivity &s, PositionSet *r) : sel_(s), type_(t), posns_(r) {}
};
...
class SelParameter : public ConstantParameter {
 ...
 public:
  ...
  SelParameter(ConstValue *v, NAMemory *h, const NAType *typ, const Selectivity s, UInt32 p)
      : ConstantParameter(v, h, typ, p), sel_(s) {}

  // copy constructor
  SelParameter(const SelParameter &cp, NAHeap *h) : ConstantParameter(cp, h), sel_(cp.sel_) {}
...
class DynamicParam : public Parameter {
 public:
  ...
  DynamicParam(const NAString &paramName, const NAString &indicatorName, CollHeap *h = 0)
      : Parameter(ITM_DYN_PARAM),
        paramName_(paramName, h),
  ...
  DynamicParam(const DynamicParam &orig, CollHeap *h = 0)
      : Parameter(orig),
        ...
        lobInlinedDataMaxLen_(orig.lobInlinedDataMaxLen_),
        lobChunkMaxLen_(orig.lobChunkMaxLen_) {}
  ...
  const NAString &getIndicatorName() const { return indicatorName_; }
  ...
  inline void setDPRowsetForInputSize() { rowsetInfo_ |= HostVar::HV_ROWSET_FOR_INPUT_SIZE; }

  inline NABoolean isDPRowsetForInputSize() { return ((rowsetInfo_ & HostVar::HV_ROWSET_FOR_INPUT_SIZE) != 0); }

  inline void setRowwiseRowsetInputMaxRowlen() { rowsetInfo_ |= HostVar::HV_ROWWISE_ROWSET_INPUT_ROWLEN; }

  inline NABoolean isRowwiseRowsetInputMaxRowlen() {
    return ((rowsetInfo_ & HostVar::HV_ROWWISE_ROWSET_INPUT_ROWLEN) != 0);
  }

  inline void setRowwiseRowsetInputBuffer() { rowsetInfo_ |= HostVar::HV_ROWWISE_ROWSET_INPUT_BUFFER; }

  inline NABoolean isRowwiseRowsetInputBuffer() {
    return ((rowsetInfo_ & HostVar::HV_ROWWISE_ROWSET_INPUT_BUFFER) != 0);
  }

  inline void setRowwiseRowsetPartnNum() { rowsetInfo_ |= HostVar::HV_ROWWISE_ROWSET_PARTN_NUM; }

  inline NABoolean isRowwiseRowsetPartnNum() { return ((rowsetInfo_ & HostVar::HV_ROWWISE_ROWSET_PARTN_NUM) != 0); }

  inline void setRowInRowwiseRowset() { rowsetInfo_ |= HostVar::HV_ROW_IN_ROWWISE_ROWSET; }
...
class RoutineParam : public Parameter {
 public:
  ...
  void setName(NAString &name) { paramName_ = name; }

  const NAType *getType() const { return paramType_; }

  void setType(NAType *type) { paramType_ = type; }

  const char *getArgType() const { return argumentType_; }

  void setArgType(char *type) { strncpy(argumentType_, type, 3); }
  const NABoolean optionalParam() const { return optionalParam_; }

  void setOptionalParam(NABoolean optional) { optionalParam_ = optional; }

  // does this entire ItemExpr qualify query to be cacheable after this phase?
  virtual NABoolean isCacheableExpr(CacheWA &cwa) { return isCacheable_; }

  virtual NABoolean isAUserSuppliedInput() const { return FALSE; }

  void setIsCacheableExpr(NABoolean cacheable) { isCacheable_ = cacheable; }
  ...
  void setRoutineDesc(RoutineDesc *rdesc) { rdesc_ = rdesc; }
...
class SelIndex : public ItemExpr {
 public:
  ...
  void setRenamedColNameInGrbyClause(NABoolean val) { renamedColNameInGrbyClause_ = val; }
