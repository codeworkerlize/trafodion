...
class SearchKey : public ScanKey {
 public:
  ...
  //
  // -----------------------------------------------------------------------
  SearchKey(const ValueIdList &keyColumns, const ValueIdList &orderOfKeyValues, const ValueIdSet &externalInputs,
            const NABoolean forwardSearch, ValueIdSet &setOfPredicates, const ValueIdSet &nonKeyColumnSet,
            const IndexDesc *indexDesc, const IndexDesc *UDindexDesc = NULL);
  ...
  // -----------------------------------------------------------------------
  SearchKey(const ValueIdList &keyColumns, const ValueIdList &orderOfKeyValues, const ValueIdSet &externalInputs,
            const NABoolean forwardSearch, ValueIdSet &setOfPredicates, const Disjuncts &associatedDisjuncts,
  ...
  // -----------------------------------------------------------------------
  SearchKey(const ValueIdList &keyColumns, const ValueIdList &orderOfKeyValues, const ValueIdSet &externalInputs,
            const NABoolean forwardSearch, ValueIdSet &setOfPredicates, const RangePartitioningFunction *rpf,
            const ValueIdSet &nonKeyColumnSet, const IndexDesc *indexDesc, const IndexDesc *UDindexDesc = NULL);
  ...
  // -----------------------------------------------------------------------
  SearchKey(const ValueIdList &keyColumns, const ValueIdList &orderOfKeyValues, const ValueIdSet &externalInputs,
            ValueIdSet &setOfPredicates, const TableHashPartitioningFunction *hpf, const ValueIdSet &nonKeyColumnSet,
            const IndexDesc *indexDesc, const IndexDesc *UDindexDesc = NULL);
  ...
  // -----------------------------------------------------------------------
  SearchKey(const ValueIdList &keyColumns, const ValueIdList &orderOfKeyValues, const ValueIdSet &externalInputs,
            ValueIdSet &setOfPredicates, const RoundRobinPartitioningFunction *hpf, const ValueIdSet &nonKeyColumnSet,
            const IndexDesc *indexDesc, const IndexDesc *UDindexDesc = NULL);

  // -----------------------------------------------------------------------
  // Code common to SearchKey constructors is put in the init method:
  // -----------------------------------------------------------------------
  void init(const ValueIdList &keyColumns, const ValueIdList &orderOfKeyValues, const ValueIdSet &externalInputs,
            const NABoolean forwardSearch, ValueIdSet &setOfPredicates);

  // -----------------------------------------------------------------------
  // Method for accessing the lower and upper bound values.
  // -----------------------------------------------------------------------
  const ValueIdList &getBeginKeyValues() const { return beginKeyValues_; }
  const ValueIdList &getEndKeyValues() const { return endKeyValues_; }

  const ValueIdList &getRowidPredicate() const { return rowidPredicate_; }
  const ValueIdList &getRowidCols() const { return rowidCols_; }
  ...
  NABoolean areKeyValuesEqualTo(const SearchKey *other, CollIndex n) const;
  ...
  // -----------------------------------------------------------------------
  ValueId computeMissingKeyValue(ValueId keyColumn, NABoolean wantMinValue);

  // -----------------------------------------------------------------------
  // Method for accessing the key predicates.
  // -----------------------------------------------------------------------
  const ValueIdSet &getKeyPredicates() const { return keyPredicates_; }
  ...
  void getKeyPredicatesByColumn(ColumnOrderList &colOrdLis, CollIndex disjunctNumber = 0) const;
  ...
  void getKeyPredicates(ValueIdSet &keyPredicates, /* out */
                        NABoolean *allKeyPredicates = NULL, CollIndex disjunctNumber = 0) const;

  void getRowidPredicates();
  ...
  virtual void replaceBegEndPivs(ValueIdSet &oldPivs, ValueIdSet &newPivs);

  inline NABoolean valuesArePartitionRange() const { return valuesArePartitionRange_; }
  ...
  void print(FILE *ofd = stdout, const char *indent = DEFAULT_INDENT, const char *title = "SearchKey") const;

  bool isFullScanPresent() {  // if we retrieved values twice for each entry, this is a full scan
 ...
 protected:
  void computeCoveredLeadingKeys(NABoolean &isCoveredColumnUnique);
 ...
 private:
  ...
  // Rowid Predicate
  // -----------------------------------------------------------------------
  ValueIdList rowidPredicate_;

  // -----------------------------------------------------------------------
  // For rowId and value pairs appearing in the query
  // that are relevant to this search key.
  // -----------------------------------------------------------------------
  ValueIdList rowidCols_;
  ValueIdList rowidValues_;
...
class SearchKeyBounds : public NABasicObject {
 public:
  ...
  // Default constructor
  // -----------------------------------------------------------------------
  SearchKeyBounds(const ValueId &keyId, NABoolean ascDescFlag, NABoolean forwardReverseFlag, const SearchKey &searchKey)
      : keyColumnId_(keyId),
        ascendingOrder_(ascDescFlag),
        forwardSearch_(forwardReverseFlag),
        beginKeyBoundType_(BOUND_TYPE_NOT_SET),
        beginKeyPredicate_(NULL_VALUE_ID),
        beginKeyValue_(NULL_VALUE_ID),
        beginValueIsExclusive_(FALSE),
        endKeyBoundType_(BOUND_TYPE_NOT_SET),
        endKeyPredicate_(NULL_VALUE_ID),
        endKeyValue_(NULL_VALUE_ID),
  ...
  // -----------------------------------------------------------------------
  enum BoundType { BOUND_TYPE_NOT_SET, BOUND_TYPE_LOWER, BOUND_TYPE_EQUAL, BOUND_TYPE_UPPER };
  ...
  void setIndexOrderToAscending(const NABoolean indexOrder) { ascendingOrder_ = indexOrder; }

  void setSearchDirection(const NABoolean forwardOrReverse) { forwardSearch_ = forwardOrReverse; }
  ...
  //
  // -----------------------------------------------------------------------
  void analyzeSearchPredicates(const ValueIdSet &setOfPredicates, const ValueIdSet &externalInputs);

  // -----------------------------------------------------------------------
  // SearchKeyBounds::computeBeginKeyAndEndKeyValues()
  // -----------------------------------------------------------------------
  void computeBeginKeyAndEndKeyValues(ValueIdSet &setOfKeyPredicates, ValueIdSet &setOfNonKeyPredicates,
                                      NABoolean &previousPredsAreEqualPreds, NABoolean &beginKeyIsExclusive,
                                      NABoolean &endKeyIsExclusive, ValueId &beginKeyExclusionExpr,
                                      ValueId &endKeyExclusionExpr, NABoolean &beginKeyMissing,
                                      NABoolean &endKeyMissing, NABoolean &allchosenPredsAreEqualPreds);
  ...
  NABoolean isEqualPredicate();
  NAColumn *getNAColumnForKeyColumn();
  ...
  void print(FILE *ofd = stdout, const char *indent = DEFAULT_INDENT, const char *title = "SearchKeyBounds") const;
 ...
 private:
  ...
  // -----------------------------------------------------------------------
  void applyABoundForTheSearch(const ValueId &predId, const ValueId &referencedInput,
  ...
  // -----------------------------------------------------------------------
  BoundType getBoundType(const ValueId &predId, const ValueId &referencedInput) const;
  ...
  // -----------------------------------------------------------------------
  void setBeginKeyBoundType(BoundType boundType) { beginKeyBoundType_ = boundType; }
  void setBeginKeyPredicate(const ValueId &predId) { beginKeyPredicate_ = predId; }
  void setBeginKeyValue(const ValueId &exprId) { beginKeyValue_ = exprId; }

  void setEndKeyBoundType(BoundType boundType) { endKeyBoundType_ = boundType; }
  void setEndKeyPredicate(const ValueId &predId) { endKeyPredicate_ = predId; }
  void setEndKeyValue(const ValueId &exprId) { endKeyValue_ = exprId; }

  // -----------------------------------------------------------------------
  // Methods for accessing the bound type.
  // -----------------------------------------------------------------------
  NABoolean isBeginKeyChosen() const { return (beginKeyBoundType_ != BOUND_TYPE_NOT_SET); }
  NABoolean isEndKeyChosen() const { return (endKeyBoundType_ != BOUND_TYPE_NOT_SET); }
  ...
  // -----------------------------------------------------------------------
  NABoolean isBeginKeyPredicateChosen() const { return (beginKeyPredicate_ != NULL_VALUE_ID); }
  NABoolean isEndKeyPredicateChosen() const { return (endKeyPredicate_ != NULL_VALUE_ID); }
  ...
  // -----------------------------------------------------------------------
  NABoolean isBeginKeyValueChosen() const { return (beginKeyValue_ != NULL_VALUE_ID); }
  NABoolean isEndKeyValueChosen() const { return (endKeyValue_ != NULL_VALUE_ID); }
...
class SearchKeyWorkSpace : public NABasicObject {
 public:
  ...
  //
  // ----------------------------------------------------------------------
  SearchKeyWorkSpace(const ValueIdList &keyColumns, const ValueIdList &indexOrder, const NABoolean forwardSearch,
  ...
  //
  // ----------------------------------------------------------------------
  void computeBeginKeyAndEndKeyValues(const ValueIdList &keyColumns, const ValueIdSet &externalInputs,
                                      ValueIdList &beginKeyValues, ValueIdList &endKeyValues,
                                      ValueIdSet &setOfKeyPredicates, ValueIdSet &setOfNonKeyPredicates,
                                      NABoolean &keyPredicatesUnique, NABoolean &beginKeyIsExclusive,
                                      NABoolean &endKeyIsExclusive, ValueId &beginKeyExclusionExpr,
 ...
 private:
  void handleMultiColumnRangePred(NABoolean isBeginKey, CollIndex keyNum, ValueIdList &boundaryValues);
  // ----------------------------------------------------------------------
...
class orcPushdownPredicate {
 public:
  orcPushdownPredicate(CollHeap *oHeap = CmpCommon::statementHeap()) : operands_(oHeap){};

  ~orcPushdownPredicate(){};

 protected:
  //   ORC_SearchArgumentType opType_;
  ExtPushdownOperatorType opType_;
  LIST(ValueId) operands_;
};

class HbaseSearchKey : public SearchKey {
 public:
  HbaseSearchKey(const ValueIdList &keyColumns, const ValueIdList &orderOfKeyValues, const ValueIdSet &externalInputs,
                 const NABoolean forwardSearch, ValueIdSet &setOfPredicates, const ValueIdSet &nonKeyColumnSet,
                 const IndexDesc *indexDesc, const IndexDesc *UDIndexDesc, const ValueIdSet &outputsByHbaseScan);
  ...
  static void makeHBaseSearchKeys(const SearchKey *builtinSearchKey, const ValueIdList &keyColumns,
                                  const ValueIdList &orderOfKeyValues, const ValueIdSet &externalInputs,
                                  const NABoolean forwardSearch, ValueIdSet &setOfPredicates,
                                  const ValueIdSet &nonKeyColumnSet, const IndexDesc *indexDesc,
                                  const ValueIdSet &outputsByHbaseScan, LIST(HbaseSearchKey *) & producedKeys);
