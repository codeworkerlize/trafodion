...
class MVCandidateException : public QRException {
 public:
   ...
   */
  MVCandidateException(const char *msgTemplate...) : QRException() { qrBuildMessage(msgTemplate, msgBuffer_); }
...
class MVCandidates {
 public:
   ...
   */
  MVCandidates(RelRoot *root, QRQueryDescriptorPtr queryDesc, QRDescGenerator &descGenerator);
   ...
   */
  NABoolean isAfavoriteMV(const NAString &mvName) { return mvqrFavoriteCandidates_->contains(mvName); }
 ...
 private:
   ...
   *         of the MV.
   */
  GroupByAgg *getGroupByAggNode(RelExpr *childNode, QRGroupByPtr groupBy, QRCandidatePtr candidate, CollHeap *heap);

  void getEqSetMembers(VEGPredicate *vegPred, ValueIdSet &vegMembers);

  ItemExpr *rewriteVegPredicate(const ElementPtrList &mvColumns, VEGPredicate *itemExpr, QRCandidatePtr candidate,
                                CollHeap *heap);
  // CollIndex tblId);
  /**
   ...
   */
  ItemExpr *rewriteItemExpr(QRElementPtr mvCol, ItemExpr *itemExpr, QRCandidatePtr candidate, CollHeap *heap);
   ...
   */
  ItemExpr *rewriteItemExpr(const ElementPtrList &mvColumns, ItemExpr *itemExpr, QRCandidatePtr candidate,
                            CollHeap *heap, CollIndex tblId = 0);
   ...
   * @param heap Heap to use for any allocations.
   */
  void rewriteRangePreds(QRCandidatePtr candidate, RelExpr *scan, RelExpr *groupBy, CollHeap *heap);
   ...
   * @param heap Heap to use for any allocations.
   */
  void rewriteResidPreds(QRCandidatePtr candidate, RelExpr *scan, RelExpr *groupBy, RelExpr *bjRoot, CollHeap *heap);
   ...
   */
  ColRefName *getColRefName(QRCandidatePtr candidate, QRElementPtr elem, CollHeap *heap);
   ...
   *         id from the query.
   */
  ValueId getRewriteVid(RETDesc *retDesc, QRElementPtr elem, QRCandidatePtr candidate, CollHeap *heap);
   ...
   */
  void populateVidMap(RelExpr *root, ElementPtrList &vidMapTopElements, ValueIdSet &vegRewriteVids,
   ...
   * @see #buildOutputExprs Creates the list of output elements used here.
   */
  void populateOneVidMap(RelExpr *root, ValueIdSet &vegRewriteVids, QRElementPtr elem, QRCandidatePtr candidate,
   ...
   */
  template <class T>
  void addPredsFromList(QRCandidatePtr candidate, Scan *scan, CANodeId tableNodeId, const NAPtrList<T> &list,
                        CollHeap *heap);
   ...
   * @param heap Heap to use for any allocations.
   */
  void addIGBPreds(QRCandidatePtr candidate, Scan *scan, CANodeId tableNodeId, QRJBBPtr jbb, CollHeap *heap);
   ...
   * @return
   */
  ItemExpr *getIGBJoinCondOp(QRElementPtr elem, QRCandidatePtr candidate, CANodeIdSet &jbbSubsetNodes,
   ...
   * @param heap Heap to use for any allocations.
   */
  void addIGBDimJoins(QRCandidatePtr candidate, CANodeIdSet &jbbSubsetNodes, QRJBBPtr jbbElem, RelExpr *&node,
                      NodeIdScanHash &dimScanHash, CollHeap *heap);
   ...
   */
  void mapDimOutputs(QRJBBPtr jbbElem, CANodeIdSet &jbbSubsetNodes, NodeIdScanHash &dimScanHash, ValueIdMap &vidMap,
                     CollHeap *heap);
   ...
   * @param heap Heap to use for any allocations.
   */
  void addBackJoins(QRCandidatePtr candidate, QRTableListPtr bjTables, RelExpr *&node, CollHeap *heap);
   ...
   */
  void buildOutputExprs(QRCandidatePtr candidate, RelRoot *root, ElementPtrList &topValElemList, CollHeap *heap);
   ...
   * @param heap Heap to use for any allocations.
   */
  void analyzeCandidate(QRCandidatePtr candidate, JBBSubset &jbbSubsetToReplace, NABoolean testRewriteSingle,
                        CANodeIdSet &jbbSubsetNodes, CollHeap *heap);
  /**
...
class MVMatch : public NAIntrusiveSharedPtrObject {
 public:
  MVMatch(JBBSubset *equivJbbSubset, ADD_MEMCHECK_ARGS_DECL(CollHeap *heap = NULL))
  ...
  JBBSubset *getEquivalentJbbSubset() const { return equivalentJbbSubset_; }

  void setEquivalentJbbSubset(JBBSubset *jbbSubset) { equivalentJbbSubset_ = jbbSubset; }

  const NAString &getMvName() const { return mvName_; }

  void setMvName(const NAString &mvName) { mvName_ = mvName; }

  RelExpr *getMvRelExprTree() const { return mvRelExprTree_; }

  void setMvRelExprTree(RelExpr *expr) { mvRelExprTree_ = expr; }

  NABoolean alreadyOptimized() { return alreadyOptimzed_; }

  void setAlreadyOptimized() { alreadyOptimzed_ = TRUE; }
