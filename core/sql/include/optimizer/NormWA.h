 ...
 public:
  ...
  SqoChangedRelExprs(RegionId vegRegion, SqoChangedRelExprEnum what, RelExpr *changedExpr, RelExpr *origExpr,
      ...
      : vregion_(vegRegion),
        ...
        originalRelExpr_(origExpr),
        origChildId_(origChildId),
        changedChildId_(changedChildId),
        subqId_(subqId) {}
...
class SqoWA : public NABasicObject {
 public:
  // Constructors
  SqoWA() : changedItemExprs_(CmpCommon::statementHeap()), changedRelExprs_(CmpCommon::statementHeap()), subqId_(0) {}

  SqoWA(const SqoWA &other)
      : changedItemExprs_(other.changedItemExprs_), changedRelExprs_(other.changedRelExprs_), subqId_(other.subqId_) {}
  ...
  void insertChangedItemExpr(ValueId changedId, SqoChangedItemExprEnum whatChanged, ItemExpr *origExpr) {
    changedItemExprs_.insert(new (CmpCommon::statementHeap())
                                 SqoChangedItemExprs(changedId, whatChanged, origExpr, subqId_));
  }
...
class NormWA : public NABasicObject {
 public:
  ...
  NormWA(CmpContext *cmpContext)
      : currentCmpContext_(cmpContext),
        subqUnderExprTreeCount_(0),
        walkingAnExprTreeCount_(0),
        complexScalarExprCount_(0),
        nullCount_(0),
        notCount_(0),
        orCount_(0),
        dontAddRedundantPredicatesCount_(0),
        inConstraintsCount_(0),
        ...
        createPredicateForFirstN_(FALSE),
  ...
  NormWA(const NormWA &other)
      : currentCmpContext_(other.currentCmpContext_),
        ...
        dontAddRedundantPredicatesCount_(other.dontAddRedundantPredicatesCount_),
        ...
        createPredicateForFirstN_(other.createPredicateForFirstN_),
  ...
  void clearStateInformation() {
    walkingAnExprTreeCount_ = complexScalarExprCount_ = nullCount_ = notCount_ = orCount_ =
        dontAddRedundantPredicatesCount_ = inConstraintsCount_ = 0;
  ...
  // --------------------------------------------------------------------
  void addVEG(const ValueId &expr1Id, const ValueId &expr2Id) { vegTable_->addVEG(expr1Id, expr2Id); }

  void addVEG(const ValueIdSet &setOfValues) { vegTable_->addVEG(setOfValues); }

  void deleteVEGMember(const ValueId &vId) { vegTable_->deleteVEGMember(vId); }
  ...
  //-----------------------------------------------------------------------
  VEGRegion *locateVEGRegion(ExprNode *ownerENptr) { return vegTable_->locateVEGRegion(ownerENptr); }
  ...
  // --------------------------------------------------------------------
  ItemExpr *performTC(const ValueId &vegMember) const { return vegTable_->performTC(vegMember); }
  ...
  // --------------------------------------------------------------------
  // The "don't add redundant predicates" flag is set when an OR is
  // encountered and we have copied up a set of predicates from the
  // legs of an OR to create a redundant top-level OR predicate.
  //
  // Adding redundant predicates is useful when
  // we have an OR join predicate and the legs select a set of values
  // from a particular column; the resulting IN predicate can be
  // pushed down to a scan while the original OR cannot, for example:
  //
  //   select * from t1 join t2 on (t1.a = 5 or t2.b = 6) and
  //                               (t1.a = 4 or t2.b = 7)
  // This can be rewritten as:
  //   select * from t1 join t2 on ((t1.a = 4) or (t1.a = 5)) and
  //                               ((t2.b = 6) or (t2.b = 7)) and
  //                               (t1.a = 5 or t2.b = 6) and
  //                               (t1.a = 4 or t2.b = 7)
  //
  // The first and second conjuncts are added redundant predicates
  // (they are implied by the original conjuncts but are not equivalent
  // to them); and the first and second conjuncts can be pushed down
  // to the t1 and t2 scans respectively.
  //
  // This transformation is not idempotent, however; if we perform
  // the transformation a second time on the transformed tree we'll
  // get duplicates of the first two conjuncts. To prevent such
  // behavior, we have this flag to suppress the transformation on
  // an already-transformed tree.
  // --------------------------------------------------------------------
  void setDontAddRedundantPredicatesFlag() { dontAddRedundantPredicatesCount_++; }
  void restoreDontAddRedundantPredicatesFlag() { dontAddRedundantPredicatesCount_--; }
  NABoolean dontAddRedundantPredicatesFlag() const { return dontAddRedundantPredicatesCount_ > 0; }
  ...
  NABoolean requiresLeftTSJLinearization() { return leftJoinConversionCount_ > 1; }
  ...
  NABoolean requiresSemanticQueryOptimization() {
    return ((correlatedSubqCount_ > 0) || containsJoinsToBeEliminated_ || checkForExtraHubTables_ ||
            containsGroupBysToBeEliminated_ || containsSemiJoinsToBeTransformed_ || commonSubExprCount_ > 0 ||
            createPredicateForFirstN_ || intersectsToXformCount_ > 0);
  }
  ...
  NABoolean containsJoinsToBeEliminated() { return containsJoinsToBeEliminated_; }
  void setContainsJoinsToBeEliminated(NABoolean val) { containsJoinsToBeEliminated_ = val; }

  NABoolean checkForExtraHubTables() { return checkForExtraHubTables_; }
  void setCheckForExtraHubTables(NABoolean val) { checkForExtraHubTables_ = val; }

  NABoolean containsGroupBysToBeEliminated() { return containsGroupBysToBeEliminated_; }
  void setContainsGroupBysToBeEliminated(NABoolean val) { containsGroupBysToBeEliminated_ = val; }

  NABoolean containsSemiJoinsToBeTransformed() { return containsSemiJoinsToBeTransformed_; }
  void setContainsSemiJoinsToBeTransformed(NABoolean val) { containsSemiJoinsToBeTransformed_ = val; }
  ...
  void setCreatePredicateForFirstN(NABoolean val) { createPredicateForFirstN_ = val; }
 ...
 private:
  ...
  NABoolean createPredicateForFirstN_;
