...
class Intersect : public RelExpr {
 public:
  // constructor
  Intersect(RelExpr *leftChild, RelExpr *rightChild);
...
class Except : public RelExpr {
 public:
  // constructor
  Except(RelExpr *leftChild, RelExpr *rightChild);
...
class Union : public RelExpr {
 public:
  // constructor
  Union(RelExpr *leftChild, RelExpr *rightChild, UnionMap *colMapTable = NULL, ItemExpr *condExpr = NULL,
        OperatorTypeEnum otype = REL_UNION, CollHeap *outHeap = CmpCommon::statementHeap(),
        NABoolean sysGenerated = FALSE, NABoolean mayBeCacheable = TRUE);
  ...
  // ---------------------------------------------------------------------
  NABoolean rppAreCompatibleWithOperator(const ReqdPhysicalProperty *const rppForUnion) const;
  ...
  enum UnionFlags {
    ...
    UNION_SYSTEMCOL = 0x080           // output syskey in union RETdesc;
  };
  ...
  // different, constant value for each child, like in this union
  // where the first column is a label:
  // select 'left', a from t1 union all select 'right', b from t2
  NABoolean isALabeledUnion() const;
  ...
  void setTrigExceptExpr(ValueIdList trigExceptExpr) { trigExceptExpr_ = trigExceptExpr; }
 ...
 private:
  // controlFlags were added for trigger project
  enum controlFlags { NO_OUTPUTS = 0x0001, IN_NOT_ATOMIC_STMT = 0x0002, IS_TEMPORARY = 0x0004 };
  ...
  void dumpChildrensRETDescs(const RETDesc &leftTable, const RETDesc &rightTable);
...
class MergeUnion : public Union {
 public:
  // constructor
  MergeUnion(RelExpr *leftChild, RelExpr *rightChild, UnionMap *unionMap = NULL,
  ...
  void synthUnionSortKeyFromChild(const ValueIdList &childSortKey, const ValueIdSet &reqArrangement,
 ...
 private:
  ...
  CostMethodMergeUnion *pCostMethod_;
