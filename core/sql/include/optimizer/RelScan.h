...
public:
 ...
 // synthesizes compRefOpt constraints.
  ...
  void setPartitionPruned(NABoolean isPartitionPruned)
                                   { isPartitionPruned_ = isPartitionPruned; }
  ...
  NABoolean skipTransactionForCursorUpdDelScanForced() { return (scanFlags_ & SKIP_TRANSACTION_FOR_CURSOR_UPDEL_SCAN_FORCED) != 0; }
  void setSkipTransactionForCursorUpdDelForced(NABoolean v)
  { (v ? scanFlags_ |= SKIP_TRANSACTION_FOR_CURSOR_UPDEL_SCAN_FORCED : scanFlags_ &= ~SKIP_TRANSACTION_FOR_CURSOR_UPDEL_SCAN_FORCED); }
...
public:
 ...
 PhysicalProperty * synthHbaseScanPhysicalProperty(const Context *context,
  ...
  static short createParqTableSchStr(Generator *generator,
                                     NAString &tableName,
                                     const NAColumnArray &hiveColArr,
                                     char* &schStr);
  
  static short createHiveColNameAndTypeLists(
       Generator * generator,
       const NAColumnArray &hiveColArr,
       Queue* &colNameList,
       Queue* &colTypeList);
 ...
 static TrafDesc *createHbaseTableDesc(const char * table_name);
  ...
  const ValueIdSet& getScanPreCond() const
  { return scanPreCond_; }
...
//  IndexDesc * indexDesc()                     { return indexDesc_; }
 ...
 virtual PlanPriority computeOperatorPriority
  ...
  inline void addMinMaxHJColumn(const ValueId &col)
        { minMaxHJColumns_.insert(col); }
  ...
  Exchange* findMergePointer(RelExpr* stop, ValueId colVid, 
                             NABoolean child1IsBroadcasting,
                             NABoolean& espSeen,
                             NABoolean& innerIsOK);

  void setUseParquetCppReader(NABoolean x)
     { useParquetCppReader_ = x; } 

  NABoolean canUseParquetCppReader()
     { return useParquetCppReader_ ; } 
...
public:
  ...
  static int createAsciiColAndCastExprForExtStorage
  (Generator * generator,
...
public:
  ...
  Describe(char * originalQuery,
           ComIdClass typeIDClass,
           const NAString &typeIDName,
           Format format = SHOWDDL_,
           ULng32 flags = 0,
           NABoolean header = TRUE)
    : Scan(REL_DESCRIBE),
  ...
  NABoolean getIsTenant() const
  {
    return (typeIDClass_ == COM_TENANT_CLASS);
  }


  NABoolean getIsUser() const
  {
    return (typeIDClass_ == COM_USER_CLASS);
  }
   
  NABoolean getIsRole() const
  {
    return (typeIDClass_ == COM_ROLE_CLASS);
  }

  NABoolean getIsRGroup() const
  {
    return (typeIDClass_ == COM_RGROUP_CLASS);
  }
  ...
  NABoolean getIsUserGroup() const
  {
     return (typeIDClass_ == COM_USER_GROUP_CLASS);
  }
  ...
  const NAString& getTypeIDName() const
  {
    return typeIDName_;
  }
