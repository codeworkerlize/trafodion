...
class ColumnSet : public ClusteredBitmap {
 public:
  ...
  // copy constructor
  ColumnSet(const ColumnSet &other, NAMemory *heap) : ClusteredBitmap(other, heap) {}
...
class MCSkewedValue : public NABasicObject {
 public:
  ...
  MCSkewedValue(NAWchar *boundary, CostScalar frequency, EncodedValue *eV, UInt32 hash, NAMemory *h = 0)
      : boundary_(boundary), frequency_(frequency), mcEncodedValue_(eV), hash_(hash), heap_(h){};
  ...
  inline NABoolean operator<(const MCSkewedValue &other) const { return (*mcEncodedValue_ < *(other.mcEncodedValue_)); }
  inline NABoolean operator<=(const MCSkewedValue &other) const {
    return (*mcEncodedValue_ <= *(other.mcEncodedValue_));
  }
  inline NABoolean operator>(const MCSkewedValue &other) const { return (*mcEncodedValue_ > *(other.mcEncodedValue_)); }
...
class MCSkewedValueList : public NAList<MCSkewedValue *> {
 public:
  // constructor
  MCSkewedValueList(NAMemory *h = 0) : NAList<MCSkewedValue *>(h), heap_(h){};
  ...
  void mergeMCSkewedValueList(MCSkewedValueList *leftSide, MCSkewedValueList *rightSide,
...
class MultiColumnHistogram : public NABasicObject {
 public:
  MultiColumnHistogram(ColumnSet &columns, CostScalar &uec, CostScalar &rows, ComUID &id,
                       MCSkewedValueList *mcSkewedValueList, ColStatsSharedPtr colStatsPtr, NAMemory *heap)
      : columns_(columns, heap),
        uec_(uec),
        rows_(rows),
        histogramID_(id),
        mcSkewedValueList_(mcSkewedValueList),
        colStatsPtr_(colStatsPtr),
        heap_(heap) {}
...
class MultiColumnHistogramList : public NAList<MultiColumnHistogram *> {
 public:
  // constructor
  MultiColumnHistogramList(NAMemory *heap) : NAList<MultiColumnHistogram *>(heap), heap_(heap) {}
...
class FrequentValue : public NABasicObject {
 public:
  FrequentValue(UInt32 hashValue, CostScalar frequency, CostScalar probability, EncodedValue value);
  ...
  FrequentValue(const EncodedValue &normValue, ConstValue *itemPtr, const NAType *, CostScalar frequency = csOne,
                CostScalar probability = csOne);
  ...
  NABoolean operator>(const FrequentValue &x) const {
    return (encodedValue_.isNullValue() && !x.encodedValue_.isNullValue()) ||
           (encodedValue_ > x.encodedValue_ || (encodedValue_ == x.encodedValue_ && hash_ > x.hash_));
  }
  ...
  void print(FILE *f, const char *prefix, const char *suffix, CollHeap *c = NULL, char *buf = NULL) const;
...
class FrequentValueList : public LIST(FrequentValue) {
 public:
  FrequentValueList(NAMemory *h, CollIndex initLen = 0) : LIST(FrequentValue)(h, initLen){};

  FrequentValueList(const FrequentValueList &fvlist, NAMemory *h) : LIST(FrequentValue)(fvlist, h) {}
  ...
  void scaleFreqAndProbOfFrequentValues(CostScalar freqScale, CostScalar probScale);
  ...
  void mergeFreqFreqValues(FrequentValueList &leftFrequentValueList, FrequentValueList &rightFrequentValueList,
                           CostScalar scaleFactor, MergeType mergeMethod, FrequentValueList *tmpLeftFreqValueList,
                           FrequentValueList *tmpRightFreqValueList);

  void scaleAndAppend(FrequentValueList &rightFrequentValueList, CostScalar adjFreq, CostScalar adjProb,
                      CostScalar scaleFactor);

  NABoolean getfrequentValueIndex(const FrequentValue &, CollIndex &index) const;

  CostScalar freqOfGivenEncodedVal(EncodedValue mfvEV, EncodedValue loBoundary, EncodedValue hiBoundary,
 ...
 public:
  ...
  COMPARE_RESULT compare(const MCboundaryValueList &other) const {
    ...
    for (int i = 0; i < this->entries(); i++) {
      if ((*this)[i].compare(other[i]) == SAME) continue;
      return ((*this)[i].compare(other[i]));
    }
    return (SAME);
  }

  NABoolean operator<(const MCboundaryValueList &other) const { return (this->compare(other) == LESS); }
  ...
  NABoolean operator>(const MCboundaryValueList &other) const { return (this->compare(other) == MORE); }
  NABoolean operator>=(const MCboundaryValueList &other) const {
    COMPARE_RESULT result = this->compare(other);
  ...
  //     vki = v(k-1)i + (hi-li)/n
  //
  void getMinMax(const MCboundaryValueList &lv, const MCboundaryValueList &hv, int numParts,
  ...
  void print(FILE *ofd = stdout, const char *indent = DEFAULT_INDENT, const char *title = "MCboundaryValueList") const;
...
class HistInt {
 ...
 public:
  ...
  HistInt(int intNum, const NAWchar *intBoundary, const NAColumnArray &columns, CostScalar card, CostScalar uec,
          NABoolean boundInc = TRUE, CostScalar card2mfv = 0);

  // copy constructor
  HistInt(const HistInt &other) : MCBoundary_(STMTHEAP) { copy(other); }
 ...
 public:
  ...
  NABoolean operator==(const HistInt &other) const {
    if (this->boundary_.compare(other.boundary_) == SAME && this->boundInc_ == other.boundInc_ &&
        this->MCBoundary_.compare(other.MCBoundary_) == SAME && (this->rows_) == other.rows_ &&
  ...
  inline CostScalar getFudgedUec() const { return (uec_ == csZero ? uec_ : MAXOF(uec_, csOne)); }
  inline NABoolean isNull() const { return boundary_.isNullValue(); }

  // ---------------------------------------------------------------------
  //  Manipulation Methods
  // ---------------------------------------------------------------------
  inline void setBoundary(const EncodedValue &intBound) { boundary_ = intBound; }
  inline void setBoundIncl(NABoolean boundIncl = TRUE) { boundInc_ = boundIncl; }
  ...
  // ----------------------------------------------------------------------
  CostScalar mergeInterval(const HistInt &left, const HistInt &right, CostScalar scaleRowCount,
                           MergeType mergeMethod = INNER_JOIN_MERGE);
  ...
  //  Utility routines
  // -----------------------------------------------------------------------
  void display(FILE *f = stdout, const char *prefix = DEFAULT_INDENT, const char *suffix = "", CollHeap *c = NULL,
               char *buf = NULL) const;
...
class Histogram : public LIST(HistInt) {
 ...
 public:
  ...
  Histogram(const Histogram &hist, NAMemory *h) : LIST(HistInt)(hist, h) {}
 ...
 public:
  ...
  // understand
  HistogramSharedPtr createMergeTemplate(const HistogramSharedPtr &otherHistogram, NABoolean equiMerge) const;
 ...
 public:
  ...
  CollIndex insertSingleValuedInterval(const EncodedValue &value, NABoolean distributeRowsAndUec = TRUE);
  ...
  // of the new Interval.
  void insertZeroInterval(const EncodedValue &loValue, const EncodedValue &hiValue, NABoolean isNewBoundIncluded);
  void insertZeroInterval(const CostScalar &loValue, const CostScalar &hiValue, NABoolean isNewBoundIncluded);

  void insertZeroInterval(const NormValueList &loValue, const NormValueList &hiValue, NABoolean isNewBoundIncluded);
  ...
  void reduceNumHistInts(Criterion reductionCriterion, Source invokedFrom = AFTER_FETCH);
  ...
  void computeExtendedIntRange(Interval &currentInt, Criterion &reductionCriterion, EncodedValue &hiBound,
                               EncodedValue &loBound, NABoolean &hiBoundInclusive, NABoolean &loBoundInclusive);
  ...
  // ----------------------------------------------------------------------------
  void compressHistogramForQueryPreds(ItemExpr *lowerBound, ItemExpr *upperBound, NABoolean hasJoinPred = FALSE);
...
class Interval : public NABasicObject {
 public:
  ...
  //
  Interval() : loIndex_(NULL_COLL_INDEX), hist_(0) {}
  ...
  inline NABoolean operator==(const Interval &other) { return (loIndex_ == other.loIndex_ && hist_ == other.hist_); }
  ...
  NABoolean getMFV(const FrequentValueList &, EncodedValue &mfv, CostScalar &freq);

  NABoolean getMFV(const MCSkewedValueList &, MCboundaryValueList &mfv, CostScalar &freq);

  void getRCSmallerThanMFV(const EncodedValue &mfv, const CostScalar &freqMFV, CostScalar &rc);

  void getRCSmallerThanMFV(const MCboundaryValueList &mfv, const CostScalar &freqMFV, CostScalar &rc);

  void makeSplits(HistogramSharedPtr &newHist, const NAType *nt, const CostScalar newHeight,
  ...
  void makeSplitsForMC(HistogramSharedPtr &newHist, const CostScalar newHeight,
                       CostScalar &newRC,        // On entry: the # to fill;
                                                 // On exit: #rc unfilled
                       CostScalar &availableRC,  // On extry: rc available;
                                                 // On exit: the # of rows remaining in the interval
 ...
 public:
  inline void setRowsAndUec(const CostScalar &rows, const CostScalar &uec);
  ...
  inline CollIndex getLoIndex() const {
    OK();
    return loIndex_;
  }
  ...
  inline NABoolean isValid() const { return ((hist_ != 0) && ((loIndex_ + 2) <= hist_->entries())); }
  ...
  // -----------------------------------------------------------------------
  static void distributeRowsAndUec(LIST(Interval) & spanList, CostScalar rows, CostScalar uecs,
                                   const EncodedValue &loBound, const EncodedValue &hiBound);
  ...
  //  Utility routines
  // -----------------------------------------------------------------------
  void display(FILE *f = stdout, const char *prefix = DEFAULT_INDENT, const char *suffix = "") const;
...
inline NABoolean Interval::isLoBoundInclusive() const { return (NOT((*hist_)[loIndex_]).isBoundIncl()); }

// set the inclusive boundary values

inline void Interval::setLoBoundInclusive(NABoolean value) { ((*hist_)[loIndex_]).setBoundIncl(NOT value); }

inline void Interval::setHiBoundInclusive(NABoolean value) {
...
inline const EncodedValue &Interval::loBound() const { return ((*hist_)[loIndex_]).getBoundary(); }

inline const MCboundaryValueList &Interval::loMCBound() const { return ((*hist_)[loIndex_]).getMCBoundary(); }

// set lo/hi boundaries

inline void Interval::setLoBound(const EncodedValue &newLo) { ((*hist_)[loIndex_]).setBoundary(newLo); }

inline void Interval::setHiBound(const EncodedValue &newHi) {
  HistInt &temp = ((*hist_)[loIndex_ + 1]);
...
inline void Interval::setUec(const CostScalar &value) {
  HistInt &temp = ((*hist_)[loIndex_ + 1]);
  temp.setUec(value);
  hiInt_ = temp;
}

inline void Interval::setRowcount(const CostScalar &value) {
  HistInt &temp = ((*hist_)[loIndex_ + 1]);
  temp.setCardinality(value);
  hiInt_ = temp;
}

inline void Interval::setRowsAndUec(const CostScalar &rows, const CostScalar &uec) {
  HistInt &temp = ((*hist_)[loIndex_ + 1]);
...
inline NABoolean Interval::isFirst() const {
  OK();
  return (loIndex_ == 0);
}

inline NABoolean Interval::isLast() const {
  OK();
  return (loIndex_ + 2 == hist_->entries());
}

// refresh HiInt
inline void Interval::refreshHiInt() { hiInt_ = ((*hist_)[loIndex_ + 1]); }
//
// histogram member functions that interface directly with
// the Interval member functions
//

inline Interval Histogram::getFirstInterval() const {
  if (numIntervals() == 0) return Interval();  // 0 intervals ==> no first exists

  HistogramSharedPtr histPtr = HistogramSharedPtr::getIntrusiveSharedPtr(this);
  return Interval(0, histPtr);
}

inline Interval Histogram::getLastInterval() const {
  if (numIntervals() == 0) return Interval();  // 0 intervals ==> no last exists

  HistogramSharedPtr histPtr = HistogramSharedPtr::getIntrusiveSharedPtr(this);
  return Interval(entries() - 2, histPtr);
}

inline Interval Histogram::getLastNonNullInterval() const {
  if (isNullInstantiated()) {
    if (entries() == 2) return Interval();  // there is only that single, NULL interval
...
inline Interval Histogram::getInterval(CollIndex index) const {
...
const int FAKEHISTOGRAMID = 10000;
...
class ColStats : public NABasicObject {
 ...
 public:
  ...
  static const int USTAT_HISTOGRAM_ID_THRESHOLD = 0x7FFFFFFF;
  ...
  //  Constructors
  // -----------------------------------------------------------------------
  ColStats(ComUID &histid, CostScalar uec = 0, CostScalar rowcount = 0, CostScalar baseRowCount = -1,
           NABoolean unique = FALSE, NABoolean shapeChanged = FALSE, const HistogramSharedPtr &desc = 0,
           NABoolean modified = FALSE, CostScalar rowRedFactor = 1.0, CostScalar uecRedFactor = 1.0,
           int avgVarcharSize = 0, NAMemory *heap = 0, NABoolean allowMinusOne = FALSE);
  ...
  ColStats(const HistogramSharedPtr &hist, NAMemory *h)
    ...
    setExpressionsHistogram(FALSE);
  }
  // deepCopy()
  // Creates a new ColStats and makes a deepCopy of it using other
  static ColStatsSharedPtr deepCopy(const ColStats &other, NAMemory *heap, NABoolean useColumnPositions = FALSE,
                                    NABoolean copyIntervals = TRUE);

  // copy histogram into cache.
  // use "lean" representation of columns.
  static ColStatsSharedPtr deepCopyHistIntoCache(const ColStats &other, NAMemory *heap) {
    return deepCopy(other, heap, TRUE);
  }

  // creates a deep copy of single-column histogram from cache.
  // sets deep copy's column to col.
  static ColStatsSharedPtr deepCopySingleColHistFromCache(const ColStats &other, NAColumn &col, NAMemory *heap,
 ...
 public:
  ...
  inline const NABoolean isSingleIntHist() { return getHistogram() ? (getHistogram()->entries() <= 2) : TRUE; }
  ...
  inline NABoolean isExpressionHistogram() const { return (_flags_ & _expressionsHistogram_) != 0; }

  int getAvgVarcharSize() const { return avgVarcharSize_; };
  // ---------------------------------------------------------------------
  //  Mutator functions
  // ---------------------------------------------------------------------
  ...
  NABoolean mergeFrequentValues(ColStatsSharedPtr &otherStats, NABoolean scaleFreq = TRUE,
                                MergeType mergeMethod = INNER_JOIN_MERGE, NABoolean adjRowCount = FALSE);

  NABoolean getTotalFreqInfoForIntervalWithValue(EncodedValue mfvEV, CostScalar &totalMfvRc, CostScalar &mfvCnt);
  ...
  // setMinValue
  inline void setMinValue(const NAWchar *theValue) { minValue_ = EncodedValue(theValue, getStatColumns()); }
  inline void setMinValue(const EncodedValue &minValue) { minValue_ = minValue; }
  // setMaxValue
  inline void setMaxValue(const NAWchar *theValue) { maxValue_ = EncodedValue(theValue, getStatColumns()); }
  ...
  void setScaleFactor(CostScalar scale) { scaleFactor_ = scale; }
  ...
  inline void setMaxIntervalCount(CollIndex number) { maxIntervalCount_ = MAXOF(number, 2); }
 ...
 public:
  ...
  // predicate to it.
  void setUecBeforePred(CostScalar value) { uecBeforePred_ = value; }
  ...
  inline void setFakeHistogram(NABoolean flag = TRUE) {
    flag ? _flags_ |= _isFakeHistogram_ : _flags_ &= ~_isFakeHistogram_;
  }
  inline void setObsoleteHistogram(NABoolean flag = TRUE) {
    flag ? _flags_ |= _isObsoleteHistogram_ : _flags_ &= ~_isObsoleteHistogram_;
  }
  inline void setOrigFakeHist(NABoolean flag = TRUE) {
    flag ? _flags_ |= _isOrigFakeHist_ : _flags_ &= ~_isOrigFakeHist_;
  }
  ...
  inline void setUpStatsNeeded(NABoolean flag) {
    flag ? _flags_ |= _updateStatsNeeded_ : _flags_ &= ~_updateStatsNeeded_;
  }
  inline void setVirtualColForHist(NABoolean flag = TRUE) {
    flag ? _flags_ |= _virtualColForHist_ : _flags_ &= ~_virtualColForHist_;
  }
  inline void setAlmostUnique(NABoolean flag) { flag ? _flags_ |= _almostUnique_ : _flags_ &= ~_almostUnique_; }
  inline void setSmallSampleHistogram(NABoolean flag = TRUE) {
    flag ? _flags_ |= _isSmallSampleHistogram_ : _flags_ &= ~_isSmallSampleHistogram_;
  }
  inline void setIsCompressed(NABoolean flag = TRUE) { flag ? _flags_ |= _isCompressed_ : _flags_ &= ~_isCompressed_; }
  inline void setIsARollingColumn(NABoolean flag = TRUE) {
    flag ? _flags_ |= _isARollingColumn_ : _flags_ &= ~_isARollingColumn_;
  }
  inline void setIsColWithBndryConflict(NABoolean flag = TRUE) {
    flag ? _flags_ |= _isColWithBndryConflict_ : _flags_ &= ~_isColWithBndryConflict_;
  }
  inline void setSelectivitySetUsingHint(NABoolean flag = TRUE) {
    flag ? _flags_ |= _selectivitySetUsingHint_ : _flags_ &= ~_selectivitySetUsingHint_;
  }
  inline void setMCforHbasePartitioning(NABoolean flag = TRUE) {
    flag ? _flags_ |= _mcForHbasePartitioning_ : _flags_ &= ~_mcForHbasePartitioning_;
  }
  inline void setExpressionsHistogram(NABoolean flag = TRUE) {
    flag ? _flags_ |= _expressionsHistogram_ : _flags_ &= ~_expressionsHistogram_;
  }
  ...
  // ---------------------------------------------------------------------
  inline NABoolean operator==(const ColStats &other) { return (this == &other); }
  ...
  void setToSingleValue(const EncodedValue &newValue, ConstValue *constExpr, CostScalar *totalRows = NULL,
                        FrequentValue *fv = NULL);
  ...
  void setToSingleInterval(const EncodedValue &newLoBound, const EncodedValue &newUpBound, CostScalar numRows,
                           CostScalar numUecs);
  ...
  void divideHistogramAlongBoundaryValue(const EncodedValue &value, OperatorTypeEnum splitOperator);
  ...
  void populateTemplateOfFakeHist(const ColStatsSharedPtr &fakeHistogram, const ColStatsSharedPtr &realHistgoram);

  // Perform an inner- or semi-join equality predicate, or an 'OR' union
  //   operation on two ColStats, updating THIS with the result.
  void mergeColStats(const ColStatsSharedPtr &otherStats, MergeType mergeMethod, NABoolean isNumeric,
                     OperatorTypeEnum exprOpCode, NABoolean mergeFVs = TRUE);
  ...
  // ------------------------------------------------------------
  void setMaxAndMinSetByPredFlags(const ColStatsSharedPtr &otherStatsCopy, NABoolean &maxSetByPred,
                                  NABoolean &minSetByPred);
  // ---------------------------------------------------------------------
  // graceful recovery in case of any error while merging two histograms,
  // ---------------------------------------------------------------------
  void recoverFromMergeColStats(const ColStatsSharedPtr &otherStats, NABoolean isNumeric,
                                MergeType mergeMethod = INNER_JOIN_MERGE);
  ...
  // ----------------------------------------------------------------------
  CostScalar mergeWithExpandedHistograms(const ColStatsSharedPtr &otherStats, NABoolean isNumeric,
                                         CostScalar &newRowcount, CostScalar &newUec,
                                         MergeType mergeMethod = INNER_JOIN_MERGE);
  ...
  // ----------------------------------------------------------------------
  CostScalar mergeCompressedHistograms(const ColStatsSharedPtr &otherStats, CostScalar &newRowcount, CostScalar &newUec,
                                       MergeType mergeMethod = INNER_JOIN_MERGE);
  ...
  // ---------------------------------------------------------------------
  CostScalar adjustSelectivity(const ColStatsSharedPtr &otherStats, const CostScalar &newUec,
                               MergeType mergeMethod = INNER_JOIN_MERGE);
  ...
  // children or not
  // ------------------------------------------------------------------------
  NABoolean populateLeftAndRightTemplates(const ColStatsSharedPtr &otherStatsCopy, HistogramSharedPtr &leftHistogram,
                                          HistogramSharedPtr &rightHistogram, HistogramSharedPtr &targetHistogram);
  ...
  Criterion decideReductionCriterion(Source invokedFrom, Criterion reductionCriterion, const NAColumn *column,
  ...
  void reduceNumHistInts(Source invokedFrom, Criterion reductionCriterion);
  ...
  HistogramSharedPtr transformOnIntervals(int numOfIntervals);
  HistogramSharedPtr transformOnIntervalsForMC(int numOfIntervals);
  ...
  // best resolution.
  void adjustMaxSelectivity(const EncodedValue &normValue, ConstValue *constExpr, CostScalar *totalRows,
 ...
 private:
  ...
  // case.
  // -----------------------------------------------------------------------
  void mergeWithEmptyHistogram(const ColStatsSharedPtr &otherStats, MergeType mergeMethod);
  ...
  enum Flags {
    ...
    _isObsoleteHistogram_ =
        0x00000800,  // Is histogram obsolete in comparison with other histograms for the same table?
    ...
    _expressionsHistogram_ = 0x00040000      // this ColStats is an expressions histogram
  };
...
inline NABoolean ColStats::isNullInstantiated() const { return histogram_->isNullInstantiated(); }

// inserting a NULL interval, if it doesn't already exist
inline void ColStats::insertNullInterval() {
  if (!isNullInstantiated()) {
...
inline NABoolean ColStats::isAJoinRelatedMerge(MergeType mergeMethod) const {
  if ((mergeMethod == INNER_JOIN_MERGE) || (mergeMethod == SEMI_JOIN_MERGE) || (mergeMethod == ANTI_SEMI_JOIN_MERGE) ||
...
class SkewedValueList : public LIST(EncodedValue) {
 public:
  ...
  NABoolean getIsNullInList() const { return isNullInList_; }

  void setIsNullInList(NABoolean v) { isNullInList_ = v; }

  NABoolean getIsNullSkewed() const { return isNullSkewed_; }

  void setIsNullSkewed(NABoolean v) { isNullSkewed_ = v; }

  NABoolean hasOnlyNonSkewedNull() const { return (!getIsNullSkewed() && getIsNullInList() && entries() == 1); }
