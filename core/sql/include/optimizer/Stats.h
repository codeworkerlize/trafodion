...
class MCboundaryValueList : public LIST(EncodedValue) {
 public:
  ...
  //     vki = v(k-1)i + (hi-li)/n
  //
  void getMinMax(const MCboundaryValueList &lv, const MCboundaryValueList &hv, Int32 numParts,
...
class HistInt {
 ...
 public:
  ...
  HistInt(Int32 intNum, const NAWchar *intBoundary, const NAColumnArray &columns, CostScalar card, CostScalar uec,
...
class ColStats : public NABasicObject {
 ...
 public:
  ...
  ColStats(ComUID &histid, CostScalar uec = 0, CostScalar rowcount = 0, CostScalar baseRowCount = -1,
           ...
           Int32 avgVarcharSize = 0, NAMemory *heap = 0, NABoolean allowMinusOne = FALSE);
