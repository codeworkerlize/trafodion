...
// If MODE_COMPATIBLE_1 is 'on',
// built-in function(such as CHAR_LENGTH,SUBSTR,REPLACE) is compatible with null as a parameter
static NABoolean CheckModeCompatible1() { return CmpCommon::getDefault(MODE_COMPATIBLE_1) == DF_ON; }
//
// -----------------------------------------------------------------------
// aggregate functions
//
...
class Aggregate : public ItemExpr {
 ...
 public:
  Aggregate(OperatorTypeEnum otype, ItemExpr *child0, NABoolean isDistinct, OperatorTypeEnum otypeSpecifiedByUser,
      ...
      : ItemExpr(otype, child0),
        ...
        frameEnd_(INT_MAX),
        isPushdown_(FALSE),
        seqOlapFuncInParser_(NULL),
        rollupGroupIndex_(-1),
        sourceColumnUec_(0.0),
        filterId_(-1) {
    setOrigOpType(otypeSpecifiedByUser);
  }

  Aggregate(OperatorTypeEnum otype, ItemExpr *child0 = NULL, NABoolean isDistinct = FALSE,
  ...
  Aggregate(OperatorTypeEnum otype, ItemExpr *child0, ItemExpr *child1, NABoolean isDistinct = FALSE,
            ValueId distinctId = NULL_VALUE_ID)
  ...
  virtual NABoolean isOlapFunction() const { return isOLAP_; }
  ...
  virtual ItemExpr *rewriteForStagedEvaluation(ValueIdList &initialAggrs, ValueIdList &finalAggrs,
                                               NABoolean sameFormat = FALSE);

  NABoolean isEquivalentForBinding(const ItemExpr *other);

  virtual ValueId mapAndRewrite(ValueIdMap &map, NABoolean mapDownwards = FALSE);
...
#define ITM_COUNT_STAR__ORIGINALLY REL_AGGREGATE
  ...
  */
  ...
  NABoolean isFrameStartUnboundedFollowing() const { return (frameStart_ == INT_MAX); }
  NABoolean isFrameEndUnboundedPreceding() const { return (frameEnd_ == -INT_MAX); }

  NABoolean isFrameEndUnboundedFollowing() const { return (frameEnd_ == INT_MAX); }
  ...
  ItemExpr *getSeqOlapFuncInParser() const { return seqOlapFuncInParser_; }
  void setSeqOlapFuncInParser(ItemExpr *expr) { seqOlapFuncInParser_ = expr; }
  void setOlapOrderBy(ItemExpr *orderBy);
  void setOlapPartitionBy(ItemExpr *partitionBy);
  void setRollupGroupIndex(Int16 v) { rollupGroupIndex_ = v; }
  Int16 getRollupGroupIndex() { return rollupGroupIndex_; }

  NABoolean isRangeOfValuesAggregate();

  Int16 getFilterId() const { return filterId_; }
  void setFilterId(Int16 x) { filterId_ = x; }

  const CostScalar getSourceColUec() const { return sourceColumnUec_; }
  void setSourceColUec(CostScalar &uec) { sourceColumnUec_ = uec; }
 ...
 private:
  ...
  // a reference to an ItmSeqOlapFunction, which created in parser,
  //  1. OLAP functions like SUM, AVG, COUNT, RANK, DENSE_RANK, ROW_NUMBER etc are created as Aggregate in parser,
  //     later in binder in Aggregate::transformOlapFunction() an ItmSeqOlapFunction is created for them.
  //  2. For LEAD and LAG,
  //     ItmLagOlapFunction and ItmLeadOlapFunction are created for them in parser,
  //     they do not go through Aggregate::transformOlapFunction() in binder.
  //  3. Multi-window has to treat all OLAP functions as Aggregate,
  //     creating an Aggregate for LEAD and LAG,
  //     and referring ItmLagOlapFunction or ItmLeadOlapFunction object by Aggregate::seqOlapFuncInParser_,
  //     later in binder, Aggregate::transformOlapFunction() just return the Aggregate::seqOlapFuncInParser_
  //     instead of creating new one.
  ItemExpr *seqOlapFuncInParser_;

  // The uec of the source column that will contribute (distinct) values
  // to insert into the aggregate via RANGE_VALUES_INSERT operation.
  CostScalar sourceColumnUec_;

  // The filter Id that this range of values aggregate represents.
  Int16 filterId_;
...
class Variance : public Aggregate {
 public:
  ...
  //
  Variance(OperatorTypeEnum otype, ItemExpr *child0 = NULL, ItemExpr *child1 = NULL, NABoolean isDistinct = FALSE)
...
class PivotGroup : public Aggregate {
 public:
  enum PivotOptionType { DELIMITER_, ORDER_BY_, MAX_LENGTH_ };

  enum { DEFAULT_MAX_LEN = 1024 };

  class PivotOption {
    friend class PivotGroup;

   public:
    PivotOption(PivotOptionType option, ItemExpr *optionNode = NULL, Long numericVal = 0)
        : option_(option), optionNode_(optionNode), numericVal_(numericVal) {}

    PivotOptionType getOptionType() { return option_; }
    ItemExpr *getOptionItemExpr() { return optionNode_; }

   private:
    PivotOptionType option_;
    ItemExpr *optionNode_;
    Long numericVal_;
  };
  ...
  // The optional second parameter (child) is the weighting factor.
  //
  PivotGroup(OperatorTypeEnum otype, ItemExpr *child0, ItemExpr *child1, NAList<PivotOption *> *pivotOptionsList,
...
class Function : public ItemExpr {
 ...
 public:
  ...
  // append an ascii-version of ItemExpr into cachewa.qryText_
  virtual void generateCacheKey(CacheWA &cwa) const { ItemExpr::generateCacheKey(cwa); }
...
class BuiltinFunction : public Function {
 ...
 public:
  ...
  BuiltinFunction(OperatorTypeEnum otype, NAMemory *h, const LIST(ItemExpr *) & children)
  ...
  virtual NABoolean protectFromVEGs() { return FALSE; }

  // append an ascii-version of ItemExpr into cachewa.qryText_
  virtual void generateCacheKey(CacheWA &cwa) const { Function::generateCacheKey(cwa); }

  virtual NABoolean calculateMinMaxUecs(ColStatDescList &histograms, CostScalar &minUec, CostScalar &maxUec);
  virtual NABoolean isBuiltinFunctionOrderable();
  NABoolean isBuiltinFunc() { return true; }
...
class CacheableBuiltinFunction : public BuiltinFunction {
 public:
  ...
  virtual NABoolean isCacheableExpr(CacheWA &cwa) { return ItemExpr::isCacheableExpr(cwa); }
...
class EvaluateOnceBuiltinFunction : public BuiltinFunction {
 public:
  ...
  inline EvaluateOnceBuiltinFunction(OperatorTypeEnum otype, const LIST(ItemExpr *) & children)
      : BuiltinFunction(otype, CmpCommon::statementHeap(), children) {}
...
class UniqueExecuteId : public EvaluateOnceBuiltinFunction {
 public:
  UniqueExecuteId() : EvaluateOnceBuiltinFunction(ITM_UNIQUE_EXECUTE_ID) {}
...
class GetTriggersStatus : public EvaluateOnceBuiltinFunction {
 public:
  GetTriggersStatus() : EvaluateOnceBuiltinFunction(ITM_GET_TRIGGERS_STATUS) {}
...
class GetBitValueAt : public BuiltinFunction {
 public:
  GetBitValueAt(ItemExpr *val1Ptr, ItemExpr *val2Ptr)
      : BuiltinFunction(ITM_GET_BIT_VALUE_AT, CmpCommon::statementHeap(), 2, val1Ptr, val2Ptr) {
...
class GenericUpdateOutputFunction : public BuiltinFunction {
 public:
  ...
  inline GenericUpdateOutputFunction(OperatorTypeEnum otype, const LIST(ItemExpr *) & children)
      : BuiltinFunction(otype, CmpCommon::statementHeap(), children) {}
  ...
  virtual NABoolean isAGenericUpdateOutputFunction() const { return TRUE; }
...
class IsBitwiseAndTrue : public BuiltinFunction {
 public:
  IsBitwiseAndTrue(ItemExpr *val1Ptr, ItemExpr *val2Ptr)
      : BuiltinFunction(ITM_IS_BITWISE_AND_TRUE, CmpCommon::statementHeap(), 2, val1Ptr, val2Ptr) {
    allowsSQLnullArg() = CheckModeCompatible1();
  }
...
class CodeVal : public CacheableBuiltinFunction {
 public:
  CodeVal(OperatorTypeEnum op, ItemExpr *val1Ptr) : CacheableBuiltinFunction(op, 1, val1Ptr) {
    allowsSQLnullArg() = CheckModeCompatible1();
  }
...
class MaxBytes : public CacheableBuiltinFunction {
 public:
  MaxBytes(OperatorTypeEnum op, ItemExpr *val1Ptr) : CacheableBuiltinFunction(op, 1, val1Ptr) {
    allowsSQLnullArg() = CheckModeCompatible1();
  }
...
class AggrMinMax : public BuiltinFunction {
 public:
  AggrMinMax(ItemExpr *val1Ptr, ItemExpr *val2Ptr)
      : BuiltinFunction(ITM_AGGR_MIN_MAX, CmpCommon::statementHeap(), 2, val1Ptr, val2Ptr) {}
...
class AggrGrouping : public BuiltinFunction {
 public:
  AggrGrouping(Int16 rollupGroupIndex = -1)
      : BuiltinFunction(ITM_AGGR_GROUPING_FUNC, CmpCommon::statementHeap()), rollupGroupIndex_(rollupGroupIndex) {}
...
class AnsiUSERFunction : public BuiltinFunction {
 public:
  AnsiUSERFunction(OperatorTypeEnum oper) : BuiltinFunction(oper) {}
...
class AnsiTenantFunction : public BuiltinFunction {
 public:
  AnsiTenantFunction(OperatorTypeEnum oper) : BuiltinFunction(oper) {}

  // virtual destructor
  virtual ~AnsiTenantFunction();
...
class MonadicUSERFunction : public BuiltinFunction {
 public:
  MonadicUSERFunction(ItemExpr *val1Ptr, OperatorTypeEnum otype = ITM_USER)
      : BuiltinFunction(otype, CmpCommon::statementHeap(), 1, val1Ptr) {}
...
class MonadicUSERIDFunction : public BuiltinFunction {
 public:
  MonadicUSERIDFunction(ItemExpr *val1Ptr) : BuiltinFunction(ITM_USERID, CmpCommon::statementHeap(), 1, val1Ptr) {}
...
class Between : public CacheableBuiltinFunction {
 public:
  ...
  Between(ItemExpr *colPtr, ItemExpr *val1Ptr, ItemExpr *val2Ptr,
  ...
  //++ MV OZ
  void setDirectionVector(const IntegerList &directionVector, CollHeap *heap);
...
class BoolResult : public BuiltinFunction {
 ...
 public:
  BoolResult(ItemExpr *val1Ptr) : BuiltinFunction(ITM_BOOL_RESULT, CmpCommon::statementHeap(), 1, val1Ptr) {}
...
class CharFunc : public BuiltinFunction {
 public:
  CharFunc(CharInfo::CharSet cs, ItemExpr *val1Ptr)
      : BuiltinFunction((cs == CharInfo::UNICODE) ? ITM_UNICODE_CHAR : ITM_CHAR, CmpCommon::statementHeap(), 1,
                        val1Ptr),
        charSet_(cs) {
    allowsSQLnullArg() = CheckModeCompatible1();
  }
 ...
 public:
  PatternMatchingFunction(OperatorTypeEnum optype, ItemExpr *matchValue, ItemExpr *pattern)
      : CacheableBuiltinFunction(optype, 2, matchValue, pattern),
        numberOfNonWildcardChars_(-1),
        patternAStringLiteral_(FALSE),
        bytesInNonWildcardChars_(-1),
        oldDefaultSelForLikeWildCardUsed_(FALSE),
        beginEndKeysApplied_(FALSE) {
    allowsSQLnullArg() = CheckModeCompatible1();
  ...
  virtual ItemExpr *copyTopNode(ItemExpr *derivedNode = NULL, CollHeap *outHeap = 0) { return NULL; }
  ...
  void setPatternAStringLiteral(NABoolean flag = TRUE) { patternAStringLiteral_ = flag; }

  CharInfo::Collation getCollation() { return collation_; }

  void setCollation(CharInfo::Collation c) { collation_ = c; }
  virtual NABoolean hasEquivalentProperties(ItemExpr *other);
...
class Like : public PatternMatchingFunction {
 public:
  Like(ItemExpr *matchValue, ItemExpr *pattern) : PatternMatchingFunction(ITM_LIKE, matchValue, pattern) {
    allowsSQLnullArg() = CheckModeCompatible1();
    setCollation(CharInfo::DefaultCollation);
  }
  ...
  ItemExpr *copyTopNode(ItemExpr *derivedNode = NULL, CollHeap *outHeap = 0);
};  // class Like

class Regexp : public PatternMatchingFunction {
 public:
  Regexp(ItemExpr *matchValue, ItemExpr *pattern) : PatternMatchingFunction(ITM_REGEXP, matchValue, pattern) {
    allowsSQLnullArg() = CheckModeCompatible1();
    setCollation(CharInfo::DefaultCollation);
  }
...
class NoOp : public BuiltinFunction {
 ...
 public:
  NoOp(ItemExpr *val1Ptr) : BuiltinFunction(ITM_NO_OP, CmpCommon::statementHeap(), 1, val1Ptr){};
...
class BoolVal : public BuiltinFunction {
 public:
  BoolVal(OperatorTypeEnum bool_type) : BuiltinFunction(bool_type) {}
  BoolVal(OperatorTypeEnum bool_type, ItemExpr *val1Ptr)
      : BuiltinFunction(bool_type, CmpCommon::statementHeap(), 1, val1Ptr) {}
...
class In : public CacheableBuiltinFunction {
 public:
  In(ItemExpr *leftPtr, ItemExpr *rightPtr) : CacheableBuiltinFunction(ITM_IN, 2, leftPtr, rightPtr) {}
...
class Concat : public CacheableBuiltinFunction {
 public:
  Concat(ItemExpr *val1Ptr, ItemExpr *val2Ptr) : CacheableBuiltinFunction(ITM_CONCAT, 2, val1Ptr, val2Ptr) {
    allowsSQLnullArg() = CheckModeCompatible1();
  }
  ...
  NABoolean isCharTypeMatchRulesRelaxable() {
    return child(0)->isCharTypeMatchRulesRelaxable() && child(1)->isCharTypeMatchRulesRelaxable();
  };
...
class ConvertHex : public CacheableBuiltinFunction {
 public:
  ConvertHex(OperatorTypeEnum op, ItemExpr *val1Ptr) : CacheableBuiltinFunction(op, 1, val1Ptr) {
    allowsSQLnullArg() = CheckModeCompatible1();
  }
...
class CharLength : public CacheableBuiltinFunction {
 public:
  CharLength(ItemExpr *val1Ptr) : CacheableBuiltinFunction(ITM_CHAR_LENGTH, 1, val1Ptr) {
    allowsSQLnullArg() = CheckModeCompatible1();
  }
...
class OctetLength : public CacheableBuiltinFunction {
 public:
  OctetLength(ItemExpr *val1Ptr) : CacheableBuiltinFunction(ITM_OCTET_LENGTH, 1, val1Ptr) {
    allowsSQLnullArg() = CheckModeCompatible1();
  }
...
class PositionFunc : public CacheableBuiltinFunction {
 public:
  PositionFunc(ItemExpr *val1Ptr, ItemExpr *val2Ptr, ItemExpr *val3Ptr, ItemExpr *val4Ptr)
      : CacheableBuiltinFunction(ITM_POSITION, 4, val1Ptr, val2Ptr, val3Ptr, val4Ptr) {
    allowsSQLnullArg() = CheckModeCompatible1();
  }
  ...
  void setCollation(CharInfo::Collation v) { collation_ = v; }
...
class Substring : public CacheableBuiltinFunction {
 public:
  Substring(ItemExpr *val1Ptr, ItemExpr *val2Ptr) : CacheableBuiltinFunction(ITM_SUBSTR, 2, val1Ptr, val2Ptr) {
    allowsSQLnullArg() = CheckModeCompatible1();
  }
  ...
  NABoolean isCharTypeMatchRulesRelaxable() { return child(0)->isCharTypeMatchRulesRelaxable(); };
...
class ConvertTimestamp : public CacheableBuiltinFunction {
 public:
  ConvertTimestamp(ItemExpr *val1Ptr, NABoolean toLocalTime)
      : CacheableBuiltinFunction(ITM_CONVERTTIMESTAMP, 1, val1Ptr), toLocalTime_(toLocalTime) {
    allowsSQLnullArg() = CheckModeCompatible1();
  }
 ...
 private:
  // if TRUE, convert juliantimestamp operand to local time zone
  NABoolean toLocalTime_;
};  // class ConvertTimestamp

class SleepFunction : public CacheableBuiltinFunction {
 public:
  SleepFunction(ItemExpr *val1Ptr) : CacheableBuiltinFunction(ITM_SLEEP, 1, val1Ptr) {}
  // virtual destructor
...
class UnixTimestamp : public CacheableBuiltinFunction {
 public:
  UnixTimestamp() : CacheableBuiltinFunction(ITM_UNIX_TIMESTAMP) {}

  UnixTimestamp(ItemExpr *val1Ptr) : CacheableBuiltinFunction(ITM_UNIX_TIMESTAMP, 1, val1Ptr) {}
  // virtual destructor
...
class CurrentTimestampRunning : public CacheableBuiltinFunction {
 public:
  CurrentTimestampRunning() : CacheableBuiltinFunction(ITM_CURRENT_TIMESTAMP_RUNNING) {}
...
class DateFormat : public CacheableBuiltinFunction {
 public:
  enum FormatType { FORMAT_GENERIC = 0, FORMAT_TO_DATE, FORMAT_TO_TIME, FORMAT_TO_CHAR };
  ...
  enum {
    ...
    NUMBER_FORMAT_STR      // formatStr_ contains number format
  };

  enum CASE_SENSITIVITY {
    NONE = 0,
    ALL_UPPERCASE = 1,        // SUNDAY
    ALL_LOWCASE = 2,          // sunday
  ...
  void unparse(NAString &result, PhaseEnum phase, UnparseFormatEnum form, TableDesc *tabId) const;
 ...
 private:
  ...
  // Uppercase/Lowercase the first character OR all character. e.g SUNDAY Sunday sunday
  CASE_SENSITIVITY caseSensitivity_;
};  // class DateFormat

class DayOfWeek : public CacheableBuiltinFunction {
 public:
  DayOfWeek(ItemExpr *val1Ptr) : CacheableBuiltinFunction(ITM_DAYOFWEEK, 1, val1Ptr) {
    allowsSQLnullArg() = CheckModeCompatible1();
  }
...
class ExplodeVarchar : public BuiltinFunction {
 public:
  ExplodeVarchar(ItemExpr *val1Ptr, const NAType *type, NABoolean forInsert = FALSE)
      : BuiltinFunction(ITM_EXPLODE_VARCHAR, CmpCommon::statementHeap(), 1, val1Ptr),
        type_(type),
...
class Extract : public CacheableBuiltinFunction {
 public:
  Extract(rec_datetime_field extractField, ItemExpr *val1Ptr, NABoolean fieldFunction)
      : CacheableBuiltinFunction(ITM_EXTRACT, 1, val1Ptr), extractField_(extractField), fieldFunction_(fieldFunction) {
    allowsSQLnullArg() = CheckModeCompatible1();
  }

  Extract(OperatorTypeEnum op, rec_datetime_field extractField, ItemExpr *val1Ptr, NABoolean fieldFunction)
      : CacheableBuiltinFunction(op, 1, val1Ptr), extractField_(extractField), fieldFunction_(fieldFunction) {
    allowsSQLnullArg() = CheckModeCompatible1();
  }
...
class ExtractOdbc : public Extract {
 public:
  ExtractOdbc(rec_datetime_field extractField, ItemExpr *val1Ptr, NABoolean fieldFunction)
...
class MonthsBetween : public CacheableBuiltinFunction {
 public:
  MonthsBetween(ItemExpr *val1Ptr, ItemExpr *val2Ptr)
      : CacheableBuiltinFunction(ITM_MONTHS_BETWEEN, 2, val1Ptr, val2Ptr) {
...
class JulianTimestamp : public CacheableBuiltinFunction {
 public:
  JulianTimestamp(ItemExpr *val1Ptr) : CacheableBuiltinFunction(ITM_JULIANTIMESTAMP, 1, val1Ptr) {
    allowsSQLnullArg() = CheckModeCompatible1();
  }
...
class Upper : public CacheableBuiltinFunction {
 public:
  Upper(ItemExpr *val1Ptr) : CacheableBuiltinFunction(ITM_UPPER, 1, val1Ptr) {
    allowsSQLnullArg() = CheckModeCompatible1();
  }
...
class Lower : public CacheableBuiltinFunction {
 public:
  Lower(ItemExpr *val1Ptr) : CacheableBuiltinFunction(ITM_LOWER, 1, val1Ptr) {
    allowsSQLnullArg() = CheckModeCompatible1();
  }
...
class Trim : public CacheableBuiltinFunction {
 ...
 private:
  ...
  // A space character is passed in that case.
  // This value is changed during bind stage based on the type of operand.
  // For binary operand, it is '\0'. For string operand, it is ' '
  NABoolean trimCharNotSpecified_;
};  // class Trim

class Translate : public CacheableBuiltinFunction {
 public:
  enum { /*UCS2*/ UNICODE_TO_SJIS,
         /*UCS2*/ UNICODE_TO_ISO88591,
         ISO88591_TO_UNICODE /*UCS2*/,
         SJIS_TO_UNICODE /*UCS2*/,
         UCS2_TO_SJIS,
         SJIS_TO_UCS2,
         UCS2_TO_UTF8,
         UTF8_TO_UCS2,
         ...
         UTF8_TO_ISO88591,
         ISO88591_TO_UTF8,
         KANJI_MP_TO_ISO88591,
         KSC5601_MP_TO_ISO88591,
         GBK_TO_UTF8,
         UNKNOWN_TRANSLATION };
  ...
  virtual void unparse(NAString &result, PhaseEnum phase = DEFAULT_PHASE, UnparseFormatEnum form = USER_FORMAT,
                       TableDesc *tabId = NULL) const;
  NAString getAssignColName() { return assignColName_; }
...
class Case : public CacheableBuiltinFunction {
 public:
  Case(ItemExpr *val1Ptr, ItemExpr *val2Ptr)
      : CacheableBuiltinFunction(ITM_CASE, 1, val2Ptr), caseOperand_(val1Ptr), caseOperandWasNullable_(FALSE) {}
...
class Cast : public CacheableBuiltinFunction {
 public:
  Cast(ItemExpr *val1Ptr, const NAType *type, OperatorTypeEnum otype = ITM_CAST, NABoolean checkForTrunc = FALSE,
       NABoolean noStringTruncationWarnings = FALSE);

  Cast(ItemExpr *val1Ptr, ItemExpr *errorOutPtr, const NAType *type,
       OperatorTypeEnum otype /* today, should be ITM_NARROW always */, NABoolean checkForTrunc = FALSE,
       NABoolean reverseDataErrorConversionFlag = FALSE, NABoolean noStringTruncationWarnings = FALSE);
  ...
  virtual NABoolean isAGenericUpdateOutputFunction() const { return child(0)->isAGenericUpdateOutputFunction(); }
  ...
  void setMatchChildType(NABoolean v) { (v ? flags_ |= MATCH_CHILD_TYPE : flags_ &= ~MATCH_CHILD_TYPE); }
  ...
  void setTgtCSSpecified(NABoolean v) { (v ? flags_ |= TGT_CHAR_SET_SPECIFIED : flags_ &= ~TGT_CHAR_SET_SPECIFIED); }

  NABoolean srcIsVarcharPtr() { return (flags_ & SRC_IS_VARCHAR_PTR) != 0; }
  void setSrcIsVarcharPtr(NABoolean v) { (v ? flags_ |= SRC_IS_VARCHAR_PTR : flags_ &= ~SRC_IS_VARCHAR_PTR); }

  NABoolean allowSignInInterval() { return ((flags_ & ALLOW_SIGN_IN_INTERVAL) != 0); };

  void setAllowSignInInterval(NABoolean v) {
    (v) ? flags_ |= ALLOW_SIGN_IN_INTERVAL : flags_ &= ~ALLOW_SIGN_IN_INTERVAL;
  }

  NABoolean convertNullWhenError() { return (flags_ & CONV_NULL_WHEN_ERROR) != 0; }
  void setConvertNullWhenError(NABoolean v) { (v ? flags_ |= CONV_NULL_WHEN_ERROR : flags_ &= ~CONV_NULL_WHEN_ERROR); }

  NABoolean noStringTruncationWarnings() { return (flags_ & NO_STRING_TRUNC_WARNINGS) != 0; }
  void setNoStringTruncationWarnings(NABoolean v) {
    (v ? flags_ |= NO_STRING_TRUNC_WARNINGS : flags_ &= ~NO_STRING_TRUNC_WARNINGS);
  }

  NABoolean padUseZero() { return (flags_ & PAD_USE_ZERO) != 0; }
  void setPadUseZero(NABoolean v) { (v ? flags_ |= PAD_USE_ZERO : flags_ &= ~PAD_USE_ZERO); }
...
class CastConvert : public Cast {
 public:
  CastConvert(ItemExpr *val1Ptr, const NAType *type) : Cast(val1Ptr, type, ITM_CAST_CONVERT) {}
...
class CastType : public Cast {
 public:
  CastType(ItemExpr *val1Ptr, const NAType *type) : Cast(val1Ptr, type, ITM_CAST_TYPE), makeNullable_(FALSE) {}

  CastType(ItemExpr *val1Ptr, NABoolean makeNullable)
      : Cast(val1Ptr, NULL, ITM_CAST_TYPE), makeNullable_(makeNullable) {}
...
class Narrow : public Cast {
 public:
  ...
  Narrow(ItemExpr *val1Ptr, ItemExpr *errorOutPtr, const NAType *type, OperatorTypeEnum otype = ITM_NARROW,
         NABoolean reverseDataErrorConversionFlag = FALSE, NABoolean matchChildNullability = FALSE)
      : Cast(val1Ptr, errorOutPtr, type, otype, FALSE, reverseDataErrorConversionFlag),
...
class InstantiateNull : public Cast {
 public:
  InstantiateNull(ItemExpr *valuePtr, const NAType *typePtr)
      : Cast(valuePtr, typePtr, ITM_INSTANTIATE_NULL), beginLOJTransform_(FALSE), NoCheckforLeftToInnerJoin(FALSE) {}
...
class DecodeBase64 : public BuiltinFunction {
 public:
  DecodeBase64(ItemExpr *val1Ptr, const NAType *type = NULL, CharInfo::CharSet charSet = CharInfo::ISO88591,
               NAMemory *h = CmpCommon::statementHeap())
      : BuiltinFunction(ITM_DECODE_BASE64, h, 1, val1Ptr), type_(type), charSet_(charSet) {
    allowsSQLnullArg() = TRUE;
  }
...
class HashCommon : public BuiltinFunction {
 ...
 public:
  ...
  void setCasesensitiveHash(NABoolean v) { (v ? flags_ |= CASESENSITIVE_HASH_ : flags_ &= ~CASESENSITIVE_HASH_); }
...
class Hash : public HashCommon {
 ...
 public:
  Hash(ItemExpr *val1Ptr) : HashCommon(ITM_HASH, 1, val1Ptr) {}
...
class HashComb : public HashCommon {
 ...
 public:
  HashComb(ItemExpr *val1Ptr, ItemExpr *val2Ptr) : HashCommon(ITM_HASHCOMB, 2, val1Ptr, val2Ptr) {}
...
class HiveHashComb : public HashCommon {
  ...
  // Returns a value of type UNSIGNED INTEGER
 public:
  HiveHashComb(ItemExpr *val1Ptr, ItemExpr *val2Ptr) : HashCommon(ITM_HIVE_HASHCOMB, 2, val1Ptr, val2Ptr) {}
...
class HashDistPartHash : public HashCommon {
  ...
  // hash value for the data item.
 public:
  HashDistPartHash(ItemExpr *val1Ptr) : HashCommon(ITM_HDPHASH, 1, val1Ptr) {}
...
class HashDistPartHashComb : public HashCommon {
  ...
  // Returns a value of type UNSIGNED INTEGER
 public:
  HashDistPartHashComb(ItemExpr *val1Ptr, ItemExpr *val2Ptr) : HashCommon(ITM_HDPHASHCOMB, 2, val1Ptr, val2Ptr) {}
...
class HiveHash : public HashCommon {
  ...
  // hash value for the data item.
 public:
  HiveHash(ItemExpr *val1Ptr) : HashCommon(ITM_HIVE_HASH, 1, val1Ptr) {}
...
class MathFunc : public CacheableBuiltinFunction {
 public:
  MathFunc(OperatorTypeEnum oper, ItemExpr *val1Ptr = NULL, ItemExpr *val2Ptr = NULL)
      : CacheableBuiltinFunction(oper, 2, val1Ptr, val2Ptr) {
    allowsSQLnullArg() = CheckModeCompatible1();
    compatibleMode_ = 1;  // default ON
  }
  ...
  void setCompatibleMode(short compatibleMode) { compatibleMode_ = compatibleMode; }

  short getCompatibleMode() const { return compatibleMode_; }

 private:
  short compatibleMode_;  // 0 - OFF, 1 - ON
};

class Abs : public MathFunc {
 public:
  Abs(ItemExpr *val1Ptr) : MathFunc(ITM_ABS, val1Ptr) { allowsSQLnullArg() = CheckModeCompatible1(); }
...
class BitOperFunc : public CacheableBuiltinFunction {
 public:
  BitOperFunc(OperatorTypeEnum oper, ItemExpr *val1Ptr = NULL, ItemExpr *val2Ptr = NULL, ItemExpr *val3Ptr = NULL)
...
class Modulus : public CacheableBuiltinFunction {
 ...
 public:
  Modulus(ItemExpr *val1Ptr, ItemExpr *val2Ptr) : CacheableBuiltinFunction(ITM_MOD, 2, val1Ptr, val2Ptr) {
    allowsSQLnullArg() = CheckModeCompatible1();
  }
...
class Repeat : public BuiltinFunction {
 public:
  Repeat(ItemExpr *val1Ptr, ItemExpr *val2Ptr, Int32 maxLength = -1)
    ...
    allowsSQLnullArg() = CheckModeCompatible1();
    if (maxLength > 0) maxLengthWasExplicitlySet_ = TRUE;
  }
...
class RegexpReplace : public Replace {
 public:
  RegexpReplace(ItemExpr *value1, ItemExpr *value2, ItemExpr *value3) : Replace(value1, value2, value3) {
    allowsSQLnullArg() = CheckModeCompatible1();
    setOperatorType(ITM_REGEXP_REPLACE);
    setOrigOpType(ITM_REGEXP_REPLACE);
  }

  // virtual destructor
  virtual ~RegexpReplace(){};
  ItemExpr *copyTopNode(ItemExpr *derivedNode = NULL, CollHeap *outHeap = 0);
...
class ReplaceNull : public BuiltinFunction {
 ...
 public:
  ReplaceNull(ItemExpr *val1Ptr, ItemExpr *val2Ptr, ItemExpr *val3Ptr)
      : BuiltinFunction(ITM_REPLACE_NULL, CmpCommon::statementHeap(), 3, val1Ptr, val2Ptr, val3Ptr) {}
...
class RegexpSubstr : public CacheableBuiltinFunction {
 public:
  RegexpSubstr(ItemExpr *value1, ItemExpr *value2) : CacheableBuiltinFunction(ITM_REGEXP_SUBSTR, 2, value1, value2) {
    allowsSQLnullArg() = CheckModeCompatible1();
  }

  RegexpSubstr(ItemExpr *value1, ItemExpr *value2, ItemExpr *value3)
      : CacheableBuiltinFunction(ITM_REGEXP_SUBSTR, 3, value1, value2, value3) {
    allowsSQLnullArg() = CheckModeCompatible1();
  }

  RegexpSubstr(ItemExpr *value1, ItemExpr *value2, ItemExpr *value3, ItemExpr *value4)
      : CacheableBuiltinFunction(ITM_REGEXP_SUBSTR, 4, value1, value2, value3, value4) {
    allowsSQLnullArg() = CheckModeCompatible1();
  }
  // virtual destructor
  virtual ~RegexpSubstr(){};
  ItemExpr *copyTopNode(ItemExpr *derivedNode = NULL, CollHeap *outHeap = 0);
...
class RegexpCount : public CacheableBuiltinFunction {
 public:
  RegexpCount(ItemExpr *value1, ItemExpr *value2) : CacheableBuiltinFunction(ITM_REGEXP_COUNT, 2, value1, value2) {
    allowsSQLnullArg() = CheckModeCompatible1();
  }

  RegexpCount(ItemExpr *value1, ItemExpr *value2, ItemExpr *value3)
      : CacheableBuiltinFunction(ITM_REGEXP_COUNT, 3, value1, value2, value3) {
    allowsSQLnullArg() = CheckModeCompatible1();
  }
...
class HashDistrib : public HashCommon {
  ...
  //
 public:
  HashDistrib(OperatorTypeEnum otype, ItemExpr *val1Ptr, ItemExpr *val2Ptr) : HashCommon(otype, 2, val1Ptr, val2Ptr) {}
...
class Hash2Distrib : public HashDistrib {
  ...
  //
 public:
  Hash2Distrib(ItemExpr *val1Ptr, ItemExpr *val2Ptr) : HashDistrib(ITM_HASH2_DISTRIB, val1Ptr, val2Ptr) {}
...
class ProgDistrib : public HashDistrib {
  ...
  //
 public:
  ProgDistrib(ItemExpr *val1Ptr, ItemExpr *val2Ptr) : HashDistrib(ITM_PROGDISTRIB, val1Ptr, val2Ptr) {}
...
class ProgDistribKey : public BuiltinFunction {
 public:
  ProgDistribKey(ItemExpr *value, ItemExpr *offset, ItemExpr *totalNumValues)
      : BuiltinFunction(ITM_PROGDISTRIBKEY, CmpCommon::statementHeap(), 3, value, offset, totalNumValues) {}
...
class PAGroup : public BuiltinFunction {
 ...
 public:
  PAGroup(ItemExpr *val1Ptr, ItemExpr *val2Ptr, ItemExpr *val3Ptr)
      : BuiltinFunction(ITM_PAGROUP, CmpCommon::statementHeap(), 3, val1Ptr, val2Ptr, val3Ptr) {}
...
class InverseOrder : public BuiltinFunction {
 ...
 public:
  InverseOrder(ItemExpr *val1Ptr) : BuiltinFunction(ITM_INVERSE, CmpCommon::statementHeap(), 1, val1Ptr) {}
...
class Increment : public BuiltinFunction {
 public:
  Increment(ItemExpr *val1Ptr) : BuiltinFunction(ITM_INCREMENT, CmpCommon::statementHeap(), 1, val1Ptr) {}
...
class Decrement : public BuiltinFunction {
 public:
  Decrement(ItemExpr *val1Ptr) : BuiltinFunction(ITM_DECREMENT, CmpCommon::statementHeap(), 1, val1Ptr) {}
...
class TriRelational : public BuiltinFunction {
 public:
  TriRelational(OperatorTypeEnum optype, ItemExpr *val1Ptr, ItemExpr *val2Ptr, ItemExpr *val3Ptr);
...
class RangeLookup : public BuiltinFunction {
 public:
  RangeLookup(ItemExpr *val1Ptr, const RangePartitioningFunction *partFunc);
...
class ScalarVariance : public CacheableBuiltinFunction {
 public:
  ScalarVariance(OperatorTypeEnum opType,  // ITM_VARIANCE or ITM_STDDEV
                 ItemExpr *SumOfValSquared, ItemExpr *SumOfVal, ItemExpr *CountOfVal)
 ...
 public:
  RowsetArrayScan(ItemExpr *source, ItemExpr *index, int maxNumElem, int elemSize, NABoolean elemNullInd,
      ...
      : BuiltinFunction(opType, CmpCommon::statementHeap(), 2, source, index),
        ...
        elemType_(elemType) {
    optLargVar_ = CmpCommon::getDefault(OPTIMIZE_LARGE_VARCHAR) == DF_ON;
  };
  ...
  // get a printable string that identifies the operator
  //
  virtual const NAString getText() const { return "RowsetArrayScan"; };
...
class UnPackCol : public BuiltinFunction {
 public:
  ...
  // get a printable string that identifies the operator
  //
  virtual const NAString getText() const { return "UnPackCol"; };
...
class RandomNum : public BuiltinFunction {
 public:
  RandomNum(ItemExpr *seed = NULL, NABoolean simpleRandom = FALSE)
      : BuiltinFunction(ITM_RANDOMNUM, CmpCommon::statementHeap(), 1, seed), simpleRandom_(simpleRandom) {
    allowsSQLnullArg() = CheckModeCompatible1();
  }
  ...
  NABoolean isSimple() const { return simpleRandom_; }
...
class Shift : public BuiltinFunction {
 public:
  Shift(OperatorTypeEnum otype,  // ITM_SHIFT_RIGHT || ITM_SHIFT_LEFT
        ItemExpr *child0, ItemExpr *child1)
...
class Mask : public BuiltinFunction {
 public:
  Mask(OperatorTypeEnum otype, ItemExpr *child0, ItemExpr *child1)
      : BuiltinFunction(otype, CmpCommon::statementHeap(), 2, child0, child1) {}
...
class ZZZBinderFunction : public BuiltinFunction {
 public:
  ZZZBinderFunction(OperatorTypeEnum oper, ItemExpr *val1Ptr = NULL, ItemExpr *val2Ptr = NULL, ItemExpr *val3Ptr = NULL,
                    ItemExpr *val4Ptr = NULL, ItemExpr *val5Ptr = NULL, ItemExpr *val6Ptr = NULL)
      : BuiltinFunction(oper, CmpCommon::statementHeap(), 6, val1Ptr, val2Ptr, val3Ptr, val4Ptr, val5Ptr, val6Ptr),
        flags_(0) {}
...
class ItmSysConnectByPathFunc : public ZZZBinderFunction {
 public:
  ItmSysConnectByPathFunc(const char *del = "", ItemExpr *val = NULL)
      : ZZZBinderFunction(ITM_SYS_CONNECT_BY_PATH, val, NULL, NULL, NULL, NULL) {
    delimiter_ = del;
    pathItemExpr_ = val;
  }
...
class ItmSequenceFunction : public BuiltinFunction {
 public:
  ItmSequenceFunction(OperatorTypeEnum itemType, ItemExpr *val1Ptr, ItemExpr *val2Ptr = NULL, ItemExpr *val3Ptr = NULL)
      : BuiltinFunction(itemType, CmpCommon::statementHeap(), 3, val1Ptr, val2Ptr, val3Ptr),
        isOLAP_(FALSE),
        olapPartitionBy_(NULL),
        olapOrderBy_(NULL),
        isTDFunction_(FALSE) {}
  ...
  NABoolean isOlapFunction() { return isOLAP(); }
  ...
  void setOLAPInfo(ItemExpr *partBy, ItemExpr *orderBy, NABoolean isTDFunction = FALSE) {
    olapPartitionBy_ = partBy;
    olapOrderBy_ = orderBy;
  ...
  // default implementation for sequence/olap functions:
  // do not change any literals into constant parameters.
  ItemExpr *normalizeForCache(CacheWA &cwa, BindWA &bindWA);
  virtual void generateCacheKey(CacheWA &cwa) const;
...
class ItmSeqRunningFunction : public ItmSequenceFunction {
 public:
  ItmSeqRunningFunction(OperatorTypeEnum itemType, ItemExpr *valPtr) : ItmSequenceFunction(itemType, valPtr) {}
  ...
  void transformNode(NormWA &normWARef, ExprValueId &locationOfPointerToMe, ExprGroupId &introduceSemiJoinHere,
                     const ValueIdSet &externalInputs);
...
//------------------------------------------------------------------------
// Running Pivot Group function class.
//
//------------------------------------------------------------------------
//
class ItmSeqRunningPivotGroup : public ItmSeqRunningFunction {
 public:
  ...
  // virtual destructor
  virtual ~ItmSeqRunningPivotGroup() {}
...
class ItmSeqOlapFunction : public ItmSequenceFunction {
 public:
  ItmSeqOlapFunction(OperatorTypeEnum itemType, ItemExpr *val1Ptr, ItemExpr *val2Ptr = NULL, ItemExpr *val3Ptr = NULL)
      : ItmSequenceFunction(itemType, val1Ptr, val2Ptr, val3Ptr), frameStart_(0), frameEnd_(0) {}
  ...
  inline NABoolean canInverseOLAPOrder() {
    return !(frameStart_ == -INT_MAX && frameEnd_ != INT_MAX &&
                 (getOperatorType() == ITM_OLAP_MIN || getOperatorType() == ITM_OLAP_MAX) ||
             getOperatorType() == ITM_OLAP_RANK || getOperatorType() == ITM_OLAP_DRANK);
  }

  inline NABoolean mustInverseOLAPOrder() {
    return (frameStart_ != -INT_MAX && frameEnd_ == INT_MAX &&
            (getOperatorType() == ITM_OLAP_MIN || getOperatorType() == ITM_OLAP_MAX));
  }

  NABoolean inverseOLAPOrder(CollHeap *heap);
...
class ItmLeadOlapFunction : public ItmSeqOlapFunction {
 public:
  ...
  // get a printable string that identifies the operator
  virtual const NAString getText() const { return "LEAD"; };
  virtual ItemExpr *normalizeNode(NormWA &normWARef);
...
class ItmLagOlapFunction : public ItmSeqOlapFunction {
 public:
  ItmLagOlapFunction(ItemExpr *seqColumn, ItemExpr *offsetExpr, ItemExpr *defaultValue = NULL)
      : ItmSeqOlapFunction(ITM_OLAP_LAG, seqColumn, offsetExpr, defaultValue), offset_(-1) {}
...
class ItmSeqDiff1 : public ItmSequenceFunction {
 public:
  ItmSeqDiff1(ItemExpr *valPtr1, ItemExpr *valPtr2 = NULL) : ItmSequenceFunction(ITM_DIFF1, valPtr1, valPtr2) {
    allowsSQLnullArg() = CheckModeCompatible1();
  }
...
class ItmSeqDiff2 : public ItmSequenceFunction {
 public:
  ItmSeqDiff2(ItemExpr *valPtr1, ItemExpr *valPtr2 = NULL) : ItmSequenceFunction(ITM_DIFF2, valPtr1, valPtr2) {
    allowsSQLnullArg() = CheckModeCompatible1();
  }
...
class ItmSeqRowsSince : public ItmSequenceFunction {
 public:
  ItmSeqRowsSince(ItemExpr *valPtr, ItemExpr *valPtr2 = NULL, NABoolean inclusionSpec = FALSE)
      : ItmSequenceFunction(ITM_ROWS_SINCE, valPtr, valPtr2), includeCurrentRow_(inclusionSpec) {
    allowsSQLnullArg() = CheckModeCompatible1();
  }
...
class ItmSeqMovingFunction : public ItmSequenceFunction {
 public:
  ItmSeqMovingFunction(OperatorTypeEnum itemType, ItemExpr *valPtr1, ItemExpr *valPtr2, ItemExpr *valPtr3 = NULL)
      : ItmSequenceFunction(itemType, valPtr1, valPtr2, valPtr3), skipMovingMinMaxTransformation_(FALSE) {
    allowsSQLnullArg() = CheckModeCompatible1();
  }
...
class ItmSeqThisFunction : public ItmSequenceFunction {
 public:
  ItmSeqThisFunction(ItemExpr *valPtr) : ItmSequenceFunction(ITM_THIS, valPtr) {}
  // virtual destructor
...
class ItmSeqNotTHISFunction : public ItmSequenceFunction {
 public:
  ItmSeqNotTHISFunction(ItemExpr *valPtr) : ItmSequenceFunction(ITM_NOT_THIS, valPtr) {}
  // virtual destructor
...
//------------------------------------------------------------------------
// Class for OLAP functions first_value() and last_value.
//------------------------------------------------------------------------
//
class ItmOlapFirstLastValue : public ItmSeqOlapFunction {
 public:
  ItmOlapFirstLastValue(OperatorTypeEnum itemType, ItemExpr *column);

  virtual ~ItmOlapFirstLastValue() {}
  virtual ItemExpr *bindNode(BindWA *bindWA);
  ...
  virtual ItemExpr *transformOlapFunction(CollHeap *heap) { return this; }

  virtual NABoolean isOlapFunction() const { return TRUE; }
  // get a printable string that identifies the operator
  virtual const NAString getText() const {
    return getOperatorType() == ITM_OLAP_FIRST_VALUE ? "olap first_value" : "olap last_value";
  }
  ValueId &counterVid() { return counterVid_; }
...
//------------------------------------------------------------------------
// Class for OLAP function PivotGroup().
//------------------------------------------------------------------------
//
class ItmOlapPivotGroup : public ItmSeqOlapFunction {
 public:
  ...
  // get a printable string that identifies the operator
  virtual const NAString getText() const { return "olap pivot group"; }
...
class HbaseColumnLookup : public BuiltinFunction {
 public:
  HbaseColumnLookup(ItemExpr *child0, NAString &hbaseCol, const NAType *naType = NULL)
      : BuiltinFunction(ITM_HBASE_COLUMN_LOOKUP, CmpCommon::statementHeap(), 1, child0),
        hbaseCol_(hbaseCol),
        naType_(naType) {}
  ...
  // get a printable string that identifies the operator
  virtual const NAString getText() const { return "HBASE_COLUMN_LOOKUP"; };
...
class HbaseColumnsDisplay : public BuiltinFunction {
 public:
  ...
  // get a printable string that identifies the operator
  virtual const NAString getText() const { return "HBASE_COLUMNS_DISPLAY"; };
...
class HbaseColumnCreate : public BuiltinFunction {
 public:
  class HbaseColumnCreateOptions {
   ...
   public:
    enum ConvType { CAST_AS, TYPE_AS, NONE };

    HbaseColumnCreateOptions(ItemExpr *colName, ItemExpr *colVal, const NAType *naType, ConvType convType)
        : colName_(colName), colVal_(colVal), naType_(naType), convType_(convType) {}
  ...
  HbaseColumnCreate(NAList<HbaseColumnCreateOptions *> *hccol)
      : BuiltinFunction(ITM_HBASE_COLUMN_CREATE, CmpCommon::statementHeap(), 0, NULL),
        resultType_(NULL),
        hccol_(hccol),
        colNameMaxLen_(0),
        colValMaxLen_(0) {}
  ...
  // get a printable string that identifies the operator
  virtual const NAString getText() const { return "HBASE_COLUMN_CREATE"; };
...
class HbaseAttribute : public BuiltinFunction {
 public:
  HbaseAttribute(OperatorTypeEnum otype, ItemExpr *col)
      : BuiltinFunction(otype, CmpCommon::statementHeap(), 1, NULL), col_(col), tsVals_(NULL), colIndex_(-1) {}

  virtual ~HbaseAttribute();

  virtual NABoolean isCovered(const ValueIdSet &newExternalInputs, const GroupAttributes &coveringGA,
                              ValueIdSet &referencedInputs, ValueIdSet &coveredSubExpr,
                              ValueIdSet &unCoveredExpr) const;
...
class HbaseAttributeRef : public BuiltinFunction {
 public:
  HbaseAttributeRef(OperatorTypeEnum otype, ItemExpr *col)
      : BuiltinFunction(otype, CmpCommon::statementHeap()), col_(col) {}
  ...
  const ItemExpr *col() const { return col_; }
  ItemExpr *col() { return col_; }
  void setCol(ItemExpr *col) { col_ = col; }
...
class HbaseVisibility : public HbaseAttribute {
 public:
  ...
  HbaseVisibility(ItemExpr *col) : HbaseAttribute(ITM_HBASE_VISIBILITY, col) {}
  ...
  // get a printable string that identifies the operator
  virtual const NAString getText() const { return "HBASE_VISIBILITY"; };

};  // class HbaseVisibility

class HbaseVisibilityRef : public HbaseAttributeRef {
 public:
  HbaseVisibilityRef(ItemExpr *col) : HbaseAttributeRef(ITM_HBASE_VISIBILITY_REF, col) {}

  virtual ItemExpr *copyTopNode(ItemExpr *derivedNode = NULL, CollHeap *outHeap = 0);

  // get a printable string that identifies the operator
  virtual const NAString getText() const { return "HBASE_VISIBILITY_REF"; };
...
class HbaseVisibilitySet : public HbaseAttributeRef {
 public:
  HbaseVisibilitySet(ItemExpr *col, NAString &visExpr)
      : HbaseAttributeRef(ITM_HBASE_VISIBILITY_SET, col), visExpr_(visExpr) {}
  ...
  // get a printable string that identifies the operator
  virtual const NAString getText() const { return "HBASE_VISIBILITY_SET"; };
...
class HbaseRowid : public HbaseAttribute {
 public:
  HbaseRowid(ItemExpr *col, UInt32 rowidLen = 0)
      : HbaseAttribute(ITM_HBASE_ROWID, col), rowidLen_(rowidLen), rowidType_(NULL) {}
  ...
  NAType *rowidValsType() { return rowidType_; }

  // get a printable string that identifies the operator
  virtual const NAString getText() const { return "HBASE_ROWID"; };

 private:
  NAType *rowidType_;  // HBase Rowkey(ROWID) type of trafodion table
  UInt32 rowidLen_;
};  // class HbaseRowid

class HbaseRowidRef : public HbaseAttributeRef {
 public:
  HbaseRowidRef(CorrName *corrName, ItemExpr *col = NULL)
      : HbaseAttributeRef(ITM_HBASE_ROWID_REF, col), corrName_(corrName) {}
  ...
  // get a printable string that identifies the operator
  virtual const NAString getText() const { return "HBASE_ROWID_REF"; };
...
class HbaseTimestamp : public HbaseAttribute {
 public:
  HbaseTimestamp(ItemExpr *col) : HbaseAttribute(ITM_HBASE_TIMESTAMP, col) {}
  ...
  // get a printable string that identifies the operator
  virtual const NAString getText() const { return "HBASE_TIMESTAMP"; };

};  // class HbaseTimestamp

class HbaseTimestampRef : public HbaseAttributeRef {
 public:
  HbaseTimestampRef(ItemExpr *col) : HbaseAttributeRef(ITM_HBASE_TIMESTAMP_REF, col) {}

  virtual ItemExpr *copyTopNode(ItemExpr *derivedNode = NULL, CollHeap *outHeap = 0);

  // get a printable string that identifies the operator
  virtual const NAString getText() const { return "HBASE_TIMESTAMP_REF"; };
...
class HbaseVersion : public HbaseAttribute {
 public:
  HbaseVersion(ItemExpr *col) : HbaseAttribute(ITM_HBASE_VERSION, col) {}
  ...
  // get a printable string that identifies the operator
  virtual const NAString getText() const { return "HBASE_VERSION"; };

};  // class HbaseVersion

class HbaseVersionRef : public HbaseAttributeRef {
 public:
  HbaseVersionRef(ItemExpr *col) : HbaseAttributeRef(ITM_HBASE_VERSION_REF, col) {}

  virtual ItemExpr *copyTopNode(ItemExpr *derivedNode = NULL, CollHeap *outHeap = 0);

  // get a printable string that identifies the operator
  virtual const NAString getText() const { return "HBASE_VERSION_REF"; };
...
class SequenceValue : public BuiltinFunction {
 public:
  SequenceValue(const CorrName &seqCorrName, NABoolean currVal = FALSE, NABoolean nextVal = TRUE)
      : BuiltinFunction(ITM_SEQUENCE_VALUE, CmpCommon::statementHeap(), 0, NULL),
  ...
  // get a printable string that identifies the operator
  virtual const NAString getText() const { return "SEQUENCE_VALUE"; };
...
class RowNumFunc : public BuiltinFunction {
 public:
  RowNumFunc() : BuiltinFunction(ITM_ROWNUM, CmpCommon::statementHeap(), 0, NULL) {}
...
class BeginKey : public BuiltinFunction {
 public:
  BeginKey(ItemExpr *valPtr) : BuiltinFunction(ITM_BEGINKEY, CmpCommon::statementHeap(), 1, valPtr) {
    allowsSQLnullArg() = CheckModeCompatible1();
  }
...
class EndKey : public BuiltinFunction {
 public:
  EndKey(ItemExpr *valPtr, Int16 bytesPerChar = 0, NAWchar maxValue = 0xffff)
      : BuiltinFunction(ITM_ENDKEY, CmpCommon::statementHeap(), 1, valPtr),
