...
class EstLogProp : public NABasicObject {
 public:
  ...
  // ---------------------------------------------------------------------
  enum SemiTSJEnum {
    NOT_SEMI_TSJ = FALSE,  // previous value of "FALSE"
    SEMI_TSJ,              // previous value of "TRUE"
    ANTI_SEMI_TSJ
  };  // new value : for case of AntiSemiJoin

  // ---------------------------------------------------------------------
  //  Constructor, copy constructor, and assignment operator
  // ---------------------------------------------------------------------
  EstLogProp(CostScalar card = -1, ValueIdSet *preds = NULL, SemiTSJEnum inputForSemiTSJ = NOT_SEMI_TSJ,
             CANodeIdSet *nodeSet = NULL, NABoolean cacheable = FALSE, NAMemory *h = HISTHEAP);
  ...
  short getBMOgrowthPercent() const { return RelExpr::bmoGrowthPercent(getResultCardinality(), getMaxCardEst()); }

  CostScalar getMaxCardEst() const {  // CCMPASSERT (maxCardinality_ >= 0) ;
    return maxCardinality_;
  }

  void setMaxCardEst(CostScalar v) {  // CCMPASSERT (v >= 0) ;
    if (v.getValue() > COSTSCALAR_MAX)
  ...
  inline const ColStatDescList &getColStats() const { return columnStats_; }

  inline const ValueIdSet &getUnresolvedPreds() const { return unresolvedPreds_; }
  inline ValueIdSet &unresolvedPreds() { return unresolvedPreds_; }

  inline SemiTSJEnum inputForSemiTSJ() { return inputForSemiTSJ_; }
  inline SemiTSJEnum getInputForSemiTSJ() const { return inputForSemiTSJ_; }
  inline CANodeIdSet *getNodeSet() const { return nodeSet_; }
  inline void setNodeSet(CANodeIdSet *vset) { nodeSet_ = vset; }

  inline void setCacheableFlag(NABoolean flag) { cacheable_ = flag; }
  ...
  // mutator:
  inline void setInputForSemiTSJ(SemiTSJEnum flag) { inputForSemiTSJ_ = flag; }
  ...
  // ---------------------------------------------------------------------
  void pickOutputs(ColStatDescList &columnStats, const EstLogPropSharedPtr &inputEstLogProp,
                   const ValueIdSet charOutputs, const ValueIdSet predSet);

  void pickOutputsForUpdate(ColStatDescList columnStats, const EstLogPropSharedPtr &inputEstLogProp,
                            const RelExpr &relExpr, const ValueIdSet updateExprOutputs, const ValueIdSet predSet);

  void mapOutputsForUpdate(const GenericUpdate &relExpr, const ValueIdMap &updateSelectValueIdMap);
