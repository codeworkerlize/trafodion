...
class MvLog : public BinderOnlyNode {
 public:
  MvLog(const QualifiedName *tableName, const ItemExpr *columnNames, ItemExpr *rangeBegin, ItemExpr *rangeEnd,
 ...
 private:
  ...
  void addClusteringIndexBoundries(LIST(ItemExprList *) & ciBoundries, const NAFileSet *pCiFileSet) const;
  void addRangePartitionBoundries(LIST(ItemExprList *) & ciBoundries,
  ...
  // returns a BoolValue
  ItemExpr *compareItems(const ItemExpr *pFirst, const ItemExpr *pSecond, OperatorTypeEnum compareType,
                         IntegerList *directionVector) const;

  ItemExpr *buildConditionalColumnList(const ConstStringList &option1names, const ConstStringList &option2names,
  ...
  void constructEndRangeBoundries(LIST(ItemExprList *) & endRangeBoundry, NABoolean needStartRange) const;
  ...
  void appendToExprList(ItemExprList &toAddto, const ConstStringList &columnNames,
