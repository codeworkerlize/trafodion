...
class ColumnDesc : public NABasicObject {
 public:
  ...
  // ---------------------------------------------------------------------
  ColumnDesc(const ColRefName &colRefName, const ValueId valId, const char *heading, CollHeap *h = 0)
      : colRefName_(colRefName, h),
        valId_(valId),
        origValId_((CollIndex)0),
        heading_(heading),
        groupedFlag_(FALSE),
        rowidGroupedFlag_(FALSE),
        viewColPosition_(-1),
        viewFileName_(NULL),
        isRenameCol_(false) {}
  ...
  // ---------------------------------------------------------------------
  void print(FILE *ofd = stdout, const char *indent = DEFAULT_INDENT, const char *title = "ColumnDesc") const {
    if (strcmp(title, "")) {
      BUMP_INDENT(indent);
      fprintf(ofd, "%s%s %s", NEW_INDENT, title, NEW_INDENT);
    }
    colRefName_.print(ofd, indent, "", TRUE /* brief */);
    fprintf(ofd, " %u%s", CollIndex(valId_), groupedFlag_ ? " grp " : "");
...
class ColumnDescList : public LIST(ColumnDesc *) {
 public:
  ...
  ColumnDescList(const ColumnDescList &cdlist, CollHeap *h) : LIST(ColumnDesc *)(cdlist, h) {}
  ...
  void getValueIdList(ValueIdList &vidList) const {
    for (CollIndex i = 0; i < entries(); i++) vidList.insert(at(i)->getValueId());
  }
  ...
  void clearAndDestroy() {
    for (CollIndex i = 0; i < entries(); i++) delete at(i);
    clear();
  }
