...
class ViewColumnConnection : public NABasicObject {
 public:
  ...
  // Accessors
  const NAString *getColName() const { return key_; }
  const QualifiedName &getTableName() const { return baseColName_.getCorrNameObj().getQualifiedNameObj(); }
  const QualifiedName &getViewName() const { return viewColName_.getCorrNameObj().getQualifiedNameObj(); }
  const ColRefName &getViewColName() const { return viewColName_; }
  const ValueId *getValueId() const { return &vid_; }
  NABoolean isComplex() const { return isComplex_; }
  NABoolean isInvalid() const { return isInvalid_; }
...
class ViewTableConnection : public NABasicObject {
 public:
  ViewTableConnection(const QualifiedName &tableName, const QualifiedName &viewName, CollHeap *heap)
...
#ifndef NDEBUG
  void display() const;
  void print(FILE *ofd = stdout, const char *indent = DEFAULT_INDENT, const char *title = "ViewTableConnection") const;
#endif
...
class ViewColumnGraph : public NABasicObject {
 public:
  ...
  static int ValueIdHash(const ValueId &vid);
...
#ifndef NDEBUG
  void display() const;
  void print(FILE *ofd = stdout, const char *indent = DEFAULT_INDENT, const char *title = "ViewColumnGraph") const;
#endif
...
class MVVegPredicateColumn : public NABasicObject {
 public:
  ...
  // Ctor for DDL
  MVVegPredicateColumn(const BaseColumn *baseCol, NABoolean isInLeftJoin, NABoolean isFromVegRef,
                       ComMVSUsageType usageType, const ItemExpr *complexExpr, CollHeap *heap);

  // copy ctor
  MVVegPredicateColumn(const MVVegPredicateColumn &other, CollHeap *heap, NABoolean joinSideOverride = FALSE)
...
#ifndef NDEBUG
  void display() const;
  void print(FILE *ofd = stdout, const char *indent = DEFAULT_INDENT, const char *title = "MVVegPredicateColumn") const;
#endif
...
class MVVegPredicate : public LIST(const MVVegPredicateColumn *) {
 public:
  ...
  MVVegPredicate(NABoolean isOnLeftJoin, CollHeap *heap, ComMVSUsageType usageType = COM_UNKNOWN_USAGE)
  ...
  void addPredicateMember(ValueId vid, NABoolean isFromVegRef, const VEG *parentVeg, LIST(const VEG *) & vegPtrList,
                          CollHeap *heap);

  // Add all the columns of the VEG to the list.
  void addVegPredicates(const VEG *veg, NABoolean isFromVegRef, LIST(const VEG *) & vegPtrList, CollHeap *heap);
...
#ifndef NDEBUG
  void display() const;
  void print(FILE *ofd = stdout, const char *indent = DEFAULT_INDENT, const char *title = "MVVegPredicate") const;
#endif
...
class MVUsedObjectColNameMap : public NABasicObject {
 public:
  MVUsedObjectColNameMap(CollHeap *heap) : colNameHash_(hashKey, 50, FALSE, heap), colPositionTable_(NULL) {}
...
class MVUsedObjectInfo : public NABasicObject {
 public:
  ...
  void initColNameMap(const NATable *naTable, CollHeap *heap) { colNameMap_.initColNameMap(naTable, heap); }
...
#ifndef NDEBUG
  void display() const;
  void print(FILE *ofd = stdout, const char *indent = DEFAULT_INDENT, const char *title = "MVColumnInfo") const;
#endif
...
class MVColumnInfo : public NABasicObject {
 public:
  ...
  MVColumnInfo(MVInfoForDDL &mvInfoObj, ComMVType mvType, const ColumnDesc *colDesc, BindWA *bindWA, CollHeap *heap);

  // Ctor for MAV dependant system columns.
  MVColumnInfo(const MVColumnInfo &other, MVInfoForDDL &mvInfoObj, MVAggFunc aggIndex, ItemExpr *colExpr,
               CollHeap *heap);

  // Ctor for MAV GroupBy columns.
  MVColumnInfo(MVInfoForDDL &mvInfoObj, ItemExpr *groupingCol, CollHeap *heap);

  // Ctor for MJV clustering key system added columns.
  MVColumnInfo(MVInfoForDDL &mvInfoObj, NAColumn *clusteringKeyCol, CollHeap *heap);
...
class MVColumns : public NABasicObject {
 public:
  ...
  int entries() const  // Only real direct columns.
  {
    return directColumnList_.entries();
  }

  // returns # of direct + extra columns.
  int getTotalNumberOfColumns() const;
...
#ifndef NDEBUG
  void display() const;
  void print(FILE *ofd = stdout, const char *indent = DEFAULT_INDENT, const char *title = "MVColumns") const;
#endif
...
class MVInfo : public NABasicObject {
 public:
  ...
  MVInfo(const NAString &nameOfMV, ComMVRefreshType refreshType, NABoolean isRewriteEnabled, ComMVStatus mvStatus,
  ...
  MVInfo(const NAString &nameOfMV, ComMVRefreshType refreshType, NABoolean isRewriteEnabled, ComMVStatus mvStatus,
         const NAString &mvSelectText, CharInfo::CharSet mvSelectTextCharSet, NABoolean isIncremental, ComMVType mvType,
         LIST(MVUsedObjectInfo *) & usedObjectsList, MVColumnInfoList &directColumnList, NABoolean isLeftLinear,
  ...
  // - for showddl
  NAString getTextForShowddl(CatROMV *pMV, CharInfo::CharSet &showddlOutputCharSet /* out param */) const;
  //
  ...
  RelRoot *buildMVSelectTree(const NAString *alternativeText = NULL,
                             CharInfo::CharSet alternativeTextCharSet = CharInfo::UTF8) const;
  ...
  QRMVDescriptorPtr prepareMvDescriptorTree(NABoolean isIncrementalMV, NAString &warningMessage,
  ...
  NABoolean isIncrementalWorkNeeded() const {
    return (isIncremental() && getRefreshType() != COM_RECOMPUTE && getRefreshType() != COM_BY_USER);
  }

  // Display/print, for debugging.
#ifndef NDEBUG
  void display() const;
  virtual void print(FILE *ofd = stdout, const char *indent = DEFAULT_INDENT, const char *title = "MVInfo") const;
#endif
 ...
 protected:
  ...
  NABoolean addTableNamesToList(BindWA *bindWA, TableNameHash &usedObjectsNames);

  // - for showddl
  NAString getMVRefreshTypaAsString() const;
  NAString getInitializationAsString() const;
  NAString getOptionalNameClause(StmtDDLCreateMV *createMvNode, const NAString &mvText,
...
class MVInfoForDDL : public MVInfo {
 public:
  MVInfoForDDL(const NAString &NameOfMV, ComMVRefreshType refreshType, NABoolean isRewriteEnabled, ComMVStatus mvStatus,
               const NAString &mvSelectText, CharInfo::CharSet mvSelectTextCharSet,
  ...
  MV_NOT_INCREMENTAL getNotIncrementalReason() const { return notIncrementalReason_; }
  ...
  CollIndex scanNodesNumber() { return scanNodes_.entries(); }
 ...
 private:
  ...
  MVUsedObjectInfo *initUsedObject(Scan *scanNode, const NATable *naTable, BindWA *bindWA);
  ...
  void addUsedObjectsClusteringInfo(TableDesc *tDesc, BindWA *bindWA);
...
class MVInfoForDML : public MVInfo {
 public:
  MVInfoForDML(const NAString &NameOfMV, ComMVRefreshType refreshType, NABoolean isRewriteEnabled, ComMVStatus mvStatus,
  ...
  MVJoinGraph *initJoinGraph(BindWA *bindWA, const DeltaDefinitionPtrList *deltaDefList, NABoolean checkOnlyInserts);
