...
class TableAccess : public NABasicObject {
 public:
  ...
  inline NABoolean isEmpty() const { return empty_; }
  inline NABoolean isOpaque() const { return (!isEmpty() && (tableID_ == NULL)); }

  // print, for debugging.
  virtual void print(FILE *ofd = stdout, const char *indent = DEFAULT_INDENT, const char *title = "TableAccess") const;
...
class ReadTableAccess : public TableAccess {
 public:
  // Default ctor - to EMPTY.
  ReadTableAccess() : TableAccess() {}

  // Explicit ctor
  ReadTableAccess(const NATable *theTable) : TableAccess(theTable) {}

  // Copy ctor
  ReadTableAccess(const ReadTableAccess &other) : TableAccess(other) {}
  ...
  virtual NABoolean isReadAccess() const { return TRUE; }
...
class WriteTableAccess : public TableAccess {
 public:
  // Default ctor - to EMPTY.
  WriteTableAccess() : TableAccess() {}

  // Explicit ctor
  WriteTableAccess(const NATable *theTable) : TableAccess(theTable) {}

  // Copy ctor
  WriteTableAccess(const WriteTableAccess &other) : TableAccess(other) {}
  ...
  virtual NABoolean isReadAccess() const { return FALSE; }
...
class SubTreeAccessSet : public NABasicObject {
 public:
  ...
  // print, for debugging.
  void print(FILE *ofd = stdout, const char *indent = DEFAULT_INDENT, const char *title = "SubTreeAccessSet") const;
