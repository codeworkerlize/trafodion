...
class RenameTable : public Rename {
 public:
  ...
  NABoolean isDual() const { return isDual_; }
...
class ConnectBy : public RelExpr {
 ...
 private:
  ...
  ValueIdSet leftPathColList_;
  ValueIdSet rightPathColList_;
  ValueIdSet priorUsedCols_;
  ValueIdList priorPredicates_;
  ValueIdList priorConds_;
  ValueIdList priorVids_;
  ValueIdList priorHostVarsVids_;
  ValueIdList priorCols_;
  ConnectByMap *connectByMap_;
  ItemExpr *condExprTree_;
  ValueIdSet condExpr_;
  NAString pathDel_;
  ValueId pathVid_;
  ValueIdList colMapTable_;
