...
class CostScalar {
 public:
  ...
  CostScalar(const double dpv) : dpv_(dpv) { ASSERT_IS_A_VALID_COSTSCALAR; }

  // Copy constructor.
  CostScalar(const CostScalar &other) : dpv_(other.dpv_) { ASSERT_IS_A_VALID_COSTSCALAR; }
  ...
  // op + : simple
  inline CostScalar operator+(const CostScalar &other) const { return dpv_ + other.dpv_; }
  ...
  inline NABoolean operator==(const CostScalar &other) const {
    return ((_ABSOLUTE_VALUE_(dpv_ - other.dpv_) / (_ABSOLUTE_VALUE_(dpv_) + _ABSOLUTE_VALUE_(other.dpv_) + 1)) <
            COSTSCALAR_EPSILON);
  }
  ...
  // op < : see note above
  inline NABoolean operator<(const CostScalar &other) const { return (NOT(*this == other) AND(dpv_ < other.dpv_)); }
  //                    ^^^^^^^^^^^^^^^^^^^^
  //                      this is essential
  ...
  // op != : simple
  inline NABoolean operator!=(const CostScalar &other) const { return NOT(*this == other); }
  // op <= : don't use op< (avoid 2nd call to op==)
  inline NABoolean operator<=(const CostScalar &other) const { return (dpv_ < other.dpv_) OR(*this == other); }
  // op >= : don't use op> (avoid 2nd call to op==)
  inline NABoolean operator>=(const CostScalar &other) const { return (dpv_ > other.dpv_) OR(*this == other); }
  ...
  inline NABoolean isZero() const { return ((dpv_ >= MINUS_COSTSCALAR_EPSILON) AND(dpv_ <= COSTSCALAR_EPSILON)); }
  ...
  inline void roundIfZero() {
    if (isZero()) dpv_ = 0;
  }

  // roundExactly : round to zero if the value is *exactly* equal to zero
  inline void roundIfExactlyZero() {
    if (isExactlyZero()) dpv_ = 0;
  }
  ...
  inline long toLong() const { return (dpv_ > LONG_MAX ? LONG_MAX : (dpv_ < LONG_MIN ? LONG_MIN : (long)dpv_)); }
  ...
  inline NABoolean isGreaterThanZero() const { return (dpv_ > COSTSCALAR_EPSILON); }
  inline NABoolean isGreaterOrEqualThanZero() const { return (dpv_ >= MINUS_COSTSCALAR_EPSILON); }
  inline NABoolean isLessThanZero() const { return (dpv_ < MINUS_COSTSCALAR_EPSILON); }

  // isGreaterThanOne uses hardcoded constant that could be replaced later
  inline NABoolean isGreaterThanOne() const { return (dpv_ > 1.000001); }
  // isLessThanOne uses hardcoded constant that could be replaced later
  inline NABoolean isLessThanOne() const { return (dpv_ < .999999); }
