...
class Refresh : public BinderOnlyNode {
 public:
  ...
  // Ctor for RECOMPUTE
  Refresh(const QualifiedName &mvName, ComBoolean noDelete, CollHeap *oHeap = CmpCommon::statementHeap());

  // Ctor for SINGLEDELTA
  Refresh(const QualifiedName &mvName, const DeltaDefinitionPtrList *pDeltaDefList, NRowsClause *pOptionalNRowsClause,
  ...
  // Mutators
  void setAdditionalPhaseNeeded() { additionalPhaseNeeded_ = TRUE; }
  ...
  NABoolean areGroupByColsAnIndexPrefix(const NAColumnArray &groupByCols, const NAColumnArray &columnArray) const;
  ...
  MvRefreshBuilder *constructMavSpecifichBuilder(BindWA *bindWA, CorrName mvCorrName, MVInfoForDML *mvInfo);
  MvRefreshBuilder *constructMjvSpecifichBuilder(BindWA *bindWA, CorrName mvCorrName, MVInfoForDML *mvInfo);
  RelExpr *constructPipelinedBuilders(BindWA *bindWA, MvBindContext *bindContext, RelExpr *firstRefreshTree);
...
class DeltaDefIUDLog : public NABasicObject {
 public:
  ...
  DeltaDefIUDLog(Option option, IUDStatistics *optionalStat = NULL) : op_(option), optionalStat_(optionalStat) {}

  virtual ~DeltaDefIUDLog() { delete optionalStat_; }
...
class DeltaDefLogs : public NABasicObject {
 public:
  DeltaDefLogs(DeltaDefRangeLog *rangeDef, DeltaDefIUDLog *iudDef) : rangeDef_(rangeDef), iudDef_(iudDef) {}
...
class DeltaOptions : public NABasicObject {
 public:
  ...
  NABoolean getIsInsertOnly() { return pDeltaDefLogs_->getIUDDef()->getOption() == DeltaDefIUDLog::INSERT_ONLY; }
 ...
 public:
  ...
  NABoolean isInsertOnly() const {
    if (isNoDE())
      ...
      return (getIudDeletedRows() == 0) && (getIudUpdatedRows() == 0);
  }

  NABoolean isDeleteOnly() const {
    return !isNoDE() && !useRangeLog() && (getIudInsertedRows() == 0) && (getIudUpdatedRows() == 0);
  }
...
class PipelineClause : public NABasicObject {
 public:
  PipelineClause(QualNamePtrList *firstPipelineDef, PipelineDefPtrList *defList = NULL)
      : firstPipelineDef_(firstPipelineDef), pipelineDefList_(defList) {}

  virtual ~PipelineClause();

  const QualNamePtrList *getFirstPipelineDef() const { return firstPipelineDef_; }
...
class DeltaDefinitionPtrList : public LIST(DeltaDefinition *) {
 public:
  DeltaDefinitionPtrList(CollHeap *h = CmpCommon::statementHeap()) : LIST(DeltaDefinition *)(h) {}

  virtual ~DeltaDefinitionPtrList() {
    DeltaDefinition *deltaDef;
    while (getLast(deltaDef)) delete deltaDef;
  }
...
class PipelineDef : public NABasicObject {
 public:
  PipelineDef(QualifiedName *fromClause, QualNamePtrList *toClause) : fromClause_(fromClause), toClause_(toClause) {}
...
class PipelineDefPtrList : public LIST(PipelineDef *) {
 public:
  PipelineDefPtrList(CollHeap *h = CmpCommon::statementHeap()) : LIST(PipelineDef *)(h) {}

  virtual ~PipelineDefPtrList() {
    PipelineDef *def;
    while (getLast(def)) delete def;
  }
...
class IncrementalRefreshOption : public NABasicObject {
 public:
  ...
  IncrementalRefreshOption(DeltaDefinitionPtrList *pDefList, NRowsClause *pOptionalNRowsClause,
  ...
  void synthesize() {
    for (CollIndex i = 0; i < pDeltaDefList_->entries(); i++) ((DeltaDefinition *)pDeltaDefList_->at(i))->synthesize();
  }
