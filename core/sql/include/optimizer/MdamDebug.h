...
enum MdamTraceLevel { MDAM_TRACE_LEVEL_NONE, MTL1, MTL2, MTL3, MDAM_TRACE_LEVEL_ALL = 100 };
...
class MdamTrace {
 ...
 public:
  ...
  static void printCostObject(ScanOptimizer *opt, const Cost *cost, const char *msg);
  static void printFileScanInfo(ScanOptimizer *opt, const ValueIdSet &partKeyPreds);
  static void printTaskMonitor(TaskMonitor &monitor, const char *msg);
  static void printBasicCost(FileScanOptimizer *opt, SimpleCostVector &prefixFR, SimpleCostVector &prefixLR,
                             const char *msg);
...
#define MDAM_DEBUG0(l, m) \
  if (l <= MdamTrace::level()) MdamTrace::print((m))
#define MDAM_DEBUG1(l, m, a) \
  if (l <= MdamTrace::level()) MdamTrace::print((m), (a))
#define MDAM_DEBUG2(l, m, a, b) \
  if (l <= MdamTrace::level()) MdamTrace::print((m), (a), (b))
#define MDAM_DEBUG3(l, m, a, b, c) \
  if (l <= MdamTrace::level()) MdamTrace::print((m), (a), (b), (c))
#define MDAM_DEBUG4(l, m, a, b, c, d) \
  if (l <= MdamTrace::level()) MdamTrace::print((m), (a), (b), (c), (d))
#define MDAM_DEBUG5(l, m, a, b, c, d, e) \
  if (l <= MdamTrace::level()) MdamTrace::print((m), (a), (b), (c), (d), (e))
#define MDAM_DEBUGX(l, f)        \
  if (l <= MdamTrace::level()) { \
