...
class RAList {
 public:
  ...
  RAList(CollHeap *heap, CollIndex initLen = 0)
  ...
  inline CollIndex entries() const { return theList_.entries(); }
  ...
  // append a new entry
  inline void insert(const T &elem) { insertAt(entries(), elem); }
...
class VEGMember : public NABasicObject {
 public:
  ...
  // ---------------------------------------------------------------------
  VEGMember(const ValueId &memberValue, const ValueId &ofVEG, NABoolean deleteMeFlag = FALSE,
            NABoolean forwardingEntry = FALSE)
      : memberM_(memberValue), groupG_(ofVEG), deleteMeFlag_(deleteMeFlag), forwardingEntry_(forwardingEntry) {}
  ...
  void print(FILE *ofd = stdout, const char *indent = DEFAULT_INDENT, const char *title = "VEGMember");
...
class VEGRegion : public NABasicObject {
 public:
  VEGRegion(VEGTable *vegTable, VEGRegion *parentVEGRegionPtr, const RegionId newRegionId, enum VEGRegionTypeEnum tev,
      ...
      : stateEnum_(ACTIVE_STATE),
        ...
        parentRegion_(parentVEGRegionPtr),
        myRegionId_(newRegionId),
        ownerExpr_(ownerExpr),
        subtreeId_(subtreeId),
        vegTable_(vegTable),
  ...
  // ---------------------------------------------------------------------
  NABoolean isActive() const { return (stateEnum_ == ACTIVE_STATE); }
  NABoolean isToBeMerged() const { return (stateEnum_ == TO_BE_MERGED_STATE); }
  NABoolean isMerged() const { return (stateEnum_ == MERGED_STATE); }
  ...
  // VEGRegion::addZone()
  // ---------------------------------------------------------------------
  void addZone(VEGRegion *parentRegion, VEGRegion *newZone) { parentRegion->zones_.insert(newZone); }
  ...
  // method returns NULL.
  // ---------------------------------------------------------------------
  VEGReference *getVEGReferenceFromCurrentVEGRegion(const ValueId &exprId, const VEGRegion *ignoreThisChild = NULL,
  ...
  // method returns NULL.
  // ---------------------------------------------------------------------
  VEGReference *getVEGReferenceFromParentVEGRegion(const ValueId &exprId) const;
  ...
  RegionId getParentVEGRegionId() const { return parentRegion_ ? parentRegion_->getRegionId() : NULL_VEG_REGION; }
  ...
  void locateDescendantVEGRegionAndMarkToBeMerged(const RelExpr *owner, const ValueId &exprId);
  ...
  void markAsMerged() {
    CMPASSERT(stateEnum_ != MERGED_STATE);
    stateEnum_ = MERGED_STATE;
  }

  // ---------------------------------------------------------------------
  // Delete the VEGMember that contains the given ValueId
  // ---------------------------------------------------------------------
  void deleteVEGMember(const ValueId &memberId) { replaceVEGMember(memberId); }
  ...
  void print(FILE *ofd = stdout, const char *indent = DEFAULT_INDENT, const char *title = "VEGRegion");
 ...
 private:
  ...
  // ---------------------------------------------------------------------
  enum VEGRegionStateEnum { ACTIVE_STATE, TO_BE_MERGED_STATE, MERGED_STATE };
  ...
  // Allocate a new Zone
  // ---------------------------------------------------------------------
  VEGRegion *allocateNewZone(const VEGRegionTypeEnum tev, const ExprNode *const ownerExpr);
  ...
  // ---------------------------------------------------------------------
  void fixupActiveZones(VEGRegion *activeParentRegion, const ValueIdSet &outerReferences);
  ...
  // ---------------------------------------------------------------------
  void mergeZonesFromSameVEGRegion(VEGRegion *fromRegion, VEGRegion *parentRegion = NULL);
  ...
  // ---------------------------------------------------------------------
  void replaceVEGMember(const ValueId &existingMemberId, const ValueId &newMemberId = NULL_VALUE_ID);
...
class VEGTable : public NABasicObject {
 public:
  ...
  // ---------------------------------------------------------------------
  RegionId getCurrentVEGRegionId() const {
    return getCurrentVEGRegion() ? getCurrentVEGRegion()->getRegionId() : NULL_VEG_REGION;
  }
  ...
  // ---------------------------------------------------------------------
  NABoolean locateVEGRegionAndCheckIfMerged(const ExprNode *const ownerExpr) {
  ...
  // ---------------------------------------------------------------------
  ItemExpr *getVEGReference(const ValueId &exprId, const VEGRegion *searchThisVegRegionFirst = NULL) const;
  ...
  // ---------------------------------------------------------------------
  ItemExpr *performTC(const ValueId &memberOfVEG) {
    return getVEGRegion(getCurrentVEGRegionId())->performTC(memberOfVEG);
  }
  ...
  void print(FILE *ofd = stdout, const char *indent = DEFAULT_INDENT, const char *title = "VEGTable");
