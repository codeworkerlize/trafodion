...
class ValueIdUnion : public ItemExpr {
 ...
 private:
  ...
  // child are distinct from the values coming from the right.  This
  // only supports labels consisting of one column.
  // For labels that extend across multiple columns, we would need a
  // list of structs with a list and a list of lists in the Union, and
...
class RangeSpecRef : public ItemExpr {
 public:
  ...
  // high row count.
  //
  // However, the ItemExpr passed to the RangeSpecRef constructor has
