...
class ExSortTcb : public ex_tcb {
 public:
  ...
  enum SortPhase { SORT_PREP_PHASE, SORT_SEND_PHASE, SORT_RECV_PHASE, SORT_MERGE_PHASE, SORT_PHASE_END };
 ...
 protected:
  ...
  static ExWorkProcRetcode sCancel(ex_tcb *tcb) { return ((ExSortTcb *)tcb)->cancel(); }

  short sortSend(ex_queue_entry *centry, ex_queue::up_status child_status, ex_queue_entry *pentry_down,
  ...
  NABoolean resizeCifRecord() const { return sortCfg_->resizeCifRecord(); }

  NABoolean considerBufferDefrag() const { return sortCfg_->considerBufferDefrag(); }

  Int16 numberOfBytesForRecordSize() const { return sortCfg_->numberOfBytesForRecordSize(); }
 ...
 public:
  ...
  static short sWorkUp(ex_tcb *tcb) { return ((ExSortTcb *)tcb)->workUp(); }
  static short sWorkDown(ex_tcb *tcb) { return ((ExSortTcb *)tcb)->workDown(); }
...
class ExSortBufferPool : public NABasicObject {
 public:
  enum PoolType { SIMPLE_BUFFER_TYPE, SQL_BUFFER_TYPE };
  ...
  inline tupp_descriptor *get_free_tupp_descriptor(int tupDataSize, SqlBuffer **buf = NULL) {
    if (sqlBufferPool_)
      ...
      if (!simplePool_->getFreeTuple(tp_)) return tp_.get_tupp_descriptor();
    }
    return NULL;
  }
  ...
  SqlBufferBase *addBuffer(int totalBufferSize, bool failureIsFatal = true) {
    ...
    if (sqlBufferPool_) {
      ...
      if (bmoStats_) bmoStats_->setSpaceBufferCount(sqlBufferPool_->get_number_of_buffers());
    }
    return buf;
  }
