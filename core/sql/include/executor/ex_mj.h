...
class ex_mj_tcb : public ex_tcb {
 ...
 public:
  ...
  // Constructor
  ex_mj_tcb(const ex_mj_tdb &mj_tdb, const ex_tcb &left_tcb, const ex_tcb &right_tcb, ex_globals *glob);
 ...
 protected:
  ...
  inline bool isOverflowEnabled() const { return mjTdb().isOverflowEnabled(); }

  inline ex_expr *mergeExpr() const { return mjTdb().mergeExpr_; }

  inline ex_expr *compExpr() const { return mjTdb().compExpr_; }

  inline ex_expr *preJoinExpr() const { return mjTdb().preJoinExpr_; }

  inline ex_expr *postJoinExpr() const { return mjTdb().postJoinExpr_; }

  inline ex_expr *leftCheckDupExpr() const { return mjTdb().leftCheckDupExpr_; }

  inline ex_expr *rightCheckDupExpr() const { return mjTdb().rightCheckDupExpr_; }

  inline ex_expr *ljExpr() const { return mjTdb().ljExpr_; }

  inline ex_expr *rightCopyDupExpr() const { return mjTdb().rightCopyDupExpr_; }

  inline ex_expr *leftEncodedKeyExpr() const { return mjTdb().leftEncodedKeyExpr(); }

  inline ex_expr *rightEncodedKeyExpr() const { return mjTdb().rightEncodedKeyExpr(); }

  Comparison compareTuples(ex_queue_entry *lentry, ex_queue_entry *rentry, bool doFullComparison = true);
  ...
  ex_expr::exp_return_type returnRow(atp_struct *leftAtp, atp_struct *rightAtp, ex_mj_private_state &pstate,
                                     bool isUniqueMj = false, ExOperStats *statsEntry = NULL);
...
inline bool ex_mj_tcb::noSavedDups(void) {
...
class ex_mj_unique_tcb : public ex_mj_tcb {
 public:
  // Constructor
  ex_mj_unique_tcb(const ex_mj_tdb &mj_tdb, const ex_tcb &left_tcb, const ex_tcb &right_tcb, ex_globals *glob);
