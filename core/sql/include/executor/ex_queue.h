...
class ex_queue : public ExGod {
 public:
  ...
  ex_queue(const queue_type type, queue_index initialSize, ex_cri_desc *criDesc, CollHeap *space,
  ...
  void allocatePstate(ex_tcb_private_state *p, ex_tcb *tcb);
  ...
  //  get the last occupied entry. Null if the queue is empty.
  ex_queue_entry *getLastOccupiedEntry() const;
  ...
  void setCancelSubtask(ExSubtask *cancelSubtask) {
    ExQueueAssert(upDown_ == DOWN_QUEUE, "");
    cancelSubtask_ = cancelSubtask;
  }
  inline void setResizeSubtask(ExSubtask *resizeSubtask) { resizeSubtask_ = resizeSubtask; }

  void setNextSubtask(ExSubtask *nextSubtask) {
    ExQueueAssert(upDown_ == DOWN_QUEUE, "");
    nextSubtask_ = nextSubtask;
  }
...
class down_state  // public down status
{
 public:
  ...
  inline NABoolean operator==(const down_state &other) const {
    return (request == other.request) && (requestValue == other.requestValue) && (parentIndex == other.parentIndex) &&
...
class up_state  // public up state
{
 ...
 public:
  inline NABoolean operator==(const up_state &other) const {
    return (parentIndex == other.parentIndex) && (status == other.status);
  };
...
inline ex_cri_desc *ex_queue_entry::criDesc() const { return atp_->getCriDesc(); }

inline unsigned short ex_queue_entry::numTuples() const { return criDesc()->noTuples(); }
...
inline void ex_queue_entry::copyAtp(atp_struct *from) { atp_->copyAtp(from); }

inline ComDiagsArea *ex_queue_entry::getDiagsArea() const { return atp_->getDiagsArea(); }

inline void ex_queue_entry::setDiagsArea(ComDiagsArea *diagsArea) { atp_->setDiagsArea(diagsArea); }
...
inline queue_index ex_queue::getTailIndex() const { return tail_; }

inline ex_queue_entry *ex_queue::getLastOccupiedEntry() const {
  if (head_ == tail_) return NULL;

  if (tail_)
    return &queue_[(tail_ & mask_) - 1];
  else  // tail_ == 0 means the last element is at queue_[size-1]
...
inline ex_queue_entry *ex_queue::getQueueEntry(const queue_index i) const {
  ExQueueAssert(!isVacant(i), "ex_queue::getQueueEntry getting an empty entry");
  return &queue_[i & mask_];  //(size_-1)];
}

inline ex_queue_entry *ex_queue::getHeadEntry() const {
  // make sure the head_ is not an empty entry
  ExQueueAssert(head_ != tail_, "ex_queue::getHeadEntry() get head on an empty queue");
...
inline void ex_queue::insert() {
  ...
  ExQueueAssert(queue_[tail_ & mask_].checkState(upDown_), "ex_queue::insert() Inserting an empty entry into a queue");
#ifdef _DEBUG
  logInsert();
#endif
...
inline queue_index ex_queue::getSize() const { return size_; }

inline ex_cri_desc *ex_queue::getCriDesc() { return criDesc_; };

inline void atp_struct::copyAtp(const ex_queue_entry *from) { copyAtp(from->getAtp()); }
...
inline void up_state::setDoneNoRowsMatchNo(void) { matchNo = GetDoneNoRowsMatchNo; }
