...
class ExOnljTcb : public ex_tcb {
  ...
  NABoolean processNonFatalErrorsInLeftUpQueue(NABoolean &project, NABoolean &consumed);
  ...
  static short sWorkPhase1(ex_tcb *tcb) { return ((ExOnljTcb *)tcb)->work_phase1(); }
...
class ExOnljPrivateState : public ex_tcb_private_state {
  ...
  NABoolean rowAlreadyRaisedNFError_;  // to remember that this particular row has already raised a NF error (used to
                                       // keep track of rowsAfeected correctly )
  // BertBert VV
