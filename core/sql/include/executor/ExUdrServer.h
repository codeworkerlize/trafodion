...
protected:
  ...
  // At this time we don't store the tenant id or name with the UDR
  // server. We rely on two things to make this unnecessary:
  // a) a UDR server can only serve one CLI context (connection)
  //    at a time and,
  // b) when an ODBC/JDBC client disconnects from the context,
  //    no prepared statements can survive.
  //
  // This means that all the statements executing in a UDR server at a
  // given time belong to the same connection and therefore to the
  // same tenant. When we load a statement into a UDR server, we make
  // sure to set the tenant id.
  ...
  // thread info for multi-threaded IPC, UDR servers cannot
