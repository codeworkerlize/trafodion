...
class TupleSpace : public NABasicObject {
 public:
  ...
  TupleSpace(ByteCount tupleLen, BufferCount nBuffers, ByteCount bufferSize, UInt32 memoryQuotaMB, ex_expr *copyExpr,
             ex_cri_desc *criDesc, NAMemory *heap, bool enableOverflow, ExExeStmtGlobals *stmtGlobals,
             UInt16 scratchThresholdPct, ScratchOverflowMode ovMode, bool yieldQuota);
  ~TupleSpace(void);
 ...
 private:
  ...
  enum ReadBufferType { BT_INVALID, BT_CACHE_BUFFER, BT_SWAP_BUFFER, BT_WRITE_BUFFER };
...
inline bool TupleSpace::empty(void) { return (nTuples_ == 0); }

inline bool TupleSpace::canReadTuples(void) { return (read_.bytesAvailable() >= tupleLen_); }

inline bool TupleSpace::isReadBuffer(char *buffer) { return BufferReference::sameBuffer(read_, buffer); }

inline bool TupleSpace::isReadBuffer(const BufferReference &br) { return BufferReference::sameBuffer(read_, br); }

inline bool TupleSpace::isWriteBuffer(char *buffer) { return BufferReference::sameBuffer(write_, buffer); }

inline bool TupleSpace::isWriteBuffer(const BufferReference &br) { return BufferReference::sameBuffer(write_, br); }
