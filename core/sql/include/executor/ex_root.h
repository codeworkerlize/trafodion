...
class ex_root_tdb : public ComTdbRoot {
 public:
  ...
  // ---------------------------------------------------------------------
  virtual ex_tcb *build(CliGlobals *cliGlobals, ex_globals *globals);
...
class ex_root_tcb : public ex_tcb {
 ...
 public:
  ex_root_tcb(const ex_root_tdb &root_tdb, const ex_tcb &child_tdb, ExExeStmtGlobals *glob);
  ...
  // to the cli.
  inline ComDiagsArea *moveDiagsAreaFromEntry(ex_queue_entry *entry) { return ex_tcb::moveDiagsAreaFromEntry(entry); }

  inline ex_root_tdb &root_tdb() const { return (ex_root_tdb &)tdb; };
  ...
  void getInputData(char *&inputData, int &inputDatalen);
  ...
  void setQueryStartedMsgStreamIsInvalid() { queryStartedStream_ = NULL; }

  // This is a way to let the root tcb know that cleanup has already been done.
  void setQueryFinishMsgStreamIsInvalid() { queryFinishedStream_ = NULL; }

  // Used by Statement to know whether I/O needs to complete.
  enum CancelBrokerCommStatus { STARTED_PENDING_ = 0x0001, FINISHED_PENDING_ = 0x0002 };

  bool isCbStartedMessageSent() const { return (cbCommStatus_ & STARTED_PENDING_) ? true : false; };

  bool isCbFinishedMessageSent() const { return (cbCommStatus_ & FINISHED_PENDING_) ? true : false; };

  bool anyCbMessages() const { return isCbStartedMessageSent() || isCbFinishedMessageSent(); };

  void setCbStartedMessageSent() { cbCommStatus_ |= STARTED_PENDING_; }

  void setCbStartedMessageReplied() { cbCommStatus_ &= ~STARTED_PENDING_; }

  void setCbFinishedMessageSent() { cbCommStatus_ |= FINISHED_PENDING_; }

  void setCbFinishedMessageReplied();

  void clearCbMessageStatus() { cbCommStatus_ = 0; }

  bool needsDeregister() { return (isCbStartedMessageSent() && !isCbFinishedMessageSent()); }
 ...
 private:
  ...
  // static version
  static ExWorkProcRetcode sWorkOnFragDir(ex_tcb *tcb) { return ((ex_root_tcb *)tcb)->workOnFragDir(); }
...
class QueryStartedMsgStream : public IpcMessageStream {
 public:
  // constructor
  QueryStartedMsgStream(IpcEnvironment *env, ex_root_tcb *rootTcb, ExSsmpManager *ssmpManager)
...
class QueryFinishedMsgStream : public IpcMessageStream {
 public:
  // constructor
  QueryFinishedMsgStream(IpcEnvironment *env, ex_root_tcb *rootTcb, ExSsmpManager *ssmpManager)
