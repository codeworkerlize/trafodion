...
public:
  ...
  static NAMemory* makeSharedHeap(LockController* controller, 
...
protected:
  ...
  NABoolean isMe() 
  {
     return (!strncmp(SHAREDCACHE_DB_EYE_CATCHER, eyeCatcher_, 4) && 
             sharedHeap_ && sizeOfSharedHeap_ == sizeof(*sharedHeap_));
  }
...
protected:
  ...
  NABoolean isMe() 
  {
     return (!strncmp(DESCRIPTOR_CACHE_EYE_CATCHER, eyeCatcher_, 4) && 
             table_ && sizeOfTable_ == sizeof(*table_));
  }

  NABoolean sanityCheck() 
  { 
    return ( table_ && table_->sanityCheck() ); 
  }
...
protected:
  ...
  {
    return (
        memoryTableDB_->tableNameToCacheEntryMap()->isComplete());
  }
  ...
  NABoolean sanityCheck()
  {
    return (memoryTableDB_ && memoryTableDB_->tableNameToCacheEntryMap()->sanityCheck());
  }
...
#define LOCK_SHARED_CACHE \
  ...
  if ( retcode == 0 ) \
  { \
    SharedCacheDB* sharedDB = \
     SharedCacheDB::locateOrCreate(&namedSemaphore);\
  }
...
#define LOCK_SHARED_CACHE \
  ...
  if (CmpCommon::context() && \
      CURRCONTEXT_CLUSTERINFO->getNumVirtualNodes() > 0) { \
  ...
  { \
    SharedCacheDB* sharedDB = \
     SharedCacheDB::locateOrCreate(&dlock);\
  }
...
#define OBSERVE_SHARED_CACHE_LOCK                                \
  SemaphoreForSharedCache *namedSemaphorePtr =                   \
      new SemaphoreForSharedCache();                             \
  retcode = namedSemaphorePtr->lockSemaphore(true);              \
  DistributedLockObserver observer(SHARED_CACHE_DLOCK_KEY);      \
  {                                                              \
    SharedCacheDB *sharedDB =                                    \
        SharedCacheDB::locateOrCreate(&observer);                \
    if (sharedDB)                                                \
    {                                                            \
...
#define OBSERVE_SHARED_DATA_CACHE_LOCK                                            \
  SemaphoreForSharedCache *namedSemaphorePtr =                                    \
      new SemaphoreForSharedCache(true);                                          \
  retcode = namedSemaphorePtr->lockSemaphore(true);                               \
  DistributedLockObserver observer(SHARED_CACHE_DATA_DLOCK_KEY);                  \
  {                                                                               \
    SharedCacheDB *sharedDB =                                                     \
        SharedCacheDB::locateOrCreate(&observer, SharedCacheDB::TABLEDATA_CACHE); \
    if (sharedDB)                                                                 \
    {                                                                             \
      sharedDB->getSharedHeap()->setNamedSem(namedSemaphorePtr);                  \
    }                                                                             \
