...
enum MonarchTableType { RANGE_PARTITIONED = 0, HASH_PARTITIONED };

// **** note:  QString structure moved to CmpSeabaseDDLmdcommon.h
...
enum SeabaseAuthsFlags {
  SEABASE_IS_ADMIN_ROLE = 0x0000000000000001,
  SEABASE_IS_TENANT_AUTH = 0x0000000000000002,
  SEABASE_IS_CONFIG_PART1 = 0x0000000000000004,
  SEABASE_IS_CONFIG_PART2 = 0x0000000000000008,
  SEABASE_IS_CONFIG_PART3 = 0x0000000000000010,
  SEABASE_IS_CONFIG_PART4 = 0x0000000000000020,
  SEABASE_IS_AUTO_CREATED = 0X0000000000000040
};
...
enum SeabaseColumnsFlags {
  ...
  // ARRAY or ROW column
  SEABASE_COLUMN_IS_COMPOSITE = 0x0000000000000004,

  SEABASE_COLUMN_IS_REPLICA = 0x0000000000000008,

  // ARRAY or ROW column
  SEABASE_COLUMN_IS_TYPE_VARCHAR2 = 0x0000000000000010,
};
...
static const QString seabaseIndexesDDL[] = {{" create table " SEABASE_INDEXES " "},
                                            {" ( "},
...
enum SeabaseIndexFlags {
  MD_IS_NORMAL_INDEX_FLG = 0x0001,          // normal index
  MD_IS_LOCAL_BASE_PARTITION_FLG = 0x0002,  // local index on parttiion base table
  MD_IS_LOCAL_PARTITION_FLG = 0x0004,       // local index on each parttiion table
  MD_IS_GLOBAL_PARTITION_FLG = 0x0008,      // gloabal index on partition
};
...
static const QString seabaseLibrariesUsageDDL[] = {{" create table " SEABASE_LIBRARIES_USAGE " "},
                                                   {" ( "},
...
static const QString seabaseLobColumnsDDL[] = {
    {" create table " SEABASE_LOB_COLUMNS " "},
    {" ( "},
    {"   table_uid largeint not null not serialized, "},
    {"   lobnum int not null not serialized, "},
    {"   storagetype smallint not null not serialized, "},
    {"   location varchar(4096) character set iso88591 not null not serialized, "},
    {"   column_name varchar(256 bytes) character set utf8 not null not serialized, "},
    {"   num_lob_files int not null not serialized, "},
    {"   inlined_data_maxlen int not null not serialized, "},
    {"   hbase_data_maxlen largeint not null not serialized, "},
    {"   lob_chunk_maxlen int not null not serialized, "},
    {"   flags largeint not null not serialized "},
    {" ) "},
    {" primary key (table_uid, lobnum) "},
    {" attribute aligned format storage hbase namespace 'RSRVD_1' "},
    {" ; "}};
...
static const QString seabaseTablePartitionDDL[] = {
    {" create table %s.\"%s\"." SEABASE_TABLE_PARTITION " "},
    {" ( "},
    {" table_uid largeint not null not serialized, "},
    {" partition_type int not null not serialized, "},
    {" partition_column varchar(512 bytes) character set utf8 not null not serialized, "},
    {" partition_column_pos varchar(128) character set iso88591 not null not serialized, "},
    {" partition_column_num int not null not serialized, "},
    {" partition_interval varchar(256 bytes) character set utf8 not null not serialized, "},
    {" partition_autolist char(1) character set iso88591 not null not serialized, "},
    {" subpartition_type int not null not serialized, "},
    {" subpartition_column varchar(512 bytes) character set utf8 not null not serialized, "},
    {" subpartition_column_pos varchar(128) character set iso88591 not null not serialized, "},
    {" subpartition_column_num int not null not serialized, "},
    {" subpartition_interval varchar(256 bytes) character set utf8 not null not serialized, "},
    {" subpartition_autolist char(2) character set iso88591 not null not serialized, "},
    {" flags largeint not null not serialized "},
    {" ) "},
...
static const QString seabasePartitionsDDL[] = {
    {" create table %s.\"%s\"." SEABASE_PARTITIONS " "},
    {" ( "},
    {" parent_uid largeint not null not serialized, "},
    {" partition_name varchar(256 bytes) character set utf8 not null not serialized, "},
    {" partition_entity_name varchar(256 bytes) character set utf8 not null not serialized, "},
    {" partition_uid largeint not null not serialized, "},
    {" partition_level char(1) character set iso88591 not null not serialized, "},
    {" partition_ordinal_position int not null not serialized, "},
    {" partition_expression varchar(1000 bytes) character set utf8 not null not serialized, "},
    {" status char(2) character set iso88591 not null not serialized, "},
    {" readonly char(2) character set iso88591 not null not serialized, "},
    {" inmemory char(2) character set iso88591 not null not serialized, "},
    {" def_time largeint not null not serialized, "},
    {" flags largeint not null not serialized, "},
    {" v1 varchar(32 bytes) character set utf8 not null not serialized, "},
    {" v2 varchar(64 bytes) character set utf8 not null not serialized, "},
    {" v3 varchar(128 bytes) character set utf8 not null not serialized, "},
    {" v4 varchar(256 bytes) character set utf8 not null not serialized "},
    {" ) "},
    {" primary key (parent_uid, partition_name) "},
    {" attribute hbase format storage hbase "},
    {" ; "}};
...
enum SeabaseObjectsFlags {
  ...
  // if this object is to be encrypted at insert time
  MD_OBJECTS_ENCRYPT_ROWID_FLG = 0x0000000000000040,
  MD_OBJECTS_ENCRYPT_DATA_FLG = 0x0000000000000080,

  // set if a backup or restore operation is in progress on this table.
  // Set when operation starts and reset after it completes successfully.
  // Table is inaccessible as long as this flag is set.
  // Backup tag is also set in TEXT table during this operation.
  // It is removed on successful completion of backup or restore.
  // See ComTextType in ComSmallDefs.h.
  MD_OBJECTS_BR_IN_PROGRESS = 0x0000000000000100,

  // if set, this object is enabled for incremental backup
  MD_OBJECTS_INCR_BACKUP_ENABLED = 0x0000000000000200,

  // if set, the table include trigger
  MD_OBJECTS_INCLUDE_TRIGGER = 0x0000000000000400,

  // if set, truncate/purgedata in progress
  MD_OBJECTS_TRUNCATE_IN_PROGRESS = 0x0000000000000800,

  // hash/range/list partition table
  MD_PARTITION_TABLE_V2 = 0x0000000000001000,

  // base table used for each partition
  MD_PARTITION_V2 = 0x0000000000002000,

  // if set , table is read_only
  MD_OBJECTS_READ_ONLY_ENABLED = 0x0000000000004000,
};
...
static const QString seabaseRefConstraintsDDL[] = {
    ...
    {"   match_option char(2) character set iso88591 not null not serialized, "},
...
static const QString seabaseRoutinesDDL[] = {
    ...
    {"   udr_type char(2) character set iso88591 not  null not serialized, "},
...
static const QString seabaseSeqGenDDL[] = {{" create table " SEABASE_SEQ_GEN " "},
                                           {" ( "},
...
enum SeabaseSeqGenFlags {
  MD_SEQGEN_REPL_SYNC_FLG = 0x0001,
  MD_SEQGEN_ORDER_FLG = 0x0002,
  MD_SEQGEN_REPL_ASYNC_FLG = 0x0004,
};

static const QString seabaseTablesDDL[] = {{" create table " SEABASE_TABLES " "},
                                           {" ( "},
                                           {"   table_uid largeint not null not serialized, "},
...
enum SeabaseTablesFlags {
  ...
  MD_TABLES_STORAGE_BIGTABLE_FLG = 0x0020,

  // if set, composite cols are stored in exploded format.
  // Otherwise stored in aligned format.
  // Set during table creation based on cqd traf_composite_datatype_format.
  MD_TABLES_COMP_COLS_IN_EXPLODED_FORMAT = 0x0040,

  // lob columns in this table have new format
  MD_TABLES_LOB_VERSION2 = 0x0080,
};
...
static const QString seabaseTableConstraintsIdxIndexDDL[] = {
    {" create index " SEABASE_TABLE_CONSTRAINTS_IDX " on " TRAFODION_SYSCAT_LIT ".\"" SEABASE_MD_SCHEMA
...
static const QString seabaseTextDDL[] = {{" create table " SEABASE_TEXT " "},
                                         {" ( "},
...
static const QString seabaseViewsDDL[] = {{" create table " SEABASE_VIEWS " "},
                                          {" ( "},
...
static const ComTdbVirtTableRoutineInfo seabaseMDCreateSPSQLInfo = {"CREATESPSQL",
                                                                    COM_PROCEDURE_TYPE_LIT,
                                                                    COM_LANGUAGE_JAVA_LIT,
                                                                    1,
                                                                    COM_READS_SQL_LIT,
                                                                    0,
                                                                    0,
                                                                    COM_STYLE_JAVA_CALL_LIT,
                                                                    COM_NO_TRANSACTION_REQUIRED_LIT,
                                                                    0,
                                                                    0,
                                                                    "org.trafodion.sql.udr.spsql.SPSQL.createSPSQL",
                                                                    COM_ROUTINE_NO_PARALLELISM_LIT,
                                                                    " ",
                                                                    COM_ROUTINE_EXTERNAL_SECURITY_INVOKER_LIT,
                                                                    COM_ROUTINE_SAFE_EXECUTION_LIT,
                                                                    " ",
                                                                    1,
                                                                    "(Ljava/lang/String;)V",
                                                                    " ",
...
static const ComTdbVirtTableColumnInfo seabaseMDCreateSPSQLColInfo[] = {
    {"BODY",
     0,
     COM_USER_COLUMN,
     REC_BYTE_V_ASCII,
     ...
     "1",
     COM_INPUT_COLUMN_LIT,
     0},
};

static const ComTdbVirtTableRoutineInfo seabaseMDDropSPSQLInfo = {"DROPSPSQL",
                                                                  COM_PROCEDURE_TYPE_LIT,
                                                                  COM_LANGUAGE_JAVA_LIT,
                                                                  1,
                                                                  COM_READS_SQL_LIT,
                                                                  ...
                                                                  0,
                                                                  0,
                                                                  "org.trafodion.sql.udr.spsql.SPSQL.dropSPSQL",
                                                                  COM_ROUTINE_NO_PARALLELISM_LIT,
                                                                  " ",
                                                                  COM_ROUTINE_EXTERNAL_SECURITY_INVOKER_LIT,
                                                                  COM_ROUTINE_SAFE_EXECUTION_LIT,
...
static const ComTdbVirtTableColumnInfo seabaseMDDropSPSQLColInfo[] = {
    {"NAME",
     0,
     COM_USER_COLUMN,
     REC_BYTE_V_ASCII,
...
static const ComTdbVirtTableRoutineInfo seabaseMDCallSPSQLInfo = {"CALLSPSQL",
                                                                  COM_PROCEDURE_TYPE_LIT,
                                                                  COM_LANGUAGE_JAVA_LIT,
                                                                  1,
                                                                  COM_MODIFIES_SQL_LIT,
                                                                  0,
                                                                  0,
                                                                  COM_STYLE_JAVA_CALL_LIT,
                                                                  COM_NO_TRANSACTION_REQUIRED_LIT,
                                                                  0,
                                                                  0,
                                                                  "org.trafodion.sql.udr.spsql.SPSQL.callSPSQL",
                                                                  COM_ROUTINE_NO_PARALLELISM_LIT,
                                                                  " ",
                                                                  COM_ROUTINE_EXTERNAL_SECURITY_INVOKER_LIT,
                                                                  COM_ROUTINE_SAFE_EXECUTION_LIT,
...
static const ComTdbVirtTableColumnInfo seabaseMDCallSPSQLColInfo[] = {
    {"NAME",
     0,
     COM_USER_COLUMN,
     REC_BYTE_V_ASCII,
...
static const ComTdbVirtTableRoutineInfo seabaseMDCallSPSQLTriggerInfo = {
    "CALLSPSQLTRIGGER",
    COM_PROCEDURE_TYPE_LIT,
    COM_LANGUAGE_JAVA_LIT,
    1,
    ...
    0,
    COM_STYLE_JAVA_CALL_LIT,
    COM_TRANSACTION_REQUIRED_LIT,
    0,
    0,
    "org.trafodion.sql.udr.spsql.SPSQL.callSPSQLTrigger",
    COM_ROUTINE_NO_PARALLELISM_LIT,
    " ",
    COM_ROUTINE_EXTERNAL_SECURITY_INVOKER_LIT,
    COM_ROUTINE_SAFE_EXECUTION_LIT,
    " ",
    1,
    "(Ljava/lang/String;IILjava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;ILjava/lang/"
    "String;ILjava/lang/String;ILjava/lang/String;)V",
    " ",
    0};

// now support max values length is 16M
static const ComTdbVirtTableColumnInfo seabaseMDCallSPSQLTriggerColInfo[] = {
    {"NAME",
     0,
     COM_USER_COLUMN,
     REC_BYTE_V_ASCII,
...
static const ComTdbVirtTableRoutineInfo seabaseMDExecuteSPSQLInfo = {"EXECUTESPSQL",
                                                                     COM_PROCEDURE_TYPE_LIT,
                                                                     COM_LANGUAGE_JAVA_LIT,
                                                                     1,
                                                                     COM_READS_SQL_LIT,
                                                                     ...
                                                                     0,
                                                                     0,
                                                                     "org.trafodion.sql.udr.spsql.SPSQL.executeSPSQL",
                                                                     COM_ROUTINE_NO_PARALLELISM_LIT,
                                                                     " ",
                                                                     COM_ROUTINE_EXTERNAL_SECURITY_INVOKER_LIT,
                                                                     COM_ROUTINE_SAFE_EXECUTION_LIT,
...
static const ComTdbVirtTableColumnInfo seabaseMDExecuteSPSQLColInfo[] = {
    {"BODY",
     0,
     COM_USER_COLUMN,
     REC_BYTE_V_ASCII,
...
static const SPSQLProc allSPSQLProcs[] = {
    {&seabaseMDCreateSPSQLInfo, seabaseMDCreateSPSQLColInfo,
     sizeof(seabaseMDCreateSPSQLColInfo) / sizeof(ComTdbVirtTableColumnInfo)},
    {&seabaseMDDropSPSQLInfo, seabaseMDDropSPSQLColInfo,
     sizeof(seabaseMDDropSPSQLColInfo) / sizeof(ComTdbVirtTableColumnInfo)},
    {&seabaseMDCallSPSQLInfo, seabaseMDCallSPSQLColInfo,
     sizeof(seabaseMDCallSPSQLColInfo) / sizeof(ComTdbVirtTableColumnInfo)},
    {&seabaseMDCallSPSQLTriggerInfo, seabaseMDCallSPSQLTriggerColInfo,
     sizeof(seabaseMDCallSPSQLTriggerColInfo) / sizeof(ComTdbVirtTableColumnInfo)},
    {&seabaseMDExecuteSPSQLInfo, seabaseMDExecuteSPSQLColInfo,
     sizeof(seabaseMDExecuteSPSQLColInfo) / sizeof(ComTdbVirtTableColumnInfo)}};
...
static const ComTdbVirtTableRoutineInfo seabaseMDValidateRoutineInfo = {
    ...
    0,
    0,
    "org.trafodion.sql.udr.LmUtility.validateMethod",
    COM_ROUTINE_NO_PARALLELISM_LIT,
    " ",
    COM_ROUTINE_EXTERNAL_SECURITY_INVOKER_LIT,
    COM_ROUTINE_SAFE_EXECUTION_LIT,
    " ",
    1,
    "(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;III[Ljava/lang/String;[I[Ljava/lang/"
    "String;)V",
    " ",
    0};

static const ComTdbVirtTableColumnInfo seabaseMDValidateRoutineColInfo[] = {{"CLASSNAME",
                                                                             0,
                                                                             COM_USER_COLUMN,
                                                                             REC_BYTE_V_ASCII,
                                                                             256,
...
struct MDTableInfo {
  ...
  const NABoolean neededForInitTraf;
};

static const MDTableInfo allMDtablesInfo[] = {
    {SEABASE_AUTHS, seabaseAuthsDDL, sizeof(seabaseAuthsDDL), NULL, 0, FALSE, TRUE},

    {SEABASE_COLUMNS, seabaseColumnsDDL, sizeof(seabaseColumnsDDL), NULL, 0, FALSE, TRUE},

    {SEABASE_DEFAULTS, seabaseDefaultsDDL, sizeof(seabaseDefaultsDDL), NULL, 0, FALSE, TRUE},

    {SEABASE_INDEXES, seabaseIndexesDDL, sizeof(seabaseIndexesDDL), NULL, 0, FALSE, TRUE},

    {SEABASE_KEYS, seabaseKeysDDL, sizeof(seabaseKeysDDL), NULL, 0, FALSE, TRUE},

    /* {SEABASE_LIBRARIES,
     seabaseLibrariesDDL, sizeof(seabaseLibrariesDDL),
     NULL, 0, FALSE, TRUE},*/

    {SEABASE_LIBRARIES_USAGE, seabaseLibrariesUsageDDL, sizeof(seabaseLibrariesUsageDDL), NULL, 0, FALSE, TRUE},

    {SEABASE_LOB_COLUMNS, seabaseLobColumnsDDL, sizeof(seabaseLobColumnsDDL), NULL, 0, FALSE, FALSE},

    {SEABASE_OBJECTS, seabaseObjectsDDL, sizeof(seabaseObjectsDDL), seabaseObjectsUniqIdxIndexDDL,
     sizeof(seabaseObjectsUniqIdxIndexDDL), FALSE, TRUE},

    {SEABASE_OBJECTS_UNIQ_IDX, seabaseObjectsUniqIdxDDL, sizeof(seabaseObjectsUniqIdxDDL), NULL, 0, TRUE, TRUE},

    {SEABASE_REF_CONSTRAINTS, seabaseRefConstraintsDDL, sizeof(seabaseRefConstraintsDDL), NULL, 0, FALSE, TRUE},

    {SEABASE_ROUTINES, seabaseRoutinesDDL, sizeof(seabaseRoutinesDDL), NULL, 0, FALSE, TRUE},

    {SEABASE_SEQ_GEN, seabaseSeqGenDDL, sizeof(seabaseSeqGenDDL), NULL, 0, FALSE, TRUE},

    {SEABASE_TABLES, seabaseTablesDDL, sizeof(seabaseTablesDDL), NULL, 0, FALSE, TRUE},

    {SEABASE_TABLE_CONSTRAINTS, seabaseTableConstraintsDDL, sizeof(seabaseTableConstraintsDDL),
     seabaseTableConstraintsIdxIndexDDL, sizeof(seabaseTableConstraintsIdxIndexDDL), FALSE, TRUE},

    {SEABASE_TABLE_CONSTRAINTS_IDX, seabaseTableConstraintsIdxDDL, sizeof(seabaseTableConstraintsIdxDDL), NULL, 0, TRUE,
     TRUE},

    {SEABASE_TEXT, seabaseTextDDL, sizeof(seabaseTextDDL), NULL, 0, FALSE, TRUE},

    {SEABASE_UNIQUE_REF_CONSTR_USAGE, seabaseUniqueRefConstrUsageDDL, sizeof(seabaseUniqueRefConstrUsageDDL), NULL, 0,
     FALSE, TRUE},

    {SEABASE_VERSIONS, seabaseVersionsDDL, sizeof(seabaseVersionsDDL), NULL, 0, FALSE, TRUE},

    {SEABASE_VIEWS, seabaseViewsDDL, sizeof(seabaseViewsDDL), NULL, 0, FALSE, TRUE},

    {SEABASE_VIEWS_USAGE, seabaseViewsUsageDDL, sizeof(seabaseViewsUsageDDL), NULL, 0, FALSE, TRUE}};

static const MDTableInfo allMDHistInfo[] = {
    {HBASE_HIST_NAME, seabaseHistogramsDDL, sizeof(seabaseHistogramsDDL), NULL, 0, FALSE, TRUE},

    {HBASE_HISTINT_NAME, seabaseHistogramIntervalsDDL, sizeof(seabaseHistogramIntervalsDDL), NULL, 0, FALSE, TRUE},

    {HBASE_PERS_SAMP_NAME, seabasePersistentSamplesDDL, sizeof(seabasePersistentSamplesDDL), NULL, 0, FALSE, TRUE}};
...
#define SEABASE_LOB_COLUMNS_OLD_MD             SEABASE_LOB_COLUMNS "_OLD_MD"
...
#define SEABASE_PARTITIONS_OLD_MD              SEABASE_PARTITIONS "_OLD_MD"
...
#define SEABASE_TABLE_PARTITION_OLD_MD         SEABASE_TABLE_PARTITION "_OLD_MD"
...
static const QString seabaseOldTrafMDv11IndexesDDL[] = {{" create table " SEABASE_INDEXES_OLD_MD " "},
                                                        {" ( "},
                                                        {"   base_table_uid largeint not null not serialized, "},
                                                        {"   keytag int not null not serialized, "},
                                                        {"   is_unique int not null not serialized, "},
...
static const QString seabaseOldTrafMDv11ObjectsUniqIdxIndexDDL[] = {
    {" create unique index " SEABASE_OBJECTS_UNIQ_IDX_OLD_MD " on " TRAFODION_SYSCAT_LIT ".\"" SEABASE_MD_SCHEMA
     "\"." SEABASE_OBJECTS_OLD_MD " "},
    {" ( "},
    {"   object_uid "},
    {" ) "},
    {" ; "}};
...
static const ComTdbVirtTableColumnInfo seabaseOldTrafMDv11MDValidateRoutineColInfo[] = {{"CLASSNAME",
                                                                                         0,
                                                                                         COM_USER_COLUMN,
                                                                                         REC_BYTE_V_ASCII,
                                                                                         256,
...
//////////////////////////////////////////////////////////////////////////////
static const QString seabaseOldTrafMDV21RefConstraintsDDL[] = {
    {" create table " SEABASE_REF_CONSTRAINTS " "},
    {" ( "},
    {"   ref_constraint_uid largeint not null not serialized, "},
    {"   unique_constraint_uid largeint not null not serialized, "},
    {"   match_option char(2) character set utf8 not null not serialized, "},
    {"   update_rule char(2) character set iso88591 not null not serialized, "},
    {"   delete_rule char(2) character set iso88591 not null not serialized, "},
    {"   flags largeint not null not serialized "},
    {" ) "},
...
static const QString seabaseOldTrafMDv21RoutinesDDL[] = {
    {" create table " SEABASE_ROUTINES " "},
    {" ( "},
    {"   udr_uid largeint not null not serialized, "},
    {"   udr_type char(2) character set utf8 not  null, "},
    {"   language_type char(2) character set iso88591 not null not serialized, "},
    {"   deterministic_bool char(2) character set iso88591 not null not serialized, "},
    {"   sql_access char(2) character set iso88591 not null not serialized, "},
    {"   call_on_null char(2) character set iso88591 not null not serialized, "},
...
static const QString seabaseOldMDv23IndexesDDL[] = {{" create table " SEABASE_INDEXES_OLD_MD " "},
                                                    {" ( "},
                                                    {"   base_table_uid largeint not null not serialized, "},
                                                    {"   keytag int not null not serialized, "},
                                                    {"   is_unique int not null not serialized, "},
...
static const QString createTrafColumnsViewQuery[] = {
    ...
    {"           cast (case when char_length(trim(sql_data_type)) = 0 then 'UNKNOWN' else sql_data_type end as "
     "char(24)) sql_data_type,        "},
...
static const QString createTrafKeysViewQuery[] = {{" create view %s.\"%s\"." TRAF_KEYS_VIEW " as "},
                                                  {" select O.catalog_name, O.schema_name, O.object_name table_name, "},
...
static const QString createTrafViewsViewQuery[] = {{" create view %s.\"%s\"." TRAF_VIEWS_VIEW " as "},
...
static const QString createTrafColumnCommentViewQuery[] = {
    ...
    {"  where O.OBJECT_UID = C.OBJECT_UID and T.TEXT_UID = O.OBJECT_UID and T.TEXT_TYPE = %s and T.SUB_ID = "
     "C.COLUMN_NUMBER "},
...
static const MDViewInfo allMDviewsInfo[] = {
    {TRAF_COLUMNS_VIEW, createTrafColumnsViewQuery, sizeof(createTrafColumnsViewQuery), FALSE},
    {TRAF_INDEXES_VIEW, createTrafIndexesViewQuery, sizeof(createTrafIndexesViewQuery), FALSE},
    {TRAF_KEYS_VIEW, createTrafKeysViewQuery, sizeof(createTrafKeysViewQuery), FALSE},
    {TRAF_REF_CONSTRAINTS_VIEW, createTrafRefConstraintsViewQuery, sizeof(createTrafRefConstraintsViewQuery), FALSE},
    {TRAF_SEQUENCES_VIEW, createTrafSequencesViewQuery, sizeof(createTrafSequencesViewQuery), FALSE},
    {TRAF_TABLES_VIEW, createTrafTablesViewQuery, sizeof(createTrafTablesViewQuery), FALSE},
    {TRAF_VIEWS_VIEW, createTrafViewsViewQuery, sizeof(createTrafViewsViewQuery), FALSE},
    {TRAF_OBJECT_COMMENT_VIEW, createTrafObjectCommentViewQuery, sizeof(createTrafObjectCommentViewQuery), FALSE},
    {TRAF_COLUMN_COMMENT_VIEW, createTrafColumnCommentViewQuery, sizeof(createTrafColumnCommentViewQuery), FALSE},
};

static const MDUpgradeInfo allMDPartTablesUpgradeInfo[] = {
    {SEABASE_PARTITIONS, SEABASE_PARTITIONS_OLD_MD, seabasePartitionsDDL, sizeof(seabasePartitionsDDL), NULL, 0, NULL,
     0, FALSE, NULL, NULL, NULL, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE},

    {SEABASE_TABLE_PARTITION, SEABASE_TABLE_PARTITION_OLD_MD, seabaseTablePartitionDDL,
     sizeof(seabaseTablePartitionDDL), NULL, 0, NULL, 0, FALSE, NULL, NULL, NULL, FALSE, FALSE, FALSE, FALSE, FALSE,
     FALSE}
