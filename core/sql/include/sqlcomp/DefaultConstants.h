...
enum DefaultConstants
{
  ...
  // for Sentry-secured Hive tables, determines how many seconds
  // must pass before a PrivMgrUserPrivs object is considered stale
  PRIV_SENTRY_RECHECK_INTERVAL,

  // if Sentry is on, use userName API instead of groupNames API
  PRIV_SENTRY_USE_USERNAME_API,
  ...
  QUERY_CACHE_SELECTIVITY_ABOSOLUTE,
  ...
  // -------------------------------------------------------------------------
  SP_ALIGNED_FORMAT,
  SP_DEFAULT_NO_TRANSACTION_REQUIRED,
  ...
  // -------------------------------------------------------------------
  // this default, if set, indicates that savepoint will be used
  // and stmt will be rolled back to the start of that savepoint.
  // Values are ON, OFF, ALL.
  // ALL indicates that savepoints will be used with esp parallel execution.
  // -------------------------------------------------------------------
  TRAF_SAVEPOINTS,

  // for implicit savepoints performance, we don't want to commit implicit savepoint
  // every single time, so use this count to decide when to commit accumulative amount
  // of savepoints in coprocessor
  TRAF_SAVEPOINT_BATCH_COMMIT_COUNT,
...
// Optimization to expand any "short" rows found in tables with added columns
 ...
 //Sort to use memory quota system.
  ...
  // This is an internal setting to control whether snapshots are token whenever a table
  // is created, It is important in that because it prevents mutations being replayed
  // for different incarnations of a table that might have different definitions.
  //
  // The reason we need to set TM_SNAPSHOT_TABLE_CREATES=1 is because we need to have a
  // snapshot and a meta record for every incremental table to serve as a starting point
  // for store, Without it an incremental backup would fail. If you make sure you do a
  // regular backup first you will be fine.
  TM_SNAPSHOT_TABLE_CREATES,
...
// Gen Sol:10-100408-9393
  ...
  UNC_PROCESS,
  USTAT_REQUIRE_DISTRIBUTED_LOCK,
  USTAT_RETRY_DELAY,
  USTAT_RETRY_LIMIT,
 ...
 //WMS child query monitoring,
  ...
  // Number of 1-second retries in NATableDB::get waiting for concurrent DDL
  // to complete
  TRAF_CONCURRENT_DDL_WAIT_RETRIES,
  ...
  // if it set to on, we will use new sequence implementation, which utilizes
  // distributed lock to resolve concurrency conflict instead of the old optimistic locking.
  TRAF_SEQUENCE_USE_DLOCK,

  // it determines the Durability of the API incrementColumnValue used to get sequence
  // next value for seq with order option
  // ON : SKIP_WAL   OFF : USE_DEFAULT(SYNC_WAL)
  TRAF_SKIP_WAL_FOR_ORDER_SEQ,
  ...
  // In some cases, an optimization could be done to use underlying HBase xns
  // instead of DTM xns.
  // Conditions are:
  //  -- it is a single table, single row IUD operation
  //  -- region xns are not being used
  //  -- cqd traf_no_dtm_xn is not set
  //  -- auto commit is on
  ...
  TRAF_ENABLE_PARQUET_FORMAT,
  TRAF_ENABLE_AVRO_FORMAT,
  ...
  // OFF: no pushdown, evaluated using traditional way (scan and aggr).
  // ON: transform in preCodeGen, eval in Parquet layer
  PARQUET_AGGR_PUSHDOWN,

  // pushdown column list to parquet
  PARQUET_COLUMNS_PUSHDOWN,

  // support for IN list in parquet
  PARQUET_IN_SUPPORT,
  ...
  TRAF_AVRO_AGGR_PUSHDOWN,

  // pushdown column list to avro
  TRAF_AVRO_COLUMNS_PUSHDOWN,
  ...
  ORC_METADATA_READ_MAX_FILENUM,
  ORC_METADATA_READ_MAX_THREADS,
  ...
  // if ON, bring up udr server when session begins
  TRAF_START_UDR,

  // Use switch compiler to avoid starting arkcmp processes
  TRAF_SWITCH_COMPILER,
  ...
  NCM_MP_ORC_SCAN_COST_ADJ_FACTOR,

  ORC_NJS_OUTER_TABLES_THRESHOLD,

  ORC_NJS_PROBES_MAXCARD_FACTOR,

  ORC_NJS_PROBES_THRESHOLD,
  ...
  // Minimal number of tables to enable data flow optimization
  DATA_FLOW_OPTIMIZATION_NUM_TABLES,
  ...
  // Metadata is loaded using 'load trafodion metadata in cache' command.
  // This cqd enables what it does.
  // If OFF, it returns without load. If ON, it will load it.
  TRAF_ENABLE_METADATA_LOAD_IN_CACHE,

  // if ON, generate stored stats after 'update statistics' command
  TRAF_USTAT_GENERATE_STORED_STATS,

  // 'ALL', 'ON', 'MINIMUM', 'OFF'
  //   MINIMUM: use estimated stats (rowcount)
  //   ALL: use histogram stats
  //   ON:  same as ALL
  //   OFF: do not use stored stats. Read them from hist tables when
  //        NATable is built.
  TRAF_USE_STORED_STATS,
  FIRSTN_PREDICATE_SAFETY_FACTOR,
  SUBQUERY_UNNESTING_EARLY_GROUP,

  INTERSECT_PUSH_GRBY_RATIO,

  // max length of hive binary datatype
  HIVE_MAX_BINARY_LENGTH,

  // 0: max; 1: min; 2 GeoMetric
  MJ_COMPUTE_CPU_RESOURCE,

  // include hive scan resouce in max. resource computation
  HIVE_SCAN_RESOURCES_INCLUSION,

  // min number of nodes for hive/tpcds queries
  HIVE_MIN_NUM_ESPS_PER_DATANODE,

  HIVE_APPLY_LOCALITY_NODE_RATIO,

  HIVE_APPLY_LOCALITY_MIN_NODES,

  // Memory unit per esp for hive/tpcds queries
  HIVE_MEMORY_UNIT_ESP,

  // Work unit per esp for hive/tpcds queries
  HIVE_WORK_UNIT_ESP,
  ...
  // Parameters needed to connect to Google bigtable
  BIGTABLE_PROJECT_ID,
  ...
  DOP_ADJUST_RTSTATS_HDFS_PATH,
  DOP_ADJUST_DEBUG,
  DOP_ADJUST_MIN_NUM_ESPS_PER_NODE,
  DOP_ADJUST_RTSTATS_LOCAL_FILE,
  ...
  AS_TOLERATE_ILL_DEFINED_TENANTS,
  ...
  GEN_HSHJ_MIN_MAX_OPT_INLIST_OPT,
  // default size of a tenant if not specified in the DDL
  TENANT_DEFAULT_SIZE,

  // If authentication is not enabled, use this value to get groups for the user
  USER_GROUPNAMES,

  HIVE_PARTITION_INSERT_REPART,
  HIVE_PARTITION_INSERT_DIST_LOCK,

  EXE_MEMORY_FOR_UNPACK_ROWS_IN_MB,
  MEMORY_LIMIT_ROWSET_IN_MB,
  MAX_ESPS_PER_NODE,

  PARQUET_READ_ROWGROUP_INFO,
  PARQUET_READ_NUM_ROWS,
  PARQUET_HDFS_STATS_LOG_FILE,
  MULTI_THREADED_ESP,

  SUPPRESS_CHAR_LIMIT_CHECK,

  NCM_PARQUET_COSTING,
  NCM_PARQUET_COSTING_APPLY_SKIP_RATIO,
  NCM_PARQUET_COSTING_DEBUG,
  NCM_PARQUET_COSTING_DEBUG_TABLE,

  NCM_MP_PARQUET_SCAN_COST_ADJ_FACTOR,

  PARQUET_NJS,
  PARQUET_NJS_CPS_DEBUG,
  PARQUET_NJS_CPS_MAXCARDINALITY,
  PARQUET_NJS_CPS_TABLE,
  PARQUET_NJS_CPS_THRESHOLD,
  PARQUET_NJS_OUTER_TABLES_THRESHOLD,
  PARQUET_NJS_PROBES_MAXCARD_FACTOR,
  PARQUET_NJS_PROBES_THRESHOLD,

  PARQUET_BATCH_READ_FILESTATS,

  HIVE_REPORT_FILESTATS_READ_ET,

  // if set, hive DDL operations (CTAS, DROP) are supported
  HIVE_DDL,

  // type of hive table created. (text, orc, parquet)
  HIVE_CTAS_TABLETYPE,

  // multiplicative factor applied to result type of regexp_replace
  // on initial estimate computed using replace synthType code.
  REGEXP_REPLACE_RESULT_TYPE_FACTOR,

  // optimization for orc/parquet to pushdown preds that have an expression
  // on the left side of the predicate.
  // Currently done for some flavors of LAST_DAY and TO_DATE.
  TRAF_EXT_PRED_PUSHDOWN_OPT,

  HIVE_COMPATIBILITY_MODE,

  // Allow users to grant privileges to roles using the With Grant Option
  ALLOW_WGO_FOR_ROLES,
  CAT_ANSI_PRIVS_FOR_TENANT,

  // this will auto POPULATE data into index while enable index
  TRAF_ALLOW_POPULATE_ON_ENABLE_INDEXES,

  TRAF_ALLOW_PURGEDATA_ON_REPLICATED_TABLES,

  TRAF_TENANT_GROUP_CHECK,
  ...
  // maximum number of tables allowed under each select query.
  // For a union query, each leg is considered one select query.
  // for ex: select * from t,t1 union all select * from t2;
  //  have 2 select queries. Left one has 2 tables, right one has 1.
  TRAF_MAX_TABLES_PER_SELECT,

  // total number of tables allowed in a query.
  // From previous example, total number of tables is 3.
  TRAF_MAX_TABLES_PER_QUERY,
  COMPRESSED_INTERNAL_FORMAT_BULK_VARCHAR_MAX,

  // if set to ON, backup repository is enabled and updated.
  // Current default is OFF until this feature is fully supported.
  TRAF_BACKUP_REPOSITORY_SUPPORT,

  // NGRAM Index, enable:'ON' or disable:'OFF', only affect query plan
  NGRAM_INDEX,
  ...
  // contains list of schemas that are passed on to table(hivemd(...))
  // command if no explicit schema is specified.
  TRAF_HIVEMD_SCHEMAS,

  ALLOW_TENANT_RESOURCE_GROUPS,
  ALLOW_TRUNCATE_IN_USER_XN,
  TENANT_OVERSUBSCRIBE_WARN_NODE_RATIO,
  TENANT_OVERSUBSCRIBE_ERR_NODE_RATIO,
  // Support some syntax of Oracle ROWNUM
  ...
  TRAF_BR_LOCKED_OBJECTS_ACCESS,

  // 0: allow Select to access objects locked during BR based on previous cqd
  // 1: allow Select + IUD to access locked objects
  // 1: allow Select + IUD + DDL to access locked objects
  TRAF_BR_LOCKED_OBJECTS_QUERY_TYPE,

  // ON:  remove snapshot locks after BR operations
  // OFF: only remove snapshot locks after failed backups and cleanup
  TRAF_BR_REMOVE_SM_LOCKS,
  ...
  //  -1: no limit
  MAX_NUM_UNIQUE_CONSTRAINTS_FOR_JOIN,

  PARQUET_USE_CPP_READER,
  ...
  // if not set, it has nanosec precision (9).
  // Default is nanosec precision
  HIVE_TIMESTAMP_PRECISION_IN_USEC,

  // See GroupByOnUnionRule for details
  GROUP_BY_PUSH_BELOW_UNION,

  // max number of nested levels within a composite datatype.
  // "INT" is 1. "INT ARRAY" is 2.  "ROW(A INT) ARRAY[5]" is 3.
  // Current default max is 3.
  TRAF_MAX_COMPOSITE_LEVELS,

  // size of ARRAY if none is specified.
  // Current default is 25.
  TRAF_DEFAULT_COMPOSITE_ARRAY_SIZE,

  // max size of ARRAY type
  TRAF_MAX_COMPOSITE_ARRAY_SIZE,

  // if set to on, then lateral view is evaluated in a single operator instead
  // of as a join. Used if only one lateral view is defined
  //  Ex: select * from T lateral view unnest (a)
  // This is an internal cqd.
  ...
  // min/max will not be considered
  RANGE_OPTIMIZED_SCAN_MAX_INNER_TABLES,

  // Define the cycle (# of rows inserted) before a sanity
  // check of the range of values will be performed.
  RANGE_OPTIMIZED_SCAN_SANITY_CHECK_CYCLE,
  ...
  // Values:  OFF, ON, ALL
  //          OFF, off
  //          ON, limited support. (selection, extraction of fields, unnest, display)
  //          ALL, enable some extra features (cast, concat, array/row constructors).
  //               These will be enabled by default after more testing.
  HIVE_COMPOSITE_DATATYPE_SUPPORT,

  // Valid values: "EXPLODED" or "ALIGNED"
  // Current default: "ALIGNED"
  TRAF_COMPOSITE_DATATYPE_FORMAT,

  // default length of 'string' columns within a struct definition
  HIVE_MAX_COMPOSITE_STRING_LENGTH_IN_BYTES,

  // transform update to scan/delete/upsert.
  // OFF, do not transform.
  // ON, transform for non-pkey columns. Not externalized yet.
  // ALL, transform for all pkey columns. Not externalized yet.
  ...
  // Control the application of rowgroup filters. Default is ON
  PARQUET_CPP_READER_FILTER_ROWGROUPS,

  // Control the application of page level filters. Default is ON
  PARQUET_CPP_READER_FILTER_PAGES,

  // parallel hive aggregation push-down.
  PARALLEL_AGGR_PUSHDOWN,
  ...
  //  Ex:  arr[1] refers to second element.
  // Traf is ansi compliant and uses 1-based indexexing.
  // If this cqd is set, then 0-based indexing will be used.
  HIVE_ARRAY_INDEX_MODE,
  ...
  HDFS_WRITE_LOCK_TIMEOUT_IN_SECS,

  // 0,  update after each table.
  // -1, dont update
  // N,  update every N secs
  // Default is 0.
  TRAF_BREI_PROGRESS_UPDATE_DELAY,

  // if ON, then lock schema instead of objects being backed up.
  // Currently only done if full database or  only schemas are being backed up.
  // Default is OFF.
  TRAF_LOCK_BACKUP_SCHEMA,

  // new version of lob
  TRAF_LOB_VERSION2,

  // 0: create/drop internal lob tables sequentially
  // 1: create/drop MD updates in main thread, hbase create/drop in pthread
  TRAF_LOB_PARALLEL_DDL,

  // if both are specified, maxchars takes precedence
  TRAF_LOB_INLINED_DATA_MAXBYTES,
  TRAF_LOB_INLINED_DATA_MAXCHARS,

  // this is the max size that can be specified for inlined data maxbytes/chars
  TRAF_LOB_INLINED_DATA_MAXLIMIT_IN_BYTES,
  ...
  TRAF_LOB_CHUNK_MAXLEN,

  TRAF_RETURN_LOB_HANDLE_WITHOUT_INLINE_DATA,

  // Determines if client statement pooling is turned on
  // In that case, the prepared query needs to be retried by the JDBC/ODBC driver
  CLIENT_MAX_STATEMENT_POOLING,

  WAIT_ON_SELECT_FOR_UPDATE,

  ALLOW_ROLE_GRANTS_TO_GROUPS,
  ...
  // When reconnecting to mxosrvr as a different user:
  //   ON:  clear metadata (NATable/NARoutine) cache
  ...
  // MD SEQ_GEN table updates need to be synchronized with remote cluster.
  // This cqd is set internally when updates to seq_gen tables are done.
  // It should not be used for user tables.
  TRAF_MD_REPLICATION,

  // if set, updates to SEQ_GEN table will be done using dtm transactions.
  // If a user transaction is in effect, then it will be suspended and
  // resumed after seq_gen update is done.
  // Only one of the 2 cqds: traf_sequence_use_dlock and use_dtm should
  // be used. If both are set, the dlock will be used and updates will
  // not be done in a dtm transaction.
  TRAF_SEQUENCE_USE_DTM,
  ...
  //
  // CQD CONFIGURE_TESTPOINT '<number>:DELAY:<seconds>' configures the
  // test point <number>. At this test point, a delay of <seconds> will
  // be inserted. This is useful for expanding time windows when testing
  // concurrent activity, such as concurrent DDL and DML operations.
  //
  // Issuing two consecutive CQD CONFIGURE_TESTPOINT to the same <number>
  // replaces the earlier setting with the later setting.
  //
  // CQD CONFIGURE_TESTPOINT RESET resets all test points to off state.
  //
  // CQD CONFIGURE_TESTPOINT ' ' (blank or empty string) does nothing.
  //
  // CQD CONFIGURE_ TESTPOINT '<anything else>' gives an error.
  CONFIGURE_TESTPOINT,

  // It is the time period during which any DDL can be done
  // By default it is 'ON', but expected to be turned off in production
  // When it is 'OFF', the DDL that are considered safe and doesn't
  // introduce integrity issue will be allowed.
  MAINTENANCE_WINDOW,

  // Object Lock for preventing concurrent DML/DDL
  TRAF_OBJECT_LOCK,
  TRAF_OBJECT_LOCK_DDL_DIST_LOCK_TIMEOUT,
  TRAF_OBJECT_LOCK_DDL_RETRY_LIMIT,
  TRAF_OBJECT_LOCK_DDL_RETRY_DELAY,
  TRAF_OBJECT_LOCK_DML_DIST_LOCK_TIMEOUT,
  TRAF_OBJECT_LOCK_DML_RETRY_LIMIT,
  TRAF_OBJECT_LOCK_DML_RETRY_DELAY,
  TRAF_OBJECT_LOCK_FORCE,

  TRAF_OBJECT_LOCK_UNSAFE_MODE,

  // Read old format of osim
  OSIM_READ_OLD_FORMAT,

  SHOW_CACHE_KEY,

  DONOT_WRITE_XDC_DDL,

  // no check whether users or groups exist in LDAP
  TRAF_IGNORE_EXTERNAL_LDAP_EXISTS_CHECK,

  // wants to ignoring password policy which is defined in environment variable?
  TRAF_IGNORE_EXTERNAL_PASSWORD_POLICY_SYNTAX,

  // default syntax of password check policy
  TRAF_PASSWORD_POLICY_SYNTAX,

  // number for counter for allow login user to log in when the password has been expired
  TRAF_PASSWORD_GRACE_COUNTER,

  TRAF_ENABLE_DATA_LOAD_IN_CACHE,
  TRAF_ENABLE_DATA_LOAD_IN_SHARED_CACHE,
