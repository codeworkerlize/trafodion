...
class BackupAttr 
{
 public:
  ...
  // constructors
  BackupAttr()
  : attrKey_(BACKUP_ATTR_UNKNOWN),
    attrValue_(NULL),
    heap_(NULL)
  {}
  
  BackupAttr(const BACKUP_ATTR_KEY attrKey,
  ...
  bool attrInvalid() 
  { return (attrKey_ == BACKUP_ATTR_UNKNOWN || attrValue_ == NULL); }
...
class BackupAttrList : public LIST(BackupAttr *)
{
  ...
  // constructor
  BackupAttrList()
  : LIST(BackupAttr *) (NULL),
    BKFormat_(BKFormat),
    heap_ (NULL)
  {}

  BackupAttrList(NAHeap *heap)
   : LIST(BackupAttr *)(heap), 
     BKFormat_(BKFormat),
     heap_(heap)
  {}
  ...
  virtual ~BackupAttrList()
  {
    for (CollIndex i = 0; i < entries(); i++)
      NADELETE(operator[](i), BackupAttr, heap_);
    clear();
  }
