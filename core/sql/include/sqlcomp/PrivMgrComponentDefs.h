...
//  to add a new operation, add an entry to this list (in alphabetic order)
//  and add a corresponding entry to the sqlOpList.
//  If part of alter, create, drop, or manage then update PrivMgr.cpp
//     isSQLManageOperation, isSQLAlterOperation, ...
...
static const ComponentOpStruct sqlOpList[] = {
    ...
    {(int32_t)SQLOperation::MANAGE_GROUPS, "GM", "MANAGE_GROUPS", true, false, false, false,
     false},  // MG has been defined for MANAGE_RESOURCE_GROUPS
    ...
    {(int32_t)SQLOperation::MANAGE_RESOURCE_GROUPS, "MG", "MANAGE_RESOURCE_GROUPS", true, false, false, false, false},
    ...
    {(int32_t)SQLOperation::MANAGE_TENANTS, "MX", "MANAGE_TENANTS", true, false, false, false, false},
    {(int32_t)SQLOperation::MANAGE_USERS, "MU", "MANAGE_USERS", true, false, false, false, false},
    {(int32_t)SQLOperation::MANAGE_SECURE, "MK", "MANAGE_SECURE", true, false, false, false, false},
    ...
    {(int32_t)SQLOperation::SHOW, "SW", "SHOW", true, true, false, false, false},
...
static const ComponentOpStruct dbmgrOpList[] = {
    {(int32_t)DBMGROperation::MANAGE_ALERTS, "MA", "MANAGE_ALERTS", true, true, false, false, false},
    {(int32_t)DBMGROperation::MANAGE_SESSIONS, "MS", "MANAGE_SESSIONS", true, true, false, false, false},
    {(int32_t)DBMGROperation::SHOW_ACTIVE_QUERIES, "AQ", "SHOW_ACTIVE_QUERIES", true, true, false, false, false},
    {(int32_t)DBMGROperation::SHOW_EVENT_LOGS, "EL", "SHOW_EVENT_LOGS", true, true, false, false, false},
    {(int32_t)DBMGROperation::SHOW_REPOS_QUERIES, "RQ", "SHOW_REPOS_QUERIES", true, true, false, false, false},
    {(int32_t)DBMGROperation::USE_QUERY_WORKBENCH, "QW", "USE_QUERY_WORKBENCH", true, true, false, false, false},
    {(int32_t)DBMGROperation::EXECUTE_SQL_SCRIPT, "ES", "EXECUTE_SQL_SCRIPT", true, true, false, false, false},
    {(int32_t)DBMGROperation::RUN_SYSTEM_COMMANDS, "SC", "RUN_SYSTEM_COMMANDS", true, true, false, false, false}};
...
enum class WMSOperation { MANAGE_WMS = 2 };

// Assign initial privileges for DBMGROperation (based on ComponentOpStruct):
static const ComponentOpStruct wmsOpList[] = {
    {(int32_t)WMSOperation::MANAGE_WMS, "MW", "MANAGE_WMS", true, true, false, false, false},
};

// List of components
static const ComponentListStruct componentList[]{
    {(int64_t)SQL_OPERATIONS_COMPONENT_UID, SQL_OPERATIONS_NAME, sizeof(sqlOpList) / sizeof(ComponentOpStruct),
     (ComponentOpStruct *)&sqlOpList},
    {(int64_t)DBMGR_COMPONENT_UID, DBMGR_NAME, sizeof(dbmgrOpList) / sizeof(ComponentOpStruct),
     (ComponentOpStruct *)&dbmgrOpList},
    {(int64_t)WMS_COMPONENT_UID, WMS_NAME, sizeof(wmsOpList) / sizeof(ComponentOpStruct),
     (ComponentOpStruct *)&wmsOpList}};
