...
class CompilerEnv : public NABasicObject {
 public:
  ...
  void unpackFromBuffer(TMUDRSerializableObject& t, const char*& buffer, int& bufferSize,
...
class HQCCacheData : public LIST (HQCCacheEntry*)
{
 public:
  ...
  NABoolean feasibleToReplace(Int32 bytesNeeded); // feasible to replace an entry
...
public:
  ...
  ULng32 getFreeSize()
   { ULng32 a = getSize(); return maxSiz_>a ? maxSiz_-a : 0; }
...
class CacheKey : public Key {
 public:
  ...
  static CacheKey* unpackFromBuffer(ComDiagsArea* diags, TMUDRSerializableObject& t, const char*& buffer, int& bufferSize);
...
class CacheData : public CData {
 public:
  ...
  CacheData(Plan* plan, const ParameterTypeList& f,
    const SelParamTypeList& s,
    LIST(Int32) hqcParamPos, LIST(Int32) hqcSelPos, LIST(Int32) hqcConstPos,
    const char* text, const char* normalized_text, Lng32 cs, Int64 queryHash, NAHeap* h)
    ...
    origStmt_((char*)text), normalizedStmt_((char*)normalized_text), textentries_(h, 10), charset_(cs),
