      ...
      for (int i=0; i<5; i++)
        ...
        if(NOT isWarning && NULL != condition.getSqlID())
        {
          ...
          if(NULL != statsGlobals)
          {
            ...
            if (0 == errorCode)
            {
              StmtStats *stmtStats = statsGlobals->getMasterStmtStats(condition.getSqlID(), str_len(condition.getSqlID()), RtsQueryId::ANY_QUERY_);
              if(NULL != stmtStats)
              {
...
Lng32 SQL_EXEC_CancelOperation(Int64 transid)
{
   ...
   {
      retcode =
      SQLCLI_CancelOperation(GetCliGlobals(), transid);
   }
   catch(...)
   {
#if defined(_THROW_EXCEPTIONS)
   ...
   /*OUT*/ struct SQLSEC_AuthDetails  * auth_details)
{
   ...
   try
   {
      ...
      threadContext->incrNumOfCliCalls();
      retcode =
      SQLCLI_GetUserAttrs(GetCliGlobals(),
                          username,
                          tenant_name,
                          users_info,
                          auth_details);
            ...
            /* OUT */     UInt32 * maxExpectedFlagsFound)
{
   ...
   try
   {
      ...
      tmpSemaphore->get();
      threadContext->incrNumOfCliCalls();
      retcode = SQLCLI_SetObjectEpochEntry(GetCliGlobals(),
                                           operation,
                                           objectNameLength,
                                           objectName,
                                           redefTime,
            ...
            /* IN */       short objectType)
{
   ...
   try
   {
     ...
     threadContext->incrNumOfCliCalls();
     retcode =
       SQLCLI_LockObjectDDL(GetCliGlobals(),
                            objectName,
                            objectType);
   }
   catch(...)
            ...
            /* IN */         short objectType)
{
   ...
   try
   {
     ...
     threadContext->incrNumOfCliCalls();
     retcode =
       SQLCLI_UnLockObjectDDL(GetCliGlobals(),
                              objectName,
                              objectType);
   }
   catch(...)
            ...
            /* IN */       short objectType)
{
   ...
   try
   {
     ...
     threadContext->incrNumOfCliCalls();
     retcode =
       SQLCLI_LockObjectDML(GetCliGlobals(),
                            objectName,
                            objectType);
   }
   catch(...)
...
Lng32 SQL_EXEC_ReleaseAllDDLObjectLocks()
{
   ...
   try
   {
     ...
     threadContext->incrNumOfCliCalls();
     retcode =
       SQLCLI_ReleaseAllDDLObjectLocks(GetCliGlobals());
   }
   catch(...)
   {
     retcode = -CLI_INTERNAL_ERROR;
...
Lng32 SQL_EXEC_ReleaseAllDMLObjectLocks()
{
   ...
   try
   {
     ...
     threadContext->incrNumOfCliCalls();
     retcode =
       SQLCLI_ReleaseAllDMLObjectLocks(GetCliGlobals());
   }
   catch(...)
   {
     retcode = -CLI_INTERNAL_ERROR;
...
)
{
   ...
   try
   {
     ...
     threadContext->incrNumOfCliCalls();
     retcode =
       SQLCLI_GetObjectEpochStats_Internal(GetCliGlobals(),
                                           objectName,
                                           objectNameLen,
                                           cpu,
                                           locked,
...
)
{
   ...
   try
   {
     ...
     threadContext->incrNumOfCliCalls();
     retcode =
       SQLCLI_GetObjectLockStats_Internal(GetCliGlobals(),
                                           objectName,
                                           objectNameLen,
                                           cpu,
                                           exStatsArea);
...
Lng32 SQL_EXEC_OrderSeqXDCCliInterface
(
 void* *cliInterface,
 void * seqGenAttrs,
 Int64 endValue
  ...
  try
    {
      ...
      tmpSemaphore->get();
      threadContext->incrNumOfCliCalls();
      retcode = SQLCLI_OrderSeqXDCCliInterface(GetCliGlobals(),
					  cliInterface,
					  seqGenAttrs,
                      endValue);
    }
...
Lng32 SQL_EXEC_ExtractFieldsFromLobHandleStr(
     /*IN*/    char *handleStr,
     /*IN*/    Lng32 handleLen,
     /*INOUT*/ Int32 *lobV2,
  ...
  try
    {
      ...
      tmpSemaphore->get();
      threadContext->incrNumOfCliCalls();
      retcode = SQLCLI_ExtractFieldsFromLobHandleStr(
           GetCliGlobals(),
           handleStr, handleLen,
           lobV2, objectUID,
...
char* SQL_EXEC_GetClientInfo()
{
    ...
    ContextCli *currContext = GetCliGlobals()->currContext();
    if (currContext != NULL)
      clientInfo = currContext->getClientInfo();
