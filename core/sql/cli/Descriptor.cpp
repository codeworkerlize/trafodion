  ...
  switch (descriptor_id.name_mode) {
    case desc_name: {
      ...
      if (module_nm_len > 0) {
        ...
        str_cpy_all(modname, descriptor_id_->module->module_name, module_nm_len);
    ...
    case desc_via_desc:
...

      {
        ...
        if (id) {
          ...
          char *id2 = (char *)heap.allocateMemory(id->identifier_len + 1);

          str_cpy_all(id2, id->identifier, id->identifier_len);
...
NABoolean Descriptor::operator==(Descriptor &other) {
  ...
  while (i < getUsedEntryCount()) {
    ...
    if ((descItem.datatype != otherDescItem.datatype) || (descItem.datetime_code != otherDescItem.datetime_code) ||
        (descItem.length != otherDescItem.length) || (descItem.nullable != otherDescItem.nullable) ||
        (descItem.charset != otherDescItem.charset) || (descItem.scale != otherDescItem.scale) ||
...
char *Descriptor::getIndItem(desc_struct &descItem, int idxrow) {
  ...
  if (descItem.ind_data)
    ...
    if (descItem.rowsetVarLayoutSize > 0) ptr += idxrow * descItem.rowsetIndLayoutSize;
  }
  return ptr;
}
...
static int desc_set_string_value_from_varchar(char *string_value, int max_string_len, char *source_string) {
  ...
  if (!string_value) {
    ...
    if (max_string_len < returned_len) returned_len = max_string_len;
...
static RETCODE setCharHostVar(char *source, char *host_var_buf, int host_var_buf_size, int *returned_len,
  ...
  if (target_type != REC_BYTE_F_ASCII && target_type != REC_BYTE_V_ASCII && target_type != REC_BYTE_V_ANSI) {
    return ERROR;
  }
  ...
  *returned_len = target_len;
  ...
  if (target) {
    if (source == 0) {
      ...
      if (target_len > source_len) *returned_len = source_len;

      ex_expr::exp_return_type expRetcode = convDoIt(source + VCPREFIX_LEN, source_len, (short)source_type, 0, 0,
                                                     target, target_len, (short)target_type, 0, 0, NULL, 0);
  ...
  *returned_len = 0;
  ...
  switch (what_to_get) {
    ...
    case SQLDESC_TEXT_FORMAT:
      if (!descItem.text_format) set_text_format(descItem);
...
RETCODE Descriptor::getDescItem(int entry, int what_to_get, void *numeric_value, char *string_value,
  ...
  switch (what_to_get) {
    ...
    case SQLDESC_NAME: {
      return setCharHostVar(descItem.output_name, string_value, max_string_len, returned_len, info_desc,
                            info_desc_index);
    } break;
    ...
    case SQLDESC_HEADING:
      ...
      {
        *returned_len = desc_set_string_value_from_varchar(string_value, max_string_len, descItem.heading);
      } else {
        *returned_len = 0;
      }
      break;
    ...
    case SQLDESC_VAR_DATA:
      *returned_len = desc_set_string_value_from_varchar(string_value, max_string_len, descItem.var_data);
      break;
    ...
    case SQLDESC_TABLE_NAME:
      ...
      {
        *returned_len = desc_set_string_value_from_varchar(string_value, max_string_len, descItem.table_name);
      } else {
        *returned_len = 0;
      }
      break;
    ...
    case SQLDESC_SCHEMA_NAME:
      ...
      {
        *returned_len = desc_set_string_value_from_varchar(string_value, max_string_len, descItem.schema_name);
      } else {
        *returned_len = 0;
      }
      break;
    ...
    case SQLDESC_CATALOG_NAME:
      ...
      {
        *returned_len = desc_set_string_value_from_varchar(string_value, max_string_len, descItem.catalog_name);
      } else {
        *returned_len = 0;
      }
      break;
    ...
    case SQLDESC_TEXT_FORMAT:
      ...
      if (descItem.text_format) {
        *returned_len = desc_set_string_value_from_varchar(string_value, max_string_len, descItem.text_format);
      } else {
        *returned_len = 0;
      }
...
void Descriptor::DescItemDefaultsForType(/*INOUT*/ desc_struct &descItem,
  ...
  if (isCharacterFSType(datatype)) {
    ...
    if (descItem.coll_name) heap.deallocateMemory(descItem.coll_name);
    ...
    const char *charset_name = CharInfo::getCharSetName((CharInfo::CharSet)descItem.charset);
    if (descItem.charset_name) heap.deallocateMemory(descItem.charset_name);
    ...
    } else  // internal Tandem FS interval datatype
    {
      ...
      DescItemDefaultsForDatetimeCode(descItem, descItem.datetime_code);
    }
    descItem.ind_data = 0;
    descItem.ind_ptr = 0;
    // 17.5 - GR 5-d
...
char *Descriptor::getCharDataFromCharHostVar(ComDiagsArea &diags, NAHeap &heap, char *host_var_string_value,
  ...
  if (source_type != REC_BYTE_F_ASCII && source_type != REC_BYTE_V_ASCII &&
      source_type != REC_BYTE_V_ANSI) {  // Host variable should be of ASCII char type!
    diags << DgSqlCode(-CLI_NOT_ASCII_CHAR_TYPE) << DgString0(the_SQLDESC_option);
    return 0;
  }
  ...
  if (the_SQLDESC_option_enum == COLUMN_NAME_option) {
    ...
    expRetcode = convDoIt(source, source_len, (short)source_type, 0, 0, target + VCPREFIX_LEN, target_len,
                          (short)REC_BYTE_F_ASCII, 0, 0, NULL, 0);
  } else {
    ...
    expRetcode = convDoIt(source, source_len, (short)source_type, 0, 0, target, target_len, target_type, 0, 0, NULL, 0);
  }
...
RETCODE Descriptor::processNumericDatatypeWithPrecision(desc_struct &descItem, ComDiagsArea &diags) {
  if (descItem.desc_flags & descItem.IS_NUMERIC)
  ...
  // represent it internally.
  {
    if ((descItem.precision >= 0) && (descItem.precision <= 4))
      descItem.datatype = getFSTypeFromANSIType(SQLTYPECODE_SMALLINT);
    else if ((descItem.precision > 4) && (descItem.precision <= 9))
      descItem.datatype = getFSTypeFromANSIType(SQLTYPECODE_INTEGER);
    else if ((descItem.precision >= 10) && (descItem.precision < 19))
      descItem.datatype = getFSTypeFromANSIType(SQLTYPECODE_LARGEINT);
    else if (descItem.precision >= 19)
      descItem.datatype = getFSTypeFromANSIType(REC_NUM_BIG_SIGNED);
  } else if (descItem.desc_flags & descItem.IS_NUMERIC_UNSIGNED) {
    if ((descItem.precision >= 0) && (descItem.precision <= 4))
      descItem.datatype = getFSTypeFromANSIType(SQLTYPECODE_SMALLINT_UNSIGNED);
    else if ((descItem.precision > 4) && (descItem.precision <= 9))
      descItem.datatype = getFSTypeFromANSIType(SQLTYPECODE_INTEGER_UNSIGNED);
    else if (descItem.precision >= 10) {
      descItem.datatype = getFSTypeFromANSIType(REC_NUM_BIG_UNSIGNED);
  ...
  if (descItem.precision >= 0) {
    if ((descItem.datatype == REC_BIN16_SIGNED) || (descItem.datatype == REC_BIN32_SIGNED) ||
        (descItem.datatype == REC_BIN64_SIGNED) || (descItem.datatype == REC_NUM_BIG_SIGNED)) {
      if (((descItem.datatype == REC_BIN16_SIGNED) && (descItem.precision > 4)) ||
        ...
        if (descItem.datatype == REC_BIN16_SIGNED)
          ...
          maxPrec = 128;
        diags << DgSqlCode(-3014) << DgInt0(descItem.precision) << DgInt1(maxPrec);
        return ERROR;
      }

      // Set the flag to indicate that it is NUMERIC.
      descItem.desc_flags |= descItem.IS_NUMERIC;
    } else if ((descItem.datatype == REC_BIN16_UNSIGNED) || (descItem.datatype == REC_BIN32_UNSIGNED) ||
               (descItem.datatype == REC_BIN64_UNSIGNED) || (descItem.datatype == REC_NUM_BIG_UNSIGNED)) {
      if (((descItem.datatype == REC_BIN16_UNSIGNED) && (descItem.precision > 4)) ||
        ...
        if (descItem.datatype == REC_BIN16_UNSIGNED)
          ...
          maxPrec = 128;
        diags << DgSqlCode(-3008) << DgString0("NUMERIC") << DgInt0(descItem.precision) << DgInt1(maxPrec);
        return ERROR;
      }
...
RETCODE Descriptor::setDescItem(int entry, int what_to_set, Long numeric_value, char *string_value,
  ...
  if (info_desc != NULL) {
    switch (what_to_set) {
      ...
      case SQLDESC_LENGTH:
      case SQLDESC_IND_TYPE:
        info_desc->getDescItem(info_desc_index + 1, SQLDESC_TYPE_FS, &sourceType, NULL, 0, NULL, 0);
        info_desc->getDescItem(info_desc_index + 1, SQLDESC_OCTET_LENGTH, &sourceLen, NULL, 0, NULL, 0);
        ...
        info_desc->getDescItem(info_desc_index + 1, whichPrecision, &sourcePrecision, NULL, 0, NULL, 0);
        info_desc->getDescItem(info_desc_index + 1, whichScale, &sourceScale, NULL, 0, NULL, 0);
    }
      ...
      case SQLDESC_VAR_DATA: {
        ...
        if (DFS2REC::isAnyCharacter(sourceType) && DFS2REC::isAnyCharacter(descItem.datatype)) {
          ...
          if (sourceCharSet != CharInfo::UNICODE && sourceCharSet != descItem.charset) {
            // Error 8896: The character set $0~string0 of a host variable does not match $1~string1 of the
            // corresponding descriptor item (entry $2~Int0).
            diags << DgSqlCode(-CLI_CHARSET_MISMATCH)
                  << DgString0(CharInfo::getCharSetName((CharInfo::CharSet)sourceCharSet))
                  << DgString1(CharInfo::getCharSetName((CharInfo::CharSet)descItem.charset)) << DgInt0(entry);
            return ERROR;
          }
        }
      } break;
  ...
  switch (what_to_set) {
    ...
    case SQLDESC_NAME: {
      ...
      descItem.output_name =
          getCharDataFromCharHostVar(diags, heap, string_value, numeric_value, "NAME", info_desc, info_desc_index);
    } break;
    ...
    case SQLDESC_HEADING:
      ...
      descItem.heading = desc_varchar_alloc_and_copy(string_value, heap);
      break;
    ...
    case SQLDESC_IND_LENGTH:
      if (descItem.ind_length == 0) {
        ...
        setIndLength(descItem);
      } else if (descItem.ind_length != numeric_value) {
#ifdef _DEBUG
      ...
      if (string_value)  // some real data area passed in.
      {
        if (info_desc != NULL) {
          ...
          descItem.var_data = desc_var_data_alloc(
              string_value, sourceType, sourceLen, (CharInfo::CharSet)sourceCharSet, &target, targetType, &targetLen,
              DFS2REC::isAnyCharacter(descItem.datatype) ? (CharInfo::CharSet)descItem.charset : CharInfo::ISO88591,
              &targetVCLen, &targetVCLenSize, heap);
          source = string_value;
        } else
          descItem.var_data = desc_varchar_alloc_and_copy(string_value, heap, numeric_value);
      }
    ...
    case SQLDESC_ROWSET_NUM_PROCESSED:
      if (((NOT rowwiseRowset()) && (numeric_value > rowsetSize)) ||
          ((rowwiseRowset()) && (numeric_value > rowwiseRowsetSize)))
        diags << DgSqlCode(-CLI_INTERNAL_ERROR);
      else
    ...
    case SQLDESC_TABLE_NAME:
      ...
      descItem.table_name = desc_varchar_alloc_and_copy(string_value, heap);
      break;
    case SQLDESC_SCHEMA_NAME:
      if (descItem.schema_name)  // data already exists
        heap.deallocateMemory(descItem.schema_name);
      descItem.schema_name = desc_varchar_alloc_and_copy(string_value, heap);
      break;
    case SQLDESC_CATALOG_NAME:
      if (descItem.catalog_name)  // data already exists
        heap.deallocateMemory(descItem.catalog_name);
      descItem.catalog_name = desc_varchar_alloc_and_copy(string_value, heap);
      break;
...
// - end testing logic *****
  ...
  if (source != NULL) {
    if (DFS2REC::isSQLVarChar(sourceType)) {
      ...
      if (!asserted++) {
        assert(SQL_VARCHAR_HDR_SIZE == sizeof(short));
               ...
               ((DFS2REC::isDateTime(sourceType)) || (DFS2REC::isInterval(sourceType)))) {
    ...
    if (::convDoIt(source, sourceLen, (short)sourceType, sourcePrecision, sourceScale, target, targetLen,
                   (short)targetType, targetPrecision, targetScale, targetVCLen, targetVCLenSize, collHeap,
    ...
    if (DFS2REC::isANSIVarChar(targetType) && CharInfo::is_NCHAR_MP((CharInfo::CharSet)descItem.charset)) {
    ...
    if (targetType >= REC_MIN_NUMERIC && targetType <= REC_MAX_NUMERIC && sourceScale != targetScale &&
        ::scaleDoIt(target, targetLen, targetType, sourceScale, targetScale, sourceType, collHeap) !=
            ex_expr::EXPR_OK) {
      return ERROR;
    }
  ...
  switch (what_to_set) {
    ...
    case SQLDESC_CHAR_SET: {
      if (charset_name == NULL) charset_name = (char *)CharInfo::getCharSetName(newCharSet);

      if ((!DFS2REC::isAnyCharacter(descItem.datatype) || !CharInfo::isCharSetSupported(newCharSet)) &&
      ...
      if (newCharSet != descItem.charset) {
        ...
        if (newCharSet == CharInfo::ISO88591) {
        ...
        } else if (newCharSet == CharInfo::UNICODE) {
          ...
          // ISO88591, KANJI, and KSC5601
          {
            ...
            if (descItem.charset == CharInfo::ISO88591) descItem.length *= SQL_DBCHAR_SIZE;
          }
        } else if (CharInfo::is_NCHAR_MP(newCharSet)) {
          if (descItem.charset == CharInfo::ISO88591)
            descItem.length *= SQL_DBCHAR_SIZE;
          else if (descItem.charset == CharInfo::UNICODE) {
            switch (descItem.datatype) {
              case REC_BYTE_F_DOUBLE:
                descItem.datatype = REC_BYTE_F_ASCII;
                break;
...
RETCODE Descriptor::alloc(int used_entries_) {
  ...
  desc = (desc_struct *)heap.allocateMemory((size_t)used_entries * sizeof(desc_struct));
...
RETCODE Descriptor::dealloc() {
  if (desc) {
    ...
    for (int i = 0; i < used_entries; i++) {
      ...
      if (desc[i].table_name) heap->deallocateMemory(desc[i].table_name);
      if (desc[i].schema_name) heap->deallocateMemory(desc[i].schema_name);
      if (desc[i].catalog_name) heap->deallocateMemory(desc[i].catalog_name);
      if (desc[i].charset_schema) heap->deallocateMemory(desc[i].charset_schema);
      if (desc[i].charset_catalog) heap->deallocateMemory(desc[i].charset_catalog);
      if (desc[i].charset_name) heap->deallocateMemory(desc[i].charset_name);
      if (desc[i].coll_schema) heap->deallocateMemory(desc[i].coll_schema);
      if (desc[i].coll_catalog) heap->deallocateMemory(desc[i].coll_catalog);
      if (desc[i].coll_name) heap->deallocateMemory(desc[i].coll_name);
      if (desc[i].text_format) heap->deallocateMemory(desc[i].text_format);
    }
...
RETCODE Descriptor::allocBulkMoveInfo() {
  ...
  if ((!bulkMoveInfo()) || ((int)bulkMoveInfo()->maxEntries() < used_entries)) {
    ...
    bmInfo_ = (BulkMoveInfo *)heap->allocateMemory(sizeof(BulkMoveInfo) +
                                                   (size_t)(used_entries - 1) * sizeof(BulkMoveInfo::BMInfoStruct));
...
NABoolean Descriptor::doSlowMove() { return (flags_ & DO_SLOW_MOVE) != 0; }

NABoolean Descriptor::bulkMoveSetup() { return (flags_ & BULK_MOVE_SETUP) != 0; }
...
SQLCLI_OBJ_ID *Descriptor::GetNameViaDesc(SQLDESC_ID *desc_id, ContextCli *context, NAHeap &heap) {
  ...
  if (!((datatype >= REC_MIN_CHARACTER) && (datatype <= REC_MAX_CHARACTER))) {
    *diagsArea << DgSqlCode(-CLI_INVALID_SQL_ID);
    return NULL;
  }

  short retcode =
      convDoIt(data, length, datatype, 0, 0, buf, length + 1, REC_BYTE_V_ANSI, 0, 0, 0, 0, &heap, &diagsArea);
  ...
  if (buf) {
    ...
    if (str_to_ansi_id(buf, ansi_id, length)) {
      diags << DgSqlCode(-CLI_INVALID_SQL_ID) << DgString0(buf);
      return NULL;
    }
  ...
  SQLCLI_OBJ_ID *obj_id = (SQLCLI_OBJ_ID *)(heap.allocateMemory(sizeof(SQLCLI_OBJ_ID)));

  switch (desc_id->name_mode) {
    case curs_via_desc:

      init_SQLCLI_OBJ_ID(obj_id, SQLCLI_CURRENT_VERSION, cursor_name, 0, ansi_id, 0, SQLCHARSETSTRING_ISO88591, length);
      break;
    case stmt_via_desc:

      init_SQLCLI_OBJ_ID(obj_id, SQLCLI_CURRENT_VERSION, stmt_name, 0, ansi_id, 0, SQLCHARSETSTRING_ISO88591, length);
      break;
    case desc_via_desc:

      init_SQLCLI_OBJ_ID(obj_id, SQLCLI_CURRENT_VERSION, desc_name, 0, ansi_id, 0, SQLCHARSETSTRING_ISO88591, length);
      break;
    default:
      return NULL;
  }
...
void Descriptor::set_text_format(desc_struct &descitem) {
  ...
  short retcode =
      convertTypeToText_basic(temp_text_format, descitem.datatype, descitem.length, descitem.precision, descitem.scale,
                              dt_start,  // rec_datetime_field
                              dt_end,    // rec_datetime_field
                              (short)descitem.precision, (short)descitem.int_leading_precision,
                              (short)0,  // descitem.upshift,
                              (short)0,  // caseinsensitive
                              (CharInfo::CharSet)descitem.charset,
                              descitem.coll_name ? (descitem.coll_name + VCPREFIX_LEN) : NULL, NULL, 0, FALSE);

  if (retcode == 0) {
    descitem.text_format = desc_varchar_alloc_and_copy(temp_text_format, heap, str_len(temp_text_format));
  } else {
