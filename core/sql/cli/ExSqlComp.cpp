...
ExSqlComp::ReturnStatus ExSqlComp::createServer() 
{
  ...
  if (!(sc_=new(h_) IpcServerClass(env_, IPC_SQLCOMP_SERVER, allocMethod_,
	...
	priority = IPC_PRIORITY_DONT_CARE;
      ...
      NAWNodeSetWrapper nodeSet(cliGlobals_->currContext()->getAvailableNodes(),
                                cliGlobals_->getNAClusterInfo());
      if ( !( server_ = sc_->allocateServerProcess(&diags, h_,nodeName_,
						   IPC_CPU_DONT_CARE,
						   priority,
						   1, TRUE, TRUE, 2, NULL, NULL, FALSE, NULL, &nodeSet
						   ) ) )
	ret = ERROR;
      if (diags) 
	{
...
ExSqlComp::ReturnStatus ExSqlComp::loadQueryCache() {
  ...
  ReturnStatus ret = sendRequest(EXSQLCOMP::PROCESSDDL,
    data, dataLen);
...
ExSqlComp::ReturnStatus ExSqlComp::resendControls(NABoolean ctxSw)   // Genesis 10-981211-5986
{
  ...
  if (ret != ERROR)
  {
    ...
    if (ctxt->getAvailableNodes())
      ctxt->getAvailableNodes()->serialize(serializedNodes);

    char userMessage [2*MAX_AUTHID_AS_STRING_LEN + 2 + 2*MAX_USERNAME_LEN + 2 + serializedNodes.size() + 2+12+COL_MAX_SCHEMA_LEN+1];
                                                ...
                                                Lng32 parentQidLen)
{
  ...
  switch (op)
    ...
    case EXSQLCOMP::XN_OPER :
      request = new(h_) 
	CmpMessageXnOper(input_data,(CmpMsgBufLenType)size,h_);
      break;
    ...
    case EXSQLCOMP::DDL_COMMIT_SHARED_CACHE :
      request = new(h_)
        CmpMessageDDLCommitSharedCache(input_data,(CmpMsgBufLenType)size,h_);
      break;

    case EXSQLCOMP::DDL_ROLLBACK_SHARED_CACHE :
      request = new(h_)
        CmpMessageDDLRollbackSharedCache(input_data,(CmpMsgBufLenType)size,h_);
      break;
