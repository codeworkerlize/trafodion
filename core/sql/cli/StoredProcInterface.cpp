...
short ExSPInitReplyBuffer(void *replyBuffer, ULng32 replyBufLen) {
  ...
  if ((rb == NULL) || (replyBufLen == 0)) return -1;
                      ...
                      ComDiagsArea *diagsDesc)  // IN: pointer to diags
{
  ...
  if (replyRow == NULL)  // indicate end of reply rows
  {
    if (diagsDesc != NULL) {
      ...
      else if (diagsDesc->getNumber(DgSqlCode::WARNING_) > 0) {
    ...
    if (rb->moveInSendOrReplyData(FALSE,  // reply
                                  ...
                                  (void *)&us, sizeof(ControlInfo), 0, 0, 0, diagsDesc,
                                  &dDesc) == SqlBuffer::MOVE_SUCCESS) {
      if (diagsDesc) diagsDesc->packObjIntoMessage(dDesc->getTupleAddress());
    } else {
      rc = 1;  // buffer is full
    }
  } else {
    ...
    if (rb->moveInSendOrReplyData(FALSE,  // reply
                                  ...
                                  TRUE,   // do move data.
                                  (void *)&us, sizeof(ControlInfo), 0, rowLen, &tdesc, 0,
...
short ExSPUnpackIOExpr(void *&extractInputExpr, void *&moveOutputExpr, CollHeap *heap) {
  ...
  if ((ie = (ExSPInputOutput *)ie->driveUnpack(extractInputExpr, &dummySPIO, NULL)) == NULL) {
  ...
  if ((oe = (ExSPInputOutput *)oe->driveUnpack(moveOutputExpr, &dummySPIO, NULL)) == NULL) {
    // ERROR during unpacking. Most likely version-not-supported.
    return -1;
  } else {
...
short ExSPExtractInputValue(void *extractInputExpr, ULng32 fieldNum, char *inputRow, char *data, ULng32 datalen,
                            NABoolean casting, ComDiagsArea *diagsArea) {
  return ((ExSPInputOutput *)extractInputExpr)->inputValue(fieldNum, inputRow, data, datalen, casting, diagsArea);
}

short ExSPMoveOutputValue(void *moveOutputExpr, ULng32 fieldNum, char *outputRow, char *data, ULng32 datalen,
                          NABoolean casting, ComDiagsArea *diagsArea, CollHeap *heap) {
  return ((ExSPInputOutput *)moveOutputExpr)->outputValue(fieldNum, outputRow, data, datalen, casting, heap, diagsArea);
}
