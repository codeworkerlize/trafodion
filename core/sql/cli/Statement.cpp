...
#ifdef _DEBUG
#include <stdarg.h>
// NOTE: The log level for all StmtDebug0-5 macros are LL_TRACE
#define StmtDebug(...)                                                                      \
  do {                                                                                      \
    char tmp[1000];                                                                         \
    snprintf(tmp, sizeof(tmp), ##__VA_ARGS__);                                              \
    StmtPrintf(__VA_ARGS__);                                                                \
    QRLogger::log(CAT_SQL_STMT, LL_TRACE, "%s FILE: %s LINE: %d", tmp, __FILE__, __LINE__); \
  } while (0)
#define StmtDebug0(s)                     StmtDebug((s))
#define StmtDebug1(s, a1)                 StmtDebug((s), (a1))
#define StmtDebug2(s, a1, a2)             StmtDebug((s), (a1), (a2))
...
class Dealloc {
 ...
 public:
  ...
  ~Dealloc() {
    if (addr_ != NULL) heap_->deallocateMemory(addr_);
  }
...
// class Statement
////////////////////////////////////////////////////////////////////
Statement::Statement(SQLSTMT_ID *statement_id_, CliGlobals *cliGlobals, StatementType stmt_type_, char *cn,
                     Module *module)
    : stmt_state(INITIAL_),
      ...
      clonedFrom_(NULL),
      parentCall_(NULL),
      heap_("Statement Heap", context_->exHeap(), 8096 /* 20480 */ /* block size */, 0 /* upperLimit */),
      ...
      spaceObject_(new (context_->exHeap()) Space(Space::EXECUTOR_SPACE, TRUE, (char *)"Stmt Space")),
      space_(*spaceObject_),
  ...
  skipEndValidDDLCheck_ = context_->getSessionDefaults()->getSkipEndValidDDLCheck();
  ...
  SQLMODULE_ID *new_module = (SQLMODULE_ID *)(heap_.allocateMemory(sizeof(SQLMODULE_ID)));
  ...
  if (context_->aqrInfo() && context_->aqrInfo()->aqrStmtInfo() &&
  ...
  switch (statement_id->name_mode) {
    ...
    case curs_via_desc: {
      ...
      SQLCLI_OBJ_ID *tmpObjId = Descriptor::GetNameViaDesc(&tmpDescId, context_, heap_);
  ...
  statementGlobals_ =
      new (&heap_) ExMasterStmtGlobals(0, /* fixup will add the real number */
                                       cliGlobals_, this, 1 /* create gui scheduler */, &space_, &heap_);
  ...
  multiThreadedEsp_ = context_->getSessionDefaults()->isMultiThreadedEsp();
  utf8sql_[0] = '\0';
...
Statement::~Statement() {
  ...
  if (rsInfo) {
    ...
    for (int i = 1; i <= numChildren; i++) {
      ...
      if (found && rsChild) delete rsChild;
    }
  ...
  if (cliGlobals_->getStatsGlobals() != NULL) {
    if (stmtStats_ != NULL) {
      int error = cliGlobals_->getStatsGlobals()->getStatsSemaphore(cliGlobals_->getSemId(), cliGlobals_->myPin());
...
NABoolean Statement::updateInProgress() {
  ...
  if (!isPubsubHoldable()) {
    if (((getState() == Statement::OPEN_ || getState() == Statement::FETCH_ ||
          getState() == Statement::RELEASE_TRANS_) &&
         (getRootTdb() && (getRootTdb()->updDelInsertQuery() || getRootTdb()->isEmbeddedUpdateOrDelete()))))
      return TRUE;
  }
  return FALSE;
}

NABoolean Statement::savepointUsed() {
  if ((context_->getTransaction()) && (context_->getTransaction()->xnInProgress()) &&
      (context_->getTransaction()->getTransMode()->getAutoCommitSavepointOn()) &&
      (context_->getTransaction()->implicitSavepointInProgress()) && (root_tdb && root_tdb->transactionReqd()) &&
      (root_tdb->savepointEnabled()))
    return TRUE;
  else
    return FALSE;
}
...
RETCODE Statement::releaseTransaction(NABoolean allWorkRequests, NABoolean alwaysSendReleaseMsg,
                                      NABoolean statementRemainsOpen) {
  short rc = 0;

  StmtDebug2("[BEGIN releaseTransaction] %p, allWorkRequests = %s", this, (allWorkRequests ? "TRUE" : "FALSE"));
  ...
  NABoolean commitSavepoint = FALSE;
  NABoolean rollbackSavepoint = FALSE;

  if (savepointUsed()) {
    if (context_->getTransaction()->getSavepointState() == SP_TO_COMMIT) {
      commitSavepoint = TRUE;
    } else if (context_->getTransaction()->getSavepointState() == SP_TO_ROLLBACK) {
      rollbackSavepoint = TRUE;
    }
  }

  NABoolean espsUsed = TRUE;
  ExRtFragTable *fragTable = statementGlobals_->getRtFragTable();
  if ((!fragTable) || (fragTable && (fragTable->getState() == ExRtFragTable::NO_ESPS_USED))) espsUsed = FALSE;

  if ((NOT espsUsed) && (commitSavepoint || rollbackSavepoint)) {
    if (commitSavepoint)
      rc = context_->getTransaction()->commitSavepoint(context_->getTransaction()->getSavepointIdWithFlag(),
                                                       context_->getTransaction()->getPSavepointIdWithFlag());
    else
      rc = context_->getTransaction()->rollbackSavepoint(context_->getTransaction()->getSavepointIdWithFlag());
  }
  ...
  if (fragTable) {
    if (root_tcb && root_tcb->fatalErrorOccurred()) {
      ...
      fragTable->releaseTransaction(allWorkRequests, alwaysSendReleaseMsg, commitSavepoint, rollbackSavepoint,
      ...
      IpcTimeout timeout = context_->getSessionDefaults()->getEspReleaseWorkTimeout();
      if (timeout <= 0) {
        if (timeout == -1)
          waitForever = true;
        else if (timeout == -2)
      ...
      while (fragTable->hasOutstandingTransactionalMsges() ||
        ...
        if (fragTable->getState() == ExRtFragTable::ERROR) {
          ...
          if (diagsPtr) context_->diags().mergeAfter(*diagsPtr);
        ...
        statementGlobals_->getIpcEnvironment()->getAllConnections()->waitOnAll(timeout, FALSE, &hasTimedout);

        if (hasTimedout) {
          // timed out waiting for esp reply. something is wrong.
          char errMsg[200];
          str_sprintf(errMsg,
                      "Statement still has %d connections with I/O outstanding - %d trans msgs and %d work reqs.",
                      statementGlobals_->getIpcEnvironment()->getAllConnections()->getNumPendingIOs(),
                      fragTable->numOutstandingTransactionalMsges(), fragTable->numOutstandingWorkRequests());

          if (!waitForever) {
            context_->diags() << DgSqlCode(-EXE_RELEASE_WORK_TIMEOUT) << DgString0(errMsg);
            ...
            if (abendEsps) {
              ...
              statementGlobals_->getIpcEnvironment()->getAllConnections()->fillInListOfPendingPhandles(phandles,
                                                                                                       numEsps);
            ...
            statementGlobals_->getIpcEnvironment()->getAllConnections()->fillInListOfPendingPins(
                logMsg + str_len(initMsg), 300 - str_len(initMsg), 8);
            str_cat(logMsg, ". ", logMsg);
  ...
  if (root_tcb && !statementRemainsOpen) root_tcb->cbMessageWait(cbWaitStartTime);
  ...
  else if (isPubsubHoldable()) {
    if (!getRootTdb()->getPsholdUpdateBeforeFetch()) setState(RELEASE_TRANS_);
  }
...
void Statement::releaseEsps(NABoolean closeAllOpens) {
  ...
  if (espReleaseTimeout != -1) espReleaseTimeout *= 6000;
  while (fragTable->hasOutstandingReleaseEspMsges()) {
    statementGlobals_->getIpcEnvironment()->getAllConnections()->waitOnAll(5 * 6000, FALSE,
                                                                           &hasTimedout);  // Wait up to 5 minutes
    if (hasTimedout) {
...
RETCODE Statement::bindTo(Statement *stmt) {
  StmtDebug2("[BEGIN bindTo] Binding clone %p to master %p", this, stmt);
...
RETCODE Statement::lockTables() {
  ...
  for (int i = 0; i < tableCount; i++) {
    ...
    if (stoi->isUserBaseTable()) {
      ...
      LOGDEBUG(CAT_SQL_LOCK, "Lock succeeded: %s", stoi->ansiName());
    }
  }
  return SUCCESS;
}

void Statement::unlockAllObjects() {
  if (cliLevel_ == 1  // Only for top level statements
      && CmpCommon::context() && context_ && context_->getTransaction() &&
      !context_->getTransaction()->xnInProgress()) {
    LOGTRACE(CAT_SQL_LOCK, "Release all DML/DDL object locks");
    CmpCommon::context()->releaseAllObjectLocks();
  }
}

NABoolean Statement::ddlValid() {
  NABoolean rc = true;  // assume success

  if (root_tdb && root_tcb) {
    SqlTableOpenInfoPtr *stoiList = root_tdb->stoiStoiList();
    if (stoiList) {
      ...
      for (int i = 0; i < root_tcb->getTableCount(); i++) {
        ...
        if (stoi->ddlValidationRequired()) {
          ...
          ObjectEpochCacheEntryName oecName(&heap_, stoi->ansiName(), COM_BASE_TABLE_OBJECT, 0);
          ObjectEpochCacheEntry *oecEntry = NULL;
          ObjectEpochCacheKey key(stoi->ansiName(), oecName.getHash(), COM_BASE_TABLE_OBJECT);
          ...
          // now validate the DDL
          ExDDLValidator ddlValidator(stoi->getExpectedEpoch(), stoi->getExpectedFlags());
          ddlValidator.setValidationInfo(oecEntry);
          if (!ddlValidator.isDDLValid())
            return FALSE;  // DDL definition of this object changed while we were in flight
        }
      }
    }
  }
...
RETCODE Statement::close(ComDiagsArea &diagsArea, NABoolean inRollback) {
  // Unlock dlock (if any). This is a safety net to make sure we have
  // unlocked the dlock.
  unlockDLockForSharedCache();
  ...
  NABoolean isStatementTriggerError = FALSE;

  if (!readyToReturn) {
    if ((stmt_state == OPEN_) && (root_tdb && root_tdb->isEmbeddedIUDWithLast1())) {
    ...
    if (root_tcb) {
      ...
      NABoolean pickUpDiags = (  // False if DML, true otherwise
      ...
      StmtDebug1("  root_tcb->cancel() returned %s", RetcodeToString((RETCODE)rc1));
      if (diagsPtr) {
  ...
  if (!readyToReturn) {
    ...
    if (rsInfo) {
      ...
      for (int i = 1; i <= numChildren && !readyToReturn; i++) {
        ...
        NABoolean closed = FALSE;
        NABoolean prepared = FALSE;
        NABoolean found = rsInfo->getRsInfo(i, proxyStmt, proxySyntax, open, closed, prepared);

        if (found && proxyStmt && proxyStmt->stmt_state != CLOSE_) {
          if (diagsFromProxy == NULL) diagsFromProxy = ComDiagsArea::allocate(&heap_);
      ...
      if (diagsFromProxy) diagsFromProxy->deAllocate();
    ...
    if (closingEmbeddedIUDCursorPrematurely) {
      ...
      if (rollbackTransaction(diagsArea)) return ERROR;
    }
    if (isStatementTriggerError) {
      diagsArea << DgSqlCode(-EXE_STATEMENT_TRIGGER_FAIL);
      if (NOT root_tdb->savepointEnabled()) {
        if (rollbackTransaction(diagsArea)) return ERROR;
      }
      rc = ERROR;
    }
    ...
    if (!inRollback) {
      if ((root_tdb && root_tdb->transactionReqd()) ||
          ((context_->getTransaction()->xnInProgress()) && (context_->getTransaction()->exeStartedXn()) &&
           (context_->getTransaction()->autoCommit()) && (autocommitXn()))) {
        if (!skipEndValidDDLCheck_ && !ddlValid()) {
          LOGERROR(CAT_SQL_STMT, "ROLLBACK transaction due to conflicting concurrent DDL.");
          // there was a conflicting concurrent DDL change; must abort
          if (rollbackTransaction(diagsArea)) diagsArea << DgSqlCode(-CLI_CONFLICTING_CONCURRENT_DDL);
          rc = ERROR;
          readyToReturn = TRUE;
        }
  ...
  // if this statement wrote to the database and the transaction
  // is still outstanding, check for valid DDL at commit time
  if (!inRollback && !skipEndValidDDLCheck_ && root_tdb && root_tdb->transactionReqd() && root_tdb->stoiStoiList() &&
      root_tcb && context_->getTransaction()->xnInProgress())
    context_->addToValidDDLCheckList(root_tdb->stoiStoiList(), root_tcb->getTableCount());

  LOGTRACE(CAT_SQL_LOCK, "Unlock all objects");
  unlockAllObjects();
...
static inline void diagsTakeOver(ComDiagsArea &diagsArea, CliGlobals *cliGlobals, short indexIntoCompilerArray) {
  ComDiagsArea &exec_diags = *cliGlobals->getArkcmp(indexIntoCompilerArray)->getDiags();
  if (&diagsArea != &exec_diags) {
    if (diagsArea.getFunction() == ComDiagsArea::NULL_FUNCTION) diagsArea.setFunction(exec_diags.getFunction());
...
NABoolean Statement::isExeDebug(char *src, int charset) {
  if (!src) {
    ...
    if (charset == SQLCHARSETCODE_UCS2) {
      ...
      return p[0] == NAWchar('$') && p[1] == NAWchar('Z') && p[2] == NAWchar('Z') && p[3] == NAWchar('E') &&
             p[4] == NAWchar('X') && p[5] == NAWchar('E') && p[6] == NAWchar('D') && p[7] == NAWchar('E') &&
             p[8] == NAWchar('B') && p[9] == NAWchar('U') && p[10] == NAWchar('G');
    } else {
...
RETCODE Statement::prepare(char *source, ComDiagsArea &diagsArea, char *passed_gen_code, int passed_gen_code_len,
  ...
  if (aqrReprepareNeeded()) setAqrReprepareNeeded(FALSE);

  if (NOT((!(cliFlags & PREPARE_AUTO_QUERY_RETRY)) && (cliFlags & PREPARE_STANDALONE_QUERY) && source &&
          (stmt_type == DYNAMIC_STMT) && unpackTdbs && (context_->getNumOfCliCalls() == 1))) {
    // do not use query text cache for explicitely prepared queries.
    cliFlags |= PREPARE_NO_TEXT_CACHE;
  }

  if (context_->aqrInfo()) context_->aqrInfo()->setXnStartedAtPrepare(FALSE);

  RETCODE rc = prepare2(source, diagsArea, passed_gen_code, passed_gen_code_len, charset, unpackTdbs, cliFlags);
...
RETCODE Statement::prepare2(char *source, ComDiagsArea &diagsArea, char *passed_gen_code, int passed_gen_code_len,
  ...
  NABoolean sentryPrivRecheck = (cliFlags & PREPARE_SENTRY_PRIV_RECHECK) != 0;
  ...
  NABoolean recompUseNATableCache = (cliFlags & PREPARE_RECOMP_USE_NATABLE_CACHE) != 0;
  ...
  ex_assert(!isCloned(), "Statement::prepare() should not be called on a clone");
  ...
  if (canUseEmbeddedArkcmp) {
    if (!(context_->initEmbeddedArkcmp())) {
      diagsArea << DgSqlCode(-2079);
      return ERROR;
    }
  }

  if (newOperation) assert((aqRetry && source) || ((!reComp) || (reComp && (!source))));
  ...
  if (getenv("TEST_INFO_EVENT")) {
    ...
    const char msgT[] =
        "SQL compiler: Optimization failed at pass two or higher. "
  ...
  if (passed_gen_code) {
    ...
    if (context_->checkAndSetCurrentDefineContext()) {
      ...
      for (short i = 0; i < getContext()->getNumArkcmps(); i++) cliGlobals_->getArkcmp(i)->endConnection();
    }
    ...
    while (retry) {
      if (newOperation) {
        ...
        if ((reComp) || (aqRetry && deCache)) {
          ...
          if (sentryPrivRecheck) c.setSentryPrivRecheck(sentryPrivRecheck);

          c.setDoNotCachePlan(doNotCache);

          if (standaloneQuery) c.setStandaloneQuery(standaloneQuery);
          ...
          dataLen = c.getLength();
          c.setRecompUseNATableCache(recompUseNATableCache);
          // Dealloc dtor will deallocate the memory allocated by next statement
          data = (char *)dealloc.getAddr(new (dealloc.setHeap(&heap_)) char[dataLen]);
          c.pack(data);
          ...
          op =
              (getStatementType() == DYNAMIC_STMT) ? EXSQLCOMP::SQLTEXT_RECOMPILE : EXSQLCOMP::SQLTEXT_STATIC_RECOMPILE;
          ...
          if (cliGlobals_->getArkcmp(indexIntoCompilerArray)->getVersion() == COM_VERS_R1_8) {
            op =
                (getStatementType() == DYNAMIC_STMT) ? EXSQLCOMP::SQLTEXT_RECOMPILE : EXSQLCOMP::SQLTEXT_STATIC_COMPILE;
          }
        ...
        if (context_->getSessionDefaults()->callEmbeddedArkcmp() && canUseEmbeddedArkcmp &&
            context_->isEmbeddedArkcmpInitialized() && CmpCommon::context() &&
        ...
        //! aqRetry  && cliGlobals_->isEmbeddedArkcmpInitialized())
        {
          ...
          if (cliGlobals_->getArkcmp() && cliGlobals_->getArkcmp()->getDiags())
            cliGlobals_->getArkcmp()->getDiags()->clear();

          compStatus = CmpCommon::context()->compileDirect((char *)data, dataLen,
                                                           ...
                                                           charset, op, fetched_gen_code, fetched_gen_code_len,
                                                           context_->getSqlParserFlags(), NULL, 0, da);
          if (da != NULL) {
          ...
          if (compStatus == ExSqlComp::SUCCESS) {
            ...
            diagsArea << DgSqlCode(-CLI_STMT_NOT_PREPARED);
            if (diagsArea.getRollbackTransaction()) rollbackTransaction(diagsArea);
            return prepareReturn(ERROR);
          }
        }

        // TEMPTEMP
        // printf ("indexIntoCompilerArray = %d\n", indexIntoCompilerArray);
        // printf ("(long)cliGlobals_->getArkcmp(indexIntoCompilerArray) = %ld\n",
        //                      (long)cliGlobals_->getArkcmp(indexIntoCompilerArray));
        // TEMPTEMP

        ExSqlComp::ReturnStatus sendStatus =
            cliGlobals_->getArkcmp(indexIntoCompilerArray)->sendRequest(op, data, dataLen, waited, 0, charset, TRUE);

        if (sendStatus == ExSqlComp::ERROR || sendStatus == ExSqlComp::WARNING) {
      ...
      if (status != ExSqlComp::FINISHED) {
        ...
        diagsTakeOver(diagsArea, cliGlobals_, indexIntoCompilerArray);
        diagsArea << DgSqlCode(-CLI_IO_REQUESTS_PENDING) << DgString0("SQLTEXT_COMPILE");
        return prepareReturn(ERROR);
      }

      // initialize pointer to returned data(from reply) in private state.
      ExSqlComp::ReturnStatus replyStatus =
          cliGlobals_->getArkcmp(indexIntoCompilerArray)->getReply(fetched_gen_code, fetched_gen_code_len);
      ...
      if (replyStatus != ExSqlComp::SUCCESS) {
        ...
        if (replyStatus == ExSqlComp::ERROR) {
          if (retry == FALSE) {
            ...
            if (ABS(diagsArea.mainSQLCODE()) != 4074 || diagsArea.getNumber() != 1)
              diagsArea << DgSqlCode(-CLI_STMT_NOT_PREPARED);
            return prepareReturn(ERROR);
          }
        } else {
  ...
  if (unpackTdbs) retcode = (short)unpackAndInit(diagsArea, indexIntoCompilerArray);
...
int Statement::unpackAndInit(ComDiagsArea &diagsArea, short indexIntoCompilerArray) {
  ...
  if (root_tdb && !thisTdb->isFromShowplan()) {
    ...
    //
    ComTdb dummyTdb;
    ex_root_tdb *newRoot = (ex_root_tdb *)root_tdb->driveUnpack((void *)root_tdb, &dummyTdb, unpackSpace_);

    if (newRoot == NULL) {
      // ERROR during unpacking.
      if ((indexIntoCompilerArray >= 0) && (cliGlobals_->getArkcmp(indexIntoCompilerArray)->getDiags()))
  ...
  if (indexIntoCompilerArray >= 0) diagsTakeOver(diagsArea, cliGlobals_, indexIntoCompilerArray);
  ...
  if (statsGlobals != NULL && stmtStats_ != NULL && root_tdb != NULL && getUniqueStmtId() != NULL) {
    ...
    if (!rootTdb->isPacked() && rootTdb->explainInRms() &&
        rootTdb->getFragDir()->getExplainFragDirEntry(fragOffset, fragLen, topNodeOffset) == 0) {
...
RETCODE Statement::closeTables(ComDiagsArea &diagsArea) {
  ...
  retcode = root_tcb->closeTables(statementGlobals_, statementGlobals_->getRtFragTable());
  if (retcode) return ERROR;
...
Statement *Statement::getCurrentOfCursorStatement(char *cursorName) {
  ...
  if (str_to_ansi_id(cursorName, pName, len)) {
    diags << DgSqlCode(-CLI_INVALID_SQL_ID) << DgString0(pName);
    return NULL;
  }
  HashQueue *cursorList = context_->getCursorList();
  ...
  while (stmt = (Statement *)cursorList->getNext()) {
    ...
    if (stmtCursorName && (strcmp(pName, stmtCursorName->identifier) == 0)) {
  ...
  if (pName != nameBuf) heap_.deallocateMemory(pName);
  return pStmt;
}

RETCODE Statement::doQuerySimilarityCheck(TrafQuerySimilarityInfo *qsi, NABoolean &simCheckFailed,
...
RETCODE Statement::fixup(CliGlobals *cliGlobals, Descriptor *input_desc, ComDiagsArea &diagsArea, NABoolean &doSimCheck,
  ...
  if ((context_->aqrInfo()) && (context_->aqrInfo()->espCleanup())) {
  ...
  if ((diagsArea.mainSQLCODE() < 0) || (!root_tcb)) {
    if ((diagsArea.contains(-EXE_TIMESTAMP_MISMATCH) && (NOT tsMismatched()))
  ...
  if (root_tdb->updateCurrentOfQuery() && root_tdb->fetchedCursorName()) {
    currentOfCursorStatement_ = getCurrentOfCursorStatement(root_tdb->fetchedCursorName());

    if (currentOfCursorStatement_ == NULL) {
      diagsArea << DgSqlCode(-CLI_NON_UPDATABLE_SELECT_CURSOR) << DgString0(root_tdb->fetchedCursorName());
      return ERROR;
    }
  }
...
static NABoolean compareTransModes(ex_root_tdb *root_tdb, ExTransaction *currTransaction, ComDiagsArea *diags = NULL,
                                   NABoolean isSysModuleStmt = FALSE) {
  TransMode &tmCompile = *root_tdb->getTransMode(), &tmRuntime = *currTransaction->getTransMode();

#ifdef _DEBUG
  static NABoolean dbg = !!getenv("TRANSMODE_DEBUG");
  if (dbg)
    cerr << "##tm: exe= " << tmRuntime.display() << " cmp= " << tmCompile.display() << " "
         << tmCompile.stmtLevelAccessOptions() << " " << root_tdb->readonlyTransactionOK() << endl;
#endif

  NABoolean recompileDueToRollbackMode = (((tmCompile.getRollbackMode() == TransMode::NO_ROLLBACK_IN_IUD_STATEMENT_) &&
  ...
  if (tmCompile.accessModeCompatible(tmRuntime) || root_tdb->readonlyTransactionOK()) {
    if (tmCompile.isolationLevelCompatible(tmRuntime) || tmCompile.stmtLevelAccessOptions()) {
      if (!recompileDueToRollbackMode && !recompileDueToMultiCommitMode) return FALSE;
    }
  }

  // The run-time transaction mode ($0~int0) differs the compile-time ($1~int1).
  if (diags && !isSysModuleStmt)
    *diags << DgSqlCode(-CLI_TRANS_MODE_MISMATCH) << DgInt0(tmRuntime.display()) << DgInt1(tmCompile.display());
  return TRUE;
}
...
RETCODE Statement::error(ComDiagsArea &diagsArea) {
  ...
  // cryptic message
  if (diagsArea.getNumber(DgSqlCode::ERROR_) == 0) diagsArea << DgSqlCode(-CLI_TCB_EXECUTE_ERROR);
...
RETCODE Statement::execute(CliGlobals *cliGlobals, Descriptor *input_desc, ComDiagsArea &diagsArea,
  ...
  NABoolean optLargVar = (CmpCommon::getDefault(OPTIMIZE_LARGE_VARCHAR) == DF_ON);
  ...
  if (state_ != FIXUP_DONE_) state_ = execute_state;
  ...
  NABoolean readyToReturn = FALSE;
  if (stmtStats_ != NULL) masterStats = stmtStats_->getMasterStats();
  while (readyToReturn == FALSE) {
    ...
    if (getenv("SHOW_STATE")) {
      ...
      switch (state_) {
        case INITIAL_STATE_:
          strcpy(buf, "INITIAL_STATE_");
          break;
        case DO_SIM_CHECK_:
          strcpy(buf, "DO_SIM_CHECK_");
          break;
        ...
        case ERROR_:
          ...
          break;
        case ERROR_RETURN_:
          strcpy(buf, "ERROR_RETURN_");
          break;
        default:
          strcpy(buf, "Unknown state!");
          break;
      }
      cout << "State " << buf << endl;
    }
    ...
    switch (state_) {
      case INITIAL_STATE_: {
        ...
        if (masterStats != NULL) {
          ...
          if (!stmtStats_->aqrInProgress()) aqrInitialExeStartTime_ = -1;
          if (!fixupOnly) masterStats->setExeStartTime(aqrInitialExeStartTime_ == -1 ? jts : aqrInitialExeStartTime_);
          masterStats->initBeforeExecute(jts);
          if (!stmtStats_->aqrInProgress()) masterStats->resetAqrInfo();
        }
        ...
        if (!root_tdb && !allocated() && (statement_id->name_mode == cursor_name) && (statement_id->identifier) &&
          ...
          Statement *bindToStmt = context_->getStatement(&tmpStmtId);
          if (!bindToStmt) {
        ...
        if (parentCall_) {
          ...
          RETCODE proxyRetcode = rsProxyPrepare(*parentRsInfo, myIndex, diagsArea);
        ...
        if (!root_tdb) {
          ...
          if ((allocated()) && (context_->getSessionDefaults()->getAqrType() != 0)) {
        ...
        if (root_tdb->isMaintenanceWindowOFF()) {
          if (root_tdb->getQueryType() == ComTdbRoot::SQL_DDL ||
              root_tdb->getQueryType() == ComTdbRoot::SQL_DDL_WITH_STATUS) {
            switch (root_tdb->getDDLSafeness()) {
              case ComTdbRoot::SQL_SAFE_DDL:
              case ComTdbRoot::SQL_DDL_SAFENESS_UNKNOWN:
                break;
              case ComTdbRoot::SQL_UNSAFE_DDL:
              default:
                char sourceStrCopy[200 + 2];
                int len = (source_length < 200 ? source_length : 200);
                str_cpy_all(sourceStrCopy, source_str, len);
                sourceStrCopy[len] = '\0';
                sourceStrCopy[len + 1] = '\0';
                diagsArea << DgSqlCode(-CLI_CANNOT_EXECUTE_DDL) << DgString0(sourceStrCopy);
                state_ = ERROR_;
                break;
            }
          }
        ...
        if (stmtStats_ != NULL) stmtStats_->setAqrInProgress(FALSE);
        ...
        if ((NOT fixupOnly) || (cliflags & PREPARE_STANDALONE_QUERY)) {
      ...
      case DO_SIM_CHECK_: {
        ...
        retcode = doQuerySimilarityCheck(root_tdb->querySimilarityInfo(), simCheckFailed, diagsArea);
        if (retcode == ERROR) {
          state_ = ERROR_;
          break;
        }
      ...
      case CHECK_DYNAMIC_SETTINGS_: {
        if (fixupState()) {
          ...
          if (statementGlobals_->timeoutSettingChanged() || statementGlobals_->udrRuntimeOptionsChanged()) {
            setFixupState(0);
          }
        }
      ...
      case FIXUP_: {
        ...
        retcode = fixup(cliGlobals, input_desc, diagsArea, doSimCheck, partitionUnavailable, donePrepare);

        StmtDebug2("[END fixup] %p, result is %s", this, RetcodeToString(retcode));

        if (((fixupOnly) || (root_tdb->aqrEnabled())) &&
            ((retcode == ERROR) || (diagsArea.mainSQLCODE() < 0) || (doSimCheck))) {
          state_ = ERROR_;
          break;
        }

        if ((retcode == ERROR) || (partitionUnavailable && partitionAvailabilityChecked)) {
          state_ = ERROR_;
          break;
        }
      ...
      case FIXUP_DONE_: {
        ...
        if (masterStats != NULL) {
          ...
          masterStats->setExeStartTime(aqrInitialExeStartTime_ == -1 ? jts : aqrInitialExeStartTime_);
        }
        state_ = EXECUTE_;
      } break;

      case EXECUTE_: {
        root_tcb->setOptLargVar(optLargVar);
        if (masterStats != NULL) {
          // Lock tables used by the statements to prevent concurrent
          // DML/DDL conflicts
          if (lockTables() < 0) {
            LOGERROR(CAT_SQL_STMT, "Lock tables for statement failed.");
            state_ = ERROR_;
            break;
          }

          masterStats->setFixupEndTime(NA_JulianTimestamp());
          if (!(context_->getSqlParserFlags() & INTERNAL_QUERY_FROM_EXEUTIL) && masterStats->getNumObjUIDs() > 0 &&
              !root_tdb->ddlQuery() && !masterStats->getValidDDL() && !context_->getSkipCheckValidPrivs()) {
            LOGWARN(CAT_SQL_STMT, "DDL redefined");
            diagsArea << DgSqlCode(-CLI_DDL_REDEFINED);
            state_ = ERROR_;
            break;
          }
          if (!masterStats->getValidHistogram()) {  // The AQR due to statistics update do only once.
            if (masterStats->getNumAqrRetries() == 0) {
              diagsArea << DgSqlCode(-CLI_HISTOGRAM_UPDATE);
              state_ = ERROR_;
              break;
            } else
              QRWARN("AQR for UpdateStats QI, but still get invalidation stats. qid=%s", uniqueStmtId_);
          }
          if ((!masterStats->getValidPrivs()) && (!context_->getSkipCheckValidPrivs())) {
        ...
        if (root_tdb) {
          ...
          if (root_tdb->getTriggersCount() > 0) {
            retcode = getTriggersStatus(root_tdb->stoiStoiList(), diagsArea);
            if (retcode == ERROR) {
              state_ = ERROR_;
              break;
            }
          ...
          if (doesUninitializedMvExist(&pMvName, diagsArea)) {
            diagsArea << DgSqlCode(-CLI_MV_EXECUTE_UNINITIALIZED) << DgTableName((const char *)pMvName);
            state_ = ERROR_;
            break;
          }
        }
        ...
        // leaf nodes
        if (isPubsubHoldable() && !root_tdb->isEmbeddedUpdateOrDelete() && !root_tdb->isStreamScan()) {
        ...
        if (root_tdb->updateCurrentOfQuery()) {
          if (root_tdb->fetchedCursorName() == NULL)  // spec via hvar
          {
            ...
            currentOfCursorStatement_ = getCurrentOfCursorStatement((char *)cursor_name_copy);
            if (cursor_name_copy) heap_.deallocateMemory(cursor_name_copy);

            if (currentOfCursorStatement_ == NULL) {
              diagsArea << DgSqlCode(-CLI_NON_UPDATABLE_SELECT_CURSOR) << DgString0((char *)((long)cursor_name));
              state_ = ERROR_;
              break;
            }
          }
          ...
          Int16 cursorTableNameLen = str_len(
              cursorTdb->getLateNameInfoList()->getLateNameInfo(cursorTdb->baseTablenamePosition()).lastUsedAnsiName());

          Int16 updelTableNameLen = str_len(
              root_tdb->getLateNameInfoList()->getLateNameInfo(root_tdb->baseTablenamePosition()).lastUsedAnsiName());
          if ((cursorTableNameLen != updelTableNameLen) ||
              ...
              (str_cmp(root_tdb->getLateNameInfoList()
                       ...
                       cursorTdb->getLateNameInfoList()
                           ...
                           .lastUsedAnsiName(),
                       updelTableNameLen) != 0)) {
            diagsArea << DgSqlCode(-CLI_NON_CURSOR_UPDEL_TABLE) << DgString0(root_tdb->fetchedCursorName());
            retcode = ERROR;
            state_ = ERROR_RETURN_;
            //                    readyToReturn = TRUE;
            break;
        ...
        if (root_tdb->updateCurrentOfQuery()) {
          ...
          char *pkeyRow = currentOfCursorStatement_->getRootTcb()->getPkeyRow();
          if (pkeyRow == NULL) {
            // Add a better error message here. TBD.
            diagsArea << DgSqlCode(-CLI_TCB_EXECUTE_ERROR);
            state_ = ERROR_;
        ...
        if ((getNotAtomicFailureLimit() != 0) && (root_tdb->isNonFatalErrorTolerated())) {
        ...
        */
        ...
        if (getRootTdb()->useDlockForSharedCache()) {
          ...
          // destructed.
          dlockForSharedCache_ = new WaitedLockController(SHARED_CACHE_DLOCK_KEY, 0);
          // Record start time
          if (dlockForSharedCache_->lockHeld()) {
            // get the distributed lock, do the work.
            rc = root_tcb->execute(cliGlobals, statementGlobals_, input_desc, diagsPtr, reExecute);
          } else {
            // can not get the lock. Bail out.
            if (dlockForSharedCache_->lockMethodFailure()) {
              // exception thrown during locking
              diagsArea << DgSqlCode(-FAIL_TO_ACQUIRE_DISTRIBUTED_LOCK) << DgString0("exception thrown");
            }
            rc = -1;
          }
        } else {
          // Normal case, no distributed lock is necessary
          rc = root_tcb->execute(cliGlobals, statementGlobals_, input_desc, diagsPtr, reExecute);
        }

        if (masterStats != NULL) masterStats->setNotBlocking();
        if (rc < 0) retcode = ERROR;
      ...
      case RE_EXECUTE_: {
        ...
        if (inputData_) heap_.deallocateMemory(inputData_);
        inputData_ = NULL;
        char *inputData;
  ...
  StmtDebug2("[END execute] %p, result is %s", this, RetcodeToString(retcode));
  return retcode;

}  // Statement::execute diags

// Since the fetch() method can be called after the no-waited execute() method
// for retrieving records, and error can occure during the process, a new
// label abnormal_fetch_return is provided as a centralized point to
// unlock any distributed lock and return from the fetch() method.
//
// When adding a new error/warning statement in this method, it must go through
// label abnormal_fetch_return via a goto statement. Please refer to some usage
// cases below.
RETCODE Statement::fetch(CliGlobals *cliGlobals, Descriptor *output_desc, ComDiagsArea &diagsArea,
  ...
  if (stmt_state == CLOSE_) {
    ...
    StmtDebug3("[END fetch] %p, result is %s, stmt state %s", this, RetcodeToString(ERROR), stmtState(getState()));
    retcode = ERROR;
    goto abnormal_fetch_return;
  }

  if (stmt_state == EOF_)  // already returned EOF once.
  {
    if (output_desc && output_desc->rowwiseRowsetEnabled())
      output_desc->setDescItem(0, SQLDESC_ROWSET_NUM_PROCESSED, 0, NULL);
    StmtDebug3("[END fetch] %p, result is %s, stmt state %s", this, RetcodeToString(SQL_EOF), stmtState(getState()));
  ...
  masterStats = (stmtStats_ ? stmtStats_->getMasterStats() : NULL);

  if (masterStats) masterStats->setIsBlocking();
  if (output_desc && output_desc->rowwiseRowsetEnabled()) {
    NABoolean eodSeen = FALSE;
    retcode = root_tcb->fetchMultiple(cliGlobals, statementGlobals_, output_desc, diagsPtr, timeout, newOperation,
    ...
    StmtDebug1("  root_tcb->fetchMultiple() returned %s", RetcodeToString((RETCODE)retcode));

  } else {
    retcode = root_tcb->fetch(cliGlobals, statementGlobals_, output_desc, diagsPtr, timeout, newOperation,
                              closeCursorOnError);
    if (output_desc && output_desc->rowwiseRowset()) {
      if ((retcode >= 0) && (retcode != 100)) {
        output_desc->setDescItem(0, SQLDESC_ROWSET_NUM_PROCESSED, 1, NULL);
      } else {
        output_desc->setDescItem(0, SQLDESC_ROWSET_NUM_PROCESSED, 0, NULL);
      }
    }

    StmtDebug1("  root_tcb->fetch() returned %s", RetcodeToString((RETCODE)retcode));
  }

  if (masterStats) masterStats->setNotBlocking();
  ...
  if (retcode < 0) {
    NABoolean spUsed = savepointUsed();
    ...
    if (closeCursorOnError) {
      ...
      if (spUsed) {
        rollbackImplicitSavepoint();
      }

      // this method will also rollback savepoint
      setState(CLOSE_);

      // if savepoint rollback failed, abort this transaction.
      if (spUsed && (context_->getTransaction()->getSavepointState() == SP_ROLLBACK_FAILED ||
                     context_->getTransaction()->getImplicitSavepointState() == SP_ROLLBACK_FAILED))
        doXnRollback = TRUE;
    }
    ...
    // rollbackTransaction flag is set in the diags area or
    // this is a query that would have dirtied the disk(like,
    // insert/update/delete) or savepoint rollback failed.
    ...
    ComMemLog::instance().memlog("%s, Rollback: %ld, %ld", getUniqueStmtId(), 0, 0);
  ...
  if (!newOperation) {
    if (handleUpdDelCurrentOf(diagsArea) == SQL_EOF) retcode = SQL_EOF;
  }

  if ((retcode != NOT_FINISHED) && (retcode != SQL_EOF) && (getRootTdb()->isEmbeddedIUDWithLast1())) {
  ...
  if (retcode == WARNING) {
    StmtDebug3("[END fetch] %p, result is %s, stmt state %s", this, RetcodeToString(WARNING), stmtState(getState()));
    retcode = WARNING;
    goto abnormal_fetch_return;
    ...
    if (!(getRootTdb()->isEmbeddedIUDWithLast1())) {
      if (getRootTdb()->getNumTrafReplicas() > 0) {
        statementGlobals_->setRowsAffected(statementGlobals_->getRowsAffected() / getRootTdb()->getNumTrafReplicas());
        diagsArea.setRowCount(diagsArea.getRowCount() / getRootTdb()->getNumTrafReplicas());
      }
      // Add the affected row count that was directly placed into
      // the master globals to those that are already in the diags
    ...
    if (stmtStats_ != NULL && stmtStats_->getMasterStats() != NULL) {
      stmtStats_->getMasterStats()->setRowsAffected(statementGlobals_->getRowsAffected());
      // ComMemLog::instance().memlog("%s, RowsAffected: %ld", this->source_str, statementGlobals_->getRowsAffected());
    }
    updateStatsAreaInContext();
    ...
    if (parentCall_) {
      ...
      if (parentRsInfo->allResultsAreClosed()) {
        ...
        StmtDebug1("  [EOF] parentCall_->close() returned %s", RetcodeToString(closeResult));
        if (closeResult != ERROR) {
          parentCall_->setState(EOF_);
        } else {
          StmtDebug3("[END fetch] %p, result is %s, stmt state %s", this, RetcodeToString(ERROR),
                     stmtState(getState()));
          retcode = ERROR;
          goto abnormal_fetch_return;
        }
    ...
    if (commitTransaction(diagsArea)) {
      ...
      diagsArea.setRowCount(0);
      if (stmtStats_) stmtStats_->getMasterStats()->setRowsAffected(0);
      retcode = ERROR;
      goto abnormal_fetch_return;
    }
    ...
    if ((context_->getSessionDefaults()->getAltpriFirstFetch()) && (context_->getNumOfCliCalls() == 1) &&
    ...
    if (getRootTdb()->updatableSelect()) resetDeletedCursor();

    StmtDebug3("[END fetch] %p, result is %s, stmt state %s", this, RetcodeToString(SUCCESS), stmtState(getState()));
    return SUCCESS;
  }

abnormal_fetch_return:

#if 0
  char buffer[100];
  sprintf(buffer, "abnormal_fetch_return: retcode=%d", retcode);
#endif
...
short Statement::handleUpdDelCurrentOf(ComDiagsArea &diags) {
  ...
  if (noRowsAffected(diags)) {
    ...
    if (retcode == SQL_EOF) diags << DgSqlCode(SQL_EOF);
...
void Statement::updateTModeValues() {
  ...
  if (!runTransMode) runTransMode = cmpTransMode;
  ...
  if ((roval != TransMode::READ_ONLY_) && (roval != TransMode::READ_ONLY_SPECIFIED_BY_USER_))
  ...
  if (cmpTransMode->getRollbackMode() == TransMode::NO_ROLLBACK_IN_IUD_STATEMENT_) rbval = TransMode::NO_ROLLBACK_;

  // if AutoAbortInterval is not set at runtime then the the compile time
  // setting is applied.
  if (aival == -1) aival = cmpTransMode->getAutoAbortIntervalInSeconds();
...
RETCODE Statement::doOltExecute(CliGlobals *cliGlobals, Descriptor *input_desc, Descriptor *output_desc,
  ...
  ExMasterStats *masterStats = NULL;
  if (root_tdb && getStmtStats() && (NULL != (masterStats = getStmtStats()->getMasterStats()))) {
  ...
  // dynamicly decide transactionReqd() for RegionXN plan
  if (root_tdb && root_tdb->useRegionXN() && NOT root_tdb->transactionReqd() &&
      context_->getTransaction()->getTransMode()->getAutoCommitOff()) {
    root_tdb->setTransactionReqd();
  }

  if (getStmtStats()) getStmtStats()->setAqrInProgress(FALSE);
  // Get the current transmode and update the values
  ...
  if (root_tdb->transactionReqd()) {
    ...
    // execution.
    if ((!context_->getTransaction()->xnInProgress()) ||
        ((context_->getTransaction()->exeStartedXn()) && (context_->getTransaction()->implicitXn()))) {
      doNormalExecute = TRUE;
      reExecute = FALSE;
      return SUCCESS;
    } else {
      if (compareTransModes(root_tdb, context_->getTransaction(), (root_tdb->aqrEnabled() ? &diagsArea : NULL)) &&
          !systemModuleStmt()) {
        if (root_tdb->aqrEnabled()) {
          // return error. AQR will handle recompile and retry.
          return (ERROR);
      ...
      statementGlobals_->getTransid() = context_->getTransaction()->getExeXnId();

      if (context_->getTransaction()->getAutoCommitSavepointOn() && root_tdb->savepointEnabled() &&
          (NOT context_->getTransaction()->implicitXn() || NOT context_->getTransaction()->autoCommit())) {
        beginImplicitSavepoint();
      }
    }
  }
  ...
  if (!fixupState())  // first time
  {
    if ((stmtStats_) && (stmtStats_->getMasterStats())) {
      stmtStats_->getMasterStats()->setFixupStartTime(NA_JulianTimestamp());
    }
    if (dealloc()) goto retError;
    ...
    if ((stmtStats_) && (stmtStats_->getMasterStats())) {
      stmtStats_->getMasterStats()->setFixupEndTime(NA_JulianTimestamp());
    }

    commitImplicitTransAndResetTmodes();
    if (retcode == ERROR) goto retError;
  }
  ...
  updateChildQid();
  if (masterStats) masterStats->setIsBlocking();
  retcode = root_tcb->oltExecute(statementGlobals_, input_desc, output_desc, diagsPtr);
  StmtDebug1("  root_tcb->oltExecute() returned %s", RetcodeToString((RETCODE)retcode));
  ...
  if (retcode < 0) {
    ...
    if (spUsed) {
      // rollback inner-implicit savepoint in user savepoint
      rollbackImplicitSavepoint();
    }
    ...
    // this is a query that would have dirtied the disk(like,
    // insert/update/delete).
    if (rollbackTransaction(diagsArea, doXnRollback)) return ERROR;
    ...
    if (getStmtStats() && (getStmtStats()->getMasterStats()))
      stmtStats_->getMasterStats()->setRowsAffected(statementGlobals_->getRowsAffected());
    if ((statementGlobals_->getRowsAffected() > 0) || (root_tdb->thereIsACompoundStatement()))
      return SUCCESS;
    else {
      if ((output_desc) || (root_tdb && (root_tdb->updDelInsertQuery()))) {
        diagsArea << DgSqlCode(SQL_EOF);
        return SQL_EOF;
      } else
        return SUCCESS;
...
int Statement::cancel() {
  ...
  switch (s) {
    ...
    case CLI_CANCEL_TCB_READY:
      ...
      switch (stmtStats_->getMasterStats()->getState()) {
        case OPEN_:
        case FETCH_:
        case STMT_EXECUTE_:
          root_tcb->passiveCancel();
          break;
        default:
          retcode = -EXE_CANCEL_NOT_POSSIBLE;
          break;
      }
      break;
  ...
  StmtDebug3("[END cancel] %p, result is %s, stmt state %s", this, RetcodeToString((RETCODE)retcode),
             stmtState(getState()));
  return retcode;
}
...
void Statement::releaseStats() {
  ...
  if (ctxStats && (ctxStats == myStats) && ((int)myStats->getCollectStatsType() != SQLCLI_NO_STATS)) {
    if (statsGlobals != NULL && stmtStats_ != NULL && (int)myStats->getCollectStatsType() != SQLCLI_ALL_STATS) {
      ...
      newStats = new (context_->exHeap()) ExStatisticsArea(context_->exHeap(), 0, myStats->getCollectStatsType());
      newStats->setStatsEnabled(myStats->statsEnabled());
      newStats->merge(myStats);
      if (newStats->getMasterStats() == NULL && stmtStats_ != NULL && stmtStats_->getMasterStats() != NULL) {
        ExMasterStats *ems = new (context_->exHeap()) ExMasterStats(context_->exHeap());
        ems->copyContents(stmtStats_->getMasterStats());
        newStats->setMasterStats(ems);
      }
      if (stmtStats_ != NULL) stmtStats_->setStatsArea(NULL);
      // Attach the new stats area to the context
      context_->setStatsArea(newStats, TRUE, FALSE, TRUE);
    }
  } else if (myOrigStatsArea != NULL) {
    // Make sure the ex_globals doesn't delete this stats area
    // in case of dynamic statements, since stmtStats is also
    // pointing to the same area
...
RETCODE Statement::dealloc(NABoolean closeAllOpens) {
  ...
  if ((context_->aqrInfo()->xnStartedAtPrepare()) && (context_->getTransaction()->xnInProgress()) &&
      (context_->getTransaction()->exeStartedXn()) && (context_->getTransaction()->autoCommit()) && (autocommitXn())) {
...
RETCODE Statement::describe(Descriptor *desc, int what_desc, ComDiagsArea &diagsArea) {
  ...
  if (root_tdb->hasCallStmtExpressions() && desc->isDescTypeWide()) {
    ...
    if (expr = root_tdb->inputExpr()) {
      ...
      expr->setInternalFormatIO(flags, context_->getSessionDefaults()->getInternalFormatIO());
      expr->setIsRWRS(flags, (root_tdb->getRWRSInfo() != NULL));
      expr->setIsDBTR(flags, context_->getSessionDefaults()->getDbtrProcess());

      expRType = expr->describeInput(desc, NULL, flags);
      if (expRType != ex_expr::EXPR_OK) retcode = -1;
    }
    ...
    if (what_desc == SQLWHAT_INPUT_DESC) {
      ...
      expr->setIsRWRS(flags, (root_tdb->getRWRSInfo() != NULL));
      expr->setIsDBTR(flags, context_->getSessionDefaults()->getDbtrProcess());
      if (expr->describeInput(desc, root_tdb->getRWRSInfo(), flags) != ex_expr::EXPR_OK) retcode = -1;
    } else {
      /* describe output */
      ...
      expr->setInternalFormatIO(flags, context_->getSessionDefaults()->getInternalFormatIO());
      expr->setIsDBTR(flags, context_->getSessionDefaults()->getDbtrProcess());
      if (expr->describeOutput(desc, flags) != ex_expr::EXPR_OK) retcode = -1;
    }
  }
  if (retcode) {
    diagsArea << DgSqlCode(-CLI_TDB_DESCRIBE_ERROR);
...
void Statement::copyGenCode(char *gen_code, int gen_code_len, NABoolean unpackTDBs) {
  ...
  if (unpackTDBs) {
    ...
    if ((stmt_type == STATIC_STMT) && (root_tdb)) setRecompWarn(root_tdb->recompWarn());
...
void Statement::setCursorName(const char *cn) {
  ...
  StmtDebug2("  Setting cursor name for statement %p to \"%s\"", this, (cn ? cn : "(NULL)"));
  ...
  if (cursor_name_) {
    ...
    if (cn && !strcmp(cn, cursor_name_->identifier)) return;
  ...
  if (cn) {
    ...
    init_SQLCLI_OBJ_ID(cursor_name_, SQLCLI_CURRENT_VERSION, cursor_name, 0, copy_of_cn, 0, SQLCHARSETSTRING_ISO88591,
                       str_len(cn));
...
short Statement::beginTransaction(ComDiagsArea &diagsArea) {
  ...
  if ((root_tdb->transactionReqd()) && ((NOT root_tdb->ddlQuery()) || (NOT context_->getTransaction()->autoCommit()))) {
    // the trans mode at compile time of this query must be the
    // same as the transaction mode at execution time.
    if (!systemModuleStmt() && compareTransModes(root_tdb, context_->getTransaction(), &diagsArea, systemModuleStmt()))
      return ERROR;
    if (context_->getTransaction()->userEndedExeXn()) {
      if (!diagsArea.contains(-CLI_USER_ENDED_EXE_XN)) diagsArea << DgSqlCode(-CLI_USER_ENDED_EXE_XN);
      return ERROR;
    }
    if (!context_->getTransaction()->xnInProgress()) {
    ...
    StmtDebug2("  Statement %p now has trans ID %s", this, TransIdToText(statementGlobals_->getTransid()));
...
short Statement::commitTransaction(ComDiagsArea &diagsArea) {
  ...
  if (savepointUsed() && autocommitSavepoint()) {
    if (isOpen()) {
      NABoolean closeStmt = FALSE;
      NABoolean releaseStmt = FALSE;

      context_->checkIfNeedToClose(this, ContextCli::CLOSE_ALL, closeStmt, releaseStmt);
      if (closeStmt)
        if (close(diagsArea)) return ERROR;
    }

    commitImplicitSavepoint();
  }

  if ((context_->getTransaction()->xnInProgress()) && (context_->getTransaction()->exeStartedXn()) &&
      (context_->getTransaction()->autoCommit()) && (autocommitXn())) {
    if (context_->aqrInfo()) context_->aqrInfo()->setXnStartedAtPrepare(FALSE);
    ...
    // check for any write DML that now has invalid DDL
    if (!context_->doValidDDLChecks(FALSE /* not in rollback */)) {
      LOGERROR(CAT_SQL_STMT, "ROLLBACK transaction due to conflicting concurrent DDL.");
      if (rollbackTransaction(diagsArea))  // have to roll back in this case
        diagsArea << DgSqlCode(-CLI_CONFLICTING_CONCURRENT_DDL);
    }
    ...
    if (context_->getTransaction()->xnInProgress()) {
      StmtDebug2("  About to COMMIT, stmt %p, tx %s...", this, TransIdToText(statementGlobals_->getTransid()));
...
short Statement::rollbackTransaction(ComDiagsArea &diagsArea, NABoolean doXnRollback) {
  ...
  if (context_->getTransaction()->xnInProgress()) {
    ...
    if ((context_->getTransaction()->exeStartedXn()) && (context_->getTransaction()->autoCommit())) {
      rollbackXn = TRUE;
      autoCommitRollback = TRUE;
    } else if (diagsArea.getRollbackTransaction())
      rollbackXn = TRUE;
    else if (root_tdb && (root_tdb->transactionReqd() != 0)) {
      if (root_tdb->getUpdAbortOnError())
        rollbackXn = TRUE;
      else if (root_tdb->getUpdPartialOnError())
        rollbackXn = FALSE;
      else if ((NOT root_tdb->savepointEnabled()) &&
               ((NOT root_tdb->getUpdErrorOnError()) && (NOT diagsArea.getNoRollbackTransaction())))
        rollbackXn = TRUE;
      ...
      if (root_tdb->aqrEnabled() &&
          ((diagsArea.mainSQLCODE() == -EXE_TIMESTAMP_MISMATCH) ||
           (diagsArea.mainSQLCODE() == -CLI_INVALID_QUERY_PRIVS) || (diagsArea.mainSQLCODE() == -CLI_DDL_REDEFINED) ||
           (diagsArea.mainSQLCODE() == -EXE_SCHEMA_SECURITY_CHANGED) ||
           (diagsArea.mainSQLCODE() == -CLI_HISTOGRAM_UPDATE))) {
        rollbackXn = FALSE;
      }
      ...
      if (root_tdb->aqrEnabled() && (diagsArea.mainSQLCODE() == -EXE_LOST_OPEN) &&
    ...
    if (rollbackXn) {
      ...
      NABoolean implicitTran = context_->getTransaction()->implicitXn();
      // get current context and close all statements
      // started under the current transaction
      context_->closeAllCursors(closeCursorType, ContextCli::CLOSE_CURR_XN, 0, TRUE);
      // clean up DDL validation checks
      context_->doValidDDLChecks(TRUE /* in rollback */);

      if (context_->ddlStmtsExecuted()) {
        ...
        if (cmpStatus == ExSqlComp::ERROR) {
          context_->ddlStmtsExecuted() = FALSE;
          diagsArea << DgSqlCode(-EXE_CANT_COMMIT_OR_ROLLBACK);
          return ERROR;
        }

        cmpStatus =
            context_->sendXnMsgToArkcmp((char *)&xnOper, sizeof(xnOper), EXSQLCOMP::DDL_NATABLE_INVALIDATE, diagsPtr);
        if (cmpStatus == ExSqlComp::ERROR) {
          context_->ddlStmtsExecuted() = FALSE;
          diagsArea << DgSqlCode(-EXE_CANT_COMMIT_OR_ROLLBACK);
          return ERROR;
      ...
      // Reset the object epoch values to their original values
      context_->ddlResetObjectEpochs(&diagsArea);

      if (!context_->getTransaction()->xnInProgress()) return 0;
      ...
      if (NOT autoCommitRollback) {
        ...
        if (context_->getTransaction()->exeStartedXn()) {
          ...
          taRetcode = context_->getTransaction()->rollbackStatement();
        } else {
          // doom the transaction
          StmtDebug1("  About to DOOM TX for statement %p...", this);
          taRetcode = context_->getTransaction()->doomTransaction();
        }
      } else {
      ...
      if ((NOT savepointUsed()) && (context_->getTransaction()->getSavepointState() == SP_GENERATED)) {
        ...
        context_->getTransaction()->setSavepointState(SP_SPNONE);
      }
      ...
      if (taRetcode != 0) {
        ...
        unlockAllObjects();
        return ERROR;
      } else if (NOT context_->getTransaction()->getIsPropropagateTX()) {
        ...
        // test environment.
        if (!implicitTran) {
          LOGERROR(CAT_SQL_STMT, "Transaction was aborted");
          diagsArea << DgSqlCode(-CLI_VALIDATE_TRANSACTION_ERROR);
          LOGTRACE(CAT_SQL_LOCK, "Unlock all objects");
          unlockAllObjects();
          return ERROR;
        } else {
          if (context_->aqrInfo()) context_->aqrInfo()->setAbortedTransWasImplicit();
        }
      }
    } else if ((root_tdb) && (root_tdb->getUpdPartialOnError())) {
    ...
    if (NOT rollbackXn && savepointUsed()) {
    }
  }
...
short Statement::beginImplicitSavepoint() {
  short ret = 0;

  if (NOT context_->getTransaction()->implicitSavepointInProgress()) {
    context_->getTransaction()->generateImplicitSavepointId();

    statementGlobals_->getSavepointId() = context_->getTransaction()->getImplicitSavepointIdWithFlag();
    statementGlobals_->getPSavepointId() = context_->getTransaction()->getImplicitPSavepointIdWithFlag();
    context_->getTransaction()->setImplicitSavepointState(SP_GENERATED);
    setAutocommitSavepoint(TRUE);
  }

  return ret;
}

short Statement::commitImplicitSavepoint() {
  // commit inner-implicit savepoint
  if (context_->getTransaction()->implicitSavepointInProgress()) {
    context_->getTransaction()->setImplicitSavepointState(SP_TO_COMMIT);
    context_->getTransaction()->commitImplicitSavepoint(context_->getTransaction()->getImplicitSavepointIdWithFlag(),
                                                        context_->getTransaction()->getImplicitPSavepointIdWithFlag());
    statementGlobals_->getSavepointId() = context_->getTransaction()->getSavepointId();
    statementGlobals_->getPSavepointId() = context_->getTransaction()->getPSavepointId();
  }
  setAutocommitSavepoint(FALSE);

  return TRUE;
}

short Statement::rollbackImplicitSavepoint() {
  // commit inner-implicit savepoint
  if (context_->getTransaction()->implicitSavepointInProgress()) {
    context_->getTransaction()->setImplicitSavepointState(SP_TO_ROLLBACK);
    context_->getTransaction()->rollbackImplicitSavepoint(context_->getTransaction()->getImplicitSavepointIdWithFlag());
    statementGlobals_->getSavepointId() = context_->getTransaction()->getSavepointId();
    statementGlobals_->getPSavepointId() = context_->getTransaction()->getPSavepointId();
  }
  setAutocommitSavepoint(FALSE);
...
void Statement::dump(ostream *outstream) {
  ...
  *outstream << "Space (total):  " << space_.getAllocatedSpaceSize() << '\n';
  ...
  if (source_str) *outstream << source_str << '\n';
...
void Statement::setState(State newState) {
  ...
  switch (stmt_state) {
    ...
    case RELEASE_TRANS_:
      ...
      if ((newState == CLOSE_) || (newState == DEALLOCATED_)) {
        ...
        stmt_state = newState;
        if (NOT oltOpt()) context_->removeFromOpenStatementList(statement_id);
  ...
  if (stmt_state != newState) {
    StmtDebug3("*** Statement %p state change: %s -> %s", this, stmtState(stmt_state), stmtState(newState));
  }

  stmt_state = newState;

  if (stmtStats_ != NULL) {
    if (stmtStats_->getMasterStats() != NULL) stmtStats_->getMasterStats()->setStmtState(newState);
...
NABoolean Statement::noRowsAffected(ComDiagsArea &diagsArea) {
  ...
  if ((root_tdb) && (root_tdb->updDelInsertQuery()) && (diagsArea.getRowCount() == 0) && getRowsAffected() == 0)
    return TRUE;
  else
    return FALSE;
}

NABoolean Statement::isSelectInto() { return (getRootTdb() && getRootTdb()->selectIntoQuery()); }

NABoolean Statement::isDeleteCurrentOf() { return (getRootTdb() && getRootTdb()->deleteCurrentOfQuery()); }

NABoolean Statement::isUpdateCurrentOf() { return (getRootTdb() && getRootTdb()->updateCurrentOfQuery()); }
// BertBert VV
NABoolean Statement::isEmbeddedUpdateOrDelete() { return (getRootTdb() && getRootTdb()->isEmbeddedUpdateOrDelete()); }
NABoolean Statement::isStreamScan() { return (getRootTdb() && getRootTdb()->isStreamScan()); }
// BertBert ^^
...
RETCODE Statement::setPubsubHoldable(ComDiagsArea &diagsArea, NABoolean h) {
  ...
  if (h && root_tdb && !root_tdb->isEmbeddedUpdateOrDelete() && !root_tdb->isStreamScan()) {
    // Holdable cursors are only supported for streaming cursors
    // and destructive cursors because there are some strange
    // side-effects (locks disappear after commit) that we don't
    // want to inflict on non-Publich/Subscribe functionality at
...
int Statement::releaseSpace() {
  ...
  if (stmt_state == INITIAL_) {
    StmtDebug2("[END releaseSpace] %p, result is %s", this, RetcodeToString(SUCCESS));
    return SUCCESS;
  }
...
NABoolean Statement::isReclaimable() {
  if (root_tdb != NULL && (root_tdb->getQueryType() == ComTdbRoot::SQL_INSERT_RWRS || root_tdb->cantReclaimQuery()))
    return FALSE;
  else
    return TRUE;
}
...
RETCODE Statement::completeUdrRequests(NABoolean allRequests) const {
  RETCODE result = SUCCESS;
  while (statementGlobals_ && ((allRequests && statementGlobals_->numUdrMsgsOut() > 0) ||
    ...
    if (conn != NULL && udrServ != NULL) {
      StmtDebug1("  About to call ExUdrServer::completeUdrRequests(%p, FALSE)...", conn);
...
void Statement::setUniqueStmtId(char *id) {
  if (uniqueStmtId_) {
    ...
    if (root_tcb && root_tcb->anyCbMessages()) releaseTransaction(TRUE);
  ...
  if (id) {
    ...
    char tmpLong[20];
    if (statement_id->name_mode == stmt_handle) str_ltoa((ULong)getStmtHandle(), tmpLong);
    ComSqlId::createSqlQueryId(uniqueStmtId_, ComSqlId::MAX_QUERY_ID_LEN + 1, uniqueStmtIdLen_,
                               strlen(context_->getSessionId()), context_->getSessionId(),
                               cliGlobals_->getNextUniqueNumber(),
                               ((statement_id->name_mode == stmt_handle) ? strlen(tmpLong) : strlen(getIdentifier())),
                               (char *)((statement_id->name_mode == stmt_handle) ? tmpLong : getIdentifier()));
  }
}
...
ex_root_tdb *Statement::assignRootTdb(ex_root_tdb *new_root_tdb) {
  if (root_tdb == new_root_tdb) return root_tdb;
...
NABoolean Statement::containsUdrInteractions() const { return (root_tdb && root_tdb->containsUdrInteractions()); }
...
NABoolean Statement::isIudTargetTable(char *tableName, SqlTableOpenInfoPtr *stoiList) {
  ...
  for (int i = 0; root_tcb && i < root_tcb->getTableCount(); i++) {
    ...
    if (!strcmp(stoi->ansiName(), tableName)) {
      found = TRUE;
      if (stoi->getInsertAccess() || stoi->getUpdateAccess() || stoi->getDeleteAccess()) return TRUE;
    }
  }
...
RETCODE Statement::mvSimilarityCheck(char *table, int siMvBitmap, int rcbMvBitmap, NABoolean &simCheckFailed,
  ...
  if (iud && rcbBitmap.getInitOnStmtMvOnMe()) {
    if (returnRecompWarn())
      diagsArea << DgSqlCode(EXE_SIM_CHECK_FAILED) << DgString0("Table has on statement MVs on it.");
    simCheckFailed = TRUE;
    return SUCCESS;
  }
  ...
  // ON STATEMENT MV on it - recompile
  if (iud && siBitmap.getInitOnStmtMvOnMe() != rcbBitmap.getInitOnStmtMvOnMe()) {
    if (returnRecompWarn())
      diagsArea << DgSqlCode(EXE_SIM_CHECK_FAILED) << DgString0("On statement MVs have been removed.");
    simCheckFailed = TRUE;
    return SUCCESS;
  }
  ...
  if (iud && siBitmap.getLoggingRequired() != rcbBitmap.getLoggingRequired()) {
    if (rcbBitmap.getLoggingRequired()) {
      if (returnRecompWarn()) diagsArea << DgSqlCode(EXE_SIM_CHECK_FAILED) << DgString0("Logging is required.");
    } else {
      if (returnRecompWarn())
        diagsArea << DgSqlCode(EXE_SIM_CHECK_FAILED) << DgString0("Logging is not required any more.");
    }
  ...
  if (siBitmap.getIsEnableRewrite() && !rcbBitmap.getIsEnableRewrite()) {
    if (returnRecompWarn()) diagsArea << DgSqlCode(EXE_SIM_CHECK_FAILED) << DgString0("MV rewrite was disabled.");
    simCheckFailed = TRUE;
    return SUCCESS;
  }
  ...
  if (siMvAuditType != rcbMvAuditType &&
      !((siMvAuditType == COM_MV_NO_AUDIT_ON_REFRESH && rcbMvAuditType == COM_MV_AUDIT) ||
        (rcbMvAuditType == COM_MV_NO_AUDIT_ON_REFRESH && siMvAuditType == COM_MV_AUDIT))) {
    if (returnRecompWarn()) diagsArea << DgSqlCode(EXE_SIM_CHECK_FAILED) << DgString0("MV audit status was changed.");
    simCheckFailed = TRUE;
    return SUCCESS;
  }
  ...
  if (iud && siBitmap.getIsInsertLog() != rcbBitmap.getIsInsertLog()) {
    if (returnRecompWarn())
      diagsArea << DgSqlCode(EXE_SIM_CHECK_FAILED) << DgString0("Table INSERTLOG attribute was changed.");
    simCheckFailed = TRUE;
    return SUCCESS;
  }
  ...
  if (iud && siBitmap.getLoggingRequired() && siRangeLogType != rcbRangeLogType &&
      !((siRangeLogType == COM_NO_RANGELOG && rcbRangeLogType == COM_MIXED_RANGELOG) ||
        (rcbRangeLogType == COM_NO_RANGELOG && siRangeLogType == COM_MIXED_RANGELOG))) {
    if (returnRecompWarn())
      diagsArea << DgSqlCode(EXE_SIM_CHECK_FAILED) << DgString0("Table RANGELOG attribute was changed.");
    simCheckFailed = TRUE;
    return SUCCESS;
  }
  ...
  if (rcbBitmap.getMvStatus() == COM_MVSTATUS_UNAVAILABLE) {
    if (returnRecompWarn())
      diagsArea << DgSqlCode(EXE_SIM_CHECK_FAILED) << DgString0("Materialized view is not available.");
    simCheckFailed = TRUE;
    return SUCCESS;
  }
...
NABoolean Statement::implicitTransNeeded(void) {
  const NABoolean noTransInProgress = (context_->getTransaction()->xnInProgress() == FALSE);
  if (noTransInProgress) anyTransWasStartedByMe_ = TRUE;
...
ExStatisticsArea *Statement::getStatsArea() {
  if (getGlobals()) return getGlobals()->getStatsArea();
  return NULL;
}

ExStatisticsArea *Statement::getOrigStatsArea() {
  if (getGlobals()) return getGlobals()->getOrigStatsArea();
  return NULL;
}

ExStatisticsArea *Statement::getCompileStatsArea() {
  ExStatisticsArea *stats;
  if ((stats = getStatsArea()) != NULL) return stats;
  if (compileStatsArea_ != NULL) return compileStatsArea_;
  if (stmtStats_ == NULL || stmtStats_->getMasterStats() == NULL) return NULL;
  compileStatsArea_ = new (&heap_)
      ExStatisticsArea(&heap_, 0, getRootTdb()->getCollectStatsType(), getRootTdb()->getCollectStatsType());
...
NABoolean Statement::isExcludedFromRMS() {
  ...
  for (int i = 0; i < len2; i++) {
    if (source_str[i] == ' ' || source_str[i] == '\t' || source_str[i] == '\r' || source_str[i] == '\n') continue;
    if (tolower(source_str[i]) != sql[pos]) return false;
    pos++;
    if (pos >= len1) break;
  }
...
void Statement::setStmtStats(NABoolean autoRetry) {
  ...
  if (stmtStats_ != NULL) {
    if (getRootTdb()) assignRootTdb(NULL);
  }
  statsGlobals = cliGlobals_->getStatsGlobals();
  if (statsGlobals != NULL) {
    if (isExcludedFromRMS()) {
      stmtStats_ = NULL;
      return;
    }
    ...
    if (autoRetry) {
      if (getUniqueStmtId() != NULL) {
        stmtStats = statsGlobals->getMasterStmtStats(getUniqueStmtId(), getUniqueStmtIdLen(), RtsQueryId::ANY_QUERY_);
    ...
    if (getUniqueStmtId() != NULL) {
      if (!stmtStatsRetained) {
        ...
        if (sd != NULL && sd->getCallEmbeddedArkcmp() && CmpCommon::context() &&
            (CmpCommon::context()->getRecursionLevel() == 0))
          useEmbeddedArk = true;
        if (limitLevel != Statement::LIMIT_RMS_USAGE_THOROUGHLY &&
            ((context_->getSqlParserFlags() & 0x20000) == 0 || useEmbeddedArk == false ||
        ...
        if (stmtStats) {
          ...
          getGlobals()->setStatsArea(NULL);
          if (myStats != NULL && context_->getStats() == myStats) context_->setStatsArea(NULL, FALSE, FALSE, FALSE);
        }
      ...
      if (stmtStats_) {
        ...
        if (parentQid != NULL)
          stmtStats_->setParentQid(parentQid, str_len(parentQid), parentQidSystem, parentQidSystemLen,
                                   cliGlobals_->myCpu(), (short)cliGlobals_->myNodeNumber());
        else
    ...
    stmtStats_ = StatsGlobals::addStmtStats(stmtHeap(), cliGlobals_->myPin(), getUniqueStmtId(), getUniqueStmtIdLen(),
...
void Statement::issuePlanVersioningWarnings(ComDiagsArea &diagsArea) {
  if (returnRecompWarn()) {
    if (fetchErrorCode_ != VERSION_NO_ERROR) {
      ...
      diagsArea << DgSqlCode(fetchErrorCode_) << DgInt0(fetchPlanVersion_) << DgString0(fetchNode_)
                << DgInt1(fetchSupportedVersion_);
    }

    if (mxcmpErrorCode_ != VERSION_NO_ERROR) {
      // A plan versioning error was detected at prepare time
      diagsArea << DgSqlCode(mxcmpErrorCode_) << DgInt0(mxcmpStartedVersion_) << DgInt1(COM_VERS_COMPILER_VERSION);
    }
  }
...
ExRsInfo *Statement::getResultSetInfo() const {
  if (statementGlobals_) return statementGlobals_->getResultSetInfo();
  return NULL;
}
...
NABoolean Statement::rsProxyCompare(const char *newSource) const {
  ...
  if (newSource != NULL && source_str != NULL && charset_ == SQLCHARSETCODE_ISO88591 &&
                                  ...
                                  ComDiagsArea &diagsArea)  // INOUT
{
  ...
  if (result != ERROR) {
    if (!preparedFlag) {
      ...
      if (stmt_state != INITIAL_ && root_tdb != NULL) {
        ...
        if (rsProxyCompare(proxySyntax)) okToReusePlan = TRUE;
      }
      ...
      if (!okToReusePlan) {
        ...
        // CALL execution.
        if (CmpCommon::getDefault(TRAF_SWITCH_COMPILER) == DF_ON) {
          ...
          result = prepare((char *)proxySyntax, diagsArea, NULL, 0L, SQLCHARSETCODE_ISO88591, TRUE,
                           PREPARE_USE_EMBEDDED_ARKCMP);
  ...
  StmtDebug3("[END rsProxyPrepare] %p, stmt state %s, retcode %s", this, stmtState(getState()),
             RetcodeToString(result));
...
RETCODE Statement::getProxySyntax(char *proxy, int maxlength, int *spaceRequired, const char *prefix,
  ...
  if (!proxy && !spaceRequired) return SUCCESS;
  ...
  Descriptor *outdesc = new (&heap_) Descriptor(&desc_id, expr->getNumEntries(), context_);
  outdesc->dealloc();
  ...
  for (int entry = 1; entry <= expr->getNumEntries(); entry++) {
    ...
    if (entry == expr->getNumEntries()) {
      ...
      if (spaceLeft < suffixLen) hasEnoughSpace = FALSE;
...
******************************************************************************/
NABoolean Statement::doesUninitializedMvExist(char **pMvName, ComDiagsArea &diagsArea) {
  ...
  if (uninitializedMvList) {
    ...
    for (short i = 0; i < uninitializedMvCount; i++) {
      ...
      if (isUninitializedMv(currentMv.getPhysicalName(), currentMv.getAnsiName(), diagsArea)) {
...
******************************************************************************/
NABoolean Statement::isUninitializedMv(const char *physicalName, const char *ansiName, ComDiagsArea &diagsArea) {
  return FALSE;
}
...
void Statement::buildConsumerQueryTemplate() {
  if (extractConsumerQueryTemplate_ == NULL) {
    ...
    while (!done) {
      ...
      // spaceRequired.
      RETCODE rc = getExtractConsumerSyntax(extractConsumerQueryTemplate_, len, &spaceRequired);
      ex_assert(rc == SUCCESS, "Unexpected error from getExtractConsumerSyntax");
...
int Statement::setParentQid(char *queryId) {
  ...
  if (parentQid_) NADELETEBASIC(parentQid_, &heap_);
  if (queryId != NULL) {
  ...
  if (stmtStats_ && stmtStats_->getMasterStats() != NULL) stmtStats_->getMasterStats()->setParentQid(parentQid_, len);
  return 0;
}
...
int Statement::initStrTarget(SQLDESC_ID *sql_source, ContextCli &currContext, ComDiagsArea &diags,
  ...
  if (sql_source->name_mode == string_data) {
    CharInfo::CharSet externalCharset = CharInfo::getCharSetEnum(sql_source->charset);
    CharInfo::CharSet internalCharset = CharInfo::UTF8;
    ComDiagsArea *diagsPtr = &diags;

    strTarget.init((char *)sql_source->identifier, sql_source->identifier_len,
                   CharInfo::getFSTypeFixedChar(externalCharset), externalCharset, internalCharset,
                   currContext.exCollHeap(), diagsPtr);
  } else {
  ...
  copyInSourceStr(strTarget.getStr(), (octetLen(strTarget.getStr(), strTarget.getIntCharSet())),
...
NABoolean Statement::updateChildQid() {
  ...
  if (statsGlobals != NULL && uniqueStmtId_ != NULL && parentQid_ != NULL && getStatsArea() != NULL &&
      stmtStats_ != NULL && stmtStats_->updateChildQid()) {
    int error = statsGlobals->getStatsSemaphore(cliGlobals_->getSemId(), cliGlobals_->myPin());
    ...
    if (ss != NULL) {
      ...
      if (parentMasterStats) {
        ...
        if (parentMasterStats->getCanceledTime() != -1) parentIsCanceled = TRUE;
      }
    }
    statsGlobals->releaseStatsSemaphore(cliGlobals_->getSemId(), cliGlobals_->myPin());
  }
...
void Statement::updateStatsAreaInContext() {
  ...
  if (stmtStats_ != NULL && statsArea &&
    ...
    if (statsArea->getMasterStats() == NULL) {
      ExMasterStats *ems = new (context_->exHeap()) ExMasterStats(context_->exHeap());
      if (stmtStats_->getMasterStats()) ems->copyContents(stmtStats_->getMasterStats());
...
int Statement::setChildQueryInfo(ComDiagsArea *diagsArea, char *uniqueQueryId, int uniqueQueryIdLen,
  ...
  if (uniqueQueryId == NULL || query_cost_info == NULL || comp_stats_info == NULL) {
    if (diagsArea != NULL) *diagsArea << DgSqlCode(-CLI_INVALID_ATTR_VALUE);
    return ERROR;
  }
  if (childQueryId_ != NULL) {
...
int Statement::getChildQueryInfo(ComDiagsArea &diagsArea, char *uniqueQueryId, int uniqueQueryIdMaxLen,
  ...
  if (childQueryIdLen_ > uniqueQueryIdMaxLen || uniqueQueryId == NULL || uniqueQueryIdLen == NULL) {
  ...
  *uniqueQueryIdLen = childQueryIdLen_;
  if (childQueryCostInfo_ != NULL && query_cost_info != NULL)
    str_cpy_all((char *)query_cost_info, (const char *)childQueryCostInfo_, sizeof(SQL_QUERY_COST_INFO));
  if (childQueryCompStatsInfo_ != NULL && comp_stats_info != NULL)
    str_cpy_all((char *)childQueryCompStatsInfo_, (const char *)comp_stats_info, sizeof(SQL_QUERY_COMPILER_STATS_INFO));
  return SUCCESS;
}

char *Statement::getUtf8Sql(NABoolean withNoReplicate) {
  NAString *res = NULL;
  if (source_length == 0) return NULL;

  if (utf8sql_[0] != '\0') return utf8sql_;

  char source_str_upper[source_length];
  if (withNoReplicate) {
    NAString upperstring = NAString(source_str, source_length);
    source_length = upperstring.length();
    upperstring.toUpper();

    char *buf = NULL;

    strncpy(source_str_upper, upperstring.data(), source_length);

    if ((buf = strrchr(source_str_upper, 'W'))) {
      if ((strncmp(buf, "WITH", 4) == 0) && !NAString(buf).contains("'") && !NAString(buf).contains('"') &&
          strstr(buf, "NO") && strstr(buf, "REPLICATE"))
        source_length = (buf - source_str_upper);
    }
  } else
    strncpy(source_str_upper, source_str, source_length);

  if (charset_ == SQLCHARSETCODE_UCS2) {
    NAString *res = NULL;
    res = unicodeToChar((NAWchar *)source_str_upper, source_length, CharInfo::UTF8, (CollHeap *)stmtHeap());

    if (res == NULL) {
      return NULL;
    }

    if (res->length() + 1 >= TRIGGER_ID_LEN) {
      strncpy(utf8sql_, res->data(), TRIGGER_ID_LEN - 1);
      if (utf8sql_[TRIGGER_ID_LEN - 2] != ';') {
        utf8sql_[TRIGGER_ID_LEN - 1] = ';';
        utf8sql_[TRIGGER_ID_LEN] = '\0';
      } else
        utf8sql_[TRIGGER_ID_LEN - 1] = '\0';
    } else {
      strncpy(utf8sql_, res->data(), res->length());
      if (utf8sql_[res->length() - 1] != ';') {
        utf8sql_[res->length()] = ';';
        utf8sql_[res->length() + 1] = '\0';
      } else
        utf8sql_[res->length()] = '\0';
    }
    delete res;
    res = NULL;
  } else {
    ...
    if (source_length + 1 >= TRIGGER_ID_LEN) {
      ...
      copyLen = source_length;
    }

    strncpy(utf8sql_, source_str_upper, copyLen);
    if (utf8sql_[copyLen - 1] != ';') {
      utf8sql_[copyLen] = ';';
      copyLen += 1;
    }
    utf8sql_[copyLen] = '\0';
...
NABoolean Statement::isInternalTransactionSql() {
  NABoolean ret = false;
  if (statement_id) {
    switch (statement_id->identifier_len) {
      case 13:
        if (strncmp(statement_id->identifier, "STMT_COMMIT_1", 13) == 0) ret = true;
        break;
      case 15:
        if (strncmp(statement_id->identifier, "STMT_TRANS_ON_1", 15) == 0 ||
            strncmp(statement_id->identifier, "STMT_ROLLBACK_1", 15) == 0 ||
            strncmp(statement_id->identifier, "STMT_INTERNAL_1", 15) == 0)
          ret = true;
        break;
      case 16:
        if (strncmp(statement_id->identifier, "STMT_TRANS_OFF_1", 16) == 0) ret = true;
        break;
      case 22:
        if (strncmp(statement_id->identifier, "STMT_BEGIN_SAVEPOINT_1", 22) == 0) ret = true;
        break;
      case 23:
        if (strncmp(statement_id->identifier, "STMT_COMMIT_SAVEPOINT_1", 23) == 0) ret = true;
        break;
      case 24:
        if (strncmp(statement_id->identifier, "STMT_INTERNAL_STATISTICS", 24) == 0) ret = true;
        break;
      case 25:
        if (strncmp(statement_id->identifier, "STMT_ROLLBACK_SAVEPOINT_1", 25) == 0) ret = true;
        break;
      default:
        break;
    }
