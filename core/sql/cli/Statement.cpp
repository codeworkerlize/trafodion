...
#define StmtDebug(...) do {                                             \
    ...
    QRLogger::log(CAT_SQL_STMT, LL_TRACE, "%s FILE: %s LINE: %d",       \
...
Lng32 Statement::getQueryType()
{
    return root_tdb->getQueryType();
}
...
{
  if ((context_->getTransaction()) &&
      (context_->getTransaction()->xnInProgress()) &&
      (context_->getTransaction()->getTransMode()->getAutoCommitSavepointOn()) &&
      (context_->getTransaction()->implicitSavepointInProgress()) &&
      (root_tdb && root_tdb->transactionReqd()) &&
                                      ...
                                      NABoolean statementRemainsOpen)
{
  ...
  if ((NOT espsUsed) &&
      (commitSavepoint || rollbackSavepoint))
    {
      if (commitSavepoint)
        rc = context_->getTransaction()->commitSavepoint
          (context_->getTransaction()->getSavepointIdWithFlag(),
           context_->getTransaction()->getPSavepointIdWithFlag());
      else
        rc = context_->getTransaction()->rollbackSavepoint
          (context_->getTransaction()->getSavepointIdWithFlag());
    }
    ...
    if (stoiList)
    {
      ...
      for (Int32 i = 0; i < root_tcb->getTableCount(); i++)
      {
        ...
        if (stoi->ddlValidationRequired())
        {
          ...
          ObjectEpochCacheEntryName oecName(&heap_,
                                            stoi->ansiName(),
                                            COM_BASE_TABLE_OBJECT,
                                            0);
          ObjectEpochCacheEntry *oecEntry = NULL;
          ObjectEpochCacheKey key(stoi->ansiName(),
            oecName.getHash(),COM_BASE_TABLE_OBJECT);
      ...
      while (retry)
	{
	  if (newOperation)
	      ...
	      if ((reComp) || (aqRetry && deCache))
		{
		  ...
		  if (sentryPrivRecheck)
		    c.setSentryPrivRecheck(sentryPrivRecheck);
               ...
               if (root_tdb->getQueryType() == ComTdbRoot::SQL_DDL ||
                  ...
                  switch (root_tdb->getDDLSafeness()) {
                     ...
                     default:
                        ...
                        diagsArea << DgSqlCode(-CLI_CANNOT_EXECUTE_DDL) 
                               << DgString0(sourceStrCopy);
                        state_ = ERROR_;
                        break;
                  }
	       }
      ...
      root_tcb->setOptLargVar(optLargVar);
		...
		masterStats->
		  setFixupEndTime(NA_JulianTimestamp());
                if (!(context_->getSqlParserFlags() & INTERNAL_QUERY_FROM_EXEUTIL) &&
                    masterStats->getNumObjUIDs() > 0 &&
                    !root_tdb->ddlQuery() &&
                    !masterStats->getValidDDL() &&
                    !context_->getSkipCheckValidPrivs())
                  {
                ...
                if (!masterStats->getValidHistogram())
                  {//The AQR due to statistics update do only once.
            ...
            if (getRootTdb()->useDlockForSharedCache())
            {
              ...
              if ( dlockForSharedCache_->lockHeld() ) {
                 ...
                 if ( dlockForSharedCache_->lockMethodFailure() ) {
                   ...
                   diagsArea << DgSqlCode(-FAIL_TO_ACQUIRE_DISTRIBUTED_LOCK)
                             << DgString0("exception thrown");
                 }
                 rc = -1;
               }
            } else {
	  ...
	  if (diagsPtr)
	    {
          ...
          if (spUsed &&
              (context_->getTransaction()->getSavepointState() == SP_ROLLBACK_FAILED ||
        ...
        if (getRootTdb()->getNumTrafReplicas() > 0)
          {
            ...
            statementGlobals_->setRowsAffected(statementGlobals_->getRowsAffected()/
                                               getRootTdb()->getNumTrafReplicas());
            diagsArea.setRowCount(diagsArea.getRowCount()/
                                  getRootTdb()->getNumTrafReplicas());
          }
	// Add the affected row count that was directly placed into
	// the master globals to those that are already in the diags
				...
				NABoolean &reExecute)
{
  ...
  if (root_tdb &&
      root_tdb->useRegionXN() &&
      NOT root_tdb->transactionReqd() &&
  ...
  if (getStmtStats())
    getStmtStats()->setAqrInProgress(FALSE);
  // Get the current transmode and update the values
      ...
      else
	{
	  ...
	  statementGlobals_->getTransid() =
	    context_->getTransaction()->getExeXnId();
          ...
          if (context_->getTransaction()->getAutoCommitSavepointOn() &&
              root_tdb->savepointEnabled() &&
              (NOT context_->getTransaction()->implicitXn() ||
      ...
      if (rollbackXn)
	{
	  ...
	  NABoolean implicitTran = 
        ...
        if (context_->ddlStmtsExecuted())
        {
            ...
            ExSqlComp::ReturnStatus cmpStatus = context_->sendXnMsgToArkcmp(
                  NULL,
                  0,
                  EXSQLCOMP::DDL_ROLLBACK_SHARED_CACHE,
                  diagsPtr);
            ...
            cmpStatus = context_->sendXnMsgToArkcmp(
                  (char *)&xnOper,
                  sizeof(xnOper),
	  ...
	  if (NOT autoCommitRollback)
		...
		->rollbackTransactionWaited();
          ...
          if ((NOT savepointUsed())
               && (context_->getTransaction()->getSavepointState() ==SP_GENERATED))
          {
                context_->getTransaction()->rollbackSavepoint(
                                context_->getTransaction()->getSavepointIdWithFlag());
...
short Statement::beginImplicitSavepoint()
{
  ...
  if (NOT context_->getTransaction()->implicitSavepointInProgress())
  {
    ...
    statementGlobals_->getSavepointId() =
            context_->getTransaction()->getImplicitSavepointIdWithFlag();
    statementGlobals_->getPSavepointId() =
...
short Statement::commitImplicitSavepoint()
{
  ...
  if (context_->getTransaction()->implicitSavepointInProgress())
  {
    ...
    context_->getTransaction()->commitImplicitSavepoint(
          context_->getTransaction()->getImplicitSavepointIdWithFlag(),
          context_->getTransaction()->getImplicitPSavepointIdWithFlag());
    statementGlobals_->getSavepointId() =
            context_->getTransaction()->getSavepointId();
    statementGlobals_->getPSavepointId() = 
            context_->getTransaction()->getPSavepointId();
  }
...
short Statement::rollbackImplicitSavepoint()
{
  ...
  if (context_->getTransaction()->implicitSavepointInProgress())
  {
    ...
    context_->getTransaction()->rollbackImplicitSavepoint(
                     context_->getTransaction()->getImplicitSavepointIdWithFlag());
    statementGlobals_->getSavepointId() =
            context_->getTransaction()->getSavepointId();
    statementGlobals_->getPSavepointId() = 
...
NABoolean Statement::isExcludedFromRMS()
{
  ...
  for (int i = 0; i < len2; i++)
  {
    if (source_str[i] == ' ' || source_str[i] == '\t' ||
      ...
      continue;
    if (tolower(source_str[i]) != sql[pos])
      return false;
    pos++;
    if (pos >= len1)
      break;
    ...
    if (getUniqueStmtId() != NULL)
    {
      if (! stmtStatsRetained)
      {
        ...
        if (limitLevel != Statement::LIMIT_RMS_USAGE_THOROUGHLY &&
            ((context_->getSqlParserFlags() & 0x20000) == 0 ||
              useEmbeddedArk == false ||
      ...
      if (!okToReusePlan)
      {
        ...
        if (CmpCommon::getDefault(TRAF_SWITCH_COMPILER) == DF_ON) {
          ...
          result = prepare((char *) proxySyntax, diagsArea, NULL, 0L,
                           SQLCHARSETCODE_ISO88591, TRUE, PREPARE_USE_EMBEDDED_ARKCMP);
...
char *Statement::getUtf8Sql(NABoolean withNoReplicate)
{
    ...
    if (withNoReplicate)
    {
      ...
      {
        if ((strncmp(buf, "WITH", 4) == 0) && !NAString(buf).contains("'") && !NAString(buf).contains('"') && strstr(buf, "NO") && strstr(buf, "REPLICATE"))
...
NABoolean Statement::isInternalTransactionSql()
{
  ...
  if (statement_id)
  {
    ...
    case 13 :
    if (strncmp(statement_id->identifier, "STMT_COMMIT_1", 13) == 0)
      ret = true;
    break;
    case 15 :
    ...
    break;
    case 16 :
    if (strncmp(statement_id->identifier, "STMT_TRANS_OFF_1", 16) == 0)
      ret = true;
    break;
    case 22 :
    if (strncmp(statement_id->identifier, "STMT_BEGIN_SAVEPOINT_1", 22) == 0)
      ret = true;
    break;
    case 23 :
    if (strncmp(statement_id->identifier, "STMT_COMMIT_SAVEPOINT_1", 23) == 0)
      ret = true;
    break;
    case 24 :
    if (strncmp(statement_id->identifier, "STMT_INTERNAL_STATISTICS", 24) == 0)
      ret = true;
    break;
    case 25 :
    if (strncmp(statement_id->identifier, "STMT_ROLLBACK_SAVEPOINT_1", 25) == 0)
      ret = true;
    break;
    default :
    break;
