  ...
  while (clause)
    {
	  ...
	  length = operand->getLength();
          ...
          if ((operand->getDatatype() >= REC_MIN_INTERVAL) &&
            ...
            output_desc->setDescItem(entry,
                                     ...
                                     getIntervalCodeLocal(operand->getDatatype()),
                                     0);
            output_desc->setDescItem(entry,
                                     SQLDESC_PRECISION,
                                     operand->getScale(),
	  ...
	  if ( DFS2REC::isAnyCharacter(operand->getDatatype()) || 
               ...
               (operand->getDatatype() == REC_CLOB )) 
            {
              ...
              if ((operand->getDatatype() == REC_BLOB) || 
                  (operand->getDatatype() == REC_CLOB ))
                {
                  output_desc->setDescItem(entry,  SQLDESC_LOB_INLINE_DATA_MAXLEN,
                                           inout_clause->lobInlinedDataMaxLen(), NULL);
                  
                  output_desc->setDescItem(entry,  SQLDESC_LOB_CHUNK_MAXLEN,
                                           inout_clause->lobChunkMaxLen(), NULL);
                }
            }
  ...
  while (clause)
    {
	  ...
	  if ((((dataType >= REC_MIN_CHARACTER) &&
                (dataType <= REC_MAX_CHARACTER)) ) || (dataType == REC_CLOB))
            {
          ...
          if ((operand->getDatatype() == REC_BLOB) || 
              (operand->getDatatype() == REC_CLOB ))
            {
              input_desc->setDescItem(entry,  SQLDESC_LOB_INLINE_DATA_MAXLEN,
                                      inout_clause->lobInlinedDataMaxLen(), NULL);

              input_desc->setDescItem(entry,  SQLDESC_LOB_CHUNK_MAXLEN,
                                      inout_clause->lobChunkMaxLen(), NULL);
            }
	...
	if (rowsetVarLayoutSize > 0) {
	  ...
	  // 12/22/97: added for Unicode Vchar.
	    ...
	    // In the case of VARCHARS, we zero out remaining elements
	      ...
	      if (DFS2REC::isSQLVarChar(sourceType)) { 
			if (sourceLen < operand->getLength())
				memset(target + sourceLen, 0, operand->getLength() - sourceLen);
	      }
	    } 
	    ...
	    else if (diagsArea && (diagsArea->mainSQLCODE() == EXE_STRING_OVERFLOW || diagsArea->mainSQLCODE() == (ExeErrorCode)(-EXE_STRING_OVERFLOW)))
	      {
		Int32 warningMark2 = diagsArea->getNumber(DgSqlCode::WARNING_);
		Int32 counter = warningMark2 - warningMark;
