   ...
   Int32        resetAttributes)
{
   ...
   if (usersInfo.extUsername == NULL)
   {
      ...
      if(completeSetAuthID(defaultUsersInfo,
                           NULL,0,
                           NULL,NULL,
                           false,
                           false,
  ...
  // Get the external user name
   ...
   if ((usersInfo.effectiveUserID != databaseUserID_) || 
   ...
   if (completeSetAuthID(usersInfo,
                         authToken,
                         authTokenLen,
                         slaName,
                         profileName,
...
//                                                                             *
// *****************************************************************************
Int32 ContextCli::completeSetAuthID(
   const USERS_INFO & usersInfo,
   const char  *authToken,
   Int32        authTokenLen,
   const char  *slaName,
...
)
{
   ...
   if (mtEnabled)
   {
     if ((usersInfo.tenantID != tenantID_) || resetAttributes)
       {
         if (retrieveAndSetTenantInfo(usersInfo.tenantID))
           return ERROR;
       }
   }
  ...
  if (statement)
    {
      ...
      if (statement->transactionReqd())
        statement->releaseTransaction(TRUE);
...
NABoolean ContextCli::reclaimStatements() 
{
  ...
  if (((Int64)totalSize) > reclaimTotalMemorySize)
  {
    ...
    while (retcode && 
      ((Int64)totalSize) > reclaimTotalMemorySize &&
...
void ContextCli::buildRuntimeOptions(NAString &result,
                                     const char *options,
                                     const char *delimiters)
{
  if (str_cmp_ne(options, "OFF") != 0 &&
      str_cmp_ne(options, "ANYTHING") != 0)
  {
      ...
      (isTdmArkcmp_ == IS_NOT_TDMARKCMP)) // and we haven't recursed off-process
  {
    ...
    for (Int32 i = 0; i < tableCount; i++)
    {
      ...
      if (stoi->ddlValidationRequired()) // if there is a write to this object
      {
        ...
        {
          if (strcmp(objectName,ddlValidator->objectName()) == 0)
            break;
        ...
        if (!ddlValidator)  // if not already on the list
        {
          ...
          ObjectEpochCacheEntryName oecName(&exHeap_,
                                            objectName,
                                            COM_BASE_TABLE_OBJECT,
                                            0);
          ObjectEpochCacheEntry *oecEntry = NULL;
          ObjectEpochCacheKey key(objectName, 
          ...
          ddlValidator = new (&exHeap_) CliDDLValidator(&exHeap_,
                                                        objectName,
                                                        stoi->getExpectedEpoch(),
                                                        stoi->getExpectedFlags(),
                                                        oecEntry);
...
{
  return CmpSeabaseDDL::ddlResetObjectEpochs(TRUE /*do abort*/, 
                                             TRUE /*clear list*/,
                                             diags);
}
...
{
  if (CmpCommon::getDefault(TRAF_START_UDR) == DF_ON &&
      cliGlobals_->isMasterProcess()){
...
void ContextCli::createMxcmpSession()
{
  ...
  if (availableNodes_)
    availableNodes_->serialize(serializedNodes);
  ...
  char userMessage [2*MAX_AUTHID_AS_STRING_LEN + 2 + 2*MAX_USERNAME_LEN + 2 + serializedNodes.size() + 12 + COL_MAX_SCHEMA_LEN+1];
  int bytesWritten = snprintf(userMessage,
                              sizeof(userMessage),
                              "%d,%d,%s,%d,%s,%s,%s",
                              authOn,
                              databaseUserID_, 
                              ...
                              tenantID_,
                              tenantName_,
                              serializedNodes.c_str(),
                              tenantDefaultSchema_);
  ex_assert(bytesWritten > 0 && bytesWritten < sizeof(userMessage),
            "message buffer to compiler is too small");
  ...
  if (authOn && GetCliGlobals()->isMasterProcess() &&
  ...
  for (int i = 0; i < numSiKeys; i++)
  {
      ...
      || CmpCommon::getDefault(GENERATE_USER_QUERYCACHE) == DF_PRELOAD)
    {
        ...
        siKeyType == COM_QI_STATS_UPDATED)
      {
        ...
        if (siKeyType == COM_QI_OBJECT_REDEF)
          str_sprintf(buf, "delete/*+TRAF_NO_DTM_XN('ON')*/ from  %s.\"%s\".%s where \
...
COM_USER_QUERYCACHE_TEXT, COM_USER_QUERYCACHE_TEXT);
          ...
          str_sprintf(buf, "update/*+TRAF_NO_DTM_XN('ON')*/using upsert  %s.\"%s\".%s set FLAGS = 1 where \
...
COM_USER_QUERYCACHE_TEXT, COM_USER_QUERYCACHE_TEXT);
  ...
  *ssmpMsgStream << *sikMsg;
  ...
  {
    (diagsArea_) << DgSqlCode(-8146)
                 << DgString0("QI request to SSMP timed out after one minute.");
  }
            ...
            /* OUT */     UInt32 * maxExpectedFlagsFound)
{
  ...
  ObjectEpochChangeRequest * oecMsg =  
    new (cliGlobals->getIpcHeap()) ObjectEpochChangeRequest(
                                      cliGlobals->getIpcHeap(),
                                      op,
                                      objectNameLength,
                                      objectName,
  ...
  *ssmpMsgStream << *oecMsg;
  ...
  {
    (diagsArea_) << DgSqlCode(-8146)
                 << DgString0("Object epoch change request to SSMP timed out after one minute.");
  }
  ...
  else 
  {
    ...
    *maxExpectedFlagsFound = ssmpMsgStream->getOecrMaxExpectedFlagsFound(); // from reply message
    ...
    {
      (diagsArea_) << DgSqlCode(-EXE_OEC_UNEXPECTED_STATE) << DgString0(objectName) 
         << DgInt0(*maxExpectedEpochFound) << DgInt1(*maxExpectedFlagsFound)
         << DgInt2(expectedEpoch) << DgInt3(expectedFlags);
    }
    ...
    else if (ssmpMsgStream->getOecrResult() == ObjectEpochChangeReply::CACHE_FULL)
    {
      ...
      switch (CmpCommon::getDefaultLong(TRAF_OBJECT_EPOCH_CACHE_FULL_BEHAVIOR))
      {
        ...
        default:
        {
          ...
          QRLogger::log(CAT_SQL_EXE, LL_INFO, "%s %s",
            "Object Epoch Cache full when creating entry for ",
            objectName);  
          break;       
        }
      }
...
Int32 ContextCli::performObjectLockRequest(const char *objectName,
                                           ...
                                           Int32 delay)
{
  ...
  while (retries < maxRetries || retryAfterClean) {
    ...
    conflictPid = -1;
    tempDiagsArea->clear();
    retcode = performObjectLockRequest(objectName,
                                       objectType,
                                       opType,
                                       nid,
                                       pid,
    ...
    if (retries < maxRetries) {
      LOGTRACE(CAT_SQL_LOCK,
               "DDL %s request failed, will attempt retry %d after %d ms",
               ObjectLockRequest::opTypeLit(opType),
               retries, delay);
      usleep(delay * 1000);
    } else if (conflictLock) {
      LOGDEBUG(CAT_SQL_LOCK,
               "Try to check if the lock holder node %d process %d is still alive",
               conflictNid, conflictPid);
      ...
      if (retryAfterClean) {
        LOGWARN(CAT_SQL_LOCK,
                "Try again after cleaned stale lock holder node %d process %d",
                conflictNid, conflictPid);
      }
    }
  }
                                           ...
                                           ComDiagsArea *tempDiagsArea)
{
  ...
  LOGINFO(CAT_SQL_LOCK, "Perform DDL %s request of %s %s for node %d process %d",
          ObjectLockRequest::opTypeLit(opType),
  ...
  ObjectLockRequest *request =
    new (ipcHeap) ObjectLockRequest(ipcHeap,
                                    objectName,
                                    strlen(objectName),
                                    objectType,
                                    opType,
  ...
  *ssmpMsgStream << *request;
  ...
  LOGDEBUG(CAT_SQL_LOCK, "Send DDL %s request to SSMP",
           ObjectLockRequest::opTypeLit(opType));
  ssmpMsgStream->send(FALSE, -1);
  Int64 startTime = NA_JulianTimestamp();
  Int64 currTime;
  ...
  if (ssmpMsgStream->getState() == IpcMessageStream::BREAK_RECEIVED)
  {
    LOGDEBUG(CAT_SQL_LOCK, "DDL %s request to SSMP cancled",
             ObjectLockRequest::opTypeLit(opType));
  ...
  {
    // if IO pending, return error.
    LOGDEBUG(CAT_SQL_LOCK, "DDL %s Request to SSMP timed out after one minute.",
             ObjectLockRequest::opTypeLit(opType));
  ...
  else
  {
    ...
    LOGDEBUG(CAT_SQL_LOCK, "DDL %s request to SSMP returned %s",
             ObjectLockRequest::opTypeLit(opType),
    ...
    if (state == ObjectLockReply::LOCK_OK) {
      ...
      if (opType == ObjectLockRequest::LOCK) {
        ...
        ObjectLockPointer lock(objectName, objectType);
        ex_assert(!lock->ddlLockedByMe(),
                  "DDL UNLOCK does not work propertly");
      ...
      LOGWARN(CAT_SQL_LOCK,
              "DDL %s request for '%s' failed due to internal error",
              ObjectLockRequest::opTypeLit(opType),
              objectName);
      (*tempDiagsArea) << DgSqlCode(-EXE_STMT_RUNTIME_ERROR)
      ...
      conflictLock = true;
      LOGWARN(CAT_SQL_LOCK,
              "DDL %s request for '%s' failed due to concurrent DML operation on node %d process %d",
              ObjectLockRequest::opTypeLit(opType), objectName,
              conflictNid, conflictPid);
      (*tempDiagsArea) << DgSqlCode(-EXE_OL_UNABLE_TO_GET_DDL_LOCK)
      ...
      conflictLock = true;
      LOGWARN(CAT_SQL_LOCK,
              "DDL %s request for '%s' failed due to concurrent DDL operation on node %d process %d",
              ObjectLockRequest::opTypeLit(opType), objectName,
              conflictNid, conflictPid);
      (*tempDiagsArea) << DgSqlCode(-EXE_OL_UNABLE_TO_GET_DDL_LOCK)
                       << DgString0(objectName)
      ...
      LOGERROR(CAT_SQL_LOCK,
	       ...
	       comObjectTypeName(objectType), objectName);
      (*tempDiagsArea) << DgSqlCode(-EXE_OL_OUT_OF_LOCK_ENTRY)
                       << DgString0(objectName)
                       << DgString1("DDL");
    } else {
      LOGERROR(CAT_SQL_LOCK,
	       "DDL %s request for '%s' failed with invalid lock state: %d",
...
Int32 ContextCli::lockObjectDDL(const char *objectName,
                                ComObjectType objectType)
{
  if (CmpCommon::getDefault(TRAF_OBJECT_LOCK) != DF_ON) {
    return 0;
  ...
  int delay = CmpCommon::getDefaultLong(TRAF_OBJECT_LOCK_DDL_RETRY_DELAY);
  int dLockTimeout =
    CmpCommon::getDefaultLong(TRAF_OBJECT_LOCK_DDL_DIST_LOCK_TIMEOUT);

  ObjectKey key(objectName, objectType);
  LOGDEBUG(CAT_SQL_LOCK,
           "Acquire distributed lock for key %s, timeout %d",
           key.keyName(), dLockTimeout);
  ...
  if (!dlock.lockHeld()) {
    ...
    diagsArea_ << DgSqlCode(-EXE_OL_UNABLE_TO_GET_DIST_LOCK)
	       << DgString0(key.keyName());
  ...
  if (lock->ddlLockedByMe()) {
    LOGDEBUG(CAT_SQL_LOCK,
             "Already holding the DDL lock for %s %s",
             comObjectTypeName(objectType), objectName);
    return 0;
  }

  int retcode = performObjectLockRequest(objectName,
                                         objectType,
                                         ObjectLockRequest::LOCK,
                                         nid, pid, maxRetries, delay);
  if (retcode < 0) {
    // LOCK failed cluster wide, but some nodes may have been locked,
    // need to perforam an UNLOCK to cleanup partial DDL locks.
    LOGDEBUG(CAT_SQL_LOCK,
             "DDL lock for %s %s failed, try clean up partial DDL locks",
             comObjectTypeName(objectType), objectName);
    if (unlockObjectDDL(objectName, objectType, nid, pid) < 0) {
      LOGERROR(CAT_SQL_LOCK,
               "Cleanup partial DDL locks failed for %s %s",
               comObjectTypeName(objectType), objectName);
      diagsArea_ << DgSqlCode(-EXE_OL_CLEANUP_PARTIAL_LOCK)
                 << DgString0(objectName);
    }
  } else {
...
Int32 ContextCli::unlockObjectDDL(const char *objectName,
                                  ComObjectType objectType,
                                  Int32 nid, Int32 pid,
                                  bool all)
{
  ...
  ObjectLockRequest::OpType opType =
    all ? ObjectLockRequest::UNLOCKALL : ObjectLockRequest::UNLOCK;
  ...
  if (retcode < 0) {
    if (all) {
      ...
      diagsArea_ << DgSqlCode(-EXE_OL_UNABLE_TO_RELEASE_DDL_LOCK)
                 << DgString0("all objects");
    } else {
      LOGERROR(CAT_SQL_LOCK,
               "Unlock DDL object lock failed for %s %s",
               comObjectTypeName(objectType), objectName);
      diagsArea_ << DgSqlCode(-EXE_OL_UNABLE_TO_RELEASE_DDL_LOCK)
                 << DgString0(objectName);
    }
...
bool ContextCli::cleanupStaleLockHolder(const LockHolder &holder)
{
  ...
  LOGDEBUG(CAT_SQL_LOCK,
           "Check status of the lock holder node %d process %d",
           nid, pid);
  Int32 rc = msg_mon_get_process_name(nid, pid, processName);
  if (rc != XZFIL_ERR_NOSUCHDEV) {
    LOGDEBUG(CAT_SQL_LOCK,
             "The lock holder is a running process %d on node %d",
             pid, nid);
    if (!force) {
      return false;
    }
  ...
  LOGWARN(CAT_SQL_LOCK,
          "%s stale locks held by node %d process %d",
          force ? "Force clean" : "Clean", nid, pid);
  // Holder process not exist, cleanup all lock held by the process
  int maxRetries = 3;
  int delay = 1000;             // in milliseconds
  rc = performObjectLockRequest("ALL",
                                COM_BASE_TABLE_OBJECT,
                                ObjectLockRequest::CLEANUP,
                                nid, pid, maxRetries, delay);
  if (rc < 0) {
    LOGERROR(CAT_SQL_LOCK,
             "Failed to cleanup all locks held by %d %d",
             nid, pid);
    return false;
  }
  return true;
...
Int32 ContextCli::lockObjectDML(const char *objectName,
                                ComObjectType objectType)
{
  if (CmpCommon::getDefault(TRAF_OBJECT_LOCK) != DF_ON) {
    return 0;
  ...
  while (retries < maxRetries) {
    ...
    if (state == ObjectLockReply::LOCK_CONFLICT_DDL) {
      LOGTRACE(CAT_SQL_LOCK,
               "Conflict with DDL operations on node %d process %d, will attemp retry %d after %d ms",
               conflictHolder.getNid(), conflictHolder.getPid(),
               retries, delay);
    } else if (state == ObjectLockReply::LOCK_OUT_OF_ENTRY) {
      LOGTRACE(CAT_SQL_LOCK,
               "Out of lock entry, will attemp retry %d after %d ms",
               retries, delay);
    } else {
      LOGERROR(CAT_SQL_LOCK,
               "DML lock failed for %s %s with invalid state %d",
               comObjectTypeName(objectType), objectName,
               state);
      ex_assert(0, "Invalid lock state");
    }
  ...
  if (state == ObjectLockReply::LOCK_CONFLICT_DDL) {
    ...
    if (cleaned) {
      ...
      if (state == ObjectLockReply::LOCK_OK) {
        LOGDEBUG(CAT_SQL_LOCK,
                 "DML lock for %s succeeded after clean stale lock",
                 objectName);
        LOGTRACE(CAT_SQL_LOCK, "ContextCli::lockObjectDML exit");
        return 0;
      }
  ...
  if (state == ObjectLockReply::LOCK_CONFLICT_DDL) {
    LOGERROR(CAT_SQL_LOCK,
             "DML lock failed for %s %s, conflict with DDL operations on node %d process %d",
             comObjectTypeName(objectType), objectName,
             conflictHolder.getNid(), conflictHolder.getPid());
    diagsArea_ << DgSqlCode(-EXE_OL_UNABLE_TO_GET_DML_LOCK)
               << DgString0(objectName)
	       << DgInt0(conflictHolder.getNid())
               << DgInt1(conflictHolder.getPid());
  } else if (state == ObjectLockReply::LOCK_OUT_OF_ENTRY) {
    LOGERROR(CAT_SQL_LOCK,
             "DML lock failed for %s %s, out of lock entry",
             comObjectTypeName(objectType), objectName);
    diagsArea_ << DgSqlCode(-EXE_OL_OUT_OF_LOCK_ENTRY)
               << DgString0(objectName)
               << DgString1("DML");
...
Int32 ContextCli::releaseAllDDLObjectLocks()
{
  ...
  CliGlobals *cliGlobals = getCliGlobals();
  ex_assert(cliGlobals,
            "CliGlobals not properly initialized");
  if (!cliGlobals->isMasterProcess()) {
    return 0;
  }
  StatsGlobals *statsGlobals = cliGlobals->getStatsGlobals();
...
ExStatisticsArea *ContextCli::getObjectEpochStats(const char *objectName,
                                                  Lng32 objectNameLen,
                                                  short cpu,
                                                  bool locked,
                                                  short &retryAttempts)
{
  ...
  IpcServer *ssmpServer = ssmpManager->getSsmpServer(exHeap(), NULL /* nodeName */,
           (cpu < 0 ?  cliGlobals->myCpu() : cpu), tempDiagsArea);
  if (ssmpServer == NULL)
 ...
 //finished, and we can corrupt some other statement's heap later on when we deallocate this stream.
  ...
  ObjectEpochStatsRequest * request =
    new (ipcHeap) ObjectEpochStatsRequest(ipcHeap,
                                          objectName,
                                          objectNameLen,
                                          cpu,
                                          locked);
  *ssmpMsgStream << *request;
  ...
  if (ssmpMsgStream->getState() == IpcMessageStream::ERROR_STATE
      && retryAttempts < 3)
  {
    QRWARN("Error state returned for ObjectEpochStatsRequest,"
           " retryAttempts: %d", retryAttempts);
    request->decrRefCount();
    DELAY(100);
    retryAttempts++;
    return getObjectEpochStats(objectName, objectNameLen,
                               cpu, locked, retryAttempts);
  }
 ...
 if (ssmpMsgStream->getState() == IpcMessageStream::BREAK_RECEIVED)
  ...
  if (! ssmpMsgStream->isReplyReceived())
  {
    (diagsArea_) << DgSqlCode(-EXE_RTS_TIMED_OUT)
                 << DgString0("GET OBJECT EPOCH CACHE")
                 << DgInt0(RtsTimeout/100);
    return NULL;
  }
...
ExStatisticsArea *ContextCli::getObjectLockStats(const char *objectName,
                                                  Lng32 objectNameLen,
                                                  short cpu,
                                                  short &retryAttempts)
{
 ...
 //finished, and we can corrupt some other statement's heap later on when we deallocate this stream.
  ...
  ObjectLockStatsRequest * request =
    new (ipcHeap) ObjectLockStatsRequest(ipcHeap,
                                         objectName,
                                         objectNameLen,
  ...
  *ssmpMsgStream << *request;
  ...
  {
    LOGWARN(CAT_SQL_LOCK,
            "Error state returned for ObjectLockStatsRequest, retryAttempts: %d",
            retryAttempts);
    request->decrRefCount();
    DELAY(100);
    retryAttempts++;
    return getObjectLockStats(objectName, objectNameLen,
                              cpu, retryAttempts);
  }
 ...
 if (ssmpMsgStream->getState() == IpcMessageStream::BREAK_RECEIVED)
  ...
  {
    (diagsArea_) << DgSqlCode(-EXE_RTS_TIMED_OUT)
                 << DgString0("GET OBJECT LOCK STATS")
                 << DgInt0(RtsTimeout/100);
    return NULL;
  }
   ...
   
{
   ...
   switch (queryType)
   {
      ...
      case TENANT_BY_NAME:
      {
         ...
         bool temp;
	 if (tenantInfo)
	   authStatus = tenantAuthInfo.getTenantDetails(authName,*tenantInfo);
      }
      break;
      ...
      case TENANT_BY_USER_ID:
      {
         authInfoPtr = &tenantAuthInfo;
	 if(tenantInfo)
	   authStatus = tenantAuthInfo.getTenantDetails(authID, *tenantInfo);
      }   
      break;
   ...
   if (authStatus == CmpSeabaseDDLauth::STATUS_GOOD)
   {
      ...
      if (queryType == QUERY_FOR_EXTERNAL_NAME)
        result = storeName(authInfoPtr->getAuthExtName().data(),authNameFromTable,
        			 authNameMaxLen);
      else
...
// Errors are returned in the CmpCommon::diags area
   switch (authStatus)
   {
      ...
      case CmpSeabaseDDLauth::STATUS_NOTFOUND:
      {
         ...
         if ((queryType == TENANT_BY_NAME) || (queryType == TENANT_BY_USER_ID))
           sqlcode = CLI_TENANT_NOT_REGISTERED;
         diagsArea_.clear();
         diagsArea_ << DgSqlCode(-sqlcode)
                    << DgString0(nameForDiags);
         break;
      }
  ...
  struct SQLSEC_AuthDetails * authDetails)
{
  ...
  // get tenant info
  if (retcode >= 0 && enabled)
    retcode = addTenantInfo( tenantname, usersInfo, authDetails, authInfo);
  ...
  CmpSeabaseDDLauth         & authInfo)
{
  ...
  */
  ...
  // User is registered
  if (authStatus == CmpSeabaseDDLauth::STATUS_GOOD ||
      authStatus == CmpSeabaseDDLauth::STATUS_WARNING)
  {
  ...
  if (COM_LDAP_AUTH == authType &&
      CmpCommon::getDefault(TRAF_AUTO_REGISTER_USER) == DF_ON)
  {
      return 0;
  }
  diagsArea_ << DgSqlCode(-1076)
             << DgString0(username);
  return -1;
}
...
//     0 - successful
// -----------------------------------------------------------------------------     
Int32 ContextCli::addTenantInfo(
  const char                * tenantname,
  USERS_INFO                * usersInfo,
  struct SQLSEC_AuthDetails * authDetails,
  CmpSeabaseDDLauth         & authInfo)
...
GET_TENANT_INFO:
    ...
    if (tname.isNull() && authDetails->num_tenants == 1)
      tname = DB__SYSTEMTENANT;
  }
  ...
  struct SQLSEC_AuthDetails *authDetails)
{
  ...
  Int32 retcode = userInfo.registerUserInternal(dbUserName, dbUserName,
                                                config, true,
                                                ROOT_USER_ID, authPassword);
  assignSqlParserFlags(flagBits);
  if (retcode == 0)
  {
...
Int32 ContextCli::getAuthErrPwdCnt(
                                   Int32 userid,
                                   Int16 &errcnt)
{
    //  1. Disable autocommit
...
Int32 ContextCli::updateAuthErrPwdCnt(
                                      Int32 userid,
                                      Int16 errcnt,
                                      bool reset)
{
...
void ContextCli::setTenantInfoInContext(Int32 tenantID,
                                        const char *tenantName,
                                        Int32 affinity,
                                        Int32 nodeSize,
                                        Int32 clusterSize,
                                        const char *tenantDefaultSchema)
{
  ...
  setTenantInfoInContext(tenantID,
                         tenantName,
                         serializedNodes.c_str(),
                         tenantDefaultSchema);
}
                                        ...
                                        const char *tenantDefaultSchema)
{
  ...
  if ( numErrors == 0 )
    ...
    //alter these values only if the cgroup setting did not return an error
      ...
      if (tenantDefaultSchema != NULL)
        strcpy(tenantDefaultSchema_,tenantDefaultSchema);
...
RETCODE ContextCli::setTenantByName(const char *tenantName)
{
  ...
  RETCODE result = authQuery(TENANT_BY_NAME,
                             ...
                             sizeof(tenantNameFromUsersTable),
                             tenantIDFromUsersTable,
			     &tenantInfo);  // OUT
  assignSqlParserFlags(flagBits);
  ...
  if (result != ERROR)
    {
      ...
      tenantInfo.getTenantNodes()->serialize(serializedTenant);
      setTenantInfoInContext(tenantInfo.getTenantID(),
                             tenantNameFromUsersTable,
                             serializedTenant.c_str(),
...
void ContextCli::setTenantNodes(const char *serializedNodes)
{
  ...
  if (serializedNodes && strlen(serializedNodes) > 0)
    {
      tenantNodes = NAWNodeSet::deserialize(serializedNodes,
                                            &exHeap_);
  ...
  Int32 *& granteeIDs)
{
  ...
  if (roleIDs_ == NULL)
  {
    ...
    {
      QRLogger::log(CAT_SQL_EXE, LL_INFO, "%s", "Recursively calling getRoleList, "
...
{
  if (externalUsername_ &&
    strcmp(externalUsername_, DEFAULT_EXTERNAL_NAME) != 0)
  ...
  result = authQuery(QUERY_FOR_EXTERNAL_NAME,
                     NULL,       
                     databaseUserID_,
                     usersNameFromAuths, //OUT
                     sizeof(usersNameFromAuths),
                     userIDFromAuths);
  ...
  if (result == ERROR)
  {
    ...
    {
      externalUsername_ = (char *)
        exHeap()->allocateMemory(strlen(DEFAULT_EXTERNAL_NAME) + 1);
  ...
  if (externalUsername_)
    exHeap()->deallocateMemory(externalUsername_);
  externalUsername_ = (char *)
    exHeap()->allocateMemory(strlen(usersNameFromAuths) + 1);
...
void ContextCli::getAuthState(Int32 &authenticationType,
...
Int32 ContextCli::switchToCmpContext(Int32 cmpCntxtType)
{
  ...
  for (i = 0; i < cmpContextInfo_.entries(); i++) {
     ...
     if (cmpCntxtInfo->isSameClass(cmpCntxtName)) {
         cmpCntxt = cmpCntxtInfo->getCmpContext();
         if (cmpCntxt->getRecursionLevel() > 0)
            continue;
...
void ContextCli::initGlobalNADefaultsEntries()
{
  CmpContext::initGlobalNADefaultsEntries();
}
...
Int32 ContextCli::loadTrafMetadataInCache()
{
  ...
  ExeCliInterface cliInterface(exHeap(), 0, this);
  Lng32 retcode = cliInterface.executeImmediate((char *) "load trafodion metadata in cache;", 
                                NULL, NULL, TRUE, NULL, 0,&diagsArea);
...
Int32 ContextCli::loadTrafMetadataIntoSharedCache()
{
  ComDiagsArea *diagsArea = &diagsArea_;
  ExeCliInterface cliInterface(exHeap(), 0, this);
  Lng32 retcode = cliInterface.executeImmediate((char *) "load trafodion metadata into shared cache;", 
                                NULL, NULL, TRUE, NULL, 0,&diagsArea);
  if (retcode ==0 || retcode == 100) 
  {
  ...
  if (retcode > 0) 
  {
      QRLogger::log(CAT_SQL_EXE, LL_INFO, "%s %d", "Load trafodion metadata into shared cache completed successfully with warning code", retcode);
  } else
  {
     QRLogger::log(CAT_SQL_EXE, LL_WARN, "%s %d", "Load trafodion metadata into shared cache failed with error code", retcode);
     QRLogger::logDiags(&diagsArea_, CAT_SQL_EXE);
     diagsArea_.clear(); 
  }
  return 0;
...
Int32 ContextCli::loadTrafDataIntoSharedCache()
{
  ComDiagsArea *diagsArea = &diagsArea_;
  ExeCliInterface cliInterface(exHeap(), 0, this);
  Lng32 retcode = cliInterface.executeImmediate((char *)"load trafodion data into shared cache;",
                                                NULL, NULL, TRUE, NULL, 0, &diagsArea);
  if (retcode == 0 || retcode == 100)
  {
  ...
  {
    QRLogger::log(CAT_SQL_EXE, LL_INFO, "%s %d", "Load trafodion data into shared cache completed successfully with warning code", retcode);
  }
  else
  {
    QRLogger::log(CAT_SQL_EXE, LL_WARN, "%s %d", "Load trafodion data into shared cache failed with error code", retcode);
    QRLogger::logDiags(&diagsArea_, CAT_SQL_EXE);
    diagsArea_.clear();
  }
  return 0;
...
void ContextCli::collectSpecialRTStats()
{
   ...
   // the query hash.
   const char* getRTStatsQuery =
        "get statistics for qid current default, options 'AS'";

   ExeCliInterface *cliInterface = new (exHeap())
                       ExeCliInterface(exHeap(),
                                      SQLCHARSETCODE_UTF8,
                                      this,
                                      NULL);

   Int32 retcode = cliInterface->
                executeImmediate((char *) getRTStatsQuery,
                                 NULL,  // outputBuf
                                 NULL,  // outputBufLen
...
void ContextCli::displayAllCmpContextInfo()
{
  ...
  for (int i = 0; i < cmpContextInfo_.entries(); i++)
  {
     ...
     cout << " CmpContext[" << i << "]=" << static_cast<void*>(cmpCntxtInfo->getCmpContext())
          <<"  "<< cmpCntxtInfo->getName()<<"  "<< cmpCntxtInfo->getCmpContext()->getCIindex()
          << endl;
  }
}
...
void ContextCli::displayAllCmpContextInUse()
{
  ...
  for (int i = 0; i < cmpContextInUse_.entries(); i++)
  {
     ...
     cout << " CmpContext[" << i << "]=" << static_cast<void*>(cmpContext)
          << endl;
  }
}
...
Lng32 ContextCli::executeCallTrigger(char * tblName,
				     Lng32 isBefore,
				     Lng32 opType,
				     char * oldSKVBuffer,
				     char * oldBufSize,
...
Int32 ContextCli::getGracePwdCnt(
    Int32 userid,
    Int16 &graceCounter)
{
    //  1. Disable autocommit
...
Int32 ContextCli::updateGracePwdCnt(
    Int32 userid,
    Int16 graceCounter)
{
    //  1. Disable autocommit
