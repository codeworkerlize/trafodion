     ...
     )
{
  ...
  if (stmtStats != NULL)
  {
     if ((masterStats = stmtStats->getMasterStats()) != NULL)
     {
        ...
        if (rootTdb != NULL )
           masterStats->setQueryHash(rootTdb->getQueryHash());
  ...
  if (rootTdb)
    {
      ...
      if (rootTdb->useDlockForSharedCache())
        return retcode;
      ...
      if (clientMaxStatementPooling > 0 && ((! stmt->isStandaloneQ()) && 
            (sqlcode == -CLI_DDL_REDEFINED || 
             sqlcode == -EXE_USER_PREPARE_NEEDED ||
      ...
      if (NOT rootTdb->aqrEnabledForSqlcode(sqlcode))
        return retcode;
    ...
    {
      if ((! stmt->isStandaloneQ()) && (sqlcode == -CLI_DDL_REDEFINED ||
                         sqlcode == -EXE_USER_PREPARE_NEEDED))
         return retcode;
      // rootTdb is null. Query failed during compile.
      // if AQR is off, return error.
      if (currContext->getSessionDefaults()->getAqrType() == 0)
        return retcode;
    }
	  ...
	  if (numRetries < retries)
	      ...
	      if (NOT isERROR(retcode))
		{
		  ...
		  if ((currContext->getSessionDefaults()->getAqrEmsEvent()) &&
            ...
            if (sqlcode == -CLI_HISTOGRAM_UPDATE)
              flags |= PREPARE_RECOMP_USE_NATABLE_CACHE;
     ...
     /*IN*/ SQLCLI_PTR_PAIRS output_ptr_pairs[])
{
  ...
  if (tasks & CLI_PT_GET_INPUT_DESC)
      ...
      if ((setPtrs) && (num_input_ptr_pairs > 0))
	{
	  ...
	  retcode = local_SetDescPointers(input_desc, 1,
	                                  num_input_ptr_pairs, num_ap, &cpy, input_ptr_pairs);
	  if (isERROR(retcode))
	    return SQLCLI_ReturnCode(&currContext,retcode);
	}
    }
  ...
  if (tasks & CLI_PT_GET_OUTPUT_DESC)
      ...
      if ((setPtrs) && (num_output_ptr_pairs > 0))
	{
	  ...
	  retcode = local_SetDescPointers(output_desc, 1,
	                                  num_output_ptr_pairs, num_ap, &cpy, output_ptr_pairs);
	  if (isERROR(retcode))
	    return SQLCLI_ReturnCode(&currContext,retcode);
	}
    }
      ...
      if (rootTdb)
        {
          ...
          if ((sentryAuthExpirationTimeStamp) &&
  ...
  if (tasks & CLI_PT_CLOSE)
	...
	return CliEpilogue(cliGlobals,statement_id,tretcode);
      ...
      if ( rootTdb && rootTdb->isAutoCollectRTStats() )
         currContext.collectSpecialRTStats();
		      ...
		      /*IN*/ SQLSTMT_ID * statement_id)
{
  ...
  return SQLCLI_PerformTasks(cliGlobals, tasks, statement_id,
			     NULL, NULL, 0, 0, 0, VA_LIST_NULL, 0, 0);
}
         ...
         /*IN*/     SQLCLI_PTR_PAIRS   ptr_pairs[])
{
  ...
  return SQLCLI_PerformTasks(cliGlobals, tasks, statement_id,
			     ...
			     num_ptr_pairs, 0, num_ap, ap, ptr_pairs, 0);
}
                        ...
                        /*IN*/     SQLCLI_PTR_PAIRS   ptr_pairs[])
{
  ...
  ULng32 tasks = CLI_PT_PROLOGUE | CLI_PT_GET_INPUT_DESC |
			 ...
			 ? CLI_PT_OPT_STMT_INFO : 0);
  return SQLCLI_PerformTasks(cliGlobals, tasks, statement_id,
                 input_descriptor, NULL, num_ptr_pairs, 0, num_ap, ap, ptr_pairs, 0);
}
          ...
          /*IN*/     SQLCLI_PTR_PAIRS   ptr_pairs[])
{
  ...
  return SQLCLI_PerformTasks(cliGlobals, tasks, statement_id, NULL,
                 output_descriptor, 0,
                 num_ptr_pairs, num_ap, ap, 0, ptr_pairs);
}
...
ULng32 tasks = CLI_PT_GET_OUTPUT_DESC | CLI_PT_FETCH |
  ...
  return SQLCLI_PerformTasks(cliGlobals, tasks, statement_id, NULL,
                 output_descriptor,
                 0, num_ptr_pairs, num_ap, ap, 0, ptr_pairs);
}
              ...
              /*IN*/   SQLCLI_QUAD_FIELDS   quad_fields[])
{
  ...
  retcode = SQLCLI_Fetch(cliGlobals, statement_id, output_descriptor, 0, 0, 
                         VA_LIST_NULL, 0);
                             ...
                             /*IN*/ Int64 transid)
{
  ...
  if (hbaseClient != NULL && currContext.getTransaction() != NULL)
  {
      if (transid == currContext.getTransaction()->getTransid())
        ...
        QRWARN("TransID unmatched, do nothing. Transid from mxosrvr = %ld, "
               "current transidi = %ld",
	  ...
	  if (cmpStatsInfo)
	    {
          ...
          if (statsGlobal && statsGlobal->getStatsHeap()) {
            ...
            query_comp_stats_info->leftShmSize = statsGlobal->getStatsHeap()->getTotalSize() -
                                                 statsGlobal->getStatsHeap()->getAllocSize();
          }
                         ...
                         /*IN OPTIONAL*/      char * string_value)
{
  ...
  else if (attrName == SQL_ATTR_ROWSET_ATOMICITY)
  {
    ...
    {
      if (numeric_value != 0)
        stmt->setSyncQueryForBinglog(TRUE);
    }
  else {
    ...
    /*OUT*/           bool       &auditingEnabled)
{
  ...
  currContext.getAuthState(authenticationType,
                              ...
                              /*IN*/ SQLCLI_PTR_PAIRS   ptr_pairs[])
{
  ...
  retcode = local_SetDescPointers(desc, starting_entry, num_ptr_pairs, num_ap, 
                                  &cpy, ptr_pairs);
  va_end(cpy);
   ...
   Int32        resetAttributes)
{
  ...
  retcode = currContext.setAuthID(usersInfo,
                                  authToken,
                                  authTokenLen,
                                  slaName,
                                  profileName,
...
{
  return cliGlobals->currContext()->setObjectEpochEntry(operation,
                                           objectNameLength,
                                           objectName,
                                           redefTime,
                                           key,
...
Lng32 SQLCLI_LockObjectDDL(CliGlobals *cliGlobals,
            ...
            /* IN */       short objectType)
{
  return cliGlobals->currContext()->lockObjectDDL(objectName,
...
Lng32 SQLCLI_UnLockObjectDDL(CliGlobals *cliGlobals,
            ...
            /* IN */         short objectType)
{
  return cliGlobals->currContext()->unlockObjectDDL(objectName,
                                                    (ComObjectType)objectType);
}
...
Lng32 SQLCLI_LockObjectDML(CliGlobals *cliGlobals,
            ...
            /* IN */       short objectType)
{
  return cliGlobals->currContext()->lockObjectDML(objectName,
                                                  (ComObjectType)objectType);
}
...
)
{
  ...
  exStatsArea = currContext->getObjectEpochStats(objectName,
                                                 objectNameLen,
                                                 cpu,
                                                 locked,
                                                 retryAttempts);
...
)
{
  ...
  exStatsArea = currContext->getObjectLockStats(objectName,
                                                 objectNameLen,
                                                 cpu,
                                                 retryAttempts);
 ...
 )
{
  ...
  switch (qType)
    ...
    case LOB_CLI_CREATE:
      {
        ...
        if (nameSpace)
          {
            strcpy(ns, " attribute namespace '");
            strcat(ns, nameSpace);
            strcat(ns, "' ,stored descriptor ");
          }
        else
          strcpy(ns, " attribute stored descriptor");

        
	// create lob descriptor handle table salted	
   	str_sprintf(query, "create ghost table %s (descPartnKey largeint not null, numChunks int not null, lobLen largeint not null) salt using 8 partitions store by (descPartnKey, syskey) %s ",
		    lobDescHandleName, ns);
	...
	if (cliRC < 0)
	    goto error_return;
       	...
       	str_sprintf(query, "create ghost table %s (descPartnKey largeint not null, descSysKey largeint not null, chunkNum int not null, chunkLen largeint not null, dataOffset largeint, stringParam varchar(400), primary key(descPartnKey, descSysKey, chunkNum)) salt using 8 partitions %s",
                    lobDescChunksName,ns); 
        lobDebugInfo(query,0,__LINE__,lobTrace);
   ...
   case LOB_CLI_PURGEDATA:
	...
	// insert into lob descriptor handle table
        if (lobType == Lob_Inline )
          str_sprintf(query, "select syskey from (insert into table(ghost table %s) values (%ld, 0, %ld)) x",
	...
	if (outLobHandle)
	  {
	    ...
	    ExpLOBoper::updLOBhandleDescKey(descSyskey, outLobHandle); 
  ...
  case LOB_CLI_UPDATE_UNIQUE:
	...
	if (cliRC < 0)
        ...
        if ((lobType != Lob_External) && blackBox)
          {
	...
	if (outLobHandle)
	  {
	    str_cpy_all(outLobHandle, inLobHandle, inLobHandleLen);
	    ExpLOBoper::updLOBhandleDescKey(inDescSyskey, outLobHandle); 
	...
	if (numChunks > 1)
	  {
	    ...
	    *blackBoxLen = -1;
        ...
        else if ((numChunks > 1 ) && (lobType == Lob_External))
          {
    ...
    case LOB_CLI_ALTER :
      {
        ...
        str_sprintf(query, "create ghost table %s (lobnum smallint not null, storagetype smallint not null, location varchar(4096) not null, column_name varchar(256 bytes) character set utf8, primary key (lobnum)) %s ",lobMDName,ns);
        lobDebugInfo(query,0,__LINE__,lobTrace);
	...
	for (Lng32 i = 0; i < numLOBs; i++)
	  {
        ...
        exLobGlob = ExpLOBoper::initLOBglobal(currContext.exHeap(), &currContext, useLibHdfs, FALSE, 0, FALSE);
        if (exLobGlob == NULL) 
          {
            cliRC = -1;
            ComDiagsArea * da = &diags;
	    ...
	    // create lob data tables
	      ...
	      (exLobGlob, &currContext,
	       ...
	       objectUID, lobNumList[i],lobMaxSize,
               ((lobTypList[i] == Lob_External) ? -1 : numLOBdatafiles));
  ...
  if (! cliInterfaceArr[qryIdx])
    {
      ...
      str_sprintf(query, qryStr,
          TRAFODION_SYSCAT_LIT, SEABASE_XDC_MD_SCHEMA, XDC_SEQUENCE_TABLE,
...
static Lng32 SeqGenCliInterfaceProcessXDC(
                          ExeCliInterface ** cliInterfaceArr,
                          SequenceGeneratorAttributes* sga,
                          ContextCli &currContext,
                          ComDiagsArea & diags,
                          ...
                          NABoolean doPrepare,
                          NABoolean doExecute,
                          Int64 nextValue = 0 /*input*/)
{
  if (!(sga->getSGAsyncRepl()))
    return 0;
  Lng32 cliRC = 0;
  if (doPrepare)
  {
    if (! cliInterfaceArr[SEQ_UPSERT_XDC_SEQ_IDX])
    {
      cliRC = SeqGenCliInterfacePrepQry(
              "upsert into %s.\"%s\".%s select CATALOG_NAME, SCHEMA_NAME, OBJECT_NAME, cast(? as largeint not null) from %s.\"%s\".%s O where O.object_uid = %ld;",
               SEQ_UPSERT_XDC_SEQ_IDX,
               "SEQ_UPSERT_XDC_SEQ_IDX",
  ...
  if (doExecute)
  {
    ...
    cliRC = cliInterface->clearExecFetchCloseOpt
            ((char*)inputValues, inputValuesLen, NULL, NULL, NULL);
    if(cliRC < 0)
    {
      if (myDiags == NULL)
        myDiags = ComDiagsArea::allocate(exHeap);
					      ...
					      Int64 &endValue)
{
  ...
  if (! cliInterfaceArr[SEQ_PROCESS_QRY_IDX])
    {
      cliRC = SeqGenCliInterfacePrepQry(
                                        "select  case when cast(? as largeint not null) = 1 then t.minVal else t.nextValue end, t.redefTS from (update %s.\"%s\".%s set next_value = (case when cast(? as largeint not null) = 1 then min_value + cast(? as largeint not null) else (case when next_value + cast(? as largeint not null) > max_value then max_value+1 else next_value + cast(? as largeint not null) end) end), num_calls = num_calls + 1 where seq_uid = %ld return \"OLD\".min_value, \"OLD\".next_value, \"OLD\".redef_ts) t(minVal, nextValue, redefTS);",
                                        SEQ_PROCESS_QRY_IDX,
                                        "SEQ_PROCESS_QRY_IDX",
                                        cliInterfaceArr, sga, currContext, diags, exHeap);
      if (cliRC < 0)
...
static Lng32 SeqGenCliInterfaceUpdAndValidateUseDlock(
					      ExeCliInterface ** cliInterfaceArr,
					      SequenceGeneratorAttributes* sga,
					      NABoolean recycleQry,
					      ContextCli &currContext,
					      ...
					      Int64 &endValue)
{
  ...
  if (! cliInterfaceArr[SEQ_PROCESS_QRY_IDX_DL])
    {
      cliRC = SeqGenCliInterfacePrepQry(
                                        "select  case when cast(? as largeint not null) = 1 then t.minVal else t.nextValue end, t.redefTS from (update %s.\"%s\".%s set next_value = (case when cast(? as largeint not null) = 1 then min_value + cast(? as largeint not null) else (case when next_value + cast(? as largeint not null) > max_value then max_value+1 else next_value + cast(? as largeint not null) end) end), num_calls = num_calls + 1, upd_ts = cast(? as largeint not null) where seq_uid = %ld return \"OLD\".min_value, \"OLD\".next_value, \"OLD\".redef_ts) t(minVal, nextValue, redefTS);",
                                        SEQ_PROCESS_QRY_IDX_DL,
                                        "SEQ_PROCESS_QRY_IDX_DL",
                                        cliInterfaceArr, sga, currContext, diags, exHeap);
      if (cliRC < 0)
       return cliRC;
  ...
  cliRC = SeqGenCliInterfaceProcessXDC(cliInterfaceArr, sga, currContext,
                                       diags, exHeap, true/*doPrepare*/,
                                       false/*doExecute*/);
  if (cliRC < 0)
    return cliRC;
  ...
  cliRC = SeqGenCliInterfaceProcessXDC(cliInterfaceArr, sga, currContext,
                                       diags, exHeap, false/*doPrepare*/,
                                       true/*doExecute*/, valueForXdc);
  lock->unlock();
  return cliRC;
}

static void SeqGenUseDtmHandleError(Int32 cliRC, 
                                    ExeCliInterface *cliInterface,
                                    DistributedLock_JNI *lock,
                                    NAHeap *exHeap,
                                    ComDiagsArea &diags)
{
  if (cliRC >= 0)
...
static Lng32 SeqGenCliInterfaceUpdUseDtm(
     ExeCliInterface ** cliInterfaceArr,
     SequenceGeneratorAttributes* sga,
     NABoolean recycleQry,
     ContextCli &currContext,
     ...
     Int64 &endValue)
{
  ...
  if (NOT xnWasSuspendedHere)
    cliInterface->saveXnState0();
						   ...
						   Int64 &endValue)
{
  ...
  if (useDlockImpl && useDtmImpl)
    useDtmImpl = FALSE;
  while ((NOT isOk) && (numTries < maxRetryNum))
    {
      if (useDtmImpl)
        {
          cliRC = SeqGenCliInterfaceUpdUseDtm(
               cliInterfaceArr,
               sga,
               recycleQry,
               currContext,
        ...
        cliRC = SeqGenCliInterfaceUpdAndValidateUseDlock(
					       cliInterfaceArr,
					       sga,
					       recycleQry,
					       currContext,
 ...
 label_return:
  if (retCliRC == 0)
    cqdCliInterface->clearGlobalDiags();
  return retCliRC;
}
...
Lng32 SQLCLI_OrderSeqXDCCliInterface
(
  CliGlobals *cliGlobals,
  void* *inCliInterfaceArr,
  void * seqGenAttrs,
...
)
{
  ...
  SequenceGeneratorAttributes* sga = (SequenceGeneratorAttributes*)seqGenAttrs;
  cliRC = SeqGenCliInterfaceProcessXDC(cliInterfaceArr, sga, currContext,
                                       diags, currContext.exHeap(),true/*doPrepare*/,
                                       true/*doExecute*/, endValue);
  return cliRC;
}
...
{
  return ExpLOBoper::extractFieldsFromHandleString(
       handleStr, handleLen, lobV2, objectUID, inlineDataLen, inlineDataOffset);
}
