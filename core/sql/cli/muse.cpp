...
void ExHeapStats::GetStatistics() {
  ...
  while (block != NULL) {
    ...
    if (parentStats_ != NULL && block->segmentId_ == NABlock::DERIVED_SEGMENT_ID)
    ...
    while (!fp->isFencePost()) {
      ...
      if (fp->cinuse()) {
        ...
        if (fragSize == PAD_REQUEST(sizeof(NAHeap))) {
          ...
          if (testHeap->type_ == NAMemory::SYSTEM_MEMORY || testHeap->type_ == NAMemory::IPC_MEMORY) {
            ipcHeap = testHeap;
          }
        }
        ...
        // ComSpace object.
          ...
          if (testSpace->ComSpace::parent_ == thisMemory_) {
            ExSpaceStats *newDerivedStats = new ExSpaceStats(this, testSpace, level_ + 1);
  ...
  while (derivedMem != NULL) {
    ExMemStats *newDerivedStats = new ExHeapStats(this, derivedMem, level_ + 1);
...
void ExHeapStats::DisplayLongMemStats(ostream &os, char *ind, size_t minSize) {
  ...
  if (thisMemory_->parent_) os << ind << "Parent address: " << thisMemory_->parent_ << endl;
  ...
  switch (thisMemory_->type_) {
    ...
    case NAMemory::IPC_MEMORY: {
      ...
      while (block != NULL) {
        os << ind
           << /*"  Seg Id " << block->segmentId_
    << */ "  NABlock@" << block << "  (" << (block->size_ >> 10) << " Kbytes)" << endl;
        block = block->next_;
      }
    } break;
    default:
  ...
  os << ind << "Maximum size of allocated fragments: " << maxAllocSize_ << endl;
  os << ind << "Derived memory level: " << level_ << endl;
  os << ind << "Total size of derived memory blocks: " << derivedMemSum_ << endl;
  ...
  for (size_t i = 0; i < numTotalBins; i++) {
    ...
    if (sizeBinCnt_[i] != 0) {
      os << ind << setw(9) << prevSize << " - " << setw(9) << setiosflags(ios::left) << binSize
         << resetiosflags(ios::left) << setw(11) << sizeBinCnt_[i] << setw(17) << sizeBinSum_[i] << endl;
    }
    prevSize = binSize + 1;
  }
}
...
void ExSpaceStats::DisplayLongMemStats(ostream &os, char *ind, size_t minSize) {
  ...
  switch (space->ComSpace::type_) {
    ...
    case ComSpace::SINGLE_BLOCK_SPACE:
      ...
      break;
    default:
      os << "Unknown space type: " << space->ComSpace::type_ << endl;
      break;
  }
  os << ind << "NAMemory address: " << thisMemory_ << endl;
  os << ind << "Derived memory level: " << level_ << endl;
