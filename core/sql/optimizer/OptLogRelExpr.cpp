                      ...
                      CostScalar initialRowcount)
{
  ...
  NABoolean computeRowCountForIndividualPred = 
       ( (CmpCommon::getDefault(COMP_BOOL_206) == DF_ON) &&
         ...
         &&
        CURRSTMT_OPTDEFAULTS->rangeOptimizedScan()
         &&
        isInnerJoin()
       );
  ...
  if ( computeRowCountForIndividualPred ) 
     joinStatDescListOriginal.makeDeepCopy(joinStatDescList) ;
  ...
  if ( computeRowCountForIndividualPred )
  {
     ...
     for ( preds.getFirst(d); preds.next(d); preds.advance(d) )  {
        ...
        CostScalar rowcount =
          joinStatDescListCopy.estimateCardinality(initialRowcount,  /*in*/
                                          pred,              /*in*/
                                          outerReferences,   /*in*/
                                          this,
                                          NULL,   
        ...
        for ( int i = 0; i < joinStatDescListCopy.entries(); i++ )
           joinStatDescListCopy.removeDeepCopyAt(0);

        vid2rowcountMap_.insert(d, rowcount);
     }

     for ( int i = 0; i < joinStatDescListOriginal.entries(); i++ )
       joinStatDescListOriginal.removeDeepCopyAt(0);
  }
...
Join::synthConstraints(NormWA * normWAPtr)
{
  ...
  if (leftHasUniqueMatches_)
  ...
  else if(rightHasUniqueMatches_ == FALSE)
    {
      ...
      Int32 maxNumConstraints = 
          (ActiveSchemaDB()->getDefaults()).
                getAsLong(MAX_NUM_UNIQUE_CONSTRAINTS_FOR_JOIN);
	   ...
	   leftConstraints.advance(lx))
	{
         ...
         if (maxNumConstraints >= 0 && 
             numConstraintsTried >= maxNumConstraints)
             break;
  ...
  if (isALabeledUnion())
    {
      ...
      for (ValueId lc=leftConstraints.init();
           leftConstraints.next(lc); 
           leftConstraints.advance(lc))
        {
	  ...
	  if (loptype == ITM_UNIQUE_OPT_CONSTRAINT)
            {
              const ValueIdSet &lUniqueCols =
                static_cast<UniqueOptConstraint *>(lx)->uniqueCols();
              ...
              if (rightGA.isUnique(rUniqueCols))
                {
                  ...
                  for (CollIndex i=0; i<cmt.entries(); i++)
                    if (static_cast<ValueIdUnion *>(
                             cmt[i].getItemExpr())->isALabel())
                      {
                        ...
                        getGroupAttr()->addConstraint(
                             new(CmpCommon::statementHeap())
                             UniqueOptConstraint(uUniqueCols));
                        uUniqueCols -= cmt[i];
                      }
                }
            }
  ...
  if(trueScalAgg)
	...
	largeTableNeedsStats = groupStatDescList[0]->getColStats()->isUpStatsNeeded();
			...
			workGroup.advance(workGroupItem))
	  {
	    ...
	    if ((CmpCommon::getDefault(COMP_BOOL_148) == DF_ON) && 
		(colExpr->getOperatorType() == ITM_CASE) &&
		(colExpr->getArity() == 1) &&
		(colExpr->child(0)->getOperatorType() == ITM_IF_THEN_ELSE) &&
		(colExpr->child(0)->getArity() == 3))
	    {
	      ...
	      colExpr->child(0)->child(1)->
		findAll( ITM_BASECOLUMN, columns, TRUE, TRUE );
	      colExpr->child(0)->child(2)->
		findAll( ITM_BASECOLUMN, columns, TRUE, TRUE );
	    }
	    else
	      colExpr->findAll( ITM_BASECOLUMN, columns, TRUE, TRUE );
