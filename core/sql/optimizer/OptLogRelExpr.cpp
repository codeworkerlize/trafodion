...
RelExpr *RelExpr::optimizeNode() {
  ...
  // optimize
  // ---------------------------------------------------------------------
  NABoolean useNewOptDriver = (CmpCommon::getDefault(NEW_OPT_DRIVER) == DF_ON);
  ...
  if (result != NULL) result->releaseBindingTree();
...
void RelExpr::cleanupAfterCompilation() {
...
// -----------------------------------------------------------------------
EstLogPropSharedPtr RelExpr::synthEstLogPropForUnaryLeafOp(const EstLogPropSharedPtr &inputEstLogProp,
  ...
  // rowcount.
  if (childStats.entries() > 0) newRowCount = childStats[0]->getColStats()->getRowcount();
  ...
  CostScalar maxRows = (childMaxCardEst ? (*childMaxCardEst) : initialCardinality);
...
#undef MONITOR_SAMEROWCOUNT
    ...
    CostScalar innerScale =
        (indexNestedJoin || inputEstLogProp->getInputForSemiTSJ() != EstLogProp::NOT_SEMI_TSJ ? 1 : newRowCount);
    ...
    else if (inputEstLogProp->getResultCardinality() >
             csOne)  // no colStats to prepend, still the rowcount needs to be adjusted
    {
    ...
    if (columnStats.entries() > 0) newRowCount = columnStats[0]->getColStats()->getRowcount();
    ...
    if (indexNestedJoin) {
      for (CollIndex i = outerRefCount; i < columnStats.entries(); i++) {
        ...
        if (oldCount != newRowCount) columnStatDesc->synchronizeStats(oldCount, newRowCount);
      }
  ...
  if (opType == REL_SCAN) {
    ...
    (void)columnStatsForMaxSel.estimateCardinality(newRowCount, getSelectionPred(), outerReferences, NULL, selHint,
                                                   cardHint, outerRefCount, myEstProps->unresolvedPreds(), mergeMethod,
                                                   opType, &maxSelectivity);

    newRowCount = columnStats.estimateCardinality(newRowCount,                   /*in*/
                                                  getSelectionPred(),            /*in*/
                                                  outerReferences,               /*in*/
                                                  NULL, selHint,                 /*in*/
                                                  cardHint,                      /*in*/
  ...
  if (!sampleUsed && !cardHint && !selHint) {
    ...
    if (!rollingColumnPresent) newRowCount = MINOF(newRowCount, inputCard * initialCardinality);
  }
  ...
  if ((statOper == REL_UNARY_INSERT) || (statOper == REL_LEAF_INSERT) || (statOper == REL_UNARY_DELETE) ||
      (statOper == REL_LEAF_DELETE) || (statOper == REL_UNARY_UPDATE) || (statOper == REL_LEAF_UPDATE)) {
    GenericUpdate *updateExpr = (GenericUpdate *)this;
...
void RelExpr::synthEstLogProp(const EstLogPropSharedPtr &inputEstLogProp) {
...
void RelExpr::synthLogProp(NormWA *normWAPtr) {
  ...
  for (int i = 0; i < numChildren; i++) {
    ...
    if (child(i).getPtr() != NULL) {
      ...
      if (child(i).getGroupAttr()->isStream()) {
        GA->setStream(TRUE);
        if (child(i).getGroupAttr()->isSkipInitialScan()) getGroupAttr()->setSkipInitialScan(TRUE);
      }
      if (child(i).getGroupAttr()->isEmbeddedUpdateOrDelete())
        GA->setEmbeddedIUD(child(i).getGroupAttr()->getEmbeddedIUD());
      // QSTUFF ^^

      if (child(i).getGroupAttr()->getHasNonDeterministicUDRs()) getGroupAttr()->setHasNonDeterministicUDRs(TRUE);

      if (child(i).getGroupAttr()->isSeabaseMDTable()) getGroupAttr()->setSeabaseMDTable(TRUE);
    }
  }
  ...
  if (!getUniqueColumns().isEmpty()) {
    GA->addConstraint(new (CmpCommon::statementHeap()) UniqueOptConstraint(getUniqueColumns()));
  }

  if (getCardConstraint()) {
    GA->addConstraint(new (CmpCommon::statementHeap())
                          CardConstraint(getCardConstraint()->getLowerBound(), getCardConstraint()->getUpperBound()));
  }
  //--MV
...
void RelExpr::finishSynthEstLogProp() {
  ...
  for (int i = 0; i < numChildren; i++) {
    ...
    if (child(i).getPtr() != NULL) {
      ...
      if (thisChildsRowCount < minRowCount) minRowCount = thisChildsRowCount;
    }
  }
...
RelExpr *RelExpr::checkAndReorderTree() {
  // ---------------------------------------------------------------------
  ...
  NABoolean areTriggersPresent = getOperatorType() == REL_ROOT && ((RelRoot *)this)->getTriggersList() != NULL;
  NABoolean containsOnStatementMV = getOperatorType() == REL_ROOT && ((RelRoot *)this)->containsOnStatementMV();

  NABoolean doReorderJoin = ((ActiveSchemaDB()->getDefaults()).getAsULong(COMP_INT_90) != 0) ||
                            getGroupAttr()->reorderNeeded() || areTriggersPresent || containsOnStatementMV;

  if (!(QueryAnalysis::Instance()->joinOrderByUser())) {
    if (doReorderJoin && ((CURRSTMT_CQSWA == NULL) || CURRSTMT_CQSWA && !CURRSTMT_CQSWA->reArrangementSuccessful_)) {
      this->reorderTree(treeModified, doReorderJoin);
      this->clearAllEstLogProp();

      if (treeModified) synthLogProp();
    }
  } else {
    if (getGroupAttr()->reorderNeeded()) *CmpCommon::diags() << DgSqlCode(-4174);
  }
  // QSTUFF
...
RelExpr *RelExpr::reorderTree(NABoolean &treeModified, NABoolean doReorderJoin) {
  ...
  if (treeModified) getGroupAttr()->clearLogProperties();
...
RelExpr *RelExpr::fixupTriggers(NABoolean &treeModified) {
  ...
  // if any of my children were reordered, then we need to
  // clear my logical properties for resynthesis.
  if (myTreeModified) getGroupAttr()->clearLogProperties();
...
RelExpr *Union::fixupTriggers(NABoolean &treeModified) {
  RelExpr *retNode = this;
  NABoolean myTreeModified = FALSE;

#ifndef NDEBUG
  if (getenv("TRIGGER_TRANSFORMATION_OFF") != NULL) return (RelExpr::fixupTriggers(treeModified));
#endif
  ...
  if (InlineInfo.isDrivingBeforeTriggers() ||
    ...
    OptTriggersBackbone *backbone = new (CmpCommon::statementHeap()) OptTriggersBackbone(this);
    backbone->init();
...
void CutOp::synthLogProp(NormWA *normWAPtr) {}

// ----------------------------------------------------------------------
// Helper method for left and full outer joins
// The method simulates the behavior of left outer joins
// ----------------------------------------------------------------------
void Join::instantiateLeftHistsWithNULLs(ValueIdSet EqLocalPreds,                 /*in*/
                                         ...
                                         CollIndex &rootStatIndex /*out*/, NAList<CollIndex> &statsToMerge /*out*/) {
  ...
  foundFlag = joinStatDescList.locateHistogramToNULLAugment(EqLocalPreds, statsToMerge, rootStatIndex, outerRefCount);
  ...
  if (foundFlag) {
    ...
    oJoinResultRows = joinStatDescList.computeLeftOuterJoinRC(foundFlag, leftColStatsList, rootStatIndex);
  }
  ...
  if (!foundFlag) {
    ...
    if (leftColStatsList.entries() > 0) lrc = leftColStatsList[0]->getColStats()->getRowcount();
  ...
  joinStatDescList.synchronizeHistsWithOJRC(statsToMerge, 0, outerRefCount, rootStatIndex, leftColStatsList,
                                            oJoinResultRows, baseRows);
  ...
  joinStatDescList.nullInstantiateHists(outerRefCount, joinStatDescList.entries(), oJoinResultRows, nulledVIds);
...
// ----------------------------------------------------------------------
void Join::instantiateRightHistsWithNULLs(CollIndex outerRefCount,                  /*in*/
                                          ColStatDescList &innerJoinCopy,           /*in/out*/
                                          ColStatDescList &joinStatDescList,        /*in/out*/
                                          const ColStatDescList &rightColStatsList, /*in*/
                                          CostScalar &oJoinResultRows /*in, out*/, CostScalar initialRowcount /*in*/,
                                          CollIndex rootStatIndex /*in*/, NAList<CollIndex> statsToMerge /*out*/) {
  ...
  if (statsToMerge.entries() > 0) {
    ...
    rightJoinCard = innerJoinCopy.computeFullOuterJoinRC(foundFlag, rightColStatsList, rootStatIndex);
  ...
  if (!foundFlag) {
    ...
    if (rightColStatsList.entries() > 0) rrc = rightColStatsList[0]->getColStats()->getRowcount();
  ...
  joinStatDescList.synchronizeHistsWithOJRC(statsToMerge, outerRefCount, joinStatDescList.entries(), rootStatIndex,
                                            rightColStatsList, oJoinResultRows, baseRows);
...
// -----------------------------------------------------------------------
CostScalar Join::commonSynthEst(const EstLogPropSharedPtr &myEstProps, const EstLogPropSharedPtr &intermedEstProps,
  ...
  if (isLeftJoin() || isFullOuterJoin())
    getJoinPred().categorizePredicates(outerReferences, EqLocalPreds, OtherLocalPreds, EqNonLocalPreds,
  ...
  NABoolean computeRowCountForIndividualPred =
      ((CmpCommon::getDefault(COMP_BOOL_206) == DF_ON) &&
       CmpCommon::context()->getCIClass() != CmpContextInfo::CMPCONTEXT_TYPE_META &&
       CURRSTMT_OPTDEFAULTS->rangeOptimizedScan() && isInnerJoin());

  // to be used by the logic at the end of the method to compute
  // row count for each individual pred in getSelectPred().
  ColStatDescList joinStatDescListOriginal;

  if (computeRowCountForIndividualPred) joinStatDescListOriginal.makeDeepCopy(joinStatDescList);
  ...
  newRowcount =
      joinStatDescList.estimateCardinality(newRowcount,     /*in*/
                                           getJoinPred(),   /*in*/
                                           outerReferences, /*in*/
                                           this, NULL,      /*for selectivityHint, which can only be given for Scan*/
  ...
  if (isFullOuterJoin()) innerJoinCopy.makeDeepCopy(joinStatDescList);
  //---------------------------------------------------------------------
  ...
  if (mergeMethod == OUTER_JOIN_MERGE) {
    instantiateLeftHistsWithNULLs(EqLocalPreds,     /*in*/
                                  ...
                                  joinStatDescList, /*in*/
                                  leftColStatsList, /*in*/
                                  newRowcount /*in, out*/, initialRowcount /*in*/, rootStatIndex, statsToMerge);
  ...
  if (mergeMethod == OUTER_JOIN_MERGE && CmpCommon::getDefault(HIST_FREQ_VALS_NULL_FIX) == DF_ON)
  ...
  // as an inner join.
  // ---------------------------------------------------------------------
  if (mergeMethod == OUTER_JOIN_MERGE) mergeMethod = INNER_JOIN_MERGE;
  ...
  newRowcount =
      joinStatDescList.estimateCardinality(newRowcount,        /*in*/
                                           ...
                                           outerRefCount, /*in/out*/
                                           myEstProps->unresolvedPreds(), /*in/out*/
                                           mergeMethod, REL_JOIN);        /*in*/

  if (computeRowCountForIndividualPred) {
    ValueIdSet preds(getSelectionPred());
    ValueId d;
    for (preds.getFirst(d); preds.next(d); preds.advance(d)) {
      ColStatDescList joinStatDescListCopy;
      joinStatDescListCopy.makeDeepCopy(joinStatDescListOriginal);

      ValueIdSet pred(d);

      CostScalar rowcount = joinStatDescListCopy.estimateCardinality(initialRowcount,                 /*in*/
                                                                     pred,                            /*in*/
                                                                     outerReferences,                 /*in*/
                                                                     this, NULL, NULL, outerRefCount, /*in/out*/
                                                                     myEstProps->unresolvedPreds(),   /*in/out*/
                                                                     mergeMethod, REL_JOIN);          /*in*/

      for (int i = 0; i < joinStatDescListCopy.entries(); i++) joinStatDescListCopy.removeDeepCopyAt(0);

      vid2rowcountMap_.insert(d, rowcount);
    }

    for (int i = 0; i < joinStatDescListOriginal.entries(); i++) joinStatDescListOriginal.removeDeepCopyAt(0);
  }
...
// -----------------------------------------------------------------------
void Join::synthEstLogPropForTSJ(const EstLogPropSharedPtr &inputEstLogProp) {
  ...
  if (isSemiJoin() || inputEstLogProp->getInputForSemiTSJ() == EstLogProp::SEMI_TSJ)
    copyLeftEstProps->setInputForSemiTSJ(EstLogProp::SEMI_TSJ);
  else if (isAntiSemiJoin() || inputEstLogProp->getInputForSemiTSJ() == EstLogProp::ANTI_SEMI_TSJ)
  ...
  newRowcount = this->commonSynthEst(myEstProps, intermedEstProps, outerRefCount, leftColStatsList, rightColStatsList,
                                     joinStatDescList, newRowcount);
  ...
  newRowcount = doCardSanityChecks(copyInputEstProp, joinStatDescList, newRowcount);
  CostScalar maxCardEst = estimateMaxCardinality(copyInputEstProp, joinStatDescList);

  if (oldCount != newRowcount) joinStatDescList.synchronizeStats(newRowcount, joinStatDescList.entries());
  ...
  getGroupAttr()->addInputOutputLogProp(inputEstLogProp, myEstProps, intermedEstProps);
...
// -----------------------------------------------------------------------
void Join::synthEstLogProp(const EstLogPropSharedPtr &inputEstLogProp) {
  if (isTSJ()) {
  ...
  for (i = 0; i < outerColStatsList.entries(); i++) {
    ...
    for (j = 0; j < leftColStatsList.entries(); j++)
      if (leftColStatsList[j]->getVEGColumn().getItemExpr()->containsTheGivenValue(
              outerColStatsList[i]->getVEGColumn())) {
        leftCol = j;
        foundMatch = TRUE;
        break;
      }
    ...
    if (foundMatch) {
      ...
      for (j = 0; j < rightColStatsList.entries(); j++)
        if (rightColStatsList[j]->getVEGColumn().getItemExpr()->containsTheGivenValue(
                outerColStatsList[i]->getVEGColumn())) {
          rightCol = j;
          foundMatch = TRUE;
          break;
    ...
    if (foundMatch) {
      ...
      joinStatDescList.insertDeepCopyAt(outerMatchCount, leftColStatsList[leftCol],
                                        rightRowCount);  // scale
      outerMatchCount++;

      joinStatDescList.insertDeepCopyAt(outerMatchCount, rightColStatsList[rightCol],
                                        leftRowCount);  // scale
      outerMatchCount++;
    }
  }  // for i
  ...
  if (outerMatchCount > 0)
    ...
    if (rightRowCount.getValue() < 1.000001) rightRowCount = CostScalar(1.0);
  ...
  if (isASemiJoin) newRowCount = leftRowCount;
  ...
  for (i = 0; i < leftColStatsList.entries(); i++) {
    ...
    for (j = 0; j < outerColStatsList.entries(); j++)
      if (leftColStatsList[i]->getVEGColumn().getItemExpr()->containsTheGivenValue(
              outerColStatsList[j]->getVEGColumn())) {
        alreadyAdded = TRUE;
        break;
      }
  ...
  for (i = 0; i < rightColStatsList.entries(); i++) {
    NABoolean alreadyAdded = FALSE;
    for (j = 0; j < outerColStatsList.entries(); j++)
      if (rightColStatsList[i]->getVEGColumn().getItemExpr()->containsTheGivenValue(
              outerColStatsList[j]->getVEGColumn())) {
        alreadyAdded = TRUE;
        break;
      }
  ...
  newRowCount = this->commonSynthEst(myEstProps, intermedEstProps, outerRefCount, leftCopy, rightColStatsList,
                                     joinStatDescList, newRowCount);
  ...
  if (inputEstLogProp->getInputForSemiTSJ() != EstLogProp::NOT_SEMI_TSJ) {
    ...
    newRowCount = this->commonSynthEst(myEstProps, newIntermedEstProps, outerRefCount, leftCopy, rightColStatsList,
                                       newStatsList, newRowCount);
    ...
    newRowCount = newStatsList.estimateCardinality(newRowCount,        /*in*/
                                                   getSelectionPred(), /*in*/
                                                   outerReferences,    /*in*/
                                                   this, NULL, /*for selectivityHint, which can only be given for Scan*/
                                                   NULL, /* for Cardinality hint, which can only be given for Scan */
    ...
    for (i = 0; i < leftCopy.entries(); i++) leftCopy.removeDeepCopyAt(0);
  }
  ...
  if (compositeUnnestJoin()) {
    // Please see comment in CompositeUnnest::synthEstLogProp()
  ...
  myEstProps->pickOutputs(joinStatDescList, inputEstLogProp, getGroupAttr()->getCharacteristicOutputs(),
                          getSelectionPred());
...
CostScalar Join::doCardSanityChecks(const EstLogPropSharedPtr &inLP, ColStatDescList &joinStatDescList,
                                    CostScalar newRowCount) {
  if (joinStatDescList.selectivityHintApplied()) return newRowCount;
  ...
  if ((joinPredicates.entries() == 1) && (joinedCols.entries() > 0)) joinOnSingleCol = TRUE;
  ...
  if (joinOnSingleCol && (skipMinCardTest == FALSE)) {
    ...
    if (((upliftCardCond == 1) || (upliftCardCond == 2)) && joinOnSingleCol && !SemiOrAntiSemiJoin) {
      ...
      CostScalar leftUec = leftEstLogProp->getColStats().getUecOfJoiningCols(joinedCols);
      CostScalar rightUec = rightEstLogProp->getColStats().getUecOfJoiningCols(joinedCols);
      ...
      if (((upliftCardCond == 1) && (minCard < currMin)) || (upliftCardCond > 1)) {
        minCard = currMin;
        skipMinCardTest = TRUE;
      }
  ...
  if ((skipMinCardTest == FALSE) && (joinStatDescList.isCapForLowBound())) {
    // multi-column join
    const JBBSubset *localJBBView = NULL;

    if ((getGroupAnalysis()) && (localJBBView = getGroupAnalysis()->getLocalJBBView()))
      ...
      if (minEstRowCount <= csZero) minEstRowCount = computeMinEstRCForGroup();
      // we estimate cardinality for GroupAttr using empty_input_logprop
      // but at this point we might have some input est logprop. So take
      // inputEstLogProp into account while determining low bound
...
NABoolean Join::matchRIConstraint(GroupAttributes &leftGA, GroupAttributes &rightGA, NormWA *normWAPtr) {
  ...
  if ((leftGA.hasRefOptConstraint() || leftGA.hasCompRefOptConstraint()) &&
    ...
    for (ValueId leftId = leftConstraints.init(); leftConstraints.next(leftId) && NOT riMatched;
      ...
      if (leftId.getItemExpr()->getOperatorType() == ITM_REF_OPT_CONSTRAINT) {
        if (rightGA.hasCompRefOptConstraint()) {
          for (ValueId ukId = rightConstraints.init(); rightConstraints.next(ukId) && NOT riMatched;
        ...
        if (rightGA.hasRefOptConstraint()) {
          for (ValueId fkId = rightConstraints.init(); rightConstraints.next(fkId) && NOT riMatched;
...
NABoolean Join::hasMatchingRIConstraint(ValueId fkConstraintId, ValueId ukConstraintId, NormWA *normWAPtr) {
  RefOptConstraint *riConstraint = (RefOptConstraint *)fkConstraintId.getItemExpr();
  ComplementaryRefOptConstraint *compRIConstraint = (ComplementaryRefOptConstraint *)ukConstraintId.getItemExpr();

  ValueIdSet matchingPreds;
  NABoolean riMatched = FALSE;

  if (riConstraint->uniqueConstraintName() == compRIConstraint->constraintName()) {
    if (hasRIMatchingPredicates(riConstraint->foreignKeyCols(), compRIConstraint->uniqueKeyCols(),
                                compRIConstraint->getTableDesc(), matchingPreds)) {
      ...
      RelExpr *referencedTable = (RelExpr *)compRIConstraint->getReferencedTable();
      uniqKeyNonPredOutputs = referencedTable->getGroupAttr()->getCharacteristicOutputs();
      ...
      if (NOT(myNonPredOutputs.referencesOneValueFromTheSet(uniqKeyNonPredOutputs))) {
        ...
        if (normWAPtr->getExtraHubVertex() && CmpCommon::getDefault(MVQR_USE_EXTRA_HUB_TABLES) == DF_ON &&
            CmpCommon::getDefault(MVQR_USE_RI_FOR_EXTRA_HUB_TABLES) == DF_ON) {
          normWAPtr->getExtraHubVertex()->getGroupAttr()->getNonEssentialCharacteristicOutputs(globalNonEssOutputs);

          if (uniqKeyNonPredOutputs.referencesAllValuesFromMe(globalNonEssOutputs)) {
            referencedTable->setIsExtraHub(TRUE);
            setIsExtraHub(TRUE);
            normWAPtr->setCheckForExtraHubTables(TRUE);

            addExtraHubNonEssentialOutputs(referencedTable->getGroupAttr()->getEssentialCharacteristicOutputs());
          }
        }
...
CostScalar Join::estimateMaxCardinality(const EstLogPropSharedPtr &inLP, ColStatDescList &joinStatDescList) {
  EstLogPropSharedPtr leftEstLogProp = child(0).outputLogProp((*GLOBAL_EMPTY_INPUT_LOGPROP));
  EstLogPropSharedPtr rightEstLogProp = child(1).outputLogProp((*GLOBAL_EMPTY_INPUT_LOGPROP));
  ...
  for (ValueId col = joinedCols.init(); joinedCols.next(col); joinedCols.advance(col)) {
    ...
    leftAvgFreq = MINOF(leftAvgFreq, CostScalar(leftMax / leftEstLogProp->colStats().getUEC(col)).minCsOne());
    rightAvgFreq = MINOF(rightAvgFreq, CostScalar(rightMax / rightEstLogProp->colStats().getUEC(col)).minCsOne());
    // avgFreq(T.z) may sometime exceed maxFreq(T.z)
  }

  // maxCardEst1 is maxCard based on maxFreq
  CostScalar maxCardEst1 = inCard * MINOF(leftMax * rightFreqMin, rightMax * leftFreqMin);
...
//---------------------------------------------------------------------------
void Join::synthConstraints(NormWA *normWAPtr) {
  // QSTUFF
  ...
  if (isSemiJoin() OR isAntiSemiJoin() OR isIndexJoin_) leftHasUniqueMatches_ = TRUE;

  // QSTUFF
  // this is a cursor update, delete or insert and therefore
  // the constraint is guaranteed to be true
  if (isCursorUpdate()) leftHasUniqueMatches_ = TRUE;
  // QSTUFF
  ...
  if (maxLeft == 1 || isIndexJoin_) rightHasUniqueMatches_ = TRUE;

  if (maxRight == 1) leftHasUniqueMatches_ = TRUE;
  ...
  if (leftHasUniqueMatches_)
    ...
    maxRows = maxRight;
  else if (maxLeft < INFINITE_CARDINALITY AND maxRight < INFINITE_CARDINALITY)
  ...
  if (minRows > 0 OR maxRows < INFINITE_CARDINALITY) {
    myGA.addConstraint(new (CmpCommon::statementHeap()) CardConstraint(minRows, maxRows));
  }
  ...
  if (CmpCommon::getDefault(OPTIMIZER_SYNTH_FUNC_DEPENDENCIES) != DF_OFF) {
    ...
    FuncDependencyConstraint::synthFunctionalDependenciesFromChild(myGA, child(0), NOT leftHasUniqueMatches_);
    FuncDependencyConstraint::synthFunctionalDependenciesFromChild(myGA, child(1), NOT rightHasUniqueMatches_);
  }
  ...
  if (rightHasUniqueMatches_) {
    ...
    for (ValueId rx = rightConstraints.init(); rightConstraints.next(rx); rightConstraints.advance(rx)) {
  ...
  if (leftHasUniqueMatches_) {
    ...
    for (ValueId lx = leftConstraints.init(); leftConstraints.next(lx); leftConstraints.advance(lx)) {
    ...
    // walk the constraints of the left child table
    for (ValueId lx = leftConstraints.init(); leftConstraints.next(lx); leftConstraints.advance(lx)) {
      if (maxNumConstraints >= 0 && numConstraintsTried >= maxNumConstraints) break;
        ...
        for (ValueId rx = rightConstraints.init(); rightConstraints.next(rx); rightConstraints.advance(rx)) {
          ...
          //        ((UniqueOptConstraint *)rc)->uniqueCols()))
          {
            ...
            UniqueOptConstraint *newConstr =
                new (CmpCommon::statementHeap()) UniqueOptConstraint(((UniqueOptConstraint *)lc)->uniqueCols());

            // ...and add the unique columns from the right table
            newConstr->uniqueCols() += ((UniqueOptConstraint *)rc)->uniqueCols();
  ...
  if (leftGA.isEmbeddedUpdateOrDelete() && !leftHasUniqueMatches_) {
    if (child(1)->getRETDesc() &&
        getRETDesc())  // soln:10_040806_8608, in the case of JoinLeftShiftRule the RETDesc_ may be NULL
    {
      child(1)->getRETDesc()->getTableList(xtnmList1, &fmtdList1);
      getRETDesc()->getTableList(xtnmList2, &fmtdList2);
      *CmpCommon::diags() << DgSqlCode(-4150) << DgString0(fmtdList1)
                          << (leftGA.isEmbeddedUpdate() ? DgString1("update") : DgString1("delete"))
                          << DgString2(fmtdList2);
    }
  }

  if (rightGA.isEmbeddedUpdateOrDelete() && !rightHasUniqueMatches_) {
    if (child(0)->getRETDesc() &&
        getRETDesc())  // soln:10_040806_8608, in the case of JoinLeftShiftRule the RETDesc_ may be NULL
    {
      child(0)->getRETDesc()->getTableList(xtnmList1, &fmtdList1);
      getRETDesc()->getTableList(xtnmList2, &fmtdList2);
      *CmpCommon::diags() << DgSqlCode(-4150) << DgString0(fmtdList1)
                          << (rightGA.isEmbeddedUpdate() ? DgString1("update") : DgString1("delete"))
                          << DgString2(fmtdList2);
    }
  }
...
void Join::synthLogProp(NormWA *normWAPtr) {
  // Check to see whether this GA has already been associated
  // with a logExpr for synthesis.  If so, no need to resynthesize
  ...
  if (NOT getGroupAttr()->existsLogExprForSynthesis()) {
    ...
    getGroupAttr()->setNumJoinedTables(child(0).getGroupAttr()->getNumJoinedTables() + 1);
  ...
  if (NOT getGroupAttr()->existsLogExprForSynthesis()) getGroupAttr()->setLogExprForSynthesis(this);
  ...
  if (isLeftJoin() && getSelectionPred().isEmpty() && (CmpCommon::getDefault(ELIMINATE_REDUNDANT_JOINS) != DF_OFF) &&
    ...
    if ((NOT equiJoinCols1.isEmpty()) && child(1)->getGroupAttr()->isUnique(equiJoinCols1)) {
      ...
      child(1)->getGroupAttr()->coverTest(myOutputs, emptySet, coveredExpr, emptySet1, &coveredSubExpr);
      if (coveredExpr.isEmpty() && coveredSubExpr.isEmpty()) {
        NABoolean usesInstNullInOutput = myOutputs.referencesOneValueFromTheSet(nullInstantiatedOutput());
        ...
        if (normWAPtr->getExtraHubVertex() && CmpCommon::getDefault(MVQR_USE_EXTRA_HUB_TABLES) == DF_ON) {
          ...
          if ((!nullOutputs.isEmpty()) && nullOutputs.referencesAllValuesFromMe(globalNonEssOutputs)) {
            ...
            while (descendant->getArity() == 1) descendant = descendant->child(0)->castToRelExpr();
...
// The reorderTree() is used also for pre-optimizer pass
// -----------------------------------------------------------------------
RelExpr *Join::reorderTree(NABoolean &treeModified, NABoolean doReorderJoin) {
  ...
  if (!doReorderJoin) return (this);

  RelExprList *OBRowcountList = new (CmpCommon::statementHeap()) RelExprList(CmpCommon::statementHeap());
  RelExprList *orderedList = new (CmpCommon::statementHeap()) RelExprList(CmpCommon::statementHeap());
  RelExprList *joinBackBoneList = new (CmpCommon::statementHeap()) RelExprList(CmpCommon::statementHeap());
  ...
  if (*OBRowcountList != *orderedList) treeModified = TRUE;
  ...
  if (NOT treeModified) {
    ...
    for (int i = 0; i < (int)joinBackBoneList->entries(); i++) {
      ...
      if (treeModified) (*joinBackBoneList)[i]->getGroupAttr()->clearLogProperties();
    }
  ...
  if ((expr->child(0)->getOperatorType() == REL_JOIN)
    ...
    while (moreJoinNodesWhosePredsWerePulledUp) {
      ...
      ValueIdSet rcOutputs = exprRChild->getGroupAttr()->getCharacteristicOutputs();

      expr->pushdownCoveredExpr(expr->getGroupAttr()->getCharacteristicOutputs(),
                                expr->getGroupAttr()->getCharacteristicInputs(), expr->selectionPred());
      ...
      expr->selectionPred().addSet(updatedRChildPred);
    ...
    // ---------------------------------------------------------------------
    VEGEqPreds.removeUnCoveredExprs(expr->child(0).getGroupAttr()->getCharacteristicOutputs());
    ...
    // ---------------------------------------------------------------------
    VEGEqPreds.removeUnCoveredExprs(expr->child(1).getGroupAttr()->getCharacteristicOutputs());
...
const MCSkewedValueList *Join::getMCSkewedValueListForJoinPreds(ValueIdList &colGroup) {
  ...
  if ((lhsCols.entries() == 0) || (rhsCols.entries() == 0) || (lhsCols.entries() != rhsCols.entries())) return NULL;

  // Extract MC Skew values for column group that are part of join predicates
  ValueIdList lhsColGrp, rhsColGrp;
  MCSkewedValueList *leftMCSkewedValueList =
      (MCSkewedValueList *)leftColStatDescList.getMCSkewedValueListForCols(lhsCols, lhsColGrp);
  MCSkewedValueList *rightMCSkewedValueList =
      (MCSkewedValueList *)rightColStatDescList.getMCSkewedValueListForCols(rhsCols, rhsColGrp);

  CostScalar avgRowcountForNonSkewValuesOnLeftSide =
      leftColStatDescList.getAvgRowcountForNonSkewedMCValues(lhsCols, leftMCSkewedValueList);
  CostScalar avgRowcountForNonSkewValuesOnRightSide =
      rightColStatDescList.getAvgRowcountForNonSkewedMCValues(rhsCols, rightMCSkewedValueList);

  MCSkewedValueList *result = new (STMTHEAP) MCSkewedValueList(STMTHEAP);
  result->mergeMCSkewedValueList(leftMCSkewedValueList, rightMCSkewedValueList, avgRowcountForNonSkewValuesOnLeftSide,
                                 avgRowcountForNonSkewValuesOnRightSide, getMergeTypeToBeUsedForSynthLogProperties());
  colGroup = lhsColGrp;
...
void Union::synthEstLogProp(const EstLogPropSharedPtr &inputEstLogProp) {
  if (getGroupAttr()->isPropSynthesized(inputEstLogProp)) return;
  ...
  CostScalar rowCount = leftEstProps->getResultCardinality() + rightEstProps->getResultCardinality();

  // The unresolved predicates of a union are those that are unresolved
  // by both children.
  myEstProps->unresolvedPreds() += leftEstProps->getUnresolvedPreds();
  myEstProps->unresolvedPreds().intersectSet(rightEstProps->getUnresolvedPreds());
  ...
  for (i = 0; i < outerColStatsList.entries(); i++) {
    ...
    const ItemExpr *outerItem = outerStatDesc->getVEGColumn().getItemExpr();
    ...
    for (j = 0; j < leftColStatsList.entries() && !foundLeftMatch; j++) {
      ...
      const ItemExpr *leftItem = lStatDesc->getVEGColumn().getItemExpr();
    ...
    for (j = 0; j < rightColStatsList.entries() && !foundRightMatch; j++) {
      ...
      const ItemExpr *rightItem = rStatDesc->getVEGColumn().getItemExpr();
    ...
    if (foundLeftMatch && foundRightMatch) {
      ...
      unionStatDescList[entry]->mergeColStatDesc(rStatDesc, UNION_MERGE, FALSE, REL_UNION);
    }  // if foundLeftMatch && foundRightMatch
  }    // for i
  ...
  for (i = 0; i < unionMapTable.entries(); i++) {
    ...
    for (j = 0; j < leftColStatsList.entries() && !foundLeftMatch; j++) {
      ColStatDescSharedPtr lStatDesc = leftColStatsList[j];
      const BaseColumn *leftItem = lStatDesc->getVEGColumn().castToBaseColumn();
    ...
    for (j = 0; j < rightColStatsList.entries() && !foundRightMatch; j++) {
      ColStatDescSharedPtr rStatDesc = rightColStatsList[j];
      const BaseColumn *rightItem = rStatDesc->getVEGColumn().castToBaseColumn();
  ...
  myEstProps->setMaxCardEst(MAXOF(leftEstProps->getMaxCardEst() + rightEstProps->getMaxCardEst(), rowCount));
  ...
  // -------------------------------------------------------------------
  myEstProps->pickOutputs(unionStatDescList, inputEstLogProp, getGroupAttr()->getCharacteristicOutputs(),
                          getSelectionPred());
...
void Union::synthLogProp(NormWA *normWAPtr) {
  // Check to see whether this GA has already been associated
  // with a logExpr for synthesis.  If so, no need to resynthesize
  // for this equivalent log. expression.
  if (getGroupAttr()->existsLogExprForSynthesis()) return;
  ...
  if (minLeft > 0 OR minRight > 0 OR(maxLeft < INFINITE_CARDINALITY AND maxRight < INFINITE_CARDINALITY)) {
    ...
    if (maxLeft == INFINITE_CARDINALITY OR maxRight == INFINITE_CARDINALITY) {
      getGroupAttr()->addConstraint(new (CmpCommon::statementHeap())
                                        CardConstraint(minLeft + minRight, (Cardinality)INFINITE_CARDINALITY));
    } else {
      getGroupAttr()->addConstraint(new (CmpCommon::statementHeap())
                                        CardConstraint(minLeft + minRight, maxLeft + maxRight));
    }
  }

  if (isALabeledUnion()) {
    // For a labeled union, if we have similar uniqueness
    // constraints in both children, we can synthesize those for the
    // union, if we include the label column. For example, if we
    // have a query
    //
    //   select * from
    //     (select 'left',  uniq1_1, uniq1_2 from t1
    //      union all
    //      select 'right', uniq2_1, uniq2_2 from t2) u(l, x, y)
    //
    // and if (uniq1_1, uniq1_2) is unique on the left side and the
    // equivalent (uniq2_1, uniq2_2) is also unique on the right
    // side, then we have a uniqueness constraint (l, x, y) for the
    // union.
    GroupAttributes &myGA = *getGroupAttr();
    GroupAttributes &leftGA = *child(0).getGroupAttr();
    GroupAttributes &rightGA = *child(1).getGroupAttr();
    ...
    for (ValueId lc = leftConstraints.init(); leftConstraints.next(lc); leftConstraints.advance(lc)) {
      ItemExpr *lx = lc.getItemExpr();
      OperatorTypeEnum loptype = lx->getOperatorType();

      if (loptype == ITM_UNIQUE_OPT_CONSTRAINT) {
        const ValueIdSet &lUniqueCols = static_cast<UniqueOptConstraint *>(lx)->uniqueCols();
        ValueIdSet uUniqueCols;
        ValueIdSet rUniqueCols;

        // Map the left uniqueness constraint into equivalent
        // sets in the union output and then the right side:
        // lUniqueCols ==> uUniqueCols ==> rUniqueCols
        unionMap_->leftColMap_.mapValueIdSetUp(uUniqueCols, lUniqueCols);
        unionMap_->rightColMap_.mapValueIdSetDown(uUniqueCols, rUniqueCols);

        if (rightGA.isUnique(rUniqueCols)) {
          // We have a uniqueness constraint on the left child
          // on lUniqueCols and a corresponding uniqueness
          // constraint on the right side on rUniqueCols, both
          // of which correspond to the union's outputs
          // uUniqueCols. Add each label column to this set
          // uUniqueCols and synthesize a uniqueness
          // constraint for these columns.
          NABoolean labelFound = FALSE;
          const ValueIdList &cmt(unionMap_->colMapTable_);

          for (CollIndex i = 0; i < cmt.entries(); i++)
            if (static_cast<ValueIdUnion *>(cmt[i].getItemExpr())->isALabel()) {
              // if some day we support labels consisting of
              // multiple values, this needs to change
              uUniqueCols += cmt[i];

              // add the uniqueness constraint for the union
              getGroupAttr()->addConstraint(new (CmpCommon::statementHeap()) UniqueOptConstraint(uUniqueCols));
              uUniqueCols -= cmt[i];
            }
        }
      }
...
// -----------------------------------------------------------------------
void GroupByAgg::synthEstLogProp(const EstLogPropSharedPtr &inputEstLogProp) {
  if (getGroupAttr()->isPropSynthesized(inputEstLogProp)) return;
  ...
  if (inputEstLogProp->getResultCardinality() > CostScalar(1) &&
    ...
    modInputLP = child(0).getGroupAttr()->materializeInputLogProp(inputEstLogProp, &multipleCalls);
  } else
  ...
  if (!trueScalAgg) {
    for (i = 0; i < outerColStatsList.entries(); i++) {
      ...
      for (ValueId idOut = requiredInput.init(); requiredInput.next(idOut) AND NOT foundFlag;
           requiredInput.advance(idOut)) {
        if (colExpr->getValueId() == idOut) {
          foundFlag = TRUE;
          break;  // jump out to outer for-loop
        } else if (idOut.getItemExpr()->getOperatorType() == ITM_VEG_REFERENCE) {
  ...
  if (trueScalAgg) {
    ...
    if (CmpCommon::getDefault(COSTING_SHORTCUT_GROUPBY_FIX) == DF_ON) {
      ...
      if (inputCard > csOne) resultCardinality = MIN_ONE(inputCard < oldCount ? inputCard : oldCount);
    }
  } else {
    ...
    if (CmpCommon::getDefault(COMP_BOOL_148) == DF_ON) {
      ...
      for (ValueId compExpWorkGroupItem = compExpWorkGroup.init(); compExpWorkGroup.next(compExpWorkGroupItem);
    ...
    for (i = 0; i < groupByTables->entries(); i++) {
      ...
      if (!(primaryKeyColumns.isEmpty()) && (primaryKeyColumns.intersect(baseColumnSet) == primaryKeyColumns)) {
    ...
    if (!columnsToRemove.isEmpty() && !interestingColSet.isEmpty()) {
      ...
      for (i = 0; i < workGroupList.entries(); i++) {
        ...
        if ((!redColCandidates.isEmpty()) && (interestingColsFromThisVeg.isEmpty())) {
      ...
      if (considerDependency)
        handleIndirectDepInGroupingcols(workGroup, interestingColSet, updatedColumnsToRemove, probableRedundantColSet,
                                        groupStatDescList);
    }
    ...
    if (appStatMan && (workGroup.entries() > 0)) {
      ...
      if (getGroupAttr()->getGroupAnalysis()) {
        ...
        for (CANodeId id = subtreeset.init(); subtreeset.next(id); subtreeset.advance(id)) {
      ...
      for (CollIndex j = 0; j < workGroupTables->entries(); j++) {
        ...
        for (ValueId workGroupItem = workGroup.init(); workGroup.next(workGroupItem);
          ...
          if ((colExpr->getOperatorType() != ITM_VEG_REFERENCE) || (columns.isEmpty())) {
        ...
        if (useTableForMaxCard) relevantGroupTables->insert(thisTable);
      }

      if (relevantGroupTables->entries() != workGroupTables->entries()) allTablesConsidered = FALSE;
      {
        for (i = 0; i < relevantGroupTables->entries(); i++) {
          ...
          if (thisTableAnalysis && thisTableAnalysis->getNodeAnalysis()) {
            CANodeId localNodeId = thisTableAnalysis->getNodeAnalysis()->getId();
            NABoolean flag = (inputEstLogProp->isCacheable() ? TRUE : FALSE);
            if (inputEstLogProp != (*GLOBAL_EMPTY_INPUT_LOGPROP)) inputEstLogProp->setCacheableFlag(FALSE);
    ...
    if (groupStatDescList.entries() > 0) largeTableNeedsStats = groupStatDescList[0]->getColStats()->isUpStatsNeeded();
    ...
    while (workGroup.entries() > 1 && ueclist != NULL) {
      baseColumnSet.clear();
      for (ValueId workGroupItem = workGroup.init(); workGroup.next(workGroupItem); workGroup.advance(workGroupItem)) {
        ItemExpr *colExpr = workGroupItem.getItemExpr();
        ValueIdSet columns;

        if (colExpr == NULL) continue;
        if ((CmpCommon::getDefault(COMP_BOOL_148) == DF_ON) && (colExpr->getOperatorType() == ITM_CASE) &&
            (colExpr->getArity() == 1) && (colExpr->child(0)->getOperatorType() == ITM_IF_THEN_ELSE) &&
            (colExpr->child(0)->getArity() == 3)) {
          // For a CASE expr, UEC is determined only by the options and
          // not the test. CASE is typically covered by compExpr code
          // This logic is for when it is not and CB_148 is ON.
          // This happens only when we have one const and a VegRef
          // as the two options. Please see IfThenElse::calculateMinMaxUecs
          // For example, if ( a = 1) then b else 2. The objective here
          // is to not include UEC(a) in the groupby UEC calculation.
          colExpr->child(0)->child(1)->findAll(ITM_BASECOLUMN, columns, TRUE, TRUE);
          colExpr->child(0)->child(2)->findAll(ITM_BASECOLUMN, columns, TRUE, TRUE);
        } else
          colExpr->findAll(ITM_BASECOLUMN, columns, TRUE, TRUE);
      ...
      if (subset.entries() < 2) {
        ...
        for (i = 0; i < groupByTables->entries(); i++) {
          ...
          if (tablesWithMissingStats && !tablesWithMissingStats->contains(thisTable) &&
            ...
            if (!ueclistOfThisTable || (ueclistOfThisTable->lookup(userColumnsSet) == csMinusOne)) {
          ...
          if (baseColumnSet.entries() == 0) break;
        }
      ...
      if (subset == prevSubset) break;  // to avoid inf loop

      // what's the group uec associated with this subset
      CostScalar subsetUec = ueclist->lookup(subset);

      if (subsetUec <= 0) break;  // some problem with statistics!
      ...
      for (i = 0; i < subsetList.entries(); i++) {
        ...
        while ((initialGrpCnt > 0) && (workGroup.entries() > 0)) {
          if (workGroup.referencesTheGivenValue(subsetList[i], referencingExpr)) workGroup.remove(referencingExpr);
          initialGrpCnt--;
        }
      }
    ...
    for (i = 0; i < groupStatDescList.entries(); i++) {
      if (groupStatDescList[i]->isExpressionHistogram()) {
        // TODO: write logic to handle expression histograms;
        // what we likely need to do is see if any expression
        // in workGroup matches this histogram, and if so,
        // insert i into singleStats.
        continue;  // for now, just ignore expression histograms
      }

      const ItemExpr *colExpr = groupStatDescList[i]->getVEGColumn().getItemExpr();

      ValueId tmp;
      if (workGroup.contains(colExpr->getValueId())) singleStats.insert(i);
      //
    ...
    for (i = 0; (i < singleStats.entries()) && (uecTotal < oldCount); i++) {
      ColStatsSharedPtr groupStats = groupStatDescList[singleStats[i]]->getColStats();

      if (groupStats->isExpressionHistogram()) {
        // TODO: write logic to handle expression histograms
        continue;  // for now, just ignore expression histograms
      }

      const ValueId columnVEG = groupStatDescList[singleStats[i]]->getVEGColumn();
    ...
    if (workGroup.entries() == 0) {
      ...
      if (loopCount == 1) {
        ColStatsSharedPtr groupStats = groupStatDescList[lastStats]->getColStatsToModify();
  ...
  if (resultCardinality > maxCard) resultCardinality = maxCard;
  ...
  for (i = 0; i < loopLimit; i++) {
    ...
    if (oldCount != resultCardinality) {
      ...
      // TODO: Replace following temporary code for expression histograms
      // with the right code.
      if (groupStatDescList[i]->isExpressionHistogram()) {
        // We skipped expressions histograms above, so it is as if
        // we are treating it as though nothing in the workGroup matches,
        // so synchronize the stats. To do this properly, we'd again
        // figure out if anything in the work group matches. Probably
        // do it in such a way that we could leverage the work we would
        // do above rather than search the workGroup again.
        groupStatDescList[i]->synchronizeStats(oldCount, resultCardinality);
        continue;
      }
      ...
      if (workGroupCopy.contains(colExpr->getValueId())) {
        ...
        groupStatDescList[i]->synchronizeStats(oldCount, resultCardinality, ColStatDesc::DO_NOT_REDUCE_UEC);
      } else {
        groupStatDescList[i]->synchronizeStats(oldCount, resultCardinality);
      }
    }
  ...
  if (isAPartialGroupByLeaf()) {
    ...
    CostScalar extGroupLength = groupKeyLength + aggregateLength + hashedRowOverhead;
    CostScalar memoryLimitInDP2 = (CostPrimitives::getBasicCostFactor(HGB_DP2_MEMORY_LIMIT));
    CostScalar groupCountInMemory = memoryLimitInDP2 * 1024. / extGroupLength;
    groupCountInMemory = MINOF(groupCountInMemory, resultCardinality);
    CostScalar mem = groupCountInMemory * extGroupLength / 1024.;

    CostScalar ipRows =
        (groupExpr_.isEmpty() ? inputEstLogProp->getResultCardinality() : childEstProps->getResultCardinality());

    CostScalar rowsEliminated = groupCountInMemory / resultCardinality * ipRows;
    resultCardinality = MAXOF(groupCountInMemory, ipRows - rowsEliminated + groupCountInMemory);
    ...
    // the wave pattern.
    if (QueryAnalysis::Instance() && QueryAnalysis::Instance()->dontSurfTheWave() && isAPartialGroupByLeaf()) {
  ...
  CostScalar maxRows = resultCardinality.minCsOne() *
                       (childEstProps->getMaxCardEst() / childEstProps->getResultCardinality().minCsOne());
  ...
  // compute maxSelectivity. toss rowcount return value.
  (void)groupStatDescListForMaxSel.estimateCardinality(resultCardinality, getSelectionPred(), outerReferences, NULL,
                                                       NULL, NULL, outerRefCount, myEstProps->unresolvedPreds(),
                                                       INNER_JOIN_MERGE, ITM_FIRST_ITEM_OP, &maxSelectivity);

  CostScalar newRowCount =
      groupStatDescList.estimateCardinality(resultCardinality,  /*in*/
                                            getSelectionPred(), /*in*/
                                            outerReferences,    /*in*/
                                            NULL, NULL,    /*for selectivityHint, which can only be given for Scan*/
                                            NULL,          /* for Cardinality hint, which can only be given for Scan */
                                            outerRefCount, /*in/out*/
                                            myEstProps->unresolvedPreds(), /*in/out*/
                                            INNER_JOIN_MERGE,              /*in*/
...
void GroupByAgg::handleIndirectDepInGroupingcols(ValueIdSet &workGroup, ValueIdSet &interestingColSet,
  ...
  for (CollIndex i = 0; i < newWorkGroupList.entries(); i++) {
    ...
    if (!interestingColsFromThisVeg.isEmpty()) {
      ...
      if (!(nonInterestingColSetFromVeg.isEmpty())) {
        ...
        if (itemCanBeRemoved) {
          for (ValueId col = interestingColsFromThisVeg.init(); interestingColsFromThisVeg.next(col);
            ...
            for (CollIndex l = 0; l < groupStatDescList.entries(); l++) {
              ...
              if (l == groupStatDescList.entries() - 1) itemCanBeRemoved = FALSE;
            }
            if (!itemCanBeRemoved) break;
          }
        }

        if (itemCanBeRemoved) workGroup.remove(newWorkGroupList[i]);
      }
    }
  }
  ...
  if (count > 1) {
    ...
    for (CollIndex k = 0; k < count; k++) {
      ...
      if ((k == count - 1) && (minUec == csZero)) break;
      ...
      for (CollIndex l = 0; l < groupStatDescList.entries(); l++) {
        ...
        if (l == groupStatDescList.entries() - 1) workGroup.remove(newWorkGroupList[cirDepCandidates[k]]);
      }
    }
  }
}

void GroupByAgg::synthLogProp(NormWA *normWAPtr) {
  // Check to see whether this GA has already been associated
  // with a logExpr for synthesis.  If so, no need to resynthesize
  // for this equivalent log. expression.
  if (getGroupAttr()->existsLogExprForSynthesis()) return;
  ...
  if (NOT isAPartialGroupByLeaf()) {
    ...
    if (groupExpr_.isEmpty()) {
      ...
      if (selectionPred().isEmpty())
        getGroupAttr()->addConstraint(new (CmpCommon::statementHeap()) CardConstraint((Cardinality)1, (Cardinality)1));
      else
        getGroupAttr()->addConstraint(new (CmpCommon::statementHeap()) CardConstraint((Cardinality)0, (Cardinality)1));
    } else {
      ...
      if (uniqueCols.isEmpty()) {
        ...
        getGroupAttr()->addConstraint(new (CmpCommon::statementHeap()) UniqueOptConstraint(uniqueCols));
...
// -----------------------------------------------------------------------
void RelRoot::synthEstLogProp(const EstLogPropSharedPtr &inputEstLogProp) {
  if (getGroupAttr()->isPropSynthesized(inputEstLogProp)) return;
...
void RelRoot::synthLogProp(NormWA *normWAPtr) {
  // Check to see whether this GA has already been associated
  // with a logExpr for synthesis.  If so, no need to resynthesize
  // for this equivalent log. expression.
  if (getGroupAttr()->existsLogExprForSynthesis()) return;
...
void Filter::synthEstLogProp(const EstLogPropSharedPtr &inputEstLogProp) {
  ...
  if (orgRowcount > 1) {
    ...
    if (orgRowcount == newRowcount) modInputLP = inputEstLogProp;
  } else
    modInputLP = inputEstLogProp;

  if (getGroupAttr()->isPropSynthesized(modInputLP) && getGroupAttr()->isPropSynthesized(inputEstLogProp)) {
  ...
  EstLogPropSharedPtr intermedEstLogProp =
      synthEstLogPropForUnaryLeafOp(modInputLP, child(0).outputLogProp(modInputLP)->getColStats(),
...
void Filter::synthLogProp(NormWA *normWAPtr) {
  // Check to see whether this GA has already been associated
  // with a logExpr for synthesis.  If so, no need to resynthesize
  // for this equivalent log. expression.
  if (getGroupAttr()->existsLogExprForSynthesis()) return;
  ...
  myGA.addSuitableRefOptConstraints(child(0).getGroupAttr()->getConstraints());
  processCompRefOptConstraints(normWAPtr);
  ...
  if (myGA.hasCardConstraint(minRows, maxRows) AND minRows > 0) {
    ...
    for (ValueId x = myConstraints.init(); myConstraints.next(x); myConstraints.advance(x)) {
      ...
      if (loptype == ITM_CARD_CONSTRAINT) {
        ...
        if (c->getLowerBound() > 0) {
          ...
          if (c->getUpperBound() < INFINITE_CARDINALITY)
            myGA.addConstraint(new (CmpCommon::statementHeap()) CardConstraint((Cardinality)0, c->getUpperBound()));
        }
      }
    }
  }
...
// -----------------------------------------------------------------------
void MapValueIds::synthEstLogProp(const EstLogPropSharedPtr &inputEstLogProp) {
  if (getGroupAttr()->isPropSynthesized(inputEstLogProp)) return;

  // Synthesize estimated logical properties from my child's

  const ColStatDescList *colStats = &(child(0).outputLogProp(inputEstLogProp)->getColStats());
  CostScalar baseCardinality = child(0).outputLogProp(inputEstLogProp)->getResultCardinality();

  if (cseRef_)  // could also do this unconditionally
  {
    ColStatDescList *mappedStats = new (CmpCommon::statementHeap()) ColStatDescList(CmpCommon::statementHeap());

    mappedStats->makeMappedDeepCopy(*colStats, map_, TRUE);
    colStats = mappedStats;
  }

  EstLogPropSharedPtr myEstLogProp = synthEstLogPropForUnaryLeafOp(inputEstLogProp, *colStats, baseCardinality);

  getGroupAttr()->addInputOutputLogProp(inputEstLogProp, myEstLogProp);

}  // MapValueIds::synthEstLogProp

void MapValueIds::synthLogProp(NormWA *normWAPtr) {
  // Check to see whether this GA has already been associated
  // with a logExpr for synthesis.  If so, no need to resynthesize
  // for this equivalent log. expression.
  if (getGroupAttr()->existsLogExprForSynthesis()) return;
  ...
  for (ValueId x = child(0).getGroupAttr()->getConstraints().init(); child(0).getGroupAttr()->getConstraints().next(x);
    ...
    if (x.getItemExpr()->getOperatorType() == ITM_UNIQUE_OPT_CONSTRAINT) {
      ...
      getMap().rewriteValueIdSetUp(uniqueCols, ((UniqueOptConstraint *)x.getItemExpr())->uniqueCols());
      myGA.addConstraint(new (CmpCommon::statementHeap()) UniqueOptConstraint(uniqueCols));
...
void Scan::synthEstLogProp(const EstLogPropSharedPtr &inputEstLogProp) {
  ...
  const ColStatDescList *colStats = &getTableDesc()->getTableColStats();
  ...
  if (commonSubExpr_) {
    ...
    ValueIdList cseCols;
    CSEInfo *info = CmpCommon::statement()->getCSEInfo(commonSubExpr_->getName());
    // The original tree is the child of the CommonSubExprRef that
    // did the analysis of whether to use CSEs
    CommonSubExprRef *analyzingCSERef = info->getConsumer(info->getIdOfAnalyzingConsumer());

    // this makes the ValueIdList of only those CSE columns that are
    // actually used in the temp table
    commonSubExpr_->makeValueIdListFromBitVector(cseCols, analyzingCSERef->getColumnList(), info->getNeededColumns());
    ...
    ValueIdMap cseToTempScanMap(tempTableVEGCols, cseCols);

    // get to the ColStatDesc of the common subexpression
    const ColStatDescList &origCSEColStats(analyzingCSERef->getEstLogProps()->colStats());

    ColStatDescList *mappedStats = new (CmpCommon::statementHeap()) ColStatDescList(CmpCommon::statementHeap());

    // now translate the col stats of the original CSE to the temp table
    mappedStats->makeMappedDeepCopy(origCSEColStats, cseToTempScanMap, FALSE);
  ...
  if ((CmpCommon::getDefault(COMP_BOOL_42) == DF_ON) && (qa && qa->isCompressedHistsViable())) {
    ...
    if (!myColStatsList.isEmpty()) myEstLogProp->setResultCardinality(myColStatsList[0]->getColStats()->getRowcount());
  }
...
NABoolean Scan::checkForCardRange(const ValueIdSet &setOfPredicates, CostScalar &newRowCount /* in and out */) {
  if (!CURRSTMT_OPTDEFAULTS->histUseSampleForCardEst()) return FALSE;
  NABoolean cardChange = FALSE;

  TableDesc *td = getTableDesc();

  if (td == NULL) return cardChange;

  if (td->getTableAnalysis() == NULL) return cardChange;
  ...
  if (derangedLocalPreds.entries() == 0) return cardChange;
  ...
  if (commonPreds.entries() == 0) return cardChange;
  ...
  if (derangedLocalPreds == predsExecuted) {
    ...
    cardChange = TRUE;
  } else if ((commonPreds.entries() < predsExecuted.entries()) &&
...
void Scan::synthLogProp(NormWA *normWAPtr) {
  ...
  tabId_->getEquivVEGCols(tabId_->getClusteringIndex()->getIndexKey(), tempClusteringKey);
  ...
  for (ValueId p = getSelectionPred().init(); getSelectionPred().next(p); getSelectionPred().advance(p)) {
    ...
    if (cv && !negateIt && cv->isAFalseConstant()) {
      ...
      getGroupAttr()->addConstraint(new (CmpCommon::statementHeap()) CardConstraint((Cardinality)0, (Cardinality)0));
  ...
  for (CollIndex indexNo = 0; indexNo < ixlist.entries(); indexNo++) {
    ...
    if ((idesc->isClusteringIndex()) && (getOptHbaseAccessOptions() && getOptHbaseAccessOptions()->tsSpecified()))
      continue;

    if (idesc->isClusteringIndex() && getTableDesc()->getNATable()->isHiveTable()) {
    ...
    tabId_->getEquivVEGCols(indexKey, tempUniqueCols);
    uniqueCols = tempUniqueCols;

    if (idesc->isClusteringIndex() OR(NOT uniqueCols.contains(clusteringKey))) {
      ...
      // unique columns
      for (ValueId cc = uniqueCols.init(); uniqueCols.next(cc); uniqueCols.advance(cc)) {
        ...
        if (computedExpr != NULL_VALUE_ID) {
          ...
          tabId_->getEquivVEGCols(underlyingCols, underlyingVEGCols);

          if (uniqueCols.contains(underlyingVEGCols)) uniqueCols -= cc;
        }
      }
  ...
  if (normWAPtr && (CmpCommon::getDefault(ELIMINATE_REDUNDANT_JOINS) != DF_OFF)) {
    ...
    const AbstractRIConstraintList RIConstraintsList = getTableDesc()->getNATable()->getRefConstraints();
    for (CollIndex i = 0; i < RIConstraintsList.entries(); i++) {
      riConstraint = (RefConstraint *)RIConstraintsList.at(i);
      if ((riConstraint->getIsEnforced()) || (CmpCommon::getDefault(ELIMINATE_REDUNDANT_JOINS) != DF_MINIMUM)) {
        keyColVEGVids.clear();
        ...
        if (myOutputs.contains(keyColVEGVidsSet)) {
          getGroupAttr()->addConstraint(new (CmpCommon::statementHeap())
                                            RefOptConstraint(keyColVEGVids, riConstraint->getUniqueConstraintName()));
  ...
  //   the number of rows.
#ifndef NDEBUG
  NAString cname(getTableDesc()->getCorrNameObj().getCorrNameAsString(), CmpCommon::statementHeap());
  ...
  if ((CURRSTMT_OPTDEFAULTS->histDefaultSampleSize() > 0) && this->getRETDesc() && this->getRETDesc()->getBindWA() &&
  ...
  for (CollIndex i = 0; i < initialStats.entries(); i++)
    if (!initialStats[i]->isExpressionHistogram())
      initialStats[i]->VEGColumn() = getTableDesc()->getEquivVEGCol(initialStats[i]->getColumn());

  // set if this scan node is trafodion SMD table
  if (getTableDesc()->getNATable()->isSeabaseMDTable()) getGroupAttr()->setSeabaseMDTable(TRUE);
...
void Scan::createUniquenessConstraintsForHiveTable() {
  ...
  const NAColumnArray &colArray = clusteringIndex->getNAFileSet()->getAllColumns();
  NABoolean addCardConstraint = TRUE;
  ...
  for (int i = 0; i < 2; i++) {
    ...
    if (i == 0) {
      if (persistentKey)  // do not add if are using persistent key
        continue;
      // uniqueness constraint #1: (short, non-persistent key)
      uniqueCols += tabId_->getColumnVEGList()[colArray.getColumn(NATable::getNameOfInputRangeCol())->getPosition()];
      uniqueCols += tabId_->getColumnVEGList()[colArray.getColumn(NATable::getNameOfRowInRangeCol())->getPosition()];
    } else {
      if (!persistentKey)  // do not add if are using persistent key
        continue;
      // uniqueness constraint #2: (long, persistent key)
      uniqueCols += tabId_->getColumnVEGList()[colArray.getColumn(NATable::getNameOfInputFileCol())->getPosition()];
      uniqueCols += tabId_->getColumnVEGList()[colArray.getColumn(NATable::getNameOfBlockOffsetCol())->getPosition()];
    }
...
void Scan::processCompRefOptConstraints(NormWA *normWAPtr) {
  if (normWAPtr && (CmpCommon::getDefault(ELIMINATE_REDUNDANT_JOINS) != DF_OFF) &&
    ...
    ValueIdSet keyColVEGVidsSet;
    const ValueIdSet &myOutputs = getGroupAttr()->getCharacteristicOutputs();
    const ValueIdSet &myEssOutputs = getGroupAttr()->getEssentialCharacteristicOutputs();

    const AbstractRIConstraintList UniqConstraintsList = getTableDesc()->getNATable()->getUniqueConstraints();
    ...
    for (CollIndex i = 0; i < UniqConstraintsList.entries(); i++) {
      ...
      for (CollIndex j = 0; j < keyCols.entries(); j++) {
        keyColVEGVids.insert(getTableDesc()->getColumnVEGList()[keyCols.at(j)->getPosition()]);
      }
      keyColVEGVidsSet = keyColVEGVids;
      ...
      if ((myOutputs.contains(keyColVEGVidsSet)) && (keyColVEGVidsSet.contains(myEssOutputs)) &&
          (uniqConstraint->hasRefConstraintsReferencingMe()) && (NOT containsReducingPredicates(keyColVEGVidsSet))) {
        getGroupAttr()->addConstraint(new (CmpCommon::statementHeap()) ComplementaryRefOptConstraint(
            keyColVEGVids, uniqConstraint->getConstraintName(), this, getTableDesc()));
...
NABoolean Scan::containsReducingPredicates(const ValueIdSet &keyCols) const {
  ...
  for (ValueId x = selPreds.init(); selPreds.next(x); selPreds.advance(x)) {
    ...
    if (ie->getOperatorType() == ITM_RANGE_SPEC_FUNC) ie = ie->child(1);
    ...
    if (ie->getOperatorType() == ITM_VEG_PREDICATE) {
      ...
      if (ie->child(0)->doesExprEvaluateToConstant(FALSE) && ie->child(1)->getOperatorType() == ITM_VEG_REFERENCE) {
      ...
      if (NOT(keyCols.contains(childId))) return TRUE;
    } else
...
void Scan::finishSynthEstLogProp() {
...
// -----------------------------------------------------------------------
void Tuple::synthEstLogProp(const EstLogPropSharedPtr &inputEstLogProp) {
  if (getGroupAttr()->isPropSynthesized(inputEstLogProp)) return;
  ...
  for (CollIndex i = 0; i < nTupleListEntries; i++) {
    ...
    outputColStats.addColStatDescForVirtualCol(1, 1, ituple, ituple, ituple, NULL);
  }

  getGroupAttr()->addInputOutputLogProp(inputEstLogProp, myEstLogProp);
}  // Tuple::synthEstLogProp

void Tuple::synthLogProp(NormWA *normWAPtr) {
  // Check to see whether this GA has already been associated
  // with a logExpr for synthesis.  If so, no need to resynthesize
  // for this equivalent log. expression.
  if (getGroupAttr()->existsLogExprForSynthesis()) return;
...
// -----------------------------------------------------------------------
void TupleList::synthEstLogProp(const EstLogPropSharedPtr &inputEstLogProp) {
  if (getGroupAttr()->isPropSynthesized(inputEstLogProp)) return;
  ...
  for (CollIndex i = 0; i < nTupleListEntries; i++) {
    ...
    if (CmpCommon::getDefault(COMP_BOOL_48) == DF_ON) {
      ...
      if (createdForInList()) defineVirtual = FALSE;
    }

    outputColStats.addColStatDescForVirtualCol(numberOfTuples, numberOfTuples, ituple, ituple, ituple, this,
                                               defineVirtual);
  }
...
void TupleList::synthLogProp(NormWA *normWAPtr) {
  // Check to see whether this GA has already been associated
  // with a logExpr for synthesis.  If so, no need to resynthesize
  // for this equivalent log. expression.
  if (getGroupAttr()->existsLogExprForSynthesis()) return;
  ...
  if (selectionPred().isEmpty())
    getGroupAttr()->addConstraint(new (CmpCommon::statementHeap())
                                      CardConstraint((Cardinality)numberOfTuples_, (Cardinality)numberOfTuples_));
  else
    getGroupAttr()->addConstraint(new (CmpCommon::statementHeap())
                                      CardConstraint((Cardinality)0, (Cardinality)numberOfTuples_));
...
// -----------------------------------------------------------------------
void GenericUpdate::synthEstLogProp(const EstLogPropSharedPtr &inputEstLogProp) {
  if (getGroupAttr()->isPropSynthesized(inputEstLogProp)) return;
  ...
  if (getArity() == 0) {
    ...
    myEstLogProp = synthEstLogPropForUnaryLeafOp(inputEstLogProp, childOutputLogProp->getColStats(),
...
void GenericUpdate::synthLogProp(NormWA *normWAPtr) {
  // Check to see whether this GA has already been associated
  // with a logExpr for synthesis.  If so, no need to resynthesize
  // for this equivalent log. expression.
  if (getGroupAttr()->existsLogExprForSynthesis()) return;
...
void GenericUpdate::finishSynthEstLogProp() {
  CostScalar minRowCount = COSTSCALAR_MAX;
  NABoolean triggersTempTable = (getTableName().getSpecialType() == ExtendedQualName::TRIGTEMP_TABLE) ? TRUE : FALSE;

  if (triggersTempTable) {
    if (getArity() == 0) {
      minRowCount = getGroupAttr()->outputLogProp((*GLOBAL_EMPTY_INPUT_LOGPROP))->getResultCardinality();
    } else {
...
// -----------------------------------------------------------------------
void ExplainFunc::synthEstLogProp(const EstLogPropSharedPtr &inputEstLogProp) {
  if (getGroupAttr()->isPropSynthesized(inputEstLogProp)) return;
...
void ExplainFunc::synthLogProp(NormWA *normWAPtr) {
  // Check to see whether this GA has already been associated
  // with a logExpr for synthesis.  If so, no need to resynthesize
  //  for this equivalent log. expression.
  if (getGroupAttr()->existsLogExprForSynthesis()) return;
...
void Transpose::synthEstLogProp(const EstLogPropSharedPtr &inputEstLogProp) {
  ...
  if (transUnionVector()[0].entries() > 0) {
    ...
    ColStatsSharedPtr keyStats(new (HISTHEAP) ColStats(id, numKeyVals, rowCount, rowCount, FALSE, FALSE, emptyHist,
                                                       FALSE, 1.0, 1.0,
    ...
    ColStatDescSharedPtr keyColStatDesc(new (HISTHEAP) ColStatDesc(keyStats, (transUnionVector()[0])[0]), HISTHEAP);

    compColStatDesc =
        ColStatDescSharedPtr(new (HISTHEAP) ColStatDesc(compColStats, (transUnionVector()[0])[0]), HISTHEAP);
  ...
  for (CollIndex vec = 1; vec < transUnionVectorSize(); vec++) {
    ...
    for (CollIndex vidu = 0; vidu < valIdList.entries(); vidu++) {
      ...
      for (CollIndex vid = 0; vid < valIdUnion->entries(); vid++) {
        ...
        CollIndex index;
        if (childColStatsList.getColStatDescIndex(index, sourceValId)) {
          if (!firstSourceID) firstSourceID = sourceValId;
        } else
          constMembers++;
      ...
      if (!firstSourceID) {
        ...
        transStatDescList.addColStatDescForVirtualCol(constMembers, rowCount, valIdUnion->getSource(0), valIdList[vidu],
                                                      valIdUnion->getValueId(), NULL);
        ...
        childColStatsList.getColStatDescIndex(firstEntry, firstSourceID);
        ...
        ColStatsSharedPtr transColStats = firstColStatDesc->getColStatsToModify();
        ...
        ColStatDescSharedPtr transStatDesc(new (HISTHEAP) ColStatDesc(transColStats, firstCol), HISTHEAP);
        ...
        for (CollIndex vid = 1; vid < valIdUnion->entries(); vid++) {
          ...
          childColStatsList.getColStatDescIndex(index, sourceColumn);

          if (index == NULL_COLL_INDEX) continue;

          ColStatDescSharedPtr colStatDesc = childColStatsList[index];

          transStatDescList[entry]->mergeColStatDesc(colStatDesc, UNION_MERGE, FALSE, REL_UNION);
  ...
  if (haveAllColStats && compColStats) {
    ...
    for (CollIndex vec = 1; vec < transUnionVectorSize(); vec++) {
      ValueIdList &valIdList = transUnionVector()[vec];
      ValueIdUnion *firstValIdUnion = (ValueIdUnion *)valIdList[0].getValueDesc()->getItemExpr();
  ...
  for (CollIndex v = 1; v < transUnionVectorSize(); v++) {
    ValueIdUnion *valIdUnion = (ValueIdUnion *)(transUnionVector()[v])[0].getValueDesc()->getItemExpr();
  ...
  // Estimated rowCount.
  //
  CostScalar rowCount = childEstProp->getResultCardinality().minCsOne() * numTransposeExprs;
  ...
  CostScalar maxRows = rowCount * (childEstProp->getMaxCardEst() / childEstProp->getResultCardinality().minCsOne());
  ...
  myEstProps->pickOutputs(transStatDescList, inputEstLogProp, getGroupAttr()->getCharacteristicOutputs(),
                          getSelectionPred());
...
//
void Transpose::synthLogProp(NormWA *normWAPtr) {
...
void Pack::synthEstLogProp(const EstLogPropSharedPtr &inEstLogProp) {
...
void Pack::synthLogProp(NormWA *normWAPtr) {
  // check to see whether properties are already synthesized.
  if (getGroupAttr()->existsLogExprForSynthesis()) return;
...
void CommonSubExprRef::synthEstLogProp(const EstLogPropSharedPtr &inputEstLogProp) {
  if (getGroupAttr()->isPropSynthesized(inputEstLogProp)) return;  // already done this

  // make the child's est log props my own
  getGroupAttr()->addInputOutputLogProp(inputEstLogProp, child(0).getGroupAttr()->outputLogProp(inputEstLogProp));
}
...
void IsolatedScalarUDF::synthLogProp(NormWA *normWAPtr) {
  ...
  if (getRoutineDesc()->getEffectiveNARoutine()->isDeterministic() == FALSE) GA->setHasNonDeterministicUDRs(TRUE);
  ...
  FuncDependencyConstraint *fdc = new (CmpCommon::statementHeap())
      FuncDependencyConstraint(GA->getCharacteristicInputs(), GA->getCharacteristicOutputs());
...
void IsolatedScalarUDF::synthEstLogProp(const EstLogPropSharedPtr &inputEstLogProp) {
  ...
  EstLogPropSharedPtr myEstProps(
      new (HISTHEAP) EstLogProp(inputEstLogProp->getResultCardinality() * getRoutineDesc()->getEffFanOut()));

  // We know that at most we will return every row seen.
  myEstProps->setMaxCardEst(inputEstLogProp->getResultCardinality() * getRoutineDesc()->getEffFanOut());

  // get the default setting for UEC.
  UInt32 defaultUdfUec = (ActiveSchemaDB()->getDefaults()).getAsULong(ROUTINE_DEFAULT_UEC);
  ...
  for (UInt32 i = 0; i < inputColStatsList.entries(); i++) {
    ...
    maxInputUec = MAXOF(inputStatDesc->getColStats()->getTotalUec(), maxInputUec);
  }

  // Associate the UEC we stored in the RoutineDesc to this
  // EstLogPropSharedPtr.
  ColStatDescList &colStatDescList = getRoutineDesc()->getEffOutParamColStatDescList();
  ...
  for (UInt32 k = 0; k < colStatDescList.entries(); k++) {
    ...
    if (maxOutputUec == csMinusOne) {
      ...
      if (defaultUdfUec > 0) maxOutputUec = MAXOF(csOne, defaultUdfUec);
      ...
      outColStatDesc->getColStats()->setRowsAndUec(outColStatDesc->getColStats()->getRowcount(), maxOutputUec);
    }
  }
  ...
  newRowCount = colStatDescList.estimateCardinality(newRowCount,                     /*in*/
                                                    getSelectionPred(),              /*in*/
                                                    outerReferences,                 /*in*/
                                                    NULL, NULL, NULL, outerRefCount, /*in/out*/
                                                    myEstProps->unresolvedPreds(),   /*in/out*/
                                                    mergeMethod,                     /*in*/
  ...
  // for the group.
  myEstProps->pickOutputs(colStatDescList, inputEstLogProp, getGroupAttr()->getCharacteristicOutputs(),
                          selectionPred());
