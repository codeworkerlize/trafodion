     ...
     NABoolean scaleByFactor)
{
  ...
  if (suggestedNewNumberOfPartitions == 1) {
    NodeMap* nodeMap = new (heap_) NodeMap(heap_, 1, 
                                           NodeMapEntry::ACTIVE, 
                                           getNodeMap()->type());
  ...
  if ( suggestedNewNumberOfPartitions != getCountOfPartitions() ) {
     NodeMap* newNodeMap = 
       new (heap_) NodeMap(heap_, suggestedNewNumberOfPartitions,
                           NodeMapEntry::ACTIVE, getNodeMap()->type());
...
PartitioningFunction*
SinglePartitionPartitioningFunction::copy(NAMemory* h) const
{
...
PartitioningFunction*
ReplicateViaBroadcastPartitioningFunction::copy(NAMemory *h) const
{
...
PartitioningFunction*
ReplicateNoBroadcastPartitioningFunction::copy(NAMemory *h) const
{
...
PartitioningFunction*
HashPartitioningFunction::copy(NAMemory *h) const
{
...
PartitioningFunction*
HashDistPartitioningFunction::copy(NAMemory *h) const
{
...
PartitioningFunction*
Hash2PartitioningFunction::copy(NAMemory *h) const
{
...
PartitioningFunction*
SkewedDataPartitioningFunction::copy(NAMemory *h) const
{
  SkewedDataPartitioningFunction *result;

  result = new (h)
    SkewedDataPartitioningFunction(*this, h);
    ...
    for(UInt32 i=0; i < boundaryStringsList_.entries(); i++)
    {
       if (!boundaryStringsList_.used(i) || !boundaryStringsList_[i]) 
       {
         ...
         boundaryValuesList_.insertAt(i,
                                      NATable::getRangePartitionBoundaryValues
...
PartitioningFunction*
RangePartitioningFunction::copy(NAMemory *h) const
...
{
    if (low == NULL || key == NULL || high == NULL)
      return FALSE;
...
PartitioningFunction*
RoundRobinPartitioningFunction::copy(NAMemory *h) const
{
...
PartitioningFunction*
HivePartitioningFunction::copy(NAMemory *h) const
{
...
HivePartitioningFunction::buildHashingExpressionForExpr(ItemExpr* expr) 
{
  ...
  ItemExpr* narrowedNegatedHiveHash = 
     new (heap) Narrow(negatedHiveHash, dataConversionErrorFlag, new (heap) SQLInt(heap, TRUE, FALSE),
