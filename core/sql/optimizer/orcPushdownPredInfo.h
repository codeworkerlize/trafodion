...
{
public:
 ExtPushdownPredInfo(enum ExtPushdownOperatorType type, 
                     const ValueId &colValId,
                     const ValueId &operValId)
      : type_(type), colValId_(colValId), operValId_(operValId),
        filterId_(extractFilterId(operValId))
  {}

 ExtPushdownPredInfo(enum ExtPushdownOperatorType type, 
                     const ValueId &colValId)
      : type_(type), colValId_(colValId),
        filterId_(-1)
  {}

 ExtPushdownPredInfo(enum ExtPushdownOperatorType type, 
                     const ValueId &colValId,
                     const ValueIdList &operValIdList)
      : type_(type), colValId_(colValId), operValIdList_(operValIdList),
        filterId_(-1)
  {}

  ExtPushdownPredInfo(enum ExtPushdownOperatorType type = UNKNOWN_OPER)
       : type_(type),
        filterId_(-1)
  {}
...
public:
   ExtPushdownPredInfoList(Lng32 ct = 0, 
