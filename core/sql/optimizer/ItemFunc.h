...
static NABoolean CheckModeCompatible1()
{ 
  return CmpCommon::getDefault(MODE_COMPATIBLE_1) == DF_ON;
}
//
// -----------------------------------------------------------------------
// aggregate functions
...
public:
  ...
  BuiltinFunction(OperatorTypeEnum otype,
                  NAMemory *h,
		  const LIST(ItemExpr *) &children)
       : Function(otype,children,h) { setIsBuiltinFuncOrderable(isBuiltinFunctionOrderable()); }
...
{
public:
  MaxBytes(OperatorTypeEnum op, ItemExpr *val1Ptr)
    : CacheableBuiltinFunction(op, 1, val1Ptr)
    { 
         allowsSQLnullArg() = CheckModeCompatible1();
	 }
...
{
public:
  AnsiTenantFunction(OperatorTypeEnum oper)
         : BuiltinFunction(oper) {}
...
public:
  ...
  DateFormat(ItemExpr *val1Ptr, ItemExpr *val2Ptr,
             Lng32 formatType, NABoolean wasDateformat = FALSE);
  // virtual destructor
  virtual ~DateFormat();
...
public:
  ...
  Trim(Int32  mode, ItemExpr *val1Ptr, ItemExpr *val2Ptr,
       NABoolean trimCharNotSpecified = FALSE)
...
public:
  ...
  { return (flags_ & PAD_USE_ZERO) != 0; }
  void setPadUseZero(NABoolean v)
  { (v ? flags_ |= PAD_USE_ZERO: flags_ &= ~PAD_USE_ZERO); }
...
public:
 ...
 enum ObjectType
 {
   ...
   EXTRACT_, EMPTY_LOB_, HANDLE_STRING_,HANDLE_STRING_WITH_DATA_,
   HANDLE_STRING_AS_CHAR_, // return result as char datatype instead of lob
   NOOP_
 };
 ...
 LOBoper(OperatorTypeEnum otype,
   ...
   : BuiltinFunction(otype,  CmpCommon::statementHeap(),
     ...
     lobInlineLimit_(CmpCommon::getDefaultNumeric(TRAF_LOB_INLINED_DATA_MAXBYTES)),
     ...
     lobInlinedDataMaxLen_(CmpCommon::getDefaultNumeric(TRAF_LOB_INLINED_DATA_MAXBYTES)),
     lobHbaseDataMaxLen_(0),
     lobDataInHbaseColLen_(-1),
     position_(1),
     flags_(0)
   {
 ...
 // copyTopNode method
  ...
  NAString &srcLobStorageLocation() { return srcLobStorageLocation_; }
  Int64 getLobMaxSize() {return lobMaxSize_; }
  Int32 computeLobInlineStorageLen(ItemExpr *op);
  ...
  Int32 &lobDataInHbaseColLen() { return lobDataInHbaseColLen_; }
  Int32 &lobChunkMaxLen() { return lobChunkMaxLen_; }

  NAString &getSchemaName() { return schName_; }
  const NAString &getSchemaName() const { return schName_; }

  NAString &getSrcSchemaName() { return srcSchName_; }
  const NAString &getSrcSchemaName() const { return srcSchName_; }

  Int64 getPosition() const { return position_; }
  void setPosition(Int64 p) { position_ = p; }
 ...
 protected:
  ...
  Int32 lobDataInHbaseColLen_;
  Int32 lobChunkMaxLen_;

  Int64 position_; // start position, 1 based. Used for select or update.

  Int32 flags_;

  // schname of table being selected/deleted/inserted/updated
  NAString schName_;

  // src schema name for insert...select
  NAString srcSchName_;
}; // LOBoper
...
class LOBinsert : public LOBoper
{
 ...
 LOBinsert(ItemExpr *val1Ptr, 
  ...
  NAString &insertedTableSchemaName() { return getSchemaName(); }
...
class LOBupdate : public LOBoper
{
 public:
  ...
  NAString &updatedTableSchemaName() { return getSchemaName(); }
...
class LOBconvert : public LOBoper
{
 public:
  ...
  LOBconvert(ItemExpr *val1Ptr, ObjectType toObj,  
             Lng32 tgtSize = CmpCommon::getDefaultNumeric(LOB_OUTPUT_SIZE),
             Int64 position = 1,
             CharInfo::CharSet charset=CharInfo::UnknownCharSet) 
   : LOBoper(ITM_LOBCONVERT, val1Ptr, NULL,NULL,toObj),
     tgtSize_(tgtSize), charSet_(charset)     
    {
...
class LOBlength : public LOBoper
{
 public:
  
 LOBlength(ItemExpr *val1Ptr)
      : LOBoper(ITM_LOBLENGTH, val1Ptr)
    {};
...
{
public:
  DecodeBase64(ItemExpr *val1Ptr, 
               const NAType *type = NULL,
               CharInfo::CharSet charSet = CharInfo::ISO88591,
               NAMemory *h = CmpCommon::statementHeap())
       : BuiltinFunction(ITM_DECODE_BASE64, h, 1, val1Ptr),
         type_(type),
         charSet_(charSet)
    { allowsSQLnullArg() =   TRUE; }
...
public:
  ...
  void setCompatibleMode(short compatibleMode)
  {
...
{
public:
  RegexpReplace(ItemExpr *value1, ItemExpr *value2, ItemExpr *value3)
...
{
public:
  RegexpSubstr(ItemExpr *value1, ItemExpr *value2)
       : CacheableBuiltinFunction(ITM_REGEXP_SUBSTR, 2, value1, value2)
  { 
    allowsSQLnullArg() = CheckModeCompatible1();
  }
...
{
public:
  RegexpCount(ItemExpr *value1, ItemExpr *value2)
       : CacheableBuiltinFunction(ITM_REGEXP_COUNT, 2, value1, value2)
  { 
    allowsSQLnullArg() = CheckModeCompatible1();
  }
...
{
public:
  ItmSeqRunningPivotGroup(OperatorTypeEnum itemType, ItemExpr *valPtr,
                                       ItemExpr * orderBy, ItemExpr * delim, const Lng32 maxLen)
         : ItmSeqRunningFunction(itemType, valPtr)
         , delimiter_(delim)
         , maxPivotLen_(maxLen)
  {
...
public:
	...
	virtual  NABoolean isOlapFunction() const { return TRUE; }
        // get a printable string that identifies the operator
        virtual const NAString getText() const    { return getOperatorType() == ITM_OLAP_FIRST_VALUE ? "olap first_value" : "olap last_value"; }
...
{
public:
    ItmOlapPivotGroup(OperatorTypeEnum itemType, ItemExpr *column,
                                ItemExpr * orderBy, ItemExpr * delim, const Lng32 maxLen);
...
public:
  HbaseRowid(ItemExpr * col, UInt32 rowidLen = 0)
    : HbaseAttribute(ITM_HBASE_ROWID,col),
      rowidLen_(rowidLen),
      rowidType_(NULL)
    {}
...
public:
  HbaseRowidRef(CorrName * corrName, ItemExpr * col = NULL)
      : HbaseAttributeRef(ITM_HBASE_ROWID_REF, col),
        corrName_(corrName)
    {}
...
{
public:
  BeginKey(ItemExpr *valPtr)
      : BuiltinFunction(ITM_BEGINKEY, CmpCommon::statementHeap(), 1, valPtr)
  {
    allowsSQLnullArg() = CheckModeCompatible1();
  }
...
public:
  EndKey(ItemExpr *valPtr, Int16 bytesPerChar = 0, NAWchar maxValue = 0xffff)
      : BuiltinFunction(ITM_ENDKEY, CmpCommon::statementHeap(), 1, valPtr), bytesPerChar_(bytesPerChar), maxValue_(maxValue)
  {
    allowsSQLnullArg() = CheckModeCompatible1();
  }
