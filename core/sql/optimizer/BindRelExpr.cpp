...
static void GU_DEBUG_Display(BindWA *bindWA, GenericUpdate *gu, const char *text, RelExpr *reDown = NULL,
                             NABoolean preEndl = FALSE, NABoolean postEndl = FALSE) {
#ifndef NDEBUG
  if (!GU_DEBUG) return;
  ...
  if (gu->getTableDesc()) {
    ...
    cerr << gu->getUpdTableNameText() << " this>td(" << text << ") "
         << gu->getTableDesc()->getCorrNameObj().getExposedNameAsAnsiString() << " " << tmp << endl;
  }
  ...
  if (bindWA->getCurrentScope()->getRETDesc() && bindWA->getCurrentScope()->getRETDesc() != rd) {
...
static RETDesc *bindRowValues(BindWA *bindWA, ItemExpr *exprTree, ValueIdList &vidList, RelExpr *parent,
  ...
  if (prevScope) inSubquery = prevScope->context()->inSubquery();
  ...
  if ((allowMultiDegreeTok == DF_ON) || (allowMultiDegreeTok == DF_SYSTEM)) allowMultiDegSubqInSelect_Enabled = TRUE;
  ...
  for (CollIndex i = 0; i < exprList.entries(); i++, j++) {
    ...
    if (inSelectList && inTrueRoot && (boundValId.getType().getTypeQualifier() == NA_UNKNOWN_TYPE) &&
        (boundValId.getItemExpr()->getOperatorType() == ITM_CONSTANT)) {
      ConstValue *constItemExpr = (ConstValue *)boundValId.getItemExpr();
      if (constItemExpr->isNull()) boundValId.coerceType(NA_NUMERIC_TYPE);
    }

    switch (itemExpr->getOperatorType()) {
      case ITM_REFERENCE: {
        ColReference *colRef = (ColReference *)itemExpr;
        ColRefName *newCRN = NULL;
        if ((!colRef->getColRefNameObj().isStar()) && (colRef->isCompCol()) &&
            (colRef->getCompArrIndexes().entries() > 0) &&
          ...
          str_itoa(indexVal, tmpStr);
          newName += "_";
          newName += tmpStr;
          newCRN = new (bindWA->wHeap()) ColRefName(newName, colRef->getColRefNameObj().getCorrNameObj());
        }

        const ColRefName &colRefName = (newCRN ? *newCRN : colRef->getColRefNameObj());
        ...
        if (colRefName.isStar()) {
          ...
          BindScope *bindScope;
          ColReference cr((ColRefName *)&colRefName);
          // ColumnNameMap *xcnmEntry = bindWA->findColumn(colRefName, bindScope);
          ColumnNameMap *xcnmEntry = bindWA->findColumn(&cr, bindScope);

          if (NOT xcnmEntry)  // ## I don't recall when this case occurs...
            resultTable->addColumn(bindWA, colRefName, boundValId, colRef->getTargetColumnClass());
          else
      ...
      case ITM_RENAME_COL: {
        ...
        if (CmpCommon::getDefault(JDBC_PROCESS) == DF_ON) {
          ...
          if (childExpr->getOperatorType() == ITM_BASECOLUMN) {
            ...
            if (heading) {
              if ((strcmp(heading, "JDBC_BLOB_COLUMN -") != 0) && (strcmp(heading, "JDBC_CLOB_COLUMN -") != 0))
                heading = NULL;
            }
          }
        }
        ...
        resultTable->addColumn(bindWA, colRefName, boundValId, renameCol->getTargetColumnClass(), heading, true);
        break;
      }
      ...
      case ITM_USER_DEF_FUNCTION: {
        ...
        NABoolean isSubquery = (itemExpr->getOperatorType() == ITM_ROW_SUBQUERY) ? TRUE : FALSE;
        ...
        if ((outColVids.entries() == 1) || (isSubquery && (!allowMultiDegSubqInSelect_Enabled))) {
          ...
          if (isSubquery) {
            ...
            resultTable->addColumn(bindWA, colRefName, boundValId, colClass, heading);
          }
          break;
        }
        ...
        for (CollIndex idx = 0; idx < outColVids.entries(); idx++) {
          ...
          NABoolean isRealOrRenameColumn =
              (outColVids[idx].getItemExpr()->getOperatorType() == ITM_BASECOLUMN) ||
                      (outColVids[idx].getItemExpr()->getOperatorType() == ITM_RENAME_COL) || !isSubquery
                  ? TRUE
                  : FALSE;
          ...
          const char *heading = isRealOrRenameColumn ? col->getHeading() : "";
          ColRefName colRefName(isRealOrRenameColumn ? col->getColName() : "");
          ColumnClass colClass(isRealOrRenameColumn ? col->getColumnClass() : USER_COLUMN);
          ...
          ValueIdProxy *proxyOutput = new (CmpCommon::statementHeap()) ValueIdProxy(boundValId, outColVids[idx], idx);
          ...
          if (!isSubquery || isRealOrRenameColumn) {
            resultTable->addColumn(bindWA, colRefName, proxyOutput->getValueId(), colClass, heading);

          } else {
            resultTable->addColumn(bindWA, colRefName, proxyOutput->getValueId());
          }
  ...
  if (prevScope) {
    ...
    NABoolean inSubqueryInSelectList = subQScope->context()->inSubquery() && subQScope->context()->inSelectList();
    ...
    if (inSelectList && (inSubqueryInSelectList || inSubqueryInGroupByClause) && !bindWA->inViewExpansion()) {
      // 10-060602-6930 End
      //  We now can check for the syntax error that we've done so much work
      //  above (and in GroupByAgg::bindNode(), BindWA.h & BindWA.cpp)
      //  to detect:

      if ((j > 1) && (!allowMultiDegSubqInSelect_Enabled)) {
...
//
static ItemExpr *bindCheckConstraint(BindWA *bindWA, CheckConstraint *constraint, const NATable *naTable,
                                     NABoolean catmanCollectUsages = FALSE, ItemExpr *viewCheckPred = NULL) {
  ...
  if (viewCheckPred) {
    ...
    constraintPred =
        parser.getItemExprTree(constraint->getConstraintText().data(), constraint->getConstraintText().length(),
                               CharInfo::UTF8  // ComGetNameInterfaceCharSet()
        );
  }
  if (constraintPred) {
    ParNameLocList *saveNameLocList = bindWA->getNameLocListPtr();
    if (!catmanCollectUsages || !bindWA->getUsageParseNodePtr() ||
  ...
  if (constraintPred) {
    ...
    for (CollIndex i = 0; i < nncols.entries(); i++) {
      ...
      if (!nacol->getType()->supportsSQLnullPhysical()) {
        ...
        nacol->mutateType()->setNullable(TRUE /*supports physical nulls*/, FALSE /*but not logical nulls */);
      }
    }
  } else {
...
static ItemExpr *intersectColumns(const RETDesc &leftTable, const RETDesc &rightTable, BindWA *bindWA) {
  ...
  for (CollIndex i = 0; i < leftTable.getDegree(); i++) {
    ...
    BiRelat *compare = new (bindWA->wHeap()) BiRelat(ITM_EQUAL, leftExpr, rightExpr, TRUE);
    if (predicate)
...
static ItemExpr *joinCommonColumns(const RelExpr *const leftRelExpr, const RelExpr *const rightRelExpr,
                                   BindWA *bindWA) {
  ...
  for (CollIndex i = 0; i < leftTable.getDegree(); i++) {
    ...
    if (NOT simpleColRefName.isEmpty()) {                                     //
      ...
      if (commonCol) {                                                        //
        ...
        BiRelat *compare = new (bindWA->wHeap()) BiRelat(ITM_EQUAL, leftExpr, rightExpr);
        if (predicate)
          predicate = new (bindWA->wHeap()) BiLogic(ITM_AND, predicate, compare);
        else
          predicate = compare;
...
//
static void findNonCommonColumns(BindWA *bindWA, OperatorTypeEnum joinType, const RETDesc &sourceTable,
                                 const RETDesc &targetTable, RETDesc &resultTable, ColumnDescList &nonCommonCols) {
  ...
  for (CollIndex i = 0; i < sourceTable.getDegree(); i++) {
    ...
    if (simpleColRefName.isEmpty())
      nonCommonCols.insert(new (bindWA->wHeap()) ColumnDesc(sourceColRefName, sourceId, NULL, bindWA->wHeap()));
    else {
      ...
      //
        ...
        *CmpCommon::diags() << DgSqlCode(-4004) << DgColumnName(simpleColRefName.getColName())
                            << DgTableName(commonCol->getColRefNameObj().getCorrNameObj().getExposedNameAsAnsiString())
        ...
        switch (joinType) {
          ...
          default: {
            ...
            UnLogic *test = new (bindWA->wHeap()) UnLogic(ITM_IS_NULL, sourceExpr);
            ItemExpr *coalesce =
                new (bindWA->wHeap()) Case(NULL, new (bindWA->wHeap()) IfThenElse(test, targetExpr, sourceExpr));
...
static void coalesceCommonColumns(BindWA *bindWA, OperatorTypeEnum joinType, const RETDesc &leftTable,
                                  const RETDesc &rightTable, RETDesc &resultTable) {
  ...
  //
  findNonCommonColumns(bindWA, joinType, leftTable, rightTable, resultTable, nonCommonCols);
  ...
  findNonCommonColumns(bindWA,
                       ...
                       rightTable, leftTable, irrelevantOnThisCall, nonCommonCols);
  if (bindWA->errStatus()) return;
  //
...
//
static void BindUtil_CollectTableUsageInfo(BindWA *bindWA, const CorrName &corrName) {
  // Task (1)
  //
  ParNameLocList *pNameLocList = bindWA->getNameLocListPtr();
  if (pNameLocList) {
    ParNameLoc *pNameLoc = pNameLocList->getNameLocPtr(corrName.getNamePosition());
    if (pNameLoc) {
      if (NOT pNameLoc->getExpandedName(FALSE).isNull())
        CMPASSERT(pNameLoc->getExpandedName() == corrName.getQualifiedNameObj().getQualifiedNameAsAnsiString());
      pNameLoc->setExpandedName(corrName.getQualifiedNameObj().getQualifiedNameAsAnsiString());
    }
    //
    // Task (2)
    //
    ...
    if (pUsageParseNode) {
      ...
      } else if (pUsageParseNode->getOperatorType() == DDL_ALTER_TABLE_ADD_CONSTRAINT_CHECK) {
        StmtDDLAddConstraintCheck &node = *pUsageParseNode->castToElemDDLNode()->castToStmtDDLAddConstraintCheck();
...
void castComputedColumnsToAnsiTypes(BindWA *bindWA, RETDesc *rd, ValueIdList &compExpr) {
  ...
  while (i--) {
    ...
    if (col->isRenameCol()) col->setOrigValueId(col->getValueId());
    ...
    if (!naType->isSupportedType()) {
      ...
      ItemExpr *theRepeat =
          new (bindWA->wHeap()) Repeat(new (bindWA->wHeap()) SystemLiteral("#"),
                                       new (bindWA->wHeap()) SystemLiteral(naType->getDisplayLength(
                                           naType->getFSDatatype(), 0, naType->getPrecision(), naType->getScale(), 0)));
      theRepeat = theRepeat->bindNode(bindWA);
      col->setValueId(theRepeat->getValueId());
      compExpr[i] = theRepeat->getValueId();
    } else if ((CmpCommon::getDefault(MODE_SPECIAL_1) == DF_ON) && (NOT bindWA->inViewDefinition()) &&
               (NOT bindWA->inMVDefinition()) && (NOT bindWA->inCTAS()) &&
               (naType->getTypeQualifier() == NA_DATETIME_TYPE &&
                ((const DatetimeType *)naType)->getSubtype() == DatetimeType::SUBTYPE_SQLDate) &&
               (!CmpCommon::context()->getSqlmxRegress()) &&
               (strcmp(ActiveSchemaDB()->getDefaults().getValue(OUTPUT_DATE_FORMAT),
                       "ANSI") != 0)) {  // Special1 DATE, return as YY/MM/DD
      ItemExpr *newChild = new (bindWA->wHeap()) Format(col->getValueId().getItemExpr(), "YY/MM/DD", FALSE);
    ...
    if ((naType->getFSDatatype() == REC_BIN64_UNSIGNED) &&
        (CmpCommon::getDefault(TRAF_LARGEINT_UNSIGNED_IO) == DF_OFF) && (NOT bindWA->inCTAS()) &&
      ...
      ItemExpr *cast = new (bindWA->wHeap())
          Cast(col->getValueId().getItemExpr(),
               new (bindWA->wHeap()) SQLBigNum(bindWA->wHeap(), MAX_HARDWARE_SUPPORTED_UNSIGNED_NUMERIC_PRECISION,
                                               nTyp->getScale(), FALSE, FALSE, naType->supportsSQLnull()));
    ...
    if ((naType->getFSDatatype() == REC_BOOLEAN) && (CmpCommon::getDefault(TRAF_BOOLEAN_IO) == DF_OFF) &&
        (NOT bindWA->inCTAS()) && (NOT bindWA->inViewDefinition())) {
      NumericType *nTyp = (NumericType *)naType;

      ItemExpr *cast = new (bindWA->wHeap())
          Cast(col->getValueId().getItemExpr(),
               new (bindWA->wHeap()) SQLChar(bindWA->wHeap(), SQL_BOOLEAN_DISPLAY_SIZE, naType->supportsSQLnull()));
    ...
    if ((DFS2REC::isBinaryString(naType->getFSDatatype())) && (CmpCommon::getDefault(TRAF_BINARY_OUTPUT) == DF_OFF) &&
        (!Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL)) && (NOT bindWA->inCTAS()) &&
        (NOT bindWA->inViewDefinition())) {
      ItemExpr *cast = NULL;
      cast = new (bindWA->wHeap()) ConvertHex(ITM_CONVERTTOHEX, col->getValueId().getItemExpr());
    ...
    if (((naType->getFSDatatype() == REC_BIN8_SIGNED) || (naType->getFSDatatype() == REC_BIN8_UNSIGNED)) &&
        (NOT bindWA->inCTAS()) && (NOT bindWA->inViewDefinition()) &&
      ...
      if (srcNum->getScale() == 0)
        newType = new (bindWA->wHeap()) SQLSmall(bindWA->wHeap(), NOT srcNum->isUnsigned(), naType->supportsSQLnull());
      else
        newType =
            new (bindWA->wHeap()) SQLNumeric(bindWA->wHeap(), sizeof(short), srcNum->getPrecision(), srcNum->getScale(),
                                             NOT srcNum->isUnsigned(), naType->supportsSQLnull());

      ItemExpr *cast = new (bindWA->wHeap()) Cast(col->getValueId().getItemExpr(), newType);
      ...
      if (CmpCommon::getDefault(BIGNUM_IO) == DF_ON)
        ...
        if ((nTyp->isBigNum()) && (((SQLBigNum *)nTyp)->isARealBigNum())) bignumIO = TRUE;
      }
      ...
      if (bignumIO)
        bignumOflow = nTyp->getPrecision() - (int)CmpCommon::getDefaultNumeric(MAX_NUMERIC_PRECISION_ALLOWED);
      else {
        if (nTyp->isSigned())
      ...
      if ((bignumOflow > 0) || (oflow > 0)) {
        if (bignumOflow > 0) {
          newPrec = (int)CmpCommon::getDefaultNumeric(MAX_NUMERIC_PRECISION_ALLOWED);
          int orgMagnitude = nTyp->getPrecision() - nTyp->getScale();
          ...
          if (newScale > newPrec) {
            *CmpCommon::diags() << DgSqlCode(-3015) << DgInt0(newScale) << DgInt1(newPrec);
          ...
          newTyp = new (bindWA->wHeap())
              SQLBigNum(bindWA->wHeap(), newPrec, newScale, ((SQLBigNum &)col->getValueId().getType()).isARealBigNum(),
          ...
          int orgMagnitude = (nTyp->getMagnitude() + 9) / 10;
          // set the newScale
          // IF there is overflow in magnitude set the scale to 0.
          // ELSE set the accomodate the magnitude part and truncate the scale
          newScale = (orgMagnitude >= newPrec) ? 0 : newPrec - orgMagnitude;
          ...
          int minScale = defs.getAsLong(PRESERVE_MIN_SCALE);
          newScale = MAXOF(minScale, newScale);

          if (op == ITM_SUM || op == ITM_AVG) {
            // AVG = DIVIDE( SUM(), COUNT() )
            ItemExpr *tmp = (op == ITM_SUM) ? ie2 : ie2->child(0).getPtr();
            ...
            //
            if (tmp->getOperatorType() == ITM_NOTCOVERED) tmp = (Aggregate *)(ItemExpr *)tmp->child(0);

            CMPASSERT(tmp->isAnAggregate());
            Aggregate *sum = (Aggregate *)tmp;
            ItemExpr *arg = (sum->getOriginalChild()) ? sum->getOriginalChild() : sum->child(0).getPtr();
            if (arg->getValueId() == NULL_VALUE_ID) arg = sum->child(0).getPtr();
            CMPASSERT(arg->getValueId() != NULL_VALUE_ID);
            int needScale = arg->getValueId().getType().getScale();
            if (needScale > newPrec) needScale = newPrec;
        ...
        ItemExpr *cast = new (bindWA->wHeap()) Cast(ie, newTyp, ITM_CAST, TRUE /*checkForTrunc*/);
        cast = cast->bindNode(bindWA);
        if (bindWA->errStatus()) return;
...
void castComputedColumnsToAnsiTypes2(BindWA *bindWA, RETDesc *rd, ValueIdList &compExpr) {
  if (!rd) return;
  ...
  while (i--) {
    ...
    if ((naType->isComposite()) && (NOT bindWA->inCTAS()) && (NOT bindWA->inViewDefinition())) {
      ItemExpr *compDisp = NULL;
      compDisp = new (bindWA->wHeap()) CompositeDisplay(col->getValueId().getItemExpr());

      compDisp = compDisp->bindNode(bindWA);
      if (bindWA->errStatus()) return;
      col->setValueId(compDisp->getValueId());
      compExpr[i] = compDisp->getValueId();

      naType = (NAType *)&compDisp->getValueId().getType();
    } else
      continue;
  }  // loop over cols in RETDesc
}  // castComputedColumnsToAnsiTypes()
...
TrafDesc *generateSpecialDesc(const CorrName &corrName) {
  ...
  if (corrName.getSpecialType() == ExtendedQualName::VIRTUAL_TABLE) {
    ...
    } else if (corrName.getQualifiedNameObj().getObjectName() == QueryInvalidationFunc::getVirtualTableNameStr()) {
      QueryInvalidationFunc qif;
      desc = qif.createVirtualTableDesc();
    ...
    } else if (corrName.getQualifiedNameObj().getObjectName() ==
               ExeUtilRegionStats::getVirtualTableClusterViewNameStr()) {
...
NARoutine *BindWA::getNARoutine(const QualifiedName &name) {
  ...
  if (!naRoutine) {
    ...
    udfMetadata = cmpSBD.getSeabaseRoutineDesc(name.getCatalogName(), name.getSchemaName(), name.getObjectName());
    if (!udfMetadata) return NULL;
    ...
    if (getSchemaDB()->getNARoutineDB()->cachingMetaData()) getSchemaDB()->getNARoutineDB()->put(naRoutine);
  }
  return naRoutine;
}
...
NATable *BindWA::getNATable(CorrName &corrName,
                            ...
                            NABoolean writeReference)            // default FALSE
{
  BindWA *bindWA = this;  // for coding convenience

  // check if objects locked by Backup/Restore should be returned.
  brLockedObjectsAccess_ = CmpCommon::getDefaultNumeric(TRAF_BR_LOCKED_OBJECTS_ACCESS);
  if (brLockedObjectsAccess_ != 0) {
    short brLockedObjectsQueryType = CmpCommon::getDefaultNumeric(TRAF_BR_LOCKED_OBJECTS_QUERY_TYPE);
    if ((bindWA->inDDL() && (brLockedObjectsQueryType < 2)) ||
        ((bindWA->getTopRoot() && bindWA->getTopRoot()->childOperType().match(REL_ANY_GEN_UPDATE)) &&
         (brLockedObjectsQueryType < 1)))
      brLockedObjectsAccess_ = 0;
  }
  ...
  NAString userName;
  if ((CmpCommon::context()->sqlSession()->volatileSchemaInUse()) && (!inTableDescStruct) &&
      (corrName.getSpecialType() != ExtendedQualName::VIRTUAL_TABLE) &&
      (corrName.getSpecialType() != ExtendedQualName::HBMAP_TABLE)) {
    CorrName newCorrName = CmpCommon::context()->sqlSession()->getVolatileCorrName(corrName);
    if (bindWA->errStatus()) goto error_return;

    // get NATable from cache
    table = bindWA->getSchemaDB()->getNATableDB()->get(newCorrName, bindWA, inTableDescStruct, writeReference);
    if (!table) {
      ...
      NABoolean isValid = CmpCommon::context()->sqlSession()->validateVolatileCorrName(corrName);

      // if this table is found in volatile schema, then
      // make sure it is a volatile table.
      if ((isValid) && (NOT table->isVolatileTable())) {
        *CmpCommon::diags() << DgSqlCode(-4190)
                            << DgTableName(table->getTableName().getQualifiedNameAsAnsiString(TRUE));
  ...
  if (!table) {
    ...
    NABoolean catNameSpecified = (NOT corrName.getQualifiedNameObj().getCatalogName().isNull());
    NABoolean schNameSpecified = (NOT corrName.getQualifiedNameObj().getSchemaName().isNull());
    ...
    if (bindWA->errStatus()) goto error_return;  // prototype value parse error

    // cannot use hbase map schema as table name
    if ((corrName.getQualifiedNameObj().getSchemaName() == HBASE_EXT_MAP_SCHEMA) &&
        (!Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL)) && (!Get_SqlParser_Flags(ALLOW_SPECIALTABLETYPE))) {
      *CmpCommon::diags() << DgSqlCode(-4261) << DgSchemaName(corrName.getQualifiedNameObj().getSchemaName());
    ...
    if ((corrName.getSpecialType() == ExtendedQualName::HBMAP_TABLE) && (NOT schNameSpecified)) {
    ...
    {
      if (violateAccessDefaultSchemaOnly(corrName.getQualifiedNameObj())) goto error_return;
    }

    // make sure that schema name is not a VOLATILE SCHEMA
    if ((!bindWA->inDDL()) || ((bindWA->inViewDefinition()) || (bindWA->inMVDefinition()))) {
      // for Histogram, support to use VOLATILE SCHEMA
      // or else, don't support
      if (!corrName.getQualifiedNameObj().isHistogramTable() &&
          !CmpCommon::context()->sqlSession()->validateVolatileQualifiedSchemaName(corrName.getQualifiedNameObj())) {
        bindWA->setErrStatus();
        goto error_return;
      }
    }
    ...
    table = bindWA->getSchemaDB()->getNATableDB()->get(corrName, bindWA, inTableDescStruct, writeReference);
    ...
    if (!table && !pubSchemaIntName.isNull()) {
      ...
      if (!pubSchema.getCatalogNamePart().isEmpty()) {
        pCorrName.getQualifiedNameObj().setCatalogName(pubSchema.getCatalogNamePart().getInternalName());
      }

      bindWA->resetErrStatus();
      table = bindWA->getSchemaDB()->getNATableDB()->get(pCorrName, bindWA, inTableDescStruct, writeReference);
      if (!bindWA->errStatus() && table) {  // if found in public schema, do not show previous error
        // and replace corrName
        CmpCommon::diags()->clear();
        corrName.getQualifiedNameObj().setCatalogName(pCorrName.getQualifiedNameObj().getCatalogName());
        corrName.getQualifiedNameObj().setSchemaName(pCorrName.getQualifiedNameObj().getSchemaName());
      }
    }
    ...
    if (!table && !schNameSpecified) {
      ...
      table = bindWA->getSchemaDB()->getNATableDB()->get(pCorrName, bindWA, inTableDescStruct, writeReference);
      if (!bindWA->errStatus() && table) {  // if found in mapped schema, do not show previous error
        // and replace corrName
        CmpCommon::diags()->clear();
        corrName.getQualifiedNameObj().setCatalogName(pCorrName.getQualifiedNameObj().getCatalogName());
        corrName.getQualifiedNameObj().setSchemaName(pCorrName.getQualifiedNameObj().getSchemaName());
    ...
    if (catmanCollectTableUsages)
      if (corrName.getSpecialType() != ExtendedQualName::TRIGTEMP_TABLE) {
        if ((bindWA->inViewDefinition()) && (corrName.isHbaseMap())) {
    ...
    if (!table) {
      if (volatileTableFound) {
        if ((CmpCommon::diags()->mainSQLCODE() == -1003) && (NOT catNameSpecified)) {
          ...
          CmpCommon::diags()->clear();
          CmpCommon::context()->sqlSession()->validateVolatileCorrName(corrName);
          bindWA->setErrStatus();
        }
      }
  ...
  if ((table) && (table->isTrafExternalTable() && (NOT table->getTableName().isHbaseMappedName()) &&
                  (!bindWA->allowExternalTables())))
  //(! Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL))))
  {
    *CmpCommon::diags() << DgSqlCode(-4258) << DgTableName(table->getTableName().getQualifiedNameAsAnsiString());
  ...
  if ((table) && table->isTrafExternalTable() && (NOT table->getTableName().isHbaseMappedName())) {
    NAString adjustedName = ComConvertTrafNameToNativeName(
        table->getTableName().getCatalogName(), table->getTableName().getUnqualifiedSchemaNameAsAnsiString(),
    ...
    NATable *nativeNATable =
        bindWA->getSchemaDB()->getNATableDB()->get(externalCorrName, bindWA, inTableDescStruct, writeReference);
    if ((bindWA->externalTableDrop()) && (bindWA->errStatus())) {
      goto error_return;
    }

    // Compare native lists (user columns only) and external table definitions.
    ...
    if (nativeNATable && !(table->getNAColumnArray().compare(nativeNATable->getNAColumnArray(), FALSE)) &&
      ...
      if (table->getNAColumnArray().entries() != nativeNATable->getNAColumnArray().entries()) compError = TRUE;
      if ((NOT compError) && (NOT table->hiveExtColAttrs()) && (NOT table->hiveExtKeyAttrs())) {
        if (NOT(table->getNAColumnArray() == nativeNATable->getNAColumnArray())) compError = TRUE;
      }
      if (compError) {
        *CmpCommon::diags() << DgSqlCode(-3078) << DgString0(adjustedName)
...

  {
    ...
    if (proto && proto->isPrototypeValid()) corrName.getPrototype()->bindNode(bindWA);
  }

  // This test is not "inAnyConstraint()" because we DO want to increment
  // the count for View With Check Option constraints.
  if (!getCurrentScope()->context()->inTableCheckConstraint() && !getCurrentScope()->context()->inRIConstraint())
...
NATable *BindWA::getNATableInternal(CorrName &corrName,
                                    NABoolean catmanCollectTableUsages,  // default TRUE
                                    TrafDesc *inTableDescStruct,         // default NULL
  ...
  if (extTableDrop) setExternalTableDrop(TRUE);

  NATable *nat = getNATable(corrName, catmanCollectTableUsages, inTableDescStruct);
...
static TableDesc *createTableDesc2(BindWA *bindWA, const NATable *naTable, CorrName &corrName, Hint *hint) {
  ...
  for (i = 0; i < naTable->getIndexList().entries(); i++) {
    ...
    IndexDesc *idesc = new (bindWA->wHeap()) IndexDesc(tdesc, nfs, bindWA->currentCmpContext());

    if (naTable->getClusteringIndex()->getFileSetName() == idesc->getIndexName()) {
      tdesc->setClusteringIndex(idesc);
      idesc->markAsClusteringIndex();
    }

    if (indexChoice.compareTo("NONE") == 0 OR indexChoice.compareTo("VERTICAL") ==
        0 OR(indexChoice.compareTo("KEYINDEXES") == 0 AND tdesc->isKeyIndex(idesc)) OR naTable->getClusteringIndex()
            ->getFileSetName() == nfs->getFileSetName()) {
      tdesc->addIndex(idesc);
      // implementation of optimizer hints
      if (hint AND hint->hasIndexHint(idesc->getNAFileSet()->getExtFileSetName())) {
        tdesc->addHintIndex(idesc);
      }
      if (idesc->isUniqueIndex()) tdesc->addUniqueIndex(idesc);
    } else {
      delete idesc;
    }
  }

  if (hint AND hint->indexCnt() > tdesc->getHintIndexes().entries() AND CmpCommon::getDefault(INDEX_HINT_WARNINGS) !=
      DF_OFF) {
    // emit a warning that we didn't process all the index hints,
    // there is probably a spelling mistake
    *CmpCommon::diags() << DgSqlCode(4371) << DgInt0(tdesc->getHintIndexes().entries()) << DgInt1(hint->indexCnt())
                        << DgTableName(naTable->getTableName().getQualifiedNameAsAnsiString());
  }
  ...
  for (i = 0; i < naTable->getVerticalPartitionList().entries(); i++) {
    if (indexChoice.compareTo("NONE") == 0 OR indexChoice.compareTo("INDEXES") ==
        0 OR indexChoice.compareTo("KEYINDEXES") == 0) {
      IndexDesc *idesc =
          new (bindWA->wHeap()) IndexDesc(tdesc, naTable->getVerticalPartitionList()[i], bindWA->currentCmpContext());
  ...
  //
  bindWA->getCurrentScope()->setRETDesc(new (bindWA->wHeap()) RETDesc(bindWA, tdesc));
  ...
  if (!bindWA->getCurrentScope()->context()->inAnyConstraint())
    bindWA->tableViewUsageList().insert(new (bindWA->wHeap()) TableViewUsage(
        tdesc->getCorrNameObj().getQualifiedNameObj(), tdesc->getCorrNameObj().getSpecialType(),
        naTable->getViewText() != NULL, bindWA->viewCount()));

  return tdesc;

}  // static createTableDesc2()

TableDesc *BindWA::createTableDesc(const NATable *naTable, CorrName &corrName, NABoolean catmanCollectUsages,
                                   Hint *hint) {
  BindWA *bindWA = this;  // for coding convenience
  ...
  // now bind any table partition bounday and attach them to our new tdesc.
  if (naTable->isPartitionV2Table()) {
    ...
    tdesc->addPartitions(bindWA, naTable, partList);
    tdesc->genPartitions(bindWA, partList);
  } else if (naTable->isPartitionEntityTable()) {
    const NAPartitionArray &partList = naTable->getPartitionArray();
    // TODO: error when entries is 0
    /*
    if (partList.entries() != 0)
    {
      // TODO: ERROR
    ...
    */
    if (partList.entries() > 0) {
      ...
      tdesc->addPartitions(bindWA, naTable, partList, FALSE);
      tdesc->genPartitions(bindWA, partList);
      tdesc->addPartnPredToConstraint(bindWA);
    }
  }
  ...
  if (ccl.entries()) {
    ...
    for (CollIndex i = 0; i < ccl.entries(); i++) {
      ItemExpr *constraintPred = bindCheckConstraint(bindWA, ccl[i], naTable, catmanCollectUsages);
      if (constraintPred)
  ...
  for (CollIndex c = 0; c < naTable->getColumnCount(); c++) {
    ...
    if (nac->isComputedColumn()) {
      ...
      computedColumnExpr = parser.getItemExprTree(nac->getComputedColumnExprString(),
                                                  str_len(nac->getComputedColumnExprString()), CharInfo::UTF8);
...
static void propagateDeleteAndStream(RelExpr *re, GroupAttributes *ga) {
  if (ga->isEmbeddedUpdateOrDelete()) re->getGroupAttr()->setEmbeddedIUD(ga->getEmbeddedIUD());

  if (ga->isStream()) re->getGroupAttr()->setStream(TRUE);
...
RelExpr *BindWA::bindView(const CorrName &viewName, const NATable *naTable, const StmtLevelAccessOptions &accessOptions,
                          ItemExpr *predicate, GroupAttributes *groupAttrs, NABoolean catmanCollectUsages) {
  BindWA *bindWA = this;  // for coding convenience
  ...
  if (viewName.isHive()) {
    SchemaName s(viewName.getQualifiedNameObj().getSchemaName(), viewName.getQualifiedNameObj().getCatalogName());
  ...
  if (CmpCommon::getDefault(ALLOW_ISOLATION_LEVEL_IN_CREATE_VIEW) == DF_OFF) {
    ...
    NAString op("ON");
    ActiveSchemaDB()->getDefaults().validateAndInsert("ALLOW_ISOLATION_LEVEL_IN_CREATE_VIEW", op, FALSE);
  }
  if (CmpCommon::getDefault(ALLOW_ORDER_BY_IN_CREATE_VIEW) == DF_OFF) {
    allowOrderByWasSet = TRUE;

    NAString op("ON");
    ActiveSchemaDB()->getDefaults().validateAndInsert("ALLOW_ORDER_BY_IN_CREATE_VIEW", op, FALSE);
  }

  NABoolean useHiveIndexingWasSet = FALSE;
  if ((viewName.isHive()) && (CmpCommon::getDefault(HIVE_ARRAY_INDEX_MODE) == DF_OFF)) {
    useHiveIndexingWasSet = TRUE;

    NAString op("ON");
    ActiveSchemaDB()->getDefaults().validateAndInsert("HIVE_ARRAY_INDEX_MODE", op, FALSE);
  }

  Parser parser(bindWA->currentCmpContext());
  ExprNode *viewTree = parser.parseDML(naTable->getViewText(), naTable->getViewLen(), naTable->getViewTextCharSet());
  ...
  if (NOT viewTree) {
    ...
    if (defSchWasChanged) bindWA->setDefaultSchema(savedSch);
  ...
  StmtDDLCreateView *createViewTree =
      ((DDLExpr *)(queryTree->getChild(0)))->getDDLNode()->castToStmtDDLNode()->castToStmtDDLCreateView();
  ...
  if (naTable->getViewCheck() && bindWA->getCurrentScope()->context()->inUpdateOrInsert() &&
  ...
  if ((groupAttrs) && (groupAttrs->isEmbeddedUpdateOrDelete() || groupAttrs->isStream()))
  ...
  bindWA->decrNumTablesPerSelect();
  bindWA->decrNumTablesPerQuery();
  ...
  if (viewRoot->hasOrderBy()) {
    RelExpr *sortNode =
        new (bindWA->wHeap()) SortLogical(queryTree->child(0)->castToRelExpr(), viewRoot->reqdOrder(), bindWA->wHeap());
  ...
  ItemExpr *columnList = new (bindWA->wHeap())
      RenameCol(NULL, new (bindWA->wHeap()) ColRefName(columns[0]->getColName(), bindWA->wHeap()));
  //
  CollIndex i = 1;
  for (i = 1; i < naTable->getColumnCount(); i++)
    columnList = new (bindWA->wHeap())
        ItemList(columnList, new (bindWA->wHeap()) RenameCol(
                                 NULL, new (bindWA->wHeap()) ColRefName(columns[i]->getColName(), bindWA->wHeap())));
  //

  queryTree = new (bindWA->wHeap()) RenameTable(TRUE /*copy tableName as is*/, queryTree->castToRelExpr(), viewName,
                                                columnList, bindWA->wHeap(), TRUE /*isView*/);
  if (predicate) queryTree->addSelPredTree(predicate);
  ((RenameTable *)queryTree)->setViewNATable(naTable);

  // this query used this view
  appendViewName(viewName.getQualifiedNameObj().getQualifiedNameAsAnsiString().data());
  ...
  CMPASSERT(columns.entries() == naTable->getColumnCount() && columns.entries() == columnsRET.entries());
  for (i = 0; i < naTable->getColumnCount(); i++) {
  ...
  if (topmostViewWithCheckOption) {
    ...
    if (bindWA->predsOfViewWithCheckOption().entries()) {
      constraint = new (bindWA->wHeap()) CheckConstraint(viewName.getQualifiedNameObj(),  // this view name
    ...
    if (constraint) {
      ...
      ItemExpr *constraintPred = bindCheckConstraint(bindWA, constraint, naTable, catmanCollectUsages, viewCheckPred);
      if (constraintPred)
        queryTree->getScanNode()->getTableDesc()->addCheckConstraint(bindWA,
...
// helper function for BindWA::validateUpdateStatisticsExpression
static ItemExpr *findUndesireables(ItemExpr *ie, CollHeap *outHeap, void *context) {
  ValueIdSet *out = (ValueIdSet *)context;
  ...
  switch (op) {
    ...
    // UDF calls (scalar; table-valued will be caught by
    // the subqueries check above)
    case ITM_USER_DEF_FUNCTION: {
      out->insert(ie->getValueId());
      break;
    }
    ...
    case ITM_SUM:
    case ITM_COUNT:         // COUNT(*)
    case ITM_COUNT_NONULL:  // COUNT(A) count non-null A's
    ...
    default: {
      // if the expression has LOB datatype, forbid it also
      const NAType &type = ie->getValueId().getType();
      if (type.getTypeQualifier() == NA_LOB_TYPE) out->insert(ie->getValueId());
      break;
    }
  }
...
short BindWA::validateUpdateStatisticsExpression(RelExpr *node, const NAType **expressionType /* out */,
                                                 NAString &unparsedExpr /* out */) {
  // This method validates expressions specified in UPDATE STATISTICS.
  // UPDATE STATISTICS passes a bound tree of the statement,
  // "SELECT <expression> FROM T". So the expression tree to be
  // validated is in the select list of the root node.
  //
  // This method checks that:
  // 1. There is only one expression in the select list
  // 2. There are no forbidden operators in the tree (as defined
  // by the findUndesireables function above)

  // Note: If we want to support multi-column histograms on expressions
  // in the future, we'll want to delete the "one expression" check
  // below, and also change the non-deterministic function check to
  // loop over all the expressions in the RETDesc (instead of assuming
  // there is just one).

  short rc = 0;
  *expressionType = NULL;  // assume nothing to return
  unparsedExpr.clear();    // assume nothing to return

  CMPASSERT(node->getOperatorType() == REL_ROOT);

  // check that there is only one expression in the select list

  RETDesc *retDesc = node->getRETDesc();
  if (retDesc->getDegree() != 1) {
    *CmpCommon::diags() << DgSqlCode(-4186);
    rc = -1;
  }

  // check for known non-deterministic functions, UDFs or subqueries

  ValueId vid = retDesc->getValueId(0);  // there's just one expression for now
  ItemExpr *ie = vid.getItemExpr();
  ValueIdSet undesireables;
  ie = ie->treeWalk(&findUndesireables, NULL, ITM_POSTFIX_WALK, (void *)&undesireables);
  if (undesireables.entries() > 0) {
    *CmpCommon::diags() << DgSqlCode(-4187);
    rc = -1;
  } else {
    *expressionType = &(ie->getValueId().getType());
    ie->unparse(unparsedExpr, DEFAULT_PHASE, USTAT_EXPRESSION_FORMAT);
  }

  return rc;
}  // BindWA::validateUpdateStatisticsExpression()
...
void RelExpr::bindChildren(BindWA *bindWA) {
  ...
  if (getInliningInfo().isTriggerRoot()) getInliningInfo().getTriggerObject()->incRecursionCounter();
  ...
  if (getInliningInfo().isTriggerRoot()) getInliningInfo().getTriggerObject()->decRecursionCounter();
  ...
  for (int j = 0; j < getArity(); j++) {
    if (child(j)) {
      if (child(j)->getGroupAttr()->isStream()) {
        ...
        if (child(j)->getGroupAttr()->isSkipInitialScan()) getGroupAttr()->setSkipInitialScan(TRUE);
      }

      if (child(j)->getGroupAttr()->isEmbeddedUpdateOrDelete() || child(j)->getGroupAttr()->isEmbeddedInsert())
        getGroupAttr()->setEmbeddedIUD(child(j)->getGroupAttr()->getEmbeddedIUD());

      if (child(j)->getGroupAttr()->reorderNeeded()) getGroupAttr()->setReorderNeeded(TRUE);
    }
  }
}
...
RelExpr *RelExpr::bindSelf(BindWA *bindWA) {
  ...
  //
  if (NOT getGroupAttr()) setGroupAttr(new (bindWA->wHeap()) GroupAttributes);
  ...
  if (predTree) {
    ...
    if (bindWA->inViewWithCheckOption()) bindWA->predsOfViewWithCheckOption() += selectionPred();
  }

  predTree = getBiConnectBy();
  if (predTree) {
    predTree = ((BiConnectBy *)predTree)->getConnectBy();
    predTree->bindPriorItem(bindWA);
  }
  ...
  if (uniqueColumnsTree) {
    uniqueColumnsTree->convertToValueIdSet(getUniqueColumns(), bindWA, ITM_ITEM_LIST);
    if (bindWA->errStatus()) return this;
  }
  // --MV
...
Scan *RelExpr::getScanNode(NABoolean assertExactlyOneScanNode) const {
  ...
  while (result) {
    if ((result->getOperatorType() == REL_SCAN) || (result->getOperatorType() == REL_HBASE_ACCESS)) break;
...
RelExpr *RelExpr::getViewScanNode(NABoolean isTopLevelUpdateInView) const {
  ...
  while (result) {
    ...
    if (result->getOperatorType() == REL_RENAME_TABLE && ((RenameTable *)result)->isView()) break;
...
RelExpr *Join::bindNode(BindWA *bindWA) {
  ...
  if ((getOperatorType() == REL_FULL_JOIN) && (CmpCommon::getDefault(COMP_BOOL_192) == DF_ON)) {
    ...
    RelExpr *rename = new (bindWA->wHeap()) RenameTable(antiJoin, leftName);
    ...
    ItemExpr *filter = new (bindWA->wHeap()) UnLogic(ITM_IS_NULL, nullCheck);
  ...
  if (getInliningInfo().isDrivingMvLogInsert()) {
    ...
    if (NULL != rightSideIUD) {
      ...
      if (ExtendedQualName::IUD_LOG_TABLE == table->getSpecialType()) {
        ...
        updateSelectValueIdMap_ = new (bindWA->wHeap()) ValueIdMap(rightSideIUD->updateToSelectMap());
      }
    }
  }
  ...
  if ((getOperatorType() == REL_FULL_JOIN && (CmpCommon::getDefault(COMP_BOOL_199) == DF_OFF)) ||  // OR
      (getOperatorType() == REL_UNION_JOIN)) {
    // 3022 Feature not yet supported
    *CmpCommon::diags() << DgSqlCode(-3022)
                        << DgString0((getOperatorType() == REL_FULL_JOIN) ? "FULL OUTER JOIN" : "UNION JOIN");
    bindWA->setErrStatus();
    return this;
  }
  ...
  // if this is a lateral view join,
  // add a predicate joining key columns from left and right operands.
  if (compositeUnnestJoin()) {
    // see sqlparser.y, right child of this join is compositeUnnest
    ...
    for (int i = 0; i < vidList.entries(); i++) {
      ...
      if (ie->getOperatorType() == ITM_REFERENCE)
        colName = ((ColReference *)ie)->getColRefNameObj().getColNameAsAnsiString();
      else if (ie->getOperatorType() == ITM_BASECOLUMN)
        colName = ((BaseColumn *)ie)->getColName();
      else
        CMPASSERT(0);

      CorrName leftCorrName(getLeftMostCorrName(), bindWA->wHeap());
      ColRefName *leftColRefName = new (bindWA->wHeap()) ColRefName(colName, leftCorrName);
      ColReference *leftColRef = new (bindWA->wHeap()) ColReference(leftColRefName);

      CorrName rightCorrName(comp->getVirtTableNameStr(), bindWA->wHeap());
      ColRefName *rightColRefName = new (bindWA->wHeap()) ColRefName(colName, rightCorrName);
      ColReference *rightColRef = new (bindWA->wHeap()) ColReference(rightColRefName);

      ItemExpr *pred = new (bindWA->wHeap()) BiRelat(ITM_EQUAL, leftColRef, rightColRef);

      if (predTree == NULL)
        predTree = pred;
      else {
        predTree = new (bindWA->wHeap()) BiLogic(ITM_AND, predTree, pred);
      }
    }  // for

    if (predTree) {
      addJoinPredTree(predTree);
    }
  }  // process lateral view join
  ...
  switch (getOperatorType()) {
    ...
    case REL_UNION_JOIN: {
      ...
      if (CmpCommon::getDefault(COMP_BOOL_198) == DF_OFF)  // don't enable FOJ transformation
      {
        ...
        for (index = 0; index < nullInstantiatedForRightJoinOutput().entries(); index++) {
  ...
  if (NOT isNaturalJoin()) {
    ...
    coalesceCommonColumns(bindWA, getOperatorType(), *leftTable, *rightTable, *resultTable);
    if (bindWA->errStatus()) return this;
  }
  ...
  if ((child(0)->getGroupAttr()->isStream()) && (child(1)->getGroupAttr()->isStream())) {
    bindWA->getTablesInScope(xtnmList, &fmtdList);
    *CmpCommon::diags() << DgSqlCode(-4158) << DgString0(fmtdList);
    bindWA->setErrStatus();
    return this;
  }
  ...
  //
  if (getGroupAttr()->isEmbeddedInsert() && !isTSJForWrite()  // the tsjForWrite flag is set for
    ...
    *CmpCommon::diags() << DgSqlCode(-4336) << DgString0(fmtdList)
  ...
  if (((child(0)->getGroupAttr()->isEmbeddedUpdateOrDelete()) &&
      ...
      ((child(0)->getGroupAttr()->isEmbeddedInsert()) && (child(1)->getGroupAttr()->isEmbeddedInsert())) ||
    ...
    *CmpCommon::diags() << DgSqlCode(-4175) << DgString0(fmtdList) << DgString1(type0) << DgString2(type1);
    bindWA->setErrStatus();
    return this;
  }

  if ((child(0)->getGroupAttr()->isEmbeddedUpdateOrDelete() || child(0)->getGroupAttr()->isStream()) &&
      (child(1)->getGroupAttr()->isEmbeddedUpdateOrDelete() || child(1)->getGroupAttr()->isStream())) {
    *CmpCommon::diags() << DgSqlCode(-4176) << DgString0(fmtdList)
                        << (getGroupAttr()->isEmbeddedUpdate() ? DgString1("UPDATE") : DgString1("DELETE"));
    bindWA->setErrStatus();
    return this;
  }
  ...
  if (getOperatorType() == REL_LEFT_JOIN) {
    if (child(1)->getGroupAttr()->isEmbeddedUpdateOrDelete()) {
      *CmpCommon::diags() << DgSqlCode(-4156) << DgString0(fmtdList)
                          << (child(1)->getGroupAttr()->isEmbeddedUpdate() ? DgString1("UPDATE") : DgString1("DELETE"));
      bindWA->setErrStatus();
      return this;
    }
    if (child(1)->getGroupAttr()->isStream()) {
      *CmpCommon::diags() << DgSqlCode(-4157) << DgString0(fmtdList);
      bindWA->setErrStatus();
      return this;
    }
  }
  ...
  if (getOperatorType() == REL_RIGHT_JOIN) {
    if (child(0)->getGroupAttr()->isEmbeddedUpdateOrDelete()) {
      *CmpCommon::diags() << DgSqlCode(-4164) << DgString0(fmtdList)
                          << (child(0)->getGroupAttr()->isEmbeddedUpdate() ? DgString1("UPDATE") : DgString1("DELETE"));
      bindWA->setErrStatus();
      return this;
    }
    if (child(0)->getGroupAttr()->isStream()) {
      *CmpCommon::diags() << DgSqlCode(-4165) << DgString0(fmtdList);
      bindWA->setErrStatus();
      return this;
    }
  }
  ...
  if (child(1)->getGroupAttr()->isStream() || child(1)->getGroupAttr()->isEmbeddedUpdateOrDelete()) {
...
ValueId Join::addNullInstIndicatorVar(BindWA *bindWA, ItemExpr *indicatorVal) {
  ...
  if (!cval) cval = new (bindWA->wHeap()) SystemLiteral(1);
  cval = cval->bindNode(bindWA);
  if (bindWA->errStatus()) return NULL_VALUE_ID;
...
ItemExpr *Join::buildDefaultOrderByForRownum(BindWA *bindWA) {
  ...
  /* Oracle is always use columns in left table as first sort keys, even it's RIGHT joins
  if (getOperator().match(REL_ANY_RIGHT_JOIN))
      childIdx = 1;
  else*/
  childIdx = 0;

  // gather order of first child
  RelExpr *subrel = child(childIdx)->castToRelExpr();
  if (subrel->getOperatorType() == REL_SCAN) {
    CorrName &corrName = ((Scan *)subrel)->getTableName();
    NATable *tab = bindWA->getNATable(corrName);
    if (!bindWA->errStatus()) {
      result1 = BinderUtils::buildSyskeyOrPKExpr(tab, corrName, bindWA->wHeap());
    }
  } else if (subrel->getOperatorType() == REL_RENAME_TABLE) {
    if (subrel->child(0)->getOperatorType() == REL_ROOT) {
      RenameTable *rt = (RenameTable *)subrel;
      RelRoot *subroot = (RelRoot *)rt->child(0)->castToRelExpr();
      result1 = subroot->buildDefaultOrderByForRownum(bindWA, &rt->getTableName());
    }
  } else if (subrel->getOperator().match(REL_ANY_JOIN)) {
    result1 = ((Join *)subrel)->buildDefaultOrderByForRownum(bindWA);
  } else if (subrel->getOperatorType() == REL_SEQUENCE) {
    result1 = ((RelSequence *)subrel)->buildDefaultOrderByForRownum(bindWA);
  }

  // gather order of another child
  subrel = child(1 - childIdx)->castToRelExpr();
  if (subrel->getOperatorType() == REL_SCAN) {
    CorrName &corrName = ((Scan *)subrel)->getTableName();
    NATable *tab = bindWA->getNATable(corrName);
    if (!bindWA->errStatus()) {
      result2 = BinderUtils::buildSyskeyOrPKExpr(tab, corrName, bindWA->wHeap());
    }
  } else if (subrel->getOperatorType() == REL_RENAME_TABLE) {
    if (subrel->child(0)->getOperatorType() == REL_ROOT) {
      RenameTable *rt = (RenameTable *)subrel;
      RelRoot *subroot = (RelRoot *)rt->child(0)->castToRelExpr();
      result2 = subroot->buildDefaultOrderByForRownum(bindWA, &rt->getTableName());
    }
  } else if (subrel->getOperator().match(REL_ANY_JOIN)) {
    result2 = ((Join *)subrel)->buildDefaultOrderByForRownum(bindWA);
  } else if (subrel->getOperatorType() == REL_SEQUENCE) {
    result2 = ((RelSequence *)subrel)->buildDefaultOrderByForRownum(bindWA);
  }

  ItemExprList resultList(bindWA->wHeap());
  if (result1 != NULL) resultList.insertTree(result1, ITM_ITEM_LIST, FALSE, FALSE);
  if (result2 != NULL) resultList.insertTree(result2, ITM_ITEM_LIST, FALSE, FALSE);

  if (resultList.entries() > 0)
    return resultList.convertToItemExpr();
  else
    return NULL;
}
...
RelExpr *Intersect::bindNode(BindWA *bindWA) {
  ...
  Join *join = new (bindWA->wHeap()) Join(child(0)->castToRelExpr(), child(1)->castToRelExpr(), REL_JOIN, predicate);

  join->setIsIntersect();

  // Bind the join.
  //
  RelExpr *boundJoin = join->bindNode(bindWA)->castToRelExpr();
  if (bindWA->errStatus()) return boundJoin;

  // Change the output of the boundJoin to just the left side.
  //
  delete boundJoin->getRETDesc();
  boundJoin->setRETDesc(new (bindWA->wHeap()) RETDesc(bindWA, leftTable));
  bindWA->getCurrentScope()->setRETDesc(boundJoin->getRETDesc());
  ...
  if (child(0)->getGroupAttr()->isStream() && child(1)->getGroupAttr()->isStream()) {
    *CmpCommon::diags() << DgSqlCode(-4159) << DgString0(fmtdList1) << DgString1(fmtdList2);
    bindWA->setErrStatus();
    return this;
  }
  ...
  if (getGroupAttr()->isEmbeddedUpdateOrDelete()) {
    *CmpCommon::diags() << DgSqlCode(-4160) << DgString0(fmtdList1) << DgString1(fmtdList2)
                        << (child(0)->getGroupAttr()->isEmbeddedUpdate() ? DgString2("UPDATE") : DgString2("DELETE"))
                        << (child(1)->getGroupAttr()->isEmbeddedUpdate() ? DgString3("UPDATE") : DgString3("DELETE"));
    bindWA->setErrStatus();
    return this;
  }
  // QSTUFF
...
RelExpr *Except::bindNode(BindWA *bindWA) {
  ...
  ItemExpr *predicate = intersectColumns(leftTable, rightTable, bindWA);
  RelExpr *join =
      new (bindWA->wHeap()) Join(child(0)->castToRelExpr(), child(1)->castToRelExpr(), REL_ANTI_SEMIJOIN, predicate);
...
RelExpr *Union::bindNode(BindWA *bindWA) {
  ...
  if (getCondUnary()) {
    ...
    ItemExpr *predicate = new (bindWA->wHeap())
        BiRelat(ITM_EQUAL, new (bindWA->wHeap()) ConstValue(1), new (bindWA->wHeap()) ConstValue(0));
  ...
  if (root) {
    if (getGroupAttr()->isStream() && root->hasOrderBy()) {
      ...
      leftTable.getTableList(xtnmList1, &fmtdList1);
      rightTable.getTableList(xtnmList2, &fmtdList2);
      *CmpCommon::diags() << DgSqlCode(-4166) << DgString0(fmtdList1) << DgString1(fmtdList2);
      bindWA->setErrStatus();
      return this;
    }
  }
  ...
  if (leftTable.getDegree() != rightTable.getDegree()) {
    ...
    if ((!getUnionForIF()) && (!getCondUnary())  // for triggers
    ) {
      ...
      int sqlcode = bindWA->getCurrentScope()->context()->inTupleList() ? -4126 : -4066;
  ...
  if (!getUnionForIF()) {
    ...
    for (CollIndex i = 0; i < leftTable.getDegree(); i++) {
      ValueIdUnion *vidUnion = new (bindWA->wHeap())
          ValueIdUnion(leftTable.getValueId(i), rightTable.getValueId(i), NULL_VALUE_ID, getUnionFlags());
    ...
    if (getUnionSystemCol() && leftTable.getSystemColumnList() && rightTable.getSystemColumnList()) {
      for (CollIndex i = 0; i < leftTable.getSysColDegree(); i++) {
        ValueIdUnion *vidUnion = new (bindWA->wHeap())
            ValueIdUnion(leftTable.getSysColValueId(i), rightTable.getSysColValueId(i), NULL_VALUE_ID, getUnionFlags());
        vidUnion->setIsTrueUnion(TRUE);
        vidUnion->bindNode(bindWA);
        if (bindWA->errStatus()) {
          delete vidUnion;
        ...
        ValueId valId = vidUnion->getValueId();
        addValueIdUnion(valId, bindWA->wHeap());
        resultTable->addColumn(bindWA, leftTable.getSysColRefNameObj(i), valId, SYSTEM_COLUMN);
      }
    }
  } else {
  ...
  if (alternateRightChildOrderExprTree) {
    alternateRightChildOrderExprTree->convertToValueIdList(alternateRightChildOrderExpr(), bindWA, ITM_ITEM_LIST);
    if (bindWA->errStatus()) {
      return NULL;
    }
  }
...
void Union::checkAccessLockModes() {
  ...
  int lockFlagSession = CmpCommon::transMode()->getDP2LockFlags().getValue();
  StmtLevelAccessOptions optionsLeft = left->accessOptions();
  StmtLevelAccessOptions optionsRight = right->accessOptions();

  int lockFlagLeft = lockFlagSession;
  int lockFlagRight = lockFlagSession;
  ...
  if (lockFlagLeft != lockFlagRight) {
    *CmpCommon::diags() << DgSqlCode(3192) << DgString0(left->getTableName().getQualifiedNameAsString())
...
void Union::dumpChildrensRETDescs(const RETDesc &leftTable, const RETDesc &rightTable) {
...
static NABoolean isRenamedColInSelList(BindWA *bindWA, ItemExpr *col, ItemExprList &origSelectList, CollIndex &indx,
                                       RETDesc *childRETDesc) {
  if (col->getOperatorType() != ITM_REFERENCE) return FALSE;
  ...
  while (j < origSelectList.entries()) {
    ...
    if (selectListEntry->getOperatorType() == ITM_RENAME_COL) {
      const ColRefName &selectListColRefName = *((RenameCol *)selectListEntry)->getNewColRefName();

      if (havingColReference->getColRefNameObj() == selectListColRefName) {
        if (found) {
          // multiple entries with the same name. Error.
          *CmpCommon::diags() << DgSqlCode(-4195) << DgString0(selectListColRefName.getColName());
          bindWA->setErrStatus();
          return FALSE;
        }

        ColumnNameMap *baseColExpr = NULL;
        if (childRETDesc) baseColExpr = childRETDesc->findColumn(selectListColRefName);
        if (NOT baseColExpr) {
          found = TRUE;
          indx = j;
        }
...
static short replaceRenamedColInHavingWithSelIndex(BindWA *bindWA, ItemExpr *expr, ItemExprList &origSelectList,
                                                   NABoolean &replaced, NABoolean &notAllowedWithSelIndexInHaving,
                                                   RETDesc *childRETDesc) {
  if (((expr->getOperatorType() >= ITM_ROW_SUBQUERY) && (expr->getOperatorType() <= ITM_GREATER_EQ_ANY)) ||
      ((expr->getOperatorType() >= ITM_AVG) && (expr->getOperatorType() <= ITM_VARIANCE_SAMP)) ||
      ((expr->getOperatorType() >= ITM_DIFF1) && (expr->getOperatorType() <= ITM_NOT_THIS))) {
  ...
  for (int i = 0; i < expr->getArity(); i++) {
    ...
    if (isRenamedColInSelList(bindWA, expr->child(i), origSelectList, j, childRETDesc)) {
      ...
      return -1;
    else if (replaceRenamedColInHavingWithSelIndex(bindWA, expr->child(i), origSelectList, replaced,
...
static short setValueIdForRenamedColsInHaving(BindWA *bindWA, ItemExpr *expr, ValueIdList &compExpr) {
  if (((expr->getOperatorType() >= ITM_ROW_SUBQUERY) && (expr->getOperatorType() <= ITM_GREATER_EQ_ANY)) ||
      ((expr->getOperatorType() >= ITM_AVG) && (expr->getOperatorType() <= ITM_VARIANCE_SAMP)) ||
      ((expr->getOperatorType() >= ITM_DIFF1) && (expr->getOperatorType() <= ITM_NOT_THIS))) {
    return 0;
...
static void fixUpSelectIndecies(ItemExpr *expr, ValueIdSet &updatedIndecies, CollIndex idx, CollIndex offset) {
  ...
  // Now check myself..
  // Only update ones that we haven't already done.
  if ((expr->getOperatorType() == ITM_SEL_INDEX) && !updatedIndecies.contains(expr->getValueId())) {
...
//
// Returns a list of SelIndecies that were updated.
static void fixUpSelectIndeciesInSet(ValueIdSet &expr, ValueIdSet &updatedIndecies, CollIndex idx, CollIndex offset) {
  for (ValueId vid = expr.init(); expr.next(vid); expr.advance(vid)) {
    // Only update ones that we haven't already done.
    if (((ItemExpr *)vid.getItemExpr())->getOperatorType() == ITM_SEL_INDEX && !updatedIndecies.contains(vid)) {
...
RelRoot *RelRoot::transformOrderByWithExpr(BindWA *bindWA) {
  ...
  if (getCompExprTree())
    ...
    if (grby->child(0) && grby->child(0)->getOperatorType() == REL_ROOT) {
      ...
      if (selRoot->getCompExprTree()) origSelectList.insertTree(selRoot->getCompExprTree());
    }
  }

  int selListCount = origSelectList.entries();
  ...
  ItemExprList newOrderByList((int)origOrderByList.entries(), bindWA->wHeap());
  NABoolean orderByExprFound = FALSE;
  for (int i = 0; i < origOrderByList.entries(); i++) {
    ...
    if (NOT((currOrderByItemExpr->getOperatorType() == ITM_SEL_INDEX) ||
      ...
      while ((NOT found) && (selListIndex < selListCount)) {
        ...
        if (NOT found) selListIndex++;
      }

      if (NOT found) {
        if (currOrderByItemExpr->getOperatorType() == ITM_USER_DEF_FUNCTION) {
          *CmpCommon::diags() << DgSqlCode(-4197) << DgString0("ORDER BY");
          bindWA->setErrStatus();
          return NULL;
        }
      } else {
  ...
  if ((orderByExprFound) && (newOrderByList.entries() > 0)) {
    removeOrderByTree();
...
ItemExpr *RelRoot::processGroupingID(ItemExpr *ie, BindWA *bindWA) {
  if (ie->getOperatorType() != ITM_GROUPING_ID) return ie;
  ...
  long multiplier = (long)pow(2, (childExprList.entries() - 1));
  SQLLargeInt *li = new (bindWA->wHeap()) SQLLargeInt(bindWA->wHeap(), FALSE, FALSE);  // +ve value, no nulls
  for (CollIndex i = 0; i < (CollIndex)childExprList.entries(); i++) {
    ItemExpr *currChildIE = ((ItemExpr *)childExprList[i])->castToItemExpr();

    ItemExpr *groupingClause = new (bindWA->wHeap()) Aggregate(ITM_GROUPING, currChildIE, FALSE);

    ItemExpr *multiplierClause = new (bindWA->wHeap()) ConstValue(li, (void *)&multiplier, sizeof(long));
    ItemExpr *groupingExpr = new (bindWA->wHeap()) BiArith(ITM_TIMES, groupingClause, multiplierClause);

    if (i == 0) {
      groupingIdExpr = groupingExpr;
    } else {
      groupingIdExpr = new (bindWA->wHeap()) BiArith(ITM_PLUS, groupingIdExpr, groupingExpr);
    }
...
RelRoot *RelRoot::transformGroupByWithOrdinalPhase1(BindWA *bindWA) {
  ...
  } else {  // sequence node above group by
    grby = (GroupByAgg *)(child(0)->child(0)->castToRelExpr());
  }

  DCMPASSERT(grby != NULL);

  if ((NOT specialMode) && (grby->getGroupExprTree() == NULL)) return this;
  ...
  ItemExprList newGroupByList((int)origGrbyList.entries(), bindWA->wHeap());

  NABoolean foundSelIndex = FALSE;

  NABoolean lookForRenamedCols = TRUE;
  if ((CmpCommon::getDefault(GROUP_OR_ORDER_BY_EXPR) == DF_OFF) && (NOT specialMode)) lookForRenamedCols = FALSE;

  NABoolean lookForExprInGroupByClause = TRUE;
  if (CmpCommon::getDefault(COMP_BOOL_92) == DF_ON) lookForExprInGroupByClause = FALSE;
  ...
  for (CollIndex i = 0; (i < (CollIndex)origGrbyList.entries()); i++) {
    ItemExpr *currGroupByItemExpr = ((ItemExpr *)origGrbyList[i])->castToItemExpr();
    ItemExpr *newGroupByItemExpr = NULL;

    if (currGroupByItemExpr->getOperatorType() == ITM_USER || currGroupByItemExpr->getOperatorType() == ITM_AUTHNAME) {
    ...
    if (currGroupByItemExpr->getOperatorType() == ITM_CONSTANT) {
      ...
      if ((cv->canGetExactNumericValue()) && (cv->getType()->getScale() == 0)) {
        ...
        if ((selIndex >= 0) && (selIndex < MAX_COMSINT32)) {
          if (selIndex == 0 || selIndex > origSelectListCount) {
            ...
            currGroupByItemExpr = new (bindWA->wHeap()) SelIndex((int)selIndex);
        }
      }
    }
    ...
    if ((currGroupByItemExpr->getOperatorType() != ITM_REFERENCE) &&
      ...
      while ((NOT found) && (j < origSelectListCount)) {
        ...
        if ((selectListEntry->getOperatorType() != ITM_REFERENCE) &&
            ((selectListEntry->getOperatorType() != ITM_RENAME_COL) ||
             ((selectListEntry->child(0)) && (selectListEntry->child(0)->getOperatorType() != ITM_REFERENCE)))) {
          ItemExpr *renameColEntry = NULL;
          if (selectListEntry->getOperatorType() == ITM_RENAME_COL) {
          ...
          found = currGroupByItemExpr->duplicateMatch(*selectListEntry);
          if (found) {
            lastMatch = j;
            if (!listOfExpressions.contains(j)) {
              ...
              if (renameColEntry) renameColEntry->setInGroupByOrdinal(TRUE);
            } else
              found = FALSE;
          }
        }
      ...
      if (lastMatch != -1) {
        ...
        if (selListIndex == -1) selListIndex = lastMatch;

        if (bindWA->inViewDefinition())
          currGroupByItemExpr = new (bindWA->wHeap()) SelIndex(selListIndex + 1, currGroupByItemExpr);
        else
          currGroupByItemExpr = new (bindWA->wHeap()) SelIndex(selListIndex + 1);
      }
    }  // expr in group by clause

    if ((NOT found) && (selIndexError) && (selIndex > 0)) {
      // this const was not found in the select list and it was
      // not a valid select index.
      // Return an error.
      *CmpCommon::diags() << DgSqlCode(-4007) << DgInt0((int)selIndex) << DgInt1((int)origSelectList.entries());
      bindWA->setErrStatus();
      return NULL;
    }
    if (compExprTreeIsNull) return this;
    ...
    if (currGroupByItemExpr->getOperatorType() == ITM_SEL_INDEX) {
      ...
      if (si->getSelIndex() > origSelectList.entries()) {
        *CmpCommon::diags() << DgSqlCode(-4007) << DgInt0((int)si->getSelIndex())
                            << DgInt1((int)origSelectList.entries());
        bindWA->setErrStatus();
        return NULL;
      }
      ...
      if (selectListEntry->getOperatorType() == ITM_USER || selectListEntry->getOperatorType() == ITM_AUTHNAME) {
        *CmpCommon::diags() << DgSqlCode(-4197) << DgString0("GROUP BY");
        bindWA->setErrStatus();
        return NULL;
      }
      ...
      if ((selectListEntry->getOperatorType() == ITM_RENAME_COL) &&
        ...
        newGroupByItemExpr = selectListEntry->child(0)->castToItemExpr()->copyTopNode(NULL, bindWA->wHeap());
        ...
        newGroupByItemExpr = selectListEntry->copyTopNode(NULL, bindWA->wHeap());
      } else {
        selectListEntry->setInGroupByOrdinal(TRUE);
        if (selectListEntry->getOperatorType() == ITM_RENAME_COL) {
          ItemExpr *sListEntry = selectListEntry->child(0);

          if (sListEntry &&
              (sListEntry->getOperatorType() == ITM_USER || sListEntry->getOperatorType() == ITM_AUTHNAME)) {
            *CmpCommon::diags() << DgSqlCode(-4197) << DgString0("GROUP BY");
            bindWA->setErrStatus();
            return NULL;
          }
          if (sListEntry && sListEntry->isAnAggregate()) sListEntry->setIsGroupByExpr(TRUE);
        } else if (!selectListEntry->isAnAggregate()) {
          selectListEntry->setIsGroupByExpr(TRUE);
        }
      ...
      ColReference *groupByColReference = (ColReference *)currGroupByItemExpr;
      ...
      if (lookForRenamedCols &&
        ...
        while (j < origSelectList.entries()) {
          ...
          if (selectListEntry->getOperatorType() == ITM_RENAME_COL) {
            ...
            if (groupByColReference->getColRefNameObj().getColName() == selectListColRefName.getColName()) {
              if (found) {
                // multiple entries with the same name. Error.
                *CmpCommon::diags() << DgSqlCode(-4195) << DgString0(selectListColRefName.getColName());
                bindWA->setErrStatus();
              ...
              newGroupByItemExpr = new (bindWA->wHeap()) SelIndex(j + 1);
              ((SelIndex *)newGroupByItemExpr)->setRenamedColNameInGrbyClause(TRUE);
        ...
        if ((NOT renamedColsInSelectList) && (j == origSelectList.entries())) lookForRenamedCols = FALSE;
      }  // lookForRenamedCols

      if (!newGroupByItemExpr) newGroupByItemExpr = currGroupByItemExpr;
    }  // else foundSelIndex
    else if ((currGroupByItemExpr->getOperatorType() == ITM_USER_DEF_FUNCTION ||
              currGroupByItemExpr->isBuiltinFuncOrderable()) &&
             (udfSubqInAggGrby_Enabled))
      newGroupByItemExpr = currGroupByItemExpr;
    else if ((currGroupByItemExpr->getOperatorType() == ITM_ROW_SUBQUERY) && (udfSubqInAggGrby_Enabled))
      newGroupByItemExpr = currGroupByItemExpr;
    else {
      *CmpCommon::diags() << DgSqlCode(-4197) << DgString0("GROUP BY");
      bindWA->setErrStatus();
  ...
  if ((foundSelIndex) && (newGroupByList.entries() > 0)) {
  ...
  if ((getOrderByTree()) && (grby->getGroupExprTree() != NULL)) {
    ItemExpr *orderByTree = getOrderByTree();
    ...
    for (CollIndex ii = 0; ii < orderByList.entries(); ii++) {
      ...
      if (colRef->getOperatorType() == ITM_INVERSE) colRef = colRef->child(0)->castToItemExpr();
      if (colRef && colRef->getOperatorType() == ITM_REFERENCE) {
        ...
        for (CollIndex j = 0; j < groupByList.entries(); j++) {
          ...
          if (gbyExpr->getOperatorType() == ITM_REFERENCE) {
            ...
            if (obyColRef->getColRefNameObj().getColName() == gbyColRef->getColRefNameObj().getColName()) {
              found = TRUE;
              break;
            }
          }  // if
...
RelRoot *RelRoot::transformGroupByWithOrdinalPhase2(BindWA *bindWA) {
  ...
  if (grby->isRollup()) {
    if (grby->groupExpr().entries() != grby->rollupGroupExprList().entries()) {
      *CmpCommon::diags() << DgSqlCode(-4384) << DgString0("Cannot have duplicate entries.");

      bindWA->setErrStatus();
      return NULL;
    }

    for (ValueId valId = grby->aggregateExpr().init(); grby->aggregateExpr().next(valId);
      ...
      if (NOT((ae->getOperatorType() == ITM_MIN) || (ae->getOperatorType() == ITM_MAX) ||
              (ae->getOperatorType() == ITM_SUM) || (ae->getOperatorType() == ITM_AVG) ||
              (ae->getOperatorType() == ITM_COUNT) || (ae->getOperatorType() == ITM_COUNT_NONULL) ||
              (ae->getOperatorType() == ITM_GROUPING))) {
        *CmpCommon::diags() << DgSqlCode(-4384) << DgString0("Unsupported rollup aggregate function.");
      ...
      if (ag->isDistinct()) {
        *CmpCommon::diags() << DgSqlCode(-4384) << DgString0("Distinct rollup aggregates not supported.");
      ...
      if (ae->getOperatorType() == ITM_GROUPING) {
        ...
        while ((NOT found) and (i < grby->rollupGroupExprList().entries())) {
          ...
          if (found) ag->setRollupGroupIndex(i);
          i++;
        }  // while
  ...
  for (ValueId vid = groupExpr.init(); groupExpr.next(vid); groupExpr.advance(vid)) {
    if (vid.getItemExpr()->getOperatorType() == ITM_SEL_INDEX) {
      CollIndex selIndexExpansionOffset = 0;
      SelIndex *si = (SelIndex *)(vid.getItemExpr());
      ValueId grpById = compExpr()[si->getSelIndex() - 1];
      ...
      switch (grpById.getItemExpr()->getOperatorType()) {
        case ITM_VALUEID_PROXY: {
          ValueId derivedId = ((ValueIdProxy *)(grpById.getItemExpr()))->isDerivedFrom();

          // If this is not the ValueIdProxy that represents the MVF or Subq
          // skip the expansion.
          if (((ValueIdProxy *)(grpById.getItemExpr()))->needToTransformChild() != TRUE) break;
          ...
          fixUpSelectIndeciesInSet(grby->groupExpr(), fixedUpIndecies, si->getSelIndex(), selIndexExpansionOffset);

          fixUpSelectIndecies(getOrderByTree(), fixedUpIndecies, si->getSelIndex(), selIndexExpansionOffset);
          break;
        }
      }
      ...
      for (ValueId id = columns.init(); columns.next(id); columns.advance(id)) {
        NAColumn *nacol = id.getNAColumn();
        if (nacol->isReferencedForHistogram()) continue;
  ...
  if (grby->isRollup()) {
    for (CollIndex i = 0; i < grby->rollupGroupExprList().entries(); i++) {
      ...
      if (NOT valId.getType().supportsSQLnull()) {
        *CmpCommon::diags() << DgSqlCode(-4384) << DgString0("Grouped columns must be nullable.");
        bindWA->setErrStatus();
        return NULL;
      }
    }
  ...
  for (ValueId vid = groupAggExpr.init(); groupAggExpr.next(vid); groupAggExpr.advance(vid)) {
  ...
  if ((grby->selPredTree()) && (grby->selIndexInHaving())) {
    setValueIdForRenamedColsInHaving(bindWA, grby->selPredTree(), compExpr());

    BindScope *currScope = bindWA->getCurrentScope();

    ItemExpr *havingPred = grby->removeSelPredTree();
    currScope->context()->inHavingClause() = TRUE;
    havingPred->convertToValueIdSet(grby->selectionPred(), bindWA, ITM_AND);
  ...
  if (orderByTree_ && seqNode && grby) {
    ...
    for (CollIndex i = 0; (i < (CollIndex)origOrderByList.entries()); i++) {
      ItemExpr *currOrderByItemExpr = ((ItemExpr *)origOrderByList[i])->castToItemExpr();
  ...
  ValueIdSet &finalGroupExpr = grby->groupExpr();
  for (ValueId vid = finalGroupExpr.init(); finalGroupExpr.next(vid); finalGroupExpr.advance(vid)) {
    if (vid.getItemExpr()->getOperatorType() == ITM_USER || vid.getItemExpr()->getOperatorType() == ITM_AUTHNAME) {
      *CmpCommon::diags() << DgSqlCode(-4197) << DgString0("implicit GROUP BY");
      bindWA->setErrStatus();
      return NULL;
    }
  }
...
void RelRoot::transformTDPartitionOrdinals(BindWA *bindWA) {
  if (!getHasTDFunctions()) return;

  if (getCompExprTree() == NULL) return;
  ...
  for (CollIndex i = 0; (i < (CollIndex)origPartitionByList.entries()); i++) {
    ItemExpr *currPartitionByItemExpr = ((ItemExpr *)origPartitionByList[i])->castToItemExpr();
    ...
    if (currPartitionByItemExpr->getOperatorType() == ITM_CONSTANT) {
      ...
      if ((cv->canGetExactNumericValue()) && (cv->getType()->getScale() == 0)) {
        ...
        if (selIndex <= 0 || selIndex > origSelectList.entries()) {  // index in error -- produce error message
...
void RelRoot::resolveAggregates(BindWA *bindWA) {
  ...
  if (NOT currScope->getUnresolvedAggregates().isEmpty()) {
    if (getHasTDFunctions()) {  // Using rank function and aggregate functions in the same scope is not supported.
    ...
    if (implicitGrouping) {
      ...
      groupByAgg = new (bindWA->wHeap()) GroupByAgg(realChildNode, REL_GROUPBY);
    ...
    if (currScope->context()->unaggColRefInSelectList()) {
      ...
      for (i = 0; i < n; i++) {
        ...
        if (!col->isGrouped())
          if (col->getColRefNameObj().isStar() || col->getValueId().getNAColumn(TRUE /*okIfNotColumn*/)) {
            sqlCode = -4012;
...
void RelRoot::resolveSequenceFunctions(BindWA *bindWA) {
  ...
  if (sequenceNode) {
    if (getHasTDFunctions() &&
        sequenceNode->child(0)->getOperatorType() ==
            REL_GROUPBY) {  // Using rank function and group by clause in the same scope is not supported.
      *CmpCommon::diags() << DgSqlCode(-4366);
      bindWA->setErrStatus();
      return;
    }
    ...
    if (getGroupAttr()->isEmbeddedUpdateOrDelete()) {
      *CmpCommon::diags() << DgSqlCode(-4202)
                          << (getGroupAttr()->isEmbeddedUpdate() ? DgString0("UPDATE") : DgString0("DELETE"));
      bindWA->setErrStatus();
      return;
    }
    ...
    // the select list.
    //
    sequenceNode->addUnResolvedSeqFunctions(currScope->getUnresolvedSequenceFunctions(), bindWA);
    ...
    if ((((const RelSequence *)sequenceNode)->sequenceFunctions().isEmpty()) &&
        (!getHasOlapFunctions() && ((const RelSequence *)sequenceNode)->requiredOrder().entries() != 0)) {
...
// add a default order-by clause for rownum, SYSKEY or PK
ItemExpr *RelRoot::buildDefaultOrderByForRownum(BindWA *bindWA, const CorrName *renameTab = NULL) {
  ItemExpr *result = NULL;

  // order-by of subquery is omitted, so we need to add order-by of subquery to ROWNUM function
  if (NULL != renameTab && NULL != getOrderByTree()) {
    result = getOrderByTree();
  } else {
    if (child(0)->getOperatorType() == REL_SCAN) {
      CorrName &corrName = ((Scan *)child(0)->castToRelExpr())->getTableName();
      NATable *tab = bindWA->getNATable(corrName);
      if (!bindWA->errStatus()) {
        result = BinderUtils::buildSyskeyOrPKExpr(tab, corrName, bindWA->wHeap());
      }
    } else if (child(0)->getOperatorType() == REL_RENAME_TABLE && NULL == renameTab) {
      if (child(0)->castToRelExpr()->child(0)->getOperatorType() == REL_ROOT) {
        RenameTable *rt = (RenameTable *)child(0)->castToRelExpr();
        RelRoot *subroot = (RelRoot *)rt->child(0)->castToRelExpr();
        ...
        if (NULL != subroot->getCompExprTree()) {
          rownumFoundInSubRoot =
              subroot->detectRownumReference(subroot->getCompExprTree(), parentItmList, parentArityList, NULL, -1);
        }
        if (FALSE == rownumFoundInSubRoot && NULL != subroot->child(0)->selPredTree()) {
          rownumFoundInSubRoot = subroot->detectRownumReference(subroot->child(0)->selPredTree(), parentItmList,
                                                                parentArityList, NULL, -1);
        }

        if (FALSE == rownumFoundInSubRoot || NULL != subroot->getOrderByTree())
          result = subroot->buildDefaultOrderByForRownum(bindWA, &rt->getTableName());
      }
    } else if (child(0)->getOperator().match(REL_ANY_JOIN)) {
      Join *join = (Join *)child(0)->castToRelExpr();
      result = join->buildDefaultOrderByForRownum(bindWA);
    } else if (child(0)->getOperatorType() == REL_SEQUENCE) {
      RelSequence *seq = (RelSequence *)child(0)->castToRelExpr();
      result = seq->buildDefaultOrderByForRownum(bindWA);
    }
  }

  if (NULL != renameTab && NULL != result) {
    NABoolean isDistinct = FALSE;
    ItemExprList resultList(result, bindWA->wHeap(), ITM_ITEM_LIST, FALSE, FALSE);
    ItemExprList selList(bindWA->wHeap());
    ItemExprList selAppend(bindWA->wHeap());

    if (NULL != getCompExprTree()) selList.insertTree(getCompExprTree(), ITM_ITEM_LIST, FALSE, FALSE);
    // When DISTINCT is used in query, the parser tree is like :
    //    ROOT1 --> GROUPBY --> ROOT2 --> SCAN
    // ROOT1 has no select list, but may have order-by list.
    // We don't want to do item check here if this is ROOT1.
    else if (REL_GROUPBY == getChild(0)->getOperatorType() && REL_ROOT == getChild(0)->getChild(0)->getOperatorType())
      isDistinct = TRUE;

    if (NOT isDistinct) {
      for (int i = 0; i < resultList.entries(); i++) {
        ItemExpr *ie = resultList[i];
        NAString colExposedName = "";

        if (ie->getOperatorType() != ITM_REFERENCE) continue;

        ColReference *col = (ColReference *)ie;

        // try to find alias for order-by items
        NABoolean isFound = FALSE;
        for (int j = 0; j < selList.entries(); j++) {
          ItemExpr *sel = selList[j];

          if (sel->getOperatorType() == ITM_RENAME_COL) {
            RenameCol *renameCol = (RenameCol *)sel;
            if (col->getCorrNameObj().getCorrNameAsString() == "" &&
                col->getColRefNameObj().getColName() == renameCol->getNewColRefName()->getColName()) {
              colExposedName = renameCol->getNewColRefName()->getColName();
              isFound = TRUE;
              break;
            }
          } else if (sel->getOperatorType() == ITM_REFERENCE) {
            ColReference *refCol = (ColReference *)sel;
            if ((col->getCorrNameObj().getCorrNameAsString() == "" ||
                 col->getCorrNameObj().getCorrNameAsString() == refCol->getCorrNameObj().getCorrNameAsString()) &&
                col->getColRefNameObj().getColName() == refCol->getColRefNameObj().getColName()) {
              colExposedName = refCol->getColRefNameObj().getColName();
              isFound = TRUE;
              break;
            }
          }
        ...
        if (NOT isFound) {
          // no alias found, then create an alias
          char tmpStr[100];
          str_itoa((long)i, tmpStr);
          NAString tmpCol("_sys_default_order_");
          tmpCol += tmpStr;

          RenameCol *newSel = NULL;
          newSel = new (bindWA->wHeap()) RenameCol(col, new (bindWA->wHeap()) ColRefName(tmpCol, bindWA->wHeap()));
          newSel->setTargetColumnClass(SYSTEM_COLUMN);

          selAppend.insertTree(newSel, ITM_ITEM_LIST, FALSE, FALSE);

          resultList[i] =
              new (bindWA->wHeap()) ColReference(new (bindWA->wHeap()) ColRefName(tmpCol, *renameTab, bindWA->wHeap()));
        } else if (NOT child(0)->getOperator().match(REL_ANY_JOIN)) {
          resultList[i] = new (bindWA->wHeap())
              ColReference(new (bindWA->wHeap()) ColRefName(colExposedName, *renameTab, bindWA->wHeap()));
        }
      }

      if (selAppend.entries() > 0) {
        addCompExprTree(selAppend.convertToItemExpr());
      }

      result = resultList.convertToItemExpr();
    }
  }
...
NABoolean RelRoot::detectRownumReference(ItemExpr *itm, NAList<ItemExpr *> &parentItmList,
  ...
  if (itm->getOperator() == ITM_ROWNUM) {
    if (parent == NULL) {
      ...
      parentItmList.insert(parent);
      parentArityList.insert(arity);
      return TRUE;
    }
  }

  NABoolean result = FALSE;
  for (int idx = 0; idx < itm->getArity(); idx++) {
    if (NULL != itm->getChild(idx))
      result = detectRownumReference(itm->getChild(idx)->castToItemExpr(), parentItmList, parentArityList, itm, idx)
                   ? TRUE
                   : result;
  }
...
void RelRoot::processRownum(BindWA *bindWA) {
  if (CmpCommon::getDefault(MODE_COMPATIBLE_1) == DF_ON) {
    /*
     * If there is a ROWNUM() predicate in REL_SCAN/REL_RENAME_TABLE, like this : ROWNUM() <|<=|= NUM,
     * We need to push up this ROWNUM() predicate to sequence node, and make ROWNUM() a column reference.
     */
    if (child(0)->getOperatorType() == REL_SCAN || child(0)->getOperatorType() == REL_RENAME_TABLE ||
        child(0)->getOperatorType() == REL_GROUPBY || child(0)->getOperator().match(REL_ANY_JOIN)) {
      NABoolean isRownumFoundInSelList = FALSE;
      NABoolean isRownumASelItm = FALSE;
      ColRefName *rowname = new (bindWA->wHeap()) ColRefName("ROWNUM");

      RelSequence *sequenceNode = NULL;
      Aggregate *rownumAgg = NULL;
      ExprValueId eRownum = (ItemExpr *)NULL;
      ItemExprTreeAsList rownumPred(&eRownum, ITM_AND, LEFT_LINEAR_TREE);
      long limitn = 0;
      ...
      if (NULL != getCompExprTree()) {
        ItemExprList selList(getCompExprTree(), bindWA->wHeap(), ITM_ITEM_LIST, FALSE, FALSE);
        NABoolean hasRownum = FALSE;

        for (int i = 0; i < selList.entries(); i++) {
          parentItmList.clear();
          parentArityList.clear();

          ItemExpr *expr = selList[i];
          hasRownum = detectRownumReference(expr, parentItmList, parentArityList, NULL, -1);

          if (hasRownum) {
            if (NULL == rownumAgg) {
              // generate a row_number() over() function
              rownumAgg = new (PARSERHEAP()) Aggregate(ITM_COUNT, new (bindWA->wHeap()) SystemLiteral(1),
                                                       FALSE /*i.e. not distinct*/, ITM_COUNT_STAR__ORIGINALLY, '!');
            }
            ...
            if (parentItmList.entries() > 0) {
              ...
              // add a new RenameCol('ROWNUM') for rownum reference
              RenameCol *renameCol = new (bindWA->wHeap()) RenameCol(rownumAgg, rowname);
              selList[i] = renameCol;

              isRownumASelItm = TRUE;
            }

            isRownumFoundInSelList = TRUE;
          }
        }
        if (isRownumASelItm == TRUE) {
          removeCompExprTree();
          addCompExprTree(selList.convertToItemExpr());
        }
      }

      // check if rownum exists in where clause
      RelExpr *whereRel = child(0);
      ItemExpr *wherePred = NULL;
      if (child(0)->getOperatorType() == REL_GROUPBY) whereRel = whereRel->child(0);
      wherePred = whereRel->selPredTree();

      if (NULL != wherePred) {
        ExprValueId eVid(whereRel->selPredTree());

        ExprValueId eCommon = (ItemExpr *)NULL;
        ExprValueId eRownum = (ItemExpr *)NULL;

        ItemExprTreeAsList commonPred(&eCommon, ITM_AND, LEFT_LINEAR_TREE);
        ItemExprTreeAsList origPredList(&eVid, ITM_AND, LEFT_LINEAR_TREE);

        ItemExpr *child0 = NULL;
        ItemExpr *child1 = NULL;
        ItemExpr *rowLimitExpr = NULL;

        for (CollIndex i = 0; i < (CollIndex)origPredList.entries(); i++) {
          NABoolean rownumInLeft = FALSE;
          NABoolean rownumInRight = FALSE;
          ItemExpr *ie = origPredList[i];
          rowLimitExpr = NULL;

          if (FALSE == ie->isAPredicate() || ie->getArity() != 2) {
            commonPred.insert(ie);
            continue;
          }

          child0 = ie->child(0)->castToItemExpr();
          child1 = ie->child(1)->castToItemExpr();
          ...
          if (FALSE == rownumInLeft && FALSE == rownumInRight) {
            ...
            commonPred.insert(ie);
            continue;
          } else if (TRUE == rownumInLeft && FALSE == rownumInRight && child0->getOperator() == ITM_ROWNUM) {
            posFlag = 1;
            rowLimitExpr = child1;
          } else if (FALSE == rownumInLeft && TRUE == rownumInRight && child1->getOperator() == ITM_ROWNUM) {
            posFlag = -1;
            rowLimitExpr = child0;
          }

          // add a first-N optimiztion if this predicate is equivalent to (ROWNUM = / < / <= N)
          if (NULL != rowLimitExpr) {
            NABoolean tmp;
            ...
            if (NULL != cv && cv->canGetExactNumericValue()) {
              ...
              if (ITM_EQUAL == ie->getOperatorType())
                ...
                predFlag = posFlag * -1;
              else if (ie->getOperatorType() == ITM_LESS || ie->getOperatorType() == ITM_LESS_EQ)
                predFlag = posFlag * 1;
              else
                predFlag = -1;

              if (predFlag > 0) {
                // if multi ROWNUM() in predicate, use the minimum one
                limitn = (limitn == 0 || cv->getExactNumericValue() < limitn) ? cv->getExactNumericValue() : limitn;
              }
            }
          }

          if (parentItmList.entries() > 0) {
            ColRefName *refName = new (bindWA->wHeap()) ColRefName(rowname->getColName(), newCorrName, bindWA->wHeap());
            ColReference *rownumRef = new (bindWA->wHeap()) ColReference(refName);

            for (int idx = 0; idx < parentItmList.entries(); idx++) {
              parentItmList[idx]->setChild(parentArityList[idx], rownumRef);
            }
          }

          rownumPred.insert(ie);
        }

        if (rownumPred.entries() > 0) {
          whereRel->removeSelPredTree();
          if (NULL != eCommon.getPtr()) whereRel->addSelPredTree(eCommon.getPtr());
        }
      }

      // no rownum found both in select list and where clause
      if (rownumPred.entries() == 0 && isRownumFoundInSelList == FALSE) return;

      // add a rownum in select list if rownum only exists in where clause
      if (rownumPred.entries() > 0 && isRownumASelItm == FALSE) {
        if (NULL == rownumAgg) {
          rownumAgg = new (PARSERHEAP()) Aggregate(ITM_COUNT, new (bindWA->wHeap()) SystemLiteral(1),
                                                   FALSE /*i.e. not distinct*/, ITM_COUNT_STAR__ORIGINALLY, '!');
        }
        ...
        RenameCol *renameCol = new (bindWA->wHeap()) RenameCol(rownumAgg, rowname);
        renameCol->setTargetColumnClass(SYSTEM_COLUMN);

        // add a new ROWNUM select item to current select list
        ItemExprList selList(getCompExprTree(), bindWA->wHeap(), ITM_ITEM_LIST, FALSE, FALSE);
        selList.insertTree(renameCol, ITM_ITEM_LIST, FALSE, FALSE);
        removeCompExprTree();
        addCompExprTree(selList.convertToItemExpr());
      }

      // add default order by for ROWNUM(row_number()over())
      rownumAgg->setOLAPInfo(NULL, buildDefaultOrderByForRownum(bindWA), -INT_MAX, 0);

      // add a Sequence Node for row_number()over() function
      sequenceNode = new (bindWA->wHeap()) RelSequence(child(0), NULL);
      sequenceNode->setHasOlapFunctions(TRUE);
      this->setChild(0, sequenceNode);

      // add a sub-query if rownum used in where clause
      if (rownumPred.entries() > 0) {
        if (0 < limitn) sequenceNode->setChild(0, new (bindWA->wHeap()) FirstN(sequenceNode->child(0), limitn, TRUE));

        RelRoot *subroot = new (bindWA->wHeap()) RelRoot(sequenceNode, REL_ROOT, getCompExprTree());
        subroot->setHasOlapFunctions(TRUE);
        this->setChild(0, new (PARSERHEAP()) RenameTable(TRUE, subroot, newCorrName));
        this->removeCompExprTree();
        this->addCompExprTree(new (bindWA->wHeap())
                                  ColReference(new (bindWA->wHeap()) ColRefName(TRUE, bindWA->wHeap())));

        this->child(0)->addSelPredTree(eRownum.getPtr());

        if (hasOrderBy()) {
          subroot->addOrderByTree(this->getOrderByTree());
        }
      } else
        this->setHasOlapFunctions(TRUE);
    }
  } else if (CmpCommon::getDefault(MODE_SPECIAL_4) == DF_ON) {
    if (!child(0)) return;

    if ((child(0)->getOperatorType() != REL_SCAN) && (child(0)->getOperatorType() != REL_RENAME_TABLE)) return;

    if (!child(0)->selPredTree()) return;

    ItemExpr *wherePred = child(0)->selPredTree();
    ItemExprList iel(wherePred, bindWA->wHeap(), ITM_AND, FALSE, FALSE);

    NABoolean found = FALSE;
    for (int i = 0; ((NOT found) && (i < iel.entries())); i++) {
      ItemExpr *ie = iel[i];
      if (ie->getArity() != 2) continue;

      if (NOT((ie->getOperatorType() == ITM_LESS) || (ie->getOperatorType() == ITM_EQUAL) ||
      ...
      if (NOT((child0->getOperatorType() == ITM_REFERENCE) && (child1->getOperatorType() == ITM_CONSTANT))) continue;
      ...
      if (NOT((catName.isNull()) && (schName.isNull()) && (objName.isNull()) && (colName == "ROWNUM"))) continue;

      ConstValue *cv = (ConstValue *)child1;
      if (NOT cv->canGetExactNumericValue()) continue;

      long val = cv->getExactNumericValue();
      if (val < 0) continue;

      if ((ie->getOperatorType() == ITM_EQUAL) && (val != 1)) continue;

      if ((ie->getOperatorType() == ITM_LESS) && (val > 0)) val--;
...
RelExpr *RelRoot::bindNode(BindWA *bindWA) {
  ...
  if (isTrueRoot()) {
    if (child(0) && ((child(0)->getOperatorType() == REL_INSERT) || (child(0)->getOperatorType() == REL_UNARY_INSERT) ||
                     (child(0)->getOperatorType() == REL_LEAF_INSERT))) {
      Insert *ins = (Insert *)child(0)->castToRelExpr();
      if (ins->isNoRollback()) {
        if ((CmpCommon::getDefault(AQR_WNR) != DF_OFF) && (CmpCommon::getDefault(AQR_WNR_INSERT_CLEANUP) != DF_OFF))
    ...
    // -- cqd must be set
    // -- this compilation must be running under a user transaction
    // -- user savepoint must be running or auto commit savepoint be on.
    //
    // Later during generation phase, savepoint may not be used based
    // on other conditions. Check generator/GenPreCode.cpp for details.
    if (child(0) && (child(0)->getOperator().match(REL_ANY_GEN_UPDATE) ||
                     child(0)->getOperator().match(REL_FORCE_ANY_SCAN) || child(0)->getOperatorType() == REL_CALLSP)) {
      if (((CmpCommon::getDefault(TRAF_SAVEPOINTS) != DF_OFF) &&
           (CmpCommon::transMode()->xnInProgress() || CmpCommon::transMode()->getAutoCommitOff())) &&
          ((CmpCommon::transMode()->savepointInProgress()) || (CmpCommon::transMode()->getAutoCommitSavepointOn()))) {
        setSavepointEnabled(TRUE);
      }
    }
  ...
  RelRoot *returnedRoot = transformGroupByWithOrdinalPhase1(bindWA);
  if (!returnedRoot) return NULL;

  returnedRoot = transformOrderByWithExpr(bindWA);
  if (!returnedRoot) return NULL;
  ...
  if (bindWA->getCurrentScope()->context()->inTableCheckConstraint()) {
    ...
    *CmpCommon::diags() << DgSqlCode(-4089)
                        << DgConstraintName(bindWA->getCurrentScope()
                                                ->context()
                                                ->inCheckConstraint()
                                                ->getConstraintName()
  ...
  if (isTrueRoot()) bindWA->setTopRoot(this);

  bindWA->setBindTrueRoot(isTrueRoot());

  if (!bindWA->getAssignmentStArea()) {
    bindWA->getAssignmentStArea() = new (bindWA->wHeap()) AssignmentStArea(bindWA);
    bindWA->getAssignmentStArea()->getAssignmentStHostVars() = new (bindWA->wHeap()) AssignmentStHostVars(bindWA);
  }
  ...
  if (isTrueRoot() && assignmentStTree()) {
    ...
    if (outputVar) {
      ...
      if (hostVar->getType()->getTypeQualifier() == NA_ROWSET_TYPE) {
        ...
        RelExpr *intoNode = new (bindWA->wHeap()) RowsetInto(this, outputVar, sizeExpr);
        ...
        // long defaultFirstNRows = newRoot->getFirstNRows();
  ...
  if (assignmentStTree() && child(0)->getOperatorType() != REL_ROWSET_INTO) {
    AssignmentStHostVars *ptr = new (bindWA->wHeap()) AssignmentStHostVars(bindWA);
    ...
    if (ptr->containsRowsets(assignmentStTree())) {
      ...
      RelExpr *intoNode = new (bindWA->wHeap()) RowsetInto(this, assignmentStTree(), outputSizeExpr);
  ...
  if (child(0)->getOperatorType() == REL_LOCK) {
    accessOptions().updateAccessOptions(TransMode::ILtoAT(TransMode::READ_COMMITTED_),
                                        ((RelLock *)child(0).getPtr())->getLockMode());
    accessOptions().updateAccessOptions(TransMode::ILtoAT(CmpCommon::transMode()->getIsolationLevel()));
  }
  ...
  if (getGroupAttr()->isEmbeddedUpdateOrDelete()) accessOptions().setUpdateOrDelete(TRUE);
  // QSTUFF

  if (accessOptions().userSpecified()) bindWA->getCurrentScope()->context()->setStmtLevelAccessOptions(accessOptions());

  if (isSubRoot() && getHostArraysArea()) getHostArraysArea()->setRoot(this);
  ...
  if (isTrueRoot()) {
    ...
    if (bindWA->getHostArraysArea() && bindWA->getHostArraysArea()->hasHostArraysInWhereClause() &&
    ...
    if (childOperType() == REL_SCAN) oltOptInfo().setOltOpt(TRUE);
    ...
    */
    ...
    // if INSERT...VALUES, olt opt is possible.
             ...
             (NOT child(0)->child(0) || child(0)->child(0)->getOperatorType() == REL_TUPLE))
      oltOptInfo().setOltOpt(TRUE);
  ...
  if (bindWA->getSpOutParams().entries()) spOutParams_ = &(bindWA->getSpOutParams());
  ...
  if (isTrueRoot()) {
    ...
    // olt opt is possible.
    if (((childOperType() == REL_UNARY_UPDATE) || (childOperType() == REL_UNARY_DELETE)) && (NOT updateCurrentOf()))
      oltOptInfo().setOltOpt(TRUE);

    // If transaction statement (begin/commit/rollback/set xn,
    // olt opt is possible.
    if (childOperType() == REL_TRANSACTION) oltOptInfo().setOltOpt(TRUE);
    ...
    if (childOperType().match(REL_ANY_GEN_UPDATE)) {
      if (bindWA->getHostArraysArea() && bindWA->getHostArraysArea()->done())  // rowsets
        rollbackOnError() = TRUE;
      else if ((childOperType() == REL_UNARY_INSERT) &&
               (child(0)->child(0) && child(0)->child(0)->getOperatorType() != REL_TUPLE))
        rollbackOnError() = TRUE;
    }
  ...
  if (NOT compExprTree) {
    ...
    if (bindWA->renameToScanTable()) {
      ...
      const CorrName corr = (getViewScanNode()->getOperatorType() == REL_RENAME_TABLE)
      ...
      while (NOT done) {
        ...
        cr->getCorrNameObj().getQualifiedNameObj().setObjectName(corr.getQualifiedNameObj().getObjectName());
      }
    }
    // QSTUFF
    ...
    if (bindWA->errStatus()) return NULL;
    // change multi-window olap functions header from internal name to empty,
    // displayed as "(EXPR)"
  ...
  // MVs --
  if (bindWA->isPropagateOpAndSyskeyColumns() && child(0)->getOperatorType() != REL_GROUPBY &&
      child(0)->getOperatorType() != REL_AGGREGATE && currScope->getUnresolvedAggregates().isEmpty() &&
      !isEmptySelectList() && !isTrueRoot())
  ...
  if (isTrueRoot() && (child(0)) &&
    ...
    int sqlcode = (CmpCommon::context()->GetMode() == STMT_DYNAMIC) ? -4093 : -4094;
    *CmpCommon::diags() << DgSqlCode(sqlcode) << DgInt0(outputVarCnt()) << DgInt1(getRETDesc()->getDegree());
    bindWA->setErrStatus();
    return NULL;
  }
  ...
  if (inputVarTree) {
    ...
    // copy params specified in the query to hbase load operator.
    // These will be moved to internal 'load transform' statement at runtime.
    if (isTrueRoot() && child(0) && (child(0)->getOperatorType() == REL_EXE_UTIL) &&
        (((ExeUtilExpr *)child(0)->castToRelExpr())->getExeUtilType() == ExeUtilExpr::HBASE_LOAD_)) {
      ExeUtilHBaseBulkLoad *hbl = (ExeUtilHBaseBulkLoad *)child(0)->castToRelExpr();

      for (CollIndex i = 0; i < inputVars().entries(); i++) {
        ValueId vid = inputVars()[i];

        if (vid.getItemExpr()->getOperatorType() == ITM_DYN_PARAM) hbl->inputParams().insert(vid);
      }
    }  // ExeUtilHBaseBulkLoad

    if (isTrueRoot() && child(0) && (child(0)->getOperatorType() == REL_EXE_UTIL) &&
        (((ExeUtilExpr *)child(0)->castToRelExpr())->getExeUtilType() == ExeUtilExpr::SNAPSHOT_UPDATE_DELETE_))
      *CmpCommon::diags() << DgSqlCode(-3242)
                          << DgString0("This statement cannot be performed by prepared with parameters");
    ...
    if (CmpCommon::context()->GetMode() == STMT_DYNAMIC) {
      for (CollIndex i = inputVars().entries(); i--;) {
        ...
        if (hostVar->getOperatorType() == ITM_HOSTVAR && hostVar->isPrototypeValid() &&
            (hostVar->isEnvVar() || hostVar->isDefine()))
  ...
  for (CollIndex i = 0; i < bindWA->inputFunction().entries(); i++) {
    if (NOT inputVars().contains(bindWA->inputFunction()[i])) inputVars().insert(bindWA->inputFunction()[i]);
  }
  ...
  if (prevScope) inRowSubquery = prevScope->context()->inRowSubquery();
  ...
  if (inRowSubquery && (CmpCommon::getDefault(COMP_BOOL_137) == DF_OFF))
    ...
    groupByAggNodeAdded = addOneRowAggregates(bindWA, getFirstNRows() == -2 /* [last 0] case */);

  returnedRoot = transformGroupByWithOrdinalPhase2(bindWA);
  if (!returnedRoot) return NULL;
  ...
  if (orderByTree) {
    ...
    if (source.isGrouped()) {
      ...
      RETDesc *src = NULL;
      if (child(0)->child(0) && child(0)->child(0)->getOperatorType() == REL_ROOT) {
      ...
      for (i = 0; i < select.getDegree(); i++) {
        ...
        origVid = select.getOrigValueId(i);
        for (CollIndex j = 0; j < sysColList.entries(); j++) {
          if (vid == sysColList[j]->getValueId()) {
            orderBy.addColumn(bindWA, sysColList[j]->getColRefNameObj(), sysColList[j]->getValueId(), SYSTEM_COLUMN);
          }
        }
        for (CollIndex k = 0; k < usrColList.entries(); k++) {
          if (vid == usrColList[k]->getValueId() ||
              (origVid == usrColList[k]->getValueId() && origVid != NULL_VALUE_ID)) {
            orderBy.addColumn(bindWA, usrColList[k]->getColRefNameObj(), usrColList[k]->getValueId(), SYSTEM_COLUMN);
          }
        }
      }
      // 10-031125-1549 -end
      ...
      if ((specialMode) && (child(0)->getOperatorType() == REL_GROUPBY) &&
                                   ...
                                   // are also in group by clause
      {
        ...
        for (CollIndex ii = 0; ii < orderByList.entries(); ii++) {
          ...
          if (colRef && colRef->getOperatorType() == ITM_REFERENCE) {
            ...
            for (CollIndex k = 0; k < usrColList.entries(); k++) {
              if (obyColRef->getColRefNameObj().getColName() == usrColList[k]->getColRefNameObj().getColName()) {
                orderBy.delColumn(bindWA, usrColList[k]->getColRefNameObj(), SYSTEM_COLUMN);

                orderBy.addColumn(bindWA, usrColList[k]->getColRefNameObj(), usrColList[k]->getValueId(),
                                  SYSTEM_COLUMN);
                break;
              }  // if
            }    // for
      ...
      for (i = 0; i < select.getDegree(); i++) orderBy.delColumn(bindWA, select.getColRefNameObj(i), SYSTEM_COLUMN);
      for (i = 0; i < source.getDegree(); i++) orderBy.addColumnToPotenialList(source.getColRefNameObj(i));
    } else {
    ...
    for (i = 0; i < select.getDegree(); i++)
      orderBy.addColumn(bindWA, select.getColRefNameObj(i), select.getValueId(i), USER_COLUMN);
    ...
    ValueIdList &pRRO = getParentForRowsetReqdOrder() ? getParentForRowsetReqdOrder()->rowsetReqdOrder_ : reqdOrder();
    ...
    while (sPtr != NULL) {
      ...
      if (sPtr->getOperatorType() == ITM_SEL_INDEX) {
        ...
        if (selIndex == 0 || selIndex > selListCount) {
          *CmpCommon::diags() << DgSqlCode(-4007) << DgInt0((int)si->getSelIndex()) << DgInt1(selListCount);
          bindWA->setErrStatus();
          return NULL;
        }
      ...
      if ((ePtr->getArity() == 2) && ePtr->child(1) != NULL && ePtr->child(1)->getOperatorType() != ITM_ITEM_LIST &&
          childIdx != 1)
        childIdx = 1;
      else
        childIdx = 0;
    ...
    if (onlyOneEntry) orderByTree = ePtr;
    ...
    if (orderByTree->isOrderSyblingsBy()) {
      ...
      if (context->connectBy()) {
        short rc = ((ConnectBy *)context->connectBy())->setOrderSiblingsItemExpr(orderByTree, bindWA);
        if (rc == -1) return NULL;
      } else {
        *CmpCommon::diags() << DgSqlCode(-4466) << DgString0("Order Siblings By must used with Connect By operator");
        bindWA->setErrStatus();
        return this;
      }
      ((ConnectBy *)context->connectBy())
          ->getOrderSiblingsItem()
          ->convertToValueIdList(((ConnectBy *)context->connectBy())->reqdOrder(), bindWA, ITM_ITEM_LIST);
    } else
      origOrderByList.convertToItemExpr()->convertToValueIdList(pRRO, bindWA, ITM_ITEM_LIST);
  ...
  // grouped or ordered.
  for (int selIndex = 0; selIndex < compExpr().entries(); selIndex++) {
  ...
  for (int selIndex = 0; selIndex < compExpr().entries(); selIndex++) {
    ...
    if (ie->getOperatorType() == ITM_BASECOLUMN) {
      if (((BaseColumn *)ie)->getColName() == "CONNECT_BY_ISLEAF") bindWA->connectByHasIsLeaf_ = TRUE;
    }
  }
  ...
  if (updateColTree_) {
    ...
    if (bindWA->errStatus()) {
      if (CmpCommon::diags()->contains(-4001)) *CmpCommon::diags() << DgSqlCode(-4117);
      return NULL;
    }
  ...
  if (ActiveControlDB()->getRequiredShape()) {
    ...
    if (!child(0)->isAControlStatement() && op != REL_DESCRIBE && op != REL_EXPLAIN && op != REL_DDL &&
        op != REL_LOCK && op != REL_UNLOCK && op != REL_SET_TIMEOUT && op != REL_STATISTICS &&
        op != REL_QUERYINVALIDATION && op != REL_TRANSACTION && op != REL_EXE_UTIL)
  ...
  if (numExtractStreams_ > 0) {
    ...
    ComUInt32 numConfiguredESPs = CURRSTMT_OPTDEFAULTS->getTotalNumberOfCPUs();

    if ((numExtractStreams == 1) || (numExtractStreams > numConfiguredESPs)) {
      *CmpCommon::diags() << DgSqlCode(-4119) << DgInt0((int)numConfiguredESPs);
      bindWA->setErrStatus();
      return NULL;
    }
    ...
    if (reqdShape_ == NULL) {
      ...
      reqdShape_ = new (bindWA->wHeap())
          ExchangeForceWildCard(new (bindWA->wHeap()) CutOp(0), ExchangeForceWildCard::FORCED_ESP_EXCHANGE,
                                ExchangeForceWildCard::ANY_LOGPART, (int)numExtractStreams_);
    } else {
      ...
      if (reqdShape_->getOperatorType() == REL_FORCE_EXCHANGE) {
        ...
        int howMany = exch->getHowMany();
        if (whichType == ExchangeForceWildCard::FORCED_ESP_EXCHANGE && howMany == (int)numExtractStreams_) {
          reqdShapeIsOK = TRUE;
        }
      }
      ...
      if (reqdShapeIsOK) {
        ...
        reqdShape_ =
            new (bindWA->wHeap()) ExchangeForceWildCard(child, ExchangeForceWildCard::FORCED_ESP_EXCHANGE,
                                                        ExchangeForceWildCard::ANY_LOGPART, (int)numExtractStreams_);
      }
  ...
  if (isTrueRoot()) {
    castComputedColumnsToAnsiTypes2(bindWA, getRETDesc(), compExpr());
    if (bindWA->errStatus()) return NULL;
  }
  ...
  if (isTrueRoot() && bindWA->hasDynamicRowsetsInQuery()) {
    ...
    for (ValueId id = inputs.init(); inputs.next(id); inputs.advance(id)) {
      ...
      while (j < inputVars().entries()) {
        ...
        if ((ieType != ITM_DYN_PARAM) || (((DynamicParam *)ie)->getRowsetSize() == 0)) {
  ...
  if (isTrueRoot()) {
    ...
    if (bindWA->inDDL()) ddlStoiList_.insert(bindWA->getStoiList());
    ...
    if (bindWA->getTriggersList()) {
      triggersList_ = new (bindWA->wHeap()) LIST(ComTimestamp)(bindWA->wHeap(), bindWA->getTriggersList()->entries());
    ...
    if (bindWA->getUninitializedMvList()) {
      uninitializedMvList_ =
          new (bindWA->wHeap()) UninitializedMvNameList(bindWA->wHeap(), bindWA->getUninitializedMvList()->entries());
  ...
  if ((isTrueRoot()) && (bindWA->isMergeStatement())) {
  ...
  if (getFirstNRowsParam()) {
    ...
    const SQLInt si(NULL, FALSE, FALSE);
  ...
  if ((getFirstNRows() != -1) || (getFirstNRowsParam())) {
    ...
    if (inRowSubquery) {
      ...
      if (groupByAggNodeAdded && ((getFirstNRows() == 1) ||   // [first 1] or [any 1]
                                  (getFirstNRows() == -2) ||  // [last 0]
                                  (getFirstNRows() == -3) ||  // [last 1]
                                  (getFirstNRowsParam())))    // dynamic parameter
      {
  ...
  if (assignmentStTree() && bindWA->getAssignmentStArea() && bindWA->getAssignmentStArea()->getAssignmentStHostVars() &&
      !bindWA->getAssignmentStArea()->getAssignmentStHostVars()->updateValueIds(compExpr(), assignmentStTree())) {
    bindWA->setErrStatus();
    return NULL;
  }
  ...
  if ((isTrueRoot()) && (bindWA->hasCompositeExpr())) setHasCompositeExpr(TRUE);
...
RelExpr *RelRoot::rewriteFirstNOrderBySubquery(BindWA *bindWA) {
  ...
  Aggregate *rowNumberOverOrderBy = new (bindWA->wHeap()) Aggregate(
      ITM_COUNT, new (bindWA->wHeap()) SystemLiteral(1), FALSE /*i.e. not distinct*/, ITM_COUNT_STAR__ORIGINALLY, '!');
...
ItemExpr *RelRoot::selectList() { return compExpr().rebuildExprTree(ITM_ITEM_LIST); }  // RelRoot::selectList()
...
bool OptSqlTableOpenInfo::checkColPriv(const PrivType privType, const PrivMgrUserPrivs *pPrivInfo) {
  CMPASSERT(pPrivInfo);
  ...
  if (!isColumnPrivType(privType)) {
    *CmpCommon::diags() << DgSqlCode(-4481) << DgString0(PrivMgrUserPrivs::convertPrivTypeToLiteral(privType).c_str())
                        << DgString1(table->getTableName().getQualifiedNameAsAnsiString()) << DgString2(columns);
    return false;
  }
  ...
  LIST(int) *colList = NULL;
  switch (privType) {
    case INSERT_PRIV: {
      colList = (LIST(int) *)&(getInsertColList());
      break;
    }
    case UPDATE_PRIV: {
      colList = (LIST(int) *)&(getUpdateColList());
      break;
    }
    case SELECT_PRIV: {
      colList = (LIST(int) *)&(getSelectColList());
      break;
    }
    default:
  ...
  for (size_t i = 0; i < colList->entries(); i++) {
    ...
    if (!(pPrivInfo->hasColPriv(privType, columnNumber))) {
      ...
      if (firstColumn && collectColumnNames) {
        columns += " ";
        firstColumn = false;
      } else if (collectColumnNames)
        columns += ", ";

      if (collectColumnNames) columns += table->getNAColumnArray()[columnNumber]->getColName();
    }
  }

  if (collectColumnNames) columns += ")";
...
NABoolean RelRoot::checkFirstNRowsNotAllowed(BindWA *bindWA) {
  ...
  if ((getFirstNRows() >= 0 || getFirstNRows() == -2 || getFirstNRows() == -3) &&  // this root has firstn
      (!((getInliningInfo().isEnableFirstNRows()) ||
         (getHostArraysArea() &&
          getHostArraysArea()->getHasSelectIntoRowsets()) ||  // firstn is allowed with a rowset subroot
                                  ...
                                  // within a cs
  {
    ...
    if (CmpCommon::getDefault(ALLOW_FIRSTN_IN_SUBQUERIES) == DF_OFF) return TRUE;
  }
  return FALSE;
}
...
NABoolean RelRoot::checkPrivileges(BindWA *bindWA) {
  ...
  if (Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL)) return TRUE;
  ...
  if (bindWA->getStoiList().entries() == 0 && bindWA->getUdrStoiList().entries() == 0 &&
  ...
  // Check privileges for hive tables when Sentry is enabled even if the current
  // user is the root user.  Sentry depends on native privs not Trafodion privs
  // Currently we only support tables for Sentry so getStoiList must have entries
  NABoolean usingSentry = NATable::usingSentry();
  NABoolean usingLinuxGroups = NATable::usingLinuxGroups();
  NABoolean usingSentryUserApi = FALSE;
  if (CmpCommon::getDefault(PRIV_SENTRY_USE_USERNAME_API) == DF_ON) usingSentryUserApi = TRUE;

  NABoolean allowRoot = (qiPath.length() == 0) && !(usingSentry && bindWA->getStoiList().entries() > 0);
  if (allowRoot && ComUser::isRootUserID()) return TRUE;
  ...
  for (int i = 0; i < (int)bindWA->getStoiList().entries(); i++) {
    ...
    // If this is not a hive table and root is the current user, then the
    // operation is allowed on this table
    if (!tab->isHiveTable() && ComUser::isRootUserID() && (qiPath.length() == 0))
      continue;  // check the rest of the tables though

    // System metadata tables do not, by default, have privileges stored in the
    // NATable structure. So we retrieve them now. Also, for Hive Sentry-secured
    // tables, we check to see if the privilege object is too old, and if so,
    // we re-retrieve the privileges.
    PrivMgrUserPrivs *pPrivInfo = tab->getPrivInfo();
    PrivMgrUserPrivs privInfo;
    if ((!pPrivInfo) || (usingSentry && tab->isHiveTable() &&
                         pPrivInfo->isStale(&now, CmpCommon::getDefaultLong(PRIV_SENTRY_RECHECK_INTERVAL)))) {
      if (pPrivInfo)
        pPrivInfo->reInit();  // clear out old info so we can reuse object
      else
        pPrivInfo = &privInfo;  // use new object on the stack
      ...
      retcode = privInterface.getPrivileges(tab, thisUserID, usingSentryUserApi, *pPrivInfo, &secKeySet);
      cmpSBD.switchBackCompiler();

      if (retcode != STATUS_GOOD) {
        tab->setRemoveFromCacheBNC(TRUE);
        bindWA->setFailedForPrivileges(TRUE);
        if (!CmpCommon::diags()->containsError(-1034)) *CmpCommon::diags() << DgSqlCode(-1034);
        return FALSE;
      }
    }
  ...
  if (bindWA->getUdrStoiList().entries()) {
    for (int i = 0; i < (int)bindWA->getUdrStoiList().entries(); i++) {
      ...
      if (QI_enabled && (rtn->getSecKeySet().entries() > 0)) insertQIKeys = TRUE;
      ...
      // Verify that the user has execute priv
        ...
        if (pPrivInfo->hasPriv(EXECUTE_PRIV)) {
          ...
          if (insertQIKeys) findKeyAndInsertInOutputList(rtn->getSecKeySet(), userHashValue, EXECUTE_PRIV, bindWA);
        }

        // plan requires privilege but user has none, report an error
        else {
          RemoveNARoutineEntryFromCache = TRUE;
          *CmpCommon::diags() << DgSqlCode(-4482) << DgString0("EXECUTE") << DgString1(udrStoi->getUdrName());
        }
      }
      ...
      if (RemoveNARoutineEntryFromCache) {
        ...
        if (cachedNARoutine != NULL) pRoutineDBCache->moveRoutineToDeleteList(cachedNARoutine, &key);
      }
    }  // for loop over UDRs
  }    // end if any UDRs.
  ...
  for (int i = 0; i < (int)bindWA->getSeqValList().entries(); i++) {
    ...
    if (!pPrivInfo) {
      ...
      retcode = privInterface.getPrivileges(tab, thisUserID, usingSentryUserApi, privInfo, &secKeySet);
      cmpSBD.switchBackCompiler();
      if (retcode != STATUS_GOOD) {
        bindWA->setFailedForPrivileges(TRUE);
    ...
    NABoolean insertQIKeys = FALSE;
    if (QI_enabled && (secKeySet.entries()) > 0) insertQIKeys = TRUE;
    if (pPrivInfo->hasPriv(USAGE_PRIV)) {
      // do this only if QI is enabled and object has security keys defined
      if (insertQIKeys) findKeyAndInsertInOutputList(secKeySet, userHashValue, USAGE_PRIV, bindWA);
    }
    ...
    // plan requires privilege but user has none, report an error
      ...
      bindWA->setFailedForPrivileges(TRUE);
      tab->setRemoveFromCacheBNC(TRUE);  // To be removed by CmpMain before Compilation retry
      *CmpCommon::diags() << DgSqlCode(-4491) << DgString0("USAGE")
...
//   COM_QI_GROUP_GRANT_ROLE: privileges granted through a role on user's groups
...
//       or when one of these roles is revoked from user or the user's group
//   if a privilege has been granted directly on an schema to a user:
//     add UserSchemaKey
//       invalidation is enforced when priv is revoked from the user
//   if a privilege has been granted directly on a schema to a role:
//     add an entry to SchemaRoleKeys
//       invalidation is enforced when priv is revoked from one of these roles,
//       or when one of these roles is revoked from user or the user's group
//
// ****************************************************************************
void RelRoot::findKeyAndInsertInOutputList(ComSecurityKeySet KeysForTab, const uint32_t userHashValue,
                                           const PrivType which, BindWA *bindWA) {
  // If no keys associated with object, just return
  if (KeysForTab.entries() == 0) return;
  ...
  // Get action type for UserObjectKey and UserSchemaKey based on the privilege (which)
  ...
  ComQIActionType objectActionType = dummyKey.convertBitmapToQIActionType(which, ComSecurityKey::OBJECT_IS_OBJECT);
  ComQIActionType schemaActionType = dummyKey.convertBitmapToQIActionType(which, ComSecurityKey::OBJECT_IS_SCHEMA);
  ...
  for (int ii = 0; ii < (int)(KeysForTab.entries()); ii++) {
    ...
    if (thisKey->getSecurityKeyType() == columnActionType) {
      if (thisKey->getSubjectHashValue() == userHashValue) {
        ...
        if (!UserColumnKey) UserColumnKey = thisKey;
      }
    ...
    // See if the key is schema related
    else if (thisKey->getSecurityKeyType() == schemaActionType) {
      if (thisKey->getSubjectHashValue() == userHashValue) {
        // Found a security key for the schemaActionType
        if (!UserSchemaKey) UserSchemaKey = thisKey;
      }
      // Found a security key for a role
      else if (qiSubjectMatchesRole(thisKey->getSubjectHashValue())) {
        SchemaRoleKeys.insert(*thisKey);
      }
    }
    // See if the key is object related
      ...
      if (thisKey->getObjectHashValue() == hashValueOfPublic) {
        if (!UserObjectPublicKey) UserObjectPublicKey = thisKey;
      }
    }

    else {
      ;
    }  // Not right action type, just continue traversing.
  }

  // Determine best key (fewer invalidations are required)
  //   UserObjectKeys are better than ObjectRoleKeys
  //   ObjectRoleKeys are better than UserSchemaKeys
  //   UserSchemaKeys are better the SchemaRoleKeys
  if (UserObjectKey)
  ...
  else if (UserSchemaKey)
    securityKeySet_.insert(*UserSchemaKey);

  else if (SchemaRoleKeys.entries() > 0) {
    for (int j = 0; j < SchemaRoleKeys.entries(); j++) {
      securityKeySet_.insert(SchemaRoleKeys[j]);

      // add a key in case the role is revoked from the user
      ComSecurityKey roleKey(userHashValue, SchemaRoleKeys[j].getSubjectHashValue());
      securityKeySet_.insert(roleKey);
    }
  }
  ...
  if (UserObjectPublicKey != NULL) securityKeySet_.insert(*UserObjectPublicKey);
}
...
RelExpr *GroupByAgg::bindNode(BindWA *bindWA) {
  ...
  if ((getGroupAttr()->isEmbeddedUpdateOrDelete()) || (bindWA->isEmbeddedIUDStatement())) {
  ...
  if (groupExprTree) {
    ...
    if (isRollup()) groupExprTree->convertToValueIdList(rollupGroupExprList(), bindWA, ITM_ITEM_LIST);
    ...
    if ((groupExprTree != NULL) && (getParentRootSelectList() != NULL)) {
      ...
      if (getSelPredTree()) {
        ...
        replaceRenamedColInHavingWithSelIndex(bindWA, havingPred, origSelectList, replaced,
  ...
  if ((havingPred) && (selIndexInHaving())) {
  ...
  if (aggregateExprTree_) {  // only Binder, not Parser, should put anything here
    ...
    CMPASSERT(aggregateExprTree_->nodeIsBound() || aggregateExprTree_->child(0)->nodeIsBound());
  ...
  aggregateExpr_ += currScope->getUnresolvedAggregates();
  currScope->getUnresolvedAggregates().clear();
  if (getRETDesc()) getRETDesc()->setGroupedFlag();
...
void Scan::bindUpdateCurrentOf(BindWA *bindWA, NABoolean updateQry) {
  ...
  for (i = 0; i < keyList.entries(); i++) {
    ...
    ItemExpr *eqPred = new (bindWA->wHeap()) BiRelat(ITM_EQUAL, vid.getItemExpr(), hv);
  ...
  if (updateQry) {
    ...
    for (i = 0; i < nonKeyColList.entries(); i++) {
      ...
      ItemExpr *eqPred = new (bindWA->wHeap()) BiRelat(ITM_EQUAL, vid.getItemExpr(), hv, TRUE);

      if (!updCheckPtr)
        updCheckPtr = eqPred;
      else
        updCheckPtr = new (bindWA->wHeap()) BiLogic(ITM_AND, updCheckPtr, eqPred);
    }  // loop over all pkey columns
...
OptSqlTableOpenInfo *setupStoi(OptSqlTableOpenInfo *&optStoi_, BindWA *bindWA, const RelExpr *re,
                               const NATable *naTable, const CorrName &corrName, NABoolean noSecurityCheck) {
  ...
  NAString fileName(naTable->getViewText() ? (NAString)naTable->getViewFileName()
                                           : naTable->getClusteringIndex()->getFileSetName().getQualifiedNameAsString(),
                    bindWA->wHeap());

  SqlTableOpenInfo *stoi_ = new (bindWA->wHeap()) SqlTableOpenInfo;

  optStoi_ = new (bindWA->wHeap()) OptSqlTableOpenInfo(stoi_, corrName, bindWA->wHeap());

  stoi_->setFileName(convertNAString(fileName, bindWA->wHeap()));

  if (naTable->getIsSynonymTranslationDone()) {
    stoi_->setAnsiName(convertNAString(naTable->getSynonymReferenceName(), bindWA->wHeap()));
  } else {
    stoi_->setAnsiName(convertNAString(naTable->getTableName().getQualifiedNameAsAnsiString(), bindWA->wHeap()));
  }
  ...
  if (NOT corrName.getCorrNameAsString().isNull()) {
    ...
    if (corrNameSpecified) {
      stoi_->setCorrName(convertNAString(corrName.getCorrNameAsString(), bindWA->wHeap()));
    }
  }
  ...
  if (naTable->isHbaseTable()) stoi_->setIsHbase(TRUE);

  stoi_->setLocationSpecified(corrName.isLocationNameSpecified() || corrName.isPartitionNameSpecified());
  ...
  if (!naTable->getViewText()) stoi_->setIsAudited(naTable->getClusteringIndex()->isAudited());

  switch (re->getOperatorType()) {
    case REL_UNARY_INSERT:
    case REL_LEAF_INSERT:
      stoi_->setInsertAccess();
      if (((Insert *)re)->getOverwriteHiveTable()) stoi_->setDeleteAccess();
      break;
    case REL_UNARY_UPDATE: {
      stoi_->setUpdateAccess();
      if (((GenericUpdate *)re)->isMerge()) stoi_->setInsertAccess();
    } break;
  ...
  if ((naTable->getClusteringIndex() && naTable->getClusteringIndex()->isSystemTable()) || (NOT validateTS))
    stoi_->setValidateTimestamp(FALSE);
  else
    stoi_->setValidateTimestamp(TRUE);

  stoi_->setUserBaseTable(naTable->isUserBaseTable());
  if (naTable->DDLValidationRequired()) {
    if (stoi_->getInsertAccess() || stoi_->getUpdateAccess() || stoi_->getDeleteAccess()) {
      stoi_->setDDLValidationRequired(TRUE);
      stoi_->setExpectedEpoch(naTable->expectedEpoch());
      stoi_->setExpectedFlags(naTable->expectedFlags());
    }
  }
  ...
  if (re->getInliningInfo().isAvoidSecurityCheck() ||
      (bindWA->isBindingMvRefresh() && (!naTable->isAnMV() || !stoi_->getInsertAccess()))) {
    return NULL;
  }
  ...
  if (re->getOperator().match(REL_ANY_GEN_UPDATE) && (((GenericUpdate *)re)->getUpdateCKorUniqueIndexKey())) {
    return NULL;
  }
  ...
  if (!stoiInList) {
    stoiInList = new (bindWA->wHeap())
        OptSqlTableOpenInfo(new (bindWA->wHeap()) SqlTableOpenInfo(*stoi_), corrName, bindWA->wHeap());
...
RelExpr *Scan::bindNode(BindWA *bindWA) {
  ...
  if (Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL)) setFirstReadBypassTM(FALSE);
  ...
  NABoolean isLookForIndex = FALSE;
  NAString indexName;
  if (getTableName().isPartitionV2() && getTableName().getSpecialType() == ExtendedQualName::INDEX_TABLE) {
    int cliRC = 0;
    NAString partitionIndexName;
    ExeCliInterface cliInterface(CmpCommon::statementHeap());
    cliRC = CmpSeabaseDDL::getPartIndexName(&cliInterface, bindWA->getDefaultSchema().getSchemaName(),
                                            getTableName().getQualifiedNameObj().getObjectName(),
                                            getTableName().getPartnClause().getPartitionName(), partitionIndexName);
    ...
    CorrName ca(partitionIndexName);
    ca.setSpecialType(ExtendedQualName::INDEX_TABLE);
    setTableName(ca);
  }
  ...
  if (getTableName().isPartitionV2() && NOT naTable->isPartitionV2Table()) {
    *CmpCommon::diags() << DgSqlCode(-8304) << DgString0(naTable->getTableName().getQualifiedNameAsAnsiString());
    bindWA->setErrStatus();
    return this;
  }

  if (naTable->isPartitionV2IndexTable() && naTable->getSpecialType() == ExtendedQualName::INDEX_TABLE) {
    *CmpCommon::diags() << DgSqlCode(-3242)
                        << DgString0("CAN NOT SELECT FROM A VIRTUAL TABLE WITHOUT A PHYSICAL TABLE");
    bindWA->setErrStatus();
    return this;
  }

  if (naTable->isHiveTable()) {
    if (naTable->getClusteringIndex() && (naTable->getClusteringIndex()->numHivePartCols() > 0)) {
      if (NOT msg_license_advanced_enabled()) {
        *CmpCommon::diags() << DgSqlCode(-4222) << DgString0("Partitioned Hive");
        bindWA->setErrStatus();
        return this;
      }
    }
  ...
  if (naTable->getViewText()) {
    ...
    // boundView == NULL implies there are errors in binding the view.
    // No more error is set here as we assume bindWA->setErrStatus() is
    // called in BindWA::bindView().
    if (!boundView) {
      //    restore ALLOW_SPECIALTABLETYPE setting
      if (specialTableFlagOn == FALSE) Reset_SqlParser_Flags(ALLOW_SPECIALTABLETYPE);
    ...
    if (!naTable->isUpdatable() && getGroupAttr()->isEmbeddedUpdateOrDelete()) {
      *CmpCommon::diags() << DgSqlCode(-4206) << DgTableName(naTable->getTableName().getQualifiedNameAsAnsiString())
                          << (getGroupAttr()->isEmbeddedUpdate() ? DgString0("UPDATE") : DgString0("DELETE"));
      bindWA->setErrStatus();
    ...
    if (!naTable->isUpdatable() && getGroupAttr()->isStream()) {
      *CmpCommon::diags() << DgSqlCode(-4151) << DgTableName(naTable->getTableName().getQualifiedNameAsAnsiString());
      bindWA->setErrStatus();
      if (specialTableFlagOn == FALSE) Reset_SqlParser_Flags(ALLOW_SPECIALTABLETYPE);
      return NULL;
    }
    ...
    if (boundView->getLeftmostScanNode()) {
      ...
      const NATable *baseTable = boundView->getLeftmostScanNode()->getTableDesc()->getNATable();

      if (getGroupAttr()->isStream()) {
        if (!baseTable->getClusteringIndex()->isKeySequenced()) {
          *CmpCommon::diags() << DgSqlCode(-4204)
                              << DgTableName(baseTable->getTableName().getQualifiedNameAsAnsiString());
          bindWA->setErrStatus();
          if (specialTableFlagOn == FALSE) Reset_SqlParser_Flags(ALLOW_SPECIALTABLETYPE);
  ...
  NABoolean os = FALSE;
  if ((bindWA->overrideSchemaEnabled()) && (!naTable->getSynonymReferenceName().isNull())) {
    os = bindWA->getToOverrideSchema();
    bindWA->setToOverrideSchema(FALSE);
  }

  if (getHint()) bindHint(getHint(), bindWA);

  TableDesc *tableDesc = NULL;

  if ((NOT isHbaseScan()) || (!getTableDesc())) {
    tableDesc = bindWA->createTableDesc(naTable, getTableName(), FALSE, getHint());
  } else {
    tableDesc = getTableDesc();
  }

  RelExpr *unionExpr = NULL;
  if (naTable->isPartitionV2Table()) {
    NABoolean hasPartMatched = TRUE;
    LIST(NAString) *partNames = NULL;
    LIST(ValueIdSet) partPreds(bindWA->wHeap());
    ItemExpr *selPred = selPredTree();
    ItemExpr *tmpPred = NULL;
    if (selPred) tmpPred = selPred->copyTree(bindWA->wHeap());
    if (isPartnNameSpecified()) {
      NAString partnName = getSpecPartnName();
      partNames = tableDesc->getMatchedPartInfo(bindWA, partnName);
      if (!partNames || partNames->entries() == 0) {
        // the specified partition does not exist
        *CmpCommon::diags() << DgSqlCode(-2301) << DgString0(partnName);
        bindWA->setErrStatus();
        return NULL;
      }
    } else if (isPartnValuesSpecified() == TRUE) {
      ItemExpr *valExpr = getSpecPartnVals();
      partNames = tableDesc->getMatchedPartions(bindWA, (ItemList *)valExpr);
      if (!partNames || partNames->entries() == 0) {
        // the specified partition does not exist
        *CmpCommon::diags() << DgSqlCode(-2302);
        bindWA->setErrStatus();
        return NULL;
      }
    } else if (tmpPred)  // has selection predicate
    {
      partNames = tableDesc->getMatchedPartInfo(bindWA, tmpPred);
      // filter condition is out-of-range
      if (partNames->entries() == 0) {
        // no partition match for the selection predicates
        hasPartMatched = FALSE;
      }
    } else {
      partNames = tableDesc->getMatchedPartInfo(bindWA->wHeap());
      CMPASSERT(partNames);
    }

    if (hasPartMatched) {
      Scan *leftScan = new (bindWA->wHeap())
          Scan(CorrName((*partNames)[0], bindWA->wHeap(), tableDesc->getNATable()->getTableName().getSchemaName(),
                        tableDesc->getNATable()->getTableName().getCatalogName()));
      leftScan->setPartitionPruned(TRUE);
      leftScan->getGroupAttr()->setEmbeddedIUD(getGroupAttr()->getEmbeddedIUD());
      if (selPred) {
        // add selection predicate to new scan node
        leftScan->addSelPredTree(selPred->copyTree(bindWA->wHeap())->castToItemExpr());
      }

      RelExpr *newNode = leftScan;
      // if more than one partition matched, generate Union node
      for (int i = 1; i < (*partNames).entries(); i++) {
        Scan *rightScan = new (bindWA->wHeap())
            Scan(CorrName((*partNames)[i], bindWA->wHeap(), tableDesc->getNATable()->getTableName().getSchemaName(),
                          tableDesc->getNATable()->getTableName().getCatalogName()));
        rightScan->setPartitionPruned(TRUE);
        rightScan->getGroupAttr()->setEmbeddedIUD(getGroupAttr()->getEmbeddedIUD());
        if (selPred) {
          // add selection predicate to new scan node
          rightScan->addSelPredTree(selPred->copyTree(bindWA->wHeap())->castToItemExpr());
        }

        newNode = new (bindWA->wHeap()) Union(newNode, rightScan);
        if (dynamic_cast<Union *>(newNode)) dynamic_cast<Union *>(newNode)->setUnionSystemCol();
      }
      newNode->bindNode(bindWA);
      return newNode;
    } else {
      // genereate a boolean false predicate and add it to the selection predicate
      BoolVal *falsePred = new (bindWA->wHeap()) BoolVal(ITM_RETURN_FALSE);
      addSelPredTree(falsePred);
    }
  } else if (naTable->isPartitionEntityTable()) {
    ItemExpr *selPred = selPredTree();
    tableDesc->addPartPredToPrecondition(bindWA, selPred);

    /* begin deal with golbal index add by kai.deng */
    NABoolean hasGlobalIndex = FALSE;
    LIST(IndexDesc *) idxList = tableDesc->getIndexes();
    for (int u = 0; u < idxList.entries(); u++) {
      if (idxList[u]->isPartGlobalIndex()) {
        hasGlobalIndex = TRUE;
      }
    }
    if (hasGlobalIndex) {
      // copy partition table origin check constraint to scan selpred
      ItemExpr *checkExpr = NULL;
      ValueIdList vil = tableDesc->getCheckConstraints();
      for (int u = 0; u < vil.entries(); u++) {
        ItemExpr *ie = vil[u].getItemExpr();
        if (u > 0)
          checkExpr = new (bindWA->wHeap()) ItemList(checkExpr, ie);
        else
          checkExpr = ie;
      }
      ...
      if (newSelPred)
        newSelPred = new (bindWA->wHeap()) BiLogic(ITM_AND, newSelPred, checkExpr);
      else
        newSelPred = checkExpr;
      // add to selpred
      addSelPredTree(newSelPred);
    }
    /* end deal with golbal index add by kai.deng*/
  }
  ...
  if (tableDesc && getHint() && getTableName().getSpecialType() == ExtendedQualName::NORMAL_TABLE) {
    double s;
    ...
    if (getHint()->getCardinality() >= 1.0) {
      ...
      if ((scanCard.getValue() - floor(scanCard.getValue())) > 0.00001) scanCard = ceil(scanCard.getValue());
  ...
  if ((CmpCommon::getDefault(ALLOW_DML_ON_NONAUDITED_TABLE) == DF_OFF) &&
    ...
    *CmpCommon::diags() << DgSqlCode(-4211) << DgTableName(naTable->getTableName().getQualifiedNameAsAnsiString());
    bindWA->setErrStatus();
    return NULL;
  }

  // read from one replica alone
  int replicaColPos = -1;
  NABoolean inSubquery = FALSE;
  BindScope *currScope = bindWA->getCurrentScope();
  BindScope *prevScope = bindWA->getPreviousScope(currScope);
  if (prevScope) inSubquery = prevScope->context()->inSubquery();
  if (naTable->isHbaseTable() && naTable->hasTrafReplicaColumn(&replicaColPos) &&
      !(bindWA->inGenericUpdate() && !inSubquery) && !currScope->context()->inUpsertXform() &&
      !bindWA->isIndexInsertStatement() && naTable->getSpecialType() != ExtendedQualName::INDEX_TABLE) {
    // Most scans for a Traf replicated table will have a system added predicate that will
    // ensure that data is read from one and only one replica. The replica we choose to read from
    // is determined by the cpu number of the compiler (i.e. master executor in most cases)
    ItemExpr *repCol = getTableDesc()->getColumnList()[replicaColPos].getItemExpr();
    ItemExpr *replicaPred = new (bindWA->wHeap()) BiRelat(
        ITM_EQUAL, repCol,
        new (bindWA->wHeap()) ConstValue((GetCliGlobals()->myCpu() % naTable->getNumTrafReplicas()), bindWA->wHeap()));
    ItemExpr *newSelPred = removeSelPredTree();
    if (newSelPred)
      newSelPred = new (bindWA->wHeap()) BiLogic(ITM_AND, newSelPred, replicaPred);
    else
      newSelPred = replicaPred;

    // now add the replica predicate
    addSelPredTree(newSelPred->bindNode(bindWA));
  }

  // restricted partitions for HBase table
  if (naTable->isHbaseTable() && (NOT naTable->isPartitionV2()) &&
      (naTable->isPartitionNameSpecified() || naTable->isPartitionRangeSpecified())) {
    ...
    const NAColumnArray &ccCols = (partFunc && partFunc->castToHash2PartitioningFunction())
    ...
    for (CollIndex i = 0; i < ccCols.entries() && !saltColFound; i++) {
      if (ccCols[i]->isComputedColumn() && ccCols[i]->getColName() == ElemDDLSaltOptionsClause::getSaltSysColName()) {
        saltColFound = TRUE;
        ...
        if (endPartNum < 0) endPartNum = partFunc->getCountOfPartitions() - 1;

        ItemExpr *partPred = NULL;
        ColReference *saltColRef = new (bindWA->wHeap())
            ColReference(new (bindWA->wHeap()) ColRefName(ccCols[i]->getFullColRefName(), bindWA->wHeap()));

        if (beginPartNum == endPartNum) {
          partPred = new (bindWA->wHeap())
              BiRelat(ITM_EQUAL, saltColRef, new (bindWA->wHeap()) ConstValue(beginPartNum, bindWA->wHeap()));
        } else {
          partPred =
              new (bindWA->wHeap()) Between(saltColRef, new (bindWA->wHeap()) ConstValue(beginPartNum, bindWA->wHeap()),
                                            new (bindWA->wHeap()) ConstValue(endPartNum, bindWA->wHeap()));
        }

        ItemExpr *newSelPred = removeSelPredTree();
        if (newSelPred)
          newSelPred = new (bindWA->wHeap()) BiLogic(ITM_AND, newSelPred, partPred);
        else
          newSelPred = partPred;
    ...
    if (!saltColFound) {
      ...
      *CmpCommon::diags() << DgSqlCode(-1276) << DgString0(buf)
                          << DgTableName(naTable->getTableName().getQualifiedNameAsAnsiString());
      bindWA->setErrStatus();
      return NULL;
    }
  ...
  // MV --
  if (getInliningInfo().isMVLoggingInlined()) projectCurrentEpoch(bindWA);
  ...
  if (getGroupAttr()->isStream()) {
    if (!naTable->getClusteringIndex()->isKeySequenced() || naTable->hasVerticalPartitions()) {
      *CmpCommon::diags() << DgSqlCode(-4204) << DgTableName(naTable->getTableName().getQualifiedNameAsAnsiString());
      bindWA->setErrStatus();
      return NULL;
    }
    if (!getTableDesc()->getClusteringIndex()->getNAFileSet()->isAudited()) {
      // Stream access not allowed on a non-audited table
      *CmpCommon::diags() << DgSqlCode(-4215) << DgTableName(naTable->getTableName().getQualifiedNameAsAnsiString());
      bindWA->setErrStatus();
      return NULL;
    }
  }
  ...
  if (getGroupAttr()->isEmbeddedUpdateOrDelete()) {
    if (!naTable->getClusteringIndex()->isKeySequenced() || naTable->hasVerticalPartitions()) {
      *CmpCommon::diags() << DgSqlCode(-4205) << DgTableName(naTable->getTableName().getQualifiedNameAsAnsiString())
                          << (getGroupAttr()->isEmbeddedUpdate() ? DgString0("UPDATE") : DgString0("DELETE"));
      bindWA->setErrStatus();
      return NULL;
    }
  ...
  const NAString *tableLockVal = ActiveControlDB()->getControlTableValue(getTableName().getUgivenName(), "TABLELOCK");
  if (*tableLockVal == "ON") naTable->setRefsIncompatibleDP2Halloween();
  ...
  // conditions occur.
  if (getGroupAttr()->isEmbeddedUpdateOrDelete() && naTable->getClusteringIndex()->isPartitioned() &&
      (CmpCommon::getDefault(ATTEMPT_ASYNCHRONOUS_ACCESS) == DF_OFF)) {
    *CmpCommon::diags() << DgSqlCode(-4321) << DgTableName(naTable->getTableName().getQualifiedNameAsAnsiString());
    bindWA->setErrStatus();
    return NULL;
  }
  ...
  if (getGroupAttr()->isStream() && (CmpCommon::getDefault(ATTEMPT_ASYNCHRONOUS_ACCESS) == DF_OFF)) {
    NABoolean atleastonenonpartitionedaccess = FALSE;
    NAFileSetList idescList = naTable->getIndexList();

    for (CollIndex i = 0; i < idescList.entries() && !atleastonenonpartitionedaccess; i++)
      if (!(idescList[i]->isPartitioned())) atleastonenonpartitionedaccess = TRUE;

    if (!atleastonenonpartitionedaccess) {
      *CmpCommon::diags() << DgSqlCode(-4320) << DgTableName(naTable->getTableName().getQualifiedNameAsAnsiString());
      bindWA->setErrStatus();
      return NULL;
    }
  }
  ...
  if (getOptHbaseAccessOptions()) {
    ...
    if ((getOptHbaseAccessOptions()->getNumVersions() != 0) && (naTable && naTable->isSQLMXAlignedTable())) {
      *CmpCommon::diags() << DgSqlCode(-3242)
                          << DgString0("versions option cannot be used on an aligned format table.");
  ...
  if (getTableDesc()->getNATable()->hasCompositeColumns()) bindWA->setHasCompositeExpr(TRUE);
...
void Scan::projectCurrentEpoch(BindWA *bindWA) {
  ItemExpr *currEpoch = new (bindWA->wHeap()) GenericUpdateOutputFunction(ITM_CURRENTEPOCH);
      ...
      if (cols.entries() > 1 && subq->getDegree() > 1) {
        ...
        *CmpCommon::diags() << DgSqlCode(-4125);
        ...
        if (CmpCommon::getDefault(COMP_BOOL_137) == DF_ON) {
          ...
          ItemExpr *agg = new (bindWA->wHeap()) Aggregate(ITM_ONE_ROW, subqSelectList.rebuildExprTree());
          RelExpr *gby = new (bindWA->wHeap()) GroupByAgg(subq->getSubquery(), REL_GROUPBY, NULL, agg);
...
RelExpr *Tuple::bindNode(BindWA *bindWA) {
  ...
  if (bindWA->getHostArraysArea() && !bindWA->getHostArraysArea()->done()) {
    ...
    if (boundExpr) return checkTupleElementsAreAllScalar(bindWA, boundExpr);
  }
...
RelExpr *TupleList::bindNode(BindWA *bindWA) {
  ...
  if (tupleExprTree()->containsSubquery() || tupleExprTree()->containsUDF()
#ifndef NDEBUG
    ...
    for (CollIndex i = 0; i < nTupleListEntries; i++) {
      ...
      if (castTo) {
        ...
        for (CollIndex j = 0; j < (CollIndex)itms.entries(); j++) {
          ...
          Assign *tmpAssign = new (bindWA->wHeap()) Assign(castToList()[j].getItemExpr(), src.getItemExpr());
  ...
  for (i = 0; i < nEntries; i++) {
    ...
    // Note that in convertToValueIdList() above, we traverse the itemExpr
    // tree rooted at tuple and bind only all non ITM_ITEM_LIST ItemExprs,
    // via the following line of code:
    // boundExpr = (ItemExpr *) (current_ie->bindNodeRoot(bindWA)).
    //
    // In the follpwing line of code, we bind the child(0) in case it is a
    // non-ITM_ITEM_LIST itemexpr. This is to fix mantis 10304.
    tupleList[i]->child(0) = tupleList[i]->child(0)->bindNode(bindWA);
    if (bindWA->errStatus()) return NULL;
    ...
    for (CollIndex j = 0; j < prevTupleNumEntries; j++) {
      ...
      if (i == 0) {
        ...
        // table.
        //
        vidUnion = new (bindWA->wHeap()) ValueIdUnion(vids, NULL_VALUE_ID);
      ...
      vidUnion->setSource((int)i, vidList[j]);

      if (NOT vidList[j].getItemExpr()->wasDefaultClause()) vidUnion->setWasDefaultClause(FALSE);
  ...
  if (!doInferCharSetFixup(bindWA, CharInfo::ISO88591, prevTupleNumEntries, tupleList.entries())) {
    return NULL;
  }
  ...
  for (CollIndex j = 0; j < prevTupleNumEntries; j++) {
    ...
    if (castTo) {
      ...
      //   because ItemExpr destructor will delete children,
      //   which we (and parent) are still referencing!
      Assign *tmpAssign = new (bindWA->wHeap()) Assign(castToList()[j].getItemExpr(), vidUnion);
    ...
    if (castTo) {
      ...
      if (vidUnion->getValueId().getItemExpr()->wasDefaultClause()) cnode->setWasDefaultClause(TRUE);
    } else
  ...
  if (inSubquery) {
    ...
    for (CollIndex k = 0; k < viewColumns->entries(); k++) {
      ...
      {
        ((InstantiateNull *)vid.getItemExpr())->NoCheckforLeftToInnerJoin = TRUE;
      }
...
RelExpr *TupleList::getTuple(BindWA *bindWA, ValueIdList &vidList, CollIndex i) {
  ExprValueId eVid(tupleExprTree());
  ItemExprTreeAsList tupleList(&eVid, ITM_ITEM_LIST);
  ItemExpr *tuple = tupleList[i]->child(0)->castToItemExpr();
  tuple->convertToValueIdList(vidList, bindWA, ITM_ITEM_LIST, this);
...
RelExpr *TupleList::needsCharSetFixup(BindWA *bindWA, CollIndex arity, CollIndex nTuples,
                                      NAList<NABoolean> &strNeedsFixup, NABoolean &needsFixup) {
...
RelExpr *TupleList::pushDownCharType(BindWA *bindWA, enum CharInfo::CharSet cs,
                                     NAList<const CharType *> &inferredCharType, NAList<NABoolean> &strNeedsFixup,
                                     CollIndex arity, CollIndex nTuples) {
...
// do INFER_CHARSET fixup
RelExpr *TupleList::doInferCharSetFixup(BindWA *bindWA, enum CharInfo::CharSet cs, CollIndex arity, CollIndex nTuples) {
  NABoolean needsFixup;
  NAList<NABoolean> strNeedsFixup(CmpCommon::statementHeap(), arity);
  RelExpr *result = needsCharSetFixup(bindWA, arity, nTuples, strNeedsFixup, needsFixup);
  if (!result ||      // something went wrong
    ...
    if (!pushDownCharType(bindWA, cs, inferredCharType, strNeedsFixup, arity, nTuples)) {
...
RelExpr *RenameTable::bindNode(BindWA *bindWA) {
    ...
    if (child(0)->getOperatorType() == REL_ROWSET) {
      ...
      if ((indexExpr != "") && newColNamesTree_) {
        ...
        if (indexExpr != lastString) {
          *CmpCommon::diags() << DgSqlCode(-30012) << DgString0(indexExpr)
  ...
  if (derivedColTree) {
    ...
    if (derivedColList.entries() != sourceTable.getDegree()) {
      ...
      *CmpCommon::diags() << DgSqlCode(-4016) << DgInt0(derivedColList.entries()) << DgInt1(sourceTable.getDegree());
      bindWA->setErrStatus();
      delete resultTable;
      return this;
    }
  ...
  for (i = 0; i < sourceTable.getDegree(); i++) {
    ...
    if (derivedColTree) {  // explicit derived column list
      ...
      simpleColNameStr = &((RenameCol *)derivedColList[i])->getNewColRefName()->getColName();
      if (*simpleColNameStr != "") {  // named column, not an expression
        if (resultTable->findColumn(*simpleColNameStr)) {
          ...
          *CmpCommon::diags() << DgSqlCode(-4017) << DgColumnName(errColName.getColRefAsAnsiString());
          bindWA->setErrStatus();
          delete resultTable;
          return this;
        }
  ...
  // select list.
  if (bindWA->isPropagateOpAndSyskeyColumns()) getRETDesc()->propagateOpAndSyskeyColumns(bindWA, FALSE);
  ...
  //
  bindWA->getCurrentScope()->getXTNM()->insertNames(bindWA, (CorrName &)tableName);
  ...
  if (getViewNATable()) {
    ...
    for (i = 0; i < natable->getColumnCount(); i++) {
      columnsRET[i]->setViewColPosition(((natable->getNAColumnArray())[i])->getPosition());
...
void RenameReference::prepareRETDescWithTableRefs(BindWA *bindWA) {
  ...
  // For each BindScope,
  while ((scope != NULL) && (refsToFind > refsFound)) {  // until we find all the references.
    ...
    if ((retDesc != NULL) && !retDesc->isEmpty()) {
      ...
      for (CollIndex i = 0; i < refsToFind; i++)
        ...
        if (getRefList().at(i).lookupTableName(retDesc)) refsFound++;
    }
  ...
  for (CollIndex i = 0; i < refsToFind; i++) getRefList()[i].bindRefColumns(bindWA);
}
...
int BeforeTrigger::getTargetColumn(CollIndex i,  // Index of Assign expr.
  ...
  ColRefName &targetColRefName = ((ColReference *)targetColReference)->getColRefNameObj();
...
void BeforeTrigger::doSetSemanticChecks(BindWA *bindWA, RETDesc *origRETDesc) {
  ...
  if (origRETDesc->getQualColumnList(oldCorr)) isUpdateOp = TRUE;

  for (CollIndex i = 0; i < setList_->entries(); i++) {
    // Get the name and position of the Assign target column.
    int targetColPosition = getTargetColumn(i, &currentCol, scanNaTable);
    ...
    if (targetColPosition == -1) {
      ...
      *CmpCommon::diags() << DgSqlCode(-11022) << DgColumnName(currentCol.getColName()) << DgTableName(tableName);
      bindWA->setErrStatus();
      return;
    }
    ...
    if (localCols.contains(targetColPosition)) {
      ...
      *CmpCommon::diags() << DgSqlCode(-4022) << DgColumnName(currentCol.getColName());
      bindWA->setErrStatus();
      return;
    }
    ...
    if (isUpdateOp && scanNaTable->getNAColumnArray().getColumn(targetColPosition)->isClusteringKey()) {
      // 4033 Column $0~ColumnName is a primary or clustering key column and cannot be updated.
      *CmpCommon::diags() << DgSqlCode(-4033) << DgColumnName(currentCol.getColName());
      bindWA->setErrStatus();
      return;
    }
  }
...
void BeforeTrigger::bindSetClause(BindWA *bindWA, RETDesc *origRETDesc, CollHeap *heap) {
  ...
  for (CollIndex i = 0; i < setList_->entries(); i++) {
    // Get the name and position of the Assign target column.
    int targetColPosition = getTargetColumn(i, &currentCol, NULL);
    ...
    if (whenClause_ == NULL)
      ...
      IfThenElse *ifExpr = new (heap) IfThenElse(whenClause_, setExpr, passThruExpr);
...
void BeforeTrigger::bindSignalClause(BindWA *bindWA, RETDesc *origRETDesc, CollHeap *heap) {
  if (bindWA->inDDL()) {
    ...
    ColumnNameMap *execIdCol = origRETDesc->findColumn(InliningInfo::getExecIdVirtualColName());
    ...
    if (whenClause_ != NULL) {
      if (whenClause_->getOperatorType() == ITM_AND || whenClause_->getOperatorType() == ITM_OR) {
    ...
    ItemExpr *condSignalExpr = new (heap) Case(NULL, new (heap) IfThenElse(whenAndSignal, passThruExpr, passThruExpr));
...
RelExpr *BeforeTrigger::bindNode(BindWA *bindWA) {
  ...
  CollIndex i = 0;
  for (i = 0; i < refsToFind; i++) getRefList().at(i).lookupTableName(origRETDesc);
  ...
  for (i = 0; i < refsToFind; i++) getRefList().at(i).bindRefColumns(bindWA);
  ...
  if (setList_ != NULL) bindSetClause(bindWA, origRETDesc, heap);

  // Use the bound condition to prepare the conditional SIGNAL
  // expression, on the ExecuteId "virtual column".
  if (signal_ != NULL) bindSignalClause(bindWA, origRETDesc, heap);
...
// -----------------------------------------------------------------------
static void bindInsertRRKey(BindWA *bindWA, Insert *insert, ValueIdList &sysColList, CollIndex i) {
  ...
  //
  const NAFileSet *fs = insert->getTableDesc()->getClusteringIndex()->getNAFileSet();
  ...
  //
  ItemExpr *partNum =
      new (heap) HostVar("_sys_hostVarInsertPartNum", new (heap) SQLInt(heap, FALSE, FALSE),  // int unsigned not null
  ...
  ItemExpr *rowPos =
      new (heap) HostVar("_sys_hostVarInsertRowPos", new (heap) SQLInt(heap, FALSE, FALSE),  // int unsigned not null
                         TRUE                                                                // is system-generated
      );
  ...
  ItemExpr *totNumParts = new (heap)
      HostVar("_sys_hostVarInsertTotNumParts", new (heap) SQLInt(heap, FALSE, FALSE),  // int unsigned not null
              TRUE                                                                     // is system-generated
      );
  ...
  Assign *assign = new (heap) Assign(sysColList[i].getItemExpr(), rrKey, FALSE /*not user-specified*/);
...
RelExpr *Insert::bindNode(BindWA *bindWA) {
  ...
  if (child(0) && child(0)->getOperatorType() == REL_ROOT)  // Indicating subquery
    if (child(0)->castToRelExpr() && child(0)->castToRelExpr()->getFirstNRows() >= 0)
      if (bindWA && bindWA->getCurrentScope() && bindWA->getCurrentScope()->context())
  ...
  RelExpr *tmpSubQuery = NULL;
  ItemExpr *tmpInsertColTree = NULL;
  ItemExpr *tmpOrderByTree = NULL;
  CorrName tmpTableName(getTableName());
  const NATable *tmpNaTable = bindWA->getNATableInternal(tmpTableName);
  if (bindWA->errStatus()) return this;
  if (tmpNaTable->isPartitionV2Table() || tmpNaTable->isPartitionEntityTable()) {
    tmpSubQuery = child(0)->copyTree(bindWA->wHeap());
    tmpInsertColTree = getInsertColTree();

    if (tmpInsertColTree) tmpInsertColTree = tmpInsertColTree->copyTree(bindWA->wHeap());
    tmpOrderByTree = getOrderByTree();
    if (tmpOrderByTree) tmpOrderByTree = tmpOrderByTree->copyTree(bindWA->wHeap());
  }
  ...
  if (withNoConflictCheck()) {
    if ((NOT isUpsert()) || (getInsertType() == UPSERT_LOAD)) {
      *CmpCommon::diags() << DgSqlCode(-3252) << DgString0("Can only be used with UPSERT.");
      bindWA->setErrStatus();
      return this;
    }

    if (getTableDesc()->getNATable()->isHiveTable()) {
      *CmpCommon::diags() << DgSqlCode(-3252) << DgString0("Cannot be used with hive tables.");
      bindWA->setErrStatus();
      return this;
    }

    if (getTableDesc()->hasSecondaryIndexes()) {
      *CmpCommon::diags() << DgSqlCode(-3252) << DgString0("Cannot be used with secondary indexes.");
      bindWA->setErrStatus();
      return this;
    }
    ...
    if (NOT((val) && (*val == "ON"))) {
      if (NAExecTrans(NULL)) {
        *CmpCommon::diags() << DgSqlCode(-3252) << DgString0("Cannot be used with a user transaction.");
        bindWA->setErrStatus();
        return this;
      }

      if (CmpCommon::transMode()->getAutoCommit() != TransMode::ON_) {
        *CmpCommon::diags() << DgSqlCode(-3252) << DgString0("Cannot be used if AUTOCOMMIT is OFF.");
        bindWA->setErrStatus();
        return this;
      }
    }
  ...
  if ((getTableName().isVolatile()) && (CmpCommon::context()->sqlSession()->volatileSchemaInUse()) &&
      ...
      (bindWA->isInsertSelectStatement()) && (NOT getTableDesc()->getNATable()->isVolatileTableMaterialized())) {
    ...
    //      if (NOT Get_SqlParser_Flags(NO_IMPLICIT_VOLATILE_TABLE_UPD_STATS))
    {
      ...
      RelExpr *loadVolTab = new (bindWA->wHeap()) ExeUtilLoadVolatileTable(getTableName(), this, bindWA->wHeap());
  ...
  NATable *nat = bindWA->getNATableInternal(getTableName());
  ...
  NABoolean view = nat->getViewText() != NULL;
  ...
  if (!getUpdateCKorUniqueIndexKey())
    ...
    getTableDesc()->getCCList(sysColList);
  ...
  if (columnTree || (view && someNonDefaultValuesSpecified)) {
    ...
    //
    CMPASSERT(NOT columnTree || columnTree->getOperatorType() == ITM_REFERENCE ||
    ...
    for (i = 0; i < columnLkp->getDegree(); i++) {
      ...
      if (bindWA->inDDL() && bindWA->isInTrigger()) {
        if (!userColListPtr->contains(source)) {
          ...
          *CmpCommon::diags() << DgSqlCode(-4001) << DgColumnName(colName.getColName())
      ...
      if (columnLkp->findColumn(colName)->isDuplicate()) {
        // 4022 column specified more than once
        *CmpCommon::diags() << DgSqlCode(-4022) << DgColumnName(colName.getColName());
        bindWA->setErrStatus();
        delete columnLkp;
        return boundExpr;
      }
    ...
    if (!getUpdateCKorUniqueIndexKey())
      ...
      getTableDesc()->getAllExceptCCList(tgtColList);  // system cols are also user cols for xformed updates, except CC
  ...
  if (naTable->isPartitionEntityTable() && CmpCommon::getDefault(TRAF_UPSERT_THROUGH_PARTITIONS) == DF_OFF &&
      !isSpecifyPartition()) {
    LIST(PartRange *) &partitions = tableDesc->getPartitions();
    CMPASSERT(partitions.entries() == 1);
    // add boundary predicate to subquery
    Scan *subScan = NULL;
    if (tmpSubQuery->getOperatorType() == REL_ROOT &&
        (tmpSubQuery->child(0)->getOperatorType() == REL_SCAN ||
         tmpSubQuery->child(0)->getOperatorType() == REL_LEAF_TABLE_MAPPING_UDF))
      subScan = (Scan *)(tmpSubQuery->child(0).getPtr());
    if (subScan && (subScan->getOperatorType() == REL_SCAN ||
                    subScan->getOperatorType() == REL_LEAF_TABLE_MAPPING_UDF))  // for udf(series)
    {
      ItemExprList origSelectList(((RelRoot *)tmpSubQuery)->getCompExprTree(), bindWA->wHeap());
      ValueIdList selList;
      Parser parser(bindWA->currentCmpContext());
      ...
      for (int n = 0; n < origSelectList.entries(); n++) {
        ...
        if (origSelectList[n]->getOperatorType() == ITM_REFERENCE &&
          ...
          for (int i = 0; i < list[n]; i++) {
            if (selList[n].getItemExpr()->getOperatorType() == ITM_VALUEIDUNION) {
              ValueIdUnion *valIdUnion = (ValueIdUnion *)selList[n].getItemExpr();
              const ValueIdList vidList = valIdUnion->getSources();
              CMPASSERT(vidList.entries());
              result = vidList[1].getNAColumn()->getColName();
            } else if (selList[n].getItemExpr()->getOperatorType() == ITM_BASECOLUMN)
              result = selList[n + i].getNAColumn()->getColName();
            else
              selList[n + i].getItemExpr()->unparse(result, PARSER_PHASE, USER_FORMAT_DELUXE);

            ItemExpr *tmpExpr = parser.getItemExprTree(result.data(), result.length(), CharInfo::UTF8);
            selColList.addMember(tmpExpr);
          }
        } else
          selColList.addMember(origSelectList[n]);
      }
      list.clear();
        ...
        for (int j = 0; j < tgtColCount; j++) {
          ...
          if (partColPos == tgtColPos) {
            if (partitionList[0]->getlowBoundary()) {
              if (!lowColExpr)
                lowColExpr = selColList[j];
              else
                lowColExpr = new (bindWA->wHeap()) ItemList(lowColExpr, selColList[j]);
            }

            if (partitionList[0]->getHighBoundary() && !isLastPart() && maxvalIdx != 0) {
              if (!highColExpr)
                highColExpr = selColList[j];
              else if (maxvalIdx == -1 || i < maxvalIdx)
                highColExpr = new (bindWA->wHeap()) ItemList(highColExpr, selColList[j]);
            }
            break;
          }
        }
      ...
      if (lowColExpr)
        lowColExpr = new (bindWA->wHeap()) BiRelat(ITM_GREATER_EQ, lowColExpr, partitionList[0]->getlowBoundary());
      if (highColExpr)
        highColExpr = new (bindWA->wHeap()) BiRelat(ITM_LESS, highColExpr, partitionList[0]->getHighBoundary());

      // generate selection predicate for subquery
      ItemExpr *subSelPred = NULL;
      if (lowColExpr && highColExpr)
        subSelPred = new (bindWA->wHeap()) BiLogic(ITM_AND, lowColExpr, highColExpr);
      else if (lowColExpr)
        subSelPred = lowColExpr;
      else if (highColExpr)
        subSelPred = highColExpr;
      if (subSelPred) {
        // subSelPred->display(); // debug
        subScan->addSelPredTree(subSelPred);
        tmpSubQuery->bindNode(bindWA);
        setChild(0, tmpSubQuery);
      }
    }
  }
  ...
  RelExpr *unionExpr = NULL;
  if (naTable->isPartitionV2Table()) {
    // for M-20225, retry sql compile with cache off. trying to support this in the future
    if (child(0) && child(0)->getOperatorType() == REL_TUPLE_LIST && child(0)->nodeIsNormalizedForCache())
    ...
    /*
        ...
        for ( int i = 0; i < colCount; i++ )
        {
          ...
          {
            if (colIdxArray[i] == colnoList[j])
              found++;
          }
        }
        if ( found != colCount )
        {
          // all partition columns must be included
          // in the insert list
          *CmpCommon::diags() << DgSqlCode(-2303);
          bindWA->setErrStatus();
          return NULL;
        }
    */
    NABoolean isSpecifyPartition = FALSE;
    LIST(NAString) *partNames = NULL;
    if (isPartnNameSpecified()) {
      isSpecifyPartition = TRUE;
      NAString partnName = getSpecPartnName();
      partNames = tableDesc->getMatchedPartInfo(bindWA, partnName);
      if (!partNames || partNames->entries() == 0) {
        // the specified partition does not exist
    ...
    ItemExpr *newOrderByTree = NULL;
    RelExpr *newSubQuery = NULL;
    ItemExpr *newInsertColTree = NULL;

    if (tmpOrderByTree) newOrderByTree = tmpOrderByTree->copyTree(bindWA->wHeap());
    if (tmpSubQuery) newSubQuery = tmpSubQuery->copyTree(bindWA->wHeap());
    if (tmpInsertColTree) newInsertColTree = tmpInsertColTree->copyTree(bindWA->wHeap());
    Insert *leftInsert = new (bindWA->wHeap())
        Insert(CorrName((*partNames)[0], bindWA->wHeap(), tableDesc->getNATable()->getTableName().getSchemaName(),
                        tableDesc->getNATable()->getTableName().getCatalogName()),
               NULL, REL_UNARY_INSERT, newSubQuery, newInsertColTree, newOrderByTree);
    if (NOT isPartnNameSpecified() && NOT isPartnValuesSpecified())
      leftInsert->setCheckCnstrntOfPartv2BaseTable(getTableDesc()->genPartv2BaseCheckConstraint(bindWA));
    leftInsert->setIsUpsert(isUpsert());

    if (isSpecifyPartition) leftInsert->isSpecifyPartition() = TRUE;

    RelExpr *newNode = leftInsert;
    for (int i = 1; i < (*partNames).entries(); i++) {
      if (tmpOrderByTree) newOrderByTree = tmpOrderByTree->copyTree(bindWA->wHeap());
      if (tmpSubQuery) newSubQuery = tmpSubQuery->copyTree(bindWA->wHeap());
      if (tmpInsertColTree) newInsertColTree = tmpInsertColTree->copyTree(bindWA->wHeap());
      Insert *rightInsert = new (bindWA->wHeap())
          Insert(CorrName((*partNames)[i], bindWA->wHeap(), tableDesc->getNATable()->getTableName().getSchemaName(),
                          tableDesc->getNATable()->getTableName().getCatalogName()),
                 NULL, REL_UNARY_INSERT, newSubQuery, newInsertColTree, newOrderByTree);
      if (NOT isPartnNameSpecified() && NOT isPartnValuesSpecified())
        rightInsert->setCheckCnstrntOfPartv2BaseTable(getTableDesc()->genPartv2BaseCheckConstraint(bindWA));
      rightInsert->setIsUpsert(isUpsert());

      if (i == (*partNames).entries() - 1) rightInsert->isLastPart() = TRUE;

      newNode = new (bindWA->wHeap()) Union(newNode, rightInsert);
    }
  ...
  if (tl != NULL) tl->castToList() = tgtColList;
  ...
  if ((getOperatorType() == REL_UNARY_INSERT) && (getTableDesc()->getNATable()->isHbaseRowTable())) {
  ...
  // query-expr child specified
  {
    if ((sourceTable.getDegree() != newTgtColList.entries()) && !bulkLoadIndex &&
        !getTableDesc()->getNATable()->hasTrafReplicaColumn()) {
      // 4023 degree of row value constructor must equal that of target table
      *CmpCommon::diags() << DgSqlCode(-4023) << DgInt0(sourceTable.getDegree()) << DgInt1(tgtColList.entries());
      bindWA->setErrStatus();
      return boundExpr;
    }
    ...
    for (CollIndex ii = 0; ii < bindWA->getStoiList().entries(); ii++) {
      if (getOptStoi() && getOptStoi()->getStoi()) {
        if (strcmp((bindWA->getStoiList())[ii]->getStoi()->fileName(), getOptStoi()->getStoi()->fileName()) == 0) {
    ...
    if (getBoundView()) viewColumns = getBoundView()->getRETDesc()->getColumnList();
    ...
    for (i = 0; i < tgtColList.entries() && i2 < newTgtColList.entries(); i++) {
      if (tgtColList[i] != newTgtColList[i2]) continue;
      ...
      if (DFS2REC::isFloat(sourceType.getFSDatatype()) && DFS2REC::isNumeric(targetType.getFSDatatype()) &&
          (getTableDesc()->getNATable()->getPartitioningScheme() == COM_HASH_V1_PARTITIONING ||
           getTableDesc()->getNATable()->getPartitioningScheme() == COM_HASH_V2_PARTITIONING)) {
        const NAColumnArray &partKeyCols =
            getTableDesc()->getNATable()->getClusteringIndex()->getPartitioningKeyColumns();

        for (CollIndex j = 0; j < partKeyCols.entries(); j++) {
          if (partKeyCols[j]->getPosition() == nacol->getPosition()) {
            ItemExpr *ie = source.getItemExpr();
            ItemExpr *cast = new (bindWA->wHeap()) Cast(ie, &targetType, ITM_CAST);
            cast = cast->bindNode(bindWA);
            if (bindWA->errStatus()) return NULL;
      ...
      } else if (sourceType.getTypeQualifier() == NA_CHARACTER_TYPE &&
                 targetType.getTypeQualifier() == NA_CHARACTER_TYPE) {
        CharType &srcCharType = (CharType &)sourceType;
        CharType &tgtCharType = (CharType &)targetType;

        // need to map the source as same isVarchar2() as target
        if (srcCharType.isVarchar2() != tgtCharType.isVarchar2()) {
          ItemExpr *cast = new (bindWA->wHeap()) Cast(source.getItemExpr(), &targetType, ITM_CAST, TRUE);
          cast = cast->bindNode(bindWA);
          if (bindWA->errStatus()) return NULL;
      ...
      Assign *assign = new (bindWA->wHeap()) Assign(target.getItemExpr(), source.getItemExpr(),
                                                    !getUpdateCKorUniqueIndexKey());  // no user exp for xformed update
      assign->bindNode(bindWA);
        ...
        if (!getBoundView())
          ...
          for (CollIndex k = 0; k < viewColumns->entries(); k++) {
            if ((*viewColumns)[k]->getValueId() == target) {
              stoiInList->addInsertColumn((int)k);
              found = TRUE;
      ...
      if (assign_child->getOperatorType() == ITM_CAST) {
        ...
        if (type.getTypeQualifier() == NA_CHARACTER_TYPE) {
          ...
          // to fix mantis 12019.
          // if we use the TRANSLATE node as the source, but ORDER By the original
          // column, it will cause compiler error
          if (assign_grndchld->getOperatorType() == ITM_TRANSLATE && orderByTree_ == NULL) {
      ...
      //        (<they are big and errors can occur> OR <charsets differ> OR <difference between tgt and src lengths is
      //        large>)))
      //      OR
      ...
      if (source != assign->getSource() &&
          ((assignSrcType.getTypeQualifier() == NA_CHARACTER_TYPE &&
            ...
            ((assign->getSource().getItemExpr()->getOperatorType() == ITM_CAST &&
              ...
              sourceType.getNominalSize() > CmpCommon::getDefaultNumeric(LOCAL_MESSAGE_BUFFER_SIZE) * 1024) ||
             ...
             ((CharType &)assignSrcType).getCharSet() != ((CharType &)sourceType).getCharSet() ||
             ...
             // If the source cannot be partioned/sorted on a column because we have assign-getSource in the
             // bottomValues then the cost is that data will be repartitioned with an additional exchange node. If the
             // difference in length between source and assignSrc is large then the cost of repartition is less than the
             // cost of allocating and using large buffers.
             sourceType.getNominalSize() >
                 (assignSrcType.getNominalSize() +
  ...
  PartitioningFunction *partFunc = getTableDesc()->getClusteringIndex()->getNAFileSet()->getPartitioningFunction();

  NABoolean isRRTable = partFunc && partFunc->isARoundRobinPartitioningFunction();
  ...
  if (defaultColCount) {
    NAWchar zero_w_Str[2];
    zero_w_Str[0] = L'0';
    zero_w_Str[1] = L'\0';  // wide version
    ...
    for (i = 0; i < totalColCount; i++) {
      ...
      if (sysColIx < sysColList.entries() && sysColList[sysColIx].getNAColumn()->getPosition() == i) {
      ...
      if (NOT newRecExprArray().used(i)) {
        ...
        if (isASystemColumn) {
          ...
          if (nacol->isComputedColumn()) {
            ...
            ValueId defaultExprValId = ((BaseColumn *)target.getItemExpr())->getComputedColumnExpr();
          ...
          } else if (nacol->isReplicaColumn()) {
            NAString rep("__REPLICA__");
            ItemExpr *replicaVal =
                new (bindWA->wHeap()) BiArith(ITM_MINUS, sourceTable.findColumn(rep)->getValueId().getItemExpr(),
                                              new (bindWA->wHeap()) SystemLiteral(1));
            replicaVal->bindNode(bindWA);
            defaultValueExpr = replicaVal->getValueId().getItemExpr();
          } else
        ...
        if (defaultValueStr) {
          ...
          if (nacol->getDefaultClass() == COM_CURRENT_DEFAULT || nacol->getDefaultClass() == COM_CURRENT_UT_DEFAULT ||
              ...
              nacol->getDefaultClass() == COM_SYS_GUID_FUNCTION_DEFAULT) {
        ...
        if (defaultValueExpr) {
          ...
          //
          if (castType) {
            defaultValueExpr = new (bindWA->wHeap()) Cast(defaultValueExpr, castType);
          }
          ...
          if ((defaultValueExpr->getOperatorType() == ITM_IDENTITY &&
              ...
              defaultValueExpr->getOperator() == ITM_CURRENT_CATALOG ||
              defaultValueExpr->getOperator() == ITM_CURRENT_SCHEMA) {
          ...
          if (((isUpsertLoad()) || ((isUpsert()) && (getTableDesc()->getNATable()->isSQLMXAlignedTable()))) &&
              (NOT defaultValueExpr->getOperatorType() == ITM_IDENTITY) && (NOT isASystemColumn)) {
          ...
          assign = new (bindWA->wHeap()) Assign(target.getItemExpr(), defaultValueExpr, FALSE /*Not user Specified */);
          if ((nacol->getDefaultClass() != COM_CURRENT_DEFAULT) &&
              ...
              (nacol->getDefaultClass() != COM_CATALOG_FUNCTION_DEFAULT) &&
              (nacol->getDefaultClass() != COM_SCHEMA_FUNCTION_DEFAULT) &&
              (nacol->getDefaultClass() != COM_SYS_GUID_FUNCTION_DEFAULT))
        ...
        if (!defaultValueExpr || bindWA->errStatus()) {
          ...
          *CmpCommon::diags() << DgSqlCode(-7001) << DgString0(defaultValueStr)
        ...
        if (--defaultColCount == 0) break;  // tiny performance hack
        ...
        if (nacol->getDefaultClass() == COM_IDENTITY_GENERATED_ALWAYS) {
          ...
          if ((NOT ie->wasDefaultClause()) && (!getUpdateCKorUniqueIndexKey()) &&
              (CmpCommon::getDefault(OVERRIDE_GENERATED_IDENTITY_VALUES) == DF_OFF)) {
            *CmpCommon::diags() << DgSqlCode(-3428) << DgString0(nacol->getColName());
            bindWA->setErrStatus();
            return boundExpr;
          }
        }
  ...
  if (identityColumnGeneratedAlways && NOT systemGeneratesIdentityValue()) {
    // The IDENTITY column type of GENERATED ALWAYS AS IDENTITY
    // can not be used with user specified values.
    // However, if the override CQD is set, then
    // allow user specified values to be added
    ...
    if ((CmpCommon::getDefault(OVERRIDE_GENERATED_IDENTITY_VALUES) == DF_OFF) && !getUpdateCKorUniqueIndexKey()) {
      *CmpCommon::diags() << DgSqlCode(-3428) << DgString0(identityColumnName.data());
      bindWA->setErrStatus();
      return boundExpr;
    }
  }
  ...
  // Triggers --
  NABoolean insertFromValuesList = getOperatorType() == REL_UNARY_INSERT &&
                                   (child(0)->getOperatorType() == REL_TUPLE ||       // VALUES (1,'b')
                                    child(0)->getOperatorType() == REL_TUPLE_LIST ||  // VALUES (1,'b'),(2,'Y')
                                    child(0)->getOperatorType() == REL_UNION);        // VALUES with subquery
  ...
  if (bindWA->findNextScopeWithTriggerInfo() != NULL && (getGroupAttr()->getCharacteristicInputs() != NULL) &&
  ...
  if (getIsTrafLoadPrep()) {
    ...
    if (naTable->getNumTrafReplicas() > 1) {
      *CmpCommon::diags() << DgSqlCode(-4500) << DgTableName(naTable->getTableName().getQualifiedNameAsAnsiString());
      bindWA->setErrStatus();
      return boundExpr;
    }
  }  // isLoadPrep
  ...
  if (isUpsertThatNeedsTransformation(isAlignedRowFormat, omittedDefaultCols, omittedCurrentDefaultClassCols,
                                      toMerge)) {
  ...
  if (getInsertType() == Insert::UPSERT_NO_DUP) setInsertType(Insert::SIMPLE_INSERT);  // this will change it as
  // UPSERT_INSERT. We have used the NO_DUP flag is isUpsertThatNeeds...()

  if (NOT(isMerge() || noIMneeded())) boundExpr = handleInlining(bindWA, boundExpr);
  ...
  if (CmpCommon::getDefault(ODBC_PROCESS) == DF_ON) {
    if (bindWA->getHostArraysArea() && (NOT bindWA->getHostArraysArea()->getRowwiseRowset()) &&
  ...
  if (isMtsStatement() || bulkLoadIndex) {
    if (isMtsStatement()) setRETDesc(new (bindWA->wHeap()) RETDesc(bindWA, getTableDesc()));
...
*/
NABoolean Insert::isUpsertThatNeedsTransformation(NABoolean isAlignedRowFormat, NABoolean omittedDefaultCols,
                                                  NABoolean omittedCurrentDefaultClassCols, NABoolean &toMerge) const {
  toMerge = FALSE;
  ...
  if (isUpsert() &&
      NOT(getIsTrafLoadPrep() || transformedUpdateNoDtmXn() || (getInsertType() == Insert::UPSERT_NO_DUP) ||
          ((getTableDesc()->isIdentityColumnGeneratedAlways() && getTableDesc()->hasIdentityColumnInClusteringKey())) ||
  ...
  if (isUpsert() && mustTryTransform && (getTableDesc()->hasSecondaryIndexes())) {
    toMerge = FALSE;
    return TRUE;
  }
...
RelExpr *Insert::xformUpsertToEfficientTree(BindWA *bindWA) {
  ...
  if (bindWA->errStatus()) return NULL;
  if ((naTable->getViewText() != NULL) && (naTable->getViewCheck())) {
    *CmpCommon::diags() << DgSqlCode(-3241) << DgString0(" View with check option not allowed.");
    bindWA->setErrStatus();
    return NULL;
  }
  ...
  Scan *targetTableScan = new (bindWA->wHeap()) Scan(CorrName(getTableDesc()->getCorrNameObj(), bindWA->wHeap()));
  targetTableScan->setSkipTransactionForBatchGet(TRUE);
  if (noRepeatableRead()) targetTableScan->setSkipTransactionForBatchGetForced(TRUE);
  ...
  for (CollIndex i = 0; i < tableCols.entries(); i++) {
    baseCol = (BaseColumn *)(tableCols[i].getItemExpr());
    if (baseCol->getNAColumn()->isSystemColumn() && !baseCol->getNAColumn()->isReplicaColumn()) continue;

    targetColRef = new (bindWA->wHeap())
        ColReference(new (bindWA->wHeap()) ColRefName(baseCol->getNAColumn()->getFullColRefName(), bindWA->wHeap()));
    ...
    if (baseCol->getNAColumn()->isClusteringKey()) {
      ...
      keyPredPrev = keyPred;
      keyPred = new (bindWA->wHeap())
          BiRelat(ITM_EQUAL, targetColRef, sourceVals[i].getItemExpr(), baseCol->getType().supportsSQLnull());
      predCount++;
      if (predCount > 1) {
        keyPred = new (bindWA->wHeap()) BiLogic(ITM_AND, keyPredPrev, keyPred);
      }
  ...
  bindWA->getCurrentScope()->xtnmStack()->removeXTNM();
  lj->convertToTsj();
  lj->getInliningInfo().setFlags(II_isEffUpsertLj);
  targetTableScan->getInliningInfo().setFlags(II_isEffUpsertLj);

  ValueId nullInstIndicator(
      lj->addNullInstIndicatorVar(bindWA, new (bindWA->wHeap()) SystemLiteral("U", CharInfo::ISO88591)));
  ...
  RelSequence *seqNode =
      new (bindWA->wHeap()) RelSequence(boundLJ, sourcePKeyVals.rebuildExprTree(ITM_ITEM_LIST), (ItemExpr *)NULL);
  ...
  ItemExpr *offset = new (bindWA->wHeap()) ConstValue(1);

  leadItem = new (bindWA->wHeap()) ItmLagOlapFunction(constLead999, offset);
  ...
  notCoveredMap.rewriteValueIdUp(notCoveredNullInstIndicator, nullInstIndicator);
  ItemExpr *nvl =
      new (bindWA->wHeap()) BuiltinFunction(ITM_NVL, bindWA->wHeap(), 2, notCoveredNullInstIndicator.getItemExpr(),
                                            new (bindWA->wHeap()) SystemLiteral("I", CharInfo::ISO88591));
...
RelExpr *Insert::xformUpsertToMerge(BindWA *bindWA) {
  ...
  Scan *inputScan = new (bindWA->wHeap()) Scan(CorrName(getTableDesc()->getCorrNameObj(), bindWA->wHeap()));
  ...
  for (CollIndex i = 0; i < tableCols.entries(); i++) {
    baseCol = (BaseColumn *)(tableCols[i].getItemExpr());
    if (baseCol->getNAColumn()->isSystemColumn()) continue;
    ...
    if (baseCol->getNAColumn()->isClusteringKey()) {
      ...
      // those to the new scan
      keyPredPrev = keyPred;
      ItemExpr *si = sourceVals[i].getItemExpr();
      // if the generated pred is comparing binary to a non-binary datatype,
      // convert it to binary before doing the comparison.
      if ((DFS2REC::isBinaryString(baseCol->getNAColumn()->getType()->getFSDatatype())) &&
          (NOT DFS2REC::isBinaryString((sourceVals[i].getType().getFSDatatype())))) {
        si = new (bindWA->wHeap()) ZZZBinderFunction(ITM_TO_BINARY, si);
      }
      keyPred = new (bindWA->wHeap()) BiRelat(ITM_EQUAL, targetColRef, si, baseCol->getType().supportsSQLnull());
      predCount++;
      if (predCount > 1) {
        keyPred = new (bindWA->wHeap()) BiLogic(ITM_AND, keyPredPrev, keyPred);
      }
    ...
    ValueIdSet dummy1, dummy2;
    // newOuterRefs must include all sourceVals except constants and
    // covered expressions on currOuterRefs, not including currOuterRefs
    // OuterRefs should be minimal
    if (!sourceVals[i].getItemExpr()->isCovered(currOuterRefs, emptyGA, newOuterRefs, dummy1, dummy2)) {
      newOuterRefs += sourceVals[i];
    }
    mergeScope->addOuterRefs(newOuterRefs);
  ...
  for (CollIndex i = 0; i < newRecExprArray().entries(); i++) {
    ...
    if (col->isSystemColumn())
      ...
      //
      if (assignExpr->isUserSpecified()) copySetAssign = TRUE;
      ...
      if (copySetAssign) {
        ...
        setCount++;
        if (setCount > 1) setAssign = new (bindWA->wHeap()) ItemList(setAssignPrev, setAssign);
      }
    }
  }
  MergeUpdate *mu = new (bindWA->wHeap())
      MergeUpdate(CorrName(getTableDesc()->getCorrNameObj(), bindWA->wHeap()), NULL, REL_UNARY_UPDATE,
                  inputScan,  // USING
...
RelExpr *HBaseBulkLoadPrep::bindNode(BindWA *bindWA) {
  ...
  Insert *newInsert = new (bindWA->wHeap()) Insert(getTableName(), NULL, REL_UNARY_INSERT, child(0)->castToRelExpr());
...
const char *Insert::getColDefaultValue(BindWA *bindWA, CollIndex i) const {
  ...
  if (colList.entries() <= pos) {
    // 4023 degree of row value constructor must equal that of target table
    *CmpCommon::diags() << DgSqlCode(-4023) << DgInt0(++pos) << DgInt1(colList.entries());
    bindWA->setErrStatus();
    return NULL;
  }
  ...
  if (nacol->getDefaultClass() == COM_USER_DEFINED_DEFAULT && nacol->getType() &&
      nacol->getType()->getTypeQualifier() == NA_CHARACTER_TYPE &&
      ((CharType *)(nacol->getType()))->getCharSet() == CharInfo::ISO88591 && mapCS_hasVariableWidth &&
      defaultValueWcs != NULL && nacol->getNATable()->getObjectSchemaVersion() >= COM_VERS_2300 &&
      (defaultValueWcsLen = NAWstrlen(defaultValueWcs)) > 6 &&
      ((ucs2StrLitPrefix = (NAWstrncmp(defaultValueWcs, NAWSTR("_UCS2\'"), 6) == 0)) ||
       (defaultValueWcsLen > 10 && NAWstrncmp(defaultValueWcs, NAWSTR("_ISO88591\'"), 10) == 0)) &&
      defaultValueWcs[defaultValueWcsLen - 1] == NAWCHR('\'')) {
    NAWcharBuf *pWcharBuf = NULL;
    if (ucs2StrLitPrefix) {
      // Strip the leading _UCS2 prefix.
      pWcharBuf = new (bindWA->wHeap()) NAWcharBuf(&defaultValueWcs[5], defaultValueWcsLen - 5, bindWA->wHeap());
    } else {
      // Keep the leading _ISO88591 prefix.
      pWcharBuf = new (bindWA->wHeap()) NAWcharBuf(defaultValueWcs, defaultValueWcsLen, bindWA->wHeap());
    }
...
RelExpr *Update::bindNode(BindWA *bindWA) {
  ...
  if (getFirstNRows() > 0) {
    *CmpCommon::diags() << DgSqlCode(-3242) << DgString0("[FIRST N] not allowed in update query.");
  ...
  if (child(0) && child(0)->getOperatorType() == REL_SCAN)
    if (child(0)->castToRelExpr() && ((Scan *)(child(0)->castToRelExpr()))->getFirstNRows() >= 0)
      if (bindWA && bindWA->getCurrentScope() && bindWA->getCurrentScope()->context())
        bindWA->getCurrentScope()->context()->firstN() = TRUE;
  ...
  if (getTableDesc()->getNATable()->isHbaseCellTable()) {
    *CmpCommon::diags()
        << DgSqlCode(-1425) << DgTableName(getTableDesc()->getNATable()->getTableName().getQualifiedNameAsAnsiString())
  ...
  if (NOT bindWA->errStatus() AND NOT getTableDesc()->getVerticalPartitions().isEmpty()) {
  ...
  RelExpr *updPartitionTableExpr = xformPartitionPruning(bindWA, boundExpr);
  if (updPartitionTableExpr != this) {
    return updPartitionTableExpr;
  }
  /*
  ...
  */
  ...
  if ((getOperatorType() == REL_UNARY_UPDATE) && (child(0)->getOperatorType() == REL_SCAN)) {
    Scan *scanNode = (Scan *)(child(0)->castToRelExpr());
    if ((scanNode->getFirstNRows() != -1) && (getGroupAttr()->isEmbeddedUpdateOrDelete())) {
      *CmpCommon::diags() << DgSqlCode(-4216);
      bindWA->setErrStatus();
      return NULL;
    }
  ...
  if ((bindWA->getHostArraysArea()) && (bindWA->getHostArraysArea()->hasHostArraysInSetClause()) &&
  ...
  if (hbaseTagExprList_.entries() > 0) {
    ItemExpr *numEnt = new (bindWA->wHeap()) ConstValue(hbaseTagExprList_.entries());
  ...
  NABoolean updUsingUpsert = FALSE;
  DefaultToken dt = CmpCommon::getDefault(TRAF_UPDATE_TO_UPSERT);
  if (isUpdUsingUpsert() || dt == DF_FORCE) {
    updUsingUpsert = TRUE;
    if (dt == DF_ON) {
      updUsingUpsert = !transformUpdateKey;
    } else if (dt == DF_ALL || dt == DF_FORCE) {
      updUsingUpsert = TRUE;
    }
  }
  ...
  if (getTableDesc()->getNATable()->isHbaseMapTable()) {
    ...
    Assign_SqlParser_Flags(savedParserFlags);
  } else if ((transformUpdateKey) && (NOT updUsingUpsert) && (NOT isMerge())) {
    boundExpr = transformUpdatePrimaryKey(bindWA);
  } else if (updUsingUpsert) {
    boundExpr = transformUpdateNoDtmXn(bindWA, transformUpdateKey);
  } else
...
RelExpr *Update::xformPartitionPruning(BindWA *bindWA, RelExpr *gu, OperatorTypeEnum op) {
  TableDesc *tableDesc = getTableDesc();
  const NATable *naTable = tableDesc->getNATable();

  if (!naTable->isPartitionV2Table()) return this;

  bindWA->setPruningPartition(TRUE);
  // only support REL_UNARY_UPDATE on partition table
  ...
  } else if (isPartnValuesSpecified()) {
    ItemExprList valList(getSpecPartnVals(), bindWA->wHeap());
    partNames = tableDesc->getMatchedPartInfo(bindWA, valList);
    if (!partNames || partNames->entries() == 0) {
      // the specified partition does not exist
  ...
  if (isPartnNameSpecified() || isPartnValuesSpecified()) {
    ItemExpr *updRecExpr = recExprTree()->copyTree(bindWA->wHeap());
    RelExpr *updExpr = new (bindWA->wHeap())
        Update(CorrName((*partNames)[0], bindWA->wHeap(), tableDesc->getNATable()->getTableName().getSchemaName(),
                        tableDesc->getNATable()->getTableName().getCatalogName()),
               NULL, REL_UNARY_UPDATE, gu->child(0)->castToRelExpr(), updRecExpr);
    RelExpr *boundUpdExpr = updExpr->bindNode(bindWA);
    bindWA->setPruningPartition(FALSE);
    return boundUpdExpr;
  }

  if (child(0)->getGroupAttr()->isEmbeddedDelete()) {
    *CmpCommon::diags() << DgSqlCode(-4172) << DgString0(getGroupAttr()->getOperationWithinGroup());
    bindWA->setErrStatus();
    return this;
  }

  // conditions restrict to single partition
  if (child(0)->getOperatorType() != REL_UNION) {
    RelExpr *scanExpr = child(0)->castToRelExpr();
    CMPASSERT(scanExpr->getOperatorType() == REL_SCAN);
    // This Scan is not from Partition Pruning. Because don't get partition
    // from given information, operation are on original node.
    if (!((Scan *)scanExpr)->isPartitionPruned()) {
      return this;
    }

    ItemExpr *updRecExpr = recExprTree()->copyTree(bindWA->wHeap());
    // rel->markAsUnBound();
    const CorrName &oriCN = scanExpr->getTableName();
    RelExpr *updExpr =
        new (bindWA->wHeap()) Update(CorrName(oriCN, bindWA->wHeap()), NULL, REL_UNARY_UPDATE, scanExpr, updRecExpr);
    ((Update *)updExpr)->setCheckCnstrntOfPartv2BaseTable(getTableDesc()->genPartv2BaseCheckConstraint(bindWA));
  ...
  // conditions restrict to multi partitions
  RelExpr *relExpr = child(0)->xformPartitionPruning(bindWA, gu, REL_UNARY_UPDATE);
  RelExpr *boundRelExpr = relExpr->bindNode(bindWA);
  bindWA->setPruningPartition(FALSE);
  return boundRelExpr;
}
...
RelExpr *MergeUpdate::bindNode(BindWA *bindWA) {
  ...
  if (needsBindScope_) bindWA->initNewScope();
  ...
  ItemExpr *precond = NULL;
  if (isMerge() && child(0) && !xformedUpsert()) {
    ItemExpr *selPred = child(0)->castToRelExpr()->selPredTree();
    precond = getPreconditionTree();
    if (selPred || where_ || andpred_ || precond) {
      ...
      NABoolean whrHasAggr = FALSE;
      NABoolean andHasSubqry = FALSE;
      NABoolean andHasAggr = FALSE;
      NABoolean precondHasSubq = FALSE;

      if (ONhasSubquery || ONhasAggr ||
          (where_ && ((whrHasSubqry = where_->containsSubquery()) || (whrHasAggr = where_->containsAnAggregate()))) ||
          (andpred_ &&
           ((andHasSubqry = andpred_->containsSubquery()) || (andHasAggr = andpred_->containsAnAggregate()))) ||
          (precond && ((precondHasSubq = precond->containsSubquery())))) {
        *CmpCommon::diags()
            << DgSqlCode(-3241)
            << DgString0(
                   ONhasSubquery
                       ? "Subquery in ON clause not allowed."
                       : (ONhasAggr
                              ? "aggregate function in ON clause not allowed."
                              : (andHasSubqry
                                     ? "subquery in WHEN MATCHED AND is not allowed."
                                     : (andHasAggr
                                            ? "aggregate function in WHEN MATCHED AND is not allowed."
                                            : (whrHasSubqry
                                                   ? "subquery in UPDATE ... WHERE clause not allowed."
                                                   : (whrHasAggr
                                                          ? "aggregate function in UPDATE ... WHERE clause not allowed."
                                                          : (precondHasSubq ? "subquery in WHEN NOT MATCHED AND clause "
                                                                              "not allowed."
                                                                            : "aggregate function in WHEN NOT MATCHED "
                                                                              "AND clause not allowed.")))))));
        bindWA->setErrStatus();
        return this;
      }
      ItemExpr *ONhasUDF = (selPred ? selPred->containsUDF() : NULL);
      ItemExpr *whereHasUDF = (where_ ? where_->containsUDF() : NULL);
      ItemExpr *andpredHasUDF = (andpred_ ? andpred_->containsUDF() : NULL);
      ItemExpr *precondHasUDF = (precond ? precond->containsUDF() : NULL);
      if (ONhasUDF || whereHasUDF) {
        *CmpCommon::diags()
            << DgSqlCode(-4471)
            << DgString0(((UDFunction *)(ONhasUDF ? ONhasUDF : whereHasUDF))->getFunctionName().getExternalName());
        bindWA->setErrStatus();
        return this;
      }
      if (andpredHasUDF || precondHasUDF) {
        *CmpCommon::diags() << DgSqlCode(-4471)
                            << DgString0(((UDFunction *)(andpredHasUDF ? andpredHasUDF : precondHasUDF))
                                             ->getFunctionName()
                                             .getExternalName());
        bindWA->setErrStatus();
        return this;
  ...
  if (isMerge() && recExprTree() && !xformedUpsert()) {
    if (recExprTree()->containsSubquery()) {
      *CmpCommon::diags() << DgSqlCode(-3241) << DgString0(" Subquery in SET clause not allowed.");
      bindWA->setErrStatus();
      return this;
    }
    if (recExprTree()->containsUDF()) {
      *CmpCommon::diags() << DgSqlCode(-4471)
                          << DgString0(
                                 ((UDFunction *)recExprTree()->containsUDF())->getFunctionName().getExternalName());
      bindWA->setErrStatus();
      return this;
    }
  }
  ...
  if (insertValues()) {
    if (insertValues()->containsSubquery() && !xformedUpsert()) {
      *CmpCommon::diags() << DgSqlCode(-3241) << DgString0(" Subquery in INSERT clause not allowed.");
      bindWA->setErrStatus();
      return this;
    }
    if (insertValues()->containsUDF() && !xformedUpsert()) {
      *CmpCommon::diags() << DgSqlCode(-4471)
                          << DgString0(
                                 ((UDFunction *)insertValues()->containsUDF())->getFunctionName().getExternalName());
      bindWA->setErrStatus();
      return this;
    }

    Tuple *tuple = new (bindWA->wHeap()) Tuple(insertValues());
    Insert *ins = new (bindWA->wHeap()) Insert(getTableName(), NULL, REL_UNARY_INSERT, tuple, insertCols(), NULL);
  ...
  if (naTable->getNumTrafReplicas() > 1) {
    *CmpCommon::diags() << DgSqlCode(-3241) << DgString0("SQL replicated tables not suuported.");
    bindWA->setErrStatus();
    return NULL;
  }

  if ((naTable->isHbaseCellTable()) || (naTable->isHbaseRowTable())) {
    *CmpCommon::diags() << DgSqlCode(-3241) << DgString0("Hbase tables not supported.");
    bindWA->setErrStatus();
    return NULL;
  }

  if (naTable->isHiveTable()) {
    *CmpCommon::diags() << DgSqlCode(-3241) << DgString0("Hive tables not supported.");
    bindWA->setErrStatus();
    return NULL;
  }

  if (naTable->isHbaseMapTable() && (CmpCommon::getDefault(TRAF_HBASE_MAPPED_TABLES_IUD) == DF_OFF)) {
    *CmpCommon::diags() << DgSqlCode(-4223) << DgString0("Insert/Update/Delete/Merge on HBase mapped tables is");
  ...
  if (getProducedMergeIUDIndicator() == NULL_VALUE_ID) {
    ItemExpr *mergeIUDIndicator = new (bindWA->wHeap())
        NATypeToItem(new (bindWA->wHeap()) SQLChar(bindWA->wHeap(), 1, FALSE, FALSE, FALSE, FALSE, CharInfo::ISO88591));
  ...
  if (checkForMergeRestrictions(bindWA)) return NULL;
  ...
  if (andpred_) {
    bindWA->getCurrentScope()->context()->inWhereClause() = TRUE;
    andpred_->convertToValueIdSet(mergeUpdatePred(), bindWA, ITM_AND);
    bindWA->getCurrentScope()->context()->inWhereClause() = FALSE;
    if (bindWA->errStatus()) return NULL;
  ...
  if (precond) {
    ValueIdSet pc;
    bindWA->getCurrentScope()->context()->inWhereClause() = TRUE;
    bindWA->getCurrentScope()->context()->inPrecondClause() = TRUE;
    precond->convertToValueIdSet(pc, bindWA, ITM_AND);
    bindWA->getCurrentScope()->context()->inPrecondClause() = FALSE;
    bindWA->getCurrentScope()->context()->inWhereClause() = FALSE;
    if (bindWA->errStatus()) return NULL;
    setPrecondition(pc);
  ...
  if (needsBindScope_) bindWA->removeCurrentScope();
...
// check wether IN list converts to an OR predicate
static NABoolean lcl_isINListConverts2OR(const ItemExpr *ie) {
  if (ie && ie->getOperatorType() == ITM_OR)
    if (((BiLogic *)ie)->createdFromINlist()) return TRUE;
  return FALSE;
}

// check wether IN list converts to a VALUES subquery
static NABoolean lcl_isINListConverts2Sub(const ItemExpr *ie) {
  if (ie && ie->getOperatorType() == ITM_EQUAL_ANY)
    if (((QuantifiedComp *)ie)->createdFromINlist()) return TRUE;
  return FALSE;
}
// -----------------------------------------------------------------------
...
RelExpr *Delete::bindNode(BindWA *bindWA) {
  ...
  if ((getFirstNRows() >= 0) && (CmpCommon::getDefault(ALLOW_FIRSTN_IN_IUD) == DF_OFF)) {
    *CmpCommon::diags() << DgSqlCode(-3242) << DgString0("[FIRST N] not allowed in delete query.");
  ...
  if (csl_) {
    if ((getTableDesc()->getNATable()->isSeabaseTable()) && (Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL))) {
  ...
  RelExpr *delPartitionTableExpr = xformPartitionPruning(bindWA, boundExpr);
  if (delPartitionTableExpr != this) {
    return delPartitionTableExpr;
  }
  ...
  */
  if (getArity() == 1 && child(0)->getOperatorType() == REL_SCAN) {
    scanNode = (Scan *)(child(0)->castToRelExpr());
    scanNode->setSkipTransactionForCursorUpdDelScan(TRUE);
    /*
        if (!scanNode->accessOptions().userSpecified()) {
          scanNode->accessOptions().updateAccessOptions
            (TransMode::ILtoAT(TransMode::SERIALIZABLE_));
  ...
  if (isMtsStatement()) bindWA->setEmbeddedIUDStatement(TRUE);

  if (scanNode && (getFirstNRows() >= 0)) {
    // M-19442
    FirstN *firstn = new (bindWA->wHeap()) FirstN(scanNode, getFirstNRows(), FALSE);
    firstn->bindNode(bindWA);
    if (bindWA->errStatus()) return NULL;
    setChild(0, firstn);
  ...
  if ((csl()) && (getTableDesc()->getNATable()->isSeabaseTable()) &&
    ...
    *CmpCommon::diags() << DgSqlCode(-3242) << DgString0("Cannot specify column name in a delete statement.");
  ...
  if ((csl()) && (NOT getTableDesc()->getNATable()->isSeabaseTable())) {
    for (int i = 0; i < csl()->entries(); i++) {
      ...
      bindWA->hbaseColUsageInfo()->insert((QualifiedName *)&getTableDesc()->getNATable()->getTableName(), nas);
    }
  }

  if ((boundExpr->getOperatorType() == REL_TSJ || boundExpr->getOperatorType() == REL_ANTI_SEMITSJ) &&
      boundExpr->child(0)->getOperatorType() == REL_UNARY_DELETE && !isMergeDelete()) {
    RelExpr *oldExpr = boundExpr->child(0)->castToRelExpr();
    ((Delete *)oldExpr)->needConvertToAsj() = needConvertToAsj();
    RelExpr *newExpr = xformDeleteToEfficientTree(bindWA, oldExpr);
    if (newExpr != oldExpr) boundExpr->setChild(0, newExpr);
  }

  // TODO:
  /*  else if (boundExpr->getOperatorType() == REL_UNARY_DELETE)
    {
      ((Delete *)boundExpr)->needConvertToAsj() = needConvertToAsj();
      boundExpr = xformDeleteToEfficientTree(bindWA, boundExpr);
    }
  */
  return boundExpr;
...
RelExpr *Delete::xformPartitionPruning(BindWA *bindWA, RelExpr *gu, OperatorTypeEnum op) {
  TableDesc *tableDesc = getTableDesc();
  const NATable *naTable = tableDesc->getNATable();
  ...
  bindWA->setPruningPartition(TRUE);
  // only support REL_UNARY_DELETE on partition table
  ...
  if (isPartnNameSpecified() || isPartnValuesSpecified()) {
    RelExpr *delExpr = new (bindWA->wHeap())
        Delete(CorrName((*partNames)[0], bindWA->wHeap(), tableDesc->getNATable()->getTableName().getSchemaName(),
                        tableDesc->getNATable()->getTableName().getCatalogName()),
               NULL, REL_UNARY_DELETE, gu->child(0)->castToRelExpr());
    RelExpr *boundDelExpr = delExpr->bindNode(bindWA);
    bindWA->setPruningPartition(FALSE);
    return boundDelExpr;
  }
  ...
  if (child(0)->getOperatorType() != REL_UNION) {
    ...
    // rel->markAsUnBound();
    const CorrName &oriCN = scanExpr->getTableName();
    RelExpr *delExpr = new (bindWA->wHeap()) Delete(CorrName(oriCN, bindWA->wHeap()), NULL, REL_UNARY_DELETE, scanExpr);
    RelExpr *boundDelExpr = delExpr->bindNode(bindWA);
    bindWA->setPruningPartition(FALSE);
    return boundDelExpr;
  }
  ...
  // conditions restrict to multi partitions
  RelExpr *relExpr = child(0)->xformPartitionPruning(bindWA, gu, REL_UNARY_DELETE);
  RelExpr *boundRelExpr = relExpr->bindNode(bindWA);
  bindWA->setPruningPartition(FALSE);
  return boundRelExpr;
}

// take an delete(src) node and transform it into
// a Anti Semi Join with old keys flowing to the IM tree.
/*
            NJ
          /     \
Anti Semi Join   IM Tree
     /       \
    /         \
  Scan       Delete
*/
RelExpr *Delete::xformDeleteToEfficientTree(BindWA *bindWA, RelExpr *deleteExpr) {
  if (!deleteExpr->getOperatorType() == REL_UNARY_DELETE) return deleteExpr;
  if (!((Delete *)deleteExpr)->needConvertToAsj()) return deleteExpr;

  GenericUpdate *oldDelete = (GenericUpdate *)deleteExpr;
  RelExpr *newDelete = oldDelete->copyTopNode(0, CmpCommon::statementHeap());
  NABoolean foundScan = FALSE;
  RelExpr *oldExpr = deleteExpr;
  while (oldExpr->getArity() == 1) {
    oldExpr = oldExpr->child(0);
    if (oldExpr->getOperatorType() == REL_SCAN) {
      newDelete->getGroupAttr()->setCharacteristicOutputs(oldExpr->getGroupAttr()->getCharacteristicInputs());
      newDelete->getGroupAttr()->setCharacteristicInputs(oldExpr->getGroupAttr()->getCharacteristicOutputs());
      foundScan = TRUE;
      break;
    }
  }
  if (!foundScan) return deleteExpr;
  GenericUpdate *newDel = (GenericUpdate *)newDelete;
  ValueIdList tempList;
  newDel->setPotentialOutputValues(tempList);
  ((Delete *)newDel)->needConvertToAsj() = FALSE;
  OperatorTypeEnum newOptype = NO_OPERATOR_TYPE;

  switch (deleteExpr->getOperatorType()) {
    case REL_UNARY_DELETE:
  ...
  newDelete->setOperatorType(newOptype);
  RelExpr *newScan = deleteExpr->child(0)->castToRelExpr();

  Join *asj = new (CmpCommon::statementHeap()) Join(newScan, newDelete, REL_ANTI_SEMITSJ);
  RelRoot *rootNode = new (CmpCommon::statementHeap()) RelRoot(asj);
  rootNode->setRootFlag(FALSE);
  rootNode->setEmptySelectList();
  rootNode->setDontOpenNewScope();

  if (getGroupAttr()->isEmbeddedUpdateOrDelete())
    rootNode->setRETDesc(bindWA->getCurrentScope()->getRETDesc());
  else
    rootNode->setRETDesc(new (CmpCommon::statementHeap()) RETDesc(bindWA));

  asj->getInliningInfo().setFlags(II_isEffDeleteAsj);
  newScan->getInliningInfo().setFlags(II_isEffDeleteAsj);
  newDelete->getInliningInfo().setFlags(II_isEffDeleteAsj);
...
RelExpr *MergeDelete::bindNode(BindWA *bindWA) {
  ...
  if ((isMerge()) && (child(0)) && (child(0)->castToRelExpr()->selPredTree())) {
    if (child(0)->castToRelExpr()->selPredTree()->containsSubquery()) {
      *CmpCommon::diags() << DgSqlCode(-3241) << DgString0(" Subquery in ON clause not allowed.");
      bindWA->setErrStatus();
      return this;
    }
    if (child(0)->castToRelExpr()->selPredTree()->containsUDF()) {
      *CmpCommon::diags() << DgSqlCode(-4471)
                          << DgString0(((UDFunction *)child(0)->castToRelExpr()->selPredTree()->containsUDF())
                                           ->getFunctionName()
                                           .getExternalName());
      bindWA->setErrStatus();
      return this;
  ...
  if (insertValues()) {
    ...
    if (CmpCommon::getDefault(COMP_BOOL_175) == DF_OFF) {
      ...
      *CmpCommon::diags() << DgSqlCode(-3241) << DgString0(" MERGE DELETE not allowed with INSERT.");
    }
...
static BoundaryValue *getPartBoundaryValue(const NAColumn *nacol) {
  ...
  const int *colIdxArray = naTable->getPartitionColIdxArray();
  int i;
  for (i = 0; i < boundList.entries(); i++) {
    if (colIdxArray[i] == nacol->getPosition()) break;
  }
  CMPASSERT(i < boundList.entries());
...
void GenericUpdate::bindUpdateExpr(BindWA *bindWA, ItemExpr *recExpr, ItemExprList &assignList, RelExpr *boundView,
                                   Scan *scanNode, SET(short) & stoiColumnSet, NABoolean onRollback) {
  RETDesc *origScope = NULL;

  ValueIdSet &newRecExpr = (onRollback == TRUE) ? newRecBeforeExpr() : this->newRecExpr();
  ValueIdArray &newRecExprArray = (onRollback == TRUE) ? newRecBeforeExprArray() : this->newRecExprArray();

  if (onRollback && ((!getTableDesc()->getClusteringIndex()->getNAFileSet()->isAudited()))) {
  ...
  if (currColumnCount = stoi_->getStoi()->getColumnListCount()) {
    ...
    for (i = 0; i < currColumnCount; i++) stoiColumnList[i] = stoi_->getStoi()->getUpdateColumn(i);
  }

  stoi_->getStoi()->setColumnList(new (bindWA->wHeap()) short[a + currColumnCount]);

  ValueIdSet &selectPreds = getScanNode()->selectionPred();
  for (i = 0; i < a; i++) {
    ...
    NABoolean cv1_is_NULL;
    const char *cv1_ConstValue = NULL;
    int cv1_length = 0;
    ConstValue *cv1 = NULL;
    if (assignList[i]->child(1)->getOperatorType() == ITM_CONSTANT) {
      cv1 = (ConstValue *)(assignList[i]->child(1)->castToItemExpr());
      cv1_is_NULL = cv1->isNull();
      cv1_ConstValue = (const char *)(cv1->getConstValue());
      cv1_length = cv1->getStorageSize();
    }

    if (getOperatorType() == REL_UNARY_UPDATE) {
      // remove unchanged columns from update list
      // Orig sql: update t1 set c1=1, c2=2, c3=3 where c1=1;
      // New Sql:  update t1 set c2=2, c3=3 where c1=1;
      // Orig sql: update t1 set c1=1 where c1=1;
      // New Sql:  update t1 set c1=1 wehre c1=1;
      NABoolean removeCol = FALSE;
      for (ValueId pred = selectPreds.init(); selectPreds.next(pred); selectPreds.advance(pred)) {
        if (pred.getItemExpr()->getOperatorType() != ITM_EQUAL) continue;
        pred.getItemExpr()->bindNode(bindWA);
        OperatorTypeEnum type0 = pred.getItemExpr()->child(0)->getOperatorType();
        OperatorTypeEnum type1 = pred.getItemExpr()->child(1)->getOperatorType();

        NABoolean cv2_is_NULL = FALSE;
        const char *cv2_ConstValue = NULL;
        int cv2_length = 0;
        ConstValue *cv2 = NULL;
        int col_index = -1;
        int const_index = -1;
        if (type0 == ITM_BASECOLUMN && type1 == ITM_CONSTANT) {
          col_index = 0;
          const_index = 1;
        } else if (type1 == ITM_BASECOLUMN && type0 == ITM_CONSTANT) {
          col_index = 1;
          const_index = 0;
        } else
          continue;

        const NAColumn *predCol = pred.getItemExpr()->child(col_index).getNAColumn();
        cv2 = (ConstValue *)(pred.getItemExpr()->child(const_index)->castToItemExpr());
        cv2_is_NULL = cv2->isNull();
        cv2_ConstValue = (const char *)(cv2->getConstValue());
        cv2_length = cv2->getStorageSize();

        if (nacol->getPosition() == predCol->getPosition() &&
            ((cv1_ConstValue != NULL && cv2_ConstValue != NULL && cv1_length == cv2_length &&
              memcmp(cv1_ConstValue, cv2_ConstValue, cv1_length) == 0) ||
             (cv1_is_NULL && cv2_is_NULL))) {
          // The assign list must have one column to be updated at least.
          // Which column is alternatives? The no primary key columns and
          // no index columns is best. We should check that in future.
          if (a > 1) {
            assignList.removeAt(i--);
            removeCol = TRUE;
            a--;
          }
          break;
    ...
    if (getOperatorType() == REL_UNARY_UPDATE && CmpCommon::getDefault(COMP_BOOL_210) == DF_ON &&
    ...
    if (NOT boundView) stoiColumnSet.insert((short)nacol->getPosition());
  ...
  if (getOperatorType() == REL_UNARY_UPDATE) {
    ValueIdSet pc;
    Parser parser(bindWA->currentCmpContext());
    for (ValueId v = newRecExpr.init(); newRecExpr.next(v); newRecExpr.advance(v)) {
      const NAColumn *nacol = v.getItemExpr()->child(0).getNAColumn();
      if (nacol->isPartV2Column()) {
        BoundaryValue *bv = getPartBoundaryValue(nacol);
        const NAType &desiredType = v.getItemExpr()->child(0)->getValueId().getType();
        ItemExpr *colItem = v.getItemExpr()->child(1)->castToItemExpr();
        ItemExpr *lowItem = NULL;
        ItemExpr *highItem = NULL;
        if (bv->lowValue) {
          lowItem = parser.getItemExprTree(bv->lowValue, strlen(bv->lowValue),
                                           CharInfo::UTF8);  // ComGetNameInterfaceCharSet()
          lowItem = new (bindWA->wHeap()) Cast(lowItem, &desiredType);
        }
        if (bv->highValue && (!bv->isMaxValue())) {
          highItem = parser.getItemExprTree(bv->highValue, strlen(bv->highValue),
                                            CharInfo::UTF8);  // ComGetNameInterfaceCharSet()
          highItem = new (bindWA->wHeap()) Cast(highItem, &desiredType);
        }

        ItemExpr *precond;
        if (!lowItem) {
          // first partition
          precond = new (bindWA->wHeap()) BiRelat(ITM_LESS, colItem, highItem);
        } else if (!highItem) {
          // last partition
          precond = new (bindWA->wHeap()) BiRelat(ITM_GREATER_EQ, colItem, lowItem);
        } else {
          Partition *part = getTableDesc()->getPartitionList()[0];
          CMPASSERT(lowItem);
          lowItem = new (bindWA->wHeap()) BiRelat(ITM_GREATER_EQ, colItem, lowItem);
          CMPASSERT(highItem);
          if (part->getFirstMaxvalIdx() > 0)
            highItem = new (bindWA->wHeap()) BiRelat(ITM_LESS_EQ, colItem, highItem);
          else
            highItem = new (bindWA->wHeap()) BiRelat(ITM_LESS, colItem, highItem);

          precond = new (bindWA->wHeap()) BiLogic(ITM_AND, lowItem, highItem);
        }
        precond->bindNodeRoot(bindWA);
        if (bindWA->errStatus()) return;
        pc.addMember(precond);
      }
    }
    setPartQualPreCond(pc);
  }
  ...
  if (NOT onRollback) {
    ...
    for (ValueId v = newRecExpr.init(); newRecExpr.next(v); newRecExpr.advance(v)) {
      ...
      if ((getOperatorType() == REL_UNARY_UPDATE) && !(((Assign *)v.getItemExpr())->onRollback())) {
        newRecExpr.remove(v);
      }
    }
  ...
  for (i = 0, assignId = newRecExpr.init();  // bizarre ValueIdSet iter
       newRecExpr.next(assignId); i++, newRecExpr.advance(assignId)) {
    j = colnoList[i];
  ...
  if ((NOT onRollback) && (NOT mergeWithIndex)) {
    for (i = 0; i < totalColCount; i++) {
      if (!(holeyArray.used(i))) {
        oldToNewMap().addMapEntry(scanNode->getTableDesc()->getColumnList()[i].getItemExpr()->getValueId(),
  ...
  for (i = j = 0; i < totalColCount; i++) {
    if (holeyArray.used(i)) {
      ...
      if (right->containsColumn()) rightContainsColumn = TRUE;
    }
  }
...
NABoolean GenericUpdate::checkForHalloweenR2(int numScansToFind) {
  ...
  // Do not support for general NEO users.
  if (CmpCommon::getDefault(MODE_SPECIAL_1) == DF_OFF) return TRUE;
  ...
  for (ValueId p = preds.init(); preds.next(p); preds.advance(p)) {
    ...
    if (pred->containsSubquery()) {
      ...
      while (subqPreds.entries()) {
        ValueIdSet children;
        for (ValueId s = subqPreds.init(); subqPreds.next(s); subqPreds.advance(s)) {
          ...
          if (term->isASubquery()) {
            ...
            if (!subq->isARowSubquery()) {
              ...
              if (halloweenScan) {
                ...
                //
                if (halloweenScan->getTableDesc()->getNATable() == getTableDesc()->getNATable()) {
  ...
  //
  if (numHalloweenScans == numScansToFind) return FALSE;

  return TRUE;
}

static void getUpdateSetListColName4CacheKey(ItemExpr *pItemExprTree, NAString &str) {
  if (!pItemExprTree) return;
  if (pItemExprTree->getArity() != 2) return;
  OperatorTypeEnum eType = pItemExprTree->getOperatorType();
  if (ITM_ITEM_LIST == eType) {
    if (pItemExprTree->child(0)) getUpdateSetListColName4CacheKey(pItemExprTree->child(0), str);
    str += pItemExprTree->getText4CacheKey();
    str += " ";
    if (pItemExprTree->child(1)) getUpdateSetListColName4CacheKey(pItemExprTree->child(1), str);

  } else if (ITM_ASSIGN == eType) {
    if (pItemExprTree->child(0) && pItemExprTree->child(0)->getOperatorType() == ITM_REFERENCE) {
      ColReference *bc = (ColReference *)(pItemExprTree->child(0)->castToItemExpr());
      str += bc->getText();
    }
  }
}
...
RelExpr *GenericUpdate::bindNode(BindWA *bindWA) {
  ...
  if (getOperatorType() != REL_UNARY_INSERT) inGenericUpdate = bindWA->setInGenericUpdate(TRUE);

  NABoolean returnScanNode = (inGenericUpdate && bindWA->inViewExpansion() &&
                              (getOperatorType() == REL_UNARY_DELETE || getOperatorType() == REL_UNARY_UPDATE));
  ...
  verifyUpdatableTrans(&accessOptions(), CmpCommon::transMode(), il, sqlcodeA, sqlcodeB);
  ...
  if ((isNoRollback() && (getOperatorType() != REL_UNARY_DELETE) &&
       (NOT(Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL)))) &&
      ((CmpCommon::transMode()->getAutoCommit() != TransMode::ON_) || (NAExecTrans(NULL)))) {
    // do not return an error if this is a showplan query being compiled
    // in the second arkcmp.
    const NAString *val = ActiveControlDB()->getControlSessionValue("SHOWPLAN");
    if (NOT((val) && (*val == "ON"))) {
  ...
  if (isNoRollback() || (CmpCommon::transMode()->getRollbackMode() == TransMode::NO_ROLLBACK_)) {
    if ((child(0)->getGroupAttr()->isStream()) || (child(0)->getGroupAttr()->isEmbeddedUpdateOrDelete()) ||
        (updateCurrentOf())) {
      if (getOperatorType() == REL_UNARY_DELETE)
  ...
  if (((getTableName().isLocationNameSpecified() || getTableName().isPartitionNameSpecified()) &&
       (NOT getTableName().isPartitionV2()) && (!Get_SqlParser_Flags(ALLOW_SPECIALTABLETYPE))) &&
  ...
  if (bindWA->findNextScopeWithTriggerInfo() != NULL) {
    ...
    if (updatedTableName_.isATriggerTransitionName(bindWA)) {
      ...
      *CmpCommon::diags() << DgSqlCode(-11020) << DgString0(getTableName().getQualifiedNameAsString());
  ...
  const NATable *naTable = bindWA->getNATable(getTableName(), TRUE, NULL, TRUE /* write reference */);
  if (bindWA->errStatus()) return this;
  ...
  if (naTable->incrBackupEnabled() && (ClusterRole::get_role() == ClusterRole::SECONDARY) &&
      !Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL)) {
    *CmpCommon::diags() << DgSqlCode(-4481) << DgString0("secondary cluster mutation");
    bindWA->setErrStatus();
    return this;
  }

  if (naTable && naTable->readOnlyEnabled() && !Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL)) {
    *CmpCommon::diags() << DgSqlCode(-4223) << DgString0("Insert/Update/Delete/Merge on READ_ONLY table is");
    bindWA->setErrStatus();
    return this;
  }

  if (naTable && naTable->isHbaseTable()) hbaseOper() = TRUE;

  if (naTable && naTable->isHbaseMapTable() && (CmpCommon::getDefault(TRAF_HBASE_MAPPED_TABLES_IUD) == DF_OFF)) {
    *CmpCommon::diags() << DgSqlCode(-4223) << DgString0("Insert/Update/Delete/Merge on HBase mapped tables is");
  ...
  if ((CmpCommon::getDefault(ALLOW_DML_ON_NONAUDITED_TABLE) == DF_OFF) && naTable && naTable->getClusteringIndex() &&
  ...
  if (getOperatorType() == REL_UNARY_INSERT && NOT naTable->isInsertable()) {
    *CmpCommon::diags() << DgSqlCode(-4027)  // 4027 table not insertable
                        << DgTableName(naTable->getTableName().getQualifiedNameAsAnsiString());
    bindWA->setErrStatus();
    return this;
  ...
  if (naTable->isAnMVMetaData() && getTableName().getSpecialType() != ExtendedQualName::MVS_UMD) {
    if (getTableName().getPrototype() == NULL ||
        getTableName().getPrototype()->getSpecialType() !=
            ExtendedQualName::MVS_UMD) {  // ERROR 12075: A Materialized View Metadata Table cannot be directly updated.
  ...
  if ((naTable->isSeabaseTable()) && (naTable->isSeabaseMDTable() || naTable->isSeabasePrivSchemaTable()) &&
      (NOT naTable->isUserUpdatableSeabaseMDTable()) && (NOT Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL))) {
    // IUD on hbase metadata is only allowed for internal queries.
    *CmpCommon::diags() << DgSqlCode(-1391) << DgString0(naTable->getTableName().getQualifiedNameAsAnsiString())
                        << DgString1("metadata");
    bindWA->setErrStatus();
    return this;
  } else if ((naTable->isSeabaseTable()) && (naTable->getTableName().getSchemaName() == SEABASE_REPOS_SCHEMA) &&
             (NOT Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL))) {
    // IUD on hbase metadata is only allowed for internal queries.
    *CmpCommon::diags() << DgSqlCode(-1391) << DgString0(naTable->getTableName().getQualifiedNameAsAnsiString())
  ...
  if ((naTable->isHbaseTable()) && (naTable->isHbaseCellTable() || naTable->isHbaseRowTable()) &&
  ...
  if ((naTable->getSpecialType() == ExtendedQualName::INDEX_TABLE) && naTable->hasTrafReplicaColumn())
    bindWA->setIndexInsertStatement(TRUE);
  ...
  if ((!insertFromValuesList) && (getOperatorType() == REL_UNARY_INSERT)) bindWA->setInsertSelectStatement(TRUE);
  ...
  if (!insertFromValuesList) {
    ...
    if (getOperatorType() == REL_UNARY_INSERT) {
      ...
      if (child(0)->getGroupAttr()->isEmbeddedUpdateOrDelete() || child(0)->getGroupAttr()->isEmbeddedInsert()) {
        *CmpCommon::diags() << DgSqlCode(-4171) << DgString0(getGroupAttr()->getOperationWithinGroup());
        bindWA->setErrStatus();
        return this;
      }
    }
    ...
    if (returnScanNode) {
      ...
      NATable *nTable = bindWA->getNATable(getTableName(), TRUE, NULL, TRUE /* write reference */);
  ...
  CMPASSERT(NOT(updateCurrentOf() && getGroupAttr()->isEmbeddedUpdateOrDelete()));
  ...
  if ((isView) && (getFirstNRows() >= 0)) {
    *CmpCommon::diags() << DgSqlCode(-3242) << DgString0("[FIRST N] not allowed on delete from a view.");

    bindWA->setErrStatus();
    return this;
  }

  NABoolean OpOriginalPartitionV2Table = FALSE;
  if (getOperatorType() == REL_UNARY_INSERT || getOperatorType() == REL_LEAF_INSERT) {
    if (isView) {  // INSERT into a VIEW:
      ...
      //
      bindWA->getCurrentScope()->xtnmStack()->createXTNM();
      boundView = bindWA->bindView(getTableName(), naTable, accessOptions(), removeSelPredTree(), getGroupAttr());
#ifndef NDEBUG
  ...
  } else if (getOperatorType() == REL_UNARY_UPDATE || getOperatorType() == REL_UNARY_DELETE) {
    if (naTableTop->isPartitionV2Table()) {
      if (child(0)->getOperatorType() == REL_SCAN) {
        RelExpr *scanExpr = child(0)->castToRelExpr();
        // This Scan is not from Partition Pruning. Because don't get partition
        // from given information, operation are on original node.
        if (!((Scan *)scanExpr)->isPartitionPruned()) {
          scanNode = (Scan *)scanExpr;
          OpOriginalPartitionV2Table = TRUE;
        }
      }
    } else {
  ...
  if (updateCurrentOf()) {
    CMPASSERT(scanNode);
    scanNode->bindUpdateCurrentOf(bindWA, (getOperatorType() == REL_UNARY_UPDATE));
    if (bindWA->errStatus()) return this;
  }
  ...
  if (isView) {
    ...
    if ((getOperatorType() == REL_INSERT) || (getOperatorType() == REL_UNARY_INSERT) ||
        (getOperatorType() == REL_LEAF_INSERT)) {
    ...
    if (scanNode) naTable = scanNode->getTableDesc()->getNATable();
    CorrName tempName(naTableTop->getTableName(), bindWA->wHeap(), "", getTableName().getLocationName(),
    ...
    if (getOperatorType() == REL_UNARY_UPDATE && this->getTableName().getCorrNameAsString() != "")
      tempName.setCorrName(this->getTableName().getCorrNameAsString());

    TableDesc *naTableToptableDesc = bindWA->createTableDesc(naTableTop, tempName);
    ...
    isScanOnDifferentTable = (naTable->getExtendedQualName() != naTableTop->getExtendedQualName());
  }

  if (bindWA->errStatus()) return this;

  if (naTableTop->isPartitionV2Table() && !OpOriginalPartitionV2Table) {
    return this;
  }

  if ((naTable->hasTrafReplicaColumn() && getOperatorType() == REL_UNARY_INSERT) && !getUpdateCKorUniqueIndexKey()) {
    ItemExpr *endVal = new (bindWA->wHeap()) SystemLiteral(0);
    Int16 numTrafReplicas = naTable->getNumTrafReplicas();
    ItemList *endl = new (bindWA->wHeap()) ItemList(endVal, NULL);
    for (int i = 1; i < numTrafReplicas; i++) {
      ItemExpr *val = new (bindWA->wHeap()) SystemLiteral(0);
      ItemList *currl = new (bindWA->wHeap()) ItemList(val, endl);
      endl = currl;
    }

    ColReference *discardColRef =
        new (bindWA->wHeap()) ColReference(new (bindWA->wHeap()) ColRefName("__discard__", bindWA->wHeap()));
    ItemExpr *tranposeValues = new (bindWA->wHeap()) ItemList(endl, discardColRef);
    // Transpose values must end with Null
    tranposeValues = new (bindWA->wHeap()) ItemList(tranposeValues, NULL);
    ItemExpr *keyCol =
        new (bindWA->wHeap()) ColReference(new (bindWA->wHeap()) ColRefName("__REPLICA__", bindWA->wHeap()));
    RelExpr *transpNode = new (bindWA->wHeap()) Transpose(tranposeValues, keyCol, child(0), bindWA->wHeap());
    child(0) = transpNode;
  }
  ...
  if ((getOperatorType() == REL_UNARY_DELETE) && (!isScanOnDifferentTable && !getUpdateCKorUniqueIndexKey())) {
    const ValueIdList &dkeys = getTableDesc()->getClusteringIndex()->getClusteringKeyCols();
    const ValueIdList &skeys = scanNode->getTableDesc()->getClusteringIndex()->getClusteringKeyCols();
    CollIndex j = skeys.entries();
    for (CollIndex i = 0; i < j; i++) {
      oldToNewMap().addMapEntry(skeys[i].getItemExpr()->getValueId(), dkeys[i].getItemExpr()->getValueId());
    }
  }

  if (getOperatorType() == REL_UNARY_UPDATE && newRecExprTree_) {
    sCacheKey4UpdateSetList_ = " updateSetList:";
    ItemExpr *p = newRecExprTree_;
    getUpdateSetListColName4CacheKey(newRecExprTree_, sCacheKey4UpdateSetList_);
    sCacheKey4UpdateSetList_ += " ";
  }
  ItemExpr *recExpr = removeNewRecExprTree();

  if (recExpr && (getOperatorType() == REL_UNARY_DELETE || getOperatorType() == REL_UNARY_UPDATE)) {
    ...
    if (getOperatorType() == REL_UNARY_UPDATE) {
      ...
      for (CollIndex i = 0; i < recList.entries(); i++) {
        ...
        if (recList[i]->child(0)->getOperatorType() == ITM_ITEM_LIST) leftIsList = TRUE;
        if (recList[i]->child(1)->getOperatorType() == ITM_ITEM_LIST) rightIsList = TRUE;

        if (((Assign *)recList[i])->onRollback()) {
          // On rollback clause currently not allowed with update lists.
          if ((leftIsList) || (rightIsList)) {
            *CmpCommon::diags() << DgSqlCode(-3242) << DgString0(" ON ROLLBACK not supported with SET lists.");
            bindWA->setErrStatus();
            return this;
          }
      ...
      if ((leftIsList) && (NOT rightIsList) && (recList.entries() == 1) &&
        ...
        if (recList[0]->child(1)->getOperatorType() == ITM_ROW_SUBQUERY) {
          RowSubquery *rs = (RowSubquery *)(recList[0]->child(1)->castToItemExpr());
          ...
          UDFunction *rudf = (UDFunction *)(recList[0]->child(1)->castToItemExpr());
        ...
        ItemExprList *exprList = (ItemExprList *)new (bindWA->wHeap()) ItemExprList(expr, bindWA->wHeap());
      ...
      if ((leftIsList) || (rightIsList))  // some elements as lists
      {
        ...
        for (CollIndex i = 0; i < recList.entries(); i++) {
          ...
          assignNode->child(0) = assignNode->child(0)->bindUDFsOrSubqueries(bindWA);
          if (bindWA->errStatus()) return this;

          // Need to bind any UDFs or Subqieries in the expression
          // so that we know the degree before we expand the list.
          assignNode->child(1) = assignNode->child(1)->bindUDFsOrSubqueries(bindWA);
          if (bindWA->errStatus()) return this;

          ItemExprList leftList(assignNode->child(0), bindWA->wHeap());
          ItemExprList rightList(assignNode->child(1), bindWA->wHeap());
          int numLeftElements = (int)leftList.entries();
          int numRightElements = (int)rightList.entries();
          ...
          DefaultToken allowSubqTok = CmpCommon::getDefault(ALLOW_SUBQ_IN_SET);
          if ((allowSubqTok == DF_ON) || (allowSubqTok == DF_SYSTEM)) allowSubqInSet_Enabled = TRUE;

          if (!allowSubqInSet_Enabled) {
            for (CollIndex j = 0; j < rightList.entries(); j++) {
              if (((numLeftElements > 1) || (numRightElements > 1)) &&
          ...
          if (numLeftElements != numRightElements) {
            *CmpCommon::diags() << DgSqlCode(-4023) << DgInt0(numRightElements) << DgInt1(numLeftElements);
            bindWA->setErrStatus();
            return this;
          }
          ...
          for (CollIndex k = 0; k < leftList.entries(); k++) {
            ...
            Assign *assign = new (bindWA->wHeap()) Assign(leftIE, rightIE);
    ...
    if (listedStoi) {
      ...
      short *stoiColumnList = new (bindWA->wHeap()) short[stoiColumnSet.entries()];
  ...
  if ((!isScanOnDifferentTable) &&
      (((getOperatorType() == REL_UNARY_INSERT) && !insertFromValuesList && !getGroupAttr()->isEmbeddedInsert()) ||
       (getOperatorType() == REL_UNARY_UPDATE) || (getOperatorType() == REL_UNARY_DELETE))) {
    ...
    if ((getOperatorType() == REL_UNARY_UPDATE || getOperatorType() == REL_UNARY_DELETE) &&
      ...
      // the second time for the scan below it. But when the table is partition
      // table, it is referenced third, once for the update/delete and other
      // times for the scan below it. For the scan, we should count once.
      if (!bindWA->isPruningPartition()) beforeRefcount = beforeRefcount + 1;
    }
    ...
    if (!isSGTableType)
      baseSeenInSrc =
          bindWA->tableViewUsageList().getViewsOnTable(begSrcUsgIx, endSrcUsgIx, bindWA->viewCount(), tableBaseName,
                                                       getTableName().getSpecialType(), viewName, viewFmtdList);
    ...
    if (CmpCommon::getDefault(R2_HALLOWEEN_SUPPORT) == DF_ON) {
      if (beforeRefcount != afterRefcount) {
        ...
        if ((baseSeenInSrc > beforeRefcount) && ((scanSrc && scanSrc->getTableName().isLocationNameSpecified()) ||
      ...
      if (halloween) {
        ...
        *CmpCommon::diags() << DgSqlCode(viewFmtdList.isNull() ? -4026 : -4060)
                            << DgTableName(tableBaseName.getQualifiedNameAsAnsiString()) << DgString0(viewFmtdList);
        bindWA->setErrStatus();
        return this;
      }
    } else {
      ...
      if (beforeRefcount != afterRefcount) {
        if (!(getOperatorType() == REL_UNARY_DELETE && child(0)->getOperatorType() == REL_SCAN)) {
          ...
          NABoolean cannotUseDP2Locks = naTableBase->getRefsIncompatibleDP2Halloween();
          ...
          if (NOT getHalloweenCannotUseDP2Locks()) setHalloweenCannotUseDP2Locks(cannotUseDP2Locks);
  ...
  if (naTable->isPartitionEntityTable() && CmpCommon::getDefault(TRAF_PARTITIONV2_CONSTRAINT_RAISE_ERROR) == DF_ON) {
    // partition entity table should raise error when constraint return false
    CMPASSERT(getTableDesc()->getCheckConstraints().entries() > 0);

    ItemExpr *trueVal = new (bindWA->wHeap()) BoolVal(ITM_RETURN_TRUE);
    ItemExpr *falseVal = new (bindWA->wHeap()) BoolVal(ITM_RETURN_FALSE);
    ItemExpr *constraint = getTableDesc()->getCheckConstraints()[0].getItemExpr();
    ItemExpr *resultCheck = new (bindWA->wHeap()) BiRelat(ITM_EQUAL, constraint, falseVal);
    if (getCheckCnstrntOfPartv2BaseTable() != 0) {
      resultCheck = new (bindWA->wHeap())
          Case(NULL, new (bindWA->wHeap()) IfThenElse(resultCheck, getCheckCnstrntOfPartv2BaseTable(), trueVal));
      resultCheck = new (bindWA->wHeap()) BiRelat(ITM_EQUAL, resultCheck, falseVal);
    }
    resultCheck = new (bindWA->wHeap())
        Case(NULL, new (bindWA->wHeap()) IfThenElse(resultCheck, new (bindWA->wHeap()) RaiseError(4403), constraint));
    resultCheck->bindNode(bindWA);
    ValueIdList vil = getTableDesc()->getCheckConstraints();
    vil.removeAt(0);
    vil.insertAt(0, resultCheck->getValueId());
    checkConstraints() = vil;
  } else {
  ...
  if (getOperatorType() == REL_UNARY_UPDATE || getOperatorType() == REL_UNARY_DELETE) {
    if (getTableDesc()->getNATable()->isORC()) {
      *CmpCommon::diags() << DgSqlCode(-4223) << DgString0("Update/Delete on ORC table is");
      bindWA->setErrStatus();
      return this;
    }

    if (getTableDesc()->getNATable()->isParquet()) {
      *CmpCommon::diags() << DgSqlCode(-4223) << DgString0("Update/Delete on PARQUET table is");
      bindWA->setErrStatus();
      return this;
    }

    if (getTableDesc()->getNATable()->isAvro()) {
      *CmpCommon::diags() << DgSqlCode(-4223) << DgString0("Update/Delete on AVRO table is");
      bindWA->setErrStatus();
      return this;
    }

    if (getTableDesc()->getNATable()->isHiveTable()) {
      *CmpCommon::diags() << DgSqlCode(-4223) << DgString0("Update/Delete on Hive table is");
      bindWA->setErrStatus();
      return this;
    }
    ...
    //
    const NAColumnArray &keyColArray = getTableDesc()->getNATable()->getClusteringIndex()->getIndexKeyColumns();
    CollIndex numKeyCols = keyColArray.entries();
    const NAColumnArray &scanColArray = scanNode->getTableDesc()->getNATable()->getNAColumnArray();
    ...
    for (CollIndex i = 0; i < numKeyCols; i++) {
      ...
      //
      int colPos = keyColArray[i]->getPosition();
      ...
      if (!isScanOnDifferentTable)
        ...
        ExtendedQualName::SpecialTableType tableType = scanNode->getTableDesc()->getCorrNameObj().getSpecialType();
        ...
        int scanColPos = scanNaCol->getPosition();
        // Get the Scan BaseColumn.
        scanCol = scanNode->getTableDesc()->getColumnList()[scanColPos].getItemExpr();
      }
      ItemExpr *newKeyPred = new (bindWA->wHeap()) BiRelat(ITM_EQUAL, guCol, scanCol);

      // nullable key values need to be compared as regular values instead
      // of returning FALSE.
      ((BiRelat *)newKeyPred)->setSpecialNulls(TRUE);

      newKeyPred->bindNode(bindWA);
      beginKeyPred().insert(newKeyPred->getValueId());

      updateToSelectMap().addMapEntry(newKeyPred->child(0)->getValueId(), newKeyPred->child(1)->getValueId());
  ...
  // QSTUFF hack !
  if (getGroupAttr()->isEmbeddedUpdate()) bindWA->setNameLocListPtr(pLoc);
  bindWA->setInGenericUpdate(inGenericUpdate);
  // QSTUFF
...
NABoolean GenericUpdate::checkForMergeRestrictions(BindWA *bindWA) {
  ...
  if (getTableDesc()->hasUniqueIndexes() && (CmpCommon::getDefault(MERGE_WITH_UNIQUE_INDEX) == DF_OFF)) {
    *CmpCommon::diags() << DgSqlCode(-3241) << DgString0(" unique indexes not allowed.");
    bindWA->setErrStatus();
    return TRUE;
  }
  ...
  {
    *CmpCommon::diags() << DgSqlCode(-3241) << DgString0(" Stream, skip conflict or SET ON ROLLBACK not allowed.");
    bindWA->setErrStatus();
    return TRUE;
  }

  if (getGroupAttr()->isEmbeddedUpdateOrDelete()) {
    *CmpCommon::diags() << DgSqlCode(-3241) << DgString0(" Embedded update/deletes not allowed.");
    bindWA->setErrStatus();
    return TRUE;
  }
  ...
  if ((getInliningInfo().hasInlinedActions()) || (getInliningInfo().isEffectiveGU())) {
    if (getInliningInfo().hasTriggers()) {
      *CmpCommon::diags() << DgSqlCode(-3241) << DgString0(" Triggers not allowed.");
      bindWA->setErrStatus();
      return TRUE;
    }
  }
...
RelExpr *LeafInsert::bindNode(BindWA *bindWA) {
  ...
  for (CollIndex i = 0; i < tgtcols.entries(); i++) {
    Assign *assign;
    assign = new (bindWA->wHeap()) Assign(tgtcols[i].getItemExpr(), baseColRefs()[i], FALSE);
    assign->bindNode(bindWA);
    if (bindWA->errStatus()) return NULL;
    newRecExprArray().insertAt(i, assign->getValueId());
    newRecExpr().insert(assign->getValueId());
...
RelExpr *LeafDelete::bindNode(BindWA *bindWA) {
  ...
  if (TriggersTempTable *tempTableObj = getTrigTemp()) {
    ...
    ItemExpr *col2 = new (bindWA->wHeap()) ColReference(new (bindWA->wHeap()) ColRefName(UNIQUEIUD_COLUMN));
    ...
    for (CollIndex i = 2; i < keycols.entries(); i++) {
      ...
      int keyColPos = keycols[i].getNAColumn()->getPosition();
      baseItemExpr = baseColRefs()[keyColPos];
      keyPred = new (bindWA->wHeap()) BiRelat(ITM_EQUAL, keyItemExpr, baseItemExpr);
...
void LeafDelete::setUpExecPredForUndoUniqueIndex(BindWA *bindWA) {
  ...
  for (CollIndex i = 0; i < indexCols.entries(); i++) {
    execPred = new (bindWA->wHeap()) BiRelat(ITM_EQUAL, indexCols[i].getItemExpr(), baseColRefs()[i]);
...
void LeafDelete::setUpExecPredForUndoTempTable(BindWA *bindWA) {
  ...
  for (CollIndex i = 0; i < tempCols.entries(); i++) {
    ...
    if (colName.data()[0] == '@' && colName.compareTo("@SYSKEY")) continue;
    execPred = new (bindWA->wHeap()) BiRelat(ITM_EQUAL, tempCols[i].getItemExpr(), baseColRefs()[i]);
    execPred->bindNode(bindWA);
    if (bindWA->errStatus()) return;
    executorPred() += execPred->getValueId();
  }
...
RelExpr *BuiltinTableValuedFunction::bindNode(BindWA *bindWA) {
  ...
  //
  if (getOperatorType() == REL_EXPLAIN || getOperatorType() == REL_STATISTICS ||
      getOperatorType() == REL_QUERYINVALIDATION || getOperatorType() == REL_HIVEMD_ACCESS ||
    ...
    if (getOperatorType() == REL_HBASE_ACCESS) {
      ...
      if (NOT naTable) {
        ...
        if (tableDesc) naTable = bindWA->getNATable(corrName, FALSE /*catmanUsages*/, tableDesc);

        if (!tableDesc || bindWA->errStatus()) return this;
      }

      // Allocate a TableDesc and attach it to this.
      //
      TableDesc *td = bindWA->createTableDesc(naTable, corrName);
      if (!td || bindWA->errStatus()) return this;
    ...
    if (getProcAllParamsTree()) {
      ...
      int len = (int)CmpCommon::getDefaultNumeric(VARCHAR_PARAM_DEFAULT_SIZE);
      SQLChar c(NULL, len);
      ...
      for (int i = 0; i < numParams(); i++) {
        ...
        if (chld_cs == CharInfo::UNICODE) {
          ie = new (bindWA->wHeap()) Translate(getProcAllParamsVids()[i].getItemExpr(), Translate::UNICODE_TO_ISO88591);
          ie = ie->bindNode(bindWA);
...
RelExpr *ControlQueryShape::bindNode(BindWA *bindWA) {
  ...
  if (alterArkcmpEnvNow()) {
    if (getShape())
      ...
      if (ActiveControlDB()->getRequiredShape()) ActiveControlDB()->getRequiredShape()->holdShape() = holdShape();
    }
  }
...
RelExpr *ControlQueryDefault::bindNode(BindWA *bindWA) {
  ...
  if (holdOrRestoreCQD_ == 0) {
    ...
    if (affectYourself) ActiveControlDB()->setControlDefault(this);
...
///////////////////////////////////////////////////////////////
// This method is called when multiple cqds are specified in
// a single statement.
//  control query defaults <cqds> [hold | restore | reset]
//
// cqds:  token1 value1 [ , token2 value 2 , ... ]
// cqds:  token1 [ , token2, ... ] [ HOLD | RESTORE | RESET ]
//
//  Example:
//   control query defaults TRAF_NO_DTM_XN 'OFF', TRAF_NUM_OF_SALT_PARTNS '8'
//   control query defaults TRAF_NO_DTM_XN, TRAF_NUM_OF_SALT_PARTNS HOLD

//
//////////////////////////////////////////////////////////////////
RelExpr *ControlQueryDefaults::bindNode(BindWA *bindWA) {
  if (nodeIsBound()) {
    bindWA->getCurrentScope()->setRETDesc(getRETDesc());
    return this;
  }
  ...
  NAString token;
  NAString value;
  DefaultConstants attrEnum;
  for (int i = 0; i < cqds_->entries(); i++) {
    token.clear();
    value.clear();

    GroupOfNAString *gon = (*cqds_)[i];
    token = gon->val1();
    value = gon->val2();

    if (((getHoldOrRestoreCQD() > 0) || (reset_)) && (NOT value.isNull())) {
      *CmpCommon::diags() << DgSqlCode(-3242)
                          << DgString0("Default value cannot be specified with HOLD, RESTORE or RESET option.");

      if (bindWA) bindWA->setErrStatus();
      return NULL;
    }

    if (getHoldOrRestoreCQD() == 0) {
      attrEnum = affectYourself ? defs.validateAndInsert(token.data(), value, reset_)
                                : defs.validate(token.data(), value, reset_);
      if (attrEnum < 0) {
        if (bindWA) bindWA->setErrStatus();
        return NULL;
      }
    ...
    } else if ((getHoldOrRestoreCQD() > 0) && (affectYourself)) {
      attrEnum = defs.holdOrRestore(token.data(), getHoldOrRestoreCQD());
      if (attrEnum < 0) {
        if (bindWA) bindWA->setErrStatus();
        return NULL;
      }
...
RelExpr *ControlTable::bindNode(BindWA *bindWA) {
  ...
  NABoolean ok =
      alterArkcmpEnvNow() ? ActiveControlDB()->setControlTableValue(this) : ActiveControlDB()->validate(this);
  if (NOT ok) {
    if (bindWA) bindWA->setErrStatus();
    return NULL;
  }
...
RelExpr *ControlSession::bindNode(BindWA *bindWA) {
  ...
  NABoolean ok =
      alterArkcmpEnvNow() ? ActiveControlDB()->setControlSessionValue(this) : ActiveControlDB()->validate(this);
  if (NOT ok) {
    if (bindWA) bindWA->setErrStatus();
    return NULL;
  }
...
RelExpr *SetSessionDefault::bindNode(BindWA *bindWA) {
  ...
  if (getOperatorType() == REL_SET_SESSION_DEFAULT) {
    ...
    //       to set parserflags
    if ((token_ == "SET_PARSERFLAGS") || (token_ == "RESET_PARSERFLAGS")) {
...
RelExpr *RelSetTimeout::bindNode(BindWA *bindWA) {
  ...
  if (!strcmp("*", userTableName_.getCorrNameAsString())) isForAllTables_ = TRUE;
...
RelExpr *Describe::bindNode(BindWA *bindWA) {
  ...
  if (getFormat() == CONTROL_DEFAULTS_) {
    if (getDescribedTableName().getQualifiedNameObj().getObjectName() == ShowSchema::ShowControlDefaultSchemaMagic()) {
      ...
      //
      *CmpCommon::diags() << DgSqlCode(-ABS(ShowSchema::DiagSqlCode())) << DgCatalogName(cat) << DgSchemaName(sch);
      bindWA->setErrStatus();
      return this;
    }
    if (getDescribedTableName().getQualifiedNameObj().getObjectName() ==
      ...
      *CmpCommon::diags() << DgSqlCode(-ABS(GetControlDefaults::DiagSqlCode())) << DgString0(cqdPairs);
      bindWA->setErrStatus();
      return this;
    }
  }
...
#define MAX_DESCRIBE_LEN 3000  // e.g., SQL/MP Views.ViewText column
  ...
  TrafDesc *column_desc = TrafMakeColumnDesc(table_desc->tableDesc()->tablename, "DESCRIBE__COL",
                                             colnumber,  // INOUT
                                             REC_BYTE_V_ASCII, MAX_DESCRIBE_LEN,
                                             offset,  // INOUT
                                             FALSE /*not null*/, SQLCHARSETCODE_UNKNOWN, NULL);
  ...
  if (!describedTableName_.getQualifiedNameObj().getObjectName().isNull()) {
    if (getIsControl()) describedTableName_.applyDefaults(bindWA, bindWA->getDefaultSchema());
    if (NOT getIsControl()) {
      ...
      if (describedTableName_.getQualifiedNameObj().getSchemaName().isNull()) setToTryPublicSchema(TRUE);

      if ((getFormat() == Describe::INVOKE_) ||
          (getFormat() == Describe::SHOWDDL_) && (getLabelAnsiNameSpace() == COM_TABLE_NAME) && (NOT getIsSchema())) {
        bindWA->getNATableInternal(describedTableName_, TRUE, NULL, FALSE, FALSE);
        if (bindWA->errStatus()) {
          return this;
        }
      } else
...
RelExpr *RelLock::bindNode(BindWA *bindWA) {
  ...
  if (isView && !naTable->isAnMV()) {
    *CmpCommon::diags() << DgSqlCode(-4222) << DgString0("Views");
    bindWA->setErrStatus();
    return this;
  } else {
  ...
  for (CollIndex i = 0; i < baseTableNameList_.entries(); i++) {
    ...
    if (!haveTDforThisBT) {
      ...
      tabNames += NAString(", ") + bt.getQualifiedNameObj().getQualifiedNameAsAnsiString();
  ...
  if ((isView) || (tabIds_.entries() > 1) || (baseTableNameList_.entries() > 1) ||
...
RelExpr *RelTransaction::bindNode(BindWA *bindWA) {
  ...
  if ((CmpCommon::getDefault(TRAF_SAVEPOINTS) == DF_OFF) &&
      ((type_ == BEGIN_SAVEPOINT_) || (type_ == COMMIT_SAVEPOINT_) || (type_ == ROLLBACK_SAVEPOINT_))) {
    *CmpCommon::diags() << DgSqlCode(-EXE_SAVEPOINT_OPERATION_ERROR);
    bindWA->setErrStatus();
    return this;
  }
  ...
  if (mode_) {
    if ((mode_->autoCommit() != TransMode::AC_NOT_SPECIFIED_) || (mode_->getAutoBeginOn() != 0) ||
      ...
      if (NOT(mode_->isolationLevel() == TransMode::IL_NOT_SPECIFIED_ &&
        ...
        *CmpCommon::diags() << DgSqlCode(-3242)
                            << DgString0(
                                   "Isolation level or Access mode options cannot be specified along with autocommit "
                                   "or autobegin options.");
        bindWA->setErrStatus();
        return this;
      }
    } else {
      ...
      if (mode_->isolationLevel() == TransMode::IL_NOT_SPECIFIED_) {
        if (CmpCommon::getDefault(ISOLATION_LEVEL_FOR_UPDATES) == DF_NONE)
          bindWA->getSchemaDB()->getDefaults().getIsolationLevel(mode_->isolationLevel());  // short int
        else
          bindWA->getSchemaDB()->getDefaults().getIsolationLevel(mode_->isolationLevel(),  // short int
      ...
      if (mode_->accessMode() == TransMode::AM_NOT_SPECIFIED_)
        mode_->updateAccessModeFromIsolationLevel(mode_->getIsolationLevel());  // enum

      // 3114 Transaction access mode RW is incompatible with isolation level RU
      if (mode_->accessMode() == TransMode::READ_WRITE_ && mode_->isolationLevel() == TransMode::READ_UNCOMMITTED_) {
...
RelExpr *Transpose::bindNode(BindWA *bindWA) {
  ...
  if (keyColumn) {
    ...
    //
    ValueIdUnion *keyVidu = new (bindWA->wHeap()) ValueIdUnion(constVals, NULL_VALUE_ID);
    ...
    for (k = 0; k < numTransVals; k++) {
      ...
      for (j = 0; j < numTransItems; j++) {
        ...
        if (transValueUnionExpr == NULL) {
          ...
          transValueUnionExpr = new (bindWA->wHeap()) ItemList(transValueUnionExpr, transItem[k]);
        }
      }
      ...
      //
      transVals.clear();
      if (transValueUnionExpr != NULL) transValueUnionExpr->convertToValueIdList(transVals, bindWA, ITM_ITEM_LIST);
      if (bindWA->errStatus()) return this;
      ...
      //
      ValueIdUnion *valVidu = new (bindWA->wHeap()) ValueIdUnion(transVals, NULL_VALUE_ID);
...
RelExpr *Pack::bindNode(BindWA *bindWA) {
  ...
  if (packingExprTree_) {
    ...
    for (CollIndex i = 0; i < packingExpr().entries(); i++) {
      ...
      resultTable->addColumn(bindWA, colRefName, packingExpr().at(i), USER_COLUMN, colName);
    }
  } else  // no packing expr tree, get all the columns from child.
  {
    ...
    for (CollIndex i = 0; i < childTableVidList.entries(); i++) {
      ...
      PackFunc *packedItemExpr = new (bindWA->wHeap()) PackFunc(childItemExpr, pfie);
      ...
      resultTable->addColumn(bindWA, colRefName, packedItemExpr->getValueId(), USER_COLUMN, heading);
      // $$$
  ...
  if (env && atol(env) > 0) {
    ...
    ItemExpr *indexHostVar =
        new (bindWA->wHeap()) HostVar(hostVarName, new (bindWA->wHeap()) SQLInt(bindWA->wHeap(), TRUE, FALSE), TRUE);
    ...
    for (CollIndex i = 0; i < packedCols.entries(); i++) {
      ...
      unPackItem = new (bindWA->wHeap())
          UnPackCol(packedCols[i].getItemExpr(), indexHostVar, width, base, colType->supportsSQLnull(), colType);

      numRows = new (bindWA->wHeap()) UnPackCol(packedCols[i].getItemExpr(), new (bindWA->wHeap()) SystemLiteral(0),
                                                typeInt->getNominalSize(), 0, FALSE, typeInt);

      unPackExpr = (unPackExpr ? new (bindWA->wHeap()) ItemList(unPackExpr, unPackItem) : unPackItem);

      rowFilter = (rowFilter ? new (bindWA->wHeap()) ItemList(rowFilter, numRows) : numRows);
    }

    RelExpr *unpack =
        new (bindWA->wHeap()) UnPackRows(pf, unPackExpr, rowFilter, NULL, this, indexHostVar->getValueId());
...
RelExpr *Rowset::bindNode(BindWA *bindWA) {
  // If this node has already been bound, we are done.
  if (nodeIsBound()) return this->transformRelexpr_;
  ...
  for (hostVarTree = inputHostvars_; hostVarTree; hostVarTree = hostVarTree->child(1)) {
    CMPASSERT(hostVarTree->getOperatorType() == ITM_ITEM_LIST);
    HostVar *hostVar = (HostVar *)hostVarTree->getChild(0);

    if (hostVar->getOperatorType() != ITM_HOSTVAR || hostVar->getType()->getTypeQualifier() != NA_ROWSET_TYPE) {
    ...
    if (maxRowsetSize == 0)
      ...
      if (hostVarType->getNumElements() < maxRowsetSize) maxRowsetSize = hostVarType->getNumElements();
    }
    ...
    if (hostVarElemType->getTypeQualifier() == NA_UNKNOWN_TYPE) hostVarElemNullInd = TRUE;
  ...
  if (!sizeExpr_ && bindWA->getHostArraysArea()) {
    ...
    if ((bindWA->getHostArraysArea()->getInputArrayMaxSize() > 0) && (!sizeExpr_)) {
      ...
      HostVar *node =
          new (bindWA->wHeap()) HostVar(name, new (bindWA->wHeap()) SQLInt(bindWA->wHeap(), TRUE, FALSE), TRUE);
  ...
  if (sizeExpr_) {
    if (sizeExpr_->getOperatorType() == ITM_CONSTANT) {
      if (((ConstValue *)sizeExpr_)->getType()->getTypeQualifier() != NA_NUMERIC_TYPE) {
      ...
      if (((ConstValue *)sizeExpr_)->getExactNumericValue() > maxRowsetSize) {
        ...
        *CmpCommon::diags() << DgSqlCode(-30002) << DgInt0((int)((ConstValue *)sizeExpr_)->getExactNumericValue())
        ...
        maxRowsetSize = (int)((ConstValue *)sizeExpr_)->getExactNumericValue();
      }
    } else if (!((sizeExpr_->getOperatorType() == ITM_HOSTVAR &&
                  ((HostVar *)sizeExpr_)->getType()->getTypeQualifier() == NA_NUMERIC_TYPE) ||
    ...
    ItemExpr *constrPred = new (bindWA->wHeap()) BiRelat(ITM_GREATER, sizeExpr_, maxSize);
    rowsetSizeExpr = new (bindWA->wHeap()) IfThenElse(constrPred, neg, sizeExpr_);
    ...
    // values.
    rowsetSizeExpr =
        new (bindWA->wHeap()) Cast(rowsetSizeExpr, new (bindWA->wHeap()) SQLInt(bindWA->wHeap(), TRUE, FALSE));
  ...
  ItemExpr *indexHostVar = new (bindWA->wHeap()) HostVar(indexName, indexType,
                                                         TRUE  // is system-generated
  );
  ...
  for (hostVarTree = inputHostvars_; hostVarTree; hostVarTree = hostVarTree->child(1)) {
    ...
    SQLRowset *hostVarType = (SQLRowset *)hostVar->getType();
    NAType *hostVarElemType = hostVarType->getElementType();
    int hostVarElemSize = hostVarElemType->getTotalSize();
    NABoolean hostVarElemNullInd = !(hostVar->getIndName().isNull());
    ...
    hostVarElemSize = ALIGN(hostVarElemSize, hostVarElemType->getDataAlignment());
    ...
    ItemExpr *unPackCol = new (bindWA->wHeap()) RowsetArrayScan(hostVar,          // Rowset Host Var array
                                                                indexHostVar,     // Index
                                                                maxRowsetSize,    // Cannot go over this size
                                                                hostVarElemSize,  // Element size in bytes
                                                                hostVarElemNullInd, hostVarElemType);
    ...
    unPackExpr = (unPackExpr ? new (bindWA->wHeap()) ItemList(unPackExpr, unPackCol) : unPackCol);
  }
  ...
  if (arrayArea && (!(arrayArea->getHasDerivedRowsets())) && arrayArea->hasHostArraysInWhereClause() &&
  ...
  if (indexExpr_) {
     ...
     */
    ItemExpr *unPackCol =
        new (bindWA->wHeap()) RowsetArrayScan(indexHostVar,   // Index
                                              indexHostVar,   // Index
                                              maxRowsetSize,  // Cannot go over this size
                                              indexType->getTotalSize(), 0, indexType, ITM_ROWSETARRAY_ROWID);
  ...
  ItemExpr *inputs = ((indexExpr_) ? new (bindWA->wHeap()) ItemList(inputHostvars_, indexHostVar) : inputHostvars_);
  ...
  RelExpr *newSubTree = (new (bindWA->wHeap()) UnPackRows(maxRowsetSize, unPackExpr, rowsetSizeExpr, NULL, tupleExpr,
  ...
  if (arrayArea && (!(arrayArea->getHasDerivedRowsets())) &&
      (arrayArea->hasInputRowsetsInSelectPredicate() != HostArraysWA::YES_)) {
...
RelExpr *RowsetRowwise::bindNode(BindWA *bindWA) {
  ...
  if ((arrayArea->rwrsMaxSize()->getOperatorType() != ITM_CONSTANT) ||
      (((ConstValue *)arrayArea->rwrsMaxSize())->getType()->getTypeQualifier() != NA_NUMERIC_TYPE)) {
    // 30003 Rowset size must be an integer host variable or an
    //       integer constant
    *CmpCommon::diags() << DgSqlCode(-30003);
    bindWA->setErrStatus();
  ...
  if (arrayArea->partnNum()) return child(0)->castToRelExpr();

  int maxRowsetSize = (int)((ConstValue *)arrayArea->rwrsMaxSize())->getExactNumericValue();

  NAType *typ = new (bindWA->wHeap()) SQLInt(bindWA->wHeap(), FALSE, FALSE);
  ItemExpr *rwrsInputSizeExpr = new (bindWA->wHeap()) Cast(arrayArea->inputSize(), typ);
  if (bindWA->errStatus()) return this;

  ItemExpr *rwrsMaxInputRowlenExpr = new (bindWA->wHeap()) Cast(arrayArea->rwrsMaxInputRowlen(), typ);
  if (bindWA->errStatus()) return this;
  ...
  // Construct the replacement tree for the Rowset operator.
  RelExpr *newSubTree = (new (bindWA->wHeap()) UnPackRows(maxRowsetSize, rwrsInputSizeExpr, rwrsMaxInputRowlenExpr,
                                                          rwrsBufferAddrExpr, child(0)));
...
RelExpr *RowsetInto::bindNode(BindWA *bindWA) {
  ...
  // Do any extra checking at this moment.
  for (hostVarTree = outputHostvars_; hostVarTree; hostVarTree = hostVarTree->child(1)) {
    numOutputHostvars++;
    CMPASSERT(hostVarTree->getOperatorType() == ITM_ITEM_LIST);
    HostVar *hostVar = (HostVar *)hostVarTree->getChild(0);
      ...
      if (((ConstValue *)sizeExpr_)->getExactNumericValue() > maxRowsetSize) {
        ...
        *CmpCommon::diags() << DgSqlCode(-30002) << DgInt0((int)((ConstValue *)sizeExpr_)->getExactNumericValue())
        ...
        return NULL;
                 ...
                 ((HostVar *)sizeExpr_)->getType()->getFSDatatype() == REC_BIN32_SIGNED)) {
      // 30003 Rowset size must be an integer host variable or an
      //       integer constant
      *CmpCommon::diags() << DgSqlCode(-30003);
      bindWA->setErrStatus();
  ...
  if (numOutputHostvars != childTableVidList.entries()) {
    // 4094 The number of output host vars  ($0) must equal the number of cols
    *CmpCommon::diags() << DgSqlCode(-4094) << DgInt0(numOutputHostvars) << DgInt1(childTableVidList.entries());
    bindWA->setErrStatus();
    return NULL;
  }
  ...
  for (hostVarTree = outputHostvars_, i = 0; hostVarTree; hostVarTree = hostVarTree->child(1), i++) {
    HostVar *hostVar = (HostVar *)hostVarTree->getChild(0);
    ...
    if (NOT targetType.isCompatible(sourceType)) {
      ...
      if (targetType.getTypeQualifier() == NA_CHARACTER_TYPE && sourceType.getTypeQualifier() == NA_CHARACTER_TYPE &&
    ...
    if (relaxCharTypeMatchingRule == TRUE)
      sourceExpr = new (bindWA->wHeap()) Translate(sourceExpr, Translate::ISO88591_TO_UNICODE);
    ...
    ItemExpr *packCol = new (bindWA->wHeap()) RowsetArrayInto(sourceExpr,
                                                              rowsetSizeExpr,   // Runtime size
                                                              maxRowsetSize,    // Cannot go over this size
                                                              hostVarElemSize,  // Element size in bytes
                                                              hostVarElemNullInd, hostVarType);
    ...
    packExpr = (packExpr ? new (bindWA->wHeap()) ItemList(packExpr, packCol) : packCol);
  }

  // Construct the replacement tree for the RowsetInto operator.
  RelExpr *newSubTree = (new (bindWA->wHeap()) Pack(maxRowsetSize, child(0)->castToRelExpr(), packExpr));
  ...
  ptrReqOrder = new (bindWA->wHeap()) ValueIdList(((RelRoot *)(RelExpr *)newSubTree->child(0))->reqdOrder());
...
RelExpr *IsolatedScalarUDF::bindNode(BindWA *bindWA) {
  if (nodeIsBound()) {
    bindWA->getCurrentScope()->setRETDesc(getRETDesc());
    return this;
  }
...
RelExpr *CallSP::bindNode(BindWA *bindWA) {
  ...
  NAString pSchema = ActiveSchemaDB()->getDefaults().getValue(PUBLIC_SCHEMA_NAME);
  ComSchemaName pubSchema(pSchema);
  NAString pubSchemaIntName = "";
  if ((getRoutineName().getSchemaName().isNull()) && (!pubSchema.getSchemaNamePart().isEmpty())) {
    pubSchemaIntName = pubSchema.getSchemaNamePart().getInternalName();
  }

  QualifiedName *packageRoutineName = NULL;
  // Invoke GetNARoutine () to retrieve the corresponding NARoutine from
  // NARoutineDB_
  QualifiedName name = getRoutineName();
  if (name.isAmbiguous()) {
    packageRoutineName = new ((NAHeap *)bindWA->wHeap()) QualifiedName(name);
    packageRoutineName->convertToPackageRoutineName();
  }
  ...
  TrafDesc *catRoutine = NULL;

  NARoutine *routine = NULL;

  // NAHeap *heap = CmpCommon::statementHeap();
  NAHeap *heap = (NAHeap *)bindWA->wHeap();

  NARoutineDBKey key1(name, heap);
  routine = bindWA->getSchemaDB()->getNARoutineDB()->get(bindWA, &key1);

  if (NULL == routine && NULL != packageRoutineName) {
    NARoutineDBKey key2(*packageRoutineName, heap);
    routine = bindWA->getSchemaDB()->getNARoutineDB()->get(bindWA, &key2);
    if (NULL != routine) {
      name = *packageRoutineName;
      packageRoutineName = NULL;
    }
  }

  // in open source, only the SEABASE catalog is allowed.
  // Return an error if some other catalog is being used.
  if (NULL == routine && (NOT name.isSeabase()) && (NOT name.isHive())) {
    // For names with three parts and the first parts is not a
    // valid catalog name, try binding them as package routines
    if (packageRoutineName) {
      name = *packageRoutineName;
      // set to NULL so that we will not check for package routine
      // again later
      packageRoutineName = NULL;
      name.applyDefaults(defaultSchema);
      catRoutine = cmpSBD.getSeabaseRoutineDesc(name.getCatalogName(), name.getSchemaName(), name.getObjectName());
    }
    if (catRoutine) {
      setRoutineName(name);
      bindWA->setCurrSPName(&name);
    } else {
      // If the procedure name cannot be interpreted as a
      // package routine, clear the error and return invalid
      // catalog name instead.
      CmpCommon::diags()->clear();
      *CmpCommon::diags() << DgSqlCode(-1002) << DgCatalogName(getRoutineName().getCatalogName());
  ...
  // try public schema
  if (!routine && !catRoutine && !pubSchemaIntName.isNull()) {
    ...
    bindWA->resetErrStatus();
    catRoutine = cmpSBD.getSeabaseRoutineDesc(getRoutineName().getCatalogName(), getRoutineName().getSchemaName(),
                                              getRoutineName().getObjectName());
    if (!bindWA->errStatus() && catRoutine) {  // if found in public schema, do not show previous error
      CmpCommon::diags()->clear();
    }
  }

  if (!routine && !catRoutine && packageRoutineName) {
    QualifiedName name = *packageRoutineName;
    name.applyDefaults(defaultSchema);
    ...
    if (catRoutine != NULL) {
      CmpCommon::diags()->clear();
      // found, set it in this NARoutine
      setRoutineName(name);
      bindWA->setCurrSPName(&name);
    } else {
      // restore previous error by removing the last error of
      // binding as package routine
      CmpCommon::diags()->removeLastErrorCondition();
    }
  }

  if (bindWA->violateAccessDefaultSchemaOnly(getRoutineName())) return NULL;

  if (NULL == routine && NULL == catRoutine) {
  ...
  if (NULL == routine) {
    ...
    // Add NARoutine to the NARoutineDB cache.
    if (bindWA->getSchemaDB()->getNARoutineDB()->cachingMetaData())
      bindWA->getSchemaDB()->getNARoutineDB()->put(routine);
  }
  ...
  if (bindWA->inDDL() && bindWA->isInTrigger()) {
    ParNameLocList *pNameLocList = bindWA->getNameLocListPtr();
    if (pNameLocList) {
      ParNameLoc *pNameLoc = pNameLocList->getNameLocPtr(getRoutineName().getNamePosition());
      CMPASSERT(pNameLoc);
  ...
  if (bindWA->isInTrigger() && getNARoutine()->hasOutParams()) {
  ...
  if (bindWA->isInTrigger() && getNARoutine()->hasResultSets()) {
  ...
  if (numSuppliedParams != numParams) {
    *CmpCommon::diags() << DgSqlCode(-UDR_BINDER_INCORRECT_PARAM_COUNT)
                        << DgTableName(getRoutineName().getQualifiedNameAsString()) << DgInt0((int)numParams)
                        << DgInt1((int)numSuppliedParams);
    bindWA->setErrStatus();
    bindWA->setBindingCall(FALSE);
    return this;
  }
  ...
  populateAndBindItemExpr(getRWProcAllParamsTree(), bindWA);
  if (bindWA->errStatus()) {
    bindWA->setBindingCall(FALSE);
    return this;
  }
  ...
  for (idx = 0; idx < numHVorDPs; idx++) {
    ...
    local_HVorDPs.insertAt(bWA_HVorDPs[idx]->getHVorDPIndex() - 1, bWA_HVorDPs[idx]);
  }
  ...
  for (idx1 = 0; idx1 < numHVorDPs; idx1++) {
    ...
    const NAString &name1 =
        (src->getOperatorType() == ITM_HOSTVAR) ? ((HostVar *)src)->getName() : ((DynamicParam *)src)->getName();
    ...
    for (idx2 = idx1; idx2 < numHVorDPs; idx2++) {
      ...
      if (!encounteredElement && dest->getHVorDPIndex() >= currParamIndex) {
        ...
        dest->setPMOrdPosAndIndex(dest->getParamMode(), dest->getOrdinalPosition(), currParamIndex);
        continue;
      }

      // The parameter is already corrected
      if (dest->getHVorDPIndex() < currParamIndex) continue;

      const NAString &name2 =
          (dest->getOperatorType() == ITM_HOSTVAR) ? ((HostVar *)dest)->getName() : ((DynamicParam *)dest)->getName();
      ComColumnDirection mode2 = dest->getParamMode();

      if (name2.compareTo("") == 0) continue;
    ...
    if (encounteredElement) currParamIndex++;
  }

  // Restore the bindWA's HVorDP list since it might be needed
  // while binding the root node in case of HVs.
  bindWA->clearHVorDPinSPDups();
  for (idx = 0; idx < numHVorDPs; idx++) bindWA->addHVorDPToSPDups(local_HVorDPs[idx]);
  ...
  if ((getProcInputParamsVids().entries() != 0) && (hasSubquery() || hasUDF())) {
    Tuple *inTuple =
        new (bindWA->wHeap()) Tuple(getProcInputParamsVids().rebuildExprTree(ITM_ITEM_LIST), bindWA->wHeap());
  ...
  if (FALSE == udrReferenced) {
    ...
    udrStoi->setAnsiName(convertNAString(getRoutineName().getQualifiedNameAsAnsiString(), bindWA->wHeap()));

    OptUdrOpenInfo *udrOpenInfo = new (bindWA->wHeap())
        OptUdrOpenInfo(udrStoi, getRoutineName().getQualifiedNameAsAnsiString(), (NARoutine *)getNARoutine());
  ...
  if (getNARoutine()->isProcedure()) bindWA->setHasCallStmts(TRUE);
...
void IsolatedNonTableUDR::populateAndBindItemExpr(ItemExpr *param, BindWA *bindWA) {
  ...
  ComColumnDirection mode = getEffectiveNARoutine()->getParams()[ordinalPosition - 1]->getColumnMode();
  ...
  if (ITM_ITEM_LIST == param->getOperatorType()) {
    ...
    populateAndBindItemExpr((*param).child(0), bindWA);

    if (bindWA->errStatus()) return;

    // Now for the right child
    CMPASSERT((ItemExpr *)NULL != (*param).child(1));
    populateAndBindItemExpr((*param).child(1), bindWA);
...
void IsolatedNonTableUDR::setInOrOutParam(ItemExpr *expr, ComColumnDirection paramMode, BindWA *bindWA) {
  // Should not get here..
  CMPASSERT(FALSE);
}
...
void CallSP::setInOrOutParam(ItemExpr *expr, ComColumnDirection paramMode, BindWA *bindWA) {
  ...
  const NABoolean isJdbc = (CmpCommon::getDefault(JDBC_PROCESS) == DF_ON ? TRUE : FALSE);
  const NABoolean isOdbc = (CmpCommon::getDefault(ODBC_PROCESS) == DF_ON ? TRUE : FALSE);

  const NAType &inputType = inputTypeId.getType();
  // 10-061031-0188-End

  if (COM_INPUT_COLUMN == paramMode || COM_INOUT_COLUMN == paramMode) {
    ...
    if (!hasSubquery()) hasSubquery() = expr->containsSubquery();

    // Check to see if we have a UDF as an input
    if (!hasUDF()) hasUDF() = (expr->containsUDF() != NULL);
    ...
    if (!(NA_UNKNOWN_TYPE == inputType.getTypeQualifier() || paramType.isCompatible(inputType) ||
      ...
      if (throwInTranslateNode == FALSE) {
        ...
        *CmpCommon::diags() << DgSqlCode(-UDR_BINDER_PARAM_TYPE_MISMATCH) << DgInt0(ordinalPosition)
                            << DgTableName(getRoutineName().getQualifiedNameAsString())
                            << DgString0(inputType.getTypeSQLname(TRUE)) << DgString1(paramType.getTypeSQLname(TRUE));
        bindWA->setErrStatus();
        return;
      }
    }  // if NOT isCompatible
    ...
    if (!(inputType == paramType)) {
      ...
      Cast *retExpr = new (bindWA->wHeap()) Cast(boundExpr, &paramType, ITM_CAST, TRUE);
  ...
  if (COM_OUTPUT_COLUMN == paramMode || COM_INOUT_COLUMN == paramMode) {
    if (!(ITM_HOSTVAR == expr->getOperatorType() || ITM_DYN_PARAM == expr->getOperatorType())) {
      *CmpCommon::diags() << DgSqlCode(-UDR_BINDER_OUTVAR_NOT_HV_OR_DP) << DgInt0(ordinalPosition)
                          << DgTableName(getRoutineName().getQualifiedNameAsString());
      bindWA->setErrStatus();
      return;
    }  // if NOT HOSTVAR or DYNAMIC PARAM

    NATypeToItem *paramTypeItem = new (bindWA->wHeap()) NATypeToItem(naColumn.mutateType());
    ...
    const NAString &userParamName =
        ...
        (ITM_HOSTVAR == expr->getOperatorType()) ? ((HostVar *)expr)->getName() : ((DynamicParam *)expr)->getName();
    ...
    if (formalParamName.isNull() || (!userParamName.isNull() && (isJdbc || isOdbc))) {
      colParamName = &userParamName;
    }

    ColRefName *columnName = new (bindWA->wHeap()) ColRefName(*colParamName, bindWA->wHeap());
...
RelExpr *TableMappingUDF::bindNode(BindWA *bindWA) {
  ...
  if ((NOT name.isSeabase()) && (NOT name.isHive())) {
    *CmpCommon::diags() << DgSqlCode(-1002) << DgCatalogName(name.getCatalogName());
  ...
  if (tmudfRoutine == NULL) {
    ...
    OperatorTypeEnum opType = PredefinedTableMappingFunction::nameIsAPredefinedTMF(tmfuncName);
    ...
    if (opType != REL_ANY_TABLE_MAPPING_UDF) {
      ...
      CmpCommon::diags()->rewind(diagsMark);
      if (!errStatus) bindWA->resetErrStatus();

      // create a new RelExpr
      result = new (bindWA->wHeap()) PredefinedTableMappingFunction(
          getArity(), tmfuncName, const_cast<ItemExpr *>(getProcAllParamsTree()), opType);
      ...
      for (int i = 0; i < getArity(); i++) result->child(i) = child(i);

      if (opType == REL_TABLE_MAPPING_BUILTIN_LOG_READER || opType == REL_TABLE_MAPPING_BUILTIN_JDBC) {
        ...
        // will be removed.
        (*CmpCommon::diags()) << DgSqlCode(4323) << DgString0(tmfuncName.getExposedNameAsAnsiString());
      }
  ...
  for (int i = 0; i < getArity(); i++) {
    ...
    for (CollIndex j = 0; j < numChildCols; j++) {
      ...
      if (childColType.getSimpleTypeName() == "NUMERIC" &&
          getBinaryStorageSize(childColType.getPrecision()) != childColType.getNominalSize()) {
        ...
        adjustedChildColType = new (heap) SQLNumeric(
            heap, getBinaryStorageSize(childColType.getPrecision()), childColType.getPrecision(),
            childColType.getScale(), ((const NumericType &)childColType).isSigned(), childColType.supportsSQLnull());
      }

      childCol = new (heap)
          NAColumn(childRetDesc->getColRefNameObj(j).getColName().data(), j, adjustedChildColType->newCopy(heap), heap);
    ...
    // get child root
    CMPASSERT(child(i)->getOperator().match(REL_ROOT) || child(i)->getOperator().match(REL_RENAME_TABLE));
    RelRoot *myChild;
    ...
    // actual columns
    for (ValueId cp = childPartition.init(); childPartition.next(cp); childPartition.advance(cp)) {
      NABoolean negate;
      ConstValue *cv = cp.getItemExpr()->castToConstValue(negate);

      if (cv && cv->canGetExactNumericValue()) {
        int scale = 0;
        long ordinal = cv->getExactNumericValue(scale);
        ...
        if (!negate && scale == 0 && ordinal >= 1 && ordinal <= vidList.entries()) {
          ...
          *CmpCommon::diags() << DgSqlCode(-11154) << DgInt0(ordinal) << DgString0("PARTITION BY")
    ...
    for (CollIndex co = 0; co < childOrder.entries(); co++) {
      ...
      if (ie->getOperatorType() == ITM_INVERSE) ie = ie->child(0);
      ...
      if (cv && cv->canGetExactNumericValue()) {
        ...
        if (!negate && scale == 0 && ordinal >= 1 && ordinal <= vidList.entries())
          if (ie == childOrder[co].getItemExpr()) {
            ...
            ItemExpr *inv = new (bindWA->wHeap()) InverseOrder(vidList[ordinal - 1].getItemExpr());
            inv->synthTypeAndValueId();
            childOrder[co] = inv->getValueId();
          }
        else {
          *CmpCommon::diags() << DgSqlCode(-11154) << DgInt0(ordinal) << DgString0("ORDER BY")
                              << DgInt1(vidList.entries());
          bindWA->setErrStatus();
          return NULL;
        }
    ...
    TableMappingUDFChildInfo *cInfo = new (heap) TableMappingUDFChildInfo(
        childName, childColumns, myChild->getPartReqType(), childPartition, childOrder, vidList);
  ...
  status = dllInteraction_->createOutputInputColumnMap(this, udfOutputToChildInputMap_);
  if (!status) {
    bindWA->setErrStatus();
    return NULL;
  }
...
RelExpr *ControlRunningQuery::bindNode(BindWA *bindWA) {
  ...
  //
  if (!isUserAuthorized(bindWA)) return NULL;
...
bool ControlRunningQuery::isUserAuthorized(BindWA *bindWA) {
  ...
  if ((action_ == Cancel) && (qs_ == ControlQid)) {
    ...
    if (retcode == 0) {
      ...
      if (username[0] == 'U') {
        long userID = str_atoi(&username[1], usernameLen - 1);
        if (sessionID == userID || sessionID == ComUser::getRootUserID()) userHasPriv = true;
      }
...
RelExpr *CommonSubExprRef::bindNode(BindWA *bindWA) {
  ...
  // list of the CTE.
  CMPASSERT(child(0)->getOperatorType() == REL_RENAME_TABLE && child(0)->child(0)->getOperatorType() == REL_ROOT);
...
RelExpr *OSIMControl::bindNode(BindWA *bindWA) {
  ...
  if (!CURRCONTEXT_OPTSIMULATOR) CURRCONTEXT_OPTSIMULATOR = new (CTXTHEAP) OptimizerSimulator(CTXTHEAP);
...
RelExpr *RtStatsControl::bindNode(BindWA *bindWA) {
  if (nodeIsBound()) {
    bindWA->getCurrentScope()->setRETDesc(getRETDesc());
    return this;
  }
  ...
  NAString data;
  if (!performControl(data)) {
    *CmpCommon::diags() << DgSqlCode(-1237) << DgString0(data.data());
    bindWA->setErrStatus();
    return this;
  }
