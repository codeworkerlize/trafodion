  ...
  for (CollIndex i = 0; i < exprList.entries(); i++, j++) 
  {
    ...
    switch (itemExpr->getOperatorType()) 
    {
      case ITM_REFERENCE: {
        ...
        if ((! colRef->getColRefNameObj().isStar()) &&
            (colRef->isCompCol()) &&
            ...
            (colRef->getCompArrIndexes()[colRef->getCompArrIndexes().entries()-1] > 0))
          {
            ...
            char tmpStr[100];
            Int32 indexVal = 
              colRef->getCompArrIndexes()[colRef->getCompArrIndexes().entries()-1];
            ...
            newName += "_";
            newName += tmpStr;
            newCRN = new(bindWA->wHeap()) 
              ColRefName(newName, colRef->getColRefNameObj().getCorrNameObj());
          }
                                    ...
                                    ValueIdList &compExpr)
{
  ...
  while (i--) {
    ...
    if (col->isRenameCol())
      col->setOrigValueId(col->getValueId());
...
void castComputedColumnsToAnsiTypes2(BindWA *bindWA,
                                    RETDesc *rd,
                                    ValueIdList &compExpr)
{
  if (! rd)
  ...
  while (i--) {
    ...
    if ((naType->isComposite()) &&
        ...
        (NOT bindWA->inViewDefinition()))
      {
        ...
        compDisp = new (bindWA->wHeap()) 
          CompositeDisplay(col->getValueId().getItemExpr());
  ...
  if (corrName.getSpecialType() == ExtendedQualName::VIRTUAL_TABLE)
    {
      ...
      else if (corrName.getQualifiedNameObj().getObjectName() == ExeUtilParquetStats::getVirtualTableNameStr())
        {
          ExeUtilParquetStats eudss;
          desc = eudss.createVirtualTableDesc();
        }
  ...
  if ((CmpCommon::context()->sqlSession()->volatileSchemaInUse()) &&
      ...
      (corrName.getSpecialType() != ExtendedQualName::HBMAP_TABLE))
    {
      ...
      table = bindWA->getSchemaDB()->getNATableDB()->
        get(newCorrName, bindWA, inTableDescStruct, writeReference);
      if (!table)
        {
  ...
  if (! table)
    {
     ...
     // try PUBLIC SCHEMA only when no schema was specified
      ...
      if (bindWA->errStatus())
        goto error_return;      // prototype value parse error
      ...
      table = bindWA->getSchemaDB()->getNATableDB()->
                                     get(corrName, bindWA, inTableDescStruct, writeReference);
      ...
      if ( !table && !pubSchemaIntName.isNull() )
      {
        ...
        bindWA->resetErrStatus();
        table = bindWA->getSchemaDB()->getNATableDB()->
                                       get(pCorrName, bindWA, inTableDescStruct, writeReference);
  ...
  if ((table) && table->isTrafExternalTable() &&
      (NOT table->getTableName().isHbaseMappedName()))
    {
      ...
      NATable *nativeNATable = bindWA->getSchemaDB()->getNATableDB()->
                                  get(externalCorrName, bindWA, inTableDescStruct, writeReference);
...
NATable *BindWA::getNATableInternal(
     ...
     NABoolean intQryFromExeutil)
{
  ULng32 savedParserFlags = Get_SqlParser_Flags (0xFFFFFFFF);
  Set_SqlParser_Flags(ALLOW_VOLATILE_SCHEMA_IN_TABLE_NAME);

  if (intQryFromExeutil)
                          ...
                          NABoolean catmanCollectUsages)
{
  ...
  if ((viewName.isHive()) &&
      (CmpCommon::getDefault(HIVE_ARRAY_INDEX_MODE) == DF_OFF))
    {
      useHiveIndexingWasSet = TRUE;

      NAString op("ON");
      ActiveSchemaDB()->getDefaults().validateAndInsert
        ("HIVE_ARRAY_INDEX_MODE", op, FALSE);
    }
...
static ItemExpr * findUndesireables(ItemExpr * ie,
                                    CollHeap *outHeap,
                                    void * context)
{
...
short BindWA::validateUpdateStatisticsExpression(RelExpr * node,
  const NAType ** expressionType /* out */, NAString & unparsedExpr /* out */)
{
      ...
      for (Int32 i = 0; i < vidList.entries(); i++)
        {
          ...
          ColRefName *leftColRefName = new(bindWA->wHeap()) 
            ColRefName(colName, leftCorrName);
          ColReference *leftColRef = new(bindWA->wHeap()) 
            ColReference(leftColRefName);

          CorrName rightCorrName(comp->getVirtTableNameStr(), bindWA->wHeap());
          ColRefName *rightColRefName = new(bindWA->wHeap()) 
            ColRefName(colName, rightCorrName);
          ColReference *rightColRef = new (bindWA->wHeap())
            ColReference(rightColRefName);

          ItemExpr *pred = new (bindWA->wHeap()) 
            BiRelat(ITM_EQUAL, leftColRef, rightColRef);
...
ItemExpr * Join::buildDefaultOrderByForRownum(BindWA * bindWA)
{
    ...
    if (subrel->getOperatorType() == REL_SCAN)
    {
        ...
        if (!bindWA->errStatus())
        {
            result1 = BinderUtils::buildSyskeyOrPKExpr(tab,
                                                            corrName,
                                                            bindWA->wHeap());
        }
    }
    ...
    if (subrel->getOperatorType() == REL_SCAN)
    {
        ...
        if (!bindWA->errStatus())
        {
            result2 = BinderUtils::buildSyskeyOrPKExpr(tab,
                                                            corrName,
                                                            bindWA->wHeap());
        }
    }
    ...
    if (result1 != NULL)
        resultList.insertTree(result1, ITM_ITEM_LIST, FALSE, FALSE);
    if (result2 != NULL)
        resultList.insertTree(result2, ITM_ITEM_LIST, FALSE, FALSE);
  ...
  if  ( !getUnionForIF() ) {
    ...
    if (getUnionSystemCol() && leftTable.getSystemColumnList() && rightTable.getSystemColumnList())
    {
      for (CollIndex i = 0; i < leftTable.getSysColDegree(); i++)
      {
        ValueIdUnion *vidUnion = new (bindWA->wHeap())
        ValueIdUnion(
          leftTable.getSysColValueId(i),
          rightTable.getSysColValueId(i),
          NULL_VALUE_ID,
          getUnionFlags()
        );
        vidUnion->setIsTrueUnion(TRUE);
  ...
  for (CollIndex i = 0; (i < (CollIndex) origGrbyList.entries());i++)
    {
      ...
      if (currGroupByItemExpr->getOperatorType() == ITM_USER
	   || currGroupByItemExpr->getOperatorType() == ITM_AUTHNAME)
        {
          *CmpCommon::diags() << DgSqlCode(-4197)
                              << DgString0("GROUP BY");
          bindWA->setErrStatus();
      ...
      if (currGroupByItemExpr->getOperatorType() == ITM_SEL_INDEX)
        {
          ...
          if (selectListEntry->getOperatorType() == ITM_USER
               ||selectListEntry->getOperatorType() == ITM_AUTHNAME)
            {
              *CmpCommon::diags() << DgSqlCode(-4197)
                                  << DgString0("GROUP BY");
              bindWA->setErrStatus();
              ...
              if (selectListEntry->getOperatorType() == ITM_RENAME_COL)
                {
                  ...
                  if (sListEntry && (sListEntry->getOperatorType() == ITM_USER
                                      || sListEntry->getOperatorType() == ITM_AUTHNAME))
                    {
                      *CmpCommon::diags() << DgSqlCode(-4197)
                                          << DgString0("GROUP BY");
                      bindWA->setErrStatus();
                  ...
                  if (sListEntry && sListEntry->isAnAggregate())
                    sListEntry->setIsGroupByExpr(TRUE);
                }
...
RelRoot * RelRoot::transformGroupByWithOrdinalPhase2(BindWA *bindWA)
{
  ...
  for (ValueId vid = finalGroupExpr.init();
       finalGroupExpr.next(vid);
       finalGroupExpr.advance(vid))
  {
    if (vid.getItemExpr()->getOperatorType() == ITM_USER
         || vid.getItemExpr()->getOperatorType() == ITM_AUTHNAME)
      {
        *CmpCommon::diags() << DgSqlCode(-4197)
                            << DgString0("implicit GROUP BY");
        bindWA->setErrStatus();
        return NULL;
      }
  }
...
ItemExpr * RelRoot::buildDefaultOrderByForRownum(BindWA * bindWA,
                                                                  const CorrName * renameTab = NULL)
{
    ...
    else
    {
        if (child(0)->getOperatorType() == REL_SCAN)
        {
            ...
            if (!bindWA->errStatus())
            {
                result = BinderUtils::buildSyskeyOrPKExpr(tab,
                                                                corrName,
                                                                bindWA->wHeap());
            }
        }
        else if (child(0)->getOperatorType() == REL_RENAME_TABLE && NULL == renameTab)
        {
            if (child(0)->castToRelExpr()->child(0)->getOperatorType() == REL_ROOT)
            {
                ...
                {
                    rownumFoundInSubRoot = subroot->detectRownumReference(subroot->getCompExprTree(),
                                                                       parentItmList, parentArityList,
                                                                       NULL, -1);
                }
                if (FALSE == rownumFoundInSubRoot && NULL != subroot->child(0)->selPredTree())
                {
                    rownumFoundInSubRoot = subroot->detectRownumReference(subroot->child(0)->selPredTree(),
                                                                       parentItmList, parentArityList,
                                                                       NULL, -1);
                }
    ...
    if (NULL != renameTab && NULL != result)
    {
        ...
        if (NULL != getCompExprTree())
            selList.insertTree(getCompExprTree(), ITM_ITEM_LIST, FALSE, FALSE);
        ...
        else if (REL_GROUPBY == getChild(0)->getOperatorType() &&
                    REL_ROOT == getChild(0)->getChild(0)->getOperatorType())
            isDistinct = TRUE;
        ...
        if (NOT isDistinct)
        {
            for (Lng32 i = 0; i < resultList.entries(); i++)
            {
                ...
                if (NOT isFound)
                {
                    ...
                    newSel = new (bindWA->wHeap()) RenameCol (col,
                                            new(bindWA->wHeap()) ColRefName(tmpCol, bindWA->wHeap()));
                    newSel->setTargetColumnClass(SYSTEM_COLUMN);

                    selAppend.insertTree(newSel, ITM_ITEM_LIST, FALSE, FALSE);

                    resultList[i] = new (bindWA->wHeap()) ColReference(
                                               new(bindWA->wHeap()) ColRefName(tmpCol, *renameTab, bindWA->wHeap()));
                }
                else if (NOT child(0)->getOperator().match(REL_ANY_JOIN))
                {
                    resultList[i] = new (bindWA->wHeap()) ColReference(
                                               new(bindWA->wHeap()) ColRefName(colExposedName,
                                                                                            *renameTab, bindWA->wHeap()));
                }
            }
...
NABoolean RelRoot::detectRownumReference(ItemExpr * itm,
                                              ...
                                              ItemExpr * parent, Int32 arity)
{
    ...
    for (int idx = 0; idx < itm->getArity(); idx++)
    {
        if (NULL != itm->getChild(idx))
            result = detectRownumReference(itm->getChild(idx)->castToItemExpr(),
                                               parentItmList,
                                               parentArityList,
                                               itm,
                                               idx) ? TRUE : result;
...
void RelRoot::processRownum(BindWA * bindWA)
{
  if (CmpCommon::getDefault(MODE_COMPATIBLE_1) == DF_ON)
  {
    ...
    if (child(0)->getOperatorType() == REL_SCAN ||
         ...
         child(0)->getOperatorType() == REL_GROUPBY ||
         child(0)->getOperator().match(REL_ANY_JOIN))
    {
        ...
        if (NULL != getCompExprTree())
        {
            ...
            for (Lng32 i = 0; i < selList.entries(); i++)
            {
                ...
                hasRownum = detectRownumReference(expr,
                                                    parentItmList, parentArityList,
                                                    NULL, -1);
        ...
        if (NULL != wherePred)
        {
            ...
            for (CollIndex i = 0; i < (CollIndex)origPredList.entries(); i++)
            {
                ...
                else if (TRUE == rownumInLeft && FALSE == rownumInRight &&
                           child0->getOperator() == ITM_ROWNUM)
                {
                    posFlag = 1;
                    rowLimitExpr = child1;
                }
                else if (FALSE == rownumInLeft && TRUE == rownumInRight &&
                           child1->getOperator() == ITM_ROWNUM)
                {
                    posFlag = -1;
                    rowLimitExpr = child0;
                }
                ...
                if (parentItmList.entries() > 0)
                {
                    ColRefName * refName = new (bindWA->wHeap()) ColRefName (rowname->getColName(),
                                                                                           newCorrName,
                                                                                           bindWA->wHeap());
            ...
            if (rownumPred.entries() > 0)
            {
                ...
                if (NULL != eCommon.getPtr())
                    whereRel->addSelPredTree(eCommon.getPtr());
            }
        }

        // no rownum found both in select list and where clause
        if (rownumPred.entries() == 0 && isRownumFoundInSelList == FALSE)
            return;
        ...
        {
            if (0 < limitn)
                sequenceNode->setChild(0, new (bindWA->wHeap()) FirstN(sequenceNode->child(0), limitn, TRUE));
            ...
            this->addCompExprTree(new (bindWA->wHeap()) ColReference(new (bindWA->wHeap()) 
                                                                                  ColRefName(TRUE, bindWA->wHeap())));
...
RelExpr *RelRoot::bindNode(BindWA *bindWA)
{
  ...
  if (isTrueRoot())
      ...
      if (getCompExprTree())
	{
      ...
      if (child(0) &&
          (child(0)->getOperator().match(REL_ANY_GEN_UPDATE) ||
           child(0)->getOperator().match(REL_FORCE_ANY_SCAN) ||
           child(0)->getOperatorType() == REL_CALLSP))
        {
          if (((CmpCommon::getDefault(TRAF_SAVEPOINTS) != DF_OFF) &&
              ...
              ((CmpCommon::transMode()->savepointInProgress()) ||
               (CmpCommon::transMode()->getAutoCommitSavepointOn())))
            {
  ...
  if (NOT compExprTree) {
    ...
    if(isOlapMultiWindowTopRoot())  {
        ...
        {
            if(getRETDesc()->getColRefNameObj(i).isOLAPInternal())
                ((ColRefName &)getRETDesc()->getColRefNameObj(i)) = crn;
        }
    }
  ...
  if (inputVarTree) {
    ...
    if (isTrueRoot() && child(0) && 
        (child(0)->getOperatorType() == REL_EXE_UTIL) &&
        (((ExeUtilExpr*)child(0)->castToRelExpr())->getExeUtilType() == ExeUtilExpr::HBASE_LOAD_))
      {
        ...
        for (CollIndex i = 0; i < inputVars().entries(); i++ ) {
          ...
          if (vid.getItemExpr()->getOperatorType() == ITM_DYN_PARAM)
            hbl->inputParams().insert(vid);
        }
    ...
    if (isTrueRoot() && child(0) && 
      ...
      *CmpCommon::diags() << DgSqlCode(-3242) << DgString0("This statement cannot be performed by prepared with parameters");
  ...
  if (orderByTree) {
      ...
      if (source.isGrouped()) {
        ...
        for(i = 0; i < select.getDegree(); i++) {
          ...
          for(CollIndex k = 0; k < usrColList.entries(); k++){
            if(vid ==  usrColList[k]->getValueId() ||
               (origVid == usrColList[k]->getValueId() &&
                origVid != NULL_VALUE_ID)){
        ...
        for (i = 0; i < source.getDegree(); i++)
          orderBy.addColumnToPotenialList(source.getColRefNameObj(i));
      }
      else {
      ...
      if(orderByTree->isOrderSyblingsBy()) 
      {
        ...
        else
        {
          *CmpCommon::diags() << DgSqlCode(-4466)
                        << DgString0("Order Siblings By must used with Connect By operator");
          bindWA->setErrStatus();
          return this;
        }
        ((ConnectBy*)context->connectBy())->getOrderSiblingsItem()->
  ...
  if ((isTrueRoot()) && (bindWA->hasCompositeExpr()))
...
NABoolean RelRoot::checkPrivileges(BindWA* bindWA)
{
  ...
  NABoolean allowRoot = (qiPath.length() == 0) && 
                        !(usingSentry && bindWA->getStoiList().entries() > 0);
  if (allowRoot && ComUser::isRootUserID())
    return TRUE;
                                          ...
                                          )
{
   ...
   ComQIActionType schemaActionType =
                   dummyKey.convertBitmapToQIActionType ( which, ComSecurityKey::OBJECT_IS_SCHEMA );
                               ...
                               NABoolean noSecurityCheck)
{
  ...
  switch (re->getOperatorType())
  {
    ...
    case REL_LEAF_INSERT:
      ...
      if (((Insert*)re)->getOverwriteHiveTable())
...
RelExpr *Scan::bindNode(BindWA *bindWA)
{
  ...
  if (getTableName().isPartitionV2() && getTableName().getSpecialType() == ExtendedQualName::INDEX_TABLE)
  {
    ...
    cliRC = CmpSeabaseDDL::getPartIndexName(&cliInterface,
      bindWA->getDefaultSchema().getSchemaName(),
      getTableName().getQualifiedNameObj().getObjectName(),
      getTableName().getPartnClause().getPartitionName(),
      partitionIndexName);
  ...
  {
    *CmpCommon::diags() << DgSqlCode(-3242) << DgString0("CAN NOT SELECT FROM A VIRTUAL TABLE WITHOUT A PHYSICAL TABLE");
    bindWA->setErrStatus();
    return this;
  }
  ...
  {
    if (naTable->getClusteringIndex() &&
        (naTable->getClusteringIndex()->numHivePartCols() > 0))
    {
...
//    restore ALLOW_SPECIALTABLETYPE setting
  ...
  if ( naTable->isPartitionV2Table() )
  {
    ...
    if (selPred)
      tmpPred = selPred->copyTree(bindWA->wHeap());
    ...
    if ( hasPartMatched )
    {
      Scan *leftScan = new(bindWA->wHeap()) Scan(
        CorrName((*partNames)[0], 
                 bindWA->wHeap(), 
                 tableDesc->getNATable()->getTableName().getSchemaName(), 
      ...
      {
        Scan *rightScan = new(bindWA->wHeap()) Scan(
          CorrName((*partNames)[i], 
                   bindWA->wHeap(), 
                   tableDesc->getNATable()->getTableName().getSchemaName(), 
                   tableDesc->getNATable()->getTableName().getCatalogName()));
        ...
        if (dynamic_cast<Union*>(newNode))
          dynamic_cast<Union*>(newNode)->setUnionSystemCol();
      }
  ...
  if (prevScope)
    inSubquery = prevScope->context()->inSubquery();
  if (naTable->isHbaseTable() && 
      naTable->hasTrafReplicaColumn(&replicaColPos) && 
      !(bindWA->inGenericUpdate() && !inSubquery) &&
      !currScope->context()->inUpsertXform() &&
      !bindWA->isIndexInsertStatement() && 
      naTable->getSpecialType() != ExtendedQualName::INDEX_TABLE)
  {
    ...
    ItemExpr *replicaPred = new(bindWA->wHeap()) BiRelat
      (ITM_EQUAL, repCol,
       new(bindWA->wHeap()) 
       ConstValue((GetCliGlobals()->myCpu() % naTable->getNumTrafReplicas()),
                  bindWA->wHeap())); 
    ItemExpr *newSelPred = removeSelPredTree();
    if (newSelPred)
  ...
  if (getOptHbaseAccessOptions())
    {
      ...
      if ((getOptHbaseAccessOptions()->getNumVersions() != 0) &&
          (naTable && naTable->isSQLMXAlignedTable()))
        {
          *CmpCommon::diags() << DgSqlCode(-3242)
  ...
  if (getTableDesc()->getNATable()->hasCompositeColumns())
...
RelExpr *Insert::bindNode(BindWA *bindWA)
{
  ...
  if (tmpNaTable->isPartitionV2Table() || tmpNaTable->isPartitionEntityTable())
  {
    ...
    if (tmpInsertColTree)
      tmpInsertColTree = tmpInsertColTree->copyTree(bindWA->wHeap());
    tmpOrderByTree = getOrderByTree();
    if (tmpOrderByTree)
      tmpOrderByTree = tmpOrderByTree->copyTree(bindWA->wHeap());
  }
  ...
  if ((child(0)->getOperatorType() == REL_TUPLE_LIST) &&
      (getTableDesc()->getNATable()->hasLobColumn()))
    {
      ...
      RelExpr *re = new(bindWA->wHeap()) RelRoot(child(0));
      
      Int64 sqAddr = (Int64)this;
      char sqAddrStr[100];
      str_itoa(sqAddr, sqAddrStr);
      NAString corrName("_X_");
      corrName += sqAddrStr;
      corrName += "_X_";
      re = new (bindWA->wHeap()) RenameTable(re, corrName);

      re = new(bindWA->wHeap()) RelRoot(re);
  ...
  if (withNoConflictCheck())
    {
      if ((NOT isUpsert()) ||
          (getInsertType() == UPSERT_LOAD))
        {
          *CmpCommon::diags() << DgSqlCode(-3252)
                              << DgString0("Can only be used with UPSERT.");
          bindWA->setErrStatus();
          return this;
        }
      ...
      if (getTableDesc()->getNATable()->isHiveTable())
        {
          *CmpCommon::diags() << DgSqlCode(-3252)
                              << DgString0("Cannot be used with hive tables.");
          bindWA->setErrStatus();
          return this;
        }
      ...
      if (getTableDesc()->getNATable()->hasLobColumn())
        {
          *CmpCommon::diags() << DgSqlCode(-3252)
                              << DgString0("Cannot be used with LOB columns.");
          bindWA->setErrStatus();
          return this;
        }
      ...
      if (getTableDesc()->hasSecondaryIndexes())
        {
          *CmpCommon::diags() << DgSqlCode(-3252)
                              << DgString0("Cannot be used with secondary indexes.");
          bindWA->setErrStatus();
          return this;
        }
      ...
      if (NOT ((val) && (*val == "ON")))
        {
          if (NAExecTrans(NULL))
            {
              *CmpCommon::diags() << DgSqlCode(-3252)
                                  << DgString0("Cannot be used with a user transaction.");
              bindWA->setErrStatus();
              return this;
            }
          ...
          if (CmpCommon::transMode()->getAutoCommit() != TransMode::ON_)
            {
              *CmpCommon::diags() << DgSqlCode(-3252)
                                  << DgString0("Cannot be used if AUTOCOMMIT is OFF.");
              bindWA->setErrStatus();
              return this;
            }
        }
  ...
  if (getTableDesc()->getNATable()->isHiveTable())
  {
    ...
    if (insertColTree_) {
      *CmpCommon::diags() << DgSqlCode(-4223)
                          << DgString0("Target column list specification for insert/upsert into a Hive table is");
      bindWA->setErrStatus();
      return this;
    }
    ...
    if (getTableDesc()->getNATable()->hasCompositeColumns())
      {
        // insert into hive tables with composite columns is not yet supported.
        *CmpCommon::diags() << DgSqlCode(-3242)
                            << DgString0("Insert into hive tables with composite columns is not allowed."); 
        bindWA->setErrStatus();
        return this;
      }
  ...
  if (naTable->isPartitionEntityTable() &&
      CmpCommon::getDefault(TRAF_UPSERT_THROUGH_PARTITIONS) == DF_OFF &&
      !isSpecifyPartition())
  {
    ...
    if ( subScan && 
         (subScan->getOperatorType() == REL_SCAN ||
          ...
          if ( partColPos == tgtColPos )
          {
            if ( partitionList[0]->getlowBoundary() )
            {
              if ( !lowColExpr )
                ...
                lowColExpr = new (bindWA->wHeap()) 
                                ItemList(lowColExpr, selColList[j]);
            }

            if ( partitionList[0]->getHighBoundary()&& !isLastPart() &&
                 maxvalIdx != 0 )
            {
              if ( !highColExpr )
                highColExpr = selColList[j];
              else if ( maxvalIdx == -1 || i < maxvalIdx )
                highColExpr = new (bindWA->wHeap()) 
                                ItemList(highColExpr, selColList[j]);
            }
            break;
          }
        }
      ...
      if ( lowColExpr )
        lowColExpr = new (bindWA->wHeap()) 
                        BiRelat(ITM_GREATER_EQ, 
                                lowColExpr, 
                                partitionList[0]->getlowBoundary());
      if ( highColExpr )
        highColExpr = new (bindWA->wHeap()) 
                        BiRelat(ITM_LESS, 
                                highColExpr, 
                                partitionList[0]->getHighBoundary());
  ...
  if ( naTable->isPartitionV2Table() )
  {
    ...
    if (child(0) && child(0)->getOperatorType() == REL_TUPLE_LIST 
      && child(0)->nodeIsNormalizedForCache())
...
*/
    ...
    if (tmpOrderByTree)
      newOrderByTree = tmpOrderByTree->copyTree(bindWA->wHeap());
    if (tmpSubQuery)
      newSubQuery = tmpSubQuery->copyTree(bindWA->wHeap());
    if (tmpInsertColTree)
      newInsertColTree = tmpInsertColTree->copyTree(bindWA->wHeap());
    Insert *leftInsert = new(bindWA->wHeap()) Insert(
      CorrName((*partNames)[0], 
                 bindWA->wHeap(), 
                 tableDesc->getNATable()->getTableName().getSchemaName(), 
                 tableDesc->getNATable()->getTableName().getCatalogName()
    ...
    if (NOT isPartnNameSpecified() && NOT isPartnValuesSpecified())
        leftInsert->setCheckCnstrntOfPartv2BaseTable
                        (getTableDesc()->genPartv2BaseCheckConstraint(bindWA));
    leftInsert->setIsUpsert(isUpsert());

    if (isSpecifyPartition)
      leftInsert->isSpecifyPartition() = TRUE;
    ...
    for (int i = 1; i < (*partNames).entries(); i++)
    {
      ...
      if (tmpInsertColTree)
        newInsertColTree = tmpInsertColTree->copyTree(bindWA->wHeap());
      Insert *rightInsert = new(bindWA->wHeap()) Insert(
        CorrName((*partNames)[i],
                 bindWA->wHeap(), 
                 tableDesc->getNATable()->getTableName().getSchemaName(), 
                 tableDesc->getNATable()->getTableName().getCatalogName()
        ...
        newOrderByTree);
      if (NOT isPartnNameSpecified() && NOT isPartnValuesSpecified())
          rightInsert->setCheckCnstrntOfPartv2BaseTable
                           (getTableDesc()->genPartv2BaseCheckConstraint(bindWA));
      rightInsert->setIsUpsert(isUpsert());

      if (i == (*partNames).entries() - 1)
        rightInsert->isLastPart() = TRUE;
  ...
  if (someNonDefaultValuesSpecified) 
    // query-expr child specified
    {
      if ((sourceTable.getDegree() != newTgtColList.entries()) && 
          !bulkLoadIndex && !getTableDesc()->getNATable()->hasTrafReplicaColumn()) {
      // 4023 degree of row value constructor must equal that of target table
      *CmpCommon::diags() << DgSqlCode(-4023)
    ...
    for (i = 0; i < tgtColList.entries() && i2 < newTgtColList.entries(); i++) {
                  ...
                  targetType.getTypeQualifier() == NA_CHARACTER_TYPE)
      {
        ...
        {
          ItemExpr *cast = new (bindWA->wHeap())
                               Cast(source.getItemExpr(), &targetType, ITM_CAST, TRUE);
          cast = cast->bindNode(bindWA);
          if (bindWA->errStatus())
            return NULL;
          ...
          if (nacol->isComputedColumn())
            {
          ...
          else  if (nacol->isReplicaColumn())
              {
                ...
                ItemExpr *replicaVal = new(bindWA->wHeap()) BiArith
                  (ITM_MINUS, sourceTable.findColumn(rep)->getValueId().getItemExpr(),
                   new(bindWA->wHeap()) SystemLiteral(1));
 ...
 if ((getOperatorType() == REL_UNARY_INSERT) &&
  ...
  if (getIsTrafLoadPrep())
  {
	...
	*CmpCommon::diags() << DgSqlCode(-4494)
    ...
    if (naTable->getNumTrafReplicas() > 1)		
      {		
        *CmpCommon::diags() << DgSqlCode(-4500) 		
                            << DgTableName(naTable->getTableName().
					   getQualifiedNameAsAnsiString());	    		
        bindWA->setErrStatus();		
        return boundExpr;		
  ...
  if (getInsertType() == Insert::UPSERT_NO_DUP)
    setInsertType(Insert::SIMPLE_INSERT); // this will change it as 
                                                  ...
                                                  NABoolean &toMerge) const
{
  ...
  if (isUpsert() && 
      NOT ( getIsTrafLoadPrep() || transformedUpdateNoDtmXn() ||
	    (getInsertType() == Insert::UPSERT_NO_DUP) ||
...
RelExpr* Insert::xformUpsertToEfficientTree(BindWA *bindWA) 
{
  ...
  if (noRepeatableRead())
    targetTableScan->setSkipTransactionForBatchGetForced(TRUE);
  ...
  for (CollIndex i = 0; i < tableCols.entries(); i++)
    {
      baseCol = (BaseColumn *)(tableCols[i].getItemExpr()) ;
      if (baseCol->getNAColumn()->isSystemColumn() && 
           !baseCol->getNAColumn()->isReplicaColumn())
	continue;
...
RelExpr* Insert::xformUpsertToMerge(BindWA *bindWA) 
{
  ...
  for (CollIndex i = 0; i<tableCols.entries(); i++)
  {
    ...
    if (!sourceVals[i].getItemExpr()->isCovered(currOuterRefs, emptyGA,
						newOuterRefs, dummy1, dummy2))
      {
...
RelExpr* Update::xformPartitionPruning(BindWA* bindWA, RelExpr *gu, OperatorTypeEnum op)
{
    ...
    if (isPartnNameSpecified() || isPartnValuesSpecified()) {
        ...
        RelExpr *updExpr = new(bindWA->wHeap()) Update(
          CorrName((*partNames)[0], 
                   bindWA->wHeap(), 
                   tableDesc->getNATable()->getTableName().getSchemaName(), 
                   tableDesc->getNATable()->getTableName().getCatalogName()
    ...
    if (child(0)->getGroupAttr()->isEmbeddedDelete()) {
        *CmpCommon::diags() << DgSqlCode(-4172)
    ...
    if (child(0)->getOperatorType() != REL_UNION) {
        ...
        const CorrName& oriCN = scanExpr->getTableName();
         RelExpr* updExpr = new (bindWA->wHeap())Update(CorrName(oriCN, bindWA->wHeap()),
         	NULL, REL_UNARY_UPDATE, scanExpr, updRecExpr);
         ((Update *)updExpr)->setCheckCnstrntOfPartv2BaseTable
             (getTableDesc()->genPartv2BaseCheckConstraint(bindWA));
...
RelExpr *MergeUpdate::bindNode(BindWA *bindWA)
{
  ...
  if (isMerge() && child(0) && !xformedUpsert())
  {
    ...
    if (selPred || where_ || andpred_ || precond)
    {
	  ...
	  (where_ && ((whrHasSubqry=where_->containsSubquery()) ||
                      (whrHasAggr = where_->containsAnAggregate()))) ||
      (andpred_ && ((andHasSubqry = andpred_->containsSubquery()) || 
                     (andHasAggr = andpred_->containsAnAggregate()))) ||
	...
	*CmpCommon::diags() << DgSqlCode(-3241) 
	  ...
	  (ONhasSubquery ? "Subquery in ON clause not allowed." :
           ...
           (andHasSubqry ? "subquery in WHEN MATCHED AND is not allowed." :
           (andHasAggr ? "aggregate function in WHEN MATCHED AND is not allowed." :
           (whrHasSubqry ? "subquery in UPDATE ... WHERE clause not allowed." :
           (whrHasAggr ? "aggregate function in UPDATE ... WHERE clause not allowed." :
            (precondHasSubq ?
             "subquery in WHEN NOT MATCHED AND clause not allowed." :
             "aggregate function in WHEN NOT MATCHED AND clause not allowed.")))))));
	bindWA->setErrStatus();
	return this;
      }
	...
	*CmpCommon::diags() << DgSqlCode(-4471)
	...
	return this;
      ...
      {
        *CmpCommon::diags() << DgSqlCode(-4471) << DgString0
        (((UDFunction *)(andpredHasUDF ? andpredHasUDF : precondHasUDF))->
                         getFunctionName().getExternalName());
        bindWA->setErrStatus();
        return this;
      }
  ...
  {		
    *CmpCommon::diags() << DgSqlCode(-3241) 		
			<< DgString0("SQL replicated tables not suuported.");	    		
    bindWA->setErrStatus();		
    return NULL;		
  }
  ...
  if (naTable->isHbaseMapTable() &&
    ...
    {
      *CmpCommon::diags() << DgSqlCode(-4223)
			  << DgString0("Insert/Update/Delete/Merge on HBase mapped tables is");
...
static NABoolean lcl_isINListConverts2OR(const ItemExpr* ie)
{
  if (ie && ie->getOperatorType() == ITM_OR) 
    if (((BiLogic*)ie)->createdFromINlist())
      return TRUE;
  return FALSE;
}
...
static NABoolean lcl_isINListConverts2Sub(const ItemExpr* ie)
{
  if (ie && ie->getOperatorType() == ITM_EQUAL_ANY)
    if (((QuantifiedComp*)ie)->createdFromINlist())
      return TRUE;
  return FALSE;
}
// -----------------------------------------------------------------------
...
if ((boundExpr->getOperatorType() == REL_TSJ || 
     boundExpr->getOperatorType() ==REL_ANTI_SEMITSJ)&& 
    boundExpr->child(0)->getOperatorType() == REL_UNARY_DELETE &&
    !isMergeDelete())
  {
...
RelExpr* Delete::xformPartitionPruning(BindWA* bindWA, RelExpr *gu, OperatorTypeEnum op)
{
    ...
    if (isPartnNameSpecified() || isPartnValuesSpecified()) {
        RelExpr *delExpr = new(bindWA->wHeap()) Delete(
          CorrName((*partNames)[0], 
                   bindWA->wHeap(), 
                   tableDesc->getNATable()->getTableName().getSchemaName(), 
                   tableDesc->getNATable()->getTableName().getCatalogName()
    ...
    if (child(0)->getOperatorType() != REL_UNION) {
        ...
        //rel->markAsUnBound();
        const CorrName& oriCN = scanExpr->getTableName();
         RelExpr* delExpr = new (bindWA->wHeap())Delete(CorrName(oriCN, bindWA->wHeap()),
             NULL, REL_UNARY_DELETE, scanExpr);
         RelExpr* boundDelExpr = delExpr->bindNode(bindWA);
         bindWA->setPruningPartition(FALSE);
         return boundDelExpr;
    }
...
{
  if (!deleteExpr->getOperatorType() == REL_UNARY_DELETE)
    return deleteExpr;
  if (!((Delete *)deleteExpr)->needConvertToAsj())
    return deleteExpr;
  ...
  Join * asj = new (CmpCommon::statementHeap())
    Join(newScan,
         newDelete,
         REL_ANTI_SEMITSJ);
...
static BoundaryValue* getPartBoundaryValue(const NAColumn *nacol)
{
    ...
    for (i = 0; i < boundList.entries(); i++) {
        if(colIdxArray[i] == nacol->getPosition())
            break;
    }
  ...
  if (getOperatorType() == REL_UNARY_UPDATE) {
      ...
      for (ValueId v = newRecExpr.init(); newRecExpr.next(v);
          ...
          if (nacol->isPartV2Column()) {
              ...
              if (bv->lowValue) {
                  lowItem = parser.getItemExprTree(bv->lowValue,
                      strlen(bv->lowValue), CharInfo::UTF8); // ComGetNameInterfaceCharSet()
                  lowItem = new (bindWA->wHeap())Cast(lowItem, &desiredType);
              }
              if (bv->highValue && (! bv->isMaxValue())) {
                  highItem = parser.getItemExprTree(bv->highValue,
                      strlen(bv->highValue), CharInfo::UTF8); // ComGetNameInterfaceCharSet()
...
static void getUpdateSetListColName4CacheKey(ItemExpr* pItemExprTree, NAString &str)
{
  if (!pItemExprTree)
    return;
  if (pItemExprTree->getArity() !=2)
    return;
  OperatorTypeEnum eType = pItemExprTree->getOperatorType();
  if (ITM_ITEM_LIST == eType)
    {
      if (pItemExprTree->child(0))
        getUpdateSetListColName4CacheKey(pItemExprTree->child(0),str);
      str += pItemExprTree->getText4CacheKey();
      str += " ";
      if (pItemExprTree->child(1))
        getUpdateSetListColName4CacheKey(pItemExprTree->child(1),str);
      
    }
  else if(ITM_ASSIGN == eType)
    {
      if (pItemExprTree->child(0) &&
          pItemExprTree->child(0)->getOperatorType() == ITM_REFERENCE)
        {
...
RelExpr * GenericUpdate::bindNode(BindWA *bindWA)
{
  ...
  if (((getTableName().isLocationNameSpecified() || 
      ...
      (NOT getTableName().isPartitionV2()) &&
     (!Get_SqlParser_Flags(ALLOW_SPECIALTABLETYPE))) &&
     (getOperatorType() != REL_UNARY_DELETE)) {
  ...
  if (naTable->incrBackupEnabled() &&
      (ClusterRole::get_role() == ClusterRole::SECONDARY) &&
      !Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL)) {
    *CmpCommon::diags()
        << DgSqlCode(-4481)
        << DgString0(
               "secondary cluster mutation");
    bindWA->setErrStatus();
    return this;
    }
  ...
  {
    *CmpCommon::diags() << DgSqlCode(-4223)
                        << DgString0("Insert/Update/Delete/Merge on READ_ONLY table is");
    bindWA->setErrStatus();
    return this;
  }
  ...
  if ((naTable->getSpecialType() == ExtendedQualName::INDEX_TABLE) &&
  ...
  if ((naTable->hasTrafReplicaColumn() && getOperatorType() == REL_UNARY_INSERT ) &&
      !getUpdateCKorUniqueIndexKey())
  {
    ...
    ColReference *discardColRef = new(bindWA->wHeap()) 
    ColReference(new(bindWA->wHeap()) ColRefName("__discard__", bindWA->wHeap()));
    ...
    ItemExpr *keyCol = new (bindWA->wHeap()) 
		     ColReference (new (bindWA->wHeap()) 
                                   ColRefName("__REPLICA__", bindWA->wHeap()));
    RelExpr* transpNode = new(bindWA->wHeap()) 
      Transpose(tranposeValues, keyCol, child(0), bindWA->wHeap());
    child(0) = transpNode;
  }
  ...
  if (naTable->isPartitionEntityTable() &&
      CmpCommon::getDefault(TRAF_PARTITIONV2_CONSTRAINT_RAISE_ERROR) == DF_ON)
  {
      ...
      resultCheck = new (bindWA->wHeap())
                        Case(NULL,
                             new (bindWA->wHeap())
                                 IfThenElse(
                                     resultCheck,
                                     getCheckCnstrntOfPartv2BaseTable(),
                                     trueVal));
    ...
    resultCheck = new (bindWA->wHeap())
                      Case(NULL,
                           new (bindWA->wHeap())
                               IfThenElse(
                                    resultCheck,
                                    new (bindWA->wHeap()) RaiseError(4403),
                                    constraint));
  ...
  if (getOperatorType() == REL_UNARY_UPDATE ||
    ...
    if (getTableDesc()->getNATable()->isORC())
      {
        *CmpCommon::diags() << DgSqlCode(-4223)
                            << DgString0("Update/Delete on ORC table is");
        bindWA->setErrStatus();
        return this;
      }
    ...
    if (getTableDesc()->getNATable()->isParquet())
      {
        *CmpCommon::diags() << DgSqlCode(-4223)
                            << DgString0("Update/Delete on PARQUET table is");
        bindWA->setErrStatus();
        return this;
      }
    ...
    if (getTableDesc()->getNATable()->isAvro())
      {
        *CmpCommon::diags() << DgSqlCode(-4223)
                            << DgString0("Update/Delete on AVRO table is");
        bindWA->setErrStatus();
        return this;
      }
...
RelExpr *BuiltinTableValuedFunction::bindNode(BindWA *bindWA)
{
  ...
  if (getOperatorType() == REL_EXPLAIN ||
      ...
      getOperatorType() == REL_QUERYINVALIDATION  ||
  ...
  for (Lng32 i = 0; i < cqds_->entries(); i++)
    {
      ...
      if (((getHoldOrRestoreCQD() > 0) || (reset_)) &&
          (NOT value.isNull()))
        {
          *CmpCommon::diags() << DgSqlCode(-3242)
      ...
      if (getHoldOrRestoreCQD() == 0)
        {
          attrEnum = affectYourself ? defs.validateAndInsert(token.data(), 
                                                             value, reset_)
...
RelExpr * RelTransaction::bindNode(BindWA *bindWA)
{
  ...
  if ((CmpCommon::getDefault(TRAF_SAVEPOINTS) == DF_OFF) &&
      ((type_ == BEGIN_SAVEPOINT_) ||
       (type_ == COMMIT_SAVEPOINT_) ||
       (type_ == ROLLBACK_SAVEPOINT_)))
    {
  ...
  for (i = 0; i < resultTable->getDegree(); i++)
    {
	  ...
	  (resultTable->getType(i)).getFSDatatype() == REC_CLOB ||
          (resultTable->getType(i)).isComposite())
	{
...
RelExpr *CallSP::bindNode(BindWA *bindWA)
{
  ...
  if (NULL == routine) {
    ...
    Int32 error = FALSE;
    routine = new (routineHeap) NARoutine ( getRoutineName(),
					    catRoutine,
					    bindWA,
					    error,
					    routineHeap);
...
RelExpr * FastExtract::bindNode(BindWA *bindWA)
{
  ...
  if (getTargetType() == FILE)
  {
    ...
    if (raiseError  &&
       (strncmp(getTargetName().data(),"hdfs://",7) != 0) &&
       (strncmp(getTargetName().data(),"s3://",5) != 0) &&
       (strncmp(getTargetName().data(),"s3a://",6) != 0) &&
       (strncmp(getTargetName().data(),"alluxio://",10) != 0) &&
       (strncmp(getTargetName().data(),"adl://",6) != 0))
    {
  ...
  if ((isHiveInsert()) &&
       ...
       hiveTableDesc_->getNATable()->getClusteringIndex()))
    {
      ...
      if( hiveTableDesc_->getNATable()->getClusteringIndex()->numHivePartCols() > 0)
      {
