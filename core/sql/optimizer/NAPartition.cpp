...
void NAPartition::print(FILE* ofd, const char* indent, const char* title,
                        CollHeap *c, char *buf)
{
  Space *space = (Space *)c;
  char mybuf[3000];
  ...
  snprintf(mybuf, sizeof(mybuf),
           ...
           head, partitionName_, head, partitionEntityName_,
           head, partPosition_, 
           subpartitionCnt_,
           lowValuetext.data(), highValueText.data(),
           maxValueBitSetToString());

  PRINTIT(ofd, c, space, buf, mybuf);
  if (subPartitions_)
    subPartitions_->print(ofd, indent, title, c, buf);
}
...
void NAPartition::deepDelete()
{
  ...
  if (partitionEntityName_)
    NADELETEBASIC(partitionEntityName_, heap_);

  for (int i = 0; i < boundaryValueList_.entries(); i++)
  {
    if (boundaryValueList_[i])
    {
      if (boundaryValueList_[i]->lowValue)
        NADELETEBASIC(boundaryValueList_[i]->lowValue, heap_);
      if (boundaryValueList_[i]->highValue)
        NADELETEBASIC(boundaryValueList_[i]->highValue, heap_);
...
void NAPartitionArray::print(FILE* ofd, const char* indent, const char* title,
                             CollHeap *c, char *buf)
{
