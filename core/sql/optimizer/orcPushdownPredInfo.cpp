...
void ExtPushdownPredInfoList::insertStartAND()
{
   append(ExtPushdownPredInfo(STARTAND));
}

void ExtPushdownPredInfoList::insertStartOR()
{
   append(ExtPushdownPredInfo(STARTOR));
}

void ExtPushdownPredInfoList::insertStartNOT()
{
   append(ExtPushdownPredInfo(STARTNOT));
}

void ExtPushdownPredInfoList::insertEND()
{
   append(ExtPushdownPredInfo(END));
}
...
void ExtPushdownPredInfoList::insertIS_NULL(const ValueId& col)
{
   append(ExtPushdownPredInfo(ISNULL, col));
}

void 
ExtPushdownPredInfoList::insertIN(const ValueId& col, const ValueIdList& val)
{
   append(ExtPushdownPredInfo(IN, col, val));
}

void 
ExtPushdownPredInfoList::insertIN(const ValueId& col, const ValueId& val)
{
   append(ExtPushdownPredInfo(IN, col, val));
}
...
NABoolean ExtPushdownPredInfoList::validatePushdownForParquet()
{
  ...
  for (Lng32 i = 0; 
       ((i < entries()) && (NOT cantPushdown));
       i++)
    {
      ExtPushdownPredInfo &ppi = (*this)[i];
      ExtPushdownOperatorType type = ppi.getType();
      
      if (NOT ((ppi.getType() == EQUALS) ||
               (ppi.getType() == LESSTHAN) ||
               (ppi.getType() == LESSTHANEQUALS) ||
               (ppi.getType() == ISNULL)
               ))
        continue;
      ...
      if ((colValId != NULL_VALUE_ID) &&
          (((colValId.getType().getHiveType() == HIVE_DECIMAL_TYPE) &&
            (ppi.getType() != ISNULL)) ||
          ...
          if (ppi.operValId().getItemExpr()->getOperatorType() == ITM_CONSTANT)
            {
              ...
              char convBuf[1000];
              short retCode = 
                convDoIt((char*)cv->getConstValue(),
                         cv->getStorageSize(),
                         (short)cv->getType()->getFSDatatype(),
                         cv->getType()->getPrecision(),
                         cv->getType()->getScale(),
                         convBuf,
                         colValId.getType().getNominalSize(),
                         colValId.getType().getFSDatatype(),
                         colValId.getType().getPrecision(),
                         colValId.getType().getScale(),
                         NULL, 0);
...
NABoolean 
ExtPushdownPredInfoList::validateOperatorsForParquetCppReader()
{
