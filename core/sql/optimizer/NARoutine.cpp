...
void NARoutine::getPrivileges(TrafDesc *priv_desc, BindWA *bindWA)
{
  ...
  CorrName cn(SEABASE_SCHEMA_OBJECTNAME,
                 STMTHEAP,
                 getRoutineName()->getQualifiedNameObj().getSchemaName(),
  ...
  if (priv_desc == NULL)
  {
    ...
    PrivStatus privStatus = 
      privInterface.getPrivileges(objectUID_, schemaOwner_, 
                                  schemaTable->objectUid().get_value(), 
                                  schemaTable->getObjectType(),
                                  objectType, *privDescs_);
  ...
  if (ComUser::getCurrentUserRoles(roleIDs, grantees, CmpCommon::diags()) != 0)
    return;
...
void NARoutineDB::reset_priv_entries()
{
  ...
  {
    snprintf(msg, len, "NARoutineDB::reset_priv_entries, user: %s, retcode: %d,  "
  ...
  while(key && routine)
  {
    ...
    if (routine->getPrivDescs())
    {
      ...
      bool result = privInfo->initUserPrivs(roleIDs, routine->getPrivDescs(), userID,
                                            routine->getRoutineID(), &secKeySet);         
      if (result)
      {
        routine->setPrivInfo(privInfo);
        routine->setSecKeySet(secKeySet);
        std::string privDetails = routine->privInfo_->print();
        snprintf(msg, len, "NARoutineDB::reset_priv_entries, user: %s obj %s, %s",
                ComUser::getCurrentUsername(),
                routine->getRoutineName()->getExtendedQualifiedNameAsString().data(),
                privDetails.c_str());
                                  ...
                                  NABoolean ddlXns, NABoolean atCommit)
{
  ...
  removeFromAllUsers(routineName, qiScope,
                     COM_USER_DEFINED_ROUTINE_OBJECT,
                     objectUIDs, ddlXns, atCommit);
}
...
//-----------------------------------------------------------------------
 ...
 SP_STATUS NARoutineCacheDeleteStoredProcedure::sp_InputFormat(
