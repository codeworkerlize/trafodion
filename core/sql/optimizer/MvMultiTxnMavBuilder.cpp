...
RelExpr *MvMultiTxnMavBuilder::buildReadPreviousContext() {
  ...
  MvLogForContextRows logTableObj(*deltaDef->getTableName(), getMvCorrName(), deltaDef->getBeginEpoch(), catchupNo,
                                  bindWA_);
  if (bindWA_->errStatus()) return NULL;

  if (isPhase1) {  // Build the phase 1 sub-tree
    ...
    Delete *deleteNode = new (heap_) Delete(scanNode->getTableName(), NULL, REL_UNARY_DELETE, scanNode);
    ...
    phase1Tree = buildPhase1SelectList(groupByNode, logTableObj.getSubjectNaTable());
  }

  if (catchupNo) {  // Build the catchup sub-tree.
    ...
    RelExpr *renameNode = new (heap_) RenameTable(rootNode, getEndCtxName());
...
RelExpr *MvMultiTxnMavBuilder::buildErrorOnNoContext(RelExpr *topNode) {
  ItemExpr *countStar = new (heap_)
      Aggregate(ITM_COUNT, new (heap_) SystemLiteral(1), FALSE /*i.e. not distinct*/, ITM_COUNT_STAR__ORIGINALLY, '!');

  ItemExpr *newAggExpr = new (heap_) BiRelat(ITM_NOT_EQUAL, countStar, new (heap_) SystemLiteral(0));
  ...
  ItemExpr *grbySelectionPred =
      new (heap_) Case(NULL, new (heap_) IfThenElse(newAggExpr, new (heap_) BoolVal(ITM_RETURN_TRUE), errorExpr));
...
// GroupBy node.
RelExpr *MvMultiTxnMavBuilder::buildPhase1SelectList(RelExpr *topNode, const NATable *baseNaTable) {
  CorrName emptyCorrName("");
  ItemExpr *ciCols = BinderUtils::buildClusteringIndexVector(baseNaTable, heap_, &emptyCorrName, SP_USE_AT_SYSKEY);
  ...
  for (CollIndex i = 0; i < ciColsList.entries(); i++) {
    ...
    if (currentCol->getOperatorType() == ITM_RENAME_COL)
      ...
      renameColList.insert(new (heap_) RenameCol(NULL, &currentColRef->getColRefNameObj()));
    }
  }

  // Use the list of MAX(col) aggregates as the RelRoot select list.
  RelRoot *topRoot = new (heap_) RelRoot(topNode, REL_ROOT, aggregatedCiColsList.convertToItemExpr());
  topRoot->setDontOpenNewScope();

  // Use the list of RenameCols in the RenameTable node.
  RelExpr *renameNode = new (heap_) RenameTable(topRoot, getStartCtxName(), renameColList.convertToItemExpr());
...
Union *MvMultiTxnMavBuilder::buildUnionBetweenRangeAndIudBlocks(RelExpr *scanIUDLogBlock,
  ...
  Union *unionNode = MvRefreshBuilder::buildUnionBetweenRangeAndIudBlocks(scanIUDLogBlock, scanRangeLogBlock);
...
//----------------------------------------------------------------------------
// Excluded from coverage test - used only with range logging.
NABoolean MvMultiTxnMavBuilder::needAlternateCIorder() const { return TRUE; }
...
ItemExpr *MvMultiTxnMavBuilder::buildSelectionPredicateForScanOnIudLog() const {
  ...
  if (pMultiTxnClause_->isCatchup() || pMultiTxnClause_->isPhase1()) {
    ...
    if (selectionPred) {
      selectionPred = new (heap_) BiLogic(ITM_AND, selectionPred, contextSelectionPred);
    } else {
...

{
  ...
  if (pMultiTxnClause_->isPhase1()) {
    ...
    ItemExpr *logVector = BinderUtils::buildClusteringIndexVector(
        getLogsInfo().getBaseNaTable(), heap_, &getLogsInfo().getIudLogTableName(), SP_USE_AT_SYSKEY);

    const CorrName startCtxName(getStartCtxName());

    ItemExpr *startCtxVector =
        BinderUtils::buildClusteringIndexVector(getLogsInfo().getBaseNaTable(), heap_, &startCtxName, SP_USE_AT_SYSKEY);

    BiRelat *selectionPredicate = new (heap_) BiRelat(ITM_GREATER_EQ, logVector, startCtxVector);

    selectionPredicate->setDirectionVector(getLogsInfo().getBaseTableDirectionVector());
  ...
  if (pMultiTxnClause_->isCatchup()) {
    ...
    ItemExpr *endCtxVector =
        BinderUtils::buildClusteringIndexVector(getLogsInfo().getBaseNaTable(), heap_, &endCtxName, SP_USE_AT_SYSKEY);

    BiRelat *selectionPredicate = new (heap_) BiRelat(ITM_LESS, logVector, endCtxVector);
...
RelExpr *MvMultiTxnMavBuilder::buildLogsScanningBlock(const QualifiedName &baseTable) {
  ...
  if (bindWA_->errStatus() || topNode == NULL) return NULL;
...
ItemExpr *MvMultiTxnMavBuilder::buildSequenceIsLastExpr() const {
  ...
  ItemExpr *runningCountPred = new (heap_)
      BiRelat(ITM_GREATER_EQ, new (heap_) ItmSeqRunningFunction(ITM_RUNNING_SUM, new (heap_) SystemLiteral(1)),
              new (heap_) SystemLiteral(pMultiTxnClause_->getCommitEach()));

  // Log.CI <> OFFSET(Log.CI, 1)
  ItemExpr *nonEqualCiPred = new (heap_)
      BiRelat(ITM_NOT_EQUAL, BinderUtils::buildClusteringIndexVector(baseNaTable, heap_, NULL, SP_USE_AT_SYSKEY),
              BinderUtils::buildClusteringIndexVector(baseNaTable, heap_, NULL, SP_USE_AT_SYSKEY | SP_USE_OFFSET));

  // isLast is the logical AND of these two expressions.
  ItemExpr *isLastExpr = new (heap_) BiLogic(ITM_AND, runningCountPred, nonEqualCiPred);
...
RelSequence *MvMultiTxnMavBuilder::buildSequenceOnScan(RelExpr *topNode, ItemExpr *isLastExpr) const {
  ...
  ItemExpr *orderVector = BinderUtils::buildClusteringIndexVector(
      baseNaTable, heap_, &getLogsInfo().getBaseTableName(), SP_USE_AT_SYSKEY,
      getLogsInfo().getBaseTableDirectionVector(), getSequenceByPrefixColName());

  if (getLogsInfo().getDeltaDefinition().useRangeLog() && !getLogsInfo().getDeltaDefinition().useIudLog()) {
...
//----------------------------------------------------------------------------
RelRoot *MvMultiTxnMavBuilder::buildRootOverSequence(RelExpr *topNode, ItemExpr *isLastExpr) const {
  // Give the isLast expression the name @ISLAST
  ItemExpr *isLastColumn = new (heap_) RenameCol(isLastExpr, new (heap_) ColRefName(getVirtualIsLastColName()));
...
RelExpr *MvMultiTxnMavBuilder::buildInsertContextTree(RelExpr *leftTopNode) {
  ...
  ItemExpr *ifCondition = new (heap_) ColReference(new (heap_) ColRefName(getVirtualIsLastColName()));

  // The conditional Union node.
  Union *ifNode =
      new (heap_) Union(insertNode, tupleNode, NULL, ifCondition, REL_UNION, CmpCommon::statementHeap(), TRUE);

  // Build the select list of a single column called @IF_OUTPUT_COL.
  ItemExpr *renameList = new (heap_) RenameCol(NULL, new (heap_) ColRefName(ifOutputColName));

  // The Rename node
  RelExpr *rightTopNode = new (heap_) RenameTable(ifNode, ifOutputTableName, renameList);
  rightTopNode = new (heap_) RelRoot(rightTopNode);

  Join *joinNode = new (heap_) Join(leftTopNode, rightTopNode, REL_SEMITSJ);
...
RelExpr *MvMultiTxnMavBuilder::buildInsertContextNode() {
  DeltaDefinition *deltaDef = getDeltaDefList()->at(0);
  MvLogForContextRows logTableObj(*deltaDef->getTableName(), getMvCorrName(), deltaDef->getBeginEpoch(),
                                  pMultiTxnClause_->getCatchup(), bindWA_);

  if (bindWA_->errStatus()) return NULL;

  RelExpr *insertNode = logTableObj.buildInsert(TRUE);

  CMPASSERT(insertNode->getOperatorType() == REL_UNARY_INSERT || insertNode->getOperatorType() == REL_LEAF_INSERT);
  ...
  ((GenericUpdate *)insertNode)->rowsAffected() = GenericUpdate::COMPUTE_ROWSAFFECTED;
...
ItemExpr *MvMultiTxnMavBuilder::buildPhase1SelectionPredicateForScanOnRangeLog() const {
  ...
  ItemExpr *predC01 = new (heap_) BiRelat(ITM_NOT_EQUAL, endRangeCI->copyTree(heap_), startCtx->copyTree(heap_));
  ...
  ItemExpr *predC = new (heap_)
      Case(rangeType,
           new (heap_) IfThenElse(new (heap_) SystemLiteral(ComMvRangeClosedUpperBound),  // If 2
                                  predC2,
                                  new (heap_) IfThenElse(new (heap_) SystemLiteral(ComMvRangeOpenBothBounds),  // If 3
                                                         predC3, predC01)));
...
ItemExpr *MvMultiTxnMavBuilder::buildRangeLogJoinPredicate() const {
  ...
  if (!pMultiTxnClause_->isPhase1() && !pMultiTxnClause_->isCatchup()) return predicate;

  ItemExpr *baseCI =
      BinderUtils::buildClusteringIndexVector(getLogsInfo().getBaseNaTable(), heap_, NULL, SP_USE_AT_SYSKEY);
  ...
  if (pMultiTxnClause_->isPhase1()) {
    ...
    BiRelat *predD = new (heap_) BiRelat(ITM_LESS_EQ, startCtxCI, baseCI);
  ...
  if (pMultiTxnClause_->isCatchup()) {
    ...
    if (pMultiTxnClause_->isPhase1()) baseCI = baseCI->copyTree(heap_);
    ...
    BiRelat *predE = new (heap_) BiRelat(ITM_GREATER, endCtxCI, baseCI);
...
RelSequence *MultiTxnDEMavBuilder::buildSequenceOnScan(RelExpr *topNode, ItemExpr *isLastExpr) const {
  RelSequence *sequenceNode = MvMultiTxnMavBuilder::buildSequenceOnScan(topNode, isLastExpr);
...
RelRoot *MultiTxnDEMavBuilder::buildRootOverSequence(RelExpr *topNode, ItemExpr *isLastExpr) const {
  const CorrName &tableNameCorr = getLogsInfo().getBaseTableName();
  ...
  ItemExpr *rangeBeginCI = BinderUtils::buildClusteringIndexVector(
      getLogsInfo().getBaseNaTable(), heap_, &tableNameCorr, SP_USE_AT_SYSKEY | SP_USE_LAST_NOT_NULL, NULL,
      &beginRangePrefix, getLastNotNullPrefixColName());

  // The "end range" vector of the range log.
  const NAString endRangePrefix(COMMV_ENDRANGE_PREFIX);
  ItemExpr *rangeEndCI = BinderUtils::buildClusteringIndexVector(getLogsInfo().getBaseNaTable(), heap_, &tableNameCorr,
                                                                 SP_USE_AT_SYSKEY | SP_USE_LAST_NOT_NULL, NULL,
                                                                 &endRangePrefix, getLastNotNullPrefixColName());

  ItemExpr *rangeIdCol = new (heap_) RenameCol(
      new (heap_)
          ItmSeqRunningFunction(ITM_LAST_NOT_NULL, new (heap_) ColReference(new (heap_) ColRefName(
                                                       COMMV_RANGE_ID_COL, getLogsInfo().getBaseTableName(), heap_))),
      new (heap_)
          ColRefName(*getLastNotNullPrefixColName() + COMMV_RANGE_ID_COL, getLogsInfo().getBaseTableName(), heap_));

  ItemExpr *rangeTypeCol = new (heap_) RenameCol(
      new (heap_)
          ItmSeqRunningFunction(ITM_LAST_NOT_NULL, new (heap_) ColReference(new (heap_) ColRefName(
                                                       COMMV_RANGE_TYPE_COL, getLogsInfo().getBaseTableName(), heap_))),
      new (heap_)
          ColRefName(*getLastNotNullPrefixColName() + COMMV_RANGE_TYPE_COL, getLogsInfo().getBaseTableName(), heap_));
...
RelRoot *MultiTxnDEMavBuilder::buildFinalRootOverSequence(RelExpr *topNode) const {
  // Give the isLast expression the name @ISLAST
  ItemExpr *isLastColumn = new (heap_) ColReference(new (heap_) ColRefName(getVirtualIsLastColName()));
  ...
  for (CollIndex i = 0; i < baseColumns.entries(); i++) {
    ...
    ColRefName *colRefName = new (heap_) ColRefName(renameColName, getLogsInfo().getBaseTableName(), heap_);

    ItemExpr *colRef = new (heap_) ColReference(colRefName);

    if (baseColumns[i]->isClusteringKey()) {
      colRef =
          new (heap_) RenameCol(new (heap_) ColReference(new (heap_) ColRefName(
                                    *getSequenceByPrefixColName() + colName, getLogsInfo().getBaseTableName(), heap_)),
                                colRefName);
    }
...
ItemExpr *MultiTxnDEMavBuilder::buildSelectionOverSequence() const {
  ...
  ItemExpr *isNotCoveredByRange = new (heap_) UnLogic(ITM_NOT, isCoveredByRange);

  ItemExpr *specialCol = new (heap_)
      ColReference(new (heap_) ColRefName(getVirtualRangeSpecialCol(), getLogsInfo().getBaseTableName(), heap_));

  // a)
  ItemExpr *isTableRecords = new (heap_) BiRelat(ITM_EQUAL, specialCol, new (heap_) SystemLiteral(TABLE_ROWS));

  // b)
  ItemExpr *isLastCol = new (heap_) ColReference(new (heap_) ColRefName(getVirtualIsLastColName()));

  // c)
  ItemExpr *isIudLogRecords =
      new (heap_) BiRelat(ITM_EQUAL, specialCol->copyTree(), new (heap_) SystemLiteral(IUD_LOG_ROWS));

  // d)
  ItemExpr *isNotCoveredIudRecords = new (heap_) BiLogic(ITM_AND, isIudLogRecords, isNotCoveredByRange);

  ItemExpr *selection =
      new (heap_) BiLogic(ITM_OR, isTableRecords, new (heap_) BiLogic(ITM_OR, isLastCol, isNotCoveredIudRecords));
...
ItemExpr *MultiTxnDEMavBuilder::buildIsCoveredByRange() const {
  ...
  ItemExpr *rangeIdCol = new (heap_) ColReference(new (heap_) ColRefName(
      *getLastNotNullPrefixColName() + COMMV_RANGE_ID_COL, getLogsInfo().getBaseTableName(), heap_));

  ItemExpr *logTsCol =
      new (heap_) ColReference(new (heap_) ColRefName(COMMV_TS_COL, getLogsInfo().getBaseTableName(), heap_));

  ItemExpr *isRangeHappendBefore = new (heap_) BiLogic(ITM_AND, new (heap_) UnLogic(ITM_IS_NOT_NULL, rangeIdCol),
                                                       new (heap_) BiRelat(ITM_LESS, rangeIdCol->copyTree(), logTsCol));

  ItemExpr *isPhysicallyCoveredByRangeBoundries = buildIsPhysicallyCoveredByRangeBoundries();

  ItemExpr *isCoveredByRange = new (heap_) BiLogic(ITM_AND, isRangeHappendBefore, isPhysicallyCoveredByRangeBoundries);
...
ItemExpr *MultiTxnDEMavBuilder::buildIsPhysicallyCoveredByRangeBoundries() const {
  ...
  ItemExpr *baseCI = BinderUtils::buildClusteringIndexVector(getLogsInfo().getBaseNaTable(), heap_, &tableNameCorr,
                                                             SP_USE_AT_SYSKEY, directionVector);

  // The "begin range" vector of the range log.
  ItemExpr *rangeBeginCI = BinderUtils::buildClusteringIndexVector(
      getLogsInfo().getBaseNaTable(), heap_, &tableNameCorr, SP_USE_AT_SYSKEY, directionVector, &beginRangePrefix);

  // The "end range" vector of the range log.
  ItemExpr *rangeEndCI = BinderUtils::buildClusteringIndexVector(getLogsInfo().getBaseNaTable(), heap_, &tableNameCorr,
                                                                 SP_USE_AT_SYSKEY, directionVector, &endRangePrefix);

  // The RANGE_TYPE column of the range log.
  ItemExpr *rangeType =
      new (heap_) ColReference(new (heap_) ColRefName(*getLastNotNullPrefixColName() + COMMV_RANGE_TYPE_COL));
...
RelRoot *MultiTxnDEMavBuilder::buildRootOverIUDLog(RelExpr *topNode) const {
  ...
  ItemExpr *sortKeyColumns = BinderUtils::buildClusteringIndexVector(
      getLogsInfo().getBaseNaTable(), heap_, &getLogsInfo().getIudLogTableName(), SP_USE_AT_SYSKEY, NULL, NULL,
  ...
  ItemExpr *specialCol = new (heap_)
      RenameCol(new (heap_) SystemLiteral(IUD_LOG_ROWS), new (heap_) ColRefName(getVirtualRangeSpecialCol()));

  ItemExpr *rangeTypeCol =
      new (heap_) RenameCol(new (heap_) SystemLiteral(), new (heap_) ColRefName(COMMV_RANGE_TYPE_COL));

  // the @TS column of the IUD log
  ItemExpr *tsCol = new (heap_) ColReference(new (heap_) ColRefName(COMMV_TS_COL, heap_));
...
RelRoot *MultiTxnDEMavBuilder::buildRootOverRangeBlock(RelExpr *topNode) const {
  ...
  ItemExpr *rangeTypeCol = new (heap_)
      ColReference(new (heap_) ColRefName(COMMV_RANGE_TYPE_COL, getLogsInfo().getRangeLogTableName(), heap_));

  ItemExpr *rangeIdCol =
      new (heap_) ColReference(new (heap_) ColRefName(COMMV_RANGE_ID_COL, getLogsInfo().getRangeLogTableName(), heap_));

  ItemExpr *specialCol = new (heap_) ColReference(new (heap_) ColRefName(getVirtualRangeSpecialCol()));

  // rangeId is the time of the logging of the range record
  ItemExpr *rangeSyskeyOfLogging = new (heap_) RenameCol(
      rangeIdCol->copyTree(), new (heap_) ColRefName(COMMV_RANGE_SYSKEY_COL, getLogsInfo().getRangeLogTableName()));
...
void MultiTxnDEMavBuilder::buildSortKeyColumnsForRangeBlock(ItemExprList &sortKeyCols) const {
  ...
  for (CollIndex i = 0; i < Columns.entries(); i++) {
    ...
    ItemExpr *colExpr =
        new (heap_) ColReference(new (heap_) ColRefName(colName, getLogsInfo().getBaseTableName(), heap_));

    NAString rangeBeginColName = COMMV_BEGINRANGE_PREFIX + Columns[i]->getColName();

    ItemExpr *rangeBeginColRef = new (heap_)
        ColReference(new (heap_) ColRefName(rangeBeginColName, getLogsInfo().getRangeLogTableName(), heap_));

    // Generate the computed CI cols
    ItemExpr *computedColExpr = new (heap_) RenameCol(
        new (heap_) Case(NULL, new (heap_) IfThenElse(new (heap_) UnLogic(ITM_IS_NULL, colExpr->copyTree()),
                                                      rangeBeginColRef->copyTree(), colExpr->copyTree())),
        new (heap_) ColRefName(*getSequenceByPrefixColName() + colName, getLogsInfo().getBaseTableName(), heap_));
...
void MultiTxnDEMavBuilder::addSyskeyToSortKeyColumnsForRangeBlock(ItemExprList &sortKeyCols) const {
  ItemExpr *colExpr =
      new (heap_) ColReference(new (heap_) ColRefName(COMMV_BASE_SYSKEY_COL, getLogsInfo().getBaseTableName(), heap_));

  ItemExpr *rangeBeginColRef =
      new (heap_) ColReference(new (heap_) ColRefName("@BR_SYSKEY", getLogsInfo().getRangeLogTableName(), heap_));

  ItemExpr *caseExpr = new (heap_)
      Case(NULL, new (heap_) IfThenElse(new (heap_) UnLogic(ITM_IS_NULL, colExpr), rangeBeginColRef, colExpr));

  ItemExpr *newColExpr =
      new (heap_) RenameCol(caseExpr, new (heap_) ColRefName(*getSequenceByPrefixColName() + COMMV_BASE_SYSKEY_COL,
                                                             getLogsInfo().getBaseTableName(), heap_));
...
RelRoot *MultiTxnDEMavBuilder::buildRootWithFilterForEmptyRanges(RelExpr *topNode) const {
  NAString firstCIColName = getLogsInfo().getBaseNaTable()->getClusteringIndex()->getIndexKeyColumns()[0]->getColName();
  ...
  ItemExpr *firstClusteringBaseCol = new (heap_) RenameCol(
      new (heap_) ColReference(new (heap_) ColRefName(firstCIColName, getLogsInfo().getBaseTableName(), heap_)),
      new (heap_) ColRefName(firstCIColNameForProjection, getLogsInfo().getRangeLogTableName(), heap_));

  ItemExpr *specialColRef = new (heap_) ColReference(new (heap_) ColRefName(getVirtualRangeSpecialCol()));

  // The root select list is (*, isLastExpr).
  ColRefName *star = new (heap_) ColRefName(1);  // isStar
  star->setStarWithSystemAddedCols();

  ItemExpr *rootSelectList = new (heap_) ItemList(new (heap_) ColReference(star), firstClusteringBaseCol);

  RelRoot *root = new (heap_) RelRoot(topNode, REL_ROOT, rootSelectList);

  ItemExpr *selection = new (heap_) BiLogic(
      ITM_OR, new (heap_) BiRelat(ITM_EQUAL, specialColRef->copyTree(), new (heap_) SystemLiteral(VIRTUAL_BEGIN_RANGE)),
      new (heap_) UnLogic(ITM_IS_NOT_NULL, firstClusteringBaseCol));
...
RelExpr *MultiTxnDEMavBuilder::buildJoinBaseTableWithRangeLog(RelExpr *scanRangeLog, RelExpr *scanBaseTable) const {
  ...
  ItemExpr *forTableRowsOnly = new (heap_) BiRelat(ITM_EQUAL, specialColRef, new (heap_) SystemLiteral(TABLE_ROWS));

  joinPredicate = new (heap_) BiRelat(ITM_AND, joinPredicate, forTableRowsOnly);

  Join *joinNode = new (heap_) Join(scanRangeLog, scanBaseTable, REL_LEFT_JOIN, joinPredicate);
...
RelExpr *MultiTxnDEMavBuilder::buildReadRangeLogBlock() const {
  ...
  ColReference *specialColRef = new (heap_) ColReference(new (heap_) ColRefName(getVirtualRangeSpecialCol(), heap_));
