                                            ...
                                            RelExpr *parent)
{
  ...
  if (bindWA->inDDL() && bindWA->inViewDefinition())
    removeIntCorrName = TRUE;
...
ItemExpr* ItemExpr::performImplicitCasting(CharInfo::CharSet cs, BindWA *bindWA)
{
  ...
  for (Int32 i = 0; i < getArity(); i++)
  {
     ...
     if ( child(i)->getOperatorType() == ITM_HOSTVAR ) {
        ...
        if ( hv->isSystemGeneratedRangeOfValuesHV() )
           continue;
      }
...
// special relax code because we want to handle Code_Value(:ucs_hv)
ItemExpr* MaxBytes::tryToRelaxCharTypeMatchRules(BindWA *bindWA)
{
  return this;
}
...
ItemExpr *ItemExpr::bindNodeRoot(BindWA *bindWA)
{
  ...
  if (isbuiltinfunc)
    bindWA->getCurrentScope()->context()->inBuiltinFunction() = true;

  ItemExpr* exp = bindNode(bindWA);

  if (isbuiltinfunc)
    bindWA->getCurrentScope()->context()->inBuiltinFunction() = false;
...
//         
///////////////////////////////////////////////////////////////////////
static NABoolean getAdjustedDate(NAString &inputStr, 
                                 NAString &outputStr)
{
  ExpDatetime edt;
  char dtVal[4];
  ...
  UInt32 fractPrec;
  DatetimeValue dtv(inputStr.data(), REC_DATE_YEAR, REC_DATE_DAY, 
                    fractPrec, FALSE);
  ...
  if (NOT dtv.scanAndReturnFields(strValue, dtf, values))
    return FALSE;
  ...
  if (NOT isValidDate)
    {
      ...
      if (ExpDatetime::validateDate(REC_DATE_YEAR, REC_DATE_DAY, dtVal,
                                    NULL, 0, lastDayPrevMonth))
        return FALSE; // invalid date

      Int64 interval;
      char intervalBignum[BigNum::BIGNUM_TEMP_LEN];
      NABoolean isBignum = FALSE;
      edt.convertDatetimeToInterval(REC_DATE_YEAR, REC_DATE_DAY, 0,
                                    REC_DATE_DAY, dtVal, interval,intervalBignum,isBignum);
      if (! isBignum) { // should never be bignum
        interval++;
      }

      edt.convertIntervalToDatetime(interval, (isBignum ?  intervalBignum : NULL), REC_DATE_YEAR, REC_DATE_DAY, 0,
                                    dtVal);
    }

  edt.computeLastDayOfMonth(REC_DATE_YEAR, REC_DATE_DAY,
                            dtVal, lastDtVal);
  
  char buf[100];
  str_sprintf(buf, "DATE '%04d-%02d-%02d'", 
              *(short*)dtVal, (int)dtVal[2], (int)dtVal[3]);
  outputStr = buf;
...
*/
ItemExpr * BiRelat::transformPredForExtPushdown(BindWA *bindWA,
                                                ItemExpr *thisPtr,
                                                OperatorTypeEnum op,
                                                ItemExpr *child0,
                                                ItemExpr *child1,
                                                ItemExpr *child2)
{
  ...
  if ((op == ITM_BETWEEN) && (! child2))
    return thisPtr;

  if (NOT ((child0->getOperatorType() == ITM_LAST_DAY) ||
           (child0->getOperatorType() == ITM_TO_DATE)))
    return thisPtr;

  if (child1->getOperatorType() != ITM_CONSTANT)
    return thisPtr;

  if (child2 && (child2->getOperatorType() != ITM_CONSTANT))
    return thisPtr;
  ...
  if (leftCol->getOperatorType() != ITM_BASECOLUMN)
    return thisPtr;
  ...
  if (NOT ((leftBC->getTableDesc()->getNATable()->isORC()) ||
           (leftBC->getTableDesc()->getNATable()->isParquet())))
    return thisPtr;
  ...
  if ((leftType->getTypeQualifier() != NA_DATETIME_TYPE) ||
      (child1Type->getTypeQualifier() != NA_CHARACTER_TYPE) ||
      (child2Type && child2Type->getTypeQualifier() != NA_CHARACTER_TYPE))
    return thisPtr;

  const DatetimeType *leftDatetime = (DatetimeType*)leftType;
  if (leftDatetime->getSubtype()  != DatetimeType::SUBTYPE_SQLTimestamp)
    return thisPtr;
  
  NAString child1CVstr((char*)child1CV->getConstValue(), 
                       child1CV->getStorageSize());

  NAString child2CVstr;
  if (child2)
    {
      child2CVstr.append((char*)child2CV->getConstValue(),
                         child2CV->getStorageSize());
    }
  ...
  if (child0->getOperatorType() == ITM_TO_DATE)
    {
      UInt32 fractionPrec;
      DatetimeValue child1DTV(child1CVstr.data(), 
                              REC_DATE_YEAR, REC_DATE_DAY, fractionPrec, FALSE);
      if (! child1DTV.isValid())
        return thisPtr;
      
      DatetimeValue child2DTV((child2 ? child2CVstr.data() : NULL),
                              REC_DATE_YEAR, REC_DATE_DAY, fractionPrec, FALSE);
      if (child2 && (! child2DTV.isValid()))
        return thisPtr;
      ...
      if (! newChild1CVstr.isNull())
        {
          newChild1 = new (bindWA->wHeap()) 
            ConstValue(newChild1CVstr.data(), bindWA->wHeap());
          thisPtr->setChild(1, newChild1);
        }

      if (! newChild2CVstr.isNull())
        {
          newChild2 = new (bindWA->wHeap()) 
            ConstValue(newChild2CVstr.data(), bindWA->wHeap());
  ...
  else 
    {
      ...
      switch (op)
        {
        case ITM_GREATER:
          dateSameAsLastDate = getAdjustedDate(child1CVstr,
                                               newChild1CVstr);
          if (newChild1CVstr.isNull()) // error case
            return thisPtr;

          if (dateSameAsLastDate)
            str_sprintf(buf, "DATE_TRUNC('MONTH', DATEADD(MONTH, 1, %s))",
                        newChild1CVstr.data());
          else
            str_sprintf(buf, "DATE_TRUNC('MONTH', %s) ",
                        newChild1CVstr.data());
...
// of view
static ItemExpr * findBaseColumns(ItemExpr * ie,
                                  CollHeap *outHeap,
                                  void * context)
{
...
void checkForInterestingExpression(ValueId operand)
{
  ...
  if (ie->getArity() > 0)  // if it is not a leaf node
    {
      if ((ie->getOperatorType() != ITM_CAST) ||
          (ie->child(0)->getArity() > 0))  // and it isn't a CAST of a leaf
        {
...
short BiRelat::handleCompositeComparison(BindWA *bindWA)
{
  ...
  if ((child1->getArity() == 0) || (child2->getArity() == 0))
  ...
  if (new_cs == CharInfo::UTF8)
    {
      ...
      if ((eType1 == NA_CHARACTER_TYPE && eOp1 == ITM_CONSTANT) &&
          (eOp2 == ITM_DYN_PARAM || eOp2 == ITM_ROWSETARRAY_SCAN))
        {
          ...
          if (ct.getCharSet() == CharInfo::ISO88591)
            nTranslateChild = 0;
        }
...
{
  if (checkForSQLnullChild(bindWA, this, (getSpecialNulls()||getCompatibleNulls())))
      return this;
  ...
  newThisPtr = 
    BiRelat::transformPredForExtPushdown(bindWA, this, getOperatorType(),
...
// the code following should be removed.
  ...
  if (((child(0)->getOperatorType() == ITM_BASECOLUMN) && !child1isCol && (type1.getFSDatatype() == REC_FLOAT32) && (type2.getFSDatatype() != REC_FLOAT64)) ||
      ((child(1)->getOperatorType() == ITM_BASECOLUMN) && !child0isCol && (type2.getFSDatatype() == REC_FLOAT32) && (type1.getFSDatatype() != REC_FLOAT64))) {
      int index = (child(0)->getOperatorType()==ITM_BASECOLUMN) ? 1 : 0;
      ItemExpr * realExpr = NULL;
      realExpr = new (bindWA->wHeap())
          Cast(new (bindWA->wHeap()) Cast(child(index),
            new (bindWA->wHeap()) SQLDoublePrecision(bindWA->wHeap(), TRUE)),
      ...
      type2.getTypeQualifier() == NA_CHARACTER_TYPE)
  {
    ...
    if (chType1.getVarLenHdrSize() == 0 &&
        chType2.getVarLenHdrSize() == 0 && //both are char
        chType1.getCharSet() != chType2.getCharSet() &&
        chType1.getCharSet() != CharInfo::UnknownCharSet &&
        chType2.getCharSet() != CharInfo::UnknownCharSet &&
        (child(0)->getOperatorType() != ITM_CONSTANT ||
         chType1.getCharSet() != CharInfo::ISO88591)     &&
        (child(1)->getOperatorType() != ITM_CONSTANT ||
         chType2.getCharSet() != CharInfo::ISO88591))
    {
      ...
      if (isTrailingBlankSensitive && atoi(isTrailingBlankSensitive) == 1)
      {
            ...
            (cs1 == CharInfo::UTF8 && cs2 == CharInfo::ISO88591))
        {
          ...
          if (byte1 > byte2)
          {
            CharType *targetType = new(bindWA->wHeap())
                                   SQLChar(bindWA->wHeap(),
                                           CharLenInfo(0, byte1),
                                           chType2.supportsSQLnull(),
                                           chType2.isUpshifted(),
                                           chType2.isCaseinsensitive(),
                                           FALSE,
                                           chType2.getCharSet(),
                                           chType2.getCollation(),
                                           chType2.getCoercibility());
          ...
          if (byte1 < byte2)
          {
            CharType *targetType = new(bindWA->wHeap())
                                   SQLChar(bindWA->wHeap(),
                                           CharLenInfo(0, byte2),
                                           chType1.supportsSQLnull(),
                                           chType1.isUpshifted(),
                                           chType1.isCaseinsensitive(),
                                           FALSE,
                                           chType1.getCharSet(),
                                           chType1.getCollation(),
                                           chType1.getCoercibility());
                 ...
                 (cs1 == CharInfo::UCS2 && cs2 == CharInfo::UTF8))
        {
          ...
          if (char1 > char2)
          {
            CharType *targetType = new(bindWA->wHeap())
                                   SQLChar(bindWA->wHeap(),
                                           CharLenInfo(char1, char1 * chType2.getBytesPerChar()),
                                           chType2.supportsSQLnull(),
                                           chType2.isUpshifted(),
                                           chType2.isCaseinsensitive(),
                                           FALSE,
          ...
          if (char1 < char2)
          {
            CharType *targetType = new(bindWA->wHeap())
                                   SQLChar(bindWA->wHeap(),
                                           CharLenInfo(char2, char2 * chType1.getBytesPerChar()),
                                           chType1.supportsSQLnull(),
                                           chType1.isUpshifted(),
                                           chType1.isCaseinsensitive(),
                                           FALSE,
  ...
  if (bInWhere)
    {
      ...
      if ((ie1->getOperatorType() == ITM_DYN_PARAM ||
          ie1->getOperatorType() == ITM_ROWSETARRAY_SCAN)
          && child(1)->getValueId().getType().getTypeQualifier() == NA_NUMERIC_TYPE)
        {
          nDynParamIdx = 0;
          nNumericIdx = 1;
        }
      else if((ie2->getOperatorType() == ITM_DYN_PARAM ||
              ie2->getOperatorType() == ITM_ROWSETARRAY_SCAN)
          ...
          if (nacol)
            {
              ...
              if (naTable && naTable->isSeabaseTable() && !naTable->isSeabaseMDTable()
                  && !naTable->isHistogramTable() && !naTable->isSeabasePrivSchemaTable()
                  && (naTable->getSpecialType() == ExtendedQualName::NORMAL_TABLE))
                bUserTable = TRUE; 
            }
          else
          ...
          if (bUserTable && type.isExact())
            {
              ...
              Lng32 nScale = 26;
              if (nScale <= type.getScale())
                nScale = type.getScale()+1;
              if (nScale > nPrecision)
                nScale = nPrecision;
  ...
  if (handleCompositeComparison(bindWA))
    return NULL;
     ...
     NABoolean convertNumToChar=FALSE)
{
  ...
  if (CmpCommon::getDefault(ALLOW_INCOMPATIBLE_OPERATIONS) == DF_ON)
  {
	...
	(type2.getTypeQualifier() == NA_NUMERIC_TYPE))
    {
      ...
      if (charType->getStrCharLimit() > UINT32_MAX_LENGTH)
        conversion = 9;
    }
    else
    ...
    switch (conversion)
    {
            ...
            newOp = new (bindWA->wHeap())
                    Cast(op,
                          new (bindWA->wHeap())
                            SQLChar(bindWA->wHeap(), 
                                    256,
                                    charType->supportsSQLnull(),
                                    charType->isUpshifted(),
                                    charType->isCaseinsensitive(),
                                    FALSE,
                                    charType->getCharSet(),
                                    charType->getCollation(),
          ...
          // scale and type specified in the char value.
           newOp = new (bindWA->wHeap())
                    Cast((srcOpIndex == 0 ? op1 : op2),
                          new (bindWA->wHeap())
                            SQLBigNum(bindWA->wHeap(), 128, 38, TRUE, TRUE, TRUE));
      ...
      case 2:
      {
        ...
        NAType *newType = (tgtOpIndex == 0 ? op1 : op2)->castToItemExpr()->getValueId()
                                                                    .getType().newCopy(bindWA->wHeap());
...
BiRelat * BiRelat::handleIncompatibleComparison(BindWA *bindWA)
{
  ...
  if ( child0->getOperatorType() == ITM_ITEM_LIST &&
       child1->getOperatorType() == ITM_ITEM_LIST)
  {
    ValueIdList vl0;
    ValueIdList vl1;
    ...
    for (Int32 i = 0; i < vl0.entries(); i++)
    {
      ...
      if ( convertNumToChar() && 
           (leftExpr->getOperatorType() == ITM_BASECOLUMN ||
            leftExpr->getOperatorType() == ITM_CONSTANT) &&
           type1.getTypeQualifier() == NA_CHARACTER_TYPE )
        numToChar = TRUE;
      result = 
        ItemExpr_handleIncompatibleComparison(bindWA,
                                              NULL,
                                              vl0[i].getItemExpr(),
                                              vl1[i].getItemExpr(),
                                              tmpChild0,
      ...
      if(bindWA->errStatus()) 
        return NULL;
      if ( !tmpChild0 )
          tmpChild0 = vl0[i].getItemExpr();
      if ( !tmpChild1 )
          tmpChild1 = vl1[i].getItemExpr();
  ...
  switch (getOperatorType())
	...
	NABoolean useCase = FALSE;
        ...
        if ( NOT typ1.isCompatible(typ2) &&
             (child(0).getPtr()->getOperatorType() != ITM_DYN_PARAM) &&
             !bOpera1IsNull)
          {
            if (typ1.getTypeQualifier() == NA_CHARACTER_TYPE &&
                child(1).getPtr()->getOperatorType() == ITM_DYN_PARAM)
            {
              ...
              const NAType *desiredType = new (bindWA->wHeap())SQLVarChar(
                                                              bindWA->wHeap(),
                                                              len,
                                                              typ1_tmp->supportsSQLnull(),
                                                              typ1_tmp->isUpshifted(),
                                                              typ1_tmp->isCaseinsensitive(),
                                                              typ1_tmp->getCharSet(),
                                                              typ1_tmp->getCollation(),
                                                              typ1_tmp->getCoercibility());
	...
	retExpr =
	  parser.getItemExprTree(
		   ...
		   0, BINDITEMEXPR_STMTCHARSET, 3, child(0),child(1),child(2));
    ...
    case ITM_AES_DECRYPT:
    {
      if (child(0))
      {
        ...
        if (type0.getTypeQualifier() == NA_CHARACTER_TYPE &&
            type0.getCharSet() == CharInfo::UTF8)
        {
...
ItemExpr *Modulus::bindNode(BindWA *bindWA)
{
  ...
  if (type_op1.isBigNum() || type_op2.isBigNum() ||
      nPrecision1 + nScale2 > 18 ||
      nPrecision2 + nScale1 > 18)
  { // we are processing big number cases here
    Int32 tempPrecison;
    char buf[100];
    ItemExpr *parseTree;
    Parser parser(bindWA->currentCmpContext());
    ...
    sprintf(buf, "CAST(@A1 - CAST(@A1 / @A2 AS NUMERIC(%d)) * @A2 AS NUMERIC(%d, %d))",
            tempPrecison,
            nScale1 > nScale2 ? nPrecision1 : nPrecision2,
            nScale1 > nScale2 ? nScale1 : nScale2);
...
ItemExpr *Concat::bindNode(BindWA *bindWA)
{
  ...
  if (CmpCommon::getDefault(ALLOW_INCOMPATIBLE_OPERATIONS) == DF_ON)
          ...
          if (type.getTypeQualifier() == NA_DATETIME_TYPE)
            {
              ...
              DatetimeType &dtType = (DatetimeType&)
                                      child(idx)->castToItemExpr()->getValueId().getType();
    ...
    if ((child(0)->getOperatorType() == ITM_DYN_PARAM ||
         child(1)->getOperatorType() == ITM_DYN_PARAM) &&
        (type1.getCharSet() != type2.getCharSet()))
    {
      ...
      for (Int32 idx = 0; idx < getArity(); idx++)
      {
        const NAType &type = child(idx)->castToItemExpr()->getValueId().getType();
        ItemExpr *newChild = NULL;
        if (CharInfo::getCharSetEnum(cs) != type.getCharSet() &&
            type.getCharSet() != CharInfo::UnknownCharSet)
        {
          if (type.getCharSet() == CharInfo::UTF8)
            continue;
          Int32 nMode = find_translate_type(type.getCharSet(), CharInfo::UTF8);
          ItemExpr *newChild =
              new (bindWA->wHeap()) Translate(child(idx), nMode);
          newChild = newChild->bindNode(bindWA);
          if (bindWA->errStatus())
            return this;
          setChild(idx, newChild);
...
{
  if ( DateFormat::FORMAT_TO_CHAR != formatType_ )
     return;
      ...
      if (toChar)
        {
          ...
          if ( NOT (mc1 && frmt == ExpDatetime::DATETIME_FORMAT_NULL) &&
               NA_DATETIME_TYPE == opType->getTypeQualifier())
            error = 1;//error 4065
        }
      else
  ...
  if (!error && toTime)
    {
      ...
      else if(ExpDatetime::DATETIME_FORMAT_TS28 == frmt
              || ExpDatetime::DATETIME_FORMAT_TS29 == frmt)
        {
          error = 1; // error 4065
        }
    }
  ...
  if (!error && toChar)
    {
      ...
      if (getArity() == 2 && eType != NA_DATETIME_TYPE
         && (eType != NA_NUMERIC_TYPE && eType != NA_CHARACTER_TYPE))
        error = 2;//error 4071
  ...
  if (!error && toDate)
    {
      ...
      if (child(0).getPtr()->getOperatorType() != ITM_DYN_PARAM &&
          ...
          (opType->getTypeQualifier() != NA_DATETIME_TYPE))
        error = 4; //error 4043
...
DateFormat::DateFormat(ItemExpr *val1Ptr, ItemExpr *val2Ptr,
                       Lng32 formatType, NABoolean wasDateformat)
     : CacheableBuiltinFunction(ITM_DATEFORMAT,
                                2, val1Ptr,val2Ptr),
       wasDateformat_(wasDateformat),
...
NABoolean DateFormat::synthesizeNull()
{
  ...
  if (getArity() > 1)
    bNullArg |= child(1)->castToItemExpr()->isNullConstant();
  if (mc1 && bNullArg)
    {
      return TRUE;
    }
...
ItemExpr * DateFormat::bindNode(BindWA * bindWA)
{
      ...
      if (formatType_ == FORMAT_TO_CHAR)
        {
          ...
          ItemExpr *newExpr = new (bindWA->wHeap())
                                    Cast(child(0),
                                         new (bindWA->wHeap())
                                         SQLVarChar(bindWA->wHeap(), nCharLen,//to_char return type is varchar.(compatible with Oracle)
                                         child(0)->castToItemExpr()->
                                         getValueId().getType().supportsSQLnull()));
  ...
  else if (FORMAT_TO_CHAR == formatType_
           && NA_DATETIME_TYPE != naType0->getTypeQualifier()
				  ...
				  const NAString &fabricatedName)
{
  ...
  if (opTyp == ITM_DYN_PARAM AND getText() == "?") {
    ...
    if (origParam != NULL)
      orig = origParam->bindNode(bindWA);
    if (orig == NULL) {
                                   ...
                                   RelExpr *parent)
{
  ...
  while (!stk.empty())
  {
      ...
      else
	{
      ...
      if (bindWA->errStatus() &&
          ((current_ie->getOperatorType() == ITM_REFERENCE &&
           ((ColReference *)current_ie)->isIgnoreUnusedColErr()) ||
          (current_ie->getOperatorType() == ITM_INVERSE &&
           current_ie->child(0)->getOperatorType() == ITM_REFERENCE &&
           ((ColReference *)(current_ie->child(0)->castToItemExpr()))
                                       ->isIgnoreUnusedColErr())) &&
...
ItemExpr *Aggregate::bindNode(BindWA *bindWA)
{
  ...
  if ((CmpCommon::getDefault(ALLOW_INCOMPATIBLE_OPERATIONS) == DF_ON) &&
      ...
      if (type1.getTypeQualifier() == NA_CHARACTER_TYPE)
	{
	  ...
	  ItemExpr * newChild =
	    ...
	    Cast(child(0),
		 new (bindWA->wHeap())
         SQLBigNum(bindWA->wHeap(), 128, 38, TRUE, TRUE, type1.supportsSQLnull()));
	  setChild(0, newChild->bindNode(bindWA));
	}
  ...
  else
  {
      ...
      // per output 
    if (childDegree != 1  &&
         (getOperatorType() != ITM_ONEROW &&
          getOperatorType() != ITM_PIVOT_GROUP))
    {
...
ItemExpr *BiArith::bindNode(BindWA *bindWA)
{
  ...
  if (checkForSQLnullChild(bindWA, this, getSupportNullOperand(), ARITH_, isUnaryNegate()))
    return this;
 ...
 Lng32 tmpvallen = 20;
		      ...
		      (numeric->isExact()))
        {
                ...
                (datetime->getTypeName() != "TIME"))
            {
                ...
                else if (datetime->getTypeName() == "TIMESTAMP")
                {
                    ...
                    //                                        day                hour                             min                                   sec(6)
                    // interval_format = parser.getItemExprTree("floor(@A1) ||' '|| floor(MOD(@A1 * 24, 24)) ||':'|| floor(MOD(@A1 * 1440, 60)) ||':'|| (CAST(MOD(@A1 * 86400, 60) AS NUMERIC(8,6)))",0,BINDITEMEXPR_STMTCHARSET, 1, child(1));
                    ...
                    // interval DAY
                    sprintf(str, "CAST((@A1) AS INTERVAL DAY(%d))", maxDigits);
                    ItemExpr * interval_format_day  = parser.getItemExprTree(str,strlen(str),BINDITEMEXPR_STMTCHARSET, 1, child(1));
                    // interval HOUR
                    ItemExpr * interval_format_hour = parser.getItemExprTree("CAST((MOD(@A1 * 24, 24)) AS INTERVAL HOUR(2))",0,BINDITEMEXPR_STMTCHARSET, 1, child(1));
                    // interval MINUTE
                    ItemExpr * interval_format_min  = parser.getItemExprTree("CAST((MOD(@A1 * 1440, 60)) AS INTERVAL MINUTE(2))",0,BINDITEMEXPR_STMTCHARSET, 1, child(1));

                    // interval SECOND
                    sprintf(str, "CAST((MOD(@A1 * 86400, 60)) AS INTERVAL SECOND(2, %d))", timeScale);
                    ItemExpr * interval_format_sec  = parser.getItemExprTree(str,strlen(str),BINDITEMEXPR_STMTCHARSET, 1, child(1));
            ...
            {
                interval = new(bindWA->wHeap()) SQLInterval(bindWA->wHeap(),
  ...
  if ((result_type->getTypeQualifier() == NA_NUMERIC_TYPE) &&
    ...
    (result_type->getTypeQualifier() == NA_NUMERIC_TYPE) &&
    (result_type->getPrecision() == 256) && (result_type->getScale() == 12))
    {
      NAType * new_result_type = 
        new (bindWA->wHeap()) SQLBigNum(bindWA->wHeap(), 128, 6, TRUE, TRUE, TRUE);
  ...
  if (bUserTable && CmpCommon::getDefault(MODE_COMPATIBLE_1) == DF_ON)
    {
          ...
          if (desiredNumericType.isExact())
            {
              ...
              if (nCastScale >nCastPrecision)
                nCastScale= nCastPrecision;
              //Add CAST to child(1)
      ...
      else if (desiredType.getTypeQualifier() == NA_NUMERIC_TYPE
          && sourceType.getTypeQualifier() == NA_NUMERIC_TYPE)
        {
          const NumericType &desiredNumericType = (NumericType&)desiredType;
  ...
  if (nacol)
  {
    ...
    if (tgtType.getTypeQualifier() == NA_CHARACTER_TYPE)
    {
      ...
      if (tgtCharType.isVarchar2() && child(1)->getArity() > 0)
      {
        ...
        if (leafNode)
          leafNode->setAssignColName(nacol->getColName());
      }
    }
  }
 ...
 targetType =  child(0)->castToItemExpr()->getValueId().getType().getTypeQualifier() ;
  ...
  if (targetType == NA_CHARACTER_TYPE) {
	...
	return boundExpr;
        ...
        // the code following should be removed.
        ItemExpr * realExpr = NULL;
        realExpr = new (bindWA->wHeap()) Cast(child(1),
               new (bindWA->wHeap()) SQLDoublePrecision(bindWA->wHeap(), TRUE));
        realExpr->bindNode(bindWA);
        if (bindWA->errStatus())
            return boundExpr;
...
ItemExpr *Cast::bindNode(BindWA *bindWA)
{
  ...
  if (CmpCommon::getDefault(MODE_COMPATIBLE_1) == DF_ON)
    {//Can not cast NUMERIC to INTERVAL YEAR TO MONTH
      if (NA_NUMERIC_TYPE == type1.getTypeQualifier() &&
          NA_INTERVAL_TYPE == type2->getTypeQualifier())
        {
          const IntervalType* Ivt = (IntervalType*)type2;
          if (REC_DATE_YEAR == Ivt->getStartField() &&
              REC_DATE_MONTH == Ivt->getEndField())
            {
  ...
  if ((type1.getFSDatatype() == REC_NUM_BIG_UNSIGNED ||
       type1.getFSDatatype() == REC_NUM_BIG_SIGNED) &&
      (type2->getFSDatatype() == REC_FLOAT32 ||
       type2->getFSDatatype() == REC_IEEE_FLOAT32)) {
      ItemExpr *ce = new(bindWA->wHeap()) Cast(child(0),
              new(bindWA->wHeap())SQLDoublePrecision(bindWA->wHeap()));
  ...
  if ((getType()->isComposite()) ||
      (child(0)->castToItemExpr() &&
       child(0)->castToItemExpr()->getValueId().getType().isComposite()))
    {
                                    ...
                                    NABoolean markGroupByForSingleInt)
{
  ...
  NAString fileName( naTable->getViewText() ?
                     ...
                     (naTable->getClusteringIndex() ? naTable->getClusteringIndex()->
...
ItemExpr *ColReference::bindNode(BindWA *bindWA)
{
     ...
     )
  {
          ...
          currScope->getRETDesc()->isGrouped() )
    {
      ...
      *CmpCommon::diags() << DgSqlCode(sqlCode)
      ...
      if (sqlCode == -4120 && isUserCol)
        setIgnoreUnusedColErr(true);
    }
  ...
  if (getColRefNameObj().getCorrNameObj().getQualifiedNameObj().getCatalogName()
    ...
    if (jPred &&
        jPred->containsRightmost(mPred) &&	// BiR/Func is rtmost in JoinPrd
  ...
  if ((currScope->context()->inGroupByClause()) AND
      ...
      (!udfSubqInAggGrby_Enabled ||
       !(currScope->context()->inUDFunction() ||
         currScope->context()->inBuiltinFunction())))
  ...
  if (bindWA->getCurrentScope()->context()->inPrecondClause() &&
      bindScope == currScope)
  {
  ...
  if (isCompCol())
    {
      if (currScope->context()->inOrderBy() ||
          currScope->context()->inGroupByClause())
	{
          *CmpCommon::diags() << DgSqlCode(-4324);
	  bindWA->setErrStatus();
	  return this;  // error
      ...
      delete colRefName_;
      colRefName_ = new (PARSERHEAP()) 
        ColRefName(compNames_[compNames_.entries()-1]);

      ItemExpr *ie = new (PARSERHEAP()) 
        CompositeExtract(this, compNames_, compArrIndexes_); 
      ie->bindNode(bindWA);
  ...
  if (type1.getTypeQualifier() == NA_NUMERIC_TYPE)
    {
      ItemExpr * newChild = new (bindWA->wHeap()) 
        Trim((Int32)Trim::TRAILING,
             new (bindWA->wHeap()) SystemLiteral(" ", WIDE_(" ")), child(0));
      ...
      if (operand2.getPrecision() == SQLDTCODE_TIMESTAMP)
        {
          ItemExpr *newChild = new (bindWA->wHeap())
                               Cast(child(1),
                                    new (bindWA->wHeap())
                                    SQLDate(bindWA->wHeap(), child(1)->castToItemExpr()->
                                            getValueId().getType().supportsSQLnull()));
          newChild = newChild->bindNode(bindWA);
          if (bindWA->errStatus())
            return this;
...
ItemExpr *RegexpSubstr::bindNode(BindWA *bindWA)
{
  ...
  if (CmpCommon::getDefault(MODE_COMPATIBLE_1) == DF_ON
      ...
      && type1.getTypeQualifier() != NA_UNKNOWN_TYPE)
    {
      ...
      if (sTypName != "FLOAT" && sTypName != "REAL"
          && sTypName != "DOUBLE PRECISION")
        {
          Lng32 nLen = type1.getDisplayLength();
          ItemExpr * newChild = new (bindWA->wHeap())
                                Cast(child(0),
                                     new (bindWA->wHeap())
                                     SQLChar(bindWA->wHeap(), nLen,
                                       child(0)->castToItemExpr()->
                                            getValueId().getType().supportsSQLnull()));
          newChild = newChild->bindNode(bindWA);
          if (bindWA->errStatus())
      ...
      if (! (type1.getTypeQualifier() == NA_NUMERIC_TYPE && !((NumericType&)type1).isExact()))
        {
          ...
          ItemExpr * newChild = new (bindWA->wHeap())
                                Cast(child(0),
                                     new (bindWA->wHeap())
                                     SQLVarChar(bindWA->wHeap(), nLen,//varchar to avoid space
                                       child(0)->castToItemExpr()->
                                            getValueId().getType().supportsSQLnull()));
          newChild = newChild->bindNode(bindWA);
          if (bindWA->errStatus())
...
ItemExpr *Subquery::bindNode(BindWA *bindWA)
{
  ...
  if( context->inStartWith() == TRUE)
  {
    ...
    for (ValueId x = bindWA->getStartWithDynParamVidSet().init();
          bindWA->getStartWithDynParamVidSet().next(x);
...
ItemExpr *QuantifiedComp::bindNode(BindWA *bindWA)
{
  ...
  if ((CmpCommon::getDefault(ALLOW_INCOMPATIBLE_OPERATIONS) == DF_ON) &&
        ...
        type1.getTypeQualifier() == NA_CHARACTER_TYPE)
    {
      ...
      if (ctype2->getVarLenHdrSize() > 0 &&
          ctype1->getVarLenHdrSize() == 0 &&
          getSubquery()->selectList()->getOperatorType() == ITM_CAST)
      {
        char *isTrailingBlankSensitive = getenv("VARCHAR_TRAILING_BLANK_SENSITIVE");
        if (isTrailingBlankSensitive && atoi(isTrailingBlankSensitive) == 1)
        {
          CharType *newType = new (bindWA->wHeap())
                              SQLChar(bindWA->wHeap(),
                                ctype2->getNominalSize(),
  ...
  if (CmpCommon::getDefault(MODE_COMPATIBLE_1) == DF_ON)
    {
      for (int i = 1; i < getArity(); i++)
      {
        ...
        if (operandi->getTypeQualifier() == NA_NUMERIC_TYPE &&
            operandi->getScale() > 0)
        {
      ...
      char buf[1000];
      Parser parser(bindWA->currentCmpContext());
      sprintf(buf, "CASE WHEN @A2 = 0 THEN 1 "
                     "     WHEN @A2 > 0 THEN @A2 "
    ...
    {                                                   // real and old metadata.
      ...
      if (functionName1.getCatalogNamePartAsAnsiString() == "")
        functionName1.setCatalogNamePart(
          bindWA->getDefaultSchema().getCatalogNameAsAnsiString());
      ...
      if (functionName1.getSchemaNamePartAsAnsiString() == "")
        functionName1.setSchemaNamePart(
          bindWA->getDefaultSchema().getUnqualifiedSchemaNameAsAnsiString());
      ...
      if (functionName3.getCatalogNamePartAsAnsiString() == "")
        functionName3.setCatalogNamePart(
          bindWA->getDefaultSchema().getCatalogNameAsAnsiString());
      if (functionName3.getSchemaNamePartAsAnsiString() == "")
        functionName3.setSchemaNamePart(
          bindWA->getDefaultSchema().getUnqualifiedSchemaNameAsAnsiString());
        ...
        if (!functionName3.isEmpty())
        {
          ...
          if (NULL == udf)
          {
            udfMetadata =  cmpSBD.getSeabaseRoutineDesc(
				       functionName3AsQualName.getCatalogName(),
...
ItemExpr * ValueIdUnion::tryToDoImplicitCasting( BindWA *bindWA )
{
  ...
  if (savedMaxLen < getBytesPerChar * getStrCharLimit)
    savedMaxLen = getBytesPerChar * getStrCharLimit;
...
static NABoolean replaceString(std::string& subject, const std::string& search,
...
ItemExpr *ZZZBinderFunction::bindNode(BindWA *bindWA)
{
  ...
  switch (getOperatorType())
    ...
    case ITM_DATE_TRUNC_HOUR:
      {
        ...
        if (CmpCommon::getDefault(MODE_COMPATIBLE_1) == DF_ON)
          strcpy(buf, "CAST( CAST(@A1 AS DATE) AS TIMESTAMP(0)) + CAST( HOUR( CAST(@A1 AS TIMESTAMP) ) AS INTERVAL HOUR);");
        else
    ...
    case ITM_DATE_TRUNC_SECOND:
      {
        if (enforceDateOrTimestampDatatype(bindWA,0,2))
          ...
          strcpy(buf, "DATE_TRUNC('MINUTE',@A1) + CAST( CAST( SECOND(CAST(@A1 AS TIMESTAMP)) AS SMALLINT) AS INTERVAL SECOND);");
      }
      break;
    ...
    case ITM_DATE_TRUNC_CENTURY:
      {
        ...
        if (CmpCommon::getDefault(MODE_COMPATIBLE_1) == DF_ON)
          strcpy(buf, "CAST( CAST(@A1 AS DATETIME YEAR) AS TIMESTAMP(0) ) - CAST( MOD(YEAR(@A1),100) AS INTERVAL YEAR(4)  );");
        else
          strcpy(buf, "CAST( CAST(@A1 AS DATETIME YEAR) AS TIMESTAMP ) - CAST( MOD(YEAR(@A1),100) AS INTERVAL YEAR(4)  );");
      }
      break;
    ...
    case ITM_DATE_TRUNC_DECADE:
      {
        ...
        if (CmpCommon::getDefault(MODE_COMPATIBLE_1) == DF_ON)
          strcpy(buf, "CAST( CAST(@A1 AS DATETIME YEAR) AS TIMESTAMP(0) ) - CAST( MOD(YEAR(@A1),10) AS INTERVAL YEAR(4)   );");
        else
          strcpy(buf, "CAST( CAST(@A1 AS DATETIME YEAR) AS TIMESTAMP ) - CAST( MOD(YEAR(@A1),10) AS INTERVAL YEAR(4)   );");
      }
      break;
    ...
    case ITM_LAST_DAY:
      {
        ...
        if (CmpCommon::getDefault(HIVE_COMPATIBILITY_MODE) == DF_OFF)
          ...
          strcpy(buf, "CAST(CAST(@A1 - CAST( DAY(@A1) -1 AS INTERVAL DAY) + INTERVAL '1' MONTH - INTERVAL '1' DAY AS DATE) AS CHAR(10) character set ISO88591);");
      }
      break;
   ...
   case ITM_LEAST:
	...
	const NAType &type0 =
	  tempBoundTree0->getValueId().getType();
	const NAType &type3 =
	  tempBoundTree3->getValueId().getType();
	// if A1 is not string, cast it to string 
	if (type0.getTypeQualifier() == NA_DATETIME_TYPE)
	{
	  ItemExpr *newchild0 = new (bindWA->wHeap())DateFormat(tempBoundTree0, "UNSPECIFIED", DateFormat::FORMAT_TO_CHAR);
	  setChild(0, newchild0);
	}
	else if(type0.getTypeQualifier() == NA_NUMERIC_TYPE)
	{
	  ItemExpr *newchild0 = new (bindWA->wHeap())Cast(tempBoundTree0, new (bindWA->wHeap())SQLVarChar(bindWA->wHeap(), MAX_NUMERIC_PRECISION, type0.supportsSQLnull()));
	  setChild(0, newchild0);
	}
	// if A4 is not string, cast it to string
	if (type3.getTypeQualifier() == NA_DATETIME_TYPE)
	{
	  ItemExpr *newchild3 = new (bindWA->wHeap())DateFormat(tempBoundTree3, "UNSPECIFIED", DateFormat::FORMAT_TO_CHAR);
	  setChild(3, newchild3);
	}
	else if(type3.getTypeQualifier() == NA_NUMERIC_TYPE)
	{
	  ItemExpr *newchild3 = new (bindWA->wHeap())Cast(tempBoundTree3, new (bindWA->wHeap())SQLVarChar(bindWA->wHeap(), MAX_NUMERIC_PRECISION, type3.supportsSQLnull()));
	  setChild(3, newchild3);
	}
	...
	strcat(buf, "|| CASE WHEN (CHAR_LENGTH(@A1) - (CAST(@A2 AS INT UNSIGNED) + CAST(@A3 AS INT UNSIGNED)) + 1) > 0 THEN RIGHT(@A1, CHAR_LENGTH(@A1) - (CAST(@A2 AS INT UNSIGNED) + CAST(@A3 AS INT UNSIGNED)) + 1) ELSE RIGHT(@A1, 0) END) END;");
      }
    break;
    ...
    case ITM_LPAD:
      {
        ...
        if (child(2))
        {
          ...
          if (type2.getTypeQualifier() == NA_DATETIME_TYPE)
          {
            ItemExpr *newchild2 = new (bindWA->wHeap())DateFormat(tempBoundTree2, "UNSPECIFIED", DateFormat::FORMAT_TO_CHAR);
            setChild(2, newchild2);
          }
          else if(type2.getTypeQualifier() == NA_NUMERIC_TYPE)
          {
            ItemExpr *newchild2 = new (bindWA->wHeap())Cast(tempBoundTree2, new (bindWA->wHeap())SQLVarChar(bindWA->wHeap(), MAX_NUMERIC_PRECISION, type2.supportsSQLnull()));
            setChild(2, newchild2);
          }
        }
          ...
          else
          {
                     ...
                     (toUpper(roundmode) == "MON") || (toUpper(roundmode) == "RM"))
            {
              ...
              if (tempBoundTree->getValueId().getType().getSimpleTypeName() == "DATE")
              {
                //date type
               strcpy(buf, "CASE "
                           "WHEN DAY(@A1) > DAY(CAST(@A1 - CAST(DAY(@A1) -1 AS INTERVAL DAY) + INTERVAL '1' MONTH - INTERVAL '1' DAY AS DATE)) / 2 THEN CAST(DATE_TRUNC('MONTH', @A1) + INTERVAL '1' MONTH AS DATE) "
              ...
              else
              {
                //timestmap type
                strcpy(buf, "CASE "
                            "WHEN DAY(@A1) > DAY(CAST(@A1 - CAST(DAY(@A1) -1 AS INTERVAL DAY) + INTERVAL '1' MONTH - INTERVAL '1' DAY AS DATE)) / 2 THEN DATE_TRUNC('MONTH', @A1) + INTERVAL '1' MONTH "
            ...
            else if ((toUpper(roundmode) == "HH24") || (toUpper(roundmode) == "HH") || (toUpper(roundmode) == "HH12"))
            {
              //round by hour
              if (tempBoundTree->getValueId().getType().getSimpleTypeName() == "DATE")
              {
            ...
            else if ((toUpper(roundmode) == "CC") || (toUpper(roundmode) == "SCC"))
            {
              ...
              {
                strcpy(buf, "CASE "
                            "WHEN MOD(YEAR(@A1), 100) = 0  THEN CAST(DATE_TRUNC('CENTURY', @A1) - INTERVAL '100' YEAR(4) AS DATE) "
                            "WHEN MOD(YEAR(@A1), 100) > 50 THEN CAST(DATE_TRUNC('CENTURY', @A1) + INTERVAL '100' YEAR(4) AS DATE) "
            ...
            else if ((toUpper(roundmode) == "DAY") || (toUpper(roundmode) == "DY") || (toUpper(roundmode) == "D"))
            {
              ...
              {
                strcpy(buf, "CASE "
                            "WHEN DAYOFWEEK(@A1) > 4 THEN @A1 + INTERVAL '7' DAY - CAST((DAYOFWEEK(@A1) - 1) AS INTERVAL DAY) "
              ...
              {
                //timestamp type
                //add date_trunc function to clear time field (e.g. 2020-11-01 16:12:12 convert to 2020-11-01 00:00:00)
                //when op1 type is timestamp and op1 is Wednesday then extract hour of op1 if its the first half of the day 
                //then the result of round shoud first day of current week else its half a day later result would be first day
                //of next week
                strcpy(buf, "CASE "
                            "WHEN DAYOFWEEK(@A1) = 4 THEN "
                              "CASE "
                              "WHEN EXTRACT(HOUR FROM @A1) > 11 THEN DATE_TRUNC('DAY', @A1 + INTERVAL '7' DAY - CAST((DAYOFWEEK(@A1) - 1) AS INTERVAL DAY)) "
                              "ELSE DATE_TRUNC('DAY', @A1 - CAST((DAYOFWEEK(@A1) - 1) AS INTERVAL DAY)) "
                              "END "
                            "WHEN DAYOFWEEK(@A1) > 4 THEN DATE_TRUNC('DAY', @A1 + INTERVAL '7' DAY - CAST((DAYOFWEEK(@A1) - 1) AS INTERVAL DAY)) "
                            "ELSE DATE_TRUNC('DAY', @A1 - CAST((DAYOFWEEK(@A1) - 1) AS INTERVAL DAY)) "
                            "END;");
              }
            }
 ...
 Lng32 roundTo = -1;
	...
	if (useOptimizedRound)
	 ...
	 Int32 i = (type_op1.getScale() - roundTo);
		...
		str_sprintf(buf, "cast(@A1 * %ld as numeric(%d,%d))", 
                  denom, nPrecision, MAXOF(type_op1.getScale(), roundTo));
	...
	if (tempBoundTree->getValueId().getType().getTypeQualifier() == NA_NUMERIC_TYPE)
      ...
      else
      {
        ...
        str_sprintf(buf, "CAST(( @A1 / %s ) as NUMERIC(%d, 0))* %s ;",
          divstr, precision, divstr);
      }
	  }
	...
	if (bindWA->errStatus()) 
      ...
      else
      {
        ...
        precision = MINOF(precision, 
                          (UInt32)CmpCommon::getDefaultNumeric(MAX_NUMERIC_PRECISION_ALLOWED));
	...
	if (bindWA->errStatus()) 
	  return this;
        ...
        if ((type0.getTypeQualifier() != NA_CHARACTER_TYPE) &&
            (type0.getTypeQualifier() != NA_DATETIME_TYPE))
          {
            
            *CmpCommon::diags() << DgSqlCode(-3242)
                                << DgString0("Operand must be String or Datetime datatype.");
        ...
        if (! child(1))
          {
            str_sprintf(buf, "((JULIANTIMESTAMP(CAST(@A1 AS TIMESTAMP)) - %ld)/1000000)",
                        COM_EPOCH_TIMESTAMP);
          }
        ...
        else
          {
            ...
            str_sprintf(buf, "((JULIANTIMESTAMP(TO_DATE(@A1, %s)) - %ld) / 1000000);",
                        cv->getText().data(),
                        COM_EPOCH_TIMESTAMP);
          }
      }
	...
	if (bindWA->errStatus()) 
	  return this;
        ...
        str_sprintf(buf, "CAST(CONVERTTIMESTAMP(%ld + @A1 * 1000000) AS CHAR(19));",
                    COM_EPOCH_TIMESTAMP);
      }
    break;
	...
	if (bindWA->errStatus()) 
	  return this;
        ...
        if (tempBoundTree2->getOperatorType() != ITM_CONSTANT)
          {
            *CmpCommon::diags() << DgSqlCode(-3242)
                                << DgString0("Invalid split value specified.");
        ...
        if (splitVal == 0)
          {
            ...
            // @A1 = <col-name>
            // @A2 = <search-str>
            str_sprintf(buf, "CASE WHEN CHAR_LENGTH(@A2) = 0 then LEFT(@A1,1) WHEN INSTR(@A1, @A2) > 0 THEN SUBSTRING(@A1 FOR (INSTR (@A1, @A2) - 1)) ELSE @A1 END");
          }
        else
          {
	...
	if (bindWA->errStatus()) 
	  return this;
        ...
        if ((child(0)->getValueId().getType().isComposite()) ||
            (child(1)->getValueId().getType().isComposite()))
          {
            if ((CmpCommon::getDefault(TRAF_COMPOSITE_DATATYPE_SUPPORT) == DF_OFF) &&
              ...
              {
                *CmpCommon::diags() << DgSqlCode(-3242)
                                    << DgString0("Cannot concatenate composite datatypes.");
                bindWA->setErrStatus();
                return this;
              }

            parseTree = new(bindWA->wHeap()) 
              CompositeConcat(child(0), child(1));
          }
        else
          {
            parseTree = new(bindWA->wHeap()) 
              Concat(child(0), child(1));
          }
...
ItemExpr* ItmOlapFirstLastValue::bindNode(BindWA * bindWA)
{
      ...
      //frames for now
      if( !isFrameStartUnboundedPreceding() ||
           ! isFrameEndUnboundedFollowing() )
        {
...
ItmOlapPivotGroup::ItmOlapPivotGroup(OperatorTypeEnum itemType, ItemExpr * column,
                                                       ItemExpr * orderBy, ItemExpr * delim, const Lng32 maxLen)
    : ItmSeqOlapFunction(itemType, column)
    , delimiter_(delim)
    , maxPivotLen_(maxLen)
{
...
bool ZZZBinderFunction::enforceDateOrTimestampDatatype(BindWA * bindWA, CollIndex childIndex, int operand)
{
  ...
  if (rewriteChild->getOperatorType() == ITM_DYN_PARAM)
    rewriteChild = new (bindWA->wHeap())Cast(rewriteChild, new(bindWA->wHeap())
                        SQLTimestamp(bindWA->wHeap(), FALSE, SQLTimestamp::DEFAULT_FRACTION_PRECISION));
  ...
  for (short i = 0; i < numEntries; i++)
    {
      ...
      if (ITM_CONSTANT == vid1.getItemExpr()->getOperatorType())
	...
	return NULL;
      ...
      if (ITM_CONSTANT == vid2.getItemExpr()->getOperatorType())
        {//convert NULL to empty string
...
ItemExpr *HbaseRowid::bindNode(BindWA *bindWA)
{
  ...
  rowidType_ = new (bindWA->wHeap())
                            SQLChar(bindWA->wHeap(), rowidLen_, FALSE);
...
ItemExpr *HbaseRowidRef::bindNode(BindWA *bindWA)
{
  ...
  if (NOT (getOperatorType() == ITM_HBASE_ROWID_REF))
    CMPASSERT(0);
  ...
  if (col() == NULL)
  {
    ...
    else
    {
      ...
      if (qn.getCatalogName().isNull() && qn.getSchemaName().isNull())
        maybeAlias = TRUE;
      ...
      {
          *CmpCommon::diags() << DgSqlCode(-4332)
            << DgTableName(corrName_->getQualifiedNameAsString())
            << DgString0(fmtdList);
          bindWA->setErrStatus();
          return NULL;
      }
    ...
    if (bindWA->getCurrentScope()->context()->inHavingClause() OR (
        bindWA->getCurrentScope()->context()->inSelectList() AND
        bindWA->getCurrentScope()->getRETDesc()->isGrouped() AND
        (NOT bindWA->getCurrentScope()->context()->inGroupByOrdinal())
       ))
        if (NOT colDesc->isRowidGrouped() AND
            NOT (bindWA->getCurrentScope()->context()->inAggregate() ))
        {
            *CmpCommon::diags() << DgSqlCode(-4005)
                                        << DgColumnName("ROWID");
            bindWA->setErrStatus();
    ...
    if (bindWA->getCurrentScope()->context()->inGroupByClause() AND
         !bindWA->getCurrentScope()->context()->inUDFunction())
  ...
  {
    *CmpCommon::diags() << DgSqlCode(-3242)
                        << DgString0("ROWID currently cannot be used on a non-Trafodion table");
    bindWA->setErrStatus();
    return NULL;
  }
...
ItemExpr *RowNumFunc::bindNode(BindWA *bindWA)
{
  ...
  if( CmpCommon::context()->isConnectByDual() ) {
    ...
    ColRefName *pColRefName = new(bindWA->wHeap()) 
      ColRefName(colNameStr,  bindWA->wHeap());
  ...
  while (currScope)
  {
    ...
    if ((currContext->inSubquery() ||
        ...
        currContext->inHavingClause() ||
        currContext->inUnion() ||
        currContext->inJoin()       ) && CmpCommon::context()->isConnectByDual() == FALSE)
      {
 	*CmpCommon::diags() << DgSqlCode(-4311)
			    << DgString0("ROWNUM");
	bindWA->setErrStatus();
...
ItemExpr *ItmSysConnectByPathFunc::bindNode(BindWA *bindWA)
{
  ...
  {
    *CmpCommon::diags() << DgSqlCode(-4466)
                        << DgString0("SYS_CONNECTBY_PATH must used with Connect By operator");
    bindWA->setErrStatus();
    return this;
  }
