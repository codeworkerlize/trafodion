...
LogsInfo::LogsInfo(DeltaDefinition &deltaDef, MVInfoForDML *mvInfo, BindWA *bindWA)
    : deltaDef_(deltaDef),
      mvInfo_(mvInfo),
      bindWA_(bindWA),
  ...
  CorrName *tableName = new (bindWA->wHeap()) CorrName(*deltaDef_.getTableName());

  currentMvIudlog_ = new (bindWA->wHeap()) MvIudLog(*tableName, bindWA, REL_SCAN);
  ...
  baseTableDirectionVector_ = BinderUtils::buildClusteringIndexDirectionVector(getBaseNaTable(), bindWA->wHeap());
...
LogsInfo::~LogsInfo() { delete currentMvIudlog_; }

//////////////////////////////////////////////////////////////////////////////
// Create a CorrName to the range log from the subject table name.
//////////////////////////////////////////////////////////////////////////////
CorrName *LogsInfo::calcRangeLogName(const CorrName &theTable, CollHeap *heap) const {
  return ChangesTable::buildChangesTableCorrName(theTable.getQualifiedNameObj(), COMMV_RANGE_LOG_SUFFIX,
                                                 ExtendedQualName::RANGE_LOG_TABLE, heap);
}
...
MvRefreshBuilder::MvRefreshBuilder(const CorrName &mvName, MVInfoForDML *mvInfo, Refresh *refreshNode, BindWA *bindWA)
...
RelExpr *MvRefreshBuilder::buildReadIudLogBlock() {
  ...
  Scan *scanNode = iudLog.buildScan(ChangesTable::ALL_ROWS);
  ...
  if (scanNode->getTableName().getCorrNameAsString() == "") {
    topNode = new (heap_) RenameTable(TRUE, topNode, getLogsInfo().getIudLogTableName());
  }
...
ItemExpr *MvRefreshBuilder::buildSelectionListForScanOnIudLog() const {
  ...
  for (CollIndex i = 0; i < numberOfColumns; i++) {
    ...
    if (!strncmp(colName->getColName().data(), COMMV_CTRL_PREFIX, sizeof(COMMV_CTRL_PREFIX) - 1)) {
...
ItemExpr *MvRefreshBuilder::buildBaseTableColumnList(int specialFlags) const {
  ...
  const QualifiedName &tableName = getLogsInfo().getBaseTableName().getQualifiedNameObj();

  const MVUsedObjectInfo *usedObject = getMvInfo()->findUsedInfoForTable(tableName);
  CMPASSERT(usedObject != NULL);

  const NAColumnArray &baseCiCols = baseTable->getClusteringIndex()->getIndexKeyColumns();
  ...
  for (CollIndex i = 0; i < numberOfColumns; i++) {
    ...
    if ("SYSKEY" == colName->getColName() && (specialFlags & SP_USE_AT_SYSKEY)) {
      colRef = new (heap_) ColReference(new (heap_) ColRefName(COMMV_BASE_SYSKEY_COL));
    } else if ("SYSKEY" != colName->getColName()) {
      // If the column is used by the MV, add it to the list
      if (usedObject->isUsedColumn(i) || baseCiCols.contains((NAColumn *)naCol)) {
...
ItemExpr *MvRefreshBuilder::buildSelectionPredicateForScanOnIudLog() const {
  if (!useUnionBakeboneToMergeEpochs()) {
    ...
    return MvIudLog::buildLogEpochPredicate(&getLogsInfo().getDeltaDefinition(), heap_);
  }
  return NULL;
}
...
RelExpr *MvRefreshBuilder::buildLogsScanningBlock(const QualifiedName &baseTable) {
  ...
  if (bindWA_->errStatus() || logsInfo_ == NULL) return NULL;
  ...
  topNode = new (heap_) RenameTable(TRUE, topNode, getLogsInfo().getBaseTableName());
...
Union *MvRefreshBuilder::buildUnionBetweenRangeAndIudBlocks(RelExpr *scanIUDLogBlock,
  ...
  Union *unionNode =
      new (heap_) Union(scanIUDLogBlock, scanRangeLogBlock, NULL, NULL, REL_UNION, CmpCommon::statementHeap(), TRUE);

  if (needAlternateCIorder()) {
    const CorrName tableNameCorr("");
    ItemExpr *baseCI =
        BinderUtils::buildClusteringIndexVector(getLogsInfo().getBaseNaTable(), heap_, &tableNameCorr, SP_USE_AT_SYSKEY,
...
//----------------------------------------------------------------------------
// Excluded from coverage test - used only with range logging.
NABoolean MvRefreshBuilder::needAlternateCIorder() const { return isGroupByAprefixOfTableCKeyColumns(); }
...
ItemExpr *MvRefreshBuilder::buildAlternateCIorder(ItemExpr *ciColumns, const CorrName &tableNameCorr) const {
  ...
  ItemExpr *rangeEndCI =
      BinderUtils::buildClusteringIndexVector(getLogsInfo().getBaseNaTable(), heap_, &tableNameCorr, SP_USE_AT_SYSKEY,
                                              getLogsInfo().getBaseTableDirectionVector(), &endRangePrefix);

  ItemExpr *rangeIdCol = new (heap_) ColReference(new (heap_) ColRefName(COMMV_RANGE_ID_COL, tableNameCorr, heap_));
...
ItemExpr *MvRefreshBuilder::constructOpExpression() const {
  ItemExpr *valueOneCond =
      new (heap_) BiLogic(ITM_OR,
                          BinderUtils::buildPredOnCol(ITM_EQUAL,  // (ROW_TYPE = 0)
                                                      COMMV_OPTYPE_COL, ComMvRowType_Insert, heap_),
                          BinderUtils::buildPredOnCol(ITM_EQUAL,  // (ROW_TYPE = 2)
                                                      COMMV_OPTYPE_COL, ComMvRowType_InsertOfUpdate, heap_));

  // If (valueOneCond) then 1 else -1
  ItemExpr *opExpr = new (heap_)
      Case(NULL, new (heap_) IfThenElse(valueOneCond, new (heap_) SystemLiteral(1), new (heap_) SystemLiteral(-1)));

  RenameCol *opColumn = new (heap_)
      RenameCol(opExpr, new (heap_) ColRefName(MavBuilder::getVirtualOpColumnName(), getLogsInfo().getBaseTableName()));
...
ItemExpr *MvRefreshBuilder::buildBeginRangeVector() const {
  ...
  return BinderUtils::buildClusteringIndexVector(getLogsInfo().getBaseNaTable(), heap_,
                                                 &getLogsInfo().getRangeLogTableName(), SP_USE_AT_SYSKEY,
                                                 getLogsInfo().getBaseTableDirectionVector(), &beginRangePrefix);
}
...
ItemExpr *MvRefreshBuilder::buildRangeLogJoinPredicate() const {
  ItemExpr *baseCI =  // The clustering index vector of the base table.
      BinderUtils::buildClusteringIndexVector(getLogsInfo().getBaseNaTable(), heap_, &getLogsInfo().getBaseTableName(),
                                              SP_USE_AT_SYSKEY);
  ...
  ItemExpr *rangeType = new (heap_) ColReference(new (heap_) ColRefName(COMMV_RANGE_TYPE_COL));
...
//      END
// Excluded from coverage test - used only with range logging.
ItemExpr *MvRefreshBuilder::buildRangeLogJoinPredicateWithCols(ItemExpr *rangeType, ItemExpr *baseCI,
                                                               ItemExpr *beginRangeCI, ItemExpr *endRangeCI) const {
  IntegerList *directionVector = getLogsInfo().getBaseTableDirectionVector();

  // a)
  BiRelat *predA = new (heap_) BiRelat(ITM_GREATER_EQ, baseCI, beginRangeCI);

  predA->setDirectionVector(directionVector);

  // b)
  BiRelat *predB = new (heap_) BiRelat(ITM_LESS_EQ, baseCI->copyTree(heap_), endRangeCI);
  ...
  ItemExpr *predC2 = new (heap_) BiRelat(ITM_NOT_EQUAL, baseCI->copyTree(heap_), beginRangeCI->copyTree(heap_));

  ItemExpr *predC1 = new (heap_) BiRelat(ITM_NOT_EQUAL, baseCI->copyTree(heap_), endRangeCI->copyTree(heap_));

  ItemExpr *predC0 = new (heap_) BiLogic(ITM_AND, predC1->copyTree(heap_), predC2->copyTree(heap_));

  // Now combine the four parts of condiion c) together:
  ItemExpr *predC = new (heap_)
      Case(rangeType, new (heap_) IfThenElse(
                          new (heap_) SystemLiteral(ComMvRangeClosedBothBounds),  // If 0
                          predC0,
                          new (heap_) IfThenElse(
                              new (heap_) SystemLiteral(ComMvRangeClosedLowerBound),  // If 1
                              predC1,
                              new (heap_) IfThenElse(new (heap_) SystemLiteral(ComMvRangeClosedUpperBound),  // If 2
                                                     predC2, predC3))));
...
RelRoot *MvRefreshBuilder::buildRootOverIUDLog(RelExpr *topNode) const {
  ItemExpr *opExpr = new (heap_)
      ColReference(new (heap_) ColRefName(MavBuilder::getVirtualOpColumnName(), getLogsInfo().getIudLogTableName()));

  RelRoot *root = buildRootWithUniformSelectList(topNode, opExpr, &getLogsInfo().getIudLogTableName(), FALSE);

  if (!getLogsInfo().getDeltaDefinition().useRangeLog()) return root;
  ...
  ItemExpr *beginRangeCI = BinderUtils::buildClusteringIndexVector(
      getLogsInfo().getBaseNaTable(), heap_, &getLogsInfo().getIudLogTableName(), SP_USE_AT_SYSKEY | SP_USE_NULL, NULL,
      &beginRangePrefix);
  // The "end range" vector of the range log.
  ItemExpr *endRangeCI = BinderUtils::buildClusteringIndexVector(getLogsInfo().getBaseNaTable(), heap_,
                                                                 &getLogsInfo().getIudLogTableName(),
                                                                 SP_USE_AT_SYSKEY | SP_USE_NULL, NULL, &endRangePrefix);

  ItemExpr *rangeIdCol = new (heap_) RenameCol(new (heap_) SystemLiteral(), new (heap_) ColRefName(COMMV_RANGE_ID_COL));
...
RelRoot *MvRefreshBuilder::buildRootOverRangeBlock(RelExpr *topNode) const {
  ...
  RelRoot *root = buildRootWithUniformSelectList(topNode, opExpr, &getLogsInfo().getBaseTableName(), TRUE);

  const NAString beginRangePrefix(COMMV_BEGINRANGE_PREFIX);
  // The "begin range" vector of the range log.
  ItemExpr *beginRangeCI = BinderUtils::buildClusteringIndexVector(getLogsInfo().getBaseNaTable(), heap_,
                                                                   &getLogsInfo().getRangeLogTableName(),
                                                                   SP_USE_AT_SYSKEY, NULL, &beginRangePrefix);

  const NAString endRangePrefix(COMMV_ENDRANGE_PREFIX);
  // The "end range" vector of the range log.
  ItemExpr *endRangeCI = BinderUtils::buildClusteringIndexVector(getLogsInfo().getBaseNaTable(), heap_,
                                                                 &getLogsInfo().getRangeLogTableName(),
                                                                 SP_USE_AT_SYSKEY, NULL, &endRangePrefix);

  ItemExpr *rangeIdCol =
      new (heap_) ColReference(new (heap_) ColRefName(COMMV_RANGE_ID_COL, getLogsInfo().getRangeLogTableName(), heap_));
...
RelRoot *MvRefreshBuilder::buildRootWithUniformSelectList(RelExpr *topNode, ItemExpr *opExpr,
                                                          const CorrName *nameOverride, NABoolean forRange) const {
  // all of the table column names used by the MV
  ItemExpr *leftRootSelectList = buildBaseTableColumnList(SP_USE_AT_SYSKEY);

  RenameCol *opColumn =
      new (heap_) RenameCol(opExpr, new (heap_) ColRefName(MavBuilder::getVirtualOpColumnName(), *nameOverride));
...
RelExpr *MvRefreshBuilder::buildReadRangesBlock() const {
  ...
  if (coveredRows != 0) {
    joinNode->addCardConstraint((Cardinality)0, (Cardinality)coveredRows);
  }
...
RelExpr *MvRefreshBuilder::buildReadBaseTable() const {
  ...
  if (getLogsInfo().getBaseNaTable()->getClusteringIndex()->hasSyskey()) {
    ItemExpr *syskeyCol = new (heap_)
        RenameCol(new (heap_) ColReference(new (heap_) ColRefName("SYSKEY", getLogsInfo().getBaseTableName())),
                  new (heap_) ColRefName(COMMV_SYSKEY_COL, getLogsInfo().getBaseTableName()));
...
//----------------------------------------------------------------------------
// Excluded from coverage test - used only with range logging.
RelExpr *MvRefreshBuilder::buildJoinBaseTableWithRangeLog(RelExpr *scanRangeLog, RelExpr *scanBaseTable) const {
  ...
  joinNode = new (heap_) Join(scanRangeLog, scanBaseTable, REL_JOIN, joinPredicate);
...
RelExpr *MvRefreshBuilder::buildReadRangeLogBlock() const {
  ...
  if (selectionPred) topNode->addSelPredTree(selectionPred);

  if (useUnionBakeboneToMergeEpochs()) {
    fixScanCardinality(topNode, 1 / getNumOfScansInUnionBackbone(),
                       (Cardinality)getLogsInfo().getDeltaDefinition().getNumOfRanges());

    topNode = buildUnionBakeboneToMergeEpochs(topNode);
  } else {
    // Fix the cardinality of the range log by the number of ranges.
    fixScanCardinality(topNode, 1, (Cardinality)getLogsInfo().getDeltaDefinition().getNumOfRanges());
  }
...
void MvRefreshBuilder::fixScanCardinality(RelExpr *node, double cardinalityFactor, Cardinality cardinality) const {
  node->getInliningInfo().BuildForceCardinalityInfo(cardinalityFactor, cardinality, heap_);
}
...
RelExpr *MvRefreshBuilder::buildUnionBakeboneToMergeEpochs(RelExpr *topNode) const {
  ...
  for (int epoch = deltaDef.getBeginEpoch() + 1; epoch <= deltaDef.getEndEpoch(); epoch++) {
    ...
    unionTree = new (heap_) Union(unionTree, newScan, NULL, NULL, REL_UNION, CmpCommon::statementHeap(), TRUE);
  }

  // Put the predicate on topNode (the first Scan node) after we are
  // done duplicating it.
  epochPred = BinderUtils::buildPredOnCol(ITM_EQUAL, COMMV_EPOCH_COL, deltaDef.getBeginEpoch(), heap_);
...
NABoolean MvRefreshBuilder::useUnionBakeboneToMergeEpochs() const { return FALSE; }
...
MultiDeltaRefreshMatrix *MvRefreshBuilder::prepareProductMatrix(NABoolean supportPhases, NABoolean checkOnlyInserts) {
  ...
  MVJoinGraph *joinGraph = getMvInfo()->initJoinGraph(bindWA_, getDeltaDefList(), checkOnlyInserts);
  ...
  // induction base.
  MultiDeltaRefreshMatrix *productMatrix = new (heap_) MultiDeltaRefreshMatrix(maxNumOfRows, joinGraph, heap_);
  productMatrix->setPhasesSupported(supportPhases);

  if (productMatrix->isDuplicatesOptimized() && avoidMultiDeltaOptimization()) {
  ...
  if (productMatrix->isTooManyDeltasError()) {
    // 12319 ZZZZZ 99999 ADVANCED MAJOR DBADMIN Too many deltas: Materialized View $0~TableName cannot be incrementally
    // refreshed because $0~Int0 of its base tables have changed.
  ...
  if (!productMatrix->isLastPhase()) {
    if (getMvInfo()->isMinMaxUsed()) {  // Multi-phase refresh not allowed on min/max MAVs. Must Recompute.
...
#ifndef NDEBUG
  if (CmpCommon::getDefault(MV_DUMP_DEBUG_INFO) == DF_ON) productMatrix->print();
#endif
...
RelRoot *MvRefreshBuilder::prepareProductFromMatrix(const MultiDeltaRefreshMatrix &productMatrix,
  ...
  for (int i = 0; i < productMatrix.getRowLength(); i++) {
    ...
    if (currentRow->isScanOnDelta(i)) {
      ...
      const QualifiedName &tableName = usedObject->getObjectName();

      // Add the table name to the switch list.
      RelExpr *scanLogBlock = buildLogsScanningBlock(tableName);
      if (bindWA_->errStatus() || scanLogBlock == NULL) return NULL;
...
RelExpr *MvRecomputeBuilder::buildRefreshTree() {
  ...
  if (noDeleteOnRecompute_) return insertToMvSubTree;
  ...
  if (!noDeleteOnRecompute_) {
    ...
    Union *unionNode = new (heap_) Union(leftSubTree, topNode, NULL, NULL, REL_UNION, CmpCommon::statementHeap(), TRUE);
...
RelExpr *MvRecomputeBuilder::buildInsertToMvSubTree() const {
  ...
  Insert *insertMVnode = new (heap_) Insert(getMvCorrName(), NULL, REL_UNARY_INSERT, mvSelectTree);
...
RelExpr *MvRecomputeBuilder::buildDeleteFromMvSubTree() const {
  ...
  Delete *deleteMVnode = new (heap_) Delete(getMvCorrName(), NULL, REL_UNARY_DELETE, scanMVnode);
...
MavBuilder::MavBuilder(const CorrName &mvName, MVInfoForDML *mvInfo, Refresh *refreshNode,
  ...
  canSkipMinMax_ = !getMvInfo()->isMinMaxUsed() || getDeltaDefList()->areAllDeltasInsertOnly();
}
...
RelExpr *MavBuilder::buildTheMavRefreshTree(RelExpr *mvSelectTree, DeltaDefinition *deltaDef) {
  RelExpr *pDCB = buildDeltaCalculationBlock(mvSelectTree);

  // For Debugging Only !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
#ifndef NDEBUG
  if (getenv("DEBUG_DCB")) return pDCB;
#endif
  ...
  Join *tsjNode = new (heap_) Join(pDCB, pDPB, isProjectingMvDelta() ? REL_LEFT_TSJ : REL_TSJ);
...
RelExpr *MavBuilder::buildDeltaCalculationBlock(RelExpr *mvSelectTree) {
  ...
  const CorrName mavCorrName(getMvCorrName().getQualifiedNameObj(), heap_, getSysMavName());
  ...
  RelExpr *deltaRootNode = rootBuilder.buildDeltaCalculationRootNodes(mvSelectTree, canSkipMinMax_, wasFullDE);
  ...
  // Build the LOJ
  ItemExpr *gbSelectionPred = buildGroupByColumnsPredicate(getSysDeltaName(), getSysMavName());
  RelExpr *lojNode = new (heap_) Join(deltaRootNode, rightRoot, REL_LEFT_JOIN, gbSelectionPred);
...
//////////////////////////////////////////////////////////////////////////////
ItemExpr *MavBuilder::buildGroupByColumnsPredicate(const NAString &table1Name, const NAString &table2Name,
  ...
  for (CollIndex i = 0; i < gbColPositionList.entries(); i++) {
    ...
    ItemExpr *colRef1 = new (heap_) ColReference(new (heap_) ColRefName(colName, table1CorrName));

    ItemExpr *colRef2 = NULL;
    if (!ignoreAlias) {
      // Normal case, use the (aliased) column name.
      colRef2 = new (heap_) ColReference(new (heap_) ColRefName(colName, table2CorrName));
    } else {
      // IgnoreAlias: use the original column name.
      colRef2 = new (heap_) ColReference(new (heap_) ColRefName(*origColName, table2CorrName));
    }

    BiRelat *equalPred = new (heap_) BiRelat(ITM_EQUAL, colRef1, colRef2);
    ...
    if (predicate == NULL) {
      ...
      predicate = new (heap_) BiLogic(ITM_AND, predicate, equalPred);
    }
  }
...
RelExpr *MavBuilder::buildDeltaProcessingBlock(DeltaDefinition *deltaDef) {
  ...
  if (!canSkipMinMax_) topNode = buildDPBMinMaxIfCondition(topNode);

  // If we cannot skip the MAV Delete node - build it.
  if (!canSkipDeleteMav) topNode = buildDPBDelete(topNode);

  // If we cannot skip the MAV Insert node - build it.
  if (!canSkipInsertMav) topNode = buildDPBInsert(deltaDef, topNode);
...
//////////////////////////////////////////////////////////////////////////////
RelExpr *MavBuilder::buildDPBUpdate(const NAString &tableForSet, const NAString &tableForWhere) const {
  ...
  for (CollIndex i = 0; i < getMvInfo()->getMVColumns().entries(); i++) {
    ...
    if (currentCol->getColType() != COM_MVCOL_AGGREGATE) continue;

    // Build the Assign expression: SET <colName> = <tableForSet>.<colName>
    const NAString &colName = currentCol->getColName();
    ItemExpr *col = new (heap_) Assign(new (heap_) ColReference(new (heap_) ColRefName(colName)),
                                       new (heap_) ColReference(new (heap_) ColRefName(colName, assignCorrName)));
  ...
  Update *mavUpdate = new (heap_) Update(getMvCorrName(), NULL, REL_UNARY_UPDATE, mavScanNode, updatedCols);
  // No need to log MAV changes if this is a pipelined refresh.
  if (isProjectingMvDelta()) mavUpdate->setNoLogOperation(TRUE);
  // Affected rows should not be counted.
...
RelExpr *MavBuilder::buildDPBDelete(RelExpr *topNode) {
  ...
  Delete *mavDelete = new (heap_) Delete(getMvCorrName(), NULL, REL_UNARY_DELETE, mavScanNode);
  // No need to log MAV changes if this is a pipelined refresh.
  if (isProjectingMvDelta()) mavDelete->setNoLogOperation(TRUE);
  // Make sure we have no outputs.
  // Affected rows should not be counted.
  ...
  ItemExpr *delCondition = BinderUtils::buildPredOnCol(ITM_EQUAL, getVirtualGopColumnName(), GOP_DELETE, heap_);

  // The IF is implemented as a conditional Union.
  topNode = new (heap_) Union(deleteRoot, topNode, NULL, delCondition, REL_UNION, CmpCommon::statementHeap(), TRUE);
  return topNode;
}
...
RelExpr *MavBuilder::buildDPBInsert(DeltaDefinition *deltaDef, RelExpr *topNode) const {
  ...
  ItemExpr *insCondition = BinderUtils::buildPredOnCol(ITM_EQUAL, getVirtualGopColumnName(), GOP_INSERT, heap_);

  // The IF is implemented as a conditional Union.
  topNode = new (heap_) Union(insertRoot, topNode, NULL, insCondition, REL_UNION, CmpCommon::statementHeap(), TRUE);
  return topNode;
}
...
RelExpr *MavBuilder::buildDPBInsertNodes(const NAString &sourceTable) const {
  ...
  for (CollIndex i = 0; i < getMvInfo()->getMVColumns().entries(); i++) {
    ...
    if (colName == "SYSKEY") continue;

    const CorrName &sourceCorrName = (mavCol->getColType() == COM_MVCOL_GROUPBY ? groupbyCorrName : aggregateCorrName);

    // Build a reference to the corresponding SYS_DELTA column.
    col = new (heap_) ColReference(new (heap_) ColRefName(colName, sourceCorrName));
  ...
  Insert *mavInsert = new (heap_) Insert(getMvCorrName(), NULL, REL_UNARY_INSERT, tupleNode);
  // No need to log MAV changes if this is a pipelined refresh.
  if (isProjectingMvDelta()) mavInsert->setNoLogOperation(TRUE);
  // Affected rows should not be counted.
...
RelExpr *MavBuilder::buildDPBMinMaxIfCondition(const RelExpr *updateNode) const {
  ...
  ItemExpr *minMaxRecomputeCondition = new (heap_) BiLogic(
      ITM_OR,
  ...
  Union *ifNode = new (heap_) Union(pMinMaxRecomputationBlock, (RelExpr *)updateNode, NULL, minMaxRecomputeCondition,
...
RelExpr *MavBuilder::buildMinMaxRecomputationBlock() const { return createSignal(); }
...
NABoolean MavBuilder::useUnionBakeboneToMergeEpochs() const {
  ...
  if (Refresh::SINGLEDELTA != getRefreshNode()->GetRefreshType()) return FALSE;

  if (deltaDef->getEndEpoch() - deltaDef->getBeginEpoch() > MAX_EPOCH_FOR_UNION_BACKBONE) return FALSE;

  if (deltaDef->getBeginEpoch() == deltaDef->getEndEpoch()) return FALSE;
  ...
  if (!isGroupByAprefixOfTableCKeyColumns()) return FALSE;
...
int MavBuilder::isGroupByAprefixOfTableCKeyColumns() const {
  ...
  for (CollIndex i = 0; i < indexColumns.entries(); i++, counter++) {
    ...
    if (currentCol == NULL || currentCol->getColType() != COM_MVCOL_GROUPBY) break;
  }
...
RelExpr *MinMaxOptimizedMavBuilder::buildMinMaxRecomputationBlock() const {
  ...
  RelExpr *renameNode = new (heap_) RenameTable(pMvSelectTree, getMinMaxMavName());
  ...
  Union *ifNode = new (heap_) Union(pUpdateNode, pInsertNode, NULL, ifCondition, REL_UNION, heap_, TRUE);
...
void MinMaxOptimizedMavBuilder::fixGroupingColumns(RelRoot *pRoot) const {
  ...
  while (pGroupByNode != NULL && pGroupByNode->getOperatorType() != REL_GROUPBY) pGroupByNode = pGroupByNode->child(0);

  if (pGroupByNode == NULL) return;  // there are no grouping columns: "select max(a) from t1;"

  CMPASSERT(REL_GROUPBY == pGroupByNode->getOperatorType());
  ItemExpr *pGroupingCols = ((GroupByAgg *)pGroupByNode)->removeGroupExprTree();
...
void MinMaxOptimizedMavBuilder::removeGroupingColsFromSelectList(RelRoot *pRoot) const {
  ...
  for (int i = selectList.entries() - 1; i >= 0; i--) {
    ...
    MVColumnInfo *mvColInfo = getMvInfo()->getMVColumns().getMvColInfoByName(mvColName);
    CMPASSERT(mvColInfo != NULL);

    // Is it used as a group-by column?
    if (mvColInfo->getColType() != COM_MVCOL_GROUPBY) continue;
...
RelExpr *MultiDeltaMavBuilder::buildRefreshTree() {
  ...
  if (productMatrix == NULL) return NULL;
  ...
  if (bindWA_->errStatus()) return NULL;
...
RelExpr *MultiDeltaMavBuilder::buildRawDeltaCalculationTree(const MultiDeltaRefreshMatrix &productMatrix) {
  ...
  while (parentOfTopJoin->getOperatorType() != REL_GROUPBY && topJoin->getOperatorType() != REL_JOIN) {
  ...
  for (int rowNumber = firstRowForThisPhase; rowNumber < firstRowForThisPhase + numOfRowsForThisPhase; rowNumber++) {
    ...
    RelRoot *product = prepareProductFromMatrix(productMatrix, rawJoinProduct, rowNumber, isLastRow);
    ...
    if (topNode == NULL)
      ...
      topNode = new (heap_) Union(topNode, product, NULL, NULL, REL_UNION, CmpCommon::statementHeap(), TRUE);
  }

  // 4. If duplicates can be optimized (when all deltas are insert-only)
  //    Set the top Union node to a DISTINCT Union.
  //    topNode may not be a Union in case of an RI optimization.
  if (isDuplicatesOptimized_ && topNode->getOperatorType() == REL_UNION) {
    Union *topUnion = (Union *)topNode;
    topUnion->setDistinctUnion();

    topNode = new (heap_) GroupByAgg(new (heap_) RelRoot(topUnion), REL_GROUPBY,
    ...
    if (mvSelectTop == parentOfTopJoin) {
      ...
      topNode = new (heap_) GroupByAgg(new (heap_) RelRoot(topNode), REL_GROUPBY, NULL);
    }
  }
...
void MultiDeltaMavBuilder::prepareRetdescForUnion(RETDesc *retDesc, NABoolean isSignPlus) {
  ...
  for (int i = userColumns->entries() - 1; i >= 0; i--) {
    ...
    if (colExpr->getOperatorType() == ITM_BASECOLUMN) {
      ...
      ItemExpr *iudLogCol = ((ValueIdUnion *)colExpr)->getLeftSource().getItemExpr();
      if (iudLogCol->getOperatorType() == ITM_BASECOLUMN) baseCol = (BaseColumn *)iudLogCol;
    }

    if (baseCol == NULL) continue;

    const QualifiedName &tableName = baseCol->getTableDesc()->getCorrNameObj().getQualifiedNameObj();
    ...
    if (baseCol->getTableDesc()->getCorrNameObj().getSpecialType() != ExtendedQualName::NORMAL_TABLE)
    ...
    if (!isColumnUsed) retDesc->delColumn(bindWA_, colRefName, USER_COLUMN);
  }
  ...
  if (!isSignPlus) {
    ...
    newOpExpr = new (heap_) BiArith(ITM_TIMES, newOpExpr, new (heap_) SystemLiteral(-1));
  }
...
NABoolean MultiDeltaMavBuilder::collectOpExpressions(RETDesc *retDesc, const ColumnDesc *columnDesc,
                                                     ItemExpr *&newOpExpr) {
  ColRefName opName(getVirtualOpColumnName());
...
NABoolean MultiDeltaMavBuilder::avoidMultiDeltaOptimization() {
  ...
  for (CollIndex i = 0; i < deltaDefList->entries(); i++) {
    ...
    if (indexCols.getColumn("SYSKEY") != NULL) return TRUE;
  }
...
NABoolean MultiDeltaMavBuilder::isTableExpressionPresent(RelExpr *currentNode) {
  ...
  while ((!groupByFound) && !(REL_ROOT == currentNode->getOperatorType() &&
  ...
  while (REL_JOIN != currentNode->getOperatorType() && REL_RENAME_TABLE != currentNode->getOperatorType()) {
    currentNode = currentNode->child(0);
    CMPASSERT(currentNode != NULL);
  }
...
PipelinedMavBuilder::PipelinedMavBuilder(const CorrName &mvName, MVInfoForDML *mvInfo, Refresh *refreshNode,
                                         NABoolean isPipelined, const QualifiedName *pipeliningSource, BindWA *bindWA)
  ...
  DeltaDefinition *noDeDeltaDef = new (heap_) DeltaDefinition((QualifiedName *)pipeliningSource);

  DeltaDefinitionPtrList *newDeltaDefList = new (heap_) DeltaDefinitionPtrList(heap_);
...
RelExpr *PipelinedMavBuilder::buildJoinWithTupleList(RelExpr *topNode) {
  ...
  ItemExpr *tupleRow1 =
      new (heap_) Convert(new (heap_) ItemList(new (heap_) SystemLiteral(GOP_INSERT), new (heap_) SystemLiteral(1)));
  ItemExpr *tupleRow2 =
      new (heap_) Convert(new (heap_) ItemList(new (heap_) SystemLiteral(GOP_DELETE), new (heap_) SystemLiteral(-1)));
  ItemExpr *tupleRow3 =
      new (heap_) Convert(new (heap_) ItemList(new (heap_) SystemLiteral(GOP_UPDATE), new (heap_) SystemLiteral(-1)));
  ItemExpr *tupleRow4 =
      new (heap_) Convert(new (heap_) ItemList(new (heap_) SystemLiteral(GOP_UPDATE), new (heap_) SystemLiteral(1)));

  // Construct the tuple expression from the four rows
  ItemExpr *tupleExpr =
      new (heap_) ItemList(tupleRow1, new (heap_) ItemList(tupleRow2, new (heap_) ItemList(tupleRow3, tupleRow4)));
  ...
  ItemExpr *renExpr =
      new (heap_) ItemList(new (heap_) RenameCol(NULL, col1RefName), new (heap_) RenameCol(NULL, col2RefName));

  rightTop = new (heap_) RenameTable(TRUE, rightTop, gopTableName, renExpr, heap_);

  // Construct the join predicate (@GOP = GOP_TABLE.GroupOp)
  ColReference *leftGopCol = new (heap_) ColReference(new (heap_) ColRefName(getVirtualGopColumnName(), heap_));
  ColReference *rightGopCol = new (heap_) ColReference(new (heap_) ColRefName(gopCol1Name, gopTableName, heap_));
  ItemExpr *joinPred = new (heap_) BiRelat(ITM_EQUAL, leftGopCol, rightGopCol);
...
RelRoot *PipelinedMavBuilder::buildRenameToLog(RelExpr *topNode) {
  ...
  const NAColumnArray &baseColumns = bindWA_->getNATable(baseCorrName)->getNAColumnArray();

  // Rename GOP_TABLE.OP to <log_name>.@OP
  ColRefName *gopTableGopColName = new (heap_) ColRefName(gopCol1Name, gopCorrName, heap_);
  ColRefName *gopTableOpColName = new (heap_) ColRefName(gopCol2Name, gopCorrName, heap_);

  // Start the select list with the OP column. The other columns are added
  // in the loop below.
  ItemExpr *selectList = new (heap_)
      RenameCol(new (heap_) ColReference(gopTableOpColName), new (heap_) ColRefName(getVirtualOpColumnName(), heap_));
  ...
  ItemExpr *opCondition = BinderUtils::buildPredOnCol(ITM_EQUAL, gopCol2Name, -1, heap_);

  ItemExpr *gopCondition = BinderUtils::buildPredOnCol(ITM_EQUAL, gopCol1Name, GOP_INSERT, heap_);

  for (CollIndex i = 0; i < baseColumns.entries(); i++) {
    const NAString &newColName = baseColumns[i]->getColName();

    // Skip the SYSKEY column.
    if (newColName == "SYSKEY") continue;
    ...
    ItemExpr *colExpr = new (heap_) Case(
        NULL, new (heap_) IfThenElse(opCondition->copyTree(heap_), mavColRef->copyTree(heap_),
                                     new (heap_) IfThenElse(gopCondition->copyTree(heap_), deltaColRef->copyTree(heap_),
                                                            calcColRef->copyTree(heap_))));

    // Give it a name: <base table name>.col
    RenameCol *renCol = new (heap_) RenameCol(colExpr, new (heap_) ColRefName(newColName, baseCorrName, heap_));
  ...
  RelRoot *topRoot = new (heap_) RelRoot(topNode, REL_ROOT, selectList);
...
void MultiDeltaRefreshMatrixRow::display() const { print(); }
#endif
...
MultiDeltaRefreshMatrix::MultiDeltaRefreshMatrix(int maxNumOfRows, MVJoinGraph *joinGraph, CollHeap *heap)
  ...
  // =========================================================
  maxJoinSizeForSinglePhase_ = CmpCommon::getDefaultLong(MV_REFRESH_MDELTA_MAX_JOIN_SIZE_FOR_SINGLE_PHASE);
  minJoinSizeForSingleProductPerPhase_ =
      CmpCommon::getDefaultLong(MV_REFRESH_MDELTA_MIN_JOIN_SIZE_FOR_SINGLE_PRODUCT_PHASE);
  phaseSizeForMidRange_ = CmpCommon::getDefaultLong(MV_REFRESH_MDELTA_PHASE_SIZE_FOR_MID_RANGE);
  maxDeltasThreshold_ = CmpCommon::getDefaultLong(MV_REFRESH_MDELTA_MAX_DELTAS_THRESHOLD);
...
MultiDeltaRefreshMatrix::~MultiDeltaRefreshMatrix() {
  ...
  for (CollIndex i = 0; i < theMatrix_.entries(); i++) delete theMatrix_[i];
}
...
void MultiDeltaRefreshMatrix::addTable(NABoolean readDelta) {
  ...
  MultiDeltaRefreshMatrixRow *lastRow = new (heap_) MultiDeltaRefreshMatrixRow(currentRowLength_, maxRowLength_, heap_);
...
void MultiDeltaRefreshMatrix::setThisPhase(int phase) {
  ...
  isLastPhase_ = (firstRowForThisPhase_ + numOfRowsForThisPhase_ >= numOfRows_);
}
...
void MultiDeltaRefreshMatrix::calculatePhases() {
  if ((isPhasesSupported() == FALSE) && (numOfRows_ > maxDeltasThreshold_) && (isDuplicatesOptimized_ == FALSE)) {
    TooManyDeltasError_ = TRUE;
    return;
  }

  // Small joins execute in a single phase.
  if ((isPhasesSupported() == FALSE) || (currentRowLength_ <= maxJoinSizeForSinglePhase_) ||
...
void MultiDeltaRefreshMatrix::print(FILE *ofd, const char *indent, const char *title) const {
  ...
  fprintf(ofd, "For phase %d, doing rows %d to %d.\n", thisPhase_, firstRowForThisPhase_,
          firstRowForThisPhase_ + numOfRowsForThisPhase_ - 1);
  fprintf(ofd, "\n");
}

void MultiDeltaRefreshMatrix::display() const { print(); }
#endif
