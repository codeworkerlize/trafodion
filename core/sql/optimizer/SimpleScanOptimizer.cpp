...
//
const ValueIdSet &SimpleFileScanOptimizer::getPartitioningKeyPredicates() const {
...
// mid-block.
//
CostScalar SimpleFileScanOptimizer::getNumBlocksForRows(const CostScalar &numRows) const {
...
//
CostScalar SimpleFileScanOptimizer::getResultSetCardinalityPerScan() const {
...
//
CostScalar SimpleFileScanOptimizer::getRowsInDP2Buffer() const {
  ...
  return (dp2MessageBufferSizeInKb / getRecordSizeInKb()).getFloor();
}

SimpleFileScanOptimizer::SimpleFileScanOptimizer(const FileScan &assocFileScan, const CostScalar &resultSetCardinality,
                                                 const Context &myContext, const ValueIdSet &externalInputs)
    : ScanOptimizer(assocFileScan, resultSetCardinality, myContext, externalInputs),
      ...
      estimatedRecordsPerBlock_(assocFileScan.getIndexDesc()->getEstimatedRecordsPerBlock()),
...
//
Cost *SimpleFileScanOptimizer::optimize(SearchKey *&searchKeyPtr /* out */
                                        ,
...
//
NABoolean SimpleFileScanOptimizer::isLogicalSubPartitioned() const {
  const LogPhysPartitioningFunction *logPhysPartFunc =
      getContext().getPlan()->getPhysicalProperty()->getPartitioningFunction()->castToLogPhysPartitioningFunction();
...
//
SearchKey *SimpleFileScanOptimizer::constructSearchKey() {
  ...
  SearchKey *searchKeyPtr = new (CmpCommon::statementHeap())
      SearchKey(getIndexDesc()->getIndexKey(), getIndexDesc()->getOrderOfKeyValues(), getExternalInputs(),
                isForwardScan(), exePreds, *curDisjuncts, nonKeyColumnSet, getIndexDesc());
...
// local datamembers
//
void SimpleFileScanOptimizer::computeSingleSubsetSize() {
  singleSubsetSize_ = 0;
...
// of ScanOptimizer.
//
void SimpleFileScanOptimizer::computeNumberOfBlocksToReadPerAccess(FileScanBasicCost *basicCostPtr) {
...
//
Cost *SimpleFileScanOptimizer::computeCostForSingleSubset() {
  ...
  if (!sharedCost) {
  ...
  } else {  // Determine whether probes order matches the scan's clustering
    ...
    if (multiProbeScan) {
      ...
      if ((ippForMe != NULL) AND SimpleFileScanOptimizer::ordersMatch(
              ippForMe, getIndexDesc(), &(getIndexDesc()->getOrderOfKeyValues()),
  ...
  if (CURRSTMT_OPTDEFAULTS->incorporateSkewInCosting()) {
    ...
    CostScalar multiplicativeFactor = (probesAtBusyStream / probesPerScan).minCsOne();
  ...
  CostScalar ocbAdjFactor = (ActiveSchemaDB()->getDefaults()).getAsDouble(OCB_COST_ADJSTFCTR);
  ...
  if ((multiProbeScan) && (CmpCommon::getDefault(COMP_BOOL_53) == DF_ON) &&
      (getContext().getReqdPhysicalProperty()->getOcbEnabledCostingRequirement()) && (dp2CostInfo != NULL) &&
...
//
NABoolean SimpleFileScanOptimizer::getSharedCostObject(FileScanBasicCost *&basicCostPtr) {
  ...
  if (sharedCostFound) {
    ...
    // object.
    //
    if (firstRow.getCPUTime() > csZero AND lastRow.getCPUTime() > csZero AND CURRSTMT_OPTDEFAULTS->reuseBasicCost()) {
      return TRUE;
    } else
...
//
CostScalar SimpleFileScanOptimizer::estimateNumberOfSeeksPerScan(const CostScalar &seqKB) const {
...
//
CostScalar SimpleFileScanOptimizer::estimateSeqKBReadPerScan() const {
...
//
void SimpleFileScanOptimizer::computeCostVectors(FileScanBasicCost *basicCostPtr) const {
  SimpleCostVector &firstRow = basicCostPtr->getFRBasicCostSingleSubset();
  ...
  const CostScalar rowsPerScan((getSingleSubsetSize() / getEstNumActivePartitionsAtRuntime()).getCeiling());
  ...
  //
  estimateCPUInstructions(seeksPerScan, rowsPerScanFR, rowsPerScan, selectedRowsPerScanFR, selectedRowsPerScan, seqKBFR,
                          seqKBLR, cpuInstructionsFR, cpuInstructionsLR);
...
//
void SimpleFileScanOptimizer::estimateKB(CostScalar &seqKBFR, CostScalar &seqKBLR) const {
  ...
  //
  CostScalar dp2BufferSize(CostPrimitives::getBasicCostFactor(DP2_MESSAGE_BUFFER_SIZE));
...
//
void SimpleFileScanOptimizer::estimateCPUInstructions(
    const CostScalar &seeksPerScan, const CostScalar &rowsPerScanFR, const CostScalar &rowsPerScanLR,
    const CostScalar &selectedRowsPerScanFR, const CostScalar &selectedRowsPerScanLR, const CostScalar &seqKBFR,
    const CostScalar &seqKBLR, CostScalar &cpuInstructionsFR, CostScalar &cpuInstructionsLR) const {
  ...
  //
  cpuInstructionsFR = instrPerReqst
                      //+ (instrPerSeek * seeksPerScan)
                      + (instrPerKB * seqKBFR) + (instrPerRow * selectedRowsPerScanFR) +
                      (instrPerExePredPerRow * rowsPerScanFR);

  // Last Row CPU instructions.
  //
  cpuInstructionsLR = instrPerReqst
                      //+ (instrPerSeek * seeksPerScan)
                      + (instrPerKB * seqKBLR) + (instrPerRow * selectedRowsPerScanLR) +
...
//
CostScalar SimpleFileScanOptimizer::estimateCPUPerReqstInstrs() const {
  ...
  */
  ...
  CostScalar entriesPerIndexBlock =
      (index->getBlockSizeInKb() * 1024 * avrgSlack / (8 + index->getKeyLength())).getCeiling();
  CostScalar instrPerReqst = (  // number of comparisons per index blocks
      1.44 * log(entriesPerIndexBlock.getValue()) *
      // comparison cost, will need a special CQD later
      index->getKeyLength() *
      CostPrimitives::getBasicCostFactor(CPUCOST_COMPARE_COMPLEX_DATA_TYPE_PER_BYTE)
...
// DP2 to the EXEINDP2.
//
CostScalar SimpleFileScanOptimizer::estimateCPUPerRowInstrs() const {
  ...
  //
  CostScalar instrPerRowMovedOutOfTheCache(CostPrimitives::getBasicCostFactor(CPUCOST_COPY_ROW_OVERHEAD) +
...
//   - VEG Predicates
//
CostScalar SimpleFileScanOptimizer::estimateCPUExePredPerRowInstrs() const {
  ...
  //
  if (fsoToUse < 2) {
    return (exePreds.entries() * CostPrimitives::getBasicCostFactor(CPUCOST_PREDICATE_COMPARISON));
  }
  ...
  //
  return (CostPrimitives::getBasicCostFactor(CPUCOST_PREDICATE_COMPARISON) * numOps) / 2;
...
// is just a default value.
//
CostScalar SimpleFileScanOptimizer::estimateCPUPerKBInstrs() const {
...
//
CostScalar SimpleFileScanOptimizer::estimateIdleTime() const {
  // Default cost to open subsequent partitions.
  //
  CostScalar idleTime(CostPrimitives::getBasicCostFactor(CPUCOST_SUBSET_OPEN_AFTER_FIRST));
...
// probe scan.
//
CostScalar SimpleFileScanOptimizer::getRepeatCount() const {
  CostScalar repeatCount =
      getContext().getPlan()->getPhysicalProperty()->getDP2CostThatDependsOnSPP()->getRepeatCountForOperatorsInDP2();

  if ((repeatCount > csOne) OR(getContext().getInputLogProp()->getColStats().entries() > 0)) {
...
CostScalar SimpleFileScanOptimizer::getProbeCacheCostAdjFactor() const {
  ...
  if (ubound > 1.0) return probeCacheCostAdj;
  ...
  if (dupSuccProbes == totalProbes) dupSuccProbes = dupSuccProbes - csOne;
  ...
  else  // Random order case
  {
    ...
    if (ippForMe != NULL) {
      ...
      if (ch0PartFunc == NULL) ch0PartFunc = ippForMe->getNjOuterOrderPartFunc();

      ValueIdSet ch0PartKey;
      if (ch0PartFunc != NULL) ch0PartKey = ch0PartFunc->getPartitioningKey();
      ...
      if (NOT(myCharInputs.isEmpty() OR ch0PartKey.isEmpty()))
        ...
        if (myCharInputs.contains(ch0PartKey)) safeToDivide = TRUE;
    }
    ...
    if (safeToDivide) uniqProbes = uniqProbes.value() / countOfStreams;
...
NABoolean SimpleFileScanOptimizer::isProbeCacheApplicable() const {
  ...
  if (getSearchKey()->isUnique() && (duplicateSuccProbes_ > 0) && (CmpCommon::getDefault(NESTED_JOIN_CACHE) == DF_ON))
    return TRUE;
  else
    return FALSE;
}
...
//
void SimpleFileScanOptimizer::categorizeMultiProbes(NABoolean *isAnIndexJoin) {
  ...
  if (getFileScan().isHbaseTable() && CmpCommon::getDefault(NCM_HBASE_COSTING) == DF_ON)
    probes_ = (getRepeatCount() * getEstNumActivePartitionsAtRuntimeForHbaseRegions()).minCsOne();
  else if (getFileScan().isHiveOrcTable() && CmpCommon::getDefault(NCM_ORC_COSTING) == DF_ON) {
  ...
  categorizeProbes(successfulProbes_  // out
                   ...
                   ,
                   probes_, singleSubsetPreds, outerHistograms, FALSE  // this is not MDAM!
                   ,
                   &dataRows_, isAnIndexJoin);
  ...
  blksPerSuccProbe_ = (rowsPerSuccessfulProbe / getEstimatedRecordsPerBlock()).getCeiling();
  ...
  if ((ippForMe != NULL) AND SimpleFileScanOptimizer::ordersMatch(
          ippForMe, getIndexDesc(), &(getIndexDesc()->getOrderOfKeyValues()),
          getRelExpr().getGroupAttr()->getCharacteristicInputs(), partiallyInOrderOK, probesForceSynchronousAccess)) {
    setInOrderProbesFlag(TRUE);
    setProbesForceSynchronousAccessFlag(probesForceSynchronousAccess);
  } else if ((ippForMe != NULL) AND(ippForMe->getAssumeSortedForCosting())
                 AND(!(ippForMe->getExplodedOcbJoinForCosting()))
...
// -----------------------------------------------------------------------
void SimpleFileScanOptimizer::categorizeProbes(CostScalar &successfulProbes /* out */
                                               ,
                                               ...
                                               ,
                                               const CostScalar &probes, const ValueIdSet &preds,
                                               const Histograms &outerHistograms, const NABoolean isMDAM,
                                               CostScalar *dataRows, NABoolean *isAnIndexJoin) const {
  ...
  if (outerHistograms.isEmpty()) {
    ...
    for (ValueId predId = preds.init(); preds.next(predId); preds.advance(predId)) {
    ...
    joinedHistograms.applyPredicatesWhenMultipleProbes(preds, *(getContext().getInputLogProp()), inputValues, isMDAM,
                                                       NULL, NULL, isAnIndexJoin, REL_SCAN);
    ...
    for (ValueId predId = preds.init(); preds.next(predId); preds.advance(predId)) {
      const VEGPredicate *vegPred = (VEGPredicate *)predId.getItemExpr();
    ...
    for (CollIndex i = 0; i < outerHistograms.entries(); i++) {
      if (columnsOfVegPreds.contains(outerHistograms[i].getColumn())
              OR columnsOfVegPreds.contains(outerHistograms[i].getVEGColumn())
                  OR inputValues.contains(outerHistograms[i].getVEGColumn())) {
    ...
    uniqueSuccProbes = MINOF(((successfulProbes / numOuterProbes) * uniqueEntryCountOfProbes), successfulProbes);

    uniqueFailedProbes = MINOF(((failedProbes / numOuterProbes) * uniqueEntryCountOfProbes), failedProbes);
...
void SimpleFileScanOptimizer::estimateIndexBlockUsageMultiProbeScan() {
  ...
  CostScalar innerBlocksPerScan =
      (getEffectiveTotalRowCount() / getEstimatedRecordsPerBlock() / getNumActivePartitions()).getCeiling();

  // Assume 8 bytes per key overhead (pointer to low level block)
  // we'' use ceiling because block with K key would have K+1 pointers)
  CostScalar entriesPerIndexBlock =
      (index->getBlockSizeInKb() * 1024 / (8 + index->getKeyLength()) * avrgSlack).getCeiling();
  ...
  if (entriesPerIndexBlock.isGreaterThanOne()) {
    ...
    CostScalar currentLevelIndexBlocks =
        MINOF(leafProbesPerScan, innerBlocksPerScan / entriesPerIndexBlock).getCeiling();

    CostScalar totalIndexBlocks = currentLevelIndexBlocks;
    while (--curLevel > 0) {
      currentLevelIndexBlocks = (currentLevelIndexBlocks / entriesPerIndexBlock).getCeiling();
...
// object are populated.
//
void SimpleFileScanOptimizer::computeCostVectorsMultiProbes(FileScanBasicCost *basicCostPtr) {
  ...
  if (getPartialOrderProbesFlag()) {
    ...
    effectiveTotalRowCount_ = (effectiveTotalRowCount_ / keyPrefixUEC_).getCeiling();
  ...
  const CostScalar rowsPerScan((getDataRows() / getNumActivePartitions()).getCeiling());
  ...
  // KB number of CPU Instructions) for First and Last row accessed.
  //
  estimateCPUInstructionsMultiProbe(seeksLR, rowsPerScanFR, rowsPerScan, selectedRowsPerScanFR, selectedRowsPerScan,
                                    seqKBFR, seqKBLR, cpuInstructionsFR, cpuInstructionsLR);
  ...
  const CostScalar probesPerScan((getProbes() / getNumActivePartitions()).getCeiling());
...
// cpuInstructionsLR - Number of CPU instructions to access the Last row.
//
void SimpleFileScanOptimizer::estimateCPUInstructionsMultiProbe(
    const CostScalar &seeksPerScan, const CostScalar &rowsPerScanFR, const CostScalar &rowsPerScanLR,
    const CostScalar &selectedRowsPerScanFR, const CostScalar &selectedRowsPerScanLR, const CostScalar &seqKBFR,
    const CostScalar &seqKBLR, CostScalar &cpuInstructionsFR, CostScalar &cpuInstructionsLR) const {
  const CostScalar probesPerScan((getProbes() / getNumActivePartitions()).getCeiling());

  const CostScalar succProbesPerScan((getSuccessfulProbes() / getNumActivePartitions()).getCeiling());
  ...
  if (probesPerScan > csZero) {
    ...
    if (succProbesPerScan > csZero) {
      instrPerReqst +=
          (succProbesPerScan * (cpuForBTreeComparisons + CostPrimitives::getBasicCostFactor(CPUCOST_DATARQST_OVHD)));
    }

    const CostScalar failedRequestsPerScan = probesPerScan - succProbesPerScan;

    if (failedRequestsPerScan > csZero) {
      instrPerReqst += (failedRequestsPerScan *
...
// a partition
//
CostScalar SimpleFileScanOptimizer::estimateNumberOfSeeksPerScanMultiProbe() const {
  // -------------------------------------------------------------
  // Compute the blocks in the inner table:
  // -------------------------------------------------------------
  const CostScalar totalRowsInInnerTable = getEffectiveTotalRowCount();

  CostScalar innerBlocksUpperBound(totalRowsInInnerTable / getEstimatedRecordsPerBlock());
  ...
  if (getInOrderProbesFlag()) {
    ...
    const CostScalar uniqueSuccProbes = MINOF(uniqueProbes, getSuccessfulProbes() - getDuplicateSuccProbes());

    // The begin blocks denote the starting data
    // block for the subset that each succ. probe generates
    //
    const CostScalar beginBlocksLowerBound = MINOF(uniqueSuccProbes, innerBlocksUpperBound);
    ...
    if (indexBlocksLowerBound >= cacheSize) {
      CostScalar cacheMissProbabilityForIndexBlock = (indexBlocksLowerBound - cacheSize) / indexBlocksLowerBound;
      extraIndexBlocks = (getProbes() / getNumActivePartitions() - cacheSize) * cacheMissProbabilityForIndexBlock;
      // each probe will cause a seek at data block level
      seekComputedWithDp2ReadAhead = getProbes();
    } else {
      cacheSize -= indexBlocksLowerBound;

      const CostScalar cacheSizeForAll(cacheSize * getNumActiveDP2Volumes());

      if (getSearchKey()->isUnique()) {
        // In this case for each probe no more than one data
        // block is accessed. Readahead won't be used.
        CostScalar estBlocksAccessedByUniqueProbes = MINOF(uniqueProbes, innerBlocksUpperBound);
        if (estBlocksAccessedByUniqueProbes <= cacheSizeForAll) {
          seekComputedWithDp2ReadAhead = estBlocksAccessedByUniqueProbes;
        } else {
          // Assume random unique probes never cause read ahead
          CostScalar cacheMissProbabilityForUniqueProbe =
              ((estBlocksAccessedByUniqueProbes - cacheSizeForAll) / estBlocksAccessedByUniqueProbes).minCsZero();

          seekComputedWithDp2ReadAhead = cacheSizeForAll +                  // First probes to fill the cache
        ...
        CostScalar uniqueFailedProbes = (uniqueProbes - uniqueSuccProbes).minCsZero();

        CostScalar innerBlocksAccessedBySuccProbes = (uniqueSuccProbes * getBlksPerSuccProbe());

        CostScalar innerBlocksAccessedByAllProbes = innerBlocksAccessedBySuccProbes + uniqueFailedProbes;

        if (innerBlocksAccessedByAllProbes <= cacheSizeForAll) {
          // inner table completely fits in cache
          const CostScalar maxDp2ReadInKB(defs.getAsULong(DP2_MAX_READ_PER_ACCESS_IN_KB));

          const CostScalar maxDp2ReadInBlocks(maxDp2ReadInKB / getBlockSizeInKb());

          if (innerBlocksAccessedByAllProbes < maxDp2ReadInKB * getNumActivePartitions()) {
            // first read will read the whole table/index
            // partition
            seekComputedWithDp2ReadAhead = getNumActivePartitions();
          } else {
            /*
            ...
            */
            ...
            seekComputedWithDp2ReadAhead = (beginBlocksLowerBound - maxDp2ReadInBlocks).minCsOne();
          }
        } else {
          if (getBlksPerSuccProbe() > cacheSizeForAll) {
            ...
            CostScalar partOfSuccProbes =
                cacheSizeForAll * innerBlocksAccessedBySuccProbes / innerBlocksAccessedByAllProbes;

            // Part of cache to keep failed probe blocks
            CostScalar partOfFailedProbes = (cacheSizeForAll - partOfSuccProbes).minCsZero();

            // part of succ probes that fits in its part of cache
            CostScalar cacheHitProbForSucc = (partOfSuccProbes / innerBlocksAccessedBySuccProbes).maxCsOne();
            ...
            if (uniqueFailedProbes.isGreaterThanZero())
              cacheHitProbForFail = (partOfFailedProbes / uniqueFailedProbes).maxCsOne();

            // assume that each random unique probe causes 1 seek
            seekComputedWithDp2ReadAhead = getUniqueProbes() +
                                           getDuplicateSuccProbes() * (csOne - cacheHitProbForSucc) +
                                           (getFailedProbes() - uniqueFailedProbes) * (csOne - cacheHitProbForFail);
          }
        }
      }
    }
  ...
  return (seekComputedWithDp2ReadAhead / getNumActivePartitions() + indexBlocksLowerBound + extraIndexBlocks)
      .getCeiling();
...
//
void SimpleFileScanOptimizer::estimateEffTotalRowCount(CostScalar &realRowCount, CostScalar &effRowCount) const {
  // CR 10-010822-4815
  ...
  IndexDescHistograms innerHistograms(*getIndexDesc(), getIndexDesc()->getIndexKey().entries());
  ...
  NABoolean isORC = getIndexDesc()->getPrimaryTableDesc()->getNATable()->isORC();

  NABoolean isParquet = getIndexDesc()->getPrimaryTableDesc()->getNATable()->isParquet();

  NABoolean isAvro = getIndexDesc()->getPrimaryTableDesc()->getNATable()->isAvro();

  if (!isORC && !isParquet && !isAvro) {
    NodeMap *nodeMapPtr =
        (NodeMap *)(getContext().getPlan()->getPhysicalProperty()->getPartitioningFunction()->getNodeMap());

    CostScalar actPart = getEstNumActivePartitionsAtRuntimeForHbaseRegions();

    if (CmpCommon::getDefault(NCM_HBASE_COSTING) == DF_OFF) actPart = getNumActivePartitions();

    effRowCount = MINOF(effRowCount, realRowCount * actPart / nodeMapPtr->getNumEntries());
  }
...
// seqKBLR - The number of KBytes to be accessed to produce the last row.
//
void SimpleFileScanOptimizer::estimateKBMultiProbe(CostScalar &seqKBFR, CostScalar &seqKBLR) const {
  // total number of blocks to be read sequentially In order to
  // produce the last row, need to read all required blocks.
  //
...
// a partition (this includes index and data blocks)
//
CostScalar SimpleFileScanOptimizer::estimateSeqKBReadPerScanMultiProbe() const {
  ...
  const CostScalar innerBlocks(totalRowsInInnerTable / getEstimatedRecordsPerBlock());

  CostScalar innerBlocksPerScan(innerBlocks / getNumActivePartitions());
  ...
  //
  const CostScalar uniqBlocks((getSuccessfulProbes() - getDuplicateSuccProbes()) * getBlksPerSuccProbe());

  CostScalar uniqBlocksPerScan(uniqBlocks / getNumActivePartitions());
  ...
  if (indexBlocksLowerBound >= cacheSize) {
    if (getInOrderProbesFlag()) {
      ...
      if (getBlksPerSuccProbe() <= cacheSize) {
        if (CmpCommon::getDefault(COMP_BOOL_28) == DF_OFF) {
          seqBlocks = (getUniqueProbes() / getProbes()) *  // ratio of unique probes
                      maxBlocksToRead;
        } else {
    ...
    if ((innerBlocks <= totCacheSize)                       // Whole table fits in cache
        OR(uniqBlocks + getFailedProbes() <= totCacheSize)  // all blocks accessed
        OR
  ...
  // partitions.
  //
  seqBlocks = (seqBlocks / getNumActivePartitions() + indexBlocksLowerBound + extraIndexBlocks).getCeiling();
...
// -----------------------------------------------------------------------
NABoolean SimpleFileScanOptimizer::ordersMatch(const InputPhysicalProperty *ipp, const IndexDesc *indexDesc,
                                               const ValueIdList *innerOrder, const ValueIdSet &charInputs,
                                               NABoolean partiallyInOrderOK, NABoolean &probesForceSynchronousAccess) {
  ...
  if (fullyInOrder && (numInOrderCols == innerOrderProbeCols.entries()) && uncoveredCols.isEmpty()) {
    return TRUE;
    ...
    CostScalar rowcount = getRelExpr().getGroupAttr()->getResultCardinalityForEmptyInput();
    ...
    if (uecList) multiColUec = uecList->lookup(inOrderbaseColProbes);
...
NABoolean SimpleFileScanOptimizer::isLeadingKeyColCovered(const ValueIdList *keys) {
  ...
  for (x = 0; x < (*keys).entries() && !foundKey; x++) {
    ...
    if (isaConstant || firstkeyCol.isSaltColumn() || firstkeyCol.isDivisioningColumn())
