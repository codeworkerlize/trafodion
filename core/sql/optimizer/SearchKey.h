...
public:
  ...
  const ValueIdList & getRowidPredicate() const
                                               { return rowidPredicate_; }
  const ValueIdList & getRowidCols() const
                                               { return rowidCols_; }
  const ValueIdList & getRowidValues() const
                                               { return rowidValues_; }
  NABoolean hasRowidPredicate() const
                                               { return rowidPredicate_.entries() > 0;}
...
public:
  ...
  const SUBARRAY(HHDFSPartitionStats *) & getMask() 
                                                { return selectedPartitions_; }

  // get number of selected partitions. The minimal value is 1
  ...
  // make PushDown predicates for external storages (orc, parquet, avro)
  void  makeHiveExtPushdownPredicates(
...
private:
  ...
  HHDFSTableStats *hdfsTableStats_;
  ...
  // hivePartColList_ has VEGRefs, here are the equivalent
  // base columns
  ValueIdList hiveBasePartCols_;
  ...
  SUBARRAY(HHDFSPartitionStats *) selectedPartitions_;
...
public:
  ...
  const HHDFSPartitionStats *getPartStats()       { return l1PartStats_; }
...
private:
  ...
  const HHDFSPartitionStats *l1PartStats_;
