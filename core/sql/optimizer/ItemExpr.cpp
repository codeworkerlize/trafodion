...
// THREAD_P int ItemExpr::origOpTypeCounter_ = 0;

void ItemExpr::cleanupPerStatement() {
  if (CURRENTSTMT) CURRENTSTMT->setItemExprOrigOpTypeBeingBound(NO_OPERATOR_TYPE);
}
...
ExprValueId::ExprValueId(const ExprValueId &other) {
  ...
  if (exprId_ != NULL_VALUE_ID) exprPtr_ = NULL;
}
...
NABoolean ExprValueId::operator==(const ItemExpr *other) const { return getPtr() == other; }
NABoolean ExprValueId::operator==(const ValueId &other) const { return (getValueId() == other); }
...
void ExprValueId::convertToStandalone() { exprMode_ = STANDALONE; }

ItemExpr *ExprValueId::getPtr() const {
  ValueId temp = exprId_;
  exprPtr_->checkInvalidObject(this);
  if (temp == NULL_VALUE_ID) {
    if (exprPtr_ == NULL || exprPtr_->getValueId() == NULL_VALUE_ID) return exprPtr_;
...
ItemExpr::ItemExpr(OperatorTypeEnum otype, ItemExpr *child0, ItemExpr *child1)
    : ExprNode(otype),
  ...
  if (currStmt) {
    ...
    if (x != NO_OPERATOR_TYPE) origOpType_ = x;
  }
}
...
void ItemExpr::transformToRelExpr(NormWA &normWARef, ExprValueId &locationOfPointerToMe,
...
// If this predicate is of the form <column> <comparison op> <constant/param/host varb>
// (or its mirror image), return the <column> node. If it is an IN list (or equivalently
// an OR tree) of predicates of this form all on the same column, we return that as well.
// Otherwise we return NULL.
ItemExpr *ItemExpr::getColumnIfThisIsNiceComparison() {
  ...
  switch (op) {
    ...
    case ITM_GREATER_EQ: {
      ...
      OperatorTypeEnum op0 = child0->getOperatorType();
      ItemExpr *child1 = child(1)->castToItemExpr();
      OperatorTypeEnum op1 = child1->getOperatorType();
      if (((op0 == ITM_BASECOLUMN) || (op0 == ITM_INDEXCOLUMN)) &&
          ((op1 == ITM_CONSTANT) || (op1 == ITM_DYN_PARAM) || (op1 == ITM_HOSTVAR)))
        result = child0;
      else if (((op1 == ITM_BASECOLUMN) || (op1 == ITM_INDEXCOLUMN)) &&
               ((op0 == ITM_CONSTANT) || (op0 == ITM_DYN_PARAM) || (op0 == ITM_HOSTVAR)))
        result = child1;
      // else result remains NULL
      break;
    }
    case ITM_OR: {
      ItemExpr *niceChild0Column = child(0)->getColumnIfThisIsNiceComparison();
      if (niceChild0Column) {
        ItemExpr *niceChild1Column = child(1)->getColumnIfThisIsNiceComparison();
        if (niceChild1Column) {
          ValueId niceChild0ColId = niceChild0Column->getValueId();
          ValueId niceChild1ColId = niceChild1Column->getValueId();
          if (niceChild0ColId == niceChild1ColId) {
            CMPASSERT(niceChild0ColId != NULL_VALUE_ID);
            result = niceChild0Column;
          }
        }
      }
      // else result remains NULL
...
NABoolean ItemExpr::referencesTheGivenValue(const ValueId &vid, NABoolean doNotDigInsideVegRefs,
  ...
  if (getValueId() == vid || getInitVId() == vid) return TRUE;
  ...
  switch (getOperatorType()) {
    ...
    case ITM_VEG_REFERENCE: {
      if (doNotDigInsideVegRefs) return FALSE;
    ...
    default: {
      ...
      if ((NOT retVal)AND isASubquery()) {
        ...
        // the subquery
        //
        const ValueIdSet inputs = ((Subquery *)this)->getSubquery()->getGroupAttr()->getCharacteristicInputs();
...
NABoolean ItemExpr::referencesOneValueFrom(const ValueIdSet &vs) const {
  for (ValueId id = vs.init(); vs.next(id); vs.advance(id)) {
    ...
    if (!id.getItemExpr()->doesExprEvaluateToConstant(FALSE))
      if (referencesTheGivenValue(id)) return TRUE;
  }
  return FALSE;
...
ItemExpr *ItemExpr::simplifyBeforeConstFolding() {
  ...
  for (int i = 0; i < nc; i++) {
    ...
    if (cExpr) setChild(i, cExpr);
  }

  return this;
}

ConstValue *ItemExpr::getConstantInVEG() {
  switch (getOperatorType()) {
    case ITM_VEG_REFERENCE: {
...
ItemExpr *ItemExpr::createMirrorPred(ItemExpr *compColPtr, ItemExpr *compColExprPtr, const ValueIdSet &underlyingCols) {
  ...
  switch (getOperatorType()) {
    case ITM_VEG_PREDICATE: {
      ...
      if (VEGGroup.containsTheGivenValue(refColId) && underlyingCols.entries() == 1) {
        ...
        ItemExpr *compPred = new (CmpCommon::statementHeap())
            BiRelat(ITM_EQUAL, compColVEGRefPtr, (foldedExpr == NULL) ? newCompExpr.getItemExpr() : foldedExpr);
    ...
    case ITM_GREATER_OR_GE: {
      ...
      if (child(0)->referencesTheGivenValue(refColId) && underlyingCols.entries() == 1) {
        keyColRef = child(0);
        keyColExpr = child(1);
      } else if (child(1)->referencesTheGivenValue(refColId) && underlyingCols.entries() == 1) {
        keyColRef = child(1);
        keyColExpr = child(0);
        keyColOnLeft = FALSE;
      } else {  // XXX print warning for now.
#ifdef DEBUG
      ...
      ItemExpr *foldedExpr = newCompExpr.getItemExpr()->constFold();
      ItemExpr *mirroredExpr = (foldedExpr == NULL) ? newCompExpr.getItemExpr() : foldedExpr;

      ItemExpr *compPred;
      if (keyColOnLeft == TRUE)
        compPred =
            new (CmpCommon::statementHeap()) BiRelat(((BiRelat *)this)->getRelaxedComparisonOpType(), compColVEGRefPtr,
                                                     mirroredExpr  // newCompExpr.getItemExpr()
            );
      else
        compPred = new (CmpCommon::statementHeap()) BiRelat(((BiRelat *)this)->getRelaxedComparisonOpType(),
                                                            mirroredExpr,  // newCompExpr.getItemExpr(),
                                                            compColVEGRefPtr);
      compPred->synthTypeAndValueId(TRUE);
      return compPred;
...
NABoolean ItemExpr::doesExprEvaluateToConstant(NABoolean strict, NABoolean considerVEG) const {
  ...
  if (!nc) {
    ...
    switch (getOperatorType()) {
      ...
      case ITM_HOSTVAR: {
        ...
        if (hv->isSystemGeneratedOutputHV()) return TRUE;
      }
      ...
      case ITM_VEG_REFERENCE:
        if (considerVEG) {
          ...
          if (literal == NULL_VALUE_ID) return FALSE;
  ...
  // value
  if (getOperatorType() == ITM_RANDOMNUM) return FALSE;

  // composite array/row creation has multiple values and is not a constant
  if (getOperatorType() == ITM_COMPOSITE_CREATE) return FALSE;
  ...
  if (isAnAggregate() || isASequenceFunction() || (getOperatorType() == ITM_INSTANTIATE_NULL) ||
  ...
  if (!nc) {
    ...
    switch (getOperatorType()) {
      ...
      case ITM_VEG_PREDICATE: {
        ...
        if (predVEG->seenBefore()) return FALSE;
...
NABoolean ItemExpr::useStatsForPred() {
  ...
  if (myType == ITM_VEG_REFERENCE OR myType == ITM_BASECOLUMN OR myType == ITM_VEG_PREDICATE OR myType ==
      ITM_INDEXCOLUMN OR myType == ITM_VALUEIDUNION OR myType == ITM_CAST OR myType == ITM_INSTANTIATE_NULL OR myType ==
      ITM_UNPACKCOL OR myType == ITM_UPPER OR myType == ITM_LOWER OR myType == ITM_TRIM OR myType == ITM_CONVERT)
    ...
    if (startPosition->getOperatorType() == ITM_CONSTANT) {
      ...
      if (c->canGetExactNumericValue() && (c->getExactNumericValue() == 1)) return TRUE;
    }
    return FALSE;
  } else
    return FALSE;
...
void ItemExpr::getLeafValueIdsForCaseExpr(ValueIdSet &lv) {
  ...
  if (nc == 0) {
    ...
    if (getOperatorType() == ITM_IF_THEN_ELSE) {
      ...
      if (child(2)) child(2)->getLeafValueIdsForCaseExpr(lv);
...
ItemExpr *ItemExpr::getLeafValueIfUseStats(NABoolean digIntoInstantiateNull) {
  ...
  if (useStatsForPred()) {
    if (digIntoInstantiateNull == FALSE && expr->getOperatorType() == ITM_INSTANTIATE_NULL) return expr;
...
ItemExpr *ItemExpr::rewriteNotEquals(BindWA *bindWA) { return this; }

ItemExpr *ItemExpr::castToItemExpr() { return this; }  // ItemExpr::castToItemExpr()

const ItemExpr *ItemExpr::castToItemExpr() const { return this; }  // ItemExpr::castToItemExpr()
...
NABoolean ItemExpr::genericDuplicateMatch(const ItemExpr &other) const {
  ...
  if ((getValueId() == other.getValueId()) && (getValueId() != NULL_VALUE_ID)) return TRUE;
...
ItemExpr *ItemExpr::copyTopNode(ItemExpr *derivedNode, CollHeap *) {
  ItemExpr *result = NULL;
  ...
  result->userTextStr_ = userTextStr_;
...
ItemExpr *ItemExpr::copyTree(CollHeap *outHeap) {
  copyNodeFuncPtrT ptr = &ItemExpr::copyTopNode;
  return copyTreeImp(ptr, outHeap);
}

ItemExpr *ItemExpr::copyTreeNonLeaves(CollHeap *outHeap) {
  copyNodeFuncPtrT ptr = &ItemExpr::copyTopNodeNonleaf;
  return copyTreeImp(ptr, outHeap);
}

ItemExpr *ItemExpr::copyTreeImp(ItemExpr::copyNodeFuncPtrT funcPtr, CollHeap *outHeap) {
  ItemExpr *result = ((*this).*funcPtr)(NULL, outHeap);
  int arity = getArity();

  for (int i = 0; i < arity; i++) {
    if (child(i)) {
      result->child(i) = child(i)->copyTreeImp(funcPtr, outHeap);
    }
  }
  return result;
}
...
NABoolean ItemExpr::containsSubquery()  // virtual method
{
  ...
  for (int i = 0; i < arity; i++)
    if (child(i) && child(i)->containsSubquery()) return TRUE;
...
ItemExpr *ItemExpr::containsUDF()  // virtual method
{
  ...
  for (int i = 0; i < arity; i++)
    if (child(i) && child(i)->containsUDF()) return child(i)->containsUDF();
...
NABoolean ItemExpr::containsIsolatedUDFunction()  // virtual method
{
  ...
  for (int i = 0; i < arity; i++)
    if (child(i) && child(i)->containsIsolatedUDFunction()) return TRUE;
...
// virtual method
{
  ...
  for (int i = 0; i < arity; i++)
    if (child(i) && child(i)->containsValueIdProxySibling(siblings)) return TRUE;
...
ItemExpr *ItemExpr::transformMultiValuePredicate(  // virtual method
    NABoolean /*flattenSubqueries*/, ChildCondition /*tfmIf*/) {
  return NULL;
}

NABoolean ItemExpr::containsAnAggregate() const {
  for (int i = 0; i < getArity(); i++) {
    if (child(i)->containsAnAggregate()) return TRUE;
  }
  return FALSE;
}
...
void ItemExpr::findAllT(OperatorTypeEnum wantedType, Result &result, NABoolean visitVEGMembers,
  ...
  if (visitVEGMembers AND(myType == ITM_VEG_REFERENCE OR myType == ITM_VEG_PREDICATE)) {
    VEG *veg;
    ValueId vegM;
    ...
    if (!veg->seenBefore()) {
      veg->markAsSeenBefore();
      for (ValueId x = veg->getAllValues().init(); veg->getAllValues().next(x); veg->getAllValues().advance(x))

        x.getItemExpr()->findAllT(wantedType, result, visitVEGMembers, visitIndexColDefs);
  ...
  if (nc == 0) switch (myType) {
      case ITM_ROW_SUBQUERY:
        if (wantedType == ITM_DYN_PARAM) {
          // add all dynparam in the subquery
          for (ValueId x = ((Subquery *)this)->dynParamInStartWith_.init();
               ((Subquery *)this)->dynParamInStartWith_.next(x); ((Subquery *)this)->dynParamInStartWith_.advance(x)) {
            result.addMember(x.getItemExpr());
            /*
                         x.getItemExpr()->
                              findAllT(wantedType,
                                       result,
      ...
      case ITM_VALUEIDUNION: {
        ...
        for (int i = 0; i < (int)tempUnion->entries(); i++) {
          ...
          if (!tempUnion->getSource(i).getItemExpr()->referencesTheGivenValue(getValueId()))
            tempUnion->getSource(i).getItemExpr()->findAllT(wantedType, result, visitVEGMembers, visitIndexColDefs);
        }
        break;
      }
...
void ItemExpr::findAll(OperatorTypeEnum wantedType, ValueIdSet &result, NABoolean visitVEGMembers,
                       NABoolean visitIndexColDefs) {
  findAllT(wantedType, result, visitVEGMembers, visitIndexColDefs);
}

void ItemExpr::findAll(OperatorTypeEnum wantedType, ItemExprList &result, NABoolean visitVEGMembers,
                       NABoolean visitIndexColDefs) {
...
void ItemExpr::findEqualityCols(ValueIdSet &result) {
  ...
  if (myType == ITM_BASECOLUMN || myType == ITM_INDEXCOLUMN) result += myVid;
  ...
  if (myType == ITM_VEG_REFERENCE || myType == ITM_VEG_PREDICATE) {
    ...
    for (ValueId vid = veg->getAllValues().init(); veg->getAllValues().next(vid); veg->getAllValues().advance(vid)) {
      result += vid;
    }
  }
...
ItemExpr *ItemExpr::treeWalk(ItemTreeWalkFunc f, CollHeap *outHeap, enum ItemTreeWalkSeq sequence, void *context) {
  ...
  if (sequence == ITM_PREFIX_WALK) result = f(result, outHeap, context);
  ...
  for (int i = 0; i < arity; i++) {
    transformedChildren[i] = result->child(i)->treeWalk(f, outHeap, sequence, context);
    if (transformedChildren[i] != child(i)) needToCopy = TRUE;
  }
  ...
  if (needToCopy) {
    ...
    for (int j = 0; j < arity; j++) result->setChild(j, transformedChildren[j]);
  }

  if (sequence == ITM_POSTFIX_WALK) result = f(result, outHeap, context);

  return result;
}

ValueId ItemExpr::mapAndRewrite(ValueIdMap &map, NABoolean mapDownwards) {
  ValueId result;
  ...
  // mapped value id
  if (result != getValueId()) return result;

  return mapAndRewriteCommon(map, mapDownwards);
}  // ItemExpr::mapAndRewrite

ValueId Aggregate::mapAndRewrite(ValueIdMap &map, NABoolean mapDownwards) {
  // Need to make sure we map and rewrite the distinctId as well.
  if (isDistinct()) distinctId_ = distinctId_.getItemExpr()->mapAndRewrite(map, mapDownwards);

  return ItemExpr::mapAndRewrite(map, mapDownwards);
}  // Aggregate::mapAndRewrite

ValueId ItemExpr::mapAndRewriteWithIndx(ValueIdMap &map, CollIndex ind) {
  ValueId result;
...
ItemExpr *ItemExpr::foldConstants(ComDiagsArea *diagsArea, NABoolean newTypeSynthesis) {
  ...
  if (result != this) result->synthTypeAndValueId(newTypeSynthesis);

  return result;
}

ItemExpr *ItemExpr::applyInverseDistributivityLaw(OperatorTypeEnum backboneType, OperatorTypeEnum innerType) {
  ...
  if (getOperatorType() != backboneType) return this;

  // a default can switch this code off if required (should be needed
  // only if bugs are found in the code)
  if (CmpCommon::getDefault(FIND_COMMON_SUBEXPRS_IN_OR) != DF_ON) return this;
  ...
  for (/* c already initialized */; disjuncts.next(c); disjuncts.advance(c)) {
    ...
    if (commonSubexprs.isEmpty()) break;
  }
  ...
  if (numCommonSubexprs > 0) {
    ...
    for (ValueId c2 = disjuncts.init(); disjuncts.next(c2); disjuncts.advance(c2)) {
      ValueIdSet conjuncts;

      // one child of the backbone, eliminate common subexpressions
      // from it
      c2.getItemExpr()->convertToValueIdSet(conjuncts, NULL, innerType, FALSE);
    ...
    ItemExpr *commonSubexprsAsItem = commonSubexprsList.rebuildExprTree(innerType);
...
ItemExpr *ItemExpr::connect2(OperatorTypeEnum op, ItemExpr *op1, ItemExpr *op2) {
  switch (op) {
    case ITM_AND:
    case ITM_OR:
...
ItemExpr *ItemExpr::applyInverseDistributivityLawByColumn(OperatorTypeEnum backboneType, OperatorTypeEnum innerType) {
  // operators other than OR will require some more code, see CMPASSERTs
  CMPASSERT(backboneType == ITM_OR);

  // Because of a previous invocation of
  // ItemExpr::applyInverseDistributivity, we might see an AND node
  // here; in that case we need to drill down one level and look for
  // an OR child, and replace that child with the result computed
  // here.

  if ((backboneType == ITM_OR) && (getOperatorType() == ITM_AND)) {
    ItemExpr *newChild0 = child(0)->applyInverseDistributivityLawByColumn(backboneType, innerType);
    newChild0->synthTypeAndValueId();
    child(0) = newChild0;
    ItemExpr *newChild1 = child(1)->applyInverseDistributivityLawByColumn(backboneType, innerType);
    newChild1->synthTypeAndValueId();
    child(1) = newChild1;
    return this;
  }
  ...
  // understand the code better)
  ValueIdSet disjuncts;
  ValueIdSet firstDisjunctConjuncts;
  ValueIdSet commonColumns;
  std::map<ValueId, ItemExpr *> newExpressions;  // for accumulating disjunct tree by column
  ValueId c;

  // get a ValueIdSet with all the "disjuncts".
  convertToValueIdSet(disjuncts, NULL, backboneType, FALSE);

  // prime the common columns with all of the interesting columns
  // of the first disjunct
  // we are looking for conjuncts of the form
  // <column> <comparison op> <constant/parameter/host varb>, when
  // we find one, we remember the <column>
  disjuncts.getFirst(c);
  c.getItemExpr()->convertToValueIdSet(firstDisjunctConjuncts, NULL, innerType, FALSE);
  ...
  for (disjuncts.advance(c); disjuncts.next(c); disjuncts.advance(c)) {
    ValueIdSet conjuncts;
    c.getItemExpr()->convertToValueIdSet(conjuncts, NULL, innerType, FALSE);
    if (conjuncts.entries() > maxNumConjuncts) maxNumConjuncts = conjuncts.entries();

    // find the interesting columns in this conjunct and intersect them
    // with the commonColumns
    conjuncts.findCommonNiceColumnComparisons(commonColumns, newExpressions, backboneType, innerType);

    // stop if we have no common columns
    if (commonColumns.isEmpty()) return this;
  }

  if (maxNumConjuncts < 2)  // if no disjunct has an AND node
    return this;            // stop; continuing would simply duplicate what is there

  // If we get here, we know commonColumns is not empty. We've accumulated
  // a disjunct backbone of predicates for each of the common columns. Now
  // glue them back into the original tree. But first we replace the top node
  // of the original tree (so later we'll synthesize a new ValueId for it,
  // which prevents the old ValueId being in a descendent node of itself,
  // creating a loop).

  int arity = result->getArity();
  ItemExpr *result2 = result->copyTopNode(NULL, CmpCommon::statementHeap());
  for (int i = 0; i < arity; i++) result2->setChild(i, result->child(i));

  result = result2;  // replaced top node, with a new ValueId

  for (commonColumns.getFirst(c); commonColumns.next(c); commonColumns.advance(c)) {
    ItemExpr *newExpression = newExpressions[c];
    result = connect2(innerType, newExpression, result);
  }
...
void ItemExpr::synthTypeAndValueId2(NABoolean redriveTypeSynthesisFlag, NABoolean redriveChildTypeSynthesis) {
  ...
  if (ddesc == NULL OR(nc > 0 AND redriveTypeSynthesisFlag)) {
    ...
    if (getOperatorType() == ITM_CASE && ((Case *)this)->caseOperandWasNullable())
      t = t->synthesizeNullableType(CmpCommon::statementHeap());

    if (ddesc == NULL) {
      ddesc = new (CmpCommon::statementHeap()) DomainDesc(ActiveSchemaDB(), *t);
...
// ItemExpr::isCovered()
// -----------------------------------------------------------------------
NABoolean ItemExpr::isCovered(const ValueIdSet &newExternalInputs, const GroupAttributes &coveringGA,
    ...
    default: {
      ...
      for (index = 0; index < nc; index++) {
        coverFlag = coveringGA.covers(child(index)->getValueId(), newExternalInputs, referencedInputs, &coveredSubExpr,
                                      &unCoveredExpr);
        if (coverFlag) coveredSubExpr += ((ItemExpr *)child(index))->getValueId();
...
OrderComparison ItemExpr::sameOrder(ItemExpr *other, NABoolean askOther) {
  // first, check whether the item expressions are identical
  if (this == other) return SAME_ORDER;
  ...
  if (this == simpThis AND other == simpOther) return DIFFERENT_ORDER;
  ...
  if (result != DIFFERENT_ORDER) {
    ...
    // by comparing them
    if (order1 == INVERSE_ORDER) inversions++;
    if (order2 == INVERSE_ORDER) inversions++;
    if (result == INVERSE_ORDER) inversions++;
...
OrderComparison ItemExpr::sameOrder(VEGReference *other, NABoolean askOther) {
  ...
  for (ValueId x = equivValues.init(); equivValues.next(x) AND result == DIFFERENT_ORDER; equivValues.advance(x)) {
...
ItemExpr *ItemExpr::simplifyOrderExpr(OrderComparison *newOrder) {
  ...
  if (newOrder) *newOrder = SAME_ORDER;
  return this;
}
...
NABoolean ItemExpr::isAColumnReference() { return FALSE; }
...
void ItemExpr::print(FILE *f, const char *prefix, const char *suffix) const {
#ifndef NDEBUG
...
void ItemExpr::display() { display(cout); }

void ItemExpr::display(UnparseFormatEnum form) { display(cout, form); }

void ItemExpr::display(ostream &out, UnparseFormatEnum form) {
  NAString result;
  unparse(result, PARSER_PHASE, form /*default to USER_FORMAT_DELUXE*/);
  out << result.data() << endl;
}
...
//
void ItemExpr::computeKwdAndFlags(NAString &kwd, NABoolean &prefixFns, NABoolean &specialPrefixFns, PhaseEnum phase,
  ...
  if (operatorType == ITM_BASECOLUMN) {
    ...
    } else if ((form == COMPUTED_COLUMN_FORMAT) || (form == HIVE_MD_FORMAT) || (form == USTAT_EXPRESSION_FORMAT))
  ...
  } else if ((operatorType == ITM_CACHE_PARAM) && (form == QUERY_FORMAT || form == CONNECT_BY_FORMAT))
  ...
  switch (operatorType) {
    ...
    case ITM_MINUS:
      if (((BiArith *)this)->isDateMathFunction() &&
          (form == QUERY_FORMAT || form == COMPUTED_COLUMN_FORMAT || form == CONNECT_BY_FORMAT)) {
...
//       recursive ItemExpr::unparse() routine.
//
void ItemExpr::computeKwdAndPostfix(NAString &kwd, NAString &postfix, UnparseFormatEnum form) const {
  ...
  switch (operatorType) {
    case ITM_CAST: {
      if (form == QUERY_FORMAT || form == USTAT_EXPRESSION_FORMAT || form == COMPUTED_COLUMN_FORMAT ||
        ...
        if (!naType->supportsSQLnull()) postfix += " NOT NULL";
        postfix += ")";
      } else
        kwd += "(";
    } break;
...
void ItemExpr::unparse(NAString &result, PhaseEnum phase, UnparseFormatEnum form, TableDesc *tabId) const {
  ...
  if (operatorType != origOpType() && form == QUERY_FORMAT || form == CONNECT_BY_FORMAT ||
    ...
    switch (origOpType()) {
      ...
      case ITM_YEARWEEK:
      case ITM_YEARWEEKD: {
        ItemExpr *unboundExpr = ZZZBinderFunction::tryToUndoBindTransformation((ItemExpr *)this);
        if (unboundExpr) {
          // we were able to undo this transformation, unparse
          // the ZZZBinderFunction instead
          unboundExpr->unparse(result, phase, form, tabId);
          return;
        }
      } break;
  ...
  switch (arity) {
    ...
    case 2:
      if (form != FILE_FORMAT && !specialPrefixFns) {
        ...
        else if ((operatorType == ITM_TRIM) && ((form == QUERY_FORMAT) || (form == USTAT_EXPRESSION_FORMAT) ||
                                                (form == CONNECT_BY_FORMAT) || (form == COMPUTED_COLUMN_FORMAT))) {
          result += kwd;
          result += "(";
          child(1)->unparse(result, phase, form, tabId);
          result += ")";
        } else if ((operatorType == ITM_ITEM_LIST || operatorType == ITM_AND) && (form != MV_SHOWDDL_FORMAT) &&
                   (form != QUERY_FORMAT) && (form != CONNECT_BY_FORMAT) && (form != USTAT_EXPRESSION_FORMAT)) {
          if (child(0))
            child(0)->unparse(result, phase, form, tabId);
          else
            result += "NULL";

          if (operatorType != ITM_ITEM_LIST) result += " ";
          result += kwd;
          result += " ";
...
NABoolean ItemExpr::equatesToAConstant() const {
  if (getOperatorType() != ITM_EQUAL) return FALSE;

  ItemExpr *lhs = child(0);
  lhs = lhs->getLeafValueIfUseStats();

  if (lhs->getOperatorType() != ITM_VEG_REFERENCE) return FALSE;

  ItemExpr *rhs = child(1);
  rhs = rhs->getLeafValueIfUseStats();

  if (rhs->getOperatorType() == ITM_CACHE_PARAM) return TRUE;

  if (!rhs->doesExprEvaluateToConstant(TRUE, TRUE)) return FALSE;
...
NABoolean ItemExpr::containsColumn() {
  ...
  if (op == ITM_BASECOLUMN) return TRUE;

  // This is a leaf item other than a base column.
  if (op >= ITM_CONSTANT && op <= ITM_VEG_REFERENCE) return FALSE;
  ...
  for (int i = 0; i < getArity(); i++) {
    ...
    if (expr->containsColumn()) return TRUE;
  }
...
ItemExpr *ItemExpr::containsRightmost(const ItemExpr *ie) {
  ...
  if (!ie || this->getArity() == 0) return NULL;

  for (int arity = getArity() - 1; arity >= 0; arity--) {
    if (child(arity)) return (child(arity) == ie) ? this : child(arity)->containsRightmost(ie);
  }
  return NULL;
}
...
NABoolean ItemExpr::hasBaseEquivalence(ItemExpr *other) {
  ...
  if (getOperatorType() != other->getOperatorType() || getArity() != other->getArity()) return FALSE;
  ...
  for (int i = 0; i < getArity(); i++) {
    ...
    if (!chld->hasBaseEquivalence(other->child(i)) || !chld->hasEquivalentProperties(other->child(i))) return FALSE;
  }
  return TRUE;
...
ItemExpr *ItemExpr::changeDefaultOrderToDesc() {
  if (getOperatorType() == ITM_ITEM_LIST) {
    ...
    return this;
  } else if (getOperatorType() == ITM_INVERSE) {
...
ValueId ItemExpr::removeInverseFromExprTree(NABoolean &invExists) {
  if (getOperatorType() == ITM_ITEM_LIST) {
    for (int i = 0; i < getArity(); i++) child(i) = child(i)->removeInverseFromExprTree(invExists);
    return getValueId();
  } else if (getOperatorType() == ITM_INVERSE)
    invExists = TRUE;
...
// -----------------------------------------------------------------------
ItemExpr *ItemExpr::removeInverseFromExprTree(NABoolean &invExists, const NABoolean onlyDouble) {
  if (getOperatorType() == ITM_ITEM_LIST) {
    for (int i = 0; i < getArity(); i++) child(i) = child(i)->removeInverseFromExprTree(invExists, onlyDouble);
    return this;
  } else {
    if (getOperatorType() == ITM_INVERSE) invExists = TRUE;

    // if the onlyDouble flag is set to TRUE, then we
    // must find two Inverse exprs in a row to remove
    if ((!onlyDouble) || getOperator().match(ITM_INVERSE) && child(0)->getOperator().match(ITM_INVERSE)) {
...
void ItemExpr::removeNotCoveredFromExprTree(ItemExpr *&ie, const ValueIdSet &seqColumns, NABoolean rootNode,
                                            NABoolean assignValueId) {
  if (rootNode) {
    while (ie->getOperatorType() == ITM_NOTCOVERED && seqColumns.contains(ie->getValueId())) {
      ie = ie->child(0);
    }
  }

  NABoolean copyNode = FALSE;

  // if the caller wants to assign new valueID, then we detect the
  // existence of NotCovered node here.
  if (assignValueId) {
    for (int i = 0; i < ie->getArity(); i++)
      if (ie->child(i)->getOperatorType() == ITM_NOTCOVERED && seqColumns.contains(ie->child(i)->getValueId())) {
        copyNode = TRUE;
        break;
      }
  }

  // If copying node is needed, do so and we also assign a new
  // value Id for the new itemExpr.
  if (copyNode) {
    ItemExpr *copiedExpr = ie->copyTopNode(NULL, STMTHEAP);

    for (int i = 0; i < ie->getArity(); i++) copiedExpr->child(i) = ie->child(i);

    copiedExpr->synthTypeAndValueId(TRUE);

    ie = copiedExpr;
  }

  // do the NodeCovered removal work here, against the original
  // or newly copied node.
  for (int i = 0; i < ie->getArity(); i++) {
    while (ie->child(i)->getOperatorType() == ITM_NOTCOVERED && seqColumns.contains(ie->child(i)->getValueId())) {
...
// -----------------------------------------------------------------------
ItemExpr *ZZZBinderFunction::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ...
  if (derivedNode == NULL) {
    switch (getArity()) {
      case 0:
        ...
        break;
      case 1:
        result = new (outHeap) ZZZBinderFunction(getOperatorType(), child(0));
        break;
      case 2:
        result = new (outHeap) ZZZBinderFunction(getOperatorType(), child(0), child(1));
        break;
      case 3:
        result = new (outHeap) ZZZBinderFunction(getOperatorType(), child(0), child(1), child(2));
        break;
      case 4:
        result = new (outHeap) ZZZBinderFunction(getOperatorType(), child(0), child(1), child(2), child(3));
        break;
      case 5:
        result = new (outHeap) ZZZBinderFunction(getOperatorType(), child(0), child(1), child(2), child(3), child(4));
        break;
    }  // switch
  } else {
...
// -----------------------------------------------------------------------
int BiArith::getArity() const { return 2; }
...
NABoolean BiArith::duplicateMatch(const ItemExpr &other) const {
  if (NOT genericDuplicateMatch(other)) return FALSE;

  const BiArith &o = (BiArith &)other;
  if (unaryNegate_ != o.unaryNegate_) return FALSE;

  if (normalizeFlags_ != o.normalizeFlags_) return FALSE;

  if (intervalQualifier_ OR o.intervalQualifier_) {
    if (intervalQualifier_ == NULL OR o.intervalQualifier_ ==
        NULL OR NOT(*intervalQualifier_ == *(o.intervalQualifier_)))
      return FALSE;
  }
  return TRUE;
}
...
ItemExpr *BiArith::simplifyOrderExpr(OrderComparison *newOrder) {
  ...
  if (child(0)->getOperatorType() == ITM_CONSTANT) {
    ...
    switch (getOperatorType()) {
      ...
      case ITM_DIVIDE:
        ...
        if (getRoundingMode() != 0 && CmpCommon::getDefault(COMP_BOOL_176) == DF_OFF) {
  ...
  } else if (child(0)->getOperatorType() == ITM_MINUS AND child(0)->child(0)->getOperatorType() ==
             ITM_CONSTANT AND child(0)->child(1)->getOperatorType() ==
             ITM_CONSTANT AND * (short *)((ConstValue *)((ItemExpr *)(child(0)->child(0))))->getConstValue() == 0)
  ...
  } else if (child(1)->getOperatorType() == ITM_MINUS AND child(1)->child(0)->getOperatorType() ==
             ITM_CONSTANT AND child(1)->child(1)->getOperatorType() ==
             ITM_CONSTANT AND * (short *)((ConstValue *)((ItemExpr *)(child(1)->child(0))))->getConstValue() == 0)
...
ItemExpr *BiArith::foldConstants(ComDiagsArea *diagsArea, NABoolean newTypeSynthesis) {
  ItemExpr *result = this;
  ...
  if (child(0)->getOperatorType() == ITM_CONSTANT AND child(1)->getOperatorType() ==
    ...
    if (scales[0] != 0 OR scales[1] != 0) canDoIt = FALSE;
    ...
    if (ops[0]<-10000000 OR ops[0]> 10000000 OR ops[1]<-10000000 OR ops[1]> 10000000) canDoIt = FALSE;
    ...
    if (canDoIt) {
      ...
      if (numResult >= INT_MIN AND numResult <= INT_MAX) {
        ...
        result = new (CmpCommon::statementHeap())
            SystemLiteral(new (CmpCommon::statementHeap()) SQLLargeInt(CmpCommon::statementHeap(), TRUE, FALSE),
                          (void *)&numResult, sizeof(numResult), &runningOutOfNames);
      }
    }
  }
...
NABoolean BiArith::isEquivalentForCodeGeneration(const ItemExpr *other) {
  ...
  if (hasBaseEquivalenceForCodeGeneration(other)) {
    ...
    if ((unaryNegate_ == otherBiArith->unaryNegate_) && (normalizeFlags_ == otherBiArith->normalizeFlags_)) {
      ...
      if (intervalQualifier_) {
        if (otherBiArith->intervalQualifier_) {
          ...
          if (intervalQualifier_->operator==(otherIntervalQualifier)) sameIntervalQualifier = TRUE;
        }
...
QR::ExprElement BiArith::getQRExprElem() const { return QR::QRBinaryOperElem; }
...
NABoolean BiArith::hasEquivalentProperties(ItemExpr *other) {
  ...
  return (this->unaryNegate_ == otherBiArith->unaryNegate_) &&
...
void BiArith::unparse(NAString &result, PhaseEnum phase, UnparseFormatEnum form, TableDesc *tabId) const {
  NABoolean isANegativeLiteral = FALSE;
  ConstValue *literal = NULL;

  if ((form == QUERY_FORMAT || form == COMPUTED_COLUMN_FORMAT) || (form == USTAT_EXPRESSION_FORMAT))
    literal = ((BiArith *)this)->castToConstValue(isANegativeLiteral);

  if (!literal || !isANegativeLiteral)
    ItemExpr::unparse(result, phase, form, tabId);
  else {
    // instead of the 0-n, just return -n for negative constants
    // (this is not just cosmetic, we rely on this, for example
    // when computing Hive partition boundaries)
    result += "-";
    literal->unparse(result, phase, form, tabId);
  }
}
...
// -----------------------------------------------------------------------
int ColReference::getArity() const { return 0; }

const NAString ColReference::getText() const {
  NAColumn *nacol = getValueId() != NULL_VALUE_ID ? getValueId().getNAColumn(true /* ok if not a column */) : NULL;
  return nacol ? nacol->getFullColRefNameAsAnsiString() : colRefName_->getColRefAsAnsiString();
}
...
NABoolean ColReference::duplicateMatch(const ItemExpr &other) const {
  ...
  if (NOT(*colRefName_ == *(o.colRefName_))) return FALSE;
...
ItemExpr *ColReference::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ...
  if (derivedNode == NULL) {
    ...
    if (colRefName_->isStar()) {
      ...
      colName = new (outHeap) ColRefName(colRefName_->getColName(), colRefName_->getCorrNameObj(), outHeap);
    }
    ColReference *newColRef = new (outHeap) ColReference(colName);
    newColRef->setTargetColumnClass(getTargetColumnClass());
    result = newColRef;
  } else
    result = (ColReference *)derivedNode;

  result->isCompCol_ = isCompCol_;
  result->isResolved_ = isResolved_;
  result->numCompParts_ = numCompParts_;
  result->numTotalParts_ = numTotalParts_;
  result->compNames_ = compNames_;
  result->compArrIndexes_ = compArrIndexes_;
...
HostVar::HostVar(const HostVar &hv, NAMemory *h)
    : ItemExpr(hv.getOperatorType()),
      varName_(hv.varName_, h),
      indicatorName_(hv.indicatorName_, h),
      prototypeValue_(hv.prototypeValue_, h),
      prototypeType_(hv.prototypeType_),
      isEnvVar_(hv.isEnvVar_),
      isDefine_(hv.isDefine_),
      ...
      heading_(hv.heading_, h),
      tablename_(hv.tablename_, h),
      lobInlinedDataMaxLen_(hv.lobInlinedDataMaxLen_),
      lobChunkMaxLen_(hv.lobChunkMaxLen_) {
  if (hv.varType_ == NULL)
    varType_ = NULL;
  else
    varType_ = hv.varType_->newCopy(h);
}

int HostVar::getArity() const { return 0; }
...
const NAString HostVar::getText() const {
  ...
  result.append(punc + varName_);

  if (isSystemGeneratedRangeOfValuesHV()) {
    char rvInfo[50];
    ...
    else if (length_in_bytes < COM_ONE_MEG) {
      sprintf(rvInfo, "(%.3fKB)", (float)(length_in_bytes) / COM_ONE_KB);
    } else {
      sprintf(rvInfo, "(%.3fMB)", (float)(length_in_bytes) / COM_ONE_MEG);
    }
    result.append(rvInfo);
  }

  if (!indicatorName_.isNull()) result.append(" " + punc + indicatorName_);
...
NABoolean HostVar::isCharTypeMatchRulesRelaxable() {
  if (getType()->getTypeQualifier() == NA_CHARACTER_TYPE && ((CharType *)getType())->getCharSet() == CharInfo::UNICODE)
    return TRUE;
  else
    return FALSE;
}
...
NABoolean RowsetArrayScan::isCharTypeMatchRulesRelaxable() {
  if (getOperatorType() == ITM_ROWSETARRAY_SCAN && getType()->getTypeQualifier() == NA_CHARACTER_TYPE &&
...
// -----------------------------------------------------------------------
ConstantParameter::ConstantParameter(const ConstValue &v, NAMemory *h, NABoolean quantizeLen, UInt32 p)
    : val_((ConstValue *)&v), type_(CONST_CAST(NAType *, v.getType())), Parameter(ITM_CACHE_PARAM) {
...
ConstantParameter::ConstantParameter(ConstValue *v, NAMemory *h, const NAType *typ, UInt32 p)
  ...
  if (v->getType()->getTypeQualifier() == NA_CHARACTER_TYPE && type_->getTypeQualifier() == NA_CHARACTER_TYPE) {
...
ConstantParameter::ConstantParameter(const ConstantParameter &cp, NAHeap *h)
    : val_(new (h) ConstValue(*cp.val_, h)),
      type_(cp.type_->newCopy(h)),
...
ItemExpr *ConstantParameter::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
...
const NAType *ConstantParameter::synthesizeType() { return getType(); }

HashValue ConstantParameter::topHash() { return val_ ? val_->topHash() : ItemExpr::topHash(); }
...
const NAString DynamicParam::getText() const {
  if (!indicatorName_.isNull())
    return NAString("?") + paramName_ + NAString(" ?") + indicatorName_;
  else
...
const NAType *DynamicParam::pushDownType(NAType &desiredType, enum NABuiltInTypeEnum defaultQualifier) {
  const NAType *currentType = &getValueId().getType();
  if (desiredType.getTypeQualifier() == NA_CHARACTER_TYPE && currentType->getTypeQualifier() == NA_CHARACTER_TYPE) {
...
// -----------------------------------------------------------------------
const NAString RoutineParam::getText() const { return NAString("?") + paramName_; }
...
ItemExpr *RoutineParam::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ...
  if (derivedNode == NULL) {
    ...
    result = new (outHeap) RoutineParam(paramName_, paramType_, ordinalPosition_, paramMode_, NULL, outHeap);
  }
  ...
  if (derivedNode != NULL) {
    ...
    if (paramType_ != NULL) result->paramType_ = paramType_->newCopy(outHeap);
...
NAColumn *BaseColumn::getNAColumn() const { return getTableDesc()->getNATable()->getNAColumnArray()[colNumber_]; }

const NAString &BaseColumn::getColName() const { return getNAColumn()->getColName(); }

const NAType &BaseColumn::getType() const { return *getNAColumn()->getType(); }

int BaseColumn::getArity() const { return 0; }
...
const NAString BaseColumn::getText() const {
  ...
  if (getenv("SIMPLE_BASECOL_DISPLAY") || getenv("SIMPLE_DISPLAY")) {
    return NAString(getTableDesc()->getNATable()->getTableName().getObjectName()) + NAString(".") +
           NAString(getTableDesc()->getNATable()->getNAColumnArray()[colNumber_]->getColName());
  }
#endif
  ...
  ColRefName name(getTableDesc()->getNATable()->getNAColumnArray()[colNumber_]->getColName(),
...
const NAString BaseColumn::getTextForConnectBy() const {
  return NAString(getTableDesc()->getNATable()->getNAColumnArray()[colNumber_]->getColName());
}
...
const NAString BaseColumn::getTextForQuery() const {
  ...
  return NAString(getTableDesc()->getNATable()->getTableName().getCatalogName()) + NAString(".") +
         NAString(getTableDesc()->getNATable()->getTableName().getSchemaName()) + NAString(".") +
         NAString(getTableDesc()->getNATable()->getTableName().getObjectName()) + NAString(".") +
         NAString(getTableDesc()->getNATable()->getNAColumnArray()[colNumber_]->getColName());
}
...
NABoolean BaseColumn::duplicateMatch(const ItemExpr &other) const {
  ...
  // compare any local data members of the derived class
  // and return FALSE if they don't match
  if (getTableDesc() != o.getTableDesc() OR colNumber_ != o.colNumber_) return FALSE;
...
ItemExpr *BaseColumn::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ...
  assert(this->getValueId() == oldBC->getValueId());

  result = ItemExpr::copyTopNode(oldBC, outHeap);
  if (derivedNode != NULL)
    ((BaseColumn *)result)->computedColumnExpr_ = ((BaseColumn *)derivedNode)->computedColumnExpr_;

  return result;
}

NABoolean BaseColumn::isCovered(const ValueIdSet &newExternalInputs, const GroupAttributes &newRelExprAnchorGA,
                                ValueIdSet &referencedInputs, ValueIdSet & /*coveredSubExpr*/,
                                ValueIdSet & /*unCoveredExpr*/) const {
  // ---------------------------------------------------------------------
  ...
  for (; equivalentIndexCols_.next(x); equivalentIndexCols_.advance(x)) {
    if (newRelExprAnchorGA.isCharacteristicOutput(x) OR newRelExprAnchorGA.isCharacteristicInput(x)) return TRUE;
...
OrderComparison BaseColumn::sameOrder(ItemExpr *other, NABoolean askOther) {
  OrderComparison result = ItemExpr::sameOrder(other, askOther);

  if (result != DIFFERENT_ORDER) return result;
...
ValueIdList *BaseColumn::getClusteringKeyCols() const {
  return (ValueIdList *)&(tableDesc_->getClusteringIndex()->getClusteringKeyCols());
}

QR::ExprElement BaseColumn::getQRExprElem() const { return QR::QRColumnElem; }
...
IndexColumn::IndexColumn(const NAFileSet *indexPtr, int indexColumnNumber, const ValueId &colDefinition)
    : ItemExpr(ITM_INDEXCOLUMN),
      ...
      indexColDefinition_(colDefinition) {}

IndexColumn::IndexColumn(const IndexColumn &other)
    : ItemExpr(other),
      index_(other.index_),
      indexColNumber_(other.indexColNumber_),
      indexColDefinition_(other.indexColDefinition_) {}

IndexColumn::~IndexColumn() {}

NAColumn *IndexColumn::getNAColumn() const { return index_->getAllColumns()[indexColNumber_]; }

const NAType &IndexColumn::getType() const { return *getNAColumn()->getType(); }

int IndexColumn::getArity() const { return 0; }
...
const NAString IndexColumn::getText() const {
  ...
  return ColRefName::getColRefAsString(getNAColumn()->getColName(),
                                       index_->getFileSetName().getQualifiedNameAsString());
}

void IndexColumn::unparse(NAString &result, PhaseEnum /*phase*/, UnparseFormatEnum form, TableDesc *tabId) const {
  ...
  if (form == EXPLAIN_FORMAT) {
    ...
    /*      if (index_->isVolatile())
            ...
            {
    */
    result += ColRefName::getColRefAsString(getNAColumn()->getColName(), index_->getExtFileSetName());
    // result += ")";
  } else {
...
NABoolean IndexColumn::duplicateMatch(const ItemExpr &other) const {
  ...
  // compare any local data members of the derived class
  // and return FALSE if they don't match
  if (index_ != o.index_ OR indexColNumber_ != o.indexColNumber_ OR NOT(indexColDefinition_ == o.indexColDefinition_))
    return FALSE;
...
ItemExpr *IndexColumn::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ...
  assert(this->getValueId() == result->getValueId());

  return ItemExpr::copyTopNode(result, outHeap);
}

OrderComparison IndexColumn::sameOrder(ItemExpr *other, NABoolean askOther) {
  OrderComparison result = ItemExpr::sameOrder(other, askOther);
...
ValueId IndexColumn::mapAndRewrite(ValueIdMap &map, NABoolean mapDownwards) {
  ...
  if (result != indexColDefinition_) return result;
...
ItemExpr *SelIndex::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ...
  ItemExpr *result = new (outHeap) SelIndex(getSelIndex(), getExprInGrbyClause());
  ((SelIndex *)result)->setRenamedColNameInGrbyClause(renamedColNameInGrbyClause());

  return ItemExpr::copyTopNode(result, outHeap);
}

int SelIndex::getArity() const { return 0; }
...
// -----------------------------------------------------------------------
int ValueIdRef::getArity() const { return 0; }

NABoolean ValueIdRef::isCovered(const ValueIdSet &newExternalInputs, const GroupAttributes &coveringGA,
                                ValueIdSet &referencedInputs, ValueIdSet &coveredSubExpr,
                                ValueIdSet & /*unCoveredExpr*/) const {
  // ---------------------------------------------------------------------
  ...
  // is covered.
  // ---------------------------------------------------------------------
  return coveringGA.covers(isDerivedFrom(), newExternalInputs, referencedInputs, &coveredSubExpr);
...
NABoolean ValueIdRef::duplicateMatch(const ItemExpr &other) const {
  ...
  // compare any local data members of the derived class
  // and return FALSE if they don't match
  if (NOT(derivedFrom_ == o.derivedFrom_)) return FALSE;
...
OrderComparison ValueIdRef::sameOrder(ItemExpr *other, NABoolean askOther) {
  OrderComparison result = ItemExpr::sameOrder(other, askOther);
...
NABoolean ValueIdProxy::isCovered(const ValueIdSet &newExternalInputs, const GroupAttributes &coveringGA,
                                  ValueIdSet &referencedInputs, ValueIdSet &coveredSubExpr,
                                  ValueIdSet &unCoveredExpr) const {
  // ---------------------------------------------------------------------
  ...
  // is covered.
  // ---------------------------------------------------------------------
  return coveringGA.covers(isDerivedFrom(), newExternalInputs, referencedInputs, &coveredSubExpr, &unCoveredExpr);
}  // ValueIdProxy::isCovered()

const NAString ValueIdProxy::getText() const {
  char cp[TEXT_DISPLAY_LENGTH];
  sprintf(cp, "ValueIdProxy (%d:%d->%d)", CollIndex(isDerivedFrom()), CollIndex(getOutputNum()),
...
NABoolean ValueIdProxy::duplicateMatch(const ItemExpr &other) const {
  ...
  // compare any local data members of the derived class
  // and return FALSE if they don't match
  if ((NOT(derivedFrom_ == o.derivedFrom_)) AND(NOT(outputValueId_ == o.outputValueId_))
          AND(NOT(outputOrdinalNumber_ == o.outputOrdinalNumber_)))
    return FALSE;
...
ItemExpr *ValueIdProxy::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ...
  if (derivedNode == NULL)
    result = new (outHeap) ValueIdProxy(derivedFrom_, outputValueId_, outputOrdinalNumber_);
  else {
    result = derivedNode;
  }
...
ItemExpr *ValueIdProxy::containsUDF() {
  ...
  if ((transformDerivedFromValueId_ == TRUE) && derivedFrom_.getItemExpr()->containsUDF())
    return this;
  else
    return 0;
...
NABoolean ValueIdProxy::containsSubquery()  // virtual method
{
  // The transfromChild_ means that this ValueIdProxy represents the
  // the subquery or UDF, thus return true..
  if ((transformDerivedFromValueId_ == TRUE) && derivedFrom_.getItemExpr()->containsSubquery())
    return TRUE;
  else
    return FALSE;
...
int ValueIdUnion::getArity() const { return 0; }

NABoolean ValueIdUnion::isCovered(const ValueIdSet &newExternalInputs, const GroupAttributes &coveringGA,
                                  ValueIdSet &referencedInputs, ValueIdSet &coveredSubExpr,
                                  ValueIdSet & /*unCoveredExpr*/) const {
  // ---------------------------------------------------------------------
  ...
  for (CollIndex i = 0; i < entries(); i++) {
    localSubExpr.clear();
    if (coveringGA.covers(getSource(i), newExternalInputs, referencedInputs, &localSubExpr)) {
...
void ValueIdUnion::getLeafValuesForCoverTest(ValueIdSet &leafValues, const GroupAttributes &coveringGA,
                                             const ValueIdSet &newExternalInputs) const {
...
NABoolean ValueIdUnion::duplicateMatch(const ItemExpr &other) const {
  ...
  // and return FALSE if they don't match
  for (CollIndex i = 0; i < entries(); i++) {
    if (getSource(i) != o.getSource(i)) return FALSE;
  }

  if (result_ != o.result_ || flags_ != o.flags_) return FALSE;
...
void ValueIdUnion::determineLabelValues() {
  ValueIdSet tempLabelValues;

  for (CollIndex i = 0; i < sources_.entries(); i++) {
    ItemExpr *s = sources_[i].getItemExpr();

    if (s->getOperatorType() == ITM_VALUEIDUNION) {
      ValueIdUnion *childVu = static_cast<ValueIdUnion *>(s);

      if (childVu->labelValues_) {
        if (tempLabelValues.entries() > 0) {
          // need to compare the new candidate values one by
          // one with the existing ones to check for any
          // duplicates
          if (!tempLabelValues.newConstantsAreDistinct(*(childVu->labelValues_)))
            // we have duplicate labels, can't use this
            // ValueIdUnion as a label
            return;
        }
        tempLabelValues += *(childVu->labelValues_);
      } else
        // child ValueIdUnion is not a label, then we are
        // not a label, either
        return;
    } else if (s->doesExprEvaluateToConstant(TRUE, TRUE)) {
      ValueIdSet vs(s->getValueId());

      if (tempLabelValues.entries() > 0 && !tempLabelValues.newConstantsAreDistinct(vs))
        // we have duplicate label values, can't use this
        // ValueIdUnion as a label
        return;

      tempLabelValues += s->getValueId();
    } else
      // source is not a constant or another union label, so
      // this is not a label, either
      return;
  }  // loop over sources

  // this ValueIdUnion has a unique label value for every leg of the
  // union backbone, remember that fact and also remember the actual
  // label values
  labelValues_ = new (CmpCommon::statementHeap()) ValueIdSet(tempLabelValues);
}

// -----------------------------------------------------------------------
// member functions for class VEG
// -----------------------------------------------------------------------
VEG::VEG() : ItemExpr(ITM_VEG), done_(FALSE), userInputs_(0), seenBefore_(FALSE), specialNulls_(FALSE) {
  allocValueId();
...
VEG::VEG(const ValueIdSet &vegSet)
    : ItemExpr(ITM_VEG), eqGroup_(vegSet), done_(FALSE), userInputs_(0), seenBefore_(FALSE), specialNulls_(FALSE) {
  allocValueId();
  // Allocate a VegReference for representing any member of the VEG.
  vegRef_ = new (CmpCommon::statementHeap()) VEGReference(getValueId());
  // Allocate a VegPredicate for replacing the = predicate
...
void VEG::insert(const ValueId &newValue) {
  if (NOT eqGroup_.contains(newValue)) {
    eqGroup_ += newValue;
    if (newValue.getItemExpr()->isAUserSuppliedInput()) userInputs_++;
  }
}  // VEG::insert()

void VEG::insert(const ValueIdSet &newValues) {
  for (ValueId exprId = newValues.init(); newValues.next(exprId); newValues.advance(exprId)) {
    if (NOT eqGroup_.contains(exprId)) {
      eqGroup_ += exprId;
      if (exprId.getItemExpr()->isAUserSuppliedInput()) userInputs_++;
    }
  }
}  // VEG::insert()
...
void VEG::merge(const VEG &other) {
  ...
  if (specialNulls_ && !other.getSpecialNulls()) specialNulls_ = FALSE;
}  // VEG::merge()

int VEG::getArity() const { return 0; }

const NAString VEG::getText() const {
  if (seenBefore()) {
    // getenv() calls are costly. avoid especially in release code.
#ifndef NDEBUG
    if (getenv("SIMPLE_VEG_DISPLAY") || getenv("SIMPLE_DISPLAY")) return NAString("[...]");
#endif
    return NAString("...");
  }
  ...
  for (ValueId x = eqGroup_.init(); eqGroup_.next(x); eqGroup_.advance(x)) {
    ...
    if ((getenv("SIMPLE_VEG_DISPLAY") || getenv("SIMPLE_DISPLAY")) &&
        x.getItemExpr()->getOperatorType() == ITM_VEG_REFERENCE)
      continue;
#endif
  }
...
NABoolean VEG::duplicateMatch(const ItemExpr &other) const {
  ...
  if (getVEGReference()->getVEG() != ((VEG &)other).getVEGReference()->getVEG()) return FALSE;
...
void VEG::getAndExpandAllValues(ValueIdSet &expandedValues) const {
  ...
  for (ValueId vid = vegGroup.init(); vegGroup.next(vid); vegGroup.advance(vid)) {
    ...
    if ((vid.getItemExpr()->getOperatorType() == ITM_VEG_REFERENCE) && (!expandedValues.contains(vid))) {
      ...
      expandedValues.insert(vid);
      ((VEGReference *)(vid.getItemExpr()))->getVEG()->getAndExpandAllValues(expandedValues);
      expandedValues -= vid;
    }
  }
...
ValueId VEG::getAConstantHostVarOrParameter() const {
  for (ValueId id = eqGroup_.init(); eqGroup_.next(id); eqGroup_.advance(id)) {
    ...
    if (ie->isAConstantHostVarParameterOrFunc()) return id;
  }
...
ValueId VEG::getAConstant(NABoolean includeCacheParam, NABoolean lookBeneathVEG) const {
  for (ValueId id = eqGroup_.init(); eqGroup_.next(id); eqGroup_.advance(id)) {
    ItemExpr *ie = id.getItemExpr();
    ...
    else if (lookBeneathVEG && oper == ITM_VEG_REFERENCE) {
      ValueId constVid;
      VEG *veg = static_cast<VEGReference *>(ie)->getVEG();
      constVid = veg->getAConstant(includeCacheParam, TRUE);
      if (constVid != NULL_VALUE_ID) return constVid;
    }
  }
...
double VEGPredicate::defaultSel() { return (1.0 / (CURRSTMT_OPTDEFAULTS->defNoStatsUec())); }
...
int VEGPredicate::getArity() const { return 0; }
...
// in the dataflow tree for the query.
// ----------------------------------------------------------------------
void VEGPredicate::getLeafValuesForCoverTest(ValueIdSet &leafValues, const GroupAttributes &coveringGA,
                                             const ValueIdSet &newExternalInputs) const {
  leafValues += ((VEGPredicate *)this)->getVEG()->getVEGReference()->getValueId();
}  // VEGPredicate::getLeafValuesForCoverTest()

NABoolean VEGPredicate::isCovered(const ValueIdSet &newExternalInputs, const GroupAttributes &coveringGA,
                                  ValueIdSet &referencedInputs, ValueIdSet & /* coveredSubExpr*/,
                                  ValueIdSet & /* unCoveredExpr*/) const {
  ...
  if (coveringGA.isCharacteristicOutput(VEGRefId)) {
    ...
    if (NOT outputs.isEmpty()) covered = TRUE;
  }
  ...
  if (covered) {
    ...
    if (newExternalInputs.contains(VEGRefId))
      ...
      referencedInputs.accumulateReferencedValues(newExternalInputs, vegMembers, doNotLookInsideVegReferences,
  ...
  // For example, 31 = T0.i1, and T0 is not a child.
    ...
    for (ValueId vid = vegMembers.init(); vegMembers.next(vid); vegMembers.advance(vid)) {
      ...
      if (NOT(vid.getItemExpr()->isAUserSuppliedInput() OR(vid.getItemExpr()->getOperatorType() == ITM_VEG_REFERENCE)
                  OR((vid.getItemExpr()->getOperatorType() == ITM_INSTANTIATE_NULL)
                         AND(vid.getItemExpr()->child(0)->getOperatorType() == ITM_VEG_REFERENCE)))) {
    ...
    if (outerReferencesSeen && NOT allVEGMembersAreInputs) {
      ...
      if (newExternalInputs.contains(VEGRefId) && userSuppliedInputsSeen) {
...
// MVs --
void VEGPredicate::unparse(NAString &result, PhaseEnum phase, UnparseFormatEnum form, TableDesc *tabId) const {
  if ((form != MVINFO_FORMAT) && (form != QUERY_FORMAT) && (form != USTAT_EXPRESSION_FORMAT) &&
      (form != CONNECT_BY_FORMAT)) {
    ItemExpr::unparse(result, phase, form, tabId);
    return;
  }
  ...
  if (form == QUERY_FORMAT || form == USTAT_EXPRESSION_FORMAT || form == MVINFO_FORMAT || form == CONNECT_BY_FORMAT)
    startIndex = 0;
  else
    startIndex = 1;
  ...
  for (CollIndex i = startIndex; i < vegMembers.entries(); i++) {
    ...
    if (nextExpr->getOperatorType() == ITM_INDEXCOLUMN) continue;
    if (((form == QUERY_FORMAT) || (form == USTAT_EXPRESSION_FORMAT) || form == CONNECT_BY_FORMAT) &&
        (nextExpr->getOperatorType() == ITM_BASECOLUMN)) {
      BaseColumn *bs = (BaseColumn *)nextExpr;
      if (bs->getTableDesc() != tabId && form != MVINFO_FORMAT) continue;
    }
    copyList.insert(vegMembers[i]);
  }

  if (copyList.entries() < 2) return;
  ...
  NABoolean getConstFromVEG = FALSE;
  if (firstMemberId.getItemExpr()->getOperatorType() == ITM_BASECOLUMN &&
      (form == QUERY_FORMAT || form == USTAT_EXPRESSION_FORMAT)) {
    getConstFromVEG = TRUE;
  }
  ...
  for (CollIndex i = 1; i < copyList.entries(); i++) {
    ...
    if (nextMemberText != "empty") result += " AND ";

    UnparseFormatEnum form1 = (getConstFromVEG && nextMemberId.getItemExpr()->getOperatorType() == ITM_VEG_REFERENCE)
                                  ? QUERY_FORMAT_VEGREF
                                  : QUERY_FORMAT;

    nextMemberText = "";
    nextMemberId.getItemExpr()->unparse(nextMemberText, phase, form1, tabId);
...
NABoolean VEGPredicate::duplicateMatch(const ItemExpr &other) const {
  ...
  // compare any local data members of the derived class
  // and return FALSE if they don't match
  if (veg_ != ((VEGPredicate &)other).veg_) return FALSE;
...
ItemExpr *ItemExpr::copyTopNodeAndValueId(CollHeap *outHeap) {
...
// -----------------------------------------------------------------------
VEGReference::VEGReference(const ValueId &ofVEG) : ItemExpr(ITM_VEG_REFERENCE), veg_(ofVEG) {
  CMPASSERT(ofVEG.getItemExpr()->getOperatorType() == ITM_VEG);
  allocValueId();
}
...
int VEGReference::getArity() const { return 0; }
...
NABoolean VEGReference::isCovered(const ValueIdSet &newExternalInputs, const GroupAttributes &coveringGA,
                                  ValueIdSet &referencedInputs, ValueIdSet &coveredSubExpr,
                                  ValueIdSet &unCoveredExpr) const {
  ...
  if (veg->seenBefore()) return FALSE;
  ...
  for (ValueId x = veg->getAllValues().init(); veg->getAllValues().next(x); veg->getAllValues().advance(x)) {
    ...
    if (coveringGA.covers(x, newExternalInputs, referencedInputs, &coveredSubExpr, &unCoveredExpr)) {
      retVal = TRUE;
      break;
    }
  }
...
OrderComparison VEGReference::sameOrder(ItemExpr *other, NABoolean /*askOther*/) {
  // first, check whether the item expressions are identical
  if (this == other) return SAME_ORDER;
  ...
  for (ValueId x = equivValues.init(); equivValues.next(x) AND result == DIFFERENT_ORDER; equivValues.advance(x)) {
    if ((x.getItemExpr()->getOperatorType()) == ITM_VEG_REFERENCE) {
      ...
      if (!innerVeg->seenBefore()) result = x.getItemExpr()->sameOrder(other, TRUE);
    } else
      result = x.getItemExpr()->sameOrder(other, TRUE);
  }

  // step 3
  if (callNotSeen) veg->markAsNotSeenBefore();
...
const NAString VEGReference::getText() const {
  ...
  if (simpleDisplay) sprintf(cp, "VR%d", CollIndex(getValueId()));
#endif
  NAString x(NAString(cp, CmpCommon::statementHeap()) + veg_.getItemExpr()->getText(), CmpCommon::statementHeap());
  x += ")";
#ifndef NDEBUG
  if (simpleDisplay) x.remove(x.length() - 1);
#endif
  return x;
}  // VEGReference::getText()

void VEGReference::unparse(NAString &result, PhaseEnum phase, UnparseFormatEnum form, TableDesc *tabId) const {
  const ValueIdSet &vegMembers = ((VEG *)veg_.getItemExpr())->getAllValues();

  if ((form == EXPLAIN_FORMAT) || (form == MVINFO_FORMAT)) {
    for (ValueId someMemberId = vegMembers.init(); vegMembers.next(someMemberId); vegMembers.advance(someMemberId)) {
      ItemExpr *someMemberExpr = someMemberId.getItemExpr();
      // unparse the first veg member for EXPLAIN_FORMAT or MVINFO_FORMAT
      someMemberExpr->unparse(result, phase, form);
      return;
    }
  } else if ((form == QUERY_FORMAT) || (form == USTAT_EXPRESSION_FORMAT) || (form == CONNECT_BY_FORMAT)) {
    ...
    */
    ...
    for (ValueId someMemberId = vegMembers.init(); vegMembers.next(someMemberId); vegMembers.advance(someMemberId)) {
      ...
      // given tableDesc; for USTAT_EXPRESSION_FORMAT unparse the
      // first base column
      if ((someMemberExpr->getOperatorType() == ITM_BASECOLUMN) &&
              (((BaseColumn *)someMemberExpr)->getTableDesc() == tabId) ||
          (CmpCommon::getDefault(MVQR_LOG_QUERY_DESCRIPTORS) == DF_DUMP_MV) || (form == USTAT_EXPRESSION_FORMAT) ||
          (form == CONNECT_BY_FORMAT)) {
        someMemberExpr->unparse(result, phase, form, tabId);
        return;
      }
    }
  } else if (form == QUERY_FORMAT_VEGREF) {
    // unparse the first VEG member that is some kind of a constant
    for (ValueId someMemberId = vegMembers.init(); vegMembers.next(someMemberId); vegMembers.advance(someMemberId)) {
      ItemExpr *someMemberExpr = someMemberId.getItemExpr();
      switch (someMemberExpr->getOperatorType()) {
        case ITM_CONSTANT:
        case ITM_HOSTVAR:
        case ITM_DYN_PARAM:
        case ITM_CACHE_PARAM:
          someMemberExpr->unparse(result, phase, QUERY_FORMAT, tabId);
          return;
    ...
    // if no constant is found, fall back to QUERY_FORMAT processing.
    this->unparse(result, phase, QUERY_FORMAT, tabId);
  } else {
    result += getText();
  }
}
...
NABoolean VEGReference::duplicateMatch(const ItemExpr &other) const {
  ...
  // compare any local data members of the derived class
  // and return FALSE if they don't match
  if (veg_ != ((VEGReference &)other).veg_) return FALSE;
...
ItemExpr *VEGReference::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ...
  if (derivedNode == NULL)
    // Comment out the old logic and adopt a new semantics
    // for this method: the VEGReference is never copied.
...
QR::ExprElement VEGReference::getQRExprElem() const { return QR::QRColumnElem; }

// -----------------------------------------------------------------------
// member functions for class CheckConstraint
// -----------------------------------------------------------------------
int CheckConstraint::getArity() const { return 0; }

ItemExpr *CheckConstraint::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result;
...
const NAString CheckConstraint::getText() const { return "CheckConstraint"; }

void CheckConstraint::unparse(NAString &result, PhaseEnum /* phase */, UnparseFormatEnum /* form */,
...
// -----------------------------------------------------------------------
ItemExpr *OptConstraint::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
...
// -----------------------------------------------------------------------
int CardConstraint::getArity() const { return 0; }
...
const NAString CardConstraint::getText() const { return "CardConstraint"; }

void CardConstraint::unparse(NAString &result, PhaseEnum /* phase */, UnparseFormatEnum /* form */,
                             TableDesc *tabId) const {
...
// -----------------------------------------------------------------------
int UniqueOptConstraint::getArity() const { return 0; }

ItemExpr *UniqueOptConstraint::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result;
...
const NAString UniqueOptConstraint::getText() const { return "UniqueOptConstraint"; }

void UniqueOptConstraint::unparse(NAString &result, PhaseEnum phase, UnparseFormatEnum form, TableDesc *tabId) const {
...
// -----------------------------------------------------------------------
int FuncDependencyConstraint::getArity() const { return 0; }

ItemExpr *FuncDependencyConstraint::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result;

  if (derivedNode == NULL)
    result = new (outHeap) FuncDependencyConstraint(determiningCols_, dependentCols_);
  else
    result = derivedNode;
...
void FuncDependencyConstraint::synthFunctionalDependenciesFromChild(GroupAttributes &ga, const RelExpr *child,
                                                                    NABoolean createNewDependencies) {
  const ValueIdSet &childConstraints = child->getGroupAttr()->getConstraints();
  ValueIdSet vegColsFromChild(child->getGroupAttr()->getCharacteristicOutputs());
  ...
  if (createNewDependencies) {
    for (ValueId co = vegColsFromChild.init(); vegColsFromChild.next(co); vegColsFromChild.advance(co)) {
      if (co.getItemExpr()->getOperatorType() != ITM_VEG_REFERENCE) vegColsFromChild -= co;
    }
  }

  // walk through the child constraints
  for (ValueId cx = childConstraints.init(); childConstraints.next(cx); childConstraints.advance(cx)) {
    ItemExpr *c = cx.getItemExpr();
    OperatorTypeEnum optype = c->getOperatorType();

    if (optype == ITM_UNIQUE_OPT_CONSTRAINT AND createNewDependencies AND NOT vegColsFromChild.isEmpty()) {
      ...
      if (NOT ga.isUnique(uc->uniqueCols()) &&
          child->getGroupAttr()->getCharacteristicOutputs().contains(uc->uniqueCols())) {
        ...
        for (ValueId ux = uc->uniqueCols().init(); uc->uniqueCols().next(ux); uc->uniqueCols().advance(ux))
          if (ux.getItemExpr()->getOperatorType() != ITM_VEG_REFERENCE) NonVEGFound = TRUE;
        ...
        if (NOT NonVEGFound) {
          ...
          if (NOT myDependentCols.isEmpty()) {
            FuncDependencyConstraint *nc =
                new (CmpCommon::statementHeap()) FuncDependencyConstraint(uc->uniqueCols(), myDependentCols);
            ga.addConstraint(nc);
          }
        }
      }
      ...
      if (child->getGroupAttr()->getCharacteristicOutputs().contains(fd->getDeterminingCols())) {
        ga.addConstraint(c);
      }
    }
  }
...
const NAString FuncDependencyConstraint::getText() const { return "FuncDependencyConstraint"; }

void FuncDependencyConstraint::unparse(NAString &result, PhaseEnum phase, UnparseFormatEnum form,
                                       TableDesc *tabId) const {
...
int CheckOptConstraint::getArity() const { return 0; }

ItemExpr *CheckOptConstraint::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result;
...
const NAString CheckOptConstraint::getText() const { return "CheckOptConstraint"; }

void CheckOptConstraint::unparse(NAString &result, PhaseEnum phase, UnparseFormatEnum form, TableDesc *tabId) const {
...
// -----------------------------------------------------------------------
int RefOptConstraint::getArity() const { return 0; }

ItemExpr *RefOptConstraint::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result;
...
const NAString RefOptConstraint::getText() const { return "RefOptConstraint"; }

void RefOptConstraint::unparse(NAString &result, PhaseEnum phase, UnparseFormatEnum form, TableDesc *tabId) const {
...
// -----------------------------------------------------------------------
int ComplementaryRefOptConstraint::getArity() const { return 0; }

ItemExpr *ComplementaryRefOptConstraint::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result;

  if (derivedNode == NULL)
    result = new (outHeap)
        ComplementaryRefOptConstraint(ucCols_, constraintName_, tabPtr_, tabDesc_, NULL, isMatchedForElimination_);
  else
    result = derivedNode;
...
const NAString ComplementaryRefOptConstraint::getText() const { return "ComplementaryRefOptConstraint"; }

void ComplementaryRefOptConstraint::unparse(NAString &result, PhaseEnum phase, UnparseFormatEnum form,
                                            TableDesc *tabId) const {
...
NABoolean Aggregate::duplicateMatch(const ItemExpr &other) const {
  ...
  if (isDistinct_ != ag.isDistinct_) return FALSE;

  if (NOT isSensitiveToDuplicates()) return FALSE;

  return TRUE;
}

NABoolean Aggregate::isCovered(const ValueIdSet &newExternalInputs, const GroupAttributes &coveringGA,
                               ValueIdSet &referencedInputs, ValueIdSet &coveredSubExpr,
                               ValueIdSet & /*unCoveredExpr*/) const {
  // ---------------------------------------------------------------------
...
// in the dataflow tree for the query.
// ----------------------------------------------------------------------
void Aggregate::getLeafValuesForCoverTest(ValueIdSet &leafValues, const GroupAttributes &coveringGA,
                                          const ValueIdSet &newExternalInputs) const {
  leafValues += getValueId();
...
const NAString Aggregate::getText() const {
  ...
  if (seqOlapFuncInParser_) return seqOlapFuncInParser_->getText();
  ...
  switch (getOperatorType()) {
    ...
    case ITM_RANGE_VALUES_MERGE:
      result = "range_values_merge";
      break;
    case ITM_RANGE_VALUES_INSERT:
      result = "range_values_insert";
      break;
    case ITM_RANGE_VALUES_COPY:
      result = "range_values_copy";
      break;
    case ITM_RANGE_VALUES_UNPACK:
      result = "range_values_unpack";
      break;
    default:
      result = "unknown aggr";
      break;
...
void Aggregate::unparse(NAString &result, PhaseEnum phase, UnparseFormatEnum form, TableDesc *tabId) const {
  if (form == USER_FORMAT_DELUXE) {
...
ItemExpr *Aggregate::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ...
  if (inScalarGroupBy()) result->setInScalarGroupBy();

  if (topPartOfAggr()) result->setTopPartOfAggr();
  ...
  result->olapPartitionBy_ = (olapPartitionBy_ ? olapPartitionBy_->copyTree(outHeap) : NULL);

  result->olapOrderBy_ = (olapOrderBy_ ? olapOrderBy_->copyTree(outHeap) : NULL);
  ...
  result->seqOlapFuncInParser_ = seqOlapFuncInParser_;
  ...
  switch (getOperatorType()) {
    ...
    case ITM_COUNT_NONULL:
      if (child(0)->getValueId().getType().supportsSQLnullLogical()) {
        ...
        result = new (CmpCommon::statementHeap())
            Case(NULL, new (CmpCommon::statementHeap())
                           IfThenElse(new (CmpCommon::statementHeap()) UnLogic(ITM_IS_NOT_NULL, child(0)),
...
ItemExpr *Aggregate::rewriteForStagedEvaluation(ValueIdList &initialAggrs, ValueIdList &finalAggrs,
  ...
  switch (getOperatorType()) {
    case ITM_AVG:
      ...
      sumOfCounts = new (CmpCommon::statementHeap())
          Aggregate(ITM_SUM, new (CmpCommon::statementHeap()) Aggregate(ITM_COUNT_NONULL, child(0)));
      partial = new (CmpCommon::statementHeap()) Aggregate(ITM_SUM, child(0));

      sumOfSums = new (CmpCommon::statementHeap()) Aggregate(ITM_SUM, partial);
      ...
      if (getValueId().getType().supportsSQLnullLogical()) {
        result = new (CmpCommon::statementHeap())
            Case(NULL, new (CmpCommon::statementHeap()) IfThenElse(
                           new (CmpCommon::statementHeap())
                               BiRelat(ITM_GREATER, sumOfCounts, new (CmpCommon::statementHeap()) SystemLiteral(0)),
                           new (CmpCommon::statementHeap()) BiArith(ITM_DIVIDE, sumOfSums, sumOfCounts),
                           new (CmpCommon::statementHeap()) SystemLiteral()));
      } else {
        result = new (CmpCommon::statementHeap()) BiArith(ITM_DIVIDE, sumOfSums, sumOfCounts);
      }
    ...
    case ITM_ORC_SUM_NV:
      ...
      partial = new (CmpCommon::statementHeap()) Aggregate(getOperatorType(), child(0));

      sumOfCounts = new (CmpCommon::statementHeap()) Aggregate(ITM_SUM, partial);
      ...
      if (sameFormat) result->getValueId().changeType(&(result->child(0)->getValueId().getType()));
    ...
    case ITM_ORC_MAX_NV:
      ...
      minOfUECs = new (CmpCommon::statementHeap()) Aggregate(ITM_MIN, partial);
    ...
    case ITM_GROUPING:
      ...
      result = new (CmpCommon::statementHeap()) Aggregate(getOperatorType(), partial);
    ...
    case ITM_ONE_TRUE:
      ...
      result = new (CmpCommon::statementHeap()) Aggregate(ITM_ANY_TRUE, partial);
...
ItemExpr *PivotGroup::rewriteForStagedEvaluation(ValueIdList &initialAggrs, ValueIdList &finalAggrs,
                                                 NABoolean sameFormat) {
  // Split the aggregate function into three parts: the initial part is
  // executed in multiple groupby nodes whose results are sent to a
  // single, "final" groupby node that executes the final aggregate expression.
  ...
  partial = new (CmpCommon::statementHeap())
      PivotGroup(getOperatorType(), child(0), child(1), pivotOptionsList_, isDistinct());

  ConstValue *cv = new (CmpCommon::statementHeap()) ConstValue("");
  cv->synthTypeAndValueId();
  result = new (CmpCommon::statementHeap()) PivotGroup(ITM_PIVOT_GROUP, partial, cv, NULL);
...
NABoolean Aggregate::isEquivalentForBinding(const ItemExpr *other) {
  ...
  if (hasBaseEquivalenceForCodeGeneration(other)) {
    ...
    if (isDistinct()) {
      ...
      if (otherAggregate->getDistinctValueId() != otherAggregate->child(0)->getValueId()) {
        return FALSE;
      }
    }
...
OperatorTypeEnum Aggregate::mapOperTypeToOlap() const {
  switch (getOperatorType()) {
    ...
    case ITM_OLAP_FIRST_VALUE:
      return ITM_OLAP_FIRST_VALUE;
    case ITM_OLAP_LAST_VALUE:
      return ITM_OLAP_LAST_VALUE;
    case ITM_OLAP_LEAD:
      return ITM_OLAP_LEAD;
    case ITM_OLAP_LAG:
      return ITM_OLAP_LAG;
    case ITM_PIVOT_GROUP:
      return ITM_OLAP_PIVOT_GROUP;
    default:
      return INVALID_OPERATOR_TYPE;
  }
}
...
ItemExpr *Aggregate::transformOlapFunction(BindWA *bindWA) {
  ...
  //
  if ((frameStart_ > frameEnd_) || (isFrameStartUnboundedFollowing()) ||  // frameStart_ == INT_MAX) ||
  ...
  if (seqOlapFuncInParser_) return seqOlapFuncInParser_;

  CollHeap *heap = CmpCommon::statementHeap();

  OperatorTypeEnum op = mapOperTypeToOlap();
  ItmSeqOlapFunction *seqFunc = 0;

  switch (op) {
    case ITM_OLAP_FIRST_VALUE:
    case ITM_OLAP_LAST_VALUE: {
      seqFunc = new (heap) ItmOlapFirstLastValue(op, child(0));
    } break;
    case ITM_OLAP_PIVOT_GROUP: {
      CMPASSERT(ITM_PIVOT_GROUP == getOperatorType())
      PivotGroup *pivot = (PivotGroup *)this;

      seqFunc = new (heap) ItmOlapPivotGroup(op, child(0), olapOrderBy_, child(1), pivot->maxLen());
    } break;
    default: {
      seqFunc = new (heap) ItmSeqOlapFunction(op, child(0));
    }
  }

  seqFunc->setOLAPInfo(olapPartitionBy_, olapOrderBy_);
  seqFunc->setOlapWindowFrame(frameStart_, frameEnd_);
  if (ITM_OLAP_PIVOT_GROUP == seqFunc->getOperatorType() && ITM_PIVOT_GROUP == getOperatorType()) {
    PivotGroup *pivot = (PivotGroup *)this;
    seqFunc->setOlapOrderBy(pivot->getOrderbyItemExpr());
  }

  return seqFunc;
}

QR::ExprElement Aggregate::getQRExprElem() const { return QR::QRFunctionElem; }

void Aggregate::setOlapOrderBy(ItemExpr *orderBy) {
  olapOrderBy_ = orderBy;
  if (seqOlapFuncInParser_) ((ItmSequenceFunction *)seqOlapFuncInParser_)->setOlapOrderBy(orderBy);
}

void Aggregate::setOlapPartitionBy(ItemExpr *partitionBy) {
  olapPartitionBy_ = partitionBy;
  if (seqOlapFuncInParser_) ((ItmSequenceFunction *)seqOlapFuncInParser_)->setOlapPartitionBy(partitionBy);
}
...
// -----------------------------------------------------------------------
PivotGroup::PivotGroup(OperatorTypeEnum otype, ItemExpr *child0, ItemExpr *child1,
                       NAList<PivotOption *> *pivotOptionsList, NABoolean isDistinct)
  ...
  if (pivotOptionsList) {
    for (CollIndex i = 0; i < pivotOptionsList->entries(); i++) {
      ...
      switch (po->option_) {
        ...
        // do nothing for DELIMITER_ because it is child(1)
        case DELIMITER_:
        default:
          break;
      }
...
ItemExpr *PivotGroup::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ...
  if (derivedNode == NULL)
    result = new (outHeap) PivotGroup(getOperatorType(), child(0), child(1), NULL, isDistinct());
  else
...
int Function::getNumChildren() const {
  ...
  while ((count > 0) AND(children_[count - 1].getPtr() == NULL)) count--;
  return count;
}
...
BuiltinFunction::BuiltinFunction(OperatorTypeEnum otype, NAMemory *h, int argumentCount, ItemExpr *child0,
  ...
  setIsBuiltinFuncOrderable(isBuiltinFunctionOrderable());
}

BuiltinFunction::~BuiltinFunction() {}

NABoolean BuiltinFunction::isBuiltinFunctionOrderable() {
  NABoolean isOrderable;
  switch (getOperatorType()) {
    case ITM_MOD:
    case ITM_ASCII:
    case ITM_CURRENT_USER:
...
int BuiltinFunction::getArity() const { return getNumChildren(); }

// -----------------------------------------------------------------------
// BuiltinFunction::isCovered()
// -----------------------------------------------------------------------
NABoolean BuiltinFunction::isCovered(const ValueIdSet &newExternalInputs, const GroupAttributes &coveringGA,
                                     ValueIdSet &referencedInputs, ValueIdSet &coveredSubExpr,
                                     ValueIdSet &unCoveredExpr) const {
  ...
  for (int i = 0; i < nc; i++) childValues += child(i).getValueId();
  return childValues.isCovered(newExternalInputs, coveringGA, referencedInputs, coveredSubExpr, unCoveredExpr);
}  // BuiltinFunction::isCovered()
...
const NAString BuiltinFunction::getText() const {
  if (!getUserTextStr().isNull()) return getUserTextStr();
...
ItemExpr *BuiltinFunction::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result = NULL;
  if (derivedNode == NULL) {
    switch (getOperatorType()) {
      case ITM_UNIQUE_ID_SYS_GUID:
      case ITM_UNIQUE_ID: {
        result = new (outHeap) BuiltinFunction(getOperatorType(), outHeap);
      } break;
      case ITM_NULLIFZERO:
      case ITM_ISIPV4:
      case ITM_ISIPV6:
      ...
      case ITM_SOUNDEX:
      case ITM_REVERSE: {
        result = new (outHeap) BuiltinFunction(getOperatorType(), outHeap, 1, child(0));
      } break;

      case ITM_NVL:
      case ITM_QUERYID_EXTRACT: {
        result = new (outHeap) BuiltinFunction(getOperatorType(), outHeap, 2, child(0), child(1));
      } break;
      case ITM_TOKENSTR: {
        if (getArity() == 3)
          result = new (outHeap) BuiltinFunction(getOperatorType(), outHeap, 3, child(0), child(1), child(2));
        else
          result = new (outHeap) BuiltinFunction(getOperatorType(), outHeap, 2, child(0), child(1));
      } break;
...
// -----------------------------------------------------------------------
// member functions for class DecodeBase64
// -----------------------------------------------------------------------
DecodeBase64::~DecodeBase64() {}

ItemExpr *DecodeBase64::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result;

  if (derivedNode == NULL)
    result = new (outHeap) DecodeBase64(child(0), type_, charSet_);
  else
    result = derivedNode;

  return BuiltinFunction::copyTopNode(result, outHeap);

}  // DecodeBase64::copyTopNode()

ItemExpr *HashCommon::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result = NULL;
...
NABoolean BoolVal::isCovered(const ValueIdSet &newExternalInputs, const GroupAttributes &coveringGA,
                             ValueIdSet &referencedInputs, ValueIdSet &coveredSubExpr,
                             ValueIdSet &unCoveredExpr) const {
  ...
  if (getNumChildren() == 0)
    ...
    return coveringGA.covers(((ItemExpr *)child(0))->getValueId(), newExternalInputs, referencedInputs, &coveredSubExpr,
                             &unCoveredExpr);
}  // BoolVal::isCovered()
...
ItemExpr *InverseOrder::simplifyOrderExpr(OrderComparison *newOrder) {
  ...
  if (newOrder) *newOrder = resultOrder;
  return result;
}

ItemExpr *InverseOrder::removeInverseOrder() { return child(0); }
...
ItemExpr *Regexp::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ...
  if (derivedNode == NULL) {
    result = new (outHeap) Regexp(NULL, NULL, numberOfNonWildcardChars_, bytesInNonWildcardChars_,
                                  patternAStringLiteral_, oldDefaultSelForLikeWildCardUsed_, beginEndKeysApplied_);
  } else
    result = derivedNode;
...
ItemExpr *Like::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ...
  if (derivedNode == NULL) {
    switch (getArity()) {
      case 2:
        result = new (outHeap) Like(child(0), child(1), numberOfNonWildcardChars_, bytesInNonWildcardChars_,
                                    patternAStringLiteral_, oldDefaultSelForLikeWildCardUsed_, beginEndKeysApplied_);
        break;
      case 3:
        result = new (outHeap) Like(child(0), child(1), child(2), numberOfNonWildcardChars_, bytesInNonWildcardChars_,
                                    patternAStringLiteral_, oldDefaultSelForLikeWildCardUsed_, beginEndKeysApplied_);
        break;
      default:
        CMPASSERT(0 == 1);
...
double PatternMatchingFunction::defaultSel() {
  ...
  if (beginEndKeysApplied(CmpCommon::statementHeap())) return 1.0;
...
double PatternMatchingFunction::computeSelForNonWildcardChars() {
  ...
  int cnt = getNoOfNonWildcardChars();
  ...
  if ((cnt <= 1) OR(reductionFactor <= MIN_SELECTIVITY)) return defaultSelectivity;
  ...
  int nonWildCardChar = ((cnt > 4) ? 4 : cnt);
  ...
  if (defaultSelectivity < MIN_SELECTIVITY) return MIN_SELECTIVITY;
  if (defaultSelectivity > 1.0) return 1.0;
...
NABoolean PatternMatchingFunction::hasEquivalentProperties(ItemExpr *other) {
  ...
  return (this->numberOfNonWildcardChars_ == tmp->numberOfNonWildcardChars_) &&
...
void PatternMatchingFunction::unparse(NAString &result, PhaseEnum phase, UnparseFormatEnum form,
                                      TableDesc *tabId) const {
  if (getArity() == 2)
...
ItemExpr *ConvertTimestamp::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result;

  if (derivedNode == NULL)
    result = new (outHeap) ConvertTimestamp(child(0), toLocalTime_);
  else
    result = derivedNode;
...
ItemExpr *SleepFunction::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result;
...
ItemExpr *UnixTimestamp::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result;
...
ItemExpr *CurrentTimestamp::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result;
...
ItemExpr *CurrentTimestamp::construct(CollHeap *heap, DatetimeType::Subtype dtCode, int fractPrec) {
  ...
  if ((fractPrec != SQLTimestamp::DEFAULT_FRACTION_PRECISION) || (dtCode != DatetimeType::SUBTYPE_SQLTimestamp)) {
    if (dtCode == DatetimeType::SUBTYPE_SQLDate)
      ie = new (heap) Cast(ie, new (heap) SQLDate(heap, FALSE));
    else if (dtCode == DatetimeType::SUBTYPE_SQLTime)
      ie = new (heap) Cast(ie, new (heap) SQLTime(heap, FALSE, fractPrec));
    else
      ie = new (heap) Cast(ie, new (heap) SQLTimestamp(heap, FALSE, fractPrec));
  }
...
ItemExpr *CurrentTimestampRunning::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result;
...
ItemExpr *DateFormat::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  DateFormat *result;

  if (derivedNode == NULL)
    result = new (outHeap) DateFormat(child(0), formatStr_, formatType_, wasDateformat_);
  else
...
NABoolean DateFormat::hasEquivalentProperties(ItemExpr *other) {
  ...
  return (this->dateFormat_ == df->dateFormat_);
}

void DateFormat::unparse(NAString &result, PhaseEnum phase, UnparseFormatEnum form, TableDesc *tabId) const {
  if (wasDateformat_)
    result += "DATEFORMAT(";
    ...
    result += "TO_CHAR(";
  else if (formatType_ == FORMAT_TO_TIME)
    result += "TO_TIME(";
  else
    result += "unknown(";
...
ItemExpr *DayOfWeek::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result;
...
ItemExpr *ExplodeVarchar::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result;
...
NABoolean Extract::duplicateMatch(const ItemExpr &other) const {
  ...
  // compare any local data members of the derived class
  // and return FALSE if they don't match
  if (extractField_ != o.extractField_) return FALSE;
...
NABoolean Extract::isAUserSuppliedInput() const {
  ...
  if ((CmpCommon::getDefault(COMP_BOOL_185) == DF_ON) && (extractChild && extractChild->isAUserSuppliedInput()) &&
...
// ----------------------------------------------------------------------
void Extract::getLeafValuesForCoverTest(ValueIdSet &leafValues, const GroupAttributes &coveringGA,
                                        const ValueIdSet &newExternalInputs) const {
...
NABoolean Extract::hasEquivalentProperties(ItemExpr *other) {
  ...
  return (this->extractField_ == tmp->extractField_) && (this->fieldFunction_ == tmp->fieldFunction_);
}

QR::ExprElement Extract::getQRExprElem() const { return QR::QRFunctionWithParameters; }

// -----------------------------------------------------------------------
// member functions for class JulianTimestamp
// -----------------------------------------------------------------------
ItemExpr *MonthsBetween::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result;

  if (derivedNode == NULL)
    result = new (outHeap) MonthsBetween(child(0), child(1));
  else
    result = derivedNode;

  return BuiltinFunction::copyTopNode(result, outHeap);
}  // MonthsBetween::copyTopNode
...
ItemExpr *JulianTimestamp::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result;
...
ItemExpr *StatementExecutionCount::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result;
...
ItemExpr *CurrentTransId::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result;
...
ItemExpr *Trim::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ...
  if (derivedNode == NULL)
    result = new (outHeap) Trim(getTrimMode(), child(0), child(1), trimCharNotSpecified_);
  else
    result = derivedNode;
...
TriRelational::TriRelational(OperatorTypeEnum optype, ItemExpr *val1Ptr, ItemExpr *val2Ptr, ItemExpr *val3Ptr)
    : BuiltinFunction(optype, CmpCommon::statementHeap(), 3, val1Ptr, val2Ptr, val3Ptr) {
  CMPASSERT(optype == ITM_LESS_OR_LE OR optype == ITM_GREATER_OR_GE);
}
...
ItemExpr *ScalarVariance::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result;

  if (derivedNode == NULL) {
    CMPASSERT(getArity() == 3);
    result = new (outHeap) ScalarVariance(getOperatorType(), child(0), child(1), child(2));
  } else
    result = derivedNode;
...
ItemExpr *UnPackCol::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result;

  if (derivedNode == NULL) {
    CMPASSERT(getArity() == 2);
    result = new (outHeap) UnPackCol(child(0), child(1), width_, base_, nullsPresent_, type_->newCopy(outHeap));
  } else
    result = derivedNode;
...
NABoolean UnPackCol::isCovered(const ValueIdSet &newExternalInputs, const GroupAttributes &newRelExprAnchorGA,
                               ValueIdSet &referencedInputs, ValueIdSet &coveredSubExpr,
                               ValueIdSet &unCoveredExpr) const {
  // ---------------------------------------------------------------------
...
void UnPackCol::getLeafValuesForCoverTest(ValueIdSet &leafValues, const GroupAttributes &coveringGA,
                                          const ValueIdSet &newExternalInputs) const {
...
ItemExpr *RowsetArrayScan::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  RowsetArrayScan *result;

  if (derivedNode == NULL) {
    CMPASSERT(getArity() == 2);
    result = new (outHeap) RowsetArrayScan(child(0), child(1), maxNumElem_, elemSize_, elemNullInd_,
                                           elemType_->newCopy(outHeap), getOperatorType());
  } else
    result = (RowsetArrayScan *)derivedNode;

  result->optLargVar_ = optLargVar_;

  return BuiltinFunction::copyTopNode(result, outHeap);
}

NABoolean RowsetArrayScan::isCovered(const ValueIdSet &newExternalInputs, const GroupAttributes &newRelExprAnchorGA,
                                     ValueIdSet &referencedInputs, ValueIdSet &coveredSubExpr,
                                     ValueIdSet &unCoveredExpr) const {
  // ---------------------------------------------------------------------
  // If the operand is covered, then return its ValueId in coveredSubExpr.
...
void RowsetArrayScan::getLeafValuesForCoverTest(ValueIdSet &leafValues, const GroupAttributes &coveringGA,
                                                const ValueIdSet &newExternalInputs) const {
...
ItemExpr *RowsetArrayInto::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result;

  if (derivedNode == NULL) {
    CMPASSERT(getArity() == 2);
    result = new (outHeap) RowsetArrayInto(child(0), child(1), maxNumElem_, elemSize_, elemNullInd_,
                                           hostVarType_->newCopy(outHeap), getOperatorType());
  } else
    result = derivedNode;
...
NABoolean RowsetArrayInto::isCovered(const ValueIdSet &newExternalInputs, const GroupAttributes &newRelExprAnchorGA,
                                     ValueIdSet &referencedInputs, ValueIdSet &coveredSubExpr,
                                     ValueIdSet &unCoveredExpr) const {
  // ---------------------------------------------------------------------
  // If the operand is covered, then return its ValueId in coveredSubExpr.
...
void RowsetArrayInto::getLeafValuesForCoverTest(ValueIdSet &leafValues, const GroupAttributes &coveringGA,
                                                const ValueIdSet &newExternalInputs) const {
  // RowsetArrayScan is considered a leaf operator for cover test.
  leafValues += getValueId();
}
...
RangeLookup::RangeLookup(ItemExpr *val1Ptr, const RangePartitioningFunction *partFunc)
    : BuiltinFunction(ITM_RANGE_LOOKUP, CmpCommon::statementHeap(), 1, val1Ptr), partFunc_(partFunc) {}
...
PackFunc::PackFunc(ItemExpr *val1Ptr, ItemExpr *pf, const NAType *unpackType)
...
NABoolean PackFunc::isCovered(const ValueIdSet &newExternalInputs, const GroupAttributes &newRelExprAnchorGA,
                              ValueIdSet &referencedInputs, ValueIdSet &coveredSubExpr,
                              ValueIdSet &unCoveredExpr) const {
  // ---------------------------------------------------------------------
  // If the operand is covered, then return its ValueId in coveredSubExpr.
...
void PackFunc::getLeafValuesForCoverTest(ValueIdSet &leafValues, const GroupAttributes &coveringGA,
                                         const ValueIdSet &newExternalInputs) const {
...
UDFunction::~UDFunction() { delete udfDesc_; }

int UDFunction::getArity() const {
  return getNumChildren();  // Function class member function.
}

const NAString UDFunction::getText() const { return functionName_.getExternalName(); }
...
NABoolean UDFunction::duplicateMatch(const ItemExpr &other) const {
  ...
  // and return FALSE if they don't match
  if (functionName_.getExternalName() != o.functionName_.getExternalName() || actionName_ != o.actionName_ ||
      hasSubquery_ != o.hasSubquery_ || inputVars_ != o.inputVars_ || outputVars_ != o.outputVars_)
    return FALSE;

  if (udfDesc_ && o.udfDesc_)  // RoutineDesc pointers
    if (!(*udfDesc_ == *o.udfDesc_)) return FALSE;
...
ItemExpr *UDFunction::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ...
  result->udfDesc_ = udfDesc_ != NULL ? new (outHeap) RoutineDesc(*udfDesc_, outHeap) : NULL;
...
void UDFunction::unparse(NAString &result, PhaseEnum phase, UnparseFormatEnum form, TableDesc *tabId) const {
...
NABoolean UDFunction::containsIsolatedUDFunction() {
  ...
  for (CollIndex i = 0; i < (CollIndex)getNumChildren(); i++) {
    if (child(i)->containsIsolatedUDFunction() == TRUE) containsIsolatedUDF = TRUE;
  }

  if (containsIsolatedUDF == TRUE) return TRUE;
...
int UDFunction::getOutputDegree() {
  ...
  if (!getRoutineDesc())
    return -1;  // Routine desc not set.  Error.
  else
...
ValueId NotIn::createEquivEquiPredicate() const {
  CMPASSERT(child(0)->getOperatorType() != ITM_ITEM_LIST && child(1)->getOperatorType() != ITM_ITEM_LIST)

  ItemExpr *newPred = new (CmpCommon::statementHeap()) BiRelat(ITM_EQUAL, child(0), child(1));
...
ValueId NotIn::createEquivNonEquiPredicate() const {
  ...
  ItemExpr *newPred = new (CmpCommon::statementHeap()) BiRelat(ITM_NOT_EQUAL, child(0), child(1));

  newPred = new (CmpCommon::statementHeap()) UnLogic(ITM_IS_TRUE, newPred);

  newPred = new (CmpCommon::statementHeap()) UnLogic(ITM_NOT, newPred);
...
void NotIn::cacheEquivEquiPredicate() {
  if (getEquivEquiPredicate() == NULL_VALUE_ID) {
    ...
    ((BiRelat *)itm)->setOuterNullFilteringDetected(getOuterNullFilteringDetected());
    ((BiRelat *)itm)->setInnerNullFilteringDetected(getInnerNullFilteringDetected());
...
void NotIn::cacheEquivNonEquiPredicate() {
  if (getEquivNonEquiPredicate() == NULL_VALUE_ID) {
    setEquivNonEquiPredicate(createEquivNonEquiPredicate());
  }
}
...
void NotIn::cacheIsOneInnerBroadcastRequired() {
  if (isOneInnerBroadcastRequired_ == NotIn::NOT_SET) {
    ...
    if (outerType.supportsSQLnull() && !getOuterNullFilteringDetected()) {
...
// -----------------------------------------------------------------------
int BiLogic::getArity() const { return 2; }
...
// BiLogic::isCovered()
// -----------------------------------------------------------------------
NABoolean BiLogic::isCovered(const ValueIdSet &newExternalInputs, const GroupAttributes &coveringGA,
                             ValueIdSet &referencedInputs, ValueIdSet &coveredSubExpr,
                             ValueIdSet &unCoveredExpr) const {
  if (getOperatorType() == ITM_OR) {
    ...
    return leafValues.isCovered(newExternalInputs, coveringGA, referencedInputs, coveredSubExpr, unCoveredExpr);
  } else
    return ItemExpr::isCovered(newExternalInputs, coveringGA, referencedInputs, coveredSubExpr, unCoveredExpr);
}  // BiLogic::isCovered()

NABoolean BiLogic::duplicateMatch(const ItemExpr &other) const { return genericDuplicateMatch(other); }
...
QR::ExprElement BiLogic::getQRExprElem() const { return QR::QRBinaryOperElem; }

ItemExpr *BiLogic::rewriteNotEquals(BindWA *bindWA) {
  child(0) = child(0)->rewriteNotEquals(bindWA);
  child(1) = child(1)->rewriteNotEquals(bindWA);
  return this;
}
...
// -----------------------------------------------------------------------
int UnLogic::getArity() const { return 1; }
...
QR::ExprElement UnLogic::getQRExprElem() const { return QR::QRUnaryOperElem; }
...
void BiRelat::adjustRowcountAndUecForLike(const ColStatDescSharedPtr &colStatDesc, CostScalar rowCountBeforePreds,
...
int BiRelat::getArity() const { return 2; }
...
NABoolean BiRelat::duplicateMatch(const ItemExpr &other) const {
  ...
  if (specialNulls_ != o.specialNulls_ OR specialMultiValuePredicateTransformation_ !=
      o.specialMultiValuePredicateTransformation_ OR directionVector_ !=
      o.directionVector_ /* ptr comparison only */ OR isaPartKeyPred_ != o.isaPartKeyPred_ OR originalLikeExprId_ !=
      o.originalLikeExprId_ OR likeSelectivity_ != o.likeSelectivity_ OR derivedFromMCRP_ !=
      o.derivedFromMCRP_ OR preferForSubsetScanKey_ != o.preferForSubsetScanKey_)
    return FALSE;
  ...
  // for predicate (a >= 1).
  if ((derivedFromMCRP_ == TRUE) && (listOfComparisonExprs_ != o.listOfComparisonExprs_)) {
    if (listOfComparisonExprIds_.entries() != o.listOfComparisonExprIds_.entries()) return FALSE;

    for (CollIndex i = 0; i < listOfComparisonExprIds_.entries(); i++) {
      if (listOfComparisonExprIds_[i] != o.listOfComparisonExprIds_[i]) return FALSE;
    }
  }
...
ItemExpr *BiRelat::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ...
  result->specialMultiValuePredicateTransformation_ = specialMultiValuePredicateTransformation_;
  ...
  if (parentIe_) result->parentIe_ = parentIe_->copyTree(outHeap)->castToItemExpr();
  if (childIe_) result->childIe_ = childIe_->copyTree(outHeap)->castToItemExpr();
  return ItemExpr::copyTopNode(result, outHeap);
}

void BiRelat::getLeftMCRPChildList(ValueIdList &leftMCRPChildList) {
  if ((!isDerivedFromMCRP()) || (!listOfComparisonExprIds_.entries())) return;
...
QR::ExprElement BiRelat::getQRExprElem() const { return QR::QRBinaryOperElem; }
...
ItemExpr *BiRelat::rewriteNotEquals(BindWA *bindWA) {
  ItemExpr *ie = this;
  if (getOperatorType() == ITM_NOT_EQUAL) {
    // transform X <> c to X < c OR X > c
    ItemExpr *le = new (CmpCommon::statementHeap()) BiRelat(ITM_LESS, child(0), child(1));
    ItemExpr *ge = new (CmpCommon::statementHeap()) BiRelat(ITM_GREATER, child(0), child(1));
    ItemExpr *oe = new (CmpCommon::statementHeap()) BiLogic(ITM_OR, le, ge);
    ie = oe->bindNode(bindWA);
  }
  return ie;
}
...
ConstValue::ConstValue(int intval, NAMemory *outHeap)
  ...
  memcpy(value_, (void *)(&intval), (int)storageSize_);
}

ConstValue::ConstValue(const NAString &strval, enum CharInfo::CharSet charSet, enum CharInfo::Collation collation,
                       enum CharInfo::Coercibility coercibility, NAMemory *outHeap)
    : ItemExpr(ITM_CONSTANT),
      isNull_(IS_NOT_NULL),
      textIsValidatedSQLLiteralInUTF8_(FALSE),
      isStrLitWithCharSetPrefix_(FALSE),
  ...
  locale_strval = new (outHeap) NAString((char *)strval.data(), strval.length(), outHeap);
}

void ConstValue::initCharConstValue(const NAString &strval, enum CharInfo::CharSet charSet,
                                    enum CharInfo::Collation collation, enum CharInfo::Coercibility coercibility,
                                    NABoolean isCaseInSensitive, NAMemory *outHeap) {
  if (strval.length() == 0) {
    // create a varchar constant of length 0, in this case.
    type_ = new (outHeap) SQLVarChar(outHeap, 0, FALSE, FALSE, FALSE, charSet, collation, coercibility);
    storageSize_ = type_->getVarLenHdrSize();
    value_ = (void *)(new (outHeap) char[storageSize_]);
    str_pad((char *)value_, (int)storageSize_, '\0');
  } else {
    ...
    if (CharInfo::isVariableWidthMultiByteCharSet(charSet)) {
      ...
      type_ = new (outHeap) SQLChar(outHeap,
                                    CharLenInfo(actualCharsCount, strval.length()  // str len in bytes
                                                ),
                                    FALSE  // allowSQLnull
                                    ,
                                    FALSE  // isUpShifted
                                    ...
                                    FALSE  // varLenFlag
                                    ,
                                    charSet, collation, coercibility, /*encoding*/ charSet);
    } else {
      if (charSet == CharInfo::BINARY)
        type_ = new (outHeap) SQLBinaryString(outHeap, strval.length(), FALSE, FALSE);
      else
        type_ =
            new (outHeap) SQLChar(outHeap, num_of_chars, FALSE, FALSE, FALSE, FALSE, charSet, collation, coercibility);
    }

    storageSize_ = strval.length();
    value_ = (void *)(new (outHeap) char[storageSize_]);
    memcpy(value_, (void *)(strval.data()), (int)storageSize_);
  }

  ((CharType *)type_)->setCaseinsensitive(isCaseInSensitive);

  text_ = new (outHeap) NAString(strval, outHeap);
  textIsValidatedSQLLiteralInUTF8_ = FALSE;
}

ConstValue::ConstValue(const NAWString &wstrval, enum CharInfo::CharSet charSet, enum CharInfo::Collation collation,
                       enum CharInfo::Coercibility coercibility, NAMemory *outHeap,
    ...
    : ItemExpr(ITM_CONSTANT),
      ...
      rebindNeeded_(FALSE),
      heap_(outHeap) {
  initCharConstValue(wstrval, charSet, collation, coercibility, FALSE, outHeap, strLitPrefixCharSet);

  locale_wstrval =
      new (CmpCommon::statementHeap()) NAWString(wstrval.data(), wstrval.length(), CmpCommon::statementHeap());
}

void ConstValue::initCharConstValue(const NAWString &strval, enum CharInfo::CharSet charSet,
                                    enum CharInfo::Collation collation, enum CharInfo::Coercibility coercibility,
                                    NABoolean isCaseInSensitive, NAMemory *outHeap,
                                    enum CharInfo::CharSet strLitPrefixCharSet) {
  if (strval.length() == 0) {
    ...
    type_ = new (outHeap) SQLVarChar(outHeap, 0, FALSE, FALSE, FALSE, charSet, collation, coercibility);
    storageSize_ = type_->getVarLenHdrSize();
    value_ = (void *)(new (outHeap) NAWchar[storageSize_]);
    wc_str_pad((NAWchar *)value_, (int)storageSize_, '\0');
  } else {
    // A "mini-cache" to avoid proc call, for performance,
    ...
    static int cachedBPC = sizeof(NAWchar);
    ...
    if (cachedCS != charSet) {
      ...
      if (cachedCS == CharInfo::UnknownCharSet) {
        ...
        CMPASSERT(cachedBPC == (int)sizeof(NAWchar));
      }
    }

    int num_of_chars = (int)strval.length();

    type_ = new (outHeap) SQLChar(outHeap, num_of_chars, FALSE, FALSE, FALSE, FALSE, charSet, collation, coercibility);
  ...
  text_ = new (outHeap) NAString((char *)strval.data(), storageSize_, outHeap);
  textIsValidatedSQLLiteralInUTF8_ = FALSE;
}

ConstValue::ConstValue(NAString strval, NAWString wstrval, enum CharInfo::Collation collation,
                       enum CharInfo::Coercibility coercibility, NAMemory *outHeap)
    : ItemExpr(ITM_CONSTANT),
      isNull_(IS_NOT_NULL),
      value_(0),
      ...
      rebindNeeded_(FALSE),
      heap_(outHeap) {
  initCharConstValue(strval, CharInfo::UnknownCharSet, collation, coercibility, FALSE, outHeap);
  ...
  */
  ...
  locale_strval =
      new (CmpCommon::statementHeap()) NAString((char *)strval.data(), strval.length(), CmpCommon::statementHeap());

  locale_wstrval =
      new (CmpCommon::statementHeap()) NAWString(wstrval.data(), wstrval.length(), CmpCommon::statementHeap());
}

const NAType *ConstValue::pushDownType(NAType &newType, enum NABuiltInTypeEnum defaultQualifier) {
  if (newType.getTypeQualifier() == NA_CHARACTER_TYPE && type_->getTypeQualifier() == NA_CHARACTER_TYPE) {
    ...
    if (ct->getCharSet() != newCS) {
      if (newCS == CharInfo::UNICODE) {
        ...
        initCharConstValue(*locale_wstrval, newCS, ct->getCollation(), ct->getCoercibility(), ct->isCaseinsensitive());
      } else {
        if (locale_strval == NULL) {
          ...
          locale_strval = new (CmpCommon::statementHeap()) NAString(buf, cLen, CmpCommon::statementHeap());
        ...
        if (CharInfo::is_NCHAR_MP(newCS) == FALSE || locale_strval->length() % SQL_DBCHAR_SIZE == 0) {
          // fix 10-070329-5979 by passing in the case-sensitivity flag.
          // Should re-implement this with collation some day.
          initCharConstValue(*locale_strval, newCS, ct->getCollation(), ct->getCoercibility(), ct->isCaseinsensitive());
        }
      }
    }
...
ConstValue::ConstValue(const NAType *type, void *value, int value_len, NAString *literal, NAMemory *outHeap)
  ...
  value_ = (void *)(new (outHeap) char[storageSize_]);
  memcpy(value_, (void *)value, (int)storageSize_);

  if (type) type_ = type->newCopy(outHeap);
  ...
  if (literal == NULL) {
    if (type_) textIsValidatedSQLLiteralInUTF8_ = type_->createSQLLiteral((const char *)value, text_, isNull, outHeap);
    DCMPASSERT(textIsValidatedSQLLiteralInUTF8_);  // for now
    if (!textIsValidatedSQLLiteralInUTF8_) text_ = new (outHeap) NAString("<unknown value>", outHeap);
  } else {
    ...
    if (type->supportsSQLnull()) {
      ...
      type->NAType::createSQLLiteral((const char *)value, dummyPtr, isNull, outHeap);
    }
  }

  if (isNull) isNull_ = IS_NULL;
}
...
ConstValue::ConstValue(const NAType *type, void *value, int value_len, NAString *lstrval, NAWString *wstrval,
  ...
  if (literal == NULL)
    text_ = new (outHeap) NAString((char *)value, (UInt32)value_len, CmpCommon::statementHeap());
  else
    text_ = new (outHeap) NAString(*literal, outHeap);
  textIsValidatedSQLLiteralInUTF8_ = FALSE;

  if (lstrval) locale_strval = new (outHeap) NAString((char *)lstrval->data(), lstrval->length(), outHeap);

  if (wstrval) locale_wstrval = new (outHeap) NAWString((NAWchar *)wstrval->data(), wstrval->length(), outHeap);
}
...
// max non-null value.
ConstValue::ConstValue(const NAType *type, const NABoolean wantMinValue, const NABoolean includeNull, NAMemory *outHeap)
    : ItemExpr(ITM_CONSTANT),
  ...
  {
    type->minRepresentableValue(&storage[startOfData], &templen, NULL, outHeap);
    text_ = new (outHeap) NAString("<min>", outHeap);
  } else {
    type->maxRepresentableValue(&storage[startOfData], &templen, NULL, outHeap);
...
//
ConstValue::ConstValue(OperatorTypeEnum otype, ConstValue *other, NAMemory *outHeap)
    : ItemExpr(otype)
      ...
      heap_(outHeap) {
  value_ = (void *)(new (outHeap) char[storageSize_]);
  memcpy(value_, (void *)other->getConstValue(), (int)storageSize_);
  text_ = new (outHeap) NAString(other->getText(), outHeap);

  NAString *otherLocaleString = other->getLocaleString();
  locale_strval = (otherLocaleString) ? new (outHeap) NAString(*otherLocaleString, outHeap) : NULL;

  NAWString *otherLocaleWString = other->getLocaleWString();
  locale_wstrval = (otherLocaleWString) ? new (outHeap) NAWString(*otherLocaleWString, outHeap) : NULL;
}
...
ConstValue::ConstValue(const ConstValue &s, NAHeap *h)
  ...
  if (s.value_) {
    ...
    memcpy(value_, (void *)s.value_, (int)storageSize_);
  }
  if (s.text_) {
    text_ = new (h) NAString(*s.text_, h);
  }

  NAString *otherLocaleString = ((ConstValue &)s).getLocaleString();
  locale_strval = (otherLocaleString) ? new (h) NAString(*otherLocaleString, h) : NULL;

  const NAWString *otherLocaleWString = ((ConstValue &)s).getLocaleWString();
  locale_wstrval = (otherLocaleWString) ? new (h) NAWString(*otherLocaleWString, h) : NULL;
}
...
ConstValue::ConstValue(const ConstValue &s)
    : ItemExpr(ITM_CONSTANT),
      ...
      rebindNeeded_(s.rebindNeeded_),
      heap_(NULL) {
  const NAString *otherLocaleString = ((ConstValue &)s).getLocaleString();
  locale_strval = (otherLocaleString) ? new NAString(*otherLocaleString) : NULL;

  const NAWString *otherLocaleWString = ((ConstValue &)s).getLocaleWString();
  locale_wstrval = (otherLocaleWString) ? new NAWString(*otherLocaleWString) : NULL;
}
...
ConstValue::~ConstValue() {
  ...
  NADELETEBASIC((char *)value_, h);  // value_ is a void*

  if (text_) NADELETEBASIC((NAString *)text_, h);
}
...
NABoolean ConstValue::isAFalseConstant() const {
  ...
  if (type_->getTypeQualifier() == NA_BOOLEAN_TYPE && !isNull()) {
    CMPASSERT(storageSize_ == sizeof(int));
    if (*(reinterpret_cast<int *>(value_)) == 0) result = TRUE;  // that means the constant is FALSE!!
  }
...
NABoolean ConstValue::isExactNumeric() const {
  return (type_->getTypeQualifier() == NA_NUMERIC_TYPE AND((NumericType *)type_)->isExact());
}
...
NABoolean ConstValue::canGetExactNumericValue() const {
  if (isExactNumeric()) {
    ...
    if ((t.getFSDatatype() == REC_BIN64_UNSIGNED) && ((*(UInt64 *)value_) > LLONG_MAX)) return FALSE;

    // for now we can't do it for arbitrary exact numeric types, sorry
    if (NOT t.isDecimal() AND NOT t.isComplexType() AND NOT t.supportsSQLnull() AND NOT t.getPrefixSize()) return TRUE;
  }
  return FALSE;
}
...
long ConstValue::getExactNumericValue(int &scale) const {
  ...
  switch (storageSize_) {
    ...
    case 4:
      if (t.isUnsigned())
        result = uint32ToInt64(*((int *)value_));
      else
        result = *((int *)value_);
      break;
...
NABoolean ConstValue::canGetApproximateNumericValue() const {
  ...
  return (type_->getTypeQualifier() == NA_NUMERIC_TYPE &&
          ((!((NumericType *)type_)->isExact() || canGetExactNumericValue())));
}
...
void ConstValue::getOffsetsInBuffer(int &nullIndOffset, int &vcLenOffset, int &dataOffset) {
  int nextOffset = 0;
...
NABoolean ConstValue::operator==(const ItemExpr &other) const  // virtual meth
{
  ...
  if (NOT((*type_) == *(otherConst.type_))) return FALSE;
...
int ConstValue::getArity() const { return 0; }

NAString ConstValue::getConstStr(NABoolean transformNeeded) const {
  if (!textIsValidatedSQLLiteralInUTF8_ && (*text_ == "<min>" || *text_ == "<max>")) {
    ...
    type_->createSQLLiteral((const char *)value_, minMaxLiteral, isNull, (NAMemory *)CmpCommon::statementHeap());
    return *minMaxLiteral;
  } else if (getType()->getTypeQualifier() == NA_DATETIME_TYPE && getType()->getPrecision() != SQLDTCODE_MPDATETIME) {
...
ConstValue *ConstValue::toUpper(CollHeap *h) {
  ...
  if (getType()->getTypeQualifier() != NA_CHARACTER_TYPE) return this;
  ...
  switch (typ->getCharSet()) {
    ...
    case CharInfo::UNICODE: {
      ...
      unicode_char_set::to_upper((NAWchar *)value_, typ->getStrCharLimit(), wcUpshift);

      // create new ConstValue with the upshifted text
      cv = new (h) ConstValue(wcUpshift, typ->getCharSet(), typ->getCollation(), typ->getCoercibility());
      cv->changeType(typ);
      break;
    }
    ...
    default: {
      ...
      upshift.toUpper();
      cv = new (h) ConstValue(upshift, typ->getCharSet(), typ->getCollation(), typ->getCoercibility());
      cv->changeType(typ);
      break;
    }
  }
...
const NAString ConstValue::getTextForQuery(UnparseFormatEnum form) const {
  if (getType()->getTypeQualifier() != NA_CHARACTER_TYPE || textIsValidatedSQLLiteralInUTF8_) return *text_;
      ...
      if (text_) {
        switch (((CharType *)getType())->getCharSet()) {
          case CharInfo::UCS2: {
            ...
            NAString *utf8Str =
                unicodeToChar((const NAWchar *)text_->data()  // source
                              ,
                              text_->length /*in_bytes*/ () / BYTES_PER_NAWCHAR, CharInfo::UTF8  // target CS
                              ,
...
const NAString ConstValue::getText() const {
  if (getType()->getTypeQualifier() == NA_CHARACTER_TYPE) {
    ...
    size_t index = 0;
    while ((index = result.first('\0', index)) != NA_NPOS && index != result.length()) result(index, 1) = "\\0";
    index = 0;
    while ((index = result.first('\377', index)) != NA_NPOS && index != result.length()) result(index, 1) = "\\377";
...
void ConstValue::unparse(NAString &result, PhaseEnum /*phase*/, UnparseFormatEnum form, TableDesc *tabId) const {
  if (form == EXPLAIN_FORMAT) {
    ...
    const int tooLong = 30;      // keep strings shorter than this
    const int shortEnough = 10;  // leave strings of this length alone
    ...
    if (fullText.length() > shortEnough) {
      ...
      if (fullText.length() > tooLong) {
        ...
        if (fullText.last('\\') != NA_NPOS) fullText.remove(fullText.last('\\'));
        truncated = TRUE;
      }
      ...
      if (NOT truncated) len--;

      // remove repeating patterns of length 1, 2, or 4 from the end
      // of the string (patterns could be ' ', '\0', '377', etc.)
      while (len >= 8 AND fullText(len - 4, 4) == fullText(len - 8, 4)) len -= 4;
      while (len >= 4 AND fullText(len - 2, 2) == fullText(len - 4, 2)) len -= 2;
      while (len >= 2 AND fullText(len - 1, 1) == fullText(len - 2, 1)) len -= 1;
    ...
    result += fullText;
  } else if ((form == MVINFO_FORMAT) || (form == QUERY_FORMAT) || (form == USTAT_EXPRESSION_FORMAT) ||
             (form == CONNECT_BY_FORMAT) || (form == MV_SHOWDDL_FORMAT) || (form == COMPUTED_COLUMN_FORMAT)) {
    if (getType()->getTypeQualifier() == NA_CHARACTER_TYPE) {
      CharType *chType = (CharType *)getType();
      ...
      if (form == MVINFO_FORMAT)
        ...
        if (!textIsValidatedSQLLiteralInUTF8_) result += chType->getCharSetAsPrefix();

        if ((form == MV_SHOWDDL_FORMAT) || (form == QUERY_FORMAT) || (form == USTAT_EXPRESSION_FORMAT) ||
            (form = CONNECT_BY_FORMAT))
      ...
      NABoolean keyWorkAdded = false;
      if (getType()->getTypeQualifier() == NA_DATETIME_TYPE && !textIsValidatedSQLLiteralInUTF8_ && isNull() == 0) {
      ...
      if (keyWorkAdded) result += "'";
    }
  }  // MVINFO_FORMAT || QUERY_FORMAT || MV_SHOWDDL_FORMAT || COMPUTED_COLUMN_FORMAT
  else if ((form == USER_FORMAT_DELUXE) || (form == HIVE_MD_FORMAT)) {
    if (getType()->getTypeQualifier() == NA_CHARACTER_TYPE) {
...
const NAString *ConstValue::getRawText() const { return text_; }
...
HashValue ConstValue::topHash() {
  ...
  if (NOT isNull()) {
    ...
    int maxc = MINOF(storageSize_, 10);
    for (int i = 0; i < maxc; i++) result ^= ((char *)value_)[i];
  }
...
NABoolean ConstValue::duplicateMatch(const ItemExpr &other) const {
  ...
  // compare any local data members of the derived class
  // and return FALSE if they don't match
  if (NOT(*type_ == *(o.type_))) return FALSE;

  if (isNull() != o.isNull()) return FALSE;

  if (isNull()) return TRUE;

  if (storageSize_ != o.storageSize_) return FALSE;

  if (str_cmp((char *)value_, (char *)o.value_, storageSize_) != 0 OR isSystemSupplied_ != o.isSystemSupplied_)
    return FALSE;
...
ItemExpr *ConstValue::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ...
  if (derivedNode == NULL) {
    ...
    if (isNull() && !value_) {
       ...
       */
      result =
          new (outHeap) ConstValue(type_, value_, storageSize_, locale_strval, locale_wstrval, text_, outHeap, isNull_);

      /* soln:10-050710-9594 end */

      result->textIsValidatedSQLLiteralInUTF8_ = textIsValidatedSQLLiteralInUTF8_;
...
NABoolean ConstValue::isCovered(const ValueIdSet & /* newExternalInputs */, const GroupAttributes & /* coveringGA */,
                                ValueIdSet & /* referencedInputs */, ValueIdSet & /* coveredSubExpr */,
                                ValueIdSet & /*unCoveredExpr*/) const {
  // ---------------------------------------------------------------------
...
UInt32 ConstValue::computeHashValue(const NAType &columnType) {
  ...
  switch (getType()->getTypeQualifier()) {
    ...
    case NA_NUMERIC_TYPE:
      ...
      // for now.
      if (getType()->getTypeName() == LiteralNumeric AND columnType.getTypeName() ==
          LiteralNumeric AND columnType.getNominalSize() <= 8) {
        ...
        // utility.
        short retCode =
            convDoIt((char *)(getConstValue()), getStorageSize(), (short)getType()->getFSDatatype(),
                     getType()->getPrecision(), getType()->getScale(), result, len, (short)(type.getFSDatatype()),
                     type.getPrecision(), type.getScale(), NULL /* null varchar ptr*/, 0 /* varchar len*/);
...
NABoolean ConstValue::isMin() { return *text_ == "<min>"; }

NABoolean ConstValue::isMax() { return *text_ == "<max>"; }

QR::ExprElement ConstValue::getQRExprElem() const { return QR::QRScalarValueElem; }

// -----------------------------------------------------------------------
// member functions for class RenameCol
// -----------------------------------------------------------------------
int RenameCol::getArity() const { return (child(0) ? 1 : 0); }

const NAString RenameCol::getText() const { return ("rename as " + newColRefName_->getColName()); }
...
NABoolean RenameCol::duplicateMatch(const ItemExpr &other) const {
  ...
  // compare any local data members of the derived class
  // and return FALSE if they don't match
  if (NOT(*newColRefName_ == *(o.newColRefName_))) return FALSE;
...
ItemExpr *RenameCol::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ...
  if (derivedNode == NULL) {
    RenameCol *newRename = new (outHeap) RenameCol(
        NULL, new (outHeap) ColRefName(newColRefName_->getColName(), newColRefName_->getCorrNameObj(), outHeap));
...
// -----------------------------------------------------------------------
int Convert::getArity() const { return 1; }
...
// -----------------------------------------------------------------------
int Assign::getArity() const { return 2; }

NABoolean Assign::isCovered(const ValueIdSet &newExternalInputs, const GroupAttributes &coveringGA,
  ...
  if (coveringGA.covers(getSource(), newExternalInputs, referencedInputs, &coveredSubExpr, &unCoveredExpr))
...
// -----------------------------------------------------------------------
int ItemList::getArity() const { return 2; }

NABoolean ItemList::isCovered(const ValueIdSet &newExternalInputs, const GroupAttributes &coveringGA,
                              ValueIdSet &referencedInputs, ValueIdSet &coveredSubExpr,
                              ValueIdSet &unCoveredExpr) const {
  ValueId exprId;
  ...
  for (int i = 0; i < (int)getArity(); i++) {
    ...
    for (CollIndex j = 0; j < childList.entries(); j++) {
      ...
      coverFlag = coveringGA.covers(exprId, newExternalInputs, referencedInputs, &coveredSubExpr, &unCoveredExpr);
      if (coverFlag) coveredSubExpr += exprId;
...
NABoolean ItemList::hasEquivalentProperties(ItemExpr *other) {
  ...
  return (this->numOfItems() == otherList->numOfItems()) && (this->constChild() == otherList->constChild());
}

NAString ItemList::getConstantText() {
  NAString result;
  ItemExpr *itm;
  ItemExpr *child0;
  ConstValue *cv;
  ...
  itm = castToItemExpr();
  do {
    child0 = itm->getChild(0)->castToItemExpr();
    if (child0->getOperatorType() != ITM_CONSTANT)
      return getText();
    else {
      cv = (ConstValue *)child0;
      result.prepend("," + cv->getText());
    }
    itm = itm->getChild(1)->castToItemExpr();
  } while (itm->getOperatorType() == ITM_ITEM_LIST);
...
NAString ItemList::getConstantTextNullReplace(NAString replacement, NABoolean reverseOrder) {
  NAString result;
  ItemExpr *itm;
  ItemExpr *child0;
  ConstValue *cv;
  ...
  do {
    ...
    if (child0->getOperatorType() != ITM_CONSTANT) {
      NAString text;
      child0->unparse(text, PARSER_PHASE, USER_FORMAT);
      if (reverseOrder)
        result.append(text + ",");
      else
        result.prepend("," + text);
      ...
      if (cv->isNull())
        ...
        if (cv->getType() && cv->getType()->getTypeQualifier() == NA_CHARACTER_TYPE)
          if (reverseOrder)
            result.append(cv->getText() + ",");
          else
            result.prepend("," + cv->getText());
        else if (reverseOrder)
          result.append(cv->getConstStr() + ",");
        else
          result.prepend("," + cv->getConstStr());
      }
    }
    itm = itm->getChild(1)->castToItemExpr();
  } while (itm->getOperatorType() == ITM_ITEM_LIST);
  ...
  if (itm->getOperatorType() != ITM_CONSTANT) {
    NAString text;
    itm->unparse(text, PARSER_PHASE, USER_FORMAT);
    if (reverseOrder)
      result.append(text);
    else
      result.prepend(text);
...
const NAString Subquery::getText() const { return "unknown subquery"; }  // Subquery::getText()

void Subquery::addLocalExpr(LIST(ExprNode *) & xlist, LIST(NAString) & llist) const {
  xlist.insert(tableExpr_);
  llist.insert("Subquery");
}

NABoolean Subquery::containsSubquery() { return TRUE; }  // Subquery::containsSubquery

// <aviv> BEGIN
ItemExpr *Subquery::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result;
  if (derivedNode == NULL) {
...
ItemExpr *RowSubquery::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result;
  if (derivedNode == NULL) {
...
// <aviv> BEGIN
ItemExpr *QuantifiedComp::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ...
  if (derivedNode == NULL) {
    ...
    result = new (outHeap)
        QuantifiedComp(getOperatorType(), (child(0))->copyTree(outHeap), tableExprCopy, getAvoidHalloweenR2());
  } else
...
// <aviv> BEGIN
ItemExpr *InSubquery::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result;
  if (derivedNode == NULL) {
    RelExpr *tableExprCopy = getSubquery()->copyTree(outHeap);
...
/// --------------------------------------------------------------
// member functions for MaxBytes
// --------------------------------------------------------------
ItemExpr *MaxBytes::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result;

  if (derivedNode == NULL)
    result = new (outHeap) MaxBytes(getOperatorType(), child(0));
  else
    result = derivedNode;
...
const NAString MaxBytes::getText() const { return "max_bytes"; }
...
Exists::Exists(RelExpr *tableExpr) : Subquery(ITM_EXISTS, tableExpr) {}
...
// <aviv> BEGIN
ItemExpr *Exists::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result;
  if (derivedNode == NULL) {
    RelExpr *tableExprCopy = getSubquery()->copyTree(outHeap);
    result = new (outHeap) Exists(tableExprCopy);
    if (isNotExists()) result->setOperatorType(getOperatorType());
  } else
    result = derivedNode;
...
void Overlaps::unparse(NAString &result, PhaseEnum phase, UnparseFormatEnum form, TableDesc *tabId) const {
  result += "(";
  child(0)->unparse(result, phase, form, tabId);
  result += ", ";
  child(1)->unparse(result, phase, form, tabId);
...
void Between::unparse(NAString &result, PhaseEnum phase, UnparseFormatEnum form, TableDesc *tabId) const {
  child(0)->unparse(result, phase, form, tabId);
...
NABoolean Between::hasEquivalentProperties(ItemExpr *other) {
  ...
  return (this->leftBoundryIncluded_ == tmp->leftBoundryIncluded_) &&
...
ItemExpr *Case::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ...
  //++MV bug fix
  if (caseOperand_) ((Case *)result)->caseOperand_ = caseOperand_->copyTree(outHeap);
...
void Case::fixIfThenElse(ItemExpr *expr, ValueIdSet &coveredSubExpr, NABoolean ifThenElseExists) const {
  if ((!expr) || (expr->getOperatorType() != ITM_IF_THEN_ELSE)) return;
...
NABoolean Case::isCovered(const ValueIdSet &newExternalInputs, const GroupAttributes &coveringGA,
                          ValueIdSet &referencedInputs, ValueIdSet &coveredSubExpr, ValueIdSet &unCoveredExpr) const {
  ValueIdSet coveredSubExprForCase;
  NABoolean retVal =
      BuiltinFunction::isCovered(newExternalInputs, coveringGA, referencedInputs, coveredSubExprForCase, unCoveredExpr);
...
void Case::unparse(NAString &result, PhaseEnum phase, UnparseFormatEnum form, TableDesc *tabId) const {
  // Is this for MV use?
  if ((form != MV_SHOWDDL_FORMAT) && (form != QUERY_FORMAT) && (form != USTAT_EXPRESSION_FORMAT) &&
      (form != CONNECT_BY_FORMAT) && (form != COMPUTED_COLUMN_FORMAT)) {
...
void Case::getLeafValuesForCoverTest(ValueIdSet &leafValues, const GroupAttributes &coveringGA,
                                     const ValueIdSet &newExternalInputs) const {
  leafValues += getValueId();
...
NABoolean Case::hasEquivalentProperties(ItemExpr *other) {
  ...
  return (this->getCaseOperand() == otherCase->getCaseOperand()) &&
...
Cast::Cast(ItemExpr *val1Ptr, const NAType *type, OperatorTypeEnum otype, NABoolean checkForTrunc,
           NABoolean noStringTrunWarnings)
    : CacheableBuiltinFunction(otype, 1, val1Ptr), type_(type), reverseDataErrorConversionFlag_(FALSE), flags_(0) {
  ...
  if (checkForTrunc)
    if (vid == NULL_VALUE_ID)
      ...
      if (type->getNominalSize() < vid.getType().getNominalSize())
        checkForTruncation_ = TRUE;
      else {  // If UTF8 and *any* chance of truncation error
...
Cast::Cast(ItemExpr *val1Ptr, ItemExpr *errorOutPtr, const NAType *type, OperatorTypeEnum otype,
           NABoolean checkForTrunc, NABoolean reverseDataErrorConversionFlag, NABoolean noStringTrunWarnings)
    : CacheableBuiltinFunction(otype, 2, val1Ptr, errorOutPtr),
      type_(type),
...
ConstValue *Cast::castToConstValue(NABoolean &negate_it) { return child(0)->castToConstValue(negate_it); }

ItemExpr *Cast::foldConstants(ComDiagsArea *diagsArea, NABoolean newTypeSynthesis) {
...
NABoolean Cast::isCovered(const ValueIdSet &newExternalInputs, const GroupAttributes &coveringGA,
                          ValueIdSet &referencedInputs, ValueIdSet &coveredSubExpr, ValueIdSet &unCoveredExpr) const {
  NABoolean coverFlag = TRUE;
  if (coveringGA.covers(getExpr()->getValueId(), newExternalInputs, referencedInputs, &coveredSubExpr, &unCoveredExpr))
...
// MVLog command.
void Cast::getLeafValuesForCoverTest(ValueIdSet &leafValues, const GroupAttributes &coveringGA,
                                     const ValueIdSet &newExternalInputs) const {
...
ItemExpr *Cast::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ...
  if (derivedNode == NULL) {
    result = new (outHeap) Cast(NULL, type_->newCopy(outHeap), this->getOperatorType(), this->checkForTruncation_);

    // Fix CR 10-010426-2464: Copy other members of the class as well
    ((Cast *)result)->reverseDataErrorConversionFlag_ = this->reverseDataErrorConversionFlag_;
...
ItemExpr *CastType::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ...
  if (derivedNode == NULL)
    result = new (outHeap) CastType(NULL, (getType() ? getType()->newCopy(outHeap) : NULL));
  else
    result = derivedNode;
...
NABoolean Cast::hasEquivalentProperties(ItemExpr *other) {
  ...
  return (this->type_->operator==(*(tmp->type_))) && (this->checkForTruncation_ == tmp->checkForTruncation_) &&
...
NABoolean IfThenElse::isCharTypeMatchRulesRelaxable() {
  NABoolean ok = child(0)->isCharTypeMatchRulesRelaxable() && child(1)->isCharTypeMatchRulesRelaxable();

  if (child(2).getPtr() != NULL) ok = ok && child(2)->isCharTypeMatchRulesRelaxable();

  return ok;
}

void IfThenElse::unparse(NAString &result, PhaseEnum phase, UnparseFormatEnum form, TableDesc *tabId) const {
  // Is this for MV use?
  if ((form != MV_SHOWDDL_FORMAT) && (form != QUERY_FORMAT) && (form != USTAT_EXPRESSION_FORMAT) &&
      (form != CONNECT_BY_FORMAT) && (form != COMPUTED_COLUMN_FORMAT)) {
    // No - sorry, use the default code.
...
const NAString RaiseError::getText() const { return NAString("RaiseError(") + getConstraintName() + ")"; }
...
ItemExpr *RaiseError::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ...
  if (derivedNode == NULL) {
    if (getArity() > 0)  // Do we have string expressions?
      result = new (outHeap) RaiseError(getSQLCODE(), getConstraintName(), child(0)->copyTree(outHeap), outHeap);
    else
      result =
          new (outHeap) RaiseError(getSQLCODE(), getConstraintName(), getTableName(), optionalStr_, type_, outHeap);
  } else
    result = derivedNode;
...
ItemExpr *Narrow::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result;
  ...
  if (derivedNode == NULL) {
    ...
    result = new (outHeap) Narrow(NULL, NULL, getType()->newCopy(outHeap), ITM_NARROW, FALSE, matchChildNullability_);
...
NABoolean Narrow::hasEquivalentProperties(ItemExpr *other) {
  ...
  return (this->matchChildNullability_ == tmp->matchChildNullability_);
}
// -----------------------------------------------------------------------
...
// in the dataflow tree for the query.
// ----------------------------------------------------------------------
void InstantiateNull::getLeafValuesForCoverTest(ValueIdSet &leafValues, const GroupAttributes &coveringGA,
                                                const ValueIdSet &newExternalInputs) const {
  leafValues += getValueId();
...
ItemExpr *InstantiateNull::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ...
  if (derivedNode == NULL)
    result = new (outHeap) InstantiateNull(NULL, getType()->newCopy(outHeap));
  else
    result = derivedNode;
...
NABoolean InstantiateNull::isAColumnReference() {
  if (child(0)) {
    ...
    if ((child(0)->getOperatorType() != ITM_VALUEIDUNION) && (NOT child(0)->isAnAggregate())) return TRUE;
  }
  return FALSE;
}
...
NABoolean InstantiateNull::hasEquivalentProperties(ItemExpr *other) {
  ...
  return (this->NoCheckforLeftToInnerJoin == tmp->NoCheckforLeftToInnerJoin) &&
...
NABoolean InstantiateNull::isEquivalentForCodeGeneration(const ItemExpr *other) { return FALSE; }

// --------------------------------------------------------------
// member functions for BitOperFunc operator
// --------------------------------------------------------------
int BitOperFunc::getArity() const {
  if (child(0) == NULL)
    return 0;
...
ItemExpr *CompEncode::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ...
  if (derivedNode == NULL)
    result = new (outHeap) CompEncode(NULL, descFlag_, length_, collationType_, regularNullability_, outHeap);
  else
    result = derivedNode;
...
int CompEncode::getEncodedLength(const CharInfo::Collation collation, const CollationInfo::CollationType ct,
  ...
  switch (ct) {
    case CollationInfo::Sort: {
      return nPasses * (srcLength + 1) + (nullable ? ExpTupleDesc::KEY_NULL_INDICATOR_LENGTH : 0);
    }
...
ItemExpr *CompDecode::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ...
  if (derivedNode == NULL)
    result = new (outHeap) CompDecode(NULL, unencodedType_->newCopy(outHeap), descFlag_, length_, collationType_,
                                      regularNullability_, outHeap);
  else
    result = derivedNode;
...
ItemExpr *HashDistPartHash::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result;
...
ItemExpr *HashDistPartHashComb::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result;
...
// --------------------------------------------------------------
// member functions for RegexpReplace operator
// --------------------------------------------------------------

ItemExpr *RegexpReplace::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result;

  if (derivedNode == NULL)
    result = new (outHeap) RegexpReplace(NULL, NULL, NULL);
  else
    result = derivedNode;

  return Replace::copyTopNode(result, outHeap);
}
...
// --------------------------------------------------------------
// member functions for RegexpSubstr operator
// --------------------------------------------------------------

ItemExpr *RegexpSubstr::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result;

  if (derivedNode == NULL) {
    switch (getArity()) {
      case 2:
        result = new (outHeap) RegexpSubstr(child(0), child(1));
        break;
      case 3:
        result = new (outHeap) RegexpSubstr(child(0), child(1), child(2));
        break;
      case 4:
        result = new (outHeap) RegexpSubstr(child(0), child(1), child(2), child(3));
        break;
      default:
        CMPASSERT(0 == 1);
        break;
...
// --------------------------------------------------------------
// member functions for RegexpCount operator
// --------------------------------------------------------------

ItemExpr *RegexpCount::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result;

  if (derivedNode == NULL) {
    switch (getArity()) {
      case 2:
        result = new (outHeap) RegexpCount(child(0), child(1));
        break;
      case 3:
        result = new (outHeap) RegexpCount(child(0), child(1), child(2));
        break;
      default:
        CMPASSERT(0 == 1);
        break;
...
ItemExpr *ProgDistribKey::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result;
...
// --------------------------------------------------------------
int NATypeToItem::getArity() const { return 0; }
const NAString NATypeToItem::getText() const { return natype_pointer->getTypeSQLname(TRUE); }
...
ItemExpr *NamedTypeToItem::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ...
  if (derivedNode == NULL)
    result = new (outHeap) NamedTypeToItem(name_.data(), natype_pointer, outHeap);
  else
    result = derivedNode;
...
const NAString NamedTypeToItem::getText() const { return name_; }
...
ItemExpr *UniqueExecuteId::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result;
...
ItemExpr *GetTriggersStatus::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result;
...
ItemExpr *GetBitValueAt::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result;
...
ItemExpr *IsBitwiseAndTrue::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result;
...
// -----------------------------------------------------------------------
// member functions for class AnsiTenantFunction
// -----------------------------------------------------------------------
AnsiTenantFunction::~AnsiTenantFunction() {}

NABoolean AnsiTenantFunction::isAUserSuppliedInput() const { return TRUE; }

ItemExpr *AnsiTenantFunction::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result;

  if (derivedNode == NULL)
    result = new (outHeap) AnsiTenantFunction(getOperatorType());
  else
    result = derivedNode;

  return BuiltinFunction::copyTopNode(result, outHeap);

}  // AnsiTenantFunction::copyTopNode()
// -----------------------------------------------------------------------
...
NABoolean MonadicUSERFunction::isCovered(const ValueIdSet &newExternalInputs, const GroupAttributes &coveringGA,
                                         ValueIdSet &referencedInputs, ValueIdSet &coveredSubExpr,
                                         ValueIdSet &unCoveredExpr) const {
...
// -----------------------------------------------------------------------
Translate::Translate(ItemExpr *valPtr, NAString *map_table_name) : CacheableBuiltinFunction(ITM_TRANSLATE, 1, valPtr) {
  if (_strcmpi(map_table_name->data(), "UNICODE_TO_SJIS") == 0 || _strcmpi(map_table_name->data(), "UTOSJ") == 0)
    map_table_id_ = Translate::UNICODE_TO_SJIS;
  else if (_strcmpi(map_table_name->data(), "UCS2TOSJIS") == 0)
    map_table_id_ = Translate::UCS2_TO_SJIS;
  else if (_strcmpi(map_table_name->data(), "UCS2TOUTF8") == 0)
    map_table_id_ = Translate::UCS2_TO_UTF8;
  else if (_strcmpi(map_table_name->data(), "UCS2TOISO88591") == 0)
    map_table_id_ = Translate::UNICODE_TO_ISO88591;
  else if (_strcmpi(map_table_name->data(), "SJIS_TO_UNICODE") == 0 || _strcmpi(map_table_name->data(), "SJTOU") == 0)
    map_table_id_ = Translate::SJIS_TO_UNICODE;
  else if (_strcmpi(map_table_name->data(), "SJISTOUCS2") == 0)
    map_table_id_ = Translate::SJIS_TO_UCS2;
  else if (_strcmpi(map_table_name->data(), "UTF8TOUCS2") == 0)
    map_table_id_ = Translate::UTF8_TO_UCS2;
  else if (_strcmpi(map_table_name->data(), "ISO88591TOUCS2") == 0)
  ...
  else if (_strcmpi(map_table_name->data(), "KANJITOISO88591") == 0)
    map_table_id_ = Translate::KANJI_MP_TO_ISO88591;
  else if (_strcmpi(map_table_name->data(), "KSC5601TOISO88591") == 0)
...
Translate::Translate(ItemExpr *valPtr, int map_table_id) : CacheableBuiltinFunction(ITM_TRANSLATE, 1, valPtr) {
...
NABoolean Translate::isCharTypeMatchRulesRelaxable() { return child(0)->isCharTypeMatchRulesRelaxable(); }

void Translate::unparse(NAString &result, PhaseEnum phase, UnparseFormatEnum form, TableDesc *tabId) const {
  if (CmpCommon::getDefault(MVQR_LOG_QUERY_DESCRIPTORS) != DF_DUMP_MV)
    return CacheableBuiltinFunction::unparse(result, phase, form, tabId);
  else {
    result += "translate(";
    ...
    switch (map_table_id_) {
      case UNICODE_TO_SJIS:
        translation = "UCS2_TO_SJIS";
        break;
      case UNICODE_TO_ISO88591:
        translation = "UCS2TOISO88591";
        break;
      case ISO88591_TO_UNICODE:
        translation = "ISO88591TOUCS2";
        break;
      case SJIS_TO_UNICODE:
        translation = "SJIS_TO_UCS2";
        break;
      case UCS2_TO_SJIS:
        translation = "UCS2TOSJIS";
        break;
      case SJIS_TO_UCS2:
        translation = "SJISTOUCS2";
        break;
      case UCS2_TO_UTF8:
        translation = "UCS2TOUTF8";
        break;
      case UTF8_TO_UCS2:
        translation = "UTF8TOUCS2";
        break;
      case UTF8_TO_SJIS:
        translation = "UTF8TOSJIS";
        break;
      case SJIS_TO_UTF8:
        translation = "SJISTOUTF8";
        break;
      case UTF8_TO_ISO88591:
        translation = "UTF8TOISO88591";
        break;
      case ISO88591_TO_UTF8:
        translation = "ISO88591TOUTF8";
        break;
      case KANJI_MP_TO_ISO88591:
        translation = "KANJITOISO88591";
        break;
      case KSC5601_MP_TO_ISO88591:
        translation = "KSC5601TOISO88591";
        break;
      case UNKNOWN_TRANSLATION:
...
NABoolean HbaseAttribute::isCovered(const ValueIdSet &newExternalInputs, const GroupAttributes &coveringGA,
                                    ValueIdSet &referencedInputs, ValueIdSet &coveredSubExpr,
                                    ValueIdSet &unCoveredExpr) const {
  // return TRUE;
  return FALSE;
...
ItemExpr *HbaseRowid::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  HbaseRowid *result;

  if (derivedNode == NULL) {
    result = new (outHeap) HbaseRowid(col(), rowidLen_);
  } else
    result = (HbaseRowid *)derivedNode;

  return HbaseAttribute::copyTopNode(result, outHeap);
}  // HbaseRowid::copyTopNode()

ItemExpr *HbaseRowidRef::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  HbaseRowidRef *result;

  if (derivedNode == NULL) {
    result = new (outHeap) HbaseRowidRef(corrName_, col());
  } else
    result = (HbaseRowidRef *)derivedNode;

  return HbaseAttributeRef::copyTopNode(result, outHeap);
}  // HbaseRowidRef::copyTopNode()
...
NABoolean RowNumFunc::isCovered(const ValueIdSet &newExternalInputs, const GroupAttributes &coveringGA,
                                ValueIdSet &referencedInputs, ValueIdSet &coveredSubExpr,
                                ValueIdSet &unCoveredExpr) const {
...
NABoolean ItmSequenceFunction::isCovered(const ValueIdSet &newExternalInputs, const GroupAttributes &newRelExprAnchorGA,
                                         ValueIdSet &referencedInputs, ValueIdSet &coveredSubExpr,
                                         ValueIdSet &unCoveredExpr) const {
  // ---------------------------------------------------------------------
  // If the operand is covered, then return its ValueId in coveredSubExpr.
  ...
  for (int i = 0; i < (int)getArity(); i++) {
    if (child(i)->getOperatorType() == ITM_ITEM_LIST) {
      ...
      ItemExprTreeAsList values(&treePtr, ITM_ITEM_LIST, RIGHT_LINEAR_TREE);

      CollIndex nc = values.entries();
      for (CollIndex m = 0; m < nc; m++) {
        if (newRelExprAnchorGA.covers(values[m]->getValueId(), newExternalInputs, referencedInputs, &localSubExpr)) {
...
void ItmSequenceFunction::getLeafValuesForCoverTest(ValueIdSet &leafValues, const GroupAttributes &coveringGA,
                                                    const ValueIdSet &newExternalInputs) const {
...
const NAString ItmSequenceFunction::getText() const {
  switch (getOperatorType()) {
    ...
    case ITM_OLAP_PIVOT_GROUP:
      return "olap pivot group";
    default:
      return "unknown sequence function";
  }  // switch
}

ItemExpr *ItmSequenceFunction::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItmSequenceFunction *result = NULL;

  if (derivedNode == NULL)
    ABORT("copyTopNode() can only be called for a derived class of ItmSequenceFunction");
  else
...
ItemExpr *ItmSequenceFunction::transformTDFunction(BindWA *bindWA) {
  ...
  if (!isTDFunction()) return this;

  if (!olapPartitionBy_) return this;

  ItmSequenceFunction *partClause = NULL;

  partClause = new (bindWA->getCurrentScope()->collHeap())
      ItmSeqRunningFunction(ITM_RUNNING_CHANGE, olapPartitionBy_->copyTree(bindWA->getCurrentScope()->collHeap()));
  ...
  ItmSequenceFunction *seqFunc =
      new (bindWA->getCurrentScope()->collHeap()) ItmSeqMovingFunction(ITM_MOVING_RANK, child(0), partClause);
...
ItemExpr *ItmSeqRunningFunction::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result;

  if (derivedNode == NULL) {
    result = new (outHeap) ItmSeqRunningFunction(getOperatorType(), child(0));
...
ItemExpr *ItmSeqRunningPivotGroup::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result = NULL;

  if (derivedNode == NULL) {
    result = new (outHeap)
        ItmSeqRunningPivotGroup(getOperatorType(), child(0), getOlapOrderBy(), getPivotDelimiter(), getPivotMaxLen());
  } else
    result = derivedNode;

  return ItmSeqRunningFunction::copyTopNode(result, outHeap);
}  // ItmSeqRunningPivotGroup::copyTopNode()

ItemExpr *ItmSeqOlapFunction::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result;

  if (derivedNode == NULL) {
    result = new (outHeap) ItmSeqOlapFunction(getOperatorType(), child(0));
  } else
    result = derivedNode;
...
NABoolean ItmSeqOlapFunction::inverseOLAPOrder(CollHeap *heap) {
...
NABoolean ItmSeqOlapFunction::hasEquivalentProperties(ItemExpr *other) {
  ...
  return (this->frameStart_ == otherOSeq->frameStart_ && this->frameEnd_ == otherOSeq->frameEnd_);
}
...
// -----------------------------------------------------------------------
int ItmSeqOffset::getArity() const {
  if (child(1)) {
    if (child(2))
      return 3;
    else
...
ItemExpr *ItmSeqOffset::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ...
  if (derivedNode == NULL) {
    switch (getArity()) {
      case 2:
        result = new (outHeap) ItmSeqOffset(child(0), child(1), NULL, nullRowIsZero());
        break;

      case 3:
        result = new (outHeap) ItmSeqOffset(child(0), child(1), child(2), nullRowIsZero());
        break;

      default:
        result =
            new (outHeap) ItmSeqOffset(child(0), nullRowIsZero(), getOffsetConstantValue(), isLeading(), winSize());
        break;
    }
...
NABoolean ItmSeqOffset::hasEquivalentProperties(ItemExpr *other) {
  ...
  return (this->offsetConstantValue_ == otherOffset->offsetConstantValue_) &&
         (this->nullRowIsZero_ == otherOffset->nullRowIsZero_) && (this->leading_ == otherOffset->leading_) &&
...
ItemExpr *ItmSeqRowsSince ::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result;
...
ItemExpr *ItmSeqMovingFunction::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result;

  if (derivedNode == NULL) {
    result = new (outHeap) ItmSeqMovingFunction(getOperatorType(), child(0), child(1), child(2));
...
NABoolean ItmSeqMovingFunction::hasEquivalentProperties(ItemExpr *other) {
  ...
  return (this->skipMovingMinMaxTransformation_ == otherMSeq->skipMovingMinMaxTransformation_);
}
// -----------------------------------------------------------------------
...
ItemExpr *ItmSeqThisFunction::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result;
...
ItemExpr *ItmSeqNotTHISFunction::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result;
...
NABoolean ItemExpr::containsOneRowAggregate() {
  if (getOperatorType() == ITM_ONE_ROW) return TRUE;

  for (int i = 0; i < getArity(); i++) {
    if (child(i)->castToItemExpr()->containsOneRowAggregate()) return TRUE;
  }
  return FALSE;
}  // ItemExpr::containsOneRowAggregate()

NABoolean ItemExpr::containsOpType(OperatorTypeEnum opType) const {
  if (getOperator().match(opType)) return TRUE;

  for (int i = 0; i < getArity(); i++) {
    if (child(i)->castToItemExpr()->containsOpType(opType)) return TRUE;
  }
  return FALSE;
...
NABoolean ItemExpr::markPathToUnTransformedNode() {
  ...
  if (!nodeIsTransformed()) retval = TRUE;
...
void HostVar::setPMOrdPosAndIndex(ComColumnDirection paramMode, int ordinalPosition, int index) {
...
NABoolean HostVar::isSystemGeneratedOutputHV() const {
  return (isSystemGenerated() && getName() == "_sys_ignored_CC_convErrorFlag");
}

NABoolean HostVar::isSystemGeneratedMinMaxHV() const {
  return (isSystemGenerated() &&
          (getName().index("_sys_MinVal", 0) != NA_NPOS || getName().index("_sys_MaxVal", 0) != NA_NPOS));
}

NABoolean HostVar::isSystemGeneratedRangeOfValuesHV() const {
  return (isSystemGenerated() && getName().index("_sys_RangeOfValues", 0) != NA_NPOS);
}

Int16 HostVar::extractFilterId() {
  size_t p = getName().index("_sys_RangeOfValues", 0);

  if (p != NA_NPOS) {
    p += strlen("_sys_RangeOfValues");
    return (Int16)(atoi(getName().data() + p));
  }

  return -1;
}

void DynamicParam::setPMOrdPosAndIndex(ComColumnDirection paramMode, int ordinalPosition, int index) {
  paramMode_ = paramMode;
  ordinalPosition_ = ordinalPosition;
  dpIndex_ = index;
}
...
int ItemExpr::getOrdinalPosition() const {
  CMPASSERT(0);
  return -1;
}

int ItemExpr::getHVorDPIndex() const {
  CMPASSERT(0);
  return -1;
}
...
NABoolean ItemExpr::isARangePredicate() const {
  ...
  if ((oper == ITM_GREATER) || (oper == ITM_GREATER_EQ) || (oper == ITM_LESS) || (oper == ITM_LESS_EQ)) {
    return TRUE;
  }
...
QR::ExprElement Function::getQRExprElem() const { return QR::QRFunctionElem; }
...
ItemExpr *ItemExpr::reverseTree() {
  ...
  // only 1 column.
  if (grammarTree->getOperatorType() != ITM_ITEM_LIST) return grammarTree;
...
QR::ExprElement ItemExpr::getQRExprElem() const { return QR::QRNoElem; }

QR::ExprElement ConstantParameter::getQRExprElem() const { return QR::QRScalarValueElem; }

// Constructor for the wrapper class RangeSpecRef:
RangeSpecRef::RangeSpecRef(OperatorTypeEnum otype, OptNormRangeSpec *range, ItemExpr *colValueId, ItemExpr *reConsIExpr)
    : ItemExpr(ITM_RANGE_SPEC_FUNC, colValueId, reConsIExpr), range_(range) {
  if (reConsIExpr && (CmpCommon::getDefault(COMP_BOOL_109) == DF_ON)) {
    // rewrite child(1) so that predicates of the form X < c OR X > c appear as X <> c
    // (this gives much better histogram estimates when c is an extremal value)
    ValueIdSet vs;
    getValueIdSetForReconsItemExpr(vs /* out */);
    transformTowardNotEquals(vs);  // do transformation
    ItemExpr *transformedReConsIExpr = vs.rebuildExprTree(ITM_AND);
    child(1) = transformedReConsIExpr->getValueId();
  }
}
...
<subranges> */
const NAString RangeSpecRef::getText() const { return "Range in"; }

short RangeSpecRef::mdamPredGen(Generator *generator, MdamPred **head, MdamPred **tail, MdamCodeGenHelper &mdamHelper,
                                ItemExpr *parent) {
  // return dummy;
  return 0;
}

int RangeSpecRef::getArity() const { return 2; }
...
void RangeSpecRef::unparse(NAString &result, PhaseEnum phase, UnparseFormatEnum form, TableDesc *tabId) const {
...
ItemExpr *RangeSpecRef::removeRangeSpecItems(NormWA *normWA) { return getRangeObject()->getRangeItemExpr(normWA); };
...
void RangeSpecRef::transformTowardNotEquals(ValueIdSet &inOutVs) {
  // If there is more than one entry, inOutVs is an AND tree; there
  // are no OR trees to transform. (Note: We expect at least one
  // entry, but if there are none, there is no harm in returning.)
  if (inOutVs.entries() != 1) return;

  ValueId theOne;
  inOutVs.getFirst(theOne);
  ItemExpr *theOneTree = theOne.getItemExpr();
  if (theOneTree->getOperatorType() != ITM_OR) return;  // no OR subtrees to transform

  ValueIdList sourceList;

  // flatten the OR tree, placing the children of the ORs into
  // the source list, then copy that to an array for convenience
  int result = theOneTree->convertToValueIdList(sourceList /* out */, NULL, ITM_OR);
  ValueIdArray source(sourceList, sourceList.entries());  // for efficient operator[]

  // RangeSpecRef::getValueIdSetForReconsItemExpr has the property
  // that intervals are represented in theOneTree in order; so
  // we can step through the ValueIdList comparing consecutive
  // interval endpoints. When we find an interval ending in  X < c
  // and the next beginning with X > c, glue the two together into
  // something like [... AND] X <> c [AND ...].
  ValueIdArray target;
  for (CollIndex i = 0; i < source.entries() - 1; i++) {
    NABoolean unchanged = TRUE;

    // flatten current AND tree (if it isn't an AND, we get just one entry)
    ValueIdList current;
    result = source[i].getItemExpr()->convertToValueIdList(current /* out */, NULL, ITM_AND);
    ValueId currentEndpoint = current.at(current.entries() - 1);
    if (currentEndpoint.getItemExpr()->getOperatorType() == ITM_LESS) {
      ValueIdList next;
      result = source[i + 1].getItemExpr()->convertToValueIdList(next /* out */, NULL, ITM_AND);
      ValueId nextStartpoint = next.at(0);
      if (nextStartpoint.getItemExpr()->getOperatorType() == ITM_GREATER) {
        ValueId currentEndpointValue = currentEndpoint.getItemExpr()->child(1);
        ItemExpr *cevie = currentEndpointValue.getItemExpr();
        ValueId nextStartpointValue = nextStartpoint.getItemExpr()->child(1);
        ItemExpr *nsvie = nextStartpointValue.getItemExpr();
        DCMPASSERT(cevie->getOperatorType() == ITM_CONSTANT && nsvie->getOperatorType() == ITM_CONSTANT);
        if (cevie->getText() == nsvie->getText()) {
          // We found the ... X > c OR X < c case so glue current
          // and next together

          ValueId X = currentEndpoint.getItemExpr()->child(0);
          BiRelat *notEquals = new (CmpCommon::statementHeap()) BiRelat(ITM_NOT_EQUAL, X.getItemExpr(), cevie);
          notEquals->synthTypeAndValueId(TRUE);

          // do the gluing by creating an AND tree of all ValueIds in current
          // except current[current.entries()-1] (which is X < c), notEquals,
          // and those in next except for next[0] (which is X > c)

          ItemExpr *resultExpr = NULL;
          if (current.entries() > 1) resultExpr = current.at(0).getItemExpr();
          for (CollIndex j = 1; j < current.entries() - 1; j++) {
            resultExpr = new (CmpCommon::statementHeap()) BiLogic(ITM_AND, resultExpr, current.at(j).getItemExpr());
            resultExpr->synthTypeAndValueId(TRUE);
          }
          if (resultExpr) {
            resultExpr = new (CmpCommon::statementHeap()) BiLogic(ITM_AND, resultExpr, notEquals);
            resultExpr->synthTypeAndValueId(TRUE);
          } else
            resultExpr = notEquals;
          for (CollIndex k = 1; k < next.entries(); k++) {
            resultExpr = new (CmpCommon::statementHeap()) BiLogic(ITM_AND, resultExpr, next.at(k).getItemExpr());
            resultExpr->synthTypeAndValueId(TRUE);
          }
          source[i + 1] = resultExpr->getValueId();
          unchanged = FALSE;
        }
      }
    }
    if (unchanged) target.insertAt(target.entries(), source[i]);
  }

  // add in the last one from sourc
  target.insertAt(target.entries(), source[source.entries() - 1]);

  if (target.entries() == source.entries()) return;  // nothing was transformed, so just return

  // create an OR tree of the transformed expressions
  ItemExpr *resultExprOr = target[0].getItemExpr();
  for (CollIndex m = 1; m < target.entries(); m++) {
    resultExprOr = new (CmpCommon::statementHeap()) BiLogic(ITM_AND, resultExprOr, target[m].getItemExpr());
    resultExprOr->synthTypeAndValueId(TRUE);
  }

  // write the transfomred OR tree back out as the only element of inOutVs
  inOutVs = resultExprOr->getValueId();
}

ItemExpr *revertBackToOldTree(CollHeap *heap, ItemExpr *newTree) {
  if (newTree->getOperatorType() != ITM_AND && newTree->getOperatorType() != ITM_OR) {
...
void revertBackToOldTreeUsingValueIdSet(ValueIdSet &inputSet /* IN */, ValueIdSet &outputSet /* OUT */) {
  ...
  for (ValueId predId = inputSet.init(); inputSet.next(predId); inputSet.advance(predId)) {
    if (predId.getItemExpr()->getOperatorType() == ITM_RANGE_SPEC_FUNC) {
      ...
      for (ValueId predIdOr = orSet.init(); orSet.next(predIdOr); orSet.advance(predIdOr)) {
      ...
      if (outorSet.entries()) inputItemExprTree = outorSet.rebuildExprTree(ITM_OR, FALSE, FALSE);
    } else {
      outputSet += predId;
      if (inputItemExprTree != NULL) {
        outputSet += inputItemExprTree->getValueId();
...
ConstValue *ItemExpr::evaluate(CollHeap *heap) {
  ...
  ex_expr::exp_return_type rc = exprs.evalAtCompileTime(0, ExpTupleDesc::SQLARK_EXPLODED_FORMAT, decodeBuf,
                                                        decodeBufLen, &resultLength, &resultOffset, CmpCommon::diags());
  ...
  if (rc == ex_expr::EXPR_OK) {
    ...
    if (dataType.getSQLnullHdrSize() > 0)
      memmove(&decodeBuf[resultOffset - dataType.getPrefixSize()], decodeBuf, dataType.getPrefixSize());
    result = new (heap) ConstValue(&dataType, (void *)&(decodeBuf[resultOffset - dataType.getPrefixSize()]),
                                   resultLength + dataType.getPrefixSize(), NULL, heap);
  }
...
ItemExpr *ItmLeadOlapFunction::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result;
...
// Called during PhysicalSequence::preCodeGen(). This method add a NotCovered()
// on top of the offset expression (child 1) and optionally the default expression (child 2) to prevent it from being
// skipped during PhysicalSequence::codeGen() in the read expression.
ItemExpr *ItmLeadOlapFunction::transformOlapFunction(CollHeap *heap) {
  if (getArity() <= 1) return this;

  for (int i = 1; i <= getArity() - 1; i++) {
    ItemExpr *newChild = new (heap) NotCovered(child(i));
...
ItemExpr *ItmOlapFirstLastValue::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItmOlapFirstLastValue *result;

  if (derivedNode == NULL) {
    result = new (outHeap) ItmOlapFirstLastValue(getOperatorType(), child(0));
    result->counterVid() = counterVid();
  } else
    result = (ItmOlapFirstLastValue *)derivedNode;

  return ItmSeqOlapFunction::copyTopNode(result, outHeap);
}

ItemExpr *ItmOlapPivotGroup::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItmOlapPivotGroup *result;

  if (derivedNode == NULL) {
    result = new (outHeap)
        ItmOlapPivotGroup(getOperatorType(), child(0), getOlapOrderBy(), getPivotDelimiter(), getPivotMaxLen());
  } else
    result = (ItmOlapPivotGroup *)derivedNode;
...
ItemExpr *ItemExpr::doBinaryRemoveNonPushablePredicatesForORC(NABoolean allowBranchTrimOff) {
  ...
  if (child(0) == NULL) return (allowBranchTrimOff) ? child(1) : NULL;

  if (child(1) == NULL) return (allowBranchTrimOff) ? child(0) : NULL;
...
ItemExpr *Function::removeNonPushablePredicatesForORC() {
  switch (getOperatorType()) {
    // Allow push-down of min of min and max of max
    // (used in min/max optimization),
    case ITM_SCALAR_MAX:
    case ITM_SCALAR_MIN:
      break;
...
void ItemExpr::bindPriorItem(BindWA *bindWA) {
  if (getOperatorType() == ITM_EQUAL) {
    if (((BiRelat *)this)->isConnectByPriorPred() == TRUE) {
      bindNode(bindWA);
      bindWA->addToConnectByVidSet(this);
    }
  }
  // recursviely go
  int nc = getArity();
...
NABoolean ItemExpr::isInvolvingAColumn() {
  if (isAColumnReference()) return TRUE;
...
NABoolean ItemExpr::isAConstantHostVarParameterOrFunc() {
  ...
  if (oper == ITM_CONSTANT || oper == ITM_HOSTVAR || oper == ITM_DYN_PARAM || oper == ITM_CACHE_PARAM)
    return TRUE;
  else if (oper == ITM_CAST) {
    Cast *c = (Cast *)this;
    if ((c->matchChildType()) && (c->child(0)->getOperatorType() == ITM_DYN_PARAM)) return TRUE;
  }

  if (isInvolvingAColumn()) return FALSE;
...
NABoolean ItemExpr::isIsNullOrIsNotNullPred() const {
  OperatorTypeEnum oper = getOperatorType();
  return (oper == ITM_IS_NULL || oper == ITM_IS_NOT_NULL);
}

NABoolean BaseColumn::appearInALocalPredTogether(ItemExpr *other) {
  if (other->getOperatorType() == ITM_BASECOLUMN) {
    if (((BaseColumn *)other)->getTableDesc()->getCorrNameObj() == getTableDesc()->getCorrNameObj()) {
      return TRUE;
    }
  } else if (other->getOperatorType() == ITM_INDEXCOLUMN) {
    ItemExpr *bc = ((IndexColumn *)other)->getDefinition().getItemExpr();

    if (bc->getOperatorType() == ITM_BASECOLUMN) {
      if (((BaseColumn *)bc)->getTableDesc()->getCorrNameObj() == getTableDesc()->getCorrNameObj()) {
        return TRUE;
      }
    }
  }

  TableAnalysis *tAnalysis = (TableAnalysis *)getTableDesc()->getTableAnalysis();

  if (tAnalysis && tAnalysis->referencedInLocalPred(other)) return TRUE;

  return FALSE;
}

NABoolean IndexColumn::appearInALocalPredTogether(ItemExpr *other) {
  ItemExpr *bc = getDefinition().getItemExpr();

  if (bc) return bc->appearInALocalPredTogether(other);
...
NABoolean BiRelat::isAPredicateBetweenColumnAndExpression() {
  ...
  if (leftC->isInvolvingAColumn() && rightC->isAConstantHostVarParameterOrFunc()) return TRUE;

  if (rightC->isInvolvingAColumn() && leftC->isAConstantHostVarParameterOrFunc()) return TRUE;

  if (leftC->isInvolvingAColumn() && rightC->isInvolvingAColumn() && (!leftC->appearInALocalPredTogether(rightC)) &&
      (!rightC->appearInALocalPredTogether(leftC)))
    return TRUE;
...
ItemExpr *UnLogic::removeNonPushablePredicatesForORC() {
  switch (getOperatorType()) {
    ...
    case ITM_IS_NULL: {
      const NAType &typ = child(0)->getValueId().getType();

      // ISNULL for date and timestam is not pushable.
      // See mantis 3340 for details.
      if (typ.getHiveType() == HIVE_DATE_TYPE || typ.getHiveType() == HIVE_TIMESTAMP_TYPE) return NULL;
    ...
    case ITM_IS_NOT_NULL: {
      ...
      // See mantis 3340 for details.
      //
      // Since IS NOT NULL will be translated to NOT(ISNULL(col)),
      // we apply the same test here.
      if (typ.getHiveType() == HIVE_DATE_TYPE || typ.getHiveType() == HIVE_TIMESTAMP_TYPE) return NULL;
...
ItemExpr *VEGPredicate::removeNonPushablePredicatesForORC() { return this; }
...
void BiLogic::generatePushdownListForExtStorage(ExtPushdownPredInfoList &result) {
  NABoolean inListOpt = CURRSTMT_OPTDEFAULTS->minMaxINlistOpt();
  ...
  switch (getOperatorType()) {
    ...
    case ITM_OR: {
      ValueId colId;
      ValueIdList vidList;
      if (inListOpt && convertToInListFeasible(colId, vidList)) {
        result.insertIN(colId, vidList);
        return;
      } else
    ...
    default:
      CMPASSERT("Invalid biLogic operator to convert to ExtPushdownPredInfo");
  }

  child(0)->generatePushdownListForExtStorage(result);
  child(1)->generatePushdownListForExtStorage(result);

  result.insertEND();
}

void UnLogic::generatePushdownListForExtStorage(ExtPushdownPredInfoList &result) {
  ...
  if (getOperatorType() == ITM_IS_NULL) {
    ...
    CMPASSERT("Invalid UnLogic operator to convert to ExtPushdownPredInfo");
}

void BiRelat::generatePushdownListForExtStorage(ExtPushdownPredInfoList &result) {
  switch (getOperatorType()) {
    case ITM_EQUAL: {
    ...
    default:
      CMPASSERT("Invalid bi-relat operator to convert to ExtPushdownPredInfo");
  }
}

// Check if this anchors a tree of BiLogic of OR predicates with
// EQUAL predicate in form <col> == <constant>. <col> has to be
// the same for all OR predicates.
NABoolean BiLogic::convertToInListFeasible(ValueId &colId, ValueIdList &result) {
  switch (getOperatorType()) {
    case ITM_OR:
      return child(0)->convertToInListFeasible(colId, result) && child(1)->convertToInListFeasible(colId, result);
      break;
...
NABoolean BiRelat::convertToInListFeasible(ValueId &colId, ValueIdList &result) {
  ItemExpr *leftC = child(0);
  ItemExpr *rightC = child(1);
  ...
  if (leftC->isInvolvingAColumn()) {
    if (colId == NULL_VALUE_ID) {
      colId = leftC->getValueId();
    } else {
      if (colId != leftC->getValueId()) return FALSE;
    }

    if (rightC->getOperatorType() == ITM_CONSTANT) {
      result.insert(rightC->getValueId());
      return TRUE;
    } else
      return FALSE;
  } else if (rightC->isInvolvingAColumn()) {
    if (colId == NULL_VALUE_ID) {
      colId = rightC->getValueId();
    } else {
      if (colId != rightC->getValueId()) return FALSE;
    }

    if (leftC->getOperatorType() == ITM_CONSTANT) {
      result.insert(leftC->getValueId());
      return TRUE;
    } else
      return FALSE;
  }
...
const NAString RangeCompare::getText() const { return "range_in"; }

ItemExpr *RangeCompare::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result;
  if (derivedNode == NULL) {
    result = new (outHeap) RangeCompare(NULL, NULL);
  } else
    result = derivedNode;
...
void RangeCompare::generatePushdownListForExtStorage(ExtPushdownPredInfoList &result) {
  result.insertIN(child(0)->getValueId(), child(1)->getValueId());
}

ItemExpr *RangeCompare::removeNonPushablePredicatesForORC() { return this; }

NABoolean ItemExpr::allValueIdsAreInvalid() {
  if (getValueId() != NULL_VALUE_ID) return FALSE;

  int arity = getArity();

  for (int i = 0; i < arity; i++) {
    if (child(i)) {
      if (child(i)->allValueIdsAreInvalid() == FALSE) return FALSE;
    }
  }
  return TRUE;
}

Int16 ItemExpr::getFilterId() {
  // extract and transfer it to the Aggregate
  if (getOperatorType() == ITM_HOSTVAR) {
    HostVar *hv = (HostVar *)this;
    return hv->extractFilterId();
  }

  return -1;
}

NABoolean Aggregate::isRangeOfValuesAggregate() {
  switch (getOperatorType()) {
    case ITM_RANGE_VALUES_MERGE:
    case ITM_RANGE_VALUES_INSERT:
    case ITM_RANGE_VALUES_COPY:
...
// -----------------------------------------------------------------------
// methods for class ExprHistDummy
// -----------------------------------------------------------------------
ExprHistDummy::ExprHistDummy() : ItemExpr(ITM_EXPR_HIST_DUMMY) {
  // nothing else to do
}

ExprHistDummy::~ExprHistDummy() {
  // nothing to do
}

int ExprHistDummy::getArity() const { return 0; }

void ExprHistDummy::unparse(NAString &result, PhaseEnum phase, UnparseFormatEnum form, TableDesc *tabId) const {
  result += "ExprHistDummy";
}

const NAString ExprHistDummy::getText() const { return "ExprHistDummy"; }

ItemExpr *ItmSysConnectByPathFunc::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItmSysConnectByPathFunc *result = NULL;

  if (derivedNode == NULL)
    result = new (outHeap) ItmSysConnectByPathFunc();
  else
    result = (ItmSysConnectByPathFunc *)derivedNode;
  result->delimiter_ = delimiter_;
  return ZZZBinderFunction::copyTopNode(result, outHeap);
}

ItemExpr *BeginKey::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result = NULL;
  if (derivedNode == NULL)
    result = new (outHeap) BeginKey(child(0));
  else
    result = derivedNode;
...
ItemExpr *EndKey::copyTopNode(ItemExpr *derivedNode, CollHeap *outHeap) {
  ItemExpr *result = NULL;
  if (derivedNode == NULL)
    result = new (outHeap) EndKey(child(0), bytesPerChar_, maxValue_);
  else
    result = derivedNode;
