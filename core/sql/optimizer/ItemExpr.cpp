                                    ...
                                    NABoolean doNotDigInsideInstNulls) const
{
  ...
  if (getValueId() == vid || getInitVId() == vid)
    return TRUE;
...
ItemExpr * ItemExpr::rewriteNotEquals(BindWA * bindWA)
{
  return this;
}
    ...
    switch ( myType )
      ...
      case ITM_ROW_SUBQUERY:
        if(wantedType == ITM_DYN_PARAM) {
          ...
          for (ValueId x= ((Subquery*)this)->dynParamInStartWith_.init(); ((Subquery*)this)->dynParamInStartWith_.next(x); ((Subquery*)this)->dynParamInStartWith_.advance(x) )
          {
...
ItemExpr * ItemExpr::applyInverseDistributivityLawByColumn(
     OperatorTypeEnum backboneType,
     OperatorTypeEnum innerType)
{
  // operators other than OR will require some more code, see CMPASSERTs
  ...
  if ((backboneType == ITM_OR) && (getOperatorType() == ITM_AND))
    {
      ItemExpr * newChild0 = child(0)->applyInverseDistributivityLawByColumn(
        backboneType, innerType);
      newChild0->synthTypeAndValueId();
      child(0) = newChild0;
      ItemExpr * newChild1 = child(1)->applyInverseDistributivityLawByColumn(
        backboneType, innerType);
  ...
  firstDisjunctConjuncts.findAndPrimeNiceColumnComparisons(commonColumns,newExpressions,
    backboneType,innerType);
  ...
  for (disjuncts.advance(c); disjuncts.next(c); disjuncts.advance(c))
    {
      ValueIdSet conjuncts;
      c.getItemExpr()->convertToValueIdSet(conjuncts,NULL,innerType,FALSE);
      if (conjuncts.entries() > maxNumConjuncts)
        maxNumConjuncts = conjuncts.entries();

      // find the interesting columns in this conjunct and intersect them
      // with the commonColumns
      conjuncts.findCommonNiceColumnComparisons(commonColumns,newExpressions,
        backboneType,innerType);
  ...
  for (Int32 i = 0; i < arity; i++)
    result2->setChild(i,result->child(i));
...
void ItemExpr::display(UnparseFormatEnum form) 
{
  display(cout, form);
}
                                             ...
                                             )
{
  ...
  if ( assignValueId ) {
    for(Int32 i=0; i<ie->getArity(); i++)
      if (ie->child(i)->getOperatorType() == ITM_NOTCOVERED &&
...
void BiArith::unparse(NAString &result,
                      PhaseEnum phase,
                      UnparseFormatEnum form,
                      TableDesc * tabId) const 
{
...
const NAString ColReference::getText() const
{
  NAColumn *nacol = getValueId() != NULL_VALUE_ID ?
    getValueId().getNAColumn(true /* ok if not a column */) :
    NULL;
  return nacol ?
...
const NAString HostVar::getText() const
{
  ...
  if ( isSystemGeneratedRangeOfValuesHV() ) {
    ...
    if ( length_in_bytes < COM_ONE_MEG ) {
  ...
  if (!indicatorName_.isNull()) 
    result.append(" " + punc + indicatorName_);
  ...
  for (CollIndex i=0; i<sources_.entries(); i++)
    {
      ...
      if (s->getOperatorType() == ITM_VALUEIDUNION)
        {
          ValueIdUnion *childVu =
            static_cast<ValueIdUnion *>(s);
              ...
              if (tempLabelValues.entries() > 0)
                {
                  ...
                  // duplicates
                  if (!tempLabelValues.newConstantsAreDistinct(
                                                   *(childVu->labelValues_)))
      ...
      else if (s->doesExprEvaluateToConstant(TRUE,TRUE))
        {
          ...
          if (tempLabelValues.entries() > 0 &&
              !tempLabelValues.newConstantsAreDistinct(vs))
...
ValueId VEG::getAConstant(NABoolean includeCacheParam, 
			  NABoolean lookBeneathVEG) const
{
  for (ValueId id = eqGroup_.init(); eqGroup_.next(id); eqGroup_.advance(id))
  {
    ItemExpr *ie = id.getItemExpr();
			   ...
			   TableDesc * tabId) const
{
  ...
  if (form == QUERY_FORMAT || form == USTAT_EXPRESSION_FORMAT || 
      form == MVINFO_FORMAT || form == CONNECT_BY_FORMAT)
    startIndex = 0;
  else 
    startIndex = 1;
  ...
  for (CollIndex i=1; i<copyList.entries(); i++)
  {
    ...
    UnparseFormatEnum form1 = 
       ( getConstFromVEG &&
         nextMemberId.getItemExpr()->getOperatorType() == ITM_VEG_REFERENCE
       ) ? 
         QUERY_FORMAT_VEGREF : QUERY_FORMAT;
      ...
      */
        ...
        vegMembers.advance(someMemberId))
      {
        ...
        if ((someMemberExpr->getOperatorType() == ITM_BASECOLUMN) &&
            ...
            (form == USTAT_EXPRESSION_FORMAT) || (form == CONNECT_BY_FORMAT))
          {
...
const NAString Aggregate::getText() const
{
  ...
  if(seqOlapFuncInParser_)
    return seqOlapFuncInParser_->getText();
...
ItemExpr *Aggregate::transformOlapFunction(BindWA *bindWA)
{
  ...
  if ( currScope->getIsFirstOlapWindowSpec() )
        ...
        case ITM_OLAP_PIVOT_GROUP:
           {
             ...
             seqFunc = new (heap) ItmOlapPivotGroup(op, child(0), olapOrderBy_,
                                                                   child(1), pivot->maxLen());
           }
           break;
        default:
           {
    ...
    if (ITM_OLAP_PIVOT_GROUP == seqFunc->getOperatorType() &&
         ITM_PIVOT_GROUP == getOperatorType())
    {
        PivotGroup *pivot = (PivotGroup *)this;
...
{ 
    ...
    if(seqOlapFuncInParser_)
    	((ItmSequenceFunction*)seqOlapFuncInParser_)->setOlapOrderBy(orderBy);
}
...
{ 
    ...
    if(seqOlapFuncInParser_)
    	((ItmSequenceFunction*)seqOlapFuncInParser_)->setOlapPartitionBy(partitionBy);
}
	...
	case ITM_TOKENSTR:
	  {
            if (getArity() == 3)
              result = new (outHeap) 
                BuiltinFunction(getOperatorType(),
                                outHeap, 3, child(0), child(1), child(2));
            else
              result = new (outHeap) 
                BuiltinFunction(getOperatorType(),
                                outHeap, 2, child(0), child(1));
...
// -----------------------------------------------------------------------
ItemExpr * MonthsBetween::copyTopNode(ItemExpr *derivedNode,
                                        CollHeap* outHeap)
{
  ItemExpr *result;
...
ItemExpr * BiRelat::copyTopNode(ItemExpr *derivedNode, CollHeap* outHeap)
{
  ...
  if(parentIe_) 
    result->parentIe_= parentIe_->copyTree(outHeap)->castToItemExpr();
  if(childIe_)
    result->childIe_ = childIe_->copyTree(outHeap)->castToItemExpr();
  return ItemExpr::copyTopNode(result, outHeap);
}
  ...
  if (getOperatorType() == ITM_NOT_EQUAL)
    {
      ...
      ItemExpr * le = new(CmpCommon::statementHeap()) BiRelat(ITM_LESS,
        child(0),child(1));
      ItemExpr * ge = new(CmpCommon::statementHeap()) BiRelat(ITM_GREATER,
        child(0),child(1));
...
: ItemExpr(ITM_CONSTANT), isNull_(IS_NOT_NULL),
  ...
  isSystemSupplied_(FALSE), locale_wstrval(0), rebindNeeded_(FALSE), heap_(NULL)
{
  ...
  , heap_(outHeap)
{
  ...
  locale_strval = ( otherLocaleString ) ? 
                 new (outHeap) NAString(*otherLocaleString, outHeap) : NULL;

  NAWString* otherLocaleWString = other->getLocaleWString();
  locale_wstrval = (otherLocaleWString) ? 
                 new (outHeap) NAWString(*otherLocaleWString, outHeap) : NULL;
}
  ...
  , heap_(h)
{
  ...
  locale_strval = ( otherLocaleString ) ? 
                 new (h) NAString(*otherLocaleString, h) : NULL;

  const NAWString* otherLocaleWString = ((ConstValue &)s).getLocaleWString();
  locale_wstrval = (otherLocaleWString) ? 
                 new (h) NAWString(*otherLocaleWString, h) : NULL;
}
  ...
  , heap_(NULL)
{
  ...
  locale_strval = ( otherLocaleString ) ? 
                 new NAString(*otherLocaleString) : NULL;

  const NAWString* otherLocaleWString = ((ConstValue &)s).getLocaleWString();
  locale_wstrval = (otherLocaleWString) ? 
                 new NAWString(*otherLocaleWString) : NULL;
}
...
ConstValue::~ConstValue()
{
  ...
  if (text_)
    NADELETEBASIC((NAString*)text_,h);
}
...
NAString ItemList::getConstantTextNullReplace(NAString replacement, NABoolean reverseOrder)
{
  ...
  do
  {
    ...
    else
    {
      ...
      {
        if (cv->getType() &&
            cv->getType()->getTypeQualifier() == NA_CHARACTER_TYPE)
          if (reverseOrder)
...
const NAString MaxBytes::getText() const
{
  return "max_bytes";
}
...
ItemExpr * LOBinsert::copyTopNode(ItemExpr *derivedNode, CollHeap* outHeap)
{
  ...
  if (derivedNode == NULL)
    result = new (outHeap) LOBinsert(NULL, NULL, obj_, append_, lobAsVarchar_);
  else
...
ItemExpr * LOBconvert::copyTopNode(ItemExpr *derivedNode, CollHeap* outHeap)
{
  ...
  if (derivedNode == NULL)
    result = new (outHeap) LOBconvert(NULL,obj_,tgtSize_, position_, charSet_);
  else
    result = derivedNode;
...
ItemExpr * LOBlength::copyTopNode(ItemExpr *derivedNode, CollHeap* outHeap)
{
  ItemExpr *result;

  if (derivedNode == NULL)
    result = new (outHeap) LOBlength(NULL);
  else
    result = derivedNode;
...
ItemExpr * ItmSeqRunningPivotGroup::copyTopNode(ItemExpr *derivedNode,
                                                                     CollHeap* outHeap)
{
  ItemExpr * result = NULL;
  ...
  if (derivedNode == NULL)
    {
      result = new (outHeap) ItmSeqRunningPivotGroup(getOperatorType(),
                                                  child(0),
                                                  getOlapOrderBy(),
                                                  getPivotDelimiter(),
                                                  getPivotMaxLen());
...
{  
  return (isSystemGenerated() &&
          ( getName().index("_sys_MinVal", 0) != NA_NPOS ||
            getName().index("_sys_MaxVal", 0) != NA_NPOS ));
}

NABoolean HostVar::isSystemGeneratedRangeOfValuesHV() const
{  
  return (isSystemGenerated() &&
            getName().index("_sys_RangeOfValues", 0) != NA_NPOS );
}
...
void RangeSpecRef::transformTowardNotEquals(ValueIdSet & inOutVs)
{
  ...
  if (inOutVs.entries() != 1)
    return;

  ValueId theOne;
  inOutVs.getFirst(theOne);
  ItemExpr * theOneTree = theOne.getItemExpr();
  if (theOneTree->getOperatorType() != ITM_OR)
    return;  // no OR subtrees to transform
  ...
  Int32 result = theOneTree->convertToValueIdList(sourceList /* out */,
                                                  NULL,
                                                  ITM_OR);
  ...
  for (CollIndex i = 0; i < source.entries()-1; i++)
    {
      ...
      ValueIdList current;
      result = source[i].getItemExpr()->convertToValueIdList(current /* out */,
                                                  NULL,
                                                  ITM_AND);
      ...
      if (currentEndpoint.getItemExpr()->getOperatorType() == ITM_LESS)
        {
          ValueIdList next;
          result = source[i+1].getItemExpr()->convertToValueIdList(next /* out */,
                                                  NULL,
                                                  ITM_AND);
          ...
          if (nextStartpoint.getItemExpr()->getOperatorType() == ITM_GREATER)
            {
              ...
              DCMPASSERT(cevie->getOperatorType() == ITM_CONSTANT
                      && nsvie->getOperatorType() == ITM_CONSTANT);
              if (cevie->getText() == nsvie->getText())
                {
                  ...
                  BiRelat * notEquals = new (CmpCommon::statementHeap()) BiRelat(ITM_NOT_EQUAL,
                    X.getItemExpr(),
                    cevie);
                  ...
                  if (current.entries() > 1)
                    resultExpr = current.at(0).getItemExpr();
                  for (CollIndex j = 1; j < current.entries()-1; j++)
                    {
                      resultExpr = new (CmpCommon::statementHeap()) BiLogic(ITM_AND,
                        resultExpr, current.at(j).getItemExpr());
                  ...
                  for (CollIndex k = 1; k < next.entries(); k++)
                    {
                      resultExpr = new (CmpCommon::statementHeap()) BiLogic(ITM_AND,
                        resultExpr, next.at(k).getItemExpr());
                      resultExpr->synthTypeAndValueId(TRUE);
                    }
      ...
      if (unchanged)
        target.insertAt(target.entries(),source[i]);
    }

  // add in the last one from sourc
  target.insertAt(target.entries(),source[source.entries()-1]);

  if (target.entries() == source.entries())
    return;  // nothing was transformed, so just return
  ...
  for (CollIndex m = 1; m < target.entries(); m++)
    {
      resultExprOr = new (CmpCommon::statementHeap()) BiLogic(ITM_AND,
                        resultExprOr, target[m].getItemExpr());
...
// on top of the offset expression (child 1) and optionally the default expression (child 2) to prevent it from being skipped during
// PhysicalSequence::codeGen() in the read expression. 
...
ItemExpr * ItmOlapPivotGroup::copyTopNode(ItemExpr *derivedNode, CollHeap* outHeap)
{
    ...
    if (derivedNode == NULL){
      result = new (outHeap) ItmOlapPivotGroup(getOperatorType(), child(0),
                                       getOlapOrderBy(), getPivotDelimiter(), getPivotMaxLen());
    }
    else
...
NABoolean ItemExpr::isAConstantHostVarParameterOrFunc() 
{
  ...
  if (oper == ITM_CONSTANT  || oper == ITM_HOSTVAR   ||
  ...
  else if (oper == ITM_CAST)
    {
      Cast * c = (Cast*)this;
      if ((c->matchChildType()) &&
          (c->child(0)->getOperatorType() == ITM_DYN_PARAM))
        return TRUE;
    }

  if ( isInvolvingAColumn() )
    return FALSE;
...
NABoolean BaseColumn::appearInALocalPredTogether(ItemExpr* other)
{
  if (other->getOperatorType() == ITM_BASECOLUMN)
    {
      if (((BaseColumn*)other)->getTableDesc()->getCorrNameObj() 
          == getTableDesc()->getCorrNameObj())
        {
          return TRUE;
        }
    }
  else if (other->getOperatorType() == ITM_INDEXCOLUMN)
    {
      ...
      if (bc->getOperatorType() == ITM_BASECOLUMN)
        {
          if (((BaseColumn*)bc)->getTableDesc()->getCorrNameObj() 
              == getTableDesc()->getCorrNameObj())
            {
              return TRUE;
            }
  ...
  if ( tAnalysis && tAnalysis->referencedInLocalPred(other) )
    return TRUE;
...
NABoolean IndexColumn::appearInALocalPredTogether(ItemExpr* other)
{
   ...
   if ( bc )
      return bc->appearInALocalPredTogether(other);
...
NABoolean BiRelat::isAPredicateBetweenColumnAndExpression()
{
  ...
  if ( leftC->isInvolvingAColumn() &&
       rightC->isAConstantHostVarParameterOrFunc() )
    return TRUE;

  if ( rightC->isInvolvingAColumn() &&
       leftC->isAConstantHostVarParameterOrFunc() )
    return TRUE;

  if ( leftC->isInvolvingAColumn() &&
       rightC->isInvolvingAColumn() &&
       (! leftC->appearInALocalPredTogether(rightC) ) &&
    ...
    case ITM_IS_NULL:
      {
        ...
        if (typ.getHiveType() == HIVE_DATE_TYPE ||
            typ.getHiveType() == HIVE_TIMESTAMP_TYPE)
           return NULL;
...
NABoolean BiLogic::convertToInListFeasible(ValueId& colId, ValueIdList& result)
{
  switch (getOperatorType()) {
    case ITM_OR:
      return child(0)->convertToInListFeasible(colId, result) &&
             child(1)->convertToInListFeasible(colId, result);
      break;
...
const NAString RangeCompare::getText() const
{
  return "range_in";
}
...
ItemExpr* RangeCompare::removeNonPushablePredicatesForORC()
{
  return this;
}
...
NABoolean ItemExpr::allValueIdsAreInvalid()
{
  ...
  for (Int32 i = 0; i < arity; i++) {
    if (child(i)) {
      if ( child(i)->allValueIdsAreInvalid() == FALSE )
        return FALSE;
    }
  }
  return TRUE;
...
Int32 ExprHistDummy::getArity() const
{
  return 0;
}

void ExprHistDummy::unparse(NAString &result,
                       PhaseEnum phase,
                       UnparseFormatEnum form ,
                       TableDesc * tabId) const
{
...
const NAString ExprHistDummy::getText() const
{
  return "ExprHistDummy";
}
...
ItemExpr * BeginKey::copyTopNode(ItemExpr *derivedNode ,
                                  CollHeap* outHeap)
{
  ItemExpr *result = NULL;
  if (derivedNode == NULL)
...
ItemExpr * EndKey::copyTopNode(ItemExpr *derivedNode ,
                                  CollHeap* outHeap)
{
  ItemExpr *result = NULL;
  if (derivedNode == NULL)
