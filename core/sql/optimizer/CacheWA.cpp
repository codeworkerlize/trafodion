...
ConstantParameters::ConstantParameters(NAHeap *h) : LIST(ConstantParameter *)(h) {}
...
SelParameters::SelParameters(NAHeap *h) : LIST(SelParameter *)(h) {}
...
CacheWA::CacheWA(NAHeap *h)
    : heap_(h),
      qryText_(cacheKeyInitStrLen, h),
      actuals_(h),
      sels_(h),
      parmTypes_(0),
      selTypes_(0),
      cacheable_(FALSE),
      conditionallyCacheable_(FALSE),
      tkey_(NULL),
      ckey_(NULL),
      phase_(CmpMain::END),
      topRoot_(NULL),
...
void CacheWA::incNofScans(TableDesc *td) {
  ...
  if (numberOfScans_ == tabArraySize_) {
    ...
    TableDescPtr *newT = (TableDescPtr *)heap_->allocateMemory(tabArraySize_ * sizeof(TableDescPtr));
    ValueIdSet **newK = (ValueIdSet **)heap_->allocateMemory(tabArraySize_ * sizeof(ValueIdSet *));
...
NABoolean CacheWA::isParameterizable(BaseColumn *base) {
  for (Int32 x = 0; x < numberOfScans_; x++) {
    if (tabDescPtr_[x] == base->getTableDesc()) {
      ...
      return usedKyPtr_[x]->coversFirstN(tabDescPtr_[x]->getClusteringIndex()->getClusteringKeyCols(),
                                         requiredPrefixKeys_);
    }
  }
  return FALSE;
...
const ParameterTypeList *CacheWA::getFormalParamTypes() {
  ...
  if (parmTypes_) {
    NADELETE(parmTypes_, ParameterTypeList, wHeap());
  }
...
const SelParamTypeList *CacheWA::getFormalSelParamTypes() {
  ...
  if (selTypes_) {
    NADELETE(selTypes_, SelParamTypeList, wHeap());
  }
...
CacheKey *CacheWA::getCacheKey(const QryStmtAttributeSet &attrs) {
  if (!ckey_) {
    ...
    // because the Key must hash to the same address as its cache entry.
    ckey_ = new (wHeap())
        CacheKey(qryText_, phase_, env, *getFormalParamTypes(), *getFormalSelParamTypes(), wHeap(), reqdShape_, isoLvl_,
                 accMode_, isoLvlIDU_, autoCmt_, flags_, rbackMode_, tables_, useView_, usePartitionTable_);
  }
  return ckey_;
}
...
void CacheWA::generateCacheKey(RelExpr *queryExpr, const char *sText, Lng32 charset, const NAString &viewsUsed) {
  ...
  if (isViewJoin_) {
    ...
    qryText_ += sText;
    char cs[20];
    sprintf(cs, "%d", charset);
    qryText_ += cs;
  }
  // save parameterized statement text into cwa.qryText_
  queryExpr->generateCacheKey(*this);
  // save cqdsInHint into cwa.qryText_
  generateCacheKeyFromCqdsInHint();
  if (CmpCommon::getDefault(SHOW_CACHE_KEY) == DF_ON) {
    QRLogger::log(CAT_SQL_COMP, LL_INFO, "generate CacheKey [%s]", qryText_.data());
  }
}

void CacheWA::generateCacheKeyFromCqdsInHint() {
  if (SqlParser_CurrentParser && SqlParser_CurrentParser->getHint() &&
...
NABoolean ConstantParameters::find(ConstValue *val, ConstantParameter **match) {
  CollIndex x, limit = entries();
  for (x = 0; x < limit; x++) {
    if (at(x)->matches(val)) {
      *match = at(x);
...
// return true & matching element if val is in this list
NABoolean SelParameters::find(ConstValue *val, SelParameter **match) {
  CollIndex x, limit = entries();
  for (x = 0; x < limit; x++) {
    if (at(x)->matches(val)) {
      *match = at(x);
...
void CacheWA::replaceWithNewOrOldConstParam(ConstValue *val, const NAType *typ, ExprValueId &tgt, BindWA &bindWA) {
  ...
  ConstantParameter *nParam = new (wHeap()) ConstantParameter(val, wHeap(), typ, ++posCounter_);
...
void CacheWA::replaceWithNewOrOldSelParam(ConstValue *val, const NAType *typ, const Selectivity sel, ExprValueId &tgt,
  ...
  SelParameter *nParam = new (wHeap()) SelParameter(val, wHeap(), typ, sel, ++posCounter_);
