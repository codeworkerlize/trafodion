...
static int valueIdHashKeyFunc(const ValueId &vid) {
  CollIndex x = vid;
  return (int)x;
}
...
void GroupAttributes::addConstraint(ItemExpr *c) {
  ...
  if (c->getValueId() == NULL_VALUE_ID) brandNewConstraint = TRUE;
  ...
  switch (c->getOperatorType()) {
    case ITM_CARD_CONSTRAINT: {
      ...
      if (cc->getLowerBound() > minRows OR cc->getUpperBound() < maxRows) {
        ...
        for (ValueId oc = constraints_.init(); constraints_.next(oc); constraints_.advance(oc))
          if (oc.getItemExpr()->getOperatorType() == ITM_CARD_CONSTRAINT) constraints_ -= oc;
      } else {
    ...
    case ITM_UNIQUE_OPT_CONSTRAINT: {
      ...
      for (ValueId ouc = constraints_.init(); constraints_.next(ouc); constraints_.advance(ouc)) {
        if (ouc.getItemExpr()->getOperatorType() == ITM_UNIQUE_OPT_CONSTRAINT) {
          const ValueIdSet &oucCols = ((UniqueOptConstraint *)ouc.getItemExpr())->uniqueCols();
        ...
        else if (ouc.getItemExpr()->getOperatorType() == ITM_FUNC_DEPEND_CONSTRAINT) {
          // try to reduce the new constraint by eliminating columns
          // that are functionally dependent on the rest
          ((FuncDependencyConstraint *)ouc.getItemExpr())->minimizeUniqueCols(uc->uniqueCols());
          ...
          if (((FuncDependencyConstraint *)ouc.getItemExpr())->getDeterminingCols().contains(uc->uniqueCols()))
            constraints_ -= ouc;
        }
    ...
    case ITM_FUNC_DEPEND_CONSTRAINT: {
      ...
      if (isUnique(fdc->getDeterminingCols())) {
        ...
        for (ValueId ofc = constraints_.init(); constraints_.next(ofc); constraints_.advance(ofc)) {
          if (ofc.getItemExpr()->getOperatorType() == ITM_FUNC_DEPEND_CONSTRAINT) {
            FuncDependencyConstraint *odc = (FuncDependencyConstraint *)ofc.getItemExpr();
            ...
            } else if (odcCols.contains(newDeterminingCols) AND newDependentCols.contains(odc->getDependentCols())) {
        ...
        if (modifiedNewConstraint) {
          c = new (CmpCommon::statementHeap()) FuncDependencyConstraint(newDeterminingCols, newDependentCols);
    ...
    case ITM_CHECK_OPT_CONSTRAINT: {
      ...
      // check existing uniqueness constraints whether they are similar
      // and combine uniqueness constraints if possible
      for (ValueId occ = constraints_.init(); constraints_.next(occ); constraints_.advance(occ)) {
        if (occ.getItemExpr()->getOperatorType() == ITM_CHECK_OPT_CONSTRAINT) {
          const ValueIdSet &occPreds = ((CheckOptConstraint *)occ.getItemExpr())->getCheckPreds();
  ...
  if (duplicateConstraint) {
    ...
    if (brandNewConstraint) delete c;
  } else {
...
NABoolean GroupAttributes::isUnique(const ValueIdSet &cols) const {
  for (ValueId x = constraints_.init(); constraints_.next(x); constraints_.advance(x)) {
    if (x.getItemExpr()->getOperatorType() == ITM_UNIQUE_OPT_CONSTRAINT) {
      UniqueOptConstraint *u = (UniqueOptConstraint *)x.getItemExpr();

      if (cols.contains(u->uniqueCols())) return TRUE;
      ...
      if (c->getUpperBound() <= 1) return TRUE;
    }
  }
...
NABoolean GroupAttributes::findUniqueCols(ValueIdSet &uniqueCols) const {
  ...
  for (ValueId v = constraints_.init(); constraints_.next(v); constraints_.advance(v)) {
    if (v.getItemExpr()->getOperatorType() == ITM_UNIQUE_OPT_CONSTRAINT) {
      ...
      if (cc->getUpperBound() <= 1) return TRUE;
    }
  }
  return FALSE;  // no unique columns found
}

NABoolean GroupAttributes::hasCardConstraint(Cardinality &minNumOfRows, Cardinality &maxNumOfRows) const {
  ...
  for (ValueId x = constraints_.init(); constraints_.next(x); constraints_.advance(x)) {
    if (x.getItemExpr()->getOperatorType() == ITM_CARD_CONSTRAINT) {
      ...
      if (minNumOfRows < c->getLowerBound()) minNumOfRows = c->getLowerBound();
      if (maxNumOfRows > c->getUpperBound()) maxNumOfRows = c->getUpperBound();
    }
  }
...
NABoolean GroupAttributes::hasConstraintOfType(OperatorTypeEnum constraintType) const {
  for (ValueId x = constraints_.init(); constraints_.next(x); constraints_.advance(x)) {
    if (x.getItemExpr()->getOperatorType() == constraintType) return TRUE;
  }

  return FALSE;
}

void GroupAttributes::getConstraintsOfType(OperatorTypeEnum constraintType, ValueIdSet &vidSet) const {
  for (ValueId x = constraints_.init(); constraints_.next(x); constraints_.advance(x)) {
    if (x.getItemExpr()->getOperatorType() == constraintType) vidSet.insert(x);
  }
}
...
void GroupAttributes::addSuitableRefOptConstraints(const ValueIdSet &vidSet) {
  for (ValueId x = vidSet.init(); vidSet.next(x); vidSet.advance(x)) {
    if (x.getItemExpr()->getOperatorType() == ITM_REF_OPT_CONSTRAINT) {
      ...
      if ((NOT riConstraint->isMatched()) && getCharacteristicOutputs().contains(fkCols)) {
        addConstraint(x);
...
void GroupAttributes::addSuitableCompRefOptConstraints(const ValueIdSet &vidSet, const ValueIdSet &preds,
                                                       const RelExpr *node) {
  for (ValueId x = vidSet.init(); vidSet.next(x); vidSet.advance(x)) {
    if (x.getItemExpr()->getOperatorType() == ITM_COMP_REF_OPT_CONSTRAINT) {
      ComplementaryRefOptConstraint *compRIConstraint = (ComplementaryRefOptConstraint *)x.getItemExpr();
      ...
      if (getCharacteristicOutputs().contains(uniqueColsSet) &&
        ...
        if (isUnique(uniqueCols) && (NOT node->getOperator().match(REL_ANY_JOIN))) {
...
NABoolean GroupAttributes::canEliminateOrderColumnBasedOnEqualsPred(ValueId col) const {
  // cast away const-ness and call the non-const method without
  // predicates, which will mean it won't side-effect "this"
  return const_cast<GroupAttributes *>(this)->tryToEliminateOrderColumnBasedOnEqualsPred(col, NULL);
}
...
NABoolean GroupAttributes::tryToEliminateOrderColumnBasedOnEqualsPred(ValueId col, const ValueIdSet *preds) {
  ...
  if (preds || hasConstraintOfType(ITM_CHECK_OPT_CONSTRAINT)) {
    ...
    getConstraintsOfType(ITM_CHECK_OPT_CONSTRAINT, checkConstraints);

    for (ValueId c = checkConstraints.init(); checkConstraints.next(c); checkConstraints.advance(c))
      checkPreds += static_cast<CheckOptConstraint *>(c.getItemExpr())->getCheckPreds();

    // also use newly provided predicates
    if (preds) checkPreds += *preds;

    // if the column is descending, then get rid of the Inverse
    // operator for the next check
    if (col.getItemExpr()->getOperatorType() == ITM_INVERSE) col = col.getItemExpr()->child(0).getValueId();
    ...
    if (col.getItemExpr()->getOperatorType() == ITM_VEG_REFERENCE) {
      const ValueIdSet &vegMembers = static_cast<VEGReference *>(col.getItemExpr())->getVEG()->getAllValues();
      ValueId dummy;

      for (ValueId b = vegMembers.init(); vegMembers.next(b); vegMembers.advance(b))
    ...
    for (ValueId p = checkPreds.init(); checkPreds.next(p); checkPreds.advance(p)) {
      ValueId dummy1, dummy2;

      if (p.getItemExpr()->getOperatorType() == ITM_EQUAL &&
          ScanKey::isAKeyPredicateForColumn(p, dummy1, dummy2, col, getCharacteristicInputs())) {
        ...
        if (preds && preds->contains(p))
          addConstraint(new (CmpCommon::statementHeap()) CheckOptConstraint(ValueIdSet(p)));
      }
    }
...
void GroupAttributes::lomerge(GroupAttributes &other, NABoolean mergeCIO) {
  ...
  CMPASSERT(isEmbeddedUpdateOrDelete() == other.isEmbeddedUpdateOrDelete());
  CMPASSERT(isGenericUpdateRoot() == other.isGenericUpdateRoot());
  // QSTUFF
  ...
  if (mergeCIO) {
    ...
    if (NOT(requiredInputs_ == other.requiredInputs_) OR NOT(requiredOutputs_ == other.requiredOutputs_))
  ...
  for (ValueId x = other.constraints_.init(); other.constraints_.next(x); other.constraints_.advance(x)) {
  ...
  for (CollIndex i = 0; i < other.getInputLogPropList().entries(); i++) {
    ...
    if (index < 0)  // this ILP does not already exist
    {
      ...
      if ((i >= other.getOutputLogPropList().entries()) || (i >= other.getIntermedOutputLogPropList().entries())) {
  ...
  if (other.getGroupAnalysis()) {
    if (!groupAnalysis_) {
      groupAnalysis_ = new (CmpCommon::statementHeap()) GroupAnalysis(*(other.getGroupAnalysis()));
    } else {
  ...
  if (other.hasNonDeterministicUDRs_) hasNonDeterministicUDRs_ = TRUE;

  if ((other.potential_ >= 0) && (other.potential_ < potential_)) potential_ = other.potential_;
...
NABoolean GroupAttributes::operator==(const GroupAttributes &other) const {
  return (requiredInputs_ == other.requiredInputs_ AND requiredOutputs_ == other.requiredOutputs_);
...
ValueIdList GroupAttributes::recommendedOrderForNJProbing(GroupAttributes *child0GA,        // IN
  ...
  const ReqdPhysicalProperty *rppForMe = rg.getStartRequirements();
  ...
  for (CollIndex i = 0; i < availableBtreeIndexes_.entries(); i++) {
    ...
    if (currentIndexPartFunc == NULL) {
      currentIndexPartFunc = new (CmpCommon::statementHeap()) SinglePartitionPartitioningFunction();
      currentIndexNumParts = 1;
      currentIndexPartKeyColsAreMappable = TRUE;
    } else {
      currentIndexNumParts = currentIndexPartFunc->getCountOfPartitions();
      currentIndexPartKey = (&(currentIndexPartFunc->getPartitioningKey()));
      if (currentIndexPartKey->isEmpty())
        currentIndexPartKeyColsAreMappable = TRUE;
      else {
        currentIndexPartKeyAsList = *currentIndexPartKey;
        currentIndexPartKeyEquiJoinCols = currentIndexPartKeyAsList.findNJEquiJoinCols(
            child0GA->getCharacteristicOutputs(), getCharacteristicInputs(), currentIndexUncoveredCols);
        if (currentIndexPartKeyEquiJoinCols.entries() == currentIndexPartKey->entries())
          currentIndexPartKeyColsAreMappable = TRUE;
        else
    ...
    if (chosenIndexPartKeyColsAreMappable AND NOT currentIndexPartKeyColsAreMappable) continue;

    currentIndexOrder = currentIndexSortKey->findNJEquiJoinCols(child0GA->getCharacteristicOutputs(),
                                                                getCharacteristicInputs(), currentIndexUncoveredCols);
    ...
    if (currentIndexOrder.entries() == 0) continue;
    ...
    if ((dp2SortOrderPartReq != NULL) AND(NOT dp2SortOrderPartReq->partReqAndFuncCompatible(currentIndexPartFunc))) {
      currentIndexSatisfiesDp2SortOrderPartReq = FALSE;
      if (sortOrderTypeReq != DP2_OR_ESP_NO_SORT_SOT) continue;
    }
    ...
    if (chosenIndexSatisfiesDp2SortOrderPartReq AND NOT currentIndexSatisfiesDp2SortOrderPartReq) continue;
    ...
    if ((partReqForMe != NULL) AND(partReqForMe->isRequirementReplicateNoBroadcast() OR(
            partReqForMe->isRequirementExactlyOne() AND(NOT rppForMe->executeInDP2() OR(currentIndexNumParts == 1))))) {
    ...
    // otherwise, can only use index if it's part key cols are mappable
      ...
      NABoolean numPartsSatisfiesFuzzyReq =
          ((partReqForMe != NULL) AND(partReqForMe->castToRequireApproximatelyNPartitions() != NULL)
      ...
      if ((numPartsRequirement == ANY_NUMBER_OF_PARTITIONS) OR(numPartsRequirement == currentIndexNumParts)
              OR(NOT rppForMe->executeInDP2() AND(numPartsRequirement < currentIndexNumParts))
                  OR(numPartsSatisfiesFuzzyReq AND NOT numPartsForced)) {
        ...
        if ((partReqForMe == NULL) OR(currentIndexNumParts == 1))
          currentIndexSatisfiesPartReq = TRUE;
        else {
          ...
          if (partReqForMe->isRequirementFuzzy()) {
            if (partReqForMe->getPartitioningKey().isEmpty() OR partReqForMe->getPartitioningKey().contains(
                    *currentIndexPartKey))
              currentIndexSatisfiesPartReq = TRUE;
          } else {
    ...
    if (NOT currentIndexSatisfiesPartReq) continue;
    ...
    if (NOT reqdArr1.isEmpty() AND NOT currentIndexUncoveredCols.satisfiesReqdArrangement(reqdArr1, this)) continue;
    if (NOT reqdOrder1.isEmpty() AND(currentIndexUncoveredCols.satisfiesReqdOrder(reqdOrder1, this) != SAME_ORDER))
      continue;
    ...
    if ((chosenIndexNumUncoveredCols == -1)                 // 1st index checked?
        OR(NOT chosenIndexPartKeyColsAreMappable AND        // always favor index
               currentIndexPartKeyColsAreMappable)          // with mappable part key
        OR(NOT chosenIndexSatisfiesDp2SortOrderPartReq AND  // favor if satis.
               currentIndexSatisfiesDp2SortOrderPartReq)    // dp2SortOrderPartReq
        OR(currentIndexNumUncoveredCols < chosenIndexNumUncoveredCols)
            OR((currentIndexNumUncoveredCols == chosenIndexNumUncoveredCols)
                   AND(currentIndexNumParts > chosenIndexNumParts))
                OR((currentIndexNumUncoveredCols == chosenIndexNumUncoveredCols)
                       AND(currentIndexNumParts == chosenIndexNumParts) AND currentIndexDesc->isClusteringIndex())) {
      ...
      chosenIndexPartKeyColsAreMappable = currentIndexPartKeyColsAreMappable;
      chosenIndexSatisfiesDp2SortOrderPartReq = currentIndexSatisfiesDp2SortOrderPartReq;
  ...
  if (chosenIndexDesc != NULL) {
    ...
    const ColStatDescList &csdl = chosenIndexDesc->getPrimaryTableDesc()->getTableColStats();
    ...
    CostScalar rowsPerBlock = (blockSizeInKb / chosenIndexDesc->getRecordSizeInKb()).getFloor();
    CostScalar numBlocks = (numRows / rowsPerBlock).getCeiling();
    CostScalar numBlocksPerPartition = (numBlocks / chosenIndexNumParts).getCeiling();
...
//
// -----------------------------------------------------------------------
void GroupAttributes::coverTest(const ValueIdSet &setOfExprOnParent, const ValueIdSet &newExternalInputs,
                                ValueIdSet &coveredExpr, ValueIdSet &referencedInputs, ValueIdSet *coveredSubExpr,
                                ValueIdSet *unCoveredExpr) const {
  for (ValueId exprId = setOfExprOnParent.init(); setOfExprOnParent.next(exprId); setOfExprOnParent.advance(exprId)) {
    if (covers(exprId, newExternalInputs, referencedInputs, coveredSubExpr, unCoveredExpr)) coveredExpr += exprId;
...
//
// -----------------------------------------------------------------------
NABoolean GroupAttributes::covers(const ValueId &exprId, const ValueIdSet &newExternalInputs,
  ...
  if (isCharacteristicInput(exprId) OR isCharacteristicOutput(exprId)) {
    // do nothing
  }
  ...
  // it is covered by the given Group Attributes or newExternalInputs
    ...
    coverFlag = iePtr->isCovered(newExternalInputs, *this, localInputs, localSubExpr, localUnCoveredExpr);
    if (coverFlag) {
    ...
    {                                       // but one of its subexpressions is
      referencedInputs += localInputs;
      if (coveredSubExpr != NULL) *coveredSubExpr += localSubExpr;
      if (unCoveredExpr != NULL) *unCoveredExpr += localUnCoveredExpr;
    } else {
      // this item expression and all its children are uncovered,
      // so only return this one as uncovered
      if (unCoveredExpr != NULL) *unCoveredExpr += exprId;
    }
...
// inputs and outputs.
// -----------------------------------------------------------------------
void GroupAttributes::computeCharacteristicIO(const ValueIdSet &newInputs, const ValueIdSet &exprOnParent,
                                              const ValueIdSet &outputExprOnParent,
                                              const ValueIdSet &essentialChildOutputs, const ValueIdSet *selPred,
                                              const NABoolean childOfALeftJoin, const NABoolean optimizeOutputs,
  ...
  for (exprId = effectiveOutputExprOnParent.init(); effectiveOutputExprOnParent.next(exprId);
    ...
    if (covers(exprId, newInputs, referencedInputs, &coveredSubExpr)) {
  ...
  for (exprId = coveredSubExpr.init(); coveredSubExpr.next(exprId); coveredSubExpr.advance(exprId)) {
    ...
    ValueId noInverseVid = exprId.getItemExpr()->removeInverseOrder()->getValueId();
    ...
    if (essentialChildOutputs.contains(valueToTest)) essentialKeptOutputs += valueToTest;
  ...
  // Look for covered expressions that the child can contribute
  for (exprId = effectiveExprOnParent.init(); effectiveExprOnParent.next(exprId);
  ...
  for (exprId = coveredSubExpr.init(); coveredSubExpr.next(exprId); coveredSubExpr.advance(exprId)) {
    ...
    if (optimizeOutputs) {
      ...
      valueToTest.removeCoveredExprs(availableValues, &essentialOutputsThatHaveBeenMarkedNonEssential);
      availableValues -= newInputs;
      essentialOutputsThatHaveBeenMarkedNonEssential -= newInputs;
    }

    if ((extraHubEssOutputs && !(extraHubEssOutputs->contains(valueToTest))) || NOT extraHubEssOutputs) {
...
// --------------------------------------------------------------------
void GroupAttributes::minimizeOutputs(ValueIdSet &nonEssentialOutputs, ValueIdSet &essentialOutputs,
  ...
  for (ValueId exprId = nonEssentialOutputs.init(); nonEssentialOutputs.next(exprId);
    ...
    if (exprId.getItemExpr()->getOperatorType() == ITM_VEG_PREDICATE) {
      ...
      coverFlag = emptyGA.covers(vegPtr->getVEGReference()->getValueId(), allOutputs, usedValues);
    } else
      coverFlag = emptyGA.covers(exprId, allOutputs, usedValues);
    if (coverFlag) {
  ...
  for (ValueId exprId = essentialOutputs.init(); essentialOutputs.next(exprId); essentialOutputs.advance(exprId)) {
    allOutputs -= exprId;
    emptyGA.setCharacteristicOutputs(allOutputs);
    if (exprId.getItemExpr()->getOperatorType() == ITM_VEG_PREDICATE) {
      VEG *vegPtr = ((VEGPredicate *)(exprId.getItemExpr()))->getVEG();
...
void GroupAttributes::resolveCharacteristicInputs(const ValueIdSet &externalInputs) {
  ...
  for (ValueId x = vegRefs.init(); vegRefs.next(x); vegRefs.advance(x)) {
    if (x.getItemExpr()->getOperatorType() == ITM_VEG_REFERENCE) {
      ...
      for (ValueId vid = allValues.init(); allValues.next(vid); allValues.advance(vid)) {
        ...
        if ((vidExpr->getOperatorType() == ITM_CURRENT_USER) || (vidExpr->getOperatorType() == ITM_CURRENT_TENANT) ||
            (vidExpr->getOperatorType() == ITM_CURRENT_TIMESTAMP) || (vidExpr->getOperatorType() == ITM_SLEEP) ||
...
// -----------------------------------------------------------------------
void GroupAttributes::resolveCharacteristicOutputs(const ValueIdSet &availableValues,
...
void GroupAttributes::clearGroupAnalysis() { groupAnalysis_->clear(); }
...
void GroupAttributes::clearAllEstLogProp() {
...
EstLogPropSharedPtr GroupAttributes::outputLogProp(const EstLogPropSharedPtr &inLP) {
  ...
  if (QueryAnalysis::Instance()) {
    ...
    if (appStatMan != NULL) {
      ...
      const JBBSubset *jbbSubsetForThisGroup = groupAnalysis->getParentJBBView();
      ...
      if ((jbbSubsetForThisGroup != NULL) && (inLP->isCacheable())) {
        ...
        if (inLP != (*GLOBAL_EMPTY_INPUT_LOGPROP)) groupNodeSet += *(inLP->getNodeSet());
        ...
        if (outputEstLogProp == NULL) {
          ...
          if (groupAnalysis->getNodeAnalysis() && groupAnalysis->getNodeAnalysis()->getJBBC())
            jbbSubsetForThisGroup = groupAnalysis->getLocalJBBView();

          if (jbbSubsetForThisGroup != NULL) {
            CANodeIdSet localNodeSet = jbbSubsetForThisGroup->getJBBCsAndGB();

            // if inLP are cacheable, then they too should have JBBSubset
            // associated with them. Proceed in the same manner as with
            // the parentJBBView

            if (inLP != (*GLOBAL_EMPTY_INPUT_LOGPROP)) localNodeSet += *(inLP->getNodeSet());
            // lookup, if outputLogProp for these inLP have exist in the cache
  ...
  Cardinality minRows, maxRows;
  if (inLP->getResultCardinality() <= CostScalar(1) && hasCardConstraint(minRows, maxRows)) {
    outputEstLogProp->setResultCardinality(MINOF(outputEstLogProp->getResultCardinality(), maxRows));

    outputEstLogProp->setResultCardinality(MAXOF(outputEstLogProp->getResultCardinality(), minRows));

    outputEstLogProp->setMaxCardEst(MINOF(outputEstLogProp->getMaxCardEst(), maxRows));

    outputEstLogProp->setMaxCardEst(MAXOF(outputEstLogProp->getMaxCardEst(), minRows));
  }

  // Set the Cardinality of EstLogProp to one , if a constraint with maxRows one exists.
  if (inLP->isCardinalityEqOne() && hasCardConstraint(minRows, maxRows) && maxRows == 1) {
...
CostScalar GroupAttributes::getResultMaxCardinalityForInput(EstLogPropSharedPtr &inLP) {
...
EstLogPropSharedPtr GroupAttributes::materializeInputLogProp(const EstLogPropSharedPtr &inLP, int *multipleReads) {
  ...
  for (CollIndex i = 0; i < inColStatsList.entries(); i++) {
    ...
    if (columnVIdList.prefixCoveredInSet(specifiedInputs) > 0) {
      ...
      for (ValueId idIn = specifiedInputs.init(); specifiedInputs.next(idIn) && !foundMatch;
           specifiedInputs.advance(idIn)) {
        if (idIn.getItemExpr()->getOperatorType() == ITM_VEG_REFERENCE) {
          VEG *nestedVEG = ((VEGReference *)(idIn.getItemExpr()))->getVEG();
          const ValueIdSet &VEGGroup = nestedVEG->getAllValues();

          for (ValueId id = VEGGroup.init(); VEGGroup.next(id) && !foundMatch; VEGGroup.advance(id)) {
            if (columnVIdList.contains(id)) foundMatch = TRUE;
          }
        }
      }
...
NABoolean GroupAttributes::addInputOutputLogProp(const EstLogPropSharedPtr &inputLP,
  ...
  if (isPropSynthesized(inputLP)) return FALSE;
  ...
  RelExpr *logExpr = getLogExprForSynthesis();
  if (logExpr && (CmpCommon::getDefault(HIST_FREQ_VALS_NULL_FIX) == DF_ON) &&
      (logExpr->getOperator().match(REL_ANY_LEFT_JOIN) || logExpr->getOperator().match(REL_ANY_FULL_JOIN))) {
    colStats.computeMaxFreq(TRUE);
  }

  if ((CURRSTMT_OPTDEFAULTS->histOptimisticCardOpt() == 3) && logExpr && (logExpr->getOperatorType() == REL_SCAN)) {
  ...
  if (intermedOutputLP) intermedOutputLP->colStats().copyAndScaleHistograms(1);
  ...
  if (QueryAnalysis::Instance()) {
    ...
    if (appStatMan != NULL) {
      ...
      if ((jbbSubsetForThisGroup != NULL) && (inputLP->isCacheable())) {
        ...
        CANodeIdSet *nodeIdSet = new (STMTHEAP) CANodeIdSet(jbbSubsetForThisGroup->getJBBCsAndGB(), STMTHEAP);

        // The estimatedLogProp in the ASM cache would be identified
        // by the NodeSet of the left child, the right child and the
        // outer child.

        if (inputLP != (*GLOBAL_EMPTY_INPUT_LOGPROP)) nodeIdSet->insert(*(inputLP->getNodeSet()));
...
SHPTR_LIST(EstLogPropSharedPtr) * GroupAttributes::getCachedStatsList() {
  SHPTR_LIST(EstLogPropSharedPtr) *statsList = new (STMTHEAP) SHPTR_LIST(EstLogPropSharedPtr)(STMTHEAP);
  ...
  if (appStatMan != NULL) {
    ...
    if (jbbSubsetForThisGroup != NULL) {
      ...
      for (CollIndex i = 0; i < inputEstLogProp_.entries(); i++) {
        if ((inputEstLogProp_[i]->isCacheable()) && (inputEstLogProp_[i] != (*GLOBAL_EMPTY_INPUT_LOGPROP)))
        ...
        if (inNodeSet) {
          ...
          outputEstLogProp = appStatMan->getCachedStatistics(&combinedWithInputNodeSet);
        } else
          outputEstLogProp = appStatMan->getCachedStatistics(&groupNodeSet);
        ...
        if (outputEstLogProp != NULL) statsList->insert(outputEstLogProp);
...
ColStatsSharedPtr GroupAttributes::getColStatsForSkewDetection(const ValueId vId, const EstLogPropSharedPtr &inLP) {
...
CostScalar GroupAttributes::getSkewnessFactor(const ValueId vId, EncodedValue &mostFreqVal,
  ...
  if (colStats == NULL) return csMinusOne;
...
SkewedValueList *GroupAttributes::getSkewedValues(const ValueId &vId, double threshold, NABoolean &statsExist,
                                                  const EstLogPropSharedPtr &inLP, NABoolean includeNullIfSkewed) {
  ColStatDescSharedPtr colStatDesc;
  ValueIdSet vidSet;
  ...
  if (cachedSkewValuesPtr_ == NULL)
    cachedSkewValuesPtr_ = new (STMTHEAP)
        NAHashDictionary<ValueId, SkewedValueList>(valueIdHashKeyFunc,             /* hash func defined above*/
                                                   20 /*number of buckets*/, TRUE, /* enforce uniqueness*/
                                                   STMTHEAP);
  else {
  ...
  switch (iePtr->getOperatorType()) {
    ...
    case ITM_VEG_REFERENCE:
      ...
      if (vidSet.entries() == 0) return NULL;
  ...
  const double total_rowcount_threshold = defs.getAsDouble(SKEW_ROWCOUNT_THRESHOLD);
  ...
  if ((modifiedMaxCard < 10 * total_rowcount_threshold) OR(modifiedCard < total_rowcount_threshold)) return NULL;
  ...
  if ((!colStats->isOrigFakeHist())) {
    const FrequentValueList &frequentValueList = colStats->getFrequentValues();
    CostScalar thresholdAdFrequency = threshold * (colStats->getRowcount()).getValue();

    for (CollIndex j = 0; j < frequentValueList.entries(); j++) {
      const FrequentValue &frequentValue = frequentValueList[j];
      EncodedValue skewed;

      if (frequentValue.getFrequency() * frequentValue.getProbability() < thresholdAdFrequency) continue;

      if (svList->entries() >= CURRSTMT_OPTDEFAULTS->maxSkewValuesDetected()) break;
      ...
      if (frequentValue.getEncodedValue().isNullValue()) {
        ...
        if (useHashValue == FALSE) {
          ...
          if (nt.getTypeQualifier() == NA_NUMERIC_TYPE && nt.getTypeName() == LiteralNumeric) {
            skewed = frequentValueList[j].getEncodedValue().computeHashForNumeric((SQLNumeric *)&nt);  // for numeric
          } else
...
double GroupAttributes::getAverageVarcharSize(const ValueId vId, const EstLogPropSharedPtr &inLP) {
  ColStatDescSharedPtr colStatDesc;
  ValueIdSet vidSet;
  ...
  if (vidSet.isEmpty() || vidSet.entries() > 1) return -1;

  ValueId colVid((CollIndex)0);
  vidSet.next(colVid);

  if (!(colVid.getType().getVarLenHdrSize() > 0)) return -1;
...
void GroupAttributes::setEssentialCharacteristicOutputs(const ValueIdSet &vset) {
  NABoolean misMatchFound = FALSE;
  for (ValueId id = vset.init(); vset.next(id) && !misMatchFound; vset.advance(id)) {
    if (NOT isCharacteristicOutput(id)) misMatchFound = TRUE;
  }
  CMPASSERT(NOT misMatchFound);
  requiredEssentialOutputs_ = vset;
}

void GroupAttributes::addEssentialCharacteristicOutputs(const ValueIdSet &vset) {
  NABoolean misMatchFound = FALSE;
  for (ValueId id = vset.init(); vset.next(id) && !misMatchFound; vset.advance(id)) {
    if (NOT isCharacteristicOutput(id)) misMatchFound = TRUE;
  }
...
void GroupAttributes::getNonEssentialCharacteristicOutputs(ValueIdSet &vset) const {
  vset += getCharacteristicOutputs();
  vset -= getEssentialCharacteristicOutputs();
}

NABoolean GroupAttributes::allHiveTables() {
  if (availableBtreeIndexes_.entries() == 0) return FALSE;

  for (CollIndex i = 0; i < availableBtreeIndexes_.entries(); i++) {
    IndexDesc *iDesc = availableBtreeIndexes_[i];
    if (!(iDesc->getPrimaryTableDesc()->getNATable()->isHiveTable())) return FALSE;
  }
  return TRUE;
}

NABoolean GroupAttributes::allHiveColumnarTables(HiveFileType type) {
  if (!allHiveTables()) return FALSE;
    ...
    switch (type) {
      case ANY:
        if (iDesc->getPrimaryTableDesc()->getNATable()->isORC() ||
            iDesc->getPrimaryTableDesc()->getNATable()->isParquet())
          break;
        else
          return FALSE;

      case ORC:
        if (!(iDesc->getPrimaryTableDesc()->getNATable()->isORC())) return FALSE;

        break;

      case PARQUET:
        if (!(iDesc->getPrimaryTableDesc()->getNATable()->isParquet())) return FALSE;
...
int GroupAttributes::computeNumOfHiveColumnarTables(HiveFileType type) {
  ...
  for (CollIndex i = 0; i < availableBtreeIndexes_.entries(); i++) {
    ...
    switch (type) {
      case ORC:
        if (iDesc->getPrimaryTableDesc()->getNATable()->isORC()) ct++;

        break;

      case PARQUET:
        if (iDesc->getPrimaryTableDesc()->getNATable()->isParquet()) ct++;
...
NAString GroupAttributes::getIndexName(CollIndex i) {
  if (i < 0 || i >= availableBtreeIndexes_.entries()) return NAString("");
...
NABoolean GroupAttributes::allHiveORCTablesSorted() {
  ...
  for (CollIndex i = 0; i < availableBtreeIndexes_.entries(); i++) {
    IndexDesc *iDesc = availableBtreeIndexes_[i];
    if (!(iDesc->isSortedORCHive())) return FALSE;
  }
  return TRUE;
}
