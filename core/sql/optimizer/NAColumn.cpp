...
void NAColumn::deepDelete()
{
  ...
  if(initDefaultValue_)
    NADELETEBASIC(initDefaultValue_,heap_);
  if(heading_)
  ...
  switch (datatype)
   ...
   case REC_BIN16_SIGNED:
	...
	SQLVarChar(heap, charCount,
      ...
      if (column_desc->getColFlags() & SEABASE_COLUMN_IS_TYPE_VARCHAR2)
        ((CharType*)type)->setVarchar2(TRUE);
      break;
    ...
    case REC_BYTE_V_ASCII_LONG:
          ...
          column_desc->characterSet() == CharInfo::UTF8)
      {
        ...
        if ( column_desc->precision > 0 )
           sizeInChars = column_desc->precision;
        type = new (heap)
          SQLLongVarChar(heap, CharLenInfo(sizeInChars, charCount/*in_bytes*/),
                         FALSE,
                         column_desc->isNullable(),
                         column_desc->isUpshifted(),
                         column_desc->isCaseInsensitive(),
    ...
    case REC_BLOB :
      {
        // this will be set later after reading this value from metadata.
        Lng32 inlineDataMaxLen = 0;
        //Mantis15498
        //We should set a reasonable value to inlineDataMaxLen,
        //otherwise we will get memory out of bounds in executor.
        //lobV2 works fine, so only influence old lob format.
        if ((tablesFlags & MD_TABLES_LOB_VERSION2) == 0)
          inlineDataMaxLen = (Lng32)CmpCommon::getDefaultNumeric(TRAF_LOB_INLINED_DATA_MAXBYTES);
        Int64 loblen = 0;
        loblen = (((Int64)column_desc->precision) << 32 | column_desc->scale&0xFFFFFFFFL);
        type = new (heap)
          SQLBlob(heap, loblen, Lob_Invalid_Storage,
                  column_desc->isNullable(), inlineDataMaxLen);
      }
      break;
    ...
    case REC_CLOB :
      {
        ...
        loblen = (((Int64)column_desc->precision) << 32 | column_desc->scale&0xFFFFFFFFL);
        type = new (heap)
          SQLClob(heap, loblen, column_desc->characterSet(), Lob_Invalid_Storage,
                  column_desc->isNullable(), inlineDataMaxLen);
      }
      break;
    ...
    case REC_ROW :
      {
        ...
        NAType *type0 = parser.parseCompositeDefinition
          (column_desc->compDefnStr,
           strlen(column_desc->compDefnStr),
           CharInfo::ISO88591);
...
void NAColumnArray::removeAt(CollIndex start, CollIndex numEntries)
{
  ...
  for (Int32 k=entries()+numEntries-1; k >= (Int32) entries(); k--)
    ascending_.remove(k);
}
