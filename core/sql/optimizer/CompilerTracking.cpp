...
************************************************************************/
static const char *getCompilerTrackingLogFilename() {
  const char *fname = ActiveSchemaDB()->getDefaults().getValue(COMPILER_TRACKING_LOGFILE);
...
************************************************************************/
inline void CompilerTrackingInfo::resetInterval() {
...
************************************************************************/
void CompilerTrackingInfo::incrementQueriesCompiled(NABoolean success) {
  if (success) {
...
************************************************************************/
void CompilerTrackingInfo::resetMetadataCacheCounters() {
...
method CompilerTrackingInfo::updateQCacheIntervalWaterMark
 ...
 QueryCache::finalize()
************************************************************************/
void CompilerTrackingInfo::updateQCacheIntervalWaterMark() {
  QueryCacheStats stats;
  CURRENTQCACHE->getCacheStats(stats);
  //
  // update the largest if its bigger
  largestQCacheIntervalWaterMark_ = (stats.intervalWaterMark > largestQCacheIntervalWaterMark_)
                                        ? stats.intervalWaterMark
...
************************************************************************/
void CompilerTrackingInfo::clearQCacheCounters() {
  qCacheHits_ = 0;
  qCacheLookups_ = 0;
  qCacheRecompiles_ = 0;
}
...
************************************************************************/
void CompilerTrackingInfo::resetQueryCacheCounters() {
  //
...
************************************************************************/
void CompilerTrackingInfo::resetHistogramCacheCounters() {
  CMPASSERT(NULL != CURRCONTEXT_HISTCACHE);
...
************************************************************************/
void CompilerTrackingInfo::updateStmtIntervalWaterMark() {
  size_t stmtIntervalWaterMark = CmpCommon::statementHeap()->getIntervalWaterMark();
  //
  // update the largest if its bigger
  largestStmtIntervalWaterMark_ =
      (stmtIntervalWaterMark > largestStmtIntervalWaterMark_) ? stmtIntervalWaterMark : largestStmtIntervalWaterMark_;
}
/************************************************************************
...
************************************************************************/
inline size_t CompilerTrackingInfo::stmtHeapIntervalWaterMark() {
  size_t stmtIntervalWaterMark = CmpCommon::statementHeap()->getIntervalWaterMark();

  return (stmtIntervalWaterMark > largestStmtIntervalWaterMark_) ? stmtIntervalWaterMark
                                                                 : largestStmtIntervalWaterMark_;
}
/************************************************************************
...
void CompilerTrackingInfo::logIntervalInPrivateTable() {
  /* ------------------------------------------------------
  ...
  NAString tableName =
      ActiveSchemaDB()->getDefaultSchema().getSchemaNameAsAnsiString() + "." + COMPILER_TRACKING_TABLE_NAME_PRIVATE;
  //
  // update the fields
  char dmlbuffer[8192];
  str_sprintf(dmlbuffer, (const char *)dmlprep, tableName.data(), endTime, compilerId, p->getPin(), beginTime,
              cpuPathLength(), longestCompile(), compilerAge(), sessionCount(), stmtHeapIntervalWaterMark(),
              cxtHeapCurrentSize(), cxtHeapIntervalWaterMark(), p->getCurrentSystemHeapSize(),
              systemHeapIntervalWaterMark(), metaDataCacheCurrentSize(), metaDataCacheIntervalWaterMark(),
              metaDataCacheHits(), metaDataCacheLookups(), qCacheCurrentSize(), qCacheIntervalWaterMark(), qCacheHits(),
              qCacheLookups(), hCacheCurrentSize(), hCacheIntervalWaterMark(), hCacheHits(), hCacheLookups(),
              successfulQueryCount(), failedQueryCount(), caughtExceptionCount(), qCacheRecompiles(), compilerInfo());

  ExeCliInterface cliInterface(CmpCommon::statementHeap(), 0, 0, CmpCommon::context()->sqlSession()->getParentQid());
...
************************************************************************/
void CompilerTrackingInfo::logIntervalInLog4Cxx() {
...
************************************************************************/
void CompilerTrackingInfo::printToFile() {
  ...
  fileout << "Interval Duration\t: " << currentIntervalDuration(endIntervalTime()) << " minutes" << endl;
