...
double EncVal_encodeString(const char *str, Lng32 strLen, CharType *cType) {
  ...
  // Use ascii value 0 instead of blank character( ascii value 32 ).
  // Because of user can use 'abc'||char(10) to insert string, char(10) is less than blank ( char(32) ).
  // So if we use char(32) here will cause some issues.
  ...
  if ((cType->isCaseinsensitive()) && (NOT cType->isUpshifted()) && cType->getBytesPerChar() == 1) {
...
#define ENCKEYBUFLEN (8 * (MAXPASSES + 1) + 2)  // Ensure Temp buffer big enough!
    ...
    if (collation != CharInfo::DefaultCollation) {
      ...
      ex_function_encode::encodeCollationKey((const UInt8 *)str, MINOF(strLen, 8), encodeKeyBuf1, ENCKEYBUFLEN, nPasses,
...
double EncVal_Char_encode(const char *theValue, const NAType *theType) {
  ...
  if (cType->supportsSQLnull()) charBufPtr += cType->getSQLnullHdrSize();
...
NormValue::NormValue(const ConstValue *constant, NABoolean negate) {
  ...
  if ((CmpCommon::getDefault(::MODE_SPECIAL_1) == DF_ON) && (theType->getTypeQualifier() == NA_CHARACTER_TYPE)) {
    CharType *cType = (CharType *)theType;
    if ((!cType->isCaseinsensitive()) && (cType->getBytesPerChar() == 1)) {
  ...
  if (theType->getTypeQualifier() == NA_CHARACTER_TYPE) {
    ...
    theValue_ = theType->encode(theValue);
  ...
  if (negate) theValue_ *= -1;

  isNullFlag_ = FALSE;
}

void NormValue::display(FILE *f, const char *prefix, const char *suffix, CollHeap *c, char *buf) const {
  Space *space = (Space *)c;
  char mybuf[1000];
...
NormValueList::NormValueList(const NormValueList &nvl, NAMemory *h)
    : NAArray<NormValue>(h ? h : CmpCommon::statementHeap(), nvl.entries()), heap_(h ? h : CmpCommon::statementHeap()) {
...
UInt32 EncodedValue::computeRunTimeHashValue(const NAColumnArray &colArray, const NAWchar *boundary,
                                             ConstValue *cvPtrs[]) {
  ...
  if (colArray.entries() != colEntries) return 0;

  // Re-arrage the colArray in the descending order of col positions. The result
  // is saved in a pos_index array below
  pos_index *posIndexArray = (pos_index *)(new (STMTHEAP) char[sizeof(pos_index) * colArray.entries()]);
  ...
  for (CollIndex j = 0; j < colEntries; j++) {
    ...
    if (nV.isNull()) {
    ...
    } else if (type->getTypeQualifier() == NA_NUMERIC_TYPE && type->getTypeName() == LiteralNumeric) {
      ...
      if (cv) {
        if (type->getTypeQualifier() == NA_CHARACTER_TYPE && ((CharType *)type)->isCaseinsensitive() &&
...
void EncodedValue::addANormValue(EncodedValue *thisPtr, ItemExpr *exprPtr, NABoolean negate) {
  if (exprPtr->doesExprEvaluateToConstant(TRUE)) {
    ...
    if (value.getValue() == _ENCODEDVALUE_NULL_VALUE_) value.setNull();  // explicitly set NULL flag
...
EncodedValue::EncodedValue(double val) : valueList_(NULL), heap_(HISTHEAP) {
  NormValue value(val);   // will never generate NULL
  this->setValue(value);  // filters out non-NULLs with NULL double-value
}

EncodedValue::EncodedValue(ItemExpr *expr, NABoolean negate) : valueList_(NULL), heap_(HISTHEAP) {
  addANormValue(this, expr, negate);
}

void EncodedValue::constructorFunction(const NAWchar *theValue, const NAColumnArray &columns,
                                       NABoolean okToReportErrors, ConstValue *cvPtrs[]) {
  ...
  {
    if (okToReportErrors) {
      *CmpCommon::diags() << DgSqlCode(CATALOG_HISTOGRM_HISTINTS_TABLES_CONTAIN_BAD_VALUE) << DgWString0(theValue)
  ...
  for (CollIndex i = 0; i < entries; i++) {
    ...
    switch (*item) {
      ...
      case L'<': {  // generate max as default.
      ...
      default: {
        if ((item[0] == L'N') && (item[1] == L'U') && (item[2] == L'L') && (item[3] == L'L')) {
          val.setNull();
          break;
        }
        ...
        if (!constVal || (entries == 1 && constVal->getType()->getTypeQualifier() != colType->getTypeQualifier())) {
    ...
    if (entries == 1)
      ...
      if (!valueList_) valueList_ = new (heap_) NormValueList(entries);
    ...
    if (i == (entries - 1) || boundaryValueTruncated) return;
...
//
// -----------------------------------------------------------------------
double EncodedValue::ratio(const EncodedValue &lowBound, const EncodedValue &upperBound) const {
  ...
  CMPASSERT(NOT std::isnan(retval));
...
void EncodedValue::display(FILE *f, const char *prefix, const char *suffix, CollHeap *c, char *buf) const {
  Space *space = (Space *)c;
  char mybuf[1000];
...
UInt32 EncodedValue::computeHashForNumeric(SQLNumeric *nt) { return getValue().computeHashForNumeric(nt); }
