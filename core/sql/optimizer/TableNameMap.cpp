...
//
void XTNM::insertNames(BindWA *bindWA, CorrName &corrName, ColumnDescList *cols) {
  ...
  if (corrName.getCorrNameAsString() != "") {
    if (prev = get(&corrName)) {
      if (prev->getTableName().getCorrNameAsString() != "")
        ...
        *CmpCommon::diags() << DgSqlCode(-4056) << DgTableName(corrName.getExposedNameAsAnsiString());
      else {
        // 4057 Correlation name X conflicts with table name C.S.X
        QualifiedName tblName(corrName.getCorrNameAsString(), bindWA->getDefaultSchema().getSchemaName(),
                              bindWA->getDefaultSchema().getCatalogName());
        *CmpCommon::diags() << DgSqlCode(-4057) << DgString0(corrName.getExposedNameAsAnsiString())
  ...
  if (prev = get(&tCorr)) {  // simple table name, i.e. qualified identifier
    if (prev->getTableName().getCorrNameAsString() != "") {
      ...
      *CmpCommon::diags() << DgSqlCode(-4057) << DgString0(tCorr.getExposedNameAsAnsiString())
  ...
  if (defaultMatch) {
    ...
    if (defaultMatch > 1)
      insert(prev = new (bindWA->wHeap()) TableNameMap(tCorr, cols, bindWA->wHeap()));  // possibly overwrite dummy
  } else if (catName == "" && schName == "")
    return;  // ## tmp! Only for our
  ...
  //
  if (NOT prev) insert(new (bindWA->wHeap()) TableNameMap(tCorr, NULL /* dummy name */, bindWA->wHeap()));
...
void TableViewUsageList::display() const { display(TRUE, 2); }
...
int TableViewUsageList::getViewsOnTable(CollIndex begIx, CollIndex endIx, int viewCount,
  ...
  for (CollIndex i = begIx; i < endIx; i++) {
    ...
    if (at(i)->isView()) {
      if (at(i)->viewCount() == viewCount) topViewName = &at(i)->getTableName();
    }

    else if (at(i)->getTableName() == baseName && at(i)->getSpecialType() == baseType) {
      baseNameSeen++;
  ...
  if (baseNameSeen) {
    if (additionalNameToFormat) tmpViewList.insert(new (h) TableNameMap(*additionalNameToFormat, NULL, h));
