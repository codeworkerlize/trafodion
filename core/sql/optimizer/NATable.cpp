    ...
    partitionV2Flags_(0)
{
  ...
  if (hasLobColumn())
    {
      if ((hasLobColumn()) &&
          (CmpSeabaseDDL::isMDflagsSet
           (table_desc->tableDesc()->tablesFlags, MD_TABLES_LOB_VERSION2)))
        setLobV2(TRUE);

      setNumLOBdatafiles(table_desc->tableDesc()->numLOBdatafiles_);

      CollIndex numCols = (CollIndex)table_desc->tableDesc()->colcount;
      ...
      short *lobNumList = new (heap_) short[numCols];
      short *lobTypList = new (heap_) short[numCols];
      char  **lobLocList = new (heap_) char*[numCols];
      char  **lobColNameList = new (heap_) char*[numCols];
      Int32 *lobInlinedDataMaxLenList = new (heap_) Int32[numCols];
      Int64 *lobHbaseDataMaxLenList = new (heap_) Int64[numCols];
      ...
      for (CollIndex i = 0; i < numCols; i++)
          ...
          if (nac->getType()->getTypeQualifier() == NA_LOB_TYPE)
	    {
          ...
          lobInlinedDataMaxLenList[i] = 0;
          lobHbaseDataMaxLenList[i] = 0;
        }      
       ...
       Lng32 numLobs = 0;
       Lng32 cliRC = 0;
       Int32 lobInlinedDataMaxlen = 0;
       Int32 lobHbaseDataMaxlen = 0;
       Int32 lobChunkMaxLen = 0;
       if (lobV2())
         {
           ExpLobOperV2::DDLArgs args;
           args.schName = (char*)schNam.data();
           args.schNameLen = schNam.length();
           args.numLOBs = &numLobs;
           args.qType = ExpLobOperV2::LOB_DDL_LOBMD_SELECT;
           args.lobNumList = lobNumList;
           args.lobTypList = lobTypList;
           args.lobLocList = lobLocList;
           args.lobColNameList = lobColNameList;
           args.lobInlinedDataMaxLenList = lobInlinedDataMaxLenList;
           args.lobHbaseDataMaxLenList = lobHbaseDataMaxLenList;
           ...
           args.numLOBdatafiles = getNumLOBdatafiles();
           args.lobParallelDDL = 0;

           ExpLobOperV2 v2Oper;
           cliRC = v2Oper.ddlInterface(objectUid().castToInt64(), args);

           lobChunkMaxLen = args.lobChunkMaxLen;
           numLOBdatafiles_ = args.numLOBdatafiles;

           if (cliRC < 0)
             {
               *CmpCommon::diags() << DgSqlCode(-3242)
                                   << DgString0("Error returned while reading LOB_COLUMNS MD table. Make sure that table exists.");
               bindWA->setErrStatus();
               return;
             }
         }
         ...
         if (CmpCommon::getDefault(TRAF_SWITCH_COMPILER) == DF_ON) {
           ...
           cliRC = SQL_EXEC_LOBddlInterface
             (
                (char*)schNam.data(),
                schNam.length(),
                ...
                lobNumList,
                lobTypList,
                lobLocList,lobColNameList,lobHdfsServer,lobHdfsPort,0,
                FALSE,NULL,getNumLOBdatafiles()
              );
           cmpSBD.switchBackCompiler();
         } else {
           cliRC = SQL_EXEC_LOBddlInterface
       ...
       if ((cliRC == 0) && (numLobs > 0))
         {
           for (Lng32 i = 0; i < numLobs; i++)
             {
               nac = colArray.getColumn(lobNumList[i]);
	       nac->lobNum() = lobNumList[i];
	       nac->lobStorageType() = (ComLobsStorageType)lobTypList[i];
	       nac->lobStorageLocation() = lobLocList[i];
               nac->lobInlinedDataMaxLen() = lobInlinedDataMaxLenList[i];
               nac->lobHbaseDataMaxLen() = lobHbaseDataMaxLenList[i];
               nac->lobChunkMaxLen() = lobChunkMaxLen;

               // add inlined len to lob handle length
               ((NAType*)nac->getType())->setNominalSize(nac->getType()->getNominalSize() +
                                                         nac->lobInlinedDataMaxLen());
               ((SQLlob*)(nac->getType()))->adjustLobHandleLen(nac->lobInlinedDataMaxLen());
	     }

           // all lobs for a table must have the same directory location.
           // Should validate that during create of table. TBD.
           // set lob location in NATable struct
           setLobStorageLocation(lobLocList[0]);
           lobHbaseDataMaxLen() = lobHbaseDataMaxLenList[0];
           lobInlinedDataMaxLen() = lobInlinedDataMaxLenList[0];

           if (lobV2())
             {
               char lobChunks[100];
               sprintf(lobChunks, "%s%020ld",
                       LOB_CHUNKS_V2_PREFIX,
                       objectUid().castToInt64());
               CorrName cn(lobChunks,
                           STMTHEAP,
                           getTableName().getSchemaName().data(),
                           getTableName().getCatalogName().data());

               NATable *chunksNaTable = bindWA->getNATable(cn);
               if ((bindWA->errStatus()) || (!chunksNaTable))
                 return;   

               NAColumn *nac = chunksNaTable->getNAColumnArray().getColumn("DATA_IN_HBASE");
               if (!nac)
                 return;
               
               setLobChunksTableDataInHbaseColLen(nac->getType()->getNominalSize());
             }           
	 } // if
    } // if hasLobColumn
