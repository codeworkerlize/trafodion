...
long HistogramsCacheEntry::getLastUpdateStatsTime() { return cmpCurrentContext->getLastUpdateStatsTime(); }

void HistogramsCacheEntry::setUpdateStatsTime(long updateTime) {
...
static int skipLeadingBlanks(const char *keyValueBuffer, const int startIndex, const int length) {
  int newIndex = startIndex;
  int stopIndex = newIndex + length;
  // Localize the search for blanks between the startIndex and stopIndex.
  while ((newIndex <= stopIndex) AND(keyValueBuffer[newIndex] == ' ')) newIndex++;
  return newIndex;
...
// character.
// -----------------------------------------------------------------------
static int skipTrailingBlanks(const char *keyValueBuffer, const int startIndex) {
  int newIndex = startIndex;
  while ((newIndex >= 0) AND(keyValueBuffer[newIndex] == ' ')) newIndex--;
  return newIndex;
...
//-------------------------------------------------------------------------
HistogramCache::HistogramCache(NAMemory *heap, int initSize)
    : heap_(heap),
  ...
  histogramsCache_ = new (heap_) NAHashDictionary<QualifiedName, HistogramsCacheEntry>(hashFunc, initSize, TRUE, heap_);
}
...
void HistogramCache::resetAfterStatement() {
  for (CollIndex x = lruQ_.entries(); x > 0; x--) {
    if (lruQ_[x - 1]->accessedInCurrentStatement()) lruQ_[x - 1]->resetAfterStatement();
  }
}
...
void HistogramCache::getHistograms(NATable &table, NABoolean useStoredStats) {
  ...
  const long &redefTime = table.getRedefTime();
  long &statsTime = const_cast<long &>(table.getStatsTime());
  long tableUID = table.objectUid().castToInt64();
  long siKeyGCinterval = CURRSTMT_OPTDEFAULTS->siKeyGCinterval();
  ...
  if (getCurrentTime() > lastTouchTime_ + siKeyGCinterval) invalidateCache();
  ...
  if (CURRSTMT_OPTDEFAULTS->cacheHistograms() && type == ExtendedQualName::NORMAL_TABLE) {  // 2//
    ...
    if (CURRSTMT_OPTDEFAULTS->preFetchHistograms() && cachedHistograms) {  // 3//
      // we do need to preFetch histograms
      if (!cachedHistograms->preFetched()) {  // 4//
  ...
  if ((xSchemaName != "_MD_") && (xSchemaName != "_PRIVMGR_MD_") && (xSchemaName != "_TENANT_MD_") &&
      (xObjectName != "SB_HISTOGRAMS") && (xObjectName != "SB_HISTOGRAM_INTERVALS") &&
  ...
  if (!CURRSTMT_OPTDEFAULTS->cacheHistograms()) invalidateCache();
...
//----------------------------------------------------------------------------
void HistogramCache::createColStatsList(NATable &table, HistogramsCacheEntry *cachedHistograms,
                                        NABoolean useStoredStats) {
  StatsList &colStatsList = *(table.getColStats());

  NAColumnArray &colArray = const_cast<NAColumnArray &>(table.getNAColumnArray());
  const QualifiedName &qualifiedName = table.getFullyQualifiedGuardianName();
  ExtendedQualName::SpecialTableType type = table.getTableType();
  const long &redefTime = table.getRedefTime();
  long &statsTime = const_cast<long &>(table.getStatsTime());
  ...
  // 1. If histogram caching is ON
  // 2. If the table is a normal table
  if (cachedHistograms && (CURRSTMT_OPTDEFAULTS->cacheHistograms() && type == ExtendedQualName::NORMAL_TABLE)) {
    // get statistics from cachedHistograms into list.
    // colArray has the columns whose histograms we need.
    coveredList = getStatsListFromCache(colStatsList, colArray, table.interestingExpressions(), cachedHistograms,
                                        singleColsFound);
  }
  ...
  if (colArray.entries() > coveredList) {
    ...
    if (CURRSTMT_OPTDEFAULTS->cacheHistograms() && type == ExtendedQualName::NORMAL_TABLE) {
      ...
      statsListForFetch =
          new (CmpCommon::statementHeap()) StatsList(CmpCommon::statementHeap(), 2 * colArray.entries());
    }
    ...
    if (CURRSTMT_OPTDEFAULTS->cacheHistograms() && CURRSTMT_OPTDEFAULTS->preFetchHistograms() &&
    ...
    for (CollIndex j = 0; j < colArray.entries(); j++) {
      ...
      if (!col->isUnique()) {
        ...
        if (col->needHistogram() && table.getCorrespondingIndex(keyColList,  // input columns
  ...
  if (CURRSTMT_OPTDEFAULTS->cacheHistograms() && type == ExtendedQualName::NORMAL_TABLE) {
    ...
    if (cachedHistograms) {
      ...
      putStatsListIntoCache((*statsListForFetch), colArray, qualifiedName, table.objectUid().castToInt64(), statsTime,
                            redefTime, allFakeStats);
  ...
  if (CURRSTMT_OPTDEFAULTS->reduceBaseHistograms()) colStatsList.reduceNumHistIntsAfterFetch(table);

  // clean up
  if (statsListForFetch != &colStatsList) delete statsListForFetch;
                                            ...
                                            ColumnSet &singleColsFound)                                     // In \ Out
{
  ...
  for (UInt32 i = 0; i < colArray.entries(); i++) {
    ...
    if (NOT columnNeedsHist) {
      ...
      if (NOT columnNeedsFullHist) column->setDontNeedHistogram();

      NABoolean copyIntervals = TRUE;
      ColStatsSharedPtr const singleColStats = cachedHistograms->getHistForCol(*column);
      ...
      list.insertAt(list.entries(),
                    ColStats::deepCopySingleColHistFromCache(*singleColStats, *column, list.heap(), copyIntervals));
    }
    ...
    if (columnNeedsHist) {
      ...
      cachedHistograms->getMCStatsForColFromCacheIntoList(mcStatsList, *column, singleColsFound);
    }
  ...
  if (interestingExpressions.entries() > 0) {
    ...
    cachedHistograms->getDesiredExpressionsStatsIntoList(list, interestingExpressions);
  }

  // append the mc stats at the end of the output lit.
  for (int i = 0; i < mcStatsList.entries(); i++) {
...
void HistogramCache::putStatsListIntoCache(StatsList &colStatsList, const NAColumnArray &colArray,
  ...
  // add it to the cache
  QualifiedName *key = const_cast<QualifiedName *>(histogramsForCache->getName());
...
NABoolean HistogramCache::enforceMemorySpaceConstraints() {
  if (size_ <= memoryLimit_) return TRUE;

  HistogramsCacheEntry *entry = NULL;
  while (lruQ_.entries()) {
    entry = lruQ_[0];
    if (entry->accessedInCurrentStatement()) return FALSE;
    deCache(&entry);
    if (size_ <= memoryLimit_) return TRUE;
  }
  return FALSE;
...
void HistogramCache::print(FILE *ofd, const char *indent, const char *title) const {
#ifndef NDEBUG
  BUMP_INDENT(indent);
  fprintf(ofd, "%s%s\n", NEW_INDENT, title);
...
void HistogramCache::openTraceFile(const char *filename) { tfd_ = fopen(filename, "w+"); }
...
void HistogramCache::openMonitorFile(const char *filename) { mfd_ = fopen(filename, "w+"); }
...
void HistogramCache::freeInvalidEntries(int numKeys, SQL_QIKEY *qiKeyArray) {
  ...
  for (int i = 0; i < it.entries(); i++) {
    it.getNext(qn, entry);
    if (qiCheckForInvalidObject(numKeys, qiKeyArray, entry->getTableUID(), dummy)) deCache(&entry);
  }
...
HistogramsCacheEntry::HistogramsCacheEntry(const StatsList &colStats, const QualifiedName &qualifiedName,
  ...
  if (singleColumnCount > 0) {
    ...
    for (UInt32 i = 0; i < colStats.entries(); i++) {
      ...
      int position = (int)colArray.getColumn(int(0))->getPosition();
      if ((colArray.entries() == 1) && (position >= 0)) {
        // keep pointer to deep copy of single-column histogram
        full_->insertAt(full_->entries(), ColStats::deepCopyHistIntoCache(*(colStats[i]), heap_));

        // update singleColumnPositions
        singleColumnPositions_ += (int)colArray.getColumn(int(0))->getPosition();
      }
    }
  }
...
void HistogramsCacheEntry::addToCachedEntry(NAColumnArray &columns, StatsList &list) {
  // update allFakeStats_
  if (allFakeStats_) allFakeStats_ = list.allFakeStats();
  ...
  for (UInt32 j = 0; j < list.entries(); j++) {
    ...
    NAColumn *column = colList.getColumn(int(0));

    // column position of first column
    int currentColPosition = column->getPosition();
    ...
    if ((requiresHistogram || NOT singleColHist) || list[j]->isFakeHistogram()) {
      ...
      if ((singleColHist) && (!singleColumnPositions_.contains(currentColPosition))) {
        ...
        full_->insertAt(full_->entries(), ColStats::deepCopyHistIntoCache(*(list[j]), heap_));
...
void HistogramsCacheEntry::addMultiColumnHistogram(const ColStats &mcStat, ColumnSet *singleColPositions) {
  if (!multiColumn_) multiColumn_ = new (heap_) MultiColumnHistogramList(heap_);

  multiColumn_->addMultiColumnHistogram(mcStat, singleColPositions);
}

const QualifiedName *HistogramsCacheEntry::getName() const { return name_; }
...
const MultiColumnHistogram *HistogramsCacheEntry::getMultiColumnAt(CollIndex x) const {
...
ColStatsSharedPtr const HistogramsCacheEntry::getHistForCol(NAColumn &col) const {
  ...
  for (UInt32 i = 0; i < full_->entries(); i++) {
    ...
    if (((*full_)[i]->getStatColumnPositions().entries() == 1) AND(*full_)[i]->getStatColumnPositions().contains(
            col.getPosition())) {
      return (*full_)[i];
    }
  }
...
// use singleColsFound to avoid duplicates
void HistogramsCacheEntry::getMCStatsForColFromCacheIntoList(
                                 ...
                                 // histograms for any of its columns are retrieved.
{
  ...
  for (UInt32 i = 0; i < multiColCount; i++) {
    ...
    if (mcHist) {
      ...
      if (!mcCols.contains(col.getPosition())) continue;  // no col

      if ((mcCols.intersectSet(singleColsFound)).entries()) continue;  // avoid dup
      ...
      if (col.getNATable()->isHbaseTable() && col.isPrimaryKey()) {
        ...
        mcStat = new (STMTHEAP) ColStats(*(mcHist->getColStatsPtr()), STMTHEAP, TRUE);
        ...
        HistInt hiInt(1, NULL, (*mcStat).statColumns(), rows, uec, boundaryInclusive, 0);
        histogram->insert(loInt);
        histogram->insert(hiInt);
        mcStat->setHistogram(histogram);

        MCSkewedValueList *mcSkewedValueList =
            new (STMTHEAP) MCSkewedValueList(*(mcHist->getMCSkewedValueList()), STMTHEAP);
...
void HistogramsCacheEntry::getDesiredExpressionsStatsIntoList(
    StatsList &list, NAHashDictionary<NAString, NABoolean> &interestingExpressions) {
  if (expressions_) {
    for (CollIndex i = 0; i < expressions_->entries(); i++) {
      ...
      if (interestingExpressions.contains(&expressionText)) {
        list.insertAt(list.entries(), ColStats::deepCopy(*expressionStats, list.heap()));
      }
    }
  }
}
...
HistogramsCacheEntry::~HistogramsCacheEntry() {
  ...
  if (multiColumn_) delete multiColumn_;
  if (name_) delete name_;
  singleColumnPositions_.clear();
}

void HistogramsCacheEntry::display() const { HistogramsCacheEntry::print(); }
...
void HistogramsCacheEntry::monitor(FILE *mfd) const {
  ...
  if (CmpCommon::getDefault(CACHE_HISTOGRAMS_MONITOR_MEM_DETAIL) == DF_ON) fprintf(mfd, "table_size:%d\n", size_);
  fflush(mfd);
}

void HistogramsCacheEntry::print(FILE *ofd, const char *indent, const char *title) const {
#ifndef NDEBUG
  BUMP_INDENT(indent);
  fprintf(ofd, "%s%s\n", NEW_INDENT, title);
  name_->print(ofd);
...
//
// -----------------------------------------------------------------------
ItemExpr *NATable::getRangePartitionBoundaryValues(const char *keyValueBuffer, const int keyValueBufferSize,
  ...
  if ((keyValueBuffer[stopIndex] == ')') && (leadingParen == TRUE)) stopIndex--;
...
static ItemExpr *getRangePartitionBoundaryValuesFromEncodedKeys(const NAColumnArray &partColArray,
                                                                const char *encodedKey, const int encodedKeyLen,
                                                                NAMemory *heap) {
  int keyColOffset = 0;
  ...
  int numProvidedCols = 0;
  int lenOfFullyProvidedCols = 0;
  int vcHdrExtraBytes = 0;
  int tmpCurrOffset = 0;
  ...
  for (CollIndex i = 0; i < partColArray.entries(); i++) {
    ...
    int colEncodedLength = pkType->getSQLnullHdrSize() + pkType->getNominalSize();
  ...
  if (encodedKeyLen < totalKeyLength) {
    ...
    int currOffset = lenOfFullyProvidedCols;
    ...
    for (CollIndex j = numProvidedCols; j < partColArray.entries(); j++) {
      ...
      int colEncodedLength = pkType->getTotalSize();
      ...
      if (columnIsPartiallyProvided) {
        ...
        if (nullHdrSize && numBytesInProvidedVal <= 0) {
          ...
          if (indicatorVal) indicatorVal = -1;
        ...
        if (columnIsPartiallyProvided) switch (pkType->getTypeQualifier()) {
            case NA_NUMERIC_TYPE: {
              NumericType *nt = (NumericType *)pkType;

              if (!nt->isExact() || nt->isDecimal() || nt->isBigNum() || (isDescending && nt->decimalPrecision()))
            ...
            case NA_DATETIME_TYPE:
              ...
              if (isDescending)
                columnIsPartiallyProvided = FALSE;
                       ...
                       static_cast<const DatetimeType *>(pkType)->getSubtype() != DatetimeType::SUBTYPE_SQLTime) {
                ...
                for (int ymd = numBytesInProvidedVal; ymd < 4; ymd++)
                  if (actEncodedKey[valOffset + ymd] == 0) actEncodedKey[valOffset + ymd] = 1;
              }
              break;
            ...
            case NA_INTERVAL_TYPE:
              // those types should tolerate zeroing out trailing bytes, but
              // not filling with 0xFF
              if (isDescending) columnIsPartiallyProvided = FALSE;
              break;
              ...
              // that got split in the middle
              {
                ...
                switch (cs) {
                  ...
                  case CharInfo::UTF8: {
                    ...
                    int validLen = lightValidateUTF8Str(&actEncodedKey[valOffset], numBytesInProvidedVal);

                    // replace the remainder of the buffer with UTF8 min/max chars
                    fillWithMinMaxUTF8Chars(&actEncodedKey[valOffset + validLen], valEncodedLength - validLen, 0,
                                            isDescending);
                    ...
                    if (pkType->getPrecisionOrMaxNumChars() > 0) {
                      ...
                      validLen = lightValidateUTF8Str(&actEncodedKey[valOffset], valEncodedLength,
      ...
      if (!columnIsPartiallyProvided) {
        ...
        if (pkType->getTypeName() == "VARCHAR") colEncodedLength += pkType->getVarLenHdrSize();
        int remainingBufLen = colEncodedLength;
        ...
        pkType->minMaxRepresentableValue(&actEncodedKey[valOffset], &remainingBufLen, isDescending, NULL, heap);
      }
  ...
  for (CollIndex c = 0; c < partColArray.entries(); c++) {
    ...
    int decodedValueLen = pkType->getNominalSize() + pkType->getSQLnullHdrSize();
    ...
    if (pkType->isEncodingNeeded() && c < numProvidedCols) {
      ...
      if (pkType->getTypeName() == "VARCHAR" || pkType->getTypeName() == "CHAR") {
        ...
        if (nullHdrSize > 0) str_cpy_all(varCharstr, encodedKeyP, nullHdrSize);

        // careful, this works on little-endian systems only!!
        str_cpy_all(varCharstr + nullHdrSize, (char *)&decodedValueLen, varLenSize);
        str_cpy_all(varCharstr + nullHdrSize + varLenSize, encodedKeyP + nullHdrSize, decodedValueLen - nullHdrSize);
      ...
      ConstValue *keyColEncVal =
          new (heap) ConstValue(pkType, (void *)encodedKeyP, decodedValueLen, &encConstLiteral, heap);
      ...
      if (keyColEncVal->isNull()) {
        ...
        keyColVal = new (heap) CompDecode(keyColEncVal, pkType, !partColArray.isAscending(c), decodedValueLen,
                                          CollationInfo::Sort, TRUE, heap);

        keyColVal->synthTypeAndValueId();

        keyColVal = keyColVal->evaluate(heap);

        if (!keyColVal) return NULL;
      }
      ...
      if (pkType->getTypeName() == "VARCHAR") decodedValueLen += pkType->getVarLenHdrSize();
      keyColVal = new (heap) ConstValue(pkType, (void *)&actEncodedKey[keyColOffset], decodedValueLen, NULL, heap);
    }

    // this and the above assumes that encoded and unencoded values
    // have the same length
    keyColOffset += decodedValueLen;
    if (pkType->isEncodingNeeded() && (pkType->getTypeName() == "VARCHAR" || pkType->getTypeName() == "CHAR") &&
        c < numProvidedCols) {
  ...
  if (actEncodedKey != encodedKey) NADELETEBASIC(actEncodedKey, heap);
...
NABoolean checkColumnTypeForSupportability(const NAColumnArray &partColArray, const char *key) {
  ...
  for (CollIndex c = 0; c < partColArray.entries(); c++) {
    ...
    if (NOT pkType->isSupportedType())
      ...
      if ((CmpCommon::getDefault(MARIAQUEST_PROCESS) == DF_OFF) && (NOT naTable->isSeabaseTable()) &&
    ...
    if (unsupportedPartnKey) {
      ...
      //
      const NAString &tableName = partColArray[c]->getNATable()->getTableName().getQualifiedNameAsAnsiString();

      if (unsupportedFloatDatatype)
        *CmpCommon::diags() << DgSqlCode(-1120);
      else
        // ERROR 1123 Unable to process the partition key values...
        *CmpCommon::diags() << DgSqlCode(-1123) << DgString0(key) << DgTableName(tableName);
...
// within a certain partition, for range-partitioned data.
// -----------------------------------------------------------------------
static RangePartitionBoundaries *createRangePartitionBoundaries(TrafDesc *part_desc_list, int numberOfPartitions,
                                                                const NAColumnArray &partColArray, NAMemory *heap) {
  // ---------------------------------------------------------------------
  ...
  if (!checkColumnTypeForSupportability(partColArray, key)) return NULL;

  // ---------------------------------------------------------------------
  // Allocate a new RangePartitionBoundaries.
  // ---------------------------------------------------------------------
  RangePartitionBoundaries *partBounds =
      new (heap) RangePartitionBoundaries(numberOfPartitions, partColArray.entries(), heap);
  ...
  while (partns_desc AND(counter < numberOfPartitions)) {
    ...
    if (heap != CmpCommon::statementHeap() && encodedKeyLen > 0) {
      ...
      str_cpy_all(encodedKey, partns_desc->partnsDesc()->encodedkey, encodedKeyLen);
    }
    ...
    if (partns_desc->partnsDesc()->firstkey)
      ...
      //
      rangePartBoundValues = NATable::getRangePartitionBoundaryValues(partns_desc->partnsDesc()->firstkey,
                                                                      partns_desc->partnsDesc()->firstkeylen, heap);
    else
      rangePartBoundValues =
          getRangePartitionBoundaryValuesFromEncodedKeys(partColArray, encodedKey, encodedKeyLen, heap);

    // Check to see if the key values parsed successfully, also check
    // for constants mistaken as a column reference.
    if (rangePartBoundValues == NULL || rangePartBoundValues->containsOpType(ITM_REFERENCE)) {
      ...
      //
      const NAString &tableName = partColArray[0]->getNATable()->getTableName().getQualifiedNameAsAnsiString();

      // The Parser will have already issued an error.
      // ERROR 1123 Unable to process the partition key values...
      *CmpCommon::diags() << DgSqlCode(-1123) << DgString0(partns_desc->partnsDesc()->firstkey)
    ...
    partBounds->defineUnboundBoundary(counter++, rangePartBoundValues, encodedKey);
...
// -----------------------------------------------------------------------
static PartitioningFunction *createRangePartitioningFunction(TrafDesc *part_desc_list,
  ...
  if (numberOfPartitions == 1) return new (heap) SinglePartitionPartitioningFunction(nodeMap, heap);

  // ---------------------------------------------------------------------
  // Create the partitioning key ranges
  // ---------------------------------------------------------------------
  RangePartitionBoundaries *boundaries =
      createRangePartitionBoundaries(part_desc_list, numberOfPartitions, partKeyColArray, heap);
...
// -----------------------------------------------------------------------
static PartitioningFunction *createRoundRobinPartitioningFunction(TrafDesc *part_desc_list, NodeMap *nodeMap,
                                                                  NAMemory *heap) {
  // ---------------------------------------------------------------------
  // Compute the number of partitions.
  // ---------------------------------------------------------------------
...
// -----------------------------------------------------------------------
static PartitioningFunction *createHashDistPartitioningFunction(TrafDesc *part_desc_list,
                                                                const NAColumnArray &partKeyColArray, NodeMap *nodeMap,
                                                                NAMemory *heap) {
  // ---------------------------------------------------------------------
  // Compute the number of partitions.
...
// -----------------------------------------------------------------------
static PartitioningFunction *createHash2PartitioningFunction(TrafDesc *part_desc_list,
                                                             const NAColumnArray &partKeyColArray, NodeMap *nodeMap,
                                                             NAMemory *heap) {
  // ---------------------------------------------------------------------
  // Compute the number of partitions.
...
static NodeMap *createNodeMapForHbase(TrafDesc *desc, const NATable *table, int numSaltBuckets, NAMemory *heap) {
  ...
  NodeMap *nodeMap = new (heap) NodeMap(heap, partns, NodeMapEntry::ACTIVE, NodeMap::HBASE);
  ...
  if (partns > 1 && (CmpCommon::getDefault(TRAF_ALLOW_ESP_COLOCATION) == DF_ON) &&
    ...
    if (table->getRegionsNodeName(partns, nodeNames)) {
      for (int p = 0; p < partns; p++) {
        ...
        if (size && size != NA_NPOS) node.remove(size);
...
static PartitioningFunction *createHash2PartitioningFunctionForHBase(TrafDesc *desc, const NATable *table,
...
RangePartitionBoundaries *createRangePartitionBoundariesFromStats(const IndexDesc *idesc, HistogramSharedPtr &hist,
  ...
  while (iter.isValid()) {
    ...
    double ev = (!iter.isLast()) ? iter.hiBound().getDblValue() : nt->getMaxValue();
    ...
    if ((partColArray.entries() == 1) && (statsColsCount == 1)) {
      ...
      if (nt->getTypeQualifier() == NA_NUMERIC_TYPE && nt->getTypeName() == LiteralNumeric)
        if ((((NumericType *)nt))->isExact()) bExact = TRUE;
      if (!bExact)
    ...
    if (!evInStr) return NULL;

    // Construct a boundary as ItemExprList of ConstValues
    ItemExpr *rangePartBoundValues =
        NATable::getRangePartitionBoundaryValues(evInStr->data(), evInStr->length(), heap, CharInfo::ISO88591);
    ...
    for (CollIndex c = 0; c < partColArray.entries(); c++) {
      ...
      short ok =
          vidList.evaluateTree(encodeExpr, encodedKeyBuffer, encodedKeyLength, &length, &offset, (CmpCommon::diags()));
    ...
    if (totalEncodedKeyLength != 0) {
      partBounds->defineUnboundBoundary(counter++, rangePartBoundValues, char_totalEncodedKeyBuf);
    }
  ...
  // -----------------------------------------------------------------
  partBounds->completePartitionBoundaries(partitioningKeyColumnsOrder, totalEncodedKeyLength);

  return partBounds;

}  // createRangePartitionBoundariesFromStats()

static PartitioningFunction *createRangePartitioningFunctionForSingleRegionHBase(const NAColumnArray &partKeyColArray,
                                                                                 NAMemory *heap) {
  NodeMap *nodeMap = NULL;

  int regionsToFake = (ActiveSchemaDB()->getDefaults()).getAsLong(HBASE_USE_FAKED_REGIONS);

  if (regionsToFake == 0) {
    nodeMap = new (heap) NodeMap(heap, 1, NodeMapEntry::ACTIVE, NodeMap::HBASE);

    return new (heap) SinglePartitionPartitioningFunction(nodeMap, heap);
  }

  nodeMap = new (heap) NodeMap(heap, regionsToFake, NodeMapEntry::ACTIVE, NodeMap::HBASE);
  ...
  //
  return createRangePartitioningFunction(head, partKeyColArray, nodeMap, heap);
}

void populatePartnDescOnEncodingKey(struct TrafDesc *prevEndKey, struct TrafDesc *tail, struct TrafDesc *hrk,
                                    NAMemory *heap) {
  if (!prevEndKey) {
    ...
    tail->partnsDesc()->encodedkeylen = len;
    tail->partnsDesc()->encodedkey = new (heap) char[len];
    memcpy(tail->partnsDesc()->encodedkey, prevEndKey->hbaseRegionDesc()->endKey, len);
  }
}
...
typedef void (*populatePartnDescT)(struct TrafDesc *prevEndKey, struct TrafDesc *tail, struct TrafDesc *hrk,
                                   NAMemory *heap);
static struct TrafDesc *convertRangeDescToPartnsDesc(TrafDesc *desc, populatePartnDescT funcPtr, NAMemory *heap) {
  TrafDesc *hrk = desc;
...
static PartitioningFunction *createRangePartitioningFunctionForMultiRegionHBase(int partns, TrafDesc *desc,
  ...
  struct TrafDesc *partns_desc = (table->isHbaseCellTable() || table->isHbaseRowTable())
...
static PartitioningFunction *createRangePartitioningFunctionForHBase(TrafDesc *desc, const NATable *table,
  ...
  return (partns > 1) ? createRangePartitioningFunctionForMultiRegionHBase(partns, desc, table, partKeyColArray, heap)
...
// on the local node.
//-------------------------------------------------------------------------
static NABoolean checkRemote(TrafDesc *part_desc_list, char *tableName) { return TRUE; }
...
// -----------------------------------------------------------------------
static NABoolean createNAColumns(TrafDesc *column_desc_list /*IN*/, NATable *table /*IN*/, long tablesFlags, /*IN*/
                                 NAColumnArray &colArray /*OUT*/, NAMemory *heap /*IN*/) {
  NAType *type;
  ...
  while (column_desc_list) {
    ...
    if (NAColumn::createNAType(column_desc, table, tablesFlags, type, heap)) return TRUE;

    // Get the column class.  The column will either be a system column or a
    // user column.
    //
    if (column_desc->colclass == *(char *)COM_SYSTEM_COLUMN_LIT) {
      if ((CmpCommon::getDefault(OVERRIDE_SYSKEY) == DF_ON) && (strcmp(column_desc->colname, "SYSKEY") == 0) &&
      ...
      *CmpCommon::diags() << DgSqlCode(-4032) << DgColumnName(makeColumnName(table, column_desc))
    ...
    if (column_desc->colname[0] != '\0') {
      ...
      if (column_desc->defaultClass() == COM_ALWAYS_COMPUTE_COMPUTED_COLUMN_DEFAULT) {
        if (column_desc->colFlags & SEABASE_COLUMN_IS_SALT) isSaltColumn = TRUE;
        if (column_desc->colFlags & SEABASE_COLUMN_IS_DIVISION) isDivisioningColumn = TRUE;
      ...
      newColumn = new (heap) NAColumn(column_desc->colname, column_desc->colnumber, type, heap, table, colClass,
                                      column_desc->defaultClass(), defaultValue, heading, column_desc->isUpshifted(),
                                      ((column_desc->colclass == *(char *)COM_ADDED_USER_COLUMN_LIT) ||
                                       (column_desc->colclass == *(char *)COM_ADDED_ALTERED_USER_COLUMN_LIT)),
                                      COM_UNKNOWN_DIRECTION, FALSE, NULL,
                                      TRUE,  // stored on disk
                                      computed_column_text, isSaltColumn, isDivisioningColumn, isReplicaColumn,
                                      ((column_desc->colclass == *(char *)COM_ADDED_ALTERED_USER_COLUMN_LIT) ||
                                       (column_desc->colclass == *(char *)COM_ALTERED_USER_COLUMN_LIT)),
                                      initDefaultValue);
    } else {
    ...
    if (isMvSystemAdded) newColumn->setMvSystemAddedColumn();

    if (table && ((table->isSeabaseTable()) || (table->isHbaseCellTable()) || (table->isHbaseRowTable()))) {
      if (column_desc->hbaseColFam) newColumn->setHbaseColFam(column_desc->hbaseColFam);
      if (column_desc->hbaseColQual) newColumn->setHbaseColQual(column_desc->hbaseColQual);

      newColumn->setHbaseColFlags(column_desc->hbaseColFlags);
    }

    if (table != NULL) {
      if (newColumn->isAddedColumn()) table->setHasAddedColumn(TRUE);

      if (newColumn->getType()->isVaryingLen()) table->setHasVarcharColumn(TRUE);

      if (hasSystemColumnAsUserColumn) table->setSystemColumnUsedAsUserColumn(TRUE);

      if (CmpSeabaseDDL::isEncodingNeededForSerialization(newColumn)) table->setHasSerializedEncodedColumn(TRUE);

      if (CmpSeabaseDDL::isSerialized(newColumn->getHbaseColFlags())) table->setHasSerializedColumn(TRUE);
    }

    colArray.insert(newColumn);

    if (column_desc->compDefnStr) hasCompositeCols = TRUE;
...
NAType *getSQColTypeForHive(const char *hiveType, NABoolean isORC, NAMemory *heap) {
  return NAType::getNATypeForHive(hiveType, isORC, FALSE, heap);
}

static NABoolean addVirtualHiveColumns(NAColumnArray &colArray, NATable *table, NABoolean isView, int maxColIx,
                                       NAMemory *heap) {
  ...
  for (int v = 0; v < 4; v++) {
    ...
    NAColumn *newVirtColumn = new (heap) NAColumn(virtColName, ++maxColIx, virtType, heap, table, SYSTEM_COLUMN);
...
NABoolean createNAKeyColumns(TrafDesc *keys_desc_list /*IN*/, NAColumnArray &colArray /*IN*/,
                             NAColumnArray &keyColArray /*OUT*/, CollHeap *heap /*IN*/) {
...
static void createHiveKeyColumns(const NAColumnArray &colArray /*IN*/, NAColumnArray &keyColArray /*OUT*/,
                                 CollHeap *heap /*IN*/) {
  ...
  const char *colName;
  NABoolean persistentKey = (CmpCommon::getDefault(HIVE_USE_PERSISTENT_KEY) != DF_OFF);
...
void NATable::statsToUse(TrafDesc *storedStatsDesc, NABoolean &useEstimatedStats, NABoolean &useStoredStats) {
  useEstimatedStats = FALSE;
  useStoredStats = FALSE;

  if ((!storedStatsDesc) || (!storedStatsDesc->tableStatsDesc()) ||
      ((NOT isPreloaded()) && (CmpCommon::getDefault(TRAF_USE_STORED_STATS) == DF_OFF)) ||
      ((!storedStatsDesc->tableStatsDesc()->histograms_desc) && (storedStatsDesc->tableStatsDesc()->rowcount == -1)))
    return;
...
// -----------------------------------------------------------------------
static NABoolean createNAFileSets(TrafDesc *table_desc /*IN*/, const NATable *table /*IN*/,
                                  const NAColumnArray &colArray /*IN*/, NAFileSetList &indexes /*OUT*/,
  ...
  while (indexes_desc AND indexes_desc->indexesDesc()->keytag) indexes_desc = indexes_desc->next;

  // must have a clustering key if not view or is a virtual table
  CMPASSERT((indexes_desc AND !indexes_desc->indexesDesc()->keytag)OR(table_desc->tableDesc()->views_desc)
                OR(((NATable *)table)->getTableType() == ExtendedQualName::VIRTUAL_TABLE));
  ...
  if (table->isHbaseTable()) {
    ...
    if (table->storedStatsAvail()) doStoredStatsOpt = TRUE;

    if (doStoredStatsOpt) {
      TrafTableStatsDesc *statsDesc = table->getStoredStatsDesc()->tableStatsDesc();
      ...
      if (hbtIndexLevels == 0) res = table->getHbaseTableInfo(hbtIndexLevels, hbtBlockSize);
    }
  }

  NABoolean doHash2 =
      (CmpCommon::getDefault(HBASE_HASH2_PARTITIONING) != DF_OFF && !(bindWA && bindWA->isTrafLoadPrep()));
  ...
  while (indexes_desc) {
    ...
    while (keys_desc) {
      ...
      if ((table->isHbaseTable()) &&
          ((currIndexDesc->keytag != 0) || (indexAlignedRowFormat && indexAlignedRowFormat != tableAlignedRowFormat))) {
      ...
      order = keys_desc->keysDesc()->isDescending() ? DESCENDING : ASCENDING;
      indexKeyColumns.setAscending(indexKeyColumns.entries() - 1, order == ASCENDING);

      if (table->isHbaseTable() && indexColumn->isSaltColumn()) {
        ...
        Parser parser(CmpCommon::context());
        ItemExpr *saltExpr = parser.getItemExprTree(saltClause, strlen(saltClause), CharInfo::UTF8);

        CMPASSERT(saltExpr && saltExpr->getOperatorType() == ITM_HASH2_DISTRIB);
      ...
      if (table->isHbaseTable() && indexColumn->isReplicaColumn() &&
          ((NATable *)table)->getTableType() != ExtendedQualName::INDEX_TABLE) {
        numTrafReplicas = str_atoi(indexColumn->getDefaultValue(), str_len(indexColumn->getDefaultValue()));
      }
    ...
    while (non_keys_desc) {
      ...
      if ((table->isHbaseTable()) &&
          ((currIndexDesc->keytag != 0) || (indexAlignedRowFormat && indexAlignedRowFormat != tableAlignedRowFormat))) {
        newIndexColumn = new (heap) NAColumn(*indexColumn);
        if (non_keys_desc->keysDesc()->keyname) newIndexColumn->setIndexColName(non_keys_desc->keysDesc()->keyname);
    ...
    if (partitioning_keys_desc) {
      ...
      while (keys_desc) {
        ...
        partitioningKeyColumns.setAscending(partitioningKeyColumns.entries() - 1, order == ASCENDING);
        keys_desc = keys_desc->next;
      }  // end while (keys_desc)
    } else {
    ...
    if (files_desc) {
      ...
      if (checkRemote(files_desc->filesDesc()->partns_desc, currIndexDesc->indexname))
      ...
      switch (currIndexDesc->partitioningScheme()) {
        case COM_ROUND_ROBIN_PARTITIONING:
          ...
          partFunc = createRoundRobinPartitioningFunction(files_desc->filesDesc()->partns_desc, nodeMap, heap);
          break;

        case COM_HASH_V1_PARTITIONING:
          // Hash partitioned table
          partFunc = createHashDistPartitioningFunction(files_desc->filesDesc()->partns_desc, partitioningKeyColumns,
                                                        nodeMap, heap);
          break;

        case COM_HASH_V2_PARTITIONING:
          // Hash partitioned table
          partFunc = createHash2PartitioningFunction(files_desc->filesDesc()->partns_desc, partitioningKeyColumns,
                                                     nodeMap, heap);
        ...
        case COM_SYSTEM_PARTITIONING: {
          ...
          if (doHash2) doHash2 = (numOfSaltedPartitions > 0);

          if (doHash2) {
            partFunc = createHash2PartitioningFunctionForHBase(NULL, table, numOfSaltedPartitions, heap);
            ...
            if (((!currIndexDesc->hbase_regionkey_desc) &&
                 (table->getSpecialType() != ExtendedQualName::VIRTUAL_TABLE) && (NOT table->isSeabaseMDTable()) &&
                 (NOT table->isHistogramTable())) &&
                ((currIndexDesc->numSaltPartns == 0) || (CmpCommon::getDefault(HBASE_HASH2_PARTITIONING) == DF_OFF) ||
            ...
            if (genHbaseRegionDesc) {
              ...
              cmpSBD.genHbaseRegionDescs(currIndexDesc, qualIndexName.getCatalogName(), qualIndexName.getSchemaName(),
                                         qualIndexName.getObjectName(), ((NATable *)table)->getNamespace());

              if (currIndexDesc->keytag == 0) tDesc->hbase_regionkey_desc = currIndexDesc->hbase_regionkey_desc;
            }

            if ((!tDesc->hbase_regionkey_desc) && ((table->getSpecialType() == ExtendedQualName::VIRTUAL_TABLE) ||
                                                   (table->isSeabaseMDTable()) || (table->isHistogramTable()))) {
              partFunc = createRangePartitioningFunction(files_desc->filesDesc()->partns_desc, partitioningKeyColumns,
                                                         nodeMap, heap);
            } else {
              partFunc = createRangePartitioningFunctionForHBase(currIndexDesc->hbase_regionkey_desc, table,
                                                                 partitioningKeyColumns, heap);
            }
          }  // else
          break;
        }
        case COM_UNKNOWN_PARTITIONING: {
          *CmpCommon::diags() << DgSqlCode(-4222) << DgString0("Unsupported partitioning");
          return TRUE;
        }
        default:
      ...
      if (partns_desc) {
        while (partns_desc) {
          if (indexLevels < partns_desc->partnsDesc()->indexlevel) indexLevels = partns_desc->partnsDesc()->indexlevel;
          partns_desc = partns_desc->next;
        }
      }
    }
    ...
    NAString extIndexName(qualIndexName.getQualifiedNameAsAnsiString(), CmpCommon::statementHeap());
    ...
    if (partFunc) numberOfFiles = partFunc->getCountOfPartitions();

    CMPASSERT(currIndexDesc->blocksize > 0);

    NAList<HbaseCreateOption *> *hbaseCreateOptions = NULL;
    if ((currIndexDesc->hbaseCreateOptions) &&
        (CmpSeabaseDDL::genHbaseCreateOptions(currIndexDesc->hbaseCreateOptions, hbaseCreateOptions, heap, NULL, NULL)))
      return TRUE;
    ...
    if (currIndexDesc->keys_desc) {
      ...
      localKeysDesc = TrafDesc::copyDescList(currIndexDesc->keys_desc, heap);
    }

    newIndex = new (heap) NAFileSet(
        qualIndexName,     // QN containing "\NSK.$VOL", FUNNYSV, FUNNYNM
        qualExtIndexName,  // :
        extIndexName,      // string containing Ansi name CAT.SCH."indx"
        KEY_SEQUENCED_FILE, isSystemTable, numberOfFiles, 100, currIndexDesc->record_length, blockSize, indexLevels,
        allColumns, indexKeyColumns, partitioningKeyColumns, noHiveSortColumns, partFunc, currIndexDesc->keytag, 0,
        files_desc ? files_desc->filesDesc()->isAudited() : 0, 0, 0, COM_NO_COMPRESSION, 0, 0, 0, isPacked,
        hasRemotePartition, ((currIndexDesc->keytag != 0) && (currIndexDesc->isUnique())),
        currIndexDesc->isNgram(),  // ngram
        currIndexDesc->isPartLocalBaseIndex(), currIndexDesc->isPartLocalIndex(), currIndexDesc->isPartGlobalIndex(), 0,
        0, (currIndexDesc->isVolatile()), (currIndexDesc->isInMemoryObject()), currIndexDesc->indexUID, localKeysDesc,
        currIndexDesc->numSaltPartns, currIndexDesc->numInitialSaltRegions, numTrafReplicas, hbaseCreateOptions, heap);

    if (isNotAvailable) newIndex->setNotAvailable(TRUE);

    newIndex->setRowFormat(currIndexDesc->rowFormat());
    // Mark each NAColumn in the list
    indexKeyColumns.setIndexKey();
    if ((table->isHbaseTable()) && (currIndexDesc->keytag != 0)) saveNAColumns.setIndexKey();

    if (currIndexDesc->isExplicit()) newIndex->setIsCreatedExplicitly(TRUE);

    // if index is unique and is on one column, then mark column as unique
    if ((currIndexDesc->isUnique()) && (indexKeyColumns.entries() == 1)) indexKeyColumns[0]->setIsUnique();
    ...
    if (indexes_desc AND !indexes_desc->indexesDesc()->keytag) indexes_desc = indexes_desc->next;
...
// -----------------------------------------------------------------------
static void markPKCols(const TrafConstrntsDesc *constrnt /*IN*/, const NAColumnArray &columnArray /*IN*/) {
  TrafDesc *keycols_desc = constrnt->constr_key_cols_desc;
  while (keycols_desc) {
    TrafConstrntKeyColsDesc *key = keycols_desc->constrntKeyColsDesc();
    // Lookup by name (not position: key->position is pos *within the PK*)
    NAColumn *nacol = columnArray.getColumn(key->colname);
    if (nacol != NULL) {
      nacol->setPrimaryKey();
      if (((TrafConstrntsDesc *)constrnt)->notSerialized()) nacol->setPrimaryKeyNotSerialized();
    }
...
// -----------------------------------------------------------------------
static NABoolean createConstraintInfo(const TrafDesc *table_desc /*IN*/, const QualifiedName &tableQualName /*IN*/,
                                      const NAColumnArray &columnArray /*IN*/,
                                      CheckConstraintList &checkConstraints /*OUT*/,
                                      AbstractRIConstraintList &uniqueConstraints,
                                      AbstractRIConstraintList &refConstraints, NAMemory *heap, BindWA *bindWA) {
    ...
    switch (constrntHdr->type) {
      ...
      case UNIQUE_CONSTRAINT: {
        UniqueConstraint *uniqueConstraint = new (heap)
            UniqueConstraint(constrntName, tableQualName, heap, (constrntHdr->type == PRIMARY_KEY_CONSTRAINT));
      ...
      case REF_CONSTRAINT: {
        char *refConstrntName = constrntHdr->referenced_constrnts_desc->refConstrntsDesc()->constrntname;
        char *refTableName = constrntHdr->referenced_constrnts_desc->refConstrntsDesc()->tablename;
        ...
        RefConstraint *refConstraint =
            new (heap) RefConstraint(constrntName, tableQualName, refConstrnt, refTable, heap);
      ...
      case CHECK_CONSTRAINT: {
        char *constrntText = constrntHdr->check_constrnts_desc->checkConstrntsDesc()->constrnt_text;
        checkConstraints.insert(new (heap) CheckConstraint(constrntName, constrntText, heap));
      } break;
      default:
        CMPASSERT(FALSE);
    }
...
static long lookupObjectUidByName(const QualifiedName &qualName, ComObjectType objectType, NABoolean reportError,
  ...
  long l_createTime = -1;
  long l_objDataUID = -1;
  objectUID = cmpSBD.getObjectInfo(&cliInterface, qualName.getCatalogName().data(), qualName.getSchemaName().data(),
                                   qualName.getObjectName().data(), objectType, objectOwner, schemaOwner, l_objFlags,
                                   l_objDataUID, reportError, FALSE, &l_createTime, NULL);

  if (objectFlags) *objectFlags = l_objFlags;

  if (createTime) *createTime = l_createTime;

  if (objDataUID) *objDataUID = l_objDataUID;
...
NABoolean NATable::fetchObjectUIDForNativeTable(const CorrName &corrName, NABoolean isView) {
  objectUID_ = 0;

  // get uid if this hive table has been registered in traf metadata.
  long regCreateTime = -1;
  long extCreateTime = -1;
  long regObjectUID = -1;
  long extObjectUID = -1;
  long objDataUID = -1;
  if ((corrName.isHive()) || (corrName.isHbase())) {
    ...
    regObjectUID = lookupObjectUidByName(corrName.getQualifiedNameObj(), objType, FALSE, &objectFlags, &regCreateTime,
                                         &objDataUID);

    if (NOT isView) {
      // then get uid for corresponding external table.
      NAString adjustedName =
          ComConvertNativeNameToTrafName(corrName.getQualifiedNameObj().getCatalogName(),
      ...
      extObjectUID = lookupObjectUidByName(extObjName, COM_BASE_TABLE_OBJECT, FALSE, NULL, &extCreateTime, &objDataUID);
    }
    ...
    if (regObjectUID > 0)  // hive table is registered
    {
      ...
      if (CmpSeabaseDDL::isMDflagsSet(objectFlags, MD_OBJECTS_INTERNAL_REGISTER)) setIsInternalRegistered(TRUE);
    }
    ...
    objectUID_ = lookupObjectUidByName(extObjName, COM_BASE_TABLE_OBJECT, FALSE, NULL, &extCreateTime, &objDataUID);
    if (objectUID_ > 0) setHasExternalTable(TRUE);
  }
...
short NATable::addEncryptionInfo() {
  ComEncryption::initEncryptionInfo(
      (baseTableUID_.get_value() == 0 ? objectUID_.get_value() : baseTableUID_.get_value()),
      (useRowIdEncryption() ? ComEncryption::AES_256_CBC : -1), (useDataEncryption() ? ComEncryption::AES_256_CBC : -1),
      encryptionInfo_);
...
NATable::NATable(BindWA *bindWA, const CorrName &corrName, NAMemory *heap, TrafDesc *inTableDesc, UInt32 currentEpoch,
                 UInt32 currentFlags)
    ...
    : heap_(heap),
      ...
      colArray_(heap),
      hiveColArray_(heap),
      interestingExpressions_(hashKey, NAHashDictionary_Default_Size, TRUE /* enforce uniqueness */, heap),
      recordLength_(0),
      indexes_(heap),
      vertParts_(heap),
      colStats_(NULL),
  ...
  int preCreateNATableWarnings = CmpCommon::diags()->getNumber(DgSqlCode::WARNING_);
  ...
  if ((corrName.isHbase()) || (corrName.isSeabase())) {
    ...
    if (CmpSeabaseDDL::isMDflagsSet(table_desc->tableDesc()->objectFlags, MD_OBJECTS_STORED_DESC)) setStoredDesc(TRUE);
  }
  ...
  if (CmpSeabaseDDL::isMDflagsSet(table_desc->tableDesc()->objectFlags, MD_OBJECTS_INCR_BACKUP_ENABLED))
    setIncrBackupEnabled(TRUE);

  if (CmpSeabaseDDL::isMDflagsSet(table_desc->tableDesc()->objectFlags, MD_OBJECTS_READ_ONLY_ENABLED))
  ...
  if (CmpSeabaseDDL::isMDflagsSet(table_desc->tableDesc()->objectFlags, MD_OBJECTS_ENCRYPT_ROWID_FLG))
    setUseRowIdEncryption(TRUE);

  if (CmpSeabaseDDL::isMDflagsSet(table_desc->tableDesc()->objectFlags, MD_OBJECTS_ENCRYPT_DATA_FLG))
    setUseDataEncryption(TRUE);

  if (CmpSeabaseDDL::isMDflagsSet(table_desc->tableDesc()->objectFlags, MD_OBJECTS_INCLUDE_TRIGGER)) {
    setHasTrigger(TRUE);
    // remove triggers from cache associated with this table
    TriggerDB::removeTriggers(getTableName(), COM_INSERT);
  }

  if (CmpSeabaseDDL::isMDflagsSet(table_desc->tableDesc()->objectFlags, MD_PARTITION_TABLE_V2))
    if (qualifiedName_.getSpecialType() == ExtendedQualName::INDEX_TABLE)
      setIsPartitionV2IndexTable(TRUE);
    else
      setIsPartitionV2Table(TRUE);

  if (CmpSeabaseDDL::isMDflagsSet(table_desc->tableDesc()->objectFlags, MD_PARTITION_V2))
    if (qualifiedName_.getSpecialType() == ExtendedQualName::INDEX_TABLE)
  ...
  if (isHbaseCell_ || isHbaseRow_) {
    if (!fetchObjectUIDForNativeTable(corrName, FALSE)) return;
  }
  ...
  if (CmpSeabaseDDL::isMDflagsSet(table_desc->tableDesc()->tablesFlags, MD_TABLES_HIVE_EXT_COL_ATTRS))
    setHiveExtColAttrs(TRUE);
  if (CmpSeabaseDDL::isMDflagsSet(table_desc->tableDesc()->tablesFlags, MD_TABLES_HIVE_EXT_KEY_ATTRS))
  ...
  if ((NOT defaultUserColFam_.isNull()) && (defaultUserColFam_ != SEABASE_DEFAULT_COL_FAMILY)) {
  ...
  if (table_desc->tableDesc()->tableNamespace) tableNamespace_ = table_desc->tableDesc()->tableNamespace;
  ...
  if ((isSeabaseTable()) && (NOT isSeabaseMDTable()) && (NOT isHistogramTable()) && (NOT isSeabasePrivSchemaTable()) &&
      ((getSpecialType() == ExtendedQualName::NORMAL_TABLE) || (getSpecialType() == ExtendedQualName::INDEX_TABLE)) &&
      (table_desc->tableDesc()->table_stats_desc) && (table_desc->tableDesc()->table_stats_desc->tableStatsDesc())) {
    storedStatsDesc_ = TrafDesc::copyDescList(table_desc->tableDesc()->table_stats_desc->tableStatsDesc(), heap);
  }

  // create column descs for seabase tables
  if ((isSeabaseTable()) && (table_desc->tableDesc()->columns_desc)) {
    // make a local copy of columns_desc in 'heap'
    columnsDesc_ = TrafDesc::copyDescList(table_desc->tableDesc()->columns_desc, heap);
  }
  ...
  //
  if (createNAColumns(table_desc->tableDesc()->columns_desc, this, table_desc->tableDesc()->tablesFlags,
                      colArray_ /*OUT*/, heap_))
    // coverity[leaked_storage]
  ...
  if (view_desc) {
    ...
    if (view_desc->viewDesc()->viewchecktext) {
      ...
      memcpy(viewCheck_, view_desc->viewDesc()->viewchecktext, viewCheckLength);
    }
    ...
    memcpy(viewFileName_, view_desc->viewDesc()->viewfilename, viewFileNameLength);
  } else {
    // keep track of memory used by NAFileSets
    int preCreateNAFileSetsMemSize = heap_->getAllocSize();

    //
    // Process indexes and vertical partitions for this table.
    //
    if (createNAFileSets(table_desc /*IN*/, this /*IN*/, colArray_ /*IN*/, indexes_ /*OUT*/, vertParts_ /*OUT*/,
                         clusteringIndex_ /*OUT*/, tableIdList_ /*OUT*/, heap_, bindWA, newColumns_, /*OUT*/
                         maxIndexLevelsPtr)) {
      return;  // colcount_ == 0 indicates an error
    }
    ...
    NABoolean errorOccurred = createConstraintInfo(
        table_desc /*IN*/, getTableName() /*IN*/, getNAColumnArray() /*IN (some columns updated)*/,
        checkConstraints_ /*OUT*/, uniqueConstraints_ /*OUT*/, refConstraints_ /*OUT*/, heap_, bindWA);
  ...
  if ((qualifiedName_.isPartitionNameSpecified()) || (qualifiedName_.isPartitionRangeSpecified())) {
  ...
  while (mvs_desc) {
    ...
    UsingMvInfo *usingMv =
        new (heap_) UsingMvInfo(mv->mvName, mv->refreshType(), mv->rewriteEnabled, mv->isInitialized, heap_);
  ...
  if (sequence_desc != NULL) {
    ...
    if (sg_desc != NULL) {
      sgAttributes_ = new (heap_) SequenceGeneratorAttributes(
          sg_desc->startValue, sg_desc->increment, sg_desc->maxValue, sg_desc->minValue, sg_desc->sgType(),
          (ComSQLDataType)sg_desc->sqlDataType, (ComFSDataType)sg_desc->fsDataType, sg_desc->cycleOption, FALSE,
          sg_desc->objectUID, sg_desc->cache, sg_desc->nextValue, sg_desc->seqOrder, 0, sg_desc->redefTime);
      if (sgAttributes_->isSystemSG()) {
        long override_seq_timeout = 0;
        ...
        if ((override_seq_timeout = CmpCommon::getDefaultNumeric(OVERRIDE_GLOBAL_SEQ_TIMEOUT)) != -1)
          ...
          sgAttributes_->setSGTimeout(
              sg_desc->increment);  // increment is overloaded to hold the timeout value in the SEQ_GEN metadata table
      }
      ...
      if ((CmpCommon::getDefault(TRAF_SEQUENCE_USE_DTM) == DF_ON) || (sgAttributes_->getSGSyncRepl()))
  ...
  if (partitionv2_desc) {
    ...
    populateColumnIdxArray(partv2_desc->partitionColIdx, &partitionColIdxAry_, partitionColCount_, heap_);
    ...
    if (subpartitionColCount_ > 0) {
      populateColumnIdxArray(partv2_desc->subpartitionColIdx, &subpartitionColIdxAry_, subpartitionColCount_, heap_);
      for (int i = 0; i < partitionColCount_; i++)
    ...
    NABoolean rc = createNAPartitions(part_desc, stlPartitionCnt_, colNameList, partArray_, heap_);
    if (rc) return;
        ...
        if (part_desc_list != NULL) {
          ...
          if (pdesc->subpartitionCnt > 0) {
            ...
            *subp = new (heap_) NAPartitionArray(heap_);
            rc = createNAPartitions(pdesc->subpart_desc, pdesc->subpartitionCnt, subColNameList, *(*subp), heap_);
            if (rc) return;
          }
        }
  ...
  if ((qualifiedName_.isCacheable()) && (NOT(isHbaseTable())) &&
      // this object is not on the statement heap (i.e. it is being cached)
      ((heap_ != CmpCommon::statementHeap()) || (OSIM_runningInCaptureMode()))) {
    char *nodeName = NULL;
    char *catStr = NULL;
    char *schemaStr = NULL;
    char *fileStr = NULL;
    ...
    if (viewText_) {
      ...
      if (!OSIM_runningSimulation()) primaryNodeNum = CURRCONTEXT_CLUSTERINFO->mapNodeNameToNodeNum(NAString(nodeName));
    } else {
      // get qualified name of the clustering index which should
      // be the actual physical file name of the table
      const QualifiedName fileNameObj = getClusteringIndex()->getRandomPartition();
  ...
  int postCreateNATableWarnings = CmpCommon::diags()->getNumber(DgSqlCode::WARNING_);
  if (postCreateNATableWarnings != preCreateNATableWarnings) tableConstructionHadWarnings_ = TRUE;
...
NATable::NATable(const NATable &other, NAMemory *h, NATableHeapType heapType)
    : initialSize_(other.initialSize_),
      ...
      clusteringIndex_(other.clusteringIndex_ ?  // some data member is shallow copied
                           new (h) NAFileSet(*other.clusteringIndex_, h)
                                              : NULL),
      ...
      viewColUsages_((other.viewColUsages_) ?  // shallow copied
                         new (h) NAList<ComViewColUsage *>(*other.viewColUsages_, h)
                                            : NULL),
      ...
      sgAttributes_((other.sgAttributes_) ? new (h) SequenceGeneratorAttributes(*other.sgAttributes_, h) : NULL),
      ...
      privDescs_((other.privDescs_) ?  // shallow copied
                     new (h) PrivMgrDescList(*other.privDescs_, h)
                                    : NULL),

      privInfo_(other.privInfo_ ? new (h) PrivMgrUserPrivs(*other.privInfo_) : NULL),
      ...
      partitionColIdxAry_(allocateAndCopyInt32(other.partitionColIdxAry_, other.partitionColCount_, h)),
      subpartitionColIdxAry_(allocateAndCopyInt32(other.subpartitionColIdxAry_, other.subpartitionColCount_, h)),
...
NABoolean NATable::operator==(const NATable &other) const {
  ...
  if (initialSize_ != other.initialSize_ || sizeAfterLastStatement_ != other.sizeAfterLastStatement_ ||
      referenceCount_ != other.referenceCount_ ||
      refsIncompatibleDP2Halloween_ != other.refsIncompatibleDP2Halloween_ ||
      isHalloweenTable_ != other.isHalloweenTable_ || flags_ != other.flags_ || flags2_ != other.flags2_ ||
      !(qualifiedName_ == other.qualifiedName_) || !(synonymReferenceName_ == other.synonymReferenceName_) ||
      isSynonymTranslationDone_ != other.isSynonymTranslationDone_ ||
      !(synonymReferenceObjectUid_ == other.synonymReferenceObjectUid_))
    return FALSE;

  if (!(fileSetName_ == other.fileSetName_) || !COMPARE_PTRS(prototype_, other.prototype_) ||
      colcount_ != other.colcount_ || !(colArray_ == other.colArray_) || !(hiveColArray_ == other.hiveColArray_) ||
      !(interestingExpressions_ == other.interestingExpressions_))
    return FALSE;

  if (recordLength_ != other.recordLength_ || !COMPARE_PTRS(clusteringIndex_, other.clusteringIndex_) ||
      !(indexes_ == other.indexes_) || !(vertParts_ == other.vertParts_) || !COMPARE_PTRS(colStats_, other.colStats_) ||
      statsFetched_ != other.statsFetched_ || originalCardinality_ != other.originalCardinality_ ||
      createTime_ != other.createTime_ || redefTime_ != other.redefTime_ || statsTime_ != other.statsTime_ ||
      catalogUID_ != other.catalogUID_ || schemaUID_ != other.schemaUID_ || objectUID_ != other.objectUID_ ||
      baseTableUID_ != other.baseTableUID_ || owner_ != other.owner_ || objectType_ != other.objectType_ ||
      schemaOwner_ != other.schemaOwner_ || partitioningScheme_ != other.partitioningScheme_ ||
      !COMPARE_CHAR_PTRS(viewFileName_, other.viewFileName_))
    return FALSE;

  if (!COMPARE_CHAR_PTRS(viewText_, other.viewText_) || viewTextInNAWchars_ != other.viewTextInNAWchars_ ||
      viewTextCharSet_ != other.viewTextCharSet_ || !COMPARE_CHAR_PTRS(viewCheck_, other.viewCheck_) ||
      !COMPARE_PTRS(viewColUsages_, other.viewColUsages_) || xnRepl_ != other.xnRepl_ ||
      storageType_ != other.storageType_ || incrBackupEnabled_ != other.incrBackupEnabled_ ||
      readOnlyEnabled_ != other.readOnlyEnabled_ ||
  ...
  if (accessedInCurrentStatement_ != other.accessedInCurrentStatement_ ||
      setupForStatement_ != other.setupForStatement_ || resetAfterStatement_ != other.resetAfterStatement_ ||
      tableConstructionHadWarnings_ != other.tableConstructionHadWarnings_ ||
      isAnMPTableWithAnsiName_ != other.isAnMPTableWithAnsiName_ || isUMDTable_ != other.isUMDTable_ ||
      isSMDTable_ != other.isSMDTable_ || isMVUMDTable_ != other.isMVUMDTable_ ||
      isTrigTempTable_ != other.isTrigTempTable_ || isExceptionTable_ != other.isExceptionTable_ ||
      insertMode_ != other.insertMode_ || !(checkConstraints_ == other.checkConstraints_) ||
  ...
  if (!COMPARE_VOID_PTRS(rcb_, rcbLen_, other.rcb_, other.rcbLen_) || keyLength_ != other.keyLength_ ||
      ...
      !COMPARE_PTRS(colsWithMissingStats_, other.colsWithMissingStats_) || !(tableIdList_ == other.tableIdList_) ||
      !COMPARE_PTRS(sgAttributes_, other.sgAttributes_))
    return FALSE;

  if (isHive_ != other.isHive_ || isHbase_ != other.isHbase_ || isORC_ != other.isORC_ ||
      isParquet_ != other.isParquet_ || isAvro_ != other.isAvro_ || isHbaseCell_ != other.isHbaseCell_ ||
      isHbaseRow_ != other.isHbaseRow_ || isSeabase_ != other.isSeabase_ || isSeabaseMD_ != other.isSeabaseMD_ ||
      isSeabasePrivSchemaTable_ != other.isSeabasePrivSchemaTable_ ||
      isUserUpdatableSeabaseMD_ != other.isUserUpdatableSeabaseMD_ ||
      resetHDFSStatsAfterStmt_ != other.resetHDFSStatsAfterStmt_ ||
      hiveDefaultStringLen_ != other.hiveDefaultStringLen_ || hiveTableId_ != other.hiveTableId_)
    return FALSE;

  if (!COMPARE_PTRS(privDescs_, other.privDescs_) || !COMPARE_PTRS(privInfo_, other.privInfo_) ||  // PrivMgrUserPrivs
      !(secKeySet_ == other.secKeySet_) ||  // ComSecurityKeySet (aka NASet())
      !(newColumns_ == other.newColumns_) || (defaultUserColFam_ != other.defaultUserColFam_) ||
      (defaultTrafColFam_ != other.defaultTrafColFam_) || !(allColFams_ == other.allColFams_) ||
      (tableNamespace_ != other.tableNamespace_) || !(minmaxCache_ == other.minmaxCache_) ||
      !(encryptionInfo_ == other.encryptionInfo_) ||
...
// and which to remove.
//
StatsList *NATable::getStatistics(NABoolean useStoredStats) {
#ifndef NDEBUG
  // this code figures out if our table is a table of interest (that is, not
  // a metadata table or histograms table)
  ...
  if (!statsFetched_) {
    ...
    if (isAnMV() || isUMDTable() || isSMDTable() || isMVUMDTable() || isTrigTempTable() || isReposTable || isXdcTable)
    ...
    for (i = 0; i < (*colStats_).entries(); /* no automatic increment */) {
      ...
      // handle them
      if (((*colStats_)[i]->getStatColumns().entries() != 1) && (!(*colStats_)[i]->isMCforHbasePartitioning())) {
    ...
    for (i = 0; i < (*colStats_).entries(); i++) {
      ...
      if (hist->entries() < 2) {
        if (stats->getMinValue() > stats->getMaxValue()) {
          *CmpCommon::diags() << DgSqlCode(CATALOG_HISTOGRM_HISTINTS_TABLES_CONTAIN_BAD_VALUE) << DgString0("")
      ...
      for (j = 1; j < hist->entries() - 1; /* no automatic increment */) {
        ...
        if ((*hist)[j].getBoundary() > (*hist)[j + 1].getBoundary()) {
          *CmpCommon::diags() << DgSqlCode(CATALOG_HISTINTS_TABLES_CONTAIN_BAD_VALUES) << DgInt0(j) << DgInt1(j + 1)
                              << DgString1(stats->getStatColumns()[0]->getFullColRefNameAsAnsiString().data());
        ...
        if ((*hist)[j].getBoundary() == (*hist)[j + 1].getBoundary()) {
          ...
          if (CmpCommon::getDefault(COMP_BOOL_79) == DF_ON) {
            ...
            if ((*hist)[j + 1].getUec() > 1) mergeIntervals = TRUE;
          }
          ...
          if (mergeIntervals) {
            ...
            //   12345.00    <               2
            //   12345.00    <=              1
            CostScalar combinedRows = (*hist)[j].getCardinality() + (*hist)[j + 1].getCardinality();
            CostScalar combinedUec = (*hist)[j].getUec() + (*hist)[j + 1].getUec();
      ...
      if ((*hist)[0].getBoundary() > (*hist)[1].getBoundary()) {
        *CmpCommon::diags() << DgSqlCode(CATALOG_HISTINTS_TABLES_CONTAIN_BAD_VALUES) << DgInt0(0) << DgInt1(1)
                            << DgString1(stats->getStatColumns()[0]->getFullColRefNameAsAnsiString().data());
      ...
      if (hist->entries() > 1 &&  // avoid the exception! might just be a single NULL
          ...
          (*hist)[0].getBoundary() == (*hist)[1].getBoundary() && (*hist)[1].getUec() > 1) {
        const double KLUDGE_VALUE = 0.0001;
        const double oldVal = (*hist)[0].getBoundary().getDblValue();
        const EncodedValue newVal = EncodedValue(oldVal - (_ABSOLUTE_VALUE_(oldVal) * KLUDGE_VALUE));  // kludge alert!
      ...
      if ((*colStats_)[i]->getRowcount() == (*colStats_)[i]->getTotalUec()) (*colStats_)[i]->setAlmostUnique(TRUE);
...
StatsList *NATable::generateFakeStats() {
  if (colStats_ == NULL) {
    // trying to get statistics for a new statement allocate colStats_
    colStats_ = new (CmpCommon::statementHeap()) StatsList(CmpCommon::statementHeap());
  }

  if (colStats_->entries() > 0) return (colStats_);
  ...
  for (CollIndex i = 0; i < colList.entries(); i++) {
    ...
    newSecondHistInt.setCardAndUec(defaultFakeRowCount, defaultFakeUec);
    ...
    ColStatsSharedPtr fakeColStats(new (HISTHEAP)
                                       ColStats(histid, defaultFakeUec, defaultFakeRowCount, defaultFakeRowCount,
                                                col->isUnique(), FALSE, emptyHist, FALSE, 1.0, 1.0,
                                                -1,  // avg varchar size
                                                HISTHEAP));
...
NABoolean NATable::verifyMvIsInitializedAndAvailable(BindWA *bindWA) const {
  ...
  if (bitmap.getIsMvUnAvailable()) {
    ...
    *CmpCommon::diags() << DgSqlCode(-12312) << DgTableName(getTableName().getQualifiedNameAsString());
    bindWA->setErrStatus();
  ...
  if (bitmap.getIsMvUnInitialized()) {
    ...
    NAString fileName(getClusteringIndex()->getFileSetName().getQualifiedNameAsString(), bindWA->wHeap());

    NAString ansiName(getTableName().getQualifiedNameAsAnsiString(), bindWA->wHeap());

    // get physical and ansi name
    bindWA->addUninitializedMv(convertNAString(fileName, bindWA->wHeap()), convertNAString(ansiName, bindWA->wHeap()));
  }
...
NABoolean NATable::getCorrespondingIndex(NAList<NAString> &inputCols, NABoolean lookForExplicitIndex,
                                         NABoolean lookForUniqueIndex, NABoolean lookForPrimaryKey,
                                         NABoolean lookForAnyIndexOrPkey, NABoolean lookForSameSequenceOfCols,
                                         NABoolean excludeAlwaysComputedSystemCols, NAString *indexName) {
  ...
  for (int i = 0; (NOT indexFound && (i < indexList.entries())); i++) {
    ...
    if ((NOT lookForPrimaryKey) && (isPrimaryKey)) continue;
    ...
    if (found) {
      ...
      {
        if ((naf->isCreatedExplicitly()) || (isPrimaryKey))
          found = TRUE;
        else
          found = FALSE;
      }
    ...
    if (numInputCols > 0) {
      ...
      for (int j = 0; j < nacArr.entries() && allColsMatched; j++) {
        ...
        if (!isPrimaryKey && nac->getIndexColName() == nac->getColName()) continue;

        if (excludeAlwaysComputedSystemCols && nac->isComputedColumnAlways() && nac->isSystemColumn()) continue;
...
NABoolean NATable::getCorrespondingConstraint(NAList<NAString> &inputCols, NABoolean uniqueConstr, NAString *constrName,
                                              NABoolean *isPkey, NAList<int> *reorderList) {
  NABoolean constrFound = FALSE;
  NABoolean lookForPrimaryKey = (inputCols.entries() == 0);

  const AbstractRIConstraintList &constrList = (uniqueConstr ? getUniqueConstraints() : getRefConstraints());
  ...
  for (int i = 0; (NOT constrFound && (i < constrList.entries())); i++) {
    ...
    if (lookForPrimaryKey && (NOT((UniqueConstraint *)ariConstr)->isPrimaryKeyConstraint())) continue;
      ...
      for (int j = 0; j < ariConstr->keyColumns().entries() && allColsMatched; j++) {
        ...
        for (int k = 0; !colFound && k < inputCols.entries(); k++)
          if (uniqueColName == inputCols[k]) {
            colFound = TRUE;
            numUniqueCols++;
            if (reorderList) reorderList->insert(k);
            if (j != k)
        ...
        if (!colFound) allColsMatched = FALSE;
      }

      if (inputCols.entries() == numUniqueCols && allColsMatched) {
        constrFound = TRUE;

        if (reorderList && !reorderNeeded) reorderList->clear();
      }
    } else {
    ...
    if (constrFound) {
      ...
      if (isPkey) {
        if ((uniqueConstr) && (((UniqueConstraint *)ariConstr)->isPrimaryKeyConstraint())) *isPkey = TRUE;
      }
    } else if (reorderList)
      reorderList->clear();
  }
...
void NATable::getPrivileges(TrafDesc *priv_desc, BindWA *bindWA) {
  ...
  if (CmpSeabaseDDL::isSeabasePrivMgrMD(qualifiedName_.getQualifiedNameObj().getCatalogName(),
                                        qualifiedName_.getQualifiedNameObj().getSchemaName()))
  ...
  if (CmpSeabaseDDL::isSeabaseReservedSchema(qualifiedName_.getQualifiedNameObj().getCatalogName(),
                                             qualifiedName_.getQualifiedNameObj().getSchemaName()))
    return;
  ...
  if (!CmpCommon::context()->isAuthorizationEnabled() || isVolatileTable() ||
      (ComUser::isRootUserID() && !gatherPrivsForRoot &&
       (!isHiveTable() && !isHbaseCellTable() && !isHbaseRowTable() && !isHbaseMapTable()))) {
  ...
  if (priv_desc == NULL) {
    ...
    if (!isSeabaseTable()) {
      ...
      readPrivileges();
      if (ctxCli) ctxCli->assignSqlParserFlags(savedParserFlags);
    } else {
      privInfo_ = NULL;
      return;
    }
    ...
    if (!isSchemaObject()) {
      CorrName cn(SEABASE_SCHEMA_OBJECTNAME, STMTHEAP, qualifiedName_.getQualifiedNameObj().getSchemaName(),
    ...
    if (retcode != 0) {
      if (CmpCommon::diags()->getNumber(DgSqlCode::ERROR_) == 0)
        *CmpCommon::diags() << DgSqlCode(-1078) << DgString0(ComUser::getCurrentUsername());
      return;
    }
    ...
    if (priv_grantees_desc) {
      ...
      if (schemaPrivs == NULL || objectPrivs == NULL) {
        *CmpCommon::diags() << DgSqlCode(1126) << DgString0(getTableName().getQualifiedNameAsString());
        return;
      }
    }
    ...
    privInfo_ = new (heap_) PrivMgrUserPrivs;
    privInfo_->initUserPrivs(roleIDs, privDescs_, currentUser, objectUID_.get_value(), &secKeySet_);
  ...
  if (!isSchemaObject()) {
    ...
    snprintf(msg, len, "NATable::getPrivileges (list of all privileges on object), user: %s obj %s, %s",
             ComUser::getCurrentUsername(), qualifiedName_.getExtendedQualifiedNameAsString().data(),
             privDetails.c_str());
...
void NATable::readPrivileges() {
  ...
  if (testError || (STATUS_GOOD != privInterface.getPrivileges((NATable *)this, ComUser::getCurrentUser(),
                                                               usingSentryUserApi, *privInfo_, &secKeySet_))) {
    if (testError)
#ifndef NDEBUG
      *CmpCommon::diags() << DgSqlCode(-8142) << DgString0("TEST_PRIV_INTERFACE_ERROR") << DgString1(tpie);
#else
      abort();
#endif
    NADELETE(privInfo_, PrivMgrUserPrivs, heap_);
...
bool NATable::isEnabledForDDLQI() const {
  if (isSeabaseMD_ || isSMDTable_ || (getSpecialType() == ExtendedQualName::VIRTUAL_TABLE))
    ...
    if (objectUID_.get_value() == 0) {
      ...
      if (!noAbendOnLp1398600 || *noAbendOnLp1398600 == '0') abort();
    }
    return true;
  }
}
...
NATable::~NATable() {
  ...
  if (!isHive_) {
    if (!shallowCopied_) {
      for (int i = 0; i < colcount_; i++) {
        ...
        if (delHeading) {
          if (col->getDefaultValue()) NADELETEBASIC(col->getDefaultValue(), heap_);
          if (col->getHeading()) NADELETEBASIC(col->getHeading(), heap_);
          if (col->getComputedColumnExprString()) NADELETEBASIC(col->getComputedColumnExprString(), heap_);
        }
  ...
  if (!shallowCopied_) {
    ...
    for (CollIndex i = 0; i < entryCount; i++) {
      if (indexes_[i]->getKeysDesc()) TrafDesc::deleteDescList(indexes_[i]->getKeysDesc(), heap_);
  ...
  if (columnsDesc_) TrafDesc::deleteDescList(columnsDesc_, heap_);

  if (storedStatsDesc_) TrafDesc::deleteDescList(storedStatsDesc_, heap_);
  ...
  if (shallowCopied_) {
    ...
    if (colsWithMissingStats_) colsWithMissingStats_->detachHashBuckets();
  }

  if (partitionColIdxAry_) NADELETEBASIC(partitionColIdxAry_, heap_);

  if (subpartitionColIdxAry_) NADELETEBASIC(subpartitionColIdxAry_, heap_);

  partArray_.deepDelete();
  partArray_.clear();

  if (partitionInterval_) NADELETEBASIC(partitionInterval_, heap_);

  if (subpartitionInterval_) NADELETEBASIC(subpartitionInterval_, heap_);
}

void NATable::resetAfterStatement()  // ## to be implemented?
{
  if (resetAfterStatement_) return;
  ...
  for (UInt32 i = 0; i < colArray_.entries(); i++) {
    // reset each NAColumn
    if (colArray_[i]) colArray_[i]->resetAfterStatement();
  }
...
void NATable::setupForStatement() {
  if (setupForStatement_) return;

  // reset the clustering index
  if (clusteringIndex_) clusteringIndex_->setupForStatement();

  // reset the fileset for indices
  for (UInt32 i = 0; i < indexes_.entries(); i++) {
    // reset the fileset for each index
    if (indexes_[i]) indexes_[i]->setupForStatement();
  }

  // reset the fileset for each vertical partition
  for (UInt32 j = 0; j < vertParts_.entries(); j++) {
    // reset the fileset for each index
    if (vertParts_[j]) vertParts_[j]->setupForStatement();
  }
...
static void formatPartitionNameString(const NAString &tbl, const NAString &pName, NAString &fmtOut) {
  fmtOut = NAString("(TABLE ") + tbl + ", PARTITION " + pName + ")";
}
static void formatPartitionNumberString(const NAString &tbl, int pNumber, NAString &fmtOut) {
  char buf[10];
  sprintf(buf, "%d", pNumber);

  fmtOut = NAString("(TABLE ") + tbl + ", PARTITION NUMBER " + buf + ")";
}

NABoolean NATable::filterUnusedPartitions(const PartitionClause &pClause) {
  if (pClause.isEmpty()) return TRUE;

  if (pClause.isPartitionV2()) return FALSE;

  if (getViewText()) {
    *CmpCommon::diags() << DgSqlCode(-1276) << DgString0(pClause.getPartitionName())
  ...
  if (pClause.partnRangeSpecified()) {
    ...
    */
    ...
    if ((pClause.getBeginPartitionNumber() == -1) ||
      ...
      oldPartFunc->setRestrictedBeginPartNumber(pClause.getBeginPartitionNumber());
    } else {
      formatPartitionNumberString(getTableName().getQualifiedNameAsAnsiString(), pClause.getBeginPartitionNumber(),
                                  errorString);
    }
    ...
    if ((pClause.getEndPartitionNumber() == -1) ||
      ...
      oldPartFunc->setRestrictedEndPartNumber(pClause.getEndPartitionNumber());
    } else {
      formatPartitionNumberString(getTableName().getQualifiedNameAsAnsiString(), pClause.getEndPartitionNumber(),
                                  errorString);
    }

    if (NOT errorString.isNull()) {
      // ERROR 1097 Unable to find specified partition...
      *CmpCommon::diags() << DgSqlCode(-1097) << DgString0(errorString)
                          << DgTableName(getTableName().getQualifiedNameAsAnsiString());
      return TRUE;
    ...
    {
      if ((pClause.getPartitionNumber() > 0) && (oldPartFunc->getCountOfPartitions() >= pClause.getPartitionNumber()))
        oldNodeMapEntry = oldNodeMap->getNodeMapEntry(pClause.getPartitionNumber() - 1);
      else {
        NAString errorString;
        formatPartitionNumberString(getTableName().getQualifiedNameAsAnsiString(), pClause.getPartitionNumber(),
                                    errorString);
    ...
    } else  // PARTITION NAME was specified
    {
      for (CollIndex i = 0; i < oldNodeMap->getNumEntries(); i++) {
        ...
        if (oldNodeMapEntry->getGivenName() == pClause.getPartitionName()) break;
        if (i == (oldNodeMap->getNumEntries() - 1))  // match not found
        {
          NAString errorString;
          formatPartitionNameString(getTableName().getQualifiedNameAsAnsiString(), pClause.getPartitionName(),
                                    errorString);
    ...
    if (!isHbaseTable()) {
      ...
      NodeMapEntry newEntry((char *)oldNodeMapEntry->getPartitionName(), (char *)oldNodeMapEntry->getGivenName(), heap_,
                            oldNodeMap->getTableIdent());
      ...
      baseTable->setHasRemotePartitions(checkRemote(NULL, (char *)oldNodeMapEntry->getPartitionName()));
      ...
      QualifiedName physicalName(oldNodeMapEntry->getPartitionName(), 1, heap_, NULL);
...
const LIST(CollIndex) & NATable::getTableIdList() const { return tableIdList_; }
...
CollIndex NATable::getUserColumnCount() const {
  ...
  for (CollIndex i = 0; i < colArray_.entries(); i++)
    if (colArray_[i]->isUserColumn()) result++;
...
NATableDB::NATableDB(NAMemory *h)
    : heap_(h),
      ...
      NAKeyLookup<ExtendedQualName, NATable>(NATableDB_INIT_SIZE, NAKeyLookupEnums::KEY_INSIDE_VALUE, h),
...
NATable *NATableDB::get(const ExtendedQualName *key, BindWA *bindWA, NABoolean findInCacheOnly) {
  ...
  NATable *cachedNATable = NAKeyLookup<ExtendedQualName, NATable>::get(key);

  // entry not found in cache
  if (!cachedNATable) return NULL;
  ...
  if ((cachedNATable->isHiveTable()) && (CmpCommon::getDefault(HIVE_DATA_MOD_CHECK) == DF_ON) &&
      (CmpCommon::getDefault(TRAF_RELOAD_NATABLE_CACHE) == DF_ON)) {
    removeEntry = TRUE;
  }

  // Found in cache.  If that's all the caller wanted, return now.
  if (!removeEntry && findInCacheOnly) return cachedNATable;
  ...
  if (!removeEntry && !cachedNATable->accessedInCurrentStatement()) {
    ...
    const long tableRedefTime = cachedNATable->getRedefTime();
    // Get last catalog operation time
    long labelCatalogOpTime = tableRedefTime;
    long currentSchemaRedefTS = 0;
    long cachedSchemaRedefTS = 0;
    ...
    // privilege info.
    if ((CmpCommon::statement() && CmpCommon::statement()->recompiling() &&
         !(CmpCommon::statement()->recompUseNATableCache()) && !(CmpCommon::statement()->sentryPrivRecheck())) ||
        (labelCatalogOpTime != tableRedefTime) || (error) || (currentSchemaRedefTS != cachedSchemaRedefTS) ||
        (!usingCache()) || (refreshCacheInThisStatement_)) {
  ...
  if (removeEntry) {
    ...
    if (cacheMetaData_) currentCacheSize_ = heap_->getAllocSize();
    ...
    if ((NOT cachedNATable->getExtendedQualName().isSpecialTable()) ||
        (cachedNATable->getExtendedQualName().getSpecialType() == ExtendedQualName::MV_TABLE) ||
        (cachedNATable->getExtendedQualName().getSpecialType() == ExtendedQualName::GHOST_MV_TABLE) ||
        (cachedNATable->getExtendedQualName().getSpecialType() == ExtendedQualName::GHOST_INDEX_TABLE) ||
        (cachedNATable->getExtendedQualName().getSpecialType() == ExtendedQualName::INDEX_TABLE))
...
void NATable::markColumnsForHistograms() {
  ...
  if (colArray_.getColumn("SYSKEY")) addSingleIntHist = TRUE;
  ...
  for (UInt32 i = 0; i < colArray_.entries(); i++) {
    ...
    NABoolean isAKeyColumn = (column->isIndexKey() OR column->isPrimaryKey() OR column->isPartitioningKey());
...
const QualifiedName &NATable::getFullyQualifiedGuardianName() {
  ...
  if (qualifiedName_.getQualifiedNameObj().getQualifiedNameAsString() != fileSetName_.getQualifiedNameAsString()) {
    fileName = new (CmpCommon::statementHeap()) QualifiedName(fileSetName_, CmpCommon::statementHeap());
  } else {
    fileName = new (CmpCommon::statementHeap())
        QualifiedName(qualifiedName_.getQualifiedNameObj(), CmpCommon::statementHeap());
  }
  return *fileName;
}

ExtendedQualName::SpecialTableType NATable::getTableType() { return qualifiedName_.getSpecialType(); }

NABoolean NATable::hasSaltedColumn(int *saltColPos) const {
  for (CollIndex i = 0; i < colArray_.entries(); i++) {
    if (colArray_[i]->isSaltColumn()) {
      if (saltColPos) *saltColPos = i;
      return TRUE;
    }
  }
  return FALSE;
...
NABoolean NATable::hasTrafReplicaColumn(int *repColPos) const {
  for (CollIndex i = 0; i < colArray_.entries(); i++) {
    if (colArray_[i]->isReplicaColumn()) {
      if (repColPos) *repColPos = i;
      return TRUE;
    }
  }
  return FALSE;
...
Int16 NATable::getNumTrafReplicas() const {
  if (isView() || getSpecialType() == ExtendedQualName::INDEX_TABLE)
    return 0;  // for updateable view we need a way to get to underlying clusteringindex
               // for index table columns default values are not currently used, so we don't have a way to get num
               // replicas this will be fixed.
...
/*
const NABoolean NATable::hasSaltedColumn(int * saltColPos) const
{
  return ((NATable*)this)->hasSaltedColumn(saltColPos);
}
*/

NABoolean NATable::hasDivisioningColumn(int *divColPos) {
  for (CollIndex i = 0; i < colArray_.entries(); i++) {
    if (colArray_[i]->isDivisioningColumn()) {
      if (divColPos) *divColPos = i;
      return TRUE;
    }
  }
  return FALSE;
...
ComBoolean NATable::usingSentry() {
  if (msg_license_advanced_enabled() || msg_license_multitenancy_enabled()) {
    ...
    if (sentryEnv && strcmp(sentryEnv, "TRUE") == 0) return TRUE;
  }
  return FALSE;
}
...
NABoolean NATable::usingLinuxGroups() {
  if (msg_license_advanced_enabled()) {
    ...
    if (sentryEnv && strcmp(sentryEnv, "SHELL") == 0) return TRUE;
  }
  return FALSE;
}
...
NABoolean NATable::usingLDAPGroups() {
  if (msg_license_advanced_enabled()) {
    static char *sentryEnv = getenv("SENTRY_SECURITY_GROUP_MODE");
    if (sentryEnv && strcmp(sentryEnv, "LDAP") == 0) return TRUE;
  }
  return FALSE;
}
...
int NATable::computeHBaseRowSizeFromMetaData() const {
  ...
  for (int colInx = 0; colInx < colcount_; colInx++) {
    ...
    ;
    int colLen = col->getType()->getNominalSize();  // data length
    int colPos = col->getPosition();                // position in table
...
long NATable::estimateHBaseRowCount(int retryLimitMilliSeconds, int &errorCode, int &breadCrumb) const {
  ...
  if (ehi) {
    HbaseStr fqTblName;
    NAString tblName = CmpSeabaseDDL::genHBaseObjName(getTableName().getCatalogName(), getTableName().getSchemaName(),
                                                      getTableName().getObjectName(), ((NATable *)this)->getNamespace(),
    ...
    if (!isAlignedFormat(NULL)) keyValueCountPerRow = colcount_;  // non-aligned; each column is a separate KV

    errorCode = ehi->estimateRowCount(fqTblName, partialRowSize, keyValueCountPerRow, retryLimitMilliSeconds,
    ...
    if (errorCode < 0) estRowCount = 100000000;
...
ExpHbaseInterface *NATable::getHBaseInterfaceRaw(ComStorageType storageType) {
  ...
  ExpHbaseInterface *ehi =
      ExpHbaseInterface::newInstance(STMTHEAP, connectParam1, connectParam2, storageType, replSync);
...
NABoolean NATable::getRegionsNodeName(int partns, ARRAY(const char *) & nodeNames) const {
  ...
  if (!ehi)
    ...
    if (corrName.isHbaseCell() || corrName.isHbaseRow())
      ...
      tblName = CmpSeabaseDDL::genHBaseObjName(getTableName().getCatalogName(), getTableName().getSchemaName(),
                                               getTableName().getObjectName(), ((NATable *)this)->getNamespace(),
                                               objDataUID_.get_value());
    ...
    int retcode = ehi->getRegionsNodeName(fqTblName, partns, nodeNames);
...
NABoolean NATable::getHbaseTableInfo(int &hbtIndexLevels, int &hbtBlockSize) const {
  ...
  if (!ehi)
    ...
    int retcode = ehi->getHbaseTableInfo(fqTblName, hbtIndexLevels, hbtBlockSize);
...
int NATable::getNumReplications() {
  ...
  if (isSeabaseMDTable()) {
    if (CmpCommon::getDefault(TRAF_METADATA_READ_REPLICA) == DF_OFF) return numReplications;
  } else {
    if (CmpCommon::getDefault(HBASE_READ_REPLICA) == DF_OFF) return numReplications;
  }
...
short NATable::updateExtTableAttrs(NATable *etTable) {
  ...
  for (CollIndex i = 0; i < colArray_.entries(); i++) {
    NAColumn *nac = new (heap_) NAColumn(*colArray_[i], heap_);
    ...
    if (nac->isUserColumn()) {
      ...
      lv_type->setHiveType(nac->getType()->getHiveType());
    }
  }
  ...
  for (CollIndex i = 0; i < myCols.entries(); i++) {
    if (myCols[i]->isHivePartColumn()) etFileset->markAsHivePartitioningColumn(i);
    if (i < numUserCols) colArray_[i]->setNATable(this);  // NATable ptr in NAColumn got
  ...
  for (CollIndex u = numUserCols; u < myCols.entries(); u++) CMPASSERT(myCols[u]->isHiveVirtualColumn());
...
void NATable::updateStoredStats(TrafDesc *tableDesc) {
  ...
  if (tableDesc && tableDesc->tableDesc() && tableDesc->tableDesc()->table_stats_desc &&
      tableDesc->tableDesc()->table_stats_desc->tableStatsDesc()) {
    storedStatsDesc_ = TrafDesc::copyDescList(tableDesc->tableDesc()->table_stats_desc->tableStatsDesc(), heap_);
  }
}
...
static NABoolean createNAPartitions(TrafDesc *partition_desc, int partCnt, LIST(NAString) & colnameAry,
  ...
  for (int i = 0; i < partCnt && part_desc_list != NULL; i++) {
    ...
    newPartition = new (heap) NAPartition(
        heap, part_desc->parentUid, part_desc->partitionUid, partName, entityName, part_desc->isSubPartition,
        part_desc->hasSubPartition, part_desc->partPosition, part_desc->isValid, part_desc->isReadonly,
        part_desc->isInMemory, part_desc->defTime, part_desc->flags, part_desc->subpartitionCnt);
    ...
    for (int k = 0; k < 2; k++) {
      ...
      for (int p = 0; p <= delPos.entries(); p++) {
        ...
        if (first) boundValueList.insert(boundValue);
        colPos++;
      }
      first = false;
    }
    ...
    for (int i = 0; i < 2; i++)
      if (bValue[i] != NULL) NADELETEBASIC(bValue[i], heap);
  }
...
void NATable::displayPartitonV2(FILE *ofd, const char *indent, const char *title, CollHeap *c, char *buf) {
  Space *space = (Space *)c;
  char mybuf[1000];
  ...
  snprintf(mybuf, 1000,
           "Partition Type : %s, Partition Column : %s, "
           "Subartition Type : %s, Subpartition Column : %s, "
           "Fisrt-Level Partition Count : %d\n",
           partitionSchemeToString((ComPartitioningSchemeV2)partitionType_), colName.data(),
           partitionSchemeToString((ComPartitioningSchemeV2)subpartitionType_), subColName.data(), stlPartitionCnt_);
                                ...
                                NATableEntryDetails &details)  // (OUT): cache entry's details
{
  ...
  if ((NumEnt == 0) || (NumEnt <= ii)) {
    ...
    if (details.hasPriv == 0) {
      if (object->privInfo_ && object->privInfo_->hasAnyPriv()) details.hasPriv = 1;
    }
...
NABoolean NATableDB::isHiveTable(CorrName &corrName) { return corrName.isHive(); }

NABoolean NATableDB::isSQUtiDisplayExplain(CorrName &corrName) {
  const char *tblName = corrName.getQualifiedNameObj().getObjectName();
  if (!strcmp(tblName, "EXE_UTIL_DISPLAY_EXPLAIN__")) return TRUE;

  if (!strcmp(tblName, "EXPLAIN__")) return TRUE;

  if (!strcmp(tblName, "HIVEMD__")) return TRUE;

  if (!strcmp(tblName, "DESCRIBE__")) return TRUE;

  if (!strcmp(tblName, "EXE_UTIL_EXPR__")) return TRUE;

  if (!strcmp(tblName, "STATISTICS__")) return TRUE;
...
NABoolean NATableDB::isSQInternalStoredProcedure(CorrName &corrName) {
  ...
  if (!strncmp(tblName, "SPTableOutQUERYCACHEENTRIES", strlen("SPTableOutQUERYCACHEENTRIES"))) return TRUE;

  if (!strncmp(tblName, "SPTableOutQUERYCACHEDELETE", strlen("SPTableOutQUERYCACHEDELETE"))) return TRUE;

  if (!strncmp(tblName, "SPTableOutQUERYCACHE", strlen("SPTableOutQUERYCACHE"))) return TRUE;

  if (!strncmp(tblName, "SPTableOutHYBRIDQUERYCACHEENTRIES", strlen("SPTableOutHYBRIDQUERYCACHEENTRIES"))) return TRUE;

  if (!strncmp(tblName, "SPTableOutHYBRIDQUERYCACHE", strlen("SPTableOutHYBRIDQUERYCACHE"))) return TRUE;

  return FALSE;
}

NABoolean NATableDB::isSQUmdTable(CorrName &corrName) { return FALSE; }
...
int accessObjectEpochCache(CorrName &corrName, NABoolean createIfNotExists, NABoolean writeReference,
  ...
  if (corrName.isSeabase() && !corrName.isSeabaseMD() && !corrName.isSeabasePrivMgrMD() && !corrName.isVolatile()) {
    ...
    ObjectEpochCacheEntryName oecName(STMTHEAP, externalName, COM_BASE_TABLE_OBJECT, 0);
      ...
      if (createIfNotExists) {
        ...
        if (retcode) {
          ...
          if (geeCode == ObjectEpochCache::OEC_ERROR_OK) {
            ...
            *CmpCommon::diags() << DgSqlCode(-4401) << DgString0(externalName.data()) << DgString1(buf);
            retcode = -4401;
          }
        }
    ...
    if ((retcode == 0) && (!writeReference) && ((currentFlags & ObjectEpochChangeRequest::READS_DISALLOWED) == 0))
      currentFlags = 0;  // read reference + reads allowed; just zero out the flags

    if ((retcode == 0) && (currentFlags & ObjectEpochChangeRequest::DDL_IN_PROGRESS) &&
...
NATable *NATableDB::get(CorrName &corrName, BindWA *bindWA, TrafDesc *inTableDescStruct, NABoolean writeReference) {
  ...
  if (!table && !inTableDescStruct && !corrName.isSeabaseMD() && !corrName.isSeabasePrivMgrMD()) {
    ...
    if (sharedDescCache && (CmpCommon::getDefault(TRAF_ENABLE_METADATA_LOAD_IN_SHARED_CACHE) == DF_ON)) {
      const ExtendedQualName &extQualName = corrName.getExtendedQualNameObj();

      const QualifiedName &qualName = extQualName.getQualifiedNameObj();
      ...
      if (CmpCommon::getDefault(TRAF_CHECK_SHARED_CACHE_LOCKED_AT_LOOKUP) == DF_ON) {
        ...
        long retryLimit = (ActiveSchemaDB()->getDefaults()).getAsLong(TRAF_CONCURRENT_DDL_WAIT_RETRIES);
      ...
      if (tableDescStr) {
        inTableDescStruct = CmpSeabaseDDL::decodeStoredDesc(*tableDescStr);
        ...
        cout << "Getting table desc for table " << corrName.getQualifiedNameObj().getQualifiedNameAsAnsiString()
             << " from shared cache and decode()" << endl;
#endif
      }
    bailout_label:
  ...
  if (NOT table || (NOT isMV && table->getSpecialType() != corrName.getSpecialType())) {
    // in open source, only the SEABASE catalog is allowed.
    // Return an error if some other catalog is being used.
    if ((NOT corrName.isHbase()) && (NOT corrName.isSeabase()) && (NOT corrName.isHive()) &&
        (corrName.getSpecialType() != ExtendedQualName::VIRTUAL_TABLE)) {
      *CmpCommon::diags() << DgSqlCode(-1002) << DgCatalogName(corrName.getQualifiedNameObj().getCatalogName())
                          << DgString0("");
    ...
    //
    if (NOT inTableDescStruct && corrName.isSpecialTable()) inTableDescStruct = generateSpecialDesc(corrName);
    ...
    // are special tables
    if (((NOT table) && cacheMetaData_ && useCache_) && corrName.isCacheable()) {
      naTableHeap = getHeap();
    ...
    if ((corrName.isHbase() || corrName.isSeabase()) && (!isSQUmdTable(corrName)) &&
        (!isSQUtiDisplayExplain(corrName)) && (!isSQInternalStoredProcedure(corrName))) {
      // ------------------------------------------------------------------
      ...
      if (isHbaseCell || isHbaseRow)  // explicit cell or row format specification
      {
        ...
        if (cmpSBD.existsInHbase(extHBaseName, NULL) != 1) {
          *CmpCommon::diags() << DgSqlCode(-1389) << DgString0(corrName.getQualifiedNameObj().getObjectName());
        ...
        tableDesc = HbaseAccess::createVirtualTableDesc(corrName.getExposedNameAsAnsiString(FALSE, TRUE).data(),
        ...
        if (CmpCommon::context()->getCIClass() == CmpContextInfo::CMPCONTEXT_TYPE_META &&
          ...
          masterNATable = CmpCommon::context()->getNATableFromFirstMetaCI(corrName.getExtendedQualNameObj());
        }
        ...
        if (masterNATable) {
          ...
          tableDesc = cmpSBD.getSeabaseTableDesc(corrName.getQualifiedNameObj().getCatalogName(),
                                                 corrName.getQualifiedNameObj().getSchemaName(),
                                                 corrName.getQualifiedNameObj().getObjectName(), COM_INDEX_OBJECT);
          objectType = COM_INDEX_OBJECT;
        } else if (corrName.isSpecialTable() && corrName.getSpecialType() == ExtendedQualName::SCHEMA_TABLE) {
          tableDesc = cmpSBD.getSeabaseTableDesc(
              corrName.getQualifiedNameObj().getCatalogName(), corrName.getQualifiedNameObj().getSchemaName(),
              corrName.getQualifiedNameObj().getObjectName(), COM_PRIVATE_SCHEMA_OBJECT);
          objectType = COM_PRIVATE_SCHEMA_OBJECT;
        } else {
          tableDesc = cmpSBD.getSeabaseTableDesc(corrName.getQualifiedNameObj().getCatalogName(),
                                                 corrName.getQualifiedNameObj().getSchemaName(),
                                                 corrName.getQualifiedNameObj().getObjectName(), COM_BASE_TABLE_OBJECT,
                                                 FALSE, bindWA->getBRLockedObjectsAccess());
          if (tableDesc) {
            if (cmpSBD.isUserUpdatableSeabaseMD(corrName.getQualifiedNameObj().getCatalogName(),
                                                corrName.getQualifiedNameObj().getSchemaName(),
        ...
        cout << "Creating NATable with tableDesc for MD table "
             << (corrName.getQualifiedNameObj()).getQualifiedNameAsAnsiString() << ", entries=" << entries()
             ...
             << ", USTATS=" << CmpContextInfo::CMPCONTEXT_TYPE_USTATS << endl;
      ...
      } else  // if (!corrName.isSeabaseMD()), or base tables
      {
        ...
        if (!inTableDescStruct)
          tableDesc = cmpSBD.getSeabaseTableDesc(
              corrName.getQualifiedNameObj().getCatalogName(), corrName.getQualifiedNameObj().getSchemaName(),
              corrName.getQualifiedNameObj().getObjectName(), objectType, FALSE, bindWA->getBRLockedObjectsAccess());
      ...
      if (masterNATable) {
        table = new (naTableHeap) NATable(*masterNATable, naTableHeap, masterNATable->getHeapType());
        ...
        if (inTableDescStruct) tableDesc = inTableDescStruct;
        ...
        if (tableDesc) {
          if (!found && (objectType == COM_BASE_TABLE_OBJECT)) {
            ...
            accessRetcode = accessObjectEpochCache(corrName, TRUE /* create if not found */, writeReference,
                                                   found /* out */, currentEpoch /* out */, currentFlags /* out */);
            // will ignore any errors here, using zero epoch values in that case
          }
          table = new (naTableHeap) NATable(bindWA, corrName, naTableHeap, tableDesc, currentEpoch, currentFlags);
      ...
      if (!table || inError) {
        if (isSeabase)
          *CmpCommon::diags() << DgSqlCode(-4082) << DgTableName(corrName.getExposedNameAsAnsiString());
        else
          *CmpCommon::diags() << DgSqlCode(-1389) << DgString0(corrName.getExposedNameAsAnsiString());
      ...
      // ------------------------------------------------------------------
      table = new (naTableHeap) NATable(bindWA, corrName, naTableHeap, inTableDescStruct, 0, 0);

    CMPASSERT(table);

    if (bindWA && bindWA->errStatus()) return NULL;
    ...
    if (NOT((table->getExtendedQualName().isSpecialTable()) &&
            ((table->getExtendedQualName().getSpecialType() == ExtendedQualName::SG_TABLE) ||
             (table->getExtendedQualName().getSpecialType() == ExtendedQualName::SCHEMA_TABLE))) &&
    ...
    if ((NOT table->getExtendedQualName().isSpecialTable()) ||
        (table->getExtendedQualName().getSpecialType() == ExtendedQualName::INDEX_TABLE) ||
        (table->getExtendedQualName().getSpecialType() == ExtendedQualName::MV_TABLE) ||
        (table->getExtendedQualName().getSpecialType() == ExtendedQualName::GHOST_MV_TABLE) ||
        (table->getExtendedQualName().getSpecialType() == ExtendedQualName::GHOST_INDEX_TABLE) ||
        (table->getExtendedQualName().getSpecialType() == ExtendedQualName::SCHEMA_TABLE))
    ...
    if (!tableInCache) {
      ...
      if ((useCache_ || isInPreloading()) && (corrName.isCacheable())) {
...
//          CMPASSERT(FALSE);
        ...
        ExtendedQualName *nonCacheableTableName = new (CmpCommon::contextHeap())
            ExtendedQualName(corrName.getExtendedQualNameObj(), CmpCommon::contextHeap());
...
TrafDesc *NATableDB::getTableDescFromCacheOrText(const ExtendedQualName &extQualName, long objectUid,
                                                 NAHeap **descHeap) {
  if (extQualName.getSpecialType() != ExtendedQualName::NORMAL_TABLE) return NULL;
  ...
  if (sharedDescCache && CmpCommon::getDefault(TRAF_ENABLE_METADATA_LOAD_IN_SHARED_CACHE) == DF_ON) {
    ...
    tableDescStr = sharedDescCache->find(qualName);
    if (tableDescStr) tableDesc = CmpSeabaseDDL::decodeStoredDesc(*tableDescStr);
  ...
  if (tableDesc) {
    if (descHeap) *descHeap = ActiveSchemaDB()->getNATableDB()->getHeap();
    return tableDesc;
  }

  // read from metadata
  CmpSeabaseDDL cmpSBD((NAHeap *)CmpCommon::statementHeap());
  if (cmpSBD.switchCompiler(CmpContextInfo::CMPCONTEXT_TYPE_META)) return NULL;
  ExeCliInterface cliInterface(STMTHEAP, 0, NULL, CmpCommon::context()->sqlSession()->getParentQid());
  short retcode = cmpSBD.checkAndGetStoredObjectDesc(&cliInterface, objectUid, &tableDesc, FALSE, getHeap());
  if (tableDesc && descHeap) *descHeap = (getHeap()) ? (getHeap()) : STMTHEAP;
  cmpSBD.switchBackCompiler();
...
void removeFromAllUsers(const QualifiedName &objName, ComQiScope qiScope, ComObjectType ot, NASet<long> &objectUIDs,
                        NABoolean ddlXns, NABoolean atCommit, NABoolean noCheck) {
  ...
  if (0 == objectUIDs.entries()) {
    long ouid = lookupObjectUidByName(objName, ot, FALSE);
  ...
  if ((ddlXns) && (NOT atCommit)) {
    for (CollIndex i = 0; i < numKeys; i++) {
      ...
      if (CmpCommon::transMode()->savepointInProgress()) ddlObj.setSvptId(CmpCommon::transMode()->getCurrSaveointId());
      ...
      NABoolean found = FALSE;
      for (int i = 0; ((NOT found) && (i < CmpCommon::context()->ddlObjsList().entries())); i++) {
        DDLObjInfo &ddlObjInList = CmpCommon::context()->ddlObjsList()[i];
        if (ddlObj.objName_ == ddlObjInList.objName_ && ddlObj.isDDLOp() == ddlObjInList.isDDLOp()) found = TRUE;
      }
  ...
  if (qiScope == REMOVE_FROM_ALL_USERS) {
    ...
    if (!noCheck && CmpCommon::getDefault(TRAF_OBJECT_LOCK) == DF_ON &&
        !Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL) && ot == COM_BASE_TABLE_OBJECT && ComIsUserTable(objName)) {
      ObjectLockPointer lock(tabName.data(), ot);
      if (!lock->ddlLockedByMe()) {
        LOGWARN(CAT_SQL_LOCK, "DDL lock released before sending invalidation message for %s", tabName.data());
    ...
    for (CollIndex i = 0; i < numKeys; i++) {
      LOGINFO(CAT_SQL_LOCK, "Adding QIKEY to invalidate %s %s (UID: %lu)", comObjectTypeName(ot), tabName.data(),
              objectUIDs[i]);
...
void NATableDB::removeNATable(CorrName &corrName, ComQiScope qiScope, ComObjectType ot, NABoolean ddlXns,
                              NABoolean atCommit, long objUID, NABoolean noCheck) {
  ...
  if (qualName.getSchemaName().index(bachupSch, bachupSch.length(), 0, NAString::exact) == 0)  // start with "_BACKUP_"
    return;
  ...
  while (key) {
    if (key->getQualifiedNameObj() == toRemove->getQualifiedNameObj()) {
      ...
      if (cachedTableList_.remove(cachedNATable) > 0) {
        ...
        // since we are deleting a caching entry
        if (cacheMetaData_) currentCacheSize_ = heap_->getAllocSize();
        if (cachedNATable->heap_ && cachedNATable->heap_ != CmpCommon::statementHeap())
          tablesToDeleteAfterStatement_.insert(cachedNATable);
      } else {
  ...
  if (NOT found && 0 < objUID) objectUIDs.insert(objUID);

  removeFromAllUsers(corrName.getQualifiedNameObj(), qiScope, ot, objectUIDs, ddlXns, atCommit, noCheck);
}
...
void NATableDB::resetAfterStatement() {
  ...
  cout << "<<< On ENTRY NATableDB::resetAfterStatement()"
       << ", this=" << static_cast<void *>(this) << ", entries=" << entries() << endl
       << endl;
  display();
  cout << ">>>" << endl;
#endif
  ...
  if (!cacheMetaData_) {
    ...
    for (i = 0; i < statementCachedTableList_.entries(); i++) {
      ...
      if (natable) {
        ...
        // statement follows.
        NABoolean mustRemove = (natable->isAnMV()) || (natable->isAnMVMetaData()) ||
                               (natable->constructionHadWarnings()) || (natable->getClearHDFSStatsAfterStmt());
  ...
  for (i = 0; i < tablesToDeleteAfterStatement_.entries(); i++) {
    ...
    if (naTable->getHeapType() == NATable::OTHER && !(naTable->isPreloaded())) {
      if (!distinctNATableSet.contains(naTable)) distinctNATableSet.insert(naTable);
    }
  }
...
NABoolean NATableDB::enforceMemorySpaceConstraints() {
  ...
  if (maxCacheSize() == 0 || heap_->getAllocSize() <= maxCacheSize()) return TRUE;
  ...
  while (heap_->getAllocSize() > maxCacheSize()) {
    ...
    if ((table) && (NOT(table->isSeabaseMDTable() || table->isPreloaded()))) {
    ...
    if (numLoops == NATABLE_MAX_REFCOUNT) return FALSE;
  }
...
void NATableDB::setCachingON() { cacheMetaData_ = TRUE; }

// Obtain a list of table identifiers for the current statement.
// Allocate the list on the heap passed.
const LIST(CollIndex) & NATableDB::getStmtTableIdList(NAMemory *heap) const {
...
void NATableDB::free_entries_with_QI_key(int numKeys, SQL_QIKEY *qiKeyArray) {
  ...
  while (currIndx < cachedTableList_.entries()) {
    ...
    if ((currTable->isSeabaseTable()) || (currTable->isHiveTable()) || (currTable->isSchemaObject()) ||
        (currTable->isHbaseCellTable()) || (currTable->isHbaseRowTable()) || (currTable->hasExternalTable()) ||
    ...
    if (qiCheckForInvalidObject(numKeys, qiKeyArray, currTable->objectUID_.get_value(), currTable->getSecKeySet())) {
      if (currTable->accessedInCurrentStatement_) statementCachedTableList_.remove(currTable);
      ...
      currIndx++;  // Increment if NOT found ... else currIndx already pointing at next entry!
  }
}
...
void NATableDB::update_entry_stored_stats_with_QI_key(int numKeys, SQL_QIKEY *qiKeyArray) {
  ...
  while (currIndx < cachedTableList_.entries()) {
    ...
    if (qiCheckForInvalidObject(numKeys, qiKeyArray, currTable->objectUID_.get_value(), currTable->getSecKeySet())) {
      const ExtendedQualName &qualName = currTable->getExtendedQualName();
      NAHeap *descHeap = NULL;
      if (currTable->isStoredDesc()) {
        TrafDesc *tableDesc = getTableDescFromCacheOrText(qualName, currTable->objectUID_.get_value(), &descHeap);
        if (tableDesc) {
...
void NATableDB::free_schema_entries() {
  UInt32 currIndx = 0;
...
#ifndef NDEBUG
      if (getenv("NATABLE_DEBUG"))
        cout << "Removing cached entry: "
             << currTable->qualifiedName_.getQualifiedNameObj().getQualifiedNameAsAnsiString() << endl;
#endif
      if (currTable->accessedInCurrentStatement_) statementCachedTableList_.remove(currTable);
      while (statementTableList_.remove(currTable))  // Remove as many times as on list!
      {
...
void NATableDB::free_entries_with_schemaUID(int numKeys, SQL_QIKEY *qiKeyArray) {
  ...
  while (currIndx < cachedTableList_.entries()) {
    ...
    if (qiCheckForSchemaUID(numKeys, qiKeyArray, currTable->schemaUID_.castToInt64())) {
#ifndef NDEBUG
      if (getenv("NATABLE_DEBUG"))
        cout << "Removing cached entry: "
             << currTable->qualifiedName_.getQualifiedNameObj().getQualifiedNameAsAnsiString() << endl;
...
void NATableDB::free_hive_tables() {
  UInt32 currIndx = 0;
...
void NATableDB::reset_priv_entries() {
  ...
  if (!ComUser::roleIDsCached()) {
    snprintf(msg, len,
             "NATableDB::reset_priv_entries, user: %s, retcode: %d,  "
  ...
  while (currIndx < cachedTableList_.entries()) {
    ...
    if (currTable->getPrivDescs()) {
      ...
      if (result) {
        ...
        snprintf(msg, len, "NATableDB::reset_priv_entries, user: %s obj %s, %s", ComUser::getCurrentUsername(),
                 currTable->qualifiedName_.getExtendedQualifiedNameAsString().data(), privDetails.c_str());
        QRLogger::log(CAT_SQL_EXE, LL_DEBUG, "%s", msg);
#ifndef NDEBUG
        if (getenv("NATABLE_DEBUG")) cout << msg << endl;
#endif
      }
      ...
      // assume no privileges, will be rechecked later.
        ...
        snprintf(msg, len, "NATableDB::reset_priv_entries, user: %s obj: %s, invalid security key",
                 ComUser::getCurrentUsername(), currTable->qualifiedName_.getExtendedQualifiedNameAsString().data());
        QRLogger::log(CAT_SQL_EXE, LL_INFO, "%s", msg);
...
//
void NATableDB::RemoveFromNATableCache(NATable *NATablep, UInt32 currIndx) {
  ...
  if (!InStatementHeap) delete NATablep;
  if (!InStatementHeap) currentCacheSize_ = heap_->getAllocSize();
}
...
//
void NATableDB::remove_entries_marked_for_removal() {
  ...
  while (currIndx < TableDB->cachedTableList_.entries()) {
    ...
    if (NATablep->isToBeRemovedFromCacheBNC())  // to be removed by CmpMain Before Next Comp. retry?
    {
      ...
      currIndx++;  // Note: No increment if the entry was removed !
  }
...
//
void NATableDB::unmark_entries_marked_for_removal() {
  NATableDB *TableDB = ActiveSchemaDB()->getNATableDB();
...
void NATableDB::display() {
  ...
  for (int i = 0; i < list.entries(); i++) {
    ...
    NAString tblName = table->getExtendedQualName().getQualifiedNameObj().getQualifiedNameAsString();
    cout << "table name=" << tblName.data() << ", isPreloaded=" << table->isPreloaded() << endl;
  }
}
