...
class OptimizerSimulator : public NABasicObject
{
  ...
  private:
    ...
    void buildHiveHistogramUnload(NAString& query, const QualifiedName* name, Int64 tableUID);
    void buildHiveHistogramIntervalUnload(NAString& query, const QualifiedName* name, Int64 tableUID);
    void buildHiveHistogramCreate(NAString& query, const QualifiedName * name);
