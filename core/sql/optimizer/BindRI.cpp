...
AbstractRIConstraint::~AbstractRIConstraint() {
  ...
  if (heap != NULL) {
    ...
    for (CollIndex i = 0; i < entryCount; i++) delete keyColumns_[i];
  }
  keyColumns_.clear();
}

NABoolean AbstractRIConstraint::constraintOverlapsUpdateCols(BindWA *bindWA, const ColSignature &updateCols) const {
  ...
  if (!bindWA->getCurrentScope()->context()->inUpdate()) return TRUE;
...
//
AbstractRIConstraint *AbstractRIConstraint::findConstraint(BindWA *bindWA,
  ...
  const AbstractRIConstraintList otherConstraints =
      (getOperatorType() == ITM_UNIQUE_CONSTRAINT) ? naTable->getRefConstraints() : naTable->getUniqueConstraints();
  ...
  *CmpCommon::diags() << DgSqlCode(-4353) << DgTableName(naTable->getTableName().getQualifiedNameAsAnsiString());
  bindWA->setErrStatus();
  return NULL;

}  // AbstractRIConstraint::findConstraint

void AbstractRIConstraint::setKeyColumns(const struct TrafConstrntsDesc *desc, CollHeap *heap) {
...
UniqueConstraint::~UniqueConstraint() {
  ...
  if (heap != NULL) {
    ...
    for (CollIndex i = 0; i < entryCount; i++) delete refConstraintsReferencingMe_[i];
  }
...
Int32 UniqueConstraint::getRefConstraints(BindWA *bindWA, const ColSignature &updateCols,
  ...
  for (CollIndex i = 0; i < constraintCnt; i++) {
    RefConstraint *rc = (RefConstraint *)findConstraint(bindWA, *refConstraintsReferencingMe_[i]);
    if (!rc) return 0;
    ...
    if (bindWA->shouldLogAccessViolations()) rc->uniqueConstraintReferencedByMe_.resetAfterStatement();

    rc->setOtherTableName();
    CMPASSERT((rc->getOtherTableName() == rc->getDefiningTableName()) ||
              (rc->getOtherTableName() == getDefiningTableName()));
...
void UniqueConstraint::setRefConstraintsReferencingMe(const struct TrafConstrntsDesc *desc, CollHeap *heap,
                                                      BindWA *bindWA) {
  ...
  while (referencingConstraintDesc) {
    char *refConstrntName = referencingConstraintDesc->refConstrntsDesc()->constrntname;
    char *refTableName = referencingConstraintDesc->refConstrntsDesc()->tablename;

    QualifiedName refConstrnt(refConstrntName, 3, heap, bindWA);
    QualifiedName refTable(refTableName, 3, heap, bindWA);

    constraintsReferencingMe = new (heap) ComplementaryRIConstraint(refConstrnt, refTable, heap);
...
void ComplementaryRIConstraint::resetAfterStatement() { keyColumns_ = NULL; }
...
void AbstractRIConstraintList::resetAfterStatement() {
  for (CollIndex i = 0; i < entries(); i++) {
    if (at(i)) at(i)->resetAfterStatement();
  }
}
...
void RefConstraint::getMatchOptionPredicateText(NAString &text, NAString *corrName) const {
  const KeyColumns keyCols = keyColumns();

  NAString tblText = ((corrName == NULL) ? getDefiningTableName().getQualifiedNameAsAnsiString() : *corrName);
  text += "(";
  for (CollIndex i = 0; i < keyCols.entries(); i++) {
    if (i) text += " OR ";
    text += tblText + "." + ToAnsiIdentifier(keyCols[i]->getColName()) + " IS NULL";
  }
  text += ")";
}
...
void RefConstraint::getPredicateText(NAString &text, const QualifiedName &tblName, const KeyColumns &keyColumns,
  ...
  for (CollIndex i = 0; i < keyColumns.entries(); i++) {
    if (keyColumns[i]->isSystemColumn()) continue;
    if (pos > 0) {
      text += ",";
    }
...
void RefConstraint::getPredicateText(NAString &text, NAString *corrName) const {
  ...
  if (isAReferencingConstraint) {
    ...
    text += "=";
    getPredicateText(text, uniqueConstraintReferencedByMe_.tableName_, *uniqueConstraintReferencedByMe_.keyColumns_);
  } else {
...
void RefConstraint::getOtherTableKeyColumns(BindWA *bindWA, LIST(int) & colPositions) const {
  ...
  if (!uniqueConstraintReferencedByMe_.keyColumns_) KeyColumnsToPositionsList(colPositions, uc->keyColumns_);
}
