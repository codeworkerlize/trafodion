...
void MvQueryRewriteHandler::createMvDescriptor(QueryAnalysis *qa, RelExpr *expr, NAString &warningMessage) {
  ...
  if (isMvqrSupportedForMV(qa, expr, warningMessage) == FALSE) return;
...
NABoolean MvQueryRewriteHandler::isMvqrSupportedForMV(QueryAnalysis *qa, RelExpr *expr, NAString &warningMessage) {
  ...
  if (expr->child(0)->getOperatorType() == REL_GROUPBY && expr->child(0)->child(0)->getOperatorType() == REL_GROUPBY) {
...
static QRRequestResult parseXML(char *xmlText, int xmlLen, XMLElementPtr &descriptor) {
  try {
    ...
    if (!descriptor) {
      QRLogger::log(CAT_SQL_COMP_QR_HANDLER, LL_MVQR_FAIL, "XMLDocument.parse() returned NULL.");
      return XMLParseError;
    } else
      ;  // debugMessage("Parsed XML document successfully.");
  } catch (XMLException &ex) {
    QRLogger::log(CAT_SQL_COMP_QR_HANDLER, LL_MVQR_FAIL, "An XMLException occurred: %s", ex.getMessage());
    return XMLParseError;
  } catch (QRDescriptorException &ex) {
    QRLogger::log(CAT_SQL_COMP_QR_HANDLER, LL_MVQR_FAIL, "A QRDescriptorException occurred: %s", ex.getMessage());
    return XMLParseError;
  } catch (...) {
    QRLogger::log(CAT_SQL_COMP_QR_HANDLER, LL_MVQR_FAIL, "An Unknown exception occurred");
...
RelExpr *MvQueryRewriteHandler::handleMvQueryRewrite(QueryAnalysis *qa, RelExpr *expr, NAString &warningMessage) {
  ...
  if (rootExpr->isAnalyzeOnly()) formatXml = true;
  ...
  if (!rewriteWorthTrying(rootExpr)) {
    ...
    if (rootExpr->isAnalyzeOnly()) rootExpr = handleAnalyzeOnlyQuery(rootExpr, warningMessage);
    return rootExpr;
  }
  ...
  if (rootExpr->isAnalyzeOnly() && CmpCommon::getDefault(MVQR_LOG_QUERY_DESCRIPTORS) == DF_DUMP_MV) {
  ...
  try {
    ...
    xmlText_ = (requestDescriptor ? descGenerator.createXmlText(requestDescriptor) : NULL);
  } catch (QRDescriptorException e) {
    // Just ignore it and leave xmlText_ as NULL to skip the rest of this method.
    QRLogger::log(CAT_SQL_COMP_QR_HANDLER, LL_MVQR_FAIL, "DescriptorException thrown: %s, %s.", e.getMessage(),
                  MVName.data());
    if (rootExpr->isAnalyzeOnly()) rootExpr = handleAnalyzeOnlyQuery(rootExpr, e.getMessage());
    warningMessage = e.getMessage();
    return rootExpr;
  } catch (...) {
    // This exception was not yet logged.
    // Log it and then skip.
    QRLogger::log(CAT_SQL_COMP_QR_HANDLER, LL_MVQR_FAIL, "Unknown exception thrown during descriptor generation, %s.",
                  MVName.data());
    if (rootExpr->isAnalyzeOnly()) rootExpr = handleAnalyzeOnlyQuery(rootExpr, "Unknown exception thrown");
    warningMessage = "Internal error";
    return rootExpr;
  }

  if (xmlText_ == NULL) {
    if (rootExpr->isAnalyzeOnly()) rootExpr = handleAnalyzeOnlyQuery(rootExpr, "No descriptor generated");
  } else {
    ...
    if (rootExpr->isAnalyzeOnly()) {
      ...
      try {
        ...
        if (publishDest != DF_NONE) {
          ...
          if (!qms) {
            QRLogger::log(CAT_SQL_COMP_QR_IPC, LL_MVQR_FAIL, "Match failed due to inability to connect to QMS.");
            if (rootExpr->isAnalyzeOnly()) rootExpr = handleAnalyzeOnlyQuery(rootExpr, "Can't connect to QMS");
          ...
          QRXmlMessageObj *xmlResponse = MvQueryRewriteServer::sendMatchMessage(qms, xmlText_, STMTHEAP);
          ...
          if (xmlResponse) {
            result = parseXML(xmlResponse->getData(), xmlResponse->getLength(), responseDescriptor);
          ...
          if (result == Success) {
            ...
            if (responseDescriptor->getElementType() != ET_ResultDescriptor) {
              ...
              QRResultDescriptorPtr resultDescriptor = static_cast<QRResultDescriptorPtr>(responseDescriptor);
        ...
        // This exception was not yet logged.
        // Log it and then skip.
        QRLogger::log(CAT_SQL_COMP_QR_HANDLER, LL_MVQR_FAIL, "Unknown exception thrown during descriptor generation.");
        delete xmlText_;
        if (rootExpr->isAnalyzeOnly()) rootExpr = handleAnalyzeOnlyQuery(rootExpr, "Unknown exception thrown");
        return rootExpr;
      }
...
RelRoot *MvQueryRewriteHandler::handleAnalyzeOnlyQuery(RelRoot *rootExpr, NAString *xmlText) {
  ...
  while (pos < maxLen - 1) {
    ...
    ItemExpr *tuple = new (heap) Convert(new (heap) SystemLiteral(line));
  ...
  ItemExpr *renExpr = new (heap) RenameCol(NULL, new (heap) ColRefName("Query Descriptor", heap));
  NAString tableName("Descriptor table");
  RelExpr *renameNode = new (heap) RenameTable(TRUE, newRoot, tableName, renExpr, heap);
