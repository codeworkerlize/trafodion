...
// ---------------------------------------------------------------------
const FuzzyPartitioningRequirement *PartitioningRequirement::castToFuzzyPartitioningRequirement() const { return NULL; }

const FullySpecifiedPartitioningRequirement *PartitioningRequirement::castToFullySpecifiedPartitioningRequirement()
    const {
  return NULL;
}

const RequireApproximatelyNPartitions *PartitioningRequirement::castToRequireApproximatelyNPartitions() const {
  return NULL;
}

const RequireExactlyOnePartition *PartitioningRequirement::castToRequireExactlyOnePartition() const { return NULL; }

const RequireReplicateViaBroadcast *PartitioningRequirement::castToRequireReplicateViaBroadcast() const { return NULL; }

const RequireReplicateNoBroadcast *PartitioningRequirement::castToRequireReplicateNoBroadcast() const { return NULL; }

const RequireHash *PartitioningRequirement::castToRequireHash() const { return NULL; }

const RequireHashDist *PartitioningRequirement::castToRequireHashDist() const { return NULL; }

const RequireHash2 *PartitioningRequirement::castToRequireHash2() const { return NULL; }

const RequireSkewed *PartitioningRequirement::castToRequireSkewed() const { return NULL; }

const RequireRange *PartitioningRequirement::castToRequireRange() const { return NULL; }

const RequireRoundRobin *PartitioningRequirement::castToRequireRoundRobin() const { return NULL; }

const RequireHive *PartitioningRequirement::castToRequireHive() const { return NULL; }
...
PartitioningFunction *PartitioningRequirement::realize(const Context *myContext,
...
const NAString PartitioningRequirement::getText() const {
  if (castToFullySpecifiedPartitioningRequirement()) {
    return castToFullySpecifiedPartitioningRequirement()->getPartitioningFunction()->getText();
  } else {
    ...
    if (NOT getPartitioningKey().isEmpty()) {
      if (result.length())
        ...
        result += "part. on (";
      ((ValueIdSet &)(getPartitioningKey())).unparse(result, DEFAULT_PHASE, EXPLAIN_FORMAT);
      result += ")";
    }
    return result;
  }
...
// -----------------------------------------------------------------------
PartitioningRequirement *PartitioningRequirement::copyAndRemap(ValueIdMap &map, NABoolean mapItUp) const {
  if (getPartitioningKey().entries() == 0) return (PartitioningRequirement *)this;
...
// -----------------------------------------------------------------------
void PartitioningRequirement::remapIt(const PartitioningRequirement *opr, ValueIdMap &map, NABoolean mapItUp) {
  ABORT("Redefine PartitioningRequirement::remapIt()");

}  // PartitioningRequirement::remapIt()

PartitioningRequirement *PartitioningRequirement::copy() const {
  ABORT("Redefine PartitioningRequirement::copy()");
  return NULL;
}

void PartitioningRequirement::print(FILE *ofd, const char *indent, const char *title) const {
...
NABoolean PartitioningRequirement::partReqAndFuncCompatible(const PartitioningFunction *other) const {
  ABORT("Redefine PartitioningRequirement::partReqAndFuncCompatible()");
  return FALSE;
}

COMPARE_RESULT PartitioningRequirement::comparePartReqToReq(const PartitioningRequirement *other) const {
...
// -----------------------------------------------------------------------
const FuzzyPartitioningRequirement *FuzzyPartitioningRequirement::castToFuzzyPartitioningRequirement() const {
  return this;
}

NABoolean FuzzyPartitioningRequirement::partReqAndFuncCompatible(const PartitioningFunction *other) const {
...
COMPARE_RESULT FuzzyPartitioningRequirement::comparePartReqToReq(const PartitioningRequirement *other) const {
...
// -----------------------------------------------------------------------
void FuzzyPartitioningRequirement::remapIt(const PartitioningRequirement *opr, ValueIdMap &map, NABoolean mapItUp) {
  // Clear because rewrite insists on it being so.
  partitioningKeyColumns_.clear();

  if (mapItUp)
    map.rewriteValueIdSetUp(partitioningKeyColumns_,
                            opr->castToFuzzyPartitioningRequirement()->partitioningKeyColumns_);
  else
    map.rewriteValueIdSetDown(opr->castToFuzzyPartitioningRequirement()->partitioningKeyColumns_,
                              partitioningKeyColumns_);

}  // FuzzyPartitioningRequirement::remapIt()

PartitioningRequirement *FuzzyPartitioningRequirement::copy() const {
  ABORT("Redefine FuzzyPartitioningRequirement::copy()");
  return NULL;
}

void FuzzyPartitioningRequirement::print(FILE *ofd, const char *indent, const char *title) const {
...
FullySpecifiedPartitioningRequirement::castToFullySpecifiedPartitioningRequirement() const {
  return this;
}
...
// -----------------------------------------------------------------------
NABoolean FullySpecifiedPartitioningRequirement::partReqAndFuncCompatible(const PartitioningFunction *other) const {
  ...
  if (other->isALogPhysPartitioningFunction() AND NOT myPartFunc->isALogPhysPartitioningFunction()) {
    other = other->castToLogPhysPartitioningFunction()->getPhysPartitioningFunction();
  }
...
// -----------------------------------------------------------------------
COMPARE_RESULT
FullySpecifiedPartitioningRequirement::comparePartReqToReq(const PartitioningRequirement *other) const {
...

  {
    ...
    if ((otherPartCount == ANY_NUMBER_OF_PARTITIONS)
            OR((myPartCount >= (otherPartCount - (otherPartCount * otherAllowedDeviation)))
                   AND(myPartCount <= otherPartCount)))
  ...
  else  // both are fully specified requirements
  {
    ...
    PartitioningFunction *otherPartFunc =
        other->castToFullySpecifiedPartitioningRequirement()->getPartitioningFunction();
...
PartitioningFunction *FullySpecifiedPartitioningRequirement::realize(const Context *myContext,
                                                                     NABoolean useContextPartitioningRequirements,
                                                                     const PartitioningRequirement *softRequirements) {
  // ---------------------------------------------------------------------
  ...
  // context.
  // ---------------------------------------------------------------------
  if (NOT useContextPartitioningRequirements OR myContext->getReqdPhysicalProperty()->getPartitioningRequirement() ==
      NULL OR myContext->getReqdPhysicalProperty()
          ->getPartitioningRequirement()
          ->castToFullySpecifiedPartitioningRequirement() ==
      this OR myContext->getReqdPhysicalProperty()->getPartitioningRequirement()->partReqAndFuncCompatible(
...
// -----------------------------------------------------------------------
void FullySpecifiedPartitioningRequirement::remapIt(const PartitioningRequirement *opr, ValueIdMap &map,
                                                    NABoolean mapItUp) {
  CMPASSERT(opr->castToFullySpecifiedPartitioningRequirement());  // I am pretty sure this will die every time
  partitioningFunction_ =
      opr->castToFullySpecifiedPartitioningRequirement()->partitioningFunction_->copyAndRemap(map, mapItUp);

}  // FullySpecifiedPartitioningRequirement::remapIt()

PartitioningRequirement *FullySpecifiedPartitioningRequirement::copy() const {
  ABORT("Redefine FullySpecifiedPartitioningRequirement::copy()");
  return NULL;
}

void FullySpecifiedPartitioningRequirement::print(FILE *ofd, const char *indent, const char *title) const {
  PartitioningRequirement::print(ofd, indent, title);
}
...
RequireApproximatelyNPartitions::RequireApproximatelyNPartitions(const ValueIdSet &partitioningKeyColumns,
  ...
  CMPASSERT(numOfPartsAllowedDeviation_ >= 0.0 AND numOfPartsAllowedDeviation_ <= 1.0);

  CMPASSERT(numberOfPartitions > 0 OR numberOfPartitions == ANY_NUMBER_OF_PARTITIONS);
}
...
// Method for performing a pointer type cast
// -----------------------------------------------------------------------
const RequireApproximatelyNPartitions *RequireApproximatelyNPartitions::castToRequireApproximatelyNPartitions() const {
  return this;
}
...
NABoolean RequireApproximatelyNPartitions::partReqAndFuncCompatible(const PartitioningFunction *other) const {
  ...
  if (getSkewProperty().isAnySkew()) {
    ...
    if ((NOT other->isASkewedDataPartitioningFunction())OR(
            NOT(getSkewProperty() == other->castToSkewedDataPartitioningFunction()->getSkewProperty())))
      return FALSE;
...
// -----------------------------------------------------------------------
COMPARE_RESULT
RequireApproximatelyNPartitions::comparePartReqToReq(const PartitioningRequirement *other) const {
  CMPASSERT(other != NULL);
  ...
  // with themselves.
  if (other->isRequirementReplicateViaBroadcast() OR other->isRequirementReplicateNoBroadcast())
    result = INCOMPATIBLE;
    ...
    // Compare the required number of partitions.
    if ((myPartCount == ANY_NUMBER_OF_PARTITIONS)
            OR((otherPartCount >= myLowerBound) AND(otherPartCount <= myPartCount)))
    ...
    if (other->isRequirementSkewed() AND NOT(getSkewProperty() == ((const RequireSkewed *)(other))->getSkewProperty()))
      result = combine_compare_results(result, INCOMPATIBLE);

    // If the fuzzy requirement requires hash2 and the part func
    // of the fully specified requirement is not hash2, then they
    // are INCOMPATIBLE.
    PartitioningFunction *opf = other->castToFullySpecifiedPartitioningRequirement()->getPartitioningFunction();
  ...
  } else  // other requirement is also a fuzzy requirement
  {
    ...
    if (getPartitioningRequirementType() !=
        other->castToFuzzyPartitioningRequirement()->getPartitioningRequirementType())
      result = INCOMPATIBLE;

    float otherAllowedDeviation = other->castToRequireApproximatelyNPartitions()->getAllowedDeviation();
    float otherLowerBound = otherPartCount - (otherPartCount * otherAllowedDeviation);

    if (myPartCount == otherPartCount) {
      if ((myPartCount == ANY_NUMBER_OF_PARTITIONS) OR(myAllowedDeviation == otherAllowedDeviation))
    ...
    {
      if (myPartKey.contains(otherPartKey) AND otherPartKey.contains(myPartKey))
        ...
        if (NOT myPartKey.isEmpty() OR(((myPartCount == ANY_NUMBER_OF_PARTITIONS) OR(myLowerBound <= 1))
                                           AND((otherPartCount == ANY_NUMBER_OF_PARTITIONS) OR(otherLowerBound <= 1))))
    ...
    if (NOT(getSkewProperty() == ((const FuzzyPartitioningRequirement *)(other))->getSkewProperty()))
      result = combine_compare_results(result, INCOMPATIBLE);

    // The other's requireHash2Only_ flag.
    NABoolean isOtherRequireHash2Only = other->castToRequireApproximatelyNPartitions()->isRequireHash2Only();

    // If requireHash2Only_ flag is same for both, then we require SAME.
    if (isRequireHash2Only() == isOtherRequireHash2Only) result = combine_compare_results(result, SAME);
...
NABoolean RequireApproximatelyNPartitions::isPartitionCountWithinRange(int numOfParts) const {
  ...
  if (numOfParts > reqPartCount OR numOfParts < lowerBound) {
    return FALSE;
  }
...
int RequireApproximatelyNPartitions::getCountOfPartitionsLowBound() const {
  ...
  if (numberOfPartitions > 0) {
    ...
    if (CURRSTMT_OPTDEFAULTS->isAdaptiveSegForHadoop()) {
      // the low bound has to be a factor of the numberOfPartitions that
      // is closest to numOfPartsLowBound computed above.
      numOfPartsLowBound =
...
PartitioningFunction *RequireApproximatelyNPartitions::realize(const Context *myContext,
                                                               NABoolean useContextPartitioningRequirements,
                                                               const PartitioningRequirement *softRequirements) {
  // ---------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------
  if (useContextPartitioningRequirements AND myContext->requiresPartitioning()) {
    ...
    rg.addPartRequirement(myContext->getReqdPhysicalProperty()->getPartitioningRequirement());
    ...
    if (rg.checkFeasibility()) {
      ...
      return rpp->getPartitioningRequirement()->realize(myContext, FALSE, softRequirements);
    } else {
  ...
  // ---------------------------------------------------------------------
  if (numOfParts < 1 AND softRequirements) numOfParts = softRequirements->getCountOfPartitions();

  if (numOfParts <= 1) return new (CmpCommon::statementHeap()) SinglePartitionPartitioningFunction();
  ...
  if (NOT partitioningKeyIsSpecified()) {
    ...
    if (softRequirements AND NOT softRequirements->getPartitioningKey().isEmpty()) {
  ...
  if (softRequirements AND softRequirements->castToFullySpecifiedPartitioningRequirement()
          AND((getSkewProperty().isAnySkew() AND partKey.contains(softRequirements->getPartitioningKey())) ||
              (!getSkewProperty().isAnySkew() AND partKey == softRequirements->getPartitioningKey()))) {
    ...
    NABoolean shouldTryScaling =
        (softPartCount != numOfParts) AND(CURRSTMT_OPTDEFAULTS->attemptESPParallelism() == DF_SYSTEM);
    ...
    if (isPartitionCountWithinRange(softPartCount) AND(!isRequireHash2Only() || spf->isAHash2PartitioningFunction() ||
                                                       spf->isASinglePartitionPartitioningFunction())
            AND NOT shouldTryScaling AND((NOT baseNumPartsOnAP)OR(numActivePartitions == softPartCount))) {
      ...
      if (NOT getSkewProperty().isAnySkew()) {
        if (spf->canHandleSkew() AND !isRequireHash2Only()) {
          spf = new (STMTHEAP) SkewedDataPartitioningFunction(spf->copy(), getSkewProperty());
          return spf;
        }

        if (spf->isASinglePartitionPartitioningFunction()) return spf;
      } else {
        spf = spf->copy();
        return spf;
      }
    ...
    // partitions.
    if (baseNumPartsOnAP AND(sNumOfParts > numActivePartitions)) sNumOfParts = numActivePartitions;

    spf = spf->copy();
    spf = spf->scaleNumberOfPartitions(sNumOfParts);
    if (isPartitionCountWithinRange(sNumOfParts) AND(!isRequireHash2Only() || spf->isAHash2PartitioningFunction() ||
                                                     spf->isASinglePartitionPartitioningFunction())) {
      if (NOT getSkewProperty().isAnySkew()) {
        if (spf->canHandleSkew() AND !isRequireHash2Only()) {
          spf = new (STMTHEAP) SkewedDataPartitioningFunction(spf, getSkewProperty());
          return spf;
        }
  ...
  if (partKey.isEmpty() OR numOfParts == 1 OR numOfParts == ANY_NUMBER_OF_PARTITIONS)
    return new (CmpCommon::statementHeap()) SinglePartitionPartitioningFunction();
  ...
  if (NOT(getSkewProperty().isAnySkew()) AND !isRequireHash2Only()) {
    ...
    newPartFunc = new (CmpCommon::statementHeap()) Hash2PartitioningFunction(partKey, partKey, numOfParts);

    newPartFunc = new (STMTHEAP) SkewedDataPartitioningFunction(newPartFunc, getSkewProperty());
  } else {
    // Check if requireHash2Only_ flag is set AND hash_type is 2 i.e. hash2.
    // If not, return NULL;
    if (isRequireHash2Only() AND hash_type != 2) return NULL;
    switch (hash_type) {
      case 0:
        newPartFunc = new (CmpCommon::statementHeap()) HashPartitioningFunction(partKey, partKey, numOfParts);
        break;

      case 1:
        newPartFunc = new (CmpCommon::statementHeap()) HashDistPartitioningFunction(partKey, partKey, numOfParts);
        break;
...
PartitioningRequirement *RequireApproximatelyNPartitions::copy() const {
  return new (CmpCommon::statementHeap()) RequireApproximatelyNPartitions(*this);
}

void RequireApproximatelyNPartitions::print(FILE *ofd, const char *indent, const char *title) const {
...
RequireExactlyOnePartition::RequireExactlyOnePartition(NABoolean withNodeMap)
    : FullySpecifiedPartitioningRequirement() {
  NodeMap *nodemap = !withNodeMap ? NULL
                                  : new (CmpCommon::statementHeap())
                                        NodeMap(CmpCommon::statementHeap(), 1, NodeMapEntry::ACTIVE);
  PartitioningFunction *partFunc = new (CmpCommon::statementHeap()) SinglePartitionPartitioningFunction(nodemap);
...
// Method for performing a pointer type cast
// -----------------------------------------------------------------------
const RequireExactlyOnePartition *RequireExactlyOnePartition::castToRequireExactlyOnePartition() const { return this; }

PartitioningRequirement *RequireExactlyOnePartition::copy() const {
  return new (CmpCommon::statementHeap()) RequireExactlyOnePartition(*this);
}

void RequireExactlyOnePartition::print(FILE *ofd, const char *indent, const char *title) const {
...
RequireReplicateViaBroadcast::RequireReplicateViaBroadcast(PartitioningFunction *childPF, NABoolean useChildsNodeMap)
    : FullySpecifiedPartitioningRequirement() {
  PartitioningFunction *partFunc = new (CmpCommon::statementHeap())
      ReplicateViaBroadcastPartitioningFunction(childPF->getCountOfPartitions(), childPF->getNodeMap()->copy());
...
// -----------------------------------------------------------------------
COMPARE_RESULT
RequireReplicateViaBroadcast::comparePartReqToReq(const PartitioningRequirement *other) const {
  if (NOT other->isRequirementFullySpecified()) return INCOMPATIBLE;

  PartitioningFunction *myPartFunc = getPartitioningFunction();
  PartitioningFunction *otherPartFunc = other->castToFullySpecifiedPartitioningRequirement()->getPartitioningFunction();
...
// Method for performing a pointer type cast
// -----------------------------------------------------------------------
const RequireReplicateViaBroadcast *RequireReplicateViaBroadcast::castToRequireReplicateViaBroadcast() const {
  return this;
}

PartitioningRequirement *RequireReplicateViaBroadcast::copy() const {
  return new (CmpCommon::statementHeap()) RequireReplicateViaBroadcast(*this);
}

void RequireReplicateViaBroadcast::print(FILE *ofd, const char *indent, const char *title) const {
  FullySpecifiedPartitioningRequirement::print(ofd, indent, title);
}
...
RequireReplicateNoBroadcast::RequireReplicateNoBroadcast(PartitioningFunction *parentPF, NABoolean useParentsNodeMap)
    : FullySpecifiedPartitioningRequirement(), parentPartFunc_(parentPF) {
  PartitioningFunction *partFunc;

  if (useParentsNodeMap) {
    partFunc = new (CmpCommon::statementHeap())
        ReplicateNoBroadcastPartitioningFunction(parentPF->getCountOfPartitions(), parentPF->getNodeMap()->copy());
  } else {
    partFunc =
        new (CmpCommon::statementHeap()) ReplicateNoBroadcastPartitioningFunction(parentPF->getCountOfPartitions());
  }
  setPartitioningFunction(partFunc);
}
...
// -----------------------------------------------------------------------
COMPARE_RESULT
RequireReplicateNoBroadcast::comparePartReqToReq(const PartitioningRequirement *other) const {
  if (NOT other->isRequirementFullySpecified()) return INCOMPATIBLE;
...
// Method for performing a pointer type cast
// -----------------------------------------------------------------------
const RequireReplicateNoBroadcast *RequireReplicateNoBroadcast::castToRequireReplicateNoBroadcast() const {
  return this;
}

PartitioningRequirement *RequireReplicateNoBroadcast::copy() const {
  return new (CmpCommon::statementHeap()) RequireReplicateNoBroadcast(*this);
}

void RequireReplicateNoBroadcast::print(FILE *ofd, const char *indent, const char *title) const {
  FullySpecifiedPartitioningRequirement::print(ofd, indent, title);
}
...
// Method for performing a pointer type cast
// -----------------------------------------------------------------------
const RequireHash *RequireHash::castToRequireHash() const { return this; }

PartitioningRequirement *RequireHash::copy() const { return new (CmpCommon::statementHeap()) RequireHash(*this); }

void RequireHash::print(FILE *ofd, const char *indent, const char *title) const {
  FullySpecifiedPartitioningRequirement::print(ofd, indent, title);
}
...
NABoolean RequireHashDist::partReqAndFuncCompatible(const PartitioningFunction *other) const {
  const HashDistPartitioningFunction *partFunc = getPartitioningFunction()->castToHashDistPartitioningFunction();

  CMPASSERT(partFunc);

  if (partFunc->comparePartFuncToFunc(*other) == SAME) return TRUE;
...
// Method for performing a pointer type cast
// -----------------------------------------------------------------------
const RequireHashDist *RequireHashDist::castToRequireHashDist() const { return this; }

PartitioningRequirement *RequireHashDist::copy() const {
  return new (CmpCommon::statementHeap()) RequireHashDist(*this);
}

void RequireHashDist::print(FILE *ofd, const char *indent, const char *title) const {
  FullySpecifiedPartitioningRequirement::print(ofd, indent, title);
}
...
NABoolean RequireHash2::partReqAndFuncCompatible(const PartitioningFunction *other) const {
  const Hash2PartitioningFunction *partFunc = getPartitioningFunction()->castToHash2PartitioningFunction();
...
// Method for performing a pointer type cast
// -----------------------------------------------------------------------
const RequireHash2 *RequireHash2::castToRequireHash2() const { return this; }

PartitioningRequirement *RequireHash2::copy() const { return new (CmpCommon::statementHeap()) RequireHash2(*this); }

void RequireHash2::print(FILE *ofd, const char *indent, const char *title) const {
  FullySpecifiedPartitioningRequirement::print(ofd, indent, title);
}
...
RequireSkewed::comparePartReqToReq(const PartitioningRequirement *other) const {
  COMPARE_RESULT result = FullySpecifiedPartitioningRequirement::comparePartReqToReq(other);

  if (other->isRequirementSkewed()) {
    if (NOT(getSkewProperty() == other->castToRequireSkewed()->getSkewProperty()))
      result = combine_compare_results(result, INCOMPATIBLE);
  } else {
    if (NOT(other->isRequirementFuzzy())
            OR(NOT(getSkewProperty() == other->castToFuzzyPartitioningRequirement()->getSkewProperty())))
      result = combine_compare_results(result, INCOMPATIBLE);
  }
...
const skewProperty &RequireSkewed::getSkewProperty() const {
  return getPartitioningFunction()->castToSkewedDataPartitioningFunction()->getSkewProperty();
}

NABoolean RequireSkewed::partReqAndFuncCompatible(const PartitioningFunction *other) const {
...
// Method for performing a pointer type cast
// -----------------------------------------------------------------------
const RequireSkewed *RequireSkewed::castToRequireSkewed() const { return this; }

PartitioningRequirement *RequireSkewed::copy() const { return new (CmpCommon::statementHeap()) RequireSkewed(*this); }

void RequireSkewed::print(FILE *ofd, const char *indent, const char *title) const {
  FullySpecifiedPartitioningRequirement::print(ofd, indent, title);
}
...
// Method for performing a pointer type cast
// -----------------------------------------------------------------------
const RequireRange *RequireRange::castToRequireRange() const { return this; }

PartitioningRequirement *RequireRange::copy() const { return new (CmpCommon::statementHeap()) RequireRange(*this); }

void RequireRange::print(FILE *ofd, const char *indent, const char *title) const {
  FullySpecifiedPartitioningRequirement::print(ofd, indent, title);
}
...
NABoolean RequireRoundRobin::partReqAndFuncCompatible(const PartitioningFunction *other) const {
  const RoundRobinPartitioningFunction *partFunc = getPartitioningFunction()->castToRoundRobinPartitioningFunction();
...
// Method for performing a pointer type cast
// -----------------------------------------------------------------------
const RequireRoundRobin *RequireRoundRobin::castToRequireRoundRobin() const { return this; }

PartitioningRequirement *RequireRoundRobin::copy() const {
  return new (CmpCommon::statementHeap()) RequireRoundRobin(*this);
}

void RequireRoundRobin::print(FILE *ofd, const char *indent, const char *title) const {
  FullySpecifiedPartitioningRequirement::print(ofd, indent, title);
}
...
COMPARE_RESULT LogicalPartitioningRequirement::compareLogPartRequirements(
  ...
  if (logPartType_ != other.logPartType_) return INCOMPATIBLE;

  if (numClients_ != other.numClients_) return INCOMPATIBLE;
...
NABoolean LogicalPartitioningRequirement::satisfied(const RelExpr *const physExpr,
                                                    const PhysicalProperty *const physProp) const {
  const PartitioningFunction *actualPartFunc = physProp->getPartitioningFunction();
  const LogPhysPartitioningFunction *lpf = actualPartFunc->castToLogPhysPartitioningFunction();
  ...
  if (lpf == NULL) {
    ...
    return ((logPartType_ == LogPhysPartitioningFunction::PA_PARTITION_GROUPING OR logPartType_ ==
             LogPhysPartitioningFunction::ANY_LOGICAL_PARTITIONING)
                AND(numClients_ == ANY_NUMBER_OF_PARTITIONS OR numClients_ == 1)
                    AND(logPartReq_ == NULL OR logPartReq_->partReqAndFuncCompatible(actualPartFunc)));
  }

  // if we required a specific type of logical partitioning, then check
  // for it here
  if (logPartType_ != LogPhysPartitioningFunction::ANY_LOGICAL_PARTITIONING AND lpf->getLogPartType() != logPartType_)
    return FALSE;

  // don't check # of clients, it is just a suggestion

  // check forced use of PAPA
  if (mustUsePapa_ AND NOT lpf->getUsePapa()) return FALSE;

  // check the logical partitioning requirement
  if (logPartReq_ AND NOT logPartReq_->partReqAndFuncCompatible(lpf->getLogPartitioningFunction())) return FALSE;

  return TRUE;
}

NABoolean FullySpecifiedPartitioningRequirement::isRequirementSkewBusterBroadcast() const {
  const SkewedDataPartitioningFunction *skpf = getPartitioningFunction()->castToSkewedDataPartitioningFunction();
