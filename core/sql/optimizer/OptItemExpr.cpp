...
// -----------------------------------------------------------------------
NABoolean ItemExpr::isAnEquiJoinPredicate(const GroupAttributes *const leftGroupAttr,
  ...
  if (NOT isAPredicate()) return FALSE;
  ...
  if (getOperatorType() == ITM_EQUAL) {
    ...
    if (CmpCommon::getDefault(COMP_BOOL_113) == DF_OFF) {
      ...
      ValueIdSet refs;
      if (leftGroupAttr->covers(leftChildValueId, inputs, refs)
              AND rightGroupAttr->covers(rightChildValueId, inputs, refs)) {
        isAnEquiJoin = TRUE;
      } else if (leftGroupAttr->covers(rightChildValueId, inputs, refs)
                     AND rightGroupAttr->covers(leftChildValueId, inputs, refs)) {
        isAnEquiJoin = TRUE;
    ...
    {
      if (leftGroupAttr->isCharacteristicOutput(leftChildValueId)
              AND rightGroupAttr->isCharacteristicOutput(rightChildValueId)) {
        isAnEquiJoin = TRUE;
      } else if (leftGroupAttr->isCharacteristicOutput(rightChildValueId)
                     AND rightGroupAttr->isCharacteristicOutput(leftChildValueId)) {
        isAnEquiJoin = TRUE;
        leftChildValueId = rightChildValueId;
        rightChildValueId = leftExpr->getValueId();
      }
    ...
    if (containsConstant) return FALSE;
    ...
    if (leftGroupAttr->isCharacteristicOutput(vegRefId)) {
      ...
      referencedOutputs.accumulateReferencedValues(leftGroupAttr->getCharacteristicOutputs(), /* IN */
                                                   VEGGroup,                                  /* IN */
                                                   doNotLookInsideVegReferences);             /* IN */
      if (referencedOutputs.entries() > 0) referencedOutputs.getFirst(leftChildValueId);
    }
    ...
    if (rightGroupAttr->isCharacteristicOutput(vegRefId)) {
      ...
      referencedOutputs.accumulateReferencedValues(rightGroupAttr->getCharacteristicOutputs(), /* IN */
                                                   VEGGroup,                                   /* IN */
                                                   doNotLookInsideVegReferences);              /* IN */
      if (referencedOutputs.entries() > 0) referencedOutputs.getFirst(rightChildValueId);
    }

    if (leftChildValueId AND rightChildValueId) isAnEquiJoin = TRUE;
...
NABoolean ItemExpr::isANestedJoinPredicate(const ValueIdSet &inputValues, const ValueIdSet &operatorValues) const {
  // This expression MUST be a binary predicate expression.
  ...
  switch (arity) {
    case 2: {
      ...
      if (!(leftExpr->doesExprEvaluateToConstant(FALSE))AND !(rightExpr->doesExprEvaluateToConstant(FALSE))) {
        ...
        if ((leftExpr->referencesOneValueFrom(iv) AND rightExpr->referencesOneValueFrom(ov))OR(
                rightExpr->referencesOneValueFrom(iv) AND leftExpr->referencesOneValueFrom(ov))) {
    ...
    case 0:
      if (getOperatorType() == ITM_VEG_PREDICATE) {
        ...
        if (NOT containsConstant) {
          ...
          iv.replaceVEGExpressionsAndCopy(inputValues);
          ov.replaceVEGExpressionsAndCopy(operatorValues);
          if (VEGGroupForPred.referencesOneValueFromTheSet(iv) AND VEGGroupForPred.referencesOneValueFromTheSet(ov)) {
            isANestedJoinPred = TRUE;
          }
        }  // endif is a constant
      }
...
NABoolean ItemExpr::isOrderPreserving() const { return TRUE; }
...
// and returned.
// -----------------------------------------------------------------------
NABoolean ItemExpr::applyDefaultPred(ColStatDescList &histograms, OperatorTypeEnum exprOpCode, ValueId predValueId,
                                     NABoolean &globalPredicate, CostScalar *maxSelectivity) {
...
//
// -----------------------------------------------------------------------
NABoolean ItemExpr::applyUnSuppDefaultPred(ColStatDescList &histograms, ValueId predValueId,
...
NABoolean ItemExpr::checkForStats(ColStatDescList &histograms, CollIndex &columnIndex, ValueIdSet &leafValues) {
...
// Default case
NABoolean ItemExpr::calculateMinMaxUecs(ColStatDescList &histograms, CostScalar &minUec, CostScalar &maxUec) {
...
NABoolean ItemExpr::calculateUecs(ColStatDescList &histograms, CostScalar &minUec, CostScalar &maxUec) {
...
void ItemExpr::resetRealBigNumFlag(ItemExpr *node) {
  ...
  if ((node->getOperatorType() == ITM_CAST) && (((Cast *)node)->getType()->getTypeQualifier() == NA_NUMERIC_TYPE)) {
    ...
    if (numType->isBigNum() && ((SQLBigNum *)numType)->isARealBigNum()) {
      ((SQLBigNum *)numType)->resetRealBigNum();
    }
  }
}

NABoolean VEGReference::calculateMinMaxUecs(ColStatDescList &histograms, CostScalar &minUec, CostScalar &maxUec) {
  return calculateUecs(histograms, minUec, maxUec);
}

NABoolean BaseColumn::calculateMinMaxUecs(ColStatDescList &histograms, CostScalar &minUec, CostScalar &maxUec) {
  return calculateUecs(histograms, minUec, maxUec);
}
...
//
// -----------------------------------------------------------------------
NABoolean BiArith::calculateMinMaxUecs(ColStatDescList &histograms, CostScalar &minUec, CostScalar &maxUec) {
  ...
  if (origOpType() == ITM_DAYOFYEAR) {
    ...
    if (child(0)->checkForStats(histograms, colIndex, leafValues)) {
      ...
      double timeEncompassedInHistogram =
          (colStats->getMaxValue().getDblValue() - colStats->getMinValue().getDblValue());
      ...
      minUec = maxUec =
          (colStats->getTotalUec() * (timeEncompassedInHistogramAfterFunctionApplied / timeEncompassedInHistogram));
      return TRUE;
    }
...
NABoolean UnLogic::applyDefaultPred(ColStatDescList &histograms, OperatorTypeEnum exprOpCode, ValueId predValueId,
                                    NABoolean &globalPredicate, CostScalar *maxSelectivity) {
  ...
  if (statsExist) {
    if (op == ITM_IS_NULL OR op == ITM_IS_NOT_NULL OR op == ITM_IS_UNKNOWN OR op == ITM_IS_NOT_UNKNOWN) {
      globalPredicate = FALSE;  // not a 'global' predicate
      ColStatDescSharedPtr statDesc = (histograms)[leftColIndex];

      if (NOT(statDesc->isPredicateApplied(predValueId))) {  // first time for this histogram
...
NABoolean BiLogic::isOrderPreserving() const { return FALSE; }

NABoolean BiLogic::synthSupportedOp() const { return TRUE; }

NABoolean BiLogic::applyDefaultPred(ColStatDescList &histograms, OperatorTypeEnum exprOpCode, ValueId predValueId,
                                    NABoolean &globalPredicate, CostScalar *maxSelectivity) {
  // maxSelectivity computation is done
  if (maxSelectivity) return TRUE;
...
NABoolean BiRelat::synthSupportedOp() const { return TRUE; }

NABoolean BiRelat::applyDefaultPred(ColStatDescList &histograms, OperatorTypeEnum exprOpCode, ValueId predValueId,
                                    NABoolean &globalPredicate, CostScalar *maxSelectivity) {
  ...
  if ((op == ITM_EQUAL) && (CmpCommon::getDefault(COMP_BOOL_40) == DF_ON)) {
    ...
    if ((rightLeafValues.entries() + leftLeafValues.entries() > 1) && (rightLeafValues != leftLeafValues)) {
      ...
      if (statsExist) {
        if ((exprOpCode != REL_SCAN) || (CmpCommon::getDefault(COMP_BOOL_74) == DF_OFF)) equiJoinWithExpr = TRUE;
  ...
  if (statsExist) {
    ...
    if ((op == ITM_EQUAL) || (op == ITM_NOT_EQUAL)) {
      ...
      if (rhsStatsExist) rightStatDesc = (histograms)[rightColIndex];
      ...
      if (equiJoinWithExpr) {
        if (maxSelectivity == NULL) {
          ...
          if (isSimpleComplexPredInvolved()) {
            ...
            if (op == ITM_NOT_EQUAL) defaultSel = 1 - defaultSel.getValue();
        ...
        if (statsExist) {
          globalPredicate = FALSE;
          applyLocalPredExpr(leftStatDesc, rightStatDesc, exprOpCode, leftLeafValues, rightLeafValues, maxSelectivity);
          if (maxSelectivity == NULL) leftStatDesc->addToAppliedPreds(predValueId);
        }
      }  // 1b: equal, not equal
    }
...
NABoolean BiRelat::isSimpleComplexPredInvolved() {
  ...
  if ((rightOp == ITM_MOD) || (leftOp == ITM_MOD)) return FALSE;
  ...
  if (leftOp == ITM_CASE || leftOp == ITM_IF_THEN_ELSE) {
    tempPred = lhs;
    if (leftOp == ITM_CASE) tempPred = tempPred->child(0);
    caseStmtInvlvd = TRUE;
  } else if (rightOp == ITM_CASE || rightOp == ITM_IF_THEN_ELSE) {
    tempPred = rhs;
    if (rightOp == ITM_CASE) tempPred = tempPred->child(0);
    caseStmtInvlvd = TRUE;
  }
  ...
  if (caseStmtInvlvd) {
    ...
    if (!flag && ((tempPred->child(2)->getOperatorType() == ITM_CONSTANT) ||
...
void BiRelat::applyEquiJoinExpr(ColStatDescSharedPtr &leftStatDesc, ColStatDescSharedPtr &rightStatDesc,
  ...
  if (CmpCommon::getDefault(COMP_BOOL_108) == DF_ON) {
    ...
    if (foundLeftUec || foundRightUec) {
      if (child(0)->containsAnAggregate()) leftUec = (leftUec * fudgeFactorForAggFn).minCsOne();

      if (child(1)->containsAnAggregate()) rightUec = (rightUec * fudgeFactorForAggFn).minCsOne();
    ...
    if (rightColStat && child(1)->containsAnAggregate()) rightUec = (rightUec * fudgeFactorForAggFn).minCsOne();
...
void BiRelat::applyLocalPredExpr(ColStatDescSharedPtr &leftStatDesc, ColStatDescSharedPtr &rightStatDesc,
  ...
  if ((rightLeafValues.isEmpty()) || ((rightLeafValues.entries() == 1) && (leftLeafValues == rightLeafValues))) {
    ...
    if (leftColStat->isFakeHistogram()) {
      defaultSel = (leftStatDesc->isSimilarPredicateApplied(op) ? csOne : this->defaultSel());
    } else {
      ...
      if (op == ITM_EQUAL) {
        ...
        if (maxSelectivity) {
          ...
          *maxSelectivity = MINOF(maxRowCount / rowCountBeforePred, *maxSelectivity);
        }
      } else {
...
void BiRelat::applyRangePredExpr(ColStatDescSharedPtr &leftStatDesc, ValueIdSet leftLeafValues,
                                 ValueIdSet rightLeafValues) {
  ItemExpr *lhs = child(0);
  ...
  if ((rightLeafValues.isEmpty()) || ((rightLeafValues.entries() == 1) && (leftLeafValues == rightLeafValues))) {
    ...
    if (defaultSel == csOne) {
      if (leftStatDesc->derivOfLikeAndSimilarPredApp(this) || leftStatDesc->isSimilarPredicateApplied(op))
        defaultSel = csOne;
      else {
...
NABoolean Case::calculateMinMaxUecs(ColStatDescList &histograms, CostScalar &minUec, CostScalar &maxUec) {
...
NABoolean IfThenElse::calculateMinMaxUecs(ColStatDescList &histograms, CostScalar &minUec, CostScalar &maxUec) {
  CostScalar firstMinUec, secondMinUec, firstMaxUec, secondMaxUec;
  ...
  if (firstOutcomeResult && secondOutcomeResult) {
    if ((child(1)->getOperatorType() == ITM_CONSTANT) || (child(2)->getOperatorType() == ITM_CONSTANT)) {
      // we have one column and one constant. Accurately estimating the uec
      // of a complex expression by adding the 1 that we get for the constant
      // to the uec of the column seems less important than getting uec
      // corrected with MC stats later. If we return FALSE here estimation
      // logic is GroupByAgg::synthEstLogProp() will handle that.
      if ((child(1)->getOperatorType() == ITM_VEG_REFERENCE) || (child(2)->getOperatorType() == ITM_VEG_REFERENCE))
        return FALSE;
...
NABoolean Substring::calculateMinMaxUecs(ColStatDescList &histograms, CostScalar &minUec, CostScalar &maxUec) {
  // Return if the function is not a SUBSTRING originally
  if (origOpType() != ITM_SUBSTR) return FALSE;
  ...
  if (leafValues.entries() != 1) return FALSE;
...
NABoolean MathFunc::calculateMinMaxUecs(ColStatDescList &histograms, CostScalar &minUec, CostScalar &maxUec) {
  if (getArity() == 1)
    return child(0)->calculateMinMaxUecs(histograms, minUec, maxUec);
  else
    return FALSE;
...
NABoolean Modulus::calculateMinMaxUecs(ColStatDescList &histograms, CostScalar &minUec, CostScalar &maxUec) {
...
NABoolean DayOfWeek::calculateMinMaxUecs(ColStatDescList &histograms, CostScalar &minUec, CostScalar &maxUec) {
  minUec = maxUec = 7;
  return TRUE;
}

NABoolean Extract::calculateMinMaxUecs(ColStatDescList &histograms, CostScalar &minUec, CostScalar &maxUec) {
  switch (getExtractField()) {
    ...
    case REC_DATE_YEARWEEK_EXTRACT: {
      ...
      if (colStats->isOrigFakeHist()) return FALSE;

      NAString typeName = colStats->getStatColumns()[0]->getType()->getTypeName();
      if (typeName != "DATE" && typeName != "TIMESTAMP") return FALSE;
...
NABoolean BuiltinFunction::calculateMinMaxUecs(ColStatDescList &histograms, CostScalar &minUec, CostScalar &maxUec) {
  switch (getOperatorType()) {
    case ITM_ASCII: {
      minUec = maxUec = 256;
      ...
      if ((getArity() == 1) && (child(0)->calculateMinMaxUecs(histograms, colMinUec, colMaxUec)) && (colMinUec < 256))
...
NABoolean ItmSequenceFunction::calculateMinMaxUecs(ColStatDescList &histograms, CostScalar &minUec,
                                                   CostScalar &maxUec) {
  minUec = maxUec = csOne;
  return TRUE;
}

NABoolean ConstValue::calculateMinMaxUecs(ColStatDescList &histograms, CostScalar &minUec, CostScalar &maxUec) {
  minUec = maxUec = csOne;
  return TRUE;
}
...
NABoolean VEGPredicate::synthSupportedOp() const { return TRUE; }

NABoolean VEGPredicate::applyDefaultPred(ColStatDescList &histograms, OperatorTypeEnum exprOpCode, ValueId predValueId,
                                         NABoolean &globalPredicate, CostScalar *maxSelectivity) {
  CostScalar defaultSel = csOne;
  NABoolean statsExist = FALSE;
  NABoolean alreadyApplied = FALSE;
  ...
  statsExist = histograms.identifyMergeCandidates(this, leftColIndex, statsToMerge);
