...
ItemExpr * BiRelat::getParentColIE()
{
  ...
  else
  {
    for(Int32 i = 0; i < arity; i++)
    { 
      if (((BiRelat*)getChild(i))->getParentColIE() != NULL)    
        return ((BiRelat*)getChild(i))->getParentColIE();
    }
  }
  return NULL;
}
ItemExpr * BiRelat::getChildColIE()
{
  ...
  else
  {
    for(Int32 i = 0; i < arity; i++)
    {
      if (((BiRelat*)getChild(i))->getChildColIE() != NULL)
        return ((BiRelat*)getChild(i))->getChildColIE();
    }
  }
  return NULL;
					  ...
					  CostScalar & maxUec)
{
  ...
  if(firstOutcomeResult && secondOutcomeResult)
  {
       ...
       (child(2)->getOperatorType() == ITM_CONSTANT))
    {
      ...
      if((child(1)->getOperatorType() == ITM_VEG_REFERENCE) || 
	 (child(2)->getOperatorType() == ITM_VEG_REFERENCE))
	return FALSE;
