...
//////////////////////////////////////////////////////////////////////////////
// Ctor for RECOMPUTE
Refresh::Refresh(const QualifiedName &mvName, ComBoolean noDelete, CollHeap *oHeap)
...
Refresh::Refresh(const QualifiedName &mvName, const DeltaDefinitionPtrList *pDeltaDefList,
                 NRowsClause *pOptionalNRowsClause, PipelineClause *pOptionalPipelineClause, CollHeap *oHeap)
    : BinderOnlyNode(REL_REFRESH, oHeap),
...
RelExpr *Refresh::bindNode(BindWA *bindWA) {
  ...
  if (deltaDefList_ != NULL) deltaDefList_->applyDefaultSchemaToAll(bindWA);
  if (pipelineClause_ != NULL) pipelineClause_->applyDefaultSchemaToAll(bindWA);

  CorrName mvCorrName(mvName_);
  MVInfoForDML *mvInfo = getMvInfo(bindWA, mvCorrName);
  if (mvInfo == NULL) return NULL;

  // User maintained MVs can only be recomputed (for initialization).
  if (mvInfo->getRefreshType() == COM_BY_USER && refreshType_ != RECOMPUTE) return NULL;

  if (verifyDeltaDefinition(bindWA, mvInfo)) return NULL;
  ...
  if (bindWA->errStatus() || refreshTree == NULL) return this;
...
RelExpr *Refresh::buildCompleteRefreshTree(BindWA *bindWA, MVInfoForDML *mvInfo) {
  ...
  if (refreshType_ == RECOMPUTE || mvInfo->getMVType() == COM_MJV) {
    delete firstBuilder;
    return topTree;
  }
  ...
  if (refreshType_ == SINGLEDELTA) {
    ...
    RelExpr *scanLogBlock = firstBuilder->buildLogsScanningBlock(*baseTableName);
    if (bindWA->errStatus() || scanLogBlock == NULL) return NULL;

    bindContext->setReplacementFor(baseTableName, scanLogBlock);
  }

  if (pipelineClause_ != NULL) topTree = constructPipelinedBuilders(bindWA, bindContext, topTree);
  if (topTree == NULL) return NULL;
...
//////////////////////////////////////////////////////////////////////////////
MvRefreshBuilder *Refresh::constructRefreshBuilder(BindWA *bindWA, MVInfoForDML *mvInfo) {
  CollHeap *heap = bindWA->wHeap();
  ...
  switch (refreshType_) {
    case RECOMPUTE: {
      treeBuilder = new (heap) MvRecomputeBuilder(mvCorrName, mvInfo, noDeleteOnRecompute_, bindWA);
      break;
    }
    ...
    case MULTIDELTA:
      ...
      switch (mvInfo->getMVType()) {
        case COM_MAV:
        case COM_MAJV: {
          treeBuilder = constructMavSpecifichBuilder(bindWA, mvCorrName, mvInfo);
          break;
        }

        case COM_MJV: {
          treeBuilder = constructMjvSpecifichBuilder(bindWA, mvCorrName, mvInfo);
          break;
        }
...
//////////////////////////////////////////////////////////////////////////////
MvRefreshBuilder *Refresh::constructMavSpecifichBuilder(BindWA *bindWA, CorrName mvCorrName, MVInfoForDML *mvInfo) {
  CollHeap *heap = bindWA->wHeap();
  MvRefreshBuilder *treeBuilder = NULL;
  ...
  if (deltaDefList_->entries() > 1) {
    ...
    treeBuilder = new (heap) MultiDeltaMavBuilder(mvCorrName, mvInfo, this, isProjectingMvDelta, bindWA);
    ...
    if (deltaDefList_->at(0)->useIudLog() && deltaDefList_->at(0)->useRangeLog()
      ...
      treeBuilder = new (heap) MultiTxnDEMavBuilder(mvCorrName, mvInfo, this, isProjectingMvDelta, bindWA);
#ifndef NDEBUG
      cout << endl << " Executing Internal refresh with Duplicate elimination " << endl;
#endif
    } else {
      treeBuilder = new (heap) MvMultiTxnMavBuilder(mvCorrName, mvInfo, this, isProjectingMvDelta, bindWA);
    }
  } else if (canUseMinMaxBuilder(bindWA, mvInfo)) {
    // We have a Min/Max MAV with a supporting index.
    treeBuilder = new (heap) MinMaxOptimizedMavBuilder(mvCorrName, mvInfo, this, isProjectingMvDelta, bindWA);
  } else {
    // Just a simple MAV.
    treeBuilder = new (heap) MavBuilder(mvCorrName, mvInfo, this, isProjectingMvDelta, bindWA);
  }
...
//////////////////////////////////////////////////////////////////////////////
MvRefreshBuilder *Refresh::constructMjvSpecifichBuilder(BindWA *bindWA, CorrName mvCorrName, MVInfoForDML *mvInfo) {
  CollHeap *heap = bindWA->wHeap();
  MvRefreshBuilder *treeBuilder = NULL;

  if (deltaDefList_->entries() > 1) {
    // We have more than one delta, we need a multi-delta builder.
    treeBuilder = new (heap) MjvOnRequestMultiDeltaBuilder(mvCorrName, mvInfo, this, bindWA);
  }
  //  else if (getNRowsClause() != NULL)
  //  {
  //    // We have a "COMMIT EACH" clause, need the multi-txn builder.
  ...
  //  }
    ...
    // Just a simple MJV.
    treeBuilder = new (heap) MjvOnRequestBuilder(mvCorrName, mvInfo, this, bindWA);
  }
...
//////////////////////////////////////////////////////////////////////////////
RelExpr *Refresh::constructPipelinedBuilders(BindWA *bindWA, MvBindContext *bindContext, RelExpr *firstRefreshTree) {
  ...
  while (currentMvName != NULL) {
    ...
    PipelinedMavBuilder *pipelinedBuilder =
        new (bindWA->wHeap()) PipelinedMavBuilder(mvCorrName, mvInfo, this, isPipelined, prevMvName, bindWA);

    RelExpr *pipelinedTree = pipelinedBuilder->buildAndConnectPipeliningRefresh(topTree);
...
NABoolean Refresh::canUseMinMaxBuilder(BindWA *bindWA, MVInfoForDML *mvInfo) {
  ...
  if (mvInfo->isMinMaxUsed() == FALSE) return FALSE;

  // Verify condition no. 2 - we are not supposed to get here otherwise.
  CMPASSERT(deltaDefList_->entries() == 1);

  // Check condition no. 3
  if (!mvInfo->isMvOnSingleTable()) return FALSE;

  // Check condition no. 4.
  if (!doesBaseTableHaveSupportingIndex(bindWA, mvInfo)) return FALSE;
...
// of the index columns).
//////////////////////////////////////////////////////////////////////////////
NABoolean Refresh::doesBaseTableHaveSupportingIndex(BindWA *bindWA, MVInfoForDML *mvInfo) const {
  CollIndex i;
  ...
  const MVColumns &pMvInfoColumnList = mvInfo->getMVColumns();
  ...
  // no supporting index.
  if (mvGroupByColumns.entries() == 0) return FALSE;
  ...
  const NAColumnArray &ciColumns = pClusteringIndexFileSet->getIndexKeyColumns();
  ...
  for (i = 0; i < indexFileSetList.entries(); i++) {
    ...
    const NAColumnArray &siColumns = pSecondaryIndexFileSet->getIndexKeyColumns();
...
NABoolean Refresh::areGroupByColsAnIndexPrefix(const NAColumnArray &groupByCols,
  ...
  if (groupByCols.entries() > columnArray.entries()) return FALSE;
...
MVInfoForDML *Refresh::getMvInfo(BindWA *bindWA, CorrName &mvToRefresh) const {
  ...
  if (!bitmap.getIsMvUnAvailable() && bitmap.getIsMvUnInitialized()) CMPASSERT(refreshType_ == RECOMPUTE);
...
NABoolean Refresh::verifyDeltaDefinition(BindWA *bindWA, MVInfoForDML *mvInfo) const {
  ...
  if (deltaDefs == NULL) return FALSE;
  ...
  for (CollIndex i = 0; i < deltaDefs->entries(); i++) {
    ...
    if (mvInfo->findUsedInfoForTable(*tableName) == NULL) {
      ...
      *CmpCommon::diags() << DgSqlCode(-12314) << DgTableName(tableName->getQualifiedNameAsString());
      bindWA->setErrStatus();
      return TRUE;
    }
  }
...
//////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////
const NAString Refresh::getText() const { return "refresh"; }
...
RelExpr *Refresh::copyTopNode(RelExpr *derivedNode, CollHeap *outHeap) {
  ...
  if (derivedNode == NULL) {
    switch (refreshType_) {
      case RECOMPUTE:
        result = new (outHeap) Refresh(mvName_, noDeleteOnRecompute_, outHeap);
        break;

      case SINGLEDELTA:
        result = new (outHeap) Refresh(mvName_, deltaDefList_, pNRowsClause_, pipelineClause_, outHeap);
        break;

      case MULTIDELTA:
        result = new (outHeap) Refresh(mvName_, deltaDefList_, phase_, pipelineClause_, outHeap);
        break;
...
void PipelineClause::applyDefaultSchemaToAll(BindWA *bindWA) {
  ...
  for (CollIndex i = 0; i < nameList.entries(); i++) nameList[i]->applyDefaults(defaultSchema);

  if (pipelineDefList_ == NULL) return;
  ...
  for (CollIndex j = 0; j < pipelineDefList_->entries(); j++) {
    ...
    for (CollIndex k = 0; k < nameList.entries(); k++) nameList2->at(k)->applyDefaults(defaultSchema);
  }
}
...
void DeltaDefinition::synthesize() {
  ...
  switch (pDeltaDefLogs->getIUDDef()->getOption()) {
    ...
    case DeltaDefIUDLog::STAT:
      useIudLog_ = TRUE;
      iudInsertedRows_ = pDeltaDefLogs->getIUDDef()->getStatistics()->getNumRowsInserted();
      iudDeletedRows_ = pDeltaDefLogs->getIUDDef()->getStatistics()->getNumRowsDeleted();
      iudUpdatedRows_ = pDeltaDefLogs->getIUDDef()->getStatistics()->getNumRowsUpdated();
      updatedColumnList_ = pDeltaDefLogs->getIUDDef()->getStatistics()->getOptionalColumnList();
      break;
...
void DeltaDefinition::processUpdateColumns(MVInfoForDML *mvInfo) {
  if (updateColumnsProcessed_) return;

  mvInfo->initUsedObjectsHash();
  const MVUsedObjectInfo *usedObject = mvInfo->findUsedInfoForTable(*getTableName());
  ...
  if (updatedCols == NULL) {
    ...
    for (CollIndex i = 0; i < updatedCols->entries(); i++) {
      ...
      if (containsIndirectUpdateColumn_ && containsDirectUpdateColumn_ && containsUpdateColUsedByMv_) break;
    }
  }
...
void DeltaDefinitionPtrList::applyDefaultSchemaToAll(BindWA *bindWA) const {
  const SchemaName &defaultSchema = bindWA->getDefaultSchema();
  for (CollIndex i = 0; i < entries(); i++) at(i)->getTableName()->applyDefaults(defaultSchema);
}
...
DeltaDefinition *DeltaDefinitionPtrList::findEntryFor(const QualifiedName &tableName) const {
  for (CollIndex i = 0; i < entries(); i++) {
    ...
    if (tableName == *deltaDef->getTableName()) return deltaDef;
  }
  return NULL;
}
...
NABoolean DeltaDefinitionPtrList::areAllDeltasInsertOnly() const {
  for (CollIndex i = 0; i < entries(); i++)
    if (!at(i)->isInsertOnly()) return FALSE;
