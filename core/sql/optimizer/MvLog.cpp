...
//----------------------------------------------------------------------------
MvLog::MvLog(const QualifiedName *tableName, const ItemExpr *columnNames, ItemExpr *rangeBegin, ItemExpr *rangeEnd,
             CollHeap *oHeap)
...
//----------------------------------------------------------------------------
const NAString MvLog::getText() const { return "mvlog"; }
...
//----------------------------------------------------------------------------
void MvLog::addLocalExpr(LIST(ExprNode *) & xlist, LIST(NAString) & llist) const {
...
RelExpr *MvLog::bindNode(BindWA *bindWA) {
  ...
  if (initializeInternalDataStructures() == FALSE) return NULL;
...
NABoolean MvLog::initializeInternalDataStructures() {
  ...
  if (pBindWA_->errStatus()) return FALSE;

  if (!isTableValidForMvLog()) return FALSE;

  isRangePartitioned_ = isTableRangePartitioned();
  pDirectionVector_ = BinderUtils::buildClusteringIndexDirectionVector(pNaTable_, heap_);
...
NABoolean MvLog::isTableValidForMvLog() const {
  ...
  if (!bitmap.getEnableMVLOGExecution()) {
    ...
    if (logType != COM_MANUAL_RANGELOG && logType != COM_MIXED_RANGELOG) {
      ...
      *CmpCommon::diags() << DgSqlCode(-12310) << DgTableName(tableName_->getQualifiedNameAsString());
    }
...
NABoolean MvLog::isTableRangePartitioned() const {
  const PartitioningFunction *partFunc = pNaTable_->getClusteringIndex()->getPartitioningFunction();
  if (partFunc->isAHashPartitioningFunction() || partFunc->isAHashDistPartitioningFunction() ||
...
void MvLog::buildBaseTableColumnNamesLists() {
  ...
  if (numberOfMvLogCols > ciColumns.entries()) {
    ...
    *CmpCommon::diags() << DgSqlCode(-12311) << DgTableName(tableName_->getQualifiedNameAsString());
    pBindWA_->setErrStatus();
    return;
  }
...
void MvLog::constructBoundriesList() {
  ...
  for (; suffixIndex < numberOfCICols; suffixIndex++) {
    ItemExpr *minValue = colsMinDefualtValList_[suffixIndex]->copyTree(heap_);

    ItemExpr *maxValue = colsMaxDefualtValList_[suffixIndex]->copyTree(heap_);

    pBeginRangeExprList_.insert(minValue);
    pEndRangeExprList_.insert(maxValue);
  }

  CMPASSERT(pBeginRangeExprList_.entries() == baseTableCiNamesList_.entries());

  CMPASSERT(pEndRangeExprList_.entries() == baseTableCiNamesList_.entries());
...
void MvLog::addClusteringIndexBoundries(LIST(ItemExprList *) & ciBoundries, const NAFileSet *pCiFileSet) const {
  PartitioningFunction *pPartFunction = pCiFileSet->getPartitioningFunction();

  if (pPartFunction->isARangePartitioningFunction()) {
    const RangePartitioningFunction *pRangePartFunction = pPartFunction->castToRangePartitioningFunction();
...
//----------------------------------------------------------------------------
void MvLog::addRangePartitionBoundries(LIST(ItemExprList *) & ciBoundries,
                                       const RangePartitioningFunction *pRangePartFunction) const {
  const RangePartitionBoundaries *pBoundries = pRangePartFunction->getRangePartitionBoundaries();
  ...
  for (; boundryIndex < (CollIndex)pBoundries->getCountOfPartitions(); boundryIndex++) {
    const ItemExprList *pBoundry = pBoundries->getBoundaryValues(boundryIndex);
...
TupleList *MvLog::constructTupleListFromBoundries(const LIST(ItemExprList *) & ciBoundries) const {
  ...
  for (CollIndex i(ciBoundries.entries() - 1); i > 0; i--) {
    ...
    ItemExpr *pAllColsExpr = BinderUtils::setItemExprFromList(allList, heap_, RIGHT_LINEAR_TREE);
  ...
  ItemExpr *tupleExpr = BinderUtils::setItemExprFromList(listOfTuplesRows, heap_, RIGHT_LINEAR_TREE);
...
RenameTable *MvLog::constructTupleListPhase1(TupleList *pTupleList) const {
  ...
  ItemExpr *pRenColsExpr = BinderUtils::setItemExprFromList(allCols, heap_, RIGHT_LINEAR_TREE);

  RenameTable *renameNode = new (heap_) RenameTable(TRUE, pTupleList, tupleListPhase1CorrName_, pRenColsExpr, heap_);
...
ItemExpr *MvLog::buildTupleListPhase2SelectResults() const {
  ...
  ItemExpr *pBeginColumnsRef =  // *_BEGIN
      BinderUtils::getNamesListAsItemExpr(beginColNamesList_, heap_, ITM_REFERENCE, tupleListPhase1CorrName_);

  ItemExpr *pEndColumnsRef =  // *_END
      BinderUtils::getNamesListAsItemExpr(endColNamesList_, heap_, ITM_REFERENCE, tupleListPhase1CorrName_);
...
RenameTable *MvLog::buildTupleListPhase2Rename(RelExpr *pTupleListP2) const {
  ...
  ItemExpr *pColNames = BinderUtils::getNamesListAsItemExpr(columnNamesList, heap_, ITM_RENAME_COL, emptyCorrName_);

  // And the Rename node itself.
  CorrName P2Name(tupleListPhase2CorrName_);
  RenameTable *pPhase2Rename = new (heap_) RenameTable(TRUE, pTupleListP2, P2Name, pColNames, heap_);
...
ItemExpr *MvLog::buildTupleListPhase3SelectResults() const {
  ...
  NAString *pMaxDecisionFactor = new (heap_) NAString(getNames().getBrIsGreater(), heap_);

  ItemExpr *pMaxColsListExpr =
      buildConditionalColumnList(brColNamesList_, beginColNamesList_, *pMaxDecisionFactor, tupleListPhase2CorrName_);
  ...
  ItemExpr *pMinColsListExpr =
      buildConditionalColumnList(erColNamesList_, endColNamesList_, *pMinDecisionFactor, tupleListPhase2CorrName_);
...
RenameTable *MvLog::buildTupleListPhase3Rename(RelExpr *pPhase3) const {
  ...
  RenameTable *pPhase3Rename = new (heap_) RenameTable(TRUE, pPhase3, P3Name, pColNames, heap_);
...
RelRoot *MvLog::constructTupleListBeginGreaterThanEndPhase(RelExpr *pPhase) const {
  ItemExpr *beginExpr = BinderUtils::getNamesListAsItemExpr(beginColNamesList_, heap_, ITM_REFERENCE, emptyCorrName_);
  ItemExpr *endExpr = BinderUtils::getNamesListAsItemExpr(endColNamesList_, heap_, ITM_REFERENCE, emptyCorrName_);

  BiRelat *selectPredicate = new (heap_) BiRelat(ITM_LESS_EQ, beginExpr, endExpr);
...
ItemExpr *MvLog::compareItems(const ItemExpr *pFirst, const ItemExpr *pSecond, OperatorTypeEnum compareType,
                              IntegerList *pDirectionVector) const {
  BiRelat *pCompare =
      new (heap_) BiRelat(compareType, ((ItemExpr *)pFirst)->copyTree(heap_), ((ItemExpr *)pSecond)->copyTree(heap_));
  ...
  IfThenElse *pIfElse =
      new (heap_) IfThenElse(pCompare, new (heap_) BoolVal(ITM_RETURN_TRUE), new (heap_) BoolVal(ITM_RETURN_FALSE));
...
//----------------------------------------------------------------------------
ItemExpr *MvLog::buildConditionalColumnList(const ConstStringList &option1names, const ConstStringList &option2names,
                                            const NAString &conditionColName, const CorrName &corrName) const {
  CMPASSERT(option1names.entries() == option2names.entries());

  ItemExprList allConditionCols(heap_);

  ColRefName *pCondColRefName = new (heap_) ColRefName(conditionColName, corrName, heap_);
  ColReference *pCondColRef = new (heap_) ColReference(pCondColRefName);
  ...
  for (CollIndex i(0); i < option1names.entries(); i++) {
    ...
    ItemExpr *pOp1Expr = BinderUtils::getNamesListAsItemExpr(option1Name, heap_, ITM_REFERENCE, corrName);
    ItemExpr *pOp2Expr = BinderUtils::getNamesListAsItemExpr(option2Name, heap_, ITM_REFERENCE, corrName);

    Case *pCase = new (heap_) Case(NULL, new (heap_) IfThenElse(pCondColRef, pOp1Expr, pOp2Expr));
...
RelExpr *MvLog::buildGetFirstNode() const {
  ...
  if (pNaTable_->getKeyCount() > 1) pScanTable->getInliningInfo().setFlags(II_MdamForcedByInlining);

  // Build a select list for just the CI columns
  ItemExpr *pCiColsSelectList =
      BinderUtils::getNamesListAsItemExpr(baseTableCiNamesList_, heap_, ITM_REFERENCE, emptyCorrName_);
  RelRoot *pRoot = new (heap_) RelRoot(pScanTable, REL_ROOT, pCiColsSelectList);
  ...
  ItemExpr *pRenamedCols =
      BinderUtils::getNamesListAsItemExpr(first1ColNamesList_, heap_, ITM_RENAME_COL, emptyCorrName_);

  RenameTable *pRenameTable = new (heap_) RenameTable(TRUE, pRoot, first1NodeCorrName_, pRenamedCols, heap_);
...
ItemExpr *MvLog::buildGetFirstSelctionPredicate() const {
  ...
  ItemExpr *cicScan = BinderUtils::getNamesListAsItemExpr(baseTableCiNamesList_, heap_, ITM_REFERENCE, emptyCorrName_);

  ItemExpr *tBegin = BinderUtils::getNamesListAsItemExpr(beginColNamesList_, heap_, ITM_REFERENCE, tupleListCorrName_);

  ItemExpr *tEnd = BinderUtils::getNamesListAsItemExpr(endColNamesList_, heap_, ITM_REFERENCE, tupleListCorrName_);
  // BEGIN <= CIC < END
  Between *pBetween = new (heap_) Between(cicScan->copyTree(heap_), tBegin->copyTree(heap_), tEnd->copyTree(heap_),
                                          TRUE,    // scan >= begin
                                          FALSE);  // scan < end
...
ItemExpr *MvLog::buildColumnNotEqualBoundriesExpr(ItemExpr *pColumn) const {
  ...
  for (CollIndex i(1); i < ciBoundries_.entries() - 2; i++) {
    ...
    BiRelat *colNEQboundry = new (heap_) BiRelat(ITM_NOT_EQUAL, pColumn->copyTree(heap_), boundryItem->copyTree(heap_));
...
RelExpr *MvLog::buildJoinWithGetFirstBlock(RelExpr *topNode) const {
...
RelExpr *MvLog::buildUnionWithEndRangeTupleList(RelExpr *topNode) const {
  ...
  TupleList *pSingleTupleList = constructTupleListFromBoundries(endRangeBoundry);
  RelRoot *pRoot = new (heap_) RelRoot(pSingleTupleList);

  topNode = new (heap_) Union(topNode, pRoot, NULL, NULL, REL_UNION, CmpCommon::statementHeap(), TRUE);
...
void MvLog::constructEndRangeBoundries(LIST(ItemExprList *) & endRangeBoundry, NABoolean needStartRange) const {
  // Create the two mirrored lists
  ItemExprList *pEndRangeList = new (heap_) ItemExprList(heap_);
  ItemExprList *pBeginRangeList = NULL;
  if (needStartRange) pBeginRangeList = new (heap_) ItemExprList(heap_);
  ...
  for (CollIndex i(0); i < numOfEntries; i++) {
    ...
    if (needStartRange) pBeginRangeList->insert(pBeginRangeExprList_[(numOfEntries - 1) - i]);
  }
...
RelExpr *MvLog::buildInsertIntoLog(RelExpr *topNode) const {
  ...
  if (pBindWA_->errStatus()) return NULL;

  RelExpr *insertNode = logTableObj.buildInsert(FALSE, ChangesTable::ALL_ROWS, FALSE, TRUE);
...
RelExpr *MvLog::buildScanEpochFromTable(RelExpr *topNode) const {
  ...
  GroupByAgg *groupByNode = new (heap_) GroupByAgg(rootNode);

  ItemExpr *selectList = buildEpochSelectList();
  RelRoot *rootNode2 = new (heap_) RelRoot(groupByNode, REL_ROOT, selectList);

  ItemExpr *renameList = new (heap_) RenameCol(NULL, new (heap_) ColRefName(InliningInfo::getEpochVirtualColName()));
  RenameTable *renameNode = new (heap_) RenameTable(rootNode2, "READ_EPOCH", renameList);
...
ItemExpr *MvLog::buildEpochSelectList() const {
  ...
  ItemExpr *minEpoch = new (heap_)
      Aggregate(ITM_MIN, new (heap_) ColReference(new (heap_) ColRefName(InliningInfo::getEpochVirtualColName())));

  // The condition is (minEpoch IS NULL AND RaiseError)
  ItemExpr *condition =
      new (heap_) BiLogic(ITM_AND, new (heap_) UnLogic(ITM_IS_NULL, minEpoch), new (heap_) RaiseError(12315));

  ItemExpr *selectList = new (heap_) Case(NULL, new (heap_) IfThenElse(condition, minEpoch, minEpoch));
...
RelExpr *MvLog::buildSimpleRangeTuple() {
  ...
  ItemExpr *renameItemList = BinderUtils::setItemExprFromList(renameColList, heap_, LEFT_LINEAR_TREE);
  RenameTable *renameNode = new (heap_) RenameTable(pRoot, getNames().getSimpleRangeTuple(), renameItemList);
...
RelExpr *MvLog::buildFinalSelectList(RelExpr *topNode) {
  ...
  ItemExpr *selectListExpr = BinderUtils::setItemExprFromList(selectList, heap_, LEFT_LINEAR_TREE);

  // Build the Root
  RelRoot *rootNode = new (heap_) RelRoot(topNode, REL_ROOT, selectListExpr);
  ...
  ItemExpr *renameListExpr = BinderUtils::setItemExprFromList(renameList, heap_, LEFT_LINEAR_TREE);

  // Build the Rename on top of the Root.
  RenameTable *renameNode = new (heap_) RenameTable(rootNode, getNames().getFinalResultName(), renameListExpr, heap_);
...
void MvLog::appendToExprList(ItemExprList &toAddto, const ConstStringList &columnNames,
                             OperatorTypeEnum itemType) const {
  BinderUtils::appendToExprList(toAddto, columnNames, heap_, itemType, emptyCorrName_);
}
