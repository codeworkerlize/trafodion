...
const char OsimHHDFSParquetFileStats::elemName[] = TAG_HHDFSPARQUETFILESTATS;
const char OsimHHDFSBucketStats::elemName[] = TAG_HHDFSBUCKETSTATS;
const char OsimHHDFSPartitionStats::elemName[] = TAG_HHDFSLISTPARTSTATS;
...
void OptimizerSimulator::buildHiveHistogramUnload(NAString& query, const QualifiedName* name, Int64 tableUID)
{
    query =   "UNLOAD WITH NULL_STRING '\\N' INTO ";
    query +=  "'" UNLOAD_HDFS_DIR"/";
    query +=  ComUser::getCurrentUsername();
...
void OptimizerSimulator::buildHiveHistogramIntervalUnload(NAString& query, const QualifiedName* name, Int64 tableUID)
{
    query =  "UNLOAD WITH NULL_STRING '\\N' INTO ";
    query += "'" UNLOAD_HDFS_DIR"/";
    query += ComUser::getCurrentUsername();
...
void OptimizerSimulator::dumpHiveHistograms()
{
    ...
    for(iterator.getNext(name, tableUID); name && tableUID; iterator.getNext(name, tableUID))
    {
        ...
        //dump histograms data to hdfs
        buildHiveHistogramUnload(query, name, *tableUID);                   
        retcode = executeFromMetaContext(query.data());
        //succeed
        if(retcode >= 0)
        {    
        ...
        buildHiveHistogramIntervalUnload(query, name, *tableUID);
        retcode = executeFromMetaContext(query.data());
        if(retcode >= 0)
        {            
            fullPath = osimLogLocalDir_;
...
OsimNAClusterInfoLinux::OsimNAClusterInfoLinux(CollHeap * heap) :
     NAClusterInfoLinux(heap, TRUE)
{
...
void OsimNAClusterInfoLinux::simulateNAClusterInfo()
{
  ...
  while(naclfile.good())
  {
    ...
    {
      naclfile >> frequency_; naclfile.ignore(OSIM_LINEMAX, '\n');
    }
    else if (!strcmp(var, "iorate_"))
    {
      naclfile >> iorate_; naclfile.ignore(OSIM_LINEMAX, '\n');
    }
    else if (!strcmp(var, "seekTime_"))
    {
      naclfile >> seekTime_; naclfile.ignore(OSIM_LINEMAX, '\n');
    }
    else if (!strcmp(var, "pageSize_"))
    {
      naclfile >> pageSize_; naclfile.ignore(OSIM_LINEMAX, '\n');
    }
    else if (!strcmp(var, "totalMemoryAvailable_"))
    {
      naclfile >> totalMemoryAvailableInKB_; naclfile.ignore(OSIM_LINEMAX, '\n');
    }
    else if (!strcmp(var, "numCPUcoresPerNode_"))
    {
      naclfile >> numCPUcoresPerNode_; naclfile.ignore(OSIM_LINEMAX, '\n');
    }
...
void OsimNAClusterInfoLinux::simulateNAClusterInfoLinux()
{
  ...
  while(nacllinuxfile.good())
  {
    ...
    if(!strcmp(var, "nodeIdAndNodeNameMap"))
    {
      ...
      int numEntries = 0;
      nacllinuxfile >> numEntries; nacllinuxfile.ignore(OSIM_LINEMAX, '\n');
    ...
    {
      nacllinuxfile >> totalMemoryAvailableInKB_; nacllinuxfile.ignore(OSIM_LINEMAX, '\n');
    }
    else if (!strcmp(var, "numCPUcoresPerNode_"))
    {
...
void OptimizerSimulator::buildHistogramCreate(NAString& query, const QualifiedName * name, NABoolean isHive)
{
    ...
    if(isHive)
        query += "TRAFODION.\"_HIVESTATS_\"." + name->getObjectName();
    else
        query += name->getQualifiedNameAsString();
    query += " (  TABLE_UID      LARGEINT NO DEFAULT NOT NULL NOT DROPPABLE NOT SERIALIZED"
...
void OptimizerSimulator::buildHistogramIntervalUpsert(NAString& query, const QualifiedName * name, NABoolean isHive)
{
    ...
    if(CmpCommon::getDefault(OSIM_READ_OLD_FORMAT) == DF_OFF)
    {
        query += " SELECT TABLE_UID"                        
        		...
        		", INTERVAL_ROWCOUNT" 
        		", INTERVAL_UEC"
        		", CAST(TRANSLATE(DECODE_BASE64(INTERVAL_BOUNDARY) USING UTF8TOUCS2) AS VARCHAR(250) CHARACTER SET UCS2 NOT NULL)"
        		", STD_DEV_OF_FREQ"
        		", V1, V2, V3, V4"
        		", CAST(TRANSLATE(DECODE_BASE64(V5) USING UTF8TOUCS2) AS VARCHAR(250) CHARACTER SET UCS2 NOT NULL)"
        		", CAST(TRANSLATE(DECODE_BASE64(V6) USING UTF8TOUCS2) AS VARCHAR(250) CHARACTER SET UCS2 NOT NULL)"
...
void OptimizerSimulator::enterSimulationMode()
{
  ...
  // a simulated one
  CmpCommon::context()->setCompilerClusterInfo(
       new(ctxHeap) OsimNAClusterInfoLinux(ctxHeap));
  //reinitialize CQDs
  NADefaults::updateSystemParameters(FALSE);
  //apply cqds
  readAndSetCQDs();
...
void OptimizerSimulator::histogramHDFSToLocal()
{
    ...
    //build hdfs handle
    hdfsFS hdfs = (GetCliGlobals()->currContext())->
                   getHdfsServerConnection((char *) "default",0);
...
void OptimizerSimulator::capturePrologue()
{
  ...
  {    
    ...
    {
      capture_CQDs();
      CURRCONTEXT_CLUSTERINFO->captureNAClusterInfo(
           *writeLogStreams_[NACLUSTERINFO]);
      //captureVPROC();
...
{
  const NAWNodeSet * tenantNodes =
    GetCliGlobals()->currContext()->getAvailableNodes();
...
void OptimizerSimulator::readLogFile_Tenant()
{
  ...
  if(isFileExists(logFilePaths_[TENANT],isDir))
  {
    ...
    {
      deserializedTenant = NAWNodeSet::deserialize(tenantNodes.c_str(),
                                                   heap_);
      if (!deserializedTenant)
        raiseOsimException("Unable to convert tenant string %s to a list of nodes.",
                           tenantNodes.c_str());
    }
  }
...
void OsimHHDFSStatsBase::serializeAttrs(XMLString & xml)
{
    ...
    xml.append("numSplitUnits='").append(std::to_string((long long)(hhstats->numSplitUnits_)).c_str()).append("' ");
...
void OsimHHDFSSplittableFileStats::serializeAttrs(XMLString & xml)
{
    if(NULL == mirror_) 
        return;
     ...
     HHDFSSplittableFileStats* hhstats = (HHDFSSplittableFileStats*)mirror_;
...
NABoolean OsimHHDFSSplittableFileStats::setValue(HHDFSStatsBase* stats, const char *attrName, const char *attrValue)
{
    if(NULL == stats)
        return FALSE;
    ...
    HHDFSSplittableFileStats* hhstats = dynamic_cast<HHDFSSplittableFileStats*>(stats);
    assert(hhstats);
    ...
    if (!strcmp(attrName, "numOfRows"))
    {
          if(strcmp(attrValue, "no_entry"))
          {
              ...
              for(Int32 i = 0; i < valueList.entries(); i++)
                hhstats->numOfRows_.insert((Int64) std::atoll(valueList[i].data()));
          }
    }
    else if (!strcmp(attrName, "offsets"))
    {
        if(strcmp(attrValue, "no_entry"))
        {
              ...
              for(Int32 i = 0; i < valueList.entries(); i++)
                hhstats->offsets_.insert((Int64) std::atoll(valueList[i].data()));
        }
    }
    else if (!strcmp(attrName, "totalBytes"))
    {
        if(strcmp(attrValue, "no_entry"))
        {
              ...
              for(Int32 i = 0; i < valueList.entries(); i++)
                hhstats->totalBytes_.insert((Int64) std::atoll(valueList[i].data()));
        }
    }
    else
      return FALSE;
...
void OsimHHDFSPartitionStats::serializeAttrs(XMLString & xml)
{
    if(NULL == mirror_) 
        return;
     ...
     HHDFSPartitionStats* hhstats = (HHDFSPartitionStats*)mirror_;
...
{      
      ...
      if (!strcmp(attrName, "position"))
        ...
        hhstats->totalRows_= std::atol(attrValue);
               ...
               !strcmp(attrName, "numSplitUnits"))
        hhstats->numSplitUnits_= std::atol(attrValue);
...
NABoolean OsimHHDFSPartitionStats::setValue(HHDFSStatsBase* stats, const char *attrName, const char *attrValue)
{      
      if(NULL == stats)
          return FALSE;
      ...
      HHDFSPartitionStats* hhstats = dynamic_cast<HHDFSPartitionStats*>(stats);
      assert(hhstats);
...
NABoolean OsimHHDFSTableStats::setValue(HHDFSStatsBase* stats, const char *attrName, const char *attrValue)
{
      ...
      if (!strcmp(attrName, "tableDir"))
        ...
        hhstats->type_= (HiveFileType)std::atoi(attrValue);
      else
        return FALSE;
...
void OsimHHDFSTableStats::startElement(void *parser, const char *elementName, const char **atts)
{
    if(!strcmp(elementName, TAG_HHDFSLISTPARTSTATS)){
        OsimHHDFSPartitionStats* entry = new (heap_) OsimHHDFSPartitionStats(this, NULL, heap_);
        HHDFSPartitionStats* hhstats = new (heap_) HHDFSPartitionStats(heap_, mirror_->getTable());
...
void OsimHHDFSPartitionStats::startElement(void *parser, const char *elementName, const char **atts)
{
    if(!strcmp(elementName, TAG_HHDFSBUCKETSTATS)){
        ...
        Int32 pos = entry->getPosition();
        addEntry(entry, pos);
        ((HHDFSPartitionStats*)mirror_)->insertAt(pos, hhstats);
...
void OsimHHDFSBucketStats::startElement(void *parser, const char *elementName, const char **atts)
{
    ...
    else if(!strcmp(elementName, TAG_HHDFSPARQUETFILESTATS)){
        OsimHHDFSParquetFileStats* entry = new (heap_) OsimHHDFSParquetFileStats(this, NULL, heap_);
        HHDFSParquetFileStats* hhstats = new (heap_) HHDFSParquetFileStats(heap_, mirror_->getTable());
        entry->restoreHHDFSStats(hhstats, atts);
        Int32 pos = entry->getPosition();
        addEntry(entry, pos);
        ((HHDFSBucketStats*)mirror_)->insertAt(pos, hhstats);
