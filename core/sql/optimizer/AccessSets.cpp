...
TableAccess::TableAccess() : tableID_(NULL), empty_(TRUE) {}

// Explicit ctor
TableAccess::TableAccess(const NATable *theTable) : tableID_(const_cast<NATable *>(theTable)), empty_(FALSE) {}

// Copy ctor
TableAccess::TableAccess(const TableAccess &other) : tableID_(other.tableID_), empty_(other.empty_) {}
...
NABoolean TableAccess::match(const TableAccess &other) const {
  return ((tableID_ == other.tableID_) && (empty_ == other.empty_));
}
...
void TableAccess::print(FILE *ofd, const char *indent, const char *title) const {
  if (isEmpty())
    ...
    BUMP_INDENT(indent);
    fprintf(ofd, "%s%s this=%p, NATable=%p, ", NEW_INDENT, title, this, tableID_);
...
// Comparison operator
NABoolean ReadTableAccess::operator==(const TableAccess &other) const { return (match(other) && other.isReadAccess()); }
...
// Ctor
SubTreeAccessSet::SubTreeAccessSet(CollHeap *heap) : opaqueForRead_(FALSE), opaqueForWrite_(FALSE) {
...
void SubTreeAccessSet::add(ReadTableAccessPtr snas) {
  ...
  for (CollIndex i = 0; i < readSet_->entries(); i++)
    if (*(readSet_->at(i)) == *snas) return;

  CMPASSERT(!snas->isEmpty());

  if (snas->isOpaque()) opaqueForRead_ = TRUE;
...
void SubTreeAccessSet::add(WriteTableAccessPtr snas) {
  ...
  for (CollIndex i = 0; i < writeSet_->entries(); i++)
    if (*(writeSet_->at(i)) == *snas) return;

  CMPASSERT(!snas->isEmpty());

  if (snas->isOpaque()) opaqueForWrite_ = TRUE;
...
void SubTreeAccessSet::add(const SubTreeAccessSet *stas) {
  ...
  CollIndex i = 0;
  for (i = 0; i < stas->readSet_->entries(); i++) add((*stas->readSet_)[i]);

  for (i = 0; i < stas->writeSet_->entries(); i++) add((*stas->writeSet_)[i]);
}
...
NABoolean SubTreeAccessSet::isConflicting(const TableAccess *snas) const {
  ...
  if (opaqueForWrite_) return TRUE;

  CollIndex i = 0;
  for (i = 0; i < writeSet_->entries(); i++)
    if ((*writeSet_)[i]->isConflicting(*snas)) return TRUE;

  // For write operations, check the read set too.
  if (!snas->isReadAccess()) {
    if (opaqueForRead_) return TRUE;

    for (i = 0; i < readSet_->entries(); i++)
      if ((*readSet_)[i]->isConflicting(*snas)) return TRUE;
  }
...
NABoolean SubTreeAccessSet::isConflicting(const SubTreeAccessSet *other) const {
  ...
  if (opaqueForWrite_ && !other->isEmpty()) return TRUE;

  // Check against each and every table access in the write set.
  CollIndex i = 0;
  for (i = 0; i < writeSet_->entries(); i++)
    if (other->isConflicting((*writeSet_)[i])) return TRUE;

  // Check against each and every table access in the read set.
  for (i = 0; i < readSet_->entries(); i++)
    if (other->isConflicting((*readSet_)[i])) return TRUE;

  return FALSE;
}

NABoolean SubTreeAccessSet::isEmpty() const { return (readSet_->isEmpty() && writeSet_->isEmpty()); }
...
void SubTreeAccessSet::print(FILE *ofd, const char *indent, const char *title) const {
  ...
  if (this == NULL)
    ...
    for (i = 0; i < readSet_->entries(); i++) (*readSet_)[i]->print(ofd, indent);
    if (i) fprintf(ofd, "\n");

    for (i = 0; i < writeSet_->entries(); i++) (*writeSet_)[i]->print(ofd, indent);
    if (i) fprintf(ofd, "\n");
  }
}
...
SubTreeAccessSet *RelExpr::calcAccessSets(CollHeap *heap) {
  if (getInliningInfo().isAccessSetNeeded()) {
    ...
    if (child(0) != NULL) {
      ...
      if (getAccessSet0() == NULL) setAccessSet0(new (heap) SubTreeAccessSet(heap));
    }

    if (child(1) != NULL) {
      setAccessSet1(child(1)->calcAccessSets(heap));
      if (getAccessSet1() == NULL) setAccessSet1(new (heap) SubTreeAccessSet(heap));
    }
...
SubTreeAccessSet *Scan::calcAccessSets(CollHeap *heap) {
  ...
  if (result == NULL) result = new (heap) SubTreeAccessSet(heap);

  const ReadTableAccessPtr readAccess = new (heap) ReadTableAccess(getTableDesc()->getNATable());
...
SubTreeAccessSet *GenericUpdate::calcAccessSets(CollHeap *heap) {
  ...
  const WriteTableAccessPtr writeAccess = new (heap) WriteTableAccess(getTableDesc()->getNATable());
