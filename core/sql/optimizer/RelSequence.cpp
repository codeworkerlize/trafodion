...
//
RelSequence::RelSequence(RelExpr *child, ItemExpr *requiredOrder, CollHeap *oHeap)
...
RelSequence::RelSequence(RelExpr *child, ItemExpr *partitionBy, ItemExpr *orderBy, CollHeap *oHeap)
    : RelExpr(REL_SEQUENCE, child, NULL, oHeap),
...
ItemExpr *RelSequence::removeRequiredOrderTree() {
...
//
NABoolean RelSequence::duplicateMatch(const RelExpr &other) const {
  ...
  if (!(partition() == o.partition())) return FALSE;

  // If the sequence functions are not the same
  // then the nodes are not identical
  //
  if (!(sequenceFunctions() == o.sequenceFunctions())) return FALSE;

  if (!(sequencedColumns() == o.sequencedColumns())) return FALSE;

  if (!(cancelExpr() == o.cancelExpr())) return FALSE;
...
// case).
//
RelExpr *RelSequence::copyTopNode(RelExpr *derivedNode, CollHeap *outHeap) {
  RelSequence *result;
  ...
  if (partitionBy_) result->partitionBy_ = partitionBy_->copyTree(outHeap)->castToItemExpr();
...
//
void RelSequence::addLocalExpr(LIST(ExprNode *) & xlist, LIST(NAString) & llist) const {
...
//
void RelSequence::getPotentialOutputValues(ValueIdSet &outputValues) const {
...
// case).
//
RelExpr *PhysSequence::copyTopNode(RelExpr *derivedNode, CollHeap *outHeap) {
  PhysSequence *result;
...
//
void RelSequence::transformNode(NormWA &normWARef, ExprGroupId &locationOfPointerToMe) {
  ...
  if (partition().transformNode(normWARef, child(0), getGroupAttr()->getCharacteristicInputs())) {
  ...
  if (sequenceFunctions().transformNode(normWARef, child(0), getGroupAttr()->getCharacteristicInputs())) {
  ...
  if (sequencedColumns().transformNode(normWARef, child(0), getGroupAttr()->getCharacteristicInputs())) {
  ...
  if (cancelExpr().transformNode(normWARef, child(0), getGroupAttr()->getCharacteristicInputs())) {
...
RelExpr *RelSequence::normalizeNode(NormWA &normWARef) {
  ...
  if (requiredOrder_.entries() > 0) {
    if (child(0)->getOperatorType() == REL_FIRST_N) {
      ...
      if (firstn->isFirstN() || firstn->getFirstNRows() < 0)  // that is, [first n], and [last n]
      {
        firstn->reqdOrder().insert(requiredOrder());
      }
    }
    ...
    ValueIdSet orderBySet(requiredOrder_), coveredOrderBySet, inputsNeededForOrderBy;

    GroupAttributes *childGAPtr = child(0).getPtr()->getGroupAttr();

    childGAPtr->coverTest(orderBySet, getGroupAttr()->getCharacteristicInputs(), coveredOrderBySet,
...
void RelSequence::pullUpPreds() {
  ...
  //
  emptyGA.coverTest(child(0)->selectionPred(), availableInputs, predicatesToPullUp, notUsed, &predicatesThatStay);
...
void RelSequence::synthEstLogProp(const EstLogPropSharedPtr &inputEstLogProp) {
  ...
  EstLogPropSharedPtr myEstProps =
      synthEstLogPropForUnaryLeafOp(inputEstLogProp, childEstProp->getColStats(), childEstProp->getResultCardinality());
...
//
void RelSequence::synthLogProp(NormWA *normWAPtr) {
...
ItemExpr *RelSequence::buildDefaultOrderByForRownum(BindWA *bindWA) {
  ItemExpr *result = NULL;

  // gather order of first child
  RelExpr *subrel = child(0)->castToRelExpr();
  if (subrel->getOperatorType() == REL_SCAN) {
    ...
    result = ((Join *)subrel)->buildDefaultOrderByForRownum(bindWA);
  } else if (subrel->getOperatorType() == REL_SEQUENCE) {
    result = ((RelSequence *)subrel)->buildDefaultOrderByForRownum(bindWA);
  }

  ItemExprList resultList(bindWA->wHeap());
  if (result != NULL) resultList.insertTree(result, ITM_ITEM_LIST, FALSE, FALSE);
...
CostMethod *PhysSequence::costMethod() const {
...
ValueIdList RelSequence::mapSortKey(const ValueIdList &sortKey) const {
  ValueIdMap seqColsMap;

  for (ValueId sequenceCol = sequencedColumns().init(); sequencedColumns().next(sequenceCol);
       sequencedColumns().advance(sequenceCol)) {
    CMPASSERT(sequenceCol.getItemExpr()->getOperatorType() == ITM_NOTCOVERED);

    seqColsMap.addMapEntry(sequenceCol, sequenceCol.getItemExpr()->child(0)->getValueId());
  }
...
PhysicalProperty *PhysSequence::synthPhysicalProperty(const Context *context, const int pn, PlanWorkSpace *pws) {
  ...
  // ---------------------------------------------------------------------
  PhysicalProperty *sequencePP = new (CmpCommon::statementHeap())
...
void RelSequence::pushdownCoveredExpr(const ValueIdSet &outputExpr, const ValueIdSet &newExternalInputs,
  ...
  //
  ValueId refVal;
  for (ValueId seqCol = sequencedColumns().init(); sequencedColumns().next(seqCol);
...
Context *RelSequence::createContextForAChild(Context *myContext, PlanWorkSpace *pws, int &childIndex) {
  ...
  if (!getHasTDFunctions()) {
    const ValueIdSet seqCols = sequencedColumns();

    // remove any NotCovered expressions
    // NotCovered expr can never pass isCovered() test during the
    // verification of sortkey with the otuputs from the child node.
    // See the following code in method
    // RequirementGenerator::checkCompatibilityWithGroupAttributes().
    //
    // 1356       ValueIdSet sortKey(*addedSortKey_);
    // 1357       stillFeasible_ = sortKey.isCovered(
    // 1358         newInputs,
    // 1359         *ga_,
    // 1360         referencedInputs,
    // 1361         coveredSubExpr,
    // 1362         uncoveredExpr);
    // 1363       coveredSubExpr.clear();
    // 1364     }
    // 1365   }
    //
    // Places where NotCoverred are added for Olap functions:
    //
    //  1. In RelSequence::bindNode() to build sequencedColumns. This is
    //     done for every column.
    //            sequencedColumns() += newColumn->getValueId();
    //
    //  2. In Aggregate::bindNode() called from
    //     Aggregate::transformOlapFunction().
    //
    //  Here the aggregate represents the olap function to which partitionBy
    //  and OrderBy of the olap function are attached.  NotCovered becomes the
    //  parent of the olap aggregate function.  This expression is then
    //  added to the sequencedColumns. Mantis 1723
    //
    ValueIdList noNotCoveredPartition(partition());
    noNotCoveredPartition.removeNotCoveredExpr(seqCols);
    sortKey.insert(noNotCoveredPartition);
  }
  ...
  if (okToAttemptESPParallelism(myContext, pws, childNumPartsRequirement, childNumPartsAllowedDeviation,
                                numOfESPsForced) AND(partition().entries() > 0)) {
...
static NABoolean checkUnresolvedSequenceFunctions(BindWA *bindWA) {
  ValueIdSet &seqs = bindWA->getCurrentScope()->getUnresolvedSequenceFunctions();

  if (seqs.isEmpty()) return FALSE;  // no error
...
RelExpr *RelSequence::bindNode(BindWA *bindWA) {
  ...
  //
  sequenceFunctions() += bindWA->getCurrentScope()->getUnresolvedSequenceFunctions();
  ...
  if (partitionBy_) {
    ...
    partitionBy_->convertToValueIdList(partition(), bindWA, ITM_ITEM_LIST);
  ...
  //
  if (checkUnresolvedSequenceFunctions(bindWA)) return this;
  ...
  if (childTable.isGrouped()) resultTable->setGroupedFlag();
  ...
  for (i = 0; i < sysColList->entries(); i++) {
    ...
    if (sysColList->at(i)->isGrouped()) {
      ColumnNameMap *cnm = resultTable->findColumn(sysColList->at(i)->getColRefNameObj());
  ...
  for (i = 0; i < colList->entries(); i++) {
    ...
    resultTable->addColumn(bindWA, colList->at(i)->getColRefNameObj(), newColumn->getValueId(), USER_COLUMN,
                           colList->at(i)->getHeading());
    ncToOldMap.addMapEntry(newColumn->getValueId(), columnValueId);
    if (colList->at(i)->isGrouped()) {
      ColumnNameMap *cnm = resultTable->findColumn(colList->at(i)->getColRefNameObj());
      cnm->getColumnDesc()->setGroupedFlag();
    }
...
NABoolean PhysSequence::isBigMemoryOperator(const Context *context, const int planNumber) {
  ...
  const double probeCount = MIN_ONE(inLogProp->getResultCardinality().value());

  CMPASSERT(numHistoryRows() > 0 && getEstHistoryRowLength() > 0);
...
// RelSequence.
//
void RelSequence::addUnResolvedSeqFunctions(ValueIdSet &unresolvedSeqFuncs, BindWA *bindWA) {
  ...
  if (sequenceFunctions_.entries()) {
    if (CmpCommon::getDefault(OLAP_CAN_INVERSE_ORDER) == DF_ON) {
      ...
      if ((mustInv && canInv) || shouldInv) {
        for (ValueId sfId = sequenceFunctions_.init(); sequenceFunctions_.next(sfId);
             sequenceFunctions_.advance(sfId)) {
    ...
    if (getHasOlapFunctions() || getHasTDFunctions()) {
      ...
      for (ValueId sfId = sequenceFunctions_.init(); sequenceFunctions_.next(sfId); sequenceFunctions_.advance(sfId)) {
        ...
        if ((getHasTDFunctions() && !sf->isTDFunction()) ||
            (getHasOlapFunctions() && !sf->isOLAP())) {  // Using rank function and sequence functions together
        ...
        if ((!getHasOlapFunctions()) && prevSF)
          if (!sf->hasBaseEquivalence(prevSF)) {  // All rank functions need to have the same expression as argument
      ...
      if (getHasOlapFunctions()) {
        ...
        if (partitionBy) {
          ...
          partitionBy->convertToValueIdList(partition(), bindWA, ITM_ITEM_LIST);
      ...
      if (orderBy) {
        ...
        orderBy->convertToValueIdList(requiredOrder(), bindWA, ITM_ITEM_LIST);
        bindWA->getCurrentScope()->context()->inOrderBy() = FALSE;
        ...
        if (seqFunc->isTDFunction()) {
          ...
          for (CollIndex i = 0; i < requiredOrder().entries(); i++) {
            ...
            if (type->supportsSQLnull()) {
              ...
              newExpr = new (bindWA->wHeap()) Cast(newExpr, ((ConstValue *)constZero)->getType()->newCopy());
...
void PhysSequence::estimateHistoryRowLength(const ValueIdSet &sequenceFunctions, const ValueIdSet &outputFromChild,
    ...
    if (valId.getItemExpr()->isASequenceFunction()) {
      ...
      switch (itmExpr->getOperatorType()) {
        ...
        case ITM_OLAP_LAST_VALUE:
          updateEstHistoryRowLength(histIds, itmExpr->child(0)->getValueId(), estimatedLength);
        ...
        case ITM_RUNNING_PIVOT_GROUP:
          updateEstHistoryRowLength(histIds, itmExpr->getValueId(), estimatedLength);
          break;
        case ITM_OLAP_SUM:
        case ITM_OLAP_COUNT:
        case ITM_OLAP_RANK:
        ...
        case ITM_OLAP_PIVOT_GROUP: {
          ItemExpr *tmpExpr = ((ItmSeqOlapFunction *)itmExpr)->transformOlapFunction(CmpCommon::statementHeap());
        ...
        case ITM_RUNNING_CHANGE:
          if (itmExpr->child(0)->getOperatorType() == ITM_ITEM_LIST) {
            ...
            for (CollIndex i = nc; i > 0; i--) {
              updateEstHistoryRowLength(histIds, changeValues[i - 1]->getValueId(), estimatedLength);
            }
          } else {
            updateEstHistoryRowLength(histIds, itmExpr->child(0)->getValueId(), estimatedLength);
            // estimatedLength +=  itmExpr->child(0)->getValueId().getType().getTotalSize();
  ...
  if (!children.isEmpty()) {
    estimateHistoryRowLength(children, outputFromChild, histIds, estimatedLength);
  }
}
...
void PhysSequence::retrieveCachedHistoryInfo(RelSequence *cacheRelSeq) {
  ...
  if (!cacheRelSeq->getHistoryInfoCached()) {
    ...
    computeHistoryRows(sequenceFunctions(), numHistoryRows, unableToCalculate, unboundedFollowing, minFollowing,
                       outputFromChild);
  ...
  CMPASSERT(cacheRelSeq->getCachedHistoryRowLength() != 0 && cacheRelSeq->getCachedNumHistoryRows() != 0 &&
