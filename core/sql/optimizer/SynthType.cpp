				    ...
				    ItemExpr *parentOp)
{
  ...
  for (CollIndex i = 0; i < exprList1.entries(); i++) {
    ...
    if ((vid1.getItemExpr()->getOperatorType() == ITM_DYN_PARAM ||
        ...
        (vid2.getItemExpr()->getOperatorType() == ITM_CONSTANT ||
         vid2.getItemExpr()->getOperatorType() == ITM_BASECOLUMN ||
         vid2.getItemExpr()->getOperatorType() == ITM_CASE ||
         vid2.getItemExpr()->isStringOutput()))
    {
      ...
      if (DFS2REC::isCharacterString(vid2.getType().getFSDatatype()))
        {
          if (strncmp(vid2.getType().getTypeName(), "CHAR", 4) == 0)
            desiredType = new HEAP SQLChar(HEAP, CharLenInfo(len,len), //DEFAULT_CHARACTER_LENGTH,
                                            ...
                                            FALSE,
                                            vid2.getType().getCharSet(),
                                            CharInfo::DefaultCollation,
                                            CharInfo::COERCIBLE);
          else
            desiredType = new HEAP SQLVarChar(HEAP, CharLenInfo(len,len), //DEFAULT_CHARACTER_LENGTH,
                                            supportNull,
                                            FALSE/*isUpShifted*/,
                                            FALSE/*isCaseinsensitive*/,
                                            vid2.getType().getCharSet(),
      ...
      else
        {
          if (vid2.getType().getFSDatatype() == REC_BINARY_STRING)
            desiredType = new HEAP SQLBinaryString(HEAP, len,
                                                 supportNull,
                                                 FALSE/*ischar*/);
          else
            desiredType = new HEAP SQLBinaryString(HEAP, len,
      ...
      continue;
             ...
             (vid1.getItemExpr()->getOperatorType() == ITM_CONSTANT ||
              ...
              vid1.getItemExpr()->getOperatorType() == ITM_CASE ||
              vid1.getItemExpr()->isStringOutput()))
    {
      Lng32 len_def = CmpCommon::getDefaultNumeric(VARCHAR_PARAM_DEFAULT_SIZE);
    ...
    if ((vid1.getItemExpr()->getOperatorType() == ITM_DYN_PARAM ||
        vid1.getItemExpr()->getOperatorType() == ITM_ROWSETARRAY_SCAN) &&
        vid1.getType().getTypeQualifier() == NA_CHARACTER_TYPE &&
        vid1.getType().getNominalSize() == 0)
    {
      Int64 length = CmpCommon::getDefaultNumeric(VARCHAR_PARAM_DEFAULT_SIZE); 
      CharType *newTp =  new HEAP SQLVarChar(HEAP, length,
                                          supportNull,
                                          FALSE,
                                          FALSE,
                                          vid1.getType().getCharSet(),
    ...
    if ( supportNull )
    {
          ...
          vid1.getItemExpr()->getOperatorType() == ITM_ROWSETARRAY_SCAN)
      {
        ...
        const NAType* synthesizedNewType = 
          vid1.getItemExpr()->pushDownType(*newType, NA_UNKNOWN_TYPE);
          ...
          vid2.getItemExpr()->getOperatorType() == ITM_ROWSETARRAY_SCAN)
      {
        ...
        newType->setNullable(TRUE);
        const NAType* synthesizedNewType = 
          vid2.getItemExpr()->pushDownType(*newType, NA_UNKNOWN_TYPE);
...
static void synthesizeType4DynamicParam(ValueId &dyamicParamId, const ValueId & anotherId) 
{
  ...
  if (dyamicParamId.getItemExpr()->getOperatorType() == ITM_DYN_PARAM ||
    ...
    {    
      if (type2.getTypeQualifier() == NA_CHARACTER_TYPE ||
          type2.getTypeQualifier() == NA_UNKNOWN_TYPE)
        {    
          NABoolean supportNull = (CmpCommon::getDefault(MODE_COMPATIBLE_1) == DF_ON);
          Lng32 len1 = CmpCommon::getDefaultNumeric(VARCHAR_PARAM_DEFAULT_SIZE);
          Lng32 len2 = len1;
          if (type2.getTypeQualifier() == NA_CHARACTER_TYPE)
            len2 = type2.getNominalSize();
          len1 = MAXOF(len1,len2);
          NAString cs = CmpCommon::getDefaultString(DYNAMIC_PARAM_DEFAULT_CHARSET);
          CharInfo::CharSet new_cs = CharInfo::getCharSetEnum(cs);
          ItemExpr *newChild = NULL;
          CharType *castType = NULL;
          castType = new (heap) SQLVarChar(heap,
                                           len1,
                                           supportNull,
                                           FALSE/*isUpShifted*/,
                                           FALSE/*isCaseinsensitive*/,
...
const NAType *BuiltinFunction::synthesizeType()
{
  ...
  switch (getOperatorType())
    ...
    case ITM_FIRSTNGRAM:
    {
        ...
        retType = new HEAP
        SQLVarChar(HEAP, 3, typ1.supportsSQLnull());
    }
    break;
    case ITM_NGRAMCOUNT:
    {
...
const NAType * DecodeBase64::synthesizeType()
{
  ...
  if ((charSet_ != CharInfo::UnknownCharSet) &&
      (NOT CharInfo::isCharSetFullySupported(charSet_)))
    {
  ...
  if (type_)
  ...
  else
    {
      ...
      Int32 decodedCharLen =
        maxByteLength / CharInfo::minBytesPerChar(charSet_);

      retType = new HEAP SQLVarChar(
           HEAP, 
           CharLenInfo(decodedCharLen, maxByteLength),
  ...
  if (ntyp1.isExact())
    {
      ...
      if (precision <= MAX_NUMERIC_PRECISION)
      ...
      else
        {
          ...
          result = new HEAP SQLBigNum(HEAP, ntyp1.getPrecision(),
                                      ntyp1.getScale(),
                                      ((SQLBigNum&)ntyp1).isARealBigNum(),
                                      ntyp1.isSigned(),
                                      ntyp1.supportsSQLnull());
        }
    }
  else
    {
...
const NAType *Aggregate::synthesizeType()
{
  ...
  case ITM_RANGE_VALUES_INSERT:
  {
   ...
   // during cstr.
    ...
    Lng32 maxLength = 
        (ActiveSchemaDB()->getDefaults()).figureOutMaxLength(uec);
...
const NAType *PivotGroup::synthesizeType()
{
  ...
  if (operand.getTypeQualifier() == NA_CHARACTER_TYPE)
  {
    ...
    return new HEAP SQLVarChar(HEAP, maxLen_, TRUE,
                                         origType.isUpshifted(),FALSE, origType.getCharSet());
  }
  else
  {
    return new HEAP SQLVarChar(HEAP, maxLen_, TRUE,
                                         FALSE, FALSE, delimiter.getCharSet());
  }
}
...
const NAType *AnsiTenantFunction::synthesizeType()
{
  return new HEAP SQLVarChar(HEAP, OPT_MAX_TENANTNAME_LEN, FALSE);
}
...
const NAType *IfThenElse::synthesizeType()
{
  ...
  if (CmpCommon::getDefault(MODE_COMPATIBLE_1) == DF_ON )
  {
    ...
    if (thenId.getItemExpr()->getOperatorType() == ITM_DYN_PARAM)
    {
      ...
      newType->setNullable(TRUE);
      const NAType* synthesizedNewType = 
        thenId.getItemExpr()->pushDownType(*newType, NA_UNKNOWN_TYPE);
    ...
    if (elseId.getItemExpr()->getOperatorType() == ITM_DYN_PARAM)
    {
      ...
      newType->setNullable(TRUE);
      const NAType* synthesizedNewType = 
        elseId.getItemExpr()->pushDownType(*newType, NA_UNKNOWN_TYPE);
      ...
      if (getType()->isComposite())
        {
          *CmpCommon::diags() << DgSqlCode(-3242)
                              << DgString0("Cannot cast param as composite type.");
          return NULL;
        }
...
static UInt32 lcl_getPercision4TimeFormat(Lng32 nFormat)
{
  ...
  if ( ExpDatetime::DATETIME_FORMAT_TS13 == nFormat )
    nPrecision = DatetimeType::MAX_FRACTION_PRECISION;
  return nPrecision;
}
// -----------------------------------------------------------------------
...
const NAType *DateFormat::synthesizeType()
{
  ...
  if (formatAsDate)
  ...
  else if (formatAsNumber)
    {
      ...
      if (formatStr_.contains("RN",NAString::ignoreCase))
        nLen = length + 15;
...
const NAType *MonthsBetween::synthesizeType()
{
  ...
  NABoolean bAllowNull = operand1.supportsSQLnullLogical() OR
                         operand2.supportsSQLnullLogical();
...
const NAType *RegexpSubstr::synthesizeType()
{
  ...
  if (getArity() >3)
    {
      if (child(3)->isNullConstant())
        bOper4IsNullConstant = TRUE;
  ...
  if (operand4 && operand4->getTypeQualifier() != NA_NUMERIC_TYPE
      && !bOper4IsNullConstant)
    {//operand4 must be integer, but allow null constant
      *CmpCommon::diags() << DgSqlCode(-4182)
                          << DgString0(getTextUpper())
                          << DgInt0(4)
                          << DgString1("Integer");
  ...
  if (getArity() >2)
    {
      if (child(2)->isNullConstant())
        bOper3IsNullConstant = TRUE;
      ValueId vid3 = child(2)->getValueId();
      SQLInt si(NULL);
  ...
  if (operand3 && operand3->getTypeQualifier() != NA_NUMERIC_TYPE
       && !bOper3IsNullConstant)
    {//operand3 must be integer, but allow null constant
      *CmpCommon::diags() << DgSqlCode(-4182)
                          << DgString0(getTextUpper())
                          << DgInt0(3)
                          << DgString1("Integer");
...
const NAType *Repeat::synthesizeType()
{
      ...
      if (foldExpr && foldExpr->castToConstValue(negate))
        {
          ...
          if (tmpCv->canGetExactNumericValue())
            tempExpr = foldExpr;
        }
    }
  ...
  else if ((tempExpr->getOperatorType() == ITM_CONSTANT) &&
      (tempExpr->castToConstValue(negate)))
    {
...
const NAType *Replace::synthesizeType()
{
  ...
  if (getOperatorType() == ITM_REGEXP_REPLACE) {
    double fudge_factor = (ActiveSchemaDB()->getDefaults()).
      getAsDouble(REGEXP_REPLACE_RESULT_TYPE_FACTOR);
...
const NAType *PatternMatchingFunction::synthesizeType()
{
  ...
  if (getArity() > 2) {   // Escape clause was specified
    ...
    const SQLChar charType(NULL, 1, TRUE, FALSE, FALSE, FALSE, typ1->getCharSet(), CharInfo::DefaultCollation, CharInfo::COERCIBLE);
    // fix charset there
...
const NAType *Upper::synthesizeType()
{
  ...
  //
    ...
    if (((CharType&)operand).getFSDatatype() == REC_BYTE_F_ASCII)
      ct->setSpaceSensitive(FALSE);
  }
...
const NAType *PositionFunc::synthesizeType()
{
  ...
  vid1.coerceType(NA_CHARACTER_TYPE, new_cs,
                  false, VARCHAR_PARAM_DEFAULT_SIZE_IN_FUNCTION);
...
*/
  ...
  // Return the result.
  //
  NABoolean resultNullable = (operand1->supportsSQLnullLogical() ||
                              operand2->supportsSQLnullLogical());
...
const NAType *Substring::synthesizeType()
{
  ...
  if (operand1->getFSDatatype() == REC_CLOB)
    {
      return new HEAP
	SQLClob(HEAP, maxLength_bytes,((SQLClob *)operand1)->getDataCharSet(), Lob_Invalid_Storage,
		operand1->supportsSQLnull() OR
		operand2->supportsSQLnull() OR
		((operand3 != NULL) AND operand3->supportsSQLnull()),((SQLlob *)operand2)->extFormatLen());
    }
  else if (DFS2REC::isCharacterString(operand1->getFSDatatype()))
    {
      NABoolean bNullable = (operand1->supportsSQLnull() OR
                             operand2->supportsSQLnull() OR
...
const NAType *Trim::synthesizeType()
{
  ...
  if ( CharInfo::getCharSetEnum(cs) != CharInfo::DefaultCharSet
       && (new_cs == CharInfo::DefaultCharSet || new_cs == CharInfo::UnknownCharSet)
       && vid1.getItemExpr() && vid2.getItemExpr()
    ...
    case ITM_IS_NOT_NULL:
      {
        ...
        if (vid1.getItemExpr()->getOperatorType() == ITM_DYN_PARAM ||
            vid1.getItemExpr()->getOperatorType() == ITM_ROWSETARRAY_SCAN)
          {
            ...
            if (NOT (new_cs == CharInfo::ISO88591 ||
                     source_cs == CharInfo::UTF8 ||
                     source_cs == CharInfo::UCS2))
              vid1.coerceType(NA_CHARACTER_TYPE, new_cs, TRUE);
          } 
      }
     break;
  ...
  if (charsetTarget != CharInfo::UnknownCharSet)
    {
      Lng32 resultLen =
        CharInfo::getMaxConvertedLenInBytes(translateSource->getCharSet(),
                                            ...
                                            charsetTarget);
      NAType *desiredType = new HEAP SQLVarChar(HEAP, CharLenInfo(0, resultLen),
                                 TRUE, FALSE, 
                                 translateSource->isCaseinsensitive(),
                                 charsetTarget,
                                 CharInfo::DefaultCollation,
                                 CharInfo::IMPLICIT);	// ANSI 6.7 SR 5b
      if (translateSource->getFSDatatype() == REC_BYTE_F_ASCII)
         ((CharType *)desiredType)->setSpaceSensitive(FALSE);
  ...
  switch (getOperatorType())
  {
    case ITM_OLAP_PIVOT_GROUP:
      if (operand.getTypeQualifier() == NA_CHARACTER_TYPE)
      {
        ...
        CharLenInfo charLen(maxPivotLen_/CharInfo::minBytesPerChar(charType->getCharSet()),
                                    maxPivotLen_);
        result = new HEAP SQLVarChar(HEAP,
                                    charLen,
        ...
        result = new HEAP SQLVarChar(HEAP, maxPivotLen_,
                                               operand.supportsSQLnull(),
                                               FALSE, FALSE,
                                               CharInfo::ISO88591,
                                               CharInfo::DefaultCollation,
                                               CharInfo::COERCIBLE);
...
const NAType * HbaseVisibility::synthesizeType()
{
  ...
  type = new HEAP SQLVarChar(HEAP, HBASE_VISIBILITY_MAXLEN,  
...
Int32 LOBoper::computeLobInlineStorageLen(ItemExpr *op)
{
  if (! op)
    return 0;

  if (lobInlinedDataMaxLen() > 0)
    return lobInlinedDataMaxLen();

  if ((op->getOperatorType() == ITM_BASECOLUMN) ||
      (op->getOperatorType() == ITM_INDEXCOLUMN))
    return op->getValueId().getNAColumn()->lobInlinedDataMaxLen();
...
const NAType *LOBoper::synthesizeType()
{
  ...
  Lng32 inlineDataMaxLen = computeLobInlineStorageLen(child(0));
 
  NAType *result = new HEAP SQLBlob(HEAP, ((Int64) CmpCommon::getDefaultNumeric(LOB_MAX_SIZE)*1024*1024),Lob_Invalid_Storage,
				    TRUE, inlineDataMaxLen);
  ...
  if (child(0))
	...
	{
	  result = new HEAP SQLClob(HEAP, ((Int64) CmpCommon::getDefaultNumeric(LOB_MAX_SIZE)*1024*1024), ((SQLClob &)typ1).getDataCharSet(),Lob_Invalid_Storage,
				    typ1.supportsSQLnull(),
                                    inlineDataMaxLen);
	}
    } 
...
const NAType *LOBinsert::synthesizeType()
{
  ...
  if (lobFsType() == REC_BLOB)
    ...
    {            
      result = new HEAP SQLClob(HEAP, lobSize(), charset,Lob_Invalid_Storage,
				(obj_ == EMPTY_LOB_)? FALSE:typ1->supportsSQLnull(),
                                inlineDataMaxLen);
    }
...
const NAType *LOBupdate::synthesizeType()
{
  ...
  NAType * result = NULL;
  Lng32 inlineDataMaxLen = computeLobInlineStorageLen(child(1));
  if (typ2->getFSDatatype() == REC_BLOB)
    ...
    {
      if (obj_ !=EMPTY_LOB_)
        charset = typ2->getCharSet();
      SQLClob &clob = (SQLClob&)*typ2;

      result = new HEAP SQLClob(HEAP, clob.getLobLength(), charset,Lob_Invalid_Storage,
				(obj_ ==EMPTY_LOB_) ? FALSE:typ2->supportsSQLnull(),
                                inlineDataMaxLen);
    }
...
const NAType *LOBconvertHandle::synthesizeType()
{
  ...
  // Return blob type
  Lng32 inlineDataMaxLen = computeLobInlineStorageLen(op);

  ValueId vid1 = op->getValueId();
  const NAType &typ1 = (NAType&)vid1.getType();

  NABoolean lobV2 = FALSE;
  if ((op) &&
      ((op->getOperatorType() == ITM_BASECOLUMN) ||
       (op->getOperatorType() == ITM_INDEXCOLUMN)) &&
      (op->getValueId().getNAColumn()->getNATable() &&
       op->getValueId().getNAColumn()->getNATable()->lobV2()))
    lobV2 = TRUE;
  ...
  if (obj_ == STRING_)
      ...
      if (typ1.getFSDatatype() == REC_BLOB)
	{
	  ...
	  result = new HEAP SQLBlob(HEAP, op1.getLobLength(), Lob_Invalid_Storage,
				    typ1.supportsSQLnull(), inlineDataMaxLen,
                                    lobV2
				    );
	}
      else if (typ1.getFSDatatype() == REC_CLOB)
	{
	  ...
	  result = new HEAP SQLClob(HEAP, op1.getLobLength(), op1.getDataCharSet(),Lob_Invalid_Storage,
				    typ1.supportsSQLnull(), inlineDataMaxLen,
                                    lobV2
				    );
	}
 ...
 else if (obj_ == HANDLE_STRING_WITH_DATA_)
    {
      inlineDataMaxLen = vid1.getNAColumn()->lobInlinedDataMaxLen();
      if (typ1.getTypeQualifier() != NA_LOB_TYPE)
	{
	  // 4043 The operand of a $0~String0 function must be character.
	  *CmpCommon::diags() << DgSqlCode(-4221) << DgString0("LOBCONVERTHANDLE")
      ...
      if (typ1.getFSDatatype() == REC_BLOB)
	{
	  ...
	  result = new HEAP SQLBlob
            (
                 HEAP, op1.getLobLength(), Lob_Invalid_Storage,
                 typ1.supportsSQLnull(), inlineDataMaxLen,
                 lobV2
             );
	}
      else if (typ1.getFSDatatype() == REC_CLOB)
	{
	  ...
	  result = new HEAP SQLClob
            (
                 HEAP, op1.getLobLength(), op1.getDataCharSet(),Lob_Invalid_Storage,
                 typ1.supportsSQLnull(), inlineDataMaxLen,
                 lobV2
             );
	}
     ...
     if (typ1.getTypeQualifier() != NA_CHARACTER_TYPE)
	{
     ...
     result = new HEAP SQLBlob(HEAP, ((Int64) CmpCommon::getDefaultNumeric(LOB_MAX_SIZE)*1024*1024), 
                               Lob_Invalid_Storage, typ1.supportsSQLnull(),
                               inlineDataMaxLen);
      return result;
    }
  else if (obj_ == HANDLE_STRING_AS_CHAR_)
    {
      NAType *result = new HEAP SQLVarChar(HEAP, LOB_HANDLE_LEN,
					   typ1.supportsSQLnull());
      return result;
    }
  else
  ...
  if (obj_ == STRING_ || obj_ == FILE_)
      ...
      if (typ1.getTypeQualifier() != NA_LOB_TYPE)
	{
	  ...
	  *CmpCommon::diags() << DgSqlCode(-4221) << DgString0("LOBCONVERT")
      ...
      CharInfo::CharSet childCharset = typ1.getCharSet();
      if (typ1.getFSDatatype() == REC_CLOB)
        {
          childCharset = ((SQLClob &)typ1).getDataCharSet();
          tgtSize = tgtSize * CharInfo::maxBytesPerChar(childCharset);
        }
      NAType *result = new HEAP SQLVarChar(HEAP, tgtSize,
					   typ1.supportsSQLnull(),1,0,childCharset);
...
const NAType *LOBextract::synthesizeType()
{
  ...
  if (CmpCommon::getDefaultString(DEFAULT_CHARSET) == "UTF8")
    cs = CharInfo::UTF8;
  NAType *result = new HEAP SQLVarChar(HEAP, tgtSize,
				       typ1.supportsSQLnull(),1,0,cs);
  return result;
}

const NAType *LOBlength::synthesizeType()
{
  // Return largeint
  ...
  if (typ1.getTypeQualifier() != NA_LOB_TYPE)
    {
      // 4043 The operand of a $0~String0 function must be blob
      *CmpCommon::diags() << DgSqlCode(-4221) << DgString0("LOBLENGTH")
			  << DgString1("LOB");
      return NULL;
    }
  
  NAType *result = new HEAP SQLLargeInt(HEAP, TRUE, typ1.supportsSQLnull());
  return result;
}
...
const NAType * JsonExistsFunc::synthesizeType()
{
    NAType * retType = NULL;
    ...
    CharType *typ1_p = (CharType *) &typ1;
    if (CharInfo::UnknownCharSet != charSet_ && charSet_ != typ1_p->getCharSet())
    {
        *CmpCommon::diags() << DgSqlCode(-8988) << DgString0(CharInfo::getCharSetName(charSet_));
        return NULL;
    }

    retType = new HEAP SQLBooleanNative(HEAP, TRUE);

    return retType;
}

const NAType * JsonValueFunc::synthesizeType()
{
    NAType * retType = NULL;
    ...
    Lng32 res_len_in_Chars = typ1_p->getStrCharLimit();
    Lng32 res_nominalSize = typ1_p->getNominalSize();

    CharLenInfo res_CharLenInfo(res_len_in_Chars, res_nominalSize);

    retType = new HEAP SQLVarChar(HEAP, res_CharLenInfo, TRUE, FALSE, FALSE,
                                         typ1_p->getCharSet(), typ1_p->getCollation(),
                                         typ1_p->getCoercibility());

    return retType;
}

const NAType * JsonQueryFunc::synthesizeType()
{
    NAType * retType = NULL;
    ...
    if (JSON_WITHOUT_WRAPPER != behaviorWrap_ && res_len_in_Chars > 0)
        res_nominalSize += 2;
...
const NAType *BeginKey::synthesizeType()
{
  ...
  return new HEAP SQLVarChar(HEAP
                             ...
                             , char_type->isUpshifted()
                             , char_type->isCaseinsensitive()
