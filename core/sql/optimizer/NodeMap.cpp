...
NodeMap::copy() const
{
  return new (heap_) NodeMap(*this,heap_);
} // NodeMap::copy()
		       ...
		       Generator *generator)
{
  ...
  for (Lng32 i = 0; i < numESPs; i++)
    {    
      ...
      int nodeId = ( CURRCONTEXT_CLUSTERINFO->hasVirtualNodes() ) ? 
                0 : ne->getNodeNumber();

      rc = msg_mon_get_node_info_detail(nodeId, &nodeInfo);

      strcpy(clusterName, nodeInfo.node[0].node_name);
      exeNodeMap->setEntry(i,
                           clusterName,
                           nodeId,
                           ne->isPartitionActive(),
                           space);
    }
                              ...
                              int balanceLevel)
{
  ...
  Int32 numSQNodes = CURRCONTEXT_CLUSTERINFO->getTotalNumberOfCPUs();
  Int32 nextDefaultPartNum = numESPs/2;

  usesLocality_ =
    (useLocalityForHiveScanInfo() &&
     hiveSearchKey->getHDFSTableStats()->blocksAreOnLocalCluster());
  ...
  while (hiveSearchKey->getNextFile(i))
    {
      p = (HHDFSPartitionStats*)i.getPartStats();
                              ...
                              for (Int32 r=0;
                                   ...
                                   r++)
                                {
                                  ...
                                  if (replicaHost == (HostId) recipientHost ||
                                      replicaHost == HHDFSFileStats::InvalidHostId ||
...
NodeMap::getText() const
{
  ...
  for (ULng32 nodeIdx = 0; nodeIdx < map_.entries(); nodeIdx++) {
    ...
    if (numNodesInRange == 1 &&
        nn >= lastNodeNumber)
      stride = nn - lastNodeNumber;

    if (numNodesInRange > 0 &&
        stride >= 0 &&
        lastNodeNumber + stride == nn)
      {
