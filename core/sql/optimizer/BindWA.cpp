...
// BindWA()
// ***********************************************************************
BindWA::BindWA(SchemaDB *schemaDB, CmpContext* cmpContext, NABoolean inDDL, NABoolean allowExtTables, NAHeap* workingHeap)
     : wHeap_(workingHeap)
...
// ***********************************************************************
ColumnNameMap *BindWA::findCompositeColumn(ColReference *colRef, 
                                           RETDesc *resultTable)
{
  ColumnNameMap *result = NULL;
  if (colRef->isCompCol() && colRef->isResolved())
    return NULL;
      ...
      if (result)
        {
          ...
          if ((colRef->isCompCol()) && (NOT colRef->isResolved()) &&
              (colRef->getCompArrIndexes().entries() >= 2))
            {
              indexes.insert(colRef->getCompArrIndexes()
                             [colRef->getCompArrIndexes().entries()-2]);
              indexes.insert(colRef->getCompArrIndexes()
                             [colRef->getCompArrIndexes().entries()-1]);
            }
          else
            {
              indexes.insert(0);
  ...
  if (!result && colRef->isCompCol() && (NOT colRef->isResolved()) &&
      (colRef->getCompArrIndexes().entries() >= 1))
    {
      // case of <c.s.t>.a[1]
  ...
  if (result)
    {
      colRef->setCompColInfo(TRUE, TRUE,
                             1, numTotalParts,
                             names, indexes);
    }
...
ColumnNameMap *BindWA::findColumn(ColReference *colRef, 
                                  BindScope *&bindScope)
{
  const ColRefName &colRefName = colRef->getColRefNameObj();
  ...
  while (bindScope) {
    ...
    if (resultTable) {
      ...
      if ((result) && ((NOT colRef->isCompCol()) || (colRef->isResolved())))
        return result;
...
short BindWA::validateNumTables(RelExpr * node)
{
  if (node->getOperatorType() == REL_SCAN)
    {
      Scan * scan = (Scan*)node;
      if ((scan->getTableName().isSeabaseMD()) ||
          (scan->getTableName().isSeabasePrivMgrMD()))
        return 0;
      ...
      if ((v >= 0) &&
        ...
        {
          char errBuf[1000];
          sprintf(errBuf, "Number of tables(%d) exceed the maximum allowed tables(%d) in a query.",
                  numTablesPerQuery(),
