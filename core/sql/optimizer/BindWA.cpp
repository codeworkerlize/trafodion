...
BindWA::BindWA(SchemaDB *schemaDB, CmpContext *cmpContext, NABoolean inDDL, NABoolean allowExtTables,
    ...
    : wHeap_(workingHeap),
      ...
      udrStoiList_(wHeap()),
      seqValList_(wHeap()),
      defaultSchema_(wHeap()),
      RETDescList_(wHeap()),
      tableViewUsageList_(wHeap()),
      inDDL_(inDDL)
      //     , inIndexMaint_(FALSE)
      //     , inRIMaint_(FALSE)
      ,
      ...
      flags_(0),
      udfList_(wHeap()),
      numTablesPerSelect_(0),
      numTablesPerQuery_(0),
      brLockedObjectsAccess_(0),
      extendedSelList_(workingHeap),
      recodeExtendSelectList_(FALSE) {
  ...
  hcui_ = new (wHeap()) HbaseColUsageInfo(wHeap());
  inSubqueryStartWith_ = FALSE;
...
// ***********************************************************************
BindWA::~BindWA() {
  if (!wHeap_) inputVars_.clearAndDestroy();
...
void BindWA::initNewScope() {
  ...
  if (i) {
    ...
    newScope->context()->inUpdateOrInsert() = scopes_[i - 1]->context()->inUpdateOrInsert();
...
BindScope *BindWA::findNextScopeWithTriggerInfo(BindScope *currentScope) {
  ...
  while ((currentScope != NULL) && (currentScope->context()->triggerObj() == NULL))
    currentScope = getPreviousScope(currentScope);
...
******************************************************************************/
void BindWA::addUninitializedMv(const char *physicalName, const char *ansiName) {
  UninitializedMvName *pMvName = new (wHeap()) UninitializedMvName;

  // first use?
  if (NULL == uninitializedMvList_) {
    uninitializedMvList_ = new (wHeap()) UninitializedMvNameList(wHeap(), 1);
  }
...
void BindWA::removeCurrentScope(NABoolean keepLocalRefs) {
  ...
  if (NOT scopes_.isEmpty()) getCurrentScope()->mergeOuterRefs(currScope->getOuterRefs(), keepLocalRefs);
...
// BindWA::findColumn()
//
// The first/second methods searches for the given name in the BindScopes.
...
//
// The third findColumn() locates the xcnmEntry to a ColumnDesc
// known to exist in the current scope (no backward searching thru scopes!).
// ***********************************************************************
ColumnNameMap *BindWA::findCompositeColumn(ColReference *colRef, RETDesc *resultTable) {
  ColumnNameMap *result = NULL;
  if (colRef->isCompCol() && colRef->isResolved()) return NULL;

  const ColRefName &colRefName = colRef->getColRefNameObj();
  const QualifiedName &qn = colRefName.getCorrNameObj().getQualifiedNameObj();

  NAList<NAString> names(CmpCommon::statementHeap());
  NAList<UInt32> indexes(CmpCommon::statementHeap());

  UInt32 numTotalParts = qn.numberExpanded() + 1;
  if (!result && (NOT qn.getObjectName().isNull())) {
    CorrName corrName(qn.getSchemaName(), NULL, qn.getCatalogName());
    ColRefName crn(qn.getObjectName(), corrName);
    result = resultTable->findColumn(crn);
    if (result) {
      names.insert(crn.getColName());
      names.insert(colRefName.getColName());

      if ((colRef->isCompCol()) && (NOT colRef->isResolved()) && (colRef->getCompArrIndexes().entries() >= 2)) {
        indexes.insert(colRef->getCompArrIndexes()[colRef->getCompArrIndexes().entries() - 2]);
        indexes.insert(colRef->getCompArrIndexes()[colRef->getCompArrIndexes().entries() - 1]);
      } else {
        indexes.insert(0);
        indexes.insert(0);
      }
  ...
  if (!result && (NOT qn.getSchemaName().isNull())) {
    CorrName corrName(qn.getCatalogName());
    ColRefName crn(qn.getSchemaName(), corrName);
    result = resultTable->findColumn(crn);
    if (result) {
      names.insert(crn.getColName());
      names.insert(qn.getObjectName());
      names.insert(colRefName.getColName());
    }
  }

  if (!result && (NOT qn.getCatalogName().isNull())) {
    CorrName corrName;
    ColRefName crn(qn.getCatalogName(), corrName);
    result = resultTable->findColumn(crn);
    if (result) {
      names.insert(crn.getColName());
      names.insert(qn.getSchemaName());
      names.insert(qn.getObjectName());
      names.insert(colRefName.getColName());
    }
  }

  if (!result && colRef->isCompCol() && (NOT colRef->isResolved()) && (colRef->getCompArrIndexes().entries() >= 1)) {
    // case of <c.s.t>.a[1]
    result = resultTable->findColumn(colRefName);
    if (result) {
      names.insert(colRefName.getColName());
      indexes.insert(colRef->getCompArrIndexes()[colRef->getCompArrIndexes().entries() - 1]);
  ...
  if (result) {
    colRef->setCompColInfo(TRUE, TRUE, 1, numTotalParts, names, indexes);
  }

  return result;
}

ColumnNameMap *BindWA::findColumn(ColReference *colRef, BindScope *&bindScope) {
  const ColRefName &colRefName = colRef->getColRefNameObj();
  ...
  while (bindScope) {
    ...
    if (resultTable) {
      ...
      if ((result) && ((NOT colRef->isCompCol()) || (colRef->isResolved()))) return result;

      // this could be a composite column. Look for it.
      ColumnNameMap *compResult = findCompositeColumn(colRef, resultTable);
      if (compResult) {
        ValueId valId = compResult->getValueId();
        if (valId.getType().isComposite()) return compResult;

        colRef->resetCompColInfo();
      }
    }
...
ColumnNameMap *BindWA::findColumn(const ColumnDesc &columnDesc) {
  ...
  if (resultTable) result = resultTable->findColumn(columnDesc.getColRefNameObj());
  CMPASSERT(result);
  return result;
}  // BindWA::findColumn() the third
...
// ***********************************************************************
ColumnDescList *BindWA::findCorrName(const CorrName &corrName, BindScope *&bindScope) {
  bindScope = getCurrentScope();
  while (bindScope) {
    RETDesc *resultTable = bindScope->getRETDesc();
    if (resultTable) {
      ColumnDescList *result = resultTable->getQualColumnList(corrName);
      if (result) return result;  // This list may have zero entries (SYSKEY the only
...
StmtLevelAccessOptions *BindWA::findUserSpecifiedAccessOption() {
  ...
  while (bindScope) {
    ...
    if (context) {
      StmtLevelAccessOptions *axOpts = CONST_CAST(StmtLevelAccessOptions *, context->stmtLevelAccessOptions());
...
// to decide whether column is in user column list
NABoolean BindWA::isPotentialUserColumn(ColReference *colRef) {
  const NAString &colName = colRef->getColRefNameObj().getColName();
  BindScope *bindScope = getCurrentScope();
  ...
  while (bindScope) {
    RETDesc *resultTable = bindScope->getRETDesc();
    if (resultTable) {
      result = resultTable->isPotentialUserColumn(colName);
...
// Same search strategy as the first findColumn().
// ***********************************************************************
void BindWA::getTablesInScope(LIST(TableNameMap *) & xtnmList, NAString *formattedList) const {
  xtnmList.clear();
...
void BindScope::getTablesInScope(LIST(TableNameMap *) & xtnmList, NAString *formattedList) const {
  xtnmList.clear();
...
ValueId BindScope::addUnresolvedAggregate(ValueId newAggrId) {
  ...
  if (newAggr) {
    ...
    //
    for (ValueId aggrId = unresolvedAggregates_.init(); unresolvedAggregates_.next(aggrId);
...
ValueId BindScope::getEquivalentItmSequenceFunction(ValueId newSeqId) {
  ...
  if (newSeq) {
    //
    for (ValueId seqId = unresolvedSequenceFunctions_.init(); unresolvedSequenceFunctions_.next(seqId);
...
void BindWA::setDefaultSchema(const SchemaName &defaultSchema) { defaultSchema_ = defaultSchema; }
...
StmtDDLCreateView *BindWA::getCreateViewParseNode() const {
  if (getUsageParseNodePtr()) {
    return getUsageParseNodePtr()->castToElemDDLNode()->castToStmtDDLCreateView();
  }
  return NULL;
}
...
NABoolean BindWA::inCheckConstraintDefinition() const {
  return getUsageParseNodePtr() && getUsageParseNodePtr()->getOperatorType() == DDL_ALTER_TABLE_ADD_CONSTRAINT_CHECK;
}

NABoolean BindWA::inViewDefinition() const {
  return getUsageParseNodePtr() && getUsageParseNodePtr()->getOperatorType() == DDL_CREATE_VIEW;
}

NABoolean BindWA::inMVDefinition() const {
  return getUsageParseNodePtr() && getUsageParseNodePtr()->getOperatorType() == DDL_CREATE_MV;
}
...
ItemExpr *BindWA::getHVorDPFromSPDups(ItemExpr *h) {
  ...
  const NAString &name1 =
      (h->getOperatorType() == ITM_HOSTVAR) ? ((HostVar *)h)->getName() : ((DynamicParam *)h)->getName();
  ...
  for (CollIndex i = 0; i < numEntries; i++) {
    ...
    const NAString &name2 =
        (expr->getOperatorType() == ITM_HOSTVAR) ? ((HostVar *)expr)->getName() : ((DynamicParam *)expr)->getName();

    ComColumnDirection mode2 = (expr->getOperatorType() == ITM_HOSTVAR) ? ((HostVar *)expr)->getParamMode()
                                                                        : ((DynamicParam *)expr)->getParamMode();

    if (name1.compareTo(name2) == 0 && mode1 == mode2) return expr;
  }
  return NULL;
}

NABoolean BindWA::checkHVorDPinSPDups(ItemExpr *h) {
  if (NULL == getHVorDPFromSPDups(h)) return FALSE;
...
NABoolean BindWA::checkMultiOutSPParams(ItemExpr *h) {
  ComColumnDirection mode =
      (ITM_HOSTVAR == h->getOperatorType()) ? ((HostVar *)h)->getParamMode() : ((DynamicParam *)h)->getParamMode();

  if (COM_INPUT_COLUMN == mode) return FALSE;

  CollIndex numEntries = spOutParams_.entries();

  for (CollIndex i = 0; i < numEntries; i++) {
    const NAString &s1 =
        (ITM_HOSTVAR == h->getOperatorType()) ? ((HostVar *)h)->getName() : ((DynamicParam *)h)->getName();
    ...
    if ((s1.compareTo(s2) == 0) && (s1.compareTo("") != 0)) return TRUE;
  }
...
void BindWA::setColumnRefsInStoi(const char *fileName, int colPosition) {
  ...
  if ((!isBindingMvRefresh()) &&
      ((!inViewExpansion()) || (inViewExpansion() && getCurrentScope()->context()->inWhereClause()))) {
    ...
    for (CollIndex ii = 0; ii < getStoiList().entries(); ii++) {
      if (strcmp((getStoiList())[ii]->getStoi()->fileName(), fileName) == 0) {
...
RelExpr *HostArraysWA::modifyTupleNode(RelExpr *node) {
  ...
  if (hasHostArrays()) {
    ...
    RelExpr *newRowSet = new (bindWA_->wHeap()) Rowset(listOfHostArrays_, indexExpr_, inputSizeExpr_);
    ...
    // Create new Rowset node
    RelExpr *newRowSet = new (bindWA_->wHeap()) RowsetRowwise(node);
...
void HostArraysWA::processArrayHostVar(ItemExpr *parent, int childNumber) {
  ...
  if (child->getOperatorType() == ITM_DYN_PARAM) {
    int inputArrayMaxSize;
    if (((inputArrayMaxSize = getInputArrayMaxSize()) > 0) || (((DynamicParam *)child)->getRowsetSize() > 0)) {
      ...
      SQLRowset *rowsetType = new (bindWA_->wHeap())
          SQLRowset(bindWA_->wHeap(), (new (bindWA_->wHeap()) SQLUnknown(bindWA_->wHeap())), size, size);
      ...
      if (!(param->getIndicatorName().isNull())) {
        node = new (bindWA_->wHeap()) HostVar(name, param->getIndicatorName(), rowsetType);
      } else {
  ...
  ItemExpr *ptr =
      new (bindWA_->wHeap()) ColReference(new (bindWA_->wHeap()) ColRefName(strcat(tmp2, tmp1), bindWA_->wHeap()));
...
void HostArraysWA::createNewNames() {
  ...
  for (int i = 0; tmp; i++, tmp = tmp->child(1)) {
    ...
    ItemExpr *tempExpr =
        new (heap) ItemList(new (heap) RenameCol(NULL, new (heap) ColRefName(strcat(tmp2, tmp1), heap)), NULL);
  ...
  if (indexExpr_) {
    ...
    ItemExpr *tempExpr = new (heap) ItemList(new (heap) RenameCol(NULL, name2), NULL);
...
short BindWA::validateNumTables(RelExpr *node) {
  if (node->getOperatorType() == REL_SCAN) {
    Scan *scan = (Scan *)node;
    if ((scan->getTableName().isSeabaseMD()) || (scan->getTableName().isSeabasePrivMgrMD())) return 0;
    ...
    if ((v >= 0) && (numTablesPerSelect() > v)) {
      ...
      sprintf(errBuf, "Number of tables(%d) exceed the maximum allowed tables(%d) for each select.",
              numTablesPerSelect(), (int)CmpCommon::getDefaultNumeric(TRAF_MAX_TABLES_PER_SELECT));
      *CmpCommon::diags() << DgSqlCode(-3242) << DgString0(errBuf);
      setErrStatus();
      return -1;
    }

    v = CmpCommon::getDefaultNumeric(TRAF_MAX_TABLES_PER_QUERY);
    if ((v >= 0) && (numTablesPerQuery() > v)) {
      char errBuf[1000];
      sprintf(errBuf, "Number of tables(%d) exceed the maximum allowed tables(%d) in a query.", numTablesPerQuery(),
              (int)CmpCommon::getDefaultNumeric(TRAF_MAX_TABLES_PER_QUERY));
      *CmpCommon::diags() << DgSqlCode(-3242) << DgString0(errBuf);
      setErrStatus();
      return -1;
    }
...
void HostArraysWA::collectHostVarsInRelExprTree(RelExpr *root, RelExpr::AtomicityType atomicity) {
  ...
  if (bindWA_->validateNumTables(root)) {
    bindWA_->setErrStatus();
    return;
  }
  ...
  if (root->getOperatorType() == REL_UNARY_UPDATE) {
    ...
    if (node->isMerge()) {
      ...
      ItemExpr *insertClause =
          (node->isMergeUpdate() ? ((MergeUpdate *)node)->insertValues() : ((MergeDelete *)node)->insertValues());
  ...
  if ((root->getOperatorType() == REL_ROWSET) || (root->getOperatorType() == REL_ROWSET_INTO)) {
  ...
  if (root->getOperatorType() == REL_UNARY_INSERT) {
    if ((CmpCommon::getDefault(ODBC_PROCESS) == DF_ON) || (CmpCommon::getDefault(JDBC_PROCESS) == DF_ON)) {
      ...
      if (root->getTolerateNonFatalError() == RelExpr::UNSPECIFIED_) root->setTolerateNonFatalError(atomicity);
    }
  }
...
RelExpr *HostArraysWA::modifyTree(RelExpr *queryExpr, RelExpr::AtomicityType atomicity) {
  ...
  if (queryExpr->child(0)->getOperatorType() == REL_ROWSETFOR) {
    ...
    tempVar->getBufferAttributes(packedFormat_, compressed_, dcompressInMaster_, compressInMaster_, partnNumInBuffer_);
  ...
  if (bindWA_->errStatus()) return queryExpr;
  ...
  if ((listOfHostArrays_) && (!hasHostArraysInTuple())) {
    ...
    RelExpr *newRoot1 =
        new (bindWA_->wHeap()) RelRoot(tempTree->child(0), ((RelRoot *)queryExpr)->accessOptions().accessType(),
                                       ((RelRoot *)queryExpr)->accessOptions().lockMode(), REL_ROOT, NULL, NULL, NULL);
    ...
    ((RelRoot *)newRoot1)->addOrderByTree(((RelRoot *)tempTree)->removeOrderByTree());
    ...
    RelExpr *newJoin = new (bindWA_->wHeap()) Join(newRename, newRoot1, REL_TSJ, NULL);
  ...
  if ((getRowwiseRowset()) && (atomicity == RelExpr::NOT_ATOMIC_)) {
...
// ***********************************************************************
CollIndex BindWA::addTrigger(const ComTimestamp &triggerId) {
  // first use?
  if (triggersList_ == NULL) triggersList_ = new (wHeap()) LIST(ComTimestamp)(wHeap(), 1);
...
RelExpr *RelExpr::xformRowsetsInTree(BindWA &wa, const int arrayMaxsize, const AtomicityType atomicity) {
...
RelExpr *RelRoot::xformRowsetsInTree(BindWA &wa, const int arrayMaxsize, const AtomicityType atomicity) {
  ...
  if (child(0)) {
    switch (child(0)->getOperatorType()) {
      ...
      default: {
        ...
        RelExpr *newExpr = hostArraysArea_->modifyTree(this, atomicity);  // open sesame!
...
void BindWA::initializeOverrideSchema() {
  ...
  if ((!osFromSchema_.isNull()) && (!osToSchema_.isNull()) && (osFromSchema_ != osToSchema_))
...
void extractOverrideSchemas(const char *value, NAString &fromSchema, NAString &toSchema) {
      ...
      (strlen(value) > (size_t)len + 1))  // value long enough for at least one more chr for toSch
  {
    ...
    if ((len > 0) && (len <= ComMAX_1_PART_EXTERNAL_UTF8_NAME_LEN_IN_BYTES)) {
      ...
      if (fromSchema(0) != '\"') fromSchema.toUpper();
      if (toSchema(0) != '\"') toSchema.toUpper();
    }
  }
...
NABoolean BindWA::violateAccessDefaultSchemaOnly(const QualifiedName &objName) {
  ...
  if (!cat.isNull() && !sch.isNull()) {
    ...
    if ((cat == "MANAGEABILITY") || (cat == "HP_SYSTEM_CATALOG") || (sch == "HP_METRICS") ||
        (sch == "HP_DEFINITION_SCHEMA"))
      return FALSE;

    // let volatile objects pass
    if (!strncmp(sch.data(), "VOLATILE_SCHEMA_MX", 18)) return FALSE;

    SchemaName objSchName(sch, cat);
    if (objSchName.matchDefaultPublicSchema()) return FALSE;
  }
...
NABoolean BindWA::queryCanUseSeaMonster() {
  ...
  if (queryCanUseSeaMonster_ == -1) {
    ...
    if (CmpCommon::getDefault(SEAMONSTER) == DF_ON) {
      ...
      if (e && e[0] == '1') queryCanUseSeaMonster_ = 1;
    }
  }
...
//----------------------------------------------------------------------
int BindWA::qualNameHashFunc(const QualifiedName &qualName) {
  int index = 0;
