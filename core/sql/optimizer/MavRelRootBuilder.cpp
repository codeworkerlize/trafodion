...
//----------------------------------------------------------------------------
//
MavRelRootBuilder::MavRelRootBuilder(const MVInfoForDML *mvInfo, CollHeap *heap)
    : heap_(heap),
...
// pointers.
MavRelRootBuilder::~MavRelRootBuilder() { extraMinMaxColRefs_.clear(); }
...
//////////////////////////////////////////////////////////////////////////////
RelExpr *MavRelRootBuilder::buildDeltaCalculationRootNodes(RelExpr *mvSelectTree, NABoolean canSkipMinMax,
                                                           NABoolean wasFullDE) {
  checkForMavWithoutGroupBy(mvSelectTree);
  fixDeltaColumns(mvSelectTree, canSkipMinMax, wasFullDE);

  RelExpr *topNode = new (heap_) RenameTable(mvSelectTree, MavBuilder::getSysDeltaName());

  if (isMavWithoutGroupBy()) topNode = buildRootForNoGroupBy(topNode);
...
void MavRelRootBuilder::checkForMavWithoutGroupBy(RelExpr *mvSelectTree) {
  ...
  while (node->child(0) != NULL && node->getOperatorType() != REL_GROUPBY) node = node->child(0);
  ...
  if (node->getOperatorType() != REL_GROUPBY)
    ...
    if ((node->child(0)->getOperatorType() == REL_ROOT) &&
      ...
      if (groupByNode->getGroupExprTree() == NULL) isMavWithoutGroupBy_ = TRUE;
    }
  }
...
// in case COUNT=0.
void MavRelRootBuilder::fixDeltaColumns(RelExpr *mvSelectTree, NABoolean canSkipMinMax, NABoolean wasFullDE) {
  ...
  for (CollIndex i = 0; i < mavSelectList.entries(); i++) {
    ...
    const NAString &colName = ((RenameCol *)selectListExpr)->getNewColRefName()->getColName();
    ItemExpr *aggregateExpr = selectListExpr;
    while ((aggregateExpr->getOperatorType() == ITM_RENAME_COL) || (aggregateExpr->getOperatorType() == ITM_CAST))
    ...
    if (!aggregateExpr->containsAnAggregate()) {
      ...
      switch (aggregateExpr->getOperatorType()) {
        case ITM_COUNT_NONULL:
          // COUNT(a) => SUM(IF (a IS NULL) THEN 0 ELSE @OP)
          newExpr = new (heap_) Aggregate(
              ITM_SUM,
              new (heap_) Case(NULL, new (heap_) IfThenElse(new (heap_) UnLogic(ITM_IS_NULL, aggregateOperand),
        ...
        case ITM_COUNT:
          // COUNT(*) => SUM(@OP)
          newExpr = new (heap_) Aggregate(ITM_SUM, new (heap_) ColReference(new (heap_) ColRefName(opName)));
          selectListExpr->child(0) = newExpr;
          break;
        ...
        case ITM_SUM:
          // SUM(a) => SUM(a * @OP)
          newExpr = new (heap_)
              BiArith(ITM_TIMES, aggregateOperand, new (heap_) ColReference(new (heap_) ColRefName(opName)));
        ...
        case ITM_MAX:
          ...
          if (!canSkipMinMax) handleDeltaMinMaxColumns(aggregateExpr, colName, newSelectList, wasFullDE);
    ...
    if (selectListExpr != NULL) newSelectList.insert(selectListExpr);
  }
...
//----------------------------------------------------------------------------
//----------------------------------------------------------------------------
void MavRelRootBuilder::handleDeltaMinMaxColumns(ItemExpr *aggregateExpr, const NAString &colName,
                                                 ItemExprList &newSelectList, NABoolean wasFullDE) {
  // 1. Add the column expressions to the MAV select list
  ItemExpr *extraInsCol = buildExtraMinMaxExpr(aggregateExpr, colName, AGG_FOR_INSERT);
  newSelectList.insert(extraInsCol);

  ItemExpr *extraDelCol = buildExtraMinMaxExpr(aggregateExpr, colName, AGG_FOR_DELETE);
  newSelectList.insert(extraDelCol);

  // 2. Create column references to the new columns.
  ColReference *colInsRef = new (heap_) ColReference(createExtraAggName(colName, AGG_FOR_INSERT));
  extraMinMaxColRefs_.insert(colInsRef->copyTree(heap_));
  ColReference *colDelRef = new (heap_) ColReference(createExtraAggName(colName, AGG_FOR_DELETE));
  ...
  addToMinMaxRecomputeCondition(colName, colInsRef, colDelRef, aggregateExpr->getOperatorType(), wasFullDE);
...
RelRoot *MavRelRootBuilder::buildRootForNoGroupBy(RelExpr *topNode) {
  ...
  for (CollIndex i = 0; i < countCols_.entries(); i++) {
    ...
    ItemExpr *sysDeltaColExpr = new (heap_) ColReference(new (heap_) ColRefName(colName, deltaCorrName_));

    ItemExpr *condExpr =
        new (heap_) Case(NULL, new (heap_) IfThenElse(new (heap_) UnLogic(ITM_IS_NULL, sysDeltaColExpr),
                                                      new (heap_) SystemLiteral(0), sysDeltaColExpr->copyTree(heap_)));

    ItemExpr *renamedColExpr = new (heap_) RenameCol(condExpr, new (heap_) ColRefName(colName, deltaCorrName_));
  ...
  RelRoot *newRoot = new (heap_) RelRoot(topNode, REL_ROOT, selectList.convertToItemExpr());
...
RelRoot *MavRelRootBuilder::buildRootForDelta(RelExpr *topNode) {
  ...
  for (CollIndex i = 0; i < otherCols_.entries(); i++) {
    ...
    switch (currentMavColumn->getOperatorType()) {
      case ITM_AVG:
        newColExpr = new (heap_) BiArith(ITM_DIVIDE, sumDepCol, countDepCol);
        break;
      ...
      case ITM_VARIANCE_POP:
        ...
        newColExpr = new (heap_)
            ScalarVariance(currentMavColumn->getOperatorType(), new (heap_) Cast(sum2DepCol, desiredType),
                           new (heap_) Cast(sumDepCol, desiredType), new (heap_) Cast(countDepCol, desiredType));
        break;
    ...
    ItemExpr *condExpr = new (heap_)
        Case(NULL, new (heap_) IfThenElse(new (heap_) BiRelat(ITM_EQUAL, countDepCol, new (heap_) SystemLiteral(0)),
                                          new (heap_) SystemLiteral(), newColExpr));
  ...
  // The select list is ready. Create the root over topNode.
  RelRoot *newRoot = new (heap_) RelRoot(topNode, REL_ROOT, newColumns.convertToItemExpr());
...
RelRoot *MavRelRootBuilder::buildRootForCount(RelExpr *topNode) {
  ...
  for (CollIndex i = 0; i < countCols_.entries(); i++) {
    ...
    ItemExpr *sysMavColExpr = new (heap_) ColReference(new (heap_) ColRefName(colName, mavCorrName_));

    ItemExpr *newCalcExpr = new (heap_) BiArith(ITM_PLUS, sysDeltaColExpr, sysMavColExpr);
...
RelRoot *MavRelRootBuilder::buildRootForSum(RelExpr *topNode) {
  ...
  for (CollIndex i = 0; i < sumCols_.entries(); i++) {
    ...
    ItemExpr *calcDep1 = buildDepColExpr(calcCorrName_, currentCol->getDepCol1());
    ...
    if (currentCol->getOperatorType() == ITM_SUM) {
      ...
      elseClause = new (heap_) BiArith(ITM_PLUS, sysDeltaColExpr, sysMavColExpr);
    } else {
      // The ELSE clause for MIN is:
      //    WHEN (SYS_MAV.col < SYS_DELTA.col) THEN SYS_MAV.col
      //    ELSE SYS_DELTA.col
      // For MAX the < operator is replaced by >.
      OperatorTypeEnum sign = (currentCol->getOperatorType() == ITM_MIN ? ITM_LESS : ITM_GREATER);
      elseClause = new (heap_)
          IfThenElse(new (heap_) BiRelat(sign, sysMavColExpr, sysDeltaColExpr), sysMavColExpr, sysDeltaColExpr);
    }

    // The 3rd WHEN clause: WHEN (SYS_DELTA.col IS NULL) THEN SYS_MAV.col
    ItemExpr *when3Clause =
        new (heap_) IfThenElse(new (heap_) UnLogic(ITM_IS_NULL, sysDeltaColExpr), sysMavColExpr, elseClause);

    // The 2nd WHEN clause: WHEN (SYS_MAV.col IS NULL) THEN SYS_DELTA.col
    ItemExpr *when2Clause =
        new (heap_) IfThenElse(new (heap_) UnLogic(ITM_IS_NULL, sysMavColExpr), sysDeltaColExpr, when3Clause);

    // The 1st WHEN clause: WHEN (SYS_CALC.dep1 = 0) THEN NULL
    ItemExpr *when1Clause =
        new (heap_) IfThenElse(new (heap_) BiRelat(ITM_EQUAL, calcDep1, new (heap_) SystemLiteral(0)),
...
RelRoot *MavRelRootBuilder::buildRootForOthers(RelExpr *topNode) {
  ...
  for (CollIndex i = 0; i < otherCols_.entries(); i++) {
    ...
    ItemExpr *calcDep1 = buildDepColExpr(calcCorrName_, currentCol->getDepCol1());
    ItemExpr *calcDep2 = buildDepColExpr(calcCorrName_, currentCol->getDepCol2());
    ...
    if (currentCol->getOperatorType() == ITM_AVG) {
      ...
      MVColumnInfo *dep2ColInfo = mavCols_.getMvColInfoByIndex(currentCol->getDepCol2());
      calcDepCount = buildDepColExpr(calcCorrName_, dep2ColInfo->getDepCol1());
    }
    ...
    if (currentCol->getOperatorType() == ITM_AVG) {
      ...
      elseClause = new (heap_) BiArith(ITM_DIVIDE, calcDep2, calcDep1);
    } else {
      // For STDDEV and VARIANCE, the ELSE clause is
      // ScalarVariance(SYS_CALC.dep3, SYS_CALC.dep2, SYS_CALC.dep1)
      elseClause =
          new (heap_) ScalarVariance(currentCol->getOperatorType(), new (heap_) Cast(calcDep3, desiredType),
                                     new (heap_) Cast(calcDep2, desiredType), new (heap_) Cast(calcDep1, desiredType));
    }

    // The WHEN clause is the same for AVG, STDDEV and VARIANCE:
    // WHEN (SYS_CALC.depCount = 0)  THEN NULL
    ItemExpr *whenClause =
        new (heap_) IfThenElse(new (heap_) BiRelat(ITM_EQUAL, calcDepCount, new (heap_) SystemLiteral(0)),
                               new (heap_) SystemLiteral(),  // NULL constant
                               elseClause);
...
RelRoot *MavRelRootBuilder::buildRootForGop(RelExpr *topNode) {
  ...
  ItemExpr *insCondition = new (heap_)  // (MAV.cnts IS NULL)
      UnLogic(ITM_IS_NULL, new (heap_) ColReference(new (heap_) ColRefName(countStarColName, mavCorrName_)));

  // The condition for Delete from MAV
  // (MAV.cnts = (0 - SYS_DELTA.cnts))
  ItemExpr *delCondition = new (heap_)
      BiRelat(ITM_EQUAL, new (heap_) ColReference(new (heap_) ColRefName(countStarColName, mavCorrName_)),
              new (heap_) BiArith(ITM_MINUS, new (heap_) SystemLiteral(0),
                                  new (heap_) ColReference(new (heap_) ColRefName(countStarColName, deltaCorrName_))));
  ...
  if (minMaxRecomputeOnUpdateCondition_ == NULL)
    ...
    lastClause = new (heap_) IfThenElse(minMaxRecomputeOnUpdateCondition_, constRecompFromUpdate, constUpdate);
  ...
  if (minMaxRecomputeOnInsertCondition_ == NULL)
    ...
    insertClause = new (heap_)
        Case(NULL, new (heap_) IfThenElse(minMaxRecomputeOnInsertCondition_, constRecompFromInsert, constInsert));

  // Build the CASE and IF operators
  ItemExpr *gopExpression =
      new (heap_) Case(NULL, new (heap_) IfThenElse(insCondition, insertClause,
                                                    new (heap_) IfThenElse(delCondition, constDelete, lastClause)));

  // Add the GOP expression as SYS_CALC.@GOP.
  ColRefName *gopColName = new (heap_) ColRefName(MavBuilder::getVirtualGopColumnName(), calcCorrName_);
...
ItemExpr *MavRelRootBuilder::buildSelfCancelingDeltaPredicate() {
  ...
  ItemExpr *mavCountStarName = new (heap_) ColReference(new (heap_) ColRefName(countStarColName, mavCorrName_));
  if (isMavWithoutGroupBy()) {
    // (SYS_MAV.cnts = 0)
    mavCountStarCondition = new (heap_) BiRelat(ITM_EQUAL, mavCountStarName, new (heap_) SystemLiteral(0));
  } else {
    // (SYS_MAV.cnts IS NULL)
    mavCountStarCondition = new (heap_) UnLogic(ITM_IS_NULL, mavCountStarName);
  }

  // (mavCountStarCondition AND SYS_DELTA.cnts = 0)
  ItemExpr *selfCancelCondition = new (heap_) BiLogic(
      ITM_AND, mavCountStarCondition,
      new (heap_) BiRelat(ITM_EQUAL, new (heap_) ColReference(new (heap_) ColRefName(countStarColName, deltaCorrName_)),
                          new (heap_) SystemLiteral(0)));

  ItemExpr *notSelfCancelingDeltaPredicate = new (heap_) UnLogic(ITM_NOT, selfCancelCondition);
...
ItemExpr *MavRelRootBuilder::buildDepColExpr(const CorrName &corrName, int depIndex) const {
  ...
  const NAString &depColName = mavCols_.getMvColInfoByIndex(depIndex)->getColName();

  ItemExpr *colRef = new (heap_) ColReference(new (heap_) ColRefName(depColName, corrName));
...
ColRefName *MavRelRootBuilder::createExtraAggName(const NAString &mavColName, extraColumnType columnType) const {
  NAString newColName = "@";
  newColName += mavColName;
  newColName +=
      columnType == AGG_FOR_DELETE ? MavBuilder::getExtraColSuffixForDel() : MavBuilder::getExtraColSuffixForIns();
...
ItemExpr *MavRelRootBuilder::buildExtraMinMaxExpr(const ItemExpr *pMinMaxExpr, const NAString &colName,
  ...
  ColReference *opCol = new (heap_) ColReference(new (heap_) ColRefName(MavBuilder::getVirtualOpColumnName()));
  ConstValue *pConstOpValue = new (heap_) SystemLiteral((columnType == AGG_FOR_DELETE) ? 1 : -1);

  Case *pCase = new (heap_) Case(NULL, new (heap_) IfThenElse(new (heap_) BiRelat(ITM_EQUAL, opCol, pConstOpValue),
                                                              new (heap_) SystemLiteral(),  // NULL
                                                              copyOfAggSubject));
  Aggregate *pNewAggregate = new (heap_) Aggregate(pMinMaxExpr->getOperatorType(), pCase);

  ColRefName *extraColName = createExtraAggName(colName, columnType);
...
ItemExpr *MavRelRootBuilder::buildMinMaxRecomputeOnUpdateCondition(const NAString &mavColName,
                                                                   ColReference *deltaInsCol, ColReference *deltaDelCol,
                                                                   OperatorTypeEnum operatorType, NABoolean wasFullDE) {
  NABoolean noDE = TRUE;
  OperatorTypeEnum logVsMavRelation;
  OperatorTypeEnum delVsInsertRelation;

  ColReference *mavColNameRef = new (heap_) ColReference(new (heap_) ColRefName(mavColName, mavCorrName_));

  if (operatorType == ITM_MIN) {
    logVsMavRelation = ITM_LESS_EQ;
    delVsInsertRelation = wasFullDE ? ITM_LESS : ITM_LESS_EQ;
  } else {
    CMPASSERT(operatorType == ITM_MAX);
    logVsMavRelation = ITM_GREATER_EQ;
    delVsInsertRelation = wasFullDE ? ITM_GREATER : ITM_GREATER_EQ;
  }

  ItemExpr *delIsNull = new (heap_) UnLogic(ITM_IS_NOT_NULL, deltaDelCol->copyTree(heap_));
  ItemExpr *insIsNull = new (heap_) UnLogic(ITM_IS_NULL, deltaInsCol->copyTree(heap_));

  BiRelat *pLogVsMav = new (heap_) BiRelat(logVsMavRelation, deltaDelCol->copyTree(heap_), mavColNameRef);
  BiRelat *pDelVsIns =
      new (heap_) BiRelat(delVsInsertRelation, deltaDelCol->copyTree(heap_), deltaInsCol->copyTree(heap_));

  // look at the drawing on top
  BiLogic *finalCondition = new (heap_) BiLogic(
      ITM_AND, delIsNull, new (heap_) BiLogic(ITM_AND, pLogVsMav, new (heap_) BiLogic(ITM_OR, insIsNull, pDelVsIns)));
...
ItemExpr *MavRelRootBuilder::buildMinMaxRecomputeOnInsertCondition(ColReference *deltaInsCol,
  ...
  ItemExpr *delIsNotNull = new (heap_) UnLogic(ITM_IS_NOT_NULL, deltaDelCol->copyTree(heap_));

  BiRelat *pDelVsIns = new (heap_) BiRelat(ITM_EQUAL, deltaDelCol->copyTree(heap_), deltaInsCol->copyTree(heap_));

  BiLogic *finalCondition = new (heap_) BiLogic(ITM_AND, delIsNotNull, pDelVsIns);
...
void MavRelRootBuilder::addToMinMaxRecomputeCondition(const NAString &mavColName, ColReference *deltaInsCol,
                                                      ColReference *deltaDelCol, OperatorTypeEnum operatorType,
                                                      NABoolean wasFullDE) {
  ItemExpr *newCondition =
      buildMinMaxRecomputeOnUpdateCondition(mavColName, deltaInsCol, deltaDelCol, operatorType, wasFullDE);

  if (minMaxRecomputeOnUpdateCondition_ == NULL)
    minMaxRecomputeOnUpdateCondition_ = newCondition;
  else
    minMaxRecomputeOnUpdateCondition_ = new (heap_) BiLogic(ITM_OR, minMaxRecomputeOnUpdateCondition_, newCondition);

  // If DE LEVEL is 3 (ALL), the next condition will never happen.
  if (wasFullDE) return;

  newCondition = buildMinMaxRecomputeOnInsertCondition(deltaInsCol, deltaDelCol);

  if (minMaxRecomputeOnInsertCondition_ == NULL)
    minMaxRecomputeOnInsertCondition_ = newCondition;
  else
    minMaxRecomputeOnInsertCondition_ = new (heap_) BiLogic(ITM_OR, minMaxRecomputeOnInsertCondition_, newCondition);
}  // MavRelRootBuilder::addToMinMaxRecomputeCondition()

//----------------------------------------------------------------------------
//----------------------------------------------------------------------------
void MavRelRootBuilder::addColsToSelectList(LIST(MVColumnInfo *) listOfCols, ItemExprList &selectList,
                                            const CorrName &corrName, const CorrName *renameTo) {
  for (CollIndex i = 0; i < listOfCols.entries(); i++) {
    const NAString &colName = listOfCols[i]->getColName();

    ItemExpr *colRef = new (heap_) ColReference(new (heap_) ColRefName(colName, corrName));
