...
DisjunctArray *mdamANDDisjunctArrays(DisjunctArray *leftDisjunctArray, DisjunctArray *rightDisjunctArray) {
  ...
  //---------------------------------------------------------------------
  if (!leftDisjunctArray OR !rightDisjunctArray) return NULL;  // Abort! -- make it know to others.
  //---------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------
  {
    DisjunctArray *disjunctArray = new (CmpCommon::statementHeap()) DisjunctArray();
    disjunctArray->andDisjunctArrays(leftDisjunctArray, rightDisjunctArray);
...
DisjunctArray::~DisjunctArray() {
  ...
  for (CollIndex index = 0; index < entries(); index++) delete at(index);
...
// -----------------------------------------------------------------------
void DisjunctArray::andDisjunct(DisjunctArray *otherDisjunctArray) {
  ...
  // ValueIdSets (bitmaps).
  // ---------------------------------------------------------------------
  for (CollIndex index = 0; index < entries(); index++) *at(index) += *(otherDisjunctArray->at(0));
...
// -----------------------------------------------------------------------
void DisjunctArray::andDisjunctArrays(DisjunctArray *leftDisjunctArray, DisjunctArray *rightDisjunctArray) {
  // ---------------------------------------------------------------------
  ...
  for (CollIndex l = 0; l < leftDisjunctArray->entries(); l++)
    for (CollIndex r = 0; r < rightDisjunctArray->entries(); r++) {
      ValueIdSet *disjunct = new (CmpCommon::statementHeap()) ValueIdSet(*(leftDisjunctArray->at(l)));
...
// -----------------------------------------------------------------------
void DisjunctArray::orDisjunctArray(DisjunctArray *otherDisjunctArray) {
  // ---------------------------------------------------------------------
  // Loop through the disjunct array passed and insert each entry into
  // the disjunct array being processed.
  // ---------------------------------------------------------------------
  for (CollIndex index = 0; index < otherDisjunctArray->entries(); index++) insert(otherDisjunctArray->at(index));
...
void DisjunctArray::display() const { print(); }

void DisjunctArray::print(FILE *ofd, const char *indent, const char *title) const {
  fprintf(ofd, "%s", title);
  fprintf(ofd, "\n");
...
void ScanKey::replicateNonKeyVEGPredicates(const ValueIdSet &setOfKeyPredicates,
  ...
  if (UDIdesc != NULL) UDIdesc->getNonKeyColumnSet(ghostNonKeyColumnSet);
  ...
  if (CmpCommon::getDefaultLong(ALLOW_INPUT_PRED_REPLICATION_REDUCTION) > 1) {
    for (ValueId inputExprId = inputs.init(); inputs.next(inputExprId); inputs.advance(inputExprId)) {
      ...
      if (inputExprPtr->getOperatorType() == ITM_VEG_REFERENCE) {
        ...
        for (ValueId refExprId = referenceValues.init(); referenceValues.next(refExprId);
             referenceValues.advance(refExprId)) {
          if (refExprId.getItemExpr()->getOperatorType() == ITM_INDEXCOLUMN) referenceValues -= refExprId;

          if (getNonKeyColumnSet().contains(refExprId)) referenceValues -= refExprId;

          if (expandedKeyCols.contains(refExprId)) referenceValues -= refExprId;
        }

        // remember the true outer references
        // we only need one value
        if (referenceValues.entries()) trueOuterReferences += referenceValues.init();
      } else {
        // remember the true outer references
  ...
  for (ValueId exprId = setOfKeyPredicates.init(); setOfKeyPredicates.next(exprId);
    ...
    if (exprPtr->getOperatorType() == ITM_VEG_PREDICATE) {
      ...
      for (ValueId elemId = predicateValues.init(); predicateValues.next(elemId); predicateValues.advance(elemId)) {
        if (elemId.getItemExpr()->getOperatorType() == ITM_INDEXCOLUMN) predicateValues -= elemId;
      ...
      if (CmpCommon::getDefaultLong(ALLOW_INPUT_PRED_REPLICATION_REDUCTION) > 0) {
        ...
        for (ValueId vid = nonKeyColSet.init(); nonKeyColSet.next(vid); nonKeyColSet.advance(vid)) {
          // Since we can call this on any ValueIdSet, make sure we actually have
          // an IndexColumn.
          if (vid.getItemExpr()->getOperatorType() == ITM_INDEXCOLUMN)
            nonKeyBaseColumns.insert(
                ((BaseColumn *)(((IndexColumn *)vid.getItemExpr())->getDefinition().getItemExpr()))->getValueId());
        }
...
NABoolean ScanKey::expressionContainsColumn(const ItemExpr &ie, const ValueId &column) {
  ...
  if (ie.getValueId() == column) {
    ...
    } else if (ie.getValueId() == col) {  // the index column matches the base column
      retVal = TRUE;
...
NABoolean ScanKey::isAKeyPredicate(const ValueId &predId, ValueId &referencedInput,
  ...
  for (CollIndex i = 0; i < keyCols.entries(); i++) {
    ...
    if (isAKeyPredicateForColumn(predId, referencedInput, intervalExclusionExpr, col)) {
      retVal = TRUE;
      break;
    }
  }
...
NABoolean ScanKey::isAKeyPredicate(const ValueId &predId) const {
  ValueId dummy1(NULL_VALUE_ID);
  ValueId dummy2(NULL_VALUE_ID);
  return isAKeyPredicate(predId, dummy1, dummy2);
}  // isAKeyPredicate

NABoolean ScanKey::isAKeyPredicateForColumn(const ValueId &predId, const ValueId &keyColumn) const {
  ValueId dummy1(NULL_VALUE_ID);
  ValueId dummy2(NULL_VALUE_ID);
  return isAKeyPredicateForColumn(predId, dummy1, dummy2, keyColumn, getOperatorInputs());
}  // isAKeyPredicate

NABoolean ScanKey::isAKeyPredicateForColumn(const ValueId &predId, ValueId &referencedInput /* out */
                                            ,
                                            ValueId &intervalExclusionExpr /* out */
                                            ,
                                            const ValueId &keyColumn) const {
  return isAKeyPredicateForColumn(predId, referencedInput, intervalExclusionExpr, keyColumn, getOperatorInputs());
}
...
NABoolean ScanKey::isAKeyPredicateForColumn(const ValueId &predId, ValueId &referencedInput /* out */
                                            ...
                                            ValueId &intervalExclusionExpr /* out */
                                            ,
                                            const ValueId &keyColumn, const ValueIdSet &inputValues) {
  ...
  switch (arity) {
    ...
    case 2:
      ...
      if (expressionContainsColumn(*(iePtr->child(0)->castToItemExpr()), keyColumn)) {
      ...
      // (but in the case of an assign (use in insert), it cannot be)
                   ...
                   AND expressionContainsColumn(*(iePtr->child(1)->castToItemExpr()), keyColumn)) {
    ...
    default:
      ...
      if (iePtr->getOperatorType() == ITM_VEG_PREDICATE) {
        ...
        // --------------------------------------------------------------
        const VEG *veg = ((VEGPredicate *)iePtr)->getVEG();
        const VEGReference *vegRef = veg->getVEGReference();
        ...
        if (expressionContainsColumn(*vegRef, keyColumn)) {
          ...
          if (NOT VEGMembers.isEmpty())  // if VEG contains external inputs
          {
            ...
            referencedInput = ((VEGPredicate *)iePtr)->getVEG()->getVEGReference()->getValueId();
          } else
  ...
  if (intervalExclusionExpr != NULL_VALUE_ID AND NOT externalInputs.contains(intervalExclusionExpr)) {
    return FALSE;
  }
...
static ItemExpr *convertCastToNarrow(ItemExpr *expr, CollHeap *outHeap, void *) {
  if (expr->getOperatorType() == ITM_CAST) {
    ...
    return new (outHeap)
        Narrow(expr->child(0),
               new (outHeap) HostVar("_sys_ignored_CC_convErrorFlag", new (outHeap) SQLInt(outHeap, TRUE, FALSE), TRUE),
...
void ScanKey::createComputedColumnPredicates(ValueIdSet &predicates,           /* in/out */
  ...
  if (predicates.isEmpty()) return;  // nothing to do
    ...
    switch (iePtr->getOperatorType()) {
      case ITM_BASECOLUMN: {
        if (((BaseColumn *)iePtr)->getNAColumn()->isComputedColumn()) {
          ...
          if (keyColsReferencedByCompExpr.entries() > 1 || !bcol->getNAColumn()->isDivisioningColumn()) {
          ...
          for (ValueId u = keyColsReferencedByCompExpr.init(); keyColsReferencedByCompExpr.next(u);
               keyColsReferencedByCompExpr.advance(u)) {
            NABoolean foundPredForThisCol = FALSE;
            for (ValueId p = potentialPredicates.init(); potentialPredicates.next(p); potentialPredicates.advance(p)) {
              ...
              if (isAKeyPredicateForColumn(p, keyValueExpr, dummy, u, operatorInputs)) {
                ...
                colToKeyValueMap.addMapEntry(bc->getTableDesc()->getColumnVEGList()[bc->getColNumber()], keyValueExpr);
                keyPredicatesOnCC.insert(p);
              }
            }
            if (!foundPredForThisCol) predsForAllUnderlyingCols = FALSE;
          }
          ...
          if (predsForAllUnderlyingCols) {
            ...
            if (bcol->getNAColumn()->isDivisioningColumn())
              generatedPredicates += keyPredicatesOnCC.createMirrorPreds(v, keyColsReferencedByCompExpr);
            else {
              ...
              compExpr = compExpr->treeWalk(convertCastToNarrow, CmpCommon::statementHeap());
              compExpr->synthTypeAndValueId();

              // create a new predicate bcol = compExpr
              ItemExpr *mirrorPred = new (CmpCommon::statementHeap()) BiRelat(ITM_EQUAL, egVid.getItemExpr(), compExpr);
              ...
              colToKeyValueMap.rewriteValueIdDown(mpValId, mpValIdRewritten);
...
void ScanKey::getKeyPredicates(ValueIdSet &keyPredicates,   /* out */
  ...
  *allKeyPredicates = TRUE;
  ...
  for (ValueId predId = disjunct.getAsValueIdSet().init(); disjunct.getAsValueIdSet().next(predId);
    ...
    if (predId.getItemExpr()->getOperatorType() == ITM_RANGE_SPEC_FUNC) {
      ...
      for (ValueId predIdi = vdset.init(); vdset.next(predIdi); vdset.advance(predIdi)) {
...
void ScanKey::splitRangeSpecRef(ColumnOrderList &keyPredsByCol, const ValueId &predId, const ValueIdList &columns,
  ...
  if (exprPtr->getOperatorType() == ITM_VEG_PREDICATE) {
    ValueIdSet predicateValues;
    predicateValues = ((VEGPredicate *)exprPtr)->getVEG()->getAllValues();
    if (predicateValues.entries() > 2 && predicateValues.referencesAConstExpr()) doBreak = FALSE;
  }
...
void ScanKey::getKeyColumnOrderList(ColumnOrderList &keyPredsByCol, /* out*/
  ...
  for (CollIndex j = 1; j < tempColumns.entries(); j++) {
    ...
    if (noDuplicateFound) columns.insert(tempColumns[j]);
  }
  ...
  for (ValueId predId = predSet.init(); predSet.next(predId); predSet.advance(predId)) {
    ...
    if ((CmpCommon::getDefault(RANGESPEC_TRANSFORMATION) == DF_ON)) {
      if (predId.getItemExpr()->getOperatorType() == ITM_RANGE_SPEC_FUNC) {
        ValueIdSet vdset;
        predId.getItemExpr()->child(1)->getLeafPredicates(vdset);
        for (ValueId predIdp = vdset.init(); vdset.next(predIdp); vdset.advance(predIdp)) {
          splitRangeSpecRef(keyPredsByCol, predIdp, columns, firstElemInRangeSpec,
...
//---------------------------------------------------------
MdamKey::MdamKey(const ValueIdList &keyColumnIdList, const ValueIdSet &operatorInputs,
                 const Disjuncts &associatedDisjuncts, const ValueIdSet &nonKeyColumnSet, const IndexDesc *indexDesc)
    : ScanKey(keyColumnIdList, operatorInputs, associatedDisjuncts, nonKeyColumnSet, indexDesc),
...

{
  ...
  if (getDisjuncts().entries()) {
    ...
    stopColumnArray_ = new (CmpCommon::statementHeap()) CollIndex[getDisjuncts().entries()];
    DCMPASSERT(stopColumnArray_ != NULL);
    setAllStopColumnsToMax();
  }

  // all columns are sparse
  sparseFlagArray_ = new (CmpCommon::statementHeap()) NABoolean[getKeyColumns().entries()];
...
MdamKey::~MdamKey() {
  ...
  if (columnOrderListPtrArrayPtr_ != NULL) {
    for (CollIndex i = 0; i < columnOrderListPtrArrayPtr_->entries(); i++) delete (*columnOrderListPtrArrayPtr_)[i];
...
void MdamKey::setAllColumnsToSparse() {
  for (CollIndex i = 0; i < getKeyColumns().entries(); i++) sparseFlagArray_[i] = TRUE;
}

void MdamKey::setStopColumn(CollIndex disjunctNumber, CollIndex columnOrder) {
...
void MdamKey::reuseMdamKeyInfo(MdamKey *other) {
  ...
  if (numOfDisjuncts > 0) {
    ...
    for (CollIndex i = 0; i < numOfDisjuncts; i++) setStopColumn(i, other->getStopColumn(i));
  }

  CollIndex numOfKeyColumns = (other->getKeyColumns()).entries();
  DCMPASSERT(numOfKeyColumns == getKeyColumns().entries());

  for (CollIndex j = 0; j < numOfKeyColumns; j++) setSparseFlag(j, other->getSparseFlag(j));

  if (other->getNoExePred()) setNoExePred();
}

void MdamKey::preCodeGen(ValueIdSet &executorPredicates, const ValueIdSet &selectionPredicates,
                         const ValueIdSet &availableValues, const ValueIdSet &inputValues, VEGRewritePairs *vegPairsPtr,
                         NABoolean replicateExpression, NABoolean partKeyPredsAdded) {
  //--------
  ...
  columnOrderListPtrArrayPtr_ = new (CmpCommon::statementHeap()) ColumnOrderListPtrArray;
  ...
  if (CmpCommon::getDefault(RANGESPEC_TRANSFORMATION) == DF_ON && selectionPredicates.entries() && !getNoExePred()) {
    ValueIdSet temp;
    ItemExpr *inputItemExprTree = selectionPredicates.rebuildExprTree(ITM_AND, FALSE, FALSE);
    ...
    executorPredicates.replaceVEGExpressions(availableValues, inputValues,
  ...
  for (i = 0; i < curDisjuncts->entries(); i++) {
    ...
    replicateNonKeyVEGPredicates(keyPredicates, vegKeyNonKeyPreds);

    // Rewrite disjunct predicates:
    // enforce generation of key predicates by:
    // 1) Setting the generate key predicates flag
    keyPredicates.replaceVEGExpressions(availableValues, inputValues,
                                        TRUE,  // generate key predicates
                                        vegPairsPtr, replicateExpression, NULL, NULL,
    ...
    ColumnOrderList *columnOrderListPtr = new (CmpCommon::statementHeap()) ColumnOrderList(getKeyColumns());
  ...
  for (i = 0; i < curDisjuncts->entries(); i++) {
    ...
    if (!partKeyPredsAdded) isEmpty = TRUE;  // assume it is empty
  ...
  if (isEmpty) {
    ...
    if (NOT getNoExePred()) {
      ...
      getColumnOrderListPtrArray().computeCommonKeyPredicates(commonKeyPredicates);
      ...
      // the wrong semantics
      vegKeyNonKeyPreds.replaceVEGExpressions(availableValues, inputValues,
                                              FALSE,  // no need for key predicate generation here
                                              vegPairsPtr, replicateExpression);
...
void MdamKey::appendKeyPredicates(ValueIdSet &keyPredicates, const ValueIdSet &predicates,
                                  const ValueIdSet &inputValues) const {
  // for each predicate in predicates:
  for (ValueId predId = predicates.init(); predicates.next(predId); predicates.advance(predId)) {
...
NABoolean MdamKey::isAPartKeyPredicateForMdam(const ValueId &predId, const ValueIdSet &inputValues) const {
  NABoolean retVal = FALSE;
  ...
  for (CollIndex i = 0; i < keyCols.entries(); i++) {
    const ValueId &col = keyCols[i];
    if (isAKeyPredicateForColumn(predId, referencedInput, intervalExclusionExpr, col, operatorInputs)) {
      retVal = TRUE;
      break;
    }
  }
...
void MdamKey::print(FILE *ofd, const char *indent, const char *title) const {
  // is print enabled?
  ...
  if (ev != NULL AND strcmp(ev, "ON") == 0) {
    ...
    // print stop columns
    CollIndex i = 0;
    for (i = 0; i < getDisjuncts().entries(); i++) fprintf(ofd, "stopColumnArray_[%d] = %d\n", i, getStopColumn(i));

    // print sparse flag:

    CollIndex maxStopColumn = getStopColumn(0);
    for (i = 1; i < getDisjuncts().entries(); i++) {
      if (maxStopColumn < getStopColumn(i)) maxStopColumn = getStopColumn(i);
    }
    for (i = 0; i <= maxStopColumn; i++)
      fprintf(ofd, "Column: [%d] is %s\n", i, (isColumnSparse(i) ? "Sparse" : "Dense"));
    ...
    if (columnOrderListPtrArrayPtr_) {
      ...
      ValueIdSet commonKeyPredicates;
      columnOrderListPtrArrayPtr_->computeCommonKeyPredicates(commonKeyPredicates);
...
void MdamKey::getKeyPredicatesByColumn(ColumnOrderList &keyPredsByCol, CollIndex disjunctNumber) const {
  // get predicates from disjunct
  Disjunct disjunct;
  NABoolean ok = getDisjuncts().get(disjunct, disjunctNumber);
  DCMPASSERT(ok);  // the disjuct must exist
...
void MdamKey::reportKeyAssignment(NAString &tname) {
  cout << "\nMDAM key assignment for " << tname.data() << ":" << endl;

  const CollIndex columns = getIndexDesc()->getIndexKey().entries();
  const CollIndex disjEntries = getKeyDisjunctEntries();

  NAList<ValueIdSet> ListOfPredicates(CmpCommon::statementHeap());
  for (CollIndex i = 0; i < disjEntries; i++) {
    ColumnOrderList kpbc(getIndexDesc()->getIndexKey());
    getKeyPredicatesByColumn(kpbc, i);

    // gather the key predicates:
    for (CollIndex j = 0; j < columns; j++) {
      if (kpbc[j]) {
        if (ListOfPredicates.getUsage(j) == UNUSED_COLL_ENTRY) {
          ValueIdSet x;
          ListOfPredicates.insertAt(j, x);
        }

        ValueIdSet &set = ListOfPredicates[j];
        set.insert(*(kpbc[j]));
      }
    }
  }
  ...
  // display predicates for each column
  for (CollIndex j = 0; j < columns; j++) {
    BaseColumn *leadingKeyCol = (BaseColumn *)(baseKeyCols[j].getItemExpr());
    NAColumn *naColumn = leadingKeyCol->getNAColumn();
    cout << "   " << naColumn->getColName().data() << ": ";

    ValueIdSet &set = ListOfPredicates[j];
    ItemExpr *ie = set.rebuildExprTree();
    if (ie) cout << ie->getText().data();
  ...
  if (entries() > 0) {
    ...
    for (CollIndex i = 0; i < entries(); i++) {
      ...
      if (CmpCommon::getDefault(RANGESPEC_TRANSFORMATION) == DF_ON) {
        ...
        for (ValueId predId = inVidset.init(); inVidset.next(predId); inVidset.advance(predId)) {
          // TRAFODION-2988
          if (ITM_RANGE_SPEC_FUNC == predId.getItemExpr()->getOperatorType()) {
            if (ITM_AND == predId.getItemExpr()->child(1)->getOperatorType()) {
              ...
              outVidset += parsedVs;
            } else if (ITM_OR == predId.getItemExpr()->child(1)->getOperatorType()) {  // add deal with OR operator
              outVidset += predId;
            } else {
...
void Disjuncts::print(FILE *ofd, const char *indent, const char *title) const {
  fprintf(ofd, "%s:\n", title);
...
void Disjuncts::print() const { print(stdout); }

NABoolean Disjuncts::get(Disjunct &disjunct, CollIndex i) const {
  if (i < entries()) {
    disjunct.clear();
    return TRUE;
  } else
...
NABoolean isOrItemExpr(ItemExpr *iePtr) { return (iePtr && iePtr->getOperatorType() == ITM_OR); }

NABoolean isAndOrItemExpr(ItemExpr *iePtr) {
  return (iePtr && (iePtr->getOperatorType() == ITM_AND || iePtr->getOperatorType() == ITM_OR));
}
...
NABoolean Disjuncts::containsSomePredsInRanges(funcPtrT funcP) const {
  ...
  for (CollIndex i = 0; i < entries(); i++) {
    ...
    for (ValueId predId = vs.init(); vs.next(predId); vs.advance(predId)) {
      if (predId.getItemExpr()->getOperatorType() == ITM_RANGE_SPEC_FUNC) {
        if ((*funcP)(predId.getItemExpr()->child(1))) return TRUE;
      }
    }
  }
  return FALSE;
...
NABoolean Disjuncts::containsOrPredsInRanges() const { return containsSomePredsInRanges(isOrItemExpr); }
...
//---------------------------------------------------------
MaterialDisjuncts::MaterialDisjuncts(const ValueIdSet &selectionPredicates) : disjunctArrayPtr_(NULL) {
...
MaterialDisjuncts::MaterialDisjuncts(Disjunct *disjunct) {
  disjunctArrayPtr_ = new HEAP DisjunctArray(new HEAP ValueIdSet(disjunct->getAsValueIdSet()));
...
MaterialDisjuncts::~MaterialDisjuncts() {
  ...
  for (CollIndex i = 0; i < disjunctArrayPtr_->entries(); i++) delete (*disjunctArrayPtr_)[i];
}

void MaterialDisjuncts::createDisjunctArray(const ValueIdSet &selectionPredicates) {
  ...
  // ---------------------------------------------------------------------
  for (ValueId exprId = selectionPredicates.init(); selectionPredicates.next(exprId);
    ...
    if (prevDisjunctArray == NULL)
      ...
      prevDisjunctArray = mdamANDDisjunctArrays(prevDisjunctArray, currDisjunctArray);
    if (!prevDisjunctArray) break;
...
NABoolean MaterialDisjuncts::get(Disjunct &disjunct, CollIndex i) const {
  NABoolean retVal = FALSE;
  ...
  switch (getType()) {
    ...
    case RANGE:
      ...
      if (getPredicates().entries() == 1) {
        OperatorTypeEnum newPredOperType = predicate.getItemExpr()->getOperatorType();

        ValueId predId = getPredicates().init();
        getPredicates().next(predId);
        OperatorTypeEnum exPredOperType = predId.getItemExpr()->getOperatorType();

        switch (newPredOperType) {
          case ITM_LESS:
          case ITM_LESS_EQ:
          case ITM_LESS_OR_LE:
            if (exPredOperType == ITM_GREATER OR exPredOperType == ITM_GREATER_EQ OR exPredOperType ==
                ITM_GREATER_OR_GE) {
              setType(RANGE);
            } else {
              setType(CONFLICT);
          ...
          case ITM_GREATER_EQ:
          case ITM_GREATER_OR_GE:
            if (exPredOperType == ITM_LESS OR exPredOperType == ITM_LESS_EQ OR exPredOperType == ITM_LESS_OR_LE) {
              setType(RANGE);
            } else {
              setType(CONFLICT);
            }
...
KeyColumns::KeyColumn::KeyColumnType KeyColumns::KeyColumn::getType(const ValueId &predId) const {
...
void KeyColumns::KeyColumn::print(FILE *ofd, const char *indent, const char * /*title*/) const {
  char message[80];
...
void KeyColumns::KeyColumn::resolveConflict() {
  if (getType() == CONFLICT_EQUALS OR getType() == CONFLICT) {
    // ------------------------------------------------------------------
    ...
    for (pred = preds.init(); preds.next(pred); preds.advance(pred)) {
      if (pred.getItemExpr()->getOperatorType() == ITM_EQUAL OR pred.getItemExpr()->getOperatorType() ==
          ITM_VEG_PREDICATE) {
        chosenPredPtr = &pred;
        break;  // important!
      }
    ...
    if (chosenPredPtr == NULL) {
      ...
      for (pred = preds.init(); chosenPredPtr == NULL AND preds.next(pred); preds.advance(pred)) {
...
KeyColumns::KeyColumns() : keyColumnPtrCache_(HEAP) {}

KeyColumns::KeyColumns(const ValueIdList &orderList) : keyColumnPtrCache_(HEAP) {
  for (CollIndex i = 0; i < orderList.entries(); i++) insertColumn(orderList[i]);
}
...
void KeyColumns::insertColumn(const ValueId &column) {
  ...
  if (NOT alreadyIn) {
    KeyColumn *keyColumnPtr = new (CmpCommon::statementHeap()) KeyColumn(column);
    DCMPASSERT(keyColumnPtr != NULL);
    keyColumnPtrCache_.insertAt(keyColumnPtrCache_.entries(), keyColumnPtr);
...

{
  ...
  ItemExpr *iePtr = NULL;
  for (ValueId predId = andPredicateExpression.init(); andPredicateExpression.next(predId);
...
KeyColumns::~KeyColumns() { clear(); }  // KeyColumns::~KeyColumns()

const KeyColumns::KeyColumn &KeyColumns::getKeyColumn(const ValueId &column) const {
  ...
  if (result == NULL) DCMPASSERT(FALSE);
  return *result;
}

KeyColumns::KeyColumn *KeyColumns::getKeyColumnPtr(
    const ValueId &column) {  // The cache must have columns and predicates:
...
const ValueIdSet &KeyColumns::getPredicatesForColumn(const ValueId &column) const {
...
void KeyColumns::getAllPredicates(ValueIdSet &allPredicates) const {
  ...
  for (ValueId column = columnSet_.init(); columnSet_.next(column); columnSet_.advance(column)) {
    allPredicates.insert(getPredicatesForColumn(column));
  }
}

void KeyColumns::insertPredicateInColumn(const ValueId &predicate, const ValueId &column) {
  insertPredicate(predicate, &column);
}

void KeyColumns::insertPredicate(const ValueId &predicate) { insertPredicate(predicate, NULL); }

void KeyColumns::insertPredicate(const ValueId &predicate, const ValueId *columnPtr) {
  ...
  DCMPASSERT((predicate.getItemExpr()->isAPredicate())OR(predicate.getItemExpr()->getOperatorType() == ITM_ASSIGN)
                 OR(predicate.getItemExpr()->getOperatorType() == ITM_LESS_OR_LE)
                     OR(predicate.getItemExpr()->getOperatorType() == ITM_GREATER_OR_GE)
                         OR(predicate.getItemExpr()->getOperatorType() == ITM_OR)  // For RangeSpecRef Item Expression
  );

  // The ValueId for the column must come from a column:
  DCMPASSERT(NOT columnPtr OR((columnPtr->getItemExpr()->getOperatorType() == ITM_BASECOLUMN)
                                  OR(columnPtr->getItemExpr()->getOperatorType() == ITM_INDEXCOLUMN)));
  ...
  if (predicate.getItemExpr()->getArity() == 2)
    if ((predicate.getItemExpr()->getConstChild(0)->castToItemExpr()->getOperatorType() == ITM_CONSTANT)
            AND(predicate.getItemExpr()->getConstChild(1)->castToItemExpr()->getOperatorType() == ITM_CONSTANT))
  ...
  if (predicate.getItemExpr()->getOperatorType() == ITM_AND) {  // the AND expression can be very complex, in general:
    insertPredicate(predicate.getItemExpr()->getConstChild(0)->castToItemExpr()->getValueId(), columnPtr);
    insertPredicate(predicate.getItemExpr()->getConstChild(1)->castToItemExpr()->getValueId(), columnPtr);
  } else  // it's an appropiate pred, insert it in the appropiate column:
  {
    for (CollIndex i = 0; i < keyColumnPtrCache_.entries(); i++) {
      ...
      if (columnPtr != NULL) {
        ...
        if (refExpr->getOperatorType() == ITM_ASSIGN) refExpr = refExpr->child(0);

        if (refExpr->referencesTheGivenValue(keyColumnPtrCache_[i]->getColumnId()) OR refExpr->referencesTheGivenValue(
                ((IndexColumn *)(keyColumnPtrCache_[i]->getColumnId().getItemExpr()))->getDefinition()))
...
void KeyColumns::clearPredicates() {
  if (NOT keyColumnPtrCache_.isEmpty()) {
    // remove the KeyColumn instances that were allocated
    // using new (by buildCacheColumns())
    for (CollIndex i = 0; i < keyColumnPtrCache_.entries(); i++) keyColumnPtrCache_[i]->clearPredicates();
  }
}

void KeyColumns::print(FILE *ofd, const char *indent, const char *title) const {
  // print list:
  fprintf(ofd, "%s", title);
  for (CollIndex i = 0; i < keyColumnPtrCache_.entries(); i++) {
    // print the column
...
ValueId ColumnOrderList::getKeyColumnId(CollIndex order) const {
  if (order >= entries() || orderKeyColumnPtrList_[order] == NULL) return NULL_VALUE_ID;
...
NABoolean ColumnOrderList::containsPredicates() const {
  ...
  for (CollIndex order = 0; order < entries(); order++) {
    ...
    // it's not EMPTY
    if (getPredicateExpressionPtr(order) AND getPredicateExpressionPtr(order)->getType() != KeyColumn::EMPTY) {
      containsPreds = TRUE;
      break;
    }
  }
...
void ColumnOrderList::print(FILE *ofd, const char *indent, const char * /*title*/) const {
...
ColumnOrderList::ColumnOrderList(const ValueIdList &listOfColumns)
    : KeyColumns(listOfColumns), columnList_(listOfColumns), orderKeyColumnPtrList_(HEAP) {
...

{
  ...
  for (CollIndex order = 0; order < entries(); order++) invalidateOrder(order);
}

const ColumnOrderList::KeyColumn *ColumnOrderList::getPredicateExpressionPtr(CollIndex order) const {
...

{
  ...
  CollIndex order = 0;
  for (order = 0; order <= stopColumn; order++) validateOrder(order);
  // The rest are invalid (i.e. they return a NULL
  // pointer when operator[] is used on the columnOrderList)
  for (; order < entries(); order++) invalidateOrder(order);
}
...
NABoolean ColumnOrderList::getSingleSubsetOrder(CollIndex &sinSubOrder /* out */) const {
  ...
  if (containsPredicates()) {
    for (order = 0; order < numOfKeyCols; order++) {
      ...
      // Exit the loop as soon as we hit an expression that
      // it's not an EQUAL:
      if (typeOfRange != KeyColumns::KeyColumn::EQUAL) break;
    }  // end of for-loop

    // not a single subset when the first pred. is an IN
    if (order == 0 AND typeOfRange == KeyColumns::KeyColumn::INLIST) {
    ...
    if (order ==
        numOfKeyCols OR  // Case 1.
            typeOfRange ==
        KeyColumns::KeyColumn::INLIST OR                  // Case 6.
...
NABoolean ColumnOrderList::getSingleSubsetOrderForMdam(CollIndex &sinSubOrder /* out */) const {
  NABoolean itIsSingleSubset = TRUE;
  CollIndex order;
  CollIndex numOfKeyCols = entries();
  KeyColumns::KeyColumn::KeyColumnType typeOfRange = KeyColumns::KeyColumn::EMPTY;
  ...
  if (containsPredicates()) {
    for (order = 0; order < numOfKeyCols; order++) {
      if (getPredicateExpressionPtr(order) != NULL)
        ...
        typeOfRange = KeyColumns::KeyColumn::EMPTY;
      if (typeOfRange != KeyColumns::KeyColumn::EQUAL && typeOfRange != KeyColumns::KeyColumn::RANGE) break;
    }  // end of for-loop
...
ColumnOrderListPtrArray::ColumnOrderListPtrArray() : ARRAY(ColumnOrderList *)(HEAP) {}

void ColumnOrderListPtrArray::computeCommonKeyPredicates(ValueIdSet &commonKeyPredicates) const {
...
void ColumnOrderListPtrArray::print(FILE *ofd, const char *indent, const char * /*title*/) const {
