...
void MdamTrace::setHeader(const char *override) { overrideHeader_ = override; }
...
void MdamTrace::mayInit() {
  ...
  const char *debugFileName = getenv(
      "MDAM_"
      "DEBUG_"
      "FILE");
  if (debugFileName) {
    outputFile_ = fopen(debugFileName, "a");
    if (!outputFile_) outputFile_ = stdout;
  } else
    outputFile_ = stdout;
...
void MdamTrace::printTaskMonitor(TaskMonitor &mon, const char *msg) {
  mayInit();

  if (!mon.goodcount()) return;
  if (!doPrint_) return;
...
void MdamTrace::printCostObject(ScanOptimizer *opt, const Cost *cost, const char *msg) {
  mayInit();
...
void MdamTrace::printFileScanInfo(ScanOptimizer *opt, const ValueIdSet &partKeyPreds) {
  mayInit();

  printf("\n%sScan Information:\n", getHeader());
  if (opt->getContext().getReqdPhysicalProperty()->getPerformanceGoal() ==
      NULL OR opt->getContext().getReqdPhysicalProperty()->getPerformanceGoal() ==
  ...
  printf("%sReceiving [%.4f] rows (probes)\n", indent_,
         opt->getContext().getInputLogProp()->getResultCardinality().value());
  if ((opt->getContext().getInputPhysicalProperty() != NULL)
          AND(opt->getContext().getInputPhysicalProperty()->getNjOuterOrder() != NULL)) {
    printf("%sIncoming rows with ordering: ", indent_);
    opt->getContext().getInputPhysicalProperty()->getNjOuterOrder()->display();
  } else {
    printf("%sIncoming rows unordered", indent_);
  }

  if ((opt->getContext().getInputLogProp()->getResultCardinality()).isGreaterThanZero()) {
...
void MdamTrace::printBasicCost(FileScanOptimizer *opt, SimpleCostVector &prefixFR, SimpleCostVector &prefixLR,
                               const char *msg) {
...
MdamTraceLevel MdamTrace::level() { return level_; }

void MdamTrace::setLevel(enum MdamTraceLevel l) { level_ = l; }

#endif  // if MDAM_TRACE

enum restrictCheckStrategy { MAJORITY_WITH_PREDICATES = 1, TOTAL_UECS = 2, BOTH = 3 };

static NABoolean checkMDAMadditionalRestriction(const ColumnOrderList &keyPredsByCol,
                                                const CollIndex &lastColumnPosition, const Histograms &hist,
                                                restrictCheckStrategy strategy, CollIndex &noOfmissingKeyColumns,
                                                CollIndex &presentKeyColumns) {
  ...
  for (index = 0; index < lastColumnPosition; index++) {
    ...
    if (typeOfRange == KeyColumns::KeyColumn::EMPTY) {
      ...
      if (checkLeadingDivColumns == FALSE || (!isLeadingDivisionColumn && !isLeadingSaltColumn))
        noOfmissingKeyColumns++;

      // accumulate the product of uecs for columns without predicates in the current
      // group
      totalUecsForCurrentPredicatelessKeyColumnGroup *= hist.getColStatsForColumn(columnVid).getTotalUec().getCeiling();
      ...
      if (currentPredicatelessKeyColumnGroup.entries() > 1) {
        ...
        ValueIdSet theLargestSubset = MCUL->largestSubset(currentPredicatelessKeyColumnGroup.convertToBaseIds());

        if (theLargestSubset.entries() == currentPredicatelessKeyColumnGroup.entries()) {
          CostScalar mcUEC = MCUL->lookup(theLargestSubset);

          if (mcUEC != csMinusOne) totalUecsForCurrentPredicatelessKeyColumnGroup = mcUEC;
        }

        currentPredicatelessKeyColumnGroup.clear();
      }

      totalUecsForPredicatelessKeyColumns *= totalUecsForCurrentPredicatelessKeyColumnGroup;
  ...
  switch (strategy) {
    ...
    case BOTH:
      return (presentKeyColumns > noOfmissingKeyColumns && totalUecsForPredicatelessKeyColumns < totalUEC_threshold);
...
class NewMDAMCostWA {
 public:
  NewMDAMCostWA(FileScanOptimizer &optimizer, NABoolean mdamForced, MdamKey *mdamKeyPtr, const Cost *costBoundPtr,
                const ValueIdSet &exePreds, const CostScalar &singleSubsetSize);
  void compute();
...
class NewMDAMOptimalDisjunctPrefixWA {
 public:
  NewMDAMOptimalDisjunctPrefixWA(FileScanOptimizer &optimizer, const ColumnOrderList &keyPredsByCol,
                                 const ValueIdSet &disjunctKeyPreds, const ValueIdSet &exePreds,
                                 const Histograms &outerHistograms, MdamKey *mdamKeyPtr,
                                 const ScanForceWildCard *scanForcePtr, NABoolean mdamForced,
                                 NABoolean isMultipleProbes, const CostScalar &incomingScanProbes,
                                 const CostScalar &estimatedRecordsPerBlock, const CostScalar &innerRowsUpperBound,
                                 const CostScalar &innerBlocksUpperBound, const CostScalar &singleSubsetSize,
 ...
 private:
  ...
  void calculateMetricsFromKeyPreds(const ValueId &keyColumn, const ValueIdSet *predsPtr, const CostScalar &maxUEC,
                                    CostScalar &UECFromPreds /*out*/, CostScalar &IntervalCountFromPreds /*out*/);

  void calculateMetricsFromKeyPred(const ValueId &keyColumn, const ValueId &keyPred, const CostScalar &maxUEC,
                                   CostScalar &UECFromPreds /*out*/, CostScalar &IntervalCountFromPreds /*out*/,
                                   int &lessCount /*in/out*/, int &greaterCount /*in/out*/);

  NABoolean keyColumnIsOnTheLeft(const ValueId &keyColumn, ItemExpr *ie);

  NABoolean isMinimalCost(Cost *currentCost, CollIndex columnPosition, NABoolean &forced /* out */);
...
class MDAMCostWA {
 public:
  MDAMCostWA(FileScanOptimizer &optimizer, NABoolean mdamForced, MdamKey *mdamKeyPtr, const Cost *costBoundPtr,
             const ValueIdSet &exePreds, SimpleCostVector &disjunctsFR, SimpleCostVector &disjunctsLR);
  void compute();
  NABoolean isMdamWon() const;
  NABoolean hasNoExePreds() const;
  const CostScalar &getNumKBytes() const;
...
class MDAMOptimalDisjunctPrefixWA {
 public:
  MDAMOptimalDisjunctPrefixWA(FileScanOptimizer &optimizer, const ColumnOrderList &keyPredsByCol,
                              const ValueIdSet &disjunctKeyPreds, const ValueIdSet &exePreds,
                              const Histograms &outerHistograms, IndexDescHistograms &firstColumnHistogram,
                              NABoolean &noExePreds, MdamKey *mdamKeyPtr, const ScanForceWildCard *scanForcePtr,
                              NABoolean mdamForced, NABoolean isMultipleProbes, const CostScalar &incomingProbes,
                              const CostScalar &estimatedRecordsPerBlock, const CostScalar &innerRowsUpperBound,
                              const CostScalar &innerBlocksUpperBound, CollIndex disjunctIndex);
  ...
  const CostScalar rcAfterApplyFirstKeyPreds() const { return rcAfterApplyFirstKeyPreds_; }
 ...
 private:
  ...
  CostScalar calculateUECFromKeyPreds(const ValueIdSet *predsPtr, CostScalar &maxUEC);
  CostScalar calculateUECFromKeyPred(const ValueId &keyPred, CostScalar &maxUEC, int &lessCount, int &greaterCount);
...
static int ScanOptimizerTest1(const FileScan &associatedFileScan, const CostScalar &resultSetCardinality,
  ...
  if (CURRSTMT_OPTDEFAULTS->reuseBasicCost()) {
    FileScanOptimizer scanOpt(associatedFileScan, resultSetCardinality, myContext, externalInputs);

    // Attempt to get a shared basic cost object.
    //
    SearchKey *searchKey = NULL;
    MdamKey *mdamKey = NULL;
    Cost *cost1 = scanOpt.optimize(searchKey, mdamKey);
    ...
    searchKey = NULL;
    mdamKey = NULL;
    Cost *cost2 = scanOpt.optimize(searchKey, mdamKey);
...
static int ScanOptimizerTest2(const FileScan &associatedFileScan, const CostScalar &resultSetCardinality,
  ...
  if (ScanOptimizer::useSimpleFileScanOptimizer(associatedFileScan, myContext, externalInputs)) {
    test2Cnt++;
    SimpleFileScanOptimizer simpleScanOpt(associatedFileScan, resultSetCardinality, myContext, externalInputs);

    FileScanOptimizer complexScanOpt(associatedFileScan, resultSetCardinality, myContext, externalInputs);
    ...
    Cost *cost1 = simpleScanOpt.optimize(searchKey1, mdamKey1);

    SearchKey *searchKey2 = NULL;
    MdamKey *mdamKey2 = NULL;
    Cost *cost2 = complexScanOpt.optimize(searchKey2, mdamKey2);
    ...
    if (mdamKey2) {
      ...
      if (et1 != et2) {
        ...
        CostScalar tc_et1 = cost1->getTotalCost().getElapsedTime(*(CURRSTMT_OPTDEFAULTS->getResourcePerformanceGoal()),
                                                                 CURRSTMT_OPTDEFAULTS->getDefaultCostWeight());
        CostScalar tc_et2 = cost2->getTotalCost().getElapsedTime(*(CURRSTMT_OPTDEFAULTS->getResourcePerformanceGoal()),
                                                                 CURRSTMT_OPTDEFAULTS->getDefaultCostWeight());
...
static int ScanOptimizerTest4(const FileScan &associatedFileScan, const CostScalar &resultSetCardinality,
  ...
  if (!ScanOptimizer::useSimpleFileScanOptimizer(associatedFileScan, myContext, externalInputs)) {
    ...
    //
    if (searchKey2 && (myContext.getInputLogProp()->getColStats().entries() == 0)) {
      test4Cnt++;
      SimpleFileScanOptimizer simpleScanOpt(associatedFileScan, resultSetCardinality, myContext, externalInputs);
      ...
      if (cost1->compareCosts(*cost2) != SAME) {
        ...
        if (et1 != et2) {
          ...
          CostScalar tc_et1 = cost1->getTotalCost().getElapsedTime(
              *(CURRSTMT_OPTDEFAULTS->getResourcePerformanceGoal()), CURRSTMT_OPTDEFAULTS->getDefaultCostWeight());
...
static int ScanOptimizerTest5(const FileScan &associatedFileScan, const CostScalar &resultSetCardinality,
  ...
  if (ScanOptimizer::useSimpleFileScanOptimizer(associatedFileScan, myContext, externalInputs)) {
    CostScalar repeatCount =
        myContext.getPlan()->getPhysicalProperty()->getDP2CostThatDependsOnSPP()->getRepeatCountForOperatorsInDP2();

    if ((repeatCount > 1.0) OR(myContext.getInputLogProp()->getColStats().entries() > 0)) {
      test5Cnt++;
      SimpleFileScanOptimizer simpleScanOpt(associatedFileScan, resultSetCardinality, myContext, externalInputs);
...

{
  ...
  if (testsToRun & 0x02) {
    ScanOptimizerTest2(associatedFileScan, resultSetCardinality, myContext, externalInputs, heap);
  }

  if (testsToRun & 0x04) {
    ScanOptimizerTest3(associatedFileScan, resultSetCardinality, myContext, externalInputs, heap);
  }

  if (testsToRun & 0x08) {
    ScanOptimizerTest4(associatedFileScan, resultSetCardinality, myContext, externalInputs, heap);
  }

  if (testsToRun & 0x10) {
    ScanOptimizerTest5(associatedFileScan, resultSetCardinality, myContext, externalInputs, heap);
  }
...
CostScalar getDP2CacheSizeInBlocks(const CostScalar &blockSizeInKb) {
  ...
  if (blockSizeInKb >= 64.)
    cacheSizeInBlocks = CostPrimitives::getBasicCostFactor(NCM_CACHE_SIZE_IN_BLOCKS);
  else if (blockSizeInKb == 32.)
    cacheSizeInBlocks = CostPrimitives::getBasicCostFactor(DP2_CACHE_32K_BLOCKS);
  else if (blockSizeInKb == 16.)
    cacheSizeInBlocks = CostPrimitives::getBasicCostFactor(DP2_CACHE_16K_BLOCKS);
  else if (blockSizeInKb == 8.)
    cacheSizeInBlocks = CostPrimitives::getBasicCostFactor(DP2_CACHE_8K_BLOCKS);
  else if (blockSizeInKb == 4.)
    cacheSizeInBlocks = CostPrimitives::getBasicCostFactor(DP2_CACHE_4096_BLOCKS);
  else if (blockSizeInKb == 2.)
    cacheSizeInBlocks = CostPrimitives::getBasicCostFactor(DP2_CACHE_2048_BLOCKS);
  else if (blockSizeInKb == 1.)
    cacheSizeInBlocks = CostPrimitives::getBasicCostFactor(DP2_CACHE_1024_BLOCKS);
  else if (blockSizeInKb == 0.5)
    cacheSizeInBlocks = CostPrimitives::getBasicCostFactor(DP2_CACHE_512_BLOCKS);
  else
    cacheSizeInBlocks = CostPrimitives::getBasicCostFactor(NCM_CACHE_SIZE_IN_BLOCKS);
...
//   None.
// -----------------------------------------------------------------------
void removeConstantsFromTargetSortKey(ValueIdList *targetSortKey, ValueIdMap *map) {
  // Map the target sort key cols to the source
  ValueIdList mappedTargetSortKey;
  map->rewriteValueIdListDown(*targetSortKey, mappedTargetSortKey);
  ...
  if (mappedTargetSortKey.entries() < targetSortKey->entries()) {
    ...
    map->rewriteValueIdListUp(remappedTargetSortKey, mappedTargetSortKey);
...
NABoolean isOrderedNJFeasible(ValueIdList leftKeys, ValueIdList rightKeys) {
  ...
  ValueId noInverseLKeyCol = lKeyCol.getItemExpr()->removeInverseOrder()->getValueId();
  NABoolean lKeyColIsDesc = FALSE;
  if (noInverseLKeyCol != lKeyCol) lKeyColIsDesc = TRUE;

  ValueId rKeyCol = rightKeys[0];
  // Remove any inverse node on the leading right table
  ValueId noInverseRKeyCol = rKeyCol.getItemExpr()->removeInverseOrder()->getValueId();
  NABoolean rKeyColIsDesc = FALSE;
  if (noInverseRKeyCol != rKeyCol) rKeyColIsDesc = TRUE;
  ...
  if (lKeyColIsDesc || rKeyColIsDesc) {
    if (lKeyColIsDesc) {
      ...
      if (lBaseColumn && lBaseColumn->getNAColumn()->isComputedColumn()) return FALSE;
    }
    if (rKeyColIsDesc) {
      BaseColumn *rBaseColumn = noInverseRKeyCol.castToBaseColumn();
      if (rBaseColumn && rBaseColumn->getNAColumn()->isComputedColumn()) return FALSE;
    }
  }
  ...
  if ((noInverseLKeyCol == noInverseRKeyCol) AND(lKeyColIsDesc == rKeyColIsDesc))
    return TRUE;
  else
    return FALSE;
}
...
NABoolean ordersMatch(const InputPhysicalProperty *ipp, const IndexDesc *indexDesc, const ValueIdList *innerOrder,
                      ...
                      NABoolean &probesForceSynchronousAccess, NABoolean noCmpAssert) {
  ...
  if ((ipp != NULL) AND(!(ipp->getAssumeSortedForCosting())) AND(!(ipp->getExplodedOcbJoinForCosting()))) {
    ...
    if ((ipp->getNjOuterOrder() == NULL) OR ipp->getNjOuterOrder()->isEmpty()) {
      if (NOT noCmpAssert) CCMPASSERT(FALSE);
      return FALSE;
    }
    ...
    const PartitioningFunction *physicalPartFunc = indexDesc->getPartitioningFunction();
    ...
    if ((ipp->getNjDp2OuterOrderPartFunc() == NULL) AND(physicalPartFunc != NULL))  // will be NULL for unpart tables
    {
      const RangePartitioningFunction *rpf = physicalPartFunc->castToRangePartitioningFunction();
      if (rpf != NULL) {
        ...
        leadingClustKeyCol = leadingClustKeyCol.getItemExpr()->removeInverseOrder()->getValueId();

        if (leadingClustKeyCol == leadingPartKeyCol) probesForceSynchronousAccess = TRUE;
    ...
    innerOrderProbeCols = innerOrder->findNJEquiJoinCols(ipp->getNjOuterCharOutputs(), charInputs, uncoveredCols);
    ...
    // be created.
    CollIndex maxInOrderCols = MINOF(njOuterOrder.entries(), innerOrderProbeCols.entries());
    fullyInOrder = TRUE;
    ...
    do {
      ...
      noInverseInnerOrderCol = innerOrderCol.getItemExpr()->removeInverseOrder()->getValueId();
      innerOrderProbeColsNoInv.insert(noInverseInnerOrderCol);
      innerOrderColIsDesc = FALSE;
      if (noInverseInnerOrderCol != innerOrderCol) innerOrderColIsDesc = TRUE;

      // Remove any inverse node on the leading outer order column
      // and remember if there was one.
      outerOrderCol = njOuterOrder[numInOrderCols];
      noInverseOuterOrderCol = outerOrderCol.getItemExpr()->removeInverseOrder()->getValueId();
      ...
      // the same. If one is DESC, they must both be DESC.
      if ((noInverseInnerOrderCol != noInverseOuterOrderCol) OR(innerOrderColIsDesc != outerOrderColIsDesc))
        fullyInOrder = FALSE;
  ...
  if (fullyInOrder) {
    ...
    const ColStatDescList &csdl = indexDesc->getPrimaryTableDesc()->getTableColStats();
    ...
    for (CollIndex keyColIndex = 0; keyColIndex < numInOrderCols; keyColIndex++) {
      ...
      if (NOT csdl.getColStatDescIndexForColumn(baseTableColIndex, innerOrderProbeColsNoInv[keyColIndex])) {
        if (NOT noCmpAssert) CCMPASSERT(FALSE);
        return FALSE;
      }
    ...
    CostScalar rowcountPerKeyValue = (rowcount / totalInOrderColsUec).getCeiling();

    CostScalar blockSizeInKb = indexDesc->getBlockSizeInKb();
    // Get the # of blocks for each unique in-order key value.
    CostScalar rowsPerBlock = (blockSizeInKb / indexDesc->getRecordSizeInKb()).getFloor();
    CostScalar blocksPerKeyValue = (rowcountPerKeyValue / rowsPerBlock).getCeiling();
...
//
// -----------------------------------------------------------------------
static NABoolean predReferencesColumn(const ItemExpr *predIEPtr, const ValueId &columnId) {
  ...
  if (colIdIEPtr->getOperatorType() == ITM_VEG_REFERENCE) {
    const VEG *exprVEGPtr = ((VEGReference *)colIdIEPtr)->getVEG();
  ...
  // predicate:
  for (ValueId colId = columnSet.init(); columnSet.next(colId); columnSet.advance(colId)) {
...
// a table
// -----------------------------------------------------------------------
static void computeBlocksUpperBound(CostScalar &totalBlocksUpperBound /* out */
                                    ,
                                    const CostScalar &totalRows, const CostScalar &recordsPerBlock) {
  totalBlocksUpperBound = CostScalar(totalRows / recordsPerBlock).getCeiling();
}
...
static void computeBeginBlocksLowerBound(CostScalar &beginBlocksLowerBound /* out */
                                         ,
                                         const CostScalar &uniqueSuccDataRqsts, const CostScalar &blocksUpperBound)

{
  beginBlocksLowerBound = CostScalar(MINOF(uniqueSuccDataRqsts.getCeiling().getValue(), blocksUpperBound.getValue()));
}
...
static void computeTotalBlocksLowerBound(CostScalar &totalBlocksLowerBound /* out */
                                         ,
                                         const CostScalar &uniqueSuccDataRqsts, const CostScalar &rowsPerSuccRqst,
                                         const CostScalar &recordsPerBlock, const CostScalar &innerBlocksUpperBound) {
  // -----------------------------------------------------------------------
  ...
  totalBlocksLowerBound = MINOF(
      (uniqueSuccDataRqsts +
       (uniqueSuccDataRqsts * ((rowsPerSuccRqst.getCeiling() - 1) / recordsPerBlock) + CostScalar(0.5)).getFloor()),
      innerBlocksUpperBound);
}

#ifndef NDEBUG

void ScanOptimizer::printCostObject(const Cost *costPtr) const {
  if (costPtr) {
    //    printf("Cost Vector: ");
    //    costPtr->print();

    printf("Elapsed time: ");
    ElapsedTime et = costPtr->convertToElapsedTime(getContext().getReqdPhysicalProperty());
    if (et <= 0.0) {
...
// -----------------------------------------------------------------------
Histograms::Histograms(const ColStatDescList &colStatDescList) : colStatDescList_(CmpCommon::statementHeap()) {
  // -----------------------------------------------------------------------
...
void Histograms::append(const ColStatDescSharedPtr &colStatDesc) {
  colStatDescList_.insertDeepCopyAt(entries(), colStatDesc);
}

void Histograms::append(const ColStatDescList &list) { colStatDescList_.appendDeepCopy(list, list.entries()); }
...
CostScalar Histograms::getRowCount() const {
  DCMPASSERT(getColStatDescList().entries() > 0);
  return getColStatDescList()[0]->getColStats()->getRowcount();
}

const ColStatDescList &Histograms::getColStatDescList() const { return colStatDescList_; }

NABoolean Histograms::containsAtLeastOneFake() const {
  return colStatDescList_.containsAtLeastOneFake();
}  // containsAtLeastOneFake()

NABoolean Histograms::getColStatDescForColumn(CollIndex index, const ValueId &column) const {
  return getColStatDescList().getColStatDescIndexForColumn(index, column);
}

const ColStats &Histograms::getColStatsForColumn(const ValueId &column) const {
  ColStatsSharedPtr colStatsPtr = getColStatDescList().getColStatsPtrForColumn(column);
...
ColStatsSharedPtr Histograms::getColStatsPtrForColumn(const ValueId &column) const {
...
void Histograms::displayHistogramForColumn(const ValueId &column) const {
  getColStatsForColumn(column).display();

}  // getColStatsForColumn(...)

void Histograms::applyPredicates(const ValueIdSet &predicates, const RelExpr &scan, const SelectivityHint *selHint,
                                 const CardinalityHint *cardHint, OperatorTypeEnum opType) {
  ...
  if (initialRowCount.isGreaterThanZero()) {
    ...
    colStatDescList_.estimateCardinality(initialRowCount, predicates, outerRefs, NULL, selHint, cardHint,
                                         numOuterColStats, unresolvedPreds, INNER_JOIN_MERGE, opType, NULL);
  }
...
//
// -----------------------------------------------------------------------
void Histograms::applyPredicatesWhenMultipleProbes(const ValueIdSet &predicates, const EstLogProp &inputEstLogProp,
                                                   const ValueIdSet &inputValues, const NABoolean isMDAM,
                                                   const SelectivityHint *selHint, const CardinalityHint *cardHint,
                                                   NABoolean *indexJoin, OperatorTypeEnum opType) {
  ...
  if (NOT isMDAM AND(indexNestedJoin = isAnIndexJoin(inputEstLogProp, &innerIndexColumnsToRemove))) {
  ...
  if (indexJoin != NULL) *indexJoin = indexNestedJoin;
  ...
  if (entries() > 0) innerRowCount = ((*this)[0]).getColStats()->getRowcount();
  ...
  if (indexNestedJoin) {
    ...
    for (CollIndex i = outerRefCount; i < entries(); i++) {
      ...
      if (oldCount != innerRowCount) columnStatDesc->synchronizeStats(oldCount, innerRowCount);
    }
  ...
  innerRowCount = innerColStatsList.estimateCardinality(innerRowCount,   /*in*/
                                                        predicates,      /*in*/
                                                        outerReferences, /*in*/
                                                        NULL, selHint,   /*in*/
                                                        cardHint,        /*in*/
...
void Histograms::applyPredicate(const ValueId &predicate, const RelExpr &scan, const SelectivityHint *selHint,
                                const CardinalityHint *cardHint, OperatorTypeEnum opType) {
  ValueIdSet vis;
...
NABoolean Histograms::isAnIndexJoin(const EstLogProp &inputEstLogProp,
  ...
  NABoolean indexNestedJoin = FALSE;
  if ((inputEstLogProp.getResultCardinality().isGreaterThanZero())AND NOT inputEstLogProp.getColStats().isEmpty())

  {
    ...
    for (CollIndex i = 0; i < outerColStatsList.entries(); i++) {
      ...
      for (CollIndex j = 0; j < innerColStatsList.entries(); j++) {
        ...
        if (outerStatDesc->getMergeState().contains(innerColumnStatDesc->getMergeState())) {
      ...
      if ((innerIndexColumnListPtr != NULL) AND indexNestedJoin) {
...
void Histograms::display() const { print(); }  // display()

void Histograms::print(FILE *ofd, const char *prefix, const char *suffix) const {
#ifndef NDEBUG
...
// created.
// -----------------------------------------------------------------------
IndexDescHistograms::IndexDescHistograms(const IndexDesc &indexDesc, const CollIndex columnPosition)
...
void IndexDescHistograms::appendHistogramForColumnPosition(const CollIndex &columnPosition) {
  ...
  if (columnPosition > 0) {
    ...
    const ColStatDescList &primaryTableCSDL = getIndexDesc().getPrimaryTableDesc()->getTableColStats();
    const ValueIdList &keyColumns = getIndexDesc().getIndexKey();

    DCMPASSERT(columnPosition > 0 AND columnPosition <= keyColumns.entries());

    CollIndex j;
    // remember that columnPosition is one based:
    if (primaryTableCSDL.getColStatDescIndexForColumn(j, keyColumns[columnPosition - 1])) {
      ...
      if (NOT contains(column)) {
        ...
        if (entries() > 1) {
          ColStatDesc &lastHistogram = *getColStatDescList()[entries() - 1];
          const ColStatDesc &previousToLastHistogram = *getColStatDescList()[entries() - 2];
          lastHistogram.synchronizeStats(lastHistogram.getColStats()->getRowcount(),
                                         previousToLastHistogram.getColStats()->getRowcount());
        }
      }
    } else {
...
NABoolean IndexDescHistograms::isMultiColUecInfoAvail() const {
  return getIndexDesc().getPrimaryTableDesc()->getTableColStats().getUecList()->containsMCinfo();
}
...
//-----------------------------------------------------------------------------
NABoolean IndexDescHistograms::estimateUecUsingMultiColUec(const ColumnOrderList &keyPredsByCol, /*in*/
  ...
  const MultiColumnUecList *MCUL = getIndexDesc().getPrimaryTableDesc()->getTableColStats().getUecList();
#ifndef NDEBUG
  if (getenv("MDAM_MCUEC")) {
  ...
  // columnList only
  LIST(ValueIdSet) *listOfSubsets = MCUL->getListOfSubsetsContainsColumns(columnList, uecCount);
  ...
  idInBaseCol =
      ((BaseColumn *)((IndexColumn *)(columnList[indexOfColumn].getItemExpr()))->getDefinition().getItemExpr())
          ->getValueId();
  ...
  for (CollIndex j = 0; j < listOfSubsets->entries(); j++) {
    ...
    if (perfectDenom AND(vidSet.entries() > entriesInSubset OR(vidSet.entries() == entriesInSubset AND MCUL->lookup(
                                                                                       vidSet) > uecWithoutColumn))) {
      perfectDenom = FALSE;
...
CostScalar IndexDescHistograms::computeFailedProbes(const Histograms &outerHistograms, const ValueIdSet &keyPreds,
                                                    const ValueIdSet &inputValues,
                                                    const ValueIdSet &operatorValues) const

{
  CostScalar maxFailedProbes = csZero;
  ...
  CostScalar currentFailedProbes = 0;
  for (ValueId keyPred = keyPreds.init(); keyPreds.next(keyPred); keyPreds.advance(keyPred)) {
    if (keyPred.getItemExpr()->isANestedJoinPredicate(inputValues, operatorValues)) {
      ...
      outerColStats = outerHistograms.getColStatsPtrForPredicate(keyPred);
      if (outerColStats != NULL) {
        // It is a joined predicate!
        // find the joined hist. for the predicate:
        joinedColStats = this->getColStatsPtrForPredicate(keyPred);
        DCMPASSERT(joinedColStats != NULL);
        // find the failed probes for these histograms:
        currentFailedProbes = outerColStats->countFailedProbes(joinedColStats);
      } else {
...
ScanOptimizer::ScanOptimizer(const FileScan &associatedFileScan, const CostScalar &resultSetCardinality,
...
//
NABoolean ScanOptimizer::useSimpleFileScanOptimizer(const FileScan &associatedFileScan, const Context &myContext,
                                                    const ValueIdSet &externalInputs) {
#ifndef NDEBUG
  if (CmpCommon::getDefault(MDAM_TRACING) == DF_ON) MdamTrace::setLevel(MTL2);
  ...
  if ((CmpCommon::getDefault(RANGESPEC_TRANSFORMATION) == DF_ON) && exePreds.entries()) {
  ...
  const LogPhysPartitioningFunction *logPhysPartFunc =
      myContext.getPlan()->getPhysicalProperty()->getPartitioningFunction()->castToLogPhysPartitioningFunction();
  ...
  if (!isHbaseTable && histograms.containsAtLeastOneFake()) {
    ...
    if (disjuncts.entries() > MDAM_MAX_NUM_DISJUNCTS) {
      ...
      if (histograms.isAnIndexJoin(*(myContext.getInputLogProp()))) return TRUE;
    }
  }
  ...
  if (exePreds.entries() > 0) {
    SearchKey searchKey(indexDesc->getIndexKey(), indexDesc->getOrderOfKeyValues(), externalInputs,
                        (NOT associatedFileScan.getReverseScan()), exePreds, *curDisjuncts, nonKeyColumnSet, indexDesc);

    if (CmpCommon::getDefault(MDAM_FSO_SIMPLE_RULE) == DF_ON) {
      // Quickly-computed special cases

      if (searchKey.isUnique()) return TRUE;  // unique access, don't need to consider MDAM

      if (exePreds.entries() == 0)  // if searchKey consumed all executor preds
        return TRUE;                // then MDAM can't do better; don't consider

      if (searchKey.getKeyPredicates().entries() == 0) return FALSE;  // single subset is a full table scan, so try MDAM
      ...
      for (CollIndex i = 0; i < keyColumns.entries(); i++) {
        ...
        if (baseColumns.contains(baseVid)) return FALSE;  // key column occurs in exePreds; so consider MDAM
      }
    ...
    if ((!searchKey.isUnique()) && (searchKey.getKeyPredicates().entries() > 0)) {
      ...
      NABoolean itIsSingleSubset = keyPredsByCol.getSingleSubsetOrder(singleSubsetPrefixColumn);
      ...
      if (CmpCommon::getDefault(RANGESPEC_TRANSFORMATION) == DF_ON) {
        if ((singleSubsetPrefixColumn + 1 < keyPredsByCol.entries()) && (curDisjuncts->entries() >= 1) &&
            curDisjuncts->containsAndorOrPredsInRanges()) {
          return FALSE;
        }
      } else {
        if ((singleSubsetPrefixColumn + 1 < keyPredsByCol.entries()) && (curDisjuncts->entries() > 1)) {
    ...
    if (CmpCommon::getDefault(RANGESPEC_TRANSFORMATION) == DF_ON) {
      if ((searchKey.getKeyPredicates().entries() == 0) && (curDisjuncts->entries() >= 1) &&
          curDisjuncts->containsAndorOrPredsInRanges()) {
        return FALSE;
      }
    } else {
...
NABoolean ScanOptimizer::canStillConsiderMDAM(const ValueIdSet partKeyPreds, const ValueIdSet nonKeyColumnSet,
                                              const Disjuncts &curDisjuncts, const IndexDesc *indexDesc,
                                              const ValueIdSet externalInputs) {
  ...
  if (NOT indexDesc->getNAFileSet()->isKeySequenced()) {
    ...
    CollIndex i = 0;
    MdamKey mdamKey(indexKey, externalInputs, curDisjuncts, nonKeyColumnSet, indexDesc);
    for (i = 0; i < mdamKey.getKeyDisjunctEntries(); i++) {
      ...
      if (preds.isEmpty()) {
        ...
        // don't consider MDAM
        canDoMdam = FALSE;
        MDAM_DEBUG1(MTL2, "disjunct[%d] has no key predicate, MDAM NOT considered", i);
        break;
      }
    }
  }
...
ScanOptimizer *ScanOptimizer::getScanOptimizer(const FileScan &associatedFileScan,
                                               const CostScalar &resultSetCardinality, const Context &myContext,
                                               const ValueIdSet &externalInputs, CollHeap *heap) {
#ifndef NDEBUG
  NADefaults &defs = ActiveSchemaDB()->getDefaults();
  if (defs.getAsULong(FSO_RUN_TESTS) > 0) {
    // Run all Scan Optimizer Tests before constructing one
    //
    ScanOptimizerAllTests(associatedFileScan, resultSetCardinality, myContext, externalInputs, heap);
  }
#endif

  if (useSimpleFileScanOptimizer(associatedFileScan, myContext, externalInputs)) {
    return new (heap) SimpleFileScanOptimizer(associatedFileScan, resultSetCardinality, myContext, externalInputs);
  } else {
    return new (heap) FileScanOptimizer(associatedFileScan, resultSetCardinality, myContext, externalInputs);
  }
}
...
//
void ScanOptimizer::setNumberOfBlocksToReadPerAccess(const CostScalar &blocks) {
...
CollIndex ScanOptimizer::getNumActivePartitions() const {
  ...
  PartitioningFunction *pf = getContext().getPlan()->getPhysicalProperty()->getPartitioningFunction();
...
CollIndex ScanOptimizer::getEstNumActivePartitionsAtRuntime() const {
  ...
  if (pf->isALogPhysPartitioningFunction()) {
    ...
    if (phyPartFunc != NULL) {
      ...
      if ((phyPartFunc->isARangePartitioningFunction())OR(phyPartFunc->isATableHashPartitioningFunction())) {
        actParts = ((NodeMap *)(pf->getNodeMap()))->getEstNumActivePartitionsAtRuntime();
      }
    }
  }

  if (actParts > 1) actParts = MINOF(actParts, getFileScan().getComputedNumOfActivePartiions());
...
CollIndex ScanOptimizer::getEstNumActivePartitionsAtRuntimeForHbaseRegions() const {
  ...
  if (estActParts == 1 AND(CmpCommon::getDefault(NCM_HBASE_COSTING) == DF_ON)) return estActParts;
  ...
  if (estActParts == 1 AND(CmpCommon::getDefault(NCM_HBASE_COSTING) == DF_ON)) actParts = estActParts;
...
//    Too many disjuncts
//
ScanForceWildCard::scanOptionEnum ScanOptimizer::getMdamStatus(const FileScan &fileScan, const Context &myContext) {
  // Soln:10-050620-8905
  // Executor does not support MDAM for streams due to some limitations in executor and DP2,
  // hence MDAM is disabled for streams.

  if (fileScan.getGroupAttr()->isStream()) return ScanForceWildCard::MDAM_OFF;
  ...
  if ((cstrMDAMStatus != NULL) && (strcmp(cstrMDAMStatus, "OFF") == 0)) {
    return ScanForceWildCard::MDAM_OFF;
  } else if ((cstrMDAMStatus != NULL) && (strcmp(cstrMDAMStatus, "ON") == 0)) {
  ...
  if (propertyPtr && propertyPtr->getMustMatch() &&
    ...
    if (scanForcePtr->getMdamStatus() == ScanForceWildCard::MDAM_FORCED) return ScanForceWildCard::MDAM_FORCED;
  }

  // Is Mdam Forced ON by a CONTROL TABLE statement for this table.
  //
  const NAString *val =
      ActiveControlDB()->getControlTableValue(fileScan.getIndexDesc()->getNAFileSet()->getExtFileSetName(), "MDAM");
  if ((val) && (*val == "ON")) {
    return ScanForceWildCard::MDAM_FORCED;
  }
  ...
  //
  if (CmpCommon::getDefault(MDAM_SCAN_METHOD) == DF_OFF) return ScanForceWildCard::MDAM_OFF;
  ...
  if (propertyPtr && propertyPtr->getMustMatch() &&
      (propertyPtr->getMustMatch()->getOperatorType() == REL_FORCE_ANY_SCAN)) {
    ScanForceWildCard *scanForcePtr = (ScanForceWildCard *)propertyPtr->getMustMatch();
    if (scanForcePtr->getMdamStatus() == ScanForceWildCard::MDAM_OFF) return ScanForceWildCard::MDAM_OFF;
  }

  // Is Mdam Forced OFF by a CONTROL TABLE statement for this table.
  //
  val = ActiveControlDB()->getControlTableValue(fileScan.getIndexDesc()->getNAFileSet()->getExtFileSetName(), "MDAM");
  if ((val) && (*val == "OFF")) {
    return ScanForceWildCard::MDAM_OFF;
  }
  ...
  // Force MDAM for ngram index
  if (fileScan.isCreatedByNgram() && fileScan.getIndexDesc()->isNgramIndex()) {
    return ScanForceWildCard::MDAM_FORCED;
  }
...
//
NABoolean ScanOptimizer::isMdamForced(const FileScan &fileScan, const Context &myContext) {
  // Soln:10-050620-8905
  // Executor does not support MDAM for streams due to some limitations in executor and DP2,
  // hence MDAM is disabled for streams.

  if (fileScan.getGroupAttr()->isStream()) return FALSE;
  ...
  if (propertyPtr && propertyPtr->getMustMatch() &&
      (propertyPtr->getMustMatch()->getOperatorType() == REL_FORCE_ANY_SCAN)) {
    ScanForceWildCard *scanForcePtr = (ScanForceWildCard *)propertyPtr->getMustMatch();
    if (scanForcePtr->getMdamStatus() == ScanForceWildCard::MDAM_FORCED) return TRUE;
    // else return FALSE;
  }
  // else
  // return FALSE;
  const NAString *val =
      ...
      ActiveControlDB()->getControlTableValue(
          fileScan.getIndexDesc()->getPrimaryTableDesc()->getCorrNameObj().getUgivenName(), "MDAM");
  // ct-bug-10-030102-3803 -End
  if ((val) && (*val == "ON")) return TRUE;
...
NABoolean ScanOptimizer::isMdamEnabled() const {
  ...
  if (getRelExpr().getGroupAttr()->isStream()) mdamIsEnabled = FALSE;
  ...
  // -----------------------------------------------------------------------
  if (mdamIsEnabled) {
    const ReqdPhysicalProperty *propertyPtr = getContext().getReqdPhysicalProperty();
    if (propertyPtr && propertyPtr->getMustMatch() &&
        (propertyPtr->getMustMatch()->getOperatorType() == REL_FORCE_ANY_SCAN)) {
      ScanForceWildCard *scanForcePtr = (ScanForceWildCard *)propertyPtr->getMustMatch();
  ...
  if (mdamIsEnabled) {
    const NAString *val =
        // ct-bug-10-030102-3803 -Begin
        ActiveControlDB()->getControlTableValue(getIndexDesc()->getPrimaryTableDesc()->getCorrNameObj().getUgivenName(),
                                                "MDAM");
    // ct-bug-10-030102-3803 -End
  ...
  NABoolean isHbaseNativeTable = idesc->getPrimaryTableDesc()->getNATable()->isHbaseCellTable() ||
                                 idesc->getPrimaryTableDesc()->getNATable()->isHbaseRowTable();
  if (isHbaseNativeTable) mdamIsEnabled = FALSE;

  // disable MDAM for ORC, parquet and avro tables
  if (idesc->getPrimaryTableDesc()->getNATable()->isExtStorage()) mdamIsEnabled = FALSE;
  ...
  if (idesc == idesc->getPrimaryTableDesc()->getClusteringIndex()) {
    const ValueIdSet divisioningColumns = idesc->getPrimaryTableDesc()->getDivisioningColumns();

    if (divisioningColumns.entries() > 0 && (CmpCommon::getDefault(MTD_GENERATE_CC_PREDS) == DF_ON))
  ...
  if (mdamIsEnabled) {
    ...
    NABoolean isInputCardinalityOne = getContext().getInputLogProp()->isCardinalityEqOne();
    if (isInputCardinalityOne) return TRUE;

    /*
     Right side Scan of a Nested Join will use MDAM disjuncts if and only if
     MDAM_SCAN_METHOD CQD's value is MAXIMUM.
    */
    if ((repeatCount.isGreaterThanOne())OR(getContext().getInputLogProp()->getColStats().entries() > 0) &&
...
const CostScalar ScanOptimizer::getIndexLevelsSeeks() const {
...
Cost *ScanOptimizer::computeCostObject(const SimpleCostVector &firstRow /* IN */
                                       ,
  ...
  {
    PartitioningFunction *partFunc = getContext().getPlan()->getPhysicalProperty()->getPartitioningFunction();
    ...
    if (partFunc->castToSinglePartitionPartitioningFunction())
      ...
      if (fileScan.isHbaseTable() || fileScan.isHiveTable() || fileScan.isSeabaseTable())
    ...
    countOfPAs = MINOF(numOfDP2Volumes, countOfPAs);
  ...
  partitionsPerCPU = ((CostScalar)countOfPAs / (CostScalar)countOfCPUsExecutingDP2s).getCeiling();
...
#ifndef NDEBUG
  if (planFragmentsPerCPU > 1 AND CURRSTMT_OPTGLOBALS->synCheckFlag) (*CURRSTMT_OPTGLOBALS->asynchrMonitor).exit();
#endif  // NDEBUG
...
FileScanBasicCost *ScanOptimizer::shareBasicCost(NABoolean &sharedCostFound) {
  const Context &currentContext = getContext();
  IndexDesc *indexDesc = (IndexDesc *)getIndexDesc();
  FileScanCostList *fileScanCostList = (FileScanCostList *)indexDesc->getBasicCostList();
  ...
  FileScanBasicCost *newBasicCostPtr =
      new (CmpCommon::statementHeap()) FileScanBasicCost((const Context *)&currentContext);
...
Cost *FileScanOptimizer::optimize(SearchKey *&searchKeyPtr /* out */
                                  ,
  ...
  IndexDescHistograms histograms(*getIndexDesc(), getIndexDesc()->getIndexKey().entries());
  ...
  NABoolean isIndexJoin = histograms.isAnIndexJoin(*(getContext().getInputLogProp()));
    ...
    if (isMdamEnabled() AND NOT mdamForced) {
      if (isIndexJoin || hasTooManyDisjuncts()) {
        ...
        if (histograms.containsAtLeastOneFake()) {
          ...
          if (NOT partKeyPreds.isEmpty()) {
            canDoMdam = ScanOptimizer::canStillConsiderMDAM(partKeyPreds, nonKeyColumnSet, *curDisjuncts, indexDesc,
                                                            externalInputs);
          } else {
            canDoMdam = ScanOptimizer::canStillConsiderMDAM(partKeyPreds, nonKeyColumnSet, getDisjuncts(), indexDesc,
                                                            externalInputs);
          }
        }
      }
  ...
  // if MDAM is forced.
  NABoolean singleSubsetMetricsNeeded = (CmpCommon::getDefault(MDAM_COSTING_REWRITE) == DF_ON);

  if (mdamForced  // Force mdam case:
          AND NOT singleSubsetMetricsNeeded AND getIndexDesc()
              ->getNAFileSet()
              ->isKeySequenced()) {  // Mdam is our only choice:

    MDAM_DEBUG0(MTL2, "MDAM only (forced).");

    // Create mdam disjuncts key:
    if (partKeyPreds.isEmpty())
      mdamKeyPtr = new HEAP MdamKey(getIndexDesc()->getIndexKey(), getExternalInputs(), getDisjuncts(), nonKeyColumnSet,
                                    getIndexDesc());
    else
      mdamKeyPtr = new HEAP MdamKey(getIndexDesc()->getIndexKey(), getExternalInputs(), *curDisjuncts, nonKeyColumnSet,
                                    getIndexDesc());

    DCMPASSERT(mdamKeyPtr != NULL);
    // Cost the mdam disjuncts:

    winnerCostPtr = computeCostForMultipleSubset(mdamKeyPtr, NULL, mdamForced, winnerCostPtrNumKBytes,
  ...
  else if (NOT isMdamEnabled() OR NOT canDoMdam) {
    ...
    if (partKeyPreds.isEmpty())
      searchKeyPtr = new (CmpCommon::statementHeap())
          SearchKey(getIndexDesc()->getIndexKey(), getIndexDesc()->getOrderOfKeyValues(), getExternalInputs(),
                    isForwardScan(), exePred, getDisjuncts(), nonKeyColumnSet, getIndexDesc());
    else
      searchKeyPtr = new (CmpCommon::statementHeap())
          SearchKey(getIndexDesc()->getIndexKey(), getIndexDesc()->getOrderOfKeyValues(), getExternalInputs(),
                    isForwardScan(), exePred, *curDisjuncts, nonKeyColumnSet, getIndexDesc());

    winnerCostPtr = computeCostForSingleSubset(*searchKeyPtr, FALSE /* do not break */
                                               ,
    ...
    Cost *singleSubsetCostPtr = NULL, *mdamCommonCostPtr = NULL, *mdamDisjunctsCostPtr = NULL;

    CostScalar singleSubsetCostPtrNumKBytes, mdamCommonCostPtrNumKBytes, mdamDisjunctsCostPtrNumKBytes;

    MdamKey *mdamCommonKeyPtr = NULL, *mdamDisjunctsKeyPtr = NULL;
    ...
    winnerCostPtr = singleSubsetCostPtr =
        computeCostForSingleSubset(*searchKeyPtr, TRUE /* Break if there is a conflict */
                                   ,
                                   singleSubsetCostPtrNumKBytes /* output */
        );
    winnerCostPtrNumKBytes = singleSubsetCostPtrNumKBytes;

    MDAM_DEBUGX(MTL2, MdamTrace::printCostObject(this, singleSubsetCostPtr, "Single Subset Cost"));
    ...
    if ((NOT partKeyPreds.isEmpty() AND curDisjuncts->entries() > 1) OR(getDisjuncts().entries() > 1)) {
      ...
      if (NOT commonPreds.isEmpty()) {
        ...
        Disjuncts *commonDisjunctsPtr = new HEAP MaterialDisjuncts(commonPreds);
        DCMPASSERT(commonDisjunctsPtr != NULL);

        mdamCommonKeyPtr = new HEAP MdamKey(getIndexDesc()->getIndexKey(), getExternalInputs(), *commonDisjunctsPtr,
                                            nonKeyColumnSet, getIndexDesc());
        ...
        if (NOT keyPreds.isEmpty()) {
          ...
          mdamCommonCostPtr = computeCostForMultipleSubset(mdamCommonKeyPtr, winnerCostPtr /* bound */
                                                           ,
                                                           mdamForced, mdamCommonCostPtrNumKBytes /* output */
                                                           ,
          ...
          MDAM_DEBUGX(MTL2, MdamTrace::printCostObject(this, mdamCommonCostPtr, "MDAM Common Cost"));
        }  // there are  key preds in common preds
        else {
          MDAM_DEBUG0(MTL2, "Common predicates has no key predicate, MDAM NOT considered");
      ...
      if (mdamCommonCostPtr != NULL) {
        ...
        winnerCostPtr = singleSubsetCostPtr;
        winnerCostPtrNumKBytes = singleSubsetCostPtrNumKBytes;
        if (mdamCommonKeyPtr != sharedMdamCommonKeyPtr) delete mdamCommonKeyPtr;
    ...
    DCMPASSERT((searchKeyPtr == NULL AND mdamCommonKeyPtr != NULL)
                   OR(searchKeyPtr != NULL AND mdamCommonKeyPtr == NULL));
    ...
    if (partKeyPreds.isEmpty())
      // Chose either the new disjuncts w/partitioning keys if we have
      //   partitoning keys, otherwise use the original
      mdamDisjunctsKeyPtr = new HEAP MdamKey(getIndexDesc()->getIndexKey(), getExternalInputs(), getDisjuncts(),
                                             nonKeyColumnSet, getIndexDesc());
    else
      mdamDisjunctsKeyPtr = new HEAP MdamKey(getIndexDesc()->getIndexKey(), getExternalInputs(), *curDisjuncts,
                                             nonKeyColumnSet, getIndexDesc());
    ...
    mdamDisjunctsCostPtr = computeCostForMultipleSubset(mdamDisjunctsKeyPtr, winnerCostPtr /* cost bound */
                                                        ,
                                                        mdamForced, mdamDisjunctsCostPtrNumKBytes /* output */
                                                        ,
                                                        eliminateExePreds  // TRUE eliminate exePreds if possible
                                                        ,
    ...
    MDAM_DEBUGX(MTL2, MdamTrace::printCostObject(this, mdamDisjunctsCostPtr, "MDAM Disjuncts Cost"));
    ...
    if (mdamDisjunctsCostPtr != NULL) {
      ...
      if (mdamDisjunctsKeyPtr != sharedMdamDisjunctsKeyPtr) delete mdamDisjunctsKeyPtr;
  ...
  MDAM_DEBUGX(MTL2, MdamTrace::printCostObject(this, winnerCostPtr, "Winner Cost"));

  // compute blocks read per access
  computeNumberOfBlocksToReadPerAccess(*winnerCostPtr, mdamIsWinner, winnerCostPtrNumKBytes);
#ifndef NDEBUG
  if (CURRSTMT_OPTDEFAULTS->optimizerHeuristic2()) {
    if (isIndexJoin) (*CURRSTMT_OPTGLOBALS->indexJoinMonitor).exit();
  }
#endif  // NDEBUG
...
void FileScanOptimizer::computeNumberOfBlocksToReadPerAccess(const Cost &scanCost, NABoolean &isMDAM,
  ...
  // probes:
  CostScalar blocksForAllProbes = (numKBytes / blockSizeInKb).getCeiling();

  CostScalar probes = MIN_ONE(scanCost.getCplr().getNumProbes());

  // Substract index blocks to obtain the data blocks:
  CostScalar indexBlocksForAllProbes = getIndexDesc()->getEstimatedIndexBlocksLowerBound(probes);

  CostScalar dataBlocksForAllProbes = MIN_ONE(blocksForAllProbes - indexBlocksForAllProbes);
  ...
  // -------------------------------------------------------------
  const CostScalar totalRowsInInnerTable = getRawInnerHistograms().getRowCount().getCeiling();

  const CostScalar estimatedRecordsPerBlock = getIndexDesc()->getEstimatedRecordsPerBlock();

  CostScalar totalBlocksInFullTableScan;
  computeBlocksUpperBound(totalBlocksInFullTableScan /* out */
                          ,
  ...
  // compromise.
    ...
    const CostScalar density = CostPrimitives::getBasicCostFactor(COST_PROBE_DENSITY_THRESHOLD);
    ...
    if (dataBlocksForAllProbes / totalBlocksInFullTableScan >= density) {
      ...
      // table size):
      blocksToRead = MINOF(totalBlocksInFullTableScan.getValue(), dataBlocksForAllProbes.getValue());
  ...
  else {  // NJ, single subset, random, maybe multiple random probe MDAM
...
// -----------------------------------------------------------------------
Cost *FileScanOptimizer::computeCostForSingleSubset(SearchKey &searchKey, const NABoolean &weAreConsideringMDAM,
                                                    CostScalar &numKBytes)

{
  ...
  SimpleCostVector &firstRow = fileScanBasicCostPtr->getFRBasicCostSingleSubset();
  SimpleCostVector &lastRow = fileScanBasicCostPtr->getLRBasicCostSingleSubset();
  ...
  for (CollIndex i = 0; i <= singleSubsetPrefixColumn; i++) {
    if (i == leadingPartPreds) {
      ...
      if (partPreds AND NOT partPreds->isEmpty()) {
        ValueId predId;
        BiRelat *predIEptr;
        for (predId = partPreds->init(); partPreds->next(predId); partPreds->advance(predId)) {
    ...
    if (keyPredsByCol.getPredicateExpressionPtr(i)) {
      NABoolean unUsablePred =
          ((keyPredsByCol.getPredicateExpressionPtr(i)->getType() == KeyColumns::KeyColumn::CONFLICT)
               OR(keyPredsByCol.getPredicateExpressionPtr(i)->getType() == KeyColumns::KeyColumn::CONFLICT_EQUALS));
      if (weAreConsideringMDAM AND unUsablePred) {
#ifndef NDEBUG
        (*CURRSTMT_OPTGLOBALS->singleSubsetCostMonitor).exit();
#endif  // NDEBUG
        MDAM_DEBUG0(MTL2, "Single Subset Cost is NULL, conflicts in predicate.");
  ...
  // -----------------------------------------------------------------------
  CostScalar seeks  // total number of disk arm movements
      ,
  ...
  if (sharedCostFound AND firstRow.getCPUTime() > csZero AND lastRow.getCPUTime() > csZero) {
    if (CURRSTMT_OPTDEFAULTS->reuseBasicCost()) {
  ...
  // partitions
  CostScalar probes = repeatCount * getNumActivePartitions(),
  ...
  if ((CmpCommon::getDefault(COMP_BOOL_34) == DF_OFF) AND searchKey.isUnique()) finalRows = probes;
  ...
  if ((repeatCount.isGreaterThanOne())OR(getContext().getInputLogProp()->getColStats().entries() > 0)) {
    ...
    computeBlocksUpperBound(innerBlocksUpperBound, totalRowsInInnerTable, estimatedRecordsPerBlock);
    ...
    Histograms outerHistograms(getContext().getInputLogProp()->getColStats());

    // Then get the histograms for the inner table (this scan):
    IndexDescHistograms innerHistograms(*getIndexDesc(), singleSubsetPrefixColumn + 1);
    ...
    *dataRows = innerHistograms.getRowCount();
    ...
    categorizeProbes(successfulProbes /* out */
                     ...
                     failedProbes /* out */
                     ,
                     uniqueFailedProbes, probes, singleSubsetPreds, outerHistograms, FALSE  // this is not MDAM!
                     ,
                     dataRows);
    CostScalar uniqueProbes = uniqueSuccProbes + uniqueFailedProbes;
    ...
    if ((CmpCommon::getDefault(COMP_BOOL_34) == DF_OFF) AND searchKey.isUnique() AND outerHistograms.isEmpty())
    ...
    CostScalar blksPerSuccProbe = (rowsPerSuccessfulProbe / estimatedRecordsPerBlock).getCeiling();
    ...
    const CostScalar totalUniqueSuccProbes = uniqueSuccProbes * getNumActivePartitions();
    CostScalar totalBlocksLowerBound;                  // the blocks in the answer set
    computeTotalBlocksLowerBound(totalBlocksLowerBound /* out */
                                 ,
                                 totalUniqueSuccProbes, rowsPerSuccessfulProbe, estimatedRecordsPerBlock,
                                 innerBlocksUpperBound /* the total blocks in the inner table */
    );

    CostScalar cacheSizeInBlocks = getDP2CacheSizeInBlocks(blockSizeInKb);
    CostScalar indexBlocksLowerBound = getIndexDesc()->getEstimatedIndexBlocksLowerBound(probes);
    ...
    computeBeginBlocksLowerBound(beginBlocksLowerBound /* out */
                                 ,
                                 uniqueSuccProbes, innerBlocksUpperBound);
    ...
    if ((ippForMe != NULL) AND ordersMatch(ippForMe, getIndexDesc(), &(getIndexDesc()->getOrderOfKeyValues()),
                                           getRelExpr().getGroupAttr()->getCharacteristicInputs(), partiallyInOrderOK,
                                           probesForceSynchronousAccess)) {
      setInOrderProbesFlag(TRUE);
      setProbesForceSynchronousAccessFlag(probesForceSynchronousAccess);
    } else if ((ippForMe != NULL) AND(ippForMe->getAssumeSortedForCosting())
                   AND(!(ippForMe->getExplodedOcbJoinForCosting()))
                       AND(ippForMe->getNjOuterOrderPartFuncForNonUpdates() == NULL))
    ...
    CostScalar cacheForAllVolumes = cacheSizeInBlocks * getNumActiveDP2Volumes() / concurrentUsers;
    ...
    if (hasAtleastOneConstExpr) {
      IndexDescHistograms innerHistograms(*getIndexDesc(), columnWithAConstExpr + 1);
    ...
    if (totalBlocksLowerBound <= cacheForAllVolumes) {
      ...
      computeIOForFullCacheBenefit(seeks /* out */
                                   ...
                                   seq_kb_read /* out */
                                   ,
                                   beginBlocksLowerBound, totalBlocksLowerBound, indexBlocksLowerBound);
      computeSeekForDp2ReadAheadAndProbeOrder(seekComputedWithDp2ReadAhead, finalRows, uniqueProbes,
                                              beginBlocksLowerBound, totalBlocksLowerBound, innerBlocksInSequence,
                                              cacheForAllVolumes, inOrderProbes);

      seeks = indexBlocksLowerBound + seekComputedWithDp2ReadAhead.getCeiling();
      ...
      if (getInOrderProbesFlag()) {
        ...
        if (NOT duplicateSuccProbes.isGreaterThanZero()) {
          ...
          computeIOForFullCacheBenefit(seeks, seq_kb_read, beginBlocksLowerBound, totalBlocksLowerBound,
        ...
        } else  // in order but duplicates exist
        {
          ...
          } else  // data blocks per duplicate does not fit in cache!
          {
            ...
            // duplicates:
            const CostScalar extraDuplicateProbes = duplicateSuccProbes;
            const CostScalar extraDuplicateBlocks = extraDuplicateProbes * blksPerSuccProbe;

            const CostScalar seqBlocksRead =
                CostScalar(totalBlocksLowerBound + indexBlocksLowerBound + extraDuplicateBlocks).getCeiling();
            ...
            seeks = CostScalar(beginBlocksLowerBound + indexBlocksLowerBound).getCeiling();
            computeSeekForDp2ReadAheadAndProbeOrder(seekComputedWithDp2ReadAhead, finalRows, uniqueProbes,
                                                    beginBlocksLowerBound, totalBlocksLowerBound, innerBlocksInSequence,
                                                    cacheForAllVolumes, inOrderProbes);
      ...
      } else  // if orders don't match
      {
        ...
        computeIOForRandomCase(seeks, seq_kb_read, blksPerSuccProbe, beginBlocksLowerBound, totalBlocksLowerBound,
                               successfulProbes, failedProbes, indexBlocksLowerBound);

        computeSeekForDp2ReadAheadAndProbeOrder(seekComputedWithDp2ReadAhead, finalRows, uniqueProbes,
                                                beginBlocksLowerBound, totalBlocksLowerBound, innerBlocksInSequence,
                                                cacheForAllVolumes, inOrderProbes);

        seeks = indexBlocksLowerBound + seeks = indexBlocksLowerBound + seekComputedWithDp2ReadAhead.getCeiling();
      }
    ...
    *dataRows = innerHistograms.getRowCount();
    ...
    if (dataRows->isGreaterThanZero()) {
      ...
      CostScalar indexBlocks = getIndexDesc()->getIndexLevels() - 1;
      ...
      CostScalar blocksRead = (*dataRows / estimatedRecordsPerBlock).getCeiling() + indexBlocks;
      ...
      NADefaults &defs = ActiveSchemaDB()->getDefaults();
      const CostScalar maxDp2ReadInBlocks = CostScalar(defs.getAsULong(DP2_MAX_READ_PER_ACCESS_IN_KB)) / blockSizeInKb;
  ...
  computeCostVectors(firstRow  // out
                     ...
                     ,
                     *dataRows, probes, successfulProbes, seeks, seq_kb_read, keyPredicates, exePreds, probes);
#ifndef NDEBUG
...
#ifndef NDEBUG
void FileScanOptimizer::runMdamTests(const MdamKey *mdamKeyPtr, const Cost *costBoundPtr, NABoolean mdamForced,
                                     ValueIdSet exePreds, NABoolean checkExePreds, NABoolean mdamTypeIsCommon) {
  ...
  MdamKey *mdamKeyPtrOld =
      new HEAP MdamKey(mdamKeyPtr->getKeyColumns(), mdamKeyPtr->getOperatorInputs(), mdamKeyPtr->getDisjuncts(),
                       mdamKeyPtr->getNonKeyColumnSet(), mdamKeyPtr->getIndexDesc());

  SET_MDAM_TRACE_HEADER("[Old Mdam Costing] ");
  DECLARE_MDAM_MONITOR(oldMdamMon);
  ENTER_MDAM_MONITOR(oldMdamMon);
  Cost *costOld = oldComputeCostForMultipleSubset(mdamKeyPtrOld, costBoundPtr, mdamForced, numKBytesOld, checkExePreds,
                                                  mdamTypeIsCommon, sharedMdamKeyPtrOld);
  ...
  Cost *costNew = newComputeCostForMultipleSubset(mdamKeyPtrNew, costBoundPtr, mdamForced, numKBytesNew, exePreds,
                                                  checkExePreds, mdamTypeIsCommon, sharedMdamKeyPtrNew);
  ...
  if (costOld && costNew) {
    ...
    if (result != SAME) {
      ...
      MDAM_DEBUGX(MTL1, MdamTrace::printCostObject(this, costOld, "Old/New Cost"));
    }
...
NABoolean FileScanOptimizer::isMDAMFeasibleForHBase(const IndexDesc *idesc, ValueIdSet &preds) {
  ...
  for (CollIndex k = 0; k < idesc->getIndexKey().entries(); k++) {
    ColStatsSharedPtr colStat = csdl.getColStatsPtrForColumn((idesc->getIndexKey()[k]));
...
Cost *FileScanOptimizer::computeCostForMultipleSubset(MdamKey *mdamKeyPtr, const Cost *costBoundPtr,
                                                      NABoolean mdamForced, CostScalar &numKBytes,
                                                      NABoolean checkExePreds, NABoolean mdamTypeIsCommon,
  ...
  */
  ...
  if (CmpCommon::getDefault(RANGESPEC_TRANSFORMATION) == DF_ON) {
    for (CollIndex i = 0; i < mdamKeyPtr->getDisjuncts().entries(); i++) {
      ...
      ValueIdSet temp;
      for (ValueId predId = vdset.init(); vdset.next(predId); vdset.advance(predId)) {
  ...
  if (CmpCommon::getDefault(SIMPLE_COST_MODEL) == DF_ON) {
    if (CmpCommon::getDefault(MDAM_COSTING_REWRITE) == DF_ON)
      return scmRewrittenComputeCostForMultipleSubset(mdamKeyPtr, costBoundPtr, mdamForced, numKBytes, exePreds,
                                                      checkExePreds, sharedMdamKeyPtr);
    else
      return scmComputeCostForMultipleSubset(mdamKeyPtr, costBoundPtr, mdamForced, numKBytes, exePreds, checkExePreds,
                                             mdamTypeIsCommon, sharedMdamKeyPtr);
  }

#ifndef NDEBUG
  if (getenv("MDAM_TEST")) {
    runMdamTests(mdamKeyPtr, costBoundPtr, mdamForced, exePreds, checkExePreds, mdamTypeIsCommon);
  }
#endif

  if (CURRSTMT_OPTDEFAULTS->useNewMdam()) {
    return newComputeCostForMultipleSubset(mdamKeyPtr, costBoundPtr, mdamForced, numKBytes, exePreds, checkExePreds,
                                           mdamTypeIsCommon, sharedMdamKeyPtr);
  } else {
    return oldComputeCostForMultipleSubset(mdamKeyPtr, costBoundPtr, mdamForced, numKBytes, checkExePreds,
                                           mdamTypeIsCommon, sharedMdamKeyPtr);
  }
}
...
Cost *FileScanOptimizer::oldComputeCostForMultipleSubset(MdamKey *mdamKeyPtr, const Cost *costBoundPtr,
                                                         NABoolean mdamForced, CostScalar &numKBytes,
                                                         NABoolean checkExePreds, NABoolean mdamTypeIsCommon,
                                                         MdamKey *&sharedMdamKeyPtr) {
  ...
  if (sharedCostFound AND sharedMdamKeyPtr AND disjunctsFRPtr->getCPUTime() > csZero AND disjunctsLRPtr->getCPUTime() >
      csZero) {
    if (CURRSTMT_OPTDEFAULTS->reuseBasicCost()) {
      MDAM_DEBUG0(MTL2, "Use cached MDAM cost");
      Cost *costPtr = computeCostObject(disjunctsFR, disjunctsLR);
      if (costBoundPtr != NULL) {
        COMPARE_RESULT result = costPtr->compareCosts(*costBoundPtr, getContext().getReqdPhysicalProperty());
      ...
      MDAM_DEBUGX(MTL2, MdamTrace::printCostObject(this, costPtr, "Returning cached MDAM Cost"));
      MDAM_DEBUG0(MTL2, "END   MDAM Costing --------\n");
      return costPtr;
    } else {
      disjunctsFR.reset();
  ...
  IndexDescHistograms firstColumnHistogram(*getIndexDesc(), CollIndex(1));
  ...
  CostScalar
      ...
      ,
      totalProbesForSubsetBoundaries, totalSeeks  // total number of disk arm movements
      ,
  ...
  // table, thus get the total probes by multiplying by the active
  // partitions
  CostScalar incomingProbes = repeatCount * getNumActivePartitions();
  ...
  const NABoolean isMultipleProbes =
      ((repeatCount.isGreaterThanOne())OR(getContext().getInputLogProp()->getColStats().entries() > 0));

  // -------------------------------------------------------------
  // Compute upper bounds:
  // -------------------------------------------------------------

  CostScalar innerRowsUpperBound = getRawInnerHistograms().getRowCount().getCeiling();

  const CostScalar estimatedRecordsPerBlock = getIndexDesc()->getEstimatedRecordsPerBlock();

  CostScalar innerBlocksUpperBound;
  computeBlocksUpperBound(innerBlocksUpperBound /* out*/
                          ,
                          innerRowsUpperBound, estimatedRecordsPerBlock);
  ...
  if (propertyPtr && propertyPtr->getMustMatch() &&
      (propertyPtr->getMustMatch()->getOperatorType() == REL_FORCE_ANY_SCAN)) {
    scanForcePtr = (ScanForceWildCard *)propertyPtr->getMustMatch();
  }
  ...
  if (isMultipleProbes) {
    ...
    outerHistogramsPtr = new HEAP Histograms(getContext().getInputLogProp()->getColStats());
  ...
  // -----------------------------------------------------------------------
  for (CollIndex disjunctIndex = 0; disjunctIndex < mdamKeyPtr->getKeyDisjunctEntries(); disjunctIndex++) {
    // -------------------------------------------------------------
    ...
    CostScalar
        ...
        ,
        uniqueProbes = csOne, disjunctSeq_kb_read;
    ...
    // disjunct subsets.
    NABoolean multiColUecInfoAvail = disjunctHistograms.isMultiColUecInfoAvail();
    NABoolean temp = FALSE;
    NABoolean *allKeyPredicates = &temp;

    // get the key preds for this disjunct:
    ValueIdSet disjunctKeyPreds;
    mdamKeyPtr->getKeyPredicates(disjunctKeyPreds, allKeyPredicates, disjunctIndex);
    // keeps a eye on the possibility of having no executorpredicates
    if (NOT(*allKeyPredicates)) noExePreds = FALSE;
    ...
    if (disjunctKeyPreds.isEmpty() AND costBoundPtr != NULL) {
      MDAM_DEBUG1(MTL2,
                  "disjunct[%d] without any key predicate,"
    ...
    if (isMultipleProbes) {
      ...
      CostScalar disSuccProbes, disUniSucPrbs /* dummy */
          ,
          disDupSucPrbs /* dummy */
          ,
          disFldPrbs /* dummy */
      ...
      categorizeProbes(disSuccProbes /* out */
                       ...
                       ,
                       disUniFailedProbes, incomingProbes, disjunctKeyPreds, *outerHistogramsPtr, TRUE  // this is MDAM!
                       ,
                       dataRows);
    ...
    CostScalar minimumRows = csZero, minimumRqsts, minimumFailedProbes, minimumProbesForSubsetBoundaries,
               minimumSeeks = csZero, minimumSeq_kb_read = csZero;
    ...
    for (; lastColumnPosition >= 0; lastColumnPosition--) {
      ...
      if (lastColumnPosition > CollIndex(0)) {
        ...
        for (CollIndex otherColumns = lastColumnPosition - CollIndex(1); (otherColumns + 1) >= 1; otherColumns--) {
        ...
        for (ValueId predId = predsPtr->init(); predsPtr->next(predId); predsPtr->advance(predId)) {
    ...
    for (CollIndex colNum = 0; colNum < keyColumns.entries(); colNum++) {
      ...
      primaryTableCSDL.getColStatDescIndexForColumn(indexInList, keyColumns[colNum]);
      sumOfUecs = sumOfUecs + primaryTableCSDL[indexInList]->getColStats()->getTotalUec().getCeiling();
    }
    ...
    for (CollIndex prefixColumnPosition = 0; prefixColumnPosition < lastColumnPosition; prefixColumnPosition++) {
      ...
      disjunctHistograms.appendHistogramForColumnPosition(prefixColumnPosition + 1);
      ...
      if (prefixColumnPosition == 0) {
        ...
        firstColRowCntBefore =
            firstColumnHistogram.getColStatsForColumn(getIndexDesc()->getIndexKey()[0]).getRowcount().getCeiling();
        predsPtr = keyPredsByCol[0];
        const SelectivityHint *selHint = getIndexDesc()->getPrimaryTableDesc()->getSelectivityHint();
        const CardinalityHint *cardHint = getIndexDesc()->getPrimaryTableDesc()->getCardinalityHint();

        if (predsPtr AND NOT predsPtr->isEmpty()) {
          firstColumnHistogram.applyPredicates(*predsPtr, getRelExpr(), selHint, cardHint, REL_SCAN);

          ValueId predId;
          BiRelat *predIEptr;
          for (predId = predsPtr->init(); predsPtr->next(predId); predsPtr->advance(predId)) {
            if (predId.getItemExpr()->getArity() == 2) {
              predIEptr = (BiRelat *)predId.getItemExpr();
              if (predIEptr->isaPartKeyPred()) {
                leadingPartPreds += 1;
        ...
        // statistics:
        if (itIsSingleSubset) {
          for (CollIndex singleSubsetColPosition = 0; singleSubsetColPosition <= singleSubsetPrefixColumn;
          ...
          disjunctSubsets = MINOF(disjunctSubsets.getValue(), innerRowsUpperBound.getValue());
      ...
      else {  // The prefix column position is in a column after
        ...
        uecForPreviousCol =
            disjunctHistograms.getColStatsForColumn(getIndexDesc()->getIndexKey()[prefixColumnPosition - 1])
                .getTotalUec()
                .getCeiling();

        CostScalar estimatedUec = csOne;
        if (multiColUecInfoAvail AND uecForPreviousCol.isGreaterThanOne() AND prefixColumnPosition >
            1 AND disjunctHistograms.estimateUecUsingMultiColUec(keyPredsByCol,            /*in*/
                                                                 prefixColumnPosition - 1, /*in*/
                                                                 estimatedUec /*out*/)) {
          uecForPreviousCol = (uecForPreviousCol / uecForPreviousColBeforeAppPreds) * estimatedUec;
        ...
        CostScalar uecPerBlock = uecForPreviousColBeforeAppPreds / blocksToReadPerUec;
        ...
        if (uecForPrevColForSeeks.isGreaterThanOne() AND NOT uecPerBlock.isLessThanOne()) {
          uecForPrevColForSeeks = MIN_ONE(uecForPrevColForSeeks / uecPerBlock);
          uecForPrevColForSeeks =
              MIN_ONE(uecForPrevColForSeeks * MAXOF((sumOfUecs - sumOfUecsSoFar) / sumOfUecs, 1 / 16));
      ...
      uecForPreviousColBeforeAppPreds =
          disjunctHistograms.getColStatsForColumn(getIndexDesc()->getIndexKey()[prefixColumnPosition])
              .getTotalUec()
              .getCeiling();
      ...
      if (predsPtr AND(NOT predsPtr->isEmpty())) {
        ...
        if (NOT crossProductApplied AND isMultipleProbes) {
          ...
          if (prefixColumnPosition == lastColumnPosition - 1) {
            ...
            MDAM_DEBUG1(MTL2, "orderRowCount comes from outerHist: %f", orderRowCount.value());

          } else {
            const SelectivityHint *selHint = getIndexDesc()->getPrimaryTableDesc()->getSelectivityHint();
            const CardinalityHint *cardHint = getIndexDesc()->getPrimaryTableDesc()->getCardinalityHint();

            disjunctHistograms.applyPredicatesWhenMultipleProbes(*predsPtr, *(getContext().getInputLogProp()),
                                                                 getRelExpr().getGroupAttr()->getCharacteristicInputs(),
                                                                 TRUE  // doing MDAM!
                                                                 ,
                                                                 selHint, cardHint, NULL, REL_SCAN);
          }
        } else {
          const SelectivityHint *selHint = getIndexDesc()->getPrimaryTableDesc()->getSelectivityHint();
          const CardinalityHint *cardHint = getIndexDesc()->getPrimaryTableDesc()->getCardinalityHint();

          disjunctHistograms.applyPredicates(*predsPtr, getRelExpr(), selHint, cardHint, REL_SCAN);
        }
      }
      prevPredIsEqual = curPredIsEqual;
      curPredIsEqual = FALSE;
      if (keyPredsByCol.getPredicateExpressionPtr(prefixColumnPosition) AND(
              keyPredsByCol.getPredicateExpressionPtr(prefixColumnPosition)->getType() ==
              KeyColumns::KeyColumn::CONFLICT_EQUALS OR keyPredsByCol.getPredicateExpressionPtr(prefixColumnPosition)
      ...
      if (getRowCount) {
        ...
        MDAM_DEBUG1(MTL2, "orderRowCount comes from disjunctHist: %f", orderRowCount.value());
      }
      if (prefixColumnPosition == 0) {
        ...
        firstColOverlaps = ((firstColRowCntBefore == firstColRowCntAfter) AND(disjunctIndex > 0)
                                AND(firstColRowCntAfter == orderRowCount));
      ...
      if (scanForcePtr && mdamForced) {
        sparseForced =
            ((scanForcePtr->getEnumAlgorithmForColumn(prefixColumnPosition) == ScanForceWildCard::COLUMN_SPARSE)
                 ? TRUE
                 : FALSE);
        denseForced =
            ((scanForcePtr->getEnumAlgorithmForColumn(prefixColumnPosition) == ScanForceWildCard::COLUMN_DENSE)
                 ? TRUE
                 : FALSE);
      }
      ...
      if (sparseForced OR denseForced) {
        ...
        if (prefixColumnPosition == 0) {
          const ColStats &firstColumnColStats =
              disjunctHistograms.getColStatsForColumn(getIndexDesc()->getIndexKey()[0]);
          ...
          const CostScalar density =
              ...
              (firstColumnColStats.getMaxValue().getDblValue() - firstColumnColStats.getMinValue().getDblValue() + 1.0);
      ...
      if (prefixColumnPosition == 0) {
        ...
        disjunctSubsets = MINOF(innerRowsUpperBound.getValue(), disjunctSubsets.getValue());
        disjunctSubsetsAsSeeks = disjunctSubsets;
      ...
      } else  // prefixColumnPosition > 0
      {
        ...
        if ((prefixColumnPosition == 1 AND NOT(firstColOverlaps)) OR prefixColumnPosition > 1) {
          ...
          if (uecForPrevColForSeeks.isGreaterThanZero()) {
            ...
            disjunctSubsetsAsSeeks = MINOF(innerRowsUpperBound.getValue(), disjunctSubsetsAsSeeks.getValue());
          }
          ...
          if (mdamKeyPtr->isColumnSparse(prefixColumnPosition - 1)) {
            ...
            disjunctProbesForSubsetBoundaries =
                MINOF(innerRowsUpperBound.getValue(), disjunctProbesForSubsetBoundaries.getValue());
          }
      ...
      if (NOT disjunctRqsts.isLessThanOne() AND disjunctRows.isGreaterThanZero()) {
      ...
      computeTotalBlocksLowerBound(disjunctBlocksToRead, disjunctSubsetsAsSeeks, rowsPerSubset,
                                   estimatedRecordsPerBlock, innerBlocksUpperBound);
      ...
      computeBeginBlocksLowerBound(
          ...
          MINOF((disjunctSubsetsAsSeeks / subsetsPerBlock).getCeiling(), disjunctSubsetsAsSeeks.getValue()),
          innerBlocksUpperBound);
      ...
      CostScalar indexBlocksLowerBound = getIndexDesc()->getEstimatedIndexBlocksLowerBound(
          MINOF(((disjunctSubsets + disjunctProbesForSubsetBoundaries) / subsetsPerBlock).getCeiling(),
      ...
      computeIOForFullCacheBenefit(disjunctSeeks /* out */
                                   ...
                                   ,
                                   beginBlocksLowerBound, disjunctBlocksToRead, indexBlocksLowerBound);
      ...
      if ((disjunctSeeks - beginBlocksLowerBound).getValue() >= 5) {
        ...
        disjunctSeq_kb_read = disjunctSeq_kb_read - CostScalar(5) * getIndexDesc()->getBlockSizeInKb();
      }
      ...
      MDAM_DEBUG2(MTL2, "Disjunct: %d, Prefix Column: %d", disjunctIndex, prefixColumnPosition);
      MDAM_DEBUG1(MTL2, "Incoming Probes: %f:", incomingProbes.value());
      MDAM_DEBUG1(MTL2, "Prefix Failed Probes: %f:", disjunctFailedProbes.value());
      MDAM_DEBUG1(MTL2, "Prefix Subsets: %f:", disjunctSubsets.value());
      MDAM_DEBUG1(MTL2, "Prefix Requests (probes * Subsets): %f:", disjunctRqsts.value());
      ...
      computeCostVectorsForMultipleSubset(prefixFR, prefixLR, seqKBytesPerScan, disjunctRows,
                                          disjunctRqsts + disjunctProbesForSubsetBoundaries, disjunctFailedProbes,
                                          disjunctSeeks, disjunctSeq_kb_read, disjunctKeyPreds, exePreds,
                                          incomingProbes, CostScalar(disjunctKeyPreds.entries()));
      ...
      // -------------------------------------------------------
      char *cstrMDAMStatus = getenv("MDAM");
      if ((cstrMDAMStatus != NULL) && (strcmp(cstrMDAMStatus, "ON") == 0)) {
        ...
        if (scanForcePtr && mdamForced) {
          proceedViaCosting = FALSE;
          if (noExePreds AND scanForcePtr->getNumMdamColumns() < lastColumnPosition) noExePreds = FALSE;
          if (prefixColumnPosition < scanForcePtr->getNumMdamColumns()) {
            ...
            if (scanForcePtr->getMdamColumnsStatus() == ScanForceWildCard::MDAM_COLUMNS_ALL) {
              // Unconditionally force all of the columns:
              minimumExceedsFrAndLrCosts = TRUE;
              // proceedViaCosting is FALSE
            } else if (scanForcePtr->getMdamColumnsStatus() == ScanForceWildCard::MDAM_COLUMNS_NO_MORE) {
              // Unconditionally reject this and later columns:
              minimumExceedsFrAndLrCosts = FALSE;
              curPredIsEqual = FALSE;
              prevPredIsEqual = FALSE;
              // proceedViaCosting is FALSE;
            } else if (scanForcePtr->getMdamColumnsStatus() == ScanForceWildCard::MDAM_COLUMNS_REST_BY_SYSTEM) {
      ...
      MDAM_DEBUGX(MTL2, MdamTrace::printBasicCost(this, prefixFR, prefixLR,
                                                  "Cost for prefix in oldComputeCostForMultipleSubset():"));
      ...
      if (proceedViaCosting) {
        ...
        minimumExceedsFrAndLrCosts = NOT exceedsBound(minimumPrefixCostPtr, prefixFR, prefixLR);
      }
      ...
      if (firstRound OR minimumExceedsFrAndLrCosts OR(
              prevColChosen AND keyPredsByCol[prefixColumnPosition] AND NOT predsPtr->isEmpty()
        ...
        minimumProbesForSubsetBoundaries = disjunctProbesForSubsetBoundaries;
        ...
        MDAM_DEBUG1(MTL2, "Minimum prefix column position: %d", prefixColumnPosition);
        MDAM_DEBUGX(MTL2, MdamTrace::printCostObject(this, minimumPrefixCostPtr, "Minimum so far"));

      }  // update minimum prefix
      else if (proceedViaCosting AND NOT minimumExceedsFrAndLrCosts) {
        if (prefixColumnPosition == lastColumnPosition - 1) {  // we have reached the lastcolumnPosition and its cost
    ...
    if (leadingPartPreds == mdamKeyPtr->getKeyDisjunctEntries()
      ...
      totalSeeks += (totalRows / estimatedRecordsPerBlock) / maxDp2ReadInBlocks / 100;
    }
    ...
    if (NOT mdamForced AND stopColumn == 0) {
      if (keyPredsByCol.getPredicateExpressionPtr(0) == NULL)
        ...
        const ColumnOrderList::KeyColumn *currKeyColumn = keyPredsByCol.getPredicateExpressionPtr(0);

        NABoolean conflict = ((currKeyColumn->getType() == KeyColumns::KeyColumn::CONFLICT)
                                  OR(currKeyColumn->getType() == KeyColumns::KeyColumn::CONFLICT_EQUALS));
    ...
    if (mdamMakeSense) {
      ...
      computeCostVectorsForMultipleSubset(disjunctsFR /* out */
                                          ...
                                          seqKBytesPerScan /* out */
                                          ,
                                          totalRows, totalRqsts, totalFailedProbes, totalSeeks, totalSeq_kb_read,
                                          totalKeyPreds, exePreds, incomingProbes, totalDisjunctPreds);
#ifndef NDEBUG
      (*CURRSTMT_OPTGLOBALS->multVectorCostMonitor).exit();
#endif  // NDEBUG
      if (exceedsBound(costBoundPtr, disjunctsFR, disjunctsLR)) {
        mdamMakeSense = FALSE;
        if (disjunctIndex + 1 < mdamKeyPtr->getKeyDisjunctEntries()) {
  ...
  MDAM_DEBUGX(MTL2, MdamTrace::printCostObject(this, costPtr, "Returning MDAM Cost"));
  MDAM_DEBUG0(MTL2, "END   MDAM Costing --------\n");
  return costPtr;
...
// -----------------------------------------------------------------------
NABoolean FileScanBasicCost::hasSameBasicProperties(const Context &currentContext) const {
  if (basicCostContext_->getGroupId() != currentContext.getGroupId()) return FALSE;
  ...
  const InputPhysicalProperty *currentIPP = currentContext.getInputPhysicalProperty();
  const InputPhysicalProperty *existingIPP = basicCostContext_->getInputPhysicalProperty();
  ...
  // currentIPP != NULL
  if (existingIPP == NULL OR existingIPP->getAssumeSortedForCosting()) return FALSE;

  // currentIPP != NULL AND existingIPP != NULL
  return (currentIPP == existingIPP OR * (currentIPP->getNjOuterOrder()) == *(existingIPP->getNjOuterOrder()));
}
...
NABoolean FileScanOptimizer::exceedsBound(const Cost *costBoundPtr, const SimpleCostVector &firstRow,
                                          const SimpleCostVector &lastRow) const {
  if (costBoundPtr == NULL) {
    return FALSE;
  } else {
    const Cost *costSoFarPtr = computeCostObject(firstRow, lastRow);

    COMPARE_RESULT result = costSoFarPtr->compareCosts(*costBoundPtr, getContext().getReqdPhysicalProperty());
...
void FileScanOptimizer::computeSeekForDp2ReadAheadAndProbeOrder(
    CostScalar &seekComputedWithDp2ReadAhead, const CostScalar &finalRows, const CostScalar &uniqueProbes,
    const CostScalar &beginBlocksLowerBound, const CostScalar &totalBlocksLowerBound,
    const CostScalar &innerBlocksUpperBound, const CostScalar &dp2CacheSize, const NABoolean inOrderProbes) const {
  NADefaults &defs = ActiveSchemaDB()->getDefaults();
  const CostScalar blockSizeInKb = getIndexDesc()->getBlockSizeInKb();
  const CostScalar maxDp2ReadInBlocks = CostScalar(defs.getAsULong(DP2_MAX_READ_PER_ACCESS_IN_KB)) / blockSizeInKb;
  const CostScalar blocksForMoreThanOneSeek = defs.getAsLong(DP2_MINIMUM_FILE_SIZE_FOR_SEEK_IN_BLOCKS);
  ...
  if (inOrderProbes) {
    ...
    seekComputedWithDp2ReadAhead = csOne + fixedSeeks + (variableSeeks) * (DBB / MSD) + Mrows * finalRows;

  } else {
    if (innerBlocksUpperBound <= dp2CacheSize) {
      seekComputedWithDp2ReadAhead = MINOF(beginBlocksLowerBound, innerBlocksUpperBound / maxDp2ReadInBlocks);
    } else {
...
void FileScanOptimizer::computeIOForFullCacheBenefit(CostScalar &seeks /* out */
                                                     ,
  ...
  seq_kb_read = (totalBlocksLowerBound + indexBlocks) * getIndexDesc()->getBlockSizeInKb();

  // There cannot be more index blocks than the log of the total data blocks:

  seeks = beginBlocksLowerBound + indexBlocks;
}

void FileScanOptimizer::computeIOForRandomCase(CostScalar &seeks /* out */
                                               ,
                                               CostScalar &seq_kb_read /* out */
                                               ,
                                               ...
                                               const CostScalar &beginBlocksLowerBound,
                                               const CostScalar &totalBlocksLowerBound,
                                               const CostScalar &successfulProbes, const CostScalar &failedProbes,
  ...
  if (availableCacheInBlocks < totalBlocksLowerBound) {
    cacheHitProbability = availableCacheInBlocks / totalBlocksLowerBound;
  }

  // sanity check:
  DCMPASSERT(cacheHitProbability >= 0 AND cacheHitProbability <= 1.0);

  const CostScalar extraBeginBlocks = successfulProbes - beginBlocksLowerBound;

  const CostScalar extraDataBlocks = successfulProbes * blksPerSuccProbe - totalBlocksLowerBound;
  // -----------------------------------------------------------------------
  ...
  // -----------------------------------------------------------------------
  seq_kb_read =
      ((totalBlocksLowerBound + indexBlocksLowerBound + failedProbes) + (csOne - cacheHitProbability) * extraDataBlocks
  ...
  // in cache:
  seeks = beginBlocksLowerBound    // data seeks for all probes
          + indexBlocksLowerBound  // index seeks
          + failedProbes + (csOne - cacheHitProbability) * extraBeginBlocks;
}

void FileScanOptimizer::computeIOForFullTableScan(CostScalar &dataRows /* out */
                                                  ,
  ...
  const CostScalar indexBlocksLowerBound = getIndexDesc()->getEstimatedIndexBlocksLowerBound(probes);
  // Assume that every record in the table will be read:

  dataRows = getRawInnerHistograms().getRowCount();

  const CostScalar blksInTable = CostScalar(dataRows / getIndexDesc()->getEstimatedRecordsPerBlock());

  seq_kb_read = (probes * blksInTable) * getIndexDesc()->getBlockSizeInKb();

  seeks = probes                    // data seeks
          + indexBlocksLowerBound;  // index seeks

}  // FileScanOptimizer::computeIOForFullTableScan(...)

void FileScanOptimizer::computeCostVectorsForMultipleSubset(
    ...
    ,
    const CostScalar &totalRows, const CostScalar &subsetRequests  // a request for a subset of data
    ,
    const CostScalar &failedProbes  // produce and EMPTY mdam tree
    ,
    const CostScalar &seeks, const CostScalar &seq_kb_read, const ValueIdSet &keyPreds, const ValueIdSet &exePreds,
  ...
  // for one probe:
  CostScalar mdamNetBuildCost = mdamNetCostOvh + mdamNetCostPerPred * mdamNetPredCnt;
  ...
  CostScalar mdamNetOverheadTime = mdamNetBuildCost * CostPrimitives::getBasicCostFactor(MSCF_ET_CPU);
  ...
  mdamNetOverheadTime = mdamNetBuildCost * CostPrimitives::getBasicCostFactor(MSCF_ET_CPU);
  ...
  computeCostVectors(firstRow  // out
                     ...
                     lastRow  // out
                     ,
                     seqKBytesPerScan, totalRows, subsetRequests, subsetRequests  // all are successful
                     ,
                     seeks, seq_kb_read, keyPreds, exePreds, incomingProbes);

}  // FileScanOptimizer::computeCostVectorsForMultipleSubset(...)

void FileScanOptimizer::computeCostVectors(SimpleCostVector &firstRow /* out */
                                           ,
                                           SimpleCostVector &lastRow /* out */
                                           ,
                                           CostScalar &seqKBytesPerScan /* out */
                                           ...
                                           ,
                                           const CostScalar &seeks, const CostScalar &seq_kb_read,
                                           const ValueIdSet &keyPreds, const ValueIdSet &exePreds,
                                           const CostScalar &incomingProbes  // probes incoming to the operator
) const {
  ...
  if (CmpCommon::getDefault(COMP_BOOL_35) == DF_OFF) totalRowsInResultTable = MINOF(totalRowsInResultTable, totalRows);
  ...
  // -----------------------------------------------------------------------
  const CostScalar rowsPerScan = CostScalar(totalRows / activePartitions).getCeiling(),
                   selectedRowsPerScan = CostScalar(totalRowsInResultTable / activePartitions).getCeiling(),
                   requestsPerScan = CostScalar(subsetRequests / activePartitions).getCeiling(),
                   successfulRequestsPerScan = CostScalar(successfulSubsetRequests / activePartitions).getCeiling(),
  ...
  CostScalar seq_kb_readPerScan = CostScalar(MAXOF(seq_kb_read / activePartitions, 4.0));

  // -----------------------------------------------------------------------
  // Set up some parameters in preparation for costing:
  // -----------------------------------------------------------------------

  const CostScalar recordsPerBlock = getIndexDesc()->getEstimatedRecordsPerBlock().getCeiling();

  const CostScalar kbPerBlock = getIndexDesc()->getBlockSizeInKb();

  const CostScalar dp2MessageBufferSizeInKb = CostPrimitives::getBasicCostFactor(DP2_MESSAGE_BUFFER_SIZE),
                   recordSizeInKb = getIndexDesc()->getRecordSizeInKb();

  // rows that fit in dp2 buffer (rfdp2):
  const double rfdp2 = (dp2MessageBufferSizeInKb / recordSizeInKb).getFloor().getValue();

  // Assume that the first row is obtained in the first
  // successful request:
  const CostScalar rowsPerScanForFirstRow = (successfulRequestsPerScan.isGreaterThanZero()
                                                 ? MINOF((rowsPerScan / successfulRequestsPerScan).getCeiling(), rfdp2)
                                                 : csZero);

  const CostScalar selectedRowsPerScanForFirstRow =
      (successfulRequestsPerScan.isGreaterThanZero()
           ? MINOF((selectedRowsPerScan / successfulRequestsPerScan).getCeiling(), rfdp2)
           : csZero);
  ...
  CostScalar cpuInstructionsFR = csZero, cpuInstructionsLR = csZero;
  ...
  if (requestsPerScan.isGreaterThanZero()) {
    ...
    const CostScalar cpuForBTreeComparisons =
        (1.44 * log((getIndexDesc()->getEstimatedRecordsPerBlock()).getValue()) *
         getIndexDesc()->getIndexKey().entries() * CostPrimitives::getBasicCostFactor(CPUCOST_PREDICATE_COMPARISON) *
    ...
    if (successfulRequestsPerScan.isGreaterThanZero()) {
      ...
      cpuInstructionsLR +=  // LR CPU INSTRUCTION CALC:
          ...
          (cpuForBTreeComparisons + CostPrimitives::getBasicCostFactor(CPUCOST_DATARQST_OVHD));
    }

    // unsuccessful requests:
    const CostScalar failedRequestsPerScan = requestsPerScan - successfulRequestsPerScan;
    if (failedRequestsPerScan.isGreaterThanZero()) {
      ...
      cpuInstructionsLR +=  // LR CPU INSTRUCTION CALC:
          ...
          ((cpuForBTreeComparisons / 2).getCeiling() + CostPrimitives::getBasicCostFactor(CPUCOST_DATARQST_OVHD));
    }
    ...
    CostScalar instrPerRowMovedOutOfTheCache =
        ...
        numberOfCopiesOfRows * recordSizeInKb * CostScalar(1024.)  // convert kb to bytes
            * CostScalar(CostPrimitives ::getBasicCostFactor(CPUCOST_COPY_ROW_PER_BYTE));
    ...
    instrPerRowMovedOutOfTheCache += locking * CostScalar(CostPrimitives ::getBasicCostFactor(CPUCOST_LOCK_ROW));
    ...
    instrPerRowMovedOutOfTheCache +=
        keyLength * CostScalar(CostPrimitives ::getBasicCostFactor(CPUCOST_SCAN_KEY_LENGTH));

    // Add a per kb cost for the cost of moving
    // every block from disk to DP2:
    CostScalar instrToCopyDataFromDiskToDP2Cache =
        seq_kb_readPerScan * CostScalar(CostPrimitives::getBasicCostFactor(CPUCOST_SCAN_DSK_TO_DP2_PER_KB));
    // Add a per seek cost for the cost of moving
    // data from disk to DP2:
    instrToCopyDataFromDiskToDP2Cache +=
        seeksPerScan * CostScalar(CostPrimitives::getBasicCostFactor(CPUCOST_SCAN_DSK_TO_DP2_PER_SEEK));
    // $$$ First row
    // Add this later
    ...
    instrPerRowMovedOutOfTheCache += recordSizeInKb *
                                     // Catch all overhead per KB:
                                     // this is a per kb read overhead:
                                     CostScalar(CostPrimitives::getBasicCostFactor(CPUCOST_SCAN_OVH_PER_KB));

    // Add a a per-row overhead:
    instrPerRowMovedOutOfTheCache += CostScalar(CostPrimitives::getBasicCostFactor(CPUCOST_SCAN_OVH_PER_ROW));

    // Only rows that are selected are moved out of the cache:
    // First row
    cpuInstructionsFR +=  // FR CPU INSTRUCTION CALC:
        instrPerRowMovedOutOfTheCache * selectedRowsPerScanForFirstRow;

    // LR CPU INSTRUCTION CALC:
    cpuInstructionsLR +=  // LR CPU INSTRUCTION CALC:
        instrPerRowMovedOutOfTheCache * selectedRowsPerScan;

    // Compute cost for executor predicates
    const CostScalar instrPerExePredEvaluationPerRow =
        CostPrimitives::getBasicCostFactor(CPUCOST_PREDICATE_COMPARISON) * exePreds.entries();

    // First row
    cpuInstructionsFR +=  // FR CPU INSTRUCTION CALC:
        instrPerExePredEvaluationPerRow * rowsPerScanForFirstRow;

    // LR CPU INSTRUCTION CALC:
    cpuInstructionsLR +=  // LR CPU INSTRUCTION CALC:
        instrPerExePredEvaluationPerRow * rowsPerScan;
  ...
  // SEEKS:
  // -----------------------------------------------------------------------
  CostScalar seeksFR = csZero, seeksLR = csZero;
  ...
  // KB TRANSFERED
  // -----------------------------------------------------------------------
  CostScalar seqKBFR = csZero, seqKBLR = csZero;
  ...
  if (successfulRequestsPerScan.isGreaterThanZero()) {
    ...
    const CostScalar maxBlksPerScanForFirstRow = CostScalar(rowsPerScanForFirstRow / recordsPerBlock).getCeiling();

    CostScalar maxKbReadPerScanForFirstRow = maxBlksPerScanForFirstRow * kbPerBlock;

    // Add bytes due to the effect of selectivity on read-ahead:
    const CostScalar totalRowsInRawTable = getRawInnerHistograms().getRowCount().getCeiling();
    ...
    if (ioTransferCostPrefetchMissesFraction.isGreaterThanZero() AND totalRowsInRawTable.isGreaterThanZero()) {
      ...
      CostScalar seq_kb_readPerScanPerRqst =
          (seq_kb_readPerScan / activePartitions).getCeiling() / successfulRequestsPerScan;
      if (seq_kb_readPerScanPerRqst > CostPrimitives::getBasicCostFactor(DP2_MESSAGE_BUFFER_SIZE)) {
        // Read ahead reduction, figure out reduction factor:
        CostScalar rowsInResultTablePerScan = (totalRowsInResultTable / activePartitions).getCeiling();

        CostScalar blocksInResultTablePerScan =
            (rowsInResultTablePerScan / getIndexDesc()->getEstimatedRecordsPerBlock()).getCeiling();

        CostScalar kbInResultTablePerScan = blocksInResultTablePerScan * getIndexDesc()->getBlockSizeInKb();

        kbReadAgainstKbSelectedRatio = kbInResultTablePerScan / seq_kb_readPerScan;
      ...
      seq_kb_readPerScan += seq_kb_readPerScan * ioTransferCostPrefetchMissesFraction * kbReadAgainstKbSelectedRatio;

      maxKbReadPerScanForFirstRow +=
          maxKbReadPerScanForFirstRow * ioTransferCostPrefetchMissesFraction * kbReadAgainstKbSelectedRatio;
    }

    // we can't read more blocks for the
    // first row than for the entire scan:
    seqKBFR = MINOF(maxKbReadPerScanForFirstRow, seq_kb_readPerScan);
    seqKBLR = seq_kb_readPerScan;
  ...
  const CostScalar cacheSizeInBlocks = getDP2CacheSizeInBlocks(getIndexDesc()->getBlockSizeInKb());
  const CostScalar dp2CacheInKb = cacheSizeInBlocks * getIndexDesc()->getBlockSizeInKb();
  ...
  // No idle time
  CostScalar idleTimeFR = csZero, idleTimeLR = csZero;
  ...
  CostScalar openFileCPU =
      CostPrimitives::getBasicCostFactor(CPUCOST_SUBSET_OPEN_AFTER_FIRST) * MAXOF(0, (activePartitions.getValue() - 1));

  idleTimeFR = idleTimeLR = openFileCPU * CostPrimitives::getBasicCostFactor(MSCF_ET_CPU);
...
// -----------------------------------------------------------------------
void ScanOptimizer::categorizeProbes(CostScalar &successfulProbes /* out */
                                     ,
                                     ...
                                     const Histograms &outerHistograms, const NABoolean isMDAM,
                                     CostScalar *dataRows) const {
  if (outerHistograms.isEmpty()) {
    ...
    if (CmpCommon::getDefault(COMP_BOOL_35) == DF_OFF) *dataRows *= probes;

  } else {
    // If there are input values we are in a nested join case,
    // else it must be a cartesian product:
    const ValueIdSet &inputValues = getRelExpr().getGroupAttr()->getCharacteristicInputs();

    // get this filescan statistics:
    IndexDescHistograms joinedHistograms(*getIndexDesc(), getIndexDesc()->getIndexKey().entries());
    ...
    joinedHistograms.applyPredicatesWhenMultipleProbes(preds, *(getContext().getInputLogProp()), inputValues, isMDAM,
                                                       selHint, cardHint, NULL, REL_SCAN);
    ...
    if (NOT inputValues.isEmpty()) {
      if (NOT preds.isEmpty()) {
        ...
        const ValueIdSet &operatorValues = getIndexDesc()->getIndexKey();
        failedProbes = joinedHistograms.computeFailedProbes(outerHistograms, preds, inputValues, operatorValues);
    ...
    for (CollIndex i = 0; i < outerHistograms.entries(); i++) {
      histUEC = outerHistograms[i].getColStats()->getTotalUec().getCeiling();
    ...
    if (uniqueEntryCountOfProbes > probes) uniqueEntryCountOfProbes = probes;
    ...
    uniqueSuccProbes =
        MINOF(((successfulProbes / probes) * uniqueEntryCountOfProbes).getValue(), successfulProbes.getValue());

    uniqueFailedProbes =
        MINOF(((failedProbes / probes) * uniqueEntryCountOfProbes).getValue(), successfulProbes.getValue());
...
MDAMCostWA::MDAMCostWA(FileScanOptimizer &optimizer, NABoolean mdamForced, MdamKey *mdamKeyPtr,
                       const Cost *costBoundPtr, const ValueIdSet &exePreds, SimpleCostVector &disjunctsFR,
    ...
    : mdamWon_(FALSE),
      ...
      ,
      innerRowsUpperBound_(optimizer.getRawInnerHistograms().getRowCount().getCeiling()),
      innerBlocksUpperBound_(0),
      estimatedRecordsPerBlock_(optimizer.getIndexDesc()->getEstimatedRecordsPerBlock()),
...
void MDAMCostWA::compute() {
  ...
  const CostScalar blockSizeInKb = optimizer_.getIndexDesc()->getBlockSizeInKb();
  NADefaults &defs = ActiveSchemaDB()->getDefaults();
  const CostScalar maxDp2ReadInBlocks = CostScalar(defs.getAsULong(DP2_MAX_READ_PER_ACCESS_IN_KB)) / blockSizeInKb;
  // -----------------------------------------------------------------------
  //  Loop through every disjunct and:
  ...
  // -----------------------------------------------------------------------
  for (CollIndex disjunctIndex = 0; disjunctIndex < mdamKeyPtr_->getKeyDisjunctEntries(); disjunctIndex++) {
    ...
    if (totalNumLeadingPartPreds == mdamKeyPtr_->getKeyDisjunctEntries()
      ...
      totalSeeks += (totalRows / estimatedRecordsPerBlock_) / maxDp2ReadInBlocks / 100;
    ...
    if (CmpCommon::getDefault(SIMPLE_COST_MODEL) == DF_ON) {
      if (optimizer_.getIndexDesc()->getPrimaryTableDesc()->getNATable()->isHbaseTable()) {
        ...
        scmCost_ = optimizer_.scmComputeMDAMCostForHbase(totRowsProcessed, totSeeks, totalSeqKBRead, incomingProbes_);
        ...
        CostScalar rowsPerScan = CostScalar(totalRows / activePartitions).getCeiling(),
                   selectedRowsPerScan = CostScalar(totalRowsInResultTable / activePartitions).getCeiling(),
        ...
        CostScalar rowSizeFactorSeqIO = optimizer_.scmRowSizeFactor(rowSize, ScanOptimizer::SEQ_IO_ROWSIZE_FACTOR);
        CostScalar rowSizeFactorRandIO = optimizer_.scmRowSizeFactor(rowSize, ScanOptimizer::RAND_IO_ROWSIZE_FACTOR);
        ...
        scmCost_ = optimizer_.scmCost(rowsPerScan, selectedRowsPerScan, csZero, ioRandPerScan, ioSeqPerScan,
                                      probesPerScan, rowSize, csZero, outputRowSize, csZero);
      }
      ...
      if (costBoundPtr_ != NULL && costBoundPtr_->scmCompareCosts(*scmCost_) == LESS) {
        mdamWon_ = FALSE;
      ...
      optimizer_.computeCostVectorsForMultipleSubset(disjunctsFR_ /* out */
                                                     ...
                                                     seqKBytesPerScan /* out */
                                                     ,
                                                     totalRows, totalRqsts, totalFailedProbes, totalSeeks,
                                                     totalSeqKBRead, totalKeyPreds, exePreds_, incomingProbes_,
                                                     totalDisjunctPreds);

      // checking some adjustments,
      // Need to introduce some Tunable factor for 100
      if ((CmpCommon::getDefault(RANGESPEC_TRANSFORMATION) == DF_ON) && (totalRows < CostScalar(100))) {
...
void MDAMCostWA::computeDisjunct() {
  ...
  ValueIdSet disjunctKeyPreds;
  mdamKeyPtr_->getKeyPredicates(disjunctKeyPreds /*out*/, &allKeyPredicates /*out*/, disjunctIndex_ /*in*/);
  if (NOT(allKeyPredicates)) noExePreds_ = FALSE;
  ...
  MDAMOptimalDisjunctPrefixWA prefixWA(optimizer_, keyPredsByCol, disjunctKeyPreds, exePreds_, outerHistograms_,
                                       firstColumnHistogram_, noExePreds_, mdamKeyPtr_, scanForcePtr_, mdamForced_,
                                       isMultipleProbes_, incomingProbes_, estimatedRecordsPerBlock_,
                                       innerRowsUpperBound_, innerBlocksUpperBound_, disjunctIndex_);
  ...
  disjunctOptProbesForSubsetBoundaries_ = prefixWA.getOptProbesForSubsetBoundaries();
  ...
  if (NOT mdamForced_ AND stopColumn == 0) {
    if (keyPredsByCol.getPredicateExpressionPtr(0) == NULL)
      ...
      NABoolean conflict = ((currKeyColumn->getType() == KeyColumns::KeyColumn::CONFLICT)
                                OR(currKeyColumn->getType() == KeyColumns::KeyColumn::CONFLICT_EQUALS) OR
                            // added for patching (since it is a single disjunct now, but not a conflict)
                            // where a =10 or a=20 or a=30
                            (currKeyColumn->getType() == KeyColumns::KeyColumn::INLIST));
  ...
  if (CURRSTMT_OPTDEFAULTS->indexEliminationLevel() != OptDefaults::MINIMUM && (!mdamForced_) &&
      ...
      (!checkMDAMadditionalRestriction(keyPredsByCol, optimizer_.computeLastKeyColumnOfDisjunct(keyPredsByCol), hist,
                                       (restrictCheckStrategy)checkOption, noOfmissingKeyColumnsTot,
...
NABoolean MDAMCostWA::isMdamWon() const { return mdamWon_; }

NABoolean MDAMCostWA::hasNoExePreds() const { return noExePreds_; }

const CostScalar &MDAMCostWA::getNumKBytes() const { return numKBytes_; }

Cost *MDAMCostWA::getScmCost() { return scmCost_; }

// Implementation of MDAMOptimalDisjunctPrefixWA methods

MDAMOptimalDisjunctPrefixWA::MDAMOptimalDisjunctPrefixWA(
    FileScanOptimizer &optimizer, const ColumnOrderList &keyPredsByCol, const ValueIdSet &disjunctKeyPreds,
    const ValueIdSet &exePreds, const Histograms &outerHistograms, IndexDescHistograms &firstColumnHistogram,
    NABoolean &noExePreds, MdamKey *mdamKeyPtr, const ScanForceWildCard *scanForcePtr, NABoolean mdamForced,
    NABoolean isMultipleProbes, const CostScalar &incomingProbes, const CostScalar &estimatedRecordsPerBlock,
    const CostScalar &innerRowsUpperBound, const CostScalar &innerBlocksUpperBound, CollIndex disjunctIndex)
...
NABoolean MDAMOptimalDisjunctPrefixWA::missingKeyColumnExists() const {
  ...
  for (index = 0; index < keyPredsByCol_.entries(); index++) {
    if (keyPredsByCol_.getPredicateExpressionPtr(index) != NULL)
      ...
      typeOfRange = KeyColumns::KeyColumn::EMPTY;
    if (typeOfRange == KeyColumns::KeyColumn::EMPTY) break;
  }
...
void MDAMOptimalDisjunctPrefixWA::compute() {
  ...
  const ColStatDescList &primaryTableCSDL = optimizer_.getIndexDesc()->getPrimaryTableDesc()->getTableColStats();
  const ValueIdList &keyColumns = optimizer_.getIndexDesc()->getIndexKey();
  for (CollIndex colNum = 0; colNum < keyColumns.entries(); colNum++) {
    CollIndex indexInList;
    primaryTableCSDL.getColStatDescIndexForColumn(indexInList, keyColumns[colNum]);
    sumOfUecs_ += primaryTableCSDL[indexInList]->getColStats()->getTotalUec().getCeiling();
  }
...
void MDAMOptimalDisjunctPrefixWA::computeProbesDisjunct() {
  if (isMultipleProbes_) {
    CostScalar disSuccProbes, disUniSucPrbs /* dummy */
        ,
        disDupSucPrbs /* dummy */
    ...
    const ValueIdSet divisioningColumns = optimizer_.getIndexDesc()->getPrimaryTableDesc()->getDivisioningColumns();

    NABoolean sameKeyPreds = FALSE;
    if (keyLength > 0 && divisioningColumns.entries() > 0 && CmpCommon::getDefault(MTD_GENERATE_CC_PREDS) == DF_ON) {
      const ValueIdSet *predsPtr = NULL;
      for (CollIndex i = 0; i < keyLength; i++) {
        ...
        if (predsPtr AND NOT predsPtr->isEmpty() && i > 0 && !sameKeyPreds) {
          optimizer_.categorizeProbes(disSuccProbes /* out */
                                      ...
                                      ,
                                      incomingProbes_, *predsPtr, outerHistograms_, TRUE  // this is MDAM!
                                      ,
    ...
    optimizer_.categorizeProbes(disSuccProbes /* out */
                                ...
                                disUniFailedProbes /* out, not used */
                                ,
                                incomingProbes_, disjunctKeyPreds_, outerHistograms_, TRUE  // this is MDAM!
                                ,
                                &multiProbesDataRows_);

    if (sameKeyPreds) rcAfterApplyFirstKeyPreds_ = multiProbesDataRows_;
    ...
    MDAM_DEBUG1(MTL2, "categorizeProbes returns rows: %f", multiProbesDataRows_.value());
...
void MDAMOptimalDisjunctPrefixWA::processLeadingColumns() {
  ...
  CostScalar firstColRowCntBefore =
      firstColumnHistogram_.getColStatsForColumn(optimizer_.getIndexDesc()->getIndexKey()[0])
          .getRowcount()
          .getCeiling();
  ...
  if (predsPtr == NULL OR predsPtr->isEmpty()) {
    ...
    if (rc.isGreaterThanZero()) prefixRows_ = rc;
  }

  firstColOverlaps_ =
      ((firstColRowCntBefore == firstColRowCntAfter) AND(disjunctIndex_ > 0) AND(firstColRowCntAfter == prefixRows_));
...
void MDAMOptimalDisjunctPrefixWA::processSingleSubsetPrefix() {
  ...
  if (NOT hasSingleSubsetPrefix) return;

  // The loops below starts at 1 because the 0th column has already been added
  for (CollIndex singleSubsetColPosition = 1; singleSubsetColPosition <= subsetOrder; singleSubsetColPosition++) {
    ...
    if (predsPtr AND NOT predsPtr->isEmpty()) prefixKeyPreds_.insert(*predsPtr);
  }
...
void MDAMOptimalDisjunctPrefixWA::processNonLeadingColumn() {
  ...
  // Note that the append method receives a one-based column position,
  // so add one to the prefix because the prefix is zero based:
  disjunctHistograms_.appendHistogramForColumnPosition(prefixColumnPosition_ + 1);
  ...
  const ColStats &colStats =
      disjunctHistograms_.getColStatsForColumn(optimizer_.getIndexDesc()->getIndexKey()[prefixColumnPosition_ - 1]);

  MDAM_DEBUGX(MTL2, colStats.print());

  uecForPreviousCol_ = colStats.getTotalUec().getCeiling();
  const ValueIdSet *prevPredsPtr = keyPredsByCol_[prefixColumnPosition_ - 1];
  if (prevPredsPtr && !prevPredsPtr->isEmpty() && !prevPredsPtr->containsOpType(ITM_CONSTANT)) {
    // There were predicates on the previous column, but none contained
    // any constants. So, no UEC reduction was done on the histogram.
    // Instead we must look at the key column predicates themselves to
    // get an idea of the true UEC. For example, a predicate of the form
    // X = ? should yield a UEC of 1.
    uecForPreviousCol_ = calculateUECFromKeyPreds(prevPredsPtr, uecForPreviousCol_);
  }

  MDAM_DEBUG1(MTL2, "processNonLeadingColumn(), total UEC, uecForPreviousCol_: %f:", uecForPreviousCol_.value());

  // The following code is incorrect. I am commenting it out for now, and will remove it
  // later. Here's why it is incorrect:
  //
  // The routine estimateUecUsingMultiColUec returns in its estimatedUec parameter the
  // UEC of the first n columns, as given by the multi-column histogram on those columns.
  // The value uecForPreviousCol_ is the UEC for the previous column, after its key
  // predicates were applied to its histogram. The value uecForPreviousColBeforeAppPreds_
  // is the UEC for the previous column before its key predicates were applied. So, the
  // ratio (uecForPreviousCol_ / uecForPreviousColBeforeAppPreds_) represents the
  // reduction in UEC for the previous column due to the key predicates on that column.
  // We then multiply that ratio by the UEC of the previous n columns (estimatedUec).
  // This is a completely meaningless quantity. At best, it could represent the UEC
  // of the previous n columns after applying the key predicates for the previous
  // column. But then we assign that to uecForPreviousCol_, which is supposed to be the
  // UEC of just one column.
  //
  // In the particular test case I was debugging, the column in question had a UEC of 10.
  // The result of this calculation raised its UEC to 40, which is clearly impossible.
  // Applying predicates to a column can NEVER increase its UEC.
  //
  // Note that the members sumOfUecsSoFar_ and uecForPreviousColBeforeAppPreds_ are
  // no longer used since this code has been commented out, and can be removed from
  // the class when the commented out code below is finally deleted.
  //
  // I think the intention of this code was to make MDAM more likely to be selected
  // if there is correlation between the key columns. Intuitively this is desireable
  // since correlation will reduce the net number of MDAM probes. The effect of this
  // code, however, was to make MDAM *less* likely to be selected, as the incorrect
  // calculation would often boost the value of uecForPreviousCol_. By commenting this
  // code out, we are abandoning (for now anyway) the attempt to take correlation
  // among key columns into account, but paradoxically improving the chances of MDAM
  // being selected due to the incorrectness of the calculation.
  ...
  /* CostScalar estimatedUec = csOne;
   ...
   if(multiColUecInfoAvail_ AND
      ...
      disjunctHistograms_.
      estimateUecUsingMultiColUec(keyPredsByCol_, /xinx/
                                  prefixColumnPosition_ - 1, /xinx/
                                  estimatedUec /xoutx/))
     {
       ...
       MDAM_DEBUG1(MTL2, "processNonLeadingColumn(), modify by MC uec, uecForPreviousCol_: %f:",
   uecForPreviousCol_.value());
...
void MDAMOptimalDisjunctPrefixWA::applyPredsToHistogram() {
  NABoolean getRowCount = TRUE;
  uecForPreviousColBeforeAppPreds_ =
      disjunctHistograms_.getColStatsForColumn(optimizer_.getIndexDesc()->getIndexKey()[prefixColumnPosition_])
          .getTotalUec()
          .getCeiling();
  ...
  if (predsPtr AND(NOT predsPtr->isEmpty())) {
    ...
    MDAM_DEBUG0(MTL2, "Applying predicates to disjunct histograms");
    MDAM_DEBUGX(MTL2, predsPtr->print());
    if (NOT crossProductApplied_ AND isMultipleProbes_) {
      MDAM_DEBUG0(MTL2, "Applying cross product");
      ...
      if (prefixColumnPosition_ == lastColumnPosition_ - 1) {
        ...
        MDAM_DEBUG1(MTL2, "prefixRows_ comes from outerHist: %f", prefixRows_.value());
      } else {
        disjunctHistograms_.applyPredicatesWhenMultipleProbes(
            *predsPtr, *(optimizer_.getContext().getInputLogProp()),
            optimizer_.getRelExpr().getGroupAttr()->getCharacteristicInputs(), TRUE  // doing MDAM!
            ,
            selHint, cardHint, NULL, REL_SCAN);
      }
    } else
  ...
  if (keyPredsByCol_.getPredicateExpressionPtr(prefixColumnPosition_)
          AND(keyPredsByCol_.getPredicateExpressionPtr(prefixColumnPosition_)->getType() ==
              KeyColumns::KeyColumn::CONFLICT_EQUALS OR keyPredsByCol_.getPredicateExpressionPtr(prefixColumnPosition_)
                  ->getType() == KeyColumns::KeyColumn::EQUAL)) {
...
void MDAMOptimalDisjunctPrefixWA::computeDensityOfColumn() {
  ...
  if (scanForcePtr_ && mdamForced_) {
    sparseForced =
        ((scanForcePtr_->getEnumAlgorithmForColumn(prefixColumnPosition_) == ScanForceWildCard::COLUMN_SPARSE) ? TRUE
                                                                                                               : FALSE);
    denseForced =
        ((scanForcePtr_->getEnumAlgorithmForColumn(prefixColumnPosition_) == ScanForceWildCard::COLUMN_DENSE) ? TRUE
                                                                                                              : FALSE);
  }
  ...
  if (sparseForced OR denseForced) {
    ...
    if (prefixColumnPosition_ == 0) {
      ...
      const ColStats &firstColumnColStats =
          disjunctHistograms_.getColStatsForColumn(optimizer_.getIndexDesc()->getIndexKey()[0]);
...
void MDAMOptimalDisjunctPrefixWA::updatePositions() {
  ...
  if (prefixColumnPosition_ == 0) {
    prefixSubsets_ = MINOF(innerRowsUpperBound_.getValue(), uecForPreviousCol_.getValue());
    prefixSubsetsAsSeeks_ = prefixSubsets_;
    prefixRqstsForSubsetBoundaries_ = csZero;
    ...
    // (i.e. we are in order 1) if we already added them
    // in a previous subset:
    if ((prefixColumnPosition_ == 1 AND NOT firstColOverlaps_) OR prefixColumnPosition_ > 1) {
      // the UEC for the previous column
      // may be zero (if the table was empty
      // or all the rows are eliminated after
      // preds were applied.) In this
      ...
      if (uecForPreviousCol_.isGreaterThanZero()) {
        ...
        prefixSubsets_ = MINOF(innerRowsUpperBound_.getValue(), prefixSubsets_.getValue());
      ...
      if (uecForPrevColForSeeks_.isGreaterThanZero()) {
        ...
        prefixSubsetsAsSeeks_ = MINOF(innerRowsUpperBound_.getValue(), prefixSubsetsAsSeeks_.getValue());
      }
      ...
      if (mdamKeyPtr_->isColumnSparse(prefixColumnPosition_ - 1)) {
        ...
        prefixRqstsForSubsetBoundaries_ =
            MINOF(innerRowsUpperBound_.getValue(), prefixRqstsForSubsetBoundaries_.getValue());
      }
    }  // non-overlapping
...
void MDAMOptimalDisjunctPrefixWA::updateStatistics() {
  // the subsets requests are issued for every probe:
  CostScalar effectiveProbes = incomingProbes_ - failedProbes_;
  ...
  if (NOT prefixRqsts_.isLessThanOne() AND prefixRows_.isGreaterThanZero()) {
  ...
  computeTotalBlocksLowerBound(disjunctBlocksToRead /*out*/
                               ,
                               prefixSubsetsAsSeeks_, rowsPerSubset, estimatedRecordsPerBlock_, innerBlocksUpperBound_);
  ...
  computeBeginBlocksLowerBound(
      ...
      ,
      MINOF((prefixSubsetsAsSeeks_ / subsetsPerBlock).getCeiling(), prefixSubsetsAsSeeks_.getValue()),
  ...
  // that will be used in for MDAM access.
  CostScalar indexBlocksLowerBound = optimizer_.getIndexDesc()->getEstimatedIndexBlocksLowerBound(MINOF(
      ((prefixSubsets_ + prefixRqstsForSubsetBoundaries_) / subsetsPerBlock).getCeiling(), prefixSubsets_.getValue()));
  // Assume that every disjunct does not overlap
  // with the previous. Since we computed all
  // rows to read (and not a lower bound),
  // the following routine will compute
  ...
  if ((prefixSeeks_ - beginBlocksLowerBound).getValue() >= 5) {
    changeBack = TRUE;
    prefixSeeks_ -= 5;
    prefixKBRead_ = prefixKBRead_ - CostScalar(5) * optimizer_.getIndexDesc()->getBlockSizeInKb();
  }
  ...
  if (changeBack) {
    prefixSeeks_ += 5;
    prefixKBRead_ += CostScalar(5) * optimizer_.getIndexDesc()->getBlockSizeInKb();
  }
}
...
void MDAMOptimalDisjunctPrefixWA::updateMinPrefix() {
  ...
  if (CmpCommon::getDefault(SIMPLE_COST_MODEL) == DF_ON) {
    ...
    CostScalar scmPrefixIOSeq =
        (prefixKBRead_ / optimizer_.getIndexDesc()->getBlockSizeInKb()).getCeiling() * rowSizeFactorSeqIO;
    ...
    if (optimizer_.getFileScan().isHbaseTable() && (CmpCommon::getDefault(NCM_HBASE_COSTING) == DF_ON))
      numActivePartitions = optimizer_.getEstNumActivePartitionsAtRuntimeForHbaseRegions();
    else
    ...
    scmCost = optimizer_.scmCost(scmPrefixRows, scmPrefixOutputRows, csZero, scmPrefixIORand, scmPrefixIOSeq,
                                 incomingProbes_, rowSize, csZero, outputRowSize, csZero);
    ...
    optimizer_.computeCostVectorsForMultipleSubset(prefixFR /*out*/
                                                   ,
                                                   prefixLR /*out*/
                                                   ,
                                                   seqKBytesPerScan /*out unused*/
                                                   ,
                                                   prefixRows_, prefixRqsts_ + prefixRqstsForSubsetBoundaries_,
                                                   failedProbes_, prefixSeeks_, prefixKBRead_, prefixKeyPreds_,
                                                   exePreds_, incomingProbes_, CostScalar(prefixKeyPreds_.entries()));
  }
  ...
  if (scanForcePtr_ && mdamForced_) {
    proceedViaCosting = FALSE;
    if (noExePreds_ AND scanForcePtr_->getNumMdamColumns() < lastColumnPosition_) noExePreds_ = FALSE;
    if (prefixColumnPosition_ < scanForcePtr_->getNumMdamColumns()) {
      ...
      if (scanForcePtr_->getMdamColumnsStatus() == ScanForceWildCard::MDAM_COLUMNS_ALL) {
        // Unconditionally force all of the columns:
        newMinimumFound = TRUE;
      } else if (scanForcePtr_->getMdamColumnsStatus() == ScanForceWildCard::MDAM_COLUMNS_NO_MORE) {
        // Unconditionally reject this and later columns:
        newMinimumFound = FALSE;
        prevPredIsEqual_ = FALSE;  // ?
        curPredIsEqual_ = FALSE;   // ?
      } else if (scanForcePtr_->getMdamColumnsStatus() == ScanForceWildCard::MDAM_COLUMNS_REST_BY_SYSTEM) {
        // Let the system decide based on costing:
        proceedViaCosting = TRUE;
      }
    }
  ...
  if (proceedViaCosting) {
    ...
    if (CmpCommon::getDefault(SIMPLE_COST_MODEL) == DF_ON) {
      ...
      newMinimumFound = (pMinCost_ == NULL) ? TRUE : (scmCost->scmCompareCosts(*pMinCost_) == LESS);
    } else {
      newMinimumFound = NOT optimizer_.exceedsBound(pMinCost_, prefixFR, prefixLR);
    }
  }
  ...
  if (firstRound_ OR newMinimumFound OR(
          prevColChosen_ AND keyPredsByCol_[prefixColumnPosition_] AND NOT prefixKeyPreds_.isEmpty()
    ...
    if (prefixColumnPosition_ == lastColumnPosition_ - 1) {  // we have reached the lastcolumnPosition and its cost
      // was exceeded for the lastcolumn so
      // there will be additional exepreds
      noExePreds_ = FALSE;
    } else  //  This is not the last column and it wasn't chosen
...
CostScalar MDAMOptimalDisjunctPrefixWA::calculateUECFromKeyPreds(const ValueIdSet *predsPtr, CostScalar &maxUEC) {
  ...
  CostScalar result = calculateUECFromKeyPred(vid, maxUEC, lessCount, greaterCount);
  predsPtr->advance(vid);

  while (predsPtr->next(vid)) {
    CostScalar result1 = calculateUECFromKeyPred(vid, maxUEC, lessCount, greaterCount);
    ...
    result = MINOF(result, result1);
    predsPtr->advance(vid);
  }

  if (result > maxUEC) result = maxUEC;

  // special handling if both A < ? and A > ? present
  if ((lessCount > 0) && (greaterCount > 0)) {
    CostScalar selectionFactor = CostPrimitives::getBasicCostFactor(HIST_DEFAULT_SEL_FOR_PRED_RANGE);
    CostScalar andedRange = maxUEC * selectionFactor * selectionFactor;
    if (andedRange < result) result = andedRange;
    if (result < csOne) result = csOne;
  }

  return result;
}

CostScalar MDAMOptimalDisjunctPrefixWA::calculateUECFromKeyPred(const ValueId &keyPred, CostScalar &maxUEC,
                                                                int &lessCount, int &greaterCount) {
  ...
  switch (ie->getOperatorType()) {
    ...
    case ITM_LESS_EQ: {
      lessCount++;
      result = maxUEC * CostPrimitives::getBasicCostFactor(HIST_DEFAULT_SEL_FOR_PRED_RANGE);
      if (result < csOne) result = csOne;
      break;
    }
    case ITM_GREATER:
    ...
    case ITM_OR: {
      ...
      result = calculateUECFromKeyPred(ie->child(0), maxUEC, localLessCount, localGreaterCount) +
               calculateUECFromKeyPred(ie->child(0), maxUEC, localLessCount, localGreaterCount);
      break;
    }
    case ITM_AND:  // shouldn't happen since we are in disjunctive normal form
    {
      result = MINOF(calculateUECFromKeyPred(ie->child(0), maxUEC, lessCount, greaterCount),
                     calculateUECFromKeyPred(ie->child(0), maxUEC, lessCount, greaterCount));
      break;
    }
    default: {
      result = csOne;  // unexpected operator
...
CollIndex MDAMOptimalDisjunctPrefixWA::getStopColumn() const { return stopColumn_; }

CollIndex MDAMOptimalDisjunctPrefixWA::getNumLeadingPartPreds() const { return numLeadingPartPreds_; }

const CostScalar &MDAMOptimalDisjunctPrefixWA::getOptRows() const { return optRows_; }

const CostScalar &MDAMOptimalDisjunctPrefixWA::getOptRqsts() const { return optRqsts_; }

const CostScalar &MDAMOptimalDisjunctPrefixWA::getFailedProbes() const { return failedProbes_; }

const CostScalar &MDAMOptimalDisjunctPrefixWA::getOptProbesForSubsetBoundaries() const {
  return optRqstsForSubsetBoundaries_;
}

const CostScalar &MDAMOptimalDisjunctPrefixWA::getOptSeeks() const { return optSeeks_; }

const CostScalar &MDAMOptimalDisjunctPrefixWA::getOptSeqKBRead() const { return optSeqKBRead_; }

const ValueIdSet &MDAMOptimalDisjunctPrefixWA::getOptKeyPreds() const { return optKeyPreds_; }

// ZZZZZ New MDAM costing code goes here

NewMDAMCostWA::NewMDAMCostWA(FileScanOptimizer &optimizer, NABoolean mdamForced, MdamKey *mdamKeyPtr,
                             const Cost *costBoundPtr, const ValueIdSet &exePreds, const CostScalar &singleSubsetSize)
    : mdamWon_(FALSE),
      noExePreds_(TRUE),
      numKBytes_(0),
      scmCost_(NULL),
      ...
      ,
      estimatedRowsPerBlock_(optimizer.getIndexDesc()->getEstimatedRecordsPerBlock()),
      disjunctIndex_(0)
...
void NewMDAMCostWA::compute() {
  ...
  if (optimizer_.getIndexDesc()->getPrimaryTableDesc()->getNATable()->isHbaseTable()) {
    ...
    CostScalar refinedSeqBlocksReadUpperBound =
        MINOF(seqBlocksReadUpperBound, seqBlocksFetchUpperBound + seqBlocksProbeUpperBound);
    ...
    if (refinedSeqBlocksReadUpperBound < seqBlocksReadUpperBound) {
      CostScalar difference = seqBlocksReadUpperBound - refinedSeqBlocksReadUpperBound;
    ...
    CostScalar totalSeqKBRead = refinedSeqBlocksReadUpperBound * optimizer_.getIndexDesc()->getBlockSizeInKb();
    ...
    scmCost_ = optimizer_.scmComputeMDAMCostForHbase(totRowsProcessed, seeks, totalSeqKBRead, incomingScanProbes_);
  ...
  if (!mdamForced_ && (costBoundPtr_ != NULL) && (costBoundPtr_->scmCompareCosts(*scmCost_) == LESS)) {
    mdamWon_ = FALSE;
    MDAM_DEBUG0(MTL2, "Mdam scan lost due to higher cost determined by scmCompareCosts()");
    return;
  }
...
void NewMDAMCostWA::computeDisjunct() {
  ...
  // compute the optimal prefix and the associated min cost
  NewMDAMOptimalDisjunctPrefixWA prefixWA(optimizer_, keyPredsByCol, disjunctKeyPreds, exePreds_, outerHistograms_,
                                          mdamKeyPtr_, scanForcePtr_, mdamForced_, isMultipleScanProbes_,
                                          incomingScanProbes_, estimatedRowsPerBlock_, innerRowsUpperBound_,
                                          innerBlocksUpperBound_, singleSubsetSize_, disjunctIndex_);
...
NABoolean NewMDAMCostWA::isMdamWon() const { return mdamWon_; }

NABoolean NewMDAMCostWA::hasNoExePreds() const { return noExePreds_; }

const CostScalar &NewMDAMCostWA::getNumKBytes() const { return numKBytes_; }

Cost *NewMDAMCostWA::getScmCost() { return scmCost_; }

// Implementation of NewMDAMOptimalDisjunctPrefixWA methods

NewMDAMOptimalDisjunctPrefixWA::NewMDAMOptimalDisjunctPrefixWA(
    FileScanOptimizer &optimizer, const ColumnOrderList &keyPredsByCol, const ValueIdSet &disjunctKeyPreds,
    const ValueIdSet &exePreds, const Histograms &outerHistograms, MdamKey *mdamKeyPtr,
    const ScanForceWildCard *scanForcePtr, NABoolean mdamForced, NABoolean isMultipleProbes,
    const CostScalar &incomingScanProbes, const CostScalar &estimatedRecordsPerBlock,
    const CostScalar &innerRowsUpperBound, const CostScalar &innerBlocksUpperBound, const CostScalar &singleSubsetSize,
    CollIndex disjunctIndex)
...
NewMDAMOptimalDisjunctPrefixWA::~NewMDAMOptimalDisjunctPrefixWA() {
  if (optimalCost_) delete optimalCost_;
}
...
void NewMDAMOptimalDisjunctPrefixWA::compute(NABoolean &noExePreds /*out*/, CostScalar &incomingScanProbes) {
  ...
  for (CollIndex i = 0; i < keyColumns.entries(); i++) {
    ...
    CostScalar ColumnUECBeforePreds = disjunctHistograms_.getColStatsForColumn(keyColumn).getTotalUec().getCeiling();
    ...
    CostScalar columnUEC = disjunctHistograms_.getColStatsForColumn(keyColumn).getTotalUec().getCeiling();
    ...
    if (predsPtr AND(NOT predsPtr->isEmpty())) {
      calculateMetricsFromKeyPreds(keyColumn, predsPtr, ColumnUECBeforePreds, MDAMUECEstimate /*out*/,
                                   MDAMIntervalEstimate /*out*/);
    ...
    if (MDAMUECEstimate < columnUEC) columnUEC = MDAMUECEstimate;
    ...
    if (multiColUecInfoAvail_) {
      ...
      NABoolean UECFromMCFound = disjunctHistograms_.estimateUecUsingMultiColUec(keyPredsByCol_, /*in*/
                                                                                 i,              /*in*/
                                                                                 UECFromMCHistograms /*out*/);
      if (UECFromMCFound) {
        MDAM_DEBUG2(MTL2, "Column UEC of prefix %d from MC histograms: %f:", i, UECFromMCHistograms.value());
        ...
        if (MCUECOfPriorPrefixFound_) {
          ...
          MDAM_DEBUG1(MTL2, "Ratio of prefix UEC to prior prefix UEC: %f:", columnUECFromMCUECRatio.value());

          if (columnUECFromMCUECRatio < columnUEC) columnUEC = columnUECFromMCUECRatio;
    ...
    CostScalar MDAMProbeSubsets = isDense ? csZero : previousColumnMDAMProbeRows * (columnUEC + MDAMIntervalEstimate);
    ...
    CostScalar MDAMFetchRows = disjunctHistograms_.getColStatsForColumn(optimizer_.getIndexDesc()->getIndexKey()[i])
                                   .getRowcount()
                                   .getCeiling();
    if (crossProductApplied_) MDAMFetchRows = MDAMFetchRows / incomingScanProbes;
    ...
    Cost *currentCost =
        optimizer_.scmComputeMDAMCostForHbase(totalRowsProcessed, totalSeeks, totalSeqKBRead, incomingScanProbes_);
    ...
    // if ((optimalCost_ == NULL) OR (currentCost->compareCosts(*optimalCost_) == LESS))
    {
      ...
      if (i == keyColumns.entries() - 1) noExePreds = FALSE;
    }
...
NABoolean NewMDAMOptimalDisjunctPrefixWA::isColumnDense(CollIndex columnPosition) {
  ...
  // Check if scan is being forced
  // if so check if density is forced too
  if (scanForcePtr_ && mdamForced_) forceOption = scanForcePtr_->getEnumAlgorithmForColumn(columnPosition);
...
void NewMDAMOptimalDisjunctPrefixWA::calculateMetricsFromKeyPreds(const ValueId &keyColumn, const ValueIdSet *predsPtr,
                                                                  const CostScalar &maxUEC, CostScalar &UECFromPreds,
                                                                  CostScalar &IntervalCountFromPreds) {
  // If the key predicates for a column are all of the form
  // column op constant (or column IS NULL), then the UEC of
  // the result after applying key predicates can be read from
  // the histogram. If even some of the key predicates are
  ...
  ValueId vid = predsPtr->init();
  predsPtr->next(vid);  // expect it to return true, as caller insured predsPtr was not empty
  calculateMetricsFromKeyPred(keyColumn, vid, maxUEC, UECFromPreds /*out*/, IntervalCountFromPreds /*out*/,
  ...
  while (predsPtr->next(vid)) {
    ...
    calculateMetricsFromKeyPred(keyColumn, vid, maxUEC, UECFromPreds1 /*out*/, IntervalCountFromPreds1 /*out*/,
                                lessCount /*in/out*/, greaterCount /*in/out*/);
    // this predicate is ANDed with the previous; so the
    // UEC is at most the min of the two (but see below for
    // special handling of the case A > ? AND A < ?)
  ...
  if (UECFromPreds > maxUEC) UECFromPreds = maxUEC;

  // special handling if both A < ? and A > ? present
  if ((lessCount > 0) && (greaterCount > 0)) {
    CostScalar selectionFactor = CostPrimitives::getBasicCostFactor(HIST_DEFAULT_SEL_FOR_PRED_RANGE);
    CostScalar andedRange = maxUEC * selectionFactor * selectionFactor;
    if (andedRange < UECFromPreds) UECFromPreds = andedRange;
    if (UECFromPreds < csOne) UECFromPreds = csOne;
  }
}

void NewMDAMOptimalDisjunctPrefixWA::calculateMetricsFromKeyPred(
    const ValueId &keyColumn, const ValueId &keyPred, const CostScalar &maxUEC, CostScalar &UECFromPreds /*out*/,
    CostScalar &IntervalCountFromPreds /*out*/, int &lessCount /*in/out*/, int &greaterCount /*in/out*/) {
  ItemExpr *ie = keyPred.getItemExpr();
  switch (ie->getOperatorType()) {
    case ITM_LESS:
    case ITM_LESS_EQ: {
    ...
    case ITM_OR: {
      ...
      calculateMetricsFromKeyPred(keyColumn, ie->child(0), maxUEC, UECFromPreds0, IntervalCountFromPreds0,
                                  localLessCount, localGreaterCount);
      CostScalar UECFromPreds1;
      CostScalar IntervalCountFromPreds1;
      calculateMetricsFromKeyPred(keyColumn, ie->child(1), maxUEC, UECFromPreds1, IntervalCountFromPreds1,
                                  localLessCount, localGreaterCount);
    ...
    case ITM_AND: {
      CostScalar UECFromPreds0;
      CostScalar IntervalCountFromPreds0;
      calculateMetricsFromKeyPred(keyColumn, ie->child(0), maxUEC, UECFromPreds0, IntervalCountFromPreds0, lessCount,
                                  greaterCount);
      CostScalar UECFromPreds1;
      CostScalar IntervalCountFromPreds1;
      calculateMetricsFromKeyPred(keyColumn, ie->child(1), maxUEC, UECFromPreds1, IntervalCountFromPreds1, lessCount,
                                  greaterCount);
...
NABoolean NewMDAMOptimalDisjunctPrefixWA::keyColumnIsOnTheLeft(const ValueId &keyColumn, ItemExpr *ie) {
  if (ie->getArity() >= 2) {
    ValueId leftChild = ie->child(0);
    ItemExpr *leftChildie = leftChild.getItemExpr();
    if (leftChildie->containsTheGivenValue(keyColumn)) return TRUE;
  }
...
NABoolean NewMDAMOptimalDisjunctPrefixWA::isMinimalCost(Cost *currentCost, CollIndex columnPosition,
  ...
  if (scanForcePtr_ && mdamForced_) {
    if (columnPosition < scanForcePtr_->getNumMdamColumns()) {
      ...
      if (scanForcePtr_->getMdamColumnsStatus() == ScanForceWildCard::MDAM_COLUMNS_ALL) {
        ...
        DCMPASSERT(scanForcePtr_->getMdamColumnsStatus() == ScanForceWildCard::MDAM_COLUMNS_REST_BY_SYSTEM);
  ...
  if (NOT forced) {
    ...
    newMinimumFound = (optimalCost_ == NULL) ? TRUE : (currentCost->scmCompareCosts(*optimalCost_) == LESS);
  }

  return newMinimumFound;
}

const CostScalar &NewMDAMOptimalDisjunctPrefixWA::getOptMDAMProbeRows() const { return optMDAMProbeRows_; }

const CostScalar &NewMDAMOptimalDisjunctPrefixWA::getOptMDAMProbeSubsets() const { return optMDAMProbeSubsets_; }

const CostScalar &NewMDAMOptimalDisjunctPrefixWA::getOptMDAMFetchRows() const { return optMDAMFetchRows_; }

const CostScalar &NewMDAMOptimalDisjunctPrefixWA::getOptMDAMFetchSubsets() const { return optMDAMFetchSubsets_; }

CollIndex NewMDAMOptimalDisjunctPrefixWA::getStopColumn() const { return optStopColumn_; }

// ZZZZZ End of new MDAM costing code

// return true if has resuable shared basic cost for this mdam
NABoolean FileScanOptimizer::getSharedCost(FileScanBasicCost *&fileScanBasicCostPtr /*out, never NULL*/
                                           ,
  ...
  sharedMdamKeyPtr = fileScanBasicCostPtr->getMdamKeyPtr(mdamTypeIsCommon);
  return (sharedCostFound AND sharedMdamKeyPtr AND
              //       disjunctsFRPtr->getCPUTime() > csZero AND
              //       disjunctsLRPtr->getCPUTime() > csZero AND
              CURRSTMT_OPTDEFAULTS->reuseBasicCost());
}

Cost *FileScanOptimizer::newComputeCostForMultipleSubset(MdamKey *mdamKeyPtr, const Cost *costBoundPtr,
                                                         NABoolean mdamForced, CostScalar &numKBytes,
                                                         ValueIdSet exePreds, NABoolean checkExePreds,
                                                         NABoolean mdamTypeIsCommon, MdamKey *&sharedMdamKeyPtr) {
  MDAM_DEBUG0(MTL2, "BEGIN MDAM Costing --------");
  MDAM_DEBUGX(MTL2, MdamTrace::printCostObject(this, costBoundPtr, "Cost Bound"));
  // MDAM only works for key sequenced files.
  DCMPASSERT(getIndexDesc()->getNAFileSet()->isKeySequenced());
  ...
  NABoolean shareCost = getSharedCost(fileScanBasicCostPtr, hasLostBefore, disjunctsFRPtr, disjunctsLRPtr, numKBytes,
                                      sharedMdamKeyPtr, mdamTypeIsCommon);
  ...
  MDAMCostWA costWA(*this, mdamForced, mdamKeyPtr, costBoundPtr, exePreds, disjunctsFR, disjunctsLR);
  costWA.compute();
  ...
  // noExePreds is true, great set the flag in MDAM
  if (CmpCommon::getDefault(RANGESPEC_TRANSFORMATION) == DF_ON) {
    if (!exePreds.entries()) noExePreds = TRUE;
  }
  if (noExePreds AND checkExePreds) {
    mdamKeyPtr->setNoExePred();
  }
...
Cost *FileScanOptimizer::scmComputeCostForMultipleSubset(MdamKey *mdamKeyPtr, const Cost *costBoundPtr,
                                                         NABoolean mdamForced, CostScalar &numKBytes,
                                                         ValueIdSet exePreds, NABoolean checkExePreds,
                                                         NABoolean mdamTypeIsCommon, MdamKey *&sharedMdamKeyPtr) {
  // MDAM only works for key sequenced files.
  ...
  MDAMCostWA costWA(*this, mdamForced, mdamKeyPtr, costBoundPtr, exePreds, a, b);

  costWA.compute();
  NABoolean isMdamWon = costWA.isMdamWon();
  if (NOT isMdamWon) return NULL;
...
Cost *FileScanOptimizer::scmRewrittenComputeCostForMultipleSubset(MdamKey *mdamKeyPtr, const Cost *costBoundPtr,
                                                                  NABoolean mdamForced, CostScalar &numKBytes,
                                                                  ValueIdSet exePreds, NABoolean checkExePreds,
                                                                  MdamKey *&sharedMdamKeyPtr) {
  // MDAM only works for key sequenced files.
  ...
  NewMDAMCostWA costWA(*this, mdamForced, mdamKeyPtr, costBoundPtr, exePreds, singleSubsetSize_);
...
const ScanForceWildCard *FileScanOptimizer::findScanForceWildCard() const {
  ...
  if (propertyPtr && propertyPtr->getMustMatch() &&
      (propertyPtr->getMustMatch()->getOperatorType() == REL_FORCE_ANY_SCAN)) {
    scanForcePtr = (const ScanForceWildCard *)propertyPtr->getMustMatch();
  }
  return scanForcePtr;
}
...
NABoolean FileScanBasicCost::hasMdamCommonLost() const { return mdamCommonLost_; }

NABoolean FileScanBasicCost::hasMdamDisjunctsLost() const { return mdamDisjunctsLost_; }

void FileScanBasicCost::setMdamCommonLost(NABoolean mdamCommonLost) { mdamCommonLost_ = mdamCommonLost; }

void FileScanBasicCost::setMdamDisjunctsLost(NABoolean mdamDisjunctsLost) { mdamDisjunctsLost_ = mdamDisjunctsLost; }
