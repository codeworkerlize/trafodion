...
private:
  ...
  CostScalar calculateUECFromKeyPred(const ValueId & keyPred, 
    CostScalar & maxUEC, int & lessCount, int & greaterCount);
...
ScanOptimizer::isMdamEnabled() const
{
  ...
  if (idesc->getPrimaryTableDesc()->getNATable()->isExtStorage())
    mdamIsEnabled = FALSE;
...
void MDAMOptimalDisjunctPrefixWA::processNonLeadingColumn()
{
  ...
  if (prevPredsPtr &&
      !prevPredsPtr->isEmpty() &&
      !prevPredsPtr->containsOpType(ITM_CONSTANT))
    {
...
CostScalar MDAMOptimalDisjunctPrefixWA::calculateUECFromKeyPred(const ValueId & keyPred, CostScalar & maxUEC, int & lessCount, int & greaterCount)
{
