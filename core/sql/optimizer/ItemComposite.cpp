...
// ----------------------------------------------------------------------
NABoolean CompositeArrayLength::isCacheableExpr(CacheWA& cwa)
{
  ...
  if (NOT (childType->getFSDatatype() == REC_ARRAY))
    {
      *CmpCommon::diags() << DgSqlCode(-3242)
                          << DgString0("Operand of array_length must be of Array datatype.");
      bindWA->setErrStatus();
      return NULL;
    }
  ...
  if (type_ == ARRAY_TYPE)
    {
      elem = (*iel)[0];
      SQLArray *arrType = 
        new HEAP SQLArray(HEAP, &elem->getValueId().getType(), numEntries);
...
ItemExpr *CompositeCreate::bindNode(BindWA *bindWA)
{
  ...
  if ((CmpCommon::getDefault(TRAF_COMPOSITE_DATATYPE_SUPPORT) == DF_OFF) &&
      (CmpCommon::getDefault(HIVE_COMPOSITE_DATATYPE_SUPPORT) == DF_OFF))
    {
      *CmpCommon::diags()
        << DgSqlCode(-3242)
        << DgString0("Cannot specify composite array or row constructor.");
      bindWA->setErrStatus();
      return NULL;
    }

  ItemExprList *iel = 
    new(bindWA->wHeap()) ItemExprList(child(0), bindWA->wHeap());

  ItemExprList *newIEL =
    new(bindWA->wHeap()) ItemExprList(iel->entries(), bindWA->wHeap());
      ...
      for (int i = 0; i < iel->entries(); i++)
        {
          ...
          if ((elem->getOperatorType() == ITM_CONSTANT) &&
              (((ConstValue*)elem)->isNull()))
            elemIsNullConst = TRUE;
          ...
          if ((!resultType) && (NOT elemIsNullConst)) 
          ...
          else
            {
              ...
              resultType = 
                resultType->synthesizeType
                (SYNTH_RULE_UNION, opR, opC, bindWA->wHeap());
              if (! resultType)
                {
                  *CmpCommon::diags() << DgSqlCode(-3242)
                                      << DgString0("ARRAY elements must have compatible types.");
                  bindWA->setErrStatus();
                  return NULL;
                } 
            }
...
const NAType * CompositeCast::synthesizeType()
{
  return type_;
}
...
ItemExpr *CompositeCast::arrayExtendOrTruncate(BindWA *bindWA)
{
  ...
  ItemExprList *ieNE = new(bindWA->wHeap()) 
    ItemExprList(myCompType.getNumElements(), bindWA->wHeap());
  ...
  Int32 nullElems = 
    (myCompType.getNumElements() > childCompType.getNumElements() 
     ? (myCompType.getNumElements() - childCompType.getNumElements()) : 0);
  ItemExpr *ie = NULL;
  ...
  ItemExpr *cc = new(bindWA->wHeap()) 
    CompositeCreate(ieNE->convertToItemExpr(), 
...
ItemExpr *CompositeCast::bindNode(BindWA *bindWA)
{
  ...
  if (child0->getOperatorType() == ITM_CONSTANT)
    if (((ConstValue*)child0)->isNull())
      childIsNullConst = TRUE;
  ...
  if (NOT type_->isComposite())
    {
      *CmpCommon::diags() << DgSqlCode(-3242)
                          << DgString0("Target must be composite(ARRAY or ROW) datatype.");
      bindWA->setErrStatus();
      return NULL;
    }
  ...
  if (NOT childType->isComposite())
    {
      *CmpCommon::diags() << DgSqlCode(-3242)
                          << DgString0("Operand must be composite(ARRAY or ROW) datatype.");
      bindWA->setErrStatus();
      return NULL;
    }
  ...
  if (NOT (childType->isCompatible(*type_)))
    {
      *CmpCommon::diags() << DgSqlCode(-3242)
                          << DgString0("Type of operand and target must be compatible.");
      bindWA->setErrStatus();
      return NULL;
    }
  ...
  if (isExtendOrTruncate_)
    {
      if ((myCompType.getFSDatatype() != REC_ARRAY) ||
          (myCompType.getElementType(1) != NULL))
        {
          *CmpCommon::diags() << DgSqlCode(-3242)
                              << DgString0("Extend or Truncate can only be used on Array datatype.");
          bindWA->setErrStatus();
          return NULL;                    
        }
  ...
  ItemExprList *iel = new(bindWA->wHeap()) 
    ItemExprList(myCompType.getNumElements(), bindWA->wHeap());
  ...
  for (int i = 0; i < childCompType->getNumElements(); i++)
    {
      ...
      ItemExpr *cast = ce;
      if (NOT (myCompType.getElementType(i+1)->equalIgnoreCoercibility
               (ce->getValueId().getType())))
        {
          cast = new(bindWA->wHeap()) 
            Cast(ce, myCompType.getElementType(i+1)); // getElementType is 1-based
        }
  ...
  ItemExpr *cc = new(bindWA->wHeap()) 
    CompositeCreate(iel->convertToItemExpr(), 
                    childCompType->getFSDatatype() == REC_ARRAY 
                    ? CompositeCreate::ARRAY_TYPE : CompositeCreate::ROW_TYPE);
  cc = cc->bindNode(bindWA);
  if(bindWA->errStatus()) 
    return this;
...
// ----------------------------------------------------------------------
NABoolean CompositeConcat::isCacheableExpr(CacheWA& cwa)
{
  return ItemExpr::isCacheableExpr(cwa);
}
...
const NAType * CompositeConcat::synthesizeType()
{
  ...
  SQLArray *sa = 
    new HEAP SQLArray
    (HEAP, child1SA.getElementType(), 
     (child1SA.getNumElements() + child2SA.getNumElements()),
     COM_SQLARK_EXPLODED_FORMAT);
...
ItemExpr *CompositeConcat::bindNode(BindWA *bindWA)
{
  ...
  if ((child1Type.getFSDatatype() != REC_ARRAY) ||
      (child2Type.getFSDatatype() != REC_ARRAY))
    {
      *CmpCommon::diags() << DgSqlCode(-3242)
                          << DgString0("Only ARRAY operands can be concatenated.");
      bindWA->setErrStatus();
      return NULL;
    }
  ...
  const NAType *resultType = 
    child1SA.getElementType()->synthesizeType
    (SYNTH_RULE_UNION, *child1SA.getElementType(), 
  ...
  if (NOT (*child1SA.getElementType() == *resultType))
    {
      // convert elementType to resultType
      SQLArray *sa = 
        new(bindWA->wHeap()) SQLArray(bindWA->wHeap(), resultType, 
                                      child1SA.getNumElements());
...
const NAType * CompositeDisplay::synthesizeType()
{
  ...
  NAType *type = new HEAP SQLVarChar(HEAP, displayLength, 
...
const NAType * CompositeExtract::synthesizeType()
{
  return resultType_;
}
...
ItemExpr *CompositeExtract::bindNode(BindWA *bindWA)
{
  ...
  if (elemNum_ > 0)
  ...
  else if (child1)
    {
      ...
      const NumericType &child1Type = 
        (NumericType&)child1->getValueId().getType();
      if (NOT ((child1Type.getTypeQualifier() == NA_NUMERIC_TYPE) &&
               (child1Type.isExact()) && (child1Type.getScale() == 0) &&
               (child1Type.getFSDatatype() == REC_BIN32_UNSIGNED)))
        {
          *CmpCommon::diags() << DgSqlCode(-3242)
                              << DgString0("ElemNum expression must be Int32 with scale of zero.");
          bindWA->setErrStatus();
          return NULL;          
        }
     ...
     if (ct.getFSDatatype() != REC_ARRAY)      
       {
         *CmpCommon::diags() << DgSqlCode(-3242)
                             << DgString0("Operand must be ARRAY datatype.");
         bindWA->setErrStatus();
         return NULL;
       }
              ...
              if (indexes_[0] > ((SQLArray&)ct).getArraySize())
                {
                  *CmpCommon::diags() 
                    << DgSqlCode(-3242)
                    << DgString0(NAString("Specified array index(")
                                 + str_itoa(indexes_[0], buf) 
      ...
      for (Int32 i = 1; i < names_.entries(); i++)
        {
          if (resultType_->getFSDatatype() != REC_ROW)
            {
              *CmpCommon::diags() << DgSqlCode(-3242) 
                                  << DgString0("Specified field " + currName + " must be of ROW type.");
              bindWA->setErrStatus();
              return NULL;
            }
                  ...
                  if (indexes_[i] > sa->getArraySize())
                    {
                      *CmpCommon::diags() 
                        << DgSqlCode(-3242)
                        << DgString0(NAString("Specified array index ")
                                     + str_itoa(indexes_[i], buf) 
                                     + " is greater than the array size "
                                     + str_itoa(sa->getArraySize(), buf2)
                                     + ".");
                      bindWA->setErrStatus();
                      return NULL;              
                    }
