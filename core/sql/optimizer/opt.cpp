...
double OptDefaults::computeRecommendedNumCPUs(double cpuResourceRequired)
{
  ...
  if ( !canUseAllClusterResources() )
    mDoPc = MAXOF(mDoPc, getTotalCoresInTenant());
...
OptDefaults::computeRecommendedNumCPUsForMemory(double memoryResourceRequired)
{
  ...
  if ( !canUseAllClusterResources() )
    memoryPerCPU = MINOF(memoryPerCPU, getTenantUnitMemoryPerCore());
...
void OptDefaults::computeNumESPsForQuery(const RequiredResources* requiredResources)
{
  ...
  if ( ((RequiredResources *)requiredResources)->getNumOfHiveTables() == 0 )
   ...
   numESPsPerQuery_ = 
         ceil(numESPsPerNode) * CmpCommon::context()->getNumOfSMPs(); 
}
...
RequiredResources * OptDefaults::estimateRequiredResources(RelExpr* rootExpr)
{
  ...
  if (CmpCommon::getDefault(ASG_FEATURE) == DF_ON)
  {
    ...
    if (!canUseAllClusterResources())
      tenantTotalCores_ = 
        availableNodes->getTenantSize() * tenantUnitCores_;
    else
      tenantTotalCores_ = 0;
...
//cout  << dDoP << "," << resourceDoP << "," << numCPUs << endl;
      ...
      if ( requiredResources && 
           requiredResources->getNumOfHiveTables() > 0 ) {

        CollIndex numOfNodes = totalNumberOfNodes_;
        double minESPsPerNode = 0;
        minESPsPerNode = 
                getDefaultAsDouble(HIVE_MIN_NUM_ESPS_PER_DATANODE);
...
//cout << "maxDoP=" << maxDoP << endl;
      ...
      if ( optForCluster ) 
         ...
         maximumDegreeOfParallelism_ = 
            closestFactor(maxDoP, numCPUs, totalNumberOfNodes_);
...
void QueryOptimizerDriver::optimizeAPass(Context *context)
{
  try{
    ...
    if (!hasEnoughMemory)
    {
      // Warning: insufficient memory for pass 2.
      if(CmpCommon::getDefault(SHOWWARN_OPT) == DF_ON &&
         !CmpCommon::diags()->containsWarning(6020))
        *CmpCommon::diags() << DgSqlCode(6020);
    }
...
NABoolean OptDefaults::isAdaptiveSegForHadoop() const
{
  ...
  return defs_->getAsLong(AS_AFFINITY_VALUE) != -2 &&
    availableNodes &&
    availableNodes->castToNAASNodes();
}
...
NABoolean OptDefaults::canUseAllClusterResources() const
{
  ...
  return
    availableNodes == NULL ||
    availableNodes->canUseAllAvailableUnits(CURRCONTEXT_CLUSTERINFO);
}
