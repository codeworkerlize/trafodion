...
void PhysicalProperty::print(FILE *ofd, const char * /* prefix */, const char * /* suffix */) const {
  /*
...
void PhysicalProperty::display() { print(); }
...
NABoolean PhysicalProperty::isPartKeyPrefixOfSortKey() const {
  ...
  if (partFunc == NULL) return TRUE;

  // The test makes no sense unless the partitioning function is a range
  // partitioning function.
  if (NOT partFunc->isARangePartitioningFunction()) return FALSE;
...
PhysicalProperty::compareProperties(const PhysicalProperty &other) const {
  ...
  if (explodedOcbJoin_ != other.explodedOcbJoin_) return INCOMPATIBLE;
  ...
  // ---------------------------------------------------------------------
  if ((dp2SortOrderPartFunc_ != NULL) AND(other.dp2SortOrderPartFunc_ != NULL))
    result =
        combine_compare_results(result, dp2SortOrderPartFunc_->comparePartFuncToFunc(*(other.dp2SortOrderPartFunc_)));
  ...
  if (location_ != other.location_) {
    ...
    if (location_ == EXECUTE_IN_DP2 OR other.location_ == EXECUTE_IN_DP2) return INCOMPATIBLE;

    // actual properties should never contain wildcards
    CMPASSERT(location_ != EXECUTE_IN_MASTER_OR_ESP AND other.location_ != EXECUTE_IN_MASTER_OR_ESP);
  ...
  if (isPartitioned() AND other.isPartitioned())
    result = combine_compare_results(
        result, getPartitioningFunction()->comparePartFuncToFunc(*(other.getPartitioningFunction())));
...
void PhysicalProperty::enforceCoverageByGroupAttributes(const GroupAttributes *groupAttr) {
...
NABoolean PerformanceGoal::isOptimizeForResourceConsumption() const { return FALSE; }
...
NABoolean OptimizeForResourceConsumption::isOptimizeForResourceConsumption() const { return TRUE; }
...
void ReqdPhysicalProperty::print(FILE *ofd, const char * /* prefix */, const char * /* suffix */) const {
...
// ---------------------------------------------------------------------
NABoolean ReqdPhysicalProperty::satisfied(EstLogPropSharedPtr inputLogProp, const RelExpr *const physExpr,
  ...
  // ---------------------------------------------------------------------
  if (arrangedBy_ != NULL AND NOT physProp->getSortKey().satisfiesReqdArrangement(*arrangedBy_, ga)) return FALSE;

  // ---------------------------------------------------------------------
  // check requirements for ordering
  // ---------------------------------------------------------------------
  if (orderedBy_ != NULL AND(physProp->getSortKey().satisfiesReqdOrder(*orderedBy_, ga) != SAME_ORDER)) return FALSE;

  // ---------------------------------------------------------------------
  // Check Sort Order Type Requirement
  // ---------------------------------------------------------------------
  if (NOT sortOrderTypeReqAndSynthCompatible(physProp->getSortOrderType())) return FALSE;

  // ---------------------------------------------------------------------
  // Check any DP2 Sort Order Partitioning Requirement
  // ---------------------------------------------------------------------
  if (dp2SortOrderPartReq_ != NULL) {
    if (NOT dp2SortOrderPartReqAndSynthCompatible(physProp)) return FALSE;
  }
  ...
  if (location_ !=
      physProp->getPlanExecutionLocation() AND NOT(
          (location_ == EXECUTE_IN_MASTER_OR_ESP AND NOT physProp->executeInDP2())
              OR(location_ != EXECUTE_IN_DP2 AND physProp->getPlanExecutionLocation() == EXECUTE_IN_MASTER_AND_ESP))) {
  ...
  if (requiresPartitioning()) {
    if (NOT getPartitioningRequirement()->partReqAndFuncCompatible(physProp->getPartitioningFunction())) return FALSE;
  ...
  if (pdr && pdr->isEmpty() == FALSE) {
    ...
    if (pdp == NULL OR pdp->isEmpty() == TRUE OR NOT(pdr->satisfied(*pdp))) return FALSE;
  }

  // ---------------------------------------------------------------------
  // check logical partitioning requirement
  // ---------------------------------------------------------------------
  if (logicalPartReq_ AND NOT logicalPartReq_->satisfied(physExpr, physProp)) return FALSE;
  ...
  const LogPhysPartitioningFunction *logPhysPartFunc =
      physProp->getPartitioningFunction()->castToLogPhysPartitioningFunction();
  ...
  if ((logPhysPartFunc != NULL) AND(logPhysPartFunc->getPhysPartitioningFunction()->shouldUseSynchronousAccess(
                                        this, inputLogProp, ga) != logPhysPartFunc->getSynchronousAccess()))
    return FALSE;
  ...
  // ---------------------------------------------------------------------
  if (mustMatch_ != NULL AND NOT physExpr->patternMatch(*mustMatch_) AND physExpr->getOperatorType() !=
      REL_MAP_VALUEIDS AND physExpr->getOperatorType() != REL_PACK AND physExpr->getOperatorType() !=
      REL_UNPACKROWS AND physExpr->getOperatorType() !=
      REL_FIRST_N AND(NOT CURRSTMT_OPTDEFAULTS->ignoreExchangesInCQS() OR physExpr->getOperatorType() != REL_EXCHANGE)
          AND(NOT CURRSTMT_OPTDEFAULTS->ignoreSortsInCQS() OR physExpr->getOperatorType() != REL_SORT))
    return FALSE;
...
ReqdPhysicalProperty::compareRequirements(const ReqdPhysicalProperty &other) const {
  ...
  if ((perfGoal_ != NULL) AND((perfGoal_->isOptimizeForLastRow() AND(NOT other.perfGoal_->isOptimizeForLastRow()))OR(
          perfGoal_->isOptimizeForFirstRow() AND(NOT other.perfGoal_->isOptimizeForFirstRow()))
                                  OR(perfGoal_->isOptimizeForResourceConsumption()
                                         AND(NOT other.perfGoal_->isOptimizeForResourceConsumption()))))
    return UNDEFINED;
  ...
  // solutions
  if (costWeight_ != other.costWeight_) result = combine_compare_results(result, UNDEFINED);
  ...
  if (orderedBy_ != NULL OR other.orderedBy_ != NULL) {
    ...
    if (mySortEntries > otherSortEntries) result = combine_compare_results(result, MORE);
  ...
  // ---------------------------------------------------------------------
  result = combine_compare_results(result, compareSortOrderTypeReqToReq(other.sortOrderTypeReq_));

  // ---------------------------------------------------------------------
  // Compare the DP2 Sort Order Partitioning Requirements
  // ---------------------------------------------------------------------
  if ((dp2SortOrderPartReq_ != NULL) AND(other.dp2SortOrderPartReq_ != NULL))
    result = combine_compare_results(result, dp2SortOrderPartReq_->comparePartReqToReq(other.dp2SortOrderPartReq_));
  ...
  if (logicalOrderOrArrangement_ != other.logicalOrderOrArrangement_) {
    ...
    if ((logicalOrderOrArrangement_ == TRUE) AND(other.logicalOrderOrArrangement_ == FALSE)) {
      result = combine_compare_results(result, MORE);
    } else {
  ...
  if (getNoEspExchangeRequirement() && !other.getNoEspExchangeRequirement())
    result = combine_compare_results(result, INCOMPATIBLE);
  else if (!getNoEspExchangeRequirement() && other.getNoEspExchangeRequirement())
    result = combine_compare_results(result, INCOMPATIBLE);
  ...
  if (requiresPartitioning() AND other.requiresPartitioning())
    result = combine_compare_results(
        result, getPartitioningRequirement()->comparePartReqToReq(other.getPartitioningRequirement()));
  // I dominate
  ...
  if (logicalPartReq_ AND other.logicalPartReq_) {
    result = combine_compare_results(result, logicalPartReq_->compareLogPartRequirements(*other.logicalPartReq_));
  ...
  // ---------------------------------------------------------------------
  if (mustMatch_ != other.mustMatch_) result = combine_compare_results(result, UNDEFINED);
  ...
  if (pushDownRequirement_ == other.pushDownRequirement_) {
    ...
    result = combine_compare_results(result, pushDownRequirement_->compare(*other.pushDownRequirement_));
  }
...
NABoolean PushDownColocationRequirement::satisfied(const PushDownProperty &other) const {
  const PushDownColocationProperty *x = other.castToPushDownColocationProperty();
  ...
  if (thisNodeMap == NULL || otherNodeMap == NULL) return FALSE;

  if (thisNodeMap == otherNodeMap) return TRUE;

  return thisNodeMap->isCoLocated(otherNodeMap);
}

NABoolean PushDownCSRequirement::satisfied(const PushDownProperty &other) const {
  ...
  if (PushDownColocationRequirement::satisfied(*x) == FALSE) return FALSE;
  ...
  if (partFunc1->partFuncAndFuncPushDownCompatible(*partFunc2) == FALSE) return FALSE;
  ...
  if (partFunc1->checkSamePartitionNeeded() == TRUE) {
    ...
    // values).
    if (skey1 == NULL OR skey2 == NULL OR skey1->areKeyValuesIdentical(skey2, n1) == FALSE) return FALSE;
  }
...
const PushDownRequirement *PushDownColocationProperty::makeRequirement() const {
  return new (CmpCommon::statementHeap()) PushDownColocationRequirement(map_);
}

const PushDownRequirement *PushDownCSProperty::makeRequirement() const {
  return new (CmpCommon::statementHeap()) PushDownCSRequirement(partFunc_, searchKey_);
}
...
void ReqdPhysicalProperty::initSortOrderTypeCompTab() {
  if (sortOrderTypeCompTab_ != NULL) return;

  sortOrderTypeCompTab_ =
      new (GetCliGlobals()->exCollHeap()) LookupTable<NABoolean>(4, 6, GetCliGlobals()->exCollHeap());
...
THREAD_P LookupTable<COMPARE_RESULT> *ReqdPhysicalProperty::sortOrderTypeContextCompTab_ = NULL;
...
void ReqdPhysicalProperty::initSortOrderTypeContextCompTab() {
  if (sortOrderTypeContextCompTab_ != NULL) return;

  sortOrderTypeContextCompTab_ =
      new (GetCliGlobals()->exCollHeap()) LookupTable<COMPARE_RESULT>(6, 6, GetCliGlobals()->exCollHeap());
...
NABoolean ReqdPhysicalProperty::dp2SortOrderPartReqAndSynthCompatible(const PhysicalProperty *const spp) const {
  ...
  DCMPASSERT((sortOrderTypeReq_ == DP2_SOT) OR(sortOrderTypeReq_ == DP2_OR_ESP_NO_SORT_SOT)
                 OR(location_ == EXECUTE_IN_DP2));
  ...
  if ((spp->getSortOrderType() == DP2_SOT) OR(location_ == EXECUTE_IN_DP2)) {
    CMPASSERT(spp->getDp2SortOrderPartFunc() != NULL);
    if (NOT dp2SortOrderPartReq_->partReqAndFuncCompatible(spp->getDp2SortOrderPartFunc())) return FALSE;
  }
...
COMPARE_RESULT InputPhysicalProperty::compareInputPhysicalProperties(const InputPhysicalProperty &other) const {
  // Compare Outer Characteristic Outputs
  if (njOuterCharOutputs_ != other.njOuterCharOutputs_) return INCOMPATIBLE;
  ...
  if ((njOuterOrder_ != NULL) OR(other.njOuterOrder_ != NULL)) {
    ...
    if ((njOuterOrder_ == NULL) OR(other.njOuterOrder_ == NULL)) return INCOMPATIBLE;

    // Compare the non-null outer order lists
    // Return INCOMPATIBLE if they are not the same object and
    // are not equivalent.
    if ((njOuterOrder_ != other.njOuterOrder_) AND(NOT(*njOuterOrder_ == *(other.njOuterOrder_)))) return INCOMPATIBLE;
  }
  ...
  if ((njOuterOrderPartFunc_ != NULL) OR(other.njOuterOrderPartFunc_ != NULL)) {
    // Is one null, but not the other?
    if ((njOuterOrderPartFunc_ == NULL) OR(other.njOuterOrderPartFunc_ == NULL)) return INCOMPATIBLE;

    // Compare the non-null outer order partitioning functions
    // Return INCOMPATIBLE if they are not the same object and
    // are not equivalent.
    if ((njOuterOrderPartFunc_ != other.njOuterOrderPartFunc_)
            AND(NOT njOuterOrderPartFunc_->comparePartFuncToFunc(*(other.njOuterOrderPartFunc_))))
      return INCOMPATIBLE;
  }

  // Compare DP2 Outer Order Partitioning Function
  // Only check if both DP2 outer order partitioning functions are not null
  if ((njDp2OuterOrderPartFunc_ != NULL) OR(other.njDp2OuterOrderPartFunc_ != NULL)) {
    // Is one null, but not the other?
    if ((njDp2OuterOrderPartFunc_ == NULL) OR(other.njDp2OuterOrderPartFunc_ == NULL)) return INCOMPATIBLE;

    // Compare the non-null DP2 outer order partitioning functions
    // Return INCOMPATIBLE if they are not the same object and
    // are not equivalent.
    if ((njDp2OuterOrderPartFunc_ != other.njDp2OuterOrderPartFunc_)
            AND(NOT njDp2OuterOrderPartFunc_->comparePartFuncToFunc(*(other.njDp2OuterOrderPartFunc_))))
      return INCOMPATIBLE;
  }

  if (assumeSortedForCosting_ != other.assumeSortedForCosting_) return INCOMPATIBLE;
  if (explodedOcbJoinForCosting_ != other.explodedOcbJoinForCosting_) return INCOMPATIBLE;
...
PushDownColocationRequirement::compare(const PushDownRequirement &other) const {
  const PushDownColocationRequirement *otherP = other.castToPushDownColocationRequirement();
  if (otherP == NULL) return INCOMPATIBLE;

  // Passed argument could be PushDownCSRequirement object,
  // make sure it is not (i.e otherCS must be NULL).
  const PushDownCSRequirement *otherCS = other.castToPushDownCSRequirement();
  if (otherCS != NULL) return INCOMPATIBLE;
...
PushDownCSRequirement::compare(const PushDownRequirement &other) const {
  COMPARE_RESULT result = PushDownColocationRequirement::compare(other);

  const PushDownCSRequirement *otherP = other.castToPushDownCSRequirement();
  if (otherP == NULL) return INCOMPATIBLE;
  ...
  if (partFunc_ == otherReq.partFunc_) {
    ...
    result = combine_compare_results(result, partFunc_->comparePartFuncToFunc(*otherReq.partFunc_));
  ...
  if (searchKey_ == otherReq.searchKey_) {
    ...
    if (NOT searchKey_->areKeyValuesIdentical(otherReq.searchKey_, numValues)) return INCOMPATIBLE;
  }

  return result;
}

void ReqdPhysicalProperty::display() const { print(); }

PlanExecutionEnum ReqdPhysicalProperty::translatePlanExecutionLocation() const {
  if (getPlanExecutionLocation() == EXECUTE_IN_MASTER_OR_ESP)
    return EXECUTE_IN_MASTER_AND_ESP;
  else
    return getPlanExecutionLocation();
}
