...
static ItemExpr *processMultipleConstValuesInVEG(const ValueIdSet &vegSet) {
  ...
  if (constSet.entries() > 1) {
    ...
    for (exprId = constSet.init(); constSet.next(exprId); constSet.advance(exprId)) {
      if (firstTime) {
        ...
        ItemExpr *eqPred = new (CmpCommon::statementHeap()) BiRelat(ITM_EQUAL, leftEntry, exprId.getItemExpr());
        if (rootPtr == NULL) {
          rootPtr = eqPred;
        } else {
          rootPtr = new (CmpCommon::statementHeap()) BiLogic(ITM_AND, rootPtr, eqPred);
        }
      }  // not firstTime
    }    // for
...
NABoolean VEGMember::operator==(const VEGMember &other) const {
  return ((memberM_ == ((const VEGMember &)other).memberM_) AND(groupG_ == ((const VEGMember &)other).groupG_));
}  // VEGMember::operator==()
...
void VEGMember::print(FILE *ofd, const char *indent, const char *title) {
  ...
  fprintf(ofd, "Member (0x%x) VEG (0x%x) VEGRef (0x%x) VEGPred (0x%x)  {%s}  {%s}\n", CollIndex(memberM_),
          CollIndex(groupG_), CollIndex(getVEG()->getVEGReference()->getVEG()->getValueId()),
          CollIndex(getVEG()->getVEGPredicate()->getVEG()->getValueId()), memberM_.getItemExpr()->getText().data(),
          groupG_.getItemExpr()->getText().data());

}  // VEGMember::print()

// To be called from the debugger
void VEGMember::display() { VEGMember::print(); }  // VEGMember::display()
...
VEGMember *VEGRegion::findVEGMember(VEGReference *vegReference) {
  ...
  if (found) return members_[index];
  return NULL;
}
...
VEGMember *VEGRegion::findVEGMember(const ValueId &vid) const {
  ...
  while ((i < ne) AND(NOT found)) {
    ...
    if (memberValueId == vid)
      ...
      ((VEGReference *)(memberValueId.getItemExpr()))->getVEG()->getAndExpandAllValues(expandedValues);
...
void VEGRegion::addVEGMember(const ValueId &memberId) {
  ...
  } else  // no existing VEGMember with memberId
  {
    ...
    members_.insert(new (CmpCommon::statementHeap()) VEGMember(memberId, vegId));
...
void VEGRegion::addVEG(const ValueId &expr1Id, const ValueId &expr2Id) {
  ...
  if (vegDesc1 == NULL) members_.insert(new (CmpCommon::statementHeap()) VEGMember(expr1Id, vegId));
...
void VEGRegion::addVEG(const ValueIdSet &vegMembers) {
  ...
  // ---------------------------------------------------------------------
  for (otherMemberId = vegMembers.init(); vegMembers.next(otherMemberId); vegMembers.advance(otherMemberId))
...
void VEGRegion::mergeForwardingEntries(const VEGRegion *fromRegion) {
  ...
  for (CollIndex i = 0; i < allForwardingEntries.entries(); i++) {
    ...
    members_.insert(new (CmpCommon::statementHeap()) VEGMember(vegMember->getMemberValueId(), newVEGId, FALSE, TRUE));
  }
}
...
// -----------------------------------------------------------------------
VEGRegion *VEGRegion::allocateNewZone(const VEGRegionTypeEnum tev, const ExprNode *const ownerExpr) {
...
// -----------------------------------------------------------------------
void VEGRegion::replaceVEGMember(const ValueId &existingMemberId, const ValueId &newMemberId) {
  ValueId exprId;
  ...
  if (newMemberId != NULL_VALUE_ID AND compInt54 >= 0) {
    ...
    if (newMemberId.getItemExpr()->getOperatorType() == ITM_VEG_REFERENCE) {
      ...
      if (oldMemVegRefPtr != NULL) {
        ...
        if (((VEGReference *)(newMemberId.getItemExpr()))->referencesVegRefValue(ofOldVegRef)) {
          if (compInt54 == 0)
            return;
          else
...
void VEGRegion::replaceAllVEGs(ValueIdSet &vidSet, ValueId vid) {
  ...
  for (ValueId memberId = vidSet.init(); vidSet.next(memberId); vidSet.advance(memberId)) {
...
// -----------------------------------------------------------------------
VEGReference *VEGRegion::getVEGReferenceFromCurrentVEGRegion(const ValueId &exprId,
  ...
  if (memberPtr == NULL && searchThisVegRegionFirst && searchThisVegRegionFirst->exportsVEG() &&
    ...
    if (currRegion) memberPtr = currRegion->findVEGMember(exprId);
  }
  ...
  while ((memberPtr == NULL) AND(count > 0)) {
    ...
    // is a member.
    if ((currRegion != ignoreThisChildVEGRegion) AND(currRegion != searchThisVegRegionFirst)
            AND(currRegion->exportsVEG()))
...
// -----------------------------------------------------------------------
VEGReference *VEGRegion::getVEGReferenceFromParentVEGRegion(const ValueId &exprId) const {
  VEGReference *vegRefPtr = NULL;
  ...
  while ((parentRegion != NULL) AND(vegRefPtr == NULL)) {
    ...
    // excepting myself.
    vegRefPtr = parentRegion->getVEGReferenceFromCurrentVEGRegion(exprId, this);
    if (vegRefPtr == NULL) parentRegion = parentRegion->getParentVEGRegion();
...
void VEGRegion::importVEGsForUnionChildVEGRegion() {
  ...
  if ((ownerUnion == NULL) OR(ownerUnion->getOperatorType() != REL_UNION)) return;
  ...
  while (parentRegion) {
    ...
    while (iterations < parentZoneCountPlusOne) {
      ...
      // Union parent.
      // -------------------------------------------------------------
      if ((parentZone == parentRegion) OR(parentZone->getOwnerExpr()->getOperatorType() == ITM_EQUAL)) {
        // ---------------------------------------------------------
        ...
        // ---------------------------------------------------------
        for (ValueId memberId = outputValues.init(); outputValues.next(memberId); outputValues.advance(memberId)) {
          // -----------------------------------------------------
          ...
          if (memberPtr) {
            ...
            if (vegMembers.isEmpty()) {
              ...
              if (parentZone != parentRegion) {
                ...
                ownerUnion->rewriteUnionExpr(vegMembers, valuesForLeftChild, valuesForRightChild);
                ValueId exprId;
                if (leftChild)
                  valuesForLeftChild.getFirst(exprId);
                else
                  valuesForRightChild.getFirst(exprId);
                unionChildRegion = allocateNewZone(parentZone->getVEGRegionTypeEnum(), exprId.getItemExpr());
              }
              ...
              valuesForLeftChild.clear();
              valuesForRightChild.clear();
              ownerUnion->rewriteUnionExpr(((VEG *)memberPtr->getVEG())->getAllValues(), valuesForLeftChild,
                                           valuesForRightChild);
      ...
      // -----------------------------------------------------------------
      if (iterations < parentZoneCount) parentZone = getParentVEGRegion()->zones_[iterations];
      iterations++;
...
void VEGRegion::replaceInstantiateNullMembers() {
  ...
  //**************************************************************
  for (index = 0; (UInt32)index < zones_.entries(); index++) zones_[index]->replaceInstantiateNullMembers();
  ...
  for (index = 0; index < ne; index++) {
    ...
    // member ...
    // -----------------------------------------------------------------
    if ((NOT memberPtr->isAForwardingEntry())AND(memberPtr->getItemExpr()->getOperatorType() == ITM_INSTANTIATE_NULL)) {
      instNullPtr = (InstantiateNull *)memberPtr->getItemExpr();

      if (instNullPtr->lojTransformInProgress()) instNullPtr->setLOJTransformComplete();
  ...
  for (index = 0; index < ne; index++) {
    ...
    members_.insert(new (CmpCommon::statementHeap()) VEGMember(instNullPtr->getValueId(), vegId, FALSE, TRUE));
  }
...
void VEGRegion::replaceOuterReferences(const ValueIdSet &outerReferences) {
  ...
  // ---------------------------------------------------------------------
  for (ValueId memberId = outerReferences.init(); outerReferences.next(memberId); outerReferences.advance(memberId)) {
...
void VEGRegion::mergeVEGRegion(VEGRegion *fromRegion) {
  ...
  // ---------------------------------------------------------------------
  for (vegId = setOfFromRegionVEGs.init(); setOfFromRegionVEGs.next(vegId); setOfFromRegionVEGs.advance(vegId)) {
    // find all the values for each VEG.
    ValueIdSet allValuesInVEG = ((VEG *)(vegId.getItemExpr()))->getAllValues();
    ValueId memberId, exprId;
    for (memberId = allValuesInVEG.init(); allValuesInVEG.next(memberId); allValuesInVEG.advance(memberId)) {
...
// -----------------------------------------------------------------------
void VEGRegion::mergeZonesFromSameVEGRegion(VEGRegion *fromRegion, VEGRegion *parentRegion)
...
void VEGRegion::processZones() {
  ...
  // is to be deleted.
  for (index = 0; index < ne; index++)
    if (members_[index]->isToBeDeleted()) deleteStack.insert(members_[index]);

  // Delete all the unwanted members.
  if (deleteStack.entries() > 0) {
    ne = deleteStack.entries();
    for (index = 0; index < ne; index++) deleteVEGMember(deleteStack[index]->getMemberValueId());
  }

  // Process all the descendant VEGRegions
  ne = zones_.entries();
  for (index = 0; index < ne; index++) zones_[index]->processZones();
...
// -----------------------------------------------------------------------
void VEGRegion::fixupActiveZones(VEGRegion *activeParentRegion, const ValueIdSet &outerReferences) {
  ...
  // -----------------------------------------------------------------
  for (index = 0; index < ne; index++) {
    const RelExpr *fullJoinExpr = zones_[index]->getOwnerExpr()->castToRelExpr();
    ...
    if (fullJoinExpr->getOperatorType() == REL_FULL_JOIN) {
      ...
      {
        const RelExpr *fullJoinExpr1 = zones_[index1]->getOwnerExpr()->castToRelExpr();
  ...
  if (NOT setOfMembers.isEmpty()) replaceOuterReferences(setOfMembers);
  ...
  for (ValueId exprId = setOfMembers.init(); setOfMembers.next(exprId); setOfMembers.advance(exprId)) {
  ...
  // ---------------------------------------------------------------------
  for (index = 0; index < ne; index++) zones_[index]->markAsProcessed();
...
void VEGRegion::fixupZonesAndParentPointers() {
  ...
  for (index = FIRST_VEG_REGION; index < (RegionId)(vegTable_->numberOfRegions()); index++) {
...
void VEGRegion::print(FILE *ofd, const char *indent, const char *title) {
  ...
  if (getOwnerExpr()) fprintf(ofd, "is a (%s) ", getOwnerExpr()->getText().data());
...
// To be called from the debugger
void VEGRegion::display() { VEGRegion::print(); }  // VEGRegion::display()
...
// -----------------------------------------------------------------------
VEGTable::VEGTable()
    : nextInSequence_(FIRST_VEG_REGION), currentRegion_(NULL), arrayEntry_(CmpCommon::statementHeap()) {}
...
VEGRegion *VEGTable::allocateVEGRegion(VEGRegion *parentRegion, const VEGRegionTypeEnum tev,
  ...
  if (getCurrentVEGRegion()) getCurrentVEGRegion()->addZone(getCurrentVEGRegion(), newRegion);
  return newRegion;
}  // VEGTable::allocateRegion()
...
VEGRegion *VEGTable::getVEGRegion(const RegionId candidateRegion) const {
  CMPASSERT(candidateRegion >= FIRST_VEG_REGION && candidateRegion < (RegionId)arrayEntry_.entries());
...
VEGRegion *VEGTable::getVEGRegion(const ValueId exprId) const {
  ...
  for (RegionId i = FIRST_VEG_REGION; i < (RegionId)arrayEntry_.entries(); i++) {
    candidateRegion = arrayEntry_[i];
    memberPtr = candidateRegion->findVEGMember(exprId);
    if (memberPtr) return candidateRegion;
  }
...
VEGRegion *VEGTable::getVEGRegion(const ExprNode *const ownerExpr, int subtreeId) const {
  ...
  while (ownerOrOrig) {
    ...
    for (RegionId i = FIRST_VEG_REGION; i < (RegionId)arrayEntry_.entries(); i++) {
      candidateRegion = arrayEntry_[i];
      if ((candidateRegion->getOwnerExpr() == ownerOrOrig) AND(candidateRegion->getSubtreeId() == subtreeId))
...
// -----------------------------------------------------------------------
ItemExpr *VEGTable::getVEGReference(const ValueId &exprId, const VEGRegion *searchThisVegRegionFirst) const {
  ...
  VEGReference *vegRefPtr =
      getCurrentVEGRegion()->getVEGReferenceFromCurrentVEGRegion(exprId, NULL, searchThisVegRegionFirst);
  ...
  // ---------------------------------------------------------------------
  if ((vegRefPtr == NULL) AND(getCurrentVEGRegion()->getParentVEGRegion()))  // search in parent VEGRegions.
  {
    vegRefPtr = getCurrentVEGRegion()->getVEGReferenceFromParentVEGRegion(exprId);
    ...
    // -----------------------------------------------------------------
    if (vegRefPtr) {
      VEGReference *vegRef2Ptr = getCurrentVEGRegion()->getVEGReferenceFromCurrentVEGRegion(vegRefPtr->getValueId());
      // -------------------------------------------------------------
      ...
      // -------------------------------------------------------------
      if (vegRef2Ptr) vegRefPtr = vegRef2Ptr;
    }
  }  // endid
...
void VEGTable::addVEGInOuterRegion(const ValueId &expr1Id, const ValueId &expr2Id) {
  ...
  if (regPtr == NULL) regPtr = getCurrentVEGRegion();
...
VEGRegion *VEGTable::locateVEGRegionAndMarkToBeMerged(const ValueId &exprId) {
  ...
  for (RegionId index = FIRST_VEG_REGION; index < (int)arrayEntry_.entries(); index++) {
    ...
    if (owner AND owner->getGroupAttr()->getCharacteristicOutputs().contains(exprId)) {
...
void VEGTable::processVEGRegions() {
  ...
  // ---------------------------------------------------------------------
  for (index = FIRST_VEG_REGION; index < (RegionId)arrayEntry_.entries(); index++)
...
// To be called from the debugger
void VEGTable::display() { VEGTable::print(); }  // VEGTable::display()
