...
CollHeap *NormWA::wHeap() { return (currentCmpContext_) ? (currentCmpContext_->statementHeap()) : 0; }
...
void NormWA::locateVEGRegionAndMarkToBeMergedRecursively(const ValueId &vid) {
  ...
  if (toBeMergedRegion) {
    ...
    for (ValueId exprId = nullInstValues.init(); nullInstValues.next(exprId); nullInstValues.advance(exprId)) {
      locateVEGRegionAndMarkToBeMergedRecursively(exprId);
    }
  }
}

NABoolean NormWA::findEquivalentInSeqFunctionsCache(ItemExpr *newItem, ValueId &cacheEquivTransSeqId) {
...
ValueId NormWA::getEquivalentItmSequenceFunction(ValueId newSeqId) {
  ...
  if (newSeq) {
    for (ValueId seqId = allSeqFunctions_.init(); allSeqFunctions_.next(seqId); allSeqFunctions_.advance(seqId)) {
  ...
  if (subqId_ == subqId) {
    switch (whatChanged_) {
      case SQO_REASSIGNED_VREGION:
        normWARef.reassignVEGRegion(changedRelExpr_, changedChildId_, originalRelExpr_, origChildId_);
        performedUndo = TRUE;
        break;
    }
  }
...
void SqoWA::undoChanges(NormWA &normWARef) {
  ...
  for (UInt32 itemIdx = 0; itemIdx < changedItemExprs_.entries(); itemIdx++) {
    if (changedItemExprs_[itemIdx]->undoChanges(normWARef, subqId_)) undoneItemExprs.insert(changedItemExprs_[itemIdx]);
  }
  for (UInt32 relIdx = 0; relIdx < changedRelExprs_.entries(); relIdx++) {
    if (changedRelExprs_[relIdx]->undoChanges(normWARef, subqId_)) undoneRelExprs.insert(changedRelExprs_[relIdx]);
  }

  // Remove the things we undid.
  for (UInt32 idx = 0; idx < undoneItemExprs.entries(); idx++) changedItemExprs_.remove(undoneItemExprs[idx]);
  for (UInt32 idx = 0; idx < undoneRelExprs.entries(); idx++) changedRelExprs_.remove(undoneRelExprs[idx]);
}
