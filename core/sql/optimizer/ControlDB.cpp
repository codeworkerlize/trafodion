...
ControlDB *ActiveControlDB() {
  if (!CmpCommon::context()->controlDB_) CmpCommon::context()->initControlDB();
...
ControlDB::ControlDB() : cqdList_(CONTROLDBHEAP), csList_(CONTROLDBHEAP) {
  requiredShape_ = NULL;
...
void ControlDB::setRequiredShape(ControlQueryShape *shape) {
  ...
  requiredShape_ = (ControlQueryShape *)shape->copyRelExprTree(CONTROLDBHEAP);
  requiredShape_->setShape(shape->getShape()->copyRelExprTree(CONTROLDBHEAP));
...
void ControlDB::setHistogramCacheState() {
  ...
  if (CmpCommon::getDefault(CACHE_HISTOGRAMS) == DF_ON)
    size = (ActiveSchemaDB()->getDefaults().getAsLong(CACHE_HISTOGRAMS_IN_KB) * 1024);
...
void ControlDB::setControlDefault(ControlQueryDefault *def) {
  ...
  if (def->getToken() == "") {
    ...
    for (CollIndex i = 0; i < cqdList_.entries(); i++) {
      ...
      const char *name = ActiveSchemaDB()->getDefaults().lookupAttrName(cqd->getAttrEnum());
      NABoolean isResetable = (NOT ActiveSchemaDB()->getDefaults().isNonResetableAttribute(name));

      if (cqd->getAttrEnum() == ROBUST_QUERY_OPTIMIZATION) resetRobustQueryOptimizationCQDs();

      if (cqd->getAttrEnum() == CACHE_HISTOGRAMS || cqd->getAttrEnum() == CACHE_HISTOGRAMS_IN_KB) {
  ...
  if (def->getAttrEnum() == SCHEMA) {
    ...
    if (!nam.getCatalogNamePart().isEmpty()) removeCat = TRUE;
  }

  // Remove attr in list if it matches me
  // (also remove CATALOG attr if I am SCHEMA)
  for (CollIndex i = 0; i < cqdList_.entries(); i++)
    if ((cqdList_[i]->getAttrEnum() == def->getAttrEnum()) || (removeCat && cqdList_[i]->getAttrEnum() == CATALOG)) {
      ControlQueryDefault *cqd = cqdList_.at(i);
      delete cqd;
      cqdList_.removeAt(i);
      if (!removeCat) break;
  ...
  if (def->reset()) {
    switch (def->getAttrEnum()) {
      ...
      case CACHE_HISTOGRAMS_TRACE_OUTPUT_FILE: {
        const char *fname = ActiveSchemaDB()->getDefaults().getValue(CACHE_HISTOGRAMS_TRACE_OUTPUT_FILE);
        ...
        if (fname) {
          ...
          if (fd) fprintf(fd, "query:CQD CACHE_HISTOGRAMS_TRACE_OUTPUT_FILE \'%s\';\n", fname);
        }
        break;
      }
      case CACHE_HISTOGRAMS_MONITOR_OUTPUT_FILE: {
        const char *fname = ActiveSchemaDB()->getDefaults().getValue(CACHE_HISTOGRAMS_MONITOR_OUTPUT_FILE);
        fname = (fname && stricmp(fname, "") != 0) ? fname : NULL;
        ...
        if (fname) {
          ...
          if (fd) fprintf(fd, "query:CQD CACHE_HISTOGRAMS_MONITOR_OUTPUT_FILE \'%s\';\n", fname);
        }
        break;
      }
    ...
    {
      ControlQueryDefault *defCopy = (ControlQueryDefault *)def->copyTree(CONTROLDBHEAP);
  ...
  if (def->getAttrEnum() == NSK_DBG_COMPILE_INSTANCE) {
    const char *compileInstance = ActiveSchemaDB()->getDefaults().getValue(NSK_DBG_COMPILE_INSTANCE);

    CURRCONTEXT_OPTDEBUG->setCompileInstanceClass(compileInstance);
  }

  if (def->getAttrEnum() == MEMORY_MONITOR || def->getAttrEnum() == MEMORY_MONITOR_IN_DETAIL ||
      def->getAttrEnum() == MEMORY_MONITOR_LOGFILE || def->getAttrEnum() == MEMORY_MONITOR_LOG_INSTANTLY) {
    DefaultConstants attrEnum = def->getAttrEnum();
  ...
  switch (def->getAttrEnum()) {
    ...
    case QUERY_CACHE_AVERAGE_PLAN_SIZE:
      CURRENTQCACHE->setAvgPlanSz(getDefaultInK(QUERY_CACHE_AVERAGE_PLAN_SIZE));
      break;
    ...
    case DEFAULT_SCHEMA_ACCESS_ONLY:
      if ((CmpCommon::getDefault(DEFAULT_SCHEMA_ACCESS_ONLY) == DF_ON) &&
        ...
        ControlQueryDefault *cqdS =
            new (STMTHEAP) ControlQueryDefault(cqdSchema, CharInfo::UTF8, "SCHEMA", schName, FALSE, 0, STMTHEAP, 0);
...
void ControlDB::do_one_CQD(const char *attrName, const char *attrValue, NABoolean reset,
  ...
  if (cqdEnum < 0) return;  // cqd name is bad

  // was it explicitly set by some user action?
  NADefaults::Provenance cqdOrigin = defs.getProvenance(cqdEnum);
  if (cqdOrigin >= NADefaults::READ_FROM_SQL_TABLE && cqdOrigin >= masterOrigin) {  // yes, it was.
    return;                                                                         // so, leave it alone.
  }
  // no, it is not user-specified.
  // (re)set it accordingly
  // put together cqd statement
  NAString text("CONTROL QUERY DEFAULT ", 200, STMTHEAP), name(attrName, STMTHEAP), value(attrValue, STMTHEAP);
  text += attrName;
  if (reset) {
    text += " RESET;";
  } else {
  ...
  ControlQueryDefault *cqdP =
      new (STMTHEAP) ControlQueryDefault(text, CharInfo::UTF8, name, value, FALSE, 0, STMTHEAP, reset);
...
void ControlDB::do_one_RQO_CQD(const char *attrName, const char *attrValue, NABoolean reset) {
  do_one_CQD(attrName, attrValue, reset, ActiveSchemaDB()->getDefaults().getProvenance(ROBUST_QUERY_OPTIMIZATION));
}
...
void ControlDB::do_one_MVQR_CQD(const char *attrName, const char *attrValue, NABoolean reset) {
...
static const size_t numControlTableTokens = sizeof(controlTableTokens) / sizeof(ControlTableOptions::CTTokens);

ControlTableOptions *ControlDB::getControlTableOption(const NAString &tableName, CollIndex &i) {
...
CollIndex ControlDB::isValidToken(const NAString &token) {
  for (CollIndex index = 0; index < numControlTableTokens; index++) {
    if (token == controlTableTokens[index].token_) return index;
  }
...
NABoolean ControlDB::validate(ControlTable *ct) {
  ...
  if (valid) {
    ...
    switch (controlTableTokens[index].const_) {
      ...
      case ControlTableOptions::IF_LOCKED: {
        if ((value == "RETURN") || (value == "WAIT")) valid = TRUE;

        valid = FALSE;  // IF_LOCKED not supported
      } break;

      case ControlTableOptions::MDAM: {
        if ((value == "OFF") || (value == "ON") || (value == "ENABLE")) valid = TRUE;
      } break;

      case ControlTableOptions::NOWAIT: {
        if ((value == "ON") || (value == "OFF")) valid = TRUE;
      ...
      case ControlTableOptions::PRIORITY_DELTA: {
        ...
        for (size_t i = 0 + (negative ? 1 : 0); (i < value.length() && valid); i++) {
          if (value.data()[i] < '0' || value.data()[i] > '9') valid = FALSE;
        }
        if (valid) {
      ...
      case ControlTableOptions::TABLELOCK: {
        if ((value == "ON") || (value == "OFF") || (value == "ENABLE")) valid = TRUE;
  ...
  if (!valid) {
    *CmpCommon::diags() << DgSqlCode(-2051) << DgString0("CONTROL TABLE") << DgString1(token) << DgString2(value);
  }
...
const NAString *ControlDB::getControlTableValue(const NAString &tableName, const NAString &token) {
  ...
  if (controlValue == NULL) {
    ...
    if (controlValue == NULL)  // Still no match, Set System defaults.
    {
      ...
      if (dc >= 0)  // valid
      {
        ...
        if (!defVal.isNull()) controlValue = new STMTHEAP NAString(defVal, STMTHEAP);
      }
      // ct-bug-10-030102-3803 -Begin
    }
    // ct-bug-10-030102-3803 -End
...
ControlTableOptions::ControlTableOptions() : tableName_(CONTROLDBHEAP) {
  tokens_ = NULL;
  values_ = NULL;
}

ControlTableOptions::ControlTableOptions(const NAString &tableName) : tableName_(tableName, CONTROLDBHEAP) {
...
void ControlTableOptions::addTokenAndValue(const NAString &token, const NAString &value) {
  remove(token);
...
void ControlDB::resetSessionValue(const NAString &token) {
  ...
  if (token == "") {
    ...
    if (cso) REMOVEAT(csList_, index);
  }
}

ControlSessionOption *ControlDB::getControlSessionOption(const NAString &token, CollIndex &i) {
  // return the 'value' for 'token'.
  for (i = 0; i < csList_.entries(); i++) {
    ControlSessionOption *cso = csList_[i];
    if (cso->getToken() == token) return cso;
  }
...
NABoolean ControlDB::validate(ControlSession *cs) {
  ...
  if (value == "")  // value=="RESET" *is* a valid Session value
  {
    ...
    if (!isalpha(*str) && !strchr("_$=", *str)) valid = FALSE;
    while (*++str && valid) {
      if (!isalnum(*str) && !strchr("_$=.", *str)) valid = FALSE;
    }
  }
  ...
  if (!valid) {
    token.prepend("'");
    token.append("'");
    *CmpCommon::diags() << DgSqlCode(-2051) << DgString0("CONTROL SESSION") << DgString1(token) << DgString2(value);
  }
  ...
  if (getCTList().entries() > 0) {
    ...
    for (CollIndex i = 0; i < getCTList().entries(); i++) {
      ...
      for (CollIndex j = 0; j < cto->numEntries(); j++) {
        size += strlen(cto->getToken(j).data()) + 1 + strlen(cto->getValue(j).data()) + 1;
      }  // j
...
void ControlSessionOption::addTokenAndValue(const NAString &token, const NAString &value) {
  delete token_;
  delete value_;
...
static NABoolean badSingleArg(const NAString &fname, ExprNodePtrList *args, ComDiagsArea *diags) {
  if (args->entries() != 1 OR args->at(0)->castToRelExpr() == NULL) {
    *diags << DgSqlCode(-3113) << DgString0(NAString("One relational argument expected for ") + fname);
    return TRUE;
  }
  return FALSE;
}

static NABoolean badTwoArgs(const NAString &fname, ExprNodePtrList *args, ComDiagsArea *diags) {
  if (args->entries() != 2 OR args->at(0)->castToRelExpr() == NULL OR args->at(1)->castToRelExpr() == NULL) {
    *diags << DgSqlCode(-3113) << DgString0(NAString("Two relational arguments expected for ") + fname);
    return TRUE;
  }
  return FALSE;
}
...
ExprNode *DecodeShapeSyntax(const NAString &fname, ExprNodePtrList *args, ComDiagsArea *diags, CollHeap *heap) {
  ...
  enum { FORCED_EXCHANGE, FORCED_JOIN, FORCED_OTHER } forcedType = FORCED_OTHER;
  ...
  if (fname == "SCAN" OR fname == "FILE_SCAN" OR fname == "INDEX_SCAN") {
    ...
    for (i = 0; i < numArgs; i++) {
      ...
      if (itm && itm->castToConstValue(dummyNegate)) {
        ...
        if (nat->getTypeQualifier() != NA_CHARACTER_TYPE) exitFlag = TRUE;
      } else
        exitFlag = TRUE;
      if (exitFlag) break;
    }
    ...
    if (firstNonStringArg >= 3) {
      ...
      *diags << DgSqlCode(-3113) << DgString0("More than 2 string arguments in scan(...) statement.");
      return NULL;
    }
    ...
    if ((numArgs >= 1) && (firstNonStringArg >= 1)) {
      ...
      if (itm == NULL OR itm->castToConstValue(dummyNegate) == NULL) {
        *diags << DgSqlCode(-3113) << DgString0("Character constant for table name expected");
        return NULL;
      }
      tableName = NAString((char *)itm->castToConstValue(dummyNegate)->getConstValue(),
    ...
    if ((numArgs >= 2) && (firstNonStringArg >= 2)) {
      ...
      if (itm == NULL OR itm->castToConstValue(dummyNegate) == NULL) {
        *diags << DgSqlCode(-3113) << DgString0("Character constant for index name expected");
        return NULL;
      }
      indexName = NAString((char *)itm->castToConstValue(dummyNegate)->getConstValue(),
                           (size_t)itm->castToConstValue(dummyNegate)->getStorageSize());
      // sorry, no delimited identifiers
      indexName.toUpper();
    }
    ...
    for (i = firstNonStringArg; i < numArgs; i++) {
      if (args->at(i)->getOperatorType() != REL_FORCE_ANY_SCAN) {
        *diags << DgSqlCode(-3113) << DgString0("Illegal scan(...) argument.");
        return NULL;
      }
      ...
      if (!((ScanForceWildCard *)result)->mergeScanOptions(*tmpScan)) {
        ...
        *diags << DgSqlCode(-3113) << DgString0("Conflicting scan options.");
        return NULL;
      }
    }

    if (((ScanForceWildCard *)result)->doesThisCoflictMasterSwitch()) {
      *diags << DgSqlCode(-3113) << DgString0("MDAM options Conflicting With Master Switch.");
      return NULL;
    }
    ...
    for (int i = 0; i < numColumns; i++) {
      ...
      short arg = itm->castToConstValue(dummyNegate)->getExactNumericValue();
    ...
    ((ScanForceWildCard *)result)
        ->setColumnOptions(numColumns, columnAlgorithms, ScanForceWildCard::MDAM_COLUMNS_NO_MORE);
  }

  else if (fname == "JOIN") {
    forcedType = FORCED_JOIN;
    wildcard = REL_FORCE_JOIN;
  } else if (fname == "TSJ" OR fname == "NESTED_JOIN") {
    forcedType = FORCED_JOIN;
    wildcard = REL_FORCE_NESTED_JOIN;
  } else if (fname == "GROUPBY") {
    if (badSingleArg(fname, args, diags)) return NULL;
    result = new (heap) WildCardOp(REL_ANY_GROUP, 0, args->at(0)->castToRelExpr());
  } else if (fname == "UNION") {
    if (badTwoArgs(fname, args, diags)) return NULL;

    result = new (heap) MergeUnion(args->at(0)->castToRelExpr(), args->at(1)->castToRelExpr());
  } else if (fname == "MVI" OR fname == "EXPR") {
    if (badSingleArg(fname, args, diags)) return NULL;

    result = new (heap) MapValueIds(args->at(0)->castToRelExpr());
  } else if (fname == "FAST_EXTRACT" || fname == "HIVE_INSERT") {
    if (badSingleArg(fname, args, diags)) return NULL;
    result = new (heap) WildCardOp(REL_ANY_EXTRACT, 0, args->at(0)->castToRelExpr());
  } else if (fname == "SORT") {
    if (badSingleArg(fname, args, diags)) return NULL;

    result = new (heap) Sort(args->at(0)->castToRelExpr());
  } else if (fname == "EXCHANGE") {
    forcedType = FORCED_EXCHANGE;
    wildcard = REL_FORCE_EXCHANGE;
    whichExch = ExchangeForceWildCard::ANY_EXCH;
  } else if (fname == "PA" OR fname == "PARTITION_ACCESS") {
    forcedType = FORCED_EXCHANGE;
    wildcard = REL_FORCE_EXCHANGE;
    whichExch = ExchangeForceWildCard::FORCED_PA;
  } else if (fname == "PAPA" OR fname == "SPLIT_TOP_PA") {
    forcedType = FORCED_EXCHANGE;
    wildcard = REL_FORCE_EXCHANGE;
    whichExch = ExchangeForceWildCard::FORCED_PAPA;
  } else if (fname == "ESP_EXCHANGE" OR fname == "REPARTITION") {
    forcedType = FORCED_EXCHANGE;
    wildcard = REL_FORCE_EXCHANGE;
    whichExch = ExchangeForceWildCard::FORCED_ESP_EXCHANGE;
  } else if (fname == "MJ" OR fname == "MERGE_JOIN") {
    forcedType = FORCED_JOIN;
  ...
  else if (fname == "HJ" OR fname == "HASH_JOIN") {
    forcedType = FORCED_JOIN;
    wildcard = REL_FORCE_HASH_JOIN;
  }

  else if (fname == "HHJ" OR fname == "HYBRID_HASH_JOIN") {
    forcedType = FORCED_JOIN;
    wildcard = REL_FORCE_HYBRID_HASH_JOIN;
  } else if (fname == "OHJ" OR fname == "ORDERED_HASH_JOIN") {
    forcedType = FORCED_JOIN;
    wildcard = REL_FORCE_ORDERED_HASH_JOIN;
  }

  // Ordered cross product type for star schema change.
  else if (fname == "OCP" OR fname == "ORDERED_CROSS_PRODUCT") {
    forcedType = FORCED_JOIN;
    ...
    result = new (heap) SortGroupBy(args->at(0)->castToRelExpr());
  } else if (fname == "HG" OR fname == "HASH_GROUPBY") {
    if (badSingleArg(fname, args, diags)) return NULL;

    result = new (heap) HashGroupBy(args->at(0)->castToRelExpr());
  } else if (fname == "SHORTCUT_GROUPBY") {
    if (badSingleArg(fname, args, diags)) return NULL;

    result = new (heap) ShortCutGroupBy(args->at(0)->castToRelExpr());
  } else if (fname == "TRANSPOSE") {
    if (badSingleArg(fname, args, diags)) return NULL;

    result = new (heap) Transpose(NULL, NULL, args->at(0)->castToRelExpr());
    ...
    result = new (heap) UnPackRows(0, NULL, NULL, NULL, args->at(0)->castToRelExpr(), NULL_VALUE_ID);
  } else if (fname == "PACK") {
    if (badSingleArg(fname, args, diags)) return NULL;
    ...
    for (int i = 0; i < numArgs; i++) {
      ...
      if (!((UDFForceWildCard *)result)->mergeUDFOptions(*tmpUDF)) {
        ...
        *diags << DgSqlCode(-3113) << DgString0("Conflicting isolated_scalar_udf options.");
        return NULL;
      }
    }
    // If action name argument given, then must also specify function name.
    if (((UDFForceWildCard *)result)->getFunctionName() == "" && ((UDFForceWildCard *)result)->getActionName() != "") {
      *diags << DgSqlCode(-3113) << DgString0("Missing isolated_scalar_udf SCALAR_UDF option.");
      return NULL;
    }
  } else if (fname == "TMUDF") {
    int numArgs = args->entries();
    ...
    if (numArgs == 1) {
      ...
      *diags << DgSqlCode(-3113) << DgString0("TMUDF operator must have 0, 1 or 2 arguments.");
      return NULL;
    }

    result = new (heap) WildCardOp(op, 0, child0, child1);
  } else {
  ...
  if (result == NULL AND forcedType == FORCED_JOIN) {
    ...
    if (args->entries() < 2 OR args->entries() > 4) {
      *diags << DgSqlCode(-3113) << DgString0("Join force operators must have 2, 3, or 4 arguments.");
      return NULL;
    }
    ...
    if (args->entries() > 2) {
      for (int i = 2; i < (int)args->entries(); i++) {
        ...
        ConstValue *cv = args->at(i)->castToItemExpr()->castToConstValue(dummyNegate);
        const NAType *nat = cv->getType();

        if (nat->getTypeQualifier() == NA_CHARACTER_TYPE) {
          NAString planNum((char *)cv->getConstValue(), (size_t)cv->getStorageSize(), STMTHEAP);
          planNum.toUpper();
          ...
          if (planNum == "PLAN0")
            ...
            *diags << DgSqlCode(-3113) << DgString0("Expected TYPE1 or TYPE2.");
            return NULL;
          }
        } else if (nat->getTypeQualifier() == NA_NUMERIC_TYPE) {
          if (cv->getStorageSize() > 2) {
            *diags << DgSqlCode(-3113) << DgString0("Number of ESPs (short int) expected.");
            return NULL;
          }
          numOfEsps = cv->getExactNumericValue();
        } else {
          *diags << DgSqlCode(-3113) << DgString0("Additional join arguments must be string or integer.");
          return NULL;
        }
      }
    }

    if (NOT(args->at(0)->castToRelExpr() AND args->at(1)->castToRelExpr())) {
      *diags << DgSqlCode(-3113) << DgString0("Join needs two arguments representing a query.");
      return NULL;
    }

    result = new (heap)
        JoinForceWildCard(wildcard, args->at(0)->castToRelExpr(), args->at(1)->castToRelExpr(), whichPlan, numOfEsps);
    ...
    if (args->entries() < 1 OR args->at(0)->castToRelExpr() == NULL) {
      *diags << DgSqlCode(-3113) << DgString0("Shape as first argument expected for exchange operator");
      return NULL;
    }
    ...
    if (args->entries() > 1) {
      for (int i = 1; i < (int)args->entries(); i++) {
        ...
        if (NOT(args->at(i)->castToItemExpr() AND args->at(i)->castToItemExpr()->castToConstValue(dummyNegate))) {
          *diags << DgSqlCode(-3113)
                 << DgString0(
                        "Additional exchange arguments must be 'GROUP', 'SPLIT', 'REPART' or a number (of ESPs).");
          return NULL;
        }
        ...
        if (nat->getTypeQualifier() == NA_CHARACTER_TYPE) {
          NAString logPart((char *)cv->getConstValue(), (size_t)cv->getStorageSize(), CmpCommon::statementHeap());
          logPart.toUpper();
          ...
          if (logPart == "GROUP")
            ...
            *diags << DgSqlCode(-3113) << DgString0("Expected GROUP, SPLIT, or REPART.");
            return NULL;
          }
        } else if (nat->getTypeQualifier() == NA_NUMERIC_TYPE) {
          if (cv->getStorageSize() > 2) {
    ...
    if ((whichExch == ExchangeForceWildCard::ANY_EXCH OR whichExch == ExchangeForceWildCard::FORCED_PA)
            AND numOfEsps > 0) {
      *diags << DgSqlCode(-3113) << DgString0("Number of ESPs can not be specified for EXCHANGE or PARTITION_ACCESS.");
      return NULL;
    }

    // can't force logical partitioning for EXCHANGE or ESP_EXCHANGE
    if ((whichExch == ExchangeForceWildCard::ANY_EXCH OR whichExch == ExchangeForceWildCard::FORCED_ESP_EXCHANGE)
            AND whichLogPart != ExchangeForceWildCard::ANY_LOGPART) {
      *diags << DgSqlCode(-3113)
             << DgString0("GROUP, SPLIT, or REPART are allowed for SPLIT_TOP_PA and PARTITION_ACCESS only.");
      return NULL;
    }

    result = new (heap) ExchangeForceWildCard(args->at(0)->castToRelExpr(), whichExch, whichLogPart, numOfEsps);
  }
