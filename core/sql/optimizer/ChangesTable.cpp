...
//////////////////////////////////////////////////////////////////////////////
MvIudLog::MvIudLog(CorrName &name, BindWA *bindWA, OperatorTypeEnum opType)
...
//////////////////////////////////////////////////////////////////////////////
MvIudLog::MvIudLog(const GenericUpdate *baseNode, BindWA *bindWA)
...
void MvIudLog::initRangeLogging() {
  if (getOpType() == REL_UNARY_INSERT) {
    const ComMvAttributeBitmap &mvBitmap = getSubjectNaTable()->getMvAttributeBitmap();
    if (mvBitmap.getAutomaticRangeLoggingRequired()) needsRangeLogging_ = TRUE;
  }
}
...
//////////////////////////////////////////////////////////////////////////////
ItemExpr *MvIudLog::createAtColExpr(const NAColumn *naColumn, NABoolean isInsert, NABoolean isUpdate,
                                    NABoolean isUndo) const {
  const NAString &colName = naColumn->getColName();
  ItemExpr *result = NULL;
...
ItemExpr *MvIudLog::createColExprForEpoch() const {
  return new (heap_) ColReference(new (heap_) ColRefName(InliningInfo::getEpochVirtualColName(), heap_));
}
...
//////////////////////////////////////////////////////////////////////////////
ItemExpr *MvIudLog::createColExprForOpType(NABoolean isInsert, NABoolean isUpdate, NABoolean isUndo) const {
  ItemExpr *result = NULL;
...
//////////////////////////////////////////////////////////////////////////////
ItemExpr *MvIudLog::createColExprForIgnore() const { return new (heap_) SystemLiteral(0); }
...
//////////////////////////////////////////////////////////////////////////////
ItemExpr *MvIudLog::createColExprForBitmap(const NAColumn *naColumn, NABoolean isUpdate) const {
  ItemExpr *result = NULL;
...
ItemExpr *MvIudLog::createColExprForRangeSize(NABoolean isInsert) const {
  ...
  if (needsRangeLogging_) {
    ...
    CMPASSERT(isInsert);
    result = new (heap_) ColReference(new (heap_) ColRefName(InliningInfo::getRowCountVirtualColName(), heap_));
  } else {
    result = new (heap_) SystemLiteral(isInsert ? 1 : 0);
  }
...
ItemExpr *MvIudLog::createColExprForAtSyskey(NABoolean isInsert) const {
  ...
  return new (heap_) ColReference(new (heap_) ColRefName("SYSKEY", corrName, heap_));
}
...
ItemExpr *MvIudLog::createSyskeyColExpr(const NAString &colName, NABoolean isInsert) const {
...
//
//////////////////////////////////////////////////////////////////////////////
ItemExpr *MvIudLog::createColExprForAtTimestamp(NABoolean isInsert, NABoolean isUpdate, NABoolean isUndo) const {
  ...
  incrementExpr = new (heap_) ItmBlockFunction(
      counterExpr,
      new (heap_) Assign(counterExpr, new (heap_) BiArith(ITM_PLUS, counterExpr, new (heap_) ConstValue(ROW_TS_INCR))));

  // Synthesize the types and value IDs for the new items
  incrementExpr->synthTypeAndValueId(TRUE);

  tsExpr =
      new (heap_) BiArith(ITM_PLUS, incrementExpr,
                          new (heap_) ColReference(new (heap_) ColRefName(InliningInfo::getMvLogTsColName(), heap_)));
...
CorrName *MvIudLog::calcTargetTableName(const QualifiedName &tableName) const {
  return buildChangesTableCorrName(tableName, COMMV_IUD_LOG_SUFFIX, ExtendedQualName::IUD_LOG_TABLE, heap_);
}
...
ItemExpr *MvIudLog::buildLogEpochPredicate(const DeltaDefinition *deltaDef, CollHeap *heap) {
  ...
  if (singleEpoch) {
    epochPred = BinderUtils::buildPredOnCol(ITM_EQUAL, COMMV_EPOCH_COL, beginEpoch, heap);
  } else {
    epochPred = new (heap) Between(new (heap) ColReference(new (heap) ColRefName(COMMV_EPOCH_COL, heap)),
                                   new (heap) SystemLiteral(beginEpoch), new (heap) SystemLiteral(endEpoch));
  }
...
ItemExpr *MvIudLog::createSpecificWhereExpr(RowsType type) const {
  ...
  result = BinderUtils::buildPredOnCol(ITM_LESS, COMMV_IGNORE_COL, beginEpoch, heap_);

  ItemExpr *opTypePred = NULL;
  switch (type) {
    case INSERTED_ROWS:
      opTypePred = new (heap_)
          BiLogic(ITM_OR, BinderUtils::buildPredOnCol(ITM_EQUAL, COMMV_OPTYPE_COL, ComMvRowType_Insert, heap_),
                  BinderUtils::buildPredOnCol(ITM_EQUAL, COMMV_OPTYPE_COL, ComMvRowType_InsertOfUpdate, heap_));
      break;

    case DELETED_ROWS:
      opTypePred = new (heap_)
          BiLogic(ITM_OR, BinderUtils::buildPredOnCol(ITM_EQUAL, COMMV_OPTYPE_COL, ComMvRowType_Delete, heap_),
                  BinderUtils::buildPredOnCol(ITM_EQUAL, COMMV_OPTYPE_COL, ComMvRowType_DeleteOfUpdate, heap_));
      break;
  ...
  if (getAddOrigScanPredicateFlag()) {
    ...
    if (origScanPredicate != NULL) result = new (heap_) BiLogic(ITM_AND, result, origScanPredicate);
  }
...
NABoolean MvIudLog::fixReferencesFromBaseTableToLog(ItemExpr *expr) const {
  ...
  if (expr->getArity() > 0) {
    ...
    if (expr->getOperatorType() == ITM_REFERENCE) {
      ...
      QualifiedName &tableName = colRef->getCorrNameObj().getQualifiedNameObj();

      // Ignore predicates that reference other tables.
      if (tableName != getSubjectTableName().getQualifiedNameObj()) return FALSE;
...
ItemExpr *MvIudLog::buildBitmapColPredicate() const {
  ...
  ItemExpr *insertRecord = BinderUtils::buildPredOnCol(ITM_EQUAL, COMMV_OPTYPE_COL, ComMvRowType_Insert, heap_);

  ItemExpr *deleteRecord = BinderUtils::buildPredOnCol(ITM_EQUAL, COMMV_OPTYPE_COL, ComMvRowType_Delete, heap_);

  return new (heap_)
      BiLogic(ITM_OR, insertRecord,
              new (heap_) BiLogic(ITM_OR, deleteRecord, new (heap_) BiLogic(ITM_OR, isBitmapColNull, isBitwiseTrue)));
}
...
ItemExpr *MvIudLog::constructUpdateBitmapFromList(const LIST(int) & columnList) const {
  ...
  for (CollIndex i = 0; i < logColumns.entries(); i++) {
    ...
    if (!currentTempCol->getColName().compareTo(COMMV_BITMAP_COL)) break;
  }
  ...
  for (int i = 0; i < bitmapSize; i++) usedColsBitmap[i] = 0;
  ...
  for (CollIndex j = 0; j < columnList.entries(); j++) {
    ...
    if (b / 8 < bitmapSize) usedColsBitmap[b / 8] |= 0x01 << b % 8;
  }
...
ItemExpr *MvIudLog::buildIndirectUpdateExpression() const {
  ...
  ItemExpr *isBitmapColNotNull = new (heap_) UnLogic(ITM_IS_NOT_NULL, bitmapCol);

  return new (heap_) BiLogic(ITM_AND, isBitmapColNotNull, isBitwiseTrue);
}

// For mvIudLog, the union above Tuples must be ordered.
void MvIudLog::setTuplesUnionType(Union *unionNode) const { unionNode->setOrderedUnion(); }
...
//////////////////////////////////////////////////////////////////////////////
MvIudLogForMvLog::MvIudLogForMvLog(CorrName &tableName, BindWA *bindWA)
...
ItemExpr *MvIudLogForMvLog::createColExprForEpoch() const {
  ...
  ItemExpr *epochExpr = new (heap_) BiArith(ITM_TIMES, epochColExpr, new (heap_) SystemLiteral(-1));
  return epochExpr;
}
...
//////////////////////////////////////////////////////////////////////////////
ItemExpr *MvIudLogForMvLog::createColExprForIgnore() const { return new (heap_) SystemLiteral(0); }

//////////////////////////////////////////////////////////////////////////////
// build the expression to insert into the @UPDATE_BITMAP column.
// Not used for context rows.
//////////////////////////////////////////////////////////////////////////////
ItemExpr *MvIudLogForMvLog::createColExprForBitmap(const NAColumn *naColumn, NABoolean isUpdate) const {
...
// Not used for context rows.
//////////////////////////////////////////////////////////////////////////////
ItemExpr *MvIudLogForMvLog::createColExprForAtTimestamp(NABoolean isInsert, NABoolean isUpdate,
                                                        NABoolean isUndo) const {
  return new (heap_) SystemLiteral(0);
}
...
//
//////////////////////////////////////////////////////////////////////////////
ItemExpr *MvIudLogForMvLog::createBaseColExpr(const NAString &colName, NABoolean isInsert, NABoolean isUpdate) const {
  ...
  return new (heap_) ColReference(new (heap_) ColRefName(virtualColumnName, heap_));
}
...
CorrName *MvLogForContextRows::calcTargetTableName(const QualifiedName &tableName) const {
  return buildChangesTableCorrName(tableName,
                                   "",  // No Suffix
                                   ExtendedQualName::RANGE_LOG_TABLE, heap_);
}

//////////////////////////////////////////////////////////////////////////////
// For the MV context log, there are only two @ column: @EPOCH and @SYSKEY.
//////////////////////////////////////////////////////////////////////////////
ItemExpr *MvLogForContextRows::createAtColExpr(const NAColumn *naColumn, NABoolean isInsert, NABoolean isUpdate,
                                               NABoolean isUndo) const {
  const NAString &colName = naColumn->getColName();
  ...
  if (colName.data()[1] == 'E') {
    ...
    return new (heap_) ColReference(new (heap_) ColRefName(COMMV_SYSKEY_COL, getCorrNameForNewRow(), heap_));
  }
}
...
ItemExpr *MvLogForContextRows::createSyskeyColExpr(const NAString &colName, NABoolean isInsert) const {
  CMPASSERT(FALSE);
  return NULL;
}
...
ItemExpr *MvLogForContextRows::createSpecificWhereExpr(RowsType type) const {
  ...
  ItemExpr *contextEpochExpr = new (heap_)
      BiRelat(ITM_EQUAL, new (heap_) ColReference(new (heap_) ColRefName(COMMV_EPOCH_COL, heap_)), contextEpochType);
