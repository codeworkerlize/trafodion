...
void JBB::enumerateReductionPaths() {
  ...
  CANodeId JBBCId;
  for (JBBCId = localLeafReducers.init(); localLeafReducers.next(JBBCId); localLeafReducers.advance(JBBCId)) {
    JBBC *currentJBBC = (JBBCId.getNodeAnalysis())->getJBBC();
  ...
  for (JBBCId = localNonLeafReducers.init(); localNonLeafReducers.next(JBBCId); localNonLeafReducers.advance(JBBCId)) {
    JBBC *currentJBBC = (JBBCId.getNodeAnalysis())->getJBBC();
    // start linear reduction paths from currentJBBC
    currentJBBC->startReductionPaths();
  }
  ...
  for (JBBCId = joinLeafReducers.init(); joinLeafReducers.next(JBBCId); joinLeafReducers.advance(JBBCId)) {
    JBBC *currentJBBC = (JBBCId.getNodeAnalysis())->getJBBC();
    // start linear reduction paths from currentJBBC
    currentJBBC->startReductionPaths();
  }
  ...
  for (JBBCId = joinNonLeafReducers.init(); joinNonLeafReducers.next(JBBCId); joinNonLeafReducers.advance(JBBCId)) {
    JBBC *currentJBBC = (JBBCId.getNodeAnalysis())->getJBBC();
    // start linear reduction paths from currentJBBC
    currentJBBC->startReductionPaths();
  }
...
void JBB::debugQueryGraph() {
  ...
  if (reductionPathString > 0) {
    ...
    currentReductionPathProps = (QueryAnalysis::ASM())->getStatsForCANodeIdSet(currentReductionPath);
  }
  ...
  while (reductionPathString > 0) {
    ...
    // with jBBCToJoin
    EstLogPropSharedPtr joinEstLogProp = (QueryAnalysis::ASM())->joinJBBChildren(currentReductionPath, jBBCToJoinSet);
...
void JBBC::printQueryGraphNodeText(FILE *f, const char *prefix, const char *suffix) const {
#ifndef NDEBUG
  fprintf(f, getQueryGraphNodeText());
#endif
};

void JBBC::displayQueryGraphNodeText() const { printQueryGraphNodeText(); };
...
void JBBC::initializeQueryGraphNode() {
  ...
  for (CANodeId JBBCId = connectedJBBCs.init(); connectedJBBCs.next(JBBCId); connectedJBBCs.advance(JBBCId)) {
    // create a QueryGraphConnection Object representing a connection
    // from JBBC with id JBBCId to 'this' JBBC
    QueryGraphConnection *newConnection = new (heap_) QueryGraphConnection(getId(), JBBCId);
...
void JBBC::extendReductionPath(CANodeIdSet currentReductionPath, CostScalar cardinalityOfCurrentReductionPath,
                               CANodeId from) {
  ...
  if (from != NULL_CA_ID) {
    ...
    for (i = 0; i < incomingConnections_->entries(); i++) {
      if ((*incomingConnections_)[i]->getConnectedJBBC() == from) {
        ...
        (*incomingConnections_)[i]->addIncomingReductionPath(reductionPathToStore, cardinalityOfCurrentReductionPath);
        break;
      }
    }
  ...
  // came from
  CANodeId JBBCId;
  for (JBBCId = connectedJBBCs.init(); connectedJBBCs.next(JBBCId); connectedJBBCs.advance(JBBCId)) {
    ...
    if ((JBBCId != from) && (!currentReductionPath.contains(JBBCId))) {
      ...
      // path to extend
      if ((cardinalityOfCurrentReductionPath * 1.1) < connectedJBBC->getAfterLocalPredCardinality()) {
...
CostScalar JBBC::getAfterLocalPredCardinality() { return afterLocalPredCardinality_; };
// JBBC::getAfterLocalPredCardinality()

NABoolean JBBC::mergeReductionPaths() {
  if ((incomingConnections_->entries() < 3) && (reductionPaths_->entries() < 2)) return FALSE;
  ...
  for (i = 0; i < incomingConnections_->entries(); i++) {
    ...
    const CANodeIdSet *bestIncomingReductionPath = (*incomingConnections_)[i]->getBestIncomingReductionPath();
    ...
    if (bestIncomingReductionPath) {
      ...
      EstLogPropSharedPtr joinEstLogProp = (QueryAnalysis::ASM())->joinJBBChildren(mergedReductionPath, tempNodeSet);
  ...
  if (mergedConnections.entries() > 1) {
    ...
    // this JBBC
    if (mergedReductionPathCardinality < (getAfterLocalPredCardinality() * .95)) {
      returnValue = TRUE;
      // insert merged reduction path in the list of reduction paths
      // store the reduction path passed in
      // store the reduction path passed in
      CANodeIdSet *reductionPathToStore = new (collHeap()) CANodeIdSet(mergedReductionPath);
      reductionPaths_->insert(reductionPathToStore);
      ...
      for (i = 0; i < incomingConnections_->entries(); i++) {
        ...
        CANodeId connectedNodeId = ((*incomingConnections_)[i])->getConnectedJBBC();
        ...
        // this avoids cycles
        if ((!mergedConnections.contains(connectedNodeId)) && (!mergedReductionPath.contains(connectedNodeId))) {
          // Get reference to connected JBBC
          JBBC *connectedJBBC = (connectedNodeId.getNodeAnalysis())->getJBBC();

          // Do a join of the merged reduction path with the connected JBBC
          CANodeIdSet connectedNode(connectedNodeId);
          joinEstLogProp = (QueryAnalysis::ASM())->joinJBBChildren(mergedReductionPath, connectedNode);
          ...
          if ((mergedReductionPathCardinality * 1.1) < connectedJBBC->getAfterLocalPredCardinality()) {
            // if the cardinality of the join is atleast 5 % lower than the
            // after local predicate cardinality of the connected JBBC,
            // extend the reduction path
            if (joinCardinality < (connectedJBBC->getAfterLocalPredCardinality() * .95)) {
              // Extend merged redution path to connected JBBC
              connectedJBBC->extendReductionPath(mergedReductionPath, joinCardinality, getId());
            }
          }
        }
      }
...
JBBSubset *JBBC::getReducingJoinSet(CANodeIdSet &availableJBBCs, CANodeIdSet *usedSet) {
  ...
  if (!usedSet) {
    ...
    for (i = 0; i < reductionPaths_->entries(); i++) {
      ...
      if (availableJBBCs.contains(*(*reductionPaths_)[i])) {
        ...
        CostScalar cardinalityOfCurrentReductionPath = (*cardinalityOfReductionPaths_)[i];
        ...
        if (cardinalityOfCurrentReductionPath < minCardinality) {
          ...
          // recomputation is required for this reduction path
            ...
            EstLogPropSharedPtr recomputedLogProps =
                (QueryAnalysis::ASM())->joinJBBChildren(currentReductionPath, recomputationSet);
...
QueryGraphConnection::QueryGraphConnection(CANodeId targetJBBC, CANodeId connectedJBBC, CollHeap *outHeap) {
...
NABoolean QueryGraphConnection::addIncomingReductionPath(CANodeIdSet *incomingReductionPath,
  ...
  if (!bestIncomingReductionPath_) {
    ...
    cardinalityOfBestIncomingReductionPath_ = cardinalityOfIncomingReductionPath;

    // return TRUE indicating bestIncomingReductionPath_ was updated
    return TRUE;
  } else {
    if (cardinalityOfIncomingReductionPath < cardinalityOfBestIncomingReductionPath_) {
...
const CANodeIdSet *QueryGraphConnection::getBestIncomingReductionPath() { return bestIncomingReductionPath_; }
