  ...
  for (i = 0; i < allColumns.entries(); i++)
      ...
      else
	{
      ...
      if (!naTable->isHiveTable())
			  ixcol->setAssignColName(baseValueId.getNAColumn()->getColName());
...
NABoolean IndexDesc::isNgramIndex() const
{
  return  getNAFileSet()->ngramIndex();
}

// if the index is a partition local index for partition base table
NABoolean IndexDesc::isPartLocalBaseIndex() const
{
  return  getNAFileSet()->partLocalBaseIndex();
}

// if the index is a partition local index for each partition table
NABoolean IndexDesc::isPartLocalIndex() const
{
  return  getNAFileSet()->partLocalIndex();
}

// if the index is a partition global index
NABoolean IndexDesc::isPartGlobalIndex() const
{
  return  getNAFileSet()->partGlobalIndex();
}
