...
//////////////////////////////////////////////////////////////////////////////
static BaseColumn *findTheBaseColumn(ItemExpr *expr, NABoolean *moreThanOne = NULL) {
  if (moreThanOne != NULL) *moreThanOne = FALSE;

  if (expr->getOperatorType() == ITM_BASECOLUMN) return (BaseColumn *)expr;
  ...
  for (ValueId leaf = leafs.init(); leafs.next(leaf); leafs.advance(leaf)) {
    ...
    if (node->getOperatorType() == ITM_BASECOLUMN) {
      if (result == NULL)
        ...
        if (moreThanOne != NULL) *moreThanOne = TRUE;
        return result;
      }
      ...
      for (ValueId column = vegMembers.init(); vegMembers.next(column); vegMembers.advance(column)) {
        ItemExpr *baseColExpr = column.getItemExpr();
        if (baseColExpr->getOperatorType() != ITM_BASECOLUMN) continue;
...
//////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////
static void findAllTheUsedColumns(ItemExpr *expr, TableDesc *currentTable, LIST(BaseColumn *) & usedCols) {
  ValueIdSet leafs;
  expr->getLeafValueIds(leafs);
  ...
  for (ValueId leaf = leafs.init(); leafs.next(leaf); leafs.advance(leaf)) {
    ...
    if (node->getOperatorType() == ITM_VEG_REFERENCE) {
      ...
      for (ValueId vegMem = vegMembers.init(); vegMembers.next(vegMem); vegMembers.advance(vegMem)) {
        ...
        if (expr->getOperatorType() != ITM_BASECOLUMN) continue;
        BaseColumn *baseCol = (BaseColumn *)expr;
        if (currentTable != NULL && baseCol->getTableDesc() != currentTable) continue;
...
static NABoolean isTreeLeftLinear(RelExpr *node) {
  ...
  if (nodeType == REL_SCAN) return TRUE;
  if (nodeType == REL_ISOLATED_SCALAR_UDF && ((RelRoutine *)node->castToRelExpr())->getNARoutine()->isDeterministic())
    return TRUE;
  ...
  if (nodeType == REL_JOIN || nodeType == REL_LEFT_JOIN || nodeType == REL_ROUTINE_JOIN) {
    if (node->child(1)->getOperatorType() == REL_SCAN ||
        (node->child(1)->getOperatorType() == REL_ISOLATED_SCALAR_UDF &&
         ((RelRoutine *)node->child(1)->castToRelExpr())->getNARoutine()->isDeterministic()))
...
static ItemExpr *SkipCast(ItemExpr *expr) {
  if (expr->getOperatorType() == ITM_CAST)
    return expr->child(0);
  else if (expr->getOperatorType() == ITM_CHAR && expr->child(0)->getOperatorType() == ITM_CAST) {
...
//////////////////////////////////////////////////////////////////////////////
static NABoolean isANonRepeatableExpression(const ItemExpr *expr, BindWA *bindWA) {
...
void Join::collectMVInformation(MVInfoForDDL *mvInfo, NABoolean isNormalized) {
  ...
  if (isNormalized) mvInfo->addJoinNode(this);
}

void Scan::collectMVInformation(MVInfoForDDL *mvInfo, NABoolean isNormalized) {
  RelExpr::collectMVInformation(mvInfo, isNormalized);
  // In the Scan nodes, we need the minimal outputs, that are
  // calculated during normalization.
  if (isNormalized) mvInfo->addScanNode(this);
}

void GroupByAgg::collectMVInformation(MVInfoForDDL *mvInfo, NABoolean isNormalized) {
  RelExpr::collectMVInformation(mvInfo, isNormalized);
  // GroupBy information is ready for use after binding.
  if (!isNormalized) mvInfo->addGroupByNode(this);
}

void RenameTable::collectMVInformation(MVInfoForDDL *mvInfo, NABoolean isNormalized) {
...
// mapped expression.
ViewColumnConnection::ViewColumnConnection(const ColumnDesc *colDesc, CollHeap *heap)
    : baseColName_(heap),
...
//----------------------------------------------------------------------------
ViewTableConnection::~ViewTableConnection() { colConnections_.clearAndDestroy(); }
...
//----------------------------------------------------------------------------
NABoolean ViewTableConnection::contains(const ColRefName &colName) const { return (findColumn(colName) != NULL); }
...
void ViewTableConnection::insert(const ViewColumnConnection *col) {
  ...
  const ViewColumnConnection *currentCol = colConnections_.getFirstValue(col->getColName());
...
#ifndef NDEBUG
void ViewTableConnection::print(FILE *ofd, const char *indent, const char *title) const {
  NAString tableName = getTableName().getQualifiedNameAsString();
  fprintf(ofd, "\n\tTable %s:\n", tableName.data());

  columnConnection &hash = (columnConnection &)colConnections_;
  NAHashDictionaryIterator<const NAString, const ViewColumnConnection> iterator(hash);
...
void ViewTableConnection::display() const { print(); }
#endif
...
ViewColumnGraph::~ViewColumnGraph() {
  for (CollIndex i = 0; i < tableConnections_.entries(); i++) delete tableConnections_[i];
}
...
ViewTableConnection *ViewColumnGraph::findTableConnection(const QualifiedName &tableName) const {
  for (CollIndex i = 0; i < tableConnections_.entries(); i++) {
    ...
    if (tableConnection->getTableName() == tableName) return tableConnection;
  }
  return NULL;
}

//----------------------------------------------------------------------------
void ViewColumnGraph::addConnection(ViewColumnConnection *colConnection, CollHeap *heap) {
  ...
  ViewTableConnection *tableConnection = findTableConnection(colConnection->getTableName());

  // If none are found, create a new one and insert it into the list.
  if (tableConnection == NULL) {
    tableConnection = new (heap) ViewTableConnection(colConnection->getTableName(), colConnection->getViewName(), heap);
    tableConnections_.insert(tableConnection);
  }

  // If the column connection is direct and not complex, insert it into the
  // table connection.
  if (colConnection->isComplex()) return;
...
void ViewColumnGraph::addTableMapping(RETDesc *retDesc, NABoolean addSysCols, CollHeap *heap) {
  ...
  if (addSysCols) viewColumns.insert(*retDesc->getSystemColumnList());

  // Create a column connection object for each column on the list, and insert
  // it into the graph.
  for (CollIndex col = 0; col < viewColumns.entries(); col++) {
    ViewColumnConnection *colConn = new (heap) ViewColumnConnection(viewColumns[col], heap);
...
NABoolean ViewColumnGraph::contains(const ColRefName &colName) const {
  ViewTableConnection *tableConnection = findTableConnection(colName.getCorrNameObj().getQualifiedNameObj());
  if (tableConnection == NULL)
    return FALSE;
  else
...
NABoolean ViewColumnGraph::isMaskedColumn(const NAColumn *keyCol) const {
  ViewTableConnection *tableConnection = findTableConnection(*keyCol->getTableName());
  if (tableConnection == NULL)
    return FALSE;
  else
...
// Display/print, for debugging.
#ifndef NDEBUG
void ViewColumnGraph::print(FILE *ofd, const char *indent, const char *title) const {
  fprintf(ofd, "\nViewColumnGraph:\n");
  for (CollIndex i = 0; i < tableConnections_.entries(); i++) tableConnections_[i]->print();

  fprintf(ofd, "ValueId mappings:\n");
  valueIdConnection &hash = (valueIdConnection &)columnValueIds_;
  NAHashDictionaryIterator<const ValueId, const ViewColumnConnection> iterator(hash);
...
void ViewColumnGraph::display() const { print(); }
...
//////////////////////////////////////////////////////////////////////////////
MVVegPredicateColumn::MVVegPredicateColumn(const BaseColumn *baseCol, NABoolean isInLeftJoin, NABoolean isFromVegRef,
                                           ComMVSUsageType usageType, const ItemExpr *complexExpr, CollHeap *heap)
    : tableName_(baseCol->getTableDesc()->getCorrNameObj().getQualifiedNameObj(), heap),
      ...
      joinSide_(!isInLeftJoin ? COM_NO_LEFT_JOIN : (isFromVegRef ? COM_LEFT_OUTER : COM_LEFT_INNER)),
      complexExpr_(complexExpr),
      usageType_(usageType),
      text_(heap) {
  if (complexExpr != NULL) complexExpr->unparse(text_, DEFAULT_PHASE, MVINFO_FORMAT);
}
...
void MVVegPredicateColumn::print(FILE *ofd, const char *indent, const char *title) const {
  ...
  if (isComplex()) {
    ...
    fprintf(ofd, "%s(%d) %s, ", tableText.data(), getColNumber(), leftJoinSide);
  }
}

void MVVegPredicateColumn::display() const { print(); }
#endif
...
MVVegPredicate::MVVegPredicate(const MVVegPredicate &other, CollHeap *heap)
  ...
  for (CollIndex i = 0; i < other.entries(); i++) insert(new (heap) MVVegPredicateColumn(*other[i], heap));
}
...
// isFromVegRef is TRUE when this column is from the outer table of a left join.
//////////////////////////////////////////////////////////////////////////////
void MVVegPredicate::addVegPredicates(const VEG *veg, NABoolean isFromVegRef, LIST(const VEG *) & vegPtrList,
                                      CollHeap *heap) {
  const ValueIdSet &vegMembers = veg->getAllValues();
  for (ValueId column = vegMembers.init(); vegMembers.next(column); vegMembers.advance(column)) {
...
//////////////////////////////////////////////////////////////////////////////
void MVVegPredicate::addPredicateMember(ValueId vid, NABoolean isFromVegRef, const VEG *parentVeg,
                                        LIST(const VEG *) & vegPtrList, CollHeap *heap) {
  ...
  switch (colType) {
    case ITM_BASECOLUMN: {
      ...
      MVVegPredicateColumn *col =
          new (heap) MVVegPredicateColumn(baseCol, isOnLeftJoin(), isFromVegRef, usageType_, NULL, heap);
      insert(col);
    } break;
    ...
    case ITM_VEG_REFERENCE: {
      ...
      if (isOnLeftJoin()) CMPASSERT(!isFromVegRef);
      ...
      if (parentVeg != NULL && vegPtrList.contains(parentVeg)) {
        ...
        const MVVegPredicateColumn *nestedCol = nestedVeg.getRepresentativeCol();
        MVVegPredicateColumn *col = new (heap) MVVegPredicateColumn(*nestedCol, heap, TRUE);
        insert(col);
      } else {
        if (parentVeg != NULL) vegPtrList.insert(parentVeg);
    ...
    default:
      ...
      BaseColumn *baseCol = findTheBaseColumn(predExpr, &usesMultipleColumns);
      ...
      if (usesMultipleColumns) isIncremental_ = FALSE;

      MVVegPredicateColumn *col =
          new (heap) MVVegPredicateColumn(baseCol, isOnLeftJoin(), isFromVegRef, usageType_, predExpr, heap);
      insert(col);
      break;
  }
}
...
const MVVegPredicateColumn *MVVegPredicate::getRepresentativeCol() const {
  for (CollIndex i = 0; i < entries(); i++) {
    ...
    if (!currentCol->isComplex()) return currentCol;
  }
  return at(0);
}
...
void MVVegPredicate::markPredicateColsOnUsedObjects(MVInfo *mvInfo, NABoolean isDDL) {
  ...
  for (CollIndex i = 0; i < entries(); i++) {
    ...
    if (currentCol->isComplex()) continue;
    ...
    if (isDDL) {
      ...
      colInfo = mvInfo->getMVColumns().getMvColInfoByBaseColumn(currentCol->getTableName(), currentCol->getColNumber());
      if (colInfo != NULL && colInfo->getColType() == COM_MVCOL_BASECOL && !mvCols.contains(colInfo)) {
  ...
  if (mvCols.entries() > 1) {
    ...
    if (!firstMvColInfo->isInMVCI()) {
      for (CollIndex ix = 1; ix < mvCols.entries(); ix++) {
        colInfo = mvCols[ix];
        if ((colInfo->getColNumber() < firstMvColInfo->getColNumber()) || colInfo->isInMVCI()) firstMvColInfo = colInfo;
      }
    }
    ...
    for (CollIndex jx = 0; jx < mvCols.entries(); jx++) {
      colInfo = mvCols[jx];
      if (colInfo->getColNumber() != firstMvCol) colInfo->setAsRedundant(firstMvCol);
    }
  }
...
Lng32 MVVegPredicate::getCoveringColFor(Lng32 colIndex, const MVInfo *mvInfo) const {
  for (CollIndex i = 0; i < entries(); i++) {
    ...
    const MVColumnInfo *mvCol =
        mvInfo->getMVColumns().getMvColInfoByBaseColumn(predCol->getTableName(), predCol->getColNumber());
    // The predicate can be on a column not selected by the MV.
    if (mvCol == NULL) continue;

    // Make sure its not a duplicate or redundant column.
    if (mvCol->getColType() == COM_MVCOL_BASECOL) return mvCol->getColNumber();
  }
...
void MVVegPredicate::insert(const MVVegPredicateColumn *newCol) {
  if (usageType_ == COM_UNKNOWN_USAGE) usageType_ = newCol->getUsageType();
...
void MVVegPredicate::print(FILE *ofd, const char *indent, const char *title) const {
  ...
  if (usageType_ == COM_DIRECT_USAGE)
    ...
    fprintf(ofd, "(Exp) ");
  for (CollIndex i = 0; i < entries(); i++) at(i)->print(ofd, indent, title);
  fprintf(ofd, ".\n");
}

void MVVegPredicate::display() const { print(); }
#endif
...
//////////////////////////////////////////////////////////////////////////////
MVUsedObjectInfo::MVUsedObjectInfo(const Scan *scanNode, NABoolean markKeyCols, CollHeap *heap)
  ...
  if (tableDesc->getNATable()->isAnMV()) objectType_ = COM_MV_OBJECT;
...
//////////////////////////////////////////////////////////////////////////////
NABoolean MVUsedObjectInfo::processSelectionPredicates(const ItemExpr *expr, BindWA *bindWA, NABoolean &toBeRemoved) {
  if (expr == NULL) return FALSE;
  ...
  for (ValueId leaf = leafs.init(); leafs.next(leaf); leafs.advance(leaf)) {
    ...
    switch (leafExpr->getOperatorType()) {
      ...
      case ITM_VEG_PREDICATE: {
        ...
        if (processSelectionPredicates(vegMembers, bindWA)) return TRUE;
        if (vegMembers.entries() <= 1) toBeRemoved = TRUE;
        break;
      }
      ...
      default:
        ...
        if (isANonRepeatableExpression(leafExpr, bindWA)) return TRUE;
        break;
    }
    ...
    if (baseCol != NULL) {
      if (getObjectName() == baseCol->getTableDesc()->getCorrNameObj().getQualifiedNameObj())
...
// Mark columns used by the selection predicates as indirectUpdate columns.
//////////////////////////////////////////////////////////////////////////////
NABoolean MVUsedObjectInfo::processSelectionPredicates(ValueIdSet &expr, BindWA *bindWA) {
  // For each ValueId in the ValueIdSet
  for (ValueId vid = expr.init(); expr.next(vid); expr.advance(vid)) {
    NABoolean toBeRemoved = FALSE;
    if (processSelectionPredicates(vid.getItemExpr(), bindWA, toBeRemoved)) return TRUE;

    if (toBeRemoved) expr.remove(vid);
  }
...
void MVUsedObjectInfo::print(FILE *ofd,           // = stdout,
  ...
  switch (getCatmanFlags().getUsageType()) {
    case COM_USER_SPECIFIED:
      usageTypeText = "UserSpecified";
      break;
    case COM_DIRECT_USAGE:
      usageTypeText = "Direct";
      break;
    case COM_EXPANDED_USAGE:
      usageTypeText = "Expanded";
      break;
    default:
      usageTypeText = "Unknown Usage Type";
      break;
  }
  ...
  switch (objectType_) {
    ...
    case COM_USER_DEFINED_ROUTINE_OBJECT:
      ...
      break;
    default:
      objectTypeText = "Unknown Object Type";
      break;
  }

  fprintf(ofd, "\tTable no. %d: %s, %s, %s, %s\n\t  Used columns: ", getOrdinalNumber(),
          getObjectName().getQualifiedNameAsString().data(), isInner, usageTypeText, objectTypeText);
  for (CollIndex i = 0; i < usedColumnList_.entries(); i++) fprintf(ofd, " %d", usedColumnList_[i]);
  fprintf(ofd, "\n\t  Indirect Update columns: ");
  for (CollIndex j = 0; j < indirectUpdateCols_.entries(); j++) fprintf(ofd, " %d", indirectUpdateCols_[j]);
  fprintf(ofd, "\n\t  MV Cols Referencing The CI: ");
  for (CollIndex k = 0; k < MVColsReferencingTheCI_.entries(); k++) fprintf(ofd, " %d", MVColsReferencingTheCI_[k]);
  fprintf(ofd, ".\n");

  if (getSelectionPredicates() != "") fprintf(ofd, "\t  Selection Predicates: %s.\n", getSelectionPredicates().data());
}

void MVUsedObjectInfo::display() const { print(); }
#endif
...
//////////////////////////////////////////////////////////////////////////////
MVColumnInfo::MVColumnInfo(MVInfoForDDL &mvInfoObj, ComMVType mvType, const ColumnDesc *colDesc, BindWA *bindWA,
                           CollHeap *heap)
  ...
  if (!((expr->getOperatorType() == ITM_CAST) && (expr->child(0)->getOperatorType() == ITM_BASECOLUMN))) {
    // Then skip the Cast
    expr = SkipCast(expr);
    if (expr->getOperatorType() == ITM_INSTANTIATE_NULL) expr = expr->child(0);
  }

  operatorType_ = expr->getOperatorType();

  if (mvType == COM_MAV || mvType == COM_MAJV) expr = findMavColumnType(expr, mvInfoObj);
  ...
  if (isComplex_ && normalizedColText_ == "") calcNormalizedTextForComplexCols(mvInfoObj, directColValueId);
  ...
  if (moreThanOne) origCol = NULL;
  if (origCol != NULL) {
    origColNumber_ = origCol->getColNumber();
    origTableName_ = origCol->getTableDesc()->getCorrNameObj().getQualifiedNameObj();
  }
...
//////////////////////////////////////////////////////////////////////////////
MVColumnInfo::MVColumnInfo(const MVColumnInfo &other, MVInfoForDDL &mvInfoObj, MVAggFunc aggIndex, ItemExpr *colExpr,
                           CollHeap *heap)
    : colNumber_(mvInfoObj.getNextColIndex()),
      colName_("", heap),
  ...
  switch (aggIndex) {
    case AGG_COUNT:
      ...
      if (colDataType_->getTypeQualifier() != NA_NUMERIC_TYPE) {
        ...
        colDataType_ = new (heap) SQLLargeInt(heap, TRUE, FALSE);  // LARGEINT SIGNED NOT NULL.
      }
      break;
  ...
  if (unBoundText_ == NULL || !mvInfoObj.isMaskedTable(origTableName_))
...
//////////////////////////////////////////////////////////////////////////////
MVColumnInfo::MVColumnInfo(MVInfoForDDL &mvInfoObj, ItemExpr *groupingCol, CollHeap *heap)
    : colNumber_(mvInfoObj.getNextColIndex()),
      colName_("", heap),
  ...
  const ColRefName *viewColName = mvInfoObj.findViewNameFor(groupingCol->getValueId());
  if (viewColName == NULL) {
    ...
    if (groupingCol->getOperatorType() != ITM_BASECOLUMN) {
      ...
      isComplex_ = TRUE;
      while ((groupingCol != NULL) && (groupingCol->getOperatorType() != ITM_BASECOLUMN))
        groupingCol = groupingCol->child(0);

      // There must be at least one.
      CMPASSERT(groupingCol != NULL && groupingCol->getOperatorType() == ITM_BASECOLUMN);
    }
    ...
    if (groupingCol->getOperatorType() != ITM_BASECOLUMN) isComplex_ = TRUE;
  }
...
//////////////////////////////////////////////////////////////////////////////
MVColumnInfo::MVColumnInfo(MVInfoForDDL &mvInfoObj, NAColumn *clusteringKeyCol, CollHeap *heap)
    : colNumber_(mvInfoObj.getNextColIndex()),
      colName_("", heap),
  ...
  const ColRefName &viewCol = mvInfoObj.findTopViewOf(baseCol);
  ...
  if (!mvInfoObj.isLeftJoinInnerTable(origTableName_)) setNotNull(TRUE);
...
//////////////////////////////////////////////////////////////////////////////
MVColumnInfo::MVColumnInfo(MVInfoForDDL &mvInfoObj, CollHeap *heap)
    : colNumber_(mvInfoObj.getNextColIndex()),
      colName_("", heap),
      colDataType_(NULL),
      operatorType_(NO_OPERATOR_TYPE),
  ...
  colDataType_ = new (heap) SQLLargeInt(heap, TRUE, FALSE);  // LARGEINT SIGNED NO NULLS
...
ItemExpr *MVColumnInfo::findMavColumnType(ItemExpr *colExpr, MVInfoForDDL &mvInfoObj) {
  ...
  if (aggIndex_ != AGG_OTHER) {
    ...
    if (operatorType_ != ITM_STDDEV_SAMP &&
        operatorType_ != ITM_VARIANCE_SAMP) {  // It's either COUNT, SUM, AVG, MIN or MAX
    ...
    } else {                  // This is either STDDEV, STDDEVW, VARIANCE or VARIANCEW
    ...
    if (aggExpr != NULL) expr = aggExpr;  // The colExpr_ saved for STDDEVW is the aggregate.
    ...
    colType_ = COM_MVCOL_GROUPBY;
    if (expr->getOperatorType() == ITM_BASECOLUMN) operatorType_ = ITM_BASECOLUMN;
  }
...
NABoolean MVColumnInfo::isColumnExpressionComplex(ItemExpr *expr) {
  if ((colType_ == COM_MVCOL_GROUPBY) || (colType_ == COM_MVCOL_BASECOL)) {
    ...
    if ((aggIndex_ == AGG_OTHER) || (aggIndex_ == AGG_STDDEVW) || (aggIndex_ == AGG_VARIANCEW)) {
...
//////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////
void MVColumnInfo::calcNormalizedTextForComplexCols(MVInfoForDDL &mvInfoObj, ValueId directColValueId) {
...
//////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////
void MVColumnInfo::setColTypeAndExpr(ItemExpr *expr, ComMVType mvType, ValueId directColValueId, BindWA *bindWA) {
  if (mvType == COM_MJV) {
    aggIndex_ = AGG_OTHER;
  ...
  if (colExpr_ != NULL && isNonRepeatableExpressionUsed(bindWA)) colType_ = COM_MVCOL_OTHER;  // Set as Not incremental
}
...
Int32 MVColumnInfo::newMavDependentColumn(ExpressionHash &expHash, const NAString &exprTextForHash,
  ...
  if (aggArray == NULL || aggArray->at(depIndex) == NULL) {
    ...
    MVColumnInfo *newDep = new (heap) MVColumnInfo(*this, mvInfoObj, depIndex, depExpr, heap);
...
//////////////////////////////////////////////////////////////////////////////
void MVColumnInfo::createDependentColumns(ExpressionHash &expHash, MVInfoForDDL &mvInfoObj, CollHeap *heap) {
  // COUNT columns and non-incremental columns don't need dependent columns.
  if ((aggIndex_ == AGG_COUNT) || (aggIndex_ == AGG_COUNTSTAR) || (aggIndex_ == AGG_OTHER)) return;
  ...
  if ((aggIndex_ != AGG_STDDEVW) && (aggIndex_ != AGG_VARIANCEW)) {
    ...
    if (isNotNull()) {
      ...
      if ((expr->getOperatorType() == ITM_TIMES) && (expr->child(0)->getValueId() == expr->child(1)->getValueId())) {
      ...
      dep1_ = newMavDependentColumn(expHash, newExprText, mvInfoObj, AGG_COUNT, expr, heap);
    }

    // SUM, MIN and MAX only need the COUNT column.
    if ((aggIndex_ == AGG_SUM) || (aggIndex_ == AGG_MIN) || (aggIndex_ == AGG_MAX)) return;

    // AVG, STDDEV and VARIANCE have SUM(expr) as the second dep column.
    dep2_ = newMavDependentColumn(expHash, exprTextForHash_, mvInfoObj, AGG_SUM, getColExpr(), heap);
    // For AVG, COUNT and SUM are enough.
    if (aggIndex_ == AGG_AVG) return;

    // STDDEV and VARIANCE have SUM(expr*expr) as the third dep column.
    depIndex = AGG_SUM;
    ItemExpr *squareExpr = new (heap) BiArith(ITM_TIMES, getColExpr(), getColExpr());
    newExprText = "";
    squareExpr->unparse(newExprText, PARSER_PHASE, MV_SHOWDDL_FORMAT);
    dep3_ = newMavDependentColumn(expHash, newExprText, mvInfoObj, AGG_SUM, squareExpr, heap);
  } else {
...
Int32 MVColumnInfo::newStddevwDepColumn(Int32 childNo, ExpressionHash &expHash, const NAString &exprTextForHash,
  ...
  return newMavDependentColumn(expHash, newExprText, mvInfoObj, AGG_SUM, depExpr, heap);
}
...
void MVColumnInfo::print(FILE *ofd,           //  = stdout,
  ...
  switch (getColType()) {
    case COM_MVCOL_GROUPBY:
      colTypeText = "GroupBy";
      break;
    case COM_MVCOL_CONST:
      colTypeText = "Const";
      break;
    case COM_MVCOL_AGGREGATE:
      colTypeText = "Aggregate";
      break;
    case COM_MVCOL_OTHER:
      colTypeText = "Other";
      break;
    case COM_MVCOL_FUNCTION:
      colTypeText = "Function";
      break;
    case COM_MVCOL_BASECOL:
      colTypeText = "BaseCol";
      break;
    case COM_MVCOL_DUPLICATE:
      colTypeText = "Duplicate";
      break;
    case COM_MVCOL_REDUNDANT:
      colTypeText = "Redundant";
      break;
    case COM_MVCOL_COMPLEX:
      colTypeText = "Complex";
      break;
  ...
  switch (getOperatorType()) {
    case ITM_COUNT:
      opTypeText = "ITM_COUNT";
      break;
    case ITM_COUNT_NONULL:
      opTypeText = "ITM_COUNT_NONULL";
      break;
    case ITM_SUM:
      opTypeText = "ITM_SUM";
      break;
    ...
    case ITM_MAX:
      ...
      break;
    case ITM_STDDEV_SAMP:
      opTypeText = "ITM_STDDEV_SAMP";
      break;
    case ITM_VARIANCE_SAMP:
      opTypeText = "ITM_VARIANCE_SAMP";
      break;
    case ITM_BASECOLUMN:
      opTypeText = "ITM_BASECOLUMN";
      break;
    default:
      opTypeText = "Unknown";
      break;
  ...
  NAString origText = (origTableName_ == "" ? NAString("NULL") : origTableName_.getQualifiedNameAsString());

  fprintf(ofd, "%sCol %d, %s, colType %s, opType %s,\n", indent, getColNumber(), colName_.data(), colTypeText,
          opTypeText);
  fprintf(ofd, "%s    isSystem %s, dataType: %s, isNotNull: %s, isInMVCI: %s\n", indent, isSysText, dataTypeText.data(),
          isNullText, mvciText);
  fprintf(ofd, "%s    Deps %2d, %2d, %2d, %s\n", indent, dep1_, dep2_, dep3_, normalizedColText_.data());
  fprintf(ofd, "%s    Orig: %s, col %d, complex %s, normalizedText %s\n", indent, origText.data(), origColNumber_,
          compText, normalizedText);
  if (strcmp(title, "")) fprintf(ofd, "\n");
}  // MVColumnInfo::print()

void MVColumnInfo::display() const { print(); }
#endif
...
void MVColumns::insert(MVColumnInfo *colInfo, NABoolean isIncremental) {
  if ((directColumnList_.entries() == 0) && colInfo->getColNumber() != 0) {
  ...
  if (firstSysAddedColumn_ == -1 && colInfo->isSystem()) firstSysAddedColumn_ = colInfo->getColNumber();
  ...
  if (colInfo->getColName() != "") {
    MVColumnInfo *prevCol = columnInfoByNameHash_.getFirstValue(&colInfo->getColName());

    // Can't have two MV cols with the same name.
    // This error is caught later by the catman, so ignore it now.
    if (prevCol == NULL) columnInfoByNameHash_.insert(&colInfo->getColName(), colInfo);
  }

  // Update the base table/column name hash.
  if (colInfo->getBaseColHashKey() != "") {
    MVColumnInfoList *prevCols = columnInfoByBaseHash_.getFirstValue(&colInfo->getBaseColHashKey());
    ...
    if (prevCols == NULL) {
      ...
      if (colInfo->getColType() == COM_MVCOL_BASECOL) {
        ...
        if (firstCol->getColType() == COM_MVCOL_BASECOL) colInfo->setAsRedundant(firstCol->getColNumber());
      }
...
MVColumnInfo *MVColumns::getMvColInfoByBaseColumn(const QualifiedName &tableName, Lng32 position) const {
  ...
  for (CollIndex i = 0; i < colInfoList->entries(); i++) {
    ...
    if (colInfo->getColType() == COM_MVCOL_BASECOL) return colInfo;
  }

  colInfo = colInfoList->at(0);
  CMPASSERT(colInfo != NULL);

  if (colInfo->getColType() != COM_MVCOL_REDUNDANT) return colInfo;
...
void MVColumns::display() const { print(); }
#endif
...
//////////////////////////////////////////////////////////////////////////////
MVInfo::MVInfo(const NAString &nameOfMV, ComMVRefreshType refreshType, NABoolean isRewriteEnabled, ComMVStatus mvStatus,
               const NAString &mvSelectText, CharInfo::CharSet mvSelectTextCharSet, CollHeap *heap)
...
//////////////////////////////////////////////////////////////////////////////
MVInfo::MVInfo(const NAString &nameOfMV, ComMVRefreshType refreshType, NABoolean isRewriteEnabled, ComMVStatus mvStatus,
               const NAString &mvSelectText, CharInfo::CharSet mvSelectTextCharSet, NABoolean isIncremental,
               ComMVType mvType, LIST(MVUsedObjectInfo *) & usedObjectsList, MVColumnInfoList &directColumnList,
               NABoolean isLeftLinear, LIST(MVVegPredicate *) & eqPredicateList, CollHeap *heap)
    : nameOfMV_(nameOfMV, heap),
      refreshType_(refreshType),
      isRewriteEnabled_(isRewriteEnabled),
      mvStatus_(mvStatus),
...
MVInfo::MVInfo(const MVInfo &other, CollHeap *heap)
  ...
  for (CollIndex i = 0; i < other.usedObjectsList_.entries(); i++)
    usedObjectsList_.insert(new (heap_) MVUsedObjectInfo(*other.usedObjectsList_[i], heap_));

  for (CollIndex l = 0; l < other.eqPredicateList_.entries(); l++)
    eqPredicateList_.insert(new (heap_) MVVegPredicate(*other.eqPredicateList_[l], heap_));
}

//////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////
MVInfo::~MVInfo() { delete joinGraph_; }
...
void MVInfo::fixMvSelectList(RelRoot *mvSelectTree) const {
  ...
  if (selectExprList == NULL) {
    if (mvSelectTree->child(0)->getOperatorType() == REL_GROUPBY) {
      ...
      if (gb->getGroupExprTree()->getOperatorType() == ITM_REFERENCE) {
        ...
        if (groupExpr->getColRefNameObj().isStar() && gb->child(0)->getOperatorType() == REL_ROOT) {
          return;
  ...
  for (CollIndex selectListIndex = 0; selectListIndex < selectList.entries(); selectListIndex++, mvColIndex++) {
...
RelRoot *MVInfo::buildMVSelectTree(const NAString *alternativeText, CharInfo::CharSet textCharSet) const {
  const NAString &MVSelectText = (alternativeText != NULL ? *alternativeText : getMVSelectText());
  if (alternativeText == NULL) textCharSet = getMVSelectTextCharSet();
  ExprNode *parsedNode;

  // Parse the SQL text.
  Parser parser(CmpCommon::context());
  if (parser.parseDML(MVSelectText.data(), MVSelectText.length(), textCharSet, &parsedNode)) return NULL;

  // Skip the DDL nodes of the CREATE MV command.
  DDLExpr *parseTree = (DDLExpr *)((StmtNode *)parsedNode->castToStatementExpr())->getQueryExpression()->getChild(0);
  CMPASSERT(parseTree->getOperatorType() == REL_DDL);
  CMPASSERT(parseTree->getDDLNode()->getOperatorType() == DDL_CREATE_MV);
  StmtDDLCreateMV *createMvNode = (StmtDDLCreateMV *)parseTree->getDDLNode();
...
NABoolean MVInfo::isEqPredicateBetween(const QualifiedName &table1, Lng32 col1, const QualifiedName &table2,
  ...
  for (CollIndex i = 0; i < eqPredicateList_.entries(); i++) {
    ...
    if ((pred->findIndexFor(table1, col1) != -1) && (pred->findIndexFor(table2, col2) != -1)) return TRUE;
  }
  return FALSE;
}
...
MVUsedObjectInfo *MVInfo::findUsedInfoForTable(const QualifiedName &tableName) const {
  for (CollIndex i = 0; i < usedObjectsList_.entries(); i++)
    if (usedObjectsList_[i]->getObjectName() == tableName) return usedObjectsList_[i];
...
// at DDL time only.
//////////////////////////////////////////////////////////////////////////////
NABoolean MVInfo::addTableNamesToList(BindWA *bindWA, TableNameHash &usedObjectsNames) {
  ...
  for (CollIndex objIdx = 0; objIdx < usedObjects.entries(); objIdx++) {
    ...
    // Get the MVInfo of the used MV.
    MVInfoForDML *usedObjMVInfo = bindWA->getNATable(usedMVName)->getMVInfo(bindWA);
...
void MVInfo::print(FILE *ofd,           // = stdout,
  ...
  switch (refreshType_) {
    case COM_ON_STATEMENT:
      refreshTypeText = "on Statement";
      break;
    case COM_ON_REQUEST:
      refreshTypeText = "on Request";
      break;
    case COM_BY_USER:
      refreshTypeText = "by User";
      break;
    case COM_UNKNOWN_RTYPE:;
    default:
      refreshTypeText = "Unknown";
      break;
  }
  ...
  fprintf(ofd, "\n\nMV Name: %s,\n    Refresh %s, Rewrite %s, Initialized %s,\n", getNameOfMV().data(), refreshTypeText,
          rewriteText, initText);
...
void MVInfo::display() const { print(); }
#endif
...
NAString MVInfo::getIgnoreChangesListAsString() const {
  ...
  for (CollIndex ci = 0; ci < usedObjects.entries(); ci++) {
    ...
    if ((catmanFlags.getUsageType() == COM_USER_SPECIFIED) && (usedInfo->getObjectType() == COM_BASE_TABLE_OBJECT) &&
...
NAString MVInfo::getOptionalNameClause(StmtDDLCreateMV *createMvNode, const NAString &mvText,
  ...
  if (NA_NPOS == firstOpenPar || createMvNode->getEndOfOptionalColumnListPosition() < firstOpenPar) {
    return opNameClause;
  }
  ...
  // to where it ends.
  opNameClause = mvText;
  opNameClause = opNameClause.remove(createMvNode->getEndOfOptionalColumnListPosition() + 1);
  ...
  for (CollIndex ci(0); ci < mvCols.entries(); ci++) {
    ...
    if (!mvCol->isSystem()) {  // skip columns that are not system added.
      continue;
    }
  ...
  while (opNameClause[lastLetterPos] == ')' || opNameClause[lastLetterPos] == ' ' ||
...
//////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////
MVInfoForDDL::MVInfoForDDL(const NAString &nameOfMV, ComMVRefreshType refreshType, NABoolean isRewriteEnabled,
                           ComMVStatus mvStatus, const NAString &mvSelectText, CharInfo::CharSet mvSelectTextCharSet,
                           const ElemDDLStoreOptKeyColumnList *userSpecifiedCI, CollHeap *heap)
...
void MVInfoForDDL::addScanNode(Scan *newNode) {
  ...
  if (newNode->getTableDesc()->getNATable()->isAnMV()) usesOtherMVs_ = TRUE;

  // Is there a correlation name to this table?
  if (newNode->getTableName().getCorrNameAsString() == "") return;

  // Yes - treat it as a view.
  if (viewColumnGraph_ == NULL) viewColumnGraph_ = new (getHeap()) ViewColumnGraph(getHeap());
...
//////////////////////////////////////////////////////////////////////////////
void MVInfoForDDL::addJoinNode(Join *newNode) { joinNodes_.insert(newNode); }

//////////////////////////////////////////////////////////////////////////////
// Add a GroupBy node to the list.
// This is the source of the grouping columns information.
//////////////////////////////////////////////////////////////////////////////
void MVInfoForDDL::addGroupByNode(GroupByAgg *newNode) { groupByNodes_.insert(newNode); }
...
void MVInfoForDDL::addViewRenameNode(RenameTable *renameNode) {
  ...
  if (renameNode->child(0)->getOperatorType() != REL_ROOT) return;
  ...
  if (renameNode->isView()) {
    ...
    */
    ...
    MVUsedObjectInfo *usedObject = new (getHeap()) MVUsedObjectInfo(renameNode, getHeap());
...
LIST(MVColumnInfoList *) * MVInfoForDDL::getOptimalMJVIndexList(const LIST(Lng32) * mvCI) {
  ...
  for (ittr = 0; ittr < objNum; ittr++) {
    ...
    const NABoolean isNotInIgnoreChanges = isUsedObjectNotIgnoreChanges(currObject->getObjectName());
  ...
  LIST(MVColumnInfoList *) *outColumnInfoLists = new LIST(MVColumnInfoList *)(getHeap());
...
// of column numbers.
MVColumnInfoList *MVInfo::buildColumnInfoListFromColIndList(const ColIndList &currentRCI) const {
  CollIndex currentColNumber;
  MVColumnInfoList *currentColumnInfoList = new (getHeap()) MVColumnInfoList(getHeap());
  size_t currentRCIsize = currentRCI.entries();
  for (size_t i(0); i < currentRCIsize; i++) {
    currentColNumber = currentRCI.at(i);
    MVColumnInfo *currentColInfo = getMVColumns().getMvColInfoByIndex(currentColNumber);
...
//
//////////////////////////////////////////////////////////////////////////////
void MVInfoForDDL::setNotIncremental(const MV_NOT_INCREMENTAL notIncrementalReason) {
...
//////////////////////////////////////////////////////////////////////////////
void MVInfoForDDL::NewSystemColumName(const NAString &midText, NAString &colNameString) {
...
//////////////////////////////////////////////////////////////////////////////
void MVInfoForDDL::addSystemColumnText(const NAString &colName, const NAString &colExp) {
...
void MVInfoForDDL::initUsedObjectsList(BindWA *bindWA) {
  ...
  if (scanNodesNumber < 1) {
    ...
    *CmpCommon::diags() << DgSqlCode(-12018) << DgString0(reason);
    bindWA->setErrStatus();
    return;
  }
  ...
  for (CollIndex scanNodeIndex = 0; scanNodeIndex < scanNodesNumber; scanNodeIndex++) {
    ...
    if (usedInfo != NULL) getUsedObjectsList().insert(usedInfo);
...
//////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////
MVUsedObjectInfo *MVInfoForDDL::initUsedObject(Scan *scanNode, const NATable *naTable, BindWA *bindWA) {
  NABoolean isMjv = (getMVType() == COM_MJV || getMVType() == COM_MAJV);

  // Build a new used object.
  MVUsedObjectInfo *usedInfo = new (getHeap()) MVUsedObjectInfo(scanNode, isMjv, getHeap());

  if (isLeftJoinInnerTable(naTable->getTableName())) usedInfo->setAsInnerTable();
  ...
  if ((CmpCommon::getDefault(RANGESPEC_TRANSFORMATION) == DF_ON) && (selectionPreds.entries())) {
  ...
  if (viewColumnGraph_ != NULL && viewColumnGraph_->isMaskedTable(naTable->getTableName()))
...
NABoolean MVInfoForDDL::isLeftJoinInnerTable(const QualifiedName &tableName) const {
  for (CollIndex i = 0; i < leftJoinInnerTables_.entries(); i++)
    if (tableName == *(leftJoinInnerTables_[i])) return TRUE;
...
void MVInfoForDDL::initJoinPredicates(BindWA *bindWA) {
  ...
  for (CollIndex i = 0; i < joinNodes_.entries(); i++) {
    ...
    for (ValueId pred = predicates.init(); predicates.next(pred); predicates.advance(pred)) {
      MVVegPredicate *mvVegPred = buildNewVegPred(pred, vegPtrList, usageType, isLeftJoin);
      if (mvVegPred == NULL) continue;

      // Make sure we have at least two members in this MVVegPredicate.
      CMPASSERT(mvVegPred->entries() >= 2);

      if (!mvVegPred->isIncremental()) setNotIncremental(MVNI_MULTIPLE_COLUMN_JOIN_PREDICATE);
...
//////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////
MVVegPredicate *MVInfoForDDL::buildNewVegPred(ValueId predValueId, LIST(const VEG *) & vegPtrList,
                                              ComMVSUsageType usageType, NABoolean isLeftJoin) {
  ...
  switch (predType) {
    case ITM_VEG_PREDICATE: {
      ...
      if (vegPtrList.contains(veg)) break;
      vegPtrList.insert(veg);

      // Construct a new MVVegPredicate object.
      mvVegPred = new (getHeap()) MVVegPredicate(isLeftJoin, getHeap(), usageType);
    ...
    case ITM_EQUAL: {
      ...
      mvVegPred->addPredicateMember(predExpr->child(0)->getValueId(), FALSE, NULL, vegPtrList, getHeap());
      mvVegPred->addPredicateMember(predExpr->child(1)->getValueId(), FALSE, NULL, vegPtrList, getHeap());
      if ((mvVegPred->entries() <= 1) && (!mvVegPred->isIncremental())) {
        setNotIncremental();
        mvVegPred = NULL;
      }
    } break;
...
void MVInfoForDDL::addMavSystemColumns(BindWA *bindWA) {
  if (!isIncrementalWorkNeeded()) return;
  ...
  for (CollIndex i = 0; i < columnList.entries(); i++) {
    ...
    if (colInfo->isCountStarColumn())
      ...
      if (colInfo->isMinMaxColumn()) setMinMaxIsUsed();
  ...
  if (getPosOfCountStar() == -1) {
    MVColumnInfo *colInfo = new (getHeap()) MVColumnInfo(*this, getHeap());
  ...
  if (groupByNodes_.entries() == 1) {
    ...
    for (CollIndex j = 0; j < columnList.entries(); j++) {
      ...
      if (colInfo->getColType() == COM_MVCOL_GROUPBY) groupingCols.remove(colInfo->getColExpr()->getValueId());
    }

    // All the remaining columns on the list should be added as system cols.
    for (ValueId groupCol = groupingCols.init(); groupingCols.next(groupCol); groupingCols.advance(groupCol)) {
      // Create an MVColumnInfo object for the new system column.
      ItemExpr *colExpr = groupCol.getItemExpr();
      MVColumnInfo *colInfo = new (getHeap()) MVColumnInfo(*this, colExpr, getHeap());
...
void MVInfoForDDL::addRecomputeMVUSedObjectsCIColumns(BindWA *bindWA) {
  if (getRefreshType() != COM_RECOMPUTE) return;

  // Now see which clustering key columns of each table are in the select list
  // of the MV
  for (CollIndex scanNodeIndex = 0; scanNodeIndex < scanNodes_.entries(); scanNodeIndex++) {
...
//////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////
void MVInfoForDDL::addUsedObjectsClusteringInfo(TableDesc *tDesc, BindWA *bindWA) {
  const ValueIdList &clusteringKeyCols = tDesc->getClusteringIndex()->getIndexKey();

  // Find the usedInfo object for the current table.
  MVUsedObjectInfo *usedInfo = findUsedInfoForTable(tDesc->getCorrNameObj().getQualifiedNameObj());
  CMPASSERT(usedInfo != NULL);
  ...
  for (CollIndex ciColIndex = 0; ciColIndex < clusteringKeyCols.entries(); ciColIndex++) {
    ...
    if (existingMvColumn == NULL) {  // The clustering key column was NOT found in the used column list
...
//////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////
void MVInfoForDDL::addClusteringIndexAsSystemAdded(TableDesc *tDesc, BindWA *bindWA) {
  const ValueIdList &clusteringKeyCols = tDesc->getClusteringIndex()->getIndexKey();
  ...
  for (CollIndex ciColIndex = 0; ciColIndex < clusteringKeyCols.entries(); ciColIndex++) {
    ...
    const NABoolean isInsertOnlyTable = tDesc->getNATable()->getMvAttributeBitmap().getIsInsertLog();
...
void MVInfoForDDL::addBaseColsUsedByComputedMvColumns() {
  ...
  for (CollIndex mvCol = 0; mvCol < columnList.entries(); mvCol++) {
    ...
    if (colInfo->getColType() != COM_MVCOL_FUNCTION) continue;
    ...
    if (usedCols.entries() < 2) continue;
    ...
    for (CollIndex i = 0; i < usedCols.entries(); i++) {
      ...
      MVColumnInfo *MVBaseColInfo =
          getMVColumns().getMvColInfoByBaseColumn(*baseCol->getTableName(), baseCol->getPosition(), mvCol);

      // If so, don't add duplicate references.
      if (MVBaseColInfo != NULL) continue;

      MVColumnInfo *newColInfo = new (getHeap()) MVColumnInfo(colInfo->getColNumber(), *baseCol->getTableName(),
                                                              baseCol->getPosition(), COM_MVCOL_COMPLEX, getHeap());
...
MVColumnInfo *MVInfoForDDL::addNewMjvSystemColumnIfNeeded(BindWA *bindWA, NAColumn *keyCol, Lng32 MVColIndex) {
  ...
  if ((existingMvColumn == NULL) ||
      (existingMvColumn->getColType() !=
       COM_MVCOL_BASECOL)) {  // The clustering key column was NOT found in the used column list
    if (alternateCol != -1) {
      // The base column itself was not found in the MV select list,
      // but there is no need to add it because it is "represented" there
      // by some alternate column that is always equal to it because there
      ...
      colInfo = new (getHeap())
          MVColumnInfo(alternateCol, *keyCol->getTableName(), keyCol->getPosition(), COM_MVCOL_DUPLICATE, getHeap());
    } else {
      // An alternate column was not found, so we need a system added column.
      if (viewColumnGraph_ != NULL && viewColumnGraph_->isMaskedColumn(keyCol) && keyCol->getColName() != "SYSKEY") {
        ...
        *CmpCommon::diags() << DgSqlCode(-12302) << DgString0(colName);
        bindWA->setErrStatus();
        return NULL;
      }

      // Add a new system column.
      colInfo = new (getHeap()) MVColumnInfo(*this, keyCol, getHeap());
    }
  } else {  // The clustering key column WAS found in the used column list.
    // Does it also appear in an EQ join predicate with another MV col?
    if ((alternateCol != -1) && (alternateCol < MVColIndex)) {
...
NABoolean MVInfoForDDL::isColumnInMVCI(const NAString &colName) {
  if (userSpecifiedCI_ == NULL) return FALSE;

  for (CollIndex i = 0; i < userSpecifiedCI_->entries(); i++) {
    if (userSpecifiedCI_->at(i)->getColumnName() == colName) return TRUE;
  }
...
void MVInfoForDDL::processBoundInformation(BindWA *bindWA) {
  ...
  initMVType();
  if (getMVType() == COM_MV_OTHER) setNotIncremental();  // use default reason
  ...
  if (groupByNodes_.entries() > 1) setNotIncremental(MVNI_MULTI_GROUP_BY_CLAUSES);

  // Add the user columns to the column list.
  MVColumns &mvColumns = getMVColumns();
  for (CollIndex i = 0; i < colDescList_->entries(); i++) {
    MVColumnInfo *colInfo = new (getHeap()) MVColumnInfo(*this, getMVType(), colDescList_->at(i), bindWA, getHeap());

    if (isColumnInMVCI(colInfo->getColName())) colInfo->setInMVCI();
    ...
    if (colInfo->getColType() == COM_MVCOL_OTHER) setNotIncremental(MVNI_NON_REPEATABLE_EXPRESSION);

    // the column is set to COM_MVCOL_COMPLEX for a MAV
    // then an aggregate function is inside a complex expression.
    if (isIncrementalWorkNeeded() && (getMVType() == COM_MAV || getMVType() == COM_MAJV) &&
        colInfo->getColType() == COM_MVCOL_COMPLEX)
      setNotIncremental(MVNI_AGGREGATE_NOT_TOP_LEVEL);
  }

  if (isIncrementalWorkNeeded() && (getMVType() == COM_MAV || getMVType() == COM_MAJV)) addMavSystemColumns(bindWA);
}
...
void MVInfoForDDL::processNormalizedInformation(BindWA *bindWA) {
  ...
  if ((getMVType() == COM_MAV || getMVType() == COM_MAJV)) {
    ...
    if (!groupByNode->getSelectionPred().isEmpty() || groupByNode->getSelPredTree() != NULL)
  ...
  if (getMVType() == COM_MJV || getMVType() == COM_MAJV) {
    ...
    while (topJoin != NULL && topJoin->getOperatorType() != REL_SCAN && !topJoin->getOperator().match(REL_ANY_JOIN))
      topJoin = topJoin->child(0);

    // Check if the Join tree is left linear.
    isLeftLinear_ = isTreeLeftLinear(topJoin);
    if (!isLeftLinear_) setNotIncremental(MVNI_JOIN_TREE_NOT_LEFT_LINEAR);
  ...
  if (getMVType() == COM_MJV && isIncrementalWorkNeeded())
...
void MVInfoForDDL::addViewsToUsedObjectList() {
  ...
  if (usedViewsList_.entries() == 0) return;
...
void MVInfoForDDL::addUDFsToUsedObjectList() {
  ...
  for (ULng32 udfIndex = 0; udfIndex < numUdfs; udfIndex++) {
    ...
    MVUsedObjectInfo *usedObject = new (currentUDF->getHeapPtr()) MVUsedObjectInfo(currentUDF, getHeap());
...
void MVInfoForDDL::verifyGroupByColumns(GroupByAgg *groupByNode) {
  ...
  for (CollIndex i = 0; i < columnList.entries(); i++) {
    ...
    MVColumnInfo *colInfo = columnList[i];
    if (colInfo->getColType() != COM_MVCOL_GROUPBY) continue;

    // Verify that the grouping columns contain this column.
    ValueId vid = colInfo->getColExpr()->getValueId();
    if (!groupingCols.containsTheGivenValue(vid)) setNotIncremental(MVNI_COLUMN_NOT_IN_GROUPING_COLUMNS);
  }
}
...
void MVInfoForDDL::initMVType() {
  ...
  if (node->getOperator().match(REL_ANY_JOIN) || node->getOperatorType() == REL_SCAN)
    mvType = COM_MJV;
  else {
    if ((node->getOperatorType() != REL_GROUPBY) && (node->getOperatorType() != REL_AGGREGATE))
      mvType = COM_MV_OTHER;
    else {
      ...
      if (node->getOperator().match(REL_ANY_JOIN)) {
        if ((node->getOperatorType() == REL_LEFT_JOIN) || (node->getOperatorType() == REL_RIGHT_JOIN))
...
RelExpr *MVInfoForDDL::getChildBelowRootAndRenameNodes(RelExpr *topNode) {
  ...
  while ((node->getOperatorType() == REL_RENAME_TABLE) || (node->getOperatorType() == REL_ROOT) ||
...
void MVInfoForDDL::print(FILE *ofd,           // = stdout,
  ...
  for (CollIndex i = 0; i < unBoundColumnsText_.entries(); i++) {
    ...
    if (text != NULL) fprintf(ofd, "%s, ", text->data());
  }
  fprintf(ofd, "\n");
}  // MVInfoForDDL::print()
#endif
...
//////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////
MVInfoForDML::MVInfoForDML(const NAString &NameOfMV, ComMVRefreshType refreshType, NABoolean isRewriteEnabled,
                           ComMVStatus mvStatus, const NAString &MVSelectText, CharInfo::CharSet mvSelectTextCharSet,
                           NABoolean isIncremental, ComMVType mvType, LIST(MVUsedObjectInfo *) & usedObjectsList,
                           MVColumnInfoList &DirectColumnList, NABoolean isLeftLinear,
                           LIST(MVVegPredicate *) & eqPredicateList, CollHeap *heap)
    : MVInfo(NameOfMV, refreshType, isRewriteEnabled, mvStatus, MVSelectText, mvSelectTextCharSet, isIncremental,
             mvType, usedObjectsList, DirectColumnList, isLeftLinear, eqPredicateList, heap),
...
void MVInfoForDML::getMavGroupByColumns(LIST(Lng32) & gbColList) const {
  ...
  for (CollIndex i = 0; i < columnList.entries(); i++) {
    ...
    if (currentCol->getColType() == COM_MVCOL_GROUPBY) gbColList.insert(currentCol->getColNumber());
  }
}
...
//////////////////////////////////////////////////////////////////////////////
NABoolean MVInfoForDML::isMvOnSingleTable() { return getUsedObjectsList().entries() == 1; }
...
MVJoinGraph *MVInfoForDML::initJoinGraph(BindWA *bindWA, const DeltaDefinitionPtrList *deltaDefList,
  ...
  for (CollIndex usedObjectsIndex = 0; usedObjectsIndex < getUsedObjectsList().entries(); usedObjectsIndex++) {
    ...
    MVJoinTable *newNode = new (getHeap()) MVJoinTable(nodeNumber, usedObjectsIndex, directTables, naTable, getHeap());

    DeltaDefinition *deltaDef = deltaDefList->findEntryFor(usedInfo->getObjectName());
    if (deltaDef != NULL) {
      ...
      if (checkOnlyInserts) {
        ...
        if (deltaDef->isInsertOnly())
          newNode->setInsertOnlyDelta();
                 ...
                 ((deltaDef->getIudUpdatedRows() > 0) && (deltaDef->containsIndirectUpdateColumn(this))))
          newNode->setInsertOnlyDelta();
        ...
        if (deltaDef->isInsertOnly())
          newNode->setInsertOnlyDelta();
        else if ((deltaDef->getIudDeletedRows() == 0) && (deltaDef->getIudUpdatedRows() == 0))
          newNode->setInsertOnlyDelta();
        else
...
void MVInfoForDML::initUsedObjectsHash() {
  ...
  if (!usedObjectsHash_.isEmpty()) return;

  for (CollIndex i = 0; i < usedObjects.entries(); i++) {
    MVUsedObjectInfo *usedObj = usedObjects[i];

    // Ignore views.
    if (usedObj->getObjectType() == COM_VIEW_OBJECT) continue;
    ...
    CorrName *logName = ChangesTable::buildChangesTableCorrName(qualName, COMMV_IUD_LOG_SUFFIX,
                                                                ExtendedQualName::IUD_LOG_TABLE, getHeap());
...
QRMVDescriptorPtr MVInfo::prepareMvDescriptorTree(NABoolean isIncrementalMV, NAString &warningMessage,
  ...
  if (getMVType() == COM_MAV) bindWA.setExpandMvTree();  // Set the binder Expanded flag
...
MVUsedObjectColNameMap::~MVUsedObjectColNameMap() {
  ...
  if (colPositionTable_ != NULL) collHeap()->deallocateMemory(colPositionTable_);
}
...
void MVUsedObjectColNameMap::initColNameMap(const NATable *naTable, CollHeap *heap) {
  ...
  if (colPositionTable_ != NULL) return;
