...
///////////////////////////////////////////////////////////////////////////////
Scan *MjvBuilder::buildScanOfMJV(const CorrName &corrName, const NATable *baseNaTable) const {
...
//
///////////////////////////////////////////////////////////////////////////////
ItemExpr *MjvBuilder::buildEqualityPredOnClusteringIndex(const CorrName &tableName, const NATable *baseNaTable) const {
  ItemExpr *eqList = NULL;
  ItemExpr *eqExpr = NULL;

  // Finding out the CI columns of the subject table
  const NAColumnArray keyColumns = baseNaTable->getClusteringIndex()->getIndexKeyColumns();
  ...
  for (CollIndex i = 0; i < keyColumns.entries(); i++) {
    ...
    MVColumnInfo *mvCol = mvCols.getMvColInfoByBaseColumn(baseNaTable->getTableName(), keyColumns[i]->getPosition());

    // Build the column references for use in the equation predicate
    ColReference *baseColumn =
        new (heap_) ColReference(new (heap_) ColRefName(keyColumns[i]->getColName(), tableName, heap_));
    CMPASSERT(mvCol);
    ColReference *mvColumn =
        new (heap_) ColReference(new (heap_) ColRefName(mvCol->getColName(), getMvCorrName(), heap_));
...
//
///////////////////////////////////////////////////////////////////////////////
ItemExpr *MjvBuilder::buildUpdatePredicate(const CorrName &inputCorrName, const NATable *baseNaTable,
  ...
  for (CollIndex i = 0; i < SetOfAffectedColumns.entries(); i++) {
    ...
    const NAString &origColName = baseCols[affectedCol->getOrigColNumber()]->getColName();

    ItemExpr *exprTree = getExpressionTreeForColumn(affectedCol, origColName, inputCorrName);

    // Building the assignment predicate
    ColReference *mvColumn =
        new (heap_) ColReference(new (heap_) ColRefName(affectedCol->getColName(), getMvCorrName(), heap_));
...
//////////////////////////////////////////////////////////////////////////////
ItemExpr *MjvBuilder::getExpressionTreeForColumn(const MVColumnInfo *affectedCol, const NAString &colName,
  ...
  if (exprText.length() > 0) {
    ...
    exprTree = parser.getItemExprTree(exprText.data(), exprText.length());
    ...
    exprTree = new (heap_) ColReference(new (heap_) ColRefName(colName, inputCorrName, heap_));
  }
...
// the calling method.
//////////////////////////////////////////////////////////////////////////////
void MjvBuilder::setupBindContext(RelRoot *topNode, RelExpr *scanBlock, const QualifiedName *baseTableName) {
...
//////////////////////////////////////////////////////////////////////////////
SET(MVColumnInfo *)
MjvBuilder::collectAllAffectedColumns(const IntegerList *updatedCols, const QualifiedName &qualTableName) const {
...
//
///////////////////////////////////////////////////////////////////////////////
RelExpr *MjvImmediateBuilder::buildDeleteOrDirectUpdateBlock(const CorrName &corrName,
...
///////////////////////////////////////////////////////////////////////////////
GenericUpdate *MjvImmediateBuilder::buildGUNodeOfMJV(RelExpr *belowSubTree) const {
...
void MjvImmediateBuilder::avoidSecurityCheckInRefreshTree(RelExpr *refreshTree) const {
  if (refreshTree->getOperatorType() == REL_SCAN || refreshTree->getOperator().match(REL_ANY_GEN_UPDATE)) {
...
//
//////////////////////////////////////////////////////////////////////////////
RelExpr *MjvImmInsertBuilder::buildRefreshTree() {
  // Get the MJV tree
  ...
  setupBindContext(topNode, scanBlock, &((getIudNode()->getTableName()).getQualifiedNameObj()));
...
//////////////////////////////////////////////////////////////////////////////
GenericUpdate *MjvImmInsertBuilder::buildActualGUOfMJV(RelExpr *subTreeBelow) const {
...
//////////////////////////////////////////////////////////////////////////////
RelExpr *MjvImmInsertBuilder::buildScanBlock() const {
  if (optimized_) {
...
//
//////////////////////////////////////////////////////////////////////////////
RelExpr *MjvImmInsertBuilder::buildOptimizedScanBlock() const {
  CorrName newCorr(NEWCorr);

  // Finding out the list of columns in the subject table
  const NAColumnArray &colList = getIudNode()->getTableDesc()->getNATable()->getNAColumnArray();
  ...
  for (CollIndex i = 0; i < colList.entries(); i++) {
    ...
    ItemExpr *col = new (heap_) ColReference(new (heap_) ColRefName(colName, newCorr));
    ...
    RenameCol *renCol = new (heap_) RenameCol(col, new (heap_) ColRefName(colName, tableName));
  ...
  RenameTable *renameNode = new (heap_) RenameTable(optTuple, tableName.getExposedNameAsString(), renameList, heap_);
...
RelExpr *MjvImmDeleteBuilder::buildRefreshTree() {
...
//////////////////////////////////////////////////////////////////////////////
GenericUpdate *MjvImmDeleteBuilder::buildActualGUOfMJV(RelExpr *subTreeBelow) const {
...
//////////////////////////////////////////////////////////////////////////////
RelExpr *MjvImmDirectUpdateBuilder::buildRefreshTree() {
  const NATable *baseNaTable = getIudNode()->getTableDesc()->getNATable();
...
//////////////////////////////////////////////////////////////////////////////
GenericUpdate *MjvImmDirectUpdateBuilder::buildActualGUOfMJV(RelExpr *subTreeBelow) const {
  ...
  SET(MVColumnInfo *) SetOfAffectedColumns = collectAllAffectedColumns(updatedCols, qualTableName);

  const NATable *baseNaTable = getIudNode()->getTableDesc()->getNATable();
  ItemExpr *updatePredicate = buildUpdatePredicate(CorrName(NAString(NEWCorr)), baseNaTable, SetOfAffectedColumns);

  return new (heap_) Update(getMvCorrName(), NULL, REL_UNARY_UPDATE, subTreeBelow, updatePredicate);
}

//////////////////////////////////////////////////////////////////////////////
// Utility method to create an IntegerList of updated columns from the STOI.
//////////////////////////////////////////////////////////////////////////////
IntegerList *MjvImmDirectUpdateBuilder::StoiToIntegerList(SqlTableOpenInfo *stoi) const {
...
RelExpr *MjvOnRequestBuilder::buildRefreshTree() {
  ...
  if (needDeletionBlock && !needInsertionBlock) {
    ...
    if (insertionBlock == NULL) return NULL;

    // And the ordered union that connects them.
    Union *unionNode =
        new (heap_) Union(deletionBlock, insertionBlock, NULL, NULL, REL_UNION, CmpCommon::statementHeap(), TRUE);
...
NABoolean MjvOnRequestBuilder::isNeedDeletionBlockSpecific(DeltaDefinition *deltaDef) {
  if ((deltaDef->getIudDeletedRows() == 0) &&
      ((deltaDef->getIudUpdatedRows() == 0) || deltaDef->updateColumnsNotUsedByMV(getMvInfo())))
    return FALSE;
  else
    return TRUE;
}
...
NABoolean MjvOnRequestBuilder::isNeedDirectUpdateSpecific(DeltaDefinition *deltaDef) {
  return (deltaDef->getIudUpdatedRows() > 0 && deltaDef->containsDirectUpdateColumn(getMvInfo()));
}
...
NABoolean MjvOnRequestBuilder::isNeedInsertionBlockSpecific(DeltaDefinition *deltaDef) {
  if ((deltaDef->getIudInsertedRows() > 0) ||
      ((deltaDef->getIudUpdatedRows() > 0) && (deltaDef->containsIndirectUpdateColumn(getMvInfo()))) ||
...
RelExpr *MjvOnRequestBuilder::buildMjvDeletionBlockSpecific(DeltaDefinition *deltaDef) {
  ...
  RelExpr *leftSide = new (heap_) RenameTable(TRUE, scanLog, logCorrName);
...
//----------------------------------------------------------------------------
RelExpr *MjvOnRequestBuilder::buildMjvDeleteSubtree(LogsInfo *logsInfo, const CorrName &logCorrName) {
  const NATable *baseNaTable = logsInfo->getBaseNaTable();

  // Build scan node of the MJV
  Scan *scanNode = buildScanOfMJV(logCorrName, baseNaTable);

  GenericUpdate *guNode = new (heap_) Delete(getMvCorrName(), NULL, REL_UNARY_DELETE, scanNode);
...
//----------------------------------------------------------------------------
RelExpr *MjvOnRequestBuilder::buildMjvUpdateSubtree(LogsInfo *logsInfo, const CorrName &logCorrName) {
  const NATable *baseNaTable = logsInfo->getBaseNaTable();
  ...
  ItemExpr *updatePredicate = buildUpdatePredicate(logCorrName, baseNaTable, SetOfAffectedColumns);

  GenericUpdate *guNode = new (heap_) Update(getMvCorrName(), NULL, REL_UNARY_UPDATE, scanNode, updatePredicate);
...
RelExpr *MjvOnRequestBuilder::buildIfNode(RelExpr *leftSide, RelExpr *rightSide) {
  ItemExpr *condition = BinderUtils::buildPredOnCol(ITM_NOT_EQUAL,
  ...
  RelExpr *ifNode =
      new (heap_) Union(leftSide, rightSide, NULL, condition, REL_UNION, CmpCommon::statementHeap(), TRUE);
...
RelExpr *MjvOnRequestBuilder::buildMjvInsertionBlock() {
  ...
  if (joinExpression == NULL) return NULL;

  // Put Insert node on top of it.
  GenericUpdate *guNode = new (heap_) Insert(getMvCorrName(), NULL, REL_UNARY_INSERT, joinExpression);
...
//----------------------------------------------------------------------------
RelExpr *MjvOnRequestBuilder::buildScanLogForOnRequestMjv(const QualifiedName &baseTableName, NABoolean isForDelete) {
  ...
  RelRoot *rootNode = new (heap_) RelRoot(scanLogBlock, REL_ROOT, selectList);
...
ItemExpr *MjvOnRequestBuilder::buildDeleteSidePredicate() {
  ...
  ItemExpr *type4 = new (heap_)
      BiLogic(ITM_AND, buildPredicateOnOpType(ComMvRowType_InsertOfUpdate), buildPredicateOnIndirect(ITM_RETURN_FALSE));

  ItemExpr *type6 = new (heap_)
      BiLogic(ITM_AND, buildPredicateOnOpType(ComMvRowType_DeleteOfUpdate), buildPredicateOnIndirect(ITM_RETURN_TRUE));

  return new (heap_) BiLogic(ITM_OR, type2, new (heap_) BiLogic(ITM_OR, type4, type6));
}
...
ItemExpr *MjvOnRequestBuilder::buildInsertSidePredicate() {
  ...
  ItemExpr *type7 = new (heap_)
      BiLogic(ITM_AND, buildPredicateOnOpType(ComMvRowType_InsertOfUpdate), buildPredicateOnIndirect(ITM_RETURN_TRUE));

  return new (heap_) BiLogic(ITM_OR, type1, type7);
}
...
ItemExpr *MjvOnRequestBuilder::buildPredicateOnOpType(ComMvIudLogRowType opType) {
  ItemExpr *opTypeCol = new (heap_) ColReference(new (heap_) ColRefName(COMMV_OPTYPE_COL));

  return new (heap_) BiRelat(ITM_EQUAL, opTypeCol, new (heap_) SystemLiteral(opType));
}
...
ItemExpr *MjvOnRequestBuilder::buildPredicateOnIndirect(OperatorTypeEnum val) {
  ItemExpr *indirectCol = new (heap_) ColReference(new (heap_) ColRefName(getVirtualIndirectColumnName()));

  return new (heap_) BiRelat(ITM_EQUAL, indirectCol, new (heap_) BoolVal(val));
}
...
ItemExpr *MjvOnRequestBuilder::buildSelectionListForScanOnIudLog() const {
  ...
  RenameCol *indirectCol =
      new (heap_) RenameCol(indirectExpr, new (heap_) ColRefName(getVirtualIndirectColumnName(), logName));
...
RelRoot *MjvOnRequestBuilder::buildRootOverIUDLog(RelExpr *topNode) const {
  ItemExpr *OperationTypeCol =
      new (heap_) ColReference(new (heap_) ColRefName(COMMV_OPTYPE_COL, getLogsInfo().getIudLogTableName(), heap_));

  ItemExpr *IndirectCol = new (heap_)
      ColReference(new (heap_) ColRefName(getVirtualIndirectColumnName(), getLogsInfo().getIudLogTableName(), heap_));
...
RelRoot *MjvOnRequestBuilder::buildRootOverRangeBlock(RelExpr *topNode) const {
  ItemExpr *OperationTypeCol =
      new (heap_) RenameCol(new (heap_) SystemLiteral(ComMvRowType_Insert), new (heap_) ColRefName(COMMV_OPTYPE_COL));

  ItemExpr *IndirectCol = new (heap_) RenameCol(new (heap_) SystemLiteral(),  // NULL
...
//----------------------------------------------------------------------------
// Exclude from coverge testing - used only for range logging
RelRoot *MjvOnRequestBuilder::buildRootWithUniformSelectList(RelExpr *topNode, ItemExpr *opExpr,
                                                             const CorrName *nameOverride) const {
  ItemExpr *rootSelectList = BinderUtils::buildClusteringIndexVector(
      getLogsInfo().getBaseNaTable(), heap_, nameOverride, SP_ALL_COLUMNS | SP_SYSKEY_AS_USER | SP_USE_AT_SYSKEY);
...
NABoolean MjvOnRequestMultiDeltaBuilder::isNeedDeletionBlock() {
  ...
  for (CollIndex i = 0; i < deltaDefList->entries(); i++) {
    ...
    if (isNeedDeletionBlockSpecific(deltaDef)) return TRUE;
  }
...
NABoolean MjvOnRequestMultiDeltaBuilder::isNeedInsertionBlock() {
  ...
  for (CollIndex i = 0; i < deltaDefList->entries(); i++) {
    DeltaDefinition *deltaDef = (*getDeltaDefList())[i];
    if (isNeedInsertionBlockSpecific(deltaDef)) return TRUE;
  }
...
RelExpr *MjvOnRequestMultiDeltaBuilder::buildMjvDeletionBlock() {
  ...
  for (CollIndex i = 0; i < deltaDefList->entries(); i++) {
    ...
    if (!isNeedDeletionBlockSpecific(deltaDef)) continue;
    ...
    if (topNode == NULL)
      ...
      Union *unionNode = new (heap_) Union(topNode, newBlock, NULL, NULL, REL_UNION, CmpCommon::statementHeap(), TRUE);
      unionNode->setOrderedUnion();
      topNode = unionNode;
    }
  }
...
//----------------------------------------------------------------------------
RelExpr *MjvOnRequestMultiDeltaBuilder::buildDeltaCalculationTree(const MultiDeltaRefreshMatrix &productMatrix) {
  ...
  for (int rowNumber = firstRowForThisPhase; rowNumber < firstRowForThisPhase + numOfRowsForThisPhase; rowNumber++) {
    ...
    if (topNode == NULL) {
      ...
      if (productMatrix.getRow(rowNumber)->isSignPlus()) {
        ...
        Join *joinNode = new (heap_) Join(topNode, rightProduct, REL_ANTI_SEMITSJ, joinPredicate);

        // Now add a root node with a selection predicate: (@LEFT.*)
        ItemExpr *selectionPredicate = new (heap_) ColReference(new (heap_) ColRefName(1, leftCorrName));

        topNode = new (heap_) RelRoot(joinNode, REL_ROOT, selectionPredicate);
      }
    }
  }
  ...
  if (productMatrix.isDuplicatesOptimized()) {
    ...
    if (topNode->getOperatorType() == REL_UNION) {
      ...
      topNode = new (heap_) RelRoot(new (heap_) GroupByAgg(
          new (heap_) RelRoot(topUnion), REL_GROUPBY, new (heap_) ColReference(new (heap_) ColRefName(TRUE, heap_))));
    }
  }
...
//----------------------------------------------------------------------------
ItemExpr *MjvOnRequestMultiDeltaBuilder::buildAntiSemiJoinPredicate(NAString rightName, NAString leftName)

{
  ...
  const NAColumnArray &indexColumns = bindWA_->getNATable(mvName)->getClusteringIndex()->getIndexKeyColumns();
  ...
  for (CollIndex i = 0; i < indexColumns.entries(); i++) {
    ...
    ItemExpr *rightColExpr = new (heap_) ColReference(new (heap_) ColRefName(colName, *rightCorrName, heap_));

    ItemExpr *leftColExpr = new (heap_) ColReference(new (heap_) ColRefName(colName, *leftCorrName, heap_));

    ItemExpr *equalExpr = new (heap_) BiRelat(ITM_EQUAL, leftColExpr, rightColExpr);
