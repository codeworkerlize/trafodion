		     ...
		     ValueIdSet & setOfPredicates)
{
  ...
  if (isPredFalse)
    workSpace->computeBeginKeyAndEndKeyValues(keyColumns,
					      ...
					      numLeadingKeyColsWithEqualPreds_
					      );
...
void SearchKeyWorkSpace::computeBeginKeyAndEndKeyValues
			     ...
			     NABoolean &allChosenPredsAreEqualPreds,
                             Int32& numLeadingKeyColsWithEquiPreds)
{
        ...
        if (convCheck)
          {
            ...
            if ((srcType.isVaryingLen()) &&
                (srcType.getNominalSize() == 0))
              return FALSE;
...
void SearchKey::getRowidPredicates()
{
  ...
  for (ValueId pred = commonPreds.init();
         commonPreds.next(pred);
         commonPreds.advance(pred))
  {
...
HivePartitionAndBucketKey::HivePartitionAndBucketKey(TableDesc *tDesc)
     : hdfsTableStats_(
          const_cast<HHDFSTableStats*>(
                (tDesc->getNATable()->getClusteringIndex()->getHHDFSTableStats())
                      )),
       selectedPartitions_(&(((HHDFSTableStats *) hdfsTableStats_)->partitionStatsList_),
                           STMTHEAP),
      ...
      if (nac->isHivePartColumn())
        {
          ...
          hiveBasePartCols_.insert(tDesc->getColumnList()[colNum]);
        }
     ...
     HHDFSStatsBase &result)
{
  for (CollIndex i=0;
       ...
       i++)
    {
      HHDFSPartitionStats *lps = selectedPartitions_.element(i);
  ...
  if (!selectionPredicates.isEmpty())
    {
      ...
      for (ValueId p=partAndVirtColPreds_.init();
           ...
           partAndVirtColPreds_.advance(p))
        {
          if (p.getItemExpr()->getOperatorType() == ITM_VEG_PREDICATE)
            {
              VEG *veg =
                static_cast<VEGPredicate *>(p.getItemExpr())->getVEG();
              ...
              // Also remove VEGPredicates that are interpreted as
              // "<col> IS NOT NULL". Right now the costing code is
              // not handling such predicates with a selectivity close
              // to 1 well, which could cause us to choose a nested
              // join when we don't have an efficient partition
              // elimination predicate on the inner table.
	      // The second TRUE below implies lookBeneathVEG. This
	      // occurs for example when we have left join. In such cases
	      // we shall be satisfied with runtime partition Elimination.
	      // When getACnstant is called a few lines below, second 
	      // argument is always FALSE. If it is set to TRUE, then we
	      // are not able to evalue a pred with VEG during compile time
	      // I suspect we will need to rewrite the text of such a
	      // predicate, which I am not attempting.
              if (veg->getAConstant(TRUE, TRUE) == NULL_VALUE_ID &&
                  !(ga->getCharacteristicInputs().contains(vegRef)))
                partAndVirtColPreds_ -= p;
            }
        }
      ...
      for (ValueId q=compileTimePartColPreds_.init();
		...
		getVEG()->getAConstant(FALSE, TRUE) == NULL_VALUE_ID)
...
int HivePartitionAndBucketKey::computeActivePartitions()
{
  ...
  // wheap will be destructed when this function ends, deleting
  // all objects created during the computation. 
  NAHeap wheap ("Compile Time Eval Heap",
      ...
      if (!compileTimePartColPreds_.isEmpty())
        {
          ...
          ComDiagsArea *da = ComDiagsArea::allocate(&wheap);
          ...
          if (!hivePartColList_.isEmpty())
            {
              ...
              if ( hdfsTableStats_->getBinaryPartColValues(p) == NULL ) {

                 // Need to make a copy of the colValuesList and assign new VIds
                 // from wheap for the copy so that in evalAtCompileTime()
                 // call, any alterations of the item expressions (such as done 
                 // in BiArith::preCodeGen()) will not affect colValuesList.
                 // This is important because later on wheap will be destroied
                 // after exit this call. We do not want colValuesList to contain
                 // any references to objects (such as ValueDesc or DomainDesc 
                 // associated with any newly created ValueIds) on wheap.
                 ValueIdList colValuesListCopy;
   
                 NAHeap* workingHeap = NULL;
   
                 if ( !colValuesListCopy.clone(colValuesList, &wheap) ) {
                    colValuesListCopy = colValuesList;
                    workingHeap = NULL;
                 } else
                    workingHeap = &wheap; 
   
                 colValuesListCopy.evalAtCompileTime(
                      1,
                ...
                hdfsTableStats_->setBinaryPartColValues(p, buf, bufLen);

              } else {
                // copy the buffered binary key to buf
                 memcpy(buf, hdfsTableStats_->getBinaryPartColValues(p), bufLen);
              }
...
NABoolean
SearchKey::areKeyValuesEqualTo(const SearchKey* other, CollIndex n) const
{
   CMPASSERT(n>=1 && n<=getBeginKeyValues().entries());
   CMPASSERT(n>=1 && n<=(other->getBeginKeyValues()).entries());
   ...
   for (CollIndex i = 0; i<n ; i++)
   {
     ...
     // Both keys should be the same.
     if ( areVidsIdentical(vid1, vid3) == FALSE  ||
          areVidsIdentical(vid2, vid4) == FALSE)
         return FALSE;
   }
...
{
  if (getHDFSTableStats()->isOrcFile() || getHDFSTableStats()->isParquetFile()) 
...
void HivePartitionAndBucketKey::makeHiveExtPushdownPredicates(
        const ValueIdSet & localPreds,
        const ValueIdSet & externalInputs,
        const IndexDesc *indexDesc,
        ValueIdSet &producedPredicates,
        ValueIdSet &usedPredicates,
        BindWA *bindWA)
{
  // predicates on partition and virtual columns cannot be pushed
  // down, since these columns aren't available to the ORC scanner

  // there may be some X <> c predicates; these can't be pushed down,
  // but the equivalent X < c OR X > c can be, so rewrite such predicates 

  ValueIdSet orcColLocalPreds = localPreds.rewriteNotEquals(bindWA);
  ...
  {
    ItemExpr* ie = e.getItemExpr()->copyTreeNonLeaves(STMTHEAP);
...
{
  const SUBARRAY(HHDFSPartitionStats *)& selPartitions = getMask();
  return selPartitions.entries();
}

void HivePartitionAndBucketKey::computeAvgAccessMetrics(FileScan* fileScan)
{
  Int32 numSelectedParts = MAXOF(getNumOfSelectedPartitions(), 1);
  ...
  NABoolean displayCostInfo = 
       SimpleFileScanOptimizer::decideDebugModeForORCCosting((IndexDesc *)fileScan->getIndexDesc())
        ||
       SimpleFileScanOptimizer::decideDebugModeForParquetCosting((IndexDesc *)fileScan->getIndexDesc());
...
void HivePartitionAndBucketKey::estimateAccessMetrics(FileScan* fileScan)
{
  ...
  if ( partitionEliminatedCTOnly() ) {
    ...
    // we can do, return the tuple processed value for PE(CT). 
     ...
     estRCInPartnsSelected_ = fileScan->getIndexDesc()->getPrimaryTableDesc()
                                      ->getNATable()->getEstRowCount();


  // PE(CT) is not feasible, but PE(RT) is.
  combinedPreds += getPartAndVirtColPreds();
       
  // if there are no predicates, bail out
  if ( combinedPreds.entries() == 0 )
    return;
