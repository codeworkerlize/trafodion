...
void SearchKey::init(const ValueIdList &keyColumns, const ValueIdList &orderOfKeyValues,
                     const ValueIdSet &externalInputs, const NABoolean forwardSearch, ValueIdSet &setOfPredicates) {
  // ---------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------
  SearchKeyWorkSpace *workSpace =
      new (CmpCommon::statementHeap()) SearchKeyWorkSpace(keyColumns, orderOfKeyValues, forwardSearch, *this);
  ...
  for (ValueId predId = setOfPredicates.init(); setOfPredicates.next(predId); setOfPredicates.advance(predId)) {
  ...
  numLeadingKeyColsWithEqualPreds_ = 0;

  if (isPredFalse)
    workSpace->computeBeginKeyAndEndKeyValues(
        keyColumns, externalInputs, endKeyValues_, beginKeyValues_, keyPredicates_, setOfNonKeyPredicates,
        keyPredicatesUnique, endKeyIsExclusive_, beginKeyIsExclusive_, endKeyExclusionExpr_, beginKeyExclusionExpr_,
        allBeginKeysMissing_, allEndKeysMissing_, allChosenPredsAreEqualPreds_, numLeadingKeyColsWithEqualPreds_);

  else
    workSpace->computeBeginKeyAndEndKeyValues(
        keyColumns, externalInputs, beginKeyValues_, endKeyValues_, keyPredicates_, setOfNonKeyPredicates,
        keyPredicatesUnique, beginKeyIsExclusive_, endKeyIsExclusive_, beginKeyExclusionExpr_, endKeyExclusionExpr_,
        allBeginKeysMissing_, allEndKeysMissing_, allChosenPredsAreEqualPreds_, numLeadingKeyColsWithEqualPreds_);
  ...
  // SQ2535.
  // ---------------------------------------------------------------------
  if (keyPredicates_.referencesBignumNumericDataType() == FALSE || keyPredicates_.entries() > 1) {
  ...
  // ---------------------------------------------------------------------
  NABoolean coveredColumnIsUnique = FALSE;
  computeCoveredLeadingKeys(coveredColumnIsUnique);

  // TRAFODION.NANRUI.A_RCVBL_FLOW_COVER1

  // isUnique is defined as either all predicates are equi or
  // one of the covered key columns is unique.
  // Currently, when isUnqiue is set to TRUE, a ROWSET select
  // is created instead of a subset scan. The ROWSET select
  // uses get HBase' get(List(GET)) to batch fetch a list of records
  // at a time. Until HBase 2.0, get(List(GET)) does not handle
  // a rowId key in the form of a partial key. Therefore the CQD
  // UNIQUE_COLUMN_OPT will be turned off until HBase 2.0 is supported
  // by EsgynDB.
  if (CmpCommon::getDefault(GEN_UNIQUE_COLUMN_OPT) == DF_ON) {
    isUnique_ |= coveredColumnIsUnique;
  }
...
//
// -----------------------------------------------------------------------
SearchKey::SearchKey(const ValueIdList &keyColumns, const ValueIdList &orderOfKeyValues,
                     const ValueIdSet &externalInputs, const NABoolean forwardSearch, ValueIdSet &setOfPredicates,
                     const ValueIdSet &nonKeyColumnSet, const IndexDesc *indexDesc, const IndexDesc *UDindexDesc)
    : ScanKey(keyColumns, externalInputs, *(new (CmpCommon::statementHeap()) MaterialDisjuncts(setOfPredicates)),
  ...
  init(keyColumns, orderOfKeyValues, externalInputs, forwardSearch, setOfPredicates);

}  // SearchKey::SearchKey()

SearchKey::SearchKey(const ValueIdList &keyColumns, const ValueIdList &orderOfKeyValues,
                     const ValueIdSet &externalInputs, const NABoolean forwardSearch, ValueIdSet &setOfPredicates,
                     const Disjuncts &associatedDisjuncts, const ValueIdSet &nonKeyColumnSet,
                     const IndexDesc *indexDesc, const IndexDesc *UDindexDesc)
    : ScanKey(keyColumns, externalInputs, associatedDisjuncts, nonKeyColumnSet, indexDesc, UDindexDesc),
      beginKeyValues_(keyColumns.entries()),
      endKeyValues_(keyColumns.entries()),
      valuesArePartitionRange_(FALSE),
      _countTimesBoundaryValReq(0) {
...
SearchKey::SearchKey(const ValueIdList &keyColumns, const ValueIdList &orderOfKeyValues,
                     const ValueIdSet &externalInputs, const NABoolean forwardSearch, ValueIdSet &setOfPredicates,
                     const RangePartitioningFunction *rpf, const ValueIdSet &nonKeyColumnSet,
                     const IndexDesc *indexDesc, const IndexDesc *UDindexDesc)
    : ScanKey(keyColumns, externalInputs, *(new (CmpCommon::statementHeap()) MaterialDisjuncts(setOfPredicates)),
              nonKeyColumnSet, indexDesc, UDindexDesc),
      valuesArePartitionRange_(FALSE),
  ...
  while (equivCols AND commonCols < rpf->getKeyColumnList().entries() AND commonCols < keyColumns.entries()) {
    ItemExpr *kc = keyColumns[commonCols].getItemExpr();
    ItemExpr *rc = rpf->getKeyColumnList()[commonCols].getItemExpr();

    if (kc != rc AND kc->getOperatorType() == ITM_INDEXCOLUMN) kc = ((IndexColumn *)kc)->getDefinition().getItemExpr();
    if (kc != rc) {
      if (rc->getOperatorType() == ITM_VEG_REFERENCE) {
        if (NOT(((VEGReference *)rc)->getVEG()->getAllValues().contains(kc->getValueId()))) equivCols = FALSE;
      } else
        equivCols = FALSE;
    }
    ...
    if (equivCols) {
      NABoolean colIsInv = (orderOfKeyValues[commonCols].getItemExpr()->getOperatorType() == ITM_INVERSE);
      NABoolean pfIsInv = (rpf->getOrderOfKeyValues()[commonCols].getItemExpr()->getOperatorType() == ITM_INVERSE);
      if (commonCols == 0)
        sameScan = (colIsInv == pfIsInv);
      else if (sameScan != (colIsInv == pfIsInv))
        equivCols = FALSE;
    }

    // try next column if we are still in the game
    if (equivCols) commonCols++;
  }
  ...
  if (commonCols >= 1) {
    ...
    for (CollIndex i = 0; i < keyColumns.entries(); i++) {
      if (i < commonCols) {
        ...
        if (forwardSearch == (orderOfKeyValues[i].getItemExpr()->getOperatorType() != ITM_INVERSE)) {
          // forward scan of ASC col or reverse scan of DESC col
          beginKeyValues_.insert(minValue);
          minMax = new (CmpCommon::statementHeap()) Case(
              NULL, new (CmpCommon::statementHeap()) IfThenElse(
                        new (CmpCommon::statementHeap()) BiRelat(ITM_NOT_EQUAL, pivs[2 * numPartKeyCols].getItemExpr(),
                                                                 new (CmpCommon::statementHeap()) SystemLiteral(0)),
                        minValue.getItemExpr(), maxValue.getItemExpr()));
          ...
          minMax = new (CmpCommon::statementHeap()) Case(
              NULL, new (CmpCommon::statementHeap()) IfThenElse(
                        new (CmpCommon::statementHeap()) BiRelat(ITM_NOT_EQUAL, pivs[2 * numPartKeyCols].getItemExpr(),
                                                                 new (CmpCommon::statementHeap()) SystemLiteral(0)),
                        maxValue.getItemExpr(), minValue.getItemExpr()));
          minMax->synthTypeAndValueId();
          endKeyValues_.insert(minMax->getValueId());
        }
...
// -----------------------------------------------------------------------
SearchKey::SearchKey(const ValueIdList &keyColumns, const ValueIdList &orderOfKeyValues,
                     const ValueIdSet &externalInputs, ValueIdSet &setOfPredicates,
                     const TableHashPartitioningFunction *hpf, const ValueIdSet &nonKeyColumnSet,
                     const IndexDesc *indexDesc, const IndexDesc *UDindexDesc)
    : ScanKey(keyColumns, externalInputs, *(new (CmpCommon::statementHeap()) MaterialDisjuncts(setOfPredicates)),
              nonKeyColumnSet, indexDesc, UDindexDesc),
      valuesArePartitionRange_(FALSE),
  ...
  while (equivCols AND colNum < numKeyCols) {
    ...
    if (keyCol != hashCol) {
      if (hashCol->getOperatorType() == ITM_VEG_REFERENCE) {
        if (NOT(((VEGReference *)hashCol)->getVEG()->getAllValues().contains(keyCol->getValueId()))) equivCols = FALSE;
      } else {
        equivCols = FALSE;
      }
    }
  ...
  if (equivCols) {
    ...
    // the usual way
    init(keyColumns, orderOfKeyValues, externalInputs, TRUE, setOfPredicates);
  }
}  // SearchKey::SearchKey()
...
SearchKey::SearchKey(const ValueIdList &keyColumns, const ValueIdList &orderOfKeyValues,
                     const ValueIdSet &externalInputs, ValueIdSet &setOfPredicates,
                     const RoundRobinPartitioningFunction *rrpf, const ValueIdSet &nonKeyColumnSet,
                     const IndexDesc *indexDesc, const IndexDesc *UDindexDesc)
    : ScanKey(keyColumns, externalInputs, *(new (CmpCommon::statementHeap()) MaterialDisjuncts(setOfPredicates)),
              nonKeyColumnSet, indexDesc, UDindexDesc),
      valuesArePartitionRange_(FALSE),
  ...
  if (equivCols) {
    ...
    if (keyCol != rrCol) {
      if (rrCol->getOperatorType() == ITM_VEG_REFERENCE) {
        if (NOT(((VEGReference *)rrCol)->getVEG()->getAllValues().contains(keyCol->getValueId()))) equivCols = FALSE;
      } else {
        equivCols = FALSE;
      }
    }
...
ItemExpr *SearchKey::getBeginKeyExclusionExpr() const {
  if (beginKeyExclusionExpr_ == NULL_VALUE_ID) return NULL;
  return beginKeyExclusionExpr_.getItemExpr();
}

ItemExpr *SearchKey::getEndKeyExclusionExpr() const {
  if (endKeyExclusionExpr_ == NULL_VALUE_ID) return NULL;
...
// -----------------------------------------------------------------------
ValueId SearchKey::computeMissingKeyValue(ValueId keyColumn, NABoolean wantMinValue) {
  const NAType &type = keyColumn.getItemExpr()->getValueId().getType();
  NAColumn *column = keyColumn.getItemExpr()->getValueId().getNAColumn();

  ItemExpr *minMaxConstant = new (CmpCommon::statementHeap())
      SystemLiteral(type.newCopy(CmpCommon::statementHeap()), wantMinValue, type.supportsSQLnull());
...
void SearchKey::display() const { SearchKey::print(); }  // SearchKey::display()
...
NABoolean SearchKeyBounds::operator==(const SearchKeyBounds &other) const {
  if ((getKeyColumnId() == other.getKeyColumnId()) AND(isAscendingOrder() == other.isAscendingOrder())
          AND(isForwardSearch() == other.isForwardSearch()) AND(getBeginKeyBoundType() == other.getBeginKeyBoundType())
              AND(getBeginKeyPredicate() == other.getBeginKeyPredicate())
                  AND(getBeginKeyValue() == other.getBeginKeyValue())
                      AND(getEndKeyBoundType() == other.getEndKeyBoundType())
                          AND(getEndKeyPredicate() == other.getEndKeyPredicate())
                              AND(getEndKeyValue() == other.getEndKeyValue()))
    return TRUE;
  else
    return FALSE;
...
//
// -----------------------------------------------------------------------
void SearchKeyBounds::analyzeSearchPredicates(const ValueIdSet &setOfPredicates, const ValueIdSet &externalInputs) {
  // ---------------------------------------------------------------------
  ...
  switch (colIEPtr->getOperatorType()) {
    ...
    case ITM_INDEXCOLUMN: {
      ...
      // -------------------------------------------------
      BaseColumn *bcIEPtrForIndexColumn = (BaseColumn *)((IndexColumn *)colIEPtr)->getDefinition().getItemExpr();
  ...
  for (ValueId predId = setOfPredicates.init(); setOfPredicates.next(predId); setOfPredicates.advance(predId)) {
    ...
    if (getSearchKey().isAKeyPredicateForColumn(predId, referencedInput, intervalExclusionExpr, columnId)) {
      // yes!
      // -------------------------------------------------------------
      ...
      if (referencedInput == NULL_VALUE_ID) {
        ItemExpr *nullConstant = new (CmpCommon::statementHeap()) SystemLiteral();
        nullConstant->synthTypeAndValueId();
        referencedInput = nullConstant->getValueId();
      }

      applyABoundForTheSearch(predId, referencedInput, intervalExclusionExpr);
    }
...
// -----------------------------------------------------------------------
void SearchKeyBounds::applyABoundForTheSearch(const ValueId &predId, const ValueId &referencedInput,
  ...
  switch (boundType) {
    case BOUND_TYPE_LOWER: {
      if (isBeginKeyChosen()) {
        ...
        if (rangePred && rangePred->isPreferredForSubsetScanKey() && getBeginKeyBoundType() != BOUND_TYPE_EQUAL) {
          // If a predicate has been marked as a preference for subset
    ...
    case BOUND_TYPE_UPPER: {
      if (isEndKeyChosen()) {
        ...
        // if a predicate has been marked as a preference for subset
        // begin/end key use that predicate to define the begin/end key
        if (rangePred && rangePred->isPreferredForSubsetScanKey() && getEndKeyBoundType() != BOUND_TYPE_EQUAL) {
          // If a predicate has been marked as a preference for subset
          // begin/end key use the bounds implied by that predicate.
          // -----------------------------------------------------------
          // Establish a new lower bound.
...
// -----------------------------------------------------------------------
SearchKeyBounds::BoundType SearchKeyBounds::getBoundType(const ValueId &predId, const ValueId &referencedInput) const {
  ...
  if (NOT isAscendingOrder()) invertBoundType = NOT invertBoundType;

  // also invert it if the expression is "expr op col" intead of "col op expr"
  if (ie->getArity() > 1 AND ie->child(1)->getValueId() != referencedInput) invertBoundType = NOT invertBoundType;
...
// -----------------------------------------------------------------------
void SearchKeyBounds::computeBeginKeyAndEndKeyValues(ValueIdSet &setOfKeyPredicates, ValueIdSet &setOfNonKeyPredicates,
                                                     NABoolean &previousPredsAreEqualPreds,
                                                     NABoolean &beginKeyIsExclusive, NABoolean &endKeyIsExclusive,
                                                     ValueId &beginKeyExclusionExpr, ValueId &endKeyExclusionExpr,
                                                     NABoolean &beginKeyMissing, NABoolean &endKeyMissing,
  ...
  if (isBeginKeyPredicateChosen()) {
    ...
    if (getBeginKeyBoundType() != BOUND_TYPE_EQUAL) allChosenPredsAreEqualPreds = FALSE;
  }
  if (isEndKeyPredicateChosen()) {
    setOfKeyPredicates += getEndKeyPredicate();
    endKeyMissing = FALSE;

    if (getEndKeyBoundType() != BOUND_TYPE_EQUAL) allChosenPredsAreEqualPreds = FALSE;
  }
  ...
  if ((getKeyColumnId().getType().supportsSQLnull())OR  //## SQLnullLogical()?
    ...
    if (isBeginKeyPredicateChosen()) setOfNonKeyPredicates += getBeginKeyPredicate();
    if (isEndKeyPredicateChosen()) setOfNonKeyPredicates += getEndKeyPredicate();
  }
  ...
  // ---------------------------------------------------------------------
  if (previousPredsAreEqualPreds AND getBeginKeyBoundType() != BOUND_TYPE_EQUAL) {
...
void SearchKeyBounds::computeBeginKeyValue(NABoolean prevBoundWasExclusive) {
  if (NOT isBeginKeyValueChosen()) {  // case:  missing key predicate
    //    Index order : ASC   direction of search : ASC
    // OR Index order : DESC  direction of search : DESC
    if ((isAscendingOrder() AND isForwardSearch())OR((NOT isAscendingOrder())AND(NOT isForwardSearch()))) {
    ...
    else if ((isAscendingOrder() AND(NOT isForwardSearch()))OR((NOT isAscendingOrder())AND isForwardSearch())) {
...
void SearchKeyBounds::computeEndKeyValue(NABoolean prevBoundWasExclusive) {
  if (NOT isEndKeyValueChosen()) {  // case:  missing key predicate
    //    Index order : ASC   direction of search : ASC
    // OR Index order : DESC  direction of search : DESC
    if ((isAscendingOrder() AND isForwardSearch())OR((NOT isAscendingOrder())AND(NOT isForwardSearch()))) {
      // Get the maximum value (Cases marked by the $ above)
...
ValueId SearchKeyBounds::computeMissingKeyValue(NABoolean wantMinValue) {
  ...
  minMaxConstant = new (CmpCommon::statementHeap())
      SystemLiteral(type.newCopy(CmpCommon::statementHeap()), wantMinValue, type.supportsSQLnull());
...
// To be called from the debugger.
void SearchKeyBounds::display() const { SearchKeyBounds::print(); }  // SearchKeyBounds::display()
...
// Method for allocating the work space.
// -----------------------------------------------------------------------
SearchKeyWorkSpace::SearchKeyWorkSpace(const ValueIdList &keyColumns, const ValueIdList &indexOrder,
                                       const NABoolean forwardSearch, const SearchKey &searchKey)
    : keyCount_(keyColumns.entries()), keyBounds_(CmpCommon::statementHeap()), searchKey_(searchKey) {
  ...
  for (CollIndex iter = 0; iter < keyColumns.entries(); iter++) {
    if (indexOrder.entries() >= iter AND indexOrder[iter].getItemExpr()->getOperatorType() == ITM_INVERSE)
      ascDescFlag = FALSE;
    else
      ascDescFlag = TRUE;
    keyBounds_.insertAt(iter, new (CmpCommon::statementHeap())
                                  SearchKeyBounds(keyColumns[iter], ascDescFlag, forwardSearch, getSearchKey()));
  }
...
//
// -----------------------------------------------------------------------
void SearchKeyWorkSpace::analyzeSearchPredicates(const ValueIdSet &setOfPredicates, const ValueIdSet &externalInputs) {
  for (CollIndex keyNum = 0; keyNum < (CollIndex)keyCount_; keyNum++) {
    ...
    keyBounds_[keyNum]->analyzeSearchPredicates(setOfPredicates, externalInputs);
    ...
    if (keyBounds_[keyNum]->isBeginValueExclusive() || keyBounds_[keyNum]->isEndValueExclusive()) {
      break;
    }
...
void SearchKeyWorkSpace::computeBeginKeyAndEndKeyValues(
    const ValueIdList &keyColumns, const ValueIdSet &externalInputs, ValueIdList &lowerBoundValues,
  ...
  numLeadingKeyColsWithEquiPreds = 0;

  NABoolean nonEquiPredicatesSeen = FALSE;
  ...
  for (CollIndex keyNum = 0; keyNum < (CollIndex)keyCount_; keyNum++) {
    ...
    keyBound->computeBeginKeyAndEndKeyValues(
        setOfKeyPredicates, setOfNonKeyPredicates, keyPredicatesUnique, beginKeyIsExclusive, endKeyIsExclusive,
        beginKeyExclusionExpr, endKeyExclusionExpr, beginKeyMissing, endKeyMissing, allChosenPredsAreEqualPreds);
    ...
    // of this value
    if (lowerBoundValues.entries() < (keyNum + 1)) lowerBoundValues.insertAt(keyNum, keyBound->getBeginKeyValue());
    ...
    if (keyBound->getBeginKeyPredicate() != NULL_VALUE_ID) handleMultiColumnRangePred(TRUE, keyNum, lowerBoundValues);

    // if the number of entries in the upper bound is less than the
    // position of the key column we are currently iterating over.
    // This basically makes sure that no other value has taken place
    // of this value
    if (upperBoundValues.entries() < (keyNum + 1)) upperBoundValues.insertAt(keyNum, keyBound->getEndKeyValue());
    ...
    if (keyBound->getEndKeyPredicate() != NULL_VALUE_ID) handleMultiColumnRangePred(FALSE, keyNum, upperBoundValues);

    if (NOT beginKeyMissing) allBeginKeysMissing = FALSE;
    if (NOT endKeyMissing) allEndKeysMissing = FALSE;

    // check if there is an equal predicate defined on the
    // current key column. If so and there is no previous
    // key column with non-equi predicate, increment
    // the counter numLeadingKeyColsWIthEquiPreds.
    if (!nonEquiPredicatesSeen) {
      NAColumn *nc = keyBound->getNAColumnForKeyColumn();

      if (nc && !(nc->isComputedColumnAlways())) {
        if (keyBound->isEqualPredicate())
          numLeadingKeyColsWithEquiPreds++;
        else
          nonEquiPredicatesSeen = TRUE;
      }
    }
  ...
  getSearchKey().replicateNonKeyVEGPredicates(setOfKeyPredicates, setOfNonKeyPredicates);

  if ((allChosenPredsAreEqualPreds) && (allBeginKeysMissing) && (allEndKeysMissing))
    allChosenPredsAreEqualPreds = FALSE;
}  // SearchKeyWorkSpace::computeBeginKeyAndEndKeyValues()

NABoolean SearchKeyBounds::isEqualPredicate() {
  return isBeginKeyChosen() && (getBeginKeyBoundType() == BOUND_TYPE_EQUAL) && isEndKeyChosen() &&
         (getEndKeyBoundType() == BOUND_TYPE_EQUAL);
}

NAColumn *SearchKeyBounds::getNAColumnForKeyColumn() {
  ItemExpr *ie = getKeyColumnId().getItemExpr();

  switch (ie->getOperatorType()) {
    case ITM_BASECOLUMN: {
      BaseColumn *bc = (BaseColumn *)ie;
      return bc->getNAColumn();
    } break;

    case ITM_INDEXCOLUMN: {
      BaseColumn *bc = (BaseColumn *)(((IndexColumn *)ie)->getDefinition().getItemExpr());
      return bc->getNAColumn();
    } break;
...
NABoolean SearchKey::isNarrowNeeded() const {
  for (CollIndex i = 0; i < beginKeyValues_.entries(); i++) {
    ...
    if (keyValVID.getType().errorsCanOccur(keyColVID.getType())) return TRUE;
  }
...
NABoolean SearchKey::areAllKeysConstants(NABoolean convCheck) const {
  ...
  for (CollIndex i = 0; i < beginKeyValues_.entries(); i++)
    for (int k = 0; k <= 1; k++) {
      ItemExpr *key = (k == 0 ? beginKeyValues_[i].getItemExpr() : endKeyValues_[i].getItemExpr());
      ...
      switch (oper) {
        case ITM_VEG_REFERENCE: {
          ...
          // is reused.
          ValueId v = (static_cast<VEGReference *>(key))->getVEG()->getAConstant(/*TRUE*/);
          if (v == NULL_VALUE_ID)
            return FALSE;
          else
      ...
      if (convCheck) {
        ...
        // if 0-byte varchar const, return. This case is not handled
        // correctly during codegen and runtime.
        if ((srcType.isVaryingLen()) && (srcType.getNominalSize() == 0)) return FALSE;

        if (srcType.errorsCanOccur(tgtType, FALSE)) return FALSE;
      }
...
void SearchKey::getKeyPredicatesByColumn(ColumnOrderList &keyPredsByCol, CollIndex disjunctNumber) const {
  ...
  for (; order < keyPredsByCol.entries(); order++) {
    ...
    if (keyPredsByCol.getPredicateExpressionPtr(order)->getType() ==
        ColumnOrderList::KeyColumn::CONFLICT OR keyPredsByCol.getPredicateExpressionPtr(order)->getType() ==
    ...
    if (keyPredsByCol.getPredicateExpressionPtr(order)->getType() !=
        ColumnOrderList::KeyColumn::EQUAL AND keyPredsByCol.getPredicateExpressionPtr(order)->getType() !=
        ColumnOrderList::KeyColumn::CONFLICT_EQUALS) {  // i.e A > 4
  ...
  if (moreColumnPositions) {
    ...
    if (keyPredsByCol.getPredicateExpressionPtr(order)->getType() ==
        ColumnOrderList::KeyColumn::RANGE) {  // A=10 AND B > 10 and B < :hv
...
void SearchKey::getKeyPredicates(ValueIdSet &keyPredicates, /* out */
                                 NABoolean *allKeyPredicates, CollIndex disjunctNumber) const {
...
void SearchKey::getRowidPredicates() {
  ValueIdSet commonPreds(getDisjuncts().getCommonPredicates());

  for (ValueId pred = commonPreds.init(); commonPreds.next(pred); commonPreds.advance(pred)) {
    if (pred.isAnValidRowidPredicate()) {
      ItemExpr *iePtr = pred.getItemExpr();

      if (iePtr->child(0)->castToItemExpr()->getOperatorType() == ITM_HBASE_ROWID) {
        rowidCols_.insert(iePtr->child(0)->castToItemExpr()->getValueId());
        rowidValues_.insert(iePtr->child(1)->castToItemExpr()->getValueId());
      } else {
        rowidCols_.insert(iePtr->child(1)->castToItemExpr()->getValueId());
        rowidValues_.insert(iePtr->child(0)->castToItemExpr()->getValueId());
      }
      rowidPredicate_.insert(pred);
    }
  }

  // only support one ROWID predicate for now
  if (rowidPredicate_.entries() > 1) {
    rowidPredicate_.clear();
    rowidCols_.clear();
    rowidValues_.clear();
  }
...
void SearchKey::preCodeGen(ValueIdSet &executorPredicates, const ValueIdSet &selectionPredicates,
                           const ValueIdSet &availableValues, const ValueIdSet &inputValues,
                           VEGRewritePairs *vegPairsPtr, NABoolean replicateExpression, NABoolean partKeyPredsAdded) {
  // -----------------------------------------------------------------------
...
void SearchKey::replaceBegEndPivs(ValueIdSet &oldPivs, ValueIdSet &newPivs) {
...
///////////////////////////////////////////////////////////////////
static NABoolean bothAreIdenticalConstValues(const ValueId &vid1, const ValueId vid2) {
  // Constant values may have different ValueId. Check their text form.
  if (vid1.getItemExpr()->getOperatorType() != ITM_CONSTANT || vid2.getItemExpr()->getOperatorType() != ITM_CONSTANT)
    return FALSE;
...
///////////////////////////////////////////////////////////////////
static NABoolean areSubsetVegReferences(const ValueId &vid1, const ValueId vid2) {
  if (vid1.getItemExpr()->getOperatorType() == ITM_VEG_REFERENCE &&
      vid2.getItemExpr()->getOperatorType() == ITM_VEG_REFERENCE) {
    const ValueIdSet &set1 = ((VEGReference *)(vid1.getItemExpr()))->getVEG()->getAllValues();

    if (set1.contains(vid2)) return TRUE;

    const ValueIdSet &set2 = ((VEGReference *)(vid2.getItemExpr()))->getVEG()->getAllValues();
...
//
///////////////////////////////////////////////////////////////////
static NABoolean containCommonConstantExpr(const ValueId &vid1, const ValueId vid2) {
  if (vid1.getItemExpr()->getOperatorType() == ITM_VEG_REFERENCE &&
      vid2.getItemExpr()->getOperatorType() == ITM_VEG_REFERENCE) {
    const ValueIdSet &set1 = ((VEGReference *)(vid1.getItemExpr()))->getVEG()->getAllValues();
    ...
    if (constExpr1.isEmpty()) return FALSE;

    const ValueIdSet &set2 = ((VEGReference *)(vid2.getItemExpr()))->getVEG()->getAllValues();
    ValueIdSet constExpr2;
    set2.getConstantExprs(constExpr2);

    if (constExpr2.isEmpty()) return FALSE;
...
NABoolean areVidsIdentical(const ValueId &x, const ValueId &y) {
  return (x == y OR bothAreIdenticalConstValues(x, y) OR areSubsetVegReferences(x, y)
                   OR containCommonConstantExpr(x, y));
}

NABoolean SearchKey::areKeyValuesIdentical(const SearchKey *other, CollIndex n) const {
  ...
  for (CollIndex i = 0; i < n; i++) {
    ...
    if (areVidsIdentical(vid1, vid3) == FALSE) return FALSE;
  }
...
void SearchKeyWorkSpace::handleMultiColumnRangePred(NABoolean isBeginKey, CollIndex keyNum,
  ...
  if (keyPredId != NULL_VALUE_ID) {
    ...
    if (keyItemExpr->isARangePredicate()) {
      ...
      if (keyPred->isDerivedFromMCRP()) {
        ...
        for (CollIndex remainingKeyCols = keyNum + 1; remainingKeyCols < (CollIndex)keyCount_; remainingKeyCols++) {
          ...
          if (listOfComparisons.entries() == predLocation) return;
          ...
          if (isBeginKey) {
            if ((comparisonToCheck == ITM_LESS) || (comparisonToCheck == ITM_LESS_EQ)) return;
          } else {
            if ((comparisonToCheck == ITM_GREATER) || (comparisonToCheck == ITM_GREATER_EQ)) return;
          }
          // End match
          ...
          if (predColExpr->getOperatorType() == ITM_VEG_REFERENCE) {
            ...
            if (predColVEG->getVEG()->getAllValues().contains(keyCol)) predContainsKeyCol = TRUE;
          } else {
            // predicate column is not a VEG, then it should be
            // the key column, otherwise no match
            if (keyCol == predCol) predContainsKeyCol = TRUE;
          }
          ...
          if (predContainsKeyCol) {
            ...
            if (literalExpr->getOperatorType() == ITM_VEG_REFERENCE) {
              ...
              ValueId keyLiteral = literalVEG->getVEG()->getAConstantHostVarOrParameter();

              if (keyLiteral == NULL_VALUE_ID) return;
            } else {
              // literal is not a VEG, then it must be a constant expression
              if (!literalExpr->doesExprEvaluateToConstant(FALSE, TRUE)) return;
            }
...
HbaseSearchKey::HbaseSearchKey(const ValueIdList &keyColumns, const ValueIdList &orderOfKeyValues,
                               const ValueIdSet &externalInputs, const NABoolean forwardSearch,
                               ValueIdSet &setOfPredicates,  // the disjunct predicate
                               const ValueIdSet &nonKeyColumnSet, const IndexDesc *indexDesc,
                               const IndexDesc *UDIndexDesc, const ValueIdSet &outputsByHbaseScan)
    : SearchKey(keyColumns, orderOfKeyValues, externalInputs, forwardSearch, setOfPredicates, nonKeyColumnSet,
                indexDesc, UDIndexDesc),
...
void SearchKey::computeCoveredLeadingKeys(NABoolean &isCoveredColumnUnique) {
  ...
  */
  ...
  for (int k = 0; k < columnsToTry; k++) {
    ...
    if (naColumn->isUnique()) {
      isCoveredColumnUnique = TRUE;
      break;
    }
  }
}
...
void SearchKey::reportKeyAssignment(NAString &tname) {
  cout << "\nSearch key assignment for " << tname.data() << ":" << endl;
  ...
  for (int k = 0; k < columnsToTry; k++) {
    ...
    if (beginKeyValues[k] == endKeyValues[k])
      cout << "unique" << endl;
    else {
      ItemExpr *b = beginKeyValues[k].getItemExpr();
      ItemExpr *e = endKeyValues[k].getItemExpr();
      cout << "[" << b->getText().data() << ", " << e->getText().data() << "]" << endl;
    }
  }
}
...
// ---------------------------------------------------------------------------
void HbaseSearchKey::makeHBaseSearchKeys(const SearchKey *builtinSearchKey, const ValueIdList &keyColumns,
        ...
        if (rangeCol->containsTheGivenValue(indexDesc->getIndexKey()[k])) {
          ...
          if (rangeBackbonesForKeyCols.used(k)) {
            ...
            ItemExprList *rangeBB = new (CmpCommon::statementHeap())
                ItemExprList(destObj->getRangeItemExpr(), CmpCommon::statementHeap(), ITM_OR, FALSE, FALSE);
  ...
  if (rangeBackbonesForKeyCols.entries() > 0) {
    ...
    for (CollIndex s = 0; s < predListForSearchKeys.entries(); s++) {
      ...
      HbaseSearchKey *searchKeyPtr = new (CmpCommon::statementHeap())
          HbaseSearchKey(keyColumns, orderOfKeyValues, tempExternalInputs, forwardSearch, predListForSearchKeys[s],
                         nonKeyColumnSet, builtinSearchKey ? builtinSearchKey->getIndexDesc() : indexDesc,
                         builtinSearchKey ? builtinSearchKey->getUDIndexDesc() : NULL, outputsByHbaseScan);
      if (searchKeyPtr->areAllKeysConstants(TRUE)) {
        ...
        if (!found) producedKeys.insert(searchKeyPtr);
    ...
    if (producedKeys.entries() > 0) {
      ...
      if (allKeyPreds.contains(rangePreds)) setOfPredicates -= rangeSpecs;
    }
  }  // found RangeSpecs
  ...
  if (producedKeys.isEmpty()) {
    ...
    HbaseSearchKey *searchKeyPtr = new (CmpCommon::statementHeap())
        HbaseSearchKey(keyColumns, orderOfKeyValues, tempExternalInputs, forwardSearch, setOfPredicates,
                       nonKeyColumnSet, builtinSearchKey ? builtinSearchKey->getIndexDesc() : indexDesc,
                       builtinSearchKey ? builtinSearchKey->getUDIndexDesc() : NULL, outputsByHbaseScan);
    if (searchKeyPtr->areAllKeysConstants(TRUE)) producedKeys.insert(searchKeyPtr);
  }
}
...
NABoolean HbaseSearchKey::isEqualTo(const HbaseSearchKey *other) const {
  if (isUnique() != other->isUnique()) return FALSE;

  if (getCoveredLeadingKeys() != other->getCoveredLeadingKeys()) return FALSE;

  // assume #beginKeyValues == #endKeyValues
  if (getBeginKeyValues().entries() != other->getBeginKeyValues().entries()) return FALSE;

  if (!(areKeyValuesIdentical(other, getBeginKeyValues().entries()) ||
        areKeyValuesEqualTo(other, getBeginKeyValues().entries())))
    return FALSE;

  if (isBeginKeyExclusive() != other->isBeginKeyExclusive() || isEndKeyExclusive() != other->isEndKeyExclusive())
    return FALSE;

  return TRUE;
}

NABoolean SearchKey::areKeyValuesEqualTo(const SearchKey *other, CollIndex n) const {
  CMPASSERT(n >= 1 && n <= getBeginKeyValues().entries());
  CMPASSERT(n >= 1 && n <= (other->getBeginKeyValues()).entries());
  ...
  for (CollIndex i = 0; i < n; i++) {
    ...
    // Both keys should be the same.
    if (areVidsIdentical(vid1, vid3) == FALSE || areVidsIdentical(vid2, vid4) == FALSE) return FALSE;
  }
