...
void displaySet(const ValueIdSet &vs) { vs.display(); }

void displayList(const ValueIdList &vl) { vl.display(); }
...
ValueDesc *ValueId::getValueDesc() const {
  ...
  if (vdesc == NULL) CMPASSERT(vdesc);
  return vdesc;
}

ItemExpr *ValueId::getItemExpr() const { return getValueDesc()->getItemExpr(); }
...
const NAType &ValueId::getType() const { return getValueDesc()->getDomainDesc()->getType(); }

// return TRUE iff i'm a string literal with unknown char set
NABoolean ValueId::inferableCharType() {
  if (getType().getTypeQualifier() != NA_CHARACTER_TYPE || getItemExpr()->getOperatorType() != ITM_CONSTANT) {
    return FALSE;
  } else {
...
void ValueId::changeType(const NAType *type) { getValueDesc()->getDomainDesc()->changeType(type); }
...
NABoolean ValueId::isInvolvedInJoinAndConst() const {
  ...
  if (!tempSet.referencesAConstExpr()) return FALSE;
...
ColAnalysis *ValueId::baseColAnalysis(NABoolean *isaConstant, ValueId &vid) const {
...
// should be inlined
PredAnalysis *ValueId::predAnalysis() const { return QueryAnalysis::Instance()->getPredAnalysis(id_); }
////////////////////////////////////////////

void ValueId::coerceType(enum NABuiltInTypeEnum desiredQualifier, enum CharInfo::CharSet charSet,
                         NABoolean bForce /*= FALSE*/, DefaultConstants defaultConst
                         /*=__INVALID_DEFAULT_ATTRIBUTE*/) {
  //
  ...
  // charset-fix
  NABoolean bSame = TRUE;
  if (originalType.getTypeQualifier() == NA_CHARACTER_TYPE) {
    if (((CharType &)originalType).getCharSet() != charSet) bSame = FALSE;
  } else if (originalType.getTypeQualifier() == NA_UNKNOWN_TYPE)
    bSame = FALSE;
  bForce = (bForce && !bSame);
  // charset-fix
  if (!bForce && (originalType.getTypeQualifier() != NA_UNKNOWN_TYPE || desiredQualifier == NA_UNKNOWN_TYPE)) return;
  // If charset is unknown then take the default charset
  if (charSet == CharInfo::UnknownCharSet) charSet = CharInfo::DefaultCharSet;
  //
  ...
  switch (desiredQualifier) {
    ...
    case NA_CHARACTER_TYPE: {
      ...
      if (defaultConst == __INVALID_DEFAULT_ATTRIBUTE)
        ...
        len = CmpCommon::getDefaultNumeric(defaultConst);

      if ((CmpCommon::getDefault(MODE_COMPATIBLE_1) == DF_ON) && (ITM_CONSTANT == getItemExpr()->getOperatorType()))
        if (((ConstValue *)getItemExpr())->isNull()) len = 0;  // set NULL's length to 0
      desiredType =
          new STMTHEAP SQLVarChar(STMTHEAP, len,  // DEFAULT_CHARACTER_LENGTH,
                                  originalType.supportsSQLnull(), FALSE /*isUpShifted*/, FALSE /*isCaseinsensitive*/,
                                  charSet, CharInfo::DefaultCollation, CharInfo::COERCIBLE);
    } break;
    case NA_NUMERIC_TYPE: {
      NABoolean bignumIO = FALSE;
      if ((CmpCommon::getDefault(BIGNUM_IO) == DF_ON) || (CmpCommon::getDefault(BIGNUM_IO) == DF_SYSTEM))
      ...
      if (bignumIO) {
        ...
        if ((CmpCommon::getDefault(MODE_COMPATIBLE_1) == DF_ON) && ITM_CONSTANT == getItemExpr()->getOperatorType() &&
          ...
          scale = 0;  // set NULL's scale to 0
        Int16 DisAmbiguate = 0;
        desiredType = new STMTHEAP SQLNumeric(STMTHEAP, TRUE,         // signed
...
void ValueId::coerceType(const NAType &desiredType, enum NABuiltInTypeEnum defaultQualifier) {
  //
  ...
  if (originalType.getTypeQualifier() == NA_CHARACTER_TYPE) {
    if (((CharType &)originalType).getCharSet() == CharInfo::ISO88591 &&
      ...
      if (((CharType &)desiredType).getCharSet() != CharInfo::BINARY &&
          (ITM_CONSTANT == getItemExpr()->getOperatorType() && ((ConstValue *)getItemExpr())->isNull()))
        ((CharType *)newType)->setCharSet(((CharType &)desiredType).getCharSet());
    ...
    // charset is the value of CQD DYNAMIC_PARAM_DEFAULT_CHARSET
    if (desiredType.getTypeQualifier() != NA_CHARACTER_TYPE) {
      newType = originalType.newCopy(STMTHEAP);
      NAString cs = CmpCommon::getDefaultString(DYNAMIC_PARAM_DEFAULT_CHARSET);
      ((CharType *)newType)->setCharSet(CharInfo::getCharSetEnum(cs));
    }

  } else {
    if (!getItemExpr()->isNullConstant() && originalType.getTypeQualifier() != NA_UNKNOWN_TYPE) return;
  ...
  if (NotaCharType &&  // We do not want to force a characters storage values to be float
      (desiredType.getFSDatatype() == REC_FLOAT32 || desiredType.getFSDatatype() == REC_FLOAT64)) {
    if (desiredType.getFSDatatype() == REC_FLOAT32)
      newType = new STMTHEAP SQLReal(STMTHEAP, desiredType.supportsSQLnull(), desiredType.getPrecision());
    else
      ...
      newType = new STMTHEAP SQLDoublePrecision(STMTHEAP, desiredType.supportsSQLnull(), desiredType.getPrecision());
  } else {
    if (newType == NULL) {
      ...
      if (((desiredType.getFSDatatype() == REC_BIN8_SIGNED) || (desiredType.getFSDatatype() == REC_BIN8_UNSIGNED)) &&
        ...
        if (numType.getScale() == 0)
          newType = new (STMTHEAP) SQLSmall(STMTHEAP, isSigned, desiredType.supportsSQLnull());
        else
          newType = new (STMTHEAP) SQLNumeric(STMTHEAP, sizeof(short), numType.getPrecision(), numType.getScale(),
                                              isSigned, desiredType.supportsSQLnull());
      ...
      } else if ((desiredType.getFSDatatype() == REC_BOOLEAN) && (CmpCommon::getDefault(TRAF_BOOLEAN_IO) == DF_OFF)) {
        newType = new (STMTHEAP) SQLVarChar(STMTHEAP, SQL_BOOLEAN_DISPLAY_SIZE, desiredType.supportsSQLnull());
        ...
        if (desiredType.getFSDatatype() == REC_BINARY_STRING)
          newType = new (STMTHEAP) SQLChar(STMTHEAP, desiredType.getNominalSize(), desiredType.supportsSQLnull());
        else
          newType = new (STMTHEAP) SQLVarChar(STMTHEAP, desiredType.getNominalSize(), desiredType.supportsSQLnull());
      }  // Binary String
        ...
        if (CmpCommon::getDefault(BIGNUM_IO) == DF_ON)
          ...
          if (((SQLBigNum &)desiredType).isARealBigNum()) bignumIO = TRUE;
        }
        ...
        if (bignumIO) {
          ...
          Int16 DisAmbiguate = 0;
          newType = new (STMTHEAP) SQLNumeric(STMTHEAP, isSigned, precision, scale,
                                              DisAmbiguate,  // added for 64bit proj.
  ...
  if ((ODBC || JDBC) &&
    ...
    if ((getItemExpr()->origOpType()) == ITM_DYN_PARAM) tempType->setNullable(*newType);
  }
  ...
  // in case one of the children could not be typed.
  //
  const NAType *synthesizedNewType = getItemExpr()->pushDownType(*newType, defaultQualifier);

  changeType(synthesizedNewType);
}

NAColumn *ValueId::getNAColumn(NABoolean okIfNotColumn) const {
  ItemExpr *ie = getItemExpr();
  switch (ie->getOperatorType()) {
    case ITM_BASECOLUMN:
...
NABoolean ValueId::isColumnWithNonNullNonCurrentDefault() const {
  ...
  if (nac && nac->getDefaultValue() && nac->getDefaultClass() != COM_NULL_DEFAULT &&
      nac->getDefaultClass() != COM_CURRENT_DEFAULT && nac->getDefaultClass() != COM_CURRENT_UT_DEFAULT)
    return TRUE;
  else
    return FALSE;
}

// for now we only support ROWID equal predicate like : ROWID = 'AABBCCDD'
NABoolean ValueId::isAnValidRowidPredicate() const {
  NABoolean ret = TRUE;

  ValueId referencedInput;

  ItemExpr *iePtr = getItemExpr();

  // ---------------------------------------------------------------------
  // The predicate must be a binary comparison predicate.
  // ---------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------
  // One operand of the predicate MUST be the ROWID column.
  // ---------------------------------------------------------------------
  CollIndex arity = iePtr->getArity();
  switch (arity) {
    case 2:
      // if the left hand side is (represents) the given ROWID column:
      if (iePtr->child(0)->castToItemExpr()->getOperatorType() ==
          ITM_HBASE_ROWID AND iePtr->child(1)->castToItemExpr()->getOperatorType() == ITM_CONSTANT) {
        // then the right hand side is the operand to the ROWID pred:
        referencedInput = iePtr->child(1)->castToItemExpr()->getValueId();
      }
      // if the rhs is the ROWID column:
      // (but in the case of an assign (use in insert), it cannot be)
      else if (iePtr->child(0)->castToItemExpr()->getOperatorType() ==
               ITM_CONSTANT AND iePtr->child(1)->castToItemExpr()->getOperatorType() == ITM_HBASE_ROWID) {
        // then the lhs is the operand to the ROWID pred:
        referencedInput = iePtr->child(0)->castToItemExpr()->getValueId();
      } else
        // the binary operator did not contain a key col in any of its
        // children, thus it cannot be a ROWID pred:
        ret = FALSE;
      break;
...
BaseColumn *ValueId::castToBaseColumn(NABoolean *isaConstant) const {
  ItemExpr *ie = getItemExpr();
...
ValueId ValueId::getBaseColumn(NABoolean *isaConstant) const {
...
ValueId ValueId::nullInstantiate(BindWA *bindWAPtr, NABoolean forceCast) const {
  ...
  InstantiateNull *instNull = new (bindWAPtr->wHeap()) InstantiateNull(getItemExpr(), resultType);
...
void ValueId::replaceBaseColWithExpr(const NAString &colName, const ValueId &vid) {
  ...
  for (int i = 0; i < thisItemExpr->getArity(); i++) {
    ...
    if (childItemExpr->getOperatorType() == ITM_BASECOLUMN) {
      if (((BaseColumn *)childItemExpr)->getColName() == colName) thisItemExpr->setChild(i, vid.getItemExpr());
    }
...
NABoolean ValueId::normalizeNode(NormWA &normWARef) {
  ...
  if (nePtr->castToItemExpr() != iePtr || nePtr->getValueId() != *this) {
    transformed = TRUE;
...
ValueIdList::ValueIdList(const ValueIdSet &vs, CollHeap *h) : NAList<ValueId>(h, vs.entries()) {
  CollIndex index = 0;
  for (ValueId vid = vs.init(); vs.next(vid); vs.advance(vid)) insertAt(index++, vid);
  CMPASSERT(index == vs.entries());
  (*counter_).incrementCounter();
}

// Initialization of list assumes array is densely populated or full
ValueIdList::ValueIdList(const ValueIdArray &va, CollHeap *h) : NAList<ValueId>(h, va.entries()) {
  for (CollIndex i = 0; i < va.entries(); i++)
    if (va.used(i)) insert(va[i]);
  (*counter_).incrementCounter();
}
...
// -----------------------------------------------------------------------
NABoolean ValueIdList::allMembersCoveredInSet(const ValueIdSet &vidSet, NABoolean exactMatch) const {
...
// ---------------------------------------------------------------------
ValueIdList ValueIdList::findNJEquiJoinCols(const ValueIdSet &child0Outputs, const ValueIdSet &child1Inputs,
  ...
  while (i < entries()) {
    ...
    const ValueId &noInverseVid = (*this)[i].getItemExpr()->removeInverseOrder()->getValueId();
...
int ValueIdList::complifyAndCheckPrefixCovered(const ValueIdSet &vidSet, const GroupAttributes *ga) {
  ...
  while (i < (int)entries()) {
    ...
    if (NOT emptyGA.covers(thisVid, vidSet, referencedInputs)) {
      ...
      const ValueId &simpThisVid = (*this)[i].getItemExpr()->simplifyOrderExpr(&order1)->getValueId();
      ...
      if (simpleVidSet.contains(simpThisVid)) {
        ...
        for (ValueId vid = vidSet.init(); vidSet.next(vid); vidSet.advance(vid)) {
          const ValueId &simpOtherVid = vid.getItemExpr()->simplifyOrderExpr(&order2)->getValueId();
        ...
        if (order1 == order2) {
          ...
          // set expression.
          ItemExpr *inverseCol;
          inverseCol = new (CmpCommon::statementHeap()) InverseOrder(newThisVid.getItemExpr());
...
NABoolean ValueIdList::normalizeNode(NormWA &normWARef) {
  ...
  for (CollIndex index = 0; index < entries(); index++) {
    ...
    if (nePtr->castToItemExpr() != iePtr || nePtr->getValueId() != at(index)) {
      transformed = TRUE;
...
// -----------------------------------------------------------------------
ItemExpr *ValueIdList::rebuildExprTree(OperatorTypeEnum op, NABoolean redriveTypeSynthesisFlag,
  ...
  // their ValueIds.
  // ---------------------------------------------------------------------
  if (leftPtr) leftPtr->synthTypeAndValueId(redriveTypeSynthesisFlag);
...
// -----------------------------------------------------------------------
void ValueIdList::replaceVEGExpressions(const ValueIdSet &availableValues, const ValueIdSet &inputValues,
                                        NABoolean thisIsAKeyPredicate, VEGRewritePairs *lookup,
                                        NABoolean replicateExpression, const ValueIdSet *outputExpr,
  ...
  for (CollIndex ix = 0; ix < entries(); ix++) {
    ...
    if (outputExpr && outputExpr->containsTheGivenValue(exprId)) replicateExpression = FALSE;

    ItemExpr *iePtr = NULL;
    if (CmpCommon::getDefault(RANGESPEC_TRANSFORMATION) == DF_ON &&
        exprId.getItemExpr()->getOperatorType() == ITM_RANGE_SPEC_FUNC) {
      iePtr = exprId.getItemExpr()->child(1)->replaceVEGExpressions(
          availableValues, inputValues, thisIsAKeyPredicate, lookup, replicateExpression, joinInputAndPotentialOutput,
          iDesc, left_ga, right_ga);
    } else {
      iePtr = exprId.getItemExpr()->replaceVEGExpressions(availableValues, inputValues, thisIsAKeyPredicate, lookup,
    ...
    if (iePtr)  // wild card expansion was successful
    {
      ...
      if (iePtr->getValueId() != exprId) at(ix) = iePtr->getValueId();
...
OrderComparison ValueIdList::satisfiesReqdOrder(const ValueIdList &reqdOrder, GroupAttributes *ga,
  ...
  if (ga) tempThis.removeCoveredExprs(ga->getCharacteristicInputs());
  ...
  if (numActEntries < numReqdEntries) return DIFFERENT_ORDER;
  ...
  for (CollIndex reqdIx = 0; reqdIx < numReqdEntries; reqdIx++) {
    ...
    while (!done) {
      ...
      if (thisIx >= numActEntries) return DIFFERENT_ORDER;

      // compare the next column of tempThis with the required order
      oneCol = reqdOrder[reqdIx].getItemExpr()->sameOrder(tempThis[thisIx].getItemExpr());
      ...
      if (oneCol == DIFFERENT_ORDER && ga &&
          (numActEntries - thisIx) > (numReqdEntries - reqdIx) &&  // extra cols in this
          ga->tryToEliminateOrderColumnBasedOnEqualsPred(tempThis[thisIx], preds)) {
    ...
    if (allCols == DIFFERENT_ORDER) return allCols;
...
NABoolean ValueIdList::satisfiesReqdArrangement(const ValueIdSet &reqdArrangement, GroupAttributes *ga,
                                                const ValueIdSet *preds) const {
  // ---------------------------------------------------------------------
  ...
  // since those should not appear in the requirements and since they
  // are not relevant to the ordering
  if (ga) arrCols.removeCoveredExprs(ga->getCharacteristicInputs());
  ...
  for (CollIndex i = startCol; i < entries() AND found; i++) {
    ...
    if (reqdArrangement.contains((*this)[i])) {
      ...
      for (ValueId x = reqdArrangement.init(); reqdArrangement.next(x); reqdArrangement.advance(x)) {
      ...
      if ((NOT found) && ga) {
        ...
        found = ga->tryToEliminateOrderColumnBasedOnEqualsPred((*this)[i], preds);
      }
    }
  }
...
// Helper function for ValueIdSet::findAndPrimeNiceColumnComparisons
// and ValueIdSet::findCommonNiceColumnComparisons
void addNewExpression(std::map<ValueId, ItemExpr *> &expressions, ValueId colId, ItemExpr *expr,
                      OperatorTypeEnum innerType) {
  std::map<ValueId, ItemExpr *>::iterator it = expressions.find(colId);
  if (it == expressions.end())  // first column reference in a conjunct
    expressions[colId] = expr;  // can't copy dynamic params, so assign with original ItemExpr*
  else                          // not the first, we have to AND it
    expressions[colId] = expr->connect2(innerType, expressions[colId], expr);
}
...
//
// We also consider an IN list (equivalently an OR tree) consisting
// of "nice" column comparisons all on the same column as a "nice"
// predicate.
//
// This method, and the next method findCommonNiceColumnComparisons,
// are used to iterate over operands of a higher level backbone.
// This method is used for the first operand, while the next method
// is used for each of the remaining operands.
// ---------------------------------------------------------------------
void ValueIdSet::findAndPrimeNiceColumnComparisons(ValueIdSet &commonColumns /* in/out */,
                                                   std::map<ValueId, ItemExpr *> &newExpressions /* in/out */,
                                                   OperatorTypeEnum backboneType, OperatorTypeEnum innerType) {
  ValueId d;

  DCMPASSERT(commonColumns.isEmpty());

  // accumulate interesting columns in
  // commonColumns and predicates in newExpressions
  for (getFirst(d); next(d); advance(d)) {
    ItemExpr *pred = d.getItemExpr();
    ItemExpr *interestingColumn = pred->getColumnIfThisIsNiceComparison();
    if (interestingColumn) {
      ValueId interestingColId = interestingColumn->getValueId();
      commonColumns.insert(interestingColId);
      addNewExpression(newExpressions, interestingColId, pred, innerType);
    }
  }
}
...
// predicate.
//
// This method, and the previous method findAndPrimeNiceColumnComparisons,
// are used to iterate over operands of a higher level backbone.
// The previous method is used for the first operand, while this method
// is used for each of the remaining operands.
// ---------------------------------------------------------------------
void ValueIdSet::findCommonNiceColumnComparisons(ValueIdSet &commonColumns /* in/out */,
                                                 std::map<ValueId, ItemExpr *> &newExpressions /* in/out */,
                                                 OperatorTypeEnum backboneType, OperatorTypeEnum innerType) {
  ValueId d;
  ...
  // intersect the interesting columns
  // in this conjunct with those in commonColumns; also OR in
  // predicates on interesting columns with those in newExpressions
  ValueIdSet niceColumns;
  std::map<ValueId, ItemExpr *> conjunctExpressions;
  for (getFirst(d); next(d); advance(d)) {
    ItemExpr *pred = d.getItemExpr();
    ItemExpr *interestingColumn = pred->getColumnIfThisIsNiceComparison();
    if (interestingColumn) {
      ValueId interestingColId = interestingColumn->getValueId();
      if (commonColumns.contains(interestingColId)) {
        niceColumns.insert(interestingColId);
        addNewExpression(conjunctExpressions, interestingColId, pred, innerType);
      }
    }
  }
  for (niceColumns.getFirst(d); niceColumns.next(d); niceColumns.advance(d)) {
    newExpressions[d] = conjunctExpressions[d]->connect2(backboneType, newExpressions[d], conjunctExpressions[d]);
  }
  commonColumns.intersectSet(niceColumns);
}

// -----------------------------------------------------------------------
// Method for applying transformNode() to a ValueIdList.
// -----------------------------------------------------------------------
NABoolean ValueIdList::transformNode(NormWA &normWARef, ExprGroupId &introduceSemiJoinHere,
  ...
  for (CollIndex index = 0; index < entries(); index++) {
    ...
    // Transform the Item Expression.
    iePtr->getReplacementExpr()->transformNode(normWARef, nePtr, introduceSemiJoinHere, externalInputs);
    // If the original expression was transformed, update the list
    ...
    if (iePtr->getReplacementExpr()->getOperatorType() == ITM_ONE_ROW) {
      ItemExpr *ReplacedExpr = iePtr->getReplacementExpr()->transformOneRowAggregate(normWARef);
  ...
  if (introduceSemiJoinHere != (const RelExpr *)saveOrigPtr) transformed = TRUE;
...
void ValueIdList::removeCoveredExprs(const ValueIdSet &newExternalInputs) {
  ...
  for (CollIndex ix = 0; ix < entries(); ix++) {
    ...
    if (exprId.getItemExpr()->getOperatorType() == ITM_VEG_PREDICATE) {
      ...
      coverFlag = emptyGA.covers(vegPtr->getVEGReference()->getValueId(), newExternalInputs, referencedInputs);
    } else
      coverFlag = emptyGA.covers(exprId, newExternalInputs, referencedInputs);
    if (coverFlag) {
      removeAt(ix);
      ix--;
    }
...
void ValueIdList::removeDuplicateValueIds() {
  ...
  for (CollIndex last = entry; last > 0; last--) {
    ...
    for (int start = 0; start < (int)last; start++) {
      ...
      // simplify the expression
      // e.g. remove any INVERSE nodes on top (i.e. DESC)
      ValueId next = (*this)[start].getItemExpr()->removeInverseOrder()->getValueId();
...
void ValueIdList::removeSubqueryOrIsolatedUDFunctionPredicates(ValueIdSet &subqueryExpr) {
  for (CollIndex ix = 0; ix < entries(); ix++) {
    ...
    if (exprId.getItemExpr()->containsValueIdProxySibling(subqueryExpr) || exprId.getItemExpr()->containsSubquery() ||
...
void ValueIdList::unparse(NAString &result, PhaseEnum phase, UnparseFormatEnum form, TableDesc *tabId) const {
  // result += "(";
  for (CollIndex i = 0; i < entries(); i++) {
    if (i > 0) result += ", ";
...
// to availableValues.
// -----------------------------------------------------------------------
void ValueIdList::replaceOperandsOfInstantiateNull(const ValueIdSet &availableValues, const ValueIdSet &inputValues) {
  for (CollIndex j = 0; j < entries(); j++) {
    at(j).getItemExpr()->replaceOperandsOfInstantiateNull(availableValues, inputValues);
  }
}  // ValueIdList::replaceOperandsOfInstantiateNull()

void ValueIdList::print(FILE *ofd, const char *indent, const char *title, CollHeap *c, char *buf,
  ...
  for (CollIndex j = 0; j < entries(); j++) {
    ...
    if (at(j).getItemExpr()) at(j).getItemExpr()->unparse(unparsed, DEFAULT_PHASE,
                                                          form);  // expression as ascii string
...
void ValueIdList::display() const { print(); }

void ValueIdList::display(UnparseFormatEnum form) const {
  print(stdout, DEFAULT_INDENT, "ValueIdList", NULL, NULL, form);
}
...
ValueIdSet::ValueIdSet() : ClusteredBitmap(CmpCommon::statementHeap()) { (*counter_).incrementCounter(); }

ValueIdSet::ValueIdSet(const ValueIdSet &other) : ClusteredBitmap(other, CmpCommon::statementHeap()) {
  (*counter_).incrementCounter();
}

ValueIdSet::ValueIdSet(const ValueId &vid) : ClusteredBitmap(CmpCommon::statementHeap()) {
  addElement(vid.id_);
  (*counter_).incrementCounter();
}

ValueIdSet::ValueIdSet(const ValueIdList &other) : ClusteredBitmap(CmpCommon::statementHeap()) {
...
ValueIdSet::ValueIdSet(ValueDescArray *arr) : ClusteredBitmap(CmpCommon::statementHeap()) {
  (*counter_).incrementCounter();
}
...
NABoolean ValueIdSet::operator==(const ValueIdSet &other) const {
  ...
  for (ValueId vid = init(); next(vid); advance(vid)) {
    if (other.contains(vid) == FALSE) return FALSE;
  }
  return TRUE;
}
...
ValueIdSet ValueIdSet::convertToBaseIds() const {
  ...
  for (ValueId vid = init(); next(vid); advance(vid)) {
    ...
    result.insert(((BaseColumn *)(((IndexColumn *)vid.getItemExpr())->getDefinition().getItemExpr()))->getValueId());
  }
  return result;
}
...
CostScalar ValueIdSet::getMinOrigUecOfJoiningCols() {
  ...
  for (ValueId vid = init(); next(vid); advance(vid)) {
    ...
    if (inputColumnIEPtr == NULL) continue;
    ...
    switch (inputColumnIEPtr->getOperatorType()) {
      case ITM_VEG_REFERENCE: {
        ...
        if (!columnSet.isEmpty()) columnSet.getFirst(baseColumnForInputColumn);
        break;
      }
    ...
    if (baseColumnForInputColumn != NULL_VALUE_ID) result = (BaseColumn *)(vid.getItemExpr());
...
NABoolean ValueIdSet::coversFirstN(const ValueIdList &other, int N) const {
  ...
  for (i = 0; i < limit; i++) {
    ...
    if (other[i].getItemExpr()->getOperatorType() == ITM_INDEXCOLUMN) {
      ...
      if (!contains(ixCol->getDefinition())) {
        return FALSE;
      }
    } else if (!contains(other[i])) {
      return FALSE;
    }
  }
...
// -----------------------------------------------------------------------
void ValueIdSet::accumulateReferencedValues(const ValueIdSet &referencedSet, const ValueIdSet &referencingSet,
...
// -----------------------------------------------------------------------
void ValueIdSet::getLeafValuesForCoverTest(ValueIdSet &leafValueIds, const GroupAttributes &coveringGA,
                                           const ValueIdSet &newExternalInputs) const {
  for (ValueId exprId = init(); next(exprId); advance(exprId)) {
    exprId.getItemExpr()->getLeafValuesForCoverTest(leafValueIds, coveringGA, newExternalInputs);
  }
...
// -----------------------------------------------------------------------
NABoolean ValueIdSet::referencesOneValueFromTheSet(const ValueIdSet &otherSet) const {
  for (ValueId myExpr = init(); next(myExpr); advance(myExpr)) {
    for (ValueId otherExpr = otherSet.init(); otherSet.next(otherExpr); otherSet.advance(otherExpr)) {
      if (myExpr.getItemExpr()->referencesTheGivenValue(otherExpr)) return TRUE;
    }
  }
  return FALSE;
}  // ValueIdSet::referencesOneValueFromTheSet()
...
// references an expression that is a member of the otherSet.
// -----------------------------------------------------------------------
NABoolean ValueIdSet::referencesAllValuesFromMe(const ValueIdSet &otherSet) const {
  ValueId id;
  for (ValueId myExpr = init(); next(myExpr); advance(myExpr)) {
    if (NOT(otherSet.referencesTheGivenValue(myExpr, id))) return FALSE;
  }
  return TRUE;
}  // ValueIdSet::referencesAllValuesFromMe()
...
// -----------------------------------------------------------------------
NABoolean ValueIdSet::referencesTheGivenValue(const ValueId &theValue, ValueId &exprId, NABoolean doNotDigInsideVegRefs,
                                              NABoolean doNotDigInsideInstNulls) const {
  for (ValueId vid = init(); next(vid); advance(vid)) {
    if (vid.getItemExpr()->referencesTheGivenValue(theValue, doNotDigInsideVegRefs, doNotDigInsideInstNulls)) {
      exprId = vid;
      return TRUE;
    }
  }
...
int ValueIdSet::membersCoveredInSet(const ValueIdSet &vidSet, NABoolean lookBelowInstantiateNull) const {
  ...
  for (ValueId vid = init(); next(vid); advance(vid)) {
    ...
    if (vidSet.contains(memberValId)) membersFound++;
  }
...
void ValueIdSet::removeCoveredVIdSet(const ValueIdSet &otherSet) {
  for (ValueId id = otherSet.init(); otherSet.next(id); otherSet.advance(id)) {
    if (id.getItemExpr()->getOperatorType() == ITM_VEG_REFERENCE) {
      const ValueIdSet &insideSet = ((VEGReference *)(id.getItemExpr()))->getVEG()->getAllValues();
      remove(insideSet);
    } else {
      remove(id);
    }
...
// othewise.
// -----------------------------------------------------------------------
void ValueIdSet::removeUnReferencedVEGPreds(const ValueIdSet &otherSet) {
  for (ValueId myExpr = init(); next(myExpr); advance(myExpr)) {
...
void ValueIdSet::removeSubqueryOrIsolatedUDFunctionPredicates(ValueIdSet &subqueryExpr) {
  for (ValueId exprId = init(); next(exprId); advance(exprId)) {
    if (exprId.getItemExpr()->containsSubquery() || (exprId.getItemExpr()->containsIsolatedUDFunction())) {
      subqueryExpr.addElement(exprId);
...
// -----------------------------------------------------------------------
NABoolean ValueIdSet::transformNode(NormWA &normWARef, ExprGroupId &introduceSemiJoinHere,
                                    const ValueIdSet &externalInputs, const NABoolean movePredicates,
  ...
  for (ValueId exprId = init(); next(exprId); advance(exprId)) {
    ...
    if (nePtr != (const ItemExpr *)iePtr) {
      ...
      // -------------------------------------------------------------
      if (nePtr.getPtr()) {
        if (nePtr.getPtr()->getOperatorType() == ITM_RETURN_TRUE && nePtr.getPtr()->getArity() == 0)
          nePtr = NULL;
        else {
          ...
          if (!udfSubqInAggGrby_Enabled)
            ...
            nePtr->convertToValueIdSet(newExpr, NULL, ITM_AND, TRUE, TRUE /*flattenlist*/);
        }
      }
    }
    // -----------------------------------------------------------------
    ...
    if (introduceSemiJoinHere.getPtr() != newJoin) {
      ...
      // -------------------------------------------------------------
      if (movePredicates) {  // this ValueIdSet represents a predicate tree
                             // ---------------------------------------------------------
        ...
        if (nePtr.getPtr()) {  // if the subquery was not eliminated
                               // -----------------------------------------------------
          ...
          nePtr->convertToValueIdSet(transformedExpr, NULL, ITM_AND, FALSE);
    ...
    if (postJoinPredicates) {
      ...
      newJoin->getGroupAttr()->getCharacteristicInputs().getOuterReferences(outerReferences);
      if (nePtr != (const ItemExpr *)iePtr) {
    ...
    // ---------------------------------------------------------------------
      ...
      if (saveOrigPtr) {
        const ValueIdSet &outputs = saveOrigPtr->getGroupAttr()->getCharacteristicOutputs();
        ItemExpr *ie;
        if (nePtr != (const ItemExpr *)iePtr) {
          if (nePtr.getPtr())  // transformation hasn't eliminated it
            ie = nePtr.getPtr();
        ...
        if (ie) {
          ...
          if (normWARef.getCurrentOwnerExpr())
            isFOJ = (normWARef.getCurrentOwnerExpr()->getOperatorType() == REL_FULL_JOIN);

          if ((NOT normWARef.walkingAnExprTree())AND(NOT isFOJ)
                  AND(ie->getOperatorType() == ITM_EQUAL OR ie->getOperatorType() ==
                      ITM_NOT_EQUAL OR ie->getOperatorType() == ITM_LESS OR ie->getOperatorType() ==
                      ITM_LESS_EQ OR ie->getOperatorType() == ITM_GREATER OR ie->getOperatorType() == ITM_GREATER_EQ)) {
            if (ie->child(0)->getOperatorType() == ITM_INSTANTIATE_NULL) {
              InstantiateNull *inst = (InstantiateNull *)(ie->child(0)->castToItemExpr());
              if (NOT inst->NoCheckforLeftToInnerJoin) {
                ...
                if (outputs.contains(ie->child(0)->getValueId())) {
                  ie->child(0) = ie->child(0)->initiateLeftToInnerJoinTransformation(normWARef);
                }
              }
            }
            if (ie->child(1)->getOperatorType() == ITM_INSTANTIATE_NULL) {
              InstantiateNull *inst = (InstantiateNull *)(ie->child(1)->castToItemExpr());
              if (NOT inst->NoCheckforLeftToInnerJoin) {
                // Only for values coming from right hand side.
                //
                if (outputs.contains(ie->child(1)->getValueId())) {
                  ie->child(1) = ie->child(1)->initiateLeftToInnerJoinTransformation(normWARef);
                }
              }
            }
...
NABoolean ValueIdSet::normalizeNode(NormWA &normWARef) {
  ...
  for (ValueId exprId = init(); next(exprId); advance(exprId)) {
    ...
    if (nePtr != iePtr || nePtr->getValueId() != exprId) {
      ...
      if (iePtr->isSelectivitySetUsingHint() && nePtr->getOperatorType() == ITM_VEG_PREDICATE) {
        VEG *predVEG = ((VEGPredicate *)nePtr)->getVEG();
        const ValueIdSet &VEGGroup = predVEG->getAllValues();
        ItemExpr *constant = NULL;
        if (VEGGroup.referencesAConstValue(&constant) || VEGGroup.referencesAConstExpr(&constant)) {
          ValueIdSet cols;
      ...
      if (nePtr) nePtr->convertToValueIdSet(newExpr);
    }
  }  // loop over given expressions
...
void ValueIdSet::lookForVEGReferences(ValueIdSet &VEGRefSet) const {
  for (ValueId vid = init(); next(vid); advance(vid)) {
    if (vid.getItemExpr()->getOperatorType() == ITM_VEG_REFERENCE) VEGRefSet += vid;
  }
...
void ValueIdSet::introduceMissingVEGRefs(ValueIdSet &grcols) const {
  ...
  for (ValueId vid = init(); next(vid); advance(vid)) {
    if (vid.getItemExpr()->getOperatorType() == ITM_EQUAL) {
      ...
      ValueId v0, v1;
      if (eq->child(0)->getOperatorType() == ITM_VEG_REFERENCE) v0 = eq->child(0)->getValueId();
      if (eq->child(1)->getOperatorType() == ITM_VEG_REFERENCE) v1 = eq->child(1)->getValueId();
...
void ValueIdSet::lookForVEGPredicates(ValueIdSet &VEGPredSet) const {
  for (ValueId vid = init(); next(vid); advance(vid)) {
    if (vid.getItemExpr()->getOperatorType() == ITM_VEG_PREDICATE) VEGPredSet += vid;
  }
...
NABoolean ValueIdSet::hasNoEquiPredicates(ValueIdSet &joinCols) const {
  ...
  if (entries() <= 0) return result;  // has no predicates at all
...
NABoolean ValueIdSet::containsAsEquiLocalPred(ValueId x) const {
  ...
  for (ValueId vid = init(); next(vid); advance(vid)) {
    ...
    switch (expr->getOperatorType()) {
      case ITM_VEG_PREDICATE: {
        ValueIdSet vegMembers = ((VEGPredicate *)expr)->getVEG()->getAllValues();

        if (vegMembers.contains(x) && vegMembers.referencesAConstExpr(&constExpr)) return TRUE;
      } break;
      ...
      case ITM_EQUAL:
        ...
        for (CollIndex i = 0; i < 2; i++) {
          if (expr->child(i)->getOperatorType() == ITM_VEG_REFERENCE) {
            ...
            found = ((VEGReference *)(childExpr))->getVEG()->getAllValues().contains(x);
          ...
          if (found) {
            ...
            if (vset.entries() == 1 && vset.referencesAConstExpr(&constExpr) == TRUE) return TRUE;
          }
        }
...
void ValueIdSet::insertList(const ValueIdList &other) {
  for (CollIndex i = 0; i < other.entries(); i++) insert(other[i]);
}
...
// -----------------------------------------------------------------------
NABoolean ValueIdSet::isCovered(const ValueIdSet &newExternalInputs, const GroupAttributes &coveringGA,
...
int ValueIdSet::removeCoveredExprs(const ValueIdSet &newExternalInputs, ValueIdSet *usedInputs) {
  ...
  for (ValueId exprId = init(); next(exprId); advance(exprId)) {
    ...
    if (coverFlag) {
      subtractElement(exprId);
      result++;
      if (usedInputs) *usedInputs += referencedInputs;
    }
  }  // for
...
void ValueIdSet::removeIsNullIsNotNullExprs(const ValueIdSet &nullPredsCols, ValueIdSet &isNullIsNotNullPred) {
  NABoolean coverFlag;
  ValueIdSet referencedInputs;
  GroupAttributes emptyGA;

  emptyGA.setCharacteristicOutputs(nullPredsCols);
  for (ValueId exprId = init(); next(exprId); advance(exprId)) {
    coverFlag = FALSE;
    if (exprId.getItemExpr()->getOperatorType() == ITM_VEG_PREDICATE) {
      VEG *vegPtr = ((VEGPredicate *)(exprId.getItemExpr()))->getVEG();
      ValueId vid = vegPtr->getVEGReference()->getValueId();
      ItemExpr *ie = vid.getItemExpr();
      if (ie->isIsNullOrIsNotNullPred())
        coverFlag = emptyGA.covers(vegPtr->getVEGReference()->getValueId(), nullPredsCols, referencedInputs);
    } else {
      ItemExpr *ie = exprId.getItemExpr();
      if (ie->isIsNullOrIsNotNullPred()) coverFlag = emptyGA.covers(exprId, nullPredsCols, referencedInputs);
    }

    if (coverFlag) {
      subtractElement(exprId);
      isNullIsNotNullPred.addElement(exprId);
    }
  }  // for
}  // ValueIdSet::removeIsNullIsNotNullExprs
// ------------------------------------------------------------------------
...
ValueIdSet ValueIdSet::simplifyOrderExpr() const {
  ...
  for (ValueId vid = init(); next(vid); advance(vid)) {
    simpleVidSet += vid.getItemExpr()->simplifyOrderExpr()->getValueId();
  }
...
ValueIdSet ValueIdSet::removeInverseOrder() const {
  ...
  for (ValueId vid = init(); next(vid); advance(vid)) {
    noInverseVidSet += vid.getItemExpr()->removeInverseOrder()->getValueId();
  }
...
// ---------------------------------------------------------------------
void ValueIdSet::findCommonSubexpressions(ValueIdSet &other, NABoolean removeCommonExprFromOther) {
  ...
  for (vid = init(); next(vid); advance(vid)) {
    ...
    for (j = 0; j < otherNumEntries; j++) {
      if (otherHashValues[j] == hashVal)
        if (vid.getItemExpr()->duplicateMatch(*(otherList[j]).getItemExpr())) {
          found = TRUE;
          break;
        }
    }
    ...
    if (found) {
      ...
      if (removeCommonExprFromOther) other -= otherList[j];
    } else {
...
//    SQR = POWER(VEG(A,2),2)
// ---------------------------------------------------------------------
ValueIdSet ValueIdSet::createMirrorPreds(ValueId &computedCol, ValueIdSet underlyingCols) {
        ...
        if (newPred != NULL) {
          ...
          if (prevPred) {
            ...
            if ((prevOp == ITM_GREATER_EQ && newOp == ITM_LESS_EQ ||
                ...
                prevPred->child(0) == newPred->child(0) && prevConst && newConst &&
              ...
              BiRelat *newEqualPred =
                  new (CmpCommon::statementHeap()) BiRelat(ITM_EQUAL, newPred->child(0), newPred->child(1));
              newEqualPred->setSpecialNulls(static_cast<BiRelat *>(newPred)->getSpecialNulls());
...
// -----------------------------------------------------------------------
ItemExpr *ValueIdSet::rebuildExprTree(OperatorTypeEnum op, NABoolean redriveTypeSynthesisFlag,
                                      NABoolean createFinalizeResultNode) const {
  ValueIdList vidList;
  vidList.insertSet(*this);
  return vidList.rebuildExprTree(op, redriveTypeSynthesisFlag, createFinalizeResultNode);
...
// root.
// -----------------------------------------------------------------------
void ValueIdSet::replaceVEGExpressionsAndCopy(const ValueIdSet &setContainingWildCards) {
  clear();
  for (ValueId vid = setContainingWildCards.init(); setContainingWildCards.next(vid);
    ...
    if (vid.getItemExpr()->getOperatorType() == ITM_VEG_REFERENCE) {
      ...
      ((VEGReference *)(vid.getItemExpr()))->getVEG()->getAndExpandAllValues(recurse);
...
// -----------------------------------------------------------------------
void ValueIdSet::getUnReferencedVEGmembers(const ValueIdSet &setContainingWildCards) {
  clear();
  for (ValueId vid = setContainingWildCards.init(); setContainingWildCards.next(vid);
       setContainingWildCards.advance(vid)) {
    if (vid.getItemExpr()->getOperatorType() == ITM_VEG_REFERENCE) {
...
// set of availableValues that is supplied.
// -----------------------------------------------------------------------
void ValueIdSet::replaceVEGExpressions(const ValueIdSet &availableValues, const ValueIdSet &inputValues,
                                       NABoolean thisIsAKeyPredicate, VEGRewritePairs *lookup,
                                       NABoolean replicateExpression, const ValueIdSet *outputExpr,
                                       const ValueIdSet *joinInputAndPotentialOutput, const IndexDesc *iDesc,
                                       const GroupAttributes *left_ga, const GroupAttributes *right_ga) {
...
void ValueIdSet::getVEGesWithMultipleConsts(ValueIdSet &keyPredsToBeEvaluated) {
  for (ValueId exprId = init(); next(exprId); advance(exprId)) {
    ...
    if ((vegItemExpr->getOperatorType() == ITM_VEG_PREDICATE) ||
      ...
      const ValueIdSet &VEGGroup = predVEG->getAllValues();
      if (VEGGroup.referencesConstExprCount() > 1) keyPredsToBeEvaluated += exprId;
    }
  }
}
...
// to availableValues.
// -----------------------------------------------------------------------
void ValueIdSet::replaceOperandsOfInstantiateNull(const ValueIdSet &availableValues, const ValueIdSet &inputValues) {
  for (ValueId exprId = init(); next(exprId); advance(exprId)) {
    exprId.getItemExpr()->replaceOperandsOfInstantiateNull(availableValues, inputValues);
  }
}  // ValueIdSet::replaceOperandsOfInstantiateNull()
...
void ValueIdSet::weedOutUnreferenced(ValueIdSet &other) const {
  ...
  // loop over other
  for (ValueId otherId = other.init(); other.next(otherId); other.advance(otherId)) {
    notFound = TRUE;

    const ValueId &otherId2 = otherId.getItemExpr()->getReplacementExpr()->getValueId();
...
void ValueIdSet::weedOutNonEquiPreds() {
  for (ValueId v = init(); next(v); advance(v)) {
    if (v.getItemExpr()->getOperatorType() != ITM_VEG_PREDICATE && v.getItemExpr()->getOperatorType() != ITM_EQUAL)
      *this -= v;
  }
}
...
NABoolean ValueIdSet::referencesAConstExpr(ItemExpr **constExprPtr) const {
  for (ValueId id = init(); next(id); advance(id)) {
    ...
    if (pred->doesExprEvaluateToConstant(FALSE, TRUE)) {
      if (constExprPtr) *constExprPtr = id.getItemExpr();
...
NABoolean ValueIdSet::doAllExprsEvaluateToConstant(NABoolean isStrict, NABoolean considerVEG) const {
  for (ValueId id = init(); next(id); advance(id)) {
    const ItemExpr *pred = id.getItemExpr();
    if (!pred->doesExprEvaluateToConstant(isStrict, considerVEG)) return FALSE;
  }
  return TRUE;
...
NABoolean ValueIdSet::newConstantsAreDistinct(const ValueIdSet &newConstValues) const {
  // Do an n*m comparison of the cartesian product of existing and new
  // values, assuming that "this" and newConstValues each contain
  // distinct values that don't have to be checked against each other.
  NABoolean result = FALSE;
  int numComparisons = entries() * newConstValues.entries();

  if (numComparisons == 0)
    return TRUE;
  else if (numComparisons > 125)
    // do this only for somewhat small numbers of comparisons, note
    // that as we add these piecemeal, we'll call this method many
    // times, so the total # of comparison is approximately the square
    // of this limit
    return FALSE;

  NAText sql;
  ItemExprList args(2 * numComparisons, CmpCommon::statementHeap());

  for (ValueId i = init(); next(i); advance(i))
    for (ValueId j = newConstValues.init(); newConstValues.next(j); newConstValues.advance(j)) {
      char buf[64];

      snprintf(buf, sizeof(buf), "@A%d <> @A%d", args.entries() + 1, args.entries() + 2);
      args.insert(i.getItemExpr());
      args.insert(j.getItemExpr());
  ...
  int numDiags = CmpCommon::diags()->getNumber();
  ItemExpr *notEqualExpr = parser.getItemExprTree(sql.c_str(), sql.length(), CharInfo::ISO88591, 0, NULL, NULL, NULL,
                                                  NULL, NULL, NULL, &args);
  ValueIdList notEqualPred;

  if (!notEqualExpr)
    // parse error, should be rare
    return FALSE;

  notEqualExpr->synthTypeAndValueId();
  notEqualPred.insert(notEqualExpr->getValueId());
  result = notEqualPred.constantFolding();

  // remove all the errors and warnings we caused and
  // let someone else deal with them if they occur in
  // the actual query
...
NABoolean ValueIdSet::referencesAHostvariableorParam() const {
  for (ValueId id = init(); next(id); advance(id)) {
    ...
    if (pred->getOperatorType() == ITM_VEG_PREDICATE) {
      ...
      predVEG->markAsNotSeenBefore();
      if (containsHostvar) return TRUE;
    }
...
ValueIdSet ValueIdSet::replaceRangeSpecRefs() const {
  // If not using rangespec, just return the original set.
  if (CmpCommon::getDefault(RANGESPEC_TRANSFORMATION) != DF_ON) return *this;
...
ValueIdSet ValueIdSet::rewriteNotEquals(BindWA *bindWA) const {
  ValueIdSet rewrittenPreds;
  ValueIdSet vs;
  ItemExpr *pred;

  // Rewrite any predicate of the form X <> c as X < c OR X > c
  for (ValueId vid = init(); next(vid); advance(vid)) {
    pred = vid.getItemExpr();
    ItemExpr *ie = pred->rewriteNotEquals(bindWA);
    rewrittenPreds += ie->getValueId();
  }

  return rewrittenPreds;
}

ValueIdSet ValueIdSet::removePredsWithHostVars() const {
...
NABoolean ValueIdSet::referencesBignumNumericDataType() const {
  for (ValueId id = init(); next(id); advance(id)) {
    ...
    if (pred->getOperatorType() == ITM_VEG_REFERENCE) {
      ...
      // We should check the type of key column about atom predicate.
      // See SearchKey::init and SearchKeyBounds::isBoundExclusive
      const NAType *type = NULL;
      if (pred->getOperatorType() == ITM_LESS || pred->getOperatorType() == ITM_GREATER) {
        type = &(pred->child(0)->getValueId().getType());
      } else {
        type = &(pred->getValueId().getType());
      }

      if ((type->getTypeQualifier() == NA_NUMERIC_TYPE && ((const NumericType *)type)->isBigNum())) {
        return TRUE;
      }
    }
  }
...
ValueIdSet ValueIdSet::removePredsWithRollingCols(TableDesc *tdesc) const {
  ...
  for (ValueId id = init(); next(id); advance(id)) {
    ...
    if (!((op == ITM_VEG_PREDICATE) || (op == ITM_EQUAL) || (op == ITM_NOT_EQUAL) || (op == ITM_LESS) ||
          (op == ITM_LESS_EQ) || (op == ITM_GREATER) || (op == ITM_GREATER_EQ) || (op == ITM_OR) || (op == ITM_AND))) {
      localPreds.insert(id);
      continue;
    }
    ...
    // then use fetchCount
    if ((op != ITM_VEG_PREDICATE) && ((pred->child(0)->getOperatorType() == ITM_EXTRACT_ODBC) ||
    ...
    for (ValueId cid = leafValues.init(); leafValues.next(cid); leafValues.advance(cid)) {
      ...
      if (cv != NULL) {
        ...
        if ((val < colStats->getMinValue()) || (val > colStats->getMaxValue())) {
          insertPred = FALSE;
          break;
        }
      }
...
void ValueIdSet::getConstants(ValueIdSet &addConstantsToThisSet, NABoolean includeCacheParams) const {
  for (ValueId id = init(); next(id); advance(id)) {
    ...
    else if (includeCacheParams == TRUE && pred->getOperatorType() == ITM_CACHE_PARAM)
      addConstantsToThisSet += id;
  }
}

void ValueIdSet::getConstantExprs(ValueIdSet &addConstantExprsToThisSet, NABoolean isStrict) const {
  for (ValueId id = init(); next(id); advance(id)) {
    ...
    if (ie->doesExprEvaluateToConstant(isStrict, TRUE) OR((ie->previousHostVar() == TRUE) && (NOT isStrict)))
...
void ValueIdSet::getOuterReferences(ValueIdSet &OuterRefs) const {
  ...
  for (ValueId id = init(); next(id); advance(id)) {
...

    {
      ...
      if (VEGGroup.referencesAConstExpr(&constant)) doNotInsert = TRUE;
...
void ValueIdSet::categorizePredicates(const ValueIdSet &outerReferences, ValueIdSet &EqLocalPreds,
                                      ValueIdSet &OtherLocalPreds, ValueIdSet &EqNonLocalPreds,
                                      ValueIdSet &OtherNonLocalPreds, ValueIdSet &BiLogicPreds,
         ...
         ((ItemExpr *)pred)->isSelectivitySetUsingHint()))  // Unless selectivity is specified by user
    {
      ...
      if (op == ITM_VEG_PREDICATE)  // Equality (transitive closure) predicate?
      {
        if (outerReferences.entries() > 0) {
          ...
          EqLocalPreds.insert(id);
      } else if ((op == ITM_LESS) || (op == ITM_LESS_EQ) || (op == ITM_GREATER) || (op == ITM_GREATER_EQ) ||
                 (op == ITM_EQUAL) || (op == ITM_NOT_EQUAL) || (op == ITM_IS_NULL) || (op == ITM_IS_NOT_NULL) ||
...
NABoolean ValueIdSet::predsReqFetchCnt(TableDesc *tdesc) {
  ...
  categorizePredicates(outerRefs, eqLocalPreds, otherLocalPreds, eqNonLocalPreds, otherNonLocalPreds, biLogicPreds,
                       defaultPreds);
  ...
  if (((otherLocalPreds.entries() > 0) && (eqLocalPreds.entries() > 0)) || (otherLocalPreds.entries() > 1) ||
      (biLogicPreds.entries() > 0) || (defaultPreds.entries() > 0) || (eqNonLocalPreds.entries() > 0) ||
  ...
  if (eqLocalPreds.entries() > 0) {
    ...
    for (ValueId predId = eqLocalPreds.init(); eqLocalPreds.next(predId); eqLocalPreds.advance(predId)) {
      ...
      if (predId.moreThanOneColFromSameTabOrCharDate(tdesc, colsOfThisTableInPred)) return TRUE;

      cols.insert(colsOfThisTableInPred);
    }

    // if the columns constitute unique index, no need to use fetchCount
    if (cols.doColumnsConstituteUniqueIndex(tdesc)) return FALSE;
...
NABoolean ValueId::anExpression() {
...
NABoolean ValueId::moreThanOneColFromSameTabOrCharDate(TableDesc *tdesc, ValueIdSet &colsOfThisTable) {
  ...
  if (predCol.entries() <= 1) checkForCharCol = TRUE;
  ...
  if (tdesc && tdesc->getTableAnalysis()) {
    ...
    if (colsOfThisTable.entries() > 1) return TRUE;
    if (checkForCharCol || (colsOfThisTable.entries() == 1)) {
      ValueId colId;
      colsOfThisTable.getFirst(colId);
      if (colId == NULL_VALUE_ID) return FALSE;

      // get column type and return TRUE if date time type
      if (colId.getType().getTypeQualifier() == NA_DATETIME_TYPE) return TRUE;
      ...
      if (colStats->isColWithBndryConflict()) return TRUE;
    }
  }
  return FALSE;
}
...
NABoolean ValueId::isSaltColumn() const {
  BaseColumn *bc = castToBaseColumn();

  if (bc)
    return (bc->getNAColumn()->isSaltColumn());
  else
    return FALSE;
}
...
NABoolean ValueIdSet::getReferencedPredicates(const ValueIdSet &outerReferences, ValueIdSet &nonLocalPreds) const {
  NABoolean found = FALSE;
  if (outerReferences.entries() > 0) {
    for (ValueId id = init(); next(id); advance(id)) {
      const ItemExpr *pred = id.getItemExpr();
...
ex_expr::exp_return_type ValueIdSet::evalPredsAtCompileTime(ComDiagsArea *da, ValueIdMap *rewrite,
                                                            NABoolean replaceVEGPreds) const {
  ValueIdSet rewrittenPreds;
  ValueIdSet copiedPreds;
  ...
  if (replaceVEGPreds) {
    ...
    for (CollIndex i = 0; i < numOrigRewrites; i++) {
      if (extendedRewrite.getTopValues()[i].getItemExpr()->getOperatorType() == ITM_VEG_REFERENCE) {
        // we found a rewrite VEGRef ==> const1
        // (don't take the "const1" too literally, could be an expr)
        VEG *veg = static_cast<VEGReference *>(extendedRewrite.getTopValues()[i].getItemExpr())->getVEG();

        // now look for a constant const2 in the VEG
        ValueId const2 = veg->getAConstant(FALSE, TRUE);
        if (const2 != NULL_VALUE_ID) {
          ValueId const1 = extendedRewrite.getBottomValues()[i];
          ItemExpr *const1EqConst2 =
              new (CmpCommon::statementHeap()) BiRelat(ITM_EQUAL, const1.getItemExpr(), const2.getItemExpr());

          const1EqConst2->synthTypeAndValueId();
          // add a new rewrite VEGPred ==> const1 = const2
          extendedRewrite.addMapEntry(veg->getVEGPredicate()->getValueId(), const1EqConst2->getValueId());
        }  // rewrite entry is a VEGReference with a const in it
      }    // rewrite entry is a VEGReference
    }      // loop over rewrite entries
    extendedRewrite.rewriteValueIdSetDown(*this, rewrittenPreds);
  } else if (rewrite)
    // perform the rewrite (sans VEGPred logic), if one was requested
    rewrite->rewriteValueIdSetDown(*this, rewrittenPreds);
  else
    rewrittenPreds = *this;

  // make a copy of all the predicates, since genEvalPredicate below
  // will call preCodeGen and other methods that may change the
  // ItemExpr trees in ways that would make the predicate invalid. For
  // example, it may replace children with new ItemExprs that are
  // allocated from a temporary BindWA heap, which is deleted once we
  // exit.
  for (ValueId v = rewrittenPreds.init(); rewrittenPreds.next(v); rewrittenPreds.advance(v)) {
    ItemExpr *copiedPred = v.getItemExpr()->copyTree(STMTHEAP);
    copiedPred->synthTypeAndValueId();
    copiedPreds += copiedPred->getValueId();
  }

  // convert to an AND tree, map NULL to FALSE
  predTree = copiedPreds.rebuildExprTree(ITM_AND, FALSE, TRUE);
...
NABoolean ValueIdSet::doColumnsConstituteUniqueIndex(TableDesc *table, NABoolean considerStats) {
  ...
  if ((primaryKeyColumns.entries() > 0) && (contains(primaryKeyColumns)))
...
NABoolean ValueIdSet::containsSubquery() const {
  for (ValueId vid = init(); next(vid); advance(vid)) {
    if (vid.getItemExpr()->containsSubquery()) return TRUE;
  }
  return FALSE;
...
ItemExpr *ValueIdSet::containsUDF() const {
  for (ValueId vid = init(); next(vid); advance(vid)) {
    if (vid.getItemExpr()->containsUDF()) return vid.getItemExpr()->containsUDF();
  }
  return 0;
...
NABoolean ValueIdSet::containsIsolatedUDFunction() const {
  for (ValueId vid = init(); next(vid); advance(vid)) {
    if (vid.getItemExpr()->containsIsolatedUDFunction()) return TRUE;
  }
  return FALSE;
...
NABoolean ValueIdSet::containsOneTrue(ValueId &refOneTrue) const {
  for (ValueId vid = init(); next(vid); advance(vid)) {
    if (vid.getItemExpr()->isAnAggregate() && (vid.getItemExpr()->getOperatorType() == ITM_ONE_TRUE)) {
      refOneTrue = vid;
      return TRUE;
    }
  }
...
NABoolean ValueIdSet::containsAnyTrue(ValueId &refAnyTrue) const {
  for (ValueId vid = init(); next(vid); advance(vid)) {
    if (vid.getItemExpr()->isAnAggregate() && (vid.getItemExpr()->getOperatorType() == ITM_ANY_TRUE)) {
      refAnyTrue = vid;
      return TRUE;
    }
  }
...
// -----------------------------------------------------------------------
void ValueIdSet::unparse(NAString &result, PhaseEnum phase, UnparseFormatEnum form, TableDesc *tabId) const {
...
void ValueIdSet::print(FILE *ofd, const char *indent, const char *title, CollHeap *c, char *buf,
                       UnparseFormatEnum form) const {
  BUMP_INDENT(indent);
  Space *space = (Space *)c;
  char mybuf[1000];

  if ((indent && strlen(indent) > 0) || (title && strlen(title) > 0)) {
    snprintf(mybuf, sizeof(mybuf), "%s%s\n", NEW_INDENT, title);
    PRINTIT(ofd, c, space, buf, mybuf);
  }
...
// To be called from the debugger
void ValueIdSet::display() const { print(); }

void ValueIdSet::display(UnparseFormatEnum form) const {
  print(stdout, DEFAULT_INDENT, "ValueIdSet", NULL, NULL, form);
}
...
ex_expr::exp_return_type ValueIdList::evalAtCompileTime(
  ...
  BindWA bindWA(ActiveSchemaDB(), CmpCommon::context(), FALSE, FALSE, wheap);

  Generator generator(CmpCommon::context(), wheap);
  ...
  expGen.generateContiguousMoveExpr(*this, addConvNodes, 0, keyAtpIndex, tf, encodedKeyLen, &keExpr, NULL,
                                    ExpTupleDesc::SHORT_FORMAT, NULL, &tempValueIdList, 0, NULL,
                                    TRUE /*disable const folding*/);
  ...
  ExpDP2Expr *keyEncodeExpr = new (space) ExpDP2Expr(keExpr, workCriDesc, space);
  keyEncodeExpr->getExpr()->fixup(0, 0, 0, space, wheap, FALSE, NULL);
  ...
  if (workAtp->getDiagsArea() != diagsArea) workAtp->setDiagsArea(diagsArea);
  // evaluate the expr(s) into resultBuffer
  ex_expr::exp_return_type rc = keyEncodeExpr->getExpr()->eval(workAtp, 0, wheap);
  ...
  if (rc != ex_expr::EXPR_ERROR && (length || offset)) {
    const ValueId &tempValueId = tempValueIdList[0].getItemExpr()->getValueId();
    Attributes *attr = generator.getMapInfo(tempValueId)->getAttr();
    if (length) {
      if (attr->getVCIndicatorLength() > 0) {
        char temp[8];
        str_cpy_all(temp, resultBuffer + attr->getVCLenIndOffset(), attr->getVCIndicatorLength());
...
short ValueIdList::evaluateTree(const ItemExpr *root, char *encodedKeyBuffer, ULng32 encodedKeyLength, int *length,
  ...
  if (encodedValueIdList.evalAtCompileTime(1, ExpTupleDesc::SQLMX_KEY_FORMAT, encodedKeyBuffer, encodedKeyLength,
...
#define RESULT_SIZE 1000
  ...
  if (type.getTypeQualifier() != NA_CHARACTER_TYPE) {
    ...
    ex_expr::exp_return_type retcode = convDoIt(value + offset, length, (short)type.getFSDatatype(),
                                                type.getPrecisionOrMaxNumChars(), type.getScaleOrCharset(), result,
                                                t,                               // target length
    ...
    if (retcode != ex_expr::EXPR_OK) {
      NADELETEBASIC(result, CmpCommon::statementHeap());
      return (-1);
    }
    ...
    item = new (CmpCommon::statementHeap()) SystemLiteral((NAType *)&type, value, offset + length, &text);
    ...
    item = new (CmpCommon::statementHeap())
        SystemLiteral(strval, ctype.getCharSet(), ctype.getCollation(), ctype.getCoercibility());
  }

  item->synthTypeAndValueId();

  // Replace the subtree just computed with the node containing the result
  if (childNumber >= 0) parent.getItemExpr()->setChild(childNumber, item);

  if (outItemExpr) *outItemExpr = item;
...
NABoolean ValueIdList::canSimplify(ItemExpr *itemExpr, const ValueId &parent, int i, int childNumber,
  ...
  if ((type >= REC_MIN_NUMERIC && type <= REC_MAX_NUMERIC) &&
    ...
    if ((!ntyp->isExact()) || (ntyp->getScale() != 0) || (ntyp->getPrecision() >= 10)) return FALSE;
  }
  ...
  // with TRUE
  if ((itemExpr->getOperatorType() == ITM_OR) && (val == 1)) {
    value[0] = 1;
    value[1] = 0;
    value[2] = 0;
    value[3] = 0;
    length = 4;
    tempStr = "1";
    item = new (CmpCommon::statementHeap()) SystemLiteral(itemExpr->synthesizeType(), value, length, &tempStr);
  ...
  if ((itemExpr->getOperatorType() == ITM_OR) && (val == 0)) {
  ...
  if ((itemExpr->getOperatorType() == ITM_AND) && (val == 0)) {
    value[0] = 0;
    value[1] = 0;
    value[2] = 0;
    value[3] = 0;
  ...
  if ((itemExpr->getOperatorType() == ITM_AND) && (val == 1)) {
    parent.getItemExpr()->setChild(childNumber, itemExpr->getChild(1 - i));
    moved = 1 - i;
    return TRUE;
  }
  ...
  // a FALSE node
  if ((itemExpr->getOperatorType() == ITM_NOT) && (val == 1)) {
    value[0] = 0;
    value[1] = 0;
    value[2] = 0;
    value[3] = 0;
  ...
  // a TRUE node
  if ((itemExpr->getOperatorType() == ITM_NOT) && (val == 0)) {
    value[0] = 1;
    value[1] = 0;
    value[2] = 0;
    value[3] = 0;
  ...
  if ((itemExpr->getOperatorType() == ITM_PLUS) && (val == 0)) {
    parent.getItemExpr()->setChild(childNumber, itemExpr->getChild(1 - i));
    moved = 1 - i;
    return TRUE;
  }
  ...
  // a 0 node
  if ((itemExpr->getOperatorType() == ITM_TIMES) && (val == 0)) {
  ...
  // the operator with its other child
  if ((itemExpr->getOperatorType() == ITM_TIMES) && (val == 1)) {
    parent.getItemExpr()->setChild(childNumber, itemExpr->getChild(1 - i));
    moved = 1 - i;
    return TRUE;
  }
  ...
  if (itemExpr->getOperatorType() == ITM_DIVIDE) {
    ...
    if (val == 1) {
      ...
      if ((((BiArith *)itemExpr)->getDivToDownscale()) && (((BiArith *)itemExpr)->getRoundingMode() > 0)) return FALSE;
      ...
      item = new (CmpCommon::statementHeap()) SystemLiteral(ntyp, &cval, 4, &nas);
      item->synthTypeAndValueId();

      BiArith *ba = new (CmpCommon::statementHeap()) BiArith(ITM_TIMES, itemExpr->getChild(0)->castToItemExpr(), item);
...
int ValueIdList::evaluateExpr(const ValueId &parent, const ValueId &ch, int childNumber, NABoolean simplifyExpr,
  ...
  // and in the future.
  if (itemExpr->isAnAggregate() || itemExpr->isASequenceFunction() || (op == ITM_INSTANTIATE_NULL) ||
      (op == ITM_TRANSLATE) || (op == ITM_ROWSETARRAY_INTO)) {
    return FALSE;
  }

  if (op == ITM_UNIQUE_ID || op == ITM_UNIQUE_ID_SYS_GUID) return TRUE;
...
// check whether the pItemExpr is compare with NULL constant
// e.g.
// col1=NULL
static NABoolean isCompareWithNullConstant(const ItemExpr *pItemExpr) {
  if (!pItemExpr) return FALSE;
  if ((pItemExpr->getArity() == 2) &&
      (pItemExpr->child(0)->isNullConstant() || pItemExpr->child(1)->isNullConstant()) &&
      (pItemExpr->getOperatorType() == ITM_EQUAL || pItemExpr->getOperatorType() == ITM_NOT_EQUAL ||
       pItemExpr->getOperatorType() == ITM_LESS || pItemExpr->getOperatorType() == ITM_LESS_EQ ||
       pItemExpr->getOperatorType() == ITM_GREATER || pItemExpr->getOperatorType() == ITM_GREATER_EQ))
    return TRUE;
  return FALSE;
}
...
NABoolean ValueIdList::constantFolding() {
  ...
  BindWA bindWA(ActiveSchemaDB(), CmpCommon::context(), FALSE, FALSE, CmpCommon::statementHeap());
  for (CollIndex i = 0; i < list.entries(); i++) {
    ItemExpr *tempItem = list[i].getItemExpr();
    BiRelat *predicate = NULL;
    if (CmpCommon::getDefault(MODE_COMPATIBLE_1) == DF_ON && isCompareWithNullConstant(tempItem)) {
      isPredFalse = TRUE;
      predicate = new (CmpCommon::statementHeap()) BiRelat(ITM_EQUAL, new (CmpCommon::statementHeap()) ConstValue(1),
                                                           new (CmpCommon::statementHeap()) ConstValue(2));
      predicate->bindNode(&bindWA);
    }

    ValueId tmpVid;
    if (isPredFalse)
      tmpVid = predicate->getValueId();
    else
      tmpVid = list[i];
    const ValueId &vid = tmpVid;
    ...
    if (allConstants) {
      ...
      allTrue = FALSE;
      if (vid.getItemExpr()->getOperatorType() == ITM_LIKE &&
          (vid.getItemExpr()->child(1).getPtr()->getText().contains("%") ||
           vid.getItemExpr()->child(1).getPtr()->getText().contains("_"))) {
        // COL1 LIKE 'abc'||'%' will be transform to COL1 LIKE 'abc%', rebuild the
        // itemExpr to convert the predicate to COL1>='abc' and COL1<'abd'
        ItemExpr *newItem = vid.getItemExpr();
        newItem->markAsUnBound();
        newItem->setValueId(NULL_VALUE_ID);
        newItem = newItem->bindNode(&bindWA);
        item->setChild(0, newItem);
      }
    }
...
void ValueIdList::findCommonElements(const ValueIdSet &other) {
  ...
  for (i = 0; i < entries(); i++) {
    ...
    if (!other.contains((*this)[i])) listCopy.insert(i);
  }
  ...
  for (i = listCopy.entries(); i > 0; i--) removeAt(listCopy[i - 1]);
}
...
ValueIdList ValueIdList::findCommonElementsFromList(const ValueIdList &other) {
  ...
  for (i = 0; i < other.entries(); i++) {
    ...
    for (j = 0; j < entries(); j++) {
      // copy the index of the element that is not contained in the "other" set
      // this should be removed later
      if ((*this)[j] == currentOtherId) listCopy.insert(currentOtherId);
    }
  }
...
ValueIdMap::ValueIdMap(const ValueIdSet &identity) {
  for (ValueId x = identity.init(); identity.next(x); identity.advance(x)) {
    topValues_.insert(x);
...
NABoolean ValueIdMap::operator==(const ValueIdMap &other) const {
  ...
  if (topValues_ == other.topValues_ AND bottomValues_ == other.bottomValues_) return TRUE;

  return FALSE;  // for now
}

void ValueIdMap::remapTopValue(const ValueId &newTopValue, const ValueId &bottomValue) {
...
void ValueIdMap::remapBottomValue(const ValueId &topValue, const ValueId &newBottomValue) {
...
void ValueIdMap::addMapEntry(const ValueId &newTopValue, const ValueId &newBottomValue) {
  topValues_.insert(newTopValue);
  bottomValues_.insert(newBottomValue);
}

void ValueIdMap::mapValueIdUp(ValueId &topValue, const ValueId &bottomValue) const {
  CollIndex ix = bottomValues_.index(bottomValue);
...
void ValueIdMap::mapValueIdUpWithIndex(ValueId &topValue, const ValueId &bottomValue, CollIndex i) const {
...
void ValueIdMap::mapValueIdDown(const ValueId &topValue, ValueId &bottomValue) const {
  CollIndex ix = topValues_.index(topValue);
...
void ValueIdMap::mapValueIdListUp(ValueIdList &topValues, const ValueIdList &bottomValues) const {
...
void ValueIdMap::mapValueIdListDown(const ValueIdList &topValues, ValueIdList &bottomValues) const {
...
void ValueIdMap::mapValueIdSetUp(ValueIdSet &topValues, const ValueIdSet &bottomValues) const {
  CMPASSERT(topValues.isEmpty());

  for (ValueId l = bottomValues.init(); bottomValues.next(l); bottomValues.advance(l)) {
    ValueId u;
    mapValueIdUp(u, l);
    topValues.insert(u);
  }
...
void ValueIdMap::mapValueIdSetDown(const ValueIdSet &topValues, ValueIdSet &bottomValues) const {
...
void ValueIdMap::rewriteValueIdUp(ValueId &topValue, const ValueId &bottomValue) {
  topValue = bottomValue.getItemExpr()->mapAndRewrite(*this, FALSE);
}

void ValueIdMap::rewriteValueIdDown(const ValueId &topValue, ValueId &bottomValue) {
  bottomValue = topValue.getItemExpr()->mapAndRewrite(*this, TRUE);
}

void ValueIdMap::rewriteValueIdUpWithIndex(ValueId &topValue, const ValueId &bottomValue, CollIndex i) {
  topValue = bottomValue.getItemExpr()->mapAndRewriteWithIndx(*this, i);
}

void ValueIdMap::rewriteValueIdListUp(ValueIdList &topValues, const ValueIdList &bottomValues) {
  CMPASSERT(topValues.entries() == 0);
...
void ValueIdMap::rewriteValueIdListUpWithIndex(ValueIdList &topValues, const ValueIdList &bottomValues) {
  CMPASSERT(topValues.entries() == 0);
...
void ValueIdMap::rewriteValueIdListDown(const ValueIdList &topValues, ValueIdList &bottomValues) {
  CMPASSERT(bottomValues.entries() == 0);
...
void ValueIdMap::rewriteValueIdSetUp(ValueIdSet &topValues, const ValueIdSet &bottomValues) {
  CMPASSERT(topValues.isEmpty());
...
void ValueIdMap::rewriteValueIdSetDown(const ValueIdSet &topValues, ValueIdSet &bottomValues) {
  CMPASSERT(bottomValues.isEmpty());
...
void ValueIdMap::augmentForVEG(NABoolean addVEGPreds, NABoolean addVEGRefs, NABoolean compareConstants,
                               const ValueIdSet *topInputsToCheck, const ValueIdSet *bottomInputsToCheck,
                               ValueIdSet *vegRefsWithDifferentConstants, ValueIdSet *vegRefsWithDifferentInputs) {
  ...
  for (CollIndex i = 0; i < ne; i++) {
    ...
    if (addVEGPreds && to == ITM_VEG_REFERENCE && bo == ITM_VEG_REFERENCE) {
      ...
      if (!topValues_.contains(topPred)) {
        ...
        if (compareConstants) ok = (constT == constB);

        if (!ok && vegRefsWithDifferentConstants)
          *vegRefsWithDifferentConstants += vegT->getVEGReference()->getValueId();

        if (ok && ((topInputsToCheck && topInputsToCheck->entries() > 0) ||
                   (bottomInputsToCheck && bottomInputsToCheck->entries() > 0))) {
          ValueIdSet topInputs;
          ValueIdSet bottomInputs;

          if (topInputsToCheck) topInputs = *topInputsToCheck;
          if (bottomInputsToCheck) bottomInputs = *bottomInputsToCheck;
          ...
          if (!ok && vegRefsWithDifferentInputs) *vegRefsWithDifferentInputs += vegT->getVEGReference()->getValueId();
        }
    ...
    if (addVEGRefs && to == ITM_VEG_PREDICATE && bo == ITM_VEG_PREDICATE) {
      ValueId topRef(static_cast<VEGPredicate *>(t)->getVEG()->getVEGReference()->getValueId());

      if (!topValues_.contains(topRef)) {
        topValues_.insert(topRef);
        bottomValues_.insert(static_cast<VEGPredicate *>(b)->getVEG()->getVEGReference()->getValueId());
      }
    }
  }
}
...
ValueDesc::ValueDesc(ItemExpr *expr) : exprPtr_(expr), domId_(NULL) { ActiveSchemaDB()->insertValueDesc(this); }
...
// ***********************************************************************
void ValueDescArray::print(FILE *ofd, const char *indent, const char *title, NABoolean dontDisplayErrors) const {
#ifndef NDEBUG
  BUMP_INDENT(indent);
  ...
  for (CollIndex i = 0; i < entries(); i++) {
    ...
    if (vd && used(i)) {
      ...
      if (otyp > 0 || !dontDisplayErrors) fprintf(ofd, "%4d %p %4d: %s\n", i, ie, otyp, unparsed.data());
    }
  }
#endif
...
/*static*/ void ValueDescArray::Display(NABoolean dontDisplayErrors) {
  ActiveSchemaDB()->getValueDescArray().print(stdout, DEFAULT_INDENT, "Global ValueDescArray", dontDisplayErrors);
}
...
void ValueIdSet::replaceOneRowbyList(NormWA &normref) {
  ...
  for (exprId = init(); next(exprId); advance(exprId)) {
    ...
    if ((thisIE->getArity() >= 1) && (thisIE->containsOneRowAggregate()))
...
ValueId ValueIdList::extractVEGRefForEquiPredicate(ValueId x) const {
  ...
  switch (ie->getOperatorType()) {
    case ITM_EQUAL: {
      for (CollIndex i = 0; i < 2; i++)
        if (contains(ie->child(i)->getValueId())) return ie->child(i)->getValueId();
    ...
    case ITM_VEG_PREDICATE: {
      ...
      for (CollIndex i = 0; i < entries(); i++) {
        ...
        if (iei->getOperatorType() == ITM_INSTANTIATE_NULL) iei = iei->child(0)->castToItemExpr();

        if (iei->getOperatorType() == ITM_VEG_REFERENCE && ((VEGReference *)iei)->getVEG() == veg) {
          return at(i);
        }
      }
      ...
      for (CollIndex i = 0; i < vegMembers.entries(); i++) {
        ...
        if (iei->getOperatorType() == ITM_VEG_REFERENCE && this->contains(vegMembers[i])) return vegMembers[i];
      }
  ...
  switch (ie->getOperatorType()) {
    ...
    default: {
      if (ie->getOperatorType() == ITM_VEG_REFERENCE) {
        ...
        if (result.entries() == 0) ie->findAll(ITM_CACHE_PARAM, result, TRUE, FALSE);
  ...
  for (i = 0; i < entries(); i++) {
    ...
    if (constVal) {
      ...
      if (constVal->isNull()) {
        ...
        NAString *mmVal = NULL;
        if (val == "<min>") {
          type.minRepresentableValue(buf, &bufLen, &mmVal, CmpCommon::statementHeap());

          if ((type.getTypeQualifier() == NA_DATETIME_TYPE) || (type.getTypeQualifier() == NA_INTERVAL_TYPE)) {
          ...
          type.maxRepresentableValue(buf, &bufLen, &mmVal, CmpCommon::statementHeap());
        ...
        if (constType->getTypeQualifier() == NA_INTERVAL_TYPE) {
          ...
          ex_expr::exp_return_type retcode =
              convDoIt((char *)constVal->getConstValue(), constVal->getStorageSize(), (short)constType->getFSDatatype(),
                       constType->getPrecision(), constType->getScale(), strval,
                       t,                        // target length
                       REC_BYTE_F_ASCII,         // target type
                       0,                        // no char limit
                       SQLCHARSETCODE_ISO88591,  // ISO 8859-1
...
//---------------------------------------------------------------------------
void ValueIdSet::replaceCastExprWithOriginal(const ValueIdSet &originalOutputs, const RelExpr *parent) {
  ValueId vid, vid1;
  RelExpr *child = parent->child(0);
  ValueIdSet childOutputs(child->getGroupAttr()->getCharacteristicOutputs());

  for (vid = originalOutputs.init(); originalOutputs.next(vid); originalOutputs.advance(vid)) {
    if (vid.getItemExpr()->getOperatorType() == ITM_CAST) {
      ...
      // compute the case statement at the root
        ...
        if (!coveredValues.isEmpty()) {
          NABoolean found = TRUE;
          for (ValueId v = coveredValues.init(); coveredValues.next(v); coveredValues.advance(v)) {
...
//-----------------------------------------------------------------------
void ValueIdSet::replaceInstnullCastAggregateWithAggregateInLeftJoins(RelExpr *joinExpr) {
  if (joinExpr->getOperatorType() != REL_LEFT_JOIN) return;

  ValueId vid;
  for (vid = init(); next(vid); advance(vid)) {
    ItemExpr *ie = vid.getItemExpr();
    if (ie->getOperatorType() != ITM_INSTANTIATE_NULL) continue;
    if (ie->child(0)->getOperatorType() != ITM_CAST) continue;
...
ValueIdSet &ValueIdSet::intersectSetDeep(const ValueIdSet &v) {
  for (ValueId tgt = init(); next(tgt); advance(tgt)) {
    ...
    for (ValueId vid = v.init(); v.next(vid) && !found; v.advance(vid)) {
      if (vid.getItemExpr()->referencesTheGivenValue(tgt, FALSE, FALSE)) found = TRUE;  // keep it
    }
    if (!found) subtractElement(tgt);  // remove unreferenced element
  }
  return *this;
}
...
NABoolean ValueIdSet::isNotNullable(const ValueId &opd) {
  ...
  for (ValueId vid = init(); next(vid); advance(vid)) {
    ...
    switch (iePtr->getOperatorType()) {
      ...
      case ITM_GREATER:
      case ITM_GREATER_EQ:
        if (iePtr->child(0)->containsTheGivenValue(opd) || iePtr->child(1)->containsTheGivenValue(opd)) {
...
char *ValueIdList::computeEncodedKey(const TableDesc *tDesc, NABoolean isMaxKey, char *&encodedKeyBuffer,
  ...
  for (int j = 0; j < count; j++) {
    ...
    if (ie->getOperatorType() != ITM_CONSTANT) {
      ...
      if (*inputStrings[j] == "<min>" || *inputStrings[j] == "<max>") inputStrings[j] = NULL;
    }
  }

  const NAFileSet *naf = naTable->getClusteringIndex();
  TrafDesc *colDescs = (TrafDesc *)naTable->getColumnsDesc();
  ...
  if ((count > 0) && (inputStrings[0]) && (naTable->isHbaseCellTable() || naTable->isHbaseRowTable())) {
...
void ValueIdSet::findAllOpType(OperatorTypeEnum type, ValueIdSet &result) const {
  for (ValueId valId = init(); next(valId); advance(valId)) {
    ...
    if (itmExpr->getOperatorType() == type) result += valId;
  }
}

NABoolean ValueIdSet::containsOpType(OperatorTypeEnum type) const {
  ValueIdSet temp;
  for (ValueId valId = init(); next(valId); advance(valId)) {
    ItemExpr *itmExpr = valId.getItemExpr();
    if (itmExpr->getOperatorType() == type) return TRUE;
    itmExpr->findAll(type, temp);
  ...
  for (ValueId valId = init(); next(valId); advance(valId)) {
    ...
    if (itmExpr->getOperatorType() == ITM_OLAP_LEAD) {
      ...
      for (ValueId j = input.init(); input.next(j); input.advance(j)) {
        ...
        ItmLeadOlapFunction *lead = new (STMTHEAP) ItmLeadOlapFunction(child, me->getOffset());
...
void ValueIdSet::generatePushdownListForExtStorage(ExtPushdownPredInfoList &result) {
  result.clear();

  ValueIdSet noConstants;
  for (ValueId e = init(); next(e); advance(e)) {
    ItemExpr *ie = e.getItemExpr();

    // Check if it is a constant and if so, do not push it down, as
    // there is no simple way to express TRUE/FALSE as a predicate
    // in SearchArgument.Builder without some work in generator
    // to pass down both the IN and the BETWEEN predicate.
    NABoolean negateIt;
  ...
  if (noConstants.isEmpty()) return;

  // ItemExpr *newIE = noConstants.rebuildExprTree();
  // result.insertStartAND();
  // newIE->generatePushdownListForExtStorage(result);
  // result.insertEND();
  //
  result.insertStartAND();
  for (ValueId e = noConstants.init(); noConstants.next(e); noConstants.advance(e)) {
    ItemExpr *ie = e.getItemExpr();
    ie->generatePushdownListForExtStorage(result);
  }
  result.insertEND();
...
NABoolean ValueIdSet::containsHiveVirtualCols() const {
  ValueIdSet cols;
  findAllReferencedBaseCols(cols);
  for (ValueId e = cols.init(); cols.next(e); cols.advance(e)) {
    BaseColumn *bc = (BaseColumn *)e.getItemExpr();
    if (bc->getNAColumn()->isHiveVirtualColumn()) return TRUE;
  }
  return FALSE;
}

void ValueIdSet::removeBitMapInPredicate() {
  for (ValueId e = init(); next(e); advance(e)) {
    ItemExpr *ie = e.getItemExpr();

    if (ie->getOperatorType() == ITM_BITMAP_IN) (*this) -= e;
  }
}

// remove NotCovered expressions that appear in cols from this list.
void ValueIdList::removeNotCoveredExpr(const ValueIdSet &columns) {
  CollIndex count = entries();
  for (CollIndex i = 0; i < count; i++) {
    ItemExpr *ie = (*this)[i].getItemExpr();
    ItemExpr::removeNotCoveredFromExprTree(ie, columns, TRUE, TRUE);
    (*this)[i] = ie->getValueId();
  }
}

void ValueIdList::getBaseColumnList(ValueIdList &baseCols) {
  ValueId vid;
  for (CollIndex i = 0; i < entries(); i++) {
    if ((*this)[i].getItemExpr()->getOperatorType() == ITM_INDEXCOLUMN)
      vid = ((IndexColumn *)((*this)[i].getItemExpr()))->getDefinition();
    else
      vid = (*this)[i];

    baseCols.insertAt(baseCols.entries(), vid);
  }
}
...
NABoolean ValueIdList::clone(const ValueIdList &source, NAHeap *wheap) {
  for (int i = 0; i < source.entries(); i++) {
    ...
    if (ie->allValueIdsAreInvalid()) {
      ...
      this->insert(ie->getValueId());
    } else {
      cout << "Internal error: copyTree() does not remove the original valueId." << endl;
      ie->display();
      return FALSE;
    }
  }
...
void ValueDescArray::insert(ValueDesc *newElem) {
  newElem->valId_ = entries();
...
NABoolean ValueIdCostScalarArray::find(ValueId vid, CostScalar &cs) const {
  CollIndex idx = vidArray_.find(vid);

  if (idx == NULL_COLL_INDEX) return FALSE;
...
NABoolean ValueId::isConnectByPred() const {
  if (getItemExpr()->isConnectByExpr() == TRUE)
    return TRUE;
  else
    return FALSE;
}

NABoolean ValueId::isStartWithPred() const {
  if (getItemExpr()->isStartWithExpr() == TRUE)
    return TRUE;
  else
    return FALSE;
}
