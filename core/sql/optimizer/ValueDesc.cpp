                         ...
                         /*=__INVALID_DEFAULT_ATTRIBUTE*/)
{
  ...
  {
    if(((CharType&)originalType).getCharSet() != charSet)
      bSame = FALSE;
  }
  ...
  //charset-fix
  if (!bForce &&
      (originalType.getTypeQualifier() != NA_UNKNOWN_TYPE ||
  ...
  case NA_CHARACTER_TYPE:
    {
      ...
      if ((CmpCommon::getDefault(MODE_COMPATIBLE_1) == DF_ON) &&
          (ITM_CONSTANT == getItemExpr()->getOperatorType()))
        if (((ConstValue*)getItemExpr())->isNull())
          len = 0;// set NULL's length to 0
  ...
  case NA_NUMERIC_TYPE:
    {
        ...
        if (bignumIO)
        {
            ...
            desiredType = new (STMTHEAP)
                SQLBigNum(STMTHEAP, precision, scale, TRUE, TRUE, TRUE);
        }
        else
        {
            Lng32 scale = 6;
	 ...
	 if (((desiredType.getFSDatatype() == REC_BIN8_SIGNED) ||
              ...
              (CmpCommon::getDefault(TRAF_TINYINT_INPUT_PARAMS) == DF_OFF)))
	   {
		 ...
		 Int16 DisAmbiguate = 0;
               ...
               {
                 Lng32 newPrec = nTyp.decimalPrecision() ? nTyp.getPrecision() : MAX_HARDWARE_SUPPORTED_UNSIGNED_NUMERIC_PRECISION;
                 newType = new (STMTHEAP)
                   SQLBigNum(STMTHEAP, newPrec,
                             nTyp.getScale(),
                             FALSE,
                             FALSE,
                             nTyp.supportsSQLnull());
...
NABoolean ValueId::isAnValidRowidPredicate() const
{
  ...
  switch (arity)
    ...
    case 2:
      ...
      if (iePtr->child(0)->castToItemExpr()->getOperatorType() == ITM_HBASE_ROWID
           AND
           iePtr->child(1)->castToItemExpr()->getOperatorType() == ITM_CONSTANT)
        {
      ...
      else if (iePtr->child(0)->castToItemExpr()->getOperatorType() == ITM_CONSTANT
                  AND
                  iePtr->child(1)->castToItemExpr()->getOperatorType() == ITM_HBASE_ROWID)
        {
...
void addNewExpression(std::map<ValueId, ItemExpr *> & expressions,
                      ValueId colId,
                      ItemExpr * expr,
                      OperatorTypeEnum innerType)
{
  ...
  if (it == expressions.end())  // first column reference in a conjunct
    ...
    expressions[colId] = expr->connect2(innerType,
                    expressions[colId],
                    expr);
}
      ...
      if (interestingColumn)
        {
          ...
          addNewExpression(newExpressions,interestingColId,
                           pred,innerType);
        }
    }   
}
...
{  
      ...
      if (interestingColumn)
        {
          ...
          if (commonColumns.contains(interestingColId))
            { 
              ...
              addNewExpression(conjunctExpressions,interestingColId,
                               pred,innerType);                 
            }
        }
    }
  for (niceColumns.getFirst(d); niceColumns.next(d); niceColumns.advance(d))
    {
      newExpressions[d] = conjunctExpressions[d]->connect2(backboneType,
                    newExpressions[d],
                    conjunctExpressions[d]);
    }
                       ...
                       ) const
{
  ...
  for (CollIndex j = 0; j < entries(); j++)
  {
    ...
    if (at(j).getItemExpr())
      at(j).getItemExpr()->unparse(unparsed,
                                   DEFAULT_PHASE,
...
void ValueIdList::display() const
{
  print();
}
...
void
ValueIdSet::removeIsNullIsNotNullExprs(const ValueIdSet & nullPredsCols, ValueIdSet& isNullIsNotNullPred)
{
  NABoolean coverFlag;
  ValueIdSet referencedInputs;
  GroupAttributes emptyGA;
...
NABoolean ValueIdSet::newConstantsAreDistinct(
     const ValueIdSet &newConstValues) const
{
  ...
  NAText sql;
  ItemExprList args(2 * numComparisons,
                    CmpCommon::statementHeap());

  for (ValueId i = init(); next(i); advance(i))
    for (ValueId j = newConstValues.init();
         newConstValues.next(j);
         newConstValues.advance(j))
      {
        char buf[64];

        snprintf(buf, sizeof(buf), "@A%d <> @A%d",
                 args.entries()+1,
                 args.entries()+2);
  ...
  ItemExpr *notEqualExpr = parser.getItemExprTree(
       sql.c_str(),
       sql.length(),
       CharInfo::ISO88591,
       0,
      ...
      if (pred->getOperatorType() == ITM_VEG_REFERENCE)
       ...
       else 
        {
          ...
          if (pred->getOperatorType() == ITM_LESS ||
              pred->getOperatorType() == ITM_GREATER)
            {
     ...
     NABoolean replaceVEGPreds) const
{
  ...
  // exit.
  for (ValueId v=rewrittenPreds.init();
       rewrittenPreds.next(v);
       rewrittenPreds.advance(v))
    {
...
{
  if ( !wheap )
    wheap = CmpCommon::context()->statementHeap();
  ...
  ex_expr::exp_return_type rc =
    keyEncodeExpr->getExpr()->eval(workAtp, 0, wheap);
...
static NABoolean isCompareWithNullConstant(const ItemExpr* pItemExpr)
{
  ...
  if ((pItemExpr->getArity() == 2) &&
      ...
      (pItemExpr->getOperatorType() == ITM_EQUAL
       || pItemExpr->getOperatorType() == ITM_NOT_EQUAL
       || pItemExpr->getOperatorType() == ITM_LESS
       || pItemExpr->getOperatorType() == ITM_LESS_EQ
       || pItemExpr->getOperatorType() == ITM_GREATER
       || pItemExpr->getOperatorType() == ITM_GREATER_EQ))
    return TRUE;
  return FALSE;
}
...
NABoolean ValueIdList::constantFolding()
{
  ...
  for (CollIndex i = 0; i < list.entries(); i++)
  {
    ...
    {
      isPredFalse = TRUE;
      predicate = new (CmpCommon::statementHeap())
                BiRelat(ITM_EQUAL,
                      new (CmpCommon::statementHeap()) ConstValue(1),
...
NABoolean ValueIdSet::containsHiveVirtualCols() const
{
   ...
   { 
      ...
      if ( bc->getNAColumn()->isHiveVirtualColumn() )
        return TRUE;
   }
   return FALSE;
}
...
void ValueIdSet::removeBitMapInPredicate()
{
   for (ValueId e=init(); next(e); advance(e)) 
   {
      ...
      if  ( ie->getOperatorType() == ITM_BITMAP_IN )
         (*this) -= e;
   }
}
...
ValueIdArray::ValueIdArray (const ValueIdList & orig, Lng32 numberOfElements ) :
  ...
  {
    for (CollIndex i = 0; i < orig.entries(); i++)
      insertAt(i,orig.at(i));
  }

NABoolean 
ValueIdCostScalarArray::find(ValueId vid, CostScalar& cs) const
{
  CollIndex idx = vidArray_.find(vid);

  if ( idx == NULL_COLL_INDEX )
    return FALSE;
...
{
  if(getItemExpr()->isConnectByExpr() == TRUE) return TRUE;
  else return FALSE;
}

NABoolean ValueId::isStartWithPred() const
{
  if(getItemExpr()->isStartWithExpr() == TRUE) return TRUE;
  else return FALSE;
}
