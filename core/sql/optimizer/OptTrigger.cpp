...
void OptTriggersBackbone::SetUnionCharacteristicInputs(RelExpr *pUnion) {
...
void OptTriggersBackbone::SetTsjCharacteristicInputs(RelExpr *pTsj) {
  // add left
...
void OptTriggersBackbone::PrepareNewBindWA(BindWA &bindWA, TriggerBindInfo *bindInfo) {
  ...
  ColRefName *execIdName = new (CmpCommon::statementHeap()) ColRefName(InliningInfo::getExecIdVirtualColName());
...
RelExpr *OptTriggersBackbone::BindAndNormalizeNewlyConstructedTree(BindWA *bindWA, RelExpr *topNode) {
  // Bind the tree.
...
OptTrigger::OptTrigger(SubTreeAccessSet *treeAccessSet, NABoolean isRowTrigger, RelExpr *triggerSubTree,
...
RelExpr *OptTriggerList::createUnionSubTree() const {
  ...
  for (CollIndex i = 1; i < entries(); i++) {
    ...
    if (pUnion == NULL) {
      pUnion = new (CmpCommon::statementHeap()) Union(pPrev, (RelExpr *)optTrigger->getTriggerSubTree(), NULL, NULL,
...
OptTriggerGroup::OptTriggerGroup(TriggerGroupFlags flag, OptTriggersBackbone *backbone)
    : groupAccessSet_(CmpCommon::statementHeap()), Flags_(flag), backbone_(backbone) {}
...
NABoolean OptTriggerGroup::isConflicting(const SubTreeAccessSet *accessSet) const {
  // If the group is empty or the access set is null return FALSE - no conflict
  if (entries() == 0 || accessSet == NULL) return FALSE;
...
void OptTriggerGroup::addTrigger(const OptTriggerPtr trigger) {
...
RelExpr *OptTriggerGroup::toTree() {
  ...
  if (pStatmentUnion == NULL) return pRowSubTree;
  if (pRowSubTree == NULL) return pStatmentUnion;

  // Connect row trigger sub-tree and statement trigger sub-tree with union
  RelExpr *pUnion = new (CmpCommon::statementHeap())
      Union(pRowSubTree, pStatmentUnion, NULL, NULL, REL_UNION, CmpCommon::statementHeap(), TRUE);
...
RelExpr *OptTriggerGroup::createRowTriggersSubTree() {
...
//
//-----------------------------------------------------------------------------------
RelExpr *OptTriggerGroup::buildTempScanTree() {
  ...
  OptColumnMapping colMapping(bindInfo, backbone_->getTriggeringNode()->getOperatorType(), CmpCommon::statementHeap());
  ...
  topNode = new (CmpCommon::statementHeap()) RelRoot(topNode, REL_ROOT, selectList);
...
RelExpr *OptTriggerGroup::buildTempTableScanNodes(OptColumnMapping &colMapping, BindWA *bindWA) {
...
RelExpr *OptTriggersBackbone::getTransformedTree() {
  RelExpr *backboneRoot = (hasBeforeTriggers() ? beforeTriggerDrivingNode_ : tempDeleteDrivingNode_);
...
NABoolean OptTriggersBackbone::hasConflicts() const {
  if (statmentTriggerDrivingNode_) {
    if (statmentTriggerDrivingNode_->getAccessSet0()->isConflicting(statmentTriggerDrivingNode_->getAccessSet1()))
      return TRUE;

    if (leftTreeHasConflicts(statmentTriggerDrivingNode_)) return TRUE;
  }

  if (pipeLineDrivingNode_) {
    if (pipeLineDrivingNode_->getAccessSet0()->isConflicting(pipeLineDrivingNode_->getAccessSet1())) return TRUE;

    if (rowTriggerDrivingNode_) {
      // conflict with RI
      if (rowTriggerDrivingNode_->getAccessSet0()->isConflicting(rowTriggerDrivingNode_->getAccessSet1())) return TRUE;

      if (leftTreeHasConflicts(rowTriggerDrivingNode_)) return TRUE;
    }
    //
...
NABoolean OptTriggersBackbone::leftTreeHasConflicts(RelExpr *drivingNode) const {
  for (RelExpr *pUnion = (RelExpr *)drivingNode->getChild(1); pUnion; pUnion = (RelExpr *)pUnion->getChild(0)) {
    // a trigger - end of sub tree
    if (pUnion->getInliningInfo().isTriggerRoot()) return FALSE;
    ...
    if (pUnion->getAccessSet0()->isConflicting(pUnion->getAccessSet1())) return TRUE;
  }
  return FALSE;
}
...
void OptTriggersBackbone::createTriggerList() {
  triggerList_ = new (CmpCommon::statementHeap())(OptTriggerList);

  if (statmentTriggerDrivingNode_) addToTriggerList(statmentTriggerDrivingNode_, FALSE);

  if (rowTriggerDrivingNode_) addToTriggerList(rowTriggerDrivingNode_, TRUE);
}
...
void OptTriggersBackbone::removeTempInsert() {
...
RelExpr *OptTriggersBackbone::removeTempDelete() {
  if (hasBeforeTriggers()) {
    // The removal of temp delete in before trigger is used only as debug option
    beforeTriggerDrivingNode_->setChild(1, (ExprNode *)tempDeleteDrivingNode_->getChild(0));
...
void OptTriggersBackbone::addToTriggerList(RelExpr *drivingNode, NABoolean isRowTrigger) {
  ...
  for (RelExpr *pUnion = (RelExpr *)drivingNode->getChild(1); pUnion; pUnion = (RelExpr *)pUnion->getChild(0)) {
    if (pUnion->getInliningInfo().isTriggerRoot()) {
      ...
      trigger = new (CmpCommon::statementHeap()) OptTrigger(treeAccessSet, isRowTrigger, pUnion, NULL);
    ...
    trigger = new (CmpCommon::statementHeap())
        OptTrigger(treeAccessSet, isRowTrigger, (RelExpr *)pUnion->getChild(1), pUnion);
    triggerList_->insert(trigger);
...
void OptTriggersBackbone::reorder() {
  ...
  for (idx = 0; idx < triggerList_->entries(); idx++) optTriggerPtrArray[idx] = (*triggerList_)[idx];
  ...
  for (Int32 i = 1; i < triggerArraySize; i++) {
    ...
    for (Int32 j = i; j > 0; j--) {
      ...
      if (accessSet1->isConflicting(accessSet2)) break;  // Conflict - can't move the current trigger any more
...
void OptTriggersBackbone::group() {
  //
  ...
  if (riDrivingNode_) riAccessSet = riDrivingNode_->getAccessSet0();

  OptTriggerGroup *pPipelinedGroup =
      new (CmpCommon::statementHeap()) OptTriggerGroup(OptTriggerGroup::PIPELINED_ROW_TRIGGERS, this);
  OptTriggerGroup *pIudParallelGroup =
      new (CmpCommon::statementHeap()) OptTriggerGroup(OptTriggerGroup::IUD_PARALLEL_STATEMENT_TRIGGER, this);
  OptTriggerGroup *pBlockedGroup =
      new (CmpCommon::statementHeap()) OptTriggerGroup(OptTriggerGroup::BLOCKED_GROUP, this);
  ...
  for (i = 0; i < triggerList_->entries(); i++) {
    ...
    if (currentTrigger->getAccessSet()->isConflicting(iudAccessSet) ||
        (riAccessSet && currentTrigger->getAccessSet()->isConflicting(riAccessSet))) {
  ...
  if (pPipelinedGroup->entries()) triggerGroups_->insert(pPipelinedGroup);
  if (pIudParallelGroup->entries()) triggerGroups_->insert(pIudParallelGroup);
  ...
  if (pBlockedGroup->entries())
    ...
    currentGroup = new (CmpCommon::statementHeap()) OptTriggerGroup(OptTriggerGroup::BLOCKED_GROUP, this);
  }
  ...
  if (currentGroup->entries()) triggerGroups_->insert(currentGroup);
}
...
RelExpr *OptTriggersBackbone::cleanDummies(RelExpr *parent) {
  RelExpr *leftChild = (RelExpr *)parent->getChild(0);
  RelExpr *rightChild = (RelExpr *)parent->getChild(1);

  if (leftChild->getInliningInfo().isDummy() && rightChild->getInliningInfo().isDummy()) return NULL;

  if (!leftChild->getInliningInfo().isDummy() && !rightChild->getInliningInfo().isDummy()) return parent;

  if (leftChild->getInliningInfo().isDummy()) return rightChild;
  return leftChild;
}
...
void OptTriggersBackbone::replacePipelinedRowTriggers(RelExpr *rowTriggerTree) {
...
RelExpr *OptTriggersBackbone::toTree() {
  // Rebuild the tree bottom up
  RelExpr *childNode = iudDrivingNode_;

  if (tempInsertDrivingNode_) childNode = tempInsertDrivingNode_;
  ...
  for (CollIndex i = 0; i < triggerGroups_->entries(); i++) {
    ...
    Union *pOrder = new (CmpCommon::statementHeap())
        Union(childNode, triggersUnion, NULL, NULL, REL_UNION, CmpCommon::statementHeap(), TRUE);
    if (optTriggerGroup->isBlockedGroup()) pOrder->setOrderedUnion();

    BindWA *bindWA = (drivingBackbone_->getRETDesc())->getBindWA();

    if (bindWA && bindWA->getHostArraysArea() && bindWA->getHostArraysArea()->getTolerateNonFatalError()) {
  ...
  if (hasBeforeTriggers()) return (beforeTriggerDrivingNode_);
...
void OptTriggersBackbone::forceCardinalityAsIud(RelExpr *expr) const {
  CostScalar card = getIudDrivingNode()->getGroupAttr()->getResultCardinalityForEmptyInput();
  expr->getInliningInfo().BuildForceCardinalityInfo(1.0,  // factor ignored
                                                    card.getValue(), CmpCommon::statementHeap());
}
...
void OptTriggersBackbone::fixCardinalityAndCollectTempUsage(RelExpr *rootNode, NABoolean enterInnerBackbone) {
  // stop condition
  ...
  if (info.isDrivingBeforeTriggers() || (info.isDrivingTempDelete() && !info.isBeforeTriggersExist())) {
    backboneRoot = TRUE;
  }
...
void OptTriggersBackbone::collectTempTableUsage(RelExpr *node) {
  ...
  for (ValueId vid = preds.init(); preds.next(vid); preds.advance(vid)) {
    ...
    for (ValueId vid2 = valuesInVEG.init(); valuesInVEG.next(vid2); valuesInVEG.advance(vid2)) {
...
void OptTriggersBackbone::fixTempInsertSubtree() {
  ...
  if ((tempTableUsage_ == (ChangesTable::INSERTED_ROWS | ChangesTable::DELETED_ROWS)) ||
      (obindWA->getHostArraysArea() && !obindWA->getHostArraysArea()->getTolerateNonFatalError())) {
  ...
  retDesc->addColumns(&bindWA, *(triggerBindInfo_->getIudColumnList()), USER_AND_SYSTEM_COLUMNS);
  ...
  CmpContext::InternalCompileEnum savedMode = CmpCommon::context()->internalCompile();
...
void OptTriggersBackbone::initBeforeTriggerDrivingNode() {
...
void OptTriggersBackbone::initTempDeleteDrivingNodeAndAdvance() {
...
void OptTriggersBackbone::initStatmentTriggerDrivingNodeAndAdvance() {
...
void OptTriggersBackbone::initPipeLineDrivingNode() {
  if (pNext_->getInliningInfo().isDrivingPipelinedActions()) {
...
void OptTriggersBackbone::initRowTriggerDrivingNode() {
...
void OptTriggersBackbone::initRiDrivingNode() {
  CMPASSERT(pNext_->getInliningInfo().isDrivingPipelinedActions())
  RelExpr *e = pNext_->getChild(1)->castToRelExpr();
...
GenericUpdate *OptTriggersBackbone::findLeftmostGU(ExprNode *expr) {
  GenericUpdate *guNode = NULL;
  CMPASSERT(expr)

  if (expr->getOperator().match(REL_ANY_GEN_UPDATE)) {
    if (((GenericUpdate *)expr)->getUpdateCKorUniqueIndexKey()) return (GenericUpdate *)expr;

    if (expr->getOperator().match(REL_UNARY_INSERT)) {
      Insert *ins = (Insert *)((GenericUpdate *)expr);
      if (ins->systemGeneratesIdentityValue()) return (GenericUpdate *)expr;
    }
...
void OptTriggersBackbone::advanceInTree() {
  pParent_ = pNext_;
...
//-----------------------------------------------------------------------------------
//-----------------------------------------------------------------------------------
OptColumnMapping::OptColumnMapping(TriggerBindInfo *bindInfo, OperatorTypeEnum iudOpType, CollHeap *heap)
    : bindInfo_(bindInfo),
...
//-----------------------------------------------------------------------------------
OptColumnMapping::ColumnMapping *OptColumnMapping::findEqualCols(MappingList &colsList, ColumnMapping *colToFind,
                                                                 CollIndex startAt) {
  for (CollIndex i = startAt; i < colsList.entries(); i++) {
    ColumnMapping *currentCol = colsList[i];
    if (colToFind->topVeg_ == currentCol->topVeg_) return currentCol;
  }
...
void OptColumnMapping::findCommonColumns() {
  ...
  if (!oldCols_.isEmpty()) {
    for (i = 0; i < oldCols_.entries() - 1; i++) {
      ...
      while ((redundantCol = findEqualCols(oldCols_, currentCol, i + 1)) != NULL) oldCols_.remove(redundantCol);
    }
  }
  ...
  if (!newCols_.isEmpty()) {
    for (i = 0; i < newCols_.entries() - 1; i++) {
      ...
      // Search the mappings after currentCol in the list
      while ((redundantCol = findEqualCols(newCols_, currentCol, i + 1)) != NULL) newCols_.remove(redundantCol);
    }
  }
...
ItemExpr *OptColumnMapping::buildSelectListFromNeededInputs() {
  ...
  for (CollIndex i = 0; i < mappingArray_.entries(); i++) {
    ...
    ItemExpr *colRef = new (heap_) ColReference(new (heap_) ColRefName(*currentCol->colName_, heap_));
...
RelExpr *OptColumnMapping::buildMapValueIdNode(RelExpr *tempScanTree) {
  ...
  for (CollIndex i = 0; i < mappingArray_.entries(); i++) {
    ...
    ValueId bottomValueId = bottomRetDesc->findColumn(*currentCol->colName_)->getValueId();

    // Find the VEG from the outputs of the temp Scan tree, that references
    // the ValueId we found in the RETDesc.
    ValueId bottomVeg;
    if (!tempScanOutputs.referencesTheGivenValue(bottomValueId, bottomVeg)) CMPASSERT(FALSE);
