...
GenericUpdate(const CorrName &name, 
  ...
  // update using upsert
  NABoolean isUpdUsingUpsert() const
  { return (flags_ & UPD_USING_UPSERT) != 0; }
  void setUpdUsingUpsert(NABoolean flag = TRUE)
  { (flag ? flags_ |= UPD_USING_UPSERT : flags_ &= ~UPD_USING_UPSERT); }
  ...
  virtual NABoolean transformedUpdateNoDtmXn() const
  {
    return FALSE;
  }
  ...
  ItemExpr * getCheckCnstrntOfPartv2BaseTable() const
                                             { return checkCnstrntOfPartv2BaseTable_; }
  void setCheckCnstrntOfPartv2BaseTable(ItemExpr * expr)
                                             { checkCnstrntOfPartv2BaseTable_ = expr; }
  ...
  inline void setPartQualPreCond(const ValueIdSet pc)
           { partQualPreCond_ = pc; exprsInDerivedClasses_ += partQualPreCond_; }
...
public:
  ...
  void setInsertType(const InsertType &insType) 
  {
	...
	(insType == UPSERT_LOAD) ||
        (insType == UPSERT_NO_DUP) ||
	(noCheck()))
      {
	if ((insType == UPSERT_LOAD)||(insType == UPSERT_NO_DUP))
	  insertType_ = insType;
  ...
  NABoolean withNoConflictCheck() const
  { return noConflictCheck_; }
  void setWithNoConflictCheck(NABoolean flag = TRUE)
  { noConflictCheck_ = flag; }
  ...
  NABoolean noRepeatableRead() const
  { return noRepeatableRead_; }
  void setNoRepeatableRead(NABoolean flag = TRUE)
  { noRepeatableRead_ = flag; }
  ...
  virtual NABoolean transformedUpdateNoDtmXn() const
  {
    return transformedUpdateNoDtmXn_;
  }
  void setTransformedUpdateNoDtmXn(NABoolean x)
  {
    transformedUpdateNoDtmXn_ = x;
  }
  ...
  void setPartColValList(ItemExprList *partColVals)
  {partColVals_ = partColVals;}
...
public:
  ...
  virtual RelExpr *xformPartitionPruning(BindWA* bindWA, RelExpr *gu=NULL,
