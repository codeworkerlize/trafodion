  ...
  isRedundant_ =
    (sppOfChild->getPlanExecutionLocation() ==
     sppForMe->getPlanExecutionLocation() AND
      ...
      AND getScanFilterInputs().entries() == 0
    ) 
     ||
...
ValueIdList NestedJoin::genWriteOpLeftChildSortReq()
{
  ...
  if (!getReqdOrder().isEmpty())
  {
    ...
    for (UInt32 i = 0; i < reqdOrder.entries(); ++i)
      if ( !nestReqdOrder.contains(reqdOrder[i]))
        nestReqdOrder.insert(reqdOrder[i]);
        ...
        )
    {
      ...
      if ( OCRJoinIsConsideredInCase2 )
      {
         ...
         if ( naTable && getNATableForRightChild()->isHiveTable() ) {
          ...
          NABoolean isRoundRobin = (ActiveSchemaDB()->getDefaults().
                getAsLong(USE_ROUND_ROBIN_FOR_RANDOMNUM) == 3);
...
NABoolean NestedJoin::JoinPredicateCoversChild1PartKey() const
{
   ...
   if ( nt && nt->isHiveTable() ) {
      ...
      if ( idesc && JoinPredicateCoversChild1PartKey(((IndexDesc *)idesc)->getHivePartCols()) )
         return TRUE;
   } 
        ...
        currentPlanIsAcceptable(planNumber,rppForMe))
    {
      ...
      if((childContext != NULL) AND childContext->hasOptimalSolution())
      {
        ...
        if (childPartFunc->isASinglePartitionPartitioningFunction())
          ...
          double broadcastInnerCap = ActiveSchemaDB()->getDefaults().
                                  getAsDouble(HJ_BROADCAST_INNER_THRESHOLD_MB);

          if ( broadcastInnerCap >= 0) {
            ValueIdSet allPreds(selectionPred());
            allPreds += getJoinPred();
            isEquiJoin = 
                ((allPreds.isEmpty() && getEquiJoinPredicates().entries() > 0) ||
                  getEquiJoinPredicates() == allPreds);
          }

          if ( (isJoinForZigZag() && innerMaxSize >= 
                outerSize * childPartFunc->getCountOfPartitions()) ||
               (isEquiJoin &&
                innerSizeMB > broadcastInnerCap && 
                childPlansToConsider > 2)||
                (broadcastInnerCap < 0)
             )
            {
                                   ...
                                   Lng32& childIndex)
{
  ...
  if (isRollup())
   ...
   if ( !parallelAggrPushdown() &&
       isFeasibleToTransformForAggrPushdown() == GroupByAgg::TVL_TRUE )
      rg.addNoEspExchangeRequirement();
...
PhysicalFastExtract::PhysicalFastExtract(RelExpr * child,
                                         CollHeap *oHeap /*= CmpCommon::statementHeap()*/)
: FastExtract(child, oHeap), concurrentHivePartitionWrites_(FALSE)
{
  if (oHeap)
    pCostMethod_ = new (oHeap) CostMethodFastExtract();
  else
    pCostMethod_ = new (CmpCommon::statementHeap()) CostMethodFastExtract();
}

//<pb>
Context * PhysicalFastExtract::createContextForAChild(Context* myContext,
                                 PlanWorkSpace* pws,
                                 Lng32& childIndex)
{
  const ReqdPhysicalProperty* rppForMe =
  ...
  if (isHiveInsert() && getIsPartitioned())
    {
      Lng32 numESPsForRepart =
        ActiveSchemaDB()->getDefaults().getAsLong(HIVE_PARTITION_INSERT_REPART);

      if (numESPsForRepart > 1)
      {
        const ValueIdList &cols(getHiveTableDesc()->getColumnList());
        ValueIdSet partCols;

        for (CollIndex i=0; i<cols.entries(); i++)
          if (dynamic_cast<BaseColumn *>(cols[i].getItemExpr())->getNAColumn()->isHivePartColumn())
            {
              partCols += getSelectList()[i];
            }

        if (partCols.entries())
          {
            rg.addPartitioningKey(partCols);
            rg.addNumOfPartitions(numESPsForRepart);
          }
      }
      else if (numESPsForRepart == 1)
        //Mantis 738
        //If the hive table is partitioned, there is an issue to create
        //partition concurrently by multiple ESPs at same time, so before
        //we fix the root issue, force a serial plan here
        rg.addNumOfPartitions(1);

      // numESPsForRepart <= 0 means don't repartition the rows on the
      // partitioning key and don't force a serial plan
    }
...
} // PhysicalFastExtract::createContextForAChild()
                                ...
                                Lng32& childIndex)
{
     ...
     )
  {
    ...
    // handle fast extract
    Lng32 hiveInsertRepartitionDoP = 
        ActiveSchemaDB()->getDefaults().getAsLong(HIVE_PARTITION_INSERT_REPART);

    if ( childOpType == REL_FAST_EXTRACT )
    {
       FastExtract* fastExtract = (FastExtract*)(x->castToRelExpr());

       // use the CQD value (if > 0) for countOfCPUs when fast extract 
       // into a hive table. This is to avoid error 2235 (Pass one skipped, 
       // but cannot produce a plan in pass two), since a countOfCPUs of 1
       // will never satisfy any parallel requirements as requested when
       // when hiveInsertRepartitionDoP > 1.
       if ( fastExtract->isHiveInsert() && 
            hiveInsertRepartitionDoP > 0 ) {
          pipelinesPerCPU = 1;
          countOfCPUs = hiveInsertRepartitionDoP;
...

{
  ...
  CostScalar totalCPUsExecutingDP2s = 
                 CURRSTMT_OPTDEFAULTS->getTotalNumberOfCPUs();
     ...
     ValueIdList   &sortOrderVEG)
{
  ...
  PlanExecutionEnum location = rppForMe->translatePlanExecutionLocation();
  ...
  Lng32 maxESPsCap = numSQNodes; 
  if ( !fakeEnv ) 
    maxESPsCap = (Lng32) ceil(((double) maxESPsCap) * numESPsPerDataNode);
  ...
  if (partReq) {
     ...
     // We can not change scan dop when a partition requirement is present
     // because in the case of aggregate push-down, the dop is 1 and
     // should be kept that way in the scan.
     if ( !fakeEnv )
     {
     ...
     if (!fakeEnv) {
       ...
       // forced dop case, when there is no part requirement.
       maxESPsCap = minESPs = maxESPs = CmpCommon::context()->getNumOfSMPs();
     }
  }

  // limit the number of ESPs to HIVE_NUM_ESPS_PER_DATANODE * nodes
   maxESPs = MAXOF(MINOF(maxESPsCap, maxESPs),1);
  ...
  NABoolean useLocality =
    (NodeMap::useLocalityForHiveScanInfo() &&
     hiveSearchKey_->getHDFSTableStats()->blocksAreOnLocalCluster());
  ...
  if ( canFreelyAdjustDoP ) {
    ...
    //     ratio = 2
    //     minNodes = 10
    Int32 ratio = getDefaultAsLong(HIVE_APPLY_LOCALITY_NODE_RATIO);
    Int32 minNumOfNodes = 
          getDefaultAsLong(HIVE_APPLY_LOCALITY_MIN_NODES);
    if (useLocality &&
        maxESPs >= numSQNodes &&
        (numESPs > numSQNodes / ratio ||
        (numSQNodes > minNumOfNodes && numESPs > numSQNodes - minNumOfNodes)))
       ...
       rpnb->getParentPartFunc() ) 
  {
     ...
     // if operate in AS mode, adjust the numESPs to a multiple of numSQNodes
     // or a factor of numSQNodes.
 ...
 // my required phys props (always non-NULL)
  ...
  if ( partReq && partReq->castToRequireExactlyOnePartition() ) {
    myPartFunc = new (CmpCommon::statementHeap())
     ...
     NABoolean fakeEnv = 
        (defs.getToken(PARALLEL_NUM_ESPS, 0) != DF_SYSTEM );
        ...
        if ( !fakeEnv ) {
           ...
           if (canFreelyAdjustDoP ) {
                  ...
                  scanSize >= numPartitionsAsDopThreshold )
             {
               ...
               if ( getDefaultAsLong(AS_AFFINITY_VALUE) != -2 && 
                    ixDescPartFunc ) 
               {
                                       ...
                                       PlanWorkSpace  *pws)
{
  ...
  if ( doParallelDDL ) {
    ...
    if ( nodeSet )
       nodeIdSet = nodeSet->castToNAWSetOfNodeIds();
  ...
  if ( nodeIdSet ) {
    ...
    if ( nodeIdsSpecifiedInDDL.entries() > 0 ) {
       ...
       NABoolean ok = 
           nodeIdArray.intersect(nodeIdsSpecifiedInDDL, finalNodeIdArray);
    ...
    //----------------------------------------------------------
    myNodeMap = new(heap)
       NodeMap(CmpCommon::statementHeap(), countOfPartitions, finalNodeIdArray);
  ...
  if ( nodeList ) {
    ...
    myNodeMap = new(heap) NodeMap(CmpCommon::statementHeap(), 
                                  countOfPartitions,
   	                          ddlExpr->getNodeIdsSpecified());
  } else {
  //----------------------------------------------------------
  ...
  //----------------------------------------------------------
     myNodeMap = new(heap)
       NodeMap(CmpCommon::statementHeap(), countOfPartitions, NodeMapEntry::ACTIVE);
  }
  ...
  if ( doSerialPlan ) 
      myPartFunc = 
         new (heap) SinglePartitionPartitioningFunction(myNodeMap);
  else {
      ...
      myPartFunc = 
         new (heap) ReplicateNoBroadcastPartitioningFunction(
                                                    countOfPartitions,
                                                    myNodeMap,
                                                    heap);
  ...
  PhysicalProperty * sppForMe = new(heap)
    PhysicalProperty(myPartFunc,
                     location,
                     SOURCE_VIRTUAL_TABLE);
                                   ...
                                   PlanWorkSpace  *pws)
{
  ...
  PhysicalProperty * sppForMe =
    new(CmpCommon::statementHeap())
    PhysicalProperty(myPartFunc,
                     existTrigger ? EXECUTE_IN_MASTER : EXECUTE_IN_MASTER_AND_ESP,
                     SOURCE_VIRTUAL_TABLE);
...
PhysCompoundStmt::PhysCompoundStmt(RelExpr *leftChild,
                                   RelExpr *rightChild,
...
PhysCompoundStmt::PhysCompoundStmt(const CompoundStmt &other) :
CompoundStmt(other) 
{
  pCostMethod_ = new (CmpCommon::statementHeap()) CostMethodCompoundStmt();
}
  ...
  if (udfDoP != 0 && udfDoP != numOfESPs && !numOfESPsForced)
    {
      ...
      switch (udfDoP)
        ...
        case tmudr::UDRPlanInfo::ONE_INSTANCE_PER_NODE:
          ...
          numOfESPs =
          udfDoP =
          maxDoP = CURRCONTEXT_CLUSTERINFO->getTotalNumberOfCPUs();
          numOfESPsForced = TRUE;
          allowedDeviation = 0.0;
          result = TRUE;
          break;
...
PhysicalTableMappingUDF::PhysicalTableMappingUDF(int arity,
                                                 CollHeap *oHeap /*= CmpCommon::statementHeap()*/)
...
Context * ConnectBy::createContextForAChild(Context* myContext,
                                 PlanWorkSpace* pws,
                                 Lng32& childIndex)
{
  SortOrderTypeEnum childSortOrderTypeReq = NO_SOT;
  ...
  {
    ValueIdList newSortKey;
    getMap(childIndex).rewriteValueIdListDown(reqdOrder(),
                                                  newSortKey);
    rg.addSortKey(newSortKey,
                      ESP_SOT,
                      dp2SortOrderPartReq);
...
PhysicalProperty*
ConnectBy::synthPhysicalProperty(const Context* context,
                              const Lng32     planNumber,
                              PlanWorkSpace  *pws)
{
  // ---------------------------------------------------------------------
...
PhysicalProperty*
ConnectByTempTable::synthPhysicalProperty(const Context* context,
                              const Lng32     planNumber,
                              PlanWorkSpace  *pws)
{
  // ---------------------------------------------------------------------
...
Context * ConnectByTempTable::createContextForAChild(Context* myContext,
                                 PlanWorkSpace* pws,
                                 Lng32& childIndex)
{
  const ReqdPhysicalProperty* rppForMe =
...
PhysicalProperty*
QueryInvalidationFunc::synthPhysicalProperty(const Context* myContext,
                                      const Lng32     planNumber,
                                      PlanWorkSpace  *pws)
{
