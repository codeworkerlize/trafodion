...
// -----------------------------------------------------------------------
CascadesGroup::CascadesGroup(CascadesGroupId groupId, GroupAttributes *groupAttr)
...
void CascadesGroup::print(FILE *f, const char *prefix, const char *) const {}  // CascadesGroup::print
...
void CascadesGroup::addLogExpr(RelExpr *expr, RelExpr *src) {
  ...
  if (expr->getArity() == 1 && CmpCommon::getDefault(GA_PROP_INDEXES_ARITY_1) == DF_ON) {
    expr->getGroupAttr()->addToAvailableBtreeIndexes(expr->child(0).getGroupAttr()->getAvailableBtreeIndexes());
  }
  ...
  if (expr) expr->setCascadesTraceInfo(src);
...
NABoolean CascadesGroup::addPhysExpr(RelExpr *&expr, RelExpr *src) {
  ...
  if (getFirstLogExpr()) expr->setBlockStmt(getFirstLogExpr()->isinBlockStmt());
...
RelExpr *CascadesGroup::unlinkLogExpr(RelExpr *expr) {
  ...
  if (pred == expr) {
    ...
    while (pred != NULL AND pred->getNextInGroup() != expr) {
...
void CascadesGroup::merge(CascadesGroup *other) {
  ...
  if (other->groupId_ < groupId_) {
    ...
    for (next_expr = other->logExprs_; (move_expr = next_expr) != NULL;) {
      next_expr = next_expr->getNextInGroup();
      addLogExpr(move_expr);
    }  // insert all expressions of "other" into "this"

    for (next_expr = other->physExprs_; (move_expr = next_expr) != NULL;) {
      next_expr = next_expr->getNextInGroup();
    ...
    for (int i = 0; i < maxg; i++) {
      ...
      if (NOT found) goals_.insert(other->goals_[i]);
    }
...
//<pb>
/* ============================================================ */
Context *CascadesGroup::shareContext(const ReqdPhysicalProperty *const reqdPhys,
  ...
  for (int i = 0; i < maxc AND NOT found; i++) {
    ...
    if (NOT existingContext->isADuplicate() AND newContext->compareContexts(*existingContext) == SAME) {
      if (existingContext->getOutstanding() == 0) {
        ...
        if (NOT result->optimizedInCurrentPass()) {
          ...
          if (costLimit AND result->getSolution() AND CURRSTMT_OPTDEFAULTS->OPHreduceCLfromPass1Solution())
            costLimit->tryToReduce(*(result->getCostForSolution()), result->getReqdPhysicalProperty());
        ...
        //     solution, reuse it as-is.
        else if (result->hasOptimalSolution()) {
          ...
          if (costLimit AND(costLimit->compareWithPlanCost((CascadesPlan *)result->getSolution(),
        ...
        // not contain  an optimal solution
          ...
          if (costLimit->compareCostLimits(result->getCostLimit(), reqdPhys) == MORE) {
        ...
        while (ancestor != NULL AND ancestor != existingContext) ancestor = ancestor->getCurrentAncestor();
  ...
  if (result == newContext) {
    ...
    if (costLimit != NULL && !setCostLimitInContext) delete costLimit;
...
//<pb>
void CascadesMemo::print(FILE *f, const char *, const char *) const {}  // CascadesMemo::print
//<pb>

void CascadesMemo::print_all_trees(CascadesGroupId root, NABoolean incl_log, NABoolean incl_phys, FILE *f) const {
...
//<pb>
RelExpr *CascadesMemo::include(RelExpr *expr, NABoolean &duplicateExprFlag, NABoolean &groupMergeFlag,
                               NABoolean GrpIdIsBinding,  // soln-10-070330-3667
                               CascadesGroupId groupId, Context *context, RelExpr *before) {
  RelExpr *result = expr;
  ...
  if (expr->isCutOp()) {
    ...
    if (groupId != INVALID_GROUP_ID AND groupId != groupIdOfCut AND(*CURRSTMT_OPTGLOBALS->memo)[groupId] !=
        (*CURRSTMT_OPTGLOBALS->memo)[groupIdOfCut]) {
      groupMergeFlag = TRUE;
    }
    ...
    if (result != NULL AND groupIdOfCut != (*CURRSTMT_OPTGLOBALS->memo)[groupIdOfCut]->getGroupId() AND GrpIdIsBinding)
      result->setGroupId(groupIdOfCut);

    if (result == NULL) result = (*CURRSTMT_OPTGLOBALS->memo)[groupIdOfCut]->getFirstPhysExpr();
  ...
  // for a logical expression, check for duplicates etc.
    ...
    for (int childIndex = 0; childIndex < arity; childIndex++) {
      ...
      result->child(childIndex) = include(result->child(childIndex).getPtr(), childDuplicateExprFlag,
                                          childGroupMergeFlag, childGrpIdIsBinding, INVALID_GROUP_ID, NULL, before)
                                      ->getGroupId();
    }
    ...
    if (expr->isLogical()) {
      ...
      if (other != NULL)  // previous duplicate found!
      {
        ...
        if (groupId != INVALID_GROUP_ID AND groupIdOfOther !=
            groupId AND
        ...
        if (other->getGroupAttr() != NULL AND expr->getGroupAttr() != NULL) {
      ...
      } else  // truly a new logical expression
      {
        ...
        if (groupId == INVALID_GROUP_ID) groupId = makeNewGroup(result->getGroupAttr());
    ...
    if (expr->isPhysical()) {
      ...
      if (context == NULL) {
        ...
        EstLogPropSharedPtr inputLP(new (CmpCommon::statementHeap()) EstLogProp((float)1.0));
        // Coverity flags "context" as an UNUSED_VALUE. That is true.
        // We don't care. So, we silence it for now.
        // coverity[returned_pointer]
        context = group_[groupId]->shareContext(NULL, NULL, NULL, NULL, inputLP);
      }
    ...
    // then delete it now
    if (NOT inserted && (CmpCommon::getDefault(COMP_BOOL_4) != DF_ON)) delete expr;
  ...
  if (NOT hash_.used(bucket)) {
    ...
    for (RelExpr *old = hash_[bucket]; old != NULL; old = old->getNextInBucket()) {
      ...
      for (childIndex = arity; --childIndex >= 0;)
        if ((*expr)[childIndex] != (*old)[childIndex]) goto not_a_duplicate;

      // compare characteristic inputs/outputs
      if ((ga = expr->getGroupAttr()) !=
          NULL AND(NOT(ga->getCharacteristicInputs() == old->getGroupAttr()->getCharacteristicInputs())
                       OR NOT(ga->getCharacteristicOutputs() == old->getGroupAttr()->getCharacteristicOutputs())))
        goto not_a_duplicate;

      // compare operators and their arguments
      if (NOT expr->duplicateMatch(*old)) goto not_a_duplicate;
...
CascadesGroupId CascadesMemo::makeNewGroup(GroupAttributes *ga) {
  ...
  group_.insertAt(newGroupId, new (CmpCommon::statementHeap()) CascadesGroup(newGroupId, ga));
  return (newGroupId);
}
...
Int32 CascadesMemo::garbageCollection() {
  ...
  for (int bucket_no = 0; bucket_no < (int)hashSize_; bucket_no++) {
    if (hash_.used(bucket_no)) {
      ...
      // -------------------------------------------------------------
      pred = NULL;
      for (e = hash_[bucket_no]; e != NULL; e = e->getNextInBucket()) {
        // ---------------------------------------------------------
        ...
        for (i = 0; i < nc; i++) {
          ...
          if (childGroupId != INVALID_GROUP_ID AND group_[childGroupId]->getGroupId() != childGroupId) {
            // -------------------------------------------------
        ...
        if (found) {
          ...
          for (i = 0; i < nc; i++) {
            e->child(i) = group_[e->child(i).getGroupId()]->getGroupId();
          }
  ...
  for (i = 0; i < (Int32)changed.entries(); i++) {
    ...
    if ((d = findDuplicate(e)) != NULL) {
      ...
      // -------------------------------------------------------------
      for (CascadesTask *task = CURRSTMT_OPTGLOBALS->task_list->getFirst(); task != NULL;
      ...
      // ------------------------------------------------------------
      if (e->getGroupAttr() != NULL AND e->getGroupAttr()->getLogExprForSynthesis() == e)
...
NABoolean CascadesMemo::consistencyCheck() const { return TRUE; }  // CascadesMemo::consistencyCheck()
