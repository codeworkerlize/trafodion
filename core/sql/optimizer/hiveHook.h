...
struct hive_sd_desc
{
   ...
   hive_sd_desc(NAHeap *heap, Int32 sdID, const char* loc, Int64 creationTS, Int32 buckets,
                ...
                const char* nf, const char* serlib,
                char knd,
      ...
      next_(NULL)
   {
     ...
     serializeLib_ = (serlib ? strduph(serlib, heap_) : NULL);
     if (pColVals)
...
struct hive_tblparams_desc
{
  ...
  Int32 getTextSkipHeaderCount() { return orcBlockPadding_; }
...
struct hive_tbl_desc
{
  ...
  struct hive_tblparams_desc* getTblParams() { return tblParams_;}
...
public:
  ...
  struct hive_tbl_desc* getTableDesc(const char* schemaName,
				     ...
				     NABoolean allowSubdirs,
                                     // do not set this to FALSE if tbldesc can be cached across statements
                                     NABoolean readPartnParamInfo = TRUE);
  struct hive_tbl_desc* getFakedTableDesc(const char* tblName);

  // validate a cached hive table descriptor
  NABoolean validate(hive_tbl_desc *hDesc, NABoolean allowSubdirs);
