...
void computeAndAddSelParamIfPossible(CacheWA &cwa, BindWA &bindWA, ExprValueId &child, BaseColumn *base,
  ...
  if (base->getNAColumn()->isHivePartColumn()) return;

  CostScalar sel;
  NABoolean canComputeSelectivity =
      hist->computeSelectivityForEquality(val, cStatsPtr->getRowcount(), cStatsPtr->getTotalUec(), sel);
  ...
  if (canComputeSelectivity == TRUE) {
    ...
    if (newType->getTypeQualifier() == NA_CHARACTER_TYPE) {
      newType = base->getNAColumn()->getType()->newCopy(cwa.wHeap());
...
ItemExpr *Assign::normalizeForCache(CacheWA &cwa, BindWA &bindWA) {
  ...
  if (cwa.getPhase() == CmpMain::PARSE) {
    ...
    if (leftO == ITM_BASECOLUMN &&
      ...
      if (!val->isNull()) {
        cParam = new (cwa.wHeap())
            ConstantParameter((ConstValue *)rightC, cwa.wHeap(), ((BaseColumn *)leftC)->getNAColumn()->getType());
...
NABoolean BaseColumn::canSafelyCoerce(ItemExpr &val) const {
  ...
  return (baseCol != NULL &&
          ((valTyp == ITM_CONSTANT && ((ConstValue *)&val)->canBeSafelyCoercedTo(*baseCol->getType()))
           ...
           || valTyp == ITM_HOSTVAR || valTyp == ITM_DYN_PARAM || valTyp == ITM_CACHE_PARAM));
}
...
NABoolean BaseColumn::isKeyColumnValue(ItemExpr &val) const {
  NAColumn *baseCol = getNAColumn();
  return (baseCol != NULL && (baseCol->isPrimaryKey() || baseCol->isPartitioningKey()) && canSafelyCoerce(val));
}
...
void BiArith::generateCacheKey(CacheWA &cwa) const {
  ...
  if (isKeepLastDay())
    cwa += "r1";
  else if (isStandardNormalization())
    cwa += "r0";
    ...
    if (getOperatorType() == ITM_EQUAL) {
      ...
      if (leftO == ITM_BASECOLUMN) {
        ...
        return TRUE;
                 ...
                 ((ItemList *)leftC)->isListOfCacheableSelPred(cwa, (ItemList *)rightC)) {
        return TRUE;
      } else {
    ...
    if (getArity() == 2) {
      if (getOperatorType() == ITM_EQUAL) {
        ...
        if (leftO == ITM_BASECOLUMN && rightO == ITM_CONSTANT) {
          parameterizeMe(cwa, bindWA, child(1), (BaseColumn *)leftC, (ConstValue *)rightC);
          ...
          parameterizeMe(cwa, bindWA, child(0), (BaseColumn *)rightC, (ConstValue *)leftC);
          // HQC collect histogram for column
          ItemExpr *cParameter = child(0);
          cwa.bindConstant2SQC((BaseColumn *)rightC, (ConstantParameter *)cParameter);
        } else if (leftO == ITM_ITEM_LIST && rightO == ITM_ITEM_LIST) {
          child(0) = ((ItemList *)leftC)->normalizeListForCache(cwa, bindWA, (ItemList *)rightC);
        }
      }
...
void BiRelat::parameterizeMe(CacheWA &cwa, BindWA &bindWA, ExprValueId &child, BaseColumn *base, ConstValue *val) {
  NABoolean parameterizableMVQRplan =
      cwa.hasRewriteEnabledMV() && CmpCommon::getDefault(MVQR_PARAMETERIZE_EQ_PRED) == DF_ON;
  NABoolean parameterizablePlan = parameterizableMVQRplan || !cwa.hasRewriteEnabledMV();
  ...
  if (val->isNull()) {
    ...
    cwa.replaceWithNewOrOldConstParam(val, base->getNAColumn()->getType(), child, bindWA);
    if (parameterizableMVQRplan) cwa.setParameterizedPred(TRUE);
...
NABoolean ConstValue::canBeSafelyCoercedTo(const NAType &target) {
  ...
  if (!type_->errorsCanOccur(target)) {
    if ((target.getTypeQualifier() == NA_CHARACTER_TYPE) && (type_->getTypeQualifier() == NA_CHARACTER_TYPE)) {
      if (((CharType &)target).isCaseinsensitive() != (((CharType *)type_)->isCaseinsensitive()))
        return FALSE;
      else {
        // fix bugzilla 2764 by skipping parameterization of long
        // char literals because backpatching those does not work.
        // not yet.
        return target.getNominalSize() < CmpCommon::getDefaultNumeric(QUERY_CACHE_MAX_CHAR_LEN);
      }
    }
    return TRUE;
  }
  ...
  // it should be ok.
  if (target.getTypeQualifier() == NA_NUMERIC_TYPE && type_->getTypeQualifier() == NA_NUMERIC_TYPE) {
    ...
    if (tgtType->shouldCheckValueFitInType()) {
      ...
      if ((val >= 0) && (val <= (tgtType->getMaxValue()))) {
        if (srcType->isExact() && tgtType->isExact() && (srcType->getScale() > tgtType->getScale()))
          return FALSE;
        else
          return TRUE;
      }
    ...
    if (srcType->isExact() && tgtType->isExact()) {
      ...
      if (srcType->isUnsigned() || tgtType->isSigned()) {
        ...
        // error can occur
        if ((srcType->getMagnitude() <= tgtType->getMagnitude()) && (srcType->getScale() <= tgtType->getScale()))
          return TRUE;
        ...
        if (type_->encode(value_) >= 0.0 && (srcType->getMagnitude() <= tgtType->getMagnitude()) &&
  ...
  if (target.getTypeQualifier() == NA_NUMERIC_TYPE && ((const NumericType &)target).isExact() &&
      ((const NumericType &)target).isUnsigned() && isExactNumeric() && ((NumericType *)type_)->isSigned() &&
  ...
  if (target.getTypeQualifier() == NA_INTERVAL_TYPE && type_->getTypeQualifier() == NA_INTERVAL_TYPE &&
...
ItemExpr *ConstValue::normalizeForCache(CacheWA &cwa, BindWA &bindWA) {
  ...
  ConstantParameter *result = new (heap) ConstantParameter(*this, heap, cwa.getPhase() == CmpMain::PARSE);
  if (result) {
...
// append an ascii-version of Cast into cachewa.qryText_
void ConvertTimestamp::generateCacheKey(CacheWA &cwa) const {
  BuiltinFunction::generateCacheKey(cwa);
  if (toLocalTime_) {
    cwa += " 1";
  }
...
void DateFormat::generateCacheKey(CacheWA &cwa) const {
  ...
  str_itoa(frmt_, dFmt);
  cwa += dFmt;
  cwa += wasDateformat_ ? "1" : "0";
  str_itoa(caseSensitivity_, dFmt);
  cwa += dFmt;
  if (NUMBER_FORMAT_STR == dateFormat_) {
    str_itoa(formatStr_.length(), dFmt);
    cwa += dFmt;
  }
}
...
void PivotGroup::generateCacheKey(CacheWA &cwa) const {
  ...
  char xFld[20];
  str_itoa(maxLen_, xFld);
  cwa += xFld;

  cwa += orderBy_ ? "1" : "0";
  if (orgReqOrder_) {
    NAString result;
    orgReqOrder_->unparse(result, BINDER_PHASE);
    cwa += result.data();
  }
}
...
NABoolean ItemExpr::isCacheableExpr(CacheWA &cwa) {
  switch (cwa.getPhase()) {
    default: {
      const NABoolean notYetImplemented = FALSE;
...
NABoolean ItemExpr::isCacheableNode(CmpPhase phase) const {
  switch (phase) {
    ...
    case CmpMain::BIND:
      return cacheable_ == ExprNode::CACHEABLE_BIND || cacheable_ == ExprNode::CACHEABLE_PARSE;
    default:
      break;
  }
  return FALSE;
    ...
    if (arity == 2) {
      ...
      if (left->getOperatorType() == ITM_CONSTANT) {
        if (right->getOperatorType() == ITM_CONSTANT) {
          ...
          return ((ConstValue *)left)->canBeSafelyCoercedTo(right->getValueId().getType());
        }
      } else if (right->getOperatorType() == ITM_CONSTANT) {
        return ((ConstValue *)right)->canBeSafelyCoercedTo(left->getValueId().getType());
      }
...
ItemExpr *ItemList::normalizeListForCache(CacheWA &cwa, BindWA &bindWA, ItemList *other) {
  ...
  if (cwa.getPhase() >= CmpMain::BIND && other && arity == other->getArity()) {
    for (int x = 0; x < arity; x++) {
      ...
      OperatorTypeEnum rightO = rightC->getOperatorType();
      if (leftO == ITM_BASECOLUMN && rightO == ITM_CONSTANT) {
        parameterizeMe(cwa, bindWA, other->child(x), (BaseColumn *)leftC, (ConstValue *)rightC);
        // HQC collect histogram for column
        ItemExpr *cParameter = other->child(x);
        cwa.bindConstant2SQC((BaseColumn *)leftC, (ConstantParameter *)cParameter);
      } else if (rightO == ITM_BASECOLUMN && leftO == ITM_CONSTANT) {
        parameterizeMe(cwa, bindWA, child(x), (BaseColumn *)rightC, (ConstValue *)leftC);
        // HQC collect histogram for column
        ItemExpr *cParameter = child(x);
        cwa.bindConstant2SQC((BaseColumn *)rightC, (ConstantParameter *)cParameter);
      } else if (leftO == ITM_ITEM_LIST && rightO == ITM_ITEM_LIST) {
        child(x) = ((ItemList *)leftC)->normalizeListForCache(cwa, bindWA, (ItemList *)rightC);
      }
    }
  }
  return this;
...
// helper to change literals of a cacheable query into input parameters
void ItemList::parameterizeMe(CacheWA &cwa, BindWA &bindWA, ExprValueId &child, BaseColumn *base, ConstValue *val) {
  NABoolean parameterizableMVQRplan =
      cwa.hasRewriteEnabledMV() && CmpCommon::getDefault(MVQR_PARAMETERIZE_EQ_PRED) == DF_ON;
  NABoolean parameterizablePlan = parameterizableMVQRplan || !cwa.hasRewriteEnabledMV();
...
// append an ascii-version of Cast into cachewa.qryText_
void Repeat::generateCacheKey(CacheWA &cwa) const {
  BuiltinFunction::generateCacheKey(cwa);

  if (child(1)->getOperatorType() == ITM_CACHE_PARAM) {
    ConstValue *cv;
    ItemExpr *expr;
    ...
    cp = (ConstantParameter *)expr;
    cv = cp->getConstVal();
    if (cv->canGetExactNumericValue())
      secondValue = cv->getExactNumericValue();
    else
      secondValue = -1;

    if (secondValue > -1)
      str_itoa(secondValue, secValueStr);
    else
      strcpy(secValueStr, "-1");
...
NABoolean Selectivity::operator==(const Selectivity &other) const {
  int absolute_ = getDefaultAsLong(QUERY_CACHE_SELECTIVITY_ABOSOLUTE);
  if (abs(val_.toLong() - other.val_.toLong()) < absolute_) return TRUE;

  CostScalar delta = CostScalar((ActiveSchemaDB()->getDefaults()).getAsDouble(QUERY_CACHE_SELECTIVITY_TOLERANCE)) + 1;

  return (val_ / delta <= other.val_) && (other.val_ <= val_ * delta);
}

// append an ascii-version of SelParameter into cachewa.qryText_
void SelParameter::generateCacheKey(CacheWA &cwa) const { cwa += "#"; }
    ...
    if (cwa.getPhase() == CmpMain::BIND) {
      if (origIe->isSafelyCoercible(cwa)) {
        ...
        if (newIe != origIe) {
          ...
          if ((origIe->getOperatorType() == ITM_CAST) && (origInputVars.contains(origIe->child(0)->getValueId()))) {
            ...
            // consistent.
            if (!newIe->referencesTheGivenValue(origIe->child(0)->getValueId()), TRUE) {
            ...
            // the inputVars set.
            if (!newIe->referencesTheGivenValue(origIe->getValueId()), TRUE) {
...
void UDFunction::generateCacheKey(CacheWA &cwa) const {
  ...
  if (cwa.getPhase() >= CmpMain::BIND && getRoutineDesc() && (routine = getRoutineDesc()->getNARoutine()) != NULL) {
    char redefTime[40];
  ...
  if (getRoutineDesc() && getRoutineDesc()->getActionNameAsGiven().length() != 0) {
    cwa += " actnam:";
    cwa += getRoutineDesc()->getActionNameAsGiven();

    if (cwa.getPhase() >= CmpMain::BIND && getRoutineDesc() &&
...
NABoolean ValueIdProxy::hasNoLiterals(CacheWA &cwa) {
  return derivedFrom_.getItemExpr()->hasNoLiterals(cwa) && derivedFrom_.getItemExpr()->isCacheableExpr(cwa);
}
...
void ItmSequenceFunction::generateCacheKey(CacheWA &cwa) const {
  BuiltinFunction::generateCacheKey(cwa);
  ...
  if (olapPartitionBy_) {
    NAString result;
    olapPartitionBy_->unparse(result, BINDER_PHASE);
    cwa += result.data();
  }
  ...
  if (olapOrderBy_) {
    NAString result;
    olapOrderBy_->unparse(result, BINDER_PHASE);
    cwa += result.data();
  }

  cwa += " isTDFunction:";
  cwa += isTDFunction_ ? "1" : "0";
}

void ItmSeqOlapFunction::generateCacheKey(CacheWA &cwa) const {
  ItmSequenceFunction::generateCacheKey(cwa);

  char value[40];
  str_itoa(frameStart_, value);
  cwa += " fs:";
  cwa += value;

  str_itoa(frameEnd_, value);
  cwa += " fe:";
  cwa += value;
}

void ItmLeadOlapFunction::generateCacheKey(CacheWA &cwa) const {
  ItmSeqOlapFunction::generateCacheKey(cwa);

  char value[40];
  str_itoa(offset_, value);
  cwa += " offset:";
  cwa += value;
}

void ItmSeqOffset::generateCacheKey(CacheWA &cwa) const {
  ItmSequenceFunction::generateCacheKey(cwa);

  cwa += " nullRowIsZero_:";
  cwa += (nullRowIsZero_) ? "1" : "0";

  char value[40];
  str_itoa(offsetConstantValue_, value);
  cwa += " offsetCV:";
  cwa += value;
  ...
  str_itoa(winSize_, value);
  cwa += " winSize:";
  cwa += value;
}
void ItmLagOlapFunction::generateCacheKey(CacheWA &cwa) const {
  ItmSeqOlapFunction::generateCacheKey(cwa);
...
void ItmSeqRowsSince::generateCacheKey(CacheWA &cwa) const {
  ItmSequenceFunction::generateCacheKey(cwa);

  cwa += " includeCurrRow:";
  cwa += (includeCurrentRow_) ? "1" : "0";
}

void ItmOlapPivotGroup::generateCacheKey(CacheWA &cwa) const {
  ItmSequenceFunction::generateCacheKey(cwa);

  cwa += " delimiter:";
  delimiter_->generateCacheKey(cwa);

  char str[100];
  sprintf(str, " maxLength:%d", maxPivotLen_);
  cwa += str;
}

void ItmSeqRunningPivotGroup::generateCacheKey(CacheWA &cwa) const {
  ItmSequenceFunction::generateCacheKey(cwa);
...
ItemExpr *ItmSequenceFunction::normalizeForCache(CacheWA &cwa, BindWA &bindWA) {
  if (!nodeIsNormalizedForCache()) markAsNormalizedForCache();
