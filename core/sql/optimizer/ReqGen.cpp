...
RequirementGenerator::RequirementGenerator(const ExprGroupId &groupForRequirement,
    ...
    : startRequirements_(startRequirements),
      ...
      addedNumOfPartsAllowedDeviation_(CURRSTMT_OPTDEFAULTS->numberOfPartitionsDeviation()),
  ...
  if (startRequirements) {
    ...
    pipelinesPerCPU_ = startRequirements->getCountOfPipelines() / availableCPUs_;
    ...
    logicalOrderOrArrangement_ = startRequirements->getLogicalOrderOrArrangementFlag();
...
void RequirementGenerator::addSortKey(const ValueIdList &additionalSortKey, SortOrderTypeEnum sortOrderTypeReq,
  ...
  if (additionalSortKey.entries() == 0) return;
  ...
  for (i = 0; i < additionalSortKey.entries(); i++) {
    ValueId simpVid = additionalSortKey[i].getItemExpr()->simplifyOrderExpr()->getValueId();
  ...
  for (i = existingEntries; i < noDupsAdditionalSortKey.entries() AND NOT sortKeyIsUnique_; i++) {
  ...
  if (addedArrangement_ != NULL AND addedArrangement_->entries() > 0) {
    ...
    if (NOT(((CollIndex)count == simpleArrangedCols_.entries()) OR((CollIndex)count == simpleSortCols_.entries())))
      stillFeasible_ = FALSE;
  }

}  // addSortKey

void RequirementGenerator::makeSortKeyFeasible(ValueIdList &proposedSortKey) {
  CollIndex i = 0;
  ...
  if (addedSortKey_ != NULL) {
    while ((i < addedSortKey_->entries()) AND(i < proposedSortKey.entries()) AND match) {
    ...
    // requirement.
    while ((i < simpleArrangedCols_.entries()) AND(i < proposedSortKey.entries()) AND match) {
      svid = proposedSortKey[i].getItemExpr()->simplifyOrderExpr()->getValueId();
      if (NOT simpleArrangedCols_.contains(svid)) match = FALSE;
      if (match) i++;
    }
...
void RequirementGenerator::makeArrangementFeasible(ValueIdSet &proposedArrangement) {
  ValueId svid;
  ...
  if ((addedSortKey_ == NULL) AND(addedArrangement_ == NULL)) return;

  // a simplified version of proposedArrangement
  ValueIdSet simpleProposedArrangement;

  for (ValueId vid = proposedArrangement.init(); proposedArrangement.next(vid); proposedArrangement.advance(vid)) {
    simpleProposedArrangement += vid.getItemExpr()->simplifyOrderExpr()->getValueId();
  }

  // If we have already added the proposed arrangement, then it
  // must be feasible, so just return.
  if (simpleArrangedCols_ == simpleProposedArrangement) return;
  ...
  if (addedSortKey_ != NULL) {
    ...
    // AB or ABCD.
    if (NOT(((CollIndex)count == simpleProposedArrangement.entries())
                OR((CollIndex)count == simpleSortCols_.entries()))) {
      ...
      if (count == 0)
        ...
        for (ValueId v = proposedArrangement.init(); proposedArrangement.next(v); proposedArrangement.advance(v)) {
          svid = v.getItemExpr()->simplifyOrderExpr()->getValueId();
          if (NOT coveredSimpleSortCols.contains(svid)) proposedArrangement -= v;
        }
      }  // end else count != 0
    ...
    // arrangement.
    if (NOT simpleArrangedCols_.contains(simpleProposedArrangement)
            AND NOT simpleProposedArrangement.contains(simpleArrangedCols_)) {
      ...
      if (simpleProposedArrangement.isEmpty())
        ...
        for (ValueId v = proposedArrangement.init(); proposedArrangement.next(v); proposedArrangement.advance(v)) {
          svid = v.getItemExpr()->simplifyOrderExpr()->getValueId();
          if (NOT simpleProposedArrangement.contains(svid)) proposedArrangement -= v;
        }
...
void RequirementGenerator::addArrangement(const ValueIdSet &additionalArrangementParam,
                                          SortOrderTypeEnum sortOrderTypeReq,
                                          PartitioningRequirement *dp2SortOrderPartReq) {
  // Do not bother if additionalArrangementParam is an empty set
  if (additionalArrangementParam.entries() == 0) return;
  ...
  for (ValueId vid = additionalArrangementParam.init(); additionalArrangementParam.next(vid);
  ...
  if ((addedSortKey_ != NULL) AND
    ...
    // AB or ABCD.
    if (NOT(((CollIndex)count == simpleAddtnlArrangement.entries()) OR((CollIndex)count == simpleSortCols_.entries())))
      stillFeasible_ = FALSE;
  }

  // perform check d) (see comment in addSortKey())
  if (stillFeasible_ AND(addedArrangement_ != NULL) AND(simpleArrangedCols_ != simpleAddtnlArrangement)) {
    ...
    if (stillFeasible_) {
      ...
      CMPASSERT(simpleArrToConvert.contains(sortColsAsSet) OR sortColsAsSet.contains(simpleArrToConvert));
      ...
      if (addedSortKey_ != NULL) {
        ...
        for (ValueId v = arrToConvert.init(); arrToConvert.next(v); arrToConvert.advance(v)) {
          svid = v.getItemExpr()->simplifyOrderExpr()->getValueId();
          if (NOT simpleArrToConvert.contains(svid)) arrToConvert -= v;
        }
      }
      ...
      for (ValueId cvid = arrToConvert.init(); arrToConvert.next(cvid); arrToConvert.advance(cvid)) {
        orderFromArr.insert(cvid);
      }

      // now the actual conversion
      addSortKey(orderFromArr, sortOrderTypeReq, dp2SortOrderPartReq);
  ...
  if (stillFeasible_ AND NOT additionalArrangement.isEmpty()
          AND((addedArrangement_ == NULL) OR(simpleArrangedCols_ != simpleAddtnlArrangement))) {
    if (addedArrangement_ == NULL) {
      addedArrangement_ = new (CmpCommon::statementHeap()) ValueIdSet(additionalArrangement);
...
ValueIdSet RequirementGenerator::combinePartKeysHeuristically(const ValueIdSet &partKey1, const ValueIdSet &partKey2)
...
//   otherwise disable sharing
{
  ...
  if (key1IsASubsetOfKey2 AND key2IsASubsetOfKey1) return result;

  // With PSSC set to 1 we require that one key being a superset of the other
  if ((pssc == 1) AND(key1IsASubsetOfKey2 OR key2IsASubsetOfKey1)) return result;
...
void RequirementGenerator::addPartitioningKey(const ValueIdSet &partKey) {
  ...
  if (partKeyHasBeenAdded_) {
    ...
    addedPartKey_ = combinePartKeysHeuristically(addedPartKey_, simplifiedPartKey);
  } else {
...
void RequirementGenerator::addPartRequirement(PartitioningRequirement *pr) {
  if (pr->isRequirementFuzzy()) {
    if (NOT pr->getPartitioningKey().isEmpty()) addPartitioningKey(pr->getPartitioningKey());
    if (pr->getCountOfPartitions() != ANY_NUMBER_OF_PARTITIONS) {
      if (pr->isRequirementApproximatelyN()) {
        addNumOfPartitions(pr->getCountOfPartitions(),
                           pr->castToRequireApproximatelyNPartitions()->getAllowedDeviation());
        requireHash2Only_ = pr->castToRequireApproximatelyNPartitions()->isRequireHash2Only();
      } else
        addNumOfPartitions(pr->getCountOfPartitions());
    }
  } else if (addedPartitioningRequirement_ != NULL) {
    if (addedPartitioningRequirement_->comparePartReqToReq(pr) != SAME) stillFeasible_ = FALSE;
  } else {
...
void RequirementGenerator::makeSkewReqFeasible(PartitioningRequirement *partR) {
  if (skewPropertyAdded_ == FALSE || partR == NULL) return;
  ...
  if (partR->isRequirementFuzzy()) {
    const skewProperty &skInFuzzyR = ((FuzzyPartitioningRequirement *)partR)->getSkewProperty();

    if (skInFuzzyR.isAnySkew()) {
      ((FuzzyPartitioningRequirement *)partR)->setSkewProperty(addedSkewProperty_);
    } else {
      if (NOT(skInFuzzyR == addedSkewProperty_)) stillFeasible_ = FALSE;
    }

    return;
  } else {
    // check the part func contained in the (fully specified) partR
    const FullySpecifiedPartitioningRequirement *fs_req = partR->castToFullySpecifiedPartitioningRequirement();

    if (fs_req) {
      PartitioningFunction *partFunc = fs_req->getPartitioningFunction();
      if (partFunc != NULL) {
        const SkewedDataPartitioningFunction *skpf = partFunc->castToSkewedDataPartitioningFunction();
        ...
        if (skpf == NULL)
          ...
          if (NOT(skpf->getSkewProperty() == addedSkewProperty_)) stillFeasible_ = FALSE;
        }
      }
    }
  }
...
void RequirementGenerator::addSkewRequirement(skewProperty::skewDataHandlingEnum x, SkewedValueList *skList,
  ...
  if (generatedPartitioningReqIsValid_ == TRUE AND generatedPartitioningRequirement_)
    makeSkewReqFeasible(generatedPartitioningRequirement_);
}

void RequirementGenerator::addLocationRequirement(PlanExecutionEnum loc) {
  if (NOT locationHasBeenAdded_) {
    addedLocation_ = loc;
    locationHasBeenAdded_ = TRUE;
  } else {
    if (loc != addedLocation_) {
      if ((addedLocation_ == EXECUTE_IN_MASTER_OR_ESP) AND((loc == EXECUTE_IN_MASTER) OR(loc == EXECUTE_IN_ESP))) {
        // change location to more specific one
        addedLocation_ = loc;
      } else {  // The only two cases we want to keep stillFeasible_ value
        ...
        if ((loc != EXECUTE_IN_MASTER_OR_ESP) OR(addedLocation_ == EXECUTE_IN_DP2)) {
...
void RequirementGenerator::addLogicalPartRequirement(LogicalPartitioningRequirement *pr) {
...
void RequirementGenerator::makeNumOfPartsFeasible(int &proposedNumOfParts,
  ...
  if (stillFeasible_ AND(generatedPartitioningRequirement_ != NULL)
          AND(generatedPartitioningRequirement_->getCountOfPartitions() != ANY_NUMBER_OF_PARTITIONS)) {
    proposedNumOfParts = generatedPartitioningRequirement_->getCountOfPartitions();
    if (generatedPartitioningRequirement_->isRequirementApproximatelyN()
            AND(proposedNumOfPartsAllowedDeviation != NULL))
      *proposedNumOfPartsAllowedDeviation =
          generatedPartitioningRequirement_->castToRequireApproximatelyNPartitions()->getAllowedDeviation();
  }
}

NABoolean RequirementGenerator::checkFeasibility() {
  if (NOT stillFeasible_) return FALSE;
  ...
  if ((addedLocation_ == EXECUTE_IN_MASTER_OR_ESP)
          AND((addedNumberOfPartitions_ * (1.0 - addedNumOfPartsAllowedDeviation_)) > 1.0)) {
...
ReqdPhysicalProperty *RequirementGenerator::produceRequirement() {
  ...
  CMPASSERT(locationHasBeenAdded_ AND availableCPUs_ > 0 AND pipelinesPerCPU_ > 0 AND costWeight_ !=
            NULL AND perfGoal_ != NULL AND(addedLocation_ == EXECUTE_IN_DP2 OR logicalPartReq_ == NULL));

  // If we're in Dp2, there must be no sort order type requirement
  CMPASSERT((addedLocation_ != EXECUTE_IN_DP2) OR(addedSortOrderTypeReq_ == NO_SOT));

  generatedRequirement_ = new (CmpCommon::statementHeap())
      ReqdPhysicalProperty(addedArrangement_, addedSortKey_, addedSortOrderTypeReq_, addedDp2SortOrderPartReq_,
                           logicalOrderOrArrangement_, generatedPartitioningRequirement_, logicalPartReq_,
                           addedLocation_, availableCPUs_, pipelinesPerCPU_, costWeight_, perfGoal_, NULL);
...
void RequirementGenerator::processStartRequirements() {
  if (startRequirements_ == NULL OR startRequirementsHaveBeenProcessed_ OR NOT stillFeasible_) return;
  ...
  if (NOT removeStartSortKey_ AND startRequirements_->getSortKey()
          AND(startRequirements_->getSortKey()->entries() > 0)) {
    addSortKey(*(startRequirements_->getSortKey()), startRequirements_->getSortOrderTypeReq(),
               startRequirements_->getDp2SortOrderPartReq());
  }
  if (NOT stillFeasible_) return;

  if (NOT removeStartArrangement_ AND startRequirements_->getArrangedCols()
          AND(startRequirements_->getArrangedCols()->entries() > 0)) {
    addArrangement(*(startRequirements_->getArrangedCols()), startRequirements_->getSortOrderTypeReq(),
                   startRequirements_->getDp2SortOrderPartReq());
  }
  if (NOT stillFeasible_) return;

  if (NOT removeStartPartitioningRequirements_ AND startRequirements_->requiresPartitioning()) {
...
void RequirementGenerator::producePartitioningRequirements() {
  ...
  if (generatedPartitioningReqIsValid_) return;
  ...
  if (addedPartitioningRequirement_) {
    ...
    if (partKeyHasBeenAdded_ AND NOT addedPartKey_.contains(addedPartitioningRequirement_->getPartitioningKey())) {
    ...
    if ((addedNumberOfPartitions_ != ANY_NUMBER_OF_PARTITIONS)
            AND((addedPartitioningRequirement_->getCountOfPartitions() > addedNumberOfPartitions_)
                    OR(addedPartitioningRequirement_->getCountOfPartitions() <
                       (addedNumberOfPartitions_ - (addedNumberOfPartitions_ * addedNumOfPartsAllowedDeviation_))))) {
    ...
    if (requireHash2Only_ AND !addedPartitioningRequirement_->castToFullySpecifiedPartitioningRequirement()
    ...
    if (stillFeasible_) generatedPartitioningRequirement_ = addedPartitioningRequirement_;
  } else if (partKeyHasBeenAdded_ OR addedNumberOfPartitions_ != ANY_NUMBER_OF_PARTITIONS) {
    // if we come here, only a partitioning key and/or a number
    // of partitions is required

    if ((partKeyHasBeenAdded_ AND addedPartKey_.isEmpty())OR(addedNumberOfPartitions_ == EXACTLY_ONE_PARTITION)) {
      ...
      if ((addedNumberOfPartitions_ != ANY_NUMBER_OF_PARTITIONS)
              AND((addedNumberOfPartitions_ - (addedNumberOfPartitions_ * addedNumOfPartsAllowedDeviation_)) >
                  EXACTLY_ONE_PARTITION))
        stillFeasible_ = FALSE;
      else
        generatedPartitioningRequirement_ = new (CmpCommon::statementHeap()) RequireExactlyOnePartition();
    } else {
      ...
      if (NOT addedPartKey_.isEmpty())
        generatedPartitioningRequirement_ = new (CmpCommon::statementHeap()) RequireApproximatelyNPartitions(
            addedPartKey_, addedNumOfPartsAllowedDeviation_, addedNumberOfPartitions_, requireHash2Only_);
      else
        generatedPartitioningRequirement_ = new (CmpCommon::statementHeap()) RequireApproximatelyNPartitions(
            addedNumOfPartsAllowedDeviation_, addedNumberOfPartitions_, requireHash2Only_);
    }
...
void RequirementGenerator::checkCompatibilityWithGroupAttributes() {
  ...
  if (stillFeasible_ AND addedSortKey_) {
    ...
    if (addedSortKey_->entries() == 0) {
      ...
      stillFeasible_ = sortKey.isCovered(newInputs, *ga_, referencedInputs, coveredSubExpr, uncoveredExpr);
      coveredSubExpr.clear();
    }
  }

  // arrangement must be covered by the group attributes
  if (stillFeasible_ AND addedArrangement_ AND NOT addedArrangement_->isEmpty()) {
    ...
    if (addedArrangement_->entries() == 0) {
      ...
      stillFeasible_ = addedArrangement_->isCovered(newInputs, *ga_, referencedInputs, coveredSubExpr, uncoveredExpr);
      coveredSubExpr.clear();
    }
  }
  ...
  if (stillFeasible_ AND generatedPartitioningRequirement_) {
    stillFeasible_ = generatedPartitioningRequirement_->getPartialPartitioningKey().isCovered(
        newInputs, *ga_, referencedInputs, coveredSubExpr, uncoveredExpr);
    coveredSubExpr.clear();
  ...
  if (stillFeasible_ AND(addedDp2SortOrderPartReq_ != NULL)) {
    NABoolean isCovered = addedDp2SortOrderPartReq_->getPartitioningKey().isCovered(newInputs, *ga_, referencedInputs,
...
THREAD_P LookupTable<SortOrderTypeEnum> *RequirementGenerator::sortOrderTypeReqCompTab_ = NULL;
...
void RequirementGenerator::initSortOrderTypeReqCompTab() {
  if (sortOrderTypeReqCompTab_ != NULL) return;

  sortOrderTypeReqCompTab_ =
      new (GetCliGlobals()->exCollHeap()) LookupTable<SortOrderTypeEnum>(6, 6, GetCliGlobals()->exCollHeap());
...
SortOrderTypeEnum RequirementGenerator::determineCompatibleSortOrderTypeReq(
...
void RequirementGenerator::addSortOrderTypeReq(SortOrderTypeEnum sortOrderTypeReq,
  ...
  if ((sortOrderTypeReq == DP2_OR_ESP_NO_SORT_SOT) OR(sortOrderTypeReq == DP2_SOT))
  ...
  SortOrderTypeEnum result = determineCompatibleSortOrderTypeReq(sortOrderTypeReq);
  ...
  if ((dp2SortOrderPartReq != NULL) AND(addedDp2SortOrderPartReq_ != NULL)) {
    if (addedDp2SortOrderPartReq_->comparePartReqToReq(dp2SortOrderPartReq) != SAME) {
      // See if we can convert to an ESP_NO_SORT requirement
  ...
  if ((addedSortOrderTypeReq_ == ESP_NO_SORT_SOT) OR(addedSortOrderTypeReq_ == ESP_VIA_SORT_SOT)
          OR(addedSortOrderTypeReq_ == ESP_SOT))
    addedDp2SortOrderPartReq_ = NULL;
