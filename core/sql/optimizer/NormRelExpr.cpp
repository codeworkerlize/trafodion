                                   ...
                                   NABoolean includeIntersects)
{
  ...
  // Left linearize R1
  R1->leftLinearizeJoinTree(normWARef,
                            transformationType,
                            includeIntersects); 
  ...
  // right child.
  //
  R1->child(0) = leftLinearizeJoinTree(normWARef,
                                       transformationType,
                                       includeIntersects);
...
RelExpr * Join::normalizeNode(NormWA & normWARef)
{
  ...
  if (isIntersect_ &&
      ActiveSchemaDB()->getDefaults().getAsDouble(INTERSECT_PUSH_GRBY_RATIO) >= 0.0)
  ...
  if (doNotUnnest == FALSE)
  {
    ...
    if (!nonLocalPreds.isEmpty())
    {
      ...
      if ((doNotUnnest == FALSE) && candidateForLeftJoin && 
          ...
          ((normWARef.getLeftJoinConversionCount() >= 
	    ActiveSchemaDB()->getDefaults().getAsLong(COMP_INT_12))|| 
	   nestedAggInSubQ))
      {
        doNotUnnest = TRUE;
	...
	// that these subqueries are not nested.
        ...
        if (CmpCommon::getDefault(SUBQUERY_UNNESTING) == DF_DEBUG) 
        {
          if (!nestedAggInSubQ)
            *CmpCommon::diags() << DgSqlCode(2997)
				<< DgString1("Skipping unnesting of Subquery due to NonNullRejecting Predicates in more than COMP_INT_12 subqueries");
          else
             *CmpCommon::diags() << DgSqlCode(2997)
...
NABoolean Join::unnestingDoEarlyGrouping(NormWA& normWARef)
{
  ...
  if (CmpCommon::getDefault(SUBQUERY_UNNESTING_EARLY_GROUP) == DF_OFF)
    return FALSE;

  if ((normWARef.getCorrelatedSubqCount() != 1) || candidateForSubqueryLeftJoinConversion())
    return FALSE;
  ...
  if(child(1)->getOperatorType() == REL_GROUPBY)
  {
    ...
    ValueIdSet outerReferences = scalarGB->getGroupAttr()->
      getCharacteristicInputs();
    outerReferences -= getGroupAttr()->getCharacteristicInputs();
    if (scalarGB->aggregateExpr().getReferencedPredicates
        (outerReferences, nonLocalAggrs))
      return FALSE;
    if (scalarGB->groupExpr().entries() != 0)
      return FALSE;
  }
  else
    return FALSE;
...
Join* Join::scalarAggrToGroupByTransformation(NormWA& normWARef)
{
  ...
  RelExpr * newRightChild = copyNodeAndSetChildren(oldGBgrandchild, stmtHeap);
  if (nestedAggInSubQ)
    newRightChild->child(0) = filterNode->child(0) ;
  ...
  newJoin->getGroupAttr()->setCharacteristicOutputs
    (getGroupAttr()->getCharacteristicOutputs());
  newJoin->getGroupAttr()->setCharacteristicInputs
    (getGroupAttr()->getCharacteristicInputs());
...
RelExpr* GroupByAgg::nullPreserveMyExprs( NormWA& normWARef)
{
  ...
  if ( mapNeeded )
  {
    ...
    for (ValueId topMapId = vic.init();
         vic.next(topMapId); vic.advance(topMapId))
    {
      ValueId bottomMapId;
      rightChildMap->rewriteValueIdUp(bottomMapId, topMapId);
      if (bottomMapId != topMapId)
        map->addMapEntry(topMapId, bottomMapId);
    }
  ...
  if (child(1)->child(0)->getOperatorType() == REL_FILTER)
	...
	if((CmpCommon::getDefault(COMP_BOOL_168) == DF_OFF) &&
	   ...
	   ((normWARef.getCorrelatedSubqCount() == 1) || 
            (newGrby && NOT(newGrby->requiresMoveUp()))) &&
...
/*---------------------------------------------------------------------------------------*/
  ...
  if (candidateForSubqueryUnnest())
  {
    ...
    if (unnestingDoEarlyGrouping(normWARef))
    {
      ...
      if (newJoin) 
      {
        if(applyInnerKeyedAccessHeuristic(NULL, normWARef))
          return this;
        ...
        // join backbone.
        if (hasNoErrors && 
          ((ActiveSchemaDB()->getDefaults().getAsLong(COMP_INT_11) < 0) ||
...
void ConnectBy::transformNode(NormWA & normWARef,
                          ExprGroupId & locationOfPointerToMe)
{
  CMPASSERT( this == locationOfPointerToMe );
...
RelExpr * GroupByAgg::semanticQueryOptimizeNode(NormWA & normWARef)
{
  ...
  if (// there are intersects in this tree that should be transformed
      ...
      child(0)->getOperatorType() == REL_JOIN &&
      static_cast<Join *>(child(0).getPtr())->isIntersect() &&
      aggregateExpr().isEmpty() &&
      groupExpr() == child(0).getGroupAttr()->getCharacteristicOutputs())
    {
      Join *join = static_cast<Join *>(child(0).getPtr());
      double ratio =
        ActiveSchemaDB()->getDefaults().getAsDouble(INTERSECT_PUSH_GRBY_RATIO);

      // cardinalities of the join and its children
      CostScalar joinCard =
        join->getGroupAttr()->outputLogProp(
             (*GLOBAL_EMPTY_INPUT_LOGPROP))->getResultCardinality();
      CostScalar joinChild0Card =
        join->child(0)->getGroupAttr()->outputLogProp(
             (*GLOBAL_EMPTY_INPUT_LOGPROP))->getResultCardinality();
      CostScalar joinChild1Card =
        join->child(1)->getGroupAttr()->outputLogProp(
             (*GLOBAL_EMPTY_INPUT_LOGPROP))->getResultCardinality();
      ...
      if (joinCard > oneToNJoinCard * ratio && ratio >= 0)
        {
          for (int c=0; c<2; c++)
            {
              ...
              if (!joinChild->getGroupAttr()->isUnique(
                       joinChild->getGroupAttr()->getCharacteristicOutputs()))
                {
                  GroupAttributes *joinChildGA = joinChild->getGroupAttr();

                  GroupByAgg *newGrby = new(CmpCommon::statementHeap())
                    GroupByAgg(joinChild);
                  ...
                  newGrby->setGroupAttr(new(CmpCommon::statementHeap()) 
                                        GroupAttributes(*joinChildGA));
                  newGrby->getGroupAttr()->clearLogProperties();
                  newGrby->synthLogProp(&normWARef); 
          ...
          join->getGroupAttr()->setCharacteristicOutputs(
               getGroupAttr()->getCharacteristicOutputs());
      ...
      else
        {
          ...
          // is to exclude them)
          child(0) = join->leftLinearizeJoinTree(
               normWARef,
               Join::NO_TRANSFORMATION,
               TRUE); 
        }
    }
  ...
  for (CollIndex ix =0; ix < ixlist.entries(); ix++)
  {
    ...
    if ((keyColIndex != NULL_COLL_INDEX)&& isBigTable)  // 'a' is a key column of this index
    {
      ...
      for (CollIndex i =0; i < keyCols.entries(); i++)
      {
        ...
        {
          if ((partKeyCols.entries() == 1) && 
              (partKeyCols[0] == colVid)) 
            {
      ...
      if (fullKeyConstant)
        return TRUE;
      if (keyConstantUptoCol && 
          ((numValues > orPredToSemiJoin) || predOnLoneSaltCol))
	return TRUE;
...
RelExpr * RelRoot::normalizeNode(NormWA & normWARef)
{
  ...
  {
    Lng32 safetyFactor = 
      ActiveSchemaDB()->getDefaults().getAsLong(FIRSTN_PREDICATE_SAFETY_FACTOR); 
    ...
    if ((safetyFactor != 0) && (reqdOrder().entries() > 0))
    {
      ...
      {
        firstOrderColVid = 
          firstOrderColVid.getItemExpr()->child(0)->getValueId() ;
      }
      NABoolean isConstant = FALSE;
      NAColumn *nacol = NULL;
      ValueId baseVid = firstOrderColVid.getBaseColumn(&isConstant);
      if (baseVid != NULL_VALUE_ID)
        nacol = baseVid.getNAColumn(TRUE/*okIfNotColumn*/);
      if (nacol) 
      {
...
{
  if ((getOperatorType() != REL_ROOT) && (getOperatorType() != REL_FIRST_N))
    return FALSE;
  ...
  Lng32 safetyFactor = 
    ActiveSchemaDB()->getDefaults().getAsLong(FIRSTN_PREDICATE_SAFETY_FACTOR);
  if ((safetyFactor == 0) || (firstNValue <= 0))
    return FALSE;
  ...
  else
  {
    ...
    if (fn->reqdOrder().entries()> 0) // FirstN with orderBy (non-rowsets case)
    ...
    else  // FirstN without orderBy
      {
        ...
        if (firstOrderColVid == NULL_VALUE_ID)
          return FALSE;
      }
  }
  ...
  EstLogPropSharedPtr childEstLogProp = child(0)->getGroupAttr()->
    outputLogProp((*GLOBAL_EMPTY_INPUT_LOGPROP));
 
  ColStatsSharedPtr firstOrderColStats = childEstLogProp->getColStats().
    getColStatsPtrForColumn(firstOrderColVid) ;
  if (!firstOrderColStats || firstOrderColStats->isFakeHistogram())
    return FALSE;
  ...
  if (!vStr || (vStr->length() == 0) || strchr(vStr->data(), '@'))
    return FALSE;
  
  ItemExpr* maxVal ;
  maxVal = NATable::getRangePartitionBoundaryValues(vStr->data(), 
                                                    vStr->length(), 
                                                    STMTHEAP); 
  if (!maxVal)
    return FALSE;
  ItemExpr* firstNPred ;
  if (!isInverse)
  firstNPred = new (STMTHEAP) 
    BiRelat(ITM_LESS_EQ, firstOrderColVid.getItemExpr(), maxVal);
  else
    firstNPred = new (STMTHEAP) 
      BiRelat(ITM_GREATER_EQ, firstOrderColVid.getItemExpr(), maxVal);
  ...
  if (CmpCommon::getDefault(GEN_SORT_TOPN) != DF_ON)
    return FALSE;
  ...
  if (info.getTempTableType() == CSEInfo::HIVE_TEMP_TABLE)
    {
      ...
      result = FastExtract::makeFastExtractTree(
         tableDesc,
         child(0).getPtr(),
         NULL,   // no required order
         TRUE,   // overwrite the table
         NULL,   //  no part col val list
