...
NABoolean ColStatDesc::expressionMatches(ItemExpr * expr)
{
  ...
  if (isExpressionHistogram())
  {
    ...
    if ((unparsedExpr == expressionText) &&
        (exprColRefs_.contains(baseCols)))
      return TRUE;
    ...
    for (CollIndex i = 0; i < nonVegEquals_.entries(); i++)
    {
      ...
      if ((unparsedExpr == tmpExpressionText) &&
          (tmp->exprColRefs_.contains(baseCols)))
      return TRUE;
    }
  }
...
NABoolean
ColStatDesc::modifyStats( ItemExpr * pred, ItemExpr * lhs, ItemExpr * rhs, 
                          ConstValue * constant, NABoolean negate,
                                 ...
                                 const NABoolean shapeChangedMask)
{
  ...
  if (source.hasExpressionHistogram_)
    hasExpressionHistogram_ = TRUE;
                                 ...
                                 const NABoolean shapeChangedMask)
{
  ...
  if (tmpColStatDescPtr->isExpressionHistogram())
    hasExpressionHistogram_ = TRUE;
} // insertDeepCopy
...
// -----------------------------------------------------------------------
ItemExpr * ColStatDescList::getNodeForStats(ItemExpr * operand,
                                            CollIndex start,
                                            CollIndex & match /* out */,
  ...
  if (hasExpressionHistogram_)
    {
      ...
      if (digIntoInstantiateNull && 
          operand->getOperatorType() == ITM_INSTANTIATE_NULL)
        opToMatch = operand->child(0);
                            ...
                            CostScalar *maxSelectivity)
{
  ...
  if (	      (	   predExprType == ITM_IS_NULL
		OR predExprType == ITM_IS_NOT_NULL
		OR predExprType == ITM_IS_UNKNOWN
		OR predExprType == ITM_IS_NOT_UNKNOWN
	      )
       OR NOT (	    predExprType == ITM_EQUAL
     ...
     )
  {
    ...
    if ((predExprType == ITM_IS_NULL) || (predExprType == ITM_IS_NOT_NULL))
      digIntoInstantiateNull = FALSE;
  
    if ( (lhs->getOperatorType() != ITM_VEG_REFERENCE) &&
         (constant || (predExprType == ITM_EQUAL) || (predExprType == ITM_NOT_EQUAL) ||
         (predExprType == ITM_IS_NULL) || (predExprType == ITM_IS_NOT_NULL)))
    {
	...
	continue;
      ...
      if ( thisColStatDesc->modifyStats
           ( pred, lhs, rhs, constant, negate, tempRowcount, 
  ...
  if ( (lhsMatchingExprHistogram != -1) OR
       (rhsMatchingExprHistogram != -1) )
...
ColStatDescSharedPtr
ColStatDescList::getColStatsDescPtrForColumn (const ValueId& inputColumn) const
{
  CollIndex index = 0;
  ...
  const ValueIdList &tableColList)
{
  ...
  for (UInt32 j = 0; j < other.entries(); j++)
  {
    ...
    if ((otherColumns.entries() == 1) && (otherColumns[0]->getPosition() < 0))
    {
      ...
      ColStatDescSharedPtr tmpStatDesc(new (HISTHEAP)
        ColStatDesc (other[j], tableColList), HISTHEAP);
