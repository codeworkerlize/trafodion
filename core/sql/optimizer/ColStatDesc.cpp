...
ColStatDesc::ColStatDesc(const ColStatsSharedPtr &stats, const ValueIdList &columnList, NAMemory *h)
    : nonVegEquals_(h) {
  ...
  // ---------------------------------------------------------------------
  if (!(stats->isExpressionHistogram()))  // do this for regular histograms, but not expressions
  {
...
//
// Note: Today this is never be called for an expression histogram.
// It would be meaningless to do so since an expression does not
// have an invariant ValueId.
ColStatDesc::ColStatDesc(const ColStatsSharedPtr &stats, const ValueId &column, NAMemory *h) : nonVegEquals_(h) {
  // Set up a reference to the ColStats structure
  colStats_ = stats;
...
void ColStatDesc::copy(const ColStatDesc &other) {
  ...
  modified_ = FALSE;
  exprColRefs_ = other.exprColRefs_;
}

void ColStatDesc::mapUpAndCopy(const ColStatDesc &other, ValueIdMap &map) {
  copy(other);
  ...
  if (column_ == other.column_ && VEGcolumn_ != other.VEGcolumn_) column_ = VEGcolumn_;
  if (VEGcolumn_ == other.VEGcolumn_ && column_ != other.column_) VEGcolumn_ = column_;

  // rewrite applied predicates
  if (!appliedPreds_.isEmpty() && (column_ != other.column_ || VEGcolumn_ != other.VEGcolumn_)) {
    appliedPreds_.clear();
    map.rewriteValueIdSetUp(appliedPreds_, other.appliedPreds_);
  }
}

void ColStatDesc::deallocate() { colStats_ = NULL; }
...
//  to colStats.
// -----------------------------------------------------------------------
ColStatsSharedPtr ColStatDesc::getColStatsToModify() {
  if (NOT isModified()) {
    colStats_ = ColStatsSharedPtr(new (HISTHEAP) ColStats(*colStats_, HISTHEAP));
    modified_ = TRUE;
  }
...
// -----------------------------------------------------------------------
//  ColStatDesc::expressionMatches
//
//  For expressions histograms, determines if the expression given
//  matches the histogram.
// -----------------------------------------------------------------------
NABoolean ColStatDesc::expressionMatches(ItemExpr *expr) {
  NABoolean rc = FALSE;  // assume not
  if (isExpressionHistogram()) {
    // For the expression to match the histogram, two things must
    // be true for either this histogram or one in its nonVEGEquals_
    // member:
    // 1. The expression text must match
    // 2. The base columns must match (that is, the columns in the
    // expression must be from the same table instance that the
    // histogram represents)

    NAString unparsedExpr;
    expr->unparse(unparsedExpr, DEFAULT_PHASE, USTAT_EXPRESSION_FORMAT);

    ValueIdSet baseCols;
    expr->findAll(ITM_BASECOLUMN, baseCols /* out */, TRUE, TRUE);

    const NAColumnArray &columns = colStats_->getStatColumns();
    const NAColumn *nac = columns[0];
    const NAString &expressionText = nac->getColName();
    if ((unparsedExpr == expressionText) && (exprColRefs_.contains(baseCols))) return TRUE;

    for (CollIndex i = 0; i < nonVegEquals_.entries(); i++) {
      ColStatDescSharedPtr tmp = nonVegEquals_[i];
      const NAColumnArray &tmpCols = tmp->colStats_->getStatColumns();
      const NAColumn *tmpNac = tmpCols[0];
      const NAString &tmpExpressionText = tmpNac->getColName();
      if ((unparsedExpr == tmpExpressionText) && (tmp->exprColRefs_.contains(baseCols))) return TRUE;
    }
  }
...
// global namespace.
// -----------------------------------------------------------------------
CostScalar ColStatDesc::calculateCorrectResultUec(const CostScalar &baseRows, const CostScalar &newRows,
  ...
  if (baseRows.isZero() || newRows.isZero() || baseUec.isZero()) return csZero;  // rows(0) ==> uec(0)
  ...
  if (firstApprox > newRows) firstApprox = newRows;  // reduce to the new rowcount
  ...
  if (NOT secondApprox.isGreaterThanZero() OR secondApprox > firstApprox) secondApprox = firstApprox;

  return secondApprox;
}

// -----------------------------------------------------------------------
//  ColStatDesc::generateExpressionHistogramMergeState
//
//  Generate a dummy ValueId that will be used to identify a particular
//  ColStatDesc for merge state purposes
// -----------------------------------------------------------------------

void ColStatDesc::generateExpressionHistogramMergeState() {
  ItemExpr *ie = new (HISTHEAP) ExprHistDummy();
  BindWA bindWA(ActiveSchemaDB(), CmpCommon::context());
  ie->bindNode(&bindWA);
  mergeState_.insert(ie->getValueId());
}
...
void ColStatDesc::applySel(const CostScalar &selectivity) {
  if (selectivity == csOne) return;  // nothing to do
  ...
  if ((originalBaseRowCount < baseRowcount) && (originalBaseRowCount != csMinusOne)) {
  ...
  if (baseUec.isZero())  // avoid div-by-zero!
  {
    ...
    newTotalUec = calculateCorrectResultUec(baseRowCountForUec, newRowcount, baseUec);

    uecSelectivity = newTotalUec / baseUec;
  }

  // If row count is anywhere >= one, then uplift the UEC to atleast one.
  if ((newRowcount >= csOne) && (newTotalUec < csOne)) newTotalUec = csOne;
...
// ----------------------------------------------------------------------
void ColStatDesc::applySelIfSpecifiedViaHint(ItemExpr *pred, const CostScalar &oldRowcount) {
...
// ----------------------------------------------------------------------
void ColStatDesc::synchronizeStats(const CostScalar &baseRowcount, const CostScalar &newRowcount,
  ...
  if (getColStats()->getRowcount() == newRowcount) return;  // nothing to do
  ...
  CostScalar rowSelectivity = (baseRowcount.isZero()) ? csOne : newRowCountCorrected / baseRowcount;
  ...
  if (colStats->getRowcount() != baseRowcount) {
    ...
    if (_ABSOLUTE_VALUE_(cardDiff.value()) > 1.0) {
      ...
      rowSelectivity = (baseRowcount.isZero()) ? csOne : colStats->getRowcount() / baseRowcount;
    }
  }
  ...
  if (baseUec.isZero())  // avoid div-by-zero
  {
    ...
    if (specialFlag == SET_UEC_TO_ONE) {
      ...
      rowCountIsOne = TRUE;
      rowSelectivity = (baseRowcount.isZero()) ? csOne : csOne / baseRowcount;
    }
  ...
  {
    newTotalUec = calculateCorrectResultUec(baseRowCountForUec, newRowCountCorrected, baseUec);
    uecSelectivity = newTotalUec / baseUec;
  }

  // If row count is one, then uplift the UEC to one also.
  if ((newRowCountCorrected >= csOne) && (newTotalUec < csOne)) newTotalUec = csOne;

  colStats->setRedFactor(colStats->getRedFactor() * rowSelectivity);
  colStats->setUecRedFactor(colStats->getUecRedFactor() * uecSelectivity);
  colStats->setRowsAndUec((rowCountIsOne ? csOne : newRowCountCorrected), newTotalUec);
}
...
//
//  Note: The work of digging out a suitable node in the lhs and
//  digging out the constant from the rhs was done in the caller,
//  so we only have to do it once per CDSL rather than doing it
//  repeatedly on a per histogram basis. Hence, we have the
//  seemingly redundant input parameters, pred, lhs, rhs and constant.
//
...
NABoolean ColStatDesc::modifyStats(ItemExpr *pred, ItemExpr *lhs, ItemExpr *rhs, ConstValue *constant, NABoolean negate,
                                   CostScalar &newRowcount, CostScalar *maxSelectivity) {
  ...
  if (isExpressionHistogram()) {
    // The ValueId comparison test doesn't work for expressions histograms;
    // we have to use a different test
    if (!expressionMatches(lhs)) return FALSE;
  } else  // regular histogram case
  {
    ...
    if (lhs->getValueId() != vegCol) {
      ...
      if (lhs->getOperatorType() == ITM_VEG_REFERENCE) {
        ...
        if (!cols.contains(vegCol)) return FALSE;
      } else
        return FALSE;
    }
  }
  ...
  if (NOT(constant != NULL OR pred->getArity() ==
          1 OR(pred->getArity() == 2 AND lhs->getValueId() == rhs->getValueId()))) {
  ...
  if (colStats->getStatColumns()[0]->getType()->getTypeQualifier() == NA_DATETIME_TYPE && val >= maxValue &&
      !colStats->isOrigFakeHist() && (op == ITM_EQUAL || op == ITM_GREATER || op == ITM_GREATER_EQ) &&
      maxSelectivity == NULL && constant != NULL)
  ...
  if ((pred->getArity() == 2) && (lhs->getValueId() == rhs->getValueId())) {
    if (maxSelectivity == NULL) colStats->simplestPreds(pred);
  } else {
  ...
  if (rowcount.isZero()
          AND NOT(hist->entries() ==
                  1 AND colStats->isMaxSetByPred() AND colStats->isMinSetByPred() AND colStats->getMaxValue() ==
                  UNINIT_ENCODEDVALUE AND colStats->getMinValue() == UNINIT_ENCODEDVALUE)) {
  ...
  if (derivOfLikeAndSimilarPredApp(pred)) {
    ...
    if (isaFakeHistogram AND NOT(hist->entries() ==
                                 1 AND colStats->isMaxSetByPred() AND colStats->isMinSetByPred()
                                     AND colStats->getMaxValue() == UNINIT_ENCODEDVALUE AND colStats->getMinValue() ==
                                 UNINIT_ENCODEDVALUE)) {
      ...
      if (op == ITM_IS_NULL || op == ITM_IS_UNKNOWN || op == ITM_EQUAL) {
  ...
  if (!isaFakeHistogram && (val != UNINIT_ENCODEDVALUE)) {
    if (op == ITM_LESS || op == ITM_LESS_EQ || op == ITM_GREATER || op == ITM_GREATER_EQ) {
      ...
      if (colStats->getRowcount() < minRowcount) synchronizeStats(rowcount, minRowcount);
    }
  }
...
NABoolean ColStatDesc::mergeColStatDescOfSameTable(ColStatDescSharedPtr &rightColStats, OperatorTypeEnum opType) {
  NABoolean checkForMergeFromScan = FALSE;

  if ((opType == REL_SCAN) || (opType == ITM_OR) || (opType == ITM_AND)) checkForMergeFromScan = TRUE;

  if ((!checkForMergeFromScan) || (CmpCommon::getDefault(COMP_BOOL_74) == DF_OFF)) return FALSE;
  ...
  if (opType == REL_SCAN) rootColStats->setBaseUec(newUec);
...
// -----------------------------------------------------------------------
void ColStatDesc::mergeColStatDesc(ColStatDescSharedPtr &mergedStatDesc, MergeType mergeMethod, NABoolean forceMerge,
                                   OperatorTypeEnum exprOpCode, NABoolean mergeFVs) {
  ColStatsSharedPtr rootColStats = getColStatsToModify();
  ...
  // parent is a Join
  if ((CmpCommon::getDefault(COMP_BOOL_45) == DF_ON) && (exprOpCode == REL_JOIN) && (mergeMethod == INNER_JOIN_MERGE) &&
  ...
  if (NOT(mergeMethod == UNION_MERGE || mergeMethod == OR_MERGE || mergeMethod == LEFT_JOIN_OR_MERGE)
      /* not really a join */
  ) {
    // --------------------------------------------------------------------
    ...
    for (CollIndex j = 0; j < mergedStatDesc->nonVegEquals_.entries(); j++) {
      ...
      if (foundFlag == FALSE) nonVegEquals_.insert(tmpDescJ);
    }  // for j
    ...
    // --------------------------------------------------------------------
    if (mergedStatDesc->getMergeState() == mergeState_ && forceMerge == FALSE) {
      ...
      if (rootColStats->getRowcount() > mergedColStats->getRowcount()) rootColStats->overwrite(*mergedColStats);
    ...
    else if (mergedStatDesc->getMergeState().contains(mergeState()) && forceMerge == FALSE) {
    ...
    if (forceMerge == TRUE || skipJoin == FALSE) {
      ...
      rootColStats->mergeColStats(mergedColStats, mergeMethod, isNumeric, exprOpCode, mergeFVs);
  ...
  else  // It's a UNION, OR or LEFT_JOIN_OR.
  {
    ...
    while (j < nonVegEquals_.entries()) {
      ...
      for (CollIndex i = 0; i < mergedStatDesc->nonVegEquals_.entries() && !foundFlag; i++) {
        ColStatDescSharedPtr tmpDescI = mergedStatDesc->nonVegEquals_[i];
        if (tmpDescI->VEGcolumn_ == tmpDescJ->VEGcolumn_) foundFlag = TRUE;
      }  // for i
    ...
    if (mergeMethod == UNION_MERGE || mergeMethod == OR_MERGE) {
      // -------------------------------------------------------------
...
void ColStatDescList::setScaleFactor(CostScalar val) {
  if (CmpCommon::getDefault(COMP_BOOL_42) == DF_OFF) return;
...
NABoolean ColStatDesc::derivOfLikeAndSimilarPredApp(const ItemExpr *predApplied) {
  ...
  if ((op == ITM_GREATER_EQ) OR(op == ITM_GREATER) OR(op == ITM_LESS) OR(op == ITM_LESS_EQ)) {
    BiRelat *br = (BiRelat *)predApplied;
    predDerivOfLike = br->derivativeOfLike();
    if (!predDerivOfLike) return FALSE;
  }

  for (ValueId id = appliedPreds_.init(); appliedPreds_.next(id); appliedPreds_.advance(id)) {
    const ItemExpr *pred = id.getItemExpr();
    OperatorTypeEnum appliedOp = pred->getOperatorType();

    if ((appliedOp == ITM_GREATER_EQ) OR(appliedOp == ITM_GREATER) OR(appliedOp == ITM_LESS)
            OR(appliedOp == ITM_LESS_EQ)) {
...
// and <, <= predicate pairs.
// ------------------------------------------------------------------------
NABoolean ColStatDesc::isSimilarPredicateApplied(const OperatorTypeEnum op) const {
  for (ValueId id = appliedPreds_.init(); appliedPreds_.next(id); appliedPreds_.advance(id)) {
    const ItemExpr *pred = id.getItemExpr();
    OperatorTypeEnum appliedOp = pred->getOperatorType();
...
// ------------------------------------------------------------------------
CostScalar ColStatDesc::selForRelativeRange(const OperatorTypeEnum op, const ValueId &column, ItemExpr *newPred) const {
  ...
  if (getColStats()->isFakeHistogram()) return csOne;
  ...
  if (savePred)  //  if we have found a between type of operation - do more
  {
    ...
    if ((saveChildPred->getOperatorType() == ITM_CAST) || (saveChildPred->getOperatorType() == ITM_NOTCOVERED))
    ...
    if (NOT((newPredArity == 0 AND(saveOp == ITM_PLUS OR saveOp == ITM_MINUS))
                OR(savePredArity == 0 AND(newOp == ITM_PLUS OR newOp == ITM_MINUS)))) {
      return csOne;
    }
    ...
    for (Int32 arity = 0; arity < twoPred->getArity(); arity++) {
      ...
      if ((operand->getOperatorType() == ITM_HOSTVAR) OR(operand->getOperatorType() == ITM_DYN_PARAM)
              OR(operand->getOperatorType() == ITM_CACHE_PARAM) OR(operand->getOperatorType() == ITM_CURRENT_USER)
                  OR(operand->getOperatorType() == ITM_CURRENT_TENANT)
      ...
      if (isAConstant) constant = isAConstant;
    }
    ...
    if ((twoOp == ITM_PLUS AND(origTwoOp == ITM_GREATER_EQ OR origTwoOp == ITM_GREATER))
            OR(twoOp == ITM_MINUS AND(origTwoOp == ITM_LESS_EQ OR origTwoOp == ITM_LESS)))
      return csZero;
    ...
    if (op == ITM_GREATER_EQ || op == ITM_LESS_EQ) rangeInc += 1;
    if (appliedOp == ITM_GREATER_EQ || appliedOp == ITM_LESS_EQ) rangeInc += 1;

    rangeInc = rangeInc * scaleMult;

    if (lo + cn + rangeInc > hi) return csOne;
    ...
    if (sel.getValue() < 0.0 || sel.getValue() > csOne.getValue()) return csOne;
    CostScalar defaultSel = CURRSTMT_OPTDEFAULTS->defSelForRangePred();
    ...
    if (sel < selForAvgRowcount) sel = selForAvgRowcount;
  }
  return sel;
}

void ColStatDesc::print(FILE *ofd, const char *prefix, const char *suffix, CollHeap *c, char *buf,
...
void ColStatDesc::display() const { print(); }
...
void ColStatDescList::destroy() {
  while (entries() > 0) removeDeepCopyAt(0);
}
...
NABoolean ColStatDescList::contains(const ValueId &column) const {
  for (CollIndex i = 0; i < entries(); i++) {
    ...
    if (msSet.contains(column)) return TRUE;
  }
...
NABoolean ColStatDescList::contains(const ValueIdList &colList) const {
...
NABoolean ColStatDescList::containsAtLeastOneFake() const {
  // -----------------------------------------------------------------------
...
NABoolean ColStatDescList::selectivityHintApplied() const {
  for (CollIndex i = 0; i < entries(); i++) {
    if ((*this)[i]->getColStats()->isSelectivitySetUsingHint()) return TRUE;
  }
  return FALSE;
}
...
#define ALLOCATE_COL_STAT_DESC_AND_SET_IT_UP(x)                                    \
  ...
  tmpColStatsPtr->setShapeChanged(shapeChangedMask && tmpColStatsPtr->isShapeChanged());

void ColStatDescList::makeDeepCopy(const ColStatDescList &source, const CostScalar &scale,
  ...
  hasExpressionHistogram_ = source.hasExpressionHistogram_;

  setUecList(source.getUecList());
}  // makeDeepCopy

void ColStatDescList::appendDeepCopy(const ColStatDescList &source, const CollIndex firstN, const CostScalar &scale,
                                     const NABoolean shapeChangedMask) {
  ...
  if (source.hasExpressionHistogram_) hasExpressionHistogram_ = TRUE;

  // add the other CSDL's uec list to this one
  insertIntoUecList(source.getUecList());
}  // appendDeepCopy

void ColStatDescList::prependDeepCopy(const ColStatDescList &source, const CollIndex firstN, const CostScalar &scale,
                                      const NABoolean shapeChangedMask) {
  for (CollIndex i = 0; i < source.entries() && i < firstN; i++) {
    ALLOCATE_COL_STAT_DESC_AND_SET_IT_UP(*source[i]);
    insertAt(i, tmpColStatDescPtr);
...
void ColStatDescList::insertDeepCopy(const ColStatDescSharedPtr &source, const CostScalar &scale,
                                     const NABoolean shapeChangedMask) {
  ALLOCATE_COL_STAT_DESC_AND_SET_IT_UP(*source);
  insert(tmpColStatDescPtr);

  if (tmpColStatDescPtr->isExpressionHistogram()) hasExpressionHistogram_ = TRUE;
}  // insertDeepCopy

void ColStatDescList::insertDeepCopyAt(const CollIndex entry, const ColStatDescSharedPtr &source,
                                       const CostScalar &scale, const NABoolean shapeChangedMask) {
  ALLOCATE_COL_STAT_DESC_AND_SET_IT_UP(*source);
...
void ColStatDescList::makeMappedDeepCopy(const ColStatDescList &source, ValueIdMap &map,
  ...
  for (CollIndex i = 0; i < source.entries(); i++) {
    ...
    if (includeUnmappedColumns || topVid != bottomVid) {
      ColStatDescSharedPtr tmpColStatDescPtr(new (HISTHEAP) ColStatDesc(), HISTHEAP);
  ...
  MultiColumnUecList *mappedMultiColUECs = new (CmpCommon::statementHeap()) MultiColumnUecList();

  mappedMultiColUECs->insertMappedList(source.getUecList(), map);
  setUecList(mappedMultiColUECs);
}  // makeMappedDeepCopy

void ColStatDescList::removeDeepCopyAt(const CollIndex entry) { removeAt(entry); }  // removeDeepCopyAt
...
void ColStatDescList::copyAndScaleHistograms(CostScalar scale) {
  for (CollIndex i = 0; i < entries(); i++) {
...
void ColStatDescList::setBaseUecToTotalUec() {
  for (CollIndex i = 0; i < entries(); i++) {
    ColStatDescSharedPtr columnStatDesc = (*this)[i];
    ColStatsSharedPtr columnStats = columnStatDesc->getColStatsToModify();
...
void ColStatDescList::computeMaxFreq(NABoolean forced) {
  for (CollIndex i = 0; i < entries(); i++) {
    ColStatDescSharedPtr columnStatDesc = (*this)[i];
    ColStatsSharedPtr columnStats = columnStatDesc->getColStatsToModify();
...
void ColStatDescList::addColStatDescForVirtualCol(const CostScalar &uec, const CostScalar &rowCount,
                                                  const ValueId colId, const ValueId vegCol, const ValueId mergeState,
                                                  const RelExpr *expr, NABoolean defineVirtual) {
  ...
  if ((defineVirtual == FALSE) && expr && (expr->getOperatorType() == REL_TUPLE_LIST)) {
    fakeHist = FALSE;
    ...
    if (tList.entries() <= CURRSTMT_OPTDEFAULTS->histTupleFreqValListThreshold()) {
      for (CollIndex i = 0; (i < (CollIndex)tList.entries()); i++) {
        ItemExpr *tupleVal = ((ItemExpr *)tList[i])->castToItemExpr();
        if (tupleVal == NULL) continue;

        ValueIdSet leafs;
        tupleVal->getLeafValueIds(leafs);

        if (leafs.entries() == 0) continue;

        for (ValueId val = leafs.init(); leafs.next(val); leafs.advance(val)) {
          if (val == NULL_VALUE_ID) continue;
          ...
          if (leafVal->getOperatorType() == ITM_CONSTANT) {
            ...
            UInt32 hashValue = 0;
            if (cv->getType()->useHashRepresentation() && cv->getType()->useHashInFrequentValue())
  ...
  ColStatsSharedPtr fakeColStats(new (HISTHEAP)
                                     ColStats(id, uec, rowCount, rowCount, FALSE, FALSE, emptyHist, FALSE, 1.0, 1.0,
  ...
  ColStatDescSharedPtr fakeStatDesc(new (HISTHEAP) ColStatDesc(fakeColStats, colId), HISTHEAP);
...
NABoolean isPredTransformedInList(const ValueId &predId, const ValueId &column, Int32 &leaves) {
  ...
  if (Rop != ITM_EQUAL) return FALSE;

  const ValueId &RidLeftChild = Rpred->child(0)->getValueId();

  if (RidLeftChild != column) return FALSE;
...
CostScalar ColStatDescList::estimateCardinality(const CostScalar &initialRowCount, const ValueIdSet &setOfPredicates,
                                                const ValueIdSet &outerReferences, const Join *expr,
                                                const SelectivityHint *selHint, const CardinalityHint *cardHint,
                                                CollIndex &numOuterColStats, ValueIdSet &unresolvedPreds,
                                                MergeType mergeMethod, OperatorTypeEnum exprOpCode,
...
#if 0
  // debug code useful for studying the application of predicates to histograms
  // the "if" below screens out all the uninteresting metadata queries and CQD
  // compilations
  bool displayOnExit = false;
  if ((CmpCommon::context()->getCIClass() != CmpContextInfo::CMPCONTEXT_TYPE_META) &&
     (maxSelectivity == NULL) && (entries() > 0) && (setOfPredicates.entries() > 0))
  {
    cout << "*** On entry to ColStatDescList::estimateCardinality ***" << endl;
    display();  // so we can debug problems in the display method
    setOfPredicates.display();  // so we can see what predicates are being applied
    displayOnExit = true;
  }
  // end debug code
#endif
  ...
  if (maxSelectivity == NULL) {
    ...
    CollIndex limit =
        ((mergeMethod == SEMI_JOIN_MERGE || mergeMethod == ANTI_SEMI_JOIN_MERGE) ? numOuterColStats : entries());
    ...
    const CostScalar &matchRowcount = (*this)[0]->getColStats()->getRowcount();
    // try this out : if the current rowcount isn't what it's "supposed" to be,
    // set it as requested and see how things go
    if (matchRowcount != newRowCount) synchronizeStats(matchRowcount, newRowCount, limit);
  }  // maxSelectivity == NULL

  ValueIdSet EqLocalPreds, OtherLocalPreds, EqNonLocalPreds, OtherNonLocalPreds, BiLogicPreds, DefaultPreds;

  setOfPredicates.categorizePredicates(outerReferences, EqLocalPreds, OtherLocalPreds, EqNonLocalPreds,
  ...
  if (CmpCommon::getDefault(RANGESPEC_TRANSFORMATION) == DF_ON) {
    for (ValueId subVid = DefaultPreds.init(); DefaultPreds.next(subVid); DefaultPreds.advance(subVid)) {
      ValueIdSet vs;
      ...
      if (pred->getOperatorType() == ITM_RANGE_SPEC_FUNC) {
        ...
        vs.categorizePredicates(outerReferences, EqLocalPreds, OtherLocalPreds, EqNonLocalPreds, OtherNonLocalPreds,
                                BiLogicPreds, subDefPreds);
  ...
  // -----------------------------------------------------------------------
  newRowCount = applyBiLogicPred(tempRowcount, BiLogicPreds, outerReferences, expr, selHint, cardHint, numOuterColStats,
                                 unresolvedPreds, mergeMethod, biLogicPredReductions, exprOpCode, maxSelectivity);
  ...
  for (id = OtherLocalPreds.init(); OtherLocalPreds.next(id); OtherLocalPreds.advance(id)) {
    ItemExpr *pred = id.getItemExpr();
  ...
  if ((mergeMethod == SEMI_JOIN_MERGE || mergeMethod == ANTI_SEMI_JOIN_MERGE) AND(numOuterColStats < this->entries())
          AND maxSelectivity == NULL) {
    semiJoinRowcountBeforePreds = (*this)[numOuterColStats]->getColStats()->getRowcount();
  }
  ...
  for (id = EqLocalPreds.init(); EqLocalPreds.next(id); EqLocalPreds.advance(id)) {
    tempRowcount = rowcountBeforePreds;

    ItemExpr *pred = id.getItemExpr();

    if (!applyVEGPred(pred, tempRowcount, numOuterColStats, mergeMethod, exprOpCode, maxSelectivity))
      DefaultPreds.insert(id);

    if (maxSelectivity == NULL) newRowCount = tempRowcount;
  }
  // -----------------------------------------------------------------------
  // Apply non-VEG equality predicates: Local and non-local
  // -----------------------------------------------------------------------

  for (id = nonVEGEqualPredSet.init(); nonVEGEqualPredSet.next(id); nonVEGEqualPredSet.advance(id)) {
    ItemExpr *pred = id.getItemExpr();

    tempRowcount = rowcountBeforePreds;

    if (!applyPred(pred, tempRowcount, numOuterColStats, mergeMethod, exprOpCode, maxSelectivity)) {
      DefaultPreds.insert(id);
  ...
  if (maxSelectivity == NULL) {
    ...
    if (((mergeMethod != SEMI_JOIN_MERGE) && (mergeMethod != ANTI_SEMI_JOIN_MERGE)) || doMCAdjust) {
      this->useMultiUecIfMultipleJoins(newRowCount,         /* in/out */
      ...
      // outlined above.
      // ---------------------------------------------------------------------
      this->useMultiUecIfCorrelatedPreds(newRowCount,         /* in/out */
                                         rowcountBeforePreds, /* in */
  ...
  for (id = DefaultPreds.init(); DefaultPreds.next(id); DefaultPreds.advance(id)) {
    tempRowcount = rowcountBeforePreds;
    ...
    if (NOT unresolvedPreds.contains(id)) {
      ...
      applyDefaultPred(pred, thisPredGlobalReduction, exprOpCode, maxSelectivity);
  ...
  if ((newRowCount.getValue() - floor(newRowCount.getValue())) > 0.00001) newRowCount = ceil(newRowCount.getValue());
  ...
  if (mergeMethod == SEMI_JOIN_MERGE || mergeMethod == ANTI_SEMI_JOIN_MERGE) {
    // if after applying MC stats, the new rowcount exceeded left row count
    // then max it out to left row count
    if (newRowCount > rowcountBeforePreds) newRowCount = rowcountBeforePreds;
  }
  ...
  if (mergeMethod == SEMI_JOIN_MERGE || mergeMethod == ANTI_SEMI_JOIN_MERGE) {
    ...
    for (i = loopLimit; i < entries(); i++) {
      if (semiJoinRowcountBeforePreds.isGreaterThanZero() /* > csZero */)
        rowred = (*this)[i]->getColStats()->getRowcount() / semiJoinRowcountBeforePreds;
      else
        rowred = csZero;
    ...
    this->useMultiUecIfMultipleJoins(newSemiJoinRowCount,         /* in/out */
    ...
    // of the left child.
    if (newSemiJoinRowCount > rowcountBeforePreds) newSemiJoinRowCount = rowcountBeforePreds;
    ...
    for (i = loopLimit; i < entries(); i++) {
      ...
      if (oldCount != semiJoinRowcountBeforePreds)  // only apply the delta
        ...
        (*this)[i]->synchronizeStats(semiJoinRowcountBeforePreds, newSemiJoinRowCount);
    }
      ...
      if ((*this)[i]->nonVegEquals().entries() != 0) {
        ...
        while ((*this)[i]->nonVegEquals().entries() != 0) {
          ...
          for (CollIndex j = 0; j < entries(); j++) {
            if (NOT found AND column == (*this)[j]->getColumn()) found = TRUE;
          }
          if (NOT found && NOT(tmpDesc->isFromInnerTable() && mergeMethod == OUTER_JOIN_MERGE)) {
            ...
            if (tmpDesc->nonVegEquals().entries() != 0) redo = TRUE;
  ...
  if (expr) {
    ...
    if (joinedMCSkewedValueList) {
      if (!mcSkewedValueLists_) mcSkewedValueLists_ = new (HISTHEAP) MultiColumnSkewedValueLists();
  ...
  if (cardHint || selHint)
    newRowCount = adjustRowcountWithHint(cardHint, selHint, setOfPredicates, newRowCount, initialRowCount);

#if 0
  // debug code useful for debugging application of predicates to histograms
  if (displayOnExit)
    {
      cout << "*** On exit from ColStatDescList::estimateCardinality ***" << endl;
      display();
    }
  // end debug code
#endif
...
CostScalar ColStatDescList::adjustRowcountWithHint(const CardinalityHint *cardHint, const SelectivityHint *selHint,
...
CostScalar ColStatDescList::getCardOfBusiestStream(const PartitioningFunction *partFunc, int numOfParts,
  ...
  if ((numOfParts == 1) || (partFunc->isASinglePartitionPartitioningFunction())) {
  ...
  for (ValueId partKeyElement = partKey.init(); partKey.next(partKeyElement); partKey.advance(partKeyElement)) {
    ...
    if (groupAttr) {
      ...
      if (grpAnalysis) {
        ...
        if (NOT treeSet.isEmpty()) myColumns = treeSet.getUsedCols();
        ...
        if (NOT myColumns.isEmpty()) baseColSet.intersectSet(myColumns);
      }
    }
  ...
  if (partFunc->isAHashPartitioningFunction() || partFunc->isATableHashPartitioningFunction()) {
    ...
    CostScalar histSkewAdjustment = (ActiveSchemaDB()->getDefaults()).getAsDouble(HIST_SKEW_COST_ADJUSTMENT);
    maxCardPerStream =
        (maxCardPerStream * histSkewAdjustment) + (uniformDistRowCountPerStream * (csOne - histSkewAdjustment));
...
NABoolean ColStatDescList::identifyMergeCandidates(ItemExpr *VEGpred, CollIndex &rootStatIndex,
  ...
  for (CollIndex i = 0; i < entries(); i++) {
    // Skip expression histograms, as a complex expression will never appear
    // in a VEG.
    if ((*this)[i]->isExpressionHistogram()) continue;
    ...
    // acceptable.
    if (NOT(colType == ITM_VEG_REFERENCE OR colType == ITM_INSTANTIATE_NULL OR colType == ITM_VALUEIDUNION OR colType ==
            ITM_UNPACKCOL OR colType == ITM_ROWSETARRAY_SCAN OR(*this)[i]->getColStats()->isVirtualColForHist())) {
    ...
    if (colType == ITM_VEG_REFERENCE) {
      ...
      for (ValueId id = VEGGroup.init(); VEGGroup.next(id); VEGGroup.advance(id)) {
        ...
        if (id.getItemExpr()->getOperatorType() == ITM_INSTANTIATE_NULL AND colType == ITM_INSTANTIATE_NULL) {
    ...
    } else if (colType == ITM_VALUEIDUNION || colType == ITM_UNPACKCOL || colType == ITM_ROWSETARRAY_SCAN) {
      if (VEGpred->containsTheGivenValue(columnsExpr->getValueId())) {
        foundCandidate = TRUE;
      }
    }
...
// -----------------------------------------------------------------------
NABoolean ColStatDescList::identifyMergeCandidates(ItemExpr *operand, CollIndexList &statsToMerge) const {
  ...
  for (CollIndex i = 0; i < entries(); i++) {
    if ((*this)[i]->isExpressionHistogram()) {
      if ((*this)[i]->expressionMatches(operand)) {
        // this histogram is an expression histogram, and the operand matches it
        statsToMerge.insert(i);
      }
      continue;
    }

    // regular histogram code path
    ...
    do {
      ...
      if (colType == ITM_INSTANTIATE_NULL && exprType == ITM_VEG_REFERENCE) {
        ...
        for (id = VEGGroup.init(); VEGGroup.next(id); VEGGroup.advance(id)) {
      ...
      else if (colType == ITM_VEG_REFERENCE && exprType == ITM_INSTANTIATE_NULL) {
      ...
      else if (colType == ITM_VEG_REFERENCE && exprType == ITM_VEG_REFERENCE) {
        const VEG *statVEG = ((VEGReference *)columnsExpr)->getVEG();
        const VEG *exprVEG = ((VEGReference *)operand)->getVEG();
        ...
        } else  // not a direct match
        {
          ...
          for (id = VEGGroup.init(); VEGGroup.next(id); VEGGroup.advance(id)) {
            if (id.getItemExpr()->getOperatorType() == ITM_VEG_REFERENCE) {
              const VEG *nestedVEG = ((VEGReference *)(id.getItemExpr()))->getVEG();
          ...
          if (!foundCandidate) {
            ...
            for (id = VEGGroup2.init(); VEGGroup2.next(id); VEGGroup2.advance(id)) {
              if (id.getItemExpr()->getOperatorType() == ITM_VEG_REFERENCE) {
                const VEG *nestedVEG = ((VEGReference *)(id.getItemExpr()))->getVEG();
          ...
          (colType == ITM_INSTANTIATE_NULL && exprType == ITM_INSTANTIATE_NULL)
          // Case 5 : ValueIdUnions
          OR(colType == ITM_VALUEIDUNION && exprType == ITM_VALUEIDUNION)
          // Case 6 : special case for packed col
          OR(colType == ITM_UNPACKCOL && exprType == ITM_UNPACKCOL)
          // Case 7 : VEG_PREDs
          OR(colType == ITM_VEG_PREDICATE && exprType == ITM_VEG_PREDICATE)
              OR((CmpCommon::getDefault(COMP_BOOL_48) == DF_ON) &&
                 (colType == ITM_VEG_REFERENCE && exprType == ITM_NATYPE))) {
      ...
      if (nonVEGCount > 0) {
        ColStatDescSharedPtr tmpColStatDesc = ((*this)[i]->getNonVegEquals())[--nonVEGCount];
...
// -----------------------------------------------------------------------
// ColStatDescList::getNodeForStats
//
// This is a wrapper for ItemExpr::getLeafValueIfUseStats. It considers
// whether the operand in question matches an expressions histogram before
// drilling down into function calls in the operand.
//
// Returns the appropriate node (which may or may not be a leaf) that
// is the best choice for matching a histogram (if any exist). If
// a matching expressions histogram was found, match is set with the
// CollIndex of that histogram.
// -----------------------------------------------------------------------
ItemExpr *ColStatDescList::getNodeForStats(ItemExpr *operand, CollIndex start, CollIndex &match /* out */,
                                           NABoolean digIntoInstantiateNull) {
  match = -1;  // assume no matching expression histogram
  if (hasExpressionHistogram_) {
    ItemExpr *opToMatch = operand;
    if (digIntoInstantiateNull && operand->getOperatorType() == ITM_INSTANTIATE_NULL) opToMatch = operand->child(0);

    for (CollIndex i = start; i < entries(); i++) {
      if ((*this)[i]->expressionMatches(opToMatch)) {
        match = i;  // found a match
        return opToMatch;
      }
    }
    // if nothing matched, fall through
  }
  return operand->getLeafValueIfUseStats(digIntoInstantiateNull);
...
// predicates at once.
// -----------------------------------------------------------------------
NABoolean ColStatDescList::applyVEGPred(ItemExpr *VEGpred, CostScalar &rowcount, CollIndex &numOuterColStats,
                                        MergeType mergeMethod, OperatorTypeEnum opType, CostScalar *maxSelectivity) {
  VEG *predVEG = NULL;
  ...
  NABoolean containsSelectivityParam = (containsConstExpr AND constExprPtr AND constExprPtr->getOperatorType() ==
                                        ITM_CACHE_PARAM AND constExprPtr->castToSelParameter());
  ...
  if (containsSelectivityParam) containsConstant = TRUE;
  ...
  NABoolean foundRoot = identifyMergeCandidates(VEGpred, rootStatIndex, statsToMerge);
  ...
  if (candidateHistograms == 0) return FALSE;  // we did not apply the predicate
  ...
  // want.
  if (foundRoot && candidateHistograms == 1 && containsHostvar) return FALSE;  // we didn't apply the predicate
  ...
  if (foundRoot) {
    ...
    if (VEGpred->isSelectivitySetUsingHint()) mergeStatePriorToJoin = rootStatDesc->getMergeState();
  ...
  if (foundRoot && ((rootColStats->getStatColumns()).entries() == 0) && (CmpCommon::getDefault(COMP_BOOL_165) == DF_ON))
  ...
  if (foundRoot && containsConstant && (tupleVirtColFix == FALSE)) {
    ...
    if (containsSelectivityParam == TRUE) constant = ((SelParameter *)constExprPtr)->getConstVal();
    ...
    if (maxSelectivity == NULL) {
      if (rootColStats->isOrigFakeHist()) {
        ...
        if (!rootColStats->isUnique()) newRC = ceil(sqrt(rowcount.getValue()));
  ...
  if (foundRoot && statsToMerge.entries() > 1)  // make sure there is work
  {
    ...
    for (i = 0; i < statsToMerge.entries(); i++) {
      if (statsToMerge[i] != rootStatIndex) {
        ...
        if (NOT(rootStatIndex < numOuterColStats && statsToMerge[i] >= numOuterColStats)) {
        ...
        if (containsConstant) {
          ...
          if (!appliedPreds.contains(predValueId)) {
            ...
            if (maxSelectivity == NULL) {
              if (!selHintSpecified)
                (*this)[statsToMerge[i]]->getColStatsToModify()->setToSingleValue(normValue, constExpr, &rowcount);
            } else
              (*this)[statsToMerge[i]]->getColStatsToModify()->adjustMaxSelectivity(normValue, constExpr, &rowcount,
                                                                                    maxSelectivity);
          }
        }
        ...
        if (maxSelectivity == NULL) {
          ...
          if (((mergeMethod == OUTER_JOIN_MERGE) && containsConstant) || joinOnOneTable) {
            // The merge has already been done, so skip mergeColStatDesc
          } else {
            // else merge the left and the right histograms
            rootStatDesc->mergeColStatDesc((*this)[statsToMerge[i]], localMergeMethod,
  ...
  if (maxSelectivity == NULL) {
    ...
    if (!containsConstant && statsToMerge.entries() > 1) {
      ...
      for (i = statsToMerge.entries() - 1; i >= 0; i--) {
        if (statsToMerge[i] != rootStatIndex) {
          if (mergeMethod == OUTER_JOIN_MERGE && statsToMerge[i] >= numOuterColStats) {
            ColStatsSharedPtr updateColStats = (*this)[statsToMerge[i]]->getColStatsToModify();
            ...
            if (statsToMerge[i] < numOuterColStats) numOuterColStats--;
          }
        }
    ...
    if (candidateHistograms <= 1)
      appliedPredicateFlag = TRUE;
    else if (candidateHistograms > statsToMerge.entries() && !containsConstant)
      appliedPredicateFlag = FALSE;
  }  // maxSelectivity == NULL

  // If user specified selectivity for this predicate, we need to make
  // adjustment in reduction to reflect that.
  if ((containsConstant || statsToMerge.entries() > 1) && selHintSpecified) {
    ...
    if (VEGpred->getSelectivityFactor() != -1) selPred = VEGpred;

    ValueIdSet mergeState = rootStatDesc->getMergeState();
    ValueIdSet predSet = ((VEGPredicate *)VEGpred)->getPredsWithSelectivities();

    for (ValueId predId = predSet.init(); predSet.next(predId); predSet.advance(predId)) {
      if (selPred) break;
      ...
      if (containsConstant) {
        ...
        if ((mergeState == ValueIdSet(tempPred->child(0))) && (tempPred->child(1)->castToConstValue(neg)))
          selPred = tempPred;
        else
          continue;
      } else {
        ...
        if (mergeState == baseCols)
          ...
          if (baseCols.contains(newColumnMerged)) {
            ...
            if (mergeStatePriorToJoin.contains(secondColumnInTheMerge)) selPred = tempPred;
          } else
            continue;
        } else
          continue;
    ...
    if (selPred) rootStatDesc->applySelIfSpecifiedViaHint(selPred, oldRowcountForSelAdj);
  }
...
//
// -----------------------------------------------------------------------
CostScalar ColStatDescList::applyBiLogicPred(CostScalar &tempRowcount, ValueIdSet &BiLogicPreds,
                                             const ValueIdSet &outerReferences, const Join *expr,
                                             const SelectivityHint *selHint, const CardinalityHint *cardHint,
                                             CollIndex &numOuterColStats, ValueIdSet &unresolvedPreds,
                                             MergeType mergeMethod,
                                             NAHashDictionary<ValueId, CostScalar> &biLogicPredReductions,  // in/mod
                                             OperatorTypeEnum exprOpCode, CostScalar *maxSelectivity) {
  ValueId id;
  ...
  for (id = BiLogicPreds.init(); BiLogicPreds.next(id); BiLogicPreds.advance(id)) {
    ItemExpr *pred = id.getItemExpr();
    ...
    if (op == ITM_AND) {
      ...
      newRowCount =
          estimateCardinality(tempRowcount, leftChildSet, outerReferences, expr, selHint, cardHint, numOuterColStats,
                              unresolvedPreds, mergeMethod, ITM_AND, doEstCard ? NULL : &maxSel1);
      tempRowcount = newRowCount;

      newRowCount =
          estimateCardinality(tempRowcount, rightChildSet, outerReferences, expr, selHint, cardHint, numOuterColStats,
                              unresolvedPreds, mergeMethod, ITM_AND, doEstCard ? NULL : &maxSel2);
      tempRowcount = newRowCount;
      // maxSelectivity(p1 AND p2)==MIN(maxSelectivity(p1),maxSelectivity(p2))
      if (!doEstCard) {
      ...
      if ((Lop == ITM_OR AND Rop ==
           ITM_EQUAL AND
                   ...
                   isPredTransformedInList(Lid, RidLeft, in_list_members) AND
                       // $$$ NB: we ARBITRARILY set the number of IN-list members
                       // $$$ for which we try to carefully perform *EXACT*
                       // $$$ histogram manipulation
                       in_list_members > HIST_MAX_IN_LIST_MEMBERS AND
                                             // just in case the constant/default has a dumb value
                                             in_list_members > 5) ||
          ((Lop == ITM_OR) AND(inListCase == FALSE)
               AND(((BiLogic *)pred)->getNumLeaves() > HIST_MAX_IN_LIST_MEMBERS))) {
        large_in_list = TRUE;
        if (maxSelectivity == NULL) {
          ...
          if (RidLeft == NULL_VALUE_ID) return newRowCount;
          ItemExpr *RidLeftExpr = RidLeft.getItemExpr();
          OperatorTypeEnum RidLeftOp = RidLeftExpr->getOperatorType();

          while ((RidLeftOp != ITM_INDEXCOLUMN) && (RidLeftOp != ITM_INSTANTIATE_NULL) &&
                 (RidLeftOp != ITM_BASECOLUMN) && (RidLeftOp != ITM_VEG_REFERENCE)) {
            // Do not assume that the left child of EQUAL will always be a column
            // it can be a SUBSTRING too, with the column CASTed.

            if ((RidLeftExpr->child(0)) && (RidLeftExpr->child(0)->getOperatorType() != ITM_CONSTANT))
          ...
          if (NOT this->getColStatDescIndexForColumn(histIdx, RidLeft)) {
            ...
            if (RidLeft.getItemExpr()->getOperatorType() == ITM_VEG_REFERENCE) {
              ...
              VEG *RidLeftVEG = ((VEGReference *)RidLeft.getItemExpr())->getVEG();
              ...
              if (containsConstant) return newRowCount;
            }
            ...
            if (inListCase && (predStats->getTotalUec() <= in_list_members)) {
              ...
              newRowCount = predStats->getRowcount() * redFactor;
      ...
      if (NOT large_in_list) {
        ...
        NAHashDictionary<ValueId, ValueIdSet> priorAppliedPredsSet(&(ValueIdHashFn), dictSize, TRUE, HISTHEAP);

        NAHashDictionary<ValueId, ValueIdSet> priorMergeStateSet(&(ValueIdHashFn), dictSize, TRUE, HISTHEAP);
        ...
        CollIndex i;
        copyStatsList.makeDeepCopy(*this,
        ...
        CostScalar maxSel1 = 1.0, maxSel2 = 1.0;
        CostScalar leftRowCount = estimateCardinality(tempRowcount, leftChildSet, outerReferences, expr, selHint,
                                                      cardHint, leftNumOuterColStats, unresolvedPreds, mergeMethod,
                                                      ITM_OR, doEstCard ? NULL : &maxSel1);
        CostScalar prob1, prob2;
        prob1 = leftRowCount / tempRowcount;

        CostScalar rightRowCount = copyStatsList.estimateCardinality(
            tempRowcount, rightChildSet, outerReferences, expr, selHint, cardHint, rightNumOuterColStats,
            originalUnResolved, mergeMethod, ITM_OR, doEstCard ? NULL : &maxSel2);
        ...
        if (maxSelectivity == NULL) {
          ...
          if (prob1 > csOne) {
            ...
            for (CollIndex i = 0; i < this->entries(); i++) (*this)[i]->synchronizeStats(leftRowCount, tempRowcount);
          ...
          newRowCount = saveTempRowCount * (prob1 + prob2 - CostScalar(prob1prob2));
          ...
          for (currentL = 0; currentL < entries(); currentL++) {
            const ItemExpr *leftItem = (*this)[currentL]->getVEGColumn().getItemExpr();
            ComUID leftHistId = (*this)[currentL]->getColStats()->getHistogramId();

            NABoolean found = FALSE;
            currentR = 0;
            while (NOT found && currentR < copyStatsList.entries()) {
              const ItemExpr *rightItem = (copyStatsList)[currentR]->getVEGColumn().getItemExpr();
              ComUID rightHistId = (copyStatsList)[currentR]->getColStats()->getHistogramId();

              if (leftItem->getValueId() == rightItem->getValueId() && (leftHistId == rightHistId))
                found = TRUE;
              else
                currentR++;
            }
            ...
            ColStatsSharedPtr leftColStats = (*this)[currentL]->getColStatsToModify();
            ColStatsSharedPtr rightColStats = (copyStatsList)[currentR]->getColStatsToModify();
            ...
            // neither changed
            if (NOT leftColStats->isShapeChanged() && NOT rightColStats->isShapeChanged()) {
              copyStatsList.removeAt(currentR);
            } else  // at least one side shape-changed.
            {
              atLeastOneChanged = TRUE;

              CostScalar saveRowcount = leftColStats->getRowcount() + rightColStats->getRowcount();
              (*this)[currentL]->mergeColStatDesc((copyStatsList)[currentR], OR_MERGE, FALSE, exprOpCode);

              // for debugging informational value, track the
              // histograms impacted by an OR.
              (*this)[currentL]->appliedPreds().insert(id);

              if (rightColStats->isSelectivitySetUsingHint()) leftColStats->setSelectivitySetUsingHint();

              // both sides changed?
              if (leftColStats->isShapeChanged() && rightColStats->isShapeChanged()) {
                CostScalar curDif = saveRowcount - (*this)[currentL]->getColStats()->getRowcount();
                if (curDif < overLappedRowcount) overLappedRowcount = curDif;
          ...
          // If user specified selectivity for this predicate, we need to make
          // adjustment in reduction to reflect that.
          if ((exprOpCode == REL_SCAN) && (pred->isSelectivitySetUsingHint())) {
          ...
          // ** --> or does this not have to be as cautious as it is?
          {
            for (i = 0; i < shapeChangedItemExprs.entries(); i++) {
              ...
              for (CollIndex j = 0; j < entries(); j++) {
                ...
                ComUID checkHistId = (*this)[j]->getColStats()->getHistogramId();
    ...
    if (exprOpCode == REL_SCAN && !maxSelectivity && (CmpCommon::getDefault(COMP_BOOL_67) == DF_ON)) {
      ...
      for (ValueId tempId = predSet.init(); predSet.next(tempId); predSet.advance(tempId)) {
        ...
        if (!tempExpr->equatesToAConstant() || (column && (lhs != column)) || (!lhs)) {
          skip = TRUE;
          break;
        } else if (!column)
          column = lhs;
      ...
      if (!getColStatDescIndexForColumn(i, colId)) continue;
...
// predicates at once.
// -----------------------------------------------------------------------
NABoolean ColStatDescList::applyPred(ItemExpr *pred, CostScalar &newRowcount, CollIndex &numOuterColStats,
                                     MergeType mergeMethod, OperatorTypeEnum exprOpCode, CostScalar *maxSelectivity) {
  ...
  CollIndex lhsMatchingExprHistogram = -1;
  CollIndex rhsMatchingExprHistogram = -1;
  ...
  if ((pred->getOperatorType() == ITM_GREATER_EQ) OR(pred->getOperatorType() == ITM_LESS)) {
    BiRelat *br = (BiRelat *)pred;
  ...
  if ((predExprType == ITM_IS_NULL OR predExprType == ITM_IS_NOT_NULL OR predExprType ==
       ITM_IS_UNKNOWN OR predExprType == ITM_IS_NOT_UNKNOWN)
          OR NOT(predExprType == ITM_EQUAL AND pred->child(0)->castToConstValue(negate) ==
                 NULL AND pred->child(1)->castToConstValue(negate) == NULL) OR(pred->equatesToAConstant())) {
    CostScalar oldRowcountForSelAdj;

    // Simple Case: look for <op> <col>,
    //                       <col> <op> <constant> or
    //                       <constant> <op> <col> predicates
    // This case also considers <expr> <op> <constant>, <constant> <op> <expr>
    //                       and <op> <expr>.

    // The code assumes the <constant> is always on the right.

    // First dig down in the right hand side to get the constant node.
    ...
    if (pred->getArity() > 1) {
      ...
      if (constant == NULL) {
        if (rhs->getOperatorType() == ITM_VEG_REFERENCE) {
          ...
          if (constId != NULL_VALUE_ID) constant = constId.getItemExpr()->castToConstValue(negate);
        } else {
          if ((predExprType == ITM_EQUAL) && (rhs->getOperatorType() == ITM_CACHE_PARAM)) {
            ItemExpr *constantExpr = ((ConstantParameter *)rhs)->getConstVal();
            if (constantExpr == NULL) return FALSE;
    ...
    // Drill down in the lhs (which is always the <col> or <expr>)
    // to the point where we have a matching expression histogram
    // or we are at a leaf node.
    ...
    if ((predExprType == ITM_IS_NULL) || (predExprType == ITM_IS_NOT_NULL)) digIntoInstantiateNull = FALSE;

    if ((lhs->getOperatorType() != ITM_VEG_REFERENCE) &&
        (constant || (predExprType == ITM_EQUAL) || (predExprType == ITM_NOT_EQUAL) || (predExprType == ITM_IS_NULL) ||
         (predExprType == ITM_IS_NOT_NULL))) {
      lhs = getNodeForStats(lhs, numOuterColStats, lhsMatchingExprHistogram);
    }
    ...
    for (i = numOuterColStats /*fix to ALM5128*/; i < entries(); i++) {
      ...
      if ((colStats->isVirtualColForHist() ||
           colStats
               ->isMCforHbasePartitioning()))  // ignore MC stats intervals for now, only used by the splitting logic
        continue;

      if (pred->isSelectivitySetUsingHint()) oldRowcountForSelAdj = colStats->getRowcount();
      ...
      if (thisColStatDesc->modifyStats(pred, lhs, rhs, constant, negate, tempRowcount,
                                       maxSelectivity ? &predMaxSel : NULL)) {
        appliedPredicateFlag = TRUE;
        if (derivedFromLike) {
          ...
          if (maxSelectivity == NULL) {
            ...
            br->adjustRowcountAndUecForLike((*this)[i], rowCountBeforePreds, totalUecBeforePreds, baseUecBeforePreds);
          }     // maxSelectivity == NULL
        } else  // !derivedFromLike
        {
          ...
          if (pred->maxSelectivitySameAsSelectivity() && maxSelectivity && newRowcount > csZero) {
  ...
  if ((*lhs) == (*rhs)) return TRUE;  // indicate that this no-op predicate was applied

  if (predExprType == ITM_EQUAL) {
    lhs = getNodeForStats(lhs, 0, lhsMatchingExprHistogram);
    rhs = getNodeForStats(rhs, 0, rhsMatchingExprHistogram);
  }
  ...
  if ((CmpCommon::getDefault(HIST_MIN_MAX_OPTIMIZATION) == DF_ON) && (predExprType == ITM_EQUAL) &&
      (leftExprType == ITM_VEG_REFERENCE) && (rightExprType == ITM_MIN || rightExprType == ITM_MAX)) {
    // Sometimes, aggregate is transformed to max(max(col)),
    // need to do the following to extract column.
    while (rhs->getOperatorType() == ITM_MIN || rhs->getOperatorType() == ITM_MAX) rhs = rhs->child(0);
    ...
    if (getColStatDescIndexForColumn(i, rhs->getValueId())) {
      ...
      if (colExpr && colExpr->getTableDesc()->getLocalPreds().entries() == 0) {
        ...
        if (getColStatDescIndexForColumn(i, lhs->getValueId())) {
          ...
          // Make a deep copy
          ColStatsSharedPtr copyOfOrigLeftColStats = ColStats::deepCopy(*origLhsColStats, HISTHEAP);
          ...
          if (localPredReduction != csOne) copyOfOrigLeftColStats->scaleHistogram(localPredReduction);
  ...
  if (leftExprType == ITM_NOTCOVERED) {
    lhs = lhs->child(0);
    if (predExprType == ITM_EQUAL) notCovered = TRUE;
  }
  ...
  if (CmpCommon::getDefault(COMP_BOOL_48) == DF_ON) {
    if ((rightExprType == ITM_NATYPE) AND(leftExprType == ITM_VEG_REFERENCE)) notCovered = TRUE;
  }

  if ((lhsMatchingExprHistogram != -1) OR(rhsMatchingExprHistogram != -1))
    // skip the following test when there are matching expression histograms
    notCovered = TRUE;

  if (!notCovered) {
    if ((rightExprType == ITM_ROWSETARRAY_SCAN) OR(leftExprType == ITM_ROWSETARRAY_SCAN)) {
    } else {
      if (NOT((leftExprType == rightExprType)
                  AND(leftExprType == ITM_INSTANTIATE_NULL OR leftExprType == ITM_VALUEIDUNION OR leftExprType ==
                      ITM_VEG_REFERENCE OR leftExprType == ITM_UNPACKCOL))) {
        return FALSE;
      }
    }
  }
  ...
  if (leftStatsToMerge.entries() == 0) return FALSE;

  identifyMergeCandidates(rhs, rightStatsToMerge);
  if (rightStatsToMerge.entries() == 0) return FALSE;
  ...
  if (exprOpCode != ITM_OR) {
    mergeSpecifiedStatDescs(leftStatsToMerge, leftRootIndex, mergeMethod, numOuterColStats, saveRowcount, saveUec,
                            FALSE,
                            exprOpCode);  // not for VEGPred

    // repeat for the right equivalence group.
    mergeSpecifiedStatDescs(rightStatsToMerge, rightRootIndex, mergeMethod, numOuterColStats, saveRowcount, saveUec,
                            FALSE,
                            exprOpCode);  // not for VEGPred
  }
  ...
  if (leftRootIndex < rightRootIndex) {
    ColStatDescSharedPtr tmpDesc(new (HISTHEAP) ColStatDesc(*rightRootDesc), HISTHEAP);
    tmpDesc->setFromInnerTable(rightRootIndex >= numOuterColStats ? TRUE : FALSE);

    if (NOT(leftRootIndex < numOuterColStats && rightRootIndex >= numOuterColStats)) {
      // even though the mergeMethod may never be used, but we are
      // initializing this for cases when COMP_BOOL_74 is OFF
      localMergeMethod = INNER_JOIN_MERGE;
    ...
    if (!joinFromSameTable) {
      leftRootDesc->mergeColStatDesc(tmpDesc, localMergeMethod,
                                     FALSE,  // don't force merge
                                     exprOpCode);
    }
    ...
    ColStatDescSharedPtr tmpDesc(new (HISTHEAP) ColStatDesc(*leftRootDesc), HISTHEAP);
    tmpDesc->setFromInnerTable(leftRootIndex >= numOuterColStats ? TRUE : FALSE);
    if (NOT(rightRootIndex < numOuterColStats && leftRootIndex >= numOuterColStats)) {
      // even though the mergeMethod may never be used, but we are
      // initializing this for cases when COMP_BOOL_74 is OFF
      localMergeMethod = INNER_JOIN_MERGE;
    ...
    if (!joinFromSameTable) {
      rightRootDesc->mergeColStatDesc(tmpDesc, localMergeMethod,
                                      FALSE,  // don't force merge
                                      exprOpCode);
    }
  ...
  if (leftReduction != csOne) rootColStats->scaleHistogram(leftReduction);

  if (rightReduction != csOne) rootColStats->scaleHistogram(rightReduction);
  ...
  while (rightCount > 0 || leftCount > 0) {
    ...
    if (nextToMerge != rootIndex) {
      if (mergeMethod == OUTER_JOIN_MERGE && nextToMerge >= numOuterColStats) {
        ColStatsSharedPtr updateColStats = (*this)[nextToMerge]->getColStatsToModify();
        updateColStats->overwrite(*rootColStats);
        ...
        // maintain the numOuterColStats value when removing
        // column statistics from the outer table's list.
        if (nextToMerge < numOuterColStats) numOuterColStats--;
      }
    }
  }  // while
...
void ColStatDescList::applyDefaultPred(ItemExpr *pred, CostScalar &globalPredicateReduction,
                                       OperatorTypeEnum exprOpCode, CostScalar *maxSelectivity) {
  // -------------------------------------------------------------------
  ...
  if (op == ITM_CONSTANT) {
    ...
    if (constPredStr == "0.") isAFalsePred = TRUE;
  }
  ...
  if (CmpCommon::getDefault(COMP_BOOL_107) == DF_ON) {
    if (pred->synthSupportedOp())
      alreadyApplied = pred->applyDefaultPred(*this, exprOpCode, origPredValueId, globalPredicate, maxSelectivity);
    else {
      // no need to pass maxSelectivity here because
      // maxSel(unsupported default predicate) == 1.0
      alreadyApplied = pred->applyUnSuppDefaultPred(*this, origPredValueId, globalPredicate);
    }
  } else {
    //
    ...
    if (pred->synthSupportedOp())  // does the operator support synthesis??
    {
      ...
      if (op == ITM_VEG_PREDICATE) {
        ...
        statsExist = identifyMergeCandidates(pred, leftColIndex, statsToMerge);
      } else {
        if ((op == ITM_EQUAL) && (CmpCommon::getDefault(COMP_BOOL_40) == DF_ON)) {
          ...
          if (((rightLeafValues.entries() + leftLeafValues.entries()) > 1) && (rightLeafValues != leftLeafValues)) {
            ...
            if ((statsExist) && ((exprOpCode != REL_SCAN) || (CmpCommon::getDefault(COMP_BOOL_74) == DF_OFF)))
            ...
            if (getColStatDescIndexForColumn(leftColIndex, column)) statsExist = TRUE;
          }
      ...
      if (statsExist) {
            ...
            } else  // veg is an "X=?" predicate
            {
              ...
              for (ValueId v = values.init(); values.next(v); values.advance(v)) {
                if (v.getItemExpr()->getOperatorType() == ITM_BASECOLUMN) {
                  ...
                  if (maxFreq > csMinusOne) break;
                }
              }
              // maxSelectivity("X=?") == max frequency(X) / total rows
              if (maxFreq > csMinusOne && rootColStats->getRowcount() > csZero)
                *maxSelectivity = MINOF(maxFreq / rootColStats->getRowcount(), *maxSelectivity);
            }  // veg is an "X=?" predicate
          }    // maxSelectivity != NULL

          rootStatDesc->synchronizeStats(oldRows, newRows, ColStatDesc::SET_UEC_TO_ONE);
          alreadyApplied = TRUE;
        }  // 1a: veg pred
        ...
        // CASE 1b : equal, not equal
          ...
          if (equiJoinWithExpr) {
            ...
            if (leftLeafValues.entries() == 1) {
              ...
              if (NOT(leftStatDesc->isPredicateApplied(origPredValueId))) {  // first time for this histogram
                ...
                if ((rightLeafValues.isEmpty()) ||
                  ...
                  if (leftStatDesc->getColStats()->isFakeHistogram()) {
                    defaultSel = (leftStatDesc->isSimilarPredicateApplied(op) ? csOne : pred->defaultSel());
                  } else {  // not a 'fake histogram'
                    ...
                    while (iter.isValid() && !iter.isNull()) {
                      ...
                      tmpRowCount = iRows / uec;

                      if (tmpRowCount > maxRowCount) maxRowCount = tmpRowCount;

                      if (tmpRowCount < minRowCount) minRowCount = tmpRowCount;
                    ...
                    if (op == ITM_EQUAL) {
                      ...
                      if (maxSelectivity) {
                        ...
                        *maxSelectivity = MINOF(defaultSel, *maxSelectivity);
                      }  // maxSelectivity != NULL
                    } else {
                ...
                else   // i.e., NOT leafValues.isEmpty()
                {
                  ...
                  if (op == ITM_EQUAL) {
                    if (leftStatDesc->isSimilarPredicateApplied(op))
                      ...
                      if ((rightLeafValues.entries() == 1) && (exprOpCode != REL_SCAN)) {
                        // <col1> = <col2>
                        ...
                        if (getColStatDescIndexForColumn(rightColIndex, rhs->getValueId())) {
                          ...
                          CostScalar maxUec =
                              (MAXOF(leftColStats->getTotalUec(), rightColStats->getTotalUec())).minCsOne();
                  ...
                  } else {  // op == ITM_NOT_EQUAL. Apply default selectivity
                    if (leftStatDesc->isSimilarPredicateApplied(op))
                      defaultSel = csOne;
                    else {
                      if (exprOpCode == REL_SCAN)
                ...
                leftStatDesc->applySel(defaultSel);
              } else {  // predicate has already been applied
                alreadyApplied = TRUE;
              }
        ...
        // CASE 1c : less, less_eq, greater, greater_eq
        else if (op == ITM_LESS OR op == ITM_LESS_EQ OR op == ITM_GREATER OR op == ITM_GREATER_EQ) {
          ItemExpr *rhs = pred->child(1);
          rhs = rhs->getLeafValueIfUseStats();
          ...
          if (NOT(statDesc->isPredicateApplied(origPredValueId))) {  // first time for this histogram
            ...
            if ((rightLeafValues.isEmpty()) ||
              ...
              if (defaultSel == csOne) {
                if (statDesc->derivOfLikeAndSimilarPredApp(pred) || statDesc->isSimilarPredicateApplied(op))
                  defaultSel = csOne;
                else {
                  BiRelat *br = (BiRelat *)pred;
              ...
              defaultSel = (statDesc->isSimilarPredicateApplied(op)
                                ? csOne
                                : CostPrimitives::getBasicCostFactor(HIST_DEFAULT_SEL_FOR_JOIN_RANGE));
            }  // end of join range
        ...
        else if (op == ITM_IS_NULL OR op == ITM_IS_NOT_NULL OR op == ITM_IS_UNKNOWN OR op == ITM_IS_NOT_UNKNOWN) {
          globalPredicate = FALSE;  // not a 'global' predicate
          ColStatDescSharedPtr statDesc = (*this)[leftColIndex];

          if (NOT(statDesc->isPredicateApplied(origPredValueId))) {  // first time for this histogram
            defaultSel = (statDesc->isSimilarPredicateApplied(op) ? csOne : pred->defaultSel());
        ...
        if (!leftLeafValues.isEmpty()) {
          ...
          if (getColStatDescIndexForColumn(leftColIndex, leftChild->getValueId())) {
            ...
            if (NOT(statDesc->isPredicateApplied(origPredValueId))) {
              ...
              if ((tempOp == ITM_LIKE) && ((Like *)tempPred)->isPatternAStringLiteral()) {
                ...
                if (colStat->isFakeHistogram())
                  ...
                  if (tempUec == csZero) tempUec = 1;

                  defaultSel = 1.0 / tempUec.value();
                  defaultSel.maxCsOne();
                }

                if (isOpTypeNot) defaultSel = 1 - defaultSel.value();
              } else
                defaultSel = pred->defaultSel();
  ...
  {
    if (isAFalsePred) defaultSel = csZero;
...
CostScalar ColStatDescList::getMaxOfMaxFreqOfCol(const ValueIdSet &baseColSet) {
  ...
  for (ValueId col = baseColSet.init(); baseColSet.next(col); baseColSet.advance(col)) {
    if (col == NULL_VALUE_ID) continue;
    ...
    if (colStat == NULL) {
      if (colAnalysis && (colAnalysis->getMaxFreq() != csMinusOne)) {
...
CostScalar ColStatDescList::getMinOfMaxFreqOfCol(const ValueIdSet &baseColSet) {
  ...
  if (baseColSet.entries() == 0) return csZero;
  ...
  for (ValueId col = baseColSet.init(); baseColSet.next(col); baseColSet.advance(col)) {
    ...
    if (colStat == NULL) {
      if (colAnalysis && (colAnalysis->getMaxFreq() > csZero)) {
        freq = colAnalysis->getMaxFreq();
...
CostScalar ColStatDescList::getMaxFreqForCaseExpr(const ValueIdSet &leafValues) {
...
CostScalar ColStatDescList::getMinUec(const ValueIdSet &baseColSet) const {
  ...
  if (baseColSet.entries() == 0) return csMinusOne;
  ...
  for (ValueId col = baseColSet.init(); baseColSet.next(col); baseColSet.advance(col)) {
    ...
    if (colStat == NULL) {
      ColAnalysis *colAnalysis = col.colAnalysis();
      if (colAnalysis && (colAnalysis->getFinalUec() != csZero)) {
        uec = colAnalysis->getFinalUec();
        if (uec < minUec) minUec = uec;
      } else {
        // if there is no way we can get the UEC for partitioning
        // column, set it equal to row count, and don't bother to
        // look at other columns
...
CostScalar ColStatDescList::getAggregateUec(const ValueIdSet &columns) const {
  ...
  if (getUecList() != NULL) {
    ...
    if (retval.isGreaterThanZero() /*> csZero*/) {
      ...
      for (ValueId column = columns.init(); columns.next(column); columns.advance(column)) {
        ...
        if (found == FALSE || origUec.isLessThanZero() /* <= csZero*/) continue;  // we continue looping
...
NABoolean ColStatDescList::getColStatDescIndexForColWithMaxUec(CollIndex &indexWithMaxUec,    /* out */
  ...
  for (ValueId id = inputColumns.init(); inputColumns.next(id); inputColumns.advance(id)) {
    CollIndex index;
    if (!(getColStatDescIndexForColumn(index, id))) continue;
    ...
    if (colStats->isOrigFakeHist() && atleastOneRealHist) continue;
...
// -------------------------------------------------------------------
void ColStatDescList::addToAppliedPredsOfAllCSDs(const ValueIdSet &inputColumns, const ValueId &newPredicate) {
  for (ValueId id = inputColumns.init(); inputColumns.next(id); inputColumns.advance(id)) {
    CollIndex index;
    if (!(getColStatDescIndexForColumn(index, id))) continue;
...
void ColStatDescList::useMultiUecIfCorrelatedPreds(
  ...
  for (i = startIndex; i < stopIndex; i++) {
    ...
    if (NOT joinHistograms.contains(i) AND(adjHistRC < adjRCBeforePreds)) {
      // Skip any histograms created for constants.

      if ((NOT largeTableNeedsStats)AND(thisColStats->isUpStatsNeeded()) AND !(thisColStats->isVirtualColForHist()))
  ...
  if ((predList.entries() + biLogicPredReductions.entries()) < 2) return;
  ...
  for (i = 0; i < predList.entries(); i++) {
    ...
    if (rowRed < highestReductionFromPreds) highestReductionFromPreds = rowRed;
  ...
  for (biLogicPredIter.getNext(biLogicPredColumn, reduction); biLogicPredColumn != NULL && reduction != NULL;
    ...
    if (*(reduction) < highestReductionFromPreds) highestReductionFromPreds = *(reduction);
  ...
  if (newRowcount.isExactlyZero()) return;

  CostScalar reductionAdjustment;

  NABoolean sufficientInformation = uecList()->useMCUecForCorrPreds(
      predReductions,              /* in/mod  */
      predCount,                   /* in : #param */
      updatedOldRowcount,          /* in */
      newRowcount,                 /* in */
      largeTableNeedsStats, *this, /* in. Pass list of histograms to be used for displayMissingStatsWarnings */
      reductionAdjustment);        /* out */

  // if we don't have sufficient MC-info, or there aren't any
  // "highly-correlated" columns, then never mind

  if (NOT sufficientInformation) return;
...
// -----------------------------------------------------------------------
void ColStatDescList::addRecentlyJoinedCols(CollIndex startIndex, CollIndex stopIndex) {
  for (CollIndex i = startIndex; i < stopIndex; i++) {
    ColStatsSharedPtr thisColStats = (*this)[i]->getColStats();

    // skip any histograms created for constants
    if (!(thisColStats->isVirtualColForHist()) && thisColStats->isRecentJoin()) {
...
void ColStatDescList::useMultiUecIfMultipleJoins(CostScalar &newRowcount,       /* in/out */
                                                 const CostScalar &oldRowcount, /* in */
  ...
  if (joinHistograms.entries() < 2) return;
  ...
  for (i = 0; i < joinHistograms.entries(); i++) {
    ValueIdList tmp = (*this)[joinHistograms[i]]->getMergeState();  // set->list
  ...
  NABoolean sufficientInformation = uecList()->getUecForMCJoin(joinValueIdPairsRemaining,                   /* in/out */
                                                               largeTableNeedsStats,                        /* in     */
                                                               expr, prodInitUec,                           /* out    */
                                                               multiColUec,                                 /* out */
                                                               baseRCForMCUEC, leftMCUec, checkForLowBound, /* out */
                                                               joinOnUnique, *this, redFromSC);

  // minimum cardinality from join should be the minimum cardinality of group
  // for empty input logical properties.
  if (checkForLowBound) setCapForLowBound();

  if (NOT sufficientInformation) return;

  // error! value not set, or simply avoiding div-by-zero! ignore MC-info in this case
  if (multiColUec.isLessThanZero()) return;
  ...
  for (i = 0; i < joinValueIdPairsRemaining.entries(); i++) {
    const ValueIdSet ithPair = joinValueIdPairsRemaining[i];  // convenience: list->set

    for (j = 0; j < joinHistograms.entries(); j++) {
      const ValueIdSet &mergeStateSet = (*this)[joinHistograms[j]]->getMergeState();
  ...
  for (i = 0; i < joinHistograms.entries(); i++) {
    ...
    if (singleColJoinRC < minSingleColJoinRC) minSingleColJoinRC = singleColJoinRC;
  }

  NABoolean aSemiJoin = (expr && (expr->isSemiJoin() || expr->isAntiSemiJoin())) ? TRUE : FALSE;
  if (!expr && mergeMethod == SEMI_JOIN_MERGE) aSemiJoin = TRUE;
  ...
  if (joinOnUnique && !aSemiJoin) {
    ...
    if (CmpCommon::getDefault(COMP_BOOL_145) == DF_OFF)
      ...
      if (baseRCForMCUEC < multiColUec) multiColUec = baseRCForMCUEC;
  ...
  if (!aSemiJoin) lowBoundFromMCUec = MINOF(lowBoundFromMCUec, oldRowcount / multiColUec);
...
void ColStatDescList::setInputCard(CostScalar rows) {
  for (CollIndex i = 0; i < entries(); i++) (*this)[i]->setInputCard(rows);
}
// ----------------------------------------------------------------------
//  ColStatDescList::synchronizeStats
//  A CSDL utility routine used to call ColStatDesc::synchronizeStats()
// ----------------------------------------------------------------------
void ColStatDescList::synchronizeStats(const CostScalar &baseRowcount, const CostScalar &newRowcount,
                                       CollIndex loopLimit) {
  for (CollIndex i = 0; i < loopLimit; i++) {
    const CostScalar &oldCount = (*this)[i]->getColStats()->getRowcount();

    if (oldCount != newRowcount) (*this)[i]->synchronizeStats(baseRowcount, newRowcount);
  }
}

void ColStatDescList::synchronizeStats(const CostScalar &newRowcount, CollIndex loopLimit) {
  for (CollIndex i = 0; i < loopLimit; i++) {
    const CostScalar &oldCount = (*this)[i]->getColStats()->getRowcount();
    (*this)[i]->synchronizeStats(oldCount, newRowcount);
  }
...
// ---------------------------------------------------------------------
CostScalar ColStatDescList::mergeListPairwise() {
  CollIndex i = 0;
  ...
  while (i < this->entries()) {
    rootStatDesc = (*this)[i];
    rootColStats = rootStatDesc->getColStatsToModify();
    if (NOT rootColStats->isShapeChanged() && NOT(*this)[i + 1]->getColStats()->isShapeChanged()) {
      rootStatDesc->mergeColStatDesc((*this)[i + 1], AND_MERGE,
                                     TRUE  // force merge
      );
      // get the aggregate results following the latest merge
      newRowcount = rootColStats->getRowcount();
      ...
      CostScalar reduct = (saveRowcount < csOne ? csOne : newRowcount / saveRowcount);
      totalReduct *= reduct;

      // remove the i+1'th entry.
      removeAt(i + 1);
    } else if (rootColStats->isShapeChanged())
      removeAt(i + 1);
    else
      removeAt(i);
...
NABoolean ColStatDescList::divideHistogramAtPartitionBoundaries(const ValueIdList &listOfPartKeys,      /*in*/
                                                                const ValueIdList &listOfPartKeyOrders, /*in*/
                                                                const LIST(EncodedValueList *) &
                                                                    listOfPartBounds,                  /*in*/
                                                                ...
                                                                CollIndexList &outputFactors) const    /*out*/
{
  ...
  {
    for (i = 0; i < numBounds; i++) targetHist->insertAt(i, HistInt(listOfPartBounds[i]->at(partKeyIndex)));
  } else  // insert the intervals in the reverse of their current order
  {
    for (i = 0; i < numBounds; i++)
      targetHist->insertAt(i, HistInt(listOfPartBounds[numBounds - i - 1]->at(partKeyIndex)));
  }
  ...
  if ((*targetHist)[numBounds - 1].getBoundary().isNullValue()) {
    (*targetHist)[numBounds - 1].setBoundary(EncodedValue(_ENCODEDVALUE_CLOSE_TO_NULL_));
  }
  ...
  ColStatsSharedPtr sourceColStats = getColStatsPtrForColumn(keyCorrespondingToOutputRows);

  if (sourceColStats == NULL || sourceColStats->getHistogram()->entries() == 0)
  ...
  HistogramSharedPtr templateHist = sourceHist->createMergeTemplate(targetHist, FALSE);
  ...
  if (templateHist->entries() != templateEntries) return FALSE;  // something went very wrong!

  // now, "squeeze" the resulting histogram so that it only keeps the
  // interval boundaries from the targetHistogram
  NABoolean result = templateHist->condenseToPartitionBoundaries(targetHist);

  if (result != TRUE) return result;  // something went wrong!
...
void ColStatDescList::compressColStatsToSingleInt() {
  for (CollIndex i = 0; i < entries(); i++) {
    ColStatsSharedPtr colStat = (*this)[i]->getColStatsToModify();
    if (!colStat->isVirtualColForHist() && !colStat->isOrigFakeHist()) colStat->compressToSingleInt();
  }
}
...
// ----------------------------------------------------------------------
void ColStatDescList::mergeSpecifiedStatDescs(const CollIndexList &statsToMerge, CollIndex rootIndex,
                                              MergeType mergeMethod, CollIndex numOuterColStats,
                                              CostScalar &newRowcount, CostScalar &newUec, NABoolean forVEGPred,
  ...
  for (CollIndex i = 0; i < statsToMerge.entries(); i++) {
    if (statsToMerge[i] != rootIndex) {
      ...
      MergeType localMergeMethod = mergeMethod;
      NABoolean joinOnOneTable = FALSE;
      ColStatDescSharedPtr tmpDesc(new (HISTHEAP) ColStatDesc(*((*this)[statsToMerge[i]])), HISTHEAP);

      if (NOT(rootIndex < numOuterColStats && statsToMerge[i] >= numOuterColStats)) {
        // even though the mergeMethod may never be used, but we are
        // initializing this for cases when COMP_BOOL_74 is OFF
        localMergeMethod = INNER_JOIN_MERGE;
      ...
      if (!joinOnOneTable) {
        rootDesc->mergeColStatDesc(tmpDesc, localMergeMethod,
                                   FALSE,  // don't force merge
                                   opType);
      }
      ...
      if (NOT forVEGPred) {
        tmpDesc->setFromInnerTable(statsToMerge[i] >= numOuterColStats ? TRUE : FALSE);
...
//
// -----------------------------------------------------------------------
NABoolean ColStatDescList::getColStatDescIndexForColumn(CollIndex &index,          /* out */
    ...
    case ITM_INDEXCOLUMN:
      ...
      // -------------------------------------------------
      {
        const BaseColumn *bcIEPtrForIndexColumn =
            (BaseColumn *)((IndexColumn *)inputColumnIEPtr)->getDefinition().getItemExpr();
  ...
  for (CollIndex i = 0; i < entries(); i++) {
    ...
    if ((msSet.contains(baseColumnForInputColumn)) || (currentColStatDesc->VEGColumn() == vegColumnForInputColumn) ||
        (currentColStatDesc->getColumn() == baseColumnForInputColumn)) {  // found!
      index = i;
      foundInBT = TRUE;
      break;
    }
...
NABoolean ColStatDescList::getColStatDescIndexForColumn(CollIndex &index,              /* out */
  ...
  if (partKeyColArray.entries() <= 1) return (getColStatDescIndexForColumn(index, inputColumn));
...
NABoolean ColStatDescList::getColStatDescIndex(CollIndex &index,           /* out */
...
ColStatsSharedPtr ColStatDescList::getColStatsPtrForColumn(const ValueId &inputColumn) const {
  ColStatDescSharedPtr colStatsDescPtr = getColStatsDescPtrForColumn(inputColumn);

  if (colStatsDescPtr)
    return colStatsDescPtr->getColStats();
  else
    return NULL;
}

ColStatDescSharedPtr ColStatDescList::getColStatsDescPtrForColumn(const ValueId &inputColumn) const {
  CollIndex index = 0;
...
// -----------------------------------------------------------------------
ColStatsSharedPtr ColStatDescList::getColStatsPtrForPredicate(const ValueId &predicate) const {
  ...
  switch (arity) {
    ...
    case 2: {
      ...
      if (leftExpr->isAPredicate()) {
        ...
        ValueIdSet vidSet;
        vidSet.insert(((VEGPredicate *)leftExpr)->getVEG()->getAllValues());
      ...
      if (colStatsPtr == NULL) {
        ...
        if (rightExpr->isAPredicate()) {
          ...
          ValueIdSet vidSet;
          vidSet.insert(((VEGPredicate *)rightExpr)->getVEG()->getAllValues());
          colStatsPtr = getColStatsPtrForVEGGroup(vidSet);
        } else {
          // It is not a predicate NOR a reference, then it
          // must be an expression, get the columns it refers
    ...
    case 0: {
      if (predIE->getOperatorType() == ITM_VEG_PREDICATE) {
        const ValueIdSet &vegGroup = ((VEGPredicate *)predIE)->getVEG()->getAllValues();
...
ColStatsSharedPtr ColStatDescList::getColStatsPtrForVEGGroup(const ValueIdSet &VEGGroup) const {
      ...
      case ITM_INSTANTIATE_NULL: {
        ...
        if (NOT inst->NoCheckforLeftToInnerJoin) {
          ...
          if (childVidIePtr->getOperatorType() == ITM_VEG_REFERENCE) {
            ...
            if (colStatsPtr != NULL) return colStatsPtr;
            break;
          } else if (childVidIePtr->getOperatorType() == ITM_BASECOLUMN OR childVidIePtr->getOperatorType() ==
                     ITM_INDEXCOLUMN) {
  ...
  if ((colStatsPtr == NULL) && (leafValuesForExpr.entries() > 0)) {
    CollIndex idx;
    if (getColStatDescIndexForColWithMaxUec(idx, leafValuesForExpr)) colStatsPtr = (*this)[idx]->getColStats();
  }
...
CostScalar ColStatDescList::getUecOfJoiningCols(ValueIdSet &joinedColSet) const {
  CostScalar minUec = COSTSCALAR_MAX;
  CostScalar currUec = csOne;

  for (ValueId vid = joinedColSet.init(); joinedColSet.next(vid); joinedColSet.advance(vid)) {
    ColStatsSharedPtr colStats = this->getColStatsPtrForColumn(vid);
    if (colStats) {
      currUec = colStats->getTotalUec();
      if (currUec < minUec) minUec = currUec;
    }  // if ColStats
...
void ColStatDescList::print(ValueIdList selectListCols, FILE *ofd, const char *prefix, const char *suffix, CollHeap *c,
                            char *buf, NABoolean hideDetail) const {
  ...
  for (CollIndex colStatDescIndex = 0; colStatDescIndex < entries(); colStatDescIndex++) {
...
void ColStatDescList::display() const {
  ValueIdList emptySelectList;
  print(emptySelectList);
}

void ColStatDescList::verifyInternalConsistency(CollIndex start, CollIndex end) const {
  CCMPASSERT(start <= end);  // misuse of function!
  if (start >= entries()) return;
  if (end > entries()) return;

  const CostScalar &matchRowcount = (*this)[start]->getColStats()->getRowcount();
  CollIndex i = start + 1;
  for (; i < end; i++) {
  ...
  for (i = start; i < end; i++) {
    if ((*this)[i]->getColStats()->getHistogram()->entries() == 0) {
      ...
      stats->setToSingleInterval(stats->getMinValue(), stats->getMaxValue(), stats->getRowcount(),
                                 stats->getTotalUec());
...
void ColStatDescList::enforceInternalConsistency(CollIndex start, CollIndex end, NABoolean printNoStatsWarning) {
  CCMPASSERT(start <= end);  // misuse of function!
  if (start >= entries()) return;
  if (end > entries()) return;
  ...
  for (; i < end; i++) {
    ...
    if (cs->getHistogram()->entries() == 0) {
      ...
      cs->setToSingleInterval(cs->getMinValue(), cs->getMaxValue(), cs->getRowcount(), cs->getTotalUec());
    ...
    if (cs->getStatColumns().entries()) {
      if (printNoStatsWarning && cs->isUpStatsNeeded()) {
        if ((cs->isFakeHistogram()) ||
            (cs->isSmallSampleHistogram()))  // if fake or small sample histogram, fire off the warning!
        {
          ...
          if (colExpr != NULL) {
            ...
            if (cs->isSmallSampleHistogram()) quickStats = TRUE;
          }
        }
      }
    }
...
// -----------------------------------------------------------------------
MultiColumnUecList::MultiColumnUecList(const StatsList &initStats, const ValueIdList &tableColumns)
    : HASHDICTIONARY(ValueIdSet, CostScalar)(&(MultiColumnUecList::HashFunction),
                                             17,    // original hash size ... why not?
                                             TRUE,  // uniqueness constraint
                                             HISTHEAP) {
  ...
  for (CollIndex i = 0; i < initStats.groupUecColumns_.entries(); i++) {
    ...
    for (CollIndex j = 0; j < uecCols.entries(); j++) {
      ...
      if (singleColUec < 1) statsMissing = TRUE;
    ...
    if (multiColUec < minMCUec) multiColUec = minMCUec;

    if (multiColUec > maxMCUec) multiColUec = maxMCUec;
...
// -----------------------------------------------------------------------
void MultiColumnUecList::insertList(const MultiColumnUecList *other) {
  ...
  while (keyEntry != NULL && uecEntry != NULL) {
    if (NOT contains(keyEntry)) insertPair(*keyEntry, *uecEntry);
...
// direction.
// -----------------------------------------------------------------------
void MultiColumnUecList::insertMappedList(const MultiColumnUecList *other, const ValueIdMap &map) {
  if (other == NULL) return;
  if (other->entries() == 0) return;
  ...
  while (keyEntry != NULL && uecEntry != NULL) {
    ...
    if (NOT contains(mappedSet)) insertPair(*mappedSet, *uecEntry);
...
// -----------------------------------------------------------------------
NABoolean MultiColumnUecList::insertPair(const ValueIdSet &columns, const CostScalar &groupUec) {
  ValueIdSet *columnsCopy = new (HISTHEAP) ValueIdSet(columns);
  CostScalar *groupUecCopy = new (HISTHEAP) CostScalar(groupUec);
  if (columnsCopy == (NAHashDictionary<ValueIdSet, CostScalar>::insert(columnsCopy, groupUecCopy)))
...
// -----------------------------------------------------------------------
NABoolean MultiColumnUecList::updatePair(const ValueIdSet &columns, const CostScalar &groupUec) {
  ValueIdSet *columnsCopy = new (HISTHEAP) ValueIdSet(columns);
  if (columnsCopy == (NAHashDictionary<ValueIdSet, CostScalar>::remove(columnsCopy))) {
    if (insertPair(columns, groupUec)) return TRUE;
  }              // update successful
...
// value
// -----------------------------------------------------------------------
CostScalar MultiColumnUecList::lookup(const ValueIdSet &key) const {
  CostScalar groupUec = csMinusOne;
  if (contains(&key)) groupUec = *(getFirstValue(&key));
  return groupUec;
}
...
// ------------------------------------------------------------------------------
void MultiColumnUecList::initializeMCUecForUniqueIndxes(TableDesc &table, const CostScalar &tableRowcount) {
  ...
  for (CollIndex listi = 0; listi < indexList.entries(); listi++) {
    if (indexList[listi]->uniqueIndex()) {
      ...
      if ((multUecRowCount = lookup(insertCols)) > 0) {
        ...
        if (multUecRowCount != tableRowcount) updatePair(insertCols, tableRowcount);
      } else
...
LIST(ValueIdSet) * MultiColumnUecList::getListOfSubsetsContainsColumns(const ValueIdList &columns /*in*/,
  ...
  for (iter.getNext(keyEntry, uecEntry); keyEntry != NULL && uecEntry != NULL; iter.getNext(keyEntry, uecEntry)) {
...
// -----------------------------------------------------------------------
ValueIdSet MultiColumnUecList::largestSubset(const ValueIdSet &columns) const {
  ...
  if (entries() == 0 OR columns.entries() == 0) return largestSet;  // no subsets at all!
  ...
  for (iter.getNext(keyEntry, uecEntry); keyEntry != NULL && uecEntry != NULL; iter.getNext(keyEntry, uecEntry)) {
    ...
    if (keyEntry->entries() >= largestSet.entries()) {
      ...
      for (ValueId keyCol = keyEntry->init(); keyEntry->next(keyCol); keyEntry->advance(keyCol))
        SCproductUec *= lookup(keyCol).value();

      independenceFactor = uecEntry->value() / SCproductUec;

      if ((keyEntry->entries() > largestSet.entries()) OR((keyEntry->entries() == largestSet.entries()) AND(
              keyEntry->entries() > 0) AND(independenceFactor < smallestIPFactor))) {
        largestSet = *keyEntry;
...
//---------------------------------------------------------------------
NABoolean MultiColumnUecList::findDenom(const ValueIdSet &columns) const {
  if (entries() == 0 OR columns.entries() == 0) return FALSE;  // no subsets at all!
...
NABoolean MultiColumnUecList::useMCUecForCorrPreds(NAHashDictionary<ValueId, CostScalar> &predReductions, /* in/mod */
                                                   ...
                                                   NABoolean largeTableNeedsStats, const ColStatDescList &scHists,
                                                   CostScalar &reductionAdjustment) /* out */
{
#define USE_MULTI_COL_UEC_INFO

  if (numPredicates < 2) return FALSE;
  ...
  NAHashDictionary<TableDesc, ValueIdList> tableColumns(&TableDescHashFn, 7, TRUE, HISTHEAP);
  ...
  for (predIter.getNext(predColumn, reduction); predColumn != NULL && reduction != NULL;
  ...
  for (tableIter.getNext(iterDesc, iterList); iterDesc != NULL && iterList != NULL;
    ...
    if (mcUec.isGreaterThanZero()) {
      ...
      if (colsWithReductions.doColumnsConstituteUniqueIndex(iterDesc)) {
        ...
        NABoolean statsCreated =
            createMCStatsForColumnSet(colsWithReductions, cumulativeColSetWithMCUEC, maxMultiColUec, baseRowCount);
  ...
  if (largestSubsetAmongLargestSubsets.entries() >= 2) {
    ...
    for (ValueId i = largestSubsetAmongLargestSubsets.init(); largestSubsetAmongLargestSubsets.next(i);
  ...
  if ((oldRowCount * uecReduction) < (initialRowCount / multiColumnUec) AND(initialRowCount / multiColumnUec) >
      csOne AND(uecReduction.getValue() > 0.0)) {
  ...
  } else if ((oldRowCount * uecReduction).isLessThanOne() AND(oldRowCount * uecReduction) <
             (initialRowCount / multiColumnUec) AND newRowCount.isGreaterThanZero() /* != csZero */) {
  ...
  count = MINOF(largestSubsetAmongLargestSubsets.entries(), numPredicates);
  return TRUE;
}

NABoolean MultiColumnUecList::createMCStatsForColumnSet(ValueIdSet colsWithReductions,
                                                        ValueIdSet &cumulativeColSetWithMCUEC,
  ...
  if (cumulativeColSetWithMCUEC.entries() <= 1) return FALSE;
  ...
  MultiColumnUecList *mcListForRemainingCols =
      createMCListForRemainingCols(colsWithReductions, cumulativeColSetWithMCUEC);
  ...
  statsCreatedWithOverlap = mcListForRemainingCols->createMCUECWithOverlappingColSets(
      colsWithReductions, cumulativeColSetWithMCUEC, maxMultiColUec, baseRowCount);
  ...
  if (statsCreatedWithOverlap) {
    ...
    if ((maxMultiColUec == baseRowCount) || (colsWithReductions.entries() <= 1)) return TRUE;
  }
  ...
  if ((maxMultiColUec < baseRowCount) AND(colsWithReductions.entries() > 1))
  ...
  // If any stats were created from partial multi-col UEC, save them in the
  // multi-col UEC list for future use.
  if (statsCreatedWithDisjoint) this->insertPair(cumulativeColSetWithMCUEC, maxMultiColUec);
...
struct MCJoinPairStruct {
 ...
 public:
  MCJoinPairStruct()
      : remainingJoinPairs_(new (CmpCommon::statementHeap()) NAList<ValueIdSet>(CmpCommon::statementHeap())) {}
...
ValueIdSet MultiColumnUecList::findMatchingColumns(const ValueIdSet &t1Cols,            /* in  */
  ...
  for (CollIndex i = 0; i < joinPairs.entries(); i++) {
    ...
    for (ValueId id = t1Cols.init(); t1Cols.next(id); t1Cols.advance(id)) {
    ...
    if (NOT inserted) remainingPairs.insert(joinPairs[i]);
  }
...
Int32 tieBreaker(CostScalar ipFactor1, CostScalar mcUec1, TableDesc *tabDesc1, CostScalar ipFactor2, CostScalar mcUec2,
  ...
  if (mcUec1 < mcUec2) return 1;

  if (mcUec1 > mcUec2) return -1;

  if (tabDesc1 == tabDesc2) return 0;

  if ((CollIndex)((tabDesc1->getColumnList())[0]) > (CollIndex)((tabDesc2->getColumnList())[0]))
    return 1;
  else
    return -1;
}
...
// -----------------------------------------------------------------------
NABoolean MultiColumnUecList::getUecForMCJoin(LIST(ValueIdList) & joinValueIdPairs,      /* in/out */
                                              const NABoolean largeTableNeedsStats,      /* in  */
                                              const Join *expr, CostScalar &prodInitUec, /* out */
                                              ...
                                              const ColStatDescList &colStats, CostScalar redFromSC) {
  if (joinValueIdPairs.entries() < 2) return FALSE;
  ...
  CollIndex mostRefs = 0, secondMost = 0;
  ...
  for (tableIter.getNext(tableDesc, colList); tableDesc != NULL && colList != NULL;
    ...
    if (!colsUnique && mcStatsPresent) {
      for (ValueId keyCol = colSet.init(); colSet.next(keyCol); colSet.advance(keyCol))
        SCproductUec *= lookup(keyCol).value();
    ...
    // that we are done.
    if (CmpCommon::getDefault(COMP_BOOL_41) == DF_ON) mcStatsPresent = FALSE;

    if ((colSet.entries() > mostRefs) ||
        ((mcStatsPresent && colSet.entries() == mostRefs) &&
         (tableOneDesc == NULL ||
          tieBreaker(ipFactorCurrent, mcUecCurrent, tableDesc, ipFactorMostRefd, mcUecMostRefd, tableOneDesc) == 1))) {
      secondMost = mostRefs;
      ...
      ipFactorMostRefd = ipFactorCurrent;
               ...
               ((mcStatsPresent && colSet.entries() == secondMost) &&
                (tableTwoDesc == NULL || tieBreaker(ipFactorCurrent, mcUecCurrent, tableDesc, ipFactorSecondMost,
                                                    mcUecSecondMost, tableTwoDesc) == 1))) {
  ...
  NAList<ValueIdList> joinPairsUsed(CmpCommon::statementHeap()), joinPairsNotUsed(CmpCommon::statementHeap());
  ...
  for (i = 0; i < joinPairsUsed.entries(); i++) {
    NABoolean tableOneRefd = FALSE, tableTwoRefd = FALSE;
  ...
  if (joinPairsUsed.entries() < 2) return FALSE;
  ...
  if (tableOneSet.entries() < 2 OR tableTwoSet.entries() < 2) return FALSE;
  ...
  if (CURRSTMT_OPTDEFAULTS->histSkipMCUecForNonKeyCols()) {
    ...
    if (!(tableOneSetUnique || tableTwoSetUnique)) return FALSE;
  }
  ...
  if ((CmpCommon::getDefault(HIST_MC_STATS_NEEDED) == DF_ON) AND largeTableNeedsStats) {
  ...
  if ((CmpCommon::getDefault(COMP_BOOL_149) == DF_ON) && isNotASemiJoin && (tableOneSetUnique || tableTwoSetUnique)) {
    rowcountOfNonUniqSide =
        getRowcountOfNonUniqueColSet(expr, tableOneList, tableTwoList, tableOneSetUnique, tableTwoSetUnique);
  ...
  if (tableOnePossibles.entries() == 0 OR tableTwoPossibles.entries() == 0) {
  ...
  for (i = 0; i < tableOnePossibles.entries(); i++) {
    ...
    ValueIdSet needInTableTwo = this->findMatchingColumns(tableOnePossibles[i], joinPairsUsed, remainingPairs,
                                                          maxInitUecProduct, minInitUecProduct, checkForLowBound);
    ...
    if (checkForLowBound) return FALSE;

    // if the table 2 MC-info doesn't exist, continue
    if (NOT tableTwoPossibles.contains(needInTableTwo)) continue;
    ...
    if (expr && !isNotASemiJoin) {
      ...
      ColStatDescList leftColStatsList =
          ((Join *)expr)->child(0).outputLogProp((*GLOBAL_EMPTY_INPUT_LOGPROP))->colStats();
    ...
    if (CmpCommon::getDefault(COMP_BOOL_141) == DF_OFF) {
      ...
      if ((mcUEC1 > mcUEC2) || ((mcUEC1 == mcUEC2) && (baseRowCountForTableOne > baseRowCountForTableTwo))) {
        if (isNotASemiJoin) {
          entry->multiColUec_ = mcUEC1;
    ...
    for (j = 0; j < remainingPairs.entries(); j++) entry->remainingJoinPairs_->insert(remainingPairs[j]);
    // -----------------------------------------------------------
  ...
  for (i = 1; i < corrList.entries(); i++) {
    if (corrList[i]->remainingJoinPairs_->entries() < corrList[bestIdx]->remainingJoinPairs_->entries()) bestIdx = i;
  }
  ...
  // ----------------------------------------------------------
  LIST(ValueIdList) oldJoinValueIdPairs(joinValueIdPairs, CmpCommon::statementHeap());
  joinValueIdPairs.clear();
  ...
  for (i = 0; i < joinValueIdPairSet.entries(); i++) {
  ...
  if (oldJoinValueIdPairs == joinValueIdPairs) {
    ...
    CostScalar moreMultiColUec = csMinusOne, moreInitUec = csMinusOne, moreLeftMCUec = csOne;
    NABoolean insuffMCInfo = FALSE;
    CostScalar newBaseRowCount = csOne;

    if (this->getUecForMCJoin(joinValueIdPairs, largeTableNeedsStats, expr, moreInitUec, moreMultiColUec,
                              newBaseRowCount, moreLeftMCUec, insuffMCInfo, joinOnUnique, colStats, csMinusOne)) {
...
// -----------------------------------------------------------------------
NABoolean MultiColumnUecList::containsMCinfo() const {
  ValueIdSet *keyEntry = NULL;
  CostScalar *uecEntry = NULL;
  MultiColumnUecListIterator iter(*this);

  for (iter.getNext(keyEntry, uecEntry); keyEntry != NULL && uecEntry != NULL; iter.getNext(keyEntry, uecEntry)) {
    if (keyEntry->entries() > 1) return TRUE;
  }
  return FALSE;
}
...
NABoolean MultiColumnUecList::createMCUECWithOverlappingColSets(ValueIdSet &remainingCols,             /* in / out */
  ...
  while (noOfColsInterested > 1) {
    ...
    for (iter.getNext(keyEntry, uecEntry), i = 1; keyEntry != NULL && uecEntry != NULL;
      ...
      ValueIdSet cols;
      cols = *keyEntry;
      if (keyEntry->entries() != noOfColsInterested) continue;
...
NABoolean MultiColumnUecList::createMCUECWithDisjointColSets(ValueIdSet &remainingCols,             /* in / out */
                                                             ValueIdSet &cumulativeColSetWithMCUEC, /* in / out */
                                                             CostScalar &mcUec /* in / out*/, CostScalar oldRowCount) {
  NABoolean statsCreated = FALSE;
...
MultiColumnUecList *MultiColumnUecList::createMCListForRemainingCols(ValueIdSet remainingCols,
                                                                     ValueIdSet cumulativeColSetWithMCUEC) {
  ...
  for (iter.getNext(keyEntry, uecEntry), i = 1; keyEntry != NULL && uecEntry != NULL;
    ...
    if (keyEntry->entries() > (noOfColsInterested + 1)) continue;
...
void MultiColumnUecList::print(FILE *ofd, const char *prefix, const char *suffix) const
...
void MultiColumnUecList::display() const { print(); }
...
// -----------------------------------------------------------------
NABoolean MultiColumnUecList::isMCStatsUseful(ValueIdSet columnSet, TableDesc *tableDesc) const {
  // This is used for only multi column stats, hence if columnSet
  // consists of single column, return FALSE
  if (columnSet.entries() == 1) return FALSE;

  // check to see if columnSet is unique, if it is, return FALSE.
  if (columnSet.doColumnsConstituteUniqueIndex(tableDesc)) return FALSE;
  ...
  if (colSetWithMCStats.entries() == 0) return TRUE;
  ...
  if (colSetWithMCStats.entries() <= columnSet.entries()) {
    ...
    if (mcUec >= uecCushion * tableRowCount) return FALSE;
  }
  return TRUE;
}  // isMCStatsUseful
...

{
  ...
  if ((numOfParts == 1) || (partFunc->isASkewedDataPartitioningFunction()) ||
...
CostScalar MultiColumnUecList::getRowcountOfNonUniqueColSet(const Join *expr, ValueIdList colList1,
                                                            ValueIdList colList2, NABoolean list1Unique,
                                                            NABoolean list2Unique) {
  CostScalar rowcountOfNonUniqSide = csMinusOne;

  // We are using unique characteristic for Joins only, atleast till R2.5 (5/29/09)
  if (!expr || !expr->isAnyJoin()) return rowcountOfNonUniqSide;
  ...
  if ((MCUec1 <= 0) || (MCUec2 <= 0)) return rowcountOfNonUniqSide;
...
void ColStatDescList::computeRowRedFactor(MergeType mergeMethod,           // input
                                          ...
                                          CostScalar &rowRedProduct)       // output
{
  ...
  for (i = 0; i < loopLimit; i++) {
    ...
    // example:
    // if rowCnt = 1.04 and rowcountBeforePreds = 1 then tempRC = 2 and tempRCBeforePreds = 1 -> do not increment
    // predCountMC if rowCnt = 1 and rowcountBeforePreds = 1 then tempRC = 1 and tempRCBeforePreds = 1 -> do not
    // increment predCountMC if rowCnt = 1 and rowcountBeforePreds = 1.04 then tempRC = 1 and tempRCBeforePreds = 1 ->
    // do not increment predCountMC if rowCnt = 0.99 and rowcountBeforePreds = 1 then tempRC = 1 and tempRCBeforePreds =
    // 1 -> do not increment predCountMC if rowCnt = 1 and rowcountBeforePreds = 0.99 then tempRC = 1 and
    // tempRCBeforePreds = 0 -> do not increment predCountMC if rowCnt = 1.04 and rowcountBeforePreds = 0.99 then tempRC
    // = 2 and tempRCBeforePreds = 0 -> do not increment predCountMC if rowCnt = 0.99 and rowcountBeforePreds = 1.04
    // then tempRC = 1 and tempRCBeforePreds = 1 -> do not increment predCountMC if rowCnt = 1 and rowcountBeforePreds =
    // 2 then tempRC = 1 and tempRCBeforePreds = 2 -> increment predCountMC if rowCnt = 1.04 and rowcountBeforePreds
    // = 1.9 then tempRC = 2 and tempRCBeforePreds = 1 -> do not increment predCountMC

    CostScalar tempRC = ceil(rowCnt.getValue());
    if (tempRC < tempRCBeforePreds) predCountMC++;
  }
    ...
    for (i = numOuterColStats; i < loopLimit; i++) {
      if (alreadyMergedHistograms.contains((*this)[i]->getColumn())) {
        ...
        if (rowcountBeforePreds.isGreaterThanZero() /* > csZero */) {
          rowred = (*this)[i]->getColStats()->getRowcount() / rowcountBeforePreds;
        }
...
void ColStatDescList::insertByPosition(const StatsList &other, const NAColumnArray &columnList,
  ...
  for (CollIndex i = 0; i < columnList.entries(); i++) {
    ...
    for (UInt32 i = 0; i < other.entries(); i++) {
      ...
      if ((otherColumns.entries() == 1) && (!otherColumns.getColumnByPos(position))) continue;

      // Actually, NATable::getStatistics filters out any multi-column
      // histograms before we get here. So we'll never see one here.
      // (Good thing too... if we go through the outer loop more than
      // once, any multi-column histogram would be added multiple times.)
      DCMPASSERT(otherColumns.entries() <= 1);

      ColStatDescSharedPtr tmpStatDesc(new (CmpCommon::statementHeap()) ColStatDesc(other[i], tableColList),
  ...
  // Add all the expression histograms at the end
  for (UInt32 j = 0; j < other.entries(); j++) {
    ColStatsSharedPtr otherStats(other[j]);
    const NAColumnArray &otherColumns = otherStats->getStatColumns();
    if ((otherColumns.entries() == 1) && (otherColumns[0]->getPosition() < 0)) {
      // TODO: it's really strange to me that the same tableColList is passed
      // in on all of these; why is that? and do we care here?
      ColStatDescSharedPtr tmpStatDesc(new (HISTHEAP) ColStatDesc(other[j], tableColList), HISTHEAP);
      // add base column references
      ValueIdSet *exprColRefs = interestingExpressions.getFirstValue(&otherColumns[0]->getColName());
      if (exprColRefs) {
        tmpStatDesc->setExprColRefs(*exprColRefs);
      }
      tmpStatDesc->generateExpressionHistogramMergeState();
      this->insertAt(this->entries(), tmpStatDesc);
      hasExpressionHistogram_ = TRUE;  // remember that this CDSL has expression histograms
    }
  }
}
...
CostScalar ColStatDescList::getHighestUecReductionByLocalPreds(ValueIdSet &cols) const {
...
// ---------------------------------------------------------------------------
NABoolean ColStatDescList::locateHistogramToNULLAugment(ValueIdSet EqLocalPreds /*in*/,
  ...
  for (ValueId id = EqLocalPreds.init(); EqLocalPreds.next(id); EqLocalPreds.advance(id)) {
    ItemExpr *pred = id.getItemExpr();

    // We are interested only in equi join predicates
    // skip any other predicates
    if (pred->getOperatorType() != ITM_VEG_PREDICATE) continue;
    ...
    for (CollIndex i = 0; i < (CollIndex)outerRefCount; i++) {
      ...
      if ((*this)[i]->getColStats()->isVirtualColForHist()) continue;
  ...
  if (!foundFlag) {
    ...
    for (CollIndex i = 0; i < (CollIndex)outerRefCount; i++) {
      ...
      if (colStats->isVirtualColForHist()) continue;
      ...
      if (colStats->isShapeChanged()) {
        ...
        if ((saveNumBuckets == 0)                                         // we should accept any value for
            || (colStats->getHistogram()->entries() > saveNumBuckets)) {  // rootStatIndex if we have none thus far!
          saveNumBuckets = colStats->getHistogram()->entries();
          rootStatIndex = i;
        }
      }
...
// -------------------------------------------------------------------
CostScalar ColStatDescList::computeLeftOuterJoinRC(NABoolean &foundFlag /*out*/,
  ...
  // left join RC
  if (NOT(statOper == ITM_VEG_REFERENCE || statOper == ITM_INSTANTIATE_NULL || statOper == ITM_VALUEIDUNION ||
          statOper == ITM_UNPACKCOL || statOper == ITM_ROWSETARRAY_SCAN || joinColStats->isVirtualColForHist() ||
          statOper == ITM_NATYPE)) {
  ...
  for (CollIndex i = 0; i < leftColStatsList.entries(); i++) {
    ...
    if (leftColStatsList[i]->getColStats()->isVirtualColForHist()) continue;
    ...
    if (originalOper == ITM_ROWSETARRAY_SCAN) continue;

    // sanity check for the original left histogram
    if (NOT(originalOper == ITM_VEG_REFERENCE || originalOper == ITM_INSTANTIATE_NULL ||
            originalOper == ITM_VALUEIDUNION || originalOper == ITM_UNPACKCOL || originalOper == ITM_NATYPE)) {
      CCMPASSERT("Incorrect expression participating in Join");
      // join result is not reliable any more
      joinColStats->setFakeHistogram(TRUE);
      // set the foundFlag to FALSE to indicate some error happened
  ...
  if (goodMatch) {
    ...
    joinStatDesc->mergeColStatDesc(origStatDesc, LEFT_JOIN_OR_MERGE, FALSE, REL_JOIN, FALSE /*dontMergeFVs*/);
...
// the joined histograms.
// -------------------------------------------------------------------
CostScalar ColStatDescList::computeFullOuterJoinRC(NABoolean &foundFlag /*out*/,
  ...
  for (CollIndex i = 0; i < origColStatsList.entries(); i++) {
    // Skip any histograms created for virtual columns
    if (origColStatsList[i]->getColStats()->isVirtualColForHist()) continue;
...
// --------------------------------------------------------------------------------
void ColStatDescList::synchronizeHistsWithOJRC(NAList<CollIndex> &statsToMerge /*in */, CollIndex startIndex /*in*/,
                                               CollIndex stopIndex /*in*/, CollIndex rootStatIndex /*in*/,
                                               const ColStatDescList &origColStatsList /*in*/,
                                               CostScalar &oJoinResultRows /*out*/, CostScalar &baseRows /*out*/) {
  // ---------------------------------------------------------------
  ...
  for (i = startIndex; i < stopIndex; i++) {
    ...
    for (j = 0; j < statsToMerge.entries() && !inPredicate; j++) {
      if (i == statsToMerge[j]) inPredicate = TRUE;
    }
    ...
    if (jColStats->isVirtualColForHist()) {
      ...
      if (oldCount != oJoinResultRows) jStatDesc->synchronizeStats(oldCount, oJoinResultRows);
      continue;
    }
    ...
    if (NOT(statOper == ITM_VEG_REFERENCE || statOper == ITM_INSTANTIATE_NULL || statOper == ITM_VALUEIDUNION ||
            statOper == ITM_UNPACKCOL || statOper == ITM_NATYPE)) {
    ...
    for (j = 0; j < origColStatsList.entries(); j++) {
      ...
      if (oColStats->isVirtualColForHist()) continue;
    ...
    if (goodMatch) {
      ...
      if (inPredicate) {
        jStatDesc->mergeColStatDesc(oStatDesc, LEFT_JOIN_OR_MERGE, FALSE, REL_JOIN);
        CostScalar oldCount = jColStats->getRowcount();
        if (oldCount != oJoinResultRows) jStatDesc->synchronizeStats(oldCount, oJoinResultRows);
      } else {
        ...
        jStatDesc->synchronizeStats(baseRows, oJoinResultRows, ColStatDesc::DO_NOT_REDUCE_UEC);
      }
    } else {
...
// ------------------------------------------------------------------------
void ColStatDescList::nullInstantiateHists(CollIndex startIndex, CollIndex stopIndex, CostScalar &oJoinResultRows,
  ...
  for (CollIndex i = startIndex; i < stopIndex; i++) {
    ...
    // Skip any histograms created for virtual columns
    if (jColStats->isVirtualColForHist()) continue;
...
MultiColumnSkewedValueLists::MultiColumnSkewedValueLists()
    : HASHDICTIONARY(ValueIdList, MCSkewedValueList)(&(MultiColumnSkewedValueLists::HashFunction), 5, TRUE, HISTHEAP){};
...
// -----------------------------------------------------------------------
MultiColumnSkewedValueLists::MultiColumnSkewedValueLists(const StatsList &initStats, const ValueIdList &tableColumns)
    : HASHDICTIONARY(ValueIdList, MCSkewedValueList)(&(MultiColumnSkewedValueLists::HashFunction), 5, TRUE, HISTHEAP) {
  // loop through the list of NAColumnArray's
  for (CollIndex i = 0; i < initStats.groupUecColumns_.entries(); i++) {
    const NAColumnArray &cols = initStats.groupUecColumns_[i];

    if (cols.entries() == 1) continue;
...
const MCSkewedValueList *MultiColumnSkewedValueLists::getMCSkewedValueList(ValueIdSet columns, ValueIdList &colGroup) {
  ...
  for (iter.getNext(keyEntry, valueEntry); keyEntry != NULL && valueEntry != NULL; iter.getNext(keyEntry, valueEntry)) {
    if (noOfCols != (*keyEntry).entries()) continue;
...
CostScalar ColStatDescList::getAvgRowcountForNonSkewedMCValues(ValueIdSet cols, MCSkewedValueList *mCSkewedValueList) {
  ...
  if (mCSkewedValueList && getUecList()) {
    ...
    if (mcUecForCols - noOfMCSkewValues > csZero) {
      ...
      avgRowcountForNonSkewedMCValues =
          (totalRowcount - totalRowcountOfMCSkewedValues) / (mcUecForCols - noOfMCSkewValues);
    }
  }
