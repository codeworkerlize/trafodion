...
RETDesc::RETDesc()
    : groupedFlag_(FALSE),
      ...
      potentialUserColumnList_(CmpCommon::statementHeap()),
...
RETDesc::RETDesc(BindWA *bindWA)
    : groupedFlag_(FALSE),
      ...
      potentialUserColumnList_(bindWA->wHeap()),
      naTypeForUserColumnList_(CmpCommon::statementHeap()),
...
RETDesc::RETDesc(BindWA *bindWA, const RETDesc &sourceTable)
  ...
  tdesc_ = (TableDesc *)sourceTable.getTableDesc();
...
RETDesc::RETDesc(BindWA *bindWA, TableDesc *tdesc, CorrName *newCorrName)
  ...
  corrName.extractAndDefaultNameParts(bindWA, bindWA->getDefaultSchema(), cat, sch, tbl);
  ...
  for (CollIndex i = 0; i < vidList.entries(); i++) {
    ...
    ColRefName colRefName(column->getColName(), corrName, CmpCommon::statementHeap());
    addColumn(bindWA, colRefName, valId, column->getColumnClass(), column->getHeading());
  }
...
RETDesc::RETDesc(BindWA *bindWA, RoutineDesc *rdesc, QualifiedName *newQualName)
  ...
  // before the loop where it is passed down and "defaulted" again and again.
  CMPASSERT(newQualName || (rdesc && rdesc->getNARoutine() && rdesc->getNARoutine()->getRoutineName()));

  const QualifiedName &qualName =
      newQualName ? *newQualName : rdesc->getNARoutine()->getRoutineName()->getQualifiedNameObj();
  NAString cat, sch, routine;
  qualName.extractAndDefaultNameParts(bindWA->getDefaultSchema(), cat, sch, routine);
  ...
  if (rdesc->isUUDFRoutine())
    naroutine = rdesc->getActionNARoutine();
  else
    naroutine = rdesc->getNARoutine();
  ...
  for (CollIndex i = 0; i < vidList.entries(); i++) {
    ...
    ColRefName colRefName(column->getColName(), qualName, CmpCommon::statementHeap());
    addColumn(bindWA, colRefName, valId, column->getColumnClass(), column->getHeading());
  }
...
RETDesc::~RETDesc() {
  ...
  if (bindWA_) bindWA_->getRETDescList().remove(this);
...
// RETDesc::addColumn()
// ***********************************************************************
void RETDesc::addColumn(BindWA *bindWA, const ColRefName &colRefName, const ValueId valId, const ColumnClass colClass,
                        const char *heading, const NABoolean isRenameCol) {
  //
  ...
  if (colClass == SYSTEM_COLUMN)
    addColumnDesc(bindWA, colRefName, valId, heading, isRenameCol, systemColumnList_);
  else
    addColumnDesc(bindWA, colRefName, valId, heading, isRenameCol, userColumnList_);
  //
...
// RETDesc::delColumn()
// ***********************************************************************
void RETDesc::delColumn(BindWA *bindWA, const ColRefName &colRefName, const ColumnClass colClass) {
  //
...
// ***********************************************************************
void RETDesc::addColumnDesc(BindWA *bindWA, const ColRefName &colRefName, const ValueId valId, const char *heading,
                            const NABoolean isRenameCol, ColumnDescList &columnList) {
  // Create a column descriptor and insert it into the column list.
  //
  ColumnDesc *column = new (bindWA->wHeap()) ColumnDesc(colRefName, valId, heading, bindWA->wHeap());
  column->setRenameCol(isRenameCol);
  columnList.insert(column);
  ...
  //
  NAString simpleColNameStr(colRefName.getColName(), CmpCommon::statementHeap());
  ...
  if (xcnmEntry) {
    if (NOT xcnmEntry->isDuplicate()) newSimpleDup = xcnmEntry->getValueId();  // *not* "newSimpleDup = valId"!
  ...
  if (corrName.getCorrNameAsString() != "") {
    ...
    cstColRefName =
        new (bindWA->wHeap()) ColRefName(simpleColNameStr, CorrName(tbl, bindWA->wHeap(), sch, cat), bindWA->wHeap());
    cstColRefName->setSpecialType(specialType);
    canonicalColRefName = cstColRefName;  // "c.s.t.col"

    if (defaultMatch) {
      stColRefName =
          new (bindWA->wHeap()) ColRefName(simpleColNameStr, CorrName(tbl, bindWA->wHeap(), sch), bindWA->wHeap());
      stColRefName->setSpecialType(specialType);

      if (defaultMatch > 1) {
        tColRefName = new (bindWA->wHeap()) ColRefName(simpleColNameStr, CorrName(tbl), bindWA->wHeap());
  ...
  if (!xcnmEntry) {
    ...
    xcnm_.insert(new (bindWA->wHeap()) ColumnNameMap(*canonicalColRefName, column, bindWA->wHeap(), isDup));
    if (defaultMatch) {
      xcnm_.insert(new (bindWA->wHeap()) ColumnNameMap(*stColRefName, column, bindWA->wHeap(), isDup));
      if (defaultMatch > 1) {
        xcnm_.insert(new (bindWA->wHeap()) ColumnNameMap(*tColRefName, column, bindWA->wHeap(), isDup));
...
// ***********************************************************************
void RETDesc::delColumnDesc(BindWA *bindWA, const ColRefName &colRefName, ColumnDescList &columnList) {
  //
  ...
  if (defaultMatch) {
    ...
    if (defaultMatch > 1) xcnm_.remove(&tColRefName);
  }
...
// ***********************************************************************
void RETDesc::nullInstantiateAndAddColumns(BindWA *bindWA, NABoolean forceCast, const ColumnDescList &columnList,
                                           const ColumnClass colClass) {
  for (CollIndex i = 0; i < columnList.entries(); i++) {
    ValueId nullId = columnList[i]->getValueId().nullInstantiate(bindWA, forceCast);
...
// ***********************************************************************
RETDesc *RETDesc::nullInstantiate(BindWA *bindWA, NABoolean forceCast, ValueIdList &nullOutputList) const {
  RETDesc *resultTable = new (bindWA->wHeap()) RETDesc();

  resultTable->nullInstantiateAndAddColumns(bindWA, forceCast, *getColumnList(), USER_COLUMN);

  resultTable->nullInstantiateAndAddColumns(bindWA, forceCast, *getSystemColumnList(), SYSTEM_COLUMN);
...
ColumnNameMap *RETDesc::findColumn(const ValueId vid) const {
  ...
  for (CollIndex i = 0; i < colNameMapList.entries(); i++) {
    ...
    if (colNameMap->getValueId() EQU vid) {
      const ColRefName &colRefName = colNameMap->getColumnDesc()->getColRefNameObj();
      if (colRefName.isFabricated()) {
        return colNameMap;
      } else if (NOT colRefName.isEmpty() AND colRefName.getCorrNameObj().getQualifiedNameObj().fullyExpanded()) {
        return colNameMap;
      } else if (!currColNameMap ||
                 (colRefName.getCorrNameObj().getQualifiedNameObj().numberExpanded() > currColNameMap->getColumnDesc()
                                                                                           ->getColRefNameObj()
                                                                                           .getCorrNameObj()
                                                                                           .getQualifiedNameObj()
                                                                                           .numberExpanded())) {
...
// ***********************************************************************
void RETDesc::getTableList(LIST(TableNameMap *) & xtnmList, NAString *textList) const {
  xtnmList.clear();
  xtnm_.dump(xtnmList);
  CollIndex i;
  ...
  for (i = xtnmList.entries(); i-- > 0;)
    if (!xtnmList[i]->getColumnList()) xtnmList.removeAt(i);
  ...
  xtnmList = xtnmCST;
  for (i = 0; i < xtnmCORR.entries(); i++) xtnmList.insert(xtnmCORR[i]);

  formatTableList(xtnmList, textList);  // a no-op if textList passed as NULL

}  // RETDesc::getTableList

/*static*/ void RETDesc::formatTableList(LIST(TableNameMap *) & xtnmList, NAString *textList,
  ...
  if (xtnmList.entries() > 1) {
    ...
    while (xtnmT.entries()) {
      ...
      if (xtnmList.entries() == 0 ||  // don't insert dups
          xtnmList[xtnmList.entries() - 1]->getTableName() != xtnmT[minTi]->getTableName())
...
// ***********************************************************************
void RETDesc::propagateColumn(BindWA *bindWA, const ColRefName &colName, NABoolean isFromRoot, ColumnClass colClass) {
  ...
  if (retDesc != NULL) colMap = retDesc->findColumn(colName);

  if (colMap != NULL) {
    // If it is not in 'this', copy it over.
    ColumnNameMap *viewCol = findColumn(colMap->getValueId());
    if (viewCol != NULL) {
      NABoolean isSyskeyOfLog = colName.getCorrNameObj().getSpecialType() == ExtendedQualName::IUD_LOG_TABLE;
      if (isFromRoot || isSyskeyOfLog)
        return;
      else {
        // this RETDesc already has the ValueId - delete it before reinserting.
        const ColRefName &viewColRefName = viewCol->getColumnDesc()->getColRefNameObj();
        delColumn(bindWA, viewColRefName, colClass);
      }
    }

    addColumn(bindWA, colMap->getColumnDesc()->getColRefNameObj(), colMap->getValueId(), colClass);
  }
}
...
void RETDesc::print(FILE *ofd, const char *indent, const char *title) const {
  ...
  if (userColumnList_.entries() || systemColumnList_.entries()) fprintf(ofd, "\n\nUSER COLUMNS:\n\n");
...
static void displayDownHelper(RelExpr *re, RETDesc *prevRD, int level) {
  ...
  cout << endl << endl << indent << "====== Operator: " << re->getText().data() << endl << flush;
...
/*static*/ void RETDesc::displayDown(RelExpr *re) { displayDownHelper(re, NULL, 1); }  // RETDesc::displayDown
