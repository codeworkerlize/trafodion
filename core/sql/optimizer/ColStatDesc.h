...
public:
 ...
 // merge two ColStatDesc's
  ...
  // clarification. For regular histograms the following are true:
  //
  // 'column_' is the column for which these stats apply.
  //
  ...
  //
  // Join predicates on expressions are always non-VEG predicates, so
  // 'nonVegEquals_' is used to track those, in the same way as normal
  // histograms.
  // ------------------------------------------------------------------------
...
private:
  ...
  // Set to TRUE if this ColStatDescList has any expression histograms
