  ...
  //    of all user-specified parts of the query.
{
  ...
  if (cwa.getPhase() >= CmpMain::BIND && 
    ...
    if (tbl->isPartitionV2Table() || tbl->isPartitionEntityTable())
      cwa.setUsePartitionTable(TRUE);

  }

  if (sCacheKey4UpdateSetList_.length()>0)
    cwa += sCacheKey4UpdateSetList_;
...
void Delete::generateCacheKey(CacheWA &cwa) const
{
  ...
  {
    if(((Scan*)(child(0)->castToRelExpr()))->skipTransactionForCursorUpdDelScanForced())
      cwa += " nrr:";
  } 
}
...
NABoolean RelRoot::isCacheableExpr(CacheWA& cwa)
{
  ...
  if (getArity() == 1 && 
      !child(0)->getText().compareTo("CONNECT_BY_STATEMENT", NAString::ignoreCase))
    return TRUE;
...
void ExeUtilConnectby::generateCacheKey(CacheWA &cwa) const
{
  ...
  if (myselection != NULL)
    myselection->generateCacheKey(cwa);
...
void ConnectBy::generateCacheKeyNode(CacheWA &cwa) const
{
   cwa += getText();
}
...
void ConnectByTempTable::generateCacheKeyNode(CacheWA &cwa) const
{
   cwa += getText();
}

NABoolean ExeUtilLobExtract::isCacheableExpr(CacheWA& cwa)
{
  if (toType_ != TO_BUFFER_)
    return FALSE;

  if (! ExeUtilExpr::isCacheableExpr(cwa))
    return FALSE;
...
// change literals of a cacheable query into ConstantParameters 
RelExpr* ExeUtilLobExtract::normalizeForCache(CacheWA& cwa, BindWA& bindWA)
{
  if (nodeIsNormalizedForCache()) { 
    return this; 
  }
  ...
  if (objectUIDexpr_)
    {
      handle_ = handle_->normalizeForCache(cwa, bindWA);
      bufAddrExpr_ = bufAddrExpr_->normalizeForCache(cwa, bindWA);
      extractSizeAddrExpr_ = extractSizeAddrExpr_->normalizeForCache(cwa, bindWA);
    }

  return ExeUtilExpr::normalizeForCache(cwa, bindWA);
}

// append an ascii-version of Merge into cachewa.qryText_
void ExeUtilLobExtract::generateCacheKey(CacheWA &cwa) const
{
  ExeUtilExpr::generateCacheKey(cwa);

   cwa += getText();
}

NABoolean ExeUtilLobUpdate::isCacheableExpr(CacheWA& cwa)
{
...
// change literals of a cacheable query into ConstantParameters 
RelExpr* ExeUtilLobUpdate::normalizeForCache(CacheWA& cwa, BindWA& bindWA)
{
  if (nodeIsNormalizedForCache()) { 
    return this; 
  }
...
// append an ascii-version of Merge into cachewa.qryText_
void ExeUtilLobUpdate::generateCacheKey(CacheWA &cwa) const
{
  ExeUtilExpr::generateCacheKey(cwa);
