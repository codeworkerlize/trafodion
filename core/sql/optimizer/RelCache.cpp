...
//    of all user-specified parts of the query.
{
  ...
  const NATable *tbl;
  if (cwa.getPhase() >= CmpMain::BIND && getTableDesc() && (tbl = getTableDesc()->getNATable()) != NULL) {
    char redefTime[40];
    convertInt64ToAscii(tbl->getRedefTime(), redefTime);
    cwa += " redef:";
    cwa += redefTime;

    if (tbl->isPartitionV2Table() || tbl->isPartitionEntityTable()) cwa.setUsePartitionTable(TRUE);
  }

  if (sCacheKey4UpdateSetList_.length() > 0) cwa += sCacheKey4UpdateSetList_;

  ItemExpr *newExpr = newRecExprTree_ ? newRecExprTree_ : newRecExpr_.rebuildExprTree(ITM_ITEM_LIST);
  if (newExpr) {
    cwa += " newRecExpr:";
  ...
  if (newRecBeforeExpr_.entries() > 0 && (setOnRollback = newRecBeforeExpr_.rebuildExprTree(ITM_ITEM_LIST))) {
    cwa += " setOnRollback:";
    setOnRollback->generateCacheKey(cwa);
  }
  ItemExpr *execPred = executorPredTree_ ? executorPredTree_ : executorPred_.rebuildExprTree();
  if (execPred) {
    cwa += " execPred:";
...
NABoolean GenericUpdate::isCacheableExpr(CacheWA &cwa) {
  ...
  if (cwa.getPhase() >= CmpMain::BIND) {
    ...
    if ((getTableName().isPartitionNameSpecified()) || (getTableName().isLocationNameSpecified()) ||
        (getTableName().isPartitionV2()) || (getTableName().isPartitionRangeSpecified()))
...
RelExpr *Scan::normalizeForCache(CacheWA &cwa, BindWA &bindWA) {
  ...
  if (origName.getPrototype() == NULL) {
    ...
    hv->setPrototypeValue(origName.getQualifiedNameObj().getQualifiedNameAsAnsiString());
    hv->synthTypeAndValueId();
    hv->setIsCachedParam(TRUE);

    CorrName cn("HostVar$", bindWA.wHeap(),
                hv->getName(),  // debugging ease
                "$bogus");

    cn.setPrototype(hv);
    NAString *tmpName = new (bindWA.wHeap()) NAString(hv->getPrototypeValue(), bindWA.wHeap());
    ...
    char *strval = new (bindWA.wHeap()) char[CACHED_MAX_ANSI_NAME_EXTERNAL_LEN];
    strcpy(strval, origName.getQualifiedNameAsString().data());
    CharType *typ = new (bindWA.wHeap()) SQLChar(bindWA.wHeap(), CACHED_MAX_ANSI_NAME_EXTERNAL_LEN, FALSE);
    ConstValue *cv = new (bindWA.wHeap()) ConstValue(typ, strval, CACHED_MAX_ANSI_NAME_EXTERNAL_LEN);

    ConstantParameter *result =
...
RelExpr *GenericUpdate::normalizeForCache(CacheWA &cwa, BindWA &bindWA) {
  ...
  if (cwa.getIsoLvlForUpdates() == TransMode::IL_NOT_SPECIFIED_) {
    ...
    if (il == TransMode::IL_NOT_SPECIFIED_) {
      ActiveSchemaDB()->getDefaults().getIsolationLevel(il, CmpCommon::getDefault(ISOLATION_LEVEL));
    }
...
void IsolatedScalarUDF::generateCacheKey(CacheWA &cwa) const {
  ...
  const ItemExpr *paramExpr = (getProcAllParamsTree() == NULL) ? getProcInputParamsVids().rebuildExprTree(ITM_ITEM_LIST)
...
void GroupByAgg::generateCacheKey(CacheWA &cwa) const {
  ...
  ItemExpr *grpExpr = groupExprTree_ ? groupExprTree_ : groupExpr_.rebuildExprTree(ITM_ITEM_LIST);
  if (grpExpr) {
    cwa += " gBy:";
...
void Insert::generateCacheKey(CacheWA &cwa) const {
  ...
  ItemExpr *orderBy = orderByTree_ ? orderByTree_ : reqdOrder_.rebuildExprTree();
  if (orderBy) {
    cwa += " order:";
    orderBy->generateCacheKey(cwa);
  }

  // include overwrite clause as part of cache key.
  if (overwriteHiveTable_) {
    cwa += " overwrite:";
  }

  const NATable *tbl;
  if (cwa.getPhase() >= CmpMain::BIND && getTableDesc() && (tbl = getTableDesc()->getNATable()) != NULL) {
    // if hive external table, specify that.
    if (tbl->isHiveExternalTable()) cwa += " hive_ext_table: ";
    ...
    // If PARTITION range has been used we must reflect that in the key.
      ...
      char str[100];
      sprintf(str, " from %d to %d", tbl->getExtendedQualName().getPartnClause().getBeginPartitionNumber(),
  ...
  if (isUpsert()) {
    if (isUpsertLoad()) {
      if (getIsTrafLoadPrep())
        ...
        cwa += " upsert using load:";
    } else
      cwa += " upsert:";

    if (withNoConflictCheck()) cwa += " ncc:";

    if (noRepeatableRead()) cwa += " nrr:";
  }
}
...
NABoolean Insert::isCacheableExpr(CacheWA &cwa) {
  ...
  if (insertType_ != SIMPLE_INSERT && insertType_ != UPSERT_LOAD) {
    return FALSE;
  }

  // if partition list to be dropped is specified, it is non-cacheable
  if (partColVals_) return FALSE;
...
// is this entire expression cacheable after this phase?
NABoolean Delete::isCacheableExpr(CacheWA &cwa) { return GenericUpdate::isCacheableExpr(cwa); }
...
void Delete::generateCacheKey(CacheWA &cwa) const {
  ...
  RelExpr *c0 = child(0);
  if (c0 && (c0->getOperatorType() == REL_SCAN)) {
    if (((Scan *)(child(0)->castToRelExpr()))->skipTransactionForCursorUpdDelScanForced()) cwa += " nrr:";
  }
}
...
void MergeUpdate::generateCacheKey(CacheWA &cwa) const {
  ...
  if (where_) {
    cwa += " wherePred:";
    where_->generateCacheKey(cwa);
  }

  if (andpred_) {
    cwa += " mergeMatchedAndPred:";
    andpred_->generateCacheKey(cwa);
  }
}

// is this entire expression cacheable after this phase?
NABoolean MergeUpdate::isCacheableExpr(CacheWA &cwa) {
  if ((insertValues_) && (insertValues_->isCacheableExpr(cwa))) {
    setNonCacheable();
    return FALSE;
  }
...
void Join::generateCacheKey(CacheWA &cwa) const {
  ...
  ItemExpr *pred = joinPredTree_ ? joinPredTree_ : joinPred_.rebuildExprTree();
  if (pred) {
    cwa += " joinPred:";
...
NABoolean Join::isCacheableExpr(CacheWA &cwa) {
  if (cwa.getPhase() >= CmpMain::BIND) {
    ...
    RelExpr *c0 = child(0);
    RelExpr *c1 = child(1);
    if ((c0->getOperatorType() == REL_RENAME_TABLE && ((RenameTable *)c0)->isView() == TRUE) ||
        (c1->getOperatorType() == REL_RENAME_TABLE && ((RenameTable *)c1)->isView() == TRUE)) {
      cwa.foundViewJoin();
    }
...
// change literals of a cacheable query into ConstantParameters
RelExpr *Join::normalizeForCache(CacheWA &cwa, BindWA &bindWA) { return RelExpr::normalizeForCache(cwa, bindWA); }
...
void RelExpr::generateCacheKeyNode(CacheWA &cwa) const {
  ...
  cwa += getText();
  ItemExpr *pred = selPredTree() ? selPredTree() : getSelectionPred().rebuildExprTree();
  if (pred) {
    cwa += " selPred:";
    pred->generateCacheKey(cwa);
  }
  ...
  if (hint_ && (CmpCommon::getDefault(GENERATE_USER_QUERYCACHE_REPLACE) != DF_ON)) {
...
void RelRoot::generateCacheKey(CacheWA &cwa) const {
  RelExpr::generateCacheKey(cwa);
  ItemExpr *cExpr = compExprTree_ ? compExprTree_ : compExpr_.rebuildExprTree();
  if (cExpr) {
  ...
  if (opts.accessType() != TransMode::ACCESS_TYPE_NOT_SPECIFIED_) {
    tmode.updateAccessModeFromIsolationLevel(TransMode::ATtoIL(opts.accessType()));
  ...
  char mode[40];
  str_itoa(tmode.getIsolationLevel(), mode);
  cwa += " il:";
  cwa += mode;
  str_itoa(tmode.getAccessMode(), mode);
  cwa += " am:";
  cwa += mode;

  // Solution: 10-060418-5903
  str_itoa(cwa.getIsoLvlForUpdates(), mode);
  cwa += " ilu:";
  cwa += mode;

  str_itoa(tmode.getAutoCommit(), mode);
  cwa += " ac:";
  cwa += mode;
  str_itoa(tmode.getFlags(), mode);
  cwa += " fl:";
  cwa += mode;
  str_itoa(tmode.getRollbackMode(), mode);
  cwa += " rm:";
  cwa += mode;
  str_itoa(tmode.getAutoAbortIntervalInSeconds(), mode);
  cwa += " ai:";
  cwa += mode;
  str_itoa(tmode.getMultiCommit(), mode);
  cwa += " mc:";
  cwa += mode;
  ...
  ItemExpr *updCol = updateColTree_ ? updateColTree_ : updateCol_.rebuildExprTree();
  if (updCol) {
    cwa += " updCol:";
    updCol->generateCacheKey(cwa);
  }

  if ((isTrueRoot()) && (((RelRoot *)this)->savepointEnabled())) {
    cwa += " sp ";
  }
...
NABoolean RelRoot::isCacheableExpr(CacheWA &cwa) {
  // for connectby operator
  if (getArity() == 1 && !child(0)->getText().compareTo("CONNECT_BY_STATEMENT", NAString::ignoreCase)) return TRUE;

  // queries prefixed by display are not cacheable e.g. display select * from ...
  if (getDisplayTree()) return FALSE;

  // Parallel extract producer queries are not cacheable
  if (numExtractStreams_ > 0) return FALSE;
...
NABoolean Scan::isCacheableExpr(CacheWA &cwa) {
  if (cwa.getPhase() >= CmpMain::BIND) {
    ...
    cwa.setConditionallyCacheable();
    if (CmpCommon::getDefaultLong(MVQR_REWRITE_LEVEL) >= 1 && QRDescGenerator::hasRewriteEnabledMVs(getTableDesc())) {
...
void ExeUtilConnectby::generateCacheKey(CacheWA &cwa) const {
  BiConnectBy *biConnBy = (BiConnectBy *)((RelExpr *)this)->getBiConnectBy();
  if (biConnBy) {
    ItemExpr *connBy = biConnBy->getConnectBy();
    if (connBy) {
      cwa += "ConnectBy:";
      connBy->generateCacheKey(cwa);
      if (((BiConnectByRelat *)connBy)->getParentColIE())
        ((BiConnectByRelat *)connBy)->getParentColIE()->generateCacheKey(cwa);
    }

    // startwith part
    cwa += biConnBy->getStartWithString();

    if (biConnBy->getNoCycle() == TRUE)
      cwa += "cycle:no";
    else
      cwa += "cycle:yes";
    if (biConnBy->getNoDup() == TRUE)
      cwa += "dup:no";
    else
      cwa += "dup:yes";
  }
  ItemExpr *myselection = getMySelection();
  if (myselection != NULL) myselection->generateCacheKey(cwa);
}
...
void Tuple::generateCacheKey(CacheWA &cwa) const {
  ...
  ItemExpr *tExpr = tupleExprTree() ? tupleExprTree() : tupleExpr_.rebuildExprTree();
  if (tExpr) {
    cwa += " tupExpr:";
...
// append an ascii-version of RelExpr into cachewa.qryText_
void FirstN::generateCacheKey(CacheWA &cwa) const {
  generateCacheKeyNode(cwa);
  generateCacheKeyForKids(cwa);
}

void FirstN::generateCacheKeyNode(CacheWA &cwa) const {
  // emit any "[firstn_sorted]" modifier
  if (firstNRows_ != -1) {
    char firstN[40];
    convertInt64ToAscii(((FirstN *)this)->getFirstNRows(), firstN);
    cwa += firstN;
    cwa += " ";
  }
...
void ConnectBy::generateCacheKey(CacheWA &cwa) const {
  BiConnectBy *biConnBy = (BiConnectBy *)((RelExpr *)this)->getBiConnectBy();
  if (biConnBy) {
    ItemExpr *connBy = biConnBy->getConnectBy();
    if (connBy) {
    ...
    ItemExpr *startWith = biConnBy->getStartWith();
    if (startWith) {
      cwa += "StartWith:";
      startWith->generateCacheKey(cwa);
    }
    if (biConnBy->getNoCycle() == TRUE)
      cwa += "cycle:no";
    else
      cwa += "cycle:yes";
    ItemExpr *myselection = biConnBy->where_clause;
    if (myselection != NULL) myselection->generateCacheKey(cwa);
  }

  ItemExpr *pathIe = getPathItemExpr();
  if (pathIe != NULL) pathIe->generateCacheKey(cwa);
...
void ConnectBy::generateCacheKeyNode(CacheWA &cwa) const { cwa += getText(); }

void ConnectByTempTable::generateCacheKey(CacheWA &cwa) const {
  generateCacheKeyNode(cwa);
  generateCacheKeyForKids(cwa);
}

void ConnectByTempTable::generateCacheKeyNode(CacheWA &cwa) const { cwa += getText(); }

void RelSequence::generateCacheKey(CacheWA &cwa) const {
  ItemExpr *orderList = requiredOrder().rebuildExprTree();
  if (orderList) {
    cwa += " requiredOrderList:";
    orderList->generateCacheKey(cwa);
  }
