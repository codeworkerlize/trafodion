...
void PartRangePerCol::display()
{
  print();
}

void PartRangePerCol::createBoundForPartCol(BindWA *bindWA, 
                                                    ...
                                                    const NABoolean genRange)
{
  ...
  ColRefName *colRefName = new (bindWA->wHeap()) 
                              ColRefName(colArray[colIdxArray[partColPos_]]->getColName(), 
                                         bindWA->wHeap());
  ...
  if ( boundary->lowValue && !boundary->isLowMaxValue())
  {
    ...
    ItemExpr *lowExpr = parser.getItemExprTree(boundary->lowValue,
                                                strlen(boundary->lowValue),
  ...
  {
    ItemExpr *highPred = parser.getItemExprTree(boundary->highValue,
                                                strlen(boundary->highValue),
                                                CharInfo::UTF8 // ComGetNameInterfaceCharSet()
                                                );
    if ( highPred )
    {
  ...
  if ( partIdx_ > 0 && boundary_->lowBound_ != NULL )
  {
    ...
    {
      lowBoundExpr = new (bindWA->wHeap()) BiRelat(ITM_EQUAL, 
                                                colName_, 
                                                boundary_->lowBound_);
    ...
    if ( partColPos_ == 0 )
    {
      lowInterval = new (bindWA->wHeap()) BiRelat(ITM_GREATER, 
                                                    colName_, 
                                                    boundary_->lowBound_);
  ...
  if ( boundary_->highBound_ && !isMaxValue())
  {
    ...
    highInterval = new (bindWA->wHeap()) BiRelat(ITM_LESS, 
                                             colName_, 
                                             boundary_->highBound_);
  ...
  if ( interval )
  {
         ...
         interval->child(1)->doesExprEvaluateToConstant(FALSE, FALSE))
    {
      ...
      if (constFoldExpr)
        interval->setChild(1,constFoldExpr);
    }
  ...
  if ( genRange && interval )
  {
    ...
    QRDescGenerator *descGen1 = new (bindWA->wHeap()) 
                                     QRDescGenerator(false, bindWA->wHeap());
    descGen1->createEqualitySets(intervalPred);
    intervalRange_ = OptRangeSpec::createRangeSpec(descGen1,
                                                   interval,
                                                   bindWA->wHeap(),
                                                   FALSE);
  }
  ...
  if ( partColPos_ != naTable->getPartitionColCount() - 1 &&
       naTable->getPartitionColCount() > 1 && highEqPred )
  {
    ...
    {
      // generate range spec
      QRDescGenerator *descGen2 = new (bindWA->wHeap()) 
                                       QRDescGenerator(false, bindWA->wHeap());
      descGen2->createEqualitySets(highBoundaryPred);
      highBoundRange_ = OptRangeSpec::createRangeSpec(descGen2,
                                                      highEqPred,
                                                      bindWA->wHeap(),
                                                      FALSE);
    }
...
void PartRange::display()
{
  print();
}


void Partition::print(FILE* ofd, 
                        const char* indent, 
                        const char* title,
                        CollHeap *c, 
                        char *buf
...
TableDesc::TableDesc(BindWA *bindWA,
         	...
         	: table_(table),
                  ...
                  TRUE /* enforce uniqueness */,bindWA->wHeap()),
...
ItemExpr * TableDesc::genPartv2BaseCheckConstraint(BindWA *bindWA)
{
  ...
  if (firstPart->getPartIndex() == 0 && firstPart->getlowBoundary() != NULL)
  {
    ...
    {
      ColRefName *colRefName = new (bindWA->wHeap())
                                  ColRefName(colArray[colIdxArray[i]]->getColName(),
                                             bindWA->wHeap());
      if ( i == 0 )
      {
        ...
        colList = new (bindWA->wHeap())
                        ItemList(colList,
                                 new (bindWA->wHeap()) ColReference(colRefName));
    }
  ...
  if (lastPart->getFirstMaxvalIdx() >= 0)
    colCount = lastPart->getFirstMaxvalIdx();
  for (int i = 0; i < colCount; i++)
  {
    ColRefName *colRefName = new (bindWA->wHeap())
                                ColRefName(colArray[colIdxArray[i]]->getColName(),
    ...
    if ( i == 0 )
      ...
      highColList = new (bindWA->wHeap())
                      ItemList(highColList,
                               new (bindWA->wHeap()) ColReference(colRefName));
  }
  colList = highColList;
...
void TableDesc::addPartnPredToConstraint(BindWA *bindWA)
{
  ...
  if ( part->getPartIndex() == 0 && part->getFirstMaxvalIdx() == 0 )
    return;
  ...
  if (part->getFirstMaxvalIdx() >= 0)
    colCount = part->getFirstMaxvalIdx();
  for (int i = 0; i < colCount; i++)
  {
    ColRefName *colRefName = new (bindWA->wHeap()) 
                                ColRefName(colArray[colIdxArray[i]]->getColName(), 
...
void TableDesc::addPartPredToPrecondition(BindWA *bindWA,
                                                    ItemExpr *selectPred)
{
  ...
  for ( int i = 0; i < colCount; i++ )
  {
    ...
    QRDescGenerator* descGenerator = new (bindWA->wHeap()) 
                                        QRDescGenerator(false, bindWA->wHeap());
    ...
    for ( int j = 0; j < selPredList.entries(); j++ )
    {
      ...
      if ( operType0 != ITM_BASECOLUMN && operType1 != ITM_BASECOLUMN )
        continue;
      if ( operType0 == ITM_BASECOLUMN &&
           (operType1 == ITM_CONSTANT ||
            operType1 == ITM_DYN_PARAM ||
            operType1 == ITM_CACHE_PARAM) )
      {
        const NAColumn *predCol = child0->getValueId().getNAColumn();
        if ( predCol->getPosition() != position )
          continue;
        item->bindNode(bindWA);
        colPredSet.addMember(item);
      }
      else if ( operType1 == ITM_BASECOLUMN &&
                (operType0 == ITM_CONSTANT ||
                 operType0 == ITM_DYN_PARAM ||
                 operType0 == ITM_CACHE_PARAM))
      {
    ...
    if ( colPredSet.entries() > 0 )
    {
      ...
      OptRangeSpec* range = OptRangeSpec::createRangeSpec(descGenerator,
                                                          colPredSet.rebuildExprTree(ITM_AND),
                                                          bindWA->wHeap(),
                                                          FALSE);
      ValueIdList vidList;
      if ( range )
      ...
      for ( int n=0; n<vidList.entries(); n++ )
      {
        ...
        else if ( tmpExpr->getOperatorType() == ITM_GREATER ||
                  tmpExpr->getOperatorType() == ITM_GREATER_EQ )
        {
          lowVal = tmpExpr;
        }
        else if ( tmpExpr->getOperatorType() == ITM_LESS||
                  tmpExpr->getOperatorType() == ITM_LESS_EQ )
        {
          highVal = tmpExpr;
        }
      }
...
void TableDesc::addPartPredToConstraint(BindWA *bindWA)
{
  ...
  if ( 1 == partColCount )
  {
    ...
    if ( intervalExpr == NULL && partCol0->isMaxValue() )
      return;
    ...
    for ( int i = partColCount-1; i >= 0; i-- )
    {
      ...
      if ( i == partColCount-1 )
      {
        ...
        if ( partIdx > 0 )
        {
          ...
          lowExpr = new (bindWA->wHeap()) BiRelat(ITM_GREATER_EQ, 
                                                  lowExpr->child(0), 
                                                  lowExpr->child(1));
        ...
        else
        {
          ...
          highExpr = new (bindWA->wHeap()) BiRelat(ITM_LESS, 
                                                   highExpr->child(0), 
                                                   highExpr->child(1));
      ...
      else
      {
        ...
        if ( partIdx > 0 )
        {
          ...
          if ( i != 0 )
          {
            ItemExpr *lowIntervExpr = new (bindWA->wHeap()) BiRelat(ITM_GREATER, 
                                                        lowExpr->child(0), 
                                                        lowExpr->child(1));
          ...
          lowExpr = new (bindWA->wHeap()) 
                         IfThenElse(lowEqPred,((*part)[i+1])->getLowConstrnExpr(), elseExpr);
            lowExpr = new (bindWA->wHeap()) Case (NULL, lowExpr);
          partCol->getLowConstrnExpr() = lowExpr;
        }
        else if ( partIdx == 0 )
        ...
        else
        {
          ...
          if ( i != 0 )
          {
            ItemExpr *highIntervExpr = new (bindWA->wHeap()) BiRelat(ITM_LESS, 
                                                        highExpr->child(0), 
                                                        highExpr->child(1));
          ...
          highExpr = new (bindWA->wHeap()) 
                       IfThenElse(highEqExpr, ((*part)[i+1])->getHighConstrnExpr(), elseExpr);
          //if ( i != 0 )
            highExpr = new (bindWA->wHeap()) Case (NULL, highExpr);
          partCol->getHighConstrnExpr() = highExpr;
        }
...
void TableDesc::addPartitions(BindWA *bindWA,
                                   const NATable *naTable,
...

{
  ...
  {
    NAString partEntityName(partList[i]->getPartitionEntityName(),
                             bindWA->wHeap());
    
    PartRange *partRange = new (bindWA->wHeap())
                                PartRange(bindWA->wHeap(), 
                                naTable, 
                                partList[i]->getPartPosition(),
                                partEntityName);
    ...
    {
      PartRangePerCol *rangePerCol = new (bindWA->wHeap()) 
                                          PartRangePerCol(bindWA->wHeap(), 
                                                          partList[i]->getPartPosition(), 
                                                          j);
      partRange->insert(rangePerCol);
      rangePerCol->createBoundForPartCol(bindWA, 
                                         naTable, 
                                         boundList[j],
                                         j + 1 < boundList.entries() ? boundList[j + 1] : NULL,
                                         genRange);
    }
...
void TableDesc::genPartitions(BindWA *bindWA,
                                   const NAPartitionArray &partList)

{
  ...
  for (int i = 0; i < partList.entries(); i++)
  {
    ...
    Partition *partition = new (bindWA->wHeap())
                                Partition(bindWA->wHeap(),
                                          Partition::RANGE_PARTITION,
                                          partList[i]->getPartPosition(),
                                          (Int32)partList.entries(),
                                          partName,
                                          partEntityName
                                          );
    ...
    if ( i == partList.entries()-1 )
      partition->isLastPart() = TRUE;
    ...
    for (int j = 0; j < boundList.entries(); j++)
    {
      ...
      if ( boundList[j]->lowValue && partition->getFirstLowMaxvalIdx() < 0)
      {
        ...
        {
          if (partition->getFirstLowMaxvalIdx() < 0)
            partition->getFirstLowMaxvalIdx() = j;
        }
        else
        {
      ...
      if ( boundList[j]->highValue && partition->getFirstMaxvalIdx() < 0)
      {
        ...
        {
          if ( partition->getFirstMaxvalIdx()<0 )
            partition->getFirstMaxvalIdx() = j;
        }
        else
        {
          memset(buf, 0, sizeof(buf));
...
const ColStatDescList &TableDesc::getTableColStats()
{
    ...
    ((NATable *)table_)->statsToUse((TrafDesc *)((NATable *)table_)->getStoredStatsDesc(),
                       useEstimatedStats, useHistStats);

    if (useEstimatedStats)
      {
        TrafTableStatsDesc * statsDesc = 
          table_->getStoredStatsDesc()->tableStatsDesc();
...
LIST(NAString) *TableDesc::getMatchedPartions(BindWA *bindWA, 
                                              ItemList *values)
{
  ...
  else if ( values && values->getOperatorType() >= ITM_PLUS && 
            values->getOperatorType() <= ITM_DIVIDE )
  {
    valCount = 1;
  }
  ...
  for (int j=0; j<partCount; j++)
  {
    ...
    if ( index == 0 )
    {
      if ( lowMatched && partitions[j]->getFirstMaxvalIdx()==0 )
...
LIST(NAString) *TableDesc::getMatchedPartInfo(BindWA *bindWA, ItemExprList valList)
{
  ...
  const NAColumnArray &colArray = getNATable()->getNAColumnArray();
  const int *colIdxArray = getNATable()->getPartitionColIdxArray();
  LIST(OptRangeSpec *) *rangeList = new (bindWA->wHeap()) 
                                      LIST(OptRangeSpec *)(bindWA->wHeap());
  for (int i = 0; i < colCount; i++)
  {
    ColRefName *colRefName = new (bindWA->wHeap()) 
                                ColRefName(colArray[colIdxArray[i]]->getColName(), 
    ...
    if (valExpr->getOperatorType() != ITM_CONSTANT)
      valExpr = valExpr->constFold();
    BiRelat *pred = new (bindWA->wHeap())
                        BiRelat(ITM_EQUAL, 
                                new (bindWA->wHeap()) ColReference(colRefName),
                                valExpr);
    QRDescGenerator* descGenerator = new (bindWA->wHeap()) 
    ...
    if ((pred->child(1)->getOperatorType() == ITM_CAST) &&
         pred->child(1)->doesExprEvaluateToConstant(FALSE, FALSE))
    {
      ItemExpr * constFoldExpr = pred->child(1)->constFold();
      if (constFoldExpr)
        pred->setChild(1,constFoldExpr);
    }
...
LIST(NAString) *TableDesc::getMatchedPartInfo(BindWA *bindWA, ItemExpr *selectPred)
{
  ...
  {
    for (int i = 0; i < partCount; i++)
      partNames->insert(partRanges[i]->getPartEntityName());
    return partNames;
  }
  ...
  for ( int m = 0; m < selPredList.entries(); m++ )
  {
    ...
    if (pred.getItemExpr()->getOperatorType() == ITM_EQUAL ||
        pred.getItemExpr()->getOperatorType() == ITM_LESS ||
        pred.getItemExpr()->getOperatorType() == ITM_LESS_EQ ||
        pred.getItemExpr()->getOperatorType() == ITM_GREATER||
  ...
  if ( rangeList->entries() > 0 )
  {
    for ( int i = 0; i < partCount; i++)
    {
      ...
      for ( int j = 0; j < rangeList->entries(); j++ )
      {
        ...
        if (j != colCount-1)
        {
          if (!partRangePerCol->getHighBoundary())
            break;

          selRange = ((*rangeList)[j])->clone(bindWA->wHeap());
          selRange->intersectRange(partRangePerCol->getHighBoundary());
          if ( selRange->getSubranges().entries() > 0 &&
               j == rangeList->entries() - 1 )
          {
            partNames->insert(partRange->getPartEntityName());
            break;
          }
...
{
  if ( eqJoinCols.entries() != 1 || localPreds.entries() == 0 )
    return TRUE;

  ValueId colValId = eqJoinCols.init();  eqJoinCols.next(colValId);

  // Do it only for integer numeric data types for now.
  if (colValId.getType().getTypeQualifier() != NA_NUMERIC_TYPE )
      return FALSE;
  ...
  NABoolean nj_compute_probes_debug = 
          CmpCommon::getDefault(ORC_NJS_CPS_DEBUG) == DF_ON;
  ...
  // q79       157         1093
  //
  snprintf(query, sizeof(query), "select count(%s), min(%s), max(%s) "
                     "from %s "
                     "where %s",
                     colName.data(), colName.data(), colName.data(), 
                     getCorrNameObj().getQualifiedNameAsString().data(),    
  ...
  if ( count == 0 ) {
    ...
    if ( colValId.getType().getFSDatatype() == REC_BIN32_UNSIGNED ||
         colValId.getType().getFSDatatype() == REC_BIN32_SIGNED )
    {
    ...
    if ( colValId.getType().getFSDatatype() == REC_BIN64_UNSIGNED ||
         colValId.getType().getFSDatatype() == REC_BIN64_SIGNED )
    {
  ...
  if ( nj_compute_probes_debug )
     printf("Computed probe stats: rows=%.0f, gap distance=%.0f over [%.0f, %.0f]\n", 
            count.getValue(), max.getValue() - min.getValue() + 1, 
