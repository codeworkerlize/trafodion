...
int hashString(const NAString &str) { return str.hash(); }
int hashValueId(const QRValueId &vid) { return vid; }

Visitor::VisitResult SetRefVisitor::visit(QRElementPtr caller) {
  NAString ref = caller->getRef();
  if (ref == "") return VR_Continue;

  // Skip if not column, table, or joinpred
  if (!strchr("CTJ", *ref.data())) return VR_Continue;

  QRElementPtr elem = idHash_.getFirstValue(&ref);
  assertLogAndThrow1(CAT_SQL_COMP_QR_DESC_GEN, LL_MVQR_FAIL, elem != NULL, QRLogicException,
...
Visitor::VisitResult XmlValidatorVisitor::visit(QRElementPtr caller) {
  if (caller->getElementType() == ET_StringVal) {
    ...
    if (na_wcschr(value, NAWCHR('&')) || na_wcschr(value, NAWCHR('%'))) {
      foundProblem_ = TRUE;
      return VR_Stop;
    }
  }
...
void EqualitySet::determineType() {
  ...
  if (isEmpty()) {
    ...
    QRLogger::log(CAT_SQL_COMP_QR_DESC_GEN, LL_MVQR_FAIL, "determineType() called for empty EqualitySet.");
    return;
  }
  ...
  for (CollIndex i = 0; i < entries(); i++) {
    ...
    if (at(i)->getOperatorType() == ITM_CONSTANT) continue;
    ...
    if (!typeInitialized) {
      ...
      magnitude = isExact ? nt.getMagnitude() : nt.getTruePrecision() * 10 + 1;
      scale = nt.getScale();
    } else {
      // Modify any type parameters that are more restrictive than those
      // of this member.
      wasExact = isExact;
      if (nt.isExact()) isExact = TRUE;
      if (!nt.isSigned()) isSigned = FALSE;
      if (!nt.supportsSQLnull()) isNullable = FALSE;
      otherMagnitude = nt.isExact() ? nt.getMagnitude() : nt.getTruePrecision() * 10 + 1;
      if (magnitude > otherMagnitude) magnitude = otherMagnitude;
      if (scale > nt.getScale() || !wasExact) scale = nt.getScale();
    }
  }
...
QRDescGenerator::~QRDescGenerator() {
  ...
  for (CollIndex i = 0; i < allEqualitySets_.entries(); i++) delete allEqualitySets_[i];
}
...
NABoolean QRDescGenerator::typeSupported(const NAType *type) {
  ...
  switch (type->getTypeQualifier()) {
    case NA_NUMERIC_TYPE: {
      ...
      if (!numType->isExact()) return TRUE;
    ...
    case NA_CHARACTER_TYPE: {
      if (CmpCommon::getDefault(MODE_SPECIAL_1) == DF_ON) return FALSE;
    ...
    case NA_INTERVAL_TYPE: {
      ...
      switch (intvType->getStartField()) {
        case REC_DATE_YEAR:
          return (SQLInterval::MAX_LEADING_PRECISION >= IntervalType::getPrecision(intvType->getStartField(),
        ...
        default:
          ...
          if (type->getScale() > SQLInterval::MAX_FRACTION_PRECISION_USEC) return FALSE;

          return (SQLInterval::MAX_LEADING_PRECISION >=
                  IntervalType::getPrecision(intvType->getStartField(), intvType->getLeadingPrecision(),
                                             REC_DATE_SECOND, SQLInterval::MAX_FRACTION_PRECISION_USEC));
      }
    }
...
// -----------------------------------------------------------------------
NABoolean QRDescGenerator::getTableId(ValueId vid,
                                      CANodeId &nodeID,       // OUT
                                      ValueId &cvid,          // OUT
                                      ...
                                      int &colIndex)        // OUT
{
  ...
  if (pExpr->getOperatorType() == ITM_VEG_REFERENCE) {
    vegrefVid = vid;
    const ValueIdSet &vegMembers = static_cast<VEGReference *>(pExpr)->getVEG()->getAllValues();
    // Search the veg members for a base column, so we can get its table id.
    for (ValueId someMemberId = vegMembers.init(); !pBC && vegMembers.next(someMemberId);
  ...
  if (pBC) {
    baseColName = pBC->getNAColumn()->getTableName()->getQualifiedNameAsAnsiString() + "." + pBC->getColName();
    ...
    if (pTD) {
      if (vegrefVid == NULL_VALUE_ID) {
        ...
        if (ie->getOperatorType() == ITM_VEG_REFERENCE) vegrefVid = ie->getValueId();
      }
...
// -----------------------------------------------------------------------
QRColumnPtr QRDescGenerator::genQRColumn(ValueId vid, UInt32 joinPredId /*= 0*/, NABoolean markAsUsed /*= TRUE*/) {
  ...
  if (!gotNodeID) {
    ...
    assertLogAndThrow(CAT_SQL_COMP_QR_DESC_GEN, LL_MVQR_FAIL, isInstNull(itemExpr), QRDescriptorException,
  ...
  if (bGenColumnRefs_) {
    if (mColumnsUsed_.contains(col_vid)) {
      ...
      if (mExtraHubColumnsUsed_.contains(col_vid)) columnElement->setExtraHub(TRUE);
      return columnElement;
    } else if (markAsUsed)  // not when called by setPredBitmap()
    {
      mColumnsUsed_ += col_vid;
      if (isExtraHub) mExtraHubColumnsUsed_ += col_vid;
    }
  }
...
NABoolean QRDescGenerator::isInstNull(ItemExpr *itemExpr) {
  ...
  if (itemExpr->getOperatorType() == ITM_INSTANTIATE_NULL) return TRUE;

  if (itemExpr->getOperatorType() != ITM_VEG_REFERENCE) return FALSE;

  const ValueIdSet &vegMembers = ((VEGReference *)itemExpr)->getVEG()->getAllValues();
...
static void addFunctionParameters(ItemExpr *ie, NAMemory *heap, QRFunctionPtr function) {
  ...
  switch (ie->getOperatorType()) {
    ...
    default:
      ...
      assertLogAndThrow1(CAT_SQL_COMP_QR_DESC_GEN, LL_MVQR_FAIL, FALSE, QRDescriptorException,
                         "Unhandled function type in addFunctionParameters(): %d", ie->getOperatorType());
      break;
  }
}
...
QRExplicitExprPtr QRDescGenerator::getExprTree(ItemExpr *itemExpr) {
  ...
  switch (itemExpr->getQRExprElem()) {
    ...
    case QR::QRFunctionWithParameters: {
      QRFunctionPtr function = new (mvqrHeap_) QRFunction(ADD_MEMCHECK_ARGS(mvqrHeap_));
    ...
    case QR::QRBinaryOperElem: {
      QRBinaryOperPtr binaryOper = new (mvqrHeap_) QRBinaryOper(ADD_MEMCHECK_ARGS(mvqrHeap_));
    ...
    case QR::QRUnaryOperElem: {
      QRUnaryOperPtr unaryOper = new (mvqrHeap_) QRUnaryOper(ADD_MEMCHECK_ARGS(mvqrHeap_));
    ...
    case QR::QRColumnElem: {
      ...
      if (isInstNull(itemExpr)) return skipInstNull(itemExpr);
      ...
      if (itemExpr->getOperatorType() == ITM_VEG_REFERENCE) {
        ValueId constVid = static_cast<VEGReference *>(itemExpr)->getVEG()->getAConstant(TRUE);
        if (constVid != NULL_VALUE_ID) return getExprTree(constVid.getItemExpr());
      }
      ...
      if (elemType == ET_Column)
        ...
        assertLogAndThrow1(CAT_SQL_COMP_QR_DESC_GEN, LL_MVQR_FAIL, FALSE, QRDescriptorException,
                           "Unhandled element type returned from genQRColumn() in getExprTree(): %d", elemType);
    } break;
    ...
    case QR::QRScalarValueElem: {
      ...
      if (op == ITM_CACHE_PARAM)
        ...
        assertLogAndThrow1(CAT_SQL_COMP_QR_DESC_GEN, LL_MVQR_FAIL, op == ITM_CONSTANT, QRDescriptorException,
                           "In getExprTree(), expected ITM_CONSTANT but got %d", op);
        constVal = static_cast<ConstValue *>(itemExpr);
      }
      if (constVal->isNull()) return new (mvqrHeap_) QRNullVal(ADD_MEMCHECK_ARGS(mvqrHeap_));
      ...
      switch (type->getTypeQualifier()) {
        case NA_NUMERIC_TYPE:
          if (((NumericType *)type)->isExact()) {
            scalar = new (mvqrHeap_) QRNumericVal(ADD_MEMCHECK_ARGS(mvqrHeap_));
        ...
        case NA_CHARACTER_TYPE:
          if (((CharType *)type)->getBytesPerChar() == 1)
            ...
            assertLogAndThrow1(CAT_SQL_COMP_QR_DESC_GEN, LL_MVQR_FAIL, ((CharType *)type)->getBytesPerChar() == 2,
                               QRDescriptorException, "Unhandled bytes-per-char: %d",
          ...
          break;
        default:
          QRLogger::log(CAT_SQL_COMP_QR_DESC_GEN, LL_MVQR_FAIL, "Unhandled data type: %d (%s)",
                        type->getTypeQualifier(), type->getTypeName().toCharStar());
          scalar = new (mvqrHeap_) QRStringVal(ADD_MEMCHECK_ARGS(mvqrHeap_));
          break;
      }
      if (scalar) {
        if (!valueWasSet) scalar->setValue(constVal->getText());
    ...
    default:
      assertLogAndThrow1(CAT_SQL_COMP_QR_DESC_GEN, LL_MVQR_FAIL, FALSE, QRDescriptorException,
                         "Unhandled ExprElement enum value: %d", itemExpr->getQRExprElem());
      return NULL;
  }
...
// -----------------------------------------------------------------------
QRExprPtr QRDescGenerator::genQRExpr(ItemExpr *pExpr, NABoolean isResidual, UInt32 joinPredId) {
  ...
  if (isDumpMvMode()) {
    ...
    if (unparsedText.first('%') != NA_NPOS || unparsedText.first('&') != NA_NPOS) {
  ...
  if (isNotProvided) {
    ...
    for (ValueId vegrefVid = vegrefsInExpr.init(); vegrefsInExpr.next(vegrefVid); vegrefsInExpr.advance(vegrefVid)) {
...
// -----------------------------------------------------------------------
NABoolean QRDescGenerator::normalizeColumnInExpression(NAString &pExprText, ValueId colvid, short paramIndex) {
...
void QRDescGenerator::markColumnsAsResidual(ValueIdSet &vegrefsInExpr) {
  ...
  for (ValueId vegrefVid = vegrefsInExpr.init(); vegrefsInExpr.next(vegrefVid); vegrefsInExpr.advance(vegrefVid)) {
    ItemExpr *itemExpr = vegrefVid.getItemExpr();
    if (itemExpr->getOperatorType() == ITM_VEG_REFERENCE &&
        static_cast<VEGReference *>(itemExpr)->getVEG()->getAConstant() == NULL_VALUE_ID)
  ...
  for (ValueId colVid = baseColsInExpr.init(); baseColsInExpr.next(colVid); baseColsInExpr.advance(colVid)) {
    setPredBitmap(colVid, ET_ResidualPred);  // mark use of resid pred on col
  }
}

// -----------------------------------------------------------------------
void QRDescGenerator::processResidualPredicate(ValueId predVid, QRJBBPtr jbbElement) {
...
// -----------------------------------------------------------------------
void QRDescGenerator::processGroupBy(RelExpr *groupByNode, CANodeId gbID, QRJBBPtr jbbElement) {
  if (groupByNode->getOperator() != REL_GROUPBY) return;
...
// -----------------------------------------------------------------------
void QRDescGenerator::processOutputList(const ValueIdSet &normOutputs, QRJBBPtr jbbElement) {
  QRTRACER("QRDescGenerator::processOutputList()");
  QROutputListPtr outputListElement = jbbElement->getOutputList();

  const ColumnDescList *mvColList = (isMvMode() ? relExpr_->getRETDesc()->getColumnList() : NULL);
  CollIndex mvNumCols = (mvColList ? mvColList->entries() : 0);
  CollIndex outInx, cdInx;
  ValueId cvid;

  for (cvid = normOutputs.init(), outInx = 0; normOutputs.next(cvid); normOutputs.advance(cvid), outInx++) {
    ...
    if (mvColList) {
      ...
      for (cdInx = 0; cdInx < mvNumCols && !found; cdInx++) {
        ...
        if (!found && cvExprOp == ITM_VEG_REFERENCE) {
          const ValueIdSet &cvVegVids = ((VEGReference *)cvExpr)->getVEG()->getAllValues();
...
static int *getJoinOrderInfo(const CANodeIdSet *const const_jbbcs, size_t &arrSize) {
  ...
  while (jbbcs.entries() > 0) {
    ...
    for (nodeId = jbbcs.init(); jbbcs.next(nodeId); jbbcs.advance(nodeId)) {
      ...
      const CANodeIdSet &predecessors = jbbc->getPredecessorJBBCs();
      for (predNodeId = predecessors.init(); reachable && predecessors.next(predNodeId);
           predecessors.advance(predNodeId)) {
        if (reached.containsThisId(predNodeId)) continue;
    ...
    assertLogAndThrow(CAT_SQL_COMP_QR_DESC_GEN, LL_MVQR_FAIL, foundOne || numSkipped == jbbcs.entries(),
                      QRDescriptorException, "One or more nodes not reachable via required predecessor nodes");
    foundOne = FALSE;
    currGroupOrderNum++;
    jbbcs -= reached;
  }  // while
...
// -----------------------------------------------------------------------
void QRDescGenerator::processJBBCList(CANodeIdSet *jbbcNodeIds, QRJBBPtr jbbElement, ValueIdSet &jbbOutputs,
                                      CANodeId &groupJbbcNodeId) {
  QRTRACER("QRDescGenerator::processJBBCList()");
  QRTablePtr *qrtableArray = 0;
  qrtableArray = (QRTablePtr *)calloc(jbbcNodeIds->entries(), sizeof(QRTablePtr));
  short numTables = 0;
  ...
  for (CANodeId nodeId = jbbcNodeIds->init(); jbbcNodeIds->next(nodeId); jbbcNodeIds->advance(nodeId)) {
    NodeAnalysis *nodeAnalysis = nodeId.getNodeAnalysis();
    ...
    if (tableAnalysis) {
      ...
      if (nodeAnalysis->getJBBC()->parentIsLeftJoin()) tableElement->setLOJParent(TRUE);
      if (isDumpMvMode()) {
      ...
      QRJBBPtr groupJbb = new (mvqrHeap_) QRJBB(genJBBid(), (CollIndex)NULL_CA_ID, ADD_MEMCHECK_ARGS(mvqrHeap_));
      const JBBSubset *jbbSubset = nodeAnalysis->getOriginalExpr()->getGroupAnalysis()->getLocalJBBView();
      if (jbbSubset) {
...
// -----------------------------------------------------------------------
void QRDescGenerator::processKeys(TableAnalysis *tableAnalysis, QRTablePtr tableElement, ValueIdSet &jbbOutputs) {
  ...
  ValueIdSet keyCols(tableAnalysis->getTableDesc()->getClusteringIndex()->getClusteringKeyCols());
  QRKeyPtr keyElement = NULL;

  if (isQueryMode()) keyElement = new (mvqrHeap_) QRKey(ADD_MEMCHECK_ARGS(mvqrHeap_));

  // check if the key columns are covered by the jbb outputs
  NABoolean keyCovered = TRUE;

  for (ValueId vid = keyCols.init(); keyCols.next(vid); keyCols.advance(vid)) {
    if (isQueryMode()) {
      QRElementPtr keyColumn = genQRColumn(vid);
      keyElement->addElement(keyColumn);
    }

    if (!jbbOutputs.containsTheGivenValue(vid)) keyCovered = FALSE;
  }

  if (isQueryMode()) tableElement->setKey(keyElement);

  if (keyCovered && (!isQueryMode())) tableElement->setIsKeyCovered(TRUE);
}
...
void QRDescGenerator::getSingleTableJBBs(QRDescriptorPtr desc, RelExpr *expr) {
  ...
  for (int i = 0; i < arity; i++) {
    ...
    while (op.match(REL_GROUPBY) && static_cast<GroupByAgg *>(subtree)->getGBAnalysis() == NULL) {
      subtree = subtree->child(0).getPtr();
      op = subtree->getOperator();
    }

    if (op.match(REL_SCAN) && (desc->getElementType() == ET_MVDescriptor ||  // always for MV desc
                               qrNeededForTable(expr, subtree))) {
      scanExpr = static_cast<Scan *>(subtree);
      nodeAnalysis = scanExpr->getGroupAnalysis()->getNodeAnalysis();
      QRJBBPtr jbb =
          new (mvqrHeap_) QRJBB(genJBBid(), (CollIndex)(nodeAnalysis->getId()), ADD_MEMCHECK_ARGS(mvqrHeap_));
      desc->addJBB(jbb);
      QRTablePtr tableElement = new (mvqrHeap_) QRTable(ADD_MEMCHECK_ARGS(mvqrHeap_));
      ...
      if (isDumpMvMode()) {
        ...
        const NAString &corrName =
            strchr(exposedName, '.') ? scanExpr->getTableDesc()->getCorrNameObj().getQualifiedNameObj().getObjectName()
                                     : exposedName;
        tableElement->setCorrelationName(corrName);
      }

      ValueIdSet jbbOutputs = nodeAnalysis->getOriginalExpr()->getGroupAttr()->getCharacteristicOutputs();
      ...
      if (expr->getOperator() == REL_GROUPBY) processGroupBy(expr, 0, jbb);
      ...
      CANodeId id = updExpr->getTableDesc()->getTableAnalysis()->getNodeAnalysis()->getId();
...
QRJBBPtr QRDescGenerator::createJbb(JBB *jbb) {
  ...
  QRJBBPtr jbbElement = new (mvqrHeap_) QRJBB(jbb->getJBBId(), jbb, ADD_MEMCHECK_ARGS(mvqrHeap_));
  ...
  if (jbb->getGBAnalysis())
    ...
    jbbOutputs =
        jbb->getJBBCs().getFirst().getNodeAnalysis()->getOriginalExpr()->getGroupAttr()->getCharacteristicOutputs();
  ...
  if (gbAnalysis != NULL) {
    processGroupBy(gbAnalysis->getOriginalGBExpr(), gbAnalysis->getGroupingNodeId(), jbbElement);
    ...
    assertLogAndThrow1(CAT_SQL_COMP_QR_DESC_GEN, LL_ERROR, relExpr->getOperatorType() == REL_GROUPBY,
                       QRDescriptorException, "Expected grouping JBBC, but op type was %d.",
                       relExpr->getOperatorType());
    processGroupBy(static_cast<GroupByAgg *>(relExpr), groupJbbcNodeId, jbbElement);
...
NABoolean QRDescGenerator::hasRewriteEnabledMVs(TableDesc *tableDesc) {
  ...
  for (CollIndex i = 0; i < mvList.entries(); i++) {
    if (mvList[i]->isRewriteEnabled()) return TRUE;
  }
...
NABoolean QRDescGenerator::qrNeededForTable(RelExpr *parent, RelExpr *scanExpr) {
  ...
  if (putAllJBBsInQD_) return TRUE;
...
static NABoolean noSemiOrTSJ(JBBSubsetAnalysis *jbbSubsetAnalysis) {
  ...
  for (node = jbbcs.init(); jbbcs.next(node); jbbcs.advance(node)) {
    ...
    if (parentJoin) {
      ...
      if (op.match(REL_ANY_SEMIJOIN) || op.match(REL_ANY_TSJ)) return FALSE;
    }
  }
...
NABoolean QRDescGenerator::processJBBs(QRDescriptorPtr descPtr, QueryAnalysis *qa) {
  ...
  if (isDumpMvMode()) {
    ...
    assertLogAndThrow(CAT_SQL_COMP_QR_DESC_GEN, LL_MVQR_FAIL, allTables.entries() > 1, QRDescriptorException,
  ...
  if (jbbElemsWereCreated && !putAllJBBsInQD_)
    for (i = 0; !usesEnabledMVs && i < qrJbbList.entries(); i++) {
      ...
      NodeAnalysis *nodeAnalysis = static_cast<CANodeId>(qrJbbList[i]->getNodeId()).getNodeAnalysis();
      if (!nodeAnalysis->isExtraHub() && hasRewriteEnabledMVs(nodeAnalysis->getTableAnalysis()->getTableDesc())) {
  ...
  for (i = 0; remainingJBBs > 0; i++) {
    if (jbbArray.used(i)) {
      ...
      JBBSubsetAnalysis *jbbSubsetAnalysis = currentJBB_->getMainJBBSubset().getJBBSubsetAnalysis();
      if (noSemiOrTSJ(jbbSubsetAnalysis)) {
        ...
        QRLogger::log(CAT_SQL_COMP_QR_DESC_GEN, LL_DEBUG,
                      ...
                      "semijoins, anti-semijoins, or TSJs",
  ...
  for (CollIndex qrJbbInx = 0; qrJbbInx < qrJbbList.entries(); qrJbbInx++) {
    ...
    if (currentJBB_) {
      ...
      processReferencingPreds(&jbbcs, qrJbbPtr->getGbExpr(), qrJbbPtr);
    } else {
      CANodeId nodeId = (CANodeId)(qrJbbPtr->getNodeId());
      assertLogAndThrow1(CAT_SQL_COMP_QR_DESC_GEN, LL_MVQR_FAIL, nodeId != NULL_CA_ID, QRDescriptorException,
                         "QRJBB at position %d in list had neither a JBB* nor a node id", qrJbbInx);
      CANodeIdSet nodes;
      nodes.addElement(nodeId);
      RelExpr *expr = nodeId.getNodeAnalysis()->getOriginalExpr();
      processReferencingPreds(&nodes, expr->getOperator() == REL_GROUPBY ? expr : NULL, qrJbbPtr);
    }
  }
  ...
  if (jbbElemsWereCreated) addRangePredicates();
...
void QRDescGenerator::logColumnBitmap(QRTablePtr table, const XMLBitmap &bitmap, ElementType predType) {
  // Exit immediately if logging is not in DEBUG level.
  if (!QRLogger::isCategoryInDebug(CAT_SQL_COMP_QR_DESC_GEN)) return;

  QRTRACER("QRDescGenerator::logColumnBitmap()");
  // Log which bitmap this is.
  if (predType == ET_RangePred) {
    QRLogger::log(CAT_SQL_COMP_QR_DESC_GEN, LL_DEBUG,
                  "For table %s, the following columns have range predicates:", table->getTableName().data());
  } else if (predType == ET_ResidualPred) {
    QRLogger::log(CAT_SQL_COMP_QR_DESC_GEN, LL_DEBUG,
                  "For table %s, the following columns have residual predicates:", table->getTableName().data());
  } else {
    QRLogger::log(CAT_SQL_COMP_QR_DESC_GEN, LL_DEBUG,
                  "Table %s has these columns set in bitmap of element type %d:", table->getTableName().data(),
                  predType);
  }
  ...
  const NAColumnArray &colsInTable =
      tblNodeId.getNodeAnalysis()->getTableAnalysis()->getTableDesc()->getNATable()->getNAColumnArray();
  assertLogAndThrow2(CAT_SQL_COMP_QR_DESC_GEN, LL_MVQR_FAIL, table->getNumCols() == colsInTable.entries(),
                     QRDescriptorException, "logColumnBitmap() expected %d columns, but found %d in list",
                     table->getNumCols(), colsInTable.entries());

  for (CollIndex i = 0; i < colsInTable.entries(); i++) {
    if (bitmap.testBit(i)) {
      QRLogger::log(CAT_SQL_COMP_QR_DESC_GEN, LL_DEBUG, "    %s", colsInTable[i]->getColName().data());
    }
  }
}  // logColumnBitmap()

QRQueryDescriptorPtr QRDescGenerator::createQueryDescriptor(QueryAnalysis *qa, RelExpr *expr) {
...
void QRDescGenerator::addJoinPred(QRJBBPtr jbbElem, QRElementPtr *qrElemArray, int *idArray, int eqCount,
  ...
  if (bSortJoinPredicateCols_) opt_qsort(qrElemArray, eqCount, sizeof(QRElementPtr), QRElement::cmpQRElement);
    ...
    if (qrElemArray[i]->isExtraHub()) {
      if (!extraHubJoinPred) {
        ...
        if (eqSet && eqSet->getJoinPredId())
          extraHubJoinPred->setAndRegisterID(eqSet->getJoinPredId(), colTblIdHash_);
        else
          extraHubJoinPred->setAndRegisterID(idArray[i], colTblIdHash_);
      }
      extraHubJoinPred->addElement(qrElemArray[i]);
      colToJoinPredMap_.addMapEntry(idArray[i], qrElemArray[i]->getReferencedElement()->getIDNum());
    } else {
      if (!hubJoinPred) {
        // hubJoinPredId will have been set to the value id of a veg pred
        // if the equality set was derived from one.
        if (hubJoinPredId == NULL_VALUE_ID) hubJoinPredId = idArray[i];  // returned to caller
        hubJoinPred = new (mvqrHeap_) QRJoinPred(ADD_MEMCHECK_ARGS(mvqrHeap_));
        hubJoinPred->setAndRegisterID(hubJoinPredId, colTblIdHash_);
      }
      hubJoinPred->addElement(qrElemArray[i]);
      colToJoinPredMap_.addMapEntry(hubJoinPredId, qrElemArray[i]->getReferencedElement()->getIDNum());
    }
  }
  ...
  if (hubJoinPred == NULL) {
    ...
    assertLogAndThrow(CAT_SQL_COMP_QR_DESC_GEN, LL_MVQR_FAIL, extraHubJoinPred, QRDescriptorException,
    ...
    if (extraHubJoinPred) {
      ...
      QRJoinPredPtr hubJoinPredRef = new (mvqrHeap_) QRJoinPred(ADD_MEMCHECK_ARGS(mvqrHeap_));
...
CANodeId QRDescGenerator::getExprNode(ItemExpr *itemExpr) {
  ...
  for (CollIndex i = 0; i < vids.entries(); i++) {
    ...
    switch (leafItemExpr->getOperatorType()) {
      case ITM_VEG_REFERENCE:
        ...
        if (static_cast<VEGReference *>(leafItemExpr)->getVEG()->getAConstant(TRUE) == NULL_VALUE_ID) inputCount++;
        break;
  ...
  if (exprNodeId)
    return (isMultiNode || inputCount != 1) ? NULL_CA_ID : exprNodeId;
    ...
    if (!currentJBB_)
      ...
      assertLogAndThrow1(CAT_SQL_COMP_QR_DESC_GEN, LL_MVQR_FAIL, currentJBB_->getGBAnalysis(), QRDescriptorException,
                         "No GBAnalysis for JBB %d, although an aggfn was used", currentJBB_->getJBBId());
...
void QRDescGenerator::processEqualitySet(QRJBBPtr jbbElem, EqualitySet &eqSet) {
  ...
  for (i = 0; i < eqCount; i++) {
    ...
    if (op == ITM_BASECOLUMN) {
      const TableAnalysis *tableAnalysis = ((BaseColumn *)itemExpr)->getTableDesc()->getTableAnalysis();
      assertLogAndThrow(CAT_SQL_COMP_QR_DESC_GEN, LL_MVQR_FAIL, tableAnalysis, QRDescriptorException,
        ...
        if (joinPredElemCount == 0)
          ...
          if (joinPredElemCount == 1) {
            if (isInstNull(firstJoinPredItem))
              ...
              qrElemArray[0] = genQRColumn(firstJoinPredItem->getValueId());
            else
              qrElemArray[0] = genQRExpr(firstJoinPredItem, FALSE);
            idArray[0] = firstJoinPredItem->getValueId();
          }
          qrElemArray[joinPredElemCount] = genQRColumn(itemExpr->getValueId());
      ...
      // the parameter.
      assertLogAndThrow(CAT_SQL_COMP_QR_DESC_GEN, LL_MVQR_FAIL, !constItem, QRDescriptorException,
      ...
      if (exprNodeId == NULL_CA_ID || joinPredNodes.containsThisId(exprNodeId) || itemExpr->containsAnAggregate()) {
        if (exprNodeId == NULL_CA_ID) rangeIneligible += nonJoinPredOperands.entries();
        nonJoinPredOperands.insert(itemExpr);
      } else {
  ...
  if (useConstItem) {
    for (CollIndex i = 0; i < nonJoinPredOperands.entries(); i++) {
      if (rangeIneligible.testBit(i))
        ...
        addEqualityRangePred(nonJoinPredOperands[i], eqSet.getType(), jbbElem, constItem);
    }

    if (hubJoinPredId) addEqualityRangePred(hubJoinPredId, eqSet.getType(), jbbElem, constItem);
  } else if (hubJoinPredId) {
    // Create residual preds that link each nonjoin operand to the hub join pred.
    for (CollIndex i = 0; i < nonJoinPredOperands.entries(); i++)
      addEqualityResidPred(jbbElem, nonJoinPredOperands[i], NULL, hubJoinPredId);
  } else {
    ...
    for (CollIndex i = 0; i < nonJoinPredOperands.entries() - 1; i++)
      addEqualityResidPred(jbbElem, nonJoinPredOperands[i], nonJoinPredOperands[i + 1]);
  }
...
VEGPredicate *QRDescGenerator::getVegPredicate(UInt32 hubJoinPredId) {
  ...
  assertLogAndThrow1(CAT_SQL_COMP_QR_DESC_GEN, LL_MVQR_FAIL, ie->getOperatorType() == ITM_VEG_REFERENCE,
                     QRDescriptorException, "Hub join pred id is not ValueId of vegref -- %d", hubJoinPredId);
  return (static_cast<VEGReference *>(ie))->getVEG()->getVEGPredicate();
}

void QRDescGenerator::addEqualityRangePred(ItemExpr *rangeItemExpr, const NAType *type, QRJBBPtr jbbElem,
...
void QRDescGenerator::addEqualityRangePred(UInt32 hubJoinPredId, const NAType *type, QRJBBPtr jbbElem,
                                           ConstValue *constItem) {
  QRTRACER("QRDescGenerator::addEqualityRangePred()");
  OptRangeSpec *range = new (mvqrHeap_) OptRangeSpec(this, mvqrHeap_);
...
void QRDescGenerator::addEqualityResidPred(QRJBBPtr jbbElem, ItemExpr *op1, ItemExpr *op2, ValueId hubJoinPredId) {
  QRTRACER("QRDescGenerator::addEqualityResidPred()");
  ItemExpr *residExpr;
  assertLogAndThrow(CAT_SQL_COMP_QR_DESC_GEN, LL_MVQR_FAIL, op1, QRDescriptorException,
                    "op1 of equality resid pred is null");
  if (op2)
    residExpr = new (mvqrHeap_) BiRelat(ITM_EQUAL, op1, op2);
  else {
    assertLogAndThrow(CAT_SQL_COMP_QR_DESC_GEN, LL_MVQR_FAIL, hubJoinPredId, QRDescriptorException,
                      "Neither op2 nor hubJoinPredId is given");
    residExpr = new (mvqrHeap_) BiRelat(ITM_EQUAL, op1, hubJoinPredId.getItemExpr());
  }

  assertLogAndThrow(CAT_SQL_COMP_QR_DESC_GEN, LL_MVQR_FAIL, relExpr_, QRDescriptorException,
...
void QRDescGenerator::processReferencingPreds(CANodeIdSet *nodeSet, RelExpr *gbNode, QRJBBPtr jbbElem) {
  ...
  for (CANodeId node = nodeSet->init(); nodeSet->next(node); nodeSet->advance(node)) {
    nodeAnalysis = node.getNodeAnalysis();
    assertLogAndThrow1(CAT_SQL_COMP_QR_DESC_GEN, LL_MVQR_FAIL, nodeAnalysis, QRDescriptorException,
  ...
  if (gbNode) preds.addSet(gbNode->getSelectionPredicates());
  ...
  for (CollIndex i = 0; i < equalitySets.entries(); i++) processEqualitySet(jbbElem, *equalitySets[i]);
  ...
  for (ValueId predVid = preds.init(); preds.next(predVid); preds.advance(predVid)) {
    OptRangeSpec *range = OptRangeSpec::createRangeSpec(this, predVid.getItemExpr(), mvqrHeap_);
    if (range)
      storeRangeInfo(range, jbbElem);
  ...
  for (CollIndex i = 0; i < equalitySets.entries(); i++) allEqualitySets_.insert(equalitySets[i]);
}
...
void QRDescGenerator::discardEqualitySets(NAList<EqualitySet *> &equalitySets) {
  ...
  NAHashDictionaryIterator<const NAString, EqualitySet> exprsHashIter(exprsUsedHash_);
...
void QRDescGenerator::combineEqSets(EqualitySet *source, EqualitySet *destination,
  ...
  for (i = 0; i < vegsHashIter.entries(); i++) {
    ...
    if (ptrListValue == source) vidsToRemove.insert(vidKey);
  }
  ...
  for (i = 0; i < exprsHashIter.entries(); i++) {
    ...
    if (ptrListValue == source) exprsToRemove.insert(exprKey);
  }
...
void QRDescGenerator::putVegMembersInEqualitySet(ItemExpr *vegPred,  // NULL if called for equality pred
                                                 const ValueId &vegVid, const ValueIdSet &vegVals,
  ...
  // be used.
  if (vegPred) eqSet->setJoinPredId(((VEGPredicate *)vegPred)->getVEG()->getVEGReference()->getValueId());
...
void QRDescGenerator::addVegPredToEqualitySets(VEGPredicate *vegPred, NAList<EqualitySet *> &equalitySets) {
  ...
  for (ValueId vid = vegVals.init(); vegVals.next(vid); vegVals.advance(vid)) {
    ...
    if (eqSet) {
      ...
      if (targetEqSet) {
        if (eqSet != targetEqSet) combineEqSets(eqSet, targetEqSet, equalitySets);
      } else
        targetEqSet = eqSet;
    }
  }
  ...
  putVegMembersInEqualitySet(vegPred, vegPred->getVEG()->getValueId(), vegVals, targetEqSet, equalitySets);
}  // addVegPredToEqualitySets()

void QRDescGenerator::addEqPredToEqualitySets(ItemExpr *pred, NAList<EqualitySet *> &equalitySets) {
  ...
  for (int i = 0; i < 2; i++) {
    ...
    if (op == ITM_VEG_REFERENCE) {
      ...
      putVegMembersInEqualitySet(NULL, veg->getValueId(), veg->getAllValues(), targetEqSet, equalitySets);
    } else {
      if (!targetEqSet) {
...
void QRDescGenerator::setPredBitmap(QRValueId colVid, ElementType elemType) {
  ...
  if (!elem) {
    ...
    if (opType == ITM_BASECOLUMN)
      ...
      ValueIdSet vegMembers = static_cast<VEGReference *>(vid.getItemExpr())->getVEG()->getAllValues();
      for (ValueId id = vegMembers.init(); vegMembers.next(id); vegMembers.advance(id)) {
        if (id.getItemExpr()->getOperatorType() == ITM_BASECOLUMN) setPredBitmap(id, elemType);
      }
      return;
    } else {
      int vidInt = vid;
  ...
  assertLogAndThrow1(CAT_SQL_COMP_QR_DESC_GEN, LL_MVQR_FAIL, elem, QRDescriptorException,
  ...
  assertLogAndThrow1(CAT_SQL_COMP_QR_DESC_GEN, LL_MVQR_FAIL, elem->getElementType() == ET_Table, QRDescriptorException,
                     "Expected a Table element type, not %d", elem->getElementType());
  ...
  if (elemType == ET_RangePred)
    ...
    assertLogAndThrow1(CAT_SQL_COMP_QR_DESC_GEN, LL_MVQR_FAIL, FALSE, QRDescriptorException,
                       "Wrong element type sent to setPredBitmap() -- %d", elemType);
}
...
void QRDescGenerator::addRangePredicates() {
  ...
  for (CollIndex i = 0; i < vidIter.entries(); i++) {
    ...
    if (rangeSpec->isFalse()) {
      ...
      if (rangePred) {
        if ((isQueryMode() || isDumpMvMode()) && !isRangeSupported(rangePred)) {
          ...
          QRRangePredPtr notProvidedRangePred = new (mvqrHeap_) QRRangePred(ADD_MEMCHECK_ARGS(mvqrHeap_));
  ...
  for (CollIndex i = 0; i < exprIter.entries(); i++) {
    ...
    if (rangePred) rangeInfo->getOwningList()->addItemOrdered(rangePred);
  }
