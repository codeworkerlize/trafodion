...
public :
  ...
  NABoolean getIsPartitioned() const
  {
...
public:
  ...
  NABoolean getConcurrentHivePartitionWrites() const
                                     { return concurrentHivePartitionWrites_; }

private:

  // indicates whether this FastExtract has the potential to write to
  // the same partition with multiple ESPs. This field is set in
  // preCodeGen, it is not available before that phase. It is set if
  // a) this is a Hive insert and b) the target table is partitioned
  // and c) we are using a parallel plan and d) we are not
  // repartitioning the data on the Hive partition columns.
  NABoolean concurrentHivePartitionWrites_;
  CostMethodFastExtract *pCostMethod_;
}; // class PhysicalFastExtract
