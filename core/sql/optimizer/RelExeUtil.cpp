...
RelExpr * DDLExpr::copyTopNode(RelExpr *derivedNode, CollHeap* outHeap)
{
  ...
  result->nodeList_ = (nodeList_) ? 
              new (heap_) ConstStringList(*nodeList_, heap_) : NULL;
...
ComTdbRoot::DDLSafenessType DDLExpr::getDDLSafeness()
{
  ...
  if (isCreate_) {
     ...
     if (getExprNode() != NULL) {
        ddlStmtNode = getExprNode()->castToStmtDDLNode();
        if (ddlStmtNode->castToStmtDDLPopulateIndex() != NULL)
           return ComTdbRoot::SQL_UNSAFE_DDL;
/*
...
RelExpr *RelDumpLoad::bindNode(BindWA* bindWA)
{
  ...
  if (dlObjType_ != DL_OBJ_SCHEMA)
      ...
      objName_.setCatalogName(bindWA->getDefaultSchema()
          .getCatalogName());
...
void RelDumpLoad::unparse(NAString &result
                          , PhaseEnum /*phase*/
                          , UnparseFormatEnum /* form */
                          , TableDesc *tabId) const
{
...
void RelGenLoadQueryCache::unparse(NAString& result
  , PhaseEnum /*phase*/
  , UnparseFormatEnum /* form */
  , TableDesc* tabId) const
{
...
RelExpr * RelBackupRestore::bindNode(BindWA *bindWA)
{
  ...
  if ((NOT getAllBackupSnapshots()) && (NOT unlockTraf()) && (NOT dropAllBackups()) &&
      (NOT exportAllBackups()) && (NOT importAllBackups()) && (NOT restoreToTS()) &&
      (NOT dropBackupMD()) && (NOT getBackupMD()) && 
      (NOT getAllBackupTags()) && (NOT getLockedObjects()) && 
      (NOT getCreateTags()) && (NOT getProgressStatus()) &&
      ...
      if (backupTag().isNull())
        {
          *CmpCommon::diags() << DgSqlCode(-5050)
                              << DgString0(cmd.data())
                              << DgString1("Reason: Tag must be specified.");
  ...
  if (NOT backupTag().isNull())
    {
      ...
      if ((NOT cleanupObjects()) &&
          (backupTag().length() > tagMaxLen))
        {
          char buf[100];
          str_sprintf(buf, "Reason: Tag cannot exceed %d bytes in length.",
                      tagMaxLen);
          *CmpCommon::diags() << DgSqlCode(-5050)
                              << DgString0(cmd.data())
                              << DgString1(buf);
  ...
  if (restoreToTS())
    {
      ...
      DatetimeValue dtValue(getTimestampVal().data(), 
  ...
  if ((NOT (backup() || restore() || exportBackup() || importBackup())) && 
      (getOverride()))
    {
      *CmpCommon::diags() << DgSqlCode(-5050)
                          << DgString0(cmd.data())
  ...
  if ((getCascade() || getForce() || getSkipLock()) &&
      (NOT (dropBackup() || dropAllBackups() || getOverride())))
    {
      *CmpCommon::diags() << DgSqlCode(-5050)
                          << DgString0(cmd.data())
  ...
  if ((getNoHeader()) &&
      (NOT (getAllBackupSnapshots() || getAllBackupTags())))
    {
      *CmpCommon::diags() << DgSqlCode(-5050)
                          << DgString0(cmd.data())
  ...
  if ((NOT backup()) && (returnStatus() || returnTag()))
    {
      *CmpCommon::diags() << DgSqlCode(-5050)
                          ...
                          << DgString1(
                               (returnStatus() ?
                                "Reason: RETURN STATUS is not supported." :
  ...
  if ((returnStatus()) || (getBackupSnapshot()) || (getAllBackupSnapshots())
      || (showObjects()) || (getBackupMD()) || (getBackupTag()) 
      || (getAllBackupTags()) || (getLockedObjects()) || (getVersionOfBackup())
      || (returnTag()) || (getProgressStatus()))
    {
      if ((returnTag()) &&
          (returnStatus() || showObjects()))
        {
          *CmpCommon::diags() << DgSqlCode(-5050)
                              << DgString0(cmd.data())
                              << DgString1("Reason: RETURN TAG cannot be specified with 'show objects' or 'return status'.");
          bindWA->setErrStatus();
          return NULL;               
        }
  ...
  if ((getProgressStatus()) &&
      (NOT ((getBREIoper() == "BACKUP") || (getBREIoper() == "EXPORT") ||
  ...
  if (NOT (backupSystem() || restoreSystem() || getBackupSnapshot()  || getAllBackupSnapshots() || 
           getBackupTag() || getAllBackupTags() || dropBackup() || unlockTraf() || exportBackup() || 
           importBackup() || getDropProgressTable() || getTruncateProgressTable()))
    {
...
void
RelBackupRestore::unparse(NAString &result,
                          PhaseEnum /* phase */,
                          UnparseFormatEnum /* form */,
                          TableDesc * tabId) const
{
					   ...
					   CollHeap* outHeap)
{
  ...
  if (derivedNode == NULL)
    result = new (outHeap) ExeUtilGetErrorInfo(errType_, errNum_,
					       outHeap);
  else
...
RelExpr * ExeUtilHiveQuery::bindNode(BindWA *bindWA)
{
  ...
  if (NOT ((hiveQuery_.index("CREATE ", 0, NAString::ignoreCase) == 0) ||
           ...
           (hiveQuery_.index("MSCK", 0, NAString::ignoreCase) == 0) ||
...
// -----------------------------------------------------------------------
ExeUtilAvroStats::ExeUtilAvroStats(const CorrName &objectName,
                                   NABoolean displayFormat,
                                   CollHeap *oHeap)
     : ExeUtilExpr(AVRO_STATS_, objectName,
		   NULL, NULL, NULL, CharInfo::UnknownCharSet, oHeap),
       errorInParams_(FALSE),
       displayFormat_(displayFormat)
{
...
RelExpr * ExeUtilAvroStats::copyTopNode(RelExpr *derivedNode, CollHeap* outHeap)
{
  ExeUtilAvroStats *result;

  if (derivedNode == NULL)
    result = new (outHeap) ExeUtilAvroStats(getTableName(),
                                            displayFormat_,
                                            outHeap);
  else
    result = (ExeUtilAvroStats *) derivedNode;
...
// member functions for class ExeUtilAvroStats
// -----------------------------------------------------------------------
RelExpr * ExeUtilAvroStats::bindNode(BindWA *bindWA)
{
  if (errorInParams_)
    {
      *CmpCommon::diags() << DgSqlCode(-4218) << DgString0("GET ");
  ...
  if (getTableName().getQualifiedNameObj().getObjectName().isNull())
    {
      *CmpCommon::diags() << DgSqlCode(-4218) << DgString0("AVRO STATS");
  ...
  if (NOT naTable->isAvro())
    {
     *CmpCommon::diags() << DgSqlCode(-4219) << DgString0(" Reason: Table must be created in avro format.");
...
// -----------------------------------------------------------------------
// Member functions for class ExeUtilGetExtSchema
// -----------------------------------------------------------------------
ExeUtilGetExtSchema::ExeUtilGetExtSchema(const CorrName &objectName,
                                         ExtStorageType type,
                                         NABoolean forRead,
                                         NABoolean forWrite,
                                         CollHeap *oHeap)
...
RelExpr * ExeUtilGetExtSchema::copyTopNode(RelExpr *derivedNode, CollHeap* outHeap)
{
  ExeUtilGetExtSchema *result;

  if (derivedNode == NULL)
    result = new (outHeap) ExeUtilGetExtSchema(getTableName(),
                                               type_,
                                               forRead_,
                                               forWrite_,
                                               outHeap);
  else
    result = (ExeUtilGetExtSchema *) derivedNode;
...
// member functions for class ExeUtilGetExtSchema
// -----------------------------------------------------------------------
RelExpr * ExeUtilGetExtSchema::bindNode(BindWA *bindWA)
{
  if (nodeIsBound()) {
    bindWA->getCurrentScope()->setRETDesc(getRETDesc());
    return this;
  ...
  if (getTableName().getQualifiedNameObj().getObjectName().isNull())
    {
      *CmpCommon::diags() << DgSqlCode(-4218) << DgString0("GET SCHEMA");
  ...
  if ((type_ == PARQUET) && (NOT naTable->isParquet()))
    {
     *CmpCommon::diags() << DgSqlCode(-4219) << DgString0(" Reason: Table must be created in parquet format.");
  ...
  if ((type_ == AVRO) && (NOT naTable->isAvro()))
    {
     *CmpCommon::diags() << DgSqlCode(-4219) << DgString0(" Reason: Table must be created in avro format.");
...
RelExpr * ExeUtilExpr::bindNode(BindWA *bindWA)
{
  ...
  if ((NOT tableName_.isEmpty()) &&
      tableName_.getSpecialType()==ExtendedQualName::NORMAL_TABLE)
...
RelExpr * DDLExpr::bindNode(BindWA *bindWA)
{
  ...
  else if (initAuth() || dropAuth() || cleanupAuth())
  {
    ...
    hbaseDDLNoUserXn_ = TRUE;
           ...
           addSchemaObjects() || updateVersion() || updateMDIndexes())
  {
    isHbase_ = TRUE;
    hbaseDDLNoUserXn_ = TRUE;
  }
  ...
  else if (purgedata())
  {
    ...
    if (CmpCommon::getDefault(ALLOW_TRUNCATE_IN_USER_XN) == DF_OFF)
      hbaseDDLNoUserXn_ = TRUE;
  }
  ...
  else if (loadTrafMetadataIntoSharedCache())
  {
    if ( !(bindWA->currentCmpContext()->isExecuteInESP()) &&
         CmpCommon::getDefault(
           TRAF_ENABLE_METADATA_LOAD_IN_SHARED_CACHE) == DF_OFF ) 
      {
    ...
    if ( nodeList_ ) {
       ...
       const NAString* badName = 
          CURRCONTEXT_CLUSTERINFO->
             NAClusterInfo::mapNodeNamesToNodeNums(nodeList_, nodeIdArray_);

       if ( badName ) {
         *CmpCommon::diags() << DgSqlCode(-1236) 
                             << DgString0(badName->data()); 
         bindWA->setErrStatus();
         return NULL;
       }
    }
  ...
  else if (loadTrafDataIntoSharedCache())
  {
    ...
    if (nodeList_)
    {
      ...
      const NAString *badName =
          CURRCONTEXT_CLUSTERINFO->NAClusterInfo::mapNodeNamesToNodeNums(nodeList_, nodeIdArray_);
  ...
  else if (getExprNode() && getExprNode()->castToStmtDDLNode())
  {
    ...
    else if (getExprNode()->castToStmtDDLNode()->castToStmtDDLDropTable())
    {
      ...
      // drop of hive table is allowed if cqd hive_ddl is set.
      if (ComUser::isRootUserID() &&
          (CmpCommon::getDefault(HIVE_DDL) == DF_ON))
        {
          if ((qualObjName_.isHive()) ||
              (bindWA->getDefaultSchema().getCatalogName() == HIVE_SYSTEM_CATALOG))
            {
              qualObjName_.applyDefaults(bindWA->getDefaultSchema());
              NAString dropStmt("drop table ");
              if (dropTableNode->dropIfExists())
                dropStmt += " if exists ";
              if (qualObjName_.getSchemaName() != HIVE_SYSTEM_SCHEMA)
                dropStmt += qualObjName_.getSchemaName() + ".";
              dropStmt += qualObjName_.getObjectName();
              
              RelExpr * hiveDrop = new (PARSERHEAP()) 
                ExeUtilHiveQuery(dropStmt, ExeUtilHiveQuery::FROM_STRING,
                                 bindWA->wHeap());
              
              RelExpr * boundExpr = hiveDrop->bindNode(bindWA);
              if (bindWA->errStatus()) 
                return NULL;
      ...
      if (getExprNode()->castToStmtDDLNode()->castToStmtDDLAlterTableAddColumn())
      ...
      else if (getExprNode()->castToStmtDDLNode()->castToStmtDDLAlterTableAttribute())
        {
          ...
          if ((fileAttrs.isXnReplSpecified()) ||
              (fileAttrs.isIncrBackupSpecified()) ||
              (fileAttrs.isReadOnlySpecified()))
            alterAttr = TRUE;
    ...
    else if (getExprNode()->castToStmtDDLNode()->castToStmtDDLDropRoutine())
    {
    ...
    else if (getExprNode()->castToStmtDDLNode()->castToStmtDDLCreatePackage())
      {
        ...
        isPackage_ = TRUE;
        qualObjName_ = getExprNode()->castToStmtDDLNode()->
          castToStmtDDLCreatePackage()->getPackageNameAsQualifiedName();
      }
    else if (getExprNode()->castToStmtDDLNode()->castToStmtDDLDropPackage())
      {
        ...
        isPackage_ = TRUE;
        qualObjName_ = getExprNode()->castToStmtDDLNode()->
          castToStmtDDLDropPackage()->getPackageNameAsQualifiedName();
      }
	...
	objName_ = getExprNode()->castToStmtDDLNode()->
	  castToStmtDDLCreateTrigger()->getTriggerName();
	qualObjName_ = getExprNode()->castToStmtDDLNode()->
	...
	objName_ = getExprNode()->castToStmtDDLNode()->
	  castToStmtDDLDropTrigger()->getTriggerName();
	qualObjName_ = getExprNode()->castToStmtDDLNode()->
    ...
    {
      ascNode = 
         getExprNode()->castToStmtDDLNode()->castToStmtDDLAlterSharedCache();
    ...
    if ((isCreateSchema || isDropSchema|| isAlterSchemaHDFSCache || isAlterSchema) ||  isRegister || isCommentOn || alterSharedCache ||
	  ...
	  isCreateTriggers || isDropTriggers) &&
         (isCreate_ || isDrop_ || purgedata() ||
          (isAlter_ && (alterAddCol || alterDropCol || alterDisableIndex || alterEnableIndex ||
                        alterAddConstr || alterDropConstr || alterRenameTable || alterSplitPartition ||
                        alterStoredDesc || alterAddPartition || alterDropPartition ||
                        alterMergePartition || alterExchangePartition ||
                        alterMountPartition || alterUnmountPartition ||
                        alterIdentityCol || alterColDatatype || alterColRename ||
                        alterHdfsCache || alterLibrary || alterRenamePartition ||
                        alterHBaseOptions || alterAttr || resetDDLLock || alterTruncatePartition || otherAlters)))))
      {
	if ((NOT isNative_) &&
            (NOT isVolatile))
	  {
      ...
      if ((qualObjName_.isSeabase()) ||
                (qualObjName_.getCatalogName() == HBASE_SYSTEM_CATALOG))
	      {
		isHbase_ = TRUE;
	      }
...
{
   if ( loadTrafMetadataIntoSharedCache() || loadTrafDataIntoSharedCache())
      return TRUE;
   ...
   if ( ddlNode ) {
     ...
     if ( alterSharedCacheNode )
        return alterSharedCacheNode->isUpdateOp();
   }
...
NABoolean DDLExpr::isTrafSharedCacheParallelOp() 
{
  ...
  {
     StmtDDLAlterSharedCache* ascNode =  
         getExprNode()->castToStmtDDLNode()
                       ->castToStmtDDLAlterSharedCache();
...
RelExpr * ExeUtilCreateTableAs::bindNode(BindWA *bindWA)
{
  ...
  withEndQueryPos = 
    createTableNode->getEndOfCreateTableAsWithClausePosition();
          ...
          for (CollIndex i = 0; i < retDesc->getDegree(); i++)
            {
              ...
              if (colType.isComposite())
                {
                  *CmpCommon::diags() << DgSqlCode(-3242) << 
                    DgString0("Hive CTAS is not allowed on composite columns.");
  ...
  if ((isHive) &&
      (createTableNode->getHiveOptions().isNull()))
    {
  ...
  {
    NAString withClause(&stmtText[asQueryPos], withEndQueryPos - asQueryPos, 
			STMTHEAP);
    viQuery_ += withClause;
    viQuery_ += " " ;
...
RelExpr * ExeUtilHiveTruncate::bindNode(BindWA *bindWA)
{
  ...
  hiveTableName_ = ComConvertTrafHiveNameToNativeHiveName
  ...
  NABoolean isPartnTruncate = FALSE;

  if (hiveTruncQuery_.isNull())
    {
      if (partColValList_ && dropPartIfExists_)
        {
          hiveTruncQuery_ = "alter table ";
          hiveTruncQuery_ += hiveTableName_;
          hiveTruncQuery_ += " drop if exists ";
        }
      else
        {
          if (partColValList_)
            isPartnTruncate = TRUE;
          hiveTruncQuery_ = "truncate table ";
          hiveTruncQuery_ += hiveTableName_;
        }
      
      NAString partColValStr;
      if (partColValList_)
        {
          partColValList_->unparse(partColValStr);
          
          hiveTruncQuery_ += " PARTITION ( ";
          
          for (Int32 i = 0; i < partColValList_->entries(); i++)
            {
              ItemExpr * ie = (*partColValList_)[i];
              
              // each partition entry has the form:  <col> = <value>
              // If <col> doesn't exist in the table or is not a partitioning col,
              // return an error.
              if (ie->getOperatorType() == ITM_EQUAL)
                {
                  ColReference * cr = 
                    (ColReference*)((BiRelat*)ie)->child(0)->castToItemExpr();
                  const NAString &colName = cr->getColRefNameObj().getColName();
                  const NAColumn *nac = 
                    naTable->getNAColumnArray().getColumn(colName.data());
                  if ((! nac) || (NOT nac->isHivePartColumn()))
                    {
                      if (!nac)
                        *CmpCommon::diags() << DgSqlCode(-3242) 
                                            << DgString0(NAString("Column '") + colName + NAString("' does not exist in the specified table."));
                      else
                        *CmpCommon::diags() << DgSqlCode(-3242) 
                                            << DgString0(NAString("Column '") + colName + NAString("' is not a partitioning column for the specified table."));
              ...
              if (ieStr[0] == '(')
                ieStr.remove(0, 1);
              if (ieStr[ieStr.length()-1] == ')')
                ieStr.remove(ieStr.length()-1, 1);
              
              hiveTruncQuery_ += ieStr;
              
              if (i < partColValList_->entries() -1)
                hiveTruncQuery_ += " , ";
            }
  ...
  if (NOT noSecurityCheck_)
    {
      // truncate requires select/delete privileges
      SqlTableOpenInfo::AccessFlags privs;
      privs.select_ = 1;
      privs.insert_ = 0;
      privs.update_ = 0;
      ...
      if (selPred.containsSubquery())
        {
          ...
          if (afterRefcount != beforeRefcount)
            {
              //Use of self-referencing in a subquery in the WHERE clause of a DELETE WITH MULTI COMMIT statement is not supported.
...
// -----------------------------------------------------------------------
void
ExeUtilCompositeUnnest::synthEstLogProp(const EstLogPropSharedPtr& inputEstLogProp)
{
  ...
  myEstLogProp->colStats().synchronizeStats(newRowCount, 
					   myEstLogProp->getColStats().entries());
...
a)  The key columns produced by unnnest have a different vid from its child scan, causing join cardinality to be overestimated. This is addressed now by forcing hoin cardinality to be same as that of unnest.

b)  The histograms after unnest should have the flattened column. Right now, I am just passing up the child??s histograms with cardinality adjusted.
  */

} // ExeUtilCompositeUnnest::synthEstLogProp

PhysicalProperty * ExeUtilCompositeUnnest::synthPhysicalProperty(const Context * context,
                                                             const Lng32 planNumber,
                                                             PlanWorkSpace *pws)
{
   const ReqdPhysicalProperty* rppForMe = context->getReqdPhysicalProperty();
...
RelExpr * ExeUtilLobExtract::copyTopNode(RelExpr *derivedNode, CollHeap* outHeap)
{
  ...
  result->objectUIDexpr_ = objectUIDexpr_;
  result->bufAddrExpr_ = bufAddrExpr_;
  result->extractSizeAddrExpr_ = extractSizeAddrExpr_;
...
short ExeUtilExpr::bindLobHandle(
     BindWA *bindWA, 
     ItemExpr *handle, 
     ItemExpr *objUIDexpr,
     NAString &location, 
     Int32 &numLOBdatafiles)
{
  ...
  if (objUIDexpr)
    {
      objUIDexpr->bindNode(bindWA);
      if (bindWA->errStatus())
        return NULL;
      
      if (objUIDexpr->getOperatorType() != ITM_CONSTANT)
        {
          *CmpCommon::diags() << DgSqlCode(-3242)
                              << DgString0("Provided LOB handle must be a constant.");
          
          bindWA->setErrStatus();
          return -1;
        }
      
      ConstValue *cv = (ConstValue*)objUIDexpr;      
      const NAString *h = cv->getRawText();

      objUID = str_atoi(h->data(), h->length());

      if (objUID <= 0)
        {
          *CmpCommon::diags() << DgSqlCode(-3242)
                              << DgString0(NAString("ExeUtilExpr::bindLobHandle. Invalid object UID ") + *h);
    ...
    {
      handle->bindNode(bindWA);
      if (bindWA->errStatus())
        return NULL;
      
      if (handle->getOperatorType() != ITM_CONSTANT)
        {
          *CmpCommon::diags() << DgSqlCode(-3242)
                              << DgString0("Provided LOB handle must be a constant.");
      ...
      ConstValue * cv = (ConstValue*)handle;
      ...
      if (strncmp(h->data(),"LOBH",4) != 0)
        {
          *CmpCommon::diags() << DgSqlCode(-3242)
                              << DgString0(NAString("ExeUtilExpr::bindLobHandle::1. Provided LOB handle is invalid. Handle=") + *h);
          
          bindWA->setErrStatus();
          return -1;
        }
      
      Int32 flags = (Lng32)str_atoi(&h->data()[4], 4);
      lobV2 = ExpLOBoper::isLOBHflagSet(flags, ExpLOBoper::LOBH_VERSION2);  
      
      Int32 rc = 0;
      char *inlineData = NULL;
      if (lobV2)
        rc = ExpLOBoper::extractFromLOBstringV2(NULL, NULL, NULL, &objUID,
                                                NULL, NULL, 
                                                (char*)h->data(), h->length());
      else
        rc = ExpLOBoper::extractFromLOBstring(&objUID, NULL, NULL, NULL, NULL,
                                              NULL, NULL, NULL, NULL, inlineData,
                                              (char*)h->data(), h->length());
      if ((rc < 0) || (objUID <= 0))
        {
          *CmpCommon::diags() << DgSqlCode(-3242)
                              << DgString0(NAString("ExeUtilExpr::bindLobHandle::2. Provided LOB handle is invalid. Handle=") + *h);
  ...
  ComUID cuid(objUID);
  CorrName *corrName = bindWA->getSchemaDB()->getNATableDB()->getObjUidToName().getFirstValue(&cuid);
  if (! corrName)
    {
      NAString catName, schName, objName;
      if (CmpSeabaseDDL::getObjectName(NULL, objUID,
                                       catName, schName, objName,
                                       NULL, FALSE, 
                                       TRUE/*look in objects index table*/))
        {
          *CmpCommon::diags() << DgSqlCode(-3242)
                              << DgString0("Provided LOB handle is invalid.");

          bindWA->setErrStatus();
          return -1;
        }
  
      ComUID *uid = new(bindWA->getSchemaDB()->getNATableDB()->getHeap()) 
        ComUID(objUID);
      corrName = new(bindWA->getSchemaDB()->getNATableDB()->getHeap()) 
        CorrName(objName, bindWA->getSchemaDB()->getNATableDB()->getHeap(), schName, catName);
      bindWA->getSchemaDB()->getNATableDB()->getObjUidToName().insert(uid, corrName);
    }
  ...
  if (! naTable->getLobStorageLocation())
    {
      CMPASSERT(naTable->getLobStorageLocation());

      bindWA->setErrStatus();
      return -1;
    }

  setUtilTableDesc(bindWA->createTableDesc(naTable, *corrName));
  if (bindWA->errStatus())
    return -1;

  location = naTable->getLobStorageLocation();
  numLOBdatafiles = naTable->getNumLOBdatafiles();
...
RelExpr * ExeUtilLobExtract::bindNode(BindWA *bindWA)
{
  ...
  if ((toType_ == TO_BUFFER_) &&
      ((CmpCommon::getDefault(IS_SQLCI) == DF_ON) ||
       (CmpCommon::getDefault(IS_TRAFCI) == DF_ON)) &&
      (!Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL)))
    {
      // this command cannot be issued directly from sqlci or trafci
     *CmpCommon::diags() << DgSqlCode(-3242)
                          << DgString0("EXTRACT LOBTOBUFFER command cannot be issued directly from sqlci or trafci.");
  ...
  if (objectUIDexpr_)
    {
      objectUIDexpr_->bindNode(bindWA);
      if (bindWA->errStatus())
        return NULL;
      ...
      if (bindWA->errStatus())
        return NULL;
      SQLChar ht(NULL, LOB_HANDLE_LEN, FALSE);
      handle_->getValueId().coerceType(ht, NA_CHARACTER_TYPE);

      bufAddrExpr_->bindNode(bindWA);
      if (bindWA->errStatus())
        return NULL;
      SQLLargeInt et(NULL, TRUE, FALSE);
      bufAddrExpr_->getValueId().coerceType(et, NA_NUMERIC_TYPE);

      if (extractSizeAddrExpr_)
        {
          extractSizeAddrExpr_->bindNode(bindWA);
          if (bindWA->errStatus())
            return NULL;

          extractSizeAddrExpr_->getValueId().coerceType(et, NA_NUMERIC_TYPE);
        }

      if (positionAddrExpr_)
        {
          positionAddrExpr_->bindNode(bindWA);
          if (bindWA->errStatus())
            return NULL;

          positionAddrExpr_->getValueId().coerceType(et, NA_NUMERIC_TYPE);
        }
    }

  NAString location;
  if (bindLobHandle(bindWA, handle_, objectUIDexpr_,
                    lobLocation_, numLOBdatafiles_))
    return this;
					    ...
					    )
{
  ...
  if (handle_)
    {
      exprOnParent += handle_->getValueId();
    }
...
RelExpr * ExeUtilLobUpdate::bindNode(BindWA *bindWA)
{
  ...
  if ((fromType_ == FROM_BUFFER_) &&
      ...
      (!Get_SqlParser_Flags(INTERNAL_QUERY_FROM_EXEUTIL)))
    {
      // this command cannot be issued directly from sqlci or trafci
     *CmpCommon::diags() << DgSqlCode(-3242)
                          << DgString0("UPDATE LOB command cannot be issued directly from sqlci or trafci.");
  ...
  NAString location;
  if (bindLobHandle(bindWA, handle_, objectUIDexpr_, lobLocation_,
                    numLOBdatafiles_))
    return this;
...
RelExpr * ExeUtilConnectby::bindNode(BindWA *bindWA)
{
  ...
  if(isDual() == TRUE)
    myselection_ = ((BiConnectBy*)getBiConnectBy())->where_clause;
...
* *** ERROR[2006] Internal error: assertion failure (NOT objName.isNull()) in file ../optimizer/ObjectNames.cpp at line 411.
* The purpose is to expose the DUAL table structure, LEVEL column, and will add ROWNUM column which are all vitual
* None of these columns/table are in _MD_ tables
*/
  ...
  if( startWithPred) 
  {
    ...
    if(startWithPred->containsSubquery() )
      bindWA->setInSubqueryStartWith(TRUE);
...
RelExpr * ExeUtilHBaseBulkLoad::bindNode(BindWA *bindWA)
{
  ...
  if (naTable->isHiveTable())
    {
      *CmpCommon::diags() << DgSqlCode(-3242)
                          << DgString0("Cannot LOAD into hive tables.");
  ...
  if (naTable->getViewText())
    {
      *CmpCommon::diags() << DgSqlCode(-3242)
                          << DgString0("Cannot LOAD into views.");
  ...
  {
    *CmpCommon::diags() << DgSqlCode(-3242)
                        << DgString0("Cannot LOAD into Partition index table.");
  ...
  if (naTable->isPartitionV2Table())
  {
    ...
    tempQuery.format("%s%s%s", 
          getUpsertUsingLoad() ? "UPSERT USING LOAD INTO " : "LOAD TRANSFORM  INTO ", 
  ...
  if ((naTable->getCheckConstraints().entries() != 0 || 
       naTable->getRefConstraints().entries() != 0  ) ||
...
// Member functions for class ExeUtilCompositeUnnest
// -----------------------------------------------------------------------
ExeUtilCompositeUnnest::ExeUtilCompositeUnnest
(
     ItemExpr *val1Ptr,
     NABoolean isOuter,
     ...
     ItemExpr *colNames,
     CollHeap *oHeap)
     : ExeUtilExpr(COMPOSITE_UNNEST_, CorrName(""),
		   NULL, NULL, NULL, CharInfo::UnknownCharSet, oHeap),
...
RelExpr * ExeUtilCompositeUnnest::copyTopNode(RelExpr *derivedNode, CollHeap* outHeap)
{
  ...
  if (derivedNode == NULL)
    result = new (outHeap) ExeUtilCompositeUnnest(colNameExpr_,
                                                  isOuter_,
                                                  outerType_,
                                                  showPos_,
                                                  addnlCols_,
...
void ExeUtilCompositeUnnest::pushdownCoveredExpr(const ValueIdSet & outputExpr,
                                                 const ValueIdSet & newExternalInputs,
                                                 ValueIdSet & predicatesOnParent,
                                                 const ValueIdSet * setOfValuesReqdByParent,
                                                 Lng32 childIndex
...
static void populateColInfoArray(ComTdbVirtTableColumnInfo *colInfoArray, 
                                 Int32 index, 
                                 const NAString &elemName,
                                 ...
                                 NABoolean isSysCol)
{
  ...
  colInfoArray[index].columnClass = 
    (isSysCol ? COM_SYSTEM_COLUMN : COM_USER_COLUMN);
          ...
          if (newColNamesTree_)
            {
              ...
              elemName = ((RenameCol *) derivedColList[elem])->
      ...
      if (newColNamesTree_)
        {
          ...
          elemName = ((RenameCol *) derivedColList[0])->
            getNewColRefName()->getColName();
        }
      else
        elemName = colName_;      
  ...
  for (Int32 elem = 0; elem < addnlColsVIDList_.entries(); elem++)
    {
      ...
      populateColInfoArray(colInfoArray, index, elemName, &naType,
                           TRUE);
      index++;
    }
...
RelExpr * ExeUtilCompositeUnnest::bindNode(BindWA *bindWA)
{
  ...
  if (! child(0))
    {
      *CmpCommon::diags() << DgSqlCode(-3242)
                          << DgString0("Unnest node must have a child.");
      bindWA->setErrStatus();
      return this;
    }
  ...
  if (colNameExpr_->getOperatorType() == ITM_REFERENCE)
    ...
    {
      *CmpCommon::diags() << DgSqlCode(-3242)
                          << DgString0("Unnest operand must be a column.");
      bindWA->setErrStatus();
      return this;
    }
  ...
  if (getScanTableName().getQualifiedNameObj().getObjectName().isNull())
    {
      *CmpCommon::diags() << DgSqlCode(-3242)
                          << DgString0("Unnest operand must be a table.");
  ...
  if ((!scanNATable) || (bindWA->errStatus()))
    return this;
  ...
  if (colName_.isNull())
    {
      if (colNameExpr_->getOperatorType() == ITM_REFERENCE)
        ...
        {
          *CmpCommon::diags() << DgSqlCode(-3242)
                              << DgString0("Operand must be a column.");
          bindWA->setErrStatus();
          return this;
        }
    }
...
RelExpr * ExeUtilSnapShotUpdataDelete::copyTopNode(RelExpr *derivedNode, CollHeap* outHeap)
{
  ...
  if (derivedNode == NULL)
    result = new (outHeap)
      ExeUtilSnapShotUpdataDelete(getTableName()
          , (char*)NULL
          , type_
          , CharInfo::UnknownCharSet
          , outHeap);
...
RelExpr * ExeUtilSnapShotUpdataDelete::bindNode(BindWA *bindWA)
{
  ...
  {
    *CmpCommon::diags() << DgSqlCode(-3242)
                        << DgString0("The "+ getText() +" can not perform a on view");
    bindWA->setErrStatus();
    return this;
  }
  ...
  {
    *CmpCommon::diags() << DgSqlCode(-3242)
                        << DgString0("The" + getText() +" can not perform a hive table.");
    bindWA->setErrStatus();
    return this;      
  }
