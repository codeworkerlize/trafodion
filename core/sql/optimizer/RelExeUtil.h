...
public:
  ...
  ExeUtilAvroStats(const CorrName &objectName,
...
class ExeUtilGetExtSchema : public ExeUtilExpr 
{
public:
  enum ExtStorageType
    {
  ...
  ExeUtilGetExtSchema(const CorrName &objectName,
                      ExtStorageType type,
                      NABoolean forRead,
                      NABoolean forWrite,
                      CollHeap *oHeap = CmpCommon::statementHeap());
...
private:
  ExtStorageType type_;

  // true, return schema read from external storage
  NABoolean forRead_; 

  // true, return schema created to write/insert rows into external storage
  NABoolean forWrite_;
};
...
public:
 ...
 ExeUtilLobExtract(ItemExpr * handle, 
      ...
      : ExeUtilExpr(LOB_EXTRACT_, CorrName("dummyExtractName"),
        ...
        objectUIDexpr_(NULL),
        bufAddrExpr_(NULL),
        extractSizeAddrExpr_(NULL),
        positionAddrExpr_(NULL)
    {
      if (stringParam)
	...
	stringParam3_ = stringParam3;
      ...
      setCacheable(CACHEABLE_BIND);
    };

 ExeUtilLobExtract(ExtractToType toType,
                   ItemExpr *objectUIDexpr,
                   ItemExpr *handle,
                   ItemExpr *bufAddrExpr,
                   ItemExpr *extractSizeAddrExpr,
                   ItemExpr *positionAddrExpr = NULL,
		   CollHeap *oHeap = CmpCommon::statementHeap())
   : ExeUtilExpr(LOB_EXTRACT_, CorrName("dummyExtractName"),
		 NULL, NULL,
		 NULL, CharInfo::UnknownCharSet, oHeap),
     toType_(toType),
     objectUIDexpr_(objectUIDexpr),
     bufAddrExpr_(bufAddrExpr),
     extractSizeAddrExpr_(extractSizeAddrExpr),
     positionAddrExpr_(positionAddrExpr),
     handle_(handle),
     bufAddr_(0),
     extractSizeAddr_(0),
     intParam_(0),
     intParam2_(0),
     handleInStringFormat_(FALSE),
     withCreate_(TRUE)
  {
    setCacheable(CACHEABLE_BIND);
  };
 ...
 private:
  ...
  // set when uid, location and size are passed in as const or param.
  ItemExpr *objectUIDexpr_;
  ItemExpr *bufAddrExpr_;
  ItemExpr *extractSizeAddrExpr_;
  ItemExpr *positionAddrExpr_;
};
...
public:
  ...
  {
    ERROR_IF_EXISTS_=1, TRUNCATE_EXISTING_, APPEND_, REPLACE_, NONE_ 
  };
 ...
 ExeUtilLobUpdate(ItemExpr * handle, 
  ...
  ExeUtilLobUpdate(UpdateFromType fromType,
                   ItemExpr *objectUIDexpr,
                   ItemExpr *handle,
                   ItemExpr *bufAddrExpr,
                   ItemExpr *extractSizeAddrExpr,
