...
//
NABoolean UpdateColumns::match(const UpdateColumns &other) const {
  ...
  if (isAllColumns() || other.isAllColumns()) return TRUE;

  CollIndex numEntries = other.columns_->entries();
  for (CollIndex i = 0; i < numEntries; i++)
    if (columns_->contains((*other.columns_)[i])) return TRUE;
...
//
void UpdateColumns::markColumnsOnBitmap(unsigned char *bitmap, CollIndex numBytes) const {
...
void UpdateColumns::print(ostream &os, const char *indent, const char *title) const {
  os << indent << title << " ";
  ...
  CollIndex numEntries = columns_->entries();
  for (CollIndex i = 0; i < numEntries; i++) os << (*columns_)[i] << ", ";
...
Trigger::~Trigger() {
  ...
  if (parsedTrigger_) delete (parsedTrigger_);
  ...
  if (updateCols_) delete updateCols_;
  if (sqlText_) delete sqlText_;
}

//
// -- Trigger:: equality
//
NABoolean Trigger::operator==(const Trigger &other) const { return name_ == other.name_; }
...
// compatability.
//
StmtDDLCreateTrigger *Trigger::parseTriggerText() const {
  ExprNode *parsedNode;
  RelExpr *triggerBody;

  CMPASSERT(sqlText_ != NULL);
  if (sqlTextCharSet_ == CharInfo::UnknownCharSet) const_cast<Trigger *>(this)->sqlTextCharSet_ = CharInfo::UTF8;

  // Parse the SQL text.
  Parser parser(CmpCommon::context());
  if (parser.parseDML(sqlText_->data(), sqlText_->length(), sqlTextCharSet_, &parsedNode)) return NULL;
  ...
  StmtDDLCreateTrigger *createTriggerNode = (StmtDDLCreateTrigger *)parseTree->getDDLNode();
...
// (modified by <aviv>)
//
RelExpr *Trigger::getParsedTrigger(BindWA *bindWA) {
  RelExpr *result = NULL;
  ...
  if (isAfterTrigger() && recursionCounter_ > MAX_RECURSION_DEPTH) {
    ...
    RaiseError *error = new (CmpCommon::statementHeap()) RaiseError(11003, getTriggerName(), getSubjectTableName());
    Tuple *tuple = new (CmpCommon::statementHeap()) TupleList(new (CmpCommon::statementHeap()) Convert(error));
    ...
    *CmpCommon::diags() << DgSqlCode(11002) << DgString0(getTriggerName()) << DgString1(getSubjectTableName());
  } else {  // recursion limit not exceeded
    if (parsedTrigger_ == NULL) {
      ...
      result = parsedTrigger_->getActionExpression()->copyTree(CmpCommon::statementHeap());
      isVirginCopy_ = TRUE;
    }
  }
...
//
// used for debugging only
void Trigger::print(ostream &os, const char *indent, const char *title) const {
  os << indent << title << " ";
  os << indent << getTriggerName() << " : " << endl;
  BUMP_INDENT(indent);
  os << indent << "Subject Table Name : " << getSubjectTableName() << endl;
  os << indent << "Granularity: " << (isStatementTrigger() ? "Statement " : "Row ") << endl;
  os << indent << "Activation: " << (isBeforeTrigger() ? "Before " : "After ") << endl;
  os << indent << "Operation: ";
  switch (getOperation()) {
    case COM_UPDATE:
      os << "Update";
  ...
  if (operation_ == COM_UPDATE) updateCols_->print(os, indent, "Subject Columns");
...
TriggerList *TriggerList::getColumnMatchingTriggers(const UpdateColumns *updateCols) {
  CollIndex i;
  TriggerList *relevantTrgs = NULL;
  Trigger *trg;

  if (updateCols == NULL) return this;

  for (i = 0; i < entries(); i++) {
    trg = (*this)[i];
    if (trg->getUpdateColumns()->match(*updateCols)) {
      if (!relevantTrgs) relevantTrgs = new (CmpCommon::statementHeap()) TriggerList(CmpCommon::statementHeap());
...
void TriggerList::sortByTimeStamp() {
  ...
  for (CollIndex idx = 0; idx < mySize; idx++) trigArray[idx] = (*this)[idx];

  // sort trigArray by timestamp
  opt_qsort(trigArray, mySize, sizeof(Trigger *), triggerTimeCompare);

  // set triggerList_ based on the reordered array optTriggerPtrArray
  this->clear();
  for (CollIndex k = 0; k < mySize; k++) this->insert(trigArray[k]);
...
//
// used for debugging only
void TriggerList::print(ostream &os, const char *indent, const char *title) const {
  os << title << endl;
  BUMP_INDENT(indent);
...
void TriggerList::clearAndDestroy() {
  Trigger *currEntry;
  for (CollIndex i = 0; i < entries(); i++) {
    currEntry = this->at(i);
    if (currEntry != NULL) delete currEntry;
  }
  this->clear();
}
...
//
// used for debugging only
void BeforeAndAfterTriggers::print(ostream &os, const char *indent, const char *title) const {
  if (getBeforeTriggers()) getBeforeTriggers()->print(os, indent, "Before Triggers: ");
  if (getAfterStatementTriggers()) getAfterStatementTriggers()->print(os, indent, "After Statement Triggers: ");
  if (getAfterRowTriggers()) getAfterRowTriggers()->print(os, indent, "After Row Triggers: ");
}
...
// Currently triggers are allocated from the statement heap.
// See method Trigger::Heap() in file Triggers.h for more details
void BeforeAndAfterTriggers::clearAndDestroy() {
...
void BeforeAndAfterTriggers::addNewAfterStatementTrigger(Trigger *newTrigger) {
  if (!afterStatementTriggers_) {
    afterStatementTriggers_ = new (CmpCommon::statementHeap()) TriggerList(CmpCommon::statementHeap());
  }
...
void BeforeAndAfterTriggers::addNewAfterRowTrigger(Trigger *newTrigger) {
  if (!afterRowTriggers_) {
    afterRowTriggers_ = new (CmpCommon::statementHeap()) TriggerList(CmpCommon::statementHeap());
  }
