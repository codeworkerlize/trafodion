...
int MVJoinTable::markRiConstraints(BindWA *bindWA, MVInfo *mvInfo) {
  ...
  for (CollIndex i = 0; i < refConstraints.entries(); i++) {
    ...
    const NAString &otherTableName = ref->getOtherTableName().getQualifiedNameAsString();
    MVJoinTable *otherTable = mvInfo->getJoinGraph()->getTableObjectFor(&otherTableName);
    if (otherTable == NULL) continue;  // The other table must be on the graph.
    ...
    for (CollIndex currentCol = 0; currentCol < myCols.entries(); currentCol++) {
      if (!mvInfo->isEqPredicateBetween(naTable_->getTableName(), myCols[currentCol], ref->getOtherTableName(),
                                        otherCols[currentCol]))
        matchingPredicatesExist = FALSE;
    }
    if (!matchingPredicatesExist) continue;
...
NABoolean MVJoinTable::isOnRI(const MVJoinGraphState &state) const {
  ...
  if (state.getNofRIs() == 0) return FALSE;
...
int MVJoinTable::calcHeuristics(const MVJoinGraphState &state) const {
  ...
  if (state.getNofRIs() == 0) return score;
  ...
  if (isOnRI(state)) score += WEIGHT_ON_RI;  // Category 1
...
void MVJoinTable::print(FILE *ofd, const char *indent, const char *title) const {
  ...
  switch (deltaType_) {
    case EMPTY_DELTA:
      deltaTypeText = "Empty Delta";
      break;
    case INSERTONLY_DELTA:
      deltaTypeText = "InsertOnly Delta";
      break;
    case NONEMPTY_DELTA:
      deltaTypeText = "Nonempty Delta";
      break;
  }
...
void MVJoinTable::display() const { print(); }
#endif
...
void MVJoinGraphSolution::pushTable(int tableIndex, NABoolean isOnRi) {
  ...
  entries_++;
  if (isOnRi) riTables_.insert(tableIndex);
}
...
void MVJoinGraphSolution::display() const { print(); }
#endif
...
MVJoinGraphState::MVJoinGraphState(int nofTables, int nofRIs, const MVTableSet &reorderGroup, CollHeap *heap)
  ...
  for (CollIndex i = 0; i < reorderGroup.entries(); i++) availableBitmap_.setBit(reorderGroup[i]);
}
...
void MVJoinGraphState::chooseBestSolution() {
  if (bestRoute_.isEmpty() || (currentRoute_.getScore() > bestRoute_.getScore())) bestRoute_ = currentRoute_;
}
...
MVJoinTable *MVJoinGraph::getTableObjectFor(const NAString *tableName) { return tableHash_.getFirstValue(tableName); }
...
void MVJoinGraph::markPredicateBetween(const QualifiedName &leftTable, const QualifiedName &rightTable) {
...
#ifndef NDEBUG
  if (CmpCommon::getDefault(MV_DUMP_DEBUG_INFO) == DF_ON) display();
#endif
...
#ifndef NDEBUG
    if (CmpCommon::getDefault(MV_DUMP_DEBUG_INFO) == DF_ON) theSolution_.display();
#endif
  }
  return isConnected;
}
...
void MVJoinGraph::findBestOrder() {
  ...
  while (!done && notStartedSet_.entries() > 0) {
    ...
    if (findSolutionFrom(state)) {
      state.chooseBestSolution();
      if (state.getBestRoute().getScore() == nofRI_) done = TRUE;  // This is as good as it gets.
    } else {
      // If we get here the graph is not fully connected!
      CMPASSERT(FALSE);
    }

    if (!done) state.reset(reorderGroupSet_);
  }
