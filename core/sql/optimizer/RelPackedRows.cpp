...
//
NABoolean UnPackRows::duplicateMatch(const RelExpr &other) const {
  ...
  //
  if (!(unPackedTable() == o.unPackedTable())) return FALSE;

  // If the unPackExprs are the same and the packingFactors are the same,
  // then the nodes are identical
  //
  if (!(unPackExpr() == o.unPackExpr())) return FALSE;

  if (!(packingFactor() == o.packingFactor())) return FALSE;

  if (!(rowwiseRowset() == o.rowwiseRowset())) return FALSE;
...
// case).
//
RelExpr *UnPackRows::copyTopNode(RelExpr *derivedNode, CollHeap *outHeap) {
  UnPackRows *result;
  ...
  {
    if (rowwiseRowset())
      result = new (outHeap) UnPackRows(0, (ItemExpr *)NULL, (ItemExpr *)NULL, (ItemExpr *)NULL, NULL);
    else
  ...
  //
  if (unPackExprTree() != NULL) result->unPackExprTree() = unPackExprTree()->copyTree(outHeap)->castToItemExpr();

  if (packingFactorTree() != NULL)
    result->packingFactorTree() = packingFactorTree()->copyTree(outHeap)->castToItemExpr();

  if (rwrsInputSizeExpr_ != NULL) result->rwrsInputSizeExpr_ = rwrsInputSizeExpr_->copyTree(outHeap)->castToItemExpr();
...
//
void UnPackRows::addLocalExpr(LIST(ExprNode *) & xlist, LIST(NAString) & llist) const {
...
//
void UnPackRows::getPotentialOutputValues(ValueIdSet &outputValues) const {
...
void UnPackRows::constructNewSyskeyPreds(ValueIdSet &predicates) {
  ...
  for (ValueId pred = predicates.init(); predicates.next(pred); predicates.advance(pred)) {
    ...
    //
    CollIndex predIndex = originalPreds() ? originalPreds()->index(pred) : NULL_COLL_INDEX;
    ...
    if (predIndex != NULL_COLL_INDEX) {
      ...
      if (oper == ITM_EQUAL || oper == ITM_LESS || oper == ITM_LESS_EQ || oper == ITM_GREATER ||
          oper == ITM_GREATER_EQ) {
        if (pred.getItemExpr()->child(0) == sysKeyId()) {
          // A match

          ItemExpr *packedSyskey = pred.getItemExpr()->child(0)->castToItemExpr()->child(0);

          ItemExpr *oldValue = pred.getItemExpr()->child(1);
          ItemExpr *maskConst = new (CmpCommon::statementHeap()) SystemLiteral(65535);

          const NAType *desiredType = &packedSyskey->getValueId().getType();

          ItemExpr *maskValue = new (CmpCommon::statementHeap()) Cast(maskConst, desiredType);

          oldValue = new (CmpCommon::statementHeap()) Cast(oldValue, desiredType);

          ItemExpr *newValue = NULL;
          switch (oper) {
            case ITM_EQUAL:
              newValue = new (CmpCommon::statementHeap()) Mask(ITM_MASK_CLEAR, oldValue, maskValue);
              break;
            case ITM_LESS:
              newValue = new (CmpCommon::statementHeap()) Mask(ITM_MASK_SET, oldValue, maskValue);
              break;
            case ITM_LESS_EQ:
              newValue = new (CmpCommon::statementHeap()) Mask(ITM_MASK_SET, oldValue, maskValue);
              oper = ITM_LESS;
          ...
          ItemExpr *newPred = new (CmpCommon::statementHeap()) BiRelat(oper, packedSyskey, newValue);
          ...
          CMPASSERT(originalPreds()->entries() == rewrittenPreds()->entries());
        }
      }
    }
  }
...
void UnPackRows::pushdownCoveredExpr(const ValueIdSet &outputExpr, const ValueIdSet &newExternalInputs,
  ...
  // the selectionPred.
  //
  for (ValueId unPackCol = unPackExpr().init(); unPackExpr().next(unPackCol); unPackExpr().advance(unPackCol)) {
  ...
  ValueIdSet emptySet;
  RelExpr::pushdownCoveredExpr(outputSet, newExternalInputs, predicatesOnParent, &emptySet, childIndex);
...
//
ItemExpr *UnPackRows::removeUnPackExprTree() {
...
// needed after the binder.
//
ItemExpr *UnPackRows::removePackingFactorTree() {
  ItemExpr *result = packingFactorTree();
  packingFactorTree() = (ItemExpr *)NULL;
  return result;
}

PhysUnPackRows::PhysUnPackRows(RelExpr *child /*= NULL*/, CollHeap *oHeap /*= CmpCommon::statementHeap()*/)
    : UnPackRows(0, NULL, NULL, NULL, child, NULL_VALUE_ID, oHeap) {
  if (oHeap)
    pCostMethod_ = new (oHeap) CostMethodUnPackRows();
  else
    pCostMethod_ = new (CmpCommon::statementHeap()) CostMethodUnPackRows();
};
...
// case).
//
RelExpr *PhysUnPackRows::copyTopNode(RelExpr *derivedNode, CollHeap *outHeap) {
...
//
void UnPackRows::transformNode(NormWA &normWARef, ExprGroupId &locationOfPointerToMe) {
  ...
  */
  ...
  if (unPackExpr().transformNode(normWARef, child(0), getGroupAttr()->getCharacteristicInputs())) {
    // -----------------------------------------------------------------
  ...
  if (packingFactor().transformNode(normWARef, child(0), getGroupAttr()->getCharacteristicInputs())) {
    // -----------------------------------------------------------------
    // Transform my new child.
    // -----------------------------------------------------------------
    child(0)->transformNode(normWARef, child(0));
  ...
  //
  if (indexValue() != NULL_VALUE_ID) indexValue() = indexValue().getItemExpr()->getReplacementExpr()->getValueId();
...
void UnPackRows::synthEstLogProp(const EstLogPropSharedPtr &inputEstLogProp) {
  ...
  CostScalar rowCount = childEstProps->getResultCardinality() * getMaxPackingFactor();
  ...
  if (unPackedTable())
    myEstProps = synthEstLogPropForUnaryLeafOp(inputEstLogProp, colStats(),
    ...
    for (ValueId charOutput = reqdOutputs.init(); reqdOutputs.next(charOutput); reqdOutputs.advance(charOutput)) {
      outputColStats.addColStatDescForVirtualCol(rowCount, rowCount, charOutput, charOutput, charOutput, NULL);
    }
  }
...
//
void UnPackRows::synthLogProp(NormWA *normWAPtr) {
  ...
  for (ValueId unPackCol = unPackExpr().init(); unPackExpr().next(unPackCol); unPackExpr().advance(unPackCol)) {
    ...
    if (packedItem->getOperatorType() != ITM_BASECOLUMN) {
      if (packedItem->getOperatorType() == ITM_VEG_REFERENCE) {
        ...
        for (ValueId colvid = packedVEGset.init(); packedVEGset.next(colvid); packedVEGset.advance(colvid)) {
      ...
      packedBaseColList.insert((BaseColumn *)packedItem);
  }
  ...
  for (i = 0; i < initialStats.entries(); i++) {
    ...
    //
    const NAColumnArray &statCols = initialStats[i]->getColStats()->getStatColumns();
    ...
    NABoolean found = FALSE;
    CollIndex j = 0;
    for (ValueId unPackCol = unPackExpr().init(); unPackExpr().next(unPackCol); unPackExpr().advance(unPackCol), j++) {
      if (packedBaseColList[j] && packedBaseColList[j]->getColName() == statCols[0]->getColName()) {
    ...
    if (!found)
      removeList.insert(initialStats[i]);
    else
      colStats().insert(initialStats[i]);
  }

  // Remove unused statistics.
  //
  for (i = 0; i < removeList.entries(); i++) initialStats.remove(removeList[i]);
...
CostMethod *PhysUnPackRows::costMethod() const {
...
PhysicalProperty *PhysUnPackRows::synthPhysicalProperty(const Context *context, const int planNumber,
  ...
  PhysicalProperty *unPackPP = new (CmpCommon::statementHeap()) PhysicalProperty(*sppOfTheChild);

  if (unPackedTable() == NULL) {  // we're a rowset unpack
    if (getGroupAttr()->getResultCardinalityForEmptyInput() > ActiveSchemaDB()->getDefaults().getAsLong(COMP_INT_5)) {
...
RelExpr *UnPackRows::bindNode(BindWA *bindWA) {
  ...
  if (unPackExprTree()) {
    ...
    removeUnPackExprTree()->convertToValueIdList(unPackExprList, bindWA, ITM_ITEM_LIST);
  ...
  if (packingFactorTree()) {
    ...
    removePackingFactorTree()->convertToValueIdSet(packingFactor(), bindWA, ITM_ITEM_LIST);
  ...
  if (unPackExprTree()) {
    if (indexValue_ == NULL_VALUE_ID) {
      ...
      for (ValueId leaf = leafValues.init(); leafValues.next(leaf); leafValues.advance(leaf)) {
  ...
  for (i = 0; i < sysColList->entries(); i++) {
    resultTable->addColumn(bindWA, sysColList->at(i)->getColRefNameObj(),
                           (rowwiseRowset() ? sysColList->at(i)->getValueId() : unPackExprList[j++]), SYSTEM_COLUMN,
  ...
  for (i = 0; i < childTable.getColumnList()->entries(); i++) {
    if (rowwiseRowset()) rwrsOutputVids_.insert(childTable.getValueId(i));

    resultTable->addColumn(bindWA, childTable.getColRefNameObj(i),
                           (rowwiseRowset() ? childTable.getValueId(i) : unPackExprList[j++]), USER_COLUMN,
