             ...
             if ((NOT normWARef.dontAddRedundantPredicatesFlag()) &&
                 (CmpCommon::getDefault(FIND_COLUMN_SUBEXPRS_IN_OR) == DF_ON))
               {
                 ...
                 locationOfPointerToMe = 
                   locationOfPointerToMe->applyInverseDistributivityLawByColumn();
                 getValueId().replaceItemExpr(locationOfPointerToMe);
                 locationOfPointerToMe->synthTypeAndValueId();
               }
   ...
   if (bothAreColumnsOrUserInput(normWARef, child(0), child(1)))
   {
     ...
     // Irena
          ...
          else
          {
            ...
            if (isStartWithExpr() == TRUE ||
               isConnectByExpr() == TRUE)
            {
              putInVeg = FALSE;
            }
      ...
      if ( dummyNegate && cv->canGetExactNumericValue() )
        {
          ...
          if (child(0))
            newExpr1 = child(0)->castToItemExpr();
          ItemExpr *newExpr2 = new (heap) ConstValue(Lng32(value));
          ItemExpr *newExpr3 = NULL;
          if (child(2))
            newExpr3 = child(2)->castToItemExpr();
          ItmLagOlapFunction * pExpr = new (heap) ItmLagOlapFunction(newExpr1,
                                                                      newExpr2,
                                                                      newExpr3);
      ...
      if ( dummyNegate && cv->canGetExactNumericValue() )
        {
          ...
          if (child(2))
            newExpr3 = child(2)->castToItemExpr();
          ItmLeadOlapFunction * pExpr = new (heap) ItmLeadOlapFunction(newExpr1,
                                                                      newExpr2,
                                                                      newExpr3);
          pExpr->setOLAPInfo(getOlapPartitionBy(),getOlapOrderBy());
          setReplacementExpr(pExpr);
...
ItemExpr *ItmOlapPivotGroup::transformOlapFunction(CollHeap *heap)
{
  ...
  ItmSequenceFunction * seqFunc =
                   new (heap) ItmSeqRunningPivotGroup(op, child(0),
                                                                    getOlapOrderBy(),
                                                                    getPivotDelimiter(),
                                                                    getPivotMaxLen());

  ItmSequenceFunction *seqOff = new (heap)
  ItmSeqOffset(seqFunc, -INT_MAX, FALSE, TRUE, INT_MAX);
