...
class REFRESH_LIB_CLASS CRURuntimeController {
 public:
  CRURuntimeController() : pPeerController_(NULL), pRequest_(NULL) {}

  virtual ~CRURuntimeController() {}

 public:
  // The controller to communicate with
  void SetPeerController(CRURuntimeController *pPeerController) { pPeerController_ = pPeerController; }

 public:
  BOOL HasWork() const { return (NULL != pRequest_); }
 ...
 protected:
  ...
  CRURuntimeController *GetPeerController() { return pPeerController_; }
...
inline void CRURuntimeController::Work() {
  RUASSERT(TRUE == this->HasWork() && FALSE == GetPeerController()->HasWork());
