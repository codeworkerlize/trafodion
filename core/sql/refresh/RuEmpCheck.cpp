...
CRUEmpCheck::CRUEmpCheck()
    : pVec_(NULL), pSQLContainer_(new CRUSQLDynamicStatementContainer(NUM_STMTS)), checkMask_(0) {}
...
void CRUEmpCheck::PrepareSQL() { pSQLContainer_->PrepareSQL(); }
...
void CRUEmpCheck::PerformCheck() {
  ...
  while (NULL != (pElem = it.GetCurrentElem())) {
    PerformSingleCheck(*pElem, CRUTbl::RANGE, CHECK_NEG_EPOCHS);

    PerformSingleCheck(*pElem, CRUTbl::SINGLE_ROW_INSERTS, CHECK_POS_EPOCHS_INSERTS);

    PerformSingleCheck(*pElem, CRUTbl::SINGLE_ROW_OTHER, CHECK_POS_EPOCHS_DELETES);
...
void CRUEmpCheck::ComposeSelectStmt(CRUTbl &tbl, CDSString &to, StmtType StmtType, TInt32 upperBound) {
  RUASSERT(CHECK_POS_EPOCHS_INSERTS == StmtType || CHECK_POS_EPOCHS_DELETES == StmtType ||
           CHECK_NEG_EPOCHS == StmtType);

  CDSString iudLogName(tbl.GetIUDLogFullName());

  CDSString epochColName("EPOCH");
  epochColName = CRUSQLComposer::ComposeQuotedColName(CRUTbl::logCrtlColPrefix, epochColName);

  CDSString opColName("OPERATION_TYPE");
  opColName = CRUSQLComposer::ComposeQuotedColName(CRUTbl::logCrtlColPrefix, opColName);
  ...
  if (0 != upperBound) {
    to += "\nAND " + epochColName + op2 + CRUSQLComposer::TInt32ToStr(upperBound);
  }
...
void CRUEmpCheck::PerformSingleCheck(CRUEmpCheckVector::Elem &elem, CRUTbl::IUDLogContentType ct, StmtType stmtType) {
  if (0 == (ct & checkMask_)) {
  ...
  CDMPreparedStatement *pStmt = pSQLContainer_->GetPreparedStatement(stmtType);
