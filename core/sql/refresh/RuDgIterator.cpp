...
CRUDependenceGraphIterator::CRUDependenceGraphIterator(CRUDependenceGraph &dg)
    : allTasksList_(dg.availableTaskList_), pCurrentTask_(NULL) {}

CRUTopologicalDGIterator::CRUTopologicalDGIterator(CRUDependenceGraph &dg, IterationDir dir)
    : inherited(dg), dir_(dir), readyTasksList_(eItemsArentOwned), allTasksLinkMap_(HASH_SIZE) {
...
void CRUTopologicalDGIterator::Build() {
  ...
  while (NULL != pos) {
    ...
    CRUTaskList &refTaskList = (DIRECT == dir_) ? pTask->GetTasksThatIDependOn() : pTask->GetTasksThatDependOnMe();
...
void CRUTopologicalDGIterator::Next() {
  ...
  CRUTaskList &refTaskList = (DIRECT == dir_) ? pTask->GetTasksThatDependOnMe() : pTask->GetTasksThatIDependOn();
