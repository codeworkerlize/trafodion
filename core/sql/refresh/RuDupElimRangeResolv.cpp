...
CRUDupElimRangeResolver::CRUDupElimRangeResolver(const CRUDupElimGlobals &globals,
                                                 CRUSQLDynamicStatementContainer &ctrlStmtContainer)
    : inherited(globals, ctrlStmtContainer, CRUDupElimConst::NUM_RNG_RESOLV_STMTS),
...
#ifdef _DEBUG
void CRUDupElimRangeResolver::DumpPerformanceStatistics(CDSString &to) const {
  CDSString msg;
...
void CRUDupElimRangeResolver::FlushCrossTypeDuplicatesToIUDLog() {
  ...
  TraverseRngCollectionAndPerformRqst(DELETE_DUPLICATE_IUDLOG, CRURangeCollectionIterator::DIR_FORWARD);

  TraverseRngCollectionAndPerformRqst(UPDATE_DUPLICATE_IUDLOG, CRURangeCollectionIterator::DIR_BACKWARD);
}
...
void CRUDupElimRangeResolver::FlushRangesToRngLog() {
  ...
  if (IS_IN_RNG_LOG == GetRangeMappingStatus(rangeCollection_.GetMinEpoch()) && 1 == rangeCollection_.GetSize()) {
    return;
  }
  ...
  TraverseRngCollectionAndPerformRqst(DELETE_PREV_IMAGE_RNGLOG, CRURangeCollectionIterator::DIR_FORWARD);
  ...
  TraverseRngCollectionAndPerformRqst(INSERT_POST_IMAGE_RNGLOG, CRURangeCollectionIterator::DIR_FORWARD);
}
...
CRUDupElimRangeResolver::RangeMappingStatus CRUDupElimRangeResolver::GetRangeMappingStatus(TInt32 epoch) {
...
void CRUDupElimRangeResolver::TraverseRngCollectionAndPerformRqst(RequestType req,
...
void CRUDupElimRangeResolver::ExecuteRngLogInsert(const CRURange &rng) {
  ...
  CDMPreparedStatement *pStmt = GetPreparedStatement(CRUDupElimConst::RANGE_LOG_INSERT);
