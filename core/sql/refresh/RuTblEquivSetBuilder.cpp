...
void CRUTblEquivSetBuilder::AddRecomputedMVs() {
  ...
  while (NULL != mvPos) {
    ...
    if (0 == pMV->GetNumDirectlyUsedMVs() || 1 == pMV->GetTablesUsedByMe().GetCount()) {
      continue;
    }
...
void CRUTblEquivSetBuilder::AddRecomputeTree(CRUMV *pMV) {
  ...
  while (NULL != tblPos) {
    ...
    RUASSERT(CDDObject::eRECOMPUTE == pNextMV->GetRefreshType());
...
void CRUTblEquivSetBuilder::DumpSets() {
  ...
  for (Int32 i = 0; i < GetNumOfSets(); i++) {
    ...
    while (NULL != tblPos) {
      ...
      CRUGlobals::GetInstance()->GetJournal().LogMessage(pTbl->GetFullName());
    }
  }
}
#endif
