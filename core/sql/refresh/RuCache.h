...
class REFRESH_LIB_CLASS CRUCache {
 public:
  ...
  //--------------------------------------------//
 public:
  CRUMVList &GetMVList() { return mvList_; }
  CRUMV *GetMV(TInt64 objUid) const;

  CRUTblList &GetTableList() { return tableList_; }
  CRUTbl *GetTable(TInt64 objUid) const;

  // Any problems about canceling the DDL locks during the build?
  BOOL DidDDLLockErrorsHappen() const { return ddlLockHandler_.DidDDLLockErrorsHappen(); }

  CDDSqlNode *GetSqlNode() { return &sqlNode_; }
 ...
 public:
  ...
  TInt32 GetMaxParallelism() const { return maxParallelism_; }

  // Maximum number of MVs to be pipelined in one statement
  TInt32 GetMaxPipelining() const { return maxPipelining_; }

  //-- FetchDefaults() callee
  // Also called from LogCleanup task.
  static void FetchSingleDefault(CDSString &name, TInt32 &to);
 ...
 private:
  ...
  void UpdateForceOptionForMV(CRUForceOptions &forceOptions, CRUMV *pMV);
 ...
 private:
  CDDSchema *GetDDSchemaByName(const CDSString &catName, const CDSString &schName);
  ...
  CDDTable *GetDDIUDLogTableByName(CDDSchema *pddSch, const CDSString &logName);

  CDDMVGroup *GetDDMVGroupByName(CDDSchema *pddSch, const CDSString &mvgName);
