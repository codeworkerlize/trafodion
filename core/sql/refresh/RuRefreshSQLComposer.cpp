...
CRURefreshSQLComposer::CRURefreshSQLComposer(CRURefreshTask *pTask)
    : inherited(), pTask_(pTask), pRootMV_(&(pTask->GetRootMV())) {}
...
void CRURefreshSQLComposer::StartInternalRefresh() {
  if (NULL == CRUGlobals::GetInstance()->GetOptions().FindDebugOption(CRUGlobals::DISPLAY_REFRESH, "")) {
...
void CRURefreshSQLComposer::AddDeltaDefClause(const CRUDeltaDef *pDdef, CDSString &fromEpoch, CDSString &toEpoch) {
...
void CRURefreshSQLComposer::AddRngDeltaStatisticsClause(const CRUDeltaDef *pDdef) {
  sql_ += "\n\t";
...
void CRURefreshSQLComposer::AddIUDDeltaStatisticsClause(const CRUDeltaDef *pDdef) {
  sql_ += "\n\t";
...
void CRURefreshSQLComposer::AddUpdatedColumnsClause(const CRUUpdateBitmap *pUpdateBitmap) {
  sql_ += "COLUMNS (";
...
//
//--------------------------------------------------------------------------//
void CRURefreshSQLComposer::ComposeCntrlTableMDAMText(CRUForceOptions::MdamOptions mdamOption, const CDSString *pName) {
  sql_ = "CONTROL TABLE ";
...
void CRURefreshSQLComposer::ComposeDeleteContextLogTable() { sql_ = " DELETE FROM TABLE " + GetContextLogName(); }
...
//--------------------------------------------------------------------------//
void CRURefreshSQLComposer::ComposeControlQueryShapeCut() { sql_ = "CONTROL QUERY SHAPE CUT;"; }
