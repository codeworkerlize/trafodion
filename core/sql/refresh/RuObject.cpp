...
CRUObject::CRUObject() : isInvolved_(FALSE), isDDLLockPending_(FALSE), canReleaseDDLLock_(TRUE), ex_() {}
...
void CRUObject::ReleaseDDLLock() {
  ...
  CRUGlobals::GetInstance()->LogDebugMessage(CRUGlobals::DUMP_DDL_LOCKS, "", msg);
#endif
...
void CRUObject::CancelDDLLock(BOOL isCancelOnly) {
  ...
  switch (pLockList->GetCount()) {
    ...
    case 1: {
      ...
      if (TRUE == isCancelOnly && FALSE == CanReleaseDDLLock()) {
        // Condition #4
