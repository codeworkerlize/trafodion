...
class REFRESH_LIB_CLASS CRURefreshTaskExecutor : public CRUTaskExecutor {
 ...
 public:
  CRURefreshTask *GetRefreshTask() { return (CRURefreshTask *)GetParentTask(); }

  const CDSString &GetRootMVName() const { return rootMVName_; }

  TInt64 GetRootMVUID() const { return rootMVUID_; }

  const CDSString &GetRootMVSchema() const { return rootMVSchema_; }

  const CDSString &GetRootMVCatalog() const { return rootMVCatalog_; }

  CDDObject::EMVRefreshType GetRootMVType() const { return rootMVType_; }

  virtual BOOL IsRecompute() const { return isRecompute_; }
 ...
 protected:
  CRUMV &GetRootMV() { return GetRefreshTask()->GetRootMV(); }

  void SetRecompute(BOOL flag) { isRecompute_ = flag; }
 ...
 private:
  ...
  void UpdateNonRootMVStatistics(CRUMV *pCurrMV, CRUMV *pPrevMV, TInt32 currDuration);

 private:
  // For a single delta refresh we use special calculations
  // for the statistics
  void UpdateSingleDeltaStatistics(CRUDeltaDef *pDdef, TInt32 currDuration);
...
class REFRESH_LIB_CLASS CRUSingleTableLockProtocol {
 ...
 public:
  ...
  BOOL IsEnsureUsedTableLockContinuityNeeded() const { return NULL != pUsedTblFileNameList_; }
...
class REFRESH_LIB_CLASS CRUTableLockProtocol {
 public:
  CRUTableLockProtocol() : LocksList_(NULL), NothingToLock_(TRUE) {}

  virtual ~CRUTableLockProtocol() {
    if (NULL != LocksList_) delete LocksList_;
  }
