...
const CRUDeltaStatisticsMap &CRUDupElimTaskExecutor::GetStatisticsMap() const {
...
//
//--------------------------------------------------------------------------//
void CRUDupElimTaskExecutor::LoadRequest(CUOFsIpcMessageTranslator &translator) {
  ...
  if (TRUE == flag) {
    pSingleRowResolver_ = new CRUDupElimSingleRowResolver(globals_, ctrlStmtContainer_);
  ...
  translator.ReadBlock(&flag, sizeof(BOOL));
  if (TRUE == flag) {
    pRangeResolver_ = new CRUDupElimRangeResolver(globals_, ctrlStmtContainer_);
...
//--------------------------------------------------------------------------//
void CRUDupElimTaskExecutor::StoreReply(CUOFsIpcMessageTranslator &translator) {
...
//--------------------------------------------------------------------------//
void CRUDupElimTaskExecutor::LoadReply(CUOFsIpcMessageTranslator &translator) {
...
//
//--------------------------------------------------------------------------//
void CRUDupElimTaskExecutor::StoreRequest(CUOFsIpcMessageTranslator &translator) {
...
void CRUDupElimTaskExecutor::InitGlobals(CRUDupElimTask *pParentTask) {
  ...
  globals_.Init(isRangeResolv, isSingleRowResolv, updateBmpSize, nCtrlColumns, tbl.GetKeyColumnList().GetCount(),
                ...
                tbl.GetRangeLogType(), tbl.IsLastDEComplete(), pParentTask->IsSkipCrossTypeResoultion());
}
...
void CRUDupElimTaskExecutor::InitTxnTimeLimit() {
  ...
  if (NULL != options.FindDebugOption(CRUGlobals::SHRINK_DE_TXN_TIMELIMIT, "")) {
...
void CRUDupElimTaskExecutor::FinalMetadataUpdate() {
  RUASSERT(GetState() == EX_EPILOGUE && FALSE == IsTransactionOpen());
...
void CRUDupElimTaskExecutor::PrepareSQL() {
  ...
  CDMPreparedStatement *pStmt = ctrlStmtContainer_.GetPreparedStatement(CRUDupElimConst::FORCE_MDAM_CT);
  ...
  switch (phase_) {
    ...
    default:
      RUASSERT(FALSE);  // Not supposed to be called
  }

  //	CONTROL TABLE * RESET
  pStmt = ctrlStmtContainer_.GetPreparedStatement(CRUDupElimConst::RESET_MDAM_CT);
  pStmt->ExecuteUpdate(TRUE /* special syntax*/);
  pStmt->Close();
}
...
BOOL CRUDupElimTaskExecutor::CanCompletePhase() {
  ...
  if (NULL != pSingleRowResolver_) {
    ...
    if (NULL != pRangeResolver_ && TRUE == pRangeResolver_->IsRangeCollectionEmpty()) {
...
void CRUDupElimTaskExecutor::DumpPerformanceStatistics() {
  ...
  if (NULL != options.FindDebugOption(CRUGlobals::DUMP_DE_STATISTICS, "")) {
    ...
    CDSString str("Task " + GetParentTask()->GetTaskName() + " has been completed in " + CDSString(txnStr) +
