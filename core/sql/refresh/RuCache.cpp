...
void CRUCache::FetchInvolvedMVsMetadata() {
  ...
  CDDSchema *pddSch = GetDDSchemaByName(options.GetCatalogName(), options.GetSchemaName());
...
void CRUCache::FetchUsedObjectsMetadataForSingleMV(CRUMV *pMV) {
  ...
  while (NULL != pos) {
    ...
    if (TRUE == pMV->IsIgnoreChanges(objUid) || TRUE == pMV->IsUsedObjectUDF(objUid)) {
      continue;
    }
...
CRUTbl *CRUCache::FetchSingleUsedObject(CRUMV *pUsingMV, const CDDUIDTriple &uidt) {
  CRUTbl *pTbl;
...
void CRUCache::FetchNonInvolvedUsedObjectsMetadata() {
  ...
  while (NULL != pos) {
    CRUMV *pMV = mvList_.GetNext(pos);
    if (FALSE == pMV->IsInvolved() || CDDObject::eON_REQUEST != pMV->GetRefreshType()) {
      continue;
    }

    if (NULL == GetTable(pMV->GetUID())) {
      CDDSchema *pddSch = GetDDSchemaByName(pMV->GetCatName(), pMV->GetSchName());
...
void CRUCache::CheckLogCleanupApplicability() {
  switch (lcType_) {
    ...
    case CRUOptions::DONTCARE_LC: {
      lcType_ = (TRUE == IsOnRequestMVInvolved()) ? CRUOptions::WITH_LC : CRUOptions::WITHOUT_LC;
...
BOOL CRUCache::IsOnRequestMVInvolved() {
  ...
  while (NULL != pos) {
    ...
    if (TRUE == pMV->IsInvolved() && CDDMV::eON_REQUEST == pMV->GetRefreshType()) {
      return TRUE;
    }
  }
...
void CRUCache::FetchDefaults() {
  ...
  FetchSingleDefault(maxPiplining, maxPipelining_);

  CDSString maxParallelism("MV_REFRESH_MAX_PARALLELISM");
  FetchSingleDefault(maxParallelism, maxParallelism_);

  if (0 == maxParallelism_ || maxParallelism_ > CUOFsTaskProcessPool::MaxProcesses) {
...
void CRUCache::FetchSingleDefault(CDSString &name, TInt32 &to) {
  to = 1;
...
void CRUCache::FetchForceData() {
  ...
  forceParser.SetFile(CRUGlobals::GetInstance()->GetOptions().GetForceFilename());
...
//--------------------------------------------------------------------------//
void CRUCache::UpdateForceOptionForMV(CRUForceOptions &forceOptions, CRUMV *pMV) {
  CRUMVForceOptionsList &mvForceOptionsList = forceOptions.GetMVForceOptionsList();
...
CDDSchema *CRUCache::GetDDSchemaByName(const CDSString &catName, const CDSString &schName) {
  // check for DEFAULT_SCHEMA_ACCESS_ONLY
  // get default catalog & schema
  CRUOptions &options = CRUGlobals::GetInstance()->GetOptions();
  CDSString curCatSch = catName + "." + schName;
  if (CDDControlQueryDefault::ViolateDefaultSchemaAccessOnly(options.GetDefaultSchema(), curCatSch)) {
    CRUException e;
...
CDDMV *CRUCache::GetDDMVByName(CDDSchema *pddSch, const CDSString &mvName) {
  ...
  if (NULL == pddMV)  // try public schema if necessary
  {
    ...
    if (pddSchP) {
      ...
      if (pddMV) {  // if can be found in the public schema
...
CDDTable *CRUCache::GetDDIUDLogTableByName(CDDSchema *pddSch, const CDSString &logName) {
  ...
  if (NULL == pddTable)  // try public schema if necessary
  {
    ...
    if (pddSchP) {
      ...
      if (pddTable) {  // if can be found in the public schema
        // update the catalog and schema
        CRUOptions &options = CRUGlobals::GetInstance()->GetOptions();
        options.SetCatalogName(pddSchP->GetCatName());
        options.SetSchemaName(pddSchP->GetName());
...
CDDMVGroup *CRUCache::GetDDMVGroupByName(CDDSchema *pddSch, const CDSString &mvgName) {
  ...
  if (NULL == pddMVG)  // try public schema if necessary
  {
    ...
    if (pddSchP) {
      ...
      if (pddMVG) {  // if can be found in the public schema
        // update the catalog and schema
        CRUOptions &options = CRUGlobals::GetInstance()->GetOptions();
        options.SetCatalogName(pddSchP->GetCatName());
        options.SetSchemaName(pddSchP->GetName());
