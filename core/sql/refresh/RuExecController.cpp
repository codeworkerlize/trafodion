...
CRUExecController::CRUExecController(BOOL useParallelism)
    : inherited(),
      ...
      processPool_(CRUExecController::taskServerName_, CUOFsTaskProcess::MXUTP, CUOFsTaskProcess::OSS,
...
void CRUExecController::HandleStartTaskRqst(CRURuntimeControllerRqst *pRqst) {
  ...
  this->PostRequest(new CRURuntimeControllerRqst(CRURuntimeControllerRqst::EXECUTE_TASK_STEP, &task));
}
...
void CRUExecController::HandleFinishTaskRqst(CRURuntimeControllerRqst *pRqst) {
  ...
  GetPeerController()->PostRequest(new CRURuntimeControllerRqst(CRURuntimeControllerRqst::FINISH_TASK, &task));
}
...
void CRUExecController::HandleExecuteTaskStepRqst(CRURuntimeControllerRqst *pRqst) {
  ...
  if (TRUE == useParallelism_ && TRUE == task.GetExecutor().IsNextStepRemotelyExecutable()) {
...
void CRUExecController::HandleAwaitEventRqst() {
  ...
  for (;;) {
    try {
      ...
      CUOFsTransManager &tm = CRUGlobals::GetInstance()->GetTransactionManager();
...
void CRUExecController::HandleLocalTaskStepExecution(CRUTask &task) {
  ...
  if (CRUTaskExecutor::EX_COMPLETE == taskEx.GetState()) {
    ...
    pRqst = new CRURuntimeControllerRqst(CRURuntimeControllerRqst::FINISH_TASK, &task);
  ...
  pRqst = new CRURuntimeControllerRqst(CRURuntimeControllerRqst::EXECUTE_TASK_STEP, &task);
  this->PostRequest(pRqst);
}
...
void CRUExecController::HandleRemoteTaskStepExecution(CRUTask &task) {
  ...
  GetPeerController()->PostRequest(new CRURuntimeControllerRqst(CRURuntimeControllerRqst::SCHEDULE));
}
...
void CRUExecController::HandleRequestFailure(CRURuntimeControllerRqst *pRqst, CDSException &ex) {
...
void CRUExecController::HandleTaskFailure(CRUTask &task, CDSException &ex) {
  ...
  // Post a request to myself to handle the task's completion
  this->PostRequest(new CRURuntimeControllerRqst(CRURuntimeControllerRqst::FINISH_TASK, &task));
}
...
void CRUExecController::SerializeTaskExecutor(CRUTaskExecutor &executor) {
  ...
  for (;;) {
    ...
    try {
      ...
      if (pTranslator->GetBufferSize() >= CUOFsIpcMessageTranslator::MaxMsgSize) {
...
void CRUExecController::HandleReturnOfRemoteExecutor(int pid) {
  ...
  if (TRUE == translator.IsSystemMessage()
      ...
      || CUOFsIpcMessageTranslator::APPLICATION_ERROR == translator.GetMessageType()
...
void CRUExecController::HandleRemoteExecutorSuccess(CRUTask *pTask, CUOFsIpcMessageTranslator &translator) {
  CRUTaskExecutor &executor = pTask->GetExecutor();
  ...
  CRURuntimeControllerRqst *pRqst = new CRURuntimeControllerRqst(CRURuntimeControllerRqst::EXECUTE_TASK_STEP, pTask);
...
void CRUExecController::ShutDownTaskProcesses() { processPool_.ShutdownAllTaskProcesses(); }
