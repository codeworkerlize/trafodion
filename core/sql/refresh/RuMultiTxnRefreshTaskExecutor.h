...
class REFRESH_LIB_CLASS CRUMultiTxnRefreshTaskExecutor : public CRURefreshTaskExecutor {
 private:
 ...
 protected:
  ...
  virtual void FinalMetadataUpdate() { inherited::FinalMetadataUpdate(); }
 ...
 private:
  ...
  enum SQL_STATIC_STATMENTS {
    UPDATE_CTX = 0  // update target epoch in MVS_UMD
  };

  enum PHASES { PHASE_0 = 0, PHASE_1 = 1, NEED_CATCHUP = TRUE, DONT_NEED_CATCHUP = FALSE };
 ...
 private:
  ...
  CDMPreparedStatement *CompileRefresh(SQL_STATEMENT type, TInt32 beginEpoch, TInt32 endEpoch, TInt32 catchupEpoch = 0);
  ...
  enum MoreTxnStatus { NO_MORE_TXN_NEEDED = FALSE, MORE_TXN_NEEDED = TRUE };
 ...
 private:
  ...
  CRUMultiTxnRefreshTaskExecutor &operator=(const CRUMultiTxnRefreshTaskExecutor &other);
