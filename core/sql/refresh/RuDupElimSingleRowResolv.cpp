...
CRUDupElimSingleRowResolver::CRUDupElimSingleRowResolver(const CRUDupElimGlobals &globals,
                                                         CRUSQLDynamicStatementContainer &ctrlStmtContainer)
    : inherited(globals, ctrlStmtContainer, CRUDupElimConst::NUM_SINGLE_RESOLV_STMTS),
      // Automaton status
...
void CRUDupElimSingleRowResolver::Resolve(CRUDupElimLogScanner &scanner) {
  ...
  if (NULL == pPrevRec && NULL == pCurrentRec && TRUE == scanner.IsEntireDeltaScanned()) {
...
void CRUDupElimSingleRowResolver::DumpPerformanceStatistics(CDSString &to) const {
  CDSString msg;
...
void CRUDupElimSingleRowResolver::ResolveInsert(CRUIUDLogRecord *pPrevRec) {
...
void CRUDupElimSingleRowResolver::ResolveDelete(CRUIUDLogRecord *pCurrentRec, CRUIUDLogRecord *pPrevRec) {
...
void CRUDupElimSingleRowResolver::MonitorUpdate(CRUIUDLogRecord *pCurrentRec) {
...
void CRUDupElimSingleRowResolver::HandleUpdateBitmapAtEndOfChain(CRUIUDLogRecord *pLastRecInChain) {
...
void CRUDupElimSingleRowResolver::ExecuteDeleteRecord(CRUIUDLogRecord *pRec) {
  CDMPreparedStatement *pStmt = GetPreparedStatement(CRUDupElimConst::IUD_LOG_SINGLE_DELETE);
  RUASSERT(NULL != pStmt);
...
void CRUDupElimSingleRowResolver::ExecuteUpdateIgnMark(CRUIUDLogRecord *pRec) {
  CDMPreparedStatement *pStmt = GetPreparedStatement(CRUDupElimConst::IUD_LOG_SINGLE_UPDATE_IGNORE);
  RUASSERT(NULL != pStmt);
...
void CRUDupElimSingleRowResolver::ExecuteUpdateBitmapInChain(CRUIUDLogRecord *pLastRecInChain) {
  CDMPreparedStatement *pStmt = GetPreparedStatement(CRUDupElimConst::IUD_LOG_UPDATE_BITMAP);
  RUASSERT(NULL != pStmt);
...
void CRUDupElimSingleRowResolver::ExecuteUpdateOpTypeInChain(CRUIUDLogRecord *pLastRecInChain) {
  CDMPreparedStatement *pStmt = GetPreparedStatement(CRUDupElimConst::IUD_LOG_UPDATE_OPTYPE);
  RUASSERT(NULL != pStmt);
