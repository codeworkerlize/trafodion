...
CRUTaskExecutor *CRURefreshTask::CreateExecutorInstance() {
  ...
  if (rootMV_.GetAuditType() != CDDObject::eAUDIT) {
    ...
    if (IsRecompute() || rootMV_.GetCommitNRows() == 0 || rootMV_.GetMVStatus() != CDDObject::eINITIALIZED) {
...
TInt32 CRURefreshTask::GetComputedCost() const {
  ...
  CRUOptions::DebugOption *pDO =
      CRUGlobals::GetInstance()->GetOptions().FindDebugOption(CRUGlobals::IGNORE_STATISTICS, "");
...
void CRURefreshTask::PullDataFromExecutor() {
  ...
  CRURefreshTaskExecutor &taskEx = (CRURefreshTaskExecutor &)GetExecutor();
...
void CRURefreshTask::PullDataFromExecutorAfterIncRefresh(CRURefreshTaskExecutor &taskEx) {
...
void CRURefreshTask::PullDataFromExecutorAfterRecompute(CRURefreshTaskExecutor &taskEx) {
...
TInt32 CRURefreshTask::GetSingleDeltaCost() const {
  ...
  TInt32 prevCost = rootMV_.GetRefreshDurationEstimate(pDdef->tblUid_);
