...
void CRUTaskServerExecController::Init(CUOFsIpcMessageTranslator &translator, CUOFsTransManager &transManager_) {
  if (NULL == pInstance_) {
  ...
  CRUGlobals::Init(pInstance_->options_, *(pInstance_->pJournal_), transManager_);
...
void CRUTaskServerExecController::LoadExecutor(CUOFsIpcMessageTranslator &translator) {
  try {
...
void CRUTaskServerExecController::Run() {
  try {
    ...
    if (CUOFsIpcMessageTranslator::RU_TEST_EXECUTOR == pTranslator_->GetMessageType()) {
      if (CRUTaskExecutor::EX_COMPLETE != pTaskExecutor_->GetState()) {
        ...
        pTranslator_->SetMessageType(CUOFsIpcMessageTranslator::RU_TEST_SYNC);
        return;
      }
    }
