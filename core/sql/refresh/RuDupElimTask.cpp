...
BOOL CRUDupElimTask::NeedToExecuteDE() const { return (TRUE == isRangeResolv_ || TRUE == isSingleRowResolv_); }
...
void CRUDupElimTask::SetupDS() {
  ...
  if (TRUE == ecVector.IsDeltaNonEmpty(GetBeginEpoch(), CRUTbl::RANGE)) {
    isRangeResolv_ = TRUE;
  }

  BOOL isSingleDeltaNonEmpty = ecVector.IsDeltaNonEmpty(GetBeginEpoch(), CRUTbl::SINGLE_ROW);
  ...
  if (TRUE == IsSingleRowResolutionEnforced() && TRUE == isSingleDeltaNonEmpty && FALSE == tbl.IsInsertLog()) {
  ...
  isSkipCrossTypeResolution_ = (FALSE == isCrossTypeResolutionEnforced || FALSE == isSingleDeltaNonEmpty);
...
BOOL CRUDupElimTask::IsSingleRowResolutionEnforced() const {
  BOOL isSingleRowResolutionEnforced = GetTable().IsUsedByIncrementalMJV();
