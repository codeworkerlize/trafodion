...
CRUUnAuditRefreshTaskExecutor::CRUUnAuditRefreshTaskExecutor(CRURefreshTask *pParentTask)
...
void CRUUnAuditRefreshTaskExecutor::StoreRequest(CUOFsIpcMessageTranslator &translator) {
  ...
  translator.SetMessageType(CUOFsIpcMessageTranslator::RU_UNAUDIT_REFRESH_EXECUTOR);
}
...
void CRUUnAuditRefreshTaskExecutor::LoadRequest(CUOFsIpcMessageTranslator &translator) {
  ...
  if (0 < numOfIndexes_) {
    pUnAuditPopIndexdynamicContainer_ = new CRUSQLDynamicStatementContainer(numOfIndexes_);
    pUnAuditAvailableIndeXdynamicContainer_ = new CRUSQLDynamicStatementContainer(numOfIndexes_);
    pUnAuditUnavailableIndeXdynamicContainer_ = new CRUSQLDynamicStatementContainer(numOfIndexes_);
...
void CRUUnAuditRefreshTaskExecutor::ComposeMySql() {
  ...
  unAuditRefreshTEDynamicContainer_.SetStatementText(RU_UNLOCK_TABLE, myComposer.GetSQL());
...
//--------------------------------------------------------------------------//
void CRUUnAuditRefreshTaskExecutor::ExecuteIndexStatmenents(CRUSQLDynamicStatementContainer &container,
                                                            int errorCode) {
...
void CRUUnAuditRefreshTaskExecutor::SetObjectsUnavailable() {
  if (TRUE == isPopindex_ && 0 < numOfIndexes_) {
    ...
    ExecuteIndexStatmenents(*pUnAuditUnavailableIndeXdynamicContainer_, IDS_RU_INDEXSTATUS_FAILED);
  }
...
void CRUUnAuditRefreshTaskExecutor::ResetObjectsAvailable() {
  ...
  if (TRUE == isPopindex_ && 0 < numOfIndexes_) {
    ...
    ExecuteIndexStatmenents(*pUnAuditAvailableIndeXdynamicContainer_, IDS_RU_INDEXSTATUS_FAILED);
  }
}
...
void CRUUnAuditRefreshTaskExecutor::UnLockMV() {
  CDMPreparedStatement *pStmt = unAuditRefreshTEDynamicContainer_.GetPreparedStatement(RU_UNLOCK_TABLE);
