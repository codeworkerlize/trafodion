...
CRUSimpleRefreshTaskExecutor::CRUSimpleRefreshTaskExecutor(CRURefreshTask *pParentTask)
...
//--------------------------------------------------------------------------//
void CRUSimpleRefreshTaskExecutor::StoreRequest(CUOFsIpcMessageTranslator &translator) {
...
//--------------------------------------------------------------------------//
void CRUSimpleRefreshTaskExecutor::StoreReply(CUOFsIpcMessageTranslator &translator) {
...
//--------------------------------------------------------------------------//
void CRUSimpleRefreshTaskExecutor::LoadRequest(CUOFsIpcMessageTranslator &translator) {
...
//--------------------------------------------------------------------------//
void CRUSimpleRefreshTaskExecutor::LoadReply(CUOFsIpcMessageTranslator &translator) {
...
void CRUSimpleRefreshTaskExecutor::ComposeMySql() {
  ...
  if (FALSE == isRecompute) {
    ...
    simpleRefreshTEDynamicContainer_.SetStatementText(INTERNAL_REFRESH, myComposer.GetSQL());
  }
  ...
  simpleRefreshTEDynamicContainer_.SetStatementText(RECOMPUTE, myComposer.GetSQL());

  // The LOCK TABLE ... statement
  myComposer.ComposeLock(GetRootMVName(), TRUE);
  simpleRefreshTEDynamicContainer_.SetStatementText(RU_LOCK_TABLE, myComposer.GetSQL());
}
...
CDMPreparedStatement *CRUSimpleRefreshTaskExecutor::PrepareRecomputeMV() {
  ...
  CDMPreparedStatement *pStat = simpleRefreshTEDynamicContainer_.GetPreparedStatement(RECOMPUTE);
...
CDMPreparedStatement *CRUSimpleRefreshTaskExecutor::PrepareSingleDeltaRefresh() {
  ...
  CDMPreparedStatement *pStat = simpleRefreshTEDynamicContainer_.GetPreparedStatement(INTERNAL_REFRESH);
...
BOOL CRUSimpleRefreshTaskExecutor::CompileMultiDeltasIRefresh(CDMPreparedStatement *&pStat) {
  simpleRefreshTEDynamicContainer_.SetCompiledTimeParam(INTERNAL_REFRESH, 0, CRUSQLComposer::TInt32ToStr(numOfPhases_));

  try {
    // The FALSE is to avoid deleting used statements.
    // They will be deleted later in the list.
    pStat = simpleRefreshTEDynamicContainer_.GetPreparedStatement(INTERNAL_REFRESH, FALSE);
  } catch (CDSException &ex) {
    if (ex.GetErrorCode(0) == MIN_MAX_RECOMPUTATION_NEEDED || ex.GetErrorCode(0) == MULTIDELTA_RECOMPUTATION_NEEDED) {
    ...
    ex.AddArgument(simpleRefreshTEDynamicContainer_.GetLastSQL(INTERNAL_REFRESH));
    throw ex;  // Re-throw
  }
...
void CRUSimpleRefreshTaskExecutor::FinalMetadataUpdate() {
  ...
  if (CDDObject::eINITIALIZED != rootMV.GetMVStatus()) {
    RUASSERT(CDDObject::eNON_INITIALIZED == rootMV.GetMVStatus() || CDDObject::eUNAVAILABLE == rootMV.GetMVStatus());
...
void CRUSimpleRefreshTaskExecutor::LockMV() {
  CDMPreparedStatement *pStat = simpleRefreshTEDynamicContainer_.GetPreparedStatement(RU_LOCK_TABLE);
