...
CRUMV::CRUMV(CDDMV *pddMV)
    : pddMV_(pddMV),
      ...
      pddIndexList_(new CDDIndexList(eItemsArentOwned)),
      // Usage lists
      pTablesUsedByMe_(new CRUTblList(eItemsArentOwned)),
      pFullySyncTablesUsedByMe_(new CRUTblList(eItemsArentOwned)),
      //
      pDeltaDefList_(new CRUDeltaDefList()  // the items are owned
                     ),
      pTblInterface_(NULL),
...
BOOL CRUMV::IsSingleDeltaRefresh() const { return (1 == GetDeltaDefList().GetCount()); }
...
void CRUMV::BuildIndexList() {
  ...
  while (NULL != pos) {
    ...
    if (TRUE == pddMV_->HasIndex(pddIndex->GetUID() /* No I/O here !*/)) {
      ...
      RUASSERT(pddMV_ == pddMV && TRUE == pddIndex->GetDDLLockList()->IsEmpty());
...
void CRUMV::GetPopIndexCatApiRequestText(CDSString &to, CDDIndex *pddIndex) {
  ...
  CDSString ddlLockName = GetCatName() + "." + GetSchName() + "." + pddLockList->GetAt(0)->GetName();
...
void CRUMV::GetUpdateIndexStatusCatApiRequestText(CDSString &to, BOOL isAvail, CDDIndex *pddIndex) {
  RUASSERT(FALSE == pddIndexList_->IsEmpty());
  to = "";
...
void CRUMV::GetToggleAuditCatApiRequestText(CDSString &to, BOOL auditFlag, CDDIndex *pddIndex) {
  RUASSERT(FALSE == pddIndexList_->IsEmpty());
  to = "";
...
void CRUMV::PropagateRecomputeProperty() {
  ...
  while (NULL != pos) {
    ...
    if (TRUE == pTbl->IsInvolvedMV() && TRUE == pTbl->GetMVInterface()->WillBeRecomputed()) {
      SetRefreshPatternMask(RECOMPUTE);
      break;
    }
  }
...
void CRUMV::PropagateEmpCheck(CRUTbl &tbl) {
  ...
  BOOL isIUDLogNonEmpty = ecVector.IsDeltaNonEmpty(beginEpoch, CRUTbl::SINGLE_ROW);

  BOOL isRangeLogNonEmpty = ecVector.IsDeltaNonEmpty(beginEpoch, CRUTbl::RANGE);

  BOOL isIUDLogInsertOnly = ecVector.IsDeltaInsertOnly(beginEpoch);

  if (FALSE == isIUDLogNonEmpty && FALSE == isRangeLogNonEmpty) {
...
BOOL CRUMV::FetchPrivilege(CDDObject::EPrivilegeType privType) {
  ...
  if (IsSuperID() || sessionInfo.BelongsToSuperRole()) return TRUE;
...
void CRUMV::SetupPurgedataAndPopindex() {
  ...
  if (TRUE == isPurgedata || CDDObject::eNON_INITIALIZED == status) {
