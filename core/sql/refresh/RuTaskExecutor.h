...
class REFRESH_LIB_CLASS CRUTaskExecutor {
 ...
 public:
  ...
  BOOL IsNextStepRemotelyExecutable() const { return state_ >= REMOTE_STATES_START; }

  BOOL IsInTaskProcess() const { return NULL == pParentTask_; }
...
void CRUTaskExecutor::LoadReply(CUOFsIpcMessageTranslator &translator) { LoadData(translator); }

// Used in the remote process side
void CRUTaskExecutor::LoadRequest(CUOFsIpcMessageTranslator &translator) { LoadData(translator); }

void CRUTaskExecutor::StoreReply(CUOFsIpcMessageTranslator &translator) { StoreData(translator); }
