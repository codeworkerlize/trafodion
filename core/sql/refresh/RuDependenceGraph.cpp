...
CRUDependenceGraph::CRUDependenceGraph(TInt32 maxPipelining)
    : allTaskList_(eItemsAreOwned), availableTaskList_(eItemsArentOwned), maxPipelining_(maxPipelining) {}
...
CRUTask *CRUDependenceGraph::GetTask(TInt64 objUid, CRUTask::Type type) {
  ...
  while (NULL != pos) {
    ...
    if (TRUE == pTask->HasObject(objUid) && type == pTask->GetType()) {
      return pTask;
    }
  }
...
void CRUDependenceGraph::RemoveFromAvailableList(CRUTask *pTask) {
  DSListPosition pos = availableTaskList_.FindTaskPos(pTask->GetId());
  RUASSERT(NULL != pos);
...
void CRUDependenceGraph::ScheduleNormalTask() {
  ...
  for (; (pTask = it.GetCurrentTask()) != NULL; it.Next()) {
    ...
    if (FALSE == pTask->IsReady() || TRUE == pTask->IsRunning()) {
      continue;
    }
...
void CRUDependenceGraph::ConsiderTaskForScheduling(CRUTask *pTask) {
  ...
  if (CRUTask::REFRESH == pTask->GetType() && FALSE == AllEmpCheckTasksComplete()) {
    return;
  }
...
CRUDependenceGraph::TypePriority CRUDependenceGraph::GetTypePriority(CRUTask *pTask) {
...
void CRUDependenceGraph::BuildPipelinedTasksList(CRUTaskList &pplTaskList) {
  ...
  for (;;) {
    ...
    if (FALSE == topMV.CanPipelineFromMe(isRoot, nextCandidateUid /*by ref*/)) {
      break;
    }
    ...
    pNewTopTask = (CRURefreshTask *)GetTask(nextCandidateUid, CRUTask::REFRESH);
