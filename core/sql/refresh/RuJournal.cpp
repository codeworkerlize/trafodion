...
void CRUJournal::Close() {
  ...
  if ((!emsOnlyLog_) && (logfile_.IsOpen())) logfile_.Close();
}
...
void CRUJournal::LogMessage(const CDSString &msg, BOOL printRowNum,
  ...
  if (TRUE == printRowNum) {
    ...
    CDSString newString;
    if (logfile_.GetTimePrint()) newString = +logfile_.GetTimeToLog().c_string();
  ...
  // if the user specified an OUTFILE option and the file is open
  if ((!emsOnlyLog_) && (logfile_.IsOpen())) logfile_.Flush();
}
