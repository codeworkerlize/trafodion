...
class REFRESH_LIB_CLASS CRUTbl : public CRUObject {
 ...
 public:
  ...
  BOOL IsInvolvedMV() const { return (NULL != pMVInterface_); }

  CRUMV *GetMVInterface() const { return pMVInterface_; }
 ...
 public:
  ...
  virtual BOOL HoldsResources() const {
    return (TRUE == IsDDLLockPending() || TRUE == IsRPOpenPending() || TRUE == IsLogRPOpenPending());
  }

  TInt64 GetTimestamp() const { return timestamp_; }

  BOOL IsRPOpenPending() const { return isRPOpenPending_; }
 ...
 public:
  ...
  BOOL IsUsedByIncrementalMJV() const { return isUsedByIncrementalMJV_; }

  BOOL IsUsedOnlyByMultiTxnMvs() const;

  // Should the table be locked (by read-protected open)
  // throughout the execution?
  BOOL IsLongLockNeeded() const { return isLongLockNeeded_; }

  // Must the table's delta be empty for consistency reasons?
  BOOL IsEmptyDeltaNeeded() const { return isEmptyDeltaNeeded_; }
 ...
 public:
  virtual TInt64 GetUID() const { return pddTblUsedByMV_->GetUID(); }

  BOOL IsNoLockOnRefresh() { return pddTblUsedByMV_->IsNoLockOnRefresh(); }

  CDDObject::ERangeLogType GetRangeLogType() { return pddTblUsedByMV_->GetRangeLogType(); }

  CDDUIDTripleList &GetUIDsOfAllMVsUsingMe() { return pddTblUsedByMV_->GetMVsUsingMe(); }

  TInt32 GetCurrentEpoch() { return pddTblUsedByMV_->GetCurrentEpoch(); }

  TInt32 GetLastDupElimEpoch() { return pddTblUsedByMV_->GetLastDupElimEpoch(); }

  TInt32 GetRecomputeEpoch() { return pddTblUsedByMV_->GetRecomputeEpoch(); }

  BOOL IsLastDEComplete() { return pddTblUsedByMV_->IsLastDEComplete(); }

  // For direct epoch increment (not through DDOL)
  CDSString GetIncEpochCatApiText() { return pddTblUsedByMV_->GetIncEpochCatApiText(); }
 ...
 public:
  ...
  CDSString GetIUDLogFullName(BOOL useNmsp = TRUE) { return GetLogFullName(CRUTbl::iudNmspPrefix, useNmsp); }

  CDSString GetRangeLogFullName(BOOL useNmsp = TRUE) { return GetLogFullName(CRUTbl::rngNmspPrefix, useNmsp); }
 ...
 public:
  ...
  void SetMVInterface(CRUMV *pMV) { pMVInterface_ = pMV; }

  // Store the pointer to the IUD log table
  void FixupIUDLogTbl(CDDTable *pddIUDLogTbl) { pddIUDLogTbl_ = pddIUDLogTbl; }
 ...
 public:
  ...
  void SetLongLockIsNeeded() { isLongLockNeeded_ = TRUE; }

  void CheckIfDENeeded();

  virtual void SetTimestamp(TInt64 ts) { timestamp_ = ts; }
  void SetEmptyDeltaNeeded() { isEmptyDeltaNeeded_ = TRUE; }

  void SetDELevel(CRUDeltaDef::DELevel deLevel) { deLevel_ = deLevel; }
 ...
 public:
  ...
  virtual void FetchMetadata() { pddTblUsedByMV_->FetchMVRefreshMetadata(); }

  // Flush the metadata changes to the SMD and UMD tables
  virtual void SaveMetadata() { pddTblUsedByMV_->Save(); }

  // Cancel saving the metadata
  virtual void CancelChanges() { pddTblUsedByMV_->CancelChanges(); }

  void IncrementCurrentEpoch(BOOL changeObjectState = TRUE) {
    pddTblUsedByMV_->IncrementCurrentEpoch(changeObjectState);
  }
  void SetLastDupElimEpoch(TInt32 ep) { pddTblUsedByMV_->SetLastDupElimEpoch(ep); }
  void SetRecomputeEpoch(TInt32 ep) { pddTblUsedByMV_->SetRecomputeEpoch(ep); }
  void SetMinLogEpoch(TInt32 ep) { pddTblUsedByMV_->SetMinLogEpoch(ep); }
  void SetLastDEComplete(BOOL flag) { pddTblUsedByMV_->SetLastDEComplete(flag); }
