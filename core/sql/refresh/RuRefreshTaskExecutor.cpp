...
CRURefreshTaskExecutor::~CRURefreshTaskExecutor() {
  ...
  if (NULL != tableLockProtocol_) delete tableLockProtocol_;
}

//--------------------------------------------------------------------------//
//	CRURefreshTaskExecutor::StoreRequest()
//--------------------------------------------------------------------------//
void CRURefreshTaskExecutor::StoreRequest(CUOFsIpcMessageTranslator &translator) {
...
//--------------------------------------------------------------------------//
void CRURefreshTaskExecutor::StoreReply(CUOFsIpcMessageTranslator &translator) {
...
//--------------------------------------------------------------------------//
void CRURefreshTaskExecutor::LoadRequest(CUOFsIpcMessageTranslator &translator) {
  ...
  if (NO_FORCE != forceFlags_) {
    ...
    pRefreshTEDynamicContainer_ = new CRUSQLDynamicStatementContainer(numOfStmtInContainer_);
    // Handle refresh executor sql dynamic container
...
//--------------------------------------------------------------------------//
void CRURefreshTaskExecutor::LoadReply(CUOFsIpcMessageTranslator &translator) {
...
void CRURefreshTaskExecutor::ComposeForceStatements() {
  ...
  if (NULL != pForceOption && CRUForceOptions::EXPLAIN_ON == pForceOption->GetExplainOption()) {
...
void CRURefreshTaskExecutor::ComposeMDAMStatements() {
  ...
  if (NULL != pForceOption && CRUForceOptions::MDAM_NO_FORCE != pForceOption->GetMDAMoption()) {
    // Compose CONTROL TABLE mv_name MDAM option
    myComposer.ComposeCntrlTableMDAMText(pForceOption->GetMDAMoption(), &rootMV.GetFullName());
...
void CRURefreshTaskExecutor::ComposeCQSStatements() {
  ...
  if (FALSE == pForceOption->GetCQSStatment().IsEmpty()) {
    pRefreshTEDynamicContainer_->SetStatementText(USER_CQS_FORCE, pForceOption->GetCQSStatment());
...
void CRURefreshTaskExecutor::ComposeControlTableStmtForUsedTable(CRUTbl &tbl, int &stmtIndex,
  ...
  if (CRUForceOptions::MDAM_NO_FORCE != mdamOpt) {
    ...
    pRefreshTEDynamicContainer_->SetStatementText(stmtIndex++, myComposer.GetSQL());
...
void CRURefreshTaskExecutor::VerifyForceTblListCorrectness() {
  const CRUMVForceOptions &forceOption = *GetRefreshTask()->GetRootMV().GetMVForceOption();
...
void CRURefreshTaskExecutor::UpdateSingleDeltaStatistics(CRUDeltaDef *pDdef, TInt32 currDuration) {
  CRUMV &rootMV = GetRootMV();
...
void CRURefreshTaskExecutor::UpdateNonRootMVStatistics(CRUMV *pCurrMV, CRUMV *pPrevMV, TInt32 currDuration) {
  ...
  TInt32 prevDuration = pCurrMV->GetRefreshDurationEstimate(tblUid);

  TInt32 nextDuration = EvaluateHeuristic(currDuration, prevDuration);
...
void CRURefreshTaskExecutor::CheckSingleDeltaRestriction() {
  ...
  while (NULL != pos) {
    ...
    if (TRUE == pTbl->IsEmptyDeltaNeeded() && TRUE == rootMV.IsDeltaNonEmpty(*pTbl)) {
...
void CRURefreshTaskExecutor::ApplyIRCompilerDefaults() {
  ...
  CRUGlobals::GetInstance()->GetJournal().LogMessage("USING THE FOLLOWING CONTROL STATMENTS :\n");
  ...
  if (FORCE_MV_MDAM & forceFlags_) {
    CDMPreparedStatement *pStat = pRefreshTEDynamicContainer_->GetPreparedStatement(MV_MDAM_STAT);

    ExecuteStatement(*pStat, MV_MDAM_STAT);

    CRUGlobals::GetInstance()->GetJournal().LogMessage(pRefreshTEDynamicContainer_->GetLastSQL(MV_MDAM_STAT));
  }
  ...
  if (FORCE_TABLE_MDAM & forceFlags_) {
    for (int i = FIRST_TBL_STAT; i < pRefreshTEDynamicContainer_->GetNumOfStmt(); i++) {
      ...
      CRUGlobals::GetInstance()->GetJournal().LogMessage(pRefreshTEDynamicContainer_->GetLastSQL(i));
    }
  }
}
...
void CRURefreshTaskExecutor::ResetIRCompilerDefaults() {
  if ((FORCE_CQS & forceFlags_) || (FORCE_USER_CQS & forceFlags_)) {
    CDMPreparedStatement *pStat = pRefreshTEDynamicContainer_->GetPreparedStatement(CQS_CUT_STAT);

    ExecuteStatement(*pStat, CQS_CUT_STAT);
  }

  if (forceFlags_ & FORCE_MV_MDAM || forceFlags_ & FORCE_TABLE_MDAM) {
    CDMPreparedStatement *pStat = pRefreshTEDynamicContainer_->GetPreparedStatement(RESET_MDAM_STAT);
...
void CRURefreshTaskExecutor::ApplyCQSForInternalRefresh() {
  CDMPreparedStatement *pStat = pRefreshTEDynamicContainer_->GetPreparedStatement(CQS_STAT);

  ExecuteStatement(*pStat, CQS_STAT);

  CRUGlobals::GetInstance()->GetJournal().LogMessage(pRefreshTEDynamicContainer_->GetLastSQL(CQS_STAT));
}
...
void CRURefreshTaskExecutor::ApplyUserCQSForInternalRefresh() {
  CDMPreparedStatement *pStat = pRefreshTEDynamicContainer_->GetPreparedStatement(USER_CQS_FORCE);

  ExecuteStatement(*pStat, USER_CQS_FORCE);

  CRUGlobals::GetInstance()->GetJournal().LogMessage(pRefreshTEDynamicContainer_->GetLastSQL(USER_CQS_FORCE));
}
...
void CRURefreshTaskExecutor::ExecuteShowExplain() {
  ...
  CDMPreparedStatement *pStat = pRefreshTEDynamicContainer_->GetPreparedStatement(SHOW_EXPLAIN);

  ExecuteStatement(*pStat, SHOW_EXPLAIN);

  CRUGlobals::GetInstance()->GetJournal().LogMessage(pRefreshTEDynamicContainer_->GetLastSQL(SHOW_EXPLAIN));
}
...
NABoolean CRURefreshTaskExecutor::StartTableLockProtocol() {
  ...
  if (TRUE == tableLockProtocol_->IsEnsureUsedTableLockContinuityNeeded() && TRUE == IsInTaskProcess()) {
...
//
//--------------------------------------------------------------------------//
void CRUTableLockProtocol::Init(CRUTblList &tblList, CRUDeltaDefList &DeltaDefList) {
...
BOOL CRUTableLockProtocol::IsEnsureUsedTableLockContinuityNeeded() const {
  if (NothingToLock_) return FALSE;
  ...
  while (pos != NULL) {
    thisLock = LocksList_->GetNext(pos);
    if (thisLock->IsEnsureUsedTableLockContinuityNeeded()) return TRUE;
  }
...
BOOL CRUTableLockProtocol::EnsureUsedTableLockContinuity() {
  ...
  while (pos != NULL) {
    thisLock = LocksList_->GetNext(pos);
    if (thisLock->EnsureUsedTableLockContinuity() == FALSE) return FALSE;
  }
...
CRUSingleTableLockProtocol::CRUSingleTableLockProtocol()
    : pUsedTblFileNameList_(NULL), pPartitionFileList_(NULL), pEmpCheck_(NULL), endEpoch_(0) {}
...
//
//--------------------------------------------------------------------------//
void CRUSingleTableLockProtocol::Init(CRUTbl *pTbl, TInt32 EndEpoch) {
  endEpoch_ = EndEpoch;
