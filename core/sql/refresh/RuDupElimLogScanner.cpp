...
CRUDupElimLogScanner::CRUDupElimLogScanner(const CRUDupElimGlobals &globals,
                                           CRUSQLDynamicStatementContainer &ctrlStmtContainer)
    : inherited(globals, ctrlStmtContainer, CRUDupElimConst::NUM_QUERY_STMTS),
...
void CRUDupElimLogScanner::Fetch() {
  ...
  if (TRUE == pResultSet_->Next()) {
    ...
    if (NULL == pPrevRec_ || pCurrentRec_->GetCKTuple() != pPrevRec_->GetCKTuple()) {
