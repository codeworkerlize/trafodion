...
class REFRESH_LIB_CLASS CRUDupElimTaskExUnit {
 ...
 public:
  void SetStatementText(short index, const CDSString &sql) { stmtContainer_.SetStatementText(index, sql); }
  void PrepareStatement(short index) { stmtContainer_.PrepareStatement(index); }
 ...
 public:
  ...
  virtual void StoreRequest(CUOFsIpcMessageTranslator &translator) { stmtContainer_.StoreData(translator); }
  virtual void LoadReply(CUOFsIpcMessageTranslator &translator) = 0;

  // Used at the remote process side
  virtual void LoadRequest(CUOFsIpcMessageTranslator &translator) { stmtContainer_.LoadData(translator); }
  virtual void StoreReply(CUOFsIpcMessageTranslator &translator) = 0;

 protected:
  const CRUDupElimGlobals &GetDupElimGlobals() const { return dupElimGlobals_; }

  CRUSQLDynamicStatementContainer &GetControlStmtContainer() const { return ctrlStmtContainer_; }

  CDMPreparedStatement *GetPreparedStatement(short index) { return stmtContainer_.GetPreparedStatement(index); }
...
class REFRESH_LIB_CLASS CRUDupElimResolver : public CRUDupElimTaskExUnit {
 ...
 public:
  ...
  BOOL CanCompletePhase() const { return canCompletePhase_; }
  ...
  //------------------------------------//
 public:
  void SetCanCompletePhase(BOOL flag) { canCompletePhase_ = flag; }
