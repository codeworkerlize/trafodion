...
static const CRULogCtlColDesc iudLogCtlColDescVec[] = {
    {"EPOCH", "INT"},           {"OPERATION_TYPE", "INT UNSIGNED"}, {"RANGE_SIZE", "INT UNSIGNED"},
    {"IGNORE", "INT UNSIGNED"}, {"UPDATE_BITMAP", "CHAR"},
};
...
static const CRULogCtlColDesc rngLogCtlColDescVec[] = {
    {"EPOCH", "INT"}, {"RANGE_ID", "LARGEINT"}, {"RANGE_TYPE", "INT UNSIGNED"}};

//--------------------------------------------------------------------------//
//	Constructor
//--------------------------------------------------------------------------//

CRUDupElimSQLComposer::CRUDupElimSQLComposer(CRUDupElimTask *pTask, CRUDupElimGlobals &globals)
    : inherited(),
...
void CRUDupElimSQLComposer::ComposeQueryText(Int32 type) {
  ...
  CDSString epochCol = ComposeQuotedColName(iudLogCtlColDescVec[CRUDupElimConst::OFS_EPOCH].name_);
...
void CRUDupElimSQLComposer::ComposeControlText(Int32 type) {
  ...
  switch (type) {
    case CRUDupElimConst::IUD_LOG_FORCE_MDAM_CQS: {
      CDSString iudLogName(table_.GetIUDLogFullName(FALSE /*without namespace*/));

      ComposeForceMDAMQryShapeText(iudLogName);
      break;
    }
    case CRUDupElimConst::RNG_LOG_FORCE_MDAM_CQS: {
      CDSString rngLogName(table_.GetRangeLogFullName(FALSE /*without namespace*/));
...
void CRUDupElimSQLComposer::ComposeQClusteringKeyColNames(CDSString &to, BOOL order) {
  to = "";
  ...
  for (;;) {
    ...
    to += name;
    if (TRUE == order) {
      to += (CDDObject::eDescending == pKeyCol->GetSortOrder()) ? " DESC" : " ASC";
    }
...
void CRUDupElimSQLComposer::ComposeQBlockForEpoch(CDSString &qryBase, CDSString &epochCol, TInt32 ep,
                                                  BOOL isPhase0Query) {
  if (0 != sql_.GetLength()) {
...
void CRUDupElimSQLComposer::ComposeQBlockForMultipleEpochs(CDSString &qryBase, CDSString &epochCol,
                                                           BOOL isPhase0Query) {
  CDSString pred1, pred2, pred;

  ComposeQBlockForMultipleEpochsBasicPred(pred1, epochCol,
                                          beginEpoch_,  // from
                                          endEpoch_,    // to
                                          isPhase0Query);

  ComposeQBlockForMultipleEpochsBasicPred(pred2, epochCol,
                                          -endEpoch_,    // from
                                          -beginEpoch_,  // to
                                          isPhase0Query);
...
void CRUDupElimSQLComposer::ComposeQBlockForMultipleEpochsBasicPred(CDSString &to, CDSString &epochCol, TInt32 fromEp,
                                                                    TInt32 toEp, BOOL isPhase0Query) {
  to = "\n(\n" + epochCol + " BETWEEN " + TInt32ToStr(fromEp) + " AND " + TInt32ToStr(toEp);
...
void CRUDupElimSQLComposer::ComposeUpdateIgnorePrefix(CDSString &cmdPrefix) {
  ...
  // SET @IGNORE=?
  cmdPrefix += "\nSET ";
  const CRULogCtlColDesc &desc = iudLogCtlColDescVec[CRUDupElimConst::OFS_IGNORE];
...
void CRUDupElimSQLComposer::ComposeIUDUpdateBitmapText() {
  ...
  const CRULogCtlColDesc &desc = iudLogCtlColDescVec[CRUDupElimConst::OFS_UPD_BMP];
...
void CRUDupElimSQLComposer::ComposeIUDUpdateOptypeText() {
  ...
  const CRULogCtlColDesc &desc = iudLogCtlColDescVec[CRUDupElimConst::OFS_OPTYPE];
...
void CRUDupElimSQLComposer::ComposeSingleIUDRecSearchPredicate(CDSString &pred) {
  pred = "\nWHERE ";
  // @EPOCH=?
  const CRULogCtlColDesc &desc = iudLogCtlColDescVec[CRUDupElimConst::OFS_EPOCH];
...
void CRUDupElimSQLComposer::ComposeUpdateRecsSearchPredicate(CDSString &pred) {
  pred = "\nWHERE \n(";
  const CRULogCtlColDesc &desc1 = iudLogCtlColDescVec[CRUDupElimConst::OFS_EPOCH];

  pred += ComposeQuotedColName(desc1.name_);
  pred += " BETWEEN " + TInt32ToStr(beginEpoch_) + " AND " + TInt32ToStr(endEpoch_) + ")\nAND ";
  ...
  CDSString type2 = TInt32ToStr(CRUDupElimConst::IS_PART_OF_UPDATE + CRUDupElimConst::IS_DELETE);

  const CRULogCtlColDesc &desc2 = iudLogCtlColDescVec[CRUDupElimConst::OFS_OPTYPE];
...
void CRUDupElimSQLComposer::ComposeForceMDAMQryShapeText(const CDSString &logName) {
...
void CRUDupElimSQLComposer::ComposeResetQryShapeText() { sql_ += "CONTROL QUERY SHAPE OFF;"; }
...
void CRUDupElimSQLComposer::ComposeRngBoundaryColNames(const CDSString &prefix, CDSString &to) {
  to = "";
...
void CRUDupElimSQLComposer::ComposeSubsetIUDRecSearchPredicate(CDSString &pred, CDSString &epochOp) {
  pred = "WHERE ";
...
void CRUDupElimSQLComposer::ComposeTupleComparisonRightOperand(CDSString &to) {
...
CDSString CRUDupElimSQLComposer::ComposeQuotedColName(const CDSString &name) {
  return CRUSQLComposer::ComposeQuotedColName(CRUTbl::logCrtlColPrefix, name);
}
...
CDSString CRUDupElimSQLComposer::ComposeIUDLogKeyColName(const CRUKeyColumn *pKeyCol) {
...
CDSString CRUDupElimSQLComposer::ComposeRngLogKeyColName(const CDSString &prefix, const CRUKeyColumn *pKeyCol) {
  return CRUSQLComposer::ComposeQuotedColName(CRUTbl::logCrtlColPrefix + prefix, pKeyCol->GetName());
}
