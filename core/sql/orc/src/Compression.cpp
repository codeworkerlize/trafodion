...
void printBuffer(std::ostream &out, const char *buffer, uint64_t length) {
  ...
  for (uint64_t line = 0; line < (length + width - 1) / width; ++line) {
    ...
    for (uint64_t byte = 0; byte < width && line * width + byte < length; ++byte) {
      out << " " << std::setfill('0') << std::setw(2) << static_cast<uint64_t>(0xff & buffer[line * width + byte]);
    }
    out << "\n";
  }
  out << std::dec;
...
void SeekableArrayInputStream::seek(PositionProvider &seekPosition) { position = seekPosition.next(); }
...
ZlibDecompressionStream::ZlibDecompressionStream(std::unique_ptr<SeekableInputStream> inStream, size_t _blockSize,
                                                 MemoryPool &_pool)
    : pool(_pool), blockSize(_blockSize), buffer(pool, _blockSize) {
...
bool ZlibDecompressionStream::Next(const void **data, int *size) {
  ...
  size_t availSize = std::min(static_cast<size_t>(inputBufferEnd - inputBuffer), remainingLength);
  if (state == DECOMPRESS_ORIGINAL) {
    ...
    *size = static_cast<int>(availSize);
    ...
    int64_t result;
    do {
      result = inflate(&zstream, availSize == remainingLength ? Z_FINISH : Z_SYNC_FLUSH);
      switch (result) {
        case Z_OK:
          remainingLength -= availSize;
          inputBuffer += availSize;
          readBuffer(true);
          availSize = std::min(static_cast<size_t>(inputBufferEnd - inputBuffer), remainingLength);
          zstream.next_in = reinterpret_cast<Bytef *>(const_cast<char *>(inputBuffer));
          zstream.avail_in = static_cast<uInt>(availSize);
          break;
        case Z_STREAM_END:
...
class SnappyDecompressionStream : public SeekableInputStream {
 public:
  SnappyDecompressionStream(std::unique_ptr<SeekableInputStream> inStream, size_t blockSize, MemoryPool &pool);
 ...
 private:
  void readBuffer(bool failOnEof) {
    int length;
    if (!input->Next(reinterpret_cast<const void **>(&inputBufferPtr), &length)) {
      if (failOnEof) {
...
SnappyDecompressionStream::SnappyDecompressionStream(std::unique_ptr<SeekableInputStream> inStream, size_t bufferSize,
                                                     MemoryPool &_pool)
    : pool(_pool),
...
bool SnappyDecompressionStream::Next(const void **data, int *size) {
  ...
  size_t availSize = std::min(static_cast<size_t>(inputBufferPtrEnd - inputBufferPtr), remainingLength);
  if (state == DECOMPRESS_ORIGINAL) {
    *data = inputBufferPtr;
    *size = static_cast<int>(availSize);
    ...
    if (remainingLength == availSize) {
      ...
      for (size_t pos = availSize; pos < remainingLength;) {
        readBuffer(true);
        size_t avail = std::min(static_cast<size_t>(inputBufferPtrEnd - inputBufferPtr), remainingLength - pos);
    ...
    if (!snappy::GetUncompressedLength(compressed, remainingLength, &outputBufferLength)) {
    ...
    if (!snappy::RawUncompress(compressed, remainingLength, outputBuffer.data())) {
      throw ParseError("SnappyDecompressionStream choked on corrupt input");
    }
...
int64_t SnappyDecompressionStream::ByteCount() const { return bytesReturned; }
