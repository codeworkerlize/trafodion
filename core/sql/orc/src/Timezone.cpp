  ...
  void computeOffsets() {
    if (!hasDst) {
      ...
      for (int64_t year = 1970; year < 1970 + 400; ++year) {
        if (startInStd) {
          offsets[static_cast<uint64_t>(year - 1970) * 2 + 1] = base + start.getTime(year) - standard.gmtOffset;
          ...
          offsets[static_cast<uint64_t>(year - 1970) * 2 + 2] = base + start.getTime(year) - standard.gmtOffset;
        }
...
const Timezone &getTimezoneByFilename(const std::string &filename) {
  ...
  while (posn < size) {
    ...
    if (ret == -1) {
      throw TimezoneError(std::string("Failure to read timezone file ") + filename + " - " + strerror(errno));
    }
 ...
 */
const Timezone &getLocalTimezone() { return getTimezoneByFilename(LOCAL_TIMEZONE); }
...
void TimezoneImpl::parseZoneFile(const unsigned char *ptr, uint64_t sectionOffset, uint64_t fileLength,
  ...
  if (fileLength < headerOffset + 6 * 4 || strncmp(reinterpret_cast<const char *>(ptr) + magicOffset, "TZif", 4) != 0) {
    std::stringstream buffer;
