...
BooleanColumnReader::BooleanColumnReader(const Type &type, StripeStreams &stripe) : ColumnReader(type, stripe) {
  rle = createBooleanRleDecoder(stripe.getStream(columnId, proto::Stream_Kind_DATA, true));
}
...
void BooleanColumnReader::next(ColumnVectorBatch &rowBatch, uint64_t numValues, char *notNull) {
  ...
  rle->next(reinterpret_cast<char *>(ptr), numValues, rowBatch.hasNulls ? rowBatch.notNull.data() : 0);
...
ByteColumnReader::ByteColumnReader(const Type &type, StripeStreams &stripe) : ColumnReader(type, stripe) {
  rle = createByteRleDecoder(stripe.getStream(columnId, proto::Stream_Kind_DATA, true));
}
...
IntegerColumnReader::IntegerColumnReader(const Type &type, StripeStreams &stripe) : ColumnReader(type, stripe) {
  ...
  rle = createRleDecoder(stripe.getStream(columnId, proto::Stream_Kind_DATA, true), true, vers, memoryPool);
}
...
TimestampColumnReader::TimestampColumnReader(const Type &type, StripeStreams &stripe)
    : ColumnReader(type, stripe), writerTimezone(stripe.getWriterTimezone()), epochOffset(writerTimezone.getEpoch()) {
  RleVersion vers = convertRleVersion(stripe.getEncoding(columnId).kind());
  secondsRle = createRleDecoder(stripe.getStream(columnId, proto::Stream_Kind_DATA, true), true, vers, memoryPool);
  nanoRle = createRleDecoder(stripe.getStream(columnId, proto::Stream_Kind_SECONDARY, true), false, vers, memoryPool);
}
...
void TimestampColumnReader::next(ColumnVectorBatch &rowBatch, uint64_t numValues, char *notNull) {
  ...
  for (uint64_t i = 0; i < numValues; i++) {
    if (notNull == nullptr || notNull[i]) {
      ...
      secsBuffer[i] = writerTime + writerTimezone.getVariant(writerTime).gmtOffset;
...
uint64_t DoubleColumnReader::skip(uint64_t numValues) {
  ...
  if (static_cast<size_t>(bufferEnd - bufferPointer) >= bytesPerValue * numValues) {
    ...
    inputStream->Skip(static_cast<int>(bytesPerValue * numValues - static_cast<size_t>(bufferEnd - bufferPointer)));
    bufferEnd = NULL;
    bufferPointer = NULL;
  }
...
StringDictionaryColumnReader::StringDictionaryColumnReader(const Type &type, StripeStreams &stripe)
    : ColumnReader(type, stripe), dictionaryBlob(stripe.getMemoryPool()), dictionaryOffset(stripe.getMemoryPool()) {
  ...
  rle = createRleDecoder(stripe.getStream(columnId, proto::Stream_Kind_DATA, true), false, rleVersion, memoryPool);
  std::unique_ptr<RleDecoder> lengthDecoder =
      createRleDecoder(stripe.getStream(columnId, proto::Stream_Kind_LENGTH, false), false, rleVersion, memoryPool);
...
StringDirectColumnReader::StringDirectColumnReader(const Type &type, StripeStreams &stripe)
  ...
  RleVersion rleVersion = convertRleVersion(stripe.getEncoding(columnId).kind());
  lengthRle =
      createRleDecoder(stripe.getStream(columnId, proto::Stream_Kind_LENGTH, true), false, rleVersion, memoryPool);
...
void StringDirectColumnReader::next(ColumnVectorBatch &rowBatch, uint64_t numValues, char *notNull) {
  ...
  if (notNull) {
    while (filledSlots < numValues &&
           (!notNull[filledSlots] || usedBytes + static_cast<size_t>(lengthPtr[filledSlots]) <= bytesBuffered)) {
    ...
    while (filledSlots < numValues && (usedBytes + static_cast<size_t>(lengthPtr[filledSlots]) <= bytesBuffered)) {
      startPtr[filledSlots] = ptr + usedBytes;
      usedBytes += static_cast<size_t>(lengthPtr[filledSlots]);
      filledSlots += 1;
    }
  ...
  if (usedBytes < bytesBuffered) {
    size_t moreBytes = static_cast<size_t>(lengthPtr[filledSlots]) - (bytesBuffered - usedBytes);
...
void StructColumnReader::next(ColumnVectorBatch &rowBatch, uint64_t numValues, char *notNull) {
  ...
  for (std::vector<ColumnReader *>::iterator ptr = children.begin(); ptr != children.end(); ++ptr, ++i) {
    (*ptr)->next(*(dynamic_cast<StructVectorBatch &>(rowBatch).fields[i]), numValues, notNull);
  }
}
...
ListColumnReader::ListColumnReader(const Type &type, StripeStreams &stripe) : ColumnReader(type, stripe) {
  ...
  const std::vector<bool> selectedColumns = stripe.getSelectedColumns();
  RleVersion vers = convertRleVersion(stripe.getEncoding(columnId).kind());
  rle = createRleDecoder(stripe.getStream(columnId, proto::Stream_Kind_LENGTH, true), false, vers, memoryPool);
...
UnionColumnReader::~UnionColumnReader() {
  for (std::vector<ColumnReader *>::iterator itr = childrenReader.begin(); itr != childrenReader.end(); ++itr) {
    delete *itr;
  }
}
...
Decimal64ColumnReader::Decimal64ColumnReader(const Type &type, StripeStreams &stripe) : ColumnReader(type, stripe) {
  ...
  RleVersion vers = convertRleVersion(stripe.getEncoding(columnId).kind());
  scaleDecoder =
      createRleDecoder(stripe.getStream(columnId, proto::Stream_Kind_SECONDARY, true), true, vers, memoryPool);
}
