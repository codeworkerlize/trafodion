  ...
  while (numValues > 0) {
    ...
    if (!repeating) {
      ...
      while (consumedBytes > 0) {
        ...
        size_t skipSize = std::min(static_cast<size_t>(consumedBytes), static_cast<size_t>(bufferEnd - bufferStart));
...
void BooleanRleDecoderImpl::next(char *data, uint64_t numValues, char *notNull) {
  ...
  if (nonNulls == 0) {
    ...
    if (notNull) {
      ...
      for (int64_t i = static_cast<int64_t>(numValues) - 1; i >= static_cast<int64_t>(position); --i, --bitsLeft) {
        uint64_t shiftPosn = (-bitsLeft) % 8;
        data[i] = (data[position + (bitsLeft - 1) / 8] >> shiftPosn) & 0x1;
      }
    }
