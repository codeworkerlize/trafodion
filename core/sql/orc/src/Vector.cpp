...
ColumnVectorBatch::ColumnVectorBatch(uint64_t cap, MemoryPool &pool)
    : capacity(cap), numElements(0), notNull(pool, cap), hasNulls(false), memoryPool(pool) {
  // PASS
}
...
uint64_t ColumnVectorBatch::getMemoryUsage() { return static_cast<uint64_t>(notNull.capacity() * sizeof(char)); }
...
StringVectorBatch::StringVectorBatch(uint64_t capacity, MemoryPool &pool)
    : ColumnVectorBatch(capacity, pool), data(pool, capacity), length(pool, capacity) {
  // PASS
}
...
std::string MapVectorBatch::toString() const {
  std::ostringstream buffer;
  buffer << "Map vector <" << keys->toString() << ", " << elements->toString() << " with " << numElements << " of "
         << capacity << ">";
  return buffer.str();
}
...
uint64_t MapVectorBatch::getMemoryUsage() {
  return ColumnVectorBatch::getMemoryUsage() + static_cast<uint64_t>(offsets.capacity() * sizeof(int64_t)) +
         keys->getMemoryUsage() + elements->getMemoryUsage();
}
