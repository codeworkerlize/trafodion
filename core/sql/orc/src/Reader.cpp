...
bool ReaderOptions::getIndexesSet() const { return privateBits->setIndexes; }

const std::list<uint64_t> &ReaderOptions::getInclude() const { return privateBits->includedColumnIndexes; }

bool ReaderOptions::getNamesSet() const { return privateBits->setNames; }

const std::list<std::string> &ReaderOptions::getIncludeNames() const { return privateBits->includedColumnNames; }
...
bool ReaderOptions::getThrowOnHive11DecimalOverflow() const { return privateBits->throwOnHive11DecimalOverflow; }
...
int32_t ReaderOptions::getForcedScaleOnHive11Decimal() const { return privateBits->forcedScaleOnHive11Decimal; }
...
class ColumnStatisticsImpl : public ColumnStatistics {
 ...
 public:
  ...
  uint64_t getNumberOfValues() const override { return valueCount; }
...
class BinaryColumnStatisticsImpl : public BinaryColumnStatistics {
 ...
 public:
  ...
  bool hasTotalLength() const override { return _hasTotalLength; }
  uint64_t getNumberOfValues() const override { return valueCount; }
  ...
  std::string toString() const override {
    std::ostringstream buffer;
    buffer << "Data type: Binary" << std::endl << "Values: " << valueCount << std::endl;
    if (_hasTotalLength) {
...
class BooleanColumnStatisticsImpl : public BooleanColumnStatistics {
 ...
 public:
  ...
  std::string toString() const override {
    std::ostringstream buffer;
    buffer << "Data type: Boolean" << std::endl << "Values: " << valueCount << std::endl;
    if (_hasCount) {
...
class DateColumnStatisticsImpl : public DateColumnStatistics {
 ...
 public:
  ...
  bool hasMinimum() const override { return _hasMinimum; }

  bool hasMaximum() const override { return _hasMaximum; }
  ...
  std::string toString() const override {
    std::ostringstream buffer;
    buffer << "Data type: Date" << std::endl << "Values: " << valueCount << std::endl;
    if (_hasMinimum) {
...
class DecimalColumnStatisticsImpl : public DecimalColumnStatistics {
 ...
 public:
  ...
  bool hasSum() const override { return _hasSum; }
  ...
  std::string toString() const override {
    std::ostringstream buffer;
    buffer << "Data type: Decimal" << std::endl << "Values: " << valueCount << std::endl;
    if (_hasMinimum) {
      buffer << "Minimum: " << minimum << std::endl;
    } else {
      buffer << "Minimum: not defined" << std::endl;
...
class DoubleColumnStatisticsImpl : public DoubleColumnStatistics {
 ...
 public:
  ...
  std::string toString() const override {
    std::ostringstream buffer;
    buffer << "Data type: Double" << std::endl << "Values: " << valueCount << std::endl;
    if (_hasMinimum) {
      buffer << "Minimum: " << minimum << std::endl;
    } else {
      buffer << "Minimum: not defined" << std::endl;
...
class IntegerColumnStatisticsImpl : public IntegerColumnStatistics {
 ...
 public:
  ...
  std::string toString() const override {
    std::ostringstream buffer;
    buffer << "Data type: Integer" << std::endl << "Values: " << valueCount << std::endl;
    if (_hasMinimum) {
      buffer << "Minimum: " << minimum << std::endl;
    } else {
      buffer << "Minimum: not defined" << std::endl;
...
class StringColumnStatisticsImpl : public StringColumnStatistics {
 ...
 public:
  ...
  std::string toString() const override {
    std::ostringstream buffer;
    buffer << "Data type: String" << std::endl << "Values: " << valueCount << std::endl;
    if (_hasMinimum) {
      buffer << "Minimum: " << minimum << std::endl;
    } else {
...
class TimestampColumnStatisticsImpl : public TimestampColumnStatistics {
 ...
 public:
  ...
  std::string toString() const override {
    std::ostringstream buffer;
    buffer << "Data type: Timestamp" << std::endl << "Values: " << valueCount << std::endl;
    if (_hasMinimum) {
      buffer << "Minimum: " << minimum << std::endl;
    } else {
      buffer << "Minimum is not defined" << std::endl;
 ...
 public:
  StatisticsImpl(const proto::StripeStatistics &stripeStats, bool correctStats) {
    for (int i = 0; i < stripeStats.colstats_size(); i++) {
      colStats.push_back(convertColumnStatistics(stripeStats.colstats(i), correctStats));
    }
  }

  StatisticsImpl(const proto::Footer &footer, bool correctStats) {
    for (int i = 0; i < footer.statistics_size(); i++) {
      colStats.push_back(convertColumnStatistics(footer.statistics(i), correctStats));
    }
  }
...
StatisticsImpl::~StatisticsImpl() {
  for (std::list<ColumnStatistics *>::iterator ptr = colStats.begin(); ptr != colStats.end(); ++ptr) {
    delete *ptr;
  }
}
...
class ReaderImpl : public Reader {
 ...
 public:
  ...
  ReaderImpl(std::unique_ptr<InputStream> stream, const ReaderOptions &options,
             std::unique_ptr<proto::PostScript> postscript, std::unique_ptr<proto::Footer> footer, uint64_t fileLength,
  ...
  std::unique_ptr<Statistics> getStripeStatistics(uint64_t stripeIndex) const override;
...
ReaderImpl::ReaderImpl(std::unique_ptr<InputStream> input, const ReaderOptions &opts,
                       std::unique_ptr<proto::PostScript> _postscript, std::unique_ptr<proto::Footer> _footer,
...
std::unique_ptr<StripeInformation> ReaderImpl::getStripe(uint64_t stripeIndex) const {
  ...
  return std::unique_ptr<StripeInformation>(new StripeInformationImpl(
      ...
      stripeInfo.numberofrows(), stream.get(), memoryPool, compression, blockSize));
}
...
uint64_t ReaderImpl::getStripeStatisticsLength() const { return postscript->metadatalength(); }

uint64_t ReaderImpl::getFileFooterLength() const { return postscript->footerlength(); }
...
const std::string &ReaderImpl::getStreamName() const { return stream->getName(); }
...
const std::vector<bool> ReaderImpl::getSelectedColumns() const { return selectedColumns; }
...
std::unique_ptr<ColumnStatistics> ReaderImpl::getColumnStatistics(uint32_t index) const {
  ...
  return std::unique_ptr<ColumnStatistics>(convertColumnStatistics(col, hasCorrectStatistics()));
}

void ReaderImpl::readMetadata() const {
  uint64_t metadataSize = postscript->metadatalength();
  uint64_t metadataStart = fileLength - metadataSize - postscript->footerlength() - postscriptLength - 1;
  if (metadataSize != 0) {
    std::unique_ptr<SeekableInputStream> pbStream =
        createDecompressor(compression,
                           std::unique_ptr<SeekableInputStream>(
                               new SeekableFileInputStream(stream.get(), metadataStart, metadataSize, memoryPool)),
...
std::unique_ptr<Statistics> ReaderImpl::getStripeStatistics(uint64_t stripeIndex) const {
  if (!isMetadataLoaded) {
    readMetadata();
  }
  ...
  return std::unique_ptr<Statistics>(
      new StatisticsImpl(metadata->stripestats(static_cast<int>(stripeIndex)), hasCorrectStatistics()));
}
...
void ReaderImpl::seekToRow(uint64_t rowNumber) {
  ...
  if ((lastStripe == static_cast<uint64_t>(footer->stripes_size()) && rowNumber >= footer->numberofrows()) ||
      (lastStripe < static_cast<uint64_t>(footer->stripes_size()) && rowNumber >= firstRowOfStripe[lastStripe])) {
    currentStripe = static_cast<uint64_t>(footer->stripes_size());
    previousRow = footer->numberofrows();
    return;
  }
...
bool ReaderImpl::hasCorrectStatistics() const { return getWriterVersion() != WriterVersion_ORIGINAL; }

proto::StripeFooter ReaderImpl::getStripeFooter(const proto::StripeInformation &info) {
...
class StripeStreamsImpl : public StripeStreams {
 ...
 public:
  StripeStreamsImpl(const ReaderImpl &reader, const proto::StripeFooter &footer, uint64_t stripeStart,
                    InputStream &input, MemoryPool &memoryPool, const Timezone &writerTimezone);
...
uint64_t ReaderImpl::getMemoryUse(int stripeIx) {
  ...
  uint64_t memory = hasStringColumn
                        ? 2 * maxDataLength
                        : std::min(uint64_t(maxDataLength), nSelectedStreams * stream->getNaturalReadSize());
...
StripeStreamsImpl::StripeStreamsImpl(const ReaderImpl &_reader, const proto::StripeFooter &_footer,
                                     uint64_t _stripeStart, InputStream &_input, MemoryPool &_memoryPool,
...
const ReaderOptions &StripeStreamsImpl::getReaderOptions() const { return reader.getReaderOptions(); }

const std::vector<bool> StripeStreamsImpl::getSelectedColumns() const { return reader.getSelectedColumns(); }
...
std::unique_ptr<SeekableInputStream> StripeStreamsImpl::getStream(uint64_t columnId, proto::Stream_Kind kind,
  ...
  for (int i = 0; i < footer.streams_size(); ++i) {
    ...
    if (stream.has_kind() && stream.kind() == kind && stream.column() == static_cast<uint64_t>(columnId)) {
      uint64_t myBlock = shouldStream ? input.getNaturalReadSize() : stream.length();
      return createDecompressor(reader.getCompression(),
                                std::unique_ptr<SeekableInputStream>(
                                    new SeekableFileInputStream(&input, offset, stream.length(), memoryPool, myBlock)),
...
MemoryPool &StripeStreamsImpl::getMemoryPool() const { return memoryPool; }
...
void ReaderImpl::startNextStripe() {
  ...
  const Timezone &writerTimezone = currentStripeFooter.has_writertimezone()
                                       ...
                                       : localTimezone;
  StripeStreamsImpl stripeStreams(*this, currentStripeFooter, currentStripeInfo.offset(), *(stream.get()), memoryPool,
                                  writerTimezone);
...
void ReaderImpl::checkOrcVersion() {
  ...
  if (version != "0.11" && version != "0.12") {
    *(options.getErrorStream()) << "Warning: ORC file " << stream->getName()
...
bool ReaderImpl::next(ColumnVectorBatch &data) {
  if (currentStripe >= lastStripe) {
    data.numElements = 0;
    if (lastStripe > 0) {
      previousRow = firstRowOfStripe[lastStripe - 1] + footer->stripes(static_cast<int>(lastStripe - 1)).numberofrows();
    } else {
      previousRow = 0;
    }
    return false;
...
std::unique_ptr<ColumnVectorBatch> ReaderImpl::createRowBatch(uint64_t capacity) const {
 ...
 */
std::unique_ptr<proto::Footer> readFooter(InputStream *stream, DataBuffer<char> *&buffer, uint64_t footerOffset,
...
std::unique_ptr<Reader> createReader(std::unique_ptr<InputStream> stream, const ReaderOptions &options) {
  ...
  if (serializedFooter.length() != 0) {
    ...
    footer = readFooter(stream.get(), buffer, footerOffset, *ps, *memoryPool);
    delete buffer;
  }
  return std::unique_ptr<Reader>(
      new ReaderImpl(std::move(stream), options, std::move(ps), std::move(footer), fileLength, postscriptLength));
}
...
ColumnStatisticsImpl::ColumnStatisticsImpl(const proto::ColumnStatistics &pb) { valueCount = pb.numberofvalues(); }

BinaryColumnStatisticsImpl::BinaryColumnStatisticsImpl(const proto::ColumnStatistics &pb, bool correctStats) {
...
BooleanColumnStatisticsImpl::BooleanColumnStatisticsImpl(const proto::ColumnStatistics &pb, bool correctStats) {
  valueCount = pb.numberofvalues();
...
DateColumnStatisticsImpl::DateColumnStatisticsImpl(const proto::ColumnStatistics &pb, bool correctStats) {
  valueCount = pb.numberofvalues();
...
DecimalColumnStatisticsImpl::DecimalColumnStatisticsImpl(const proto::ColumnStatistics &pb, bool correctStats) {
  valueCount = pb.numberofvalues();
...
StringColumnStatisticsImpl::StringColumnStatisticsImpl(const proto::ColumnStatistics &pb, bool correctStats) {
  valueCount = pb.numberofvalues();
...
TimestampColumnStatisticsImpl::TimestampColumnStatisticsImpl(const proto::ColumnStatistics &pb, bool correctStats) {
  valueCount = pb.numberofvalues();
...
void ReaderImpl::updateSelected(const std::list<uint64_t> &fieldIds) {
  ...
  for (std::list<uint64_t>::const_iterator i = fieldIds.begin(); i != fieldIds.end(); ++i) {
    if (*i >= childCount) {
      std::stringstream buffer;
      buffer << "Invalid column selected " << *i << " out of " << childCount;
      throw ParseError(buffer.str());
    }
    const Type &child = *schema->getSubtype(*i);
    for (size_t c = child.getColumnId(); c <= child.getMaximumColumnId(); ++c) {
...
void ReaderImpl::updateSelected(const std::list<std::string> &fieldNames) {
  uint64_t childCount = schema->getSubtypeCount();
  for (std::list<std::string>::const_iterator i = fieldNames.begin(); i != fieldNames.end(); ++i) {
