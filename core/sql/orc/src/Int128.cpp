...
void buildFromArray(Int128 &value, uint32_t *array, int64_t length) {
  switch (length) {
    ...
    case 4:
      value =
          Int128((static_cast<int64_t>(array[0]) << 32) + array[1], (static_cast<uint64_t>(array[2]) << 32) + array[3]);
      break;
    case 5:
      if (array[0] != 0) {
      ...
      value =
          Int128((static_cast<int64_t>(array[1]) << 32) + array[2], (static_cast<uint64_t>(array[3]) << 32) + array[4]);
      break;
    default:
...
std::string Int128::toHexString() const {
  std::stringstream buf;
  buf << std::hex << "0x" << std::setw(16) << std::setfill('0') << highbits << std::setw(16) << std::setfill('0')
      << lowbits;
  return buf.str();
}
