...
short ListCount::process(SqlciEnv *sqlci_env) {
  if (!get_argument()) {
    ...
    if (tmp == get_argument()) { /* ##?emit an errmsg, illegal value? */
    } else
...
short FCRepeat::process(SqlciEnv *sqlci_env) {
  ...
  if (!sqlci_env->isInteractiveNow()) return retval;
  ...
  if (cmd != 0)  // Character string was provided...
  {
    ...
    if (!cmd_num)
      ...
      if (neg_num) cmd_num = (sqlci_env->getSqlciStmts()->last_stmt() - cmd_num) + 1;
...
short FixCommand::process(SqlciEnv *sqlci_env) {
  ...
  if (input_stmt) {
    ...
    *fc_input_stmt = input_stmt;
      ...
      if (!fc_input_stmt->isEmpty()) {
        ...
        do {
          ...
          if (!quotePos) {
            ...
            if (is_single_stmt == DUNNO) is_single_stmt = (!packedEnd || fc_input_stmt->isEmpty(packedEnd)) ? YES : NO;

            if (packedEnd)  // semicolon seen
            {
              packedEndC = *packedEnd;
              *packedEnd = '\0';
              if (is_single_stmt == YES && packedEndC) is_single_stmt = NO;
            }
    ...
    if (is_single_stmt != YES) delete fc_input_stmt;
  } else {
    SqlciError(SQLCI_NO_STMT_MATCH, (ErrorParam *)0);
  }
