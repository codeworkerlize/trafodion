...
class Formatter {
 public:
  ...
  static Int32 buffer_it(SqlciEnv *sqlci_env, char *data, Int32 datatype, Lng32 length, Lng32 precision, Lng32 scale,
                         char *ind_data,
                         // display length is printed len in single-wide chars
                         Int32 display_length,
                         // display buffer len may be longer for UTF-8
                         Int32 display_buf_length, Int32 null_flag, Int32 vcIndLen, char *buf, Lng32 *curpos,
                         NABoolean separatorNeeded = FALSE, NABoolean checkShowNonPrinting = FALSE);

  static Lng32 display_length(Lng32 datatype, Lng32 length, Lng32 precision, Lng32 scale, Lng32 charsetEnum,
