...
Int32 InputStmt::getLine(char *input_str, FILE *nonstdin, Int32 first_line) {
  ...
  enum InputState {
    ...
    NORMAL,
    PROBABLY_COMMENT,
    PROBABLY_INITIAL_COMMENT,
    CONSUME,
    NORMAL_OR_SHELL,
    EOL,
    EOSTMT,  // , ESCAPE_CHAR
    PROBABLY_QUOTED_BLOCK,
    QUOTED_BLOCK,
    PROBABLY_END_QUOTED_BLOCK
  };
  ...
  while (state != EOL) {
    ...
    switch (state) {
      ...
      case EOSTMT: {
        if (a != '\n') {
          ...
          if (!nonstdin) {
            ...
            if (ungetc(a, nonstdin) == EOF) return -3;
          }
        }
        state = EOL;
      } break;
      ...
      case QUOTED_BLOCK:
        if (a == '/') {
          state = PROBABLY_END_QUOTED_BLOCK;
        }
        break;

      case PROBABLY_END_QUOTED_BLOCK:
        if (a == '/') {
          state = NORMAL;
          normalstart = i;
        } else {
      ...
      case PROBABLY_QUOTED_BLOCK:
        if (a == '/') {
          if (i >= 6) {
            // do not take // as QUOTED_BLOCK if
            // input_str contains import, export or hdfs:
            std::string str;
            str.assign(input_str, i);
            transform(str.begin(), str.end(), str.begin(), ::tolower);
            if (str.find("import") != std::string::npos || str.find("export") != std::string::npos ||
                str.find("hdfs:") != std::string::npos) {
              state = NORMAL;
              normalstart = i;
              break;
            }
        ...
        // FIXME: can we avoid this goto?
        goto normal;

      case PROBABLY_COMMENT:
      case PROBABLY_INITIAL_COMMENT:
        if (a == '-') {
          if (state == PROBABLY_INITIAL_COMMENT && first_line) retcode = 0;
          state = CONSUME;
          break;
        } else {
          normalstart = i;
      ...
      case NORMAL_OR_SHELL: {
        ...
        if (shellCmd_) state = CONSUME;  // just consume to end of line for sh command
      } break;
    ...
    if (state == EOL) {
      ...
      if (state == INITIAL || state == INITIAL_WHITESPACE || state == NORMAL ||
  ...
  if (retcode >= 0 && white)
    if (retcode > 0 || first_line) input_str[0] = '\0';
...
Int32 InputStmt::readStmt(FILE *nonstdin, Int32 suppress_blank_line_output) {
    ...
    do {
      ...
      switch (error) {
        case 1:  // no ";" seen yet, keep looking
        {
          ...
          if ((!sqlci_env->isInteractiveNow()) || (input_str[0] == '\0')) break;
          ...
          if ((input_str[i] != 'f') && (input_str[i] != 'F') && (input_str[i] != 'e') && (input_str[i] != 'E')) break;

          // find the next blank or end of line, whichever comes first.
          UInt32 k = i + 1;
          while ((k < len) && (input_str[k] != ' ')) k++;
          ...
          UInt32 j = 0;
          for (j = 0; j < cmdLen; j++, i++) cmd[j] = (char)toupper(input_str[i]);

          cmd[j] = 0;

          if ((strcmp(cmd, "FC") == 0) || (strcmp(cmd, "E") == 0) || (strcmp(cmd, "EXIT") == 0)) {
            // found a non-semicolon terminating command.
            // If an 'E' is seen, change it to 'EXIT'. Sqlci
            // parser only recognizes an 'EXIT'.
            if (strcmp(cmd, "E") == 0) strcpy(input_str, "EXIT");
        ...
        case -3:  // error while reading input / obey file.
        {
          ...
          SqlciError(SQLCI_ERROR_READING_FILE, p1, (ErrorParam *)0);
          delete p1;
          // return -99;
          error = -99;
          goto return_error;  //	      return error;
        ...
        case SqlciEnv::MAX_FRAGMENT_LEN_OVERFLOW:
          SqlciError(SQLCI_FRAGMENT_LEN_REACHED, (ErrorParam *)0);
          //              return error;
          goto return_error;  //	      return error;
      ...
      if (input_str[0] == '\0' && !suppress_blank_line_output)
        if (eolSeenOrig || eolSeenOrig == sqlci_env->eolSeenOnInput()) {
          ...
          if (sqlci_env->get_logfile()->IsOpen()) sqlci_env->get_logfile()->Write(">>", 2);
        }
  ...
  do {
    if (skip_first_fragment) {
      ...
      while (prev_fragment->next) prev_fragment = prev_fragment->next;
  ...
  if (packed_string_len) {
    ...
    if (isIgnoreStmt_ || ignoreJustThis_) {
      ...
      while (curr_fragment) {
          ...
          for (; j < fraglen; j++) {
            switch (curr_fragment->fragment[j]) {
              ...
              case '-':
                if (!quote_seen) {
                  ...
                  if (curr_fragment->fragment[j + 1] == '-') skip_it = -1;
                }
            ...
            if (!skip_it) {
              ...
              *****/
              if (a == '\t' && quote_seen != '\'') {
                a = ' ';
                if (sqlci_env->isInteractiveNow()) curr_fragment->fragment[j] = ' ';
              }
              packed_string[i++] = a;
            }
          }
        ...
        curr_fragment = curr_fragment->next;
        if (curr_fragment && i) packed_string[i++] = ' ';  // insert a blank separator.
      }
    }
  }

  packed_string[i] = '\0';
  if (isEmpty()) packed_string[0] = '\0';
...
char *InputStmt::findEnd(char *s, size_t &quote_seen_pos) {
  char *orig = s;
  char quote_seen;
  for (quote_seen = quote_seen_pos = 0;; s++) switch (*s) {
      case '\'':
      case '"':
        if (!quote_seen) {
          quote_seen = *s;
...
#define obeybangeof " !EOF EOF!"
  ...
  SqlciError(SQLCI_INPUT_PREMATURE_EOF, p1, (ErrorParam *)0);
  delete p1;
  delete[] err;
}  // syntaxErrorOnEof
...
void InputStmt::logStmt(NABoolean noPrompt) const {
  ...
  while (fragment) {
    char *log_str = new char[2 /* to display sqlci prompt */ + strlen(fragment->fragment)];
...
size_t InputStmt::getCommandLen() const {
  ...
  while (!done) {
    ...
    else if ((command[command_pos + i] == '/') && (command[command_pos + i + 1] == '/'))
      done = -1;
    else
      i++;
  }
...
void InputStmt::processInsert() {
  ...
  if ((command_len > 0) && (text_maxlen > 0)) {
    ...
    if (text_pos > text_maxlen) text_maxlen = text_pos;
  }
}  // processInsert()
...
//////////////////////////////////////////////////////
InputStmt::Option InputStmt::fix_string(const char *in_data, char *fixed_data, size_t data_maxlen) {
...
Int32 InputStmt::fix(Int32 append_only) {
  ...
  if (!append_only) {
    ...
    while (fragment) {
      ...
      if (option == ABORT_O) return -20;  // caller is responsible for deleting this InputStmt
...
Int32 InputStmt::sectionMatches(const char *section_name) {
  ...
  char tmp[10];
  size_t i = 0;
  for (i = 0; i < 8; i++) tmp[i] = toupper(str[i]);
  tmp[i] = '\0';
  tmp[0] = '?';  // #SECTION -> ?SECTION
  if (strncmp(tmp, "?SECTION", 8) != 0) return 0;

  if (IS_SECTION_NAME_CHAR(str[8])) return 0;
  ...
  for (i = 8; str[i] == ' '; i++)
    ;  // now skip whitespace to section name

  size_t s = 0;
  for (s = 0; IS_SECTION_NAME_CHAR(str[i]) && section_name[s]; i++, s++)
    if (toupper(str[i]) != toupper(section_name[s])) return 0;

  if (IS_SECTION_NAME_CHAR(str[i]) || section_name[s]) return 0;
...
static NABoolean isIfdefStmtTransition(NAString &nsUpTrim,  // toUpper+trimmed
  ...
  for (s = str; *s && !isspace((unsigned char)*s); s++)
    ;  // find end of 1st tok   // For VS2003
  c = *s;
  *s = '\0';
  if (strcmp(str, "IFDEF") == 0)
    which = IFDEF_;
  ...
  if (ignoreJustThis) *ignoreJustThis = TRUE;
  ...
  if (which == IFDEF_ || which == IFNDEF_) {
    if (level >= MAX_IFDEF_NESTING_LEVEL) {
      ...
      *s = c;
      for (; isspace((unsigned char)*s) || *s == '$' || *s == '='; s++)
        ;  // beg of 2nd tok  //  For VS2003
  ...
  if (level <= MAX_IFDEF_NESTING_LEVEL && level > 0) return (execute[level] != execute[level - 1]);

  if (level == 0 && ignoreJustThis) *ignoreJustThis = FALSE;
  return FALSE;
...
Int32 InputStmt::isIgnoreStmt(const char *str, NABoolean *ignoreJustThis) {
  ...
  if (ignoreJustThis) *ignoreJustThis = FALSE;
  ...
  if (ignoreOnLINUX || ignoreOnMX) ignoreAll = TRUE;
  ...
  if (xbeg < xend) {
    ...
    if (tempStr) {
      ...
      while (p) {
        if (shellCmd_) {
        ...
        } else if (searchForShellCmd && (_stricmp(p, "SH") == 0 || _stricmp(p, "SHELL") == 0)) {
          shellCmd_ = 1;
        } else if (_stricmp(p, "BEGIN") == 0) {
          if ((p = strtok(NULL, tokenDelimiter)) != NULL && _stricmp(p, "WORK") == 0) {
          ...
          if ((p = strtok(NULL, tokenDelimiter)) != NULL && _stricmp(p, "BALANCE") == 0)
            p = strtok(NULL, tokenDelimiter);
          continue;
        } else if (_stricmp(p, "END") == 0) {
          blockStmt_--;
          if ((p = strtok(NULL, tokenDelimiter)) != NULL && _stricmp(p, "IF") == 0) {
          ...
          if ((p = strtok(NULL, tokenDelimiter)) != NULL && _stricmp(p, "WHEN") == 0) {
