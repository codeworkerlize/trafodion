...
{
  SqlCmd::executeQuery("CONTROL QUERY DEFAULTS TRAF_TINYINT_SUPPORT 'ON' , TRAF_TINYINT_RETURN_VALUES 'ON' , TRAF_TINYINT_INPUT_PARAMS 'ON' , TRAF_LARGEINT_UNSIGNED_IO 'ON' , TRAF_BOOLEAN_IO 'ON' ", this);
...
static void SqlciEnv_prologue_to_run(SqlciEnv *sqlciEnv) 
{
  ...
  if (lsh)
  {
    ...
    if (retcode == 0)
      cout << "Loaded Trafodion Metadata into Shared Cache." << endl;
        ...
        if (costTh >= 0)
          {
            ...
            if (time2 - time1 > costTh)
              QRWARN("SQLCI executeCommands PID %d txID %ld TTC %ld",
...
short Env::process(SqlciEnv *sqlci_env)
{
  ...
  Int32 rc = sqlci_env->getAuthState2((Int32&)authenticationType,
...
short ChangeUser::process (SqlciEnv * sqlci_env)
{
  ...
  if (tenantName)
    sqlci_env->setTenantNameFromCommandLine(tenantName);
...
// Retrieve the database user ID from CLI
Int32 SqlciEnv::getAuthState2(Int32 &authenticationType,
                             bool &authorizationEnabled,
                             bool &authorizationReady,
                             bool &auditingEnabled)
{
...
Int32 SqlciEnv::getTenantID(Int32 &uid)
{
  ...
  Int32 localUID = 0;
  Int32 rc = SQL_EXEC_GetSessionAttr(SESSION_TENANT_ID,
                                     &localUID,
                                     NULL, 0, NULL);
  HandleCLIError(rc, this);
...
Int32 SqlciEnv::getTenantName(NAString &tenantName)
{
  ...
  char buf[1024] = "";
  Int32 rc = SQL_EXEC_GetSessionAttr(SESSION_TENANT_NAME,
                                     NULL,
                                     buf, 1024, NULL);
  HandleCLIError(rc, this);
