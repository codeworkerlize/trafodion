...
CmpContext::CmpContext(UInt32 f, CollHeap *h)
    : heap_((NAHeap *)h),
      ...
      transMode_(TransMode::IL_NOT_SPECIFIED_,  // init'd below
                 TransMode::READ_WRITE_, TransMode::OFF_),
  ...
  //##	if (!isISP())
  {
    ...
    if (!schemaDB_->getDefaults().getSqlParser_NADefaults_Ptr()) return;
  ...
  tmfudf_dll_interface_host_data_ = new (CmpCommon::contextHeap()) char[SQLUDR_STATEAREA_BUFFER_SIZE + 128];

  CMPASSERT(tmfudf_dll_interface_host_data_);
  memset((char *)tmfudf_dll_interface_host_data_, 0, SQLUDR_STATEAREA_BUFFER_SIZE + 128);
...
CmpContext::~CmpContext() {
  ...
  if (diags_)
    if (diags_->decrRefCount() == 0) diags_ = NULL;
  ...
  if (optSimulator_) delete optSimulator_;
  optSimulator_ = NULL;
  ...
  delete envs_;
  envs_ = 0;
  if (outFstream_ != NULL) delete outFstream_;
...
NAHeap *CmpContext::statementHeap() { return statement() ? statement()->heap() : 0; }

void CmpContext::setSecondaryMxcmp() {
  if (mxcmpPrimarySecondaryStatusSet_) return;
...
void CmpContext::setCompilerClusterInfo(NAClusterInfo *nac) {
  if (iOwnClusterInfo_ && clusterInfo_ && clusterInfo_ != nac) delete clusterInfo_;
...
void CmpContext::initAvailableNodes() {
  ...
  Generator::computeAvailableNodes(availableNodes_, availableNodesFromCLI_, availableNodesFromOSIM_, previousAffinity_,
                                   heap_);

  const char *asErrMsg = NULL;

  if (availableNodes_) asErrMsg = availableNodes_->isInvalid(CURRCONTEXT_CLUSTERINFO);
  ...
  if (asErrMsg) {
    ...
    //        tenants.
    DefaultToken tolerate = CmpCommon::getDefault(AS_TOLERATE_ILL_DEFINED_TENANTS);

    if (tolerate != DF_ON) {
      if (tolerate != DF_OFF) {
        NAWNodeSet *fixedSet = availableNodes_->copy(heap_);
        bool isFixed = fixedSet->tryToMakeValid(CURRCONTEXT_CLUSTERINFO, availableNodesFromOSIM_ == NULL);

        // default CQD setting SYSTEM: Generate an event log
        char buf[200];

        snprintf(buf, sizeof(buf), "Tenant of size %d on a cluster of %d nodes is not valid. Reason: %s",
                 availableNodes_->getTenantSize(), availableNodes_->getNumNodes(), asErrMsg);
        SQLMXLoggingArea::logSQLMXPredefinedEvent(buf, LL_WARN);

        if (isFixed) {
          availableNodes_ = fixedSet;
          snprintf(buf, sizeof(buf), "Tenant altered to size %d, number of nodes %d for this session.",
                   availableNodes_->getTenantSize(), availableNodes_->getNumNodes());
          SQLMXLoggingArea::logSQLMXPredefinedEvent(buf, LL_WARN);
        }
      } else {
        ...
        *CmpCommon::diags() << DgSqlCode(-7030) << DgInt0(affinity) << DgInt1(availableNodes_->getTenantSize())
                            << DgInt2(availableNodes_->getNumNodes()) << DgString0(asErrMsg);

        CmpInternalException(exText, __FILE__, __LINE__).throwException();
      }
    }
  }  // handling an ill-defined tenant

  if (!availableNodes_)
    CmpInternalException("Unable to determine available nodes", __FILE__, __LINE__).throwException();
...
void CmpContext::setAvailableNodesForOSIM(NAWNodeSet *osimNodes) {
  ...
  if (availableNodesFromOSIM_) delete availableNodesFromOSIM_;
  if (osimNodes)
...
void CmpContext::setAuthorizationState(Int32 state) {
  switch (state) {
    ...
    default:
      ...
      *diags_ << DgSqlCode(-1001) << DgString0(__FILE__) << DgInt0(__LINE__)
...
void CmpContext::cleanup(NABoolean exception) {
  ...
  if (optSimulator_) optSimulator_->cleanupAfterStatement();
  if (cmpMemMonitor) cmpMemMonitor->cleanupPerStatement();
  if (invocationInfos_.entries() > 0) {
    ...
    for (CollIndex p = 0; p < planInfos_.entries(); p++) TMUDFInternalSetup::deleteUDRPlanInfo(planInfos_[p]);
...
void CmpContext::reserveMemory() {
  if (reservedMemory_ == NULL) reservedMemory_ = heap_->allocateMemory(2048);
}
...
void CmpContext::switchContext() {
  SqlParser_NADefaults_Glob = ActiveSchemaDB()->getDefaults().getSqlParser_NADefaults_Ptr();
...
CmpStatementISP *CmpContext::getISPStatement(Int64 id) {
  ...
  for (CollIndex i = 0; i < statements.entries(); i++)
    if (statements[i] && (ispStatement = (statements[i])->ISPStatement()) && ispStatement->ISPReqId() == id)
      return ispStatement;
  return 0;
}
...
Int32 CmpContext::compileDirect(char *data, UInt32 data_len, CollHeap *outHeap, Int32 charset,
  ...
  if (jRc) {  // longjmp here
    ...
    // populate error
    if (diagsArea == NULL) diagsArea = ComDiagsArea::allocate(outHeap);
    if (diagsArea)
    ...
    if (jRc == MEMALLOC_FAILURE) CmpErrLog("Memory allocation failure");
  ...
  if (jRc2) {  // longjmp here
    ...
    // Log this error to the file indicated by CMP_ERR_LOG_FILE CQD.
    if (jRc2 == MEMALLOC_FAILURE) CmpErrLog("Memory allocation failure");
  ...
  try {
    switch (op) {
      ...
      case (CmpMessageObj::DDL): {
        ...
        cmpStatement = new CTXTHEAP CmpStatement(this);
        CmpMessageDDL ddlStmt(data, data_len, CTXTHEAP, charset, parentQid, parentQidLen);
      ...
      case (CmpMessageObj::DDL_WITH_STATUS): {
        // request is from ExDDLTcb::work() to get statement explain
        cmpStatement = new CTXTHEAP CmpStatement(this);
        CmpMessageDDLwithStatus ddlStmt(data, data_len, CTXTHEAP, charset, parentQid, parentQidLen);
      ...
      {
        char emsText[120];
        str_sprintf(emsText, "Embedded arkcmp does not support this operation: %d.", op);
    ...
    if (cmpStatement) {
      ...
      *diags << DgSqlCode(arkcmpErrorAssert) << DgInt0(0) << DgString0("from CMPASSERT")
  ...
  // cleanup and return
  if (cmpStatement && cmpStatement->readyToDie()) delete cmpStatement;
  ...
  decrRecursionLevel();
  if (rs == CmpStatement::CmpStatement_SUCCESS) return 0;  // SUCCESS
  return 2;                                                // FAILURE
}

// set/reset an env in compiler envs
void CmpContext::setArkcmpEnvDirect(const char *name, const char *value, NABoolean unset) {
  if (unset) {
...
CompilerTrackingInfo *CmpContext::getCompilerTrackingInfo() {
  if (compilerTrackingInfo_ == NULL) compilerTrackingInfo_ = new (heap_) CompilerTrackingInfo(heap_);
...
ULng32 CmpContext::getTMFUDF_DLL_InterfaceHostDataBufferLen() { return SQLUDR_STATEAREA_BUFFER_SIZE; }

// Set the SQL text for later use. If the buffer is not empty, this
// call does nothing.
//
void CmpContext::setLogmxEventSqlText(const NAWString &x) {
  if (sqlTextBuf_ == NULL) sqlTextBuf_ = new (heap_) NAWString(x, heap_);
}
...
void CmpContext::resetAllCaches(NABoolean resetHistogramCache) {
  ...
  if (!isAuthorizationEnabled()) return;
  ...
  if (CmpCommon::getDefault(TRAF_CLEAR_METADATA_CACHE) == DF_ON)
    ...
    if (resetHistogramCache && histogramCache_) histogramCache_->invalidateCache();
  }
}

void CmpContext::initGlobalNADefaultsEntries() { NADefaults::initGlobalEntries(); }
...
NATable *CmpContext::getNATableFromFirstMetaCI(const ExtendedQualName &name) {
  ...
  if (allInfo.entries() <= 1) return NULL;
...
void DDLObjInfoList::clearList() {
  ...
  if ((scope_ == NOT_IN_DDL_OPERATION) || (scope_ == UNKNOWN)) clear();  // NAList<DDLObjInfo>::clear()
}
...
void CmpContext::releaseAllDDLObjectLocks() { SQL_EXEC_ReleaseAllDDLObjectLocks(); }

void CmpContext::releaseAllDMLObjectLocks() { SQL_EXEC_ReleaseAllDMLObjectLocks(); }
