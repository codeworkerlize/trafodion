...
Lng32 CmpSqlSession::getUserInfoFromCLI()
{
  ...
  if (doDebug)
    printf("[DBUSER:%d]   SQL_EXEC_GetSessionAttr for user ID returned %d\n",
  ...
  if (sqlcode >= 0)
  {
    sqlcode = SQL_EXEC_GetSessionAttr(SESSION_TENANT_ID,
                                      &localTenantID, NULL, 0, NULL);
    if (sqlcode != 0)
    {
      SQL_EXEC_MergeDiagnostics_Internal(*CmpCommon::diags());
      SQL_EXEC_ClearDiagnostics(NULL);
  ...
  if (sqlcode >= 0)
  {
    sqlcode = SQL_EXEC_GetSessionAttr(SESSION_TENANT_NAME,
                                      NULL,
                                      localTenantName,
                                      sizeof(localTenantName),
                                      NULL);
    if (sqlcode != 0)
    {
      SQL_EXEC_MergeDiagnostics_Internal(*CmpCommon::diags());
  ...
  if (sqlcode >= 0)
  {
    ...
    if (doDebug)
    {
      ...
      printf("[DBUSER:%d]   Retrieved tenant ID %d, tenant name [%s]\n",
             (int) getpid(), (int) localTenantID, localTenantName);
    }
  }
...
//    members.
Lng32 CmpSqlSession::setDatabaseUserAndTenant(Int32 userID,
                                              const char *userName, 
					      Int32 tenantID,
                                              ...
                                              const char *tenantDefaultSchema)
{
  ...
  {
    sqlcode = SQL_EXEC_SetSessionAttr_Internal(SESSION_TENANT_ID,
                                               tenantID, (char *)tenantName);
    if (sqlcode != 0)
    {
      SQL_EXEC_MergeDiagnostics_Internal(*CmpCommon::diags());
      SQL_EXEC_ClearDiagnostics(NULL);
    }
    sqlcode = SQL_EXEC_SetSessionAttr_Internal(SESSION_TENANT_NAME,
                                               tenantID, (char *)tenantName);
    if (sqlcode != 0)
    {
      SQL_EXEC_MergeDiagnostics_Internal(*CmpCommon::diags());
    ...
    if (doDebug)
      printf("[DBUSER:%d]   SQL_EXEC_SetSessionAttr on tenant returned %d\n",
             (int) getpid(), (int) sqlcode);
    
    sqlcode = SQL_EXEC_SetSessionAttr_Internal(SESSION_TENANT_NODES,
                                               0, tenantNodes);
    if (sqlcode != 0)
    {
      SQL_EXEC_MergeDiagnostics_Internal(*CmpCommon::diags());
    ...
    if (tenantDefaultSchema !="")
      {
	sqlcode = SQL_EXEC_SetSessionAttr_Internal(SESSION_TENANT_DEFAULT_SCHEMA,
						   0,(char *)tenantDefaultSchema);
	if (sqlcode != 0)
	  {
	    SQL_EXEC_MergeDiagnostics_Internal(*CmpCommon::diags());
	    SQL_EXEC_ClearDiagnostics(NULL);
  ...
  if ((sqlcode >= 0) && (databaseUserID_ != userID))
  {
    ...
    if (doDebug)
      printf("[DBUSER:%d]   SQL_EXEC_SetSessionAttr on user ID returned %d\n",
             (int) getpid(), (int) sqlcode);
  }
