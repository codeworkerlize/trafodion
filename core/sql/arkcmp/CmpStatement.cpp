...
NABoolean CmpStatement::isUserDMLQuery(char* query, Lng32 len)
{
   ...
   for (int i=0; i<sizeof(prefixLen)/sizeof(int); i++) { 
     if ( len > prefixLen[i] && 
          !strncmp(query, prefix[i], prefixLen[i]) ) {
        return FALSE;
     }
   }
...
CmpStatement::process (const CmpMessageSQLText& sqltext)
{
  ...
  if (CmpCommon::getDefaultLong(QUERY_LOGGING) == 1 &&
      ...
      isUserDMLQuery(sqlStr, sqlStrLen))
  {
    fstream& out = getPrintHandle();

    for (Int32 i=0; i<sqlStrLen; i++ )
      out << sqlStr[i];
 ...
 // so that it can be used during binding.
  ...
  if (typ == CmpMessageObj::SQLTEXT_RECOMPILE) {
    recompiling_ = TRUE;
    if (recompUseNATableCache)
      recompUseNATableCache_ = TRUE;
  }
...
CmpStatement::process (const CmpMessageDDL& statement)
{
  ...
  if (statement.getCmpCompileInfo()->isHbaseDDL() ||
    statement.getCmpCompileInfo()->isGenLoadQueryCache())
    {
      CmpMain::ReturnStatus rs = CmpMain::SUCCESS;
...
CmpStatement::ReturnStatus
CmpStatement::process(const CmpMessageDDLwithStatus &statement, NABoolean needToDoWork)
{
  CmpMain cmpmain;
...
CmpStatement::ReturnStatus
CmpStatement::process (const CmpMessageXnOper& statement)
{
...
// ----------------------------------------------------------------------------
CmpStatement::ReturnStatus
CmpStatement::process (const CmpMessageDDLCommitSharedCache& statement)
{
  CmpSeabaseDDL cmpSBD(heap_);
...
// ----------------------------------------------------------------------------
CmpStatement::ReturnStatus
CmpStatement::process (const CmpMessageDDLRollbackSharedCache& statement)
{
  CmpContext* cmpContext = CmpCommon::context();
...
CmpStatement::process(const CmpMessageDatabaseUser &statement)
{
  ...
  Lng32 sqlcode = session->setDatabaseUserAndTenant(userID,
                                                    userName,
                                                    tenantID,
                                                    tenantName,
                                                    tenantNodes,
