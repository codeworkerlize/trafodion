...
SP_STATUS SP_FILEINFO_Process(SP_PROCESS_ACTION action, SP_ROW_DATA inputData, SP_EXTRACT_FUNCPTR eFunc,
  ...
  if (action == SP_PROC_OPEN) {
    ...
    *spProcHandle = 0;
    ...
    if (eFunc(0, inputData, 10, tempStr, 0) != SP_NO_ERROR)
      ...
      return SP_FAIL;
    else if (eFunc(2, inputData, 66, desc, 0) != SP_NO_ERROR)
      return SP_FAIL;
    else if (eFunc(3, inputData, 64, dt1, 0) != SP_NO_ERROR)
      return SP_FAIL;
    else {
...
SP_STATUS SP_CASTING_Compile(SP_COMPILE_ACTION /* action */, SP_COMPILE_HANDLE * /* compHandle */,
                             SP_HANDLE /* spHandle */, SP_ERROR_STRUCT * /* error */) {
  return SP_SUCCESS;
}
...
inline void varchar2String(char *varchar, short size, char *&string, short &len) {
  ...
  if ((UInt32)len + sizeof(short) < (UInt32)size) string[len] = 0;
}

inline short string2Varchar(char *string, char *varchar, short size) {
  if ((UInt32)strlen(string) + sizeof(short) > (UInt32)size) return 0;
...
SP_STATUS SP_CASTING_Process(SP_PROCESS_ACTION action, SP_ROW_DATA inputData, SP_EXTRACT_FUNCPTR eFunc,
                             SP_ROW_DATA outputData, SP_FORMAT_FUNCPTR fFunc, SP_KEY_VALUE, SP_KEYVALUE_FUNCPTR,
                             SP_PROCESS_HANDLE *spProcHandle, SP_HANDLE /* spHandle */, SP_ERROR_STRUCT *error) {
  ...
  if (action == SP_PROC_FETCH) {
    ...
    if (counter) {
      if (counter->current >= counter->max || (counter->fd && !fgets(string, 256, counter->fd)))
        return SP_SUCCESS;
      else {
  ...
  if (action == SP_PROC_CLOSE) {
    CounterStruct *counter = (CounterStruct *)(*spProcHandle);
    if (counter->fd) fclose(counter->fd);
    delete counter;
    return SP_SUCCESS;
  }
...
SP_STATUS SP_NOINPUT_Compile(SP_COMPILE_ACTION /* action */, SP_COMPILE_HANDLE * /* compHandle */,
                             SP_HANDLE /* spHandle */, SP_ERROR_STRUCT * /* error */) {
  return SP_SUCCESS;
}
...
SP_STATUS SP_NOINPUT_Process(SP_PROCESS_ACTION action, SP_ROW_DATA inputData, SP_EXTRACT_FUNCPTR eFunc,
                             SP_ROW_DATA outputData, SP_FORMAT_FUNCPTR fFunc, SP_KEY_VALUE, SP_KEYVALUE_FUNCPTR,
                             SP_PROCESS_HANDLE *spProcHandle, SP_HANDLE /* spHandle */, SP_ERROR_STRUCT *error) {
  struct InfoStruct {
    char varchar[256];
...
SP_STATUS SP_PARSING_PARSER(char *param,                 /* input, null terminated */
...
SP_STATUS SP_PARSING_Process(SP_PROCESS_ACTION action, SP_ROW_DATA inputData, SP_EXTRACT_FUNCPTR eFunc,
                             SP_ROW_DATA outputData, SP_FORMAT_FUNCPTR fFunc, SP_KEY_VALUE, SP_KEYVALUE_FUNCPTR,
                             SP_PROCESS_HANDLE *spProcHandle, SP_HANDLE /* spHandle */, SP_ERROR_STRUCT *error) {
  struct InfoStruct {
    int param;  // 1 or 2
...
SP_STATUS SP_DELAY_Compile(SP_COMPILE_ACTION action, SP_COMPILE_HANDLE *cmpHandle, SP_HANDLE /* spHandle */,
                           SP_ERROR_STRUCT * /* error */) {
  return SP_SUCCESS;
}
...
SP_STATUS SP_DELAY_Process(SP_PROCESS_ACTION action, SP_ROW_DATA inputData, SP_EXTRACT_FUNCPTR eFunc,
                           SP_ROW_DATA outputData, SP_FORMAT_FUNCPTR fFunc, SP_KEY_VALUE, SP_KEYVALUE_FUNCPTR,
                           SP_PROCESS_HANDLE *spProcHandle, SP_HANDLE /* spHandle */, SP_ERROR_STRUCT *error) {
  struct InfoStruct {
    int delay;
...
SP_STATUS SP_ERROR_PARSER(char *param,                 /* input, null terminated */
                          SP_COMPILE_HANDLE cmpHandle, /* input */
                          SP_HANDLE spHandle,          /* input */
                          SP_ERROR_STRUCT *error       /* output */
) {
...
SP_STATUS SP_ERROR_Process(SP_PROCESS_ACTION action, SP_ROW_DATA inputData, SP_EXTRACT_FUNCPTR eFunc,
                           SP_ROW_DATA outputData, SP_FORMAT_FUNCPTR fFunc, SP_KEY_VALUE, SP_KEYVALUE_FUNCPTR,
                           SP_PROCESS_HANDLE *spProcHandle, SP_HANDLE /* spHandle */, SP_ERROR_STRUCT *error) {
  if (action == SP_PROC_OPEN) {
...
SP_STATUS SP_COL16_Compile(SP_COMPILE_ACTION action, SP_COMPILE_HANDLE *cmpHandle, SP_HANDLE /* spHandle */,
                           SP_ERROR_STRUCT * /* error */) {
  return SP_SUCCESS;
}
...
SP_STATUS SP_COL16_Process(SP_PROCESS_ACTION action, SP_ROW_DATA inputData, SP_EXTRACT_FUNCPTR eFunc,
                           SP_ROW_DATA outputData, SP_FORMAT_FUNCPTR fFunc, SP_KEY_VALUE, SP_KEYVALUE_FUNCPTR,
                           SP_PROCESS_HANDLE *spProcHandle, SP_HANDLE /* spHandle */, SP_ERROR_STRUCT *error) {
  struct InfoStruct {
    int max;
...
SP_STATUS SP_ROW998_Compile(SP_COMPILE_ACTION action, SP_COMPILE_HANDLE *cmpHandle, SP_HANDLE /* spHandle */,
                            SP_ERROR_STRUCT * /* error */) {
  return SP_SUCCESS;
}
...
SP_STATUS SP_ROW998_Process(SP_PROCESS_ACTION action, SP_ROW_DATA inputData, SP_EXTRACT_FUNCPTR eFunc,
                            SP_ROW_DATA outputData, SP_FORMAT_FUNCPTR fFunc, SP_KEY_VALUE, SP_KEYVALUE_FUNCPTR,
                            SP_PROCESS_HANDLE *spProcHandle, SP_HANDLE /* spHandle */, SP_ERROR_STRUCT *error) {
  struct InfoStruct {
    int max;
...
int SQLISP_INIT(SP_REGISTER_FUNCPTR rFunc, SP_DLL_HANDLE *h) {
  ...
  rFunc(funcName, SP_FILEINFO_Compile, SP_FILEINFO_InputFormat, 0, SP_FILEINFO_NumOutputs, SP_FILEINFO_OutputFormat,
        SP_FILEINFO_Process, 0, CMPISPVERSION);
  strcpy(funcName, "CASTING");
  rFunc(funcName, SP_CASTING_Compile, SP_CASTING_InputFormat, 0, SP_CASTING_NumOutputs, SP_CASTING_OutputFormat,
        SP_CASTING_Process, 0, CMPISPVERSION);
  strcpy(funcName, "NOINPUT");
  rFunc(funcName, SP_NOINPUT_Compile, SP_NOINPUT_InputFormat, 0, SP_NOINPUT_NumOutputs, SP_NOINPUT_OutputFormat,
        SP_NOINPUT_Process, 0, CMPISPVERSION);
  strcpy(funcName, "PARSING");
  rFunc(funcName, SP_PARSING_Compile, SP_PARSING_InputFormat, SP_PARSING_PARSER, SP_PARSING_NumOutputs,
        SP_PARSING_OutputFormat, SP_PARSING_Process, 0, CMPISPVERSION);
  strcpy(funcName, "DELAY");
  rFunc(funcName, SP_DELAY_Compile, SP_DELAY_InputFormat, 0, SP_DELAY_NumOutputs, SP_DELAY_OutputFormat,
        SP_DELAY_Process, 0, CMPISPVERSION);
  strcpy(funcName, "ERROR");
  rFunc(funcName, SP_ERROR_Compile, SP_ERROR_InputFormat, SP_ERROR_PARSER, SP_ERROR_NumOutputs, SP_ERROR_OutputFormat,
        SP_ERROR_Process, 0, CMPISPVERSION);
  strcpy(funcName, "COL16");
  rFunc(funcName, SP_COL16_Compile, SP_COL16_InputFormat, 0, SP_COL16_NumOutputs, SP_COL16_OutputFormat,
        SP_COL16_Process, 0, CMPISPVERSION);
  strcpy(funcName, "ROW998");
  rFunc(funcName, SP_ROW998_Compile, SP_ROW998_InputFormat, 0, SP_ROW998_NumOutputs, SP_ROW998_OutputFormat,
        SP_ROW998_Process, 0, CMPISPVERSION);
