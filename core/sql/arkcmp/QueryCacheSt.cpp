...
SP_STATUS
QueryCacheStatStoredProcedure::sp_Process(SP_PROCESS_ACTION action, SP_ROW_DATA inputData, SP_EXTRACT_FUNCPTR eFunc,
  ...
  if (action == SP_PROC_OPEN) {
    QueryCacheStatsISPIterator *it = new (GetCliGlobals()->exCollHeap()) QueryCacheStatsISPIterator(
  ...
  if (action == SP_PROC_FETCH) {
    ...
    switch (stats.optimLvl) {
      case CompilerEnv::OPT_MINIMUM:
        strcpy(optimizationLevel, "0      ");
        break;
      case CompilerEnv::OPT_MEDIUM_LOW:
        strcpy(optimizationLevel, "2      ");
        break;
      case CompilerEnv::OPT_MEDIUM:
        strcpy(optimizationLevel, "3      ");
        break;
      case CompilerEnv::OPT_MAXIMUM:
        strcpy(optimizationLevel, "5      ");
        break;
      default:
...
void QueryCacheStatStoredProcedure::Initialize(SP_REGISTER_FUNCPTR regFunc) {
  regFunc("QUERYCACHE", sp_Compile, sp_InputFormat, 0, sp_NumOutputFields, sp_OutputFormat, sp_Process, 0,
          CMPISPVERSION);
}
...
SP_STATUS QueryCacheEntriesStoredProcedure::sp_OutputFormat(SP_FIELDDESC_STRUCT *format,
  ...
  strcpy(&((format++)->COLUMN_DEF[0]), "Hash     		LARGEINT");
  strcpy(&((format++)->COLUMN_DEF[0]), "hdfs_offset     		LARGEINT");
  strcpy(&((format++)->COLUMN_DEF[0]), "flags     		SMALLINT");
  return SP_SUCCESS;
}
...
// Copies information on the query cache by the interface provided in
// QueryCache.h and processes it
SP_STATUS QueryCacheEntriesStoredProcedure::sp_Process(SP_PROCESS_ACTION action, SP_ROW_DATA inputData,
  ...
  switch (action) {
    case SP_PROC_OPEN: {
      QueryCacheEntriesISPIterator *it = new (GetCliGlobals()->exCollHeap()) QueryCacheEntriesISPIterator(
          inputData, eFunc, error, GetCliGlobals()->currContext()->getCmpContextInfo(), GetCliGlobals()->exCollHeap());
    ...
    case SP_PROC_FETCH: {
      ...
      if (details.ignoreCqdOrCqs) {
        char temp[4096];
        strncpy(temp, "[!] ", 4);
        int len = strlen(details.qryTxt) > 4000 ? 4000 : strlen(details.qryTxt);
      ...
      switch (details.phase) {
        case CmpMain::PREPARSE:
          strcpy(phase, "PREPARSING");
          break;
        case CmpMain::PARSE:
          strcpy(phase, "PARSING   ");
          break;
        case CmpMain::BIND:
          strcpy(phase, "BINDING   ");
          break;
        default:
      ...
      switch (details.accMode) {
        ...
        case TransMode::READ_ONLY_SPECIFIED_BY_USER_:
          strcpy(accessMode, "READ ONLY    ");
          break;
        case TransMode::READ_WRITE_:
          strcpy(accessMode, "READ/WRITE   ");
          break;
        default:
      ...
      switch (details.autoCmt) {
        case TransMode::ON_:
          strcpy(autoCommit, "ON           ");
          break;
        case TransMode::OFF_:
          strcpy(autoCommit, "OFF          ");
          break;
        default:
      ...
      switch (details.rbackMode) {
        case TransMode::ROLLBACK_MODE_WAITED_:
          strcpy(rollBackMode, "WAITED       ");
          break;
        case TransMode::ROLLBACK_MODE_NOWAITED_:
          strcpy(rollBackMode, "NO WAITED    ");
          break;
        default:
      ...
      fFunc(21, outputData, sizeof(details.hdfs_offset), &(details.hdfs_offset), 0);

      fFunc(22, outputData, sizeof(Int16), &(details.flags), 0);
...
void QueryCacheEntriesStoredProcedure::Initialize(SP_REGISTER_FUNCPTR regFunc) {
  regFunc("QUERYCACHEENTRIES", sp_Compile, sp_InputFormat, 0, sp_NumOutputFields, sp_OutputFormat, sp_Process, 0,
          CMPISPVERSION);
}
...
SP_STATUS QueryCacheDeleteStoredProcedure::sp_Process(SP_PROCESS_ACTION action, SP_ROW_DATA inputData,
                                                      SP_EXTRACT_FUNCPTR eFunc, SP_ROW_DATA outputData,
  ...
  switch (action) {
    case SP_PROC_OPEN: {
      QueryCacheDeleter *deleter = new (GetCliGlobals()->exCollHeap()) QueryCacheDeleter(
          inputData, eFunc, error, GetCliGlobals()->currContext()->getCmpContextInfo(), GetCliGlobals()->exCollHeap());
...
void QueryCacheDeleteStoredProcedure::Initialize(SP_REGISTER_FUNCPTR regFunc) {
  regFunc("QUERYCACHEDELETE", sp_Compile, sp_InputFormat, 0, sp_NumOutputFields, sp_OutputFormat, sp_Process, 0,
          CMPISPVERSION);
}
...
void HybridQueryCacheStatStoredProcedure::Initialize(SP_REGISTER_FUNCPTR regFunc) {
  regFunc("HYBRIDQUERYCACHE", sp_Compile, sp_InputFormat, 0, sp_NumOutputFields, sp_OutputFormat, sp_Process, 0,
          CMPISPVERSION);
}
...
SP_STATUS HybridQueryCacheStatStoredProcedure::sp_OutputFormat(SP_FIELDDESC_STRUCT *format,
  ...
  strcpy(&((format++)->COLUMN_DEF[0]), "hqc_heap_size                      INT UNSIGNED");
  return SP_SUCCESS;
}

SP_STATUS HybridQueryCacheStatStoredProcedure::sp_Process(SP_PROCESS_ACTION action, SP_ROW_DATA inputData,
                                                          SP_EXTRACT_FUNCPTR eFunc, SP_ROW_DATA outputData,
                                                          SP_FORMAT_FUNCPTR fFunc, SP_KEY_VALUE keys,
                                                          SP_KEYVALUE_FUNCPTR kFunc, SP_PROCESS_HANDLE *spProcHandle,
                                                          SP_HANDLE spObj /* spHandle */, SP_ERROR_STRUCT *error) {
  switch (action) {
    case SP_PROC_OPEN: {
      HybridQueryCacheStatsISPIterator *it = new (GetCliGlobals()->exCollHeap()) HybridQueryCacheStatsISPIterator(
          inputData, eFunc, error, GetCliGlobals()->currContext()->getCmpContextInfo(), GetCliGlobals()->exCollHeap());
      *spProcHandle = it;
    ...
    case SP_PROC_FETCH: {
      ...
      fFunc(4, outputData, sizeof(stats.nHQCHeapSize), &(stats.nHQCHeapSize), 0);
...
void HybridQueryCacheEntriesStoredProcedure::Initialize(SP_REGISTER_FUNCPTR regFunc) {
  regFunc("HYBRIDQUERYCACHEENTRIES", sp_Compile, sp_InputFormat, 0, sp_NumOutputFields, sp_OutputFormat, sp_Process, 0,
          CMPISPVERSION);
}
...
SP_STATUS HybridQueryCacheEntriesStoredProcedure::sp_Process(SP_PROCESS_ACTION action, SP_ROW_DATA inputData,
                                                             SP_EXTRACT_FUNCPTR eFunc, SP_ROW_DATA outputData,
                                                             SP_FORMAT_FUNCPTR fFunc, SP_KEY_VALUE keys,
                                                             SP_KEYVALUE_FUNCPTR kFunc, SP_PROCESS_HANDLE *spProcHandle,
                                                             SP_HANDLE spObj /* spHandle */, SP_ERROR_STRUCT *error) {
  switch (action) {
    case SP_PROC_OPEN: {
      HybridQueryCacheEntriesISPIterator *it = new (GetCliGlobals()->exCollHeap()) HybridQueryCacheEntriesISPIterator(
          inputData, eFunc, error, GetCliGlobals()->currContext()->getCmpContextInfo(), GetCliGlobals()->exCollHeap());
...
NABoolean ISPIterator::initializeISPCaches(SP_ROW_DATA inputData, SP_EXTRACT_FUNCPTR eFunc, SP_ERROR_STRUCT *error,
  ...
  if (heap_)
    receivingField = new (heap_) char[maxSize + 1];
  else
