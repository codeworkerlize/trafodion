...
void ArkcmpErrorMessageBox(const char *msg, ArkcmpErrorSeverity sev, NABoolean doExit, NABoolean doDelay,
                           NABoolean doCerr) {
  ...
  if (doMsgbox) {
    ...
    if (!ev || ev[0] == '0') doMsgbox = FALSE;
  }

  if (doMsgbox) {
    UINT mbflags = MB_ICONERROR;
    if (sev == WARNING_SEV)
      mbflags = MB_ICONWARNING;
    else if (sev == INFO_SEV)
...
ExCmpMessage::ExCmpMessage(IpcEnvironment *ipcEnv)
    : IpcMessageStream(ipcEnv, CmpMessageObj::EXE_CMP_MESSAGE, EXECMPIPCVERSION, 0, TRUE) {
...
void ExCmpMessage::setCmpContext(CmpContext *cmpContext) { cmpContext_ = cmpContext; }
...
static void sendErrorOnLongJump(CmpStatement *cmpStatement, ExCmpMessage *msg, Int32 errcode, CollHeap *heap) {
  if (cmpStatement) {
    ...
    if (errcode == MEMALLOC_FAILURE) {
      ...
      *cmpStatement->diags() << DgString0("from longjmp(NAAssert)") << DgString1(__FILE__) << DgInt0(__LINE__);
    }
    *msg << *cmpStatement->diags();
    if (cmpStatement->reply()) *msg << *cmpStatement->reply();
  } else {
    ComDiagsArea *diags = ComDiagsArea::allocate(heap);
    *diags << DgSqlCode(arkcmpErrorAssert) << DgString0("from longjmp(NAAssert)") << DgString1(__FILE__)
           << DgInt0(__LINE__);
    *msg << *diags;
    diags->decrRefCount();
  }
...
void ExCmpMessage::actOnReceive(IpcConnection *) {
  if (getState() == ERROR_STATE) {
    ArkcmpFatalError(ARKCMP_ERROR_PREFIX "ExCmpMessage::actOnReceive, error from receiving message.");
  }
  ...
  try {
    ...
    switch (typ = getNextObjType()) {
      ...
      case CmpMessageObj::SQLTEXT_COMPILE: {
        ...
        CmpMessageSQLText sqltext(NULL, 0, CTXTHEAP, SQLCHARSETCODE_UNKNOWN, (CmpMessageObj::MessageTypeEnum)typ);
      ...
      case CmpMessageObj::SQLTEXT_STATIC_COMPILE: {
        cmpStatement = new CTXTHEAP CmpStatement(cmpContext_);
        CmpMessageCompileStmt compilestmt(NULL, 0, (CmpMessageObj::MessageTypeEnum)typ, CTXTHEAP);
      ...
      case (CmpMessageObj::INTERNALSP_GETNEXT): {
        ...
        CmpStatementISP *ispStatement = getISPStatement(ispGetNext.ispRequest());
        if (!ispStatement) {
      ...
      case (CmpMessageObj::CONNECTION_TYPE): {
        ...
        CmpMainISPConnection = (connectionType.connectionType() == CmpMessageConnectionType::ISP);
        break;
      }
      default:
        break;
    ...
    if (!CmpCommon::diags()->getNumber())
      *CmpCommon::diags() << DgSqlCode(arkcmpErrorAssert) << DgString0("EHBreakException") << DgString1(x.getFileName())
                          << DgInt0(x.getLineNum());
    ...
    *this << last_message;
    ...
    if (cmpStatement) {
      ...
      ComDiagsArea *diags = ComDiagsArea::allocate(ipcHeap);
      *diags << DgSqlCode(arkcmpErrorAssert) << DgString0("from longjmp(CmpConnection::actOnReceive)")
  ...
  if (cmpStatement) {
    ...
    if (diags->getNumber() > 0) *this << *cmpStatement->diags();
    if (cmpStatement->reply()) *this << *cmpStatement->reply();
  }
...
{
  if (getState() == ERROR_STATE) {
    ArkcmpFatalError(ARKCMP_ERROR_PREFIX "Error from sending reply back to executor.");
  }
}
...
void CmpIpcEnvironment::initControl(IpcServerAllocationMethod allocMethod, Int32 sockArg, Int32 portArg) {
  switch (allocMethod) {
    ...
    case IPC_SPAWN_OSS_PROCESS: {
      GuaReceiveControlConnection *cc = new (this) CmpGuaControlConnection(this);
...
CmpGuaControlConnection::CmpGuaControlConnection(IpcEnvironment *env, short receiveDepth)
