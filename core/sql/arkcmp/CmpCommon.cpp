...
#define VAILD_RANGE_CHECK(num)                                                \
    {                                                                         \
        if (num < 0 ||                                                        \
            (num != 0xee && num != 0xff && num > syntax.max_password_length)) \
        {                                                                     \
            isInvaild = TRUE;                                                 \
        }                                                                     \
    }
...
static NABoolean CheckPasswordPolicySyntax(ComPwdPolicySyntax &syntax)
 {
     ...
     if (syntax.min_password_length > syntax.max_password_length)
         isInvaild = TRUE;
    // min & max are not 0
     if (0 == syntax.max_password_length || 0 == syntax.min_password_length)
         isInvaild = TRUE;
    // min & max are less then MAX_PASSWORD_LENGTH
     if (syntax.max_password_length > MAX_PASSWORD_LENGTH ||
         syntax.min_password_length > MAX_PASSWORD_LENGTH)
         isInvaild = TRUE;
     if (isInvaild)
     {
         goto END;
     ...
     if (syntax.digital_chars_number == 0xee && syntax.lower_chars_number == 0xee &&
         syntax.upper_chars_number == 0xee && syntax.symbol_chars_number == 0xee)
     {
         isInvaild = TRUE;
         goto END;
     }
     ...
     if (syntax.digital_chars_number == 0xff && syntax.lower_chars_number == 0xff &&
         syntax.upper_chars_number == 0xff && syntax.symbol_chars_number == 0xff)
     {
         //reserves 3 chars
         charNums = 3;
     }
     //too long
     if (charNums > syntax.max_password_length)
         isInvaild = TRUE;
     //too short
...
NABoolean CmpCommon::ConvertPasswordPolicyString(const char *syntaxStr, ComPwdPolicySyntax &syntax, const char *where, bool justWarning)
{
...
//get from globals
ComAuthenticationType CmpCommon::getAuthenticationType()
{
    return GetCliGlobals()->getAuthenticationType();
}
...
//save to globals
void CmpCommon::setPasswordCheckSyntax(ComPwdPolicySyntax pwdPolicy) { GetCliGlobals()->setPasswordCheckSyntax(pwdPolicy); }
