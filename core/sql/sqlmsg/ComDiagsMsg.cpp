...
NABoolean ComSQLSTATE(int theSQLCODE, char *theSQLSTATE) {
  ...
  if (GetSqlstateInfo(theSQLCODE, theSQLSTATE, fabricatedSqlstate)) return fabricatedSqlstate;

  // We need to lookup the SQLSTATE for this SQLCODE.
  NAWchar *source;

  // Get the Message string for this error. The first field is the SQLSTATE.
  NABoolean sourceIsNotKnown = GetErrorMessage(MAIN_ERROR, theSQLCODE, source, SQL_STATE);
  ...
  if (!sourceIsNotKnown) {
    ...
    if (NAWstrcmp(source, WIDE_("ZZZZZ")) == 0)
      ...
      sourceIsAWarning = (theSQLSTATE[0] == '0' && (theSQLSTATE[1] == '1' || theSQLSTATE[1] == '2'));
      if ((theSQLCODE > 0 && sourceIsAWarning) || (theSQLCODE < 0 && !sourceIsAWarning))
...
static const char *returnClassOrigin(int theSQLCODE, size_t offset) {
  ...
  if ((sqlstate[offset] >= '0' && sqlstate[offset] <= '4') || (sqlstate[offset] >= 'A' && sqlstate[offset] <= 'H'))
    return "ISO 9075";
  else
...
const char *ComCondition::getClassOrigin() const { return ComClassOrigin(theSQLCODE_); }

const char *ComCondition::getSubClassOrigin() const { return ComSubClassOrigin(theSQLCODE_); }
...
ComDiagBigInt ComCondition::getMessageLength() {
  if (!isLocked_) getMessageText();
  return messageLen_;
}

ComDiagBigInt ComCondition::getMessageOctetLength() { return getMessageLength(); }
...
void appendSafeStringInW(NAWchar *dest, CollHeap *heap, const char *s, CharInfo::CharSet cs = CharInfo::ISO88591) {
  ...
  pOutputBuf = checkSpace(heap, len /* # of NAWchar's */, pOutputBuf, TRUE /* addNullAtEnd */);
  ...
  switch (convStatus) {
    ...
    case CNV_ERR_BUFFER_OVERRUN:
      if (outLenInW <= len) pOutputBuf->data()[outLenInW] = WIDE_('\0');
      NAWsprintf(dest, WIDE_("%s"), pOutputBuf->data());  // append to dest
      break;
...
UInt32 computeMsgLen(const ComCondition &cond) {
  ...
  if (((ComCondition &)cond).getCustomSQLState()) totallen += strlen(((ComCondition &)cond).getCustomSQLState());
...
#define INT_TO_DEST(x_)                   \
  {                                       \
    NAWsprintf(wbuffer, WIDE_("%d"), x_); \
    WSTRING_TO_DEST(wbuffer);             \
  }
...
#define STRING_TO_DEST_IF(x_)                                        \
  ...
  for (i = 0; i < ComCondition::NumOptionalParms; i++)
    if (cc.getOptionalInteger(i) != ComDiags_UnInitialized_Int) INT_TO_DEST(cc.getOptionalInteger(i));
...
void ComEMSSeverity(int theSQLCODE, char *theEMSSeverity) {
  ...
  NABoolean sourceIsNotKnown = GetErrorMessage(MAIN_ERROR, theSQLCODE, source, EMS_SEVERITY);
...
void ComEMSEventTarget(int theSQLCODE, char *theEMSEventTarget, NABoolean forceDialout) {
  ...
  if ((theSQLCODE == 0) || (theSQLCODE == 100)) {
    ...
    NABoolean sourceIsNotKnown = GetErrorMessage(MAIN_ERROR, theSQLCODE, source, EMS_EVENT_TARGET);

    if (!sourceIsNotKnown) {
      // Message found.
      numBytes = UnicodeStringToLocale(CharInfo::ISO88591, source, 7, theEMSEventTarget, 7, FALSE);
...
void ComEMSExperienceLevel(int theSQLCODE, char *theEMSExperienceLevel) {
  ...
  NABoolean sourceIsNotKnown = GetErrorMessage(MAIN_ERROR, theSQLCODE, source, EMS_EXPERIENCE_LEVEL);
...
const NAWchar *const ComCondition::getMessageText(NABoolean prefixAdded, CharInfo::CharSet isoMapCS) {
  if (!isLocked_) {
    ...
    // UR2 CNTNSK
    NABoolean msgNotFound = GetErrorMessage(MAIN_ERROR, arg, source, ERROR_TEXT, NULL, 0, prefixAdded);
    assert(source);

    // dbg: NAWstrcat(source, WIDE_(" $ $$ $ab $9 $~ $~~ $~0 $0~ $~a $a~ $0x $0x~int0 $int0~x # $0~int0 $int0~0 $0 $00
    // $0$0 $int0 $int0$int0"));
        ...
        case 3:
          if (isAlNum8859_1(*source) || *source == ERRORPARAM_TYPESEP)
            ...
            *theToken = L'\0';
            ...
            for (theToken = tokenBuf; *theToken; theToken++) *theToken = na_towlower(*theToken);
            theToken = tokenBuf;
            UInt32 tokIndex = 0;
            ...
            switch (tokIndex) {
...

              {
                ...
                if (CharInfo::isSingleByteCharSet(cs)) {
                  ...
                  if (availChars < len + 1) assert(FALSE);
                  appendSafeStringInW(dest, collHeapPtr_, optStr, cs);
                } else {
                  const NAWchar *optStrInW = getSafeWString(getOptionalWString(optStrNum));
              ...
              case INT0:
                NAWsprintf(dest, WIDE_("%d"), optionalInteger_[0]);
                break;
              case INT1:
                NAWsprintf(dest, WIDE_("%d"), optionalInteger_[1]);
                break;
              case INT2:
                NAWsprintf(dest, WIDE_("%d"), optionalInteger_[2]);
                break;
              case INT3:
                NAWsprintf(dest, WIDE_("%d"), optionalInteger_[3]);
                break;
              case INT4:
                NAWsprintf(dest, WIDE_("%d"), optionalInteger_[4]);
                break;
              case INTERNAL_SQLCODE:
                NAWsprintf(dest, WIDE_("%d"), theSQLCODE_);
                break;
              case CONDITION_NUMBER:
                NAWsprintf(dest, WIDE_("%d"), conditionNumber_);
                break;
              case SERVER_NAME:
              ...
              case CURSOR_NAME:
                ...
                break;
              case ROW_NUMBER:
                NAWsprintf(dest, WIDE_("%d"), rowNumber_);
                break;
              case NSK_CODE:
                NAWsprintf(dest, WIDE_("%d"), nskCode_);
                break;
