...
enum CM_SQLCODE_CHECK { CM_CHECK_0, CM_CHECK_100, CM_CHECK_0_100 };
enum CM_OPERATION { CM_DELETE, CM_INSERT, CM_SELECT, CM_UPDATE };

extern ComString CmUtilGetSMDLocation(ComString sys_name, ComString catalog_name, COM_VERSION version);

inline void CmGetComNameAsLit(const ComAnsiNameSpace enumValue, char *literal) {
  ComNameSpaceEnumToLiteral(enumValue, literal);
};

inline ComAnsiNameSpace CmGetComNameAsAnsiName(const char *literal) { return ComNameSpaceLiteralToEnum(literal); };

inline void CmGetComObjectAsLit(const ComObjectType enumValue, char *literal) {
  ComObjectTypeEnumToLiteral(enumValue, literal);
};

inline ComObjectType CmGetComObjectAsComObjectType(const char *literal) { return ComObjectTypeLiteralToEnum(literal); };

inline void CmGetComPrivAsLit(const ComPrivilegeType enumValue, char *literal) {
  ComPrivTypeEnumToLiteral(enumValue, literal);
};

inline ComPrivilegeType CmGetComPrivAsPrivilegeType(const char *literal) { return ComPrivTypeLiteralToEnum(literal); };
...
extern ComRoutineTransactionAttributes CmGetComRoutineTransactionAttributesAsRoutineTransactionAttributes(
    const char *literal);
...
inline ComSchemaOperation CmGetSchemaOperation(const char *literal) { return ComSchemaOperationLiteralToEnum(literal); }
...
inline void CmOwnerGrantorToUserGrantor(char *grantor_type) {
  if (!strcmp(grantor_type, COM_SCHEMA_OWNER_GRANTOR_LIT)) strcpy(grantor_type, COM_USER_GRANTOR_LIT);
}

inline void CmOwnerGranteeToUserGrantee(char *grantee_type) {
  if (!strcmp(grantee_type, COM_SCHEMA_OWNER_GRANTEE_LIT)) strcpy(grantee_type, COM_USER_GRANTEE_LIT);
}
