...
QRGroupLattice::QRGroupLattice(CollHeap *heap, ADD_MEMCHECK_ARGS_DEF(CollIndex maxNumKeys))
...
QRGroupLattice::~QRGroupLattice() { deletePtr(lattice_); }
...
void QRGroupLattice::insert(QRJoinSubGraphMapPtr map, const QRJBBPtr jbb) {
  ...
  assertLogAndThrow(CAT_GRP_LATTCE_INDX, LL_ERROR, ok, QRLogicException,
...
NABoolean QRGroupLattice::getGroupingLatticeKeys(LatticeKeyList &keyList, QRJoinSubGraphMapPtr map,
                                                 DescriptorDetailsPtr queryDetails, const QRJBBPtr jbb,
                                                 NABoolean primaryOnly, NABoolean insertMode) {
  ...
  if (groupBy->isEmpty()) return TRUE;
...
NABoolean QRGroupLattice::elementListToKeyList(const ElementPtrList &elementList, LatticeKeyList &keyList,
                                               QRJoinSubGraphMapPtr map, DescriptorDetailsPtr queryDetails,
  ...
  for (CollIndex i = 0; i < maxEntries; i++) {
    ...
    if (key == NULL) {
      ...
      // Cleanup
      for (CollIndex j = 0; j < i; j++) delete keyList[j];
      return FALSE;
    }
    keyList.insert(key);
  }
...
MVCandidatesForJBBSubsetPtr QRGroupLattice::search(QRJBBPtr queryJbb, MVCandidatesForJBBPtr mvCandidates,
                                                   QRJoinSubGraphMapPtr map, ElementPtrList *minimizedGroupingList) {
  ...
  if (minimizedGroupingList) {
    if (!elementListToKeyList(*minimizedGroupingList, keyList, map, queryDetails, FALSE)) return NULL;
  } else {
    ...
    if (!getGroupingLatticeKeys(keyList, map, queryDetails, queryJbb, TRUE, FALSE)) return NULL;
  }
  ...
  QRLogger::log(CAT_GRP_LATTCE_INDX, LL_DEBUG, "Searching LatticeIndex for: %s.", keysText.data());
  ...
  MVCandidatesForJBBSubsetPtr jbbSubset = new (heap_) MVCandidatesForJBBSubset(mvCandidates, ADD_MEMCHECK_ARGS(heap_));
  jbbSubset->setSubGraphMap(map);
  jbbSubset->setGroupBy(TRUE);

  if (minimizedGroupingList != NULL) jbbSubset->setMinimizedGroupingList(minimizedGroupingList);
  ...
  for (CollIndex i = 0; i < nodes.entries(); i++) {
    ...
    for (CollIndex j = 0; j < matchingMVs.entries(); j++) {
      ...
      QRLogger::log(CAT_GRP_LATTCE_INDX, LL_DEBUG, "Found MV: %s!", mv->getMVName().data());
...
LatticeIndexablePtr QRGroupLattice::elementToKey(const QRElementPtr element, QRJoinSubGraphMapPtr map,
