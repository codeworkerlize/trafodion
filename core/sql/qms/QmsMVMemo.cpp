...
MVMemoPhysicalExpression::~MVMemoPhysicalExpression() { deletePtr(map_); }

const NAString &MVMemoPhysicalExpression::getMVName() { return mv_->getMVName(); }
...
MVMemoLogicalExpressionPtr MVMemoGroup::addLogicalExpression(const NAString &key,
  ...
  MVMemoLogicalExpressionPtr newExpr =
      new (heap) MVMemoLogicalExpression(key, getGroupNumber(), ADD_MEMCHECK_ARGS_PASS(heap));
 ...
 *****************************************************************************
 */
NABoolean MVMemoGroup::addPhysicalExpression(const QRJoinSubGraphMapPtr map, const MVDetailsPtr mv, const QRJBBPtr jbb,
                                             NABoolean hasGroupby, ADD_MEMCHECK_ARGS_DEF(CollHeap *heap)) {
  ...
  MVMemoPhysicalExpressionPtr newExpr = new (heap) MVMemoPhysicalExpression(map, getGroupNumber(), mv, heap);
  ...
  if (hasGroupby) {
    try {
      ...
      QRLogger::log(CAT_GRP_LATTCE_INDX, LL_MVQR_FAIL, "Caught exception creating group lattice or inserting mv");
      throw;
    }
  }
 ...
 *****************************************************************************
 */
void MVMemoGroup::getPhysicalExpressions(MVCandidatesForJBBPtr mvCandidates, QRJoinSubGraphMapPtr querySubGraphMap,
                                         QRJBBPtr jbb, CollHeap *heap, NABoolean isPreferredMatch,
  ...
  if (groupLattice_ != NULL) {
    ...
    MVCandidatesForJBBSubsetPtr jbbSubset = new (heap) MVCandidatesForJBBSubset(mvCandidates, ADD_MEMCHECK_ARGS(heap));
    ...
    for (CollIndex j = 0; j < iterator.entries(); j++) {
      ...
      QRLogger::log(CAT_MVMEMO_JOINGRAPH, LL_DEBUG, "Found MV: %s!", mvName->data());
...
void MVMemoGroup::removePhysicalExpression(MVMemoPhysicalExpressionPtr expr) {
  ...
  if (groupLattice_ != NULL) {
    try  //@ZXtry
    {
      ...
      QRLogger::log(CAT_GRP_LATTCE_INDX, LL_MVQR_FAIL, "Caught exception removing entry from group lattice");
      throw;
    }
  }
}
  ...
  if (expr != NULL) {
    if (groupLattice_) {
      try  //@ZXtry
      {
        ...
        QRLogger::log(CAT_GRP_LATTCE_INDX, LL_MVQR_FAIL, "Caught exception removing mvDetails from group lattice");
        throw;
      }
    }
...
void MVMemoGroup::startRenameMV(const NAString &oldName) {
  ...
  assertLogAndThrow(CAT_MVMEMO_JOINGRAPH, LL_MVQR_FAIL, expr != NULL, QRLogicException,
...
void MVMemo::insert(QRJBBPtr jbb, MVDetailsPtr mv) {
  ...
  if (mv->hasGroupBy() || isWorkloadAnalysisMode) op = OP_INSERT_TOP;
  ...
  do  // For each subgraph produced by the HubIterator
  {
    ...
    if (subGraph->isSelfJoin() && !isSelfJoinTooBig && !isDuplicate && (!found || (NULL != map && map->isFull()))) {
...
void MVMemo::search(QRJBBPtr jbb, MVCandidatesForJBBPtr mvCandidates, CollHeap *heap) {
  QRLogger::log(CAT_MVMEMO_JOINGRAPH, LL_DEBUG, "==========>>> Matching JBB: %s.", jbb->getID().data());
  ...
  do  // For each subexpression hashKey produced by the HubIterator
  {
    ...
    QRLogger::log(CAT_MVMEMO_JOINGRAPH, LL_DEBUG, "Probing for hash key: \n%s", map->getHashKey().data());
 ...
 *****************************************************************************
 */
HubIteratorPtr MVMemo::createHubIteratorForJBB(const QRJBBPtr jbb, MVDetailsPtr mv, OperationType op, CollHeap *heap) {
 ...
 *****************************************************************************
 */
NABoolean MVMemo::handleInsertedSubgraph(QRJoinSubGraphMapPtr map, QRJBBPtr jbb, MVDetailsPtr mv,
                                         NABoolean &isDuplicate) {
  isDuplicate = FALSE;
  const NAString &key = map->getHashKey();
  MVMemoGroupPtr group = NULL;
  QRLogger::log(CAT_MVMEMO_JOINGRAPH, LL_DEBUG, "Checking hash key: \n%s", key.data());
  ...
  if (found)  // Was the expression found in the hash table?
  {
    ...
    QRLogger::log(CAT_MVMEMO_JOINGRAPH, LL_DEBUG, "  Found existing group %d.\n", groupNumber);
  } else {
    ...
    MVMemoLogicalExpressionPtr newExpr = group->addLogicalExpression(key, ADD_MEMCHECK_ARGS(heap_));
    insertExpression(newExpr);

    QRLogger::log(CAT_MVMEMO_JOINGRAPH, LL_DEBUG, "  Inserted hash key in new group %d.\n", group->getGroupNumber());
  ...
  if (map->isFull()) {
    ...
    QRLogger::log(CAT_MVMEMO_JOINGRAPH, LL_DEBUG, "  Inserted the MV in group %d.\n", group->getGroupNumber());
  }
...
MVMemoGroupPtr MVMemo::probeForSubGraph(HubIteratorPtr hubIterator, const QRJoinSubGraphMapPtr map) {
  ...
  if (contains(key))  // Was the expression found in the hash table?
  {
    ...
    if (expr->isPhysical()) {
      ...
      QRLogger::log(CAT_MVMEMO_JOINGRAPH, LL_DEBUG, "Found group no. %d.\n", group->getGroupNumber());
...
void MVMemo::reportStats(int numOfMVs) {
  ...
  for (currentGroupSize = 0; groupsProcessed < maxEntries; currentGroupSize++) {
    if (!groupsOrderedArray.used(currentGroupSize)) continue;
 ...
 *****************************************************************************
 */
GroupingListMinimizer::GroupingListMinimizer(const QRJBBPtr jbb, const QRJoinSubGraphMapPtr map,
                                             ADD_MEMCHECK_ARGS_DEF(CollHeap *heap))
...
NABoolean GroupingListMinimizer::isReducedTable(const NAString &tableID) {
  ...
  for (CollIndex i = 0; i < maxEntries; i++) {
    if (tablesToReduce_[i]->getID() == tableID) return TRUE;
  }
  ...
  for (CollIndex i = 0; i < maxEntries; i++) {
    ...
    if (groupingElem->getElementType() == ET_JoinPred) {
      ...
      if (eqList[0]->getElementType() == ET_Column) {
        ...
        assertLogAndThrow(CAT_MVMEMO_JOINGRAPH, LL_MVQR_FAIL, groupingElem->getElementType() == ET_Column,
                          QRLogicException, "Grouping by join preds on expressions not yet supported.");
      }
    } else {
      assertLogAndThrow(CAT_MVMEMO_JOINGRAPH, LL_MVQR_FAIL, groupingElem->getElementType() == ET_Column,
                        QRLogicException, "Grouping by expressions not yet supported.");
...
void GroupingListMinimizer::reduceTable(JoinGraphTablePtr table) {
  ...
  QRLogger::log(CAT_MVMEMO_JOINGRAPH, LL_DEBUG, "Table %s was reduced.", table->getName().data());
}
...
void GroupingListMinimizer::dumpResult(ElementPtrList *result) {
  QRLogger::log(CAT_MVMEMO_JOINGRAPH, LL_DEBUG, "All dimension tables were successfully reduced!");
...
ElementPtrList *GroupingListMinimizer::calcIndirectGroupingList() {
  ...
  if (verifyAggregateFunctions() == FALSE) return NULL;
  ...
  if (tablesToReduce.entries() > 0) {
    ...
    QRLogger::log(CAT_MVMEMO_JOINGRAPH, LL_DEBUG, "Table %s could not be reduced.",
                  tablesToReduce[0]->getName().data());
    return NULL;
  }

  // Check that partitionedGroupingList_ does not contain any of the tables in tablesToReduce_
  if (verifyTablesWereReduced() == FALSE) return NULL;
