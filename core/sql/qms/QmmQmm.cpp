...
void QmmGuaReceiveControlConnection::actOnSystemMessage(short messageNum, IpcMessageBufferPtr sysMsg,
  ...
  switch (messageNum) {
    case ZSYS_VAL_SMSG_OPEN: {
      ...
      QmmMessageStream *msgStream =
          new QmmMessageStream(const_cast<IpcEnvironment *>(qmm_->getEnvironment()), qmmName, qmm_);
    ...
    case ZSYS_VAL_SMSG_PROCDEATH:
      ...
      if (connection) connection->setFatalError(NULL);  //@ZX
      break;

    case ZSYS_VAL_SMSG_CPUDOWN: {
      zsys_ddl_smsg_cpudown_def *msg = (zsys_ddl_smsg_cpudown_def *)sysMsg;
      QRLogger::log(CAT_QR_IPC, LL_INFO, "Cpu %d on qmm's segment has gone down.", msg->z_cpunumber);
      qmm_->getQmsStub(1, msg->z_cpunumber)->disable(QmsStub::CPU_NOT_REACHABLE);
    } break;

    case ZSYS_VAL_SMSG_CPUUP: {
      zsys_ddl_smsg_cpuup_def *msg = (zsys_ddl_smsg_cpuup_def *)sysMsg;
      QRLogger::log(CAT_QR_IPC, LL_INFO, "Cpu %d on qmm's segment has come back up.", msg->z_cpunumber);
    ...
    case ZSYS_VAL_SMSG_REMOTECPUDOWN: {
      ...
      QRLogger::log(CAT_QR_IPC, LL_INFO, "Cpu %d on segment %d has gone down.", msg->z_cpunumber, msg->z_nodenumber);

      qmm_->getQmsStub(msg->z_nodenumber, msg->z_cpunumber)->disable(QmsStub::CPU_NOT_REACHABLE);
    } break;

    case ZSYS_VAL_SMSG_REMOTECPUUP: {
      zsys_ddl_smsg_remotecpuup_def *msg = (zsys_ddl_smsg_remotecpuup_def *)sysMsg;
      QRLogger::log(CAT_QR_IPC, LL_INFO, "Cpu %d on segment %d has come back up.", msg->z_cpunumber, msg->z_nodenumber);
    ...
    case ZSYS_VAL_SMSG_NODEDOWN: {
      ...
      QRLogger::log(CAT_QR_IPC, LL_INFO, "Segment %d has gone down.", msg->z_nodenumber);
      for (short cpu = 0; cpu < CPUS_PER_SEGMENT; cpu++)
        qmm_->getQmsStub(msg->z_nodenumber, cpu)->disable(QmsStub::SEGMENT_NOT_REACHABLE);
    } break;

    case ZSYS_VAL_SMSG_NODEUP: {
      zsys_ddl_smsg_nodeup_def *msg = (zsys_ddl_smsg_nodeup_def *)sysMsg;
      QRLogger::log(CAT_QR_IPC, LL_INFO, "Segment %d has come back up.", msg->z_nodenumber);
      for (short cpu = 0; cpu < CPUS_PER_SEGMENT; cpu++)
...
Qmm::Qmm(CollHeap *heap) : qmsPool_(NULL), qmsCount_(0), qmp_(NULL), heap_(heap) {
  ...
  // processes.
  QmsStub::setQmsServerClass(new (heap_) IpcServerClass(ipcEnv_, IPC_SQLQMS_SERVER, IPC_SPAWN_OSS_PROCESS));
}
...
void Qmm::handleClientExit(const short *phandle, short messageNum) {
  ...
  QRLogger::log(CAT_QR_IPC, LL_INFO, "dead process is on cpu <%d> and pin <%d>", cpu, pin);
  ...
  if (!qmpDied) {
    ...
    for (int i = 0; i < qmsCount_ && !found; i++) {
      ...
      if (result || (lc_cpu < 0) || (lc_pin < 0)) processDoesNotExist = TRUE;
    ...
    if (!found) {
      if (messageNum == ZSYS_VAL_SMSG_CLOSE)
        QRLogger::log(CAT_QR_IPC, LL_INFO, "*** Unknown client process has ended. ***");
      else if (messageNum == ZSYS_VAL_SMSG_PROCDEATH)
        QRLogger::log(CAT_QR_IPC, LL_INFO, "*** Unknown child process has died. ***");
      else
        QRLogger::log(CAT_QR_IPC, LL_INFO, "*** Unknown process has terminated, message=%d. ***", messageNum);
    }
  ...
  if (messageNum == ZSYS_VAL_SMSG_CLOSE)
    QRLogger::log(CAT_QR_IPC, LL_INFO, "*** CLOSE message received for process %d,%d on segment %d ***", cpu, pin,
                  segmentNumber);
  else if (messageNum == ZSYS_VAL_SMSG_PROCDEATH)
    QRLogger::log(CAT_QR_IPC, LL_INFO, "*** PROCDEATH message received for process %d,%d on segment %d ***", cpu, pin,
                  segmentNumber);
  else
    QRLogger::log(CAT_QR_IPC, LL_INFO, "*** Process %d,%d on segment %d has terminated, message=%d. ***", cpu, pin,
                  segmentNumber, messageNum);

  if (qmp_ && *qmp_ == *(SB_Phandle_Type *)phandle) {
    QRLogger::log(CAT_QR_IPC, LL_INFO, "*** QMP has died, will attempt to restart...");
    ...
    QRLogger::log(CAT_QR_IPC, LL_WARN, "Could not identify terminated process as either QMP or QMS");
}
...
void Qmm::allocateQms() {
  ...
  short result = NODENUMBER_TO_NODENAME_(-1, segmentName, SEGMENT_NAME_LEN, &segmentNameLen);
  segmentName[segmentNameLen] = '\0';
  QRLogger::log(CAT_QR_IPC, LL_DEBUG, "Result for caller's segment is %d, segment name is %s", result, segmentName);
  qmsCount_ = 1;
  ...
  for (Int16 i = 1; i <= maxRetries && needRetry; i++) {
    ...
    for (Int16 cpuInx = 0; cpuInx < qmsCount_; cpuInx++) {
      if (qmsPool_[cpuInx]->getStatus() == QmsStub::NOT_RUNNING) {
        ...
        if (!qmsPool_[cpuInx]->start()) needRetry = TRUE;  // at least 1 qms didn't start
      }
    }
  }
}
...
NABoolean QmpStub::start() {
  ...
  if (phandle != NULL) {
    QRLogger::log(CAT_QR_IPC, LL_INFO, "qmm found existing qmp process, not starting new one.");
  ...
  switch ((int)qmpStartOpt_) {
    ...
    case NONE:
      ...
      break;
    default:
      assertLogAndThrow1(CAT_QR_IPC, LL_ERROR, FALSE, QmmException, "Unknown option for starting QMP -- %d",
                         qmpStartOpt_);
      break;
  }
...
void QmpStub::allocateProcess(IpcEnvironment &ipcEnv, short cpu) {
  if (!qmpServerClass_) qmpServerClass_ = new (heap_) IpcServerClass(&ipcEnv, IPC_SQLQMP_SERVER, IPC_SPAWN_OSS_PROCESS);
  ...
  short result = NODENUMBER_TO_NODENAME_(-1, localSegmentName, SEGMENT_NAME_LEN, &localSegmentNameLen);
  assertLogAndThrow1(CAT_QR_IPC, LL_ERROR, result == 0, QmmException,
                     "Could not get name of local segment, error is %d", result);
  localSegmentName[localSegmentNameLen] = '\0';
  qmpServer_ = qmpServerClass_->allocateServerProcess(&diagsArea, heap_, localSegmentName + 1, cpu,
                                                      IPC_PRIORITY_DONT_CARE, 1, TRUE, TRUE);
  if (qmpServer_) {
    QRLogger::log(CAT_QR_IPC, LL_DEBUG, "QMP process started on cpu #%d", qmpServer_->getServerId().getCpuNum());
    setProcessHandle(qmpServer_->getServerId().getPhandle().phandle_);
  } else {
    QRLogger::log(CAT_QR_IPC, LL_ERROR, "Failed to allocate server process for QMP on cpu %d ", cpu);
...
#define SET_ARGV(argv, argc, argval)                \
  {                                                 \
    argv[argc] = (char *)calloc(strlen(argval), 1); \
  ...
  msg_mon_start_process(prog,                     /* prog */
                        process_name,             /* name */
                        process_name,             /* output process name */
                        largc,                    /* args */
                        largv, &p_handle, 0,      /* open */
                        &server_oid,              /* oid */
...
void Qmm::relayPendingPubsToQms() {
  ...
  QRLogger::log(CAT_QR_IPC, LL_DEBUG, "Sent %d Publish requests to QMS list", pendingPubs_.entries());
  for (CollIndex i = 0; i < pendingPubs_.entries(); i++) pendingPubs_[i]->decrRefCount();
  pendingPubs_.clear();
}

void Qmm::executeMessageLoop() {
  // Have to allocate this from heap, because ~IpcEnvironment deletes it.
  QmmGuaReceiveControlConnection *conn = new (heap_) QmmGuaReceiveControlConnection(ipcEnv_, this);
  ...
  if (listenOpt_ == WAITONALL) {
    ...
    assertLogAndThrow1(CAT_QR_IPC, LL_ERROR, listenOpt_ == RECEIVE, QmmException, "Unknown listen opt -- %d",
                       listenOpt_);
    firstClient = conn->getConnection();
    msgStream.addRecipient(firstClient);
    ...
    while (TRUE) {
      ...
      if (!responseObj) responseObj = new QRStatusMessageObj(QR::InternalError);
...
QRRequestResult Qmm::handleAllocateRequest(QRMessageStream *msgStream) { return QR::Unable; }
...
QRMessageObj *Qmm::processRequestMessage(QRMessageStream *msgStream) {
  ...
  if (!msgStream->moreObjects()) {
    QRLogger::log(CAT_QR_IPC, LL_ERROR, "QMM received an empty message stream.");
    ...
    switch (msgStream->getNextObjType()) {
      ...
      case PUBLISH_REQUEST:
        ...
        if (result == QR::Success)
          ...
          QRLogger::log(CAT_QR_IPC, LL_ERROR, "PUBLISH failed, status is %d", result);
      ...
      case ALLOCATE_REQUEST:
        ...
        if (result == QR::Success)
          ...
          QRLogger::log(CAT_QR_IPC, LL_ERROR, "ALLOCATE failed, status is %d", result);
        responseMsgPtr = new QRStatusMessageObj(result);
        break;

      case DEFAULTS_REQUEST:
        QRLogger::log(CAT_QR_IPC, LL_ERROR, "'DEFAULTS' request not yet handled by QMM");
        responseMsgPtr = new QRStatusMessageObj(QR::InvalidRequest);
        msgStream->clearAllObjects();
        break;

      default:
        QRLogger::log(CAT_QR_IPC, LL_ERROR, "QMM received unexpected message type: %d", msgStream->getNextObjType());
        responseMsgPtr = new QRStatusMessageObj(QR::InvalidRequest);
        msgStream->clearAllObjects();
        break;
    }
...
IpcTimeout Qmm::getWaitTimeout() {
  ...
  if (restartListEntries == 0) return IpcInfiniteTimeout;
...
QRProcessStub::QRProcessStub(CollHeap *heap) : lockoutEndTS_(0), retryNumber_(0), heap_(heap) {
...
void QRProcessStub::checkRestarts() {
  ...
  for (CollIndex i = restartList_.entries(); i > 0; i--) {
    ...
    if (processStub->lockoutEndTS_ < NA_JulianTimestamp()) {
      ...
      if (processStub->start()) restartList_.removeAt(i - 1);
...
QmsStub::QmsStub(short segmentNumber, char *segmentName, short cpuNumber, short segmentStatus, NABoolean cpuExists,
                 NABoolean cpuReachable,  // QRMessageStream* qmsMsgStream,
                 IpcEnvironment *ipcEnv, CollHeap *heap)
    : QRProcessStub(heap),
  ...
  if (!cpuExists) {
    ...
    QRLogger::log(CAT_QR_IPC, LL_DEBUG, "Processor %d on segment %s is not reachable.", cpuNumber, segmentName);
    return;
...
NABoolean QmsStub::start() {
  ...
  assertLogAndThrow(CAT_QR_IPC, LL_ERROR, qmsServerClass_, QmmException, "qmsServerClass_ is NULL");
  ...
  if (qmsServer_) {
    ...
    if (status_ == RUNNING) {
      ...
      // be started below.
      QRLogger::log(CAT_QR_IPC, LL_DEBUG, "-- QMS process has status %d, will restart.", status_);
      disable(NOT_RUNNING);
    }
  }
  ...
  qmsServer_ = qmsServerClass_->allocateServerProcess(&diagsArea, heap_, segmentName_ + 1, cpuNumber_,
                                                      IPC_PRIORITY_DONT_CARE, 1, TRUE, TRUE, 2, NULL, qmsProcessName_);
  ...
  if (qmsServer_) {
    ...
    QRLogger::log(CAT_QR_IPC, LL_DEBUG, "QMS process started on cpu #%d", qmsServer_->getServerId().getCpuNum());
    MvQueryRewriteServer::initQms(qmsServer_, heap_);
    return TRUE;
  } else {
    status_ = NOT_RUNNING;
    QRLogger::log(CAT_QR_IPC, LL_ERROR, "Failed to allocate server process for QMS on cpu %d of segment %s", cpuNumber_,
                  segmentName_);
    QRLogger::logDiags(diagsArea, CAT_QR_IPC);
    return FALSE;
  }
