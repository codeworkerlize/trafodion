...
class QRLatticeIndexNode : public NAIntrusiveSharedPtrObject {
 ...
 public:
  ...
  const LatticeKeySubArray &getKeyBitmap() const { return keyBitmap_; }
   ...
   * @return 1 if the two nodes are equal, otherwise 0.
   */
  NABoolean isEqual(const QRLatticeIndexNode &other) { return *this == other; }
   ...
   */
  NABoolean isSupersetOf(const QRLatticeIndexNode &other) const { return keyBitmap_.contains(other.keyBitmap_); }
   ...
   */
  NABoolean isSubsetOf(const QRLatticeIndexNode &other) const { return other.isSupersetOf(*this); }

  /**
   * Returns a reference to the list of MVs associated with this node.
   * @return Reference to list of pointers to MVDetails.
   */
  const SubGraphMapList &getMVs() const { return mapList_; }
   ...
   */
  const NAPtrList<QRLatticeIndexNodePtr> &getParents() const { return parents_; }
   ...
   */
  const NAPtrList<QRLatticeIndexNodePtr> &getChildren() const { return children_; }
   ...
   */
  virtual NABoolean isLatticeNode() const { return TRUE; }
   ...
   */
  NABoolean isValid() const { return isValid_; }
   ...
   *            DOT text.
   */
  void dumpLattice(NAString &graphText, NAString &graphLabel);
 ...
 protected:
   ...
   * @param heap Heap to use for any memory allocations.
   */
  QRLatticeIndexNode(QRLatticeIndexPtr lattice, NABoolean noNewEntry = FALSE,
  ...
  NABoolean hasKey(CollIndex i) const { return keyBitmap_.testBit(i); }
 ...
 private:
   ...
   * @param heap Heap to use for any memory allocations.
   */
  QRLatticeIndexNode(const char *const name, QRLatticeIndexPtr lattice, LatticeKeyArray *keyArray,
                     ADD_MEMCHECK_ARGS_DECL(CollHeap *heap = NULL));
   ...
   */
  void addMV(QRJoinSubGraphMapPtr map) { mapList_.insert(map); }
...
class QRLatticeIndexSearchNode : public QRLatticeIndexNode {
 public:
   ...
   * @param heap Heap to use for any memory allocations.
   */
  QRLatticeIndexSearchNode(const LatticeKeyList &keys, QRLatticeIndexPtr lattice, NABoolean noNewKeys = FALSE,
                           ADD_MEMCHECK_ARGS_DECL(CollHeap *heap = NULL))
      : QRLatticeIndexNode(keys, lattice, SEQNUM_UNASSIGNED, noNewKeys, ADD_MEMCHECK_ARGS_PASS(heap)) {}
   ...
   * @param heap Heap to use for any memory allocations.
   */
  QRLatticeIndexSearchNode(QRLatticeIndexPtr lattice, NABoolean noNewKeys = FALSE,
                           ADD_MEMCHECK_ARGS_DECL(CollHeap *heap = NULL))
   ...
   * @return FALSE
   */
  virtual NABoolean isLatticeNode() const { return FALSE; }
...
class QRLatticeIndex : public NAIntrusiveSharedPtrObject {
 ...
 public:
   ...
   */
  QRLatticeIndex(CollHeap *heap = NULL, ADD_MEMCHECK_ARGS_DECL(CollIndex maxNumKeys = 128));
   ...
   * @return
   */
  NABoolean addKeysToBitmap(const LatticeKeyList &keys, NABoolean noNewEntry, LatticeKeySubArray &keyBitmap);
   ...
   * @see QRLatticeIndexLock
   */
  void endTraversal() { inTraversal_ = FALSE; }
   ...
   */
  void findSubsets(QRLatticeIndexNode &supersetNode, NAPtrList<QRLatticeIndexNodePtr> &subsets);
   ...
   */
  void findSupersets(QRLatticeIndexNode &subsetNode, NAPtrList<QRLatticeIndexNodePtr> &supersets);
   ...
   */
  CollIndex getMaxNumKeys() const { return maxNumKeys_; }
   ...
   */
  const QRLatticeIndexNode &getTopNode() const { return *topNode_; }
   ...
   */
  const QRLatticeIndexNode &getBottomNode() const { return *bottomNode_; }

  LatticeKeyArray *getKeyArr() { return &keyArr_; }
 ...
 private:
   ...
   *                    insertion point.
   */
  void insertNode(QRLatticeIndexNodePtr nodeToInsert, QRLatticeIndexNodePtr nodeToCheck);
   ...
   */
  void findAndAttachSubset(QRLatticeIndexNodePtr insertedNode, QRLatticeIndexNodePtr searchRoot);
   ...
   * @param supersets The list to which any found superset nodes are added.
   */
  void findSupersets_(QRLatticeIndexNode &subsetNode, QRLatticeIndexNodePtr parentNode,
                      NAPtrList<QRLatticeIndexNodePtr> &supersets);

  void getSupersets(QRLatticeIndexNodePtr node, NAPtrList<QRLatticeIndexNodePtr> &supersets);
   ...
   * @param subsets The list to which any found subset nodes are added.
   */
  void findSubsets_(QRLatticeIndexNode &supersetNode, QRLatticeIndexNodePtr childNode,
                    NAPtrList<QRLatticeIndexNodePtr> &subsets);

  void getSubsets(QRLatticeIndexNodePtr node, NAPtrList<QRLatticeIndexNodePtr> &subsets);
...
class QRLatticeIndexLock {
 public:
   ...
   */
  QRLatticeIndexLock(QRLatticeIndexPtr lattice) : lattice_(lattice) { lattice_->startTraversal(); }

  /**
   * Ends the traversal and releases the lock.
   */
  virtual ~QRLatticeIndexLock() { lattice_->endTraversal(); }
