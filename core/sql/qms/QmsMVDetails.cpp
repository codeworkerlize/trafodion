...
Visitor::VisitResult RefFinderVisitor::visit(QRElementPtr caller) {
  ...
  if (caller->getRef() == "") return VR_Continue;

  QRElementPtr elem = descDetails_->getElementForID(caller->getRef());
  assertLogAndThrow(CAT_MATCHTST_MVDETAILS, LL_MVQR_FAIL, elem != NULL, QRLogicException,
...
void BaseTableDetails::addRangePredicateOnColumn(const QRRangePredPtr rangePred, const QRColumnPtr rangeCol) {
  ...
  assertLogAndThrow(CAT_MATCHTST_MVDETAILS, LL_MVQR_FAIL, index != -1, QRLogicException,
...
void JBBDetails::initBaseTables(CollHeap *heap) {
  ...
  for (CollIndex i = 0; i < hubTables.entries(); i++) {
    ...
    if (jbbc->getElementType() != ET_Table) continue;  // Skip reference to other JBBs.

    const QRTablePtr table = jbbc->downCastToQRTable();
    if (table->hasLOJParent()) hasLOJs_ = TRUE;

    // Rest of the stuff only needed for MVs.
    if (!isAnMV_) continue;

    BaseTableDetailsPtr tableDetails = new (heap) BaseTableDetails(table, TRUE, ADD_MEMCHECK_ARGS(heap));
  ...
  if (extraHubTables != NULL) {
    for (CollIndex j = 0; j < extraHubTables->entries(); j++) {
      ...
      BaseTableDetailsPtr tableDetails = new (heap) BaseTableDetails(table, FALSE, ADD_MEMCHECK_ARGS(heap));
      baseTablesByID_.insert(&tableDetails->getID(), tableDetails);
    }
  }
}
    ...
    if (rangeElem->getElementType() == ET_Column) {
      ...
      if (!descDetails_->isFromJoin(rangeColumn)) {
        ...
        for (CollIndex i = 0; i < equalitySet.entries(); i++) {
          ...
          if (halfPred->getElementType() != ET_Column) continue;
      ...
      assertLogAndThrow(CAT_MATCHTST_MVDETAILS, LL_MVQR_FAIL, (rangeElem->getElementType() == ET_JoinPred),
                        QRLogicException, "Unexpected JoinPred element.");
      ...
      assertLogAndThrow(CAT_MATCHTST_MVDETAILS, LL_MVQR_FAIL, (inputs.entries() <= 1), QRLogicException,
...
DescriptorDetails::~DescriptorDetails() {
  for (CollIndex i = 0; i < jbbDetailsList_.entries(); i++) deletePtr(jbbDetailsList_[i]);
...
void DescriptorDetails::init(CollHeap *heap) {
  ...
  for (CollIndex i = 0; i < jbbs.entries(); i++) {
    QRJBBPtr jbb = jbbs[i];
    JBBDetailsPtr jbbDetails = new (heap) JBBDetails(jbb, this, isAnMV_, ADD_MEMCHECK_ARGS(heap));
...
void DescriptorDetails::initJoinPredList(const QRJoinPredListPtr jpList) {
  ...
  for (CollIndex i = 0; i < maxEntries; i++) {
    ...
    for (CollIndex i = 0; i < equalitySet.entries(); i++) {
      ...
      if (getElementForID(eqSetColumn->getTableID()) == NULL) continue;

      // Skip columns from LOJ tables.
      if (isColumnFromLojTable(eqSetColumn)) continue;
...
NABoolean DescriptorDetails::isColumnFromLojTable(const QRColumnPtr col) {
  ...
  const QRElementPtr tableElem = getElementForID(tableID);
  assertLogAndThrow(CAT_MATCHTST_MVDETAILS, LL_MVQR_FAIL, tableElem, QRLogicException, "Table not found in JBB.");
  const QRTablePtr table = tableElem->downCastToQRTable();
...
// ***************************************************************************
MVDetails::~MVDetails() {
  if (joinGraph_) deletePtr(joinGraph_);
  ...
  if (mvMemoGroups_.entries() > 0) disengage();
...
void MVDetails::init(CollHeap *heap) {
  ...
  const NAPtrList<QRJBBPtr> &jbbs = descriptor_->getJbbList();
  assertLogAndThrow(CAT_MATCHTST_MVDETAILS, LL_MVQR_FAIL, jbbs.entries() == 1, QRLogicException,
...
void MVDetails::initOutputs(const QRJBBPtr jbb, CollHeap *heap) {
  ...
  assertLogAndThrow(CAT_MATCHTST_MVDETAILS, LL_MVQR_FAIL, outputList != NULL, QRLogicException, "Output list is null.");
    ...
    switch (outputItem->getElementType()) {
      case ET_Column: {
        ...
        if (!isFromJoin(col) || hasLOJParent) {
          ...
          const NAString *idQualifiedColName = calcIDQualifiedColumnName(col->getTableID(), col->getColumnName(), heap);
      ...
      case ET_JoinPred: {
        assertLogAndThrow(CAT_MATCHTST_MVDETAILS, LL_MVQR_FAIL, (outputItem->getElementType() == ET_JoinPred),
                          QRLogicException, "Unexpected JoinPred element.");
      ...
      case ET_Expr: {
        ...
        QRLogger::log(CAT_MATCHTST_MVDETAILS, LL_DEBUG, "Adding output expression: %s.", exprText.data());
        ...
        if (rootExpr->getElementType() == ET_Function) {
          ...
          if (func->getAggregateFunc() == QRFunction::AFT_COUNTSTAR) {
            ...
            countStar_ = output;
          } else if (countStar_ == NULL && func->isCountStarEquivalent(heap)) {
...
// ***************************************************************************
// ***************************************************************************
void MVDetails::addEqualitySet(QRJoinPredPtr joinPredElement, NABoolean isFromExpr, QROutputPtr output,
                               CollHeap *heap) {
  const ElementPtrList &equalitySet = joinPredElement->getEqualityList();
  ...
  for (CollIndex i = 0; i < equalitySet.entries(); i++) {
    ...
    if (tableElem->downCastToQRTable()->hasLOJParent()) continue;
...
// ***************************************************************************
const QROutputPtr MVDetails::getOutputByColumnName(const NAString &id, const NAString &name, CollHeap *heap) {
...
const QROutputPtr MVDetails::getOutputByExprText(const NAString &text) {
  ...
  QRLogger::log(CAT_MATCHTST_MVDETAILS, LL_DEBUG, "Checking for output expression: %s, %s.", text.data(),
...
// ***************************************************************************
const QROutputPtr MVDetails::getOutputByInputColumns(const NAString &id, const NAString &name, CollHeap *heap) {
  const NAString *idName = calcIDQualifiedColumnName(id, name, heap);
