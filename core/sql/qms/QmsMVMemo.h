...
class MVMemoLogicalExpression : public NAIntrusiveSharedPtrObject {
 public:
  ...
  NABoolean operator==(const MVMemoLogicalExpression &other) { return !hashKey_.compareTo(other.hashKey_); }
...
class MVMemoPhysicalExpression : public MVMemoLogicalExpression {
 public:
   ...
   * @return
   */
  const QRJoinSubGraphMapPtr getMvSubGraphMap() { return map_; }
...
class MVMemoGroup : public NAIntrusiveSharedPtrObject {
 public:
   ...
   */
  virtual ~MVMemoGroup() {
    if (groupLattice_) deletePtr(groupLattice_);
  }
   ...
   */
  NABoolean hasPhysicalExpressions() { return (physicalExprsHash_.entries() > 0); }

  /**
   * Add the MVs in this group to the list of candidates so far.
   */
  void getPhysicalExpressions(MVCandidatesForJBBPtr candidateList, QRJoinSubGraphMapPtr querySubGraphMap, QRJBBPtr jbb,
                              CollHeap *heap, NABoolean isPreferredMatch, ElementPtrList *minimizedGroupingList);

  /**
   * Add a logical expression to the group's list of logical expressions.
   * @param expr The logical expression to add.
   */
  MVMemoLogicalExpressionPtr addLogicalExpression(const NAString &key, ADD_MEMCHECK_ARGS_DECL(CollHeap *heap));

  /**
   * Add a physical expression to the group's list of physical expressions.
   */
  NABoolean addPhysicalExpression(const QRJoinSubGraphMapPtr map, const MVDetailsPtr mv, const QRJBBPtr jbb,
                                  NABoolean hasGroupby, ADD_MEMCHECK_ARGS_DECL(CollHeap *heap));
   ...
   */
  void setGroupLattice(QRGroupLatticePtr lattice) { groupLattice_ = lattice; }
...
class MVMemo : public NAIntrusiveSharedPtrObject {
 public:
  ...
  MVMemoGroupPtr allocateNewGroup() {
    MVMemoGroupPtr newGroup = new (heap_) MVMemoGroup(nextGroup_++, ADD_MEMCHECK_ARGS(heap_));
   ...
   */
  NABoolean contains(const NAString &key) { return expressionHash_.contains(&key); }
   ...
   */
  MVMemoLogicalExpressionPtr getExpression(const NAString &key) { return expressionHash_.getFirstValue(&key); }

  /**
   * Insert the specified expression into the expression hash table.
   * @param expr
   */
  void insertExpression(MVMemoLogicalExpressionPtr expr) { expressionHash_.insert(&expr->getHashKey(), expr); }

  NABoolean handleInsertedSubgraph(QRJoinSubGraphMapPtr map, QRJBBPtr jbb, MVDetailsPtr mv, NABoolean &isDuplicate);

  MVMemoGroupPtr probeForSubGraph(HubIteratorPtr hubIterator, const QRJoinSubGraphMapPtr map);
...
class GroupingListMinimizer : public NAIntrusiveSharedPtrObject {
 public:
   ...
   * @return
   */
  GroupingListMinimizer(const QRJBBPtr jbb, const QRJoinSubGraphMapPtr map, ADD_MEMCHECK_ARGS_DECL(CollHeap *heap));
