...
void ProposedMV::initFrom(QRJoinSubGraphMapPtr map) {
  ...
  // easily check for duplicates.
  if (jbb->getGroupBy() && jbb->getGroupBy()->getPrimaryList() && jbb->getGroupBy()->getPrimaryList()->entries() > 0) {
  ...
  if (rangePreds && rangePreds->entries()) rangePredList_.insert(rangePreds->getList());

  // Initialize residual predicates.
  QRResidualPredListPtr residualPreds = hub->getResidualPredList();
  if (residualPreds && residualPreds->entries()) residualPredList_.insert(residualPreds->getList());
}
...
void ProposedMV::initializeMVPairList() {
  ...
  for (CollIndex i = 1; i < howmanyQueries; i++) {
    ...
    MVPairPtr pair = new (heap_) MVPair(firstQuery, otherQuery, ADD_MEMCHECK_ARGS_PASS(heap_));
  ...
  for (CollIndex i = 0; i < maxEntries; i++) {
    ...
    if (!rangeBitmap.isEmpty() || !residualBitmap.isEmpty()) {
      ...
      for (CollIndex j = 0; j < pairs; j++) {
        ...
        if (rangeBitmap.isEmpty() && residualBitmap.isEmpty()) break;
      }
    }
    ...
    if (anyTableWithCommonRangePreds && anyTableWithCommonResidualPreds) break;
  }  // for (jbbcs)
...
void ProposedMV::addMvSideColumnToSelectList(const QRColumnPtr mvCol, MVPairPtr thisPair) {
  ...
  const BaseTableDetailsPtr baseTable = thisPair->getQueryTableForMvID(mvCol->getTableID());

  // Look it up in the query side hash table.
  const NAString &querySideTableID = baseTable->getTableElement()->getID();
  QRElementPtr queryCol = queryDetails->getOutputByColumnName(querySideTableID, mvCol->getColumnName(), heap_);
...
void ProposedMV::handleResidualPredicates() {
  ...
  if (canSkipResidualPreds_) return;
...
// ***********************************************************************
// ***********************************************************************
NABoolean ProposedMV::isAddedLojTable(const NAString &id) { return addedLojTables_.contains(&id); }
...
void ProposedMV::addElementToSelectList(const QRElementPtr elem) {
  ...
  if (hasGroupBy_ && elem->getElementType() == ET_Column) {
    if (groupingColumns_.getFirstValue(colName) == NULL) groupingColumns_.insert(colName, elem);
  }
...
void ProposedMV::reportResults(NAString &text, NABoolean addComments) {
  ...
  char buffer[100];
  assertLogAndThrow(CAT_MVMEMO_JOINGRAPH, LL_ERROR, mapList_.entries() > 0, QRLogicException,
...
void ProposedMV::unparseGroupByClause(NAString &text) {
  ...
  for (CollIndex i = 1; i <= maxEntries; i++) {
    ...
    if (groupingColumns_.getFirstValue(elemText)) {
      ...
      if (!groupingColumns_.isEmpty()) text += ", ";
    }
  }
...
void ProposedMV::unparseFromClause(NAString &text) {
  ...
  QRJoinSubGraphPtr subGraph = new (heap_) QRJoinSubGraph(joinGraph, ADD_MEMCHECK_ARGS(heap_));
  if (hasGroupBy_) subGraph->setGroupBy();
  ...
  while (!subGraph->isFull()) {
    ...
    if (tableElem->hasLOJParent() || addedLojTables_.contains(&tableElem->getID()))
...
NAString ProposedMV::getOnClauseFor(QRJoinSubGraphPtr subGraph, JoinGraphTablePtr nextTable) {
  ...
  for (CollIndex i = 0; i < eqSets.entries(); i++) {
    ...
    if (!eqSet->isConnectedTo(subGraph)) continue;
    ...
    for (CollIndex j = 0; j <= halfPreds.entries(); j++) {
      ...
      if (halfPred == targetHalfPred) continue;

      // Skip half preds pointing to tables still not in the subGraph.
      if (!subGraph->contains(halfPred->getTable())) continue;
...
NABoolean WorkloadAnalysis::addProposedMV(ProposedMVPtr pmv) {
  ...
  if (mapList.isEmpty()) {
    ...
    QRLogger::log(CAT_QMS_MAIN, LL_DEBUG, "Ignoring duplicate queries: %s.", skippedList.data());
    return FALSE;
  } else {
    // Set the name of this proposed MV from its ordinal number.
    pmv->setName(nextMV_++);
    proposedMVsList_.insert(pmv);
    QRLogger::log(CAT_QMS_MAIN, LL_DEBUG, "Inserting ProposedMV: %s using: %s and ignoring: ", pmv->getName().data(),
                  addedList.data(), skippedList.data());
    return TRUE;
  }
}
...
void WorkloadAnalysis::reportResults(ofstream &ofs, int minQueriesPerMV) {
  ...
  QRLogger::instance().log(CAT_QMS_MAIN, LL_INFO, "Workload analysis result:\n");
...
void WorkloadAnalysis::findReducedPredicateSet() {
  ...
  for (int i = proposedMVsList_.entries() - 1; i >= 0; i--) {
    ...
    try {
      ...
      QRLogger::instance().log(CAT_QMS_MAIN, LL_ERROR, "Exception thrown during workload analysis. MV %s skipped.",
                               pmv->getName().data());
