...
class JoinGraphTable : public NAIntrusiveSharedPtrObject {
 public:
   ...
   */
  JoinGraphTable(CollIndex num, const QRTablePtr tableElement, NABoolean isHub, const BaseTableDetailsPtr baseTable,
  ...
  NABoolean operator==(const JoinGraphTable &other) { return ordinalNumber_ == other.ordinalNumber_; }
  ...
  void removeEqualitySet(JoinGraphEqualitySetPtr pred) { predList_.remove(pred); }
  ...
  /** Find an equality set from the ones this table is connected to, that match otherHalfPred, which is from a different
   * join graph. */
  ...
  void reduceConnection() { reducedConnections_++; }

  NABoolean isOnTheEdge();

  void setSelfJoinTable() { isSelfJoinTable_ = TRUE; }

  NABoolean isSelfJoinTable() { return isSelfJoinTable_; }
...
class JoinGraphHalfPredicate : public NAIntrusiveSharedPtrObject {
 public:
   ...
   * @param heap     Heap from which to allocate internal data members.
   */
  JoinGraphHalfPredicate(const JoinGraphTablePtr tablePtr, const QRElementPtr elementPtr, const NAString &expr,
                         ADD_MEMCHECK_ARGS_DECL(CollHeap *heap))
      : NAIntrusiveSharedPtrObject(ADD_MEMCHECK_ARGS_PASS(heap)),
        tablePtr_(tablePtr),
   ...
   */
  NABoolean match(JoinGraphHalfPredicatePtr other) const { return getExpr() == other->getExpr(); }
...
class JoinGraphEqualitySet : public NAIntrusiveSharedPtrObject {
 public:
   ...
   * @param heap  Heap from which to allocate internal data members.
   */
  JoinGraphEqualitySet(const NAString &id, ADD_MEMCHECK_ARGS_DECL(CollHeap *heap))
      : NAIntrusiveSharedPtrObject(ADD_MEMCHECK_ARGS_PASS(heap)),
        halfPreds_(heap),
  ...
  void addHalfPredicate(JoinGraphHalfPredicatePtr pred) {
    ...
    if (pred->getExpr().length() > maxExprSize_) maxExprSize_ = pred->getExpr().length();
  }
...
class QRJoinGraph : public NAIntrusiveSharedPtrObject {
 ...
 public:
   ...
   */
  JoinGraphTablePtr getTableByID(const NAString &id) { return tableHashByID_.getFirstValue(&id); }
   ...
   * @return Pointer to JoinGraphTablePtr object.
   */
  JoinGraphTablePtr getTableByOrdinal(CollIndex ix) { return tableArray_[ix]; }
   ...
   * @return
   */
  CollIndex getNumberOfTables() { return nextTable_; }

  JoinGraphTableList &getTablesToReduce() { return tablesToReduce_; }

  QRJoinSubGraphPtr getMinimizedSubGraph() { return minimizedSubGraph_; }
   ...
   */
  void addTable(const QRTablePtr tableElement, NABoolean isHub, const BaseTableDetailsPtr baseTable);
   ...
   */
  void addHalfPredicate(JoinGraphEqualitySetPtr equalitySet, JoinGraphTablePtr tablePtr, const QRElementPtr elementPtr,
                        const NAString &expr);
...
class QRJoinSubGraph : public NAIntrusiveSharedPtrObject {
 public:
  ...
  NABoolean contains(JoinGraphTablePtr table) { return subArray_.contains(table->getOrdinalNumber()); }

  /** Return the pointer to the parent JoinGraph. */
  QRJoinGraphPtr getParentGraph() { return parentGraph_; }

  /** equals operator. */
  NABoolean operator==(const QRJoinSubGraph &other) {
    return subArray_ == other.subArray_ && hasGroupBy_ == other.hasGroupBy_;
  }

  void updateTables(const QRJoinSubGraphPtr other) { subArray_ = other->subArray_; }
  ...
  void setStillUsed(NABoolean used) { stillUsed_ = used; }
  NABoolean stillUsed() { return stillUsed_; }
  ...
  NABoolean isFull() const {
    return subArray_.entries() == parentGraph_->getHubSize() && hasGroupBy_ == parentGraph_->hasGroupBy();
  }
  ...
  void setSelfJoin() { isSelfJoin_ = TRUE; }

  /**
   *
   * @return
   */
  SelfJoinHandlerPtr getSelfJoinHandler() { return selfJoinHandler_; }
...
class QRJoinSubGraphMap : public NAIntrusiveSharedPtrObject {
 public:
   ...
   * @param heap   Heap from which to allocate internal data members.
   */
  QRJoinSubGraphMap(QRJoinSubGraphPtr subGraph, NABoolean isMV, CollIndex size, ADD_MEMCHECK_ARGS_DECL(CollHeap *heap))
      : NAIntrusiveSharedPtrObject(ADD_MEMCHECK_ARGS_PASS(heap)),
        subGraph_(subGraph),
        hashKey_(heap),
   ...
   * @return
   */
  NABoolean isFull() const { return subGraph_->isFull(); }

  void setHashKey(const NAString &key) { hashKey_ = key; }

  const NAString &getHashKey() { return hashKey_; }

  QRJoinSubGraphPtr getSubGraph() { return subGraph_; }

  NABoolean contains(const NAString &id) { return indexHashByID_.contains(&id); }

  void setMVDetails(MVDetailsPtr mv) { mvDetails_ = mv; }

  MVDetailsPtr getMVDetails() const { return mvDetails_; }
  ...
  const NAString &getShiftVector() { return shiftVector_; }
 ...
 private:
  class UnsignedInteger : public NABasicObject {
   public:
    // Default constructor
    UnsignedInteger() : data_(0) {}

    UnsignedInteger(CollIndex data) : data_(data) {}

    virtual ~UnsignedInteger() {}

    // Copy construction
    UnsignedInteger(const UnsignedInteger &other) : data_(other.data_) {}
    ...
    NABoolean operator==(const UnsignedInteger &other) { return data_ == other.data_; }

    CollIndex getInt() { return data_; }
...
class HubIterator : public NAIntrusiveSharedPtrObject {
 public:
   ...
   * @param heap         Heap from which to allocate internal data members.
   */
  HubIterator(QRJoinGraphPtr joinGraph, OperationType isForInsert, ADD_MEMCHECK_ARGS_DECL(CollHeap *heap))
      : NAIntrusiveSharedPtrObject(ADD_MEMCHECK_ARGS_PASS(heap)),
   ...
   */
  NABoolean hasNext() {
    assertLogAndThrow(CAT_MVMEMO_JOINGRAPH, LL_MVQR_FAIL, subGraphsVisited_.entries() > 0, QRLogicException,
                      "This method should not be called before trying any subgraphs.");

    // Check if we have subgraphs to work on.
    if (subGraphsReady_.entries() > 0) return TRUE;

    // Did we check the top subgraph with the GroupBy flag yet?
    if (joinGraph_->hasGroupBy() && !usedGroupBy_) return TRUE;
