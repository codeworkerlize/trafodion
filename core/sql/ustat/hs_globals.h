...
enum SortState
  {
    ...
    DONT_PROCESS,   // Just use in if not exists, don't create histogram
...
struct HSColGroupStruct : public NABasicObject
  {
    ...
    NABoolean sameGroup(const HSColGroupStruct * other) const;
...
private:
    ...
    void stowPreprocessedText(const char * text,size_t length);
    void stowExpressionText(const char * text, size_t length);
    void stowPartitionForText(const char * text, size_t length);

    void stowPartitionV2KeyText(const char * text, size_t length);
public:
    // preprocess input text looking for EXPRESSSION ( <expression text> )
    ...
    Lng32 GetStatistics4PartitionTableV2(char* &outBuf, ULng32 &outBufLen, CollHeap *heap);
    ...
    // validate expression used for expression histograms
    Lng32 validateExpression(const char * expressionText, HSColGroupStruct * group);

                                              /*==============================*/
                                              /*     OBJECT INFORMATION       */
                                              /*==============================*/
    HSTableDef    *objDef;                         /* object definition       */
    HSTableDef    *objGenStatsSource;              /* only use for GENERATE STATISTICS */
    ...
    Int64          minRowCtPerPartition_;          /* minimal rows per partition */
                                              ...
                                              /*==============================*/
    NAString *     preprocessedInput;         /* preprocessed input text if any */
    NAArray<NAString *> * expressionText;     /* text from EXPRESSION ( <text> ) */
    NAString *     partitionV2NameText;       /* text from partition V2 name */
    NAString *     partitionV2ForClauseText;  /* text from partition V2 for clause */
    NABoolean      isPartitionV2TableHasForClause; /* is partition table has for clause */
    NABoolean      isPartitionV2Table;        /* is a partition V2 table    */
    NAString *     onClauseText;              /* text from on CLAUSE        */
    ...
    NABoolean      isGenerateStatsSourceTable;     /* is generate stats source table*/
    Lng32           groupCount;   /* total #column groups, excluding expr groups  */
    Lng32           singleGroupCount;               /* #single-column groups   */
    Lng32           singleExpressionGroupCount;    /* #single-col expr groups  */
    HSColGroupStruct *singleGroup;                 /* single-column group list*/
    HSColGroupStruct *multiGroup;                  /* multi-column group list */
    HSColGroupStruct *singleExpressionGroup;   /* single expression group list */
