...
void HSTableDef::setNATable()
  {
    ...
    naTbl_ = bindWA.getNATableInternal(corrName);
    if (!bindWA.errStatus())
      {
        HS_ASSERT(naTbl_);
    ...
    if (hs_globals_obj && hs_globals_obj->isPartitionV2Table &&
        naTbl_ &&
        naTbl_->isPartitionV2Table())
      {    
        NAString sPartitionName;
        for ( int i=0; i<naTbl_->getNAPartitionArray().entries(); i++) 
          {    
            NAString s(naTbl_->getNAPartitionArray()[i]->getPartitionName());
            if (s == *hs_globals_obj->partitionV2NameText)
              {    
                sPartitionName = naTbl_->getNAPartitionArray()[i]->getPartitionEntityName();
                break;
              }    
          }    
        if (sPartitionName.length() > 0) 
          {    
            CorrName partitionName(sPartitionName, STMTHEAP, *schema_, *catalog_);
            naTbl_ = bindWA.getNATableInternal(partitionName);
            *object_ = sPartitionName;
          }    
      }
  }
...
NAString computeAnsiName(const NAString & columnName)
  {
    ...
    for (Int32 i = 0; i < numCols_; i++)
      {
        ...
        *colInfo_[i].externalColumnName = computeAnsiName(*colInfo_[i].colname);
                                                  /*== GET COLUMN DATATYPE ==*/
        retcode_ = SQL_EXEC_GetDescItem(outputDesc, entry,
                                        SQLDESC_TYPE_FS,
                                        &colInfo_[i].datatype,
      ...
      if (!nac->isHiveVirtualColumn() || collectVirtColStats)
        {
          ...
          *(colInfo_[currColEntry].externalColumnName) = computeAnsiName(colName);
...
NABoolean HSTableDef::setObjectUID(NABoolean createExternalTable)
{
  ...
  if (createExternalTable && objectUID_ <= 0 && 
   ...
   if ((catName == HIVE_SYSTEM_CATALOG) &&
    ...
    NAString schNameInt = getSchemaName(INTERNAL_FORMAT);
    NAString objNameInt = getObjectName(INTERNAL_FORMAT);
    CorrName corrName(objNameInt, STMTHEAP, schNameInt, catName);
...
// Function to parse and bind an expression (for an expression histogram),
// returning its NAType information
Lng32 parseAndBindExpression(const char * expressionText, 
