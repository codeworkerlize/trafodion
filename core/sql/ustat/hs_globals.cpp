  ...
  if (!caseInsensitive) {
    if (charset != CharInfo::UNICODE)
    {
      ...
      /*return compareWcharWithBlankPadding((const wchar_t*)(content+VARCHAR_LEN_FIELD_IN_BYTES),
       ...
       return wcharStringCompareWithPad((wchar_t*)(content+VARCHAR_LEN_FIELD_IN_BYTES),
                                        *((short*)content) / sizeof(NAWchar),
                                        (wchar_t*)(rhs.content+VARCHAR_LEN_FIELD_IN_BYTES),
                                        *((short*)rhs.content) / sizeof(NAWchar),
        ...
        if (isFixedChar)
          {
            ...
            qry += temp;
            qry += " CHAR)";
            if (charset == CharInfo::ISO88591 || charset == CharInfo::BINARY)
              qry += " CHARACTER SET ISO88591";
            qry += ")";
          }
        // doesn't matter here what we rename it to, but we don't want to
        // use externalColumnName->data() because that itself might be
        // an expression
        qry += " AS \"X\"";
      }
    else
...
HSGlobalsClass::HSGlobalsClass(ComDiagsArea &diags)
  : catSch(new(STMTHEAP) NAString(STMTHEAP)),
    ...
    groupCount(0), singleGroupCount(0), singleExpressionGroupCount(0),
    singleGroup(NULL), multiGroup(NULL),
    singleExpressionGroup(NULL),
    parserError(ERROR_SYNTAX), errorCount(0), diagsArea(diags),
    sampleTableUsed(FALSE),
    samplingUsed(FALSE),

    preprocessedInput(NULL), expressionText(NULL),
    partitionV2NameText(NULL), isPartitionV2Table(FALSE), onClauseText(NULL),
    isPartitionV2TableHasForClause(FALSE), partitionV2ForClauseText(NULL),
    ...
    isGenerateStatsSourceTable(FALSE),
...
HSGlobalsClass::~HSGlobalsClass()
{
  ...
  if (preprocessedInput)
    delete preprocessedInput;

  if (expressionText)
    {
      for (CollIndex i = 0; i < expressionText->entries(); i++)
        {
          NAString * text = (*expressionText)[i];
          delete text;
        }
      delete expressionText;
    }
...
Lng32 HSGlobalsClass::Initialize()
  {
    ...
    if (!hs_globals->objDef->getNATable()->isPartitionV2Table())
      {
      ...
      //For the partition table, we don't care what the rowcount value is,
      //so we set a value arbitrarily here.
      actualRowCount =10000;
    LM->StopTimer();
    if (LM->LogNeeded())
      {
    ...
    if (currentRowCountIsEstimate_ && !(optFlags & CLEAR_OPT) &&
        !hs_globals->objDef->getNATable()->isPartitionV2Table())
      {
    ...
    // Move the single expression histogram chain to the single column
    // group chain; from here on out, we'll process them together. 

    HSColGroupStruct * lastSingleGroup = singleGroup;
    if (!lastSingleGroup)
      {
        singleGroup = singleExpressionGroup; 
      }
    else
      {
        while (lastSingleGroup->next)
          {
            lastSingleGroup = lastSingleGroup->next;
          } 
        lastSingleGroup->next = singleExpressionGroup;
      }   
    singleExpressionGroup = NULL; 

    // This can't be done before checking columns implied by NECESSARY (above).
    if (((groupCount == 0) &&                     // (no grouplist specified
         (singleExpressionGroupCount == 0)) ||    // and no expressions groups)
        (optFlags & CLEAR_OPT))                   // or CLEAR option used
      {
            ...
            switch (optFlags & SAMPLE_REQUESTED)
              {
                ...
                {
                  xSampleSet = getDefaultSampleRowSize(actualRowCount);
                  xSampleSet = MAXOF(xSampleSet, numPartitions*2);

                  sampleTblPercent = convertInt64ToDouble(xSampleSet) * 100.0 / 
...
// helper function for preprocessInput; returns TRUE if the input begins
// with the keyword EXPRESSION
NABoolean isExpression(const char * input)
{
  NABoolean rc = ((*input == 'E') || (*input == 'e'));
  if (rc)
    {
      char localBuf[10+1];
      strncpy(localBuf,input,10);
      localBuf[10] = '\0';
      for (size_t i = 0; i < 10; i++)
        localBuf[i] = toupper(localBuf[i]);

      if (strcmp(localBuf,"EXPRESSION") == 0)
        {
          if (!isalnum(input[10]) && (input[10] != '_'))
            rc = TRUE;
          else
            rc = FALSE; // it's an identifier that begins with EXPRESSION
        }
      else
        rc = FALSE;
    }
...
NABoolean isPartitionFor(const char * input, size_t & length)
{
  NABoolean rc = ((*input == 'P') || (*input == 'p'));
  if (rc)
    {
      char localBuf[9+1];
      strncpy(localBuf,input,9);
      localBuf[9] = '\0';
      for (size_t i = 0; i < 9; i++)
        localBuf[i] = toupper(localBuf[i]);

      if (strcmp(localBuf,"PARTITION") == 0)
        {
          if (!isalnum(input[9]) && (input[9] != '_'))
          {
            length = 9;
            int i = 9;
            while(input[i] == ' ')
            ...
            rc = ((input[i] == 'F') || (input[i] == 'f')) && ((input[i+1] == 'O') || (input[i+1] == 'o')) && ((input[i+2] == 'R') || (input[i+2] == 'r'));
            if (rc)
            {
              length += 3;
            }
...
// helper methods for preprocessInput
void HSGlobalsClass::stowPreprocessedText(const char * text, size_t length)
{
  if (!preprocessedInput)
    preprocessedInput = new NAString(text,length,STMTHEAP);
  else
    preprocessedInput->append(text,length);
}

void HSGlobalsClass::stowExpressionText(const char * text, size_t length)
{
  if (!expressionText)
    expressionText = new NAArray<NAString *>(STMTHEAP);

  // add the expression text to the array; note preprocessedInput is known
  // to be allocated by the time we get here
  
  NAString * savedText = new NAString(text,length,STMTHEAP);
  expressionText->resize(expressionText->entries()+1);  // TODO: be smarter about this later
  expressionText->insertAt(expressionText->entries(),savedText);  // I wish there was a simple append method

  // generate replacement text to put into preprocessed input

  char buffer[20]; // enough room for '$ <number>'
  sprintf(buffer,"$ %d",expressionText->entries()-1);

  (*preprocessedInput) += buffer;
}

void HSGlobalsClass::stowPartitionForText(const char * text, size_t length)
{
  if (!partitionV2ForClauseText)
    partitionV2ForClauseText = new(STMTHEAP) NAString(text,length,STMTHEAP);
  // generate replacement text to put into preprocessed input
...
// *****************************************************************************
// *                                                                           *
// * Function: HSGlobalsClass::preprocessInput                                 *
// *                                                                           *
// * UPDATE STATISTICS has a separate parser from the main SQL parser. The     *
// * SQL parser in general recognizes just enough to tell that a given SQL     *
// * statement is UPDATE STATISTICS, and then packages up the statement text   *
// * and passes that to UPDATE STATISTICS. This has worked OK for much of the  *
// * history of this code. However in more recent years the shortcomings of    *
// * that design become more apparent.                                         *
// *                                                                           *
// * In particular, UPDATE STATISTICS now accepts fairly arbitrary SQL         *
// * expressions within its text. When encountered, UPDATE STATISTICS then     *
// * plays the game in reverse: It bundles up these little pieces of text      *
// * and ships them off to the SQL parser.                                     *
// *                                                                           *
// * The first example of this was incremental UPDATE STATISTICS, with its     *
// * WHERE clause. We got away with a hack on this one: WHERE is always the    *
// * last thing in the UPDATE STATISTICS text, so once we saw the WHERE        * 
// * keyword we could simply package up all the text after it.                 *
// *                                                                           *
// * The second and more recent example is the support for expression          *
// * histograms. There, in the ON clause, we want to be able to specify        *
// * one or more expressions on which we want to build a histogram. The ON     *
// * clause might not be the last thing in the statement, so the UPDATE        * 
// * statistics lexer and parser need to be able to tell where an expression   *
// * ends. This is hard in general; to do it absolutely right requires         *
// * gluing in the SQL lexer and parser. But if we are going to do that, we    *
// * should bite the bullet and integrate the two parsers. That's a big job,   *
// * given the very different nature of semantic actions in the two parsers.   *
// *                                                                           *
// * So for the time being, we resort to another hack. The user will specify   *
// * expressions using the syntax EXPRESSION ( <expression text> ). The        *
// * ending parenthesis gives us what we need to find the end of the           * 
// * expression. This is hard to do within the existing lexer and parser,      *
// * though. Lookahead issues get in the way. So, we do some ad hoc            *
// * preprocessing here to make it easier on the lexer and parser:             * 
// * We look for occurrances of EXPRESSION ( <expression text> ), and          *
// * when we find one, we replace the expression text with tokens of the       *
// * form $ n, saving the expression text in an array that the parser can      *
// * then access in its semantic action.                                       *
// *                                                                           *
// * Aside: I considered various alternative syntaxes:                         *
// *                                                                           *
// * EXPERSSION <expression text> -- doesn't work, as the lexer can't tell     *
// * where the <expression text> ends                                          *
// * [ <expression text> ] -- can be done in the lexer and parser, but it      *
// * wouldn't allow a square bracket within a string literal                   *
// * ( <expression text> ) -- conflicts with existing syntax; (A,B) is         *
// * used to specify a multi-column histogram on columns A and B.              *
// * '<expression text>' -- that is, use a string literal. That requires       *
// * the user to escape any single quote in the expression text. As            * 
// * string literals are likely in such text, this seems undesireable.         *
// *                                                                           *
// *****************************************************************************
// *                                                                           *
// *  Parameters:                                                              *
// *                                                                           *
// *  <input>   UPDATE STATISTICS statement text                               *
// *                                                                           *
// * returns:                                                                  *
// *   a pointer to the preprocessed text                                      *
// *                                                                           *
// * Presently this routine does not return any errors. We can rely on the     *
// * lexer and parser to report syntax errors.                                 *
// *                                                                           *
// *****************************************************************************
char * HSGlobalsClass::preprocessInput(char * input)
{
  // hand-coded state machine
  enum State { INIT, SAW_EXPRESSION, LPAREN, IN_STRING, IN_DELIMITED_IDENTIFIER, IN_IDENT, SAW_PARTITION_FOR, PARTFOR };
  State state = INIT;
  State pushedState = INIT;
  int parenNesting = 0;
  char * next = input;
  char * rc = input;
  char * currentTextFragment = input;
              ...
              if (isalpha(*next))
                {
                  ...
                  if (isExpression(next))
                    {
                      state = SAW_EXPRESSION;
                      next = next+strlen("EXPRESSION")-1;  // -1 because of next++ at bottom of loop
                    }
                  else if (isPartitionFor(next, length))
                    {
                      isPartitionV2TableHasForClause = TRUE;
                      next = next + length -1;  // -1 because of next++ at bottom of loop
                      state = SAW_PARTITION_FOR;
                    }
                            ...
                            if (*(next+2) == ' ' || *(next+2) == '(')
                              {
                                if (!onClauseText)
                                  {
                                    onClauseText = new NAString(next,strlen(next),STMTHEAP);
                                  }
                              } 
                        }
                      state = IN_IDENT;
                ...
                state = IN_DELIMITED_IDENTIFIER;
              // else remain in INIT state
              break;
            }
          case SAW_EXPRESSION:
          case SAW_PARTITION_FOR:
            {
              if (*next == '(')
                {
                  if (state == SAW_EXPRESSION)
                    state = LPAREN;
                  else
                    state = PARTFOR;
                  parenNesting = 0;
                  size_t currentTextFragmentLength = (next+1) - currentTextFragment;
                  stowPreprocessedText(currentTextFragment,currentTextFragmentLength);
                  currentTextFragment = next+1;  // expression text starts after (
                }
              else if (*next == '\'')
                state = IN_STRING;
              else if (*next == '"')
                ...
                state = INIT;  
              // else remain in SAW_EXPRESSION state          
              break;
            }
          case LPAREN:
          case PARTFOR:
              ...
              if (*next == ')')
                {
                  if (parenNesting == 0)
                    {
                      size_t currentExpressionLength = next - currentTextFragment;
                      if (state == LPAREN)
                        stowExpressionText(currentTextFragment,currentExpressionLength);
                      else
                        stowPartitionForText(currentTextFragment,currentExpressionLength);
                      currentTextFragment = next;  // expression ends before )
                      state = INIT;
                    }
                  else
                    parenNesting--;
          ...
          case IN_STRING:
            {
              ...
              // else remain in IN_STRING state
              break;
            }
          case IN_DELIMITED_IDENTIFIER:
            {
              ...
              // else remain in IN_DELIMITED_IDENTIFER state
              break;
            }
          case IN_IDENT:
            {
  ...
  if (input == currentTextFragment)  // if no expressions were found
    rc = input;
  else
    {
      // stow any remaining fragment
      if (next > currentTextFragment)
        stowPreprocessedText(currentTextFragment,next - currentTextFragment); 

      // Ugh... have to cast away constness here; the flex lexer requires 
      // a char * even though it never modifies the text. We could avoid
      // this by creating another NAString-like class whose data method
      // returned a non-const char *, or we could add code here to make a 
      // copy of the data to a char buffer that the caller provides, but 
      // that seems like a lot of unnecessary trouble for the moment.
      // Can revisit later if needed.
      rc = (char *)preprocessedInput->data();
    }
...
Lng32 HSGlobalsClass::MakeAllHistid()
  {
    ...
    if (missingHistograms.length() > 0
        && (!(optFlags & NECESSARY_OPT)
            && !(optFlags & CLEAR_OPT)
            && !(optFlags & IF_NOT_EXISTS_OPT)))
      {
  ...
  while (group)
    {
     ...
     else if ((group->colNames->data()[0] == '"') || (group->colSet[0].colnum == -1))
...
void HSGlobalsClass::improveEstimateRowCount()
{
...
Lng32 HSGlobalsClass::CollectStatistics()
  {
    ...
    NAString innerColumnName, columnName, dblQuote="\"";
    ...
    NABoolean hasImproveEstimateRowCount = FALSE;
    ...
    if (useBackingSample)
    ...
    else if (! IsNAStringSpaceOrEmpty(sampleTableFromCQD))
      {
        *hssample_table = sampleTableFromCQD;
        ...
        sampleTableUsed     = TRUE;
        samplingUsed        = TRUE;
        sampleRowCount = actualRowCount / 100;// 1% sample for row
      }
        ...
        while (numColsToProcess > 0)
          {
            ...
            if (!hasImproveEstimateRowCount)
              {
                //after readColumnsIntoMem, we can improve estimate row count
                hasImproveEstimateRowCount = TRUE;
                improveEstimateRowCount();
              }
    ...
    while (group != NULL)
      {
        if (group->state == PROCESSED || DONT_PROCESS == group->state)
          {
            group = group->next;
            continue;
          }
        ...
        bool expressionHistogram = (group->colSet[0].colnum == -1);

        // Surround column name with double quotes, if not already delimited.
        if ((group->colNames->data()[0] == '"') || expressionHistogram)
          innerColumnName=group->colNames->data();
        else                                   
          innerColumnName=dblQuote+group->colNames->data()+dblQuote;

        // Pick a column name to use in the outer SELECTs
        if (expressionHistogram)
          columnName = "\"X\"";   // use "X" for expressions
        else
        ...
        if (isOverSized || expressionHistogram)
        {
          // Stick in a nested select that truncates the string and/or
          // packages up an expression as a column. We have to do it
          // here so the truncated string or expression can be used as
          ...
          if (isOverSized)
          {
            ...
            group->clistr->append(innerColumnName.data());
...
void genArkcmpInfo(NAString& nidpid)
{
  ...
  msg_mon_get_my_info(&mon_nid,       // out, mon node-id
                      &mon_pid,       // out, mon process-id
                      buf,            // out, mon name
                      sizeof(buf)-1,  // in, mon name-len
                      NULL,           // ignored, mon process-type
                      NULL,           // ignored, mon zone-id
                      NULL,           // ignored, os process-id
                      NULL);          // ignored, os thread-id

  buf[sizeof(buf)-1] = '\0';  // for safety

  char buf2[128];  // max size of SB_PERSISTENT_SAMPLES.UPDATER_INFO

  snprintf(buf2,sizeof(buf2),"Process %s nid:pid %d:%d",buf,mon_nid,mon_pid);
   ...
   while (group) {
      ...
      if ( group->cbf && group->state == PROCESSED ) {
        ...
        if ( fd != -1 ) {
           ...
           char* buffer = bufptr;
           sz = group->cbf->packIntoBuffer(buffer);
           HS_ASSERT( sz <= bufSz);
...
Lng32 HSGlobalsClass::FlushStatistics(NABoolean &statsWritten)
  {
    ...
    else if ((groupCount == 0) && (singleExpressionGroupCount == 0)) 
      {
    ...
    if ((optFlags & IUS_OPT) || (optFlags & IUS_PERSIST))
      {
        if (singleExpressionGroupCount)
          {
            // Give a warning: Expression histograms not supported for IUS yet
            HSFuncMergeDiags(UERR_EXPRESSION_HISTOGRAMS_IGNORED_FOR_IUS);
          }
      }
...
Lng32 HSGlobalsClass::WriteStatistics()
  {
    ...
    //
    // In C++11, auto_ptr is replaced by unique_ptr.
#if __cplusplus < 201103L 
    std::auto_ptr<HSinsertHist> histRS(NULL);
    std::auto_ptr<HSinsertHistint> histintRS(NULL);
#else
    std::unique_ptr<HSinsertHist> histRS(nullptr);
    std::unique_ptr<HSinsertHistint> histintRS(nullptr);
#endif
    ...
    //Int64 hv3           = 0;
    //Int64 hv4           = 0;
    // V5 used for expression text for expression histogram in SB_HISTOGRAMS
    HSDataBuffer hv5;
    //HSDataBuffer hv6(L"");
        ...
        for (i = 0; i < group->colCount; i++)
          {
            col = &group->colSet[i];
            if ((group->colCount == 1) &&
                (group->colSet[0].colnum == -1))
              {
                // expression histogram; put expression text in V5; note that
                // it is in UTF-8 format (we will rely on TRANSLATE to make it
                // UCS2)
                NAString * unparsedExpr = group->colNames;
                hv5.copyFromUTF8(unparsedExpr->data(),unparsedExpr->length());
                if (hv5.length() > V5_MAX_LEN)
                {
                  HSFuncMergeDiags(-4228);
                  retcode = -1;
                  HSHandleError(retcode);
                }
              }
            ...
            if (group->newHistid)
              retcode = histRS->addRow(objDef->getObjectUID(),
                                      ...
                                      avgVarCharSize,
                                      0, // v3, currently unused
                                      0, // v4, currently unused
                                      hv5
                                      // currently unused.
                                      );
            else if (LM->LogNeeded())
...
Lng32 HSGlobalsClass::groupListFromTable(HSColGroupStruct*& groupList,
                                         ...
                                         NABoolean exclusive)
  {
    ...
    char expressionText[4*V5_MAX_LEN+1+2]; // allow up to 4 bytes per char UCS2 -> UTF8, 2 bytes for len field
    ...
    NAString qry = "SELECT HISTOGRAM_ID, COL_POSITION, COLUMN_NUMBER, COLCOUNT, "
                   "cast(READ_TIME as char(19) character set iso88591), REASON, TRANSLATE(V5 USING UCS2TOUTF8) "
                   "FROM ";
    ...
    while (retcode == 0)
        {
          ...
          // else it's an expression histogram; leave col with default values
          ...
          if (colNum >= 0)
            ...
            {
              size_t expressionTextLength = *(short *)expressionText; // varchar length field
              char * expressionTextBody = expressionText+2;  // skip over length field
              expressionTextBody[expressionTextLength] = '\0';
              columnName = expressionTextBody;  // get expression text from column V5
            }
          ...
          if (colNum >= 0)
            ...
            *group->colNames += columnName;  // expression, already in right format
          *group->colNames += ",";

          for (Int32 i=1; i < colCount; i++)  // GET ALL RELATED ROWS (multi-column histograms)
            {
              if (tableFormat == SQLMX && HSGlobalsClass::schemaVersion >= COM_VERS_2300)
                retcode = cursor103.fetch (7,
                                          (void *)&histid, (void *)&colPos,
...
Lng32 HSGlobalsClass::ComputeMCStatistics(NABoolean usingIS)
{
    ...
    HSCursor *cursor;
    HSLogMan *LM = HSLogMan::Instance();
    NAWchar *highval = new(STMTHEAP) NAWchar[HS_MAX_BOUNDARY_LEN + 1];
    NAWchar *lowval = new(STMTHEAP) NAWchar[HS_MAX_BOUNDARY_LEN + 1];
    ...
    while (mgroup != NULL)
    {
        ...
        if(((!usingIS) || (coveredByIS)) && !collectMCSkewedValues)
        {
            ...
            for (Int32 i=0; i<mgroup->colCount && !(lowIsFull && hiIsFull); i++)
            {
               ...
               sgroup = findGroup(col->colnum);
               if ( sgroup->state == DONT_PROCESS && (optFlags & IF_NOT_EXISTS_OPT))
                 {
                   //get single column hight value/low value for histogram
                   NAString qry = "SELECT LOW_VALUE,HIGH_VALUE FROM ";
                   qry.append(hstogram_table->data())
                      .append(" WHERE TABLE_UID=");
                   char sbuf[25];
                   sprintf(sbuf, PF64, objDef->getObjectUID());
                   qry.append(sbuf)
                      .append(" AND COLUMN_NUMBER=");
                   sprintf(sbuf,"%d",col->colnum);
                   qry.append(sbuf)
                      .append(" AND COLCOUNT=1")
                      .append(" FOR READ COMMITTED ACCESS");
                   HSCursor cursor;
                   retcode = cursor.prepareQuery(qry.data(),0,2);
                   HSHandleError(retcode);
                   retcode = cursor.open();
                   HSHandleError(retcode);
                   memset(highval,0,HS_MAX_BOUNDARY_LEN + 1);
                   memset(lowval,0,HS_MAX_BOUNDARY_LEN + 1);
                   retcode = cursor.fetch(2,(char *)&lowval[0],(char *)&highval[0]);
                   HSHandleError(retcode);

                   char numbuffer[2*HS_MAX_BOUNDARY_LEN];
                   ...
                   LocaleToUTF8(cnv_version1,
                                ...
                                &outputDataLen,
                                TRUE);
                   NAWString sTmpL(CharInfo::UTF8,numbuffer);
                   LocaleToUTF8(cnv_version1,
                                (const char *) &highval[1],
                                (short)highval[0],
                                numbuffer,
                                sizeof(numbuffer),
                                cnv_UTF16,
                                ...
                                &outputDataLen,
                                TRUE);
                   NAWString sTmpH(CharInfo::UTF8,numbuffer);
                   //remove '()'
                   sTmpL.remove(0,1);
                   sTmpL.remove(sTmpL.length() - 1);
                   sTmpH.remove(0,1);
                   sTmpH.remove(sTmpH.length() - 1);
                   boundLo = sTmpL.data();
                   boundHi = sTmpH.data();
                 }
               else
                 {
                   ...
                   if (DFS2REC::isBigNum(col->datatype))
                   {
                     myVarChar vc = myVarChar();
...
Lng32 HSGlobalsClass::GetStatistics(NAString& displayData, Space& space)
{
    ...
    if (objDef->getNATable() && objDef->getNATable()->isPartitionEntityTable())
      displayData += "Histogram data for Partition ";
    else
    ...
    for(Int32 twice=0;twice<2;twice++){
	                                      ...
	                                      /*=================================*/			
        {
            ...
            {
                if(tableGroup->sameGroup(listedGroup) && // order doesn't matter
...
Lng32 HSGlobalsClass::GetStatistics4PartitionTableV2(char* &outBuf, ULng32 &outBufLen, CollHeap *heap)
{
  Lng32 retcode = 0;

  NATable *pSourceTable = objDef->getNATable();
  if (!pSourceTable)
    return retcode;

  NAString sShowStats = "SHOWSTATS FOR TABLE %s ";
  if (onClauseText)
   sShowStats += *onClauseText;
  ...
  
  {
    for ( int i=0; i<pSourceTable->getNAPartitionArray().entries(); i++)
      {
        Int64 objUID = pSourceTable->getNAPartitionArray()[i]->getPartitionUID();
        char sbuf[25];
        sprintf(sbuf, PF64, objUID);
    
        NAString qry = " SELECT [FIRST 1] ROWCOUNT FROM ";
        qry.append(hstogram_table->data())
           .append(   " WHERE TABLE_UID = ")
           .append(sbuf)
           .append(   " ORDER BY COLUMN_NUMBER ASC");

        Int64 rowCount = 0;
        HSCursor histData(STMTHEAP, "GET_PARTITIONV2_ROWCOUNT");
        retcode = histData.prepareQuery(qry.data(), 0, 1);
        HSHandleError(retcode);

        if ((retcode=histData.open())<0)
          return retcode;

        retcode = histData.fetch(1,(char *)&rowCount);
  ...
  if (optFlags & DETAIL_OPT)
    s += "Detailed ";
  s += "Histogram data for Table ";
  char total[25];
  sprintf(total, PF64, totalRowCount);
  s += user_table->data();
  ...
  {
    Space space;
    space.allocateAndCopyToAlignedSpace(s, s.length(), sizeof(short));
    Lng32 bufLen = space.getAllocatedSpaceSize();
    char *buf = new (heap) char[bufLen];
    space.makeContiguous(buf, bufLen);
    outStr.append(buf,bufLen);
    outBufLen += bufLen;
  }
  ...
  for ( int i=0; i<pSourceTable->getNAPartitionArray().entries(); i++)
    {
      ...
      if (pName)
        {
          char query[sShowStats.length()+300];
          snprintf(query, sizeof(query), sShowStats.data() ,pName);
          //retcode = HSFuncExecQuery(query);
          char *result = NULL;
          ULng32 resultlen = 0;
          retcode = ShowStats(query, result, resultlen, heap);
          outStr.append(result,resultlen);
          outBufLen += resultlen;
          
          NADELETEBASIC(result,heap);
          HSExitIfError(retcode);
        }
    }
  outBuf = new(heap) char[outBufLen];
  str_cpy_all(outBuf,outStr.data(),outBufLen);
  return retcode;
}
              ...
              for (i=0; i<rowsRead; i++)
                {
                  if (nullInd && *nullInd == -1)
                    {
                      nulls++;
                      if (group->ISdatatype == REC_BYTE_V_ASCII && group->mcs_usingme > 0)
                        {
                          Int32 start = ((ISVarChar*)group->nextData - (ISVarChar*)group->data) + group->nullCount;
                          group->mcis_nullIndBitMap->setBit(start+i);
                        }
                      if (compacted)
...
void formatFixedNumeric(Int64 value, Lng32 scale, char* buffer)
{
  ...
  //If value= -9223372036854775808, -value will cause overflow here. So I commented code above.
  Int64 xval = value;
  Int32 numDigits = 0;
        ...
        case REC_BINARY_STRING:
          {
            *len = (short)MINOF(colDesc.length, maxCharBoundaryLen);
            ...
            if (!bEmptyStr)
              *len = (short)MAXOF(1, *len);//Boundary value can't be null
            }
          break;
        ...
        case REC_BYTE_F_DOUBLE:
          {
            *len = (short)MINOF(colDesc.length, maxCharBoundaryLen*2); // in bytes
            ...
            if (!bEmptyStr)
              *len = (short)MAXOF(2, *len);//Boundary value can't be null
          }
        break;
...
Lng32 setBufferValue(myVarChar& value,
                      ...
                      HSDataBuffer &boundary)
  {
    ...
    {
      const HSColumnStruct &colDesc = group->colSet[0];
      retcode = FormatRow(&colDesc, (char*)&value, boundary, true);
      HSHandleError(retcode);
    }
    return retcode;
  }
...
Lng32 managePersistentSamples()
{
  ...
  {
    NATable *pSourceTable = hs_globals->objDef->getNATable();
    if (pSourceTable->isPartitionV2Table())
      {
        NAString sampleOpt(" REMOVE SAMPLE");
        if (hs_globals->optFlags & CREATE_SAMPLE_OPT)
          {
            sampleOpt = " CREATE SAMPLE ";
            if (hs_globals->optFlags & SAMPLE_RAND_1)
              {
                char sbuf[50];
                snprintf(sbuf, sizeof(sbuf), " RANDOM %f PERCENT", (double)hs_globals->sampleValue1/HS_SAMP_PCNT_UPSCALE);
                sampleOpt += sbuf;
              } 
          }
        for ( int i=0; i<pSourceTable->getNAPartitionArray().entries(); i++)
          {
            ...
            if (pName)
              {
                 NAString sUps = "UPDATE STATISTICS FOR TABLE ";
                 sUps += pName;
                 sUps += sampleOpt;
                 if (LM->LogNeeded())
                   {
                 ...
                 retcode = HSFuncExecQuery(sUps.data());
                 if (LM->LogNeeded())
                   {
                     sprintf(LM->msg, "\n%s end. retcode=%d\n",sUps.data(),retcode);
                     LM->Log(LM->msg);
...
NAString HSColGroupStruct::generateTextForColumnCast()
{
  ...
  for (Int32 i=0; i<colCount; i++)
  {
    ...
    if (colSet[0].colnum == -1)  // if this is an expression histogram
      columnName = "\"X\"";  // use "X" as the column name
    else
      {
        columnName = ToAnsiIdentifier(col.colname->data());
...
NABoolean HSColGroupStruct::sameGroup(const HSColGroupStruct * other) const
{
  NABoolean rc = FALSE; // assume not
  if (other)
    {
      if (colSet == other->colSet)
        {
          if (colSet[0].colnum == -1)  // if expressions histograms
            // need to match column names too
            rc = (*(colNames) == *(other->colNames));
          else
            rc = TRUE;
        }
    }
...
// -----------------------------------------------------------------------
// Helper function for AddExpression. Validate that the
// expression references at least one column in the table and does 
// not reference any non-deterministic functions.
//
// TODO: part of this has been refactored out to function 
// parseAndBindExpression in hs_la.cpp. Should rewrite this to
// use that function.
// -----------------------------------------------------------------------
Lng32 HSGlobalsClass::validateExpression(const char * expressionText, HSColGroupStruct * group)
  {
    Lng32 retcode = 0;

    // Note: If we want to support multi-column expression histograms in 
    // the future, we will need to check expressionText for surrounding
    // parentheses, and remove one pair if found.

    Parser parser(CmpCommon::context());

    // put the expression text in a "SELECT <expression> FROM T" statement
    // for validation checks

    NAString queryString = "SELECT ";
    queryString += expressionText;
    queryString += " FROM ";
    queryString += objDef->getObjectFullName();

    // parse the expression and report any syntax issues
        ...
        if (diagsArea.contains(-UERR_SYNTAX_ERROR))
          {
            ...
            HSFuncMergeDiags(retcode, expressionText);
            HSHandleError(retcode);
          }
        else
          {
    ...
    // Drill down to the query expression part

    CMPASSERT(stmt->isAQueryStatement());
    RelExpr * queryExpr = stmt->getQueryExpression();

    // bind the expression and report any bind-time errors

    BindWA bindWA(ActiveSchemaDB(), CmpCommon::context(), FALSE, 
                  isNativeCat(*(objDef->getCatalog())));
    queryExpr = queryExpr->bindNode(&bindWA);
    
    if ((!queryExpr) || bindWA.errStatus())
      {
        retcode = -UERR_EXPRESSION_SEMANTICS;
        HSFuncMergeDiags(retcode, expressionText);
        HSHandleError(retcode);     
      }

    // check that there is only one expression (no multi-column histogram
    // support on expressions in this release), and that there are no
    // references to non-deterministic functions in the expression

    HSColumnStruct newColumn = HSColumnStruct();
    newColumn.colnum  = -1; // indicates expression histogram
    const NAType * expressionType = NULL;
    NAString unparsedExpr;
    short rc = bindWA.validateUpdateStatisticsExpression(queryExpr,
      &expressionType /* out */, unparsedExpr /* out */);
    if (rc)
      {
        retcode = -UERR_EXPRESSION_SEMANTICS;
        HSFuncMergeDiags(retcode, expressionText);
    ...
    newColumn.datatype = expressionType->getFSDatatype();
    newColumn.length = expressionType->getNominalSize();
    newColumn.precision = expressionType->getPrecision();
    newColumn.scale = expressionType->getScale();
    newColumn.nullflag = expressionType->supportsSQLnullLogical();
    newColumn.charset = expressionType->getCharSet();
    if (DFS2REC::isAnyCharacter(newColumn.datatype))
      {
        newColumn.colCollation =
          (static_cast<const CharType *>(expressionType))->getCollation();
        newColumn.caseInsensitive =
          (static_cast<const CharType *>(expressionType))->isCaseinsensitive();
      }

    *(newColumn.colname) = unparsedExpr.data();  // use unparsed expression text for column name
    *group->colNames = newColumn.colname->data();
   
    bool isOverSized = DFS2REC::isAnyCharacter(newColumn.datatype) &&
              (newColumn.length > maxCharColumnLengthInBytes);
    if (isOverSized)
      {
        hasOversizedColumns = TRUE;
      }
    group->colSet.insert((const HSColumnStruct) newColumn);
