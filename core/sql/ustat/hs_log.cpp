  ...
  if (hs_globals->objDef)
    {
      ...
      NAString sTableName = hs_globals->objDef->getObjectName();
      if (hs_globals->objDef->getNATable()->isPartitionV2Table())
        {
          if (hs_globals->partitionV2NameText)
            {
              sTableName = *hs_globals->partitionV2NameText;
            }
        }
      diagsArea << DgTableName(sTableName.data());
    }
  if (string0 != NULL)
    {
