...
NABoolean ColumnExistsInHistogram(const Lng32 colNumber)
  {
    HSGlobalsClass *hs_globals = GetHSContext();
    if ( !hs_globals || !hs_globals->objDef )
      return FALSE;
    //only in "if not exists"
    if ( NOT(hs_globals->optFlags & IF_NOT_EXISTS_OPT))
      return FALSE;

    Lng32 retcode;

    Int64 objID = hs_globals->objDef->getObjectUID();
    char sbuf[25];
    sprintf(sbuf, PF64, objID);

    HSCursor cursor(STMTHEAP,"ColumnExistsInHistogram");
    Lng32 colCnt;
    NAString qry = "SELECT COUNT(*) FROM ";
    qry.append(hs_globals->hstogram_table->data())
       .append(" WHERE TABLE_UID = ")
       .append(sbuf)
       .append(" and COLCOUNT=1 AND COLUMN_NUMBER=");
    sprintf(sbuf,"%d",colNumber);
    qry.append(sbuf).append(" FOR READ COMMITTED ACCESS");

    retcode = cursor.prepareQuery(qry.data(), 0, 1);
    if ( 0 != retcode )
      return FALSE;
    ...
    if ( 0 != retcode )
      return FALSE;
    retcode = cursor.fetch(1,(void *)&colCnt);
    if ( 0 == retcode && colCnt > 0 )
      return TRUE;
...
Lng32 HSFuncParseStmt()
                ...
                if (NOT ColumnExists(col.colnum))
                  {
                    ...
                    HSHandleError(retcode);
                    if (ColumnExistsInHistogram(col.colnum))
                      {//in "if not exists" syntax
                       //if singlecolum already has histogram,dont create histogram again
                       //add single colum just to generate multi colum
                        HSColGroupStruct *pSingleCol = hs_globals->singleGroup;
                        while (pSingleCol)
                          {
                            if (pSingleCol->colSet[0].colnum == col.colnum)
                              {
                                pSingleCol->state = DONT_PROCESS;
                                break;
                              }
                            pSingleCol = pSingleCol->next;
                          }
                      }
                  }
              }
 ...
 static NABoolean IsReservedSchema4Ustats(const NAString &catName,
                                          const NAString &schName,
                                          const NAString &objName)
   {
     if ((schName != SEABASE_REPOS_SCHEMA)
         && ComIsTrafodionReservedSchema("",catName,schName))
       return TRUE;
     if ( "SB_HISTOGRAMS" == objName ||
          "SB_HISTOGRAM_INTERVALS" == objName ||
          "SB_PERSISTENT_SAMPLES" == objName )
       return TRUE;
...
Lng32 AddTableName( const hs_table_type type
                 ...
                 )
  {
    ...
    //mantis-5585
    if (IsReservedSchema4Ustats(catName,schName,objName))
      {
    ...
    //mantis-5585
    HSTableDef **pDefTmp = &hs_globals->objDef;
    if(hs_globals->isGenerateStatsSourceTable)
        pDefTmp = &hs_globals->objGenStatsSource;
    *pDefTmp = NULL;
    ...
    if ((CmpCommon::context()->sqlSession()->volatileSchemaInUse()) &&
        ...
        (! catalog))
      {
        ...
        (*pDefTmp) = HSTableDef::create(STMTHEAP,
                                                volIntName,
                                                hs_globals->tableType,
                                                hs_globals->nameSpace);

        if (NOT (*pDefTmp)->objExists(hs_globals->isUpdatestatsStmt))
          {
	...
	if (NOT intName.isValid())
	  {
       ...
       NABoolean bCreateExternalTable = hs_globals->isUpdatestatsStmt;
       ...
       NABoolean objExists = (*pDefTmp)->objExists(bCreateExternalTable);
    ...
    if ((((*pDefTmp)->getObjectType() != COM_BASE_TABLE_OBJECT) &&
         ((*pDefTmp)->getObjectType() != COM_MV_OBJECT)) ||
        ((*pDefTmp)->getNATable()->isSeabaseMDTable()) ||
        ((*pDefTmp)->getNATable()->isSeabasePrivSchemaTable())) 
      {
        HSFuncMergeDiags(-UERR_INVALID_OBJECT, extName);
        retcode = -1;
        HSHandleError(retcode);
      }
    retcode = (*pDefTmp)->getColumnNames();
    ...
    hs_globals->tableFormat = (*pDefTmp)->getObjectFormat();
    *hs_globals->catSch = (*pDefTmp)->getPrimaryLoc(HSTableDef::EXTERNAL_FORMAT);
    *hs_globals->user_table = (*pDefTmp)->getObjectFullName();
    hs_globals->tableFormat = (*pDefTmp)->getObjectFormat();
    ...
    if (hs_globals->tableFormat == SQLMX)
      {
        ...
        NAString catName((*pDefTmp)->getCatName());
      ...
      {
        *hs_globals->hstogram_table = (*pDefTmp)->getCatalogLoc(HSTableDef::EXTERNAL_FORMAT);
        hs_globals->hstogram_table->append(".HISTOGRM");
        
        *hs_globals->hsintval_table = (*pDefTmp)->getCatalogLoc(HSTableDef::EXTERNAL_FORMAT);
...
//for partition table V2
//If the source table is a partition V2 table, do some check...
Lng32 HSCheckPartitionV2Table(const char* input)
{
  HSGlobalsClass *hs_globals = GetHSContext();
  if (!hs_globals->isPartitionV2Table)
    return 0;

  //UPDATE STATISTICS ..LIKE is not supported for partition v2 table.
  if ( hs_globals->optFlags & GEN_STATS_LIKE)
    {
      hs_globals->diagsArea << DgSqlCode(-UERR_PAR_TABLE_NOT_SUPPORT1);
      return -1;
    }

  //UPDATE STATISTICS ..if not exists is not supported for partition v2 table.
  if ( hs_globals->optFlags & IF_NOT_EXISTS_OPT)
    {
      hs_globals->diagsArea << DgSqlCode(-UERR_PAR_TABLE_NOT_SUPPORT2);
      return -1;
    }

  //IUS is not supported for partition v2 table.
  if ( hs_globals->optFlags & IUS_PERSIST || hs_globals->optFlags & IUS_OPT)
    {
      hs_globals->diagsArea << DgSqlCode(-UERR_PAR_TABLE_NOT_SUPPORT3);
      return -1;
    }
...
Lng32 RemoveExistingColumns()
  {
    HSGlobalsClass *hs_globals = GetHSContext();
    if ( !hs_globals )
      return -1;

    if (!hs_globals->singleGroup && !hs_globals->multiGroup)
      return -1;

    if ( 0 == hs_globals->groupCount )
      return 0;

    if ( !hs_globals->objDef )
      return -1;

    if ( !hs_globals->hstogram_table )
      return -1;

    Lng32 retcode = 0;
    // if the table is a native hbase table,hive table or table under seabase schema,
    // need to check if the table SB_HISTOGRAMS exist
    NAString schemaName;
    if (strcmp(hs_globals->hstogram_table->data(), "TRAFODION.\"_HBASESTATS_\".SB_HISTOGRAMS") == 0)
      schemaName = "_HBASESTATS_";
    else if (strcmp(hs_globals->hstogram_table->data(), "TRAFODION.\"_HIVESTATS_\".SB_HISTOGRAMS") == 0)
      schemaName = "_HIVESTATS_";
    else if (strcmp(hs_globals->hstogram_table->data(), "TRAFODION.SEABASE.SB_HISTOGRAMS") == 0)
      schemaName = "SEABASE";
    if (!schemaName.isNull())
      {
        ...
        HSCursor cursor(STMTHEAP,"readHistogramTableExists");
    ...
    //single column
    if (hs_globals->singleGroup)
    {
      Lng32 colNum;
      NAString qry = "SELECT COLUMN_NUMBER FROM ";
      qry.append(hs_globals->hstogram_table->data())
         .append(" WHERE TABLE_UID = ")
         .append(sbuf)
         .append(" and COLCOUNT=1 ORDER BY HISTOGRAM_ID")
         .append(" FOR READ COMMITTED ACCESS");

      HSCursor cursor(STMTHEAP,"readSingleColHist");
      retcode = cursor.prepareQuery(qry.data(), 0, 1);
      HSHandleError(retcode);
      ...
      while ( 0 == retcode )
        {
          retcode = cursor.fetch(1,(void *)&colNum);
          if ( 0 == retcode )
            {
              HSColGroupStruct *pGroup = hs_globals->singleGroup;
              while (pGroup)
                {
                  if ( pGroup->colSet[0].colnum == colNum )
                    {
                      hs_globals->removeGroup(pGroup);
                      break;
                    }
                  else
                    pGroup = pGroup->next;
    ...
    HSHandleError(retcode);
    //mulit column
    if (hs_globals->multiGroup)
    {
      NAString qry = "SELECT COUNT(*) FROM (SELECT HISTOGRAM_ID FROM ";
      qry.append(hs_globals->hstogram_table->data())
         .append(" WHERE COLUMN_NUMBER IN (#1) AND")
         .append(" COLCOUNT=#2 AND TABLE_UID=#3)")
         .append(" S GROUP BY S.HISTOGRAM_ID")
         .append(" HAVING COUNT(*)=#2");

      std::vector<HSColGroupStruct *> vDel;
      HSColGroupStruct *pGroup = hs_globals->multiGroup;
      while (pGroup)
        {
          NAString sColNum;
          for ( Int32 i = 0; i < pGroup->colCount; ++i )
            {
              HSColumnStruct aCs = pGroup->colSet[i];
              char sTmp[10];
              sprintf(sTmp, "%d", aCs.colnum);
              if ( 0 != i )
                sColNum.append(",");
              sColNum.append(sTmp);
            }
          NAString s = qry;
          size_t nIdx = s.index("#1");
          s.replace(nIdx,2,sColNum);
          char sTmp[10];
          sprintf(sTmp, "%d", pGroup->colCount);
          nIdx = s.index("#2");
          s.replace(nIdx,2,sTmp);
          nIdx = s.index("#2");
          s.replace(nIdx,2,sTmp);
          nIdx = s.index("#3");
          s.replace(nIdx,2,sbuf);

          HSCursor cursor(STMTHEAP,"readMultiColHist");
          retcode = cursor.prepareQuery(s.data(), 0, 1);
          HSHandleError(retcode);
          retcode = cursor.open();
          HSHandleError(retcode);
          ULng32 cnt;
      ...
      if (vDel.size() > 0)
        {
          for (Int32 i = 0; i< vDel.size(); ++i)
            {
              hs_globals->removeGroup(vDel[i]);
            }
        }
    }
    return retcode;
...
// -----------------------------------------------------------------------
// Add the single-column group for an expression. Validate that the
// expression references at least one column in the table and does 
// not reference any non-deterministic functions.
// -----------------------------------------------------------------------
Lng32 AddExpression(const char * expressionText)
  {
    HSGlobalsClass *hs_globals = GetHSContext();
    HSColGroupStruct *newGroup = new(STMTHEAP) HSColGroupStruct;
    Lng32 retcode = 0;

    newGroup->colCount = 1; // single expression; can generalize for multi- later  

    // Introduce new scope for instance of HSTranController; its destructor
    // terminates the transaction. We need a transaction for this because
    // validateExpression() prepares a query. An
    // implicit transaction started by the query would remain open and retain
    // the locks.
    // TODO: See if this is true... or does compiler terminate its metadata read transaction?
    { 
      HSTranController TC("Validate expression", &retcode);
      HSHandleError(retcode);
      retcode = hs_globals->validateExpression(expressionText, newGroup);
      HSHandleError(retcode);
    }

    // check to see if this expression is a duplicate; if so silently discard
    // but log it

    HSColGroupStruct *group = hs_globals->singleExpressionGroup;
    bool found = false;
    const char * newGroupName = newGroup->colSet[0].colname->data();
    while ((group) && (!found))
      {
        const char * groupName = group->colSet[0].colname->data();
        if (strcmp(newGroupName,groupName) == 0)
          found = true;
        else
          group = group->next;
      }
    ...
    if (found)
      {
        ...
        if (LM->LogNeeded())
          {
            sprintf(LM->msg, "Duplicate expression ignored: %s\n",
                    expressionText);
            LM->Log(LM->msg);
          }       
      }
      ...
      {
        // Add the new group to the single expression group chain

        newGroup->next = hs_globals->singleExpressionGroup;
        hs_globals->singleExpressionGroup = newGroup;
        hs_globals->singleExpressionGroupCount++;
      }
