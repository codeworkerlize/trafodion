...
class HSDataBuffer
  {
    public:
     ...
     ~HSDataBuffer() 
      ...
      // For some uses, the data in this buffer is UTF8, and we use TRANSLATE to
      // convert it to UCS2 in the SB_HISTOGRAMS table
      HSDataBuffer& copyFromUTF8(const char *newVal, short newLen)
        {
          if (newLen > len)
          {
            NADELETEBASIC (val, STMTHEAP);
...
public:
  ...
  inline Lng32 getRow()
  {
    ...
    retcode_ = SQL_EXEC_Fetch(stmt_,outputDesc_, 0, 2,
