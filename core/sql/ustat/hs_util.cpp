...
#include "seabed/ms.h"  // to get msg_mon_get_my_process_name
...
NABoolean isHBaseMeta(const ComObjectName& objectName)
{
    ...
    ( CmpSeabaseDDL::isSeabaseMD (objectName) || 
      ...
      CmpSeabaseDDL::isREPOSSchema(objectName) ||
      CmpSeabaseDDL::isXDCSchema(objectName)
    );
}
               ...
               bool hasConvertedToUCS2)
{
    ...
    if (DFS2REC::isAnyCharacter(type))
      {
         wStr = WIDE_("'");
         if ( CharInfo::UTF8 == srcDesc->charset && (!hasConvertedToUCS2) )
           {//process UTF8
             const char *c = src + sizeof(short);
             NAString sUTF8;
             for (short i = 0; i < inDataLen;)
               {
                 if ( c[i] == '\0' )
                   {
                     sUTF8 += '\1';
                   ...
                   {
                     sUTF8.append(c[i]);//wchar to char
                     if (c[i]=='\'')
                       {
                         sUTF8.append("'");
                       }
                   }
                 i = i+2;//skip empty character
               }

             //for mantis-15078, the length of sUTF8 is wrong, so we fix it
             NAWString *wUTF8 = charToUnicode(srcDesc->charset,sUTF8.data(), sUTF8.length(),STMTHEAP);
             if ( wUTF8 )
               wStr.append(*wUTF8);
             delete wUTF8;
           }
...
Int64 getDefaultSampleRowSize(Int64 tblRowCount)
{
   Int64 sampleRows;
   if (CmpCommon::getDefault(USTAT_USE_SLIDING_SAMPLE_RATIO) == DF_ON)
     sampleRows = getDefaultSlidingSampleSize(tblRowCount);
   else
     sampleRows = getDefaultSampleSize(tblRowCount);
   return sampleRows;
}
...
HSLongHeldLockController::HSLongHeldLockController(const char * tableName, 
  const char * commandText, NAHeap * heap) 
: LongHeldLockController((NAString("ustat_")+NAString(tableName)).data(), 
  commandText, true, heap)
{
}
