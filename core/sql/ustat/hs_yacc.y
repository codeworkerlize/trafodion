...
%token  SYSTEM EXPRESSION
%token  LIKE TOK_IF TOK_NOT TOK_EXISTS
%token  TOK_PARTITION
%%
...
table_identifier : identifier optional_partition_table_v2
                   {
            ...
            |  identifier '.' identifier optional_partition_table_v2
                   {
            ...
            |  identifier '.' identifier '.' identifier optional_partition_table_v2
                   {
...
optional_partition_table_v2 : TOK_PARTITION '(' identifier ')'
                   {
                     hs_globals_y->isPartitionV2Table = TRUE;
                     hs_globals_y->partitionV2NameText = new(STMTHEAP) NAString(STMTHEAP);
                     *hs_globals_y->partitionV2NameText = $3->data();
                   }
                   |TOK_PARTITION FOR '(' '$' ')'
                   |/* empty */
;
...
histogram_options : CLEAR
                 ...
                 |  LIKE { hs_globals_y->isGenerateStatsSourceTable = TRUE; } table_identifier
                    {
                      hs_globals_y->optFlags |= GEN_STATS_LIKE;
                      hs_globals_y->isGenerateStatsSourceTable = FALSE;
                    }
                 | /* empty */
;


on_clause :         ON predefined_groups optional_us_if_not_exists
                 |  ON predefined_groups ',' regular_group_list optional_us_if_not_exists
                 |  ON regular_group_list optional_us_if_not_exists
;
...
optional_us_if_not_exists :       TOK_IF TOK_NOT TOK_EXISTS
                                  {
                                    Lng32 retcode = 0;
                                    if ( hs_globals_y->optFlags & EXISTING_OPT )
                                      HSFuncMergeDiags(-USERR_CONFLICT_ERROR);
                                    hs_globals_y->optFlags |= IF_NOT_EXISTS_OPT;
                                    if ( retcode = RemoveExistingColumns())
                                      HSHandleError(retcode);
                                  }
                                  | /* empty */
;
...
regular_group :     '(' multi_column_list ')'
                 ...
                 |  EXPRESSION '(' '$' INTEGER_NUMBER ')'
                      {
                        NAArray<NAString *> * temp = hs_globals_y->expressionText;
                        NAString * exprText = (*temp)[$4];
                        Lng32 retcode = AddExpression(exprText->data());
                        if (retcode)
                          HSHandleError(retcode);
                      }
...
multi_column_list : column_name ',' column_name
                      {
                        $$ = new (STMTHEAP) HSColSet (STMTHEAP);
                        Lng32 colnum1 = hs_globals_y->objDef->getColNum($1->colname->data());
                        Lng32 colnum2 = hs_globals_y->objDef->getColNum($3->colname->data());
                        if (colnum1 < 0 || colnum2 < 0)
                          {
                            Lng32 retcode = -1;
                            HSHandleError(retcode);
                          }
                        *$1 = hs_globals_y->objDef->getColInfo(colnum1);
                        $1->colnum = colnum1;
                        $$->insert(*$1);
                        *$3 = hs_globals_y->objDef->getColInfo(colnum2);
                        $3->colnum = colnum2;
                        $$->insert(*$3);
                      }
