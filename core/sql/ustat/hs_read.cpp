                            ...
                            NABoolean &updateReadTime)
{
  ...
  {
    retcode_ = SQL_EXEC_Fetch(cursor102_->getStmt(), cursor102_->getOutDesc(), 4, 2,
                                (void *)(&statTime),   NULL, 
                                (void *)(&colNumber),  NULL,
                                (void *)(&readTime),   NULL,
                                (void *)(&readCount),  NULL);
...
Lng32 HSStatsTimeCursor::update(const NAColumnArray &colArray)
{
  ...
  while(TRUE) // Loop until break statement below.
  {
    retcode_ = SQL_EXEC_Fetch(cursor106_->getStmt(), cursor106_->getOutDesc(), 5, 2,
                              (void *)(&statTime),   NULL, 
                              (void *)(&colNumber),  NULL,
...
private:
  ...
  Int64 v1_; //,v3_,v4_;
  // expressionText_ comes from the V5 column
  char *expressionText_;
  //wchar_t *v6_;
  NABoolean updatable_;
...
public:
  ...
  void addExpressionHistogram( const ULng32 histid
                             , const Lng32 colCount
                    ...
                    )
{
          ...
          if (fileType == SQLMX)
            {
             ...
             NABoolean isMappedTableName =
                (objectName->getCatalogNamePart().getInternalName() == 
                      TRAFODION_SYSTEM_CATALOG)
                         &&
                (objectName->getSchemaNamePart().getInternalName() == 
                      HBASE_EXT_MAP_SCHEMA);

             if (isMappedTableName) 
                     // A special case of _CELL_ and _ROW_  where the 
                     // catalog name is TRAFODION (not HBASE) and
                     // the schema name is "_HB_MAP_" (not "_MAP_").
             {
                histLoc = TRAFODION_SYSTEM_CATALOG "." HBASE_STATS_SCHEMA;
  ...
  retcode = readHistograms(tabDef, fullQualName, histogramTableName, histintsTableName, 
                           specialTable, type, colArray, interestingExpressions,
                           statsTime, allFakeStat, preFetch, 
  ...
  bool generateHistogramForOrcOrParquet = isOrcOrParquet && 
  ...
  if (generateHistogramForOrcOrParquet)
    {
      ...
      Lng32 oldHiveMaxStringLength =
       (ActiveSchemaDB()->getDefaults()).getAsLong(HIVE_MAX_STRING_LENGTH_IN_BYTES); 
      HSFuncExecQuery("CONTROL QUERY DEFAULT HIVE_MAX_STRING_LENGTH_IN_BYTES '120'");
      ...
      snprintf(resetCQD, sizeof(resetCQD), 
               "CQD HIVE_MAX_STRING_LENGTH_IN_BYTES '%d'", 
          ...
          if (preFetch || colArray[k]->needHistogram())
            {
              ...
              if (generateHistogramForOrcOrParquet) // if we want to generate an ORC histogram
                {
...
//                           set to TRUE.
//            interestingExpressions - a dictionary of expressions for which
//                           expressions histograms are requested
...
HSHistogrmCursor::HSHistogrmCursor
 ...
 : heap_(heap),
   ...
   storedStatsDesc_(storedStatsDesc),
...
//#endif
   ...
   expressionText_(new(heap) char[4*V5_MAX_LEN+1+2]),
   maxHistid_(0)

{
  *readTime_timestamp0_ = '\0';
  *((short *)expressionText_) = 0;  // varchar length field init to zero
  expressionText_[sizeof(short)] = '\0';
  validCursor_ = TRUE;
}
  ...
  do
    {
      ...
      if (tableColNum_ >= 0)
        {
          ...
          nacol = cs.colArray()[tableColNum_];
      ...
      else // must be an expression histogram
        {
          // No NAColumn for expression histograms, instead we have to look up 
          // the expression in the NAHashDictionary. Note: Today there is no need
          // for compressed histograms on expressions.
          NAString expressionLookupKey(expressionText_+sizeof(short));
          if (preFetch || interestingExpressions.contains(&expressionLookupKey))
            {
              needHistogram = TRUE;
              needHistints = TRUE;
            }
      ...
      if (updatable_ && nacol && nacol->needFullHistogram())
        {
      ...
      try
        {
          ...
          if (needHistogram)
          {
            if (tableColNum_ >= 0)  // vanilla histogram
            ...
            else // expression histogram
              cs.addExpressionHistogram( histid_
                           , colCount_
                           , expressionText_ + sizeof(short)
                           , intCount_
                           , totalRowCount_
                           , totalUec_ // might change by this call to be == totalRowCount_
                           , avgVarCharCount_
...
// Fetched values and datatypes must remain in sync with method
// TrafTableStatsDesc::fetchHistogramCursor in sqlcat/TrafDDLdesc.cpp.
// -----------------------------------------------------------------------
Lng32 HSHistogrmCursor::get()
{
  ...
  if (storedStatsDesc_)
    {
      retcode_ = storedStatsDesc_->tableStatsDesc()->fetchHistogramCursor
        (
             (void *)&histid_,
             (void *)&tableColNum_,
             (void *)&colCount_,
...
//#endif
  ...
  // On the other hand, expressionText_ is retrieved as translated to UTF8.
  // Add a null terminator to the end of the text.
  size_t expressionLength = *(short *)expressionText_;
  char * expressionTextBody = expressionText_+sizeof(short);
  expressionTextBody[expressionLength] = '\0';
                                ...
                                NAString &histintsTableName)
{
  ...
  NAString ansiCatName = ToAnsiIdentifier(catName);
  NAString ansiSchName = ToAnsiIdentifier(schName);

  if ( HSGlobalsClass::isNativeHbaseCat(ansiCatName)) {
    name = HBASE_STATS_CATALOG "." HBASE_STATS_SCHEMA;
  } else {  
    name = ansiCatName + "." + ansiSchName + ".";
  }

  histogramTableName = name + HBASE_HIST_NAME;
  histintsTableName = name + HBASE_HISTINT_NAME;

  return 0;
}
                             
// queries defined here must remain in sync with method
// CmpSeabaseDDL::generateStoredStats in sqlcomp/CmpSeabaseDDLtable.cpp.       
                             ...
                             NAString *histintsQuery)
{
  if (histogrmQuery)
    {
      NABoolean needMCStats = 
        (CmpCommon::getDefault(HIST_MC_STATS_NEEDED) == DF_ON);
      
      char sbuf[100];
      strcpy(sbuf, (! needMCStats ? " and colcount = 1 " : " "));
      
      char histogramQueryBuf[2000];
      str_sprintf(histogramQueryBuf, "select histogram_id, column_number, colcount, interval_count, rowcount, total_uec, juliantimestamp(stats_time), low_value, high_value, juliantimestamp(read_time), read_count, sample_Secs, col_secs, sample_percent, cv, reason, v1, v2, TRANSLATE(V5 USING UCS2TOUTF8) from %s where table_uid = %ld and reason != ' ' %s order by table_uid, histogram_id, col_position",
                  histogrmTableName, 
                  tableUID,
                  sbuf);
      
      *histogrmQuery = histogramQueryBuf;
    }
  
  if (histintsQuery)
    {
      char histintsQueryBuf[1000];
      
      str_sprintf(histintsQueryBuf, "select histogram_id, interval_number, interval_rowcount, interval_uec, interval_boundary, cast(std_dev_of_freq as double precision), v1, v2, v5 from %s where table_uid = %ld order by table_uid, histogram_id, interval_number",
                  histintsTableName,
                  tableUID);

      *histintsQuery = histintsQueryBuf;
    }
...
{
  NAString qry;
  HSCursorGetHistQueries(histogramTableName_, 
                         NULL,
                         tableUID_.get_value(),
                         &qry, NULL);
  ...
  if (storedStatsDesc_)
    {
      storedStatsDesc_->tableStatsDesc()->initHistogramCursor();
    }
  else
    {
      retcode = cursor101_->prepareQuery(qry.data(), 0, 19);
      HSHandleError(retcode);
...
Lng32 HSHistintsCursor::open()
{
 ...
 if (cursor201_)
  ...
  if (storedStatsDesc_)
    {
      storedStatsDesc_->tableStatsDesc()->initHistintCursor();
    }
  else
    {
      retcode = cursor201_->prepareQuery(qry.data(), 0, 9);
      HSHandleError(retcode);
...
//
// Fetched values and datatypes must remain in sync with method
// TrafTableStatsDesc::fetchHistintCursor in sqlcat/TrafDDLdesc.cpp.
// -----------------------------------------------------------------------
  ...
  while (histid_ <= histid)     // assumes rows are sorted! [PK]
    {
      ...
      if (storedStatsDesc_)
        {
          retcode_ = storedStatsDesc_->tableStatsDesc()->fetchHistintCursor
            (
                 (void *)&histid_,
                 (void *)&short1_,
                 (void *)&tempIntRowCount,
                 (void *)&tempIntUec,
                 (void *)buf_,
                 (void *)&stdDevOfFreq_,
                 (void *)&mfvRowCnt,
               ...
               )
{
  ...
  SQL_EXEC_ClearDiagnostics(desc);
  retcode = SQL_EXEC_Exec(desc, NULL, 2, 2, param1Addr, NULL,
                                         param2Addr, NULL);
  if (retcode)
...
// -----------------------------------------------------------------------
// Initialize ColStats for an expression histogram.
// -----------------------------------------------------------------------
void HSColStats::addExpressionHistogram
                 ( const ULng32 histid
                 , const Lng32 colCount
                 , const char * expressionText
                 , const Lng32 intCount
                 ...
                 )
{
  ...
  // Determining whether the expression is nullable, and determining its
  // type for encoding lowval and highval, requires us to have the NAType
  // of the expression. To get that, we parse and bind it here.

  const NAType * expressionType = NULL;
  NAString unparsedExpr;  // not used here
  Lng32 retcode = parseAndBindExpression(expressionText, 
                             objectName,
                             &expressionType /* out */,
                             unparsedExpr /* out */);
  // Why might errors occur here? Some possibilities: 1. The text is no longer
  // valid (because a column was changed or dropped, or because someone edited the 
  // text). 2. Metadata read errors. 3. Bugs in the metadata read code (unlikely
  // outside of development).
  if (retcode < 0)  // TODO: test the behavior of going through this path
    return;

  lastInt_ = intCount;
  if (expressionType->supportsSQLnull())
    lastInt_ = intCount - 1;

  // The ColStats class uses an NAColumnArray to get information about
  // columns and to get NAType information. For an expression, the column
  // is not a real column in the table, so we fake it here.

  NAColumn * expressionStandIn = CMPNEW NAColumn(expressionText /* use expr text for column name */,
                                                 -1 /* not a real column */,
                                                 expressionType->newCopy(heap_),
                                                 heap_);
  // TODO: consider whether to create a new value for ColumnClass enum for
  // histogram expressions, and if so, pass that in this constructor
      
  colStats_->statColumns().insert(expressionStandIn);
  colStats_->setExpressionsHistogram();
  ...
  // Set flags indicating whether fake or small sample histograms.
  colStats_->setFakeHistogram(FALSE);
  colStats_->setSmallSampleHistogram(FALSE);
...
void getPreviousUECRatios(HSColGroupStruct *groupList)
{
  ...
  // Use a linked list for expression histograms. (Note: I tried using an
  // NAHashDictionary for this purpose but templates on local types are not
  // supported until C++11, and it wasn't worth the effort to move the
  // PrevUecInfo class into a header at this time.) In the current use case
  // we only expect small numbers of these so a linked list is fine here.
  PrevUecInfo * expressionUecInfo = NULL;
  ...
  while (!retcode)
    {
      retcode = cursor.get();
      // single-col groups only
      if (!retcode && cursor.colCount_ == 1)  
        {
          if (cursor.tableColNum_ >= 0)
            {
              // normal columns
          ...
          else
            {
              ...
              PrevUecInfo * prevUecInfo = new (STMTHEAP) PrevUecInfo;
              prevUecInfo->expressionText = cursor.expressionText_ + sizeof(short); // +2 to skip varchar len field
              prevUecInfo->totalRows = (Int64)cursor.totalRowCount_;
              prevUecInfo->totalUec = (Int64)cursor.totalUec_;
              // the avgVarCharCount_ (V2 column in SB_HISTOGRAMS) is 100 times the
              // average varchar length
              if (cursor.avgVarCharCount_ > 0)  // if the column is set
                prevUecInfo->oldAvgVarcharSize = 
                  (Int64)(cursor.avgVarCharCount_+99)/100;

              if (expressionUecInfo)
                expressionUecInfo->next = prevUecInfo;
              else    
                expressionUecInfo = prevUecInfo;        
            }
        }
    }
  ...
  while (group)
    {
      ...
      if (colNum >= 0) // if this is not an expression histogram
        {
        ...
        {
          PrevUecInfo * nextUecInfo = expressionUecInfo;
          while (nextUecInfo)
            {
              if (*(group->colNames) == nextUecInfo->expressionText)
                {
                  group->prevRowCount = nextUecInfo->totalRows;
                  group->prevUEC = nextUecInfo->totalUec;
                  group->oldAvgVarCharSize = nextUecInfo->oldAvgVarcharSize;
                  nextUecInfo = NULL;
                }
              else
                nextUecInfo = nextUecInfo->next;
            }        
        }
      if (LM->LogNeeded())
        {
