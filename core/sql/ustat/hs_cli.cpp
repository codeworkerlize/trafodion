      ...
      if (retcode >= 0) // ignore warnings
        {
          ...
          if (checkForMdam)
            checkMdam(stmt);
          retcode = SQL_EXEC_ExecFetch(stmt,0,0,0);
        }
    }
  else
    {
      retcode = SQL_EXEC_ExecDirect(stmt, srcDesc, 0, 0, 0);
    }
...
Lng32 HSSample::create(NAString& tblName, NABoolean unpartitioned, NABoolean isPersSample)
  {
    ...
    tableOptions += " WITHOUT NAMESPACE ";
    tableOptions += " WITHOUT REGION REPLICATION ";
    tableOptions += " WITHOUT INDEXES ";
    tableOptions += " WITHOUT CONSTRAINTS ";
    if (!isPersSample)
      tableOptions += " WITHOUT INCREMENTAL BACKUP ";
    HSTranMan *TM = HSTranMan::Instance();
    ...
    //remove replication attribute
    if (objDef && objDef->getNATable())
      {
        if (COM_REPL_NONE != objDef->getNATable()->xnRepl())
          {
            NAString q = "ALTER TABLE ";
            q += tempTabName;
            q += " ATTRIBUTE NO REPLICATION";
            retcode = HSFuncExecDDL(q,
                                    -UERR_GENERIC_ERROR,
                                    NULL,
                                    tempTabName, objDef);
            HSHandleError(retcode);
    ...
    retcode_(0)
{
  ...
  stmt_.handle = 0;
  retcode_ = SQL_EXEC_SetDescPointers(pInputDesc_, PRESET_VARS_NUM, (numVars_ - PRESET_VARS_NUM + 1), 2,
...
Lng32 HSCliStatement::open()
  {
    ...
    HSHandleError(retcode);
    retcode = SQL_EXEC_Exec(&stmt_, NULL, numVars_, 2,      /* OPEN CURSOR */
...
Lng32 HSCliStatement::fetch(Lng32 numParam,
                          ...
                          )
  {
    ...
    retcode = SQL_EXEC_Fetch(&stmt_, NULL, numParam, 2,
...
Lng32 HSCliStatement::execFetch(const char *dml, NABoolean hideError)
{
  ...
  retcode_ = SQL_EXEC_ExecFetch(&stmt_, pInputDesc_, numVars_, 2,
                                in01_, NULL, in02_, NULL, in03_, NULL,
                                in04_, NULL, in05_, NULL, in06_, NULL,
                                in07_, NULL, in08_, NULL, in09_, NULL,
                                in10_, NULL, in11_, NULL, in12_, NULL,
...
Lng32 HSCursor::fetchRowset()
  {
    ...
    retcode_ = SQL_EXEC_Fetch(stmt_,outputDesc_, 0, 0);
    HSHandleError(retcode_);
...
void HSinsertHist::setText()
  {
    ...
    // the V5 column needs to be translated
    stmtText_.append("TRANSLATE(?[").append(rowsetSizeStr).append("] USING UTF8TOUCS2), ");
...
Lng32 HSinsertHist::flush()
  {
    ...
    if (numRows_ > 0 && retcode_ == 0)
      {
        retcode_ = SQL_EXEC_ExecFetch(&stmt_,&desc_,0, 0);
        if (LM->LogNeeded())
          {
...
Lng32 HSinsertHistint::flush()
  {
    ...
    if (numRows_ > 0 && retcode_ == 0)
      {
        retcode_ = SQL_EXEC_ExecFetch(&stmt_,&desc_,0, 0, 0);
        if (LM->LogNeeded())
          {
                                             ...
                                             NABoolean isPersSample)
  {
    ...
    if (objDef->getObjectFormat() == SQLMP
     ...
     if ( HSGlobalsClass::isNativeCat(objDef->getCatName()) )
     {
           ...
           ComUInt32 numConfiguredESPs = defs.getTotalNumOfESPs(fakeEnv);
...
Lng32 printPlan(SQLSTMT_ID *stmt)
  {
    ...
    while (retcode == 0)
      {
        retcode = SQL_EXEC_Fetch(ppStmtId, ppOutputDesc, 9, 2
                                 ,(void*)&row.lc[0], NULL
                                 ,(void*)&row.rc[0], NULL
...
Lng32 checkMdam(SQLSTMT_ID *stmt)
{
  ...
  Int32 rowCount = 0;
  retcode = SQL_EXEC_Fetch(stmtId, outputDesc, 1, 2, &rowCount, NULL);
  if (retcode == 0)
    {
      if (rowCount > 0)
...
Lng32 HSCursor::open(Lng32 numParams, void * in01)
{
  ...
  HSHandleError(retcode);
  retcode = SQL_EXEC_Exec(stmt_, inputDesc_, numParams, 2, in01, NULL);
  HSHandleError(retcode);
  closeStmtNeeded_ = TRUE;
  lastFetchReturned100_ = FALSE;
  return retcode;
                      ...
                      void *  out25)
{
  ...
  Lng32 retcode = SQL_EXEC_Fetch(stmt_, outputDesc_, numResults, 2,
                                 out01, NULL, out02, NULL, out03, NULL,
                                 out04, NULL, out05, NULL, out06, NULL,
                                 out07, NULL, out08, NULL, out09, NULL,
                                 out10, NULL, out11, NULL, out12, NULL,
...
Lng32 HSCursor::fetchV(Lng32 numResults, Int64 * int64result, void * wcharResults[])
{
  ...
  retcode = SQL_EXEC_Fetch(stmt_, outputDesc_, 0, 0); 
  HSHandleError(retcode); 
