...
void ComCondition::unpackObj(IpcMessageObjType objType, IpcMessageObjVersion objVersion, NABoolean sameEndianness,
  ...
  if (usageMap_ & USED_FLAGS) unpackBuffer(buffer, flagsTBS_);
  if (usageMap_ & USED_SQLCODE)
    unpackBuffer(buffer, theSQLCODE_);
  else
    assert(0);  // should never have a missing SQLCODE field
  if (usageMap_ & USED_SERVER_NAME) unpackBuffer(buffer, serverName_, collHeapPtr_);
  if (usageMap_ & USED_CONNECTION_NAME) unpackBuffer(buffer, connectionName_, collHeapPtr_);
  if (usageMap_ & USED_CONSTRAINT_CATALOG) unpackBuffer(buffer, constraintCatalog_, collHeapPtr_);
  if (usageMap_ & USED_CONSTRAINT_SCHEMA) unpackBuffer(buffer, constraintSchema_, collHeapPtr_);
  if (usageMap_ & USED_CONSTRAINT_NAME) unpackBuffer(buffer, constraintName_, collHeapPtr_);
  if (usageMap_ & USED_TRIGGER_CATALOG) unpackBuffer(buffer, triggerCatalog_, collHeapPtr_);
  if (usageMap_ & USED_TRIGGER_SCHEMA) unpackBuffer(buffer, triggerSchema_, collHeapPtr_);
  if (usageMap_ & USED_TRIGGER_NAME) unpackBuffer(buffer, triggerName_, collHeapPtr_);
  if (usageMap_ & USED_CATALOG_NAME) unpackBuffer(buffer, catalogName_, collHeapPtr_);
  if (usageMap_ & USED_SCHEMA_NAME) unpackBuffer(buffer, schemaName_, collHeapPtr_);
  if (usageMap_ & USED_TABLE_NAME) unpackBuffer(buffer, tableName_, collHeapPtr_);
  if (usageMap_ & USED_COLUMN_NAME) unpackBuffer(buffer, columnName_, collHeapPtr_);
  if (usageMap_ & USED_SQLID) unpackBuffer(buffer, sqlID_, collHeapPtr_);
  if (usageMap_ & USED_ROW_NUMBER) unpackBuffer(buffer, rowNumber_);
  if (usageMap_ & USED_NSK_CODE) unpackBuffer(buffer, nskCode_);
  if (usageMap_ & USED_CUSTOM_SQLSTATE) unpackBuffer(buffer, customSQLState_, collHeapPtr_);
  ...
  if ((usageMap_ & USED_FUTURE_FIELDS) || foundUprevFieldsInBuffer) flagsTBS_ |= FLAGS_TBS_SUPPRESSED_UPREV_FIELDS;
}
...
void ComCondition::unpackObj32(IpcMessageObjType objType, IpcMessageObjVersion objVersion, NABoolean sameEndianness,
                               IpcMessageObjSize objSize, IpcConstMessageBufferPtr buffer) {
  // NOTE: changes to any of the following methods also require
  // corresponding changes in the others:
  // - packedLength()
...
IpcMessageObjSize ComCondition::packedLength(void) {
  ...
  for (int i = NumOptionalParms; i--;) {
    ...
    if (optionalInteger_[i] != ComDiags_UnInitialized_Int && numIntParamsUsed_ == 0) {
...
IpcMessageObjSize ComCondition::packObjIntoMessage(char *buffer) { return packObjIntoMessage(buffer, FALSE); }
...
IpcMessageObjSize ComCondition::packObjIntoMessage(char *buffer, NABoolean swapBytes) {
  // NOTE: changes to any of the following methods also require
  // corresponding changes in the others:
  // - packedLength()
  // - packObjIntoMessage()
  ...
  if (usageMap_ & USED_FLAGS) size += packIntoBuffer(buffer, flagsTBS_, swapBytes);
  if (usageMap_ & USED_SQLCODE) size += packIntoBuffer(buffer, theSQLCODE_, swapBytes);
  if (usageMap_ & USED_SERVER_NAME) size += packCharStarIntoBuffer(buffer, serverName_, swapBytes);
  if (usageMap_ & USED_CONNECTION_NAME) size += packCharStarIntoBuffer(buffer, connectionName_, swapBytes);
  if (usageMap_ & USED_CONSTRAINT_CATALOG) size += packCharStarIntoBuffer(buffer, constraintCatalog_, swapBytes);
  if (usageMap_ & USED_CONSTRAINT_SCHEMA) size += packCharStarIntoBuffer(buffer, constraintSchema_, swapBytes);
  if (usageMap_ & USED_CONSTRAINT_NAME) size += packCharStarIntoBuffer(buffer, constraintName_, swapBytes);
  if (usageMap_ & USED_TRIGGER_CATALOG) size += packCharStarIntoBuffer(buffer, triggerCatalog_, swapBytes);
  if (usageMap_ & USED_TRIGGER_SCHEMA) size += packCharStarIntoBuffer(buffer, triggerSchema_, swapBytes);
  if (usageMap_ & USED_TRIGGER_NAME) size += packCharStarIntoBuffer(buffer, triggerName_, swapBytes);
  if (usageMap_ & USED_CATALOG_NAME) size += packCharStarIntoBuffer(buffer, catalogName_, swapBytes);
  if (usageMap_ & USED_SCHEMA_NAME) size += packCharStarIntoBuffer(buffer, schemaName_, swapBytes);
  if (usageMap_ & USED_TABLE_NAME) size += packCharStarIntoBuffer(buffer, tableName_, swapBytes);
  if (usageMap_ & USED_COLUMN_NAME) size += packCharStarIntoBuffer(buffer, columnName_, swapBytes);
  if (usageMap_ & USED_SQLID) size += packCharStarIntoBuffer(buffer, sqlID_, swapBytes);
  if (usageMap_ & USED_ROW_NUMBER) size += packIntoBuffer(buffer, rowNumber_, swapBytes);
  if (usageMap_ & USED_NSK_CODE) size += packIntoBuffer(buffer, nskCode_, swapBytes);
  if (usageMap_ & USED_CUSTOM_SQLSTATE) size += packCharStarIntoBuffer(buffer, customSQLState_, swapBytes);
  // if (usageMap_ & USED_ISO_MAPPING_CHARSET)
...
IpcMessageObjSize ComCondition::packObjIntoMessage32(char *buffer) { return packObjIntoMessage32(buffer, FALSE); }

// Let us observe consistency (and thereby, hopefully, simplicity) by
// packing this object following the same order of visiting member data
// as we did in the packedLength() member function above.

IpcMessageObjSize ComCondition::packObjIntoMessage32(char *buffer, NABoolean swapBytes) {
  // NOTE: changes to any of the following methods also require
  // corresponding changes in the others:
  // - packedLength32()
...
NABoolean ComCondition::checkObj(IpcMessageObjType objType, IpcMessageObjVersion objVersion, NABoolean sameEndianness,
  ...
  if (!checkBaseClass(objType, objVersion, sameEndianness, objSize, buffer)) return FALSE;
  ...
  if (!checkBuffer(buffer, sizeof(emsEventVisits_), lastByte)) return FALSE;

  // Some fields are packed conditionally
  if (map & USED_FLAGS)
    if (!checkBuffer(buffer, sizeof(flagsTBS_), lastByte)) return FALSE;

  if (map & USED_SQLCODE) {
    if (!checkBuffer(buffer, sizeof(theSQLCODE_), lastByte)) return FALSE;
  } else {
    return FALSE;
  }
  ...
  if (map & USED_ROW_NUMBER)
    if (!checkBuffer(buffer, sizeof(rowNumber_), lastByte)) return FALSE;
  if (map & USED_NSK_CODE)
    if (!checkBuffer(buffer, sizeof(nskCode_), lastByte)) return FALSE;
...
void ComCondition::setSQLCODE(int newSQLCODE) {
        ...
        reqError == theError)                        // and this one is the requested error
    {
      ...
      while (loopError)  // To exit loop in gdb do: set var loopError=0
      {
        ...
        if (loopCount % 20 == 0) {
          ...
          snprintf(timeString, 40, "%04d-%02d-%02d %02d:%02d:%02d.%06d", tx.tm_year + 1900, tx.tm_mon + 1, tx.tm_mday,
        ...
        if (loopCount == 60) SQLMXLoggingArea::logSQLMXDebugEvent("Loop on error", reqError, __LINE__);
      ...
      (theError == EXE_INTERNAL_ERROR))       // 8001
  {
    ...
    if (!InternErrorMakesCorefileInitialized) {
      ...
      if (envvar && envvar[0] == '1') corefile8816 = true;
      envvar = getenv("SQLMX_COREFILE_8898");
      if (envvar && envvar[0] == '1') corefile8898 = true;
      envvar = getenv("SQLMX_COREFILE_8001");
      if (envvar && envvar[0] == '1') corefile8001 = true;
    }
    if ((corefile8816 && theError == CLI_TCB_EXECUTE_ERROR) || (corefile8898 && theError == CLI_INTERNAL_ERROR) ||
        (corefile8001 && theError == EXE_INTERNAL_ERROR))
      genLinuxCorefile((char *)"Generating core-file to capture internal error scenario.");
  }
}
...
const char *ComCondition::getOptionalString(int index) const {
  ...
  // without considering the above assert
  // coverity[overrun_local]
  if (isSingleByteCharSet(optionalStringCharSet_[index])) return (const char *)optionalString_[index];
  return NULL;
}
...
const NAWchar *ComCondition::getOptionalWString(int index) const {
  ...
  // without considering the above assert
  // coverity[overrun_local]
  if (optionalString_[index] == NULL) return NULL;
  if (!isSingleByteCharSet(optionalStringCharSet_[index])) return (const NAWchar *)optionalString_[index];
  return NULL;
}
...
ComDiagsArea::ComDiagsArea(CollHeap *ptr)
...
void ComDiagsArea::unpackObj(IpcMessageObjType objType, IpcMessageObjVersion objVersion, int sameEndianness,
  ...
  if (lengthLimit_ != ComCondition::NO_LIMIT_ON_ERROR_CONDITIONS) assert(getNumber() <= lengthLimit_);
...
IpcMessageObjSize ComDiagsArea::packObjIntoMessage(char *buffer) { return packObjIntoMessage(buffer, FALSE); }

IpcMessageObjSize ComDiagsArea::packObjIntoMessage(char *buffer, NABoolean swapBytes) {
  // NOTE: changes to any of the following methods also require
  // corresponding changes in the others:
  // - packedLength()
  // - packObjIntoMessage()
  ...
  CollIndex index = 0;
  while (index != (CollIndex)num) {
    IpcMessageObjSize temp = (errors_[index])->packDependentObjIntoMessage(buffer, swapBytes);
    size += temp;
    buffer += temp;
    index++;
  }
  ...
  index = 0;
  while (index != (CollIndex)num) {
    IpcMessageObjSize temp = (warnings_[index])->packDependentObjIntoMessage(buffer, swapBytes);
    size += temp;
    buffer += temp;
    index++;
  }
...
IpcMessageObjSize ComDiagsArea::packObjIntoMessage32(char *buffer) { return packObjIntoMessage32(buffer, FALSE); }

IpcMessageObjSize ComDiagsArea::packObjIntoMessage32(char *buffer, NABoolean swapBytes) {
  // NOTE: changes to any of the following methods also require
  // corresponding changes in the others:
  // - packedLength32()
  // - packObjIntoMessage32()
  ...
  CollIndex index = 0;
  while (index != (CollIndex)num) {
    IpcMessageObjSize temp = (errors_[index])->packDependentObjIntoMessage32(buffer, swapBytes);
    size += temp;
    buffer += temp;
    index++;
  }
  ...
  index = 0;
  while (index != (CollIndex)num) {
    IpcMessageObjSize temp = (warnings_[index])->packDependentObjIntoMessage32(buffer, swapBytes);
    size += temp;
    buffer += temp;
    index++;
  }
...
NABoolean ComDiagsArea::checkObj(IpcMessageObjType objType, IpcMessageObjVersion objVersion, NABoolean sameEndianness,
                                 IpcMessageObjSize objSize, IpcConstMessageBufferPtr buffer) const {
  // NOTE: changes to any of the following methods also require
  // corresponding changes in the others:
  // - packedLength()
  ...
  if (!checkBuffer(buffer,
                   sizeof(rowCount_) + sizeof(avgStreamWaitTime_) + sizeof(cost_) + sizeof(theSQLFunction_) +
                       sizeof(maxDiagsId_) + sizeof(flags_),
                   lastByte))
    return FALSE;

  short extraFieldsLen = 0;
  if (!checkAndUnpackBuffer(buffer, sizeof(extraFieldsLen), (char *)&extraFieldsLen, lastByte)) return FALSE;
  if (!sameEndianness) swapTwoBytes(extraFieldsLen);
  ...
  if (!checkBuffer(buffer, extraFieldsLen, lastByte)) return FALSE;
  ...
  // Error conditions
  if (!checkAndUnpackBuffer(buffer, sizeof(numErrors), (char *)&numErrors, lastByte)) return FALSE;
  if (!sameEndianness) swapTwoBytes(numErrors);

  for (i = 0; i < numErrors; i++)
    if (!cond.checkDependentObj(buffer, sameEndianness)) return FALSE;

  // Warning conditions
  if (!checkAndUnpackBuffer(buffer, sizeof(numWarnings), (char *)&numWarnings, lastByte)) return FALSE;
  if (!sameEndianness) swapTwoBytes(numWarnings);
  ...
  // Check fillers_ part
  if (!checkBuffer(buffer, sizeof(fillers_), lastByte)) return FALSE;
...
int ComDiagsArea::getNumber(DgSqlCode::ErrorOrWarning type) const {
  switch (type) {
    ...
    case DgSqlCode::WARNING_:
      return warnings_.entries();
    default:
      return -1;
  }
}

NABoolean ComDiagsArea::areMore() const { return (areMore_ != ComCondition::NO_MORE); }
...
void ComDiagsArea::setAvgStreamWaitTime(ComDiagBigInt avgStreamWaitTime) { avgStreamWaitTime_ = avgStreamWaitTime; }

double ComDiagsArea::getCost() const { return cost_; }

void ComDiagsArea::setCost(double newCost) { cost_ = newCost; }
...
void ComDiagsArea::setFunction(FunctionEnum newFunction) { theSQLFunction_ = newFunction; }

ComDiagsArea::FunctionEnum ComDiagsArea::getFunction() const { return (ComDiagsArea::FunctionEnum)theSQLFunction_; }
...
static const char *const functionNames[ComDiagsArea::MAX_FUNCTION_ENUM] = {"NULL_FUNCTION",
                                                                           "ALLOCATE_CURSOR",
                                                                           "ALLOCATE_DESCRIPTOR",
                                                                           "ALTER_DOMAIN",
                                                                           "ALTER_TABLE",
...
const char *ComDiagsArea::getFunctionName() const { return functionNames[theSQLFunction_]; }
...
ComDiagsArea::DiagsCondition::DiagsCondition(CollHeap *ptr) : ComCondition(ptr) {}

ComDiagsArea::DiagsCondition::DiagsCondition() : ComCondition() {}
...
IpcMessageObjSize ComDiagsArea::DiagsCondition::packObjIntoMessage(IpcMessageBufferPtr buffer) {
...
IpcMessageObjSize ComDiagsArea::DiagsCondition::packObjIntoMessage(IpcMessageBufferPtr buffer, NABoolean swapBytes) {
  // NOTE: changes to any of the following methods also require
  // corresponding changes in the others:
  // - packedLength()
  // - packObjIntoMessage()
...
IpcMessageObjSize ComDiagsArea::DiagsCondition::packObjIntoMessage32(IpcMessageBufferPtr buffer) {
  return packObjIntoMessage32(buffer, FALSE);
}
IpcMessageObjSize ComDiagsArea::DiagsCondition::packObjIntoMessage32(IpcMessageBufferPtr buffer, NABoolean swapBytes) {
  // NOTE: changes to any of the following methods also require
  // corresponding changes in the others:
  // - packedLength32()
  // - packObjIntoMessage32()
...
void ComDiagsArea::DiagsCondition::unpackObj(IpcMessageObjType objType, IpcMessageObjVersion objVersion,
  ...
  ComCondition::unpackObj(objType, objVersion, sameEndianness, objSize, buffer);
  buffer += objSize - sizeof(diagsId_);
  unpackBuffer(buffer, diagsId_);
}

void ComDiagsArea::DiagsCondition::unpackObj32(IpcMessageObjType objType, IpcMessageObjVersion objVersion,
                                               NABoolean sameEndianness, IpcMessageObjSize objSize,
                                               IpcConstMessageBufferPtr buffer) {
  // NOTE: changes to any of the following methods also require
  // corresponding changes in the others:
  ...
  ComCondition::unpackObj32(objType, objVersion, sameEndianness, objSize, buffer);
  buffer += objSize - sizeof(diagsId_);
  unpackBuffer(buffer, diagsId_);
}

NABoolean ComDiagsArea::DiagsCondition::checkObj(IpcMessageObjType t, IpcMessageObjVersion v, NABoolean sameEndianness,
                                                 IpcMessageObjSize objSize, IpcConstMessageBufferPtr buffer) const {
  // NOTE: changes to any of the following methods also require
  // corresponding changes in the others:
  // - packedLength()
  ...
  if (!ComCondition::checkObj(t, v, sameEndianness, objSize, buffer)) return FALSE;

  if (!checkBuffer(buffer, sizeof(diagsId_), lastByte)) return FALSE;
...
void ComDiagsArea::insertNewWarning() { warnings_.insert(newCondition_); }

void ComDiagsArea::insertNewEODWarning() { warnings_.insert(newCondition_); }
...
void ComDiagsArea::insertNewError() {
  ...
  if (getNonFatalErrorSeen()) setNonFatalErrorSeen(FALSE);
}
...
ComCondition &ComDiagsArea::operator[](int index) const {
  ...
  while (pos < index) {
    if (errorIdx == numErrors)
      ...
      if (errors_[errorIdx]->getDiagsId() < warnings_[warnIdx]->getDiagsId())
        errorIdx++;
      else if (errors_[errorIdx]->getDiagsId() > warnings_[warnIdx]->getDiagsId())
        warnIdx++;
      else {
  ...
  if (sqlCode > 0) {
    for (i = 0; i < warnings_.entries(); ++i)
      if (warnings_[i]->getSQLCODE() == sqlCode) {
        c = warnings_[i];
        if (entryNumber != NULL) *entryNumber = i;
        break;
      }
    return c;
  }
...
const char *ComDiagsArea::getSignalSQLSTATE() const {
  if (errors_.entries() > 0) {
    ...
    if (signal->getSQLCODE() == -ComDiags_SignalSQLCODE) return signal->getOptionalString(0);
  }
  return NULL;
}
  ...
  if (errors_.entries() > 0) {
    if (containsError(-EXE_CANCELED))
      ...
      return errors_[0]->getSQLCODE();
                                         ...
                                         warnings_[0]->getSQLCODE() == 1233))  // CAT_SHARED_CACHE_LOAD_ERROR
...
void ComDiagsArea::negateCondition(CollIndex index) {
  ...
  NABoolean sourceIsWarning = (numErrorEntries == 0 || (index > numErrorEntries - 1));
  ...
  while (insertionPoint != dest.entries() && dest[insertionPoint]->getDiagsId() < ptr->getDiagsId()) insertionPoint++;
...
void ComDiagsArea::mergeAfter(const ComDiagsArea &source) {
  ...
  // they are the same.
  if (((source.mainSQLCODE() == EXE_NONFATAL_ERROR_SEEN) && (mainSQLCODE() == EXE_NONFATAL_ERROR_SEEN)) ||
      ((source.mainSQLCODE() == EXE_NONFATAL_ERROR_ON_ALL_ROWS) && (mainSQLCODE() == EXE_NONFATAL_ERROR_ON_ALL_ROWS))) {
  ...
  } else if ((lengthLimit_ != ComCondition::NO_LIMIT_ON_ERROR_CONDITIONS) && (source.lengthLimit_ > lengthLimit_)) {
  ...
  if (source.avgStreamWaitTime_ != -1) avgStreamWaitTime_ = source.avgStreamWaitTime_;

  if (areMore_ == ComCondition::NO_MORE) areMore_ = source.areMore_;
...
ComDiagsArea *ComDiagsArea::copy() {
  ...
  CollIndex i = 0;
  for (i = 0; i < warnings_.entries(); i++) diagsArea->warnings_.insert(warnings_[i]->copy());
  ...
  if (destPtr != NULL && destPtr != this) {
    ...
    for (i = 0; i != errors_.entries(); i++)
      if (errors_[i]->getDiagsId() > markValue) {
        ...
        DiagsCondition *newPtr = DiagsCondition::allocate(destPtr->collHeapPtr_);
...
void ComCondition::destroyMe() { this->~ComCondition(); }

void ComDiagsArea::destroyMe() { this->~ComDiagsArea(); }

void ComDiagsArea::DiagsCondition::destroyMe() { this->DiagsCondition::~DiagsCondition(); }

ComDiagsArea::DiagsCondition &ComDiagsArea::DiagsCondition::operator=(const ComDiagsArea::DiagsCondition &d) {
...
void ComDiagsArea::removeFinalCondition100() {
  ...
  if ((w100->getDiagsId() != maxDiagsId_) || (w100->getSQLCODE() != 100)) {
    return;
  }
...
void ComDiagsArea::removeLastNonFatalCondition() {
  ...
  if ((wNF->getDiagsId() != maxDiagsId_) ||
      ((wNF->getSQLCODE() != EXE_NONFATAL_ERROR_SEEN) && (wNF->getSQLCODE() != EXE_NONFATAL_ERROR_ON_ALL_ROWS))) {
    return;
  }
...
int ComDiagsArea::markDupNFConditions() {
  ...
  else if ((lnfw = returnIndex(EXE_NONFATAL_ERROR_SEEN)) != NULL_COLL_INDEX)
    warningMark = warnings_[lnfw - errors_.entries()];
...
void ComDiagsTranslator::analyzeCondition(const ComCondition &cond) { assert(0); }
