...
public class DumpCommand extends ArgsOnlyCommand {
    ...
    private static class timer {
       ...
       {
          System.out.println(msg + elapsedTime_ + "ms");
       }

       public String getElapsedTime(String msg)
       {
          return msg + elapsedTime_ + "ms";
       }
    ...
    static {
        ...
        Option st = OptionBuilder.withLongOpt("scan-data")
                                 .withDescription("scan column data only. No dump")
                                 .create('s');
    ...
    public void execute(CommandLine options) throws Exception {
        ...
        boolean scandt = options.hasOption('s');
        ...
        dump(out, metaData, schema, inpath, showmd, showdt, scandt, showColumns);

        tmr.stop();
        out.format("\n%s\n", tmr.getElapsedTime("Total time:"));
        out.flushColumns();

        //PrintWriter writer = new PrintWriter("dumptime.txt");
        //writer.println(tmr.getElapsedTime("Total scan time:"));
        //writer.close();
    }

    public static void dump(PrettyPrintWriter out, ParquetMetadata meta, MessageType schema, Path inpath, boolean showmd, boolean showdt, boolean scandt, Set<String> showColumns) throws IOException {
        Configuration conf = new Configuration();
        conf.set("parquet.strings.signed-min-max.enabled", "true");
                ...
                try {
                    ...
                    while (store != null) {
                        ColumnReadStoreImpl crstore = new ColumnReadStoreImpl(
                            store, new DumpGroupConverter(), schema,
                            meta.getFileMetaData().getCreatedBy());
                        scan(out, crstore, column, page++, total, offset);
    ...
    public static void scan(PageReadStore store, ColumnDescriptor column) throws IOException {
        PageReader reader = store.getPageReader(column);
    ...
    public static void scan(final PrettyPrintWriter out, ColumnReadStoreImpl crstore, ColumnDescriptor column, long page, long total, long offset) throws IOException {
        ...
        for (long i = 0, e = creader.getTotalValueCount(); i < e; ++i) {
            ...
            if (dlvl == dmax) {
                ...
                case BINARY:  creader.getBinary(); break;
                case BOOLEAN: creader.getBoolean(); break;
                case DOUBLE:  creader.getDouble(); break;
                case FLOAT:   creader.getFloat(); break;
                case INT32:   creader.getInteger(); break;
                case INT64:   creader.getLong(); break;
                case INT96:   creader.getBinary(); break;
                case FIXED_LEN_BYTE_ARRAY: creader.getBinary(); break;
