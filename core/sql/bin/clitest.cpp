...
int main(int argc, const char * argv[])
{
  ...
  retcode = SQL_EXEC_CreateContext(&defContext, NULL, 0);
  ...
  setSessionBegin(cliGlob);
  char tablename[50] = {'\0'};
  char columnname[50] = {'\0'};
  char columnname2[50] = {'\0'};
  char filename[50] = {'\0'};
  switch (option) 
    {
    case 1:
      ...
      if (retcode < 0)
	{
      ...
      if (retcode < 0)
	{
	  cout << "extractLengthOfLobColumn2 returned " << retcode <<endl;
	 
	  delete lobHandle;
	  return retcode;
	}
      cout << "LOB data length 2 :" << lengthOfLob << endl;

      cout << "Extracting lob data into user buffer in a loop 2..." << endl;
      retcode = extractLobToBuffer(cliGlob,lobHandle, lengthOfLob,columnname,tablename);
      if (retcode < 0)
	{
	  cout << "extractLobToBuffer returned " << retcode <<endl;
      ...
      lengthOfLob= 0;
      retcode = extractLengthOfLobColumnPrepExec(cliGlob, lobHandle, lengthOfLob,columnname,(char *)tablename);
      if (retcode < 0)
	{
	  cout << "extractLengthOfLobColumnPrepExec returned " << retcode <<endl;
	 
	  delete lobHandle;
	  return retcode;
	}
      cout << "LOB data length PrepExec :" << lengthOfLob << endl;
	...
	retcode = updateBufferToLobHandle(cliGlob,lobHandle);
        if (retcode < 0)
          cout << "updateBufferToLobHandle returned " << retcode <<endl;
	return retcode;
      }
      break;
case 7:
	...
	retcode = updateAppendBufferToLobHandle(cliGlob,lobHandle,20,(Int64)source);
        if (retcode < 0)
          cout << "updateAppendBufferToLobHandle returned " << retcode <<endl;
	return retcode;
      }
      break;
case 8:
	...
	retcode = updateTruncateLobHandle(cliGlob,lobHandle);
        if (retcode < 0)
          cout << "updateTruncateLobHandle returned " << retcode <<endl;
	return retcode;
      }
      break;
...
case 9:
	...
	cout <<"*************************************"  <<endl;
	cout << "Blob test update 2 columns using lobhandle " << endl;
	cout << "Input lob table name :" << endl;
	cin.getline(tablename,40);
	cout << "Table name : " << tablename << endl;

	cout << "Input lob column name 1 to get handle from :" << endl;
	cin.getline(columnname,40); 
	cout << "Column Name 1 : " << columnname << endl;

	cout << "Input lob column name 2 to get handle from :" << endl;
	cin.getline(columnname2,40); 
	cout << "Column Name 2 : " << columnname2 << endl;

        //extract lob handle for column 1
        char *lobHandle = new char[1024];
        str_cpy_all(lobHandle," ",1024);
        cout << "Extracting  lob handle for column " << columnname << "..." << endl;
        retcode = extractLobHandle(cliGlob, lobHandle, (char *)columnname,tablename);
        ...
        char *lobHandle2 = new char[1024];
        str_cpy_all(lobHandle2," ",1024);
        cout << "Extracting  lob handle for column 2 " << columnname2 << "..." << endl;
        retcode = extractLobHandle(cliGlob, lobHandle2, (char *)columnname2, tablename);
        if (retcode < 0)
          {
            cout << "extractLobHandle returned " << retcode <<endl;
        ...
        cout << "LOB handle for "<< columnname2 << ": " << lobHandle2 << endl;


	retcode = updateBufferToLobHandle(cliGlob, lobHandle, lobHandle2);
        if (retcode < 0)
          cout << "updateBufferToLobHandle returned " << retcode <<endl;
	return retcode;
      }
