...
// in double quotes.
void processOption(Int32 argc, char *argv[], Int32 &i, const char *&in_filename, const char *&input_string,
                   const char *&out_filename, char *&sock_port, NAString &user_name, NAString &tenant_name,
                   char *&max_heap) {
  Int32 iorig = i;
  char op = '\0';
  ...
  if (argAfterOp) {
    switch (op) {
      case 's':
        sock_port = argAfterOp;  // socket port number
        break;
      case 'i':
        in_filename = argAfterOp;  // input file
        break;
      case 'o':
        out_filename = argAfterOp;  // output file
        break;
      case 'q':
        input_string = argAfterOp;  // input qry
        break;
      ...
      default:;  // ok -- may be an X option, processed later in InitializeX
    ...
    switch (op) {
      ...
      case 'p':
      case 't':
        cerr << argv[0] << ": " << argv[iorig] << " option requires an argument -- ignored" << endl;
        break;

      case '\0':
        cerr << argv[0] << ": " << argv[iorig] << " option is unknown -- ignored" << endl;
        break;
...
void *thread_main(void *p_arg) {
  ...
  if (g_sync_with_stdio) ios::sync_with_stdio();
  ...
  Int32 i = 1;
  for (; i < g_argc; i++)
    processOption(g_argc, g_argv, i, (const char *&)g_in_filename, (const char *&)g_input_string,
                  (const char *&)g_out_filename, (char *&)g_sock_port, g_user_name, g_tenant_name, (char *&)g_max_heap);
  ...
  if (g_user_name.length() > 0) sqlci->setUserNameFromCommandLine(g_user_name);

  if (g_tenant_name.length() > 0) sqlci->setTenantNameFromCommandLine(g_tenant_name);
  ...
  if ((!g_in_filename) && (g_out_filename)) {
    ...
    char *logf = new char[strlen("LOG ") + strlen(g_out_filename) + strlen(" clear;") + 1];
...
Int32 main(Int32 argc, char *argv[]) {
  ...
  try {
    ...
    cerr << "Error while initializing messaging system. Please make sure Trafodion is started and up. Exiting..."
         << endl;
    exit(1);
  }
