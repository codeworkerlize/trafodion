...
Int32 runESP(Int32 argc, char** argv, GuaReceiveFastStart *guaReceiveFastStart)
{
  ...
  IpcThreadInfo mainThreadInfo("ESP main thread",
                               0,
                               GETTID());
  ...
  if (multiThreaded)
    {
      ...
      if (retcode != 0) {
        ...
        snprintf(errorBuf, sizeof(errorBuf),
                 "Error %d in pthread_attr_init of waiter thread", retcode);
        ex_assert(retcode == 0, errorBuf);
      }

      retcode = pthread_create(&waiterThreadId,
                               &waiterThreadAttr,
                               waiterThreadStart,
                               (void *)ipcEnvPtr);
      if (retcode != 0) {
        ...
        snprintf(errorBuf, sizeof(errorBuf),
                 "Error %d in pthread_create for waiter thread", retcode);
        ex_assert(retcode == 0, errorBuf);
      }
    }
  ...
  while (espFragInstanceDir.getNumMasters() > 0)
    {
      if (multiThreaded)
        ...
        while (espIpcControlMessage.hasRequest())
          espIpcControlMessage.workOnReceivedMessage();
      ...
      if (multiThreaded)
        ...
        ipcEnvPtr->getAllConnections()->
          waitOnAll(30000/*5mins*/, TRUE, &timeout, &prevWaitTime); // TRUE means: Called by ESP main

      if (timeout && NOT espFragInstanceDir.allInstanceAreFree())
      {
        QRLogger::log(CAT_SQL_ESP, LL_WARN, "Active ESP wait message timeout, timeout=%d, time=%ld.",
  ...
  switch (messageNum)
      ...
      if (initialized_)
	{
	  ...
	  EspNewIncomingConnectionStream *newStream = new(getEnv()->getHeap())
	    EspNewIncomingConnectionStream(getEnv(),
                                           espFragInstanceDir_,
