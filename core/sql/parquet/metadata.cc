...
static std::shared_ptr<RowGroupStatistics> MakeTypedColumnStats(
  ...
  // If ColumnOrder is defined, return max_value and min_value if both are defined
  if (descr->column_order().get_order() == ColumnOrder::TYPE_DEFINED_ORDER &&
      // Esgyn extension: check on __isset.min_value and max_value
      // before use the min_value and max_value version of min and max.
...
class ColumnChunkMetaData::ColumnChunkMetaDataImpl {
 public:
  ...
  //    Eg: UTF8
  inline bool is_stats_set(bool skipCorrectStatsCheck) const {
    DCHECK(writer_version_ != nullptr);

    bool canSkipCorrectStatsCheck = 
           skipCorrectStatsCheck && Comparator::isSupported(descr_);

    return column_->meta_data.__isset.statistics &&
           (canSkipCorrectStatsCheck ||
            writer_version_->HasCorrectStatistics(type(), descr_->sort_order()));
  }

  inline std::shared_ptr<RowGroupStatistics> statistics(bool skipCorrectStatsCheck) const {
    if (stats_ == nullptr && is_stats_set(skipCorrectStatsCheck)) {
...
// Esgyn extension: skipCorrectStatsCheck
std::shared_ptr<RowGroupStatistics> ColumnChunkMetaData::statistics(bool skipCorrectStatsCheck) const {
  return impl_->statistics(skipCorrectStatsCheck);
}

// Esgyn extension: skipCorrectStatsCheck
bool ColumnChunkMetaData::is_stats_set(bool skipCorrectStatsCheck) const { return impl_->is_stats_set(skipCorrectStatsCheck); }
...
#if 0
// esgyn port, reg_match() and reg_search() do not work with GCC 4.8. Use
// strtok() instead.
//
// esgyn port, remove boost dependency
/*
...
*/
  ...
  std::regex app_regex(ApplicationVersion::APPLICATION_FORMAT, std::regex_constants::basic);
  std::regex ver_regex(ApplicationVersion::VERSION_FORMAT, std::regex_constants::basic);
  ...
  std::transform(created_by_lower.begin(), created_by_lower.end(),
   ...
   char* created_by_copy = strdup(created_by_lower.c_str());
   token = strtok(created_by_copy, s);

   if ( token )
     application_ = token;
   else {
     free(created_by_copy);
     return;
   }
   ...
   if ( token ) {
      // find major, minor etc
      const char* dot = ".";

      char* version_str = strdup(token);
      token = strtok(version_str, dot);
...
//
// BinaryStatistics currently only have a min/max, which are compared 
// as signed byte[]. However, for real UTF8-friendly lexicographic 
// comparison, e.g. for string columns, we would want to calculate 
// the BinaryStatistics based off of a comparator that treats 
// the bytes as unsigned.
//
// fixed in 1.9.0, 1.8.2
//
// Java code changes are here: 
//
// https://github.com/apache/parquet-mr/commit/de99127d77dabfc6c8134b3c58e0b9a0b74e5f37#diff-2c3b5a2baa7bf9ea11ff87c9b1fd1216
//
bool ApplicationVersion::HasCorrectStatistics(Type::type col_type,
  ...
  // Esgyn extension: adding the following comment. 
  //
  // PARQUET-251: 
  //
  // From https://issues.apache.org/jira/browse/PARQUET-251:
  // I think it is a common practice when inserting table data as 
  // parquet file, one would always reuse the same object among rows, 
  // and if a column is byte[] of fixed length, the byte[] would 
  // also be reused.  If I use ByteArrayBackedBinary for my byte[], 
  // the bug occurs: All of the row groups created by a single task 
  // would have the same max & min binary value, just as the 
  // last row's binary content.
  //
  // Fixed in 1.8.0.
  //
  // If there is only one row-group in the data file, this bug has
  // no effect.
  //
