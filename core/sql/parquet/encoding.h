...
class Decoder {
 public:
  ...
  virtual void SetData(int num_values, const uint8_t* data, int len, bool skip=false) = 0;
  ...
  virtual int DecodeSpaced(T* buffer, int num_values, int null_count,
    ...
    if ( !skip_ ) {
       ...
       memset(buffer, 0, num_values * sizeof(T));

#ifdef DEBUG_SKIP_DECODE
       std::cout << "<typename DType>::Decode(): skip " 
 ...
 protected:
  ...
  // Esgyn performance extension: whether to skip the data 
  // during Decode(). Set via SetData() call. 
  bool skip_;
};
