...
struct Type {
  enum type {
    ...
    /// Esgyn Extension
    /// Time as signed Parquet Int96 (struct of 3 32-bit integers), 
...
//Esgyn Extension
class ARROW_EXPORT Time96Type : public TimeType {
 public:
  static constexpr Type::type type_id = Type::TIME96;
  using c_type = ::parquet::Int96;

  int bit_width() const override { return static_cast<int>(sizeof(c_type) * CHAR_BIT); }

  explicit Time96Type(TimeUnit::type unit = TimeUnit::NANO);

  Status Accept(TypeVisitor* visitor) const override;
  std::string ToString() const override;

  std::string name() const override { return "time96"; }
};
