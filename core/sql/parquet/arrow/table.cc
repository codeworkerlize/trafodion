...
// Esgyn extension
int64_t ChunkedArray::totalValuesSelected() const 
{
  int64_t ct = 0;
  for (const std::shared_ptr<Array>& chunk : chunks_) {
    ct += chunk->totalValuesSelected();
  }
  return ct;
}
...
void Column::display(std::ostream& out, int64_t maxNumRows) const
{
  out << "type:" << this->type()->name();
  out << ", #chunks:" << data_->num_chunks() << std::endl;

  int ct = 0;
  for (int i = 0; i < data_->num_chunks(); ++i) {
     std::shared_ptr<Array> chk = data_->chunk(i);

     switch ( this->type()->id() ) 
      {
        case Type::INT32:
        {
           auto numChk = std::dynamic_pointer_cast<Int32Array>(chk);
           numChk->display(out, maxNumRows);
           break;
        }

        case Type::FLOAT:
        {
           auto numChk = std::dynamic_pointer_cast<FloatArray>(chk);
           numChk->display(out, maxNumRows);
           break;
        }

        default:
          out << "Not supported yet. Please update Column::display()" << std::endl;
          break;
     }
  }
  out << std::endl;
...
void Column::scan(std::ostream& out) const
{
  //out << "type:" << this->type()->name();
  //out << ", #chunks:" << data_->num_chunks() << std::endl;
     ...
     switch ( this->type()->id() ) 
      {
        ...
        default:
          out << "Not supported yet. Please update Column::scan()" << std::endl;
          break;
     }
  }
  //out << std::endl;
...
class SimpleTable : public Table {
 public:
  ...
  Status Validate() const override {
    ...
    for (int i = 0; i < num_columns(); ++i) {
      const Column* col = columns_[i].get();
      // Esgyn extension: some columns can have few rows due to
      // filtering. Just make sure each column has no more rows
      // than num_rows_.
      if (col->length() > num_rows_) {
        std::stringstream ss;
  ...
  void display(std::ostream& out, int64_t maxNumRows) const
  {
    for (int i = 0; i < num_columns(); ++i) {

      out << "Column #:" << i << std::endl;

      const Column* col = columns_[i].get();
      col->display(out, maxNumRows);
    }
  }

  void scan(std::ostream& out) const
  {
    for (int i = 0; i < num_columns(); ++i) {
...
class TableBatchReader::TableBatchReaderImpl {
 public:
  ...
  Status ReadNext(std::shared_ptr<RecordBatch>* out) {
    ...
    std::vector<int64_t> columnsEvalTime(table_.num_columns());
    ...
    for (int i = 0; i < table_.num_columns(); ++i) {
      ...
      columnsEvalTime[i] = chunk->getValueFilterEvalTime();
    ...
    *out = RecordBatch::Make(table_.schema(), chunksize, std::move(batch_data));
    ...
    (*out)->setColumnsEvalTime(columnsEvalTime);

    if ( !isSliced ) {

       for (int i=0; i<(*out)->num_columns(); i++ ) {
         auto arrayPtr = (*out) -> column(i);
         const skipVecType& vec = chunks[i]->getSkipVec();
         arrayPtr->transferSkipVec(&vec);
       }
    }
