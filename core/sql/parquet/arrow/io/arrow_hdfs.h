...
struct HdfsConnectionConfig {
  ...
  HdfsConnectionConfig(const char* h, int p, const char* u,
                       const char* k="", 
                       HdfsDriver r=HdfsDriver::LIBHDFS
                      ) :
    host(h)
    , port(p)
    , user(u)
...
class ARROW_EXPORT HadoopFileSystem : public FileSystem {
 public:
  ...
  // Esgyn extension: reuse an existing hdfs client
  static Status Reuse(hdfsFS& knownHDFS,
