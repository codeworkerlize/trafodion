...
static std::vector<std::string> get_potential_libjvm_paths();
static std::vector<std::string> get_potential_libhdfs_paths();
static std::vector<std::string> get_potential_libhdfs3_paths();
static arrow::Status try_dlopen(std::vector<std::string> potential_paths, const char* name,
#ifndef _WIN32
                                void*& out_handle);
#else
...
static std::vector<std::string> get_potential_libhdfs_paths() {
  std::vector<std::string> libhdfs_potential_paths;
...
// OS-specific file name
  ...
  // Common paths
  std::vector<std::string> search_paths = {std::string(""), std::string(".")};

  // Path from environment variable
  // esgyn port
  //const char* hadoop_home = std::getenv("HADOOP_HOME");
  const char* hadoop_home = std::getenv("TRAF_HOME");
  if (hadoop_home != nullptr) {
    auto path = std::string(hadoop_home) + "/export/lib64d";
    search_paths.push_back(path);

    path = std::string(hadoop_home) + "/export/lib64";
    search_paths.push_back(path);
  }
  ...
  if (libhdfs_dir != nullptr) {
    search_paths.push_back(std::string(libhdfs_dir));
  }
#endif
...
// OS-specific file name
  ...
  if (libhdfs3_dir != nullptr) {
    search_paths.push_back(std::string(libhdfs3_dir));
  }
...
// expecting users to set an environment variable
  ...
  for (auto& prefix : search_prefixes) {
    for (auto& suffix : search_suffixes) {
      auto path = (std::string(prefix) + "/" + std::string(suffix) + "/" + std::string(file_name));
...
Status ConnectLibHdfs(LibHdfsShim** driver) {
  ...
  if (!shim_attempted) {
    ...
    std::vector<std::string> libjvm_potential_paths = get_potential_libjvm_paths();
    RETURN_NOT_OK(try_dlopen(libjvm_potential_paths, "libjvm", libjvm_handle));

    std::vector<std::string> libhdfs_potential_paths = get_potential_libhdfs_paths();
...
Status ConnectLibHdfs3(LibHdfsShim** driver) {
  ...
  if (!shim_attempted) {
    ...
    std::vector<std::string> libhdfs3_potential_paths = get_potential_libhdfs3_paths();
