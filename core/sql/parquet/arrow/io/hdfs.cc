...
class HadoopFileSystem::HadoopFileSystemImpl {
 public:
  ...
  // Esgyn 
  Status Reuse(hdfsFS& fs, const HdfsConnectionConfig* config) {

    if ( fs == nullptr ) {
      return Status::Invalid("hdfsFS to be reused is null");
    }
    ...
    if ( driver_ == nullptr ) {
      return Status::IOError("The shim library is not initialized");
    }
...
{ 
    impl_.reset(new HadoopFileSystemImpl()); 
}
...
Status HadoopFileSystem::Reuse(hdfsFS& hdfs,
                               const HdfsConnectionConfig* config,
                               std::shared_ptr<HadoopFileSystem>* fs) {
  // ctor is private, make_shared will not work
  *fs = std::shared_ptr<HadoopFileSystem>(new HadoopFileSystem());

  RETURN_NOT_OK((*fs)->impl_->Reuse(hdfs, config));
  return Status::OK();
}
