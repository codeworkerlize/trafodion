...
class RecordReader::RecordReaderImpl {
 public:
  RecordReaderImpl(const ColumnDescriptor* descr, MemoryPool* pool)
    ...
    selected_bits_ = std::make_shared<PoolBuffer>(pool);
  ...
  {
    PageFilter* pf = getPageFilter();
    return ( pf ) ? pf->getMergedSkipVec() : NULL;
  }

  std::shared_ptr<PoolBuffer> ReleaseIsSelected() {
    auto result = selected_bits_;
    selected_bits_ = std::make_shared<PoolBuffer>(pool_);

    // Fast set each bit to 1 if selected, or to 0, skipped
    auto selected_bits = result->mutable_data();

    skipVecType* skipVec = getMergedSkipVec();

    if ( !skipVec ) {
      arrow::BitUtil::SETAllBits(selected_bits, values_written());
    } else {

       arrow::BitUtil::ClearAllBits(selected_bits, values_written());
    
       int64_t offset = 0;
       for (int i=0; i < skipVec->size(); i++) {
         int64_t n = (*skipVec)[i].len;

         if ( n > 0 ) {
           arrow::BitUtil::SetBits(selected_bits, offset, offset+n-1);
           offset += n;
         } else {
           offset += (-n);
         }
  ...
  void ReserveValues(int64_t capacity) {
    ...
    int64_t selected_bytes_new = BitUtil::BytesForBits(values_capacity_);
    if (selected_bits_->size() < selected_bytes_new) {
        int64_t selected_bytes_old = BitUtil::BytesForBits(values_written_);
        PARQUET_THROW_NOT_OK(selected_bits_->
                          Resize(selected_bytes_new, false));

        // Avoid valgrind warnings
        memset(selected_bits_->mutable_data() + selected_bytes_old, 0,
               selected_bytes_new - selected_bytes_old);
    }
  }
  ...
  void ResetValues() {
    if (values_written_ > 0) {
      ...
      PARQUET_THROW_NOT_OK(selected_bits_->Resize(0, false));
      values_written_ = 0;
      values_capacity_ = 0;
      null_count_ = 0;
    }
    ...
    // Esgyn extension
    PageFilter* filter = getPageFilter();
    if ( filter )
      filter->resetCounter();
  }

  void setPageFilter(PageFilter* x)
  { pageFilter_ =x; }
 ...
 protected:
  ...
  std::shared_ptr<::arrow::PoolBuffer> selected_bits_;
...
class TypedRecordReader : public RecordReader::RecordReaderImpl {
 public:
  ...
  inline void displayStats(const EncodedStatistics& stats) 
    {
       std::cout << "Need a special version of displayStats()." << std::endl;
    };
  ...
  bool HasNext(int64_t& recordsSkipped) {
    // Either there is no data page available yet, or the data page has been
    // exhausted
    if (num_buffered_values_ == 0 || num_decoded_values_ == num_buffered_values_) {
      if (!ReadNewPage(recordsSkipped) || num_buffered_values_ == 0) {
        return false;
      }
    }
    return true;
  ...
  int64_t ReadRecords(int64_t num_records) override {
    ...
    if (records_read == 0 && !HasNext(records_skipped)) {
      return 0;
    }
    ...
    // enough records
    while (!at_record_start_ || records_read+records_skipped < num_records) {
      // Is there more data to read in this row group?
      if (!HasNext(records_skipped)) {
        break;
      }
 ...
 private:
  ...
  bool ReadNewPage(int64_t& recordsSkipped);
...
inline void TypedRecordReader<ByteArrayType>::ReadValuesDense(int64_t values_to_read) {
  ...
  // Esgyn extension
  // length() is the current end of the data or the starting point of the append operation.
  // After the following loop, the value of length() will be the new end of
  // data. Hence the need to capture the value here.
  auto start = builder->length();
  ...
  builder->appendToIsSelected(start, values_to_read, getMergedSkipVec());
...
template <typename DType>
bool TypedRecordReader<DType>::ReadNewPage(int64_t& recordsSkipped) {
  ...
  while (true) {
    ...
    if (current_page_->type() == PageType::DICTIONARY_PAGE) {
      ...
      if ( pf )
        skipPage = pf->evalAndUpdateSkipVec(page);
      ...
      current_decoder_->SetData(static_cast<int>(num_buffered_values_), buffer,
                                static_cast<int>(data_size), skipPage);
      return true;
    } else {
...
std::string Int96ToTimestamp(Int96 x)
{
  int64_t julianTimestamp = Int96ToInt64Timestamp(x);

  char buf[50];
  sprintf(buf, "%ld", julianTimestamp);

  return std::string(buf);

}
std::string Int96ToInt32s(int* ptr)
{
  char buf[50];
  sprintf(buf, "%d-%d-%d", *ptr, *(ptr+1), *(ptr+2));
  return std::string(buf);
}
...
std::shared_ptr<PoolBuffer> RecordReader::ReleaseIsSelected() {
  return impl_->ReleaseIsSelected();
}
...
void RecordReader::setPageFilter(PageFilter* x)
{ impl_->setPageFilter(x); }

PageFilter* RecordReader::getPageFilter()
{ return impl_->getPageFilter(); }
