...
class SimpleRecordBatch : public RecordBatch {
 public:
  ...
  std::shared_ptr<Array> column(int i) const override {
    if (!boxed_columns_[i]) {
      ...
      // esgyn extension
      boxed_columns_[i]->incValueFilterEvalTime(columnsEvalTime_[i]);
    }
  ...
  // esgyn extension
  std::vector<int64_t>& getColumnsEvalTime() { return columnsEvalTime_; }

  void setColumnsEvalTime(std::vector<int64_t>& x) 
     { columnsEvalTime_ = x; }
 ...
 private:
  ...
  // esgyn extension
  std::vector<int64_t> columnsEvalTime_;
};
