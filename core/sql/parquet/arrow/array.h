...
// Array level
const int NULL_BITMAP = 0;
const int SELECTED_BITMAP = 1;

// PrimitiveArray level
const int PA_VALUES = 2;

// ListArray level
const int LA_VALUE_OFFSETS = 2;

// BinaryArray level
const int BA_VALUE_OFFSETS = 2;
const int BA_VALUE_DATA = 3;

// UnionArray level
const int UA_TYPE_IDS = 2;
const int UA_VALUE_OFFSETS = 3;
...
class ARROW_EXPORT Array {
 public:
  ...
  bool IsSelected(int64_t i) const {
    return selected_bitmap_data_ != NULLPTR &&
           BitUtil::GetBit(selected_bitmap_data_, i + data_->offset);
  }
  ...
  std::shared_ptr<Buffer> null_bitmap() const 
      { return data_->buffers[NULL_BITMAP]; }

  std::shared_ptr<Buffer> selected_bitmap() const 
      { return data_->buffers[SELECTED_BITMAP]; }
  ...
  const uint8_t* selected_bitmap_data() const { return selected_bitmap_data_; }
 
  // Esgyn extension
  int64_t totalValuesSelected();
  bool NextSelected(int64_t& pos);
  ...
  virtual void display(int64_t maxNumRows) {};
  virtual void scan() {};

  int64_t getValueFilterEvalTime() const
    { return valueFilterEvalTime_; }
   
  void incValueFilterEvalTime(int64_t et)
    { valueFilterEvalTime_ += et; }

  void transferSkipVec(const skipVecType* x) 
   {  
      if (x) 
        skipVec_ = (*x); 
   };

  const skipVecType& getSkipVec() const
   { return skipVec_; } 

 protected:
  Array() : valueFilterEvalTime_(0) {}

  std::shared_ptr<ArrayData> data_;
  const uint8_t* null_bitmap_data_;
  const uint8_t* selected_bitmap_data_;

  skipVecType skipVec_;

  int64_t valueFilterEvalTime_; // in uec

  // Esgyn extension
  internal::BitmapReader selectedBitmapReader_;

  /// Protected method for constructors
  inline void SetData(const std::shared_ptr<ArrayData>& data) {
    if (data->buffers.size() > 0 && data->buffers[NULL_BITMAP]) {
      null_bitmap_data_ = data->buffers[NULL_BITMAP]->data();
    } else {
      null_bitmap_data_ = NULLPTR;
    }

    if (data->buffers.size() > 1 && data->buffers[SELECTED_BITMAP]) {
      selected_bitmap_data_ = data->buffers[SELECTED_BITMAP]->data();
    } else {
      selected_bitmap_data_ = NULLPTR;
    }

    data_ = data;

    selectedBitmapReader_.init(selected_bitmap_data_, offset(), length());
  }
...
class ARROW_EXPORT PrimitiveArray : public FlatArray {
 public:
  PrimitiveArray(const std::shared_ptr<DataType>& type, int64_t length,
                 ...
                 const std::shared_ptr<Buffer>& selected_bitmap,
                 const std::shared_ptr<Buffer>& null_bitmap = NULLPTR,
                 int64_t null_count = 0, int64_t offset = 0);

  /// Does not account for any slice offset
  std::shared_ptr<Buffer> values() const 
      { return data_->buffers[PA_VALUES]; }

 protected:
  PrimitiveArray() {}

  inline void SetData(const std::shared_ptr<ArrayData>& data) {
    auto values = data->buffers[PA_VALUES];
...
class ARROW_EXPORT NumericArray : public PrimitiveArray {
 public:
  ...
  void display(std::ostream& out, int64_t maxNumRows);
  void scan();
...
class ARROW_EXPORT ListArray : public Array {
 public:
  ...
  std::shared_ptr<Buffer> value_offsets() const 
      { return data_->buffers[LA_VALUE_OFFSETS]; }
...
class ARROW_EXPORT BinaryArray : public FlatArray {
 public:
  ...
  /// Note that this buffer does not account for any slice offset
  std::shared_ptr<Buffer> value_offsets() const 
     { return data_->buffers[BA_VALUE_OFFSETS]; }

  /// Note that this buffer does not account for any slice offset
  std::shared_ptr<Buffer> value_data() const 
     { return data_->buffers[BA_VALUE_DATA]; }
...
class ARROW_EXPORT StringArray : public BinaryArray {
 public:
  ...
  // Esgyn Extension. Assume buf has enough space to accept the string
  // value.
  void GetString(int64_t i, char* buf, int32_t& len) const {
    const uint8_t* str = GetValue(i, &len);
    memmove(buf, str, len);
  }
  // Esgyn Extension. Assume buf has enough space to accept the string
  // value plus the length of type int32_t at the beginning.
  void GetStringAndLength(int64_t i, char* buf, int32_t& len) const {
    GetStringAndLength(i, buf+sizeof(int32_t), len);
    ((int32_t*)buf)[0] = len;
  }
};
...
class ARROW_EXPORT StructArray : public Array {
 public:
  ...
  StructArray(const std::shared_ptr<DataType>& type, int64_t length,
              ...
              std::shared_ptr<Buffer> selected_bitmap, 
...
class ARROW_EXPORT UnionArray : public Array {
 public:
  ...
  std::shared_ptr<Buffer> type_ids() const 
        { return data_->buffers[UA_TYPE_IDS]; }

  /// Note that this buffer does not account for any slice offset
  std::shared_ptr<Buffer> value_offsets() const 
        { return data_->buffers[UA_VALUE_OFFSETS]; }
...
ARROW_EXTERN_TEMPLATE NumericArray<Time96Type>;
...
typedef arrow::NumericArray<arrow::Int8Type> NumericInt8ArrayType;
typedef arrow::NumericArray<arrow::Int16Type> NumericInt16ArrayType;
typedef arrow::NumericArray<arrow::Int32Type> NumericInt32ArrayType;
typedef arrow::NumericArray<arrow::FloatType> NumericFloatArrayType;
typedef arrow::NumericArray<arrow::DoubleType> NumericDoubleArrayType;
typedef arrow::NumericArray<arrow::Int64Type> NumericInt64ArrayType;
typedef arrow::NumericArray<arrow::Time96Type> NumericTime96ArrayType;
