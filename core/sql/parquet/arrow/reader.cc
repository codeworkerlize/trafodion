...
#include "arrow/util/ttime.h"

#include "parquet/arrow/record_reader.h"
#include "parquet/arrow/arrow_schema.h"
...
impala_timestamp_to_nanoseconds(const Int96& impala_timestamp, 
                                int64_t& result) 
{
  ...
  if ( days_since_epoch >= 0 ) {
    ...
    result = days_since_epoch * kNanosecondsInADay + nanoseconds;

    if ( result >= 0 ) 
      return Status::OK();
  }
  
  return Status::Invalid("a nano-second Unix timestamp overflows or underflows int64_t");
}
 ...
 public:
  explicit FileColumnIterator(int column_index, ParquetFileReader* reader)
      : column_index_(column_index),
        ...
        pageFilter_(reader->getPageFilter(column_index)),
        valueFilter_(reader->getValueFilter(column_index))
  { }
  ...
  PageFilter* getPageFilter() const { return pageFilter_; }
  ValueFilter* getValueFilter() const { return valueFilter_; }
...
class FileReader::Impl {
 public:
  ...
  void setFilters(std::vector<Filters>* x) 
   { reader_->setFilters(x); }

  ParquetFileReader* reader() { return reader_.get(); }

  std::shared_ptr<FileMetaData> getFileMetaData() { return reader_->metadata(); }
...
class PARQUET_NO_EXPORT PrimitiveImpl : public ColumnReader::ColumnReaderImpl {
 public:
  PrimitiveImpl(MemoryPool* pool, std::unique_ptr<FileColumnIterator> input)
    ...
    record_reader_->setPageFilter(input_->getPageFilter());
 ...
 private:
  ...
  Status applyValueFilter(ValueFilter*, std::shared_ptr<Array>* data);
...
#define DEBUG_READ_ROWGROUP 1
#undef DEBUG_READ_ROWGROUP 
...
#ifdef DEBUG_READ_ROWGROUP
     std::cout << "Result array: elemnts=" << array->length()
               << std::endl;
#endif
...
#ifdef DEBUG_READ_ROWGROUP
   std::cout << "To read a column chunk for " << num_columns
             << " columns with nthreads=" << nthreads 
             << std::endl;
   parquet::StopWatch x;
   x.Start();
#endif
...
#ifdef DEBUG_READ_ROWGROUP
   std::cout << "Read done: total time(us)=" << x.Stop()
             << std::endl;
#endif
...
Status PrimitiveImpl::WrapIntoListArray(std::shared_ptr<Array>* array) {
  ...
  if (descr_->max_repetition_level() > 0) {
    ...
    // Esgyn extension
    std::shared_ptr<Buffer> selected_bitmap = nullptr;
    ...
    for (int64_t j = list_depth - 1; j >= 0; j--) {
      ...
      output = std::make_shared<::arrow::ListArray>(
          list_type, list_lengths[j], offsets[j], output, selected_bitmap, valid_bits[j], null_counts[j]);
    }
    *array = output;
  }
  return Status::OK();
...
#define TRANSFER_SKIP_VEC \
    PageFilter* pf = reader->getPageFilter(); \
    if ( pf ) {                               \
      (*out)->transferSkipVec(pf->getSkipVec());   \
    }
...
struct TransferFunctor {
  ...
  Status operator()(RecordReader* reader, MemoryPool* pool,
    ...
    std::shared_ptr<PoolBuffer> is_selected = reader->ReleaseIsSelected();
    ...
    if (reader->nullable_values()) {
      ...
      *out = std::make_shared<ArrayType<ArrowType>>(type, length, data, is_selected);
    }
...
struct TransferFunctor<ArrowType, ParquetType,
  ...
  Status operator()(RecordReader* reader, MemoryPool* pool,
    ...
    if (reader->nullable_values()) {
      ...
      *out = std::make_shared<ArrayType<ArrowType>>(type, length, values, is_selected);
    }
...
struct TransferFunctor<::arrow::BooleanType, BooleanType> {
  Status operator()(RecordReader* reader, MemoryPool* pool,
    ...
    if (reader->nullable_values()) {
      ...
      *out = std::make_shared<BooleanArray>(type, length, data, is_selected);
    }
...
// original transfer functor that converts 
// Int96 to arrow timestamp (int64)
template <>
struct TransferFunctor<::arrow::TimestampType, Int96Type> {
  Status operator()(RecordReader* reader, MemoryPool* pool,
    ...
    int64_t nsTM = 0;
    for (int64_t i = 0; i < length; i++) {
      RETURN_NOT_OK(impala_timestamp_to_nanoseconds(values[i], nsTM));
      *data_ptr++ = nsTM;
    }
    ...
    if (reader->nullable_values()) {
      ...
      *out = std::make_shared<TimestampArray>(type, length, data, is_selected);
    }
...
struct TransferFunctor<::arrow::Date64Type, Int32Type> {
  Status operator()(RecordReader* reader, MemoryPool* pool,
    ...
    if (reader->nullable_values()) {
      ...
      *out = std::make_shared<::arrow::Date64Array>(type, length, data, is_selected);
    }
...
struct TransferFunctor<
  ...
  Status operator()(RecordReader* reader, MemoryPool* pool,
    ...
    //
    // Do not transfer is_selected here since each section of the
    // bitmap is transferred during ReadValuesDense() and
    // ReadValuesSpaced() for ByteArrayType and FLBAType. See 
    // their implementations in record_reader.cc.
    //
    // The reader will crash if we do so since ResetValues() called
    // at the end of the above methods will reset values_written_ and
    // trigger the assert in ReleaseIsSelected().
    //
    //std::shared_ptr<PoolBuffer> is_selected = reader->ReleaseIsSelected();
    //reader->builder()->setIsSelected(is_selected);
...
struct TransferFunctor<::arrow::Decimal128Type, FLBAType> {
  Status operator()(RecordReader* reader, MemoryPool* pool,
    ...
    *out = std::make_shared<::arrow::Decimal128Array>(
        type, length, data, 
        fixed_size_binary_array.selected_bitmap(), // Essyn extension
        fixed_size_binary_array.null_bitmap(), null_count);
...
static Status DecimalIntegerTransfer(RecordReader* reader, MemoryPool* pool,
  ...
  if (reader->nullable_values()) {
    ...
    *out = std::make_shared<::arrow::Decimal128Array>(type, length, data, is_selected);
  }
  TRANSFER_SKIP_VEC;
  return Status::OK();
}
...
Status PrimitiveImpl::NextBatch(int64_t records_to_read, std::shared_ptr<Array>* out) {
  ...
  switch (field_->type()->id()) {
    ...
    case ::arrow::Type::TIMESTAMP: {
      ...
      // Esgyn Extension
      TRANSFER_CASE(TIME96, ::arrow::Time96Type, Int96Type)
    default:
      std::stringstream ss;
  ...
  // esgyn extension: filter values 
  ValueFilter* vf = input_->getValueFilter();
  if ( vf ) {
     RETURN_NOT_OK(applyValueFilter(vf, out));
  }

  return Status::OK();
}

#define CaseEval(arrowType, elementType, ArrayType) \
        case ::arrow::arrowType: \
        {                           \
         std::shared_ptr<::arrow::ArrayType>* array =  \
               reinterpret_cast<std::shared_ptr<::arrow::ArrayType>*>(data); \
         while (vf) { \
            TypedValueFilter<elementType>* typedVF =  \
                dynamic_cast<TypedValueFilter<elementType>*>(vf); \
            typedVF->eval(array); \
            vf = dynamic_cast<ValueFilter*>(vf->getNext());\
         } \
        } \
        break;

Status PrimitiveImpl::applyValueFilter(ValueFilter* vf, 
                                     std::shared_ptr<Array>* data) 
{
#ifdef NEED_INSTRUMENT_ARROW_INTERFACE
   int64_t currentTime = ::currentTimeInUsec();
#endif

   switch ( (*data)->type()->id() )
    {
     CaseEval(Type::INT8, ::arrow::Int8Type, NumericInt8ArrayType);
     CaseEval(Type::INT16, ::arrow::Int16Type, NumericInt16ArrayType);
     CaseEval(Type::INT32, ::arrow::Int32Type, NumericInt32ArrayType);
     CaseEval(Type::FLOAT, ::arrow::FloatType, NumericFloatArrayType);
     CaseEval(Type::DOUBLE, ::arrow::DoubleType, NumericDoubleArrayType);
     CaseEval(Type::INT64, ::arrow::Int64Type, NumericInt64ArrayType);
     CaseEval(Type::TIMESTAMP, ::arrow::Int64Type, NumericInt64ArrayType);
     CaseEval(Type::TIME96, ::arrow::Time96Type, NumericTime96ArrayType);
     CaseEval(Type::STRING, std::string, StringArray);
     CaseEval(Type::BOOL, ::arrow::BooleanType, BooleanArray);
     default:
       return Status::Invalid("Array data type is not handled");
       break;
    }

#ifdef NEED_INSTRUMENT_ARROW_INTERFACE
   (*data)->incValueFilterEvalTime(::currentTimeInUsec() - currentTime);
#endif
...
Status StructImpl::NextBatch(int64_t records_to_read, std::shared_ptr<Array>* out) {
  ...
  *out = std::make_shared<StructArray>(field()->type(), struct_length, children_arrays, selected_bitmap, 
