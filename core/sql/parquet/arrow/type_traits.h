...
// Esgyn Extension
template <>
struct TypeTraits<Time96Type> {
  using ArrayType = Time96Array;
  using BuilderType = Time96Builder;

  static inline int64_t bytes_required(int64_t elements) {
    return elements * sizeof(::parquet::Int96);
  }
  constexpr static bool is_parameter_free = true;

  static inline std::shared_ptr<DataType> type_singleton() { return time96(); }
};
