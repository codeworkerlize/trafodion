...
Status ArrayBuilder::Init(int64_t capacity) {
  ...
  // Esgyn extension
  is_selected_bitmap_ = std::make_shared<PoolBuffer>(pool_);
  RETURN_NOT_OK(is_selected_bitmap_->Resize(to_alloc));
  auto is_selected_data = is_selected_bitmap_->mutable_data();
  memset(is_selected_data, 0, static_cast<size_t>(byte_capacity));
...
Status ArrayBuilder::Resize(int64_t new_bits) {
  ...
  // Esgyn extension
  RETURN_NOT_OK(is_selected_bitmap_->Resize(new_bytes));
  if (old_bytes < new_bytes) {
    auto is_selected_data = is_selected_bitmap_->mutable_data();
    memset(is_selected_data + old_bytes, 0,
           static_cast<size_t>(byte_capacity - old_bytes));
  }
...
// This method assumes that the bitmap for selected contains
// all 0's, realized during ArrayBuilder::Init::Resize() and 
// ArrayBuilder::Init::Init(). 
void 
ArrayBuilder::appendToIsSelected(int64_t start, int64_t len, 
                                 skipVecType* skipVec)
{
   if ( !is_selected_bitmap_ )
     return;

   auto selected_bits = is_selected_bitmap_->mutable_data();
    
   if (!skipVec) {
      // If no skip vec, assume all bits are selected and 
      // set to 1.
      arrow::BitUtil::SetBits(selected_bits, start, start+len-1);
   } else {
      ...
      for (int i=0; i < skipVec->size(); i++) {
        ...
        if ( n > 0 ) {
          // Set the bits for the accumulated sections (e.g., pages)
          // that are selected by the predicate.
      ...
      // append
      skipVec_.insert(skipVec_.end(), skipVec->begin(), skipVec->end());
   }
}
...
Status PrimitiveBuilder<T>::FinishInternal(std::shared_ptr<ArrayData>* out) {
  ...
  *out = ArrayData::Make(type_, length_, {null_bitmap_, is_selected_bitmap_, data_}, null_count_);
...
Status AdaptiveIntBuilder::FinishInternal(std::shared_ptr<ArrayData>* out) {
  ...
  *out = ArrayData::Make(output_type, length_, {null_bitmap_, is_selected_bitmap_, data_}, null_count_);
...
Status BooleanBuilder::FinishInternal(std::shared_ptr<ArrayData>* out) {
  ...
  *out = ArrayData::Make(boolean(), length_, {null_bitmap_, is_selected_bitmap_, data_}, null_count_);
...
Status Decimal128Builder::FinishInternal(std::shared_ptr<ArrayData>* out) {
  ...
  *out = ArrayData::Make(type_, length_, {null_bitmap_, is_selected_bitmap_, data}, null_count_);
  return Status::OK();
}
...
Status ListBuilder::FinishInternal(std::shared_ptr<ArrayData>* out) {
  ...
  *out = ArrayData::Make(type_, length_, {null_bitmap_, nullptr, offsets}, null_count_);
...
Status BinaryBuilder::FinishInternal(std::shared_ptr<ArrayData>* out) {
  ...
  *out = ArrayData::Make(type_, length_, {null_bitmap_, is_selected_bitmap_, offsets, value_data}, null_count_,
                         0);
  Reset();
  return Status::OK();
}
...
Status StructBuilder::FinishInternal(std::shared_ptr<ArrayData>* out) {
  *out = ArrayData::Make(type_, length_, {null_bitmap_, nullptr}, null_count_);
