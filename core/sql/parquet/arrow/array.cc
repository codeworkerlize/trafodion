...
PrimitiveArray::PrimitiveArray(const std::shared_ptr<DataType>& type, int64_t length,
                               ...
                               int64_t null_count, int64_t offset) {
  SetData(ArrayData::Make(type, length, {null_bitmap, selected_bitmap, data}, null_count, offset));
}
...
template <typename T>
void NumericArray<T>::display(std::ostream& out, int64_t maxNumValues)
{
   int64_t ct = 0;
   const value_type* raw = raw_values();
...
template <typename T>
void NumericArray<T>::scan()
{
   const value_type* raw = raw_values();
   for (size_t j = 0; j < Array::length(); ++j ) {
      raw[j]; // access the value
   }
}
...
BooleanArray::BooleanArray(int64_t length, const std::shared_ptr<Buffer>& data,
                           ...
                           int64_t null_count,
                           int64_t offset)
    : PrimitiveArray(boolean(), length, data, selected_bitmap, null_bitmap,  null_count, offset) {}
...
ListArray::ListArray(const std::shared_ptr<DataType>& type, int64_t length,
                     ...
                     int64_t offset) {
  auto internal_data =
      ArrayData::Make(type, length, {null_bitmap, selected_bitmap, value_offsets}, null_count, offset);
...
Status ListArray::FromArrays(const Array& offsets, const Array& values, MemoryPool* pool,
  ...
  if (offsets.null_count() > 0) {
    ...
    buffers.emplace_back(nullptr); // for selected_bitmap
...
void ListArray::SetData(const std::shared_ptr<ArrayData>& data) {
  ...
  auto value_offsets = data->buffers[LA_VALUE_OFFSETS];
...
void BinaryArray::SetData(const std::shared_ptr<ArrayData>& data) {
  DCHECK_EQ(data->buffers.size(), 4);
  auto value_offsets = data->buffers[BA_VALUE_OFFSETS];
  auto value_data = data->buffers[BA_VALUE_DATA];
  this->Array::SetData(data);
...
BinaryArray::BinaryArray(int64_t length, const std::shared_ptr<Buffer>& value_offsets,
                         ...
                         int64_t null_count,
                         int64_t offset)
    : BinaryArray(binary(), length, value_offsets, data, selected_bitmap, null_bitmap, null_count,
                  offset) {}
...
BinaryArray::BinaryArray(const std::shared_ptr<DataType>& type, int64_t length,
                         ...
                         int64_t null_count,
                         int64_t offset) {
  SetData(ArrayData::Make(type, length, {null_bitmap, selected_bitmap, value_offsets, data}, null_count,
                          offset));
}
...
StringArray::StringArray(int64_t length, const std::shared_ptr<Buffer>& value_offsets,
                         ...
                         int64_t null_count,
                         int64_t offset)
    : BinaryArray(utf8(), length, value_offsets, data, selected_bitmap, null_bitmap, null_count, offset) {}
...
FixedSizeBinaryArray::FixedSizeBinaryArray(const std::shared_ptr<DataType>& type,
                                           ...
                                           const std::shared_ptr<Buffer>& null_bitmap,
                                           int64_t null_count, int64_t offset)
    : PrimitiveArray(type, length, data, selected_bitmap, null_bitmap, null_count, offset),
...
StructArray::StructArray(const std::shared_ptr<DataType>& type, int64_t length,
                         ...
                         int64_t null_count,
                         int64_t offset) {
  SetData(ArrayData::Make(type, length, {null_bitmap, selected_bitmap}, null_count, offset));
...
Status StructArray::Flatten(MemoryPool* pool, ArrayVector* out) const {
  ...
  std::shared_ptr<Buffer> null_bitmap = data_->buffers[NULL_BITMAP];
  ...
  for (auto& child_data : data_->child_data) {
    ...
    std::shared_ptr<Buffer> child_null_bitmap = child_data->buffers[NULL_BITMAP];
    ...
    flattened_data->buffers[NULL_BITMAP] = flattened_null_bitmap;
    flattened_data->buffers[SELECTED_BITMAP] = nullptr;
...
void UnionArray::SetData(const std::shared_ptr<ArrayData>& data) {
  ...
  auto type_ids = data_->buffers[UA_TYPE_IDS];
  auto value_offsets = data_->buffers[UA_VALUE_OFFSETS];
  raw_type_ids_ =
...
UnionArray::UnionArray(const std::shared_ptr<DataType>& type, int64_t length,
  ...
  auto internal_data = ArrayData::Make(
      type, length, {null_bitmap, selected_bitmap, type_ids, value_offsets}, null_count, offset);
  for (const auto& child : children) {
...
int64_t Array::totalValuesSelected() 
{
   return selectedBitmapReader_.totalBitsSet();
}

bool Array::NextSelected(int64_t& pos) 
{
   return selectedBitmapReader_.NextBitSet(pos);
}
...
template class NumericArray<Time96Type>;
