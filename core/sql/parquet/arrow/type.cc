...
// Esgyn Extension
Time96Type::Time96Type(TimeUnit::type unit) : TimeType(Type::TIME96, unit) {
  DCHECK(unit == TimeUnit::NANO)
      << "Must be nanoseconds";
}

std::string Time96Type::ToString() const {
  std::stringstream ss;
  ss << "time96[" << this->unit_ << "]";
  return ss.str();
}
...
TYPE_FACTORY(time96,  Time96Type);
...
std::shared_ptr<DataType> time96(TimeUnit::type unit) {
  return std::make_shared<Time96Type>(unit);
}
