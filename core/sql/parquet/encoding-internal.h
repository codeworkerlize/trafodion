...
class PlainDecoder : public Decoder<DType> {
 public:
  ...
  virtual void SetData(int num_values, const uint8_t* data, int len, bool skip = false) {
...
inline int DecodePlain(const uint8_t* data, int64_t data_size, int num_values,
                       int type_length, T* out, bool skip) {
  ...
  if ( !skip ) {
     ...
     memset(out, 0, bytes_to_decode);
#ifdef DEBUG_SKIP_DECODE
     std::cout << "<typename T>::DecodePlain(): skip " 
...
inline int DecodePlain<ByteArray>(const uint8_t* data, int64_t data_size, int num_values,
                                  int type_length, ByteArray* out, bool skip) {
  int bytes_decoded = 0;
  int increment;
  if ( !skip ) {
     ...
     //
     // Esgyn extension. Skip the fill of actual data to out[].
     // Each element stats with a length in int32_t, followed by 
     // # of byte in length], Data contains <n> elements.
     //
     // The part to compuate the bytes_decoded is still expensive though.
     for (int i = 0; i < num_values; ++i) {
       out[i].len = 0;
       out[i].ptr = nullptr;
...
#ifdef DEBUG_SKIP_DECODE
     std::cout << "template <>::Decode<ByteArray>(): " 
               << ", values skipped=" << num_values
               << ", bytes_decoded=" << bytes_decoded << std::endl;
#endif
  }
...
inline int DecodePlain<FixedLenByteArray>(const uint8_t* data, int64_t data_size,
                                          ...
                                          FixedLenByteArray* out, bool skip) {
...
#ifdef DEBUG_SKIP_DECODE
     std::cout << "template <>::Decode<FixedLenByteArray>(): " 
               << ", values skipped=" << num_values
               << ", bytes_to_decode=" << bytes_to_decode << std::endl;
#endif
  }
  return bytes_to_decode;
}
...
inline int PlainDecoder<DType>::Decode(T* buffer, int max_values) {
  ...
  int bytes_consumed = DecodePlain<T>(data_, len_, max_values, type_length_, buffer, skip_);
 ...
 public:
  ...
  int Decode(uint8_t* buffer, int max_values) {
    ...
    if ( !skip_ ) {
       ...
       // To skip max_values, just do nothing.
#ifdef DEBUG_SKIP_DECODE
       std::cout << "<>::PlainDecode<BooleanType>::Decode(uint8_t*, int): skip " 
                 << max_values << " values" << std::endl;
#endif
    }
  ...
  virtual int Decode(bool* buffer, int max_values) {
    ...
    if ( !skip_ ) {
       ...
       // To skip max_values, just do nothing.
#ifdef DEBUG_SKIP_DECODE
       std::cout << "<>::PlainDecode<BooleanType>::Decode(bool*, int): skip " 
                 << max_values << " values" << std::endl;
#endif
    }
...
class DictionaryDecoder : public Decoder<Type> {
 public:
  ...
  void SetData(int num_values, const uint8_t* data, int len, bool skip=false) override {
    num_values_ = num_values;
    if (len == 0) return;
  ...
  int Decode(T* buffer, int max_values) override {
    ...
    int decoded_values = max_values;

    // Esgyn performance extension: skip decoding if the page is skipped 
    if ( !skip_ ) {
       decoded_values = 
...
#ifdef DEBUG_SKIP_DECODE
      std::cout << "DictionaryDecoder::Decode(): skip " 
                << max_values << " values" << std::endl;
#endif
    }
...
#ifdef DEBUG_SKIP_DECODE
      std::cout << "DictionaryDecoder::DecodeSpaced(): skip " 
                << num_values << " values" << std::endl;
#endif
    }
