...
inline bool operator==(const ByteArray& left, const ByteArray& right) {
  ...
  return left.len == right.len && 0 == memcmp(left.ptr, right.ptr, left.len);
}
...
MANUALLY_ALIGNED_STRUCT(1) Int96 { 
 ...
 Int96()
  { value[2] = 0; *(int64_t*)value = 0; }

 Int96(int32_t x, int64_t y)
  { value[2] = x; *(int64_t*)value = y; }

 Int96(const Int96& x)
  { value[2] = x.value[2]; *(int64_t*)value = *(int64_t*)x.value; }

 // Esgyn extension, Parquet Timestamp comparison semantics
 //  value[2] : days since Julian epoch time
 //  value[0,1] : ns as int64_t
 bool operator<(const Int96& x)
  { return ( (value[2] < x.value[2]) ||
            ( value[2] == x.value[2] &&
            *(int64_t*)value < *(int64_t*)x.value )); }
...
static inline std::ostream& operator<<(std::ostream& os, const Int96& x) 
{ os << Int96ToString(x); return os; }
