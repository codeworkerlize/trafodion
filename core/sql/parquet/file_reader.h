...
class PARQUET_EXPORT ParquetFileReader {
 public:
  ...
  PageFilter* getPageFilter(int i)
  { 
     if ( !filters_ || i>=filters_->size() )
       return NULL;
     else
       return (*filters_)[i].getPageFilter(); 
  }

  ValueFilter* getValueFilter(int i)
  { 
     if ( !filters_ || i>=filters_->size() )
       return NULL;
     else
       return (*filters_)[i].getValueFilter();
  }

  void setFilters(std::vector<Filters>* filters)
  {   filters_ = filters; }
