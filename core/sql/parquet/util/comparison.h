...
class PARQUET_EXPORT Comparator {
 public:
  ...
  // Esgyn extension: answer the question whether a comparison 
  // is supported for a combination of descr->sort_order() and
  // descr->physical_type(), derived from the above Make() method.
  // The comparison is used to order column values when computing
  // the column stats.
  static bool isSupported(const ColumnDescriptor* descr);
};
