...
ScratchSpace::ScratchSpace(CollHeap *heap, SortError *sorterror, Lng32 blocksize, Int32 scratchIOVectorSize,
  ...
  scrFilesMap_ = new (heap_) ScratchFileMap(heap_, sorterror,
                                            FALSE,  // Break Enabled?
                                            MAXSCRFILES);
...
void ScratchSpace::setCallingTcb(ex_tcb *tcb) {
  ...
  if (callingTcb_ != NULL) {
    ...
    if (stat) bmoStats_ = stat->castToExBMOStats();
  }
  if (bmoStats_ != NULL) bmoStats_->setScratchIOSize((scratchIOVectorSize_ * blockSize_) >> 10);
}
...
/* Scenarios:
  ...
  1.	Sort operator encounters ENOSPC error during write operation.
    a.ScratchSpace::WriteFile() encounters  this error and createsANewScratchFile() is invoked that takes of writing to
  new scratch file.
  ...
  2.	Sort operator overflow with vector partially filled and no more records to overflow.
    ...
    b.	Sort uses double buffer, each of size 512kb. Before each buffer gets reused, any Io on that buffer is completed
  forcibly.

  3.	Subsequent writes encounter 2gb file limit in scratchspace. Triggers creating a new scratch file.          How
  does sort react? a.	ScratchSpace::WriteFile() encounters  this and createsANewScratchFile(). b.	Because, vector
  for sort is set to 1, followed by double buffer being written serially, there is no      scenario of completing IO on
  previous file. Hash:
  ------
  ...
  6.	Hash operator partially fills vector and no more records to overflow.
    a.	Hash operator follows the protocol of invoking scratchSpace::checkio(checkAll) when switching between write and
  read  operations. This causes any partially filled vector to be flushed out to disk and complete any pending writes.
  Any ENOSPC errors encountered during this operation becomes scenario 5.

  7.	Vector is partially filled, however subsequent writes encounter self imposed 2gb file limit in scratchspace.
  Triggers creating a new scratch file. What happens to partially filled vector? a.	ScratchSpace::WriteFile()
  encounters this check. CreateANewScratchFile() is invoked which will accept the buffer as new vector element in new
...
RESULT ScratchSpace::CreateANewScrFileAndWrite(char *buffer, Int32 blockNum, UInt32 blockLen, NABoolean waited) {
  ...
  currentIOScrFile_ = currentWriteScrFile_ =
      scrFilesMap_->createNewScrFile(this, scratchMgmtOption_, scratchMaxOpens_, preAllocateExtents_, asyncReadQueue_);
  ...
  if (tempRefScrFile != NULL && tempRefScrFile->lastError() == ENOSPC) {
    ...
    // ScratchSpace::CheckIO(CheckAll)
    retval = currentWriteScrFile_->writeBlock(fileIndex, buffer, blockLen, iowaittime, blockNum, NULL, waited);
  }
  ...
  if (bmoStats_) bmoStats_->incScratchBufferBlockWritten();
  ...
  if (retval == IO_NOT_COMPLETE)
    return retval;
  else {  // The write to the new scratch file was successful
...
RESULT ScratchSpace::writeThru(char *buf, ULng32 len, DWORD &blockNum) {
  ...
  if (currentWriteScrFile_ != NULL) {
    ...
    if (retval) {
      ...
      if ((sortError_->getSysError() == 45) || (sortError_->getSysError() == 43)) {
        retval = FILE_FULL;
  ...
  if ((currentWriteScrFile_ != NULL) && (retval != FILE_FULL) && (fileIndex >= 0)) {
    ...
    totalIoWaitTime_ += iowaittime;
    if (retval == SCRATCH_SUCCESS && bmoStats_) bmoStats_->incScratchBufferBlockWritten();
    ...
    if (retval == SCRATCH_FAILURE) {
      if (sortError_->getSysError() == ENOSPC) retval = FILE_FULL;
    }
  }
  ...
  if ((currentWriteScrFile_ == NULL) || (retval != SCRATCH_SUCCESS)) {
    switch (retval) {
      ...
      case FILE_FULL:
        ...
        if (retval == IO_NOT_COMPLETE) return IO_NOT_COMPLETE;
...
RESULT ScratchSpace::readThru(char *buf,
  ...
  if (readScratchFile != NULL) {
    ...
    currentIOScrFile_ = currentReadScrFile_ = scrFilesMap_->mapBlockNumToScrFile(blockNum, blockOffset);
  ...
  retval = currentReadScrFile_->readBlock(fileIndex, buf, blockSize_, iowaittime, 0 /* asynchronous read */);
  if (retval != SCRATCH_SUCCESS) {
    return retval;
  }  // read returns error or eof or IO_NOT_COMPLETE
  totalIoWaitTime_ += iowaittime;
  if (bmoStats_) bmoStats_->incScratchBufferBlockRead();
...
RESULT ScratchSpace::writeFile(char *block, UInt32 blockNum, UInt32 blockLen) {
  ...
  if (currentWriteScrFile_ != NULL) {  // assume writing to the current write file
    ...
    SBN firstBlockNum = scrFilesMap_->getFirstScrBlockNum(currentWriteScrFile_);
    if (firstBlockNum == -1) {
      sortError_->setErrorInfo(EScrFileNotFound  // sort error
                               ,
                               0  // syserr: the actual FS error
  ...
  if (currentWriteScrFile_ != NULL && retval != FILE_FULL && (fileIndex >= 0)) {  //?? added if
    retval = currentWriteScrFile_->writeBlock(fileIndex, block, blockLen, iowaittime, blockNum, NULL, FALSE);
    if (retval == SCRATCH_SUCCESS && bmoStats_) bmoStats_->incScratchBufferBlockWritten();
    totalIoWaitTime_ += iowaittime;
    ...
    if (tempFile != NULL) {
      ...
      for (Int32 index = 0; index < scratchMaxOpens_; index++) {
        if (!tempFile->fileHandle_[index].IOPending) {
          ...
          if ((retval != SCRATCH_SUCCESS) && (retval != IO_NOT_COMPLETE)) return retval;
      ...
      for (Int32 index = 0; index < scratchMaxOpens_; index++) {
        ...
        if ((retval == WRITE_EOF) || (retval == FILE_FULL)) {
          retval =
              CreateANewScrFileAndWrite(tempFile->fileHandle_[index].IOBuffer, tempFile->fileHandle_[index].blockNum,
        ...
        if (retval == IO_NOT_COMPLETE) continue;
        if ((retval != IO_COMPLETE) && (retval != SCRATCH_SUCCESS)) return retval;
        ...
        if (tempFile->isVectorPartiallyFilledAndPending(index)) {
          if (tempFile->executeVectorIO() != SCRATCH_SUCCESS) {
            if (sortError_->getSysError() == ENOSPC) {
              ...
              if ((retval != SCRATCH_SUCCESS) && (retval != IO_NOT_COMPLETE) && (retval != IO_COMPLETE)) return retval;
...
void ScratchSpace::close(void) { scrFilesMap_->closeFiles(); }
...
//
//-----------------------------------------------------------------------
DiskPool *ScratchSpace ::getDiskPool() { return diskPool_; }
...
NABoolean ScratchSpace::generateDiskTable(SortError *sorterror) {
  ...
  char diskPattern[8];
  strcpy(diskPattern, "*");
  retvalue = diskPool_->generateDiskTable(scratchDirListSpec_, numDirsSpec_, diskPattern);
  ...
  if (diskPool_->printDiskTable() == SORT_FAILURE) {
    return SORT_FAILURE;
  }
#endif
...
//
//-----------------------------------------------------------------------
void ScratchSpace::configure(const ExExeStmtGlobals *stmtGlobals, ExSubtask *ioEventHandler,
...
ScratchFileMap *ScratchSpace::getScrFilesMap() const { return scrFilesMap_; }
...
Int16 ScratchSpace::getLastSqlCode(void) {
  Int16 exeError = 0;
...
RESULT SortScratchSpace::flushRun(NABoolean endrun, NABoolean waited) {
  ...
  if ((retval != SCRATCH_SUCCESS) && (retval != IO_COMPLETE)) return retval;
  ...
  retval = ScratchSpace::writeFile(currentBlock_, blockHead_.thisBlockNum_, blockSize_);
...
RESULT SortScratchSpace::initiateSortMergeNodeRead(SortMergeNode *sortMergeNode, NABoolean waited) {
  ...
  if (sortMergeNode->nextIOBlockNum_ > sortMergeNode->endBlockNum_) return SCRATCH_SUCCESS;

  mbTemp = sortMergeNode->readQHead_;
  while (mbTemp != NULL) {
    if ((mbTemp->state_ == QUEUED) || (mbTemp->state_ == READING)) numOutStandingIO++;
  ...
  for (Int32 count = 0;
    ...
    mb->currentScrFile_ = scrFilesMap_->mapBlockNumToScrFile(sortMergeNode->nextIOBlockNum_, blockOffset);
    ...
    if ((retval != SCRATCH_SUCCESS) && (retval != IO_COMPLETE) && (retval != IO_NOT_COMPLETE)) {
      return retval;
    }
  }  // for loop
...
RESULT SortScratchSpace::readSortMergeNode(SortMergeNode *sortMergeNode, char *&rec, ULng32 reclen, ULng32 &actRecLen,
  ...
  if (sortMergeNode->numRecsRead_ == (sortMergeNode->blockHead_).numRecs_) {
    ...
    if (sortMergeNode->blockHead_.nextBlockNum_ == -1)
      ...
      if (sortMergeNode->readQHead_ &&
          sortMergeNode->readQHead_->state() ==
              BEING_CONSUMED) {  // State begins with Queued, reading, readcomplete,being_consumed, idle
      ...
      if ((retval != IO_COMPLETE) && (retval != SCRATCH_SUCCESS)) return retval;  // Possible to return IO_NOT_COMPLETE.

      totalIoWaitTime_ += ioWaitTime;

      // set up data structures for returning rows; return first row

      memcpy(&sortMergeNode->blockHead_, sortMergeNode->readQHead_->scrBlock_, sizeof(ScrBlockHeader));

      sortMergeNode->nextReadPosition_ = sortMergeNode->readQHead_->scrBlock_ + OVERHEAD;
...
SortMergeBuffer *SortScratchSpace::getFreeSortMergeBuffer(void) {
  ...
  if (freeSortMergeBufferPool_) freeSortMergeBufferPool_ = freeSortMergeBufferPool_->mnext_;
...
RESULT HashScratchSpace::writeThru(char *buf, UInt32 clusterID) {
  RESULT retval = WRITE_EOF;
  ...
  retval = ScratchSpace::writeFile(buf, totalNumOfScrBlocks_, blockSize_);
...
RESULT HashScratchSpace::readThru(char *buf, UInt32 clusterID, ClusterPassBack *cPassBack) {
  ...
  retval = ScratchSpace::readThru(buf, blockNumToRead, blockSize_, readScratchFile, readBlockOffset);
...
RESULT HashScratchSpace::checkIORead(ClusterPassBack *cPassBack, UInt32 clusterID) {
  ...
  if (retval == IO_COMPLETE) {
    if (sFile->isNewVecElemPossible(blockOffset * blockSize_, blockSize_)) {
      return IO_COMPLETE;
    } else {
...
RESULT HashScratchSpace::checkIOAll(void) { return ScratchSpace::checkIO(NULL, TRUE); }
...
DWORD HashScratchSpace::getClusterBlockNum(UInt32 clusterID, ClusterPassBack *cPassBack, NABoolean getCurrentBlock) {
  ...
  } else {  // cpassback->cblock is NULL means, we need to search from the
  ...
  if (getCurrentBlock && (Int32)blockNumToRead > 0) {
  ...
  } else if (cPassBack->cBlock_ != NULL) {  // store the scratch File of next cpassback for use in checkio.
    ...
    cPassBack->scratchFile_ = scrFilesMap_->mapBlockNumToScrFile(cPassBack->cBlock_->blockNum, cPassBack->blockOffset_);

    ex_assert(cPassBack->scratchFile_ != NULL, "HashScratchSpace::getClusterBlockNum, block number is invalid1");
  ...
  if (currentCDir_ == NULL) {
    ...
    for (index = 0; index < numClusters_; index++) {
      if (cdtemp[index].clusterID == clusterID) {
        ...
        currentCDir_->last = temp;
        break;
      } else if (cdtemp[index].clusterID == 0) {  // new clusterID received, probably HJ can tell us to optimize.
        currentCDir_ = &cdtemp[index];
        currentCDir_->clusterID = clusterID;
        CBlock *temp = (CBlock *)heap_->allocateMemory(sizeof(CBlock), FALSE);
        if (temp == NULL) {
    ...
    {
      ClusterDirectory *tempClusterDList_ =
          (ClusterDirectory *)heap_->allocateMemory(numClusters_ * 2 * sizeof(ClusterDirectory), FALSE);
...
SortMergeNode::SortMergeNode(Lng32 associatedrun, SortScratchSpace *sortScratchSpace)
  ...
  blockHead_.thisBlockNum_ = blockHead_.nextBlockNum_ = blockHead_.bytesUsed_ = blockHead_.runNum_ =
      blockHead_.numRecs_ = 0;
};
...
RESULT SortMergeNode::checkIO(Int64 &ioWaitTime, NABoolean waited) {
  ...
  if (readQHead_->state_ == QUEUED) {
    ...
    if (!waited && ((readQHead_->state_ == QUEUED) || (readQHead_->state_ == READING))) return IO_NOT_COMPLETE;
  }
  ...
  if (readQHead_->state_ == READING) {
    for (Int32 index = 0; index < readQHead_->currentScrFile_->getNumOpens(); index++) {
      ...
      if (readQHead_->state_ != READING) break;
    }
    if (readQHead_->state_ == READING) return IO_NOT_COMPLETE;
  }
...
void SortMergeBuffer::processMisc(void) { sortMergeNodeRef_->numOutStandingIO_--; };
