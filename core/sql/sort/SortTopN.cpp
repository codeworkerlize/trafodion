...
Lng32 SortTopN::sortSend(void *rec, ULng32 len, void *tupp) {
  ...
  if (!isHeapified_) {
    ...
    topNKeys_[loopIndex_].key_ = newRec->extractKey(keySize_, sortUtil_->config()->numberOfBytesForRecordSize());
...
void SortTopN::insertRec(void *rec, ULng32 len, void *tupp) {
  ...
  insertRecKey_.key_ = newrec->extractKey(keySize_, sortUtil_->config()->numberOfBytesForRecordSize());
...
UInt32 SortTopN::getOverheadPerRecord(void) { return (sizeof(RecKeyBuffer) + sizeof(Record)); }
