                               ...
                               int blockSize, Int64 lobMaxSize, ExLobGlobals *lobGlobals)
{
  ...
  useHdfsWriteLock_ = lobGlobals->useHdfsWriteLock();
...
Ex_Lob_Error ExLob::getDesc(ExLobDesc &desc,char * handleIn, Int32 handleInLen, char *blackBox, Int32 *blackBoxLen, Int64 transId) 
{
    Ex_Lob_Error err = LOB_OPER_OK; 
...
{ 
    ...
    if (! useLibHdfs_ ) {
      offset = hdfsClient_->hdfsWriteImmediate(
           data, size, hdfsClientRetcode, TRUE, useHdfsWriteLock_, hdfsWriteLockTimeoutInSecs_ * 1000); 
...
Ex_Lob_Error ExLob::readSourceFile(char *srcfile, char *&fileData, Int64 &size,
                                   Int64 offset, Int32 *readDetailError)
 {
...
Ex_Lob_Error ExLob::getLength(char *handleIn, Int32 handleInLen,Int64 &outLobLen,LobsSubOper so, Int64 transId)
{
  ...
  Int64 descSyskey;
  Int64 descTS = -1;
  ExpLOBoper::extractFromLOBhandle(NULL, &lobType, &lobNum, &uid,
		       &descSyskey, NULL, //descTS, 
		       NULL, NULL,
		       handleIn);
  if (lobType == Lob_Inline)
    {
      outLobLen = ExpLOBoper::lobHandleGetInlineDataLength(handleIn);
    }
  if (so != Lob_External_File)
    {
      
      cliErr = SQL_EXEC_LOBcliInterface(handleIn, handleInLen,NULL,NULL,NULL,NULL,
                                        LOB_CLI_SELECT_LOBLENGTH,LOB_CLI_ExecImmed, 
                                        0,&outLobLen, 0, 0,0,transId,lobTrace_,NULL);
  ...
  if (so != Lob_External_File)
    {
      ...
      cliErr = SQL_EXEC_LOBcliInterface(handleIn, handleInLen,NULL,NULL,NULL,NULL,
                                        LOB_CLI_SELECT_LOBOFFSET,LOB_CLI_ExecImmed,&outLobOffset,
                                        0, 0, 0,0,transId,lobTrace_,NULL);
...
Ex_Lob_Error ExLob::writeDesc(Int64 &sourceLen, char *source, LobsSubOper subOper, 
                              Int64 &descNumOut, Int64 &dataOffset, Int64 lobMaxSize, 
                              char * handleIn, Int32 handleInLen, 
                              char *blackBox, Int32 *blackBoxLen, char *handleOut, Int32 &handleOutLen, 
                              Int64 xnId, void *lobGlobals)
{
  Ex_Lob_Error err=LOB_OPER_OK; 
...
Ex_Lob_Error ExLob::readToMem(char *memAddr, Int64 size,  Int64 &operLen,char * handleIn, Int32 handleInLen, char *blackBox, Int32 blackBoxLen, char * handleOut, Int32 &handleOutLen, Int64 transId)
{
   ...
   //If its inline return the data from the handle itself
   Int64 uid;
   Lng32 lobType;
   Lng32 lobNum;
   Int64 descKey;
   ...
   ExpLOBoper::extractFromLOBhandle(&flags, &lobType, &lobNum, &uid,
    ...
    if (lobType == Lob_Inline)
      {
        Int32 returnReadLen=0;
        ExpLOBoper::lobHandleReadData(size,memAddr,handleIn,returnReadLen);
        operLen = returnReadLen;
        return err;
      }
   err = getDesc(desc,handleIn,handleInLen,blackBox, &blackBoxLen,transId);
   if (err != LOB_OPER_OK)
     {	   
       return err;
     }
                                 ...
                                 Int32 &handleOutLen,LobsSubOper so, Int64 xnId,void *lobGlobals)
{
  ...
  if (so == Lob_External_Lob)
    {
      ...
      err = getDesc(desc,source,sourceLen,blackBox,&blackBoxLen,xnId);
      if (err != LOB_OPER_OK)
        return LOB_DESC_READ_ERROR;
      // we have received the external data file name from the descriptor table
      // replace the contents of the lobDataFile with this name      
  ...
  cliRC = SQL_EXEC_LOBcliInterface(source, sourceLen,
                                   NULL,NULL,NULL,NULL,
                                   LOB_CLI_SELECT_LOBLENGTH,LOB_CLI_ExecImmed,
                                   0,&inputLobDataLen, &dummy, &dummy,0,0,
                                   FALSE,NULL);
  if (cliRC < 0)
    {
  ...
  /* if (handleOutLen > 0)
    {
      ...
      ExpLOBoper::updLOBhandleDescKey(descSyskey, handleIn); 
      }*/
...
Ex_Lob_Error ExLob::append(char *data, Int64 size, LobsSubOper so, Int64 headDescNum, Int64 &operLen, Int64 lobMaxSize,Int64 lobMaxChunkMemSize, Int64 lobGCLimit, char *handleIn, Int32 handleInLen,  char * handleOut, Int32 &handleOutLen,Int64 xnId,void *lobGlobals)
{
   ...
   // Check for max limit
   Int64 currLobSize = 0;
   err = getLength(handleIn, handleInLen,currLobSize,so,0);
   if (err != LOB_OPER_OK)
     return err;
   if (currLobSize+inputSize > lobMaxSize)
     {
...
Ex_Lob_Error ExLob::dropLob()
{
    char logBuf[4096];
    if (! useLibHdfs_) { // use JNI/Java
       HDFS_Client_RetCode hdfsClientRetcode;
       hdfsClientRetcode = HdfsClient::hdfsDeletePath(lobDataFile_.data());
       if (hdfsClientRetcode != HDFS_CLIENT_OK)
	  return LOB_DATA_FILE_DELETE_ERROR;
       
       // lob location could be the root storage dir (/user/trafodion/lobs)
       // or it could include tenant ID (/user/trafodion/lobs/TRAF_*).
       // In these cases, do not do anything else.
       // Otherwise it is a user table based location and is empty,
       // delete that location.
       // 
       size_t lastSlash = lobStorageLocation_.last('/');
       if ((lobStorageLocation_ != COM_LOB_STORAGE_ROOT_DIR) &&
           (lobStorageLocation_(lastSlash+1,5) != TRAF_NAMESPACE_PREFIX))
         {
           Lng32 numFiles = -1;
           hdfsClientRetcode = HdfsClient::getNumFiles((char*)lobStorageLocation_.data(),
                                                       TRUE, // get all files
                                                       numFiles);
           if ((hdfsClientRetcode == HDFS_CLIENT_OK) &&
               (numFiles == 0))
             {
               hdfsClientRetcode = HdfsClient::hdfsDeletePath(lobStorageLocation_.data());
               if (hdfsClientRetcode != HDFS_CLIENT_OK)
                 return LOB_DATA_FILE_DELETE_ERROR;
             }
           else if (hdfsClientRetcode != HDFS_CLIENT_OK)
     ...
     if (hdfsDelete(fs_, lobDataFile_.data(), 0) != 0)
       {
         ...
         lobDebugInfo("In ExLob::dropLob",0,__LINE__,lobTrace_);
	 return LOB_DATA_FILE_DELETE_ERROR;
       }
...
Ex_Lob_Error ExLob::purgedataLob()
{
  if (!HdfsClient::hdfsTruncateFile(lobDataFile_.data()))
    return LOB_DATA_FILE_DELETE_ERROR;
...
ExLobDesc::ExLobDesc(Int64 offset, Int64 size) :
    dataOffset_(offset),
    dataSize_(size)
{
}
...
Ex_Lob_Error ExLob::openLocalFileToWrite(char *fileName, Int32 fileflags,
                                         int &fdDestFile)
{
  // open the target file for writing 
  int filePerms = S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH;
  int openFlags = O_RDWR |O_DSYNC; // O_DIRECT needs mem alignment
  ...
  if (fdDestFile == -1) 
    {
      if (((LobTgtFileFlags)fileflags == Lob_Append_Or_Error) ||
        ...
        {
          openFlags = O_CREAT | O_RDWR | O_SYNC ;
          fdDestFile = open(fileName, openFlags, filePerms);
          if (fdDestFile == -1)	  
            return LOB_TARGET_FILE_OPEN_ERROR; 
        }
...
Ex_Lob_Error ExLob::writeToLocalFile(int &fdDestFile, char *lobData,
                                     Int64 chunkSize, Int64 &tgtOffset,
                                     Int64 &writeOperLen)
{
  int fserr = 0;
  ...
  if (writeOperLen <= 0)
    {
      closeLocalFile(fdDestFile);
      return LOB_TARGET_FILE_WRITE_ERROR;
    }   

  fserr = fsync(fdDestFile);
  if(fserr)
    {
      closeLocalFile(fdDestFile);
      return LOB_TARGET_FILE_WRITE_ERROR;
...
Ex_Lob_Error ExLob::closeLocalFile(int &fdDestFile)
{
  if (fdDestFile < 0)
    return LOB_OPER_OK;
...
Ex_Lob_Error ExLob::readDataToLocalFile(char *fileName,  Int64 offset, Int64 size, Int64 &writeOperLen, 
                                        Int64 lobMaxChunkMemSize, Int32 fileflags,char *handleIn,Int32 handleInLen, 
                                        NABoolean multipleChunks,Int64 transId)
{ 
    Ex_Lob_Error err;
    Int64 operLen = 0;
    ...
    int fdDestFile = -1;
    err = openLocalFileToWrite(fileName, fileflags, fdDestFile);
    if (err != LOB_OPER_OK)
      return err;
	...
	if (err != LOB_OPER_OK)
	  {
        ...
        err = writeToLocalFile(fdDestFile, lobData, srcLen, tgtOffset,
                               writeOperLen);
	getLobGlobalHeap()->deallocateMemory(lobData);
        if (err != LOB_OPER_OK)
          return err;
	...
	while ( srcLen > 0)
	  {
		...
		return LOB_DATA_READ_ERROR;
            ...
            err = writeToLocalFile(fdDestFile, lobData, chunkSize, tgtOffset,
                                   writeOperLen);
	    getLobGlobalHeap()->deallocateMemory(lobData);
            if (err != LOB_OPER_OK)
              return err;
...
Ex_Lob_Error ExLob::openHdfsFileToWrite(char *fileName, Int32 fileflags,
                                        HdfsClient* &tgtHdfsClient)
{
  ...
  ex_assert(hdfsClientRetcode == HDFS_CLIENT_OK, "Internal error: HdfsClient::newInstance returned an error");
  if (tgtHdfsClient->hdfsCreate(fileName, overwrite, append, FALSE) !=  HDFS_CLIENT_OK)
    return LOB_TARGET_FILE_OPEN_ERROR; 

  return LOB_OPER_OK;
}

Ex_Lob_Error ExLob::writeToHdfsFile(HdfsClient* &tgtHdfsClient, char *lobData,
                                    Int64 chunkSize, Int64 lobMaxChunkMemLen,
                                    Int64 &writeOperLen)
{
  if (! tgtHdfsClient)
    return LOB_TARGET_FILE_WRITE_ERROR;
  ...
  HDFS_Client_RetCode hdfsClientRetcode;
  writeOperLen += tgtHdfsClient->hdfsWrite(lobData, chunkSize, 
                                           hdfsClientRetcode, 
                                           lobMaxChunkMemLen);
  if (hdfsClientRetcode != HDFS_CLIENT_OK) {
    NADELETEBASIC(lobData, lobGlobalHeap_);
    closeHdfsFile(tgtHdfsClient);
...
Ex_Lob_Error ExLob::closeHdfsFile(HdfsClient* &tgtHdfsClient)
{
  if (! tgtHdfsClient)
    return LOB_OPER_OK;

  tgtHdfsClient->hdfsClose();
...
{ 
  ...
  int openFlags = O_WRONLY;
  if (! useLibHdfs_) {
    err = openHdfsFileToWrite(tgtFileName, fileflags, tgtHdfsClient);
    if (err != LOB_OPER_OK)
      return err;
  } else {
  if ((LobTgtFileFlags)fileflags == Lob_Append_Or_Error )
   ...
   if (! useLibHdfs_) {
     ...
     while (remainLen > 0) 
     {
        ...
        if (hdfsClientRetcode == HDFS_CLIENT_OK) 
          {
            bytesWritten = 0;
            err = writeToHdfsFile(tgtHdfsClient, lobData, 
                                  bytesRead, lobMaxChunkMemLen,
                                  bytesWritten);
            if (err != LOB_OPER_OK)
              return err; 
...
Ex_Lob_Error ExLobsOper (
			 ...
			 char        *srcLobStorageLocation,           // directory in the storage
			 ComLobsStorageType storage,           // storage type
  ...
  if (globPtr != NULL)
  {
      ...
      if (it == lobMap->end())
	{
	  ...
	  err = lobPtr->initialize(fileName, (operation == Lob_Create) ? EX_LOB_CREATE : EX_LOB_RW, 
                                   lobStorageLocation, storage, hdfsServer, hdfsPort, lobStorageLocation,
                                   bufferSize, replication, blockSize,lobMaxSize,lobGlobals);
	  if (err != LOB_OPER_OK)
            {
              char buf[5000];
              str_sprintf(buf,"Lob initialization failed;filename:%s;location:%s;hdfsserver:%s;lobMaxSize:%ld",
                          fileName,lobStorageLocation,hdfsServer,lobMaxSize);
  ...
  */
  switch(operation)
    ...
    case Lob_InsertDesc:
      ...
      err = lobPtr->writeDesc(sourceLen, source, subOperation, descNumOut, 
                              hdfsDataOffset, lobMaxSize, handleIn,handleInLen,
                              (char *)blackBox, &blackBoxLen,handleOut,handleOutLen,transId,lobGlobals);
      if (err != LOB_OPER_OK)
        {
    ...
    case Lob_InsertData:
      err = lobPtr->insertData(source, sourceLen, subOperation, hdfsDataOffset, 
                               retOperLen, lobMaxSize,lobMaxChunkMemSize,lobGCLimit,
                               handleIn,handleInLen,(char *)blackBox, blackBoxLen,handleOut,handleOutLen,lobGlobals);
      if (err != LOB_OPER_OK)
        {
    ...
    case Lob_InsSel:
      {
        ...
        srcLobName = ExpLOBoper::ExpGetLOBname(
             uid, lobNum,
             descPartnKey, numSrcLOBdatafiles,
             srcLobNameBuf, LOB_NAME_LEN);        
        lobMap_it it2;
        ...
        if (it2 == lobMap->end())
	{
	  ...
	  err = srcLobPtr->initialize(srcLobName, EX_LOB_RW, srcLobStorageLocation, storage, 
                                      hdfsServer, hdfsPort, lobStorageLocation,bufferSize, 
                                      replication, blockSize,lobMaxSize,lobGlobals);
	  if (err != LOB_OPER_OK)
            {
              char buf[5000];
              str_sprintf(buf,"Lob initialization failed;filename:%s;location:%s;hdfsserver:%s;lobMaxSize:%ld",
                          srcLobName,lobStorageLocation,hdfsServer,lobMaxSize);
              lobDebugInfo(buf,err,__LINE__,lobGlobals->lobTrace_);
              return err;
            }
	  ...
	  lobMap->insert(pair<string, ExLob*>(string(srcLobName), srcLobPtr));
        ...
        err = lobPtr->insertSelect(srcLobPtr,handleIn, handleInLen, source, sourceLen, retOperLen, lobMaxSize, 
                                   lobMaxChunkMemSize,lobGCLimit,(char *)blackBox, blackBoxLen,handleOut,
                                   handleOutLen,subOperation,transId, lobGlobals);
        if (err != LOB_OPER_OK)
          {
    ...
    case Lob_Read:
      ...
      if (subOperation == Lob_Memory)
        {
          err = lobPtr->readToMem(source,sourceLen,retOperLen,handleIn,handleInLen,
                                  (char *)blackBox, blackBoxLen,handleOut,handleOutLen,transId);
          if (err != LOB_OPER_OK)
            {
        ...
        {
          err = lobPtr->readToFile(source, sourceLen, retOperLen, lobMaxChunkMemSize,  
                                   openType,handleIn,handleInLen,(char *)blackBox, 
  ...
  case Lob_GetOffset:
    ...
    case Lob_GetFileName:
      {
        err = lobPtr->getFileName(handleIn, handleInLen, 
                                  (char *)blackBox, blackBoxLen,  subOperation, transId);
      }
      break;
    case Lob_ReadDesc: // read desc only. Needed for pass thru.
      err = lobPtr->getDesc(desc,handleIn,handleInLen,
                            (char *)blackBox, &blackBoxLen,transId); 
      retOperLen = 0;
      break;
    case Lob_OpenCursor:
    ...
    case Lob_OpenDataCursorSimple:
	  ...
	  preOpenObj = new (lobGlobals->getHeap()) ExLobPreOpen(lobPtr, fn, 
          ...
          err = lobPtr->openDataCursor(fn, Lob_Cursor_Simple, descNumIn, sourceLen, 
                                       cursorBytes, waited, lobGlobals, (Int32 *)blackBox);
        } else
          err = LOB_SUBOPER_ERROR;
      }
      break;
    ...
    case Lob_ReadDataCursorSimple:
      {
        ...
        err = lobPtr->readDataCursorSimple(fn, source, sourceLen, retOperLen, 
                                           uncompressedRetOperLen, compressionWA, lobGlobals);
      }
      break;
    ...
    case Lob_Append:
      if ((subOperation == Lob_Memory) ||(subOperation == Lob_Buffer) || (subOperation ==Lob_External_File))
        {
          err = lobPtr->append(source, sourceLen, subOperation, descNumIn, retOperLen,
                               lobMaxSize, lobMaxChunkMemSize,lobGCLimit,handleIn,handleInLen,
                               handleOut,handleOutLen,transId, lobGlobals);
          if (err != LOB_OPER_OK)
            {
        ...
        {
          err = lobPtr->append(source, -1, subOperation, descNumIn, retOperLen,lobMaxSize, 
                               lobMaxChunkMemSize,lobGCLimit,handleIn,handleInLen,handleOut,
                               handleOutLen,transId,lobGlobals);
          if (err != LOB_OPER_OK)
            {
    ...
    case Lob_Update:
      if ((subOperation == Lob_Memory)||(subOperation == Lob_Buffer)||(subOperation ==Lob_External_File))
        {
          err = lobPtr->update(source, sourceLen, subOperation, descNumIn, retOperLen, 
                               lobMaxSize, lobMaxChunkMemSize,lobGCLimit,handleIn,
                               handleInLen,handleOut,handleOutLen,transId, lobGlobals);
          if (err != LOB_OPER_OK)
            {
        ...
        {
          err = lobPtr->update(source, -1, subOperation,descNumIn, retOperLen,lobMaxSize, 
                               lobMaxChunkMemSize,lobGCLimit,handleIn,handleInLen,
                               handleOut,handleOutLen,transId,lobGlobals); 
          if (err != LOB_OPER_OK)
            {
              lobDebugInfo("update(Memory,Buffer) failed ",err,__LINE__,lobGlobals->lobTrace_);
    ...
    case Lob_Drop:
      ...
      lobPtr = NULL;
      if (err != LOB_OPER_OK)           
        lobDebugInfo("dropLob failed ",err,__LINE__,lobGlobals->lobTrace_);
      break;

    case Lob_Purgedata:
      err = lobPtr->purgedataLob();
      it = lobMap->find(string(lobName));
      lobMap->erase(it);
      NADELETE(lobPtr, ExLob,lobGlobals->getHeap()) ;
      ...
      lobPtr = NULL;
      if (err != LOB_OPER_OK)           
        lobDebugInfo("purgedataLob failed ",err,__LINE__,lobGlobals->lobTrace_);
      break;
...
ExLobGlobals::~ExLobGlobals()
{
    ...
    {
        lobMap_itV2 it2; 
        for (it2 = lobMapV2_->begin(); it2 != lobMapV2_->end() ; ++it2)
        {
           ExLobV2 *lobPtr = it2->second; 
           NADELETE(lobPtr, ExLobV2, heap_);
        } 
        lobMapV2_->clear();
        NADELETE(lobMapV2_,lobMap_tV2,heap_);
        lobMapV2_ = NULL;
    }
...
Ex_Lob_Error ExLobGlobals::initialize()
{
    ...
    lobMapV2_ = (lobMap_tV2 *) new (getHeap())lobMap_tV2;  
    if (lobMapV2_ == NULL)
      return LOB_INIT_ERROR;
...
void ExLobRequest::setValues(char *descFileName, Int64 descNumIn, Int64 handleInLen, 
                             char *handleIn, ComLobsStorageType storage, Int64 transId,
