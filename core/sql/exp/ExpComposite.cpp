...
// ExpCompositeBase
////////////////////////////////////////////////////////////////////
ExpCompositeBase::ExpCompositeBase(OperatorTypeEnum oper_type,
                                   short type,
                                   ULng32 numElements,
                                   short numAttrs,
                                   Attributes ** attr,
...
Lng32 ExpCompositeBase::unpack(void *base, void * reallocator)
{
  if (compExpr_)
    {
      if(compExpr_.unpackShallow(base)) 
        return -1;
  ...
  if(compCriDesc_.unpack(base, reallocator)) 
    return -1;
  if (compAttrs_.unpack(base, reallocator))
    return -1;

  return unpackClause(base, reallocator);
}

ex_expr::exp_return_type ExpCompositeBase::fixup(Space * space,
                                                   CollHeap * exHeap,
                                                   char * constantsArea,
                                                   char * tempsArea,
                                                   char * persistentArea,
                                                   ...
                                                   NABoolean spaceCompOnly)
{
  if (getCompExpr())
    {
      getCompExpr()->fixup(0, ex_expr::PCODE_NONE, NULL,  space, exHeap, 
                           FALSE, NULL);
...
void ExpCompositeBase::displayContents(Space * space, 
                                       ...
                                       ULng32 flag)
{
  ...
  str_sprintf(buf, "  %s #%d: %s:L%d, type: %s, numElements: %d",
              "Clause",
              clauseNum,
              displayStr,
              exprNum,
...
static short getAttrDataPtr(Attributes *op, char *rowDataPtr,
                            char* &opdata, 
                            Lng32 &varLen,
                            NABoolean &isNullVal)
{
  ...
  if (op->getVCIndicatorLength() > 0) // varchar
    {
      if (tdf == ExpTupleDesc::SQLMX_ALIGNED_FORMAT)
        {
          offset = 
            ExpTupleDesc::getVoaOffset(rowDataPtr, op->getVoaOffset(), tdf);
...
///////////////////////////////////////////////////////////
ExpCompositeArrayLength::ExpCompositeArrayLength(OperatorTypeEnum oper_type,
                                                 short type,
                                                 Attributes ** attr,
                                                 Space * space)
     : ExpCompositeBase(oper_type, type,
                        0, 2, attr, 
                        0, NULL, NULL, NULL, space)
{
}

void ExpCompositeArrayLength::displayContents(Space * space, 
                                              const char * /*displayStr*/, 
                                              Int32 clauseNum, 
                                              char * constsArea,
                                              ULng32 flag)
{
  ExpCompositeBase::displayContents(space, "ExpCompositeArrayLength", 
                                    clauseNum, constsArea, flag);
}

ex_expr::exp_return_type ExpCompositeArrayLength::eval(char *op_data[], 
                                                       CollHeap *heap, 
...
///////////////////////////////////////////////////////////
ExpCompositeArrayCast::ExpCompositeArrayCast(OperatorTypeEnum oper_type,
                                             short type,
                                             Attributes ** attr,
                                             AttributesPtr compAttrs,
...
{
 if (compAttrsChild1_.unpack(base, reallocator))
    return -1;

  return ExpCompositeBase::unpack(base, reallocator);
}

void ExpCompositeArrayCast::displayContents(Space * space, 
                                            const char * /*displayStr*/, 
                                            Int32 clauseNum, char * constsArea)
{
  char buf[200];

  str_sprintf(buf, "  Clause #%d: ExpCompositeArrayCast, type: %s",
              clauseNum,
              (type_ == 1 ? "ARRAY" : "ROW")
              );
  space->allocateAndCopyToAlignedSpace(buf, str_len(buf), sizeof(short));
  ...
  str_sprintf(buf, "    compFormat: %d, childCompFormat: %d", 
              compAttrs->getCompFormat(),
...
ex_expr::exp_return_type ExpCompositeArrayCast::eval(char *op_data[], 
                                                     CollHeap *heap, 
                                                     ComDiagsArea **diagsArea)
{
  char *srcData = op_data[1];
  ...
  if (srcOp->getCompFormat() == tgtOp->getCompFormat())
    {
      ...
      *(Int32*)tgtData = copyElems;
      ...
      if (tgtOp->getVCIndicatorLength() > 0)
        tgtOp->setVarLength(srcLen, op_data[-MAX_OPERANDS]);
  ...
  *(Int32*)tgtData = numCopyElems;
  ...
  if (tgtOp->getCompFormat() == COM_SQLMX_ALIGNED_FORMAT)
    {
      ...
      eaf->setupHeaderAndVOAs(numCopyElems, numCopyElems,
         (isVarchar ? numCopyElems : 0));
    }
  else
    {
      currTgtNullData = tgtData;
      currTgtData = tgtData + 
        (isNullable ? tgtElemAttr->getNullIndicatorLength() : 0);
    }
  ...
  for (Int32 srcElem = 1; srcElem <= numCopyElems; srcElem++)
    {
      ...
      attrLen = (((isVarchar) && (varLen >= 0)) 
                 ? varLen : srcElemAttr->getLength(attrData));
  ...
  if (getOperand(0)->getVCIndicatorLength() > 0)
    getOperand(0)->setVarLength(tgtLen, op_data[-MAX_OPERANDS]);

  return ex_expr::EXPR_OK;
}

ex_expr::exp_return_type displayAttr(Space *space,
                                     Attributes * attr,
                                     Int32 operandNum,
                                     char * constsArea)
{
  ...
  if (NOT ((attr->getDatatype() == REC_ARRAY) ||
           (attr->getDatatype() == REC_ROW)))
    {
  ...
  Int32 numElemsToDisplay = (attr->getDatatype() == REC_ARRAY 
                             ? 1 : compAttrs->getNumElements());
...
///////////////////////////////////////////////////////////
ExpCompositeHiveCast::ExpCompositeHiveCast(OperatorTypeEnum oper_type,
                                           NABoolean fromHive,
                                           short type,
                                           Attributes ** attr,
                                           AttributesPtr compAttrs,
...
void ExpCompositeHiveCast::displayContents(Space * space, 
                                           const char * /*displayStr*/, 
                                           Int32 clauseNum, char * constsArea)
{
  char buf[200];

  str_sprintf(buf, "  Clause #%d: ExpCompositeHiveCast, type: %s",
              clauseNum,
              (type_ == 1 ? "ARRAY" : "ROW")
              );
  space->allocateAndCopyToAlignedSpace(buf, str_len(buf), sizeof(short));
...
// 
ex_expr::exp_return_type ExpCompositeHiveCast::copyHiveSrcToTgt(
     Attributes *tgtAttr,
     char* &srcData, char * tgtData,
     CollHeap *heap, 
     ComDiagsArea **diagsArea,
     NABoolean isRoot)
{
      ...
      if (elemLen >= 0)
        {
          if (DFS2REC::isSQLFixedChar(tgtAttr->getDatatype()))
            byte_str_cpy(tgtElemData,
                         tgtAttr->getLength(), srcData, elemLen, ' ');
          else if (tgtAttr->isNumericWithPrecision())
            {
              rc = convHiveDecimalToNumeric
                (srcData, elemLen, tgtElemData, tgtAttr, heap, diagsArea);
  ...
  if (tgtCompAttrs->getNumElements() < numElems)
    {
      ExRaiseSqlError(heap, diagsArea, -3242);
      *(*diagsArea) << DgString0(NAString("Expected number of elements(") + str_itoa(tgtCompAttrs->getNumElements(), errBuf_) + ") cannot be less than the returned number of elements(" + str_itoa(numElems, errBuf2_) + ").");
  ...
  for (Int32 i = 0; i < numElems; i++)
    {
      ...
      if (tgtElemAttr->isComposite())
        srcData += sizeof(Int32); // skip length
      rc = copyHiveSrcToTgt(tgtElemAttr, srcData, tgtCompData,
                            heap, diagsArea, FALSE);
      if (rc != ex_expr::EXPR_OK)
        return rc;
    } // for

  if ((NOT isRoot) && (tgtAttr->isVariableLength()))
    tgtAttr->setVarLength(tgtAttr->getLength(), tgtCompVarData);

  return ex_expr::EXPR_OK;
}

ex_expr::exp_return_type ExpCompositeHiveCast::eval(char *op_data[], 
                                                    CollHeap *heap, 
                                                    ComDiagsArea **diagsArea)
{
  ex_expr::exp_return_type rc;
...
// ExpCompositeConcat
///////////////////////////////////////////////////////////
ExpCompositeConcat::ExpCompositeConcat(OperatorTypeEnum oper_type,
                                       short type,
                                       Attributes ** attr,
                                       AttributesPtr compAttrs,
                                       AttributesPtr compAttrsChild1,
...
Lng32 ExpCompositeConcat::unpack(void *base, void * reallocator)
{
 if (compAttrsChild1_.unpack(base, reallocator))
    return -1;
 if (compAttrsChild2_.unpack(base, reallocator))
    return -1;

  return ExpCompositeBase::unpack(base, reallocator);
}

ex_expr::exp_return_type ExpCompositeConcat::eval(char *op_data[], 
                                                  CollHeap *heap, 
                                                  ComDiagsArea **diagsArea)
{
  ...
  if (getOperand(0)->isSQLMXAlignedFormat())
    {
      ...
      eaf->setupHeaderAndVOAs
        (numElems, numElems, 
         (resultAttr->getVCIndicatorLength() > 0 ? numElems : 0));
      ...
      if (resultAttr->getNullFlag())
        {
          if (isNullVal)
            {
              // move null value to result.
              ExpTupleDesc::setNullValue(currResultNullData,
                                         resultElem,
                                         getOperand(0)->getTupleFormat());
              continue;
            }

          ExpTupleDesc::clearNullValue(currResultNullData,
                                       resultElem,
                                       getOperand(0)->getTupleFormat() );
...
void ExpCompositeConcat::displayContents(Space * space, 
                                         const char * /*displayStr*/, 
                                         Int32 clauseNum, char * constsArea,
                                         ULng32 flag)
{
  ExpCompositeBase::displayContents(space, "ExpCompositeConcat", 
                                    clauseNum, constsArea, flag);
}
...
// ExpCompositeCreate
////////////////////////////////////////////////////////////////////
ExpCompositeCreate::ExpCompositeCreate(OperatorTypeEnum oper_type,
                                       short type,
                                       ULng32 numElements,
                                       short numAttrs,
                                       Attributes ** attr,
                                       ...
                                       AttributesPtr compAttrs,
                                       Space * space)
     : ExpCompositeBase(oper_type, type, numElements, numAttrs, attr,
                        compRowLen, compExpr, compCriDesc, compAttrs,
                        space)
{
}

Long ExpCompositeCreate::pack(void * space)
{
  return ExpCompositeBase::pack(space);
}

Lng32 ExpCompositeCreate::unpack(void *base, void * reallocator)
{
  return ExpCompositeBase::unpack(base, reallocator);
}

ex_expr::exp_return_type ExpCompositeCreate::eval(char *op_data[], 
                                                  atp_struct *atp1, 
                                                  atp_struct *atp2,
                                                  atp_struct *atp3,
                                                  CollHeap *heap, 
                                                  ComDiagsArea **diagsArea)
{
  ...
  ex_expr::exp_return_type evalRetCode = 
    getCompExpr()->eval(atp1, atp2, compAtp_,
                        heap, -1,
                        &actualRowLen,
                        NULL, NULL);
  ...
  if ((evalRetCode == ex_expr::EXPR_ERROR) &&
      (getOperand(0)->getDatatype() == REC_ARRAY) &&
  ...
  if (evalRetCode == ex_expr::EXPR_ERROR)
    {
      ...
      if (((!diagsArea) || (! *diagsArea)) ||
          (diagsArea && *diagsArea && (NOT (*diagsArea)->contains(-8146))))
        {
  ...
  if (actualRowLen == 0)
    actualRowLen = maxTgtLen;

  if (actualRowLen > maxTgtLen)
    {
      char buf[200];
      sprintf(buf, "actualRowLen(%d) is greater than maxRowLen(%d) in method ExpCompositeCreate::eval.",
              actualRowLen, maxTgtLen);
      ExRaiseSqlError(heap, diagsArea, -8146);
...
void ExpCompositeCreate::displayContents(Space * space, 
                                         const char * /*displayStr*/, 
                                         Int32 clauseNum, char * constsArea,
                                         ULng32 flag)
{
  ExpCompositeBase::displayContents(space, "ExpCompositeCreate", 
                                    clauseNum, constsArea, flag);
}
...
// ExpCompositeDisplay
///////////////////////////////////////////////////////////
ExpCompositeDisplay::ExpCompositeDisplay(OperatorTypeEnum oper_type,
                                         short type,
                                         ULng32 numElements,
                                         short numAttrs,
                                         Attributes ** attr,
                                         ...
                                         Space * space)
     : ExpCompositeBase(oper_type, type, numElements, numAttrs, attr, 
                        0, NULL, compCriDesc, compAttrs, space)
{
}

static Lng32 getDisplayLength(Lng32 datatype,
                              Lng32 length,
                              Lng32 precision,
                              Lng32 scale)
{
...
static short displayValue(Attributes *attr, char *tgtData, 
                          UInt32 &currTgtIndex, char *srcData,
                          Lng32 varLen)
{
...
short displayValues(Attributes * attr,
                    char * tgtPtr,
                    char * srcPtr,
                    Lng32 varLen,
                    UInt32 &currTgtIndex)
{
  if (NOT ((attr->getDatatype() == REC_ARRAY) ||
           (attr->getDatatype() == REC_ROW)))
    {
      if (displayValue(attr, &tgtPtr[currTgtIndex], currTgtIndex, 
                       srcPtr, varLen))
        return -1;
  ...
  for (Int32 i = 0; i < numElements; i++)
    {
      ...
      if (isNullVal)
        ...
        {
          if (displayValues(attr, tgtPtr, attrData, varLen, currTgtIndex))
            return -1;
        }
...
void ExpCompositeDisplay::displayContents(Space * space, 
                                          const char * /*displayStr*/, 
                                          Int32 clauseNum, char * constsArea)
{
  char buf[200];

  str_sprintf(buf, "  Clause #%d: ExpCompositeDisplay, type: %s, numElements: %d",
              clauseNum,
              (type_ == 1 ? "ARRAY" : "ROW"),
              numElements_
              );
...
ex_expr::exp_return_type ExpCompositeDisplay::eval(char *op_data[], 
                                                   CollHeap *heap, 
                                                   ComDiagsArea **diagsArea)
{
  Lng32 srcLen = getOperand(1)->getLength(op_data[-MAX_OPERANDS+1]);
...
// ExpCompositeExtract
///////////////////////////////////////////////////////////
ExpCompositeExtract::ExpCompositeExtract(OperatorTypeEnum oper_type,
                                         short type,
                                         ULng32 numElements,
                                         Lng32 elemNum,
                                         short numAttrs,
...
{
  if(searchAttrTypeList_.unpack(base))
    return -1;
  if(searchAttrIndexList_.unpack(base))
    return -1;

  return ExpCompositeBase::unpack(base, reallocator);
}

void ExpCompositeExtract::displayContents(Space * space, 
                                          const char * /*displayStr*/, 
                                          Int32 clauseNum, char * constsArea)
{
  char buf[200];

  str_sprintf(buf, "  Clause #%d: ExpCompositeExtract, type: %s",
              clauseNum,
              (type_ == 1 ? "ARRAY" : "ROW")
              );
  space->allocateAndCopyToAlignedSpace(buf, str_len(buf), sizeof(short));

  str_sprintf(buf, "  numElements: %d, elemNum: %d",
              numElements_, elemNum_
              );
  space->allocateAndCopyToAlignedSpace(buf, str_len(buf), sizeof(short));
  ...
  if (elemNum_ < 0)
    {
      ...
      for (Int32 i = 0; i < numSearchAttrs_; i++)
        {
          str_sprintf(buf, "  searchAttrType[%d]: %3d, searchAttrIndex[%d]: %2d",
                      i, getSearchAttrType(i), i, getSearchAttrIndex(i));
          space->allocateAndCopyToAlignedSpace(buf, str_len(buf), sizeof(short));
        }
    } // elemNum_ < 0
...
ex_expr::exp_return_type 
ExpCompositeExtract::extractValue(Attributes * inAttrs,
                                  Lng32 elemNum,
                                  char * tgtPtr,
                                  char * srcPtr,
                                  Int32 maxNumElems,
  ...
  if (inAttrs->getDatatype() == REC_ARRAY)
    {
      if (elemNum > maxNumElems) // greater than max num elements
        {
          ExRaiseSqlError(heap, diagsArea, -3242);
          *(*diagsArea) << DgString0(NAString("Element to be extracted must be between 1 and max elements(") + str_itoa(maxNumElems, errBuf) + ").");
...
ex_expr::exp_return_type 
ExpCompositeExtract::searchAndExtractValue(Attributes * inAttrs,
                                           char * tgtPtr,
                                           char * srcPtr,
                                           NABoolean &isNullVal,
                                           ULng32 &attrLen,
  ...
  for (Int32 i = 0; i < numSearchAttrs(); i++)
    {
      ...
      getAttrDataPtr(currAttr, currCompAttrsData, currAttrData, 
                     varLen, isNullVal);
      if (isNullVal)
        return ex_expr::EXPR_OK;
...
ex_expr::exp_return_type ExpCompositeExtract::eval(char *op_data[], 
                                                   CollHeap *heap, 
                                                   ComDiagsArea **diagsArea)
{
  ex_expr::exp_return_type rc;
  ...
  if (ex_clause::processNulls(null_data, heap, diagsArea) == ex_expr::EXPR_NULL)
    return ex_expr::EXPR_OK;
  ...
  if ((numSearchAttrs() == 0) &&
    ...
    {
      ExRaiseSqlError(heap, diagsArea, -8146);
      *(*diagsArea) << DgString0("In method ExpCompositeExtract::eval, numSearchAttrs() == 0  and elemNum_ == -1 and getOperand(2) == NULL");
  ...
  if ((elemNum < 0) && (numSearchAttrs() <= 0) && 
      (getOperand(2)) && (op_data[2]))
    {
  ...
  if (numSearchAttrs() > 0)
    {
      rc = searchAndExtractValue(compAttrs_, 
                                 op_data[0], op_data[1], 
                                 isNullVal, tgtLen,
                                 heap, diagsArea);
    }
    ...
    {
      Int32 numElems = 0;
      rc = extractValue(compAttrs_, elemNum, 
                        op_data[0], op_data[1], 
                        numElements(),
                        isNullVal, tgtLen,
                        numElems,
                        heap, diagsArea);
      if ((compAttrs_->getDatatype() == REC_ARRAY) &&
          (numElems > 0) &&
          (elemNum > numElems))
        {
