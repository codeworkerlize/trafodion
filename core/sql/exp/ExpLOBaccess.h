...
Ex_Lob_Error ExLobsOper (
    ...
    Int64       &descNumOut,       // output desc Num (for flat files
                                   // only)
    ...
    char        *srcDir,           // src directory in the storage
...
{
  public:
    ExLobDesc(Int64 offset, Int64 size);
  ExLobDesc() :
      dataOffset_(0),
      dataSize_(0)
    {};
    ~ExLobDesc();
    Int64 getSize() { return dataSize_; }
    void setSize(Int64 size) { dataSize_ = size; }
    Int64 getOffset() { return dataOffset_; }
    void setOffset(Int64 offset) { dataOffset_ = offset; }
...
class ExLob : public NABasicObject
{
  public:
    ...
    void setUseHdfsWriteLock(NABoolean useHdfsWriteLock) 
       { useHdfsWriteLock_ = useHdfsWriteLock; }
    NABoolean useHdfsWriteLock() { return useHdfsWriteLock_; }
 
    void setHdfsWriteLockTimeoutInSecs(int timeout)
       { hdfsWriteLockTimeoutInSecs_ = timeout; }

    int hdfsWriteLockTimeoutInSecs() { return hdfsWriteLockTimeoutInSecs_; }

    Ex_Lob_Error initialize(const char *lobFile, Ex_Lob_Mode mode, char *dir, 
                            ComLobsStorageType storage, char *hdfsServer, Int64 hdfsPort,
                            char *lobLocation,
  ...
  Ex_Lob_Error readCursorData(char *tgt, Int64 tgtSize, cursor_t &cursor, Int64 &operLen,char *handleIn, Int32 handeLenIn,Int64 transId);
    ...
    Ex_Lob_Error readDataCursorSimple(const char *fileName, char *tgt, Int64 tgtSize, Int64 &operLen, Int64 &uncompressedOperLen, ExpCompressionWA * compressionWA, ExLobGlobals *lobGlobals);
  ...
  Ex_Lob_Error dropLob();
  Ex_Lob_Error purgedataLob();
  ...
  Ex_Lob_Error doSanityChecks(char *dir, ComLobsStorageType storage,
  ...
  Ex_Lob_Error getDesc(ExLobDesc &desc,char * handleIn, Int32 handleInLen, char *blackBox, Int32 *blackBoxLen,  Int64 transId);
  ...
  Ex_Lob_Error openLocalFileToWrite(char *filename, Int32 fileflags,
                                    int &fdDestFile);
  Ex_Lob_Error writeToLocalFile(int &fdDestFile, char *lobData,
                                Int64 chunkSize, Int64 &tgtOffset,
                                Int64 &writeOperLen);
  Ex_Lob_Error closeLocalFile(int &fdDestFile);
  ...
  Ex_Lob_Error openHdfsFileToWrite(char *fileName, Int32 fileflags,
                                   HdfsClient* &tgtHdfsClient);
  Ex_Lob_Error writeToHdfsFile(HdfsClient* &tgtHdfsClient, char *lobData,
                               Int64 chunkSize, Int64 lobMaxChunkMemLen,
                               Int64 &writeOperLen);
  Ex_Lob_Error closeHdfsFile(HdfsClient* &tgtHdfsClient);
  ...
  public:
    ...
    ComLobsStorageType storage_;
    NAString lobStorageLocation_; // lob data directory
    char *hdfsServer_;
    Int64 hdfsPort_;
    hdfsFS fs_;
    hdfsFile fdData_;
    ...
    short hdfsWriteLockTimeoutInSecs_;
    HdfsClient *hdfsClient_;
    NABoolean useHdfsWriteLock_;
};

typedef map<string, ExLob *> lobMap_t;
typedef map<string, ExLob *>::iterator lobMap_it;

class ExLobV2 : public ExLob
{
public:
  ExLobV2(NAHeap * heap, ExHdfsScanStats *hdfsAccessStats);
  virtual ~ExLobV2();

  Ex_Lob_Error initialize(const char *lobFile,
  ...
  Ex_Lob_Error writeData(Int64 &offset, char *data, Int32 size, Int64 &operLen);
  Ex_Lob_Error readData(Int64 offset, char *tgt, Int32 tgtSize, Int64 &operLen);
  
};
typedef map<string, ExLobV2 *> lobMap_tV2;
typedef map<string, ExLobV2 *>::iterator lobMap_itV2;
...
class ExLobGlobals
{
  public :
    ...
    lobMap_tV2 * getLobMapV2() { return lobMapV2_; }
  ...
  void realTraceMessage(const char *logMessage, void *c, int line, long num = 0);
    ...
    short hdfsWriteLockTimeout() { return hdfsWriteLockTimeout_; }
    void setHdfsWriteLockTimeout(NABoolean hdfsWriteLockTimeout)
       { hdfsWriteLockTimeout_ = hdfsWriteLockTimeout; } 
    
  
  public :
    lobMap_t *lobMap_;
    lobMap_tV2 *lobMapV2_;
    hdfsFS fs_;
