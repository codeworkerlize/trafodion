...
ex_expr::exp_return_type
convHiveDecimalToNumeric(char *sourceData, Lng32 srcLen,
                         char *targetData, Attributes *attr,
...
public:
  ...
  char * getUserTextStr() 
  { 
    return (userTextStr_[0] == 0 ? NULL : userTextStr_);
  }
  const char * getUserTextStr() const
  { 
    return (userTextStr_[0] == 0 ? NULL : userTextStr_);
  }
...
public:
  ExFunctionRangeOfValues(OperatorTypeEnum oper_type,
                           Attributes ** attr,
                           Space * space,
                           UInt32 cap,
                           UInt32 numChildren=1,
                           Int16 filterId = 0,
                           NABoolean dumpDebugData = FALSE,
                           NABoolean dateAsTimeStamp = TRUE,
                           NABoolean nativeByteOrder = TRUE,
  ...
  {
     return ( x == ITM_RANGE_VALUES_MERGE ||
              x == ITM_RANGE_VALUES_INSERT ||
              x == ITM_RANGE_VALUES_COPY ||
              x == ITM_RANGE_VALUES_UNPACK ||
              x == ITM_RANGE_VALUES_IN 
            );
  }

  ex_expr::exp_return_type fixup(Space * space,
                                 CollHeap * exHeap,
                                 char * constantsArea,
                                 char * tempsArea,
                                 char * persistentArea,
  ...
  void packRanges(char* target, 
                  char* vcLenPtr, Lng32 vcLenSize,
                  NABoolean clearWhenDone = TRUE,
  ...
  static 
  ex_expr::exp_return_type 
       conditionalPack(ex_clause* clause, atp_struct* atp,
                       OperatorTypeEnum op, const char* msg = NULL,
                       NABoolean clearWhenDone = TRUE,
                       ScanFilterStatsList* stats = NULL,
                       ScanFilterStats::MergeSemantics = ScanFilterStats::COPY
                      );
  ...
  // in bad shape.
  static ex_expr::exp_return_type 
       conditionalSanityCheck(ex_clause* clause, OperatorTypeEnum op);

  void displayContents(Space * space, const char * displayStr, 
                      Int32 clauseNum, char * constsArea);
 
  static 
   void conditionalDisplay(ex_clause* cptr, OperatorTypeEnum op, const char* msg);

  static 
   void conditionalPrint(ostream& out, ex_clause* cptr, OperatorTypeEnum op, const char* msg);

  static 
   void showRSA(ex_clause* cptr, OperatorTypeEnum op, const char* msg);
