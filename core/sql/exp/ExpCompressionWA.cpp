				...
				Int64& uncompressedBytesProduced)
{
	 ...
	 (uncompressedBytesProduced < targetMaxLen))
  {
	...
	setScratchBufUsedSize(blockHeaderLen);
      ...
      if (hasCheckSum)
        checkSum = read32(getScratchBuf() + 8);
      if (uncompressedLen > compressionBlockSize || 
	  compressedLen > compressionBlockSize ||
	  compressedLen == 0 )
	return COMPRESS_FAILURE;

      if (compressedLen > (getScratchBufUsedSize() - blockHeaderLen) 
	...
	if (uncompressedLen < (targetMaxLen - uncompressedBytesProduced))
	{
	  ...
	  int r = lzo1x_decompress_safe((unsigned char *)getScratchBuf()+blockHeaderLen, 
					compressedLen, 
    ...
    else if (srcLength - compressedBytesRead >= blockHeaderLen)
    {
      ...
      if (hasCheckSum)
        checkSum = read32(src + compressedBytesRead + 8);
      if (uncompressedLen > compressionBlockSize || 
	  compressedLen > compressionBlockSize ||
	  compressedLen == 0 )
	return COMPRESS_FAILURE;
	...
	if (uncompressedLen < (targetMaxLen - uncompressedBytesProduced))
	{
	  ...
	  int r = lzo1x_decompress_safe((unsigned char *)
					src+compressedBytesRead+blockHeaderLen, 
					compressedLen, 
					(unsigned char *)target, 
					&newUncompressedLen, 
					NULL);
...
ExpLzopCompressionWA::ExpLzopCompressionWA(const ComCompressionInfo *ci, 
                                           CollHeap* heap) :
  ExpLzoCompressionWA(ci, heap),
  headerSeen_(FALSE)
{}

ExpCompressionWA::CompressionReturnCode 
ExpLzopCompressionWA::decompress(char* src, Int64 srcLength, 
				char* target, Int64 targetMaxLen, 
				Int64& compressedBytesRead, 
				Int64& uncompressedBytesProduced)
{
  ...
  if(!headerSeen_)
  {
    ...
    if (rc != COMPRESS_SUCCESS)
      return rc;
    src = src + headerLen;
    srcLength -= headerLen;
  }
  rc = ExpLzoCompressionWA::decompress(src, srcLength, target, targetMaxLen,
                                       compressedBytesRead,
...
ExpCompressionWA::CompressionReturnCode 
ExpLzopCompressionWA::processHeader(char* src, Int64 srcLength, 
                                    UInt32& headerLen)
{
  UInt32 offset = 0;
  if ((srcLength > sizeof(lzop_magic)) && 
      memcmp(src,lzop_magic,sizeof(lzop_magic)) != 0)
  ...
  {
    libVersion = read16(src+offset);
    if ((libVersion > LZOP_VERSION) || (libVersion < 0x0900))
      return COMPRESS_HEADER_ERROR;
    offset += 2 ;
  }
