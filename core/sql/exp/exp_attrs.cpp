...
switch (datatype)
  ...
  case REC_ARRAY: return extFormat ? (char*)"ARRAY" : (char*)"REC_ARRAY";
  case REC_ROW: return extFormat ? (char*)"ROW" : (char*)"REC_ROW";
  ...
  if (isComposite())
    {
      ...
      str_sprintf(buf, "      compFormat = %s", 
                  (ca->getCompFormat() == ExpTupleDesc::SQLMX_ALIGNED_FORMAT
                   ...
                   : (ca->getCompFormat() == ExpTupleDesc::SQLARK_EXPLODED_FORMAT
                      ? "SQLARK_EXPLODED_FORMAT" : "UNINITIALIZED_FORMAT")));
      space->allocateAndCopyToAlignedSpace(buf, str_len(buf), sizeof(short));
    }
