...
//	}
      ...
      if (reduceExpon)
	{
      ...
      if (std::isinf(doubleIntMantissa))
         return -1;
...
ex_expr::exp_return_type convLargeDecToAscii(char *target,
  ...
  while ((currPos < startPos) && source[currPos] == 0)
    currPos++;
		         ...
		         ) 
{
	...
	(NAWchar*)(&source[targetLen*BYTES_PER_NAWCHAR]),
      ...
      if ( doPad == TRUE && targetLen > convertedLen ) 
        str_pad(&target[convertedLen], targetLen - convertedLen, padVal);
    }
    return retcode;
}
			...
			) 
{
    ...
    if (copyLen < convertedLen)
      ...
      if ( doPad == TRUE && targetLen > copyLen ) 
          ...
          if (ignoreTrailingBlanksInSource)
            {
              ...
              canIgnoreOverflowChars = 
                !(str_find_first_nonpad(firstUntranslatedChar, remainingScanStringLen, padVal));
            }
          ...
          if (ignoreTrailingBlanksInSource && rc == CNV_ERR_BUFFER_OVERRUN)
            {
              ...
              // check whether the characters from firstUntranslatedChar onwards are all blanks
              canIgnoreOverflowChars = 
                !(str_find_first_nonpad(firstUntranslatedChar, remainingSourceStringLen, padVal));
            }
              ...
              if (ignoreTrailingBlanksInSource)
                {
                  ...
                  relevantCharsGotTruncated = 
                    (str_find_first_nonpad(&(source[outputDataLen]),
                                           remainingSourceStringLen, padVal) != NULL);
                }
              else
...
ex_expr::exp_return_type convHiveDecimalToNumeric(
     char *sourceData, Lng32 srcLen,
  ...
  if (extValPrecision <= 18) // short, int, long
    {
        ...
        case REC_NUM_BIG_SIGNED:
          {
            ex_expr::exp_return_type rc =
              convDoIt((char*)&extInt64Val, sizeof(Int64), 
                       REC_BIN64_SIGNED, attr->getPrecision(), 
                       attr->getScale(),
                       targetData,
                       attr->getLength(), REC_NUM_BIG_SIGNED, 
                       attr->getPrecision(), attr->getScale(),
                       NULL, 0, heap, diagsArea,
                       CONV_BIN64S_BIGNUM, NULL, 0);
      ...
      if (copyLen > 0)
        str_cpy_all(targetData, copyData, copyLen);
    }
  else 
    {
      ...
      ex_expr::exp_return_type rc =
        convDoIt(numData, copyLen, REC_BYTE_F_ASCII, 0, 0,
                 targetData,
                 attr->getLength(), REC_NUM_BIG_SIGNED, 
                 attr->getPrecision(), attr->getScale(),
                 NULL, 0, heap, diagsArea,
...
convDoIt(char * source,
  ...
  if (flags)
	...
	allowSignInInterval = TRUE;
      ...
      if (flags & CONV_NOT_TRIM_TAIL_ZERO)
        bTrimTailZero = FALSE;
    }
          ...
          if (targetLen > copyLen) 
          ...
          else  // targetLen <= copyLen
            {
              ...
              if (copyLen < sourceLen) 
                {
                  char * first_nonblank = str_find_first_nonpad(
                       &source[copyLen], sourceLen - copyLen, padVal);
...
ex_expr::exp_return_type ex_conv_clause::eval(char *op_data[],
  ...
  default: {
    ...
    if( retcode != ex_expr::EXPR_OK && 
        ( flags_ & CONV_TO_NULL_WHEN_ERROR ) != 0)
    {
      if (*diagsArea)
        {
  ...
  if (retcode != ex_expr::EXPR_OK &&
      (((*diagsArea)->mainSQLCODE() == -EXE_CONVERT_STRING_ERROR) ||
