  ...
  {                                                                                                               \
    if (NExprDbgLvl_ >= VV_XD) {                                                                                  \
      ...
      sprintf(NExBuf,                                                                                             \
              ...
              PCOp->getStackIndex(), PCOp->getOffset(), PCOp->getLen(), PCOp->getType(), PCOp->getNullBitIndex(), \
              PCOp->getCurrJitVal());                                                                             \
      NExLog(NExBuf);                                                                                             \
    }                                                                                                             \
  }
...
void PCodeCfg::computeDomTree() {
  ...
  //
  FOREACH_BLOCK_REV_DFO(PCBlk, firstPCInst, lastPCInst, indx) { PCBlk->onesVector.clear(); }
  ENDFE_BLOCK_REV_DFO
...
void PCodeCfg::addToTempsList(PCodeOperand *PCOp, PCIT::AddressingMode OprTyp, int Offset, int Num) {
  ...
  if (operand != NULL && operand != PCOp) PRINT_DETAILS_OF_WROp(0, operand);
#endif
  ...
  if (inTempsList(Offset, OprTyp)) return;  // We will resuse the existing allocated space
...
NABoolean PCodeCfg::canGenerateNativeExpr() {
  ...
  switch (opc) {
    ...
    case PCIT::NOT_NULL_BRANCH_COMP_MBIN32S_MBIN32S_IATTR3_IBIN32S: {
      ...
      if (read->isTemp() && !inTempsList(read->getOffset(), read->getType())) {
        ...
        if (NExprDbgLvl_ >= VV_I2) {
          DPT2("VVN030: ", VV_I3,
               "Cannot GENERATE for PCODE opcode "
      ...
      if (PCInst->getWOps().entries()) {
        ...
        for (int iii = 0; iii < opList.entries(); iii++) {
          ...
          if (PCOp->getStackIndex() == 2)  // DON'T YET KNOW HOW TO HANDLE THIS with LLVM!
          {
            ...
            if (NExprDbgLvl_ >= VV_I2) {
              DPT1("VVN001: ", VV_I3,
                   "Cannot GENERATE for PCODE "
                   "opcode %3d - Writes To Temp Var\n",
                   PCInst->getOpcode());
              DPT6("VVN001a: ", VV_I3,
                   "Details of Bad Temp Var:\n"
                   "stackIndex_ = %d, offset_ = %d, len_=%d, "
                   "operandType_=%d, nullBitIdx=%d, CJV=%p\n",
                   PCOp->getStackIndex(), PCOp->getOffset(), PCOp->getLen(), PCOp->getType(), PCOp->getNullBitIndex(),
                   PCOp->getCurrJitVal());
    ...
    case PCIT::MOVE_MBIN8_MBIN8: {
      ...
      if (src1->isConst()) {
        if (src1->getType() == PCIT::MPTR32) {
          ...
          DPT3("VVY003: ", VV_I3,
               "CAN    GENERATE for PCODE opcode "
               "%3d  %s  type=%d\n",
               opc, NatExprName(opc), (int)src1->getType());
          PCodeOperand *write = PCInst->getWOps()[0];
          if (write->isTemp()) addToTempsList(write, write->getType(), write->getOffset());
        }
      }
      break;
    }
    ...
    case PCIT::CLAUSE_EVAL: {
      ...
      switch (clause->getClassID()) {
        case ex_clause::AGGR_MIN_MAX_ID: {
          ...
          if (NExprDbgLvl_ >= VV_I2) {
            DPT1("VVN005: ", VV_I3,
                 "Cannot GENERATE for PCODE opcode %d -"
          ...
          if (!processNulls) {
            ...
            if (isSupportedClauseOperandType(clause->getOperand(0))) {
              DPT2("VVY004: ", VV_I3, "CAN    GENERATE for PCODE opcode %3d  %s\n", opc, NatExprName(opc));
              break;
            }
          }
        ...
        case ex_clause::COMP_TYPE: {
          ...
          switch (((ex_comp_clause *)clause)->getInstruction()) {
            ...
            case GE_DATETIME_DATETIME: {
              ...
              long dtCode = ((ExpDatetime *)(clause->getOperand(1)))->getPrecision();

              // The lengths of the source operands must be the same.
              if ((clause->getOperand(1)->getLength()) != (clause->getOperand(1)->getLength())) {
#if NExprDbgLvl >= VV_I2
                if (NExprDbgLvl_ >= VV_I2) {
                  DPT1("VVN007: ", VV_I3,
                       "Cannot GENERATE for PCODE "
              ...
              switch (dtCode) {
                ...
                case REC_DTCODE_TIMESTAMP:
                  DPT2("VVY008: ", VV_I3, "CAN    GENERATE for PCODE opcode %3d  %s\n", opc, NatExprName(opc));
                  break;
                default: {
#if NExprDbgLvl >= VV_I2
                  if (NExprDbgLvl_ >= VV_I2) {
    ...
    case PCIT::FILL_MEM_BYTES: {
      PCodeOperand *write = PCInst->getWOps()[0];
      if (write->isTemp()) addToTempsList(write, write->getType(), write->getOffset(), PCInst->code[3]);
      break;
    }
    ...
    case PCIT::FILL_MEM_BYTES_VARIABLE: {
      ...
      if (write->getVoaOffset() == -1) {
        ...
        if (write->isTemp()) addToTempsList(write, write->getType(), write->getOffset(), PCInst->code[6]);
        break;
      }
    ...
    case PCIT::NULL_TEST_MBIN32S_MATTR5_IBIN32S_IBIN32S: {
      ...
      if (write->isTemp()) addToTempsList(write, write->getType(), write->getOffset());
      DPT2("VVY018: ", VV_I3, "CAN    GENERATE for PCODE opcode %3d  %s\n", opc, NatExprName(opc));
      break;
    }
    ...
    case PCIT::SUM_MATTR3_MATTR3_IBIN32S_MBIN64S_MBIN32S: {
      ...
      DPT2("VVY019: ", VV_I3, "CAN    GENERATE for PCODE opcode %3d  %s\n", opc, NatExprName(opc));
...
#if NExprDbgLvl >= VV_I2
if (NumBad == 1) {
  DPT2("VVN099: ", VV_I2, "Cannot GENERATE for PCODE opcode %3d  %s ONLY\n\n", BadOpc, NatExprName(BadOpc));
}
return RTNV;
#else
return TRUE;
...
NABoolean PCodeCfg::canGenerateNativeExpr() {
  ...
  FOREACH_BLOCK_REV_DFO(PCBlk, firstPCInst, lastPCInst, indx){FOREACH_INST_IN_BLOCK(
      PCBlk, inst){switch (inst->getOpcode()){case PCIT::BRANCH : case PCIT::BRANCH_OR : case PCIT::
                                              BRANCH_AND : case PCIT::BRANCH_OR_CNT : case PCIT::BRANCH_AND_CNT : break;
  ...
  case PCIT::FILL_MEM_BYTES_VARIABLE: {
    // Only support non-aligned format case.  Otherwise we have to start
    // tracking local varOffset (which we can do, but just not there yet).
    if (inst->getWOps()[0]->getVoaOffset() == -1) break;
  ...
  case PCIT::MOVE_MATTR5_MATTR5: {
    ...
    // Since we plan on copying everyting (length + string) in one shot,
    // make sure the vc lengths of the operands are the same.
    if (read->getVcIndicatorLen() != write->getVcIndicatorLen()) return FALSE;

    // For now, no support for temp varchars (since temp varchars will
    // likely need to come from a separate temps area.
    if (write->isTemp() || read->isTemp()) return FALSE;

    // Also, target max length must be greater than that of source
    if (read->getVcMaxLen() > write->getVcMaxLen()) return FALSE;
  ...
  case PCIT::NULL_TEST_MBIN32S_MATTR5_IBIN32S_IBIN32S: {
    ...
    if ((src->getOffset() == -1) && !src->forAlignedFormat()) return FALSE;
  ...
  case PCIT::STRLEN_MBIN32U_MATTR5: {
    ...
    if (inst->getROps()[0]->isVarchar()) break;
  ...
  case PCIT::MOVE_MBIN32S: {
    // Only supported if RETURN follows it (as it always should)
    if (inst->next && (inst->next->code[0] == PCIT::RETURN)) break;
  ...
  case PCIT::HASH_MBIN32U_MBIN64_IBIN32S_IBIN32S: {
    ...
    if (!inst->getROps()[0]->isVar()) return FALSE;

    // If we're hashing a fixed string, only support it for reasonable
    // sizes (TODO: until code is written to make the hash a loop).
    if ((inst->getOpcode() == PCIT::HASH_MBIN32U_MPTR32_IBIN32S_IBIN32S) && (inst->getROps()[0]->getLen() > 1000))
      return FALSE;
  ...
  case PCIT::CLAUSE_EVAL: {
    ...
    switch (clause->getClassID()) {
      case ex_clause::AGGR_MIN_MAX_ID: {
        ...
        if (!processNulls) {
          // Can only tolerate certain types for now.
          if (isSupportedClauseOperandType(clause->getOperand(0))) break;
        }
      ...
      case ex_clause::COMP_TYPE: {
        // No special nulls allowed for now.
        if (clause->isSpecialNulls()) return FALSE;
        ...
        switch (((ex_comp_clause *)clause)->getInstruction()) {
          ...
          case GE_DATETIME_DATETIME: {
            ...
            // The lengths of the source operands must be the same.
            if ((clause->getOperand(1)->getLength()) != (clause->getOperand(1)->getLength())) return FALSE;
  ...
  case PCIT::GENFUNC_MATTR5_MATTR5_IBIN32S: {
    if ((inst->code[11] == ITM_UPPER) || (inst->code[11] == ITM_LOWER)) break;
...
void PCodeOperand::clearJitValues(PCodeCfg *cfg) {
  ...
  if (operand != this) return operand->clearJitValues(cfg);
...
//
void PCodeOperand::setJitValue(PCodeCfg *cfg, IRBldr_t *Bldr, jit_value_t val, PCodeBlock *b,
  ...
  if (operand != this) return operand->setJitValue(cfg, Bldr, val, b, noSave);
  ...
  if (!noSave) {
    ...
    CDPT2("VV0c10: ", VV_BD,
          "In setJitValue(): setting jitValue_ in PCodeOp "
          "at %p to %p\n",
          this, jitValue_);
    CDPT1("VV0c11: ", VV_BD, "jitValue_'s Type ID is %d\n", (int)val->getType()->getTypeID());
    CDPT2("VV0c12: ", VV_BD,
          "In setJitValue(): setting jitValueBlock_ = "
          "%p for PCOp = %p\n",
          b, this);
    jitValueBlock_ = b;
  }
...
#ifdef NA_LINUX_LIBJIT
// FIX: Overlapped types can have problems
void PCodeOperand::setJitValue(PCodeCfg *cfg, jit_function_t f, jit_value_t val, PCodeBlock *b,
                               NABoolean noSave = FALSE) {
  // If this was called to store into a non-temp, storeJitValue should be
  // called instead.
  if (!isTemp()) return storeJitValue(cfg, f, jit_value_get_type(val), val, b, NULL, noSave);

  // Also call storeJitValue if it's a temp and the temp already has a different
  // jit value associated with it.
  if (isTemp() && (jitValue_ != val)) return storeJitValue(cfg, f, jit_value_get_type(val), val, b, NULL, noSave);

  // Get operand in hash table
  CollIndex i = bvIndex_;
  PCodeOperand *operand = cfg->getMap()->getFirstValue(&i);
  if (operand != this) return operand->setJitValue(cfg, f, val, b, noSave);
...
void PCodeCfg::setupClauseOperand(PCodeCfg *cfg, OPLIST &opList, PCodeOperand **opData, int index,
  ...
  opList.clear();
  DPT1("VV0a20: ", VV_XD,
       "AFTER clearing opList, 'operand=OLD opList[0] == "
       "%p\n",
       operand);
...
jit_value_t PCodeCfg::IR_LoadRelativeWithType(IRBldr_t *Bldr, jit_value_t int8p, long byteOffset,
  ...
  if (byteOffset == 0)
    ...
    DPT1("VV0101: ", VV_VD, "int8p2 = Bldr->CreateGEP( int8p, byOff = %ld)\n", byteOffset);
    int8p2 = Bldr->CreateGEP(int8p, byOff);
  }

  DPT0("VV0102: ", VV_VD, "finalPtr   = Bldr->CreatePointerCast(int8p2, typeToLoad->getPointerTo() )\n");
  finalPtr = Bldr->CreatePointerCast(int8p2, typeToLoad->getPointerTo());

  DPT0("VV0103: ", VV_VD, "Load_inst = Bldr->CreateLoad( finalPtr, 'DataVal' )\n");
...
void PCodeCfg::IR_StoreRelativeWithType(IRBldr_t *Bldr, jit_value_t val, jit_value_t tgt8ByPtr, long byteOffset,
  ...
  if (byteOffset == 0)
    ...
    DPT1("VV0201: ", VV_VD, "Tgt_8p2 = Bldr->CreateGEP( tgt8ByPtr, byOff = %ld ) \n", byteOffset);
    tgt8ByPtr2 = Bldr->CreateGEP(tgt8ByPtr, byOff);
  }

  DPT0("VV0202: ", VV_VD, "finalTgt = Bldr->CreatePointerCast(tgt8ByPtr2, typeToStore->getPointerTo() )\n");
  finalTgt = Bldr->CreatePointerCast(tgt8ByPtr2, typeToStore->getPointerTo());

  DPT0("VV0202: ", VV_VD, "Bldr->CreateStore( val, finalTgt )\n");
...
// as opposed to LIBJIT.
//
jit_value_t PCodeOperand::getJitValue(PCodeCfg *cfg, IRBldr_t *Bldr, jit_type_t IRtype, PCodeBlock *blk,
                                      PCodeOperand *orig = NULL, NABoolean noSave = FALSE) {
#if NExprDbgLvl >= VV_NO
  char NExBuf[500];
#endif
  ...
  if (IRtype == cfg->getInt1PtrTy()) {
    ...
    if (isVar() || isConst() || (!jitValue_ && orig->isNonNativeTemp())) {
...
#if NExprDbgLvl >= VV_BD
      if (isVar()) CDPT0("VV0323: ", VV_BD, "In getJitValue, found isVar() TRUE\n");
      if (isConst()) CDPT0("VV0324: ", VV_BD, "In getJitValue, found isConst() TRUE\n");
      if (isNonNativeTemp()) CDPT0("VV0325: ", VV_BD, "In getJitValue, found isNonNativeTemp() TRUE\n");
#endif
      ...
      if (getType() == PCIT::MATTR5) {
        ...
        if (!isVarchar() || (getVoaOffset() == -1)) {
          ...
          jit_value_t tmp1 = ConstantInt::get(cfg->getInt64Ty(), offset_ - vcIndicatorLen_);
          CDPT2("VV0330: ", VV_BD, "jitValuePtr_ = Bldr->CreateGEP( param, tmp1 = %d - %d )\n", offset_,
                vcIndicatorLen_);
          ...
          jit_value_t vcLen = cfg->IR_LoadRelativeWithType(Bldr, param, voaOffset_, MyTy);
          ...
          CDPT0("VV0334: ", VV_BD, "unsignedVcLen  = Bldr->CreateZExt( vcLen, cfg->getInt64Ty() , 'ZExtVcLen' )\n");
          unsignedVcLen = Bldr->CreateZExt(vcLen, cfg->getInt64Ty(), "ZExtVcLen");

          ////      jitValuePtr_ = jit_insn_add(f, param, v);

          CDPT0("VV0335: ", VV_BD, "jitValuePtr_ = Bldr->CreateGEP( param, unsignedVcLen )\n");
          ...
          if (vcNullIndicatorLen_) {
            ...
            CDPT0("VV0340: ", VV_BD, "jitValuePtr_ = Bldr->CreatePointerCast( jitValuePtr_, cfg->getInt8PtrTy() )\n");
            ...
            CDPT0("VV0341: ", VV_BD, "jitValuePtr_ = Bldr->CreateGEP( jitValuePtr_ , NI_len )\n");
            jitValuePtr_ = Bldr->CreateGEP(jitValuePtr_, NI_len);
          }
        }
        CDPT0("VV0345: ", VV_BD, "jitValuePtr_ = Bldr->CreatePointerCast( jitValuePtr_, cfg->getInt8PtrTy() )\n");
        jitValuePtr_ = Bldr->CreatePointerCast(jitValuePtr_, cfg->getInt8PtrTy());
        ...
        CDPT1("VV0348: ", VV_BD, "jitValuePtr_ = Bldr->CreateGEP( param, tmp1 = %d )\n", offset_);
        jitValuePtr_ = Bldr->CreateGEP(param, tmp1);
      }
      ...
      CDPT0("VV0355: ", VV_BD, "jitValuePtr_ = jitValue_ ; // USE jitValue_ AS ITS ADDRESS\n");
      jitValuePtr_ = jitValue_;  // For LLVM, we can use the Value ptr *AS* its address!

      CDPT2("VV0357: ", VV_BD,
            "jitValuePtr_ at %p is POINTING to Value rep by "
            "jitValue_ at %p\n",
    ...
    CDPT2("VV0361: ", VV_BD,
          "In getJitValue(): setting jitValuePtrBlock_ = %p "
          "for PCop = %p\n",
          blk, this);
  ...
  CDPT2("VV0368: ", VV_BD,
        "IRtype != cfg->getInt1PtrTy() for PCodeOp at %p "
        "with jitValue_ = %p\n",
        this, jitValue_);
  ...
  if (jitValue_) {
    ...
    CDPT2("VV0374: ", VV_BD, "In getJitValue(): valType->getTypeID() = %d, IRtype->getTypeID()=%d \n",
    ...
    if ((isTemp() || blk->onesVector.testBit(jitValueBlock_->getBlockNum())) &&
        ((valType->getTypeID() == IRtype->getTypeID()) ||
         (valType->isPointerTy() && valType->getContainedType(0)->getTypeID() == IRtype->getTypeID()) ||
         (isTemp() && valType->isPointerTy() && valType->getContainedType(0)->isPointerTy()))) {
      ...
      if (valType->isPointerTy()) {
        CDPT0("VV0378: ", VV_BD, "actualVal = Bldr->CreateLoad( jitValue_, 'loadTemp' )\n");
  ...
  if (isConst()) {
    ...
    if (IRtype == cfg->getInt64Ty()) {
      ...
      PCodeOperand *op = ((getType() == PCIT::MFLT64) || (getType() == PCIT::MFLT32) || (getType() == PCIT::MBIGS) ||
                          (getType() == PCIT::MBIGU))
                             ? orig
                             : this;
      ...
      CDPT2("VV0386: ", VV_BD, "In getJitValue(): setting jitValue_ in PCodeOp at %p to %p\n", this, jitValue_);
    ...
    CDPT0("VV0400: ", VV_BD, "param = Bldr->CreatePointerCast(parm1, cfg->getInt8PtrTy() )\n");
    jit_value_t param = Bldr->CreatePointerCast(parm1, cfg->getInt8PtrTy());
    ...
    if (nullBitIndex_ != -1) {
      ...
      jit_value_t mask = ConstantInt::get(cfg->getInt8Ty(), ((UInt8)0x1 << (7 - (nullBitIndex_ & 7))));

      ////  jit_value_t nByte = jit_insn_load_relative(f, param,
      ////                        offset_ + (nullBitIndex_ >> 3), jit_type_ubyte);

      jit_value_t nByte = cfg->IR_LoadRelativeWithType(Bldr, param, offset_ + (nullBitIndex_ >> 3), cfg->getInt8Ty());

      ////  jitValue_ = jit_insn_and(f, nByte, mask);

      CDPT1("VV0410: ", VV_BD, "jitValue_ = Bldr->CreateAnd( nByte, mask = 0x%x )\n",
            (UInt8)0x1 << (7 - (nullBitIndex_ & 7)));
      ...
      CDPT2("VV0415: ", VV_BD,
            "In getJitValue(): setting jitValue_ in PCodeOp at "
            "%p to %p\n",
            this, jitValue_);
    ...
    CDPT0("VV0420: ", VV_BD, "ABOUT TO DO assert (FALSE) -- undefined temp is not allowed\n");
  ...
  if (noSave) {
    ...
    CDPT1("VV0VV0422: ", VV_BD,
          "In getJitValue(): NULLing jitValue_ in PCodeOp at "
          "%p\n",
          this);
...
//
jit_value_t PCodeCfg::jitGenCompare(IRBldr_t *Bldr, enum cmpKind kindOfCompare, jit_value_t oper1, jit_value_t oper2,
  ...
  DPT2("VV0502: ", VV_XD,
       "In jitGenCompare(): oper1 has TypeID = "
       "%d, oper2 has %d\n",
       oper1->getType()->getTypeID(), oper2->getType()->getTypeID());

#if NExprDbgLvl >= VV_BD
  if ((NExprDbgLvl_ >= VV_XD) && (oper1->getType()->getTypeID() != oper2->getType()->getTypeID())) {
    DPT2("VV0504: ", VV_XD, "In jitGenCompare(): oper1 at %p, oper2 at %p\n", oper1, oper2);
  } else if (oper1->getType()->getTypeID() == Type::IntegerTyID) {
    if (oper1->getType()->getIntegerBitWidth() != oper2->getType()->getIntegerBitWidth()) {
      DPT2("VV0506: ", VV_XD, "In jitGenCompare(): oper1 width = %d, oper2 width = %d\n",
           oper1->getType()->getIntegerBitWidth(), oper2->getType()->getIntegerBitWidth());
      DPT2("VV0508: ", VV_XD, "In jitGenCompare(): oper1 at %p, oper2 at %p\n", oper1, oper2);
    }
  }
#endif
  switch (kindOfCompare) {
    case IntCompare_EQ: {
      ...
      // a 64-bit comparison!
        ...
        "IsZero = Bldr->CreateICmpEQ( oper1, oper2, 'ICmpEQ' )\n" );
         IsZero = Bldr->CreateICmpEQ( oper1, oper2, "ICmpEQ" );
#else
      DPT0("VV0510: ", VV_VD, "Rslt = Bldr->CreateXor( oper1, oper2, 'IXor' )\n");
      ...
      //
      DPT0("VV0511: ", VV_VD, "IsZero  = Bldr->CreateICmpEQ( Rslt, Zero, 'check0' )\n");
    ...
    case IntCompare_NE: {
      DPT0("VV0520: ", VV_VD, "IsZero = Bldr->CreateICmpNE( oper1, oper2, 'ICmpNE' )\n");
      IsZero = Bldr->CreateICmpNE(oper1, oper2, "ICmpNE");
      break;
    }
    case IntCompare_SGT: {
      DPT0("VV0523: ", VV_VD, "IsZero = Bldr->CreateICmpSGT( oper1, oper2, 'ICmpSGT' )\n");
      IsZero = Bldr->CreateICmpSGT(oper1, oper2, "ICmpSGT");
      break;
    }
    case IntCompare_UGT: {
      DPT0("VV0525: ", VV_VD, "IsZero = Bldr->CreateICmpUGT( oper1, oper2, 'ICmpUGT' )\n");
      IsZero = Bldr->CreateICmpUGT(oper1, oper2, "ICmpUGT");
      break;
    }
    case IntCompare_UGE: {
      DPT0("VV0526: ", VV_VD, "IsZero = Bldr->CreateICmpUGE( oper1, oper2, 'ICmpUGE' )\n");
      IsZero = Bldr->CreateICmpUGE(oper1, oper2, "ICmpUGE");
      break;
    }
    case IntCompare_SGE: {
      DPT0("VV0527: ", VV_VD, "IsZero = Bldr->CreateICmpSGE( oper1, oper2, 'ICmpSGE' )\n");
      IsZero = Bldr->CreateICmpSGE(oper1, oper2, "ICmpSGE");
      break;
    }
    case IntCompare_SLT: {
      DPT0("VV0528: ", VV_VD, "IsZero = Bldr->CreateICmpSLT( oper1, oper2, 'ICmpSLT' )\n");
      IsZero = Bldr->CreateICmpSLT(oper1, oper2, "ICmpSLT");
      break;
    }
    case IntCompare_SLE: {
      DPT0("VV0529: ", VV_VD, "IsZero = Bldr->CreateICmpSLE( oper1, oper2, 'ICmpSLE' )\n");
      IsZero = Bldr->CreateICmpSLE(oper1, oper2, "ICmpSLE");
      break;
    }
    case IntCompare_ULT: {
      DPT0("VV0530: ", VV_VD, "IsZero = Bldr->CreateICmpULT( oper1, oper2, 'ICmpULT' )\n");
      IsZero = Bldr->CreateICmpULT(oper1, oper2, "ICmpULT");
      break;
    }
    case IntCompare_ULE: {
      DPT0("VV0531: ", VV_VD, "IsZero = Bldr->CreateICmpULE( oper1, oper2, 'ICmpULE' )\n");
  ...
  DPT2("VV0551: ", VV_VD, "Bldr->CreateCondBr( IsZero, ThenBB = %p, ElseBB = %p )\n", ThenBB, ElseBB);
  Bldr->CreateCondBr(IsZero, ThenBB, ElseBB);

  DPT1("VV0552: ", VV_VD, "ExpFn->getBasicBlockList().push_back( ThenBB = %p )\n", ThenBB);
  ExpFn->getBasicBlockList().push_back(ThenBB);

  DPT1("VV0553: ", VV_VD, "Bldr->SetInsertPoint( ThenBB = %p )\n", ThenBB);
  ...
  DPT1("VV0555: ", VV_VD, "Bldr->CreateBr( MergeBB = %p )\n", MergeBB);
  Bldr->CreateBr(MergeBB);

  DPT1("VV0556: ", VV_VD, "ExpFn->getBasicBlockList().push_back( ElseBB = %p )\n", ElseBB);
  ExpFn->getBasicBlockList().push_back(ElseBB);

  DPT1("VV0557: ", VV_VD, "Bldr->SetInsertPoint( ElseBB = %p )\n", ElseBB);
  ...
  DPT1("VV0560: ", VV_VD, "Bldr->CreateBr( MergeBB = %p )\n", MergeBB);
  Bldr->CreateBr(MergeBB);

  // Now set up to insert code into the "merge back" common place
  DPT1("VV0561: ", VV_VD, "ExpFn->getBasicBlockList().push_back( MergeBB = %p )\n", MergeBB);
  ExpFn->getBasicBlockList().push_back(MergeBB);

  DPT1("VV0562: ", VV_VD, "Bldr->SetInsertPoint( MergeBB = %p )\n", MergeBB);
  ...
  //                         addIncoming(...)
  //
  DPT0("VV0570: ", VV_VD, "PN = Bldr->CreatePHI( trueRslt->getType(), 2, 'iftmp' )\n");
  PHINode *PN = Bldr->CreatePHI(trueRslt->getType(), 2, "iftmp");  // '2' is LLVM version # !!
  DPT2("VV0571: ", VV_VD, "PN->addIncoming( ThenVal at %p, from IR blk at %p )\n", ThenVal, ThenBB);
  PN->addIncoming(ThenVal, ThenBB);

  DPT2("VV0572: ", VV_VD, "PN->addIncoming( ElseVal at %p, from IR blk at %p )\n", ElseVal, ElseBB);
...
jit_type_t PCodeOperand::getJitType(PCodeCfg *cfg, PCIT::AddressingMode oprTyp) {
  ...
  if (oprTyp != PCIT::AM_NONE) OperTyp = oprTyp;

  switch (OperTyp) {
    case PCIT::MBIN8:
      return cfg->getInt8Ty();
    case PCIT::MBIN16U:
      return cfg->getInt16Ty();
    case PCIT::MBIN16S:
      return cfg->getInt16Ty();
    case PCIT::MBIN32U:
      return cfg->getInt32Ty();
    case PCIT::MBIN32S:
      return cfg->getInt32Ty();
    case PCIT::MBIN64S:
      return cfg->getInt64Ty();
    case PCIT::MASCII:
    case PCIT::MATTR5:
    case PCIT::MBIGS:
...
//
void PCodeCfg::genMemCopyLoop(IRBldr_t *Bldr, jit_value_t srcPtr, jit_value_t tgtPtr, jit_value_t maxLen) {
#if NExprDbgLvl > VV_NO
  char NExBuf[500];
#endif
  ...
  DPT0("VV0606: ", VV_VD, "RndMaxLen         = Bldr->CreateUDiv( MaxLen64, BytesPer, 'UDiv' )\n");
  RndMaxLen = Bldr->CreateUDiv(MaxLen64, BytesPer, "UDiv");

  DPT0("VV0607: ", VV_VD, "RndMaxLen         = Bldr->CreateMul( RndMaxLen, BytesPer, 'Mul' )\n");
  RndMaxLen = Bldr->CreateMul(RndMaxLen, BytesPer, "Mul");

  DPT0("VV0610: ", VV_VD, "AllocaInst *  ByteCt = Bldr->CreateAlloca( int64Ty_ )\n");
  AllocaInst *ByteCt = Bldr->CreateAlloca(int64Ty_);

  DPT0("VV0611: ", VV_VD, "Bldr->CreateStore( Zero, ByteCt )\n");
  Bldr->CreateStore(Zero, ByteCt);

  DPT0("VV0612: ", VV_VD, "CmpRslt = Bldr->CreateICmpULT( RndMaxLen, BytesPer, 'ICmp.RndMaxLen' )\n");
  CmpRslt = Bldr->CreateICmpULT(RndMaxLen, BytesPer, "ICmp.RndMaxLen");

  DPT0("VV0613: ", VV_VD, "Bldr->CreateCondBr( CmpRslt, endLabel, fastLoopLabel )\n");
  Bldr->CreateCondBr(CmpRslt, endLabel, fastLoopLabel);

  // Start fast loop here

  DPT1("VV0615: ", VV_VD, "ExpFn->getBasicBlockList().push_back( fastLoopLabel = %p )\n", fastLoopLabel);
  ExpFn->getBasicBlockList().push_back(fastLoopLabel);

  DPT0("VV0616: ", VV_VD, "Bldr->SetInsertPoint( fastLoopLabel )\n");
  Bldr->SetInsertPoint(fastLoopLabel);

  // Copy multiple bytes at a time.
  DPT0("VV0617: ", VV_VD, "ByteCtVal = Bldr->CreateLoad( ByteCt, 'loadByteCt' )\n");
  ByteCtVal = Bldr->CreateLoad(ByteCt, "loadByteCt");  // Re-load within loop

  DPT0("VV0618: ", VV_VD, "ThisSrc = Bldr->CreateGEP( srcPtr, ByteCtVal, 'ThisSrc' )\n");
  jit_value_t ThisSrc = Bldr->CreateGEP(srcPtr, ByteCtVal, "ThisSrc");

  DPT0("VV0619: ", VV_VD, "ThisTgt = Bldr->CreateGEP( tgtPtr, ByteCtVal, 'ThisTgt' )\n");
  jit_value_t ThisTgt = Bldr->CreateGEP(tgtPtr, ByteCtVal, "ThisTgt");

  jit_value_t MultiBytes = IR_LoadRelativeWithType(Bldr, ThisSrc, 0, int64Ty_);
  IR_StoreRelativeWithType(Bldr, MultiBytes, ThisTgt, 0, int64Ty_);

  // Increment count of bytes copyied
  DPT0("VV0621: ", VV_VD, "ByteCtVal = Bldr->CreateAdd( ByteCtVal, BytesPer, 'nextIncr')\n");
  ByteCtVal = Bldr->CreateAdd(ByteCtVal, BytesPer, "nextIncr");

  DPT0("VV0622: ", VV_VD, "Bldr->CreateStore( ByteCtVal, ByteCt, 'storeByteCt' )\n");
  Bldr->CreateStore(ByteCtVal, ByteCt, "storeByteCt");

  // while (ByteCtVal < RndMaxLen) branch to fastLoop
  DPT0("VV0623: ", VV_VD, "CmpRslt = Bldr->CreateICmpULT( ByteCtVal, RndMaxLen, 'ICmp.ByteCt' )\n");
  CmpRslt = Bldr->CreateICmpULT(ByteCtVal, RndMaxLen, "ICmp.ByteCt");

  DPT0("VV0624: ", VV_VD, "Bldr->CreateCondBr( CmpRslt, fastLoopLabel, endLabel )\n");
  Bldr->CreateCondBr(CmpRslt, fastLoopLabel, endLabel);

  DPT1("VV0625: ", VV_VD, "ExpFn->getBasicBlockList().push_back( endLabel = %p )\n", endLabel);
  ExpFn->getBasicBlockList().push_back(endLabel);

  DPT0("VV0626: ", VV_VD, "Bldr->SetInsertPoint( endLabel )\n");
  ...
  DPT0("VV0630: ", VV_VD, "ByteCtVal = Bldr->CreateLoad( ByteCt, 'loadByteCt' )\n");
  ByteCtVal = Bldr->CreateLoad(ByteCt, "loadByteCt");

  DPT0("VV0632: ", VV_VD, "CmpRslt = Bldr->CreateICmpUGE( ByteCtVal, MaxLen64, 'ICmp.RndMaxLen' )\n");
  CmpRslt = Bldr->CreateICmpUGE(ByteCtVal, MaxLen64, "ICmp.RndMaxLen");

  DPT0("VV0633: ", VV_VD, "Bldr->CreateCondBr( CmpRslt, end2Label, slowLoopLabel )\n");
  Bldr->CreateCondBr(CmpRslt, end2Label, slowLoopLabel);

  // Start slow loop here

  DPT1("VV0635: ", VV_VD, "ExpFn->getBasicBlockList().push_back( slowLoopLabel = %p )\n", slowLoopLabel);
  ExpFn->getBasicBlockList().push_back(slowLoopLabel);

  DPT0("VV0636: ", VV_VD, "Bldr->SetInsertPoint( slowLoopLabel )\n");
  Bldr->SetInsertPoint(slowLoopLabel);

  // Copy multiple bytes at a time.
  DPT0("VV0637: ", VV_VD, "ByteCtVal = Bldr->CreateLoad( ByteCt, 'loadByteCt' )\n");
  ByteCtVal = Bldr->CreateLoad(ByteCt, "loadByteCt");  // Re-load within the loop

  DPT0("VV0638: ", VV_VD, "CurrSrc = Bldr->CreateGEP( srcPtr, ByteCtVal, 'CurrSrc' )\n");
  jit_value_t CurrSrc = Bldr->CreateGEP(srcPtr, ByteCtVal, "CurrSrc");

  DPT0("VV0639: ", VV_VD, "CurrTgt = Bldr->CreateGEP( tgtPtr, ByteCtVal, 'CurrTgt' )\n");
  jit_value_t CurrTgt = Bldr->CreateGEP(tgtPtr, ByteCtVal, "CurrTgt");

  jit_value_t OneByte = IR_LoadRelativeWithType(Bldr, CurrSrc, 0, int8Ty_);
  IR_StoreRelativeWithType(Bldr, OneByte, CurrTgt, 0, int8Ty_);

  // Increment loop count
  DPT0("VV0641: ", VV_VD, "ByteCtVal = Bldr->CreateAdd( ByteCtVal, BytesPer, 'nextIncr')\n");
  ByteCtVal = Bldr->CreateAdd(ByteCtVal, BytesPer, "nextIncr");

  DPT0("VV0642: ", VV_VD, "Bldr->CreateStore( ByteCtVal, ByteCt, 'storeByteCt' )\n");
  Bldr->CreateStore(ByteCtVal, ByteCt, "storeByteCt");

  // while (ByteCtVal < MaxLen64) branch to slowLoop
  DPT0("VV0643: ", VV_VD, "CmpRslt = Bldr->CreateICmpULT( ByteCtVal, MaxLen64, 'ICmp.ByteCt' )\n");
  CmpRslt = Bldr->CreateICmpULT(ByteCtVal, MaxLen64, "ICmp.ByteCt");

  DPT0("VV0644: ", VV_VD, "Bldr->CreateCondBr( CmpRslt, slowLoopLabel, end2Label )\n");
  Bldr->CreateCondBr(CmpRslt, slowLoopLabel, end2Label);

  DPT1("VV0645: ", VV_VD, "ExpFn->getBasicBlockList().push_back( end2Label = %p )\n", end2Label);
  ExpFn->getBasicBlockList().push_back(end2Label);

  DPT0("VV0646: ", VV_VD, "Bldr->SetInsertPoint( end2Label )\n");
...
//
jit_value_t PCodeCfg::genMemCmpLoop(IRBldr_t *Bldr, enum cmpKind KindOfCmp, jit_value_t src1Ptr, jit_value_t src2Ptr,
                                    jit_value_t maxLen) {
#if NExprDbgLvl > VV_NO
  char NExBuf[500];
#endif
  ...
  DPT0("VVa606: ", VV_VD, "RndMaxLen         = Bldr->CreateSub( MaxLen64, One, 'Sub1' )\n");
  RndMaxLen = Bldr->CreateSub(MaxLen64, One, "Sub1");

  DPT0("VVa607: ", VV_VD, "RndMaxLen         = Bldr->CreateUDiv( RndMaxLen, BytesPer, 'UDiv' )\n");
  RndMaxLen = Bldr->CreateUDiv(RndMaxLen, BytesPer, "UDiv");

  DPT0("VVa608: ", VV_VD, "RndMaxLen         = Bldr->CreateMul( RndMaxLen, BytesPer, 'Mul' )\n");
  RndMaxLen = Bldr->CreateMul(RndMaxLen, BytesPer, "Mul");

  DPT0("VVa610: ", VV_VD, "AllocaInst *  ByteCt = Bldr->CreateAlloca( int64Ty_ )\n");
  AllocaInst *ByteCt = Bldr->CreateAlloca(int64Ty_);

  DPT0("VVa611: ", VV_VD, "Bldr->CreateStore( Zero, ByteCt )\n");
  Bldr->CreateStore(Zero, ByteCt);
  ...
  //
  DPT0("VVa612: ", VV_VD, "Is_LT = Bldr->CreateICmpSLT( RndMaxLen, BytesPer, 'ICmp.RndMaxLen' )\n");
  Is_LT = Bldr->CreateICmpSLT(RndMaxLen, BytesPer, "ICmp.RndMaxLen");

  DPT0("VVa613: ", VV_VD, "Bldr->CreateCondBr( Is_LT, slowCmpLpLabel, fastCmpLpLabel )\n");
  Bldr->CreateCondBr(Is_LT, slowCmpLpLabel, fastCmpLpLabel);

  // Start fast loop here

  DPT1("VVa615: ", VV_VD, "ExpFn->getBasicBlockList().push_back( fastCmpLpLabel = %p )\n", fastCmpLpLabel);
  ExpFn->getBasicBlockList().push_back(fastCmpLpLabel);

  DPT1("VVa616: ", VV_VD, "Bldr->SetInsertPoint( fastCmpLpLabel = %p )\n", fastCmpLpLabel);
  Bldr->SetInsertPoint(fastCmpLpLabel);

  // Copy multiple bytes at a time.
  DPT0("VVa617: ", VV_VD, "ByteCtVal = Bldr->CreateLoad( ByteCt, 'loadByteCt' )\n");
  ByteCtVal = Bldr->CreateLoad(ByteCt, "loadByteCt");  // Re-load within loop

  DPT0("VVa618: ", VV_VD, "ThisSrc1 = Bldr->CreateGEP( src1Ptr, ByteCtVal, 'ThisSrc1' )\n");
  jit_value_t ThisSrc1 = Bldr->CreateGEP(src1Ptr, ByteCtVal, "ThisSrc1");

  DPT0("VVa619: ", VV_VD, "ThisSrc2 = Bldr->CreateGEP( src2Ptr, ByteCtVal, 'ThisSrc2' )\n");
  jit_value_t ThisSrc2 = Bldr->CreateGEP(src2Ptr, ByteCtVal, "ThisSrc2");

  jit_value_t MultiBytes1 = IR_LoadRelativeWithType(Bldr, ThisSrc1, 0, int64Ty_);
  jit_value_t MultiBytes2 = IR_LoadRelativeWithType(Bldr, ThisSrc2, 0, int64Ty_);
  // NOTE: Always compare for EQ in the fast loop.
  // The slow loop will do the real comparison -- once we find a difference
  DPT0("VVa619a: ", VV_VD, "IsZero = Bldr->CreateICmpEQ( MultiBytes1, MultiBytes2, 'cmp8Bytes' )\n");
  IsZero = Bldr->CreateICmpEQ(MultiBytes1, MultiBytes2, "cmp8Bytes");

  DPT2("VVa620: ", VV_VD, "Bldr->CreateCondBr( IsZero, fastCmpNxtLabel = %p, slowCmpLpLabel = %p )\n", fastCmpNxtLabel,
       slowCmpLpLabel);
  Bldr->CreateCondBr(IsZero, fastCmpNxtLabel, slowCmpLpLabel);

  DPT1("VVa620a: ", VV_VD, "ExpFn->getBasicBlockList().push_back( fastCmpNxtLabel = %p )\n", fastCmpNxtLabel);
  ExpFn->getBasicBlockList().push_back(fastCmpNxtLabel);

  DPT1("VVa620b: ", VV_VD, "Bldr->SetInsertPoint( fastCmpNxtLabel = %p )\n", fastCmpNxtLabel);
  Bldr->SetInsertPoint(fastCmpNxtLabel);

  // Increment count of bytes copied
  DPT0("VVa621: ", VV_VD, "ByteCtVal = Bldr->CreateAdd( ByteCtVal, BytesPer, 'nextIncr')\n");
  ByteCtVal = Bldr->CreateAdd(ByteCtVal, BytesPer, "nextIncr");

  DPT0("VVa622: ", VV_VD, "Bldr->CreateStore( ByteCtVal, ByteCt, 'storeByteCt' )\n");
  Bldr->CreateStore(ByteCtVal, ByteCt, "storeByteCt");

  // while (ByteCtVal < RndMaxLen) branch to fastLoop
  DPT0("VVa623: ", VV_VD, "Is_LT = Bldr->CreateICmpULT( ByteCtVal, RndMaxLen, 'ICmp.ByteCt' )\n");
  Is_LT = Bldr->CreateICmpULT(ByteCtVal, RndMaxLen, "ICmp.ByteCt");

  DPT0("VVa624: ", VV_VD, "Bldr->CreateCondBr( Is_LT, fastCmpLpLabel, slowCmpLpLabel )\n");
  ...
  DPT1("VVa635: ", VV_VD, "ExpFn->getBasicBlockList().push_back( slowCmpLpLabel = %p )\n", slowCmpLpLabel);
  ExpFn->getBasicBlockList().push_back(slowCmpLpLabel);

  DPT1("VVa636: ", VV_VD, "Bldr->SetInsertPoint( slowCmpLpLabel = %p )\n", slowCmpLpLabel);
  ...
  DPT0("VVa637: ", VV_VD, "ByteCtVal = Bldr->CreateLoad( ByteCt, 'loadByteCt' )\n");
  ByteCtVal = Bldr->CreateLoad(ByteCt, "loadByteCt");  // Re-load within the loop

  DPT0("VVa638: ", VV_VD, "CurrSrc1 = Bldr->CreateGEP( src1Ptr, ByteCtVal, 'CurrSrc1' )\n");
  jit_value_t CurrSrc1 = Bldr->CreateGEP(src1Ptr, ByteCtVal, "CurrSrc1");

  DPT0("VVa639: ", VV_VD, "CurrSrc2 = Bldr->CreateGEP( src2Ptr, ByteCtVal, 'CurrSrc2' )\n");
  jit_value_t CurrSrc2 = Bldr->CreateGEP(src2Ptr, ByteCtVal, "CurrSrc2");

  jit_value_t OneByte1 = IR_LoadRelativeWithType(Bldr, CurrSrc1, 0, int8Ty_);
  jit_value_t OneByte2 = IR_LoadRelativeWithType(Bldr, CurrSrc2, 0, int8Ty_);
  switch (KindOfCmp) {
    default:
    case ByteCompare_EQ:
      DPT0("VVa640eq: ", VV_VD, "IsZero = Bldr->CreateICmpEQ( OneByte1, OneByte2, 'cmp1ByteEQ' )\n");
      IsZero = Bldr->CreateICmpEQ(OneByte1, OneByte2, "cmp1ByteEQ");
      break;
    case ByteCompare_NE:
      DPT0("VVa640ne: ", VV_VD, "IsZero = Bldr->CreateICmpNE( OneByte1, OneByte2, 'cmp1ByteNE' )\n");
      IsZero = Bldr->CreateICmpNE(OneByte1, OneByte2, "cmp1ByteNE");
      break;
    case ByteCompare_LT:
      DPT0("VVa640ult: ", VV_VD, "IsZero = Bldr->CreateICmpULT( OneByte1, OneByte2, 'cmp1ByteLT' )\n");
      IsZero = Bldr->CreateICmpULT(OneByte1, OneByte2, "cmp1ByteLT");
      break;
    case ByteCompare_LE:
      DPT0("VVa640ule: ", VV_VD, "IsZero = Bldr->CreateICmpULE( OneByte1, OneByte2, 'cmp1ByteLE' )\n");
      IsZero = Bldr->CreateICmpULE(OneByte1, OneByte2, "cmp1ByteLE");
      break;
    case ByteCompare_GT:
      DPT0("VVa640ugt: ", VV_VD, "IsZero = Bldr->CreateICmpUGT( OneByte1, OneByte2, 'cmp1ByteGT' )\n");
      IsZero = Bldr->CreateICmpUGT(OneByte1, OneByte2, "cmp1ByteGT");
      break;
    case ByteCompare_GE:
      DPT0("VVa640uge: ", VV_VD, "IsZero = Bldr->CreateICmpUGE( OneByte1, OneByte2, 'cmp1ByteGE' )\n");
      IsZero = Bldr->CreateICmpUGE(OneByte1, OneByte2, "cmp1ByteGE");
      break;
  }

  DPT0("VVa641: ", VV_VD, "IsEqual = Bldr->CreateICmpEQ( OneByte1, OneByte2, 'cmp1ByteEQ' )");
  IsEqual = Bldr->CreateICmpEQ(OneByte1, OneByte2, "cmp1ByteEQ");

  DPT2("VVa642: ", VV_VD, "Bldr->CreateCondBr( IsEqual, slowContLabel = %p, slowCmpEndLabel = %p )\n", slowContLabel,
       slowCmpEndLabel);
  Bldr->CreateCondBr(IsEqual, slowContLabel, slowCmpEndLabel);

  DPT1("VVa642a: ", VV_VD, "ExpFn->getBasicBlockList().push_back( slowContLabel = %p )\n", slowContLabel);
  ExpFn->getBasicBlockList().push_back(slowContLabel);

  DPT1("VVa642b: ", VV_VD, "Bldr->SetInsertPoint( slowContLabel = %p )\n", slowContLabel);
  Bldr->SetInsertPoint(slowContLabel);

  // Increment loop count
  DPT0("VVa643: ", VV_VD, "ByteCtVal = Bldr->CreateAdd( ByteCtVal, BytesPer, 'nextIncr')\n");
  ByteCtVal = Bldr->CreateAdd(ByteCtVal, BytesPer, "nextIncr");

  DPT0("VVa644: ", VV_VD, "Bldr->CreateStore( ByteCtVal, ByteCt, 'storeByteCt' )\n");
  Bldr->CreateStore(ByteCtVal, ByteCt, "storeByteCt");

  // while (ByteCtVal < MaxLen64) branch to slowLoop
  DPT0("VVa645: ", VV_VD, "Is_LT = Bldr->CreateICmpULT( ByteCtVal, MaxLen64, 'ICmp.ByteCt' )\n");
  Is_LT = Bldr->CreateICmpULT(ByteCtVal, MaxLen64, "ICmp.ByteCt");

  DPT0("VVa646: ", VV_VD, "Bldr->CreateCondBr( Is_LT, slowCmpLpLabel, slowCmpEndLabel )\n");
  Bldr->CreateCondBr(Is_LT, slowCmpLpLabel, slowCmpEndLabel);

  DPT1("VVa647: ", VV_VD, "ExpFn->getBasicBlockList().push_back( slowCmpEndLabel = %p )\n", slowCmpEndLabel);
  ExpFn->getBasicBlockList().push_back(slowCmpEndLabel);

  DPT1("VVa648: ", VV_VD, "Bldr->SetInsertPoint( slowCmpEndLabel = %p )\n", slowCmpEndLabel);
...
void PCodeOperand::allocJitValue(PCodeCfg *cfg, IRBldr_t *Bldr, PCIT::AddressingMode OprTyp, int Len, int Num) {
  ...
  if (operand != this) return operand->allocJitValue(cfg, Bldr, OprTyp, Len, Num);
...
// FIX: Overlapped types can have problems too
jit_value_t PCodeOperand::getJitValue(PCodeCfg *cfg, jit_function_t f, jit_type_t type, PCodeBlock *b,
                                      PCodeOperand *orig = NULL, NABoolean noSave = FALSE) {
  // Get operand in hash table
  CollIndex i = bvIndex_;
  PCodeOperand *operand = cfg->getMap()->getFirstValue(&i);
  if (operand != this) return operand->getJitValue(cfg, f, type, b, this, noSave);
  ...
  if (type == jit_type_void_ptr) {
    ...
    if (isVar() || isConst() || (!jitValue_ && orig->isNonNativeTemp())) {
      ...
      if (getType() == PCIT::MATTR5) {
        ...
        // we don't need to go through the VOA offset.
        if (!isVarchar() || (getVoaOffset() == -1)) {
          jitValuePtr_ = jit_insn_add_relative(f, param, (offset_ - vcIndicatorLen_));
        } else {
          jit_type_t t = ((vcIndicatorLen_ == 2) ? jit_type_ushort : jit_type_uint);
          ...
          // Now set up pointer, moving past null indicator if required.
          if (vcNullIndicatorLen_) jitValuePtr_ = jit_insn_add_relative(f, jitValuePtr_, vcNullIndicatorLen_);
        }
      } else
        jitValuePtr_ = jit_insn_add_relative(f, param, offset_);
    } else {
  ...
  if (jitValue_) {
    ...
    if (isTemp() || b->onesVector.testBit(jitValueBlock_->getBlockNum())) {
      ...
      // an overlap :()
      jit_type_t jitValType = jit_value_get_type(jitValue_);
      if (jit_type_get_size(jitValType) > jit_type_get_size(type)) return jit_insn_convert(f, jitValue_, type, 0);
  ...
  if (isConst()) {
    ...
    if (nullBitIndex_ != -1) {
      ...
      jit_value_t mask = jit_value_create_nint_constant(f, jit_type_ubyte, (UInt8)0x1 << (7 - (nullBitIndex_ & 7)));

      jit_value_t nByte = jit_insn_load_relative(f, param, offset_ + (nullBitIndex_ >> 3), jit_type_ubyte);
      jitValue_ = jit_insn_and(f, nByte, mask);
    } else
      jitValue_ = jit_insn_load_relative(f, param, offset_, type);
  } else {
...
//
jit_value_t PCodeCfg::genStringSetup(jit_function_t f, jit_value_t *tStr, PCodeOperand *src,
  ...
  if (padExists) {
    ...
    jit_value_t spaceJitVal = jit_value_create_nint_constant(f, jit_type_ubyte, ' ');
...
jit_value_t PCodeCfg::genHash(jit_function_t f, jit_value_t tStr, jit_value_t tStrLen, jit_value_t loopIndex,
  ...
  if ((tStrLen == NULL) && (constantLen > 32)) tStrLen = jit_value_create_nint_constant(f, jit_type_int, constantLen);
...
void PCodeCfg::genMemSetLoop(IRBldr_t *Bldr, int val, jit_value_t tgtPtr, jit_value_t maxLen) {
  ...
  DPT0("VV0706: ", VV_VD, "RndMaxLen = Bldr->CreateUDiv( MaxLen64, BytesPer, 'UDiv' )\n");
  RndMaxLen = Bldr->CreateUDiv(MaxLen64, BytesPer, "UDiv");

  DPT0("VV0707: ", VV_VD, "RndMaxLen = Bldr->CreateMul( RndMaxLen, BytesPer, 'Mul' )\n");
  RndMaxLen = Bldr->CreateMul(RndMaxLen, BytesPer, "Mul");

  DPT0("VV0710: ", VV_VD, "AllocaInst * ByteCt = Bldr->CreateAlloca( int64Ty_ )\n");
  AllocaInst *ByteCt = Bldr->CreateAlloca(int64Ty_);

  DPT0("VV0711: ", VV_VD, "Bldr->CreateStore( Zero, ByteCt )\n");
  Bldr->CreateStore(Zero, ByteCt);

  DPT0("VV0712: ", VV_VD, "CmpRslt = Bldr->CreateICmpULT( RndMaxLen, BytesPer, 'ICmp.RndMaxLen' )\n");
  CmpRslt = Bldr->CreateICmpULT(RndMaxLen, BytesPer, "ICmp.RndMaxLen");

  DPT0("VV0713: ", VV_VD, "Bldr->CreateCondBr( CmpRslt, endLabel, fastLoopLabel )\n");
  Bldr->CreateCondBr(CmpRslt, endLabel, fastLoopLabel);

  // Start fast loop here

  DPT1("VV0715: ", VV_VD, "ExpFn->getBasicBlockList().push_back( fastLoopLabel = %p )\n", fastLoopLabel);
  ExpFn->getBasicBlockList().push_back(fastLoopLabel);

  DPT0("VV0716: ", VV_VD, "Bldr->SetInsertPoint( fastLoopLabel )\n");
  Bldr->SetInsertPoint(fastLoopLabel);

  // Copy multiple bytes at a time.
  DPT0("VV0718: ", VV_VD, "ByteCtVal = Bldr->CreateLoad( ByteCt, 'loadByteCt' )\n");
  ByteCtVal = Bldr->CreateLoad(ByteCt, "loadByteCt");  // Re-load within the loop

  DPT0("VV0719: ", VV_VD, "ThisTgt = Bldr->CreateGEP( tgtPtr, ByteCtVal, 'ThisTgt' )\n");
  jit_value_t ThisTgt = Bldr->CreateGEP(tgtPtr, ByteCtVal, "ThisTgt");

  IR_StoreRelativeWithType(Bldr, MultiBytes, ThisTgt, 0, int64Ty_);

  // Increment count of bytes handled
  DPT0("VV0721: ", VV_VD, "ByteCtVal = Bldr->CreateAdd( ByteCtVal, BytesPer, 'nextIncr')\n");
  ByteCtVal = Bldr->CreateAdd(ByteCtVal, BytesPer, "nextIncr");

  DPT0("VV0722: ", VV_VD, "Bldr->CreateStore( ByteCtVal, ByteCt, 'storeByteCt' )\n");
  Bldr->CreateStore(ByteCtVal, ByteCt, "storeByteCt");

  // while (ByteCtVal < RndMaxLen) branch to fastLoop
  DPT0("VV0723: ", VV_VD, "CmpRslt = Bldr->CreateICmpULT( ByteCtVal, RndMaxLen, 'ICmp.ByteCt' )\n");
  CmpRslt = Bldr->CreateICmpULT(ByteCtVal, RndMaxLen, "ICmp.ByteCt");

  DPT0("VV0724: ", VV_VD, "Bldr->CreateCondBr( CmpRslt, fastLoopLabel, endLabel )\n");
  Bldr->CreateCondBr(CmpRslt, fastLoopLabel, endLabel);

  DPT1("VV0725: ", VV_VD, "ExpFn->getBasicBlockList().push_back( endLabel = %p )\n", endLabel);
  ExpFn->getBasicBlockList().push_back(endLabel);

  DPT0("VV0726: ", VV_VD, "Bldr->SetInsertPoint( endLabel )\n");
  Bldr->SetInsertPoint(endLabel);
  ...
  DPT0("VV0730: ", VV_VD, "ByteCtVal = Bldr->CreateLoad( ByteCt, 'loadByteCt' )\n");
  ByteCtVal = Bldr->CreateLoad(ByteCt, "loadByteCt");

  DPT0("VV0732: ", VV_VD, "CmpRslt = Bldr->CreateICmpUGE( ByteCtVal, MaxLen64, 'ICmp.RndMaxLen' )\n");
  CmpRslt = Bldr->CreateICmpUGE(ByteCtVal, MaxLen64, "ICmp.RndMaxLen");

  DPT0("VV0733: ", VV_VD, "Bldr->CreateCondBr( CmpRslt, end2Label, slowLoopLabel )\n");
  Bldr->CreateCondBr(CmpRslt, end2Label, slowLoopLabel);

  // Start slow loop here

  DPT1("VV0735: ", VV_VD, "ExpFn->getBasicBlockList().push_back( slowLoopLabel = %p )\n", slowLoopLabel);
  ExpFn->getBasicBlockList().push_back(slowLoopLabel);

  DPT0("VV0736: ", VV_VD, "Bldr->SetInsertPoint( slowLoopLabel )\n");
  Bldr->SetInsertPoint(slowLoopLabel);

  // Copy multiple bytes at a time.
  DPT0("VV0738: ", VV_VD, "ByteCtVal = Bldr->CreateLoad( ByteCt, 'loadByteCt' )\n");
  ByteCtVal = Bldr->CreateLoad(ByteCt, "loadByteCt");  // Re-load within the loop

  DPT0("VV0739: ", VV_VD, "CurrTgt = Bldr->CreateGEP( tgtPtr, ByteCtVal, 'CurrTgt' )\n");
  jit_value_t CurrTgt = Bldr->CreateGEP(tgtPtr, ByteCtVal, "CurrTgt");

  IR_StoreRelativeWithType(Bldr, OneByte, CurrTgt, 0, int8Ty_);

  // Increment count of bytes handled
  DPT0("VV0740: ", VV_VD, "ByteCtVal = Bldr->CreateAdd( ByteCtVal, BytesPer, 'nextIncr')\n");
  ByteCtVal = Bldr->CreateAdd(ByteCtVal, BytesPer, "nextIncr");

  DPT0("VV0742: ", VV_VD, "Bldr->CreateStore( ByteCtVal, ByteCt, 'storeByteCt' )\n");
  Bldr->CreateStore(ByteCtVal, ByteCt, "storeByteCt");

  // while (ByteCtVal < MaxLen64) branch to slowLoop
  DPT0("VV0743: ", VV_VD, "CmpRslt = Bldr->CreateICmpULT( ByteCtVal, MaxLen64, 'ICmp.ByteCt' )\n");
  CmpRslt = Bldr->CreateICmpULT(ByteCtVal, MaxLen64, "ICmp.ByteCt");

  DPT0("VV0744: ", VV_VD, "Bldr->CreateCondBr( CmpRslt, slowLoopLabel, end2Label )\n");
  Bldr->CreateCondBr(CmpRslt, slowLoopLabel, end2Label);

  DPT1("VV0745: ", VV_VD, "ExpFn->getBasicBlockList().push_back( end2Label = %p )\n", end2Label);
  ExpFn->getBasicBlockList().push_back(end2Label);

  DPT0("VV0746: ", VV_VD, "Bldr->SetInsertPoint( end2Label )\n");
  Bldr->SetInsertPoint(end2Label);
}
...
void PCodeCfg::genUnalignedMemset(IRBldr_t *Bldr, jit_value_t tgtPtr, int val, int length) {
  ...
  if (length < 32) {
    ...
    for (i = 0; length >= 8; i += 8, length -= 8) {
      ...
      DPT0("VV0750: ", VV_VD, "IR_StoreRelativeWithType( Bldr, jVal8, tgtPtr, i, int64Ty_ )\n");
    ...
    for (i = 0; length >= 4; i += 4, length -= 4) {
      ...
      DPT0("VV0760: ", VV_VD, "IR_StoreRelativeWithType( Bldr, jVal4, tgtPtr, i, int32Ty_ )\n");
    ...
    for (; length >= 2; i += 2, length -= 2) {
      ...
      DPT0("VV0770: ", VV_VD, "IR_StoreRelativeWithType( Bldr, jVal2, tgtPtr, i, int16Ty_ )\n");
    ...
    for (; length >= 1; i += 1, length -= 1) {
      ...
      DPT0("VV0780: ", VV_VD, "IR_StoreRelativeWithType( Bldr, jVal1, tgtPtr, i, int8Ty_ )\n");
...
void PCodeCfg::genUnalignedMemcpy(IRBldr_t *Bldr, jit_value_t tgt, jit_value_t src, jit_value_t len,
  ...
  if ((len == NULL) && (constantLen < 32)) {
    ...
    for (i = 0; length >= 8; i += 8, length -= 8) {
      ...
      DPT0("VV0812: ", VV_VD, "IR_StoreRelativeWithType( Bldr, val, tgt, i, int64Ty_ )\n");
    ...
    for (; length >= 4; i += 4, length -= 4) {
      ...
      DPT0("VV0814: ", VV_VD, "IR_StoreRelativeWithType( Bldr, val, tgt, i, int32Ty_ )\n");
    ...
    for (; length >= 2; i += 2, length -= 2) {
      ...
      DPT0("VV0816: ", VV_VD, "IR_StoreRelativeWithType( Bldr, val, tgt, i, int16Ty_ )\n");
    ...
    for (; length >= 1; i += 1, length -= 1) {
      ...
      DPT0("VV0819: ", VV_VD, "IR_StoreRelativeWithType( Bldr, val, tgt, i, int8Ty_ )\n");
  ...
  {
    DPT0("VV0820: ", VV_BD, "GOT into genUnalignedMemcpy() with long fixed-length string\n");
  ...
  {
    DPT0("VV0830: ", VV_BD, "GOT into genUnalignedMemcpy() with len != NULL ptr\n");
...
void PCodeCfg::genBignumSub(PCodeCfg *cfg, jit_function_t f, PCodeOperand *res, jit_value_t src1, jit_value_t src2,
  ...
  jit_value_t jitValMask16 = jit_value_create_nint_constant(f, jit_type_ushort, 0x8000);
  jit_value_t jitValNotMask16 = jit_value_create_nint_constant(f, jit_type_ushort, 0x7fff);
  jit_value_t jitValNotMask32 = jit_value_create_nint_constant(f, jit_type_ushort, 0x7fffffff);
...
// FIX: Overlapped types can have problems too
void PCodeOperand::storeJitValue(PCodeCfg *cfg, IRBldr_t *Bldr, jit_type_t IRtype, jit_value_t value, PCodeBlock *b,
                                 jit_value_t *len = NULL, NABoolean noSave = FALSE) {
#if NExprDbgLvl >= VV_NO
  char NExBuf[500];
#endif
  CDPT3("VV0900: ", VV_BD,
        "In storeJitValue() on PCode Operand at %p, IRtype=%p, "
        "value=%p\n",
        this, IRtype, value);
  ...
  if (operand != this) return operand->storeJitValue(cfg, Bldr, IRtype, value, b, len, noSave);
  ...
  //
  if (valType->isPointerTy()) {
    jit_value_t size = ((len) ? *len : ConstantInt::get(cfg->getInt64Ty(), getLen()));
    ...
    if (jitValuePtr_) {
      ...
      // then we are assured that jitValuePtr_ is valid.
      //
      if (!b->onesVector.testBit(jitValuePtrBlock_->getBlockNum())) jitValuePtr_ = NULL;
    }
    ...
    if (!jitValuePtr_) {
      ...
      if (isVar() || isNonNativeTemp()) {
        CDPT1("VV0908: ", VV_BD,
              "In storeJitValue, getting param using stackIndex_ = "
              "%d\n",
              stackIndex_);

        jit_value_t parm1 = cfg->getJitParams()[stackIndex_];

        CDPT0("VV0910: ", VV_BD, "param  = Bldr->CreatePointerCast(parm1, cfg->getInt8PtrTy() )\n");
        jit_value_t param = Bldr->CreatePointerCast(parm1, cfg->getInt8PtrTy());

        ////    jitValuePtr_ = jit_insn_add_relative(f, param, offset_); */

        CDPT1("VV0912: ", VV_BD,
              "In storeJitValue, getting param using offset_ = "
              "%d\n",
              offset_);

        jit_value_t tmp1 = ConstantInt::get(cfg->getInt64Ty(), offset_);

        CDPT1("VV0914: ", VV_BD, "jitValuePtr_ = Bldr->CreateGEP( param, tmp1 = %d )\n", offset_);
        jitValuePtr_ = Bldr->CreateGEP(param, tmp1);
      } else if (jitValue_)  // If space already allocated
      {
        CDPT0("VV0918: ", VV_BD, "jitValuePtr_ = Bldr->CreateGEP( jitValue_, cfg->getZeroJitVal() )\n");
      ...
      CDPT2("VV0919: ", VV_BD,
            "In storeJitValue(): setting jitValuePtrBlock_ = %p "
            "for PCop = %p\n",
            b, this);
      jitValuePtrBlock_ = b;
    ...
    CDPT1("VV0920: ", VV_BD,
          "In storeJitValue, calling genUnalignedMemcpy() "
          "with length=%d\n",
          getLen());
    ...
    if (noSave) {
      ...
      if (jitValue_)
        CDPT2("VV0925: ", VV_BD,
              "In storeJitValue(): NULLing jitValue_ at %p "
              "-- in PCodeOp at %p\n",
              &jitValue_, this);
  ...
  if (isVar()) {
    ...
    CDPT0("VV0940: ", VV_BD, "param  = Bldr->CreatePointerCast(parm1, cfg->getInt8PtrTy() )\n");
    jit_value_t param = Bldr->CreatePointerCast(parm1, cfg->getInt8PtrTy());

    ////jit_insn_store_relative(param, offset_, value);

    CDPT0("VV0942: ", VV_BD, "cfg->IR_StoreRelativeWithType( Bldr, value, param, offset_, IRtype )\n");
    ...
    if (!jitValue_) {
      ...
      CDPT0("VV0944: ", VV_BD, "jitValue_ = (llvm::Value *)Bldr->CreateAlloca( valType )\n");
      jitValue_ = (llvm::Value *)Bldr->CreateAlloca(valType);

      CDPT2("VV0945: ", VV_BD,
            "Just set jitValue_ in PCodeOp at %p to %p "
            "via CreateAlloca()\n",
            this, jitValue_);
    } else {
      ...
      if (jitValuePtr_ && ((!b->onesVector.testBit(jitValuePtrBlock_->getBlockNum())) ||
                           (jitValueBlock_ && (!b->onesVector.testBit(jitValueBlock_->getBlockNum())))))
        jitValuePtr_ = NULL;
    }
    ...
    CDPT0("VV0949: ", VV_BD, "jit_value_t storePtr = Bldr->CreatePointerCast( jitValue_, valType->getPointerTo() )\n");
    jit_value_t storePtr = Bldr->CreatePointerCast(jitValue_, valType->getPointerTo());

    CDPT0("VV0950: ", VV_BD, "Bldr->CreateStore( value, storePtr )\n");
    Bldr->CreateStore(value, storePtr);

    CDPT2("VV0951: ", VV_BD,
          "In storeJitValue(): setting jitValueBlock_ = %p "
          "for PCop = %p\n",
          b, this);
    jitValueBlock_ = b;
  }
...
#ifdef NA_LINUX_LIBJIT
// FIX: Overlapped types can have problems too
void PCodeOperand::storeJitValue(PCodeCfg *cfg, jit_function_t f, jit_type_t type, jit_value_t value, PCodeBlock *b,
                                 jit_value_t *len = NULL, NABoolean noSave = FALSE) {
  // Get operand in hash table
  CollIndex i = bvIndex_;
  PCodeOperand *operand = cfg->getMap()->getFirstValue(&i);
  if (operand != this) return operand->storeJitValue(cfg, f, type, value, b, len, noSave);

  //
  // Are we being asked to perform a memcpy?
  //
  if (jit_value_get_type(value) == jit_type_void_ptr) {
    jit_value_t size = ((len) ? *len : jit_value_create_nint_constant(f, jit_type_int, getLen()));

    if (jitValuePtr_) {
      assert(jitValueBlock_ != NULL);
      if (!b->onesVector.testBit(jitValueBlock_->getBlockNum())) jitValuePtr_ = NULL;
    }
  ...
  if (isVar()) {
    ...
    if (!jitValue_) {
      ...
      // entry to the routine (and therefore available everywhere).  Addresses
      // are not, however, and therefore may need to be recalculated.
      if (jitValuePtr_ && (!b->onesVector.testBit(jitValueBlock_->getBlockNum()))) jitValuePtr_ = NULL;
    }
...
//
void PCodeCfg::IR_insn_branch_if_zero(IRBldr_t *Bldr, jit_value_t value, p_IR_block_t *tgt_IR_block) {
#if NExprDbgLvl > VV_NO
  char NExBuf[500];
#endif
  ...
  DPT0("VV1002: ", VV_VD, "IsZero = Bldr->CreateICmpEQ( value, Zero, 'check0' )\n");
  jit_value_t IsZero = Bldr->CreateICmpEQ(value, Zero, "check0");

  DPT2("VV1004: ", VV_VD, "Bldr->CreateCondBr( IsZero, *tgt_IR_block = %p, ifNZ_Blk = %p )\n", *tgt_IR_block, ifNZ_Blk);
  Bldr->CreateCondBr(IsZero, *tgt_IR_block, ifNZ_Blk);

  DPT1("VV1006: ", VV_VD, "ExpFn->getBasicBlockList().push_back( ifNZ_Blk = %p )\n", ifNZ_Blk);
  ExpFn->getBasicBlockList().push_back(ifNZ_Blk);

  DPT1("VV1008: ", VV_VD, "Bldr->SetInsertPoint( ifNZ_Blk = %p )\n", ifNZ_Blk);
...
//     to the "if not" IR block.
//
void PCodeCfg::IR_insn_branch_if_not_zero(IRBldr_t *Bldr, jit_value_t value, p_IR_block_t *tgt_IR_block) {
#if NExprDbgLvl > VV_NO
  char NExBuf[500];
#endif
  ...
  DPT0("VV1102: ", VV_VD, "IsZero = Bldr->CreateICmpEQ( value, Zero, 'check0' )\n");
  jit_value_t IsZero = Bldr->CreateICmpEQ(value, Zero, "check0");

  DPT2("VV1104: ", VV_VD, "Bldr->CreateCondBr( IsZero, if_0_Blk = %p, *tgt_IR_block = %p )\n", if_0_Blk, *tgt_IR_block);
  Bldr->CreateCondBr(IsZero, if_0_Blk, *tgt_IR_block);

  DPT1("VV1106: ", VV_VD, "ExpFn->getBasicBlockList().push_back( if_0_Blk = %p )\n", if_0_Blk);
  ExpFn->getBasicBlockList().push_back(if_0_Blk);

  DPT1("VV1108: ", VV_VD, "Bldr->SetInsertPoint( if_0_Blk = %p )\n", if_0_Blk);
  Bldr->SetInsertPoint(if_0_Blk);
}

void PCodeOperand::storeNullJitValueAndBranch(PCodeCfg *cfg, IRBldr_t *Bldr, jit_value_t value, p_IR_block_t tgtLabel,
  ...
  if (operand != this) return operand->storeNullJitValueAndBranch(cfg, Bldr, value, tgtLabel, forComp, b);
  ...
  if (TRUE) {
    ...
    if (nullBitIndex_ != -1) {
      ...
      CDPT0("VV1210: ", VV_BD, "param  = Bldr->CreatePointerCast(parm1, cfg->getInt8PtrTy() )\n");
      jit_value_t param = Bldr->CreatePointerCast(parm1, cfg->getInt8PtrTy());
      ...
      CDPT1("VV1214: ", VV_BD, "res = Bldr->CreateAnd( nByte, mask = %d )\n", ~m);
      res = Bldr->CreateAnd(nByte, mask);

      //    jit_insn_store_relative(f, param, offset_ + (nullBitIndex_ >> 3), res);

      CDPT0("VV1217: ", VV_BD, "cfg->IR_StoreRelativeWithType( Bldr, res, param, offset_ + ..., res->getType() )\n");
      cfg->IR_StoreRelativeWithType(Bldr, res, param, offset_ + (nullBitIndex_ >> 3), res->getType());

      // Branch to target label
      //    jit_insn_branch(f, targetLabel);

      CDPT1("VV1219: ", VV_BD, "Bldr->CreateBr( tgtLabel = %p )\n", tgtLabel);
      Bldr->CreateBr(tgtLabel);

      CDPT1("VV1220: ", VV_BD,
            "In storeNullJitVal: CREATING BRANCH TO tgtLabel at "
            "( %p )\n",
            tgtLabel);
      ...
      CDPT1("VV1220: ", VV_BD, "ExpFn->getBasicBlockList().push_back( nullLabel = %p )\n", nullLabel);
      ExpFn->getBasicBlockList().push_back(nullLabel);

      CDPT1("VV1222: ", VV_BD, "Bldr->SetInsertPoint( nullLabel = %p )\n", nullLabel);
      Bldr->SetInsertPoint(nullLabel);

      CDPT1("VV1223: ", VV_BD, "In storeNullJitVal: Setting Insert Point to nullLabel at %p\n", nullLabel);
      ...
      //                          offset_ + (nullBitIndex_ >> 3), jit_type_ubyte);
      nByte = cfg->IR_LoadRelativeWithType(Bldr, param, offset_ + (nullBitIndex_ >> 3), cfg->getInt8Ty());

      // Set the null bit and store it back
      //    res = jit_insn_or(f, nByte, mask);

      CDPT1("VV1224: ", VV_BD, "res = Bldr->CreateOr( nByte, mask = %d )\n", m);
      res = Bldr->CreateOr(nByte, mask);

      //    jit_insn_store_relative(f, param, offset_ + (nullBitIndex_ >> 3), res);

      CDPT0("VV1224: ", VV_BD, "cfg->IR_StoreRelativeWithType( Bldr, res, param, offset_ + ..., res->getType() )\n");
      cfg->IR_StoreRelativeWithType(Bldr, res, param, offset_ + (nullBitIndex_ >> 3), res->getType());
      ...
      if (isVar()) {
        ...
        CDPT0("VV1230: ", VV_BD, "param  = Bldr->CreatePointerCast(parm1, cfg->getInt8PtrTy() )\n");
        jit_value_t param = Bldr->CreatePointerCast(parm1, cfg->getInt8PtrTy());
        ...
        CDPT0("VV1240: ", VV_BD, "cfg->IR_StoreRelativeWithType( Bldr, res, param, offset_, res->getType() )\n");
        cfg->IR_StoreRelativeWithType(Bldr, res, param, offset_, res->getType());

        //      jit_insn_branch(f, targetLabel);

        CDPT1("VV1242: ", VV_BD, "Bldr->CreateBr( tgtLabel = %p )\n", tgtLabel);
        Bldr->CreateBr(tgtLabel);
        ...
        CDPT1("VV1244: ", VV_BD, "ExpFn->getBasicBlockList().push_back( nullLabel = %p )\n", nullLabel);
        ExpFn->getBasicBlockList().push_back(nullLabel);

        CDPT1("VV1246: ", VV_BD, "Bldr->SetInsertPoint( nullLabel = %p )\n", nullLabel);
        Bldr->SetInsertPoint(nullLabel);
        ...
        CDPT0("VV1250: ", VV_BD, "cfg->IR_StoreRelativeWithType( Bldr, res, param, offset_, res->getType() )\n");
        cfg->IR_StoreRelativeWithType(Bldr, res, param, offset_, res->getType());
        ...
        if (1 /* forComp ? */) {
          ...
          CDPT1("VV1255: ", VV_BD, "Bldr->CreateBr( tgtLabel = %p )\n", tgtLabel);
          Bldr->CreateBr(tgtLabel);
          ...
          CDPT1("VV1260: ", VV_BD, "ExpFn->getBasicBlockList().push_back( nullLabel = %p )\n", nullLabel);
          ExpFn->getBasicBlockList().push_back(nullLabel);

          CDPT1("VV1261: ", VV_BD, "Bldr->SetInsertPoint( nullLabel = %p )\n", nullLabel);
          Bldr->SetInsertPoint(nullLabel);
...
#ifdef NA_LINUX_LIBJIT
void PCodeOperand::storeNullJitValueAndBranch(PCodeCfg *cfg, jit_function_t f, jit_value_t value,
                                              jit_label_t *targetLabel, NABoolean forComp, PCodeBlock *b) {
  // Get operand in hash table
  CollIndex i = bvIndex_;
  PCodeOperand *operand = cfg->getMap()->getFirstValue(&i);
  if (operand != this) return operand->storeNullJitValueAndBranch(cfg, f, value, targetLabel, forComp, b);
  ...
  if (TRUE) {
    ...
    if (nullBitIndex_ != -1) {
      ...
      mask = jit_value_create_nint_constant(f, jit_type_ubyte, m);
      nByte = jit_insn_load_relative(f, param, offset_ + (nullBitIndex_ >> 3), jit_type_ubyte);
      ...
      if (isVar()) {
        jit_value_t param = cfg->getJitParams()[stackIndex_];
        jit_insn_store_relative(f, param, offset_, (forComp ? cfg->getZeroJitVal() : zeroJitVal16));
        jit_insn_branch(f, targetLabel);

        // Now handle NULL case.

        jit_insn_label(f, &nullLabel);
        jit_insn_store_relative(f, param, offset_, (forComp ? cfg->getNeg1JitVal() : neg1JitVal16));
      } else {
        if (forComp) {
          storeJitValue(cfg, f, jit_type_int, cfg->getZeroJitVal(), b, NULL);
          jit_insn_branch(f, targetLabel);
...
void PCodeCfg::jitProcessFloatExceptionCheck(jit_function_t f, PCodeOperand *fRes, jit_label_t *errorJitLabel,
                                             PCodeBlock *block) {
...
NABoolean PCodeCfg::jitProcessPredicate(PCodeInst *comp, jit_function_t f, jit_label_t *falseLabel,
                                        jit_label_t *trueLabel, NABoolean isFirstBlockTrue) {
  CollIndex i;
  ...
  if (comp->next && comp->next->isAnyLogicalBranch()) {
    ...
    if (branchSrc->getBvIndex() == compTgt->getBvIndex()) {
      ...
      for (i = 0; i < 2; i++) {
        if ((isFirstBlockTrue && (i == 0)) || (!isFirstBlockTrue && (i == 1))) {
          jit_insn_label(f, trueLabel);

          // Dump true block first
          if (srcLive) compTgt->storeJitValue(this, f, jit_type_int, oneJitVal_, block);

          if (tgtLive) branchTgt->storeJitValue(this, f, jit_type_int, oneJitVal_, block);

          if ((branch->getOpcode() == PCIT::BRANCH_OR) || (branch->getOpcode() == PCIT::BRANCH_OR_CNT))
            jit_insn_branch(f, block->getTargetBlock()->getJitLabel());
          else
            jit_insn_branch(f, block->getFallThroughBlock()->getJitLabel());
        }
        ...
        if ((!isFirstBlockTrue && (i == 0)) || (isFirstBlockTrue && (i == 1))) {
          ...
          if (srcLive) compTgt->storeJitValue(this, f, jit_type_int, zeroJitVal_, block);

          if (tgtLive) branchTgt->storeJitValue(this, f, jit_type_int, zeroJitVal_, block);

          if ((branch->getOpcode() == PCIT::BRANCH_AND) || (branch->getOpcode() == PCIT::BRANCH_AND_CNT))
            jit_insn_branch(f, block->getTargetBlock()->getJitLabel());
          else
            jit_insn_branch(f, block->getFallThroughBlock()->getJitLabel());
        }
...
void PCodeCfg::layoutNativeCode() {
  ...
  static const int SaveSigs[] = {SIGHUP,  SIGINT,  SIGQUIT, SIGILL,  SIGBUS, SIGFPE,
                                 SIGSEGV, SIGUSR2, SIGPIPE, SIGTERM, SIGXFSZ};
  static const int NumSaveSigs = sizeof(SaveSigs) / sizeof(SaveSigs[0]);

  static THREAD_P struct { struct sigaction SigAct; } SavedSigInfo[NumSaveSigs];

  static struct { struct sigaction SigAct; } SavedLLVMhandlers[NumSaveSigs];
  ...
  if (NExprDbgLvl_ >= VV_I0) {
    ...
    DPT2("VV90000: ", VV_I3, "STARTING layoutNativeCode() at %s : Microseconds: %d\n", StTime,
         (int)begTime.ru_utime.tv_usec);

    if (NExprDbgLvl_ >= VV_I1 && NExDbgInfoPtr_->getNExStmtPrinted() == FALSE) {
...
#ifdef NEW_VERSION_LLVM
  std::unique_ptr<llvm::Module> TheModule = make_unique<llvm::Module>("ExpressionJit", LLContxt);
  llvm::Module *TheModulePtr = TheModule.get();
#else
...
#ifdef NEW_VERSION_LLVM
  llvm::HelpingMemoryManager *helpingMemoryManager = new llvm::HelpingMemoryManager();
  std::unique_ptr<llvm::RTDyldMemoryManager> MemMgr(helpingMemoryManager);
  // Build engine with JIT
  llvm::EngineBuilder factory(std::move(TheModule));
  factory.setEngineKind(llvm::EngineKind::JIT);
  factory.setTargetOptions(target_opts);
  factory.setMCJITMemoryManager(std::move(MemMgr));
  llvm::ExecutionEngine *TheExecutionEngine = factory.create();
#else
  llvm::ExecutionEngine *TheExecutionEngine = EngineBuilder(TheModule)
                                                  .setErrorStr(&ErrStr)
                                                  .setMAttrs(MAttrs)
  ...
  DPT3("VV90000a: ", VV_VD, "int1PtrTy_ = %p, int8Ty_ = %p,     int16Ty_ = %p\n", int1PtrTy_, int8Ty_, int16Ty_);

  DPT2("VV90000b: ", VV_VD, "int32Ty_ = %p,   int64Ty_ = %p\n", int32Ty_, int64Ty_);

  DPT3("VV90000c: ", VV_VD, "int8PtrTy_  = %p, int16PtrTy_ = %p, int32PtrTy_ = %p\n", int8PtrTy_, int16PtrTy_,
       int32PtrTy_);

  DPT2("VV90000d: ", VV_VD, "int64PtrTy_ = %p, floatPtrTy_ = %p\n", int64PtrTy_, floatPtrTy_);
  ...
  ************ KEEP IN COMMENTS FOR NOW */
  ...
  FunctionType *functype = FunctionType::get(int32Ty_, args, false);

  // Now that we have the function signature (i.e. functype), we
  // can tell LLVM to create the "function".
  //
#ifdef NEW_VERSION_LLVM
  llvm::Function *ExpFn = llvm::Function::Create(functype, llvm::Function::ExternalLinkage, "testExpr", TheModulePtr);
#else
  llvm::Function *ExpFn = llvm::Function::Create(functype, llvm::Function::ExternalLinkage, "testExpr", TheModule);
#endif
  ...
  //
  unsigned Idx = 0;
  for (llvm::Function::arg_iterator AI = ExpFn->arg_begin(); Idx != Args.size(); ++AI, ++Idx) {
    AI->setName(Args[Idx]);
#ifdef NEW_VERSION_LLVM
  ...
  DPT1("VV90001: ", VV_VD, "Bldr->SetInsertPoint( IRBlk = %p )\n", IRBlk);
  Bldr->SetInsertPoint(IRBlk);

  jit_value_t globTableJitVal = NamedValues[Args[0]];

  // Allocate incoming parameters
  DPT1("VV90002: ", VV_XD, "SIZING jitParams_[] to array of size 4+pCode[0] == %d\n", 4 + pCode[0]);
  ...
  for (i = 0, j = 1; i < (CollIndex)pCode[0]; i++, j += 2) {
    ...
    DPT1("VV90008: ", VV_XD, "Passing pCode[j+1] = %d to getTuppForNativeExpr()\n", pCode[j + 1]);
    off1 = (char *)(&(tempAtp.getTuppForNativeExpr(pCode[j + 1]))) - (char *)(&tempAtp);

    DPT0("VV90010: ", VV_VD, "Int8p_t1 = Bldr->CreatePointerCast(t1, int8PtrTy_ )\n");
    jit_value_t Int8p_t1 = Bldr->CreatePointerCast(t1, int8PtrTy_);

    DPT1("VV90012: ", VV_VD, "V_tuppArr = IR_LoadRelativeWithType( Bldr, Int8p_t1, off1=%ld, int8PtrTy_ )\n", off1);
    ...
    DPT2("VV90013: ", VV_XD, "SETTING jitParams_[%d] = rtnval from IR_LoadRelativeWithType(...,V_tuppArr, %ld, ...)\n",
         4 + i, off2);
    jitParams_[4 + i] = t3;
  }
  ...
  DPT1("VV90022: ", VV_VD, "jitParams_[2] = IR_LoadRelativeWithType( Bldr, exprJitVal, TempsOff = %ld, int8PtrTy_ )\n",
       TempsOff);
  jitParams_[2] = IR_LoadRelativeWithType(Bldr, exprJitVal, TempsOff, int8PtrTy_);
  ...
  for (PCBlkIndex = 0; (PCBlkIndex < physBlockList.entries()) && !getJitFailureSeen(); PCBlkIndex++) {
    ...
    *IR_Blk_Label = BasicBlock::Create(LLContxt, "physBlk");
    ...
    DPT2("VV90040: ", VV_XD, "In First Loop: Created BB AT ( %p ) for PCBlk = %p\n", *IR_Blk_Label, PCBlk);

    if (*IR_Blk_Label == IR_block_undefined) setJitFailureSeen();
  }
  ...
  DPT1("VV90050: ", VV_XD, "Bldr->CreateBr( *(physBlockList[0]->getJitBlkLbl()) = %p )\n",
  ...
  DPT1("VV90052: ", VV_XD, "Starting loop creating one BLOCK at a time, with BLK COUNT=%d\n", physBlockList.entries());
  ...
  for (PCBlkIndex = 0; (PCBlkIndex < physBlockList.entries()) && !getJitFailureSeen(); PCBlkIndex++) {
    ...
    if (*(PCBlk->getJitBlkLbl()) != NULL) {
      ...
      DPT1("VV90060: ", VV_VD, "ExpFn->getBasicBlockList().push_back( IR_Blk_Ptr= %p )\n", IR_Blk_Ptr);
      ExpFn->getBasicBlockList().push_back(IR_Blk_Ptr);

      DPT1("VV90062: ", VV_VD, "Bldr->SetInsertPoint( IR_Blk_Ptr = %p )\n", IR_Blk_Ptr);
    ...
    FOREACH_INST_IN_BLOCK(PCBlk, PCInst) {
      ...
      DPT0("VV90100: ", VV_VD, "In Main Loop creating one INSTRUCTION at a time\n");
      DPT2("VV90102: ", VV_XD, "In Main Loop: PCInst at %p, PCInst->next = %p \n", PCInst, PCInst->next);
#if NExprDbgLvl >= VV_VD
      if (PCInst->next)
        DPT2("VV90103: ", VV_XD, "In Main Loop: next at %p has opc == %d\n", PCInst->next,
             (int)PCInst->next->getOpcode());
#endif
      ...
      if (skipInst > 0) {
        DPT0("VV90107: ", VV_XD, "skipInst is > 0 so we are doing a 'continue'\n");
        skipInst--;
        continue;
      }
      //
      ...
      //
      if (opc != PCIT::RETURN) justGeneratedRET = FALSE;

      if (justGeneratedRET == FALSE) switch (opc) {
          case PCIT::MOVE_MATTR5_MATTR5: {
            ...
            if (src1->getVcIndicatorLen() == 2) {
              ...
              DPT0("VV91002: ", VV_XD, "In case PCIT::MOVE_MATTR5_MATTR5: vcLenJitVal = 2 (16-bit)\n");
            } else {
              ...
              DPT0("VV91003: ", VV_XD, "In case PCIT::MOVE_MATTR5_MATTR5: vcLenJitVal = 4 (32-bit)\n");
            }
            ...
            jit_value_t srcLen = IR_LoadRelativeWithType(Bldr, src1JitVal, 0, vcType);
            ...
            DPT0("VV91014: ", VV_VD, "srcLen = Bldr->CreateAdd( srcLen , vcLenJitVal , 'IntAdd' )\n");
            srcLen = Bldr->CreateAdd(srcLen, vcLenJitVal, "IntAdd");

            ////      genUnalignedMemcpy(f, tRes, tSrc1, srcLen);
            DPT0("VV91016: ", VV_XD, "genUnalignedMemcpy( Bldr, resJitVal, src1JitVal, srcLen )\n");
          ...
          case PCIT::BRANCH: {
            ...
            DPT1("VV92010: ", VV_VD, "Bldr->CreateBr( TgtBlk = %p )\n", TgtBlk);
          ...
          case PCIT::BRANCH_AND: {
            ...
            jit_value_t takenJitVal =
                ((opc == PCIT::BRANCH_OR) || (opc == PCIT::BRANCH_OR_CNT)) ? IR_Const_1_ : IR_Const_0_;
            ...
            tRes = jitGenCompare(Bldr, IntCompare_EQ, src1JitVal, takenJitVal, IR_Const_1_, IR_Const_0_);
          ...
          case PCIT::NNB_SPECIAL_NULLS_MBIN32S_MATTR3_MATTR3_IBIN32S_IBIN32S: {
            ...
            DPT3("VV93000: ", VV_XD, "src1LabelNull = %p, ret0Label = %p, endLabel = %p\n", src1LabelNull, ret0Label,
                 endLabel);
            ...
            DPT1("VV93005: ", VV_VD, "Bldr->CreateBr( *IR_Blk_Label = %p )\n", *IR_Blk_Label);
            ...
            DPT1("VV93020: ", VV_VD, "ExpFn->getBasicBlockList().push_back( src1LabelNull = %p )\n", src1LabelNull);
            ExpFn->getBasicBlockList().push_back(src1LabelNull);

            DPT1("VV93030: ", VV_VD, "Bldr->SetInsertPoint( src1LabelNull = %p )\n", src1LabelNull);
            ...
            DPT1("VV93040: ", VV_VD, "Bldr->CreateBr( endLabel = %p )\n", endLabel);
            ...
            DPT1("VV93060: ", VV_VD, "ExpFn->getBasicBlockList().push_back( ret0Label = %p )\n", ret0Label);
            ExpFn->getBasicBlockList().push_back(ret0Label);

            DPT1("VV93070: ", VV_VD, "Bldr->SetInsertPoint( ret0Label = %p )\n", ret0Label);
            ...
            DPT1("VV93080: ", VV_VD, "Bldr->CreateBr( endLabel = %p )\n", endLabel);
            Bldr->CreateBr(endLabel);  // Must complete the current block

            DPT1("VV93090: ", VV_VD, "ExpFn->getBasicBlockList().push_back( endLabel = %p )\n", endLabel);
            ExpFn->getBasicBlockList().push_back(endLabel);

            DPT1("VV93098: ", VV_VD, "Bldr->SetInsertPoint( endLabel = %p )\n", endLabel);
            Bldr->SetInsertPoint(endLabel);
          ...
          case PCIT::NOT_NULL_BRANCH_COMP_MBIN32S_MBIN32S_MBIN32S_IATTR4_IBIN32S: {
            ...
            NABoolean forComp = (opc == PCIT::NOT_NULL_BRANCH_COMP_MBIN32S_MBIN32S_MBIN32S_IATTR4_IBIN32S);
            ...
            DPT0("VV94110: ", VV_VD, "orResJitVal = Bldr->CreateOr( src1JitVal, src2JitVal, 'BitwiseOr' )\n");
            ...
            if (res->isVar() || PCBlk->isOperandLiveInSuccs(res)) {
              ...
              ////                                        forComp, block);
              res->storeNullJitValueAndBranch(this, Bldr, orResJitVal, *IR_Blk_Label, forComp, PCBlk);
            } else {
          ...
          case PCIT::NOT_NULL_BRANCH_MBIN32S_MBIN32S_IATTR3_IBIN32S: {
            ...
            NABoolean forComp = (opc == PCIT::NOT_NULL_BRANCH_COMP_MBIN32S_MBIN32S_IATTR3_IBIN32S);

            //        src1JitVal = src1->getJitValue(this, f, src1->getJitType(), PCBlk);

            src1IRtype = src1->getJitType(this);

            DPT1("VV95010: ", VV_XD, "In NOT_NULL_BRANCH_MBIN32S... src1's Type ID is %d\n",
            ...
            if (src1IRtype->isPointerTy())
              DPT1("VV95012: ", VV_XD, "In NOT_NULL_BRANCH_MBIN32S... src1's Contained Type ID is %d\n",
                   (int)src1IRtype->getContainedType(0)->getTypeID());
#endif

            jit_value_t src1JitVal = src1->getJitValue(this, Bldr, src1IRtype, PCBlk);

            DPT1("VV95014: ", VV_XD, "In NOT_NULL_BRANCH_MBIN32S... Val32's Type ID is %d\n",
            ...
            if (res->isVar() || PCBlk->isOperandLiveInSuccs(res)) {
              ...
              //                                          forComp, PCBlk);
              res->storeNullJitValueAndBranch(this, Bldr, src1JitVal, *IR_Blk_Label, forComp, PCBlk);
            } else {
              //          jit_insn_branch_if_not(f, src1JitVal, jitLabel);
              IR_insn_branch_if_zero(Bldr, src1JitVal, IR_Blk_Label);
            }
          ...
          case PCIT::FILL_MEM_BYTES: {
            ...
            if (res->getStackIndex() == 2) {
              if (opc == PCIT::FILL_MEM_BYTES) {
                DPT0("VV95101: ", VV_VD, "found PCIT::FILL_MEM_BYTES: WITH stackIndex_ = 2\n");
              } else {
                DPT0("VV95101: ", VV_VD, "found PCIT::FILL_MEM_BYTES_VARIABLE: WITH stackIndex_ = 2\n");
              }
            }
#endif
            ...
            if (opc == PCIT::FILL_MEM_BYTES_VARIABLE) {
              ...
              DPT0("VV95110: ", VV_XD, "IR_StoreRelativeWithType( Bldr, lenJitVal, resJitVal, 0, vcType )\n");
              ...
              DPT1("VV95112: ", VV_VD, "resJitVal = Bldr->CreateGEP( resJitVal, %d )\n", vcLen);
              ...
              DPT2("VV95130: ", VV_XD, "FILL_MEM_BYTES_VARIABLE: Fill Value=0x%x, Fill Len = %d\n", fillVal, fillLen);
            } else {
              fillLen = PCInst->code[3];
              fillVal = PCInst->code[4];

              DPT2("VV95140: ", VV_XD, "FILL_MEM_BYTES: Fill Value=0x%x, Fill Len = %d\n", fillVal, fillLen);
            }
          ...
          case PCIT::GE_MBIN32S_MASCII_MASCII: {
            ...
            switch (opc) {
              ...
              case PCIT::NE_MBIN32S_MASCII_MASCII: {
                KindOfCmp = ByteCompare_NE;
                break;
              }
              case PCIT::LT_MBIN32S_MASCII_MASCII: {
                KindOfCmp = ByteCompare_LT;
                break;
              }
              case PCIT::LE_MBIN32S_MASCII_MASCII: {
                KindOfCmp = ByteCompare_LE;
                break;
              }
              case PCIT::GT_MBIN32S_MASCII_MASCII: {
                KindOfCmp = ByteCompare_GT;
                break;
              }
              case PCIT::GE_MBIN32S_MASCII_MASCII: {
                KindOfCmp = ByteCompare_GE;
                break;
              }
            }
            ...
            if (jit_value_t src1CurrJitVal = src1->getCurrJitVal()) {
              if ((src1CurrJitVal->getType() == int64Ty_) && (MaxlenInt32 >= 8)) {
                jit_value_t Maxlen64 = ConstantInt::get(int64Ty_, MaxlenInt32);

                DPT0("VV95154: ", VV_VD, "src1JitVal = src1->getJitValue( this, Bldr, int64Ty_, PCBlk )\n");
                src1JitVal = src1->getJitValue(this, Bldr, int64Ty_, PCBlk);

                DPT0("VV95155: ", VV_VD,
                     "jit_value_t IsZero1  = Bldr->CreateICmpEQ( src1JitVal, Maxlen64, 'junkCk' )\n");
                jit_value_t IsZero1 = Bldr->CreateICmpEQ(src1JitVal, Maxlen64, "junkCk");
              } else if ((src1CurrJitVal->getType() == int32Ty_) && (MaxlenInt32 >= 4)) {
                DPT0("VV95156: ", VV_VD, "src1JitVal = src1->getJitValue( this, Bldr, int32Ty_, PCBlk )\n");
                src1JitVal = src1->getJitValue(this, Bldr, int32Ty_, PCBlk);

                DPT0("VV95157: ", VV_VD, "jit_value_t IsZero1  = Bldr->CreateICmpEQ( src1JitVal, Maxlen, 'junkCk' )\n");
                jit_value_t IsZero1 = Bldr->CreateICmpEQ(src1JitVal, Maxlen, "junkCk");
              } else if ((src1CurrJitVal->getType() == int16Ty_) && (MaxlenInt32 >= 2)) {
                jit_value_t Maxlen16 = ConstantInt::get(int16Ty_, MaxlenInt32);

                DPT0("VV95158: ", VV_VD, "src1JitVal = src1->getJitValue( this, Bldr, int16Ty_, PCBlk )\n");
            ...
            jit_value_t IsZero = genMemCmpLoop(Bldr, KindOfCmp, src1JitVal, src2JitVal, Maxlen);
            DPT2("VV95160: ", VV_VD, "Bldr->CreateCondBr( IsZero, trueLabel = %p, falseLabel = %p )\n", trueLabel,
                 falseLabel);
            Bldr->CreateCondBr(IsZero, trueLabel, falseLabel);

            DPT1("VV95162: ", VV_VD, "ExpFn->getBasicBlockList().push_back( trueLabel = %p )\n", trueLabel);
            ExpFn->getBasicBlockList().push_back(trueLabel);

            DPT1("VV95164: ", VV_VD, "Bldr->SetInsertPoint( trueLabel = %p )\n", trueLabel);
            Bldr->SetInsertPoint(trueLabel);

            res->storeJitValue(this, Bldr, int32Ty_, IR_Const_1_, PCBlk);

            DPT1("VV95166: ", VV_VD, "Bldr->CreateBr( endLabel = %p )\n", endLabel);
            Bldr->CreateBr(endLabel);

            DPT1("VV95168: ", VV_VD, "ExpFn->getBasicBlockList().push_back( falseLabel = %p )\n", falseLabel);
            ExpFn->getBasicBlockList().push_back(falseLabel);

            DPT1("VV95170: ", VV_VD, "Bldr->SetInsertPoint( falseLabel = %p )\n", falseLabel);
            Bldr->SetInsertPoint(falseLabel);

            res->storeJitValue(this, Bldr, int32Ty_, IR_Const_0_, PCBlk);

            DPT1("VV95172: ", VV_VD, "Bldr->CreateBr( endLabel = %p )\n", endLabel);
            Bldr->CreateBr(endLabel);  // Must complete the current block

            DPT1("VV95174: ", VV_VD, "ExpFn->getBasicBlockList().push_back( endLabel = %p )\n", endLabel);
            ExpFn->getBasicBlockList().push_back(endLabel);

            DPT1("VV95179: ", VV_VD, "Bldr->SetInsertPoint( endLabel = %p )\n", endLabel);
            Bldr->SetInsertPoint(endLabel);
          ...
          case PCIT::EQ_MBIN32S_MBIN64S_MBIN64S: {
...
#if NExprDbgLvl >= VV_VD
            if (src1 == src2) DPT0("VV96002: ", VV_XD, "In EQ_MBIN32S_MBINxxS_MBINxxS: src1 == src2 !!!\n");
#endif

            ////      src1JitVal = src1->getJitValue(this, f, src1->getJitType(), PCBlk);
            src1IRtype = src1->getJitType(this);

            DPT1("VV96004: ", VV_XD, "In EQ_MBIN32S_MBINxxS_MBINxxS: src1IRtype Width = %d\n",
            ...
            DPT1("VV96006: ", VV_XD, "In EQ_MBIN32S_MBINxxS_MBINxxS: src2IRtype Width = %d\n",
            ...
            resJitVal = jitGenCompare(Bldr, IntCompare_EQ, src1JitVal, src2JitVal, IR_Const_1_, IR_Const_0_);

            DPT0("VV96024: ", VV_XD, "In EQ_MBIN32S_MBINxxS_MBINxxS: Returned from jitGenCompare\n");

            ////      inst->getWOps()[0]->setJitValue(this, f, resJitVal, PCBlk);
            res->setJitValue(this, Bldr, resJitVal, PCBlk);

            DPT0("VV96026: ", VV_XD, "In EQ_MBIN32S_MBINxxS_MBINxxS: Returned from setJitValue\n");
            break;
          }
          ...
          case PCIT::LT_MBIN32S_MBIN32U_MBIN32U: {
            ...
            resJitVal = jitGenCompare(Bldr, KindOfCmp, src1JitVal, src2JitVal, IR_Const_1_, IR_Const_0_);
          ...
          case PCIT::SUM_MATTR3_MATTR3_IBIN32S_MBIN64S_MBIN32S: {
            ...
            if (!src2->isConst()) {
              ...
              DPT0("VV96904: ", VV_XD, "GOT INTO if (!src2->isConst()) section.\n");
              ...
              DPT0("VV96906: ", VV_XD, "GOT INTO if (!src2->isConst()) section.\n");
            }
            ...
            src1->storeNullJitValueAndBranch(this, Bldr, Const_0, doneLabel, FALSE, PCBlk);
            ...
            DPT1("VV96920: ", VV_VD, "Bldr->CreateBr( sumLabel = %p )\n", sumLabel);
            Bldr->CreateBr(sumLabel);  // Must complete the current block

            DPT1("VV96922: ", VV_VD, "ExpFn->getBasicBlockList().push_back( sumLabel = %p )\n", sumLabel);
            ExpFn->getBasicBlockList().push_back(sumLabel);

            DPT1("VV96924: ", VV_VD, "Bldr->SetInsertPoint( sumLabel = %p )\n", sumLabel);
            Bldr->SetInsertPoint(sumLabel);

            ////      resJitVal = jit_insn_add(f, resJitVal, src3JitVal);

            DPT0("VV96927: ", VV_VD, "resJitVal = Bldr->CreateAdd( resJitVal, src3JitVal, 'IntAdd' )\n");
            ...
            DPT1("VV96930: ", VV_VD, "Bldr->CreateBr( doneLabel = %p )\n", doneLabel);
            Bldr->CreateBr(doneLabel);  // Must complete the current block

            DPT1("VV96932: ", VV_VD, "ExpFn->getBasicBlockList().push_back( doneLabel = %p )\n", doneLabel);
            ExpFn->getBasicBlockList().push_back(doneLabel);

            DPT1("VV96934: ", VV_VD, "Bldr->SetInsertPoint( doneLabel = %p )\n", doneLabel);
            Bldr->SetInsertPoint(doneLabel);

            // Lastly, clear out jit values for 32-bit source operand if needed.
            if (resWidth > src3Width) src3->clearJitValues(this);
          ...
          case PCIT::MOVE_MBIN64S_MBIN32U: {
            ...
            DPT0("VV97010: ", VV_VD, "src1JitVal = Bldr->CreateZExt( src1JitVal, (IntegerType *)resIRtype, 'ZExt' )\n");
            ...
            // Lastly, clear out jit values for 32-bit source operand if needed.
            if ((opc == PCIT::MOVE_MBIN64S_MBIN16U) || (opc == PCIT::MOVE_MBIN64S_MBIN32U)) src1->clearJitValues(this);
          ...
          case PCIT::MOVE_MBIN64S_MBIN32S: {
            ...
            llvm::IntegerType *IntTyp = IntegerType::get(LLContxt, resIRtype->getIntegerBitWidth());
            ...
            DPT0("VV98016: ", VV_VD, "resJitVal = Bldr->CreateSExtOrTrunc( src1JitVal, IntTyp, 'SExt' )\n");
            ...
            // Lastly, clear out jit values for 32-bit source operand if needed.
            if ((opc == PCIT::MOVE_MBIN64S_MBIN16S) || (opc == PCIT::MOVE_MBIN64S_MBIN32S)) src1->clearJitValues(this);
          ...
          case PCIT::MOVE_MBIN32S_IBIN32S: {
            ...
            DPT1("VV99010: ", VV_VD, "res->storeJitValue(this, Bldr, resIRtype, src1JitVal=Const of %d, PCBlk)\n", val);
            res->storeJitValue(this, Bldr, resIRtype, src1JitVal, PCBlk);
          ...
          case PCIT::MOVE_MBIN64S_MBIN64S: {
...
#endif
            if (src1IRtype == int8PtrTy_) assert(opc == PCIT::MOVE_MBIN8_MBIN8_IBIN32S);
            ...
            if (opc == PCIT::MOVE_MBIN8_MBIN8_IBIN32S) {
              ...
              if (src1IRtype->isPointerTy())
                DPT1("VV9A022: ", VV_XD, "src1IRtype->getContainedType(0)->getTypeID() = %d\n",
                     src1IRtype->getContainedType(0)->getTypeID());
            }
#endif

            DPT0("VV9A026: ", VV_VD, "src1JitVal = src1->getJitValue( this, Bldr, src1IRtype, PCBlk )\n");
            ...
            DPT2("VV9A027D: ", VV_XD, "src1IRtype is at %p, src1JitVal->getType() is at %p\n", src1IRtype,
                 src1JitVal->getType());

            DPT1("VV9A028: ", VV_XD, "src1JitVal is at %p\n", src1JitVal);

            DPT0("VV9A030: ", VV_VD, "res->storeJitValue(this, Bldr, src1IRtype, src1JitVal, PCBlk)\n");
          ...
          case PCIT::NULL_TEST_MBIN32S_MATTR5_IBIN32S_IBIN32S: {
            ...
            if (PCInst->next && PCInst->next->isAnyLogicalBranch()) {
              ...
              if ((bSrc->getBvIndex() == res->getBvIndex()) && !srcLive && !tgtLive) {
                ////          jitLabel = PCBlk->getTargetBlock()->getJitLabel();
                p_IR_block_t *IR_Blk_Label = PCBlk->getTargetBlock()->getJitBlkLbl();
              ...
              if (res->getBvIndex() == move->getROps()[0]->getBvIndex()) {
                ...
                resJitVal = jitGenCompare(Bldr, KindOfCmp, src1JitVal, Zero, IR_Const_TRUE_, IR_Const_FALSE_);

                DPT0("VV9A360: ", VV_VD, "returned from jitGenCompare()\n");

                DPT0("VV9A361: ", VV_VD, "Bldr->CreateRet( resJitVal )\n");
            ...
            jit_value_t Zero = ConstantInt::get(src1JitVal->getType(), 0);
            resJitVal = jitGenCompare(Bldr, KindOfCmp, src1JitVal, Zero, IR_Const_1_, IR_Const_0_);
          ...
          case PCIT::MINMAX_MBIN8_MBIN8_MBIN32S_IBIN32S: {
            ...
            DPT1("VV9A520: ", VV_VD, "Bldr->CreateBr( doneLabel = %p )\n", doneLabel);
            Bldr->CreateBr(doneLabel);  // Must complete the current block

            DPT1("VV9A522: ", VV_VD, "ExpFn->getBasicBlockList().push_back( doneLabel = %p )\n", doneLabel);
            ExpFn->getBasicBlockList().push_back(doneLabel);

            DPT1("VV9A524: ", VV_VD, "Bldr->SetInsertPoint( doneLabel = %p )\n", doneLabel);
            Bldr->SetInsertPoint(doneLabel);
          ...
          case PCIT::OPDATA_MBIN16U_IBIN32S:
          case PCIT::OPDATA_MATTR5_IBIN32S: {
            OPLIST opList = (PCInst->getWOps().entries() ? PCInst->getWOps() : PCInst->getROps());
            ...
            if (PCInst->getWOps().entries()) {
              for (int nnn = 0; (nnn <= index) && (nnn < opList.entries()); nnn++) {
                ...
                DPT0("VV9B006a: ", VV_XD, "Oper->allocJitValue( this, Bldr, Oper->getType(), Oper->getLen(), 1 )\n");
          ...
          case PCIT::CLAUSE_EVAL: {
            ...
            switch (clause->getClassID()) {
              case ex_clause::AGGR_MIN_MAX_ID: {
                DPT0("VV9B120: ", VV_VD, "CLAUSE_EVAL - found ClassID = AGGR_MIN_MAX_ID \n");
                //
                // Format:
                //
                // attr[0] - Aggregate
                ...
                if (clause->getOperand(0)->getNullFlag()) {
                  ...
                  if (clause->getOperand(1)->getNullFlag()) {
                    ...
                    src1IRtype = src1->getJitType(this);
                    src1JitVal = src1->getJitValue(this, Bldr, src1IRtype, PCBlk, NULL, TRUE /* no assign */);
                  ...
                  opDataNulls[0]->setJitValue(this, Bldr, IR_Const_16bit_0_, PCBlk, TRUE);
                }
                ...
                if (res->isVarchar()) {
                  ...
                  resIRtype = res->getJitType(this);
                  resJitVal = res->getJitValue(this, Bldr, resIRtype, PCBlk, NULL, TRUE /* no assign */);
                  ...
                  DPT0("VV9B130: ", VV_VD, "srcLen = Bldr->CreateAdd( srcLen , vcLenJitVal , 'IntAdd' )\n");
                  srcLen = Bldr->CreateAdd(srcLen, vcLenJitVal, "IntAdd");
                  ...
                  res->storeJitValue(this, Bldr, resIRtype, src1JitVal, PCBlk, NULL, TRUE);
                }

                // Reached the end point
                ////          jit_insn_label(f, &doneLabel);

                DPT1("VV9B134: ", VV_VD, "Bldr->CreateBr( doneLabel = %p )\n", doneLabel);
                Bldr->CreateBr(doneLabel);  // Must complete the current block

                DPT1("VV9B136: ", VV_VD, "ExpFn->getBasicBlockList().push_back( doneLabel = %p )\n", doneLabel);
                ExpFn->getBasicBlockList().push_back(doneLabel);

                DPT1("VV9B138: ", VV_VD, "Bldr->SetInsertPoint( doneLabel = %p )\n", doneLabel);
                Bldr->SetInsertPoint(doneLabel);
              ...
              case ex_clause::COMP_TYPE: {
                DPT0("VV9B200: ", VV_VD, "CLAUSE_EVAL - found ClassID = COMP_TYPE \n");
                ...
                DPT3("VV9B201: ", VV_VD, "trueLabel = %p, falseLabel = %p, nullLabel = %p\n", trueLabel, falseLabel,
                     nullLabel);
                ...
                switch (((ex_comp_clause *)clause)->getInstruction()) {
                  ...
                  case LE_DATETIME_DATETIME:
                  case GE_DATETIME_DATETIME: {
                    DPT0("VV9B210: ", VV_VD, "CLAUSE_EVAL - COMP_TYPE - found DATETIME case index\n");
                    ...
                    DPT3("VV9B211: ", VV_XD, "COMP_TYPE - res = %p, src1 = %p, src2 = %p \n", res, src1, src2);

                    ////              src1JitVal = src1->getJitValue(this, f, jit_type_void_ptr,
                    ////                                             block, NULL);
                    src1JitVal = src1->getJitValue(this, Bldr, int1PtrTy_, PCBlk, NULL);

                    ////              src2JitVal = src2->getJitValue(this, f, jit_type_void_ptr,
                    ////                                             block, NULL);
                    src2JitVal = src2->getJitValue(this, Bldr, int1PtrTy_, PCBlk, NULL);
                    DPT2("VV9B212: ", VV_XD, "COMP_TYPE - src1JitVal at %p, src2JitVal at %p \n", src1JitVal,
                         src2JitVal);
                    if (processNulls) {
                      PCodeOperand *nullOp;
                      ...
                      if (clause->getOperand(1)->getNullFlag()) {
                        DPT1("VV9B213: ", VV_XD, "COMP_TYPE - opDataNulls[1] == %p\n", opDataNulls[1]);
                        ...
                        nullJitVal = nullOp->getJitValue(this, Bldr, nullOp->getJitType(this), PCBlk, NULL,
                                                         TRUE /* no assign */);

                        DPT1("VV9B214: ", VV_XD, "COMP_TYPE - nullJitVal at %p\n", nullJitVal);

                        ////                  jit_insn_branch_if(f, nullJitVal, &nullLabel);

                        DPT1("VV9B215: ", VV_XD, "IR_insn_branch_if_not_zero( Bldr, nullJitVal, &nullLabel = %p )\n",
                             nullLabel);
                      ...
                      if (clause->getOperand(2)->getNullFlag()) {
                        ...
                        DPT1("VV9B216: ", VV_XD, "COMP_TYPE-opDataNulls[2] == %p\n", opDataNulls[2]);
                        ...
                        DPT1("VV9B217: ", VV_XD, "COMP_TYPE-nullJitVal at %p\n", nullJitVal);

                        ////                  jit_insn_branch_if(f, nullJitVal, &nullLabel);

                        DPT1("VV9B218: ", VV_XD, "IR_insn_branch_if_not_zero( Bldr, nullJitVal, &nullLabel = %p )\n",
                             nullLabel);
                        IR_insn_branch_if_not_zero(Bldr, nullJitVal, &nullLabel);
                      }
                    }
                    ...
                    switch (dtCode) {
                      ...
                      case REC_DTCODE_TIMESTAMP: {
                        ...
                        for (i = 0; i < size; i += typeSize) {
                          ...
                          year1 = IR_LoadRelativeWithType(Bldr, src1JitVal, i, type);

                          ////                    year2 = jit_insn_load_relative(f, src2JitVal, i, type);
                          year2 = IR_LoadRelativeWithType(Bldr, src2JitVal, i, type);

                          DPT2("VV9B219: ", VV_XD, "COMP_TYPE-year1 at %p, year2 at %p\n", year1, year2);
                          ...
                          switch (((ex_comp_clause *)clause)->getInstruction()) {
                            case NE_DATETIME_DATETIME:
                              ...
                              resJitVal = jitGenCompare(Bldr, IntCompare_NE, year1, year2, IR_Const_1_, IR_Const_0_);
                            ...
                            case EQ_DATETIME_DATETIME:
                              ////                        resJitVal = jit_insn_ne(f, year1, year2);
                              resJitVal = jitGenCompare(Bldr, IntCompare_EQ, year1, year2, IR_Const_1_, IR_Const_0_);
                            ...
                            case LT_DATETIME_DATETIME:
                              ...
                              resJitVal = jitGenCompare(Bldr, IntCompare_ULT, year1, year2, IR_Const_1_, IR_Const_0_);
                              ...
                              resJitVal = jitGenCompare(Bldr, IntCompare_UGT, year1, year2, IR_Const_1_, IR_Const_0_);
                        ...
                        if (!dumpTrueBlockFirst) {
                          DPT1("VV9B250: ", VV_VD, "Bldr->CreateBr( falseLabel = %p )\n", falseLabel);
                          Bldr->CreateBr(falseLabel);  // Must complete the current block

                          // False label first.
                          ////                    jit_insn_label(f, &falseLabel);

                          DPT1("VV9B251: ", VV_VD, "ExpFn->getBasicBlockList().push_back( falseLabel = %p )\n",
                               falseLabel);
                          ExpFn->getBasicBlockList().push_back(falseLabel);

                          DPT1("VV9B252: ", VV_VD, "Bldr->SetInsertPoint( falseLabel = %p )\n", falseLabel);
                          Bldr->SetInsertPoint(falseLabel);

                          ////                    res->storeJitValue(this, f, jit_type_int, zeroJitVal_,
                          ////                                       block);
                          DPT0("VV9B253: ", VV_XD, "res->storeJitValue(this, Bldr, int32Ty_, IR_Const_0_, PCBlk) \n");
                          res->storeJitValue(this, Bldr, int32Ty_, IR_Const_0_, PCBlk);
                          ////                    jit_insn_branch(f, &doneLabel);

                          DPT1("VV9B254: ", VV_VD, "Bldr->CreateBr( doneLabel = %p )\n", doneLabel);
                          Bldr->CreateBr(doneLabel);

                          // True label next.
                          ////                    jit_insn_label(f, &trueLabel);

                          DPT1("VV9B256: ", VV_VD, "ExpFn->getBasicBlockList().push_back( trueLabel = %p )\n",
                               trueLabel);
                          ExpFn->getBasicBlockList().push_back(trueLabel);

                          DPT1("VV9B258: ", VV_VD, "Bldr->SetInsertPoint( trueLabel = %p )\n", trueLabel);
                          Bldr->SetInsertPoint(trueLabel);

                          ////                    res->storeJitValue(this, f, jit_type_int, oneJitVal_,
                          ////                                       block);
                          DPT0("VV9B259: ", VV_XD, "res->storeJitValue(this, Bldr, int32Ty_, IR_Const_1_, PCBlk )\n");
                          res->storeJitValue(this, Bldr, int32Ty_, IR_Const_1_, PCBlk);
                          ////                    jit_insn_branch(f, &doneLabel);

                          DPT1("VV9B260: ", VV_VD, "Bldr->CreateBr( doneLabel = %p )\n", doneLabel);
                          Bldr->CreateBr(doneLabel);
                        } else {
                          DPT1("VV9B261: ", VV_VD, "Bldr->CreateBr( trueLabel = %p )\n", trueLabel);
                          Bldr->CreateBr(trueLabel);  // Must complete the current block

                          // True label first.
                          ////                    jit_insn_label(f, &trueLabel);

                          DPT1("VV9B262: ", VV_VD, "ExpFn->getBasicBlockList().push_back( trueLabel = %p )\n",
                               trueLabel);
                          ExpFn->getBasicBlockList().push_back(trueLabel);

                          DPT1("VV9B264: ", VV_VD, "Bldr->SetInsertPoint( trueLabel = %p )\n", trueLabel);
                          Bldr->SetInsertPoint(trueLabel);

                          ////                    res->storeJitValue(this, f, jit_type_int, oneJitVal_,
                          ////                                       block);
                          DPT0("VV9B265: ", VV_XD, "res->storeJitValue(this, Bldr, int32Ty_, IR_Const_1_, ...)\n");
                          res->storeJitValue(this, Bldr, int32Ty_, IR_Const_1_, PCBlk);
                          ////                    jit_insn_branch(f, &doneLabel);

                          DPT1("VV9B266: ", VV_VD, "Bldr->CreateBr( doneLabel = %p )\n", doneLabel);
                          Bldr->CreateBr(doneLabel);

                          // False label next.
                          ////                    jit_insn_label(f, &falseLabel);

                          DPT1("VV9B268: ", VV_VD, "ExpFn->getBasicBlockList().push_back( falseLabel = %p )\n",
                               falseLabel);
                          ExpFn->getBasicBlockList().push_back(falseLabel);

                          DPT1("VV9B270: ", VV_VD, "Bldr->SetInsertPoint( falseLabel = %p )\n", falseLabel);
                          Bldr->SetInsertPoint(falseLabel);

                          ////                    res->storeJitValue(this, f, jit_type_int, zeroJitVal_,
                          ////                                       block);

                          DPT0("VV9B271: ", VV_XD, "res->storeJitValue(this, Bldr, int32Ty_, IR_Const_0_, PCBlk )\n");
                          res->storeJitValue(this, Bldr, int32Ty_, IR_Const_0_, PCBlk);
                          ////                    jit_insn_branch(f, &doneLabel);

                          DPT1("VV9B272: ", VV_VD, "Bldr->CreateBr( doneLabel = %p )\n", doneLabel);
                          Bldr->CreateBr(doneLabel);
                        }
                        ...
                        DPT1("VV9B274: ", VV_VD, "ExpFn->getBasicBlockList().push_back( nullLabel = %p )\n", nullLabel);
                        ExpFn->getBasicBlockList().push_back(nullLabel);

                        DPT1("VV9B276: ", VV_VD, "Bldr->SetInsertPoint( nullLabel = %p )\n", nullLabel);
                        Bldr->SetInsertPoint(nullLabel);

                        ////                  res->storeJitValue(this,f,jit_type_int,neg1JitVal_,block);

                        DPT0("VV9B277: ", VV_XD, "res->storeJitValue(this, Bldr, int32Ty_, IR_Const_neg1_, PCBlk )\n");
                        res->storeJitValue(this, Bldr, int32Ty_, IR_Const_neg1_, PCBlk);

                        ////                  jit_insn_label(f, &doneLabel);

                        DPT1("VV9B278: ", VV_VD, "Bldr->CreateBr( doneLabel = %p )\n", doneLabel);
                        Bldr->CreateBr(doneLabel);  // Must complete the current block

                        DPT1("VV9B279: ", VV_VD, "ExpFn->getBasicBlockList().push_back( doneLabel = %p )\n", doneLabel);
                        ExpFn->getBasicBlockList().push_back(doneLabel);

                        DPT1("VV9B280: ", VV_VD, "Bldr->SetInsertPoint( doneLabel = %p )\n", doneLabel);
                        Bldr->SetInsertPoint(doneLabel);
          ...
          case PCIT::SUM_MBIN64S_MBIN64S: {
...
#if NExprDbgLvl >= VV_VD
            if (src1->isConst()) DPT0("VV9C005: ", VV_VD, "case PCIT::SUM_MB...  src1 is a CONSTANT !\n");

            if (src2->isConst()) DPT0("VV9C006: ", VV_VD, "case PCIT::SUM_MB...  src2 is a CONSTANT !\n");
#endif

            ////      resJitVal = jit_insn_add(f, src1JitVal, src2JitVal);

            DPT0("VV9C010: ", VV_VD, "resJitVal = Bldr->CreateAdd( src1JitVal , src2JitVal , 'IntAdd' )\n");
            ...
            // Lastly, clear out jit values for 32-bit source operand if needed.
            if (opc == PCIT::SUM_MBIN64S_MBIN32S) src2->clearJitValues(this);
          ...
          case PCIT::ADD_MBIN64S_MBIN32S_MBIN64S: {
            ...
            DPT0("VV9C110: ", VV_VD, "resJitVal = Bldr->CreateAdd( src1JitVal , src2JitVal , 'IntAdd' )\n");
            resJitVal = Bldr->CreateAdd(src1JitVal, src2JitVal, "IntAdd");
            ...
            // Lastly, clear out jit values for 32-bit source operand if needed.
            if (opc == PCIT::ADD_MBIN64S_MBIN32S_MBIN64S) src1->clearJitValues(this);
          ...
          case PCIT::SUB_MBIN32S_MBIN32S_MBIN16S: {
            ...
            DPT0("VV9C210: ", VV_VD, "resJitVal = Bldr->CreateSub( src1JitVal , src2JitVal , 'IntSub' )\n");
          ...
          case PCIT::MUL_MBIN32S_MBIN16S_MBIN32S: {
            ...
            DPT0("VV9C310: ", VV_VD, "resJitVal = Bldr->CreateMul( src1JitVal , src2JitVal , 'IntMul' )\n");
          ...
          case PCIT::MOVE_MBIN32S: {
            ...
            DPT1("VV9D002: ", VV_XD, "Processing MOVE_MBIN32S - 1, src1 = PCodeOp at %p\n", src1);

            ////      src1JitVal = src1->getJitValue(this, f, src1->getJitType(), PCBlk);
            src1JitVal = src1->getJitValue(this, Bldr, src1->getJitType(this), PCBlk);

            DPT1("VV9D003: ", VV_XD, "Processing MOVE_MBIN32S - 1, src1JitVal at %p\n", src1JitVal);
            ...
            jit_value_t Rslt =
                jitGenCompare(Bldr, IntCompare_EQ, src1JitVal, IR_Const_1_, IR_Const_TRUE_, IR_Const_FALSE_);

            DPT0("VV9D005: ", VV_VD, "returned from jitGenCompare()\n");

            DPT0("VV9D010: ", VV_VD, "Bldr->CreateRet( Rslt )\n");
            Bldr->CreateRet(Rslt);
          ...
          case PCIT::RETURN_IBIN32S: {
            ...
            DPT0("VV9E010: ", VV_VD, "Bldr->CreateRet( resJitVal )\n");
            Bldr->CreateRet(resJitVal);
          ...
          case PCIT::RETURN: {
            DPT0("VV9F001: ", VV_VD, "Bldr->CreateRet( IR_Const_OK_ )\n");
...
#if NExprDbgLvl >= VV_VD
      if (PCInst)
        DPT2("VVzz000: ", VV_VD, "At end of Main Loop : PCInst at %p, PCInst->next = %p \n", PCInst, PCInst->next);

      if (PCInst && PCInst->next)
        DPT2("VVzz001: ", VV_VD, "At end of Main Loop: next at %p has opc == %d\n", PCInst->next,
             (int)PCInst->next->getOpcode());
#endif
    }
    ENDFE_INST_IN_BLOCK
    ...
    if ((opc != PCIT::BRANCH) &&                      // if last one was not a BRANCH
        (justGeneratedRET == FALSE) &&                // and was not a RETURN of any sort, and if
      ...
      DPT1("VVzzz00: ", VV_VD, "Bldr->CreateBr( TgtBlk = %p )\n", TgtBlk);
      Bldr->CreateBr(TgtBlk);
    }
  ...
  if (justGeneratedRET == FALSE && justHandledUnconditionBranch == FALSE) {
    ...
    //
    DPT0("VVzzz12: ", VV_VD, "Bldr->CreateRet( IR_Const_OK_ )\n");
    Bldr->CreateRet(IR_Const_OK_);
  }
...
#if NExprDbgLvl >= VV_BD
  if (NExprDbgLvl_ >= VV_BD) std::cout << std::flush;  // Flush anything in the data buffer
#endif
...
// LLVM Requires this function be called!
  ...
  if (1) {
#ifdef NEW_VERSION_LLVM
    llvm::legacy::FunctionPassManager ExprFPM(TheModulePtr);

    // Set up the optimizer pipeline.
    // Start with registering info about how the
    // target lays out data structures.
    TheModulePtr->setDataLayout(TheExecutionEngine->getDataLayout());
    // Basic AliasAnslysis support for GVN. 3.8.1 has no createBasicAliasAnalysisPass
    // ExprFPM.add(llvm::createBasicAliasAnalysisPass());
    // Promote allocas to registers.
    ExprFPM.add(llvm::createPromoteMemoryToRegisterPass());
    // Do simple "peephole" optimizations and bit-twiddling optzns.
    ExprFPM.add(llvm::createInstructionCombiningPass());
    // Reassociate expressions.
    ExprFPM.add(llvm::createReassociatePass());
    // Eliminate Common SubExpressions.
    ExprFPM.add(llvm::createGVNPass());
    // Simplify the control flow graph (deleting unreachableblocks, etc).
    ExprFPM.add(llvm::createCFGSimplificationPass());
  ...
  if (NExprDbgLvl_ >= VV_BD) {
    ...
    if (NExDbgInfoPtr_->getNExLogPath() && NExDbgInfoPtr_->getNExLogPath()[0] != '\0') {
#ifdef NEW_VERSION_LLVM
      std::error_code myErrorInfoStr;
      raw_fd_ostream *myOuts =
          new raw_fd_ostream((const char *)NExDbgInfoPtr_->getNExLogPath(), myErrorInfoStr, llvm::sys::fs::F_Append);
#else
      std::string myErrorInfoStr;
      raw_fd_ostream *myOuts = new raw_fd_ostream((const char *)NExDbgInfoPtr_->getNExLogPath(), myErrorInfoStr,
  ...
  void *FPtr = TheExecutionEngine->getPointerToFunction(ExpFn);
  size_t funcBufLen = helpingMemoryManager->getFunctionSize(FPtr);
#else
...
#if NExprDbgLvl >= VV_BD
  if ((NExprDbgLvl_ >= VV_BD) &&
      (((NExDbgInfoPtr_->getNExStmtSrc() != NULL) && (NExDbgInfoPtr_->getNExStmtSrc()[0] != '\0')))) {
    // ********************************************************************
    ...
    while (ud_disassemble(&ud_obj)) {
      ...
      for (int jjj = 0; jjj < insnLen; jjj++, ooo++) {
        ...
        //
        if ((!NExLogPth || *NExLogPth == '\0') && (insnLine[jjj] == '%')) {
          insnLine2[ooo] = '%';
          ooo++;
        }
...
void PCodeCfg::layoutNativeCode(Space *showplanSpace = NULL) {
  ...
  if (getenv("NATIVE_EXPR_DEBUG")) debug = TRUE;
  ...
  // First see if this graph can be compiled natively
  if (!canGenerateNativeExpr()) return;
  ...
  */
  ...
  jit_value_t jitValMask16 = jit_value_create_nint_constant(f, jit_type_ushort, 0x8000);
  jit_value_t jitValMask32 = jit_value_create_nint_constant(f, jit_type_ushort, 0x80000000);
  jit_value_t jitValNotMask16 = jit_value_create_nint_constant(f, jit_type_ushort, 0x7fff);
  jit_value_t jitValNotMask32 = jit_value_create_nint_constant(f, jit_type_ushort, 0x7fffffff);
  ...
  jit_value_t nullTable = jit_insn_load_relative(f, globTableJitVal, JIT_GLOB_NULL_TABLE, jit_type_void_ptr);

  hashTableJitVal_ = jit_insn_load_relative(f, globTableJitVal, JIT_GLOB_RANDOM_HASH_VALS_TABLE, jit_type_void_ptr);

  jit_value_t compareWithPadJitVal =
      jit_insn_load_relative(f, globTableJitVal, JIT_GLOB_COMPARE_WITH_PAD_FUNC, jit_type_void_ptr);

  jit_value_t strcmpJitVal = jit_insn_load_relative(f, globTableJitVal, JIT_GLOB_STRCMP_FUNC, jit_type_void_ptr);

  jit_value_t strcpyJitVal = jit_insn_load_relative(f, globTableJitVal, JIT_GLOB_STRCPY_FUNC, jit_type_void_ptr);

  jit_value_t dblLeJitVal = jit_insn_load_relative(f, globTableJitVal, JIT_GLOB_DBL_LE_CMP_FUNC, jit_type_void_ptr);
  jit_value_t dblGeJitVal = jit_insn_load_relative(f, globTableJitVal, JIT_GLOB_DBL_GE_CMP_FUNC, jit_type_void_ptr);
  jit_value_t dblLtJitVal = jit_insn_load_relative(f, globTableJitVal, JIT_GLOB_DBL_LT_CMP_FUNC, jit_type_void_ptr);
  jit_value_t dblGtJitVal = jit_insn_load_relative(f, globTableJitVal, JIT_GLOB_DBL_GT_CMP_FUNC, jit_type_void_ptr);
  jit_value_t dblEqJitVal = jit_insn_load_relative(f, globTableJitVal, JIT_GLOB_DBL_EQ_CMP_FUNC, jit_type_void_ptr);
  jit_value_t dblNeJitVal = jit_insn_load_relative(f, globTableJitVal, JIT_GLOB_DBL_NE_CMP_FUNC, jit_type_void_ptr);
  jit_value_t reportErrJitVal = jit_insn_load_relative(f, globTableJitVal, JIT_GLOB_REPORT_ERR_FUNC, jit_type_void_ptr);
  jit_value_t bigSubJitVal = jit_insn_load_relative(f, globTableJitVal, JIT_GLOB_BIG_SUB_FUNC, jit_type_void_ptr);

  jit_type_t compareParams[5] = {jit_type_void_ptr, jit_type_int, jit_type_void_ptr, jit_type_int, jit_type_ubyte};
  jit_type_t strcmpParams[5] = {jit_type_void_ptr, jit_type_void_ptr, jit_type_uint};
  jit_type_t strcpyParams[5] = {jit_type_void_ptr, jit_type_void_ptr, jit_type_uint};
  jit_type_t dblParams[2] = {jit_type_float64, jit_type_float64};
  jit_type_t reportErrParams[2] = {jit_type_void_ptr, jit_type_void_ptr};
  jit_type_t bigParams[4] = {jit_type_void_ptr, jit_type_void_ptr, jit_type_void_ptr, jit_type_uint};

  jit_type_t compareSig = jit_type_create_signature(jit_abi_cdecl, jit_type_int, compareParams, 5, 1);

  jit_type_t strcmpSig = jit_type_create_signature(jit_abi_cdecl, jit_type_int, strcmpParams, 3, 1);
  jit_type_t strcpySig = jit_type_create_signature(jit_abi_cdecl, jit_type_int, strcpyParams, 3, 1);
  jit_type_t dblSig = jit_type_create_signature(jit_abi_cdecl, jit_type_int, dblParams, 2, 1);
  jit_type_t reportErrSig = jit_type_create_signature(jit_abi_cdecl, jit_type_int, reportErrParams, 2, 1);
  jit_type_t bigSig = jit_type_create_signature(jit_abi_cdecl, jit_type_int, bigParams, 4, 1);
  ...
  for (blockIndex = 0; (blockIndex < physBlockList.entries()) && !getJitFailureSeen(); blockIndex++) {
    ...
    FOREACH_INST_IN_BLOCK(block, inst) {
      ...
      switch (opc) {
        ...
        case PCIT::BRANCH_AND_CNT: {
          ...
          jit_value_t takenJitVal =
              ((opc == PCIT::BRANCH_OR) || (opc == PCIT::BRANCH_OR_CNT)) ? oneJitVal_ : zeroJitVal_;
        ...
        case PCIT::NOT_NULL_BRANCH_COMP_MBIN32S_MBIN32S_MBIN32S_IATTR4_IBIN32S: {
          ...
          if (res->isVar() || block->isOperandLiveInSuccs(res)) {
            res->storeNullJitValueAndBranch(this, f, orRes, jitLabel, forComp, block);
          } else {
            jit_insn_branch_if_not(f, orRes, jitLabel);
          }
        ...
        case PCIT::NOT_NULL_BRANCH_MBIN32S_MBIN32S_IATTR3_IBIN32S: {
          ...
          if (res->isVar() || block->isOperandLiveInSuccs(res)) {
            res->storeNullJitValueAndBranch(this, f, src1JitVal, jitLabel, forComp, block);
          } else {
            jit_insn_branch_if_not(f, src1JitVal, jitLabel);
          }
        ...
        case PCIT::COMP_MBIN32S_MBIGS_MBIGS_IBIN32S_IBIN32S: {
          ...
          jit_value_t lenJitVal = jit_value_create_nint_constant(f, jit_type_int, len);
          ...
          jit_value_t decrJitVal = jit_value_create_nint_constant(f, jit_type_int, 2);
          ...
          if ((subOpc == ITM_EQUAL) || (subOpc == ITM_LESS_EQ) || (subOpc == ITM_GREATER_EQ))
            res->storeJitValue(this, f, jit_type_int, oneJitVal_, block);
          else
            res->storeJitValue(this, f, jit_type_int, zeroJitVal_, block);
        ...
        case PCIT::GE_MBIN32S_MASCII_MASCII: {
          ...
          NABoolean isEqOrNe = (opc == PCIT::EQ_MBIN32S_MASCII_MASCII) || (opc == PCIT::NE_MBIN32S_MASCII_MASCII);
          ...
          if (!jitProcessPredicate(inst, f, &falseLabel, &trueLabel, dumpTrueBlockFirst)) {
            if (dumpTrueBlockFirst) {
              // True block
              jit_insn_label(f, &trueLabel);
              res->storeJitValue(this, f, jit_type_int, oneJitVal_, block);
        ...
        case PCIT::SUM_MATTR3_MATTR3_IBIN32S_MBIN64S_MBIN32S: {
          ...
          // Clear null value of target and clear out it's value.
          res->storeJitValue(this, f, res->getJitType(), src2JitVal, block);
          src1->storeNullJitValueAndBranch(this, f, zeroJitVal_, &doneLabel, FALSE, block);
          ...
          // Lastly, clear out jit values for 32-bit source operand if needed.
          if (opc == PCIT::SUM_MATTR3_MATTR3_IBIN32S_MBIN64S_MBIN32S) src2->clearJitValues(this);
        ...
        case PCIT::ADD_MFLT64_MFLT64_MFLT64: {
          ...
          if (opc == PCIT::ADD_MFLT64_MFLT64_MFLT64) jitProcessFloatExceptionCheck(f, res, &errorJitLabel, block);
        ...
        case PCIT::SUB_MFLT64_MFLT64_MFLT64: {
          ...
          if (opc == PCIT::SUB_MFLT64_MFLT64_MFLT64) jitProcessFloatExceptionCheck(f, res, &errorJitLabel, block);

          // No opt support for this instruction
          if (opc == PCIT::SUB_MBIN64S_MBIN64S_MBIN64S) lowerOptLevel = TRUE;
        ...
        case PCIT::MUL_MFLT64_MFLT64_MFLT64: {
          ...
          if (opc == PCIT::MUL_MFLT64_MFLT64_MFLT64) jitProcessFloatExceptionCheck(f, res, &errorJitLabel, block);
        ...
        case PCIT::DIV_MFLT64_MFLT64_MFLT64: {
          ...
          if (opc == PCIT::DIV_MFLT64_MFLT64_MFLT64) jitProcessFloatExceptionCheck(f, res, &errorJitLabel, block);
        ...
        case PCIT::MOVE_MBIN64S_MBIN32S: {
          ...
          // Lastly, clear out jit values for 32-bit source operand if needed.
          if ((opc == PCIT::MOVE_MBIN64S_MBIN16U) || (opc == PCIT::MOVE_MBIN64S_MBIN16S) ||
              (opc == PCIT::MOVE_MBIN64S_MBIN32U) || (opc == PCIT::MOVE_MBIN64S_MBIN32S))
        ...
        case PCIT::STRLEN_MBIN32U_MATTR5: {
          ...
          if (src1->getVcIndicatorLen() == 2) resJitVal = jit_insn_convert(f, resJitVal, jit_type_int, 0);
        ...
        case PCIT::SWITCH_MBIN32S_MBIN64S_MPTR32_IBIN32S_IBIN32S: {
          ...
          jit_value_t invalidInt32JitVal = jit_value_create_nint_constant(f, jit_type_uint, INVALID_INT32);

          jit_value_t hashTableSizeJitVal = jit_value_create_nint_constant(f, jit_type_uint, inst->code[7]);
        ...
        case PCIT::SWITCH_MBIN32S_MATTR5_MPTR32_IBIN32S_IBIN32S: {
          ...
          jit_value_t hashTableSizeJitVal = jit_value_create_nint_constant(f, jit_type_uint, inst->code[10]);
          ...
          t2 = jit_insn_load_relative(f, resJitVal, 4, jit_type_uint);
          src2JitVal = jit_insn_load_elem_address(f, jitParams_[1], t2, jit_type_ubyte);

          jit_value_t args[3] = {src1JitVal, src2JitVal, len};
          t1 = jit_insn_call_indirect(f, strcmpJitVal, strcmpSig, args, 3, 0);
        ...
        case PCIT::HASH_MBIN32U_MBIN64_IBIN32S_IBIN32S: {
          ...
          if (opc == PCIT::HASH_MBIN32U_MATTR5) len = genStringSetup(f, &src1JitVal, src1, TRUE);
        ...
        case PCIT::SUB_MBIGS_MBIGS_MBIGS_IBIN32S: {
          ...
          jit_value_t lenJitVal = jit_value_create_nint_constant(f, jit_type_int, src1->getLen());
          ...
          resJitVal = jit_insn_call_indirect(f, bigSubJitVal, bigSig, args, 4, 0);
        ...
        case PCIT::ADD_MBIGS_MBIGS_MBIGS_IBIN32S: {
          ...
          genBignumSub(this, f, res, src1JitVal, src2JitVal, t1, t2, tSrc1, tSrc2, block);
          ...
          for (i = 0; i < len; i += 2) {
            ...
            if (i != 0) t3 = jit_insn_add(f, t3, carry);
            ...
            case ex_clause::AGGR_MIN_MAX_ID: {
              ...
              if (clause->getOperand(0)->getNullFlag()) {
                ...
                if (clause->getOperand(1)->getNullFlag()) {
                  src1 = opDataNulls[1];
                  src1JitVal =
                      src1->getJitValue(this, f, src1->getJitType(), block, NULL, TRUE /* no assign jit val */);
              ...
              src1JitVal = src1->getJitValue(this, f, src1->getJitType(), block, NULL, TRUE /* no assign */);
              ...
              if (res->isVarchar()) {
                ...
                resJitVal = res->getJitValue(this, f, res->getJitType(), block, NULL, TRUE /* no assign */);
                ...
                res->storeJitValue(this, f, res->getJitType(), src1JitVal, block, NULL, TRUE);
            ...
            case ex_clause::COMP_TYPE: {
              switch (((ex_comp_clause *)clause)->getInstruction()) {
                ...
                case GE_DATETIME_DATETIME: {
                  ...
                  src1JitVal = src1->getJitValue(this, f, jit_type_void_ptr, block, NULL);
                  src2JitVal = src2->getJitValue(this, f, jit_type_void_ptr, block, NULL);
                  if (processNulls) {
                    PCodeOperand *nullOp;
                    jit_value_t nullJitVal;
                    ...
                    if (clause->getOperand(1)->getNullFlag()) {
                      nullOp = opDataNulls[1];
                      nullJitVal =
                          nullOp->getJitValue(this, f, nullOp->getJitType(), block, NULL, TRUE /* no assign jit val */);
        ...
        case PCIT::CONCAT_MATTR5_MATTR5_MATTR5: {
          ...
          assert((src1->getVcMaxLen() + src2->getVcMaxLen() <= res->getVcMaxLen()));
  ...
  if (errorJitLabel != jit_label_undefined) {
    ...
    resJitVal = jit_insn_call_indirect(f, reportErrJitVal, reportErrSig, args, 2, 0);
  ...
  if (debug) printBlocks("NativeExpr", FALSE);

  // Set optimization level for jit compilation.
  if (!lowerOptLevel) jit_function_set_optimization_level(f, 2);
  ...
  // Dump function
  if (debug) jit_dump_function(stdout, f, "X86:");
  ...
  if (showplanSpace) {
    ...
    for (i = 0; i < INDENT_START; i++) line[i] = ' ';
    ...
    while (fgets(&(line[INDENT_START]), sizeof line, out) != NULL) {
      ...
      // starting as "fffff..".
      if (line[INDENT_START] != 'f') continue;
