...
BigNum::BigNum() { setClassID(BigNumID); }
...
int BigNum::setTempSpaceInfo(OperatorTypeEnum operType, ULong offset, int length) {
  ...
  } else if (operType == ITM_DIVIDE || operType == ITM_CAST ||
...
void BigNum::fixup(Space *space, char *constantsArea, char *tempsArea, char *persistentArea, short fixupConstsAndTemps,
                   NABoolean spaceCompOnly) {
  if ((tempSpaceLength_ > 0) && (NOT spaceCompOnly)) tempSpacePtr_ = (Long)(tempsArea + tempSpaceOffset_);

  Attributes::fixup(space, constantsArea, tempsArea, persistentArea, fixupConstsAndTemps, spaceCompOnly);
}

// Convert (i.e. copy) the Big Num from op_data[1] ("source") to op_data[0] ("this").

short BigNum::conv(Attributes *source, char *op_data[]) {
  return BigNumHelper::ConvBigNumWithSignToBigNumWithSignHelper(((BigNum *)source)->getLength(), getLength(),
                                                                op_data[1], op_data[0]);
}
...
short BigNum::comp(OperatorTypeEnum compOp, Attributes *other, char *op_data[]) {
  ...
  if (allZeroes)
    ...
    if ((thisSign == 0) && (otherSign))  // this +ve, other -ve
      compCode = 1;
    else if ((thisSign) && (otherSign == 0))  // this -ve, other +ve
      compCode = -1;
  ...
  // Reset sign bits
  if (thisSign) BIGN_SET_SIGN(op_data[1], getLength());

  if (otherSign) BIGN_SET_SIGN(op_data[2], getLength());
...
short BigNum::add(Attributes *left, Attributes *right, char *op_data[]) {
  // Extract signs.
  ...
  // Reset sign bits
  if (leftSign) BIGN_SET_SIGN(op_data[1], getLength());

  if (rightSign) BIGN_SET_SIGN(op_data[2], getLength());
...
short BigNum::sub(Attributes *left, Attributes *right, char *op_data[]) {
  // Extract signs.
  char leftSign = BIGN_GET_SIGN(op_data[1], getLength());
  char rightSign = BIGN_GET_SIGN(op_data[2], getLength());
  ...
  if ((!isUnsigned()) && ((leftSign) || (rightSign))) {
    if (leftSign == 0) {  // left - (-right) => left + right
      BigNumHelper::AddHelper(getLength(), op_data[1], op_data[2], op_data[0]);
      code = 0;
    } else if (rightSign == 0) {  // -left - right => -(left + right)
      BigNumHelper::AddHelper(getLength(), op_data[1], op_data[2], op_data[0]);
      code = 1;
...
short BigNum::mul(Attributes *left, Attributes *right, char *op_data[]) {
  // Extract signs.
  ...
  BigNumHelper::MulHelper(getLength(), ((BigNum *)left)->getLength(), ((BigNum *)right)->getLength(), op_data[1],
                          op_data[2], op_data[0]);

  // Reset sign bits
  if (leftSign) BIGN_SET_SIGN(op_data[1], ((BigNum *)left)->getLength());

  if (rightSign) BIGN_SET_SIGN(op_data[2], ((BigNum *)right)->getLength());
...
short BigNum::div(Attributes *left, Attributes *right, char *op_data[], NAMemory *heap, ComDiagsArea **diagsArea) {
  // Extract signs.
  // Extract signs.
  char leftSign = BIGN_GET_SIGN(op_data[1], ((BigNum *)left)->getLength());
  char rightSign = BIGN_GET_SIGN(op_data[2], ((BigNum *)right)->getLength());
  ...
  if (rightLengthInShorts == 0) {
    if (heap) ExRaiseSqlError(heap, diagsArea, EXE_DIVISION_BY_ZERO);
  ...
  if (rightLengthInShorts == 1) {
    BigNumHelper::SimpleDivHelper(((BigNum *)left)->getLength(), 2, op_data[1], op_data[2], op_data[0]);
  } else {
    BigNumHelper::DivHelper(((BigNum *)left)->getLength(), 2 * rightLengthInShorts, op_data[1], op_data[2], op_data[0],
                            (char *)tempSpacePtr_);
  }
...
short BigNum::castFrom(Attributes *source, char *op_data[], NAMemory *heap, ComDiagsArea **diagsArea) {
  ...
  switch (sourceType) {
    ...
    case REC_DECIMAL_LSE: {
      ...
      BigNumHelper::ConvAsciiToBigNumHelper(source1->getLength(), getLength(), op_data[1], op_data[0]);

      // Set sign in this (must be done after conversion to prevent overwrite)
      if (temp & 0200) BIGN_SET_SIGN(op_data[0], getLength());
    ...
    case REC_FLOAT32: {
      ...
      if (convDoIt(op_data[1], 4, REC_FLOAT32, 0, 0, tempTarget, SQL_REAL_DISPLAY_SIZE, REC_BYTE_F_ASCII, 0, 0, NULL, 0,
      ...
      if (tempTarget[11] == '-') exponent = -exponent;
      ...
      BigNumHelper::MulHelper(getLength(), 4, tempPowersOfTenLength, (char *)&mantissa, tempPowersOfTen, op_data[0]);

      // Set sign of this.
      if (tempTarget[0] == '-') BIGN_SET_SIGN(op_data[0], getLength());
    ...
    case REC_FLOAT64: {
      ...
      // where '-' is '-' or ' '; '+' is '+' or '-'; n is null character;
      //   and 'd' is a digit in ascii.
      if (convDoIt(op_data[1], 8, REC_FLOAT64, 0, 0, tempTarget, SQL_FLOAT_DISPLAY_SIZE, REC_BYTE_F_ASCII, 0, 0, NULL,
      ...
      if (tempTarget[21] == '-') exponent = -exponent;
      ...
      // Multiply mantissaData with tempPowersOfTen and store in this.
      BigNumHelper::MulHelper(getLength(), 8, tempPowersOfTenLength, (char *)&mantissa, tempPowersOfTen, op_data[0]);
      ...
      if ((Long)tempPowersOfTen != tempSpacePtr_) NADELETEBASIC(tempPowersOfTen, heap);
...
short BigNum::round(Attributes *left, Attributes *right, char *op_data[], NAMemory *heap, ComDiagsArea **diagsArea) {
  ...
  if (right) {
    ...
    switch (right->getDatatype()) {
      ...
      default: {
        ...
        if (heap) ExRaiseSqlError(heap, diagsArea, EXE_INTERNAL_ERROR);
        return -1;
      } break;
    }
  ...
  // Clear sign bits
  BIGN_CLR_SIGN(op_data[1], left->getLength());
  ...
  // Reset sign bits
  if (leftSign) BIGN_SET_SIGN(op_data[1], left->getLength());
...
void BigNum::encode(const char *inBuf, char *outBuf, short desc) {
  ...
  // Reset sign
  if (neg) BIGN_SET_SIGN(inBuf, getLength());
...
void BigNum::decode(const char *inBuf, char *outBuf, short desc) {
  ...
  if (!neg) BIGN_CLR_SIGN(outBuf, getLength());
}
...

{
  ...
  // Set sign bit
  if (str[0] == '-') BIGN_SET_SIGN(op_data, getLength());
}
