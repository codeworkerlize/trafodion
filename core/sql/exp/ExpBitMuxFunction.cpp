...
ex_expr::exp_return_type ExpBitMuxFunction::pCodeGenerate(Space *space, UInt32 f) {
  ...
  for (; i < numOperands; i++) {
    // Src must be ATP
    //
    if (attrs[i]->getAtpIndex() < 2) return ex_clause::pCodeGenerate(space, f);

    // No varchars
    //
    if (attrs[i]->getVCIndicatorLength() > 0) return ex_clause::pCodeGenerate(space, f);

    // No nulls. Fix for genesis case 10-980114-7618 : AS 01/22/98
    //
    if (attrs[i]->getNullIndicatorLength() > 0) return ex_clause::pCodeGenerate(space, f);
  ...
  for (i = 1; i < numOperands; i++) {
    if (attrs[i]->getNullFlag()) {
      OL ol(attrs[0]->getAtp(), attrs[0]->getAtpIndex(), offset, attrs[i]->getAtp(), attrs[i]->getAtpIndex(),
    ...
    OL ol(attrs[0]->getAtp(), attrs[0]->getAtpIndex(), offset, attrs[i]->getAtp(), attrs[i]->getAtpIndex(),
          attrs[i]->getOffset(), attrs[i]->getLength());
    PCI pci(PCIT::Op_MOVE, aml, ol);
    code.append(pci);
...
//
ex_expr::exp_return_type ExpBitMuxFunction::eval(char *op_data[], CollHeap *, ComDiagsArea **) {
  ...
  for (Int32 i = 1; i < getNumOperands(); i++) {
    ...
    if (attrs[i]->getVCIndicatorLength() > 0) {
      if (isNull)  // a null value, make datalength 0.
        ...
        // comparison rules go.
        while ((dataLength > 0) && (srcData[dataLength - 1] == ' ')) dataLength--;
      }
