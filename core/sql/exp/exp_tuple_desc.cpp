...
ExpTupleDesc::ExpTupleDesc(UInt32 num_attrs, Attributes **attrs, UInt32 tupleDataLength, TupleDataFormat tdataF,
  ...
  if (space != NULL) {
...
#endif
    {
      ...
      // 'this' class.
      attrs_ = (AttributesPtr *)(space->allocateAlignedSpace(num_attrs * sizeof(AttributesPtr)));
    ...
    tupleDescLength_ = sizeof(*this) + space->getAllocatedSpaceSize() - tupleDescLength_;
  } else {
    flags_ = 0;
    attrs_ = attrs;
...
ExpTupleDesc::ExpTupleDesc(UInt32 num_attrs, AttributesPtr *attrs, UInt32 tupleDataLength, TupleDataFormat tdataF,
...
ExpTupleDesc::ExpTupleDesc() : NAVersionedObject(-1) {}
...
// were added.
static Int16 orderFixedFieldsByAlignment(Attributes **attrs, NAList<UInt32> *fixedFields) {
  Int16 rc = 0;
...
Int16 ExpTupleDesc::computeOffsets(Attributes *attr, TupleDataFormat tf, UInt32 &datalen, UInt32 start_offset) {
  UInt32 rtnFlags;
  ExpTupleDesc::computeOffsets(1, &attr, tf, datalen,
...
static void computeOffsetOfFixedField(Attributes **attrs, UInt32 fieldIdx, UInt32 bitmapOffset, UInt32 &fixedOffset,
                                      UInt32 &firstField, UInt32 &prevIdx, UInt16 &nullBitIdx,
                                      ExpHdrInfo *hdrInfo = NULL, NABoolean alignFormat = FALSE) {
  Attributes *field = attrs[fieldIdx];

  if (prevIdx != ExpOffsetMax) attrs[prevIdx]->setNextFieldIndex(fieldIdx);
  ...
  if (firstField == ExpOffsetMax) {
    ...
    if (!alignFormat) field->setVoaOffset(0);

    if (hdrInfo != NULL) hdrInfo->setFirstFixedOffset(firstField);
  }
  ...
  switch (tf) {
    case PACKED_FORMAT: {
      ...
      for (UInt32 i = 0; i < num_attrs; i++) {
        ...
        if (attrs[i]->getNullFlag()) attrs[i]->setNullIndicatorLength(NULL_INDICATOR_LENGTH);
        if (attrs[i]->getVCIndicatorLength() > 0) attrs[i]->setVCIndicatorLength(VC_ACTUAL_LENGTH);

        // all fields except char/varchar/binary fields may need alignment
        // at runtime.
        if (NOT DFS2REC::isAnyCharacter(attrs[i]->getDatatype())) attrs[i]->needDataAlignment();
    ...
    case SQLMX_KEY_FORMAT: {
      for (UInt32 i = 0; i < num_attrs; i++) {
        ...
        if (attrs[i]->getNullFlag()) attrs[i]->setNullIndicatorLength(KEY_NULL_INDICATOR_LENGTH);
        if (attrs[i]->getVCIndicatorLength() > 0) attrs[i]->setVCIndicatorLength(VC_ACTUAL_LENGTH);
    ...
    case SQLARK_EXPLODED_FORMAT: {
      // If offsets are passed in, then use them directly without
      // any verification. We must compute total length.
      if (offsets) {
        offset = 0;
        ...
        for (UInt32 i = 0; i < num_attrs; i++) {
          ...
          attrs[i]->setNullIndOffset(offsets[i]->getNullIndOffset());

          attrs[i]->setVCLenIndOffset(offsets[i]->getVCLenIndOffset());

          attrs[i]->setOffset(offsets[i]->getOffset());

          UInt32 endOffset = attrs[i]->getOffset() + attrs[i]->getLength();

          if (endOffset > offset) offset = endOffset;
        }
      ...
      for (UInt32 i = 0; i < num_attrs; i++) {
        ...
        offset = ExpTupleDesc::sqlarkExplodedOffsets(
            offset, (UInt32)attrs[i]->getDataAlignmentSize(), attrs[i]->getDatatype(), attrs[i]->getNullFlag(),
            &nullIndicatorOffset, &vcIndicatorOffset,
        ...
        if ((attrs[i]->getRowsetSize() == 0) &&
            ((attrs[i]->getDatatype() == REC_FLOAT32) || (attrs[i]->getDatatype() == REC_FLOAT64)))
    ...
    case SQLMX_FORMAT: {
      ...
      Int16 varIndLen = (Int16)(alignedFormat ? (Int16)ExpAlignedFormat::OFFSET_SIZE : (Int16)SQLMX_VC_ACTUAL_LENGTH);
      UInt32 voaSz = (alignedFormat ? ExpAlignedFormat::OFFSET_SIZE : ExpVoaSize);
      ...
      *rtnFlags = 0;
      ...
      for (i = 0; i < num_attrs; i++) {
        ...
        if (attrs[i]->isAddedCol()) {
          *rtnFlags |= ExpTupleDesc::ADDED_COLUMN;
          ...
          if ((attrs[i]->getNullFlag() && alignedFormat) || (attrs[i]->getVCIndicatorLength() > 0) ||
      ...
      if (alignedFormat) {
        ...
        // pad bytes.
        if ((NULL != hdrInfo) && (varFields.entries() == 0)) adjustFirstFixed = TRUE;
      }
      ...
      if (headerSizePtr) *headerSizePtr = fixedOffset;

      // Set offsets for all the fixed fields ...
      while (fixedFields->entries()) {
        fixedFields->getFirst(fieldIdx);
        computeOffsetOfFixedField(attrs, fieldIdx, bitmapOffset, fixedOffset, firstField, prevIdx, nullBitIdx, hdrInfo,
                                  alignedFormat);
      }
      ...
      while (guFields.entries()) {
        ...
        computeOffsetOfFixedField(attrs, fieldIdx, bitmapOffset, fixedOffset, firstField, prevIdx, nullBitIdx);
      }
      ...
      if (fixedFields->entries() && ((*rtnFlags & ADDED_COLUMN) > 0)) *rtnFlags |= ExpTupleDesc::ADDED_FIXED_PRESENT;
      ...
      for (int k = 0; k < (int)num_attrs; k++) {
        ...
        fprintf(stderr,
                ...
                k, attr->getDatatype(), attr->getNullFlag(), (attr->getVCIndicatorLength() > 0 ? 1 : 0),
                attr->getOffset(), attr->getVoaOffset(), attr->getDataAlignmentSize());
      }
#endif
    } break;
...
UInt32 ExpTupleDesc::sqlarkExplodedOffsets(UInt32 offset, UInt32 length, Int16 dataType, NABoolean isNullable,
                                           UInt32 *nullIndicatorOffset, UInt32 *vcIndicatorOffset,
                                           UInt32 vcIndicatorSize) {
  if (isNullable) {
    offset = ADJUST(offset, ExpTupleDesc::NULL_INDICATOR_LENGTH);
    if (nullIndicatorOffset) *nullIndicatorOffset = offset;
    offset += ExpTupleDesc::NULL_INDICATOR_LENGTH;
  }

  if (DFS2REC::isAnyVarChar(dataType)) {
    offset = ADJUST(offset, vcIndicatorSize);
    if (vcIndicatorOffset) *vcIndicatorOffset = offset;
    offset += vcIndicatorSize;
  }

  // column value alignment
  if (DFS2REC::isNumeric(dataType)) offset = ADJUST(offset, length);
...
Int16 ExpTupleDesc::operator==(ExpTupleDesc *other) {
  if ((numAttrs_ != other->numAttrs_) || (tupleDataFormat_ != other->tupleDataFormat_)) return 0;

  for (UInt32 i = 0; i < numAttrs_; i++) {
    if (!(*getAttr(i) == *other->getAttr(i))) return 0;
  }
