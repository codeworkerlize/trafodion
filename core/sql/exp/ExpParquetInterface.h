...
#ifndef EXP_PARQUET_INTERFACE_H
#define EXP_PARQUET_INTERFACE_H
...
class ExpParquetInterface : public ExpExtStorageInterface
{
 public:
  static ExpParquetInterface* newInstance(CollHeap* heap, 
                                          const char* server = NULL, 
                                          const Lng32 port = -1);
  
  ExpParquetInterface(CollHeap* heap, 
                      const char* server, 
                      const Lng32 port);
  ...
  // filePath:   location and name of parquet file
  ...
  // parquetRow:   pointer to buffer where Parquet will return the row.
 ...
 //
  ...
  Lng32 getFileAndBlockAttributes(char * rootDir, 
                                  NAArray<HbaseStr> ** retFileStats);
  ...
  Lng32 getSplitUnitInfo(char* path,
