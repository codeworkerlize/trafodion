...
const char * getOperTypeEnumAsString(Int16 /*OperatorTypeEnum*/ ote)
{
  switch (ote)
    ...
    case ITM_CURRENT_TENANT: return "ITM_CURRENT_TENANT";
    case ITM_CURRENT_CATALOG: return "ITM_CURRENT_CATALOG";
    case ITM_CURRENT_SCHEMA: return "ITM_CURRENT_SCHEMA";
    ...
    case ITM_RUNNING_PIVOT_GROUP : return "ITM_RUNNING_PIVOT_GROUP";
    ...
    case ITM_REGEXP_REPLACE: return "ITM_REGEXP_REPLACE";
    case ITM_REGEXP_SUBSTR: return "ITM_REGEXP_SUBSTR";
    case ITM_REGEXP_COUNT: return "ITM_REGEXP_COUNT";
    ...
    case ITM_NUMBERFORMAT: return "ITM_NUMBERFORMAT";
    ...
    case ITM_HBASE_ROWID: return "ITM_HBASE_ROWID";

    case ITM_SEQUENCE_VALUE: return "ITM_SEQUENCE_VALUE";
    case ITM_RANGE_VALUES_MERGE: return "ITM_RANGE_VALUES_MERGE";
    case ITM_RANGE_VALUES_INSERT: return "ITM_RANGE_VALUES_INSERT";
    case ITM_RANGE_VALUES_IN: return "ITM_RANGE_VALUES_IN";
    case ITM_RANGE_VALUES_COPY: return "ITM_RANGE_VALUES_COPY";
    case ITM_RANGE_VALUES_UNPACK: return "ITM_RANGE_VALUES_UNPACK";

    case ITM_COMPOSITE_CONCAT:      return "COMPOSITE_CONCAT";
    case ITM_COMPOSITE_CREATE:      return "COMPOSITE_CREATE";
    case ITM_COMPOSITE_DISPLAY:     return "COMPOSITE_DISPLAY";
    case ITM_COMPOSITE_CAST:        return "COMPOSITE_CAST";
    case ITM_COMPOSITE_EXTRACT:     return "COMPOSITE_EXTRACT";
    case ITM_COMPOSITE_ARRAY_LENGTH:return "COMPOSITE_ARRAY_LENGTH";
    case ITM_COMPOSITE_ARRAY_CAST:  return "COMPOSITE_ARRAY_CAST";
    case ITM_COMPOSITE_HIVE_CAST:   return "COMPOSITE_HIVE_CAST";

    case ITM_ENCODE_BASE64: return "ITM_ENCODE_BASE64";
    case ITM_DECODE_BASE64: return "ITM_DECODE_BASE64";
    case ITM_BEGINKEY:      return "ITM_BEGINKEY";
    case ITM_ENDKEY:       return "ITM_ENDKEY";
                                ...
                                const char * instrText)
{
  ...
  if (displayStr)
    {
      if (compExprNum > 0)
        str_sprintf(buf, "  Clause L%d:#%d  %s", compExprNum, 
                    clauseNum, displayStr); 
      else
...
void ExFunctionHbaseRowid::displayContents(Space * space, const char * /*displayStr*/, Int32 clauseNum, char * constsArea)
{
  char buf[100];
                                          ...
                                          NABoolean ignoreSpace)
{
  ...
  unsigned char * s2 = (unsigned char *)in_s2;
   ...
   if (isTrailingBlankSensitive && atoi(isTrailingBlankSensitive) == 1 && !ignoreSpace)
    strictCmp = true;
...
void
ExFunctionRangeOfValues::displayContents(Space * space, const char * displayStr,
                                  Int32 clauseNum, char * constsArea)
{
...
ExFunctionRangeOfValues::ExFunctionRangeOfValues(OperatorTypeEnum oper_type,
                      Attributes ** attr, Space * space, 
                      UInt32 cap, UInt32 numChildren, Int16 filterId,
                      NABoolean dumpDebugData, NABoolean dateAsTimestamp,
                      NABoolean nativeByteOrder,
                      Int32 maxEntries
                      )
...
Int16 convertDateTimeTimestampToAscii(char* target, Lng32 targetLen, 
                                      char* source, Lng32 sourceLen, 
                                      Int16 precision, Int16 scale, Int32 format,
                                      CollHeap *heap)
{
   ...
   Lng32 realTargetLen = 
     srcDatetimeOpType.convDatetimeToASCII(source,  // source
                                           target,  // target
...
void ExFunctionRangeOfValues::displayOpData(fstream& out, const char* msg, char * op_data[], CollHeap *heap)
{
   ...
   if ( DFS2REC::isDateTime(dt) ) {
     ...
     switch(sourceAttr->getPrecision())
     {
      case REC_DATETIME_CODE::REC_DTCODE_DATE:
       {
         Lng32 realTargetLen = convertDateTimeTimestampToAscii(target, sizeof(target),
                                  source, sourceAttr->getLength(), 
                                  REC_DTCODE_DATE, sourceAttr->getScale(),
      ...
      case REC_DATETIME_CODE::REC_DTCODE_TIME:
       {
         Lng32 realTargetLen = convertDateTimeTimestampToAscii(target, sizeof(target),
                                  source, sourceAttr->getLength(), 
                                  REC_DTCODE_TIME, sourceAttr->getScale(),
                                  ExpDatetime::DATETIME_FORMAT_DEFAULT, heap
                                  );
   ...
   if ( DFS2REC::isDecimal(dt) ) {
     if (sourceAttr->getScale() == 0 )
     {
        ...
        Int64 value = 0;
        if ( convDecToInt64(value,
                            source,
                            sourceAttr->getLength(),
                            heap,
...
NABoolean 
ExFunctionRangeOfValues::insertData(char * op_data[], CollHeap *heap)
{
   if ( dumpDebugData_ ) 
      displayOpData("insert", op_data, heap);
...
NABoolean 
ExFunctionRangeOfValues::lookupData(char * op_data[], CollHeap *heap)
{
   if ( dumpDebugData_ ) 
      displayOpData("lookupInExp", op_data, heap);
   ...
   if ( DFS2REC::isDateTime(dt) ) {
     ...
     switch(sourceAttr->getPrecision())
     {
      case REC_DATETIME_CODE::REC_DTCODE_DATE:
       {
         Lng32 realTargetLen = 
             convertDateTimeTimestampToAscii(
...
ex_expr::exp_return_type ExFunctionRangeOfValues::eval(char * op_data[],
                                                CollHeap *heap,
                                                ComDiagsArea **diagsArea)
{
  char * source = (char *)op_data[1];
    ...
    case OperatorTypeEnum::ITM_BITMAP_COPY:
     {
       ...
       if ( sourceLen != targetLen ) {
          ExRaiseFunctionSqlError(heap, diagsArea, 
                                  EXE_RANGE_VALUES_COPY_LENGTHS_DIFF);
          return ex_expr::EXPR_ERROR;
       }
...
void 
ExFunctionRangeOfValues::packRanges(char* target, 
                                    char* vcLenPtr, Lng32 vcLenSize,
                                    NABoolean clearWhenDone,
                                    ScanFilterStatsList* sfStats,
                                    ...
                                    )
{
   if ( rangeOfValues_->isEnabled() ) {
      ...
      if ( rangeOfValues_->totalSizeCap() < packLen || 
           !(rangeOfValues_->isUseful(maxNumEntries_)) ) 
      {
...
ExFunctionRangeOfValues::conditionalPack(ex_clause* clause, 
                                         atp_struct* atp,
                                         OperatorTypeEnum op, 
                                         const char* msg,
                                         NABoolean clearWhenDone,
                                         ...
                                         )
{
   ...
   while (clause) {
       ...
       if ( clause->getOperType() == op )
       {
          ...
          ((ExFunctionRangeOfValues*)clause)->
               packRanges(target, vardata, vcLenSize, clearWhenDone, 
                          sfStats, semantics);
...
ex_expr::exp_return_type 
ExFunctionRangeOfValues::conditionalSanityCheck(ex_clause* clause, OperatorTypeEnum op)
{
   Attributes* attr = NULL;
   while (clause) {

       if ( clause->getOperType() == op &&
            ExFunctionRangeOfValues::IsRangeOfValuesEnum(op) )
       {
