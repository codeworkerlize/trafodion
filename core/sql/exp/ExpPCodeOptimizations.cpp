...
void PCodeCfg::printConstants() {
  ...
  if (constToOffsetMap_) {
    ...
    for (i = 0; i < iter.entries(); i++) {
      ...
      sprintf(NExBuf, "data: %p, len: %d, off: %d   ", key->getData(), key->getLen(), *value);
      NExLog(NExBuf);
...
void PCodeCfg::printBlocks(const char *header, NABoolean fullDebug) {
  ...
  for (CollIndex i = 0; i < allBlocks_->entries(); i++) {
    if (allBlocks_->at(i)->print(fullDebug)) NExLog("\n");
  }
  NExLog("\n");
}
...
NABoolean PCodeBlock::print(NABoolean fullDebug) {
...
#if 1
  if ((first_ == NULL) && (last_ == NULL) && (predsList_.entries() == 0) && (succsList_.entries() == 0)) return FALSE;
#endif
  ...
  for (tempInst = first_; tempInst && (tempInst != last_); tempInst = tempInst->next) {
    tempInst->print(cfg_, fullDebug);
  }

  if (last_) last_->print(cfg_, fullDebug);
...
NABoolean PCodeOperand::canOverlap(PCodeOperand *op2) {
  ...
  if (getBvIndex() == op2->getBvIndex()) return TRUE;

  if (getStackIndex() != op2->getStackIndex()) return FALSE;
  ...
  if (((r1x >= r2x) && (r1x < r2y)) || ((r2x >= r1x) && (r2x < r1y))) return TRUE;
...
NABoolean PCodeConstants::isAnyKnownConstant(CollIndex bvIndex, NABitVector &zeroes, NABitVector &ones,
                                             NABitVector &neg1) {
...
NABoolean PCodeConstants::canBeOne(CollIndex bvIndex, NABitVector &ones) { return (ones.testBit(bvIndex)); }

NABoolean PCodeConstants::canBeZero(CollIndex bvIndex, NABitVector &zeroes) { return (zeroes.testBit(bvIndex)); }

NABoolean PCodeConstants::canBeNeg1(CollIndex bvIndex, NABitVector &neg1) { return (neg1.testBit(bvIndex)); }
...
int PCodeConstants::copyConstantVectors(CollIndex bvIndexFrom, CollIndex bvIndexTo, NABitVector &zeroes,
  ...
  if ((isOne = ones.testBit(bvIndexFrom))) ones += bvIndexTo;

  if ((isNeg1 = neg1.testBit(bvIndexFrom))) neg1 += bvIndexTo;
...
//
void PCodeConstants::mergeConstantVectors(NABitVector &newZeroes, NABitVector &newOnes, NABitVector &newNeg1,
                                          NABitVector &zeroes, NABitVector &ones, NABitVector &neg1) {
  CollIndex i;
  ...
  // are set or not.
  CollIndex maxIndex =
      ((temp.getLastStaleBit() > temp2.getLastStaleBit()) ? temp.getLastStaleBit() : temp2.getLastStaleBit());

  // Loop through all elements that are included in the sets.
  for (i = 0; i <= maxIndex; i++) {
    if (!temp.testBit(i)) {
      if (temp2.testBit(i)) clearConstantVectors(i, zeroes, ones, neg1);
    } else {
      if (!temp2.testBit(i)) clearConstantVectors(i, newZeroes, newOnes, newNeg1);
    }
  }
...
PCodeInst *ReachDefsTable::find(CollIndex bv, NABoolean isInTab) {
  ...
  while (node) {
    if (node->bvIndex_ == bv) return node->inst_;
...
//
PCodeInst *ReachDefsTable::find(CollIndex bv) {
  PCodeInst *ret = find(bv, FALSE /* Out */);
  if (!ret) ret = find(bv, TRUE /* In */);
...
NABoolean PCodeBlock::isEntryBlock() { return (cfg_->getEntryBlock() == this); }

void PCodeBlock::setToEntryBlock() { cfg_->setEntryBlock(this); }

PCodeInst *PCodeBlock::insertNewInstAfter(PCodeInst *inst, PCIT::Instruction instr) {
...
PCodeInst *PCodeBlock::insertInstAfter(PCodeInst *inst, PCodeInst *newInst) {
  ...
  if (inst->isOpdata()) {
    while ((inst != last_) && (inst->getOpcode() != PCIT::CLAUSE_EVAL)) inst = inst->next;
  }
...
PCodeInst *PCodeBlock::insertNewInstBefore(PCodeInst *inst, PCIT::Instruction instr) {
  PCodeInst *newInst = cfg_->createInst(instr);
  return insertInstBefore(inst, newInst);
}

PCodeInst *PCodeBlock::insertInstBefore(PCodeInst *inst, PCodeInst *newInst) {
  PCodeInst *temp;
  ...
  if (inst->isOpdata() || inst->isClauseEval()) {
    while (inst->prev && inst->prev->isOpdata()) inst = inst->prev;
  }
...
void PCodeBlock::deleteInst(PCodeInst *inst) {
  ...
  if (inst->isClauseEval()) {
    while (inst->prev && inst->prev->isOpdata()) deleteInst(inst->prev);
  }
...
// block.
//
void PCodeBlock::fixupConstantVectors(PCodeBlock *targetBlock, NABitVector &newZeroes, NABitVector &newOnes,
                                      NABitVector &newNeg1) {
  // Don't process this block if it doesn't meet the requirements.
  if (isEmpty() || !last_->isBranch() || last_->isUncBranch()) return;
  ...
  switch (opc) {
    ...
    case PCIT::NOT_NULL_BRANCH_BULK: {
      ...
      if (getTargetBlock() == targetBlock) {
        ...
        if (index != -1) {
          ...
          PCodeConstants::setConstantInVectors(-1, srcIdx, newZeroes, newOnes, newNeg1);
        }
      }
      break;
    }
    ...
    case PCIT::NOT_NULL_BRANCH_MBIN16S_MBIN16S_MBIN16S_IBIN32S: {
      ...
      if (getTargetBlock() == targetBlock) {
        ...
        if ((PCIT::NNB_SPECIAL_NULLS_MBIN32S_MATTR3_MATTR3_IBIN32S_IBIN32S == opc)) break;
        ...
        if ((opc != PCIT::NOT_NULL_BRANCH_MBIN32S_MBIN16S_MBIN16S_IBIN32S) &&
            ...
            (opc != PCIT::NOT_NULL_BRANCH_MBIN32S_MBIN32S_MBIN32S_IATTR4_IBIN32S) &&
            (opc != PCIT::NOT_NULL_BRANCH_COMP_MBIN32S_MBIN32S_MBIN32S_IATTR4_IBIN32S))
    ...
    FOREACH_INST_IN_BLOCK_BACKWARDS_AT(this, inst, last_->prev) {
      if (inst->isMove()) {
        ...
        if ((moveSrcIdx == propIdx) && (!tempBv.contains(moveWriteIdx))) {
          PCodeConstants::clearConstantVectors(moveWriteIdx, newZeroes, newOnes, newNeg1);
          PCodeConstants::copyConstantVectors(propIdx, moveWriteIdx, newZeroes, newOnes, newNeg1);
        } else if ((moveWriteIdx == propIdx) && (!tempBv.contains(moveSrcIdx))) {
          PCodeConstants::clearConstantVectors(moveSrcIdx, newZeroes, newOnes, newNeg1);
          PCodeConstants::copyConstantVectors(propIdx, moveSrcIdx, newZeroes, newOnes, newNeg1);

          // We have a new propagation index to follow
          propIdx = moveSrcIdx;
        }
      } else if ((inst->getOpcode() == PCIT::NULL_TEST_MBIN32S_MATTR5_IBIN32S_IBIN32S) &&
        ...
        if ((constant == 1) && (inst->code[9] != 0))
          ...
          PCodeConstants::setConstantInVectors(0, readIdx, newZeroes, newOnes, newNeg1);
      ...
      for (i = 0; i < inst->getWOps().entries(); i++) tempBv += inst->getWOps()[i]->getBvIndex();

      // If we've already now seen the def of the prop idx, break out
      if (tempBv.contains(propIdx)) break;
    }
...
NABoolean PCodeInst::containsIndirectVarcharOperand() {
  ...
  for (i = 0; i < readOps.entries(); i++)
    if (readOps[i]->getOffset() < 0) return TRUE;

  for (i = 0; i < writeOps.entries(); i++)
    if (writeOps[i]->getOffset() < 0) return TRUE;
...
//
void PCodeInst::getBranchTargetOffsetPointers(NAList<PCodeBinary *> *targetOffPtrs, PCodeCfg *cfg) {
...
//
void PCodeInst::getBranchTargetPointers(NAList<Long *> *targetOffPtrs, PCodeCfg *cfg) {
  ...
  assert(prev && prev->isSwitch() && !prev->isInListSwitch());
...
PCodeInst::~PCodeInst() { destroy(); }

void PCodeInst::destroy() { liveVector_.clear(); }
...
#if defined(_DEBUG)
  if (getenv("PCODE_LLO_DEBUG")) debug = 1;
...
void PCodeCfg::optimize() {
  ...
  if (getenv("NO_PCODE_LLO")) enableOpt = 0;
  ...
  if (!canPCodeBeOptimized(pCode, pcodeExprIsCacheable, savedUnOptPCodeLen) || !enableOpt) {
#if defined(_DEBUG)
    if (debugSome) NExLog("FOUND: PCODE EXPRESSION NOT EVEN OPTIMIZABLE\n");
#endif  // defined(_DEBUG)
    return;
  }
  ...
  if (optFlags_ & OPT_PCODE_CACHE_DISABLED) pcodeExprIsCacheable = FALSE;
  ...
  if (pcodeExprIsCacheable) {
    ...
    if (cachedPCE = CURROPTPCODECACHE->findPCodeExprInCache(pCode, expr_->getConstantsArea(), optFlags_ & NATIVE_EXPR,
                                                            savedUnOptPCodeLen, origConstantsLen
#if OPT_PCC_DEBUG == 1
                                                            ,
      ...
      cachedNEConstsLen = (optFlags_ & NATIVE_EXPR) ? cachedPCE->getNEConstsLen() : cachedNewConstsLen;
    ...
    if (usingCachedPCodeExpr) {
      ...
      if (debugAll) {
        char NExBuf[200];
        sprintf(NExBuf,
                "PCODE EXPR FOUND in cache: ThisPtr=%p: Loc=%p, oldLen=%d, newLen=%d, #Lookups=%ld, #Hits=%ld\n",
                CURROPTPCODECACHE->getThisPtr(), cachedPCE, savedUnOptPCodeLen, cachedPCodeLen,
                CURROPTPCODECACHE->getNumLookups(), CURROPTPCODECACHE->getNumHits());
        NExLog(NExBuf);
      }
#endif  // OPT_PCC_DEBUG==1
      if (!space_) {
      ...
      } else  // Otherwise, we allocate space to hold the optimized code & constants
      {
        ...
        if ((optFlags_ & NATIVE_EXPR) && (cachedNewConstsLen < cachedNEConstsLen)) {
...
#if OPT_PCC_DEBUG == 1
      if (debugSome) NExLog("PCODE EXPRESSION NOT FOUND IN CACHE\n");
#endif  // OPT_PCC_DEBUG==1
  ...
  if (usingCachedPCodeExpr) goto considerNativeCodeGen;
  ...
  if (pcodeExprIsCacheable && CURROPTPCODECACHE->getPCECLoggingEnabled()) (void)getrusage(RUSAGE_THREAD, &begOpt);
  ...
  if (optFlag && addOverlappingOperands(TRUE)) overlapFound = TRUE;

  // Perform short circuiting
  if (optFlag && shortCircuitOpt(overlapFound)) cfgRewiring(rewiringFlags);
  ...
  if (optFlag && (optFlags_ & INDIRECT_BRANCH) && counters_.logicalBranchCnt_) {
    ...
    if (rewire) cfgRewiring(rewiringFlags);
  ...
  for (i = 0; optFlag && (i < 3); i++) {
    ...
    if (optFlag) restart = constantPropagation(TRUE /* Peeling */);
    ...
    if (optFlags_ & CSE) {
      ...
      if (i == 0) removeReachingDefs();
    }

    if (restart) DUMP_PHASE("Aggressive [9]", debugSome, debugAll);
  ...
  if (optFlag) {
    ...
    if (optFlag && rewire) cfgRewiring(rewiringFlags);
  }
  ...
  if (optFlags_ & REORDER_PREDICATES) {
    if ((counters_.logicalBranchCnt_) && (expr_->getType() == ex_expr::exp_SCAN_PRED)) {
      computeReachingDefs(0);
  ...
  if (optFlags_ & INLINING) inlining();
...
considerNativeCodeGen:
  ...
  if (!usingCachedPCodeExpr) {
    ...
    if (optFlags_ & NATIVE_EXPR) {
      ...
      if (CURROPTPCODECACHE->getPCECLoggingEnabled()) (void)getrusage(RUSAGE_THREAD, &begNEtime);
#endif  // OPT_PCC_DEBUG==1
  ...
  if (!usingCachedPCodeExpr) {
    ...
    if (pcodeExprIsCacheable) {
      ...
      if (CURROPTPCODECACHE->getPCECLoggingEnabled()) (void)getrusage(RUSAGE_THREAD, &begAdd);
      ...
      if (foundCachedPCodeExpr)  // TRUE if we are in Compare-Only mode
      {
        ...
        if (cachedPCodeLen != newPCodeLen)
          fatals |= 0x2;
        else if (memcmp(optimizedPCode, newPCode, sizeof(PCodeBinary) * newPCodeLen) != 0)
          fatals |= 0x4;

        if (cachedNEConstsLen != expr_->getConstsLength())
          fatals |= 0x8;
        else if (memcmp(cachedConstsArea, expr_->getConstantsArea(), cachedNEConstsLen) != 0)
          fatals |= 0x10;
        assert(fatals == 0);
      ...
      } else  // It wasn't already in cache
      {
        ...
        PCECacheEntry *newPCE =
            new (cHeap) PCECacheEntry(cHeap, savedUnOptPCodePtr, newPCode, expr_->getConstantsArea(),
                                      savedUnOptPCodeLen, newPCodeLen, oldConstsAreaLen  // Before PC optimization
                                      ,
...
#if OPT_PCC_DEBUG == 1
                                      ,
                                      totalOptTime, totalNEgenTime, CURROPTPCODECACHE->genNewUniqCtrVal()
#endif  // OPT_PCC_DEBUG==1
            );
...
#if OPT_PCC_DEBUG == 1
                                        ,
                                        totalNEgenTime, begAdd.ru_utime, NExDbgInfoPtr_->getNExStmtSrc()
#endif  // OPT_PCC_DEBUG==1
        );
        ...
        if (debugAll) {
          ...
          sprintf(
              NExBuf,
              "CACHED PCODE EXPR: ThisPtr=%p: Loc=%p oldLen=%d, newLen=%d, numExprCached=%d, curSiz=%d, maxSiz=%d\n",
              CURROPTPCODECACHE->getThisPtr(), newPCE, savedUnOptPCodeLen, newPCodeLen,
              CURROPTPCODECACHE->getNumEntries(), CURROPTPCODECACHE->getCurrSize(), CURROPTPCODECACHE->getMaxSize());
          NExLog(NExBuf);
        }
#endif  // OPT_PCC_DEBUG==1
      }
...
void PCodeCfg::generateShowPlan(PCodeBinary *pCode, Space *space) {
  ...
  FOREACH_BLOCK(block, firstInst, lastInst, index) {
    ...
    if (block->getFirstInst() && block->getFirstInst()->getOpcode() == PCIT::END) continue;
    ...
    for (i = 0; i < block->getPreds().entries(); i++) {
      ...
      if (pred->branchesTo(block)) {
        if (!found) str_cat(buf, "(Preds: ", buf);
    ...
    FOREACH_INST_IN_BLOCK(block, inst) {
      ...
      if (inst->getOpcode() == PCIT::END) continue;
...
NABoolean PCodeCfg::updateConstVectors(PCodeOperand *operand) {
  ...
  if (operand->isEmptyVarchar()) return TRUE;
...
void PCodeCfg::initConstants() {
  ...
  constToOffsetMap_ = new (heap_) NAHashDictionary<PCodeConstants, CollIndex>(&constHashFunc, 10, TRUE, heap_);

  offsetToConstMap_ = new (heap_) NAHashDictionary<CollIndex, PCodeConstants>(&collIndexHashFunc, 10, TRUE, heap_);
  ...
  for (i = 0; i < allInsts_->entries(); i++) {
    ...
    for (j = 0; j < inst->getROps().entries(); j++) {
      ...
      if (offsetPtr != NULL) {
        ...
        if (op->isVarchar()) off = off + op->getVcIndicatorLen() + op->getVcNullIndicatorLen();
  ...
  for (i = 0; i <= maxBVIndex_; i++) {
    ...
    if (oldValue == PCodeConstants::UNKNOWN_CONSTANT) PCodeConstants::clearConstantVectors(i, *zeroes_, *ones_, *neg1_);
  }
}
...
CollIndex *PCodeCfg::addConstant(PCodeOperand *op) {
  ...
  if (constPtr) {
    ...
    if (constPtr->getLen() >= len) return NULL;
...
void PCodeCfg::getBlocksInDFORecursive(BLOCKARRAY &list, PCodeBlock *block) {
  ...
  for (i = 0; i < block->getSuccs().entries(); i++) {
    ...
    if (!succ->getVisitedFlag()) getBlocksInDFORecursive(list, succ);
  }
...
void PCodeCfg::computeLiveness(NABoolean performDCE) {
  ...
  FOREACH_BLOCK_FAST(block, firstInst, lastInst, inst) { block->getLiveVector().clear(); }
  ENDFE_BLOCK_FAST
  ...
  do {
    ...
    FOREACH_BLOCK_DFO(block, firstInst, lastInst, index) {
      ...
      for (j = 0; j < block->getSuccs().entries(); j++) tempBv += block->getSuccs()[j]->getLiveVector();
          ...
          if (inst->isClauseEval()) {
            ...
            for (; prev && prev->isOpdata(); prev = prev->prev) {
              for (i = 0; i < prev->getWOps().entries(); i++) {
                ...
                if (!(op->isTemp() || op->isGarbage()) || tempBv.testBit(op->getBvIndex())) {
                  isLive = TRUE;
                  break;
                }
                writeFound = TRUE;
            ...
            for (i = 0; i < inst->getWOps().entries(); i++) {
              ...
              if (!(op->isTemp() || op->isGarbage()) || tempBv.testBit(op->getBvIndex()) || op->isEmptyVarchar()) {
                isLive = TRUE;
                break;
              }
            }
            ...
            if (!isLive)
              if (removeOrRewriteDeadCodeInstruction(inst)) continue;
          }
        }
...
//
void PCodeCfg::updateReachingDefs(PCodeBlock *block, CollIndex bvIndex, PCodeInst *oldInst, PCodeInst *newInst) {
  CollIndex i = 0;

  // Reaching defs must be available when this routine is called.
  if (rDefsHeap_ == NULL) return;
  ...
  FOREACH_BLOCK_REV_DFO(block, firstInst, lastInst, index) {
    ...
    if (block->getPreds().entries() > 0) {
      ...
      *(rDefTab->getRDefVec()) = *(pred->reachingDefsTab_->getRDefVec());
      ...
      for (i = 1; i < block->getPreds().entries(); i++) {
        ...
        for (j = reachBits->getLastStaleBit(); (j = reachBits->prevUsed(j)) != NULL_COLL_INDEX; j--) {
          PCodeInst *prevInst;
          ...
          if (!prevInst) prevInst = predRDefTab->find(j, TRUE /* In */);
          ...
          if ((prevInst != NULL) && (prevInst == rDefTab->find(j, TRUE))) continue;
    ...
    FOREACH_INST_IN_BLOCK(block, inst) {
      for (i = 0; i < inst->getWOps().entries(); i++) {
        ...
        if (noTemps && value->isTemp()) continue;
...
void PCodeCfg::createPhysListRecurse(PCodeBlock *block, BLOCKLIST &list) {
  ...
  if (block->getVisitedFlag() || block->isEmpty()) return;
  ...
  for (i = 0; i < block->getPreds().entries(); i++) {
    PCodeBlock *pred = block->getPreds()[i];
    if (pred->getVisitedFlag() || (pred->getFallThroughBlock() != block)) continue;
    ...
    if (lastInst && lastInst->isUncBranch()) continue;
  ...
  if ((block->getPreds().entries() != 0) || (block == entryBlock_)) list.insert(block);
  ...
  for (CollIndex i = 0; i < block->getSuccs().entries(); i++) {
    ...
    if (((i == 0) && !lastInst->isUncBranch()) || (block->getSuccs()[i]->getPreds().entries() == 1)) {
...
void PCodeCfg::createPhysList(BLOCKLIST &list) {
  ...
  FOREACH_BLOCK(block, firstInst, lastInst, index) { createPhysListRecurse(block, list); }
  ENDFE_BLOCK

  // As a sanity check, make sure all blocks have been visited and inserted
  FOREACH_BLOCK(block, firstInst, lastInst, index) {
    if (!block->isEmpty()) assert(block->getVisitedFlag());
  }
  ENDFE_BLOCK
  ...
  for (CollIndex i = 1; i < list.entries(); i++) {
    ...
    if (prevLastInst->isUncBranch() && prevBlock->getTargetBlock() == list[i]) prevBlock->deleteInst(prevLastInst);
  }
}
    ...
    if (inst && inst->isBranch()) {
      ...
      for (j = 0; j < targetOffsetPtrs.entries(); j++) {
        ...
        if (succ->getBlockTopOffset() < block->getBlockTopOffset()) {
          ...
          // target block.
          targetOffset = succ->getBlockTopOffset() - block->getBlockBottomOffset() - 1;
        } else {
          // Branching forwards
          ...
          targetOffset = succ->getBlockTopOffset() -
                         (nextBlock->getBlockTopOffset() - PCode::getInstructionLength(inst->code)) - 1;
        }
      ...
      for (j = 0; j < targetPtrs.entries(); j++) {
        ...
        PCodeBlock *succ = (inst->isBranch() ? block->getTargetBlock() : block->getSuccs()[j]);
...
NABoolean PCodeCfg::layoutCode() {
  ...
  for (i = 0; i < physBlockList.entries(); i++) {
    ...
    FOREACH_INST_IN_BLOCK(block, inst) { totalLength += PCode::getInstructionLength(inst->code); }
    ENDFE_INST_IN_BLOCK
  }
...
void PCodeCfg::inlining() {
  ...
  if (entryBlock_->getSuccs().entries() != 0) return;

  // There must be at least 1 tuple, but no more than 2.
  pcode = expr_->getPCodeBinary();
  if ((pcode[0] == 0) || (pcode[0] > 2)) return;
  ...
  for (i = 0; i < inst->getROps().entries(); i++)
    if ((inst->getROps()[i]->getStackIndex() < 4) && (inst->getROps()[i]->getStackIndex() != 1)) return;
  ...
  switch (inst->getOpcode()) {
    ...
    case PCIT::NULL_TEST_MBIN32S_MATTR5_IBIN32S_IBIN32S: {
      ...
      if (inst && inst->getOpcode() == PCIT::MOVE_MBIN32S) {
        ...
        if ((bvIndex != inst->getROps()[0]->getBvIndex()) || (!inst->getROps()[0]->isTemp())) {
          assert(FALSE);
          break;
        }
    ...
    case PCIT::MOVE_MBIN64S_MBIN64S: {
      ...
      if (inst->getWOps()[0]->getStackIndex() < 4) return;
...
PCodeInst *PCodeCfg::extendPaddedNullIndMoves(PCodeInst *moveInst) {
  ...
  if (moveInst->getOpcode() == PCIT::MOVE_MBIN16U_MBIN16U) {
    ...
    if (moveInst->getROps()[0]->isVar() && moveInst->getWOps()[0]->isVar()) {
      ...
      NullTriple src(atpMap_[srcIndex], atpIndexMap_[srcIndex], moveInst->getROps()[0]->getOffset());
        ...
        if (*srcPad == *dstPad) {
          switch (*srcPad) {
            ...
            default: {
              ...
              PCodeInst *newInst = block->insertNewInstAfter(moveInst, PCIT::MOVE_MBIN8_MBIN8_IBIN32S);
...
void PCodeCfg::flattenNullBranches() {
  ...
  FOREACH_BLOCK(nullBlock, firstInst, branchInst, index) {
    ...
    if (!branchInst || !branchInst->isNullBranch() ||
        (branchInst->getOpcode() != PCIT::NOT_NULL_BRANCH_MBIN32S_MBIN32S_IATTR3_IBIN32S) ||
        branchInst->getWOps()[0]->forAlignedFormat() || branchInst->getROps()[0]->forAlignedFormat())
      continue;
    ...
    // Verify pattern
    if ((ftBlock->getSuccs().entries() != 1) || (tgtBlock->getSuccs().entries() != 1) ||
        (ftBlock->getSuccs()[0] != tgtBlock->getSuccs()[0]))
      continue;

    // Check fill mem instruction
    fillInst = ftBlock->getFirstInst();
    if (!fillInst || (fillInst->next != ftBlock->getLastInst()) || (fillInst->getOpcode() != PCIT::FILL_MEM_BYTES) ||
        !fillInst->next->isUncBranch())
      continue;

    // Check move instruction
    moveInst = tgtBlock->getFirstInst();
    if (!moveInst->isMove()) moveInst = tgtBlock->getLastInst();

    if (!moveInst || !moveInst->isMove()) continue;
    ...
    if (tgtBlock->getFirstInst()->mayFailIfNull()) continue;

    // Make sure the two match
    if (moveInst->getWOps()[0]->getBvIndex() != fillInst->getWOps()[0]->getBvIndex()) continue;
    ...
    if (moveInst->getOpcode() == PCIT::MOVE_MBIGS_MBIGS_IBIN32S_IBIN32S) {
      if (moveInst->code[5] != moveInst->code[6]) continue;
    } else if (moveInst->getWOps()[0]->getType() == PCIT::MBIGS) {
      if (!moveInst->getWOps()[0]->isTemp()) continue;
    }
...
PCodeInst *PCodeBlock::findDef(PCodeInst *inst, PCodeOperand *operand, NABoolean globalSearch) {
  CollIndex i;
  assert(inst != NULL);

  // Constant operands are always (and only) defined upon entry to the routine
  if (operand->isConst()) return (PCodeInst *)-1;
  ...
  FOREACH_INST_IN_BLOCK_BACKWARDS_AT(inst->block, def, inst->prev) {
    for (i = 0; i < def->getWOps().entries(); i++)
      if (def->getWOps()[i]->getBvIndex() == bvIndex) return def;
  }
  ENDFE_INST_IN_BLOCK_BACKWARDS_AT
  ...
  if (head->isClauseEval()) {
    ...
    while (prevHead && prevHead->isOpdata()) {
      if (prevHead->getROps().entries() != 0) {
        ...
        if ((defH == NULL) || (defT == NULL) || (defH != defT)) return FALSE;
      }
...
NABoolean PCodeCfg::doesInstQualifyForCSE(PCodeInst *inst) {
  ...
  if (!inst->isClauseEval() && ((inst->getWOps().entries() != 1) || (inst->getROps().entries() == 0))) return FALSE;

  // Moves with constant read operands are better off being handled by copy
  // propagation and constant folding.
  if (inst->isMove() && inst->getROps()[0]->isConst()) return FALSE;
...
void PCodeCfg::setupForCSE(INSTLIST **cseList) {
  ...
  for (i = 0; i < MAX_NUM_PCODE_INSTS; i++) cseList[i] = new (heap_) INSTLIST(heap_);

  FOREACH_BLOCK_REV_DFO(block, firstInst, lastInst,
                        index){FOREACH_INST_IN_BLOCK(block, inst){if (!doesInstQualifyForCSE(inst)) continue;
...
NABoolean PCodeCfg::localCSE(INSTLIST **parent, PCodeBlock *tailBlock, NABoolean *rDefsComputed) {
  CollIndex i, j;
    ...
    for (i = 0; i < parent[tail->getOpcode()]->entries(); i++) {
      ...
      if (head->isClauseEval()) {
        ...
        switch (clauseHead->getClassID()) {
          case ex_clause::CONV_TYPE:
            match = ((((ex_conv_clause *)clauseHead)->getInstruction() ==
            ...
            if (match) {
              ...
              if ((instr == CONV_DATETIME_DATETIME) ||
                ...
                match =
                    ((srcH->getDatatype() == srcT->getDatatype()) && (srcH->getPrecision() == srcT->getPrecision()) &&
                     (srcH->getScale() == srcT->getScale()) && (tgtH->getDatatype() == tgtT->getDatatype()) &&
                     (tgtH->getPrecision() == tgtT->getPrecision()) && (tgtH->getScale() == tgtT->getScale()));
              }
            }
            break;

          case ex_clause::ARITH_TYPE:
            match = (((ex_arith_clause *)clauseHead)->getInstruction() ==
        ...
        while (match && prevHead && prevTail && prevHead->isOpdata()) {
          ...
          if (prevHead->getWOps().entries() && prevTail->getWOps().entries()) {
            if (prevHead->getWOps()[0]->getLen() != prevTail->getWOps()[0]->getLen()) match = FALSE;
          ...
          } else if (prevHead->getROps().entries() && prevTail->getROps().entries()) {
            readSeen = TRUE;
            if (prevHead->getROps()[0]->getBvIndex() != prevTail->getROps()[0]->getBvIndex()) match = FALSE;

            if (prevHead->getROps()[0]->getLen() != prevTail->getROps()[0]->getLen()) match = FALSE;
          } else {
            match = FALSE;
          }
        ...
        if (!match || (readDef == NULL) || (readSeen == FALSE)) continue;
        ...
        if (tailBlock->findDef(tail, readOperand, TRUE) != readDef) continue;

        // Do the read operands of the head change along the path to this member
        if (!doOperandsHaveSameDef(head, tail)) continue;

        if (!readOperand->hasKnownLen() && !readOperand->isVarchar()) continue;
      } else {
        ...
        for (j = 0; match && (j < head->getROps().entries()); j++) {
          if (head->getROps()[j]->getBvIndex() != tail->getROps()[j]->getBvIndex()) match = FALSE;

          if (head->getROps()[j]->getLen() != tail->getROps()[j]->getLen()) match = FALSE;
        }
        ...
        if (match) {
          switch (head->getOpcode()) {
            ...
            case PCIT::COMP_MBIN32S_MATTR5_MATTR5_IBIN32S:
            case PCIT::LIKE_MBIN32S_MATTR5_MATTR5_IBIN32S_IBIN32S: {
              for (j = 3; j < (CollIndex)PCode::getInstructionLength(head->code); j++)
        ...
        // First verify that the head def reaches the tail inst
        if (tailBlock->findDef(tail, head->getWOps()[0], TRUE) != head) continue;

        // Do the read operands of the head change along the path to this member
        if (!doOperandsHaveSameDef(head, tail)) continue;

        if (!head->getWOps()[0]->hasKnownLen() && !head->getWOps()[0]->isVarchar()) continue;
      ...
      if (writeOperand->isVarchar()) {
        newInst = tailBlock->insertNewInstAfter(tail, PCIT::MOVE_MATTR5_MATTR5);
      } else
        newInst = tailBlock->insertNewInstAfter(tail, PCIT::MOVE_MBIN8_MBIN8_IBIN32S);
...
NABoolean PCodeCfg::globalCSE() {
  ...
  while (restart) {
    ...
    if (restart) copyPropagation(2, FALSE);
  }

  destroyForCSE(cseList);

  if (rDefsComputed) removeReachingDefs();
...
//
NABoolean PCodeCfg::copyPropForwardsHelper2(PCodeInst *use, PCodeOperand *useSrc, PCodeOperand *moveSrc) {
  NABoolean restart = FALSE;
  ...
  if (moveSrc->hasSameLength(useSrc)) {
    ...
    switch (use->getOpcode()) {
      ...
      case PCIT::SUM_MATTR3_MATTR3_IBIN32S_MBIN64S_MBIN64S:
        if ((useSrc->getType() != PCIT::MBIN64S) || (moveSrc->getType() != PCIT::MBIN32S)) return restart;

        newOpc = (use->getOpcode() == PCIT::SUM_MBIN64S_MBIN64S) ? PCIT::SUM_MBIN64S_MBIN32S
...
void PCodeCfg::copyPropForwardsHelper(PCodeBlock *block, PCodeInst *start, PCodeOperand *moveTgt, PCodeOperand *moveSrc,
  ...
  if (level >= COPY_PROP_MAX_RECURSION) return;
  ...
  FOREACH_INST_IN_BLOCK_AT(block, use, start) {
    for (i = 0; i < use->getROps().entries(); i++) {
      ...
      if (useSrc->getBvIndex() == moveTgt->getBvIndex()) res = copyPropForwardsHelper2(use, useSrc, moveSrc);
  ...
  for (i = 0; i < block->getSuccs().entries(); i++) {
    PCodeBlock *succ = block->getSuccs()[i];
    if (succ->getPreds().entries() > 1) continue;
...
NABoolean PCodeCfg::copyPropBackwardsHelper(INSTLIST &list, PCodeBlock *block, PCodeInst *start, PCodeOperand *moveTgt,
  ...
  if (level >= COPY_PROP_MAX_RECURSION) return FALSE;
  ...
  FOREACH_INST_IN_BLOCK_BACKWARDS_AT(block, def, start) {
    ...
    if ((def->code[0] == PCIT::FILL_MEM_BYTES) || def->getWOps().entries() == 1) defTgt = def->getWOps()[0];

    // Assignment has to involve source operand.
    if (defTgt && (defTgt->getBvIndex() == moveSrc->getBvIndex()) && moveTgt->hasSameLength(defTgt)) {
      list.insert(def);
      return TRUE;
    } else {
      for (i = 0; i < def->getWOps().entries(); i++) {
      ...
      for (i = 0; i < def->getROps().entries(); i++) {
        if (def->getROps()[i]->getBvIndex() == moveTgt->getBvIndex()) return FALSE;
      }
    }
  }
  ENDFE_INST_IN_BLOCK_BACKWARDS_AT
  ...
  for (i = 0; i < block->getPreds().entries(); i++) {
    if (block->getPreds()[i]->getSuccs().entries() > 1) return FALSE;
  }
  ...
  for (i = 0; i < block->getPreds().entries(); i++) {
    ...
    if (!copyPropBackwardsHelper(list, pred, predLastInst, moveTgt, moveSrc, level + 1)) {
      return FALSE;
    }
  }
...
void PCodeCfg::copyPropagation(int phase, NABoolean reachDefsAvailable) {
  ...
  FOREACH_BLOCK(block, firstInst, lastInst, index){FOREACH_INST_IN_BLOCK(block, inst){PCIT::Instruction opc;

  // First perform some constant folding.  We do this beforehand to help
  // further propagate these constants downwards/upwards
  if (doConstantFolding && ((inst = constantFold(inst, reachDefsAvailable)) == NULL)) continue;
  ...
  if (phase == 2) {
    if (reachDefsAvailable) {
      for (i = 0; i < inst->getROps().entries(); i++) {
        ...
        if ((def == (PCodeInst *)-1) || !def || !def->isMove() || (!def->getWOps()[0]->isTemp())) continue;
        ...
        if (!moveTgt->hasSameLength(moveSrc)) {
          ...
          if (def->getOpcode() == PCIT::MOVE_MATTR5_MASCII_IBIN32S) {
            ...
            if (!def || (def != block->findDef(inst, moveSrc, TRUE))) continue;
        ...
        if (copyPropForwardsHelper2(inst, op, moveSrc)) constantFold(inst, reachDefsAvailable);
      }
      ...
      // No support for mixed sized moves during forward prop just yet
      if (!moveTgt->hasSameLength(moveSrc)) continue;
  ...
  // Backwards Propagation
    ...
    if (moveTgt->getType() == PCIT::MATTR5 && moveTgt->isVarchar() && moveTgt->getVoaOffset() != -1) {
      continue;
    }

    INSTLIST list(heap_);

    if (copyPropBackwardsHelper(list, block, inst->prev, moveTgt, moveSrc, 0) && (list.entries() > 0)) {
      for (i = 0; i < list.entries(); i++) {
        ...
        PCodeInst *newInst = (def->isBranch()) ? defBlock->getSuccs()[0]->insertNewInstBefore(NULL, opc)
  ...
  FOREACH_BLOCK_FAST(block, firstInst, lastInst, index) {
    ...
    FOREACH_INST_IN_BLOCK_BACKWARDS(block, inst) {
      ...
      for (i = 0; remove && (i < inst->getWOps().entries()); i++)
        if (!writeWithNoReadList.contains(inst->getWOps()[i]->getBvIndex())) remove = FALSE;
      ...
      // Add writes to list
      for (i = 0; i < inst->getWOps().entries(); i++) writeWithNoReadList += inst->getWOps()[i]->getBvIndex();

      // Remove reads from list
      for (i = 0; i < inst->getROps().entries(); i++) writeWithNoReadList -= inst->getROps()[i]->getBvIndex();
    }
    ENDFE_INST_IN_BLOCK_BACKWARDS
  }
  ENDFE_BLOCK_FAST
...
NABoolean PCodeCfg::removeOrRewriteDeadCodeInstruction(PCodeInst *inst) {
  ...
  switch (inst->getOpcode()) {
    ...
    case PCIT::BRANCH_OR: {
      ...
      while (TRUE) {
        if (fallThroughBlock->isEmpty() && fallThroughBlock->getFallThroughBlock())
      ...
      if (tgtBlock == fallThroughBlock) remove = TRUE;
      ...
      // we can delete the block in this case too.
               ...
               (fallThroughBlock->getFirstInst() == fallThroughBlock->getLastInst()) &&
               (tgtBlock->getFirstInst()->getOpcode() == fallThroughBlock->getFirstInst()->getOpcode())) {
        // If PCIT::RETURN, no problem.  Else need to check value of
        // PCIT::RETURN_IBIN32S
        if ((tgtBlock->getFirstInst()->getOpcode() == PCIT::RETURN) ||
            (tgtBlock->getFirstInst()->code[1] == fallThroughBlock->getFirstInst()->code[1])) {
          remove = TRUE;
        }
      }
    ...
    case PCIT::NOT_NULL_BRANCH_COMP_MBIN32S_MBIN32S_IATTR3_IBIN32S: {
      ...
      if (targetFirstInst &&
          (((targetFirstInst->getOpcode() == PCIT::MOVE_MBIN32S_IBIN32S) && targetFirstInst->code[3] == constant) ||
           ((targetFirstInst->getOpcode() == PCIT::MOVE_MBIN32U_MBIN32U) && targetFirstInst->getROps()[0]->isConst() &&
        ...
        if (!targetSucc->getLiveVector().testBit(newWriteOp->getBvIndex()) &&
          ...
          inst->code[1 + inst->getWOps()[0]->getStackIndexPos()] = newWriteOp->getStackIndex();

          inst->code[1 + inst->getWOps()[0]->getOffsetPos()] = newWriteOp->getOffset();
          ...
          if (targetSucc->getPreds().entries() == 1) targetSucc->deleteInst(targetFirstInst);
...
PCodeOperand *PCodeBlock::isIndirectBranchCandidate(PCodeBlock *headBlock, PCodeOperand *column, NABoolean forInList) {
  ...
  if (!branch || !branch->isLogicalBranch() || !eqComp || !eqComp->isEqComp()) return NULL;
  ...
  if (!forInList) {
    if (branch->getOpcode() != PCIT::BRANCH_AND) return NULL;
  } else {
    if ((headBlock == NULL) && (branch->getOpcode() != PCIT::BRANCH_OR)) return NULL;
  }
  ...
  if (headBlock) {
    if (getFirstInst() != eqComp) return NULL;
  ...
  if (!eqComp->getWOps()[0]->isTemp() || (eqComp->getWOps()[0]->getBvIndex() != branch->getROps()[0]->getBvIndex()))
    return NULL;
  ...
  if (eqComp->isIntEqComp() || eqComp->isFloatEqComp()) {
    ...
    if (iVal == PCodeCfg::INVALID_INT64) return NULL;
  }
...
CollIndex *PCodeCfg::createLookupTableForSwitch(OPLIST &constants, BLOCKLIST &blocks, UInt32 &tableSize,
                                                NABoolean forInList) {
  CollIndex i, j;
  ...
  if (constants.entries() <= 1) return NULL;
  ...
  assert((constants.entries() >= 1) &&
         ((strVals.entries() == constants.entries()) || (intVals.entries() == constants.entries())));
  ...
  while (restart && (primeIdx < MAX_NUM_OF_PRIMES)) {
      ...
      switch (constant->getType()) {
        ...
        case PCIT::MATTR5: {
          ...
          if (table[index] == INVALID_INT32) {
            ...
            if (!forInList) {
              jumpTable[(index >> 1)] = (Long)(blocks[i]->getFallThroughBlock());
            }
          } else {
            // Collision.
            ...
            if (j == maxChainLength) restart = TRUE;
          }
    ...
    if (!restart && collisionOccurred) {
      ...
      if (((float)totalWorstCaseChecks / (float)(totalCount)) > 2.0) restart = TRUE;
    }
  ...
  // the switch table.
  if (!forInList) jumpTable[tableSize] = (Long)(lastBlock->getTargetBlock());
...
//
NABoolean PCodeCfg::indirectBranchReqMet(PCodeInst *eqComp, PCodeOperand *var, CollIndex entries) {
  //
...
void PCodeCfg::indirectBranchOpt(NABoolean forInList) {
  ...
  FOREACH_BLOCK_REV_DFO(headBlock, firstInst, lastInst, index) {
    ...
    if (headBlock->getVisitedFlag()) continue;
    ...
    if ((constant = headBlock->isIndirectBranchCandidate(NULL, var, forInList))) {
      ...
      while (TRUE) {
        PCodeBlock *child = (forInList) ? parent->getFallThroughBlock() : parent->getTargetBlock();

        // Parent must dominate child block
        if (child->getPreds().entries() != 1) break;

        // If child block is NOT a candidate, break out.
        if (!(constant = child->isIndirectBranchCandidate(parent, var, forInList))) {
          break;
        }
        ...
        if (forInList) {
          NABoolean isAndBranch = (child->getLastInst()->getOpcode() == PCIT::BRANCH_AND);

          // False branch needs to fall-through to TRUE block
          if (isAndBranch && (child->getFallThroughBlock() != parent->getTargetBlock())) break;

          // True branch needs to target to TRUE block
          if (!isAndBranch && (child->getTargetBlock() != parent->getTargetBlock())) break;
        }
        ...
        if (forInList && (child->getLastInst()->getOpcode() == PCIT::BRANCH_AND)) break;
        ...
        if (constants.entries() >= maxItemsForInListOpt) break;
      ...
      if (!indirectBranchReqMet(eqComp, var, constants.entries())) continue;
      ...
      offsetPtr = createLookupTableForSwitch(constants, childBlocks, tableSize, forInList);
      if (offsetPtr == NULL) return;
      ...
      if (eqComp->isIntEqComp() || eqComp->isFloatEqComp()) {
        ...
        newInst->code[6] = (var->isVarchar()) ? var->getVcMaxLen() : var->getLen();
      ...
      if (forInList) {
        ...
        if (!keepCompare) headBlock->deleteInst(eqComp);
        ...
        PCodeBlock *falseBlock = (blocks[blocks.entries() - 1]->getTargetBlock() == headBlock->getTargetBlock())
        ...
        for (i = 0; i < targetOffPtrs.entries(); i++) headBlock->addEdge((PCodeBlock *)(*(targetOffPtrs[i])));
      }
...
void PCodeCfg::cfgRewiring(int flags) {
    ...
    FOREACH_BLOCK(block, firstInst, lastInst, index) {
      if (!block->isEmpty() && lastInst->isBranch()) {
        ...
        if (lastInst->isLogicalBranch() && (tgtBlock == fallThroughBlock)) {
          PCodeInst *newInst = block->reduceBranch(tgtBlock, *zeroes_, *ones_, *neg1_);
        ...
        if (!lastInst->isUncBranch() && fallInst && fallInst->isUncBranch()) {
          ...
          for (i = 0; i < trampBlock->getPreds().entries(); i++) {
            ...
            if (predFallThru && predFallThru == trampBlock) {
              if (!pred->getLastInst() || !pred->getLastInst()->isUncBranch()) remove = FALSE;
            }
          }
          ...
          if (remove == TRUE) {
            ...
            if (fallThroughBlock->getPreds().entries() == 0) deleteBlock(fallThroughBlock);
          }
        }
        ...
        // T1: BRANCH
        //
        if (lastInst->isUncBranch() && tgtInst && tgtInst->isLogicalBranch() && !tgtInst->isUncBranch() && FALSE) {
          // Fix up source block
          block->removeEdge(tgtBlock);
          block->deleteInst(lastInst);
          ...
          // Delete trampoline block if it's now empty
          if (tgtBlock->getPreds().entries() == 0) deleteBlock(tgtBlock);
        } else if (tgtInst && (tgtInst->isUncBranch() || ((flags & LIVE_ANALYSIS_AVAILABLE) &&
                                                          tgtInst->isLogicalBranch() && lastInst->isLogicalBranch()))) {
          ...
          if (tgtInst->isLogicalBranch()) {
            ...
            if (liveVector.testBit(wBvIndex)) continue;

            if ((tgtInst->getROps()[0]->getBvIndex() != lastInst->getWOps()[0]->getBvIndex()) &&
                (tgtInst->getROps()[0]->getBvIndex() != lastInst->getROps()[0]->getBvIndex()))
              continue;

            if (tgtInst->getOpcode() != lastInst->getOpcode()) trampBlock = tgtBlock->getFallThroughBlock();
          }
  ...
  if (flags & MERGE_BLOCKS) {
    FOREACH_BLOCK(block, firstInst, lastInst, index) {
      ...
      for (i = 0; i < block->getPreds().entries(); i++) {
        PCodeBlock *pred = block->getPreds()[i];
        if (pred->getLastInst() && pred->getLastInst()->isIndirectBranch()) found = TRUE;
      }
      ...
      if (block == entryBlock_) entryBlock_ = ftBlock;
    }
    ENDFE_BLOCK
  }

  // Remove empty blocks
  if (flags & REMOVE_EMPTY_BLOCKS) FOREACH_BLOCK(block, firstInst, lastInst, index) {
      if (block->isEmpty()) {
        deleteBlock(block);
      }
    }
...
void PCodeCfg::peepholeConstants() {
  FOREACH_BLOCK(block, firstInst, lastInst, index){FOREACH_INST_IN_BLOCK_BACKWARDS(
      block, inst){//
  ...
  //
  if ((inst->getOpcode() == PCIT::FILL_MEM_BYTES) && (inst->code[4] == 0)) {
  ...
  if (inst->isMove()) {
    if (inst->getWOps()[0]->getBvIndex() == inst->getROps()[0]->getBvIndex()) {
      block->deleteInst(inst);
      continue;
    }
  }
  ...
  if (inst->getOpcode() == PCIT::SUM_MBIN64S_MBIN64S) {
    ...
    if ((def == (PCodeInst *)-1) || !def || (def->getOpcode() != PCIT::MOVE_MBIN64S_MBIN32S)) continue;
...
void PCodeCfg::deleteBlock(PCodeBlock *block) {
  ...
  // Get rid of preds
  while (block->getPreds().entries()) block->getPreds()[0]->removeEdge(block);
...
void PCodeCfg::clearVisitedFlags() {
  FOREACH_BLOCK(block, firstInst, lastInst, index) { block->setVisitedFlag(FALSE); }
  ENDFE_BLOCK
}
...
void PCodeCfg::markReachableBlocks(PCodeBlock *block) {
  block->setVisitedFlag(TRUE);
  for (CollIndex i = 0; i < block->getSuccs().entries(); i++)
    if (!block->getSuccs()[i]->getVisitedFlag()) markReachableBlocks(block->getSuccs()[i]);
}
...
// blocks.
//
PCodeBlock *PCodeCfg::copyCfg(PCodeBlock *start, PCodeBlock *end, PCodeBlock **map) {
  CollIndex i;
  ...
  if (start != end) {
    for (i = 0; i < start->getSuccs().entries(); i++) {
      ...
      block->addEdge((succ->getVisitedFlag()) ? map[succ->getBlockNum()] : copyCfg(succ, end, map));
    }
  }
...
void PCodeCfg::createCfg() {
  ...
  for (tempInst = allInsts_->at(0); tempInst; tempInst = tempInst->next) {
    tempBlock = new (heap_) PCodeBlock(++maxBlockNum_, tempInst, heap_, this);
    ...
    while (!tempInst->isBranch() && !tempInst->isIndirectBranch() && (tempInst->getOpcode() != PCIT::RETURN) &&
           (tempInst->getOpcode() != PCIT::RETURN_IBIN32S) && (tempInst->next != NULL) &&
    ...
    // has a fall-through
    if (tempInst->isUncBranch() || tempInst->isIndirectBranch() || (tempInst->getOpcode() == PCIT::RETURN) ||
...
//
NABoolean PCodeCfg::canPCodeBeOptimized(PCodeBinary *pCode, NABoolean &pExprCacheable /* OUT */
                                        ,
  ...
  for (ex_clause *clause = expr_->getClauses(); clause; clause = clause->getNextClause()) {
    // We can't distinguish read/write operands for INOUT clauses
    if (clause->getClassID() == ex_clause::INOUT_TYPE) return FALSE;
    ...
    else if (clause->getClassID() == ex_clause::FUNC_COMPOSITE_CONCAT)
      return FALSE;

    else if (clause->getClassID() == ex_clause::FUNC_COMPOSITE_CREATE)
      return FALSE;

    else if (clause->getClassID() == ex_clause::FUNC_COMPOSITE_DISPLAY)
      return FALSE;

    else if (clause->getClassID() == ex_clause::FUNC_COMPOSITE_EXTRACT)
      return FALSE;
  }
...
NABoolean PCodeCfg::cfgHasLoop() {
  ...
  for (i = 0; i < allInsts_->entries(); i++) {
    ...
    if (inst->isIndirectBranch()) return TRUE;
    ...
    if (inst->isBranch()) {
      ...
      if (*(targetOffPtrs[0]) < 0) return TRUE;
    }
  }
  ...
  //
  if (brnch_cnt > expr_->getPCodeMaxOptBrCnt()) return TRUE;
  if (inst_cnt > expr_->getPCodeMaxOptInCnt()) return TRUE;
...
NABoolean PCodeCfg::createInsts(PCodeBinary *pcode) {
  ...
  indexToOperandMap_ = new (heap_) NAHashDictionary<CollIndex, PCodeOperand>(&collIndexHashFunc, 100, TRUE, heap_);

  targets_ = new (heap_) NAHashDictionary<ULong, PCodeBinary>(&targetHashFunc, 100, TRUE, heap_);
...
NABoolean PCodeCfg::addOverlappingOperands(NABoolean detectOnly) {
  ...
  FOREACH_BLOCK_DFO(block, firstInst, lastInst,
                    index){FOREACH_INST_IN_BLOCK(block, inst){CollIndex numOfReads = inst->getROps().entries();
  for (i = 0; i < numOfReads; i++) {
    PCodeOperand *op = inst->getROps()[i];

    // Exclude constant operands and operands already inserted
    if (op->isConst() || tempBv.testBit(op->getBvIndex())) continue;
  ...
  for (i = 0; i < numOfWrites; i++) {
    ...
    if (exclude && tempWriteBv.testBit(op->getBvIndex())) continue;
...
for (i = 0; i < writes.entries(); i++) {
  ...
  for (j = 0; j < reads.entries(); j++) {
    ...
    if (readOp->getBvIndex() == writeBv) continue;
    ...
    if (op->canOverlap(readOp)) {
      ...
      for (k = 0; k < inst->getWOps().entries(); k++) {
        if (inst->getWOps()[k]->getBvIndex() == readOp->getBvIndex()) allOpsOverlap = FALSE;
      }

      if (allOpsOverlap == FALSE) continue;

      // Overlap found
      found = TRUE;

      // If we're just detecting, return found
      if (detectOnly) return found;
...
void PCodeCfg::addOperand(PCodeBinary *pcode, OPLIST &opListPtr, int stackIdxPos, int offsetPos, int nullBitIdx,
  ...
  if (found) {
    ...
    operandPtr =
        new (heap_) PCodeOperand(stackIdx, offset, nullBitIdx, stackIdxPos, offsetPos, *val, am, len, voaOffset);
  } else {
    ...
    *bvIndexPtr = maxBVIndex_++;
    ...
    operandPtr =
        new (heap_) PCodeOperand(stackIdx, offset, nullBitIdx, stackIdxPos, offsetPos, *bvIndexPtr, am, len, voaOffset);
    ...
    if ((zeroOffset_ != -1) && (operandPtr->isConst())) updateConstVectors(operandPtr);
  }
...
ex_clause *PCodeCfg::findClausePtr(PCodeInst *inst) {
  ...
  if (inst->next == NULL) {
    pcode = inst->code;
    while (pcode[0] != PCIT::CLAUSE_EVAL) pcode += PCode::getInstructionLength(pcode);
  } else {
    while (inst->code[0] != PCIT::CLAUSE_EVAL) inst = inst->next;
    pcode = inst->code;
  }
...
void PCodeInst::modifyOperandsForVarchar(PCodeCfg *cfg) {
  ...
  switch (getOpcode()) {
    case PCIT::MOVE_MATTR5_MATTR5:
      ...
      nullLen = comboPtr[0];
      vcLen = comboPtr[1];
      getWOps()[0]->setVarcharFields(2, -1, pCode[2], -1, pCode[3], nullLen, vcLen);

      comboLen = pCode[9];
      nullLen = comboPtr[0];
      vcLen = comboPtr[1];
      getROps()[0]->setVarcharFields(7, -1, pCode[7], -1, pCode[8], nullLen, vcLen);
      break;
    ...
    case PCIT::COMP_MBIN32S_MUNIV_MUNIV_IBIN32S:
      ...
      nullLen = comboPtr[0];
      vcLen = comboPtr[1];
      getROps()[0]->setVarcharFields(4, -1, pCode[4], -1, pCode[5], nullLen, vcLen);

      comboLen = pCode[11];
      nullLen = comboPtr[0];
      vcLen = comboPtr[1];
      getROps()[1]->setVarcharFields(9, -1, pCode[9], -1, pCode[10], nullLen, vcLen);
    ...
    case PCIT::CONCAT_MATTR5_MATTR5_MATTR5:
      ...
      nullLen = comboPtr[0];
      vcLen = comboPtr[1];
      getROps()[1]->setVarcharFields(12, -1, pCode[12], -1, pCode[13], nullLen, vcLen);
    ...
    case PCIT::UPDATE_ROWLEN3_MATTR5_IBIN32S:
      ...
      nullLen = (char)comboPtr[0];
      vcLen = (char)comboPtr[1];
      getROps()[0]->setVarcharFields(2, -1, pCode[2], -1, pCode[3], nullLen, vcLen);
      break;
  }
}
...
void PCodeCfg::loadOperandsOfInst(PCodeInst *newInst) {
  ...
  switch (opc) {
    ...
    case PCIT::CLAUSE_EVAL: {
      ...
      while (opdata->prev && opdata->prev->isOpdata()) opdata = opdata->prev;

      // If opdatas for this clause have already been processed, then no need
      // to reprocess.
      if (!opdata->isOpdata() || (opdata->clause_ != NULL)) break;
        ...
        for (i = 0; (i < clause->getNumOperands()) && (sz == -1); i++) {
          ...
          if (operand->getStackIndex() == si) {
            ...
            if (analyzeClause && (operand->getOffset() == off))
              sz = op->getLength();
            else if (op->getNullFlag() && operand->getOffset() == op->getNullIndOffset())
    ...
    case PCIT::FILL_MEM_BYTES: {
      ...
      off = pcode[1] + 4;
      len = pcode[2] - 4;
      if (pcode[2] > 4) addOperand(pcode, newInst->getWOps(), 0, -off, -1, PCIT::MPTR32, len);
    ...
    case PCIT::HASHCOMB_MBIN32U_MBIN32U_MBIN32U:
      ...
      if (opc == PCIT::HASHCOMB_MBIN32U_MBIN32U_MBIN32U) counters_.hashCombCnt_++;
    ...
    case PCIT::NOT_NULL_BRANCH_BULK: {
      ...
      // Sub-opc determines whether all operands are from the same row or not
      if (pcode[1] == 0) {
        for (i = 3; i < length - 2; i += 1) addOperand(pcode, newInst->getROps(), 2, i, -1, PCIT::MBIN16S, 2);
      } else {
        for (i = 2; i < length - 2; i += 2) addOperand(pcode, newInst->getROps(), i, i + 1, -1, PCIT::MBIN16S, 2);
      }
      break;
    }
    ...
    case PCIT::NOT_NULL_BRANCH_MBIN32S_MBIN32S_MBIN32S_IATTR4_IBIN32S: {
      ...
      addOperand(pcode, newInst->getWOps(), 0, 1, attrs->op1NullBitIndex_,
                 ((attrs->fmt_.op1Fmt_ == ExpTupleDesc::SQLMX_ALIGNED_FORMAT) ? PCIT::MPTR32 : PCIT::MBIN16S),
                 ((attrs->fmt_.op1Fmt_ == ExpTupleDesc::SQLMX_ALIGNED_FORMAT) ? -1 : 2));

      addOperand(pcode, newInst->getROps(), 2, 3, attrs->op2NullBitIndex_,
                 ((attrs->fmt_.op2Fmt_ == ExpTupleDesc::SQLMX_ALIGNED_FORMAT) ? PCIT::MPTR32 : PCIT::MBIN16S),
                 ((attrs->fmt_.op2Fmt_ == ExpTupleDesc::SQLMX_ALIGNED_FORMAT) ? -1 : 2));

      addOperand(pcode, newInst->getROps(), 4, 5, attrs->op3NullBitIndex_,
                 ((attrs->fmt_.op3Fmt_ == ExpTupleDesc::SQLMX_ALIGNED_FORMAT) ? PCIT::MPTR32 : PCIT::MBIN16S),
                 ((attrs->fmt_.op3Fmt_ == ExpTupleDesc::SQLMX_ALIGNED_FORMAT) ? -1 : 2));
    ...
    case PCIT::MOVE_MUNI_MUNI_IBIN32S_IBIN32S:
    case PCIT::MOVE_MASCII_MASCII_IBIN32S_IBIN32S:
      type = (opc == PCIT::MOVE_MUNI_MUNI_IBIN32S_IBIN32S) ? PCIT::MUNI : PCIT::MASCII;
    ...
    case PCIT::STRLEN_MBIN32U_MUNIV: {
      ...
      PCIT::AddressingMode type =
          (((opc == PCIT::HASH_MBIN32U_MUNIV) || (opc == PCIT::STRLEN_MBIN32U_MUNIV)) ? PCIT::MUNIV : PCIT::MATTR5);
    ...
    case PCIT::POS_MBIN32S_MATTR5_MATTR5: {
      ...
      PCIT::AddressingMode type = ((opc == PCIT::COMP_MBIN32S_MUNIV_MUNIV_IBIN32S) ? PCIT::MUNIV : PCIT::MATTR5);
    ...
    case PCIT::HDR_MPTR32_IBIN32S_IBIN32S_IBIN32S_IBIN32S_IBIN32S: {
      ...
      if (pcode[5] > 0) addOperand(pcode, newInst->getWOps(), 0, 6, -1, PCIT::MBIN16S, 2);
    ...
    case PCIT::SUBSTR_MATTR5_MATTR5_MBIN32S_MBIN32S: {
      ...
      if (((char *)&pcode[9])[2]) addOperand(pcode, newInst->getROps(), 12, 13, -1, PCIT::MBIN32S, 4);
...
//
void OptPCodeCache::addToCreateOrderList(PCECacheEntry *PCEtoAdd) {
  ...
  if (createOrderTail_) createOrderTail_->setNextInCrOrder(PCEtoAdd);

  createOrderTail_ = PCEtoAdd;

  if (createOrderHead_ == NULL) createOrderHead_ = PCEtoAdd;
}

//
// addToMRUList -- Add a PCE Cache Entry to the MRU list
//
void OptPCodeCache::addToMRUList(PCECacheEntry *PCEtoAdd) {
  ...
  if (MRUHead_) MRUHead_->setPrevInMRUOrder(PCEtoAdd);

  MRUHead_ = PCEtoAdd;

  if (MRUTail_ == NULL) MRUTail_ = PCEtoAdd;
}

//
// removeFromCreateOrderList -- remove a PCE Cache Entry from the Creation Order list
//
void OptPCodeCache::removeFromCreateOrderList(PCECacheEntry *PCEtoDel) {
  ...
  if (nextInCrOrder != NULL) nextInCrOrder->setPrevInCrOrder(prevInCrOrder);

  if (prevInCrOrder != NULL) prevInCrOrder->setNextInCrOrder(nextInCrOrder);

  if (PCEtoDel == createOrderHead_) createOrderHead_ = nextInCrOrder;

  if (PCEtoDel == createOrderTail_) createOrderTail_ = prevInCrOrder;
}

//
// removeFromMRUList -- Remove a PCE Cache Entry from the MRU list
//
void OptPCodeCache::removeFromMRUList(PCECacheEntry *PCEtoRem) {
  ...
  if (nextInMRUOrder != NULL) nextInMRUOrder->setPrevInMRUOrder(prevInMRUOrder);
  if (prevInMRUOrder != NULL) prevInMRUOrder->setNextInMRUOrder(nextInMRUOrder);

  if (PCEtoRem == MRUHead_) MRUHead_ = nextInMRUOrder;
  if (PCEtoRem == MRUTail_) MRUTail_ = prevInMRUOrder;
}
...
//
NABoolean PCECacheEntry::matches(PCodeBinary *unOptPCodePtr, char *unOptConstantsArea, UInt32 unOptPCodeLen,
                                 UInt32 unOptConstsLen, UInt32 NEflag) {
  if ((unOptConstsLen != getOptConstsLen()) || (NEflag && (getOptConstsLen() < getNEConstsLen()))) {
    ...
    if (memcmp(unOptConstantsArea, entryConstsArea, unOptConstsLen) != 0) return FALSE;
  }

  // We know the unOptimized PC length matches, now compare PCode
  PCodeBinary *entryUnOptPC = getUnOptPCptr();
  if (memcmp(unOptPCodePtr, entryUnOptPC, sizeof(PCodeBinary) * unOptPCodeLen) != 0) return FALSE;
...
#endif  // OPT_PCC_DEBUG==1
//
void OptPCodeCache::addPCodeExpr(PCECacheEntry *newPCE
#if OPT_PCC_DEBUG == 1
                                 ,
                                 UInt64 NEgenTime, timeval begAdd, char *sqlStmt
#endif  // OPT_PCC_DEBUG==1
) {
  addToMRUList(newPCE);
  ...
  if (maxOptPCodeSize_ < newPCE->getOptPClen()) maxOptPCodeSize_ = newPCE->getOptPClen();
...
//
PCECacheEntry *OptPCodeCache::findPCodeExprInCache(PCodeBinary *unOptPCodePtr, char *unOptConstantsArea,
                                                   UInt32 NEflag  // Native Expr in use?
                                                   ,
                                                   UInt32 unOptPCodeLen, UInt32 unOptConstsLen
#if OPT_PCC_DEBUG == 1
                                                   ,
                                                   char *sqlStmt
#endif  // OPT_PCC_DEBUG==1
  ...
  if (nextEntry != NULL) nextEntry = nextEntry->getNextInCrOrder();
  ...
  for (; nSrchd <= numEntries_; nSrchd++, currEntry = nextEntry) {
    ...
    //
    if (unOptPCodeLen != currEntry->getUnOptPClen()) continue;  // cannot be a match

    if (unOptConstsLen != currEntry->getUnOptConstsLen()) continue;  // cannot be a match

    match_found = currEntry->matches(unOptPCodePtr, unOptConstantsArea, unOptPCodeLen, unOptConstsLen, NEflag);
  ...
  if (match_found) {
    ...
    numHits_++;
    if (NEflag && (currEntry->getOptConstsLen() < currEntry->getNEConstsLen())) numNEHits_++;
    UInt64 nHits = currEntry->incrPCEHits();
    if (nHits > maxHits_) maxHits_ = nHits;
...
//
void OptPCodeCache::setPCDlogDirPath(NAString *logDirPth) {
...
void OptPCodeCache::throwOutExcessCacheEntries() {
  while (currSize_ > maxSize_ && numEntries_ > 0) {
    ...
    if (PCEtoDel->getPCEHits() > maxHitsDel_) maxHitsDel_ = PCEtoDel->getPCEHits();

    if (PCEtoDel == lastMatchedEntry_) lastMatchedEntry_ = NULL;
...
#define MAX_UNIQ_PART (7 + 4 + 16)
  if ((PCECHeaderWritten_ == 0) && (logDirPathLen <= (LNGBUFLEN - MAX_UNIQ_PART - 1))) {
    PCECHeaderWritten_ = 1;  // Remember so we put file hdr out only once
    sprintf(longBuf,
            "Ev Typ\tUniqCtr\tPCE Hits\t"
            "PC Len\t"
            "Opt PC Len\t"
            "NE Con Len\tOpt Time\t"
            "NEgen Tm\t"
  ...
  if (PCECHeaderWritten_ == 0) return;
  ...
  sprintf(longBuf,
          ...
          "%ld\t "                    // maxOptPCodeSize_
          ,
          eventType, PCEptr->getUniqCtr(), PCEptr->getPCEHits(), PCEptr->getUnOptPClen() * sizeof(PCodeBinary),
          PCEptr->getOptPClen() * sizeof(PCodeBinary), PCEptr->getNEConstsLen(), PCEptr->getOptTime(),
          PCEptr->getNEgenTime(), numLookups_, numHits_, numNEHits_, totSrchd_, numSrchd_, currSize_, numEntries_,
          uniqueCtr_, totalSavedTime_, totalSearchTime_, totalOptTime_, totalNEgenTime_, totByCfC_, maxHits_,
          maxHitsDel_, maxOptPCodeSize_ * sizeof(PCodeBinary));
  fileout << longBuf;
  if (sqlStmt != NULL) {
...
//
void OptPCodeCache::printPCodeExprCacheStats() {
