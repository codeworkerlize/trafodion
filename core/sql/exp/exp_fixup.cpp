...
static void getCaseDatatypes(short attr_type1, int attr_len1, short &type_op1, short attr_type2, int attr_len2,
  ...
  if ((type_op1 >= REC_MIN_INTERVAL) && (type_op1 <= REC_MAX_INTERVAL)) {
    if (((type_op2 >= REC_MIN_BINARY_NUMERIC) && (type_op2 <= REC_MAX_BINARY_NUMERIC)) ||
        ((type_op2 >= REC_MIN_DECIMAL) && (type_op2 <= REC_MAX_DECIMAL)) || (type_op2 == REC_DATETIME) ||
        ((type_op2 >= REC_MIN_INTERVAL) && (type_op2 <= REC_MAX_INTERVAL) && (endField1 == endField2) &&
         (scaleDifference == 0))) {
      switch (attr_len1) {
        case SQL_SMALL_SIZE:
  ...
  if ((type_op2 >= REC_MIN_INTERVAL) && (type_op2 <= REC_MAX_INTERVAL)) {
    if (((type_op1 >= REC_MIN_BINARY_NUMERIC) && (type_op1 <= REC_MAX_BINARY_NUMERIC)) ||
        ((type_op1 >= REC_MIN_DECIMAL) && (type_op1 <= REC_MAX_DECIMAL)) || (type_op1 == REC_DATETIME)) {
      switch (attr_len2) {
        case SQL_SMALL_SIZE:
...
static void getConvCaseDatatypes(short attr_type1, int attr_len1, short &type_op1, short attr_type2, int attr_len2,
  ...
  //
  if ((attr_type1 >= REC_MIN_INTERVAL) && (attr_type1 <= REC_MAX_INTERVAL) && (attr_type2 >= REC_MIN_CHARACTER) &&
      (attr_type2 <= REC_MAX_CHARACTER)) {
    type_op1 = REC_INT_YEAR;
    type_op2 = attr_type2;
  } else if ((attr_type2 >= REC_MIN_INTERVAL) && (attr_type2 <= REC_MAX_INTERVAL) &&
...
ex_expr::exp_return_type ex_expr::fixup(int /*base*/, unsigned short mode, const ex_tcb *tcb, Space *space,
  ...
  if (space && tempsAreaLength_ > 0)  // temps used
  {
    ...
    if (NOT computeSpaceOnly) tempsArea_ = tempAllocArea;
  }

  ex_clause *clause = clauses_;
  while (clause) {
    retcode = clause->fixup(space, exHeap, constantsArea_, tempsArea_, persistentArea_, (short)getFixupConstsAndTemps(),
                            computeSpaceOnly);
    clause->setProcessNulls();
    if (retcode == EXPR_ERROR) return retcode;
  ...
  if (!computeSpaceOnly) setEvalPtr(TRUE /* isSamePrimary */);

  if (mode & INJECT_ERROR)
    errorInjection_ = TRUE;
  else if (mode & INJECT_WARNING)
...
ex_expr::exp_return_type AggrExpr::fixup(int base, unsigned short mode, const ex_tcb *tcb, Space *space,
  ...
  if (initExpr_) {
    if ((retcode = initExpr_->fixup(base, mode, tcb, space, exHeap, computeSpaceOnly, glob)) != EXPR_OK) return retcode;
  }

  if (perrecExpr_) {
    if ((retcode = perrecExpr_->fixup(base, mode, tcb, space, exHeap, computeSpaceOnly, glob)) != EXPR_OK)
      return retcode;
  }

  if (finalNullExpr_) {
    if ((retcode = finalNullExpr_->fixup(base, mode, tcb, space, exHeap, computeSpaceOnly, glob)) != EXPR_OK)
      return retcode;
  }

  if (finalExpr_) {
    if ((retcode = finalExpr_->fixup(base, mode, tcb, space, exHeap, computeSpaceOnly, glob)) != EXPR_OK)
      return retcode;
  }

  if (groupingExpr_) {
    if ((retcode = groupingExpr_->fixup(base, mode, tcb, space, exHeap, computeSpaceOnly, glob)) != EXPR_OK)
      return retcode;
  }

  // fixup the expression to do perrec aggregate evaluation.
  if ((retcode = ex_expr::fixup(base, mode, tcb, space, exHeap, computeSpaceOnly, glob)) != EXPR_OK) return retcode;
...
ex_expr::exp_return_type ex_branch_clause::fixup(Space *space, CollHeap *exHeap, char *constantsArea, char *tempsArea,
                                                 char *persistentArea, short fixupFlag, NABoolean spaceCompOnly) {
  if (getNextClause() != saved_next_clause) setNextClause(saved_next_clause);
...
int ex_arith_clause::findIndexIntoInstrArray(ArithInstruction ci) {
  ...
  if (i < max_array_size) return i;

  return -1;  // not found
}

const ArithInstruction ex_arith_clause::computeCaseIndex(OperatorTypeEnum op, Attributes *attr1, Attributes *attr2,
                                                         Attributes *result) {
  ...
  short type_op2 = -1;
  short type_result = -1;
  getCaseDatatypes(attr1->getDatatype(), attr1->getLength(), type_op1, result->getDatatype(), result->getLength(),
                   type_result, 0 /* don't need to take scale difference into account here */);

  // attr2 eill be null for unary operands.
  // Some methods below expect 2 operands. Change oper2 to be the same as oper1
  // to avoid null pointer exception.
  if (!attr2) attr2 = attr1;
  getCaseDatatypes(attr2->getDatatype(), attr2->getLength(), type_op2, result->getDatatype(), result->getLength(),
                   type_result, 0 /* don't need to take scale difference into account here */);

  const ArithInstrStruct *as = getMatchingRow(op, type_op1, type_op2, type_result);
  ...
  if (as) {
    ...
    if (((op == ITM_PLUS) || (op == ITM_MINUS)) &&
        (((result->getDatatype() >= REC_MIN_BINARY_NUMERIC) && (result->getDatatype() <= REC_MAX_BINARY_NUMERIC)) ||
         ((result->getDatatype() >= REC_MIN_INTERVAL) && (result->getDatatype() <= REC_MAX_INTERVAL)))) {
    ...
    else if ((op == ITM_DIVIDE) && (arithRoundingMode_ != 0)) {
  ...
  if (instruction == ARITH_NOT_SUPPORTED) setInstrArrayIndex(-1);
...
void ex_arith_clause::setInstruction() {
  ...
  if (getOperand(0)->isComplexType()) {
    ...
    instruction =
        computeCaseIndex(getOperType(), getOperand(1), (getNumOperands() == 3 ? getOperand(2) : NULL), getOperand(0));
  }

  if ((instruction == ARITH_NOT_SUPPORTED) && (getInstrArrayIndex() >= 0)) {
    // this is an error, reset instr array index
    setInstrArrayIndex(-1);
  }
}

void ex_arith_clause::setInstruction(OperatorTypeEnum op, Attributes *attr1, Attributes *attr2, Attributes *result) {
...
short ex_arith_clause::isArithSupported(OperatorTypeEnum op, Attributes *attr1, Attributes *attr2, Attributes *result) {
  if (computeCaseIndex(op, attr1, attr2, result) == ARITH_NOT_SUPPORTED)
    return 0;
  else
    return -1;
}

ex_expr::exp_return_type ex_arith_clause::fixup(Space *space, CollHeap *exHeap, char *constants_area, char *temps_area,
                                                char *persistentArea, short fixupConstsAndTemps,
                                                NABoolean spaceCompOnly) {
  setInstruction();
  return ex_clause::fixup(space, exHeap, constants_area, temps_area, persistentArea, fixupConstsAndTemps,
                          spaceCompOnly);
}
...
const ex_comp_clause::CompInstrStruct ex_comp_clause::compInstrInfo[] = {
    ...
    {ITM_ANY_COMP, REC_UNKNOWN, REC_UNKNOWN, instrAndText(BINARY_COMP_VAR)},
    ...
    {ITM_LESS, REC_BOOLEAN, REC_BOOLEAN, instrAndText(LT_BOOL_BOOL)},
    ...
    {ITM_LESS_EQ, REC_BOOLEAN, REC_BOOLEAN, instrAndText(LE_BOOL_BOOL)},
    ...
    {ITM_GREATER, REC_BOOLEAN, REC_BOOLEAN, instrAndText(GT_BOOL_BOOL)},
    ...
    {ITM_GREATER_EQ, REC_BOOLEAN, REC_BOOLEAN, instrAndText(GE_BOOL_BOOL)},
...
const CompInstruction ex_comp_clause::computeCaseIndex(OperatorTypeEnum op, Attributes *attr1, Attributes *attr2) {
  CompInstruction instruction = COMP_NOT_SUPPORTED;

  short type_op1;
  short type_op2;
  getCaseDatatypes(attr1->getDatatype(), attr1->getLength(), type_op1, attr2->getDatatype(), attr2->getLength(),
                   type_op2, 0 /* don't need to take into account scale difference here */);

  const CompInstrStruct *cs = getMatchingRow(op, type_op1, type_op2);
  if (cs) {
    instruction = cs->instruction;

    // Further checks are needed. Do them.
    if ((attr1->getDatatype() == REC_BYTE_F_ASCII) && (attr2->getDatatype() == REC_BYTE_F_ASCII)) {
      if (attr1->getLength() != attr2->getLength()) {
        instruction = ASCII_COMP;
        setInstrArrayIndex(findIndexIntoInstrArray(instruction));
      }
    } else if ((attr1->getDatatype() == REC_BYTE_F_ASCII && attr2->getDatatype() == REC_BYTE_V_ASCII)) {
      if (!attr2->isSpaceSensitive()) {
        instruction = ASCII_COMP;
        setInstrArrayIndex(findIndexIntoInstrArray(instruction));
      }
    } else if ((attr1->getDatatype() == REC_BYTE_V_ASCII && attr2->getDatatype() == REC_BYTE_F_ASCII)) {
      if (!attr1->isSpaceSensitive()) {
        instruction = ASCII_COMP;
        setInstrArrayIndex(findIndexIntoInstrArray(instruction));
      }
    }

    if ((attr1->getDatatype() == REC_NCHAR_F_UNICODE) && (attr2->getDatatype() == REC_NCHAR_F_UNICODE)) {
      if (attr1->getLength() != attr2->getLength()) {
    ...
    if (((attr1->getDatatype() >= REC_MIN_BINARY_NUMERIC) && (attr1->getDatatype() <= REC_MAX_BINARY_NUMERIC)) ||
        ((attr1->getDatatype() >= REC_DECIMAL_UNSIGNED) && (attr1->getDatatype() <= REC_DECIMAL_LSE)) ||
        (attr1->getDatatype() == REC_DATETIME) ||
        ((attr1->getDatatype() >= REC_MIN_INTERVAL) && (attr1->getDatatype() <= REC_MAX_INTERVAL))) {
      // numeric, datetime or interval types
      if (attr1->getScale() != attr2->getScale()) instruction = COMP_NOT_SUPPORTED;

      // for decimal datatypes, comparison done only if lengths are same.
      if ((attr1->getDatatype() >= REC_DECIMAL_UNSIGNED) && (attr1->getDatatype() <= REC_DECIMAL_LSE)) {
        if (attr1->getLength() != attr2->getLength()) instruction = COMP_NOT_SUPPORTED;
      }
    }  // numeric, datetime or interval types
  }    // comparison supported
  else {
    if (((attr1->getDatatype() == REC_BINARY_STRING) || (attr1->getDatatype() == REC_VARBINARY_STRING)) &&
        ((attr2->getDatatype() == REC_BINARY_STRING) || (attr2->getDatatype() == REC_VARBINARY_STRING))) {
      if (attr1->getDatatype() == REC_BINARY_STRING && attr2->getDatatype() == REC_BINARY_STRING)
        instruction = BINARY_COMP;
      else
        instruction = BINARY_COMP_VAR;
      setInstrArrayIndex(findIndexIntoInstrArray(instruction));
    }
  }

  if (instruction == COMP_NOT_SUPPORTED) setInstrArrayIndex(-1);
...
void ex_comp_clause::setInstruction() {
  ...
  if ((instruction == COMP_NOT_SUPPORTED) && (getInstrArrayIndex() >= 0)) {
    // this is an error, reset instr array index
    setInstrArrayIndex(-1);
  }
}

void ex_comp_clause::setInstruction(OperatorTypeEnum op, Attributes *attr1, Attributes *attr2) {
  // Only simple types are handled here.
...
short ex_comp_clause::isComparisonSupported(OperatorTypeEnum op, Attributes *attr1, Attributes *attr2) {
  if (computeCaseIndex(op, attr1, attr2) == COMP_NOT_SUPPORTED)
    return 0;
  else
    return -1;
}

ex_expr::exp_return_type ex_comp_clause::fixup(Space *space, CollHeap *exHeap, char *constants_area, char *temps_area,
                                               char *persistentArea, short fixupConstsAndTemps,
                                               NABoolean spaceCompOnly) {
  setInstruction();
  return ex_clause::fixup(space, exHeap, constants_area, temps_area, persistentArea, fixupConstsAndTemps,
...
const ex_conv_clause::ConvInstrStruct ex_conv_clause::convInstrInfo[] = {
    ...
    {REC_ARRAY, REC_ARRAY, instrAndText(CONV_ARRAY_ARRAY)},
    {REC_ROW, REC_ROW, instrAndText(CONV_ROW_ROW)},
...
void ex_conv_clause::populateInstrOffsetIndex() {
  ...
  while (i < lv_max_array_size) {
    if (ex_conv_clause::convInstrInfo[i].type_op1 != lv_source_type) {
      ...
      if ((lv_source_type > -1) && (lv_MaxIndex < (lv_MaxIndexCapacity - 1))) {
  ...
  int *lv_convIndexSparse = (int *)calloc(sizeof(int), (sv_MaxOpTypeValue + 1));
  // Initialize to -1
...
ConvInstruction ex_conv_clause::findInstruction(short sourceType, int sourceLen, short targetType, int targetLen,
  ...
  if (instruction == CONV_NOT_SUPPORTED) {
    if (targetType == REC_BINARY_STRING) {
      ...
      setInstrArrayIndex(findIndexIntoInstrArray(instruction));
    } else if ((DFS2REC::isNumeric(sourceType)) && (DFS2REC::isTinyint(targetType))) {
...
void ex_conv_clause::setInstruction() {
  ...
  if ((getOperand(0)->getRowsetSize() > 0) && (getOperand(1)->getRowsetSize() > 0)) {
    ...
    instruction =
        findInstruction(getOperand(1)->getDatatype(), getOperand(1)->getLength(), getOperand(0)->getDatatype(),
                        getOperand(0)->getLength(), getOperand(1)->getScale() - getOperand(0)->getScale());
  ...
  if ((instruction == CONV_NOT_SUPPORTED) && (getInstrArrayIndex() >= 0)) {
    // this is an error, reset instr array index
    setInstrArrayIndex(-1);
  }
};

ex_expr::exp_return_type ex_conv_clause::fixup(Space *space, CollHeap *exHeap, char *constants_area, char *temps_area,
                                               char *persistentArea, short fixupConstsAndTemps,
                                               NABoolean spaceCompOnly) {
  setInstruction();
  return ex_clause::fixup(space, exHeap, constants_area, temps_area, persistentArea, fixupConstsAndTemps,
...
ex_expr::exp_return_type ExFunctionRangeOfValues::fixup(Space *space, CollHeap *exHeap, char *constantsArea,
                                                        char *tempsArea, char *persistentArea, short fixupFlag,
                                                        NABoolean spaceCompOnly) {
  rangeOfValues_ = (nativeByteOrder_) ? new (exHeap) NativeBloomFilterRT(totalSizeCap_, exHeap)
                                      : new (exHeap) BloomFilterRT(totalSizeCap_, exHeap);
