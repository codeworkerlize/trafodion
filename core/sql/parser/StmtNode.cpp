...
const StmtNode *StmtNode::castToStatementExpr() const { return this; }
...
const NAString StmtQuery::getText() const { return "StmtQuery"; }  // StmtQuery::getText()

void StmtQuery::print(FILE *f, const char *prefix, const char *) const {
  // print operator text
...
NABoolean StmtModule::applyDefaults(NABoolean wantR18behavior) {
  ...
  if (charSet().isNull()) charSet() = CharInfo::getCharSetName(CharInfo::DefaultCharSet);
  ...
  if (name().getCatalogName().isNull()) {
    ...
    const SchemaName &defcs =
        ActiveSchemaDB()->getDefaultSchema(SchemaDB::REFRESH_CACHE | SchemaDB::FORCE_ANSI_NAMETYPE);
  ...
  if (wantR18behavior) {
    ...
    if (!ActiveSchemaDB()->getDefaults().setCatalog(catName)) err = TRUE;

    NAString schName(name().getUnqualifiedSchemaNameAsAnsiString());
    if (!ActiveSchemaDB()->getDefaults().setSchema(schName)) err = TRUE;
...
NABoolean StmtModule::unparse(NAString &result, NABoolean wantR18behavior) {
  ...
  result += NAString("MODULE ") + name().getQualifiedNameAsAnsiNTFilenameString() + " NAMES ARE " + charSet() + ";";

  return err;
}

NABoolean StmtModule::unparseSimple(NAString &result, NABoolean wantR18behavior) {
  NABoolean err = applyDefaults(wantR18behavior);
  // assume caller will handle err

  result += NAString("MODULE ") + name().getQualifiedNameAsAnsiString() + " NAMES ARE " + charSet() + ";";

  return err;
}

void StmtModule::applyModuleCatalogSchema(const NAString &cat, const NAString &sch) {
  // apply moduleCatalog if necessary
  if (name().getCatalogName().isNull()) {
    if (!cat.isNull()) name().setCatalogName(cat);
  }

  // apply moduleSchema if necessary
  if (name().getSchemaName().isNull()) {
    if (!sch.isNull()) name().setSchemaName(sch);
  }
  ...
  if (name().getCatalogName().isNull() || name().getSchemaName().isNull()) {
    const SchemaName &defcs =
        ActiveSchemaDB()->getDefaultSchema(SchemaDB::REFRESH_CACHE | SchemaDB::FORCE_ANSI_NAMETYPE);
    if (name().getCatalogName().isNull()) name().setCatalogName(defcs.getCatalogName());
    if (name().getSchemaName().isNull()) name().setSchemaName(defcs.getSchemaName());
  }
}
...
StmtAllocStaticDesc::StmtAllocStaticDesc(NamePlusDesc *namePlusDescPtr, StmtOrCursEnum theTag, NAString *theName)
    : StmtNode(STM_ALLOC_STATIC_DESC),
      mainDescriptor(namePlusDescPtr),
      entityTag(theTag),
      entityName(theTag == NAME_ABSENT ? NULL : theName) {}

NamePlusDesc::NamePlusDesc(NAString *descName, NABoolean isInputFlag, DescTypeList *theArray)
    : descriptorName(descName), isInput(isInputFlag), descriptorArray(theArray) {}

StmtDeclStatCurs::StmtDeclStatCurs(NAString *new_curs_name, RelExpr *new_curs_spec,
                                   NABoolean holdable  // QSTUFF
                                   )
    : StmtNode(STM_DECL_STATCURS, holdable), cursorName(new_curs_name), cursorSpec(new_curs_spec) {}

StmtDeclDynCurs::StmtDeclDynCurs(NAString *new_curs_name, NAString *new_stmt_name,
                                 NABoolean holdable  // QSTUFF
                                 )
    : StmtNode(STM_DECL_DYNCURS, holdable), cursorName(new_curs_name), stmtName(new_stmt_name) {}
...
NABoolean StaticDescItem::isDataTypePointer() const { return (dataTypePointer != NULL); }

NABoolean StaticDescItem::isLiteralPointer() const { return (literalPointer != NULL); }
...
StmtDescBase::~StmtDescBase() { delete descriptorName; }
