...
void ParInsertNameLocForStar(ColRefName *pColRefName) {
  ...
  StringPos starPos;
  const ParScannedTokenQueue::scannedTokenInfo &tokInfo = ParScannedTokens->getScannedTokenInfo(0);
  if (tokInfo.tokenStrLen EQU(size_t) 1 AND NAString(&pInputStr[tokInfo.tokenStrPos], (size_t)1) EQU "*") {
    starPos = tokInfo.tokenStrPos;
  } else {
    const ParScannedTokenQueue::scannedTokenInfo &prevTokInfo = ParScannedTokens->getScannedTokenInfo(-1);
    ComASSERT(prevTokInfo.tokenStrLen EQU(size_t) 1 AND NAString(&pInputStr[prevTokInfo.tokenStrPos], (size_t)1) EQU
              "*");
...
NABoolean ParSetTextEndPos(ElemDDLConstraintCheck *pCkCnstrntNode) {
  ...
  pCkCnstrntNode->setEndPosition(tokInfo->tokenStrPos + tokInfo->tokenStrLen - 1);
  return FALSE;  // no error
}

NABoolean ParSetTextEndPos(ElemDDLDivisionClause *pDivClauseParseNode) {
  if (pDivClauseParseNode == NULL) return TRUE;  // error
  ...
  while (ParScannedTokens->isQueueIndexWithinRange(tokIndex) AND tokStr NEQ ")") {
    tokIndex--;
    if (ParScannedTokens->isQueueIndexOutOfRange(tokIndex)) break;
  ...
  if (ParScannedTokens->isQueueIndexOutOfRange(tokIndex) OR tokStr NEQ ")") return TRUE;  // syntax error
  pDivClauseParseNode->setEndPosition(pTokInfo->tokenStrPos + pTokInfo->tokenStrLen - 1);
  return FALSE;  // no error
}
...
NABoolean ParSetTextEndPos(StmtDDLCreateView *pCreateViewParseNode) {
  ...
  if (isEndOfInputStr OR tokStr NEQ "OPTION") {
    ...
    if (isoStr.contains("_ISO88591"))
      ...
      isoOffset = 7;
    tokStr = NAString(&pInputStr[tokInfo->tokenStrPos], tokInfo->tokenStrInputLen + isoOffset);
  ...
  pCreateViewParseNode->setEndPosition(tokInfo->tokenStrPos + tokInfo->tokenStrInputLen - 1 + isoOffset);
  return FALSE;  // no error
}
...
NABoolean ParSetTextEndPos(StmtDDLCreateTrigger *pCreateTriggerParseNode) {
  ...
  pCreateTriggerParseNode->setEndPosition(tokInfo->tokenStrPos + tokInfo->tokenStrLen - 1);
  return FALSE;  // no error
}
...
void ParSetTextStartPosForCreateTrigger(ParNameLocList *pNameLocList) {
  ...
  if (tokStr NEQ "TRIGGER")  // If not on "TRIGGER", try one step back
  {
    ...
    tokStr = NAString(&pInputStr[triggerTokInfo.tokenStrPos], triggerTokInfo.tokenStrLen);
    tokStr.toUpper();
  ...
  if (createTokStr EQU "REPLACE") {
    prevTokIndex = triggerTokIndex - 3;
    ComASSERT(ParScannedTokens->isQueueIndexWithinRange(prevTokIndex));
    const ParScannedTokenQueue::scannedTokenInfo &prevTokInfo1 = ParScannedTokens->getScannedTokenInfo(prevTokIndex);
    NAString prevTokStr1(&pInputStr[prevTokInfo1.tokenStrPos], prevTokInfo1.tokenStrLen);
    ComASSERT(IsNAStringSpace(prevTokStr1));

    createTokIndex = triggerTokIndex - 4;
    ComASSERT(ParScannedTokens->isQueueIndexWithinRange(createTokIndex));
    const ParScannedTokenQueue::scannedTokenInfo &createTokInfo1 =
        ParScannedTokens->getScannedTokenInfo(createTokIndex);
    NAString createTokStr1(&pInputStr[createTokInfo1.tokenStrPos], createTokInfo1.tokenStrLen);

    createTokStr1.toUpper();
    ComASSERT(createTokStr1 EQU "OR");

    prevTokIndex = triggerTokIndex - 5;
    ComASSERT(ParScannedTokens->isQueueIndexWithinRange(prevTokIndex));
    const ParScannedTokenQueue::scannedTokenInfo &prevTokInfo2 = ParScannedTokens->getScannedTokenInfo(prevTokIndex);
    NAString prevTokStr2(&pInputStr[prevTokInfo2.tokenStrPos], prevTokInfo2.tokenStrLen);
    ComASSERT(IsNAStringSpace(prevTokStr2));

    createTokIndex = triggerTokIndex - 6;
    ComASSERT(ParScannedTokens->isQueueIndexWithinRange(createTokIndex));
    const ParScannedTokenQueue::scannedTokenInfo &createTokInfo2 =
        ParScannedTokens->getScannedTokenInfo(createTokIndex);
    NAString createTokStr2(&pInputStr[createTokInfo2.tokenStrPos], createTokInfo2.tokenStrLen);

    createTokStr2.toUpper();
    ComASSERT(createTokStr2 EQU "CREATE");
    pNameLocList->setTextStartPosition(createTokInfo2.tokenStrPos);
  } else {
...
void ParSetEndOfOptionalColumnListPos(ParNameLocList *pNameLocList) {
  ...
  if (tokStr NEQ ")")  // Just in case: If not on ")"  --->>> try one step back
  {
    ...
    const ParScannedTokenQueue::scannedTokenInfo &MVTokInfo = ParScannedTokens->getScannedTokenInfo(MVTokIndex);
    tokStr = NAString(&pInputStr[MVTokInfo.tokenStrPos], MVTokInfo.tokenStrLen);
    tokStr.toUpper();
...
void ParSetBeginOfMVQueryPos(ParNameLocList *pNameLocList) {
  ComASSERT(pNameLocList EQU ParNameLocListPtr AND NULL NEQ pNameLocList);
...
void ParSetTextStartPosForCreateMV(ParNameLocList *pNameLocList) {
  ...
  if (ghostTokStr EQU "GHOST") {
    ...
    ComASSERT(ParScannedTokens->isQueueIndexWithinRange(prevTokIndex));
    const ParScannedTokenQueue::scannedTokenInfo &prevTokInfo = ParScannedTokens->getScannedTokenInfo(prevTokIndex);
    prevTokStr = NAString(&pInputStr[prevTokInfo.tokenStrPos], prevTokInfo.tokenStrLen);
    ComASSERT(IsNAStringSpace(prevTokStr));
    //
...
void ParSetBeginOfCreateTableAsQueryPos(ParNameLocList *pNameLocList) {
  ComASSERT(pNameLocList EQU ParNameCTLocListPtr AND NULL NEQ pNameLocList);
  ...
  if (space != "(") ComASSERT(IsNAStringSpace(space));
...
//----------------------------------------------------------------------------
// Keep (in a global var) the position of the with clause of
// a create table as stmt.
...
NABoolean ParSetTextEndPos(StmtDDLCreateTable *pCreateCTParseNode) {
  pCreateCTParseNode->setStartOfCreateTableQueryPosition(ParBeginingOfCreateTableQueryPos);
...
void ParSetTextStartPosForCheckConstraint(ParNameLocList *pNameLocList) {
  ...
  if (tokStr NEQ "(") {
    ...
    tokStr = "";
    tokStr.append(&pInputStr[leftParenTokInfo.tokenStrPos], leftParenTokInfo.tokenStrLen);
    TrimNAStringSpace(tokStr);
...
void ParSetTextStartPosForDivisionByClause(ParNameLocList *pNameLocList) {
  ComASSERT(pNameLocList EQU ParNameDivByLocListPtr AND pNameLocList NEQ NULL);
  ...
  TrimNAStringSpace(tokStr);
  tokStr.toUpper();
  while (ParScannedTokens->isQueueIndexWithinRange(tokIndex) AND tokStr NEQ "BY" AND tokStr NEQ "USING") {
    if (tokStr EQU "(") {
  ...
  PARSERASSERT(ParScannedTokens->isQueueIndexWithinRange(tokIndex) AND(tokStr EQU "BY" OR tokStr EQU "USING")
  ...
  {                        //                  ^...we_are_here...
    ...
    tokIndex--;
    if (ParScannedTokens->isQueueIndexOutOfRange(tokIndex)) return;
    pTokInfo = ParScannedTokens->getScannedTokenInfoPtr(tokIndex);
    tokStr.replace(0, tokStr.length(), &pInputStr[pTokInfo->tokenStrPos], pTokInfo->tokenStrLen);
    if (IsNAStringSpace(tokStr)) {
...
void ParSetTextStartPosForDisplayExplain(ParNameLocList *pNameLocList) {
  ...
  if ((c != 0) && (tokStr.data()[0] NEQ '\'')) {
    ...
    const ParScannedTokenQueue::scannedTokenInfo &leftParenTokInfo =
        ParScannedTokens->getScannedTokenInfo(leftParenTokIndex);
    tokStr = NAString(&pInputStr[leftParenTokInfo.tokenStrPos], leftParenTokInfo.tokenStrLen);
...
NABoolean ParGetTextStartEndPosForDisplayExplain(ParNameLocList *pNameLocList, StringPos &start, StringPos &end) {
  const char *pInputStr = pNameLocList->getInputStringPtr();
  ComASSERT(pInputStr NEQ NULL);
  ...
  if ((NOT IsNAStringSpace(tokStr)) && (tokStr NEQ ")") && (tokStr NEQ ";")) {
    tokInfo = ParScannedTokens->getScannedTokenInfoPtr(-1);
    tokStr = NAString(&pInputStr[tokInfo->tokenStrPos], tokInfo->tokenStrLen);

    if ((NOT IsNAStringSpace(tokStr)) && (tokStr NEQ ")") && (tokStr NEQ ";")) return TRUE;
  }
...
void ParUpdateNameLocForDotStar(const ColRefName *pColRefName) {
  ...
  const ParScannedTokenQueue::scannedTokenInfo &tokInfo = ParScannedTokens->getScannedTokenInfo(0 /*token index*/);
  if (tokInfo.tokenStrLen EQU(size_t) 2 AND NAString(&pInputStr[tokInfo.tokenStrPos], (size_t)2) EQU ".*") {
    ...
    ComASSERT(prevTokInfo.tokenStrLen EQU(size_t) 2 AND NAString(&pInputStr[prevTokInfo.tokenStrPos], (size_t)2) EQU
              ".*");
    dotStarPos = prevTokInfo.tokenStrPos;
  }

  ParNameLoc *pNameLoc = ParNameLocListPtr->getNameLocPtr(pColRefName->getCorrNameObj().getNamePosition());
...
ParNameLoc &ParNameLoc::operator=(const ParNameLoc &rhs) {
  if (this EQU & rhs) {
    return *this;
  }
  ...
  noneExpandedName_ = rhs.noneExpandedName_;

  return *this;
}

const NABoolean ParNameLoc::operator==(const ParNameLoc &rhs) const {
  if (this EQU & rhs) {
    return TRUE;
  }

  return (namePos_ EQU rhs.namePos_ AND nameLen_ EQU rhs.nameLen_ AND expandedName_ EQU rhs.expandedName_);
}
...
ParNameLocList::ParNameLocList(const ParNameLocList &rhs, CollHeap *heap) : LIST(ParNameLoc *)(heap), heap_(heap) {
  copy(rhs);
}
...
ParNameLocList &ParNameLocList::operator=(const ParNameLocList &rhs) {
...
//
ParNameLoc *ParNameLocList::getNameLocPtr(const StringPos namePos) {
  for (CollIndex i = 0; i < entries(); i++) {
...
void ParNameLocList::copy(const ParNameLocList &rhs) {
  if (this EQU & rhs) return;
  ...
  for (CollIndex i = 0; i < rhs.entries(); i++) {  // deep copy
    insert(rhs[i]);                                // insert() allocates space from heap_
  }
}

NABoolean ParNameLocList::insert(const ParNameLoc &nameLoc) {
...
// (such as tables).
NABoolean ParNameLocList::insertInOrder(const ParNameLoc &nameLoc) {
  ParNameLoc *pNameLoc = getNameLocPtr(nameLoc.getNamePosition());
  if (pNameLoc EQU NULL)  // not found
  {
    // ok to insert
