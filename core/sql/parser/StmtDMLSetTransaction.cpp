...
void StmtDMLSetTransaction::setAttribute(ItemExpr * child)
{
  switch(child->getOperatorType())
	...
	break;
      ...
      {
	if ((beginAO() != TransMode::AO_NOT_SPECIFIED_) ||
            (commitAO() != TransMode::AO_NOT_SPECIFIED_) ||
...
TxnAutoModeItem::TxnAutoModeItem(TransMode::AutoOption beginAO, 
                                 TransMode::AutoOption commitAO, 
                                 TransMode::AutoOption savepointAO)
     : beginAO_(beginAO),
       commitAO_(commitAO),
       savepointAO_(savepointAO),
       ItemExpr(ITM_SET_TRANS_AUTO_MODE)
{
}
