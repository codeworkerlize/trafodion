...
ParDDLFileAttrsAlterTable::setFileAttr(ElemDDLFileAttr * pFileAttr)
{
  ...
  case ELM_FILE_ATTR_NAMESPACE_ELEM :
    *SqlParser_Diags << DgSqlCode(-3242)
                     << DgString0(" Cannot alter namespace attribute.");
    break;

    // cannot alter encryption
  case ELM_FILE_ATTR_ENCRYPT_ELEM :
    *SqlParser_Diags << DgSqlCode(-3242)
                     << DgString0(" Cannot alter encrypt attribute.");
    break;
  ...
  case ELM_FILE_ATTR_INCR_BACKUP_ELEM:
    ...
    incrBackupEnabled_ = 
      pFileAttr->castToElemDDLFileAttrIncrBackup()->incrBackupEnabled();
  ...
  case ELM_FILE_ATTR_READ_ONLY_ELEM:
    ...
    readOnlyEnabled_ = 
      pFileAttr->castToElemDDLFileAttrReadOnly()->readOnlyEnabled();
...
ParDDLFileAttrsCreateIndex::setFileAttr(ElemDDLFileAttr * pFileAttr)
{
 ...
 case ELM_FILE_ATTR_COMPRESSION_ELEM :
  ...
  default :
    //NAAbort("ParDDLFileAttrs.C", __LINE__, "internal logic error");
    *SqlParser_Diags << DgSqlCode(-3242)
                     << DgString0("Unsupported attribute in create index.");
    break;
  }
    ...
    if (isStoredDescSpec_)
      {
        // Duplicate
        *SqlParser_Diags << DgSqlCode(-3183)
                         << DgString0("generate stored descriptor");
      }
    isStoredDescSpec_ = TRUE;
    ComASSERT(pFileAttr->castToElemDDLFileAttrStoredDesc() NEQ NULL);
    {
      ElemDDLFileAttrStoredDesc * pStoredDesc =
        pFileAttr->castToElemDDLFileAttrStoredDesc();
      storedDesc_ = TRUE;
    }
    break;
