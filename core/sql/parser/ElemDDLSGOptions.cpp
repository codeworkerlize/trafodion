...
ElemDDLSGOptions::ElemDDLSGOptions(Int32 sgType,
...
ElemDDLSGOptions::setSGOpt(ElemDDLNode * pSGOpt)
{
  ...
  case ELM_SG_OPT_RESTART_VALUE_ELEM :
    ...
    startValue_ =
      pSGOpt->castToElemDDLSGOptionRestartValue()->getValue();
    break;
  ...
  if (queryType == 1) // alter
    {
      if ((isMinValueSpecified()|| isStartValueSpecified() || isOrderSpecified()))
        {
          *CmpCommon::diags() << DgSqlCode(-1592)
                              << (isMinValueSpecified() ? DgString0("MINVALUE") :
                                  (isStartValueSpecified() ? DgString0("START WITH") :
                                   DgString0("ORDER or NOORDER")))
                              << DgString1(queryTypeStr);
  ...
  else
    {
      if (isResetSpecified() || isRestartValueSpecified())
        {
          *CmpCommon::diags() << DgSqlCode(-1592)
                              << (isResetSpecified() ? DgString0("RESET") : DgString0("RESTART WITH"))
                              << DgString1(queryTypeStr);
  ...
  if (isSystemSpecified())
    {
      ...
      if (isStartValueSpecified() ||  isRestartValueSpecified() || isIncrementSpecified() || isMinValueSpecified() || isCycleSpecified() || isCacheSpecified() || isResetSpecified())
        {
  *CmpCommon::diags() << DgSqlCode(-1592)
                      <<  DgString0("Other Sequence Generator  options")
