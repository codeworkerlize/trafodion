  ...
  case ELM_WITH_SCOPE_OPTION_ELEM :
    {
      ...
      if (isPartLocalIndex_ && isPartGlobalIndex_)
        *SqlParser_Diags << DgSqlCode(-3183) << DgString0("SCOPE");
...
StmtDDLCreateIndex::synthesize()
{
  ...
  if (getChild(INDEX_ADDNL_COLUMN_REF_LIST))
    pAddnlColumnList =
      getChild(INDEX_ADDNL_COLUMN_REF_LIST)->castToElemDDLNode();
  ...
  if (NOT fileAttrs.isStorageTypeSpecified())
    {
      ...
      ((ParDDLFileAttrsCreateIndex &)fileAttrs).setStorageType
...
StmtDDLCreateRoutine::StmtDDLCreateRoutine(const QualifiedName & aRoutineName,
        ...
        : StmtDDLNode(DDL_CREATE_ROUTINE),
          ...
          userVersion_(" ", heap),//the value of userVersion_ will be inserted into table "_MD_".ROUTINES, and the column(USER_VERSION) can't be NULL, so here can't use ''(NULL)
...
StmtDDLCreateRoutine::synthesize()
{
  ...
  if (getLanguageType() == COM_LANGUAGE_SQL)
  {
    ...
    NAString *lib = new (PARSERHEAP())
      NAString(SEABASE_SPSQL_LIBRARY, PARSERHEAP());
    NAString *sch = new (PARSERHEAP())
      NAString(SEABASE_MD_SCHEMA, PARSERHEAP());
    NAString *cat = new (PARSERHEAP())
      NAString("TRAFODION", PARSERHEAP());
    QualifiedName *qn = new (PARSERHEAP()) QualifiedName(*lib, *sch, *cat, PARSERHEAP());
    setRoutineOption(new (PARSERHEAP())
		     ElemDDLUdrLibrary(*qn));
  }
...
: StmtDDLNode(DDL_CREATE_SCHEMA),
  ...
  encryptRowid_(FALSE), encryptData_(FALSE),
  storedDesc_(storedDesc), incrBackupEnabled_(incrBackupEnabled)
{
}
...
StmtDDLCreateSchema::synthesize()
{
  if (NOT encryptionOptions_.isNull())
    {
      if (NOT ComEncryption::validateEncryptionOptions(&encryptionOptions_,
                                                       encryptRowid_, encryptData_))
        {
          *SqlParser_Diags << DgSqlCode(-3066)
                           << DgString0("Invalid encryption options specified. Valid options must be 'r', 'd' or a combination of them.");
        }
    }
...
StmtDDLCreateTable::synthesize()
{
  ...
  if (pCreateTableAttrList NEQ NULL)
      ...
      if (pCreateTableAttrList->castToElemDDLOptionList() NEQ NULL)
	{
	  for (CollIndex i = 0; i < pCreateTableAttrList->entries(); i++)
		  ...
		  NEQ NULL)
		{
          ...
          else if (pTableOption->castToElemDDLPartitionClauseV2()
          NEQ NULL)
        {
          pbcV2 =
            pTableOption->castToElemDDLPartitionClauseV2();
        }
                  ...
                  if (isLobStorageOptionsSpecified())
                    {
                      // Error 3183 - Duplicate $0~string0 clauses were specified.
                      *SqlParser_Diags << DgSqlCode(-3183)
                                       << DgString0("LOB STORAGE OPTIONS");
                    }
                  pLobStorageOptionsParseNode_ =
                    pTableOption->castToElemDDLLobStorageOptions();
      ...
      else
	{
	  ...
	  if (pCreateTableAttrList->castToElemDDLPartitionClause() 
      ...
      else if (pCreateTableAttrList->castToElemDDLPartitionClauseV2()
          NEQ NULL)
        {
          pbcV2 =
            pCreateTableAttrList->castToElemDDLPartitionClauseV2();
        }
	    ...
	    {
	      pLobStorageOptionsParseNode_ =
		pCreateTableAttrList->castToElemDDLLobStorageOptions();
	      isLobStorageOptionsSpec_ = TRUE;
	    }
	}
...
void StmtDDLPartition_visitSubpartitionElement(ElemDDLNode * pPartition,
                                               CollIndex /* index */,
                                               ElemDDLNode *pElement)
{
...
//
void
StmtDDLCreateTable_visitPartitionV2Element(ElemDDLNode * pCreateTableNode,
                                         CollIndex /* index */,
                                         ElemDDLNode * pElement)
{
  ComASSERT(pCreateTableNode NEQ NULL  AND  pElement NEQ NULL);
  ...
  if (pPartition->hasSubpartition())
  {
    ...
    pSubpartition->traverseList(pPartition,
                                StmtDDLPartition_visitSubpartitionElement);
  }
...
void
StmtDDLCreateTable::setPartitionV2ColRef(ElemDDLPartitionClauseV2 * pPartitionClauseV2)
{
  ...
  ComASSERT(partitionColList NEQ NULL AND
            partitionColList->castToElemDDLPartitionByColumnList() NEQ NULL);

  ElemDDLNode * pPartitionByCols = partitionColList->castToElemDDLPartitionByColumnList()
                                                   ->getPartitionKeyColumnList();
  for (CollIndex index = 0; index < pPartitionByCols->entries(); ++index)
  {
    ComASSERT((*pPartitionByCols)[index]->castToElemDDLColRef() NEQ NULL);
    if(isSubClause)
...
void
StmtDDLCreateTable::setPartitionsV2(ElemDDLPartitionClauseV2 * pPartitionClauseV2)
{
  ...
  {
    ElemDDLPartitionClauseV2 *subPartitionClause = pPartitionClauseV2
             ->getSubpartitionClause()->castToElemDDLPartitionClauseV2();
