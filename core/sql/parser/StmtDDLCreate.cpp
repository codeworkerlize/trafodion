...
static const literalAndEnumStruct ComUudfParamKindXlateArray[] = {
...
static void ComGetUudfParamKindAsLit(const ComUudfParamKind pv_enumValue  // in
                                     ,
                                     NAString &pr_literal  // out
) {
  NABoolean lv_found;
  char la_lit[100];
  enumToLiteral(ComUudfParamKindXlateArray, occurs(ComUudfParamKindXlateArray), pv_enumValue, la_lit, lv_found);
  ComASSERT(lv_found);
  pr_literal = la_lit;
}
...
NABoolean ParIsTracingViewUsages() { return (getenv("SQLMX_TRACE_VIEW_USAGES") NEQ NULL); }
...
ParViewTableColsUsage::ParViewTableColsUsage(const ColRefName &colName, CollHeap *h)
    : tableName_(colName.getCorrNameObj().getQualifiedNameObj(), h), columnName_(colName.getColName(), h) {}
...
NABoolean ParViewTableColsUsage::operator==(const ParViewTableColsUsage &rhs) const {
  if (this EQU & rhs) {
    return TRUE;
  }
  return (getColumnName() EQU rhs.getColumnName() AND getTableQualName() EQU rhs.getTableQualName());
}
...
ParViewTableColsUsageList::ParViewTableColsUsageList(CollHeap *heap)
    : LIST(ParViewTableColsUsage *)(heap), heap_(heap) {}
...
ParViewTableColsUsage *const ParViewTableColsUsageList::find(const ColRefName &colName) {
  for (CollIndex i = 0; i < entries(); i++) {
    if (operator[](i)
...
NABoolean ParViewTableColsUsageList::insert(const ColRefName &colName) {
  ...
  if (pCu EQU NULL)  // not found
  {
    ...
    if (ParIsTracingViewUsages()) {
      NAString traceStr = "v-tcu: " + colName.getCorrNameObj().getQualifiedNameObj().getQualifiedNameAsAnsiString() +
...
ParViewColTablesUsage::ParViewColTablesUsage(const CollIndex usingViewColumnNumber,
                                             const ExtendedQualName &usedObjectName, CollHeap *h)
    : usingViewColumnNumber_(usingViewColumnNumber), usedObjectName_(usedObjectName, h) {}
...
NABoolean ParViewColTablesUsage::operator==(const ParViewColTablesUsage &rhs) const {
  if (this EQU & rhs) {
    return TRUE;
  }
  return (getUsingViewColumnNumber() EQU rhs.getUsingViewColumnNumber() AND getUsedObjectName()
              EQU rhs.getUsedObjectName());
}
...
ParViewColTablesUsageList::ParViewColTablesUsageList(CollHeap *heap)
    : LIST(ParViewColTablesUsage *)(heap), heap_(heap) {}
...
ParViewColTablesUsage *const ParViewColTablesUsageList::find(const CollIndex usingViewColNum) {
  for (CollIndex i = 0; i < entries(); i++) {
...
const ParViewColTablesUsage *const ParViewColTablesUsageList::find(const CollIndex usingViewColNum) const {
  for (CollIndex i = 0; i < entries(); i++) {
    if (operator[](i).getUsingViewColumnNumber() EQU usingViewColNum) {
      return &operator[](i);
    }
...
NABoolean ParViewColTablesUsageList::insert(const CollIndex usingViewColumnNumber,
                                            const ExtendedQualName &usedObjectName) {
  ParViewColTablesUsage *vctu = find(usingViewColumnNumber);
  if (vctu EQU NULL)  // not found
  {
    // ok to insert
    vctu = new (heap_) ParViewColTablesUsage(usingViewColumnNumber, usedObjectName);
...
ParViewColTableColsUsage::ParViewColTableColsUsage(CollHeap *h) : usingViewColumnNumber_(0), usedObjectColumnName_(h) {}

ParViewColTableColsUsage::ParViewColTableColsUsage(const CollIndex usingViewColumnNumber,
                                                   const ColRefName &usedObjectColumnName, CollHeap *h)
    : usingViewColumnNumber_(usingViewColumnNumber), usedObjectColumnName_(usedObjectColumnName, h) {}
...
NABoolean ParViewColTableColsUsage::operator==(const ParViewColTableColsUsage &rhs) const {
  if (this EQU & rhs) {
    return TRUE;
  }
  return (getUsingViewColumnNumber() EQU rhs.getUsingViewColumnNumber() AND getUsedObjectColumnName()
              EQU rhs.getUsedObjectColumnName());
}
...
ParViewColTableColsUsageList::ParViewColTableColsUsageList(CollHeap *heap)
    : LIST(ParViewColTableColsUsage *)(heap), heap_(heap) {}
...
ParViewColTableColsUsage *const ParViewColTableColsUsageList::find(const CollIndex usingViewColNum) {
  for (CollIndex i = 0; i < entries(); i++) {
    if (operator[](i).getUsingViewColumnNumber() EQU usingViewColNum) {
      return &operator[](i);
    }
...
ParViewColTableColsUsage *const ParViewColTableColsUsageList::find(const CollIndex usingViewColNum,
  ...
  for (CollIndex i = 0; i < entries(); i++) {
    ...
    if (u->getUsingViewColumnNumber() EQU usingViewColNum AND uColRefName.getCorrNameObj()
            .getQualifiedNameObj() EQU usedColRefName.getCorrNameObj()
            .getQualifiedNameObj() AND uColRefName.getColName() EQU usedColRefName.getColName()) {
      return u;
    }
  }
  return NULL;
...
const CollIndex ParViewColTableColsUsageList::getIndex(const CollIndex usingViewColNum) const {
  for (CollIndex i = 0; i < entries(); i++) {
    if (operator[](i).getUsingViewColumnNumber() EQU usingViewColNum) {
      return i;
    }
...
const CollIndex ParViewColTableColsUsageList::getIndexOfNextElem(const CollIndex usingViewColNum) const {
  for (CollIndex i = 0; i < entries(); i++) {
...
void ParViewColTableColsUsageList::clear() {
...
NABoolean ParViewColTableColsUsageList::insert(const CollIndex usingViewColumnNumber,
  ...
  if (vctcu EQU NULL OR  // not found
          find(usingViewColumnNumber, usedObjectColumnName) EQU NULL) {
    // ok to insert
    ParViewColTableColsUsage *u = new (heap_) ParViewColTableColsUsage(usingViewColumnNumber, usedObjectColumnName);
    CMPASSERT(u NEQ NULL);
    if (vctcu NEQ NULL)  // more than 1 elem. with same usingViewColumnNumber
    {
      LIST(ParViewColTableColsUsage *)::insertAt(getIndex(usingViewColumnNumber), u);
    } else {
...
const ParViewColTableColsUsageList &ParViewUsages::getViewColTableColsUsageList() const {
  return viewColTableColsUsageList_;
}

ParViewColTableColsUsageList &ParViewUsages::getViewColTableColsUsageList() { return viewColTableColsUsageList_; }
...
StmtDDLCreateSynonym::StmtDDLCreateSynonym(const QualifiedName &synonymName, const QualifiedName &objectReference,
                                           ElemDDLNode *pOwner)
...
StmtDDLCreateSynonym *StmtDDLCreateSynonym::castToStmtDDLCreateSynonym() { return this; }
...
const NAString StmtDDLCreateSynonym::displayLabel1() const { return NAString("Synonym name: ") + getSynonymName(); }

const NAString StmtDDLCreateSynonym::displayLabel2() const {
  return NAString("Object Reference: ") + getObjectReference();
}

const NAString StmtDDLCreateSynonym::getText() const { return "StmtDDLCreateSynonym"; }
...
StmtDDLCreateExceptionTable *StmtDDLCreateExceptionTable::castToStmtDDLCreateExceptionTable() { return this; }

//
// for tracing
//

const NAString StmtDDLCreateExceptionTable::displayLabel1() const {
  return NAString("Exception table name: ") + getExceptionName();
}

const NAString StmtDDLCreateExceptionTable::displayLabel2() const {
  return NAString("Object Reference: ") + getObjectReference();
}

const NAString StmtDDLCreateExceptionTable::getText() const { return "StmtDDLCreateExceptionTable"; }
...
// constructor
//
StmtDDLCreateCatalog::StmtDDLCreateCatalog(const NAString &aCatalogName, ElemDDLNode *pAttributeList)
...
// cast
//
StmtDDLCreateCatalog *StmtDDLCreateCatalog::castToStmtDDLCreateCatalog() { return this; }
...
//
void StmtDDLCreateCatalog::setAttribute(ElemDDLNode *pAttrNode) {
  ...
  if (pAttrNode->castToElemDDLLocation() NEQ NULL) {
    ...
    if (NOT pLocation->getPartitionName().isNull()) *SqlParser_Diags << DgSqlCode(-3405);
...
const NAString StmtDDLCreateCatalog::displayLabel1() const { return NAString("Catalog name: ") + getCatalogName(); }

const NAString StmtDDLCreateCatalog::displayLabel2() const {
...
const NAString StmtDDLCreateCatalog::displayLabel3() const {
  if (NOT getLocationName().isNull()) {
...
const NAString StmtDDLCreateCatalog::getText() const { return "StmtDDLCreateCatalog"; }
...
StmtDDLCreateComponentPrivilege *StmtDDLCreateComponentPrivilege::castToStmtDDLCreateComponentPrivilege() {
  return this;
}
...
const NAString StmtDDLCreateComponentPrivilege::displayLabel1() const {
...
const NAString StmtDDLCreateComponentPrivilege::displayLabel2() const {
...
// Constructor
StmtDDLCreateTrigger::StmtDDLCreateTrigger(const QualifiedName &aTriggerName, ParNameLocList *nameLocList,
                                           NABoolean isAfter, NABoolean hasRowOrTableInRefClause,
                                           ComOperation iud_event, ElemDDLNode *columnList,
                                           const QualifiedName &aTableName, NAString *oldName, NAString *newName,
                                           NABoolean isStatement, ElemDDLNode *pOwner, CollHeap *heap)
...
//
StmtDDLCreateTrigger *StmtDDLCreateTrigger::castToStmtDDLCreateTrigger() { return this; }
...
//
void StmtDDLCreateTrigger::synthesize() {
...
const NAString StmtDDLCreateTrigger::displayLabel1() const { return NAString("Trigger name: ") + getTriggerName(); }

const NAString StmtDDLCreateTrigger::displayLabel2() const { return NAString("Table name: ") + getTableName(); }

NATraceList StmtDDLCreateTrigger::getDetailInfo() const {
  NAString detailText;
...
const NAString StmtDDLCreateTrigger::getText() const { return "StmtDDLCreateTrigger"; }
...
// constructor
//
StmtDDLCreateIndex::StmtDDLCreateIndex(NABoolean isUnique, const NAString &anIndexName, const QualifiedName &aTableName,
                                       ElemDDLNode *pColumnList, ElemDDLNode *pOptionList,
                                       ElemDDLNode *pAddnlColumnList, CollHeap *heap)
    : StmtDDLNode(DDL_CREATE_INDEX),
      ...
      addnlColumnRefArray_(heap),
      isPopulated_(TRUE),  // default is always populated.
      isPartLocalIndex_(FALSE),
      isPartGlobalIndex_(FALSE),
  ...
  setChild(INDEX_ADDNL_COLUMN_REF_LIST, pAddnlColumnList);
}
...
// cast virtual function
//
StmtDDLCreateIndex *StmtDDLCreateIndex::castToStmtDDLCreateIndex() { return this; }
...
const NAString StmtDDLCreateIndex::getTableName() const { return tableQualName_.getQualifiedNameAsAnsiString(); }
...
//
void StmtDDLCreateIndex::setFileAttributes(ElemDDLFileAttrClause *pFileAttrClause) {
...
void StmtDDLCreateIndex::setIndexOption(ElemDDLNode *pIndexOption) {
  switch (pIndexOption->getOperatorType()) {
    ...
    case ELM_WITH_SCOPE_OPTION_ELEM: {
      ElemDDLIndexScopeOption *indexscope = pIndexOption->castToElemDDLIndexScopeOption();
      ComASSERT(indexscope NEQ NULL);
      setIsPartition(TRUE);
      isPartLocalIndex_ = indexscope->getIndexLocalOption();
      isPartGlobalIndex_ = indexscope->getIndexGlobalOption();
      if (isPartLocalIndex_ && isPartGlobalIndex_) *SqlParser_Diags << DgSqlCode(-3183) << DgString0("SCOPE");

      if (isPartLocalIndex_)
        setPartitionIndexType(PARTITION_INDEX_LOCAL);
      else if (isPartGlobalIndex_)
        setPartitionIndexType(PARTITION_INDEX_GLOBAL);
      else
        setPartitionIndexType(PARTITION_INDEX_NONE);
    } break;
    ...
    case ELM_DIVISION_CLAUSE_ELEM:
      ...
      if (isDivisionClauseSpecified()) {
        ...
        *SqlParser_Diags << DgSqlCode(-3183) << DgString0("DIVISION");
      }
      pDivisionClauseParseNode_ = pIndexOption->castToElemDDLDivisionClause();
      isDivisionClauseSpec_ = TRUE;
      break;

    case ELM_PARALLEL_EXEC_ELEM: {
      ElemDDLParallelExec *pParallelExec = pIndexOption->castToElemDDLParallelExec();
    ...
    case ELM_FILE_ATTR_POS_TABLE_SIZE_ELEM: {
      if (pIndexOption->castToElemDDLFileAttrPOSTableSize()->getNumRows() != -1) {
        ...
        numRows_ = pIndexOption->castToElemDDLFileAttrPOSTableSize()->getNumRows();
    ...
    case ELM_HBASE_OPTIONS_ELEM: {
      if (isHbaseOptionsSpecified()) {
        // Error 3183 - Duplicate $0~string0 clauses were specified.
        *SqlParser_Diags << DgSqlCode(-3183) << DgString0("HBASE_OPTIONS");
      }
      pHbaseOptionsParseNode_ = pIndexOption->castToElemDDLHbaseOptions();
    ...
    case ELM_PARTITION_NAME_AND_FOR_VALUES: {
      partitionName_ = pIndexOption->castToElemDDLPartitionNameAndForValues()->getPartitionName();
      setIsPartition(TRUE);
      setPartitionIndexType(PARTITION_INDEX_NORMAL);
    } break;
    default:
      NAAbort("StmtDDLCreate.C", __LINE__, "internal logic error");
      break;
...
// to this object.
//
void StmtDDLCreateIndex::setPartitions(ElemDDLPartitionClause *pPartitionClause) {
  ...
  if (isPartitionClauseForSplit_) *SqlParser_Diags << DgSqlCode(-1208);
    ...
    if (pPartitionByOption->castToElemDDLPartitionByOpt() NEQ NULL) {
      ...
      switch (pPartitionByOption->getOperatorType()) {
        case ELM_PARTITION_BY_COLUMN_LIST_ELEM:
          ComASSERT(pPartitionByOption->castToElemDDLPartitionByColumnList() NEQ NULL);
          {
            //
            ...
            //
            ElemDDLNode *pPartitionByCols =
                pPartitionByOption->castToElemDDLPartitionByColumnList()->getPartitionKeyColumnList();
            ...
            for (CollIndex index = 0; index < pPartitionByCols->entries(); index++) {
              ...
              partitionKeyColRefArray_.insert((*pPartitionByCols)[index]->castToElemDDLColRef());
            }
          }
          break;
...
//
void StmtDDLCreateIndex::setPrimaryPartition(ElemDDLNode *pFirstSecondaryPartition) {
  if (pFirstSecondaryPartition EQU NULL)  // single partition
  {
    if (getPartitioningScheme() EQU COM_HASH_V1_PARTITIONING || getPartitioningScheme() EQU COM_HASH_V2_PARTITIONING)
...
void StmtDDLCreateIndex::setSecondaryPartition(ElemDDLPartition *pPartition) {
  ...
  switch (pPartition->getOperatorType()) {
    ...
    case ELM_PARTITION_SYSTEM_ELEM:
      ...
      if (pPartition->castToElemDDLPartitionSystem()->getOption() NEQ ElemDDLPartition::ADD_OPTION) {
...
// to the current parse node.
//
void StmtDDLCreateIndex::synthesize() {
  indexQualName_ =
      QualifiedName(getIndexName(), tableQualName_.getSchemaName(), tableQualName_.getCatalogName(), PARSERHEAP());

  if (isVolatile()) indexQualName_.setIsVolatile(TRUE);

  ComASSERT(getChild(INDEX_COLUMN_REF_LIST) NEQ NULL);

  ElemDDLNode *pColumnList = getChild(INDEX_COLUMN_REF_LIST)->castToElemDDLNode();
  ...
  if (getChild(INDEX_OPTION_LIST) NEQ NULL) {
    ElemDDLNode *pOptionList = getChild(INDEX_OPTION_LIST)->castToElemDDLNode();
  ...
  ElemDDLNode *pAddnlColumnList = NULL;
  if (getChild(INDEX_ADDNL_COLUMN_REF_LIST))
    pAddnlColumnList = getChild(INDEX_ADDNL_COLUMN_REF_LIST)->castToElemDDLNode();
  ...
  if (pAddnlColumnList) {
    nbrColList = pAddnlColumnList->entries();
    for (i = 0; i < nbrColList; i++) {
      ComASSERT((*pAddnlColumnList)[i]->castToElemDDLColRef() NEQ NULL);
      addnlColumnRefArray_.insert((*pAddnlColumnList)[i]->castToElemDDLColRef());
    }
  }
  ...
  ElemDDLPartitionSystem *pSystemPart = pPrimaryPartition_->castToElemDDLPartitionSystem();
  ...
  if (NOT fileAttrs.isStorageTypeSpecified()) {
    ...
    ((ParDDLFileAttrsCreateIndex &)fileAttrs)
        .setStorageType(st == "HBASE"
                            ? COM_STORAGE_HBASE
                            : (st == "MONARCH" ? COM_STORAGE_MONARCH
                                               : (st == "BIGTABLE" ? COM_STORAGE_BIGTABLE : COM_STORAGE_UNKNOWN)));
  }
  ...
  if (partitionArray_.entries() > 1)  // PARTITION clause specified
  {
    for (CollIndex i = 0; i < getPartitionArray().entries(); i++)
      if (getPartitionArray()[i]->castToElemDDLPartitionRange()
              EQU NULL) {  // FIRST KEY phrase(s) not specified in PARTITION clause
        ...
        if (partitioningScheme_ EQU COM_HASH_V1_PARTITIONING || partitioningScheme_ EQU COM_HASH_V2_PARTITIONING) {
...
const NAString StmtDDLCreateIndex::displayLabel1() const { return NAString("Index name: ") + getIndexName(); }

const NAString StmtDDLCreateIndex::displayLabel2() const { return NAString("Table name: ") + getTableName(); }

NATraceList StmtDDLCreateIndex::getDetailInfo() const {
  NAString detailText;
  NATraceList detailTextList;
...
const NAString StmtDDLCreateIndex::getText() const { return "StmtDDLCreateIndex"; }
...
// constructor
//
StmtDDLPopulateIndex::StmtDDLPopulateIndex(NABoolean populateAll, NABoolean populateAllUnique,
                                           NABoolean purgedataSpecified, const NAString &anIndexName,
                                           const QualifiedName &aTableName, CollHeap *heap)
    : StmtDDLNode(DDL_POPULATE_INDEX),
      populateAll_(populateAll),
      populateAllUnique_(populateAllUnique),
      purgedataSpecified_(purgedataSpecified),
      indexName_(anIndexName, heap),
      indexQualName_(heap),
      origTableQualName_(heap),
      tableQualName_(aTableName, heap) {
  populateIndexOnPartition_ = FALSE;
  partitionName_ = NAString("DUMMY", heap);
}
...
// cast virtual function
//
StmtDDLPopulateIndex *StmtDDLPopulateIndex::castToStmtDDLPopulateIndex() { return this; }
...
// to the current parse node.
//
void StmtDDLPopulateIndex::synthesize() {
  indexQualName_ =
      QualifiedName(getIndexName(), tableQualName_.getSchemaName(), tableQualName_.getCatalogName(), PARSERHEAP());
...
const NAString StmtDDLPopulateIndex::displayLabel1() const { return NAString("Index name: ") + getIndexName(); }

const NAString StmtDDLPopulateIndex::displayLabel2() const { return NAString("Table name: ") + getTableName(); }

const NAString StmtDDLPopulateIndex::getText() const { return "StmtDDLPopulateIndex"; }
...
StmtDDLCreateLibrary::StmtDDLCreateLibrary(NABoolean isSystem, const QualifiedName &libraryName,
                                           const NAString &libraryFilename, ElemDDLNode *clientName,
                                           ElemDDLNode *clientFilename, ElemDDLNode *pOwner, CollHeap *heap)
...
StmtDDLCreateLibrary *StmtDDLCreateLibrary::castToStmtDDLCreateLibrary() { return this; }
...
// to the current parse node.
//
void StmtDDLCreateLibrary::synthesize() {}

//
// for tracing
//

const NAString StmtDDLCreateLibrary::displayLabel1() const { return NAString("Library name: ") + getLibraryName(); }

const NAString StmtDDLCreateLibrary::displayLabel2() const { return NAString("Library filename: ") + getLibraryName(); }

const NAString StmtDDLCreateLibrary::getText() const { return "StmtDDLCreateLibrary"; }
...
// constructor
//
StmtDDLCreateRoutine::StmtDDLCreateRoutine(const QualifiedName &aRoutineName, const QualifiedName &anActionName,
                                           ElemDDLNode *pParamList, ElemDDLNode *pReturnsList,
                                           ElemDDLNode *pPassThroughParamList, ElemDDLNode *pOptionList,
    ...
    : StmtDDLNode(DDL_CREATE_ROUTINE),
      ...
      userVersion_(" ", heap),  // the value of userVersion_ will be inserted into table "_MD_".ROUTINES, and the
                                // column(USER_VERSION) can't be NULL, so here can't use ''(NULL)
  ...
  if (CmpCommon::getDefault(SP_DEFAULT_NO_TRANSACTION_REQUIRED) == DF_ON) {
    transactionAttributes_ = COM_NO_TRANSACTION_REQUIRED;
  }
...
// cast virtual function
//
StmtDDLCreateRoutine *StmtDDLCreateRoutine::castToStmtDDLCreateRoutine() { return this; }
...
// to the current parse node.
//
void StmtDDLCreateRoutine::synthesize() {
  ...
  if (getChild(INDEX_ROUTINE_PARAM_LIST) NEQ NULL) {
    if (getRoutineType() EQU COM_UNIVERSAL_UDF_TYPE) {
      *SqlParser_Diags << DgSqlCode(-3260);
      ...
      ElemDDLNode *pParamList = getChild(INDEX_ROUTINE_PARAM_LIST)->castToElemDDLNode();
      ...
      for (i0 = 0; i0 < nbrParamList; i0++) {
        ...
        if ((getRoutineType() == COM_PROCEDURE_TYPE)
                AND(pParamDef0->getParamDataType()->getTypeQualifier() == NA_NUMERIC_TYPE)) {
          if (((NumericType *)pParamDef0->getParamDataType())->isUnsigned()) {
            *SqlParser_Diags << DgSqlCode(-3208);
            return;
          }
        }
        if ((getRoutineType() == COM_SCALAR_UDF_TYPE) AND((pParamDef0->getParamDirection() == COM_OUTPUT_PARAM)
                                                              OR(pParamDef0->getParamDirection() == COM_INOUT_PARAM))) {
  ...
  if (getChild(INDEX_ROUTINE_RETURNS_LIST) NEQ NULL) {
    ...
    } else  // NOT an SPJ
    {
      ...
      ElemDDLNode *pReturnsList = getChild(INDEX_ROUTINE_RETURNS_LIST)->castToElemDDLNode();
      ...
      for (i2 = 0; i2 < nbrReturnsList; i2++) {
        ...
        if (NOT isOnlyOutParamModeSupportedMsgAlreadyIssued2 AND pParamDef2->getParamDirection() NEQ COM_OUTPUT_PARAM) {
  ...
  if (getChild(INDEX_ROUTINE_PASSTHROUGH_LIST) NEQ NULL) {
    ...
    ElemDDLNode *pPassThroughList = getChild(INDEX_ROUTINE_PASSTHROUGH_LIST)->castToElemDDLNode();
  ...
  if (getChild(INDEX_ROUTINE_OPTION_LIST) NEQ NULL) {
    ElemDDLNode *pOptionList = getChild(INDEX_ROUTINE_OPTION_LIST)->castToElemDDLNode();
    ComASSERT(pOptionList NEQ NULL);
    if (pOptionList->castToElemDDLOptionList() NEQ NULL) {
  ...
  // SPSQL
  if (getBody() && !languageTypeSpecified_) {
    languageType_ = COM_LANGUAGE_SQL;
    languageTypeSpecified_ = TRUE;
  }

  // SPSQL disguised in SPJ
  if (getLanguageType() == COM_LANGUAGE_SQL) {
    if (getRoutineType() == COM_PROCEDURE_TYPE) {
      languageType_ = COM_LANGUAGE_JAVA;
      externalName_ = SEABASE_SPSQL_CONTAINER "." SEABASE_SPSQL_CALL;
      // FIXME: Set max allowed result sets to 255 for SPSQL if not
      // explicitly specified so that PRINT, which dependent on
      // result sets to work properly, can be used within SPSQL
      if (!maxResultSetsSpecified_) {
        maxResults_ = 255;
        maxResultSetsSpecified_ = TRUE;
      }
    } else {
      // FIXME: For SPSQL function, it will first be disguised as a
      // C scalar function, the LANGUAGE and STYLE will be changed
      // to JAVA before writing to metadata, so that it will be
      // disguised as an SPJ when called.
      languageType_ = COM_LANGUAGE_C;
      externalName_ = SEABASE_SPSQL_CONTAINER "." SEABASE_SPSQL_CALL_FUNC;
    }
    externalNameSpecified_ = TRUE;

    NAString *lib = new (PARSERHEAP()) NAString(SEABASE_SPSQL_LIBRARY, PARSERHEAP());
    NAString *sch = new (PARSERHEAP()) NAString(SEABASE_MD_SCHEMA, PARSERHEAP());
    NAString *cat = new (PARSERHEAP()) NAString("TRAFODION", PARSERHEAP());
    QualifiedName *qn = new (PARSERHEAP()) QualifiedName(*lib, *sch, *cat, PARSERHEAP());
    setRoutineOption(new (PARSERHEAP()) ElemDDLUdrLibrary(*qn));
  }
  ...
  if (NOT isStmtDDLAlterRoutineParseNode() AND NOT externalNameSpecified_ AND getRoutineType()
          NEQ COM_ACTION_UDF_TYPE) {
  ...
  if (getRoutineType() == COM_PROCEDURE_TYPE) {
    ...
    if ((javaMethodName_.length() < 1) || (javaClassName_.length() < 1)) {
      *SqlParser_Diags << DgSqlCode(-3204);  // Badly formed
      return;
    }
  ...
  if (getRoutineType() NEQ COM_PROCEDURE_TYPE)  // is a function or routine action
  {
    ...
    if (NOT isStmtDDLAlterRoutineParseNode()  // registration of a routine
      ...
      ElemDDLNode *pUudfParamKindListParseTree = getChild(INDEX_ROUTINE_UUDF_PARAM_KIND_LIST)->castToElemDDLNode();

      if (pUudfParamKindListParseTree NEQ NULL) {
        for (CollIndex i7 = 0; i7 < pUudfParamKindListParseTree->entries(); i7++) {
          ElemDDLUudfParamDef *pUudfParamKind = (*pUudfParamKindListParseTree)[i7]->castToElemDDLUudfParamDef();
          ...
          switch (uudfParamKind) {
            ...
            case COM_UUDF_PARAM_ACTION:
              if (actionPositionSpecified_) {
                if (NOT isErrorFound) {
                  ...
                  *SqlParser_Diags << DgSqlCode(-3183) << DgString0("ACTION");
                  isErrorFound = TRUE;
                }
              }
            ...
            default: {
              for (CollIndex i9 = 0; i9 < uudfParamKindList_.entries(); i9++) {
                if (NOT isErrorFound AND uudfParamKindList_[i9] EQU uudfParamKind) {
                  ...
                  *SqlParser_Diags << DgSqlCode(-3183) << DgString0(paramKindName.data());
                  isErrorFound = TRUE;
                }
              }
              if (uudfParamKind EQU COM_UUDF_PARAM_SAS_FORMAT) {
                if (NOT isErrorFound AND actionPositionSpecified_) {
        ...
        if (NOT isErrorFound) {
          ...
          for (CollIndex i2 = 0; i2 < uudfParamKindList_.entries(); i2++) {
            ...
            pParamDef2 = new (STMTHEAP) ElemDDLParamDef(pParamDataType2, pParamName2  // optional_param_name
                                                        ,
                                                        COM_INPUT_PARAM, STMTHEAP);
            paramArray_.insert(pParamDef2);
          }  // for (CollIndex i2 = 0; ...
        }    // if no error found
      }      // if (pUudfParamKindListParseTree NEQ NULL)
        ...
        switch (paramStyle_) {
          case COM_STYLE_JAVA_CALL:
            *SqlParser_Diags << DgSqlCode(-3270);
            ...
            break;
          case COM_STYLE_SQL:
            if (getRoutineType() EQU COM_TABLE_UDF_TYPE OR getRoutineType() EQU COM_ACTION_UDF_TYPE OR getRoutineType()
                    EQU COM_UNIVERSAL_UDF_TYPE) {
    ...
    if (getRoutineType() EQU COM_TABLE_UDF_TYPE && (finalCallSpecified_ OR stateAreaSizeSpecified_)) {
    ...
    if (numberOfUniqueOutputValuesSpecified_ AND NOT isStmtDDLAlterRoutineParseNode())  // registration of a routine
    {
      ...
      if (getRoutineType() NEQ COM_UNIVERSAL_UDF_TYPE) {
        size_t numberOfOutputParams = getParamArray().entries() - getFirstReturnedParamPosWithinParamArray();
    ...
    if (isErrorFound) return;  // play it safe

  }  // if (getRoutineType() NEQ COM_PROCEDURE_TYPE)

}  // StmtDDLCreateRoutine::synthesize()

void StmtDDLCreateRoutine::setRoutineOption(ElemDDLNode *pRoutineOption) {
  ...
  switch (pRoutineOption->getOperatorType()) {
    ...
    case ELM_UDR_DETERMINISTIC: {
      if (deterministicSpecified_) {
        *SqlParser_Diags << DgSqlCode(-3183) << DgString0("[NOT] DETERMINISTIC");
      }

      ElemDDLUdrDeterministic *pDeterministic = pRoutineOption->castToElemDDLUdrDeterministic();
    ...
    case ELM_UDR_EXTERNAL_NAME: {
      if (externalNameSpecified_) {
        *SqlParser_Diags << DgSqlCode(-3183) << DgString0("EXTERNAL NAME");
      }

      ElemDDLUdrExternalName *pExternalName = pRoutineOption->castToElemDDLUdrExternalName();
    ...
    case ELM_UDR_EXTERNAL_PATH: {
      if (externalPathSpecified_) {
        *SqlParser_Diags << DgSqlCode(-3183) << DgString0("EXTERNAL PATH");
      }
      if (libraryNameSpecified_) {
        *SqlParser_Diags << DgSqlCode(-3183) << DgString0("LIBRARY");
      }

      ElemDDLUdrExternalPath *pExternalPath = pRoutineOption->castToElemDDLUdrExternalPath();
    ...
    case ELM_UDR_ISOLATE: {
      if (isolateSpecified_) {
        *SqlParser_Diags << DgSqlCode(-3183) << DgString0("[NO] ISOLATE");
      }
    ...
    case ELM_UDR_LANGUAGE: {
      if (languageTypeSpecified_) {
        *SqlParser_Diags << DgSqlCode(-3183) << DgString0("LANGUAGE");
      }

      ElemDDLUdrLanguage *pLanguage = pRoutineOption->castToElemDDLUdrLanguage();
    ...
    case ELM_UDR_LIBRARY: {
      ...
      ElemDDLUdrLibrary *pLibrary = pRoutineOption->castToElemDDLUdrLibrary();

      BindWA *pBindWA = new (STMTHEAP) BindWA(ActiveSchemaDB(), CmpCommon::context(), TRUE /*inDDL*/);
    ...
    case ELM_UDR_MAX_RESULTS: {
      if (maxResultSetsSpecified_) {
        *SqlParser_Diags << DgSqlCode(-3183) << DgString0("DYNAMIC RESULT SETS");
      }

      ElemDDLUdrMaxResults *pMaxResults = pRoutineOption->castToElemDDLUdrMaxResults();
    ...
    case ELM_UDR_PARAM_STYLE: {
      if (paramStyleSpecified_) {
        *SqlParser_Diags << DgSqlCode(-3183) << DgString0("PARAMETER STYLE");
      }

      ElemDDLUdrParamStyle *pParamStyle = pRoutineOption->castToElemDDLUdrParamStyle();
    ...
    case ELM_UDR_SQL_ACCESS: {
      if (sqlAccessSpecified_) {
        *SqlParser_Diags << DgSqlCode(-3183) << DgString0("SQL access mode");
      }

      ElemDDLUdrSqlAccess *pSqlAccess = pRoutineOption->castToElemDDLUdrSqlAccess();
    ...
    case ELM_UDR_TRANSACTION_ATTRIBUTES: {
      if (transactionAttributesSpecified_) {
        *SqlParser_Diags << DgSqlCode(-3183) << DgString0("transaction attributes");
      }

      ElemDDLUdrTransaction *pTransactionAttributes = pRoutineOption->castToElemDDLUdrTransaction();
      transactionAttributes_ = pTransactionAttributes->getTransactionAttributes();
    } break;

    case ELM_UDR_EXTERNAL_SECURITY: {
      if (externalSecuritySpecified_) {
        *SqlParser_Diags << DgSqlCode(-3183) << DgString0("EXTERNAL SECURITY");
      }

      ElemDDLUdrExternalSecurity *pExternalSecurity = pRoutineOption->castToElemDDLUdrExternalSecurity();
    ...
    case ELM_UDF_EXECUTION_MODE: {
      if (executionModeSpecified_) {
        ...
        *SqlParser_Diags << DgSqlCode(-3183) << DgString0("EXECUTION MODE");
      }
    ...
    case ELM_UDF_FINAL_CALL: {
      if (finalCallSpecified_) {
        *SqlParser_Diags << DgSqlCode(-3183) << DgString0("[NO] FINAL CALL");
      }
    ...
    case ELM_UDF_PARALLELISM: {
      if (parallelismSpecified_) {
        ...
        *SqlParser_Diags << DgSqlCode(-3183) << DgString0("PARALLELISM");
      }
    ...
    case ELM_UDF_SPECIAL_ATTRIBUTES: {
      if (specialAttributesSpecified_) {
        ...
        *SqlParser_Diags << DgSqlCode(-3183) << DgString0("ATTRIBUTES");
      }

      ElemDDLUdfSpecialAttributes *pUdfSpecialAttrs = pRoutineOption->castToElemDDLUdfSpecialAttributes();
    ...
    case ELM_UDF_OPTIMIZATION_HINT: {
      ...
      if (NOT udfAttrName.isNull()) {
        ...
        *SqlParser_Diags << DgSqlCode(-3183) << DgString0(udfAttrName.data());
      }
    } break;
    ...
    case ELM_UDF_VERSION_TAG: {
      if (userVersionSpecified_) {
        ...
        *SqlParser_Diags << DgSqlCode(-3183) << DgString0("VERSION TAG");
      }
...
const NAString StmtDDLCreateRoutine::displayLabel1() const { return NAString("Routine name: ") + getRoutineName(); }

const NAString StmtDDLCreateRoutine::displayLabel2() const { return NAString("Table name: ") + getRoutineName(); }

NATraceList StmtDDLCreateRoutine::getDetailInfo() const {
  NAString detailText;
  NATraceList detailTextList;
...
const NAString StmtDDLCreateRoutine::getText() const { return "StmtDDLCreateRoutine"; }
...
// initialize constructor
//
StmtDDLCreateSchema::StmtDDLCreateSchema(const ElemDDLSchemaName &aSchemaNameParseNode, ComSchemaClass schemaClass,
                                         CharType *pCharType, NAString *namespace1, NAString *encryptionOptions,
                                         NABoolean storedDesc, NABoolean incrBackupEnabled, CollHeap *heap)
    ...
    : StmtDDLNode(DDL_CREATE_SCHEMA),
      ...
      useEncryption_(encryptionOptions ? TRUE : FALSE),
      encryptionOptions_(encryptionOptions ? *encryptionOptions : NAString("")),
      encryptRowid_(FALSE),
      encryptData_(FALSE),
      storedDesc_(storedDesc),
      incrBackupEnabled_(incrBackupEnabled) {}

//
// virtual destructor
//
StmtDDLCreateSchema::~StmtDDLCreateSchema() { delete pCharType_; }

//
// cast virtual function
//
StmtDDLCreateSchema *StmtDDLCreateSchema::castToStmtDDLCreateSchema() { return this; }
...
// to the current parse node.
//
void StmtDDLCreateSchema::synthesize() {
  if (NOT encryptionOptions_.isNull()) {
...
const NAString StmtDDLCreateSchema::displayLabel1() const { return NAString("Schema name: ") + getSchemaName(); }

const NAString StmtDDLCreateSchema::displayLabel2() const {
...
const NAString StmtDDLCreateSchema::getText() const { return "StmtDDLCreateSchema"; }
...
// constructor
//
StmtDDLCreateSequence::StmtDDLCreateSequence(const QualifiedName &seqQualName, ElemDDLSGOptions *pSGOptions,
                                             ComBoolean alter, NABoolean if_not_exists, CollHeap *heap)
    : StmtDDLNode(DDL_CREATE_SEQUENCE),
      seqQualName_(seqQualName, heap),
      pSGOptions_(pSGOptions),
      alter_(alter),
      ifNotExistsSet_(if_not_exists) {}
...
// cast virtual function
//
StmtDDLCreateSequence *StmtDDLCreateSequence::castToStmtDDLCreateSequence() { return this; }
...
const NAString StmtDDLCreateSequence::displayLabel1() const {
  return NAString("Sequence name: ") + seqQualName_.getQualifiedNameAsAnsiString();
}

NATraceList StmtDDLCreateSequence::getDetailInfo() const {
  NAString detailText;
  NATraceList detailTextList;
...
const NAString StmtDDLCreateSequence::getText() const { return "StmtDDLCreateSequence"; }
...
// constructor
//
StmtDDLCreateTable::StmtDDLCreateTable(const QualifiedName &aTableQualName, ElemDDLNode *pTableDefBody,
                                       ElemDDLNode *pCreateTableAttrList, ElemDDLNode *pInsertColumnsList,
                                       RelExpr *queryExpression, CollHeap *heap)
    : StmtDDLNode(DDL_CREATE_TABLE),
      ...
      isHbaseOptionsSpec_(FALSE),
      pHbaseOptionsParseNode_(NULL),
      isLobStorageOptionsSpec_(FALSE),
      pLobStorageOptionsParseNode_(NULL),
      pSaltOptions_(NULL),
      pReplicateClause_(NULL),
      ...
      isPartitionClauseSpec_(FALSE),
      isPartitionByClauseSpec_(FALSE),
      isPartitionByClauseV2Spec_(FALSE),
      partitionType_(COM_NO_PARTITION),
      subpartitionType_(COM_NO_PARTITION),
      pPrimaryPartition_(NULL),
      isSplitByClauseSpec_(FALSE),
      columnDefArray_(heap),
      primaryKeyColRefArray_(heap),
      partitionArray_(heap),
      partitionV2Array_(heap),
      partitionKeyColRefArray_(heap),
      partitionV2ColRefArray_(heap),
      subpartitionColRefArray_(heap),
      ...
      endOfCreateTableAsAttrList_(0),
      endOfCreateTableAsWithClause_(0),
...
// cast virtual function
//
StmtDDLCreateTable *StmtDDLCreateTable::castToStmtDDLCreateTable() { return this; }
...
ComPartitioningScheme StmtDDLCreateTable::getPartitioningScheme() const {
...
//
void StmtDDLCreateTable::computeDefaultPrimaryPartition() {
  //
  ...
  //
  if (/*1a*/ (isStoreBySpecified() AND getStoreOption() EQU COM_NONDROPPABLE_PK_STORE_OPTION)OR
      /*1b*/ (NOT isStoreBySpecified() AND getIsConstraintPKSpecified() AND NOT getAddConstraintPK()->isDroppable())) {
    pPrimaryPartition_ = new (PARSERHEAP()) ElemDDLPartitionRange();
  ...
  //
  else if (NOT isStoreBySpecified() AND(NOT getIsConstraintPKSpecified() OR getAddConstraintPK()->isDroppable())) {
  ...
  //     scheme.
  //
  else if (isStoreBySpecified() AND getStoreOption() EQU COM_KEY_COLUMN_LIST_STORE_OPTION) {
    pPrimaryPartition_ = new (PARSERHEAP()) ElemDDLPartitionRange();
  }
  //
  ...
  //
  else if (isStoreBySpecified() AND getStoreOption() EQU COM_ENTRY_ORDER_STORE_OPTION) {
    pPrimaryPartition_ = new (PARSERHEAP()) ElemDDLPartitionSystem();
  }
  //
...
void StmtDDLCreateTable::setConstraint(ElemDDLNode *pElement) {
  switch (pElement->getOperatorType()) {
    ...
    case ELM_CONSTRAINT_PRIMARY_KEY_COLUMN_ELEM: {
      ...
      ElemDDLNode *pColRefList = pElement->castToElemDDLConstraintPK()->getColumnRefList();
      ComASSERT(pColRefList NEQ NULL);

      for (CollIndex index = 0; index < pColRefList->entries(); index++) {
        primaryKeyColRefArray_.insert((*pColRefList)[index]->castToElemDDLColRef());
      }
...
void StmtDDLCreateTable_visitTableDefElement(ElemDDLNode *pCreateTableNode, CollIndex /* index */,
                                             ElemDDLNode *pElement);

static NABoolean isColumnInStoreByNullable(const NAString &rName, ElemDDLStoreOptKeyColumnList *pSbkcl) {
  NABoolean bRet = FALSE;
  if (!pSbkcl) return bRet;

  if (pSbkcl->isNullableSpecified()) {
    const ElemDDLColRefArray &keyCols = pSbkcl->getKeyColumnArray();
    for (CollIndex index = 0; index < keyCols.entries(); index++) {
      ElemDDLColRef *sbkCol = (keyCols[index])->castToElemDDLColRef();
      if (sbkCol && rName == sbkCol->getColumnName()) {
        bRet = TRUE;
        break;
      }
    }
...
// to the current parse node.
//
void StmtDDLCreateTable::synthesize() {
  ...
  if (getChild(INDEX_TABLE_DEFINITION) NEQ NULL) {
    pTableDefBody = getChild(INDEX_TABLE_DEFINITION)->castToElemDDLNode();
  ...
  if (getChild(INDEX_ATTRIBUTE_LIST) NEQ NULL) {
    pCreateTableAttrList = getChild(INDEX_ATTRIBUTE_LIST)->castToElemDDLNode();
  ...
  ElemDDLPartitionClauseV2 *pbcV2 = NULL;      // new partition by implement
  ...
  if (pCreateTableAttrList NEQ NULL) {
    if (pCreateTableAttrList->castToElemDDLOptionList() NEQ NULL) {
      for (CollIndex i = 0; i < pCreateTableAttrList->entries(); i++) {
        ...
        if (pTableOption->castToElemDDLStoreOptNondroppablePK() NEQ NULL) {
          sbpk = pTableOption->castToElemDDLStoreOptNondroppablePK();
        }
        if (pTableOption->castToElemDDLPartitionClause() NEQ NULL) {
          pbc = pTableOption->castToElemDDLPartitionClause();
        } else if (pTableOption->castToElemDDLPartitionClauseV2() NEQ NULL) {
          pbcV2 = pTableOption->castToElemDDLPartitionClauseV2();
        } else if (pTableOption->castToElemDDLDivisionClause() NEQ NULL)  // The user specified the DIVISION BY clause
        {
          if (isDivisionByClauseSpec_ AND NOT isDuplicateDivisionByClauseErrorMsgAlreadyIssued) {
            // Error 3183 - Duplicate $0~string0 clauses were specified.
            *SqlParser_Diags << DgSqlCode(-3183) << DgString0("DIVISION BY");
            isDuplicateDivisionByClauseErrorMsgAlreadyIssued = TRUE;
          }
          pDivisionByClauseParseNode_ = pTableOption->castToElemDDLDivisionClause();
          ...
          pHbaseOptionsParseNode_ = pTableOption->castToElemDDLHbaseOptions();
          isHbaseOptionsSpec_ = TRUE;
        } else if (pTableOption->castToElemDDLLobStorageOptions() NEQ NULL) {
          if (isLobStorageOptionsSpecified()) {
            // Error 3183 - Duplicate $0~string0 clauses were specified.
            *SqlParser_Diags << DgSqlCode(-3183) << DgString0("LOB STORAGE OPTIONS");
          }
          pLobStorageOptionsParseNode_ = pTableOption->castToElemDDLLobStorageOptions();
          isLobStorageOptionsSpec_ = TRUE;
        }
      }
    } else {
      ...
      if (pCreateTableAttrList->castToElemDDLStoreOptNondroppablePK() NEQ NULL)
        sbpk = pCreateTableAttrList->castToElemDDLStoreOptNondroppablePK();
      if (pCreateTableAttrList->castToElemDDLPartitionClause() NEQ NULL)
        pbc = pCreateTableAttrList->castToElemDDLPartitionClause();
      else if (pCreateTableAttrList->castToElemDDLPartitionClauseV2() NEQ NULL) {
        pbcV2 = pCreateTableAttrList->castToElemDDLPartitionClauseV2();
      } else if (pCreateTableAttrList->castToElemDDLDivisionClause()
                     NEQ NULL)  // The user specified the DIVISION BY clause
      {
        pDivisionByClauseParseNode_ = pCreateTableAttrList->castToElemDDLDivisionClause();
        isDivisionByClauseSpec_ = TRUE;
      } else if (pCreateTableAttrList->castToElemDDLHbaseOptions() NEQ NULL) {
        pHbaseOptionsParseNode_ = pCreateTableAttrList->castToElemDDLHbaseOptions();
        isHbaseOptionsSpec_ = TRUE;
      } else if (pCreateTableAttrList->castToElemDDLLobStorageOptions() NEQ NULL) {
        pLobStorageOptionsParseNode_ = pCreateTableAttrList->castToElemDDLLobStorageOptions();
        isLobStorageOptionsSpec_ = TRUE;
      }
    }
  ...
  if ((CmpCommon::getDefault(TRAF_MAKE_PKEY_COLUMNS_NOT_NULL) == DF_ON) ||
      (CmpCommon::getDefault(MODE_SPECIAL_1) == DF_ON) || (isVolatile()) ||
    ...
    while (currListElem) {
      ...
      if (currElem->getOperatorType() EQU ELM_COL_DEF_ELEM) {
        ...
        if (col->getConstraintPK()) userSpecifiedPKey = TRUE;
        ...
        static const int volTabSbyHier[] = {
            ...
            REC_INT_DAY,        REC_INT_HOUR,          REC_INT_DAY_HOUR,   REC_INT_HOUR_MINUTE, REC_INT_DAY_MINUTE,
            REC_INT_YEAR,       REC_INT_MONTH,         REC_INT_YEAR_MONTH,

            REC_DATETIME,

            REC_BYTE_F_ASCII,

            REC_NUM_BIG_SIGNED, REC_NUM_BIG_UNSIGNED,

            REC_DECIMAL_LSE,    REC_DECIMAL_UNSIGNED,

            REC_BIN16_SIGNED,   REC_BIN16_UNSIGNED,    REC_BIN32_SIGNED,   REC_BIN32_UNSIGNED,  REC_BIN64_UNSIGNED,
            REC_BIN64_SIGNED};
        ...
        if ((CmpCommon::getDefault(VOLATILE_TABLE_FIND_SUITABLE_KEY) != DF_OFF) && (NOT userSpecifiedPKey) &&
          ...
          if (found) {
            if (volTabSbyCol == NULL) {
              ...
              if ((j > volTabSbyColPos) || ((j == volTabSbyColPos) && ((col->isNotNullConstraintSpecified() ==
        ...
        if (currElem->getOperatorType() EQU ELM_CONSTRAINT_PRIMARY_KEY_ELEM) {
          userSpecifiedPKey = TRUE;

          keyColsList = currElem->castToElemDDLConstraintPK()->getColumnRefList();

          nullablePKeySpecified = currElem->castToElemDDLConstraintPK()->isNullableSpecified();
        }
      }
    }  // while
    ...
    if ((isVolatile()) && (sbkcl == NULL) && (NOT userSpecifiedPKey) && (col != NULL) &&
        (CmpCommon::getDefault(VOLATILE_TABLE_FIND_SUITABLE_KEY) == DF_OFF)) {
      // make the storeby list the primary key
      ElemDDLConstraintPKColumn *pk = new (PARSERHEAP()) ElemDDLConstraintPKColumn();
      ...
      if (col->getChild(ElemDDLColDef::INDEX_ELEM_DDL_COL_ATTR_LIST))
        pColAttrList = col->getChild(ElemDDLColDef::INDEX_ELEM_DDL_COL_ATTR_LIST)->castToElemDDLNode();

      ElemDDLNode *newColAttrList = NULL;
      if (pColAttrList)
        newColAttrList = new (PARSERHEAP()) ElemDDLList(pColAttrList, pk);
      else
        newColAttrList = pk;
      col->setChild(ElemDDLColDef::INDEX_ELEM_DDL_COL_ATTR_LIST, newColAttrList);

      userSpecifiedPKey = TRUE;
    }

    if ((sbkcl) && (pTableDefBody) && (NOT userSpecifiedPKey) && (sbkcl->getKeyColumnList()->entries() > 0)) {
      keyColsList = sbkcl->getKeyColumnList();

      if (sbkcl->isUniqueStoreBy()) {
        // make the storeby list the primary key
        ElemDDLConstraintPK *pk = new (PARSERHEAP()) ElemDDLConstraintPK(NULL, sbkcl->getSerializedOption());
        pk->setColumnRefList(keyColsList);
        pk->setConstraintKind(ElemDDLConstraint::TABLE_CONSTRAINT_DEF);
        pk->setConstraintAttributes(NULL);

        pTableDefBody = new (PARSERHEAP()) ElemDDLList(pTableDefBody, pk);
      }
    }
    ...
    //    store by (<first-col>)
    //
    if ((isVolatile()) && (sbkcl == NULL) && (NOT userSpecifiedPKey) && (volTabSbyCol != NULL) && (pbc == NULL) &&
        (pbcV2 == NULL) && (CmpCommon::getDefault(VOLATILE_TABLE_FIND_SUITABLE_KEY) != DF_OFF)) {
      sbkcl = new (PARSERHEAP()) ElemDDLStoreOptKeyColumnList(
          new (PARSERHEAP()) ElemDDLColRef(volTabSbyCol->getColumnName(), COM_UNKNOWN_ORDER, PARSERHEAP()));

      if (pCreateTableAttrList) {
        pCreateTableAttrList = new (PARSERHEAP()) ElemDDLOptionList(pCreateTableAttrList, sbkcl);
      } else
    ...
    if (((NOT isVolatile()) && (CmpCommon::getDefault(TRAF_MAKE_PKEY_COLUMNS_NOT_NULL) == DF_ON) &&
         (NOT nullablePKeySpecified)) ||
        ((isVolatile()) && (CmpCommon::getDefault(VOLATILE_TABLE_FIND_SUITABLE_KEY) == DF_OFF))) {
      ...
      while (currListElem) {
        ...
        if (NOT col->isNotNullConstraintSpecified()) {
          ...
          if (col->isPrimaryKeyConstraintSpecified()) {
            ...
            if (col->getConstraintPK() && col->getConstraintPK()->isNullableSpecified()) makeThisColNNND = FALSE;

          } else if (sbkcl && sbkcl->isNullableSpecified()) {
            // If 'col' is not primary key, and is store by nullable key, make
            // 'col' nullable(set makeThisColNNND = FALSE).
            // check whether 'col' is primary key and nullable specified
            if (userSpecifiedPKey && keyColsList && !nullablePKeySpecified) {
              // create table t1(c1 int, c2 int ,primary key(c1)) store by nullable(c1)
              // In the case above, c1 should be not null.
              for (CollIndex index = 0; index < keyColsList->entries(); index++) {
                ElemDDLColRef *kCol = (*keyColsList)[index]->castToElemDDLColRef();
                if (kCol && kCol->getColumnName() == col->getColumnName()) {
                  makeThisColNNND = TRUE;
                }
              }
            }
            // Now 'col' is not primary key and makeThisColNNND is FALSE, so we need do nothing.
          ...
          if (makeThisColNNND && col->getColumnDataType()) {
            ElemDDLConstraintNotNull *nn = new (PARSERHEAP()) ElemDDLConstraintNotNull(PARSERHEAP());
            ElemDDLConstraintAttrDroppable *nd = new (PARSERHEAP()) ElemDDLConstraintAttrDroppable(FALSE);
  ...
  if (pTableDefBody) {
    if (pTableDefBody->castToElemDDLLikeCreateTable() NEQ NULL) {
      ...
      likeSourceTableCorrName_ = pTableDefBody->castToElemDDLLikeCreateTable()->getDDLLikeNameAsCorrName();
      likeOptions_ = pTableDefBody->castToElemDDLLikeCreateTable()->getLikeOptions();

      if ((NOT isExternal()) && (pTableDefBody->castToElemDDLLikeCreateTable()->forExtTable())) {
        *SqlParser_Diags << DgSqlCode(-3242)
                         << DgString0("'for' clause can only be specified when creating an 'external' table.");
        return;
      } else if ((isExternal()) && (NOT pTableDefBody->castToElemDDLLikeCreateTable()->forExtTable())) {
        *SqlParser_Diags << DgSqlCode(-3242)
      ...
      pTableDefBody->traverseList(this, StmtDDLCreateTable_visitTableDefElement);
    }
  }
  ...
  if (isVolatile() && !isDroppable()) *SqlParser_Diags << DgSqlCode(-1286);  // volatile table must be droppable
  ...
  if (getStoreOption() EQU COM_ENTRY_ORDER_STORE_OPTION AND isPartitionSpecified()) {
  ...
  **********/
  ...
  if (getStoreOption() NEQ COM_ENTRY_ORDER_STORE_OPTION AND getStoreOption() NEQ COM_RELATIVE_ORDER_STORE_OPTION) {
    ...
    if (partitionArray_.entries() > 1)  // PARTITION clause specified
    {
      for (CollIndex i = 0; i < getPartitionArray().entries(); i++) {
        ...
        {  // Neither FIRST KEY phrase nor HASH specified in PARTITION clause
          if (getIsHashV1PartitionSpecified() EQU FALSE && getIsHashV2PartitionSpecified() EQU FALSE) {
  ...
  //
  ElemDDLPartitionSystem *pSysPart = pPrimaryPartition_->castToElemDDLPartitionSystem();
  ...
  if (isStoreBySpecified() AND getStoreOption() EQU COM_NONDROPPABLE_PK_STORE_OPTION) {
    if (getIsConstraintPKSpecified()) {
      ...
      } else if (getAddConstraintPK()->isDroppableSpecifiedExplicitly()) {
  ...
  if ((isVolatile()) && ((getAddConstraintCheckArray().entries() > 0) ||  // no check constraint
  ...
  if ((isVolatile()) && (CmpCommon::getDefault(POS) != DF_OFF)) {
    ...
    if (NOT isPOSNumPartnsSpecified()) {
      if (CmpCommon::getDefault(POS_NUM_OF_TEMP_TABLE_PARTNS, 0) == DF_SYSTEM)
        ...
        posNumPartns_ = ActiveSchemaDB()->getDefaults().getAsLong(POS_NUM_OF_TEMP_TABLE_PARTNS);

        if (posNumPartns_ == 1) posNumPartns_ = 0;
  ...
  if (isPOSDiskPoolSpecified_) {
    if (posNumDiskPools_ != 0) {
      if (posDiskPool_) {
        if ((posDiskPool_ != MAX_COMSINT32) && (posDiskPool_ > posNumDiskPools_)) {
          *SqlParser_Diags << DgSqlCode(-3417) << DgInt0(posDiskPool_) << DgInt1(posNumDiskPools_);
        }
      } else {
        if (ActiveSchemaDB()->getDefaults().getAsLong(POS_DISK_POOL) > posNumDiskPools_) {
          *SqlParser_Diags << DgSqlCode(-3417) << DgInt0(ActiveSchemaDB()->getDefaults().getAsLong(POS_DISK_POOL))
                           << DgInt1(posNumDiskPools_);
        }
      }
    } else if ((ActiveSchemaDB()->getDefaults().getAsLong(POS_NUM_DISK_POOLS) > 0) &&
               ((posDiskPool_ != MAX_COMSINT32) &&
                (posDiskPool_ > ActiveSchemaDB()->getDefaults().getAsLong(POS_NUM_DISK_POOLS)))) {
      *SqlParser_Diags << DgSqlCode(-3417) << DgInt0(posDiskPool_)
                       << DgInt1(ActiveSchemaDB()->getDefaults().getAsLong(POS_NUM_DISK_POOLS));
    }
  }

  if (isPOSDiskPoolSpecified_ && isPOSNumPartnsSpecified_ && posNumPartns_ < 0) {
...
void StmtDDLCreateTable_visitTableDefElement(ElemDDLNode *pCreateTableNode, CollIndex /* index */,
                                             ElemDDLNode *pElement) {
  ComASSERT(pCreateTableNode NEQ NULL AND pCreateTableNode->castToStmtDDLCreateTable() NEQ NULL AND pElement NEQ NULL);

  StmtDDLCreateTable *pCreateTable = pCreateTableNode->castToStmtDDLCreateTable();

  if (pElement->castToElemDDLLikeCreateTable() NEQ NULL) {
    pCreateTable->likeSourceTableCorrName_ = pElement->castToElemDDLLikeCreateTable()->getDDLLikeNameAsCorrName();
    pCreateTable->likeOptions_ = pElement->castToElemDDLLikeCreateTable()->getLikeOptions();

    if ((NOT pCreateTable->isExternal()) && (pElement->castToElemDDLLikeCreateTable()->forExtTable())) {
      *SqlParser_Diags << DgSqlCode(-3242)
                       << DgString0("'for' clause can only be specified when creating an 'external' table.");
      return;
    } else if ((pCreateTable->isExternal()) && (NOT pElement->castToElemDDLLikeCreateTable()->forExtTable())) {
      *SqlParser_Diags << DgSqlCode(-3242)
                       << DgString0("'like' clause cannot be specified when creating an external table.");
      return;
    }
    ...
    if (pColDef->getColumnDataType() == NULL) {
      ...
      if (pCreateTable->getQueryExpression() == NULL) {
        *SqlParser_Diags << DgSqlCode(-1174) << DgString0(pColDef->getColumnName());
        return;
      } else {
...
//
void StmtDDLCreateTable_visitPartitionElement(ElemDDLNode *pCreateTableNode, CollIndex /* index */,
                                              ElemDDLNode *pElement) {
...
//
void StmtDDLCreateTable::setPartitions(ElemDDLPartitionClause *pPartitionClause) {
  ComASSERT(pPartitionClause NEQ NULL);

  if (pPartitionClause->getIsForSplit()) {
    if (isSplitByClauseSpec_) *SqlParser_Diags << DgSqlCode(-3103) << DgString0("SPLIT BY");
  ...
  if ((isPartitionSpecified()) || ((isPOSNumPartnsSpecified()) && (posNumPartns_ == 0))) {
    ...
    *SqlParser_Diags << DgSqlCode(-3103) << DgString0("PARTITION");
  }
  ...
  // are defined in the partition clause.
  //
  if (pPartitions NEQ NULL) pPartitions->traverseList(this, StmtDDLCreateTable_visitPartitionElement);
...
void StmtDDLPartition_visitSubpartitionElement(ElemDDLNode *pPartition, CollIndex /* index */, ElemDDLNode *pElement) {
  ComASSERT(pPartition NEQ NULL AND pElement NEQ NULL);

  ElemDDLPartitionV2 *pPartitionV2 = pPartition->castToElemDDLPartitionV2();
  ComASSERT(pPartitionV2 NEQ NULL);

  ElemDDLPartitionV2 *pSubpartition = pElement->castToElemDDLPartitionV2();
  pPartitionV2->getSubpartitionArray()->insert(pSubpartition);
}

//
// Copies the information in the specified pElement
// to the Create Table parse node pointed to by pCreateTableNode.
//
void StmtDDLCreateTable_visitPartitionV2Element(ElemDDLNode *pCreateTableNode, CollIndex /* index */,
                                                ElemDDLNode *pElement) {
  ComASSERT(pCreateTableNode NEQ NULL AND pElement NEQ NULL);
  ...
  ElemDDLPartitionV2 *pPartition = pElement->castToElemDDLPartitionV2();
  ComASSERT(pPartition NEQ NULL);

  if (pPartition->hasSubpartition()) {
    ElemDDLNode *pSubpartition = pPartition->getSubpartition();
    pSubpartition->traverseList(pPartition, StmtDDLPartition_visitSubpartitionElement);
  }
  pCreateTable->partitionV2Array_.insert(pPartition);

}  // StmtDDLCreateTable_visitPartitionV2Element()

void StmtDDLCreateTable::setPartitionV2ColRef(ElemDDLPartitionClauseV2 *pPartitionClauseV2) {
  NABoolean isSubClause = pPartitionClauseV2->isSubpartition();

  ElemDDLNode *partitionColList = pPartitionClauseV2->getPartitionColList();
  ComASSERT(partitionColList NEQ NULL AND partitionColList->castToElemDDLPartitionByColumnList() NEQ NULL);

  ElemDDLNode *pPartitionByCols = partitionColList->castToElemDDLPartitionByColumnList()->getPartitionKeyColumnList();
  for (CollIndex index = 0; index < pPartitionByCols->entries(); ++index) {
    ComASSERT((*pPartitionByCols)[index]->castToElemDDLColRef() NEQ NULL);
    if (isSubClause)
      subpartitionColRefArray_.insert((*pPartitionByCols)[index]->castToElemDDLColRef());
    else
      partitionV2ColRefArray_.insert((*pPartitionByCols)[index]->castToElemDDLColRef());
  }
}

// for new partition table implement
void StmtDDLCreateTable::setPartitionsV2(ElemDDLPartitionClauseV2 *pPartitionClauseV2) {
  ComASSERT(pPartitionClauseV2 NEQ NULL);
  isPartitionByClauseV2Spec_ = true;

  ElemDDLNode *pPartitions = pPartitionClauseV2->getPartitionDefBody();
  ComASSERT(pPartitions NEQ NULL);

  pPartitions->traverseList(this, StmtDDLCreateTable_visitPartitionV2Element);

  setPartitionV2ColRef(pPartitionClauseV2);
  setPartitionType(pPartitionClauseV2->getPartitionType());
  if (pPartitionClauseV2->getSubpartitionClause()) {
    ElemDDLPartitionClauseV2 *subPartitionClause =
        pPartitionClauseV2->getSubpartitionClause()->castToElemDDLPartitionClauseV2();
    setPartitionV2ColRef(subPartitionClause);
    setSubpartitionType(subPartitionClause->getPartitionType());
  }
}
...
// to the data member fileAttributes_.
//
void StmtDDLCreateTable::setFileAttributes(ElemDDLFileAttrClause *pFileAttrClause) {
  ComASSERT(pFileAttrClause NEQ NULL);
...
// This method is only invoked when the partition clause appears.
//
void StmtDDLCreateTable::setPrimaryPartition(ElemDDLNode *pFirstSecondaryPartition) {
  if (pFirstSecondaryPartition EQU NULL)  // single partition
  {
    if (getIsHashV1PartitionSpecified() || getIsHashV2PartitionSpecified())
      // The HASH PARTITION or HASH2 PARTITION clause was specified.
...
void StmtDDLCreateTable::setTableOption(ElemDDLNode *pTableOption) {
    ...
    switch (pTableOption->getOperatorType()) {
      ...
      case ELM_STORE_OPT_KEY_COLUMN_LIST_ELEM:
        ...
        storeOption_ = COM_KEY_COLUMN_LIST_STORE_OPTION;
        {
          ...
          // information easier.
          //
          ElemDDLNode *pKeyCols = pTableOption->castToElemDDLStoreOptKeyColumnList()->getKeyColumnList();
  ...
  switch (pTableOption->getOperatorType()) {
    ...
    case ELM_PARTITION_CLAUSE_V2_ELEM:
      // new partition table clause
      setPartitionsV2(pTableOption->castToElemDDLPartitionClauseV2());
      break;
    ...
    case ELM_FILE_ATTR_POS_NUM_PARTNS_ELEM:
      if (isPOSNumPartnsSpecified_) {
        ...
        if ((posNumPartns_ == 0) && (pTableOption->castToElemDDLFileAttrPOSNumPartns()->getPOSNumPartns() == 0)) {
      ...
      posNumPartns_ = pTableOption->castToElemDDLFileAttrPOSNumPartns()->getPOSNumPartns();
    ...
    case ELM_FILE_ATTR_POS_DISK_POOL_ELEM: {
      if (isPOSDiskPoolSpecified_) *SqlParser_Diags << DgSqlCode(-3422);

      posDiskPool_ = pTableOption->castToElemDDLFileAttrPOSDiskPool()->getPOSDiskPool();
      posNumDiskPools_ = pTableOption->castToElemDDLFileAttrPOSDiskPool()->getPOSNumDiskPools();

      isPOSDiskPoolSpecified_ = TRUE;
    } break;

    case ELM_FILE_ATTR_POS_TABLE_SIZE_ELEM: {
      if (pTableOption->castToElemDDLFileAttrPOSTableSize()->getPOSInitialTableSize() != -1) {
        if (isPOSInitialTableSizeSpecified_) {
          // Duplicate TABLE SIZE phrases.
          *SqlParser_Diags << DgSqlCode(-3407);
        }
        posInitialTableSize_ = pTableOption->castToElemDDLFileAttrPOSTableSize()->getPOSInitialTableSize();
        isPOSInitialTableSizeSpecified_ = TRUE;
      }

      if (pTableOption->castToElemDDLFileAttrPOSTableSize()->getPOSMaxTableSize() != -1) {
        if (isPOSMaxTableSizeSpecified_) {
          // Duplicate TABLE SIZE phrases.
          *SqlParser_Diags << DgSqlCode(-3407);
        }
        posMaxTableSize_ = pTableOption->castToElemDDLFileAttrPOSTableSize()->getPOSMaxTableSize();
        isPOSMaxTableSizeSpecified_ = TRUE;
      }
      ...
      if (pTableOption->castToElemDDLFileAttrPOSTableSize()->getNumRows() != -1) {
        ...
        numRows_ = pTableOption->castToElemDDLFileAttrPOSTableSize()->getNumRows();
        isNumRowsSpecified_ = TRUE;
      }
      ...
      if (pTableOption->castToElemDDLFileAttrPOSTableSize()->getIndexLevels() != -1) {
        ...
        indexLevels_ = pTableOption->castToElemDDLFileAttrPOSTableSize()->getIndexLevels();
      ...
      if (pTableOption->castToElemDDLFileAttrPOSTableSize()->getPartnEOF() != -1) {
        ...
        partnEOF_ = pTableOption->castToElemDDLFileAttrPOSTableSize()->getPartnEOF();
    ...
    case ELM_FILE_ATTR_POS_IGNORE_ELEM: {
      if (isPOSIgnoreSpecified_) *SqlParser_Diags << DgSqlCode(-3090);

      posIgnore_ = pTableOption->castToElemDDLFileAttrPOSIgnore()->getIgnorePOS();

      isPOSIgnoreSpecified_ = TRUE;
    } break;
    case ELM_DIVISION_CLAUSE_ELEM:
    case ELM_HBASE_OPTIONS_ELEM:
    case ELM_LOB_STORAGE_OPTIONS_ELEM:
      // Do nothing here.
    ...
    case ELM_REPLICATE_CLAUSE_ELEM: {
      if (pReplicateClause_)
        // Error 3183 - Duplicate $0~string0 clauses were specified.
        *SqlParser_Diags << DgSqlCode(-3183) << DgString0("REPLICATE");
      else
        pReplicateClause_ = pTableOption->castToElemDDLReplicateClause();
    } break;
    default:
...
const NAString StmtDDLCreateTable::displayLabel1() const { return NAString("Table name: ") + getTableName(); }

NATraceList StmtDDLCreateTable::getDetailInfo() const {
  NAString detailText;
  NATraceList detailTextList;
  ...
  const StmtDDLAddConstraintCheckArray &checkConstraintList = getAddConstraintCheckArray();
  ...
  const StmtDDLAddConstraintRIArray &rIConstraintList = getAddConstraintRIArray();
  ...
  const StmtDDLAddConstraintUniqueArray &uniqueConstraintList = getAddConstraintUniqueArray();
...
const NAString StmtDDLCreateTable::getText() const { return "StmtDDLCreateTable"; }
...
// constructor
//
StmtDDLCreateHbaseTable::StmtDDLCreateHbaseTable(const QualifiedName &aTableQualName, ConstStringList *csl,
                                                 ElemDDLHbaseOptions *hbaseOptions, CollHeap *heap)
...
// cast virtual function
//
StmtDDLCreateHbaseTable *StmtDDLCreateHbaseTable::castToStmtDDLCreateHbaseTable() { return this; }

const NAString StmtDDLCreateHbaseTable::displayLabel1() const { return NAString("Table name: ") + getTableName(); }

NATraceList StmtDDLCreateHbaseTable::getDetailInfo() const {
  NAString detailText;
  NATraceList detailTextList;
...
// constructor
//
StmtDDLCreateMV::StmtDDLCreateMV(const QualifiedName &MVQualName, const ParNameLocList &nameLocList,
                                 ElemDDLNode *optionalMVColumnList, ComMVRefreshType refreshType,
                                 ElemDDLNode *pAttributeTableLists,
                                 // ComMVStatus            mvStatus,
                                 MvInitializationType mvInitType, ComBoolean isRewriteEnabled,
                                 ElemDDLNode *optionalFileOptionsClause, RelExpr *queryExpression,
                                 ElemDDLNode *pOptionalOwner, CollHeap *heap)
  ...
  theMVInfo_ = new (heap)
      MVInfoForDDL(MVQualName_.getQualifiedNameAsAnsiString(), refreshType, isRewriteEnabled, mvStatus,
                   nameLocList.getInputStringPtr(), nameLocList.getInputStringCharSet(), pStoreByOption_, heap);
...
// cast
//
StmtDDLCreateMV *StmtDDLCreateMV::castToStmtDDLCreateMV() { return this; }
...
void StmtDDLCreateMV::incrCurViewColNum() {
...
void StmtDDLCreateMV::synthesize() {
  //
...
NABoolean StmtDDLCreateMV::statementHasAttributeTableLists() { return (NULL != pAttributeTableLists_); }

void StmtDDLCreateMV::processAttributeTableLists() {
  ...
  for (CollIndex i = 0; i < pAttributeTableLists_->entries(); i++) {
    ElemDDLCreateMVOneAttributeTableList *pAttributeList =
        (*pAttributeTableLists_)[i]->castToElemDDLCreateMVOneAttributeTableList();
...
//----------------------------------------------------------------------------
void StmtDDLCreateMV::ensureCorrectMVFileOptions() {
  if (getChild(INDEX_FILE_OPTIONS_CLAUSE) NEQ NULL) {
    ElemDDLNode *pCreateTableAttrList = getChild(INDEX_FILE_OPTIONS_CLAUSE)->castToElemDDLNode();
...
//----------------------------------------------------------------------------
void StmtDDLCreateMV::checkFileOption(ElemDDLNode *pTableOption) {
  ComASSERT(pTableOption NEQ NULL);

  // STORE BY
  if (pTableOption->castToElemDDLStoreOpt() NEQ NULL) {
    checkStoreByClause(pTableOption->castToElemDDLStoreOpt());
    pStoreByOption_ = pTableOption->castToElemDDLStoreOpt()->castToElemDDLStoreOptKeyColumnList();
  }
  // ATTRIBUTES
  ...
  // PARTITION
    ...
    checkPartitionDefinitionclause(pTableOption->castToElemDDLPartitionClause());
...
//----------------------------------------------------------------------------
void StmtDDLCreateMV::checkStoreByClause(ElemDDLStoreOpt *pTableOption) {
...
//----------------------------------------------------------------------------
void StmtDDLCreateMV::checkFileAttributeClause(ElemDDLFileAttrClause *pTableOption) {
...
//----------------------------------------------------------------------------
void StmtDDLCreateMV::checkFileAttribute(ElemDDLFileAttr *pFileAttr) {
...
//----------------------------------------------------------------------------
void StmtDDLCreateMV::checkLocationClause(ElemDDLLocation *pTableOption) {
  ...
  if (NOT pTableOption->getPartitionName().isNull()) *SqlParser_Diags << DgSqlCode(-3405);
}
...
//----------------------------------------------------------------------------
void StmtDDLCreateMV::checkPartitionDefinitionclause(ElemDDLPartitionClause *pTableOption) {
...
//----------------------------------------------------------------------------
void StmtDDLCreateMV::checkMVFileAttributeClause(ElemDDLMVFileAttrClause *pTableOption) {
...
void StmtDDLCreateMV::extractMvAttributesFromFileOptions() {
  if (FALSE == isMVAttributeClauseSpecified_ || NULL == getChild(INDEX_FILE_OPTIONS_CLAUSE)) {
    return;
  }
  ...
  if (pCreateTableAttrList->castToElemDDLOptionList() NEQ NULL) {
    for (CollIndex i = 0; i < pCreateTableAttrList->entries(); i++) {
      if (NULL != ((*pCreateTableAttrList)[i])->castToElemDDLMVFileAttrClause()) {
        pAttribs = ((*pCreateTableAttrList)[i])->castToElemDDLMVFileAttrClause();
      }
    }
  } else {
    // pCreateTableAttrList points to a single Create Table
  ...
  for (CollIndex j = 0; j < pFileAttrs->entries(); j++) {
    ElemDDLFileAttr *pAttr = (*pFileAttrs)[j]->castToElemDDLFileAttr();
    ...
    switch (pAttr->getOperatorType()) {
      case ELM_FILE_ATTR_MVAUDIT_ELEM:
        ...
        mvAuditType_ = (pAttr->castToElemDDLFileAttrMvAudit())->getMvAuditType();
      ...
      case ELM_FILE_ATTR_MV_COMMIT_EACH_ELEM:
        ...
        commitEachNRows_ = (pAttr->castToElemDDLFileAttrMVCommitEach())->getNRows();
...
const NAString StmtDDLCreateMV::displayLabel1() const { return NAString("MV name: ") + getMVName(); }

NATraceList StmtDDLCreateMV::getDetailInfo() const {
  NAString detailText;
  NATraceList detailTextList;
...
const NAString StmtDDLCreateMV::getText() const { return "StmtDDLCreateMV"; }
...
// constructor
//
StmtDDLCreateView::StmtDDLCreateView(const QualifiedName &viewQualName, const ParNameLocList &nameLocList,
                                     ElemDDLNode *optionalViewColumnList, ElemDDLNode *optionalLocationClause,
                                     RelExpr *queryExpression, ElemDDLNode *optionalWithCheckOption,
                                     ComCreateViewBehavior createViewBehavior, ElemDDLNode *pOwner, CollHeap *heap)
...
// cast
//
StmtDDLCreateView *StmtDDLCreateView::castToStmtDDLCreateView() { return this; }
...
void StmtDDLCreateView::incrCurViewColNum() {
  if (isProcessingViewColList()) {
    setCurViewColNum(getCurViewColNum() + (CollIndex)1);
  } else {
    setCurViewColNum((CollIndex)0);
...
void StmtDDLCreateView::synthesize() {
  //
  // Collects pointers to column name parser nodes for easier access
  //
  ...
  if (pLocationClause_ NEQ NULL) {
    ...
    if (NOT pLoc->getPartitionName().isNull()) *SqlParser_Diags << DgSqlCode(-3405);
  ...
  if (pWithCheckOption_ NEQ NULL) {
    ElemDDLWithCheckOption *pWCkOpt = pWithCheckOption_->castToElemDDLWithCheckOption();
...
const NAString StmtDDLCreateView::displayLabel1() const { return NAString("View name: ") + getViewName(); }

const NAString StmtDDLCreateView::displayLabel2() const {
  return NAString("is With Ck Opt spec? ") + YesNo(isWithCheckOptionSpecified());
}

const NAString StmtDDLCreateView::displayLabel3() const {
...
NATraceList StmtDDLCreateView::getDetailInfo() const {
  NAString detailText;
  NATraceList detailTextList;
...
const NAString StmtDDLCreateView::getText() const { return "StmtDDLCreateView"; }
...
StmtDDLCommentOn::StmtDDLCommentOn(COMMENT_ON_TYPES objType, const QualifiedName &objName, const NAString &commentStr,
                                   CollHeap *heap)
...
StmtDDLCommentOn *StmtDDLCommentOn::castToStmtDDLCommentOn() { return this; }
