        ...
        case ElemDDLColDefault::COL_DEFAULT:
          {
            ...
            if (pDefault_ && 
                ((pDefault_->getOperatorType() == ITM_CURRENT_USER) ||
                 (pDefault_->getOperatorType() == ITM_SESSION_USER) ||
                ...
                (! isIdentityColumn))
              {
                ...
                *SqlParser_Diags << DgSqlCode(-1186)
                     ...
                     (pDefault_->getOperatorType() == ITM_CURRENT_TIMESTAMP) &&
                     (! isIdentityColumn))
              {
                if ((pColumnDataType->getTypeQualifier() != NA_DATETIME_TYPE) ||
                  ...
                  *SqlParser_Diags << DgSqlCode(-1186)
                     ...
                     (pDefault_->getOperatorType() == ITM_CAST) &&
                     (! isIdentityColumn))
              {
                ...
                if(!pColumnDataType->isCompatible(*cvTyp))
                  {
                    *SqlParser_Diags << DgSqlCode(-1186)
                     ...
                     (pDefault_->getOperatorType() == ITM_CONSTANT) &&
                     (!((ConstValue*)pDefault_)->isNull()) && 
                     (!isIdentityColumn))
              {
                ...
                else if (NOT isAnErrorAlreadyIssued &&
                         pColumnDataType->getTypeQualifier() == NA_CHARACTER_TYPE &&
