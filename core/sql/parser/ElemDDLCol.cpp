...
// constructor
ElemDDLColDef::ElemDDLColDef(const NAString *columnFamily, const NAString *columnName, NAType *pColumnDataType,
                             ElemDDLNode *pColAttrList, CollHeap *heap)
  ...
  if (pColumnDataType NEQ NULL) {
    ...
    if (dataTypeName == "DATETIME" || dataTypeName == "DATE" || dataTypeName == "TIME" || dataTypeName == "TIMESTAMP") {
      ...
      if (((DatetimeIntervalCommonType *)pColumnDataType)
        ...
        if (!SqlParser_Diags->contains(-3195)) {
          *SqlParser_Diags << DgSqlCode(-3195) << DgString0("DATETIME");
        }
        return;
      }
    }
    // Create table with data type INTERVAL with FRACTION field(s) not supported
      ...
      if (((DatetimeIntervalCommonType *)pColumnDataType)
        ...
        // Only put error into diags if it doesn't already contain it
        if (!SqlParser_Diags->contains(-3195)) {
          *SqlParser_Diags << DgSqlCode(-3195) << DgString0("INTERVAL with FRACTION field(s)");
        }
        return;
      }
      ...
      if (!((SQLInterval *)pColumnDataType)->isSupportedType()) {
        // Only put error into diags if it doesn't already contain it
        if (!SqlParser_Diags->contains(-3195)) {
          *SqlParser_Diags << DgSqlCode(-3195) << DgString0("INTERVAL SECOND with leading precision 0");
        }
        return;
      }
    }
  ...
  if (pSGOptions_)  // isIdentityColumn
  {
    ...
    if (NOT getIsConstraintNotNullSpecified()) isNotNullSpec_ = TRUE;
    ...
    if (pConstraintNotNull_) {
      ...
      if (pConstraintNotNull_->isDroppableSpecifiedExplicitly()) {
        *SqlParser_Diags << DgSqlCode(-3413) << DgColumnName(ToAnsiIdentifier(getColumnName()));
        return;
      } else {
        // add the NOT DROPPABLE attribute to the NOT NULL .
        pConstraintNotNull_->setConstraintAttributes(new (PARSERHEAP()) ElemDDLConstraintAttrDroppable(FALSE));
      }
    } else {
  ...
  if (getIsConstraintNotNullSpecified() AND NOT getConstraintNotNull()->isDroppable()) {
    isNotNullNondroppable_ = TRUE;
    if (columnDataType_) columnDataType_->setNullable(FALSE);
  }
...
// cast
ElemDDLColDef *ElemDDLColDef::castToElemDDLColDef() { return this; }
...
void ElemDDLColDef::setDefaultAttribute(ElemDDLNode *pColDefaultNode) {
  ...
  if (pColDefault NEQ NULL) {
    switch (pColDefault->getColumnDefaultType()) {
      ...
      case ElemDDLColDefault::COL_DEFAULT: {
        ...
        if (pDefault_ &&
            ((pDefault_->getOperatorType() == ITM_CURRENT_USER) || (pDefault_->getOperatorType() == ITM_SESSION_USER) ||
             (pDefault_->getOperatorType() == ITM_CURRENT_TENANT) ||
             (pDefault_->getOperatorType() == ITM_CURRENT_CATALOG) ||
             (pDefault_->getOperatorType() == ITM_CURRENT_SCHEMA)) &&
            (pColumnDataType->getTypeQualifier() != NA_CHARACTER_TYPE) && (!isIdentityColumn)) {
          // default USER/TENANT. Column must be CHARACTER datatype.
          *SqlParser_Diags << DgSqlCode(-1186) << DgColumnName(ToAnsiIdentifier(getColumnName()))
                           << DgString0(pColumnDataType->getTypeSQLname(TRUE /*terse*/)) << DgString1("CHARACTER");
        } else if (pDefault_ && (pDefault_->getOperatorType() == ITM_CURRENT_TIMESTAMP) && (!isIdentityColumn)) {
          if ((pColumnDataType->getTypeQualifier() != NA_DATETIME_TYPE) ||
              (pColumnDataType->getTypeName() != LiteralTimestamp))
            // default CURRENT/CURRENT_TIMESTAMP. Column must be TIMESTAMP datatype.
            *SqlParser_Diags << DgSqlCode(-1186) << DgColumnName(ToAnsiIdentifier(getColumnName()))
                             << DgString0(pColumnDataType->getTypeSQLname(TRUE /*terse*/)) << DgString1("TIMESTAMP");
        } else if (pDefault_ && (pDefault_->getOperatorType() == ITM_CAST) && (!isIdentityColumn)) {
          // Column datatype must be compatible with default value datatype.
          // Currently used for default CURRENT_DATE, CURRENT_TIME. Column must be DATETIME datatype.
          Cast *cvDef = (Cast *)pDefault_;
          ...
          if (cvTyp->getTypeQualifier() == NA_CHARACTER_TYPE &&
              ...
              cvTyp->getCharSet() != pColumnDataType->getCharSet()) {
            Cast *newDef = new (PARSERHEAP()) Cast(cvDef, pColumnDataType);
            newDef->setCheckTruncationError(TRUE);
            newDef->setTgtCSSpecified(TRUE);

            pDefault_ = newDef;
            pColDefault->setDefaultValueExpr(pDefault_);
            cvTyp = newDef->getType();
          }

          if (!pColumnDataType->isCompatible(*cvTyp)) {
            *SqlParser_Diags << DgSqlCode(-1186) << DgColumnName(ToAnsiIdentifier(getColumnName()))
                             << DgString0(pColumnDataType->getTypeSQLname(TRUE /*terse*/))
                             << DgString1(cvTyp->getTypeSQLname(TRUE /*terse*/));
          }
        } else if (pDefault_ && (pDefault_->getOperatorType() == ITM_CONSTANT) &&
                   (!((ConstValue *)pDefault_)->isNull()) && (!isIdentityColumn)) {
          ...
          if (cvTyp->getTypeQualifier() == NA_CHARACTER_TYPE) {
            ...
            if (getCharSetInferenceSetting(inferCharSetFlag) == TRUE &&
              ...
              if (pColumnDataType->getTypeQualifier() == NA_CHARACTER_TYPE &&
                  ...
                  SqlParser_DEFAULT_CHARSET == CharInfo::UCS2 && defaultValueCS == CharInfo::ISO88591) {
                *SqlParser_Diags << DgSqlCode(-1186) << DgColumnName(ToAnsiIdentifier(getColumnName()))
                                 << DgString0(pColumnDataType->getTypeSQLname(TRUE /*terse*/))
                                 << DgString1(cvTyp->getTypeSQLname(TRUE /*terse*/));
              ...
              cvTyp = cvDef->pushDownType(*columnDataType_, NA_CHARACTER_TYPE);
            } else if (CmpCommon::getDefault(ALLOW_INCOMPATIBLE_OPERATIONS) == DF_ON) {
              if (pColumnDataType && NA_NUMERIC_TYPE == pColumnDataType->getTypeQualifier())
                cvTyp = cvDef->pushDownType(*columnDataType_, NA_CHARACTER_TYPE);
            }
          }  // column default value has character data type

          if (NOT isAnErrorAlreadyIssued && pColumnDataType->getTypeQualifier() == NA_CHARACTER_TYPE &&
              cvTyp->getTypeQualifier() == NA_CHARACTER_TYPE &&
            ...
            if (cvCharSet == CharInfo::ISO88591)                     // default value is a _ISO88591 str lit
            {
              ...
              {
                cnvErrStatus = UTF8ToLocale(cnv_version1  // in  - const enum cnv_version version
                                            ,
                                            pInStr  // in  - const char *in_bufr
                                            ,
                                            inStrLen  // in  - const int in_len
              ...
              switch (cnvErrStatus) {
                ...
                case CNV_ERR_NOINPUT:  // an empty input string will get this error code
                {
                  ...
                  if (cdCharSet != CharInfo::UNICODE) {
                    ...
                    pMBStrLitConstValue =
                        new (PARSERHEAP()) ConstValue(mbs2, cdCharSet  // use this for str lit prefix
                                                      ,
                                                      CharInfo::DefaultCollation, CharInfo::COERCIBLE, PARSERHEAP());
                  } else {
                ...
                case CNV_ERR_INVALID_CHAR: {
                  ...
                  *SqlParser_Diags << DgSqlCode(-1401) << DgColumnName(ToAnsiIdentifier(getColumnName()))
          ...
          else if (NOT isAnErrorAlreadyIssued && pColumnDataType->getTypeQualifier() == NA_CHARACTER_TYPE &&
                   cvTyp->getTypeQualifier() == NA_NUMERIC_TYPE &&
      ...
      case ElemDDLColDefault::COL_FUNCTION_DEFAULT: {
        ...
        if (pDefault_->getOperatorType() != ITM_DATEFORMAT)
          ...
          if (itr->getOperatorType() != ITM_CAST)
            ...
            if (itr->getOperatorType() != ITM_CURRENT_TIMESTAMP) valid = FALSE;
          }
        }
        if (valid == FALSE) {
          *SqlParser_Diags << DgSqlCode(-1084) << DgColumnName(ToAnsiIdentifier(getColumnName()));
        }
      } break;
...
void ElemDDLColDef::setColumnAttribute(ElemDDLNode *pColAttr) {
  switch (pColAttr->getOperatorType()) {
    case ELM_COL_HEADING_ELEM:
      if (isHeadingSpec_) {
        // Duplicate HEADING clauses in column definition.
        *SqlParser_Diags << DgSqlCode(-3051) << DgColumnName(ToAnsiIdentifier(getColumnName()));
      }
      ...
      if (heading_.length() > ElemDDLColHeading::maxHeadingLength) {
        *SqlParser_Diags << DgSqlCode(-3132) << DgColumnName(ToAnsiIdentifier(getColumnName()));
      }
      break;
    ...
    case ELM_CONSTRAINT_NOT_NULL_ELEM:
      ...
      if (isNotNullSpec_) {
        ...
        *SqlParser_Diags << DgSqlCode(-3052) << DgString0("NOT NULL")
    ...
    case ELM_LOGGABLE:
      ComASSERT(NULL NEQ pColAttr->castToElemDDLLoggable()) if (TRUE == isLoggableSpec_) {
        // Duplicate LOGGABLE in column definition.
        *SqlParser_Diags << DgSqlCode(-12064) << DgColumnName(ToAnsiIdentifier(getColumnName()));
      }
    ...
    case ELM_CONSTRAINT_PRIMARY_KEY_COLUMN_ELEM: {
      ...
      ComASSERT(pColAttr->castToElemDDLConstraintPKColumn()->getConstraintKind()
                    EQU ElemDDLConstraint::COLUMN_CONSTRAINT_DEF);
      if (isPrimaryKeySpec_) {
        // Duplicate PRIMARY KEY clauses in column definition.
        *SqlParser_Diags << DgSqlCode(-3053) << DgColumnName(ToAnsiIdentifier(getColumnName()));
      }
      isPrimaryKeySpec_ = TRUE;
      ElemDDLConstraintPKColumn *pColPKConstraint = pColAttr->castToElemDDLConstraintPKColumn();
      //
      ...
      ElemDDLColRef *pColRef =
          new (PARSERHEAP()) ElemDDLColRef(getColumnName(), pColPKConstraint->getColumnOrdering(), PARSERHEAP());
    ...
    case ELM_CONSTRAINT_REFERENTIAL_INTEGRITY_ELEM: {
      ...
      //
      ElemDDLConstraintRI *pColRIConstraint = pColAttr->castToElemDDLConstraintRI();
      ComASSERT(pColRIConstraint->getReferencingColumnNameList() EQU NULL);
      ElemDDLColName *pColName = new (PARSERHEAP()) ElemDDLColName(getColumnName());
    ...
    case ELM_CONSTRAINT_UNIQUE_ELEM: {
      ...
      // the column name.
      //
      ElemDDLConstraintUnique *pColUniqueConstraint = pColAttr->castToElemDDLConstraintUnique();
      ComASSERT(pColUniqueConstraint->getColumnRefList() EQU NULL);
      ElemDDLColRef *pColRef = new (PARSERHEAP()) ElemDDLColRef(getColumnName(), COM_ASCENDING_ORDER, PARSERHEAP());
      pColUniqueConstraint->setColumnRefList(pColRef);
    } break;

    case ELM_LOBATTRS: {
      ComASSERT(NULL NEQ pColAttr->castToElemDDLLobAttrs()) if (TRUE == isLobAttrsSpec_) {
        // Duplicate LOB attrs in column definition.
        *SqlParser_Diags << DgSqlCode(-3052) << DgString0("LOB") << DgColumnName(ToAnsiIdentifier(getColumnName()));
      }
    ...
    case ELM_SEABASE_SERIALIZED: {
      ComASSERT(NULL NEQ pColAttr->castToElemDDLSeabaseSerialized()) if (TRUE == isSeabaseSerializedSpec_) {
        // Duplicate SERIALIZED attrs in column definition.
        *SqlParser_Diags << DgSqlCode(-3052) << DgString0("SERIALIZED")
                         << DgColumnName(ToAnsiIdentifier(getColumnName()));
      }
    ...
    case ELM_COL_DEFAULT_ELEM: {
      ...
      if (TRUE == isColDefaultSpec_) {
        ...
        *SqlParser_Diags << DgSqlCode(-3052) << DgString0("DEFAULT") << DgColumnName(ToAnsiIdentifier(getColumnName()));
      }
...
void ElemDDLColDef::setDefaultValueExpr(ItemExpr *pNewDefValNode) {
  if (pDefault_ EQU pNewDefValNode) return;
...
NAString ElemDDLColDef::getColDefAsText() const {
  ...
  for (CollIndex i = 0; i < columnConstraintArray_.entries(); i++) {
    ...
    if ((pColAttr) && (pColAttr->castToElemDDLConstraintUnique()) &&
        (pColAttr->castToElemDDLConstraintUnique()->getConstraintKind() EQU ElemDDLConstraint::COLUMN_CONSTRAINT_DEF)) {
      text += "UNIQUE ";
    }
  }
...
void ElemDDLColDef::setSGOptions(ElemDDLSGOptions *pSGOptions) { pSGOptions_ = pSGOptions; }
...
const NAString ElemDDLColDef::displayLabel1() const { return NAString("Column name: ") + getColumnName(); }

const NAString ElemDDLColDef::displayLabel2() const {
  return NAString("Data type:   ") + getColumnDataType()->getSimpleTypeName();
}

NATraceList ElemDDLColDef::getDetailInfo() const {
  NAString detailText;
...
const NAString ElemDDLColDef::getText() const { return "ElemDDLColDef"; }
...
// constructor
ElemDDLColDefArray::ElemDDLColDefArray(CollHeap *heap) : LIST(ElemDDLColDef *)(heap) {}
...
// cast
ElemDDLColDefault *ElemDDLColDefault::castToElemDDLColDefault() { return this; }
...
const NAString ElemDDLColDefault::displayLabel1() const {
...
const NAString ElemDDLColDefault::displayLabel2() const {
  if ((getColumnDefaultType() EQU COL_DEFAULT) || getColumnDefaultType() EQU COL_FUNCTION_DEFAULT) {
    return (NAString("Default value: ") + getDefaultValueExpr()->getText());
  } else {
    return NAString();
  }
}

const NAString ElemDDLColDefault::getText() const { return "ElemDDLColDefault"; }
...
// cast
ElemDDLColHeading *ElemDDLColHeading::castToElemDDLColHeading() { return this; }

// methods for tracing

const NAString ElemDDLColHeading::displayLabel1() const {
...
const NAString ElemDDLColHeading::displayLabel2() const {
  switch (getColumnHeadingType()) {
    case COL_HEADING:
...
const NAString ElemDDLColHeading::getText() const { return "ElemDDLColHeading"; }
...
ElemDDLColName *ElemDDLColName::castToElemDDLColName() { return this; }

// methods for tracing

const NAString ElemDDLColName::getText() const { return "ElemDDLColName"; }

const NAString ElemDDLColName::displayLabel1() const { return NAString("Column name: ") + getColumnName(); }
...
// constructor
ElemDDLColNameArray::ElemDDLColNameArray(CollHeap *heap) : LIST(ElemDDLColName *)(heap) {}
...
// cast virtual function
ElemDDLColRef *ElemDDLColRef::castToElemDDLColRef() { return this; }

// accessor
NAString ElemDDLColRef::getColumnOrderingAsNAString() const {
...
const NAString ElemDDLColRef::getText() const { return "ElemDDLColRef"; }

const NAString ElemDDLColRef::displayLabel1() const { return NAString("Column name:   ") + getColumnName(); }

const NAString ElemDDLColRef::displayLabel2() const {
...
// constructor
ElemDDLColRefArray::ElemDDLColRefArray(CollHeap *heap) : LIST(ElemDDLColRef *)(heap) {}
...
ComBoolean ElemDDLColRefArray::contains(ElemDDLColRefArray &other, int &firstUnmatchedEntry) {
  ...
  for (i = 0; i < otherEntryCount; i++) {
    for (j = 0; j < thisEntryCount; j++) {
      if ((*this)[j]->getColumnName() == other[i]->getColumnName()) break;
    }
...
ComBoolean ElemDDLColRefArray::hasEntry(ElemDDLColRef &colRef) {
  ...
  for (j = 0; j < thisEntryCount; j++) {
    if ((*this)[j]->getColumnName() == colRef.getColumnName()) break;
  }
  // if we are past the last element in this array, we did not match
  // the column name.
  if (j == thisEntryCount)
...
ComBoolean ElemDDLColRefArray::operator==(ElemDDLColRefArray &other) {
...
// Compare column names and their order with other.
ComBoolean ElemDDLColRefArray::operator!=(ElemDDLColRefArray &other) { return !((*this) == other); }
// -----------------------------------------------------------------------
// methods for class ElemDDLColViewDef
// -----------------------------------------------------------------------

// constructor
ElemDDLColViewDef::ElemDDLColViewDef(const NAString &columnName, ElemDDLNode *pColAttrList)
...
// cast
ElemDDLColViewDef *ElemDDLColViewDef::castToElemDDLColViewDef() { return this; }
...
void ElemDDLColViewDef::setColumnAttribute(ElemDDLNode *pColAttr) {
  switch (pColAttr->getOperatorType()) {
    case ELM_COL_HEADING_ELEM:
      if (isHeadingSpec_) {
        // Duplicate HEADING clauses in column definition.
...
const NAString ElemDDLColViewDef::displayLabel1() const { return NAString("Column name: ") + getColumnName(); }

const NAString ElemDDLColViewDef::displayLabel2() const {
...
NATraceList ElemDDLColViewDef::getDetailInfo() const {
  NAString detailText;
  NATraceList detailTextList;
...
const NAString ElemDDLColViewDef::getText() const { return "ElemDDLColViewDef"; }
...
ElemProxyColDef::ElemProxyColDef(QualifiedName *tableName, NAString &colName, NAType *type, ElemDDLNode *colAttrs,
                                 CollHeap *heap)
    : ElemDDLColDef(NULL, &colName, type, colAttrs, heap), tableName_(tableName) {}
