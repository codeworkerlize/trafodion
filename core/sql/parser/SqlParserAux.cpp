...
// For ex:  {a.b.c}.d.e
//   will populate c,d,e.  
short ShortStringSequence::populateLists(NAList<NAString> &names, 
                                         NAList<UInt32> &indexes)
{
  Int32 startNum = (numCompositeColRefParts() > 0 
                    ? numCompositeColRefParts() - 1 : 0);
...
{
  assert(names);
  UInt32 index = (names->numCompositeColRefParts() > 0 
                  ? names->numCompositeColRefParts() : names->numParts());
  assert(index>0);
      ...
      if (attr->ptr1_ && attr->ptr2_)
        {
          ...
          else if ((objType == "PROCEDURE") || (objType == "PROCEDURES") ||
                   (objType == "FUNCTION") || (objType == "FUNCTIONS") ||
                   (objType == "ROUTINE") || (objType == "ROUTINES"))
            {
              br->setBRProcs(TRUE);
                          ...
                          SQLRow* &ss)
{
  ...
  NAArray<NAString> *fieldNames = 
    new(PARSERHEAP()) NAArray<NAString>(PARSERHEAP());
  NAArray<NAType*>  *fieldTypes = 
    new(PARSERHEAP()) NAArray<NAType*>(PARSERHEAP());
        ...
        case STYLE_UPSHIFT:
          {
            ...
            returnValue = new (PARSERHEAP())
              SQLChar(PARSERHEAP(), CharLenInfo(characterLimit, maxLenInBytes),
                      TRUE,TRUE,isCaseinsensitive,FALSE,
                      charset,collation,coerc,eEncodingCharSet);  
            assert(returnValue);
          }
          break;
...
//          a) If its child has same signature as specified, leave it in orignial select list and remove it from olapFunctions().
//          b) If it has different signature than specified, replace the RenameCol node with ColReference.
// 3) If itemExprTree is in olapFunctions(), which means its parent is not a RenameCol:
//          a) If its child has same signature as specified, leave it in orignial select list and remove it from olapFunctions().
//          b) If it has different signature than specified, create a RenameCol parent for it and replace it with ColReference.
//
...
void OlapMultiWindowExpander::setSignature(Aggregate* aggr)
{
    olapPartitionOrderSignature_ = extractSignature(aggr);
}
...
{   
	...
	ItemExpr* firstRenameCol =olapFunctions()[0];
        ...
        for(CollIndex i = 0; i < olapFunctions().entries(); i++)
        {
             ...
             ColReference * newColRef = new (heap_) ColReference((ColRefName *)((RenameCol*)olapFunctions()[i])->getNewColRefName());
             newselectlist = new(heap_) ItemList(newselectlist, newColRef);
        }
        ...
        {
             CMPASSERT(renamedAggregates()[i]->getOperatorType() == ITM_RENAME_COL);
             ColReference * newColRef = new (heap_) ColReference((ColRefName *)((RenameCol*)renamedAggregates()[i])->getNewColRefName());
             newselectlist = new(heap_) ItemList(newselectlist, newColRef);
        }
...
//                              |
//                             Scan
void OlapMultiWindowExpander::expand(RelRoot* root,                    
                                      RelExpr* topNode,
...
void handleMultiWindowOlapFunctions( RelRoot *  root )//INOUT: Top RelRoot of sequence of 
{
      ...
      //which means it is possible to have multiwindow olap functions in this selectlist(as olap functions may appear in where cluase incorrectly), 
      //then RelRoot::isPotentialMultipleOlapWindow() is set to true, and we can traverse the select list to examine further.
      if(CmpCommon::getDefault(OLAP_MULTIWINDOWS) == DF_OFF
         ||!root 
         ||!root->hasPotentialMultipleOlapWindow())
          return;
      RelExpr * upperNode = root->child(0);
      if(   upperNode 
         && upperNode->getOperatorType() == REL_SEQUENCE
         && upperNode->child(0).getPtr()
         ...
         )
      {
          ...
          if(expander->olapFunctions().entries() > 1)
              expander->expand(root, topNode, bottomNode);
...
void OlapMultiWindowExpander::collectOlapFunctions(ItemExpr* itemExprTree)
{
    ...
    if(itemExprTree->isOlapFunction())
        olapFunctions().insert(itemExprTree);

    for(Int32 i = 0; i < itemExprTree->getArity(); i++)
        collectOlapFunctions(itemExprTree->child(i));
}
...
void OlapMultiWindowExpander::collectReferencedColumns(ItemExpr* itemExprTree)
{
    ...
    if(itemExprTree->getOperatorType() == ITM_REFERENCE){
        ...
        if(hasReferencedColumns(itemExprTree->getText()))
            return;
    ...
    //collect children
    for(Int32 i = 0; i < itemExprTree->getArity(); i++)
        collectReferencedColumns(itemExprTree->child(i));
...
NABoolean OlapMultiWindowExpander::hasReferencedColumns(const NAString & string)
{
        for(CollIndex ix = 0; ix < referencedColumns().entries(); ix++)
            if(referencedColumns()[ix]->getText() == string)
                return TRUE;
        return FALSE;
}
...
ItemExpr * OlapMultiWindowExpander::replaceAggrWithAlias(ItemExpr* itemExprTree)
{
    ...
    if(itemExprTree->getOperatorType() == ITM_RENAME_COL 
        && itemExprTree->child(0)->isAnAggregate() 
    ...
    if(itemExprTree->isAnAggregate()&& !itemExprTree->isOlapFunction()){
        ...
        if(hasRenamedAggregate(*colRefName))
            return colRef;
...
{
  QualifiedName *qualifiedProcName = new (PARSERHEAP())
    QualifiedName(SEABASE_SPSQL_EXECUTE_SPJ,
                  SEABASE_MD_SCHEMA,        // schema name
                  TRAFODION_SYSTEM_CATALOG, // catalog name
                  PARSERHEAP());
  CallSP *expr = new (PARSERHEAP ())CallSP ( *qualifiedProcName,
                                             PARSERHEAP () );
  ...
  if(startWithStr != NULL) return;
	...
	for(int i = 0; startwith_index[i] != string::npos; ++i)
	{
       ...
       while (startwith_index[j] != string::npos && startwith_index[j] < connectby_index[reference_connectby])
		...
		sub[i].str = src_str.substr(startwith_index[i]+begin_pat.length(), connectby_index[offset+reference_connectby] - startwith_index[i] - begin_pat.length());
		++number_start_with;
