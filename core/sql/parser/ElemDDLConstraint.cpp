...
// cast
ElemDDLConstraint *ElemDDLConstraint::castToElemDDLConstraint() { return this; }
...
void ElemDDLConstraint::setConstraintAttributes(ElemDDLNode *pConstraintAttributes) {
  ...
  if (pConstraintAttributes) {
    ...
    for (CollIndex index = 0; ((index < num) && NOT hasDroppableConstrSpecified); index++) {
      ...
      if (pCnstrntAttr->castToElemDDLConstraintAttrDroppable()) hasDroppableConstrSpecified = TRUE;
    }
  }
  if (NOT hasDroppableConstrSpecified)  // no constraint attributes specified
  {
    ...
    if (isConstraintNotNull()) {
      setDroppableFlag(CmpCommon::getDefault(NOT_NULL_CONSTRAINT_DROPPABLE_OPTION) == DF_ON);
    } else if (castToElemDDLConstraintPK()) {
      setDroppableFlag(CmpCommon::getDefault(PRIMARY_KEY_CONSTRAINT_DROPPABLE_OPTION) == DF_ON);
    } else {
      // Does nothing.  isDroppable_ was already set to TRUE.
    }
    if (NOT pConstraintAttributes) return;
  }
  ...
  for (CollIndex index = 0; index < entries; index++) {
    ...
    if (pCnstrntAttr->castToElemDDLConstraintAttrDroppable()) {
      ...
      if (NOT isConstraintNotNull() AND NOT castToElemDDLConstraintPK()
              AND NOT pCnstrntAttr->castToElemDDLConstraintAttrDroppable()
                  ->isDroppable()) {
      ...
      setDroppableFlag(pCnstrntAttr->castToElemDDLConstraintAttrDroppable()->isDroppable());
      ...
      if (NOT castToElemDDLConstraintRI() AND NOT pCnstrntAttr->castToElemDDLConstraintAttrEnforced()->isEnforced()) {
      ...
      setEnforcedFlag(pCnstrntAttr->castToElemDDLConstraintAttrEnforced()->isEnforced());
    } else {
...
void ElemDDLConstraint::setDroppableFlag(const NABoolean setting) { isDroppable_ = setting; }

void ElemDDLConstraint::setEnforcedFlag(const NABoolean setting) { isEnforced_ = setting; }

//
// methods for tracing
//

const NAString ElemDDLConstraint::displayLabel1() const {
...
NATraceList ElemDDLConstraint::getDetailInfo() const {
  NAString detailText;
...
const NAString ElemDDLConstraint::getText() const { return "ElemDDLConstraint"; }
...
ElemDDLConstraintCheck::ElemDDLConstraintCheck(ItemExpr *pSearchCondition, const ParNameLocList &nameLocs,
                                               CollHeap *heap)
...
// cast
ElemDDLConstraintCheck *ElemDDLConstraintCheck::castToElemDDLConstraintCheck() { return this; }
...
//
NABoolean ElemDDLConstraintCheck::isConstraintNotNull() const { return getSearchCondition()->isISNOTNULL(); }
NABoolean ElemDDLConstraintCheck::getColumnsNotNull(ItemExprList &il) {
...
const NAString ElemDDLConstraintCheck::displayLabel2() const { return "Constraint type: Check constraint"; }

const NAString ElemDDLConstraintCheck::getText() const { return "ElemDDLConstraintCheck"; }
...
ElemDDLConstraintName *ElemDDLConstraintName::castToElemDDLConstraintName() { return this; }

//
// methods for tracing
//

const NAString ElemDDLConstraintName::getText() const { return "ElemDDLConstraintName"; }

const NAString ElemDDLConstraintName::displayLabel1() const {
  return NAString("Constraint name: ") + getConstraintName();
}
...
// cast
ElemDDLConstraintNotNull *ElemDDLConstraintNotNull::castToElemDDLConstraintNotNull() { return this; }

// methods for tracing

const NAString ElemDDLConstraintNotNull::displayLabel2() const { return "Constraint type: Not Null constraint"; }

const NAString ElemDDLConstraintNotNull::getText() const { return "ElemDDLConstraintNotNull"; }
...
// cast
ElemDDLConstraintPK *ElemDDLConstraintPK::castToElemDDLConstraintPK() { return this; }

//
// methods for tracing
//

const NAString ElemDDLConstraintPK::displayLabel2() const { return "Constraint type: Primary Key constraint"; }

const NAString ElemDDLConstraintPK::getText() const { return "ElemDDLConstraintPK"; }
...
// cast
ElemDDLConstraintPKColumn *ElemDDLConstraintPKColumn::castToElemDDLConstraintPKColumn() { return this; }

// methods for tracing

const NAString ElemDDLConstraintPKColumn::getText() const { return "ElemDDLConstraintPKColumn"; }
...
// constructor
ElemDDLConstraintRI::ElemDDLConstraintRI(ElemDDLNode *pReferencedTableAndColumns, ComRCMatchOption matchType,
                                         ElemDDLNode *pReferentialTriggeredActions, CollHeap *heap)
  ...
  ElemDDLReferences *pReferences = pReferencedTableAndColumns->castToElemDDLReferences();
  ...
  if (pReferencedColumns) {
    for (CollIndex index = 0; index < pReferencedColumns->entries(); index++) {
      referencedColumnNameArray_.insert((*pReferencedColumns)[index]->castToElemDDLColName());
    }
  }
  ...
  if (pReferentialTriggeredActions) {
    for (CollIndex i = 0; i < pReferentialTriggeredActions->entries(); i++) {
      ...
      setReferentialTriggeredAction((*pReferentialTriggeredActions)[i]->castToElemDDLRefTrigAct());
    }
  }
...
// cast
ElemDDLConstraintRI *ElemDDLConstraintRI::castToElemDDLConstraintRI() { return this; }
...
NAString ElemDDLConstraintRI::getMatchTypeAsNAString() const {
  switch (getMatchType()) {
...
NAString ElemDDLConstraintRI::getReferencedTableName() const {
  return getReferencesNode()->getReferencedNameAsQualifiedName().getQualifiedNameAsAnsiString();
}

ElemDDLReferences *ElemDDLConstraintRI::getReferencesNode() const {
...
void ElemDDLConstraintRI::setReferencingColumnNameList(ElemDDLNode *pReferencingColumnNameList) {
  setChild(INDEX_REFERENCING_COLUMN_NAME_LIST, pReferencingColumnNameList);
  ...
  if (pReferencingColumnNameList) {
    for (CollIndex i = 0; i < pReferencingColumnNameList->entries(); i++) {
      ComASSERT((*pReferencingColumnNameList)[i] AND(*pReferencingColumnNameList)[i]->castToElemDDLColName());
      referencingColumnNameArray_.insert((*pReferencingColumnNameList)[i]->castToElemDDLColName());
    }
  }
}  // ElemDDLConstraintRI::setReferencingColumnNameList()

void ElemDDLConstraintRI::setReferentialTriggeredAction(ElemDDLRefTrigAct *pRefTrigAct) {
  ...
  if (pRefTrigAct->castToElemDDLRefTrigActDeleteRule()) {
    ...
    deleteRule_ = pRefTrigAct->castToElemDDLRefTrigActDeleteRule()->getDeleteRule();
    ...
    updateRule_ = pRefTrigAct->castToElemDDLRefTrigActUpdateRule()->getUpdateRule();
  } else {
    ABORT("internal logic error");
  }
}
...
const NAString ElemDDLConstraintRI::displayLabel2() const {
  return "Constraint type: Referential Integrity constraint";
}

NATraceList ElemDDLConstraintRI::getDetailInfo() const {
  //
  ...
  ElemDDLReferences *pRefd = children_[INDEX_REFERENCED_TABLE_AND_COLUMNS]->castToElemDDLReferences();
...
const NAString ElemDDLConstraintRI::getText() const { return "ElemDDLConstraintRI"; }
...
ElemDDLConstraintUnique::ElemDDLConstraintUnique(ElemDDLNode *pColRefList, CollHeap *heap)
    : ElemDDLConstraint(heap, ELM_CONSTRAINT_UNIQUE_ELEM), columnRefList_(pColRefList), keyColumnArray_(heap) {}

ElemDDLConstraintUnique::ElemDDLConstraintUnique(OperatorTypeEnum operType, ElemDDLNode *pColRefList, CollHeap *heap)
    : ElemDDLConstraint(heap, operType), columnRefList_(pColRefList), keyColumnArray_(heap) {}
...
// cast
ElemDDLConstraintUnique *ElemDDLConstraintUnique::castToElemDDLConstraintUnique() { return this; }
...
void ElemDDLConstraintUnique::setColumnRefList(ElemDDLNode *pColumnRefList) {
  ...
  if (getColumnRefList()) {
    ...
    for (CollIndex i = 0; i < getColumnRefList()->entries(); i++) {
      ComASSERT((*getColumnRefList())[i] AND(*getColumnRefList())[i]->castToElemDDLColRef());
...
const NAString ElemDDLConstraintUnique::displayLabel2() const { return "Constraint type: Unique constraint"; }

NATraceList ElemDDLConstraintUnique::getDetailInfo() const {
  NATraceList detailTextList = ElemDDLConstraint::getDetailInfo();
...
const NAString ElemDDLConstraintUnique::getText() const { return "ElemDDLConstraintUnique"; }
