...
void yyULexer::yyULexer_ctor(const NAWchar *str, int charCount) {
  ...
  if (inJoinSpec == NULL)
    inJoinSpec = new LimitedStack;  // Note this is a THREAD_P global variable, so system heap is appropriate
...
yyULexer::~yyULexer() {
  if (yy_current_buffer_) {
    if (yy_current_buffer_->yy_ch_buf) NADELETEBASIC(yy_current_buffer_->yy_ch_buf, PARSERHEAP());
...
static NAString *removeConsecutiveQuotes(const NAWchar *s, int len, NAWchar quote, YYSTYPE *lvalp,
  ...
  lvalp->stringval_with_charset.resetflags_Bit(
      StringvalWithCharSet::eUSE_wstringval_FIELD_BIT_MASK);  // i.e., use stringval field
  ...
  lvalp->stringval_with_charset.setflags_Bit(
      StringvalWithCharSet::eINFER_CS_UNKNOWN_CS_BIT_MASK,
      (isInferCharSetEnabled && literalPrefixCS == CharInfo::UnknownCharSet  // the QUOTED_STRING case
  ...
  //
  if (!s || len <= 0 || ((len == 1) && (literalPrefixCS == CharInfo::UnknownCharSet))) {
  ...
  if (literalPrefixCS != CharInfo::UnknownCharSet)  // the TOK_SBYTE_LITERAL case
  {
    ...
    PARSERASSERT(targetCS == CharInfo::ISO88591 || targetCS == CharInfo::UTF8 || targetCS == CharInfo::BINARY);
    //
  ...
  } else  // literalPrefixCS == CharInfo::UnknownCharSet - i.e., the QUOTED_STRING case
  {
    ...
    // Check if the string is all ASCII characters (i.e., 0 <= code_points <= 127)
    {
      ...
      //
      if (containsOnlyASCII == TRUE) targetCS = CharInfo::ISO88591;
    }
  ...
  } else          // conversion was successful
  {
    ...
    if (literalPrefixCS == CharInfo::UnknownCharSet)  // the QUOTED_STRING case
      PARSERASSERT(targetCS == CharInfo::UTF8 ||
                   (targetCS == CharInfo::ISO88591 && NAStringHasOnly7BitAsciiChars(*pTempStr)));
    delete pTempStr;
    pTempStr = NULL;
...
int yyULexer::setStringval(int tokCod, const char *dbgstr, YYSTYPE *lvalp) {
  ...
  if ((tokCod == DELIMITED_IDENTIFIER || tokCod == IDENTIFIER || tokCod == BACKQUOTED_IDENTIFIER) &&
...
int yyULexer::aStringLiteralWithCharSet(CharInfo::CharSet cs, const NAWchar *str, int len, NAWchar quote,
  ...
  if (cachedBPC == 1 || CharInfo::is_NCHAR_MP(cs) || cs == CharInfo::UTF8 || cs == CharInfo::SJIS ||
      cs == CharInfo::UnknownCharSet) {
...
int yyULexer::aHexStringLiteralWithCharSet(CharInfo::CharSet cs, const NAWchar *str, int len, NAWchar quote,
  ...
  enum hex_conversion_code code = verifyAndConvertHex(str, len, quote, cs, PARSERHEAP(), result);
  ...
  switch (code) {
    ...
    case NOT_SUPPORTED:
      ...
      *SqlParser_Diags << DgSqlCode(-3401) << DgString0(CharInfo::getCharSetName(cs));
    ...
    case INVALID:
      ...
      *SqlParser_Diags << DgSqlCode(-3402) << DgString0(CharInfo::getCharSetName(cs));
      break;
    ...
    if (cc == quote)
      if (isHex == FALSE AND(cc = peekChar()) == quote)
        ...
        if (isHex) {
          ...
          return aHexStringLiteralWithCharSet(cs, YYText() + introducerLen, YYLeng() - introducerLen - 1, quote, lvalp);
        } else
          // a national char string specified by
          //  _charsetname'([^']|"''")*'
          return aStringLiteralWithCharSet(cs, YYText() + introducerLen, YYLeng() - introducerLen - 1, quote, lvalp);
      }
  }
  return prematureEOF(lvalp);
}
...
int yyULexer::yylex(YYSTYPE *lvalp) {
  ...
  CharInfo::CharSet cs;
  NABoolean possibleStmtBlock = FALSE;
  ...
  while (1) /* loops until end-of-file is reached */
  {
    ...
    switch (cc = peekChar()) {
      case L'_': {
        ...
        do {
          advance();
        } while (U_isASqlIdentiferChar(cc = peekChar()));
        doBeforeAction();
        ...
        if (cc == L'\'' || cc == quote) {
          ...
          if (CharInfo::isCharSetFullySupported(cs)) {
            if (cs == CharInfo::BINARY) isHexStringLiteral = TRUE;
        ...
        if (tokIfNotCompound = notCompoundKeyword(keyWordEntry1, cc, lvalp)) {
          if (tokIfNotCompound == IDENTIFIER && U_isspace(cc)) {
            ...
            if (cs != CharInfo::UnknownCharSet) {
              endp = mark();
              undoBeforeAction();
              while ((cc = peekChar()) != WEOF && U_isspace(cc)) advance();
              if (cc == L'\'' || cc == quote) {
                ...
                *SqlParser_Diags << DgSqlCode(+3159) << DgString0(yynarrow_);
              }
              retractToMark(endp);
            }
            NAWstrcpy(yytext_, preserveCase);
      ...
      case L'c':
        ...
        do {
          advance();
        } while (U_isASqlIdentiferChar(holdChar1 = peekChar()));
        doBeforeAction();
        ...
        if (tokIfNotCompound = notCompoundKeyword(keyWordEntry1, holdChar1, lvalp)) return tokIfNotCompound;
        ...
        switch (keyWordEntry1->getTokenCode()) {
          ...
          case TOK_CALL:
            ...
            if (U_isAsciiAlNumUnd(cc)) {
              ...
              do {
                advance();
                cc = peekChar();
              } while (SqlParser_WheneverClause ? U_isAsciiAlNumUndHyphen(cc) : U_isAsciiAlNumUnd(cc));
              ...
              switch (keyWordEntry2->getTokenCode()) {
                ...
                case TOK_WITH:
                  ...
                  *end1 = holdChar1;
                  ...
                  // scan 3rd part
                  beginRun3 = mark();
                  while (U_isASqlIdentiferChar(cc = peekChar())) advance();
                  ...
                  if (!keyWordEntry3->canBeThird() || keyWordEntry3->getTokenCode() != TOK_HOLD) {
          ...
          case TOK_CLEANUP:
            if (!U_isAsciiAlNumUnd(cc)) {
              ...
              switch (keyWordEntry2->getTokenCode()) {
                case TOK_OBSOLETE: {
                  return eitherCompoundOrSimpleKeyword(keyWordEntry2->getTokenCode() == TOK_OBSOLETE,
                                                       TOK_CLEANUP_OBSOLETE, keyWordEntry1->getTokenCode(), end1,
                                                       holdChar1, lvalp);

                } break;
                case TOK_BACKUP: {
                  return eitherCompoundOrSimpleKeyword(keyWordEntry2->getTokenCode() == TOK_BACKUP, TOK_CLEANUP_BACKUP,
      ...
      case L'g':
        ...
        if ((keyWordEntry1->getTokenCode() == TOK_GOTO || keyWordEntry1->getTokenCode() == TOK_GO) &&
            U_isAsciiAlNumUnd(cc)) {
          // scan CASED_IDENTIFIER
          do {
            advance();
          } while (U_isAsciiAlNumUndHyphen(peekChar()));
          // un-null-terminate 1st kwd
          *end1 = holdChar1;
          doBeforeAction();
          // a compound token specified by
      ...
      case L'n': {
        ...
        if ((peekChar() == L'\'' || peekChar() == quote)) {
          ...
          return constructStringLiteralWithCharSet(isHexRep, SqlParser_NATIONAL_CHARSET, lvalp, quote);

        } else  // identifier or keyword prefix specified by [Nn]
        {
          retractToMark(end1);

          while (U_isASqlIdentiferChar(holdChar1 = peekChar())) advance();
          doBeforeAction();
              ...
              switch (keyWordEntry1->getTokenCode()) {
                case TOK_NO:
                  return eitherCompoundOrSimpleKeyword(
                      keyWordEntry2->getTokenCode() == TOK_DEFAULT || keyWordEntry2->getTokenCode() == TOK_PARTITION ||
                          keyWordEntry2->getTokenCode() == TOK_PARTITIONS || keyWordEntry2->getTokenCode() == TOK_LOAD,
                      ((keyWordEntry2->getTokenCode() == TOK_DEFAULT)
                           ? TOK_NO_DEFAULT
                           : ((keyWordEntry2->getTokenCode() == TOK_LOAD)
                                  ? TOK_NO_LOAD
                                  : ((keyWordEntry2->getTokenCode() == TOK_PARTITION) ? TOK_NO_PARTITION
                                                                                      : TOK_NO_PARTITIONS))),
                      keyWordEntry1->getTokenCode(), end1, holdChar1, lvalp);
                  break;
                case TOK_NOT:
                  return eitherCompoundOrSimpleKeyword(
                      keyWordEntry2->getTokenCode() == TOK_IN || keyWordEntry2->getTokenCode() == TOK_BETWEEN ||
                      ...
                      ((keyWordEntry2->getTokenCode() == TOK_IN)
                           ...
                           : ((keyWordEntry2->getTokenCode() == TOK_BETWEEN)
                                  ...
                                  : ((keyWordEntry2->getTokenCode() == TOK_DROPPABLE)
                                         ? TOK_NOT_DROPPABLE
                                         : ((keyWordEntry2->getTokenCode() == TOK_CASESPECIFIC) ? TOK_NOT_CASESPECIFIC
                                                                                                : TOK_NOT_ENFORCED)))),
                      keyWordEntry1->getTokenCode(), end1, holdChar1, lvalp);
                  break;
                default:
      ...
      case L'p':
        ...
        if (keyWordEntry1->getTokenCode() == TOK_PRIMARY) {
          ...
          {
            return eitherCompoundOrSimpleKeyword(keyWordEntry2->getTokenCode() == TOK_INDEX, TOK_PRIMARY_INDEX,
                                                 keyWordEntry1->getTokenCode(), end1, holdChar1, lvalp);
          }
        } else if (keyWordEntry1->getTokenCode() == TOK_PERFORM && SqlParser_WheneverClause && U_isAsciiAlNumUnd(cc)) {
          // scan CASED_IDENTIFIER
          do {
            advance();
          } while (U_isAsciiAlNumUndHyphen(peekChar()));
          ...
          *end1 = holdChar1;
            ...
            while (cc == L'X' || cc == L'x') {
              ...
              if ((cc = peekChar()) == L'(') {
                ...
                if (U_isAsciiAlpha(cc)) {
                  ...
                  holdChar2 = cc;
                  setCurrChar(0);
                  const ParKeyWord *keyWordEntry = ParKeyWords::lookupKeyWord(bp_char);

                  // restore null-termination
                  setCurrChar(holdChar2);

                  if (keyWordEntry == NULL || keyWordEntry->getTokenCode() != TOK_CHARACTERS) break;  // error for now.
          ...
          if (keyWordEntry2->getTokenCode() == TOK_BY) {
            ...
            if (keyWordEntry3->getTokenCode() == TOK_RANGE) {
              *end1 = holdChar1;
              doBeforeAction();
              return aCompoundKeyword(TOK_PARTITION_BY_RANGE, lvalp);
            } else if (keyWordEntry3->getTokenCode() == TOK_LIST) {
              *end1 = holdChar1;
              doBeforeAction();
              return aCompoundKeyword(TOK_PARTITION_BY_LIST, lvalp);
            } else if (keyWordEntry3->getTokenCode() == TOK_HASH) {
              *end1 = holdChar1;
              doBeforeAction();
              return aCompoundKeyword(TOK_PARTITION_BY_HASH, lvalp);
            } else {
        ...
        while ((cc = peekAdvance()) != WEOF) {
          if (cc == L'"')
            if ((cc = peekChar()) == L'"')
              ...
              //
              return setStringval(DELIMITED_IDENTIFIER, DBGMSG("Delimited identifier %s\n"), lvalp);
            }
        }
        return prematureEOF(lvalp);
      case L'`':
        ...
        while ((cc = peekAdvance()) != WEOF) {
          if (cc == L'`')
            if ((cc = peekChar()) == L'`')
              ...
              // delimited identifiers.
              //
              return setStringval(BACKQUOTED_IDENTIFIER, DBGMSG("Backquoted identifier %s\n"), lvalp);
            }
        }
        return prematureEOF(lvalp);
      case L'.':
      ...
      case L'*':
        ...
        if ((cc = peekChar()) == L'*') {
          ...
          advance();
          doBeforeAction();
          return setTokval(TOK_EXPONENTIATE, DBGMSG("Exponentiation operator %s\n"), lvalp);
        } else if (cc == L'/') {
          // optimizer hint end specified by */
          advance();
          doBeforeAction();
      ...
      case L':':
        ...
        if ((cc = peekChar()) == L'"') {
          ...
          while ((cc = peekAdvance()) != WEOF) {
            if (cc == L'"')
              if ((cc = peekChar()) == L'"')
                ...
                lvalp->stringval = removeHostVarQuotes(YYText(), YYLeng());
          ...
          do {
            advance();
          } while (U_isASqlIdentiferChar(peekChar()));
          doBeforeAction();
          return setStringval(HOSTVAR, DBGMSG("Host variable %s\n"), lvalp);
        } else if ((SqlParser_CurrentParser->modeSpecial4()) && (U_isdigit(cc))) {
           ...
           */
          doBeforeAction();
          return setTokval(NON_SQLTEXT_CHARACTER, DBGMSG("Non-SQLTEXT character <%s>\n"), lvalp);
        }
        break;
      case L'?':
        advance();
        if (U_isalphaund(peekChar())) {
          ...
          lvalp->stringval = unicodeToChar(YYText() + 1, YYLeng() - 1, CharInfo::ISO88591, PARSERHEAP());
          return PARAMETER;
        } else {
      ...
      case L'\\':
        ...
        if (U_isAsciiAlpha(peekChar())) {
          ...
          do {
            advance();
          } while (U_isAsciiAlNumUnd((cc = peekChar())));
          if (cc != L'.') {
            doBeforeAction();
            ...
            if (peekChar() == L'$') {
              ...
              if (U_isAsciiAlpha(peekAdvance())) {
                ...
                // invalid NSK name
                doBeforeAction();
                return setTokval(NON_SQLTEXT_CHARACTER, DBGMSG("Invalid NSK sys-vol name <%s>\n"), lvalp);
              }
            } else if (U_isdigit(peekAdvance())) {
              while (U_isdigit(peekChar())) {
                advance();
              }
              ...
              // invalid NSK name
              doBeforeAction();
              return setTokval(NON_SQLTEXT_CHARACTER, DBGMSG("Invalid SYSTEM CPU Idenitfier <%s>\n"), lvalp);
            }
          } else {
            // invalid NSK name
            doBeforeAction();
      ...
      case L'(': {
        if (NOT SqlParser_CurrentParser->modeSpecial1()) {
          ...
          do {
            advance();
          } while (U_isspace(peekChar()));  // skip whitespace
          beginRun2 = mark();
          while (U_isASqlIdentiferChar(holdChar1 = peekChar())) advance();  // end of word
          setCurrChar(0);
          ...
          if (keyWordEntry1->getTokenCode() == TOK_FORMAT)  // (FORMAT...
          {
            ...
            do {
              advance();
            } while (U_isspace(holdChar2 = peekChar()));  // advance and skip whitespace
            if (holdChar2 == '\'') {
              // it is '(' in (FORMAT '...
          ...
          } else if (keyWordEntry1->getTokenCode() == TOK_DATE)  // (DATE...
          {
            ...
            if (holdChar2 == '(')                                 // '(' in (DATE (...
            {
              ...
              do {
                advance();
              } while (U_isspace(peekChar()));  // advance and skip whitespace
              beginRun3 = mark();
              while (U_isASqlIdentiferChar(holdChar3 = peekChar())) advance();  // end of word
              setCurrChar(0);
        ...
        if (keyWordEntry1->getTokenCode() == TOK_NAMED) {
          ...
          if ((keyWordEntry1->getTokenCode() == TOK_DECIMAL) || (keyWordEntry1->getTokenCode() == TOK_NUMERIC) ||
              (keyWordEntry1->getTokenCode() == TOK_INTEGER) || (keyWordEntry1->getTokenCode() == TOK_CHAR) ||
              (keyWordEntry1->getTokenCode() == TOK_CHARACTER) || (keyWordEntry1->getTokenCode() == TOK_SMALLINT)) {
      ...
      case L'/':
        ...
        if (peekChar() == L'*') {
          ...
          // Double slash quoting
          advance();
          while ((cc = peekAdvance()) != WEOF) {
            if (cc == L'/' && peekChar() == L'/') {
              advance();
              doBeforeAction();
              return aQuotedBlock(lvalp);
            }
          }
          return prematureEOF(lvalp);
        } else {
      ...
      case L'x':
        ...
        cc = peekChar();
        if (cc == L'\'') {
          return constructStringLiteralWithCharSet(TRUE, SqlParser_DEFAULT_CHARSET,
                                                   // CharInfo::BINARY,
                                                   lvalp, L'\'');
        } else {
          retractToMark(endp);
      ...
      case L'z':
        if (cc == L'V' || cc == L'v') {
          ...
          if (*next == L'$') {
            ...
            advance();
            doBeforeAction();
            return anSQLMXKeyword(TOK_V_DOLLAR_DYNAMICVIEW, lvalp);
          }
        }
        ...
        if ((keyWordEntry1->getTokenCode() == TOK_DATE) || (keyWordEntry1->getTokenCode() == TOK_TIME)) {
          cc = holdChar1;
          // skip white space
          while (U_isspace(cc)) {
            advance();
        ...
        if ((keyWordEntry1->getTokenCode() == TOK_BEGIN) || (keyWordEntry1->getTokenCode() == TOK_DECLARE)) {
          possibleStmtBlock = TRUE;
        } else if (tokIfNotCompound = notCompoundKeyword(keyWordEntry1, holdChar1, lvalp))
          return tokIfNotCompound;
        // else, a possible compound Trafodion keyword
        // save first token end
        end1 = mark();
        ...
        // check 2nd part of compound token
        if (!U_isAsciiAlpha(cc)) {
          if (possibleStmtBlock && cc != L';') {
            *end1 = holdChar1;
            return aCompoundStmtBlock(STMT_BLOCK, lvalp);
          }
          // 2nd part is not a keyword.
          // return 1st token as a Trafodion keyword.
          return anSQLMXKeyword(keyWordEntry1->getTokenCode(), lvalp);
        } else  // 2nd part may be a keyword
        {
          ...
          // restore null-termination of 2nd part.
          setCurrChar(holdChar2);
          if (possibleStmtBlock) {
            if ((keyWordEntry1->getTokenCode() == TOK_BEGIN &&
                 (keyWordEntry2->getTokenCode() != TOK_WORK && keyWordEntry2->getTokenCode() != TOK_TRANSACTION &&
                  keyWordEntry2->getTokenCode() != TOK_SAVEPOINT)) ||
                (keyWordEntry1->getTokenCode() == TOK_DECLARE && keyWordEntry2->getTokenCode() != TOK_CURSOR)) {
              *end1 = holdChar1;
              return aCompoundStmtBlock(STMT_BLOCK, lvalp);
            }
          }
            ...
            switch (keyWordEntry1->getTokenCode()) {
              case TOK_FOR:
                switch (keyWordEntry2->getTokenCode()) {
                  ...
                  case TOK_GROUP:
                    *end1 = holdChar1;
                    doBeforeAction();
                    return aCompoundKeyword(TOK_FOR_GROUP, lvalp);
                  case TOK_LIBRARY:
                    *end1 = holdChar1;
                    doBeforeAction();
                  ...
                  case TOK_TENANT:
                    *end1 = holdChar1;
                    doBeforeAction();
                    return aCompoundKeyword(TOK_FOR_TENANT, lvalp);
                  case TOK_USER:
                    *end1 = holdChar1;
                    doBeforeAction();
                    ...
                    return aCompoundKeyword(
                        keyWordEntry2->getTokenCode() == TOK_REPEATABLE
                            ...
                            : (keyWordEntry2->getTokenCode() == TOK_SKIP
                                   ? TOK_FOR_SKIP  // QSTUFF
                                   : (keyWordEntry2->getTokenCode() == TOK_SERIALIZABLE ? TOK_FOR_SERIALIZABLE
                                                                                        : TOK_FOR_MAXRUNTIME)),
                        lvalp);
                  ...
                  case TOK_READ:
                    ...
                    *end1 = holdChar1;
                    ...
                    // restore null-termination of 3rd part.
                    setCurrChar(holdChar3);
                    if (!keyWordEntry3->canBeThird() || keyWordEntry3->getTokenCode() != TOK_ONLY) {
                      // suffix is not part of a 3-part compound token,
                      // ie, FOR READ blah
                      // return 1st part as a 2-part compound token.
                      retractToMark(beginRun3);
                ...
                break;
              case TOK_DROP:
                if (keyWordEntry2->getTokenCode() == TOK_BACKUP)
                  return eitherCompoundOrSimpleKeyword(keyWordEntry2->getTokenCode() == TOK_BACKUP, TOK_DROP_BACKUP,
                                                       keyWordEntry1->getTokenCode(), end1, holdChar1, lvalp);
                else if (keyWordEntry2->getTokenCode() == TOK_OBSOLETE)
                  return eitherCompoundOrSimpleKeyword(keyWordEntry2->getTokenCode() == TOK_OBSOLETE, TOK_DROP_OBSOLETE,
                                                       keyWordEntry1->getTokenCode(), end1, holdChar1, lvalp);
                else if (keyWordEntry2->getTokenCode() == TOK_ALL)
                  return eitherCompoundOrSimpleKeyword(keyWordEntry2->getTokenCode() == TOK_ALL, TOK_DROP_ALL,
                                                       keyWordEntry1->getTokenCode(), end1, holdChar1, lvalp);
                else {
                  // DROP is a simple keyword.
                  // retract to end of kwd1.
                  retractToMark(end1);
                  return anSQLMXKeyword(keyWordEntry1->getTokenCode(), lvalp);
                }
              ...
              case TOK_INITIALIZE:
                if (keyWordEntry2->getTokenCode() == TOK_MAINTAIN)
                  return eitherCompoundOrSimpleKeyword(keyWordEntry2->getTokenCode() == TOK_MAINTAIN,
                                                       TOK_INITIALIZE_MAINTAIN, keyWordEntry1->getTokenCode(), end1,
                                                       holdChar1, lvalp);
                else {
              ...
              case TOK_LOCK:
                return eitherCompoundOrSimpleKeyword(
                    (SqlParser_CurrentParser->modeSpecial1() && keyWordEntry2->getTokenCode() == TOK_ROW), TOK_LOCK_ROW,
                    keyWordEntry1->getTokenCode(), end1, holdChar1, lvalp);
                break;
              case TOK_ON:
                return eitherCompoundOrSimpleKeyword(keyWordEntry2->getTokenCode() == TOK_COMMIT, TOK_ON_COMMIT,
                                                     keyWordEntry1->getTokenCode(), end1, holdChar1, lvalp);
                break;
              case TOK_REINITIALIZE:
                if (keyWordEntry2->getTokenCode() == TOK_MAINTAIN)
                  return eitherCompoundOrSimpleKeyword(keyWordEntry2->getTokenCode() == TOK_MAINTAIN,
                                                       TOK_REINITIALIZE_MAINTAIN, keyWordEntry1->getTokenCode(), end1,
                                                       holdChar1, lvalp);
                break;
              case TOK_REPEATABLE:
                return eitherCompoundOrSimpleKeyword(
                    keyWordEntry2->getTokenCode() == TOK_ACCESS || keyWordEntry2->getTokenCode() == TOK_READ,

                    keyWordEntry2->getTokenCode() == TOK_ACCESS ? TOK_REPEATABLE_ACCESS : TOK_REPEATABLE_READ,
                    keyWordEntry1->getTokenCode(), end1, holdChar1, lvalp);
                break;
              case TOK_REPLICATE:
                return eitherCompoundOrSimpleKeyword(keyWordEntry2->getTokenCode() == TOK_PARTITION,
                                                     TOK_REPLICATE_PARTITION, keyWordEntry1->getTokenCode(), end1,
                                                     holdChar1, lvalp);
                break;
              case TOK_SAMPLE:
                switch (keyWordEntry2->getTokenCode()) {
                    ...
                    *end1 = holdChar1;
                    ...
                    return aCompoundKeyword(
                        keyWordEntry2->getTokenCode() == TOK_FIRST
                            ? TOK_SAMPLE_FIRST
                            : (keyWordEntry2->getTokenCode() == TOK_PERIODIC ? TOK_SAMPLE_PERIODIC : TOK_SAMPLE_RANDOM),
                        lvalp);
                  default:
                ...
                break;
              case TOK_SEQUENCE:
                return eitherCompoundOrSimpleKeyword(keyWordEntry2->getTokenCode() == TOK_BY, TOK_SEQUENCE_BY,
                                                     keyWordEntry1->getTokenCode(), end1, holdChar1, lvalp);
                break;
              case TOK_SHOWDDL:
                if (keyWordEntry2->getTokenCode() == TOK_COMPONENT)
                  return eitherCompoundOrSimpleKeyword(keyWordEntry2->getTokenCode() == TOK_COMPONENT,
                                                       TOK_SHOWDDL_COMPONENT, keyWordEntry1->getTokenCode(), end1,
                                                       holdChar1, lvalp);
                else if (keyWordEntry2->getTokenCode() == TOK_ROLE)
                  return eitherCompoundOrSimpleKeyword(keyWordEntry2->getTokenCode() == TOK_ROLE, TOK_SHOWDDL_ROLE,
                                                       keyWordEntry1->getTokenCode(), end1, holdChar1, lvalp);
                else if (keyWordEntry2->getTokenCode() == TOK_TENANT)
                  return eitherCompoundOrSimpleKeyword(keyWordEntry2->getTokenCode() == TOK_TENANT, TOK_SHOWDDL_TENANT,
                                                       keyWordEntry1->getTokenCode(), end1, holdChar1, lvalp);
                else if (keyWordEntry2->getTokenCode() == TOK_GROUP)
                  return eitherCompoundOrSimpleKeyword(keyWordEntry2->getTokenCode() == TOK_GROUP, TOK_SHOWDDL_GROUP,
                                                       keyWordEntry1->getTokenCode(), end1, holdChar1, lvalp);

                else if (keyWordEntry2->getTokenCode() == TOK_LIBRARY)
                  return eitherCompoundOrSimpleKeyword(keyWordEntry2->getTokenCode() == TOK_LIBRARY,
                                                       TOK_SHOWDDL_LIBRARY, keyWordEntry1->getTokenCode(), end1,
                                                       holdChar1, lvalp);
                else if (keyWordEntry2->getTokenCode() == TOK_SEQUENCE)
                  return eitherCompoundOrSimpleKeyword(keyWordEntry2->getTokenCode() == TOK_SEQUENCE,
                                                       TOK_SHOWDDL_SEQUENCE, keyWordEntry1->getTokenCode(), end1,
                                                       holdChar1, lvalp);
                else if (keyWordEntry2->getTokenCode() == TOK_USER)
                  return eitherCompoundOrSimpleKeyword(keyWordEntry2->getTokenCode() == TOK_USER, TOK_SHOWDDL_USER,
                                                       keyWordEntry1->getTokenCode(), end1, holdChar1, lvalp);
                break;
              case TOK_SERIALIZABLE:
                return eitherCompoundOrSimpleKeyword(keyWordEntry2->getTokenCode() == TOK_ACCESS,
                                                     TOK_SERIALIZABLE_ACCESS, keyWordEntry1->getTokenCode(), end1,
                                                     holdChar1, lvalp);
                break;
              case TOK_SKIP:  // QSTUFF
                ...
                break;
              case TOK_START:
                return eitherCompoundOrSimpleKeyword(keyWordEntry2->getTokenCode() == TOK_WITH, TOK_START_WITH,
                                                     keyWordEntry1->getTokenCode(), end1, holdChar1, lvalp);
                break;
              case TOK_UNION:
                return eitherCompoundOrSimpleKeyword(keyWordEntry2->getTokenCode() == TOK_JOIN, TOK_UNION_JOIN,
                                                     keyWordEntry1->getTokenCode(), end1, holdChar1, lvalp);
                break;
              case TOK_UPDATE:
                return eitherCompoundOrSimpleKeyword(keyWordEntry2->getTokenCode() == TOK_LOB, TOK_UPDATE_LOB,
                                                     keyWordEntry1->getTokenCode(), end1, holdChar1, lvalp);
                break;
              default:
                int BadCompoundKeywordPrefix = 0;
...
void yyULexer::addTokenToGlobalQueue(NABoolean isComment) {
  ParScannedTokens->insert(ParScannedTokenPos, YYLeng(), ParScannedTokenOffset, isComment);
