...
inline static Int32 U_isASqlIdentiferChar(NAWchar c)
{
    return U_isalnumund(c) || isChineseCharacter(c);
}
...
Int32 yyULexer::aQuotedBlock(YYSTYPE *lvalp)
{
  ...
  lvalp->stringval = unicodeToChar
    (YYText()+2,YYLeng()-4,targetMBCS,PARSERHEAP());
...
Int32 yyULexer::aCompoundStmtBlock(Int32 tokCode, YYSTYPE *lvalp)
{
  ...
  retractToMark(end);
  doBeforeAction();
  return setStringval(tokCode,
                      DBGMSG("Compound statement block %s\n"),
                      lvalp);
}
  ...
  if (SqlParser_CurrentParser->internalExpr_)
    {
      ...
      static const Int32 SqlParser_starting_token[] = 
        ...
        TOK_INTERNAL_COMPOSITE_DEFINITION};
  ...
  while ( 1 )           /* loops until end-of-file is reached */
    {
      ...
      switch (cc=peekChar()) 
        ...
        case L'_': {
          ...
          advance();
          do { advance(); } while (U_isASqlIdentiferChar(cc=peekChar()));
          doBeforeAction();
        ...
        case L'C': case L'c':
          ...
          do { advance(); } while (U_isASqlIdentiferChar(holdChar1=peekChar()));
          doBeforeAction();
          ...
          case TOK_CLEANUP:
            if (!U_isAsciiAlNumUnd(cc)) {
              ...
              case TOK_BACKUP:
                {
                  return eitherCompoundOrSimpleKeyword
                    (keyWordEntry2->getTokenCode() == TOK_BACKUP,
                     TOK_CLEANUP_BACKUP,
                     keyWordEntry1->getTokenCode(),
                     end1, holdChar1, lvalp);
        ...
        case L':':
          ...
          if ((cc=peekChar()) == L'"')
          ...
          else if (U_isalphaund(cc))
            {
              ...
              do { advance(); } while (U_isASqlIdentiferChar(peekChar()));
              doBeforeAction();
	...
	case L'x':
	  ...
	  if ((keyWordEntry1->getTokenCode() == TOK_BEGIN) ||
	      (keyWordEntry1->getTokenCode() == TOK_DECLARE))
	    {
          ...
          if (!U_isAsciiAlpha(cc))
          ...
          else // 2nd part may be a keyword
            {
              ...
              if (possibleStmtBlock)
                {
                  if ((keyWordEntry1->getTokenCode() == TOK_BEGIN &&
                       (keyWordEntry2->getTokenCode() != TOK_WORK &&
                        keyWordEntry2->getTokenCode() != TOK_TRANSACTION &&
			keyWordEntry2->getTokenCode() != TOK_SAVEPOINT)) ||
                      (keyWordEntry1->getTokenCode() == TOK_DECLARE &&
                       keyWordEntry2->getTokenCode() != TOK_CURSOR))
                    {
                      *end1 = holdChar1;
                      return aCompoundStmtBlock(STMT_BLOCK, lvalp);
                    }
			...
			case TOK_MAXRUNTIME:
                    ...
                    case TOK_DROP:	
                      if (keyWordEntry2->getTokenCode() == TOK_BACKUP)
                    	...
                    	return eitherCompoundOrSimpleKeyword		
                          (keyWordEntry2->getTokenCode() == TOK_ALL,
                           TOK_DROP_ALL,
                           keyWordEntry1->getTokenCode(),
                           end1, holdChar1, lvalp);
                      else {
			   ...
			   end1, holdChar1, lvalp);
                   ...
                   case TOK_SHOWDDL:
                      if (keyWordEntry2->getTokenCode() == TOK_COMPONENT)
                        ...
                        return eitherCompoundOrSimpleKeyword
                          ( keyWordEntry2->getTokenCode() == TOK_TENANT,
                            TOK_SHOWDDL_TENANT,
                            keyWordEntry1->getTokenCode(),
                            end1, holdChar1, lvalp);
                      else if (keyWordEntry2->getTokenCode() == TOK_GROUP)
                        return eitherCompoundOrSimpleKeyword
                          ( keyWordEntry2->getTokenCode() == TOK_GROUP,
                            TOK_SHOWDDL_GROUP,
                            keyWordEntry1->getTokenCode(),
                            end1, holdChar1, lvalp);
