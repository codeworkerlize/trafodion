...
function usage {
  echo ""
  echo "Description: executes regression tests"
  echo ""
...
# Skip tests
#----------------------------------------
# Do not skip if explicitly specified
if [ "$1" = "" ]; then
  SKIPTESTSFILE="$REGRTSTDIR/SKIPTESTS"
  skipTheseTests=""
  if [ -f "$SKIPTESTSFILE" ]; then
    skipTheseTests=$(sed 's/#.*//g' "$SKIPTESTSFILE")
  fi
