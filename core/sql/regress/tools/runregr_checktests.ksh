...
if [ $ct1 -eq 1 ]; then
    ...
    setupAllTests "$diffOnly" "$seabase2CT" "seabase2"
fi

if [ $ct2 -eq 1 ]; then
    setupAllTests "$diffOnly" "$seabaseCT" "seabase"
    setupAllTests "$diffOnly" "$hiveCT" "hive"
    setupAllTests "$diffOnly" "$hive2CT" "hive2"
...
# keeps track on how ddl object epoch cache is increasing for checktests
# clear file at beginning of test
rm $TRAF_HOME/ddlcache 2> /dev/null
echo "Starting test: `date +%Y-%m-%d@%H:%M:%S` `tdm_arkcmp -testObjectEpochCache`"  > $TRAF_HOME/ddlcache
echo
...
for i in $testsToRun; do
    ...
    # report current size of ddl object epoch cache
    echo
    echo "`tdm_arkcmp -testObjectEpochCache`" 
    echo "`date +%Y-%m-%d@%H:%M:%S` (TEST$tnum) `tdm_arkcmp -testObjectEpochCache`"  >> $TRAF_HOME/ddlcache
    echo
