...
export TEST_CATALOG='hive'
export TEST_SCHEMA_NAME='hive'
...
if [ "$REGREXCLUSIVE" -ne 0 ]; then
  ...
  if [ $# -eq 0 ]; then
     prettyfiles=`ls -1 q* | sed -e /~$/d -e /.bak$/d | sort -fu` 2>$NULL
  else
     prettyfiles=`ls -1 $* | sed -e /~$/d -e /.bak$/d | sort -fu` 2>$NULL
  fi
fi
...
cp -f $REGRTSTDIR/stdcqds $REGRRUNDIR 2>$NULL
cp -f $REGRTSTDIR/explains $REGRRUNDIR 2>$NULL
cp -f $REGRTSTDIR/q* $REGRRUNDIR 2>$NULL
rm -f $REGRRUNDIR/osim.all.sf1000.09022016 2>$NULL
ln -s $REGRTSTDIR/osim.all.sf1000.09022016 osim.all.sf1000.09022016 2>$NULL
...
for i in $prettyfiles; do
  ...
  tnum=$i
  tnum=`basename $tnum`
  if [ `expr substr $tnum 1 1 | tr a-z A-Z` = "Q" ]; then
    # tnum=`expr substr $tnum 5 99`  -- this doesn't work right in ksh
    tnum=`echo $tnum | cut -c 2-` 
  fi
  ...
  testrun=$REGRRUNDIR/TEST$tnum

  # generate the test file
  echo "log LOG$tnum clear;"  > $testrun
  echo "obey stdcqds;"  >> $testrun
  echo "obey q$tnum;" >> $testrun
  echo "obey explains;" >> $testrun
