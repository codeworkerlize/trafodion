...
export HDFS_URL=$(sh regr_hdfs.ksh getconf -confKey fs.default.name | grep hdfs)
...
skipTheseTests="TEST021 TEST042"

if [ "$AMPOOL_HOME" == "" -o ! -d "$AMPOOL_HOME" ] ; then
  skipTheseTests="$skipTheseTests TEST041"
fi

# Skip the following tests when running as the trafodion ID
# The trafodion ID is used when running tests through Jenkins
#   TEST053 TEST054 tests privileges and backup/restore
scriptuser=`whoami`
if [ "$scriptuser" = "trafodion" ]; then
  skipTheseTests="$skipTheseTests TEST053 TEST054 TEST058"
fi
...
if [ "$CHECK_TEST1" == "1" ]; then
    skipTheseTests="$skipTheseTests $seabase2CT"
fi
...
for i in $prettyfiles; do
  ...
  if [ $diffOnly -eq 0 ]; then
    ...
    if [ -r $REGRTSTDIR/userdefs ]; then
      defsfile="$REGRTSTDIR/userdefs"
    fi
