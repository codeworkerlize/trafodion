...
if [ "$seabase" -ne 0 ]; then
  sbtestfiles="TEST001 TEST004 TEST005 TEST006 TEST011 TEST012 TEST013 TEST015 TEST023 TEST070 TEST071 TEST072 TEST073 TEST074 TEST076 TEST077 TEST078 TEST079 TEST080 TEST081 TEST042 TEST045 TEST049 TEST051 TEST052 TEST053 TESTTOK TEST062"
  sbprettyfiles=
...
skipTheseTests="TEST030 TEST044 TEST049 TEST050 TEST051 TEST065 TESTNIST"

# skip these tests for SQLMX and SQLMP tables on NSK platform

# TEST003 is being disabled for NSK until the issue is resolved.

skipTheseTests="$skipTheseTests TEST012 TEST075 TEST088"
...
if [ `uname` = "Linux" ]; then 
  skipTheseTests="$skipTheseTests TEST066"
fi
...
echo "copying FILTER062 to $REGRRUNDIR"
cp $REGRTSTDIR/FILTER062 $REGRRUNDIR 2>$NULL

echo "copying FILTER072 to $REGRRUNDIR"
cp $REGRTSTDIR/FILTER072 $REGRRUNDIR 2>$NULL

echo "copying FILTER073 to $REGRRUNDIR"
cp $REGRTSTDIR/FILTER073 $REGRRUNDIR 2>$NULL

echo "copying FILTER074 to $REGRRUNDIR"
cp $REGRTSTDIR/FILTER074 $REGRRUNDIR 2>$NULL

echo "copying FILTER076 to $REGRRUNDIR"
cp $REGRTSTDIR/FILTER076 $REGRRUNDIR 2>$NULL

echo "copying FILTER077 to $REGRRUNDIR"
cp $REGRTSTDIR/FILTER077 $REGRRUNDIR 2>$NULL

echo "copying FILTER078 to $REGRRUNDIR"
cp $REGRTSTDIR/FILTER078 $REGRRUNDIR 2>$NULL
...
for i in $prettyfiles; do
  ...
  if test "$tnum" = "042" ; then
    if [ "$REGRTSTDIR" != "$REGRRUNDIR" ]; then
       cp -f $REGRTSTDIR/hqc_key.* $REGRRUNDIR 2>$NULL
       cp -f $REGRTSTDIR/cache_key.* $REGRRUNDIR 2>$NULL
       cp -f $REGRTSTDIR/hqc_tests.txt  $REGRRUNDIR 2>$NULL
    fi
  fi
