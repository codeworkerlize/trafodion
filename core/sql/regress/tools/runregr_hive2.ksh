...
# skip these tests
skipTheseTests="TEST034 TEST037"

#TEST056 is consistently failing during jenkins run. It is being
#skipped until the code or testware change is checked in to fix it.
#tracked by mantis 7318
#TEST057 consistently fails during Jenkins run. It is being skipped.
#
#TEST123 causes core on some Jenkins run. It is being skipped for now.
#Once the core issue is fixed, it should be enabled.
#
# Skip the following tests when running as the trafodion ID
# The trafodion ID is used when running tests through Jenkins
scriptuser=`whoami`
if [ "$scriptuser" = "trafodion" ]; then
  skipTheseTests="$skipTheseTests TEST035 TEST056 TEST057 TEST123"
fi

# Run TEST058 (native Parquet Reader test), if it BUILD_NATIVE_PARQUET_READER'=
# is set.

if [ -z "$BUILD_NATIVE_PARQUET_READER" ]; then
  skipTheseTests="$skipTheseTests TEST058"
fi
