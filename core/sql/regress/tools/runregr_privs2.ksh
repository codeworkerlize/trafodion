...
#
#  TEST135 (redundant):  
#    tbl_tests covered by TEST139
#    view_tests covered by TEST143
#    udr_tests covered by TEST144
#-------------------------------------------------------
skipTheseTests="TEST135"
...
for ix in $testfiles; do
  ...
  if [ $diffsonly -eq 0 ]; then
    if [ $runTheTest -eq 1 ]; then
      ...
      #switch authentication to LOCAL for TEST147
      if [ "${ix}" == "TEST147" ]; then
        cp -a ${TRAF_VAR}/ms.env ${TRAF_VAR}/ms.env.bak
        echo "TRAFODION_AUTHENTICATION_TYPE=LOCAL" >> ${TRAF_VAR}/ms.env
      fi
      ...
      if [ "${ix}" == "TEST147" ]; then
        mv ${TRAF_VAR}/ms.env.bak ${TRAF_VAR}/ms.env
      fi
