...
{
  const ExprNode *qTree = (const ExprNode *) tree;
  if (qTree != NULL)
  {
  ...
  if (qTree != NULL && parentTreeItem != NULL)
    {
      ...
      for (Int32 i = qTree->getArity() - 1; i >= 0; i--)
        {
          DisplayItemExprChild((void*)qTree->getConstChild(i), treeItem);
        }
    }
}

void ItemExpressionView::FillItemExprDetails(const ExprNode * en,
...
void ItemExpressionView::on_treeWidget_itemClicked(QTreeWidgetItem * item, int column)
{
    QClipboard *clipboard = QApplication::clipboard();
    QString addr = item->toolTip(0);
    clipboard->setText(addr, QClipboard::Selection);
}
