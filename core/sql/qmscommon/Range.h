...
inline NABoolean operator==(const RangeDate& s1, const RangeDate& s2)
{ 
  return RangeDate::cmp(s1, s2) == 0;
}
...
inline NABoolean operator==(const RangeTime& s1, const RangeTime& s2)
{ 
  return RangeTime::cmp(s1, s2) == 0;
}
...
class RangeTimestamp: public RangeString
{
  public:
   ...
   RangeTimestamp(short year, short month, short day,
   ...
   void set_timestamp(const char* x, Lng32 len) 
       { memcpy(timestamp_, x, MINOF(len, sizeof(timestamp_))); };
...
class SubrangeBase : public NABasicObject
{
  ...
  public:
    ...
    */
    ...
    virtual 
    void appendSingleValueForOrc(OrcSearchArg&) = 0;
    virtual 
    void outputRangeForOrc(std::vector<std::string>& ppiVec, const char* colName, const char* colType) = 0;
...
*/
  ...
  protected:
    SubrangeBase(ElementType et = ET_INVALID,
                 NABoolean startInclusive = FALSE,
                 NABoolean endInclusive = FALSE
                )
...
{
  public:
    Subrange(logLevel ll, ElementType et = ET_INVALID,
             NABoolean startInclusive = FALSE,  
             NABoolean endInclusive = FALSE
            )
      : logLevel_(ll), 
        SubrangeBase(et, startInclusive, endInclusive)
      {}
    ...
    void appendSingleValueForOrc(OrcSearchArg&);
...
class RangeSpec : public NABasicObject
{
  ...
  public:
    ...
    void addSubrange(QRScalarValuePtr startVal,
                     QRScalarValuePtr endVal) 
      { addSubrange(startVal, endVal, TRUE, TRUE); };
...
*/
    ...
    // convert to ORC pushdown predicates and store the result
    // push-down predicate in text, and use no more bytes than 
    // what is available in argument bytes. Subtract # of bytes
    // used from the argument on return.
    NABoolean convertToOrcPredicates(
