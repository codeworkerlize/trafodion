...
bool StatsGlobals::addProcess(pid_t pid, NAHeap *heap)
{
  ...
  LOGINFO(CAT_SQL_LOCK,
          "Add process %d, DML lock array size: %d, DDL lock array size: %d",
          pid, dmlLockSize, ddlLockSize);
...
void StatsGlobals::cleanupDanglingSemaphore(NABoolean checkForSemaphoreHolders)
{
  ...
  { 
     if (lockedTimeInSecs >= rmsDeadLockSecs_)
...
int StatsGlobals::getStatsSemaphore(Long &semId, pid_t pid,
                                    NABoolean calledByVerifyAndCleanup)
{
  int error = 0;
  timespec ts;
    ...
    if (retryCount > timeoutRetryCount)
    {
      if (calledByVerifyAndCleanup && pid == getSsmpPid())
      {
        ...
        snprintf(msg, sizeof(msg),
                 "SSMP getStatsSemaphore exhausted retry times(%d * 3s)", retryCount);
        SQLMXLoggingArea::logExecRtInfo(__FILE__, __LINE__, msg, 0);
      ...
      else
      {
        QRLogger::log(CAT_SQL_EXE, LL_ERROR,
          "StatsGlobals::getStatsSemaphore : sem_timedwait() reached its maximum(%d), retry count(%d), exit process!!!",
...
int StatsGlobals::openStatsSemaphoreWithRetry(Long &semId)
{
  ...
  for (retry_cnt = 0; retry_cnt < retry; ++retry_cnt)
  {
    ...
    if (error != 0)
    {
      QRLogger::log(CAT_SQL_EXE, LL_WARN, "openStatsSemaphore() failed with error = %d, will sleep 100ms then do %dth retry",
                    error, retry_cnt);
      usleep(100000); //100ms
    }
    else
                                 ...
                                 Lng32 tdbNameLen)
{
  ...
  if (ss &&
      ((statsCollectionType == ComTdb::ALL_STATS) ||
...
StatsGlobals * shareStatsSegment(Int32 &shmid, NABoolean checkForSSMP)
{
  ...
  if ((shmid = shmget((key_t) getStatsSegmentId(),
			     0,  // size doesn't matter unless we are creating.
                             shmFlag )) == -1)
  {
     QRLogger::log(CAT_SQL_EXE, LL_WARN, "shmget() failed with errno = %d, SegmentId = %d, shmFlag = %d",
                   errno, getStatsSegmentId(), shmFlag);
     return NULL;
  }
  ...
  if (statsGlobals)
  {
     ...
     { 
       LOGTRACE(CAT_SQL_LOCK,
                "Acquiring RTS semaphore for pid %d", cliGlobals->myPin());
       error = statsGlobals->getStatsSemaphore(cliGlobals->getSemId(), cliGlobals->myPin());
       LOGTRACE(CAT_SQL_LOCK,
                "Acquired RTS semaphore for pid %d, error: %d",
                cliGlobals->myPin(), error);
       retcode = 1;
...
void releaseRTSSemaphore()
{
  ...
  if (statsGlobals)
  {
    ...
    {
      LOGTRACE(CAT_SQL_LOCK,
               "Releasing RTS semaphore for pid %d", cliGlobals->myPin());
      ...
      LOGTRACE(CAT_SQL_LOCK,
               "Released RTS semaphore for pid %d", cliGlobals->myPin());
    }
  }
}
...
ObjectEpochCacheEntryName::ObjectEpochCacheEntryName(NAHeap *heap,
  const char * objectName,ComObjectType objectType,Int64 redefTime) :
...
ObjectEpochCache::ObjectEpochCache(StatsGlobals *statsGlobals,
                                   NAHeap *heap)
  : statsGlobals_(statsGlobals)
  , epochHeap_(heap)
{
  statsGlobals_->setObjectEpochCache(this);
  epochList_ = new (epochHeap())
    NAHashDictionary<ObjectEpochCacheKey, ObjectEpochCacheEntry>(&hashFunc,
                                                                 1021, // prime, to avoid skew
                                                                 TRUE,
                                                                 epochHeap(),
                                                                 FALSE /* heap allocation failure is not fatal */);
}

short ObjectEpochCache::createEpochEntry(ObjectEpochCacheEntry* &entry,
                                         ...
                                         UInt32 flags)
{
  ...
  ObjectEpochCacheKey *k =
    new (epochHeap()) ObjectEpochCacheKey(key, epochHeap());
  if (k == NULL) {
    if (semRetCode == 1)
       releaseRTSSemaphore();
...
short ObjectEpochCache::getEpochEntry(ObjectEpochCacheEntry* &entry,
                                      const ObjectEpochCacheKey &key)
{
  short semRetCode = getRTSSemaphore();
...
short ObjectEpochCache::updateEpochEntry(ObjectEpochCacheEntry* &entry,
                                         const ObjectEpochCacheKey &key,
                                         UInt32 newEpoch,
                                         UInt32 newFlags)
{
...
void ObjectEpochCache::appendToStats(ExStatisticsArea *statsArea,
                                     const char *objectName, bool locked)
{
  ...
  NAHashDictionaryIterator<ObjectEpochCacheKey,
                           ObjectEpochCacheEntry> epochIter(*epochList_, NULL, NULL, heap);
  ...
  while (epochIter.getNext(key, entry))
  {
    ...
    if ((objectName != NULL) &&
        (strcmp(key->objectName(), objectName) != 0)) {
      continue;
    }
    ExObjectEpochStats *epochStats = new (heap)
      ExObjectEpochStats(heap,
                         cpu,
                         key->objectName(),
                         key->objectType(),
                         key->hash(),
...
ObjectEpochCacheKey::ObjectEpochCacheKey(const char *objectName, ULng32 hash,
...
void testObjectEpochCache(Int32 argc, char **argv)
{
  ...
  for (size_t i = 2; i < argc; i++)
  {
    ...
    *nextTarget = '\0';
    ...
    ObjectEpochCacheEntryName oecName(NULL,
                                      objectName,
    ...
    else
    {
      ...
      if (dots != 2)
        cout << " (please add catalog and schema name)";
      cout << endl;
    }
...
{ 
   ...
   if (epochCache_ == NULL) {
      ...
      short semRetCode = getRTSSemaphore();
      NAHeap *epochHeap = new (getStatsHeap())
           NAHeap("Epoch Heap", (NAHeap *)getStatsHeap(),
                 8192,
...
NAHeap *StatsGlobals::getEpochHeap() 
{
   ...
   if (epochCache_ != NULL)
       epochHeap = epochCache_->epochHeap();
   return epochHeap;
}
...
void StatsGlobals::populateQueryInvalidateStats(ExStatisticsArea *statsArea)
{
  ...
  while (NULL != (recentSikey = (RecentSikey *)recentSikeys_->getNext()))
  {
    ...
    ComQIActionType siKeyType = ComQIActionTypeLiteralToEnum(
                                   recentSikey->s_.operation);
    if (siKeyType == COM_QI_OBJECT_REDEF ||
        siKeyType == COM_QI_STATS_UPDATED)
    {
...
void StatsGlobals::unlockIfHeapMutexLocked()
{
  ...
  if (ret != 0)
  {
    ...
    char msg[256];
    snprintf(msg, sizeof(msg),
             "StatsGlobals::statsHeap_ is locked by noexisting process, destory and reinitialize it");
    SQLMXLoggingArea::logExecRtInfo(__FILE__, __LINE__, msg, 0);
...
bool ObjectLockEntry::match(const char* objectName,
                            ComObjectType objectType) const
{
...
ObjectLockEntry *ObjectLockArray::findLock(const char *objectName,
					   ComObjectType objectType)
{
...
ObjectLockEntry* ObjectLockArray::addLock(const char* objectName,
                                          ComObjectType objectType,
                                          const LockHolder &holder)
{
  ...
  i = entries_;
  if (i >= maxEntries()) {
    LOGERROR(CAT_SQL_LOCK, "Cannot add lock %s %s due to lock entry exhaustion",
  ...
  entries_++;
  LOGDEBUG(CAT_SQL_LOCK, "Added lock entry %u (%p) of %s %s for %d %d",
           i, lock, comObjectTypeName(objectType), objectName,
           holder.getNid(), holder.getPid());
  return lock;
}

bool ObjectLockArray::lockedBy(const char *objectName,
                               ComObjectType objectType,
                               const LockHolder &holder)
{
...
bool ObjectLockArray::hasConflictLock(const char *objectName,
                                      ComObjectType objectType,
                                      const LockHolder &holder,
                                      LockHolder &conflictHolder)
{
  ...
  if (lock != NULL && lock->locked() && lock->holder() != holder) {
    ...
    LOGDEBUG(CAT_SQL_LOCK,
             "Found a conflict lock for %s %s (holder %d %d) currently held by %d %d",
             comObjectTypeName(objectType), objectName,
             holder.getNid(), holder.getPid(),
...
bool ObjectLockArray::acquireLock(const char *objectName,
                                  ComObjectType objectType,
                                  const LockHolder &holder,
                                  LockHolder &conflictHolder,
                                  bool &outOfEntry)
{
  ...
  if (lock == NULL) {
    LOGERROR(CAT_SQL_LOCK,
             "Acquire lock %s %s (holder %d %d) failed due to out of lock entry",
             comObjectTypeName(objectType), objectName,
             holder.getNid(), holder.getPid());
    outOfEntry = true;
    return false;
...
void ObjectLockArray::deepRecycle()
{
  ...
  LOGINFO(CAT_SQL_LOCK,
          "Deep recyle recollected %d lock entries, current entries: %d",
          oldEntries - entries_, entries_);
}
...
void ObjectLockArray::removeLocksHeldBy(const LockHolder &holder)
{
  ...
  LOGDEBUG(CAT_SQL_LOCK,
           "Removed %d locks held by %d %d, recycled %d entries, %d entries left",
           removedEntries,
           holder.getNid(), holder.getPid(),
           recycledEntries,
           entries);
...
void ObjectLockArray::releaseLock(const char *objectName,
                                  ComObjectType objectType,
                                  const LockHolder &holder)
{
  ObjectLockEntry* lock = NULL;
  ...
  LOGDEBUG(CAT_SQL_LOCK, "Recycled %d entries, %d entries left",
           oldEntries - entries_, entries_);
}
...
void ObjectLockArray::copyLocks(const ObjectLockArray &from)
{
  ...
  for (unsigned short i = 0; i < from.entries(); i++) {
    ...
    LOGDEBUG(CAT_SQL_LOCK, "Copy lock entry %s %s (holder %d %d)",
             comObjectTypeName(lock->objectType()), lock->objectName(),
             lock->holder().getNid(), lock->holder().getPid());
    locks_[i] = *lock;
  }
...
ObjectLockArray *ObjectLockController::dmlLockArray(pid_t pid)
{
  ...
  ex_assert(objectLockArray,
	    "DML ObjectLockArray not properly initialized");
...
ObjectLockArray *ObjectLockController::ddlLockArray(pid_t pid)
{
  ...
  ex_assert(objectLockArray,
	    "DDL ObjectLockArray not properly initialized");
  return objectLockArray;
}
...
bool ObjectLockController::hasConflictDDL(const LockHolder &holder,
                                          LockHolder &conflictHolder /* OUT */)
{
  return ddlLockArray()->hasConflictLock(objectName_, objectType_,
...
{
  LOGDEBUG(CAT_SQL_LOCK, "DML lock %s %s",
           comObjectTypeName(objectType_), objectName_);
  LockHolder me(GetCliGlobals());
...

  {
    ...
    if (dmlLock != NULL) {
      ...
      LOGDEBUG(CAT_SQL_LOCK, "DML lock %s %s is already held by me (%d %d)",
               comObjectTypeName(objectType_), objectName_,
               me.getNid(), me.getPid());
    ...
    if (dmlLock == NULL) {
      LOGERROR(CAT_SQL_LOCK, "DML lock %s %s failed due to out of lock entry",
               comObjectTypeName(objectType_), objectName_);
  ...
  if (hasConflictDDL(me, conflictHolder)) {
    LOGDEBUG(CAT_SQL_LOCK, "DML lock %s %s failed due to conflict DDL operations",
             comObjectTypeName(objectType_), objectName_);
    dmlLock->release(me);
...
ObjectLockController::Result
ObjectLockController::lockDDL(const LockHolder &holder,
                              LockHolder &conflictHolder /* OUT */)
{
  LOGDEBUG(CAT_SQL_LOCK, "DDL lock %s %s (holder %d %d)",
           comObjectTypeName(objectType_), objectName_,
           holder.getNid(), holder.getPid());
  ...
  for (pid = 0; pid <= maxPid; pid++) {
    ...
    LOGDEBUG(CAT_SQL_LOCK,
	     "Acquire DDL lock %s %s in process %d",
	     comObjectTypeName(objectType_), objectName_, pid);
    bool outOfEntry = false;
    if (!ddlLockArray->acquireLock(objectName_, objectType_,
                                   holder, conflictHolder, outOfEntry)) {
      if (outOfEntry) {
        LOGWARN(CAT_SQL_LOCK,
                "Acquire DDL lock %s %s failed in process %d due to out of lock entry",
                comObjectTypeName(objectType_), objectName_, pid);
        return Result::LOCK_OUT_OF_ENTRY;
      }
      LOGWARN(CAT_SQL_LOCK,
              "Acquire DDL lock %s %s failed in process %d due to conflict with other DDL operations",
              comObjectTypeName(objectType_), objectName_, pid);
      return Result::LOCK_CONFLICT_DDL;
    }
    ObjectLockArray* dmlLockArray = statsGlobals->dmlLockArray(pid);
    ex_assert(dmlLockArray, "DML lock array is NULL");
    if (dmlLockArray->hasConflictLock(objectName_, objectType_,
                                      holder, conflictHolder)) {
      LOGWARN(CAT_SQL_LOCK,
              "DDL lock %s %s conflict with DML operations in process %d",
              comObjectTypeName(objectType_), objectName_, pid);
...
{
  LOGDEBUG(CAT_SQL_LOCK, "DDL unlock %s %s",
           comObjectTypeName(objectType_), objectName_);
  CliGlobals *cliGlobals = GetCliGlobals();
  ex_assert(cliGlobals,
	    "CliGlobals not properly initialized");
  ...
  for (pid = 0; pid <= maxPid; pid++) {
    ...
    LOGDEBUG(CAT_SQL_LOCK,
	     "Release DDL lock %s %s in process %d",
	     comObjectTypeName(objectType_), objectName_, pid);
...
{
  LOGDEBUG(CAT_SQL_LOCK, "Release all DDL locks held by %d %d",
           holder.getNid(), holder.getPid());
  CliGlobals *cliGlobals = GetCliGlobals();
  ex_assert(cliGlobals,
	    "CliGlobals not properly initialized");
...
//Hot spot optimization of SSMP process function in arm architecture
  ...
  for (pid = 0; pid <= maxPid; pid++) {
    ...
    LOGDEBUG(CAT_SQL_LOCK,
             "Remove all DDL locks by holder %d %d in process %d",
             holder.getNid(), holder.getPid(), pid);
...
{
  LOGDEBUG(CAT_SQL_LOCK, "Release all DML locks held by %d %d",
           holder.getNid(), holder.getPid());
  CliGlobals *cliGlobals = GetCliGlobals();
  ex_assert(cliGlobals,
	    "CliGlobals not properly initialized");
  ...
  if (dmlLockArray != NULL && dmlLockArray->maxEntries() > 0) {
    LOGDEBUG(CAT_SQL_LOCK,
             "Remove all DML locks in process %d",
             holder.getPid());
...
void ObjectLockArray::appendToStats(ExStatisticsArea *statsArea,
				    short cpu, pid_t pid,
                                    ...
                                    bool ddlLock)
{
  ...
  if (objectName != NULL) {
    ...
    if (lock != NULL && lock->locked()) {
      ...
      ExObjectLockStats *lockStats = new (heap)
	ExObjectLockStats(heap,
			  cpu,
			  objectName,
			  objectType,
			  pid,
...
void ObjectLockController::appendToStats(ExStatisticsArea *statsArea,
                                         const char *objectName,
                                         ComObjectType objectType)
{
  LOGDEBUG(CAT_SQL_LOCK,
           "Append lock stats of %s %s",
           comObjectTypeName(objectType), objectName);
  CliGlobals *cliGlobals = GetCliGlobals();
  ex_assert(cliGlobals,
	    "CliGlobals not properly initialized");
  ...
  for (pid = 0; pid <= maxPid; pid++) {
    ...
    if (dmlLockArray != NULL) {
      ...
      dmlLockArray->appendToStats(statsArea, cpu, pid,
                                  objectName, objectType, false /* ddlLock */);
    }
    ObjectLockArray* ddlLockArray = statsGlobals->ddlLockArray(pid);
    if (ddlLockArray != NULL) {
      LOGDEBUG(CAT_SQL_LOCK, "Add DDL lock stats in process %d", pid);
      ddlLockArray->appendToStats(statsArea, cpu, pid,
                                  objectName, objectType, true /* ddlLock*/);
    }
  }
}
...
void DDLScope::startDDLStmt()
{
  ...
  ex_assert(myProcessStats_,
            "ProcessStats not properly initialized");
...
void DDLScope::endDDLStmt()
{
  ...
  UInt32 level = myProcessStats_->ddlStmtLevel();
  LOGDEBUG(CAT_SQL_LOCK,
           "End DDL Stmt: level = %d",
           level);

  bool xnInProgress = false;
  if (cliGlobals->currContext() &&
      cliGlobals->currContext()->getTransaction() &&
  ...
  if (level == 1 &&
      !xnInProgress &&
      cmpContext_ != NULL &&
      cmpContext_->ddlObjsList().entries() > 0) {
