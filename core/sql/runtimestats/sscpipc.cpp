...
SscpGlobals::SscpGlobals(NAHeap *sscpheap, StatsGlobals *statsGlobals)
  ...
  error = ComRtGetProgramInfo(programDir, 100, processType, myCpu, myPin_, myNodeNumber, myNodeName, myNodeNameLen,
                              myStartTime, myProcessNameString);
  ...
  NAHeap *statsHeap = (NAHeap *)statsGlobals_->getStatsHeap()->allocateHeapMemory(sizeof *statsHeap, FALSE);
  ...
  if (reincarnated) statsGlobals_->logProcessDeath(myCpu, myPin_, "Process reincarnated before RIP");
  ...
  if (((error = getDefineNumericValue(defineName, &zeroMeansNo)) == 0) && (zeroMeansNo != 0))
    doLogCancelKillServers_ = true;
}

SscpGlobals::~SscpGlobals() { sem_close((sem_t *)semId_); }

void SscpGuaReceiveControlConnection::actOnSystemMessage(short messageNum, IpcMessageBufferPtr sysMsg,
  ...
  switch (messageNum) {
    case ZSYS_VAL_SMSG_OPEN: {
      SscpNewIncomingConnectionStream *newStream = new (getEnv()->getHeap())
          SscpNewIncomingConnectionStream((NAHeap *)getEnv()->getHeap(), getEnv(), getSscpGlobals());
...
SscpNewIncomingConnectionStream::SscpNewIncomingConnectionStream(NAHeap *heap, IpcEnvironment *env,
    ...
    : IpcMessageStream(env, IPC_MSG_SSCP_REPLY, CurrSscpReplyMessageVersion,
    ...
    switch (objType) {
      case RTS_QUERY_ID: {
        ...
        *this >> *queryId;
        ...
        int error = statsGlobals->getStatsSemaphore(sscpGlobals->getSemId(), sscpGlobals->myPin());
        ...
        while (stmtStats != NULL) {
          ...
          if (stats != NULL && ((statsType = stats->getCollectStatsType()) == ComTdb::ACCUMULATED_STATS ||
                                statsType == ComTdb::PERTABLE_STATS || statsType == ComTdb::OPERATOR_STATS)) {
            if (mergedStats == NULL) {
              switch (queryId->getStatsMergeType()) {
                case SQLCLI_ACCUMULATED_STATS:
                  mergedStats = new (getHeap())
                      ExStatisticsArea(getHeap(), 0, ComTdb::ACCUMULATED_STATS, stats->getOrigCollectStatsType());
                  break;
                case SQLCLI_PERTABLE_STATS:
                  mergedStats = new (getHeap())
                      ExStatisticsArea(getHeap(), 0, ComTdb::PERTABLE_STATS, stats->getOrigCollectStatsType());
                  break;
                case SQLCLI_PROGRESS_STATS:
                  mergedStats = new (getHeap())
                      ExStatisticsArea(getHeap(), 0, ComTdb::PROGRESS_STATS, stats->getOrigCollectStatsType());
                  break;
                default:
                  if (reqType == SQLCLI_STATS_REQ_QID_DETAIL)
                    mergedStats = new (getHeap())
                        ExStatisticsArea(getHeap(), 0, ComTdb::QID_DETAIL_STATS, ComTdb::QID_DETAIL_STATS);
                  else
                    mergedStats = new (getHeap())
...
#endif
        if (mergedStats != NULL) reply->incNumCpus();
        *this << *reply;
  ...
  *this << *reply;
  ...
  switch (reqType) {
    ...
    case SQLCLI_STATS_REQ_SE_OFFENDER:
      mergedStats =
          new (getHeap()) ExStatisticsArea(getHeap(), 0, ComTdb::SE_OFFENDER_STATS, ComTdb::SE_OFFENDER_STATS);
      mergedStats->setSubReqType(subReqType);
      break;
    case SQLCLI_STATS_REQ_ET_OFFENDER:
      mergedStats =
          new (getHeap()) ExStatisticsArea(getHeap(), 0, ComTdb::ET_OFFENDER_STATS, ComTdb::ET_OFFENDER_STATS);
      break;
    case SQLCLI_STATS_REQ_RMS_INFO:
      mergedStats = new (getHeap()) ExStatisticsArea(getHeap(), 0, ComTdb::RMS_INFO_STATS, ComTdb::RMS_INFO_STATS);
      mergedStats->setSubReqType(subReqType);
      break;
  ...
  if (reqType != SQLCLI_STATS_REQ_RMS_INFO && reqType != SQLCLI_STATS_REQ_MEM_OFFENDER) {
    int error = statsGlobals->getStatsSemaphore(sscpGlobals->getSemId(), sscpGlobals->myPin());
    SyncHashQueue *stmtStatsList = statsGlobals->getStmtStatsList();
    ...
    if (reqType == SQLCLI_STATS_REQ_ET_OFFENDER) {
      ...
      while ((stmtStats = (StmtStats *)stmtStatsList->getNext()) != NULL) {
        ...
        if (masterStats != NULL) mergedStats->appendCpuStats(masterStats, FALSE, subReqType, filter, currTimestamp);
      }
    } else if (reqType == SQLCLI_STATS_REQ_SE_OFFENDER) {
      clock_gettime(CLOCK_MONOTONIC, &currTimespec);
      while ((stmtStats = (StmtStats *)stmtStatsList->getNext()) != NULL) {
        stats = stmtStats->getStatsArea();
        if (stats != NULL) mergedStats->appendCpuStats(stats, FALSE, filter, currTimespec);
      }
    } else {
      currTimestamp = -1;
      while ((stmtStats = (StmtStats *)stmtStatsList->getNext()) != NULL) {
        stats = stmtStats->getStatsArea();
        if (stats != NULL) mergedStats->appendCpuStats(stats, FALSE);
      }
    }
    statsGlobals->releaseStatsSemaphore(sscpGlobals->getSemId(), sscpGlobals->myPin());
  }
  ...
  if (mergedStats->numEntries() > 0) *this << *mergedStats;
  send(FALSE, -1, &bytesReplied_);
  NADELETE(mergedStats, ExStatisticsArea, getHeap());
  reply->decrRefCount();
  request->decrRefCount();
...
int reportStops(int alreadyStoppedCnt, int stoppedCnt) { return alreadyStoppedCnt + stoppedCnt; }
...
void SscpNewIncomingConnectionStream::processKillServersReq() {
  ...
  CancelQueryKillServersRequest *request = new (getHeap()) CancelQueryKillServersRequest(INVALID_RTS_HANDLE, getHeap());
  ...
  *this >> *request;
  ...
  ex_assert(objType == RTS_QUERY_ID, "CancelQueryKillServersRequest came with unknown msg obj.");
  ...
  *this >> *queryId;
  ...
  while (NULL != (kqStmtStats = (StmtStats *)stmtStatsList->getNext())) {
    ...
    if (kqStmtStats->getStatsArea()) rootStats = kqStmtStats->getStatsArea()->getRootStats();
    ...
    if (rootOperStats && (request->getExecutionCount() != rootOperStats->getExecutionCount())) continue;

    if (measStats && (request->getExecutionCount() != measStats->getExecutionCount())) continue;
    ...
    if (request->getMasterPhandle() == gph) continue;
    ...
    error = statsGlobals->getStatsSemaphore(sscpGlobals->getSemId(), sscpGlobals->myPin());
  ...
  if (sscpGlobals->shouldLogCancelKillServers() || request->getCancelLogging()) {
    ...
    str_sprintf(msg,
                ...
                queryId->getQueryId(), stoppedCnt, (request->getMakeSaveabend() ? stoppedCnt : 0));

    SQLMXLoggingArea::logExecRtInfo(__FILE__, __LINE__, msg, 0);
  }

  RtsHandle rtsHandle = (RtsHandle)this;
  CancelQueryKillServersReply *reply = new (getHeap()) CancelQueryKillServersReply(rtsHandle, getHeap());
  *this << *reply;
...
void SscpNewIncomingConnectionStream::suspendActivateSchedulers() {
  ...
  SuspendActivateServersRequest *request = new (getHeap()) SuspendActivateServersRequest(INVALID_RTS_HANDLE, getHeap());
  ...
  *this >> *request;
  ...
  ex_assert(objType == RTS_QUERY_ID, "SuspendActivateServersRequest came with unknown msg obj.");
  ...
  *this >> *queryId;
  ...
  while (NULL != (kqStmtStats = (StmtStats *)stmtStatsList->getNext())) {
    ...
    ex_assert(rootOperStats || measStats, "suspending/activating unregistered query.")

        if (rootOperStats) rootOperStats->setFragSuspended(request->isRequestToSuspend());
    else if (measStats) measStats->setFragSuspended(request->isRequestToSuspend());
    espFragCnt++;
  }
  ...
  if (request->getSuspendLogging()) {
    char msg[80 + ComSqlId::MAX_QUERY_ID_LEN];

    str_sprintf(msg, "MXSSCP has %s %d fragment(s) for query %s.",
                (request->isRequestToSuspend() ? "suspended" : "reactivated"), espFragCnt, qid);
    SQLMXLoggingArea::logExecRtInfo(__FILE__, __LINE__, msg, 0);
  }
...
void SscpNewIncomingConnectionStream::processSecInvReq() {
  ...
  SecInvalidKeyRequest *request = new (getHeap()) SecInvalidKeyRequest(getHeap());
  ...
  *this >> *request;
  ...
  if (numSiks) {
    if (!revokeTimerInitialized) {
      ...
      if (r && *r != '0') revokeTimer = true;
    }
    SscpGlobals *sscpGlobals = getSscpGlobals();
    StatsGlobals *statsGlobals = sscpGlobals->getStatsGlobals();
    int error = statsGlobals->getStatsSemaphore(sscpGlobals->getSemId(), sscpGlobals->myPin());
    ExTimeStats timer;
    if (revokeTimer) timer.start();
...
void SscpNewIncomingConnectionStream::processSnapshotLockReq() {
  ...
  SnapshotLockRequest *request = new (getHeap()) SnapshotLockRequest(getHeap());
  ...
  *this >> *request;
  ...
  SscpGlobals *sscpGlobals = getSscpGlobals();
  StatsGlobals *statsGlobals = sscpGlobals->getStatsGlobals();
  short error = statsGlobals->getStatsSemaphore(sscpGlobals->getSemId(), sscpGlobals->myPin());
...
void SscpNewIncomingConnectionStream::processSnapshotUnLockReq() {
  ...
  SnapshotUnLockRequest *request = new (getHeap()) SnapshotUnLockRequest(getHeap());
...
void SscpNewIncomingConnectionStream::processObjectEpochChangeReq() {
  ...
  ObjectEpochChangeRequest *request = new (getHeap()) ObjectEpochChangeRequest(getHeap());
  *this >> *request;
  ...
  ObjectEpochCacheKey key(request->objectName(), request->key(),
                          COM_BASE_TABLE_OBJECT);  // TODO: get object type from request
  ObjectEpochCacheEntry *oecEntry = NULL;
  short retcode = objectEpochCache->getEpochEntry(oecEntry /* out */, key);
  if ((retcode == ObjectEpochCache::OEC_ERROR_OK) || (retcode == ObjectEpochCache::OEC_ERROR_ENTRY_NOT_EXISTS)) {
    switch (request->operation()) {
      case ObjectEpochChangeRequest::CREATE_OBJECT_EPOCH_ENTRY: {
        if (oecEntry)  // if entry already exists
        {
          ...
          if ((request->expectedEpoch() != replyEpoch) || (request->expectedFlags() != replyFlags))
            result = ObjectEpochChangeReply::UNEXPECTED_VALUES_FOUND;
        } else {
          // TODO: add redef timestamp and object name when method supports it
          retcode =
              objectEpochCache->createEpochEntry(oecEntry /* out */, key, request->newEpoch(), request->newFlags());
        }
        break;
      }
      ...
      case ObjectEpochChangeRequest::COMPLETE_DDL_OPERATION: {
        if (oecEntry) {
          ...
          if ((request->expectedEpoch() == replyEpoch) && (replyFlags | ObjectEpochChangeRequest::DDL_IN_PROGRESS))
            oecEntry->update(request->newEpoch(), request->newFlags());
          else
            result = ObjectEpochChangeReply::UNEXPECTED_VALUES_FOUND;
        } else {
  ...
  ObjectEpochChangeReply *reply = new (getHeap()) ObjectEpochChangeReply(result, replyEpoch, replyFlags, getHeap());
...
void SscpNewIncomingConnectionStream::processObjectEpochStatsReq() {
  ...
  *this >> *request;
  ex_assert(!moreObjects(), "unknown object follows ObjectEpochStatsRequest.");
...
void SscpNewIncomingConnectionStream::processObjectLockReq() {
  ...
  *this >> *request;
  ex_assert(!moreObjects(), "unknown object follows ObjectLockRequest.");
  ...
  LOGDEBUG(CAT_SQL_LOCK, "SSCP Processing object %s request", ObjectLockRequest::opTypeLit(opType));
  ...
  if (opType == ObjectLockRequest::LOCK) {
    ...
    if (result == ObjectLockController::Result::LOCK_OK) {
      LOGDEBUG(CAT_SQL_LOCK, "DDL lock for %s %s succeeded", comObjectTypeName(objectType), objectName);
    } else if (result == ObjectLockController::Result::LOCK_CONFLICT_DML) {
      LOGWARN(CAT_SQL_LOCK, "DDL lock for %s %s conflict with DML operations on NID %d PID %d",
      ...
      LOGWARN(CAT_SQL_LOCK, "DDL lock for %s %s conflict with DDL operations on NID %d PID %d",
              comObjectTypeName(objectType), objectName, conflictHolder.getNid(), conflictHolder.getPid());
    } else if (result == ObjectLockController::Result::LOCK_OUT_OF_ENTRY) {
      LOGWARN(CAT_SQL_LOCK, "DDL lock for %s %s failed due to out of lock entry", comObjectTypeName(objectType),
              objectName);
    } else {
      LOGERROR(CAT_SQL_LOCK, "DDL lock for %s %s failed with invalid lock result: %d", comObjectTypeName(objectType),
               objectName, result);
    ...
    LOGERROR(CAT_SQL_LOCK, "Unsupported object lock operation: %d", opType);
  ...
  *this << *reply;
  ...
  LOGDEBUG(CAT_SQL_LOCK, "Sending object %s reply state: %s", ObjectLockRequest::opTypeLit(opType),
...
void SscpNewIncomingConnectionStream::processObjectLockStatsReq() {
  ...
  *this >> *request;
  ex_assert(!moreObjects(), "unknown object follows ObjectLockStatsRequest.");

  LOGDEBUG(CAT_SQL_LOCK, "Processing object lock stats request of %s %s", comObjectTypeName(request->getObjectType()),
