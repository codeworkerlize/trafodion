  ...
  *this >> *request;
  ...
  if (request->getCpu() == -1) {
    ...
    *sscpMsgStream << *request;
    QRDEBUG("Forwarding ObjectEpochStatsRequest to all nodes");
    sscpMsgStream->send(FALSE);
  } else {
...
void SsmpNewIncomingConnectionStream::actOnObjectLockStatsReq(IpcConnection *connection)
{
  ...
  *this >> *request;
  ...
  if (request->getCpu() == -1) {
    ...
    sscpMsgStream->clearAllObjects();
    *sscpMsgStream << *request;
    LOGDEBUG(CAT_SQL_LOCK, "Forwarding ObjectLockStatsRequest to all nodes");
    sscpMsgStream->send(FALSE);
  } else {
...
void SscpClientMsgStream::actOnObjectLockReply()
{
  ...
  *this >> *reply;
  ...
  ObjectLockReply::LockState currState = getObjectLockState();
