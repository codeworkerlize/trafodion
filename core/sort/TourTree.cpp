...
Tree::Tree(int numruns, int runsize, int recsize, NABoolean doNotAllocRec, int keysize, SortScratchSpace *scratch,
  ...
  for (nodenum = 0; nodenum < numRuns_; nodenum++) {
    rootNode_[nodenum].initialize(nodenum, runnum++,
                                  &rootNode_[nodenum / 2],               // fi_
                                  &rootNode_[(nodenum + numRuns_) / 2],  // fe_
                                  rootRecord_, heap_, sorterror, scratch_, merge, waited);
...
void Tree::determineNewWinner() {
  ...
  do {
    ...
    if ((temp1->getRun() < winnerRun_) || ((temp1->getRun() == winnerRun_) &&
...
int Tree::sortReceive(void *rec, int &len) {
  ...
  if (numRuns_ == 1) {
    ...
    if (retcode == IO_NOT_COMPLETE) return SORT_IO_IN_PROGRESS;
    //-------------------------------------------------------------
  ...
  while (TRUE_L) {
    ...
    if (retcode == SORT_SUCCESS) {
      if ((winnerRun_ == 0) || (compare(keyOfLastWinner_, winner_->getKey()) >= KEY1_IS_GREATER)) {
        winnerRun_++;
        if (winnerRun_ > maxRuns_) maxRuns_ = winnerRun_;
      }
    }
...
int Tree::generateInterRuns() {
  ...
  while (TRUE_L) {
    if (winnerRun_ != currentRun_) {
      ...
      if (winnerRun_ > maxRuns_) {  // We are all done so return SORT_SUCCESS
        return SORT_SUCCESS;
      } else {
        currentRun_ = winnerRun_;
      }
  ...
  while (TRUE_L) {
    if ((winnerRun_ != currentRun_)) {
      if ((currentRun_ != 0) && sendNotDone_) {
        if (scratch_->flushRun(TRUE_L)) return SORT_FAILURE;
      }
      if (winnerRun_ > maxRuns_) return SORT_SUCCESS;
      currentRun_ = winnerRun_;
    }

    if (winnerRun_) {
      if (outputWinnerToScr()) return SORT_FAILURE;
    }
    winnerRun_ = maxRuns_ + 1;
    determineNewWinner();
  }
...
int Tree::sortSend(void *rec, int len, void *tupp) {
  ...
  winner_->setRecordTupp(rec, recSize_, keySize_, tupp, sortUtil_->config()->numberOfBytesForRecordSize());
  if ((winnerRun_ == 0) || (compare(keyOfLastWinner_, winner_->getKey()) >= KEY1_IS_GREATER)) {
    winnerRun_++;
    if (winnerRun_ > maxRuns_) maxRuns_ = winnerRun_;
  }
