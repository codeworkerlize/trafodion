...
NABoolean SortUtil::scratchInitialize(void) {
  if (!scratch_) {
    scratch_ = new (config_->heapAddr_)
        SortScratchSpace(config_->heapAddr_, &sortError_, explainNodeId_, config_->scratchIOBlockSize_,
                         config_->getScratchIOVectorSize(), config_->logInfoEvent_, config_->scratchMgmtOption_);
...
NABoolean SortUtil::sortInitialize(SortUtilConfig &config, int topNSize) {
  ...
  if (config.topNSort_ && topNSize) {
    sortAlgo_ = new (config.heapAddr_)
        SortTopN(topNSize, config.maxMem_, config.recSize_, config.sortType_.doNotAllocRec_, config.keySize_, scratch_,
                 TRUE, config.heapAddr_, &sortError_, explainNodeId_, bmoStats_, this);

  } else {
    sortAlgo_ = new (config.heapAddr_)
        Qsort(config.runSize_, config.maxMem_, config.recSize_, config.sortType_.doNotAllocRec_, config.keySize_,
              scratch_, TRUE, config.heapAddr_, &sortError_, explainNodeId_, bmoStats_, this);
  }
  if (sortAlgo_ == NULL) {
    sortError_.setErrorInfo(EScrNoMemory  // sort error
                            ,
...
int SortUtil::sortSendEnd(NABoolean &internalSort) {
  ...
  if (config_->logInfoEvent()) {
    char msg[500];
    str_sprintf(msg, "Sort is performing %s sort: NumRecs:%ld", internalSort ? "internal" : "external",
                stats_.numRecs_);
...
int SortUtil::sortReceivePrepare(void) {
  ...
  // Total memory used
  stats_.memSizeB_ = sortAlgo_->getRunSize() * sizeof(Record) + sortAlgo_->getRunSize() * sizeof(RecKeyBuffer);
  ...
  if (memoryQuotaSystemEnabled()) {
    ...
    UInt32 maxMergeMemory = estimateMemoryToAvoidIntMerge(stats_.numRuns_, config_->sortMergeBlocksPerBuffer_);
  ...
  if (result != SCRATCH_SUCCESS) {
    if (memoryQuotaSystemEnabled() || (mergeOrderUsed <= 2)) {
      ...
      while (result != SCRATCH_SUCCESS) {
        ...
        result = scratch_->setupSortMergeBufferPool(mergeOrderUsed * SORT_MERGENODE_NUM_BUFFERS *
                                                    config_->sortMergeBlocksPerBuffer_);
      }
      if (result != SCRATCH_SUCCESS) {
        sortError_.setErrorInfo(EScrNoMemory  // sort error
  ...
  if (stats_.numRuns_ > (int)config_->mergeOrder_) {
    ...
    while (TRUE_L) {
      ...
      stats_.firstMergeOrder_ = 1 + (stats_.numRuns_ - (stats_.numInterPasses_ * stats_.mergeOrder_));
      if (stats_.firstMergeOrder_ > 1) {
        sortAlgo_ = new (config_->heapAddr_)
            Tree(stats_.firstMergeOrder_, initialRunSize, config_->recSize_, FALSE_L, config_->keySize_, scratch_,
                 config_->heapAddr_, &sortError_, explainNodeId_, bmoStats_, this, runnum, TRUE_L, TRUE);
        if (sortError_.getSortError() != 0) return SORT_FAILURE;
    ...
    if (config_->logInfoEvent()) {
      char msg[500];
      str_sprintf(
          msg, "Sort is not performing intermediate merges, NumRuns:%d, MergeOrder:%d, NumRecs:%ld, ScratchBlocks:%d",
          stats_.numRuns_, config_->mergeOrder_, stats_.numRecs_, stats_.scrNumBlocks_);
  ...
  stats_.memSizeB_ += stats_.finalMergeOrder_ * stats_.scrBlockSize_ * 2 + stats_.finalMergeOrder_ * sizeof(TreeNode) +
                      stats_.finalMergeOrder_ * sizeof(Record);

  // Final Merge
  sortAlgo_ = new (config_->heapAddr_)
      Tree(stats_.finalMergeOrder_, initialRunSize, config_->recSize_, FALSE_L, config_->keySize_, scratch_,
           config_->heapAddr_, &sortError_, explainNodeId_, bmoStats_, this, runnum, TRUE_L, FALSE);
  if (sortAlgo_ == NULL) {
    sortError_.setErrorInfo(EScrNoMemory  // sort error
                            ,
                            0  // syserr: the actual FS error
...
int SortUtil::sortReceive(void *record, int &len) {
  ...
  if ((len == 0) && (!config_->partialSort_)) {
    ...
    if (config_->logInfoEvent()) {
      char msg[500];
      str_sprintf(msg, "Sort elapsed time : %ld; Num runs : %d; runsize :%d", stats_.elapsedTime_, stats_.numInitRuns_,
                  sortAlgo_->getRunSize());
      SQLMXLoggingArea::logExecRtInfo(NULL, 0, msg, explainNodeId_);
    }
  }
  return status;
...
//
//----------------------------------------------------------------------
void SortUtil::getSortError(SortError *srterr) { memcpy((char *)srterr, (char *)&sortError_, sizeof(SortError)); }
...
//
//----------------------------------------------------------------------
short SortUtil::getSortError() const { return sortError_.getSortError(); }
...
void SortUtil::setErrorInfo(short sorterr, short syserr, short syserrdetail, char *errorMsg) {
  sortError_.setErrorInfo(sorterr, syserr, syserrdetail, errorMsg);
}
...
NABoolean SortUtil::consumeMemoryQuota(UInt32 bufferSizeBytes) {
  ...
  if (((config_->memoryQuotaUsedBytes_ + bufferSizeBytes) / ONE_MB) <= (UInt32)config_->memoryQuotaMB_) {
    ...
    // leftover quota.
    UInt32 memNeededMB =
        (config_->memoryQuotaUsedBytes_ + bufferSizeBytes - (config_->memoryQuotaMB_ * ONE_MB)) / ONE_MB;

    ExExeStmtGlobals *exe_glob = config_->callingTcb_->getGlobals()->castToExExeStmtGlobals();
...
void SortUtil::returnConsumedMemoryQuota(UInt32 bufferSizeBytes) {
  // BMO only enabled if greater than zero.
  if (config_ == NULL || config_->initialMemoryQuotaMB_ <= 0) return;
  config_->memoryQuotaUsedBytes_ -= bufferSizeBytes;
}
...
void SortUtil::returnExcessMemoryQuota(UInt32 overheadPerRecord) {
  ...
  // in memory.
  maxEstimateMemoryBytes = maxEstimateMemoryBytes + ((maxEstimateMemoryBytes / tuppSize) * overheadPerRecord);

  excessMemoryQuotaMB = config_->memoryQuotaMB_ - (Int16)(maxEstimateMemoryBytes / ONE_MB);
...
UInt32 SortUtil::getMaxAvailableQuotaMB(void) {
  // BMO only enabled if greater than zero.
  if (config_ == NULL || config_->initialMemoryQuotaMB_ <= 0) return 0;

  ExExeStmtGlobals *exe_glob = config_->callingTcb_->getGlobals()->castToExExeStmtGlobals();

  UInt32 maxAvailableQuotaMB =
      config_->memoryQuotaMB_ - (config_->memoryQuotaUsedBytes_ / ONE_MB) + exe_glob->unusedMemoryQuota();
...
NABoolean SortUtil::withinMemoryLimitsAndPressure(long reqMembytes) {
  ...
  */
  ...
  Explanation:
  -------------
  bmoEstErrorPenalty can also be viewed as uncertainty penalty factor. Higher the uncertainty of expected memory
  consumption, better to overflow early.
  ...
  Executor usage:
  ---------------
  On Seaquest, operators overflow once memory quota is reached or physical memory pressure is detected. Memory pressure
  is currently disabled but eventually memory pressure detection mechanisms would be enabled.

  Memory quota limits and memory pressure checks do help enforce limits, however BMO operators could utilize compiler
  hints to trigger overflow and behave as a good citizen even before quota limits and memory pressure enforcements kick
  in.

  BMO operators often consume small chunks of memory or buffer, in repeated cycles until memory quota limit is reached.
  Compiler hints would be exercised each time a buffer allocation is required. However the size of buffer allocation is
  not relevant in the calculation below.

  C = Total consumed memory by BMO operator. Initially this is zero.

  E = Total estimated size of memory that will be consumed by BMO. This is initially the bmoExpectedSize hint given by
  compiler, but adjusted if C started exceeding the estimate. E = max(bmoExpectedSize, C * (1+bmoEstErrorPenalty) ) This
  allow us to modify the estimate at runtime after receiving more data and penalize operators that have the chance of
  being much bigger than initially thought. Note, it's possible for the runtime logic to increase the value of
  bmoEstErrorPenalty on its own after certain threshold (e.g C > 10*bmoExpectedSize) but bmoEstErrorPenalty should not
  exceed 1 i.e. 100%.

  Z = CitizenshipFactor which is a parameter representing how much of the available physical memory an operator can
  assume for itself as compared to leaving it to other potential players. It can have the value of 1>= Z > 0. While we
  can suggest an initial value e.g. 0.5 we think this is better to set after tuning based on some performance tests. In
  the future this may also be adjusted as input from WMS based on existing concurrency on the system.

  M = Available physical memory at that instant.

  U = Contingency physical memory (e.g. 10% of Phys memory in the node. Point it to keep always phys memory available
  for executables, buffers, non-BMO ops, etc)
  ...
  */
  ...
  if (!config_->getDisableCmpHintsOverflow()) {
    ...
    if (state_ < SORT_SEND_END) {
      ...
      // available quota.
      if (E > config_->memoryQuotaMB_) return FALSE;
#endif
      ...
      if (m > (Z * (M - U))) {
        if (config_->logInfoEvent()) {
          ...
          str_sprintf(msg,
                      ...
                      "memContingency U=%f, citizenshipFactor Z=%f",
                      m, M, E, C, estimateErrorPenalty, U, Z);
          SQLMXLoggingArea::logExecRtInfo(NULL, 0, msg, explainNodeId_);
        }
        return FALSE;
      }
  ...
  if ((config_->memoryQuotaUsedBytes_ / ONE_MB) >= config_->bmoMaxMemThresholdMB_) return FALSE;
...
UInt32 SortUtil::estimateMergeOrder(UInt32 maxMergeMemory, int sortMergeBlocksPerBuffer) {
  UInt32 memoryPerTreeNode =
      ((SORT_MERGENODE_NUM_BUFFERS * sortMergeBlocksPerBuffer *
       ...
       sizeof(TreeNode) +
       sizeof(Record) + sizeof(SortMergeNode));
