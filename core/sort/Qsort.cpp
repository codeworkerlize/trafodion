...
//------------------------------------------------------------------------
Qsort::~Qsort(void) {
  for (; recNum_ < loopIndex_; recNum_++) recKeys_[recNum_].rec_->releaseTupp();
...
int Qsort::sortSend(void *rec, int len, void *tupp) {
  ...
  rootRecord_[loopIndex_].initialize(len /*recSize_*/, doNotallocRec_, heap_, sortError_);
  rootRecord_[loopIndex_].setRecordTupp(rec, recSize_, tupp);  //??? recSize_ passed but not used???

  recKeys_[loopIndex_].key_ =
      rootRecord_[loopIndex_].extractKey(keySize_, sortUtil_->config()->numberOfBytesForRecordSize());
  recKeys_[loopIndex_].rec_ = &rootRecord_[loopIndex_];
  if (++loopIndex_ < allocRunSize_) return SORT_SUCCESS;
  ...
  if (sortUtil_->config()->getScratchOverflowMode() != SCRATCH_MMAP) {
    if (isIterativeSort_) {
      UInt32 estimateSize = 2 * allocRunSize_ * (sizeof(RecKeyBuffer) + sizeof(Record));
      ...
      if (sortUtil_->memoryQuotaSystemEnabled()) {
        ...
        if (((2 * allocRunSize_ * sizeof(RecKeyBuffer)) > MAX_ALLOC_SIZE) ||
            ((2 * allocRunSize_ * sizeof(Record)) > MAX_ALLOC_SIZE) || (!sortUtil_->consumeMemoryQuota(estimateSize))) {
          overFlow = TRUE;
        }
      }
      ...
      if (overFlow == FALSE) {
        ...
        Record *tempRootRecord = (Record *)heap_->allocateMemory(sizeof(Record) * allocRunSize_, FALSE);

        // allocateMemory failureIsFatal FALSE means we handle memory failure error.
        RecKeyBuffer *tempRecKeys = (RecKeyBuffer *)heap_->allocateMemory(sizeof(RecKeyBuffer) * allocRunSize_, FALSE);
        ...
        if ((tempRootRecord == NULL) || (tempRecKeys == NULL)) {
          ...
          if (sortUtil_->memoryQuotaSystemEnabled()) sortUtil_->returnConsumedMemoryQuota(estimateSize);

          if (sortUtil_->config()->logInfoEvent() && (!sortUtil_->config()->logInfoEventDone())) {
            char msg[500];
            str_sprintf(msg, "QSort::SortSend, Sort has overflowed due to unavailable memory request of size %ld MB",
...
int Qsort::generateARun() {
  ...
  if (internalSort_) {
    ...
    for (loopIndex_ = 0; loopIndex_ < runSize_; loopIndex_++) {
      ...
      if (status == SCRATCH_FAILURE) return SORT_FAILURE;
    }
...
NABoolean Qsort::quickSort(RecKeyBuffer keysToSort[], long left, long right) {
  ...
  if (left + 1 < right) {  // Atleast three elements
    ...
    if (left < right) {
      if (compare(keysToSort[left].key_, keysToSort[right].key_) >= KEY1_IS_GREATER)
...
void Qsort::siftDown(RecKeyBuffer keysToSort[], long root, long bottom) {
  ...
  while ((root * 2 <= bottom) && (!done)) {
    if (root * 2 == bottom)
      maxChild = root * 2;
    else if (compare(keysToSort[root * 2].key_, keysToSort[root * 2 + 1].key_) >= KEY1_IS_GREATER)
      maxChild = root * 2;
    else
      maxChild = root * 2 + 1;
...
char *Qsort::median(RecKeyBuffer keysToSort[], long left, long right) {
  ...
  if (compare(keysToSort[center].key_, keysToSort[right].key_) >= KEY1_IS_GREATER)
...
void Qsort::cleanUpMemoryQuota(void) {
  sortUtil_->returnConsumedMemoryQuota((allocRunSize_ - initialRunSize_) * (sizeof(RecKeyBuffer) + sizeof(Record)));
}

UInt32 Qsort::getOverheadPerRecord(void) { return (sizeof(RecKeyBuffer) + sizeof(Record)); }
